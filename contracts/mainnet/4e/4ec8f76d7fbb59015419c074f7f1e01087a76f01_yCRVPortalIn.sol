...
/// @author Portals.fi
/// @notice This contract mints yCRV with any token

/// SPDX-License-Identifier: GPL-3.0
pragma solidity 0.8.11;

import "../base/PortalBaseV2.sol";
import "./interface/IyCRV.sol";
...
contract yCRVPortalIn is PortalBaseV2 {
    using SafeTransferLib for address;
    using SafeTransferLib for ERC20;

    address immutable yCRV;
    ...
    /// @notice Mints Yearn CRV (yCRV) with network tokens/ERC20 tokens
    /// @param sellToken The ERC20 token address to spend (address(0) if network token)
    /// @param sellAmount The quantity of sellToken to Portal in
    ...
    function portalIn(
        ...
        amount = _execute(sellToken, amount, CRV, target, data);

        uint256 balance = _getBalance(msg.sender, yCRV);

        _approve(CRV, yCRV, amount);
        IyCRV(yCRV).mint(amount, msg.sender);

        buyAmount = _getBalance(msg.sender, yCRV) - balance;
