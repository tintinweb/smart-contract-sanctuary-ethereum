...
contract Quadron is IQuadron, ERC20, ReserveFormula {
    ...
    uint256 private phase1EndDate; // End date of phase 1
    uint256 private phase1EndSupply; // Supply at end of phase 1
    ...
    uint32 internal constant PHASE_1_DURATION = 17366400; // time in seconds for the phase 1 period
    uint32 internal constant PHASE_2_DURATION = 7889400; // time in seconds for the phase 2 period
    uint256 internal constant PHASE_1_END_SUPPLY = 145e6 * 1e18; // supply of quadrons at the end of phase 1
    uint256 internal constant PHASE_1_MIN_SUPPLY = 50e6 * 1e18; // minimum supply of quadrons at the end of phase 1
    uint256 internal constant PHASE_3_START_SUPPLY = 3333333333333333333333333; // supply of quadrons to raise in phase 3
    ...
    constructor(
        ...
        phase1EndDate = block.timestamp + PHASE_1_DURATION;
        phase1EndSupply = PHASE_1_END_SUPPLY;
    ...
    function updatePhase() public override {
        if((block.timestamp >= (phase1EndDate + PHASE_2_DURATION) ||
            totalSupply() >= (phase1EndSupply + PHASE_3_START_SUPPLY)) && contractPhase == 2) {
            
            // In phase 3
            contractPhase = 3;

        } else if(((block.timestamp >= (tokenEpoch + PHASE_1_DURATION) && (totalSupply() >= PHASE_1_MIN_SUPPLY)) ||
            totalSupply() >= PHASE_1_END_SUPPLY) && contractPhase == 1) {
            
            // In phase 2
            phase1EndDate = block.timestamp;
            phase1EndSupply = totalSupply();
            contractPhase = 2;
        }
    }
