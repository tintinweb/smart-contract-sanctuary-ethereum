...
/**
Luna-Chain Capital: $LCC
- You buy on Ethereum, we farm on multiple chains and return the profits to $LCC holders.

Tokenomics:
5% of each buy/sell goes to existing holders.
5% of each buy/sell goes into multi-chain farming to add to the treasury and buy back LCC tokens.
...
contract LunaChainCapital is Context, IERC20, Ownable {
  ...
  string private _name = 'LunaChainCapital';
  ...
  address payable public _LCCWalletAddress;
  ...
  uint256 private _maxTxAmount = 300000000000000000e9;
  ...
  constructor(
    address payable LCCWalletAddress,
    address payable marketingWalletAddress
  ) {
    _LCCWalletAddress = LCCWalletAddress;
  ...
  function sendETHToTeam(uint256 amount) private {
    _LCCWalletAddress.call{ value: amount.div(2) }('');
  ...
  function _setLCCWallet(address payable LCCWalletAddress) external onlyOwner {
    _LCCWalletAddress = LCCWalletAddress;
  }
