...
contract ETHLeverExchange is OwnableUpgradeable, IExchange {
    ...
    function swapExactETH(uint256 input, uint256 output) external override onlyLeverSS {
        require(IERC20(stETH).balanceOf(_msgSender()) >= input, "INSUFFICIENT_STETH");
        require(IERC20(stETH).allowance(_msgSender(), address(this)) >= input, "INSUFFICIENT_ALLOWANCE");
        ...
        TransferHelper.safeTransferFrom(stETH, _msgSender(), address(this), toSwap);
