...
contract TheFanClub is ERC721A, Ownable {
  ...
  uint256 public freeMintSupply = 1800;
  ...
  constructor() ERC721A("The FanClub", "FanClub") payable {
      _mint(msg.sender, 1);
  }
  ...
  function mint(uint256 quantity_) external payable mintCompliance(quantity_) {
      unchecked {
          ...
          if (totalSupply() < freeMintSupply) {
            if (freeWalletBalance[msg.sender] + quantity_ <= 3) {
                price = 0;
                freeWalletBalance[msg.sender] += quantity_;
            } else if (freeWalletBalance[msg.sender] + quantity_ > 3 && freeWalletBalance[msg.sender] < 3) {
                uint256 freeBalance = 3 - freeWalletBalance[msg.sender];
