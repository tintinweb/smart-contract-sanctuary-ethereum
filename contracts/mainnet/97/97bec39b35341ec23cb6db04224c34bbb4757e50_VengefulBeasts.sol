...
contract VengefulBeasts is ERC721A, Ownable, ReentrancyGuard {
    using Strings for uint256;

    //NOTE Token values incremented for gas efficiency
    uint256 public maxSalePlusOne = 10001;

    uint256 private mintCost1650 = 0.06 ether;
    uint256 private mintCost8500 = 0.08 ether;
    ...
    uint256 public totalToPay;
    uint256 public totalPayed;
    uint public amountOfTokensGotPayout;

    constructor() ERC721A("Vengeful Beasts", "VB") {
        placeholderURI = "ipfs://.../";

        //address that will recieve mint funds
        recipient1 = 0x545D6ACe692AdE79BE0b53932612e28De6F669Ff;
        recipient2 = 0x34Cd8063c8ca772f497Ff0a5A8F4737C0F529821;

        //mint the first 150 NFTs for owner
        _safeMint(msg.sender, 150);
    }

    receive() payable external {
        _withdraw(payable(recipient1), (msg.value * 40) / 100);
        totalToPay += (msg.value * 60) / 100;
    }
     ...
     */
    function mint(uint256 quantity, bytes32[] calldata proof, bytes32 leaf)
        payable
        external
        noBots
        isContractState(ContractState.ON)
        withinMintLimit(quantity)
        correctValue(quantity * mintCost())
    {
     ...
     */
    function setRecipient1(address newRecipient) external onlyOwner {
        require(newRecipient != address(0), "Cannot be the 0 address!");
        recipient1 = newRecipient;
    }
     ...
     * @param newRecipient The new address that will recieve funds
     */
    function setRecipient2(address newRecipient) external onlyOwner {
        require(newRecipient != address(0), "Cannot be the 0 address!");
        recipient2 = newRecipient;
    }
    ...
    function withdraw() public onlyOwner {
        ...
        _withdraw(payable(recipient2), (balance * 5) / 100); 
        _withdraw(payable(recipient1), accountBalance()); 
    }
    ...
    /*
     * Get the current cost to mint
     */
    function mintCost() public view returns(uint) {
        if (_totalMinted() < 1650) {
            return mintCost1650;
        }
        else {
            return mintCost8500;
