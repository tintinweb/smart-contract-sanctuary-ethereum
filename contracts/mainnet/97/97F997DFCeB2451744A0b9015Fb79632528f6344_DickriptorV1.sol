...
contract DickriptorV1 is IDickriptor {
    struct Color {
        string value;
        string name;
    }
    ...
    function tokenURI(uint256 tokenId, uint256 seed) external pure override returns (string memory) {
        ...
        Trait memory topshaft = getTopShaft(seed % 1000000000000 / 10000000000, colors[1]);
        Trait memory bottomshaft = getBottomShaft(seed % 100000000 / 1000000, colors[2]);
        ...
        string memory description = 'Ascii Dicks inspired by owls.';
            ...
            abi.encodePacked(
                ...
                Base64.encode(
                    bytes(
                        abi.encodePacked(
                            '{',
                            '"name":"Dick #', tokenId.toString(), '",',
                            '"description":"', description, '",',
                            '"image": "', 'data:image/svg+xml;base64,', encodedSvg, '",',
                            '"attributes": [{"trait_type": "Head", "value": "', head.name,' (',head.color.name,')', '"},',
                            '{"trait_type": "Top Shaft", "value": "', topshaft.name,' (',topshaft.color.name,')', '"},',
                            '{"trait_type": "Bottom Shaft", "value": "', bottomshaft.name,' (',bottomshaft.color.name,')', '"},',
    ...
    function getTopShaft(uint256 seed, uint256 colorSeed) private pure returns (Trait memory) {
        Color memory color = getColor(colorSeed);
        string memory content;
        string memory name;
        if (seed == 10) {
    ...
    function getBottomShaft(uint256 seed, uint256 colorSeed) private pure returns (Trait memory) {
        Color memory color = getColor(colorSeed);
        string memory content;
        string memory name;
        if (seed == 10) {
