...
contract NameTag is ERC721Enumerable, Ownable, StringUpper, ReentrancyGuard {
    ...
    event AddedToAllowList(address indexed _address, uint256 indexed _price, uint8 _limit);
    event RemovedFromAllowList(address indexed _address, uint8 _purchasedAmount);
    ...
    function addToAllowList(
        address[] memory addresses, uint256[] memory prices, uint8[] memory limits
    ) external onlyOwner {
        uint256 length = addresses.length;
        require(length <= addToAllowListLimit, "NT: List of addresses is too large");
        require(length == prices.length && length == limits.length, "NT: All lists should be the same length");
        for(uint index = 0; index < length; index += 1) {
            _allowList.add(addresses[index ]);
            allowListParams[addresses[index]] = AllowParams(prices[index], limits[index], 0);
            emit AddedToAllowList(addresses[index], prices[index], limits[index]);
        }
    }
    ...
    function removeFromAllowList(address[] memory addresses) external onlyOwner {
        ...
        for(uint index = 0; index < addresses.length; index += 1) {
            if (_allowList.remove(addresses[index])) {
                emit RemovedFromAllowList(addresses[index], allowListParams[addresses[index]].purchasedAmount);
                delete allowListParams[addresses[index]];
            }
        }
    }
    ...
    function allowAddressByIndex(uint256 index) external view returns (address) {
        require(index < _allowList.length(), "NT: Index out of bounds");
    ...
    function _preValidatePurchase(uint256 tokensAmount) internal view returns(bool) {
        ...
        if (
            ...
            && tokensAmount + allowListParams[msg.sender].purchasedAmount <= allowListParams[msg.sender].limit
        ) {
            require(allowListParams[msg.sender].price * tokensAmount <= msg.value, "NT: Presale, insufficient funds");
            return true;
        }
    ...
    function buyTokens(string[] memory _names) external payable whenAnySaleActive nonReentrant returns (uint256[] memory) {
        ...
        if (usePresale) {
            allowListParams[msg.sender].purchasedAmount += uint8(_names.length);
        } else {
