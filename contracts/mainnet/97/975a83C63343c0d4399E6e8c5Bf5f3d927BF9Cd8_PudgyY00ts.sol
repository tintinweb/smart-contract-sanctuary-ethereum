...
contract PudgyY00ts is ERC721AQueryable, Ownable, ReentrancyGuard {
  ...
  function mint(uint256 _mintAmount) public payable mintCompliance(_mintAmount) {
    ...
    else if((totalFreeMinted + _mintAmount <= totalFree) && (_mintAmount <= maxFreePer - _mintedFreeAmount[msg.sender])){
      require(msg.value >= 0, 'Insufficient funds!');
      totalFreeMinted += _mintAmount;
      _mintedFreeAmount[msg.sender] += _mintAmount;       
    }

    else if((totalFreeMinted < totalFree) && (totalFreeMinted + maxFreePer - _mintedFreeAmount[msg.sender] >= totalFree)){
      require(msg.value >= (_mintAmount * cost) - ((totalFree-totalFreeMinted) * cost), 'Insufficient funds!');
      totalFreeMinted += totalFree-totalFreeMinted;
      _mintedFreeAmount[msg.sender] += totalFree-totalFreeMinted;                    
    }

    else{
      require(msg.value >= (_mintAmount * cost) - ((maxFreePer - _mintedFreeAmount[msg.sender]) * cost), 'Insufficient funds!');    
