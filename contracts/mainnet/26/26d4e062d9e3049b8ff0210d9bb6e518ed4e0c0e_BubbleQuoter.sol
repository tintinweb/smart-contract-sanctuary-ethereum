...
import 'IBubbleSwapCallback.sol';
...
contract BubbleQuoter is IQuoter, IBubbleSwapCallback, PeripheryImmutableState {
    ...
    function getPool(
        ...
        address tokenB,
        uint24 fee
    ) private view returns (IBubblePool) {
        return IBubblePool(PoolAddress.computeAddress(factory, PoolAddress.getPoolKey(tokenA, tokenB, fee)));
    }

    /// @inheritdoc IBubbleSwapCallback
    function bubbleSwapCallback(
        int256 amount0Delta,
        int256 amount1Delta,
        bytes memory path
