...
contract BlindboxDex is Ownable, DexStorage, SecondDexStorage {
 ...
 event OnAuction(address indexed seller, address nftContract, uint256 indexed tokenId, uint256 startPrice, uint256 endTime, uint256 baseCurrency);
    ...
    function setBlindAddress(address _addr) public {
    require(msg.sender == admin,"only owner");
    blindAddress = _addr;
  }

 function addSupportNFT(address _addr, bool isRoyaltyAllowed) public {
    require(msg.sender == admin,"only owner");
    _supportNft[_addr] = true;
    collectionsWithRoyalties[_addr] = isRoyaltyAllowed;
  }

  function blindBoxMintedNFT(address collection, uint256[] memory tokenIds, uint256[] memory royalty, address bankAddress ) public {
      require(admin == msg.sender, "not authorize");
      for(uint256 i=0; i< tokenIds.length; i++){
          _tokenAuthors[tokenIds[i]][collection]._address = bankAddress;
          _tokenAuthors[tokenIds[i]][collection].royalty = royalty[i];
          isBlindNft[tokenIds[i]][collection] = true;
      }
  }


  function mintBlindbox(address collection, address to, uint256 quantity, address from, uint256 royalty) public returns(uint256 fromIndex,uint256 toIndex) {
  ...
  function setBlindSecondSaleClose() public {
      // require(admin == msg.sender, "no authorize");
      blindNFTOpenForSale = false;
  }
   function setBlindSecondSaleOpen() public {
      // require(admin == msg.sender, "no authorize");
      blindNFTOpenForSale = true;
  }
  ...
  function sellNFT(address nft_a,uint256 tokenId, address seller, uint256 price, uint256 baseCurrency, uint256[] memory allowedCurrencies) isValid(nft_a) public{
    ...
    require(!isBlindNft[tokenId][nft_a] || blindNFTOpenForSale, "can't sell" );
...
function calculatePrice(uint256 _price, uint256 base, uint256 currencyType, uint256 tokenId, address seller, address nft_a) public view returns(uint256 price) {
  ...
  require(!isBlindNft[_tokenId][_contract] || blindNFTOpenForSale, "can't sell" );
