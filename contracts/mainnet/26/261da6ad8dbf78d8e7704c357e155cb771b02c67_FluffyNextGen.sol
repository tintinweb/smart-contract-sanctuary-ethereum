...
contract FluffyNextGen is ERC721AQueryable, Ownable, ReentrancyGuard
{
        ...
        supplyExists(_quantity + _freeMints)
    {
        ...
        uint256[] memory fluffs;

        // checking if person is an active staker
        fluffs = ffs.tokensStakedByAddress(msg.sender);
        if(fluffs.length > 0) {
            return stakerMint(fluffs.length, msg.sender, _quantity, _freeMints, msg.value);
        }

        require(_quantity > 0, "No point minting no fluffs.");

        // checking if person is an active holder
        fluffs = ffxr.tokensOfOwner(msg.sender);
        if (fluffs.length > 0) {
    ...
    function setFluffyStakerContract(address payable _contract)
        public
        onlyOwner
    {
        ffs = FluffyStaker(_contract);
    }
    ...
    {
        uint256[] memory fluffs = ffs.tokensStakedByAddress(_minter);
        if(fluffs.length > 1) {
            (freeMints, maxMints) = howManyCanStakerMint(fluffs.length);
            return (
