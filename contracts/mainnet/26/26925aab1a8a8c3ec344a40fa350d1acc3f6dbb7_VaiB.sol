...
The year is 2094. It is 100 years after the birth of Vitalik Buterin, Co-Founder of Ethereum.
The world in in chaos and the Artificial Intelligence (AI) machines have taken over (beginning 12/24/2031). 
Thankfully, before his presumed death in the war of 2074. the AI Resistance, with the help of Vitalik's 
previous work were able to make what they sought to destroy (VaiB) in order to save mankind. 

http://www.vaibeth.com
https://medium.com/@VaiB_ETH/vbai-the-year-is-2094-vol-1-5b14bed61e66
https://twitter.com/VaiB_ETH

VaiB CA Features:

	- Buy tax: 4% 
- 1% Burn (20% of EVERY transaction - Tokens deleted from existence)
- 2% Marketing / Development (To ensure project longevity)   	
- 1% Auto Liquidity (20 % of EVERY transaction will increase liquidity)
...
- 4/5 tax after 1st 60 min of launch (1st 5 min - 30/30, next 15min - 10/20, next 30min - 0/20, Final 4/5) 
...
- 2% Max Buy/Wallet at launch. Limits will be removed before renouncing.
- 1% Max Sell Tx at Launch. Will stay for the duration of the project.

- 20% of the supply will be given to VB pre-launch & 5% of the supply will be burned pre-launch.
- VaiB Comic book series on our Medium. vol. 1 released at launch. 
...
contract VaiB is ERC20, Ownable {
    ...
    constructor() ERC20("VaiB", "VaiB") {
        ...
        marketingAddress = 0x3bfE5B0d301104Fd858eB1Ae15154d2655EAc869;
    ...
    function updateLiquidityAddress(address newWallet) external onlyOwner {
      require(!_renounceWalletChanges, "Cannot update wallet after renouncement");
    ...
    function swapBack() private {
        ...
        if(tokensForMarketing > 0) swapForMarketing(tokensForMarketing);
        if(tokensForLiquidity > 0)  swapForLiquidity(tokensForLiquidity);
    }

    function swapForMarketing(uint _tokens) private {
        uint256 initalBalance = address(this).balance;
        swapTokensForEth(_tokens);
        uint256 RecievedBalance = address(this).balance - initalBalance;
        payable(marketingAddress).transfer(RecievedBalance);
        tokensForMarketing = tokensForMarketing - _tokens;
    }

    function swapForLiquidity(uint _tokens) private {
        uint half = _tokens / 2;
        uint otherhalf = _tokens - half;
        uint256 initalBalance = address(this).balance;
        swapTokensForEth(half);
        uint256 RecievedBalance = address(this).balance - initalBalance;
        addLiquidity(otherhalf,RecievedBalance);
        tokensForLiquidity = tokensForLiquidity - _tokens;
    }
