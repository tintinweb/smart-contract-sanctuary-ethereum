...
// File contracts/kimono.sol

// Precious Vc contracts v1.0
...
contract Kimono is ERC165, IERC1155, IERC1155MetadataURI, Ownable {
    using Strings for uint256;

    using Address for address;
    using Counters for Counters.Counter;

    uint256 public constant MAX_SUPPLY_PLUS_ONE = 101; // start counting at 1, don't use <= equations
    uint256 public constant MAX_KIMONOS_PER_USER = 1;

    mapping(uint256 => address) public mintedTo;

    mapping(address => bool) public mintedWithKimono;

    bytes32 public merkleRoot =
        0x6e02bbb3e0a292dc2dd8a359fda1740813d62f4a0484ce8782f367856b5b7d37;
    ...
    mapping(address => bool) public isArmour;

    mapping(address => bool) private isNull;

    // Used as the URI for all token types by relying on ID substitution, e.g. https://token-cdn-domain/{id}.json
    string private _uri;

    // Precious  Vc mint price
    ...
    // Kimono discounts used
    mapping(address => uint256) public usedKimonoDiscounts;
    ...
    modifier onlyArmour() {
        require(isArmour[_msgSender()], "Caller not Armour");
        _;
    }
    ...
    mapping(address => uint256[]) private _kimonosMintedFor;
    ...
    function kimonosMintedFor(address _samurai)
        ...
        returns (uint256[] memory)
    {
        return _kimonosMintedFor[_samurai];
    }

    function setNullAddress(address _address, bool value) external onlyOwner {
        require(isNull[_address] != value, "value set to same");
        isNull[_address] = value;
    }

    function setArmour(address[] calldata _armors, bool _value)
        external
        onlyOwner
    {
        for (uint256 i = 0; i < _armors.length; i++) {
            isArmour[_armors[i]] = _value;
        }
    }
    ...
    function claim(uint256 _amount, bytes32[] calldata _merkleProof)
        ...
        returns (uint256)
    {
        require(tx.origin == msg.sender, "VC: We love People.");
        require(_paused == false, "VC: Minting is paused");
        require(
            verifyClaim(_msgSender(), _amount, _merkleProof),
            "VC: Not eligible for a claim"
        );
        require(!rewardClaimed[_msgSender()], "VC: Reward already claimed");
    ...
    function useKimonoDiscount(address _samurai)
        external
        onlyArmour
        returns (bool)
    {
        if (
            usedKimonoDiscounts[_samurai] < 3 &&
            _kimonosMintedFor[_samurai].length > 0
        ) {
    ...
    function canClaimWithKimono(address _samurai) external view returns (bool) {
        return (_kimonosMintedFor[_samurai].length > 0 &&
            !mintedWithKimono[_samurai]);
    }

    function onMintingWithKimono(address _samurai) public onlyArmour {
        _onMintingWithKimono(_samurai);
    }

    function _onMintingWithKimono(address _samurai) private {
        require(
            !mintedWithKimono[_samurai],
            "Samurai can claim with kimono only once"
        );
        require(_kimonosMintedFor[_samurai].length > 0, "Kimono: no kimonos");
        mintedWithKimono[_samurai] = true;
    }
    ...
    function _safeTransferFrom(
        ...
        require(
            _balances[to] <= MAX_KIMONOS_PER_USER || isNull[to],
            "Max Kimonos per Samurai reached"
        );
    ...
    function _safeBatchTransferFrom(
        ...
        for (uint256 i = 0; i < ids.length; ++i) {
            ...
            if (amounts[i] == 1) {
                _owners[id] == to;
                transferedKimonos = transferedKimonos + 1;
            }
        }
        _balances[to] = _balances[to] + transferedKimonos;
        _balances[from] = _balances[from] - transferedKimonos;
    ...
    function _mint(
        ...
        if (amount == 1) {
            ...
            mintedTo[id] = to;
            _balances[to] = _balances[to] + 1;
            _kimonosMintedFor[to].push(id);
        }
        require(
            _balances[to] <= MAX_KIMONOS_PER_USER,
            "Max Kimonos per Samurai reached"
        );
