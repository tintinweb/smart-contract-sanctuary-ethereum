...
contract AvoForwarder is Initializable {
    ...
    function verify(
        ...
        return avoWallet_.verify(actions_, validUntil_, gas_, source_, id_, metadata_, signature_);
    }
...
interface IAvoWallet {
    ...
    struct Action {
        uint8 operation; // 0 -> .call; 1 -> .delegateCall, 2 -> flashloan (via .call), id must be 0 or 2
