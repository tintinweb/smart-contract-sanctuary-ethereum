...
//                                                               //
//                        Magic Printers                         //
//   Contract Development by https://twitter.com/ViperwareLabs   //
//                                                               //
///////////////////////////////////////////////////////////////////
...
contract MagicPrinters is ERC721A, Ownable {
    bytes32 public claimRoot;
    bytes32 public whitelistRoot;
	using Strings for uint;

	uint public WL_PRICE = 0.03 ether;
	uint public PUBLIC_PRICE = 0.04 ether;
	uint public MAX_NFT_PER_TRAN = 5;
    uint public MAX_PER_WALLET = 50;
	uint public maxSupply = 4444;

    uint public MAX_FREE_MINT = 2;
    uint public MAX_PRINTER_CLAIMS = 1000;
    uint public freeClaims = 0;
    ...
    function setMaxFreeMints(uint newFreeMints) external onlyOwner {
		MAX_FREE_MINT = newFreeMints;
	}

    function setMaxPrinterClaims(uint newMaxPrinters) external onlyOwner {
		MAX_PRINTER_CLAIMS = newMaxPrinters;
	}
	...
	function isWhitelistValid(bytes32[] memory proof, bytes32 leaf) public view returns (bool) {
        return MerkleProof.verify(proof, whitelistRoot, leaf);
    }

    function isClaimValid(bytes32[] memory proof, bytes32 leaf) public view returns (bool) {
        return MerkleProof.verify(proof, claimRoot, leaf);
    }
    ...
    function claimFreePrinters(bytes32[] memory proof) public {
        uint count = MAX_FREE_MINT - _freeMintedCount[msg.sender];
        
		require(isWhitelistMint, "Whitelist mint has not started");
        require(isClaimValid(proof, keccak256(abi.encodePacked(msg.sender))), "Wallet not on the allowlist");
		require(_totalMinted() + count <= maxSupply,'Exceeds max supply');
		require(freeClaims + count <= MAX_PRINTER_CLAIMS,'Exceeds max free claim supply.');
        require(count > 0,'No free claims available.');
    ...
    function whitelistMintPrinter(uint count, bytes32[] memory proof) public payable {
		require(isWhitelistMint, "Whitelist mint has not started");
        require(isWhitelistValid(proof, keccak256(abi.encodePacked(msg.sender))), "Wallet not on the allowlist");
		require(_totalMinted() + count <= maxSupply,'Exceeds max supply');
    ...
    function publicMintPrinter(uint count) external payable {
