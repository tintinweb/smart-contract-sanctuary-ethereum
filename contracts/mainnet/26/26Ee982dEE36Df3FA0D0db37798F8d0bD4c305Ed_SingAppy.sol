...
/**
* @title SingAppy (Free Content by RatKingSociety)
* @author Zeeshan Jan
* @notice This contract manages the SingAppy NFTs by https://ratkingsociety.com/
*/

/// @custom:security-contact [email protected]
contract SingAppy is ERC721, ERC721Enumerable, Pausable, Ownable, ReentrancyGuard, DefaultOperatorFilterer {
    using Counters for Counters.Counter;

    /// Counter for Tokens
    Counters.Counter private _tokenIdCounter;
    
    /// Mappings for RatKing NFTs to trace which RatKing ID has been used to mint SingAppy NFT.
    ...
    /**
    * @notice Sets the (IPFS) URL of SingAppy NFTs
    * @param baseURI_ is the (IPFS) URL for SingAppy NFTs
    */
    function setBaseURI(string memory baseURI_) external onlyOwner() {
        _baseURIextended = baseURI_;
    }
    ...
    /**
    * @notice Gets the (IPFS) URI of SingAppy NFTs
    */
    function _baseURI() internal view virtual override returns (string memory) {
        return _baseURIextended;
    }
    ...
    */
    ...
    /**
    * @notice Minting of the SingAppy NFTs
    * @param ratKing is the ID of RatKingSociety NFT.
    * The function checks if the msg.sender owns that RatKing, and if that RatKing has already been used to mint SingAppy NFT.
    */
    function mintSingAppy(uint256 ratKing) public whenNotPaused {
        if(_tokenIdCounter.current() >= MAX_SING_APPY_SUPPLY) revert Errors.MaximumPublicSupplyLimitReached();
    ...
    /**
    * @notice Checks if a RatKing ID has been used to mint SingAppy
    * @param ratKing is the ID of RatKing
    */
    function checkSingAppyMinted(uint256 ratKing) public view returns (bool) {
