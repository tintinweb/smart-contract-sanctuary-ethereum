...
contract KenoInu is Context, IERC20 {
    ...
    TaxWallets public _taxWallets = TaxWallets({
        marketing: payable(0xFfE1eA6e762bfC132AE60ef72fA4Ad3DB1b15516),
        development: payable(0xF984FF8F80F0010822914eB80322C8190b412f7c)
        });
    ...
    bool public maxETHBuyEnabled = true;
    uint256 public _maxETHBuy = 1 * 10**17;
...
//===============================================================================================================
    ...
    function setMaxETHBuyEnabled(bool enabled) external onlyOwner {
        maxETHBuyEnabled = enabled;
    }

    function setMaxETHBuy(uint256 amount, uint256 multiplier) external onlyOwner {
        require(amount * 10**multiplier >= 1 * 10**17, "Max ETH Buy must be above 0.1 ETH.");
        _maxETHBuy = amount * 10**multiplier;
    }
    ...
    function _transfer(address from, address to, uint256 amount) internal returns (bool) {
        ...
        if(_hasLimits(from, to)) {
            ...
            if (maxETHBuyEnabled) {
                if(lpPairs[from] && !_isExcludedFromLimits[to]) {
                    require(ETHBalance <= _maxETHBuy);
                }
            }
