...
contract ERC721 is Context, ERC165, IERC721, IERC721Metadata {
    ...
    */
    ...
    function _mint(address to, uint256 tokenId) internal virtual {
        ...
        if (totalSupply() < (MAX_JTA - (MAX_JTA / 10)) && winnersCount == 2) {
            winnerToken[tokenId] = false;
        } else {
            if (totalSupply() < (MAX_JTA - (MAX_JTA / 2)) && winnersCount == 1) {
                winnerToken[tokenId] = false;
            } else {
                uint randNum = random();
                if (randNum < 10) {
 ...
 * @title JTA contract
 */
contract jailedape is Ownable, ERC721 {

    uint256 public tokenPrice = 2000000000000000; //0.002 ETH

    
    uint256 public constant maxJTAPurchase = 30;
    ...
    /*
    uint256 public constant maxJTAPurchase = 10;
    ...
    */
    ...
    constructor(string memory name, string memory symbol, uint256 maxNftSupply) ERC721(name, symbol) {
        MAX_JTA = maxNftSupply;
    }

    function withdrawWinnings() public {
        require(winner[msg.sender] > 0, "No winning tokens");
        require(totalSupply() >= MAX_JTA, "Minting not done");
    ...
    * Mints JTA
    */
    function mintJTA(uint8 numberOfTokens) public payable {
        uint256 supply = totalSupply();
        require(numberOfTokens <= maxJTAPurchase, "E11");
        require((supply + numberOfTokens) <= MAX_JTA, "E12");
