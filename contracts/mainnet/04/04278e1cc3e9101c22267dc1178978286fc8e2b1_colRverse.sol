...
contract colRverse is Ownable, ERC721A, ReentrancyGuard {
    using Strings for uint256;


  uint256 public MAX_PER_Transtion = 10; // maximam amount that user can mint per transaction

  uint256 public  PRICE = 0.025 ether; //0.10 during public sale  

  uint256 private constant TotalCollectionSize_ = 1496; // total number of nfts
  uint256 private constant MaxMintPerBatch_ = 10; //max mint per trx
 
  mapping(address => bool) private whitelistedAddressesForMint;
  mapping(address => bool) private whitelistedAddressesForPreMint;
 


  string private _baseTokenURI;

  uint public status = 0; //0 - sale pause, 1 - free sale, 2 - pre launch, 3 - public sale

  constructor() ERC721A("colRverse","colRverse", MaxMintPerBatch_, TotalCollectionSize_) {
   
    _baseTokenURI= "https://gateway.pinata.cloud/ipfs/QmbZuqe7LtFKUkVhVZM2UVp34LfoguZLv2RZa12NT1qtES/";
...
function mint(uint256 quantity) external payable callerIsUser {
    ...
    require(whitelistedAddressesForPreMint[msg.sender], "You are not White Listed For Pre Launch");
    require(quantity > 0,"Mint Quantity should be more than 0");
    require(quantity <= 2,"One Address can mint upto 2 NFTs in Pre Launch");
    require(numberMinted(msg.sender) <= 1, "Already minted the quota" );
  ...
  function isWhitelistedForPreMint(address _user) public view returns (bool) {
    return whitelistedAddressesForPreMint[_user];
  }
  ...
  function addNewWhitelistUserForFreeMint(address[] calldata _users) public onlyOwner {
   ...
   function addNewWhitelistUserForPreMint(address[] calldata _users) public onlyOwner {
    // ["","",""]
    for(uint i=0;i<_users.length;i++)
        whitelistedAddressesForPreMint[_users[i]] = true;
  }
