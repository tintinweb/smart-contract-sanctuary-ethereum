...
contract STARPIXELWARS is Ownable, ERC721A{
    ...
    constructor() ERC721A("STARPIXELWARS", "SPW") {}
    ...
    //airdrop
    function airdrop_single(uint256 _quantity, address _recipient) external onlyOwner {
        require(totalSupply() + _quantity <= maxSupply, "Too many");
    ...
    // add address(es) to an on-chain whitelist
    ...
    function onWhiteList(address _address) external view returns (bool) {
        return whiteList[_address];
    }

    // removes address(es) from the whitelist   
