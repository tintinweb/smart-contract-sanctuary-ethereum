...
contract JuiceBlender is Administration {
    ...
    function mergeTier4(address account, uint256 value) public whenNotPaused {
        if (!_allowMerge) revert MergeNotPermitted();
        if (_pinaverse == address(0)) revert MustSetPinaverse();

        uint256 preBalance = _juice.balanceOf(account, 3);
        uint256 eligibleToMint = value / 10;
        uint256 amountToTransfer = eligibleToMint * 10;
        ...
        unchecked {
            // Verify Tier 5 is still mintable
            if (_juice.totalMinted(4) + eligibleToMint > 4) {
                revert MaxJuiceSupplyReached();
            }
        }

        // Transfer Tier 4 NFTs to Pinaverse
        _juice.safeTransferFrom(account, _pinaverse, 3, amountToTransfer, "");

        unchecked {
            // Verify balance after transfer is less value
            if (_juice.balanceOf(account, 3) != preBalance - amountToTransfer) {
                revert TranfserFailed();
            }
        }

        // Mint eligible Tier 5 NFTs
        _juice.mint(account, 4, eligibleToMint);
    }
