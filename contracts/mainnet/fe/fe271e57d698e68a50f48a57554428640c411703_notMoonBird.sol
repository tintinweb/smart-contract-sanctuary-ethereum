...
contract notMoonBird is ERC721A, Ownable, ReentrancyGuard {

    bool public paused = false;

    uint256 private mintedPublic = 0; 
    uint256 public price = 0.01 ether;
    uint256 public bundlePrice = 0.1 ether;
    uint256 public bundle = 10;
    uint256 public maxSupply = 2333;

    string private _baseTokenURI;

    mapping(uint256 => uint256) private mintedTime;

    constructor(uint256 maxSizeBatch, uint256 maxSupply_, string memory baseUri) ERC721A("NotMoonBird", "NMB", maxSizeBatch, maxSupply_) {
    ...
    function mint() external payable callerIsUser {
      ...
      require(mintedPublic + 1 <= maxSupply, "Count exceeds the maximum allowed supply for public mint.");
    ...
    // mintBundle
    function mintBundle() external payable callerIsUser {
      require(paused == false, "Contract paused.");
      require(mintedPublic + bundle <= maxSupply, "Count exceeds the maximum allowed supply for public mint.");
