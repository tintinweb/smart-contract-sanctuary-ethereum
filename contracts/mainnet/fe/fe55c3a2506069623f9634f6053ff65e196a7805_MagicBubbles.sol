...
contract MagicBubbles is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    constructor() ERC20("Magic Bubbles", "BUBBLE") {
        
        IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D);
        
        airDropLimitLiftDate = block.timestamp + 5 days;
        airDropLimitInEffect = true;
        airDropDailySellPerc = 20;
        ...
        if(airDropLimitInEffect){ // Check if Limit is in effect
            if(airDropLimitLiftDate <= block.timestamp){
                ...
                if(_isAirdoppedWallet[from] && senderBalance.sub(amount) < _airDropTokensRemaining[from]){
                    ...
                    require(_airDropAddressNextSellDate[from] <= block.timestamp && block.timestamp >= airDropLimitLiftDate.sub(4 days), "_transfer:: Please read the contract for your next sale date.");
                    uint256 airDropMaxSell = getWalletMaxAirdropSell(from); // airdrop 20% max sell of total airdropped tokens per day for 5 days
