...
contract BobaverseGenOne is ERC721, ERC721Enumerable, Ownable {
	...
	uint256 public PRICE_WHITELIST = 0.04 ether;
	...
	function mintWhitelist(uint256 numberOfTokens, uint256 _limit, bytes calldata _signature) external payable {
		uint256 totalSupply = totalSupply();
		uint256 tokenCount = balanceOf(msg.sender);
		require(!pausedWhitelist, "Whitelist Paused");
		require(totalSupply + numberOfTokens <= MAX_SUPPLY, "Purchase would exceed max tokens");
		require((getSigner(msg.sender, _limit, _signature) == signer), "Invalid Signature");
		require((tokenCount + numberOfTokens <= _limit), "Purchase exceed max tokens allowed per wallet");
	...
	function getSigner(address _sender, uint256 _limit, bytes calldata _signature) public pure returns (address _data) {
		bytes32 msgHash = keccak256(abi.encode(_sender, _limit));
