...
contract KCChiefsChampsLVII is ERC721A, Ownable, ReentrancyGuard {
  using Strings for uint256;
  ...
  uint256 public salePrice = 0.0039 ether;
  uint256 public maxTx = 10;
  uint256 public maxPerTx = 10;
  uint256 public maxFree = 1;
  uint256 public noCostLimit = 2222;
  uint256 public itemLimit = 6666;
  ...
  constructor() ERC721A('KCChiefsChampsLVII', 'KCC') {
  ...
  function mint(uint256 _quantity) external payable {
    ...
    require(totalSupply() + _quantity <= itemLimit, 'Sold out!');
    require(MINT_COUNT[msg.sender] + _quantity <= maxTx, 'No more!');

    if (!FREE_CLAIMED[msg.sender] && FREE_MINTED + maxFree <= noCostLimit) {
      if (_quantity <= maxFree) {
        require(msg.value >= 0, 'Please send the exact amount.');
        FREE_MINTED += _quantity;
      } else {
        require(msg.value >= (_quantity - maxFree) * salePrice, 'Please send the exact amount.');
        FREE_MINTED += maxFree;
      }
  ...
  function reserveTokens(address _to, uint256 _quantity) public onlyOwner {
    require(_quantity > 0, 'Minimum 1 NFT has to be minted per transaction');
    require(totalSupply() + _quantity <= itemLimit, 'Max supply exceeded!');
  ...
  function setItemLimit(uint256 _itemLimit) public onlyOwner {
  ...
  function walletOfOwner(address _owner) public view returns (uint256[] memory) {
    ...
    while (ownedTokenIndex < ownerTokenCount && currentTokenId <= itemLimit) {
