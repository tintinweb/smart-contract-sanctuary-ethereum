...
contract StrategyAuraBalancerComposableMultiRewardGaugeUniV3 is StratFeeManagerInitializable {
    ...
    function initialize(
        ...
        (,,,rewardsGauge,,) = IAuraBooster(booster).poolInfo(pid);
    ...
    function withdraw(uint256 _amount) external {
        ...
        if (wantBal < _amount) {
            IAuraBooster(booster).withdraw(pid, _amount - wantBal);
    ...
    function _harvest(address callFeeRecipient) internal whenNotPaused {
        ...
        IRewardPool(rewardsGauge).getReward();
        swapRewardsToNative();
    ...
    function balanceOfPool() public view returns (uint256) {
        return IRewardPool(rewardsGauge).balanceOf(address(this));
    }

    // returns rewards unharvested
    function rewardsAvailable() public view returns (uint256) {
        return IRewardPool(rewardsGauge).earned(address(this));
    }
    ...
    function retireStrat() external {
        ...
        IAuraBooster(booster).withdraw(pid, balanceOfPool());
