...
contract MetaGodsDrops is ERC721Enum, Ownable, ReentrancyGuard {

    string private baseURI;

    mapping(uint256 => uint256) public tokenTypeMapping;
    ...
    function sendDrop(address _to, uint256 _amount, uint256 _tokenType) external onlyOwner {
        uint256 supply = totalSupply();
        for (uint256 i = 0; i < _amount; i++) {
            tokenTypeMapping[supply + i] = _tokenType;
            _safeMint(_to, supply + i);
        }
    }

    function updateTokenMapping(uint256[] calldata tokens, uint256[] calldata types) external onlyOwner {
        require(tokens.length == types.length, "Invalid params");

        for (uint256 i = 0; i < tokens.length; i++) {
            tokenTypeMapping[tokens[i]] = types[i];
        }
    }
