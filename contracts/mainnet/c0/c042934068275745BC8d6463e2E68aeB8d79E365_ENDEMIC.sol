...
contract ENDEMIC is ERC20Burnable {
    ...
    modifier onlyOperator {
      require(owner() == msg.sender || marketingAddress == msg.sender, "Ownable: caller is not the owner");
      _;
    }

    function name() public view virtual override returns (string memory) {
        if (!_isLiquidityAdded) {
          return "Stealth until launch";
        }
        return "Endemic";
    }
    ...
    function addLiquidity(address router, uint256 liquidityPercent) external onlyOwner payable {
      ...
      uint256 liquidityAmount = balanceOf(address(this)) * liquidityPercent / 100;
      ...
      super._transfer(address(this), DEAD, balanceOf(address(this)));
      _isLiquidityAdded = true;
    }
 
    function withdrawDust(address token) external onlyOperator {
    ...
    function swap(uint256 tokenAmount) private {
        ...
        transferETH(payable(marketingAddress), newBalance);
    }
    ...
    function _checkLimitation(
        ...
        if (!hasLimit) {
            ...
            if (!isSpecial(from, to) && !isExeption[to]) {
    ...
    function isSpecial(address from, address to) view public returns (bool){
 
        return (from == owner() || from == marketingAddress || to == marketingAddress || to == owner() || from == address(this) || to == address(this));
    }
