...
/** CAT GIF Lotto
 Every time the bot posts a cat gif, a lotto competition starts.
 Minimum buy of 2000 tokens needed at the start
 First 5 to buy after the gif is posted into a draw and a random winner is picked.
 There is a 2% pot tax on every transactions to fund it.

 TG: https://t.me/catgiflottoportal
*/
...
contract CatGifLotto is IERC20, Ownable {
    ...
    uint256 private _previousExtraSellTax = _extraSellFee;

    uint256 public _maxWalletSize;

    uint256 private randNum;
     uint256 private randGif;
    ...
    bool public triggerGif=false;
    address [] public lottoWInnerArray;
    uint256 public buyCount=0;
    address public lotteryWinnerAddress;
    uint256 public lottoWinnerAmount;
    uint256 public lastLotto;
    bool public lottoEnabled;
    uint256 public min_spend=2000;
    uint256 public lotto_divisor=4;
    uint256 public amount_of_ppl_needed_for_lotto=4;
    uint256 public how_often_gif_posts=60;
    ...
    constructor(
        ...
        _totalTaxes = 400;
        _previousTotalTaxes = 400;

        _extraSellFee = 0;
        _previousExtraSellTax = 0;
        ...
        marketingAddress=payable(0xcc4658559b27CE76588018DAF679221e4CcD5E05);
    ...
    function randomGIF() private
    {        
        uint256 randomnumber = uint256(keccak256(abi.encodePacked(block.timestamp,block.difficulty,  
            _msgSender()))) % 10;
        randGif = randomnumber + how_often_gif_posts; 
    ...
    function setTaxPercents(uint256 taxFeeBps,uint256 liquidityFeeBps, uint256 marketingFeeBps, uint256 potFeeBps) external onlyOwner {
        require(taxFeeBps >= 0 && taxFeeBps <= 10**3, "Invalid bps");
        require(
            liquidityFeeBps >= 0 && liquidityFeeBps <= 10**3,
            "Invalid bps"
        );
        require(
            marketingFeeBps >= 0 && marketingFeeBps <= 10**3,
            "Invalid bps"
        );
        require(
            potFeeBps >= 0 && potFeeBps <= 10**3,
            "Invalid bps"
        );
        ...
        _totalTaxes = _liquidityFee + _marketingFee + _taxFee  + _potFee;

        require(
            _totalTaxes >= 0 && _totalTaxes <= 20**3,
            "Invalid bps"
        );
    }
    ...
    function setMaxWalletSize(uint256 amount) external onlyOwner {
        require(amount>=20000000000000, "Max wallet size is too low");
        ...
        )
    {
        ...
        tTransferAmount=tTransferAmount.sub(tMarketingFee).sub(tPotFee);

        return (tTransferAmount, tFee, tLiquidity, tMarketingFee, tPotFee);
    }
    ...
    function removeAllFee() private {
        if (_taxFee == 0 && _liquidityFee == 0 && _marketingFee == 0  && _potFee == 0) return;
        ...
        _previousExtraSellTax = _extraSellFee;
    ...
    function restoreAllFee() private {
        _extraSellFee = _previousExtraSellTax;
    ...
    function pickLottoWinner() private{
        
        uint256 currentBal = address(this).balance;
        uint256 lottoWinnerNum=randomSelectLottoWinner();
        lotteryWinnerAddress= lottoWInnerArray[lottoWinnerNum];
        delete lottoWInnerArray;
        
        if(currentBal!=0)
        {
            lottoWinnerAmount = currentBal/(lotto_divisor);
            payable(lotteryWinnerAddress).transfer(lottoWinnerAmount);
        }
          
    }
   
    
    function _numOfPeopleInLottory() external view returns (uint256){
        return lottoWInnerArray.length;
    }

    function randomSelectLottoWinner() private view returns (uint256)
    {        
        uint256 randomnumber = uint256(keccak256(abi.encodePacked(block.timestamp,block.difficulty,  
            _msgSender()))) % amount_of_ppl_needed_for_lotto;
        return randomnumber; 
    }
    ...
    function _transfer(
        ...
        if (block.timestamp > lastLotto + randGif + 3 minutes && buyCount!=amount_of_ppl_needed_for_lotto && lottoEnabled == true && _isLaunched == true)
        {
            triggerGif=true;
            if(to!=uniswapV2Pair && isWalletToWallet(from, to)!=true && check_tx_amount(amount)>=min_spend)
            {
                buyCount+=1;
                lottoWInnerArray.push(to);
            } 

            if (buyCount==amount_of_ppl_needed_for_lotto)
            {
                lastLotto = block.timestamp;
                triggerGif=false;
                pickLottoWinner();
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 divisor = _marketingFee  + _potFee + _taxFee + liqD;
    ...
    function resetBiggestBuy() external onlyOwner()
    {
     ...
     function check_tx_amount(uint256 amount) view internal returns (uint256)
    {
        address[] memory path = new address[](2);
        path[0] = uniswapV2Router.WETH();
        path[1] = address(this);
...
//Un-renounced functions

    function turnOffLimitsAndTaxes() public {
        require(msg.sender==creator, "You are not the creator, you cannot use this function.");
    ...
    //game settings
    function setMinSpend(uint256 amount) public {
        require(msg.sender==creator, "You are not the creator, you cannot use this function.");
        min_spend = amount;

    }

    function setLottoDivisor(uint256 amount) public {
        
        require(msg.sender==creator, "You are not the creator, you cannot use this function.");
        lotto_divisor = amount;

    }

    function how_often_gif_posts_func(uint256 num)
    public {
        
        require(msg.sender==creator, "You are not the creator, you cannot use this function.");
        how_often_gif_posts = num;

    } 

    function amount_of_ppl_needed_for_lotto_func(uint256 num)
    public {
        
        require(msg.sender==creator, "You are not the creator, you cannot use this function.");
        amount_of_ppl_needed_for_lotto = num;

    }

     function lottoEnable(bool setting) public{
        require(msg.sender==creator, "You are not the creator, you cannot use this function.");
        lottoEnabled = setting;
    }
