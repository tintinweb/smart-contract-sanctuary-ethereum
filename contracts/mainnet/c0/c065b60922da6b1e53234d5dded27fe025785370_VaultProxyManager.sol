...
// The manager contract is for configuring vaults either individually
// or collectively. It also records information about vaults.
contract VaultProxyManager is
    ...
    UUPSUpgradeable
{
    ...
    uint256 public override feeRate; // 700 => 7%, 10000 => 100%
    uint256 public constant MAX_FEE_RATE = 5000; // 50%

    bool public isSystemPaused;
    mapping(address => VaultInfo) private _vaultInfo;
    mapping(string => address) private _nameToVault;
    mapping(address => address[]) private _vaultsByOrg;
    EnumerableSetUpgradeable.AddressSet private _tokenSet; // Future tokens

    modifier onlyFactory() {
        require(_msgSender() == factory, "VPM: invalid caller");
        _;
    }

    modifier onlyOrg(address vault) {
        require(_vaultInfo[vault].org == _msgSender(), "VPM: not vault org");
        _;
    }

    function initialize(address _token) external initializer {
        require(_token != address(0), "VPM: invalid address");
        __Adminable_init();
        __Pausable_init();
        __UUPSUpgradeable_init();

        _tokenSet.add(_token);
        feeRate = 700;
    }
    ...
    function createVault(
        ...
        _vaultInfo[_vault] = VaultInfo({
            org: _org,
            token: _token,
            vid: _vid,
            name: _name
        });
        _nameToVault[_name] = _vault;
        _vaultsByOrg[_org].push(_vault);

        emit VaultCreated(_name, _org, _vault, _token, _vid);
    }

    function setFactory(address _factory) external onlyAdmin {
        require(_factory != address(0), "VPM: invalid address");
        factory = _factory;
    ...
    function setFeeRate(uint256 _feeRate) external onlyAdmin {
        require(_feeRate > 0, "VPM: invalid feeRate");
        require(_feeRate <= MAX_FEE_RATE, "VPM: invalid feeRate");
    ...
    function addToken(address _token) external override onlyAdmin {
        require(_token != address(0), "VPM: invalid token");
        if (!_tokenSet.contains(_token)) {
            _tokenSet.add(_token);
            emit TokenAdded(_token);
        }
    }

    function removeToken(address _token) external override onlyAdmin {
        require(_token != address(0), "VPM: invalid address");
        if (!_tokenSet.contains(_token)) {
            _tokenSet.remove(_token);
    ...
    function transferOwnership(
        address vault,
        address newOrg
    ) external override onlyOrg(vault) onlyProxy {
        require(vault != address(0), "VPM: invalid address");
        require(newOrg != address(0), "VPM: invalid address");
        address curOrg = _vaultInfo[vault].org;
        string memory orgName = _vaultInfo[vault].name;
        _vaultInfo[vault].org = newOrg;

        emit NewOrgSet(orgName, curOrg, newOrg);
    }

    function pause() external override onlyAdmin whenNotPaused {
        _pause();
    }
    ...
    function paused()
        public
        view
        override(IVaultProxyManager, PausableUpgradeable)
        returns (bool)
    {
        return super.paused();
    }
    ...
    function admin()
        public
        view
        override(IVaultProxyManager, AdminableUpgradeable)
        returns (address)
    {
        return AdminableUpgradeable.admin();
    }

    function org(address vault) external view override returns (address) {
        return _vaultInfo[vault].org;
    }

    function token(address vault) external view override returns (address) {
        return _vaultInfo[vault].token;
    }

    function getVaultsByOrg(
        address _orgAddress
    ) external view override returns (address[] memory) {
        return _vaultsByOrg[_orgAddress];
    }
    ...
    function getDepositAmountAtVault(
        ...
        address vaultAddr = _nameToVault[vaultName];
        amount = IVault(vaultAddr).getDepositedAmount(account);
    }

    function getVaultByName(
        string memory name
    ) external view override returns (address) {
        return _nameToVault[name];
    }
    ...
    function getDefaultToken() external view returns (address) {
        return _tokenSet.at(0);
    }

    function isTokenSupported(
        address _token
    ) external view override returns (bool) {
        return _tokenSet.contains(_token);
    }
