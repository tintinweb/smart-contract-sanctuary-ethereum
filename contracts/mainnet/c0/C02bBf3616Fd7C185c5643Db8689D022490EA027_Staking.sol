...
// File: contracts/MineNationsStaking.sol
...
contract Staking is IERC721Receiver {
    ...
    modifier onlyOwner() {
        ...
        require(msg.sender == 0x1B3FEA07590E63Ce68Cb21951f3C133a35032473, "not admin");
        _;
    }

    mapping(address => EnumerableSet.UintSet) stakes;
    
    function stake(address _user, uint16[] memory _tokenIds) external {
        require(msg.sender == _user, "please call as token owner");

        for(uint i = 0; i < _tokenIds.length; i++) {
            uint16 tokenId = _tokenIds[i];
            stakes[_user].add(tokenId);
            ERC721(minenations).safeTransferFrom(_user, address(this), tokenId);
        }
    }
    function unstake(address _user, uint16[] memory _tokenIds) external {
        require(msg.sender == _user, "please call as token owner");
    ...
    function setMinenationsAddress(address _address) external onlyOwner {
        minenations = _address;
    }
