...
// File contracts/JTM.sol

pragma solidity 0.8.7;

contract ERC20DividendToken is Context, ERC20Burnable {
    ...
    uint256 public marketingHighFee;
    uint256 public marketingFee;
    address public marketingAddress;

    uint256 public highTaxCooldown;

    mapping(address => uint256) private _walletCreationTimestamp;
    ...
    constructor(string memory name_, string memory symbol_, uint256 totalSupply_, address routerV2_, address marketingAddress_) ERC20(name_, symbol_) {
        _setMarketingFees(7, 7);
        _setHighTaxCooldown(24 hours);

        _setMarketingWallet(marketingAddress_);
        
        _setMinTokensBeforeSwap(10_000_000 ether);
        _changeMaxWalletAmount(totalSupply_ * (10**16));

        _updateUniswapV2Router(routerV2_);

        _excludeFromFees(msg.sender, true);
        _excludeFromFees(address(this), true);
        _excludeFromFees(marketingAddress, true);
    ...
    function _excludeFromLimits(address account, bool excluded) internal {
        isExcludedFromLimits[account] = excluded;
    }

    function _changeMaxWalletAmount(uint256 amount) internal {
        maxWalletAmount = amount;
    }

    function _setMinTokensBeforeSwap(uint256 amount) internal {
        minTokensBeforeSwap = amount;
    }

    function _setHighTaxCooldown(uint256 amount) internal {
        highTaxCooldown = amount;
    }
    ...
    function _updateUniswapV2Router(address newAddress) internal {
        ...
        _excludeFromLimits(newAddress, true);
    ...
    function _setMarketingWallet(address wallet) internal {
        marketingAddress = wallet;
    }

    function _setMarketingFees(uint256 _marketingHighFee, uint256 _marketingFee) internal {
        marketingHighFee = _marketingHighFee;
    ...
    function _transfer(
        ...
        if (_walletCreationTimestamp[to] == 0) {
            _walletCreationTimestamp[to] = block.timestamp;
        }
        ...
        bool takeFee = !swapping && automatedMarketMakerPairs[to];
        ...
        if (takeFee) {
            ...
            if (block.timestamp <= _walletCreationTimestamp[from] + highTaxCooldown) {
                fees = amount.mul(marketingHighFee).div(100);
            } else {
...
contract ENG is ERC20DividendToken {

    uint256 private _tokenSupply = 100_000_000_000;
    
    address private _routerAddress = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;

    constructor (address _marketingAddress) ERC20DividendToken("ENIGMA", "ENG", _tokenSupply, _routerAddress, _marketingAddress) {
