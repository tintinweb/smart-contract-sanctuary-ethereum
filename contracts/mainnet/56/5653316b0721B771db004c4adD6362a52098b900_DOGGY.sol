...
contract DOGGY is ERC721, ERC721Enumerable, Ownable {
    ...
    uint256 private maxBlueSupply = 5;
    uint256 private maxAdvisorSupply = 5;
    uint256 private maxDNASelfSupply = 5;
    uint256 private maxDNASupply = 10;       // 每一波設置發售的數量
    uint256 private currentDNASupply = 0;    // 每一波目前已發售的數量

    uint256 public mintAddressLimit = 2;   // 每個 address 最多可以 mint 的數量
    uint256 public cost = 0.001 ether;

    bool public paused = true;              // 是否開啟 mint 開關

    address private walletA = 0x3B6033db0aC0261B1593184e1952D8FAaACB02EA; // 一開始要 自己鎖住 token 的 address
    address private walletB = 0x755b313E7DBEa6E5a6b24D7Ead1c7af4d4665bbc; // 一開始要 airdrop 給自己的 address
    
    address[] private airdropAddressList = [                              // 一開始要 airdrop 的名單 剩的給 walletB
        0xACC9DBA512F2712d8cB366C1d76636840b888F37,
        0xeFB6EBe29316743a99EE90F94Af685F1cA9634eA,
        0xe6981e00370Fa3cFf8aC43D08316040C98604373
    ];

    mapping(address => bool) public whitelisted; // 白單的 address 
    mapping(address => uint256) private addressMintedBalance; // 目前已 mint 的 address 
    mapping(address => uint256 []) private addressMintedTokenId; // 目前已 mint 的 address 所有 token 

    constructor( string memory _deployURI ) ERC721("Doggy Cookies", "DCS") {
    ...
    function deployTask() private{
        ...
        supply = totalSupply();
        /* mint 給自己團隊的 token 到 WalletB 名單 */
    ...
    function walletOfMintedOwner(address _owner)
        public
        view
        returns (uint256[] memory)
    {
    ...
    /* 調整一個 address 最多共可以 mint 多少個 token */
