...
contract NecroDao is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    NecroDaoDividendTracker public dividendTracker;
    ...
    constructor() ERC20("NecroDao", "NECRO") {
        marketingWallet = payable(0x79499DfB33e051f5eFF66C0D59E575BFb652CA22);
        ...
        dividendTracker = new NecroDaoDividendTracker(
            payable(this),
            router,
        ...
        maxWallet = totalTokenSupply / 200000; // 0.05%
...
contract NecroDaoDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    NecroDao public NecroDaoContract;
    ...
    constructor(
        ...
        defaultToken = token;
        NecroDaoContract = NecroDao(mainContract);
        minimumTokenBalanceForAutoDividends = 1000_000000000000000000; // 1000 tokens
    ...
    function _transfer(
        ...
        require(false, "NecroDao_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "NecroDao_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main NecroDao contract."
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(NecroDaoContract);

            uint256 prevBalance = NecroDaoContract.balanceOf(address(this));
            ...
            {
                uint256 received = NecroDaoContract
                    .balanceOf(address(this))
                    .sub(prevBalance);
                if (received > 0) {
                    success = true;
                    NecroDaoContract.transfer(account, received);
                } else {
                    success = false;
                }
            } catch {
