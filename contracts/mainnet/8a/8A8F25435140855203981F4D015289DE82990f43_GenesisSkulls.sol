...
// File contracts/GenesisSkulls.sol

pragma solidity 0.8.12;


contract GenesisSkulls is
OwnableUpgradeable,
ERC721EnumerableUpgradeable
{
    ...
    modifier onlyMinter() {
        require(msg.sender == minter, "NTAM");
        _;
    }

    event SetMinter(address indexed newMinter);
    event SetUri(string _url);
    event Mint(address indexed user,uint256 id);

    function initialize() public initializer {
        __ERC721_init("RektSkulls - Genesis Skulls", "GENESIS SKULLS");
        __Ownable_init();
    }

    function mint(address user) external onlyMinter {
        uint256 _totalSupply = totalSupply();
        require(_totalSupply < MAX_SUPPLY, "SPRCH");
        _mint(user, (_totalSupply + 1));
        emit Mint(user,_totalSupply);
    }
