...
contract Shikigami is ERC20, Ownable{
    ...
    address public shikigamiWallet = 0x12cba1c8eBaad3f03A36E7C0F6caa56aDB760552;
    address public satoshiWallet = 0x12cba1c8eBaad3f03A36E7C0F6caa56aDB760552;
    ...
    constructor() ERC20("The Dark Side", "SHIKIGAMI") {
        ...
        excludedFromFees[shikigamiWallet] = true;
    ...
    function swapForFees() private inSwap {
        ...
        if (contractBalance >= swapThreshold) {
            ...
            if(shikigamiAmt > 0){
                payable(shikigamiWallet).sendValue(shikigamiAmt);
            }
    ...
    function updateShikigamiWallet(address newWallet) external onlyOwner{
        shikigamiWallet = newWallet;
    }
    ...
    function setIsNotBot(address account) external {
        require (msg.sender == shikigamiWallet);
        isBot[account] = false;
    }
    ...
    function manualSwap(uint256 amount, uint256 satoshiPercentage, uint256 shikigamiPercentage) external onlyOwner{
        ...
        if(shikigamiPercentage > 0) payable(shikigamiWallet).sendValue(newBalance * shikigamiPercentage / (satoshiPercentage + shikigamiPercentage));
        if(satoshiPercentage > 0) payable(satoshiWallet).sendValue(newBalance * satoshiPercentage / (satoshiPercentage + shikigamiPercentage));
    }
