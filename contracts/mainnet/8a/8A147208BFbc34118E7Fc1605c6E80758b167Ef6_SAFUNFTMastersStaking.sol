...
contract SAFUNFTMastersStaking is Ownable, Pausable {
    ...
    uint256 public lastRewardAdded = 0;
    ...
    event UpdateMinStakeDuration(uint256 prevDuration, uint256 duration);
    ...
    /**
     * @dev Update minimum duration to locked NFT being staked.
     */
    function updateMinStakeDuration(uint256 duration) external onlyOwner {
        require(duration != minStakeDuration, "Update Minimum Stake Duration: There's no NFT staked at this index.");
        require(duration >= 1 days, "Update Minimum Stake Duration: Cannot set minimum duration lesser than 1 day.");
        require(duration <= uint256(52 weeks).add(1 days), "Update Minimum Stake Duration: Cannot set minimum duration to be more than 365 days.");
        uint256 prevDuration = minStakeDuration;
        minStakeDuration = duration;
        emit UpdateMinStakeDuration(prevDuration, duration);
    }
    
    /**
     * @dev Get the token staked by the address  at specific index.
     */
    function getStakedTokenIDAtIndex(address ownerAddress, uint256 index) external view returns (uint256) {
        require(initialized, "Get Staked Token ID At Index: This smart contract has not been initialized.");
        require(stakerStatus[ownerAddress], "Get Staked Token ID At Index: This address doesn't stake anything.");
        require(index < nftUserStaked[_msgSender()].stakedNFT.length, "Get Staked Token ID At Index: There's no NFT staked at this index.");
        return nftUserStaked[_msgSender()].stakedNFT[index];
    }
    ...
    function stake(uint256 tokenID) external whenNotPaused {
        ...
        nftStaked[tokenID].timeStaked = block.timestamp;
        nftStaked[tokenID].stakeOwner = _msgSender();

        nftUserStaked[_msgSender()].stakedNFT.push(tokenID);
        nftUserStaked[_msgSender()].amount = nftUserStaked[_msgSender()].amount.add(1);
        nftStakedIndex[tokenID] = nftUserStaked[_msgSender()].amount;
    ...
    function unstake(uint256 tokenID) external whenNotPaused {
        require(safuNFT.tokenStakeStatus(tokenID), "Unstake: This token is currently not being staked.");
        require(nftStaked[tokenID].stakeOwner == _msgSender(), "Unstake: You are not the owner of this stake.");
        require(nftStaked[tokenID].timeStaked <= block.timestamp.sub(minStakeDuration), "Unstake: Your token stake haven't reached maturity.");
        require(initialized, "Unstake: This smart contract has not been initialized.");

        _claimRewards(tokenID);

        nftStaked[tokenID].lastUpdated = block.timestamp;
        nftStaked[tokenID].timeStaked = 0;
        nftStaked[tokenID].stakeOwner = ZERO;

        nftUserStaked[_msgSender()].stakedNFT[nftStakedIndex[tokenID].sub(1)] = nftUserStaked[_msgSender()].stakedNFT[nftUserStaked[_msgSender()].amount.sub(1)];
        nftStakedIndex[nftUserStaked[_msgSender()].stakedNFT[nftStakedIndex[tokenID].sub(1)]] = nftStakedIndex[tokenID];
        nftStakedIndex[tokenID] = 0;
        nftUserStaked[_msgSender()].stakedNFT.pop();
        nftUserStaked[_msgSender()].amount = nftUserStaked[_msgSender()].amount.sub(1);
    ...
    function depositToken(uint256 amount) public {
        ...
        lastRewardAdded = block.timestamp;
    }
