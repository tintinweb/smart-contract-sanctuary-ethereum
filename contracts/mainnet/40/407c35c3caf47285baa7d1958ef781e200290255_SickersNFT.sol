...
import "@os/OperatorFilterer.sol";

contract SickersNFT is ERC721A, Ownable, SignedAllowance, OperatorFilterer { 

   using Strings for uint256;

   uint256 public constant MAX_ITEMS = 5555;
   ...
   uint256 public regularPrice = 0.039 ether; 
   //uint256 public alPrice = 0.029 ether; 
   ...
   function publicMint(uint256 qty) public payable {
        ...
        require(totalSupply() + qty <= MAX_ITEMS, ">MaxSupply");
        require(qty <= MAX_PER_TXN, ">Max Per Txn");
        require (msg.value >= regularPrice * qty, "Not Enough Eth");
   ...
   function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        require(_exists(tokenId), "Sickers: this token does not exist");
   ...
   function withdraw(uint256 amt, address payable to) public onlyOwner {
