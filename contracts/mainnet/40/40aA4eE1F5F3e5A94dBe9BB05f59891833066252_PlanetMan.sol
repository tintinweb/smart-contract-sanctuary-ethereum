...
contract PlanetMan is ERC721A, IERC2981, Ownable {
    enum Status {
        Start,
        Finish
    }
    ...
    uint256 public constant MAX_RESERVE = 23;
    uint256 public constant PRICE = 0.081 ether;
  ...
  // Metadata
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        ...
        if (revealed) {
            ...
            return string(abi.encodePacked(baseURI_, "blackhole.json"));
        }
    }
  ...
  // Reserve
    function reserveMint(uint256 quantity) external onlyOwner {
        ...
        require(numberMinted(msg.sender) + quantity <= MAX_RESERVE, "Exceed owner's reserve limit.");
  ...
  // Mint
    ...
    function mint(uint256 quantity) external payable {
        require(tx.origin == msg.sender, "Contract not allowed.");
        require(totalSupply() + quantity <= TOTAL_SUPPLY - MAX_RESERVE, "Exceed max public supply.");
        require(status == Status.Start, "Mint is over.");
