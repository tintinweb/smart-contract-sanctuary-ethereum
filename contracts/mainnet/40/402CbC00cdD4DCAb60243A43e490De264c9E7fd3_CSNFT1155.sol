    ...
    Pausable
{
    ...
    function mintFrom(
        ...
        _mint(_from, _to, _id, _quantity, _data);
    }
...
contract AssetContract is ERC1155Tradable {
    ...
    function safeTransferFrom(
        ...
        if (mintedBalance < _amount) {
            ...
            mintFrom(_from, _to, _id, _amount - mintedBalance, _data);
            if (mintedBalance > 0) {
 ...
 */
contract CSNFT1155 is AssetContract, ReentrancyGuard {
    // Migration contract address
    CSNFT1155 public migrationTarget;
    ...
    modifier creatorOnly(uint256 _id) {
        require(
            ...
            "CSNFT1155#creatorOnly: ONLY_CREATOR_ALLOWED"
        );
        _;
    }
    ...
    modifier onlyFullTokenOwner(uint256 _id) {
        require(
            ...
            "CSNFT1155#onlyFullTokenOwner: ONLY_FULL_TOKEN_OWNER_ALLOWED"
        );
        _;
    }
    ...
    constructor(
        ...
        migrationTarget = CSNFT1155(_migrationAddress);
    }
    ...
    function disableMigrate() public onlyOwnerOrProxy {
        migrationTarget = CSNFT1155(address(0));
    }
    ...
    function migrate(Ownership[] memory _ownerships) public onlyOwnerOrProxy {
        CSNFT1155 _migrationTarget = migrationTarget;
        require(
            _migrationTarget != CSNFT1155(address(0)),
            "CSNFT1155#migrate: MIGRATE_DISABLED"
        );
        ...
        for (uint256 i = 0; i < _ownerships.length; ++i) {
            ...
            require(
                owner != address(0),
                "CSNFT1155#migrate: ZERO_ADDRESS_NOT_ALLOWED"
            );
    ...
    function batchMint(
        ...
        for (uint256 i = 0; i < _ids.length; i++) {
            require(
                ...
                "CSNFT1155#_batchMint: ONLY_CREATOR_ALLOWED"
            );
        }
        _batchMint(_to, _ids, _quantities, _data);
    }
    ...
    function setCreator(uint256 _id, address _to) public creatorOnly(_id) {
        require(
            _to != address(0),
            "CSNFT1155#setCreator: INVALID_ADDRESS."
        );
    ...
    function _requireMintable(address _address, uint256 _id) internal view {
        require(
            ...
            "CSNFT1155#_requireMintable: ONLY_CREATOR_ALLOWED"
        );
    }
