...
contract WeiDragons is ERC721, Ownable, ReentrancyGuard {
  using Strings for uint256;

  //collection details
  string public _collectionName= "Wei Dragons";
  string public _collectionSymbol="DWEI";
  ...
  //mint details
  uint256 public cost = 222 ether;
  uint256 public maxSupply = 222;
  uint256 public maxMintAmount = 222;
  ...
  constructor() ERC721(_collectionName, _collectionSymbol)
   {
    setNotRevealedURI("ipfs://QmeQ8EigX6ZfruRUnVxjPL4XDaxUAAsuhoYQGe9b98EFje/HiddenMetadata.json");
...
if (msg.sender != owner()) {
  ...
  function airdropTo(address[] memory addressList, uint256[] memory amountRecievedList) external payable onlyOwner{
     ...
     //does not require less than max collection amount be cautious to output max (maxsupply-amountMinted)
