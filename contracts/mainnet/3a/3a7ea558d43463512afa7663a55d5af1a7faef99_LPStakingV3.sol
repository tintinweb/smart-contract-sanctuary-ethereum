...
contract LPStakingV3 is Initializable, ReentrancyGuardUpgradeable, OwnableUpgradeable {
    ...
    function stake(
        ...
        (, , address token0, address token1, , , , uint128 amount, , , , ) = uniV3.positions(tokenId);
        //check for ETH/MPWR pool pair
        require(
            (address(WETH) == token0 && pools[_poolId].reward == token1) ||
                (address(WETH) == token1 && pools[_poolId].reward == token0),
            "Invalid LP pool"
        );
