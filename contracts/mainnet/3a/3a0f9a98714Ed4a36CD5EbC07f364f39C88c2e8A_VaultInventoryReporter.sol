 ...
 * The item hash encodes the token address and token ID, with the token ID
 * being overwritten to 0 for ERC20 tokens. This ensures that repeated
 * reporting of the same ERC20 token can update the stored amount on the same
 * hash, and repeated reporting of ERC721 tokens with a different amount
 * will not add a new item to the enumeration. With this approach, the
 * enumeration is guaranteed to only contain unique entries for a single
 * ERC20 token, and for a single token ID of a given ERC721, ERC1155,
 * or CryptoPunk.
 */
contract VaultInventoryReporter is IVaultInventoryReporter, VaultOwnershipChecker, EIP712, Ownable {
    ...
    function add(address vault, Item[] calldata items) public override validate(msg.sender, vault) {
        ...
        for (uint256 i = 0; i < numItems; i++) {
            ...
            // Overwrite irrelevant variables based on itemType
            if (item.itemType == ItemType.ERC_20) {
                item.tokenId = 0;
            } else if (
                item.itemType == ItemType.ERC_721 ||
                item.itemType == ItemType.PUNKS
            ) {
                item.tokenAmount = 1;
            }
    ...
    function _hash(Item memory item) internal pure returns (bytes32) {
        return keccak256(abi.encodePacked(item.tokenAddress, item.tokenId));
    }
