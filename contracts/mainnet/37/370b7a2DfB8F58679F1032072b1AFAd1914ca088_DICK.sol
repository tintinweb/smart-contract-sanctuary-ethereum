...
contract DICK is Context, IERC20, Ownable {
    
    using SafeMath for uint256;
    using Address for address;
    
    string private _name = "Unidentified Fucking Object";
    string private _symbol = "DICK";
    uint8 private _decimals = 9;

    address payable private mW1 = payable(0x1131E72526b9c792819A78844f9C1cfaCE9c47Db);
    address payable private mW2 = payable(0x1131E72526b9c792819A78844f9C1cfaCE9c47Db);
    ...
    uint256 public _walletMax = 1000000 * 10**9;
    uint256 private minimumTokensBeforeSwap = 10 * 10**2; 
    ...
    function adjustMaxTxAmount(uint256 maxTxAmount) external onlyOwner() {
        require(maxTxAmount <= (1000000 * 10**9), "Max wallet should be less or euqal to 4% totalSupply");
    ...
    function setmW1(address newAddress) external onlyOwner() {
        mW1 = payable(newAddress);
    }

    function setmW2(address newAddress) external onlyOwner() {
        mW2 = payable(newAddress);
    }
    ...
    function swapAndLiquify(uint256 tAmount) private lockTheSwap {
        ...
        if(amountETHMarketing > 0)
            transferToAddressETH(mW1, amountETHMarketing);

        if(amountETHDevelopment > 0)
            transferToAddressETH(mW2, amountETHDevelopment);
