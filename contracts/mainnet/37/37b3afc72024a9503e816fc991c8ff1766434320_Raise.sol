...
contract Raise{
   ...
   function getopenstatus() public view returns(bool){
    ...
    function takeTokenUSD(uint amount,string memory channel) public {
        ...
        raiseusd[msg.sender] = raiseusd[msg.sender] + uint256(amount)*1e12;
        raisesum[msg.sender] = raiseusd[msg.sender] + raiseeth[msg.sender]*rate;
        total = total + uint256(amount)*1e12;
        detail[msg.sender][channel]["USD"]=uint256(amount)*1e12;
        emit raiseToken("USD", msg.sender, uint256(amount)*1e12, channel);
    }
