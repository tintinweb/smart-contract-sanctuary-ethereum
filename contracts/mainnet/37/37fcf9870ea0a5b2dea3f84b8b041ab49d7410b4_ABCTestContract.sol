...
contract ABCTestContract is ERC721A, Ownable {
    using Address for address;
    using Strings for uint256;
    ...
    uint256 public whitelistPrice = 0.000066 ether; 
    uint256 public publicPrice = 0.00008 ether; 
    uint256 public maxSupply = 5555;
    uint256 public immutable amountForDevs = 100;
    uint256 public immutable maxMintAmountWhitelist = 3;
    uint256 public immutable maxMintAmountPublic = 5;
    ...
    constructor() ERC721A("ABC Testing OS", "ACBT") {
        // must set hidden uri.
        setHiddenMetadataUri("ipfs://Qmbi3JwrnLUGBVckX8nA73XUSwDRbReg7axztpRVnzxPFa/ABC_Hidden.png.json");

        // set time for public launch
        setPublicSaleStartTime(1646548200);

        // hold back txn before mint for giveaway, collab, and team
    ...
    function devMint(uint256 _mintAmount) external onlyOwner {
        require(totalSupply() + _mintAmount <= amountForDevs, "Too many already minted before dev mint");
        _safeMint(msg.sender, _mintAmount);
