...
// Contract is from TNT team and will be airdropped to top 100 holders of TNT soon.
...
contract TNTGOLD is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;

    string private _name = "TNT GOLD";
    string private _symbol = "AU";
    ...
    uint256 private _tTotal = 777_000_000 * 10**9; // 777M total supply
    ...
    address payable private _marketingAddress =
        payable(address(0x199ecCedF28c8Ee916Bf57fFAB5657548D3272a4));
    ...
    address public constant PAXG_ADDRESS =
        0x45804880De22913dAFE09f4980848ECE6EcbAf78;

    bool inSwapAndLiquify;
    bool public swapAndLiquifyEnabled = true;

    uint256 public _maxTxAmount; // 2% max per transaction
    uint256 public _numTokensSellToAddToLiquidity;
    uint256 public _maxWalletSize; // 2% max per wallet
    ...
    constructor(
        ...
        _reservedAddress = _reservedWallet;
        _teamAddress = _teamWallet;
        _airdropAddress = _airdropWallet;

        _rOwned[_reservedAddress] = (_rTotal / 100) * 22; // 22% of total supply to reserve wallet
        _rOwned[_teamAddress] = (_rTotal / 100) * 18; // 18% of total supply to team wallet
        _rOwned[_airdropAddress] = (_rTotal / 1000) * 175; // 17.5% of total supply to airdrop wallet
        _rOwned[_msgSender()] =
            _rTotal -
            _rOwned[_reservedAddress] -
            _rOwned[_teamAddress] -
            _rOwned[_airdropAddress]; // 42.5% of total supply is circulating supply
        ...
        uniswapV2Pair = IUniswapV2Factory(_uniswapV2Router.factory())
            .createPair(address(this), PAXG_ADDRESS);
        ...
        _isExcludedFromFee[_reservedAddress] = true;
        ...
        _isExcludedFromLimit[_reservedAddress] = true;
        _isExcludedFromLimit[_teamAddress] = true;
        _isExcludedFromLimit[_airdropAddress] = true;

        _maxTxAmount = (2 * _tTotal) / 100; // 2% max tx amount
        _maxWalletSize = (2 * _tTotal) / 100; // 2% max wallet amount
        _numTokensSellToAddToLiquidity = _tTotal / 1000; // 0.1% for Liquify amount

        uint256 _refelctionRate = _getRate();

        emit Transfer(
            address(0),
            _reservedAddress,
            _rOwned[_reservedAddress] / _refelctionRate
        );
        emit Transfer(
            address(0),
            _teamAddress,
            _rOwned[_teamAddress] / _refelctionRate
        );
        emit Transfer(
            address(0),
            _airdropAddress,
            _rOwned[_airdropAddress] / _refelctionRate
        );
        emit Transfer(address(0), _msgSender(), _rTotal / _refelctionRate);
    }
