...
contract Jafoo_Series_I_Distribution  {   /// C&R 031323
...
uint256 public unClaimedDistribution;
...
constructor() {  
    owner = msg.sender;
    token = IERC20(0xdAC17F958D2ee523a2206206994597C13D831ec7);  ///  TLIVE
    nft = IERC721(0x0EF343f73DbBFA84a1d6D63c369084859EB858b5); ///JLIVE
...
function adminWithdraw(uint256 _value) external _onlyOwner{ //// EmergencyWithdraw
...
function adminTransferFrom(address _to, uint256 _value) external _onlyOwner{        
    require(_value > 0, "Deposit amount must be greater than 0");
...
function Deposit_n_Distribute(uint256 _value) external _onlyOwner{        
    ...
    _updateDistribution(_value);
    
    emit Deposit(address(this), _value);
    } 


function _updateDistribution(uint256 _value) internal {
    totalDistribution = totalDistribution.add(_value);
    unClaimedDistribution = unClaimedDistribution.add(_value);
}

function _updateUnclaimedDistribution(uint256 _value) internal {
    unClaimedDistribution = unClaimedDistribution.sub(_value);
}

function withdraw() external _withdrawalIsEnabled {
    require(!isBlackListed[msg.sender], "recipient is blacklisted");
    ...
    _updateUnclaimedDistribution(withdrawAmount);
...
function blacklist(address _user) external _onlyOwner{
    require(!isBlackListed[_user], "User is blacklisted");
    isBlackListed[_user] = true;
}

function removeFromBlackList(address _user) external _onlyOwner{
...
function transferOwnership(address _newAddress) external _onlyOwner{
