  ...
  IERC721Enumerable
{
  ...
  */
   ...
   * by default, can be overriden in child 
   */
...
abstract contract AmberHardERC721A is 
    Ownable,
    ERC721A,
    Withdrawable,
    ReentrancyGuard  , TimedDrop {
    constructor(
    ...
    string public _baseTokenURI = "ipfs://QmbksLtdi1yDJdHcPgVBc5cLyGGs5j4rMq4a4WXFFZu2XG/";

    bool public mintingOpen = true;

    uint256 public constant NUMBER_OF_SUPPLY_APPLYING_PRICE1 = 2500;

    uint256 public PRICE1 = 0 ether;
    
    uint256 public PRICE2 = 0.005 ether;
    ...
    */
    function mintToAdmin(address _to) public onlyOwner {
        require(getNextTokenId() <= collectionSize, "Cannot mint over supply cap");
    ...
    */
    function mintTo(address _to) public payable {
        ...
        require(msg.value >= getPrice(1), "Value below required mint fee");
    ...
    */
    ...
    function setPrice(uint256 _feeInWei1, uint256 _feeInWei2) public onlyOwner {
        PRICE1 = _feeInWei1;
        PRICE2 = _feeInWei2;
    }

    function getPrice(uint256 _count) private view returns (uint256) {
        if(totalSupply() + _count <= NUMBER_OF_SUPPLY_APPLYING_PRICE1){
            return PRICE1.mul(_count);
        } 

        return PRICE2.mul(_count);
    }
...
// File: AmberHardContract.sol

pragma solidity ^0.8.0;

contract AmberHardContract is AmberHardERC721A {
    constructor() AmberHardERC721A("AmberHard", "AmbH"){}

    function contractURI() public pure returns (string memory) {
      return "https://amber-hard.ams3.digitaloceanspaces.com/contract-metadata.json";
    }
}