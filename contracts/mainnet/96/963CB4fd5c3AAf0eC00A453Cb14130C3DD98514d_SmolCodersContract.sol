...
// https://smolcoders.com/ December 20th, 2022.
...
contract SmolCodersContract is ERC721A, ERC721AQueryable, Ownable, OperatorFilterer {
    ...
    constructor() ERC721A("SmolCoders", "SMOL") OperatorFilterer(address(0x3cc6CddA760b79bAfa08dF41ECFA224f810dCeB6), true) {}

    function mintAllowlist(bytes32[] calldata _proof, uint256 nMints) external validTxn(nMints) {
        ...
        if (nMints > MAX_ALLOWLIST_MINTS) revert ExceedsAllowlistLimit();
    ...
    function mintPublic(uint256 nMints) external payable validTxn(nMints) {
        ...
        if (msg.value != publicMintPrice * nMints) revert IncorrectAmountSent();
        _mint(msg.sender, nMints);
        emit Minted(msg.sender, nMints);
    }
    ...
    function airdrop(address _to, uint256 _amount) external onlyOwner validTxn(_amount) {
        _mint(_to, _amount);
    }
