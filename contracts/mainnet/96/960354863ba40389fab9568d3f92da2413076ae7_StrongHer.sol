...
contract StrongHer is ERC721, Ownable {
    using Strings for uint256;
    ...
    function mint(uint256 _amount) external payable {
        ...
        require(
            ...
            "StrongHer - max NFT limit exceeded"
        );
        if (msg.sender != owner()) {
            require(
                _amount <= maxMintAmount,
                "StrongHer - max mint amount limit exceeded"
            );
            uint256 ownerMintedCount = addressMintedBalance[msg.sender];
            require(
                ownerMintedCount + _amount <= maxMintAmount,
                "StrongHer - max NFT per address exceeded"
            );
            require(
                msg.value >= cost * _amount,
                "StrongHer - insufficient ethers"
            );
        }
