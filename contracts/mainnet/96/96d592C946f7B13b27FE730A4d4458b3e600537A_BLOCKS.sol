...
contract BLOCKS is ERC721AQueryable, Ownable {
  using Strings for uint256;

  uint256 public TOTAL_MAX_SUPPLY = 2660;
  uint256 public totalFreeMints = 440;
  ...
  constructor() ERC721A("GPTBLOCKS", "BLOCKS") {}

  modifier callerIsUser() {
    require(tx.origin == msg.sender, 'BLOCKS: The caller is another contract');
    _;
  }
  ...
  function mint(uint256 _quantity) external payable callerIsUser underMaxSupply(_quantity) {
    require(balanceOf(msg.sender) < maxPublicMintPerWallet, "GPTBLOCKS: Mint limit exceeded");
    require(saleStarted, "GPTBLOCKS: Sale has not started");
    if (_totalMinted() < (TOTAL_MAX_SUPPLY)) {
      if (freeMintCount >= totalFreeMints) {
        require(msg.value >= _quantity * publicTokenPrice, "BLOCKS: More ETH required");
        ...
        if (_quantity <= _mintableFreeQuantity) {
          ...
          require(
            ...
            "BLOCKS: Not enough ETH"
          );
        }
        _mint(msg.sender, _quantity);
      } else {
        require(msg.value >= (_quantity * publicTokenPrice), "BLOCKS: Not enough ETH");
        _mint(msg.sender, _quantity);
      }
    }
  }
  ...
  function withdrawFunds() external onlyOwner {
    ...
    require(success, "BLOCKS: Transfer failed");
  }
