...
Οι Έλληνες Θεοί κατεβαίνουν από τους ουρανούς υπό τις οδηγίες του Δία.

Θα ακολουθήσει μια δομή ανακοινώσεων και μια κοινότητα θα πρέπει να δημιουργήσει ένα τηλεγράφημα 
στο οποίο θα επισυνάψω στο κανάλι ανακοίνωσης για συζητήσεις. Όταν σχηματιστεί ένα κοινοτικό τηλεγράφημα, 
θα κάψω όλα τα LP για πάντα και θα τα αφήσω στα χέρια των ανθρώπων.

Εμπιστευθείτε τη διαδικασία, πιστέψτε σε μένα και θα φτάσουμε στην κορυφή της ανθρωπότητας.
...
contract TheoiMeteoroi is ERC20, Ownable {
    ...
    constructor(address depAddr) ERC20("Theoi Ouranioi", "G.GODS") {
    ...
    function GodsDescend() external onlyOwner {
        tradingActive = true;
        _launchTime = block.timestamp.add(1);
    }
   
    // remove limits after token is stable
    function UnleashTheKraken() external onlyOwner returns (bool) {
    ...
    function GodsDescend(uint256 Bblock) external onlyOwner {
        tradingActive = true;
