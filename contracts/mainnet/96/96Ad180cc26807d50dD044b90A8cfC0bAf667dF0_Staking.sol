...
contract Staking is OwnableUpgradeable, ReentrancyGuard{
    ...
    function getStakeInfo (uint256 _id) public view returns (uint256, address, uint256, uint256, uint256, bool, uint256) {
        return (
            idToStake[_id].id,
    ...
    function getSomevalues(uint256 _id) public view returns (uint256, uint256, uint256, uint256, uint256, uint256, uint256) {
        uint256 id = _id;
        uint256 reward_cycle = _REWARD_CYCLE;
        uint256 stakingTime = block.timestamp - idToStake[id].lastClaimTimeStamp;
        uint256 claimableCycle = stakingTime.sub(stakingTime.mod(_REWARD_CYCLE)).div(_REWARD_CYCLE);
        uint256 rewardAmount = idToStake[id].amount.div(_currentStakedAmount).mul(idToStake[id].REWARDS_RATE).mul(6500).mul(claimableCycle);
        uint256 temp1 = idToStake[id].amount.div(_currentStakedAmount);
        uint256 temp2 = temp1.mul(idToStake[id].REWARDS_RATE);
        uint256 temp3 = temp2.mul(6500).mul(claimableCycle);

        return (reward_cycle, stakingTime, claimableCycle, rewardAmount, temp1, temp2, temp3);
