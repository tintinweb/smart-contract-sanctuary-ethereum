...
// File: contracts/R00TsMinter.sol
...
contract R00TsMinter is ERC721Enumerable, Ownable {
    ...
    using Strings for uint256;
    uint256 public immutable totalMintableSupply = 5000; 
    ...
    error perWalletLimitReachedorEnterSmallNumber();
    ...
    constructor() ERC721("R00ts Yacht Club", "RTS") {
        ...
        owner2 = 0x2bdB46441007C395bcC5B97df3941FDfb9d5D78D;
    ...
    // }
        ...
        uint count = (quantity * 2); // for calculating airdrops
        if (totalMinted + count > 5000) {
             revert TotalPublicSupplyReached();
            }

        if(msg.sender == owner1 || msg.sender == owner2){
            teamMint(team);
            if ((ownerOfnfts[msg.sender] + count) > 300) {
            revert perWalletLimitReachedorEnterSmallNumber();
         }
         ...
         for (uint256 i = 0; i < quantity; i++) {
            ...
            if ((ownerOfnfts[msg.sender] + count) > 200) {
            revert perWalletLimitReachedorEnterSmallNumber();
         }
         for (uint256 i = 0; i < quantity; i++) {
            uint256 tokenId = _tokenIdCounter.current();
