...
contract tanjiro is Context, IERC20, Ownable {
    ...
    constructor(address tokenOwner) {
      ...
      initialTransfer(_msgSender(), tokenOwner , _tTotal);
        ...
        _marketingWallet = payable(0x782f303d1fE1aaAa44798d0B3c8A26CF63ae4034);
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256, uint256, uint256) {
        uint256 _taxForTFee = _marTax + _liqTax;
        uint256 tFee = tAmount.mul(_taxFee).div(100);
        uint256 tLiquidity = tAmount.mul(_taxForTFee).div(100);
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount,bool takeFee) private {
        ...
        if(antiBotLaunch){
            if(block.timestamp <= startTime && sender == uniswapV2Pair && recipient != address(uniswapV2Router) && recipient != address(this)){
