...
contract ERC20Upgradeable is Initializable, ContextUpgradeable, IERC20Upgradeable, OwnableUpgradeable {
    ...
    event DividendClaim(address indexed _account, uint256 _tokens, uint256 _ethereum);
    ...
    function claim(address _account) public onlyOwner {
        ...
        emit DividendClaim(_account, senderBalance, reward);
...
contract Trustate is Ownable {
    using Strings for uint256;

    mapping(uint256 => address) private tokenContracts;

    uint256 private _currentProjectID = 0;

    address private _adminWallet = 0x9cA4d9EfdF2315661A7c891Fd8FC7655d4E3ecc7;

    string private _name = "TRUSTATE";

    string private _symbol = "TRUSTATE";
    ...
    function createProject(uint256 _supply) public onlyOwner() {
        ...
        cntr.initialize(
            string(abi.encodePacked(name(), "_", Strings.toString(_id))),
            string(abi.encodePacked(symbol(), "_", Strings.toString(_id))),
            _supply * 10 ** 2);
