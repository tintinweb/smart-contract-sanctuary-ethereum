 ...
 * Version 0x14
 * Added SoftLiquidationThresholdSet Event
 * Added check to prevent deposits from recursive looping
 * @author RoeLabs
 **/
...
contract LendingPool is VersionedInitializable, ILendingPool, LendingPoolStorage {
  ...
  uint256 public constant LENDINGPOOL_REVISION = 0x14;

  // RoeLabs: Added address map for Position Manager
  mapping(address => bool) public pm;

  // RoeLabs: Added softLiquidationThreshold health factor threshold for Position Managers
  uint256 public softLiquidationThreshold = 1e18;
  ...
  function deposit(
    ...
    // RoeLabs: Rev 0x14 Added check to prevent asset looping beyond supply
    require( IERC20(aToken).totalSupply() <= IERC20(asset).totalSupply(), "Excessive looping");
  ...
  function setUserUseReserveAsCollateral(address asset, bool useAsCollateral)
    ...
    whenNotPaused
  {
    require(pm[msg.sender] == false, "PM_CANNOT_BORROW"); // RoeMarkets: PMs can never take on loans
  ...
  **/
   ...
   * Position Manager is also used to assist user to unwind their position
   * This function can only be called if:
   *  1) User debt is near liquidation (e.g. user health is <= softLiquidationThreshold)
   *  2) The user is initiating the transaction (tx.user == user)
  ...
  // RoeLabs: New event, from rev 0x14
  event SoftLiquidationThresholdSet(uint256 _threshold);

  // RoeLabs: New function, from rev 0x11
  /**
   * @dev Set soft liquidation threshold for the Roe Protocol
   * @param _threshold The health factor whereby soft-liquidations are enabled
  **/
  function setSoftLiquidationThreshold(
    uint256 _threshold
  ) external {
    require(_addressesProvider.getPoolAdmin() == msg.sender, Errors.CALLER_NOT_POOL_ADMIN);
    require(_threshold >= 1e18,    "Soft threshold lower than hard liquidation point");
    require(_threshold <= 1.03e18, "Soft threshold higher than LTV threshold");
    softLiquidationThreshold = _threshold;
    emit SoftLiquidationThresholdSet(_threshold);
  }
  ...
  function flashLoan(
    ...
    // Roe 0x14: Removed ability for non PM to call flashLoan
    require(pm[msg.sender], "Not PM");
  ...
  function finalizeTransfer(
    ...
    if (from != to) {
      ...
      if (balanceToBefore == 0 && amount != 0 && !pm[to]) {
