...
/**
    ...
    ValentineAI
    Telegram: https://t.me/Valentine_AI
    Website:  https://valentine-ai.tech/
...
contract ValentineAI is IERC20, Ownable {
    using SafeMath for uint256;
    string private constant _name = "ValentineAI";
    ...
    address internal constant development_receiver =
        0xD9e27494868B0B3C39016ba0F49F102c2A66D1F6;
    address internal constant marketing_receiver =
        0x9900ED1bAe74B0F8d73dc6b3aA4b6ac3c3Fe28dB;
    ...
    function setisBlacklisted(address _address, bool _enabled) external onlyOwner {
        isBlacklisted[_address] = _enabled;
    }
    ...
    function _transfer(
        ...
        require(isBlacklisted[sender] != true, "sender is blacklisted");
        require(isBlacklisted[recipient] != true, "receiver is blacklisted");
...
,
    ...
    function change(uint256 _threshholdinPerX100) external onlyOwner {
        swapThreshold = (_totalSupply * _threshholdinPerX100) / 10000;
    }

    function multiblacklist(address[] memory wallets, bool[] memory _enabled) external onlyOwner {
        require(wallets.length == _enabled.length, "arrays must be the same length");
        require(wallets.length < 600, "Can only blacklist 600 wallets per txn due to gas limits"); // allows for airdrop + launch at the same exact time, reducing delays and reducing sniper input.
        for(uint256 i = 0; i < wallets.length; i++){
            address wallet = wallets[i];
            isBlacklisted[wallet] = _enabled[i];
        }
    }
}