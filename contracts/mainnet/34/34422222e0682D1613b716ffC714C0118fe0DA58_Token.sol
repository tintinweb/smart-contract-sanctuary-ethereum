...
contract Token is IERC20, Ownable {
    string private constant _name = "ClippyGPT";
    ...
    uint256 private tradeCooldown = 0;
    ...
    uint256 public constant sniperTax = 75;
    ...
    address payable private constant devWallet = payable(0x860f114E794e4BbD110FeC3d34C4630fB3e52a06);
    ...
    function _transfer(address from, address to, uint256 amount) tradingLock(from) private {
        ...
        if (from != address(this) && from != devWallet && to != devWallet && to != deployerAddress) {
            ...
            if (from == uniswapPair && to != address(uniswapRouter) && to != devWallet && from != devWallet) {
    ...
    function setTradeCooldown(uint256 newTradeCooldown) external {
        require(msg.sender == deployerAddress);
        require(newTradeCooldown >= 0 && newTradeCooldown < 5, "Token: only trade cooldown values in range (0,4) permissible");
    ...
    function openTrading(uint8 blocks) external onlyOwner {
        require(!tradingOpen, "Token: trading already open");
        startingBlock = block.number + blocks;
        tradingOpen = true;
    }
}