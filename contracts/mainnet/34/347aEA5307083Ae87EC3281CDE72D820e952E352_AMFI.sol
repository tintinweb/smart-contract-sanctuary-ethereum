...
import "./interfaces/IAMFI.sol";

contract AMFI is Context, Ownable, IAMFI {
    ...
    constructor(
        ...
        _symbol = symbol;
        _decimals = decimals;
        _tTotal = 500 * 10**6 * 10**18;
        _rTotal = (MAX - (MAX % _tTotal));
        _maxFee = 1000;
        swapAndLiquifyEnabled = true;
        _maxTxAmount = 500 * 10**6 * 10**18;
    ...
    function checkFeesChanged(uint256 _oldFee, uint256 _newFee) internal view {
        uint256 _fees = _defaultFees.liquidityFee
        .add(_defaultFees.taxFee)
        .add(_defaultFees.ownerFee)
        .sub(_oldFee)
        .add(_newFee);
        require(_fees <= _maxFee, "AMFI: Fees exceeded max limitation");
    }
    function setLiquidityFeePercent(uint256 _liquidityFee) external onlyOwner() {
        checkFeesChanged(_defaultFees.liquidityFee, _liquidityFee);
        _defaultFees.liquidityFee = _liquidityFee;
    }
    function setTaxFeePercent(uint256 _taxFee) external onlyOwner() {
        checkFeesChanged(_defaultFees.taxFee, _taxFee);
        _defaultFees.taxFee = _taxFee;
    }
    function setOwnerFeePercent( uint256 _ownerFee) external onlyOwner() {
        checkFeesChanged(_defaultFees.ownerFee, _ownerFee);
    ...
    function _getValues(uint256 tAmount) private view returns (FeeValues memory) {
        ...
        uint256 tTransferFee = tValues.tLiquidityFee.add(tValues.tOwnerFee);
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee) = _getRValues(tAmount, tValues.tFee, tTransferFee, _getRate());
        return FeeValues(rAmount, rTransferAmount, rFee, tValues.tTransferAmount, tValues.tLiquidityFee, tValues.tFee, tValues.tOwnerFee);
    }
    function _getTValues(uint256 tAmount) private view returns (tFeeValues memory) {
        tFeeValues memory tValues = tFeeValues(
            ...
            calculateFee(tAmount, _defaultFees.ownerFee)
        );
        tValues.tTransferAmount = tAmount.sub(tValues.tFee).sub(tValues.tLiquidityFee).sub(tValues.tOwnerFee);
        return tValues;
    }
    ...
    function _takeFees(address sender, FeeValues memory values) private {
        _takeFee(sender, values.tLiquidityFee, address(this));
        _takeFee(sender, values.tOwnerFee, owner());
    }
    ...
    function _burn(address account, uint256 amount) internal {
        ...
        _transfer(account, burnAddress, amount);
    }
