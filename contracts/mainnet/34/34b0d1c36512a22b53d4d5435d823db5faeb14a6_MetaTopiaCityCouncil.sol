...
contract MetaTopiaCityCouncil is ERC721A, Ownable, ReentrancyGuard {
    ...
    bool public mintWithETH = true;
    bool public mintWithToken;
    ...
    modifier mintPriceCompliance(uint256 _mintAmount) {
        if(_mintAmount == 10 && discountEnabled) {
            require(msg.value == cost * 9, "Incorrect payment amount");
        } else {
    ...
    modifier tokenMintPriceCompliance(uint256 _mintAmount, uint256 _tokenamount) {
        if(_mintAmount == 10 && discountEnabled) {
            require(_tokenamount == tokenCost * 9, "Incorrect payment amount");
        } else {
    ...
    function setDiscountEnabled(bool _discountEnabled) external onlyOwner {
