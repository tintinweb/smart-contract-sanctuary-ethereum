...
contract ExchangeWithOrionPool is Exchange, IPoolSwapCallback {
    ...
    function swapThroughOrionPool(
        ...
        bool isCheckPosition = LibPool.doSwapThroughOrionPool(
            LibPool.SwapData(amount_spend, amount_receive, is_exact_spend, path, _orionpoolRouter, false, false, false, address(0)),
...
library LibPool {
    ...
    function calcAmounts(
        ...
        console.log("isFromWallet %s %s %s", d.isFromWallet, d.isInContractTrade, uint(assetBalances[d.order.senderAddress][d.asset_spend]));
        console.log("amount_spend %s", uint(d.amount_spend));
    ...
    function calcAmountInOutAfterSwap(
        ...
        if (d.isRetainFee) {
            ...
            console.log("isRetainFee:171 %s %s", d.amountIn, actualFee);
            // Pay to matcher
...
library LibExchange {
    ...
    function creditUserAssets(uint tradeType, address user, int amount, address asset,
        ...
        console.log("Credit %s %s %s", uint(toUpdate), uint(sentAmount), uint(amount));
        if (toUpdate != 0) {
