...
contract ERC721A is Context, ERC165, IERC721, IERC721Metadata {
    ...
    function _burn(uint256 tokenId) internal virtual {
        ...
        //TODO
        // Si burn, mint d'un autre NFT (URL différent, JSON etc... New collection)
...
contract KOPOKOMafiaPet is ERC721A, Ownable, ReentrancyGuard {
    ...
    //URI of the NFTs when token is burn
    string private burnURI;
    ...
    mapping(uint256 => mapping(address => bool)) userExists;

    //Keep a track of the nft burn
    mapping(uint256 => bool) burnNFT;
    
    //Constructor of the collection
    constructor() ERC721A("KPK Pet", "KPKPet") {   
    ...
    **/
    function init(string memory _theBaseURI, string memory _notRevealedURI,string memory _burnURI) external onlyOwner {
        baseURI = _theBaseURI;
    ...
    **/
    ...
    /**
    * @notice Change the burn URI
    *
    * @param _burnURI The new burn URI
    **/
    function seturnURI(string memory _burnURI) external onlyOwner {
        burnURI = _burnURI;
    }
    ...
    *
    * @param tokenId Token of your NFT
    **/
    function burn(uint256 tokenId) external {
        require(ownerOf(tokenId) == msg.sender, "You are not the owner of the NFT");
        _burn(tokenId);
        burnNFT[tokenId] = true;

    }

    /**
    * @notice Send one NFT as a gift to all holders
    *
    * @param holders The accounts of the happy new owner of NFTs
    **/
    function giftForAll(address[] memory holders) external onlyOwner {
        require(holders.length > 0, "no holders");
        uint supply = totalSupply();
        for (uint256 index = 0; index < holders.length; index++) {
            require(supply + 1 <= MAX_SUPPLY, "Sold out");
           
                _safeMint(holders[index], 1);
                nftsPerWallet[holders[index]] += 1;
