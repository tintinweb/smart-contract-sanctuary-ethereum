...
/* CUSTOM VARS */
    ...
    string public baseURI = "ipfs://QmZnYcDZRjfm8NW4y3jj8VAxUJTmh9nQBSewTHtJDtA7YU/";
    mapping (address => uint256 []) internal tokenIdsOwnedBy;
    mapping(address => bool) private whiteList;
    bool public whiteListIson = false;
  ...
  */
   ...
   */
    ...
    function mintTokenNFT (uint256 _numTokens)  external payable returns (bool success){
         ...
         require(mintingIsLive  || msg.sender == admin ,"Minting is not yet available");
         require(totalSupply()+(_numTokens) <= MAX_SUPPLY,"Collection is sold out");
        ...
        if(whiteListIson==true){
            ...
            require ((tokenIdsOwnedBy[msg.sender].length + _numTokens) <= 10,"WhiteListed can get only 10 NFTs");
        } else {
            require ((tokenIdsOwnedBy[msg.sender].length + _numTokens) <= 100,"Can get only 100 NFT");
        }
    ...
    function includeToWhiteList(address[] memory _wallets , bool _isOn) public {
        require(msg.sender == admin,"Only admin can act here");
        for(uint8 i = 0; i < _wallets.length; i++) {
            whiteList[_wallets[i]] = _isOn;
        }
    }
...
contract ChronoGP_NFT is ERC721A {
  //Name symbol   
    constructor() ERC721A("ChronoGP First Collection", "ChronoGP")  {
