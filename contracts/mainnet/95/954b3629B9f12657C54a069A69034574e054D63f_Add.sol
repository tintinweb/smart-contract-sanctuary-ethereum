...
contract Add is Initializable, ERC20Upgradeable, PausableUpgradeable, OwnableUpgradeable, ERC20PermitUpgradeable, ERC20VotesUpgradeable, UUPSUpgradeable {
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal override(ERC20Upgradeable){      
		...
		if (!swapping && canSwap && swapEnable && automatedMarketMakerPairs[recipient]) 
		{
			...
			uint256 totalFee = stakingPoolFee[1] + DAOFee[1] + marketingFee[1];
			
			uint256 treasuryPart = newBalance * DAOFee[1] / totalFee;
			uint256 marketingPart = newBalance * marketingFee[1] / totalFee;
			uint256 stakingPart = newBalance - treasuryPart - marketingPart;
			
			if(newBalance > 0)
			{
			   (bool stakingSuccess, ) = staking.call{value: stakingPart}("");
			   (bool treasurySuccess, ) = treasury.call{value: treasuryPart}("");	
			   (bool marketingSuccess, ) = marketing.call{value: marketingPart}("");
			   require(marketingSuccess && treasurySuccess && stakingSuccess, "Address: unable to send value, recipient may have reverted");
			}
			swapping = false;
		}
		...
		else 
		{
            ...
            super._transfer(sender, address(this), _DAOfee.add(_stakingFee).add(_marketingFee));
