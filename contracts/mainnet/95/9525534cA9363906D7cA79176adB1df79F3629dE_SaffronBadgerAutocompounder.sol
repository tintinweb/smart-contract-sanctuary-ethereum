...
contract SaffronBadgerAutocompounder is ReentrancyGuard {
  ...
  function _withdraw_rewards(address[] calldata tokens, uint256[] calldata cumulativeAmounts) internal {
    ...
    // Set the cumulative amount for each reward token
    ...
    // Withdraw the bTokens for underlying tokens: Aura BAL, AURA, bb-a-USD if the withdraw amount
    // is greater than or equal to the REWARD_WITHDRAW_MIN
    if (bauraBAL_amount >= REWARD_WITHDRAW_MIN) {
      bAuraBal.withdraw(bauraBAL_amount);
    }
    if (graviAURA_amount >= REWARD_WITHDRAW_MIN) {
      bGravAura.withdraw(graviAURA_amount);
    }
    if (bbbaUSD_amount >= REWARD_WITHDRAW_MIN) {
