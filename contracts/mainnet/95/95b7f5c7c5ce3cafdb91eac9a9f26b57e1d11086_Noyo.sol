...
// Supply: 5555
// 3000 free, 2 max/tx & wallet
// 0.003 ETH, 10 max/tx & wallet
...
contract Noyo is ERC721A, Ownable, ReentrancyGuard {
    string public baseURI = "ipfs://Noyo/";
    ...
    function mint(uint256 count) external payable {
        ...
        if (isFree) {
            ...
            require(totalSupply() + count <= maxSupply, "No more nerds.");
            require(count <= maxPerTx, "Max per TX reached.");
            _mintedFreeAmount[msg.sender] += count;
        }
        else{
        require(!paused, "The contract is paused!");
        require(msg.value >= count * cost, "Send the exact amount: 0.004*(count)");
        require(totalSupply() + count <= maxSupply, "No more nerds.");
        require(count <= maxPerTx, "Max per TX reached.");
        }
