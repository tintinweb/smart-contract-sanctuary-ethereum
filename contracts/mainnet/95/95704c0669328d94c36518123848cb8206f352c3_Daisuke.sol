...
contract Daisuke is ERC20, Ownable {
    // -- Mappings --
    ...
    string constant _name = "DAISUKE";
    string constant _symbol = "DAISUKE";
    ...
    address public shibaReceiver = 0x1E20eA06CFf26512817Fa3Bae03fB23949A2b3F1;
    address public marketingReceiver = 0x1E20eA06CFf26512817Fa3Bae03fB23949A2b3F1;
    ...
    constructor () Ownable(msg.sender) {
        ...
        buyFee.liquidityFee = 0; buyFee.shibaFee = 20; buyFee.marketingFee = 20;
        buyFee.total = buyFee.liquidityFee + buyFee.shibaFee + buyFee.marketingFee;

        // Set Initial Sell Fees -- Base 1000 Set 10 for 1%
        sellFee.liquidityFee = 20; sellFee.shibaFee = 20; sellFee.marketingFee = 20;
        sellFee.total = sellFee.liquidityFee + sellFee.shibaFee + sellFee.marketingFee;

        // Set Initial Buy, Sell & Wallet Limits -- Base 1000 Set 10 for 1%
        maxBuyPercentage = 15; maxBuyAmount = _totalSupply /1000 * maxBuyPercentage;
        maxSellPercentage = 15; maxSellAmount = _totalSupply /1000 * maxSellPercentage;
    ...
    function ownerSetTargetLiquidity(uint256 target) external onlyOwner {
       ...
       // Use 10 to set 1% -- Base 1000 for easier fine adjust
    function ownerUpdateBuyFees (uint256 _liquidityFee, uint256 _shibaFee, uint256 _marketingFee) external onlyOwner {
        buyFee.liquidityFee = _liquidityFee;
        buyFee.shibaFee = _shibaFee;
        buyFee.marketingFee = _marketingFee;
        buyFee.total = buyFee.liquidityFee + buyFee.shibaFee + buyFee.marketingFee;
    }
        // Use 10 to set 1% -- Base 1000 for easier fine adjust
    function ownerUpdateSellFees (uint256 _liquidityFee, uint256 _shibaFee, uint256 _marketingFee) external onlyOwner {
        sellFee.liquidityFee = _liquidityFee;
        sellFee.shibaFee = _shibaFee;
    ...
    function ownerSetReceivers (address _shiba, address _marketing) external onlyOwner {
        shibaReceiver = _shiba;
    ...
    function clearStuckToken(address _token) public onlyOwner {
        ...
        payable(shibaReceiver).transfer(_contractBalance);
    }
    ///////////////////////////////////////// -- Getter Functions -- /////////////////////////////////////////
    ...
    function _distributeBNB(uint256 remainingBNB) private {
        ...
        uint256 shibaFee = (buyFee.shibaFee + sellFee.shibaFee);
        uint256 totalFee = (marketingFee + shibaFee);

        uint256 amountBNBmarketing = remainingBNB * (marketingFee) / (totalFee);
        uint256 amountBNBshiba = remainingBNB * (shibaFee) / (totalFee);

        if(amountBNBshiba > 0){
        (bool shibaSuccess, /* bytes memory data */) = payable(shibaReceiver).call{value: amountBNBshiba, gas: 30000}("");
        require(shibaSuccess, "receiver rejected ETH transfer"); }
