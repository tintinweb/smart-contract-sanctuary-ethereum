...
contract TheBastardAccess is ERC721A, EIP712, Ownable{
    ...
    uint256 public whitelistMintFee = 0.06 ether;
    ...
    constructor() 
    ERC721A("The Bastard Access", "TBA")
    EIP712("TBAC", "1.0.0"){
        _safeMint(0xdD7E9c3eb29e76E1F906f70F993292a8Db87BBD3,1);
    }
    ...
    function ogmint(bytes calldata _signature, uint256 _nftIndex, uint256 _quantity) external payable mintCompliance(){
        uint256 _mintedAmount = mintedAmount[msg.sender];
        require(msg.value >= OGMintFee * _quantity, "Invalid ETH");
    	require( !isClaimed(_nftIndex), "Whitelist Spot has been Claimed." );
        require(_verify(_whitelistHash(msg.sender, _nftIndex), _signature, false), "NFT: Invalid Claiming!");
		require(_mintedAmount + _quantity <= 1, "You already minted 1");
          _setClaimed( _nftIndex);		
        _mint( msg.sender, _quantity);
        mintedAmount[ msg.sender ] = _mintedAmount + _quantity;
 ...
 function publicmint(uint256 _quantity) external payable mintCompliance(){
             require(publicSale, "Public Mint Not Open");  
             require(msg.value >= whitelistMintFee * _quantity, "Invalid ETH");
                uint256 _mintedAmount = mintedAmount[msg.sender];
 ...
 function teammint(uint256 _quantity) external onlyOwner mintCompliance (){
    ...
    function tokenURI(uint256 _tokenId) public view virtual override returns (string memory) {
        ...
        if(revealed==false)
        {
            return "ipfs://bafybeiepbh42l4dz2bnmmxay7l2zwrfdgqrpthsj4lqhpkhvszmh6qcmum/0.json";
        }
