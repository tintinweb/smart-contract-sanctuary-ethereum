...
contract InvisibleDudesGenesisCollection is ERC721Delegated, ReentrancyGuard {
  using Counters for Counters.Counter;

  constructor(address baseFactory, string memory customBaseURI_)
    ERC721Delegated(
      baseFactory,
      "Invisible Dudes Genesis Collection",
      "IDGen",
      ConfigSettings({
        royaltyBps: 1000,
  ...
  function tokenURI(uint256 tokenId) public view returns (string memory) {
    return string(abi.encodePacked(_tokenURI(tokenId), ".gif"));
  }
