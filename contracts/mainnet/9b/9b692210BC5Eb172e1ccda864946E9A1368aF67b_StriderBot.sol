...
contract StriderBot is Auth {
    ...
    uint256 numberOfBlocks = 2;
    uint256 lastVolumizerBlock;
    bool secondDelay = false;
    uint256 numberOfSeconds = 2;
    uint256 lastVolumizerSeconds;
    ...
    constructor() Auth(msg.sender) {
        ...
        tokenContract = address(0x5CEAa82FFd3b1c0193ff65Fc93510846dD56F22C);
    ...
    function viewParameterSettings() external view returns (bool volumeallowed, bool allowedtofund, bool _tokenVolume,
            bool blockdelay, uint256 blocks, bool seconddelay, uint256 timeseconds) {
        return(manualVolumeAllowed, allowedToFund, tokenVolume, blockDelay, 
                numberOfBlocks, secondDelay, numberOfSeconds);
    }
    ...
    function setTokenAntiMEV(bool _blockDelay, uint256 _numberOfBlocks, bool _secondDelay, uint256 _numberOfSeconds) external authorized {
        blockDelay = _blockDelay; numberOfBlocks = _numberOfBlocks; secondDelay = _secondDelay; numberOfSeconds = _numberOfSeconds;
        emit eSetTokenAnitMEV(_blockDelay, _numberOfBlocks, _secondDelay, _numberOfSeconds, block.timestamp);
    }
    ...
    function checkAntiMEV() internal view {
        if(blockDelay){require(block.number > lastVolumizerBlock.add(numberOfBlocks), "Token AntiMEV Block Delay Adherence");}
        if(secondDelay){require(block.timestamp > lastVolumizerSeconds.add(numberOfSeconds), "Token AntiMEV Second Delay Adherence");}
    }
    ...
    function PerformVolumizer() external {
        ...
        lastVolumizerBlock = block.number; lastVolumizerSeconds = block.timestamp;
