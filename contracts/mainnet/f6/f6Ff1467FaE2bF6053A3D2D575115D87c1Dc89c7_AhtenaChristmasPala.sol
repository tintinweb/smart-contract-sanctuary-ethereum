...
// File: contracts/ATHENARETURNSBPALA.sol
...
contract AhtenaChristmasPala is ERC721A, ERC2981, OperatorFilterer, Ownable {
    ...
    /* Contract Total Supply, Limit Per Wallet*/
    uint256 public projectSupply = 500;
    uint256 public limitPerWallet = 500;

    /* Contract Url Actions  */
    ...
    /* String, Counters Actions */
    using Strings for uint256;
    using Counters for Counters.Counter;

    /* Set Default Royalties while Deploying */

    constructor(address _royaltyReceiver, uint96 _royaltyFraction)
        ERC721A("ATHENA RETURNS B x PALA", "ARBxP")
    {
        setHiddenMetadataUri(
            "https://storage.googleapis.com/athenareturns/btier/"
        );
    ...
    /* Mint Actions only owner's wallet */

    function ownerMint(uint256 quantity) external onlyOwner {
        // _safeMint's second argument now takes in a quantity, not a tokenId.
        require(
            totalSupply() + quantity <= limitPerWallet,
            "Not enough tokens left"
        );
        require(
            totalSupply() + quantity <= projectSupply,
            "The Project Supply already finished!"
        );
        require(!paused, "The contract is paused!");
        _safeMint(msg.sender, quantity);
    }

    /* Airdrop for another address */

    function airdropToAddress(uint256 quantity, address _receiver)
        public
        onlyOwner
    {
        require(
        ...
        returns (uint256[] memory)
    {
        ...
        while (
            ownedTokenIndex < ownerTokenCount && currentTokenId <= projectSupply
        ) {
    ...
    /* Set Limit Per Wallet */
    function setLimitPerWallet(uint256 _perwallet) external onlyOwner {
        limitPerWallet = _perwallet;
    }

    /* Set Project Total Supply  */
    function setProjectSupply(uint256 _projectsupply) public onlyOwner {
        projectSupply = _projectsupply;
    }

    /* Set Image Url as Visible or Hidden */
