...
contract GenerationalWealthSocietyTreasury is Ownable {
    ...
    enum MANAGING {RESERVEDEPOSITOR, RESERVESPENDER, RESERVETOKEN, RESERVEMANAGER, LIQUIDITYDEPOSITOR, LIQUIDITYTOKEN, LIQUIDITYMANAGER, DEBTOR, REWARDMANAGER, SGWS}
    ...
    address public sGWS;
    uint public sGWSQueue; // Delays change to sGWS address
...
//        isLiquidityToken[_GWSDAI] = true;
//        liquidityTokens.push(_GWSDAI);

        blocksNeededForQueue = _blocksNeededForQueue;
    }

    /**
        @notice allow approved address to deposit an asset for GWS
        @param _amount uint
    ...
    function deposit(uint _amount, address _token, uint _profit) external returns (uint send_) {
        ...
        // mint GWS needed and store amount of rewards for distribution
        send_ = value.sub(_profit);
        IERC20Mintable(GWS).mint(msg.sender, send_);
    ...
    /**
        @notice allow approved address to burn GWS for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw(uint _amount, address _token) external {
        ...
        uint value = valueOf(_token, _amount);
        IGWSERC20(GWS).burnFrom(msg.sender, value);
    ...
    function incurDebt(uint _amount, address _token) external {
        ...
        uint maximumDebt = IERC20(sGWS).balanceOf(msg.sender);
        // Can only borrow against sGWS held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with GWS
        @param _amount uint
     */
    function repayDebtWithGWS(uint _amount) external {
        require(isDebtor[msg.sender], "Not approved");

        IGWSERC20(GWS).burnFrom(msg.sender, _amount);

        debtorBalance[msg.sender] = debtorBalance[msg.sender].sub(_amount);
        totalDebt = totalDebt.sub(_amount);

        emit RepayDebt(msg.sender, GWS, _amount, _amount);
    }
    ...
    function mintRewards(address _recipient, uint _amount) external {
        ...
        IERC20Mintable(GWS).mint(_recipient, _amount);
    ...
    function excessReserves() public view returns (uint) {
        return totalReserves.sub(IERC20(GWS).totalSupply().sub(totalDebt));
    }
    ...
    /**
        @notice returns GWS valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOf(address _token, uint _amount) public view returns (uint value_) {
        if (isReserveToken[_token]) {
            // convert amount to match GWS decimals
            value_ = _amount.mul(10 ** IERC20(GWS).decimals()).div(10 ** IERC20(_token).decimals());
