...
contract PlaygroundCompatibleContract is ERC20, Ownable, IPlaygroundToken {
    ...
    address public feeReceiver1Address;
    address public feeReceiver2Address;
    ...
    uint256 public tax1Fee;
    uint256 public tax2Fee;
    ...
    event UpdatedFeeAddresses(address indexed newWallet, address indexed newWallet2);

    event UpdatedMaxTxnAmount(uint256 newAmount);

    event RemovedLimits();

    constructor() ERC20("D", "D") payable {
        ...
        tax2Fee = 5;
        liquidityFee = 5;
        totalFees = tax1Fee + tax2Fee + liquidityFee;

        feeReceiver1Address = address(msg.sender);

        whitelist(newOwner, true);
        whitelist(address(this), true);
        whitelist(address(0xdead), true);
        whitelist(address(feeReceiver1Address), true);
    ...
    function updateFees(uint256 _tax1Fee, uint256 _tax2Fee, uint256 _liquidityFee) external onlyOwner {
        tax1Fee = _tax1Fee;
        tax2Fee = _tax2Fee;
    ...
    function claimFees(uint256 amount, address token) external {
        ...
        uint256 amountForTax1Fee = amount * tax1Fee / totalFees;
        uint256 amountForTax2Fee = amount * tax2Fee / totalFees;
        if(amountForTax1Fee > 0){
            SafeERC20.safeTransfer(IERC20(token), feeReceiver1Address, amountForTax1Fee);
        }
        if(amountForTax2Fee > 0){
            SafeERC20.safeTransfer(IERC20(token), feeReceiver2Address, amountForTax2Fee);
        }
    }
    ...
    function setFeeReceivers(address _receiver1, address _receiver2) external onlyOwner {
        require(_receiver1 != address(0) && _receiver2 != address(0), "address cannot be 0");
        feeReceiver1Address = payable(_receiver1);
        feeReceiver2Address = payable(_receiver2);
        emit UpdatedFeeAddresses(feeReceiver1Address, feeReceiver2Address);
    }
