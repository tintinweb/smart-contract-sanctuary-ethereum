...
interface IMilkToken {
    ...
    function fragmentToMilk(uint256 value) external view returns (uint256);

    function milkToFragment(uint256 milk) external view returns (uint256);

    function balanceOfUnderlying(address who) external view returns (uint256);
}

contract MilkCarton is Ownable, ReentrancyGuard {
    ...
    // MILK
    IMilkToken public milk;
    // MILK LP address
    IUniswapPair public milkLp;
    // MILK tokens reward per block.
        ...
        returns (uint256)
    {
        ...
        if (address(pool.lpToken) == address(milk)) {
            lpSupply = milk.balanceOfUnderlying(address(this));
        }
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            uint256 milkReward = ((block.number - pool.lastRewardBlock) *
                rewardPerBlock *
                pool.allocPoint) / totalAllocPoint;
            accRewardPerShare += (milkReward * 1e12) / lpSupply;
        }
        return
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 milkReward = ((block.number - pool.lastRewardBlock) *
            rewardPerBlock *
            pool.allocPoint) / totalAllocPoint;
        pool.accRewardPerShare += (milkReward * 1e12) / lpSupply;
        pool.lastRewardBlock = block.number;
    ...
    // Deposit tokens to MilkChef for MILK allocation.
    function deposit(uint256 _pid, uint256 _amount) external nonReentrant {
        ...
        if (address(pool.lpToken) == address(milk)) {
            _amount = milk.fragmentToMilk(_amount);
        }
    ...
    // Withdraw tokens from MilkCarton.
    function withdraw(uint256 _pid, uint256 _amount) external nonReentrant {
        ...
        user.rewardDebt = (user.amount * pool.accRewardPerShare) / 1e12;
        if (address(pool.lpToken) == address(milk)) {
            _amount = milk.milkToFragment(_amount);
        }
