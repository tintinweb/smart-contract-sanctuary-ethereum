...
/// @title a ERC721 for recika ip info
/// @author recika
/// @notice a nft can verify by ipnetwork cmartcontract.

contract UniCaskSBT is
Initializable,
OwnableUpgradeable,
...
HasSecondarySaleFees
{
    ...
    string public constant defaultBaseURI = "http://wallet.recika.io:3334/token/";

    string public customBaseURI;

    mapping(uint256 => bytes32) private mataDataHashs;

    mapping(uint256 => bool) private nftOperPerm;
    ...
    function mint(address[] memory _toList, uint256[] memory _tokenIdList) public onlyOwner {
        ...
        for (uint256 i = 0; i < _tokenIdList.length; i++) {
            ...
            nftOperPerm[_tokenIdList[i]] = false;
            _approve(owner(), _tokenIdList[i]);
        }
    }
    ...
    function _beforeTokenTransfer(
        ...
        // nft can't Transfer or burn
        if (address(0) != from && owner() != msg.sender){
            require(nftOperPerm[tokenId], "This nft does not allow any operations.");
        }
    ...
    function contractURI() public view returns (string memory) {
        if (bytes(customBaseURI).length > 0) {
            return string(abi.encodePacked(customBaseURI, "contract"));
        } else {
            return
            string(
                abi.encodePacked(
    ...
    function setMetaDataHash(uint256 _tokenId, bytes32 _metaDataHash) public {
        require((super.ownerOf(_tokenId) == msg.sender), "Operateable: caller is not the owner of NFT");
        mataDataHashs[_tokenId] = _metaDataHash;
    }

    function getMetaDataHash(uint256 _tokenId) public view returns (bytes32)  {
        return mataDataHashs[_tokenId];
    }

    function allowNftOperation(uint256 _tokenId) public onlyOwner {
        require(_exists(_tokenId), "token must exist");
        nftOperPerm[_tokenId] = true;
    }

    function banNftOperation(uint256 _tokenId) public onlyOwner {
        require(_exists(_tokenId), "token must exist");
        nftOperPerm[_tokenId] = false;
    }

    function canNftOperation(uint256 _tokenId) public view returns(bool) {
        return nftOperPerm[_tokenId];
    }
