...
contract MOMOToken is ERC20, ERC20Burnable, Pausable, Ownable {
    using SafeMath for uint256;
    uint256 private _maxCap = 421000000000000 * 10**decimals();
    uint256 public constant D_ZERO = 10000;
    uint256 public sellTax; 
    uint256 public buyTax;
    address public feeWallet = 0x7a3b5671f6c67b2C56f4605b474C44AfB207f824;
    mapping(address => bool) public lps;
    mapping(address => bool) public blacklisted;
    mapping(address => bool) public whitelisted;
    event SetLiquidPair(address LP, bool Status);

    uint256 public prizePool = 64200000000000 * 10**18; // 
    uint256 public teamSupply = 256481000000000 * 10**18; 
    uint256 public initialMintAmount = 8942042000 * 10**18; 
    uint256 public investAmount = 894204200 * 10**18; // 2m
    uint256 public clipCost = 250000000 * 10**18; // 25000k
    uint256 public remainmintnum = 79990000000000 * 10**18; 
    // uint256 public value = 0.001 * 10**18; 


    uint256 public lastClipUpdate;
    address public clipOwner;
    address private _fund = 0x7a3b5671f6c67b2C56f4605b474C44AfB207f824;
    string public clip = "[email protected] CCTV";
    ...
    constructor() ERC20('MOMO CCTV TOKEN', 'MOMO') {
        sellTax = 500;
        buyTax = 500;
        _mint(address(this), _maxCap);
        _transfer(address(this), _fund, teamSupply);
        clipOwner = msg.sender;
        whitelisted[msg.sender] = true;
        whitelisted[_fund] = true;
    ...
    function setinitialMintAmount(uint256 amount) external onlyOwner {
    ...
    function mintClips(address invester) payable external {
        require( 
            block.timestamp >= lastMintTime[msg.sender] + 1 days,
            "You can only mint once every 24 hours"
        );
       require(msg.value == 0.001 ether, "Please send 0.001 ether as gas fee");
        uint256 mintAmount;
        ...
        require(
            ...
            "Not enough CLIPS left to mint"
        );
        lastMintValue[msg.sender] = mintAmount;
        lastMintTime[msg.sender] = block.timestamp;
        _transfer(address(this), msg.sender, mintAmount);
        _transfer(address(this), invester, investAmount);

        remainmintnum = remainmintnum - mintAmount - investAmount;
     ...
     function mint() payable  external {
        ...
        remainmintnum = remainmintnum - mintAmount;
     ...
     function setClip(string memory message) external maxLength(message) {
        ...
        if (msg.sender != clipOwner) {
            require(
                balanceOf(msg.sender) >= clipCost,
                "Insufficient CLIPS to set CLIP"
            );
    ...
    function claimPrizePool() external {
        require(
            block.timestamp >= lastClipUpdate + 3 days,
            "Prizepool can be claimed if 3 days have passed without a CLIP update"
        );
        require(
            msg.sender == clipOwner,
            "Only the current clipOwner can claim the prizepool"
        );
    ...
    function safe() external onlyOwner {
        ...
        payable(_fund).transfer(balance);
    }
