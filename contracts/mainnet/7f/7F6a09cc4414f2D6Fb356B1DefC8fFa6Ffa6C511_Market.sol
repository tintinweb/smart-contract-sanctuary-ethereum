...
contract Market is Ownable, ReentrancyGuard, ERC721Holder, ERC1155Holder {
    ...
    event PurchasCompleted(bytes32 collectibleHash, address indexed purchaser, address indexed collection, uint256 tokenId,uint256 amount);
    ...
    function purchase(bytes32 collectibleHash, uint256 amount) payable external nonReentrant {
        Collectible storage collectible = collectibles[collectibleHash];
        ...
        if (collectible.amount == 0) {
            delete collectibles[collectibleHash];
            emit PurchasCompleted(collectibleHash, _msgSender(), collectible.collection, collectible.tokenId,collectibleHashAmount[collectibleHash]);
        }
    }
