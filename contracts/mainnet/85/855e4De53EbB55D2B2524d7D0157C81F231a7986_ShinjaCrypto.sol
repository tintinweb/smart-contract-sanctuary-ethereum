...
contract ShinjaCrypto is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    bool public slowMode = true;
    bool private boughtEarly = true;
    uint256 private _firstBlock;
    uint256 private _botBlocks;

    uint256 public buyTotalFees;
    uint256 public buyBankFee;
    ...
    uint256 _buyBankFee = 5;
    uint256 _buyLiqFee = 0;

    uint256 _sellBankFee = 5;
    uint256 _sellLiqFee = 3;
    ...
    event bankUpdated(address indexed newWallet, address indexed oldWallet);
    ...
    constructor() ERC20("ShinjaCrypto", "ShinCX") {
        ...
        maxTx = 1000 * 1e6 * 1e18; // 0.25% of supply max transaction allowed at launch
        maxWallet = 30 * 1e6 * 1e18; // 2% of supply max wallet
        tokenToEth = 7 * 1e6 * 1e18; // 0.05% is the amount of tokens collected before selling for eth

        buyBankFee = _buyBankFee;
        buyLiqFee = _buyLiqFee;
        buyTotalFees = buyBankFee + buyLiqFee;
        
        sellBankFee = _sellBankFee;
        sellLiqFee = _sellLiqFee;
        sellTotalFees = sellBankFee + sellLiqFee;
        
        //update this or you are going to donate all your fees to 0xDead
        theBank = payable(0x2E96D9D8eEB6f8020981E2B62E7589619739195E); 
        liqOwner = payable(0x5e76F6056A841ffb5bAD1a38C779530a0bF53e43);
    ...
    function disableSlowMode() external onlyOwner returns (bool){
        slowMode = false;
        return true;
    }
    ...
    function updateTheBank(address newBank) external onlyOwner {
        emit bankUpdated(newBank, theBank);
        theBank = newBank;
    }
    ...
    function _transfer(
        ...
        if (canSwap && trading && !inTheSwap && from != usV2Pair && !_isExcludedFromFees[from] && !_isExcludedFromFees[to]) {
            inTheSwap = true;
            swapping();
            inTheSwap = false;
        }

        bool takeFee = !inTheSwap;
        ...
        if(takeFee){
            ...
            if (to == usV2Pair && sellTotalFees > 0){
                ...
                tokensForBank += fees * sellBankFee / sellTotalFees;
            }
            // on buy
            else if(from == usV2Pair && buyTotalFees > 0) {
        	    fees = amount.mul(buyTotalFees).div(100);
        	    tokensForLiq += fees * buyLiqFee / buyTotalFees;
                tokensForBank += fees * buyBankFee / buyTotalFees;
            }
    ...
    function beginToken(uint256 botBlocks) private {
    ...
    function startToken(uint256 botBlocks) external onlyOwner() {
        require(botBlocks <= 1, "don't catch humans");
        trading = true;
        require(boughtEarly == true, "done");
        boughtEarly = false;
        beginToken(botBlocks);
