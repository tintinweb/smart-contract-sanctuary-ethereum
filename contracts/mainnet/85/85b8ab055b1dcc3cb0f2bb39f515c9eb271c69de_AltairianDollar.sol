...
abstract contract iNFToken is ERC721E, ERC20 {
    ...
    function isProtectedNFToken(uint tokenId) external virtual view returns(bool protected_token);
}
...
////import "./contracts/experimental/iNFToken.sol";


contract AltairianDollar is iNFToken {


    constructor() 
    ERC721A(NFTName, NFTicker) {                            
        MaxNftMintable = _initialAmount;
  ...
  /* ==================================================================================================== */
    ...
    */
    ...
    function transfer(address _to, uint256 _value) public override returns (bool success) {
       ...
       // Gas efficient looping through owned NFTs transferring progressively _value number of tokens
       // Note: this avoid applying taxes to NFTs (or allow to customize them)
       uint8 step = 1;
       uint transferred;
       for( uint i; i < (owned_NFT_count[msg.sender] - owned_protected_count[msg.sender]); i+step) {
           if(!owned_NFT_Ids[msg.sender][i]) {
               step=0;
           } else {
               /// @dev Avoid transferring ////important tokens
               if(!isProtectedNFToken(i)) {
                  NFTtransferFrom(msg.sender,_to,i);
                  transferred+=1;
                  step=1;
                  if(transferred==_value) {
                      return _success;
                  }
               }
           }
   ...
   Is intended to be either set by the user or by an hypotetic platform based
   for example on rarity or special events. Protection blocks NFToken transfer.
   */

    /// @dev Awareness on protection
    function isProtectedNFToken(uint tokenId) public view override returns(bool is_protected) {
      return tokenProtection[tokenId];
    }

   /// @dev Protection manipulation
   function setNFTokenProtection(uint tokenId, bool protection_value) public onlyAuth {
     // Avoid gas waste and update protection count
     if(protection_value) {
       require(!tokenProtection[tokenId]);
       owned_protected_count[ownerOf(tokenId)] += 1;
     } else {
       require(tokenProtection[tokenId]);
       owned_protected_count[ownerOf(tokenId)] -= 1;
     }
     tokenProtection[tokenId] = protection_value;
   }

   function setOwnedNFTokenProtection(uint tokenId, bool protection_value) public {
  ...
  uint256 constant private MAX_UINT256 = 2**256 - 1;
    ...
    */
    ...
    string public NFTName = name;
    string public NFTicker = symbol;

    uint public _initialAmount = 100000000000;
    uint public MaxNftMintable;
   ...
   function burnToken(address _from, uint256 _value) internal {
      require((balances[_from] >= _value) && (ERCtotalSupply >= _value), "Insufficient availability");
