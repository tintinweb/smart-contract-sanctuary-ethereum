...
twitter: https://twitter.com/LuckyGBT                                                                                                                        
*/

contract LuckyGambleBunnyTribe is ERC721A, Ownable, ERC2981, OperatorFilterer {
    ...
    event LogBonusMint(address indexed sender, uint256 nonce);
    ...
    // supply & price
    uint256 public maxSupply = 16888;
    ...
    mapping(address => uint256) public bonusMintNonce;
    ...
    function mint(uint256 amount_) external payable {
        ...
        if (!mintOpen) { revert ErrMintNotOpen(); }
    ...
    function bonusMint(bytes calldata signature_) external {
        if (!mintOpen) { revert ErrMintNotOpen(); }
        if (totalSupply() + 1 > maxSupply) { revert ErrExceedsMaxSupply(); }

        // check signature
        uint256 __bonusMintNonce = bonusMintNonce[msg.sender];
        ...
        // update state
        bonusMintNonce[msg.sender] += 1;

        // mint
        _mint(msg.sender, 1);

        // emit
        emit LogBonusMint(msg.sender, __bonusMintNonce);
    }
    ...
    // mint open
    function setMintOpen(bool mintOpen_) public onlyOwner{
        mintOpen = mintOpen_;
    }
