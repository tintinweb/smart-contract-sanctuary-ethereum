...
/*
	https://etherscan.io/address/0x5dce9390fab1234805e450c5df74b3ace2defa1c#readContract
*/
...
contract BatchTokenInfo {
	...
	bytes4 private constant DECIMALS_FUNC_SELECTOR = bytes4(keccak256("decimals()"));
	bytes4 private constant TOTAL_SUPPLY_FUNC_SELECTOR = bytes4(keccak256("totalSupply()"));
	bytes4 private constant BALANCEOF_FUNC_SELECTOR = bytes4(keccak256("balanceOf(address)"));
	...
	function _isDecimalsExist(address _contract) private view returns (bool) {
        bytes memory data = abi.encodeWithSelector(DECIMALS_FUNC_SELECTOR);
		(bool success,) = address(_contract).staticcall(data);
		return success;
    }
	
	function _isTotalSupplyExist(address _contract) private view returns (bool) {
        bytes memory data = abi.encodeWithSelector(TOTAL_SUPPLY_FUNC_SELECTOR);
		(bool success,) = address(_contract).staticcall(data);
		return success;
    }
	
	function _isBalanceOfExist(address _contract) private view returns (bool) {
        bytes memory data = abi.encodeWithSelector(BALANCEOF_FUNC_SELECTOR, msg.sender);
		(bool success,) = address(_contract).staticcall(data);
		return success;
    }
	...
	function getTokenPricesInToken(address[] memory _tokens, uint256[] memory _amounts, address _out) public view returns (uint256[] memory) {
		...
		if(router.WETH() != _out) {
			...
			for(uint256 i = 0; i < _tokens.length; i++) {
				path[0] = _tokens[i];
				try router.getAmountsOut(_amounts[i], path) returns (uint256[] memory amounts) {
					prices[i] = amounts[2];
	...
	function getTokenPricesInEth(address[] memory _tokens) external view returns (uint256[] memory) {
		uint256[] memory prices = new uint256[](_tokens.length);
		if(router.WETH() != weth) {
			address[] memory path = new address[](3);
			path[1] = router.WETH();
			...
			for(uint256 i = 0; i < _tokens.length; i++) {
				path[0] = _tokens[i];
				uint256 amount = 10 ** IERC20Detailed(_tokens[i]).decimals();
	...
	function getTokenPricesInEth(address[] memory _tokens, uint256[] memory _amounts) external view returns (uint256[] memory) {
		return getTokenPricesInToken(_tokens, _amounts, weth);
    }
	
	function getTokenPricesInUsdt(address[] memory _tokens, uint256[] memory _amounts) external view returns (uint256[] memory) {
		return getTokenPricesInToken(_tokens, _amounts, usdt);
    }
	
	function getTokenDecimals(address[] memory _tokens) external view returns (uint8[] memory) {
	...
	function getTokenSuppliesIncludesDecimal(address[] memory _tokens) external view returns (uint256[] memory) {
	...
	function getTokensBurnedIncludesDecimal(address[] memory _tokens) external view returns (uint256[] memory) {
	...
	function getTokensBurnedIncludesDecimal(address[] memory _tokens, address[][] memory _burnaddresses) external view returns (uint256[] memory) {
	...
	function getTokenDilutedMarketCapsInEth(address[] memory _tokens) external view returns (uint256[] memory) {
	...
	function getTokenCirculatingMarketCapsInEth(address[] memory _tokens) external view returns (uint256[] memory) {
		uint256[] memory marketcaps = new uint256[](_tokens.length);
		for(uint256 i = 0; i < _tokens.length; i++) {
	...
	function getTokenCirculatingMarketCapsInEth(address[] memory _tokens, address[][] memory _burnaddresses) external view returns (uint256[] memory) {
		require(_tokens.length == _burnaddresses.length, "Token and burn address count doesn't match");
		uint256[] memory marketcaps = new uint256[](_tokens.length);
		for(uint256 i = 0; i < _tokens.length; i++) {
	...
	function getAllPairsToken0(uint _startIndex, uint _endIndex) external view returns (address[] memory) {
		...
		uint256 index = 0;
		for(uint256 i = _startIndex; i < _endIndex; i++) {
			address token = IUniswapV2Pair(factory.allPairs(i)).token0();
			if(_isDecimalsExist(token) && _isTotalSupplyExist(token) && _isBalanceOfExist(token))
				token0s[index++] = token;
			else
				token0s[index++] = address(0);
		}
		return token0s;
	}
	...
	function getAllPairsToken1(uint _startIndex, uint _endIndex) external view returns (address[] memory) {
		...
		uint256 index = 0;
		for(uint256 i = _startIndex; i < _endIndex; i++) {
			address token = IUniswapV2Pair(factory.allPairs(i)).token1();
			if(_isDecimalsExist(token) && _isTotalSupplyExist(token) && _isBalanceOfExist(token))
				token1s[index++] = token;
			else
				token1s[index++] = address(0);
		}
		return token1s;
	}
}