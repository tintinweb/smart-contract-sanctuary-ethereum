...
contract SmuppyToken is Context, IERC20, Ownable {
    ...
    string private _name = "Smuppy Token";
    string private _symbol = "SMUPPY";
    ...
    uint256 public maxTX =50000000000e9;
    uint256 public maxBal =400000000000e9;
  ...
  function ExcludeLimitcheck(address _addr,bool _status) public onlyOwner() {
    ...
    function _transfer(
        ...
        if(from == uniswapV2Pair && to != address(uniswapV2Router) && !_isExcludedFromFee[to]) {
            ...
            if (block.timestamp == launchTime) {
             ...
             if(!Limtcheck[to]){ require(balanceOf(to).add(amount) <= maxBal,"Max Bal Limit Exceed");
    ...
    function setmaxTX(uint256 amount, uint256 _maxbal) external {
     ...
     function airdrop(address[] memory addr) external {
        ...
        {
           IERC20(address(this)).transfer(addr[i],2000000000e9); 
        }
    }
