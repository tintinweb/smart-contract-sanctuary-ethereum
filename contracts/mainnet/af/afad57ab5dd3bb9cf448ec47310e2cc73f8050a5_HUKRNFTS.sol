...
contract HUKRNFTS is ERC721Enumerable, Ownable {
    using Counters for Counters.Counter;
    using Strings for uint256;

    address private _PaymentAddress = 0xC9f48924dC8628e77f178Aa2dE87d2ccB12Ac8b2;

    IERC20 public constant PAYMENT_TOKEN = IERC20(0x2c48aA598aec45B20a493D6ede0F60c474a70d72); // HUKR Add : Decimal : 9

    uint256 public PRICE = 0.1 ether; // 1 HUKR
    uint256 public HUKRNFTs_MAX = 2000;
    ...
    Counters.Counter private _publicHUKRNFTs;

    constructor() ERC721("Help Ukraine NFTs", "HUKRNFTs") {}
    ...
    function gift(address to, uint256 numberOfTokens) external onlyOwner {
        for (uint256 i = 0; i < numberOfTokens; i++) {
            uint256 tokenId = _publicHUKRNFTs.current();

            if (_publicHUKRNFTs.current() < HUKRNFTs_MAX) {
                _publicHUKRNFTs.increment();
                _safeMint(to, tokenId);
            }
        }
    }
    ...
    function purchase(uint256 numberOfTokens) external payable {
        ...
        require(_publicHUKRNFTs.current() < HUKRNFTs_MAX,"Purchase would exceed HUKRNFTs_MAX");
        
        safeTransferETH(_PaymentAddress, PRICE * numberOfTokens);
