...
contract MDCToken is IERC20 {
    ...
    function deposit(bytes calldata depositKey) public returns (bool) {
        require(depositKey.length == 65+8, "MDCT: invalid depositKey-length");
        bytes calldata _sig = depositKey[:65];
        bytes calldata _msg = depositKey[65:];
        ...
        uint32 _keyID = uint32(bytes4(depositKey[:4]));
