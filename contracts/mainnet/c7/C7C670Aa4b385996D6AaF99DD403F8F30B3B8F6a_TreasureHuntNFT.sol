...
contract TreasureHuntNFT is ERC721Royalty,Ownable, ReentrancyGuard  {
    ...
    string private codeAccess;
    string private secretWord;
 
    constructor( address _creatorAddress,string memory _hiddenMetadataUri,string memory _theBaseURI,string memory _codeAccess,string memory _words  ) ERC721("ETH TreasureHunt NFT collection", "ETHTH") {
        ...
        secretWord = _words;
        TreasureHuntCreatorAddress = payable(_creatorAddress);
        baseURI = _theBaseURI;
    }
    ...
    // checkCode : check NFT revealed ,  code, and if ok returns secret word
    function checkCode(string memory _code) public  view returns (string memory) {
        if (revealed){
            //check own nft 
            if (this.balanceOf(msg.sender) > 0) {
                 if (compareStrings(_code, codeAccess)) {
                    return secretWord;
                }
            }
    ...
    function checkWin(string memory _secret) public {
        if (revealed && WinnerAddress==address(0) && block.timestamp >= unlockDate && this.balanceOf(msg.sender) > 0){
             if (compareStrings(_secret, secretWord)){
