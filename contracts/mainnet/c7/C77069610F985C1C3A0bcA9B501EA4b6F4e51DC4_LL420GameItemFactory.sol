...
contract LL420GameItemFactory is Initializable, OwnableUpgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function buy(
        ...
        if (taken + _amount > MAX_PER_WALLET || _amount == 0 || _amount > MAX_PER_TX) revert InvalidAmount();
    ...
    function canMint(address who) public view returns (bool) {
        if (isOGMint) {
            return IERC1155Upgradeable(ogPassAddress).balanceOf(who, OG_TOKEN_ID) > 0;
        }
        return true;
    }
    ...
    function setSupply(uint256[] memory _caps) external onlyOwner {
        ...
        for (uint256 i; i < _caps.length; i++) {
...
error NotEnoughBurnt();
error SupplyLimit();
error VerifyFailed();
error WrongCondition();
