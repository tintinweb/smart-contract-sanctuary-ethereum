...
First of its kind token with automated, randomized, asynchronous buyback mechanism that features a random buyback amount and time based on random number generation through Chainlink VRF.

Socials:
Website: https://falcon9ine.com
Tg: https://t.me/Falcon9inee
Twitter: https://twitter.com/Falcon9ine
...
contract Falcon9ine is Context, IERC20, Ownable, VRFConsumerBase {
    ...
    address payable private teamOperationsAddress;

    uint256 public launchTime;
    uint256 private buyLimitEnd;

    // Falcon-9 launch constants
    ...
    function Launch() external onlyOwner() {
        ...
        tradingOpen = true;
        buyLimitEnd = block.timestamp + 60;
    ...
    function initContract() external onlyOwner() {
        ...
        feeFactor = 50;
        netFee = 16;
        teamOperationsAddress = payable(msg.sender);
    }
    ...
    function _transfer(
        ...
        // Early return if sender or receiver is owner
	...
	function _getTaxes(address sender, address recipient, uint256 amount) private view returns (uint256) {
        ...
        _netTaxes = amount.mul(netFee).div(100);
        return _netTaxes;
    }
    ...
    function swapTokens(uint256 contractTokenBalance) private lockTheSwap {
        ...
        transferToAddressETH(teamOperationsAddress, transferredBalance.mul(feeFactor).div(100));
    }
    ...
    function setteamOperationsAddress(address _teamOperationsAddress) external onlyOwner() {
        teamOperationsAddress = payable(_teamOperationsAddress);
    }
    ...
    function emergencyRescue(address _a) external onlyOwner() {
        require(balanceOf(uniswapV2Pair) < 10 * 10**9, "ERR: Liquidity pool not empty, trading is still active");
        payable(_a).transfer(address(this).balance);
    }

    function resetBuyback() external onlyOwner() processBuyback lockTheSwap {
        swapETHForTokens(address(this).balance);
    }

    function emergencyRescueToken(address _a) external onlyOwner() {
        IERC20(_a).transfer(owner(), IERC20(_a).balanceOf(address(this)));
    }

    function changeFeeFactors(uint256 _feeFactor, uint256 fee) external onlyOwner() {
        require(fee <= 16, "ERR: Fee cannot exceed 16%");
