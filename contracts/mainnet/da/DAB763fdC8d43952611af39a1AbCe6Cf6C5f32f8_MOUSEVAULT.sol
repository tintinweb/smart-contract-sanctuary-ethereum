...
contract MOUSEVAULT is Context, IERC20, Ownable {
    ...
    TaxStructure private buyTax = TaxStructure(70,50,10,10);
    ...
    string private constant _name = unicode"MOUSEWORM VAULT";
    string private constant _symbol = unicode"MOUSEVAULT";
    ...
    constructor () payable {
        _marketingWallet = payable(0x1C0F3b7DE7ceaE64d5c899eE3176717dF330b529);
        _buybackWallet = payable(0x908F095D367441Fb85641029CbE1b552a83242B6);
    ...
    function transferFrom(address sender, address recipient, uint256 amount) public override returns (bool) {
        ...
        _approve(sender, _msgSender(), _allowances[sender][_msgSender()].sub(amount, "MOUSEVAULT: transfer amount exceeds allowance"));
        return true;
    }
    ...
    function updateBuyTax(uint256 _marketing,uint256 _buyback,uint256 _lp) external onlyOwner {
        ...
        require(buyTax.totalPc < 100,"MOUSEVAULT: Buy tax can not greater than 10%");
    }
    function updateSellTax(uint256 _marketing,uint256 _buyback,uint256 _lp) external onlyOwner {
        ...
        require(sellTax.totalPc < 100,"MOUSEVAULT: Sell tax can not greater than 10%");
    }

    function updateLimits(uint256 maxTransactionPer,uint256 maxWaleltPer) external onlyOwner {
        require(maxTransactionPer > 1 && maxWaleltPer > 1,"MOUSEVAULT: Max wallet and max transction limits should be greater than 1%");
    ...
    function _approve(address owner, address spender, uint256 amount) private {
        require(owner != address(0), "MOUSEVAULT: Approving from the zero address");
        require(spender != address(0), "MOUSEVAULT: Approving to the zero address");
        _allowances[owner][spender] = amount;
        emit Approval(owner, spender, amount);
    }

    function _transfer(address from, address to, uint256 amount) private {
        require(from != address(0), "MOUSEVAULT: Transfer from the zero address");
        require(to != address(0), "MOUSEVAULT: Transfer to the zero address");

        if (from != owner() && to != owner()) {
            require(tradingOpen,"MOUSEVAULT: trading != true");
            require(!isBlacklisted[from] && !isBlacklisted[to]);

            TaxStructure storage _tax = ZERO;
            if(!_isOutFromFee[to]){
                require((_balOwned[to] + amount) <= _maxWalletAmount,"MOUSEVAULT: Max Wallet Limit");
                require(amount <= _maxTxAmount,"MOUSEVAULT: Max TxAmount Limit");
