...
contract HDRNStaking is Ownable, ReentrancyGuard {
    ...
    uint256 public totalAmountBuyandBurn;
    uint256 public stakingPeriod = 369 days;
    ...
    function BuyAndBurn() external {
        ...
        totalAmountBuyandBurn = totalAmountBuyandBurn.add(hedronBurn);
    }
    ...
    function CompoundReward(uint256 stakeId) external hasStaked(stakeId) {
        ...
        stakersData[stakeId].amount = stakers[stakeId].amount.add(reward);
    ...
    function totalActiveStakes() public view returns (uint256) {
        ...
        for (uint256 i = 0; i < stakersData.length; i++) {
            if (
                stakersData[i].activeStaked &&
                !hasCompletedStakingPeriod(stakersData[i].stakeId)
            ) {
