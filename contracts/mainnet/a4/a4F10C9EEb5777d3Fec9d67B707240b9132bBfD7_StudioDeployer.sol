    ...
    UUPSUpgradeable
{
    ...
    // only meant to be called once for 1st few ERC721 collection contracts that had supply tracking
    // setting the collection supply to the highest possible number, because these contracts have a supply check on the mint function
    function batchRemoveSupplyConstrain(address[] calldata contracts)
        public
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        for (uint32 c_idx = 0; c_idx < contracts.length; c_idx++) {
            ICollectionSupplyDeprecated(contracts[c_idx]).setSupply(
                type(uint256).max
            );
        }
    }
