...
contract StakeContract is Ownable, ReentrancyGuard {
    ...
    // Info of each pool.
    struct PoolInfo {
        IERC20 tokenAddress;           // Address of token contract.
    ...
    constructor(
        ...
        stakingToken = IERC20(0x7adc29cec54b124a7E8E1B1415DFf95DDD13B173);
    ...
    function deposit(uint256 _amount) public nonReentrant {
        ...
        if(_amount > 0) {
            if (user.amount == 0) {
                // Only increment totalUsers if the user has not deposited before
                totalUsers += 1;
            }
            uint256 initialBalance = pool.tokenAddress.balanceOf(address(this));
            pool.tokenAddress.safeTransferFrom(address(msg.sender), address(this), _amount);
            amountTransferred = pool.tokenAddress.balanceOf(address(this)) - initialBalance;
