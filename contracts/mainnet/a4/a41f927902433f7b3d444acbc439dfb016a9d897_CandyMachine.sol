...
// File: contracts/wonderland_nft_new/CandyMachineAccessControl.sol

pragma solidity ^0.8.0;



abstract contract CandyMachineAccessControl is AccessControl, Ownable {
...
// File: contracts/wonderland_nft_new/CandyMachine.sol
...
contract CandyMachine is ERC721, CandyMachineAccessControl {
    using Strings for uint256;
    ...
    function mintCandyBatch(address _to, uint256[] memory _tokenIds)
        public
    ...
    function mintCandy(address _to, uint256 _tokenId)
        public
        onlyRole(MINTER_ROLE)
    {
        _mint(_to, _tokenId);
    ...
    function _setIPFSBaseURI(string memory _ipfsBase) internal {
        ipfsBaseUri = _ipfsBase;
    }

    function setIPFSBaseUri(string memory _ipfsBase) public onlyOwner {
        emit IPFSBaseURIUpdated(ipfsBaseUri, _ipfsBase);
    ...
    function ipfsTokenURI(uint256 tokenId) public view returns (string memory) {
        ...
        return
            bytes(_baseURI_).length > 0
                ? string(abi.encodePacked(_baseURI_, tokenId.toString()))
                : "";
    }

    function setonFire(uint256 _tokenId) public whenBurnNotPaused {
        require(
