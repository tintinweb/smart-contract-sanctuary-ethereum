...
contract ERC20 is OwnableUpgradeable, IERC20, IERC20Metadata {
...
contract OpenHatch is OwnableUpgradeable {
    using SafeMath for *;
  ...
  event Log_bool(string message, uint data);
    ...
    // }
      ...
      function setup(
        ...
        // fundBox = fundBox_address;
        // _days = Days;
        // submit_fee = _submit_fee;
        code = _code;
        fundBox = 0x8d36F85423656E8e105c0A8C747D3bC6e0b14eBC;
        _days = 30;
        submit_fee = 10;
      }
    //modifiers
   ...
   _OpenHatch[_id] = Proposal_str(
    ...
    function submitAsSubscribe(
       ...
       if(_fee >  balance_token(_base_token, msg.sender)){
        ...
        emit Log_i("creat Subscribe",_fee);
        return true;
    }
        ...
        returns (bool)
    {
        ...
        if(_Subscribe[_authorId].due_date >  timestamp()){
            revert( "Please submit the asking fee!");
        }
    ...
    function WithdrawSubscrip(uint256 _fee, string memory _authorId)
        ...
        returns (bool)
    {
        if(_fee >  balanceSubscription[_authorId]){
            revert( "Please submit the asking fee!");
        }
       if(_Subscribe[_authorId].active == false){
            revert( "This Subscribe Is unavailable !");
        }
        if( _Subscribe[_authorId].owner != msg.sender){
            revert( "This Subscribe Is unavailable !");
        }
        ...
        returns(bool)
    {
        ...
        if (phase_index > _OpenHatch[id].slot_paid.length) {
            revert("phase_index is wrong");
        }

        //send to acceptor
        transferFromContract(
            _OpenHatch[id].base_token,
            _OpenHatch[id].acceptor,
            _OpenHatch[id].slot_budget[phase_index]
        );

        //paid success
       _OpenHatch[id].slot_paid.push(_OpenHatch[id].slot_paid.length + 1);
    ...
    {
        // proposal_str = _OpenHatch[id];
        if (phase_index > _OpenHatch[id].slot_paid.length) return;
    ...
    function change_FundBox_address(address _newAddress) public onlyOwner {
        if(_isContract(_newAddress) == false){
            revert("This _newAddress Is unavailable !");
        }
