...
contract DegenPigeonsStaking is ERC20Burnable, Ownable {
    ...
    function getReward(uint256 tokenId) internal view returns(uint256) {
        ...
        if (tokenId == 7 || tokenId == 44 || tokenId == 101 || tokenId == 152 || tokenId == 235 || tokenId == 279 || tokenId == 306 ) {
    ...
    function _getBonus(address staker) internal view returns (uint256 bonus) {
        ...
        if (balance < 3 && bonusPigeons) return 0;
        if (balance < 5 && bonusPigeons) return 20;
        if (balance < 10 && bonusPigeons) return 50;
        return 100;
    }
}