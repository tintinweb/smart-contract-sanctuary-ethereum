...
contract spinContract is Ownable, VRFConsumerBaseV2{
    ...
    event spinFinished(address indexed from, uint256[] array);
   ...
   function getReservesHel() public view returns (uint256, uint256){
    ...
    function random(uint256 i, uint256 min, uint256 max) private view returns (uint256) {
        return min + uint256(keccak256(abi.encodePacked(block.difficulty, block.timestamp, s_randomWords[i%numWords], counter))) % (max - min + 1);
    }
    ...
    function spin(uint256 _type) public payable{
      ...
      for(uint i = 0; i<spins; i++){
        randomNumber = random(i, 0, 100);
        if(i%numWords == 0){
            counter += random(i, 0, 10000);
        }
      ...
      lastWin[msg.sender] = totalWin.mul(1e9);
      lastBundleUsed[msg.sender] = _type;
      counter += random(0, 0, 10000);
      emit spinFinished(msg.sender, multipliers);
    }
