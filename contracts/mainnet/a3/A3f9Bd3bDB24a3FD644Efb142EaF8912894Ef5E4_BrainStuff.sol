...
contract BrainStuff is Ownable, ERC721A, PaymentSplitter, DefaultOperatorFilterer {
    ...
    uint public  MAX_SUPPLY = 8640;
    uint public  MAX_TOTAL_PUBLIC = 8640;
    uint public  MAX_TOTAL_WL = 1000;


    uint public MAX_PER_WALLET_PUBLIC = 30;
    uint public MAX_PER_WALLET_WL = 5;
    ...
    constructor(uint96 _royaltyFeesInBips, address[] memory _team, uint[] memory _teamShares, bytes32 _merkleRootWL, string memory _baseURI) ERC721A("Brain Stuff", "BrainStuff")
    ...
    function pricePUBLIC(address _account, uint _quantity) public view returns (uint256) {
        if (amountNFTsperWalletPUBLIC[_account] == 0){
               return publicSalePrice * (_quantity - 1);
           }
        return publicSalePrice * _quantity;
    ...
    function priceWHITELIST(address _account, uint _quantity) public view returns (uint256) {
        ...
        if (amountNFTsperWalletWL[_account] == 0){
            ...
            if(_quantity > 1){
                return wlSalePrice * (_quantity - 2);
            }
        }
        if (amountNFTsperWalletWL[_account] == 1){
            return wlSalePrice * (_quantity - 1);
        }
   ...
   function whitelistMint(uint _quantity, bytes32[] calldata _proof) external payable callerIsUser {
        ...
        require(msg.value >= priceWHITELIST(msg.sender, _quantity), "Not enought funds");
    ...
    function publicSaleMint(uint _quantity) external payable callerIsUser {
        ...
        require(_quantity > 0, "Mint at least one NFT.");
        require(msg.value >= pricePUBLIC(msg.sender, _quantity), "Not enought funds");
