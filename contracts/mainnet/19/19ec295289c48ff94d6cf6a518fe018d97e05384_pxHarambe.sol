...
//`MM'    `MM'                                          MM                
// MM      MM                                           MM                
// MM      MM    ___   ___  __    ___   ___  __    __   MM____     ____   
// MM      MM  6MMMMb  `MM 6MM  6MMMMb  `MM 6MMb  6MMb  MMMMMMb   6MMMMb  
// MMMMMMMMMM 8M'  `Mb  MM69 " 8M'  `Mb  MM69 `MM69 `Mb MM'  `Mb 6M'  `Mb 
// MM      MM     ,oMM  MM'        ,oMM  MM'   MM'   MM MM    MM MM    MM 
// MM      MM ,6MM9'MM  MM     ,6MM9'MM  MM    MM    MM MM    MM MMMMMMMM 
// MM      MM MM'   MM  MM     MM'   MM  MM    MM    MM MM    MM MM       
// MM      MM MM.  ,MM  MM     MM.  ,MM  MM    MM    MM MM.  ,M9 YM    d9 
//_MM_    _MM_`YMMM9'Yb_MM_    `YMMM9'Yb_MM_  _MM_  _MM_MYMMMM9   YMMMM9 
...
contract pxHarambe is ERC721A, Owneable {

    string public baseURI = "ipfs://QmdQCiY8hXzKeqMajT8PrCxAZCcpfyB4N8vrmohoAdTXXd/";
    ...
    constructor() ERC721A("pxHarambe", "pxH") {}
    ...
    function PublicMintTWO() external payable {

        address _caller = _msgSender();
        require(!paused, "Paused");
        require(max_supply >= totalSupply() + 2, "All Gone");
        require(2 > 0, "No 0 mints");
        require(tx.origin == _caller, "No Contract minting.");
        require(MAX_PER_TX >= 2 , "Excess max per paid tx");
        
      if(free_max_supply >= totalSupply()){
            require(MAX_PER_TX_FREE >= 2 , "Excess max per free tx");
        }else{
            require(MAX_PER_TX >= 2 , "Excess max per paid tx");
            require(2 * price == msg.value, "Invalid funds provided");
        }
    ...
    function Reserve(uint256 quantity, address) external onlyOwner {
