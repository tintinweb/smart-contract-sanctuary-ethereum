...
// File: contracts/SINCE3000GENESIS.sol
...
contract SINCE3000GENESIS is ERC721Enumerable, Ownable, ERC2981 {
  using Strings for uint256;
  ...
  address payable gownsForGood;
  mapping(address => uint256) public addressMintedBalance;

  constructor(
    address _beneficiary, address _gownsForGood, 
    string memory _name,
    ...
    gownsForGood = payable(_gownsForGood);
  ...
  function mint(uint256 _mintAmount) public payable {
    ...
    if (msg.sender != owner()) {
     ...
     for (uint256 i = 1; i <= _mintAmount; i++) {
      ...
      _setTokenRoyalty(supply + i, beneficiary, 1000);
    }
  }

  function mintg4g(uint256 _mintAmount) public onlyOwner {
     uint256 supply = totalSupply();
     require(supply + _mintAmount <= maxSupply, "max NFT limit exceeded");
     for (uint256 i = 1; i <= _mintAmount; i++) {
      addressMintedBalance[msg.sender]++;
      _safeMint(gownsForGood, supply + i);
      _setTokenRoyalty(supply + i, gownsForGood , 1000);
    }
  }
