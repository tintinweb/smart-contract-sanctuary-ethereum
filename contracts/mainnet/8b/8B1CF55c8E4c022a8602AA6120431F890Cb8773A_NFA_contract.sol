    ...
    ISeaDropTokenContractMetadata
{
    ...
    function setRoyaltyInfo(RoyaltyInfo calldata newInfo) external {
        ...
        // Revert if the new basis points is greater than 1000!
        if (newInfo.royaltyBps > 1000) {
...
pragma solidity 0.8.17;
contract NFA_contract is ERC721PartnerSeaDrop {

    uint256 priceWei;
    uint256 maxWalletLimit;   

    bool private enableMint = false;
    address private ROYALTY_ADDRESS = 0xab40C46aacE88d0b6db6a155aDCEB8663d9B61fc;

    constructor(address[] memory allowedSeaDrop) ERC721PartnerSeaDrop(
        "The 10k collection: NFA.space", "NFA10k", 
        msg.sender, 
        allowedSeaDrop) {
        _royaltyInfo.royaltyAddress = ROYALTY_ADDRESS;
        _royaltyInfo.royaltyBps = 1000;
    }

    function setEnableMint(bool _flag) external onlyOwnerOrAdministrator {
        enableMint = _flag;
    }

    function setPriceMint(uint256 _wei) external onlyOwnerOrAdministrator {
        priceWei = _wei;
    }

    function setMaxWalletLimit(uint256 _value) external onlyOwnerOrAdministrator {
        maxWalletLimit = _value;
    }


    function mint(uint256 quantity) external payable {
        require(enableMint, "Minting hasn't started yet");
        require(quantity > 0, 'Error: Invalid value');
        require(totalSupply() + quantity <= maxSupply(), "Cannot mint over MAX_SUPPLY");
        if (msg.sender != owner()) {
            if (msg.sender != administrator) {
                require(balanceOf(msg.sender) + quantity <= maxWalletLimit , "Cannot mint over MAX_WALLET_LIMIT");
            }
        }

        uint256 amount = priceWei * quantity;
    ...
    function burn(uint256 tokenId) external {
        require(msg.sender == ownerOf(tokenId), "ERC721: Only the owner of the token can burn it.");
        _burn(tokenId);
    }
    ...
    function withdraw(address wallet, uint256 amount) public onlyOwner  {
