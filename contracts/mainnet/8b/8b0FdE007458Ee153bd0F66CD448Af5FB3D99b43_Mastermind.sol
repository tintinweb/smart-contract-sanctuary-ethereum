...
contract Mastermind is IERC20, Auth {
    ...
    string private constant  _name = "Mastermind";
    string private constant _symbol = "MASTERMIND";
    ...
    address payable public teamWallet = payable(0xa61596355D989815c71F366A4bb6A2803bb62066);
    ...
    uint256 private maxBuy = 10000001 * (10 ** _decimals);
    uint256 public swapThresholdAmount = 4500000 * (10**_decimals);
    ...
    function setTicketPrice(uint256 price) external onlyOwner {
        ticketPrice = price * (10 ** _decimals);
    }

    function setSaleOpen(bool status, int id) external onlyOwner {
        if (status) {
            gameID = id;
        }
        isSaleOpen = status;
    ...
    function burn() external {
        require (isSaleOpen, "Purchasing tickets is not allowed at this time");
        require (tickets[msg.sender] != gameID, "Ticket for this game has already been purchased");
        require (balanceOf(msg.sender) >= ticketPrice);
        _basicTransfer(msg.sender, DEAD, ticketPrice);
        tickets[msg.sender] = gameID;
    }    
}