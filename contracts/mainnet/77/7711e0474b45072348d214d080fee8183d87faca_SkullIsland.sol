...
// File: contracts/SkullIsland.sol
...
contract SkullIsland is ERC721A, Ownable, AccessControlEnumerable {

  uint256 public maxTotalSupply = 3165;
  uint256 public holderMintPrice = 1200 * 10 ** 18;
  uint256 public publicMintPrice = 0.007 ether;
  uint8 private maxTokenPrivate = 1;
  ...
  mapping(address => uint256) privateMinted;
  mapping(address => uint256) publicMinted;

  string _baseTokenURI;

  constructor(address _tokenContract) ERC721A("SkullIsland", "SKULL") {
    tokenContract = _tokenContract;
    _setupRole(DEFAULT_ADMIN_ROLE, _msgSender());
  }

  function holderMint(uint256 amount) external {
    ...
    require(maxMintPerAddress[msg.sender] > 0, "You are not a holder");
    require(amount + mintedPerAddress[msg.sender] <= maxMintPerAddress[msg.sender], "sender address cannot mint more than maxMintPerAddress lands");
    IERC20(tokenContract).transferFrom(msg.sender, address(this), holderMintPrice * amount);
  ...
  function privateMint(uint256 amount, bytes32[] calldata proof) public payable {
    ...
    require(amount + publicMinted[msg.sender] <= maxTokenPrivate, "sender address cannot mint more than maxMintPerAddress lands");
  ...
  function setMaxMintPerAddress(address[] calldata _addresses, uint256[] calldata _mints) public onlyOwner{
    for(uint256 i = 0; i < _addresses.length; i++) {
        maxMintPerAddress[_addresses[i]] = _mints[i];
    }
  }
