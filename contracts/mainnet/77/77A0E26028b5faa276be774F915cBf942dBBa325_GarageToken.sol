    ...
    DefaultOperatorFiltererUpgradeable
{
    ...
    mapping(uint16 => uint256) public categoryOffset;
    ...
    function initialize(address _factory) public initializer {
        __ERC721_init("War Riders Garage Property Title", "NFTPT");
    ...
    function mintFor(
        ...
        IGarageFactory f = IGarageFactory(factory);

        (, , uint256 dailyLimit, , uint256 currentDayTotal) = f.categoryData(
            category
        );

        if (currentDayTotal == 0) {
            // This is a bug, so we must save the size
            // variable as an offset
            categoryOffset[category] = size;

            // We'll check the daily limit on the next line with the offset
        }

        require(
            currentDayTotal + categoryOffset[category] <= dailyLimit,
            "limit exceeded"
        );
...
interface IGarageFactory {
    ...
    function categoryData(uint16 category)
        external
        view
        returns (
            uint256,
