...
contract WWDispensary is Ownable, ReentrancyGuard {
	...
	/**
	 * @dev claims all rewards for account - both for v1 and and v2.
	 */
	...
	/**
	 * @dev claims rewards for account v1 - unstaked only.
	 */
	function claimRewardsv1() public nonReentrant {
		address account = msg.sender;
		uint256 totalEtherOwed = _getTotalEtherOwed(account);
		uint256 _totalEtherOwedPerPG = _getTotalEtherPerPG();
		uint256 _totalEtherOwedPerAP = _getTotalEtherPerAP();
	...
	/**
	 * @dev claims rewards for account v1 PG only - will save some gas if you only hold PG characters.
	 */
	function claimRewardsv1PGOnly() public nonReentrant {
		address account = msg.sender;
		uint256 _totalEtherOwedPerPG = _getTotalEtherPerPG();
		uint256 balance = PG.balanceOf(account);
		uint256 totalEtherOwed = _totalEtherOwedPerPG * balance;
	...
	/**
	 * @dev claims rewards for account v1 AP only - will save some gas if you only hold AP characters.
	 */
	function claimRewardsv1APOnly() public nonReentrant {
		address account = msg.sender;
		uint256 _totalEtherOwedPerAP = _getTotalEtherPerAP();
		uint256 balance = AP.balanceOf(account);
		uint256 totalEtherOwed = _totalEtherOwedPerAP * balance;

		uint256 totalEtherClaimed;
		for (uint256 i; i < AP.balanceOf(account); i++) {
			uint256 tokenId = AP.tokenOfOwnerByIndex(account, i);
			totalEtherClaimed += _etherClaimedByAP[tokenId];
			uint256 diff = _totalEtherOwedPerAP - _etherClaimedByAP[tokenId];
			_etherClaimedByAP[tokenId] += diff;
		}
