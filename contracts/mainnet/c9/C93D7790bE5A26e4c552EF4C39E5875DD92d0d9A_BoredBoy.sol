...
contract BoredBoy is ERC721A, Ownable, ReentrancyGuard {
    using Strings for uint256;

    bytes32 public merkleRoot;
    uint256 public whitelistGeneration = 0;
        ...
        mintPriceCompliance(_mintAmount)
    {
        ...
        require(
            !whitelistClaimed[whitelistGeneration][_msgSender()],
        ...
        whitelistClaimed[whitelistGeneration][_msgSender()] = true;
        _safeMint(_msgSender(), _mintAmount);
    }

    function setWhitelistGeneration(uint256 _generation) public onlyOwner {
        whitelistGeneration = _generation;
    }

    function setSplitter(address _splitterAddress) public onlyOwner {
        splitter = _splitterAddress;
    }
    ...
    function withdraw() public onlyOwner nonReentrant {
        require(
            splitter != address(0),
            "Splitter is not set to a valid address"
        );

        uint256 totBalance = address(this).balance;

        (bool hs1, ) = payable(splitter).call{value: totBalance}("");
        require(hs1);
    }
