...
contract AieggsChef is Ownable, ReentrancyGuard {
    ...
    // AIEGGS LP address
    IUniswapPair public aieggsLp;
    // AIEGGS tokens reward per block.
    ...
    // The block number when AIEGGS mining starts.
    ...
    constructor(
        IAieggsToken _aieggs,
        IUniswapPair _aieggsLp,
    ...
    // Update the given pool's AIEGGS allocation point. Can only be called by the owner.
    function set(
        uint256 _pid,
        uint256 _allocPoint,
        bool _withUpdate
    ...
    // View function to see pending AIEggs on frontend.
        ...
        returns (uint256)
    {
        ...
        if (address(pool.lpToken) == address(aieggs)) {
            lpSupply = aieggs.balanceOfUnderlying(address(this));
        }
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            uint256 aieggsReward = ((block.number - pool.lastRewardBlock) *
                rewardPerBlock *
                pool.allocPoint) / totalAllocPoint;
            accRewardPerShare += (aieggsReward * 1e12) / lpSupply;
        }
        return
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 aieggsReward = ((block.number - pool.lastRewardBlock) *
            rewardPerBlock *
            pool.allocPoint) / totalAllocPoint;
        pool.accRewardPerShare += (aieggsReward * 1e12) / lpSupply;
        pool.lastRewardBlock = block.number;
    ...
    // Deposit tokens to AIEggsChef for AIEGGS allocation.
    function deposit(uint256 _pid, uint256 _amount) external nonReentrant {
        ...
        if (address(pool.lpToken) == address(aieggs)) {
            _amount = aieggs.fragmentToAieggs(_amount);
        }
    ...
    // Withdraw tokens from AIEggsChef.
    function withdraw(uint256 _pid, uint256 _amount) external nonReentrant {
        ...
        user.rewardDebt = (user.amount * pool.accRewardPerShare) / 1e12;
        if (address(pool.lpToken) == address(aieggs)) {
            _amount = aieggs.aieggsToFragment(_amount);
        }
