 ...
 * @title Loans manager
 */
contract LoansManager is Ownable {
    // Quick summary of the loan
    struct LoanEntry {
        uint8 loanType;
        address loanAddress;
        address lenderAddr;
    ...
    /**
     * @notice The total number of loans deployed
     */
    uint256 public totalLoans;

    /**
     * @notice The loans deployed
     */
    mapping (uint256 => LoanEntry) public loans;

    /**
     * @notice The total number of loans granted to a borrower
     */
    mapping (address => uint256) public totalByBorrower;

    /**
     * @notice The loan indexes by borrower
     */
    mapping (address => mapping (uint256 => uint256)) public loansByBorrower;

    /**
     * @notice This event is fired whenever a new loan gets deployed
     * @param newLoanAddress The address of the newly created loan
     */
    event OnLoanDeployed (address newLoanAddress);

    /**
     * @notice Constructor
     * @param ownerAddr The owner of this smart contract
     */
    ...
    function deployOpenTermLoan (
        ...
        // Deploy a new loan
        OpenTermLoan instance = new OpenTermLoan(
                                                lenderAddr, 
                                                borrowerAddr,
                                                newPrincipalToken,
                                                newOracle,
        ...
        // Update the records
        loans[totalLoans] = LoanEntry(uint8(0), address(instance), lenderAddr, borrowerAddr, loanAmountInPrincipalTokens, newAprWithTwoDecimals);
        loansByBorrower[borrowerAddr][totalByBorrower[borrowerAddr]] = totalLoans;
        totalLoans++;
        totalByBorrower[borrowerAddr]++;
    }
}