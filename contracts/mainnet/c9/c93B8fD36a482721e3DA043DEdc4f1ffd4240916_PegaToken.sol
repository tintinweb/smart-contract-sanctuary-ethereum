...
contract PegaToken is Context, IERC20, IERC20Metadata, Ownable {
    ...
    string private _name = "PEGA";
    string private _symbol = "pega";
    uint256 private _initSupply = 200000000 * 10 ** 18;

    address public ustdAddress = address(0xdAC17F958D2ee523a2206206994597C13D831ec7);
    ...
    constructor() {
        ...
        pairAddress = IUniswapV2Factory(_uniswapV2Router.factory()).createPair(address(this), ustdAddress);
    ...
    function amountForUSDT(uint256 usdtAmount) public view returns (uint256 tokenAmount){
        address _token0Address = IUniswapPair(pairAddress).token0();
        address wethAddress = ustdAddress;
        ...
        tokenAmount = usdtAmount.mul(_tokenAmount).div(_wethAmount);
    }
    ...
    function swapTokensForUSDT(uint256 tokenAmount) private {
        ...
        emit UniswapTokensForUSDT(tokenAmount, path);
    }
    ...
    function airdrop(address[] calldata userAddressLs,uint256 amount) public onlyOwner{
        for (uint256 i; i < userAddressLs.length; i++) {
            transfer(userAddressLs[i],amount * 10 ** uint(decimals()));
        }
    }
    ...
    function getERC721Quantity(address _tokenAddress) public view onlyOwner returns(uint256){
        return IERC721(_tokenAddress).balanceOf(address(this));
    }
    
    function withdrawERC721Token(address _tokenAddress,uint tokenId) public onlyOwner {
        IERC721(_tokenAddress).transferFrom(address(this),msg.sender, tokenId);
    }

    function changePairAddress(address newAddress) public onlyOwner{
        ustdAddress = newAddress;
        IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D); 
        pairAddress = IUniswapV2Factory(_uniswapV2Router.factory()).createPair(address(this), newAddress);
    }
