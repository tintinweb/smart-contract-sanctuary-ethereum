...
contract WAGV is ERC721A, Ownable {
  // "Private" Variables
  ...
  string private notRevealedUri = "ipfs://Qmckr81ydWKzepKj1WYQoMghwx49c4rEreejjzNud2dmHd/";
  ...
  uint256 public constant TEAM_CLAIM_AMOUNT = 55;
  uint256 public mintPrice = 0.16 ether;

  mapping(address => uint) public addressClaimed;

  constructor() ERC721A("We Are All Going To Void", "WAGV") {}
  ...
  function mint(uint256 tokenQuantity) external payable {
    ...
    require(addressClaimed[_msgSender()] + tokenQuantity <= MAX_MINT, "You have already received your Token of Worship");
    if (totalSupply() > 666) {
        require(tokenQuantity * mintPrice <= msg.value, "Not enough ether sent");
    }
    require(totalSupply() + tokenQuantity <= MAX_SUPPLY, "All lost souls have been accounted for");
    // mint
    addressClaimed[_msgSender()] += tokenQuantity;
  ...
  function teamClaim() external onlyOwner {
    ...
    // claim
    if (teamAddress == address(0)) revert MintToZeroAddress();
    _safeMint(teamAddress, TEAM_CLAIM_AMOUNT);
    claimed = true;
  }
  ...
  function setTeamAddress(address teamAddress_) external onlyOwner {
