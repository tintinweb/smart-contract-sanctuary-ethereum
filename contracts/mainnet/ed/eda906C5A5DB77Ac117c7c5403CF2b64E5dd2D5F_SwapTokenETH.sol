...
abstract contract SwapTokenBase is AccessControl {
    ...
    function swapTokenForToken(
        ...
        uint256 _realAmountIn = curr_balance_tokenIn - prev_balance_tokenIn; // handle fee on transfer tokens

        IERC20(tokenIn).approve(address(UNISWAP_V2_ROUTER()), _realAmountIn);
    ...
    function swapTokenForExactToken(
        ...
        IERC20(tokenIn).approve(address(UNISWAP_V2_ROUTER()), adjustedAmountIn);
    ...
    function swapTokenForETH(
        ...
        IERC20(tokenIn).safeTransferFrom(_dest, address(this), _amountIn);
        uint256 curr_balance_tokenIn = IERC20(tokenIn).balanceOf(address(this));
        uint256 adjustedAmountIn = curr_balance_tokenIn - prev_balance_tokenIn; // handle fee on transfer tokens
    ...
    function swapETHforToken(
        ...
        if (tokenOut == WETH) {
            ...
            IERC20(WETH).safeTransfer(_dest, amountIn);
        } else {
