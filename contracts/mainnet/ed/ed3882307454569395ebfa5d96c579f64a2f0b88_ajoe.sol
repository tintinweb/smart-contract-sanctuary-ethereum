...
contract ajoe is ERC721A, Ownable, ReentrancyGuard {
    ...
    uint256 public RESERVED_AMOUNT = 255;
    uint256 public MAX_SUPPLY = 4000;
    uint256 public PRICE = 0.09 * 10**18; // 0.09 ETH
    uint256 public PUBLIC_SALE_MAX_ALLOWED = 2;
    address public signer = 0x85C08833984550477ca85045556D74A3d82C4484;
    address public marketingWallet = 0xe563a688BE5fEE0e7D14d075af182767E9f223A3;
    mapping(address => uint256)  publicSaleMinted;
    ...
    function mint(uint256 amount) external payable {
        ...
        require(
            publicSaleMinted[msg.sender] + amount <= PUBLIC_SALE_MAX_ALLOWED,
            "Max mint amount per wallet exceeded."
        );
        require(PRICE * amount == msg.value, "Price incorrect.");

        publicSaleMinted[msg.sender] += uint64(amount);
        _safeMint(msg.sender, amount);
    ...
    function setMaxAllowed(uint256 newValue) external onlyOwner {
        PUBLIC_SALE_MAX_ALLOWED = newValue;
    }
    ...
    function burnMulti(uint256[] calldata tokenIds) public virtual {
        for(uint256 i; i < tokenIds.length; ++i){
            require(burnLocked == false, "Burning is not enabled.");
            require(ownerOf(tokenIds[i]) == _msgSender(), "Not owner.");
            _burn(tokenIds[i]);
        }
    }
}