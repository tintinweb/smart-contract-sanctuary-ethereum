...
contract LifeDeath is ERC721A, OperatorFilterer, Ownable {
    ...
    uint256 public MAX_SUPPLY = 2525;
    
    uint256 public FREE_LIMIT = 1;

    uint256 public PAID_PRICE = 0.001 ether;
    uint256 public PAID_PER_TX = 10;
    uint256 public PAID_LIMIT = 20;
    ...
    function freeTokensRemainingForAddress(address who) public view returns (uint256) {
        if (mintState == MintState.Open)
            return FREE_LIMIT - _getAux(who);
        else revert("Mint state mismatch");
    }

    function paidTokensRemainingForAddress(address who) public view returns (uint256) {
        if (mintState == MintState.Open)
            return PAID_LIMIT + _getAux(who) - _numberMinted(who);
        else revert("Mint state mismatch");
    }

    function mintNewLifeFree() external onlyExternallyOwnedAccount {
        uint256 quantity = 1;
    ...
    function mintNewLifePaid(uint256 quantity) external payable onlyExternallyOwnedAccount {
        require(this.totalSupply() + quantity <= MAX_SUPPLY, "Mint exceeds max supply");
        require(quantity <= PAID_PER_TX, "Mint exceeds per transaction amount");
        require(mintState == MintState.Open, "Mint state mismatch");
        require(msg.value >= PAID_PRICE * quantity, "Insufficient value");
    ...
    function withdraw() external onlyOwner {
        ...
        address owner           = 0xFEdc36cD52320c34E694475911a91D926A49Bb58;
