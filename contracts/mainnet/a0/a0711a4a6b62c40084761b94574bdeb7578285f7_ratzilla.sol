...
pragma solidity ^0.8.0;
contract ratzilla is ERC721A, Ownable, ReentrancyGuard {
    // limits
    uint256 public maxPerTransaction = 3;
    uint256 public maxPerWallet = 3;
    uint256 public maxTotalSupply = 6000;
    uint256 public chanceFreeMintsAvailable = 1000;
    uint256 public freeMintsAvailable = 0;
    ...
    constructor() ERC721A("ratzilla", "ratzilla") {}

    function mintPublic(uint256 _amount) external payable nonReentrant {
        require(isPublicLive, "!live");
        require(_amount > 0, "zero");
        require(totalSupply() + _amount <= maxTotalSupply, "exceeds!");
        require(_amount <= maxPerTransaction, "!max per transaction");
        require(mintsPerWallet[_msgSender()] + _amount <= maxPerWallet, "!max per wallet");

        // 1 guaranteed free per wallet if freeMintsAvailable?
        uint256 pricedAmount = _amount;
        if(freeMintsAvailable > 0 && mintsPerWallet[_msgSender()] == 0) {
            pricedAmount = _amount - 1;
            freeMintsAvailable = freeMintsAvailable - 1;
        }
        
        require(mintPrice * pricedAmount <= msg.value, "!enough eth");
    ...
    function mintWhitelist(bytes32[] memory _proof) external nonReentrant {
        require(isWhitelistLive, "!live");
        require(totalSupply() + 1 <= maxTotalSupply, "exceeds total supply");
        require(whitelistMintsPerWallet[_msgSender()] < 1, "exceeds max per wallet");
