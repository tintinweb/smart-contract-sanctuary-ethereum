...
abstract contract SubCollectionHelper is OwnableUpgradeable {
    error InvalidNFT(uint256 _idx);

    IERC721Upgradeable public nftContract;

    function initialize(IERC721Upgradeable _nftContract) external initializer {
        __Ownable_init();
    ...
    function ownerOf(uint256 _idx) external view returns (address) {
        ...
        address account = nftContract.ownerOf(_idx);
    ...
    function transferFrom(address _from, address _to, uint256 _idx) external onlyOwner {
        if (!isValid(_idx))
            revert InvalidNFT(_idx);
        ...
        nftContract.transferFrom(_from, _to, _idx);
    }

    /// @param _from The sender address
    /// @param _to The recipient address
    /// @param _idx The index of the rock to transfer
    function safeTransferFrom(address _from, address _to, uint256 _idx) external onlyOwner {
        if (!isValid(_idx))
            revert InvalidNFT(_idx);
        
        address _owner = owner();
        IERC721Upgradeable _nftContract = nftContract;
