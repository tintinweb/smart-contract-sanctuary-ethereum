...
contract lVault is Ownable {
    ...
    address public sLAI;
    uint public sLAIQueue; // Delays change to sLAI address
    ...
    /**
        @notice allow approved address to deposit an asset for LAI
        @param _amount uint
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        // mint LAI needed and store amount of rewards for distribution
        send_ = value.sub( _profit );
        IERC20Mintable( LAI ).mint( msg.sender, send_ );
    ...
    /**
        @notice allow approved address to burn LAI for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOf( _token, _amount );
        ILAIERC20( LAI ).burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint LAImumDebt = IERC20( sLAI ).balanceOf( msg.sender ); // Can only borrow against sLAI held
        uint availableDebt = LAImumDebt.sub( debtorBalance[ msg.sender ] );
    ...
    /**
        @notice allow approved address to repay borrowed reserves with LAI
        @param _amount uint
     */
    function repayDebtWithLAI( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved" );

        ILAIERC20( LAI ).burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, LAI, _amount, _amount );
    }
    ...
    function mintRewards( address _recipient, uint _amount ) external {
        ...
        IERC20Mintable( LAI ).mint( _recipient, _amount );
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( IERC20( LAI ).totalSupply().sub( totalDebt ) );
    }
    ...
    /**
        @notice returns LAI valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match LAI decimals
            value_ = _amount.mul( 10 ** IERC20( LAI ).decimals() ).div( 10 ** IERC20( _token ).decimals() );
    ...
    function toggle(
        ...
        } else if ( _managing == MANAGING.SLAI ) { // 9
            sLAIQueue = 0;
            sLAI = _address;
            result = true;
