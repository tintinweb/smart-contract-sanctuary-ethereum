...
https://twitter.com/print_ethereum
...
/**
 * @dev PRINT token vester interface
 */
...
contract StakeRewards is IStakeRewards, Ownable {
  ...
  modifier onlyToken() {
    require(
      msg.sender == address(sP) || msg.sender == address(slP),
      'must be stake token'
    );
    _;
  }

  constructor(address _p, address _dexRouter) {
    p = _p;
  ...
  function getsP() external view returns (address) {
    return address(sP);
  }

  function getslP() external view returns (address) {
    return address(slP);
  }
  ...
  function setsP(address _sP) external onlyOwner {
    sP = IERC721(_sP);
  }
...
/**
 * @dev PRINT token interface
 */

interface IP is IERC20 {
...
contract PVester is Ownable {
  ...
  function withdraw(uint256 _index) external {
    ...
    _vest.withdrawsCompleted = _withdrawsAllowed;
    _p.transfer(_user, _tokensPerWithdrawPeriod * _withdrawsToComplete);
    _p.addToBuyTracker(
      _user,
  ...
  function getP() external view returns (address) {
    return address(_p);
  }
...
contract PStake is ERC721Enumerable, Ownable {
  using Strings for uint256;
  ...
  function _stake(
    ...
    _amountStaked = _amountStaked == 0
      ...
      : _amountStaked;
    _amountPBaseEarn = _amountPBaseEarn == 0
      ? _amountStaked
      : _amountPBaseEarn;
    require(
      _amountStaked > 0 && _amountPBaseEarn > 0,
    ...
    stakes[_ids.current()] = Stake({
      ...
      amountPBaseEarn: _amountPBaseEarn,
  ...
  function unstake(uint256 _tokenId) public {
    ...
    if (_isUnstakingEarly) {
      ...
      if (address(stakeToken) == address(p)) {
        p.burn(_tokenStake.amountStaked - _earnedAmount);
      } else {
    ...
    returns (uint256)
  {
    ...
    return
      (_tokenStake.amountPBaseEarn * _tokenStake.apr * _secondsStaked) /
      PERCENT_DENOMENATOR /
      ONE_YEAR;
  }
  ...
  function _afterTokenTransfer(
    ...
    if (_from != address(0)) {
      ...
      rewards.setShare(_from, _tokenStake.amountPBaseEarn, true);
    }
    ...
    if (_to != address(0)) {
      ...
      rewards.setShare(_to, _tokenStake.amountPBaseEarn, false);
    }
...
contract slP is PStake {
  ...
  function zapAndStakeETHOnly(uint256 _lockOptIndex) external payable {
    ...
    uint256 _pBalanceBefore = p.balanceOf(address(this));
    IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(_uniswapRouter);

    // swap half the ETH for PRINT
    uint256 _tokensToReceiveNoSlip = _getTokensToReceiveOnBuyNoSlippage(
      msg.value / 2
    );
    address[] memory path = new address[](2);
    ...
    _addLp(p.balanceOf(address(this)) - _pBalanceBefore, msg.value / 2);
    ...
    _returnExcessP(msg.sender, _pBalanceBefore);

    emit ZapETHOnly(msg.sender, msg.value, _lpBalanceToStake);
  }

  function zapAndStakeETHAndP(uint256 _amountP, uint256 _lockOptIndex)
    external
    payable
  {
    require(msg.value > 0, 'need to provide ETH to zap');
    ...
    uint256 _ethBalBefore = address(this).balance - msg.value;
    uint256 _pBalBefore = p.balanceOf(address(this));
    p.transferFrom(msg.sender, address(this), _amountP);
    uint256 _pToProcess = p.balanceOf(address(this)) - _pBalBefore;

    uint256 _lpBalBefore = stakeToken.balanceOf(address(this));
    _addLp(_pToProcess, msg.value);
    uint256 _lpBalanceToStake = stakeToken.balanceOf(address(this)) -
      _lpBalBefore;
    _stakeLp(msg.sender, _lpBalanceToStake, _lockOptIndex, false);

    _returnExcessETH(msg.sender, _ethBalBefore);
    _returnExcessP(msg.sender, _pBalBefore);

    emit ZapETHAndP(msg.sender, msg.value, _amountP, _lpBalanceToStake);
  }

  function zapAndStakePOnly(uint256 _amountP, uint256 _lockOptIndex)
    external
  {
    require(
      _aprLockOptions[_lockOptIndex].lockTime > 0,
      'cannot zap and stake P only without lockup period'
    );
    ...
    p.approve(address(_uniswapV2Router), _pToProcess / 2);
    ...
    uint256 _lpBalBefore = stakeToken.balanceOf(address(this));
    _addLp(_pToProcess / 2, address(this).balance - _ethBalBefore);
    uint256 _lpBalanceToStake = stakeToken.balanceOf(address(this)) -
      _lpBalBefore;
    _stakeLp(msg.sender, _lpBalanceToStake, _lockOptIndex, false);
    ...
    emit ZapPOnly(msg.sender, _amountP, _lpBalanceToStake);
  }

  function _addLp(uint256 tokenAmount, uint256 ethAmount) private {
    IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(_uniswapRouter);
    p.approve(address(_uniswapV2Router), tokenAmount);
  ...
  function _getETHToReceiveOnSellNoSlippage(uint256 _amountP)
    internal
    view
    returns (uint256)
  {
    ...
    (uint112 _r0, uint112 _r1, ) = pair.getReserves();
    if (pair.token0() == IUniswapV2Router02(_uniswapRouter).WETH()) {
      return (_amountP * _r0) / _r1;
    } else {
      return (_amountP * _r1) / _r0;
    }
  }
  ...
  function _stakeLp(
    ...
    uint256 pReserves = t0 == address(p) ? res0 : res1;
    uint256 singleSideTokenAmount = (_amountStakeToken * pReserves) /
  ...
  function _returnExcessP(address _user, uint256 _initialBal) internal {
    uint256 _currentBal = p.balanceOf(address(this));
    if (_currentBal > _initialBal) {
      p.transfer(_user, _currentBal - _initialBal);
      require(p.balanceOf(address(this)) >= _initialBal, 'took too much');
    }
  }
...
contract PRINT is IP, ERC20, Ownable {
  ...
  sP private _stakedP;
  slP private _stakedPLP;
  PVester private _vester;
  ...
  modifier onlyStake() {
    require(
      address(_stakedP) == _msgSender() ||
        address(_stakedPLP) == _msgSender(),
  ...
  constructor() ERC20('PRINT', 'PRINT') {
    ...
    _vester = new PVester(address(this));
    ...
    _stakedP = new sP(
      ...
      address(_vester),
      address(_rewards),
      'https://api.print.com/sp/metadata/'
    );
    _stakedP.setPaymentAddress(msg.sender);
    _stakedP.setRoyaltyAddress(msg.sender);
    _stakedP.transferOwnership(msg.sender);
    ...
    _stakedPLP = new slP(
      ...
      address(_vester),
      address(_rewards),
      'https://api.print.com/slp/metadata/'
    );
    _stakedPLP.setPaymentAddress(msg.sender);
    _stakedPLP.setRoyaltyAddress(msg.sender);
    _stakedPLP.transferOwnership(msg.sender);

    _vester.addStakingContract(address(_stakedP));
    _vester.addStakingContract(address(_stakedPLP));
    _vester.transferOwnership(msg.sender);

    _rewards.setsP(address(_stakedP));
    _rewards.setslP(address(_stakedPLP));
    ...
    isTaxExcluded[address(_stakedPLP)] = true; // allow zapping without taxes
  ...
  function _transfer(
    ...
    // handles transferring to a fresh wallet or wallet that hasn't bought PRINT before
  ...
  function getStakedP() external view returns (address) {
    return address(_stakedP);
  }

  function getStakedPLP() external view returns (address) {
    return address(_stakedPLP);
  }
