...
contract StakingRewards is IStakingRewards, RewardsDistributionRecipient, ReentrancyGuard {
    ...
    /// @notice Early unstaking tax
    uint256 public earlyUnstakeTax = 10;
    /// @notice Address to send the early unstaking tax to
    address taxAddress;
    ...
    function withdraw(uint256 amount) public override nonReentrant updateReward(msg.sender) {
        ...
        //unstaking too early -> pay fee
        if(lockEnd[msg.sender] >= block.timestamp){
            uint256 _unstakeTax = earlyUnstakeTax.mul(amount).div(100);
            uint256 _amountToSend = amount.sub(_unstakeTax);
            stakingToken.safeTransfer(taxAddress, _unstakeTax);
            stakingToken.safeTransfer(msg.sender, _amountToSend);
        } else stakingToken.safeTransfer(msg.sender, amount);
    ...
    /// @notice Update early unstake tax
    /// @param _earlyUnstakeTax New unstake tax
    function setEarlyUnstakeTax(uint256 _earlyUnstakeTax) external onlyOwner {
        earlyUnstakeTax = _earlyUnstakeTax;
    }

    /// @notice Update the reward token
    /// @param _rewardsToken is the new reward token address
    function setRewardToken(address _rewardsToken) external onlyOwner {
        emit RewardTokenUpdated(rewardsToken, IERC20(_rewardsToken));
        rewardsToken = IERC20(_rewardsToken);
    }
