...
// File: Blimpie/ERC721EnumerableLite.sol
...
contract MutantCrew is Delegated, ERC721EnumerableLite {
    //set up our TOKENURI variables and supply and contract state
    //STATES 0 = deployed, not minting || 1 = minting, ready for rng || 2 = revealed
    ...
    uint256 public price = 1.7 ether;

    constructor()
    Delegated()
    ERC721B("Mutant Crew", "MCREW", 0)
    {}

    // change base token URI in case metadata needs to be updated for any reason
    function updateBaseURI(string calldata _baseTokenURI) external onlyDelegates {
        baseURI = _baseTokenURI;
    }

    // change mint price 
    function updatePrice(uint256 _price) external onlyDelegates {
        price = _price;
    }

    // opens mint - this will cause metaMint() to be called and token 0 will be minted 
    function openMint() external onlyOwner {
        require(state == 0, "Already open!");
        metaMint();
        state = 1;
    }

    // generates random number for tokenURI generation based on block difficulty and timestamp of the block this tx is included in
    function reveal() external onlyDelegates {
        require(state == 1, "Not ready for reveal!");
        randNum = uint(keccak256(abi.encodePacked(block.difficulty, block.timestamp)));
        state = 2;
    }
        ...
        returns (string memory)
    {
        ...
        //if we have revealed, set our metadata
        if (state == 2) {
            //set metas tokenid0 to URI 0
            if (_tokenId == 0) {
                return
                    string(
                ...
                if ((randNum % 10) + _tokenId < MAX_SUPPLY) {
                    return
                        string(
                            abi.encodePacked(
                                baseURI,
            ...
            return string(abi.encodePacked(baseURI, "11"));
        }
    }

    //this function is to save tokenid 0 for Meta. This will get called when mint is opened.
    function metaMint() internal {
        require(!_exists(0), "Token 0 already minted.");

        //mint our token and increment our count
        _mint(owner(), 0);
        tokenCount++;
    }

    function mint(uint256 amount_) public payable {
        require(state > 0, "Minting is not open");
        require(price * amount_ == msg.value, "Invalid funds provided.");
        require(tokenCount + amount_ <= MAX_SUPPLY, "Minting would exceed max supply.");
