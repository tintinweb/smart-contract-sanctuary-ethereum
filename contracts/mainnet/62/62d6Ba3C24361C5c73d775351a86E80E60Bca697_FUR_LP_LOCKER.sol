...
/**
 * @dev This contract allows owner to lock an ERC721 NFT (in this case a UniswapV3 LP token)
 * and at some point in the future trigger a 6-month timer. After the timer expires, owner can withdraw the locked NFT.
 * no other action may be taken in the interim, including claiming fees.
 */
contract FUR_LP_LOCKER {
    ...
    constructor(address _nftContract) { // UniswapV3 Positions NFT - 0xC36442b4a4522E871399CD717aBDD847Ab11FE88
        lpOwner = msg.sender;
    ...
    function lockNFT(uint256 tokenId) external onlylpOwner {
    ...
    function triggerWithdrawal() external onlylpOwner {
    ...
    function cancelWithdrawalTrigger() external onlylpOwner {
        require(isNFTLocked, "NFT is not locked");
    ...
    function withdrawNFT(uint256 tokenId) external onlylpOwner {
        require(isNFTLocked, "NFT is not locked");
        require(lockUpEndTime != 0, "Withdrawal is not triggered");
    ...
    function changeOwner(address newOwner) external onlylpOwner {
