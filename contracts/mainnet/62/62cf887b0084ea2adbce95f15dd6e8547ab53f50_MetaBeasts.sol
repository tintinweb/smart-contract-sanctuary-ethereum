...
contract MetaBeasts is ERC1155, Ownable, ERC1155Burnable, ERC1155Supply {
    ...
    uint256 public MB_TEAM_RESERVE = 250;
    uint256 public MB_PUBLIC = 250;
    uint256 public MB_PRIVATE = 2000;
    uint256 public MB_PUBLIC_PRICE = 0.13 ether;
    uint256 public MB_PRIVATE_PRICE = 0.10 ether;
    uint256 public MB_PER_WALLET = 2;
    ...
    address public teamWallet = 0x8F2232D2c9480fd38570562Dc2b437c9e9c7f944;

    constructor() ERC1155("https://gateway.pinata.cloud/ipfs/QmWSA5jL9eNg9xgjtSbRi9Dru1uCPpZZco8xgB6NcqpZm3/{id}") {
    ...
    function gift(address[] calldata receivers) external onlyTeam {
        require(teamChestsMinted + (receivers.length / 2) <= MB_TEAM_RESERVE, "EXCEED_TEAM_RESERVE");
    ...
    function founderMint(uint256 tokenQuantity) external onlyTeam {
        require(teamChestsMinted + (tokenQuantity / 2) <= MB_TEAM_RESERVE, "EXCEED_TEAM_RESERVE");
    ...
    function giftChest(address[] calldata receivers) external onlyTeam {
        require(teamChestsMinted + receivers.length <= MB_TEAM_RESERVE, "EXCEED_TEAM_RESERVE");
    ...
    function founderMintChest(uint256 tokenQuantity) external onlyTeam {
        require(teamChestsMinted + tokenQuantity <= MB_TEAM_RESERVE, "EXCEED_TEAM_RESERVE");
    ...
    function buyChests(uint256 quantity) external payable notContract {
        ...
        require(MB_PUBLIC_PRICE * quantity <= msg.value, "INSUFFICIENT_ETH");
    ...
    function privateBuy(uint256 quantity, bytes memory signature) external payable notContract {
        ...
        require(MB_PRIVATE_PRICE * quantity <= msg.value, "INSUFFICIENT_ETH");
    ...
    function setSigner(address newSigner) external onlyTeam {
        PRIVATE_SIGNER = newSigner;
    }

    function setPublicPrice(uint256 newPrice) external onlyTeam {
        MB_PUBLIC_PRICE = newPrice;
    }

    function setPrivatePrice(uint256 newPrice) external onlyTeam {
        MB_PRIVATE_PRICE = newPrice;
    }
    ...
    function withdraw() external onlyTeam {
        ...
        Address.sendValue(payable(0x00000040f69B8E3382734491cBAA241B6a863AB3), currentBalance * 525 / 10000);
