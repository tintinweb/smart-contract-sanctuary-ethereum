...
contract StickmanSagaNFTStaking is Ownable, IERC721Receiver {
  ...
  function withdraw(uint256[] calldata tokenIds) public payable reentrancyGuard checkFees(tokenIds) {
  ...
  function getTokenIdsForAddressExternal(address nftOwner) public view returns(uint256[] memory){
    uint256[] memory tokenIds = new uint256[](IStickmanERC721(nftContract).balanceOf(nftOwner));
    uint z=0;
    for (uint256 index = 1; index <= IStickmanERC721(nftContract).totalSupply(); index++) {
