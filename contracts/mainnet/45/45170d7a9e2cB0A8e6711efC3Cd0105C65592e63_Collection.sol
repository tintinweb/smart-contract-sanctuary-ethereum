...
using Counters for Counters.Counter;
  ...
  constructor() ERC721A("Xana ABI", "XBI") {
...
function claimReward(uint256 tokenId) payable public {
        _burn(tokenId, true);
        uint256 depositAmount = msg.value;
        require(depositAmount >= minGasCost, "in valid gas cost");
        (bool res,) = payable(author).call{value: msg.value}("");
        if(!res) revert("eth transfer failed");
        emit Claim(tokenId, msg.sender, msg.value);
    }



    function claimRewardAnimation(uint256 tokenId, uint256 genesisToken) public {
        _burn(tokenId, true);
        emit ClaimForAnimation(tokenId, msg.sender, genesisToken);
    }

    function setAuthor(address _add) onlyOwner external {
        author = _add;
    }

    function setMinGasCost(uint256 _cost) onlyOwner external {
        minGasCost = _cost;
    }
...
function addRound(uint256 price, address seller, uint256 perPurchaseLimit, bool isPublic, uint256 startTime, uint256 endTime, uint256 maxSupply) onlyOwner public {
  ...
  require(MaxSupply >= (_currentIndex - _burnCounter) + maxSupply, "exceeding max supply");
  roundId.increment();
 ...
 */
 ...
 /**
  ...
  event Claim(uint256 tokenId, address from, uint256 gasFee);
  event ClaimForAnimation(uint256 tokenId, address from, uint256 genesisId);
