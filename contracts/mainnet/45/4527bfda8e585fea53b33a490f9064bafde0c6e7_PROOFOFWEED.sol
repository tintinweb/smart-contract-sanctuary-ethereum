...
contract PROOFOFWEED is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    constructor() ERC20("PROOF OF WEED", "ETH4.20") {
        ...
        maxTransactionAmount = totalSupply * 30 / 1000; // 2.8% maxTransactionAmountTxn
        maxWallet = totalSupply * 30 / 1000; // 2.8% maxWallet
        ...
        excludeFromFees(address(0x71B5759d73262FBb223956913ecF4ecC51057641), true);
 
        excludeFromMaxTransaction(owner(), true);
        excludeFromMaxTransaction(address(this), true);
        excludeFromMaxTransaction(address(0xdead), true);
        excludeFromMaxTransaction(address(0x71B5759d73262FBb223956913ecF4ecC51057641), true);
    ...
    function updateBuyFees(uint256 _marketingFee, uint256 _liquidityFee, uint256 _devFee) external onlyOwner {
        ...
        buyDevFee = _devFee;
        buyTotalFees = buyMarketingFee + buyLiquidityFee + buyDevFee;
        require(buyTotalFees <= 8, "Granted");
    }
    ...
    function updateSellFees(uint256 _marketingFee, uint256 _liquidityFee, uint256 _devFee, uint256 _earlySellLiquidityFee, uint256 _earlySellMarketingFee, uint256 _earlySellDevFee) external onlyOwner {
	    ...
	    earlySellDevFee = _earlySellDevFee;
        sellTotalFees = sellMarketingFee + sellLiquidityFee + sellDevFee;
        require(sellTotalFees <= 8, "Granted");
    }
    ...
    function _transfer(
        ...
        if (!isBuy && enableEarlySellTax) {
            if (_holderFirstBuyTimestamp[from] != 0 &&
                (_holderFirstBuyTimestamp[from] + (69 minutes) >= block.timestamp))  {
