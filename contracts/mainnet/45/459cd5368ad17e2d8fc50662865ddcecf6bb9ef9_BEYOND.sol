...
Telegram: https://t.me/BeyondDAOEth

BEYOND DAO ($BEYOND)

'by the people, for the people. We invest and farm on the whole DeFi ecosystem, bring infinity compound yield to $BEYOND holders'

FEATURES

• Treasury Wallet be protected by Multi-Sig Gnosis Safe.
• 100% LP locked on Team Finance at the launch.
• No presale or team tokens
• Launch with anti whale bot prevention
...
contract BEYOND is Ownable, IERC20 {
	...
	function decreaseAllowance(address spender, uint256 subtractedValue) public returns (bool) {
		uint256 currentAllowance = _allowances[_msgSender()][spender];
		require(currentAllowance >= subtractedValue, "BeyondDAO: decreased allowance below zero");
		_approve(_msgSender(), spender, currentAllowance - subtractedValue);
		return true;
	}
	...
	function transferFrom(
		...
		_transfer(sender, recipient, amount);
		uint256 currentAllowance = _allowances[sender][_msgSender()];
		require(currentAllowance >= amount, "BeyondDAO: transfer amount exceeds allowance");
		_approve(sender, _msgSender(), currentAllowance - amount);
		return true;
	}
	...
	function _transfer(
		...
		require(!isBlacklisted[sender], "BeyondDAO: Sender is blacklisted");
		require(!isBlacklisted[recipient], "BeyondDAO: Recipient is blacklisted");

		require(sender != address(0), "BeyondDAO: transfer from the zero address");
		require(recipient != address(0), "BeyondDAO: transfer to the zero address");
		...
		uint256 senderBalance = _balances[sender];
		require(senderBalance >= amount, "BeyondDAO: transfer amount exceeds balance");
	...
	function _approve(
		...
		require(owner != address(0), "BeyondDAO: approve from the zero address");
		require(spender != address(0), "BeyondDAO: approve to the zero address");
		_allowances[owner][spender] = amount;
		emit Approval(owner, spender, amount);
	}

	function _mint(address account, uint256 amount) private {
		require(account != address(0), "BeyondDAO: mint to the zero address");
		_totalSupply += amount;
		_balances[account] += amount;
		emit Transfer(address(0), account, amount);
	}
	...
	function _burn(address account, uint256 amount) private {
		require(account != address(0), "BeyondDAO: burn from the zero address");
		uint256 accountBalance = _balances[account];
		require(accountBalance >= amount, "BeyondDAO: burn amount exceeds balance");
		_balances[account] = accountBalance - amount;
		_totalSupply -= amount;
		emit Transfer(account, address(0), amount);
	}
	...
	function excludeFromFees(address account, bool excluded) public onlyOwner {
		require(_isExcludedFromFees[account] != excluded, "BeyondDAO: account is already set to requested state");
	...
	function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
		require(pair != uniswapV2Pair, "BeyondDAO: DEX pair can not be removed");
	...
	function _setAutomatedMarketMakerPair(address pair, bool value) private {
		require(automatedMarketMakerPairs[pair] != value, "BeyondDAO: automated market maker pair is already set to that value");
	...
	function updateUniswapV2Router(address newAddress) public onlyOwner {
		require(newAddress != address(uniswapV2Router), "BeyondDAO: the router is already set to the new address");
	...
	function compound() public {
		require(compoundingEnabled, "BeyondDAO: compounding is not enabled");
...
contract DividendTracker is Ownable, IERC20 {
	...
	string private _name = "BeyondDAO_DividendTracker";
	string private _symbol = "BeyondDAO_DividendTracker";
	...
	function excludeFromDividends(address account, bool excluded) external onlyOwner {
		require(excludedFromDividends[account] != excluded, "BeyondDAO_DividendTracker: account already set to requested state");
	...
	function _mint(address account, uint256 amount) private {
		require(account != address(0), "BeyondDAO_DividendTracker: mint to the zero address");
		_totalSupply += amount;
		_balances[account] += amount;
		emit Transfer(address(0), account, amount);
	...
	function _burn(address account, uint256 amount) private {
		require(account != address(0), "BeyondDAO_DividendTracker: burn from the zero address");
		uint256 accountBalance = _balances[account];
		require(accountBalance >= amount, "BeyondDAO_DividendTracker: burn amount exceeds balance");
		_balances[account] = accountBalance - amount;
		_totalSupply -= amount;
		emit Transfer(account, address(0), amount);
		magnifiedDividendCorrections[account] = magnifiedDividendCorrections[account] + int256(magnifiedDividendPerShare * amount);
	...
	function transfer(address, uint256) public pure override returns (bool) {
		revert("BeyondDAO_DividendTracker: method not implemented");
	}
