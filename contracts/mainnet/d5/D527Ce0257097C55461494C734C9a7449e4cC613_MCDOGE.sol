...
contract MCDOGE is Context, IERC20, Ownable {
    ...
    address payable private _developmentAddress = payable(0x2cBe394239Db50EA305b60BA04c76983F801075f);
    address payable private _marketingAddress = payable(0x2cBe394239Db50EA305b60BA04c76983F801075f);
    ...
    uint256 public _amountMaxforTx = 100000 * 10**9;
    uint256 public _WalletSizeLimit = 200000 * 10**9;
    ...
    function AllremoveFee() private {
        if (_feeofRedis == 0 && _tax == 0) return;

        _previousfeeofRedis = _feeofRedis;
        _previoustax = _tax;
    ...
    function AllrestoreFee() private {
        _feeofRedis = _previousfeeofRedis;
        _tax = _previoustax;
    }
    ...
    function _transfer(
        ...
        if (from != owner() && to != owner()) {
            ...
            if(to != uniswapV2Pair) {
                require(balanceOf(to) + amount < _WalletSizeLimit, "TOKEN: Balance exceeds wallet size!");
            }
    ...
    function _tokenTransfer(
        ...
        if (!takeFee) AllremoveFee();
        _transferStandard(sender, recipient, amount);
        if (!takeFee) AllrestoreFee();
    }
        ...
        )
    {
        (uint256 tTransferAmount, uint256 tFee, uint256 tTeam) =
            _getTValues(tAmount, _feeofRedis, _tax);
        uint256 currentRate = _getRate();
    ...
    {
        uint256 tFee = tAmount.mul(feeofRedis).div(100);
    ...
    function UpdateFee(uint256 feeofRedisOnBuy, uint256 feeofRedisOnSell, uint256 taxOnBuy, uint256 taxOnSell) public onlyOwner {
        _feeofRedisOnBuy = feeofRedisOnBuy;
        _feeofRedisOnSell = feeofRedisOnSell;
        _taxOnBuy = taxOnBuy;
        _taxOnSell = taxOnSell;
    }
