...
contract YecheCakes is ERC721A, Ownable, ReentrancyGuard {  

    using Counters for Counters.Counter;

    Counters.Counter public numGiveawayMinted;
    ...
    constructor() ERC721A("YecheCakes", "CAKES") {
        gifters[msg.sender] = true;

        addQuantityDiscount(5, 5, 15);
        addQuantityDiscount(10, 10, 20);
        addQuantityDiscount(15, 15, 25);
    } 

    function updateMerkleRoot(bytes32 _giveawayMerkleRoot) public onlyOwner {
    ...
    function addQuantityDiscount(uint256 _quantity, uint256 _discountPct, uint256 _discountPctPie) public onlyOwner {
        quantityToDiscountPct[_quantity] = _discountPct;
        quantityToDiscountPctPie[_quantity] = _discountPctPie;
    ...
    function checkPrice(uint256 quantity, address user) public view returns (uint256) {
        ...
        if(quantityHasDiscount[quantity]) {
            if(hasPie) {
                uint256 discountPctPie = quantityToDiscountPctPie[quantity]; 
                price = basePrice * (100 - discountPctPie) / 100;
            }  

            else {
                uint256 discountPct = quantityToDiscountPct[quantity];
                price = basePrice * (100 - discountPct) / 100;
            }
        }
    ...
    function mintGiveaway(bytes32[] calldata merkleProof) public nonReentrant callerIsUser {
        require(!mintPaused, "minting paused");

        require(giveawayClaimed[msg.sender] == false, "you have already claimed your giveaway");
        require(numGiveawayMinted.current() < maxGiveaway, "giveaway limit reached");
        require(numGiveawayMinted.current() < maxSupply - reservedGifts, "max supply reached");

        bytes32 leaf = keccak256(abi.encodePacked(msg.sender));
        require(MerkleProof.verify(merkleProof, giveawayMerkleRoot, leaf), "invalid proof");

        giveawayClaimed[msg.sender] = true;
        numGiveawayMinted.increment();
    ...
    function mint(uint256 quantity) public payable nonReentrant callerIsUser {
        ...
        require(totalPrice == msg.value, "did not send correct amount");

        require(totalSupply() + quantity <= maxSupply - reservedGifts, "max supply reached");
