...
contract OnnaMusha is ERC721AQueryable, ERC721ABurnable, Ownable, DefaultOperatorFilterer {
    ...
    mapping(address => uint256) public wlMintedByWallet;
    ...
    function mint(uint256 amount) external payable {
        ...
        require(saleState == 2, "Sale is closed!");
    ...
    function mintwl(uint256 amount, uint256 allowance, bytes32[] memory proof) external payable {
        require(msg.sender == tx.origin, "not allowed");
        require(saleState == 1, "Sale is closed!");
        require(_totalMinted() + amount <= MAX_SUPPLY, "Exceed MAX_SUPPLY");
        require(amount > 0, "Amount can't be 0");
        require(verify(proof, msg.sender, allowance),"Not Whitelisted");
        require(amount + wlMintedByWallet[msg.sender] <= allowance, "Exceed maxByWallet");

        wlMintedByWallet[msg.sender] += amount;
