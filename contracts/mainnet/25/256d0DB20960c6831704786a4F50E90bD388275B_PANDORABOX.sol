...
/*
 _______    ______   __     __  ________         ______   __    __      __ 
|       \  /      \ |  \   |  \|        \       /      \ |  \  |  \    /  \
| $$$$$$$\|  $$$$$$\| $$   | $$| $$$$$$$$      |  $$$$$$\| $$   \$$\  /  $$
| $$  | $$| $$__| $$| $$   | $$| $$__          | $$__| $$| $$    \$$\/  $$ 
| $$  | $$| $$    $$ \$$\ /  $$| $$  \         | $$    $$| $$     \$$  $$  
...
contract PANDORABOX is IERC20, Context, Ownable {
    ...
    string private constant _name = "Pandora Box";
    string private constant _symbol = "pBOX";

    uint256 public Pandora_Max_Buy_Limit = 50000 * 10**9;
    uint256 public Pandora_Max_Sell_Limit = 50000 * 10**9;

    address private Pandora_Marketing_Wallet = 0x3d2ad72043Ed81F9C8BDd7518998d6f413874e47;
    address private Pandora_Team_Wallet = 0x39D9693a340ADdBDB6AEAcA668105B2da7D1aB7e;

    uint256 public Pandora_Max_Wallet_Limit = 50000  * 10**9;
    ...
    Taxes public sellTaxes = Taxes(0, 70, 10, 10);
    Taxes public taxes = Taxes(0, 70, 10, 10);
    ...
    constructor (address routerAddress) {
        ...
        _isExcludedFromFee[Pandora_Marketing_Wallet] = true;
        _isExcludedFromFee[Pandora_Team_Wallet] = true;
        
        allowedTransfer[address(this)] = true;
        allowedTransfer[owner()] = true;
        allowedTransfer[pair] = true;
        allowedTransfer[Pandora_Marketing_Wallet] = true;
        allowedTransfer[Pandora_Team_Wallet] = true;
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if(from == pair && !_isExcludedFromFee[to] && !swapping){
            require(amount <= Pandora_Max_Buy_Limit, "You are exceeding Pandora_Max_Buy_Limit");
            require(balanceOf(to) + amount <= Pandora_Max_Wallet_Limit, "You are exceeding Pandora_Max_Wallet_Limit");
        }
        
        if(from != pair && !_isExcludedFromFee[to] && !_isExcludedFromFee[from] && !swapping){
            require(amount <= Pandora_Max_Sell_Limit, "You are exceeding Pandora_Max_Sell_Limit");
            if(to != pair){
                require(balanceOf(to) + amount <= Pandora_Max_Wallet_Limit, "You are exceeding Pandora_Max_Wallet_Limit");
            }
            if(coolDownEnabled){
    ...
    function swapAndLiquify(uint256 contractBalance, Taxes memory temp) private lockTheSwap{
        ...
        if(marketingAmt > 0){
            payable(Pandora_Marketing_Wallet).sendValue(marketingAmt);
        }
        uint256 teamAmt = unitBalance * 2 * temp.team;
        if(teamAmt > 0){
            payable(Pandora_Team_Wallet).sendValue(teamAmt);
        }
    }
    ...
    function updateMaxTxLimit(uint256 maxBuy, uint256 maxSell) external onlyOwner{
        Pandora_Max_Buy_Limit = maxBuy * 10**decimals();
        Pandora_Max_Sell_Limit = maxSell * 10**decimals();
    }
    
    function updatePandora_Max_Wallet_Limit(uint256 amount) external onlyOwner{
        Pandora_Max_Wallet_Limit = amount * 10**decimals();
    }
