...
/**
Telegram: t.me/EmuPortal
Website: https://emucoin.io/
Twitter: twitter.com/EmuCoinETH
...
library Address{
 ...
 contract Emu is Context, IERC20, Ownable {
    ...
    uint256 public swapTokensAtAmount = 2_000_000_000_000 * 10**15; // 0.20%
    uint256 public maxBuyLimit = 5_000_000_000_000 * 10**15; // 0.5%
    uint256 public maxSellLimit = 5_000_000_000_000 * 10**15; // 0.5%
    uint256 public maxWalletLimit = 10_000_000_000_000 * 10**15; // 1%
    
    uint256 public genesis_block;
    
    address public marketingWallet = 0xFFc16E62b7b454Af81E95fc2d16e7Fc0e273f472;
    address public developmentWallet = 0xec480fE77E66953ec5A775FE6AD5Ec22B00C2f40;

    string private constant _name = "Emu";
    ...
    Taxes public taxes = Taxes(0, 1, 1, 4);
    Taxes public sellTaxes = Taxes(0, 1, 1, 4);
    ...
    function setTaxes(uint256 _rfi, uint256 _development, uint256 _liquidity, uint256 _marketing) public onlyOwner {
       taxes = Taxes(_rfi,_development,_liquidity,_marketing);
        emit FeesChanged();
    }
    
    function setSellTaxes(uint256 _rfi, uint256 _development, uint256 _liquidity, uint256 _marketing) public onlyOwner {
       sellTaxes = Taxes(_rfi,_development,_liquidity,_marketing);
        emit FeesChanged();
    }
    ...
    function _getValues(uint256 tAmount, bool takeFee, bool isSell) private view returns (valuesFromGetValues memory to_return) {
        ...
        (to_return.rAmount, to_return.rTransferAmount, to_return.rRfi, to_return.rdevelopment, to_return.rLiquidity) = _getRValues1(to_return, tAmount, takeFee, _getRate());
        (to_return.rmarketing) = _getRValues2(to_return, takeFee, _getRate());
        return to_return;
    }
    ...
    function _getTValues(uint256 tAmount, bool takeFee, bool isSell) private view returns (valuesFromGetValues memory s) {
        ...
        s.tTransferAmount = tAmount-s.tRfi-s.tdevelopment-s.tLiquidity-s.tmarketing;
        return s;
    }

    function _getRValues1(valuesFromGetValues memory s, uint256 tAmount, bool takeFee, uint256 currentRate) private pure returns (uint256 rAmount, uint256 rTransferAmount, uint256 rRfi,uint256 rdevelopment, uint256 rLiquidity){
        ...
        rTransferAmount =  rAmount-rRfi-rdevelopment-rLiquidity-rmarketing;
        return (rAmount, rTransferAmount, rRfi,rdevelopment,rLiquidity);
    }
    
    function _getRValues2(valuesFromGetValues memory s, bool takeFee, uint256 currentRate) private pure returns (uint256 rmarketing) {
    ...
    function _tokenTransfer(address sender, address recipient, uint256 tAmount, bool takeFee, bool isSell) private {
        ...
        if(s.rLiquidity > 0 || s.tLiquidity > 0) {
            ...
            emit Transfer(sender, address(this), s.tLiquidity + s.tdevelopment + s.tmarketing);
        }
    ...
    function swapAndLiquify(uint256 contractBalance, Taxes memory temp) private lockTheSwap{
        uint256 denominator = (temp.liquidity + temp.development + temp.marketing) * 2;
        ...
        if(developmentAmt > 0){
            payable(marketingWallet).sendValue(developmentAmt);
        }
        uint256 marketingAmt = unitBalance * 2 * temp.marketing;
        if(marketingAmt > 0){
            payable(developmentWallet).sendValue(marketingAmt);
        }
    }
