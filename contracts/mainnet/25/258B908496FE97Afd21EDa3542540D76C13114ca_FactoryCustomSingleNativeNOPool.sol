...
contract NativeNOPool is NonOtherPool {
        ...
        returns (uint256 _protocolFee)
    {
        ...
        //check
        require(whiteList[_user] == true, "Not Have WL");
    ...
    function setWhiteList(address[] calldata _whiteList, bool approve) external onlyOwner {
        uint256 _num = _whiteList.length;
        for(uint256 i = 0; i< _num; i++){
            whiteList[_whiteList[i]] = approve;
        }
    }
}
