...
// File: tiny-erc721/contracts/TinyERC721.sol
...
contract BunnyPunks is TinyERC721, Ownable, DefaultOperatorFilterer {
    ...
    mapping(address => uint256) public addressMintedBalanceFree;

    constructor(string memory _baseURI, address _lead)
        TinyERC721("Bunny Punks", "BUNNYPUNK", 0)
    {
        baseURI = _baseURI;
        lead = _lead;
    }
        ...
        callerIsUser
    {
        ...
        require(
            addressMintedBalance[msg.sender] < maxPerWalletPaid,
            "Over maxPerWalletPaid"
        );
    ...
    function freeMint(uint256 _quantity) public publicnotPaused callerIsUser {
        require(_quantity <= maxPerTxFree, "Over Tx Limit");
        require(_quantity + supply <= maxSupply, "SoldOut");
        require(
            addressMintedBalanceFree[msg.sender] < maxPerWalletFree,
            "Over MaxPerWallet"
        );
        addressMintedBalanceFree[msg.sender] += _quantity;

        _safeMint(msg.sender, _quantity);
    }

    function airdrop(uint256 _quanitity) public onlyOwner {
