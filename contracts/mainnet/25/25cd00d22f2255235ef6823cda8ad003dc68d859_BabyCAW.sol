...
/*
    Website: https://babycawcoin.com/
    Twitter: https://twitter.com/thebabycaw
    Telegram: https://t.me/babycawcoin

    Contract Name: BabyCAW
    Contract Version: 1.20
    Contract Supply: 333,333,333,333,333 /333 Tillion
    Contract Tokenomics:
    4% Buyback.
    2% Liquidity.
    4% Marketing.
    ...
    Deployed on Ethereum Blockchain under:
    Owner can't change max tx amount.
    Owner can’t blacklist an address.
    Owner can't pause trading.
    Owner can't mint tokens.
    Owner can’t set buy and sell fees higher than 10%.
...
contract BabyCAW is Context, IERC20, Ownable {
    ...
    address payable public marketingWalletAddress = payable(0xDa876E778009A864d5286CDb4D7f9f20D539c132); // Marketing Wallet Address
    address payable public buybackWalletAddress = payable(0xbc2A0614f78dA5C8192E97191AAfE67950146bc2); // buyback Wallet Address
    address payable public devWalletAddress = payable(0x219Ffad28740628653DAa9447f33210267738D52); // dev Wallet Address
    ...
    uint256 private _tTotal = 333_333_333_333_333 * 10**18;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    string private _name = "BabyCAW";
    string private _symbol = "BabyCAW";
    ...
    uint256 public totalSwapableFee = _liquidityFee.add(_devFee).add(_marketingFee).add(_buybackFee);
    ...
    uint256 public totalSwapableSaleFee = _saleLiquidityFee +_saleMarketingFee + _saleBuybackFee;
    ...
    uint256 public buybackTokensCollected = 0;
    ...
    {   
        ...
        uint256 walletsTotal = devTokensCollected.add(marketingTokensCollected).add(buybackTokensCollected);

        uint256 ethForMarketing = newBalance.mul(marketingTokensCollected).div(walletsTotal);
        uint256 ethForBuyback = newBalance.mul(buybackTokensCollected).div(walletsTotal);
    ...
    function countUpFeeShare(uint256 amount) private
    {
        ...
        buybackTokensCollected += amount.mul(_buybackFee).div(100);
    }
    ...
    function restoreAllFee() private {
        ...
        totalSwapableFee = _liquidityFee.add(_devFee).add(_marketingFee).add(_buybackFee);
    }
    ...
    function setAllFeePercent(uint256 taxFee, uint256 liquidityFee, uint256 devFee, uint256 marketingFee, uint256 buybackFee) 
    external onlyOwner() 
    {
        _taxFee = taxFee;
        ...
        totalSwapableFee = _liquidityFee.add(_devFee).add(_marketingFee).add(_buybackFee);
        require(totalSwapableFee <= 10, "Must be less than 10% total");
    }

    function setSaleFeePercent(uint256 taxFee, uint256 liquidityFee, uint256 devFee, uint256 marketingFee, uint256 buybackFee) 
    external onlyOwner() 
    {
        _saleTaxFee = taxFee;
        ...
        totalSwapableSaleFee = _saleLiquidityFee +_saleMarketingFee + _saleBuybackFee + _saleTaxFee + _saleDevFee;
        require(totalSwapableSaleFee <= 10, "Must be less than 10% total");
    }
    ...
    function setBuybackWalletAddress(address _buybackWallet) external onlyOwner() {
        require(_buybackWallet != address(0), "setBuybackWalletAddress: ZERO");
        buybackWalletAddress = payable(_buybackWallet);
    }
