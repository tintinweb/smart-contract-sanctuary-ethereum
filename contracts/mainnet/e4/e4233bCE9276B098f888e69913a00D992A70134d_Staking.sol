...
contract Staking is OwnableUpgradeable, ReentrancyGuard{
    ...
    function claimReward(uint256 _id) external nonReentrant{
        ...
        uint256 rewardAmount = idToStake[_id].amount.mul(idToStake[_id].REWARDS_RATE).mul(6500).mul(30).mul(claimableCycle).div(_currentStakedAmount);
    ...
    function getClaimable (uint256 _id) public view returns (uint256) {
        uint256 id = _id;
        bool claimable_ = block.timestamp - idToStake[id].lastClaimTimeStamp >= _REWARD_CYCLE;
        if(claimable_) {
            ...
            uint256 temp2 = temp1.mul(6500).mul(claimableCycle).mul(30);
            uint256 temp3 = temp2.div(_currentStakedAmount);
            uint256 rewardAmount = temp3;
