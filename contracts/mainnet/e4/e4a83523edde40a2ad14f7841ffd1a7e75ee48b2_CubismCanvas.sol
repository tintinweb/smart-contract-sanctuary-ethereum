...
// |      _||       ||  _   | |   | |_____  ||       |
// |     |_ |       || |_|   ||   |  _____| || ||_|| |
// |_______||_______||_______||___| |_______||_|   |_| C A N V A S
                                                                                
// www.nftmintclub.com
// Cubism Canvas Collection
...
contract CubismCanvas is ERC721Enumerable, Ownable {
  using Strings for uint256;
...
string memory combinedData = string(bytes.concat(bytes(stringWallet), bytes(stringContract), bytes(stringAllocation)));
  ...
  if (preSale == true) {
    ...
    require(!executed[preSaleGroup][txHash], "Transaction already executed: Mint process stopped");
