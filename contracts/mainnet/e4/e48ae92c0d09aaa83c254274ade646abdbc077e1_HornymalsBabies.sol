...
contract  HornymalsBabies is ERC1155Upgradeable,  OwnableUpgradeable, UUPSUpgradeable, ISlave{
    ...
    uint256[13] public animalLimits;
    uint256[4] public multiplierLimits;
    uint256 public constant  STARTING_INDEX = 5034;
    ...
    function initialize(address _masterAddress, address _stardustTokenAddress) initializer public{
        ...
        rate = 140e12;
        animalLimits =[730, 1449, 2164, 2874, 3569, 4263, 4943, 5596, 5953, 6299, 6637, 6775, 6910];
        startingBlock = block.number;
        expiration = block.number + 4851693;
        __ERC1155_init("https://hornymals.com/babies/jsons/{id}.json");
        __Ownable_init();
    ...
    function masterMint(address mintTo, uint256 parentId, uint8 /*breedInfo*/) external onlyMaster{
        uint256 babyId = getAnimalIndex(parentId);
       _mint(mintTo, babyId, 1, "");
    }
    ...
    returns (bool)
    {
        return interfaceId == type(ISlave).interfaceId || super.supportsInterface(interfaceId);
    }


    function setMaster(address newMaster) public onlyOwner{
        masterAddress= newMaster;
    }



    function getMultiplier(uint256 tokenId)public pure returns (uint8){
        if(tokenId<5)return 3;
        if(tokenId<9)return 5;
        if(tokenId<12)return 7;
        if(tokenId<14)return 10;
        return 15;
    }
    function getRateForToken(uint256 tokenId)public view returns(uint256){
        return getMultiplier(tokenId)* rate;
    }
...
function getPseudoId(uint256 parentId) public pure returns(uint256) {
    uint256 newId = (STARTING_INDEX +parentId)%6969;
return newId/69 + (newId%69)*101;
}

    function getAnimalIndex(uint256 parentId)public view returns(uint256){
        uint256 pseudoIndex = getPseudoId(parentId);
        for(uint256 arrayIndex=0;arrayIndex<13; arrayIndex++){
            if(pseudoIndex<animalLimits[arrayIndex]){
                return arrayIndex+1;
            }
        }
        return 14;
