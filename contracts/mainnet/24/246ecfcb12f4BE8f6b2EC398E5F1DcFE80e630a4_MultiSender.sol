...
contract MultiSender {
    ...
    function sendMultiERC20(
        ...
        require(listReceivers.length == _values.length, "Total number of recipients and values are not equal");
        uint256 _totalToSend;
        for (uint256 i = 0; i < _values.length; i++) {
            _totalToSend += _values[i];
        }
        uint256 totalReceivers = listReceivers.length;
        IERC20 erc20 = IERC20(addressERC20);
        require(
            erc20.balanceOf(msg.sender) >= _totalToSend,
            "Total balance not enough"
        );

        uint256 allowance = erc20.allowance(msg.sender, address(this));
        require(
            allowance >= _totalToSend,
            "Check the token allowance"
        );
        for (uint256 i = 0; i < totalReceivers; i++) {
            erc20.transferFrom(msg.sender, listReceivers[i], _values[i]);
        }
        return true;
    }
}