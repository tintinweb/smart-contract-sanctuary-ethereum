...
contract NuggetWars is Context, ERC165, IERC721, IERC721Metadata {
    ...
    uint256 private _merkleTreeRoot;
    address private _mintPassAddr;

    uint256 immutable _openTimestamp;
    /**
     * @dev Initializes the contract by setting a `name` and a `symbol` to the token collection.
     */
    constructor(string memory name_, string memory symbol_, string memory baseURI_, uint256 root, address mpaddr, uint256 openTimestamp_) {
        _name = name_;
        _symbol = symbol_;
        _owner = msg.sender;
        _baseURI = baseURI_;
    ...
    function setMerkleTreeRoot(uint256 root) public onlyOwner {
        _merkleTreeRoot = root;
    }

    function setMintPassAddr(address addr) public onlyOwner {
        _mintPassAddr = addr;
    }
    ...
    function mint(uint256[] memory mintPassId, uint256[] memory nwTokenId, bytes32[][] memory proof) public {
        uint256 i;
        require(block.timestamp >= _openTimestamp, "mint error: mint is not opened yet");
        for(i = 0; i < mintPassId.length; i ++) {
            require(NWMintPass(_mintPassAddr).ownerOf(mintPassId[i]) == msg.sender, "mint error: invalid Mint Pass owner");
            require(!_exists(nwTokenId[i]), "mint error: already minted");
            _verifyProof(mintPassId[i] + 100000, nwTokenId[i], _merkleTreeRoot, proof[i]);
            totalSupply = totalSupply + 1;
            NWMintPass(_mintPassAddr).burn(mintPassId[i]);
            _safeMint(msg.sender, nwTokenId[i]);
        }
    }

    function devMint(uint256[] memory mintPassId, uint256[] memory nwTokenId, bytes32[][] memory proof) public onlyOwner {
        uint256 i;
        for(i = 0; i < mintPassId.length; i ++) {
            require(NWMintPass(_mintPassAddr).ownerOf(mintPassId[i]) == msg.sender, "mint error: invalid Mint Pass owner");
            require(!_exists(nwTokenId[i]), "mint error: already minted");
    ...
    function approve(address to, uint256 tokenId) public virtual override {
        address owner_ = NuggetWars.ownerOf(tokenId);
    ...
    function _transfer(
        ...
        require(NuggetWars.ownerOf(tokenId) == from, "ERC721: transfer from incorrect owner");
    ...
    function _approve(address to, uint256 tokenId) internal virtual {
        ...
        emit Approval(NuggetWars.ownerOf(tokenId), to, tokenId);
    }
    ...
    function _verifyProof(
        ...
        require(keccak256(abi.encode(leafPair)) == proof[0], "Invalid Proof");
