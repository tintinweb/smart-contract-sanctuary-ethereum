...
contract GhostBusdy is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    GhostBusdyDividendTracker public dividendTracker;
    ...
    constructor() ERC20("GhostBusdy", "$GBST") {
        marketingWallet = payable(0xc7448A7f4475082c77DaDbb8e5bED6B87C2d7d97);
        devWallet = payable(0xBc37B985e0470d080402D876c9DbE9De789d3B1c);
        ...
        dividendTracker = new GhostBusdyDividendTracker(payable(this), router, 0xA0b86991c6218b36c1d19D4a2e9Eb0cE3606eB48,
...
contract GhostBusdyDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    GhostBusdy public GhostBusdyContract;
    ...
    constructor(address payable mainContract, address router, address token, string memory _name,
        ...
        defaultToken = token;
        GhostBusdyContract = GhostBusdy(mainContract);
    ...
    function withdrawDividend() public pure override {
        require(false, "withdrawDividend disabled. Use the 'claim' function on the main GhostBusdy contract.");
    }
    ...
    function _reinvestDividendOfUser(address account) private returns (uint256) {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(GhostBusdyContract);

            uint256 prevBalance = GhostBusdyContract.balanceOf(address(this));
            ...
            {
                uint256 received = GhostBusdyContract.balanceOf(address(this)).sub(prevBalance);
                if (received > 0) {
                    success = true;
                    GhostBusdyContract.transfer(account, received);
                } 
                else {
                    success = false;
                }
