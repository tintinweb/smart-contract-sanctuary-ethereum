...
contract Nothing is Context, IERC20, Ownable {
    ...
    address payable private marketingWallet = payable(0xE40539D20479077004492578E33dBf8595034976); // Marketing Wallet
    address payable private devWallet = payable (0xE40539D20479077004492578E33dBf8595034976); // dev Wallet
    ...
    function _transfer(
        ...
        if(launchedAt>0 && from!= owner() && block.number >= (launchedAt + deadBlocks)  && antiBotmode){
                currenttotalFee=950;    //95% -> only on launch -> cant be enabled in future
        }
