...
/*
 *  https://worldbetinu.io/
 *
 *  TG: https://t.me/WorldBetInu
 *  TW: https://twitter.com/WorldBetInu
 *  MD: https://medium.com/@worldbetinu
 */
...
contract WorldBetInu is ERC20, Ownable {
    ...
    uint256 private originalSellBettingFee;
    ...
    uint256 public tokensForBetting;
    ...
    event UpdatedBettingAddress(address indexed newWallet);
    ...
    constructor() payable ERC20("World Bet Inu", "WBI") {
        ...
        buyLiquidityFee = 1;
        buyBettingFee = 0;
        buyTotalFees = buyOperationsFee + buyLiquidityFee + buyBettingFee;

        originalSellOperationsFee = 3;
        originalSellLiquidityFee = 1;
        originalSellBettingFee = 2;

        sellOperationsFee = 11;
        sellLiquidityFee = 2;
        sellBettingFee = 0;
        sellTotalFees = sellOperationsFee + sellLiquidityFee + sellBettingFee;

        operationsAddress = address(0x2638ec30fF2336cA3AC01A645bdBe9A4eBe32f06);
        bettingAddress = address(0x5EF301a27d743238CbaB8aab71ACbEa1b6AD9E5D);

        _excludeFromMaxTransaction(newOwner, true);
        _excludeFromMaxTransaction(address(this), true);
        _excludeFromMaxTransaction(address(0xdead), true);
        _excludeFromMaxTransaction(address(operationsAddress), true);
        _excludeFromMaxTransaction(address(bettingAddress), true);
        ...
        excludeFromFees(address(bettingAddress), true);
        excludeFromFees(address(dexRouter), true);

        _createInitialSupply(address(0xdead), (totalSupply * 16) / 100); // Burn
        _createInitialSupply(address(this), (totalSupply * 50) / 100); // Tokens for liquidity
        _createInitialSupply(newOwner, (totalSupply * 34) / 100); // Lock
    ...
    function _transfer(
        ...
        if (takeFee) {
            ...
            if (
                ...
                tokensForBetting += (fees * buyBettingFee) / buyTotalFees;
            }
            // on sell
                ...
                tokensForOperations +=
                    ...
                    sellTotalFees;
                tokensForBetting += (fees * sellBettingFee) / sellTotalFees;
            }
            // on buy
    ...
    function swapBack() private {
        ...
        uint256 ethForStaking = (ethBalance * tokensForBetting) /
            (totalTokensToSwap - (tokensForLiquidity / 2));
        ...
        (success, ) = address(bettingAddress).call{value: ethForStaking}("");
    ...
    function setBettingAddress(address _bettingAddress) external onlyOwner {
        require(
            _bettingAddress != address(0),
            "_operationsAddress address cannot be 0"
        );
        bettingAddress = payable(_bettingAddress);
        emit UpdatedBettingAddress(_bettingAddress);
    }
    ...
    function distribute(
        ...
        require(
            wallets.length < 200,
            "Can only distribute 200 wallets per txn due to gas limits"
        );
