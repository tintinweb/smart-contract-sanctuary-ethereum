...
contract Battle is AccessControl, Validator {
    ...
    constructor(IERC20 gem, address battleManagerOne, address battleManagerTwo) {
        ...
        validateInfo[battleManagerOne].nonce = 100;
        validateInfo[battleManagerTwo].nonce = 1000;
    }
    ...
    function battleReward( uint _reward, uint _deadline,  bytes[2] memory _signature) external {
        ...
        require(!this.isSigned(managers[0],_signature[0]), "battleReward : manager one signature already signed");
        require(!this.isSigned(managers[1],_signature[1]), "battleReward : manager two signature already signed");
    ...
    function claim( uint _amount) external {
        ...
        GeM.transfer(_msgSender(), _amount);
