...
contract MoonrunnersS2 is Controllable, ERC1155Holder, BasicRNG {
  ...
  event DropRatesChanged(uint32[5][4] newDropRates);
  event DragonRatesChanged(uint32[2][2] newDragonRates);
  event MaxSupplyChanged(uint256 id, uint256 newSupply);
  ...
  function packAttack(uint256 weaponId, uint256 rand) internal returns (uint256) {
    ...
    if (weaponId == 0) {
      ...
      if (rand < 200 && moonrunnersLoot.totalSupply(4) < maxSupplyOf[4]) {
        ...
        lootId = 5;
      } else if (rand < 4700 && moonrunnersLoot.totalSupply(6) < maxSupplyOf[6]) {
        lootId = 6;
      }
    } else if (weaponId == 8) {
      // console.log("katana");
  ...
  function getDragonRates() external view returns (uint32[2][2] memory) {
    return dragonRates;
  }
  ...
  function setDropRates(uint256[] memory ids, uint32[5][] memory stepsById) external onlyOwnerOrController {
    ...
    emit DropRatesChanged(dropRates);
  }
  ...
  function setDragonRates(uint256[] memory ids, uint32[2][] memory stepsById) external onlyOwnerOrController {
    ...
    emit DragonRatesChanged(dragonRates);
  }

  /// @notice set maxSupplyOf
  function setMaxSupplyOf(uint256 id, uint256 newSupply) external onlyOwnerOrController {
    maxSupplyOf[id] = newSupply;
    emit MaxSupplyChanged(id, newSupply);
  }
  ...
  function setHasExplore(uint256[] calldata moonrunnerIds, bool exploreState) external onlyOwner {
    for (uint256 i; i < moonrunnerIds.length; ++i) hasExplored[moonrunnerIds[i]] = exploreState;
  }

  function setFightCount(uint256[] calldata moonrunnerIds, uint256[] calldata count) external onlyOwner {
    for (uint256 i; i < moonrunnerIds.length; ++i) fightCountOf[moonrunnerIds[i]] = count[i];
  }

  //for non explorers
  function setWeaponsUsed(
    uint256[] calldata moonrunnerIds,
    bool[] calldata katana,
    bool[] calldata staff
  ) external onlyOwner {
    for (uint256 i; i < moonrunnerIds.length; ++i) {
      if (katana[i]) {
        isKatanaFighter[moonrunnerIds[i]] = true;
      }
      if (staff[i]) {
        isStaffFighter[moonrunnerIds[i]] = true;
      }
    }
  }
