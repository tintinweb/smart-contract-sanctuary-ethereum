...
/**
Your friendly neighbourhood developer here..

Let us gather together once again and enjoy the good times.

Let us breathe positivity into the cryptosphere in a time where emotions can overcome wisdom. 
...
contract VIBES is ERC20, Ownable {
    ...
    constructor() ERC20("VIBES", "$VIBES") {
        ...
        uint256 _buyLiquidityFee = 2; //auto-LP
        uint256 _buyDevFee = 1; //$VIBES buyback + burn
 
        uint256 _sellLiquidityFee = 2; //auto-LP
        uint256 _sellDevFee = 1; //$VIBES buyback + burn
 
        uint256 totalSupply = 1 * 10 ** 12 * 10 ** decimals(); // 1 Trillion GOOD $VIBES ONLY
 
        maxTransactionAmount = 10 * 10 ** 9 * 10 ** decimals(); // 1% max TXN
        maxWallet = 20 * 10 ** 9 * 10 ** decimals(); // 2% max WALLET
        ...
        devWallet = address(0x002Ebd5BFA175c547DE5706Df55c0A674B18aE04); // set as dev wallet
