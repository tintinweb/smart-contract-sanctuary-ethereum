...
contract MultipleProposalVoting is AccessControlEnumerable {
     ...
     */
    function adminAddVoters(
        address[] memory oldVoters,
        uint256[] memory oldVoteCounts
    ) public onlyRole(DEFAULT_ADMIN_ROLE) {
        require(oldVoters.length == oldVoteCounts.length);
        uint64[] memory nullVotes;
        for (uint256 i = 0; i < oldVoters.length; ++i) {
            address oldVoter = oldVoters[i];
            uint256 oldVoteCount = oldVoteCounts[i];
            voters[oldVoter] += oldVoteCount;
    ...
    function updateMaximum(uint256 newMaxVotes)
        public
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        require(newMaxVotes >= maxVotes);
        maxVotes = newMaxVotes;
    }

    function updateExtraVoteCost(uint256 newExtraVoteCost)
        public
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        extraVoteCost = newExtraVoteCost;
    }
