...
contract DelusionStaking is Ownable, IERC721Receiver {
  ...
  constructor(address _nft, address _token) { 
    ...
    perdayreward = 1000 ether; // 10 $DEL A DAY
  }
  ...
  function _claim(address account, uint256[] calldata tokenIds, bool _unstake) internal {
    ...
    for (uint i = 0; i < tokenIds.length; i++) {
      ...
      rewardmath = perdayreward * (block.timestamp - stakedAt) / 86400 ;
  ...
  function setPerDayReward(uint256 newReward) external onlyOwner {
    perdayreward = newReward;
  } 
