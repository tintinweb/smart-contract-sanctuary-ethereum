...
contract OverTheCounter is IERC721Receiver, Ownable {
  ...
  function poolRemove(uint256 _tokenId) external {
  ...
  function packageWithdrawal(uint256 _tokenId) external {
    OTC storage _otc = otcs[_tokenId];
    Package storage _pkg = _otc.package;
    address _user = otcNFT.ownerOf(_tokenId);
    require(msg.sender == _user, 'PKGWITH: owner');
  ...
  function removePackageOffer(uint256 _tokenId, uint256 _offerIndex) external {
    PkgOffer memory _offer = pkgOffers[_tokenId][_offerIndex];
    require(
  ...
  function buyItNow(
    ...
    address _from = _pkg.buyItNowAsset == address(0)
      ? address(this)
      : msg.sender;
    if (_buyFee > 0) {
  ...
  function _refundETH(address _user) internal {
    uint256 _bal = address(this).balance;
    if (_bal == 0) {
      return;
    }
    (bool _sent, ) = payable(_user).call{ value: _bal }('');
    require(_sent, 'REFETH: could not refund');
  }
