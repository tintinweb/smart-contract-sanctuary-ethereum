...
contract WhikoBox is ERC721A, Ownable {
    using SafeMath for uint256;
    using Address for address;
    ...
    /* type = 0 */
    string public silverTypeURI;
    /* type =1 */
    string public goldTypeURI;
     ...
     */
    event NFTMintEvent(uint typ, uint256 token, address to, uint256 quantity);
    ...
    event SilverTypeURIEvent(string _silverTypeURI);

    event GoldTypeURIEvent(string _goldTypeURI);

    constructor(
        ERC721Interface _nft,
        address _executorAddress,
        string memory baseUri,
        string memory _commonTokenURI,
        string memory _silverTypeURI,
        string memory _goldTypeURI,
        uint256 _maxTotalSupply,
        uint256 _openBoxTime
    ) ERC721A("Whiko Box", "WhikoBox"){
        whiko = ERC721Interface(_nft);
        executorAddress = _executorAddress;
        baseURI = baseUri;
        commonTokenURI = _commonTokenURI;
        silverTypeURI = _silverTypeURI;
        goldTypeURI = _goldTypeURI;
    ...
    function setSilverTypeURI(string memory _silverTypeURI) public onlyExecutor {
        require(bytes(_silverTypeURI).length > 0, "URI length not null");
        silverTypeURI = _silverTypeURI;
        emit SilverTypeURIEvent(_silverTypeURI);
    }

    function setGoldTypeURI(string memory _goldTypeURI) public onlyExecutor {
        require(bytes(_goldTypeURI).length > 0, "URI length not null");
        goldTypeURI = _goldTypeURI;
        emit GoldTypeURIEvent(_goldTypeURI);
    }
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        if (openBoxTime > block.timestamp) {
            uint tpe = tokenType[tokenId];
            string memory _tokenTypeURI = '';
            if (tpe == 0) {
                _tokenTypeURI = silverTypeURI;
            } else if (tpe == 1) {
                _tokenTypeURI = goldTypeURI;
            }
            if (bytes(_tokenTypeURI).length > 0) {
                uri = string(abi.encodePacked(uri, _tokenTypeURI));
            }
        } else {
            ...
            if (bytes(tokenURI).length > 0) {
                ...
                uri = string(abi.encodePacked(uri, tokenId));
            }
        }
        return uri;
    }
    ...
    function mint(address to) public nftIsOpen {
        ...
        require(totalSupply() <= maxTotalSupply, "Total limit");

        bool minted = mintedPool[to];
        require(!minted, "This address has mint");

        uint256 balance = whiko.balanceOf(to);
        require(balance > 0, "This address hasn't hold nft");
        ...
        tokenType[tokenId] = _tokenType;
        mintedPool[to] = true;
        _mint(to, quantity);
        emit NFTMintEvent(_tokenType, tokenId, to, quantity);
    }
