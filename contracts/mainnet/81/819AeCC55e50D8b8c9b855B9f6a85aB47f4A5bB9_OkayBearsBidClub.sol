...
/**
 * @title OkayBearsBidClub contract
...
contract OkayBearsBidClub is ERC721A, Ownable, ReentrancyGuard {
  ...
  constructor() ERC721A("OkayBearsBidClub", "OBRC"){}

  function mint(uint256 amt) external payable
  {
    require(msg.sender == tx.origin,"Sender and origin should be same.");
    require(totalSupply() + amt < maxSupply + 1,"No more guilty bears.");
    require(mintEnabled, "Mint is not enabled.");
    require(numberMinted(msg.sender) + amt <= maxPerWallet,"Max mint per wallet is 5.");
