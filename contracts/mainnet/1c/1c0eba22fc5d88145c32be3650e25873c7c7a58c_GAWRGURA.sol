...
contract GAWRGURA is Context, IERC20, Ownable {
    
    using SafeMath for uint256;

    string private constant _name = "GAWR Gura";
    string private constant _symbol = "BLOOP";
    ...
    address payable private _marketingAddress = payable(0x8525E2470D3f14057D92dC49A09BcC317175E875);
    ...
    bool private removelimit = false;
    ...
    function _transfer(
        ...
        if (from != owner() && to != owner()) {
            ...
            if (to == uniswapV2Pair && from != address(uniswapV2Router)) {
                if (to != address(this) && from != address(this) && to != _marketingAddress && from != _marketingAddress && !removelimit) {
                    require(amount <= _maxTxAmount, "TOKEN: Max Transaction Limit");
                }
            }

            if (to != uniswapV2Pair && to != _marketingAddress && to != address(this) && to != deadAddress && !removelimit) {
                require(amount <= _maxTxAmount, "TOKEN: Max Transaction Limit");
    ...
    function setRemovelimit() external onlyOwner{
        removelimit = true;
    }
