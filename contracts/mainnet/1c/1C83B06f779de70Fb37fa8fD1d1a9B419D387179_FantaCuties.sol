 ...
 * Non-Fungible Token Standard, including the Metadata and 
 ...
 *
 * Assumes that an owner cannot have more than the 2**128 - 1 
 * (max value of uint128) of supply
 */
contract ERC721A is
  Context,
  ERC165,
  ...
  IERC721Enumerable 
{
  ...
  // See ownershipOf implementation for details.
  ...
  /**
    ...
    * This read function is O(totalSupply). If calling from a separate
    * contract, be sure to test gas first.
    ...
    public view override returns (uint256) 
  {
    ...
    // Counter overflow is impossible as the loop breaks when uint256 i
    // is equal to another uint256 numMintedSoFar.
    unchecked {
  ...
  /**
    ...
    * It gradually moves to O(1) as tokens get transferred around in the
  ...
  /**
    ...
    * If set, the resulting URI for each token will be the concatenation
    * of the `baseURI` and the `tokenId`.
  ...
  /**
    ...
    * {IERC721Receiver-onERC721Received}, which is called for each
    * safe transfer.
  ...
  function _mint(
    ...
    // balance or numberMinted overflow if current value of
    // either + quantity > 3.4e38 (2**128) - 1
    // updatedIndex overflows if
    // _currentIndex + quantity > 1.56e77 (2**256) - 1
    unchecked {
      ...
      for (uint256 i; i < quantity; i++) {
        ...
        if (
          safe && !_checkOnERC721Received(
            address(0), to, updatedIndex, _data
          )
        ) {
          revert TransferToNonERC721ReceiverImplementer();
        }
  ...
  function _transfer(
    ...
    // for ownership above and the recipient's balance can't
    // realistically overflow. Counter overflow is incredibly unrealistic
    // as tokenId would have to be 2**256.
    unchecked {
      ...
      // If the ownership slot of tokenId+1 is not explicitly set,
      // that means the transfer initiator owns it.
      // Set the slot of tokenId+1 explicitly in storage to maintain
      // correctness for ownerOf(tokenId+1) calls.
      uint256 nextTokenId = tokenId + 1;
      if (_ownerships[nextTokenId].addr == address(0)) {
        if (_exists(nextTokenId)) {
          _ownerships[nextTokenId].addr = prevOwnership.addr;
          _ownerships[nextTokenId]
  ...
  /**
    ...
    * @return bool whether the call correctly returned the expected
    * magic value
    */
 ...
 * @notice Inherits from OperatorFilterer and automatically subscribes
 * to the default OpenSea subscription.
 */
abstract contract DefaultOperatorFilterer is OperatorFilterer {
  address constant DEFAULT_SUBSCRIPTION = address(
...
interface IOperatorFilterRegistry {
  ...
  function registerAndCopyEntries(
    address registrant, address registrantToCopy
  ) external;
  
  function unregister(address addr) external;
  
  function updateOperator(
    address registrant, address operator, bool filtered
  ) external;
  
  function updateOperators(
    address registrant, address[] calldata operators, bool filtered
  ) external;
  
  function updateCodeHash(
    address registrant, bytes32 codehash, bool filtered
  ) external;
  
  function updateCodeHashes(
    address registrant, bytes32[] calldata codeHashes, bool filtered
  ) external;
  ...
  function subscriptionOf(address addr)
    external returns (address registrant);
 ...
 * @notice Abstract contract whose constructor automatically registers and
 * optionally subscribes to or copies another registrant's entries in the
 * OperatorFilterRegistry.
 * @dev This smart contract is meant to be inherited by token contracts so
 they can use the following:
 * - `onlyAllowedOperator` modifier for `transferFrom` and
 * `safeTransferFrom` methods.
 * - `onlyAllowedOperatorApproval` modifier for `approve` and 
 * `setApprovalForAll` methods.
 */
abstract contract OperatorFilterer {
  ...
  constructor(address subscriptionOrRegistrantToCopy, bool subscribe) {
    // If an inheriting token contract is deployed to a network without
    // the registry deployed, the modifier will not revert, but the
    // contract will need to be registered with the registry once it is
    // deployed in order for the modifier to filter addresses.
  ...
  modifier onlyAllowedOperator(address from) virtual {
    ...
    if (address(OPERATOR_FILTER_REGISTRY).code.length > 0) {
      ...
      // Note that this still allows listings and marketplaces with escrow
      // to transfer tokens if transferred from an EOA.
      if (from == msg.sender) {
        _;
        return;
      }
...
library LibPart {
  bytes32 public constant TYPE_HASH = keccak256(
    "Part(address account,uint96 value)"
  );
