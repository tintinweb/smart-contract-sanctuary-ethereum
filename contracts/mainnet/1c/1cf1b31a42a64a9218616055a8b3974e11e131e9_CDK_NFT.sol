...
abstract contract RandomlyAssigned is WithLimitedSupply {
    ...
    function nextToken() internal override ensureAvailability returns (uint256) {
        uint256 maxIndex = tSupply() - tokenCount();
...
contract CDK_NFT is ERC721Enumerable, Ownable, RandomlyAssigned {
    using Strings for uint256;
    ...
    bytes32 public root =
0xcd1fd97134dba471350ffed785517b4f16e5dc38739e1fbcb45768c430cf9a89;

    constructor(string memory _initBaseURI, string memory _initRevealURI)
        ERC721("CyberDoodleKids", "CDK")
    ...
    function preMint(uint _amount, bytes32[] memory _proof) public payable {
        uint256 supply = totalSupply();
    ...
    function addFreeMint(address[] memory toAdd) public onlyOwner {
        for(uint i=0; i < toAdd.length; i++) {
            freeMint[toAdd[i]] = true;
        }
    }
