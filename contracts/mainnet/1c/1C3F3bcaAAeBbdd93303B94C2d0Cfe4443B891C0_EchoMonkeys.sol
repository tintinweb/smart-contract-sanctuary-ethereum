    ...
    DefaultOperatorFilterer
{
    ...
    uint private constant tokensReserved = 2;
    bool public isMinting = true;
    bool public canUpdateSeed = true;

    IEchoMonkeysDescriptor public descriptor;

    constructor(IEchoMonkeysDescriptor newDescriptor) ERC721("EchoMonkeys", "echomon")
    {
    	descriptor= newDescriptor;

	    for(uint256 i = 0; i < tokensReserved; i++) {
    ...
    function mint(uint32 numberOfTokens) external payable {
        ...
        require(_tokenIdCounter.current() + numberOfTokens <= maxSupply, "Exceeds total supply.");
	...
	function setMaxMintPerTx(uint256 number) external onlyOwner {
        maxMintPerTx = number;
    }

    function setDescriptor(IEchoMonkeysDescriptor newDescriptor) external onlyOwner {
        descriptor = newDescriptor;
    }

    function setSeedUpdate() external onlyOwner {
        canUpdateSeed = !canUpdateSeed;
    }
    ...
    function generateSeed(uint256 tokenId) private view returns (uint256) {
        ...
        uint256 headSeed = 100 * (r % 10 + 10) + ((r >> 48) % 20 + 10);
        uint256 faceSeed = 100 * ((r >> 96) % 7 + 10) + ((r >> 96) % 20 + 10);
        uint256 noseSeed = 100 * ((r >> 144) % 5 + 10) + ((r >> 144) % 20 + 10);
        uint256 bodySeed = 100 * ((r >> 192) % 8 + 10) + ((r >> 192) % 20 + 10);
        return 10000 * (10000 * (10000 * headSeed + faceSeed) + noseSeed) + bodySeed;
    }
     ...
     * A 10% royalty, would use a feeNumerator of 1000 (1000/10000=.1)
     */
