...
contract NINJAZ is ERC721A, Ownable {
  using Strings for uint256;
  uint256 public presalePrice = 0 ether;
  uint256 public mintPrice = 0.008 ether;
  uint256 public supply = 800;
  string private baseURI = "ipfs://GRDSsmUdsdFDCDFdssdWRdssaDSdssVV/";
  bytes32 public wlMerkleRoot;
  uint8 public phase = 1;
  uint8 public maxWalletPublic = 5;
  uint8 public wlMax = 1;

  mapping(address => uint8)  public walletBuys;

  constructor() ERC721A("NINJAZ", "NINJAZ") {
  }
  ...
  function mint(uint256 quantity) external payable {
    ...
    require(walletBuys[msg.sender] + quantity <= maxWalletPublic, "Buy limit reached");
  ...
  function setMaxWallet(uint8 _maxWalletPublic) external onlyOwner {
      maxWalletPublic = _maxWalletPublic;
  }

  function wlSpots(uint256 _newspots) external onlyOwner {
      supply = _newspots;
  }
  ...
  function withdrawAll() external onlyOwner{
        ...
        address[1] memory addresses = [
            0x7Fe9B1f10B340639DcC706Ed71FCbBe7f2f0bC1c
