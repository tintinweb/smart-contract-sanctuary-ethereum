...
contract OrdinalMorphs is ERC721A, Ownable, ReentrancyGuard, DefaultOperatorFilterer {
  ...
  string public uriPrefix = 'ipfs://QmcQjbnPpwxRBvnmhWUxhPdn7j1a8HZVbF92DmjEjua62m/';
  ...
  function fuse(uint64[] memory tokens) public {
        ...
        for (uint i = 0; i < tokens.length; i++) {
            ...
            sum = sum + getOrdinalMorphs(tokens[i]);
        }
        if (sum > 1000) {
        ...
        baseColor[tokens[0]] = randOrdinalMorphs(tokens[0], 1, 4);
  }

  function getOrdinalMorphs(uint64 tokenId) public view returns (uint64) {
        if (!_exists(tokenId)) {
            return 0;
  ...
  function randOrdinalMorphs(uint64 input, uint64 min, uint64 max) internal pure returns (uint64) {
