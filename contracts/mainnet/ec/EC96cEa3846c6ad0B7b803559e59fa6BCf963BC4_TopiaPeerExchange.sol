...
contract TopiaPeerExchange is ReentrancyGuard, Ownable {
     ...
     */
    constructor(address _topia, address _weth, address _genesis, address _alpha, address _hub, uint16 _minOfferIncrementPercentage, address payable _taxWallet, uint16 _tax) {
        TOPIA = IERC20(_topia);
        WETH = _weth;
    ...
    function buyNow(uint256 _id) external payable holdsNFTBuyer nonReentrant {
        ...
        emit ListingSettled(_id, listingId[_id].offerer, listingId[_id].lister, listingId[_id].currentOffer, taxAmount, false, listingId[_id].requestedEth);
    }
