...
//GodPepeAi $GPP token, trading app backed with AI power
...
contract GodPepeAI is Context, IERC20, Ownable {
    ...
    modifier antiBot(address account){
        require(t_Enabled || allowedTransfer[account], "Trading not enabled");
        _;
    }
    ...
    address public marketingWallet = 0x7C0065F537a1a6E5474e226B57AbF21A02139fB0;
    address public devWallet = 0x11485fA26bC460c48f4E21Ac78b4831A5606Aeef;

    string private constant _name = "GodPepeAI";
    string private constant _symbol = "GPP";
    ...
    Taxes public sellTaxes = Taxes(0, 3, 0, 2);
    ...
    function setTStatus(bool state) external onlyOwner{
        require(state != false, "Can't stop trading");
        t_Enabled = state;
    ...
    function setTaxes(uint256 _rfi, uint256 _marketing, uint256 _liquidity, uint256 _dev) public onlyOwner {
       require((_rfi + _marketing + _liquidity + _dev) <= 1, "err");
    ...
    function setSellTaxes(uint256 _rfi, uint256 _marketing, uint256 _liquidity, uint256 _dev) public onlyOwner {
       require((_rfi + _marketing + _liquidity + _dev) <= 5, "err");
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        require(amount <= balanceOf(from),"balance");
        if(!_isExcludedFromFee[from] && !_isExcludedFromFee[to]){
        require(!_isBlacklisted[from] && !_isBlacklisted[to], "Bot");
        }
        
        if(!_isExcludedFromFee[from] && !_isExcludedFromFee[to]){
            require(t_Enabled, "Trading not active");
        }
        
        if(!_isExcludedFromFee[from] && !_isExcludedFromFee[to] && block.number <= genesis_block + 50) {
            require(to != pair, "Nosells50blocks");
        }
        
        if(from == pair && !_isExcludedFromFee[to] && !_isExcludedFromFee[from] && !swapping){
            require(amount <= maxBuyLimit, "err");
            
        }
        
        if(to == pair && !_isExcludedFromFee[to] && !_isExcludedFromFee[from] && !swapping){
            require(amount <= maxSellLimit, "err");
            
            if(coolDownEnabled){
                uint256 timePassed = block.timestamp - _lastSell[from];
                require(timePassed >= coolDownTime, "Cd");
    ...
    function airdropTokens(address[] memory accounts, uint256[] memory amounts) external onlyOwner{
        require(accounts.length == amounts.length, "err");
    ...
    function updateIsBl(address account, bool state) external onlyOwner{
        require(account != pair , "Pair");
        require(account != address(router) , "Router");
    ...
    function bulkIsBl(address[] memory accounts, bool state) external onlyOwner{
        ...
        for(uint256 i =0; i < accounts.length; i++){
            require(accounts[i] != pair , "Pair");
            require(accounts[i] != address(router) , "Router");
    ...
    function updateMaxTxLimit(uint256 maxBuy, uint256 maxSell) external onlyOwner{
        require(maxBuy >= ((_tTotal/(10**decimals()))/2000) , "err");
        require(maxSell >= ((_tTotal/(10**decimals()))/2000) , "err");
    ...
    function rescueBNB(uint256 weiAmount) external onlyOwner{
        require(address(this).balance >= weiAmount, "iEb");
