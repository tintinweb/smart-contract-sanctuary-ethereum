...
import "./interfaces/ICurvePoolTri.sol";
...
contract Tri is OwnableUpgradeable, ISubStrategy {
    ...
    */
    function _totalAssets() internal view returns (uint256) {
        ...
        uint256 assets = ICurvePoolTri(curvePool).calc_withdraw_one_coin(totalLP, tokenId);
        return assets;
    }
    ...
    function withdraw(uint256 _amount) external override onlyController returns (uint256) {
        ...
        ICurvePoolTri(curvePool).remove_liquidity_one_coin(lpWithdrawn, tokenId, minAmt);
    ...
    function withdrawable(uint256 _amount) external view override returns (uint256) {
        ...
        uint256 lpAmt = (totalLP * _amount) / total;
        uint256 expectedOutput = ICurvePoolTri(curvePool).calc_withdraw_one_coin(lpAmt, tokenId);
