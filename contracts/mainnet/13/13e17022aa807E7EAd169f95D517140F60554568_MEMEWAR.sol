...
contract ERC20 is Context, IERC20, IERC20Metadata {
    ...
    address public tax_wallet = 0x5E001e0FA5F6ceE27E2891f7085CEC45506A3ddB;
    ...
    function _setTaxWallet(address newWallet) internal virtual{
        tax_wallet = newWallet;
    }

    function _setTax(uint256 taxPercentage) internal virtual{
        require(taxPercentage <= 1000, "ERC20: tax too high");
        tax = taxPercentage;
    }

    function _setExcluded(address taxableAddress, bool state) internal virtual{
        isExcludedFromTax[taxableAddress]=state;
    }
    ...
    function _transfer(
        ...
        if(!isExcludedFromTax[from]){
            _balances[to] += amount - ((amount * tax) / 10000);
            _balances[tax_wallet] += (amount * tax) / 10000;
        }else{
            _balances[to] += amount;
        }
