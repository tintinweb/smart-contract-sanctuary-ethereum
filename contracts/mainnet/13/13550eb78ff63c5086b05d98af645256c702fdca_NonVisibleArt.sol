...
contract NonVisibleArt is ERC721A, Ownable {
    using SafeMath for uint256;
    using Strings for uint256;
    uint256 public constant maxSupply = 777;
    uint256 public constant freeMints = 222;
    uint256 public constant maxFreeMintsPerWallet = 2;
    uint256 public constant maxTotalMintsPerWallet = 3;
    uint256 public reservedAssets = 40;
    uint256 public priceIncreaseX = 3;
    uint256 public maxAmountPerTX = 2;
    ...
    mapping (address => uint256) private totalMintsWallet;

    constructor(string memory baseURI, address _proxyRegistryAddress) ERC721A("Non Visible Art", "NVA") {
        setBaseURI(baseURI);
        isSaleActive = false;
    ...
    function mintNFT(uint256 numAssets) external payable {
        ...
        require(numAssets >= 0 && numAssets <= maxAmountPerTX,
            "You can only mint 2 art pieces per transaction!");
        require(totalMintsWallet[msg.sender].add(numAssets) <= maxTotalMintsPerWallet,
                "Maximum 3 art pieces per wallet.");
        require(totalSupply().add(numAssets) <= maxSupply - reservedAssets,
            "Not enough art pieces left to mint... But it's on Opensea!");

        if(totalSupply().add(numAssets) > freeMints){
            require(msg.value >= _price.mul(numAssets),
                "Not enough ETH for this purchase!");

            if((totalSupply() / 111) >= priceIncreaseX){
            ...
            if(freeMintsWallet[msg.sender].add(numAssets) > maxFreeMintsPerWallet){
                ...
                "You can only mint 2 art pieces for free! You can mint another one after 222 pieces are minted.");
                freeMintsWallet[msg.sender] += numAssets;
            }
        }
        totalMintsWallet[msg.sender] += numAssets;
    ...
    function gift(uint[] calldata quantity, address[] calldata recipient) external onlyOwner{
    	...
    	require( totalSupply() + totalQuantity <= maxSupply, "Too many" );
