...
abstract contract Ownable is Context {
 ...
 abstract contract ERC721AQueryable is ERC721A
 {
    ...
    contract MathClub is ERC721AQueryable, Ownable {
    using Strings for uint256;
 ...
 uint256 public OGCost = 0.06 ether;
 uint8 public giftNft = 2;
  uint16 public constant maxSupply = 500;
  
                                                             
  bool public WLpaused = false;
  bool public paused = false;
  bool public OGpaused = false;
  bool public reveal;
  ...
  uint8 public maxWLMintAmountPerWallet = 2;  
   uint8 public maxOGMintAmountPerWallet = 2;  
  ...
  bytes32 public OGMerkleRoot = 0x0cc622db75575674b2bc8dfe29c3ac04df24f1567b769fa31f6d4ef384ea66d2;
  mapping(address => bool) public projectProxy;

  constructor() ERC721A("Math Club", "MC") {
  }
  ...
  function mint(uint8 _mintAmount) external payable  {
    ...
    require(nft + _mintAmount <= maxMintAmountPerWallet, "Exceeds max supply.");
 ...
 function setMaxLimitPerOGAddress(uint8 _limit) external onlyOwner{
    maxOGMintAmountPerWallet = _limit;
   delete _limit;

}
   function setOGPaused() external onlyOwner {
    OGpaused = !OGpaused;
  
  }
  function setOGCost(uint256 _cost) external onlyOwner {
    OGCost = _cost;
    delete _cost;
  }
...
function whitelistMint(uint8 _mintAmount, bytes32[] calldata merkleProof) external payable {
    ...
    require(!WLpaused, "Sale is Paused!");
    ...
    function OGMint(uint8 _mintAmount, bytes32[] calldata merkleProof) external payable {
       ...
       require (_txPerAddress + _mintAmount <= maxOGMintAmountPerWallet, "Exceeds max nft allowed per wallet");
      


    require(!OGpaused, "Sale is Paused!");
  ...
  function _baseURI() internal view  override returns (string memory) {
    return string(abi.encodePacked(uriPrefix, "/"));
  }
  function Gift(address giftAddress, uint16 amount) external onlyOwner{
      require(amount<= giftNft, "Limit excceeded");
       uint16 totalSupply = uint16(totalSupply());
    require(totalSupply<= maxSupply, "Exceeds max supply.");
       _safeMint(giftAddress , giftNft);
  }
}