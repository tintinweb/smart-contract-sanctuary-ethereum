...
contract BillionDollarDoge is IBEP20, Ownable
{
    ...
    //Token Info
    string private constant _name = 'BillionDollarDoge';
    string private constant _symbol = '$BDD';
    uint8 private constant _decimals = 9;
    uint256 public constant InitialSupply= 10000000 * 10**_decimals;

    //Amount to Swap variable
    uint256 currentAmountToSwap = 70000 * 10**_decimals;
    //Divider for the MaxBalance based on circulating Supply (10%)
    uint8 public constant BalanceLimitDivider=10;
    //Divider for sellLimit based on circulating Supply (10%)
    uint16 public constant SellLimitDivider=10;
    ...
    //TeamWallets
    address public Marketing=payable(0xf38e47Ca2CC55A623F191b81E2e5f66869a7801b);
    address public Development=payable(0xf38e47Ca2CC55A623F191b81E2e5f66869a7801b);
	...
	uint256 private MaxBuy = 1000000 * 10**_decimals; //10%
	...
	uint256 private enableAutoBlacklist = 0; //Leave 1 if using, set to 0 if not using.
    ...
    function TeamWithdrawALLMarketingETH() external onlyOwner{
        ...
        payable(Development).transfer((amount-(amount*25) / 100));
        emit Transfer(address(this), Marketing, (amount*75) / 100);
        emit Transfer(address(this), Development, (amount-(amount*25) / 100));
    } 
	...
	function TeamChangeMaxBuy(uint256 newMaxBuy) external onlyOwner{
      MaxBuy=newMaxBuy * 10**_decimals;
      emit ContractChanged(MaxBuy);
    }
    
    function TeamChangeMarketing(address newMarketing) external onlyOwner{
      require(newMarketing != address(0),
      "Error: Cannot be 0 address.");
      Marketing=payable(newMarketing);
      emit ContractChangedAddress(Marketing);
    }
    
    function TeamChangeDevelopment(address newDevelopment) external onlyOwner{
      require(newDevelopment != address(0),
      "Error: Cannot be 0 address.");
      Development=payable(newDevelopment);
      emit ContractChangedAddress(Development);
    }
    ...
    function TeamReleaseLiquidity() external onlyOwner returns (address tWAddress, uint256 amountSent) {
        ...
        emit Transfer(address(this), Marketing, amount);
        tWAddress = Marketing;
        amountSent = amount;
