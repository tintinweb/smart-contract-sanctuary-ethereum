...
contract PyeClaim is Ownable, ReentrancyGuard {
    ...
    function claimTokens() external nonReentrant notContract() {
        ...
        uint256 dailyApple = (claimableApple.mul(15625 * 10**18)).div(1 * 10**5);
        uint256 dailyCherry = (claimableCherry.mul(15625 * 10**18)).div(1 * 10**5);
    ...
    function calculateWithdrawableAmounts(address _address) external view returns (uint256, uint256, uint256) {
        require(getElapsedTime() > 30 days , "You have not waited the 30-day cliff period! Your withdrawable amounts are 0.");
