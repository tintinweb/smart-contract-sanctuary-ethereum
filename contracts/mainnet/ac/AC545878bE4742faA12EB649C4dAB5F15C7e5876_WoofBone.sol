...
contract WoofBone is tokenStaking, IERC20, Ownable {
    ...
    string private constant _symbol = 'WBNE';
    ...
    uint256 private treasuryFee = 300;
    ...
    function compoundRewards() external feelessTransaction {
        uint256 initialToken = balanceOf(msg.sender);
        uint256 amount = getUnpaidEarnings(msg.sender);
        claimToCompoundDividend(msg.sender, amount);
        swapRewardsforTokens(amount, msg.sender);
        uint256 afterToken = balanceOf(msg.sender).sub(initialToken);
        internalDeposit(msg.sender, afterToken);
        emit Compound(msg.sender, afterToken, block.timestamp);
    }
    ...
    function swapAndLiquify(uint256 tokens) private lockTheSwap {
        ...
        if(treasuryAmount > 0){payable(treasury_receiver).transfer(treasuryAmount);}
        ...
        if(boneAmount > 0){payable(bone_receiver).transfer(boneAmount);}
    ...
    function swapRewardsforTokens(uint256 RewardAmount, address receiver) internal {
		_approve(address(this), address(router), RewardAmount);
        address[] memory path = new address[](2);
    ...
    function claimToCompoundDividend(address shareholder, uint256 amount) internal {
        if(shares[shareholder].amount == 0){ return; }
        if(amount > 0){
            totalDistributed = totalDistributed.add(amount);
            currentDividends = currentDividends.sub(amount);
