...
contract MirrorBirds is ERC721A, Ownable, Pausable, ReentrancyGuard {
    using Strings for uint256;
    ...
    uint256 public maxperAddressPublicMint = 11;

    mapping(address => uint256) public addressFreeMintedBalance;

    constructor() ERC721A("MirrorBirds", "MIRBS") {
        setBaseURI("");
    ...
    function mintFree(uint256 _mintAmount) public payable nonReentrant{
        ...
        require(addressFreeMintedCount + _mintAmount <= maxperAddressFreeLimit, "Max MirrorBirds per address exceeded");
		require(_mintAmount > 0, "Cant mint 0 MirrorBirds" );
		require(s + _mintAmount <= maxFree, "Cant exceed MirrorBird supply" );
    ...
    function mint(uint256 _mintAmount) public payable nonReentrant {
        uint256 s = totalSupply();
        require(_mintAmount > 0, "Cant mint 0 MirrorBirds");
        require(_mintAmount <= maxperAddressPublicMint, "Cant mint more MirrorBirds" );
        require(s + _mintAmount <= maxSupply, "Cant exceed MirrorBird supply");
