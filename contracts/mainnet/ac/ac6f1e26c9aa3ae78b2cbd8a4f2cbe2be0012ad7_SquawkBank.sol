...
contract DividendDistributor is IDividendDistributor {
    ...
    address public immutable SQUAWK = address(0xeff3f1b9400D6D0f1E8805BddE592F61535F5EcD); //UNI
    ...
    function distributeDividend(address shareholder) internal {
        ...
        if(amount > 0){
            ...
            shares[shareholder].totalExcluded = getCumulativeDividends(shares[shareholder].amount);
            IERC20(SQUAWK).transfer(shareholder, amount);
        }
    }
    ...
    function getDividendsClaimedOf (address shareholder) external view returns (uint256) {
        require (shares[shareholder].amount > 0, "You're not a SQUAWKBANK shareholder!");
        return shares[shareholder].totalClaimed;
    }

    }

contract SquawkBank is IERC20, Auth {
    using SafeMath for uint256;
    ...
    string private constant  _name = "Bank Of Squawk";
    string private constant _symbol = "SQUAWK BANK";
    ...
    address payable public marketingWallet = payable(0xb0963230813F13FD04ca3a75B3BfB7AE41D36CF9);
    ...
    function swapBack() internal swapping {
        ...
        swapTokensForSQUAWK(amountToSwap.div(2));

        uint256 dividends = IERC20(SQUAWK).balanceOf(address(this));

        bool success = IERC20(SQUAWK).transfer(address(distributor), dividends);
    ...
    function swapTokensForSQUAWK(uint256 tokenAmount) private {
