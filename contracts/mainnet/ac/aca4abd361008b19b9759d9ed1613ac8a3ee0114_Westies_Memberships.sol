    ...
    constructor() {
        ...
        DOMAIN_SEPARATOR = keccak256(
            abi.encode(
                ...
                keccak256(bytes("WestiesWhitelist")),
                keccak256(bytes("1")),
                block.chainid,
                address(this)
            )
...
░░░╚═╝░░░╚═╝░░╚══════╝╚═════╝░░░░╚═╝░░░╚═╝╚══════╝╚═════╝░
          ...
          Westies | Membership Contract | October 22                                                                   
*/

contract Westies_Memberships is ERC1155, EIP712Whitelisting, ERC2981ContractWideRoyalties, CantBeEvil(LicenseVersion.CBE_PR_HS){
  ...
  constructor(address _proxyRegistryAddress, string memory contractURI_) ERC1155("") EIP712Whitelisting(){
    ...
    name = "Westies Memberships" ;
    symbol = "WESTIES";
  }
 
  event totalTokenUpdate(uint256 totalAmountMinted);

  function initiateToken(uint _tokenId, uint _amountToInit, uint _tokenSupply) public onlyOwner{
    require(!tokenList[_tokenId].validation, "Token already exists.");
    _mint(msg.sender, _tokenId, _amountToInit, "");
    tokenList[_tokenId].amountMinted += _amountToInit;
    tokenList[_tokenId].tokenSupply = _tokenSupply;
    tokenList[_tokenId].validation = true;
    tokenList[_tokenId].isPaused = true;
    emit totalTokenUpdate(tokenList[_tokenId].amountMinted);
  }
  ...
  function adminMint(address[] calldata _receivers, uint _tokenId) public onlyOwner {
    ...
    require(tokenList[_tokenId].validation, "Token doesn't exist.");
	  require(tokenList[_tokenId].amountMinted + length <= tokenList[_tokenId].tokenSupply, "Out of stock/not enough supply.");
	  for(uint i = 0; i < length; i++) {
        _mint(_receivers[i], _tokenId, 1, "");
        tokenList[_tokenId].amountMinted += 1;
        tokenList[_tokenId].userRedeemed[_receivers[i]] = true;
    }
    emit totalTokenUpdate(tokenList[_tokenId].amountMinted);
  }
  
  function publicMint(uint _tokenId, bytes calldata _signature) public payable requiresWhitelist(_signature){
    require(tokenList[_tokenId].userRedeemed[msg.sender] == false, "User redeemed membership already.");
    require(tokenList[_tokenId].amountMinted <= (tokenList[_tokenId].tokenSupply), "Current membership supply has run dry.");
    require(tokenList[_tokenId].validation, "Token doesn't exist.");
  ...
  function setTokenURI(uint _tokenId, string memory _uri) external onlyOwner {
    tokenList[_tokenId].tokenURI = _uri;
  ...
  function queryToken(uint _tokenId) public view returns (bool, uint, uint){
    return(tokenList[_tokenId].validation, tokenList[_tokenId].amountMinted, 
    tokenList[_tokenId].tokenSupply);
  }
