...
/**
 ...
 These punks are rewards to buyers of Gutter Punks during the month of October.
*/
...
/**
 * @title Apocalypse Punks contract.
 * @author The Gutter Punks team.
 *
 */
contract ApocalypsePunks is ERC721A, Ownable {
    using Strings for uint256;

    string public constant TOKEN_URI_EXTENSION = ".json";
    uint256 public constant MAX_SUPPLY = 1031;
    ...
    function claim(uint256 maxClaim, bytes32[] calldata merkleProof) external {
        uint256 _numClaimed = _claimed[msg.sender];
        uint256 _numToMint = maxClaim - _numClaimed;
        ...
        bytes32 leaf = keccak256(abi.encodePacked(msg.sender, maxClaim));
        require(MerkleProof.verify(merkleProof, _merkleRoot, leaf));

        _claimed[msg.sender] = _numClaimed + _numToMint;
    ...
    function mint(uint256 _numToMint) external payable {
        ...
        require(_numToMint > 0);
        uint256 _remainingSupply = MAX_SUPPLY - totalSupply();
        require(_numToMint <= _remainingSupply);
