...
contract TransferProxyInternal is OwnableOperatorRole {
...
contract TransferProxyForDeprecatedInternal is OwnableOperatorRole {
...
contract ERC20TransferProxyInternal is OwnableOperatorRole {
...
contract ExchangeDomainV1 {
    ...
    struct OrderKey {
        ...
        /* what wants owner */
        Asset buyAsset;
        /* Relayer Fee receipient */
        FeeRecipients relayerFee;
        /* Relayer Fee referral */
        address referralFee;
        /* blockchain custom key */
...
contract ExchangeStateV1Internal is OwnableOperatorRole {
...
contract ExchangeOrdersHolderV1Internal {
...
contract ExchangeV1 is Ownable, ExchangeDomainV1 {
    ...
    TransferProxyInternal public transferProxy;
    TransferProxyForDeprecatedInternal public transferProxyForDeprecated;
    ERC20TransferProxyInternal public erc20TransferProxy;
    ExchangeStateV1Internal public state;
    ExchangeOrdersHolderV1Internal public ordersHolder;

    constructor(
        TransferProxyInternal _transferProxy, TransferProxyForDeprecatedInternal _transferProxyForDeprecated, ERC20TransferProxyInternal _erc20TransferProxy, ExchangeStateV1Internal _state,
        ExchangeOrdersHolderV1Internal _ordersHolder, address payable _beneficiary, uint256 _blockchainId
    ) public {
    ...
    function setBlockchainId(uint256 _blockchainId) external onlyOwner {
        blockchainId = _blockchainId;
    }
    ...
    function exchange(
        ...
        require(sellOrder.key.blockchainId == blockchainId && buyOrder.order.key.blockchainId==blockchainId,"Invalid network");
        validateOrderSig(sellOrder, sellSig);
        if (buyer == address(0x0)) {
             buyer = msg.sender;
         }
        if(buyer!=msg.sender){
            validateBuyOrderSignature(buyOrder,buySig, buyer);
        }
        validateOrderParameters(sellOrder,buyOrder.order);
        uint paying = sellOrder.buying.mul(buyOrder.quantity).div(sellOrder.selling);
        require(paying == buyOrder.buyingAmount,"Invalid amount");
        verifyOpenAndModifyOrderState(sellOrder.key, sellOrder.selling, buyOrder.quantity);
         require(sellOrder.key.sellAsset.assetType != AssetType.ETH &&
         sellOrder.key.sellAsset.assetType != AssetType.ERC20
          , "ETH is not supported on sell side");
         if (sellOrder.key.buyAsset.assetType == AssetType.ETH) {
             validateEthTransfer(paying);
        }

        
         transferWithFeesPossibility(sellOrder,paying,buyer);

         transferSellAsset(sellOrder,buyOrder,buyer);

        emitBuy(sellOrder, buyOrder.quantity, buyer);
    }
    ...
    function cancel(OrderKey calldata key) external {
        require(key.blockchainId == blockchainId,"Invalid network");
    ...
    function validateBuyOrderSignature(
        BuyOrder memory order,
        Sig memory sig,
        address buyer
    ) internal pure {
            require(prepareMessageBuyOrder(order).recover(sig.v, sig.r, sig.s) == buyer, "incorrect signature");
        }
    

    function validateOrderParameters(Order memory sellOrder, Order memory buyOrder) internal pure 
    {
        require((buyOrder.key.owner==address(0) || sellOrder.key.owner==buyOrder.key.owner) &&
            sellOrder.key.sellAsset.token == buyOrder.key.sellAsset.token &&
            sellOrder.key.sellAsset.tokenId == buyOrder.key.sellAsset.tokenId &&
            sellOrder.key.sellAsset.assetType == buyOrder.key.sellAsset.assetType &&
            sellOrder.key.buyAsset.token == buyOrder.key.buyAsset.token &&
            sellOrder.key.buyAsset.tokenId == buyOrder.key.buyAsset.tokenId &&
            sellOrder.key.buyAsset.assetType == buyOrder.key.buyAsset.assetType &&
            sellOrder.key.salt == buyOrder.key.salt &&
            sellOrder.key.relayerFee.feeRecipient == buyOrder.key.relayerFee.feeRecipient &&
            sellOrder.key.relayerFee.fee == buyOrder.key.relayerFee.fee &&
            (buyOrder.key.referralFee==address(0) || sellOrder.key.referralFee == buyOrder.key.referralFee) &&
            sellOrder.buying == buyOrder.buying,"Invalid order");
    }
    ...
    function prepareMessageBuyOrder(BuyOrder memory order) public pure returns (string memory) {
        return keccak256(abi.encode(order)).toString();
    }

    function transferSellAsset(Order memory sellOrder,BuyOrder memory buyOrder,address buyer) internal
    {   
        transfer(sellOrder.key.sellAsset, buyOrder.quantity, sellOrder.key.owner, buyer);
    }

    function transferWithFeesPossibility(Order memory sellOrder,uint256 paying,address buyer) internal {
      //  transferWithFees(sellOrder.key.buyAsset, paying, buyer, sellOrder.key.owner, sellOrder.sellerFee,sellOrder.key.relayerFee,sellOrder.key.referralFee);

        uint restValue = transferFeeToBeneficiary(sellOrder.key.buyAsset, buyer, paying, sellOrder.sellerFee);
        if(sellOrder.key.relayerFee.feeRecipient!=address(0) && sellOrder.key.relayerFee.fee>0){
            (uint newRestValue, uint current) = subFeeInBp(restValue, paying, sellOrder.key.relayerFee.fee);
            restValue = newRestValue;
            transfer(sellOrder.key.buyAsset, current, buyer, sellOrder.key.relayerFee.feeRecipient);
        }
        if(sellOrder.key.referralFee!=address(0) && referralPercentage>0){
            (uint newRestValue, uint current) = subFeeInBp(restValue, paying, referralPercentage);
            restValue = newRestValue;
            transfer(sellOrder.key.buyAsset, current, buyer, sellOrder.key.referralFee);
            emit RefEarning(
         sellOrder.key.sellAsset.token, sellOrder.key.sellAsset.tokenId,
        sellOrder.key.owner,
        sellOrder.key.referralFee,
        current
    );
        }
        address payable toPayable = address(uint160(sellOrder.key.owner));
        transfer(sellOrder.key.buyAsset, restValue, buyer, toPayable);
    }
    ...
    function transferWithFees(Asset memory firstType, uint value, address from, address to, uint256 sellerFee, FeeRecipients memory relayerFees,FeeRecipients memory referralFee) internal {
        uint restValue = transferFeeToBeneficiary(firstType, from, value, sellerFee);
        if(relayerFees.feeRecipient!=address(0) && relayerFees.fee>0){
            (uint newRestValue, uint current) = subFeeInBp(restValue, value, relayerFees.fee);
            restValue = newRestValue;
            transfer(firstType, current, from, relayerFees.feeRecipient);
        }
        if(relayerFees.feeRecipient!=address(0) && referralFee.fee>0){
            (uint newRestValue, uint current) = subFeeInBp(restValue, value, referralFee.fee);
            restValue = newRestValue;
            transfer(firstType, current, from, referralFee.feeRecipient);
    ...
    function transferFeeToBeneficiary(Asset memory asset, address from, uint total, uint sellerFee) internal returns (uint) {
        (uint restValue, uint sellerFeeValue) = subFeeInBp(total, total, sellerFee);

        if (sellerFeeValue > 0) {
            transfer(asset, sellerFeeValue, from, beneficiary);
        }
        return restValue;
    }
