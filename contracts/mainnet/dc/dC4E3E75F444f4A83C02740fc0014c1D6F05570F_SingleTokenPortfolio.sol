...
contract SingleTokenPortfolio is Portfolio, ReentrancyGuard {
     ...
     */
    function deposit(uint256 _amountBaseToken, bytes calldata _data) external override nonReentrant returns (uint256) {
     ...
     * i.e. `bytes.concat(abi.encodePacked(address swapWrapper), SWAP_WRAPPER_BYTES)`.
     */
    function redeem(uint256 _amountShares, bytes calldata _data) external override nonReentrant returns (uint256) {
