...
contract Goo is ERC20("Goo", "GOO", 18) {
    ...
    function mintForGobblers(address to, uint256 amount)
        external
        only(artGobblers)
    {
        _mint(to, amount);
    ...
    // / @param  The address of the user to burn goo from.
    /// @param amount The amount of goo to burn

    function burnForGobblers(uint256 amount) external only(artGobblers) {
