...
contract InuNFT is ERC721, Ownable, ERC721Burnable, ERC721Pausable {
    ...
    constructor()
    ERC721("THE INU NFT CLUB", "InuClub") 
    {
        pause(false);
    ...
    function freeMint(uint256 _count) public payable saleIsOpen noContract {
        uint256 total = totalSupply();
        require(publicSaleOpen, "Public sale not open yet");
        require(total + _count <= MAX_ELEMENTS, "Max limit");
        require(_count <= MAX_BY_MINT, "Exceeds number");
        require(MINT_NUMBER <= 900, "max 900 free mint");
    ...
    //     uint256 maxMintAmount = MAX_BY_MINT;
