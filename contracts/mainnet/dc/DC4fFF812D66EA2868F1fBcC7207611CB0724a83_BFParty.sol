...
contract BFParty is ERC721A, Ownable {
    ...
    uint8 constant MAX_PUBLIC_MINT = 3;

    uint256 constant WL_MINT_PRICE = 0.169 ether;
    uint256 constant PUBLIC_MINT_PRICE = 0.2 ether;
    address constant FUND_WALLET =0x2eC9A9a67c80DaDDfBcbEBc7A152496C25bD3a03;
    address constant TECH_WALLET= 0x6f0E887dd23dEB37d7804fCB8EF4285a93DA485a;
    ...
    function withdraw() external onlyOwner {
        ...
        require(successTech || successFunds,"withdrawal unsuccessful for all.");        
    }
    ...
    function publicMint(        
        ...
        require(
            signerAddress == hashTransaction(msg.sender, "PM").recover(signature),
    ...
    {
        require(
            signerAddress == hashTransaction(msg.sender, "WM").recover(signature),
            "Direct minting disallowed"
        );        
        require(
    ...
    function freeMint(bytes memory signature) external {
        require(
            signerAddress == hashTransaction(msg.sender, "FM").recover(signature),
            "Direct minting disallowed"
        );        
        require(
