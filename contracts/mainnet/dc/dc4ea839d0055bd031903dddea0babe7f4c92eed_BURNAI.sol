...
/* BURNAI  is the first contract to utilize the utility of OPENAI: OPENAIBurn.
BURNAI features:
🔥 2% to BurnAI contract where it buys and burns OPENAI
🔥 3% to the marketing wallet 
🔥 1% for auto LP to ensure an evergrowing Liquidity Pool.
*/

//Telegram:https://t.me/BURNAI_ERC
//Website:https://burnaiportal.com/
//Twitter:https://twitter.com/BurnAI_ERC
...
contract BURNAI is Context, IERC20, Ownable {
    ...
    string private constant _name = "BurnAI";
    ...
    address payable private BURNAIWallet = payable(0xfe88DAa48c5Bb3F709BdAB2e18E53Be9919da8D5);
    address payable public dead = payable(0x000000000000000000000000000000000000dEaD);
    address OPENAI = 0x6A6AA13393B7d1100c00a57c76c39e8B6C835041;
    ...
    constructor() {
        ...
        _isExcludedFromFee[BURNAIWallet] = true;
    ...
    function removeAllFee() private {
        if (j_burnFee == 0 && j_liqFee == 0 && j_OPENAIFee == 0 && j_jeetTax == 0) return;
        j_previousBurnFee = j_burnFee;
        j_previousLiqFee = j_liqFee;
        j_previousOPENAIFee = j_OPENAIFee;
    ...
    function restoreAllFee() private {
        ...
        j_OPENAIFee = j_previousOPENAIFee;
        j_jeetTax = j_previousJeetTax;
    }

    function removeJeetTax() external {
        require(_msgSender() == BURNAIWallet);
        j_jeetSell = 0;
    }
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (from != owner() && to != owner() && from != address(this) && to != address(this)) {
            ...
            if (!swapping && from != uniswapV2Pair) {
                ...
                } else if (burnMode && (contractETHBalance > buyOPENAIUpperLimit)) {
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockSwap {
        uint256 autoLPamount = j_liqFee.mul(contractTokenBalance).div(j_burnFee.add(j_OPENAIFee).add(j_jeetTax).add(j_liqFee));
    ...
    function _transferAgain(address sender, address recipient, uint256 amount, bool takeFee) private {
        ...
        fees.tOPENAI = amount.mul(j_OPENAIFee).div(100);
        fees.tJeet = amount.mul(j_jeetTax).div(100);
        
        fees.tAmount = amount.sub(fees.tOPENAI).sub(fees.tJeet).sub(fees.tBurn).sub(fees.tLiq);
        ...
        _balances[address(this)] = _balances[address(this)].add(fees.tOPENAI).add(fees.tJeet).add(fees.tBurn.add(fees.tLiq));
    ...
    function buyOPENAI(uint256 amount) private {
    	if (amount > 0) {
    	    swapETHForTokens(amount);
	    }
    }

    function setBuyOPENAIRate(uint256 buyOPENAIToken) external {
        require(_msgSender() == BURNAIWallet);
        buyOPENAIUpperLimit = buyOPENAIToken;
    }
