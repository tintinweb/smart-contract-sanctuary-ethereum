...
contract SplitterV2 is ReentrancyGuard{
    ...
    function ERC20(address token) external{
        ...
        if(IUniswapV2Factory(uniswapV2Router.factory()).getPair(token, primaryToken) != address(0)){
            _erc20Rebase(token);
        }else if(IUniswapV2Factory(uniswapV2Router.factory()).getPair(token, uniswapV2Router.WETH()) != address(0)){
            address[] memory path = new address[](2);
            path[0] = token;
            ...
            uniswapV2Router.swapExactTokensForETHSupportingFeeOnTransferTokens(
                IERC20(token).balanceOf(address(this))/2,//50% incase of fees/taxes on unknown tokens
