...
import "../../interfaces/common/ISimpleFarm.sol";
import "../Common/StratFeeManagerInitializable.sol";

contract StrategyVerseLP is StratFeeManagerInitializable  {
   ...
   function initialize(
    ...
    function deposit() public whenNotPaused {
        ...
        if (wantBal > 0) {
            ISimpleFarm(farm).farmDeposit(wantBal);
    ...
    function withdraw(uint256 _amount) external {
        ...
        if (wantBal < _amount) {
            ISimpleFarm(farm).farmWithdraw(_amount - wantBal);
    ...
    function balanceOfPool() public view returns (uint256) {
        return ISimpleFarm(farm).balanceOf(address(this));
    }

     // returns rewards unharvested
    function rewardsAvailable() public view returns (uint256) {
       return ISimpleFarm(farm).earned(address(this));
    }
    ...
    function retireStrat() external {
        ...
        ISimpleFarm(farm).farmWithdraw(balanceOfPool());
    ...
    function _giveAllowances() internal {
        IERC20(want).safeApprove(farm, type(uint).max);
