...
//SPDX-License-Identifier: MIT
pragma solidity ^0.8.0;
// https://etherscan.io/address/0xd09656a2EE7E5Ee3404fAce234e683D3337dA014#code
// https://etherscan.io/address/0x7b0fce54574d9746414d11367f54c9ab94e53dca#code
...
contract CafeStaking is Ownable, IERC721Receiver, ReentrancyGuard, Pausable {
...
/* STAKING MECHANICS */
        ...
        returns (uint256)
    {
        ...
        if (_deposits[account].contains(tokenId)) {
            return rate * (Math.min(block.number, expiration) - _depositBlocks[account][tokenId]);
        } else {
            return 0;
        }
    }
    ...
    function claimRewards(uint256[] calldata tokenIds) public whenNotPaused {
        uint256 reward;
        uint256 curBlockNumber = Math.min(block.number, expiration);
        for (uint256 i; i < tokenIds.length; i++) {
            reward += calculateReward(msg.sender, tokenIds[i]);
            _depositBlocks[msg.sender][tokenIds[i]] = curBlockNumber;
        }
        if (reward > 0) {
