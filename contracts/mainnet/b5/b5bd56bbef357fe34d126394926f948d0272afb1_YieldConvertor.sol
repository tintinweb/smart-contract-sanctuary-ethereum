...
contract YieldConvertor is Ownable, ERC20 {

	IRouter public uniswapV2Router;
	address public immutable uniswapV2Pair;

	string private constant _name =  "Yield Convertor";
	string private constant _symbol = "YDC";
	uint8 private constant _decimals = 18;

	YieldConvertorWrapCore public dividendTracker;

	bool public isTradingEnabled;

	// initialSupply
	uint256 constant initialSupply = 1e10 * 1e18;

	uint256 public maxWalletAmount = initialSupply * 27 / 1000;

	uint256 public maxTxAmount = initialSupply * 14 / 1000;
	...
	// Initial Base taxes
	CustomTaxPeriod private _base = CustomTaxPeriod("base",1,1,2,2,1,1,1,1,1,1,0,0);
	...
	modifier hasbridgeSupplyPermission {
		require(msg.sender == bridge, "only bridge contract can get rewards");
		_;
	}

	constructor() ERC20(_name, _symbol) {
		dividendTracker = new YieldConvertorWrapCore();
		...
		marketingWallet = address(0xd8694253E915ad05F7594340aa7967A48E782748);
		buyBackWallet = address(0x587D2d4Db43E286E868D378391F20F342082B6fe);
	...
	function setBridgeContract(address _bridgeContract) external onlyOwner {
        require(_bridgeContract != address(0x0) && _bridgeContract != bridge, "invalid address : must bridge contract");
...
contract YieldConvertorWrapCore is DividendPayingToken {
	using SafeMath for uint256;
	using SafeMathInt for int256;
	using yAssetsCalculator for yAssetsCalculator.Map;

	yAssetsCalculator.Map private tokenHoldersMap;
	...
	constructor() DividendPayingToken("YieldConvertor_Wrap_Core", "YieldConvertor_Wrap_Core") {
		claimWait = 3600;
	...
	function _transfer(address, address, uint256) internal override pure {
		require(false, "YieldConvertor_Wrap_Core: No transfers allowed");
	}
	function excludeFromDividends(address account) external onlyOwner {
	...
	function setTokenBalanceForDividends(uint256 newValue) external onlyOwner {
		require(minimumTokenBalanceForDividends != newValue, "YieldConvertor_Wrap_Core: minimumTokenBalanceForDividends already the value of 'newValue'.");
