...
interface iShroomates {
    function genBalance(address owner) external view returns (uint256);
}

contract GnarCoin is ERC20, Ownable {
    iShroomates public Shroomates;
    ...
    constructor(address ShroomAddress) ERC20("GnarCoin", "GNAR") {
        Shroomates = iShroomates(ShroomAddress);
        START = block.timestamp;
    }

    function updateReward(address from, address to) external {
        require(msg.sender == address(Shroomates));
        if (from != address(0)) {
    ...
    function burn(address user, uint256 amount) external {
        require(
            allowedAddresses[msg.sender] || msg.sender == address(Shroomates),
