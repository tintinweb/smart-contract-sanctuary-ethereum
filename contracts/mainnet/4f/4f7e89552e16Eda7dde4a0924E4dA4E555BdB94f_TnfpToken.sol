...
contract TnfpToken is ERC1155, Ownable {
    ...
    function mint(
        ...
        require(
            _msgSender() == nft.ownerOf(nftTokenId),
    ...
    function mintProxy(
        ...
        ERC721 nft = ERC721(nftAddress);
        require(
            originator == nft.ownerOf(nftTokenId),
            "You must own the NFT being minted to a tNFP"
        );
        return
            _finaliseMint(
    ...
    /// @notice Allows transfer to trusted TNFP Market place for less gas.
    /**
     * @dev See {IERC1155-safeTransferFrom} modified to remove approval for trader account.
     */
    function safeTransferFrom(
        ...
        if (to != _tNfpTokenTraderAddress) {
            ...
            require(
                _tNfpTokenTraderAddress == msg.sender,
                "ERC1155: only market place can transfer to self"
            );
        }
