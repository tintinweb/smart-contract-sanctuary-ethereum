...
contract OrdinalMona is ERC721A, Ownable, ReentrancyGuard {
    using Strings for uint256;
    ...
    uint256 public  btcAddress = 62;
    uint256 public  maxSupply = 3000;
    uint256 public  freeSupply = 2000;
    uint256 public  cost = 0.002 ether;
    uint256 public  inscriptionFee = 0 ether;
    address private burnAddress = 0x000000000000000000000000000000000000dEaD;

    mapping(uint256 => string) public migration;
    event MIGRATE(uint256 tokenId, string destination);

    constructor() ERC721A("Ordinal Mona", "OM") {}

    function mint(uint256 _mintAmount) public payable nonReentrant {
        require(state == 2, "Inactive");
        require(
    ...
    function freeMint(uint256 _mintAmount) public nonReentrant {
        require(state == 1, "Inactive");
        require(
            _mintAmount > 0 && _mintAmount <= maxFreeMint,
            "Invalid amount"
        );
        require(free + _mintAmount <= freeSupply, "Free supply");
        require(
            totalSupply() + _mintAmount <= maxSupply,
            "Max supply"
        );
    ...
    function migrateMona(uint256 tokenId, string memory destination)
        public
        payable
        nonReentrant
    {
        require(burn, "Inactive");
        require(msg.value >= inscriptionFee, "Fee invalid");
        require(
            bytes(destination).length == btcAddress,
            "Please use a taproot address"
        );
        safeTransferFrom(msg.sender, burnAddress, tokenId);
        migration[tokenId] = destination;
        emit MIGRATE(tokenId, destination);
    }
