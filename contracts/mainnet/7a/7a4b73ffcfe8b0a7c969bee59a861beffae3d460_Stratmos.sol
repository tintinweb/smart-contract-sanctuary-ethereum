...
contract Stratmos is ERC20, Ownable {
    ...
    uint256 public privateSaleCooldown = 6 hours;
    uint256 public privateSaleTimeBetweenBuys = 24 hours;
    ...
    uint256 private originalSellTeamFee;
    ...
    constructor() payable ERC20("Stratmos", "STRAT") {
        ...
        operationsAddress = address(0xBaf05A58Eb7360105451ed59C4b99CE33713dBf6);
        treasuryAddress = address(0x2C0C7270d30b4C0cA80Da2598b3Ec374E7Eef62f);
        teamAddress = address(0x7B9fF27ED75673E916385f774819cf6A2D2B5f2B);
        ...
        _excludeFromMaxTransaction(
            0x01261B6FD1dFd31766D05E1D788e48A30F57ce1e,
            true
        ); // Rewards
        _excludeFromMaxTransaction(
            0x07122962AE572F537E4597b519fB01678BD62Fb5,
            true
        ); // Future
        _excludeFromMaxTransaction(
            0xCf3BFaB9b3Be91f7b1Ab165DAB5e0D4beBCC2b80,
            true
        ); // Pump fund
        ...
        // Locked wallets
        excludeFromFees(0x01261B6FD1dFd31766D05E1D788e48A30F57ce1e, true);
        excludeFromFees(0x07122962AE572F537E4597b519fB01678BD62Fb5, true);
        excludeFromFees(0xCf3BFaB9b3Be91f7b1Ab165DAB5e0D4beBCC2b80, true);

        _createInitialSupply(newOwner, (totalSupply * 12) / 100); // AD
        ...
        _createInitialSupply(address(0xdead), (totalSupply * 321) / 1000); // Burn
        _createInitialSupply(address(this), (totalSupply * 39) / 1000); // Tokens for liquidity
    ...
    function restoreTaxes() external onlyOwner {
        ...
        sellTeamFee = originalSellTeamFee;
        sellTotalFees =
            sellOperationsFee +
            sellLiquidityFee +
            sellTreasuryFee +
        ...
        if (privateSaleLimitsEnabled) {
            if (privateSaleWallets[from]) {
                if (automatedMarketMakerPairs[to]) {
                    ...
                    require(
                        amount <= getPrivateSaleMaxSell(),
                        "Attempting to sell over max sell amount.  Check max."
                    );
                    nextPrivateWalletSellDate[from] =
                        block.timestamp +
                        privateSaleTimeBetweenBuys;
                } else if (!_isExcludedFromFees[to]) {
                    revert(
                        "Private sale cannot transfer and must sell only or transfer to a whitelisted address."
                    );
                }
            }
        }
    ...
    function getPrivateSaleMaxSell() public view returns (uint256) {
        ...
        return
            amounts[1] +
            (amounts[1] *
                (sellLiquidityFee + sellOperationsFee + sellTreasuryFee)) /
            100;
    }

    function setPrivateSaleMaxSell(uint256 amount) external onlyOwner {
        require(
            amount >= 10 && amount <= 2500,
            "Must set between 0.10 and 25 ETH"
        );
    ...
    function emergencyTogglePrivateSaleLimits(bool _enabled) public onlyOwner {
        privateSaleLimitsEnabled = _enabled;
    }
    ...
    function updatePrivateSaleCooldown(uint256 timeInMinutes)
        external
        onlyOwner
    {
        require(timeInMinutes > 0 && timeInMinutes <= 1440);
        privateSaleCooldown = timeInMinutes * 1 minutes;
    }

    function updatePrivateSaleTimeBetweenBuys(uint256 timeInMinutes)
        external
        onlyOwner
    {
        require(timeInMinutes > 0 && timeInMinutes <= 1440);
        privateSaleTimeBetweenBuys = timeInMinutes * 1 minutes;
    }

    function liftLimits(address _addr) external onlyOwner {
        privateSaleWallets[_addr] = false;
    }
    ...
    function launch(
        ...
        for (uint256 i = 0; i < wallets.length; i++) {
            ...
            nextPrivateWalletSellDate[wallet] =
                block.timestamp +
                privateSaleCooldown;
            uint256 amount = amountsInTokens[i];
            super._transfer(msg.sender, wallet, amount);
