...
import {IGuildURIHandler} from "../interfaces/IGuildURIHandler.sol";
...
contract Raid is IRaid, Initializable, AccessControlUpgradeable {
    ...
    function setGuild(IGuildURIHandler _guild)
        external
    ...
    function _rewardCalculation(
        ...
        unchecked {
            ...
            blocksDefeated =
                // Math.min(block.number, round.finalBlock)
                (
                    block.number < round.finalBlock
                        ? block.number
                        : round.finalBlock
                ) -
                // Math.max(round.startBlock, raider.startBlock)
                (
            ...
            reward = blocksDefeated * bossMultiplier * raider.dpb;
        }
    }
}
