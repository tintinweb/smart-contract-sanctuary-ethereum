...
contract PiGamificationStaking2 is
    Initializable,
    OwnableUpgradeable,
    ...
    TokensRecoverable
{
        ...
        returns (uint32[] memory)
    {
        ...
        for (uint32 i = 0; i < arrStaked.length; i++)
            if (isStaked[arrStaked[i]] && ownerOfNFT[arrStaked[i]] == account)
                newArrSize++;
        ...
        updateReward(_msgSender())
    {
        ...
        for (uint32 i = 0; i < tokenIds.length; i++) {
            ...
            uint256 value = BLLPoints[tokenIds[i]];
            uint256 newTotalSupply = _totalSupply.sub(value);
            uint256 newBalance = _balances[_msgSender()].sub(value);
            _totalSupply = newTotalSupply;
    ...
    function _withdrawAll() internal updateReward(_msgSender()) {
        uint256 totalValue;
        uint32[] memory tokenIDarr = tokenIdsStaked(_msgSender());
        stakingToken.batchTransferFromSmallInt(
            address(this),
            _msgSender(),
            tokenIDarr
