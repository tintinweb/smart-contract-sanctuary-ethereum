...
contract WildJunePass is WildNFT {
    constructor(address _minter, uint256 _maxSupply, string memory _baseURI, address[] memory _payees, uint256[] memory _shares, uint96 _feeNumerator) WildNFT('Wild June Pass', 'JUNEPASS', _minter, _maxSupply, _baseURI, _payees, _shares, _feeNumerator) {}
    ...
    function tokenURI(uint256 _tokenId) public view override returns (string memory) {
        ...
        return baseURI; //string(abi.encodePacked(baseURI, Strings.toString(_tokenId), '.json'));
    }

    function setMaxSupply(uint256 _maxSupply) public onlyOwner {
        require(_maxSupply >= totalSupply(), 'Max supply must be greater than or equal to the current supply.');
