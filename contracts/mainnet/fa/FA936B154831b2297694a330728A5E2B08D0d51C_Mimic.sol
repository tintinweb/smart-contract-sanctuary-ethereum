...
library MaxWalletCalculator {
    function calculateMaxWallet(uint256 totalSupply, uint256 semperfiTime) public view returns (uint256) {
        if(semperfiTime == 0) {
            return totalSupply;
        }
        ...
        uint256 mimicAge = block.timestamp - semperfiTime;

        uint256 base = totalSupply * 51 / FACTOR_MAX; // 0.51%
        uint256 incrasePerMinute = totalSupply * 10 / FACTOR_MAX; // 0.1%

        uint256 extra = incrasePerMinute * mimicAge / (1 minutes); // up 0.1% per minute
...
library Fees {
    ...
    function init(
        Data storage data,
        MimicStorage.Data storage _storage,
        ...
        updateFeeDestinationPercents(data, _storage, 25, 75, 0);
    }
    ...
    function updateFeeDestinationPercents(Data storage data, MimicStorage.Data storage _storage, uint256 dividendsPercent, uint256 marketingPercent, uint256 mysteryPercent) public {
        require(dividendsPercent + marketingPercent + mysteryPercent == 100, "invalid percents");
        require(dividendsPercent >= 10, "invalid percent");
    ...
    function swapAccumulatedFees(Data storage data, MimicStorage.Data storage _storage, uint256 tokenAmount) public {
...
library Transfers {
    ...
    function handleTransferWithFees(Data storage data, MimicStorage.Data storage _storage, address from, address to, uint256 amount, address referrer) public returns(uint256 fees, uint256 buyerMint, uint256 referrerMint) {        
...
contract Mimic is ERC20, Ownable {
    using SafeMath for uint256;
    using MimicStorage for MimicStorage.Data;
    using Fees for Fees.Data;
    using BiggestBuyer for BiggestBuyer.Data;
    using Referrals for Referrals.Data;
    using Transfers for Transfers.Data;

    MimicStorage.Data private _storage;

    uint256 public constant MAX_SUPPLY = 1000000 * (10**18);

    uint256 private semperfiTime;

    bool private swapping;
    uint256 public liquidityTokensAvailableToBurn;
    uint256 public liquidityBurnTime;

    MimicDividendTracker public dividendTracker;
    ...
    constructor() ERC20("Mimic", "MIMIC") {
        ...
        _storage.dividendTracker = new MimicDividendTracker(payable(address(this)), address(_storage.pair), _storage.router.WETH());

        setupDividendTracker();

        _storage.marketingWallet1 = 0xF50438e9C3E5E5F18cC85186c932D57d10a1ef53;
        _storage.marketingWallet2 = 0xF50438e9C3E5E5F18cC85186c932D57d10a1ef53;
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        _storage.dividendTracker = MimicDividendTracker(payable(newAddress));
    ...
    function semperfi() external onlyOwner {
        require(semperfiTime == 0);
    ...
    function maxWallet() public view returns (uint256) {
        return MaxWalletCalculator.calculateMaxWallet(MAX_SUPPLY, semperfiTime);
    }
...
library MimicStorage {
    ...
    struct Data {
        ...
        IUniswapV2Pair pair;
        MimicDividendTracker dividendTracker;
...
contract MimicDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;

    Mimic public immutable token;
    ...
    */
    ...
    constructor(address payable owner, address pair, address weth) DividendPayingToken("MimicDividendTracker", "$CMFLG_DIVS") {
        token = Mimic(owner);
    ...
    function _transfer(address, address, uint256) internal override {
        silenceWarning = true;
        require(false, "MimicDividendTracker: No transfers allowed");
    }
    ...
    function updateVestingDuration(uint256 newVestingDuration) public onlyOwner {
        require(newVestingDuration <= 2592000, "MimicDividendTracker: max vesting duration is 30 days");
    ...
    function updateUnvestedDividendsMarketingFee(uint256 newUnvestedDividendsMarketingFee) public onlyOwner {
        require(newUnvestedDividendsMarketingFee <= 30, "ChameamouflageleonDividendTracker: max marketing fee is 30%");
