...
// Root file: contracts/MysticWizardsFamiliars.sol


/*
*
* Mystic Wizards Familiars Contract
* 
...
contract MysticWizardsFamiliars is ERC721, Ownable {
    using Address for address;
    using Strings for uint256;
    using Counters for Counters.Counter;
    using ECDSA for bytes32;

    address public mainContract = 0xbE6A2F5960B27c7fde12E073d61b962D9C6c3cB7;
    string public baseTokenURI = "https://storage.hashku.com/api/mystic-wizards/familiars/";
    bool public isOpen;

    constructor() ERC721("Mystic Wizards Familiars", "MYSTICFAMILIARS") {}

    // external contract used for checking owners and existence of redeemed tokens
    function setMainContract(address _address) external onlyOwner {
        mainContract = _address;
    }

    function getMainContractOwner(uint256 _tokenId) public view returns (address) {
        IERC721 main = IERC721(mainContract);
        return main.ownerOf(_tokenId);
    }

    // owner set the contract to closed
    function setIsOpen(bool _open) external virtual onlyOwner {
        isOpen = _open;
    }

    function redeem(uint256[] calldata _tokenIds) public {
        require(isOpen, "not_open");

        for (uint256 i = 0; i < _tokenIds.length; i++) {
            _safeMint(getMainContractOwner(_tokenIds[i]), _tokenIds[i]);
        }
    }

    function redeemOwn(uint256[] calldata _tokenIds) external {
        for (uint256 i = 0; i < _tokenIds.length; i++) {
            require(getMainContractOwner(_tokenIds[i]) == _msgSender(), "not_owner");
        }
