...
interface IRewards {
  ...
  function depositEpochRewardAmount(uint256 _epochRewardAmount) external;
...
contract HaloHalo is ERC20('Rainbow Pool', 'xRNBW') {
    ...
    function leave(uint256 _share) public {
        ...
        uint256 haloAmount = _share.mul(halo.balanceOf(address(this))).div(
            totalShares
        );
    ...
    function getCurrentHaloHaloPrice() public view returns (uint256) {
        ...
        uint256 haloHaloPrice = halo.balanceOf(address(this)).mul(DECIMALS).div(
            totalShares
        );
