...
contract Worochi is Context, IERC20, Ownable {
    ...
    address public developmentWallet = 0xb875452508347a5260c1D0502527E695d5bb6F9A;
    address public treasuryWallet = 0xb875452508347a5260c1D0502527E695d5bb6F9A;

    string private constant _name = "Worochi no Ara-masa";
    string private constant _symbol = "WOROCHI";
    ...
    Taxes public taxes = Taxes(0, 0, 0, 4);
    Taxes public sellTaxes = Taxes(0, 0, 0, 4);
    ...
    function setTaxes(uint256 _rfi, uint256 _development, uint256 _liquidity, uint256 _treasury) public onlyOwner {
        require(_rfi+_development+_liquidity+_treasury < 10);
       taxes = Taxes(_rfi,_development,_liquidity,_treasury);
        emit FeesChanged();
    }
    
    function setSellTaxes(uint256 _rfi, uint256 _development, uint256 _liquidity, uint256 _treasury) public onlyOwner {
        require(_rfi+_development+_liquidity+_treasury < 10);
       sellTaxes = Taxes(_rfi,_development,_liquidity,_treasury);
        emit FeesChanged();
    }
    ...
    function _getValues(uint256 tAmount, bool takeFee, bool isSell) private view returns (valuesFromGetValues memory to_return) {
        ...
        (to_return.rAmount, to_return.rTransferAmount, to_return.rRfi, to_return.rburn, to_return.rLiquidity) = _getRValues1(to_return, tAmount, takeFee, _getRate());
        (to_return.rtreasury) = _getRValues2(to_return, takeFee, _getRate());
        return to_return;
    }
    ...
    function _getTValues(uint256 tAmount, bool takeFee, bool isSell) private view returns (valuesFromGetValues memory s) {
        ...
        s.tTransferAmount = tAmount-s.tRfi-s.tburn-s.tLiquidity-s.ttreasury;
        return s;
    }

    function _getRValues1(valuesFromGetValues memory s, uint256 tAmount, bool takeFee, uint256 currentRate) private pure returns (uint256 rAmount, uint256 rTransferAmount, uint256 rRfi,uint256 rburn, uint256 rLiquidity){
        ...
        rTransferAmount =  rAmount-rRfi-rburn-rLiquidity-rtreasury;
        return (rAmount, rTransferAmount, rRfi,rburn,rLiquidity);
    }
    
    function _getRValues2(valuesFromGetValues memory s, bool takeFee, uint256 currentRate) private pure returns (uint256 rtreasury) {
    ...
    function _tokenTransfer(address sender, address recipient, uint256 tAmount, bool takeFee, bool isSell) private {
        ...
        if(s.rLiquidity > 0 || s.tLiquidity > 0) {
            ...
            emit Transfer(sender, address(this), s.tLiquidity + s.tburn + s.ttreasury);
        }
    ...
    function swapAndLiquify(uint256 contractBalance, Taxes memory temp) private lockTheSwap{
        uint256 denominator = (temp.liquidity + temp.burn + temp.treasury) * 2;
        ...
        if(burnAmt > 0){
            payable(developmentWallet).sendValue(burnAmt);
        }
