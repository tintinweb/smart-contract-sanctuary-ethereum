...
contract CRYPTOBILIA is ERC721Enum, Ownable, ReentrancyGuard {
    ...
    uint256 public SALE_NFT = 4780;
	uint256 public GIVEAWAY_NFT = 220;
	
	uint256 public MAX_MINT_PRESALE = 4780;
	uint256 public MAX_MINT_SALE = 4780;
	...
	uint256 public SALE_PRICE = 0.14 ether;
	...
	function mintGiveawayNFT(address[] calldata _to, uint256[] calldata _count) external onlyOwner nonReentrant{
        require(_to.length == _count.length,"Mismatch between Address and count");
		for(uint i=0; i < _to.length; i++){
		    uint256 totalSupply = totalSupply();
		    require(
				GIVEAWAY_MINTED + _count[i] <= GIVEAWAY_NFT, 
				"Max limit"
			);
