...
contract BDK is ERC20, Ownable {
    ...
    constructor() ERC20("Bank of DoKwon", "BDK") {
        ...
        uint256 totalSupply = 12121212121 * 1e18;
        ...
        marketingWallet = address(0xA314d6C9e356834eD90DBB886A5b921FB6fc5F06);
    ...
    function _transfer(
        ...
        if(takeFee){
			// buy
			if (automatedMarketMakerPairs[to] && TaxFee > 0){
			    fees = amount.mul(TaxFee).div(100);
				tokensForTax += fees;
			}
			// sell
			else if(automatedMarketMakerPairs[from] && TaxFee > 0) {
 			    fees = amount.mul(TaxFee).div(100);
				tokensForTax += fees;
 			}
