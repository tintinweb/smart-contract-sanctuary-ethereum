...
contract katosa is Context, IERC20, Ownable {
  using SafeMath for uint256;
  using Address for address;

  address payable public liquidityAddress =
    payable(0x8dB8028C5EE495aFeb23D241C09e9e3900419A8b); // liquidity Address
  ...
  string private _name = 'Katosa Shrine';
  string private _symbol = 'Katosa';
  ...
  function swapTokens(uint256 contractTokenBalance) private lockTheSwap {
    ...
    if (contractETHBalance > 0) {
      sendETHToliquidity(address(this).balance);
    }
  }

  function sendETHToliquidity(uint256 amount) private {
    // Ignore the boolean return value. If it gets stuck, then retrieve via `emergencyWithdraw`.
    liquidityAddress.call{ value: amount }('');
  }
