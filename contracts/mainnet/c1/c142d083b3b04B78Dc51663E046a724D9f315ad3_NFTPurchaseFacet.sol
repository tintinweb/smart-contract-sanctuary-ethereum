...
// File contracts/facets/NFTPurchaseFacet.sol
...
contract NFTPurchaseFacet {
     ...
     * Note that `value` may be zero.
     */
    event NFTPurchase(address from, address collectionAddress, uint256 value, uint8 currency, uint32 tokenId, uint32 quantity);
    ...
    function buyNFT(address collectionAddress, uint256 tokenId, uint256 quantity) external payable {
        emit NFTPurchase(msg.sender, collectionAddress, msg.value, 0, uint32(tokenId), uint32(quantity));
    }
    ...
    function buyNFTWithMNFT(address collectionAddress, uint256 tokenId, uint256 quantity, uint256 mnftTokenValue) external payable {
        ...
        emit NFTPurchase(msg.sender, collectionAddress, mnftTokenValue, 1, uint32(tokenId), uint32(quantity));
    }
