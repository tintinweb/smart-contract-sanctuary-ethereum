...
contract EggShop is IEggShop, ERC1155, Ownable, Pausable, Base64, ERC2981ContractWideRoyalties {
  using EnumerableSet for EnumerableSet.UintSet;
  using Strings for uint256;
  using SafeMath for uint256;

  // Events
  event Mint(uint16 indexed typeId, address indexed owner, uint16 quantity);
  event Burn(uint16 indexed typeId, address indexed owner, uint16 quantity);
  event UpdateTypeSupplyExchange(uint16 indexed typeId, uint16 maxSupply, uint256 eggMintAmt, uint256 eggBurnAmt);
  event TypeNameUpdated(uint16 indexed typeId, string name);
  ...
  mapping(uint256 => EggShopImage) public traitRLEData;
  ...
  /**
   * @dev Modifer to check all contracts are set up correctly
   */

  modifier requireContractsSet() {
    require(address(eggToken) != address(0), 'Contracts not set');
    _;
  }
  ...
  function mint(
    ...
    if (eggAmt > 0) {
      require(eggToken.balanceOf(tx.origin) >= eggAmt, 'Not enough $EGG');
      eggToken.transferFrom(tx.origin, address(this), eggAmt);
    } else if (typeInfo[typeId].eggMintAmt > 0) {
      // NOTE: This will fail if the origin doesn't have the required amount of $EGG
      require(eggToken.balanceOf(tx.origin) >= typeInfo[typeId].eggMintAmt * quantity, 'Not enough $EGG');
    ...
    emit Mint(uint16(typeId), recipient, quantity);
  }
  ...
  function burn(
    ...
    address burnFrom,
    uint256 eggAmt
  ) external override whenNotPaused onlyController {
    ...
    emit Burn(uint16(typeId), msg.sender, quantity);
  }
  ...
  function safeTransferFrom(
    ...
    uint256 amount,
    bytes memory data
  ) public virtual override(ERC1155, IEggShop) {
  ...
  function _setSupplyExchangeAmt(
    ...
    emit UpdateTypeSupplyExchange(uint16(typeId), maxSupply, eggMintAmt * 10**18, eggBurnAmt * 10**18);
  }
  ...
  function _uploadRLEImage(uint256 typeId, EggShopImage calldata image) internal {
    ...
    emit TypeNameUpdated(uint16(typeId), image.name);
  }
  ...
  /**
   * @notice Enables owner to pause / unpause eggshop minting
   */

  function setPaused(bool _paused) external onlyController requireContractsSet {
    if (_paused) _pause();
    else _unpause();
  }
