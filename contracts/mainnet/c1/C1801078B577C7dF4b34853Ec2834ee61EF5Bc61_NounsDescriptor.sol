...
contract NounsDescriptor is INounsDescriptor, Ownable {
    ...
    /**
     * @notice Given a token ID and seed, construct a token URI for an official BeachBum.
    ...
    /**
     * @notice Given a token ID and seed, construct a base64 encoded data URI for an official BeachBum.
     */
    function dataURI(uint256 tokenId, INounsSeeder.Seed memory seed) public view override returns (string memory) {
        string memory nounId = tokenId.toString();
        string memory name = string(abi.encodePacked('BeachBum ', nounId));
        string memory description = string(
            abi.encodePacked('BeachBum ', nounId, ' is an attendee or supporter of Hodlercon')
        );
