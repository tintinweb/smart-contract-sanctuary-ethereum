...
contract Buyback is BalanceManagement {
    using SafeERC20 for IERC20;

    address public immutable wNative;
    address public immutable buybackToken;
    address public routerToNative;
    address public routerFromNative;
    uint256 public swapTolerance; // Value in milli-percent
    ...
    constructor(
        ...
        address _routerToNative, // router that uses to swap tokens to native coins
        address _routerFromNative, // router that uses to swap tokens from native coins
        uint256 _swapTolerance, // Value in milli-percent
        address _owner,
        address[] memory _managers,
        bool _addOwnerToManagers
    ) {
        buybackToken = _buybackToken;
        routerToNative = _routerToNative;
        routerFromNative = _routerFromNative;
    ...
    /**
     * @notice Sets the router that will swap USDT/USDC to wrapped native coin
     * @param _swapRouter is the new router
     */
    function setRouterToNative(address _swapRouter) external onlyManager {
        require(_swapRouter != address(0), 'Buyback: provided router can not be zero address');
        routerToNative = _swapRouter;
    }

    /**
     * @notice Sets the router that will swap wrapped native coin to ITP
     * @param _swapRouter is the new router
     */
    function setRouterFromNative(address _swapRouter) external onlyManager {
        require(_swapRouter != address(0), 'Buyback: provided router can not be zero address');
        routerFromNative = _swapRouter;
    }

    /**
     * @notice Sets the swap tolerance value
     * @dev The value is in milli-percent, e.g., 100 is 0.1%
     * @param _swapTolerance The swap tolerance value
     */
    function setSwapTolerance(uint256 _swapTolerance) external onlyManager {
        if (_swapTolerance > Constants.MILLIPERCENT_FACTOR) {
    ...
    /**
     * @notice Withdraw buyback token by Manager
     */
    function withdraw() external onlyManager {
        uint256 balance = IBuybackToken(buybackToken).balanceOf(address(this));

        if (balance > 0) {
            TransferHelper.safeTransfer(buybackToken, msg.sender, balance);
        }
    }

    /**
     * @dev Swap all `_tokenAddress` amount of tokens to `buybackToken`
     * @param _tokenAddress is the token that will be swapped
     * @param _desiredAmountOut is the desired amount of `buybackToken` that will be got
     */
    function buybackForToken(
        address _tokenAddress,
        uint256 _desiredAmountOut
    ) external onlyManager {
        _approveToken(_tokenAddress, routerToNative);
        _approveToken(wNative, routerFromNative);

        address[] memory path = new address[](2);
        path[0] = _tokenAddress;
        path[1] = wNative;
        // here we do not care about amounts,
        // we care only about the buybackToken desired amount
        _swapTokenToToken(path);
        path[0] = wNative;
        path[1] = buybackToken;

        uint256 currentBalance = IBuybackToken(buybackToken).balanceOf(address(this));
        _swapTokenToToken(path, _desiredAmountOut);
        uint256 newBalance = IBuybackToken(buybackToken).balanceOf(address(this)) - currentBalance;

        totalBuyback += newBalance;
    }
    ...
    function isReservedToken(address _tokenAddress) public view override returns (bool) {
        return _tokenAddress == buybackToken;
    }

    /**
     * @dev Swap all amount of provided tokens for provided path
     * @param _path is the list of tokens that will be swapped
     */
    function _swapTokenToToken(address[] memory _path) internal {
        __swapTokenToToken(routerToNative, _path, 0, false);
    }

    /**
     * @dev Swap all amount of provided tokens to the token2
     * @param _path is the list of tokens that will be swapped
     * @param _desiredAmountOutMin is the min amount of path[1] token for swapping
     */
    function _swapTokenToToken(address[] memory _path, uint256 _desiredAmountOutMin) internal {
        __swapTokenToToken(routerFromNative, _path, _desiredAmountOutMin, true);
    }

    /**
     * @dev Swap all amount of provided tokens to the token2
     * @param _router is the provided router for swapping
     * @param _path is the list of tokens that will be swapped
     * @param _desiredAmountOutMin is the min amount of path[1] token for swapping
     * @param _isFinal is the parameter that checks the final desired amount of tokens after the swap
     */
    function __swapTokenToToken(
        address _router,
        address[] memory _path,
        uint256 _desiredAmountOutMin,
        bool _isFinal
    ) internal {
        uint256 currentBalance = tokenBalance(_path[0]);
        uint256[] memory amounts = ISwapRouter(_router).getAmountsOut(currentBalance, _path);
        uint256 amountOutMin = (amounts[1] * (Constants.MILLIPERCENT_FACTOR - swapTolerance)) /
            Constants.MILLIPERCENT_FACTOR;
        // if the error occurs it means that not enough tokens on the Buyback contract for
        // provided _desiredAmountOutMin value of `native coins`
        if (_isFinal) {
            require(
                amountOutMin >= _desiredAmountOutMin,
                'Buyback: expected value is less than desired amount value'
            );
        }

        _swapExactTokensForTokens(_router, currentBalance, amountOutMin, _path);
    }

    /**
     * @dev Direct swap from token to token with collected parameters
     * @param _router is the provided router for swapping
     * @param _amountTotal is the current balance for swapping
     * @param _amountOutMin is the min amount of tokens for swapping
     * @param _path is the list of tokens that will be swapped
     */
    function _swapExactTokensForTokens(
        ...
        ISwapRouter(_router).swapExactTokensForTokens(
            _amountTotal,
            _amountOutMin,
            _path,
            address(this),
    ...
    /**
     * @dev Approve tokens for maximum value
     * @param _token is the token that will be approved
     * @param _who is the account that can spend the tokens
     */
    function _approveToken(address _token, address _who) internal {
        IERC20(_token).safeApprove(_who, 0);
        IERC20(_token).safeApprove(_who, Constants.INFINITY);
    }
}
...
/**
 * @title IBuybackToken
 * @notice Buyback token interface
 */
...
/**
 * @title ISwapRouter
 * @notice Swap router interface
 */
interface ISwapRouter {
    /**
     * @notice Getter of the output token amounts
     * @return amounts Output token amounts
     */
    ...
    /**
     * @notice Swaps the exact number of input tokens for output tokens
     * @param amountIn Amount of tokens we are sending in
     * @param amountOutMin The minimum amount of tokens we want out of the trade
     * @param path List of token addresses we are going to trade in
     * @param to The address we are going to send the output tokens to
     * @param deadline The last time that the trade is valid for
     * @return amounts Output token amounts
     */
