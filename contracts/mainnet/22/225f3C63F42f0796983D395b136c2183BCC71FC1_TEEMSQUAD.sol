...
contract TEEMSQUAD is ERC721Enumerable,  Ownable, ReentrancyGuard {
    ...
    constructor(string memory _theBaseURI, string memory _notRevealedURI) ERC721("TEEMSQUAD", "TSQD") {
    ...
    function publicMint(uint256 _ammount) external payable nonReentrant {
        ...
        require(_ammount <=5,"You can't mint more than 5 NFTs at the same time");

        require(sellingStep != Steps.SoldOut, "Sorry, no NFT left.");
        ...
        require((nftsPerWallet[msg.sender]+_ammount) <= 5, "You can only get 5 NFTs on Public Sale");
        //Did the user then enought Ethers to buy amount NFTs ?
        require(msg.value == priceSale * _ammount, "Please send the correct amount.");
        //If the user try to mint any non-existent token
        require(numberNftSold + _ammount <= MAX_SUPPLY, "Sale is almost done and we don't have enough NFTs left.");
        //Add the amount of NFTs minted by the user to the total he minted
