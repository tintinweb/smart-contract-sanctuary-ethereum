...
contract ThreadsOfficial is Ownable, ERC721A, ReentrancyGuard, ERC721ABurnable, DefaultOperatorFilterer{
...
constructor() ERC721A("Threads Official", "THREADS") {}
        ...
        nonReentrant
    {
        ...
        require(isPublicMintEnabled, "Threads not ready yet!");
        require(totalSupply() + quantity <= COLLECTION_SIZE, "No more Threads!");

        require(balanceOf(msg.sender) + quantity <= SUPPLY_PER_WALLET, "Tried to total mint Threads per wallet over limit");

        require(quantity <= MAX_BATCH_SIZE, "Tried to mint Threads over limit, retry with reduced quantity");
