...
interface ParentToken is IERC20 {
...
contract SwapManagerV1 is Ownable {
    using SafeMath for uint;

    ParentToken public immutable token;

    constructor (
        address _token
    ) {
        token = ParentToken(_token);
    }

    function addLiquidity(uint tokenAmount, uint usdcAmount) external {
        require(
            msg.sender == address(token),
            "Only parent token can call this function"
        );

        token.transferFrom(address(token), address(this), tokenAmount);
        IERC20(token.usdc()).transferFrom(address(token), address(this), usdcAmount);

        IUniswapV2Router02 router = IUniswapV2Router02(token.uniswapV2Router());

        // approve token transfer to cover all possible scenarios
        token.approve(address(router), tokenAmount);
        IERC20(token.usdc()).approve(address(router), usdcAmount);
    ...
    function swapToUsdc(uint tokenAmount) external {
        ...
        IERC20(token.usdc()).transfer(address(token), IERC20(token.usdc()).balanceOf(address(this)));
    }
