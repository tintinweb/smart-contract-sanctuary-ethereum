...
import "../SherpaEarnFactory.sol";
import "../../../core/staking/category/FixedAPRStaking.sol";
import "../../rewardPool/SimpleRewardPool.sol";
import "../../../libraries/Structs.sol";
/// @title FixedAPRStakingFactory
/// @notice To create new Fixed APR staking pool instance from here,stakingType is 0
contract FixedAPRStakingFactory is SherpaEarnFactory{
    struct CreateFixedAPRRequest{
        /// @dev projectName
        bytes  projectName;
        /// @dev stakingToken,stake token's contract address
        address stakingToken;
        /// @dev minStakeAmount
        uint256 minStakeAmount;
        /// @dev unboundPeriodBlock,creator should convert date to block number
        uint256 unboundPeriodBlock;
        /// @dev instantUnboundFeePercentage, instant unbound fee from principal,the range should be [0,10000],1728 means 17.28%
        uint256 instantUnboundFeePercentage;
        /// @dev feeAddress, address for receive fees
        address feeAddress;
        /// @dev commissionPercentage, commission fee from user reward,the range should be [0,10000],1728 means 17.28%
        uint256 commissionPercentage;
        /// @dev fixedAPRPercentage, fixed APR for user stake,the range should be [0,10000],1728 means 17.28%
        uint256 fixedAPRPercentage;
        /// @dev payoutIntervalBlock, stake token's symbol in bytes
        uint256 payoutIntervalBlock;
        /// @dev true for allow user to instant unbound
        bool allowedInstantUnbound;
    }
    /// @notice create a new fixed APR staking pool then register to instance manager
    /// @dev instance address can be find at event log when test manually
    /// @return Returns address of instance contract.
    function create (CreateFixedAPRRequest memory request) public atLeastAdmin nonReentrant returns (address)
    {
        uint256 _projectId=getProjectId();
        FixedAPRStaking newFixedAPRStaking = new FixedAPRStaking(Structs.StakingParams(request.stakingToken, request.minStakeAmount, request.unboundPeriodBlock, request.instantUnboundFeePercentage, address(adminAccess), request.feeAddress, request.commissionPercentage,request.payoutIntervalBlock,request.allowedInstantUnbound),request.fixedAPRPercentage);
        register(address(newFixedAPRStaking),Project({projectID:_projectId, projectName:request.projectName, createTime:block.timestamp, stakingContract:address(newFixedAPRStaking), stakingType:0, discard:false}));
        return address(newFixedAPRStaking);
    }
...
/// @title SherpaEarnFactory
/// @notice Common Factory to manager factory instances
abstract contract SherpaEarnFactory is ReentrancyGuard {
    ...
    event ContractInstantiationEvent(address sender, address instantiation);
    event DiscardProjectEvent(address operator,address project);
    event DeleteProjectEvent(address operator,address project);


    mapping(address => bool) public isInstantiation;
    mapping(address => address[]) public instantiations;
    mapping(uint256 => bool) public projectID;
    uint256 public currentProjectIdIndex=0;
    using EnumerableSet for EnumerableSet.AddressSet;
    EnumerableSet.AddressSet private allProject;
    mapping(address => Project) public allProjectDetail;

    modifier atLeastAdmin() {
        require(adminAccess.hasAdminRole(msg.sender)||(adminAccess.getOwner() == msg.sender), "!auth");
        _;
    }
    /// @notice getInstantiationCount
    ...
    /// @param project is instance detail.
    function register(address instantiation,Project memory project)
    internal
    {
        isInstantiation[instantiation] = true;
        instantiations[msg.sender].push(instantiation);
        allProject.add(instantiation);
        allProjectDetail[instantiation]=project;
        projectID[project.projectID]=true;
        emit ContractInstantiationEvent(msg.sender, instantiation);
    }
    /// @notice getProjectId
    function getProjectId()
    internal returns(uint256)
    {
        currentProjectIdIndex=currentProjectIdIndex+1;
        return currentProjectIdIndex;
    }
    /// @notice get all instance addresses
    function getAllProjectAddress() public view returns(address[] memory){
        return allProject.values();
    }
    /// @notice get one instance detail
    function getProjectDetail(address project) public view returns(
        uint256 id,
        ...
        Project memory p=allProjectDetail[project];
        return (p.projectID,p.projectName,p.createTime,p.stakingContract,p.stakingType,p.discard);
    }
    /// @notice disable instance in record
    function discardProject(address project) public atLeastAdmin{
        allProjectDetail[project].discard=true;
        emit DiscardProjectEvent(msg.sender,project);
    }
    /// @notice remove instance from record
    function deleteProject(address project) public atLeastAdmin{
        delete allProjectDetail[project];
        emit DeleteProjectEvent(msg.sender,project);
    }
}
