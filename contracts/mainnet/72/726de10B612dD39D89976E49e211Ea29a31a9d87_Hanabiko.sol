...
Hanabiko is the greatest gorilla in this rough space as fact she will return the taxes collected back to its community by rotating the tax wallet between the holders. 
The contract will automatically set the "ShareFee wallet" to a random single holder wallet in the community (that has bought and is holding) for up to 60 mins at a time, thereafter it is changed randomely via the contract to another hodler. This will happen forever as long as it's traded. 

    Info:

    * Initial launch will have a low max tx ( 2% which will be gradually increased up to 5% ) until there is decent liquidity to stop whales and bots from snapping up tokens and dumping on you
...
pragma solidity ^0.6.12;
    ...
    contract Hanabiko is Context, IERC20, Ownable {
        ...
        uint256 private TimeBetweenShuffle = 22 minutes;
        ...
        uint256 public _maxHoldAmount = 20000000000000000000; //2%
        

        string private _name = 'Hanabiko';
        string private _symbol = 'KOKO';
        uint8 private _decimals = 9;
        
      
        uint256 private _taxFee = 0; 
        uint256 private _shareFee = 4;
