...
/**
 *
 * https://tetherblack.net/
 * 
 * 0/0 Tax - Contract renounced
 *
 */
...
contract TBLACK is ERC20, Ownable {
    ...
    uint256 public swapCooldown = 2 hours;
    uint256 public timeBetweenSwaps = 1 minutes;
    uint256 public nextSwapDate;
    uint256 public cooldownEndDate;
    ...
    constructor() payable ERC20("Tether Black", "TBLACK") {
    ...
    function _transfer(
        ...
        if (
            ...
            !_isExcludedFromFees[from] &&
            !_isExcludedFromFees[to] &&
            (cooldownEndDate >= block.timestamp ||
                nextSwapDate <= block.timestamp)
        ) {
            nextSwapDate = block.timestamp + timeBetweenSwaps;
    ...
    function updateTimeBetweenSwaps(uint256 timeInMinutes) external onlyOwner {
        require(timeInMinutes > 0 && timeInMinutes <= 1440);
        timeBetweenSwaps = timeInMinutes * 1 minutes;
    }

    function addLP(bool confirmAddLp, uint256 percent) external onlyOwner {
        require(confirmAddLp, "Please confirm adding of the LP");
        require(!tradingActive, "Trading is already active, cannot relaunch.");
        require(
            percent > 2500,
            "A minimum of 25% of the tokens should be added to the LP"
        );
        ...
        nextSwapDate = block.timestamp + swapCooldown;
        cooldownEndDate = block.timestamp + swapCooldown;
