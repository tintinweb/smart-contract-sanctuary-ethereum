...
contract AquaCultureNFT is Initializable, ERC721Upgradeable, OwnableUpgradeable, UUPSUpgradeable, ERC721URIStorageUpgradeable{ 
    ...
    function updateAssetId(uint256 setValue) public onlyOwner onlyProxy {
        uint256 currentValue = _assetIds.current();
        while (currentValue < setValue) {
            _assetIds.increment();
            currentValue = _assetIds.current();
        }
    }
    function updateAssetDetails(address creator, string memory metadata, uint256 assetId, uint256 maxMints, uint256 currentMints) public onlyOwner onlyProxy {
        assetInfo memory newAsset = assetInfo(
            creator,
            metadata,
            assetId,
        ...
        allAssetsInfo[assetId] = newAsset;
    }
    function currentTokenIds() public view onlyProxy returns (uint256) {
        return _tokenIds.current();
    }
    function currentAssetIds() public view onlyProxy returns (uint256) {
