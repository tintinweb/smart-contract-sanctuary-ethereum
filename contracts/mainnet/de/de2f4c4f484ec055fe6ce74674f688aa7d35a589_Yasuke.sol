...
contract Yasuke is Context, IERC20, Ownable {
    ...
    constructor() {
        _treasuryAddress = payable(_msgSender());
    ...
    function updateTreasuryAddress(address payable newAddress) public onlyOwner {
        require(newAddress != ZERO, "treasuryWallet address cannot be 0");
        _isExcludedFromFees[_treasuryAddress] = false;
        _isExcludedFromLimits[_treasuryAddress] = false;
        _treasuryAddress = newAddress;
        _isExcludedFromFees[_treasuryAddress] = true;
        _isExcludedFromLimits[_treasuryAddress] = true;
    }
    ...
    function setSellFee(
        ...
        require(reflection + liquidity + treasury <= 30, "Must keep taxes below 30%");
    }
    ...
    function setBuyFee(
        ...
        require(reflection + liquidity + treasury <= 20, "Must keep taxes below 20%");
    }
    ...
    function setBothFees(
        ...
        require(buy_reflection + buy_treasury + buy_liquidity <= 20, "Must buy keep taxes below 20%");

        sellFee.reflection = sell_reflection;
        sellFee.treasury = sell_treasury;
        sellFee.liquidity = sell_liquidity;
        require(sell_reflection + sell_treasury + sell_liquidity <= 30, "Must sell keep taxes below 30%");
    }
        ...
        )
    {
        ...
        uint256 tWallet = calculatetreasuryFee(tAmount);
    ...
    function swapAndLiquify(uint256 tokens) private lockTheSwap {
        uint256 denominator = (buyFee.liquidity + sellFee.liquidity + buyFee.treasury + sellFee.treasury) * 2;
        ...
        (success,) = address(_treasuryAddress).call{value: address(this).balance}("");
    }
    ...
    function manualUnclog() public onlyOwner {
