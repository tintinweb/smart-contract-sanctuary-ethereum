...
contract VoteBonus is Ownable{
    using ECDSA for bytes32;

    event ClaimBonus(uint256 globalVoteId, address voter, address token, uint256 amount);
    ...
    function claimBonus(uint256 _globalVoteId, address _token, uint256 _amount, bytes memory _rsv) external {

        require(_amount > 0, "amount invalid");

        bytes32 hash = keccak256(abi.encode("claimBonus", address(this), msg.sender, _globalVoteId, _token, _amount));
        checkSigner(hash, _rsv, signer);

        require(IERC20(_token).balanceOf(address(this)) >= _amount, "token insufficient");

        IERC20(_token).transfer(msg.sender, _amount);

        emit ClaimBonus(_globalVoteId, msg.sender, _token, _amount);
    }
