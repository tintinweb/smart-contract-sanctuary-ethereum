...
// File: contracts/LowGasSolidity_V6.2.sol
...
/**
    ...
    by minting, transferring, reading data, etc. you indemnify the creators of this contract from
    any liability. The creators of this contract offer no warranty as to the functionality therein, 
    and take no responsibility for any loss or damaged incurred by a user of this contract.
*/
...
contract SimpleNftLowerGas is ERC721, Ownable, ERC2981 {
  using Strings for uint256;
  ...
  mapping(address => uint) amountNFTperWallet;

  bool public paused = true;

  address[] private whitelistedAddresses;

  constructor(address _royaltyReceiver, uint16 _royaltyPoints) 
    ERC721("Vain Ego", "EGO")
    ERC2981(_royaltyReceiver, _royaltyPoints) {
    setUriSuffix(".json");
    //supply.increment();
  }
  ...
  function mint(uint256 _mintAmount) public payable mintCompliance(_mintAmount) {
    ...
    require(amountNFTperWallet[msg.sender] + _mintAmount <= maxMintAmountPerAddress, "Max mint per address exceeded!");

    amountNFTperWallet[msg.sender] += _mintAmount;
