...
contract LuckyTickat is ERC721Enumerable, Ownable {
  ...
  function addWl(address _address) external onlyOwner {
      require(_address != address(0), "Invalid address");
      wl.push(_address);
  }
  function addManyWl(address[] memory _address) external onlyOwner {
      uint i;
  ...
  function getWhitelist(address _address) public view returns (bool) {
      uint256 i;
      uint256 length = wl.length;
      for(i = 0; i < length; i++) {
          address _addressArr = wl[i];
  ...
  function mint(address _to, uint256 _mintAmount) public payable {
    ...
    if (msg.sender != owner()) {
        if(getWhitelist(msg.sender) != true) {
