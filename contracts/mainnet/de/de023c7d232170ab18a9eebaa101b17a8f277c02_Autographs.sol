...
contract Autographs is ERC721("Autographs", "ATG"), Ownable{
    ...
    mapping (bytes32 => string) public autographSvgs;
    ...
    constructor(){
        baseTokenURI = "http://autographsorsomthing.com";
        //changing to https://vitaliksautograph.com/baseTokenURI;
    }

    function mint(string memory name, string memory svg) public onlyOwner{
        bytes32 svgHash = keccak256(abi.encodePacked(svg));
        require(!exists[svgHash], "identical svg already exists");
        _safeMint(owner(),  _tokenIdTracker.current());
        names[svgHash] = name;
        tokenSvgHash[_tokenIdTracker.current()] = svgHash;
        autographSvgs[svgHash] = svg;
