...
contract PrePayTechnoAndChill is Ownable, ERC721A, PaymentSplitter {
    ...
    mapping(address => uint) public technoLoversInvitedBy;
    ...
    *
    * @param _invitedBy Invited by an Ambassador

    **/
    function prepayInvitedBy(uint _quantity, address _invitedBy) external payable callerIsUser {
        ...
        bool invitedByIsAmbassador = false;
        for (uint256 index = 0; index < ambassadors.length; index++) {
            
            if(ambassadors[index] == _invitedBy){

                invitedByIsAmbassador = true;
            }

        }

        if(prepayPerWallet[_invitedBy] > 0 || invitedByIsAmbassador == true){
            technoLoversInvitedBy[_invitedBy] += _quantity;
            

            if(invitedByIsAmbassador == false){
                    ambassadors.push(_invitedBy);
            }
    ...
    */
    ...
    /**
    * @notice Get Ambassadors Wallets
    *
    */
