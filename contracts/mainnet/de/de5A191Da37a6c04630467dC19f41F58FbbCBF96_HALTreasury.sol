...
contract HALTreasury is Ownable {
    ...
    enum MANAGING { RESERVEDEPOSITOR, RESERVESPENDER, RESERVETOKEN, RESERVEMANAGER, LIQUIDITYDEPOSITOR, LIQUIDITYTOKEN, LIQUIDITYMANAGER, DEBTOR, REWARDMANAGER, SHAL }
    ...
    address public sHAL;
    uint public sHALQueue; // Delays change to sHAL address
    ...
    /**
        @notice allow approved address to deposit an asset for HAL
        @param _amount uint
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        // mint HAL needed and store amount of rewards for distribution
        send_ = value.sub( _profit );
        IERC20Mintable( HAL ).mint( msg.sender, send_ );
    ...
    /**
        @notice allow approved address to burn HAL for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOf( _token, _amount );
        IHALERC20( HAL ).burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( sHAL ).balanceOf( msg.sender ); // Can only borrow against sHAL held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with HAL
        @param _amount uint
     */
    function repayDebtWithHAL( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved" );

        IHALERC20( HAL ).burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, HAL, _amount, _amount );
    }
    ...
    function mintRewards( address _recipient, uint _amount ) external {
        ...
        IERC20Mintable( HAL ).mint( _recipient, _amount );
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( IERC20( HAL ).totalSupply().sub( totalDebt ) );
    }
    ...
    /**
        @notice returns HAL valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match HAL decimals
            value_ = _amount.mul( 10 ** IERC20( HAL ).decimals() ).div( 10 ** IERC20( _token ).decimals() );
    ...
    function queue( MANAGING _managing, address _address ) external onlyManager() returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.SHAL ) { // 9
            sHALQueue = block.number.add( blocksNeededForQueue );
        } else return false;
