...
contract Bohemians is ERC721, ThreeChiefOfficersWithRoyalties {
    ...
    function prepareDrop(
        ...
        require(quantityForSale > 0, "quantity may not be zero");
        require(quantityForSale <= MAX_DROP_SIZE, "drop is too large");
        require(drops[dropID].quantityForSale == 0, "This drop was already prepared");
        require(bytes(tokenURIBase).length > 0, "missing URI base");
    ...
    function abortDrop(uint64 dropID) external onlyOperatingOfficer {
        require(drops[dropID].quantitySold == 0, "this drop has already started selling");
        delete (drops[dropID]);
    }
    ...
    function setDropPhases(uint64 dropID, DropPhase[] calldata dropPhases) external onlyOperatingOfficer {
        Drop storage drop = drops[dropID];
        require(drop.quantityForSale > 0, "this drop has not been prepared");
        delete drop.phases;
    ...
    function setTokenURIBase(uint64 dropID, string calldata tokenURIBase) external onlyOperatingOfficer {
        Drop storage drop = drops[dropID];
        require(drop.quantityForSale > 0, "this drop has not been prepared");
        require(drop.quantityFrozenMetadata == 0, "metadata freezing already started");
        drop.tokenURIBase = tokenURIBase;
    }
    ...
    function mintRandom(
        ...
        Drop storage drop = drops[dropID];
        require(quantity > 0, "missing purchase quantity");
        require(quantity + drop.quantitySold <= drop.quantityForSale, "not enough left for sale");
        require(drop.accumulatedRandomness != 0, "no randomness in this drop, use mintChosen instead");

        DropPhase memory dropPhase = _getEffectivePhase(drop);
        require(msg.value >= dropPhase.ethPrice * quantity, "not enough Ether paid");
    ...
    function mintChosen(
        ...
        Drop storage drop = drops[dropID];
        require(tokenIDsInDrop.length > 0, "missing tokens to purchase");
        require(tokenIDsInDrop.length + drop.quantitySold <= drop.quantityForSale, "not enough left for sale");
        require(tokenIDsInDrop.length < type(uint64).max);
        require(drop.accumulatedRandomness == 0, "this drop uses randomness, use mintRandom instead");

        DropPhase memory dropPhase = _getEffectivePhase(drop);
        require(msg.value >= dropPhase.ethPrice * tokenIDsInDrop.length, "not enough Ether paid");
        ...
        for (uint256 index = 0; index < tokenIDsInDrop.length; index++) {
            require(tokenIDsInDrop[index] < drop.quantityForSale, "invalid token ID");
    ...
    function finalizeRandomDrop(uint64 dropID, string calldata password) external onlyOperatingOfficer {
        Drop storage drop = drops[dropID];
        require(drop.passwordHash != bytes32(0), "this drop does not have a password (anymore)");
        require(drop.quantitySold == drop.quantityForSale, "this drop has not completed selling");
        require(keccak256(abi.encode(password)) == drop.passwordHash, "wrong secret");
    ...
    function freezeMetadataForDrop(uint64 dropID, uint256 quantityToFreeze) external {
        Drop storage drop = drops[dropID];
        require(drop.quantitySold == drop.quantityForSale, "this drop has not sold out yet");
        require(drop.passwordHash == bytes32(0), "this random drop has not been finalized yet");
        require(drop.quantityFrozenMetadata < drop.quantityForSale, "all metadata is already frozen");
    ...
    function _getEffectivePhase(Drop storage drop) internal view returns (DropPhase memory) {
        require(drop.phases.length > 0, "no drop phases are set");
        require(drop.phases[0].startTime != 0, "first drop phase has no start time");
        require(drop.phases[0].startTime <= block.timestamp, "first drop phase has not started yet");
    ...
    function _requireValidMerkleProof(
        ...
        require(MerkleProof.verify(accessListProof, accessListRoot, merkleLeaf), "invalid access list proof");
    }
