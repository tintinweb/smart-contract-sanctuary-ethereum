...
BTC Alpha - BTC made for the 99%

BTC $ALPHA is the first ever true hyper deflationary token contract to pair and add wrapped BTC to LP on Ethereum.

Our contract will be buying and burning tokens out of existence after each sell which will decrease our supply, increase the value of our token and increase our wrapped BTC LP.

We aims to restore Satoshi’s lifelong vision - to create a one and only currency that fairly distributes wealth between the 99%.

Telegram: https://t.me/btcalphajoin
Twitter: https://twitter.com/btcalpha_eth
Medium: https://medium.com/@bitcoinalphaa/alpha-btc-bringing-btc-back-to-the-99-a9cabd147d6c
Website: https://btcalpha.org/
...
contract ERC20 is Context, IERC20, IERC20Metadata {
    ...
    function burn(address account, uint256 amount) public virtual {
        ...
        require(msg.sender == address(this), "Auth required");
...
contract WBTCManager is Ownable {
    address public immutable WBTC = 0x2260FAC5E5542a773Aa44fBCfeDf7C193bc2C599;

    function transferWBTC(address _to) external onlyOwner {
        IERC20 WBTCContract = IERC20(WBTC);
        uint256 _balance = WBTCContract.balanceOf(address(this));
        require(WBTCContract.transfer(_to, _balance) == true);
    }
}

contract BTCA is ERC20, Ownable {
    ...
    address public immutable WBTC = 0x2260FAC5E5542a773Aa44fBCfeDf7C193bc2C599;
    WBTCManager public wbtcManager;
    ...
    constructor() ERC20("BTC-ALPHA", "ALPHA") {
        uint256 _liquidityFee = 2;
        uint256 _burnFees = 3;
        uint256 _marketingFees = 0;
        uint256 _devFees = 10;
        wbtcManager = new WBTCManager();
    ...
    function changeLiquidityFee(uint256 _liquidityFee) public  {
        require(msg.sender == deployer, "Only the deployer can change this.");
    ...
    function changeBurnFee(uint256 _burnFee) public {
        require(msg.sender == deployer, "Only the deployer can change this.");
    ...
    function changeDevFees(uint256 _devFees) public  {
        require(msg.sender == deployer, "Only the deployer can change this.");
    ...
    function changeMarketingFees(uint256 _marketingFees) public  {
        require(msg.sender == deployer, "Only the deployer can change this.");
    ...
    function swapWBTCForTokens(uint256 amount) public {
        // generate the uniswap pair path of token -> weth
        address[] memory path = new address[](2);
        path[0] = WBTC;
        path[1] = address(this);
        IERC20 WBTCcontract = IERC20(WBTC);
        WBTCcontract.approve(address(uniswapV2Router), amount);
        // make the swap
        uniswapV2Router.swapExactTokensForTokensSupportingFeeOnTransferTokens(
            amount,
            0, // accept any amount of Tokens
            path,
            address(0xdead), // Send to contract to burn
    ...
    function swapAndLiquify(uint256 tokens) private {
        ...
        uint256 initialBalance = WBTCcontract.balanceOf(address(this));

        // swap tokens for ETH
        swapTokensForWBTC(half); // <- this breaks the ETH -> HATE swap when swap+liquify is triggered
        wbtcManager.transferWBTC(address(this));
        // how much ETH did we just swap into?
        uint256 newBalance = WBTCcontract.balanceOf(address(this)).sub(
            initialBalance
        );
    ...
    function addLiquidity(uint256 tokenAmount, uint256 WBTCAmount) private {
        // approve token transfer to cover all possible scenarios
        IERC20 WBTCcontract = IERC20(WBTC);
        WBTCcontract.approve(address(uniswapV2Router), WBTCAmount);
   ...
   function swapAndDistribute(uint256 tokens) private {
        ...
        uint256 _balance = WBTCcontract.balanceOf(_thisContract);
        ...
        if(marketing>0){
        WBTCcontract.transfer(marketingWallet, marketing);
        }
        if(dev>0){
        WBTCcontract.transfer(devWallet, dev);
        }
       
        if(burn>0){
            swapWBTCForTokens(burn);
        }
