...
contract KatsumiStaking6 is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    // The block number when KISHI reward ends.
    uint256 public bonusEndBlock;

    // The block number when KISHI reward starts.
    uint256 public startBlock;

    // The block number of the last pool update
    uint256 public lastRewardBlock;

    // KATSUMI tokens reward per block.
    ...
    function pendingReward(address _user) external view returns (uint256) {
        ...
        if (block.number > lastRewardBlock && stakedTokenSupply != 0) {
            ...
            uint256 kishiReward = multiplier.mul(rewardPerBlock);
            uint256 adjustedTokenPerShare =
                accTokenPerShare.add(kishiReward.mul(PRECISION_FACTOR).div(stakedTokenSupply));
    ...
    function _updatePool() internal {
        ...
        uint256 multiplier = _getMultiplier(lastRewardBlock, block.number);
        uint256 kishiReward = multiplier.mul(rewardPerBlock);
        accTokenPerShare = accTokenPerShare.add(kishiReward.mul(PRECISION_FACTOR).div(stakedTokenSupply));
        lastRewardBlock = block.number;
    }
