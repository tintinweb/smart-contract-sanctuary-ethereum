...
contract EnigmaV5 is Ownable, ERC721A {
    // Interface imports
    ...
    uint256 public startTime = 1677970500;
    ...
    // constructor
    constructor() ERC721A("EnigmaV5", "EnigmaV5") {
     ...
     * tokenType = 0 (Ethereum), 1 (USDC) or 2 (USDT)
     */
        ...
        noContracts
    {
        ...
        if (_tokenType == TokenType.ETH) {
            ...
            token.transferFrom(msg.sender, address(this), _mintPrice*(10**18));
            _mint(msg.sender, _mints);
            presaleMintedCounter += _mints;
        }
    }
     ...
     * Treasury mint
     */
    function treasuryMint(uint256 _quantity, address _address) external onlyOwner {
        require(
    ...
    function setPresaleReserveCounter(uint256 _presaleReserveCounter) external onlyOwner {
        presaleReserveCounter = _presaleReserveCounter;
    }

    function setPresaleRedeemCounter(uint256 _presaleReedemCounter) external onlyOwner {
        presaleRedeemCounter = _presaleReedemCounter;
    }

    function setPresaleMintedCounter(uint256 _presaleMintedCounter) external onlyOwner {
        presaleMintedCounter = _presaleMintedCounter;
    }

    function setPublicMintedCounter(uint256 _publicMintedCounter) external onlyOwner {
