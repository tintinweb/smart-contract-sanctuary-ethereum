...
interface ILendingPool {
  ...
  /// @param receivemToken              `true` if the liquidators wants to receive the collateral mTokens, `false` if
  ...
  /// @param onBehalfOf   The address that will receive the mTokens, same as msg.sender if the user wants to receive
  ...
  /// @param user            The address of the borrower getting liquidated.
  /// @param debtToCover     The debt amount of borrowed `asset` the liquidator wants to cover.
  /// @param receivemToken   `true` if the liquidators wants to receive the collateral mTokens, `false` if he wants to
...
interface IRefineryV1Events {
    ...
    /// @notice Emitted when a the admin sweeps all of one reward token from the Refinery
    ///
...
interface IRefineryV1Immutables {
    /// @notice Returns the version of the refinery.
    ///
    /// @return The version.
...
interface IRefineryV1AdminActions {
    ...
    /// @notice Sweep all of 'rewardtoken' from the refinery into the admin.
    ///
    /// @notice `msg.sender` must be the admin or this call will revert with an {Unauthorized} error.
    ...
    /// @notice `msg.sender` must be the admin or this call will revert with an {Unauthorized} error.
    ///
    /// @param transferAdapterAddress The address of the V1 transfer adapter to be set in the refinery.
...
interface IRefineryV1Actions {
    ...
    /// @notice IERC20(ydai).approve(refineryAddress, amount);
    /// @notice RefineryV1(refineryAddress).deposit(ydai, amount, msg.sender);
    /// @notice ```
    ///
    ...
    /// @notice address ydai = 0xdA816459F1AB5631232FE5e97a05BBBb94970c95;
    /// @notice uint256 amount = 50000;
    /// @notice RefineryV1(refineryAddress).depositUnderlying(ydai, amount, msg.sender, 1);
    /// @notice ```
    ///
    ...
    /// @notice ```
    /// @notice address ydai = 0xdA816459F1AB5631232FE5e97a05BBBb94970c95;
    /// @notice uint256 pps = RefineryV1(refineryAddress).getYieldTokensPerShare(ydai);
    /// @notice uint256 amtYieldTokens = 5000;
    /// @notice RefineryV1(refineryAddress).withdraw(ydai, amtYieldTokens / pps, msg.sender);
    /// @notice ```
    ///
    ...
    /// @notice uint256 pps = RefineryV1(refineryAddress).getYieldTokensPerShare(ydai);
    /// @notice uint256 amtYieldTokens = 5000;
    /// @notice RefineryV1(refineryAddress).withdrawFrom(msg.sender, ydai, amtYieldTokens / pps, msg.sender);
    /// @notice ```
    ///
    ...
    /// @notice ```
    /// @notice address ydai = 0xdA816459F1AB5631232FE5e97a05BBBb94970c95;
    /// @notice uint256 pps = RefineryV1(refineryAddress).getUnderlyingTokensPerShare(ydai);
    /// @notice uint256 amountUnderlyingTokens = 5000;
    /// @notice RefineryV1(refineryAddress).withdrawUnderlying(ydai, amountUnderlyingTokens / pps, msg.sender, 1);
    /// @notice ```
    ///
    /// @param yieldToken       The address of the yield token to withdraw.
    /// @param shares           The number of shares to burn.
    ...
    /// @notice RefineryV1(refineryAddress).withdrawUnderlying(msg.sender, ydai, amtUnderlyingTokens / pps, msg.sender, 1);
    /// @notice ```
    ///
    /// @param owner            The address of the account owner to withdraw from.
    /// @param yieldToken       The address of the yield token to withdraw.
    ...
    /// @notice RefineryV1(refineryAddress).mint(amtDebt, msg.sender);
    /// @notice ```
    ///
    /// @param amount    The amount of tokens to mint.
    /// @param recipient The address of the recipient.
    ...
    /// @notice ```
    /// @notice uint256 amtDebt = 5000;
    /// @notice RefineryV1(refineryAddress).mintFrom(msg.sender, amtDebt, msg.sender);
    /// @notice ```
    ///
    ...
    /// @notice RefineryV1(refineryAddress).burn(amtBurn, msg.sender);
    /// @notice ```
    ///
    /// @param amount    The amount of tokens to burn.
    /// @param recipient The address of the recipient.
    ...
    /// @notice RefineryV1(refineryAddress).repay(dai, amtRepay, msg.sender);
    /// @notice ```
    ///
    ...
    /// @notice RefineryV1(refineryAddress).liquidate(ydai, amtSharesLiquidate, 1);
    /// @notice ```
    ///
    ...
    /// @notice RefineryV1(refineryAddress).liquidate(dai, amtSharesLiquidate, 1);
    /// @notice ```
    ///
...
interface IStaticmToken is IERC20 {
  ...
  /// @dev Converts a static amount (scaled balance on mToken) to the mToken/underlying value, using the current
  ...
  /// @dev Converts an mToken or underlying amount to the what it is denominated on the mToken as scaled balance,
  ...
  /// @dev Returns the Aave liquidity index of the underlying mToken, denominated rate here as it can be considered as
...
interface IMTokenGatway {
    /// @dev Returns the address of the whitelist used by the IMTokenGatway
    ///
    /// @return The address of the whitelist.
    function whitelist() external returns (address);

    /// @dev Returns the address of the refinery used by the IMTokenGatway
    ///
    /// @return The address of the refinery.
    function refinery() external returns (address);

    /// @dev Wraps mTokens in a StaticmToken wrapper a deposits the resulting tokens into the Refinery.
    ///
    /// @param yieldToken       The address of the static mToken wrapper.
    /// @param amount           The amount of mTokens to wrap.
    /// @param recipient        The account in the `refinery` that will recieve the resulting static mTokens. 
    /// @return sharesIssued    The amount of shares issued in the `refinery` to the account owned by `recipient`.
    function deposit(address yieldToken, uint256 amount, address recipient) external returns (uint256 sharesIssued);

    /// @dev Withdraws StaticmTokens from the Refinery and unwraps them into mTokens.
    ///
    /// @param yieldToken       The address of the static mToken wrapper.
    /// @param shares           The amount of shares to withdraw from the `refinery`.
    /// @param recipient        The account that will receive the resulting mTokens. 
    /// @return amountWithdrawn The amount of mTokens withdrawn to `recipient`.
...
contract MTokenGatway is IMTokenGatway, Ownable {
    ...
    /// @notice The address of the refinery.
