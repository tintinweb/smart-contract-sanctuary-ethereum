/*
            _     _                       _         
           | |   (_)                     | |        
  ___  _ __| |__  _  __ _ _ __ __ _ _ __ | |__  ___ 
...
0.01 Public mint.
0.008 Holder mint.
Max 5 per wallet for publicMint.
Max # of Lifeforms holdings for holderMint.
838 Supply.
*/
...
contract Orbigraphs is Ownable, ERC721A, DefaultOperatorFilterer  {
    ...
    uint256 public holderSaleCost = 0.008 ether;

    uint256 public holderQ;
    uint64 public maxSupply = 838;
    uint64 public publicMaxSupply = 838;
    ...
    mapping(address => uint256) public holderMintAmount;


    constructor() ERC721A("Orbigraphs by Guggenhiem", "ORBIS"){}
    ...
    ///Allows holders to mint at discount when the public sale is open
    function holderMint(uint64 _mintAmount) public payable mintCompliance(_mintAmount) {
        ...
        require(holderMintAmount[msg.sender] + _mintAmount <= holderQ, "HOLDERMINT_MAXED");
        require(publicTotalSupply + _mintAmount <= publicMaxSupply, "Mint limit exceeded." );
        require(msg.value >= holderSaleCost * _mintAmount, "Insufficient funds!");

        unchecked {
            holderMintAmount[msg.sender] += _mintAmount;
        }
    ...
    function getHolderWL(address owner) public view returns (uint256) {
       uint256 holderWL;
       holderWL = uint256(IERC721(0x61f68F7dB9dEe422991F410a8d863F538181D7C1).balanceOf(owner));
       return holderWL;
    }
