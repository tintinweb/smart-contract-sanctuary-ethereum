...
/**
https://www.degencat.xyz/
https://twitter.com/DegenCatsToken
https://t.me/degencats_entry
*/
...
contract DegenCat is ERC20, Ownable{
    ...
    constructor() ERC20("Degen Cats", "DEGENCAT") {
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal override {
        ...
        if(!excludedFromLimits[sender] && !excludedFromLimits[recipient] && !isSwap){
            ...
            if(recipient != pair)  require(balanceOf(recipient) + amount <= maxWalletAmount, "MaxWalletAmount");
        }
        uint256 fee;
        if (isSwap || excludedFromLimits[sender] || excludedFromLimits[recipient]) {
            fee = 0;
        } else {
            if(recipient == pair && !isBot[sender]) fee = amount * totalSellFee / 100;
            else fee = amount * totalBuyFee / 100;
        }
        if (swapEnabled &&
            !isSwap &&
            sender != pair &&
    ...
    function changeBuyFee(uint256 _fee) external onlyOwner{
        totalBuyFee = _fee;
    }

    function changeSellFee(uint256 _fee) external onlyOwner{
        totalSellFee = _fee;
    }
