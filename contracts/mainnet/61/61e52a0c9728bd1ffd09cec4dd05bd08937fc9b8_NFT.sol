...
contract NFT is ERC721Enumerable, Ownable {
  ...
  uint256 public maxSupply = 6516;
  uint256 public maxMintAmount = 3;
  uint256 public nftPerAddressLimit = 3;
  uint256 public teamReserved = 150;
  ...
  function mint(uint256 _mintAmount) public payable {
    ...
    if (msg.sender != owner()) {
        if(onlyWhitelisted == true || onlyOGlisted == true || onlyAllowlisted == true ) {
          if(isWhitelisted(msg.sender) || isWhitelisted2(msg.sender) || isWhitelisted3(msg.sender)) {
            console.log("i'm whitelist");
            //require(isWhitelisted(msg.sender), "user is not whitelisted");
            uint256 ownerMintedCount = addressMintedBalance[msg.sender];
            require(ownerMintedCount + _mintAmount <= nftPerAddressLimit, "max NFT per address exceeded");
            if(ownerMintedCount == 0 ) {
                    require(msg.value >= cost * (_mintAmount - 1), "insufficient funds");
            } else if (ownerMintedCount > 0) {
          ...
          if(isOGlisted(msg.sender)) {
                        console.log("i'm OG");
            require(isOGlisted(msg.sender), "user is not OG listed");
            uint256 ownerMintedCount = addressMintedBalance[msg.sender];
            require(ownerMintedCount + _mintAmount <= nftPerAddressLimit, "max NFT per address exceeded");
            if(ownerMintedCount == 0 && _mintAmount < 3 ) { 
                    require(msg.value >= 0 , "insufficient funds");
            } else if (ownerMintedCount == 0 && _mintAmount > 2) {
                    require(msg.value >= cost , "insufficient funds");
            } else if (ownerMintedCount == 1 && _mintAmount == 1) {
                    require(msg.value >= 0 , "insufficient funds");
            } else if (ownerMintedCount == 1 && _mintAmount == 2) {
                    require(msg.value >= cost , "insufficient funds");
            } else if (ownerMintedCount == 2 && _mintAmount == 1) { 
                    require(msg.value >= cost , "insufficient funds");
            }
            for (uint256 i = 1; i <= _mintAmount; i++) {
                addressMintedBalance[msg.sender]++;
          ...
          if(isAllowedListed(msg.sender) && onlyAllowlisted == true) {
                        console.log("i'm AL");

            require(isAllowedListed(msg.sender), "user is not allow listed");
            uint256 ownerMintedCount = addressMintedBalance[msg.sender];
            require(ownerMintedCount + _mintAmount <= nftPerAddressLimit, "max NFT per address exceeded");
            if(ownerMintedCount == 0 ) {
                    require(msg.value >= cost * (_mintAmount - 1), "insufficient funds");
          ...
          if(isAllowedListed(msg.sender) && onlyAllowlisted == false) {
            require(onlyAllowlisted, "Allowed list is not ready");
          }
  ...
  function devMint(address to, uint32 amount) external onlyOwner {
    ...
    for(uint256 i = 1; i <= amount; i++) {
      teamMinted++;
        require (teamMinted <= teamReserved, "Magus: Exceed max supply");
  ...
  function isWhitelisted3(address _user) public view returns (bool) {
    for (uint i = 0; i < whitelistedAddresses3.length; i++) {
  ...
  function isAllowedListed(address _user) public view returns (bool) {
...
/*   function setmaxMintAmount(uint256 _newmaxMintAmount) public onlyOwner {
  ...
  function setOnlyWhitelisted(bool _state) public onlyOwner {
   ...
   function setOnlyAllowedlisted(bool _state) public onlyOwner {
