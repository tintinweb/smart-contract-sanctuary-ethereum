...
contract Loan {
    ...
    uint256 private _fundingDeadline;
    ...
    function getDebt () public view returns (
        ...
        diffDays = (diffSeconds < 86400) ? 1 : ((diffSeconds % 86400 == 0) ? diffSeconds / 86400 : (diffSeconds / 86400) + 1);

        require(_loanAmountInPrincipalTokens >= principalRepaid, "Loans:N1");
        principalDebtAmount = _loanAmountInPrincipalTokens - principalRepaid;

        if (principalDebtAmount > 0) {
            interestDebtAmount = _loanAmountInPrincipalTokens * apr * diffDays / 365 / 1e4;
            require(interestDebtAmount > 0, "Loans:N2");

            require(_loanAmountInPrincipalTokens + interestDebtAmount >= principalRepaid, "Loans:N3");
            ...
            require(minInterestAmount > 0, "Invalid interest amount");
    ...
    /**
     * @notice Calculates the outstanding debt
     * @param currentUnixEpoch The current unix epoch
     * @param fundingEpoch The unix epoch in which the loan was funded
     * @param loanAmt The loan amount
     * @param theApr The APR of the loan
     * @param totalRepaid The repayments done to this contract so far
     * @return interestAccrued The interest accrued at the point in time specified
     * @return outstandingDebt The outstanding debt at the point in time specified
     * @return diffDays The number of days elapsed since the funding date specified
     */
    function calculateOutstandingDebt (uint256 currentUnixEpoch, uint256 fundingEpoch, uint256 loanAmt, uint256 theApr, uint256 totalRepaid) public pure returns (uint256 interestAccrued, uint256 outstandingDebt, uint256 diffDays) {
        require(fundingEpoch > 0, "Loan was not funded yet");
        require(currentUnixEpoch >= fundingEpoch, "Invalid date");

        uint256 diffSeconds = currentUnixEpoch - fundingEpoch;
        diffDays = (diffSeconds < 86400) ? 0 : ((diffSeconds % 86400 == 0) ? diffSeconds / 86400 : (diffSeconds / 86400) + 1);
        interestAccrued = loanAmt * theApr * diffDays / 365 / 1e4;
        outstandingDebt = loanAmt + interestAccrued - totalRepaid;
    }

    /**
     * @notice Gets the outstanding debt
     * @return interestAccrued The interest accrued at this point in time
     * @return outstandingDebt The outstanding debt at this point in time
     * @return diffDays The number of days elapsed since the funding date
     */
    function getOutstandingDebt () public view returns (uint256 interestAccrued, uint256 outstandingDebt, uint256 diffDays) {
        return calculateOutstandingDebt(block.timestamp, fundedOn, _loanAmountInPrincipalTokens, apr, repaid); // solhint-disable-line not-rely-on-time
    }
