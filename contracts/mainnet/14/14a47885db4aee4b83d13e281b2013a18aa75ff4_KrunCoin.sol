...
contract KrunCoin is Ownable, AccessControl, ERC20 {
  uint256 constant MILLION = 1_000_000 * 10**uint256(18);
  uint256 constant MAX_SUPPLY = 44 * MILLION;
  bytes32 public constant MINTER_ROLE = keccak256("MINTER_ROLE");

  constructor(address minter) ERC20("KRUN Coin", "KRUN") {
    _setupRole(MINTER_ROLE, minter);
    _setupRole(DEFAULT_ADMIN_ROLE, minter);
  }

  /// @notice Allows an account with the minter role to mint new tokens.
  /// @dev Cannot mint more than max supply
