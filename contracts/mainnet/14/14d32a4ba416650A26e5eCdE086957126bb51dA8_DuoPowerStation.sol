...
contract DuoPowerStation is Initializable, UUPSUpgradeable, OwnableUpgradeable,IDUOPowerStation{
  ...
  Pool[] public pools; // Staking pools
    ...
    function stakerJoinPool(uint256 _pid, uint256 shares, string memory payby) public override {
        ...
        renFil.safeTransferFrom(msg.sender,filAddr, poolUSDT.poolInfoVars[5] * shares);
        emit PayPledge(msg.sender, poolUSDT.poolInfoVars[5] * shares);

        ///@dev pay for expenses

        //Data+Gas
        renFil.safeTransferFrom(msg.sender, filAddr, 
                    ( ( poolUSDT.poolInfoVars[0]+poolUSDT.poolInfoVars[1]) * shares));          // 15/10 is ultility/T/month
        ...
        if (keccak256(abi.encodePacked(payby)) == keccak256(abi.encodePacked("USDT"))){
            ...
            usdt.safeTransferFrom(msg.sender,usdAddr, (poolUSDT.poolInfoVars[4] + (poolUSDT.timing[2]-poolUSDT.timing[1])/30 days * 
                                                           poolUSDT.poolInfoVars[2]) * shares);
            ...
            //RGP
            rgp.safeTransferFrom(msg.sender,usdAddr, poolRGP.poolInfoVars[3] * shares);
            emit PayPower(msg.sender, poolRGP.poolInfoVars[3] * shares);

            //IDC
            usdt.safeTransferFrom(msg.sender, usdAddr, (poolUSDT.timing[2]-poolUSDT.timing[1])/30 days * 
                                                            poolRGP.poolInfoVars[2]*shares);
    ...
    function withdraw(uint256 _pid) public override{
        ...
        renFil.transfer(msg.sender, remainingRewardsPerShareUSDT* stakerUSDT.shares + 
                                        remainingRewardsPerShareRGP* stakerRGP.shares);
    ...
    function topUpPool(uint256 _pid, uint256 _allocPoint) public override{
        ...
        renFil.safeTransferFrom(msg.sender, filAddr, _allocPoint - allocatedRewardsRGP - allocatedRewardsUSDT);
        renFil.safeTransferFrom(msg.sender, address(this), allocatedRewardsRGP + allocatedRewardsUSDT);
