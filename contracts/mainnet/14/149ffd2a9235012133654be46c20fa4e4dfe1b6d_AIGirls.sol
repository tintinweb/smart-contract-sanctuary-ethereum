...
contract AIGirls is ERC721A, Ownable {
  ...
  constructor() ERC721A("AIGirls","AIG") {}
  ...
  function mint(uint256 _mintAmount) public payable {
    ...
    uint256 cost = price;
    uint256 maxTx = maxPerTx;
    if(supply + _mintAmount < maxFreeSupply + 1) {
      cost = 0;
      maxTx = 3;
    }
    require(mintEnabled, "Minting is not live yet, hold on");
    require(_mintAmount < maxTx + 1, "Max per TX reached");
    require(supply + _mintAmount < maxSupply + 1,"No more NFTs");
  ...
  function setMaxFreeSupply(uint256 _totalFree) external onlyOwner {
    maxFreeSupply = _totalFree;
  }
