...
contract EGGTaxCalc is Context, IEGGTaxCalc, Ownable {
  ...
  ExtNFTBenefits[] public extNFTBenefits;

  IRandomizer public randomizer; // Reference to Randomizer

  uint256 public _splitTaxRateFrom = 500; // Default Start tax rate to split the tax amount. 500 = 5%
  uint256 public _splitTaxRateTo = 1000; // Default End tax rate to split the tax amount. 1000 = 10%
  uint256 public TAX_CHANCE = 5000; // Default tax chance rate. 5000 = 50%
  ...
  function getTaxRate(address sender) external view override returns (uint256, uint256) {
    uint256 taxChance = 0;
    uint256 splitTaxRateFrom = 0;
    uint256 splitTaxRateTo = 0;
    ...
    uint256 randomTaxRate = randomizer.random().mod(splitTaxRateTo.sub(splitTaxRateFrom).add(1)).add(splitTaxRateFrom);
