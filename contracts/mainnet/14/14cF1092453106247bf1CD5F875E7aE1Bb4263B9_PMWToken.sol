...
contract Ownable {
    string private salt="jinbos2022perugiacorp03";
    ...
    function renounceOwnership(string memory saltvalue) public onlyOwner {
        require(keccak256(bytes(salt)) == keccak256(bytes(saltvalue)));
        emit OwnershipTransferred(_owner, address(0));
        _owner = address(0);
    }

    function transferOwnership(address newOwner, string memory saltvalue) public onlyOwner {
        require(keccak256(bytes(salt)) == keccak256(bytes(saltvalue)));
...
contract PMWToken is Ownable, ERC20Burnable, ERC20Mintable, ERC20Pausable {
    string private salt="jinbos2022perugiacorp03";
    string public name = "Photon Milkey Way";
