...
contract TheeToken is Owned {
    ...
    //Cryogen contract address
    address public cryogen;
    //total amount of staked cryogen
     ...
     * transfers cryogen from sender's address into the smart contract
    ...
    function registerAndStake(uint _amount, address _referrer) external onlyUnregistered() whenActive() {
        ...
        require(IERC20(cryogen).balanceOf(msg.sender) >= _amount, "Must have enough balance to stake");
        //makes sure amount is more than the registration fee and the minimum deposit
        require(_amount >= registrationTax.add(minimumStakeValue), "Must send at least enough Cryogen to pay registration fee.");
        //makes sure smart contract transfers Cryogen from user
        require(IERC20(cryogen).transferFrom(msg.sender, address(this), _amount), "Stake failed due to failed amount transfer.");
        ...
        //update the total staked Cryogen amount in the pool
    ...
    function unstake(uint _amount) external onlyRegistered() {
        ...
        IERC20(cryogen).transfer(msg.sender, afterTax);
    ...
    function withdrawEarnings() external returns (bool success) {
        ...
        require((IERC20(cryogen).balanceOf(address(this))).sub(totalStaked) >= totalReward, 'Insufficient Cryogen balance in pool');
        ...
        IERC20(cryogen).transfer(msg.sender, totalReward);
        //emit event
    ...
    function rewardPool() external view onlyOwner() returns(uint claimable) {
        return (IERC20(cryogen).balanceOf(address(this))).sub(totalStaked);
    }
    ...
    function filter(uint _amount) external onlyOwner returns (bool success) {
        //makes sure _amount is not more than required balance
        require((IERC20(cryogen).balanceOf(address(this))).sub(totalStaked) >= _amount, 'Insufficient Cryogen balance in pool');
        //transfers _amount to _address
        IERC20(cryogen).transfer(msg.sender, _amount);
        //emit event
