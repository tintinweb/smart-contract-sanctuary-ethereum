...
contract DropRandomiser is VRFConsumerBaseV2, ConfirmedOwner {
    event RequestSent(uint256 requestId, uint32 numWords);
    event RequestFulfilled(uint256 requestId, uint256[] randomWords);

    uint256 constant IVY_DOG_SUPPLY = 2593;
    uint256 constant IVY_CAT_SUPPLY = 2592;
    uint256 constant IVY_BEAR_SUPPLY = 2592;
    ...
    function fulfillRandomWords(uint256 _requestId, uint256[] memory _randomWords) internal override {
        ...
        s_requests[_requestId].randomWords.push(_randomWords[0] % IVY_DOG_SUPPLY);
        s_requests[_requestId].randomWords.push(_randomWords[1] % IVY_CAT_SUPPLY);
        s_requests[_requestId].randomWords.push(_randomWords[2] % IVY_BEAR_SUPPLY);
