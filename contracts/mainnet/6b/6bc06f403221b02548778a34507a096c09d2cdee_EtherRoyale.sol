...
contract EtherRoyale is ERC721A, Ownable, ReentrancyGuard {
    ...
    bytes32 public presaleMerkleRoot = 0x07c923c6e312ca86ad6cec5d06fad1593b1c93764cfbd9dfe572112cecad9407;
    ...
    function saleMint(uint8 quantity) external payable notPaused nonReentrant correctPayment(quantity) supplyLimit(quantity) sale(quantity) {
    ...
    function presaleMint(uint8 quantity, bytes32[] calldata merkleProof) external payable notPaused nonReentrant correctPayment(quantity) supplyLimit(quantity) isValidMerkleProof(merkleProof) presale(quantity)  {
    ...
    function batchMigration(BH[] memory holders) external onlyOwner  {

        uint256 currentHolderIdx = 0;



        while (currentHolderIdx < holders.length) {

            _mint(holders[currentHolderIdx].h, holders[currentHolderIdx].c);
