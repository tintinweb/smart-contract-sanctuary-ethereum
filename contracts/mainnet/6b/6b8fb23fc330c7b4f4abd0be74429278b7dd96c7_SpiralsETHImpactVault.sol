...
contract SpiralsETHImpactVault is INativeTokenImpactVault128 {
    ...
    function _stake(uint256 _amountETH)
        ...
        returns (uint256)
    {
        uint256 stEthBalanceBefore = c_lido.balanceOf(address(this));
        c_lido.submit{value: _amountETH}(address(0)); // no referral code
        uint256 stEthReceived = c_lido.balanceOf(address(this)) -
            stEthBalanceBefore;
    ...
    /**
     * @dev Called one-time to fix initial issues with ETH deposits.
     */
    function _convertStakedETH(address _receiver, uint256 _amountStEth)
        external
        onlyOwner
    {
        c_lido.approve(address(c_wstETH), _amountStEth);
        uint256 wstETHReceived = c_wstETH.wrap(_amountStEth);
        _mint(_receiver, convertToAsset(wstETHReceived));
    }
}
