...
/**
 * @dev interface of a registry contract that can mint tokens.
 */
...
contract InstantMintCheckout is Checkout {
    ...
    struct PostCheckoutCallData {
        ...
        address tokenRegistryAddress;   // the {TokenRegistry} where the token will be minted post-checkout.
     ...
     * @param tokenRegistryAddress the {TokenRegistry} where the token will be minted post-checkout.
    ...
    /**
     * @dev a post-checkout function that mints a new token in the appropriate {TokenRegistry}.
     ...
     *      - {decoded.tokenRegistryAddress} must be the address of a {TokenRegistry} contract.
     * 
     */
     function _postCheckoutAction(string memory checkoutId, bytes memory data) internal override {
        ...
        try TokenRegistry(decoded.tokenRegistryAddress).mintToken(decoded.tokenReceiver, decoded.tokenProofOfIntegrity) {
            emit TokenMinted(
            ...
            revert("InstantMintCheckout: Post-checkout action failure - transaction reverted.");
        }
     }
...
abstract contract Checkout is Pausable, Ownable, ReentrancyGuard {
    ...
    address public paymentReceiver;         // the account that will be transferred the payments.
                                            // It is the responsibility of the checkout service provider to make sure that
                                            // the payment receiver, if it is a contract, can receive ETH and ERC20 tokens.
    ...
    function nativeCheckout(
        ...
        require(success, "Checkout: Error - Failed to process the payment.");
        emit PaymentProcessed(caller, paymentReceiver, request.paymentToken, request.paymentValue);
        _wrapUp(caller, request.checkoutId, request.postCheckoutCallData);
    }
