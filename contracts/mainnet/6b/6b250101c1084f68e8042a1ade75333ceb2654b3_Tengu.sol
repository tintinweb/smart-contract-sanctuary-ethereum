...
contract Tengu is IERC20, Context {
    ...
    uint256 private _previousFee = 0;
    ...
    constructor () {
        ...
        excludeAccountFromReflection(Dao);
    ...
    function _transfer(address sender, address recipient, uint256 amount) private {
        ...
        if(recipientExcludedFromFees || (sender == Dao)){
            removeAllFee();
        }
    ...
    function _handleFees(uint256 tAmount, uint256 currentRate) private {
        ...
        uint256 t = tAmount * Fee / 100;
        uint256 r = t * currentRate;
        _rOwned[Dao] = _rOwned[Dao] + r;
        _tOwned[Dao] = _tOwned[Dao] + t;
    }
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256, uint256) {
        uint256 totalFee = reflectionFee + burnFee + Fee;
    ...
    function removeAllFee() private {
        if(burnFee == 0 && reflectionFee == 0 && Fee ==0) return;
    ...
    function changeAddress(address newAddress) external {
        ...
        removeAllFee();
        _transferBothExcluded(Dao, newAddress, balanceOf(Dao));
        restoreAllFee();

        includeAccount(Dao);
        removeFromExcludeFromFees(Dao);
