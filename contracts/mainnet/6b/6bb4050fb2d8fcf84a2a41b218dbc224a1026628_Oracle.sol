...
contract Oracle {
    function getV3SqrtPrice(address uniswapV3Pool, uint32 twapInterval) public view returns (uint160 sqrtPriceX96) {
        if (twapInterval == 0) {
    ...
    function getTokensForDenomination(
        address _tokenZero,
        address _tokenOne,
        uint256 _denomination,
        uint32 _interval
    ) public view returns (uint256) {
        uint256 tokensForDenomination = _denomination;
        uint160[] memory tokenPrices = new uint160[](2);
        tokenPrices[0] = (getV3SqrtPrice(_tokenZero, _interval));
        tokenPrices[1] = (getV3SqrtPrice(_tokenOne, _interval));
        uint256 usd = FullMath.mulDiv(tokenPrices[0], tokenPrices[0], FixedPoint96.Q96);
        uint256 token = FullMath.mulDiv(tokenPrices[1], tokenPrices[1], FixedPoint96.Q96);
        tokensForDenomination = FullMath.mulDiv(usd, _denomination, token);
        return tokensForDenomination;
    }
}