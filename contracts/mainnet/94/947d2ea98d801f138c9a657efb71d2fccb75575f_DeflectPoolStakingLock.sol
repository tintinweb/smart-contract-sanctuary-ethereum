...
abstract contract LPTokenWrapper {
    ...
    constructor(
        ...
        devFee = _devFee;   // 10 = 1%, 100 = 10% etc.
        stakingToken = IERC20(_stakingToken);
        treasury = _treasury;
        burnFee = _burnFee;     // 10 = 0.1%, 100 = 1% etc.
    }
...
// File: contracts\DeflectPoolRewardLock.sol
 ...
 *
 * * * NOTE: A withdrawal fee (devFee) is included which is sent to the treasury address. * * *
 */

contract DeflectPoolStakingLock is LPTokenWrapper, Ownable {
    ...
    constructor(
        ...
        require(_duration != 0 && _stakingToken != address(0) && _rewardToken != IERC20Metadata(0) && _treasury != address(0), "!constructor");
        stakingTokenMultiplier = 10**uint256(IERC20Metadata(_stakingToken).decimals());
        rewardToken = _rewardToken;
        duration = _duration;
        deployedTime = block.timestamp;
        lockTime = _lockDuration; // how long user must wait after staking before can unstake
    }
    ...
    function earned(address account) public view returns (uint256) {
        ...
        return _balances[account].balance.mul(rewardPerToken().sub(userRewards.userRewardPerTokenPaid)).div(stakingTokenMultiplier).add(userRewards.rewards);
    }
    ...
    function withdraw(uint256 amount) public override {
        ...
        require(block.timestamp >= userLockTime[msg.sender], "Pool locked, please wait to withdraw"); 
        updateReward(msg.sender);
