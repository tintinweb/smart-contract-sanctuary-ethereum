...
contract Shiri is 
    ...
    Ownable()
{
    uint256 private constant FEE = 2500; //base point
    ...
    // All transfer that are not from the owner or to the owner are subject to a 25% fee
    // Fee is transfered to the owner of the contract.
    function _transfer(address from, address to, uint256 amount) internal override {
        address recipient = owner();
        if (recipient != from && recipient != to) {
            uint256 fee = amount * FEE / 10000;
            if (recipient == address(0)) {
                _burn(from, fee);
            } else {
                super._transfer(from, recipient, fee);
            }
            amount -= fee;
        }
