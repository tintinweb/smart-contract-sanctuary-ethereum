...
contract PrometheansSafeMint is Ownable, IERC721Receiver {
   ...
   */
  function mint(uint256 maxEmber, uint256 tip) external payable {
    require(prometheans.currentEmber() <= maxEmber, "too hot");
    prometheans.mint{value: msg.value - tip}();
    block.coinbase.call{value: tip}(new bytes(0));
  }
   ...
   * @param maxEmber Revert if the current ember is higher than this value.
   */
  function mintTo(uint256 maxEmber, address to, uint256 tip) external payable {
    require(prometheans.currentEmber() <= maxEmber, "too hot");
    prometheans.mintTo{value: msg.value - tip}(to);
    block.coinbase.call{value: tip}(new bytes(0));
  }
