...
contract CombatBaze is ERC721, ERC721Enumerable, ERC721URIStorage, Ownable {
   ...
   // Counters.Counter private _tokenIdCounter;
    ...
    TokenPrice[] public TokenPriceInfo;
    ...
    mapping(uint256=>TokenPrice) public TokenPriceInfoData;
    ...
    function addTokensManual(uint256 _tokenId, uint256 _rarity) external onlyOwner {
    ...
    function addTokens(uint256 _tokenId, uint256 _rarity) internal {
    ...
    function updateTokenPrice(uint256 _rarity, uint256 _costEth, uint256 _costToken0 ) external onlyOwner {
         TokenPrice memory newRequest = TokenPrice({
                rarity: _rarity,
                costEth: _costEth,
                costToken0: _costToken0
            });

        TokenPriceInfoData[_rarity] = newRequest;
    }

    function checkTokenIsPurchased(uint256 _tokenId) public view returns(uint256 isPurchased){
      TokenRarity storage token_data = TokenData[_tokenId];
      return token_data.isPurchased;
    }

    function buyMembershipErc(uint256 _tokenId, uint256 _rarity, uint256 _pid) external payable {
        TokenInfo storage tokens = AllowedCrypto[_pid];
        TokenRarity storage token_data = TokenData[_tokenId];
        TokenPrice storage token_price = TokenPriceInfoData[_rarity];
        paytoken = tokens.paytoken;
        require(token_data.isPurchased == 0, "NFT Already Sold");
        require(msg.value >= token_price.costToken0, "Price Error");
        paytoken.transferFrom(msg.sender, address(this), msg.value);
        mintMembership(_tokenId,_rarity);
    }

    function buyMembershipEth(uint256 _tokenId, uint256 _rarity) external payable {
        TokenRarity storage token_data = TokenData[_tokenId];
        TokenPrice storage token_price = TokenPriceInfoData[_rarity];
        require(token_data.isPurchased == 0, "NFT Already Sold");
        require(msg.value >= token_price.costEth, "Price Error");
        mintMembership(_tokenId,_rarity);
    }

    function mintMembership(uint256 _tokenId, uint256 rarityItem) internal {
        require(isMintEnabled, "Not For Sale");
        require(totalSupply() < maxSupply, "Sold Out");

        addTokens(_tokenId, rarityItem);
        _safeMint(msg.sender, _tokenId);
    }

    function ownerBuy(uint256 _tokenId, uint256 _rarity) external onlyOwner {
        addTokens(_tokenId, _rarity);
        _safeMint(msg.sender, _tokenId);
    }
