...
import './abstracts/ERC20Transfer.sol';
import '@openzeppelin/contracts-upgradeable/token/ERC20/extensions/draft-ERC20PermitUpgradeable.sol';
import './abstracts/IRewardToken.sol';
...
contract ERC20Reward is ExtendedERC20, IRewardToken {
     ...
     * @dev - _userInfo store each individual users staked info. 
        ...
        {stakeQueue} - when user stake GTD, the amount and mature blockNum will add to this queue
    ...
    */
     ...
     * @dev - _poolInfo general pool info . 
        ...
        {accAdjust} - most of time total GTD staked value is greater than reward. It will make this calculation (reward/totalStaked) be 0. ACC_ADJUST is default 10**25 that corrects the calculation  
        ...
        {stakeQueue} - the staked amout that still waiting for lockupPeriod, will add into totalStaked once lockupPeriod is mature
    */
    ...
    function settings() external view returns (uint256 halveBlocks, uint256 initialBlock) {
        return (_halveBlocks, _initalBlock);
    }
     ...
     */
    function _getMintableAllowance(address account) internal view returns (uint256 mintAllowance) {
        uint256 mintableAllowance = 0;
        ...
        return mintableAllowance;
    }

    function getUnclaimedReward(address account) external view returns (uint256 amount) {
        uint256 amount;
        (uint256 futureAcc, uint256 futureStaked, uint256 latsUpdate) = _getFutureAcc(block.number + 1);
        if (futureStaked > 0) {
            futureAcc += (_getRangeReward(latsUpdate, block.number + 1) * ACC_ADJUST) / futureStaked;
        }
     ...
     */
    function _getFutureReward(address account, uint256 blockNum) private view returns (uint256 mintAllowance) {
        uint256 mintableAllowance = 0;
        uint256 debtCount = _userInfo[account].debtCount;
        if (debtCount > 0) {
            for (uint256 i = debtCount; i > 0; ) {
                if (blockNum >= _userInfo[account].debtBlock[i - 1]) {
                    mintableAllowance =
                        _userInfo[account].pendingMinting[i - 1] +
                        ((_poolInfo.userDebtShare *
                            (_userInfo[account].amountStaked + _lookupLockupPeriodForUser(account, blockNum))) /
                            ACC_ADJUST) -
                        _userInfo[account].debt[i - 1];
                    break;
                }
    ...
    {
        uint256 _futureAcc = _poolInfo.accGatePerShare;
        uint256 _futureStaked = _poolInfo.totalStaked;
        uint256 _lastBlock = _poolInfo.lastUpdatedBlock;
        if (!StakeQueue.isEmpty(_poolInfo.queue)) {
            for (uint256 i = _poolInfo.queue.first; i <= _poolInfo.queue.last; ) {
                if (_futureStaked > 0 && blockNum >= _poolInfo.queue.data[i].blockNum) {
                    _futureAcc +=
                        (_getRangeReward(_lastBlock, _poolInfo.queue.data[i].blockNum) * ACC_ADJUST) /
                        _futureStaked;
                }
                unchecked {
                    _lastBlock = _poolInfo.queue.data[i].blockNum;
                    _futureStaked += _poolInfo.queue.data[i].amount;
                    ++i;
                }
            }
        }
        return (_futureAcc, _futureStaked, _lastBlock);
    }
    ...
    function getRangeReward(uint256 start, uint256 end) external view returns (uint256) {
     ...
     */
    function _getRangeReward(uint256 start, uint256 end) internal view returns (uint256) {
    ...
    /**
        @dev check if user.stakeQueue is matured at blockNum, return total matured value
        this function is view because individual user.amountStake can only be changed when updateAccumulated() is called and getMintableAllowance is a view function     
    */
    function _lookupLockupPeriodForUser(address account, uint256 blockNum) private view returns (uint256) {
        uint256 pending;
    ...
    /**
        ...
        this function is view because individual user.amountStake can only be changed when updateAccumulated() is called and getMintableAllowance is a view function     
    */
    function _proccessLockupPeriodForUser(address account, uint256 blockNum) private returns (uint256) {
        uint256 pending;
        uint32 last = _userInfo[account].queue.last;
        for (uint256 i = _userInfo[account].queue.first; i <= last; ) {
            if (blockNum > _userInfo[account].queue.data[i].blockNum) {
    ...
    /**
        ...
        1. Proccess user stakeQueue, this will add pending amount to user.amountStaked if lockupPeriod is ended
    ...
    function updateAccumulatedWhenStake(address account, uint256 amount) external virtual onlyGTD returns (bool) {
        ...
        uint256 _pastTotalStaked = GTD.getPastTotalStaked(block.number - 1);
    ...
    function updateAccumulatedWhenUnstake(address account, uint256 amount) external virtual onlyGTD returns (bool) {
        ...
        _userInfo[account].debt[_userInfo[account].debtCount] =
            (_userInfo[account].amountStaked * _poolInfo.accGatePerShare) /
            ACC_ADJUST;
    ...
    /**
        ...
        This function will 
        1. check _poolInfo.stakeQueue array see if lockupPeriod matured then add into totalStaked value
        2. calculates the accGatePerShare value base on the latest totalStaked value
        3. update lastUpdatedBlock value
    */
...
interface IStakedVotesUpgradeable {
     ...
     */
    function stakeOf(address stakeholder, uint256 blockNumber) external view returns (uint256);
