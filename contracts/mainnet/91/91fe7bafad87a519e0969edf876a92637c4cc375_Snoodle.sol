...
contract Snoodle is ERC721Enumerable, Ownable {
    ...
    uint public maximumFreeMint = 1000;

    // withdraw addresses
    address private constant t1 = 0x6c8B306806D3D4f3d43Cd435ac27d2afddEa511B;


    constructor(string memory baseURI) ERC721("Snoodles", "SNDL")  {
        setBaseURI(baseURI);

      uint supply = totalSupply();
      uint i;
      for (i = 1; i <= _reserved; i++) {
    ...
    function mint(uint256 num) public payable {
        ...
        require(num>0);
        require( num < 21,                              "You can adopt a maximum of 20 Snoodles" );
        require( supply + num <= 8888-_reserved,      "Exceeds maximum Snoodles supply" );
     ...
     function freeMint(uint quantity) external {
        uint256 supply = totalSupply();
        require(!_paused, "the sale is not active yet");
    ...
    returns (string memory)
  {
    ...
    function withdrawAll() public payable onlyOwner {
        uint256 _t1_bal = address(this).balance;
    
        require(payable(t1).send(_t1_bal));
