...
import "./libs/EthereumCroDefiSwapLibrary.sol";
import "../../crossChain/interfaces/IWrapper.sol";
import "@openzeppelin/contracts/token/ERC20/IERC20.sol";
import "@openzeppelin/contracts/token/ERC20/utils/SafeERC20.sol";

contract O3EthereumCroDefiSwapAggregator is Ownable {
    ...
    address public factory = 0x9DEB29c9a4c7A88a3C0257393b7f3335338D9A9D;
    ...
    function _swapExactTokensForTokensSupportingFeeOnTransferTokens(
        ...
        if (pull) {
            IERC20(path[0]).safeTransferFrom(
                msg.sender, EthereumCroDefiSwapLibrary.pairFor(factory, path[0], path[1]), amountIn
            );
        } else {
            IERC20(path[0]).safeTransfer(EthereumCroDefiSwapLibrary.pairFor(factory, path[0], path[1]), amountIn);
        }
    ...
    function _swapExactETHForTokensSupportingFeeOnTransferTokens(
        ...
        assert(IWETH(WETH).transfer(EthereumCroDefiSwapLibrary.pairFor(factory, path[0], path[1]), amountIn));
        uint balanceBefore = IERC20(path[path.length - 1]).balanceOf(address(this));
        _swapSupportingFeeOnTransferTokens(path, address(this));
        uint amountOut = IERC20(path[path.length - 1]).balanceOf(address(this)) - balanceBefore;
    ...
    function _swapSupportingFeeOnTransferTokens(address[] memory path, address _to) internal virtual {
        for (uint i; i < path.length - 1; i++) {
            ...
            (address token0,) = EthereumCroDefiSwapLibrary.sortTokens(input, output);
            IUniswapV2Pair pair = IUniswapV2Pair(EthereumCroDefiSwapLibrary.pairFor(factory, input, output));
            uint amountInput;
            uint amountOutput;
            { // scope to avoid stack too deep errors
                ...
                amountOutput = EthereumCroDefiSwapLibrary.getAmountOut(amountInput, reserveInput, reserveOutput, factory);
            }
            (uint amount0Out, uint amount1Out) = input == token0 ? (uint(0), amountOutput) : (amountOutput, uint(0));
            address to = i < path.length - 2 ? EthereumCroDefiSwapLibrary.pairFor(factory, output, path[i + 2]) : _to;
