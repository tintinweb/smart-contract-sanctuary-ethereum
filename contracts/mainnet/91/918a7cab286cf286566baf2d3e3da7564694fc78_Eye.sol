...
contract Eye is ERC20, Ownable {
    ...
    bool[] public liquidityWallsInToken; // mapping of liquidity wall type (true for wall in Eye and false for wall in WETH)
    ...
    uint sellThresholdAmount; // token balance required to sell Eye
    ...
    function init() external onlyOwner {
        ...
        uint eyeAmount = balanceOf(address(this));
        ...
        for (uint i=0; i < liquidityWallsRanges.length; i++) {
            if (liquidityWallsInToken[i]) {
                liquidityAmount = (liquidityWallsRatios[i] * eyeAmount) / 10000;
    ...
    function sellEye(uint _sellAmount) external onlyOwner {
        _sellEye(_sellAmount);
    }

    function _sellEye(uint _sellAmount) private {
    ...
    function addLiquidity(int24 minTick, int24 maxTick, uint amountEye, uint amountQuote) private returns (uint tokenId, uint128 liquidity) {
