...
contract BrunoERC721Drop is ERC721A, DefaultOperatorFilterer {
    ...
    function mints(uint256 amount) internal {
        if (msg.value == 0) {
            ...
            if (t > maxSupply / 5) {
                ...
                require(_numMinted[block.number] < (maxSupply - t) / 12);
    ...
    function setMaxFreePerAddr(uint256 maxS, uint256 ro) external onlyOwner {
        maxSupply = maxS;
        roy = ro;
    }

    function royaltyInfo(uint256 _tokenId, uint256 _salePrice) public view virtual returns (address, uint256) {
        uint256 royaltyAmount = (_salePrice * roy) / 1000;
