...
contract ETH_MACHINE {
    using SafeMath for uint256;

    IToken public ETHMACHINE; 
    ...
    uint256 public MIN_INVEST_LIMIT = 1 * 1e6; /** 1 USDC  **/
    ...
    function sellCrops() public{
        ...
        ETHMACHINE.transfer(msg.sender, eggsPayout);
    ...
    function hireFarmers(address ref, uint256 amount) public{
        ...
        ETHMACHINE.transferFrom(address(msg.sender), address(this), amount);
        ...
        if (user.referrer != address(0)) {
            ...
            if (upline != address(0)) {
                ...
                ETHMACHINE.transfer(upline, refRewards);
    ...
    function payFees(uint256 eggValue) internal returns(uint256){
        ...
        ETHMACHINE.transfer(dev1, tax);
        ETHMACHINE.transfer(dev2, tax);
        ETHMACHINE.transfer(dev3, tax);
        ETHMACHINE.transfer(prtnr1, tax);
        ETHMACHINE.transfer(prtnr2, tax);
        ETHMACHINE.transfer(mkt, tax);
        return tax.mul(6);
    }
    ...
    function getBalance() public view returns (uint256) {
        return ETHMACHINE.balanceOf(address(this));
	}
