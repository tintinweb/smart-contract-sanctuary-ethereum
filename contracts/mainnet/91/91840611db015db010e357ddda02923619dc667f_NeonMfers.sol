...
contract NeonMfers is ERC721A, Ownable {
    using SafeMath for uint256;
    using Strings for uint256;
    uint256 public constant MAXASSETS = 2000;
    uint256 public constant freeMints = 1750;
    uint256 public constant maxFreeMintsPerWallet = 40;
    ...
    constructor(string memory baseURI, address _proxyRegistryAddress) ERC721A("Neon Mfers", "nMfers") {
        setBaseURI(baseURI);
        isSaleActive = false;
    ...
    function mintNFT(uint256 numAssets) external payable {
        ...
        require(numAssets >= 0 && numAssets <= maxPurchase,
            "You can only mint 10 assets at a time!");
        require(totalSupply().add(numAssets) <= MAXASSETS - reservedAssets,
        ...
        if(totalSupply().add(numAssets) > freeMints){
            ...
            require(freeMintsWallet[msg.sender].add(numAssets) <= maxFreeMintsPerWallet, 
                "You can only mint 40 assets for free!");
    ...
    function mintNFTS(address _to, uint256 _amount) external onlyOwner() {
        require(totalSupply().add(_amount) <= MAXASSETS - reservedAssets,
