...
contract FullSetup {
    ...
    {
        (address comptroller, address primary, address backup, address guardian, uint[] memory accountFees) = abi.decode(_data,(address,address,address,address,uint[]));
        uint len;
        ...
        // Update Comptroller
        actions[actions.length-1] = _updateComptrollerAction(_data,_onBehalf);
    ...
    function _updateComptrollerAction(bytes memory _data, address _onBehalf) internal view returns (bytes memory){
        (address comptroller, address primary, address backup, , uint[] memory accountFees) = abi.decode(_data,(address,address,address,address,uint[]));
        return abi.encodePacked(uint8(0),comptroller,uint256(0),uint256(292),abi.encodeWithSignature(
        "registerUser(address[],uint256[],address,address,uint256)", _getClientSafes(_onBehalf), accountFees, primary, backup, _getAdvisorId(comptroller,_onBehalf)));
    }
