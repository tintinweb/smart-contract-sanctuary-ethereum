...
contract CheddarverseERC721 is ERC721, Ownable, ReentrancyGuard, Trustable {
  ...
  uint256 internal _secondPresaleSupply;
  uint256 internal _tokenOffset;
  string internal _baseTokenURI;
  uint256 internal _remainSaleSupply;
  ...
  uint256 constant public MAX_PRESALE = 250;
  uint256 constant public MAX_SECOND_PRESALE = 250;
  ...
  function remainSale() public view returns (uint256) {
        return _presaleSupply + _secondPresaleSupply;
  }
    ...
    onlyWhitelisted
  {
    ...
    require(presaleMints[msg.sender] + amount <= PRESALE_MAX_MINT, "Exceeds remaining presale balance");
    require(_secondPresaleSupply + amount <= MAX_SECOND_PRESALE, "Exceeds maximum number of Second-Presale supply");

    _mintAmount(amount, msg.sender);

    presaleMints[msg.sender] += amount;
    _secondPresaleSupply += amount;
  }
