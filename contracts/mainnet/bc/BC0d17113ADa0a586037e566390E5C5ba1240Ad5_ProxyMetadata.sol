...
contract ProxyMetadata is IMetadata, PrimaryEns {
    IManager public immutable domainManager;
    IMetadata public immutable defaultMetadata;
    SubscriptionRegistrationRules public immutable subscriptionMetadata;

    constructor(
        address _esf,
        address _metadata,
        address _subscriptionMetadata
    ) {
        domainManager = IManager(_esf);
        defaultMetadata = IMetadata(_metadata);
        subscriptionMetadata = SubscriptionRegistrationRules(
    ...
    function getMetadata(uint256 _tokenId) private view returns (IMetadata) {
        ...
        IMetadata metadata = subscriptionMetadata.mintCount(parentId) > 0
