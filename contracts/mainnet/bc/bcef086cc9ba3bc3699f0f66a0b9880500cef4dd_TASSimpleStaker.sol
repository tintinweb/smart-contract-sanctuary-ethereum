...
// File contracts/TimepieceApeSociety.sol
...
contract TimepieceApeSociety is ERC721A, Ownable {
  ...
  constructor() ERC721A("Timepiece Ape Society", "TAS") {
    setBaseURI("");
    setNotRevealedURI("");
  }
...
// File contracts/SimpleStaker.sol
...
contract TASSimpleStaker is AdministratorOrOwner, IERC721Receiver {
    mapping(address => uint16[]) stakedTokens;
    TimepieceApeSociety public collection;
    ...
    (
        ...
        override
        isActive
        onlyFromCollection(msg.sender)
        returns(bytes4)
    {
        stakedTokens[from].push(uint16(tokenId));
     ...
     * STAKING
     */
    function stake(uint16[] calldata _tokenIds)
        public
        isActive
    ...
    function unstake(uint16[] calldata _tokenIds)
        public
        isCollectionSet
    {
        uint16[] memory tmpData = stakedTokens[msg.sender];
        uint16[] memory endResult = new uint16[](stakedTokens[msg.sender].length - _tokenIds.length);

        bool found;
        for (uint256 i; i < _tokenIds.length; ++i) {
            found = false;

            for (uint256 j; j < tmpData.length - i; ++j) {
                if (tmpData[j] == _tokenIds[i]) {
                    tmpData[j] = tmpData[tmpData.length - 1 - i];
            ...
            require(found, "You do not own this token.");
        }

        for (uint256 i; i < endResult.length; ++i) {
            endResult[i] = tmpData[i];
        }
        stakedTokens[msg.sender] = endResult;
    }

    /**
     * ADMIN TOOLS
     */
    function setCollection(address _collectionAddress) 
        public
        onlyOwnerOrAdministrator
    {
        collection = TimepieceApeSociety(_collectionAddress);
    }
    ...
    function totalStaked()
        ...
        returns(uint256)
    {
        return collection.balanceOf(address(this));
    }

    function ownedByAddress(address _owner)
        public
        view
     ...
     */
    modifier isActive() {
        require(active, "Staking is paused");
        _;
    }

    modifier isApproved(address _staker) {
        bool approved = collection.isApprovedForAll(_staker, address(this));
        require (approved, "You have not approved staking");
        _;
    }

    modifier isCollectionSet() {
        require(address(collection) != address(0x0), "Collection is not set");
        _;
    }

    modifier onlyFromCollection(address _from) {
        require(_from == address(collection), "You are trying to stake a token that does not belong here");
        _;
    }
}