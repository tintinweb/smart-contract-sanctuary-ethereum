...
// import "https://raw.githubusercontent.com/OpenZeppelin/openzeppelin-contracts/master/contracts/utils/cryptography/MerkleProof.sol";
...
contract AirDropper is Ownable {
    ...
    function addAirdropWithExpiration(
        uint256 airdropId,
        address token,
        uint256 amount,
        bytes32 merkleRoot,
        ...
        airDrop.merkleRoot = merkleRoot;
        airDrop.amount = amount;
        airDrop.expirationTimestamp = uint64(block.timestamp) + expirationTime;
    ...
    function claim(uint256 airdropId, uint256 index, address account, uint256 amount, bytes32[] calldata merkleProof) external {
        ...
        if (airDrop.expirationTimestamp > 0) {
    ...
    function withdrawTokensFromExpiredAirdrop(uint256 airdropId) external {
        ...
        require(airDrop.expirationTimestamp > 0, "Airdrop doesn't have expiration date");
        ...
        airDrop.amount = 0;
        IERC20(airDrop.token).safeTransfer(msg.sender, amount);
    }
