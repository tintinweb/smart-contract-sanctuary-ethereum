...
contract STZVoucherRedeemer is Recoverable, Pausable, ERC1155TokenReceiver {
    ...
    function setVoucherValues(
        ...
        require(
            ...
            "STZVoucherRedeemer: invalid length of array"
        );
        for (uint256 i; i < tokenIds.length; ++i) {
            uint256 amount = amounts[i];
            require(amount > 0, "STZVoucherRedeemer: invalid amount");
     ...
     */
    function getVoucherValue(uint256 tokenId)
        external
        view
        virtual
        returns (uint256)
     ...
     */
    function _voucherValue(uint256 tokenId)
        internal
        view
        virtual
        returns (uint256)
    {
        ...
        require(tokenValue > 0, "STZVoucherRedeemer: invalid voucher");
        return tokenValue;
    }
    ...
    function onERC1155Received(
        ...
        _requireNotPaused();
        require(
            msg.sender == address(vouchersContract),
            "STZVoucherRedeemer: wrong sender"
        );
...
import {ManagedIdentity} from "./interfaces/ManagedIdentity.sol";
import {Ownable} from "./interfaces/Ownable.sol";
import {IWrappedERC20, ERC20Wrapper} from "./interfaces/ERC20Wrapper.sol";
