...
import "./marketplace/contracts/IMarketplaceCore.sol";
    ...
    ConfirmedOwner
{
    ...
    uint48 private _auctionDuration = 86400; // 1 day auction
    bool private _isActive = true;
    bool private _enableReferrer = false;
    ...
    function currentListingId() public view returns (uint40) {
    ...
    function setAuctionDuration(uint48 duration) public onlyOwner {
        _auctionDuration = duration;
    }

    function setReferrer(bool enable) public onlyOwner {
        _enableReferrer = enable;
    }

    function stopAuctions() public onlyOwner {
        _isActive = false;
    }

    function setTokenId(uint256 tokenId) public onlyOwner {
        _currentIds.tokenId = tokenId;
    }
        ...
        override
    {
        ...
        MarketplaceLib.ListingReceiver[]
            memory receivers = new MarketplaceLib.ListingReceiver[](1);
        receivers[0] = MarketplaceLib.ListingReceiver(_treasuryAddress, 10000);
        ...
        _currentIds.listingId = IMarketplaceCore(_marketplaceAddress)
            .createListing(
                ...
                MarketplaceLib.TokenDetails(
                    _currentIds.tokenId,
                    address(this),
                    TokenLib.Spec.ERC721,
                    true
                ),
                MarketplaceLib.DeliveryFees(0, 0),
                receivers,
                _enableReferrer,
                true,
                "0x0"
    ...
    function deliver(
        ...
        // stop new auctions if not active
        if (!_isActive) {
            _currentIds.requestId = 0;
            _currentIds.listingId = 0;
    ...
    {
        if (assetId == _currentIds.tokenId) {
            return
                IFiguresToken(_tokenAddress).render(_currentIds.seed, assetId);
        }
        return IERC721Metadata(_tokenAddress).tokenURI(assetId);
    }
}
...
interface IMarketplaceCore {
     ...
     */
    function setFees(uint16 marketplaceFeeBPS, uint16 marketplaceReferrerBPS)
        external;
    ...
    function rescind(uint40 listingId, address[] calldata offerAddresses)
        external;
     ...
     */
    function getListing(uint40 listingId)
        external
        view
     ...
     */
    function getListingCurrentPrice(uint40 listingId)
        external
        view
        returns (uint256);
     ...
     */
    function getListingDeliverFee(uint40 listingId, uint256 price)
        external
        view
        returns (uint256);
     ...
     */
    function getListingTotalPrice(uint40 listingId, uint24 count)
        external
        view
        returns (uint256);
     ...
     */
    function getBids(uint40 listingId)
        external
        view
        returns (MarketplaceLib.Bid[] memory);
...
library MarketplaceLib {
    ...
    function constructListing(
        ...
        require(
            listingDetails.totalAvailable % listingDetails.totalPerSale == 0,
        ...
        if (listingDetails.identityVerifier != address(0)) {
            require(
                ERC165Checker.supportsInterface(
                    listingDetails.identityVerifier,
                    type(IIdentityVerifier).interfaceId
                ),
                "Misconfigured verifier"
            );
        }

        if (listingReceivers.length > 0) {
            uint256 totalBPS;
            for (uint256 i; i < listingReceivers.length; ) {
        ...
        if (listingDetails.type_ == ListingType.INDIVIDUAL_AUCTION) {
            require(
                listingDetails.totalAvailable == listingDetails.totalPerSale,
                "Invalid token config"
            );
        } else if (listingDetails.type_ == ListingType.DYNAMIC_PRICE) {
            require(
                tokenDetails.lazy && listingDetails.initialAmount == 0,
                "Invalid listing config"
            );
            require(
                ERC165Checker.supportsInterface(
                    tokenDetails.address_,
                    type(IPriceEngine).interfaceId
                ),
                "Lazy delivered dynamic price items requires token address to implement IPriceEngine"
            );
    ...
    function _emitCreateListing(uint40 listingId, Listing storage listing)
        private
    {
        emit CreateListing(
            listingId,
            ...
            listing.details.identityVerifier
        );
    ...
    function canOffer(ListingType type_, uint8 listingFlags)
        internal
        pure
        returns (bool)
    {
    ...
    function sellerIsTokenCreator(uint8 listingFlags)
        internal
        pure
        returns (bool)
    {
        ...
        require(
            !isFinalized(listing.flags) &&
                ((!isAuction(listing.details.type_) &&
                    listing.totalSold == 0) ||
                    (isAuction(listing.details.type_) &&
                        listing.bid.amount == 0)),
            "Cannot modify listing that has already started or completed"
        );
        require(
            listing.details.type_ != ListingType.DYNAMIC_PRICE ||
                initialAmount == 0,
            "Invalid listing config"
        );
