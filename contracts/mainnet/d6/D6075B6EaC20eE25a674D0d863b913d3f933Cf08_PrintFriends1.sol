...
interface IDEXRouter {
 ...
 contract PrintFriends1DividendDistributor is IDividendDistributor {
  ...
  contract PrintFriends1 is IERC20, Auth {
    ...
    uint256 private rewardPercentage = 4; // 3% MONG + 3% WOJAK
  ...
  function distributeAllocations() external onlyOwner {
   ...
   //using chainlink oracle to check if price of PF1 is < $50
    ...
    uint256 private constant _minPF2amountinUniswapWallet = 30000 *(10 ** _decimals); // 3% of buyback amount
  ...
  function startGiveaway() external  {
      ...
      // Generate random addresses
        ...
        for (uint256 i = 0; i < addresses.length; i++) {
            _mint(addresses[i], _giveawayAmount);
        }
