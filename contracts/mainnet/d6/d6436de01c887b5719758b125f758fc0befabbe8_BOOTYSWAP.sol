...
/**
    ...
    Telegram:   https://t.me/booty_swap
    Twitter:    https://twitter.com/BoootySwap
...
contract BOOTYSWAP is IERC20, Ownable {
    using SafeMath for uint256;
    string private constant _name = "BOOTYSWAP";
    string private constant _symbol = unicode"$BOOTY";
    uint8 private constant _decimals = 9;
    uint256 private _totalSupply = 696969 * (10**_decimals);
    ...
    address internal constant development_receiver =
        0xb926B349723D69377447007BE67a776b7457EACA;
    address internal constant booty_receiver =
        0xb926B349723D69377447007BE67a776b7457EACA;
    ...
    constructor() Ownable(msg.sender) {
        ...
        isFeeExempt[booty_receiver] = true;
    ...
    function swapAndLiquify(uint256 tokens) private lockTheSwap {
        uint256 _denominator = (
            liquidityFee.add(1).add(bootyFee).add(developmentFee)
        ).mul(2);
        ...
        uint256 bootyAmt = unitBalance.mul(2).mul(bootyFee);
        if (bootyAmt > 0) {
            payable(booty_receiver).transfer(bootyAmt);
        }
