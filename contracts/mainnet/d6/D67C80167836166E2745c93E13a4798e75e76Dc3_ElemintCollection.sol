  ...
  ERC2981Upgradeable
{
  ...
  modifier onlyOwnerOrDefaultOwner() {
    require(
      (_msgSender() == owner()) || (_msgSender() == _getDefaultOwner()),
      "Ownable: caller is not the owner or default owner"
    );
    _;
  }
  ...
  function setName(string memory name_) external onlyOwnerOrDefaultOwner {
    _setName(name_);
  }

  function setSymbol(string memory symbol_) external onlyOwnerOrDefaultOwner {
    _setSymbol(symbol_);
  }
   ...
   */
  function setDefaultRoyalty(address receiver, uint96 feeNumerator) external onlyOwnerOrDefaultOwner {
   ...
   *
   */
  function deleteDefaultRoyalty() external virtual onlyOwnerOrDefaultOwner {
  ...
  function setTokenRoyalty(
    ...
    address receiver,
    uint96 feeNumerator
  ) external virtual onlyOwnerOrDefaultOwner {
   ...
   *
   */
  function resetTokenRoyalty(uint256 tokenId) external virtual onlyOwnerOrDefaultOwner {
  ...
  function safeMint(
    ...
    uint32 quantity,
    string memory uri
  ) public onlyOwnerOrDefaultOwner {
  ...
  function setBaseUriBacktrackLength(uint16 baseUriBacktrackLength) external onlyOwnerOrDefaultOwner {
  ...
  function setTokensURIMap(
    ...
    string memory uri,
    uint64 startIndex
  ) external onlyOwnerOrDefaultOwner {
 ...
 *
 * Assumes that the maximum token id cannot exceed 2**64 - 1 (max value of uint64).
 */
  ...
  IERC721ConsecutiveTransfer
{
  ...
  // The tokenId of the next token to be minted. actually it has type uint64
  ...
  function _mint(
    ...
    require(startTokenId + quantity < type(uint64).max, "Exceeds uint64 max");
