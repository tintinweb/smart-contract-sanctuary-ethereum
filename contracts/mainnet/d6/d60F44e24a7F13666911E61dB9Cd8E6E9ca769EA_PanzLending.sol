...
library Utils {
    ...
    function verifyBorrowSignature(address _signer, BorrowData calldata _borrow, address _tokenContract, bytes20 tokenIds) internal view returns (bool) {
        ...
        return SignatureChecker.isValidSignatureNow(_signer, ECDSA.toEthSignedMessageHash(message), _borrow.signature);
    }
...
// File: contracts/PanzLending.sol
...
contract PanzLending is AccessControl, ConfirmedOwner, ReentrancyGuard {
     ...
     */
    function addOffer(uint32 _offerId, address _tokenContract, uint32 _amount, uint8 _count) external payable returns (uint32) {
        ...
        offers[offerId] = OfferData(msg.sender, collectionId, _amount, _count, _count);
    ...
    function borrow(BorrowData calldata _borrow, address _tokenContract, uint16[] calldata _tokenIds) external nonReentrant returns (uint32) {
        ...
        loans[++config.loanId] = LoanData(msg.sender, _borrow.offerId, _borrow.loanAmount, 
                                          ...
                                          _borrow.durationDays, LoanStatus.ACTIVE, uint8(tokenLen), packed);
    ...
    function repay(uint32 _loanId, address _tokenContract) external payable nonReentrant {
        ...
        if(loanData.offerId == 0) revert InvalidParams();
        // wrong loan status
        if(loanData.status != LoanStatus.ACTIVE) revert IllegalState();
        // caller is not the borrower
        if(loanData.borrower != msg.sender) revert PermissionDenied();
        ...
        {
            (bool sent,) = payable(address(offerData.owner)).call{value: repayWei - fee}("");
