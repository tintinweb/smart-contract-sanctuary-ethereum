    ...
    ReentrancyGuard
{
    ...
    uint256 public maxSupply = 10000; //0 - 9999
    uint256 public mintPriceETH = 0.05 ether;
    bool public saleActive = true;
    uint256 public maxLegendaryUsage = 1;

    mapping(uint256 => uint256) public legendaryID;

    LLContract private immutable LLaddr;
    NFT2PrintLegendaryPass private immutable legendPass;

    
    constructor(string memory _initBaseURI, address LLAddress, address PassAddress)
        ERC721("MetaLions3D", "ML3D")
    {
        setBaseURI(_initBaseURI);
        LLaddr = LLContract(LLAddress);
    ...
    // public
    function mintEth(uint256[] memory _tokenIds) public payable nonReentrant {

        uint256 totalTokenIds = _tokenIds.length;
        uint256 supply = totalSupply();

        require(supply + totalTokenIds < maxSupply, "Can not mint more than max supply.");
        require(saleActive, "Sale is disabled");
        require(msg.value >= mintPriceETH * totalTokenIds, "Ether value sent is not correct");        
        
        for (uint256 i = 0; i < totalTokenIds; i++) {
            require(!_exists(_tokenIds[i]), "MetaLion already minted");
            require(LLaddr.ownerOf(_tokenIds[i]) == msg.sender, "You must own the orignal Lazy Lion NFT");       
            _safeMint(msg.sender, _tokenIds[i]);
        }
       
    }

      function mintLP(uint256[] memory _tokenIds, uint256 _legendaryPassId) public nonReentrant {
        ...
        require(legendaryID[_legendaryPassId] + totalTokenIds <= maxLegendaryUsage, "Out of free Legendary pass mints");
        ...
        for (uint256 i = 0; i < totalTokenIds; i++) {
            ...
            _safeMint(msg.sender, _tokenIds[i]);
            legendaryID[_legendaryPassId]++;
        }

    }

    function mintERC20(uint256[] memory _tokenIds, uint256 _pid) public nonReentrant {
        ...
        returns (string memory)
    {
        require(
            _exists(tokenId),
            "ERC721Metadata: URI query for nonexistent MetaLion 3D"
        );
    ...
    function setmintPriceEth(uint256 newPrice) public onlyOwner {
        mintPriceETH = newPrice;
    }

    function mintFor(address _toAddress, uint256[] memory _tokenIds) public onlyOwner {       
        uint256 supply = totalSupply();
        uint256 totalTokenIds = _tokenIds.length;
        ...
        for (uint256 i = 0; i < totalTokenIds; i++) {
            require(!_exists(_tokenIds[i]), "MetaLion already minted");
            require(LLaddr.ownerOf(_tokenIds[i]) == _toAddress, "Address must own the orignal Lazy Lion NFT");       
            _safeMint(msg.sender, _tokenIds[i]);
        }
    }
    ...
    function setLegendaryMaxUsage(uint256 _usage) external onlyOwner {
