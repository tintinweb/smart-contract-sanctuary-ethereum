...
Ritual0x will be our own ritual for surviving in the crypto space and the way it works is that all our top holders can partake in our daily/weekly rituals. We have many deep connections in the space and we get to know a lot of upcoming SAFU launches. Together, with our holders, we will be performing rituals and aping into new launches. We will then be proceeding to entering new tokens telegram channels, hyping them and raiding in order to pump the tokens so we can all earn.

Tax will also be used as a fund to ape into SAFU launches at launch and distribute profits back to our holders based on hold percentages.

Max Tx 200 (2%)

Suggested Slippage 8%

Telegram http://t.me/ritual0x
**/
...
contract Ritual0x is Context, IERC20, Ownable {
    ...
    uint256 private constant _tTotal = 10_000 * 10**8;
    ...
    string private constant _name = "0xRitual";
    string private constant _symbol = "0xR";
    ...
    uint256 public _maxTxAmount = 200 * 10**8;
    uint256 public _maxWalletSize = 200 * 10**8;
