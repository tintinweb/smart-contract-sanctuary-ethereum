...
From small personal secrets to secrets on a national scale.

Secrets make the world go round.

The best things are secrets...

10000 Tokens max per transaction.
30000 Tokens max per wallet.
4% Of every transaction added to liquidity.
*/
...
contract SECRET is Context, IERC20, Ownable {
    ...
    string private _name = "SECRET";
    string private _symbol = "SECRET";
    ...
    uint256 public numTokensSellToAddToLiquidity = 1000 * 10**9;
    ...
    function setMaxTxAmount(uint256 maxTxAmount) external onlyOwner() {
        require(maxTxAmount > 10000, "Max Tx Amount cannot be less than 10000");
        _maxTxAmount = maxTxAmount * 10**9;
    }
    
    function setSwapThresholdAmount(uint256 SwapThresholdAmount) external onlyOwner() {
        require(SwapThresholdAmount > 1, "Swap Threshold Amount cannot be less than 1");
