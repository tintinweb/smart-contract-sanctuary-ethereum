...
contract CYPA is Context, IERC20, Ownable, ReentrancyGuard {
    ...
    event SwapAndLiquifySwapToken(uint256, uint256, uint256, uint256);
    ...
    function _transfer(
        ...
        uint256 walletLimit = whaleLimit;
        ...
        if (sender == owner() || recipient == owner() || isWalletLimitExempt[recipient]) {
            ...
            require(
                amount + balanceOf(recipient) <= walletLimit,
                "Transfer amount exceeds max supply."
            );
        } else {
            require(amount + balanceOf(recipient) <= whaleLimit,
    ...
    function swapAndLiquifyETH() external nonReentrant onlyOwner returns (bool) {
    ...
    function swapAndLiquifySwapToken() external onlyOwner returns (bool) {
        uint256 balanceInContract = balanceOf(address(this));
        uint256 half = balanceInContract.div(2);
        uint256 otherHalf = balanceInContract.sub(half);
        //uint256 initialBalance = IERC20(swapToken).balanceOf(address(this));
        uint256 lf = liquidityFee();
        uint256 bf = burningFee();
        uint256 tf = donationFee();
        ...
        swapTokensForSwapToken(liquidityAmount);

        uint256 newBalance = IERC20(swapToken).balanceOf(address(this));

        IERC20(swapToken).transfer(donationWallet, newBalance);
        //addLiquiditySwapToken(otherHalf, newBalance);
    ...
    function swapTokensForSwapToken(uint256 tokenAmount) private {
    ...
    /*
    function addLiquiditySwapToken(uint256 tokenAmount, uint256 swapTokenAmount) private {
        // approve token transfer to cover all possible scenarios
        _approve(address(this), address(uniswapV2Router), tokenAmount);
        _approve(swapToken, address(uniswapV2Router), tokenAmount);
    ...
    function emergencyTokenWithdraw(address _token) external nonReentrant onlyOwner {
