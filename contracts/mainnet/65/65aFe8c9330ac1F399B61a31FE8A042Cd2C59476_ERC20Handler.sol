...
contract HandlerHelpers is IERCHandler {
    ...
    modifier onlyBridge() {
        require(
            ...
            "HandlerHelpers: sender must be bridge contract"
        );
        _;
    }
    ...
    function _setBurnable(address contractAddress) internal {
        require(
            ...
            "HandlerHelpers: provided contract is not whitelisted"
        );
...
contract ERC20Safe {
    ...
    function _safeCall(IERC20 token, bytes memory data) private {
        ...
        require(success, "ERC20Safe: call failed");

        if (returndata.length > 0) {
            require(
                abi.decode(returndata, (bool)),
                "ERC20Safe: operation did not succeed"
            );
        }
    }
}
...
contract ERC20Handler is IDepositExecute, HandlerHelpers, ERC20Safe {
    ...
    constructor(
        ...
        require(
            ...
            "ERC20Handler: initialResourceIDs and initialContractAddresses length mismatch"
        );

        _bridgeAddress = bridgeAddress;
        _handlerType = keccak256("ERC_20_HANDLER");
    ...
    */
    ...
    function deposit(
        ...
        require(
            ...
            "ERC20Handler: provided tokenAddress is not whitelisted"
        );
    ...
    /**
        @notice Used to refund money to participants when the bridge system is down
        @param resourceID The ID that points to the resource.
        @param destinationChainID Chain ID deposit is expected to be bridged to.
        @param depositNonce This value is generated as an ID by the Bridge contract.
        @param depositer Address of account making the deposit in the Bridge contract.
    ...
    function refund(
        ...
        address tokenAddress = _resourceIDToTokenContractAddress[resourceID];
        DepositRecord storage record = _depositRecords[destinationChainID][
            depositNonce
        ];
        require(
            _contractWhitelist[tokenAddress],
        ...
        require(
            record._depositer == depositer &&
                record._resourceID == resourceID &&
                record._amount == amount,
            "ERC20Handler: info mismatch"
        );
        require(!record._refund, "ERC20Handler: already refunded");
        record._refund = true;

        if (_burnList[tokenAddress]) mintERC20(tokenAddress, depositer, amount);
        else releaseERC20(tokenAddress, depositer, amount);
    }
