...
// File: contracts/Goblinversewtf.sol
...
contract Goblinversewtf is ERC721A, Ownable, ReentrancyGuard {
  ...
  ) ERC721A("Goblinverse.wtf", "GBLNVRS") {
  ...
  function mint(uint256 tokens) public payable nonReentrant {
    require(!paused, "Oops contract goblin is paused");
    uint256 supply = totalSupply();
    require(tokens > 0, "Need to mint at least 1 Goblin");
    require(tokens <= MaxperWallet, "Max mint amount per tx exceeded Ratz");
    require(supply + tokens <= maxSupply, "We Soldout");
    require(_numberMinted(_msgSender()) + tokens <= MaxperWallet, "Max Goblin Per Wallet exceeded");
    ...
    require(!paused, "oops contract goblin is paused");
    uint256 supply = totalSupply();
    require(_numberMinted(_msgSender()) + tokens <= MaxperWalletFree, "Max Goblin  Per Wallet exceeded");
    require(tokens > 0, "need to mint at least 1 Goblin");
    require(tokens <= MaxperWalletFree, "max mint per Tx exceeded");
    require(supply + tokens <= FreeSupply, "Free Goblin exceeded");
