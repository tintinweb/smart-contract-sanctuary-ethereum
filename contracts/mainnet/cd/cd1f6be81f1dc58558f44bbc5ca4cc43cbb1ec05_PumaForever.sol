...
/**
PUMA Nitro Collection Smart Contract Development by @WumboLabs
**/
...
contract PumaForever is ERC721A, Ownable {
    event NewPumaForeverMinted(address sender, uint256 tokenId);
    ...
    /// Minting Variables
    string public baseURI = "https://puma-blackbox-prod.herokuapp.com/api/forever/pass/";
    Status public state;
    uint256 public mintCount;

    constructor() ERC721A("PUMA Nitro Collection", "NITRO") {
    }
    ...
    function mint(address _address, uint256 _quantity) external {
        require(msg.sender == NitroPassContract, "PUMA Nitro Collection: Can only mint by burning from Nitropass contract");
        require(state == Status.PASS_MINT, "PUMA Nitro Collection: Mint Is Not Live");
        for (uint256 i = 1; i <= _quantity; i++) {
            emit NewPumaForeverMinted(msg.sender, totalSupply() + i);
        }
