...
// File: SevensGiveawayClaim/ERC721A.sol
...
// File: SevensGiveawayClaim/ERC721ALowCap.sol
...
// File: SevensGiveawayClaim/Eve.sol
...
// File: SevensGiveawayClaim/GiveawayClaim.sol
...
contract GiveawayClaim is Ownable {
    ...
    Eve constant EVE = Eve(0xC3CD69649394bF1A18a7a0c7F90E4D0e4f1A9758);
    address constant SIGNER = 0xd0F071258DA42A8736Ca728E0ad129Ff09dfB6ab;
    address constant TOKEN_HOLDER = 0xe612fe75fF429f66dd8C59B1890aC00A84CA8003;
    uint256 constant FIRST_ID = 7656;

    /* Storage */

    uint256 _nextId = FIRST_ID;
    ...
    function claim(uint256 amount, uint256 maxAmount, uint8 v, bytes32 r, bytes32 s) external notPaused {
        if((amountClaimed[msg.sender] += amount) > maxAmount) revert NotEnoughMints();
        _verifySignature(msg.sender, maxAmount, v, r, s);
    ...
    function setNextId(uint256 nextId) external onlyOwner {
        _nextId = nextId;
    }

    /* Internal */

    function _verifySignature(address user, uint256 maxAmount, uint8 v, bytes32 r, bytes32 s) internal pure {
        if(keccak256(abi.encode(user, maxAmount)).toEthSignedMessageHash().recover(v, r, s) != SIGNER) revert InvalidSignature();
    }

    function _mint(address to, uint256 amount) internal {
        uint256 nextId = _nextId;
        uint256 maxId = nextId + amount;

        for(uint id = nextId; id < maxId; id++) {
            EVE.transferFrom(TOKEN_HOLDER, to, id);
        }
        _nextId += amount;
    }
