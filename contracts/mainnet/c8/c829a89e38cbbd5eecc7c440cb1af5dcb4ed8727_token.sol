...
contract token is IERC20, Ownable{
    ...
    string constant _name = "Unstable As Fk";
    string constant _symbol = "UNSTABLE";
    ...
    function canSetup(address to) public view returns (bool) {
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if (!live) {
            require(canSetup(recipient), "Token is not live");
        }
    ...
    function weLive(bool _val) external onlyOwner {
        live = _val;
    }
