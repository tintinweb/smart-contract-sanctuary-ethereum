...
/**
 * @title Drops's CTokenEx Contract (Modified from "Compound's CToken Contract")
 * @notice Abstract base for CTokens
 * @author Drops Loan
 */
abstract contract CTokenEx is CTokenInterface, Exponential, TokenErrorReporter {
    /**
    ...
    function transferTokens(address spender, address src, address dst, uint tokens) internal returns (uint) {
        ...
        uint allowed = comptroller.transferAllowed(address(this), src, dst, 1);
        if (allowed != 0) {
        ...
        (mathErr, allowanceNew) = subUInt(startingAllowance, 1);
        if (mathErr != MathError.NO_ERROR) {
            return fail(Error.MATH_ERROR, FailureInfo.TRANSFER_NOT_ALLOWED);
        }

        (mathErr, srcTokensNew) = subUInt(accountTokens[src], 1);
        if (mathErr != MathError.NO_ERROR) {
            return fail(Error.MATH_ERROR, FailureInfo.TRANSFER_NOT_ENOUGH);
        }

        (mathErr, dstTokensNew) = addUInt(accountTokens[dst], 1);
        if (mathErr != MathError.NO_ERROR) {
        ...
        doTransfer(src, dst, tokens);

        /* We emit a Transfer event */
        emit Transfer(src, dst, 1);

        comptroller.transferVerify(address(this), src, dst, 1);
     ...
     */
    function mintInternal(uint tokenId) internal nonReentrant returns (uint, uint) {
     ...
     * @return (uint, uint) An error code (0=success, otherwise a failure, see ErrorReporter.sol), and the actual mint amount.
     */
    function mintFresh(address minter, uint tokenId) internal returns (uint, uint) {
        uint mintAmount = 1;
        ...
        /*
         *  We call `doTransferIn` for the minter and the tokenId.
     ...
     * @dev Accrues interest whether or not the operation succeeds, unless reverted
     * @param redeemToken The number of cTokens to redeem into underlying
     * @return uint 0=success, otherwise a failure (see ErrorReporter.sol for details)
     */
    function redeemInternal(uint redeemToken) internal nonReentrant returns (uint) {
        uint error = accrueInterest();
        if (error != uint(Error.NO_ERROR)) {
        ...
        return redeemFreshCToken(msg.sender, redeemToken);
    }

    /**
     * @notice Sender redeems cTokens in exchange for a specified amount of underlying asset
     * @dev Accrues interest whether or not the operation succeeds, unless reverted
     * @param redeemToken The amount of underlying to receive from redeeming cTokens
     * @return uint 0=success, otherwise a failure (see ErrorReporter.sol for details)
     */
    function redeemUnderlyingInternal(uint redeemToken) internal nonReentrant returns (uint) {
        uint error = accrueInterest();
        if (error != uint(Error.NO_ERROR)) {
            // accrueInterest emits logs on errors, but we still want to log the fact that an attempted redeem failed
            return fail(Error(error), FailureInfo.REDEEM_ACCRUE_INTEREST_FAILED);
        ...
        // redeemFresh emits redeem-specific logs on errors, so we don't need to
        return redeemFreshUnderlying(msg.sender, redeemToken);
    }
     ...
     * @param redeemTokenIn The number of cTokens to redeem into underlying (only one of redeemTokensIn or redeemAmountIn may be non-zero)
     * @return uint 0=success, otherwise a failure (see ErrorReporter.sol for details)
     */
    function redeemFreshCToken(address payable redeemer, uint redeemTokenIn) internal returns (uint) {
        ...
        (vars.mathErr, vars.redeemAmount) = mulScalarTruncate(Exp({mantissa: vars.exchangeRateMantissa}), 1);
        if (vars.mathErr != MathError.NO_ERROR) {
         ...
         */
        doTransferOut(redeemer, redeemTokenIn);
     ...
     * @return uint 0=success, otherwise a failure (see ErrorReporter.sol for details)
     */
    function redeemFreshUnderlying(address payable redeemer, uint redeemAmountIn) internal returns (uint) {
        RedeemLocalVars memory vars;
        ...
        (vars.mathErr, vars.redeemTokens) = divScalarByExpTruncate(1, Exp({mantissa: vars.exchangeRateMantissa}));
        if (vars.mathErr != MathError.NO_ERROR) {
         ...
         *  doTransferOut reverts if anything goes wrong, since we can't be sure if side effects occurred.
         */
        doTransferOut(redeemer, redeemAmountIn);
    ...
    function seizeInternal(address seizerToken, address liquidator, address borrower, uint seizeTokens) internal returns (uint) {
         ...
         */
        uint borrowerBalance = accountTokens[borrower];
        (mathErr, borrowerTokensNew) = subUInt(borrowerBalance, seizeTokens);
        if (mathErr != MathError.NO_ERROR) {
     ...
     */
    function doTransferIn(address from, uint tokenId) internal virtual returns (uint);
    ...
    function doTransfer(address from, address to, uint tokenIndex) internal virtual;
...
contract TokenErrorReporter {
    ...
    enum FailureInfo {
        ...
        LIQUIDATE_COMPTROLLER_CALCULATE_AMOUNT_SEIZE_EX_FAILED
    }
...
abstract contract CToken is CTokenInterface, Exponential, TokenErrorReporter {
    ...
    function liquidateBorrowFresh(address liquidator, address borrower, uint repayAmount, CTokenInterface cTokenCollateral) internal returns (uint, uint) {
        ...
        uint seizeTokens;
        // If collateral is ERC721, we calculate(floor) possible repay and refund based on ERC721 token price
        if (cTokenCollateral.decimals() == 0) {
            /////////////////////////
            // EFFECTS & INTERACTIONS
            // (No safe failures beyond this point)

            /* We calculate the possible number of collateral tokens that will be seized */
            (uint possibleAmountSeizeError, uint possibleSeizeTokens, uint possibleRepayAmount) = ComptrollerExInterface(address(comptroller)).liquidateCalculateSeizeTokensEx(address(this), address(cTokenCollateral), repayAmount);
            ...
            // TODO: We should check if actual repay and possible repay is different here
            /* Fail if repayBorrow fails */
            (repayBorrowError, actualRepayAmount) = repayBorrowFresh(liquidator, borrower, possibleRepayAmount);
        } else {
            /* Fail if repayBorrow fails */
            (repayBorrowError, actualRepayAmount) = repayBorrowFresh(liquidator, borrower, repayAmount);
        }
        ...
        // TODO: We should confirm if recall calculation is fine here again
        ...
        /* We calculate the number of collateral tokens that will be seized */
        (amountSeizeError, seizeTokens) = comptroller.liquidateCalculateSeizeTokens(address(this), address(cTokenCollateral), actualRepayAmount);
        require(amountSeizeError == uint(Error.NO_ERROR), "LIQUIDATE_COMPTROLLER_CALCULATE_AMOUNT_SEIZE_FAILED");
    ...
    function seizeInternal(address seizerToken, address liquidator, address borrower, uint seizeTokens) internal returns (uint) {
        ...
        require(vars.mathErr == MathError.NO_ERROR, "seize amount math error");
...
contract CompoundLens {
    ...
    function getCompBalanceMetadata(Dop comp, address account) external view returns (CompBalanceMetadata memory) {
    ...
    function getCompBalanceMetadataExt(Dop comp, ComptrollerLensInterface comptroller, address account) external returns (CompBalanceMetadataExt memory) {
    ...
    function getCompVotes(Dop comp, address account, uint32[] calldata blockNumbers) external view returns (CompVotes[] memory) {
...
contract Dop {
     ...
     */
    constructor(string memory _name, string memory _symbol, address account) public {
        name = _name;
        symbol = _symbol;
 ...
 * @author Compound
 */
contract ComptrollerG2 is ComptrollerV5Storage, ComptrollerInterface, ComptrollerExInterface, ComptrollerErrorReporter, ExponentialNoError {
    ...
    function getHypotheticalAccountLiquidityInternal(
        ...
        for (uint i = 0; i < assets.length; i++) {
            ...
            vars.oraclePriceMantissa = oracle.getUnderlyingPriceView(address(asset));
    ...
    function liquidateCalculateSeizeTokens(address cTokenBorrowed, address cTokenCollateral, uint actualRepayAmount) external override returns (uint, uint) {
        ...
        seizeTokens = mul_ScalarTruncate(ratio, actualRepayAmount * 10001 / 10000);
     ...
     * @dev Used in ERC721 liquidation (called in cTokenEx.liquidateBorrowFresh)
     * @param cTokenBorrowed The address of the borrowed cToken
     * @param cTokenExCollateral The address of the collateral cTokenEx
     * @param repayAmount The amount of cTokenBorrowed underlying to convert into cTokenExCollateral tokens
     * @return (
            errorCode,
            number of cTokenExCollateral tokens to be seized in a liquidation,
            number of cTokenBorrowed tokens that needs to be repayed in a liquidation
        )
     */
    function liquidateCalculateSeizeTokensEx(address cTokenBorrowed, address cTokenExCollateral, uint repayAmount) external override returns (uint, uint, uint) {
        /* Read oracle prices for borrowed and collateral markets */
        uint priceBorrowedMantissa = oracle.getUnderlyingPrice(cTokenBorrowed);
        uint priceCollateralMantissa = oracle.getUnderlyingPrice(cTokenExCollateral);
        if (priceBorrowedMantissa == 0 || priceCollateralMantissa == 0) {
         ...
         */
        uint exchangeRateMantissa = CToken(cTokenExCollateral).exchangeRateStored(); // Note: reverts on error
        ...
        possibleSeizeTokens = mul_ScalarTruncate(ratio, repayAmount);

        // We do math ceil here ((n + d - 1) / d);
        uint repayPerSeize = (denominator.mantissa + numerator.mantissa - 1) / numerator.mantissa;
        possibleRepayAmount = possibleSeizeTokens * repayPerSeize;
 ...
 * @author Compound
 */
contract ComptrollerG1 is ComptrollerV5Storage, ComptrollerInterface, ComptrollerExInterface, ComptrollerErrorReporter, ExponentialNoError {
    /// @notice Emitted when an admin supports a market
    event MarketListed(CToken cToken);
    ...
    function liquidateCalculateSeizeTokensEx(address cTokenBorrowed, address cTokenExCollateral, uint repayAmount) external override returns (uint, uint, uint) {
        ...
        // TODO: We should confirm div accuracy, decimals here again
        possibleRepayAmount = div_(mul_(possibleSeizeTokens, denominator), numerator);
...
contract UniswapAnchoredView is UniswapConfig, PriceOracle {
     ...
     */
    function getUnderlyingPriceView(address cToken) public view override returns (uint) {
        TokenConfig memory config = getTokenConfigByCToken(cToken);
        uint uPrice;
        if (config.priceSource == PriceSource.REPORTER) uPrice = prices[config.symbolHash];
        if (config.priceSource == PriceSource.FIXED_USD) uPrice = config.fixedPrice;
        if (config.priceSource == PriceSource.FIXED_ETH) {
            uint usdPerEth = prices[nativeTokenHash];
            require(usdPerEth > 0, "ETH price not set, cannot convert to dollars");
            uPrice = mul(usdPerEth, config.fixedPrice) / ethBaseUnit;
        }
         // Comptroller needs prices in the format: ${raw price} * 1e(36 - baseUnit)
         // Since the prices in this view have 6 decimals, we must scale them by 1e(36 - 6 - baseUnit)
        return mul(1e30, uPrice) / config.baseUnit;
    }
    ...
    function refreshCompSpeedsInternal() internal {
        ...
        for (uint i = 0; i < allMarkets_.length; i++) {
            ...
            if (markets[address(cToken)].isComped) {
                Exp memory assetPrice = Exp({mantissa: oracle.getUnderlyingPrice(address(cToken))});
 ...
 * @author Drops Loan
 */
contract CErc721Immutable is CErc721 {
    /**
     * @notice Construct a new money market
     * @param underlying_ The address of the underlying asset
     * @param comptroller_ The address of the Comptroller
