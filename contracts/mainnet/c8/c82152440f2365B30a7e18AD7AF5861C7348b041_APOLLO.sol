...
contract APOLLO is ERC721AQueryable, Ownable {
  using Strings for uint256;

  uint256 public constant TOTAL_MAX_SUPPLY = 4000;
  uint256 public totalFreeMints = 1000;
  uint256 public maxFreeMintPerWallet = 1;
  uint256 public maxPublicMintPerWallet = 20;
  uint256 public publicTokenPrice = .0025 ether;
  string _contractURI;
  ...
  constructor() ERC721A("APOLLO", "APOLLO") {}

  modifier callerIsUser() {
    require(tx.origin == msg.sender, 'APOLLO: The caller is another contract');
    _;
  }
  ...
  modifier underMaxSupply(uint256 _quantity) {
    require(
      ...
      "APOLLO: Purchase exceeds max supply"
    );
    _;
  }

  function mint(uint256 _quantity) external payable callerIsUser underMaxSupply(_quantity) {
    require(balanceOf(msg.sender) < maxPublicMintPerWallet, "APOLLO: Mint limit exceeded");
    require(saleStarted, "APOLLO: Sale has not begun");
    if (_totalMinted() < (TOTAL_MAX_SUPPLY)) {
      if (freeMintCount >= totalFreeMints) {
        require(msg.value >= _quantity * publicTokenPrice, "APOLLO: More ETH required to save one of the survivors!");
        ...
        if (_quantity <= _mintableFreeQuantity) {
          ...
          require(
            ...
            "APOLLO: Insufficient ETH"
          );
        }
        _mint(msg.sender, _quantity);
      } else {
        require(msg.value >= (_quantity * publicTokenPrice), "APOLLO: Not enough ETH to save one of the survivors");
        _mint(msg.sender, _quantity);
      }
    }
  }
  ...
  function withdrawFunds() external onlyOwner {
    ...
    require(success, "APOLLO: Transfer failed.");
  }
