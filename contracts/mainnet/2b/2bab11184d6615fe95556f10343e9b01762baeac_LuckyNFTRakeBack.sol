...
// File contracts/LuckyNFTRakeBack.sol
...
contract LuckyNFTRakeBack is Ownable, EIP712, ReentrancyGuard {
    ...
    mapping(uint => uint) public channelMaxMintNFT;
    mapping(uint => uint) public channelTotalMintNFT;
    ...
    function setChannelMaxMintNFT(
        uint[] calldata channels,
        uint[] calldata maxs
    )
    external
    onlyOwner
    {
        require(channels.length == maxs.length, "array length dont match");
        for (uint i = 0; i < channels.length; i++) {
            channelMaxMintNFT[channels[i]] = maxs[i];
        }
    }
    ...
    {
        require(open, "not open");
        require(channelTotalMintNFT[channelId] + quantity <= channelMaxMintNFT[channelId], "over channel max minting limit");
        require(nonce == (nftNonce[to][channelId] + 1), "invalid nonce");
        bytes32 digest = _hashTypedDataV4(keccak256(abi.encode(
        ...
        channelTotalMintNFT[channelId] += quantity;
