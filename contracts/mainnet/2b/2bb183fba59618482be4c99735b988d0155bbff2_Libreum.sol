...
contract Libreum is ERC20, Ownable {
    ...
    uint256 private originalSellVaultFee;
    ...
    event UpdatedVaultAddress(address indexed newWallet);
    ...
    constructor() payable ERC20("Libreum", "LIBRA") {
        ...
        uint256 totalSupply = 99999999 * 1e18; 
        ...
        buyLiquidityFee = 1;
        buyVaultFee = 2;
        buyTotalFees = buyOperationsFee + buyLiquidityFee + buyVaultFee;

        originalSellOperationsFee = 1;
        originalSellLiquidityFee = 1;
        originalSellVaultFee = 2;

        sellOperationsFee = 1;
        sellLiquidityFee = 1;
        sellVaultFee = 2;
        sellTotalFees = sellOperationsFee + sellLiquidityFee + sellVaultFee;

        operationsAddress = address(0xf0357f153cB9966968c127192BaeCb127A66C41B);
        vaultAddress = address(0x9fcf92F2178F1A5e7c37933bd3b468ec6ed57513);

        _excludeFromMaxTransaction(newOwner, true);
        _excludeFromMaxTransaction(address(this), true);
        _excludeFromMaxTransaction(address(0xdead), true);
        _excludeFromMaxTransaction(address(operationsAddress), true);
        _excludeFromMaxTransaction(address(vaultAddress), true);

        excludeFromFees(newOwner, true);
        excludeFromFees(address(this), true);
        excludeFromFees(address(0xdead), true);
        excludeFromFees(address(operationsAddress), true);
        excludeFromFees(address(vaultAddress), true);

        _createInitialSupply(address(0xdead), (totalSupply * 10) / 100);
    ...
    function swapBack() private {
        ...
        uint256 ethForStaking = (ethBalance * tokensForVault) /
            (totalTokensToSwap - (tokensForLiquidity / 2));
        ...
        (success, ) = address(vaultAddress).call{value: ethForStaking}("");
    ...
    function setVaultAddress(address _vaultAddress) external onlyOwner {
        ...
        emit UpdatedVaultAddress(_vaultAddress);
    }
