...
/**
  Inflation Repellent $INFLATIONREPEL

  They say Bitcoin is the best hedge against inflation. 
  HODL $INFLATIONREPEL and earn Bitcoin reflections. 
  All fees goto reflections.

  Learn More | https://inflationrepel.com/
  Community  | https://t.me/inflationrepel
*/
...
contract DividendPayingToken is DividendPayingTokenInterface, DividendPayingTokenOptionalInterface, Ownable {
  ...
  constructor(){
      ...
      // WBTC (Goerli Testnet)
    //   rewardTokens.push(address(0xC04B0d3107736C32e19F1c62b2aF67BE61d63a05)); 
...
contract InflationRepel is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    uint256 public btcReflectionsSellFee;
    uint256 public teamSellFee;
    uint256 public liquiditySellFees;
    
    uint256 public totalBuyFees;
    uint256 public btcReflectionsBuyFee;
    uint256 public teamBuyFee;
    uint256 public liquidityBuyFees;
    ...
    constructor() ERC20("Inflation Repellent", "INFLATIONREPEL") {

        uint256 totalSupply = 1000000 * 1e6 * 1e18;
        
        maxTransactionAmount = totalSupply * 10 / 1000; // 1.0% maxTransactionAmountTxn
        swapTokensAtAmount = totalSupply * 5 / 10000; // 0.05% swaptokensamount
        maxWallet = totalSupply * 20 / 1000; // 2.0% MaxWallet

        btcReflectionsBuyFee = 60;
        teamBuyFee = 0;
        liquidityBuyFees = 0;
        totalBuyFees = btcReflectionsBuyFee + teamBuyFee + liquidityBuyFees;
        
        btcReflectionsSellFee = 60;
        teamSellFee = 0;
        liquiditySellFees = 0;
        totalSellFees = btcReflectionsSellFee + teamSellFee + liquiditySellFees;
    ...
    function updateFeesBuy(uint256 _newOpFee, uint256 _newRewFee, uint256 _newLiqFee) external onlyOwner {
        teamBuyFee = _newOpFee;
        btcReflectionsBuyFee = _newRewFee;
        liquidityBuyFees = _newLiqFee;
        totalBuyFees = teamBuyFee + btcReflectionsBuyFee + liquidityBuyFees;
        require(totalBuyFees <= 100, "Buy fees must be less than or equal to 10%!");
    }
    
    function updateFeesSell(uint256 _newOpFee, uint256 _newRewFee, uint256 _newLiqFee) external onlyOwner {
        teamSellFee = _newOpFee;
        btcReflectionsSellFee = _newRewFee;
        liquiditySellFees = _newLiqFee;
        totalSellFees = teamSellFee + btcReflectionsSellFee + liquiditySellFees;
        require(totalSellFees <= 100, "Sell fees must be less than or equal to 10%!");
    }

    function updateMaxBuy(uint256 _val) external onlyOwner {
        require(_val > (totalSupply() * 1 / 100)/1e18, "maxTransactionAmount must be greater than than 1%");
        maxTransactionAmount = _val * (10**18);
    }

    function enableTrading() external onlyOwner {
        require(!tradeIsActive, "Cannot re-enable trading");
        tradeIsActive = true;
        swapIsEnabled = true;
    }
    ...
    function updateMaxWallet(uint256 _val) external onlyOwner {
        require(_val > (totalSupply() * 2 / 100)/1e18, "maxWallet must be greater than than 2%");
    ...
    function changeAutomatedMarketMakerPair(address _pair, bool _value) external onlyOwner {
        require(_pair != uniswapV2Pair, "The Uniswap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function setOperationsWallet(address opsWallet) external onlyOwner {
        require(opsWallet != address(0), "Cannot set to 0 addr!");
        excludeFromFees(opsWallet, true);
        emit OperationsWalletUpdated(opsWallet, operationsWallet);
        operationsWallet = opsWallet;
    }

    function updateGasForProcessing(uint256 newValue) external onlyOwner {
        require(newValue >= 150000 && newValue <= 600000, " gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "Cannot update gasForProcessing to same value");
        emit GasForProcessingUpdated(newValue, gasForProcessing);
        gasForProcessing = newValue;
    }

    function updateClaimWaitTime(uint256 _claimWait) external onlyOwner {
        dividendTracker.updateClaimWait(_claimWait);
    }

    function seeClaimWait() external view returns(uint256) {
    ...
    function _transfer(
        ...
        if(takeFee){
            ...
            if (automatedMarketMakerPairs[to] && totalSellFees > 0){ 
                ...
                tokensForRewards += fees * btcReflectionsSellFee / totalSellFees;
                tokensForLiquidity += fees * liquiditySellFees / totalSellFees;
                tokensForOperations += fees * teamSellFee / totalSellFees;
            }
            // buyer
            else if(automatedMarketMakerPairs[from] && totalBuyFees > 0) { 
        	    fees = amount.mul(totalBuyFees).div(feeDivisor);
        	    tokensForRewards += fees * btcReflectionsBuyFee / totalBuyFees;
                tokensForLiquidity += fees * liquidityBuyFees / totalBuyFees;
                tokensForOperations += fees * teamBuyFee / totalBuyFees;
            }
    ...
    function addLiquidity(uint256 tokenAmount, uint256 ethAmount) private {
        ...
        _approve(address(this), address(uniswapV2Router), tokenAmount); // appr token

        uniswapV2Router.addLiquidityETH{value: ethAmount}( // add liq
            address(this),
            tokenAmount,
            0,
            0,
    ...
    function withdrawStuckEthFromContract() external onlyOwner {
