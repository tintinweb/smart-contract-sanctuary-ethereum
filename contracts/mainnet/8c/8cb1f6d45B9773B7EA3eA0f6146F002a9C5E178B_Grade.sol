...
contract Grade is Initializable, ERC20Upgradeable, ERC20BurnableUpgradeable, PausableUpgradeable, AccessControlUpgradeable, ERC20FlashMintUpgradeable {
    bytes32 public constant PAUSER_ROLE = keccak256("PAUSER_ROLE");
    bytes32 public constant MINTER_ROLE = keccak256("MINTER_ROLE");
    address private _feeCollector;

    function initialize(address feeCollector) initializer public {
        __ERC20_init("Grade", "GGC");
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal override {
        uint256 feeAmount = amount / 100; // calculate 1% fee
        super._transfer(sender, _feeCollector, feeAmount); // send fee to feeCollector
        super._transfer(sender, recipient, amount - feeAmount); // send the rest to recipient
    }
}