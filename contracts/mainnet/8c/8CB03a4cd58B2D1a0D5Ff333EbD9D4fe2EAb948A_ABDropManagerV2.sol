...
contract ABDropManagerV2 is Initializable, OwnableUpgradeable, ABErrors {
    ...
    /**
     * @notice
     *  Update the Drop `_dropId` price per unit
     *  Only the contract owner can perform this operation
     *
     * @param _dropId drop identifier of the drop to be updated
     * @param _price price for one unit
     */
    ...
    /**
     * @notice
     *  Update the Drop `_dropId` total supply
     *  Only the contract owner can perform this operation
     *
     * @param _dropId drop identifier of the drop to be updated
     * @param _supply total supply for the drop
     */
    function updateDropSupply(
        uint256 _dropId,
        uint256 _supply
    ) external onlyOwner {
        ...
        if (drop.tokenInfo.supply > _supply) revert UnauthorizedUpdate();

        // Update the supply info
        drop.tokenInfo.supply = _supply;
    ...
    /**
     * @notice
     *  Update the Drop `_dropId` royalty share per token
     *  Only the contract owner can perform this operation
     *
     * @param _dropId drop identifier of the drop to be updated
     * @param _share amount of royalty share attached to the token
     */
    function updateDropRoyaltyShare(
        uint256 _dropId,
        uint256 _share
    ) external onlyOwner {
        if (_share <= 0) revert InsufficientRoyalties();

        // Get the drop to be updated
        Drop storage drop = drops[_dropId];

        // Update the royalty share info
        drop.tokenInfo.royaltySharePerToken = _share;
...
contract ABErrors {
    ...
    **/
    ...
    /**
    @notice Error returned if the provided gas limit is not correct
    **/
    error IncorrectGasLimit();
}
