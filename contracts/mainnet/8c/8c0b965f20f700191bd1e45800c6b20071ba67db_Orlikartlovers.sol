...
abstract contract Ownable is Context {
    ...
    /**
     * @dev Leaves the contract without owner. It 0xEdDB36BA4014A012CDf4724cBCD8064D503005D4will not be possible to call
...
contract Orlikartlovers is DefaultOperatorFilterer, IERC721, IERC2981, Ownable, Functional {
     ...
     */
    constructor() {
        _name = "Orlik Art Lovers";
        _symbol = "OAL";
        _baseURI = "https://orlikartlovers.art/metadata/json";
        _hideTokens = false;
        
        totalTokens = 30;
        price = 100 * (10 ** 15); // Replace leading value with price in finney
		
        royaltyReciever = address(0x5944643B497226992B19797c09F437f3250FDbA1);
        maxPerWallet = 2;
    }
    ...
    function supportsInterface(bytes4 interfaceId) public view virtual override returns (bool) {
        return  interfaceId == type(IERC721).interfaceId ||
                ...
                interfaceId == Orlikartlovers.onERC721Received.selector;
    }

    function royaltyInfo( uint256 tokenId_, uint256 salePrice_ ) external override view returns ( address receiver, uint256 royaltyAmount ){
        uint256 royaltyAmt = salePrice_ / 10;

        return (royaltyReciever, royaltyAmt);
    }

    function setRoyaltyWallet( address newWallet ) external onlyOwner {
        royaltyReciever = newWallet;
    }
    ...
    function airDrop(address _to, uint256 tokenId) external onlyOwner {
        require((numberMinted + 1) > numberMinted, "Math overflow error");
    ...
    function mint(uint256 qty) external payable reentryLock {
        ...
        require((qty + trackingValue) <= totalTokens, "Mint: Not enough avaialability");
        require((_mintTracker[_to] + qty) <= maxPerWallet, "Mint: Max tkn per wallet exceeded");
        require(mintActive, "Mint: Not Open");
        
        uint256 mintSeedValue = trackingValue; //Store the starting value of the mint batch
    ...
    function issueReward(uint256[] memory _tokenIds, address _to, uint256 _newTokenId) external onlyOwner{
    	//Mark the Tickets so they can't be used again
    	for (uint256 i=0; i < _tokenIds.length; i++) {
    		require (_ticketsUsed[_tokenIds[i]] == false, "already used this tokenId");
    		_ticketsUsed[_tokenIds[i]] = true;
    	}
    ...
    function checkUsed(uint256 tokenId) external view returns(bool) {
    	return _ticketsUsed[tokenId];
    }
    ...
    function approve(address operator, uint256 tokenId) public virtual override onlyAllowedOperatorApproval(operator){
        //super.approve(operator, tokenId);  << -- changed "to" to "operator" for this function.
        
        address owner = ownerOf(tokenId);
        require(operator != owner, "ERC721: approval current owner");
