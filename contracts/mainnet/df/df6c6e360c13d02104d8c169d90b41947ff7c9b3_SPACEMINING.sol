...
contract SPACEMINING is ERC721Enumerable, Ownable {
    using Strings for uint256;
    event mintSpaceMining(address indexed sender, uint256 startWith, uint256 times);

    //supply counters 
    uint256 public totalSPACEMINING;
    uint256 public totalPublicSPACEMINING;
    uint256 public totalPrivSPACEMINING;
    uint256 public maxPublic = 150;
    ...
    function mintDev(address to, uint256 _times) external {
        ...
        require(totalPrivSPACEMINING + _times <= maxPrivate, "max supply reached for owner");
        emit mintSpaceMining(_msgSender(), totalSPACEMINING+1, _times);
        for(uint256 i=0; i< _times; i++){
            totalPrivSPACEMINING++;
            _mint(to, 1 + totalSPACEMINING++);
        }
    }  
    ...
    function mint(uint256 _times) payable public {
        ...
        require(totalPublicSPACEMINING + _times <= maxPublic, "max supply reached!");
        require(msg.value == _times * price, "value error, please check price.");
        payable(owner()).transfer(msg.value);
        emit mintSpaceMining(_msgSender(), totalSPACEMINING+1, _times);
        for(uint256 i=0; i< _times; i++){
            totalPublicSPACEMINING++;
            _mint(_msgSender(), 1 + totalSPACEMINING++);
        }
    }  
}