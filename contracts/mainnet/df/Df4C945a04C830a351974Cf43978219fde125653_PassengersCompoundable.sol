...
RevokableDefaultOperatorFiltererUpgradeable {
    ...
    */
	function attach(
		...
		require(msg.sender == owner() || isController[msg.sender], "Please use the Armory");
		//this line will not allow any user apart from owner or controller to move forward inside this function
	...
	*/
	...
	/**
		* @notice This function is used to detach and burn TSD
		* @param passengerId The passenger id
		* @param tsdID The TSD id
	*/
	function detachAndBurnTSD(uint256 passengerId, uint256 tsdID) external {
			require(topOwnerOf(passengerId) == msg.sender, "Detach: Not the true owner of head");
			require(headOf_721[crateContract][tsdID] == passengerId, "Detach: Tail not attached to the head");
			IPassengersCrate(crateContract).burnToken(tsdID);
			IERC721Upgradeable(crateContract).transferFrom(address(this), armoryContract, tsdID);
			popTailToken(crateContract, passengerId, tsdID);
			if (tailsOf_721[passengerId][crateContract].length == 0) {
				popTailAddress(passengerId, crateContract);
			}
	}
	
	/**
		* @notice This function is used to detach and burn TSD by controller
		* @param userAddress The user address
		* @param passengerId The passenger id
		* @param tsdID The TSD id
	*/
	function controllerDetachAndBurn(address userAddress, uint256 passengerId, uint256 tsdID) external onlyController {
			require(topOwnerOf(passengerId) == userAddress, "Detach: Not the true owner of head");
			require(headOf_721[crateContract][tsdID] == passengerId, "Detach: Tail not attached to the head");
			IPassengersCrate(crateContract).burnToken(tsdID);
			IERC721Upgradeable(crateContract).transferFrom(address(this), armoryContract, tsdID);
			popTailToken(crateContract, passengerId, tsdID);
	...
	address public armoryContract;
	
	function setArmoryContract (address armory) external onlyOwner {
		armoryContract = armory;
	}
}
