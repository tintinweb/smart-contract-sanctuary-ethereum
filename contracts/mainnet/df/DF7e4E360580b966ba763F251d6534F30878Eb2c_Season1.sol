...
contract Season1 is ERC721Enumerable, Ownable {
  using Strings for uint256;
  string public baseURI;
  string public baseExtension = ".json";
  uint256 public cost = 0.08 ether;
  uint256 public maxSupply = 1000;
  uint256 public maxLimitWhitelist = 1;
  uint256 public maxLimitNonWhitelist = 20;
  ...
  function mint(address _to, uint256 _mintAmount) public payable {
    ...
    if (msg.sender != owner()) {
        if(whitelisted[_to] == true) {
            require(_mintAmount <= maxLimitWhitelist - _alreadyMinted[_to], "Insufficient mints left");
        }else{
            require(onlyWhiteList==false, "only whitelist addresses can mint");
            require(_mintAmount <= maxLimitNonWhitelist - _alreadyMinted[_to], "Insufficient mints left");
        }
        require(msg.value == cost * _mintAmount, "pay price of nft"); 
        _alreadyMinted[_to] += _mintAmount;
    }
