...
contract NFTContract is ERC721A, Whitelist {
    ...
    address DAO_POOL =  0x0c87f1750C78fAE7e70Dfb46C8151b496ce5448f;
    ...
    function mintSale1(uint256 quantity) external payable onlyWhitelisted {
        ...
        if(block.timestamp <  (BEGIN_SALE1 + 7200)) {
            require(
                whitelist[msg.sender],
                "First two hours only whitelisted addresses are allowed to mint."
            );
        }
    ...
    function mintSale2(uint256 quantity) external payable onlyWhitelisted {
        require(totalSupply() + quantity <= MAX_SUPPLY, "Not enough tokens left");
        require(msg.value == (mintRate * quantity), "Not enough ether sent");
        require(
    ...
    function mintSale3(uint256 quantity) external payable onlyWhitelisted {
        require(totalSupply() + quantity <= MAX_SUPPLY, "Not enough tokens left");
        require(msg.value == (mintRate * quantity), "Not enough ether sent");
        require(
