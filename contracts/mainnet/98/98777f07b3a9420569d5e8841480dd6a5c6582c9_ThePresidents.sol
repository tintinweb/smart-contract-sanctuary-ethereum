...
contract ThePresidents is Ownable, ERC721A, ReentrancyGuard {

    ITheAmericansNFT public immutable theamericanNFT;
    ITheAmericansStake public immutable theamericanStake;

    bool public publicSale = false;
    bool public nftSale = false;
    ...
    string public notRevealedUri = "ipfs://QmXhLe5nuh4SpNwmKCwkM6dPpzuzQXpccqUpZ3yJN17KW7/";

    uint256[40] claimedBitMap;

    constructor(string memory _NAME, string memory _SYMBOL, address _theAmericansNFT, address _theAmericansStake) ERC721A (_NAME, _SYMBOL, 250, maxToken) {
        theamericanNFT = ITheAmericansNFT(_theAmericansNFT);
        theamericanStake = ITheAmericansStake(_theAmericansStake);
    }
    ...
    function mint(uint256 quantity, uint256[] memory tokenIds) external payable callerIsUser {
        require(publicSale || nftSale, "SALE_HAS_NOT_STARTED_YET");
        if(nftSale){
            uint256 tokensToMint;
            for(uint256 i; i < tokenIds.length; ++i){
                if(theamericanNFT.ownerOf(tokenIds[i]) == msg.sender){
                    if (!isClaimed(tokenIds[i])) {
                        _setClaimed(tokenIds[i]);
                        tokensToMint++;
                    }
                } else if(theamericanStake.getStaker(tokenIds[i]) == msg.sender){
                    if (!isClaimed(tokenIds[i])) {
                        _setClaimed(tokenIds[i]);
                        tokensToMint++;
                    }
            ...
            require(tokensToMint > 0, "NOTHING_TO_USE");
    ...
    function flipNFTSale() external onlyOwner {
        nftSale = !nftSale;
    }
