...
contract BlueChipSafe is ERC721Enumerable, Ownable, ReentrancyGuard {
	...
	uint256 public BURN_REPLACE_PRICE = 0.05 ether;
	bool public PAUSED = true;
	bool public SELF_MINT_PAUSED = true;
	...
	//Mints BCS Token to caller.
	function mintVerificationTokenToSelf() external payable nonReentrant {
		require( !SELF_MINT_PAUSED, "Minting paused" );
		require( msg.value >= PRICE, "Incorrect ether amount" );
		address sender = msg.sender;
		require( tokenOwnerToIdMapping[sender] == 0, "There is already a token associated with this wallet");
		_safeMint(sender, NUM_MINTED);
		tokenOwnerToIdMapping[sender] = NUM_MINTED;
		tokenIdToOwnerMapping[NUM_MINTED] = sender;
		NUM_MINTED += 1;
	}
	...
	function mintVerificationTokenWithDiscount(address associatedWallet, address discountContractAddress) external payable nonReentrant {
		require( !PAUSED, "Minting paused" );
		require( discountPricingMapping[discountContractAddress] > 0, "No discounts available for discount contract address");
		require( IERCStandard(discountContractAddress).balanceOf(associatedWallet) > 0, "Associated wallet is inelegible for discount." );
		require( msg.value >= discountPricingMapping[discountContractAddress], "Incorrect ether amount" );
		require( tokenOwnerToIdMapping[associatedWallet] == 0, "There is already a token associated with this wallet");
		_safeMint(associatedWallet, NUM_MINTED);
		tokenOwnerToIdMapping[associatedWallet] = NUM_MINTED;
		tokenIdToOwnerMapping[NUM_MINTED] = associatedWallet;
	...
	//Mints BCS Token to caller with discount pricing.
	function mintVerificationTokenToSelfWithDiscount(address discountContractAddress) external payable nonReentrant {
		require( !SELF_MINT_PAUSED, "Minting paused" );
		address sender = msg.sender;
		require( discountPricingMapping[discountContractAddress] > 0, "No discounts available for discount contract address");
		require( IERCStandard(discountContractAddress).balanceOf(sender) > 0, "Caller is inelegible for discount." );
		require( msg.value >= discountPricingMapping[discountContractAddress], "Incorrect ether amount" );
		require( tokenOwnerToIdMapping[sender] == 0, "There is already a token associated with this wallet");
		_safeMint(sender, NUM_MINTED);
		tokenOwnerToIdMapping[sender] = NUM_MINTED;
	...
	function burnAndReplace() external payable nonReentrant {
		address associatedWallet = msg.sender;
		require( msg.value >= BURN_REPLACE_PRICE, "Incorrect ether amount" );
	...
	function setDiscount(address discountContractAddress, uint256 discountPrice) external onlyOwner {
		require(discountPrice <= PRICE, "Discount price cannot be higher than public price");
	...
	//Sets burn/replace price.
	function setBurnReplacePrice(uint256 _newBurnReplacePrice) external onlyOwner {
		BURN_REPLACE_PRICE = _newBurnReplacePrice;
	}
	...
	//Pause self mint.
	function pauseSelfMint(bool val) external onlyOwner {
		SELF_MINT_PAUSED = val;
	}
