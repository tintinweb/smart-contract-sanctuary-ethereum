...
contract IdolMarketplace is VirtueStaking {
  ...
  constructor(address _idolMintAddress, address _idolMainAddress)
    VirtueStaking(_idolMintAddress)
  {
  ...
  */
  function postGodListing(uint _godId, uint _salePriceInWei) external onlyGodOwner(_godId) {
  ...
  */
  function postGodListingForAddress(uint _godId, uint _salePriceInWei, address _toAddress) external onlyGodOwner(_godId) {
  ...
  */
  function removeGodListing(uint _godId) external {
  ...
  */
  function acceptBidForGod(uint _godId, uint _minPrice) external onlyGodOwner(_godId) {
    Bid memory existingBid = godBids[_godId];
...
contract VirtueStaking is ReentrancyGuard, Ownable {
  ...
  constructor(address _mintContractAddress)
  {
    cumulativeETH = 0;
    mintContractAddress = _mintContractAddress;
  }
