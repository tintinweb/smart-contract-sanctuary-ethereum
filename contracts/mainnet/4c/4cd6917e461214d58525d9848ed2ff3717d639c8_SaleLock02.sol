...
contract SaleLock02 is ISaleLock {
    ...
    // ID padding from SaleLock v1, as there is a lack of a pausing mechanism
    ...
    //event UpdateFee(address changer, uint256 oldFee);
    //event UpdateVault(address newVault, address oldVault);
    ...
    function lock(
        ...
        uint256 unlockDate,
        string memory description
    ) external override payable returns (uint256 id) {
        ...
        amount = _safeTransferFromEnsureExactAmount(
            token,
            msg.sender,
            address(this),
            amount
    ...
    function _multipleVestingLock(
        ...
        uint256[] memory temps = _transAmount(token, amounts);
    ...
    function _transAmount(address token, uint256[] calldata amounts) internal returns(uint256[] memory) {
        uint256 sumAmount = _sumAmount(amounts);

        uint256 tempVal = _safeTransferFromEnsureExactAmount(
            token,
            msg.sender,
            address(this),
            sumAmount
        ...
        tempVal = (tempVal*1000)/sumAmount;  

        uint256[] memory temps = new uint256[](amounts.length);
        for(uint256 i=0; i < amounts.length; i++){
            temps[i] = (tempVal*amounts[i])/1000;
            if(temps[i] > amounts[i]){
                temps[i] = amounts[i];
            }
        }
    ...
    function editLock(
        ...
        if (newAmount > 0) {
            ...
            if (diff > 0) {
                ...
                diff = _safeTransferFromEnsureExactAmount(
                    userLock.token,
                    msg.sender,
                    address(this),
                    diff
                ...
                userLock.amount = userLock.amount + diff;
                CumulativeLockInfo storage tokenInfo = cumulativeLockInfo[
                    userLock.token
                ];
    ...
    function _safeTransferFromEnsureExactAmount(
        ...
        require(
            newRecipientBalance > oldRecipientBalance,
            "Not enough token was transfered"
        );
        return newRecipientBalance - oldRecipientBalance;
    }
