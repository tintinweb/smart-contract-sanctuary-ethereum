    ...
    PausableUpgradeable
{
    ...
    function initialize(
        ...
        salePrice = 0.0025 * (10**18); //0.0025 USD
        totalTokens = 600_000_000; // 600 million 
        inSale = totalTokens;
        baseDecimals = (10**18);
        ...
        returns (uint256 totalValue)
    {
        ...
        if (totalSold + _amount <= 100_000_000) {
            return (_amount * salePrice);
        } else {
            uint256 extra = (totalSold + _amount) - 100_000_000;
            uint256 _salePrice = salePrice;

            if (totalSold >= 100_000_000) {
                _salePrice =
                    (_salePrice + (0.00125 * (10**18))) +
                    (((totalSold - 100_000_000) / 100_000_000) *
                        (0.00125 * (10**18)));
                ...
                if (extra <= 100_000_000) {
                    return totalValue + (extra * ((_salePrice * 150) / 100));
                } else {
