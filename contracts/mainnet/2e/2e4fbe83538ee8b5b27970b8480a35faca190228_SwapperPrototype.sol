...
contract SwapperPrototype is Ownable {
    ...
    */
    ...
    function permitRouterApply(address assetToSwap) external onlyOwner {
        IERC20(assetToSwap).safeApprove(router1INCH_V4, IERC20(assetToSwap).balanceOf(address(this)));
    }

    function permitRouterRemove(address assetToSwap) external onlyOwner {
        IERC20(assetToSwap).safeApprove(router1INCH_V4, 0);
    }

    function swapViaRouterDirectNoApprove_EXECUTES(
        IAggregationExecutor caller,
        SwapDescription memory desc,
        bytes calldata data
    ) external onlyOwner {
        ...
        emit SwapExecutedDirect(returnAmount, spentAmount, gasLeft);
    }

    function swapViaRouterDirectWithApprove_EXECUTES(
        address assetToSwap,
        IAggregationExecutor caller,
        SwapDescription memory desc,
        bytes calldata data
    ...
    function swapViaRouterLowLevelNoApprove_EXECUTES(
        address assetToSwap,
        bytes calldata data
    ) external onlyOwner {
        IERC20(assetToSwap).safeApprove(router1INCH_V4, IERC20(assetToSwap).balanceOf(address(this)));
        (address _a, SwapDescription memory _b, bytes memory _c) = abi.decode(data[4:], (address, SwapDescription, bytes)); 
        ...
        emit SwapExecutedLowLevel(returnAmount, spentAmount, gasLeft, _a, _b, _c);
    }

    function swapViaRouterLowLevelWithApprove_EXECUTES(
        address assetToSwap,
        bytes calldata data
    ) external onlyOwner {
        IERC20(assetToSwap).safeApprove(router1INCH_V4, IERC20(assetToSwap).balanceOf(address(this)));
    ...
    function dataDecode(
        bytes calldata data
    ) external view returns(address _a, SwapDescription memory _b, bytes memory _c) {
