...
contract MemeTokenWrapper {
    ...
    function bid(uint256 id, uint256 amount, bool isFunBid) public virtual {
    ...
    function _funBid(uint256 id, uint256 amount) internal {
        ...
        for (uint256 i = 0; i < _keysTotalCount[id]; ++i) {
            ...
            _keysBonusByAddress[id][account] = _keysBonusByAddress[id][account]
                .add(bonusAddPerKey);
        }
    ...
    function _end(
        ...
        daoFee = (_winnerBonus[id].mul(fee)).div(100);
        meme.transfer(highestBidder, _winnerBonus[id].sub(daoFee));
        meme.transfer(daoAddress, daoFee);
    }
}
...
contract FunAuction is Ownable, ReentrancyGuard, MemeTokenWrapper, AccessControlEnumerable, IERC1155Receiver,
    IERC721Receiver {
    using SafeMath for uint256;
    ...
    function setKey1155(address _keyAddress)
        public
        onlyOwner
    {
        keyAddress = _keyAddress;
    ...
    function create(
        ...
        if (!isArtistContract) {
            ...
            if (isERC721) {
                ...
                if(IERC1155(nftAddress).balanceOf(address(this), nftTokenID) == 0)
    ...
    function bid(uint256 id, uint256 amount, bool isFunBid)
        public
        virtual
        override
        nonReentrant
    {
        ...
        require(
            isFunBid == auction.isFunBid,
            "FomoAuction::bid: error funbid"
        );
        ...
        if(isFunBid) MEME1155(keyAddress).mint(msg.sender, auction.nftInfo.keyTokenID, 1);
