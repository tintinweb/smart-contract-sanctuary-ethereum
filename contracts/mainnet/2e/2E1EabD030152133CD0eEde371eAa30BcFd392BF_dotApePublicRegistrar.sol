...
contract dotApePublicRegistrar is apeErc721Impl, apeRegistryImpl, apeNamehash {
    ...
    function getAllNamesOfOwner(address owner) public view returns (string[] memory) {
        ...
        for (uint256 tokenId = 0; tokenId < totalTokens; tokenId++) {
            address tokenOwner = getOwner(tokenId);
            if (tokenOwner == owner) {
                string memory tokenName = getName(tokenId); // Assuming there's a function to retrieve the name of a token given its ID
    ...
    function getAllTokensOfOwner(address owner) public view returns (Token[] memory) {
        ...
        for (uint256 tokenId = 0; tokenId < totalTokens; tokenId++) {
            address tokenOwner = getOwner(tokenId);
            if (tokenOwner == owner) {
                string memory tokenName = getName(tokenId);
