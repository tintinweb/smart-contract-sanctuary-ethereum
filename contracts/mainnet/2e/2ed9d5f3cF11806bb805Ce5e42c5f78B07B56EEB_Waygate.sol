...
abstract contract TokenDistributor {
    ...
    function _addReceipient(
        address _recipient,
        uint256 _allocationPercent
    ) internal {
        require(
            !isRecipient[_recipient],
            "Distributor: Recipient already exists"
        );
        require(
    ...
    PausableUpgradeable
{
    ...
    function setTaxReciever(address _taxReciever) public onlyOwner {
        require(
            _taxReciever != address(0),
            "Taxable: Tax reciever cannot be zero address"
        );
    ...
    function getTaxReciever() public view returns (address) {
        return taxReciever;
    }
    ...
    function transferFrom(
        ...
        uint256 _taxAmount = (amount * taxRate) / NUMERATOR;

        if (_taxAmount > 0) {
            _transfer(from, taxReciever, _taxAmount);
        }

        address spender = _msgSender();
        _spendAllowance(from, spender, (amount - _taxAmount));
        _transfer(from, to, (amount - _taxAmount));
        return true;
    }
