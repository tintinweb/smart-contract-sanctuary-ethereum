...
contract FILABIO is Ownable, ERC721A {
    ...
    // public sale start time is 2022/09/08 16:00:00(GMT+8)
    uint32 publicSaleStartTime = 1662624000;
    // uint32 publicSaleStartTime = 1662364800;
    // max mint count 25
    uint256 MAX_MINT = 25;
    ...
    function setFreelist(address[] memory addresses, uint256[] memory numSlots) external onlyOwner
    {
        require(addresses.length == numSlots.length, "addresses does not match numSlots length");
        for (uint256 i = 0; i < addresses.length; i++) {
