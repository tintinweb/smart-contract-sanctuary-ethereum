...
contract MossLandNFT is LandNFT_Authorization {
    ...
    mapping(uint256 => string[]) public tokenResources;

    event OwnershipTransferred(address indexed previousOwner, address indexed newOwner);
    event ProjectAdded(address sender, Project project);
    event ResourceAdded(address sender, uint256 indexed projectId, string uri);
        ...
        initializer
    {
        ...
        _setupRole(ADMIN_ROLE, operator);
    ...
    ***************************************/
    ...
    function addResource(
        ...
        onlyOperator
    {
        require(isRegistered(id), "MossLandNFT: Project not registered");

        tokenResources[id].push(resource);

        emit ResourceAdded(msg.sender, id, resource);
    }
