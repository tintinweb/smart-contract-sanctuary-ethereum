...
contract ENUM is ERC721Upgradeable, OwnableUpgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    // contract is unpaused. It pauses makeReservations and mintItem.
    //
    ...
    function makeReservation(
        ...
        require(
            _pendingTokenOwner != address(0),
            "3NUM: address zero is not a valid reserver"
        );

        require(
            reservations[_pendingTokenOwner] == 0,
        ...
        uint256 _newReservationId = reservationIds.current();

        reservations[_pendingTokenOwner] = _newReservationId;

        emit ReservationMade(_pendingTokenOwner, _newReservationId);

        return _newReservationId;
    }

    function reservationOf(address _reserver) external view returns (uint256) {
        return reservations[_reserver];
    }
    ...
    function mintItem(
        ...
        require(
            verifyNumberSignature(_tokenId, reservations[_pendingTokenOwner], _signature),
    ...
    function verifyNumberSignature(
        ...
        bytes32 _messageHash = getMessageHash(_tokenId, _reservationId);
    ...
    function getMessageHash(
        ...
        return
            keccak256(
                abi.encodePacked(abi.encodePacked(_tokenId, _reservationId))
            );
    }
