...
contract GoldenTicket is ERC721AQueryable, Ownable, ReentrancyGuard, ILayerZeroReceiver {
    ...
    uint256 public constant WHITELIST_SALE_PRICE = 0.035 ether;
    uint256 public constant PUBLIC_SALE_PRICE = 0.045 ether;
    
    uint256 public constant MAX_WHITELIST_MINTS = 3;
    ...
    function whitelistMint(uint256 _mintAmount, bytes32[] calldata _merkleProof) public payable mintCompliance(_mintAmount) {
        ...
        require((whitelistMints[_msgSender()] + _mintAmount) <= MAX_WHITELIST_MINTS, "Your wallet is out of whitelist mints!");
    ...
    function PublicMint(uint256 _mintAmount) public payable mintCompliance(_mintAmount) {
        ...
        require((totalSupply() + _mintAmount) <= maxSupply, "Beyond Max Supply");
        require(
    ...
    function claimPresale(uint256 _mintAmount, bytes32[] calldata _merkleProof) public payable mintCompliance(_mintAmount)  {
        require(!paused, "Minting is paused.");
        require(presaleClaim, "The claiming process hasn't begun!");
        require((presaleClaims[_msgSender()] + 1) <= MAX_PRESALE_CLAIM, "Your wallet is out of whitelist mints!");
        bytes32 leaf = keccak256(abi.encodePacked(_msgSender()));
