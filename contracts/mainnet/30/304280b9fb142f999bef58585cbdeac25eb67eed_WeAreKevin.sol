...
contract WeAreKevin is Ownable, ERC721A, ReentrancyGuard {
  ...
  ) ERC721A("WeAreKevin", "KEVINS", maxBatchSize_, collectionSize_) {
  ...
  ///////////// MINT SECTION /////////////
    uint256 public pricePer = 0.01 ether;
    uint256 maxTotalSupply = 2222;
    ...
    uint256 public publicCurrent = 0;
    uint256 public maxPUBLIC = 1200;
    function setpublicCurrent(uint256 newMax) public onlyOwner{
      publicCurrent = newMax;
    }
    function setMaxPublic(uint256 newMax) public onlyOwner{
      maxPUBLIC = newMax;
    }

    function mint(uint256 quantity) external payable{
      require(publicSaleStart, "Sale not started");
      require(publicCurrent + quantity <= maxPUBLIC, "Sold out");
      require(mintedPub[msg.sender] + quantity < 11, "Can't mint more than 10");
      require(msg.value >= pricePer * quantity, "Not enough Eth");
      publicCurrent += quantity;
      mintedPub[msg.sender] += quantity;

      _safeMint(msg.sender, quantity);
    }

    uint256 public WLCurrent = 0;
    uint256 public maxWL = 1000;
    uint256 public maxWLMint = 3;
    mapping(address => uint256) public minted;
    mapping(address => uint256) public mintedPub;

    function setWLCurrent(uint256 newMax) public onlyOwner{
      WLCurrent = newMax;
    }
    function setMaxWLMint(uint256 newMax) public onlyOwner{
      maxWLMint = newMax;
    }
    ...
    if(onlyWhitelist){
      ...
      require(WLCurrent + quantity <= maxWL, "Sold out");
      WLCurrent += quantity;
