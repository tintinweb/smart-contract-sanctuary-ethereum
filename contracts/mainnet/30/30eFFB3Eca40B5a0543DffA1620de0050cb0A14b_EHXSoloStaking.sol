...
contract EHXSoloStaking is OwnableUpgradeable,ReentrancyGuardUpgradeable {
    ...
    function updatePool() internal {
        ...
        if(USDC.balanceOf(address(this)) > 0){
            USDC.safeTransfer(address(dividendTracker), USDC.balanceOf(address(this)));
    ...
    function setDividentTracker(address _newTracker) external onlyOwner{
        dividendTracker = DividendTrackers(payable(_newTracker));        
    }
}
...
contract DividendPayingToken is DividendPayingTokenInterface, DividendPayingTokenOptionalInterface, Ownable {
  ...
  function _withdrawDividendOfUser(address payable user) internal returns (uint256) {
    ...
    if (_withdrawableDividend > 0) {
      ...
      IERC20(token).safeTransfer(user, _withdrawableDividend);
