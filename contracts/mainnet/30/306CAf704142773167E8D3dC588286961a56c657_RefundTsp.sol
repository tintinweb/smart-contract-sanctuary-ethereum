...
contract RefundTsp is Configurable,IERC721ReceiverUpgradeable {
    ...
    function tokenIdInRange(uint tokenId) private view returns(bool isIn){
            uint seg = limitId.length/2;
            isIn = false;   
            for (uint i=0;i<seg;i++){
                if (limitId[i]<=tokenId && limitId[i+1]>=tokenId){
                    isIn = true;
                    break;
                }
            }
    ...
    function canBeRefunded(address refunder,uint256 tokenId) public view returns (bool) {
        return tokenIdInRange(tokenId) && begin<=block.timestamp && end>=block.timestamp &&!tokenIdRefunded[tokenId] && refundedNum[refunder]<refundMax ;
    }
