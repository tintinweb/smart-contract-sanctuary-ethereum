...
contract ThankYouBidenYellen is ERC721A, Ownable {
    // mint variables
    uint256 public PRICE = 0.000777 ether;
    string public imageURI;
    string private _description =
        "Thank you! to President Joseph R. Biden and Treasury Secretary Janet Yellen for creating and narrowly avoiding the 2023 Banking Crisis. Open edition. Artwork by David Rudnick.";
    ...
    constructor(string memory _imageURI) ERC721A("Thank You Brandon & Yellen", "TYBY") {
        imageURI = _imageURI;
        mintCloseTime = block.timestamp + 86400;
    }
    ...
    function mint(uint256 quantity) external payable {
        ...
        if (msg.value != PRICE * quantity) revert IncorrectPrice();

        _mint(msg.sender, quantity);
    }

    function adminMint(uint256 quantity, address dest) external onlyOwner {
        if (!_mintOpen()) revert MintClosed();
        _mint(dest, quantity);
    }
        ...
        string memory json = Base64.encode(
            bytes(
                string(
                    abi.encodePacked(
                        "{",
                        '"name": "Thank you! The 2023 Banking Crisis #',
