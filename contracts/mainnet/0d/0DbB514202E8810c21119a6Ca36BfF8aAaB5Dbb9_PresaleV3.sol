...
contract PresaleV3 is Initializable, ReentrancyGuardUpgradeable, OwnableUpgradeable, PausableUpgradeable {
  ...
  function buyWithUSDT(uint256 amount, uint256 lockup_months) external checkSaleState(amount) whenNotPaused returns (bool) {
    ...
    uint256 newBonus = update(amount, lockup_months ,usdEq);
  ...
  function update(uint256 amount, uint256 lockup_months , uint256 _linearPriceUsdRaised) internal returns (uint256) {
    ...
    userDeposits[_msgSender()].push(UserDeposits(amount * baseDecimals, newBonus, (((amount * baseDecimals) + newBonus) * initialClaimPercent) / 10000, 0, time));
    totalBonus += (newBonus);
    return newBonus;
  }
