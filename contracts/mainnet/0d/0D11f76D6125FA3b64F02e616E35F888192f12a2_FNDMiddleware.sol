...
error NFTDropMarketFixedPriceSale_General_Access_Must_Come_After_Early_Access_Start_Time();
...
error NFTDropMarketFixedPriceSale_Must_Have_Non_Zero_Early_Access_Period();
...
abstract contract NFTDropMarketFixedPriceSale is MarketFees {
  ...
  modifier onlyCollectionAdmin(address nftContract) {
    if (!IAccessControl(nftContract).hasRole(DEFAULT_ADMIN_ROLE, msg.sender)) {
      // The msg.sender must have admin permissions to this collection.
    ...
    onlyValidMerkle(merkleRoot, merkleTreeUri)
  {
    ...
    if (saleConfig.seller == payable(0)) {
      // Confirm this collection has already been listed.
    ...
    if (saleConfig.generalAvailabilityStartTime == saleConfig.earlyAccessStartTime) {
      // Must have non-zero early access duration.
      revert NFTDropMarketFixedPriceSale_Must_Have_Non_Zero_Early_Access_Period();
    }
   ...
   * @param limitPerAccount The max number of NFTs an account may mint in this sale.
   * @dev This API is deprecated and will be removed in the future, `createFixedPriceSaleV2` should be used instead.
   * Notes:
  ...
  ) external txDeadlineNotExpired(txDeadlineTime) validStartTimeSize(generalAvailabilityStartTime) {
   ...
   * @dev Notes:
   *   a) The sale is final and can not be updated or canceled.
   *   b) The sale can be scheduled to start immediately or start sometime in the future.
   *   c) Any collection that abides by `INFTDropCollectionMint` and `IAccessControl` is supported.
   */
  function createFixedPriceSaleWithEarlyAccessAllowlist(
    ...
    external
    txDeadlineNotExpired(txDeadlineTime)
    onlyValidMerkle(merkleRoot, merkleTreeUri)
    validStartTimeSize(generalAvailabilityStartTime)
  {
    ...
    if (earlyAccessStartTime >= generalAvailabilityStartTime) {
      ...
      revert NFTDropMarketFixedPriceSale_General_Access_Must_Come_After_Early_Access_Start_Time();
    }
