...
contract BirdPass is ERC1155Supply, Ownable 
{
    bool public saleIsActive = false;
    uint public passId = 1;
    uint public maxPerTransaction = 5;
    uint public maxSupply = 999;

    uint public constant NUMBER_RESERVED_TOKENS = 29;
    uint256 public PRICE = 80000000000000000; //0.08
    string public name = "Bird Pass"; // Token name
    
    uint public reservedTokensMinted = 0;
    
    string public contractURIstr = "";

    constructor() ERC1155("https://ipfs.io/ipfs/QmTyraLLiFbLXE78gJdWoKz93LxHYtH6Nrab1hqi35rxV3/{id}.json") {}
    ...
    function mintToken(uint256 amount) external payable
    {
        ...
        require(
            totalSupply(passId) + amount <= maxSupply - (NUMBER_RESERVED_TOKENS - reservedTokensMinted), 
            "Purchase would exceed max supply"
        );
        
        require(PRICE * amount <= msg.value, "Not enough ETH for transaction");
    ...
    function changeSaleDetails(uint _passId, uint _maxPerTransaction, uint _maxSupply) external onlyOwner 
    {
        passId = _passId;
    ...
    function setPassPrice(uint _price) external onlyOwner 
    {
        PRICE = _price;
    }
}