...
// File contracts/CivVaultGetter.sol

pragma solidity 0.8.17;
/// @title  Civ Vault Getter function
/// @author Ren


contract CivVaultGetter is Ownable, ReentrancyGuard {
    ...
    modifier onlyVault() {
        require(_msgSender() == address(civVault), "not owner");
        _;
    }

    constructor(ICivVault _civVault) {
        civVault = _civVault;
    }
    ...
    function addPair(uint256 _pid,IUniswapV2Pair _pair) internal{
        ...
        FixedPoint.uq112x112 memory price0Average = FixedPoint.uq112x112(uint224((price0Cumulative - _pair.price0CumulativeLast()) / timeElapsed));
        FixedPoint.uq112x112 memory price1Average = FixedPoint.uq112x112(uint224((price1Cumulative - _pair.price1CumulativeLast()) / timeElapsed));
        pairInfo[_pid].push(
            PairInfo({
                pair: _pair,
    ...
    function addUniPair(uint256 _pid, address _token0, address _token1) external onlyVault {
        WETH_ADDRESS = IUniswapV2Router01(UNISWAP_ROUTER).WETH();
    ...
    /// @notice Balance of the user based on NAV
    /// @dev View function so does not cost any gas
    /// @param _pid Pool Id
    /// @param _user Address of the user
    /// @return balance Current Balance of User in a given pool
    /// @return shareAmount Current Share Balance of User in a given pool
        ...
        returns (uint256, uint256)
    {
        ...
        if( poolInfo.totalShares == 0) return (0,0);
        uint256 currentNAV = poolInfo.NAV - poolInfo.unpaidFee;
        uint256 shares = poolInfo.fundRepresentToken.balanceOf(_user);
        uint256 balance = currentNAV * shares / poolInfo.totalShares;
    ...
    function getPrice(uint256 _pid, uint256 amountIn) public virtual view returns(uint amountOut) {
        (PoolInfo memory poolInfo,)=civVault.getPoolInfo(_pid);
        PairInfo[] memory curPairInfo = pairInfo[_pid];
        if( curPairInfo.length == 1 ) {
            if( address(poolInfo.lpToken) == curPairInfo[0].token0)
                amountOut = curPairInfo[0].price0Average.mul(amountIn).decode144();
            else
                amountOut = curPairInfo[0].price1Average.mul(amountIn).decode144();
        }
        else {
    ...
    function getReversePrice(uint256 _pid, uint256 amountIn) public virtual view returns(uint amountOut) {
        (PoolInfo memory poolInfo,)=civVault.getPoolInfo(_pid);
        PairInfo[] memory curPairInfo = pairInfo[_pid];
        if( curPairInfo.length == 1 ) {
            if( address(poolInfo.guaranteeToken) == curPairInfo[0].token0)
    ...
    /// @dev Get Deposit Pending Users Length on a given pool
    /// @param _pid Pool Id
    /// @return users Current Deposit Pending Users Length in a given pool
    function getDepositUsers(uint256 _pid) external view returns(uint256) {
        (,VaultInfo memory vaultInfo)=civVault.getPoolInfo(_pid);
        return vaultInfo.curDepositUser;
    }

    /// @dev Get Withdraw Pending Users Length on a given pool
    /// @param _pid Pool Id
    /// @return users Current Withdraw Pending Users Length in a given pool
    function getWithdrawUsers(uint256 _pid) external view returns(uint256) {
        (,VaultInfo memory vaultInfo)=civVault.getPoolInfo(_pid);
        return vaultInfo.curWithdrawUser;
    }
}