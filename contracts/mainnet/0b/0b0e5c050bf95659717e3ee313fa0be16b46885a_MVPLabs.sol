...
// File: contracts/mvplabs.sol


pragma solidity ^0.8.13;


contract MVPLabs is ERC20, Ownable {
    ...
    uint256 public maxBuyAmount = 2 * 10**9 * 10**9; //
    uint256 public maxSellAmount = 2 * 10**9 * 10**9; //2% of the supply
    uint256 public maxWalletAmount = 2 * 10**9 * 10**9; // 2% of the supply

    uint16 private totalFee;
    bool public swapEnabled;
   

    address payable marketingWallet = payable(address(0xF4C5d78fae2293f3C8D5b7f1f9849b3beDbdc8F9)); //  marketing Wallet 
    ...
    constructor() ERC20("MVP Labs", "MVPX") {
        ...
        */
        _mint(owner(), 120 * 10**9 * (10**9)); // 120,000,000,000 Tokens 
    ...
    function setFees(
       ...
       uint16 sell
         ...
         buyFee.marketingFee = buy;
         sellFee.marketingFee = sell;
          
        
        totalFee = buyFee.marketingFee + sellFee.marketingFee;
        require (totalFee < 30, "max fees should be less than 30%");
    }
   ...
   function setSwapTokensAtAmount(uint256 amount) external onlyOwner {
    ...
    function _transfer(
        ...
        require(isTradingEnabled || _isExcludedFromFees[from],"trading is not enabled yet");
        ...
        if (takeFee) {
            ...
            if (automatedMarketMakerPairs[to]) {
                fees = sellFee.marketingFee;
            } else if (automatedMarketMakerPairs[from]) {
                fees = buyFee.marketingFee;
            }

            if (!_isExcludedFromLimit[from] && !_isExcludedFromLimit[to]) {
                if (automatedMarketMakerPairs[to]) {
                    require(amount <= maxSellAmount, "Sell exceeds limit");
                    require(block.timestamp - _isBought[from] > cooldown, "cool down is enabled,pls wait some time");
                } else if (automatedMarketMakerPairs[from]) {
                    require(amount <= maxBuyAmount, "Buy exceeds limit");
                    _isBought[to]= block.timestamp;
                }
