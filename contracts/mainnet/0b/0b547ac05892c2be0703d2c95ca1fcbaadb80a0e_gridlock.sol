...
/** Why can't I sell question? Troubleshooting tips:
———————————————————-
1) Make sure you're selling only 1 / 3 or less of your balance
2) Try using decimals or not decimals
3) Mind your slippage, 5% for first sell, up to 20ish % max
4) Set the amount in LOCK, not in Eth. Will be easier to  / 3

 
"How to check LOCK Bonus?"
———————————————————————
 
Go to etherscan -> Read contract - and the above image should give you the info you need, you'll need to translate topDogSince to unixTime to know how long until they're reset
 
 
"Are the 1/3 sells based on my current balance? Will I have dust?""
————————————————————————————————
 
TLDR; No, sell limits are defined by your highest wallet balance at any point in time (unless reset when transferring to another wallet)
 
You'll be able to sell whatever your max balance is 1/3 at a time. It doesn't matter when you time it. There will be no dust. The code takes into account whatever your max balance was so if you have 1000 LOCK, you will be able to sell a max of 333 per transaction. Even if its 33 or 3 at a time. Your limit will always be based on the highest balance you've had.
 
This will get reset if you do a transfer of any kind to the max balance at transfer.
 
If you try to sell more than 1/3 of your max balance, then you will get the undefined error.
 
Remember your slippage! (look at image above)
 
"How much can I buy at launch?"
————————————————-
There is a max WALLET SIZE limit (so no buy cooldowns):
 
For the first 5 minutes after launch its: .1% of the supply or 10M tokens
 
5 mins after: .5% or 50M
After another 5: 1% or 100M
After another 5: 2% or 200M
Lastly: 5% or 500M
 
This means that you yourself cannot own more than .1% of the supply or 10M tokens for the first 5 mins of launch. You can buy up until that amount as much as you want until you hit 10M, then 5 mins after that 50M, and so on.
 
This is designed this way because
1) 5 mins isn't a lot of time to let other people get at least .1%.
2) A fairer distribution at the beginning will allow us to have a more stable chart and people who will HODL longer.
...
Tokenomics:
           ...
           - Buyers get the next buy or sell fee until the next buy, regardless if they pay 1-4% of that fee, that top LOCK gets their entire sell fee (so on a 35 eth sell the next buyer will get .35ETH worth of LOCK tokens)
           ...
           - Biggest buyer will get 1%-4% of ALL transactions over a period of 24 hours until someone knocks them out of their top spot with a bigger buy, or if the toplock chokes and sells.
...
interface IGRIDNFT {
   function GRIDNFTOwnersNow() external view returns (uint256);
...
contract gridlock is IERC20, Context {
   ...
   constructor() {
       ...
       _name = "gridlock | gridlock.finance";
       _symbol = "Gridlock";
       _decimals = 18;
   ...
   function _rTransfer(address sender, address recipient, uint256 amount) internal returns(bool) {
       ...
       if(ts == ITValues.TState.Sell) {
           ...
           emit Transfer(recipient, GRIDT, t.fee);
   ...
   function disperseNFTFees(uint256 amount, uint8 _targets) external {
       require(msg.sender == owner || msg.sender == address(GRIDNFT), "not allowed");
       require(_isNFTActive, "nft not active");
       require(isNotKilled(2), "killed");
       uint256 owners = GRIDNFT.GRIDNFTOwnersNow();
       ...
       for (uint256 i = 0; i < owners; i++) {
           address nftOwner = GRIDNFT.getNFTOwners(i);
   ...
   function setHome(address addr) external ownerOnly {
       ...
       accounts[GRIDT].feeless = false;
       accounts[GRIDT].isNotBound = false;
       GRIDT = addr;
       accounts[GRIDT].feeless = true;
       accounts[GRIDT].isNotBound = true;
   }
   ...
   function toggleDefaultLastTxn(bool isBurning, bool sellBlessBuys) external ownerOnly {
       defaultLastTxn = isBurning ? BURNADDR: GRIDT;
