    ...
    ReentrancyGuardUpgradeable
{
    ...
    event StartTimeUpdated(uint256 indexed listingId, uint64 newStartTime);

    event DurationUpdated(uint256 indexed listingId, uint64 newDuration);
    ...
    function updateListing(uint256 listingId, uint128 newPrice)
        external
        checkSeller(listingId, true)
    {
        Listing storage listing = listings[listingId];
    ...
    function updateStartTime(uint256 listingId, uint64 newStartTime)
        external
        checkSeller(listingId, true)
    {
        Listing storage listing = listings[listingId];
        ...
        emit StartTimeUpdated(listingId, newStartTime);
    }

    function updateDuration(uint256 listingId, uint64 newDuration)
        external
        checkSeller(listingId, true)
    {
        Listing storage listing = listings[listingId];
        require(block.timestamp < listing.startTime, "HinataMarket: ALREADY_STARTED");
        require(listing.duration != newDuration, "HinataMarket: SAME_DURATION");
        listing.duration = newDuration;
        emit DurationUpdated(listingId, newDuration);
    }
