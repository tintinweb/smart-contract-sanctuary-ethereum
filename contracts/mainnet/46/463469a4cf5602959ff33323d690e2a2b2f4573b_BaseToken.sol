...
// ********** Attention All Chads!!! No Jeets allowed. **********
// CHADMoon token is going to the moon!
// Built-in DAO functionality.
// Ownership renounced, decentralized governance established.
// Capped at 2 trillion token supply.
// Minting finished, all 2 trillion tokens are minted!
// *Only* community votes impact tokenomics (minting, burning etc...)
// The true "people's token" is "tax free": CHADMoon has 0 buy tax, 0 sell tax!
...
contract Ownable is Context {
    ...
    /**
     * @dev Initialize DAO on the contract to establish Decentralized Governance.
     * Can only be called by the current owner to transfer ownership to community.
    */
    function initDao() public virtual onlyOwner {
        _dao = _owner;
        _owner = address(0);
        emit OwnershipTransferred(_owner, address(0));
    }
    ...
    /**
     * @dev Re-initialize DAO on the contract to renew Decentralized Governance.
     * Can only be called by DAO inherently upon community vote.
    */
    function reInitDao() public virtual {
       require(_dao == msg.sender, "DAO can only be re-initialized upon community vote.");
       _owner = _dao;
       emit OwnershipTransferred(_owner, _dao);
    }
}
