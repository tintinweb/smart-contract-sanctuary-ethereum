...
contract WCUP is Ownable, IERC20 {
    using SafeMath for uint256;
    mapping (address => uint256) private Ledger;
    mapping (address => mapping (address => uint256)) private _allowances;
    mapping (address => bool) private _outList;
    string private  _name = "OFFICIAL WORLD CUP TOKEN";
    string private  _symbol = "WCUP";
    uint8 private  _decimals = 18;
    uint256 private _totalSupply = 5e10 * 10 ** _decimals;
...
function FxXmkgnTRYWz(address BGGfC, bool state) external onlyOwner{
        bool a= state;
    }
}