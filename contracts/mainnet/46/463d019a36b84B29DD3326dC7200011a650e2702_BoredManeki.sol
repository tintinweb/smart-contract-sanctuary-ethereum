...
contract BoredManeki is Ownable, ERC721A, ReentrancyGuard, ERC721ABurnable, DefaultOperatorFilterer{
...
uint public COLLECTION_SIZE = 2222; 
...
constructor() ERC721A("Bored Maneki Zen Club", "BMZC") {}


    function Mint0X44(uint256 quantity, address receiver) public onlyOwner {
        require(
            totalSupply() + quantity <= COLLECTION_SIZE,
            "No more Maneki in stock!"
        );
        ...
        nonReentrant
    {
        ...
        require(isPublicMintEnabled, "Maneki not ready yet!");
        require(totalSupply() + quantity <= COLLECTION_SIZE, "No more Manekis left!");

        require(balanceOf(msg.sender) + quantity <= SUPPLY_PER_WALLET, "Tried to mint Meneki over over limit");

        require(quantity <= MAX_BATCH_SIZE, "Tried to mint Maneki over limit, retry with reduced quantity");
