...
abstract contract StakeBaseUpgradeable is Initializable, UUPSUpgradeable, AccessControlUpgradeable {
    ...
    */
    ...
    @param tokenId_ The token id that will be staked
    @param index_ The index of the tokenId in the users array of staked tokens
    */
    function _unstake(uint256 tokenId_, uint256 index_) private {
        tokenContract.safeTransferFrom(address(stakingContract), msg.sender, tokenId_);
        tokensOfOwner[msg.sender][index_] = tokensOfOwner[msg.sender][tokensOfOwner[msg.sender].length - 1];
