...
contract IconicNFT721a is ERC721A, ReentrancyGuard, Ownable{
 ...
 function setMintSize(uint64 mintSize_) public {
    require(_msgSenderERC721A() == admin, "Not Authorized");
  ...
  function setMaxMintsPerAddress(uint64 maxMintsPerAddress_) public {
    require(_msgSenderERC721A() == admin, "Not Authorized");

    maxMintsPerAddress = maxMintsPerAddress_;
  }

  function setAllowList(bytes32 merkleRoot) public {
    require(_msgSenderERC721A() == admin, "Not Authorized");
  ...
  function allowlistMint(bytes32[] calldata merkleProof, uint16 amount) external payable nonReentrant() {
    ...
    function mint(uint16 amount) external payable nonReentrant() {
        ...
        require(balanceOf(_msgSenderERC721A()) + amount <= maxMintsPerAddress, "Exceeds maximum allowed mints");
