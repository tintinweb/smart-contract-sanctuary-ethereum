...
contract RadcoinV2 is ERC20Upgradeable, ReentrancyGuardUpgradeable, SignatureVerifier {
    ...
    /*//////////////////////////////////////////////////////////////
                 CLAIMING GAS OPTIMIZATIONS
    //////////////////////////////////////////////////////////////*/

    // the time when newly minted bro claiming starts
    uint256 public newMintStartTime;
    ...
    /// @notice Set the start time for new mint claiming.
    function setNewMintStartTime(uint256 startTime) external onlyOperator {
        newMintStartTime = startTime;
    }
    ...
    function claimRadcoin(
        ...
        for (uint256 i; i < _ids.length; ) {
            ...
            if (claim.startTime == 0 && address(_contract) == address(radbroV2)) {
                if (id < radbroV2.V1_UPRGADE_MINTS()) {
                    claim.startTime = upgradeStartTime;
                } else {
                    claim.startTime = newMintStartTime;
                }
            }
            require(claim.startTime != 0, "NOT_INITIALIZED");
    ...
    function _getClaimReward(
        ...
        // if contract is radbroV2 and id is less than radbroV2.V1_UPRGADE_MINTS, set start time to upgradeStartTime
        if (claim.startTime == 0 && address(_contract) == address(radbroV2)) {
            if (_id < radbroV2.V1_UPRGADE_MINTS()) {
                claim.startTime = upgradeStartTime;
            } else {
                claim.startTime = newMintStartTime;
            }
