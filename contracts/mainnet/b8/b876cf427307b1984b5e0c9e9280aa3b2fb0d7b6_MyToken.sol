...
// File: contracts/MutantMemaws.sol
...
contract MyToken is ERC1155, Ownable {
    ...
    function setURI(string memory newIpfs) public onlyOwner {
        ipfs = newIpfs;
    }
    ...
    function buildMetadata() public view returns(string memory) {
        return string(abi.encodePacked(
                'data:application/json;base64,', Base64.encode(bytes(abi.encodePacked(
                            '{"name": "Mutant Memaw Serum', 
                            '", "description":"', 
                            'Can be redeemed for one Mutant Memaw from the future collection',
                            '", "image": "', ipfs,
                            '"}')))));
    }
    function mintMemaw(uint256 amount)
        public payable 
    {
        require(count + amount <= max, "too many to mint, we have reached 10k");
        require(price * amount <= msg.value, "not sending enough eth");
        _mint(msg.sender, 0, amount, '');
        count += amount;
    }

    receive() external payable {
        require(msg.value % price == 0, "sending weird increments");
        mintMemaw(msg.value / price);
    }
