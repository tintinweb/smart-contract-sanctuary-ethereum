...
contract SlothNFT is ERC721A, Ownable, INFT {
    using Strings for uint256;
    event PurchaseEvent(address purchaseWallet, uint256 nftID, uint256 purchaseTimestamp);
    uint256 public constant INVENTORY = 38777;
    uint256 private anti_hacker_limit = 0.002 ether;
    uint8 private _free_mint_available = 1;
    uint256 private _free_mint_start = 1655038800; //2022-06-12 13:00(UTC)
    uint256 private MINT_LIMIT = 2;
    bool private _is_revealed = false;
    string private _base_uri = "";
    string private _blindbox_uri = "https://ipfs.slothnft.org/sloth/blind/";
    
    function _baseURI() internal view override returns (string memory) {
        return _base_uri;
    }

    constructor() ERC721A("Sloth NFT", "SlothNFT") {
        mintTo(msg.sender, 1); 
    }

    function freeMint(uint256 amount) external payable {
        freeMintValidator(msg.sender, amount);
        mintTo(msg.sender, amount);
    }

    function freeMintValidator(address mintUser, uint256 amount) private {
        ...
        require(msg.value >= anti_hacker_limit * amount, "value less than anti_hacker_limit!");
        require(isEnough(amount), "free mint limit reached!");
        require((_numberMinted(mintUser) + amount) <= MINT_LIMIT, "free mint over limit");
    } 
