...
contract EDGE is ERC20, Ownable {
    ...
    IERC20 public constant USDC = IERC20(0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2); //

    uint256 public buyTotalFees;
    uint256 public buyOperationsFee;
    uint256 public buyLiquidityFee;
    uint256 public buyEdgeFee;

    uint256 public sellTotalFees;
    uint256 public sellOperationsFee;
    uint256 public sellLiquidityFee;
    uint256 public sellEdgeFee;

    uint256 public tokensForOperations;
    uint256 public tokensForLiquidity;
    uint256 public tokensForEdge;
    ...
    constructor() ERC20("EDGE", "EDGE") {
        ...
        maxBuyAmount = totalSupply * 1 / 4000000;
        maxSellAmount = totalSupply * 1 / 1000000000;
        maxWalletAmount = totalSupply * 2 / 5000000;
        swapTokensAtAmount = totalSupply * 25 / 100000; // 0.025% swap amount

        buyOperationsFee = 0;
        buyLiquidityFee = 15;
        buyEdgeFee = 0;
        buyTotalFees = buyOperationsFee + buyLiquidityFee + buyEdgeFee;

        sellOperationsFee = 0;
        sellLiquidityFee = 15;
        sellEdgeFee = 0;
        sellTotalFees = sellOperationsFee + sellLiquidityFee + sellEdgeFee;
    ...
    function updateMaxBuyAmount(uint256 newNum) external onlyOwner {
        require(newNum >= (totalSupply() * 1 / 10000000)/1e18, "Cannot set max buy amount lower than 0.1%");
        maxBuyAmount = newNum * (10**18);
        emit UpdatedMaxBuyAmount(maxBuyAmount);
    }
    
    function updateMaxSellAmount(uint256 newNum) external onlyOwner {
        require(newNum >= (totalSupply() * 1 / 10000000)/1e18, "Cannot set max sell amount lower than 0.1%");
        maxSellAmount = newNum * (10**18);
        emit UpdatedMaxSellAmount(maxSellAmount);
    }

    function updateMaxWalletAmount(uint256 newNum) external onlyOwner {
        require(newNum >= (totalSupply() * 3 / 10000000)/1e18, "Cannot set max wallet amount lower than 0.3%");
    ...
    function updateBuyFees(uint256 _operationsFee, uint256 _liquidityFee, uint256 _EdgeFee) external onlyOwner {
        buyOperationsFee = _operationsFee;
        buyLiquidityFee = _liquidityFee;
        buyEdgeFee = _EdgeFee;
        buyTotalFees = buyOperationsFee + buyLiquidityFee + buyEdgeFee;
        require(buyTotalFees <= 15, "Must keep fees at 30% or less");
    }

    function updateSellFees(uint256 _operationsFee, uint256 _liquidityFee, uint256 _EdgeFee) external onlyOwner {
        sellOperationsFee = _operationsFee;
        sellLiquidityFee = _liquidityFee;
        sellEdgeFee = _EdgeFee;
        sellTotalFees = sellOperationsFee + sellLiquidityFee + sellEdgeFee;
        require(sellTotalFees <= 15, "Must keep fees at 50% or less");
    }
    ...
    function _transfer(address from, address to, uint256 amount) internal override {
        ...
        if(takeFee){
            ...
            // on sell
                ...
                tokensForEdge += fees * sellEdgeFee / sellTotalFees;
            }
            // on buy
                ...
                tokensForEdge += fees * buyEdgeFee / buyTotalFees;
            }
    ...
    function swapBack() private {
        ...
        uint256 totalTokensToSwap = tokensForLiquidity + tokensForOperations + tokensForEdge;
        ...
        uint256 usdcForEdge = usdcBalance * tokensForEdge / (totalTokensToSwap - (tokensForLiquidity/2));

        usdcForLiquidity -= usdcForOperations + usdcForEdge;
        ...
        if(usdcForEdge > 0){
            USDC.transfer(yashaAddress, usdcForEdge);
        }
