...
contract SPOXTOKEN is ERC20, Ownable, Pausable {

    uint128 internal DAY = 24 * 3600; // 1 day

    using SafeMath for uint256;

    struct LockupInfo {
        uint256 releaseTime;
        uint256 unlockAmountPerDay;
    ...
    function releaseTimeLock(address _holder) internal returns(bool) {
        ...
        if (lockupInfo[_holder].lockupBalance <= lockupInfo[_holder].unlockAmountPerDay) {
            releaseAmount = lockupInfo[_holder].lockupBalance;
            delete lockupInfo[_holder];
            locks[_holder] = false;
        } else {            
            releaseAmount = lockupInfo[_holder].unlockAmountPerDay;
            lockupInfo[_holder].releaseTime = lockupInfo[_holder].releaseTime.add(DAY);
