...
contract SwapProxy is Ownable {
    ...
    function dataDecode(
        ...
        (address _c, SwapDescription memory desc, bytes memory _d) = abi.decode(data[4:], (address, SwapDescription, bytes));
        emit Test(_c, desc, _d);
    }
    ...
    function swap(uint minOut, bytes calldata _data) external onlyOwner {
        ...
        if (succ) {
            (uint returnAmount, uint gasLeft,) = abi.decode(_data, (uint, uint, uint));
