...
contract APTOKI is ERC2981, ERC721A, Ownable {
    ...
    uint256 public immutable _prePrice = 0.01 ether;
    uint256 public immutable _publicPrice = 0.0123 ether;
    ...
    string public _metadataURI = "https://aptoki.mypinata.cloud/ipfs/QmPqTUoQZ5iLxAELoyaBgK2KGsE8gb3Vd2Z1J9aUb3t3BX";
    ...
    function preMint(uint32 amount) external payable {
        if (_stage != Stage.PreSale) revert ErrorPreSaleNotStarted();
        if (amount + _totalMinted() > _maxSupply) revert ErrorExceedMaxSupply();
        if (amount > _txLimit) revert ErrorExceedTransactionLimit();

        uint256 requiredValue = amount * _prePrice;
    ...
    function publicMint(uint32 amount) external payable {
        if (_stage != Stage.PublicSale) revert ErrorPublicSaleNotStarted();
        if (amount + _totalMinted() > _maxSupply) revert ErrorExceedMaxSupply();
        if (amount > _txLimit) revert ErrorExceedTransactionLimit();

        uint256 requiredValue = amount * _publicPrice;
        uint64 userMinted = _getAux(msg.sender);
