...
contract ZhakNFT is ERC721A, Ownable, ReentrancyGuard, DefaultOperatorFilterer   {
  ...
  constructor() ERC721A("ZhakNFT", "ZHAK") {}
  ...
  function mint(uint256 tokens) public payable nonReentrant {
    require(!paused, "ZHAK: oops contract is paused");
    require(!preSale, "ZHAK: Sale Hasn't started yet");
    require(tokens <= MaxperWallet, "ZHAK: max mint amount per tx exceeded");
    require(totalSupply() + tokens <= maxSupply, "ZHAK: We Soldout");
    require(PublicMintofUser[_msgSenderERC721A()] + tokens <= MaxperWallet, "ZHAK: Max NFT Per Wallet exceeded");
    require(msg.value >= cost * tokens, "ZHAK: insufficient funds");
...
/// @dev presale mint for whitelisted
    ...
    require(!paused, "ZHAK: oops contract is paused");
    require(preSale, "ZHAK: Presale Hasn't started yet");
    require(MerkleProof.verify(merkleProof, merkleRoot, keccak256(abi.encodePacked(msg.sender))), "ZHAK: You are not Whitelisted");
    require(WhitelistedMintofUser[_msgSenderERC721A()] + tokens <= MaxperWallet, "ZHAK: Max NFT Per Wallet exceeded");
    require(tokens <= MaxperWallet, "ZHAK: max mint per Tx exceeded");
    require(totalSupply() + tokens <= maxSupply, "ZHAK: Whitelist MaxSupply exceeded");
    require(msg.value >= cost * tokens, "ZHAK: insufficient funds");
