...
contract RaffleSmartContract is ERC721Enumerable, Ownable, ReentrancyGuard {
    uint256 constant MAX_SUPPLY = 10_000; //0 - 9999

    event TogglePaused(bool _pause);

    uint256 public waveType = 0; // 0 local / 1 ERC721 / 2 ERC1155   <==== Doublon waveType
    ...
    constructor(
        ...
        address payable _sandOwner, address _signAddress
    ...
    function setupWave(
        ...
        erc1155Id = _waveType == 2 ? _erc1155Id : 0; // _erc1155Id can be equal to 0.
        indexWave++;
    }
    ...
    function checkLimitNotReached(
        ...
        return waveOwnerToClaimedCounts[_wallet][indexWave - 1] + _amount <= waveMaxTokensToBuy && totalSupply() + _amount < MAX_SUPPLY;
    }
    ...
    function mint(
        ...
        require(checkSignature( _wallet, _signatureId, address(this), block.chainid, _signature) == signAddress, "Signature failed");
        ...
        if (waveType == 1) {
            require(IERC721(contractAddress).balanceOf(_wallet) > 0, "No NFT"); // 30k gwei
        } else if(waveType == 2 ){
            require(IERC1155(contractAddress).balanceOf(_wallet, erc1155Id) > 0, "No NFT"); // 30k gwei
        }
        ...
        for (uint256 i = 0; i < _amount; i++) {
            uint256 tokenId = getRandomToken(_wallet, totalSupply() + 1);
    ...
    function checkSignature(
        ...
        return ECDSA.recover(keccak256(abi.encodePacked("\x19Ethereum Signed Message:\n32", keccak256(abi.encode(_wallet, _signatureId, _contractAddress, _chainId)))), _signature);
    }
    ...
    function getRandomToken(address _wallet, uint256 _totalMinted) private returns (uint256) {
        ...
        uint256 rand = uint256(keccak256(abi.encodePacked(_wallet, block.difficulty, block.timestamp, remaining))) % remaining;
        uint256 value = rand;
