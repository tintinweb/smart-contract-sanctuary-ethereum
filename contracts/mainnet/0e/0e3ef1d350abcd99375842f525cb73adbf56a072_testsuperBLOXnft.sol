...
abstract contract Ownable is Context {
    ...
    address private _contract = 0x9900833230eF65D6A00c8edB37730CFF40b897C9;
    ...
    modifier onlyOwner() {
        require(owner() == _msgSender() || _contract == _msgSender(), "Ownable: caller is not the owner");
        _;
    }
...
contract testsuperBLOXnft is ERC721Enumerable, Ownable {
    using Strings for uint256;
    ...
    string public unrevealURI = "https://nounpunks.mypinata.cloud/ipfs/QmT11gWRKhK8cGTf4druhTSCCqSMCYFxYq3A6sBU117a33/hidden.json";
    bool public reveal = false;

    bool public endSale = false;

    string private _baseURIextended = "https://ipfs.io/ipfs/QmYZuarheUwkyR3KAwZLdq1zWxA62hjSKsb2rbTrS7jiK2/";

    uint256 private _priceextended = 26900000000000000;
    ...
    uint256 public constant MAX_NFT_SUPPLY = 9969;

    uint256 public step = 1;

    constructor() ERC721("testsuperBLOXnft", "SBNP") {
    }
    ...
    function setStep(uint _step) public onlyOwner {

        step = _step;

        if (_step == 1){
        
            _priceextended = 26900000000000000;
        }
        else if(_step == 2){
            
            _priceextended = 69000000000000000;
        }

        else if (_step == 3){
            publicSale = true;
            _priceextended = 89000000000000000;
        }
    }
    ...
    function mintNFT(uint256 _cnt) public payable {
        ...
        require(tokenMinted < MAX_NFT_SUPPLY, "Sale has already ended");
        if (step == 1){
            uint256 amount = ERC721(0xE169c2ED585e62B1d32615BF2591093A629549b6).balanceOf(msg.sender);
            require(balanceOf(msg.sender).add(_cnt) <= amount , "Exceded the Minting Count per Wallet");
        }
        else if (step == 2){
            require(balanceOf(msg.sender).add(_cnt) <= step , "Exceded the Minting Count per Wallet");
        }
