...
contract NFTTester092111 is ICHNOPS, ERC721AQueryable, ERC721ABurnable, Ownable, ReentrancyGuard, ERC2981 {
  ...
  function masterName() public view returns (string memory) {
    return name();
  }

  function masterSymbol() public view returns (string memory) {
    return symbol();
  }
...
interface ICHNOPS {
  ...
  function totalMasterSupply() external view returns (uint256);
  function masterName() external view returns (string memory);
  function masterSymbol() external view returns (string memory);
