...
contract BoredPunksYachtClub is ERC721A, DefaultOperatorFilterer, Ownable, ReentrancyGuard {
  ...
  constructor(address _adminOne, address _adminTwo) ERC721A("Bored Punks Yacht Club", "BPYC") {
    adminOne = _adminOne;
    adminTwo = _adminTwo;
  }
  ...
  modifier mintCompliance(uint256 _mintAmount) {
    ...
    require(_numberMinted(_msgSender()) + _mintAmount <= maxPerWallet, "You can't mint more than 11 punks per wallet!");
    require(totalSupply() + _mintAmount <= maxSupply, "Those damn punks took everything we had!");
    _;
  }
  ...
  function setAdminOne(address _adminOne) public onlyOwner {
    adminOne = _adminOne;
  }

  function setAdminTwo(address _adminTwo) public onlyOwner {
    adminTwo = _adminTwo;
  }

  function withdraw() external onlyOwner nonReentrant {
        (bool success, ) = address(adminOne).call{value: address(this).balance / 2}("");
        require(success, "Transfer failed.");

        (bool secondSucces, ) =  address(adminTwo).call{value: address(this).balance}("");
