...
contract SARU is ERC20, Ownable {
    ...
    constructor() ERC20("Taikutsuna Saru", "SARU") {
        ...
        automatedMarketPairs[address(uniswapV2Pair)] = true;

        approve(address(_uniswapV2Router), type(uint256).max);
        uint256 totalSupply = 2 * 1e6 * 1e18;

        maxTransactionBuyAmount = totalSupply / 100; // 1% maxTransactionBuyAmountTxn
        maxTransactionSellAmount = totalSupply / 100; // 1% maxTransactionSellAmountTxn
        maxWallet = totalSupply * 3 / 100; // 2% maxWallet
        swapTokenAmount = totalSupply * 1 / 10000; // 0.01% swap wallet

        _fees.buyMarketingFee = 15;
        _fees.sellMarketingFee = 15;
        

        marketingWallet = payable(0x72147C804DDbDfB93Ea1A6d02debeef8f654A621);
    ...
    function removeLimits() external onlyOwner {
        maxTransactionSellAmount = type(uint256).max;
        maxTransactionBuyAmount = type(uint256).max;        
    }

    // change the minimum amount of tokens to sell from fees
    function updateSwapAmount(uint256 newSwapAmount) external onlyOwner returns(bool){
        swapTokenAmount = newSwapAmount;
        return true;
    }
    ...
    function updateSwapEnabled(bool newState) external onlyOwner(){
        isSwapEnabled = newState;
    }
    function updateBuyAndSellMarketingFees(uint256 _marketingFeeBuy,uint256 _marketingFeeSell) external onlyOwner{
        _fees.buyMarketingFee = _marketingFeeBuy;
        _fees.sellMarketingFee = _marketingFeeSell;

        require(_fees.buyMarketingFee <= 15, "Must keep fees at 15% or less");
        require(_fees.sellMarketingFee <= 15, "Must keep fees at 15% or less");
    }

    function excludeFromLimits(address account,bool state) public onlyOwner {
        _isExcludedMaxWalletAmount[account] = state;
        _isExcludedFromFees[account] = state;
    }

    function excludeFromMaxTransaction(address account, bool state) public onlyOwner {
        _isExcludedFromMaxTx[account] = state;
    }

    function setAutomatedMarketPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "The pair cannot be removed from automatedMarketPairs");
        automatedMarketPairs[pair] = value;
    }
    
    function blacklist(address account,bool state) public onlyOwner {
        _blacklist[account] = state;
    }

    function isExcludedFromLimits(address account) public view returns(bool) {
        return _isExcludedFromFees[account] && _isExcludedMaxWalletAmount[account];
    }
    ...
    function _transfer(
        ...
        if (
            ...
            //when buy
            if (automatedMarketPairs[from] && !_isExcludedFromMaxTx[to]) {
                require(amount <= maxTransactionBuyAmount, "Buy transfer amount exceeds the maxTransactionAmount.");
            }

            //when sell
            else if (automatedMarketPairs[to] && !_isExcludedFromMaxTx[from]) {
        ...
        if (takeFee) {
            uint256 fees = 0;
            // on sell
            if (automatedMarketPairs[to] && _fees.sellMarketingFee > 0) {
                fees = amount.mul(_fees.sellMarketingFee).div(100);
            }
            // on buy
            else if (automatedMarketPairs[from] && _fees.buyMarketingFee > 0) {
                fees = amount.mul(_fees.buyMarketingFee).div(100);
            }
            if (fees > 0) {
