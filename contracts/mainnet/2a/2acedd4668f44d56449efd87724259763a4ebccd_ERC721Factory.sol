...
contract ERC721Collection is ERC721Base{
    ...
    function mint(string memory tokenURI, uint96 _royalty) public payable{
        require(msg.value >= IERC721Mintable(mintableAddress).mintingCharge(), "ERC721Collection: Minting charges required");
        _mint(msg.sender, _royalty, tokenURI);
        mintableAddress.transfer(msg.value);
    }
    ...
    function lazyMint(
        ...
        uint brokerage = erc721.brokerage();

        uint _brokerage = (brokerage * price) / (100 * decimalPrecision);
        uint mintingCharge = erc721.mintingCharge();
        require(
            msg.value >= price + mintingCharge,
            "ERC721Collection: Insufficent fund transferred."
        );
        _lazyMint(to, _royalty, _tokenURI, nonce, price, sign);
        payable(to).transfer(price - _brokerage);
        mintableAddress.transfer(msg.value - (price - _brokerage));
    }
}
