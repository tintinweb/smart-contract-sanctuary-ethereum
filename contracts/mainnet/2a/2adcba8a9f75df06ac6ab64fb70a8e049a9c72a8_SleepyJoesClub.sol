...
contract SleepyJoesClub is ERC721, Ownable {
  using Strings for uint256;
  ...
  uint256 public maxPresaleMintAmount = 1;
  ...
  constructor(string memory _name, string memory _symbol, address _initProxyAddress,string memory _hiddenMetadataUri) ERC721(_name, _symbol) {
  ...
  function mint(uint256 _mintAmount) public payable mintCompliance(_mintAmount) {
    ...
    if (presale) { 
        if ( !isInWhiteList(msg.sender))  {
            revert("You are not Whitelisted, Please wait for the Public Sale");
        }
        
        if ( balanceOf(msg.sender)+_mintAmount > maxPresaleMintAmount)
            revert("You have minted the max Whitelisted amount");
    } 
  ...
  function setAddress(uint addy, address _address) public onlyOwner() {
        require (addy<1,"Choose address 0 or 1");   
        if (addy==0){
  ...
  function withdraw() public onlyOwner {
    (bool bw, ) = payable(owner()).call{value: address(this).balance}("");
    require(bw);
  }
