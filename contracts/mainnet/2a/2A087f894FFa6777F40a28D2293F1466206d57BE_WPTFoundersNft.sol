...
import {Address, ERC721, IERC165} from "@openzeppelin/contracts/token/ERC721/ERC721.sol";
...
abstract contract ERC721Enumerable is ERC721, IERC721Enumerable {
    ...
    function tokenOfOwner(address owner) public view returns (bytes32[] memory) {
...
contract WPTFoundersNft is ERC721Enumerable, Ownable, DefaultOperatorFilterer {
    ...
    uint256 public feeAmount = 1268 * 10 ** 6;
    uint256 public performanceFee = 0.0019 ether;
    uint256 public maxSupply = 1000;
    uint256 public maxLimit = 5;
    uint256 public preMintCount = 150;
    ...
    string private _galaxyName = "WAR PIGS BEASTIE FOUNDER COLLECTION";
    string private _galaxySymbol = "B3DF";
    ...
    /* ============ Constructor ============ */
    constructor() ERC721("", "") {
        _setBaseURI("ipfs://QmRQozmWm7uJHXkoWyguFoP4s6Lq8ar1eJduxBoVPywQ8t/");
    }
    ...
    function _transferFee(uint256 amount) internal {
        ...
        if (_feemintCount > 0) {
            ...
            require(IERC20(feeToken).balanceOf(msg.sender) >= _feeAmount, "Not Enough Fee");
    ...
    function mint(address account) external payable returns (uint256) {
        ...
        require(balanceOf(msg.sender) + 1 <= maxLimit, "Cannot exceed maxLimit");
        _transferFee(1);
    ...
    function burn(uint256 id) external {
        require(_isApprovedOrOwner(_msgSender(), id), "caller is not approved or owner");
        _burn(id);
        delete _cids[id];
    }
    ...
    function burnBatch(uint256[] calldata ids) external {
        for (uint256 i = 0; i < ids.length; i++) {
            require(_isApprovedOrOwner(_msgSender(), ids[i]), "caller is not approved or owner");
            _burn(ids[i]);
            delete _cids[ids[i]];
        }
    }
    ...
    function setWhiteLists(address[] memory _users, uint256[] memory _amounts) external onlyOwner {
        ...
        emit SetWhitelist(_users, _amounts);
    }
    ...
    function setTransferable(bool _transferable) external onlyOwner {
        ...
        emit SetTransferAble(_transferable);
    }

    function setEnableMint(bool _enable) external onlyOwner {
        enableMint = _enable;
        emit SetEnableMint(_enable);
    }
    ...
    function setPreMintCount(uint256 _newCount) external onlyOwner {
        ...
        emit SetPremintCount(_newCount);
    }
    ...
    function random(uint256 sum) private view returns (uint256) {
        return uint256(keccak256(abi.encodePacked(block.difficulty, block.timestamp + sum * 1000)));
    }
    ...
    function _getRandomMintNum(uint256 _numToMint, uint256 i) internal returns (uint256) {
        uint256 randomNum = uint256(
            keccak256(
                abi.encode(
                    msg.sender, tx.gasprice, block.number, block.timestamp, blockhash(block.number - 1), _numToMint, i
                )
            )
        );
