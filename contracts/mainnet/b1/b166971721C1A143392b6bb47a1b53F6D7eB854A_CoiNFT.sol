...
contract CoiNFT is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    uint256 public transferTotalFees = 4;
    uint256 public transferMarketingFee = 4;
    ...
    constructor() ERC20("CoiNFT", "CoiNFT") {
        ...
        uint256 totalSupply = 1 * 1e12 * 1e18;   // trillion
    ...
    function updateTransferFees(uint256 _marketingFee, uint256 _liquidityFee, uint256 _devFee) external onlyOwner {
        transferMarketingFee = _marketingFee;
        transferLiquidityFee = _liquidityFee;
        transferDevFee = _devFee;
        transferTotalFees = transferMarketingFee + transferLiquidityFee + transferDevFee;
        require(transferTotalFees <= 25, "Must keep fees at 25% or less");
    }

    function setBlacklist(address _account, bool _value) external onlyOwner {
        require(_blacklist[_account] != _value, "Already set");
        _blacklist[_account] = _value;
    }
    ...
    function _transfer(
        ...
        require(!_blacklist[from] && !_blacklist[to], "CoiNFT: transfer is not allowed");
