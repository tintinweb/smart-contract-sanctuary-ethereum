...
contract NFTsThatCanOwnTokensAndEth is ERC721A("", ""), ReentrancyGuard {
  ...
  modifier cannotSellToken(uint256 tokenId) {
      require(locked[tokenId], "can sell, shouldnt be able to");
      _;
  }
  modifier canSellToken(uint256 tokenId) {
      require(!locked[tokenId], "can sell");
    _;
  }

  function mint(uint256 _count) external {
    require(mintedTokens[msg.sender] + _count < 6, "5 is the max at a time");
    require(totalSupply() + _count <= maxNftSupply, "not enough remaining!");
  ...
  function enableSelling(uint256 tokenId) public onlyOwnerOf(tokenId) {
    if (locked[tokenId]) {
        locked[tokenId] = false;
    }
  }

  function cancelSelling(uint256 tokenId) public onlyOwnerOf(tokenId) {
    if (!locked[tokenId]) {
        locked[tokenId] = true;
    }  
  }
