...
contract AswangTribe is ERC721ABase {
    ...
    IAswang public immutable ASWANG_V0;
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        ...
        if (revealed) {
            return string(abi.encodePacked(
                isGenesis(tokenId) ? genesisURI : manananggalURI,
     ...
     * @dev Relays value from V0 contract
     */
    function tokenOffset() public view override returns (uint256) {
        return ASWANG_V0.tokenOffset();
    }
    ...
    function provenanceHash() public view override returns (string memory) {
        return ASWANG_V0.provenanceHash();
    }
    ...
    function startPraying(uint256[] calldata tokenIds) external {
        ...
        for (uint256 i; i < tokenIds.length;) {
            ...
            if (tokenInfo[tokenId].prayingStartedAt != 0) revert AswangTribe_TokenAlreadyPraying();
            if (!isGenesis(tokenId) && !tokenInfo[tokenId].hatched) revert AswangTribe_EggNotHatched();
    ...
    function stopPraying(uint256[] calldata tokenIds) external {
        for (uint256 i; i < tokenIds.length;) {
            ...
            _stopPraying(tokenId, _msgSender());
            unchecked { ++i; }
        }
    }

    /**
     * @notice Internal function to stop !praying
     * @param tokenId Token to stop !praying
     * @param dugoReceiver Address to receive the earned $DUGO
     */
    function _stopPraying(uint256 tokenId, address dugoReceiver) internal {
        if (address(Dugo) != address(0)) Dugo.claim(dugoReceiver, tokenId);
    ...
    function summon(uint256 tokenId, uint256 amount) external {
        if (!isGenesis(tokenId)) revert AswangTribe_InvalidTokenId();
        if (ownerOf(tokenId) != _msgSender()) revert AswangTribe_SenderNotTokenOwner();
        if (tokenInfo[tokenId].prayingStartedAt == 0) revert AswangTribe_TokenNotPraying();
    ...
    /**
     * @inheritdoc ERC721A
     * @dev Stop !praying and claim tokens to transfer receiver
     */
    function transferFrom(address from, address to, uint256 tokenId)
        public
        override(IERC721A, ERC721A)
    {
        if (tokenInfo[tokenId].prayingStartedAt > 0) _stopPraying(tokenId, to);
