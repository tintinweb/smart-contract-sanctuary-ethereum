...
contract DinoMarket is Ownable {
    IERC20 public Dino;
    ...
    function buyTicket(uint256 ticketId) public {
        ...
        uint256 allowance = Dino.allowance(msg.sender, address(this));
        require(allowance >= IdToTicket[ticketId].cost, "Check the token allowance");
        require(Dino.transferFrom(msg.sender, address(this), IdToTicket[ticketId].cost), "Failed to send");
    ...
    function tokenWithdraw() public onlyOwner {
        uint256 balance = Dino.balanceOf(address(this));
        Dino.transfer(msg.sender, balance);
    }
}