...
/// @title LIITA
/// @notice LIITA contract for Particle NFTs representing Love is in the air by Banksy
/// @dev Using ERC721Enumerable from OpenZeppelin for ERC-721 standard
contract LIITA is ERC721, ERC721Enumerable, ERC2981, Manageable {
    using Strings for uint256;

    string public LIITABaseURI;

    constructor(string memory liitaURI) ERC721("Genesis Particles", "GENPRTCL") {
        LIITABaseURI = liitaURI;
    }

    /// @dev Sets baseURI for LIITA
    /// @notice Only managers can call this function
    /// @param newURI new base URI for NFT metadata
    function setBaseLIITAURI(string memory newURI) external onlyManagers {
        LIITABaseURI = newURI;
    }
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        return bytes(LIITABaseURI).length > 0 ? string(abi.encodePacked(LIITABaseURI, tokenId.toString())) : "";
    }
