...
contract ArityClaimV1 {
    ...
    function calcClaim(bool isToPay, address _sender) public view returns(uint256){
        ...
        if (slot.silverTokens > 0) {
            slot.porcentByNftsSilver += porcentSilver[
                getIterationSilver(slot.silverTokens)
            ];
            slot.valueByNft5YearsSilver =
                BASE_PRICE +
    ...
    //function getNumOwnedTokens(address _colaboratorAddress)
    //    public
    //    view
    //    returns (uint256)
    //{
    //    return balanceOf(_colaboratorAddress);
    //}
    ...
    function getGoldPrice() public view returns (uint256) {
        //return s_goldPrice.getConversionRate(priceFeed);
        return s_goldPrice;
    }
