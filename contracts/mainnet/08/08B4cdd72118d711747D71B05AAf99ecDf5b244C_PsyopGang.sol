...
contract PsyopGang is Ownable, ERC721A, ReentrancyGuard, DefaultOperatorFilterer{
    string public CONTRACT_URI = "";
    mapping(address => uint) public userHasMinted;
    bool public REVEALED;
    string public UNREVEALED_URI = "";
    string public BASE_URI = "https://api.psyopgang.global/";
    bool public isPublicMintEnabled = false;
    uint public COLLECTION_SIZE = 10000; 
    uint public MINT_PRICE = 0.04 ether; 
    uint public MINT_PRICE_SUB = 0.02 ether; 
    uint public MAX_BATCH_SIZE = 25;
    uint public SUPPLY_PER_WALLET = 50;
    uint public FREE_SUPPLY_PER_WALLET = 2;

    address public token1address = 0x3007083EAA95497cD6B2b809fB97B6A30bdF53D3;
    address public token2address = 0x9bf1D7D63dD7a4ce167CF4866388226EEefa702E;

    constructor() ERC721A("PSYOP GANG", "PSYOP") {}

    function MintVIP(uint256 quantity, address receiver) public onlyOwner {
        require(
            totalSupply() + quantity <= COLLECTION_SIZE,
            "No more PSYOP in stock!"
        );
        ...
        nonReentrant
    {
        ...
        free = FREE_SUPPLY_PER_WALLET - userHasMinted[msg.sender];

        uint token1 = IERC20(token1address).balanceOf(msg.sender);
        uint token2 = IERC20(token2address).balanceOf(msg.sender);

        if (token1 > 0 || token2 > 0) {
            if (quantity >= free) {
                price = (MINT_PRICE_SUB) * (quantity - free);
            ...
            price = MINT_PRICE * quantity;
        }

        require(isPublicMintEnabled, "PSYOP not ready yet!");
        require(totalSupply() + quantity <= COLLECTION_SIZE, "No more PSYOP left!");

        require(balanceOf(msg.sender) + quantity <= SUPPLY_PER_WALLET, "Tried to mint PSYOP over over limit");

        require(quantity <= MAX_BATCH_SIZE, "Tried to mint PSYOP over limit, retry with reduced quantity");
