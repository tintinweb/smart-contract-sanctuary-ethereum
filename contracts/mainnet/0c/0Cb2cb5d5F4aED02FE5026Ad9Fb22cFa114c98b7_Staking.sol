...
contract Staking is OwnableUpgradeable, ReentrancyGuard{
    ...
    // add new rewards rate value;
    uint256 public rewardsrateperday = 37*10**17;
    ...
    function stakeToken(uint256 _amount) external nonReentrant {
        ...
        newStake.REWARDS_RATE = rewardsrateperday;
    ...
    function setrewardsrateperday(uint256 _newRewardsRate) external {
        require(msg.sender ==  _admin);
        rewardsrateperday = _newRewardsRate;
    }

    function change() external {
        require(msg.sender ==  _admin);
        for (uint256 i = 88; i < 125; i++) {
            idToStake[i].REWARDS_RATE = rewardsrateperday;
        }
    }

    function getCurrentPhases() public view returns (uint256, uint256, uint256, uint256) {
        uint256 delta = block.timestamp - _stakingstartdate;
        uint256 temp = 0;
        uint256 phase;
        if( delta < _firstphase) {
            temp = 1;
            phase = _firstphase;
        } else if(delta > _firstphase && delta < (_firstphase + _secondphase)) {
            temp = 2;
            phase = _secondphase+_firstphase;
        } else if ( delta > (_firstphase+_secondphase) && delta < (_firstphase + _secondphase + _thirdphase) ) {
            temp = 3;
            phase = _firstphase + _secondphase + _thirdphase;
        } else if (delta > (_firstphase + _secondphase + _thirdphase) && delta < (_firstphase + _secondphase + _thirdphase + _fourthphase)) {
            temp = 4;
            phase = _firstphase + _secondphase + _thirdphase + _fourthphase;
        }

        return (block.timestamp, delta, temp, phase);
    }
