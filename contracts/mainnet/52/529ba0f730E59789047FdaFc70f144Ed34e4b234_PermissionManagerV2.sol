...
contract PermissionManagerV2 is Initializable, AccessControlUpgradeable, PermissionManagerStorage {
     ...
     *
     * @param _permissionItems The address of the new Permission module.
     */
    function initialize(address _permissionItems, address _admin) external initializer {
     ...
     */
    function assignTier1(address[] calldata _accounts) external onlyPermissionsAdmin {
     ...
     */
    function assignTier2(UserProxy[] calldata _usersProxies) external onlyPermissionsAdmin {
     ...
     */
    function suspendUser(UserProxy[] calldata _usersProxies) external onlyPermissionsAdmin {
        for (uint256 i = 0; i < _usersProxies.length; i++) {
            UserProxy memory userProxy = _usersProxies[i];
     ...
     *                      but proxy can be optional if it is set to zero address.
     */
    function rejectUser(UserProxy[] calldata _usersProxies) external onlyPermissionsAdmin {
        for (uint256 i = 0; i < _usersProxies.length; i++) {
            UserProxy memory userProxy = _usersProxies[i];
     ...
     */
    function revokeTier1(address[] calldata _accounts) external onlyPermissionsAdmin {
        for (uint256 i = 0; i < _accounts.length; i++) {
     ...
     *                      An array of the struct UserProxy where user and proxy are bout required.
     */
    function revokeTier2(UserProxy[] calldata _usersProxies) external onlyPermissionsAdmin {
        for (uint256 i = 0; i < _usersProxies.length; i++) {
            UserProxy memory userProxy = _usersProxies[i];
     ...
     *                      but proxy can be optional if it is set to zero address.
     */
    function unsuspendUser(UserProxy[] calldata _usersProxies) external onlyPermissionsAdmin {
        for (uint256 i = 0; i < _usersProxies.length; i++) {
            UserProxy memory userProxy = _usersProxies[i];
     ...
     *                      but proxy can be optional if it is set to zero address.
     */
    function unrejectUser(UserProxy[] calldata _usersProxies) external onlyPermissionsAdmin {
        for (uint256 i = 0; i < _usersProxies.length; i++) {
            UserProxy memory userProxy = _usersProxies[i];
     ...
     * @param _accounts The addresses to assign Tier1.
     */
    function assignItem(uint256 _itemId, address[] calldata _accounts) external onlyPermissionsAdmin {
        for (uint256 i = 0; i < _accounts.length; i++) {
     ...
     * @param _accounts The addresses to assign Tier1.
     */
    function removeItem(uint256 _itemId, address[] calldata _accounts) external onlyPermissionsAdmin {
        for (uint256 i = 0; i < _accounts.length; i++) {
    ...
    function editLastSecurityTokenId(uint256 _newId) external onlyPermissionsAdmin {
        require(_newId > 99, "PermissionManager: security id needs to be larger than 99");
    ...
    function generateSecurityTokenId(address _tokenContract) external onlyPermissionsAdmin returns (uint256) {
        require(_tokenContract != address(0), "PermissionManager: Invalid token address");
        require(securityTokens[_tokenContract] == 0, "PermissionManager: Id already generated for this token");
    ...
    function hasSecurityToken(address _user, uint256 _itemId) external view returns (bool) {
        // require(_itemId > 99, "PermissionManager: _itemId too small to be security");
        return PermissionItems(permissionItems).balanceOf(_user, _itemId) > 0;
    }
}