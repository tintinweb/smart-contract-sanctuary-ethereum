...
abstract contract DefaultOperatorFilterer is OperatorFilterer {
    ...
    contract Prints is ERC721A, DefaultOperatorFilterer , Ownable {
    using Strings for uint256;
  ...
  constructor(string memory _uriPrefix) ERC721A("Prints", "PRINTS") {
      uriPrefix = _uriPrefix;
        _safeMint(msg.sender , 1);
  }
 
  function print(uint8 _mintAmount) external payable  {
  ...
  function resmint(uint16 _mintAmount, address _receiver) external onlyOwner {
     uint16 totalSupply = uint16(totalSupply());
    require(totalSupply + _mintAmount <= maxSupply, "Exceeds max supply.");
