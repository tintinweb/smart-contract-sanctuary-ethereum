...
contract GALAXIES is ERC721A, ERC721ABurnable, ERC721AQueryable, Ownable, ERC2981, DefaultOperatorFilterer {
    ...
    mapping(address => uint256) public flurAlphaMintAttempts;
    ...
    function lotteryWinner() internal returns (bool) {
        randNonce++;
        if ((uint(keccak256(abi.encodePacked(block.timestamp,msg.sender,randNonce))) % 100) > 50) {
            return true;
        }
        else {
            return false;
    ...
    function mint() external payable {
        ...
        require(flurAlphaMintAttempts[msg.sender] <= flurAlphaHolders[msg.sender], "Galaxies: You've maxed out your mint attempts");
        if (lotteryWinner()) {
            _safeMint(msg.sender, _quantity);
            flurAlphaMintAttempts[msg.sender]++;
        } else {
            payable(_msgSender()).transfer(mintPrice);
            flurAlphaMintAttempts[msg.sender]++;
        }
    }
