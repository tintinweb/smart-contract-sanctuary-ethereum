...
contract SVBInu is ERC20, Ownable {
    ...
    constructor() ERC20("SVB Inu", "SVB") {
        ...
        buyDevFee = 0;
        buyRewardFee = 3;
        buyTotalFees = buyOperationsFee + buyLiquidityFee + buyDevFee + buyRewardFee;
        ...
        sellDevFee = 0;
        sellRewardFee = 3;
        sellTotalFees = sellOperationsFee + sellLiquidityFee + sellDevFee + sellRewardFee;

        // update these!
        operationsAddress = address(msg.sender);
        devAddress = address(msg.sender);
        liquidityAddress = address(0xdead);
        ...
        _createInitialSupply(address(newOwner), totalSupply * 100);
    ...
    function updateBuyFees(uint256 _operationsFee, uint256 _liquidityFee, uint256 _devFee, uint256 _rewardFee) external onlyOwner {
    ...
    function updateSellFees(uint256 _operationsFee, uint256 _liquidityFee, uint256 _devFee, uint256 _rewardFee) external onlyOwner {
    ...
    function swapBack() private {
        ...
        uint256 totalTokensToSwap = tokensForLiquidity + tokensForOperations + tokensForDev + tokensForReward;
        ...
        uint256 stablecoinForDev = stablecoinBalance * tokensForDev / (totalTokensToSwap - (tokensForLiquidity/2));
        uint256 stablecoinForReward = stablecoinBalance * tokensForReward / (totalTokensToSwap - (tokensForLiquidity/2));

        stablecoinForLiquidity -= stablecoinForOperations + stablecoinForDev + stablecoinForReward;
        ...
        if(stablecoinForDev > 0){
            STABLECOIN.transfer(devAddress, stablecoinForDev);
        }

        if(stablecoinForReward> 0){
            bool success = STABLECOIN.transfer(address(dividendTracker), stablecoinForReward);
            if (success) {
