...
// File: contracts/TAXHAVENINU_Staking.sol
...
contract TAXHAVENINU_STAKING is Ownable {
    IERC20 public TOKEN;
    ...
    function getTotalStakersAmount() public view returns (uint256) {
    ...
    function stake(uint256 tokenAmount) external {
        ...
        if (tokenAmount > 0) {
            if (shares[msg.sender].amount == 0) addShareholder(msg.sender);
            TOKEN.transferFrom(msg.sender, address(this), tokenAmount);
            totalShares = totalShares + tokenAmount;
        }

        shares[msg.sender].amount = shares[msg.sender].amount + tokenAmount;
        shares[msg.sender].excluded = getCumulativeDividends(shares[msg.sender].amount);
    }
    ...
    function unstake(uint256 tokenAmount) external {
        ...
        if (shares[msg.sender].amount <= tokenAmount) {
    ...
    receive() external payable {
        ...
        dividendsPerShare = totalDividends / totalShares;
    }
    ...
    function withdrawETH(address _addr) external onlyOwner {
        ...
        totalDividends = totalDividends - balance;
        dividendsPerShare = totalDividends / totalShares;
    }
}