...
contract bunnies is ERC721Enumerable, Ownable, RandomlyAssigned {
  using Strings for uint256;
  
  string public baseExtension = ".json";
  uint256 public cost = 0 ether;
  uint256 public maxBR = 10000;
  uint256 public maxMintAmount = 10;
  bool public paused = false;
  
  string public baseURI = "https://bunniez.wtf/api/data/";
  ...
  function mint(uint256 _mintAmount) public payable {
    ...
    require(totalSupply() + _mintAmount <= maxBR);
    require(msg.value >= cost * _mintAmount);

    for (uint256 i = 1; i <= _mintAmount; i++) {
        uint256 mintIndex = nextToken();
     if (totalSupply() < maxBR) {
