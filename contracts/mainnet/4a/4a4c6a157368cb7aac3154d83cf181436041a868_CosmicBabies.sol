...
contract CosmicBabies is ERC721A, Ownable, ReentrancyGuard {
    ...
    uint256 constant cutRequired = 1300 ether;
    string public baseURI;
    string public baseExtension = ".json";

    constructor() ERC721A("Cosmic Babies", "CBABIES", 30) {
        
    }

    function init(string memory _initBaseURI, address clabsAddress, address cutAddress) public onlyOwner
    {
        setBaseURI(_initBaseURI);
    ...
    function createBabies(uint256[] memory tokenIds, uint256 mintAmount) public nonReentrant onlySender
    {
        require(tokenIds.length == 2, "You are sending incorrect amount of token ids");
        require(cosmicNft.ownerOf(tokenIds[0]) == msg.sender && cosmicNft.ownerOf(tokenIds[1]) == msg.sender, "You are not the owner of these ducks");
        require(tokenIds[0] <= 1000 && tokenIds[1] <= 1000, "These are not genesis!");
        require(cosmictoken.balanceOf(msg.sender) >= (cutRequired * mintAmount), "You Don't Have enough Cosmic Tokens!");  
        require((totalSupply() + mintAmount) <= maxSupply, "All Babies Are Born");
        
        cosmictoken.transferFrom(msg.sender, address(this), (cutRequired * mintAmount));
