...
// File contracts/PixelatedGreatestGoats.sol


contract PixelatedGreatestGoats is ERC721A, Ownable {

    string public baseURI = "ipfs://QmXszTJoGN6dA6KyCz1X33wxdGydvxWoopBDdxExgBvmDj/";
    string public contractURI = "ipfs://QmVS3YVAjesZZJUhHYaRpcQRxRdbJCdeKsmuKUhoL7GGUe";

    uint256 public constant MAX_PER_TX_FREE = 1;
    uint256 public FREE_MAX_SUPPLY = 5900;
    uint256 public constant MAX_PER_TX = 10;
    uint256 public constant MAX_PER_WALLET = 0;
    uint256 public constant MAX_SUPPLY = 5900;
    uint256 public price = 0.005 ether;

    bool public paused = true;

    constructor() ERC721A("Pixelated Greatest Goats", "PGG") {}
    ...
    function freeMint(uint256 _amount) external payable {
        ...
        require(MAX_SUPPLY >= totalSupply() + _amount, "Exceeds max supply");
        require(tx.origin == _caller, "No contracts");
        require(MAX_PER_TX_FREE >= uint256(_getAux(_caller)) + _amount, "Excess max per free wallet");
        require(FREE_MAX_SUPPLY - _amount >= 0, "No more free mints, sorry <3");

        _setAux(_caller, uint64(_amount));
        _safeMint(_caller, _amount);
        unchecked{ FREE_MAX_SUPPLY -= _amount; }
    }
