...
0xSHARK v(Perfect Cell) by 0xSHARKDEV
 */
...
contract SHARKS is ERC721A, Ownable {
    ...
    uint256 public Price = 0.02 ether; //price
    ...
    constructor() ERC721A("0xsharks", "0xsharks", MAX_TOKENS_PER_MINT,MAX_SUPPLY) {
    }
    // Used to set price
    function setPrice(uint256 _newPrice) public onlyOwner {
        Price = _newPrice;
        emit PriceChanged(Price,block.timestamp);
    }
    ...
    function mint(uint256 tokens) external payable {
        ...
        require(tokens <= MAX_TOKENS_PER_MINT, "Cannot purchase this many tokens in a transaction");
        require(totalSupply() + tokens <= MAX_SUPPLY, "max supply reached");
        require(tokens > 0, "cannot mint 0 sharkx");
        require(msg.value >= Price * tokens, "not enough eth");
         _safeMint(msg.sender, tokens);
    }
    // dev mint function - should be executed before publicsale
    function mintDev(uint256 DevTokens) public onlyOwner {
        require(totalSupply() + MAX_DEV_MINT <= MAX_SUPPLY, "max supply reached");
        require(DevTokens % MAX_TOKENS_PER_MINT == 0,"can only mint a multiple of 20");
        uint256 numChunks = DevTokens / MAX_TOKENS_PER_MINT;
        for (uint256 i = 0; i < numChunks; i++) {
          _safeMint(msg.sender, MAX_TOKENS_PER_MINT);
        }
    }
    /// withdraw 
