...
contract LendingPool is VersionedInitializable, ILendingPool, LendingPoolStorage {
  ...
  uint256 public constant LENDINGPOOL_REVISION = 0x15;
  ...
  // RoeLabs: Added borrowCap and supplyCap adjustable by poolAdmin
  ...
  function deposit(
    ...
    // RoeLabs: Rev 0x14 Added check to limit supply to admin-capped amount
    require( IERC20(aToken).totalSupply() <= supplyCap[asset], "Exceeded Supply Cap" ); 
  ...
  **/
  ...
  // RoeLabs: New function, from rev 0x14
  event BorrowCapUpdated(address _asset, uint256 _cap);
  event SupplyCapUpdated(address _asset, uint256 _cap);

  // RoeLabs: New function, from rev 0x14
  /**
   * @dev Set supply cap for an asset for the Roe Protocol
   * @param _asset Address of asset
   * @param _supplyCap Amount to limit supply to
   * @param _borrowCap Amount to limit borrow to
  **/
  function setCap(
    address _asset,
    uint256 _supplyCap,
    ...
    require(_addressesProvider.getPoolAdmin() == msg.sender, Errors.CALLER_NOT_POOL_ADMIN);
    supplyCap[_asset] = _supplyCap;
    borrowCap[_asset] = _borrowCap;
    emit SupplyCapUpdated(_asset, _supplyCap);
    emit BorrowCapUpdated(_asset, _borrowCap);
  }
  ...
  function flashLoan(
    ...
    for (vars.i = 0; vars.i < assets.length; vars.i++) {
      ...
      // RoeLabs: Enable flash loan premiums only for non-PMs
      if (pm[msg.sender] == false) {
  ...
  function _executeBorrow(ExecuteBorrowParams memory vars) internal {
    ...
    // RoeLabs: Rev 0x14 Added check to limit borrows to admin-capped amount
    require( IERC20(reserve.variableDebtTokenAddress).totalSupply() <= borrowCap[vars.asset], "EBC" ); 
