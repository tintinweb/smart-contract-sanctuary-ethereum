...
contract EthicalMiladys is ERC721A, Ownable{ 
    using Strings for uint256;
    ...
    uint256 public UKRAINE_PERCENTAGE = 80;
    address public constant UKRAINE_ADDRESS = 0x165CD37b4C644C2921454429E7F9358d18A45e14;

    mapping(address => uint256) private freeMintAddressData;

    constructor(uint256 price, uint256 maxSupply, string memory baseUri, uint256 freeMintLimitPerWallet, uint256 maxMintPerTX, bool isSaleActive, uint256 freeMintSupply) ERC721A("EthicalMiladys", "ETHICALMILADYS"){
        PRICE = price;
    ...
    function lowerMaxSupply(uint256 newMaxSupply) external onlyOwner{
        require (newMaxSupply < MAX_SUPPLY, "Too large");
        require (newMaxSupply >= _currentIndex, "Too small");
    ...
    modifier mintRegulation(uint256 _mintAmount){
        require(_mintAmount > 0 && _mintAmount <= MAX_MINT_PER_TX, "Amount < 1 or > than max mint");
    ...
    function mint(uint256 _mintAmount) public payable mintRegulation(_mintAmount) {
        require(IS_SALE_ACTIVE, "Sale inactive");

        uint256 totalPayable = _mintAmount * PRICE;

        if (_currentIndex < FREE_MINT_SUPPLY) {
            uint256 remainingFreeMint = FREE_MINT_LIMIT_PER_WALLET - freeMintAddressData[msg.sender];
    ...
    function withdraw() public onlyOwner{
        ...
        require(balance > 0);
        // 80% of withdrawal is sent to ukraine.eth
        uint256 ukraineShare = balance * UKRAINE_PERCENTAGE / 100;
        
        payable(UKRAINE_ADDRESS).transfer(ukraineShare);
