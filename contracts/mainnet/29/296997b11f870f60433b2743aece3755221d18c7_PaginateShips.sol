...
import "./Miaocraft.sol";

contract PaginateShips {
    function paginateShips(
        address miaocraft_,
        uint256 offset,
        uint256 limit
    ) public view returns (ShipInfoExtended[] memory shipInfos) {
        Miaocraft miaocraft = Miaocraft(miaocraft_);
        IERC20Resource butter = miaocraft.butter();
        ISpatialSystem spatialSystem = miaocraft.spatialSystem();
        uint256 total = miaocraft.nextId();
        if (offset >= total) {
            return shipInfos;
        }
        ...
        for (uint256 id = offset; id < end; id++) {
            try miaocraft.ownerOf(id) returns (address owner) {
                uint256 entityId = tokenToEntity(miaocraft_, id);
                shipInfos[id - offset] = ShipInfoExtended({
                    id: id,
                    owner: owner,
                    balance: butter.balanceOf(entityId),
                    shipInfo: miaocraft.getShipInfo(id),
                    locationInfo: spatialSystem.getLocationInfo(entityId)
                });
            } catch {}
        }
    }
