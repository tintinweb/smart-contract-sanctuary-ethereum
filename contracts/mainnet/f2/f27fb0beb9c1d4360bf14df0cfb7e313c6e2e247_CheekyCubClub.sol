...
contract CheekyCubClub is ERC721("Cheeky Cub Club", "CCC") {
    ...
    mapping(uint => uint) BreededLion;
    mapping(uint => uint) BreededCougar;
    ...
    {
        require(
            isBreedCubActive && circulatingSupply <= 10333,
            "Breed is not active"
        );
    ...
    // ✅ done Checked
    function startBreedCub() external onlyOwner {
        isBreedCubActive = true;
    }
    // ✅ done Checked
    function stopBreedCub() external onlyOwner {
        isBreedCubActive = false;
    }
        ...
        tokensAvailable(_howMany)
    {
        ...
        require( circulatingSupply <= 10333, "can't mint more then circulating supply" );
