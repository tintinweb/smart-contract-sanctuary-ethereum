...
contract wwStaking is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;

    struct UserInfo {
        uint256 stakedAmount; // How many ww tokens the user has provided.
        ...
        updateReward(msg.sender)
    {
        ...
        IERC20(wwToken).safeTransferFrom(msg.sender, address(this), _amount);
  ...
  **/
        ...
        updateReward(msg.sender)
    {
        ...
        if (isEmergency) {
            ...
            IERC20(wwToken).safeTransfer(msg.sender, outAmount);
    ...
    function claim() external nonReentrant {
        ...
        require(
            IERC20(wwToken).balanceOf(address(this)) >= receiveAmount,
            "staking contract has not enough ww token"
        );

        IERC20(wwToken).safeTransfer(msg.sender, receiveAmount);
        totalStakedAmount = IERC20(wwToken).balanceOf(address(this)).sub(
            receiveAmount
        );
