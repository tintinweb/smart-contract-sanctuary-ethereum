...
contract ServiceV19 {
  ...
  function getRewardAll(address entity, uint256 blockNumber) external view returns (uint256) {
  ...
  function approveBYONNode(address entity, uint128 nodeId) external {
    require(msg.sender == admin || msg.sender == serviceAdmin || msg.sender == superAdmin);
  ...
  function suspendBYONNode(address entity, uint128 nodeId) external {
    require(msg.sender == admin || msg.sender == serviceAdmin || msg.sender == superAdmin);
  ...
  function setNodeIsActive(address entity, uint128 nodeId, bool isActive) external {
    require(msg.sender == admin || msg.sender == serviceAdmin || msg.sender == superAdmin);
    bytes memory id = getNodeId(entity, nodeId);
  ...
  function setNodeIsNaaS(address entity, uint128 nodeId, bool isNaaS) external {
    require(msg.sender == admin || msg.sender == serviceAdmin || msg.sender == superAdmin);
    bytes memory id = getNodeId(entity, nodeId);
  ...
  function claimAll(uint256 blockNumber, bool toStrongPool) external payable {
  ...
  function payAll(uint256 nodeCount) external payable {
