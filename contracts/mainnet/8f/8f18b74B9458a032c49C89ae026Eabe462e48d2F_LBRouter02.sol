...
import "./ILBRouter02.sol";
import "./IERC20Permit.sol";
import "./ILBPool.sol";
import "./IWETH.sol";
import "./LBLibrary.sol";

contract LBRouter02 is ILBRouter02 {
    ...
    modifier ensure(uint256 deadline) {
        require(deadline >= block.timestamp, "LBRouter: EXPIRED");
        _;
    }
    ...
    function _addLiquidity(
        ...
        ) = LBLibrary.getTradeInfo(pool, tokenA, tokenB);
        if (reserveA == 0 && reserveB == 0) {
            (amountA, amountB) = (amountADesired, amountBDesired);
        } else {
            uint256 amountBOptimal = LBLibrary.quote(
                amountADesired,
                reserveA,
                reserveB
            );
            if (amountBOptimal <= amountBDesired) {
                require(
                    ...
                    "LBRouter: INSUFFICIENT_B_AMOUNT"
                );
                (amountA, amountB) = (amountADesired, amountBOptimal);
            } else {
                uint256 amountAOptimal = LBLibrary.quote(
                    amountBDesired,
                    reserveB,
                    reserveA
                );
                ...
                require(
                    ...
                    "LBRouter: INSUFFICIENT_A_AMOUNT"
                );
            ...
            require(
                ...
                "LBRouter: OUT_OF_BOUNDS_VRESERVE"
            );
        }
    }
        ...
        )
    {
        ...
        liquidity = ILBPool(pool).mint(to);
    }
        ...
        )
    {
        ...
        if (ampBpsAndFeeBps[0] == BPS) {
            pool = ILBFactory(factory).getUnamplifiedPool(tokenA, tokenB);
        }
        if (pool == address(0)) {
            pool = ILBFactory(factory).createPool(
                tokenA,
                tokenB,
                ampBpsAndFeeBps[0],
        ...
        )
    {
        address pool;
        if (ampBpsAndFeeBps[0] == BPS) {
            pool = ILBFactory(factory).getUnamplifiedPool(token, weth);
        }
        if (pool == address(0)) {
            pool = ILBFactory(factory).createPool(
                token,
        ...
        returns (uint256 amountA, uint256 amountB)
    {
        ...
        (uint256 amount0, uint256 amount1) = ILBPool(pool).burn(to);
        (IERC20 token0, ) = LBLibrary.sortTokens(tokenA, tokenB);
        (amountA, amountB) = tokenA == token0
            ? (amount0, amount1)
            : (amount1, amount0);
        require(amountA >= amountAMin, "LBRouter: INSUFFICIENT_A_AMOUNT");
        require(amountB >= amountBMin, "LBRouter: INSUFFICIENT_B_AMOUNT");
    }
    ...
    function _swap(
        ...
        for (uint256 i; i < path.length - 1; i++) {
            ...
            (IERC20 token0, ) = LBLibrary.sortTokens(input, output);
            uint256 amountOut = amounts[i + 1];
            (uint256 amount0Out, uint256 amount1Out) = input == token0
                ? (uint256(0), amountOut)
                : (amountOut, uint256(0));
            address to = i < path.length - 2 ? poolsPath[i + 1] : _to;
            ILBPool(poolsPath[i]).swap(
                amount0Out,
                amount1Out,
                to,
                new bytes(0)
        ...
        returns (uint256[] memory amounts)
    {
        ...
        amounts = LBLibrary.getAmountsOut(amountIn, poolsPath, path);
        require(
            amounts[amounts.length - 1] >= amountOutMin,
            "LBRouter: INSUFFICIENT_OUTPUT_AMOUNT"
        );
    ...
    function swapTokensForExactTokens(
        ...
        verifyPoolsPathSwap(poolsPath, path);
        amounts = LBLibrary.getAmountsIn(amountOut, poolsPath, path);
        require(amounts[0] <= amountInMax, "LBRouter: EXCESSIVE_INPUT_AMOUNT");
    ...
    function swapExactETHForTokens(
        ...
        returns (uint256[] memory amounts)
    {
        require(path[0] == weth, "LBRouter: INVALID_PATH");
        verifyPoolsPathSwap(poolsPath, path);
        amounts = LBLibrary.getAmountsOut(msg.value, poolsPath, path);
        require(
            amounts[amounts.length - 1] >= amountOutMin,
            "LBRouter: INSUFFICIENT_OUTPUT_AMOUNT"
        );
    ...
    function swapTokensForExactETH(
        ...
        require(path[path.length - 1] == weth, "LBRouter: INVALID_PATH");
        verifyPoolsPathSwap(poolsPath, path);
        amounts = LBLibrary.getAmountsIn(amountOut, poolsPath, path);
        require(amounts[0] <= amountInMax, "LBRouter: EXCESSIVE_INPUT_AMOUNT");
        path[0].safeTransferFrom(msg.sender, poolsPath[0], amounts[0]);
        ...
        returns (uint256[] memory amounts)
    {
        ...
        verifyPoolsPathSwap(poolsPath, path);
        amounts = LBLibrary.getAmountsIn(amountOut, poolsPath, path);
        require(amounts[0] <= msg.value, "LBRouter: EXCESSIVE_INPUT_AMOUNT");
        IWETH(weth).deposit{value: amounts[0]}();
        weth.safeTransfer(poolsPath[0], amounts[0]);
        _swap(amounts, poolsPath, path, to);
        // refund dust eth, if any
    ...
    function _swapSupportingFeeOnTransferTokens(
        ...
        for (uint256 i; i < path.length - 1; i++) {
            (IERC20 input, IERC20 output) = (path[i], path[i + 1]);
            (IERC20 token0, ) = LBLibrary.sortTokens(input, output);
            ILBPool pool = ILBPool(poolsPath[i]);
            uint256 amountOutput;
            {
                ...
                ) = LBLibrary.getTradeInfo(poolsPath[i], input, output);
                uint256 amountInput = IERC20(input)
                    .balanceOf(address(pool))
                    .sub(reserveIn);
                amountOutput = LBLibrary.getAmountOut(
                    amountInput,
                    reserveIn,
                    reserveOut,
                    vReserveIn,
    ...
    function quote(
        ...
        return LBLibrary.quote(amountA, reserveA, reserveB);
    }
    ...
    function getAmountsOut(
        ...
        verifyPoolsPathSwap(poolsPath, path);
        return LBLibrary.getAmountsOut(amountIn, poolsPath, path);
    }
    ...
    function getAmountsIn(
        ...
        verifyPoolsPathSwap(poolsPath, path);
        return LBLibrary.getAmountsIn(amountOut, poolsPath, path);
    }
    ...
    function verifyPoolsPathSwap(
        ...
        require(path.length >= 2, "LBRouter: INVALID_PATH");
        require(
            poolsPath.length == path.length - 1,
            "LBRouter: INVALID_POOLS_PATH"
        );
    ...
    function verifyPoolAddress(
        ...
        require(
            ILBFactory(factory).isPool(tokenA, tokenB, pool),
            "LBRouter: INVALID_POOL"
        );
    }
}
...
library LBLibrary {
    ...
    {
        require(tokenA != tokenB, "LBLibrary: IDENTICAL_ADDRESSES");
        (token0, token1) = tokenA < tokenB ? (tokenA, tokenB) : (tokenB, tokenA);
        require(address(token0) != address(0), "LBLibrary: ZERO_ADDRESS");
    }
        ...
        )
    {
        ...
        uint256 vReserve0;
        uint256 vReserve1;
        (reserve0, reserve1, vReserve0, vReserve1, feeInPrecision) = ILBPool(pool).getTradeInfo();
    ...
    function getReserves(
        ...
        (IERC20 token0, ) = sortTokens(tokenA, tokenB);
        (uint256 reserve0, uint256 reserve1) = ILBPool(pool).getReserves();
    ...
    function quote(
        ...
        require(amountA > 0, "LBLibrary: INSUFFICIENT_AMOUNT");
        require(reserveA > 0 && reserveB > 0, "LBLibrary: INSUFFICIENT_LIQUIDITY");
    ...
    function getAmountOut(
        ...
        require(amountIn > 0, "LBLibrary: INSUFFICIENT_INPUT_AMOUNT");
        require(reserveIn > 0 && reserveOut > 0, "LBLibrary: INSUFFICIENT_LIQUIDITY");
        ...
        require(reserveOut > amountOut, "LBLibrary: INSUFFICIENT_LIQUIDITY");
    }
    ...
    function getAmountIn(
        ...
        require(amountOut > 0, "LBLibrary: INSUFFICIENT_OUTPUT_AMOUNT");
        require(reserveIn > 0 && reserveOut > amountOut, "LBLibrary: INSUFFICIENT_LIQUIDITY");
...
import "./ILBRouter01.sol";

interface ILBRouter02 is ILBRouter01 {
    function removeLiquidityETHSupportingFeeOnTransferTokens(
        IERC20 token,
        address pool,
        uint256 liquidity,
