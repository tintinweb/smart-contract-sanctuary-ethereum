...
contract NexusVoyagers is ERC721A, IERC2981, Ownable, ReentrancyGuard, DefaultOperatorFilterer {
    ...
    // ~~~ ====> Admin
    address public withdrawalAddress = 0xEc8773481C1222b012DEcbE76e8EfEA71E15F940;
    ...
    constructor(
		...
		uint256 numToMint,
		bytes32 _merkle
    ) ERC721A("Nexus Voyagers", "VOYAGER") {
    ...
    modifier crewSpotsAvailable(uint256 numToMint) {
        require(
            totalSupply() + numToMint <= MAX_CREW_SIZE,
     ...
     * @dev Checks for the correct amount of ETH
     */
    modifier eligibleForPreboarding(uint256 numToMint, bytes32[] calldata _merkleProof) {
        require(
     ...
     */
    function preMint(uint256 numToMint, bytes32[] calldata _merkleProof)
        public
        payable
        nonReentrant
        isPreboardingOpen
        ...
        eligibleForPreboarding(numToMint, _merkleProof)
    {
        _safeMint(msg.sender, numToMint);

		payable(withdrawalAddress).transfer(msg.value);
    }

	function withdraw() public onlyOwner{
		uint bal = address(this).balance;
		payable(withdrawalAddress).transfer(bal);
	}
     ...
     */
    function airdrop(address recipient, uint256 numToAward) public onlyOwner {
        _safeMint(recipient, numToAward);
    }
