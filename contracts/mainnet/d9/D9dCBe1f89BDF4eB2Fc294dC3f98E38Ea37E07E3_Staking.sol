...
contract Staking is Initializable, Upgradeable {
    ...
    function stake(uint256[] calldata _tokenIds) external {
        ...
        for (uint256 i; i < _tokenIds.length; ++i) {
            ...
            StakedAsset memory stakedAsset = StakedAsset(msg.sender, _tokenIds[i], block.timestamp, rewardPerToken, 0, true, 0, 0, 0, 0);
    ...
    function claimRewards(uint256 tokenId) external {
        ...
        exp.accumulateRewards += calculatedRewards;

        updateLastClaimDate(tokenId);

        StakedAsset memory stakedToken = stakedAssets[tokenId];

        uint256 stakeDays = BokkyPooBahsDateTimeLibrary.diffDays(stakedToken.stakedAt,stakedToken.lastClaimedDate);
        ...
        returns (uint256 _rewards)
    {
        for (uint index = 0; index < stakeholders[msg.sender].stakedAssets.length; index++) {
            _rewards += rewardCalculator(stakeholders[msg.sender].stakedAssets[index].tokenId,false);
        }
    ...
    function getAssets(address _user) public view returns(StakedAsset[] memory allAsset){
        ...
        StakedAsset[] memory tempStakedAssets = stakeholders[_user].stakedAssets;
        uint256 nextPayoutAt = 0;
        uint256 nextPayoutDays = 0;
        uint256 totalExp = 0;
        uint256 totalRewards = 0;

        for (uint256 index = 0; index < tempStakedAssets.length; index++) {

            (nextPayoutAt, nextPayoutDays) = getNextPayout(tempStakedAssets[index].tokenId);
            (totalRewards, totalExp) = getExperience(tempStakedAssets[index].tokenId);

            tempStakedAssets[index].totalExp = totalExp;
            tempStakedAssets[index].totalRewards = totalRewards;
            tempStakedAssets[index].nextPayoutAt = nextPayoutAt;
            tempStakedAssets[index].nextPayoutDays = nextPayoutDays;
        }

        return tempStakedAssets;
    }
    ...
    function getAssetsCount(address _user) public view returns(uint256){
        ...
        for (uint256 index = 0; index < tempStakedAssets.length; index++) {
            if(tempStakedAssets[index].owner == _user) {
                stackedAssetsCount++;
            }
        }
    ...
    function updateStakedAtDate(uint256 _stakeToken, uint256 stakedAt) external onlyOwner {
        StakedAsset memory stakedToken = stakedAssets[_stakeToken];

        stakedToken.stakedAt = stakedAt;
        stakedAssets[_stakeToken] = stakedToken;
