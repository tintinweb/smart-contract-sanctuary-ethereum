...
contract VirtualCubeWorld is Ownable, ERC721A, PaymentSplitter {

    using Strings for uint;

    string public baseURI;

    address public addressMint = 0x19e0Dc63F64AC300878aC4775d15C69620e9962D;

    mapping(address => uint) public amountNFTsperWalletWhitelistSale;

    uint private teamLength;

    constructor(address[] memory _team, uint[] memory _teamShares, string memory _baseURI) ERC721A("Virtual Cube World", "VCW")
    ...
    function Mint(address _account, uint _quantity) external payable callerIsUser {
        require(msg.sender == addressMint, "Only Owner can Mint");
