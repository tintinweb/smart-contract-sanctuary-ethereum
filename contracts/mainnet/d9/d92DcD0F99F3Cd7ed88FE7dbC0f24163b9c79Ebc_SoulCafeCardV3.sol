    ...
    ERC1155LockerUpgradeable
{
    ...
    function _autounstake(address account, uint256 amount) internal {
        StakeRequest[] memory msr = StakeRequest( 
            _stakingTrack,
            TOKEN_ID.arrayify(),
            amount.arrayify()
        ...
        StakeAction[][] memory actions = new StakeAction[][](1);
        actions[0] = StakeAction.Unstake.arrayify();
    ...
    function _beforeTokenTransfer(
        ...
        for (uint256 t = 0; t < ids.length; t++) {
            ...
            uint256 tLocked = locked(from, tokenId);
            uint256 tBalance = balanceOf(from, tokenId);
            uint256 tFree = tBalance - tLocked;
            // defer out-of-balance revert to the underlying transfer fn
            if (tokenAmount > tFree && tokenAmount <= tBalance) {
                // but if the total balance is sufficient, unstake the qt that is required
                // to execute the transfer, all the way up to max amount locked

                // (tokenAmount - tFree) is always <= tLocked and > 0 here 
                _autounstake(from, tokenAmount - tFree);
            }
        }
    }
}
...
interface ICafeStaking {
    ...
    function tracksCount() external view returns (uint256);

    function stakeInfoERC20(uint256 trackId, address account) external view returns (uint256);
}
