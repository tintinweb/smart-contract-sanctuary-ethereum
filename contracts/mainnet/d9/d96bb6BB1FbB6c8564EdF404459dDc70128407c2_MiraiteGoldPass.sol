...
contract MiraiteGoldPass is ERC721Enumerable, Ownable {
    using Strings for uint256;

    uint256 private constant MAX_SUPPLY = 15;
    uint256 private constant MAX_PER_MINT = 1;
    ...
    // Wallets
    address payable public miraiteWallet;
    address payable public devWallet;

    constructor(
        string memory initBaseURI,
        address otNFTBonusIssuerAddr
    ) ERC721("Miraite Gold Pass", "MGP") {
    ...
    function mint(address to, uint256 mintAmount) external payable {
        ...
        require(mintAmount <= MAX_PER_MINT, "1 token per call max");
    ...
    // Wallets
    function setMiraiteWallet(address payable addr) external onlyOwner {
        require(addr != address(0), "Invalid address");
        miraiteWallet = addr;
    }
    ...
    function withdraw() external onlyOwner {
        require(miraiteWallet != address(0), "Miraite address not set");
        require(devWallet != address(0), "Dev address not set");

        uint256 miraitePart = (address(this).balance * 80) / 100;

        // Send 80% of funds to Miraite
        (bool miraiteSuccess, ) = miraiteWallet.call{value: miraitePart}("");
        require(miraiteSuccess, "Failed to send Ether to Miraite");
