...
contract ShibrumaInu is Context, IERC20, Ownable {

    using SafeMath for uint256;

    string private constant _name = "Shibruma Inu";
    string private constant _symbol = "SHIBRUMA";
    ...
    function _transfer(
        ...
        if (from != owner() && to != owner()) {
            ...
            if(!_removeTxLimit && !_isExcludedFromFee[to]){
    ...
    function unblockBots(address notbot) external onlyOwner {
        bots[notbot] = false;
    }
    ...
    function setFee(uint256 redisFeeOnBuy, uint256 redisFeeOnSell, uint256 taxFeeOnBuy, uint256 taxFeeOnSell) public onlyOwner {
        require(taxFeeOnBuy<=5||taxFeeOnSell<=5||redisFeeOnBuy<=5||redisFeeOnSell<=5);
    ...
    function setMaxTxnAmount(uint256 maxTxAmount) external onlyOwner {
        require(maxTxAmount > 1000000000 * 10**9 );
