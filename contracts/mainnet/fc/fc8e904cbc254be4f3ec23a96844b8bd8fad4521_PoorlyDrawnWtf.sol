...
contract PoorlyDrawnWtf is ERC721A, Ownable, ReentrancyGuard {

  string public        baseURI;
  uint public          price             = 0.0035 ether;
  uint public          maxFreePerWallet  = 3;
  uint public          maxPerTxn         = 5;
  ...
  constructor() ERC721A("poorlydrawn.wtf", "PD.WTF"){
  }

  function mint(uint256 amount) external payable
  {
    require(mintEnabled, "Minting is not live yet, hold on man");
    require(totalSupply() + amount <= maxSupply,"No more nfts");
    require(msg.value >= amount * price,"Please send the exact amount");
    require(amount <= maxPerTxn, "EXCEEDS_MAX_PER_TRANSACTION!");
  ...
  {
    require(mintEnabled, "Minting is not live yet, hold on man");
    require(totalSupply() + amount <= totalFree, "No more free nfts");
