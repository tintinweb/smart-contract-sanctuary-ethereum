...
// File: contracts/PhudgyPepes.sol
...
contract PhudgyPepes is ERC721A, Ownable, ReentrancyGuard {
    ...
    function mint(uint256 qty) external payable
    {
        ...
        require(totalSupply() + qty <= maxMint,"Notice Phudgy Pepes:  Soldout");
        _safemint(qty);
    }
    ...
    function _safemint(uint256 qty) internal
    {
        if(WalletMint[msg.sender] < freeMint) 
        {
           ...
           require(msg.value >= (qty - freeMint) * MintPrice,"Notice Phudgy Pepes:  Claim Free NFT");
            WalletMint[msg.sender] += qty;
           _safeMint(msg.sender, qty);
        }
        else
        {
           require(msg.value >= qty * MintPrice,"Notice Phudgy Pepes:  Fund not enough");
            WalletMint[msg.sender] += qty;
           _safeMint(msg.sender, qty);
        }
    }
