...
contract Apez is ERC721A, Ownable, ReentrancyGuard, DefaultOperatorFilterer {
  ...
  string public uriPrefix = 'ipfs://QmThqSXVitzhpgPTGR4fopnkTv4QKgwsumAWZA57u4cDfY/';
  ...
  function fuse(uint64[] memory tokens) public {
        ...
        for (uint i = 0; i < tokens.length; i++) {
            ...
            sum = sum + getApez(tokens[i]);
        }
        if (sum > 6464) {
            revert("sum must be under 6464");
        }
        ...
        baseColor[tokens[0]] = randoApez(tokens[0], 1, 4);
  }

  function getApez(uint64 tokenId) public view returns (uint64) {
        if (!_exists(tokenId)) {
            return 0;
  ...
  function randoApez(uint64 input, uint64 min, uint64 max) internal pure returns (uint64) {
