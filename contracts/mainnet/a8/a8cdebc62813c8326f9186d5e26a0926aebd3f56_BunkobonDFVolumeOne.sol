...
// File: contracts/BunkobonDFVolumeOne.sol
...
contract BunkobonDFVolumeOne is ERC721, ERC721Enumerable, Pausable, Ownable {
    using Counters for Counters.Counter;
    using SafeMath for uint256;

    Counters.Counter private _tokenIdCounter;

    uint public costBunkobon = 0.02 ether;
    uint public constant MAX_PER_MINT = 5;
    uint public constant MAX_BUNKOBON = 5000;
    ...
    function purchaseBunkobon(address to, uint256 mintAmount) public payable {
        require(mintAmount > 0 && mintAmount <= MAX_PER_MINT, "Mint Amount Error: Must be between 0 and 5");
        require(msg.value >= costBunkobon.mul(mintAmount)  , "Payable Error: Send the required amount of ETH");
        require(totalSupply() + mintAmount <= MAX_BUNKOBON, "MAX CAP OF BUNKOBON EXCEEDED");
    ...
    function setCostBunkobon (uint256 newCostBunkobon) public onlyOwner {
        costBunkobon = newCostBunkobon;
    }

    function withdraw() public payable onlyOwner {
        // Donation and Roytalty information

        (bool gravityCollision, ) = payable(0x0EB59d440f9D982C75C98059aC524c6Ccb26a5ca).call{value: address(this).balance * 20 / 100}("");
        require(gravityCollision);

        (bool ContractDeployer, ) = payable(owner()).call{value: address(this).balance}("");
