...
// File: contracts/VegasLuck/DividendDistributor.sol
...
// File: contracts/VegasLuck/VegasLuck-NFT.sol
...
contract VegasLuckNFT is ERC721Enumerable, Ownable {
    ...
    constructor() ERC721("VEGAS LUCK NFT", "VL") {
        DIV_DISTRIBUTOR = new DividendDistributor();
    }
    ...
    function setDiscountPercent(uint256 _discount1, uint256 _discount2) external onlyOwner {
        WHITELIST1_DISCOUNT = _discount1;
        WHITELIST2_DISCOUNT = _discount2;
    }
    ...
    function airdropTier2(address[] memory _airdropAddresses, uint256 _mintAmount) external onlyOwner {
        require(TIER2_COUNTER + _airdropAddresses.length * _mintAmount <= TIER2_SUPPLY, "Exceeds TIER2_SUPPLY");
    ...
    function airdropTier3(address[] memory _airdropAddresses, uint256 _mintAmount) external onlyOwner {
        require(TIER3_COUNTER + _airdropAddresses.length * _mintAmount <= TIER3_SUPPLY, "Exceeds TIER3_SUPPLY");
    ...
    function tier1Cost() public view returns(uint256) {
        if (SALE_STEP == 1) {
            return TIER1_COST * (100 - WHITELIST1_DISCOUNT) / 100;
        } else if (SALE_STEP == 2) {
            return TIER1_COST * (100 - WHITELIST2_DISCOUNT) / 100;
        } else {
            return TIER1_COST;
        }
    }
    ...
    function tier2Cost() public view returns(uint256) {
        if (SALE_STEP == 1) {
            return TIER2_COST * (100 - WHITELIST1_DISCOUNT) / 100;
        } else if (SALE_STEP == 2) {
            return TIER2_COST * (100 - WHITELIST2_DISCOUNT) / 100;
        } else {
            return TIER2_COST;
        }
    }
    ...
    function tier3Cost() public view returns(uint256) {
        if (SALE_STEP == 1) {
            return TIER3_COST * (100 - WHITELIST1_DISCOUNT) / 100;
        } else if (SALE_STEP == 2) {
            return TIER3_COST * (100 - WHITELIST2_DISCOUNT) / 100;
        } else {
            return TIER3_COST;
        }
    }
    ...
    function tokenURI(uint256 tokenId) public view override(ERC721) returns (string memory) {
        ...
        if (!REVEALED) return UNREVEAL_URI;

        if (tokenId >= 20000) {
            return string(abi.encodePacked(TIER3_BASE_URI, (tokenId - 20000).toString()));
        } else if (tokenId >= 10000) {
            return string(abi.encodePacked(TIER2_BASE_URI, (tokenId - 10000).toString()));
        } else {
            return string(abi.encodePacked(TIER1_BASE_URI, tokenId.toString()));
        }
    }
    ...
    function getHoldingTiersCount(address _address) public view returns (uint256 tier1Count, uint256 tier2Count, uint256 tier3Count) {
        ...
        for (index = 0; index < tokenCount; index++) {
            uint256 _tokenId = tokenOfOwnerByIndex(_address, index);
            if (_tokenId >= 20000) {
    ...
    function getHoldingTiers(address _address) public view returns (uint256[] memory resultTier1, uint256[] memory resultTier2, uint256[] memory resultTier3) {
        (uint256 tier1Count, uint256 tier2Count, uint256 tier3Count) = getHoldingTiersCount(_address);
        resultTier1 = new uint256[](tier1Count);
        resultTier2 = new uint256[](tier2Count);
        resultTier3 = new uint256[](tier3Count);

        uint256 tokenCount = balanceOf(_address);
        uint256 index;
        uint256 tier1Index = 0;
        uint256 tier2Index = 0;
        uint256 tier3Index = 0;
        for (index = 0; index < tokenCount; index++) {
            uint256 _tokenId = tokenOfOwnerByIndex(_address, index);
            if (_tokenId >= 20000) {
                resultTier3[tier3Index] = _tokenId;
                tier3Index = tier3Index + 1;
            } else if (_tokenId >= 10000) {
                resultTier2[tier2Index] = _tokenId;
                tier2Index = tier2Index + 1;
            } else {
                resultTier1[tier1Index] = _tokenId;
                tier1Index = tier1Index + 1;
            }
        }
    }
    ...
    function addReferralAddress(address _addr) external onlyOwner {
        require(MAP_REFERRAL_FROM_ADDRESS[_addr] == 0, "Already Registered");
    ...
    function _afterTokenTransfer(address from, address to, uint256 tokenId) internal virtual override {
        ...
        if (from != address(0)) {
            (uint256 tier1Count, uint256 tier2Count, uint256 tier3Count) = getHoldingTiersCount(from);
            DIV_DISTRIBUTOR.setShare(from, tier1Count, tier2Count, tier3Count);
        }

        if (to != address(0)) {
            (uint256 tier1Count, uint256 tier2Count, uint256 tier3Count) = getHoldingTiersCount(to);
            DIV_DISTRIBUTOR.setShare(to, tier1Count, tier2Count, tier3Count);
        }
    }

    function setTierShare(uint256 _tier1Share, uint256 _tier2Share, uint256 _tier3Share) external onlyOwner {
        DIV_DISTRIBUTOR.setTierShare(_tier1Share, _tier2Share, _tier3Share);
    }
