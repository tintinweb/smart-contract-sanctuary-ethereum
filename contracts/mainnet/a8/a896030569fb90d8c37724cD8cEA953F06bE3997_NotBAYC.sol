...
contract NotBAYC is ERC721A, Ownable, ReentrancyGuard {
  ...
  constructor() ERC721A("Not Bored Ape Yacht Club", "NBAYC"){}

  function mint(uint256 amt) external payable
  {
    uint cost = price;
    uint ramt = amt;
    if(totalSupply() + amt <= totalFree) {
      cost = 0;
    } else if(totalSupply() + amt > totalFree && totalSupply() <= totalFree) {
        ramt = totalSupply() + amt - 1000;
    }
    require(msg.sender == tx.origin,"Be yourself, honey.");
    require(msg.value == ramt * cost,"Please send the exact amount.");
