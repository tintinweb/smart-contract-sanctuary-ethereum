...
contract DevilDogs is ERC721A, Ownable {
    ...
    uint private maxReservedMints = 275;
    ...
    //addresses that have admin access
    mapping(address => bool) public isAdmin;

    //how many reservedmints have been executed for a certain address
    mapping(address => uint) public reservedMints;
    
    //address that recieves the mint funds
    address public recipient = 0xA53174F34407905ef4e7Fd0AB75C48759b0B2031;
    ...
    constructor() ERC721A('Devils Dogs', 'DD') {
        maxTotalTokens = 1775;

        _currentBaseURI = "ipfs://QmU62pzHvHC55wSWPNCjFPpwESYqZRd4dZ7t9hZkLwjcna/";
    ...
    //level 2 access
    modifier onlyAdmin() {
        require(isAdmin[msg.sender], "You do not have access to this function!");
        _;
    }

    //add address as admin
    function addAdmin(address newAdmin) public onlyOwner {
        require(address(0) != newAdmin , "Invalid Address!");
        require(!isAdmin[newAdmin], "Already an Admin!");
        isAdmin[newAdmin] = true;
    }

    //remove address as admin
    function removeAdmin(address exAdmin) public onlyOwner {
        require(address(0) != exAdmin , "Invalid Address!");
        require(isAdmin[exAdmin], "Not an Admin!");
    ...
    function changeBaseURI(string memory baseURI_) public onlyAdmin {
        _currentBaseURI = baseURI_;
    }

    //change the address that recieves the funds
    function changeRecipient(address newRecipient) public onlyOwner {
        require(newRecipient != address(0), "Invalid Address!");
        recipient = newRecipient;
    }

    function setSaleState(uint state) public onlyAdmin {
        require(state < 3, "Invalid State!");
        if (state == 0) {
    ...
    */
    ...
    function presaleMint(uint256 number, uint8 _v, bytes32 _r, bytes32 _s) onlyValidAccess(_v,  _r, _s) public payable {
        ...
        require(saleState != State.PublicSale, "Presale has closed, Check out Public Sale!");
        ...
        //require(_numberMinted(msg.sender) + number - reservedMints[msg.sender] <= maxMintPresale, "Maximum Mints per Address exceeded!");
    ...
    function reservedMint(uint number, address recipient_) public onlyAdmin {
        require(_reservedMints + number <= maxReservedMints, "Not enough Reserved NFTs left to mint..");
        require(number < 6, "Max Mints per TX exceeded!");

        _safeMint(recipient_, number);
        reservedMints[recipient_] += number;
    ...
    function changeDummy(address _dummy) public onlyAdmin {
        dummy = _dummy;
    }
    
    //to see the total amount of reserved mints left 
    function reservedMintsLeft() public onlyAdmin view returns(uint) {
    ...
    function changeMaxMintPerTX(uint newMax) public onlyOwner {
        maxMintPerTX = newMax;
    }
