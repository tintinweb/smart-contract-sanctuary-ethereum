...
contract GOAT is Context, IERC20, ERC20Ownable {
    using SafeMath for uint256;
    string private constant _name = "GOATAMA";
    ...
    uint256 private constant _tTotal = 1e17 * 10**9;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;
    uint256 private _maxWalletSize = 2500000000000000 * 10**9;
    uint256 private minTokensBeforeSwap;
    uint256 private tokensForLiquidityToSwap;
    uint256 private tokensForMarketingToSwap;

    uint private _marTax = 10; // tax for marketing
    uint private _previousMarTax = _marTax;
    uint private _liqTax = 4; // tax for liquidity
    ...
    constructor() {
        ...
        marketingAddress = payable(0xF175E99897545d3DCFD1F8fD44d02d5D605BCd96);
    ...
    function _transfer(address from,address to,uint256 amount) private {
        ...
        if (_buyLimits == true && from == uniV2Pair) {
            require(amount <= 750000000000000 * 10**9, "Limits are in place, please lower buying amount");
        }
        if(_initLaunch == true) {
