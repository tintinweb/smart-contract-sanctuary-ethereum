...
contract RealtySwap is ERC721, Ownable, ERC721Burnable, ReentrancyGuard,ERC721Enumerable , Initializable , UUPSUpgradeable{
    ...
    function initialize()    initializer public {
       ...
       Listingprice = 0.05 ether;
       Mintingprice = 0.05 ether;
    }
    ...
    function updateListPrice(uint256 _listPrice) public payable {
        require(ContractOwnerAddress == msg.sender, "Only Contract owner can update listing price");
    ...
    function mint(address account, string memory tokenuri)  public payable returns (uint) {  
        require(msg.value == Mintingprice, "Hopefully sending the correct price");
    ...
    function ListNFTForSale(uint _biddingStartTime,uint _biddingendtime , address payable _beneficiary , uint256 tokenId,uint256 _minBidAmount)  payable  public onlyTokenHolders(tokenId) returns(uint256)
    {
        require(msg.value == Listingprice, "Hopefully sending the correct price");
        uint256 newauctionid = auctions.length;
        auctions.push(Auction(false,true,tokenId,newauctionid,_biddingStartTime,_biddingendtime,0,_minBidAmount,address(0),_beneficiary));
        emit AuctionStart(newauctionid);
        ContractOwnerAddress.transfer(msg.value);
        return newauctionid;
    }        

    function CancelNFTListing(uint256 _tokenId) public  onlyTokenHolders(_tokenId) returns(bool)
    {
        require(auctions[_tokenId].OpenForBidding ,"Item Not Currently Listed");
        require(_owners[_tokenId] == msg.sender, "Only Owner can cancel Listing");
        delete auctions[_tokenId];
        return true;
    }   


    function CloseBiding( uint256 _tokenId) public {
        require(auctions[_tokenId].OpenForBidding ,"Item Not Currently Listed");
        require(_owners[_tokenId] == msg.sender, "Only Owner can cancel Listing");
        auctions[_tokenId].OpenForBidding = false;
        address  currentBidOwner = auctions[_tokenId].currentBidOwner;
        uint256  currentBidAmount = auctions[_tokenId].currentBidAmount;
    ...
    {   
        ...
        uint256  minBidAmount = auctions[Id].minBidAmount;
     
        if(msg.value <  minBidAmount) {
            revert("Miniumim Bid Amount Not Met");
        }
    ...
    function endNFTSale(uint256 Id) public 
    {
        ...
        //payable(nftminter).transfer(royaltyfee);
        //payable(ContractOwnerAddress).transfer(amountfee);
