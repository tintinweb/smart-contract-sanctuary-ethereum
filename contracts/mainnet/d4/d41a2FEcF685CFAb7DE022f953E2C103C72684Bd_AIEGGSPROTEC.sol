...
// File: contracts/AIEGGSPROTEC.sol


pragma solidity ^0.8.0;

interface IAieggsToken {
    function mint(address to, uint256 amount) external;
    function totalSupply() external view returns (uint256);
    function transferUnderlying(address to, uint256 value) external returns (bool);
    function fragmentToAieggs(uint256 value) external view returns (uint256);
    function aieggsToFragment(uint256 aieggs) external view returns (uint256);
    function balanceOfUnderlying(address who) external view returns (uint256);
    function burn(uint256 amount) external;
}

contract AIEGGSPROTEC is Ownable {
    ...
    constructor(IAieggsToken _aieggs, uint256 _lockDuration, bool _depositsEnabled) {
        aieggs = _aieggs;
    ...
    function deposit(uint256 _amount) external {
        ...
        IERC20(address(aieggs)).safeTransferFrom(address(msg.sender), address(this), _amount);
        aieggs.burn(_amount);
    ...
    function withdraw(uint256 _amount) external {
        ...
        user.amount -= _amount;
        totalStaked -= _amount;
        aieggs.mint(address(msg.sender), _amount);
