...
contract CoreHelpers is AdminModule {
     ...
     * @notice internal function which handles supplies.
     */
...
contract InstaVaultImplementation is CoreHelpers {
     ...
     * @notice User function to supply (WETH or STETH).
     ...
     * @notice User function to withdraw (to get WETH or STETH).
     ...
     * @notice Rebalancer function to leverage and rebalance the position.
     */
    function rebalanceOne(
        ...
        if (excessDebt_ > 0) {
            ...
            v_.calldatas[2] = abi.encodeWithSignature(
            ...
            v_.calldatas[3] = abi.encodeWithSignature(
                "deposit(address,uint256,uint256,uint256)",
                stEthAddr,
                type(uint256).max,
                0,
            ...
            v_.calldatas[4] = abi.encodeWithSignature(
            ...
            v_.calldatas[5] = abi.encodeWithSignature(
     ...
     * @notice Rebalancer function for saving. To be run in times of making position less risky or to fill up the withdraw amount for users to exit
     */
    function rebalanceTwo(
        ...
        targets_[2] = "1INCH-A";
        calldata_[2] = abi.encodeWithSignature(
            "sell(address,address,uint256,uint256,bytes,uint256)",
            wethAddr,
            stEthAddr,
            saveAmt_,
        ...
        calldata_[3] = abi.encodeWithSignature(
            "payback(address,uint256,uint256,uint256,uint256)",
            wethAddr,
            0,
            2,
        ...
        calldata_[4] = abi.encodeWithSignature(
            "withdraw(address,uint256,uint256,uint256)",
            flashTkn_,
            flashAmt_,
            0,
        ...
        calldata_[5] = abi.encodeWithSignature(
            "flashPayback(address,uint256,uint256,uint256)",
            flashTkn_,
            flashAmt_,
            0,
...
contract Helpers is Events {
     ...
     * @notice Helper function to get current eth borrow rate on aave.
     */
     ...
     * @notice Helper function to get current steth collateral on aave.
     */
     ...
     * @notice Helper function to get current eth debt on aave.
     */
     ...
     * @notice Helper function to get ideal eth/steth amount in vault or vault's dsa.
     */
     ...
     * @notice Helper function to get net assets everywhere (not substracting revenue here).
     */
    function netAssets()
        public
        view
     ...
     * @notice Helper function to get current exchange price and new revenue generated.
     */
        ...
        returns (uint256 exchangePrice_, uint256 newRevenue_)
    {
        ...
        if (exchangePriceWithRevenue_ > lastRevenueExchangePrice) {
            uint256 revenueCut_ = ((exchangePriceWithRevenue_ -
                lastRevenueExchangePrice) * revenueFee) / 10000; // 10% revenue fee cut
    ...
    /**
     * @dev Helper function to validate the safety of aave function after rebalancing.
     * @notice Helper function to validate the safety of aave function after rebalancing.
     */
