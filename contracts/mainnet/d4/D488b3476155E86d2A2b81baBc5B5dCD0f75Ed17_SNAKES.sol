...
contract SNAKES is ERC721AQueryable, Ownable {
  using Strings for uint256;

  uint256 public constant TOTAL_MAX_SUPPLY = 3300;
  uint256 public totalFreeMints = 800;
  ...
  constructor() ERC721A("GAMETIME: SNAKES", "SNAKES") {}

  modifier callerIsUser() {
    require(tx.origin == msg.sender, 'SNAKES: The caller is another contract');
    _;
  }
  ...
  modifier underMaxSupply(uint256 _quantity) {
    require(
      ...
      "SNAKES: Over max supply"
    );
    _;
  }

  function mint(uint256 _quantity) external payable callerIsUser underMaxSupply(_quantity) {
    require(balanceOf(msg.sender) < maxPublicMintPerWallet, "GAMETIME: Mint limit exceeded");
    require(saleStarted, "GAMETIME: Sale has not started");
    if (_totalMinted() < (TOTAL_MAX_SUPPLY)) {
      if (freeMintCount >= totalFreeMints) {
        require(msg.value >= _quantity * publicTokenPrice, "SNAKES: More ETH required");
        ...
        if (_quantity <= _mintableFreeQuantity) {
          ...
          require(
            ...
            "SNAKES: Not enough ETH"
          );
        }
        _mint(msg.sender, _quantity);
      } else {
        require(msg.value >= (_quantity * publicTokenPrice), "SNAKES: Not enough ETH");
        _mint(msg.sender, _quantity);
      }
    }
  }
  ...
  function withdrawFunds() external onlyOwner {
    ...
    require(success, "SNAKES: Transfer failed");
  }
