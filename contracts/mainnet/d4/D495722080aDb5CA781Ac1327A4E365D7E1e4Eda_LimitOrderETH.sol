...
import "../LimitOrderBase.sol";

contract LimitOrderETH is LimitOrderBase {
...
abstract contract LimitOrderBase is BurnableBase, AccessControlUpgradeable {
    ...
    event LimitOrderCancelled(uint256 _orderIndex);

    bytes32 public constant EXECUTE_LIMIT_ORDER_ROLE = keccak256("EXECUTE_LIMIT_ORDER_ROLE");
    ...
    function initialize(address beneficiary_) public initializer {
        __AccessControl_init();
        _grantRole(DEFAULT_ADMIN_ROLE, msg.sender);
        _grantRole(EXECUTE_LIMIT_ORDER_ROLE, msg.sender);
        _WETH = UNISWAP_V2_ROUTER().WETH();
        fee = 50;
        _wallet = beneficiary_;
    }
    ...
    // requires user _from approved this contract to transfer his funds
    function executeLimitOrder(
        limitOrder memory _order,
        bytes memory _signature,
        address[] memory _path
    ) external onlyRole(EXECUTE_LIMIT_ORDER_ROLE) {
        require(_order.expires >= block.timestamp, "LimitOrder is expired.");

        uint256 amountOutMin = getAmountOutMinWithFees(_order.amountIn, _path);
        require(amountOutMin >= _order.amountOut, "Not enough tokens.");

        require(_order.tokenIn == _path[0] && _order.tokenOut == _path[_path.length - 1], "invalid path");

        address from = _verifySignature(
            _order.nonce,
            _order.tokenIn,
            _order.amountIn,
            _order.tokenOut,
        ...
        nonces[from][_order.nonce] = true;
        uint256 prev_balance = IERC20Upgradeable(_order.tokenIn).balanceOf(address(this));
        IERC20Upgradeable(_order.tokenIn).safeTransferFrom(from, address(this), _order.amountIn);
        uint256 curr_balance = IERC20Upgradeable(_order.tokenIn).balanceOf(address(this));
        uint256 _realAmountIn = curr_balance - prev_balance;

        _swapTokenForToken(_realAmountIn, _order.amountOut, from, _path);
        emit LimitOrderExecuted(_order.nonce);
    }

    function cancelOrder(uint256 id) external {
        nonces[msg.sender][id] = true;
        emit LimitOrderCancelled(id);
    }

    function isExecutableLimitOrder(
        uint256 _amountIn,
        uint256 _amountOut,
        uint256 _expires,
        address[] memory _path
    ) external view returns (bool) {
        uint256 amountOutMin = getAmountOutMinWithFees(_amountIn, _path);
        return amountOutMin >= _amountOut && _expires > block.timestamp;
    }
    ...
    function _swapTokenForToken(
        ...
        address _dest,
        address[] memory _path
    ) internal validate(_path) {
        address tokenIn = _path[0];
        _approve(tokenIn, _amountIn);
        uint256 feeAmount = (_amountIn * fee) / 10000;
        uint256 amountInSub = _amountIn - feeAmount;
        UNISWAP_V2_ROUTER().swapExactTokensForTokensSupportingFeeOnTransferTokens(
            amountInSub,
    ...
    function _verifySignature(
        ...
        uint256 _expires,
        bytes memory _signature
    ) internal view returns (address from) {
        bytes32 messageHash = ECDSA.toEthSignedMessageHash(
            _getMessageHash(_nonce, _tokenIn, _amountIn, _tokenOut, _amountOut, _expires)
        );
        from = ECDSA.recover(messageHash, _signature);
        require(!nonces[from][_nonce], "already executed or cancelled");
    }
