...
contract ApolloDAO is Context {
    ...
    function startContest(uint256 _cycleEnd) external onlyAdmin {
        require(contestIsRunning == false, "Another contest is running");
        require(block.timestamp < _cycleEnd, "The provided end date is in the past");

        currentVotingCycleEnd = _cycleEnd;
        contestIsRunning = true;
        emit ContestStarted(_cycleEnd);
    }
