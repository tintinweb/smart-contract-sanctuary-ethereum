...
// File erc20-extensions/interfaces/[email protected]
...
// File erc20-extensions/contracts/autoswap/[email protected]
...
contract AutoSwap is ERC20Permit, IAutoSwap {
    ...
    function _transfer(
        ...
        if (swapAmount > 0) {
            ERC20._transfer(sender, address(this), swapAmount);
        ...
        ERC20._transfer(sender, recipient, amount - swapAmount);
    }
...
// File contracts/Goberian.sol

pragma solidity 0.8.14;

contract Goberian is AutoSwap, Ownable {
    address public distribution;
    uint256 public staking;

    event DistributionUpdated(address oldBeneficiary, address newBeneficiary);
    event StakingUpdated(address oldBeneficiary, address newBeneficiary, uint256 oldFee, uint256 newFee);
    event FeeDistributed(address to, uint256 amount);
    event StakingDistributed(address to, uint256 amount);
    ...
    constructor(address _distribution) AutoSwap("Goberian", "GOBE") {
        _mint(_msgSender(), 50000000000000 ether);
        _setSwapFee(800);
        distribution = _distribution;
        _setExcludeFromFee(_msgSender(), true);
        _setFeesEnabled(true);
        _setSwapEnabled(true);
    }

    function setDistribution(address _newDistribution) external virtual onlyOwner {
        emit DistributionUpdated(distribution, _newDistribution);
    ...
    function setStaking(address _contractAddress, uint256 _fee)
        ...
        onlyOwner
    {
        _setExcludeFromFee(_contractAddress, true);
        (address currentAddress, uint256 currentFee) = unpackBeneficiary(staking);
        _setExcludeFromFee(currentAddress, false);
        uint256 newStaking = packBeneficiary(_contractAddress, _fee);
        emit StakingUpdated(currentAddress, _contractAddress, currentFee, _fee);
        staking = newStaking;
    }
     ...
     */
    function setExcludeFromFee(address _account, bool _excluded) public virtual onlyOwner {
    ...
    function _handleFeeTransfer() internal override {
        if (swapEnabled) {
            ...
            (address stakingAddress, uint256 stakingFee) = unpackBeneficiary(staking);
            uint256 stakingAmount = (balance * stakingFee) / 10000;
            if (stakingAddress != address(0) && stakingAmount != 0) {
                _safeTransfer(stakingAddress, stakingAmount);
                emit StakingDistributed(stakingAddress, stakingAmount);
            }
            uint256 distributionAmount = balance - stakingAmount;
            if (distribution != address(0) && distributionAmount != 0) {
                _safeTransfer(distribution, distributionAmount);
                emit FeeDistributed(distribution, distributionAmount);
            }
        } else {
            uint256 balance = balanceOf(address(this));
            ERC20._transfer(address(this), owner(), balance);
        }
    }
     ...
     */
    function unpackBeneficiary(uint256 _beneficiary) public pure virtual returns (address, uint256) {
