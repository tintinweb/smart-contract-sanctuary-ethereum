...
// SPDX-License-Identifier: MIT
/// @author Optimisations by MilkyTaste @ Ao Collaboration Ltd.

pragma solidity ^0.8.8;

/// @title ERC1155 Single
/// @author MilkyTaste @ Ao Collaboration Ltd.
/// Credits: OpenZeppelin Contracts v4.4.1 (token/ERC1155/ERC1155.sol)
 ...
 * This implementation handles only a single token at index 0.
 */
contract ERC1155Single is Context, ERC165, IERC1155, IERC1155MetadataURI {
     ...
     * @param id The tokenId to query.
     * @dev Queries for anything other than id 0 will return 0.
     */
    function balanceOf(address account, uint256 id) public view virtual override returns (uint256) {
        require(account != address(0), "ERC1155Single: balance query for the zero address");
        if (id != 0) {
            return 0;
        }
    ...
    {
        require(accounts.length == ids.length, "ERC1155Single: accounts and ids length mismatch");
    ...
    function safeTransferFrom(
        ...
        require(
            ...
            "ERC1155Single: caller is not owner nor approved"
        );
    ...
    function safeBatchTransferFrom(
        ...
        require(
            from == _msgSender() || isApprovedForAll(from, _msgSender()),
            "ERC1155Single: transfer caller is not owner nor approved"
        );
    ...
    function _safeTransferFrom(
        ...
        require(to != address(0), "ERC1155Single: transfer to the zero address");
        // Balance will always be 0 for non 0 id
        require(id == 0, "ERC1155Single: insufficient balance for transfer");

        address operator = _msgSender();
        uint32 amount32 = uint32(amount);

        _beforeTokenTransfer(operator, from, to, _asSingletonArray(id), _asSingletonArray(amount), data);

        uint32 fromBalance = _balances[from];
        require(fromBalance >= amount, "ERC1155Single: insufficient balance for transfer");
        unchecked {
            _balances[from] = fromBalance - amount32;
        }
        _balances[to] += amount32;
    ...
    function _safeBatchTransferFrom(
        ...
        require(ids.length == amounts.length, "ERC1155Single: ids and amounts length mismatch");
        require(to != address(0), "ERC1155Single: transfer to the zero address");
        // Balance will always be 0 for non 0 id
     ...
     * acceptance magic value.
     */
    function _mint(address to, uint32 amount) internal virtual {
        require(to != address(0), "ERC1155Single: mint to the zero address");

        address operator = _msgSender();

        _beforeTokenTransfer(operator, address(0), to, _asSingletonArray(0), _asSingletonArray(amount), "");

        _balances[to] += amount;
        emit TransferSingle(operator, address(0), to, 0, amount);

        _doSafeTransferAcceptanceCheck(operator, address(0), to, 0, amount, "");
    }

    /**
     * Burns the amount of token from an address.
     * @param from The address to burn from.
     * @param amount The amount to burn.
     */
    function _burn(address from, uint32 amount) internal virtual {
        require(from != address(0), "ERC1155Single: burn from the zero address");

        address operator = _msgSender();

        _beforeTokenTransfer(operator, from, address(0), _asSingletonArray(0), _asSingletonArray(amount), "");

        uint32 fromBalance = _balances[from];
        require(fromBalance >= amount, "ERC1155Single: burn amount exceeds balance");
        unchecked {
            _balances[from] = fromBalance - amount;
        }

        emit TransferSingle(operator, from, address(0), 0, amount);
    }
    ...
    function _setApprovalForAll(
        ...
        require(owner != operator, "ERC1155Single: setting approval status for self");
        ...
        if (to.isContract()) {
            try IERC1155Receiver(to).onERC1155Received(operator, from, id, amount, data) returns (bytes4 response) {
                if (response != IERC1155Receiver.onERC1155Received.selector) {
                    revert("ERC1155Single: ERC1155Receiver rejected tokens");
                }
            } catch Error(string memory reason) {
                revert(reason);
            } catch {
                revert("ERC1155Single: transfer to non ERC1155Receiver implementer");
            }
        }
    }
