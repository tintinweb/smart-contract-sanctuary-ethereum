...
/// @title MakerDualOracle
/// @author Drake Evans (Frax Finance) https://github.com/drakeevans
/// @notice  An oracle for Maker in Frax units
contract MakerDualOracle is
    DualOracleBase,
    Timelock2Step,
    UniswapV3SingleTwapOracle,
    ChainlinkOracleWithMaxDelay,
    EthUsdChainlinkOracleWithMaxDelay
{
    address public immutable MKR_ERC20;
    ...
    constructor(
        ...
        DualOracleBase(
            DualOracleBaseParams({
                ...
                quoteToken0: _params.mkrErc20,
                quoteToken0Decimals: 18,
                baseToken1: _params.mkrErc20,
        ...
        UniswapV3SingleTwapOracle(
            UniswapV3SingleTwapOracleParams({
                ...
                quoteToken: _params.mkrErc20
            })
        )
        ChainlinkOracleWithMaxDelay(
            ChainlinkOracleWithMaxDelayParams({
                chainlinkFeedAddress: _params.mkrUsdChainlinkFeed,
    ...
    function name() external pure returns (string memory) {
        return "Maker Dual Oracle Chainlink with Staleness Check and Uniswap V3 TWAP";
    }
    ...
    /// @return _mkrPerUsd The Mkr per USD price
    function getMkrPerUsdTwap() public view returns (bool _isBadData, uint256 _mkrPerUsd) {
        uint256 _mkrPerWeth = _getUniswapV3Twap();
        uint256 _usdPerEth;
        (_isBadData, , _usdPerEth) = _getEthUsdChainlinkPrice();
        _mkrPerUsd = (_mkrPerWeth * ETH_USD_CHAINLINK_FEED_PRECISION) / _usdPerEth;
    }

    /// @notice The ```getMkrPerUsdChainlink``` function returns Mkr per USD using the Chainlink oracle
    /// @return _isBadData If the Chainlink oracle is stale
    /// @return _mkrPerUsd The Mkr per USD price
    function getMkrPerUsdChainlink() public view returns (bool _isBadData, uint256 _mkrPerUsd) {
        uint256 _usdPerMkrChainlinkRaw;
        (_isBadData, , _usdPerMkrChainlinkRaw) = _getChainlinkPrice();
        _mkrPerUsd = (ORACLE_PRECISION * CHAINLINK_FEED_PRECISION) / _usdPerMkrChainlinkRaw;
    }
    ...
    function _getPrices() internal view returns (bool _isBadData, uint256 _priceLow, uint256 _priceHigh) {
        (bool _isBadDataChainlink, uint256 _mkrPerUsdChainlink) = getMkrPerUsdChainlink();

        (bool _isBadDataTwap, uint256 _mkrPerUsdTwap) = getMkrPerUsdTwap();
        if (_isBadDataChainlink && _isBadDataTwap) {
            revert("Both Chainlink and TWAP are bad");
        }

        _isBadData = _isBadDataChainlink || _isBadDataTwap;
        _priceLow = _mkrPerUsdTwap < _mkrPerUsdChainlink ? _mkrPerUsdTwap : _mkrPerUsdChainlink;
        _priceHigh = _mkrPerUsdChainlink > _mkrPerUsdTwap ? _mkrPerUsdChainlink : _mkrPerUsdTwap;
    }
