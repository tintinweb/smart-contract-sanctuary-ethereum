...
contract RewardManager is Initializable, ReentrancyGuardUpgradeable, OwnableUpgradeable {
  ...
  /// @dev penalty of user result from early withdraw
  mapping(address => uint256) public penaltyOf;

  /// @dev list of whitelist feeder 
  ...
  modifier onlyWhitelistedFeeder() {
    require(msg.sender == owner() || whitelistedFeeder[msg.sender], "!whitelist feeder");
    _;
  }
  ...
  /// @notice increase penalty of "_user"
  /// @param _user The address to increase penalty
  /// @param _amount penalty to increase
  function increasePenaltyOf(address _user, uint256 _amount) external nonReentrant onlyLive onlyXGF {
    penaltyOf[_user] += _amount;
    emit LogPenaltyIncrease(_user, _amount);
  }
  ...
  function pendingRewardsOf(address _user) external nonReentrant onlyLive returns (uint256) {
    ...
    return _toDistrobite - penaltyOf[_user];
  }
  ...
  function claim(address _user) external nonReentrant onlyLive onlyXGF returns (uint256) {
    ...
    if (_amount != 0) {
      uint256 _penalty = penaltyOf[_user];
      lastTokenBalance = lastTokenBalance - _amount;
      if (_penalty != 0) {
        // transfer penalty to treasury
        rewardToken.safeTransfer(treasury, _penalty);
        penaltyOf[_user] = 0;
      }
      // transfer remaining back to _user
      rewardToken.safeTransfer(_user, _amount - _penalty);
    }
  ...
  function feed(address _from, uint256 _amount) external nonReentrant onlyLive onlyWhitelistedFeeder returns (bool) {
  ...
  function setTreasury(address _treasury) external onlyOwner {
    ...
    treasury = _treasury;
    emit LogSetTreasury(msg.sender, _oldTreasury, _treasury);
  }
  ...
  function setWhitelistedFeeder(address [] calldata _callers, bool _ok) external onlyOwner {
       for (uint256 _idx = 0; _idx < _callers.length; _idx++) {
      whitelistedFeeder[_callers[_idx]] = _ok;
      emit LogSetWhitelistedFeeder(msg.sender, _callers[_idx], _ok);
    } 
  }
}
