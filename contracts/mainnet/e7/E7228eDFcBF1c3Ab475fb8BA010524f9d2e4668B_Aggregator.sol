...
contract Aggregator is Ownable, Pausable, TokenHolder, TransferHelper {
    ...
    constructor(
        ...
        WETH.approve(SEAPORT, 2**256 - 1); //Gas less than "type(uint256).max"
    ...
    /// @param protocolFeeAmount protocol fee
    function batchBuy(
    ...
    function _trade(
        ...
        for (uint256 i = 0; i < _tradeDetails.length; i++) {
            ...
            // if (status) {
            //     successNum = abi.decode(result, (uint256));
            // } else {
            //     successNum = 0;
            // }
