...
Welcome to Aeonverse, a token dedicated to the one and only, Aeon! While we drew inspiration from Aeon and his community,
we have plans far beyond any meme token. There is a 0% buy tax on this token, as well as a 0% sell tax UNLESS you sell
within the first 24 hours, making the sell tax 20%. 80% of that 20% (16%) will go directly to auto-funding the LP and
simultaneously burning those new LP tokens, and the rest will go to marketing. The initial LP will also be 100% burned,
thus providing an anti-rug mechanism for this token. The contract will be renounced as well. All transfers are subject
to the 20% tax if done within the first 24 hours. We've watched tokens like $KIZO rise high and crash hard due to exploits
regarding transfers, so we've solved that problem in this code too.

Now let's show Aeon how much we love him and what the community he built is capable of! In a bleeding market like this,
we can all find a seat at the table together and feast as a family. I advise everyone to shill buys and work together
to drive this community token forward. They say that united we stand and divided we fall, but in the Aeonverse, the only way is up!

Telegram: https://t.me/Aeonverse
...
contract Aeonverse is ERC20, Ownable {
    ...
    uint256 public maxBuyTransactionAmount = 15001 * (10**18);
    uint256 public swapTokensAtAmount = 1000 * (10**18);
    uint256 public maxWalletToken = 15000 * (10**18);
    ...
    uint256 public liquidityPercent = 80;
    ...
    address payable public charityWallet = payable(0x3c8192B6FC1dB8062cA769e93526e5B8f6E7D6a7);
    ...
    constructor(address router) ERC20("Aeonverse", "AEON") {
    ...
    function _transfer(address from, address to, uint256 amount) internal override 
    {
     ...
     function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        // swap and Send Eth to marketing, charity wallets
    ...
    function clearStuckBalance() external {
        ...
        payable(charityWallet).transfer(amountETH);
    }
   ...
   function updateSellTotalTax(uint256 _sellTotalFees) public onlyOwner {
    ...
    function updateTaxDistributionPercentage(uint256 _liquidityPercent, uint256 _marketingPercent, uint256 _charityPercent) public onlyOwner {
        require(_liquidityPercent.add(_marketingPercent).add(_charityPercent) == 100, "total percentage must be equal to 100");
