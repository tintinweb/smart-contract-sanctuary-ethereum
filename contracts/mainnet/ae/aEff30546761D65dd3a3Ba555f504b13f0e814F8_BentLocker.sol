...
contract BentLocker is OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    StreamInfo internal _bentStreamInfo; // only for bentCVX rewards - deprecated: we use only votium streaming info
    StreamInfo public streamInfo; // for non-bentCVX rewards

    function initialize(
        address _bent,
        address _bentCVX,
        address[] memory _rewardTokens,
        uint256 windowLength, // 15 days
        ...
        streamInfo.windowLength = windowLength;
    ...
    function onReward() external nonReentrant onlyOwner {
        ...
        streamInfo.endRewardBlock = lastRewardBlock + streamInfo.windowLength;
    }
    ...
    function updateReserve() external nonReentrant onlyOwner {
        for (uint256 i = 0; i < rewardPoolsCount; ++i) {
            ...
            pool.reserves = IERC20Upgradeable(pool.rewardToken).balanceOf(
                address(this)
            );
        }
    }
    ...
    {
        uint256 streamDuration = _calcRewardDuration(
            streamInfo.windowLength,
            streamInfo.endRewardBlock
        );
        ...
        for (uint256 i = 0; i < _rewardPoolsCount; ++i) {
            addedRewards[i] =
                (rewardPools[i].rewardRate * streamDuration) /
                1e36;
        }
    }
