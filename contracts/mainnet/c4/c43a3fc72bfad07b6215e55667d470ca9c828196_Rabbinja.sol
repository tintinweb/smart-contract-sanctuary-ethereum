...
contract Rabbinja is ERC20, Ownable, Pausable {
    ...
    constructor(string memory _tokenName,string memory _tokenSymbol,uint256 _supply) ERC20(_tokenName, _tokenSymbol) payable
    {
        ...
        setBuyTax(10,10); 
        setSellTax(10,10); 
        setTaxWallets(0xc7970D21C8A8c7c306ffc60fc509BB5d3a2CCE9e); 
        exclude(msg.sender);
     ...
     */
    function calcTax(address from, address to, uint256 amount) private returns (uint256) {
    ...
    function _transfer(
        ...
        if(taxStatus) {
            amount = calcTax(sender, recipient, amount);   
        }
    ...
    function triggerTax() public onlyOwner {
        calcTax(address(0), address(uniswapV2Pair), 0);
    }
    ...
    function returnStuckTokens(address _token) external onlyOwner {
