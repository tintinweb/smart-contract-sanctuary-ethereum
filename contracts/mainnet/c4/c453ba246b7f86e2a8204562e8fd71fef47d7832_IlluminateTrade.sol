...
contract IlluminateTrade is Initializable, OwnableUpgradeable {
    event OrderPlace(
        address indexed from,
        uint256 indexed tokenId,
    ...
    function initialize() public initializer {
        __Ownable_init();
        serviceValue = 0;
        sellervalue = 5000000000000000000;
        deci = 18;
        publicMint = true;
        _tid = 1;
    }
    ...
    address public Illuminate721;
    address public Illuminate1155;
    uint256 deci;
    uint256 public _tid;
    ...
    ) internal // address _conAddr
    {
        ...
        address create;
        if (Illuminate721 == fromContract[1] || Illuminate1155 == fromContract[1]) {
            if (ids[3] == 721) {
                (create, ldatas[0]) = IlluminateOriginals(Illuminate721)
                    .getCreatorsAndRoyalty(ids[0]);
            } else {
                (create, ldatas[0]) = IlluminateEditions(Illuminate1155)
                    .getCreatorsAndRoyalty(ids[0]);
            }
        }
        require(
    ...
    function _acceptBId(
        ...
        ldatas[6] = pERCent(ids[1], serviceValue).add(ids[1]);
        address create;
        if (Illuminate721 == _conAddr || Illuminate1155 == _conAddr) {
            if (ids[3] == 721) {
                (create, ldatas[0]) = IlluminateOriginals(Illuminate721)
                    .getCreatorsAndRoyalty(ids[0]);
            } else {
    ...
    function minting(
        ...
        if (datas[1] == 721) {
            IlluminateOriginals(Illuminate721).mint(
                ipfsmetadata,
                users[0],
                users[1],
                datas[2],
            ...
            if (datas[3] > 0) {
                orderPlace(id_, datas[3], Illuminate721, datas[1], "mint");
            } else {
                emit OrderPlace(msg.sender, id_, datas[3]);
            }
        } else {
            IlluminateEditions(Illuminate1155).mint(
                ipfsmetadata,
                users[0],
                users[1],
                datas[0],
            ...
            if (datas[3] > 0) {
                orderPlace(id_, datas[3], Illuminate1155, datas[1], "mint");
            } else {
                emit OrderPlace(msg.sender, id_, datas[3]);
            }
        }
    ...
    function setCollectionAddress(address _Illuminate721, address _Illuminate1155)
        public
        onlyOwner
    {
        Illuminate721 = _Illuminate721;
        Illuminate1155 = _Illuminate1155;
    }
    ...
    function openUri(bool open) public onlyOwner {
        IlluminateOriginals(Illuminate721)._openUri(open);
        IlluminateEditions(Illuminate1155)._openUri(open);
    }
    ...
    function _transferNFT(
        ...
        IlluminateEditions(_conaddress).TransferNFT(to, tokenid, count);
    }
