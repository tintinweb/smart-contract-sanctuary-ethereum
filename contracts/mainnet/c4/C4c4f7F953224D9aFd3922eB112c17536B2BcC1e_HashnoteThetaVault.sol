...
library GnosisAuction {
        ...
        returns (uint256 auctionID)
    {
        ...
        // shift decimals to correspond to decimals of vaultParams.decimals
        ...
        auctionID = IGnosisAuction(auctionDetails.gnosisEasyAuction)
            ...
            auctionDetails.whitelistAddress,
    ...
    function claimAuctionOtokens(
        ...
        IGnosisAuction(gnosisEasyAuction).claimFromParticipantOrder(
            IHashnoteThetaVault(counterpartyThetaVault).optionAuctionID(),
            orders
        );
    }
...
library VaultLifecycle {
    ...
    function commitNextOption(
        ...
        require(commitParams.strikePrice != 0, "!strikePrice");

        // retrieve address if option already exists, or deploy it
        otokenAddress = getOrDeployOtoken(commitParams, vaultParams, expiry);
    ...
    function verifyOtoken(
        ...
        require(
            otoken.collateralAsset() == commitParams.collateralAsset,
            "Wrong collateralAsset"
        );
        require(
            otoken.strikeAsset() == commitParams.strikeAsset,
            "Wrong strikeAsset"
        );

        uint256 readyAt = block.timestamp.add(commitParams.delay);
    ...
    function createShort(
        ...
        if (oToken.isPut()) {
            ...
            .mul(10**18).div( // we use 10**18 to give extra precision
                    oToken.strikePrice().mul(10**(10 + collateralDecimals))
                );
        } else {
            mintAmount = depositAmount;
        ...
        returns (uint256)
    {
        ...
        // Burning `amount` of oTokens from the Hashnote vault,
        ...
        )
    {
        ...
        uint256 _managementFeeInAsset = managementFeePercent > 0
            ? lockedBalanceSansPending.mul(managementFeePercent).div(
                100 * Vault.FEE_MULTIPLIER
            )
            : 0;

        uint256 _vaultFee = _managementFeeInAsset;
        ...
        if (lockedBalanceSansPending > lastLockedAmount) {
            ...
            _vaultFee = _vaultFee.add(_performanceFeeInAsset);
        }
    ...
    function getOrDeployOtoken(
        ...
        address otokenFromFactory = factory.getOtoken(
            ...
            commitParams.strikeAsset,
            commitParams.collateralAsset,
            commitParams.strikePrice,
            expiry,
            vaultParams.isPut
        );
        ...
        verifyOtoken(otoken, commitParams, vaultParams);
    ...
    ERC20Upgradeable
{
    ...
    // This is to prevent storage collisions. All storage variables should be appended to HashnoteThetaVaultStorage
     ...
     * @param newWhitelist is the address of the new keeper
     */
    ...
    function _validateWhitelisted(address account) internal view {
        if (whitelist != address(0)) {
            require(
                IWhitelistManager(whitelist).isCustomerWhitelisted(account),
                "Account not permitted to deposit"
            );
        }
    }
