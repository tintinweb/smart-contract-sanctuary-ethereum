...
contract NFTStaking is Ownable {
    ...
    function leaveStaking(uint256 _stakeId) external {
    ...
    function getUserStakesByIndex(address _user, uint256 _index) external view returns (uint256 stakeNumber, uint256 tokenNumber, uint256 startTimestamp) {
        uint256 totalStakes = _stakeIds.current() + 1;
        for(uint256 i=0; i < totalStakes; i++ ){
            if(stakeOwner[i] == _user && isActive[i] == true) {
                if(_index == 0){
                   
                    return (i, stakes[i].tokenId, stakes[i].startTimestamp);
                }
                _index = _index - 1;
            }
        }
