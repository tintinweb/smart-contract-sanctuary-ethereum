...
/// @dev Interface that post-withdraw hooks must implement to interop with StealthPay
interface IStealthPayHookReceiver {
  /**
   * @notice Method called after a user completes an StealthPay token withdrawal
...
contract StealthPay is Ownable {
  // =========================================== Events ============================================
  ...
  function collectTolls() external {
    require(msg.sender == tollCollector, "StealthPay: Not toll collector");
   ...
   * see https://github.com/CryptoAdong/StealthPay-contracts-core/ for more information
  ...
  function sendEth(
    ...
    require(_tollCommitment == toll, "StealthPay: Invalid or outdated toll commitment");

    // also protects from underflow
    require(msg.value > toll, "StealthPay: Must pay more than the toll");
  ...
  function sendToken(
    ...
    require(msg.value == toll, "StealthPay: Must pay the exact toll");
    require(tokenPayments[_receiver][_tokenAddr] == 0, "StealthPay: Cannot send more tokens to stealth address");
  ...
  function withdrawToken(address _acceptor, address _tokenAddr) external {
    _withdrawTokenInternal(msg.sender, _acceptor, _tokenAddr, address(0), 0, IStealthPayHookReceiver(0), "");
  }
  ...
  function withdrawTokenOnBehalf(
    ...
    _validateWithdrawSignature(
      ...
      _sponsor,
      _sponsorFee,
      IStealthPayHookReceiver(0),
      "",
      _v,
      _r,
      _s
    );
    _withdrawTokenInternal(_stealthAddr, _acceptor, _tokenAddr, _sponsor, _sponsorFee, IStealthPayHookReceiver(0), "");
  }
  ...
  function _withdrawTokenInternal(
    ...
    // also protects from underflow
    require(_amount > _sponsorFee, "StealthPay: No balance to withdraw or fee exceeds balance");
  ...
  function _validateWithdrawSignature(
    ...
    require(_recoveredAddress != address(0) && _recoveredAddress == _stealthAddr, "StealthPay: Invalid Signature");
  }
}