...
/*
Welcome To TWITTLA.                                                                                  
Elon Twitter Takeover PLAY!!
Buy/Sell = 3%/5%
max buy 1%
max wallet 4%
Ownership Renounced + Locked for 7 days
*/
...
contract ELONOWNSTWITTERNOW is Context, IERC20, Ownable { 
    mapping (address => uint) private _owned;
    mapping (address => mapping (address => uint)) private _allowances;
    mapping (address => bool) private _isExcludedFromFee;
    mapping (address => User) private cooldown;
    uint private constant _totalSupply = 42000000000000 * 10**decimals;

    string public constant name = "TWITTLA"; 
    string public constant symbol = "TWITTLA"; 
    ...
    function _transfer(address from, address to, uint amount) private {
        ...
        if(from != owner() && to != owner()) {
            ...
            if(from == uniswapV2Pair && to != address(uniswapV2Router) && !_isExcludedFromFee[to]) {
                ...
                if((_launchedAt + (3 seconds)) > block.timestamp) {
