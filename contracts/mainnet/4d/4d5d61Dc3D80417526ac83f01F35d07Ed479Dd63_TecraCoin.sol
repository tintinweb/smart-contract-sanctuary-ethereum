...
abstract contract AccessControl is IAccessControl, ERC165 {
    ...
    function renounceRole(
        ...
        if (account != msg.sender) revert CanOnlyRenounceRolesForSelf();
        _revokeRole(role, account);
    }
...
contract ERC20 is IERC20, IERC20Metadata {
    ...
    function _transfer(
        ...
        if (from == address(0)) revert TransferFromTheZeroAddress();
    ...
    function _mint(address account, uint256 amount) internal virtual {
        if (account == address(0)) revert MintToTheZeroAddress();
    ...
    function _burn(address account, uint256 amount) internal virtual {
        if (account == address(0)) revert BurnFromTheZeroAddress();

        _beforeTokenTransfer(account, address(0), amount);

        uint256 accountBalance = _balances[account];
        if (amount > accountBalance) revert BurnAmountExceedsBalance();
        unchecked {
    ...
    function _approve(
        ...
        if (owner == address(0)) revert ApproveFromTheZeroAddress();
...
contract TecraCoin is ERC20Pausable, ERC20Burnable, AccessControlEnumerable {
    ...
    constructor(
        ...
        if (bytes(name_).length < 3) revert();
        if (bytes(symbol_).length < 2) revert();
    ...
    */
     ...
     */
    function getBlacklistTimestamp(
        address user
    ) external view returns (uint256) {
    ...
    function getMintRequest(
        uint256 idx
    ) external view returns (address user, uint256 amount, uint256 signed) {
