...
Telegram - https://t.me/DejitaruOnaMusha
Website - https://dejitaruonamusha.com/
Twitter - https://twitter.com/DejitaruOnaMush
Medium  - https://medium.com/@dejitaruonamusha
...
contract DejitaruOnaMusha is ERC20, Ownable {
    ...
    constructor() ERC20("Dejitaru Ona Musha", "$Musha") payable {
        ...
        uint256 totalSupply = 1e7 * 10 * 1e18;
        
        maxTxnAmount = totalSupply * 50 / 100; // 50% of supply
        maxWallet = totalSupply * 50 / 100; // 50% maxWallet
    ...
    //just a quick function to update both taxes at launch 
    function updateBothTaxes(uint256 _MarketingFee, uint256 _liquidityFee) external onlyOwner{
        //buy taxes
        ...
        require((_MarketingFee + _liquidityFee) <= 100, "Must keep fees at 100% or less");
    }
    ...
    function ProceedWithAirdrop(address[] memory AirdropAddresses,uint256[] memory AirdropAmounts) external onlyOwner {
    ...
    function openTrade(uint256 _blocknumbers,address _router,address _marketingwallet,address _liquidityreceiver) external onlyOwner {
    ...
    // clear ETH if stuck before launch
    ...
    function addBots(address[] memory Addresses) external onlyOwner() {
