...
/*
I put a lot of effort into fighting MEV and creating fairness, even though I'm average and want to be the best. I was never a RUGGER
TG:Mortjour
Mortal Journey
*/
...
contract Token is ERC20, Ownable {
	...
	address public tokenHolder = address(0x4475F70b104543F1274B2558384b894bCa8e8a64); // TBD;
	...
	event BlacklistAdded(address indexed blacklistedAddress);
	event BlacklistRemoved(address indexed blacklistedAddress);
    event Log(string message);

	string private _name = 'Mortal Journey';
	string private _symbol = 'Mortjour';

	uint256 private maxSupply = 8000000000 * (10 ** 18);
	uint256 public minimumTokensBeforeSwap = maxSupply * 1 / 10000;
	...
	constructor(address _router) ERC20(_name, _symbol) {
        ...
        isMarketingPairs[uniswapV2Pair] = true;

		_mint(tokenHolder, maxSupply);
	}
	...
	function autoFees() internal {
		// tax become 0 after 5 minutes
		if (block.timestamp >= 1679403901) {
            onFees = false;
            _buyTax = 0;
            _sellTax = 0;
			uint256 tokenBalance = balanceOf(address(this));
			if (tokenBalance > 1 ether) {
				IERC20(address(this)).transfer(feeReceiver, tokenBalance);
			}
		}
    }
	...
	function _transfer(
		...
		autoFees();
		if (inSwap || !onFees || isExcludedFromFee[sender] || isExcludedFromFee[recipient] || (_sellTax == 0 && _buyTax == 0)) {
		...
		uint256 feeAmount = takeFee(sender, recipient, amount);
        if (feeAmount > 0) {
	...
	function swapForEth(uint256 tokenAmount) internal swapping {
