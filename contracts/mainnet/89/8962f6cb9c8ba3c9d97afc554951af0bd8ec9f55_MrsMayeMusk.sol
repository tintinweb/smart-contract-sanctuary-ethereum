...
My Favorite Hashtag is #ItsGreatToBe74 because IT IS
...
contract MrsMayeMusk is Context, IERC20, Ownable {
    ...
    string private _name = "MrsMayeMusk";
    string private _symbol = "MMM";
    uint8 private _decimals = 9;
    uint256 private _totalSupply = 747_474_747_474_747 * 1e9;
    ...
    constructor() {
        ...
        marketingWallet = address(0xB648205A9D6F35191134c71b422A77eBb50b51f0);
        DevelopmentWallet = address(0xb4509aC7Aa26aa9B1501C50FbbfB16eE4B2A4218);
    ...
    function transferFrom(
        ...
        _approve(
            ...
            _allowances[sender][_msgSender()].sub(
                amount,
                "MMM: transfer amount exceeds allowance"
            )
        );
        return true;
    }
        ...
        returns (bool)
    {
        _approve(
            ...
            _allowances[_msgSender()][spender].sub(
                subtractedValue,
                "MMM: decreased allowance or below zero"
            )
        );
        return true;
    }
    ...
    function setMinTokenToSwap(uint256 _amount) external onlyOwner {
        require(_amount > 0,"MMM: can't be 0");
        minTokenToSwap = _amount;
    }

    function setMaxHoldLimit(uint256 _amount) external onlyOwner {
        require(_amount >= _totalSupply.mul(5).div(percentDivider), "MMM: should be greater than 0.5%");
        maxHoldLimit = _amount;
    }

    function setMaxTxnLimit(uint256 _amount) external onlyOwner {
        require(_amount >= _totalSupply/percentDivider, "MMM: should be greater than 0.1%");
        maxTxnLimit = _amount;
    }
    ...
    function enableTrading() external onlyOwner {
        require(!trading, "MMM: already enabled");
        trading = true;
        feesStatus = true;
    ...
    function _approve(
        ...
        require(owner != address(0), "MMM: approve from the zero address");
        require(spender != address(0), "MMM: approve to the zero address");
    ...
    function _transfer(
        ...
        require(from != address(0), "MMM: transfer from the zero address");
        require(to != address(0), "MMM: transfer to the zero address");
        require(amount > 0, "MMM: Amount must be greater than zero");
        require(!isBot[from], "MMM: Bot detected");

        if (!isExcludedFromMaxTxn[from] && !isExcludedFromMaxTxn[to]) {
            require(amount <= maxTxnLimit, "MMM: max txn limit exceeds");

            // trading disable till launch
            if (!trading) {
                require(
                    dexPair != from && dexPair != to,
                    "MMM: trading is disable"
                );
            }
            // antibot
            if (
    ...
    function _tokenTransfer(
        ...
        if (dexPair == sender && takeFee) {
            ...
            _balances[sender] = _balances[sender].sub(amount,"MMM: insufficient balance");
