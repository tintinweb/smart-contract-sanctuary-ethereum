...
// File: contracts/LioniaClubHeroes.sol


pragma solidity ^0.8.0;


contract LioniaClubHeroes is ERC721, Ownable, RandomlyAssigned {
  using Strings for uint256;
  using Counters for Counters.Counter;

  Counters.Counter private supply;

  string public uriPrefix = "ipfs://bafybeid3evpkynzpeu4urarcr7rfyamalnfc56qu3iqd377cnaqogohyvu/";
  ...
  function mint(uint256 _mintAmount) public payable mintCompliance(_mintAmount) {
    require(!paused, "Minting is not available!");
  ...
  function mintedTokens()
    ...
    returns (uint256[] memory)
  {
    uint256 mintedTokenCount = tokenCount();
    uint256[] memory mintedTokenIds = new uint256[](mintedTokenCount);
    uint256 currentTokenId = 1;
    uint256 foundTokenIndex = 0;

    while (foundTokenIndex < mintedTokenCount && currentTokenId <= maxSupply) {
        if (_exists(currentTokenId)) {
            mintedTokenIds[foundTokenIndex] = currentTokenId;
            foundTokenIndex++;
        }
        currentTokenId++;
    }
