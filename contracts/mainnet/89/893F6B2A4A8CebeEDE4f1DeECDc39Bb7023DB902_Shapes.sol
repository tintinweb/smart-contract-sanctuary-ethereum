  ...
  OwnableUpgradeable 
{
  ...
  // (tokenId => timestamp)
  mapping (uint => uint) public changeDeadline;
  ...
  error ZeroAddress();
  error Unauthorized();
  error Wait(uint deadline);
  error Locked();
  error Ended();

  // modifiers
  function _salesIsOpen() private view {
  ...
  function _canBeChanged(uint _tokenId) internal view {
    if (changeDeadline[_tokenId] > block.timestamp) {
      revert Wait(changeDeadline[_tokenId]);
    }
    if(isLocked[_tokenId] == true) {
      revert Locked();
    }
  ...
  * NFT functions 
  */
  function mint(address _recipient) payable public returns (uint256) {
    _salesIsOpen();
    _projectNotEnded();
    if(msg.value < priceToMint) { 
      revert InsufficientPrice(); 
    }
    uint tokenId = _mintShape(_recipient, defaultShapeIndex, 0);
    return tokenId;
  }
  ...
  function batchMint(address _recipient, uint256 numberOfNfts) public payable {
    _salesIsOpen();
    _projectNotEnded();
    ...
    for (uint i = 0; i < numberOfNfts; i++) {
        _mintShape(_recipient, defaultShapeIndex, 0);
    }
  }
              ...
              Base64.encode(
                  bytes(
                      abi.encodePacked(
                        ...
                        '","image":"',
                        svgString,
                        '", "external_link": "https://makio135.io/shapes"}'
                        )
                  )
              )
          )
  ...
  function tokenURI(uint256 tokenId) 
    ...
    returns (string memory) 
    {
      ...
      (string memory svgString, string memory attributes) = IShape(shapesAddresses[shape(tokenId)]).generateSVG(tokenId);
  ...
  function setDefaultShapeIndex(uint _shapeIndex) public onlyOwner {
    if(_shapeIndex > shapesAddresses.length -1) {
      revert WrongNumber();
    }
    defaultShapeIndex = _shapeIndex;
  }
  ...
  function changeToken(
    ...
    _nextSeed.increment();
    changeDeadline[_tokenId] = block.timestamp + interval;
  }
