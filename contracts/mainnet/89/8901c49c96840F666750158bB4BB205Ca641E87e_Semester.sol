...
contract Semester is ERC721  {
    ...
    constructor()
        ERC721("Semester", "Semester")
    {}

    function mint() external payable {
        
        if (userMints[msg.sender] >= maxMints) revert MintLimit();
        if (totalSupply + 1 > 105) revert SoldOut();
        ...
        returns (string memory)
    {
        return
            string(
                abi.encodePacked("ipfs://bafybeigz4w5dx7wrpexu7wzaenq6s45ie3wnfq4fzc6nao4yaykqfrsnua/", Strings.toString(tokenId), ".json")
            );
    }
