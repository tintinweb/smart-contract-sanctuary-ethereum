...
contract CannyWolvesPack2 is ERC721A, Ownable, ReentrancyGuard {
    using Address for address;
    ...
    uint256 public WLmintPrice = 0.009 ether;
    uint256 public collectionSize = 4000;
    uint256 public whitelistMintMaxSupply = 1500;
    uint256 public devMintLimit = 250;
    uint256 public maxItemsPerWalletWL = 3;
    uint256 public maxItemsPerWalletPublic = 10;
    uint256 public maxItemsPerTx = 10;

    bool public whitelistMintPaused = true;
    bool public freeWLMintPaused = true;
    bool public publicMintPaused = true;

    bytes32 whitelistMerkleRoot;
    bytes32 freeWLMerkleRoot;

    mapping(address => uint256) public publicMintedAmount;
    mapping(address => uint256) public whitelistMintedAmount;
    mapping(address => uint256) public freeWLMintedAmount;

    // ===== Constructor =====
    constructor() ERC721A("Canny Wolves Pack 2.0", "CWP2", 10) {}
    ...
    function devMint(uint256 amount) external onlySender onlyOwner {
        require(amount <= devMintLimit, "Minting amount exceeds reserved size");
        ...
        nonReentrant
    {
        ...
        uint256 amount = _getMintAmount(msg.value, WLmintPrice);

        require(
            whitelistMintedAmount[msg.sender] + amount <= maxItemsPerWalletWL,
    ...
    // ===== freeWL mint =====
    function freeWLMint(bytes32[] memory proof)
        external
        onlySender
        nonReentrant
    {
        require(!freeWLMintPaused, "freeWL mint is paused");
        require(
            isAddressOnfreeWL(proof, msg.sender),
            "You are not eligible for a freeWL mint"
        );
        require(whitelistMintMaxSupply >= 1, "Whitelist mint is sold out");
        require(
            freeWLMintedAmount[msg.sender]  < 1,
            "Minting amount exceeds allowance per wallet"
        );
        require((totalSupply() + 1) <= collectionSize, "Sold out!");
        whitelistMintMaxSupply = whitelistMintMaxSupply - 1;
        freeWLMintedAmount[msg.sender] += 1;
    ...
    function publicMint() external payable onlySender nonReentrant {
        ...
        require(
            publicMintedAmount[msg.sender] + amount <= maxItemsPerWalletPublic,
            "Minting amount exceeds allowance per wallet"
        );
        require(
            amount <= collectionSize-totalSupply()-whitelistMintMaxSupply,
            "Minting amount exceeds public mint supply"
        );
    ...
    // ===== Helper =====
    function _getMintAmount(uint256 value, uint256 price) internal view returns (uint256) {
        uint256 remainder = value % price;
    ...
    function isAddressOnfreeWL(bytes32[] memory proof, address _address)
        public
        view
        returns (bool)
    {
        return isAddressInMerkleRoot(freeWLMerkleRoot, proof, _address);
    }
    ...
    function setfreeWLMintPaused(bool _freeWLMintPaused) external onlyOwner {
        freeWLMintPaused = _freeWLMintPaused;
    }
    ...
    function setfreeWLMintMerkleRoot(bytes32 _freeWLMerkleRoot)
        external
        onlyOwner
    {
    ...
    function setMaxItemsPerWalletWL(uint256 _maxItemsPerWalletWL)
        external
        onlyOwner
    {
        maxItemsPerWalletWL = _maxItemsPerWalletWL;
    }
    function setMaxItemsPerWalletPublic(uint256 _maxItemsPerWalletPublic)
        external
        onlyOwner
    {
        maxItemsPerWalletPublic = _maxItemsPerWalletPublic;
    }
    function setCollectionSize(uint256 _collectionSize)
        external
        onlyOwner
    {
