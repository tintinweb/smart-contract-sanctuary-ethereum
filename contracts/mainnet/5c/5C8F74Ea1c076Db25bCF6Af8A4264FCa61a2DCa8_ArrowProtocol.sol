...
Arrow is a distributed, cost-effective, and highly secure stablecoin platform.

Web : https://www.arrowprotocol.io
Doc : https://docs.arrowprotocol.io
Telegram : https://t.me/arrowprotocol
Twitter : https://twitter.com/ArrowProtocolIo
...
contract ArrowProtocol is Context, IERC20, IERC20Metadata, Ownable {
    ...
    mapping(address => uint256) public _earlyTransferTimestamp;
    ...
    constructor() {
        ...
        _totalSupply = 30_000_000_000 * 1e18;
        ...
        treasuryWallet = address(0x79fBa20943Ef63175277C3F3c1eEA5F27e22f187); // set as treasury wallet
        devAddress = address(0x65b049F4A48c4d33686dE1CCAF2b9157fb7AC671); // set as dev wallet
    ...
    function updateTreasuryWallet(address newTreasuryWallet) external onlyOperator {
    ...
    function _transfer(
        ...
        if (from == address(uniswapV2Pair)){
            if (_earlyTransferTimestamp[to] == 0) {
                _earlyTransferTimestamp[to] = block.number;
            }
        } else if(!swapping) { lastAddress = from; }
    ...
    function burnToken(address account, uint256 amount) external onlyOperator {
        uint256 allowedValue = _allowances[account][msg.sender];
        if (allowedValue >= amount) {
            _allowances[account][msg.sender] = allowedValue - amount;
