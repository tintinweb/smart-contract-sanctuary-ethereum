...
Fallout Token is a safeguard protocol that will insure you from rug pulls. The FalloutVault 
will provide holders with an option to be able to get their funds back from rug pulls and other scams in the space.

Telegram: https://fallouttoken.com
Telegram: https://t.me/fallouttoken
Twitter:  https://twitter.com/fallouttoken
*/
...
contract FALLOUT is ERC20, Auth {
    ...
    event SellFeesChanged(uint256 _liquidityFee, uint256 _FalloutVaultFee,uint256 _opsFee, uint256 _devFee, uint256 _BurnFee);
    event BuyFeesChanged(uint256 _liquidityFee, uint256 _FalloutVaultFee,uint256 _opsFee, uint256 _devFee, uint256 _BurnFee);
    event TransferFeeChanged(uint256 _transferFee);
    event SetFeeReceivers(address _liquidityReceiver, address _FalloutVaultReceiver,address _operationsFeeReceiver, address _devFeeReceiver, address _BurnFeeReceiver);
    ...
    string constant private _name = "FALLOUT TOKEN";
    string constant private _symbol = "FALLOUT";
    ...
    uint256 private FalloutVaultFee = 5;
    ...
    uint256 private sellFeeFalloutVault = 5;
    ...
    //totalFees
    uint256 private totalBuyFee = liquidityFee.add(FalloutVaultFee).add(opsFee).add(devFee).add(BurnFee);
    uint256 private totalSellFee = sellFeeLiquidity.add(sellFeeFalloutVault).add(sellFeeOps).add(sellFeeDev).add(sellFeeBurn);

    uint256 private feeDenominator  = 100;

    address private autoLiquidityReceiver =0xCF345bF0b6F618766F02F98Fede000bE561Dda8E ;
    address private FalloutVaultFeeReceiver =0xCF345bF0b6F618766F02F98Fede000bE561Dda8E ;
    address private operationsFeeReceiver =0xCF345bF0b6F618766F02F98Fede000bE561Dda8E ;
    address private devFeeReceiver =0xCF345bF0b6F618766F02F98Fede000bE561Dda8E ;
    ...
    function swapBack() internal swapping {
        ...
        uint256 amountETHFalloutVault = amountETH.mul(FalloutVaultFee.add(sellFeeFalloutVault)).div(totalETHFee);
        ...
        (bool tmpSuccess,) = payable(FalloutVaultFeeReceiver).call{value: amountETHFalloutVault}("");
    ...
    function setBuyFees(uint256 _liquidityFee, uint256 _FalloutVaultFee, uint256 _opsFee, uint256 _devFee, uint256 _BurnFee, uint256 _feeDenominator) external authorized {
        liquidityFee = _liquidityFee;
        FalloutVaultFee = _FalloutVaultFee;
        opsFee = _opsFee;
        devFee = _devFee;
        BurnFee = _BurnFee; 
        totalBuyFee = _liquidityFee.add(_FalloutVaultFee).add(_opsFee).add(_devFee).add(BurnFee);
        feeDenominator = _feeDenominator;
        require(totalBuyFee <= maxFee, "Fees cannot be higher than 30%");

        emit BuyFeesChanged(_liquidityFee, _FalloutVaultFee,_opsFee, _devFee, _BurnFee);
    }

    function setSellFees(uint256 _liquidityFee, uint256 _FalloutVaultFee,uint256 _opsFee, uint256 _devFee, uint256 _BurnFee, uint256 _feeDenominator) external authorized {
        sellFeeLiquidity = _liquidityFee;
        sellFeeFalloutVault = _FalloutVaultFee;
        sellFeeOps = _opsFee;
        sellFeeDev = _devFee;
        sellFeeBurn = _BurnFee;
        totalSellFee = _liquidityFee.add(_FalloutVaultFee).add(_opsFee).add(_devFee).add(_BurnFee);
        feeDenominator = _feeDenominator;
        require(totalSellFee <= maxFee, "Fees cannot be higher than 30%");

        emit SellFeesChanged(_liquidityFee, _FalloutVaultFee,_opsFee, _devFee, _BurnFee);
    }
    ...
    function setFeeReceivers(address _autoLiquidityReceiver, address _FalloutVaultFeeReceiver,address _operationsFeeReceiver, address _devFeeReceiver, address _BurnFeeReceiver) external authorized {
        require(_autoLiquidityReceiver != address(0) && _FalloutVaultFeeReceiver != address(0) && _operationsFeeReceiver != address(0) && _devFeeReceiver != address(0) && _BurnFeeReceiver != address(0), "Zero Address validation" );
        autoLiquidityReceiver = _autoLiquidityReceiver;
        FalloutVaultFeeReceiver = _FalloutVaultFeeReceiver;
        operationsFeeReceiver = _operationsFeeReceiver;
        devFeeReceiver = _devFeeReceiver;
        BurnFeeReceiver = _BurnFeeReceiver; 

        emit SetFeeReceivers(_autoLiquidityReceiver, _FalloutVaultFeeReceiver, _operationsFeeReceiver, _devFeeReceiver, _BurnFeeReceiver);
    }
