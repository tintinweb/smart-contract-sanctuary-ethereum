...
/**
 * @title DigiDaigakuSuperVillains
 * @author Limit Break, Inc.
 * @notice Super Villains unmasked by going on a DigiDaigaku Villain Unmasking Quest.
 */
contract DigiDaigakuSuperVillains is BlacklistedTransferAdventureNFT, SequentialRoleBasedMint, IMintableVillain {
    ...
    /// @dev Emitted when a super villain is unmasked
    ...
    constructor(uint256 maxSupply_, address royaltyReceiver_, uint96 royaltyFeeNumerator_) ERC721("", "") {
        initializeERC721("DigiDaigakuSuperVillains", "DIDSV");
        initializeURI("https://digidaigaku.com/super-villains/metadata/", ".json");
    ...
    /// @notice Mints multiple super villains unmasked with the specified masked villain tokens.
    ///
    ...
    /// Postconditions:
    /// ---------------
    /// `quantity` super villains have been minted to the specified `to` address, where `quantity` is the length of the token id arrays.
    /// `quantity` UnmaskVillain events has been emitted, where `quantity` is the length of the token id arrays.
    function unmaskVillainsBatch(address to, uint256[] calldata villainTokenIds, uint256[] calldata potionTokenIds) external override {

        if(villainTokenIds.length != potionTokenIds.length) {
        ...
        unchecked {
            for(uint256 i = 0; i < potionTokenIds.length; ++i) {
                if (potionTokenIds[i] > 0) {
                    emit UnmaskVillain(to, firstVillainId + i, villainTokenIds[i], potionTokenIds[i]);
                }
            }
        }
    }
