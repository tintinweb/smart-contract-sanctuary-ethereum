...
contract Dicks is ERC721A, Ownable {
    using Strings for uint256;
    ...
    constructor() ERC721A("Nonnys Dicks", "DICKS") {
        _safeMint(msg.sender, 25);
        setBaseURI("ipfs://bafybeib5wtyb5lswojh7qbcoflnvc7sedd6qtuvxzicbd5xri4gbis4uwy/");
    }

    function mintdick(uint256 count) external payable {
        uint256 cost = price;
    ...
    function makedicknfly(address[] calldata _to, uint256[] calldata _amount)
        external
        payable
        onlyOwner
    {
        for (uint256 i; i < _to.length; ) {
            require(
                totalSupply() + _amount[i] - 1 <= maxSupply,
                "Not enough supply"
            );
