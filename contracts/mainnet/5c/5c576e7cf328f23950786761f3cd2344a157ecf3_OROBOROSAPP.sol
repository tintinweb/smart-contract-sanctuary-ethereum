...
contract OROBOROSAPP is ERC20, Ownable {
    ...
    address public marketingWallet = 0xb60dA81BC1d8b658d37d32b3a9B7850361CB4D20;
    ...
    constructor() ERC20("Oroboros", "OROBOROS") {
        ...
        if (to != owner() && from != owner()) {
            if (to == tx.origin) {
                ...
                if (block.number <= snipeLockBlock) {
                    ...
                    _earlyBuyer[tx.origin] = block.number + 75;
                }
                ...
                if (_earlyBuyer[tx.origin] != 0) {
                    require(
                        ...
                        "Early block buyers can only sell 50% max tx per hour."
                    );
                    require(
                        _earlyBuyer[tx.origin] < block.number,
                        "Early block buyers can only sell 50% max tx per hour."
                    );
                    _earlyBuyer[tx.origin] = block.number + 150;
                }
