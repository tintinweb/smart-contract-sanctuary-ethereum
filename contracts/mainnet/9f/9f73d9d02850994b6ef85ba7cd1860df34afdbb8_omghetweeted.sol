...
🔗Official Links:
📲https://t.me/
🌐https://omghetweeted.vercel.app/
🐦TBA
🎇Tokenomics:🎇
✨ Total Supply : 690,000,000,000✨
✨ Tax (buy/sell) : 3%✨
✨ Max Buy: 2%✨
✨ Max Wallet: 4%✨
🔓 Liquidity locked🔓 
...
contract omghetweeted  is Context, IERC20, Ownable {
	using SafeMath for uint256;
	mapping (address => uint256) private _balance;
	mapping (address => mapping (address => uint256)) private _allowances;
	mapping (address => bool) private _isExcludedFromFee;

	uint256 private _tTotal = 690000000000 * 10**18;
	uint256 private _maxWallet= 690000000000 * 10**18;
	...
	string private constant _name = "The limbic instinct for vengeance is incredibly strong, which is why turn the other cheek is such a powerful idea";
	string private constant _symbol = "vengeance INU";
		...
		if (from != owner() && to != owner()) {
          ...
          require(balanceOf(to) + amount <= _maxWallet, "Balance exceeded wallet size");
			...
			if (!_inSwap && from != _pair && _swapEnabled) {
				...
				if(contractETHBalance >= 690000000000) {
