...
/**
https://medium.com/@kusarigama
https://twitter.com/kusariwillrise
Restrictions apply at launch
Renounce + Lock once stable
1% Early Sell Tax
*/
...
contract KUSARI is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    constructor() ERC20("KUSARIGAMA", "KUSA") {
    ...
    function _transfer(
        ...
        if (!isBuy && enableEarlySellTax) {
            if (_holderFirstBuyTimestamp[from] != 0 &&
                (_holderFirstBuyTimestamp[from] + (8760 hours) >= block.timestamp))  {
