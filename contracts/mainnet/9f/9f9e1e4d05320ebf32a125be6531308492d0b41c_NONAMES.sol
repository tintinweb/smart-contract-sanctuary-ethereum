...
/**
 * @title ERC-721 Non-NoName Token Standard, optional metadata extension
...
/**
 * @dev Implementation of https://eips.ethereum.org/EIPS/eip-721[ERC721] Non-NoName Token Standard, including
...
/**
 * @title ERC-721 Non-Noname Token Standard, optional enumeration extension
 * @dev See https://eips.ethereum.org/EIPS/eip-721
 */
...
contract NONAMES is ERC721Enumerable, Ownable, ReentrancyGuard {
    using Counters for Counters.Counter;
    using Strings for uint256;

    uint256 public constant NONAMES_PUBLIC = 9000;
    uint256 public constant NONAMES_MAX = NONAMES_PUBLIC;
    ...
    Counters.Counter private _publicNONAMES;

    constructor() ERC721("NoNames", "NoNames") {
    ...
    {
        require(
            _publicNONAMES.current() < NONAMES_PUBLIC,
            "Purchase would exceed NONAMES_PUBLIC"
        );

        for (uint256 i = 0; i < numberOfTokens; i++) {
            uint256 tokenId = _publicNONAMES.current();

            if (_publicNONAMES.current() < NONAMES_PUBLIC) {
                _publicNONAMES.increment();
                _safeMint(to, tokenId);
            }
        }
    }
