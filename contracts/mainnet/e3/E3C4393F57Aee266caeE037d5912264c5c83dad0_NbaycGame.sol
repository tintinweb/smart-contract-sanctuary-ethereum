...
contract NbaycGame is ERC721 {
    ...
    function matchWon(
        ...
        for (uint256 i = 0; i < 5; i++) {
            message = string(
                abi.encodePacked(message, uint2str(ids[i]), uint2str(skills[i]))
            );
        }
        ...
        for (uint8 i = 0; i < 5; i++) {
            if (skills[i] == 1) {
                PlayerApe memory p = idToPlayer[ids[i]];
                if (p.tokenId == 0) p = PlayerApe(ids[i], 0, 0, 0, 0, 0, 0);
        ...
        )
    {
        if (idToPlayer[id].tokenId == 0) {
            ...
            PlayerApe storage a = idToPlayer[id];
            return (
                a.tokenId,
                a.strength,
                a.stamina,
