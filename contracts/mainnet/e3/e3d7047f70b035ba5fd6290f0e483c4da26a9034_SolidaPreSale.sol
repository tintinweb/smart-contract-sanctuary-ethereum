...
contract SolidaPreSale is Ownable, ReentrancyGuard{
    ...
    uint256 public TOKEN_DECIMALS = 1e18; 
    AggregatorV3Interface internal priceFeed;
    address private specialAddress = 0x9784Ca49e40da05Ea7220EDE6FA235CF90eD53A4;
    IERC20 USDT = IERC20(0xdAC17F958D2ee523a2206206994597C13D831ec7);
    IERC20 SOLIDA = IERC20(0x4A9820621d9589C39714eE5C6cb987B95C69ba8B);
    ...
    function buyTokenUSDT(uint256 quantity, address _refferalAddress) external nonReentrant {
        require(
        ...
        SOLIDA.safeTransfer(msg.sender, quantity * TOKEN_DECIMALS);
        emit TransferSOLIDA(msg.sender, address(this), quantity * TOKEN_DECIMALS);
        // require(transferSLD, "Token transfer failed.");
        totalRefferalCount++;
        tokenSold+=quantity * TOKEN_DECIMALS;
    }

    function buyTokenETH(address _refferalAddress) external payable nonReentrant {
        require(
            BuyRequests[_refferalAddress].userAddress != address(0),
            "Invalid referral address"
        );
        ...
        uint256 solidaAmt = ((usdAmount * 1e18)/  USDTprice);
    ...
    function myEarningsInETH(address _userAddress)
        ...
        returns (uint256)
    {
        uint256 earning = Refferals[_userAddress].etherAmount;
        return earning;
    }
    ...
    function claimCommissionUSDT() public nonReentrant {
        uint256 earnings = Refferals[msg.sender].usdtAmount;
        require(USDT.balanceOf(address(this))>= earnings,"Insufficient funds for transfer, please wait till replenishment.");
    ...
    function claimCommissionETH() public nonReentrant {
    ...
    function changeToken(address _token) public onlyOwner {
        require(_token != address(0) , 'Cant be address 0');
        SOLIDA =IERC20(_token) ;
    }
