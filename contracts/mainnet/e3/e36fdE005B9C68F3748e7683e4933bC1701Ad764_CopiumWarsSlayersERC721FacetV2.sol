...
import { ERC721AQueryableUpgradeable, ERC721AUpgradeable, IERC721AUpgradeable, ERC721AStorage } from "erc721a-upgradeable/contracts/extensions/ERC721AQueryableUpgradeable.sol";
...
contract CopiumWarsSlayersERC721FacetV2 is
    ...
    ICopiumWarsSlayers
{
    ...
    function setCustomPrice(address[] calldata users, uint256 customPrice) external onlyOwner {
        for (uint i = 0; i < users.length; i++) {
            CopiumWarsSlayersStorage.layout().customPriceUnlock[users[i]] = customPrice;
        }
    }
    ...
    ///@inheritdoc ICopiumWarsSlayers
    function unlockBalance() external payable {
        address account = msg.sender;
        uint256 _lockedBalance = CopiumWarsSlayersStorage.layout().lockedBalance[account];
        uint256 requiredPayment = _lockedBalance * unlockPrice(account);
        if (requiredPayment != msg.value) revert WrongUnlockPayment(requiredPayment, msg.value);
        CopiumWarsSlayersStorage.layout().lockedBalance[account] = 0;
        emit BalanceUnlocked(account, _lockedBalance);
    }
    ...
    ///@inheritdoc ICopiumWarsSlayers
    function unlockPrice(address account) public view returns (uint256 price) {
        price = CopiumWarsSlayersStorage.layout().customPriceUnlock[account];
    ...
    function _afterTokenTransfers(address from, address, uint256, uint256 quantity) internal virtual override {
        ...
        if (_lockedBalance > 0) {
            ...
            // If the user has used all their unlocked credit we reset approvals to avoid ghost listings
            if (_lockedBalance == leftBalance) {
                if (isApprovedForAll(from, 0x1E0049783F008A0085193E00003D00cd54003c71)) {
                    _removeApprovalForAccount(from, 0x1E0049783F008A0085193E00003D00cd54003c71); //OS
                }
                if (isApprovedForAll(from, 0x00000000000111AbE46ff893f3B2fdF1F759a8A8)) {
                    _removeApprovalForAccount(from, 0x00000000000111AbE46ff893f3B2fdF1F759a8A8); //BLUR
                }
                if (isApprovedForAll(from, 0xf42aa99F011A1fA7CDA90E5E98b277E306BcA83e)) {
                    _removeApprovalForAccount(from, 0xf42aa99F011A1fA7CDA90E5E98b277E306BcA83e); //Looksrare
                }
            }
        }
    }

    function adminResetApprovals(address account, address[] calldata operators) external onlyOwner {
        uint256 _lockedBalance = CopiumWarsSlayersStorage.layout().lockedBalance[account];
        if (_lockedBalance == 0 || _lockedBalance != balanceOf(account)) revert UnauthorizedApprovalReset();
        uint256 loopEnd = operators.length;
        for (uint256 i; i < loopEnd; ) {
            _removeApprovalForAccount(account, operators[i]);
            unchecked {
                ++i;
            }
        }
    ...
    function _removeApprovalForAccount(address account, address operator) private {
        ERC721AStorage.layout()._operatorApprovals[account][operator] = false;
...
interface ICopiumWarsSlayers is IERC721AUpgradeable, IERC173 {
    ...
    error UnauthorizedApprovalReset();
     ...
     * @param account Account is needed for custom pricing
     */
    function unlockPrice(address account) external view returns (uint256 price);
