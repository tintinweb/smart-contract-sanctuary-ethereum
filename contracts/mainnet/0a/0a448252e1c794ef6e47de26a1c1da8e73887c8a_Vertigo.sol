...
contract Vertigo is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    constructor() ERC20("Vertigo", "DIZZY") {
        ...
        feeWallet = address(_msgSender()); // set as fee wallet
    ...
    function updateFee(uint256 _fee) external onlyOwner {
        fee = _fee;
        require(fee <= 20, "Must keep fees at 20% or less");
    }
    ...
    function _transfer(
        ...
        if(takeFee){
            if ((automatedMarketMakerPairs[to] || automatedMarketMakerPairs[from]) && fee > 0){
                feesAmt = amount.mul(fee).div(100);
                tokensForFee += feesAmt;
            }
    ...
    function swapBack() private {
        ...
        uint256 maxSwapAmt = maxTransactionAmount; 
        if(contractBalance > maxSwapAmt){
          contractBalance = maxSwapAmt;
        }
