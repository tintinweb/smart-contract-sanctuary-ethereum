...
contract HeroStaking is Ownable,ERC1155Receiver {
    ...
    constructor(IERC20 _rewardToken, IERC1155 _plotAddress) {
    ...
    function deposit(uint256 _amount,uint256 collectionId) external {
      ...
      plotAddress.safeTransferFrom(msg.sender,address(this),collectionId,_amount,"");
    ...
    function claimRewards(uint256[] memory collectionIds) public
    {
     ...
     function checkclaimRewards(uint256[] memory collectionIds,address account) public view returns (uint256)
    {
        uint256 totalClaimableAmount;
        uint256[] memory amounts =  new uint[](collectionIds.length);
        for(uint256 i =0;i<collectionIds.length;i++)
    ...
    function withdraw(uint256[] memory collectionIds) public 
    {
        ...
        rewardToken.transfer(msg.sender,totalClaimableAmount);
        plotAddress.safeBatchTransferFrom(address(this),msg.sender,collectionIds,amounts,"");
        
        emit WithDraw(collectionIds,amounts,totalClaimableAmount);
    }
