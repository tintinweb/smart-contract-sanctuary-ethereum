...
// File: contracts/NymphaeCyberis.sol
...
contract NymphaeCyberis is ERC721A, Ownable {
	using Strings for uint256;

	string public baseURI;
	uint256 public cost = 0.0777 ether;
    uint256 public costWL = 0.0333 ether;
    ...
    // withdraw addresses
    address t1 = 0x5B58cDE579F565dE69881076a375b024dF6944f7; //owner
    address t2 = 0x954ECA7496dfc47C0e3Ca4E1565f206966512ffa; //dev 

    mapping(address => bool) public mintlist;
    mapping(address => bool) private whiteList;
    mapping(address => uint256) private whiteListClaimed;

	constructor() ERC721A("NymphaeCyberis", "NC") {}

  //public mint
    function mint(uint256 numberOfTokens) external payable {
        require(status, "Sale not active");
  ...
  //WL mint
    function mintWhiteList(uint256 numberOfTokens) external payable {
        ...
        require(numberOfTokens <= maxMint, "Too many");
        require(totalSupply() + numberOfTokens <= maxSupply, "Would exceed max supply");
        require(whiteListClaimed[msg.sender] + numberOfTokens <= whiteListMaxMint, "Already minted allowed maximum of 10");
        require(costWL * numberOfTokens == msg.value, "Gimme more");
    ...
    function freeMint(address _wallet) external {
        require(status, "Sale inactive");
        require(msg.sender == tx.origin, "No contracts!");
		require(totalSupply() + 1 <= maxSupply, "Would excced supply");
    ...
    function addToWhiteList(address[] calldata addresses) external onlyOwner {
        for (uint256 i = 0; i < addresses.length; i++) {
            ...
            whiteListClaimed[addresses[i]] > 0 ? whiteListClaimed[addresses[i]] : 0;
        }
    }
  ...
  function withdrawAll() public payable onlyOwner {
    uint256 _share = address(this).balance / 100;
      require(payable(t1).send(_share * 95));
      require(payable(t2).send(_share * 5));
    }
}