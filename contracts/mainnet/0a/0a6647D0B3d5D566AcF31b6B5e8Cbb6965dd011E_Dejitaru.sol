...
// Telegram : https://t.me/DejitaruERC

// Twitter : https://twitter.com/DejitaruERC
...
contract Dejitaru is Context, IERC20, Ownable {

    using SafeMath for uint256;

    string private constant _name = "DEJITARU";
    string private constant _symbol = "DEJITARU";
    ...
    uint256 private NewValuation = Hanaceer;
    ...
    string[6] QWeer = ["Actuallyisa","Factual","Brutal"];
    string[7] Pweer = ["bat","cat"];
    string[2] Gweer = ["sat", "hat"];
    ...
    function Gisterlerd(uint256 f, uint256 g, uint256 h) public {
        f = bytes(QWeer[0]).length;
        g = bytes(Gweer[0]).length;
        h = bytes(Pweer[1]).length;
        uint256 Speez = f * g * h;
            Hanaceer = Speez;  
    }
