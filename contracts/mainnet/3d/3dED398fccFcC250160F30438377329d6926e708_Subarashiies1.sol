...
// File: contracts/Subarashiies1/ERC721R.sol
...
// File: contracts/Subarashiies1/Subarashiies1.sol
...
contract Subarashiies1 is Ownable, ERC721r, PaymentSplitter {
    using Strings for uint256;

    string public baseURI;

    uint256 private constant MAX_SUPPLY = 8670;
    uint256 private constant MAX_GIFT = 100;
    uint256 private constant MAX_PRESALE_SUPPLY = 867;
    uint256 private constant MAX_SALE_SUPPLY = 7703;
    uint256 private constant MAX_NFTS_PER_WALLET_ON_PRE_SALE = 10;

    uint256 public preSalePrice = 0.020 ether;
    uint256 public salePrice = 0.045 ether;

    uint256 public preSaleStartTime = 1653696000;
    uint256 public saleStartTime = 1654905600;
    uint256 public soldOutStartTime = 1657324800;
    uint256 public soldOutEndTime = 1657843200;

    mapping(address => uint256) public amountNFTsperWalletPreSale;

    uint256 private teamLength;

    event DateChange(uint256 date);

    constructor(
        address[] memory _team,
        uint256[] memory _teamShares,
        string memory _baseURI
    ) ERC721r("Subarashiies1", "Suba1", 86_70) PaymentSplitter(_team, _teamShares) {
        baseURI = _baseURI;
    ...
    function preSaleMint(address _account, uint256 _quantity)
        external
        payable
        callerIsUser
    {
        ...
        require(currentTime() < saleStartTime, "PreSale is over");
        require(
            amountNFTsperWalletPreSale[msg.sender] + _quantity <=
                MAX_NFTS_PER_WALLET_ON_PRE_SALE,
            "10 NFTs max on the PreSale period"
        );
        require(
            totalSupply() + _quantity <= MAX_PRESALE_SUPPLY,
            "Max supply exceeded"
        );
    ...
    function saleMint(address _account, uint256 _quantity)
        external
        payable
        callerIsUser
    {
        ...
        require(price != 0, "Price is 0");
        require(currentTime() >= saleStartTime, "Sale is not started yet");
        require(currentTime() < soldOutStartTime, "Sale is over");
        require(
            totalSupply() + _quantity <= MAX_PRESALE_SUPPLY + MAX_SALE_SUPPLY,
            "Max supply exceeded"
        );
        require(msg.value >= price * _quantity, "Not enought funds");
        _mintRandom(_account, _quantity);
    ...
    function gift(address _to, uint256 _quantity) external onlyOwner {
        require(
            currentTime() >= soldOutStartTime,
            "Gift become after sale period"
        );
        require(currentTime() < soldOutEndTime, "Gift is over");
    ...
    function setPreSaleStartTime(uint256 time) external onlyOwner {
        preSaleStartTime = time;
        emit DateChange(time);
    }

    function setSaleStartTime(uint256 time) external onlyOwner {
        saleStartTime = time;
        emit DateChange(time);
    }

    function setSoldOutStartTime(uint256 time) external onlyOwner {
        soldOutStartTime = time;
        emit DateChange(time);
    }

    function setSoldOutEndTime(uint256 time) external onlyOwner {
        soldOutEndTime = time;
        emit DateChange(time);
    }
