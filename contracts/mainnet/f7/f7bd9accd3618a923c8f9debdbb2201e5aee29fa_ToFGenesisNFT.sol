...
contract ToFGenesisNFT is ERC721, ERC721Enumerable, Ownable, ReentrancyGuard {
    ...
    constructor() ERC721("Token of Fire", "Rhllor Genesis NFT") {
        _tokenIds.increment();
    }

    function contractURI() internal view returns (string memory) {
		return _contract_entry;
	}
    ...
    //Standard Mint
    function mint() public payable nonReentrant canMint {
        uint256 balance = IPRE(token).balanceOf(msg.sender);
        require(msg.value >= PRICE);
        require(isTrading);
        if(balance > MIN_REQUIRED_TOF * 10**18) { 
    ...
    // Set Min ToF requirements to mint
    ...
    //Update Base URI
     function setBaseURI(string memory _newBaseURI) public onlyOwner {
        _api_entry = _newBaseURI;
    }

    //Update Contact Entry URI
    function setContactEntryURI(string memory _newContactEntryURI) external onlyOwner { 
        _contract_entry = _newContactEntryURI;
    }
