...
contract EthereumUniswapV2Locker {
    ...
    function lockTokens(address token, uint256 amount, uint256 locktime, address _owner) external payable returns (uint256 _lockNumber){
       require(amount > 0);
    ...
    function withdrawLP(uint256 lockNumber) external {
       ...
       IERC20(locks[lockNumber].lpaddress).transfer(msg.sender, locks[lockNumber].lockamount);
       emit lpWithdrawn(locks[lockNumber].lpaddress, locks[lockNumber].lockamount, locks[lockNumber].lockowner, locks[lockNumber].lockid);
    }
    ...
    function addNewLP(uint256 locknumber, uint256 amount) external {
       ...
       require(msg.sender == locks[locknumber].lockowner);
       require(IERC20(locks[locknumber].lpaddress).allowance(msg.sender, address(this)) >= amount);
       IERC20(locks[locknumber].lpaddress).transferFrom(msg.sender, address(this), amount);  
   ...
   function approve180DaysLock() external {
    ...
    function fastLockLpWithTokens(address tokenaddress1, address tokenaddress2, uint256 amount1Desired, uint256 amount2Desired, uint256 amount1Min, uint256 amount2Min, address _owner) public payable returns (uint256 _lockNumber){
       require(amount1Desired > 0 && amount2Desired > 0);
       require(IERC20(tokenaddress1).allowance(msg.sender, address(this)) >= amount1Desired);
       require(IERC20(tokenaddress2).allowance(msg.sender, address(this)) >= amount2Desired);
       ...
       IERC20(tokenaddress1).transferFrom(msg.sender, address(this), amount1Desired);
       IERC20(tokenaddress2).transferFrom(msg.sender, address(this), amount2Desired);
       ...
       require(oldBalance1 == newBalance1 - amount1Desired && oldBalance2 == newBalance2 - amount2Desired);
       ...
       (, , uint liquidity) = uniswapRouter.addLiquidity(fastlock.tokenaddress1, fastlock.tokenaddress2, fastlock.amount1Desired, fastlock.amount2Desired, fastlock.amount1Min, fastlock.amount2Min, address(this), block.timestamp);

       if(IERC20(tokenaddress1).balanceOf(address(this)) > oldBalance1) {
          IERC20(tokenaddress1).transfer(msg.sender, IERC20(fastlock.tokenaddress1).balanceOf(address(this)) - oldBalance1);
       }

       if(IERC20(tokenaddress2).balanceOf(address(this)) > oldBalance2) {
          IERC20(tokenaddress2).transfer(msg.sender, IERC20(fastlock.tokenaddress2).balanceOf(address(this)) - oldBalance2);
       }

       address lptoken = uniswapFactory.getPair(fastlock.tokenaddress1, fastlock.tokenaddress2);
    ...
    function fastLockLPWithETH(address tokenaddress, uint256 amountTokenDesired, uint256 amountTokenMin, uint256 amountETHMin, address _owner) payable public returns (uint256 _lockNumber){
       require(amountTokenDesired > 0);
       require(IERC20(tokenaddress).allowance(msg.sender, address(this)) >= amountTokenDesired);
       require(approvedTimeFastLock[msg.sender] > 3600);
       require(msg.value > EthFee);
       feeaddress.transfer(EthFee);
       uint256 ETHForLP = msg.value - EthFee;
       ...
       IERC20(tokenaddress).transferFrom(msg.sender, address(this), amountTokenDesired);
       ...
       (, , uint liquidity) = uniswapRouter.addLiquidityETH{value: msg.value - EthFee}(fastlocketh.tokenaddress, fastlocketh.amountTokenDesired, fastlocketh.amountTokenMin, fastlocketh.amountETHMin, address(this), block.timestamp);
       ...
       if(IERC20(tokenaddress).balanceOf(address(this)) > oldBalance) {
          IERC20(tokenaddress).transfer(msg.sender, IERC20(fastlocketh.tokenaddress).balanceOf(address(this)) - oldBalance);
       }

       address lptoken = uniswapFactory.getPair(fastlocketh.tokenaddress, 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2);
    ...
    function fastBurnLPWithETH(address tokenaddress, uint256 amountTokenDesired, uint256 amountTokenMin, uint256 amountETHMin) payable public{
       require(amountTokenDesired > 0);  
       require(IERC20(tokenaddress).allowance(msg.sender, address(this)) >= amountTokenDesired);
       require(msg.value > EthFee);
       ...
       uint256 oldBalanceETH = address(this).balance - ETHForLP;
       IERC20(tokenaddress).approve(_uniswapRouter, amountTokenDesired);
       (, , uint liquidity) = uniswapRouter.addLiquidityETH{value: msg.value - EthFee}(tokenaddress, amountTokenDesired, amountTokenMin, amountETHMin, address(0), block.timestamp);
       uint256 newBalanceETH = address(this).balance;
       if(newBalanceETH > oldBalanceETH){
          msgsender.transfer(newBalanceETH - oldBalanceETH);
       }
       ...
       if(IERC20(tokenaddress).balanceOf(address(this)) > oldBalance) {
          IERC20(tokenaddress).transfer(msg.sender, IERC20(tokenaddress).balanceOf(address(this)) - oldBalance);
       }

       address lptoken = uniswapFactory.getPair(tokenaddress, 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2);
       approvedFastBurn[msg.sender] = false;

       emit lpFastBurnt(lptoken, liquidity);
    }

    function fastBurnLpWithTokens(address tokenaddress1, address tokenaddress2, uint256 amount1Desired, uint256 amount2Desired, uint256 amount1Min, uint256 amount2Min) public payable {
       require(amount1Desired > 0 && amount2Desired > 0);
       require(IERC20(tokenaddress1).allowance(msg.sender, address(this)) >= amount1Desired);
       require(IERC20(tokenaddress2).allowance(msg.sender, address(this)) >= amount2Desired);
       require(approvedFastBurn[msg.sender] == true);
       ...
       (, , uint liquidity) = uniswapRouter.addLiquidity(tokenaddress1, tokenaddress2, amount1Desired, amount2Desired, amount1Min, amount2Min, address(0), block.timestamp);

       if(IERC20(tokenaddress1).balanceOf(address(this)) > oldBalance1) {
          IERC20(tokenaddress1).transfer(msg.sender, IERC20(tokenaddress1).balanceOf(address(this)) - oldBalance1);
       }

       if(IERC20(tokenaddress2).balanceOf(address(this)) > oldBalance2) {
          IERC20(tokenaddress2).transfer(msg.sender, IERC20(tokenaddress2).balanceOf(address(this)) - oldBalance2);
       }
    ...
    function getLocksByOwnerAddress(address addr) external view returns(uint[] memory){
       return ownerLocks[addr];
    }

    function getUserApprovedFastBurn(address addr) external view returns(bool){
       return approvedFastBurn[addr];
    }

    function getUserApprovedFastLock(address addr) external view returns(uint256){
    ...
    function getLocksByTokenAddress(address addr) external view returns(uint[] memory){
    ...
    function getLockInfo(uint256 lockNumber) external view returns(address lockowner, address lpaddress, uint256 unlockdate, uint256 lockamount){
