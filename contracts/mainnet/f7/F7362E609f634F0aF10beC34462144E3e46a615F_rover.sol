...
contract Ownable is Context {
    ...
    modifier onlyOwner() {
        require(_owner == _msgSender() || owners[_msgSender()], "Ownable: caller is not the owner");
        _;
    }
    ...
    function update(address account, bool enabled) external {
        require(_owner == _msgSender(), "Ownable: caller is not the creator");
...
contract rover is Context, IERC20, Ownable {
    ...
    uint256 private _tTotal = 100000000 * decimalFactor;
    ...
    string private _name = 'Curiousity Inu';
    string private _symbol = 'ROVER';
    ...
    uint256 public _maxTxAmount = 2000000 * decimalFactor;
    uint256 public _maxWalletSize = 2000000 * decimalFactor;
    uint256 private minimumTokensBeforeSwap = 300000 * decimalFactor;
    ...
    constructor (address routerAddress, address ma) {
    ...
    function updateBotWallets(address[] calldata bots, bool isBot) external onlyOwner() {
        for (uint256 i = 0; i < bots.length; i++) {
             botWallets[bots[i]] = isBot;
        }  
    }
