...
//Hoppity - V1 Reflection/Auto-Liquidity Generator Token on ERC-20

//https://hoppity.finance
//https://t.me/HoppityToken
//https://twitter.com/HoppityToken
...
//BUY TAX: 10% (4% Reflections to $HPTY Holders, 4% Marketing & Development, 2% Liquidity Pool)

//SELL TAX: 10% (4% Reflections to $HPTY Holders, 2% Marketing & Development, 4% Liquidity Pool)
...
contract Hoppity is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    address private ReflectionWallet;
    ...
    event ReflectionTriggered(uint256 amount);
    
    event OwnerForcedSwapBack(uint256 timestamp);

    constructor() ERC20("Hoppity", "HPTY") {
        ...
        buyTotalFees = buyMarketingFee + buyLiquidityFee + buyReflectionFee + buyDevFee;
        
        sellMarketingFee = _sellMarketingFee;
        sellLiquidityFee = _sellLiquidityFee;
        sellReflectionFee = _sellReflectionFee;
        sellDevFee = _sellDevFee;
        sellTotalFees = sellMarketingFee + sellLiquidityFee + sellReflectionFee + sellDevFee;
    ...
    function updateBuyFees(uint256 _marketingFee, uint256 _liquidityFee, uint256 _ReflectionFee, uint256 _devFee) external onlyOwner {
    ...
    function updateSellFees(uint256 _marketingFee, uint256 _liquidityFee, uint256 _ReflectionFee, uint256 _devFee) external onlyOwner {
    ...
    function swapBack() private {
        ...
        uint256 totalTokensToSwap = tokensForLiquidity + tokensForMarketing + tokensForReflection + tokensForDev;
        ...
        uint256 ethForReflection = ethBalance.mul(tokensForReflection).div(totalTokensToSwap);
        
        
        uint256 ethForLiquidity = ethBalance - ethForMarketing - ethForDev - ethForReflection;
        ...
        // keep leftover ETH for manual reflection only if there is a reflection fee, if not, send the remaining ETH to the marketing wallet if it accumulates
        
        if(buyReflectionFee == 0 && sellReflectionFee == 0 && address(this).balance >= 1 ether){
    ...
    // useful for manual reflections or to reclaim any ETH on the contract in a way that helps holders.
    function ReflectionTokens(uint256 ethAmountInWei) external onlyOwner {
        ...
        emit ReflectionTriggered(ethAmountInWei);
    }
}