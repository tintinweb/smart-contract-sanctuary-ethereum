...
contract SPARCEDependency {
  ...
  modifier onlyMarkets() {
    require(whitelistedMarkets[msg.sender]);
    _;
  }
  ...
  function setMarkets(address _market, bool _whitelisted) external onlyWhitelistSetter {
    require(whitelistedMarkets[_market] != _whitelisted);
...
contract SPARCECore is ERC721, SPARCEDependency {
  ...
  constructor() ERC721("SPARC-E", "SPARC-E", "https://ipfs.io/ipfs/") {
    owner = msg.sender;
    sparce.push(SPARCE(0, block.timestamp, msg.sender, true));
  ...
  function UpdateNewBornSparce( uint _sparceId) external onlyMarkets {
      require(sparce[_sparceId].isNewlyBorn == true);
      sparce[_sparceId].isNewlyBorn = false;
  }
    ...
    returns (uint256 /* _genes */, uint256 /* _bornAt */, address /* _operator */, bool /* _newlyBorn */)
  {
    ...
    return (_sparce.genes, _sparce.bornAt, _sparce.operator, _sparce.isNewlyBorn);
  }
  ...
  function spawnMultipleSparce(
    uint256[] memory _genes
  )
    external
    onlySpawner
  {
    require(_genes.length <= 30, "exceed array length");
    for(uint i=0;i<_genes.length;i++) {
        require(
            address(spawningManager) == address(0) ||
            spawningManager.isSpawningAllowed(_genes[i], owner),
            "spawning is not allowed"
        );
        _spawnSparce(_genes[i], owner);    
    }
  }
  ...
  function _spawnSparce(uint256 _genes, address _owner) private returns (uint256 _sparceId) {
    sparce.push(SPARCE(_genes, block.timestamp, msg.sender, true));
