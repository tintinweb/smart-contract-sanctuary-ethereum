...
// File contracts/SergeysVision.sol

// SPDX-License-Identifier: GPLv3

pragma solidity 0.8.17;

/// @title Sergey's Vision
contract SergeysVision is Governable, IERC721Receiver {
    using SafeERC20 for IERC20;

    /// @notice Input token (LINK) for Sergey's Vision
    address public constant LINK = 0x514910771AF9Ca656af840dff83E8264EcF986CA;
    uint256 public constant LINK_REQUIRED_TO_MINT_SV = 1e18; // 1 LINK (18 decimals)
    uint256 private constant CAPSULE_MINT_FEE = 0.001 ether;

    /// @notice Max amount of Sergey's Visions allowed to be in circulation
    uint16 public constant MAX_SV_SUPPLY = 777;

    /// @notice Number Sergey's Visions in circulation
    uint16 public currentSVSupply = 0;

    /// @notice Flag indicating whether the Sergey's Vision mint is enabled.
    bool public isMintEnabled = false;

    // Events
    event SergeysVisionMinted(address indexed user, uint256 indexed id);
    event SergeysVisionBurnt(address indexed user, uint256 indexed id);
    event MintToggled(bool mintStatus);

    // Capsule Collection Interface
    ...
    /// @notice Sergey's Vision collection
    ICapsule public immutable capsuleCollection;

    constructor(ICapsule capsuleCollection_) {
        require(address(capsuleCollection_) != address(0), "null-collection-address");
        capsuleCollection = capsuleCollection_;
    }

    /// @notice Mint a SV to owner address
    function governorMint() external payable onlyGovernor returns (uint256 _id) {
        return _mintSV();
    }

    /// @notice Mint a SV to caller address
    function mint() external payable {
        // Check if mint is possible at all
        require(isMintEnabled, "SV Minting unavailable.");
        _mintSV();
    }

    function _mintSV() private returns (uint256 _capsuleId) {
        require(msg.value == CAPSULE_MINT_FEE, "fee-not-paid");
        require(currentSVSupply < MAX_SV_SUPPLY, "max-circulation-reached");
        // Transfer LINK amount & increase mint spending allowance
        address _caller = msg.sender;

        IERC20(LINK).safeTransferFrom(_caller, address(CAPSULE_MINTER), LINK_REQUIRED_TO_MINT_SV);

        currentSVSupply++;
        _capsuleId = capsuleCollection.counter();

        // SV collection will be using baseURL from an external contract
        // and do not need URI for individual NFTs if users are allowed to upload.
        // Hence passing empty token URI to mint function below.
        CAPSULE_MINTER.mintSingleERC20Capsule{value: CAPSULE_MINT_FEE}(
            address(capsuleCollection),
            LINK,
            LINK_REQUIRED_TO_MINT_SV,
            "",
            _caller
        );

        emit SergeysVisionMinted(_caller, _capsuleId);
    }

    /**
     * @notice Burn a SV and get 1 LINK back
     * @param id_ SV id to burn
    ...
    function burn(uint256 id_) external {
        ...
        currentSVSupply--; // Decrement SVs in circulation
        emit SergeysVisionBurnt(_caller, id_);
    }
    ...
    /// @notice onlyGovernor:: Toggle minting status of the Sergey's Vision
    ...
    /**
     * @notice onlyGovernor:: Transfer ownership of the Sergey's Vision Capsule collection
    ...
    /**
     * @notice onlyGovernor:: Transfer metamaster of the Sergey's Vision Capsule collection
    ...
    /**
     * @notice onlyGovernor:: Set a contract to be in charge of tokenURI calls.
     * @param metadataProvider_ The Address of the contract which will have the Link price feed, dynamic tokenURI
     */
    function updateMetadataProvider(address metadataProvider_) external onlyGovernor {
        capsuleCollection.setMetadataProvider(metadataProvider_);
    }

    /**
     * @notice onlyGovernor:: Update royalty receiver and rate in Sergey's Vision collection
    ...
    /**
     * @notice Update collection burner. Add self address as collection burner for SV
     */
