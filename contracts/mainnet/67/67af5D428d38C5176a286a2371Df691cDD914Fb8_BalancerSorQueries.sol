...
contract BalancerSorQueries {
    ...
    function getPoolTokenBalancesWithUpdatesAfterBlock(
    ...
    function getWrappedTokenRateForLinearPools(
    ...
    function getSwapFeePercentageForPools(
        address[] memory poolAddresses,
        SwapFeeType[] memory swapFeeTypes
    ) external view returns (uint256[] memory) {
    ...
    function getTotalSupplyForPools(
        address[] memory poolAddresses,
        TotalSupplyType[] memory totalSupplyTypes
    ) external view returns (uint256[] memory) {
        uint256[] memory totalSupplies = new uint256[](poolAddresses.length);

        for (uint256 i = 0; i < poolAddresses.length; i++) {
            if (totalSupplyTypes[i] == TotalSupplyType.VIRTUAL_SUPPLY) {
                totalSupplies[i] = IPoolWithVirtualSupply(poolAddresses[i]).getVirtualSupply();
            } else if (totalSupplyTypes[i] == TotalSupplyType.ACTUAL_SUPPLY) {
                totalSupplies[i] = IPoolWithActualSupply(poolAddresses[i]).getActualSupply();
            } else {
