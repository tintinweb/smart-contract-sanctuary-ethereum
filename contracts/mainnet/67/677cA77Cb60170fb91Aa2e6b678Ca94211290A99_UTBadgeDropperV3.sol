...
contract UTBadgeDropperV3 {
    ...
    uint8                           public constant     traitType = 0;     // Badge (1bit trait)
    ...
    function requestRandomDrop(uint32 _communityId, uint16 _traitId, uint32 _registryNum, uint16 _rangeStart, uint16 _rangeEnd, uint16 _traitsToDrop) external {
        ...
        require(CommunityRegistry(crAddress).community_admin() == msg.sender || CommunityRegistry(crAddress).hasRole(TRAIT_DROPPER, msg.sender), "UTBadgeDropper: Unauthorised");
        ...
        require(IECRegistry(ECrAddress).getTrait(_traitId).traitType == 0, "UTBadgeDropper: badge type traits only");
    ...
    function finalizeRandomDrop(uint32 _dropIndex) public {         // onlyAllowed(_communityId, TRAIT_DROPPER) {
        ...
        require(IECRegistry(ECrAddress).getTrait(drop.traitId).traitType == 0, "UTBadgeDropper: badge type traits only");
