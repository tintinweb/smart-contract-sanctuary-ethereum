...
contract FastFoodTown is ERC721A, ERC721AQueryable, ERC721APausable, ERC721ABurnable, Ownable, ReentrancyGuard {
    uint public PRICE;
    uint public MAX_SUPPLY;
    uint public MAX_MINT_AMOUNT_PER_TX;
    uint16 public MAX_FREE_MINTS_PER_WALLET;
    uint32 public MAX_FREE_MINTS_GLOBAL;
    ...
    constructor(uint price,
        ...
        uint32 maxFreeMintsGlobal,
        string memory baseUri) ERC721A("FastFoodTown", "FFT") {
        PRICE = price;
        MAX_SUPPLY = maxSupply;
        MAX_MINT_AMOUNT_PER_TX = maxMintPerTx;
        MAX_FREE_MINTS_PER_WALLET = maxFreeMintsPerWallet;
        MAX_FREE_MINTS_GLOBAL = maxFreeMintsGlobal;
        BASE_URI = baseUri;
    }
    ...
    function setMaxFreeMintsGlobal(uint32 maxFreeMintsGlobal) external onlyOwner {
        MAX_FREE_MINTS_GLOBAL = maxFreeMintsGlobal;
    }
    ...
    function getJob(uint32 _mintAmount) public payable mintCompliance(_mintAmount) {
        ...
        if (totalFreeMinted < MAX_FREE_MINTS_GLOBAL) {
    ...
    function giveJob(address _to, uint _mintAmount) public mintCompliance(_mintAmount) onlyOwner {
