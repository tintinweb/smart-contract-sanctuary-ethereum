...
contract PurseHeads is ERC2981, ERC721AQueryable, Ownable {
    ...
    constructor(uint256 price, uint32 maxSupply, uint32 txLimit, uint32 walletLimit, uint32 teamReserved) ERC721A("PurseHeads", "PURSE") {
        _price = price;
    ...
    function mint(uint32 amount) external payable {
        ...
        require(amount + _totalMinted() <= _maxSupply - _teamReserved, "Sold out");
        require(amount <= _txLimit, "Exceeds transaction limit");
        ...
        require(minted + amount <= _walletLimit, "Exceeds wallet limit");
    ...
    function devMint(address to, uint32 amount) external onlyOwner {
        _teamMinted += amount;
        require (_teamMinted <= _teamReserved, "Exceeds max supply");
        _safeMint(to, amount);
    }
