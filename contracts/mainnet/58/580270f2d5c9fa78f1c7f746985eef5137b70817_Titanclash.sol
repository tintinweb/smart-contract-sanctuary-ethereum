...
contract Titanclash is ERC721A, Ownable, ReentrancyGuard {
  using Strings for uint256;
  ...
  uint256 public maxPresaleWalletLimit = 50;
  uint256 public maxPublicSaleWalletLimit = 50;
  ...
  constructor() ERC721A("Titanclash", "Ttnclash") {
  }
  ...
  function mintForSelf(uint256 _mintAmount) public selfMintCompliance(_mintAmount) onlyOwner {
    ...
    function Minted(address[] memory __receivers, uint256[] memory _mintAmount) public onlyOwner {
        for(uint256 i = 0; i < __receivers.length; i++){
            require(_mintAmount[i] > 0, "Invalid mint amount!");
            require(totalSupply() + _mintAmount[i] <= maxSupply, "Max supply exceeded!");

            _mintLoop(__receivers[i], _mintAmount[i]);
        }
    }

    function airdrop(address[] memory __receivers) public onlyOwner {
        require(totalSupply() + __receivers.length <= maxSupply, "Max supply exceeded!");

        for(uint256 i = 0; i < __receivers.length; i++){
            _mintLoop(__receivers[i], 1);
        }
    }

    function PresaleMint(bytes32[] calldata _merkleProof, uint256 _mintAmount) public payable mintCompliance(_mintAmount) {
        require(presaleActive, "Sale is not Active");
        require(msg.value >= cost() * _mintAmount, "Insufficient funds!");
        require(addressMintedBalance[msg.sender] + _mintAmount <= maxWalletLimit(), "Max mint amount per wallet reached");

        bytes32 leaf = keccak256(abi.encodePacked(msg.sender));
        require(verify(merkleRoot, leaf,_merkleProof), "Only WHITELIST can mint.");
