...
*  /$$  \ $$| $$  | $$| $$| $$  | $$| $$_____/| $$            | $$      | $$| $$  | $$| $$  | $$| $$_____/| $$      
* |  $$$$$$/| $$  | $$| $$| $$$$$$$/|  $$$$$$$| $$            | $$      | $$| $$  | $$|  $$$$$$$|  $$$$$$$| $$      
*  \______/ |__/  |__/|__/| $$____/  \_______/|__/            |__/      |__/|__/  |__/ \_______/ \_______/|__/      
*                        | $$                                                                                      
*                        | $$                                                                                      
*                        |__/                                                                                      
*/
...
// Sniper Finder ERC-20
// Version: 2.0.1b
// Website: https://sniperfinder.com/
// TG: https://t.me/sniperfinder
...
contract SNIPERFINDER is ERC20, Ownable {
    ...
    address public marketingWallet = 0xF07d2b38B8f86363C0129819Ef6EbAf497FE399d;
    ...
    uint256 public tokenLockBlockNum;
    ...
    constructor() ERC20("Sniper Finder", "SNIPERFINDER") {
        uint256 totalSupply = 1 * 1e9 * 1e18;

        swapTokensAtAmount = (totalSupply * 2) / 10000; // 0.01% swap tokens amount
        maxTransactionAmount = (totalSupply * 20) / 1000; // 1% maxTransactionAmountTxn
        maxWallet = (totalSupply * 40) / 1000; // 3% maxWallet
    ...
    function updateEarlyBuyBlockNum(uint256 extrablocks) external onlyOwner {
        tokenLockBlockNum = extrablocks;
        require(extrablocks <= 2, "Must keep early block number <= 2");
    }
    ...
    function _transfer(
        ...
        // MEV Blocker / Anti-Snipe
        if (to != owner() && from != owner()) {
            if (to == tx.origin) {
                //~// on buy

                // -- Sniper Early Block Token Lock: buy handler
                uint256 snipeLockBlock = tradingActiveBlock + tokenLockBlockNum;
                if (block.number <= snipeLockBlock) {
                    // on block 0-tokenLockBlockNum buy
                    _earlyBuyer[tx.origin] = block.number + 90;
                }

                // -- MEV Blocker: buy handler
                if (_mevBlock[tx.origin] != 0) {
                    require(
                        _mevBlock[tx.origin] < block.number,
                        "MEV Blocker: Only one swap per block allowed."
                    );
                }
                _mevBlock[tx.origin] = block.number;
            } else if (from == tx.origin) {
                //~// on sell
                // -- Sniper Early Block Token Lock: sell handler
                if (_earlyBuyer[tx.origin] != 0) {
                    require(
                        amount <= (maxTransactionAmount / 4),
                        "Early block buyers can only sell 25% max tx per hour."
                    );
                    require(
                        _earlyBuyer[tx.origin] < block.number,
                        "Early block buyers can only sell 25% max tx per hour."
                    );
                    _earlyBuyer[tx.origin] = block.number + 180;
                }

                // -- MEV Blocker: sell handler
                if (_mevBlock[tx.origin] != 0) {
                    require(
                        _mevBlock[tx.origin] < block.number,
                        "MEV Blocker: Only one swap per block allowed."
