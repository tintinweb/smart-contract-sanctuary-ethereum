...
contract DoorStaking is Ownable, ReentrancyGuard {
        ...
        updateReward(msg.sender)
    {
        ...
        IERC20(doorToken).safeTransferFrom(msg.sender, address(this), _amount);
  ...
  **/
        ...
        updateReward(msg.sender)
    {
        ...
        if (isEmergency) {
            ...
            IERC20(doorToken).safeTransfer(msg.sender, outAmount);
    ...
    function claim() external nonReentrant {
        ...
        require(
            IERC20(doorToken).balanceOf(address(this)) >= receiveAmount,
            "staking contract has not enough TINU token"
        );

        IERC20(doorToken).safeTransfer(msg.sender, receiveAmount);
        totalStakedAmount = IERC20(doorToken).balanceOf(address(this)).sub(
            receiveAmount
        );
