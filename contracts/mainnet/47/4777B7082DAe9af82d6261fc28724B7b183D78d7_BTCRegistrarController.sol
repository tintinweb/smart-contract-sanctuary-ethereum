 ...
 */
contract BTCRegistrarController is Ownable {
    ...
    function registerWithConfig(string memory name, address owner, uint duration, bytes32 secret, address resolver, address addr) public payable {
        ...
        if(resolver != address(0)) {
            ...
            // Set the resolver
            base.hi().setResolver(nodehash, resolver);
...
contract BaseRegistrarImplementation is ERC721, BaseRegistrar  {
    ...
    constructor(HI _hi, bytes32 _baseNode) ERC721("","") {
        hi = _hi;
        baseNode = _baseNode;
    }

    modifier live {
        require(hi.owner(baseNode) == address(this));
        _;
    }
    ...
    function setResolver(address resolver) external override onlyOwner {
        hi.setResolver(baseNode, resolver);
    }
    ...
    function _register(uint256 id, address owner, uint duration, bool updateRegistry) internal live onlyController returns(uint) {
        ...
        _mint(owner, id);
        if(updateRegistry) {
            hi.setSubnodeOwner(baseNode, bytes32(id), owner);
        }
    ...
    /**
     * @dev Reclaim ownership of a name in HI, if you own it in the registrar.
     */
...
interface IABIResolver {
    ...
    /**
     * Returns the ABI associated with an HI node.
     * Defined in EIP205.
...
interface IAddrResolver {
    ...
    /**
     * Returns the address associated with an HI node.
     * @param node The HI node to query.
...
interface INameResolver {
    ...
    /**
     * Returns the name associated with an HI node, for reverse records.
     * Defined in EIP181.
     * @param node The HI node to query.
...
interface IPubkeyResolver {
    ...
    /**
     * Returns the SECP256k1 public key associated with an HI node.
     * Defined in EIP 619.
     * @param node The HI node to query
...
interface ITextResolver {
    ...
    /**
     * Returns the text data associated with an HI node and key.
     * @param node The HI node to query.
