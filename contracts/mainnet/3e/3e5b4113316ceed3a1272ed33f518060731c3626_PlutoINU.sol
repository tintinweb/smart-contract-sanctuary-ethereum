...
// ----------------------------------------------------------------------------
// 'Pluto INU AI' token AND staking contract
...
// token transfers
// ----------------------------------------------------------------------------
contract PlutoINU is IERC20, Owned {
    using SafeMath for uint256;
   
    string public symbol = "PLUTO";
    string public  name = "Pluto INU AI";
    uint256 public decimals = 18;
    address airdropContract;
    uint256 _totalSupply = 1000000000000 * 10 ** (18); // 1,000,000
    ...
    constructor(address owner) public {
        ...
        balances[address(owner)] =   1000000000000  * 10 ** (18); // 1,000,000
        emit Transfer(address(0), address(owner), 1000000000000  * 10 ** (18));
    ...
    function deductionsToApply(uint256 tokens) private view returns(uint256){
        ...
        if(_totalSupply > minSupply && msg.sender != airdropContract){
            ...
            deduction = onePercent(tokens).mul(2); // 2% transaction cost
    ...
    function STAKE(uint256 _tokens) external returns(bool){
        ...
        // gets PlutoINU tokens from user to contract address
