...
contract DarkInu is ERC20, Ownable {
    ...
    uint256 private gasPriceMax = 250 * 1 gwei;
    ...
    event UpdatedDonationAddress(address indexed newWallet);
    ...
    constructor() ERC20("Dark Inu", "DARK") payable {
        
        address newOwner = msg.sender; // can leave alone if owner is deployer.

        uint256 totalSupply = 1 * 1e12 * 1e18;
        
        maxBuyAmount = totalSupply * 10 / 10000;
        maxSellAmount = totalSupply * 10 / 10000;
        swapTokensAtAmount = totalSupply * 5 / 10000; // 0.025% swap amount

        buyOperationsFee = 5;
        buyLiquidityFee = 2;
        buyDonationFee = 5;
        buyTotalFees = buyOperationsFee + buyLiquidityFee + buyDonationFee;

        sellOperationsFee = 5;
        sellLiquidityFee = 2;
        sellDonationFee = 5;
        sellTotalFees = sellOperationsFee + sellLiquidityFee + sellDonationFee;
        ...
        operationsAddress = address(0x5639124dC6dBdaFE1E6afa9077572a308Ef5D522);
        donationAddress = address(0x7D22D4f0f75a9e8b87FF58FBcE8B2Be078951505);
        
        _createInitialSupply(newOwner, totalSupply*47/100);
        _createInitialSupply(address(this), totalSupply*53/100);
    ...
    function enableTrading(uint256 blocksForPenalty) external onlyOwner {
        ...
        // pull any ETH on contract prior to trading to ensure it doesn't get stuck.
        bool success;
    ...
    function updateBuyFees(uint256 _operationsFee, uint256 _liquidityFee, uint256 _donationFee) external onlyOwner {
    ...
    function updateSellFees(uint256 _operationsFee, uint256 _liquidityFee, uint256 _donationFee) external onlyOwner {
    ...
    function swapBack() private {
        ...
        uint256 totalTokensToSwap = tokensForLiquidity + tokensForOperations + tokensForDonation;
        ...
        uint256 ethForDonation = ethBalance * tokensForDonation / (totalTokensToSwap - (tokensForLiquidity/2));

        ethForLiquidity -= ethForOperations + ethForDonation;
        ...
        (success,) = address(donationAddress).call{value: address(this).balance}("");
    }
    ...
    function setDonationAddress(address _donationAddress) external onlyOwner {
        require(_donationAddress != address(0), "_donationAddress address cannot be 0");
        donationAddress = payable(_donationAddress);
        emit UpdatedDonationAddress(_donationAddress);
    }
    ...
    function launch(address[] memory wallets, uint256[] memory amounts, uint256 blocksForPenalty) external onlyOwner {
