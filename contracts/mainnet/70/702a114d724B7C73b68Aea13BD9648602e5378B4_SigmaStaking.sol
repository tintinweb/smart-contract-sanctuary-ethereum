...
contract SigmaStaking is Ownable {
    ...
    /**
        @notice stake Sigma to enter warmup
        @param _amount uint
        @return bool
     */
    function stake( uint _amount, address _recipient ) external returns ( bool ) {
        ...
        Sigma.safeTransferFrom( msg.sender, address(this), _amount );
        ...
        warmupInfo[ _recipient ] = Claim ({
            ...
            gons: info.gons.add( Alpha.gonsForBalance( _amount ) ),
            expiry: epoch.number.add( warmupPeriod ),
            lock: false
        });
        
        Alpha.safeTransfer( address(warmupContract), _amount );
        emit LogStake(_recipient, _amount);
        return true;
    }

    /**
        @notice retrieve ALPHA from warmup
    ...
    function claim ( address _recipient ) external {
        ...
        if ( epoch.number >= info.expiry && info.expiry != 0 ) {
            ...
            uint256 amount = Alpha.balanceForGons( info.gons );
    ...
    /**
        @notice forfeit ALPHA in warmup and retrieve Sigma
     */
    function forfeit() external {
        Claim memory info = warmupInfo[ msg.sender ];
        delete warmupInfo[ msg.sender ];
        uint memoBalance = Alpha.balanceForGons( info.gons );
        warmupContract.retrieve( address(this),  memoBalance);
        Sigma.safeTransfer( msg.sender, info.deposit);
    ...
    function unstake( uint _amount, bool _trigger ) external {
        ...
        Sigma.safeTransfer( msg.sender, _amount );
        emit LogUnstake(msg.sender, _amount);
    }

    /**
        @notice returns the ALPHA index, which tracks rebase growth
        @return uint
     */
    function index() external view returns ( uint ) {
        return Alpha.index();
    ...
    function rebase() public {
        if( epoch.endTime <= uint32(block.timestamp) ) {
            ...
            Alpha.rebase( epoch.distribute, epoch.number );
            ...
            uint staked = Alpha.circulatingSupply();
    ...
    /**
        @notice returns contract Sigma holdings, including bonuses provided
        @return uint
     */
    function contractBalance() public view returns ( uint ) {
        return Sigma.balanceOf( address(this) ).add( totalBonus );
    }
