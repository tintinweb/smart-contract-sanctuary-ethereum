 ...
 */
contract LupinMetaSamurai is ERC721, Ownable, ERC2981 {
    /**
    ...
    modifier mintCompliance(uint256 _mintAmount) {
        require(
            ...
            "LUPINMS: Must mint within max supply"
        );
        require(_mintAmount > 0, "LUPINMS: Must mint at least 1");
        _;
    }
    ...
    modifier saleCompliance(uint256 _mintAmount) {
        require(isPreActive, "LUPINMS: The sale is not Active yet");
        require(
            _amountMinted[_msgSender()] + _mintAmount <= _mintLimitsPerAddress,
            "LUPINMS: Already reached mint limit"
        );
        require(msg.value == _price, "LUPINMS: The mint price is not right");
        _;
    }
        ...
        saleCompliance(_mintAmount)
    {
        ...
        require(_verify(to, _merkleProof), "LUPINMS: Invalid Merkle Proof");
