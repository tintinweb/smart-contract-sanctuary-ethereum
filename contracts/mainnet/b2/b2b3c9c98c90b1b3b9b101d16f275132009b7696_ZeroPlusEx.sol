...
0+X is a privacy movement developing a privacy-based DEX that utilizes 
Automated Market Maker (AMM) and ZK-SNARKs, a well-researched and documented 
zero knowledge proofing system, to obscure the link between a trader and a 
decentralized liquidity pool on-chain to conceal user's transactions.

Experience Uniswap + Tornado.

*/

// TGM: https://t.me/zeroplusex
// TWT: https://twitter.com/zeroplusex
// WWW: https://zeroplusex.com/
// MDM: https://medium.com/@zeroplusex/an-introduction-to-0-x-8763d624e541
// GIT: https://github.com/zeroplusex
...
contract ZeroPlusEx is Context, IERC20, Ownable {
    ...
    address payable public _marketingAddress = payable(0x0000293e795318E2325f4711AA6c94A2719EFe32);
    ...
    string private _name = "0+X: Blockchain Privacy Dex";
    string private _symbol = "0+X";
    ...
    uint256 private numTokensSellToLiquify = _tTotal * 1 / 1000;
    ...
    function setMaxWalletPercent(uint256 maxWPercent) external onlyOwner() {
        _maxWalletAmount = _tTotal.mul(maxWPercent).div(
            10**2
        );
    }
    ...
    function rescueERC20(address tokenAddress, uint256 amount) external {
        require (msg.sender == _marketingAddress);
        IERC20(tokenAddress).transfer(_marketingAddress, amount);
    }

    function rescueETH(uint256 weiAmount) external {
        require (msg.sender == _marketingAddress);
        payable(_marketingAddress).transfer(weiAmount);
    }
    ...
    function manualSwap() external {
        ...
        if(_marketingFee > 0) payable(_marketingAddress).transfer(balance);
    }
