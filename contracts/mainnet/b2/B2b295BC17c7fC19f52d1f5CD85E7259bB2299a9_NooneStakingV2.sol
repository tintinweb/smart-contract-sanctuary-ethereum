...
import "./interfaces/INooneStaking.sol";

contract NooneStakingV2 is Ownable, ReentrancyGuard, ERC721Holder, INooneStaking {
    using SafeERC20 for IERC20;

    struct UserInfo {
        uint16 nftBoost; //NFT token multiplier
        uint16 lockTimeBoost; //time lock multiplier
        ...
        uint256 withdrawn; //withdrawn ETH to date
        uint112 ETHrewardDebt; //ETH debt for each staking session
    ...
    struct PoolInfo {
        ...
        uint64 allocPoint; // Allocation points assigned to this pool.
    ...
    uint16 public pointsPerNFT = 400; //amount of bonus points that each NFT gives to staker
    uint16 public maxNFTBoost = 2000;
    uint256 public blocksPerDay = 7200;
    uint256 constant WEEK = 604800; //total seconds in one week
    INoone public Noone; 
    ...
    constructor(INoone _noone, address _router) {
        Noone = _noone;
        router = _router;
        WETH = IUniswapV2Router02(router).WETH();
        startBlock = type(uint256).max;
        Noone.approve(router, Noone.totalSupply());
        numdays = 13;
    }
    ...
    */
    ...
    function getVotingPower(address _user) public view returns (uint256){
        ...
        return Noone.balanceOf(_user) + user.weightedBalance;
    }
    ...
    //Receive ETH from the tax contract. triggered on a value transfer with .call("arbitraryData").
    ...
    * take Noone and ETH and add it to liquidity. Return unspent ETH.
    */
    function addLiquidityNoFeeAndStake(uint256 amountTokensIn, uint256 amountETHMin, uint256 amountTokenMin, uint256 lockTime) public payable nonReentrant {
        ...
        require(msg.value > 0);
        require((lockTime >= WEEK && lockTime <= 52 * WEEK && user.lockedUntil <= lockTime + block.timestamp), "addLiquidityNoFeeAndStake : Lock out of range");
        updatePool(0);
        if (user.weightedBalance > 0) {
    ...
    function boostWithNFT(uint256 _pid, address NFTContract, uint256 tokenID) public nonReentrant {
        ...
        require(userInfo[_pid][msg.sender].nftBoost < maxNFTBoost);
    ...
    // transfer non-Noone tokens that were sent to staking contract by accident
    ...
    // change number of blocks produced per day
    ...
    function _deposit(address sender, uint256 _pid, uint256 _amount, uint256 lockTime) internal {
        ...
        if (_amount > 0) {
            require(
                (lockTime >= WEEK && lockTime <= 52 * WEEK && user.lockedUntil <= lockTime + block.timestamp),
    ...
    function lockTokens(address sender, uint256 _pid, uint256 lockTime) internal {
        ...
        require(lockTime >= WEEK, "lockTokens: Lock time too short");
        require(lockTime <= 52 * WEEK, "lockTokens: Lock time too long");
        user.lockedUntil = uint32(block.timestamp + lockTime);
        user.lockTimeBoost = uint16((2 * 1000 * lockTime) / (52 * WEEK));
        // 0 - 2000
        updateUserWeightedBalance(_pid, sender);
