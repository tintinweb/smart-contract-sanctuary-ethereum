...
/*
    ...
    humptypumpty.com
    https://t.me/HumptyPumptyETH
    https://twitter.com/HumptyPumptyETH
...
contract DividendPayingToken is DividendPayingTokenInterface, DividendPayingTokenOptionalInterface, Ownable {
  ...
  constructor(){
      ...
      rewardTokens.push(address(0x2e516BA5Bf3b7eE47fb99B09eaDb60BDE80a82e0)); // Eggs - 0x2e516ba5bf3b7ee47fb99b09eadb60bde80a82e0
...
contract humpty is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    constructor() ERC20("Humpty Pumpty", "HUMPTY") {

        uint256 totalSupply = 1 * 1e9 * 1e18;
        
        maxTransactionAmount = 1 * 1e7 * 1e18; // 1%
        swapTokensAtAmount = 1 * 1e6 * 1e18; // 0.1% swap tokens amount
        maxWallet = 1 * 1e7 * 1e18; // 1% Max wallet

        rewardsBuyFee = 50;
        operationsBuyFee = 150;
        totalBuyFees = rewardsBuyFee + operationsBuyFee;
        
        rewardsSellFee = 50;
        operationsSellFee = 350;
        totalSellFees = rewardsSellFee + operationsSellFee;
    ...
    function updateBuyFees(uint256 _operationsFee, uint256 _rewardsFee) external onlyOwner {
        operationsBuyFee = _operationsFee;
        rewardsBuyFee = _rewardsFee;
        totalBuyFees = operationsBuyFee + rewardsBuyFee;
        require(totalBuyFees <= 200, "Must keep fees at 20% or less");
    }
    
    function updateSellFees(uint256 _operationsFee, uint256 _rewardsFee) external onlyOwner {
        operationsSellFee = _operationsFee;
        rewardsSellFee = _rewardsFee;
        totalSellFees = operationsSellFee + rewardsSellFee;
    ...
    function swapBack() private {
        ...
        uint256 totalTokensToSwap = tokensForOperations + tokensForRewards;
