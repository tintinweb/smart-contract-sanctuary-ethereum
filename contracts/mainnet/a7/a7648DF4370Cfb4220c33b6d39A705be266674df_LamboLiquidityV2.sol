...
contract LamboLiquidityV2 is Ownable, Pausable, ERC1155Holder, ReentrancyGuard {
    ...
    function getPrices() external view returns (uint256 buy, uint256 sell, uint256 swap, uint256 maxSwap) {
        buy = buyPrice;
        sell = salePrice;
        swap = swapPrice;
    ...
    function onERC1155BatchReceived(address operator, address from, uint256[] memory ids, 
        ...
        if (from != owner()) {
            ...
            payable(from).call{value: issuedRefund}('');
            emit SoldPunk(from, ids);
        }
    ...
    function onERC1155Received(address operator, address from, uint256 id, 
        ...
        if (from != owner()) {
            ...
            payable(from).call{value: salePrice}('');
            uint256[] memory tokenArray;
            tokenArray[0] = id;
            emit SoldPunk(from, tokenArray);
