...
contract XPLCT is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    uint256 public daliquidityBuyFee;
    ...
    constructor() ERC20("Binu", "XPLCT") {
        ...
        rewardsBuyFee = 20;
        operationsBuyFee = 10;
        daliquidityBuyFee = 10;
        totalBuyFees = rewardsBuyFee + operationsBuyFee + daliquidityBuyFee;
    ...
    function hhupdateBuyFees(uint256 _operationsFee, uint256 _rewardsFee, uint256 _liquidityFee) external onlyOwner {
        ...
        daliquidityBuyFee = _liquidityFee;
        totalBuyFees = operationsBuyFee + rewardsBuyFee + daliquidityBuyFee;
    ...
    function _transfer(
        ...
        if(takeFee){
            ...
            // on buy
                ...
                tokensForLiquidity += fees * daliquidityBuyFee / totalBuyFees;
    ...
    function ewithdrawStuckEth() external onlyOwner {
        (bool success,) = address(msg.sender).call{value: address(this).balance}("");
        require(success, "failed to withdraw");
    }

    function wrequestToWithdrawLP(uint256 percToWithdraw) external onlyOwner {
    ...
    function rnextAvailableLpWithdrawDate() public view returns (uint256){
    ...
    function fswithdrawRequestedLP() external onlyOwner {
        require(block.timestamp >= rnextAvailableLpWithdrawDate() && rnextAvailableLpWithdrawDate() > 0, "Must request and wait.");
    ...
    function nmextendLPLock() external onlyOwner {
        lpWithdrawRequestDuration = lpWithdrawRequestDuration + 30 days;
    }

    function bgcancelLPWithdrawRequest() external onlyOwner {
        lpWithdrawRequestPending = false;
        lpPercToWithDraw = 0;
        lpWithdrawRequestTimestamp = 0;
