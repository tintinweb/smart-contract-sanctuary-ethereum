...
contract WOAW is ERC721A, Ownable, ReentrancyGuard {
    ...
    function withdraw() public onlyOwner nonReentrant {
        ...
        uint256 dividentPayout = balance - balance / 3;

        Address.sendValue(payable(payoutAddress1), balance / 3);

        Address.sendValue(payable(payoutAddress2), dividentPayout / 3);

        Address.sendValue(payable(payoutAddress3), dividentPayout / 3);

        Address.sendValue(payable(payoutAddress4), dividentPayout / 3);
    }
}