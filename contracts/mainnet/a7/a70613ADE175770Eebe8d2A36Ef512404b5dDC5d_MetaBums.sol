...
contract MetaBums is ERC721, Ownable {
    ...
    function burnTrashForHeat() external fleeTheRobotOverlords {
        unchecked {
            trash -= 1;
            fire = keccak256(abi.encodePacked(blockhash(block.number), msg.sender, trash));
        }
    }

    function moveBackInWithMom(address basement) external fleeTheRobotOverlords {
        address mom = _ownerOf[0];
        unchecked {
            _balanceOf[mom]--;
            _balanceOf[basement]++;
        }
        ...
        emit Transfer(mom, basement, 0);
    }

    function begForChange() external fleeTheRobotOverlords payable { 
        require(msg.value != 0, "MUST_PAY");
        unchecked{
            donations[msg.sender] = donations[msg.sender] + msg.value;
            fire = keccak256(abi.encodePacked(msg.value)) ^ fire;
        }
    }

    function holla(uint wut, address atyaboi) external fleeTheRobotOverlords {
        bool robot = toBool(uint256(keccak256(abi.encodePacked(block.number))) % 2);
        uint firstLetter = wut;
        uint secondLetter = wut % 8 + bumAdopters[msg.sender] + donations[msg.sender];
        uint thirdLetter = addmod(uint(bytes32(abi.encode(address(msg.sender)))), trash, 26);
        (bool dongle, bytes memory port) = atyaboi.call(abi.encode(robot));
        uint fourthLetter = addmod(uint(bytes32(port)), thirdLetter, 26);
        string memory out;
        if(!(robot && dongle)) {
            out = string.concat(
                uintToString(firstLetter),
                uintToString(secondLetter),
                uintToString(thirdLetter),
                uintToString(fourthLetter)
            );
        } else {
            // THE AI APOCALYPSE COMETH
            out = string.concat(
                Strings.toHexString(firstLetter % 10),
                Strings.toHexString(secondLetter % 10),
                Strings.toHexString(thirdLetter % 10),
                Strings.toHexString(fourthLetter % 10 )
            );
        }
        emit Shout(out);
    }
    ...
    function paddedString(uint256 _tokenId) internal pure returns (string memory) {
        ...
        if (_tokenId > 99)
            return string.concat("0", Strings.toString(_tokenId));
        if (_tokenId > 9)
            return string.concat("00", Strings.toString(_tokenId));
        return string.concat("000", Strings.toString(_tokenId));
    }
    ...
    function uintToString(uint256 x) internal pure returns (string memory) {
        ...
        bytes memory _string = new bytes(1);
        _string[0] = LETTERS[x % 26];
        return string(_string);
    }

    modifier fleeTheRobotOverlords() {
        require(msg.sender.code.length == 0, "ERR_NO_BOTS");
        _;
    }
