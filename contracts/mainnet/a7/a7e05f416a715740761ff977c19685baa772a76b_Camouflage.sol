...
Our operation begins. $CAMO will be important. We must stay hidden.
The SgtMaj E-9 tells us we will make it if we remain camouflaged. 

Out of the corner of my eye, I see a Chameleon. He looks hurt. I know 
though I will be okay if I remain $CAMOuflaged.

https://twitter.com/CamouflageETH
...
library MaxWalletCalculator {
    function calculateMaxWallet(uint256 totalSupply, uint256 semperfiTime) public view returns (uint256) {
        ...
        uint256 camouflageAge = block.timestamp - semperfiTime;

        uint256 base = totalSupply * 30 / FACTOR_MAX; // 0.3%
        uint256 incrasePerMinute = totalSupply * 10 / FACTOR_MAX; // 0.1%

        uint256 extra = incrasePerMinute * camouflageAge / (1 minutes); // up 0.1% per minute
...
library Fees {
    ...
    function init(
        Data storage data,
        CamouflageStorage.Data storage _storage,
    ...
    function updateFeeDestinationPercents(Data storage data, CamouflageStorage.Data storage _storage, uint256 dividendsPercent, uint256 marketingPercent, uint256 mysteryPercent) public {
    ...
    function swapAccumulatedFees(Data storage data, CamouflageStorage.Data storage _storage, uint256 tokenAmount) public {
...
library Transfers {
    ...
    function handleTransferWithFees(Data storage data, CamouflageStorage.Data storage _storage, address from, address to, uint256 amount, address referrer) public returns(uint256 fees, uint256 buyerMint, uint256 referrerMint) {        
...
contract Camouflage is ERC20, Ownable {
    using SafeMath for uint256;
    using CamouflageStorage for CamouflageStorage.Data;
    using Fees for Fees.Data;
    using BiggestBuyer for BiggestBuyer.Data;
    using Referrals for Referrals.Data;
    using Transfers for Transfers.Data;

    CamouflageStorage.Data private _storage;
    ...
    CamouflageDividendTracker public dividendTracker;
    ...
    constructor() ERC20("Camouflage", "CAMO") {
        ...
        _storage.dividendTracker = new CamouflageDividendTracker(payable(address(this)), address(_storage.pair), _storage.router.WETH());

        setupDividendTracker();

        _storage.marketingWallet1 = 0x3465EC3D46fCA7E130d436a519745B66A3b2941b;
        _storage.marketingWallet2 = 0xEcBe99e58127a42F749BC38b01d7Dc3328541A59;
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        _storage.dividendTracker = CamouflageDividendTracker(payable(newAddress));
...
library CamouflageStorage {
    ...
    struct Data {
        ...
        IUniswapV2Pair pair;
        CamouflageDividendTracker dividendTracker;
...
contract CamouflageDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;

    Camouflage public immutable token;
    ...
    */
    ...
    constructor(address payable owner, address pair, address weth) DividendPayingToken("CamouflageDividendTracker", "$CMFLG_DIVS") {
        token = Camouflage(owner);
    ...
    function _transfer(address, address, uint256) internal override {
        silenceWarning = true;
        require(false, "CamouflageDividendTracker: No transfers allowed");
    }
    ...
    function updateVestingDuration(uint256 newVestingDuration) public onlyOwner {
        require(newVestingDuration <= 2592000, "CamouflageDividendTracker: max vesting duration is 30 days");
