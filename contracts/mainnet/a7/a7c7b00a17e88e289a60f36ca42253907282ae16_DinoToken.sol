...
// File: DieHardDinos.sol



// DinoPunks Team ~ DinoSphere
pragma solidity >=0.8.0;
...
contract DinoToken is ERC20, Ownable {
    ...
    function lockUnlockAll() public onlyOwner{
        isUnlockedForAll = !isUnlockedForAll;
    }
    ...
    /*
     * @dev unlock user and allow him to mint
     * @param _holder holders address
     * @param _genNumber NFT collection number
     */
    function unlockUser(address _holder, uint256 _genNumber) public onlyOwner {
        require(
            stakesTimeLock[_genNumber][_holder] > 0,
            "No Timelock on This User"
        );
        stakesTimeLock[_genNumber][_holder] = block.timestamp - _TIMELOCK * 2;
    }

    /*
     * @dev set timelock for unstaking
     * @param _timelock number of days
     */
    ...
    /*
     * @dev get rewards multipliers for current account,
     * multipliers depend on the number of NFTs held and the number of collections
    ...
    /*
     * @dev set rewards multipliers for a generation,
     * a threshold can be set for an additional multiplier bonus
     * @param _genNumber NFT collection generation
     * @param _threshold threshold for bonus multiplier
     * @param _multiplier bonus value
     */
    ...
    /*
     * @dev set rewards multipliers for holding multiple generations
     * @param _multiplier bonus value
     */
    ...
    function getUserLastTimeStamp(address _stakeholder)
        public
        view
        returns (uint256)
    {
        return stakes[_stakeholder].lastBlockTimeStamp;
    }

    /*
     * @dev set reward rate for each generation
     * @param _genNumber NFT collection generation
    ...
    function setRewardRate(uint256 _genNumber, uint256 _rate) public onlyOwner {
        ...
        require(
            collections[_genNumber].nftType > 0,
    ...
    /*
     * @dev Get number of NFTs held by holder for current generation
     * @param _genNumber NFT collection generation
     * @param _holder holder address
     */
    ...
    /*
     * @dev get staked tokens for a set generation for a holder
     * @param _genNumber NFT collection generation
     * @param _holder holder address
     */
    ...
    /*
     * @dev update holder balance of NFTs
     * @param _genNumber NFT collection generation
     * @param _staker holder address
     * @param _tokenIds tokens staked by current address
     */
    function updateBalance(
        ...
        stakes[_staker].lastBlockTimeStamp = block.timestamp;
    }
    ...
    /**
     * @notice A method for a stakeholder to remove all stake.
     */
    function unstakeAll() public {
        for (uint256 i = 1; i <= nbOfCollections; i++) {
     ...
     * @param _genNumber generation number
     */
    function unstakeNFTByGen(uint256 _genNumber) public {
        if (
            stakesTimeLock[_genNumber][msg.sender] == 0 &&
            isUnlockedForAll == false
        ) {
            stakesTimeLock[_genNumber][msg.sender] =
                block.timestamp +
                _TIMELOCK;
        } else {
            if (isUnlockedForAll == false)
                require(
                    stakesTimeLock[_genNumber][msg.sender] <= block.timestamp,
                    "Unstaking is Currently Locked, Please Wait For Unlock and Try Again"
                );
        ...
        returns (uint256)
    {
        ...
        uint256 lastDiffClaim = block.timestamp -
            stakes[_stakeholder].lastBlockTimeStamp;
        return
            ((reward * getRewardsMultiplier(_stakeholder) * lastDiffClaim) /
                SECONDS_PER_DAY) / 100;
    }
