...
// File: contracts/AzukiUpgradeable.sol
...
contract AzukiUpgradeable is ERC721AUpgradeable, OwnableUpgradeable {

    // should set in initialize since upgradeable
    string public uriPrefix = '';
    string public uriSuffix = '';
    bool public revealed = true;
    string public hiddenMetadataUri;
    uint256 public startTokenId;
    uint256 public tokenIdPadding;
    mapping(uint256 => string) public specificTokenUrl;
    string public specificCommonUrl = '';
  ...
  function initialize(string memory name_,
        ...
        uint256 startTokenId_) initializer public {
    // should be set before erc721a initialize
  ...
  function setUriSuffix(string memory _uriSuffix) public onlyOwner {
    ...
    // used for special padding case
    function setTokenIdPadding(uint256 _digits) public onlyOwner {
    tokenIdPadding = _digits;
  }
    ...
    function tokenURI(uint256 _tokenId) public view virtual override returns (string memory) {
        ...
        string memory specificUrl = specificTokenUrl[_tokenId];
        if (bytes(specificUrl).length > 0) {
            return specificUrl;
        }
        if (bytes(specificCommonUrl).length > 0) {
            return specificCommonUrl;
        }

        string memory currentBaseURI = _baseURI();
        string memory tokenIdInUrl = Strings.toString(_tokenId);
        if (tokenIdPadding > 0) {
            tokenIdInUrl = toStringPadding(_tokenId, tokenIdPadding);
        }
    ...
    function setSpecificTokenUrl(uint256 _tokenId, string memory _url) public onlyOwner {
        specificTokenUrl[_tokenId] = _url;
    }
    function setSpecificCommonUrl(string memory _url) public onlyOwner {
        specificCommonUrl = _url;
    }
    ...
    function transferERC20Balance(address erc20ContractAddress, address payable to, uint256 ammount) onlyOwner() public{
        ...
        /**
     * @dev Converts a `uint256` to its ASCII `string` decimal representation.
     * digits represent the total length
     */
    function toStringPadding(uint256 value, uint256 digits) internal pure returns (string memory) {
        // Inspired by OraclizeAPI's implementation - MIT licence
        // https://github.com/oraclize/ethereum-api/blob/b42146b063c7d6ee1358846c198246239e9360e8/oraclizeAPI_0.4.25.sol
