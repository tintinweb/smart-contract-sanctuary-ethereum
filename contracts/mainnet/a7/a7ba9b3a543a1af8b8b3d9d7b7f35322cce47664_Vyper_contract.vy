...
def add_base_pool(
    ...
    assert msg.sender == self.ownership_admin, "Only DAO owner can add base pool"
...
def set_fee_receiver(_target: address, _base_pool: address, _fee_receiver: address):
    """
    @notice Sets `_fee_receiver` for all metapools paired with `_base_pool`
    @param _target `Factory` deployment address
    @param _base_pool Base pool address
    @param _fee_receiver Fee receiver address
    """
    assert msg.sender in [self.ownership_admin, self.emergency_admin], "Not authorized"
