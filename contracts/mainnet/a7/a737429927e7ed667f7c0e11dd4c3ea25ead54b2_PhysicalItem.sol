...
// File: contracts/TransparentPattern/WolfDen/PhysicalItem.sol
...
contract PhysicalItem is Initializable, AdminUpgradeable, PausableUpgradeable{

    address public mainRedeem;

    bytes32 public typeContract;

    

    mapping(address => uint256) public CounterNumber;
    ...
    function initialize(bytes32 _type, address _mainRedeem) external initializer {

        ___Adminable_init();

        typeContract = _type;

        mainRedeem = _mainRedeem;
    ...
    function setRedeemContract(address _mainRedeem) external onlyAdmin{

        require(_mainRedeem != address(0), "must not zero address");
    ...
    //redeem an item from main contract

    function redeemItem(

        address _redeemer, 

        address _contractRedeem, 
        ...
        require(msg.sender == mainRedeem, "only call from main redeem");

        require(_contractRedeem == address(this), "different contract redeem");

        require(_amount > 0, "amount must not zero");



        uint256 _tmp = CounterNumber[_redeemer];

        CounterNumber[_redeemer] = CounterNumber[_redeemer] +1;



        require(isAdmin(recoverSigner(prefixed(keccak256(abi.encodePacked(
                            ...
                            ))), _signature)) == true, "You have not access for redeem");



        emit RedeemedItem(_redeemer, _redeemId, _data);
