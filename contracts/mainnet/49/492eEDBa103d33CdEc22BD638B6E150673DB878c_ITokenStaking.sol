...
/**
 * @title Pawnfi's ITokenStaking Contract
 * @author Pawnfi
 */
contract ITokenStaking is ReentrancyGuardUpgradeable, OwnableUpgradeable {
     ...
     * @member depositAmount The amount of tokens deposited into the contract.
     * @member adjustedAmount The user's effective balance after boosting, used to calculate emission rates.
     * @member rewardDebt Claimed rewards
     * @member claimable Claimable rewards
     */
    struct UserInfo {
     ...
     * @member adjustedSupply Adjusted supply
     * @member rewardsPerSecond Reward per second 
     * @member lastRewardTime Last second that reward distribution occurs.
     * @member accRewardPerShare Accumulated rewards per share, times 1e12.
     */
    struct PoolInfo {
    ...
    /// @notice PAWN reward amount for iToken staking
    uint256 public maxMintableTokens;

    /// @notice Distributed reward amount
    uint256 public mintedTokens;

    /// @notice Return the lending markets allowed for staking
    ...
    /// @notice The timestamp when reward mining starts.
     ...
     * @param incentiveVoting_ incentiveVoting contract address
     * @param tokenSAFE_ tokenSAFE contract address
     * @param maxMintable_ PAWN reward amount for iToken staking
     */
    function initialize(address owner_, IERC20Upgradeable rewardToken_, IIncentiveVoting incentiveVoting_, ISAFE tokenSAFE_, uint256 maxMintable_) external initializer {
        ...
        maxMintableTokens = maxMintable_;
    }
     ...
     * @return uint256 The number of lending markets allowed for staking
     */
     ...
     * @param token lending market address
     * @return bool result
     */
    function addPool(address token) external returns (bool) {
     ...
     * @param newReceiver Claim receiver address
     */
     ...
     * @param userAddr User to query pending rewards for
     * @param tokens Array of token addresses to query
     * @return uint256[] Unclaimed rewards
     */
    function claimableReward(address userAddr, address[] calldata tokens) external view returns (uint256[] memory) {
        uint256[] memory claimable = new uint256[](tokens.length);
    ...
    /**
     * @notice Get updated reward data for the given token
     * @param token Array of token addresses to query
     * @return accRewardPerShare Reward per share
     * @return rewardsPerSecond Reward per second
     */
    function _getRewardData(address token) internal view returns (uint256 accRewardPerShare, uint256 rewardsPerSecond) {
        PoolInfo storage pool = poolInfo[token];
        ...
        if (lpSupply == 0) {
            return (0, incentiveVoting.getRewardsPerSecond(token, currentWeek));
        }
        ...
        if (rewardWeek < currentWeek) {
            while (rewardWeek < currentWeek) {
                uint256 nextRewardTime = (rewardWeek + 1) * WEEK + start;
                duration = nextRewardTime - lastRewardTime;
                reward = reward + duration * rewardsPerSecond;
                rewardWeek += 1;
                rewardsPerSecond = incentiveVoting.getRewardsPerSecond(token, rewardWeek);
     ...
     * @return accRewardPerShare Reward per share
     */
    function _updatePool(address token) internal returns (uint256 accRewardPerShare) {
        PoolInfo storage pool = poolInfo[token];
        uint256 lastRewardTime = pool.lastRewardTime;
        require(lastRewardTime > 0, "Invalid pool");
        
        // claim admin fees for each pool once per day
        try feeManager.withdrawFee(token) {
        ...
        (accRewardPerShare, pool.rewardsPerSecond) = _getRewardData(token);
    ...
    /**
     * @notice calculate adjusted balance and total supply, used for boost
                boost calculations are modeled after veCRV, with a max boost of 2.5x
     * @param userAddr User address
     * @param token lending market address
     * @param depositAmount iToken staked amount
     * @param accRewardPerShare Reward per share
     */
    function _updateLiquidityLimits(address userAddr, address token, uint256 depositAmount, uint256 accRewardPerShare) internal {
        uint256 userWeight = tokenSAFE.userWeight(userAddr);
        uint256 adjustedAmount = depositAmount * 40 / 100;
        if (userWeight > 0) {
            uint256 lpSupply = IERC20Upgradeable(token).balanceOf(address(this));
            uint256 totalWeight = tokenSAFE.totalWeight();
            uint256 boost = lpSupply * userWeight / totalWeight * 60 / 100;
            adjustedAmount += boost;
            if (adjustedAmount > depositAmount) {
                adjustedAmount = depositAmount;
            }
        }
        UserInfo storage user = userInfo[token][userAddr];
        uint256 newAdjustedSupply = poolInfo[token].adjustedSupply - user.adjustedAmount;
        user.adjustedAmount = adjustedAmount;
        poolInfo[token].adjustedSupply = newAdjustedSupply + adjustedAmount;
        user.rewardDebt = adjustedAmount * accRewardPerShare / 1e12;
    }

    /**
     * @notice Deposit LP tokens into the contract
     * @dev Also updates the receiver's current boost
     * @param token LP token address to deposit.
     * @param amount Amount of tokens to deposit.
     * @param claimRewards If true, also claim rewards earned on the token.
     * @return uint256 Claimed reward amount
     */
    function deposit(address token, uint256 amount, bool claimRewards) external nonReentrant returns (uint256) {
        ...
        uint256 depositAmount = user.depositAmount + amount;
        user.depositAmount = depositAmount;
        _updateLiquidityLimits(msg.sender, token, depositAmount, accRewardPerShare);
     ...
     * @param token LP token address to withdraw.
     * @param amount Amount of tokens to withdraw.
     * @param claimRewards If true, also claim rewards earned on the token.
     * @return uint256 Claimed reward amount
     */
    function withdraw(address token, uint256 amount, bool claimRewards) external nonReentrant returns (uint256) {
        require(amount > 0, "Cannot withdraw zero");
        uint256 accRewardPerShare = _updatePool(token);
        UserInfo storage user = userInfo[token][msg.sender];
        uint256 depositAmount = user.depositAmount;
        require(depositAmount >= amount, "withdraw: not good");
     ...
     * @param userAddr Address to claim rewards for. Reverts if the caller is not the
                     claimer and the claimer has blocked third-party actions.
     * @param tokens Array of LP token addresses to claim for.
     * @return uint256 Claimed reward amount
     */
    function claim(address userAddr, address[] calldata tokens) external returns (uint256) {
        if (msg.sender != userAddr) {
            require(!blockThirdPartyActions[userAddr], "Cannot claim on behalf of this account");
        }
        ...
        for (uint i = 0; i < tokens.length; i++) {
            ...
            user.claimable = 0;
            _updateLiquidityLimits(userAddr, token, user.depositAmount, accRewardPerShare);
        }
        return _mintRewards(userAddr, pending);
    }

    /**
     * @notice Send PAWN reward
     * @param userAddr User address
     * @param amount Reward amount
     */
    function _mintRewards(address userAddr, uint256 amount) internal returns (uint256) {
        uint256 minted = mintedTokens;
        if (minted + amount > maxMintableTokens) {
            amount = maxMintableTokens - minted;
        }
        if (amount > 0) {
            mintedTokens = minted + amount;
            address receiver = claimReceiver[userAddr];
            if (receiver == address(0)) receiver = userAddr;
            rewardToken.safeTransfer(receiver, amount);

            emit ClaimedReward(msg.sender, userAddr, receiver, amount);
        }
        return amount;
    }
     ...
     * @param userAddr Address of the user to update boosts for
     * @param tokens Array of LP tokens to update boost for
     */
    function updateUserBoosts(address userAddr, address[] calldata tokens) external {
        for (uint i = 0; i < tokens.length; i++) {
            address token = tokens[i];
            uint256 accRewardPerShare = _updatePool(token);
            UserInfo storage user = userInfo[token][userAddr];
...
interface IIncentiveVoting {
    function tokenSAFE() external view returns(address);
    ...
    function rewardsPerSecond(uint256 _index) external view returns(uint256);
    function rewardsPerSecondLength() external view returns(uint256);
    function totalVotes(uint256 _index) external view returns(uint256);
    function tokenVotes(address _token, uint256 _index) external view returns(uint256);
    function approvedTokensLength() external view returns(uint256);
    function approvedTokens(uint256 _index) external view returns(address);
    function userVotes(address _user, uint256 _week) external view returns(uint256);
    function userTokenVotes(address _user, address _token, uint256 _week) external view returns(uint256);
}
