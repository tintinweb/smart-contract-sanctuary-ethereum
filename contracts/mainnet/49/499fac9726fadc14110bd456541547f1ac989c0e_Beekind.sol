...
contract Beekind is ERC721A, ERC2981, Ownable, DefaultOperatorFilterer  {
    ...
    constructor(address _royaltyReceiver, address _publicSigner, address _beelistSigner) ERC721A("Beekind", "BKIND") {
    ...
    function beelistbZzZz(bytes calldata _signature) external isMintable noContract validSignature(_signature, beelistSigner) {
    ...
    function bZzZz(bytes calldata _signature) external payable isMintable noContract validSignature(_signature, publicSigner)  {
        require(saleState == 1, "Public sale not active.");
        require(publicMints + 1 <= publicAllocation, "Public allocation is full.");
