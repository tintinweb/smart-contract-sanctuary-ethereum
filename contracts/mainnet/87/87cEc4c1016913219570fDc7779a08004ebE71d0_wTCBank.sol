...
contract wTCBank{
    ...
    address public stakingcontract; //Staking
   ...
   function manageClients(uint _option,bool _clearbalance,address _holder) external {
       require(msg.sender==Owner || msg.sender==stakingcontract || msg.sender ==rafflecontract,"(Bank) Not Authorized");
       if (_option==1) //Remove
       {
   ...
   function manageBalances(uint _option,address _user,uint _amount) external {
    ...
    require(msg.sender == Owner || msg.sender == stakingcontract || msg.sender == rafflecontract,"(Bank) Not Auth!");
  ...
  //Pull ETH from contract if any ends up in contract
