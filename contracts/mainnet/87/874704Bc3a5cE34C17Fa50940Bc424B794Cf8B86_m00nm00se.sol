...
contract m00nm00se is ERC721A, Ownable, ReentrancyGuard {
    ...
    {
        require(mintStatus , "m00nm00se: Minting Public Pause");
        require(qty <= maxPerTx, "m00nm00se: Limit Per Transaction");
        require(totalSupply() + qty <= maxSupply,"m00nm00se: Soldout");
        _safemint(qty);
    }
    ...
    function _safemint(uint256 qty) internal
    {
        if(WalletMint[msg.sender] < freeMint) 
        {
           ...
           require(msg.value >= (qty - freeMint) * MintPrice,"m00nm00se: Fund not enough");
            WalletMint[msg.sender] += qty;
           _safeMint(msg.sender, qty);
        }
        else
        {
           require(msg.value >= qty * MintPrice,"m00nm00se: Fund not enough");
            WalletMint[msg.sender] += qty;
           _safeMint(msg.sender, qty);
        }
    }
