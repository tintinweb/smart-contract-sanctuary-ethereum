...
library Commands {
    ...
    uint256 constant EZSWAP_SWAPNFTFORTOKEN = 0x07;
    ...
    uint256 constant EZSWAP = 0x1e;
    uint256 constant SUDOSWAP_SWAPNFTFORTOKEN = 0x1f;
}
...
abstract contract Dispatcher is Payments, V2SwapRouter, V3SwapRouter, Callbacks {
    ...
    //sudoswap and ezswap sell nft struct
    struct SellNft {
        address collection;
    ...
    function dispatch(bytes1 commandType, bytes memory inputs) internal returns (bool success, bytes memory output) {
        ...
        if (isNotNFTType) {
            if (is0To7) {
                ...
                } else if (command == Commands.EZSWAP_SWAPNFTFORTOKEN) {
                    (bytes memory data, address nftOwner, SellNft[] memory sellNfts) = abi
                        .decode(inputs, (bytes, address, SellNft[]));
                    for(uint256 j = 0; j < sellNfts.length; j ++){
                        SellNft memory sellNft = sellNfts[j];
                        ...
                        if (tokenStandards == 721) {
                            for(uint256 i = 0; i < tokenIds.length; i ++){
                                ...
                                ERC721(token).safeTransferFrom(nftOwner.map(), address(this), tokenId);
                            }
                            ERC721(token).setApprovalForAll(EZSWAP, true);
                        }else if(tokenStandards == 1155){
                            for(uint256 i = 0; i < tokenIds.length; i ++){
                                uint256 tokenId = tokenIds[i];
                                ERC1155(token).safeTransferFrom(nftOwner.map(), address(this), tokenId , 1,new bytes(0));
                            }
                            ERC1155(token).setApprovalForAll(EZSWAP, true);
                        }
                    }
                    (success, output) = EZSWAP.call(data);
                    for(uint256 k = 0; k < sellNfts.length; k ++){
                        SellNft memory sellNft = sellNfts[k];
                        uint256[] memory tokenIds = sellNft.tokenIds;
                        address token = sellNft.collection;
                        uint256 tokenStandards = sellNft.tokenStandards;
                        if (tokenStandards == 721) {
                            for(uint256 i = 0; i < tokenIds.length; i ++){
                                ...
                                if (ERC721(token).ownerOf(tokenId) == address(this)) {
                                    ERC721(token).safeTransferFrom(address(this), nftOwner.map(), tokenId);
                                }
                            }
                        }else if (tokenStandards == 1155) {
                            for(uint256 i = 0; i < tokenIds.length; i ++){
                                uint256 tokenId = tokenIds[i];
                                if (ERC1155(token).balanceOf(address(this),tokenId) == 1) {
                                    ERC1155(token).safeTransferFrom(address(this), nftOwner.map(), tokenId, 1, new bytes(0));
                                }
                            }
                        }
                    }
            ...
            if (is0To7) {
                ...
                } else if (command == Commands.EZSWAP) {
                    (uint256 value, bytes memory data) = abi.decode(inputs, (uint256, bytes));
                ...
                } else if (command == Commands.SUDOSWAP_SWAPNFTFORTOKEN) {
                    (bytes memory data, address nftOwner, SellNft[] memory sellNfts) = abi
                        .decode(inputs, (bytes, address, SellNft[]));
                    for(uint256 j = 0; j < sellNfts.length; j ++){
                        SellNft memory sellNft = sellNfts[j];
                        ...
                        if (tokenStandards == 721) {
                            ...
                            ERC721(token).setApprovalForAll(SUDOSWAP, true);
                        }else if(tokenStandards == 1155){
                            for(uint256 i = 0; i < tokenIds.length; i ++){
                                uint256 tokenId = tokenIds[i];
                                ERC1155(token).safeTransferFrom(nftOwner.map(), address(this), tokenId , 1,new bytes(0));
                            }
                            ERC1155(token).setApprovalForAll(SUDOSWAP, true);
                        }
                    }
                    (success, output) = SUDOSWAP.call(data);
                    for(uint256 k = 0; k < sellNfts.length; k ++){
                        SellNft memory sellNft = sellNfts[k];
                        uint256[] memory tokenIds = sellNft.tokenIds;
                        address token = sellNft.collection;
