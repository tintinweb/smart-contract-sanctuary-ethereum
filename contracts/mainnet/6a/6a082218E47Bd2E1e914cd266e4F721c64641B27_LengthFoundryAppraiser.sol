...
contract FoundryPriceOracle is Ownable {
  event PoolAdded(address indexed token, address indexed pool);
  event PoolRemoved(address indexed token, address indexed pool);

  int32 public secondsAgo = 3600;
  IERC20Metadata public nativeToken;
  mapping(address => IUniswapV3Pool) public pools;

  constructor(IERC20Metadata _nativeToken) {
    nativeToken = _nativeToken;
  }

  modifier onlyValidPool(address token) {
    require(address(pools[token]) != address(0), "Pool not found");
    _;
  }

  function setSecondsAgo(int32 _secondsAgo) public onlyOwner {
    secondsAgo = _secondsAgo;
  }

  function addPool(IUniswapV3Pool pool) public onlyOwner {
    address token0 = pool.token0();
    address token1 = pool.token1();

    require(
      token0 == address(nativeToken) || token1 == address(nativeToken),
      "Must be a pool with native token."
    );

    if (token0 == address(nativeToken)) {
      pools[token1] = pool;
      emit PoolAdded(token1, address(pool));
    } else {
      pools[token0] = pool;
      emit PoolAdded(token0, address(pool));
    }
  }

  function removePool(address token) public onlyOwner onlyValidPool(token) {
    IUniswapV3Pool pool = pools[token];
    delete pools[token];
    emit PoolRemoved(token, address(pool));
  }

  function toNative(uint128 amount, address token)
    public
    view
    onlyValidPool(token)
    returns (uint128)
  {
    int24 tick = getTick(pools[token]);
    return uint128(getQuoteAtTick(tick, amount, token, address(nativeToken)));
  }

  function fromNative(uint128 amount, address token)
    public
    view
    onlyValidPool(token)
    returns (uint128)
  {
    int24 tick = getTick(pools[token]);
    return uint128(getQuoteAtTick(tick, amount, address(nativeToken), token));
  }
  ...
  function convert(
    ...
    if (from == address(nativeToken)) {
      return fromNative(amount, to);
    }

    if (to == address(nativeToken)) {
      return toNative(amount, from);
    }

    uint128 nativeAmount = toNative(amount, from);
    return fromNative(nativeAmount, to);
  }

  function getTick(IUniswapV3Pool pool) internal view returns (int24) {
    ...
    int24 tick = int24(int256(tickCumulativesDelta) / int256(secondsAgo));
  ...
  function getQuoteAtTick(
    ...
    address fromToken,
    address toToken
  ) public pure returns (uint256 toAmount) {
    uint160 sqrtRatioX96 = getSqrtRatioAtTick(tick);

    // Calculate ratio with better precision if it doesn't overflow when multiplied by itself
    if (sqrtRatioX96 <= type(uint128).max) {
      uint256 ratioX192 = uint256(sqrtRatioX96) * sqrtRatioX96;
      toAmount = fromToken < toToken
        ? mulDiv(ratioX192, fromAmount, 1 << 192)
        : mulDiv(1 << 192, fromAmount, ratioX192);
    } else {
      uint256 ratioX128 = (sqrtRatioX96 * sqrtRatioX96) / (1 << 64);
      toAmount = fromToken < toToken
        ? (ratioX128 * fromAmount) / (1 << 128)
        : ((1 << 128) * fromAmount) / ratioX128;
    }
  }
