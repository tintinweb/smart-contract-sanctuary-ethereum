...
contract HEADLINES is ERC721AQueryable, Ownable {
  using Strings for uint256;

  uint256 public constant TOTAL_MAX_SUPPLY = 4440;
  ...
  constructor() ERC721A("UNLIKELY HEADLINES", "HEADLINES") {}

  modifier callerIsUser() {
    require(tx.origin == msg.sender, 'HEADLINES: The caller is another contract');
    _;
  }
  ...
  modifier underMaxSupply(uint256 _quantity) {
    require(
      ...
      "HEADLINES: Purchase exceeds max supply"
    );
    _;
  }

  function mint(uint256 _quantity) external payable callerIsUser underMaxSupply(_quantity) {
    require(balanceOf(msg.sender) < maxPublicMintPerWallet, "HEADLINES: Mint limit exceeded");
    require(saleStarted, "HEADLINES: Sale has not begun");
    if (_totalMinted() < (TOTAL_MAX_SUPPLY)) {
      if (freeMintCount >= totalFreeMints) {
        require(msg.value >= _quantity * publicTokenPrice, "HEADLINES: More ETH required to save one of the survivors!");
        ...
        if (_quantity <= _mintableFreeQuantity) {
          ...
          require(
            ...
            "HEADLINES: Insufficient ETH"
          );
        }
        _mint(msg.sender, _quantity);
      } else {
        require(msg.value >= (_quantity * publicTokenPrice), "HEADLINES: Not enough ETH to save one of the survivors");
        _mint(msg.sender, _quantity);
      }
    }
  }
  ...
  function withdrawFunds() external onlyOwner {
    ...
    require(success, "HEADLINES: Transfer failed.");
  }
