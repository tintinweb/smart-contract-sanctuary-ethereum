...
contract   KevinBearContract is Ownable, ERC721A, ReentrancyGuard {
 ...
 constructor(
    uint256 maxBatchSize_,
    uint256 TotalToken_
  ) ERC721A("Kevin Bear", "KevinBear", maxBatchSize_, TotalToken_) {
  ...
  function SetIsFreeActive() public onlyOwner {
  ...
  function MintTo(address _to,uint256 mintamount) external onlyOwner {        
    require(totalSupply() + mintamount <= TotalCollectionSize, "Can not mint more than 4000");

    _safeMint(_to, mintamount);
    }

  function Claim(uint256 mintamount) external payable {
    require(IsFreeActive, "Sale is not started");
    require(mintamount > 0 && mintamount <= MaxMintPerTx, "Can not mint more than 20");  
    require(totalSupply() + mintamount <= MaxClaimTokenTotalForFree, "Can not mint more than 500");

    _safeMint(msg.sender, mintamount);
  }

  function Mint(uint256 mintamount) external payable {
    require(Pause, "Sale is not started");
    require(totalSupply() + mintamount <= TotalCollectionSize, "Can not mint more than 4000");    
    require(mintamount > 0 && mintamount <= MaxMintPerTx, "Can not mint more than 20");
