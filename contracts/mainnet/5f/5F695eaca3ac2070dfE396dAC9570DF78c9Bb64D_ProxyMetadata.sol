...
interface SubscriptionMetadata {
    ...
    function expires(bytes32 _hash) external view returns (uint256);

    function Manager() external view returns (IManager);

    function ensToken() external view returns (IENSToken);

    function DefaultImage() external view returns (string memory);

    function mintCount(uint256 _tokenId) external view returns (uint256);
}
...
contract ProxyMetadata is IMetadata, PrimaryEns {
    ...
    SubscriptionMetadata public immutable subscriptionMetadata;
    ...
    constructor(
        ...
        subscriptionMetadata = SubscriptionMetadata(_subscriptionMetadata);
    ...
    function json(
        ...
        string memory locked = (subscriptionMetadata.ensToken().ownerOf(
            ownerId
        ...
        uint256 expiry = subscriptionMetadata.ensToken().nameExpires(ownerId);

        uint256 subExpires = subscriptionMetadata.expires(
            subdomainHash(ownerId, label)
        );
        ...
        string memory description = subscriptionMetadata.descriptions(ownerId);
        ...
        bytes memory data = abi.encodePacked(
            ...
            (
                bytes(image).length == 0
                    ? subscriptionMetadata.DefaultImage()
                    : image
            ),
    ...
    function tokenURI(uint256 tokenId) external view returns (string memory) {
        uint256 parentId = domainManager.IdToOwnerId(tokenId);

         
        if (subscriptionMetadata.mintCount(parentId) > 0) {
            
            return json(tokenId, parentId);
        } else {
