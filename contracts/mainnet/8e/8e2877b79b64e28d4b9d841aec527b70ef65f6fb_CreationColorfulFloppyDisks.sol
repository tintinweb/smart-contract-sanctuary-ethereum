...
contract CreationColorfulFloppyDisks is ERC721 , DefaultOperatorFilterer {
    ...
    constructor()
        ERC721("CreationColorfulFloppyDisks", "CCFD")
    {}

    function mint() external payable {
        
        if (userMints[msg.sender] >= maxMints) revert MintLimit();
        if (totalSupply + 1 > 114) revert SoldOut();
        ...
        returns (string memory)
    {
        return
            string(
                abi.encodePacked("ipfs://bafybeibw4gxueyzy7253lqokmyf5m7slfj4znhtv3bfj6iy2u65nmyca3i/", Strings.toString(tokenId), ".json")
            );
    }
