...
contract cryptodoge is ERC721A {
    ...
    function claim(uint256 amount) canmint(amount) payable public {
        require(totalSupply() + amount <= maxSupply);
        _safeMint(msg.sender, amount);
    }

    function giveaway(address addr, uint256 amount) public onlyOwner {
        require(totalSupply() + amount <= maxSupply);
    ...
    constructor() ERC721A("Crypt0 D0ge Club", "D0ge") {
        owner = msg.sender;
    }

    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        return string(abi.encodePacked("ipfs://QmUmiSofpCBfxLDGbGdbnyqgmb34EdKKFQyemRNwJ6oAYQ/", _toString(tokenId), ".json"));
    }

    function FreeNum() internal returns (uint256){
        return (maxSupply - totalSupply()) / 10;
    }

    function royaltyInfo(uint256 _tokenId, uint256 _salePrice) public view virtual returns (address, uint256) {
        uint256 royaltyAmount = (_salePrice * 75) / 1000;
