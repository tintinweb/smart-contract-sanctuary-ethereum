...
/// @author Portals.fi
/// @notice This contract adds liquidity to Aave like pools using any ERC20 token or the network token.
...
contract AavePortalIn is PortalBaseV1 {
    ...
    /// @notice Add liquidity to Aave like pools with network tokens/ERC20 tokens
    /// @param sellToken The ERC20 token address to spend (address(0) if network token)
    /// @param sellAmount The quantity of sellToken to Portal in
    /// @param intermediateToken The intermediate token to swap to (must be the underlying token)
    /// @param buyToken The Aave like market address (i.e. the aToken)
    ...
    function portalIn(
        ...
        _approve(intermediateToken, address(LENDING_POOL), amount);
        LENDING_POOL.deposit(intermediateToken, amount, msg.sender, 0);
