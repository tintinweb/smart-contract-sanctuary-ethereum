...
contract Plans is Ownable {
    // Struct for a Plan
    struct Plan {
        uint192 price; // price of the plan in wei
        uint64 duration; // duration of the plan in seconds
        string name; // name of the plan
    }

    // Array of Plan structs
    Plan[] public plans;

    // Adds a new Plan to plans
    function addPlan(uint192 _price, uint64 _duration, string calldata _name) 
        external
        onlyOwner
    {
        // Add new Plan to plans
        plans.push(Plan({price: _price, duration: _duration, name: _name}));
    }
    
    // Update Plan
    function updatePlan(uint _index, uint192 _price, uint64 _duration, string calldata _name) 
        external
        onlyOwner
    {
        Plan storage plan = plans[_index];
        plan.price = _price;
...
contract NiftyBot is Ownable, Plans, ERC721Enumerable, ERC5643 {
    using Strings for uint256;
    ...
    constructor(
        ...
        ERC5643("NIFTYBOT.XYZ", "NIFTYBOT")
    {}


    /// Enable/discable contract
    bool paused = false;
    /// Base URI
    string baseURI = "https:///niftybot.xyz/api/token/";
    /// Map token id to subscription plan
    mapping(uint256 => string) public tokenPlan;


    /**
    * Mint activation pass and start a 
    * new subscription for that token
    */
        ...
        payable
    {
        ...
        /// Map token to supplied plan
        tokenPlan[tokenId] = _plan;

        /// Get Plan from tokenId
        Plan memory plan = getPlan(tokenId);

        /// Price must match that of the Plan
        require(msg.value == plan.price, "Not enough ETH provided.");

        /// Mint token
        _mint(to, tokenId);

        /// Subscribe
        _extendSubscription(tokenId, plan.duration);
    }
    ...
    /**
    * Get the renewal price based on the Plan
    * the tokenId is subscriped to
    */
    function _getRenewalPrice(
        ...
        returns (uint256)
    {
        /// Get Plan struct based on tokenId
        Plan memory activePlan = getPlan(tokenId);

        /// Finally, return the renewal price
        return activePlan.price;
    }

    /// Plan index lookup by name
    function _getPlanIndexByName(string memory _plan) 
        public 
        view 
        returns(uint)
    {
        bytes32 encodedElement = keccak256(abi.encode(_plan));
        for (uint i = 0 ; i < getPlansLength(); i++) {
            if (encodedElement == keccak256(abi.encode(plans[i].name))) {
                return i;
            }
        }
        revert("The plan provided is invalid");
    }

    /// Plan (struct) lookup by tokenId
    function getPlan(uint256 _tokenId) 
        public 
        view 
        returns(Plan memory)
    {
        /// Plan name of the tokenId
        string memory _plan = tokenPlan[_tokenId];

        /// Index of the Plan
        uint _index = _getPlanIndexByName(_plan);

        /// The actual Plan as a struct
        Plan storage plan = plans[_index];
    ...
    /**
    * tokenURI concats the baseURI with the tokenPlan
    * and the tokenId
    */
        ...
        returns (string memory)
    {
        return string.concat(string.concat(
            baseURI, 
            string.concat(tokenPlan[tokenId], "/")
        ), tokenId.toString());
    }
    ...
    function supportsInterface(bytes4 interfaceId)
        public
        view
        override(ERC5643, ERC721Enumerable)
        returns (bool)
    {
    ...
    /// Set the min renewal duration
    function setMinimumRenewalDuration(uint64 duration) 
        external 
        onlyOwner 
    {
    ...
    /// Set the max renewal duration
    function setMaximumRenewalDuration(uint64 duration) 
        external 
        onlyOwner 
    {
