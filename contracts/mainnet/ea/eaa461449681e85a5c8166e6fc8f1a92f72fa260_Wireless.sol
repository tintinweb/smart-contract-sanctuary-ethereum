...
contract Wireless is Ownable, DefaultOperatorFilterer, ERC721A {

    uint256 public walletMax = 3;
    using Strings for uint256;

    mapping(uint256 => string) private _tokenURIs;
    bool public publicSaleOpen = false;
    string public baseURI = "ipfs://updatejson/";
    string public _extension = ".json";
    uint256 public price = 0 ether;
    uint256 public maxSupply = 5210;

    constructor() ERC721A("Wireless", "WLESS"){}
    
    function mintNFT(uint256 _quantity) public payable {
        require(_quantity > 0 && _quantity <= walletMax, "Wallet full! Too much wire!");
        require(totalSupply() + _quantity <= maxSupply, "Fully Wirelessed!");
        require(msg.value == price * _quantity, "Not enough ETH!");
        require(getMintedCount(msg.sender) + _quantity <= walletMax, "Check max per wire!");
    ...
    function sendGifts(address[] memory _wallets) external onlyOwner{
        require(totalSupply() + _wallets.length <= maxSupply, "Reached Max Supply.");
        for(uint i = 0; i < _wallets.length; i++)
            _safeMint(_wallets[i], 1);

    }
   function sendGiftsToWallet(address _wallet, uint256 _num) external onlyOwner{
               require(totalSupply() + _num <= maxSupply, "Reached Max Supply.");
