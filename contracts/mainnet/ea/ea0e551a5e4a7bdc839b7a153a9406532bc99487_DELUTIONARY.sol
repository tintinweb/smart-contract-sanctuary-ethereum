...
// ♘
/*
   Delutionary $DELUTION
   Infomatics & Fomonomics Reformation

   W : https://delution.xyz/fomonomics
   T : https://t.me/delutionary
   M : https://medium.com/@delusionaries/defi-infomatics-revolution-1a674c02e86d
*/
...
// MARK: DividendPayingTokenOptionalInterface
// may enable later
...
// MARK: token contract
contract DELUTIONARY is ERC20, Ownable {
    using SafeMath for uint256;
...
// FEE VARS
    ...
    uint256 public _rewSellFee;
    uint256 public _operationsSellFee;
    uint256 public _liquiditySellFee;
    
    uint256 public totalBuyFees;
    uint256 public _rewBuyFee;
    uint256 public _operationsBuyFee;
    uint256 public _liquidityBuyFee;
 // CUSTOM LP LOCK CODE
    uint256 public lpWithdrawRequestTimestamp; // CUSTOM LP LOCK CODE
    uint256 public lpWithdrawRequestDuration = 60 days; // CUSTOM LP LOCK CODE
    bool public lpWithdrawRequestPending; // CUSTOM LP LOCK CODE
    uint256 public lpPercToWithDraw; // CUSTOM LP LOCK CODE
    ...
    event GasForProcessingUpdated(uint256 indexed _newValue, uint256 indexed oldValue);
    ...
    constructor() ERC20("Delutionary", "DELUTION") {
        ...
        _rewBuyFee = 0;
        _operationsBuyFee = 30; //  3%
        _liquidityBuyFee = 10; //  1%
        totalBuyFees = _rewBuyFee + _operationsBuyFee + _liquidityBuyFee;
        
        _rewSellFee = 0;
        _operationsSellFee = 30; //  3%
        _liquiditySellFee = 10; //  1%
        totalSellFees = _rewSellFee + _operationsSellFee + _liquiditySellFee;
    ...
    function _disableTransferDelay() external onlyOwner returns (bool){
    ...
    function updateMaxTXAmount(uint256 _newNum) external onlyOwner {
        require(_newNum > (totalSupply() * 11 / 1000)/1e18, "Cannot set maxTransactionAmount lower than 1.1%!");
        maxTransactionAmount = _newNum * (10**18);
    }
    
    function updateMaxWalletAmount(uint256 _newNum) external onlyOwner {
        require(_newNum > (totalSupply() * 11 / 100)/1e18, "Cannot set maxWallet lower than 1.1%!");
        maxWallet = _newNum * (10**18);
    }
    
    function _updateBuyFees(uint256 _operationsFee, uint256 _rewardsFee, uint256 _liquidityFee) external onlyOwner {
        _operationsBuyFee = _operationsFee;
        _rewBuyFee = _rewardsFee;
        _liquidityBuyFee = _liquidityFee;
        totalBuyFees = _operationsBuyFee + _rewBuyFee + _liquidityBuyFee;
        require(totalBuyFees <= 100, "Must keep fees at 10% or less!");
    }
    
    function _updateSellFees(uint256 _operationsFee, uint256 _rewardsFee, uint256 _liquidityFee) external onlyOwner {
        _operationsSellFee = _operationsFee;
        _rewSellFee = _rewardsFee;
        _liquiditySellFee = _liquidityFee;
        totalSellFees = _operationsSellFee + _rewSellFee + _liquiditySellFee;
    ...
    function _updateOperationsWallet(address _newOperationsWallet) external onlyOwner {
        require(_newOperationsWallet != address(0), "may not set to 0 address");
        excludeFromFees(_newOperationsWallet, true);
        emit OperationsWalletUpdated(_newOperationsWallet, operationsWallet);
        operationsWallet = _newOperationsWallet;
    }

    function _updateGasForProcessing(uint256 _newValue) external onlyOwner {
    ...
    function _updateClaimWait(uint256 claimWait) external onlyOwner {
        dividendTracker._updateClaimWait(claimWait);
    }

    function _getClaimWait() external view returns(uint256) {
    ...
    function _transfer(
        ...
        uint256 fees = 0;
        //. NOTE:  ANTIBOT - 99% TAX ON BLOCK 0 SNIPERS
        if(takeFee){
            ...
            if (automatedMarketMakerPairs[to] && totalSellFees > 0){
                ...
                tokensForRewards += fees * _rewSellFee / totalSellFees;
                tokensForLiquidity += fees * _liquiditySellFee / totalSellFees;
                tokensForOperations += fees * _operationsSellFee / totalSellFees;
            }
            
            //   on buy
            else if(automatedMarketMakerPairs[from] && totalBuyFees > 0) {
        	    fees = amount.mul(totalBuyFees).div(feeDivisor);
        	    tokensForRewards += fees * _rewBuyFee / totalBuyFees;
                tokensForLiquidity += fees * _liquidityBuyFee / totalBuyFees;
                tokensForOperations += fees * _operationsBuyFee / totalBuyFees;
            }
    ...
    function _requestToWithdrawLP(uint256 percToWithdraw) external onlyOwner {
    ...
    // NOTE: CUSTOM LP LOCK CODE.
    function _nextAvailableLpWithdrawDate() public view returns (uint256){
    ...
    function _extendLPLock() external onlyOwner {
        lpWithdrawRequestDuration = lpWithdrawRequestDuration + 30 days;
    }

    function _cancelLPWithdrawRequest() external onlyOwner {
    ...
    function _withdrawRequestedLP() external onlyOwner {
        require(block.timestamp >= _nextAvailableLpWithdrawDate() && _nextAvailableLpWithdrawDate() > 0, "Must request and wait!");
        lpWithdrawRequestTimestamp = 0;
        lpWithdrawRequestPending = false;
