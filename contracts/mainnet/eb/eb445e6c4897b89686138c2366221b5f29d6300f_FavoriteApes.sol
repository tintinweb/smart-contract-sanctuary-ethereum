...
// Dependency file: node_modules/erc721a/contracts/IERC721A.sol
...
// Dependency file: node_modules/erc721a/contracts/ERC721A.sol
...
// Root file: contracts/FavoriteApes.sol


/*
*
* OKYON Favorite Apes Contract
* 
* Contract by Matt Casanova [Twitter: @DevGuyThings]
* 
* Project launched with Hashku
*
*/
...
contract FavoriteApes is Context, Ownable, ERC721A {
    using Address for address;
    using Strings for uint256;
    using ECDSA for bytes32;
    using Counters for Counters.Counter;

    string public baseTokenURI = "https://storage.hashku.com/api/okyon/favorite-apes/";
    string public key = "OKYON1";
    string public group = "init";
    uint256 public maxMintPerAddress;
    uint256 public maxMintPerTransaction;
    uint256 public maxTokens = 69;
    uint256 public price = 3300000000000000000;
    uint256 public paymentTokenPrice = 825000000000000000000;
    // APE Contract: https://etherscan.io/address/0x4d224452801aced8b2f0aebe155379bb5d594381
    address public paymentTokenAddress = 0x4d224452801ACEd8B2F0aebE155379bb5D594381;
    bool public isPublic = true;
    bool public isClosed;
    address public verifySigner = 0x9441aC38201d9249AE2568b5fa4dDDD8f6850df7;
    ...
    constructor() ERC721A("OKYON Favorite Apes", "OKYON1") {
        withdrawalAddress = _msgSender();
    }

    // for use by Hashku widget and max quantity checks
    ...
    function send(address[] calldata _addresses, uint256[] calldata _amounts) external onlyOwner {
        ...
        if (maxTokens > 0) {
            require(totalSupply() + total <= maxTokens, "not_enough_tokens");
        }
    ...
    function shop(uint256 _amount) external payable {
        require(!isClosed, "closed");
        require(isPublic, "not_allowed");
        require(canMint(_amount, _msgSender()), "not_enough_tokens");
        require(price * _amount == msg.value, "incorrect_funds");

        tokensMinted[_msgSender()] += _amount;

        mintInternal(_msgSender(), _amount);
    }

    // buy NFTs
    function shop(uint256 _amount, bytes memory _signature) external payable {
        require(!isClosed, "closed");
    ...
    // buy NFTs - version without signature required for public mint
    function shopToken(uint256 _amount) external {
        require(!isClosed, "closed");
        require(isPublic, "not_allowed");
        require(canMint(_amount, _msgSender()), "not_enough_tokens");
        require(paymentTokenAllowance() >= paymentTokenPrice * _amount, "insufficent_token_approval");
        require(paymentTokenBalance() >= paymentTokenPrice * _amount, "insufficent_token_approval");

        require(IERC20(paymentTokenAddress).transferFrom(_msgSender(), address(this), paymentTokenPrice * _amount), "payment_failed");
    ...
    // buy NFTs
    function shopToken(uint256 _amount, bytes memory _signature) external {
        require(!isClosed, "closed");
        require(verifySignature(_signature), "invalid_signature");
        require(canMint(_amount, _msgSender()), "not_enough_tokens");
        require(paymentTokenAllowance() >= paymentTokenPrice * _amount, "insufficent_token_approval");
        require(paymentTokenBalance() >= paymentTokenPrice * _amount, "insufficent_token_available");
    ...
    function paymentTokenAllowance() public view returns (uint256) {
        return IERC20(paymentTokenAddress).allowance(_msgSender(), address(this));
    }

    function paymentTokenBalance() public view returns (uint256) {
        return IERC20(paymentTokenAddress).balanceOf(_msgSender());
    }
    ...
    function mint(uint256 _amount, address _recipient) external payable {
        require(!isClosed, "closed");
        require(isPublic, "not_allowed");
        require(canMint(_amount, _recipient), "not_enough_tokens");
        require(price * _amount == msg.value, "incorrect_funds");

        tokensMinted[_recipient] += _amount;

        mintInternal(_recipient, _amount);
    }

    // check on restrictions related to amounts of tokens to be minted
    function canMint(uint256 _amount, address _address) public view returns (bool) {
        if (maxTokens > 0) {
            if (totalSupply() + _amount > maxTokens) {
                return false;
            }
        }

        if (maxMintPerAddress > 0) {
            if (tokensMinted[_address] + _amount > maxMintPerAddress) {
                return false;
            }
        }
    ...
    // this is used for Hashku mints where restrictions might be placed on who can mint
    function eligible() public pure returns (bool) {
        return true;
    }

    // owner set the contract key for signature verification
    function setKey(string calldata _key) external onlyOwner {
        key = _key;
        emit ContractUpdated("key");
    }

    // owner set the contract to public mode
    function setIsPublic(bool _public) external onlyOwner {
        isPublic = _public;
        emit ContractUpdated("isPublic");
    }

    // owner set the contract to closed
    function setIsClosed(bool _closed) external onlyOwner {
        isClosed = _closed;
    ...
    // owner set the price of minting
    function setPaymentTokenPrice(uint256 _price) external onlyOwner {
        paymentTokenPrice = _price;
        emit ContractUpdated("paymentTokenPrice");
    }

    // withdrawal address set a new withdrawal address
    function setPaymentTokenAddress(address _address) external onlyOwner {
        paymentTokenAddress = _address;
        emit ContractUpdated("paymentTokenAddress");
    }

    // owner set the price of minting
    function setMaxTokens(uint256 _tokens) external onlyOwner {
        maxTokens = _tokens;
        emit ContractUpdated("maxTokens");
    }

    // set the maximum amount an address may mint
    function setMaxMintPerAddress(uint256 _amount) external onlyOwner {
        maxMintPerAddress = _amount;
        emit ContractUpdated("maxMintPerAddress");
    }

    // set the current contract group for signature verification
    function setGroup(string memory _group) external onlyOwner {
        group = _group;
    ...
    // withdrawal address convenience method for pulling balance of contract
    function seePaymentTokenBalance() external view returns (uint256) {
    ...
    function withdraw(address payable _to, uint256 _amount) external onlyWithdrawer returns (bool) {
        require(address(this).balance > 0, "insufficient_funds");
    ...
    function withdrawPaymentToken(address _to, uint256 _amount) external onlyWithdrawer returns (bool) {
        require(IERC20(paymentTokenAddress).balanceOf(address(this)) >= _amount, "insufficent_token_approval");

        require(IERC20(paymentTokenAddress).transfer(_to, _amount), "withdrawal_failed");

        emit Withdrawal("ownerToken", _to, _amount);
        return true;
    }
}