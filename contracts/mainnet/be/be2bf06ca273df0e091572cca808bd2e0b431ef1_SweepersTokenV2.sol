...
interface ISweepersToken is IERC721Upgradeable{
    ...
    function migrate(uint256 sweeperId) external;

    function getMigratable(address owner) external view returns (uint256[] memory migratable);

    function migrateManyByOwner(uint256[] calldata sweeperIds) external;
