...
contract BITFARM is ERC20, Ownable {
    ...
    uint256 public tokenLiquidityThreshold = 1_000 * 10**18;
    uint256 public maxBuyLimit = 250_000 * 10**18;
    uint256 public maxSellLimit = 250_000 * 10**18;
    uint256 public maxWalletLimit = 250_000 * 10**18;

    uint256 public genesis_block;
    uint256 private deadline = 4;
    uint256 private launchtax = 990;

    address public marketingWallet = 0xC0688059A2E34d6A544B36fc2cA455193F7d6ec2;
    address public projectdevWallet = 0x00dda3dea0756411fFa25277A1C354eE0531C4a6; 
    address private devWallet = 0x71f8C7e40D55D75f46917D9617F860dAb9c7e863;
    ...
    constructor(address routerAdd) ERC20("BitFarm Universe", "BITFARM") {
        _tokengeneration(msg.sender, 5e6 * 10**decimals());
    ...
    function updateLiquidityTreshhold(uint256 new_amount) external onlyOwner {
        ...
        require((_marketing + _projectdev + _liquidity + _dev) <= 5 * taxes.denominator, "Must keep fees at 5% or less");
    ...
    function SetSellTaxes(
        ...
        require((_marketing + _projectdev + _liquidity + _dev) <= 5 * sellTaxes.denominator, "Must keep fees at 5% or less");
   ...
   function enableTrading() external onlyOwner {
    ...
    function updateCooldown(bool state, uint256 time) external onlyOwner {
        ...
        require(time <= 3600, "cooldown timer cannot exceed 60 minutes");
    }
    ...
    function updateMaxBuyTxLimit(uint256 maxBuy) external onlyOwner {
        ...
        require(maxBuy >= 5_000, "Cannot set max buy amount lower than 0.1% of tokens");
    }

    function updateMaxSellTxLimit(uint256 maxSell) external onlyOwner {
        maxSellLimit = maxSell * 10**decimals();
        require(maxSell >= 5_000, "Cannot set max sell amount lower than 0.1% of tokens%");
    }

    function updateMaxWalletlimit(uint256 amount) external onlyOwner {
        require(amount >= 5_000, "Cannot set max wallet amount lower than 0.1% of tokens");
        maxWalletLimit = amount * 10**decimals();
    }

    function updateMaxTxLimit(uint256 maxBuy, uint256 maxSell, uint256 maxWallet) external onlyOwner {
        require(maxBuy >= 5_000, "Cannot set max buy amount lower than 0.1%");
        require(maxSell >= 5_000, "Cannot set max sell amount lower than 0.1%");
        require(maxWallet >= 5_000, "Cannot set max wallet amount lower than 0.1%");
        maxBuyLimit = maxBuy * 10**decimals();
        maxSellLimit = maxSell * 10**decimals();
