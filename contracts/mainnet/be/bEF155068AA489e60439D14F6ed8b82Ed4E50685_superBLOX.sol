...
/**
 * @dev Based on Azuki's awesome implementation of https://eips.ethereum.org/EIPS/eip-721[ERC721] Non-Fungible Token Standard, including
 ...
 */
contract ERC721AS is Context, ERC165, IERC721, IERC721Metadata, IERC721Enumerable {
   ...
   // Token name
    ...
    // Mapping owner address to address data (replaces _balences)
    ...
    function tokenOfOwnerByIndex(address owner, uint256 index) public view override returns (uint256) {
        require(index < balanceOf(owner), 'ERC721AS: owner index out of bounds');
    ...
    function balanceOf(address owner) public view override returns (uint256) {
        require(owner != address(0), 'ERC721AS: balance query for the zero address');
        return uint256(_addressData[owner].balance);
    }

    function _numberMinted(address owner) internal view returns (uint256) {
        require(owner != address(0), 'ERC721AS: number minted query for the zero address');
 ...
 function ownershipOf(uint256 tokenId)
    ...
    returns (address)
  {
    ...
    for (uint256 curr = tokenId; curr >= lowestTokenToCheck; curr--) {
        address tokenOwner=_owners[curr];
   ...
   /**
    ...
    function getApproved(uint256 tokenId) public view override returns (address) {
        require(_exists(tokenId), 'ERC721AS: approved query for nonexistent token');
    ...
    function setApprovalForAll(address operator, bool approved) public override {
        // from OZ _setApprovalForAll
        require(_msgSender() != operator , 'ERC721AS: approve to caller');
   ...
   /**
    ...
    function _mint(
        ...
        require(quantity != 0, 'ERC721AS: quantity must be greater than 0');
    ...
    function _transfer(
        ...
        address previousOwner = ERC721AS.ownerOf(tokenId);
        require(previousOwner == from, "ERC721AS: transfer from incorrect owner");
        require(to != address(0), 'ERC721AS: transfer to the zero address');

        _beforeTokenTransfers(from, to, tokenId, 1);
        // Clear approvals from the previous owner
        _approve(address(0), tokenId, previousOwner);
        ...
        unchecked {
            ...
            if (_owners[nextTokenId] == address(0)) {
                if (_exists(nextTokenId)) {
                    _owners[nextTokenId] = previousOwner;
                }
            }
        }
    ...
    function _checkOnERC721Received(
        ...
        if (to.isContract()) {
            try IERC721Receiver(to).onERC721Received(_msgSender(), from, tokenId, _data) returns (bytes4 retval) {
                ...
                if (reason.length == 0) {
                    revert('ERC721AS: transfer to non ERC721Receiver implementer');
                } else {
                    assembly {
