...
// File contracts/ScrollPunks.sol
pragma solidity 0.8.4;

contract ScrollPunks is NFTokenEnumerableMetadata, Ownable {
	using SafeMath for uint256;

	uint256 public price = 0.02 ether;
	uint256 public maxSupply = 10000;
	string public baseUri = 'https://ipfs.io/ipfs/QmPnsBsgVefAiefjwUgjMeWtgRnVMYUfC73Y53TYbmAyCT/';
	uint16 public refShare = 1000; // in basis points, so it's 10%
	uint256 public startTime = 5000000000;
	uint256 public premint = 0;
	uint256 public maxMintCountPerTx = 5;
	bool public enableGifts = true;
	...
	function _mintTo(address _user, uint value) internal {
        uint amount = value / price;

        require(amount > 0 && amount <= maxMintCountPerTx);
        require(this.totalSupply() + amount <= maxSupply, 'Sold out');
	...
	function mintTo(address _user) external payable {
		require(msg.sender == owner() || enableGifts);
	    require(msg.sender == owner() || block.timestamp >= startTime);

		_mintTo(_user, msg.value);
	}
	...
	function setEnableGifts(bool _enableGifts) external onlyOwner {
