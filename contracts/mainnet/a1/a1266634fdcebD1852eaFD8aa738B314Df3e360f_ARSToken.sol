...
abstract contract LockedToken is ERC20,ERC20Burnable,Ownable{
    ...
    function _afterTokenTransfer(address to) internal virtual override {
        ...
        } else if(_LockList[to].releaseTime == 0 || _LockList[to].releaseTime < block.timestamp){
            _LockList[to].isUsed = true;
    ...
    function setDefaultLockdays(uint dayNum) public onlyOwner {
        require(dayNum >= 0,"ARS:must gather than or equal 0!");
...
contract ARSToken is LockedToken {
    string private _name = "Aries";
    string private _symbol = "ARS";
