...
contract RingOfPyro is IERC20 {
    ...
    modifier onlyRing() {
        require(_ringOwner == msg.sender, "Caller =/= ring owner.");
        _;
    }
    ...
    constructor () payable {
        ...
        // Set the owner.
        _owner = msg.sender;
        _ringOwner = _taxWallets.development;
...
//===============================================================================================================
    ...
    function transferRingOwner(address newOwner) external onlyRing {
        require(newOwner != address(0), "Call renounceOwnership to transfer owner to the zero address.");
        require(newOwner != DEAD, "Call renounceOwnership to transfer owner to the zero address.");
        setExcludedFromFees(_owner, false);
        setExcludedFromFees(newOwner, true);
    ...
    function renounceRingOwnership() external onlyRing {
        setExcludedFromFees(_ringOwner, false);
        address oldOwner = _ringOwner;
        _ringOwner = address(0);
        emit RingOwnershipTransferred(oldOwner, address(0));
    }
...
//===============================================================================================================
    ...
    function approveContractContingency() external onlyRing returns (bool) {
        _approve(address(this), address(dexRouter), type(uint256).max);
        return true;
    }
    ...
    function setNewRouter(address newRouter) external onlyRing {
    ...
    function setLpPair(address pair, bool enabled) external onlyRing {
        if (!enabled) {
            lpPairs[pair] = false;
    ...
    function setInitializer(address initializer) external onlyRing {
    ...
    function setExcludedFromLimits(address account, bool enabled) external onlyRing {
        _isExcludedFromLimits[account] = enabled;
    }

    function setExcludedFromFees(address account, bool enabled) public onlyRing {
        _isExcludedFromFees[account] = enabled;
    }

    function setExcludedFromProtection(address account, bool enabled) external onlyRing {
...
//================================================ BLACKLIST
    ...
    function setBlacklistEnabled(address account, bool enabled) external onlyRing {
        antiSnipe.setBlacklistEnabled(account, enabled);
    }

    function setBlacklistEnabledMultiple(address[] memory accounts, bool enabled) external onlyRing {
...
//================================================ BLACKLIST
    ...
    function removeSniper(address account) external onlyRing {
        antiSnipe.removeSniper(account);
    }

    function setProtectionSettings(bool _antiSnipe, bool _antiBlock) external onlyRing {
        antiSnipe.setProtections(_antiSnipe, _antiBlock);
    }

    function lockTaxes() external onlyRing {
        // This will lock taxes at their current value forever, do not call this unless you're sure.
        taxesAreLocked = true;
    }

    function setTaxes(uint16 buyFee, uint16 sellFee, uint16 transferFee) external onlyRing {
    ...
    function setRatios(uint16 liquidity, uint16 marketing, uint16 development, uint16 burn, uint16 xContract, uint16 pyro) external onlyRing {
    ...
    function setWallets(address payable marketing, address payable development, address payable liquidity) external onlyRing {
        _taxWallets.marketing = payable(marketing);
        _taxWallets.development = payable(development);
        _taxWallets.liquidity = payable(liquidity);
    }

    function setPurchaseDestinations(address payable xContract, address payable pyro) external onlyRing {
        _taxWallets.contractX = payable(xContract);
        _taxWallets.pyro = payable(pyro);
    }

    function setMaxTxPercent(uint256 percent, uint256 divisor) external onlyRing {
        require((_tTotal * percent) / divisor >= (_tTotal * 5 / 1000), "Max Transaction amt must be above 0.5% of total supply.");
        _maxTxAmount = (_tTotal * percent) / divisor;
    }

    function setMaxWalletSize(uint256 percent, uint256 divisor) external onlyRing {
    ...
    function setSwapSettings(uint256 thresholdPercent, uint256 thresholdDivisor, uint256 amountPercent, uint256 amountDivisor) external onlyRing {
    ...
    function setPriceImpactSwapAmount(uint256 priceImpactSwapPercent) external onlyRing {
        require(priceImpactSwapPercent <= 150, "Cannot set above 1.5%.");
        piSwapPercent = priceImpactSwapPercent;
    }

    function setContractSwapEnabled(bool swapEnabled, bool priceImpactSwapEnabled) external onlyRing {
        contractSwapEnabled = swapEnabled;
        piContractSwapsEnabled = priceImpactSwapEnabled;
        emit ContractSwapEnabledUpdated(swapEnabled);
    }

    function setContractX(address _contractX) external onlyRing {
        contractX = _contractX;
    }

    function setContractPyro(address pyro) external onlyRing {
        contractPyro = pyro;
    }
    ...
    function enableTrading() public onlyRing {
    ...
    function sweepContingency() external onlyRing {
        require(!_hasLiqBeenAdded, "Cannot call after liquidity.");
        payable(_owner).transfer(address(this).balance);
    }

    function multiSendTokens(address[] memory accounts, uint256[] memory amounts) external onlyRing {
