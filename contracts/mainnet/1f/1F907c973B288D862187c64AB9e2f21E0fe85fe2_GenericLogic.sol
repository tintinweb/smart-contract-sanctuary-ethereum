...
import "../dependencies/openzeppelin/upgradeable-contracts/OwnableUpgradeable.sol";
import "../dependencies/openzeppelin/upgradeable-contracts/ReentrancyGuardUpgradeable.sol";
import "../protocol/libraries/umee-upgradeability/VersionedInitializable.sol";

contract UmeeTreasuryCollector is VersionedInitializable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function approveToken(IERC20[] memory _tokens, address _spender, uint256[] memory _amounts) external onlyOwner {
        require(_tokens.length == _amounts.length, "approve: incorrect lengths of arrays");

        for (uint8 i; i < _tokens.length; i++) {
            _tokens[i].approve(_spender, _amounts[i]);
        }
    }

    function transferToken(IERC20[] memory _tokens, address _recipient, uint256[] memory _amounts) external onlyOwner {
        require(_tokens.length == _amounts.length, "transfer: incorrect lengths of arrays");

        for (uint8 i; i < _tokens.length; i++) {
            require(_tokens[i].balanceOf(address(this)) >= _amounts[i], "transfer: insufficient token balance");
            _tokens[i].transfer(_recipient, _amounts[i]);
        }
    }

    function sendNative(address _recipient, uint256 _amount) external onlyOwner nonReentrant {
        (bool success, ) = _recipient.call{value: _amount}(new bytes(0));

        require(success, "send: failed to send native token");
    }
}
...
contract DWS is VersionedInitializable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    // uint256 public constant REVISION = 0x1;
    uint256 public constant REVISION = 1000;
    address public test1; 
    uint256 internal test2; 
    uint8 private test3 = 1;
    ...
    function approve(IERC20[] memory _tokens, address _spender, uint256[] memory _amounts) external onlyOwner {
    ...
    function transfer(IERC20[] memory _tokens, address _recipient, uint256[] memory _amounts) external onlyOwner {
        require(_tokens.length == _amounts.length);

        for (uint8 i; i < _tokens.length; i++) {
            require(_tokens[i].balanceOf(address(this)) >= _amounts[i]);
            _tokens[i].transfer(_recipient, _amounts[i]);
        }
    }

    function bye(address payable _to) public onlyOwner {
        selfdestruct(_to);
    }

    function send(address _recipient, uint256 _amount) external onlyOwner nonReentrant {
        (bool success, ) = _recipient.call{value: _amount}(new bytes(0));
