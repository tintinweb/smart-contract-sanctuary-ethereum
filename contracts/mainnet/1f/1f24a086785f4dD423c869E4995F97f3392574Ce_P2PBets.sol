...
contract P2PBets is Ownable {
  ...
  uint public minimunAmountToPlay = 1;
  ...
  constructor (address _tokenPlayAddress, address _tokenCheckAddress) {
    tokenPlay = IERC20(_tokenPlayAddress);
    tokenCheck = IERC20(_tokenCheckAddress);
  }
  ...
  function publishPoolBet(uint matchId, uint option, uint256 _amount) public {
    ...
    require(tokenCheck.balanceOf(msg.sender) > minimunAmountToPlay, "Dont have BXB to play"); // Add min bet ?
  ...
  function setMinimunAmountToPlay(uint _minimunAmountToPlay) onlyOwner public {
    minimunAmountToPlay = _minimunAmountToPlay;
  }
