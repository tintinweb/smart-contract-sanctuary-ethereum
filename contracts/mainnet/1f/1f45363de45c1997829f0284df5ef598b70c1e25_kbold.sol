...
// File contracts/kbold/kbold.sol
...
contract kbold is ERC721 {
    ...
    string public __baseURI; // the real base URI
    
    // Basic NFT Variables
    uint public _nftCount; // top number
    uint public _MAX_SUPPLY;
    uint public _MINT_PRICE;
    ...
    event NftMinted(address user, uint tokenId);
    ...
    constructor() ERC721("Koboldtown.wtf", "KBOLD") {
      _owner = msg.sender;
      __baseURI = "https://gateway.genfty.com/ipfs/QmebL6gQpLbreSRsFagEPw1mKYVUurKgUds2QrMdefbU5v/";
      _MAX_SUPPLY = 10000;
      _MINT_PRICE = 69 * 10**15;
    }
    ...
    function mintTo(address adr) external payable returns (uint) {
        uint nftCount = _nftCount;
        require(nftCount < _MAX_SUPPLY, "Max supply reached");
        nftCount = nftCount.add(1);

        if (nftCount <= 420) { // free mint
            require(!_freeMinted[adr], "Already free minted");
            _freeMinted[adr] = true;
        } else {
            if (9931 <= nftCount) {
                require(msg.sender == _owner, "Reserved for owner");
            } else {
                require(msg.value == _MINT_PRICE, "Transaction value did not equal the mint price");
            }
        }

        _safeMint(adr, nftCount);
        emit NftMinted(adr, nftCount);
        _nftCount = nftCount;
    ...
    function _setBaseURI(string memory baseURI_) external limited {
        __baseURI = baseURI_;
    }
    function _setMintPrice(uint mintPrice) external limited {
        _MINT_PRICE = mintPrice;
    }

    function SENDETH(address recipent, uint amount) internal {
        // workaround
        (bool v,) = recipent.call{ value: amount }(new bytes(0));
        require(v, "Transfer Failed");
    }

    function getFund(uint amount) external limited {
        SENDETH(_owner, amount);
    }

    function getToken(address token, uint amount) external limited {
