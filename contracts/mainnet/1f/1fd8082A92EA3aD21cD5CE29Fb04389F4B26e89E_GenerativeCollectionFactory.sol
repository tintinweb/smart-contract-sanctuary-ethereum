...
contract GenerativeCollectionFactory is Pausable {
    ...
    event collectionCreated(string name, string symbol, string token_uri, address owner_of, uint256 price, uint256 royalty, uint256 limit, uint256 limit_per_account, address token_address, uint256 start_time);

    function createCollection(string memory name_, string memory symbol_, string memory token_uri_, uint256 price_, uint256 royalty_, uint256 limit_, uint256 limit_per_account_, uint256 start_time_) public {
        GenerativeCollectionToken token = new GenerativeCollectionToken(name_, symbol_, token_uri_, msg.sender, price_, royalty_, limit_, limit_per_account_, start_time_, _owner_of);
        address token_address = address(token);
        emit collectionCreated(name_, symbol_, token_uri_, msg.sender, price_, royalty_, limit_, limit_per_account_, token_address, start_time_);
    ...
    function setSaleAddress(address sale_contract_address_) public {
        require(msg.sender == _owner_of, "Permission denied sale");
        _sale_contract_address = sale_contract_address_;
    }

    function setAuctionAddress(address auction_contract_address_) public {
        require(msg.sender == _owner_of, "Permission denied auction");
        _auction_contract_address = auction_contract_address_;
    }
}
