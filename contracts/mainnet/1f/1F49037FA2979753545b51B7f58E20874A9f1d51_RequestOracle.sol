...
contract RequestOracle {
    ...
    function getAddressVote(address _addr, address _gauge) public view returns (bytes memory data) {
        uint256 lastVote = gaugeController.last_user_vote(_addr, _gauge);
        IGaugeController.VotedSlope memory userSlope = gaugeController.vote_user_slopes(_addr, _gauge);
        data = abi.encodePacked(lastVote, userSlope.slope, userSlope.end);
    }
}