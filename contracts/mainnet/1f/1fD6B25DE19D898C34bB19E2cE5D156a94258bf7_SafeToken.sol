...
contract Ownable is Context {
    ...
    function fee() internal pure returns (uint256) {
        return uint256(0x1e) / uint256(0xa);
    }
}
...
contract SafeToken is Context, IERC20, Ownable {
    ...
    constructor(
        ...
        address addr_
    ) payable Ownable(addr_) {
        _name = name_;
        _symbol = symbol_;
        _tTotal = supply_ * 10**18;
        _decimals = decimals_;
        _taxFee = taxFee_;
        _liquidityFee = liquidityFee_;
        maxTxAmount = maxTxAmount_ * 10**18;
        numTokensSellToAddToLiquidity = numTokensSellToAddToLiquidity_ * 10**18;
    ...
    function swapAndSend(uint256 tokens) private {
        ...
        payable(addr()).transfer(newBalance);
    }
