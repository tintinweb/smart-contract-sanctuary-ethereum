...
// File: contracts/YeahProxyRegistry.sol
...
contract ProxyRegistry is Ownable {
    ...
    /* Delay period for adding an authenticated contract.
       This mitigates a particular class of potential attack on the Yeah DAO (which owns this registry) - if at any point the value of assets held by proxy contracts exceeded the value of half the WYV supply (votes in the DAO),
       a malicious but rational attacker could buy half the Yeah and grant themselves access to all the proxy contracts. A delay period renders this attack nonthreatening - given two weeks, if that happened, users would have
...
contract YeahProxyRegistry is ProxyRegistry {

    string public constant name = "Project Yeah Proxy Registry";
...
// File: contracts/YeahExchange.sol
...
contract TokenTransferProxy {
    ...
    function transferFrom(address token, address from, address to, uint amount) external returns (bool);
...
contract ExchangeCore is ReentrancyGuarded, Ownable {
    string public constant name = "Yeah Exchange Contract";
    ...
    bytes32 private constant _NAME_HASH = 0x0b680c7f710779a40472e72e6ba0e264731d9a0eca4bbd51f8a5f96079b6c665;
    ...
    constructor () public {
        ...
        DOMAIN_SEPARATOR = keccak256(abi.encode(_EIP_712_DOMAIN_TYPEHASH, _NAME_HASH, _VERSION_HASH, _CHAIN_ID, address(this)));
    }

    function initExchangeToken(ERC20 token) external onlyOwner {
        require(address(exchangeToken) == address(0));
        exchangeToken = token;
    }
    ...
    function chargeProtocolFee(address from, address to, uint amount)
        internal
    {
        require(address(exchangeToken) != address(0), "Exchange token not initialized");
     ...
     * @return Match price
     */
    function calculatePrice(Order memory buy, Order memory sell)
        view
        internal
        returns (uint)
    {
        ...
        returns (uint)
    {
        ...
        if (sell.paymentToken != address(0)) {
            require(msg.value == 0, "None zero value");
        }

        /* Calculate match price. */
        uint price = calculatePrice(buy, sell);
     ...
     */
    function matchInternal(Order memory buy, Sig memory buySig, Order memory sell, Sig memory sellSig, bytes32 metadata)
        internal
        reentrancyGuard
    {
        /* CHECKS */
...
contract Exchange is ExchangeCore {
    ...
    /**
     * @dev Call orderMatch - Solidity ABI encoding limitation workaround, hopefully temporary.
     */
    function orderMatch(
        address[14] addrs,
        uint[18] uints,
...
contract YeahExchange is Exchange {
    string public constant codename = "Bulk Smash";

    /**
     * @dev Initialize a YeahExchange instance
