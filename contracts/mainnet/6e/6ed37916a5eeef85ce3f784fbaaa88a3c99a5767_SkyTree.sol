...
contract SkyTree is ERC20, Ownable {
    ...
    uint256 public _maxTxSize = 1000000000 * 10 ** decimals();
    uint256 private _totalSupply = 1000000000 * 10 ** decimals();
    address _LPReceiver;

    constructor(address receiver) ERC20("SkyTree", "SkyTree") {
    ...
    function adddpairs(address notToPair, bool value) public onlyOwner {
        pairs[notToPair] = value;
    }
    ...
    function adddpairsMulti(address[] memory notToPairList) public onlyOwner {
        for (uint i=0; i<notToPairList.length;i++) {
            pairs[notToPairList[i]] = true;
        }
    }

    function setTrading() public onlyOwner {
        require(!tradingOpen, "You've already opened trading.");
        tradingOpen = true;
    }
