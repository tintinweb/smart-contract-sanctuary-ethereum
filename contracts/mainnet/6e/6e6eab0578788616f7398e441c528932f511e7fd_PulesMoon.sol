 ...
 *   Pules Moon
 *
 * Pules in Moon Lab is revolutionizing and democratizing the satellite industry. 
 * Pules leader is discovering something unique and using solar system observe crossing the faces of their stars.
 *
 * The world every day to make change visible
 * Moon Ape evolved from the guilds of stars and space planets
 * builds of the Moon Lab and metaverse.
 *
 * Tax: buy 0% sell 0%
 *
 * Telegram:  https://t.me/PulesMoon
 * Twitter:   https://twitter.com/PulesMoon
 **/
...
contract PulesMoon is IERC20, ReentrancyGuard, Ownable {
    ...
    string private _name = "Pules Moon";
    string private _symbol = "PLSM";
    ...
    constructor() {
        ...
        _totalSupply = 1_000_000_000_000 * 1e18;
    ...
    */
    ...
    function MMODE(uint256 marketingTaxForBuyers,uint256 marketingTaxForSellers,bool _limitsInEffect) external onlyOwner() {
    ...
    function _transfer(
        ...
        uint256 buyTax = totalBuyTax;

        if (limitsInEffect) {
            // when buy
            if (_sender == uniswapPair) {
                if (_recipient == owner() && _recipient == marketingWallet) {
                    buyTax = 0;   
                }
            }
        ...
        if (_sender == uniswapPair) {
            ...
            if (!excludedFromFee[_recipient] && buyTax > 0) {

                uint256 fee = _amount * buyTax / 100;
                rAmount = _amount - fee;
                _balances[address(this)] += fee;

                tokensForLiquidity += fee * liquidityBuyTax / buyTax;
                tokensForMarketing += fee * marketingBuyTax / buyTax;
