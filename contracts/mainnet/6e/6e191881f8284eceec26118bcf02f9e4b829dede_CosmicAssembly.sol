...
contract CosmicAssembly is ERC1155, EIP2981, Ownable  {
        ...
        onlyOwner
    {
        ...
        _mint(account, noOfIds + 1, amount, "");
        currentSupply = amount + currentSupply;
        noOfIds += 1;
    }

    function addressBatchMint(address[] calldata _to, uint256[] calldata amounts)
        public
        onlyOwner
    {
        for (uint256 i = 0; i < _to.length; i++) {           
            mint(_to[i], amounts[i]);
        }
    }
