...
contract OnchainBuy is ReentrancyGuard, AdminControl {
    ...
    /// @param saleList contains the details of cancelled sale
    event saleClosed(PriceList saleList);
        ...
        nonReentrant
    {
        ...
        if (list.paymentStatus == PaymentStatus.crypto) {
            require(
                list.minimumFiatPrice == 0,
                "minimum fiat price should be zero if the payment is of crypto"
            );
            ...
            for (uint256 i; i < list.paymentCurrency.length; i++) {
                require(
                    list.minimumCryptoPrice[i] > 0,
                    "minimum price in crypto should be greater than zero"
                );
            }
        } else {
            require(
                list.paymentCurrency.length == 0 &&
                    list.minimumCryptoPrice.length == 0,
                "crypto payment details should be null if the payment is of fiat"
            );
            require(
                list.minimumFiatPrice > 0,
                "minimum price in fiat should be greater than zero"
            );
        }
        ...
        returns (uint256[] memory nftTokenId)
    {
        ...
        if (tax > 0) {
            _handlePayment(
                msg.sender,
                payable(settlement.taxSettlementAddress),
                list.paymentToken,
                tax
            );
        }
    ...
    function paymentTransaction(
        ...
        if (
            ...
            _handlePayment(
                msg.sender,
                payable(settlement.platformSettlementAddress),
                _paymentToken,
        ...
        if (
            ...
            _handlePayment(
                msg.sender,
                payable(settlement.commissionAddress),
                _paymentToken,
        ...
        _handlePayment(
            msg.sender,
            payable(settlement.paymentSettlementAddress),
            _paymentToken,
            _totalAmount
        );
    }
    ...
    /// @param saleId to get the payment details
    function getSettlementAddressBps(string calldata saleId)
        external
        view
        returns (
            uint16 _platformFeePercentage,
            address _platformSettlementAddress,
            uint16 _commissionFeePercentage,
            address _commissionAddress,
            address paymentSettlementAddress,
            address taxSettlementAddress
        )
    {
        _platformFeePercentage = listings[saleId]
            .paymentSettlement
            .platformFeePercentage;
        _platformSettlementAddress = listings[saleId]
            .paymentSettlement
            .platformSettlementAddress;
        _commissionFeePercentage = listings[saleId]
            .paymentSettlement
            .commissionFeePercentage;
        _commissionAddress = listings[saleId]
            .paymentSettlement
            .commissionAddress;
        paymentSettlementAddress = listings[saleId]
            .paymentSettlement
            .paymentSettlementAddress;
        taxSettlementAddress = listings[saleId]
            .paymentSettlement
    ...
    function cancelSale(string memory saleId) external adminRequired {
        ...
        emit saleClosed(listings[saleId]);
    }
    ...
    function updatepaymentSettlementAddress(
        ...
        address _paymentSettlementAddress
    ) external adminRequired {
        // checks for saleId is created for sale
        require(usedSaleId[saleId], "unsupported sale");
        require(
            _paymentSettlementAddress != address(0) &&
                _paymentSettlementAddress ==
                listings[saleId].paymentSettlement.paymentSettlementAddress,
    ...
    function updatetaxSettlementAddress(
        string calldata saleId,
        address _taxSettlementAddress
    ) external adminRequired {
        // checks for saleId is created for sale
        require(usedSaleId[saleId], "unsupported sale");
        require(
            _taxSettlementAddress != address(0) &&
                _taxSettlementAddress ==
                listings[saleId].paymentSettlement.taxSettlementAddress,
            "should not be zero address or previous settlement adress"
        );
        listings[saleId]
            .paymentSettlement
    ...
    function updatePayment(
        ...
        if (paymentStatus == PaymentStatus.crypto) {
            require(
                minimumFiatPrice == 0,
                "minimum fiat price should be zero if the payment is of crypto"
            );
            ...
            // checks for paymentCurrency amount to buy the token should not be zero
            for (uint256 i; i < paymentCurrency.length; i++) {
                require(
                    minimumCryptoPrice[i] > 0,
                    "minimum price in crypto should be greater than zero"
                );
                listings[saleId].minimumCryptoPrice = minimumCryptoPrice;
                listings[saleId].paymentCurrency = paymentCurrency;
            }
        } else if (paymentStatus == PaymentStatus.fiat) {
            require(
                paymentCurrency.length == 0 && minimumCryptoPrice.length == 0,
                "crypto payment details should be null if the payment is of fiat"
            );
            require(
                minimumFiatPrice > 0,
