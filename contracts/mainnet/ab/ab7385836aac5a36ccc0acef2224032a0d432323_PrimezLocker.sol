...
contract PrimezLocker is Ownable , ILocker{
    ...
    **/
    function lock(address _user, uint256 duration, uint256 amountLock) external onlyOwner {
        ...
        require(!whitelist[_user], "This address has been locked");
    ...
    * duration: (uint256): seconds
    **/
    function transferAndLock(address _user, uint256 duration, uint256 amountLock) external onlyOwner {
        whitelist[_user] = true;
        uint256 startTime = block.timestamp;
        uint256 endTime = startTime + duration;
        LockItem memory lockItem = LockItem({
    ...
    function setSystemLock(bool _systemLock) public onlyOwner {
