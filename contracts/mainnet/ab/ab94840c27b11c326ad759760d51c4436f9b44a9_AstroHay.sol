...
//       _____            __                _________                
//      /  _  \   _______/  |________  ____ \_   ___ \  ______  _  __
//     /  /_\  \ /  ___/\   __\_  __ \/  _ \/    \  \/ /  _ \ \/ \/ /
//    /    |    \\___ \  |  |  |  | \(  <_> )     \___(  <_> )     / 
//    \____|__  /____  > |__|  |__|   \____/ \______  /\____/ \/\_/  
//            \/     \/                             \/               
//                      https://AstroCow.Finance    
...
contract AstroHay is ERC20("AstroCow.Finance Astro Hay", "AHAY"), Ownable {
    ...
    modifier antiBot(address from, address to, uint256 amount) {
        ...
        // After trading time reached
            ...
            if (block.timestamp < restrictionLiftTime) {
                require(amount <= MAX_ALLOWED_TOKENS, 'Amount greater than max limit');
            ...
            if (!_isWhitelisted[from] && !_isWhitelisted[to]) {
                require(_lastTx[from] + requiredDelay <= block.timestamp && _lastTx[to] + requiredDelay <= block.timestamp, 'Only one tx/min in restricted mode!');
            ...
            // If recipient not whitelisted, update data for recipient
                ...
                require(_lastTx[to] + requiredDelay <= block.timestamp, 'Only one tx/min in restricted mode!');
            ...
            // If sender not whitelisted, update data for sender
                ...
                require(_lastTx[from] + requiredDelay <= block.timestamp, 'Only one tx/min in restricted mode!');
    ...
    function setTradingStart(uint256 time) external {
        ...
        require(authorized[msg.sender], "Not authorized");
        tradingTime = time;
        restrictionLiftTime = tradingTime + RESTRICTED_TRANSFER_DELAY;
    }
    ...
    function _beforeTokenTransfer(address from, address to, uint256 amount) internal antiBot(from, to, amount) virtual override {
