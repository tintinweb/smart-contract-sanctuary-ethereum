...
contract InvestmentFactory is Ownable {
    ...
    Investment[] public _listOfInvestments;

    uint256 public amountOfInvestments = 0;

    address public _feeHandlerAddress;
    ...
    function addressIsIncludedInPoolList(address _address) internal view returns (bool) {
        for(uint i = 0; i < _listOfInvestments.length; i++){ 
            if(address(_listOfInvestments[i]) == _address){
                return true;
            }
        }
        return false;
    ...
    function changeFeeHandlerAddress(address _address) public onlyOwner {
        _feeHandlerAddress = _address;
    }

    function getPoolsOwnedByAddress(address _address) public view virtual returns (address[] memory) {
        uint256 amountOfPoolsOwned = 0;
        for(uint i = 0; i < _listOfInvestments.length; i++){
            // IOwnable pool = IOwnable(_listOfInvestments[i]);
            if(IOwnable(address(_listOfInvestments[i])).owner() == _address){
                amountOfPoolsOwned++;
            }
        }
        address[] memory pools = new address[](amountOfPoolsOwned);
        uint256 poolIndex = 0;
        for(uint i = 0; i < _listOfInvestments.length; i++){
            if(IOwnable(address(_listOfInvestments[i])).owner() == _address){
                pools[poolIndex] = address(_listOfInvestments[i]);
                poolIndex++;
            }
        }
    ...
    function createInvestment(string memory name_, string memory symbol_, string memory risk_, uint256 poolFee_) external {
        bytes32 risk = keccak256(bytes(risk_));
        require(risk == keccak256("low") || risk == keccak256("medium") || risk == keccak256("high") || risk == keccak256("very high"), "Invalid risk level!");
        require(poolFee_ < 250000000000000000);
        ...
        _listOfInvestments.push(newInvestment);
        newInvestment.transferOwnership(msg.sender);
        amountOfInvestments++;
        emit poolCreated(msg.sender, address(newInvestment), name_, risk_, poolFee_);
    }
