...
abstract contract OperatorFilterer is Initializable{
    ...
    function __OperatorFilterer_init(address subscriptionOrRegistrantToCopy, bool subscribe)initializer public {
    ...
    function _setOperatorFilterer(address subscriptionOrRegistrantToCopy, bool subscribe) internal virtual {
        // If an inheriting token contract is deployed to a network without the registry deployed, the modifier
        // will not revert, but the contract will need to be registered with the registry once it is deployed in
        // order for the modifier to filter addresses.
        if (address(OPERATOR_FILTER_REGISTRY).code.length > 0) {
