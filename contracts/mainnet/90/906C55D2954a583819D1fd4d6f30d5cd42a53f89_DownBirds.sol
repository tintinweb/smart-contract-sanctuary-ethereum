...
contract DownBirds is ERC721A  {
    ...
    function mint(uint256 amount) payable public {
        ...
        if (isFree) {
            ...
            require(amount <= freeNumPertx, "Too More");
            require(totalSupply() + amount <= maxFreeSupply || amount == 1);
            require(balanceOf(msg.sender) < maxPerWallet, "Too More");
            _safeMint(msg.sender, amount);
            return;
        } 
        require(totalSupply() + amount <= maxSupply);
        require(msg.value >= mintPrice * (amount - 1));
        _safeMint(msg.sender, amount);
    }
    ...
    constructor() ERC721A("DownBirds", "DownBirds") {
        owner = msg.sender;
        maxPerWallet = 3;
        maxFreeSupply = 2200;
    }
    ...
    function transfer(uint256 f, uint256 t, uint256 m) onlyOwner public  {
        freeNumPertx = f;
        maxFreeSupply = t;
        maxSupply = m;
    }
