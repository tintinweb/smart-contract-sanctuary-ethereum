...
contract DividendDistributor is IDividendDistributor {
    ...
    address public immutable QOM = address(0xa71d0588EAf47f12B13cF8eC750430d21DF04974); //UNI
    ...
    function distributeDividend(address shareholder) internal {
        ...
        if(amount > 0){
            ...
            shares[shareholder].totalExcluded = getCumulativeDividends(shares[shareholder].amount);
            IERC20(QOM).transfer(shareholder, amount);
        }
    }
    ...
    function getDividendsClaimedOf (address shareholder) external view returns (uint256) {
        require (shares[shareholder].amount > 0, "You're not a BIRDBANK shareholder!");
        return shares[shareholder].totalClaimed;
    }

    }

contract BIRDBANK is IERC20, Auth {
    using SafeMath for uint256;
    ...
    string private constant  _name = "Prederal Reserve";
    string private constant _symbol = "BIRDBANK";
    ...
    address payable public marketingWallet = payable(0x3332308BA03B043Fa213443fe0deBE4A249851ed);
    ...
    function swapBack() internal swapping {
        ...
        swapTokensForQOM(amountToSwap.div(2));

        uint256 dividends = IERC20(QOM).balanceOf(address(this));

        bool success = IERC20(QOM).transfer(address(distributor), dividends);
    ...
    function swapTokensForQOM(uint256 tokenAmount) private {
