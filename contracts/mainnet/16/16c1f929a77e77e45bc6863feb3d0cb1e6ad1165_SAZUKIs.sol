...
contract SAZUKIs is ERC721A, Ownable {
  ...
  uint256 public maximumAllowedTokensPerWallet = 5555;

  constructor(string memory baseURI) ERC721A("SAZUKIs", "SAZUKIs") {
    setBaseURI(baseURI);
  }
  ...
  function setMaxFreeTokens(uint256 _count) public onlyOwner {
    MAX_FREE_PER_TX = _count;
  }
  ...
  function setMaxFREEPerWallet(uint256 _count) public onlyOwner {
    MAX_FREE_PER_WALLET = _count;
  }
  ...
  function mint(uint256 _count) public payable saleIsOpen {
    ...
    if(mintIndex < FREE_MAX_SUPPLY) {
      require(balanceOf(msg.sender) + _count <= MAX_FREE_PER_WALLET, "Max FREE holding cap reached.");
      require( _count <= MAX_FREE_PER_TX, "Exceeds FREE allowed tokens per TX");
    }
