...
contract AbstractOriginalsV2 is Ownable, ERC721A, ReentrancyGuard {
    using Strings for uint256;


  uint256 public MAX_PER_Transaction = 3; // maximum amount that user can mint/Transaction
  uint256 public MAX_PER_WALLET = 3; // maximum amount that user can mint/Wallet
  ...
  constructor() ERC721A("Abstract Originals V2","AOV2") {
    _uriBeforeRevel = "";
  }
 ...
 function mintReserveTokens(uint quantity) public onlyOwner {
  ...
  function airdrop(address sendTo, uint quantity)public onlyOwner{
    ...
    uint256 public itemPriceWhiteList = 0.045 ether;
    mapping(address => uint256) public whiteListClaimedBy;
    uint256 public TotalWLavailable = 666; // total number of nfts
    ...
    function inWhitelist(bytes32[] memory _proof, address _owner) public view returns (bool) {
       ...
       function purchaseWhiteListTokens(uint256 _howMany ,  bytes32[] calldata _proof) external payable {
        ...
        require(totalSupply()+_howMany<=TotalWLavailable-reserve,"Quantity must be lesser then MaxSupply");
        require(inWhitelist(_proof, msg.sender), "You are not in presale");

        whiteListClaimedBy[msg.sender] += _howMany;
        if (whiteListClaimedBy[msg.sender] == 1){
            itemPriceWhiteList = 0 ether;
        }
        ...
        itemPriceWhiteList = 0.045 ether;
