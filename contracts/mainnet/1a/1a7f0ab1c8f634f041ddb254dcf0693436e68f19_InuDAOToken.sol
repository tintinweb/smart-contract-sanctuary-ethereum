...
contract InuDAOToken is ERC721ICompliant("InuDAO", "InuDAO"), Ownable {
    ...
    function _mint(address to_) internal {
        require(balanceOf(to_) == 0,
            "Address already has token");
    ...
    function _burn(address from_) internal {
        require(balanceOf(from_) > 0,
            "Address has no token");
    ...
    function mint(address[] calldata to_) external onlyOwner {
        for (uint256 i = 0; i < to_.length; i++) {
            _mint(to_[i]);
        }
    }
    
    function burn(address[] calldata from_) external onlyOwner {
