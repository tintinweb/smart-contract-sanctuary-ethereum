...
contract HofaNFT is ERC721Upgradeable, ERC721URIStorageUpgradeable, ERC721BurnableUpgradeable, IERC2981, AccessControlUpgradeable {
    ...
    // v1.2 fields:
    mapping(uint256 => address) public _artists;

    // v1.3 fields:
    address public _royaltyReceiver;

    event SetRoyaltyReceiver(address indexed royaltyReceiver);

    function version() external pure returns (string memory) { return "HofaNFT v1.3"; }
    ...
    function initialize(
        ...
        require(minter != address(0), "Zero minter address");
    ...
    function _setRoyaltyReceiver(address royaltyReceiver) private {
        require(royaltyReceiver != address(0), "Zero royalty receiver address");
        require(_royaltyReceiver != royaltyReceiver, "Already done");
    ...
    function mint(
        ...
        mint(uri, hash, msg.sender, royalties);
    }
    ...
    function mint(
        ...
        address artist,
        uint16 royalties
    ) public onlyValidURI(uri) onlyRole(MINTER_ROLE) {
        ...
        _artists[tokenId] = artist != address(0) ? artist : msg.sender;
    ...
    function supportsInterface(bytes4 interfaceId) public view override(ERC721Upgradeable, AccessControlUpgradeable, IERC165) returns (bool) {
    ...
    function royaltyInfo(uint256 tokenId, uint256 value) external view override returns (address receiver, uint256 royaltyAmount) {
        ...
        return (_royaltyReceiver, (value * _royalties[tokenId]) / 10_000);
    }
