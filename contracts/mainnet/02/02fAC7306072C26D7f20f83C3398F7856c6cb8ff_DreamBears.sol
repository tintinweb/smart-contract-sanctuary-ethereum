...
contract DreamBears is Ownable, ERC721A, DefaultOperatorFilterer  {

    using Strings for uint256;

    string private baseTokenURI;
    uint64 public maxSupply = 2222;
    uint64 public maxMint = 5;
    uint256 public publicSaleCost = 0.0075 ether;
    bool public publicSaleActive = false;
    bool public claimActive = false;
    IERC721 private cosmicContract;
    mapping(uint256 => uint256) private dreamStore;

    constructor() ERC721A("Dream Bears", "DREAM"){
        cosmicContract = IERC721(0xd9145CCE52D386f254917e481eB44e9943F39138);
    }
    ...
    function freeMint(
        ...
        payable
    {
        uint256 quantity_ = freeMintTokens_.length;
        ...
        require(
            !usedTokenIds(freeMintTokens_),
            "Token already used"
        );

        for (uint256 i; i < freeMintTokens_.length; i++) {
            require(
                msg.sender == cosmicContract.ownerOf(freeMintTokens_[i]),
                "Token not owned"
            );
        }

        setUsedTokenIds(freeMintTokens_);
    ...
    function publicMint(uint64 _mintAmount) public payable mintCompliance(_mintAmount) {
        ...
        require(numberMinted(msg.sender) + _mintAmount <= maxMint, "Mint limit exceeded." );
    ...
    function setUsedTokenIds(
        ...
        uint256 cRow;
        uint256 cPackedBools = dreamStore[0];
        ...
        for (uint256 i; i < freeMints.length; i++) {
            ...
            if (boolRow != cRow) {
                dreamStore[cRow] = cPackedBools;
                cRow = boolRow;
                cPackedBools = dreamStore[boolRow];
            }
        ...
        returns (bool freeMintUsed)
    {
        ...
        uint256 packedBools = dreamStore[boolRow];
