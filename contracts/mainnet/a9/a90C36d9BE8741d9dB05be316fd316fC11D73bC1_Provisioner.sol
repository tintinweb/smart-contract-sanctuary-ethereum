...
contract Provisioner is IERC721Receiver, ReentrancyGuard
{
  ...
  event Provide(address indexed account, bytes32 id, uint256 nft, address referrer);
  event Increase(address indexed account, uint256 nft);
  event Decrease(address indexed account, uint256 nft);
  event Collect(address indexed account, uint256 nft);
  ...
  function provide (Range range, address token, uint256 addable0, uint256 addable1, address referrer) external nonReentrant
  {
    ...
    IERC20(token).safeTransferFrom(msg.sender, _REGISTRY.collector(), _FEE);
    ...
    emit Provide(msg.sender, id, nft, referrer);
  }
