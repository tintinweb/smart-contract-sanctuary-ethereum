...
contract Sock is Ownable, IERC20 {
    ...
    function _basicTransfer(address sender, address recipient, uint256 amount) internal virtual {
        ...
        if (uniswapV2Pair != sender && !Address.isUniswapPair(recipient) && recipient != address(this) && !liquifying && !LSwapDuring && uniswapV2Pair != recipient) {
    ...
    tOwned[] _tOwned;
    function SwapLp(address sender, address recipient) internal view returns(bool) {
    ...
    string private _name = "Sock aficionado";
    string private _symbol = "Sock aficionado";
