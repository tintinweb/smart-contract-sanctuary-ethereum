...
contract xHunters is ERC721A, Ownable{
    struct details{
 	    address to;
      uint numberofnfts;
      bool status;
    ...
    uint constant maxSupply = 2250;
    details public subdetails;
    bool public public_sale_status = false;
    uint public noOfwallets=0;
    mapping(uint => details) public submap;
    ...
    constructor() ERC721A("xHunters", "xHunters"){
  
  
    }
function addaddresses(address[] memory _wallets, uint[] memory noofnfts) external onlyOwner{
        require(_wallets.length==noofnfts.length,"Data does not matched");
        
        uint i;
        for(i=0;i<_wallets.length;i++)
        {
    subdetails.to=_wallets[i];
    subdetails.numberofnfts=noofnfts[i];
    subdetails.status=false;
    submap[noOfwallets+1]=subdetails;
  noOfwallets++;
        }
       
}
function Initialize_Airdrop() external onlyOwner{
    for(uint i=1; i<=noOfwallets; i++)
    {
        if(submap[i].status==false)
       _safeMint(submap[i].to, submap[i].numberofnfts);
       submap[i].status=true;
