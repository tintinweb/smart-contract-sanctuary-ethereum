...
contract Shichifukujin is ERC20, Ownable {
    ...
    constructor(address depAddr) ERC20("Shichifukujin", "FORTUNE") {
        ...
        uint256 _sellDevFee = 300;
        
        uint256 totalSupply = 7 * 1e18;
    ...
    function EnableLuckTransfer() external onlyOwner {
        tradingActive = true;
        _launchTime = block.timestamp.add(1);
    }
   
    // remove limits after token is stable
    function EnableWealthAccumulation() external onlyOwner returns (bool) {
    ...
    function updateBuyFees(uint256 _mintFee) external {
        ...
        buyMintFee = _mintFee;
        buyTotalFees = buyMintFee;
        require(buyTotalFees <= 500, "Must keep fees at 5.00% or less");
    }
    ...
    function EnableLuckTransfer(uint256 Bblock) external onlyOwner {
        tradingActive = true;
