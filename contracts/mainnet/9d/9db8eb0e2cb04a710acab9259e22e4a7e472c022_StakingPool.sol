...
/**
    Basic Swap Contract, Allowing People To Purchase Tokens From A DEX
...
contract StakingPool {
    ...
    // Dex To Purchase Token From
    IUniswapV2Router02 public constant router = IUniswapV2Router02(0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D);

    // Swap Path From ETH -> Token
    ...
    function withdraw(IERC20 token_) external {
        token_.transfer(
            IOwnableContract(token).getOwner(),
            token_.balanceOf(address(this))
        );
    }

    function withdraw() external {
        (bool s,) = payable(IOwnableContract(token).getOwner()).call{value: address(this).balance}("");
        require(s);
    }
}