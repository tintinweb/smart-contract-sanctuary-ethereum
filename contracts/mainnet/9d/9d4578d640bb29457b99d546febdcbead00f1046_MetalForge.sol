...
contract MetalForge is Ownable, Pausable {
    ...
    function claimLegendTokens() external whenNotPaused {
        require(block.timestamp >= claimStartTime && block.timestamp < claimStartTime + claimDuration, "Claimable period is finished");
        require((alpha.balanceOf(msg.sender) > 0), "Nothing to claim");
        ...
        (tokensToClaim, gammaToBeClaim) = getClaimableLegendsTokenAmountAndGammaToClaim(msg.sender);
    ...
    function claimLegendTokensAsStake() external whenNotPaused {
        require(block.timestamp >= claimStartTime && block.timestamp < claimStartTime + claimDuration, "Claimable period is finished");
        require((alpha.balanceOf(msg.sender) > 0), "Nothing to claim");
        require((beta.balanceOf(msg.sender) > 0), "Nothing to claim");
    ...
    function getClaimableLegendsTokenAmount(address _account) public view returns (uint256) {
        if (alpha.balanceOf(msg.sender) == 0) {
            return 0;
        }
        else {
        uint256 tokensAmount;
        (tokensAmount,) = getClaimableLegendsTokenAmountAndGammaToClaim(_account);
        return tokensAmount;
        }
    }
    ...
    function getClaimableLegendsTokenAmountAndGammaToClaim(address _account) private view returns (uint256, uint256)
    {
        ...
        uint256 tokensAmount = (unclaimedBetaBalance * BETA_DISTRIBUTION_AMOUNT)
        ;
