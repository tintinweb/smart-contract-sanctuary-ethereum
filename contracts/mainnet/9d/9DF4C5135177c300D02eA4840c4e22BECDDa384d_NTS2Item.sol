    ...
    ReentrancyGuardUpgradeable
{
    ...
    function initialize(
        ...
        __ERC721_init("Neo Tokyo Outer Item Caches V2", "NTOITEM");
        __ERC2981_init();
    ...
    function getTokenClaimedByIdentityTokenId(
        ...
        uint256 v1Token = NTS2Item(config.findComponent(NTComponent.S2_ITEM, false))
            .getTokenClaimedByIdentityTokenId(identityTokenId);
    ...
    function getClaimantIdentityIdByTokenId(
        ...
        uint256 v1Claimant = NTS2Item(config.findComponent(NTComponent.S2_ITEM, false)).getClaimantIdentityIdByTokenId(tokenId);
    ...
    function emergencyClaim(
        ...
        require(tokenId > 0 && tokenId <= boughtItemOffset, "Token ID invalid");
        require(!_exists(tokenId), "That token already exists");
        require(
            _identityClaims[identityId] == 0,
            "That identity has already claimed"
        );
    ...
    function migrateAsset(address sender, uint256 tokenId) public nonReentrant {
        ...
        IERC721Upgradeable v1Contract = IERC721Upgradeable(
            config.findComponent(NTComponent.S2_ITEM, false)
        );
