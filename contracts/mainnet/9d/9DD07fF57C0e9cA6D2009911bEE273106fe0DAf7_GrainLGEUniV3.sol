...
contract GrainLGEMetis is IGrainLGE, Ownable {
    ...
    function addNftBonus(address nft, address onBehalfOf) public {
        if (block.timestamp < lgeStart) {
            revert GrainLGE__NotLive();
        }
        if (onBehalfOf != msg.sender) {
        ...
        if (IERC721(nft).balanceOf(onBehalfOf) == 0) {
...
interface IGrainLGE {
    ...
    function buy(address token, uint256 amount, uint256 minUsdcAmountOut, uint256 numberOfReleases, address onBehalfOf, address nft) external returns (uint256 usdcValue,uint256 vp);
...
contract WETHGateway is IWETHGateway, Ownable {
  ...
  function depositETH(
    ...
    IGrainLGE(grainLge).buy(address(WETH), msg.value, minUsdcAmountOut, numberOfReleases, msg.sender, address(0));
  }
