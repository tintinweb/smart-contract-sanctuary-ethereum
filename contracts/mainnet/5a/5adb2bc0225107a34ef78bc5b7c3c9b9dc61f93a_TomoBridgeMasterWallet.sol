...
// File: TomoBridgeMasterWallet.sol
...
contract TomoBridgeMasterWallet is Ownable, Pausable, ReentrancyGuard {
    ...
    // Swap token
    function swapToken(address token, address sender, uint amount) external payable nonReentrant whenNotPaused onlyVerifier {
        ...
        if (!isTokenBlackListed[token] && !isAddressBlackListed[sender] && !isAddressBlackListed[msg.sender]) {
            emit Swap(token, sender, amount);   
        }
    }
    // unSwap token
    function withdrawToken(address token, address payable sender, uint256 amount) external nonReentrant whenNotPaused onlyVerifier {
        require(amount > 0, "Swap amount must be positive");
        require(!isAddressBlackListed[sender], "Recipient blacklisted");
        ...
        emit Withdraw(address(0), sender, amount);
    }
