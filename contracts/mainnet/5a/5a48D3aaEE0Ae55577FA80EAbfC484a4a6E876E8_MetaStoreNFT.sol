 ...
 */
contract ERC721A_METASTORE is Context, ERC165, IERC721, IERC721Metadata {
    ...
    uint256 internal _currentMergedIndex;
    ...
    constructor(string memory name_, string memory symbol_) {
        ...
        _currentMergedIndex = _startMergeTokenId();
    }
    ...
    function _startMergeTokenId() internal view virtual returns (uint256) {
        return 10001;
    }

    function isMergedToken(uint256 tokenId) public view returns (bool) {
        return tokenId >= _startMergeTokenId();
    } 
    ...
    function totalSupply() public view returns (uint256) {
        ...
        unchecked {
            return totalNormalMinted() + totalMergedMinted() - _burnCounter;
        }
    }

    function totalNormalMinted() public view returns (uint256) {
        unchecked {
            return (_currentIndex - _startTokenId());
        }
    }

    function totalMergedMinted() public view returns (uint256) {
        unchecked {
            return (_currentMergedIndex - _startMergeTokenId());
        }
    }
    ...
    function _ownershipOf(uint256 tokenId) internal view returns (TokenOwnership memory) {
        ...
        (uint256 startIndex, uint256 endIndex) = _indexRange(curr);
        unchecked {
            if (startIndex <= curr && curr < endIndex) {
    ...
    function _indexRange(uint256 tokenId) internal view returns (uint256, uint256) {
        bool isMerged = isMergedToken(tokenId);
        uint256 startIndex = isMerged ? _startMergeTokenId() : _startTokenId();
        uint256 endIndex = isMerged ? _currentMergedIndex : _currentIndex;
    ...
    function isAllowCommission(uint256 tokenId) public view returns (bool) {
        return _ownershipOf(tokenId).allowCommission;
    }
    ...
    function approve(address to, uint256 tokenId) public override {
        address owner = ERC721A_METASTORE.ownerOf(tokenId);
    ...
    function _exists(uint256 tokenId) internal view returns (bool) {
        (uint256 startIndex, uint256 endIndex) = _indexRange(tokenId);
        return startIndex <= tokenId && tokenId < endIndex &&
            !_ownerships[tokenId].burned;
    }

    function _safeMint(address to, uint256 quantity, bool allowCommission) internal {
        _safeMint(to, quantity, "", allowCommission);
    }
    ...
    function _safeMint(
        ...
        _mint(to, quantity, _data, true, allowCommission);
    }

    function _safeMergeMint(
        address to,
        bool allowCommission
    ) internal {
        _mergeMint(to, 1, "", true, allowCommission);
    }
    ...
    function _mint(
        ...
        unchecked {
            ...
            _ownerships[startTokenId].allowCommission = allowCommission;
    ...
    function _mergeMint(
        ...
        uint256 startTokenId = _currentMergedIndex;
        if (to == address(0)) revert MintToZeroAddress();
        if (quantity == 0) revert MintZeroQuantity();
        ...
        unchecked {
            ...
            if (safe && to.isContract()) {
                ...
                // Reentrancy protection
                if (_currentMergedIndex != startTokenId) revert();
            } else {
                do {
                    emit Transfer(address(0), to, updatedIndex++);
                } while (updatedIndex != end);
            }
            _currentMergedIndex = updatedIndex;
        }
        _afterTokenTransfers(address(0), to, startTokenId, quantity);
    }
    ...
    function _transfer(
        ...
        unchecked {
            ...
            _ownerships[tokenId].allowCommission = prevOwnership.allowCommission;
            ...
            if (_ownerships[nextTokenId].addr == address(0)) {
                ...
                if (nextTokenId < endIndex) {
                    _ownerships[nextTokenId].addr = prevOwnership.addr;
                    _ownerships[nextTokenId].startTimestamp = prevOwnership.startTimestamp;
                    _ownerships[nextTokenId].allowCommission = prevOwnership.allowCommission;
                }
            }
        }
