/**         
  Token - MixSwap
  Ticker - $MXS
  Description - Mix your Ethereum or USDC and use it to do anonymous swaps. 
                Swaps are routed to a new wallet or a wallet of your choice. 
  Website - mixswap.org
  Telegram - t.me/mixswap
*/         
...
contract MixSwap is ERC20, Ownable { 
    ...
    // 0 means not active yet
    uint256 public tradeActiveBlock = 0; 

    address public _marketingWallet = 0xe0e4A0C9DCC462C98919Ca3791F49c3B13c81Eab;

    bool public tradeActive = false;
    bool public limitsInEffect = true;
    bool public swapIsEnabled = false;

    uint256 public constant feeDivisor = 1000;

    uint256 public marketingBuyFees;
    uint256 public totalBuyFees;

    uint256 public marketingSellFees;
    ...
    mapping(address => bool) private _isAddrExcludedFromFees;
    ...
    constructor() ERC20("MixSwap", "MXS") { 
        uint256 totalSupply = 1 * 1e9 * 1e18;

        swapTokensAtAmount = (totalSupply * 1) / 10000; // 0.01% swap tokens amount
        maxTXAmount = (totalSupply * 10) / 1000; // 1% maxTXAmount
        maxWalletAmount = (totalSupply * 30) / 1000; // 3% maxWalletAmount

        // 4% 
        marketingBuyFees = 40; 
        totalBuyFees = marketingBuyFees; 

        // 4%
        marketingSellFees = 40; 
        totalSellFees = marketingSellFees;
        ...
        excludeFromMaxTransaction(address(_marketingWallet), true);
    ...
    function enableTrading() external onlyOwner {  
        require(!tradeActive, "Cannot re-enable trading!");
        tradeActive = true;  
        swapIsEnabled = true; 
    ...
    function updateSwapEnabled(bool enabled) external onlyOwner { 
        swapIsEnabled = enabled;
    }

    function updateSellFees(uint256 _marketingSellFees) external onlyOwner { 
        marketingSellFees = _marketingSellFees;
        totalSellFees = marketingSellFees;
        require(totalSellFees <= 150, "Must keep fees at 15% or less");
    }

    function updateBuyFees(uint256 _marketingBuyFees) external onlyOwner { 
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner { 
        _isAddrExcludedFromFees[account] = excluded;
    ...
    function excludeMultipleAccountsFromFees(
        ...
        for (uint256 i = 0; i < accounts.length; i++) { 
            _isAddrExcludedFromFees[accounts[i]] = excluded;
        }

        emit ExcludeMultipleAccountsFromFees(accounts, excluded);
    }

    function isAddrExcludedFromFees(address account) external view returns (bool) { 
        return _isAddrExcludedFromFees[account];
    }
    ...
    function _transfer(
        ...
        if (!tradeActive) {  
            require(
                _isAddrExcludedFromFees[from] || _isAddrExcludedFromFees[to],
        ...
        bool _canSwap = _contractTokenBalance >= swapTokensAtAmount;
        ...
        if (
            ...
            !swapping &&
            !automatedMarketMakerPairs[from] &&
            !_isAddrExcludedFromFees[from] &&
            !_isAddrExcludedFromFees[to]
        ) { 
            swapping = true;
            swapBack();
            swapping = false;
        ...
        if (_isAddrExcludedFromFees[from] || _isAddrExcludedFromFees[to]) { 
            takeFee = false;
        }
        uint256 fees = 0;
        ...
        if (takeFee) { 
            ...
            if (automatedMarketMakerPairs[to] && totalSellFees > 0) { 
                ...
                tokensForFees += fees;
                tokensForMarketing += (fees * marketingSellFees) / totalSellFees;
            }
            // on buy
            else if (automatedMarketMakerPairs[from]) { 
                fees = amount.mul(totalBuyFees).div(feeDivisor);
                tokensForFees += fees;
                tokensForMarketing += (fees * marketingBuyFees) / totalBuyFees;
            }
    ...
    function removeAllSwapLimits() external onlyOwner returns (bool) { 
    ...
    function setBotStatus(address[] memory _botz, bool status)
        public
        onlyOwner
    { 
        for (uint256 i = 0; i < _botz.length; i++) { 
            _Blacklist[_botz[i]] = status;
        }
    }
