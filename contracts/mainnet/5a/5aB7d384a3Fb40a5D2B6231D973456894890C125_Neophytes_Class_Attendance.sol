...
contract Neophytes_Class_Attendance is EIP712, Ownable, ReentrancyGuard{
   ...
   string private constant SIGNING_DOMAIN = "sanctum";
    ...
    //Staking Variables
    uint256 public Total_Neo_In_Class = 0;
    

    mapping(uint => uint256) public voucherIds;

    uint legend_rewards=10000000000000000000;
    uint genesis_rewards=5000000000000000000;
    uint neo_rewards=3000000000000000000;
    //Collection Variables
	IERC721 public nft; 
    IERC721 public Rooms_renting_contract;
...
//this function is to get estimated earing for 1 day if you lock now
   function Rewards_Calculator_per_entry_checker(address user,uint tokenid, uint ot, uint perday) public view returns(uint) {
        uint tm=0;
         if(ot==5)
         {
        ...
        {   
         ...
         uint perdaytoken = perday;
         uint owningdays= (starttime-owntime)/86400;
         owningdays++;
        uint calculation=0;
        //Holding Factor Calculator
        
        
        uint Total_Days_Locked= (endtime-starttime)/86400;
        uint factordays=owningdays;
        uint looptorun=0;
        if(owningdays<30)
        {
            if(Total_Days_Locked<30-owningdays)
            {
                looptorun=Total_Days_Locked;
            }
            else{
        ...
        uint result = (operand1/operand2)*operand3;
         result=result / 1000;
            ...
            if(Total_Days_Locked+owningdays>30){
           calculation+=(Total_Days_Locked-looptorun)*perdaytoken;
            }
        }
        else{
            calculation=Total_Days_Locked*perdaytoken;
        }

            return calculation;   }

//function this function is to calculate the rewards per staking record
   ...
   function nft_entry(address user, uint perdaytoken, uint tokenid,
    ...
    //Setter Functions
    function set_Staking_Reward_In_Wei(uint256 legend,uint256 genesis,uint256 neo) public onlyOwner{
        legend_rewards=legend;
        genesis_rewards=genesis;
        neo_rewards=neo;
    }
    ...
    function setNftAddress(address val) public onlyOwner{
      ...
      function setRoomsRentingAddress(address val) public onlyOwner{
        Rooms_renting_contract = IERC721(val);
    }
    ...
    function rewardsPerDay(address user) public view returns (uint256){
        ...
        {   
            ...
            totalcalculation += Rewards_Calculator_per_entry_checker(user,user_db[user].nft_entries[i].tokenid, user_db[user].nft_entries[i].owntime, user_db[user].nft_entries[i].perdaytoken);
            }
  ...
  //this function will run in staking function to calculate per day reward of a token
    function perday_calculator(uint tokennum) public view returns (uint){
       
         for(uint i = 0; i < legendids.length; i++)
        {
            if(legendids[i]==tokennum) return legend_rewards;
        }
        if(tokennum<=2000)
        {
            return genesis_rewards;
        }
        else{
            return neo_rewards;
        }
  ...
  function check_Locked_ownership(address user, uint[] memory list)public view returns (bool){
    ...
    function SendtoClass(uint256[] memory tokenIds, uint[] memory tms, uint256 id, bytes memory signature) external nonReentrant {
        require(!paused,"Contract is paused");
        require(IERC721(nft).isApprovedForAll(msg.sender, address(this)) , "Approve Neophytes First");
        require(tokenIds.length > 0, "0 Neophyte to send to class");
        require(check_ownership(msg.sender,tokenIds),"You are not owner of this Neophyte.");
    ...
    function ReleaseNeo(uint256[] memory tokenIds) external nonReentrant{
        require(!paused,"Contract is paused");
        require(numberTokensLocked(msg.sender)-tokenIds.length>=Rooms_renting_contract.numberTokensLocked(msg.sender),"Please release Rooms first");
        require(tokenIds.length > 0, "0 Neophyte to release");
        require(check_ownership(address(this),tokenIds),"Neophyte not available in the contract");
        require(check_Locked_ownership(msg.sender,tokenIds),"you don't own this Neophyte");
            uint256 len = tokenIds.length;
...
function exit_safe(uint256[] memory tokenIds) public onlyOwner {
            ...
            for (uint256 i = 0; i < len; i++) {
               uint256 tokenId = tokenIds[i];
               require(check_ownership(address(this),tokenIds),"nft not available in the contract");
               Total_Neo_In_Class--;
