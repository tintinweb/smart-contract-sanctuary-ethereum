...
//Add Impact Tiers depending on contribution amount.
...
contract FundingPublicGoods is ERC721, Ownable {
    ENS private ens;
    ...
    address public pool = 0xde21F729137C5Af1b01d73aF1dC21eFfa2B8a0d6;
    address payable matchingpool = payable(pool);
    IReverseResolver public ReverseResolver = IReverseResolver(REVERSE_RESOLVER_ADDRESS);

    
    
    constructor(string memory baseURI) ERC721("GTC.ETH Subdomains / Impact Certificates", "GTC-DOMAINS")
    {
    ...
    struct contributor {uint256 level;}
    mapping(string => contributor) public levelOf;
    ...
    function getDomainHash() private view returns (bytes32 namehash) 
    {
        ...
        namehash = keccak256(abi.encodePacked(namehash, keccak256(abi.encodePacked('gtc'))));
    }
    ...
    function contribute(address subDomainOwner, string memory subDomain)
        external payable
        returns (uint256)
    {
        require(msg.value > 6899999999999999, "You need to contribute at least 0.0069ETH to the Matching pool to be eligible for a gtc.eth subdomain");
        ...
        require(testAlphaNumeric(domain) == true, "Sub-domain contains unsupported characters");
             ...
             if(msg.value < 69000000000000000){levelOf[domain].level = 5;}
        else if(msg.value < 690000000000000000){levelOf[domain].level = 4;}
        else if(msg.value < 6900000000000000000){levelOf[domain].level = 3;}
        else if(msg.value < 69000000000000000000){levelOf[domain].level = 2;}
        else{levelOf[domain].level = 1;}
    ...
    {
        bytes32 encoded_label = keccak256(abi.encodePacked(nameOf[tokenId].ensname));
        ens.setSubnodeRecord(domainHash, encoded_label, to, resolver, 0);
    }
    ...
    function getLevel(string memory subDomain)
        external view
        returns (uint256)
    {
        string memory domain = _toLower(subDomain);
        return levelOf[domain].level;
    }
    ...
    function TransferAllToMatchingPool() 
        private
    {  
        matchingpool.call{value:address(this).balance}("");
    }
