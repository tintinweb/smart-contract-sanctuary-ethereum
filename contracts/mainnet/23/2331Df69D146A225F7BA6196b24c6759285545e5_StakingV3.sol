...
contract StakingV3 is IStakingV2, StakingV3Base, ReentrancyGuardUpgradeable {
  ...
  function _isUnlocked(Stake memory _stake) internal view override returns (bool) {
    if (unlockStakes) {
      return true;
    }
...
contract StakingV3Base is IStaking, ERC1155Upgradeable, BasePauseableUpgradeable {
  ...
  function _isUnlocked(Stake memory _stake) internal view virtual returns (bool) {
    return _getBlockTimestamp() > (_stake.startTime + _stake.lockPeriod * SECONDS_PER_MONTH);
  }
