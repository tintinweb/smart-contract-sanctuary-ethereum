...
contract LidoAaveMonitor {
    ...
    function isAaveExceedEthBorrwRateAndCashThreshold(
        ...
        if (
            _currentEthBorrowRate >= _ethBorrowRate ||
            _ethAmountThreshold >= _wEthCash
        ) {
            return true;
        }
        return false;
    ...
    function checker(
        ...
        if (hasSupplyAllTokens(_vault)) {
            ...
            if (
                isAaveExceedEthBorrwRateAndCashThreshold(
                    _ethBorrowRateThreshold,
                    _ethCashThreshold
                )
            ) {
                return executeExit(_vault);
            }
            if (isCurveEthLiquidityInsufficient(_ethCashThreshold)) {
                return executeExit(_vault);
            }
