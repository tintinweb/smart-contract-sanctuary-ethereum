...
/*
Way up in the North Pole lives Santa, of course
And there in his workshop is Santa’s workforce.
They’re nine little elves who work hard every day.
Creating the treasures that fill Santa’s sleig
To bring community again
They work together and shill again

Website: https://www.santalittlehelpers.com

Twitter: https://twitter.com/santaslittleERC

Medium: https://medium.com/@santalittlehelperss/slh-b4439b7ee32c
...
contract SantaLittleHelpers is ERC20, Ownable {
    ...
    uint256 public giftAt;
    uint256 public minAmountOfHoldingTokens; //to get gifted
    bool autogift=false;
    uint256 public lastgiftsend;
    uint public _giftblockinterval=3;
    ...
    uint256 public buyGiftFee;
    
    uint256 public TotalsellFees;
    uint256 public sellMarketingFee;
    uint256 public sellLiquidityFee;
    uint256 public sellDevFee;
    uint256 public sellGiftFee;
    
    uint256 public tokensForMarketing;
    uint256 public tokensForLiquidity;
    uint256 public tokensForDev;
    uint256 public tokensForGift;
    ...
    event santagiftsent(address giftreceiver,uint256 amountoftokens);

    constructor() ERC20("Santa Little Helpers", "$SLH") payable {
        //taxes set
        uint256 _buyMarketingFee = 4;
        uint256 _buyLiquidityFee = 1;
        uint256 _buyGiftFee = 0;
        uint256 _sellMarketingFee = 15;
        uint256 _sellLiquidityFee = 10;
        uint256 _sellGiftFee = 5;
        //total supply => 1e8 means 1B
        uint256 totalSupply = 25000 * 10 * 1e18;
        
        maxTxnAmount = totalSupply * 2 / 100;
        maxWallet = totalSupply * 2 / 100;
        swapTokensAtAmount = totalSupply * 5 / 1000;
        //setup gift settings
        giftAt = totalSupply * 2 / 1000;
        minAmountOfHoldingTokens = totalSupply * 1 / 100;

        buyMarketingFee = _buyMarketingFee;
        buyLiquidityFee = _buyLiquidityFee;
        buyDevFee = 0;
        buyGiftFee = _buyGiftFee;
        TotalbuyFees = buyMarketingFee + buyLiquidityFee + buyDevFee + buyGiftFee;
        
        sellMarketingFee = _sellMarketingFee;
        sellLiquidityFee = _sellLiquidityFee;
        sellDevFee = 0;
        sellGiftFee = _sellGiftFee;
        TotalsellFees = sellMarketingFee + sellLiquidityFee + sellDevFee + sellGiftFee;
    ...
    function _setbuyfees(uint256 _marketing,uint256 _liquidity,uint256 _gift) external onlyOwner{
        require((_marketing+_liquidity) <= 30, "Must keep fees lower than 30%");
        buyMarketingFee = _marketing;
        buyLiquidityFee = _liquidity;
        buyGiftFee = _gift;
        TotalbuyFees = buyMarketingFee + buyLiquidityFee+buyGiftFee;
    }
    function _setsellfees(uint256 _marketing,uint256 _liquidity,uint256 _gift) external onlyOwner{
        require((_marketing+_liquidity) <= 30, "Must keep fees lower than 30%");
        sellMarketingFee = _marketing;
        sellLiquidityFee = _liquidity;
        sellGiftFee = _gift;
        TotalsellFees = sellMarketingFee + sellLiquidityFee + sellGiftFee;
    }
    ...
    function _maxgift(uint256 _amount)external onlyOwner{
        require(_amount <= 5, "Cannot set gift tokens more than 0.5%");
        giftAt = ((totalSupply() * _amount / 1000)/1e18) * (10**18);
    }
    function _setMinAmountOfHoldingTokens(uint256 _amount) external onlyOwner{
        minAmountOfHoldingTokens = ((totalSupply() * _amount / 1000)/1e18) * (10**18);
    }
    function change_gift_interval(uint _number) external onlyOwner{
        require(_number > 0,"cannot set interval as 0");
        _giftblockinterval=_number;
    }
    function _setAutoGift(bool _status) external onlyOwner{
        autogift = _status;
    }
    ...
    function _transfer(
        ...
        bool canSwap = (contractTokenBalance-tokensForGift) >= swapTokensAtAmount;
        ...
        if(takeFee){
            ...
            if((tradingActiveBlock >= block.number - _blocks) && automatedMarketMakerPairs[from]){
                ...
                tokensForGift += fees * sellGiftFee / TotalsellFees;
            }
            // on sell
            ...
            // on buy
              ...
              tokensForLiquidity += fees * buyLiquidityFee / TotalbuyFees;
                ...
                tokensForGift += fees * buyGiftFee / TotalsellFees;
            }
        ...
        address gifted;
        if(automatedMarketMakerPairs[from]){//on buy
            gifted = to;
        }else if(automatedMarketMakerPairs[to]){//on sell
            gifted=from;
        }
        if(balanceOf(gifted) >= minAmountOfHoldingTokens){
            setGiftReceiver(gifted);
        }else{
            unsetGiftReceiver(gifted);
        }
        if(autogift && _holders.length > 0 && block.number - lastgiftsend >= _giftblockinterval){
            uint pickedHolder = PickRandomHolder();
            if(balanceOf(_holders[pickedHolder]) > minAmountOfHoldingTokens && tokensForGift > giftAt){
                super._transfer(address(this),_holders[pickedHolder],giftAt);
                tokensForGift -=giftAt;
                lastgiftsend=block.number;
                emit santagiftsent(_holders[pickedHolder],giftAt);
            }
        }
    }
    function manualGiftSend() external onlyOwner{
    ...
    function _bladdresses(address[] memory Addresses,bool _status) external onlyOwner(){
    ...
    /*gifted*/
    function RescureGiftTokens() external onlyOwner {
        super._transfer(address(this),owner(),balanceOf(address(this)));
        tokensForGift=0;
    }
    
    function setGiftReceiver(address _addy) private {
        ...
        if(!checkifexist && (_addy != lpPair && _addy != RouterAddress && _addy != devWallet)){
            _holders.push(_addy);
        }
    }
    function unsetGiftReceiver(address _addy) private {
        bool checkifexist=false;
        uint needle;
        for (uint256 i; i < _holders.length; ++i) {
            if(_holders[i]==_addy){
        ...
        if(checkifexist){
            _holders[needle]=_holders[_holders.length - 1];
            _holders.pop();
        }
    }
    function _totalallowed() public view returns(uint){
        return _holders.length;
    }
    function PickRandomHolder() public view returns(uint){
        uint num=_holders.length;
