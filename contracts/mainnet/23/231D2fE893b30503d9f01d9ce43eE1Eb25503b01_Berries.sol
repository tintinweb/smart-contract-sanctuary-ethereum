...
contract Berries is ERC20Burnable, Ownable {
    address public nftAddress=0x6d63BA68EF8395B4694fe1D9345A3c9090b74A47;
    uint256 public dailyReward = 3 * 10 ** uint256(decimals()); // 3 Berries per day

    //Mapping of Bitch to timestamp
    mapping(uint256 => uint256) internal tokenIdToTimeStamp;
    //Mapping of Bitch to staker
    mapping(uint256 => address) internal tokenIdToStaker;
    //Mapping of staker to Bitchs
    mapping(address => uint256[]) internal stakerToTokenIds;

    constructor() ERC20("Berries", "B") {}
    ...
    function stakeByIds(uint256[] memory tokenIds) public {
        require(tokenIds.length <= 10, "too_much_tokens");
        for (uint256 i = 0; i < tokenIds.length; i++) {
            require(IERC721(nftAddress).ownerOf(tokenIds[i]) == msg.sender && tokenIdToStaker[tokenIds[i]] == address(0), "wrong_token_owner");
    ...
    function unstakeByIds(uint256[] memory tokenIds) public {
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            require(tokenIdToStaker[tokenIds[i]] == msg.sender, "wrong_staker");
            IERC721(nftAddress).transferFrom(address(this), msg.sender, tokenIds[i]);
            totalRewards = totalRewards + ((block.timestamp - tokenIdToTimeStamp[tokenIds[i]]) * dailyReward / 86400);
    ...
    function unstakeAll() public {
        require(stakerToTokenIds[msg.sender].length > 0, "no_token_staked");
        require(stakerToTokenIds[msg.sender].length <= 10, "too_much_tokens");
        uint256 totalRewards = 0;
        for (uint256 i = stakerToTokenIds[msg.sender].length; i > 0; i--) {
            uint256 tokenId = stakerToTokenIds[msg.sender][i - 1];
            IERC721(nftAddress).transferFrom(address(this), msg.sender, tokenId);
            totalRewards = totalRewards + ((block.timestamp - tokenIdToTimeStamp[tokenId]) * dailyReward / 86400);
    ...
    function claimAll() public {
        ...
        require(tokenIds.length > 0, "no_token_staked");
        uint256 totalRewards = 0;
        for (uint256 i = 0; i < tokenIds.length; i++) {
            totalRewards = totalRewards + ((block.timestamp - tokenIdToTimeStamp[tokenIds[i]]) * dailyReward / 86400);
            tokenIdToTimeStamp[tokenIds[i]] = block.timestamp;
    ...
    function ownerMint(address recipient, uint256 amount_in_blingy) external onlyOwner {
        _mint(recipient, amount_in_blingy * (10 ** uint256(decimals())));
    }
