...
contract SoundBound is ERC721Enumerable, Ownable, AccessControl {
    ...
    event LogNFTCreatedForDomain(uint256 indexed curTokenId, address indexed ownerWalletAddress, string name, uint32 genre);
    event LogNFTCreatedForSubdomain(uint256 indexed curTokenId, address indexed ownerWalletAddress, string subdomain, uint32 genre);
    ...
    function mintNFTForDomain(string memory name, uint32 genreId) external payable {
        ...
        emit LogNFTCreatedForDomain(newItemId, msg.sender, name, genreId);
    }
    ...
    function mintNFTForSubdomain(string memory subdomain, uint32 genreId) external payable {
        ...
        domainToTokenId[name] = newItemId;
        purchaseStatus[name] = true;
        emit LogNFTCreatedForSubdomain(newItemId, msg.sender, subdomain, genreId);
    }
    ...
    function mintNFTByAdmin(string[] memory domainMap, address[] memory addressMap, uint32 genreId) external onlyMember {
        ...
        for (uint i=0; i<domainMap.length; i++) {
            ...
            domainToTokenId[name] = newItemId;
            purchaseStatus[name] = true;
            emit LogNFTCreatedForSubdomain(newItemId, msg.sender, domainName, genreId);
        }
    }
    ...
    function getTokenIdByNode(string memory name) external view returns (uint256) {
        require(purchaseStatus[name] == true,  "The NFT has yet to be purchased");
