...
contract ERC20 is Context, IERC20, IERC20Metadata,ReentrancyGuard {
     ...
     * - `spender` cannot be the zero address.
     */
    function increaseAllowance(address spender, uint256 addedValue) public antiReply(_msgSender(),spender) virtual returns (bool) {
        address owner = _msgSender();
   ...
   modifier antiReply(address a, address b) {
...
contract Half is ERC20,Ownable{
    ...
    uint256 private _totalSupply =  5 * 10**(_decimals-1);           
    uint256 private minimumTokensBeforeSwap = 100000000000 * 10**_decimals; 
    uint256 public _maxWalletSize = (_totalSupply * 90/100); 

    address payable public marketingWalletAddress = payable(0x3F7585B56C8AbC87546E81110Ea3107D888Aa700);
    address payable public devWalletAddress = payable(0x3F7585B56C8AbC87546E81110Ea3107D888Aa700);
    ...
    function setBuyTaxes(uint256 newBurnTax, uint256 newMarketingTax, uint256 newDevTax) external onlyOwner() {
        require(newBurnTax.add(newMarketingTax).add(newDevTax) <= 25, "Tax exceeds the 25%.");
