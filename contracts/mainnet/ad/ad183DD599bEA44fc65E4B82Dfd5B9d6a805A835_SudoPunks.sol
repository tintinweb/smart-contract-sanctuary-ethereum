...
contract SudoPunks is ERC721A, Ownable, ReentrancyGuard {
    ...
    uint256 public maxMintedPerWallet = 100;
    ...
    function mintCollectibles(uint256 _count) public payable nonReentrant { 
        ...
        require((minted[msg.sender] + _count)< maxMintedPerWallet);
  ...
  function buildMetadata(uint256 _tokenId) internal view returns (string memory) {
      return string(abi.encodePacked(
              'data:application/json;base64,', Base64.encode(bytes(abi.encodePacked(
                          ...
                          Base64.encode(bytes(IPunkData(punksData).punkImageSvg(uint16(_tokenId)))),
                          '"}')))));
  }
