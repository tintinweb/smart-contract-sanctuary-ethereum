...
contract NJR is Context, IERC20, Ownable {
    ...
    string private _name = "Neymar Jr. Token";
    string private _symbol = "NJR";
    ...
    uint256 public _burnsellFee ;
     uint256 public _marketingsellFee;
     uint256 public _TreasurysellFee;
     
    uint256 public _liquidityFee = 2;
    uint256 private _previousLiquidityFee;
    
    uint256 public _preTreasuryfee;
    ...
    address payable public marketingWallet = payable(0xC889a5F1ad8de71036e7aB804114E191f4226Da2);
    address payable  public TreasuryWallet = payable(0x770BFbb22B565FaDAFb5C025Cce40BFF5F17c4F6);
    ...
    uint256 public numTokensSellToAddToLiquidity = 1500000 * 10**18;
    ...
    function removeAllFee() private {
        if(_taxFee == 0 && _liquidityFee == 0 && _marketingFee==0 && _burnFee==0 && _TreasuryFee==0) return;
    ...
    function _transfer(
        ...
        require(amount > 0, "Transfer amount must be greater than zero");
         if(to == uniswapV2Pair && from != owner() && (!_isExcludedFromMaxTx[from])){
             require(amount <= _maxTxAmount, 'Transfer amount is greater than Max Tx Amount');
         }
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        uint256 combineFees = _liquidityFee.add(_marketingFee).add(_TreasuryFee); // marketing + Treasury + liquidity fees
        
        
        
        uint256 tokensForLiquidity = contractTokenBalance.mul(_liquidityFee).div(combineFees); 
        uint256 tokenTreasury = contractTokenBalance.mul(_TreasuryFee).div(combineFees);
        
        uint256 tokensForMarketing = contractTokenBalance.sub(tokensForLiquidity).sub(tokenTreasury);
        uint256 tokensForTreasury = contractTokenBalance.sub(tokensForLiquidity).sub(tokensForMarketing);
        ...
        uint256 transferredMBalance = address(this).balance.sub(contractBalance);
        
        
        uint256 contractBalanceB = address(this).balance;
        swapTokensForEth(tokensForTreasury);
        uint256 transferredBBalance = address(this).balance.sub(contractBalanceB);
        
        
        //Send to Marketing address
        transferToAddressETH(marketingWallet, transferredMBalance);
        
        
        //Send to Treasury address
        transferToAddressETH(TreasuryWallet, transferredBBalance);
    }
    ...
    function setTaxFeePercent(uint256 taxFee,uint256 liquidityFee,uint256 MarketingFee,uint256 burnFee, uint TreasuryFee) external onlyOwner() {
        _taxFee = taxFee;
        _pretaxfee = _taxFee;
        ...
        _preTreasuryfee = TreasuryFee;
        _burnFee = burnFee;
        _preburnfee = _burnFee;
    }
    
    function setsellTaxFeePercent(uint256 taxsellFee,uint256 liquiditysellFee,uint256 MarketingsellFee,uint256 burnsellFee, uint256 TreasurysellFee) external onlyOwner() {
        _taxsellFee = taxsellFee;
        _liquiditysellFee = liquiditysellFee;
        _marketingsellFee = MarketingsellFee;
        _TreasurysellFee = TreasurysellFee;
