...
contract OriginItems is ERC721Enum, Ownable, ReentrancyGuard {
	using Strings for uint256;
	uint256 public NFT_MINTED;
    string private baseURI;

	address public LootBoxes;
	
    constructor() ERC721P('Crypto Maze Origin Items', 'CM') {}
	...
	function mint(address recipient, uint256 count) external nonReentrant{
	    require(_msgSender() == LootBoxes, "Incorrect Request");
	...
	function setLootBoxesAddress(address newAddress) external onlyOwner {
        require(newAddress != address(0), "Invalid new address");
        LootBoxes = newAddress;
    }
}