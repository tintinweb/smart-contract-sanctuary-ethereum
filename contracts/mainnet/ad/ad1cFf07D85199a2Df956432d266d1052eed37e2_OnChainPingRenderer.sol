...
import "../code/ICode.sol";

contract OnChainPingRenderer is IPingRenderer, Ownable
{
    address public _paramsAddr;
    address public _codeAddr;

    function render(uint256 tokenId, PingAtts memory atts, bool isSample)
        external
        view
        virtual
        override
    ...
    {
        IPingParams pingParams = IPingParams(_paramsAddr);
        ICode code = ICode(_codeAddr);

        string memory paramsStr = pingParams.getParmsSequence(tokenId, atts, isSample);
        string memory htmlStr = code.getCode(paramsStr);
    ...
    function validateContract() external view returns (string memory) {
        if (address(_paramsAddr) == address(0)) {
            return "No params address";
        }
        else {
            return "";
        }
