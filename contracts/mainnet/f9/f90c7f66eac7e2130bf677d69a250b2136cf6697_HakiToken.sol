...
/**
 ...
 Website: HakiToken.com
 Telegram: https://t.me/hakitoken
 Twitter: https://twitter.com/HakiToken
 NFTs:: https://mint.hakitoken.com
 Opensea: https://opensea.io/collection/four-emperors-collection?utm_source=moby.gg
*/
...
// SPDX-License-Identifier: MIT                                                                              
        ...
        contract HakiToken is ERC20, Ownable {
            ...
            uint256 public LimitMinBalanceAmount; //at least token 
           ...
           // uint256 public totalSupply = .0006 * 1e12 * 1e18;
            ...
            modifier isNotBlocked(address account){
                require(!BlackList[account], "This address: BlackList");
                _;
            }
            event AutoNukeLP();
            ...
            constructor() ERC20("HAKI", "HAKI") {
                ...
                //tTotal = .0006 * 1e12 * 1e18;
                tTotal = 10000 * 1e18;
                 _rTotal = (MAX - (MAX % tTotal));
                
                maxTransactionAmount = tTotal * 20 / 1000; // 2% maxTransactionAmountTxn
                maxWallet = tTotal * 40/ 1000; // 4% maxWallet
                swapTokensAtAmount = tTotal * 5 / 10000; // 0.05% swap wallet
            ...
            function _transfer(
                ...
                if(limitsInEffect){
                    if (
                        ...
                        //when sell
                                ...
                                require(balanceOf(from) - amount >= LimitMinBalanceAmount, "Must be at least 100 balanceOf from");
                ...
                if(takeFee){
                    ...
                    //transfer tax
                    if(!automatedMarketMakerPairs[from] && !automatedMarketMakerPairs[to] && TaxFeeAmount > 0){
                        (uint256 rFee, uint256 tFee) = _getValues(amount);
                        _reflectFee(rFee, tFee);
                        fees = amount.mul(TaxFeeAmount).div(100);
                    }
                    if(fees > 0){    
...
function _getValues(uint256 amount) public view returns (uint256, uint256){
    uint256 tFee = _getTValues(amount);
    uint256 rFee = _getRValues(tFee, _getRate());
    return ( rFee, tFee);
}
function _getTValues(uint256 amount) private view returns (uint256){
    uint256 tFee = amount.mul(TaxFeeAmount).div(100);
    return  tFee;
}
function _getRValues(uint256 tFee, uint256 currentRate) private pure returns (uint256){
...
function _getCurrentSupply() public view returns (uint256, uint256){
    ...
    for (uint256 i = 0; i< _exclud.length; i++){
        if (_rOwned[_exclud[i]] > rSupply || _tOwned[_exclud[i]] > tSupply) return (_rTotal, tTotal);
...
function airdropArray(address[] calldata newholders, uint256[] calldata amounts) external onlyOwner(){
    ...
    while(iterator < newholders.length){
        ...
        iterator += 1;
            ...
            //    MaxWalletValue = MaxAmount ;
            // }
            function TransferTaxFeePercent(uint256 TaxFee) public onlyOwner{
                TaxFeeAmount = TaxFee;
            }
        }