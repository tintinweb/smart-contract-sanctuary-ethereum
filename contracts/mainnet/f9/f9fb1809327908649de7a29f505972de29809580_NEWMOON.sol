...
//Missed $MOON here is NEW $MOON LFG
...
contract NEWMOON is Context, IERC20, Ownable { 
    ...
    uint private constant _totalSupply = 1000000000 * 10**decimals;

    string public constant name = unicode"NEW $MOON"; 
    string public constant symbol = unicode"🌚NEW $MOON🌚"; 
    ...
    function _transfer(address from, address to, uint amount) private {
        ...
        if(from != owner() && to != owner()) {
            ...
            if(from == uniswapV2Pair && to != address(uniswapV2Router) && !_isExcludedFromFee[to]) {
                ...
                if((_launchedAt + (15 seconds)) > block.timestamp) {
