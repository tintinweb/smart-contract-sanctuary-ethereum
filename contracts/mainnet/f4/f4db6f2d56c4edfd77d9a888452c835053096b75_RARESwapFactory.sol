...
interface IRARESwapCallee {
    function rareSwapCall(address sender, uint amount0, uint amount1, bytes calldata data) external;
}
...
contract RARESwapERC20 is IRARESwapERC20 {
    using SafeMath for uint;

    string public constant name = 'RARESwap-LP';
    string public constant symbol = 'RARE-LP';
    ...
    function permit(address owner, address spender, uint value, uint deadline, uint8 v, bytes32 r, bytes32 s) external {
        require(deadline >= block.timestamp, 'RARESwap: EXPIRED');
        ...
        require(recoveredAddress != address(0) && recoveredAddress == owner, 'RARESwap: INVALID_SIGNATURE');
...
contract RARESwapPair is IRARESwapPair, RARESwapERC20 {
    using SafeMath  for uint;
    ...
    IRARESwapRouter public routerAddress;
    ...
    modifier lock() {
        require(unlocked == 1, 'RARESwap: LOCKED');
        unlocked = 0;
        _;
        unlocked = 1;
    }
    ...
    function _safeTransfer(address token, address to, uint value, bool isSwapping) private {
        ...
        if (routerAddress.pairFeeAddress(address(this)) == token && isSwapping){
            ...
            if(adminFee != 0){
                ...
                require(success && (data.length == 0 || abi.decode(data, (bool))), 'RARESwap: TRANSFER_FAILED');
                value = value.sub(getOutFee);
            }
            (bool success1, bytes memory data1) = token.call(abi.encodeWithSelector(SELECTOR, to, value));
            require(success1 && (data1.length == 0 || abi.decode(data1, (bool))), 'RARESwap: TRANSFER_FAILED');
        }else {
    ...
    function initialize(address _token0, address _token1) external {
        require(msg.sender == factory, 'RARESwap: FORBIDDEN'); // sufficient check
        token0 = _token0;
        token1 = _token1;
        routerAddress = IRARESwapRouter(IRARESwapFactory(factory).routerAddress());
    }

    // update reserves and, on the first call per block, price accumulators
    function _update(uint balance0, uint balance1, uint112 _reserve0, uint112 _reserve1) private {
        require(balance0 <= uint112(-1) && balance1 <= uint112(-1), 'RARESwap: OVERFLOW');
    ...
    function _mintFee(uint112 _reserve0, uint112 _reserve1) private returns (bool feeOn) {
        address feeTo = IRARESwapFactory(factory).feeTo();
    ...
    function mint(address to) external lock returns (uint liquidity) {
        ...
        require(liquidity > 0, 'RARESwap: INSUFFICIENT_LIQUIDITY_MINTED');
        _mint(to, liquidity);
    ...
    function burn(address to) external lock returns (uint amount0, uint amount1) {
        ...
        require(amount0 > 0 && amount1 > 0, 'RARESwap: INSUFFICIENT_LIQUIDITY_BURNED');
    ...
    function swap(uint amount0Out, uint amount1Out, uint amount0Fee, uint amount1Fee, address to, bytes calldata data) external lock {
        require(amount0Out > 0 || amount1Out > 0, 'RARESwap: INSUFFICIENT_OUTPUT_AMOUNT');
        (uint112 _reserve0, uint112 _reserve1,,) = getReserves(); // gas savings
        require(amount0Out < _reserve0 && amount1Out < _reserve1, 'RARESwap: INSUFFICIENT_LIQUIDITY');

        if(baseToken != address(0)) {
            require(amount0Fee > 0 || amount1Fee > 0, "RARESwap: INSUFFICIENT_FEE");
        }
        ...
        { // scope for _token{0,1}, avoids stack too deep errors
            ...
            require(to != _token0 && to != _token1, 'RARESwap: INVALID_TO');
            if (amount0Out > 0) {
            ...
            if (data.length > 0) IRARESwapCallee(to).rareSwapCall(msg.sender, amount0Out, amount1Out, data);
            balance0 = IERC20(_token0).balanceOf(address(this));
            balance1 = IERC20(_token1).balanceOf(address(this));
        }
        ...
        require(amount0In > 0 || amount1In > 0, 'RARESwap: INSUFFICIENT_INPUT_AMOUNT');
    ...
    function skim(address to) external lock {
        address _token0 = token0; // gas savings
        address _token1 = token1; // gas savings
        _safeTransfer(_token0, to, IERC20(_token0).balanceOf(address(this)), false);
        _safeTransfer(_token1, to, IERC20(_token1).balanceOf(address(this)), false);
    }
    ...
    // Token contract needs to implement a function to set the base token in its Rare-LP pair
    function setBaseToken(address _baseToken) external {
        require( msg.sender == token0 || msg.sender == token1, "RARESwap: NOT_ALLOWED");
        require(_baseToken == token0 || _baseToken == token1, "RARESwap: WRONG_ADDRESS");
    ...
    // This function can ONLY be called by the token contract
    // Token contract needs to implement a function to update fees in its Rare-LP pair
    function updateTotalFee(uint _totalFee) external returns (bool) {
        ...
        require(feeTaker == msg.sender, "RARESwap: NOT_ALLOWED");
...
contract RARESwapFactory is IRARESwapFactory {
    bytes32 public constant INIT_CODE_PAIR_HASH = keccak256(abi.encodePacked(type(RARESwapPair).creationCode));
    ...
    function routerInitialize(address _router) external {
        require(!routerInit, "RARESwap: INITIALIZED_ALREADY");
    ...
    function createPair(address tokenA, address tokenB) external returns (address pair) {
        require(tokenA != tokenB, 'RARESwap: IDENTICAL_ADDRESSES');
        (address token0, address token1) = tokenA < tokenB ? (tokenA, tokenB) : (tokenB, tokenA);
        require(token0 != address(0), 'RARESwap: ZERO_ADDRESS');
        require(getPair[token0][token1] == address(0), 'RARESwap: PAIR_EXISTS'); // single check is sufficient
        bytes memory bytecode = type(RARESwapPair).creationCode;
        bytes32 salt = keccak256(abi.encodePacked(token0, token1));
        assembly {
            pair := create2(0, add(bytecode, 32), mload(bytecode), salt)
        }
        IRARESwapPair(pair).initialize(token0, token1);
    ...
    function setFeeTo(address _feeTo) external {
        require(msg.sender == feeToSetter, 'RARESwap: FORBIDDEN');
        feeTo = _feeTo;
    }
