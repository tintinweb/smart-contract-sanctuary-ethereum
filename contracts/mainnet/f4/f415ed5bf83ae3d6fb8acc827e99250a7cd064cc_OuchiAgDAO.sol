...
abstract contract Transferable is Ownable {
  mapping (address => bool) public whitelist;

  constructor() {}

  modifier canTransfer(address _to) {
    require(!isContractLocal(msg.sender) || whitelist[msg.sender], "Transferable: sender is not whitelisted contract");
    require(!isContractLocal(_to) || whitelist[_to], "Transferable: receiver is not whitelisted contract");
    _;
  }

  function setWhitelist(address _contract) public virtual onlyOwner {
    whitelist[_contract] = true;
  }

  function removeWhitelist(address _contract) external onlyOwner returns (bool) {
    require(whitelist[_contract], "Transferable: _contract is not whitelisted");
    whitelist[_contract] = false;
    return true;
  }

  // Utils
  function isContractLocal(address account) private view returns (bool) {
    uint256 size;
    assembly {
...
// Contract
contract OuchiAgDAO is Mintable, Transferable {
  // ERC721
  ...
  // ERC721Metadata
  string public name = 'OuchiAgDAO';
  string public symbol = 'OAG';
  ...
  // Customized (public)
  function setBaseUrl(string memory _baseUrl) external onlyOwner returns (bool) {
    baseUrl = _baseUrl;
    return true;
  }
  ...
  function bulkMint(address _to, uint256 _count) external onlyOwner returns (bool) {
  ...
  function transferFrom(address from, address to, uint256 tokenId) public canTransfer(to) returns (bool) {
    require(_isApprovedOrOwner(msg.sender, tokenId), "ERC721: transfer caller is not owner nor approved");
    _transfer(from, to, tokenId);
    return true;
  }

  function safeTransferFrom(address from, address to, uint256 tokenId) public canTransfer(to) {
    safeTransferFrom(from, to, tokenId, "");
  }

  function safeTransferFrom(address from, address to, uint256 tokenId, bytes memory _data) public canTransfer(to) returns (bool) {
    require(_isApprovedOrOwner(msg.sender, tokenId), "ERC721: transfer caller is not owner nor approved");
  ...
  function _mint(address _to) private {
    ...
    require(tokenId > 0, "ERC721: tokenId should be non-zero");
  ...
  function tokenURI(uint256 _tokenId) public view returns (string memory) {
    ...
    return string(abi.encodePacked(baseUrl, toString(_tokenId), ".json"));
  }
