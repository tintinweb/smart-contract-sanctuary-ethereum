...
//**  Loobr Marketplace Contract trade using GAS(ETH)*/
...
// File contracts/interfaces/INftWhiteList.sol
...
// File contracts/interfaces/ILoobrConfig.sol



pragma solidity ^0.8.0;

interface ILoobrConfig {
...
// File contracts/LoobrContractBase.sol



//** Marketplace Base Contract, other marketplace contract will
//** inherit this contract */
//** Author ishanshahzad :  Marketplace Base Contract 2022.5 */
...
abstract contract LoobrContractBase is Ownable, ReentrancyGuard, IERC721Receiver {
    enum SellMode {
        FixedPrice,
        Auction
    }
    ...
    ILoobrConfig public loobrConfig;
    ...
    constructor(address _owner, ILoobrConfig _loobrConfig) {
        require(_owner != address(0), "Invalid owner address");
        _transferOwnership(_owner);
        loobrConfig = _loobrConfig;
    ...
    function _addListing(
        ...
        if (_sellMode != SellMode.FixedPrice) {
            ...
            require(
                _duration >= loobrConfig.getMinDuration() && _duration <= loobrConfig.getMaxDuration(),
                "Invalid duration"
            );
        }
    ...
    function _deduceRoyalties(
        ...
        if (royaltiesAmount > 0) {
            uint256 royaltyFee = (royaltiesAmount * loobrConfig.getRoyalityFeeRate()) / FEE_RATE_BASE;
            if (royaltyFee > 0) {
                _transferToken(msg.sender, loobrConfig.getRevenueAddress(), royaltyFee);
...
// File contracts/LoobrMarketPlace.sol



//**  Marketplace Contract trade using GAS(ETH)*/
//** Author ishanshahzad :  Marketplace) Contract 2022.7 */

pragma solidity ^0.8.0;


contract LoobrMarketPlace is LoobrContractBase {
    constructor(
        address _owner,
        ILoobrConfig _loobrConfig
    ) LoobrContractBase(_owner,  _loobrConfig) {}
    ...
    function _processFee(uint256 price) internal override {
        uint256 fee = (price * feeRate) / FEE_RATE_BASE;
        payable(loobrConfig.getRevenueAddress()).transfer(fee);
        revenueAmount += fee;
    }
