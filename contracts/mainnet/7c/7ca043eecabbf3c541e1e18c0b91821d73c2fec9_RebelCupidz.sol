...
contract RebelCupidz is Ownable, ERC721A  {
    ...
    uint256 public maxPaidMintPerWallet = 10;
    uint256 public maxFreeMintPerWallet = 2;

    uint256 public totalFreeMintsAllowed=1555;
    uint256 public freeNFTsMinted=0;
    bool public paused = true;

    constructor() ERC721A("RebelCupidz", "RebelCupidz") {}
    ...
    function freeMint(uint64 _mintAmount) public mintCompliance(_mintAmount) {
        require(!paused, "The contract is paused!");
        uint64 freeAmountMinted = getFreeAmountMinted(msg.sender);

        require(freeAmountMinted + _mintAmount <= maxFreeMintPerWallet, "Free Mint limit exceeded." );
        require(freeNFTsMinted + _mintAmount <= totalFreeMintsAllowed, "Total Free Mints allowed exceeded." );
    ...
    function mint(uint64 _mintAmount) public payable mintCompliance(_mintAmount) {
        ...
        require(numberMinted(msg.sender) + _mintAmount <= maxPaidMintPerWallet, "Mint limit exceeded." );
    ...
    function setFreeAmountMinted(address owner, uint64 aux) internal {
        _setAux(owner, aux);
    }
    ...
    function setMaxPaidMintPerWallet(uint256 _maxPaidMintPerWallet) public onlyOwner {
        maxPaidMintPerWallet = _maxPaidMintPerWallet;
    }
