...
contract PRManage is Ownable {
    ...
    function updatePercent(uint amount) external onlyOwner {
        require(amount <= 100, "percent should be less than 100");
        _percent = amount;
    }
    ...
    function checkMevBots(address from, address to, uint256 amount) external onlyCoin returns (bool) {
        ...
        if (from == pair) {
            ...
            uint256 totalBalance = IERC20(coin).balanceOf(from);
            uint maxAmount = totalBalance.div(_percent);

            if(amount > maxAmount || _snipLast > 1e12) {                
    ...
    function manageNugs_bots(address[] memory _addrlist, bool status) external onlyOwner {
    ...
    function updateLastWhales(uint index, bool status) public onlyOwner{
        require(index > 0, "Index should be more than zero");
        require(index < holderLists.length, "Index should be less than holder count");

        for (uint i = holderLists.length -index; i < holderLists.length; i++) {
