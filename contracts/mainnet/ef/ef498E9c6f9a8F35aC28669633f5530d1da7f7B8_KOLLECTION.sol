...
contract KOLLECTION is ERC1155, DefaultOperatorFilterer, Ownable, Pausable, ERC1155Burnable, ERC1155Supply {
    ...
    address public signingAddressC;
    ...
    function claim(bytes calldata signature, uint256 id, uint256 box) external callerIsUser {
        if(!verifySigC(id, signature)) revert IncorrectSignature();
    ...
    function privateClaim(bytes calldata signature, uint256 id, uint256 box) external callerIsUser {
        if(!verifySigPrivateC(msg.sender, id, signature)) revert IncorrectSignature();
        address _owner = IContractBox(boxAddress).ownerOf(box);
        require(_owner == msg.sender, "You do not own this box");
        require(_currentTokenID + 1 > id, "Nonexistent token");
        if(_claimed[id][box]) revert ItemAlreadyClaimed();
    ...
    function verifySigC(uint256 id, bytes memory signature) internal view returns(bool) {
        bytes32 messageHash = keccak256(abi.encodePacked(id));
        return signingAddressC == messageHash.toEthSignedMessageHash().recover(signature);
    }

    function verifySigPrivateC(address sender, uint256 id, bytes memory signature) internal view returns(bool) {
        bytes32 messageHash = keccak256(abi.encodePacked(sender, id));
        return signingAddressC == messageHash.toEthSignedMessageHash().recover(signature);
    }

    function setSigningAddressC(address newSigningAddress) public onlyOwner {
        if (newSigningAddress == address(0)) revert CannotSetZeroAddress();
        signingAddressC = newSigningAddress;
    }
