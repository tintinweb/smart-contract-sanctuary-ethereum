...
contract DMEX_Base {
    ...
    function depositToken(address token, uint128 amount) public {
        if (destroyed) revert("Contract destroyed");
        if (!Token(token).transferFrom(msg.sender, address(this), amount)) revert("Unable to transfer token, chack allowance"); // attempts to transfer the token to this contract, if fails throws an error
