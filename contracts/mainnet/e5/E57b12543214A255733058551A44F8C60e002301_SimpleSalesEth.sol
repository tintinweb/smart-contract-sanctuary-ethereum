...
contract SimpleSalesEth is Ownable {
        ...
        external
    {
        ...
        _createSale(_nftContractAddress, _tokenId, _erc20Token, _price);
    }

    function createSaleBulk(
        address[] calldata _nftContractAddress,
        uint256[] calldata _tokenId,
        address[] calldata _erc20Token,
        uint256[] calldata _price
    )
        external
    {
        require(sellers[msg.sender], "Unauthorized");
        uint256 length = _nftContractAddress.length;
        require(length == _tokenId.length && length == _erc20Token.length && length == _price.length, "Data missmatch");
        for(uint256 i = 0; i < length; ) {
            _createSale(_nftContractAddress[i], _tokenId[i], _erc20Token[i], _price[i]);
            unchecked {
                i++;
            }
        }
    ...
    function withdrawSale(
        ...
        external
    {
        _withdrawSale(_nftContractAddress, _tokenId);
    }

    function withdrawSaleBulk(
        address[] calldata _nftContractAddress,
        uint256[] calldata _tokenId
    )
        external
    {
        uint256 length = _nftContractAddress.length;
        require(length == _tokenId.length, "Data missmatch");
        for(uint256 i = 0; i < length; ) {
            _withdrawSale(_nftContractAddress[i], _tokenId[i]);
            unchecked {
                i++;
            }
        }
        ...
        payable
    {
        ...
        if (sale.erc20Token != address(0)) { // Check if sale is in ERC20 or in native currency
            ...
            IERC20(sale.erc20Token).safeTransferFrom(
                msg.sender,
                sale.nftSeller,
                sale.price
            );
