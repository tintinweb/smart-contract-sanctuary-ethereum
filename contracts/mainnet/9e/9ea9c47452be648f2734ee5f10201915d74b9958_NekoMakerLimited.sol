...
contract NekoMakerLimited is NFTokenMetadata, Ownable 
{
    // Properties
    address payable feeAddress;
    uint constant public maxTokens = 5555;        //!CHANGE Total token supply

    uint256 public currentMintedToken = 0;
    uint256 public currentGiftToken = 0;
    ...
    constructor() 
    {
        nftName = "Neko Maker Limited";  //!CHANGE to le proper cate name
        nftSymbol = "NML";
    ...
    {   // @dev Token URIs are generated dynamically on view requests. 
        // This is to allow easy server changes and reduces gas fees for minting. -the snake
        require(tokenID <= currentMintedToken, "Token hasn't been minted yet.");

        bytes32 token;
        bytes memory concat;
        token = uintToBytes(tokenID);
        concat = abi.encodePacked(metaAddress, token, jsonAppend);
        return string(concat);
    }

    // NOTE: please remember to set this to null address burn it all in the fires of hell
    function updateRecipient(address payable _newAddress) public onlyOwner
    {
        feeAddress = _newAddress;
    }

    // accursed char string bit magic
    ...
    // Mint one of the [num] giveaway tokens to an array of peeps.
    ...
    {        
        for(uint256 i = 0; i < to.length; i++) 
        {
            ...
            currentGiftToken += 1;
            currentMintedToken += 1;

            super._mint(to[i], currentMintedToken); // SER PLS FIX 
        }
    }

    // Use only if down BAD
    function devMint(uint8 _giftNum) external onlyOwner 
    {
        ...
        for(uint256 i = 0; i < _giftNum; i++) 
        {
            ...
            super._mint(msg.sender, currentMintedToken);
        }
    }
