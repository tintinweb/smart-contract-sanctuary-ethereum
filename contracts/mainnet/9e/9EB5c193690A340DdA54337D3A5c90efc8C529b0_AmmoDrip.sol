...
contract AmmoDrip is Ownable {
    error NotOwnerOrBanditsContract();

    address public onChainBandits;
    address public ammoToken;

    function startDripping(address addr, uint128) external {
        if (msg.sender != onChainBandits && msg.sender != owner())
            revert NotOwnerOrBanditsContract();
            IAmmoToken(ammoToken).startDripping(addr, 1);
    }

    function stopDripping(address, uint128) external view {}
