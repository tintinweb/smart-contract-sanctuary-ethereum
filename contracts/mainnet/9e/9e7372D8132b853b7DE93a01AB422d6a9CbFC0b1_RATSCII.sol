...
contract RATSCII is ERC721AQueryable, Ownable {
  using Strings for uint256;

  uint256 public constant TOTAL_MAX_SUPPLY = 8880;
  ...
  constructor() ERC721A("RATSCII", "RATSCIICII") {}

  modifier callerIsUser() {
    require(tx.origin == msg.sender, 'RATS: The caller is another contract');
    _;
  }
  ...
  modifier underMaxSupply(uint256 _quantity) {
    require(
      ...
      "RATSCII: Purchase exceeds max supply"
    );
    _;
  }

  function mint(uint256 _quantity) external payable callerIsUser underMaxSupply(_quantity) {
    require(balanceOf(msg.sender) < maxPublicMintPerWallet, "RATSCII: Mint limit exceeded");
    require(saleStarted, "RATSCII: Sale has not begun");
    if (_totalMinted() < (TOTAL_MAX_SUPPLY)) {
      if (freeMintCount >= totalFreeMints) {
        require(msg.value >= _quantity * publicTokenPrice, "RATSCII: Send more ETH!");
        ...
        if (_quantity <= _mintableFreeQuantity) {
          ...
          require(
            ...
            "RATSCII: Insufficient ETH"
          );
        }
        _mint(msg.sender, _quantity);
      } else {
        require(msg.value >= (_quantity * publicTokenPrice), "RATSCII: Insufficient ETH");
        _mint(msg.sender, _quantity);
      }
    }
  }
  ...
  function withdrawFunds() external onlyOwner {
    ...
    require(success, "RATSCII: Transfer failed.");
  }
