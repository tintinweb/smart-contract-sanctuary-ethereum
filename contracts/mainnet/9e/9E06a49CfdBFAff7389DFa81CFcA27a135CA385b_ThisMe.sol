...
contract ThisMe is ERC20, Ownable {
    ...
    uint256 public totTax;
    uint256 public totSellTax;
    ...
    constructor(
        ...
        require(
            buyTaxes.length == 3 && _sellTaxes.length == 3,
            "Taxes incorrect"
        );
        _mint(msg.sender, supply);
        excludedFromFees[msg.sender] = true;

        IRouter _router = IRouter(routerAdress);
        address _pair = IFactory(_router.factory()).createPair(
            address(this),
            _router.WETH()
        );
        taxes = Taxes(buyTaxes[0], buyTaxes[1], buyTaxes[2]);
        sellTaxes = Taxes(_sellTaxes[0], _sellTaxes[1], _sellTaxes[2]);
        for (uint256 i = 0; i < buyTaxes.length; i++) {
            totTax += buyTaxes[i];
        }
        for (uint256 i = 0; i < _sellTaxes.length; i++) {
            totSellTax += _sellTaxes[i];
        }
