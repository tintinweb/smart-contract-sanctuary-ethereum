...
contract BullsIncNFT is ERC721Enumerable, Ownable {
  using Strings for uint256;
  ...
  // allow burning - and for future use - like fusion
  ...
  //only team
  function mint_promo(uint256 _mintAmount, address _recipient) public onlyTeam {
    require(_mintAmount > 0, "Can't mint 0");
    ...
    for (uint256 i = 1; i <= _mintAmount; i++) {
      _safeMint(_recipient, maxSupply + initialSupply +i);
    }
