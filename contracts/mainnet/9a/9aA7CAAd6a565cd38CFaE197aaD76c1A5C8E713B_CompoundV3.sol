...
import "./interfaces/ICurvePoolCompound.sol";
...
contract CompoundV3 is OwnableUpgradeable, ISubStrategy {
    ...
    */
    ...
    function _deposit(uint256 _amount) internal returns (uint256) {
        ...
        ICurvePoolCompound(curvePool).add_liquidity(amounts, minOutput);
    ...
    function withdraw(uint256 _amount) external override onlyController returns (uint256) {
        ...
        ICurvePoolCompound(curvePool).remove_liquidity_one_coin(lpWithdrawn, tokenId, minAmt);
    ...
    function withdrawable(uint256 _amount) external view override returns (uint256) {
        ...
        uint256 lpAmt = (totalLP * _amount) / total;
        uint256 expectedOutput = ICurvePoolCompound(curvePool).calc_withdraw_one_coin(lpAmt, tokenId);
