...
contract NemoLandV2 is Admin, ERC721Upgradeable {
    struct OnSaleLand {
        uint256 price;
        address saleOwner;
    ...
    event mintDiscount(
        uint256 indexed state,
        uint256 indexed land,
        uint256 indexed orignalPrice,
        uint256 discountedPrice,
        uint256 currencyType,
        uint8 discountPercentage 
    );
    modifier onlySigner() {
        require(msg.sender == signer_, "ONLY SIGNER");
        _;
    }
    ...
    /// @notice sets Zone price in HZM Coin
    /// @param zoneId price of Land in particular Zone
    /// @param amount price of Land in particular Zone particular currency
    function setZonePriceInHZMCoin(
        uint256 zoneId,
        uint256 amount
    ) external onlySigner {
        // currency type should be HZM COIN only
        uint8 HZM = 3;
        AllowedCrypto[HZM].zoneIdToPrice[zoneId] = amount;
        emit ZonePrice(zoneId, amount, HZM);
    }
    ...
    function mintLand(
        ...
        // orignal amount before discountPercentage
        uint256 orgPrice = 0;
        // amount after applying discountPercentage
        ...
        for (counter = 0; counter < _zoneId.length; counter++) {
            ...
            orgPrice =
                ...
                _zoneId[counter][1];
            landsCount = landsCount + _zoneId[counter][1];
        }


        // calculate discount if applied
        if(applyDiscount){
            discountPercentage = getDiscount(msg.sender, currencyType);
            discountedPrice = orgPrice - orgPrice*discountPercentage/100;
        }else{
            discountedPrice = orgPrice;
        }

        require(lands.length == landsCount, "Invalid Lands Count");
        ...
        if (1 == currencyType) {
            require(msg.value >= discountedPrice, "not enough eth sent");
            uint256 leftOver = msg.value - discountedPrice;

            if (leftOver > 0) {
                payable(msg.sender).transfer(leftOver);
            }
            payable(receivingWallet).transfer(discountedPrice);
        } else {
            require(_amount >= discountedPrice, "not enough eth sent");

            if (AllowedCrypto[currencyType].isReturnsBool) {
                IMyERC20 paytoken = IMyERC20(
                    AllowedCrypto[currencyType].paytoken
                );
                paytoken.transferFrom(msg.sender, receivingWallet, discountedPrice);
            } else {
        ...
        if (state > 0) {
            stateToLands[state] = lands;
              // emitted once in case of state
            emit mintDiscount(state, state, orgPrice, discountedPrice, currencyType, discountPercentage);
            _mint(msg.sender, state);
            for (counter = 0; counter < lands.length; counter++) {
                landToState[lands[counter]] = state;
                emit LandMint(state, lands[counter], referral, currencyType, orgPrice, discountedPrice, discountPercentage);
            }
        } else {
            for (counter = 0; counter < lands.length; counter++) {
                // emitted multiple times in case of lands
                emit mintDiscount(state, lands[counter], orgPrice, discountedPrice, currencyType, discountPercentage);
    ...
    /*
    * Function to calculate discountPercentage
    */
    function getDiscount(address _sender, uint256 _currenyType) internal view returns (uint8){
               // get balance of HZM Coin of owner
        IMyERC20 HZM_COIN = IMyERC20(
            // HZM Coin Address 
                AllowedCrypto[3].paytoken
        );

        uint256 hzmBalance = HZM_COIN.balanceOf(_sender);
        uint8 discountPercentage = 0;

        // discountPercentage when buing in USDT 
        if(_currenyType == 2){

            if( hzmBalance >= (1000000 * (10**8))){
                discountPercentage = 30;
            }else if( hzmBalance >= (100000 * (10**8))){
                discountPercentage = 20;
            }else if( hzmBalance >= (20000 * (10**8))){
                discountPercentage = 15;
            }else if(hzmBalance >= (5000 * (10**8))){
        ...
        // discountPercentage when buing in HZM 
        if(_currenyType == 3){
