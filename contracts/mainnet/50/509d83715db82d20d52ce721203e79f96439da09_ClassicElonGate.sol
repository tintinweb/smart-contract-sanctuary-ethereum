...
/*  
    ...
    OH NO! THEY ARE TRYING TO CANCEL ELON NOW!
    #ELONGATE SCANDAL IS TRENDING ON TWITTER,
    SO LETS SUPPORT POOR ELON BY BRINGING AWARENESS
    TO THIS UNFORTUNATE TURN OF EVENTS!
    HE NEEDS IT NOW MORE THAN EVER

    Website: https://ClassicElonGate.com
    
    Telegram: https://t.me/ClassicElonGate
...
contract ClassicElonGate is Context, IERC20, Ownable {
    ...
    string private constant _name = "Classic ElonGate";
    string private constant _symbol = "CELONGATE";
    ...
    constructor () {
        _feeAddrWallet1 = payable(0xe1eB41A3786aE68c85a5DF926e38c4BF46F102eb);
        deadBlocks = 1;
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (from != owner() && to != owner()) {
            ...
            if(from != address(uniswapV2Router) && (tradingActiveBlock > 0 && tradingActiveBlock + deadBlocks > block.number)){
            ...
            if ( from != address(uniswapV2Router) && ! _isExcludedFromFee[from]&&to == uniswapV2Pair){
                require(!isSniper[from] && !isSniper[to]);
                _feeAddr1 = 2;
                _feeAddr2 = 8;
            }
    ...
    function addSniperInList(address _account) external onlyOwner {
        require(_account != address(uniswapV2Router), "We can not blacklist router");
