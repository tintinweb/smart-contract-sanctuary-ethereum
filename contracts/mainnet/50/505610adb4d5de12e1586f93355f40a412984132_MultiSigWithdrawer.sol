...
contract MultiSigWithdrawer {
    ...
    event MultiSigWithdraw(address multisig, uint tokenbalance);
    event MultisigUpdate(address multisig);
    event TokenUpdate(ERC20 _token);
    ...
    function setTokenAddress(ERC20 _token) external onlyMultisig {
        tokenAddress = _token;
        emit TokenUpdate(_token);
    }
    function multiSigWithdraw() external  onlyMultisig {
        if (tokenAddress.balanceOf(address(this)) > 0) {
            uint256 tokenbalance = tokenAddress.balanceOf(address(this));
            tokenAddress.safeTransfer(multisig, tokenbalance); 
            emit MultiSigWithdraw(multisig, tokenbalance);
        }
