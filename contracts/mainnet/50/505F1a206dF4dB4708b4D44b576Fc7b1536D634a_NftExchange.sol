...
contract NftExchange is Ownable, ExchangeDomain {
    ...
    function exchange(
        ...
        require(order.expired > block.timestamp, "order expired");
    ...
    function validateBuyerFeeSig(
        ...
        require(prepareBuyerFeeMessage(order, buyerFee).toEthSignedMessageHash().recover(sig.v, sig.r, sig.s) == buyerFeeSigner, "incorrect buyer fee signature");
    }
    ...
    function transferWithFees(Asset memory firstType, uint value, address from, address to, uint256 sellerFee, uint256 buyerFee, Asset memory secondType) internal {
        ...
        if (
            secondType.assetType == AssetType.ERC1155 && IERC165(secondType.token).supportsInterface(_INTERFACE_ID_FEES) ||
            (secondType.assetType == AssetType.ERC721 || secondType.assetType == AssetType.ERC721Deprecated) && IERC165(secondType.token).supportsInterface(_INTERFACE_ID_FEES)
        ) {
