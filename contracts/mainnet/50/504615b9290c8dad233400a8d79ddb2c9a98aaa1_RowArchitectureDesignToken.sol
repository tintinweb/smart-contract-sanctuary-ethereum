    ...
    ReentrancyGuard
{
    ...
    uint256[maxSupply] private publicPrices; // TODO: Probably better to make a fixed size array of length 30
     ...
     */
    function setPublicSale(bool active) external onlyAdminRole {
     ...
     */
    function mint(address to, uint256 tokenId, uint256 paymentAmount) isValidToken(tokenId) external payable nonReentrant {
        // 1. Require public sale to be live, token to not exist, the public price to be set
        require(isPublicSaleActive, "RowADT: Public minting not permitted at this time.");
        ...
        require(publicPrices[tokenId] <= paymentAmount, "RowADT: Insufficient payment.");
        ...
        if (msg.value > 0) { // If msg has value, estimate value in USDC. If 5% greater than sale price, convert to USDC
            ...
            require(valueInUSDC >= Math.mulDiv(salePrice, 21, 20), "RowADT: Insufficient payment post conversion."); // NOTE: Bit of a hacky way to get 5%

            uint deadline = block.timestamp + 120; // TODO: Review 2 minute deadline
...
library OracleLibrary {
    ...
    function getBlockStartingTickAndLiquidity(address pool) internal view returns (int24, uint128) {
        ...
        tick = int24((tickCumulative - int56(int32(prevTickCumulative))) / int56(int32(delta)));
        uint128 liquidity =
            uint128(
        ...
        returns (int24 weightedArithmeticMeanTick)
    {
        ...
        for (uint256 i; i < weightedTickData.length; i++) {
            // int256(uint128(weightedTickData[i].weight))
            numerator += int256(uint256(uint24(int24(weightedTickData[i].tick))) * uint256(uint256(weightedTickData[i].weight)));
