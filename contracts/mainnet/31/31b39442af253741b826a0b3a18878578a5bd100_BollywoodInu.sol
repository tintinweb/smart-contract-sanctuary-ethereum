...
// Official Telegram ----- t.me/BollyWoodInu
...
contract BollywoodInu  is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;

    uint8 private _decimals = 9;

    string private _name = "BollyWood Inu";                                          
    string private _symbol = "BWI";                                           
    ...
    uint256 public _marketingFee = BuyFee;
    uint256 private _previousMarketingFee = _marketingFee;
    uint256 public SellFee = 10;

    bool public feesOnSellersAndBuyers = true;

    uint256 public _maxTxAmount = _tTotal.div(1).div(133);
    uint256 public numTokensToExchangeForMarketing = _tTotal.div(100).div(100);
    address payable public marketingWallet = payable(0x61888c5C2430eAf1D9dFDd9b8e6e3cDC0bbfd85c);
    ...
    function setFees(address recipient) private {
        ...
        _marketingFee = BuyFee;
        if (recipient == uniswapV2Pair) { // sell
            _marketingFee = SellFee;
        }
    }
    ...
    function setBuyFee(uint256 marketingFee) external onlyOwner() {
        BuyFee = marketingFee;
    }

    function setSellersFee(uint256 sellersfee) external onlyOwner() {
        SellFee = sellersfee;
    }
