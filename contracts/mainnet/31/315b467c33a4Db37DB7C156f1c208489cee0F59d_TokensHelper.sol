...
contract TokensHelper is Ownable {
    address public registry;

    mapping(address => bool) public externalToken;

    event ExternalTokensUpdated(
    ...
    function setExternalTokens(address[] calldata tokens, bool add) external onlyOwner()
    {
        require(tokens.length > 0, "!tokens");
        uint256 length = tokens.length;
        for (uint256 index = 0; index < length; index++) {
            externalToken[tokens[index]] = add;
            emit ExternalTokensUpdated(tokens[index], add);
        }
    }
