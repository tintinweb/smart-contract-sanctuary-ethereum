...
contract MagicSoup is ERC721, Pausable, AccessControl, ERC721Burnable, Ownable, DefaultOperatorFilterer {
    using Strings for uint256;
    ...
    constructor(
        ...
        // 발생시킨 사람(msg.sender) 에게 admin, 정지, mint 권한 부여
        _grantRole(DEFAULT_ADMIN_ROLE, msg.sender);
        _grantRole(PAUSER_ROLE, msg.sender);
        _grantRole(MINTER_ROLE, msg.sender);
        // token URI의 base가 되는 URI
    ...
    function _mintToken(address newOwner, uint256 amount) internal {
        for (uint256 i = 0; i < amount; i++) {
            _mint(newOwner, nextMintIndex);
            nextMintIndex += 1;
        }
    }

    struct MappedAirdrop {
        address receiver;
        uint256 tokenId;
    }
    
    function mappedAirdrop(MappedAirdrop[] calldata recievers) external onlyOwner {
        for (uint256 i = 0; i < recievers.length; i++) {
            _mint(recievers[i].receiver, recievers[i].tokenId);
        }
    }

    function airdrop(address[] memory targetAddresses) external {
        // 순서대로 token mint
        for (uint256 i = 0; i < targetAddresses.length; i++) {
            _mintToken(targetAddresses[i], 1);
        }
    }
    ...
    function mintPublicSale(uint256 _mintAmount) external payable {
        ...
        require(_totalSupply + _mintAmount <= publicMintLimit, "Cannot mint the beyond max publicMintLimit");
        // 남는 value가 donation되는 값
        require(publicMintPrice * _mintAmount <= msg.value, "Payment is below the price");

        _mintToken(msg.sender, _mintAmount);

        // 설정된 addr로 입력된 value만큼 전송됨
        // msg.value : 송금 보낸 코인의 값, message와 함께 보낸 wei의 값
        payable(_withdrawAddress).transfer(msg.value);
    }

    // public sale 진행 시 받을 값(==가격) 설정
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        // tokenURI = str(baseURI + tokenID)
        // tokenURI : 조회 함수, tokenURI를 이용해 data를 가져옴
        require(_exists(tokenId), "Invalid token ID");

        uint256 shiftedTokenId = ((tokenId + _tokenOffset) % _maxSupply);
        string memory baseURI = _baseURI();
