...
contract Hoichi is ERC20 {
    ...
    function _swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        uint256 liquidityBalance = contractTokenBalance -
            _totalDevelopmentAmount;
        _swapTokensForEth(_totalDevelopmentAmount, developmentWallet); // swaps dev tax collection amount
        _totalDevelopmentAmount = 0; // resets dev tax collection amount after swap
    ...
    function _swapTokensForEth(uint256 tokenAmount, address recipient) private {
    ...
    function _transfer(
        ...
        if (
            ...
            if (
                ...
                _swapAndLiquify(_numTokensSellToAddToSwap);
            }
            if (_isExcludedFromFee[from] || _isExcludedFromFee[to]) {
                ...
                if (from == uniswapV2Pair) {
                    ...
                    uint256 developmentAmount = ((amount * sellDevelopmentTax) /
                        100);
                    _totalDevelopmentAmount += developmentAmount; // keeps track of dev tax collection until swapped
                    transferAmount =
                        amount -
                        liquidityAmount -
                        developmentAmount;
