...
// File: contracts/ERC1155TokenController.sol
...
contract ERC1155TokenController is Controller {
    ...
    constructor(address token, address signer) Controller(signer) {
        require(token != address(0), "ERC1155TokenController: token must be a valid address");
        require(token.isContract(), "ERC1155TokenController: token must be a contract");
    ...
    function mintNonFungibleBatch(uint256[] calldata identifiers, uint256[] calldata checksums, address beneficiary, uint256 timestamp, bytes calldata signature) external onlyValidSignatureAndTimestamp(signature, timestamp) {
        require(identifiers.length > 0, "ERC1155TokenController: at least one identifier is required");
        require(identifiers.length == checksums.length, "ERC1155TokenController: length of identifiers and checksums must be the same");
