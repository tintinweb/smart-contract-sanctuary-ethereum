...
contract TheShrapes is ERC721A, Ownable, ReentrancyGuard {
  
    address private _AdventureContract;
    uint   private _totalStake;
    bool   public QuestPhase = false;
    uint128 internal _burnCounter; 
    uint   public price             = 0.005 ether;
    uint   public maxTx          = 30;
    uint   public maxFreePerWallet  = 1;
    uint   public maxShrapes          = 10000;
    ...
    constructor() ERC721A("The Shrapes", "SHRAPE"){}
    ...
    function shrapeBurn(uint256 tokenId) internal virtual {
        TokenOwnership memory prevOwnership = ownershipOf(tokenId);
        ...
        // Donkey Metamorphosis unlocked after burn phase
    ...
    function reservedMint(uint256 Amount) external onlyOwner
    {
        ...
        require(totalSupply() + Amount <= maxShrapes, "No more shrapes to Be minted");
    ...
    function mint(uint256 Amount) external payable {
        ...
        if (((totalSupply() + Amount < maxShrapes + 1) && (_FreeMinted[msg.sender] < maxFreePerWallet))) 
        {
        require(totalSupply() + Amount <= maxShrapes, "No more shrapes to be minted");
        require(mintEnabled, "Not live yet, shrapes are coming");
    ...
    function setAdventureContract(address _contract) public onlyOwner {
        _AdventureContract = _contract;
    }

    function toggleQuestPhase() public onlyOwner {
