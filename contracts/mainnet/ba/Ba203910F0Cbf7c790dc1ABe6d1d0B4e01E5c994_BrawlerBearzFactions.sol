...
contract BrawlerBearzFactions is ERC1155, Ownable {
     ...
     */
    function claimAsOwner(address to, uint256 _factionId) external onlyOwner {
        require(validFactions[_factionId], "Invalid faction.");
        _mint(msg.sender, _factionId, 1, "");
    }
     ...
     * @notice Revokes a faction association by burning the NFT
     */
    function revokeAsOwner(address to) external onlyOwner {
        Faction storage instance = factions[to];
        require(instance.faction > 0, "Not part of faction.");
        _burn(to, instance.faction, 1);
    }

    /**
     * @notice Add a faction as owner of ecosystem
     */
    function addFaction(uint256 factionId) external onlyOwner {
        validFactions[factionId] = true;
    }

    /**
     * @notice Remove a faction as owner of ecosystem
     */
    function removeFaction(uint256 factionId) external onlyOwner {
        validFactions[factionId] = false;
    }
