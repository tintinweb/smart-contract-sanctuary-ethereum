...
contract NFTInsight is ERC721A, Ownable, DefaultOperatorFilterer {
    ...
    address public receivingWallet = 0x0Bd2364b44dB9c1AbEA219d0d4AfF8Dc75e41506;
    
    string public tokenBaseUrl = "https://bafybeiehtkmrfwztilo7etxyfikc6vpfgniahdzu5z2puq4yndhhdpuuti.ipfs.w3s.link/metadata/";
    string public tokenUrlSuffix = ".json";

    constructor () ERC721A("NFTInsight Lifetime Pass", "NFTInsight") {
    }
    ...
    function setReceiving(address _receivingWallet) external onlyOwner {
    ...
    function WhitelistMint(uint256 _numTokens, bytes32[] memory _proof) external payable onlyOrigin wlmintCompliance(_numTokens) {
        require(isValid(_proof , keccak256(abi.encodePacked(msg.sender))), "You are not in the Whitelist.");
        _safeMint(msg.sender, _numTokens);
    ...
    modifier mintCompliance(uint256 _numTokens) {
        ...
        require(_numTokens > 0, "You must mint at least one token.");
        if(totalSupply() + _numTokens < 2000){
            require(msg.value == (WL_MINT_PRICE * _numTokens), "Value supplied is incorrect");
        }else{
