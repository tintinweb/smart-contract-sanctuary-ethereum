...
contract DvaultBondingCalculator is IBondingCalculator {
    ...
    function markdown( address _pair ) external view returns ( uint ) {
        ...
        uint reserve;
        if ( IUniswapV2Pair( _pair ).token0() == address(Dvault) ) {
            reserve = reserve1;
        } else {
            require(IUniswapV2Pair( _pair ).token1() == address(Dvault), "not a Dvault lp pair");
            reserve = reserve0;
        }
        return reserve.mul( 2 * ( 10 ** Dvault.decimals() ) ).div( getTotalValue( _pair ) );
    }
