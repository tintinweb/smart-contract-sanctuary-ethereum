...
📱 TELEGRAM : https://t.me/BankOfLINU

Total Tax : 8% with 4% LINU rewards
...
contract DividendDistributor is IDividendDistributor {
    ...
    address public immutable LINU = address(0x78132543D8E20D2417d8a07D9ae199D458A0D581); //UNI
    ...
    function distributeDividend(address shareholder) internal {
        ...
        if(amount > 0){
            ...
            shares[shareholder].totalExcluded = getCumulativeDividends(shares[shareholder].amount);
            IERC20(LINU).transfer(shareholder, amount);
        }
    }
    ...
    function getDividendsClaimedOf (address shareholder) external view returns (uint256) {
        require (shares[shareholder].amount > 0, "You're not a Claim LINU shareholder!");
        return shares[shareholder].totalClaimed;
    }

    }

contract BankOfLINU is IERC20, Auth {
    using SafeMath for uint256;
    ...
    string private constant  _name = "BankOfLINU";
    string private constant _symbol = "BOLINU";
    ...
    function swapBack() internal swapping {
        ...
        swapTokensForLINU(amountToSwap.div(2));

        uint256 dividends = IERC20(LINU).balanceOf(address(this));

        bool success = IERC20(LINU).transfer(address(distributor), dividends);
    ...
    function swapTokensForLINU(uint256 tokenAmount) private {
