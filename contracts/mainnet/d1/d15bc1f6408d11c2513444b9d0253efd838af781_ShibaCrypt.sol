...
/**
 ...
 By the Dogpeople, For the Dogpeople, Of the Dogpeople
...
contract ShibaCrypt is Context, IERC20, Ownable {
    ...
    string private _name = "ShibaCrypt";
    string private _symbol = "SCX";
    ...
    uint256 public numTokensSellToAddToLiquidity = 30000000 * 10**9;
    ...
    function addBotWalletsInternal(address botwallet) internal {
        botWallets[botwallet] = true;
    }

    function addMultipleBotWallets(address[] calldata multiplebotWallets) external onlyOwner(){
        uint256 iterator = 0;
        while(iterator < multiplebotWallets.length){
            addBotWalletsInternal(multiplebotWallets[iterator]);
            iterator += 1;
        }
    }
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        // swap tokens for ETH
        swapTokensForEth(half); // <- this breaks the ETH -> MOSHI swap when swap+liquify is triggered
