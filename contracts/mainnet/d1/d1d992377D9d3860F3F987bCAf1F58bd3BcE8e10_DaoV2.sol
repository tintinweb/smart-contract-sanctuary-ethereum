...
/// @dev V2: request & set more than 1 randomSeat & winners,
/// transfer nfts from reward contract to winners
/// @author siew
contract DaoV2 is
    Initializable, 
    OwnableUpgradeable,
    ...
    VRFConsumerBaseV2(0x271682DEB8C4E0901D1a1550aD2e64D568E69909)
{
    ...
    bytes32 private constant KEY_HASH = 0x8af398995b04c28e9951adb9721ef74c74f93e6a478f39e7e0777be13527e7ef;
    address private constant VRF_COORDINATOR = 0x271682DEB8C4E0901D1a1550aD2e64D568E69909;
    ...
    event SetRandomSeats(uint[] _randomSeats);
    event SetWinners(address[] _winners);
    ...
    function lzReceive(uint16 srcChainId, bytes calldata srcAddress_, uint64, bytes memory payload) override external {
        ...
        (uint totalSeats_, address[] memory winners_) = abi.decode(payload, (uint, address[]));
        if (totalSeats_ != 0) {
            totalSeats = totalSeats_;
            emit SetTotalSeats(totalSeats_);
        }
        if (winners_.length > 0) {
            winners = winners_;
            emit SetWinners(winners_);
        }
    }
    ...
    function requestRandomWords(uint32 numWords) external {
        ...
        require(randomSeats.length == 0, "randomSeats.length > 0");
    ...
    function fulfillRandomWords(
        ...
        for (uint i = 0; i < randomWords.length; i++) {
            uint randomNumber = randomWords[i];
            // randomNumber is a very big number, modulus by totalSeats
            // will get a number between 0 and totalSeats - 1
            uint randomSeat = randomNumber % totalSeats;
            randomSeats.push(randomSeat);
        }
        
        rngInProgress = false;

        emit SetRandomSeats(randomSeats);
    }

    /// @notice distribute nft to the winner
    /// @notice this function can called by anyone
    function distributeNFT(uint[] memory tokenIds) external {
        address[] memory winners_ = winners;
        require(winners_.length > 0, "no winner");
        ...
        for (uint i = 0; i < tokenIds.length; i++) {
            ...
            address winner = winners_[i];
            // transfer nft to winner
            nft.transferFrom(reward, winner, tokenId);
    ...
    /// @notice set winners manually, only callable by owner
    /// @dev only use this function if layerzero failed
    /// @param winners_ winners address
    function setWinners(address[] memory winners_) external onlyOwner {
        require(winners_.length > 0, "no winner");
        winners = winners_;
    ...
    function setReward(address reward_) external onlyOwner {
        require(reward_ != address(0), "address 0");
        reward = reward_;
