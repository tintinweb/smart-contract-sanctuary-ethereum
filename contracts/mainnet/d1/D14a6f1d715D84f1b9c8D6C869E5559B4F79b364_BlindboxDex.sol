...
contract BlindboxDex is Ownable, DexStorage, SecondDexStorage {
 ...
 event OnAuction(address indexed seller, address nftContract, uint256 indexed tokenId, uint256 startPrice, uint256 endTime, uint256 baseCurrency);
    ...
    function getTokenSeriesId(uint256 tokenId, address collection) public view returns(uint256) {
        return seriesIds[tokenId][collection];
    }

    function isNFTAvailableForSale(uint256 tokenId, address collection) public view returns(bool) {
        return blindNFTOpenForSale[seriesIds[tokenId][collection]];
    }
