...
contract GenesisOfRealm is ERC721A, Ownable {
    ...
    uint256 public constant SALE_WL_PRICE = 0.035 ether;
    uint256 public constant SALE_PRICE = 0.045 ether;
    uint256 public totalMintSupply = 0;
    ...
    bytes32 private merkleRootWait;
    // Minted count per stage per wallet.
    mapping(uint256 => mapping(address => uint256)) private _stageMintedCountsPerWallet;
    mapping(address => uint256) public totalMint;

    constructor(bytes32 _merklerootWhite, bytes32 _merklerootWait)
        ERC721A("Genesis Of Realm", "GOR")
    {
        merkleRootWhite = _merklerootWhite;
        merkleRootWait = _merklerootWait;
    }
    ...
    function mint(uint256 _quantity) external payable callerIsUser {
        ...
        require((totalMintSupply + _quantity) <= MAX_SUPPLY, "Beyond Max Supply");        
        require(msg.value >= (SALE_PRICE * _quantity), "Payment is below the price");

        _stageMintedCountsPerWallet[2][msg.sender] += _quantity;
        totalMint[msg.sender] += _quantity;
        totalMintSupply += _quantity;
    ...
    function whitelistMint(bytes32[] memory _merkleProof, uint256 _quantity) external payable callerIsUser {
        ...
        _stageMintedCountsPerWallet[0][msg.sender] += _quantity;
        totalMint[msg.sender] += _quantity;
        totalMintSupply += _quantity;
        emit Mint(msg.sender, _quantity);
    }

    function waitlistMint(bytes32[] memory _merkleProof, uint256 _quantity) external payable callerIsUser {
        require(whiteListSale, "Waitlist Sale Is Not Active.");
        require((totalMintSupply + _quantity) <= MAX_SUPPLY,"Beyond Max Supply");
        require((totalMint[msg.sender] + _quantity) <= MAX_MINT,"Beyond waitlist max mint!");
        require(msg.value >= (SALE_WL_PRICE * _quantity),"Payment is below the price");

        bytes32 sender = keccak256(abi.encodePacked(msg.sender));
        require(MerkleProof.verify(_merkleProof, merkleRootWait, sender),"You are not waitlisted");

        _stageMintedCountsPerWallet[1][msg.sender] += _quantity;
        totalMint[msg.sender] += _quantity;
        totalMintSupply += _quantity; 
        emit Mint(msg.sender, _quantity);
    }
    ...
    function sendMintTokens(uint256 mintStage, address to) public onlyOwner {
        uint256 qty = stageReservations(mintStage, to);
        
        if((totalSupply() + qty) > MAX_SUPPLY) return;
        if (qty < 1) return;
        _safeMint(to, qty);
        _stageMintedCountsPerWallet[mintStage][to] -= qty;
    }
    ...
    function isValidWaitList(bytes32[] memory proof, bytes32 leaf) public view returns (bool)
    {
        return MerkleProof.verify(proof, merkleRootWait, leaf);
    }
    ...
    function setMerkleRootWait(bytes32 _merkleRoot) external onlyOwner {
        merkleRootWait = _merkleRoot;
    }
    ...
    function getMerkleRootWait() external view returns (bytes32) {
        return merkleRootWait;
    }
