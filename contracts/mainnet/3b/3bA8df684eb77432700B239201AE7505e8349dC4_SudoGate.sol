...
contract SudoGate {
    ...
    */
    ...
    function adjustBuyPrice(uint256 price) public view returns (uint256 adjustedPrice) {
        /* add fees to the price so you know how much ETH to send when you call a buy function */
        uint256 priceWithContributorFee = price * (1000 + contributorFeePerThousand) / 1000; 
        adjustedPrice = priceWithContributorFee * (1000 + protocolFeePerThousand) / 1000;
    }
    
    function adjustSellPrice(uint256 price) public view returns (uint256 adjustedPrice) {
    ...
    function buy(address nft) public payable returns (uint256 tokenID) {
        ...
        uint256 adjustedPrice = adjustBuyPrice(bestPrice);
    ...
    function buyQuoteWithFees(address nft) public view returns (uint256 bestPrice, address bestPool) {
        ...
        */ 
        (bestPrice, bestPool) = buyQuote(nft);
        bestPrice = adjustBuyPrice(bestPrice);
    }
    ...
    function sellQuoteWithFees(address nft) public view returns (uint256 bestPrice, address bestPool) {
        ...
        */ 
        (bestPrice, bestPool) = sellQuote(nft);
        bestPrice = adjustSellPrice(bestPrice);
    }
