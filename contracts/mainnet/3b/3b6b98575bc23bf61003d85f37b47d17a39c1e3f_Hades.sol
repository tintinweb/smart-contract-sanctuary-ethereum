...
contract Hades is Context, IERC20, Ownable {
   string private constant _name = "Hader";
   string private constant _symbol = "HDR";
   uint8 private constant _decimals = 18;

   address public constant deadAddress =
       0x000000000000000000000000000000000000dEaD;
   address payable public autoLiquidityReceiver =
       payable(0x000000000000000000000000000000000000dEaD); // Auto LP Receiver Address
   address payable public marketingWalletAddress =
       payable(0x25B005558ccffd5551531bd008f54cDd5a3f9cEb); // Marketing Address
   address payable public blackListFeeReceiver =
       payable(0x21Ca50514525B12e1b9bAC578fc8e017611129b4); // Black List Fee Receiver Address
   ...
   uint256 public buyTax = 777;
   uint256 public sellTax = 777;

   uint256 public lpShare = 300;
   uint256 public marketingShare = 400;
   uint256 public autoBurnShare = 77;

   uint256 private constant _totalSupply = 7777777 * 10**_decimals;
   uint256 public swapThreshold = 1000 * 10**_decimals;
   uint256 public maxTxAmount = 33333 * 10**_decimals;
   uint256 public walletMax = 77777 * 10**_decimals;
   ...
   event BlackListFeeReceiverChanged(address operationsWalletAddress_);
   ...
   constructor() {
       ...
       isExcludedFromFee[address(blackListFeeReceiver)] = true;

       isTxLimitExempt[owner()] = true;
       isTxLimitExempt[address(this)] = true;
       isTxLimitExempt[address(marketingWalletAddress)] = true;
       isTxLimitExempt[address(blackListFeeReceiver)] = true;
       ...
       isWalletLimitExempt[address(blackListFeeReceiver)] = true;
       ...
       returns (address newPairAddress)
   {
       ...
       newPairAddress = IDexFactory(dexRouter_.factory()).getPair(
           address(this),
           dexRouter_.WETH()
       );
       ...
       if (newPairAddress == address(0)) {
           ...
           newPairAddress = IDexFactory(dexRouter_.factory()).createPair(
               address(this),
               dexRouter_.WETH()
           );
       }
   ...
   function setBlackListFeeReceiver(address blackListFeeReceiver_)
       external
       onlyOwner
   {
       require(
           blackListFeeReceiver_ != address(0),
           "New address cannot be zero address"
       );
       blackListFeeReceiver = payable(blackListFeeReceiver_);
       emit BlackListFeeReceiverChanged(blackListFeeReceiver_);
   }

   function setAutoLiquidityReceiver(address autoLiquidityReceiver_)
       external
       onlyOwner
   {
       require(
           autoLiquidityReceiver_ != address(0),
           "New address cannot be zero address"
       );
   ...
   function takeFee(
       ...
       if (isEarlyBuyer[sender] || isEarlyBuyer[recipient]) {
           ...
           feeReceiver = blackListFeeReceiver;
