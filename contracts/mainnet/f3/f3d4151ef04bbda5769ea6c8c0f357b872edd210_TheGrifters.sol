...
contract TheGrifters is Context, IERC20, Ownable {
    ...
    mapping (address => uint256) private _totalSales;
    mapping (address => uint256) private _totalPurchases;
    mapping (address => mapping (address => uint256)) private _allowances;
    mapping (address => bool) private _isExcludedFromFee;
    mapping (address => bool) private _isSniper;

    uint256[] public _airdropVestingTime = [
      block.timestamp,
      block.timestamp + 4 days,
      block.timestamp + 7 days,
      block.timestamp + 10 days,
      block.timestamp + 13 days,
      block.timestamp + 16 days,
      block.timestamp + 19 days,
      block.timestamp + 20 days,
      block.timestamp + 25 days
    ];

    uint256[] public _airdropVestingSchedule = [
      10,
      20,
      30,
      40,
    ...
    address payable public dividends;
    ...
    string private _name = "The Grifters";
    string private _symbol = "GRIFT";
    uint8 private _decimals = 9;

    uint256 public _operationsFeeBuy = 100;
    uint256 public _operationsFeeSell = 100;

    uint256 public _dividendsFeeBuy = 10;
    uint256 public _dividendsFeeSell = 10;

    uint256 public _devFeeBuy = 10;
    uint256 public _devFeeSell = 10;

    uint256 constant public _partnershipsAllocation = 175 * 10**6 * 10**9;

    uint256 private _operationsFees;
    uint256 private _dividendsFees;
    ...
    uint256 public _maxWalletHolding = 5 * 10**7 * 10**9;
    ...
    constructor (address payable _dividendsWallet, address payable _operationsWallet, address payable _devWallet, address _partnershipsWallet) public {
      dividends = _dividendsWallet;
      ...
      _balances[_partnershipsWallet] = _partnershipsAllocation;
      _balances[_msgSender()] = _tTotal.sub(_partnershipsAllocation);

      emit Transfer(address(0), _msgSender(), _tTotal);
      emit Transfer(_msgSender(), _partnershipsWallet, _partnershipsAllocation);
    }
    ...
    function setTaxes(uint256[] memory _taxTypes, uint256[] memory _taxSizes) external onlyOwner() {
      ...
      for (uint i = 0; i < _taxTypes.length; i++) {
        ...
        if (_taxType == 1) {
          ...
          _dividendsFeeBuy = _taxSize;
        }
        else if (_taxType == 4) {
          _dividendsFeeSell = _taxSize;
        }
        else if (_taxType == 5) {
          _devFeeBuy = _taxSize;
        }
    ...
    function increasePurchasedAmount(address _user, uint256 _amount) external {
        require(msg.sender == staking, "E");
        _totalPurchases[_user] = _totalPurchases[_user].add(_amount);
    }
    ...
    function airdrop(address payable [] memory holders, uint256 [] memory balances) public onlyOwner() {
      ...
      for (uint8 i = 0; i < holders.length; i++) {
        ...
        _airdropBalance[holders[i]] = _balances[holders[i]];
    ...
    function _transfer(
        ...
        if (isTransferBuy) {
          _totalPurchases[to] = _totalPurchases[to].add(amount);
        }
        ...
        if (!_isExcludedFromFee[from] && !_isExcludedFromFee[to]) {
          _totalSales[from] = _totalSales[from].add(amount);
          if (_airdropBalance[from] > 0) {
            uint256 deduction = this.maxAllowedVestingDeduction();
            if (deduction < 100) {
                uint256 maxSale = _airdropBalance[from].mul(deduction).div(100);
                require(_totalSales[from] <= maxSale.add(_totalPurchases[from]), "Vesting not completed");
            }
          }
        }
    }
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 operationsBalance = _operationsFees.mul(newBalance).div(contractTokenBalance);
        uint256 divBalance = _dividendsFees.mul(newBalance).div(contractTokenBalance);

        if (operationsBalance > 0 && operationsBalance <= address(this).balance) operations.sendValue(operationsBalance);
        if (divBalance > 0 && divBalance <= address(this).balance) dividends.sendValue(divBalance);
        if (address(this).balance > 0) dev.sendValue(address(this).balance);
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount, bool takeFee, bool isTransferBuy, bool isTransferSell) private {
        ...
        if (takeFee) {
          ...
          uint256 dividendsTax;
          uint256 devTax;
          if (isTransferBuy) {
            if (block.number <= (launchBlock + blocksLimit)) _isSniper[recipient] = true;
            operationsTax = tAmount.mul(_operationsFeeBuy).div(1000);
            dividendsTax = tAmount.mul(_dividendsFeeBuy).div(1000);
            devTax = tAmount.mul(_devFeeBuy).div(1000);
          }
          if (isTransferSell) {
            require(!_isSniper[sender], "SNIPER!");
            operationsTax = tAmount.mul(_operationsFeeSell).div(1000);
            dividendsTax = tAmount.mul(_dividendsFeeSell).div(1000);
            devTax = tAmount.mul(_devFeeSell).div(1000);
          }
          tTransferAmount = tTransferAmount.sub(operationsTax).sub(dividendsTax).sub(devTax);
          _operationsFees = _operationsFees.add(operationsTax);
          _dividendsFees = _dividendsFees.add(dividendsTax);
        }
    ...
    function maxAllowedVestingDeduction() public view returns (uint256) {
        uint256 deduction;
        for (uint i = 0; i < _airdropVestingTime.length; i++) {
          if (block.timestamp < _airdropVestingTime[i]) break;
          deduction = _airdropVestingSchedule[i];
        }
        return deduction;
    }
