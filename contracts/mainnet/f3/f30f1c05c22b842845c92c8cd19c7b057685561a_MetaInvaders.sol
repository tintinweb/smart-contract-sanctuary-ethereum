...
contract MetaInvaders is Ownable, ERC721A, ReentrancyGuard, DefaultOperatorFilterer {
  using Strings for uint256;

  uint256 public MAX_PER_Transaction = 8; // maximam amount that user can mint/Transaction
  uint256 public MAX_PER_WALLET = 8; // maximam amount that user can mint/Wallet
  ...
  uint256 public TotalPhaseSize_ = 100;
  ...
  uint256 public whiteListPerWallet = 1; //maximum amount that user can mint/Wallet in WL
  uint256 public MAX_PER_WL_TRANSACTION = 1; // maximum amount that user can mint/Transaction in WL
  mapping(address => uint256) public whiteListClaimedBy;
  uint256 private TotalWLavailable = 100; // total number of nfts in WL
  uint256 public WL_PRICE = 0 ether;

  constructor() ERC721A("Meta Invaders","MNV") {
  }
  ...
  function mint(uint256 quantity) external payable callerIsUser {
    ...
    if(status == 1){
     ...
     require(totalSupply()+quantity<=TotalWLavailable,"Quantity must be lesser then MaxSupply");
     require(msg.value >= quantity * WL_PRICE, "Try to send more ETH");
     require(quantity <= MAX_PER_WL_TRANSACTION , "can not mint this many");

     whiteListClaimedBy[msg.sender] += quantity;
     require(whiteListClaimedBy[msg.sender] <= whiteListPerWallet, "Purchase exceeds max allowed");

    } else if(status == 2) {

    require(totalSupply() + quantity <= TotalPhaseSize_, "reached max phase supply");
  ...
  function setTotalPhaseSize(uint256 _collection) external onlyOwner
    {
        TotalPhaseSize_ = _collection;
    }
  ...
  function setWL_Price(uint256 _newPrice) public onlyOwner() {
        WL_PRICE = _newPrice;
    }
