...
abstract contract Ownable is Context {
    ...
    constructor () {
        _owner = 0x1FD6ffe335BC745dbdE02a13D5271517FdD00122;
...
contract ZOOMER is Context, IERC20, Ownable {
    ...
    address private _developmentWalletAddress = 0x1FD6ffe335BC745dbdE02a13D5271517FdD00122;
    uint256 private constant MAX = ~uint256(0);
    uint256 private _tTotal = 1000000000000 * 10**18;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;
    string private _name = "ZOOMER";
    string private _symbol = "ZOOM";
    ...
    function _getValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256, uint256) {
        (uint256 tTransferAmount, uint256 tFee, uint256 tDevelopment) = _getTValues(tAmount);
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee) = _getRValues(tAmount, tFee, tDevelopment, _getRate());
    ...
    function _getRValues(uint256 tAmount, uint256 tFee, uint256 tDevelopment, uint256 currentRate) private pure returns (uint256, uint256, uint256) {
        uint256 rAmount = tAmount.mul(currentRate);
        uint256 rFee = tFee.mul(currentRate);
        uint256 rDevelopment = tDevelopment.mul(currentRate);
        uint256 rTransferAmount = rAmount.sub(rFee).sub(rDevelopment);
