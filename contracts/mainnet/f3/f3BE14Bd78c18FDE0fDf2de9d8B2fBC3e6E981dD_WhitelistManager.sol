...
contract WhitelistManager is OwnableUpgradeable, PausableUpgradeable, UUPSUpgradeable {
    ...
    function isSystem(address _address) external view returns (bool) {
        return hasRole(SYSTEM_ROLE, _address);
    }
    ...
    function isAllowed(address _address) public view returns (bool) {
        ...
        if (hasRole(VAULT_ROLE, _address)) return true;
        if (hasRole(SYSTEM_ROLE, _address)) return true;

        if (_sanctioned(_address)) return false;

        if (hasRole(CUSTOMER_ROLE, _address)) return true;
        if (hasRole(LP_ROLE, _address)) return true;
