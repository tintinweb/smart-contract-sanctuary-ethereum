...
contract MonthlyArtDistributor is VRFConsumerBaseV2, Ownable {
    ...
    function distribute(uint256 _tokenForPassOne) external onlyOwner {
        require(artSet, "art has not been set");
        require(_randomWords.length >= 1, "at least one random word is needed");
        require(_tokenForPassOne >= startToken && _tokenForPassOne <= startToken + 249, "token for pass 1 not in range");
        
        address passHolderOne = rarePassContract.ownerOf(1);
        nftContract.transferFrom(artist, passHolderOne, _tokenForPassOne);
        ...
        for (uint256 i = 0; i < 250; i++) {
            ...
            if (nftTokenId != _tokenForPassOne) {
