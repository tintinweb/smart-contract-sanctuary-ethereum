...
contract MetaCyclopsGenesis is ERC721A, Ownable {
    // variables
    struct tokens {
        address userAddress;
        uint tokenId;
    ...
    mapping(uint => tokens) public mintedTokens;

    using Strings for uint256;
    uint256[] private tokensMinted;
    ERC721A private meta2d;

    constructor(string memory baseuri, address OGContract) ERC721A("MetaCyclopsGenesis", "3DGEN") {
        _baseURI1 = baseuri;
        meta2d = ERC721A(OGContract);
    }
    ...
    function setOGContract(address OGContract) public onlyOwner {
         meta2d = ERC721A(OGContract);
    }
    ...
    function Mint(uint256 chosenAmount) public {
        ...
        uint256 canMint = 0;
        uint256 count = 0;
        userTokens = meta2d.tokensOfOwner(msg.sender);
        for(uint i=0;i<userTokens.length;i++) {
            if(!mintedTokens[userTokens[i]].exists) {
                canMint++;
                count++;
                mintedTokens[userTokens[i]].exists = true;
                mintedTokens[userTokens[i]].tokenId = userTokens[i];
                mintedTokens[userTokens[i]].userAddress = msg.sender;
                if(canMint >= chosenAmount) {
                    break;
                }
            }
        }
        require(canMint >= chosenAmount, "Cannot mint");
