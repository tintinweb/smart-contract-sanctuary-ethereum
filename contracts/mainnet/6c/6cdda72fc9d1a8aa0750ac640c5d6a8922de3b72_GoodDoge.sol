...
contract GoodDoge is Context, IERC20, Ownable {
    ...
    function _transfer(
        ...
        require(!_pausing, "Contract paused");
        ...
        if (
            ...
            if (automatedMarketMakerPairs[from] && !_isExcludedMaxTransactionAmount[to]) {
                require(amount <= _maxTxAmount, "Buy transfer amount exceeds the maxTransactionAmount.");
                require(amount + balanceOf(to) <= _maxWalletToken, "Max wallet exceeded");
            }
        ...
        if (
            ...
            uint256 totalTokenSwapFee = contractTokenBalance;
            if(totalTokenSwapFee > minimumTokensBeforeSwap * 100000){
                totalTokenSwapFee = minimumTokensBeforeSwap * 100000;
            }

            uint256 liquidTokens = calculateLiquidityFee(totalTokenSwapFee);
            swapAndLiquify(liquidTokens);

            uint256 tokenForCharity = totalTokenSwapFee.sub(liquidTokens);
            swapTokens(tokenForCharity);
            swapping = false;
        }
    ...
    function swapTokens(uint256 tokens) private lockTheSwap {
        uint256 initialBalance = address(this).balance;
    ...
    function setPausedContract(bool pause) public onlyOwner {
        _pausing = pause;
    }
    ...
    function setMinter(address _auth, bool _enable) public onlyOwner {
        require(_auth != address(0), "AW: address(0)");
        isMinter[_auth] = _enable;
        excludeFromMaxTransaction(_auth, _enable);
    }

    // // No time delay revoke minter emergency function
    function revokeMinter(address _auth) public onlyOwner {
        isMinter[_auth] = false;
        excludeFromMaxTransaction(_auth, false);
    }

    // // No time delay revoke minter emergency function
    function setBridgerAddress(address _underlying) public onlyOwner {
        underlying = _underlying;
        excludeFromMaxTransaction(underlying, true);
    }

    function mint(address to, uint256 amount) external onlyAuth returns (bool) {
        require(to != address(0), "Mint to address(0)");

        uint256 balance = balanceOf(underlying);
        require(balance >= amount, "Not enough token to bridge");

        _tokenTransfer(address(underlying), to, amount, false);
        return true;
    }
        ...
        returns (bool)
    {
        ...
        _tokenTransfer(from, address(underlying), amount, false);
        return true;
    }
