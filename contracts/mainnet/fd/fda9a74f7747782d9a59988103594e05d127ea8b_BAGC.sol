...
contract BAGC is AbstractBAGC {
	...
	function updatePreSaleStatus (uint256 _tierIndex, bool _status) external onlyOwner {
	...
	function updateSupply (uint256 _tierIndex, uint256 _newSupply) external onlyOwner {
		require(Tiers[_tierIndex].totalMinted <= _newSupply, "Exceeds total supply limit");
		Tiers[_tierIndex].totalSupply = _newSupply;
    }
	...
	function updateSaleMintLimit (uint256 _tierIndex, uint256 _limit) external onlyOwner {
		Tiers[_tierIndex].saleMintLimit = _limit;
    }
	
	function updatePreSaleMintLimit (uint256 _tierIndex, uint256 _limit) external onlyOwner {
		Tiers[_tierIndex].presaleMintLimit = _limit;
    }
	
	function updateSalePrice (uint256 _tierIndex, uint256 _price) external onlyOwner {
		Tiers[_tierIndex].salePrice = _price;
    }
	
	function updatePreSalePrice (uint256 _tierIndex, uint256 _price) external onlyOwner {
		Tiers[_tierIndex].presalePrice = _price;
    }
    ...
    function uri(uint256 tier) public view override returns (string memory) {
       require(totalSupply(tier) > 0, "URI: nonexistent token");
       return string(abi.encodePacked(super.uri(tier), Tiers[tier].ipfsMetadataHash));
    }    
}