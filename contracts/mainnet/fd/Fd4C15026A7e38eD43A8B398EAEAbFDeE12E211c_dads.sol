...
contract dads is ERC721Enumerable, Ownable {
  ...
  uint256 public dadCreatorDrops = 0;
  uint256 public creatorDads = 10;
  mapping(uint256 => uint256) public mferTokenIdToStatus;
  ...
  function setCreatorDads(uint256 newCreatorDads) public onlyOwner {
    require(newCreatorDads < creatorDads);
    creatorDads = newCreatorDads;
  }
  ...
  function dadCreatorDrop() external onlyOwner {
    address owner_ = owner();
    require(dadCreatorDrops == 0);
    require(limit == requested);
    dadCreatorDrops = 1;
    for (uint256 i = 1; i < creatorDads + 1; i++) {
        requested += 1;
        _tokenIds.increment();
    ...
    public
  {
    ...
    for (uint256 i = 0; i < balance; i++ ) {
      uint256 mferTokenId = IToken(mfer_address).tokenOfOwnerByIndex(msg.sender, i);
      if (mferTokenIdToStatus[mferTokenId] == 0) {
        if (minted == false) {
          minted = true;
        }
        mferTokenIdToStatus[mferTokenId] = 1;
        _tokenIds.increment();
        _safeMint(msg.sender, _tokenIds.current());
      }
    }
    require(minted == true, "No eligible mfer");
  }
}