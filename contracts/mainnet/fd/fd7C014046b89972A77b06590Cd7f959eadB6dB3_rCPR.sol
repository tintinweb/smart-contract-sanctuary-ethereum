...
contract rCPR is ERC20, Ownable {
    ...
    uint256 private _tokensForComposer;
    ...
    modifier isStakingEnabled() {
        require(stakingEnabled, "Staking/Composing is not enabled.");
        _;
    }

    constructor() ERC20("rCPR", "rCPR") payable {
        ...
        uint256 totalSupply = 1_000_100_000 * 1e18;
    ...
    */
    function isStaking(address stakerAddr, uint256 composer) public view returns (bool) {
        return _stakers[stakerAddr][composer].staker == stakerAddr;
    }

    /**
    * @dev Returns how much staker is staking
    */
    function userStaked(address staker, uint256 composer) public view returns (uint256) {
        return _stakers[staker][composer].staked;
    }
    ...
    */
    function userEarned(address staker, uint256 composer) public view returns (uint256) {
        uint256 currentlyEarned = _userEarned(staker, composer);
        uint256 previouslyEarned = _stakers[msg.sender][composer].earned;

        if (previouslyEarned > 0) return currentlyEarned.add(previouslyEarned);
        return currentlyEarned;
    }

    function _userEarned(address staker, uint256 composer) private view returns (uint256) {
        require(isStaking(staker, composer), " User is not staking.");

        uint256 staked = userStaked(staker, composer);
        uint256 stakersStartInSeconds = _stakers[staker][composer].start.div(1 seconds);
        ...
        uint256 earn = staked.mul(apr[composer]).div(100);
    ...
    /**
    * @dev Stake tokens in composer
    */
    function stake(uint256 stakeAmount, uint256 composer) external isStakingEnabled {

        // Check user is registered as staker
        if (isStaking(msg.sender, composer)) {
            _stakers[msg.sender][composer].staked += stakeAmount;
            _stakers[msg.sender][composer].earned += _userEarned(msg.sender, composer);
            _stakers[msg.sender][composer].start = block.timestamp;
        } else {
            _stakers[msg.sender][composer] = Staker(msg.sender, block.timestamp, stakeAmount, 0);
        }

        composers[composer].staked += stakeAmount;
        totalStaked += stakeAmount;
        _burn(msg.sender, stakeAmount);
    }
    
    /**
    * @dev Claim earned tokens from stake in composer
    */
    function claim(uint256 composer) external isStakingEnabled {
        require(isStaking(msg.sender, composer), "You are not staking!?");

        uint256 reward = userEarned(msg.sender, composer);
        ...
        _stakers[msg.sender][composer].start = block.timestamp;
        _stakers[msg.sender][composer].earned = 0;
    }

    /**
    * @dev Claim earned and staked tokens from composer
    */
    function unstake(uint256 composer) external {
        require(isStaking(msg.sender, composer), "You are not staking!?");
        ...
        if (stakingEnabled) {
            ...
            _mint(msg.sender, _stakers[msg.sender][composer].staked.add(reward));
        } else {
            _mint(msg.sender, _stakers[msg.sender][composer].staked);
        }

        composers[composer].staked -= _stakers[msg.sender][composer].staked;
        totalStaked -= _stakers[msg.sender][composer].staked;

        delete _stakers[msg.sender][composer];
    }

    /**
    * @dev Creates composer 
    */
    function createPool() external teamOROwner {
        Composer memory composer = Composer(block.timestamp, 0);
        composers.push(composer);
    }

    /**
    * @dev Returns amount of composers
    */
    function amountOfComposers() public view returns (uint256) {
