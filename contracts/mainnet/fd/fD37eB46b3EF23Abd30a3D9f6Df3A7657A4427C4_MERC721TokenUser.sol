...
contract MERC721TokenUser is MTokenUser, ERC721("MERC721","MERC721"), MERC721UserInterface {
    ...
    RCF - "Redeem from contract failed"
    TOF - "Transfer out failed"
    OOW - "Only Owner"
     */
    ...
    function addAuctionBid(uint240 mToken) external payable nonReentrant2 {
        if (mToken == mtroller.getAnchorToken(address(this))) {
            require(tokenAuction.addOfferETH.value(msg.value)(mToken, msg.sender, address(0), false) == 0, "IIS");
        }
        else {
            address payable oldOwner = Address.toPayable(ownerOf(mToken)); // reverts if mToken does not exist
            uint256 askPrice = askingPrice[mToken];
            bool directSale = ((tokenAuction.getOffer(mToken, msg.sender) + msg.value) >= askPrice && askPrice > 0);
            if (directSale) {
                _tokenApprovals[mToken] = address(tokenAuction); // approve tokenAuction contract to transfer the mToken
            }
            uint256 paid = tokenAuction.addOfferETH.value(msg.value)(mToken, msg.sender, oldOwner, directSale);
            if (directSale) {
                require(paid >= askPrice, "IIS");
            }
        }
    }
     ...
     * @param favoriteBidder If this is nonzero, the mToken is sold to this address instead of the highest bidder
     */
    function instantSellToHighestBidder(uint240 mToken, uint256 minimumPrice, address favoriteBidder) public nonReentrant2 {
        address payable oldOwner = Address.toPayable(ownerOf(mToken)); // reverts if owner = 0 (non-existant)
        uint256 askPrice = askingPrice[mToken];
        _tokenApprovals[mToken] = address(tokenAuction); // approve tokenAuction contract to transfer the mToken
        ( , uint256 maxOffer, , ) = tokenAuction.acceptHighestOffer(mToken, oldOwner, oldOwner, 0, minimumPrice, favoriteBidder);
     ...
     * @return (uint) Unless reverted, returns:
     *         (error code: 0=success, otherwise a failure (see ErrorReporter.sol for details))
     */
    function liquidateToPaymentToken(uint240 mToken) external nonReentrant2 returns (uint) {
        /* If liquidation is not allowed (anymore) we reset the gracePeriod and fail */
        if (mtroller.liquidateERC721Allowed(mToken) != uint(Error.NO_ERROR)) {
            lastBlockGracePeriod[mToken] = 0;
            preferredLiquidator[mToken] = address(0);
            return (fail(Error.MTROLLER_REJECTION, FailureInfo.LIQUIDATE_MTROLLER_REJECTION));
        }
        ...
        if (getBlockNumber() <= (gracePeriod + preferredLiquidatorHeadstart) 
            ...
            return (fail(Error.UNAUTHORIZED, FailureInfo.LIQUIDATE_NOT_PREFERRED_LIQUIDATOR));
        }
        ...
        /* seize mToken, i.e. transfer from oldOwner to this contract. Revert on error. */
        address payable oldOwner = Address.toPayable(ownerOf(mToken));
        uint err = seizeInternal(mToken, address(this), oldOwner, mToken, oneUnit);
        ...
        super._transferFrom(oldOwner, address(this), mToken);
        require(ownerOf(mToken) == address(this), "LNE");

        /* accepts highest auction bid, reverts on error */
        _tokenApprovals[mToken] = address(tokenAuction); // approve tokenAuction contract to transfer the mToken
        (address maxOfferor, , uint256 auctioneerTokens, uint256 oldOwnerTokens) 
                = tokenAuction.acceptHighestOffer(mToken, oldOwner, msg.sender, liquidatorAuctionFeeMantissa, minimumOffer, address(0));
