...
contract TRVLBurner {
    TRVL public trvl;
    constructor(address _trvlAddress) {
        trvl = TRVL(_trvlAddress); 
    }

    function burnTokens(uint256 amount) external {
        uint256 allowance = trvl.allowance(msg.sender, address(this));
        require(amount > 0, "Amount is 0");
        require(allowance >= amount, "Amount is bigger than allowance");
        trvl.transferFrom(msg.sender, address(this), amount);
        trvl.burn(address(this), amount);
    }
}