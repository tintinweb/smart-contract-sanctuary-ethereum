...
contract ClearPoolStrategy is PrimitivePoolStrategy {
    ...
    /**
     * @notice Gets the supply APR informed by ClearPool
     * @return Returns the supply APR
     */
    function getSupplyAPR () public view ifInitialized returns (uint256) {
        return IClearPoolBase(poolAddress).getSupplyRate() * 86400 * 365;
    }

    /**
     * @notice Gets the CPOOL ARP
     * @return Returns the CPOOL APR
     */
    function getCPoolAPR (uint256 cpoolPrice) public view ifInitialized returns (uint256) {
        uint256 poolSize = IClearPoolBase(poolAddress).poolSize();
        uint256 rewardsPerSecond = IClearPoolBase(poolAddress).rewardPerSecond();

        return (rewardsPerSecond * SECONDS_PER_YEAR * cpoolPrice) / poolSize;
    }
     ...
     */
    function getAPR (uint256 cpoolPrice) public view ifInitialized returns (uint256) {
        return getSupplyAPR() + getCPoolAPR(cpoolPrice);
    }
}