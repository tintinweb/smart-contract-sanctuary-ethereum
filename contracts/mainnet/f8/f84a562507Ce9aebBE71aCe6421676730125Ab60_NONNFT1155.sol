...
contract AssetContract is ERC1155Tradable {
    ...
    function safeTransferFrom(
        ...
        if (
            mintedBalance < _amount &&
            TokenIdentifiers.tokenCreator(_id) == _from
        ) {
 ...
 * @title NONNFT1155
 */
contract NONNFT1155 is AssetContract, ReentrancyGuard {
    ...
    event SetProxyRegistryAddress(address proxyRegistryAddress);

    event AddSharedProxyAddress(address sharedProxyAddress);

    event RemoveSharedProxyAddress(address sharedProxyAddress);
    ...
    modifier creatorOnly(uint256 _id) {
        require(
            ...
            "NONNFT1155#creatorOnly: ONLY_CREATOR_ALLOWED"
        );
        _;
    }
    ...
    modifier onlyFullTokenOwner(uint256 _id) {
        require(
            ...
            "NONNFT1155#onlyFullTokenOwner: ONLY_FULL_TOKEN_OWNER_ALLOWED"
        );
        _;
    }
    ...
    function batchMint(
        ...
        for (uint256 i = 0; i < _ids.length; i++) {
            require(
                ...
                "NONNFT1155#_batchMint: ONLY_CREATOR_ALLOWED"
            );
        }
        _batchMint(_to, _ids, _quantities, _data);
    }
    ...
    function setCreator(uint256 _id, address _to) public creatorOnly(_id) {
        require(_to != address(0), "NONNFT1155#setCreator: INVALID_ADDRESS.");
    ...
    function _requireMintable(address _address, uint256 _id) internal view {
        require(
            ...
            "NONNFT1155#_requireMintable: ONLY_CREATOR_ALLOWED"
        );
    }
