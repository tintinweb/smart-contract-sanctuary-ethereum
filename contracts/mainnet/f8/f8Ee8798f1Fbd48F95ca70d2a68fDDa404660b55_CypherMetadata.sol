...
contract CypherMetadata is Ownable {
    ...
    constructor(CypherDrawing drawing, string memory imageBaseUri) {
        _drawing = drawing;
    ...
    function metadata(
        ...
        string memory imageUri = bytes(_imageBaseUri).length > 0
            ? string(abi.encodePacked(_imageBaseUri, tokenId.toString()))
            : "";
    ...
    function setDrawing(CypherDrawing drawing) public onlyOwner {
        _drawing = drawing;
    }

    function setImageBaseUri(string memory imageBaseUri) public onlyOwner {
        _imageBaseUri = imageBaseUri;
    }
    ...
    function _commonMetadata(
        ...
        SB.writeStr(
            b,
            '"description": "Cypher is a generative art project by Hideki Tsukamoto comprised of 1024 tokens calculated and drawn via smart-contract, by the Ethereum Virtual Machine. Cypher is part one of the \'Apex\' series.",'
            '"name": "Cypher #'
        );
        SB.writeUint(b, tokenId);
        SB.writeStr(b, '",' '"background_color": "1a181b",' '"attributes": [');
        if (tokenId == 0) {
            SB.writeStr(
                b,
                "{"
        ...
        SB.writeStr(b, "{" '"trait_type": "Generation",' '"value": "');
        SB.writeUint(b, generation);
        SB.writeStr(b, '"},' "{" '"trait_type": "Intricacy",' '"value":"');
        ...
        SB.writeStr(b, "}," "{" '"trait_type": "Chaos",' '"value":"');
        ...
        SB.writeStr(b, "}," "{" '"trait_type": "Channels",' '"value":"');
        ...
        SB.writeStr(b, "}," "{" '"trait_type": "Decay",' '"value":"');
        ...
        SB.writeStr(b, "}," "{" '"trait_type": "Level",' '"value":"');
