    ...
    ReentrancyGuardUpgradeable
{
    ...
    modifier requirePlan(address wNFTContract, uint256 wNFTTokenId) {
        require(
    ...
    modifier onlyActivePlan(address wNFTContract, uint256 wNFTTokenId) {
        require(
            _paymentPlan[wNFTContract][wNFTTokenId].totalNumberOfPayments != 0,
            "No payment plan found"
        );
    ...
    function fundBNPL(address wNFTContract, uint256 wNFTTokenId)
        ...
        nonReentrant
        onlyRole(CYAN_ROLE)
        requirePlan(wNFTContract, wNFTTokenId)
    {
        require(
        ...
        requirePlan(wNFTContract, wNFTTokenId)
    {
        ...
        (uint256 unpaidAmount, , , , uint256 dueDate) = getEarlyPaymentInfo(
            wNFTContract,
            wNFTTokenId
        );
    ...
    function pay(address wNFTContract, uint256 wNFTTokenId)
        external
        payable
        onlyActivePlan(wNFTContract, wNFTTokenId)
        nonReentrant
    {
        (
            uint256 payAmountForCollateral,
    ...
    /**
     * @notice Pay all the remaining payments for the payment plan
     * @param wNFTContract Wrapped NFT contract address
     * @param wNFTTokenId Wrapped NFT token ID
     */
    function payEarly(address wNFTContract, uint256 wNFTTokenId)
        external
        payable
        onlyActivePlan(wNFTContract, wNFTTokenId)
        nonReentrant
    {
        (
            ...
            uint256 dueDate
        ) = getEarlyPaymentInfo(wNFTContract, wNFTTokenId);

        require(remainingPayment == msg.value, "Wrong payment amount");
        require(dueDate >= block.timestamp, "Payment due date is passed");
        CyanWrappedNFTV1 _cyanWrappedNFTV1 = CyanWrappedNFTV1(wNFTContract);
        require(
            _cyanWrappedNFTV1.exists(wNFTTokenId),
        ...
        address receiver = _cyanWrappedNFTV1.ownerOf(wNFTTokenId);
        _cyanWrappedNFTV1.unwrap(wNFTTokenId, false);
        uint8 numOfPayingPayments = _paymentPlan[wNFTContract][wNFTTokenId]
            .totalNumberOfPayments -
            _paymentPlan[wNFTContract][wNFTTokenId].counterPaidPayments;
     ...
     * @return Due date (Due date is the same as the next payment date)
     */
    function getEarlyPaymentInfo(address wNFTContract, uint256 wNFTTokenId)
        public
        view
        requirePlan(wNFTContract, wNFTTokenId)
        returns (
        ...
        )
    {
        ...
        if (plan.totalNumberOfPayments - plan.counterPaidPayments == 1) {
            return getNextPayment(wNFTContract, wNFTTokenId);
        }
        ...
        uint8 remainingNumOfPayments = plan.totalNumberOfPayments -
            plan.counterPaidPayments;
        if (plan.status == PaymentPlanStatus.BNPL_ACTIVE) {
            payAmountForInterest -=
                (payAmountForInterest *
                    BASE_DISCOUNT_RATE *
                    remainingNumOfPayments) /
                (10000 * (plan.totalNumberOfPayments - 1));
        } else if (plan.status == PaymentPlanStatus.PAWN_ACTIVE) {
            payAmountForInterest -=
                (payAmountForInterest *
                    BASE_DISCOUNT_RATE *
                    remainingNumOfPayments) /
                (10000 * plan.totalNumberOfPayments);
        }
    ...
    /**
     * @notice Updating base discount rate of early repayment
     */
    function updateBaseDiscountRate(uint256 baseDiscountRate)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        require(baseDiscountRate < 10000, "Base discount rate is too big");
        BASE_DISCOUNT_RATE = baseDiscountRate;
    }
}
