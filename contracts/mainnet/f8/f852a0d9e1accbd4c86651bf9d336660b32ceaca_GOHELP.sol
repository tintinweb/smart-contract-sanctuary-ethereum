...
contract GOHELP is IERC20, OWNED {
	mapping(address => uint256) private _balances;
	mapping(address => mapping(address => uint256)) private _allowances;
	uint8 private constant _decimals = 9;
	uint256 private constant _totalSupply = 1_000_000_000 * 10**_decimals;
	string private constant _name = "Go Help ETH 2.0";
	string private constant _symbol = "GOHELP";
	...
	uint256 private constant taxMcBracket1 = 20_000_000; // below this MC tax is 6% (2 LP, 2 eth2LP, 2 Marketing)
	uint256 private constant taxMcBracket2 = 50_000_000; // below this MC tax is 4% (2 eth2LP, 2 Marketing)
	uint256 private constant taxMcBracket3 = 200_000_000; // below this MC tax is 2% (only eth2LP), above it tax is 0%

	mapping(address => bool) private _excluded;
	address private _marketingWallet = address(0xAe6759E6f3Fb5df9122e5fAc36f3fB84828E126c);
	
	address private constant _eth2 = address(0x04A6b6DE116Fb8bF57e5eE8b05e0293EA3639fE8); 
	...
	HELPER_Sidecar private immutable _sidecarContract;
	...
	uint256 private _addTime = 1; //trading opens 1s after adding liquidity
	...
	constructor() OWNED(msg.sender)  {
		...
		_sidecarContract = new HELPER_Sidecar();
		_sidecarAddress = address(_sidecarContract);
		_usdcDecimals = IERC20(_usdc).decimals();

		_changeLimits(20,30); //set max TX to 2%, max wallet 3%
	...
	function addressList() external view returns (address owner, address sidecar, address marketing, address eth2, address usdc, address swapRouter, address primaryLP) {
		return (_owner, _sidecarAddress, _marketingWallet, _eth2, _usdc, _swapRouterAddress, _primaryLP);
	}
	...
	function _transfer(address from, address to, uint256 amount) private {
		...
		(uint256 eth2LP, uint256 werLP, uint256 marketing) = _getTaxTokens(from, to, amount);
		uint256 taxTokens = eth2LP + werLP + marketing;
	...
	function _getTaxRates() private view returns (uint8 eth2Rate, uint8 werRate, uint8 marketingRate) {
		uint8 _eth2Rate; uint8 _werRate; uint8 _marketingRate;
		uint256 currentDilutedUsdMC = _getCurrentDilutedMcUSD();
		if (currentDilutedUsdMC < taxMcBracket1 ) {
			_eth2Rate = 2; _werRate = 2; _marketingRate = 2;
		} else if (currentDilutedUsdMC >= taxMcBracket1 && currentDilutedUsdMC < taxMcBracket2) {
			_eth2Rate = 2; _werRate = 0; _marketingRate = 2;
		} else if (currentDilutedUsdMC >= taxMcBracket2 && currentDilutedUsdMC < taxMcBracket3) {
			_eth2Rate = 2; _werRate = 0; _marketingRate = 0;
		} else { 
			_eth2Rate = 0; _werRate = 0; _marketingRate = 0;
		}
		return (_eth2Rate, _werRate, _marketingRate);
	}
	function _getTaxTokens(address from, address to, uint256 amount) private view returns (uint256 eth2LP, uint256 werLP, uint256 marketing) {
		uint256 _eth2LP; uint256 _werLP; uint256 _marketing;
		if ( (_isLP[from] && !_excluded[to]) || (_isLP[to] && !_excluded[from]) ) { 
			(uint8 eth2Rate, uint8 werRate, uint8 marketingRate) = _getTaxRates();
			_eth2LP = amount * eth2Rate / 100;
		...
		return (_eth2LP, _werLP, _marketing);
	}  
	...
	function tax() external view returns (uint8 Liquidityeth2, uint8 LiquidityHELPER, uint8 Marketing) { 
		(uint8 eth2Rate, uint8 werRate, uint8 marketingRate) = _getTaxRates();
		return (eth2Rate, werRate, marketingRate);
	}
	...
	function _processTaxTokens() private lockSwap {
		...
		(uint8 eth2Rate, uint8 werRate, uint8 marketingRate) = _getTaxRates();
		uint8 totalRate = eth2Rate + werRate + marketingRate;
		uint256 swapAmount = _balances[address(this)];
		if (totalRate>0 && swapAmount >= thresholdTokens) {
			swapAmount = thresholdTokens;

			uint256 tokensForeth2 = (swapAmount * eth2Rate / totalRate);
			uint256 tokensForWerLP = (swapAmount * werRate / totalRate)/2;
			uint256 tokensForMarketing = swapAmount * marketingRate / totalRate;

			uint256 tokensToSwap = tokensForeth2 + tokensForMarketing + tokensForWerLP;
			if (tokensToSwap >= 10**_decimals) {
				uint256 swappedOutputUSDC = _swapTokens(address(this), _usdc, tokensToSwap, true); //swap HELPER for USDC, use sidecar contract
				uint256 usdcForWerLP = swappedOutputUSDC * tokensForWerLP / tokensToSwap; //calc USDC for HELPER liquidity
				uint256 usdcToSpendOneth2 = (swappedOutputUSDC * tokensForeth2 / tokensToSwap) / 2; //calc USDC for eth2 liquidity
				uint256 usdcForMarketing = swappedOutputUSDC * tokensForMarketing / tokensToSwap; //calc USDC for marketing

				if (werRate>0) { _addLiquidity(address(this), tokensForWerLP, usdcForWerLP, true); } //add HELPER liquidity and burn LP tokens

				if (eth2Rate>0) {
					uint256 eth2Purchased = _swapTokens(_usdc, _eth2, usdcToSpendOneth2, false); //purchase eth2 for liquidity, sidecar not used
					_addLiquidity(_eth2, eth2Purchased, usdcToSpendOneth2, true); //add eth2 liquidity and burn LP tokens
				}
