...
contract DAO is OwnableUpgradeable {
    ...
    struct DaoProposals {
        ...
        address[] forVotesAddress;
        address[] againstVotesAddress;
        address paidBy;
        Status status;
    }
    ...
    function vote(uint256 proposalId, bool supportProposal, uint256 _amount) external {
        ...
        daoProposal.status = Status.Active;
         ...
         if (supportProposal){
            ...
            daoProposal.forVotesAddress.push(msg.sender);
        }else{
            daoProposal.votesAgainst = daoProposal.votesAgainst + amount;
            daoProposal.againstVotesAddress.push(msg.sender);
        }
    ...
    function getVotes(uint proposalId) public view returns(UserVotes[] memory) {
    ...
    function payProposal(uint256 proposalId) external {
        ...
        if(block.timestamp <= daoProposal.livePeriod)
        ...
        if(daoProposal.votesAgainst > daoProposal.votesFor) {
            daoProposal.votingPassed = false;    
        }
