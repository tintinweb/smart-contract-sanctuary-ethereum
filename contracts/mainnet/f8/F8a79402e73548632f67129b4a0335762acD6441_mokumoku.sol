...
// File: contracts/mokumoku.sol

pragma solidity ^0.8.13;

contract mokumoku is ERC721Psi, ERC2981, Ownable, ReentrancyGuard{
    using Strings for uint256;
    uint256 public maxSupply = 700;
    uint256 public prePrice = 0.03 ether;
    uint256 public pubPrice = 0.04 ether;
    uint256 public piePrice = 0.04 ether;
    bool public preSaleStart = false;
    bool public pubSaleStart = false;
    bool public piementSaleStart = false;
    uint256 public mintLimit = 1;
    bytes32 public merkleRoot = 0xa91e4d9e2b9aa0e98f14dcb713595fb6059db859e7559ffe71e475db70c0aea2;
    string private _baseTokenURI = "https://arweave.net/xAhYewcxWgJDpa3WuZMUM_1WNZhw9pcIMLmk7rfwcdM/bronze.json";
    mapping(address => uint256) public claimed;
    mapping(uint256 => string) public tokenRankUri;
    mapping(uint256 => uint256) public tokenRank;

    constructor() ERC721Psi ("MOKUMOKU", "MKMK") {
        _setDefaultRoyalty(0x6cad2Dc5662149A7aB7E1D94EEA395a5cf379533, 1000);
        setTokenRankUri(0, "https://arweave.net/xAhYewcxWgJDpa3WuZMUM_1WNZhw9pcIMLmk7rfwcdM/bronze.json");
    }
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        uint256 _tokenRank = tokenRank[tokenId];
        string memory _tokenURI = tokenRankUri[_tokenRank];
    ...
    // mintCheck
    function _mintCheck(uint256 _quantity,uint256 _supply,uint256 _cost) private view {
        require(_supply + _quantity <= maxSupply, "Max supply over");
        require(msg.value >= _cost, "Not enough funds");
        require(claimed[msg.sender] + _quantity <= mintLimit, "Already claimed max");
    }
    ...
    function piementMint(address _receiver, uint256 _quantity) public payable nonReentrant {
        ...
        require(piementSaleStart, "Before sale begin.");
        require(supply + _quantity <= maxSupply, "Max supply over");
        require(msg.value >= cost, "Not enough funds");
        require(claimed[_receiver] + _quantity <= mintLimit, "Already claimed max");
    ...
    // TokenRankURI
    function setTokenRankUri(uint256 _tokenRank, string memory _uri) public virtual onlyOwner {
      tokenRankUri[_tokenRank] = _uri;
    }

    // tokenID Rank
    function setTokenRank(uint256 tokenId, uint256 _rank) public virtual onlyOwner {
        require(_exists(tokenId), "ERC721Metadata: URI set of nonexistent token");
        tokenRank[tokenId] = _rank;
    }
    ...
    // price
    function setPrePrice(uint256 _price) external onlyOwner {
        prePrice = _price;
    }
    function setPubPrice(uint256 _price) external onlyOwner {
        pubPrice = _price;
    }

    function setPiePrice(uint256 _price) external onlyOwner {
        piePrice = _price;
    }

    // saleState
    function setPreSaleStart(bool _state) external onlyOwner {
        preSaleStart = _state;
    }
    function setPubSaleStart(bool _state) external onlyOwner {
        pubSaleStart = _state;
    }

    function setPimentSaleStart(bool _state) external onlyOwner {
        piementSaleStart = _state;
    }

    // limit
    function setMintLimit(uint256 _quantity) external onlyOwner {
        mintLimit = _quantity;
    }
    ...
    function withdraw() external onlyOwner {
        ...
        Address.sendValue(payable(owner()), ((balance * 10000) / 10000));
    }
