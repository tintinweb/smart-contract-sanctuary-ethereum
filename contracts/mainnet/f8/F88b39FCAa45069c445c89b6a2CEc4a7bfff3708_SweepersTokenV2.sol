...
contract SweepersTokenV2 is ISweepersToken, OwnableUpgradeable, ERC721CheckpointableUpgradeable, IERC721ReceiverUpgradeable, ReentrancyGuardUpgradeable {
     ...
     */
    function migrate(uint256 sweeperId) external onlyOwner {
    ...
    /**
     * @notice Holder migrate their owned sweepers in batches of <= 95.
     * @dev Transfers V1 Sweeper to this contract and then burns it.
     */
    function migrateManyByOwner() external {
        uint256 b = sweepersV1.balanceOf(msg.sender);
        uint256 l;
        if(b > 95) {
            l = b - 95;
        }
        for(uint i = b; i > l;) {
            uint256 sweeperId = sweepersV1.tokenOfOwnerByIndex(msg.sender, i - 1);
            _mintExistingTo(msg.sender, sweeperId);
            sweepersV1.safeTransferFrom(msg.sender, address(this), sweeperId);
            sweepersV1.burn(sweeperId);
            unchecked{i--;}
        }
    }
...
import './IERC721CheckpointableUpgradeable.sol';

abstract contract ERC721CheckpointableUpgradeable is ERC721EnumerableUpgradeable, IERC721CheckpointableUpgradeable {
     ...
     */
    function votesToDelegate(address delegator) public view virtual override returns (uint96) {
        return safe96(balanceOf(delegator), 'votesToDelegate: amount exceeds 96 bits');
    }
     ...
     */
    function delegates(address delegator) public view virtual override returns (address) {
    ...
    function _beforeTokenTransfer(
        ...
        _moveDelegates(delegates(from), delegates(to), uint96(batchSize));
    }
    ...
    function _moveDelegates(
        ...
        if (srcRep != dstRep && amount > 0) {
            if (srcRep != address(0)) {
                ...
                uint96 srcRepNew = sub96(srcRepOld, amount, '_moveDelegates: amount underflows');
            ...
            if (dstRep != address(0)) {
                ...
                uint96 dstRepNew = add96(dstRepOld, amount, '_moveDelegates: amount overflows');
