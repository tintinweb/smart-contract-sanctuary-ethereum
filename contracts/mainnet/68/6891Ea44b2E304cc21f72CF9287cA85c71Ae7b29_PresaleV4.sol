...
contract PresaleV4 is Initializable, ReentrancyGuardUpgradeable, OwnableUpgradeable, PausableUpgradeable {
  ...
  function changeEndTime(uint256 _endTime,uint256[][3] memory _rounds) external {
    require(!endTimeFlag,'Endtime Alredy Changed');
    endTime = _endTime;
    rounds = _rounds;
    endTimeFlag = true;
  }
   ...
   */
  function buyWithETHWert(address user, uint256 amount) external payable checkSaleState(amount) whenNotPaused nonReentrant returns (bool) {
