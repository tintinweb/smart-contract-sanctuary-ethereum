...
contract Gunx is ERC721A{
    ...
    constructor() ERC721A(){
	    _owner = msg.sender;
	    //_mintERC2309(msg.sender, 3);
    }

    function mint(uint256 amount) external payable{
        require(totalSupply() + amount <= MAX_SUPPLY, "Sold Out");
        require(amount*COST <= msg.value, "Value to Low");
        require(amount + _numberMinted(msg.sender) <= MAX_PER_WALLET, "Max per Wallet");
    ...
    function freeMint() external{
        uint256 amount = 1;
        require(totalSupply() + amount <= MAX_SUPPLY, "Sold Out");
        require(amount + _numberMinted(msg.sender) <= MAX_FREE_PER_WALLET, "Max Free per Wallet");
