  ...
  Ownable
{
  ...
  mapping(uint => uint) regensByTokenId;
      ...
      return string(
        abi.encodePacked(
          ...
          Base64.encode(
            bytes(
              abi.encodePacked(
                '{"description": "An on-chain (*)piece.","name":"Hypnoscopes","image":"<svg viewBox=\\"0 0 128 128\\" xmlns=\\"http://www.w3.org/2000/svg\\"></svg>"}'
              )
            )
          )
        )
  ...
  function tokenURI(uint256 tokenId) 
    ...
    returns (string memory) 
    {
      ...
      bytes memory attributes = abi.encodePacked(
        ...
        regensByTokenId[tokenId].toString(),
        '"}]'
      );
      ...
      return string(
        abi.encodePacked(
          ...
          Base64.encode(
            bytes(
              abi.encodePacked(
                '{"description":"An on-chain (*)piece.","name":"Hypnoscopes #',
                tokenId.toString(),
                '","image":"<svg viewBox=\\"0 0 1024 1024\\" width=\\"1024\\" height=\\"1024\\" xmlns=\\"http://www.w3.org/2000/svg\\" style=\\"background:white;\\"><defs>',
                defs,
                '</defs><g><animateTransform attributeName=\\"transform\\" type=\\"rotate\\" from=\\"0 512 512\\" to=\\"360 512 512\\" dur=\\"30s\\" repeatCount=\\"indefinite\\"/>',
                uses,
                '</g><mask id=\\"cut\\"><rect width=\\"100%\\" height=\\"100%\\" fill=\\"white\\"/><circle cx=\\"512\\" cy=\\"512\\" r=\\"512\\" fill=\\"#02075d\\"/></mask><rect x=\\"0\\" y=\\"0\\" width=\\"1024\\" height=\\"1024\\" mask=\\"url(#cut)\\" fill=\\"black\\"/></svg>",',
                attributes,
                '}'
              )
            )
  ...
  function regenerate(uint256 tokenId)
  public
  payable {
    require(tokenId < totalSupply(),
      "Token ID does not exist.");
    require(msg.sender == ownerOf(tokenId),
      "Must be owner to regenerate");
    require(msg.value >= PRICE,
      "Incorrect price.");
    _generateArt(1, tokenId);
    regensByTokenId[tokenId] += 1;
  }
    ...
    uint256 randomHash;
      ...
      for (uint x = 0; x < numLayers; x++) {
        for(uint256 y = 0; y < 3; y++){
          randomHash = uint256(keccak256(abi.encodePacked(block.timestamp, msg.sender, i, x, y, regensByTokenId[thisTokenId])));
          newColors[x][y] = randomHash % 255;
        }
        for(uint256 y= 0; y < 2; y++){
          randomHash = uint256(keccak256(abi.encodePacked(block.timestamp, msg.sender, i, x, y, regensByTokenId[thisTokenId])));
          newStats[x][y] = randomHash % 8;
          if (newCoords[x][y] % 2 != 0) newCoords[x][y] += 1; 
        }
        for(uint256 y= 0; y < 6; y++){
          randomHash = uint256(keccak256(abi.encodePacked(block.timestamp, msg.sender, i, x, y, regensByTokenId[thisTokenId])));
          newCoords[x][y] = randomHash % 384;
        }
