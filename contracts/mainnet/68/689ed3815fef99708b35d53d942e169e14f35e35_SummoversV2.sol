...
// interface to check balance of summoner for user
interface IstakingV2 {
    function balanceOf(address account) external view returns (uint256);
}

contract SummoversV2 is
    Initializable,
    ContextUpgradeable,
    IERC2981Upgradeable,
    OwnableUpgradeable,
    ...
    ERC721EnumerableUpgradeable
{
    ...
    address public pixelatedApe;
    address public summonersStaking;

    // map holders who have claimed
    mapping(address => bool) public holdersClaimed;
    ...
    //set holder amount
    function updateHolderAmount(uint256 _amount) public onlyOwner {
        holdersAmount = _amount;
    }

    // set pixelated ape address
    function setPixelatedApe(address _pixelatedApe) public onlyOwner {
        pixelatedApe = _pixelatedApe;
    }

    // set summoners staking address
    function setSummonersStaking(address _summonersStaking) public onlyOwner {
        summonersStaking = _summonersStaking;
    }
    ...
    function updatePixelatedApeStaking(
        address _pixelatedApeStaking
    ) public onlyOwner {
    ...
    // holder minting
    function holdersMint(uint256[] memory tokenId) public payable {
        // require that the stage is summoning
        require(stage == Stages.whitelist, "Minting not started");
        require(
            tokenId.length <= holdersAmount,
            "max amount of tokens to mint could be exceeded"
        );

        // check if holder has already minted
        require(
            !holdersClaimed[msg.sender],
            "You have already minted your tokens"
        );

        // check values be equal to whitelist price
        require(
            msg.value >= (tokenId.length * priceWhitelist),
            "insufficient funds"
        );

        IstakingV2 staking = IstakingV2(summonersStaking);
        uint256 balanceStaking = staking.balanceOf(msg.sender);

        IstakingV2 pixelatedApe_ = IstakingV2(pixelatedApe);
        uint256 balanceApes = pixelatedApe_.balanceOf(msg.sender);

        // get balance of sender
        uint256 currentBalance = balanceOf(msg.sender);

        // balance of pixape stahed
        IstakingV2 pixApeStaked = IstakingV2(pixelatedApeStaking);
        uint256 pixApeStakedBalance = pixApeStaked.balanceOf(msg.sender);

        // must have moree than 1 1 stakedsummoner or staked pixelated ape or summoner and pixelated ape
        require(
            balanceStaking > 0 ||
                balanceApes > 0 ||
                pixApeStakedBalance > 0 ||
                currentBalance > 0,
            "not permitted to mint"
        );

        // add to holdersClaimed
        holdersClaimed[msg.sender] = true;

        for (uint256 i = 0; i < tokenId.length; i++) {
            require(
                tokenId[i] >= 0 && tokenId[i] < MAX_SUPPLY,
                "Token ID invalid"
            );
            require(!_exists(tokenId[i]), "Token already minted");
            // if token is 10000 replace with 94
            if (tokenId[i] == 10000) {
                _safeMint(msg.sender, 94);
            } else {
    ...
    function mintWhitelistSummoners(
        ...
        // max of 2 tokens
        require(tokenIds_.length <= whitelistAmount, "max claimable exceeded");
    ...
    function mintAllSummonersBatch(uint256[] memory tokenIds_) public payable {
        require(stage == Stages.PublicMint, "Public Minting not started");
        require(
            msg.value >= pricePublic * tokenIds_.length,
            "Price is too low"
        );
        for (uint256 i = 0; i < tokenIds_.length; i++) {
            require(
