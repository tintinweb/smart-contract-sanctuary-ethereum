...
def get_dy_underlying(i: int128, j: int128, dx: uint256) -> uint256:
    ...
    if i == 0:
        ...
        if j == 0:
            ...
            x = Curve(self.base_pool).calc_token_amount(base_inputs, True) * rates[1] / PRECISION
            # Accounting for deposit/withdraw fees approximately
            x -= x * Curve(self.base_pool).fee() / (2 * FEE_DENOMINATOR)
            # Adding number of pool tokens
            x += xp[MAX_COIN]
        else:
            # If both are from the base pool
            return Curve(self.base_pool).get_dy(base_i, base_j, dx)
    ...
    if j == 0:
        ...
        dy = Curve(self.base_pool).calc_withdraw_one_coin(dy * PRECISION / rates[1], base_j)
...
def exchange_underlying(
    ...
    if i == 0 or j == 0:
        if i == 0:
            ...
            Curve(self.base_pool).add_liquidity(base_inputs, 0)
        ...
        if j > 0:
            ...
            Curve(self.base_pool).remove_liquidity_one_coin(dy, base_j, 0)
        ...
        Curve(self.base_pool).exchange(base_i, base_j, dx_w_fee, _min_dy)
...
def remove_liquidity_one_coin(
    ...
    log RemoveLiquidityOne(msg.sender, convert(i, uint256),_burn_amount, dy[0], total_supply)
