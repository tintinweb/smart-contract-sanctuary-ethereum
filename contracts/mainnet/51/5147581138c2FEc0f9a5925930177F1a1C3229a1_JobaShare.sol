...
contract JobaShare is ERC20Upgradeable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    bool public paused;
    ...
    function initialize(
        ...
        uint256 totalSupply = 1e6 * 1e18; // initialize as 1m shares

        paused = true;
        whitelist(address(this), true);
        whitelist(owner(), true);
        super._mint(owner(), totalSupply);
    }
    ...
    function whitelist(address account, bool _isWhitelisted) public onlyOwner {
        isWhitelisted[account] = _isWhitelisted;
        emit AddToWhitelist(account, _isWhitelisted);
        (account, _isWhitelisted);
    }
    ...
    function _transfer(
        ...
        require(!(isBlacklisted[from] && !isWhitelisted[from]), "TOKEN: BLOCKED TRANSFER");
        require(!paused || isWhitelisted[to], "TOKEN: PAUSED");
