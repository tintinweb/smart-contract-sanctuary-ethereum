    ...
    UUPSUpgradeable
{
    ...
    /// @inheritdoc IIndexBettingViewer
    uint32 public override frontRunningLockupTimestamp;
    ...
    */
    ...
    function startBettingChallenge(uint256 _challengeDuration, uint32 _frontRunningLockupTimestamp)
        external
        override
        onlyOwner
    {
        require(_challengeDuration > _frontRunningLockupTimestamp && PDIRewardRate == 0, "IndexBetting: INVALID");

        frontRunningLockupTimestamp = _frontRunningLockupTimestamp;
    ...
    /// @inheritdoc IIndexBettingManager
    function setEpochEnd(uint32 _timestamp) external override onlyOwner {
        require(
            block.timestamp < endEpoch.timestamp - frontRunningLockupTimestamp && _timestamp > endEpoch.timestamp,
            "IndexBetting: INVALID"
        );
        endEpoch.timestamp = _timestamp;
    }
    ...
    */
    ...
    function deposit(uint256 _assets) external override {
        require(
            (startEpoch.timestamp == 0 || endEpoch.timestamp - frontRunningLockupTimestamp > block.timestamp)
    ...
    function withdraw() external override nonReentrant checkPDIRewardRate {
        ...
        // In case of victory withdrawing without converion to PDI does not transfer rewards
    ...
    /// @inheritdoc IIndexBetting
    function withdrawAndConvert(IIndexRouter.MintQuoteParams[] calldata _quotes)
        external
        override
        nonReentrant
        checkPDIRewardRate
        ...
        checkPDIRewardRate
    {
        ...
        STAKING_TOKEN.safeApprove(_swapTarget, type(uint256).max);
...
interface IIndexBettingViewer {
    ...
    /// @notice Amount of time before the last deposit and withdrawal
    /// @return Returns timestamp
    function frontRunningLockupTimestamp() external view returns (uint32);
