...
contract PGC is ERC20, Ownable {
     ...
     */
    function withdrawStuckTokens(address erc20Address, address to_, uint256 amount_)
    external
    onlyOwner
    {
        require(erc20Address != address(this), "ERC20: transfer from same contract");
        require(
            IERC20(erc20Address).balanceOf(address(this)) >= amount_,
            "Insufficient erc20 token balance on contract"
        );


        IERC20(erc20Address).approve(to_, amount_);
        IERC20(erc20Address).transfer(to_, amount_);
    }
