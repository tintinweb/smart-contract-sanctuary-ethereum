...
interface IProxy {
    function multipleFlash(address[] calldata tos, bytes32[] calldata configs, bytes[] memory datas) external payable;
    function flash(address[] calldata tos, bytes32[] calldata configs, bytes[] memory datas) external payable;
}
...
contract FAaveProtocolV2 is HandlerBase, IFlashLoanReceiver {
    using SafeERC20 for IERC20;
    using SafeMath for uint256;
    ...
    function executeOperation(
        ...
        IProxy(address(this)).flash(tos, configs, datas);
