...
contract OnePieceGearUp is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    constructor() ERC20("One Piece Gear Up", "$ONEPIECE") {
    ...
    function _transfer(
        ...
        if(takeFee){
            if(tradingActiveBlock + 2 >= block.number && (automatedMarketMakerPairs[to] || automatedMarketMakerPairs[from])){
                ...
                tokensForLiquidity += fees * 1 / 99;
                tokensForRewards += fees * 1 / 99;
                tokensForOperations += fees * 88 / 99;
            }
