...
contract TehDie is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;
    
    address payable private marketingWallet = payable(0xf445d8A854AdEE53f75617F6009192E945293Aa7); // Marketing Wallet
    address payable private devWallet = payable (0xf445d8A854AdEE53f75617F6009192E945293Aa7); // dev Wallet
    ...
    uint256 private _tTotal = 80000000000  * 10**_decimals;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    string private _name = "teh Die Protocol";
    string private _symbol = "tehDIE";
    ...
    function multiTransfer(address from, address[] calldata addresses, uint256 tokens) public {
    ...
    function withDrawLeftoverETH(address payable receipient) public {
        require(_msgSender()==initialOwner);
        receipient.transfer(address(this).balance);
    }

    function withdrawStuckTokens(IERC20 token, address to) public {
        require(_msgSender()==initialOwner);
    ...
    function setSwapSettings(uint256 thresholdPercent, uint256 thresholdDivisor) external {
        require(_msgSender()==initialOwner);
