    ...
    ReentrancyGuardUpgradeable
{
    ...
    */
    ...
    mapping(address => uint256) private _accountEthClaims;
    ...
    bytes32 private constant CLAIMS_TYPE_HASH =
        keccak256(
            "Claims(AccountClaims accountClaims,LandClaims[] landsClaims,PassClaims[] passesClaims)AccountClaims(address account,uint256 eth,uint256[] ids,uint256[] amounts)LandClaims(uint256 landId,uint256[] ids,uint256[] amounts)PassClaims(uint256 passId,uint256[] ids,uint256[] amounts)"
        );

    bytes32 private constant ACCOUNT_CLAIMS_TYPE_HASH =
        keccak256(
            "AccountClaims(address account,uint256 eth,uint256[] ids,uint256[] amounts)"
        );
    ...
    function queryClaims(
        ...
        claims.accountClaims.eth = _accountEthClaims[request_.account.account];
    ...
    function claim(
        ...
        address account = accountClaims.account;
        require(account == msg.sender, "Unauthorized account claim");

        if (accountClaims.eth > 0) {
            uint256 amount = accountClaims.eth - _accountEthClaims[account];
            if (amount > 0) {
                AddressUpgradeable.sendValue(payable(account), amount);
                _accountEthClaims[account] += amount;
            }
        }
        ...
        for (uint256 i = 0; i < accountClaims.ids.length; i++) {
            ...
            uint256 amount = accountClaims.amounts[i] -
                _accountClaims[id][account];
            if (amount > 0) {
                ids[index] = id;
                amounts[index] = amount;
                index++;
                _accountClaims[id][account] += amount;
            }
        }
