...
// File: contracts/Aethirverse.sol
...
interface YieldToken {
    function updateReward(address from, address to, uint256 tokenId) external;
}
...
contract SimpleCollectible is ERC721A, Ownable {
    ...
    //count mints for presale
    mapping(address => uint256) count;

    constructor (address _wl) ERC721A ("Raggedy Army","RAGGEDY", 7777, 100)  {
        setBaseURI('ipfs://QmVGD88yqDmzobWJCRXc2WWBQA57Jr7dqyDRkQW8UfDgij');
        setBaseSuffix('.json');
        setWLNFT(_wl);
    }
    ...
    function mintPresaleCollectibles(uint256 _count, bool isUsingWLNFT) public payable {
        ...
        require(_count < 6, "Cant mint more than 5 at once");
        require((_count + count[msg.sender] < 11), "cant mint more than 10");
        require((_count + tokenCounter) <= getPresaleSupply(), "Ran out of NFTs for presale! Sry!");
        require(msg.value >= (.077 ether * _count) || (isUsingWLNFT && msg.value >= (.03 ether * _count)), "Ether value sent is too low");
        if (isUsingWLNFT) {
            IERC1155(wl_nft).safeTransferFrom(msg.sender, 0x000000000000000000000000000000000000dEaD, WL_NFT_TOKEN_ID, _count, '');
        }
    ...
    function ownerMint(uint256 _count, address[] memory users) external onlyOwner {
        require((_count * users.length) + tokenCounter < _totalSupply, "Cant mint more than mintMax");

        for(uint256 i = 0; i < users.length; i++) {
            createCollectibles(users[i], _count);
        }
    }
	...
	function transferFrom(address from, address to, uint256 tokenId) public override {
        if (yieldToken != address(0)) {
		    YieldToken(yieldToken).updateReward(from, to, tokenId);
        }
