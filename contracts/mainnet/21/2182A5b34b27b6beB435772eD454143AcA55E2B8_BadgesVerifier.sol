...
contract BadgesVerifier is Ownable {
    ...
    function claimableWalletBadges(address wallet) external view returns (uint256[] memory) {
        uint256[] memory genesisMice = _getGenesisRelevantTokens(wallet);
        uint256[] memory babyMice = _getBabiesRelevantTokens(wallet);
    ...
    function getRelevantMice(address wallet) external view returns (uint256[] memory, uint256[] memory) {
        uint256[] memory relevantGenesis = _getGenesisRelevantTokens(wallet);
        uint256[] memory relevantBabies = _getBabiesRelevantTokens(wallet);
    ...
    function _getGenesisRelevantTokens(address wallet) internal view returns (uint256[] memory) {
        ...
        returns (WalletCollection memory walletCollection)
    {
        for (uint256 index = 0; index < tokens.length; index++) {
            ...
            if (chipId == 0) {
                ...
                uint8[8] memory traits = IDNAChip(dnaChipAddress).getTraitsArray(tokenId);
    ...
    function _getBabiesRelevantTokens(address wallet) internal view returns (uint256[] memory) {
...
interface IAnonymice is IERC721Enumerable {
    ...
    function fpi(string memory _hash) external view returns (string memory);
