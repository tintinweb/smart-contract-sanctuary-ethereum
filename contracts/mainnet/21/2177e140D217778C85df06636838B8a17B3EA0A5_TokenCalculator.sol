...
/// @dev This contract is used as an adapter to get a calculated token value for an address
contract TokenCalculator is IVaultCalculator {
    address sharesHolder;
    uint256 private constant DECIMALS = 18;

    constructor(address _sharesHolder) {
        require(
            _sharesHolder != address(0),
            "shares holder can not be the zero address"
        );

        sharesHolder = _sharesHolder;
    }

    /// @notice The decimals are assumed to be 18
    /// @dev Read the token quantity value
    /// @param vault The address that holds tokens
    function calculate(address vault) external view override returns (uint256) {
        uint256 decimals = uint256(ERC20(vault).decimals());

        require(DECIMALS >= decimals);

        return
            IERC20(vault).balanceOf(sharesHolder) *
            10**uint256(DECIMALS - decimals);
    }
}
