...
contract StakingPool is OwnableUpgradeable {
    ...
    struct UserInfo {
        ...
        uint256 rewardDebt; // Reward debt -> accTokenPerShare * amount - rewardDebt = claimable token
    }
    ...
    struct PoolInfo {
        ...
        uint256 lastRewardTimestamp; // Last block timestamp that distribution occurs.
        uint256 accTokenPerShare; // Accumulated TOKENs per share, times precision. See below.
        uint256 startTime; // time when rewards are starting
        uint256 endTime; // time when rewards are ending
        uint256 precision; // precision is per pool, since tokens with 2 and 18 decimals cant both use 1e12 precision
        uint256 totalStaked; // total tokens staked in the pool
        uint256 totalReward; // total reward released lineary
        address owner; // owner of the pool that can stop reward
    }

    // Info of each pool.
    PoolInfo[] public poolInfo;

    // user => poolId => UserInfo
    ...
    error PrecisionTooBig();

    function initialize() external initializer {
        __Ownable_init();
    }

    // creates staking pool and sends reward tokens to contract
    function addPool(
        ...
        if (precision > 36) revert PrecisionTooBig();
        ...
        rewardTokenInterface.safeTransferFrom(msg.sender, address(this), totalReward);

        emit PoolCreated(stakingToken, rewardToken, startTime, endTime, 10**precision, totalReward);
    }

    // stops reward and returns rest tokens to owner
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 lpSupply = pool.totalStaked;
        if (lpSupply == 0 || pool.startTime > block.timestamp) {
        ...
        pool.lastRewardTimestamp = min(block.timestamp, end);
    }

    // deposit staking tokens to pool and claim potential rewards
    function deposit(uint256 _amount, uint256 poolId) external {
        ...
        // first deposit, handle separately to save gas
        if (amount == 0) {
            user.amount = newAmount;
            pool.totalStaked += _amount;
            user.rewardDebt = (newAmount * pool.accTokenPerShare) / pool.precision;
            ...
            uint256 pending = (amount * accTokenPerShare) / precision - user.rewardDebt;
            ...
            if (pending == 0) {
                ...
                if (rewardToken == stakingToken) {
                    if (pending > _amount) {
                        stakingToken.safeTransfer(address(msg.sender), pending - _amount);
                    } else if (pending < _amount) {
                        stakingToken.safeTransferFrom(address(msg.sender), address(this), _amount - pending);
                    }
                } else {
        ...
        emit Deposit(msg.sender, _amount, poolId);
    }
    ...
    // EMERGENCY ONLY function used for saving funds if something goes wrong
    function saveMe(address tokenAddress, uint256 amount) external onlyOwner {
