 ...
 */
contract FeeRecipient_R1 is IFeeRecipient, ReentrancyGuardUpgradeSafe, OwnableUpgradeSafe {
    ...
    function claim() external override nonReentrant {
        ...
        if (balance >= MAX_COMMISSION) {
            ...
            require(success, "FeeRecipient: could not transfer fee");
        }
    }

    function _takeFee(uint256 amount) internal view returns (uint256 fee, uint256 rewards) {
        fee = amount.mul(commission).div(MAX_COMMISSION);
        rewards = amount.sub(fee);
    }
