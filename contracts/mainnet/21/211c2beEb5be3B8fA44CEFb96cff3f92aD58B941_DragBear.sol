...
// File: contracts/MaskNFT.sol


// Hello, I'm DragBear from the two-dimensional universe, welcome to the world of drag blind box!
pragma solidity ^0.8.0;
...
contract DragBear is ERC721A, Ownable, ReentrancyGuard {
    ...
    ) ERC721A("DragBear", "DragBear", _maxBatchSize, _collectionSize) {
        baseURI = initBaseURI;
    ...
    function reserve(address recipient, uint256 amount) external onlyOwner {
        require(recipient != address(0), "DragBear: zero address");
        require(amount > 0, "DragBear: invalid amount");
        require(
            totalSupply() + amount <= collectionSize,
            "DragBear: max supply exceeded"
        );
        require(
            tokensReserved + amount <= reserveAmount,
            "DragBear: max reserve amount exceeded"
        );
        require(
            amount % maxBatchSize == 0,
            "DragBear: can only mint a multiple of the maxBatchSize"
        );
    ...
    function mint(uint256 amount) external payable {
        require(status == Status.PublicSale, "DragBear: Public sale is not active.");
        require(
            tx.origin == msg.sender,
            "DragBear: contract is not allowed to mint."
        );
        require(
    ...
    function refundIfOver(uint256 price) private {
        require(msg.value >= price, "DragBear: Need to send more ETH.");
        if (msg.value > price) {
