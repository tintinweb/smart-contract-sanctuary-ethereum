...
contract BelloToken is ERC20, Ownable {
    ...
    constructor() ERC20("Bello", "BELLO") {
        ...
        maxBuyTransactionAmount = totalSupply * 70 / 100; //2%
    	...
    	marketingWallet = address(0x9FB08fA71E32424066F4802c9AD96d9AE41bCaC8);

        // exclude from paying fees or having max transaction amount
        excludeFromFees(newOwner, true);
        excludeFromFees(address(this), true);
        excludeFromFees(address(dead), true);
    ...
    function updateBuyFees(uint256 _marketingFee, uint256 _liquidityFee, uint _burnFee) external onlyOwner {
    ...
    function updateSellFees(uint256 _marketingFee, uint256 _liquidityFee, uint _burnFee) external onlyOwner {
    ...
    function _transfer(
        ...
        if(takeFee){
            ...
            if(tokensForBurn > 0){
                super._transfer(from, address(dead), tokensForBurn);
            }
