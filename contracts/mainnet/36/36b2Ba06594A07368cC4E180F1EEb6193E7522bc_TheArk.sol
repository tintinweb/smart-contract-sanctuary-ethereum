...
import "./utils/CollectionURI.sol";
import "./utils/ERC2981.sol";
import "./utils/String.sol";

contract TheArk is CollectionURI, Ownable, Mintable, ERC2981 {

    //Collection Creator's Address
    mapping(uint256 => address) private _collectionCreator;

    //get collection creator's address by tokenID
    function getCollectionCreator(uint256 tokenId) public view returns (address) {
        return _collectionCreator[tokenId];
    }

    constructor(
        address _owner,
        address _imx
    ) ERC721("The Ark", "ARK") Mintable(_owner, _imx) {}
    ...
    function _mintFor(
        ...
        (address tmpCollectionCreator, string memory tmpCollectionUri, string memory tmptokenUri, uint256 tmpPercent) = _deserializeMintingBlob(blueprint);
        _mintNFT(user, tmptokenUri, tmpCollectionUri, tokenID, tmpCollectionCreator, tmpPercent);
    }

    function _mintNFT(address recipient, string memory tmpTokenURI, string memory tmpCollectionUri, uint256 tmpTokenId, address tmpCollectionCreator, uint256 royalty)
        internal
        returns (uint256)
    {
        _safeMint(recipient, tmpTokenId);
        _setTokenURI(tmpTokenId, tmpTokenURI);
        _setCollectionURI(tmpTokenId, tmpCollectionUri);
        _collectionCreator[tmpTokenId] = tmpCollectionCreator;

        //Set Royalty to tokenID
        _setReceiver(tmpTokenId, tmpCollectionCreator);
        _setRoyaltyPercentage(tmpTokenId, royalty);

        return tmpTokenId;
    }
    
    //Helper function to break mintingBlob Data and store it on chain
    function _deserializeMintingBlob(bytes memory mintingBlob) internal pure returns (address, string memory, string memory, uint256) { 
        ...
        string memory tmpCollectionCreatorString = idParams[0];
        string memory tmpCollectionUri = idParams[1];
        string memory tmpTokenUri = idParams[2];
        string memory tmpPercent = idParams[3];
        
        address eventCreator = String.toAddress(tmpCollectionCreatorString);
        uint256 percent = String.toUint(tmpPercent);

        return (eventCreator, tmpCollectionUri, tmpTokenUri, percent);
    }
...
library String {
    ...
    function toAddress(string memory _a) internal pure returns (address _parsedAddress) {
...
// SPDX-License-Identifier: MIT
// A modification of https://github.com/MaxflowO2/ERC2981
// Thanks to MaxflowO2
...
abstract contract CollectionURI is ERC721URIStorage{
    
    // Optional mapping for token URIs
    mapping(uint256 => string) private _collectionURIs;

    /**
     * @dev See {IERC721Metadata-collectionURI}.
     */
    function collectionURI(uint256 tokenId) public view virtual returns (string memory) {
        require(_exists(tokenId), "CollectionURI: URI query for nonexistent token");

        string memory _collectionURI = _collectionURIs[tokenId];
        ...
        // If both are set, concatenate the baseURI and collectionURI (via abi.encodePacked).
        if (bytes(_collectionURI).length > 0) {
            return string(abi.encodePacked(base, _collectionURI));
        }

        return collectionURI(tokenId);
    }

    /**
     * @dev Sets `_collectionURI` as the collectionURI of `tokenId`.
     *
     * Requirements:
     *
     * - `tokenId` must exist.
     */
    function _setCollectionURI(uint256 tokenId, string memory _collectionURI) internal virtual {
        require(_exists(tokenId), "CollectionURI: URI set of nonexistent token");
        _collectionURIs[tokenId] = _collectionURI;
    }
    ...
    function _burn(uint256 tokenId) internal virtual override {
        ...
        if (bytes(_collectionURIs[tokenId]).length != 0) {
            delete _collectionURIs[tokenId];
        }
    }
}
