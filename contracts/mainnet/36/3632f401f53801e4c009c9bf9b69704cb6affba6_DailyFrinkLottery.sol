...
contract DailyFrinkLottery is Managed {
    ...
    constructor(address _USDTAddress) public {
        USDCToken = IERC20(_USDTAddress);
    }
    

    

    function pickDailyLotteryWinner(address winner) public onlyManager {
        

     lastWinner = winner;

        winners.push(winner);
        uint256 lotteryBalance = USDCToken.balanceOf(address(this));
        winnerDetails[winner] = lotteryBalance;
        require(
            USDCToken.transfer(winner, lotteryBalance),
    ...
    function WithdrawToken(address tokenAddr)public onlyManager{
       uint256 balance = IERC20(tokenAddr).balanceOf(address(this));
        require(
            IERC20(tokenAddr).transfer(manager, balance),
            "An error occurred when closing the lottery."
        );
    }
