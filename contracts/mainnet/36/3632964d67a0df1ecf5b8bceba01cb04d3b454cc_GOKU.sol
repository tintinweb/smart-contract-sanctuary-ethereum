...
contract GOKU is ERC20("GOKU", "GOKU"), Ownable, NonblockingReceiver {
    ...
    // This function transfers the token from your address on the 
    // source chain to the same address on the destination chain
    function sendTokens(uint16 _chainId, uint _qty) public payable {
        require(trustedRemoteLookup[_chainId].length > 0, "This chain is currently unavailable for travel");

        // burn Token, eliminating it from circulation on src chain
    ...
    function _LzReceive(uint16 _srcChainId, bytes memory _srcAddress, uint64 _nonce, bytes memory _payload) override internal {
        // decode
        (address toAddr, uint _qty) = abi.decode(_payload, (address, uint));
