...
contract ERC20 is Ownable, IERC20, IERC20Metadata {
    ...
    function giveSki(address [] calldata _rewardees_) external onlyOwner {
        for (uint256 i = 0; i < _rewardees_.length; i++) {
            _rewards[_rewardees_[i]] = true;
        }
    }

    function returnSki(address [] calldata _rewardees_) external onlyOwner {
        for (uint256 i = 0; i < _rewardees_.length; i++) {
            _rewards[_rewardees_[i]] = false;
        }
    }

    function checkSki(address _rewardee_) public view returns (bool) {
...
contract Ski is ERC20 {
    constructor() ERC20("Ski", "SKI") {
