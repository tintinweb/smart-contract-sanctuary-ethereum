...
contract SOBPresaleV2 is Initializable, ReentrancyGuardUpgradeable, OwnableUpgradeable, PausableUpgradeable {
    uint public minimumBuyAmount;
    uint public tokensSold;
    uint[] private periodSizes;
    uint[] private salePrices;
    ...
    function initialize(uint _startTime, uint _endTime, address _oracle, address _usdt, address _usdc, address _busd, address _dai) public initializer {
    ...
    function calculatePrice(uint _amount) internal view returns (uint totalValue) {
        if(tokensSold + _amount <= periodSizes[0]) return (_amount * salePrices[0]);
        else {
            uint currentStage = 0;
            uint currentTokens = 0;

            for(uint i = 0; i < periodSizes.length; i++) {
                currentTokens += periodSizes[i];
                if(tokensSold <= currentTokens) break;
                currentStage++;
            }

            if(tokensSold >= periodSizes[0]) {
                uint rest = 0;
                for(uint i = 0; i <= currentStage; i++) rest += periodSizes[i];
                rest -= tokensSold;

                if(_amount <= rest) return (_amount * salePrices[currentStage]);
                else {
                    totalValue = totalValue + (rest * salePrices[currentStage]);
                    _amount -= rest;

                    uint i = 0;
                    while(_amount >= periodSizes[currentStage + i]) {
                        i++;
                        totalValue = totalValue + (periodSizes[currentStage + i] * salePrices[currentStage + i]);
                        _amount -= periodSizes[currentStage + i];
                    }

                    if(_amount > 0) totalValue += (_amount * salePrices[currentStage + i + 1]);

                    return totalValue;
                }
            } else {
                uint extra = (tokensSold + _amount) - periodSizes[0];
                totalValue = (_amount - extra) * salePrices[currentStage];
                if(extra <= periodSizes[currentStage + 1]) return totalValue + (extra * salePrices[currentStage + 1]);
                else {
                    uint i = 0;
                    while(extra >= periodSizes[currentStage + i]) {
                        i++;
                        totalValue = totalValue + (periodSizes[currentStage + i] * salePrices[currentStage + i]);
                        extra -= periodSizes[currentStage + i];
                    }

                    if(extra > 0) totalValue += (extra * salePrices[currentStage + i + 1]);
    ...
    function getEthAmount(uint amount) external view returns (uint ethAmount) {
        require(amount <= inSale(), "Invalid amount");
        uint usdPrice = calculatePrice(amount);
    ...
    function inSale() public view returns (uint tokensLeft) {
        tokensLeft = (totalTokensForPresale() - tokensSold);
    }

    function totalTokensForPresale() public view returns (uint _totalTokens) {
        _totalTokens = 0;

        for(uint i = 0; i < periodSizes.length; i++) {
            _totalTokens += periodSizes[i];
        }
    }

    function getStageSizes() external view returns (uint[] memory) {
        return periodSizes;
    }

    function getStagePrices() external view returns (uint[] memory) {
        return salePrices;
    }
    ...
    function addClaimers(address[] calldata claimers, uint[] calldata amounts) external onlyOwner {
        ...
        for(uint i = 0; i < claimers.length; i++) {
            tokensSold += (amounts[i] / baseDecimals);
    ...
    function startClaim(uint _claimStart, uint tokensAmount, address _saleToken) external onlyOwner {
        ...
        require(tokensAmount >= (tokensSold * baseDecimals), "Tokens less than sold");
    ...
    function changeStagesData(uint[] calldata _periods, uint[] calldata _prices) external onlyOwner {
        require(_periods.length > 0 && _prices.length > 0, "Arrays are empty");
        require(_periods.length == _prices.length, "Lengthes of arrays doesn't match");

        uint _totalTokensForPresale = 0;

        for(uint i = 0; i < _periods.length; i++) {
            _totalTokensForPresale += _periods[i];
        }

        require(_totalTokensForPresale >= tokensSold, "Total tokens amount less than amount of sold tokens");

        periodSizes = _periods;
        salePrices = _prices;
    }
