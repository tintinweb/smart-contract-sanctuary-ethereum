...
contract TTX is ERC20Capped, Ownable {
    ...
    uint256 public constant MAX_TRANSFER_TAX_PERCENTAGE = 2;
     uint256 public Current_TRANSFER_TAX_PERCENTAGE = 0;
    ...
    constructor() ERC20("Tetra X Network", "TTX") ERC20Capped(2000000000 * (10 ** 18)) {
    ...
    function setTransferTaxPercentage (uint256 _taxPercentage) external onlyOwner {
       Current_TRANSFER_TAX_PERCENTAGE = _taxPercentage <= MAX_TRANSFER_TAX_PERCENTAGE ? _taxPercentage : 0;
       require(_taxPercentage <= 2, "Tax Percentage cannot be greater than 2 percent");
       require(Current_TRANSFER_TAX_PERCENTAGE != _taxPercentage, "Making an attempt to set the same value");
    }
    ...
    function _transfer(address from, address to, uint amount) internal override {
        ...
        if(
            ...
            tax = (amount * MAX_TRANSFER_TAX_PERCENTAGE)/100;
    ...
    function retrieveTokens(address token, uint amount) external onlyOwner {
        ...
        require(token != address(this), "Cannot be TTX address");
