...
contract SHIBRINTER is Context, IERC20, Ownable {
    ...
    string private _name = "SHIBRINTER";
    string private _symbol = "SHIBRINTER";
...
contract DividendTracker is IERC20, Context, Ownable {
    ...
    string private _name = "SHIBRINTER TRACKER";
    string private _symbol = "SHIBRINTER_T";
    uint8 private _decimals = 9;
    ...
    function setTokenBalance(address account) public {
        uint256 balance = Shibrinter.balanceOf(account);
        if (!Shibrinter.isExcludedFromRewards(account)) {
    ...
    function setERC20Contract(address contractAddr) external onlyOwner {
        Shibrinter = SHIBRINTER(payable(contractAddr));
    }
