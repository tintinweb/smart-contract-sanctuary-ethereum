...
// Contract for claiming CRETH2 and optionally USDC.
contract CRETH2Repayment is Ownable, ReentrancyGuard {
    ...
    address public immutable UsdcRepayment;
    ...
    constructor(bytes32 _merkleRoot, address _creth2, address _usdcRepayment, address _cream, address _creamReceiver) {
        merkleRoot = _merkleRoot;
        CRETH2 = _creth2;
        UsdcRepayment = _usdcRepayment;
        CREAM = _cream;
    ...
    function claimAll(
        uint256 creth2Amount, uint256 creamReturnAmount, bytes32[] memory creth2Proof,
        uint256 usdcAmount, bytes32[] memory usdcProof
    ) external {
        _claimAndTransfer(msg.sender, msg.sender, creth2Amount, creamReturnAmount, creth2Proof);
        RepaymentContract(UsdcRepayment).claimFor(msg.sender, usdcAmount, usdcProof);
    }
