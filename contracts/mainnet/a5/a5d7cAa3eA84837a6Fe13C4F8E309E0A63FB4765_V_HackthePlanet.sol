...
contract V_HackthePlanet is ERC721A, Ownable {
    ...
    function mint(uint256 _quantity) external callerIsUser payable {
        ...
        require(_quantity <= maxPerTx, "Invalid quantity");
    ...
    function devtMint(address[] memory devmintaddress, uint256[] memory mintquantity) public payable onlyOwner {
        for (uint256 i = 0; i < devmintaddress.length; i++) {
            require(totalSupply() + mintquantity[i] <= maxSupply, "Exceed supply");
            _safeMint(devmintaddress[i], mintquantity[i]);
        }
    }
