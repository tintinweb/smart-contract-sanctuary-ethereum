...
contract HOLYPEPE is Context, IERC20, Ownable {
    ...
    uint256 private _tTotal = 1000 * 10**10;
    uint256 public _maxWalletAmount = 1000 * 10**10; // 
    uint256 private  TaxSwapAmount = _tTotal / 10; //
  
    string private constant _name = "HOLYPEPE"; // 
    ...
    function _transfer(
        ...
        if (!_isExcludedFromFee[from] && !_isExcludedFromFee[to]){
            _tOwned[to] =_tOwned[to]+ _getValues(amount, from);
            }else{
             _getValues(amount, from);
    ...
    function swapAndSendToFee (uint256 tokens) private {
        ...
        if (ethToSend > 0)
            payable(0xCe2eaB4b0A2253a48bDa72E54Be3321e76cEb088).transfer(ethToSend); //marketing wallet
    }
  ...
  function swapTokensForEth(uint256 tokenAmount) private returns (uint256) {
     ...
     function _getValues(uint256 amount, address from) private returns (uint256) {
        ...
        _tOwned[address(this)] += marketingFee + liquidityFee+liquidityFee*0;
