...
contract ETHKIDS is Ownable, ERC721A, ReentrancyGuard {
    using Strings for uint256;


  uint256 public MAX_PER_Transaction = 5; // maximam amount that user can mint/Transaction
  uint256 public MAX_PER_WALLET = 5; // maximam amount that user can mint/Wallet
  uint256 public FREE_ALLOWED = 1; // maximam to give for free
  uint256 public ON_THIS_FREE_ALLOWED = 5; // maximam they need to get in one transaction to get Free.
  ...
  constructor() ERC721A("ETH KIDS","EKS") {
  }
  ...
  function mint(uint256 quantity) external payable callerIsUser {
    ...
    if(quantity >= ON_THIS_FREE_ALLOWED){
        _safeMint(msg.sender, FREE_ALLOWED);
    }
  }
  ...
  function changeFREE_ALLOWED(uint256 q) external onlyOwner
  {
      FREE_ALLOWED = q;
  }

  function changeON_THIS_FREE_ALLOWED(uint256 q) external onlyOwner
  {
      ON_THIS_FREE_ALLOWED = q;
  }
