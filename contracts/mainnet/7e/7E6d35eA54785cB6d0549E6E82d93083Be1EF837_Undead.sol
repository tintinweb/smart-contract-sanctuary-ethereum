...
contract Undead is Ownable, ERC721A, ReentrancyGuard, ERC721ABurnable, DefaultOperatorFilterer{
...
constructor() ERC721A("Elon's Undead Upgrades", "Modificant") {}


    function PreMint(uint256 quantity, address receiver) public onlyOwner {
        require(
            totalSupply() + quantity <= COLLECTION_SIZE,
            "No more Undead in stock!"
        );
        ...
        nonReentrant
    {
        ...
        require(isPublicMintEnabled, "Undead not ready yet!");
        require(totalSupply() + quantity <= COLLECTION_SIZE, "No more Undead left!");

        require(balanceOf(msg.sender) + quantity <= SUPPLY_PER_WALLET, "Tried to mint Undead over over limit");

        require(quantity <= MAX_BATCH_SIZE, "Tried to mint Undead over limit, retry with reduced quantity");
