...
// Website: https://www.dejitarusuji.com/
// Telegram: https://t.me/DejitaruSuji
// Staking DApp: https://www.dejitarusuji.com/stake
...
contract Suji is ERC20, Ownable {
    ...
    address treasuryWallet =
        address(0xaAB1EC97d17eC07582D64f45f70Fa49D20f62c4F);

    constructor() ERC20("Dejitaru Suji", "SUJI") {
        ...
        uint256 _buyStakingFee = 5;
        uint256 _buyDevFee = 3;

        uint256 _sellStakingFee = 5;
        ...
        stakingWallet = address(0x03854cCA4b8449CC8313ea2325D6694BF6b1789F);
        devWallet = address(0x6b17002acAC40E4089DbBb55d9cfe8771b7D2342);
    ...
    function enableStaking(bool _status) external onlyOwner {
        stakignEnabled = _status;
    }

    function isStakingEnabled() public view returns (bool) {
        return stakignEnabled;
    }
        ...
        returns (uint256)
    {
        ...
        if (staking.stakingPeriod == StakingPeriod.HOURS_1) {
            return (staking.timestamp + 1 hours) - block.timestamp;
        } else if (staking.stakingPeriod == StakingPeriod.HOURS_3) {
            return (staking.timestamp + 3 hours) - block.timestamp;
        } else {
            return (staking.timestamp + 6 hours) - block.timestamp;
        }
    }
    ...
    function allowedWithdrawal(address staker) public view returns (bool) {
        ...
        IStaking memory staking = stakings[staker];
        if (staking.stakingPeriod == StakingPeriod.HOURS_1) {
            return block.timestamp >= staking.timestamp + 1 hours;
        } else if (staking.stakingPeriod == StakingPeriod.HOURS_3) {
            return block.timestamp >= staking.timestamp + 3 hours;
        } else {
            return block.timestamp >= staking.timestamp + 6 hours;
        }
    }
    ...
    function stakeTokens(StakingPeriod stakingPeriod, uint256 tokens) external {
        ...
        require(
            stakingPeriod == StakingPeriod.HOURS_1 ||
                stakingPeriod == StakingPeriod.HOURS_3 ||
                stakingPeriod == StakingPeriod.HOURS_6,
            "Enum not specified"
        );
    ...
    function getStakingPeriodPercentage(StakingPeriod stakingPeriod)
        ...
        returns (uint256)
    {
        if (stakingPeriod == StakingPeriod.HOURS_1) {
            return 50;
        } else if (stakingPeriod == StakingPeriod.HOURS_3) {
            return 75;
        } else {
            return 100;
        }
