...
contract SwapForMe {
  ...
  function swap(
    ...
    for (uint i=1; i < amounts.length; i++) {
      ...
      address swapTo = (i + 1 == amounts.length) ? msg.sender : pairs[i + 1];
      if (i == 0) {
        IERC20 iTokenIn = IERC20(tokenIn);
        iTokenIn.transferFrom(msg.sender, pairAddress, amountIn);
      }

      v2Pair.swap(amount0, amount1, swapTo, new bytes(0));
    }
  }
