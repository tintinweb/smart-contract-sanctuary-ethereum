...
contract TheGrindsetters is ERC721A, Ownable, ReentrancyGuard {
    ...
    function CalculateCost(
        ...
        if (
            (_balance + _mintAmount) <= (maxLimitPerWallet - freeMintPerWallet)
        ) {
            // All items to be priced with payable cost
            return _mintAmount * costPayable;
        } else if (
            (_balance + _mintAmount) >
            (maxLimitPerWallet - freeMintPerWallet) &&
            (_mintAmount <= freeMintPerWallet)
        ) {
            // All items to be priced with free cost
            return _mintAmount * costFree;
        } else {
            return
                ((_mintAmount - freeMintPerWallet) * costPayable) +
                (freeMintPerWallet * costFree);
        }
    }
    ...
    function withdraw() public onlyOwner nonReentrant {
        for (uint i = 0; i < beneficiaryList.length; i++) {
            (bool success, ) = payable(beneficiaryList[i]).call{ // Distribute contract balance to beneficary list with defined percents
                value: (address(this).balance / 100) * beneficiaryPercents[i]
            }("");
