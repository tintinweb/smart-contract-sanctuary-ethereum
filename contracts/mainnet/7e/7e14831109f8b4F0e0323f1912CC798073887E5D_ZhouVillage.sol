 ...
 */
contract ZhouVillage is IERC721A, Ownable, DefaultOperatorFilterer, ERC2981{
    ...
    string public baseURI_ = "https://zhouvillage.mypinata.cloud/ipfs/QmPpEKcodPKHpdbkuq2vs9FzwYYFCJ73h9FSVcMaovJRqR/";
    string public uriSuffix = "";
    string public contractURI = "https://zhouvillage.mypinata.cloud/ipfs/Qme5XewmSCfmhm8PExyG56trawZPK9FNQgTvmDLhEjSzYS";
    Ibark bark;
    ...
    function publicMint(uint256 amount) public payable {
        ...
        require( amount <= maxPertransaction, "zhou exceed  max per transaction");
        require(totalSupply() + amount <= maxSupply, "zhou exceed  maxsupply");
    ...
    function whiteListMint(uint256 amount, bytes32[] memory proof) public payable {
        require(mintingStarted, "minting has not started");
        require( amount <= maxPertransaction, "zhou exceed max per transaction");
        require(isValid( proof,msg.sender), "wallet not whiteListed");
        require(totalSupply() + amount <= maxSupply, "zhou exceed maxsupply"); 
        require(mintedPerWallet[msg.sender] + amount <= maxPerWallet, "exceeded max per wallet");
    ...
    function setCost(uint256 _cost,uint256 _whiteListCost ) external onlyOwner {
        cost = _cost;
     ...
     * - The caller must own the token or be an approved operator.
     */
    function approve(address to, uint256 tokenId) public payable virtual override onlyAllowedOperatorApproval(to) {
