    ...
    UUPSUpgradeable
{
    ...
    // change nft address
    function changeNftAddress(address _nft) public onlyOwner {
        nft = IERC721Upgradeable(_nft);
    }
    ...
    function changeOwner(uint256 _tokenId, address _newOwner) public {
        ...
        require(
            ...
            "you cannot send to your own address"
        );
        // remove from staker
    ...
    // reset stake time to stakes time - claimed time
    function claimReward(uint256 _tokenId, uint256 claimDays) public {
        address owner_address = tokenOwner[_tokenId];
        // get staker from address
        Staker storage staker = stakers[owner_address];
        uint256 claimDaysInSeconds = claimDays * 1 days;
        // time staked must be greater than claim time
        require(
            staker.tokenStakingTime[_tokenId] + claimDaysInSeconds <= block.timestamp,
            "Staking System: the staking time must be greater than the claim time"
        );
        // only address in claimerAdresses can claim
        for (uint256 i = 0; i < claimerAdresses.length; i++) {
            if (msg.sender == claimerAdresses[i]) {
                staker.tokenStakingTime[_tokenId] = block.timestamp;
                return;
            }
        }
        // raise error if address is not in claimerAdresses
        revert("user must be in claimerAdresses");
    }
    ...
    function checkTokenStakedPeriodForUser(uint256 _tokenId, address user_)
        ...
        returns (uint256)
    {
        // tokenOwner[_tokenId] should be equal to user_
        require(
            stakers[user_].tokenStakingTime[_tokenId] > 0,
            "user must have staked the nft"
        );
        return stakers[tokenOwner[_tokenId]].tokenStakingTime[_tokenId];
    }
    ...
    // get tokens staked for more than a week for a user
        ...
        returns (uint256[] memory)
    {
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            if (
                stakers[user_].tokenStakingTime[tokenIds[i]] +
                    7 days <
                block.timestamp
            ) {
        ...
        uint256[] memory tokenIdsStakedForMoreThanAWeek_ = new uint256[](
            count
        );
