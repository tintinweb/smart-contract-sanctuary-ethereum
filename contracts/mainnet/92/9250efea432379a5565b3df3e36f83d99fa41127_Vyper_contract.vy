...
# The goal is to be able to calculate INTEG(rate * balance / totalSupply dt) from 0 till checkpoint
# All values are kept in units of being multiplied by 1e18
period: public(int128)
period_timestamp: public(uint256[100000000000000000000000000000])

# 1e18 * INTEG(rate(t) / totalSupply(t) dt) from 0 till checkpoint
integrate_inv_supply: public(uint256[100000000000000000000000000000])  # bump epoch when rate() changes

# 1e18 * INTEG(rate(t) / totalSupply(t) dt) from (last_action) till checkpoint
integrate_inv_supply_of: public(HashMap[address, uint256])
integrate_checkpoint_of: public(HashMap[address, uint256])

# INTEG(balance * rate(t) / totalSupply(t) dt) from 0 till checkpoint
...
# depositors which have permission to deposit
depositor: public(HashMap[address, uint256])
...
def deposit(_value: uint256, _addr: address = msg.sender, _claim_rewards: bool = False):
    ...
    assert self.depositor[_addr] > 0 # dev: granted addresses only
...
@external
def set_depositor(_depositor: address, _perm: uint256):
    """
    @notice Grant or revoke depositor's permission
    """
    assert msg.sender == self.admin

    self.depositor[_depositor] = _perm
