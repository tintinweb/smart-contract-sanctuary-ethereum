...
contract Gaimin is ERC20, Ownable {
    ...
    constructor() ERC20("Gaimin", "GMRX") {
        ...
        // Set default fees
        _buyMarketingFee = 2; // 0.2%
        _buyLiquidityFee = 4; // 0.4%

        _sellMarketingFee = 2; // 0.2%
        _sellLiquidityFee = 4; // 0.4%
    ...
    function createPair() external onlyOwner {
        require(_uniswapV2Pair == address(0), "GMRX: pair already created");
        // Create pair
     ...
     *
     * This function can only be called by the owner. It will fund the pair contract with WETH + GMRX token combinations.
     * all GMRX token will be transfered to the pair
     *
    ...
    function allowTrading() external onlyOwner {
        require(_uniswapV2Pair != address(0), "GMRX: pair not yet created");
        require(!_isTradingActive, "GMRX: trade is already open");
        // Add liquidity
        ...
        onlyOwner
    {
        require(
            ...
            "GMRX: cannot be lower than 0.001% total supply."
        );
        require(
            newAmount <= (totalSupply() * 5) / 1000,
            "GMRX: cannot be higher than 0.5% total supply."
        );
    ...
    function removeFromBlacklist(address account) external onlyOwner {
        require(blacklist[account] == true, "GMRX: account is not in the blacklist");
    ...
    function _transfer(
        ...
        if (!(_isExcludedFromFees[from] || _isExcludedFromFees[to])) require(_isTradingActive, "GMRX: trading is not active");
        ...
        if (_hasLimits(from, to, takeFee)) {
            ...
            require(
                amount <= maxTxAmount,
                "GMRX: max transaction amount exceeded"
            );
            if (automatedMarketMakerPairs[from]) {
                require(
                    (amount + balanceOf(to)) <= maxWallet,
                    "GMRX: max wallet amount exceeded"
                );
            }
        }
     ...
     *
     * Internal function to swap GMRX tokens to WETH
     *
     */
    function _swapTokensForEth(uint256 tokenAmount) private {
        // generate the uniswap pair path of GMRX -> WETH
     ...
     *
     * Internal function, it will fund the pair contract with WETH + GMRX token combinations.
     * all GMRX token will be transfered to the pair
     *
     * Emits an {Liquidity Added} event.
     *
     ...
     *
     * Internal function to swap back the GMRX tokens to WETH for further use
     *
     */
     ...
     * @dev Swap back the marketing + liquidity fees to WETH
     *
     * External function to force a swap back the GMRX tokens to WETH for further use
     *
     */
