...
contract FraxlendPair is IERC20Metadata, FraxlendPairCore {
    ...
    /// @param _oldOracle The old oracle address
    /// @param _oldMaxOracleDeviation The old max oracle deviation
    /// @param _newOracle The new oracle address
    /// @param _newMaxOracleDeviation The new max oracle deviation
    event SetOracleInfo(
        address _oldOracle,
        uint32 _oldMaxOracleDeviation,
        address _newOracle,
    ...
    event SetMaxLTV(uint256 _oldMaxLTV, uint256 _newMaxLTV);
    ...
    /// @param _oldRateContract The old rate contract
    /// @param _newRateContract The new rate contract
    event SetRateContract(address _oldRateContract, address _newRateContract);
    ...
    /// @param _oldCleanLiquidationFee The old clean liquidation fee
    /// @param _oldDirtyLiquidationFee The old dirty liquidation fee
    /// @param _oldProtocolLiquidationFee The old protocol liquidation fee
    ...
    event SetLiquidationFees(
        uint256 _oldCleanLiquidationFee,
        uint256 _oldDirtyLiquidationFee,
        uint256 _oldProtocolLiquidationFee,
    ...
    event WithdrawFees(uint128 _shares, address _recipient, uint256 _amountToTransfer, uint256 _collateralAmount);
...
/// @notice  An abstract contract which contains the access control logic
abstract contract FraxlendPairAccessControl is Timelock2Step, Ownable2Step, FraxlendPairAccessControlErrors {
    ...
    /// @param _borrowLimit The final permanent borrow limit
    event RevokeBorrowAccessControl(uint256 _borrowLimit);
    ...
    /// @param _limit The new borrow limit
    event SetBorrowLimit(uint256 _limit);
    ...
    /// @param _depositLimit The final permanent deposit limit
    event RevokeDepositAccessControl(uint256 _depositLimit);
    ...
    /// @param _limit The new deposit limit
    event SetDepositLimit(uint256 _limit);
    ...
    event PauseRepay(bool _isPaused);
    ...
    /// @param _isPaused The new paused state
    event PauseWithdraw(bool _isPaused);
    ...
    /// @param _isPaused The new paused state
    event PauseLiquidate(bool _isPaused);
    ...
    /// @param _isPaused The new paused state
    event PauseInterest(bool _isPaused);
...
abstract contract FraxlendPairCore is FraxlendPairAccessControl, FraxlendPairConstants, ERC20, ReentrancyGuard {
    ...
    event WarnOracleData(address _oracle);
