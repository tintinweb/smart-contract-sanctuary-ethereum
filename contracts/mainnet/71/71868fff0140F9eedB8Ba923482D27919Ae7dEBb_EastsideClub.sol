...
contract EastsideClub is IERC721A { 
    ...
    uint256 public constant COST = 0.0044 ether;

    string private constant _name = "Eastside Club";
    string private constant _symbol = "ESC";
    ...
    function freeMint() external{
        uint256 amount = MAX_FREE_PER_TX;
        address _caller = _msgSenderERC721A();

        require(saleIsActive, "Mint is not active right now.");
        require(totalSupply() + amount <= MAX_SUPPLY, "Sold out");
        require(amount + _numberMinted(msg.sender) <= 5, "Acc has token limit");
