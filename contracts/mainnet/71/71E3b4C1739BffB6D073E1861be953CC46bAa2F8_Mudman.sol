...
contract Mudman is Ownable, ERC721A, ReentrancyGuard {
    ...
    uint256 public immutable maxSupply = 1992;
    ...
    constructor(
        string memory name, //Mudman
        string memory symbol, //Mudman
        address payable _shareholderAddress, //0x69627CDa8418888CF519A367b761B9092d91ACC3
        string memory _baseURI // ipfs://bafybeiez4ogj5jlyx7vl2zx6ioi2ocmobiml7zovqejkhpgkl2npziejum/metadata/
    )ERC721A(name, symbol){
    ...
    function mint(uint256 _amount) external payable nonReentrant returns (bool) {
        require(totalSupply() <= maxSupply, "Mudman: exceeds the total amount.");
        require((mintMap[msg.sender] + _amount) <= maxMint, "Mudman: You have already minted.");
        uint256 _price = 0;
    ...
    returns (string memory)
    {
        require(
            _exists(tokenId),
            "Mudman: URI query for nonexistent token"
        );
        return string(
            abi.encodePacked(
                _baseTokenURI,
