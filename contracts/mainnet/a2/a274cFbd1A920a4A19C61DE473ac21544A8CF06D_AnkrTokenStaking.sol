...
abstract contract Staking is Initializable, Multicall, IStaking {
    ...
    function _processUnlockedDelegations(
        ...
        uint256 availableAmount = delegation.getStaked() * BALANCE_COMPACT_PRECISION;
        ...
        delegation.shrinkDelegations(uint112(expectedAmount / BALANCE_COMPACT_PRECISION), beforeEpochExclude);
    ...
    function _calcUnlockedDelegatedAmount(ValidatorDelegation storage delegation) internal view returns (uint256 unlockedAmount) {
        ...
        uint256 pendingWithdrawals = delegation.calcWithdrawalAmount(beforeEpochExclude, true) * BALANCE_COMPACT_PRECISION;
        if (unlockedAmount < pendingWithdrawals) {
            return 0;
        }
    ...
    function _undelegateFrom(address toDelegator, address fromValidator, uint256 amount) internal {
        ...
        // + re-save validator because last affected epoch might change
        _touchValidatorSnapshot(validator, beforeEpoch).safeDecreaseDelegated(uint112(amount / BALANCE_COMPACT_PRECISION));
        ...
        _processUnlockedDelegations(delegation, beforeEpoch, toDelegator, fromValidator, amount);
