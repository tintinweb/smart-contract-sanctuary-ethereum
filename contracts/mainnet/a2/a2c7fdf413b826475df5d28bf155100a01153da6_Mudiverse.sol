 ...
 * |  Y Y  \|  |  // /_/ | |  | \   / \  ___/ |  | \/\___ \ \  ___/  
 * |__|_|  /|____/ \____ | |__|  \_/   \___  >|__|  /____  > \___  > 
 *       \/             \/                 \/            \/      \/  
 *
...
contract Mudiverse is ERC721A, Ownable {
    ...
    uint16 public constant MAX_LOCATIONS = 20000;

    mapping(address => uint256) internal playerPositions;
    mapping(uint256 => Location) internal locations;
    mapping(uint256 => bool) internal frozen;
    ...
    function setFrozen(uint256 tokenId, bool isFrozen) external onlyOwner {
        if(!_exists(tokenId)) revert InvalidTokenId();

        frozen[tokenId] = isFrozen;
    }

    function setMetadata(uint256 tokenId, Location calldata data) external {
        if(!_exists(tokenId)) revert InvalidTokenId();
    ...
    function respawn(uint256 tokenId) external {
        if(!_exists(tokenId)) revert InvalidTokenId();
    ...
    function positionId() public view returns (uint256) {
        ...
        // Value will default to zero, but 0 is reserved for a "no exit" uint256 value.
    ...
    function tokenURI(uint256 tokenId) override public view returns (string memory) {
        Location memory data = locations[tokenId];
        ...
        traits[0] = string(abi.encodePacked('{"trait_type":"North","value":"', (data.north > 0 ? _toString(data.north) : "n/a"), '"},'));
        traits[1] = string(abi.encodePacked('{"trait_type":"South","value":"', (data.north > 0 ? _toString(data.south) : "n/a"), '"},'));
        traits[2] = string(abi.encodePacked('{"trait_type":"East","value":"', (data.north > 0 ? _toString(data.east) : "n/a"), '"},'));
        traits[3] = string(abi.encodePacked('{"trait_type":"West","value":"', (data.north > 0 ? _toString(data.west) : "n/a"), '"}'));
        ...
        parts[9] = _exists(data.north) ? locations[data.north].name : "n/a";
        parts[10] = '</tspan></text><text x="10" y="120" class="base" fill="#FFF">South: <tspan class="destination">';
        parts[11] = _exists(data.south) ? locations[data.south].name : "n/a";
        parts[12] = '</tspan></text><text x="10" y="140" class="base" fill="#FFF">East: <tspan class="destination">';
        parts[13] = _exists(data.east) ? locations[data.east].name : "n/a";
        parts[14] = '</tspan></text><text x="10" y="160" class="base" fill="#FFF">West: <tspan class="destination">';
        parts[15] = _exists(data.west) ? locations[data.west].name : "n/a";
        parts[16] = '</tspan></text></svg>';

        string memory output = string(abi.encodePacked(parts[0], parts[1], parts[2], parts[3], parts[4], parts[5], parts[6], parts[7], parts[8]));
        output = string(abi.encodePacked(output, parts[9], parts[10], parts[11], parts[12], parts[13], parts[14], parts[15], parts[16]));
        
        string memory json = Base64.encode(bytes(string(abi.encodePacked('{"name": "', locations[tokenId].name, '", "description": "', locations[tokenId].description ,'", "attributes": [', traits[0], traits[1], traits[2], traits[3], '], "image": "data:image/svg+xml;base64,', bytes(output), '"}'))));
