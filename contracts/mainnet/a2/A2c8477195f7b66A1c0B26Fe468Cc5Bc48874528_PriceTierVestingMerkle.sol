...
abstract contract AdvancedDistributor is Ownable, Distributor, Sweepable, IAdjustable, IVotesLite {
	...
	function getVotes(address beneficiary) external view override(IVotesLite) returns (uint256) {
		...
		return
			((records[beneficiary].total - records[beneficiary].claimed) * voteFactor) /
			fractionDenominator;
	}

	function getTotalVotes() external view override(IVotesLite) returns (uint256) {
		// Return total voting power for this distributor across all users
		return ((total - claimed) * voteFactor) / fractionDenominator;
	}

	function getVoteFactor(address) external view override(IVotesLite) returns (uint256) {
		return voteFactor;
	}
