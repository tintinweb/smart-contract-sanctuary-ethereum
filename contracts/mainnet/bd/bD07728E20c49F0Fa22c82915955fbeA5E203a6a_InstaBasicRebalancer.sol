...
contract InstaFlashReceiver {
    ...
    function _approve(
        ...
        TokenInterface tokenContract_ = TokenInterface(token);
        try tokenContract_.approve(recipient, amount) {
            
        } catch {
            IERC20 token_ = IERC20(token);
            token_.safeApprove(recipient, 0);
            token_.safeApprove(recipient, amount);   
        }
    }
