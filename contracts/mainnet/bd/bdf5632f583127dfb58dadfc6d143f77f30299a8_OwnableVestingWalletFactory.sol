...
import { OwnableVestingWallet } from "./OwnableVestingWallet.sol";

/**
 * @title OwanableVestingWalletFactory
 */
contract OwnableVestingWalletFactory {
    ...
    /**
     * @dev Set the implementation address of the vesting wallet
     */
    ...
    /**
     * @dev Create vesting wallet for a given
     * the beneficiary, start timestamp and vesting duration,
     * and owner of the vesting wallet.
     */
    function createVestingWallet(
        ...
        bytes32 salt = keccak256(
            abi.encode(msg.sender, beneficiary, block.chainid, startTimestamp)
        );
        OwnableVestingWallet wallet = OwnableVestingWallet(
            payable(implementation.cloneDeterministic(salt))
        );
        wallet.initialize(beneficiary, startTimestamp, durationSeconds, owner);

        emit VestingWalletCreated(address(wallet), beneficiary, startTimestamp, durationSeconds);
