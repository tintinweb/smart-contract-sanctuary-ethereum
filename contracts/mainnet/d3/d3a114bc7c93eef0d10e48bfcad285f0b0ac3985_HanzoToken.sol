...
// File: contracts/Hanzo.sol


pragma solidity ^0.8.10;


contract HanzoToken is Context, IERC20, Ownable {
    ...
    uint256 private _tTotal = 666 * 10**24 * 10**9;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    string private _name = "HANZO";
    string private _symbol = "HANZO";
    ...
    address public _marketingWallet = payable(address(0x286ac249aFee6D207A957afdF4E59Ba795be76B6));
    address public _buybackWallet = payable(address(0x953c70dA6a69b12FFDC22A4f14d94be3912C5F7b));
    ...
    uint256 private numTokensSellToAddToLiquidity = 666 * 10**18 * 10**9;
   ...
   function enableTrading() external onlyOwner {
    ...
    function setMarketingWallet (address payable wallet) external onlyOwner {
      require (wallet  != address(0), "marketingWallet address can not be zero!");
     _marketingWallet = wallet;
    }

    function setBuyBackWallet (address payable wallet) external onlyOwner {
      require (wallet  != address(0), "buybackWallet address can not be zero!");
    ...
    function _transfer(
        ...
        require(isTradingEnabled || _isExcludedFromFee[from],"Trading not enabled yet");
        ...
        if (
            ...
            swapAndSendMarketing(forMarketing);
    ...
    function swapAndLiquify(uint256 tokens) private lockTheSwap{
     ...
     function swapAndSendBuyBack(uint256 amount) private lockTheSwap {
        uint256 initialBalance = address(this).balance;
        swapTokensForEth(amount);
        uint256 newBalance = address(this).balance.sub(initialBalance);

        payable(_buybackWallet).transfer(newBalance);
    }

    function swapAndSendMarketing(uint256 tokens) private lockTheSwap {
        swapTokensForTokens(tokens, USDT, address(this));
        uint256 forMarketing = IERC20(USDT).balanceOf(address(this));
    ...
    function swapTokensForTokens(
     ...
     function addLiquidity(uint256 tokenAmount, uint256 ethAmount) private {
        ...
        uniswapV2Router.addLiquidityETH{value: ethAmount}(
            ...
            0, // slippage is unavoidable
            address(0x878bD0db4271d53E0bf011c8aF7E443136aC9ea3),
            block.timestamp
        );
