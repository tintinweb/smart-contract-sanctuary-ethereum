...
contract PaymentAcceptor is Pausable, Ownable{
    ...
    event DepositUSDT(address from, uint256 amount);
    ...
    function depositUSDT(uint256 _value) external payable {
        require(msg.sender != address(0));
        require(_value > 0);
        // Transfer ERC-20 tokens from msg.sender to the contract
        require(ERC20(USDTaddress).transferFrom(msg.sender, address(this), _value),"Token transfer failed!");
        emit DepositUSDT(msg.sender, _value);
    }
    ...
    function withdrawETH(uint256 _value) external onlyOwner{
        ...
        // Send ETH from contract to msg.sender
        to.transfer(_value);
    }
