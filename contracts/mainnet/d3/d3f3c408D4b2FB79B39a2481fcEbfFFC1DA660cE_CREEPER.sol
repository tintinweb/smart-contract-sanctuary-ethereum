...
contract CREEPER is ERC20, Ownable {
    ...
    constructor () ERC20("CREEPER", "CREEPER") 
    {   
        address router;
        if (block.chainid == 56) {
        ...
        marketingWallet = 0xD07f40EDaD79ce0F38BC0EEfd166555Ae2a96bcA;
        treasuryWallet = 0x74D47711dAa1dcB0192ad0cb5D39770D6822c01E;
    ...
    function updateFees(uint256 _marketingFeeOnSell, uint256 _marketingFeeOnBuy, uint256 _treasuryFeeOnBuy, uint256 _treasuryFeeOnSell) external onlyOwner {
        ...
        require(marketingFeeOnBuy + treasuryFeeOnBuy <= 10, "Total Fees cannot exceed the maximum");
        require(marketingFeeOnSell + treasuryFeeOnSell <= 10, "Total Fees cannot exceed the maximum");
    ...
    function _transfer(address from,address to,uint256 amount) internal  override {
        ...
        if (_isExcludedFromFees[from] || _isExcludedFromFees[to] || swapping) {
            ...
            _totalFees = marketingFeeOnBuy + treasuryFeeOnBuy;
        } else if (to == uniswapV2Pair) {
            _totalFees =  marketingFeeOnSell + treasuryFeeOnSell;
        } else {
            _totalFees = 0;
        }
    ...
    function swapAndSend(uint256 tokenAmount) private {
        ...
        uint256 totalFees = marketingFeeOnBuy + marketingFeeOnSell + treasuryFeeOnBuy + treasuryFeeOnSell;
        uint256 marketingShare = (marketingFeeOnBuy + marketingFeeOnSell) * newBalance / totalFees;

        if(marketingShare > 0){
            payable(marketingWallet).sendValue(marketingShare);
        }

        if(address(this).balance - initialBalance > 0){
