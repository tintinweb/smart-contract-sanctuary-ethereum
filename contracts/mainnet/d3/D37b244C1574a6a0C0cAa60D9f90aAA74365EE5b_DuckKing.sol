...
contract DuckKing is ERC721A, Ownable, ReentrancyGuard {
  ...
  //ipfs://QmaRvza1XBZKxXRi6kimpQa2YAZT6AcrMBFnzLSnrwhwwY
    ...
    ) ERC721A("DuckKing", "DuckKing", _maxBatchSize, _collectionSize) {
        baseURI = initBaseURI;
    ...
    function reserve(address recipient, uint256 amount) external onlyOwner {
        require(recipient != address(0), "DuckKing: zero address");
        require(amount > 0, "DuckKing: invalid amount");
        require(
            totalSupply() + amount <= collectionSize,
            "DuckKing: max supply exceeded"
        );
        require(
            tokensReserved + amount <= reserveAmount,
            "DuckKing: max reserve amount exceeded"
        );
        require(
            amount % maxBatchSize == 0,
            "DuckKing: can only mint a multiple of the maxBatchSize"
        );
    ...
    function mint(uint256 amount) external payable {
        require(status == Status.PublicSale, "DuckKing: Public sale is not active.");
        require(
            tx.origin == msg.sender,
            "DuckKing: contract is not allowed to mint."
        );
        require(
    ...
    function refundIfOver(uint256 price) private {
        require(msg.value >= price, "DuckKing: Need to send more ETH.");
        if (msg.value > price) {
