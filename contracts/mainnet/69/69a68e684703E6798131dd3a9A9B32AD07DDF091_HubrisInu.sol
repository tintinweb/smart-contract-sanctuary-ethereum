...
https://twitter.com/elonmusk/status/1530694232251432961

Elon had a car named Hubris.

Here is the deal;
- There won't be any tg created for this token, since the main goal is to gift this project to the community
- Initial lock will be 3 days, fully extending it at;
   -20k for 7 days
   -50k for 14 days
   -150k for a month
- No team tokens, no dev wallet, all straight into liquidity!
- In case of clogg, we will unclogg before locking and renouncing
- Taxes, 1% in (Dev), 3% out(Liquidity), for teh community!
- 2% max buy ( 30 sec cooldown on buys )
- Once we lock the liquidity, we are going to lift the limits and renounce the contract for your own safety ( people like that )
   **once we lift the limits, we will be ready to renounce, but we dont
     want to lift them super fast, because we want good wallet distro.

     This token will launch at 13.30pm UTC
*/
...
contract HubrisInu is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    constructor() ERC20("Hubris Inu", "HUNU") {
    ...
    ; uint256 totalSupply = 1 * 1e12 * 1e18;
        ...
        maxWallet = totalSupply * 1000 / 1000; // 4% maxWallet
