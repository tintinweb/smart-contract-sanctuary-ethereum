...
contract newFarm is Ownable {
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of BRDG
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint; // How many allocation points assigned to this pool. BRDG to distribute per block.
        uint256 lastRewardBlock; // Last block number that BRDG distribution occurs.
        uint256 accBridgePerShare; // Accumulated BRDG per share, times 1e12. See below.
    }
    // The BRDG TOKEN!
    IERC20 public Bridge;
    // Block number when bonus BRDG period ends.
    uint256 public bonusEndBlock;
    // BRDG tokens created per block.
    uint256 public BridgePerBlock;
    // Bonus multiplier for early Bridge makers.
    ...
    // The block number when BRDG mining starts.
    ...
    constructor(
        ...
        require(address(_Bridge) != address(0x0), "Bridge: _Bridge cannot be 0x0");
        Bridge = _Bridge;
    ...
    function add(uint256 _allocPoint, IERC20 _lpToken) public onlyOwner {
        massUpdatePools();
        require(address(_lpToken) != address(0x0), "Bridge: lpToken cant be 0x0");
        require(addedLPTokens[address(_lpToken)] == false, "Bridge: _lpToken already exists");
        addedLPTokens[address(_lpToken)] = true;
        uint256 lastRewardBlock = block.number > startBlock ? block.number : startBlock;
        totalAllocPoint = totalAllocPoint.add(_allocPoint);
        poolInfo.push(PoolInfo({lpToken: _lpToken, allocPoint: _allocPoint, lastRewardBlock: lastRewardBlock, accBridgePerShare: 0}));
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        updatePool(_pid);
        if (user.amount > 0) {
            uint256 _pending = user.amount.mul(pool.accBridgePerShare).div(1e12).sub(user.rewardDebt);
            safeBridgeTransfer(msg.sender, _pending);
        }
    ...
    /// @notice Safe transfer function, just in case if rounding error causes pool to not have enough BRDG.
    ...
    function safeBridgeTransfer(address _to, uint256 _amount) internal {
        ...
        if (_amount > BridgeBal) {
            IERC20(Bridge).safeTransfer(_to, BridgeBal);
        } else {
            IERC20(Bridge).safeTransfer(_to, _amount);
        }
    }

    function setBridgePerBlock(uint256 _BridgePerBlock) external onlyOwner {
        massUpdatePools();
        BridgePerBlock = _BridgePerBlock;
    }
