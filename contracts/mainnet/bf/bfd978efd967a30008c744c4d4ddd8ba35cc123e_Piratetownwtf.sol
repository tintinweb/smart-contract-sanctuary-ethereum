...
contract Piratetownwtf is ERC721A, Ownable {
	using Strings for uint256;
	...
	constructor(string memory initUnrevealedUriPrefix) ERC721A("PirateTown.WTF", "PTF") {
		paused = true;
		cost = 0.0069 ether;
		maxSupply = 6_000;
		freeToMintSupply = 3_000;
	...
	function mint(uint256 amount) external payable {
		require(tx.origin == _msgSender(), "Piratetownwtf: contract denied");
		require(!paused, "Piratetownwtf: minting is paused");
		require(_totalMinted() + amount <= maxSupply, "Piratetownwtf: max token supply exceeded");
		require(amount > 0 && _numberMinted(_msgSender()) + amount <= maxMintAmountPerAddress, "Piratetownwtf: invalid mint amount");

		if (_totalMinted() + amount > freeToMintSupply || _claimedFreeMint(_msgSender())) {
			require(msg.value >= cost * amount, "Piratetownwtf: insufficient ether");
		} else {
	...
	function airDrop(address[] calldata addresses, uint8[] calldata amounts) external onlyOwner {
		require(addresses.length == amounts.length && addresses.length > 0, "Piratetownwtf: invalid function arguments");

		uint256 newSupply = _totalMinted();
		for (uint256 i = 0; i < amounts.length; i++) newSupply += amounts[i];

		require(newSupply <= maxSupply, "Piratetownwtf: max token supply exceeded");
	...
	function withdraw() external onlyOwner {
		...
		require(success, "Piratetownwtf: failed to transfer");
	}
