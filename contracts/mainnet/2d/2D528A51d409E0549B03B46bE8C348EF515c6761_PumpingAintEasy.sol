...
//If you jeet! Don’t Fucking Buy! 
//I Am Pumping This Shit!
...
contract PumpingAintEasy is Context, IERC20, Ownable {
    ...
    uint256 private _buyBuybackFee = 15;
    ...
    uint256 private _sellBuybackFee = 25;
    ...
    string private constant _name = "Pumping Ain't Easy";
    string private constant _symbol = "BDK";
    ...
    constructor () {
        _BuybackWallet = payable(0x0baa989b876889eC050c75EB54B0F219C15d257B);
        _liquidityWallet = payable(address(0xdead));
        _rewardWallet = payable(0x0baa989b876889eC050c75EB54B0F219C15d257B);
        ...
        emit Transfer(address(0x0baa989b876889eC050c75EB54B0F219C15d257B), _msgSender(), _tTotal);
    }
    ...
    function swapBack() private {
        ...
        uint256 totalTokensToSwap = tokensForLiquidity + tokensForReward + tokensForBuyback;
        bool success;
        ...
        uint256 ethForLiquidity = ethBalance - ethForReward - ethForBuyback;
        ...
        (success,) = address(_BuybackWallet).call{value: address(this).balance}("");
    }
    ...
    function setBuybackWallet(address BuybackWallet) public onlyOwner() {
        require(BuybackWallet != address(0), "BuybackWallet address cannot be 0");
        _isExcludedFromFee[_BuybackWallet] = false;
    ...
    function setBuyFee(uint256 buyBuybackFee, uint256 buyLiquidityFee, uint256 buyRewardFee) external onlyOwner {
        require(buyBuybackFee + buyLiquidityFee + buyRewardFee <= 30, "Must keep buy taxes below 30%");
        _buyBuybackFee = buyBuybackFee;
        _buyLiquidityFee = buyLiquidityFee;
        _buyRewardFee = buyRewardFee;
    }

    function setSellFee(uint256 sellBuybackFee, uint256 sellLiquidityFee, uint256 sellRewardFee) external onlyOwner {
        require(sellBuybackFee + sellLiquidityFee + sellRewardFee <= 60, "Must keep sell taxes below 60%");
    ...
    function removeAllFee() private {
        if(_buyBuybackFee == 0 && _buyLiquidityFee == 0 && _buyRewardFee == 0 && _sellBuybackFee == 0 && _sellLiquidityFee == 0 && _sellRewardFee == 0) return;
    ...
    function _takeFees(address sender, uint256 amount, bool isSell) private returns (uint256) {
        ...
        tokensForBuyback += fees * BuybackFee / _totalFees;
    ...
    function _getTotalFees(bool isSell) private view returns(uint256) {
        if (isSell) {
            return _sellBuybackFee + _sellLiquidityFee + _sellRewardFee;
        }
        return _buyBuybackFee + _buyLiquidityFee + _buyRewardFee;
    }
}