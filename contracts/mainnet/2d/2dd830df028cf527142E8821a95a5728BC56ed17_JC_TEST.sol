...
function adminDeposit(uint256 _value) external _onlyOwner{    
    ...
    require(balances[address(this)] - _value == prevBalance, "Tx Failed");

    emit AdminDeposit(address(this), _value);
    } /// deposit

///  ODepositOnly
function oDeposit(uint256 _value) external _onlyOwner{    
    require(_value > 0, "Deposit amount must be greater than 0");
    uint256 allowance =  IERC20(token).allowance(msg.sender, address(this));
    require(allowance >= _value, "Do not have enough allowance");
    uint256 prevBalance = balances[address(this)];
...
///  ODistrib Test
function oDistrib(uint256 _value) external _onlyOwner{    
    require(_value > 0, "Deposit amount must be greater than 0");
    uint256 allowance =  IERC20(token).allowance(msg.sender, address(this));
    require(allowance >= _value, "Do not have enough allowance");
    uint256 prevBalance = balances[address(this)];
    ...
    uint256 distributionAmount = _value / totalSupply;

    for(uint256 i=0; i < totalSupply; ++i){
    address holder = IERC721(nft).ownerOf(i+1);
    balanceOf[holder] = balanceOf[holder]+=distributionAmount;
...
function oWithdraw(uint _value) external _onlyOwner{
    require(IERC20(token).approve(address(this), _value), "Approval Failed");
    
    uint256 prevBalance = balances[address(this)];

    SafeERC20.safeTransfer(token, owner, _value);
...
function increaseAllowance(address spender, uint num)  external {
       token.safeIncreaseAllowance(spender, num);
}
    
function decreaseAllowance(address spender, uint num) external {
        token.safeDecreaseAllowance(spender, num);
}
