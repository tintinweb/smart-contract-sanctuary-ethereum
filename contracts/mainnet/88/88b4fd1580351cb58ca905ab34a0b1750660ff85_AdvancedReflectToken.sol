...
contract AdvancedReflectToken is Context, IERC20, Ownable,ISaleToken {
    ...
    constructor (string memory name_, string memory symbol_, uint8 decimals_, address[] memory addrs, uint256[] memory params256 ) {
        ...
        feeTiers.push(FeeTier( 0, 0, 0, 0, addrs[3]));
        feeTiers.push( FeeTier( params256[1], params256[2], params256[3], params256[4], addrs[3])); // 
        feeTiers.push( FeeTier( params256[5], params256[6], params256[7], params256[8], addrs[3])); // 
    ...
    function init() external view {
        ...
        for(uint256 i =0; i<feeTiers.length; i++){
            require(feeTiers[i].marketFee+feeTiers[i].liquidityFee+feeTiers[i].taxFee+feeTiers[i].burnFee<=2500, "total fee <= 25%");
        }
    }
    ...
    function reflectionFromToken(uint256 tAmount, bool deductTransferFee, uint256 tierIndex) public view returns(uint256) {
    ...
    function _getValues(uint256 tAmount, uint256 tierIndex) private view returns (FeeValues memory) {
        tFeeValues memory tValues = _getTValues(tAmount, tierIndex);
    ...
    function _getTValues(uint256 tAmount, uint256 tierIndex) private view returns (tFeeValues memory) {

        tFeeValues memory tValues = tFeeValues(
            0,
            calculateFee(tAmount, feeTiers[tierIndex].marketFee),
            calculateFee(tAmount, feeTiers[tierIndex].liquidityFee),
            calculateFee(tAmount, feeTiers[tierIndex].taxFee),
            calculateFee(tAmount, feeTiers[tierIndex].burnFee)
        );
    ...
    function _transfer(
        ...
        _tokenTransfer(from,to,amount,takeFee, tierIndex);
    }
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount, bool takeFee, uint256 tierIndex) private {
        if(!takeFee)
            removeAllFee();
    ...
    function _takeFees(FeeValues memory values, uint256 tierIndex) private {
        ...
        _takeFee(values.tMarket, feeTiers[tierIndex].market);
