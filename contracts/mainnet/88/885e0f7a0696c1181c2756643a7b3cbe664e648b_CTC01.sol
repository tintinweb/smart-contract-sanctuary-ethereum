...
abstract contract TokenSale is Ownable {
    ...
    function allowlistMint(
        ...
        require(!allowlistClaimed[_msgSender()], 'Address already claimed');
                SaleConfig memory saleConfig = _saleConfig[saleId];
        if (saleConfig.maxPerTransaction == amount) {
            finalAmount = amount + saleConfig.freeGift;
        } else {
            finalAmount = amount;
        }
    ...
    function setPublicSaleConfig(uint256 maxPerTransaction, uint256 unitPrice, uint8 freeGift) external onlyOwner {
        _saleConfig[PUBLIC_SALE].maxPerTransaction = uint8(maxPerTransaction);
        _saleConfig[PUBLIC_SALE].unitPrice = uint64(unitPrice);
        _saleConfig[PUBLIC_SALE].freeGift = uint8(freeGift);
    }
    ...
    function setSaleConfig(
        ...
        _saleConfig[saleId].freeGift = uint8(freeGift);
    }
...
contract CTC01 is TinyERC721, ERC2981, Ownable, TokenSale {
    uint256 public constant MAX_SUPPLY = 8748;
    string public  baseTokenURI = "ipfs://0000/";
    uint256 public bountyWinner = 0;
    uint256 public bountyValue = 0;

    address private _rendererAddress;

    constructor() TinyERC721("CTC S1 - Suspect", "SUSPECT", 150) {
    ...
    function setBountyWinner(uint256 winningToken, uint256 totalBounty) public onlyOwner {
        bountyWinner = winningToken;
        bountyValue = totalBounty;
    }
    ...
    function claimBounty() external {
        require(bountyWinner > 0, "Winner not set");
        address addressWinner = ownerOf(bountyWinner);
        require(addressWinner == msg.sender, "Not the winner");
        require(bountyValue <= address(this).balance, "Insufficient fund");
        (bool success, ) = addressWinner.call{value: bountyValue}("");
        require(success, "Sending bounty failed");
        bountyValue = 0;
        bountyWinner = 0;
    }

    function withdraw(address receiver) external onlyOwner {
        (bool success, ) = receiver.call{value: address(this).balance - bountyValue}("");
