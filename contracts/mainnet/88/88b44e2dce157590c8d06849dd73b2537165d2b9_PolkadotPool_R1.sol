...
contract PolkadotPool_R1 is PausableUpgradeable, ReentrancyGuardUpgradeable, OwnableUpgradeable, ERC165Upgradeable, IStakingPool_R1 {
    ...
    function claimBonds(address bond, uint256 claimId, uint256 amount, uint256 claimBeforeBlock, address account, bytes memory signature) external nonReentrant whenNotPaused override {
        ...
        IAnkrBond_R1(bond).mintSharesTo(account, amount);
    ...
    // todo think about adding indexes
    function burnBond(address bond, uint256 amount, bytes calldata polkadotRecipient) external nonReentrant whenNotPaused override {
        ...
        uint256 shares = IAnkrBond_R1(bond).balanceToShares(amount);
        IAnkrBond_R1(bond).burnSharesFrom(account, shares);
        ...
        */
        emit TokensBurned(bond, account, polkadotRecipient, amount, shares);
    }
    ...
    function registerBond(string memory name, uint8 decimals) public onlyOperator returns (address) {
        bytes memory call = abi.encodeWithSelector(
            IAnkrBond_R1.initialize.selector, _operator, address(this), name, decimals
        );
    ...
    function registerFuture(address forBond, string memory name, uint256 defaultMaturity, uint8 decimals, string memory baseUri) public onlyOperator returns (address) {
        ...
        require(ERC165CheckerUpgradeable.supportsInterface(forBond, type(IAnkrBond_R1).interfaceId), "Bond contract is not correct");
        bytes memory call = abi.encodeWithSelector(
    ...
    function supportsInterface(bytes4 interfaceId) public view virtual override returns (bool) {
        ...
        || interfaceId == type(IStakingPool_R1).interfaceId;
    }
