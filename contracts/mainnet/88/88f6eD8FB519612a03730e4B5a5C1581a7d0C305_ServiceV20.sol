...
contract ServiceV20 {
  ...
  function getEntityNodeCount(address entity) external view returns (uint256) {
  ...
  function migrateAll(address _contract, uint256 _blockNumber, uint256 _totalClaimed, bytes memory signature) external payable {
    bytes32 hash = prefixed(keccak256(abi.encodePacked(msg.sender, _totalClaimed)));
    address signer = recoverSigner(hash, signature);

    require(entityNodeCount[msg.sender] > 0, "0 nodes");
    require(signer == admin || signer == parameterAdmin || signer == superAdmin, "wrong signer");

    uint valueLeft = msg.value;
    uint nodeCount = 0;
    uint totalSeconds = 0;
    uint totalClaimed = _totalClaimed + getRewardAll(msg.sender, _blockNumber);

    for (uint128 nodeId = 1; nodeId <= entityNodeCount[msg.sender]; nodeId++) {
      require(valueLeft > 0, "not enough");
      uint paid = migrate(nodeId, _blockNumber);

      if (paid > 0) {
        nodeCount++;
        valueLeft = valueLeft.sub(paid);
        totalSeconds = totalSeconds.add(block.timestamp
          - ((block.number - entityNodePaidOnBlock[getNodeId(msg.sender, nodeId)]) * secondsPerBlock));
      }
    }

    if (nodeCount == 0) revert("expired");

    entityNodeDeactivatedCount[msg.sender] = entityNodeCount[msg.sender];
    INodePackV3(_contract).migrateNode(msg.sender, 1, nodeCount, totalSeconds / nodeCount, totalClaimed);
    if (valueLeft > 0) sendValue(payable(msg.sender), valueLeft);
  }

  function migrate(uint128 _nodeId, uint256 _blockNumber) internal returns (uint256) {
    address sender = msg.sender == address(this) ? tx.origin : msg.sender;
    bytes memory id = getNodeId(sender, _nodeId);

    uint256 paid = 0;
    if (!hasNodeExpired(sender, _nodeId) && !isNodeBYON(sender, _nodeId)) {
      uint256 fee = getRewardByBlock(sender, _nodeId, _blockNumber).mul(claimingFeeNumerator).div(claimingFeeDenominator);
      paid = this.claim{value : fee}(_nodeId, _blockNumber, false, 0, "0x0");
...
interface INodePackV3 {
  ...
  function migrateNode(address _entity, uint _nodeType, uint _nodeCount, uint _lastPaidAt, uint _totalClaimed) external returns (bool);
}
