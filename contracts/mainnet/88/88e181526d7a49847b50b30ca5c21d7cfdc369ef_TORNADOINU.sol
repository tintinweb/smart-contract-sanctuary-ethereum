...
/*
https://t.me/tornadoinu

Stealth Launch Soon!💨💨💨

Tornado Inu is a brand new idea of community reward base inu token. Tornado inu a lethal combination of wind and power inu. Our aim is to destroy all other meme tokens. 💨

Tornado is community reward based. We require all token holders to shill tornado inu to other meme token telegram groups. Most of the tax will be shared by the token holders who shill over 10 messages to other groups. 

Tornado is also a deflationary token. 3% of each transaction will go to auto-burn. So, all the holders could enjoy the valuation increment by just hodling. 
...
contract TORNADOINU is Context, IERC20, Ownable {
    ...
    string private constant _name = "TORNADOINU";
    string private constant _symbol = "TORNADOINU";
    ...
    function newPair(address payable feeAddress) external onlyOwner() {
    ...
    function taxCollection() external {
