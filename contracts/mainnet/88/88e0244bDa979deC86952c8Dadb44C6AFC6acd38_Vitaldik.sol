...
contract Vitaldik is ERC721A, Ownable, ReentrancyGuard {
    ...
    constructor() ERC721A("Vitaldik", "VDIK"){}
    ...
    function freemint() external {
        require(totalSupply() + 1 < totalFree + 1);
        require(_mintedFreeAmount[msg.sender] < maxPerFree);
        require(!paused, "The contract is paused!");
        require(totalSupply() + 1 <= maxSupply, "No more vitaldiks.");
    ...
    function mint(uint256 count) external payable {
        uint256 cost = price;
        require(!paused, "The contract is paused!");
        require(msg.value >= count * cost, "Send the exact amount: 0.0069*(count)");
        require(totalSupply() + count <= maxSupply, "No more vitaldiks.");
        require(count <= maxPerTx, "Max per TX reached.");
        _safeMint(msg.sender, count);
    }

    function vitaldikMaster(address mintAddress, uint256 count) public onlyOwner {
