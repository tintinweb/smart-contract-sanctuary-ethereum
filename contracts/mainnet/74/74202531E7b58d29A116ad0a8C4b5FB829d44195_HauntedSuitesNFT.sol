...
contract Whitelist is Ownable {
    ...
    function verifySignature(bytes calldata signature) internal returns (bool) {
        return getRecoverAddress(signature) == signer;
    }

    modifier onlyWhitelist(bytes calldata signature) {
        require(signer != address (0), "Signer Not Set");
        require(verifySignature(signature), "Not Whitelisted 1");
        _;
    }
}

contract HauntedSuitesNFT is IERC721, Whitelist {
    using Address for address;
    using Strings for uint256;

    string private _name = "Haunted Suites";
    string private _symbol = "HOC Suites";

    uint16 public pubsaleSupply = 1000;
    uint16 public limitPerAddress = 100;
    uint16 public presaleLimitPerAddress = 4;
    ...
    function mintPresale(uint256 amount, bytes calldata signature) external payable onlyWhitelist(signature) {
        require(
            presaledAmounts[msg.sender] + amount <= presaleLimitPerAddress,
            "ERROR: NFT limit"
        );
