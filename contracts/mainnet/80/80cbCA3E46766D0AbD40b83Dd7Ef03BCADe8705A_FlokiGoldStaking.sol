...
contract FlokiGoldStaking is Ownable, ReentrancyGuard {
        ...
        updateReward(msg.sender)
    {
        ...
        IERC20(flokiToken).safeTransferFrom(msg.sender, address(this), _amount);
  ...
  **/
        ...
        updateReward(msg.sender)
    {
        ...
        if (isEmergency) {
            ...
            IERC20(flokiToken).safeTransfer(msg.sender, outAmount);
    ...
    function claim() external nonReentrant {
        ...
        require(
            IERC20(flokiToken).balanceOf(address(this)) >= receiveAmount,
            "staking contract has not enough MARS token"
        );

        IERC20(flokiToken).safeTransfer(msg.sender, receiveAmount);
        totalStakedAmount = IERC20(flokiToken).balanceOf(address(this)).sub(
            receiveAmount
        );
