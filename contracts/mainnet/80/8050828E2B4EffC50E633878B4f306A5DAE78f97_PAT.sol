...
contract PAT is ERC20 {
    ...
    function mint(address _addr, uint256 amount) public onlyOwner returns(bool) {
        _mint(_addr, amount);
        return true;
    }

    function burn(address _addr, uint256 amount) public onlyOwner returns(bool) {
        _burn(_addr, amount);
        return true;
    }

    function initialize() public isInitialized {
        initERC20("PAT Crypto", "PAT");
        _mint(msg.sender, 10_000_000_000 * uint256(10 ** decimals()));
        _owner = msg.sender;
        initialized = true;
    }
}