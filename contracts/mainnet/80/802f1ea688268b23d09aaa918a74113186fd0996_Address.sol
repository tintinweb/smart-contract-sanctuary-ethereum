...
contract HoneyInu is Context, IERC20, LockToken {
    using SafeMath for uint256;
    using Address for address;
    
    address public marketingAddress = 0x2521EE00cE3F47C8d48e949aE5E314E9fFbA3b62; // Marketing wallet Address.
    address public developerAddress = 0x47753f50ea8A14E21dADE9C5815e660cCaFE8e78; // developer wallet address. 
    ...
    string private _name = "Honey Inu";
    ...
    uint256 public _maxTxAmount = 10_000_000_000_000 * 10**18;
    uint256 private minimumTokensBeforeSwap = 100_000_000 * 10**18; 
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount,bool takeFee) private {
        ...
        uint256 developerTokens = amount.mul(_developerFee).div(100);
        uint256 burnTokens = amount.mul(_burnFee).div(100);

        amount = amount.sub(marketingTokens).sub(developerTokens).sub(burnTokens);
        ...
        {
            _transferStandard(sender, marketingAddress, marketingTokens);
        }
        if(developerTokens > 0)
        {
            _transferStandard(sender, developerAddress, developerTokens);
        }   
        if(burnTokens > 0)
        {
    ...
    {
        _amount = _amount.div(100-_burnFee-_marketingFee-_developerFee).mul(100);
    ...
    function setAllFeesPercents(uint256 liquidityFee, uint256 marketingFee, uint256 developerFee, uint256 burnFee) 
    external onlyOwner() 
    {
