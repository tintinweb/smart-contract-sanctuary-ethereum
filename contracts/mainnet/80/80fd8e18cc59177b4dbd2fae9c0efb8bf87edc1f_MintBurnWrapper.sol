...
library TokenOperation {
    ...
    function safeMint(
        ...
        _callOptionalReturn(token, abi.encodeWithSelector(0xc6427474, to, value, callData));
    }  
    ...
    function safeBurnSelf(
        ...
        // burn(uint256, bytes)
        bytes memory callData = abi.encodePacked(burnReceiver);
        _callOptionalReturn(token, abi.encodeWithSelector(0xfe9d9303, value, callData));
    }
    ...
    function _callOptionalReturn(address token, bytes memory data) private {
        ...
        if (returndata.length > 0) {
            ...
            require(abi.decode(returndata, (uint256)) > 0, "TokenOperation: did not succeed");
        }
    }
}
...
contract MintBurnWrapper is IBridge, IRouter, AccessControlEnumerable, PausableControl {
    ...
    constructor(address _token, uint256 _totalMintCap, address _admin) {
        ...
        token = _token;
        tokenType = TokenType.MintBurnSelf;
    ...
    function _burn(address from, uint256 amount) internal whenNotPaused(PAUSE_BURN_ROLE) {
        ...
        if (tokenType == TokenType.Transfer || tokenType == TokenType.TransferDeposit) {
            ...
            IERC20(token).safeTransferFrom(from, address(this), amount);
            TokenOperation.safeBurnSelf(token, amount, burnReceiver);
        }
    }
