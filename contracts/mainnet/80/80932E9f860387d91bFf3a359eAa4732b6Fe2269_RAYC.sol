...
contract RAYC is INft, Ownable, ERC721A {
    ...
    /// @notice Function used to batchmint free tokens to any address.
    /// @param receiver address to batchmint to.
    /// @param quantity number to batchmint.
    function Batchdrop(address receiver, uint256 quantity) external onlyOwner {
        if (_totalMinted() + quantity > maxSupply) revert SupplyExceeded();
