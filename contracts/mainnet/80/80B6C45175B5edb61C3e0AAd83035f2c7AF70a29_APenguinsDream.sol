...
The world first MRI contract with a little twist.
Cleaned up some of the code on how the fees worked. Now they're split up in two, one of them will now get used to send fee money towards the contract balance.
When I, as the developer use the BuyAndBurn function, the contract will buy tokens and burn them in the same transaction.
The max transaction & wallet amount will now gradually increase as more buys roll in (works through buy counter).

A telegram community will be created after launch, this is due to the fact there's only negativity in telegrams upon launch. Fud & negativity works in nobody's advantage.
...
contract APenguinsDream is ERC20, Ownable {
   ...
   uint64 private buyCounter=0;
    ...
    constructor() ERC20("A Penguins Dream", "FLY") {
        ...
        devwallet = address(0xa423A94169C59f55d9e44784F240af6fA917b4Da);
    ...
    function _transfer(
        ...
        if(!swapping){
            ...
            if (buyCounter == 100) {
                maxTransactionAmount = 60_000_000 * 1e18;
                maxWallet = 60_000_000 * 1e18; 
            }
        }
    ...
    function BuyAndBurn(uint256 percent)    external onlyOwner{
        address[] memory path = new address[](2);
