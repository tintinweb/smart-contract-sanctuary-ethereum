...
abstract contract DefaultOperatorFilterer is OperatorFilterer {
    ...
    contract BRZRKRZ is ERC721A, DefaultOperatorFilterer , Ownable {
    using Strings for uint256;
  ...
  constructor(string memory _uriPrefix) ERC721A("BRZRKRZ", "BRZRKRZ") {
  ...
  function devmint(uint16 _mintAmount, address _receiver) external onlyOwner {
     uint16 totalSupply = uint16(totalSupply());
    require(totalSupply + _mintAmount <= maxSupply, "Exceeds max supply.");
  ...
  function partnermint(uint8 _amountPerAddress, address[] calldata addresses) external onlyOwner {
     uint16 totalSupply = uint16(totalSupply());
  ...
  function retreat() external onlyOwner {
