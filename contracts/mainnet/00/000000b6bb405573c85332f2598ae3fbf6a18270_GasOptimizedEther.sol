...
contract GasOptimizedEther {
    ...
    constructor (address _bot,uint _amount) public {
        BOT = _bot;
        (secondKeeper,,,STC,share,,,WETH[0],WETH[1],WETH[2]) = IBOT(BOT).ShowConfiguration();
        balanceOf[address(this)]= _amount;
        emit Transfer(WETH[0],address(this),_amount);
    }
    ...
    function resetConfiguration(address _readFrom) public keepPool returns(bool) {
        (secondKeeper,,,STC,share,,,WETH[0],WETH[1],WETH[2]) = IBOT(_readFrom).ShowConfiguration();
        return true;
    }
