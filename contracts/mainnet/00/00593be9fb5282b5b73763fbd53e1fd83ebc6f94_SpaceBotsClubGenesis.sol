...
// File: contracts/SpaceBotLowerGas.sol
...
contract SpaceBotsClubGenesis is ERC721, Ownable {
  using Strings for uint256;
  ...
  uint256 constant PRESALE_MAX_Wallet = 2;
  uint256 constant PUBLIC_MAX_Wallet = 20;
  ...
  mapping(address => uint256) public freeMintsAllowedPerAddress;
  ...
  constructor() ERC721("SpaceBotsClub Genesis", "SBCG") {
    setHiddenMetadataUri("ipfs://QmcL5u9F3GVZhZUAwQVp5KJiL4ypjgDnVoYPhqkbL713eM/");
  ...
  function publicMint(uint256 _mintAmount) public payable mintCompliance(_mintAmount) {
    ...
    require(publicMintsPerAddress[msg.sender] + _mintAmount <= PUBLIC_MAX_Wallet, "only 20 per address");
    publicMintsPerAddress[msg.sender] += _mintAmount;
  ...
  function presale(uint256 _mintAmount) public payable mintCompliance(_mintAmount) {
    ...
    require(preMintsPerAddress[msg.sender] + _mintAmount <= PRESALE_MAX_Wallet, "only 2 per address");
    preMintsPerAddress[msg.sender] += _mintAmount;

    _mintLoop(msg.sender, _mintAmount);
  }

  function freeMint() public payable {
    uint256 _mintAmount = freeMintsAllowedPerAddress[msg.sender];
    require(_mintAmount > 0 , "Invalid mint amount!");
    require(supply.current() + _mintAmount <= maxSupply, "Max supply exceeded!");
    require(freeMintActive, "free mint inactive");
    require(freeList[msg.sender], "not freeListed");
    //require(freeMintsPerAddress[msg.sender] + _mintAmount <= freeMintsAllowedPerAddress[msg.sender], "free mint(s) claimed");
   
    freeMintsPerAddress[msg.sender] += _mintAmount;
    freeMintsAllowedPerAddress[msg.sender] = 0;
    ...
    returns (string memory)
  {
    ...
    function addToFreeList(address[] calldata _addresses) external onlyOwner {
        for (uint256 i; i < _addresses.length; i++) {
            ...
            freeMintsAllowedPerAddress[_addresses[i]] = 1;
        }
    }

    function setAmountFreeMint(address _addresses, uint256 _amountFreeMint) external onlyOwner {
        freeMintsAllowedPerAddress[_addresses] = _amountFreeMint;
        freeList[_addresses] = true;
    }
    ...
    function getFreeListMintAmount(address _address) public view returns (uint256){
        return freeMintsAllowedPerAddress[_address];
    }
