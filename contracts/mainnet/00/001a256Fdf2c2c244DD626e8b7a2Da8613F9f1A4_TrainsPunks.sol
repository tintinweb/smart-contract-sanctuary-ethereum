...
import "./ITrainsPunks.sol";

contract TrainsPunks is
    ITrainsPunks,
    ERC721AQueryable,
    PaymentSplitter,
    Ownable,
    ...
    DefaultOperatorFilterer
{
    ...
    ) ERC721A("Trainspunks", "$TRAIN") PaymentSplitter(_payees, _shares) {}
  ...
  //////////////////////////////////////////////////////////////*/
    ...
    function whitelistMint(
        ...
        uint256 _nonFreeAmount = _calculateNonFreeAmount(
            msg.sender,
            _amount,
            mintRules.whitelistFreePerWallet
        );

        if (
            _nonFreeAmount != 0 &&
            msg.value < mintRules.whitelistPrice * _nonFreeAmount
        ) {
        ...
        if (
            ...
            mintRules.whitelistMaxPerWallet
        ) {
    ...
    function mint(uint256 _amount) external payable {
        uint256 _nonFreeAmount = _calculateNonFreeAmount(
            msg.sender,
            _amount,
            mintRules.freePerWallet
        );

        if (
            _nonFreeAmount != 0 && msg.value < mintRules.price * _nonFreeAmount
        ) {
            revert InvalidEtherValue();
        }
        ...
        if (_totalMinted() + _amount > mintRules.publicSupply) {
            revert TotalSupplyOverflow();
        }
...
interface ITrainsPunks is IERC721AQueryable {
