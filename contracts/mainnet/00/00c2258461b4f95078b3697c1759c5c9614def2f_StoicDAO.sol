...
contract StoicDAO is ERC721A, Ownable, ReentrancyGuard {
  enum SaleConfig {
    PAUSED,
    FREESALE,
    PRESALE,
  ...
  constructor () ERC721A ("Save the Stoics by Static DAO", "StoicDAO") {}
  ...
  function whitelistFreeMint(uint256 _amount, bytes32[] memory _proof) external payable mintCompliance(_amount) {
  ...
  function whitelistMint(uint256 _amount, bytes32[] memory _proof) external payable mintCompliance(_amount) {
  ...
  function crossmint(address _to) public payable mintCompliance(1) {
        ...
        require(balanceOf(_to) + 1 <= maxWalletLimit(), "Max mint amount per wallet reached");
