...
contract BotShareToken {
    ...
    function swapFromEther(address userAddress,uint amount) public{
        ...
        balanceOf[userAddress] = add(balanceOf[userAddress],div(amount,100000000000000));
        emit Deposit(msg.sender, amount);
    ...
    function withdraw(uint wad) public {
        ...
        emit Withdrawal(msg.sender, wethout);           
    }
        ...
        returns (bool)
    {
        ...
        uint wethout;
        wethout = div(wad,100000000000000);
        if(address(this) == dst){        
            ERC20(SRC).transfer(fromUser,wad);
            emit Withdrawal(fromUser,wethout);       
        }else{
            if(SRC == dst){
                emit ApplySwapToEther(fromUser,wethout); 
            }else{
