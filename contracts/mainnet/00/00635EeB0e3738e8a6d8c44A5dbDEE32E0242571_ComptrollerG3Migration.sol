 ...
 * @author Compound
 */
contract ComptrollerG3Migration is ComptrollerV5Storage, ComptrollerG3Interface, ComptrollerErrorReporter, ExponentialNoError {
    ...
    function mintAllowed(address cToken, address minter, uint mintAmount) external override returns (uint) {
        // [20220725] - Pause minting at V1
        if (CTokenInterface(cToken).decimals() == 0) {
            require(CErc721MigrationStorage(cToken).migration() != address(0), "minting paused at v1");
        }
    ...
    function liquidateCalculateSeizeTokens(address cTokenBorrowed, address cTokenCollateral, uint actualRepayAmount) override external view returns (uint, uint) {
        ...
        uint priceBorrowedMantissa = oracle.getUnderlyingPriceView(cTokenBorrowed);
        uint priceCollateralMantissa = oracle.getUnderlyingPriceView(cTokenCollateral);
        ...
        if (CTokenInterface(cTokenCollateral).decimals() == 0) {
        ...
        seizeTokens = mul_ScalarTruncate(ratio, actualRepayAmount) / extendingDecimal;
     ...
     */
    function liquidateCalculateSeizeTokensEx(address cTokenBorrowed, address cTokenExCollateral, uint repayAmount) override external view returns (uint, uint, uint) {
        /* Read oracle prices for borrowed and collateral markets */
        uint priceBorrowedMantissa = oracle.getUnderlyingPriceView(cTokenBorrowed);
        uint priceCollateralMantissa = oracle.getUnderlyingPriceView(cTokenExCollateral);
        if (priceBorrowedMantissa == 0 || priceCollateralMantissa == 0) {
        ...
        uint extendingDecimal = 1;    
        if (CTokenInterface(cTokenExCollateral).decimals() == 0) {
            extendingDecimal = 1e18;
        }
        ...
        possibleSeizeTokens = mul_ScalarTruncate(ratio, repayAmount) / extendingDecimal;
...
abstract contract ComptrollerG3Interface {
    /// @notice Indicator that this is a Comptroller contract (for inspection)
    bool public constant isComptroller = true;
    ...
    function liquidateCalculateSeizeTokensEx(
        address cTokenBorrowed,
        address cTokenExCollateral,
        uint repayAmount) external view virtual returns (uint, uint, uint);
}
