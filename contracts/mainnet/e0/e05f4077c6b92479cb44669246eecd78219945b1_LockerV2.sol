 ...
 * Legacy LOCKER Holders to receive 100% Tokens at TGE with an imposed 0.5 ETH daily sell limit.
 *
...
contract LockerV2 is ERC20, Ownable {
    ...
    bool public oldHolderSellLimitsEnabled = true;
    uint256 public oldHolderCooldown = 1 minutes;
    uint256 public oldHolderTimeBetweenBuys = 24 hours;

    mapping(address => bool) public oldHolderWallets;
    mapping(address => uint256) public nextOldHolderSellDate;
    uint256 public maxOldHolderSell = .5 ether;
    ...
    event UpdatedOldHolderMaxSell(uint256 amount);
    ...
    function flagV1Holder(address wallet) external onlyOwner {
        require(!oldHolderWallets[wallet], "Wallet is already flagged.");
        oldHolderWallets[wallet] = true;
    }

    function unflagV1Holder(address wallet) external onlyOwner {
        require(oldHolderWallets[wallet], "Wallet is already not flagged.");
        oldHolderWallets[wallet] = false;
    }
    ...
    function _transfer(
        ...
        if (oldHolderSellLimitsEnabled) {
            if (oldHolderWallets[from]) {
                if (automatedMarketMakerPairs[to]) {
                    //enforce max sell restrictions.
                    require(
                        nextOldHolderSellDate[from] <= block.timestamp,
                        "Cannot sell yet"
                    );
                    require(
                        amount <= getOldHolderMaxSell(),
                        "Attempting to sell over max sell amount.  Check max."
                    );
                    nextOldHolderSellDate[from] =
                        block.timestamp +
                        oldHolderTimeBetweenBuys;
                } else if (!_isExcludedFromFees[to]) {
                    revert(
                        "V1 Holders cannot transfer and must sell only or transfer to a whitelisted address."
                    );
                }
            }
        }
    ...
    function getOldHolderMaxSell() public view returns (uint256) {
        address[] memory path = new address[](2);
        path[0] = dexRouter.WETH();
        path[1] = address(this);

        uint256[] memory amounts = new uint256[](2);
        amounts = dexRouter.getAmountsOut(maxOldHolderSell, path);
        return
            amounts[1] +
            (amounts[1] * (sellLiquidityFee + sellOperationsFee)) /
            100;
    }
    ...
    function setOldHolderMaxSell(uint256 amount) external onlyOwner {
        require(
            amount >= 10 && amount <= 2500,
            "Must set between 0.10 and 25 ETH"
        );
        maxOldHolderSell = amount * 1e16;
        emit UpdatedOldHolderMaxSell(amount);
    }

    function emergencyToggleOldHolderSellLimits(bool _enabled)
        public
        onlyOwner
    {
        oldHolderSellLimitsEnabled = _enabled;
    }

    function updateOldHolderCooldown(uint256 timeInMinutes) external onlyOwner {
        require(timeInMinutes > 0 && timeInMinutes <= 1440);
        oldHolderCooldown = timeInMinutes * 1 minutes;
    }

    function updateOldHolderTimeBetweenBuys(uint256 timeInMinutes)
        external
        onlyOwner
    {
        require(timeInMinutes > 0 && timeInMinutes <= 1440);
        oldHolderTimeBetweenBuys = timeInMinutes * 1 minutes;
    }
    ...
    function addLpWithoutLaunch(bool confirmAddLp) external onlyOwner {
    ...
    /**
     * Airdrop WITHOUT applying sell restrictions
     */
    ...
    /**
     * Sell restrictions will be applied to V1 Holders
     */
    function airdropV1Holders(
        address[] memory wallets,
        uint256[] memory amountsInTokens
    ) external onlyOwner {
        require(
        ...
        for (uint256 i = 0; i < wallets.length; i++) {
            ...
            oldHolderWallets[wallet] = true;
            nextOldHolderSellDate[wallet] = block.timestamp + oldHolderCooldown;
            super._transfer(msg.sender, wallet, amount);
        }
    }
}