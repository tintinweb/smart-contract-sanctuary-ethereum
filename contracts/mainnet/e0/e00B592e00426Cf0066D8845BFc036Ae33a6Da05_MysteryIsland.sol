...
contract MysteryIsland is ERC721A, Ownable, ReentrancyGuard {
    ...
    ) ERC721A("GoldenBox", "GB", _maxBatchSize, _collectionSize) {
        baseURI = initBaseURI;
    ...
    function reserve(address recipient, uint256 amount) external onlyOwner {
        require(recipient != address(0), "GoldenBox: zero address");
        require(amount > 0, "GoldenBox: invalid amount");
        require(
            totalSupply() + amount <= collectionSize,
            "GoldenBox: max supply exceeded"
        );
        require(
            tokensReserved + amount <= reserveAmount,
            "GoldenBox: max reserve amount exceeded"
        );
        require(
            amount % maxBatchSize == 0,
            "GoldenBox: can only mint a multiple of the maxBatchSize"
        );
    ...
    function mint(uint256 amount) external payable {
        require(status == Status.PublicSale, "GoldenBox: Public sale is not active.");
    ...
    function refundIfOver(uint256 price) private {
        require(msg.value >= price, "GoldenBox: Need to send more ETH.");
        if (msg.value > price) {
