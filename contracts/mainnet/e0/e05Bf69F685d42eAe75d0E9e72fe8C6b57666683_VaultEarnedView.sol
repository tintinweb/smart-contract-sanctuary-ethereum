...
contract VaultEarnedView{    
    ...
    function earned(address _stakingAddress, address _wrapper, address _extrarewards, address _vault) external returns (address[] memory token_addresses, uint256[] memory total_earned) {
        ...
        uint256[] memory stakedearned = IFraxFarmERC20(_stakingAddress).earned(address(this));
        IConvexWrapperV2.EarnedData[] memory convexrewards = IConvexWrapperV2(_wrapper).earnedView(address(this));
        ...
        if(_extrarewards != address(0)){
            IRewards.EarnedData[] memory extraRewards = IRewards(_extrarewards).claimableRewards(address(this));
