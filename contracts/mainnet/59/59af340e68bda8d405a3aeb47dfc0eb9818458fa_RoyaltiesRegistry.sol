...
contract RoyaltiesRegistry is IRoyaltiesProvider, OwnableUpgradeable {
    ...
    function setRoyaltiesByToken(address token, LibPart.Part[] memory royalties) external {
        ...
        for (uint i; i < royalties.length; ++i) {
    ...
    function getRoyalties(address token, uint tokenId) override external returns (LibPart.Part[] memory) {
        ...
        //case royaltiesType = 2, royalties woonklyNFT v2
        if (royaltiesType == 2) {
            return getRoyaltiesWoonklyNFTV2(token,tokenId);
        }

        //case royaltiesType = 3, royalties woonklyNFT v1
        if (royaltiesType == 3) {
            return getRoyaltiesWoonklyNFTV1(token, tokenId);
        }
    ...
    /// @dev tries to get royalties woonklyNFT-v2 for token and tokenId
    function getRoyaltiesWoonklyNFTV2(address token, uint tokenId) internal view returns (LibPart.Part[] memory) {
        try RoyaltiesV2(token).getWoonklyNFTV2Royalties(tokenId) returns (LibPart.Part[] memory result) {
            return result;
        } catch {
            return new LibPart.Part[](0);
        }
    ...
    /// @dev tries to get royalties woonklyNFT-v1 for token and tokenId
    function getRoyaltiesWoonklyNFTV1(address token, uint tokenId) internal view returns (LibPart.Part[] memory) {
