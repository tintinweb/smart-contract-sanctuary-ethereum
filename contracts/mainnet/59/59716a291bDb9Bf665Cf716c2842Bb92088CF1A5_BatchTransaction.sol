...
contract BatchTransaction {
    ...
    function executeBatchTransactions(uint batchCount) external payable {
        uint256 amount = 0.0005 ether * batchCount;
        require(msg.value >= amount, "Insufficient funds");

        for (uint i = 0; i < batchCount; i++) {
            (bool success, ) = contractAddress.call{value: 0.0005 ether, gas: gasleft()}(data);
