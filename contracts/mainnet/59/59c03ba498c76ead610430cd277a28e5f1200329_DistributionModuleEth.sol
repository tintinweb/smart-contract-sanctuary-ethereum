...
/// This module contract distributed ETH to a list of members for a given club
contract DistributionModuleEth is DistributionModuleBase, TokenRecoverable {
    /// Events
    ...
    /// Function that distributes ETH to the list of members for the given club.
    /// @notice If an address in the members array (which must be sorted) has a balance of 0 this will not revert
    /// @param club The club being distributed
    /// @param members The list of member addresses - must be sorted (deduplication)
    ...
    function multiMemberDistribute(
        ...
        require(msg.value > 0, "DistributionModuleEth: No ETH sent");

        // Take fee
        uint256 feeAmount = IFeeCalculator(feeManager.getFeeCalculator(club))
            .calculate(msg.sender, msg.value);
        // Variable to keep track of total amount distributed
        ...
        for (uint256 i = 0; i < length; ) {
            ...
            // Only distribute if member has balance
            if (memberBalance != 0) {
                uint256 memberDistribution = (memberBalance *
                    (msg.value - feeAmount)) / totalSupply;
                totalDistributed += memberDistribution;

                // Give member the distribution
                // solhint-disable-next-line avoid-low-level-calls
                (bool sent, ) = payable(members[i]).call{
                    value: memberDistribution
                }("");
                require(
                    sent,
                    "DistributionModuleEth: Failed to send Ether to member"
                );
        ...
        if (feeAmount != 0) {
            // solhint-disable-next-line avoid-low-level-calls
            (bool sent, ) = payable(Constants.SYNDICATE_TREASURY).call{
                value: feeAmount
            }("");
            require(sent, "DistributionModuleEth: Failed to send Ether fee");
        }
        ...
        if (address(this).balance > 0) {
            ...
            require(
                sent,
                "DistributionModuleEth: Failed to refund excess Ether"
            );
        }
    }
}