...
contract FluffyWorld is Ownable, ERC721A, PaymentSplitter {
    ...
    uint private constant MAX_PUBLIC = 444;
    uint private constant MAX_WL = 444;

    uint public publicSalePrice = 0.0088 ether;
    ...
    constructor(address[] memory _team, uint[] memory _teamShares, address _signerAddressWL, string memory _baseURI) ERC721A("Fluffy World", "FLUFFYWORLD")
    ...
    function WLMint(uint _quantity, bytes calldata signature) external {
        ...
        if(mintedAmountNFTsperWalletWhitelistSale[msg.sender] + _quantity > maxMintAmountPerWhitelist) revert("Max Whitelist Sale exceeded for user");
