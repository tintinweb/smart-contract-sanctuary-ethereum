    ...
    OwnableUpgradeable
{
    ...
    function createFullTrack(
        ...
        for (uint256 i = 0; i < royaltyAddresses.length; i++) {
            require(
                ...
                "ArtistV2: Invalid royalties data"
            );
        ...
        require(
            total == 10_000,
            "ArtistV2: Royalties addition must be equal to 100%"
        );
    ...
    function buyFullTrack(
        ...
        require(
            ...
            "ArtistV2: Purchase for non existent fullTrackId"
        );
        require(
            msg.value >= fullTrackIdToFullTrack[fullTrackId].price,
            "ArtistV2: Please submit the asking price"
        );
        require(
            fullTrackIdToFullTrack[fullTrackId].startTime < block.timestamp,
            "ArtistV2: FullTrack is not available yet"
        );
        require(
            fullTrackIdToFullTrack[fullTrackId].supply >
                fullTrackIdToFullTrack[fullTrackId].sold,
            "ArtistV2: FullTrack is sold out"
        );

        address signer = _verify(signature, fullTrackId, buyerAddress);
        require(
            hasRole(MINTER_ROLE, signer),
            "ArtistV2: Signature invalid or unauthorized"
        );
        ...
        for (uint256 i = 0; i < _royaltiesReceivers.length; i++) {
            ...
            require(success, "ArtistV2: Transfer failed");
        }
        _tokenIds.increment();
        fullTrackIdToFullTrack[fullTrackId].sold++;
    ...
    {
        require(_exists(tokenId), "ArtistV2: URI query for nonexistent token");
        return
