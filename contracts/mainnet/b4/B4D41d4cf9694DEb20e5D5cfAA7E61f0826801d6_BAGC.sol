...
contract BAGC is ERC721URIStorageUpgradeable, ERC721EnumerableUpgradeable, BAGCCore {
    ...
    function userMint(
        ...
        require(expiredAt > block.timestamp, "BAGC: signature is expired");
    ...
    function claimLock(uint256 tokenId, uint256 value) public onlyClaimContract {
        if (getLockStatus(tokenId) == 2) {
            revert BAGC_Already_Locked();
        } else if (getLockStatus(tokenId) == 0) {
            _locked[tokenId] = value;
        }
    ...
    function isStaked(uint256 tokenId) public view returns (bool) {
        return _endTime[_poolId[tokenId]] >= block.timestamp;
    }
    ...
    function _beforeTokenTransfer(
        ...
        require(!isStaked(tokenId), "BAGC: staked");
...
contract BAGCCore is OwnableUpgradeable {
    ...
    uint256[] public additionalTokenIds;
    ...
    function updateAddtionalTokenIds(uint256[] memory tokenIds) public onlyOwner {
        for (uint256 i = 0; i < tokenIds.length; i++) {
            additionalTokenIds.push(tokenIds[i]);
        }
    }
    ...
    function getRandomAvailableTokenId(address to, uint256 salt) internal returns (uint256) {
        ...
        uint256 randomIndex = (randomNum) % (_numUserAvailableTokens + additionalTokenIds.length);

        if (randomIndex >= _numUserAvailableTokens) {
            uint256 additionalIndex = randomIndex - _numUserAvailableTokens;
            uint256 _tokenId = additionalTokenIds[additionalIndex];

            additionalTokenIds[additionalIndex] = additionalTokenIds[additionalTokenIds.length - 1];
            additionalTokenIds.pop();
