...
import "./ArcJungle.sol";

contract ArcJungleFactory is Ownable {
    event NewArcJungleContract(address indexed arcJungle);
     ...
     * @return address of new arc jungle contract
     */
    function deployPool(
        IBEP20 _stakedToken,
        IBEP20 _rewardToken,
        ...
        bytes memory bytecode = type(ArcJungleInitializable).creationCode;
        bytes32 salt = keccak256(
            abi.encodePacked(_stakedToken, _rewardToken, _startBlock)
        );
        address arcJungleAddress;

        assembly {
            arcJungleAddress := create2(
                0,
                add(bytecode, 32),
                mload(bytecode),
                salt
        ...
        ArcJungleInitializable(arcJungleAddress).initialize(
            _stakedToken,
            _rewardToken,
            _rewardPerBlock,
            _startBlock,
        ...
        emit NewArcJungleContract(arcJungleAddress);
    }
}
...
contract ArcJungleInitializable is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    using SafeBEP20 for IBEP20;

    // The address of the arc jungle factory
    address public ARC_JUNGLE_FACTORY;
    ...
    // The block number when ARC mining ends.
    uint256 public bonusEndBlock;

    // The block number when ARC mining starts.
    ...
    function initialize(
        ...
        require(msg.sender == ARC_JUNGLE_FACTORY, "Not factory");
        require(
