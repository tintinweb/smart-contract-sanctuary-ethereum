...
import {FunMint4} from "./FunMint4.sol";

contract FunMintFactory4 is Ownable {
    ...
    function create(uint256 id) public returns (address) {
        ...
        address deployed = deploy(conf.salt, type(FunMint4).creationCode);
        FunMint4(payable(deployed)).init(conf.name, conf.symbol, conf.owner);
        return deployed;
    }
}
