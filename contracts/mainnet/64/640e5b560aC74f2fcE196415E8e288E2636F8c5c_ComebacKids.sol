...
contract ComebacKids is ERC721A, Ownable, DefaultOperatorFilterer, ReentrancyGuard {
    ...
    bytes32 public merkleRoot = 0x43e4d815f6ed28a05c049d809f85fb0e125907c531470793af9549c17baaa020;
    bytes32 public ogRoot = 0xeb37a3f3a00b4806b9d5260fa4a1013b48d822168df0056775306b95ef3fc483;

    mapping(address => uint256) public ogPublicMints;
    mapping(address => uint256) public wlPublicMints;

    string public baseURI = "ipfs://QmTdsQX1BkmRkUySVT9aF3EFbBRbVCWxFGnsDxzUpychST/ck.json";
    
    uint32 publicSaleStartTime;

    uint256 public PRICE = 0.03 ether;
    uint256 public SUPPLY_MAX = 2500;
    ...
    constructor() ERC721A("Comebackids", "CK") payable {
        presaleConfig = PresaleConfig({
            startTime: 1679158800,
            endTime: 1679162400,
            whitelistMintPerWalletMax: 7,
            whitelistPrice: 0.025 ether
        });
        ogConfig = OGConfig({
            startTime: 1679155200,
            ogMintPerWalletMax: 9,
            ogMaxSupply: 100,
            ogPrice: 0.015 ether
        });
        publicSaleStartTime = 1679162400;
    ...
    function withdraw() external onlyOwner {
        ...
        payable(0x40dcBD5F84b597154B2BDb72acB9C960040B0CE9).transfer(balance * 60/100); // 60% Zack
        payable(0xBaABbD58fbD97F462f3731e6C8ccf9b82A856AE2).transfer(balance * 20/100); // 20% Squirt
        payable(0xf17e7f8557C1102090d5D5B9a30D9D34A9A03485).transfer(balance * 10/100); // 10% Kuya
        payable(0x446B7f1EC4749fddAfC50CcaA0c8f82d4665FB61).transfer(balance * 10/100); // 10% Stonk
    }
