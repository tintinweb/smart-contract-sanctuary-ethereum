...
contract TheBabyPomeranian is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;
    
    string private _name = "The Baby Pomeranian";
    string private _symbol ="TBPOM";
    uint8 private _decimals = 18;

    address payable public treasuryWallet = payable(0x90D96e4C9Cc031644f9ef96fEE7ce6D9358bAf0E);
    ...
    uint256 public _TreasuryFeeSell = 5;
    ...
    function swapAndLiquify(uint256 tAmount) private swapIsLocked {
        ...
        uint256 amountETHTeam = amountReceived.mul(_marketingShare).div(totalETHFee);
        uint256 amountETHMarketing = amountReceived.sub(amountETHLiquidity).sub(amountETHTeam);

        if(amountETHMarketing > 0)
            transferToAddressETH(treasuryWallet, amountETHMarketing);
        if(amountETHTeam > 0)
            transferToAddressETH(treasuryWallet, amountETHTeam);
