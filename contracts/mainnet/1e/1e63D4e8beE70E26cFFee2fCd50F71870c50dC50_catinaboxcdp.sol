...
contract catinaboxcdp is Multicall, AccessControl {
    ...
    function Owing(address _depositor) public view returns(uint256) {
        ...
        uint256 weight = deposited[_depositor] - debt[_depositor];
    ...
    modifier sync() {
        uint256 excess = collateral.balanceOf(address(this)) - totaldeposits;
        if (excess > 100) {
            // 1% of yield + 1/4 yield * total debt / total deposits
            uint256 protocolFees = excess * idleFee / 100 + excess * totalDebt / (totaldeposits * 4 + 1);
            uint256 value = excess - protocolFees;
            uint256 totalWeight = totaldeposits - totalDebt;
            totalPoints = totalPoints + (value * pointMultiplier / totalWeight);
            unclaimed += value;
            totaldeposits += value;
            // send amount to staking module
            collateral.transfer(feeSplitAddress, protocolFees);
        }
        _;
    }
    ...
    function setFee(uint256 _fee) external onlyRole(ADMIN_ROLE) {
        require(_fee <= 75, "the fees are too damn high");
        idleFee = _fee;
    }
