...
contract BulkRegister is Ownable {
    ...
    function mint(RegisterRequest[] calldata requests, address owner, bool thirdParty) external payable {
        ...
        // third party callers must not receive any funds
    ...
    function verifyRegisterParams(RegisterRequest[] calldata requests, uint256 paymentAmount) external view returns (string memory) {
        ...
        for (uint i = 0; i < requests.length; i++) {
            if (requests[i].tldNameHash == FOREVER_NAMEHASH) {
                ...
                (string memory ensError, uint256 cost) = _verifyENS(requests[i]);
                if (bytes(ensError).length > 0) {
                    return ensError;
                }
                totalCost += cost;
            } else {
    ...
    function _mintETH(RegisterRequest calldata request, address owner, uint256 totalPaid) internal returns (uint256) {
        uint cost = IENSController(request.controller).rentPrice(request.label, request.duration);
        ...
        if (request.targetCost != cost) {
            require(request.targetCost != 0 && cost != 0, "BulkRegister: price changed");

            // If price has increased more than +5% of the target cost for this name, revert.
            // This guarantees that the duration is not reduced by more than 5% which can be unexpected for users.
            require(request.targetCost + ((request.targetCost * 500) / 10000) > cost, "BulkRegister: price changed");
        ...
        IENSController(request.controller).registerWithConfig{value : cost}(
            request.label,
            owner,
            duration,
            request.secret,
    ...
    function _verifyForever(RegisterRequest calldata request) internal view returns (string memory, uint256) {
        ...
        uint256 cost = controller.price(request.label);
        if (cost != request.targetCost && request.targetCost + ((request.targetCost * 500) / 10000) > cost) {
            return ("Prices have changed please try again", cost);
        }
    ...
    function _verifyImpervious(RegisterRequest calldata request) internal view returns (string memory, uint256) {
        ...
        if (request.secret == 0 && IImperviousRegisterNowController(request.controller).commitmentOnly(request.tldNameHash)) {
    ...
    function _verifyENS(RegisterRequest calldata request) internal view returns (string memory, uint256) {
        IENSController controller = IENSController(request.controller);
        if (!controller.available(request.label)) {
            return ("Name not available", 0);
        }
...
abstract contract IENSController {
...
abstract contract IImperviousRegisterNowController {
    function commitmentOnly(bytes32 node) public virtual view returns (bool);
