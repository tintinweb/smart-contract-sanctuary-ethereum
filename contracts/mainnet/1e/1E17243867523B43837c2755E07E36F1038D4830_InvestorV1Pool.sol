...
contract InvestorV1Pool is IInvestorV1Pool {
    ...
    uint256 public immutable override fundId;
    uint256 public immutable override capacity;
    ...
    function expectedRevenue(address _account) public override view returns (uint256) {
        uint256 liquidityTotal = (10000 - fee);
        liquidityTotal = liquidityTotal * (10000 + interestRate);
        liquidityTotal = capacity.mul(liquidityTotal);
        liquidityTotal = liquidityTotal.div(100000000);

        uint256 poolClaim = 0;
        uint256 restakeClaim = 0;

        if(pooledAmt[_account] > 0) {
            poolClaim = liquidityTotal.mul(pooledAmt[_account]);
            poolClaim = poolClaim.div(capacity);   
        }

        if(restakeAmt[_account] > 0 && exited > 0) {
            restakeClaim = liquidityTotal.mul(exited);
            restakeClaim = restakeClaim.mul(restakeAmt[_account]);
            restakeClaim = restakeClaim.div(capacity);
            restakeClaim = restakeClaim.div(restaked);
        }

        return poolClaim.add(restakeClaim);

    }

    function getInfo(address _account) public override view returns (string memory, string memory, uint256, uint256, uint256, uint256, uint256, uint24) {
        return (name, getPoolState(), capacity, funded, pooledAmt[_account], restakeAmt[_account], expectedRevenue(_account), interestRate);
    }

    function getExtra() public override view returns (address, uint256, uint256, uint256, string memory, string memory, string memory) {
        return (operator, oraclePrice, restaked, exited, collateralDocument, collateralHash, detailLink);
    }
    ...
    function update() public override returns (bool) {
        if(poolState == PoolState.Opened && block.timestamp > stageTime) {
            if(funded >= capacity) { 
                ...
                exited = capacity - funded;
                emit PoolActiviated(funded);
            }
            else { 
                poolState = PoolState.Reverted; 
                emit PoolReverted(capacity, funded);
            }
            return true;
        }
        ...
        if(poolState == PoolState.Active && block.timestamp > endTime) {
            ...
            else estimated = capacity.mul(estimated);
            estimated = estimated.div(100000000);
    ...
    function setOraclePrice(uint256 _oraclePrice) public override onlyOperator returns (bool) {
        ...
        uint256 minDeposit = capacity.mul(100);
        minDeposit = minDeposit.div(_oraclePrice);
        require(minDeposit > 0, "InvestorV1Pool: minDeposit too small");
        if (capacity.mod(_oraclePrice) != 0) { minDeposit = minDeposit.add(1); }
    ...
    function openPool() public override onlyOperator returns (bool) {
        ...
        require(minDeposit > 0, "InvestorV1Pool: minDeposit too small");
        if (capacity.mod(oraclePrice) != 0) { minDeposit = minDeposit.add(1); }
        minDeposit = minDeposit.mul(10**12);
    ...
    function deposit(uint256 amount) public override returns (bool) {
        ...
        require(funded.add(amount) <= capacity, "InvestorV1Pool: deposit over capacity");
    ...
    function withdrawAll(address to) public returns (bool) {
        return withdraw(pooledAmt[msg.sender], to);
    }

    function exitAll(address to) public returns (bool) {
        return exit(pooledAmt[msg.sender], to);
    }

    function unstakeAll(address to) public returns (bool) {
        return unstake(restakeAmt[msg.sender],to);
    }
