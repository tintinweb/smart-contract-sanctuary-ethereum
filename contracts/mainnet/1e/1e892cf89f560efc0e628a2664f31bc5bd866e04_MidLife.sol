...
contract MidLife is ERC721A, Ownable, ReentrancyGuard {
    ...
    bool public publicSaleActived = false;
    ...
    bytes32 public merkleRoot = 0x698fc3a5e7e3b153c4794c83173976d04fa5cc4cfd571f43a9e6a5440a8b930b;

    event Minted(address minter, uint256 amount);
    event PublicMinted(address minter, uint256 amount);
    event SetFlipSaleActive(bool isActive);
    event SetFlipPublicSaleActive(bool isPublicActive);
    ...
    function publicMintML(uint256 tokenQuantity) external payable {
        require(publicSaleActived, "Public sale must be active to mint Middle Life");
        require(totalSupply() + tokenQuantity <= MAX_SUPPLY, "Sale would exceed max supply");
        require(tokenQuantity * mintPrice <= msg.value, "Not enough ether sent");

        _safeMint(msg.sender, tokenQuantity);
        emit PublicMinted(msg.sender, tokenQuantity);
    }
    ...
    function flipPublicSaleActive() external onlyOwner {
        ...
        emit SetFlipPublicSaleActive(publicSaleActived);
    }
