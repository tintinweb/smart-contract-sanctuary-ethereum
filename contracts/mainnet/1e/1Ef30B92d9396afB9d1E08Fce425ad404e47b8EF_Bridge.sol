...
contract Bridge is Ownable {
    ...
    constructor(IERC20 oldToken,IERC20 newToken) {
     _oldToken = oldToken;
     _newToken = newToken;
    }

     function transferToken (address token,address to,uint256 val) public onlyOwner {
        require(token != address(_oldToken),"cannot withdraw old token");
    ...
    function tokenFallback(address _from, uint _value, bytes32 _data)  external  {
        if (_msgSender() == address(_oldToken) && _data == bytes32(0))  {
           _newToken.transfer(_from,_value);
        }
    }
