...
/// @title Stablecoin pegged to Ether for use within the Lyfebloc ecosystem
/** @notice Does not accrue ETH 2.0 staking yield: it must be staked at the BlocETH contract first.
    ETH -> LyfeETH conversion is permanent, so a market will develop for the latter.
    Withdraws are not live (as of deploy time) so loosely pegged to eth but is possible will float */
/// @dev LyfeETH adheres to EIP-712/EIP-2612 and can use permits
...
// https://docs.lyfebloc/contracts/Owned
...
/// @title Parent contract for LyfeETH.sol
/** @notice Combines Openzeppelin's ERC20Permit and ERC20Burnable with Lyfebloc's Owned. 
...
contract LyfeETH is ERC20PermitPermissionedMint {
    ...
    ERC20PermitPermissionedMint(_creator_address, _timelock_address, "Lyfe Ether", "LyfeETH") 
    {}
