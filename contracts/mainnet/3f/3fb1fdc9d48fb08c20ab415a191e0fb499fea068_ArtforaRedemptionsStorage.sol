...
contract ArtforaRedemptionsStorage is Ownable {
    ...
    function isRedeemed(address _contractAddress, uint256 _tokenId) external view returns (bool) {
        return redemptions[_contractAddress][_tokenId];
    }

    // authorized only 
    function redeem(address _contractAddress, uint256 _tokenId) external onlyAuthorizedCaller {
    ...
    function setRedemptions(address[] calldata _addresses, uint256[] calldata _tokenIds, bool[] calldata _states) external onlyOwner {
        require(_addresses.length == _tokenIds.length && _tokenIds.length == _states.length, "Invalid input");
