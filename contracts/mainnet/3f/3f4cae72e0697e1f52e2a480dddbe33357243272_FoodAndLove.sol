...
// File: contracts/FoodAndLove.sol
...
contract FoodAndLove is ERC721A, ERC2981{
    ...
    constructor(string memory _uriPrefix) ERC721A("Food and Love", "F&L") {
        owner = msg.sender;
        operator = msg.sender;
        uriPrefix=_uriPrefix;
    }

    function supportsInterface(bytes4 interfaceId) public pure override(ERC721A, ERC2981) returns (bool) {
        return
            interfaceId == 0x01ffc9a7 || // ERC165 interface ID for ERC165.
            interfaceId == 0x80ac58cd || // ERC165 interface ID for ERC721.
            interfaceId == 0x5b5e139f || // ERC165 interface ID for ERC721Metadata.
    ...
    function setBaseURI(string memory uri) onlyOperator external{
        uriPrefix = uri;
    }

    function mint(address to, uint256 quantity) onlyOperator external {
        _mint(to, quantity);
    }
