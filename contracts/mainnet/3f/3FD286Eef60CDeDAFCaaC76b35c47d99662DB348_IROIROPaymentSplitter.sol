...
contract IROIROPaymentSplitter {
    using SafeERC20Upgradeable for IERC20Upgradeable;

    address constant IROIRO = 0xc78b8E9f12EDbc74A708F9B5A0472B33b3B286ce; 
    ...
    function withdrawToken(address token) public {
        if (token == address(0)) {
            ...
            sendValue(payable(IROIRO), address(this).balance);
        } else {
            uint256 balance = IERC20Upgradeable(token).balanceOf(address(this));
            uint256 uwulabs = (0.1 gwei*balance)/1 gwei;
            IERC20Upgradeable(token).safeTransfer(payable(UWU_LABS), uwulabs);
            IERC20Upgradeable(token).safeTransfer(payable(IROIRO), IERC20Upgradeable(token).balanceOf(address(this)));
        }
    }
}