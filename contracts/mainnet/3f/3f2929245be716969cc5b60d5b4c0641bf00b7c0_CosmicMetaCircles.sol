...
// ERC-721 Smart Contract for the Cosmic Meta Circles NFT Collection - https://cosmicmeta.io
...
contract CosmicMetaCircles is ERC721, ERC721Enumerable, Pausable, Ownable {
    using Strings for uint256;

    string public baseTokenURI;
    string public contractURI;
    uint256 public constant MAX_TOKENS = 4444; // 4444 Circles
    uint256 public maxMint = 10; // Max 10 Circles per transaction
    uint256 public price = 0 ether; // Each Circles cost 0 ETH to mint
    uint256 public preMintPrice = 0 ether; // Lower price for pre-sale
    ...
    constructor(string memory _baseTokenURI, string memory _contractURI) ERC721("Cosmic Meta Circles", "CMC") {
        setBaseURI(_baseTokenURI);
        setContractURI(_contractURI);

        // The first 50 Circles are minted for the team, giveaways and our community
        mint(msg.sender, 50);
    }
    ...
    function preMint(address _to, uint256 _quantity) public payable {
        ...
        require(hasPresaleAccess(_to), "You are not whitelisted for the Circles pre-sale!");
        require(!mainSale, "You can't pre-mint Circles at the moment!");
        require(_quantity > 0 && _quantity <= maxMint, "You can only mint 1 to 10 Circles!");
    ...
    function mint(address _to, uint256 _quantity) public payable {
        ...
        if (msg.sender != owner()) {
            require(mainSale, "The Circles main sale is not open!");
            require(_quantity > 0 && _quantity <= maxMint, "You can only mint 1 to 20 Circles!");
