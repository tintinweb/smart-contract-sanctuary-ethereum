...
contract SirToken is IERC20, Context {
    ...
    constructor (
        ...
        A[0xA0] = 0x6e90cc2aa4821d2B90f93dfBED7E548d7Aef398e; // burn
        A[0xA1] = 0x4f8e60aDaBEf220bCfb525BFA02560c0C61dB616; // liquidity
        A[0xA2] = 0xCDAaC508aCE4BD8B5a81cFBe71e95cF9957a657e; // treasury
        A[0xA3] = payable(proxy);                             // proxy
        A[0xA4] = 0x5fe2e522838E2ee97080Ea29eE61338544cd6ecc; // dev
        A[0xA5] = 0x321a5927B36B01986D0E898C9A672223733cF374; // advisors
        A[0xA6] = 0xa6DB2D8bDA916a3Bc7ba467f55B93E0373F94A82; // team
        A[0xA7] = 0x6AC429Dee5dB61629334Ae0febe80D2d791772fb; // marketing
        A[0xA8] = 0x03722591f206B4eEc43FcFd6140cF072F4090Cdc; // reserve 
        A[0xA9] = 0xF064140b3190628dB96e352937E7AD19942c4eC9; // seed investor
        A[0xAA] = 0x26450202aE5b09E937f9FbB030C41C75877E2174; // airdrops
        A[0xAB] = 0xd37F165D6C80e82CC28C22aF25E886b92a4A5E5F; // LP provider address
        A[0xAC] = 0xeD48cb2e529f44698edE2f052b3044Dc6b51EA82; // liquify reserve address
        _locked[A[0xAB]] = 0;
        ...
        __mint(A[0xAB], 350000000 * multiplier);   // Main LP
        __mint(A[0xA6],  10000000 * multiplier);   // team
        __mint(A[0xA8],  30000000 * multiplier);   // reserve
        __mint(A[0xA9],  37500000 * multiplier);   // seed investor
        __mint(A[0xA7],   5000000 * multiplier);   // marketing
        __mint(A[0xA5],   3000000 * multiplier);   // advisors
        __mint(A[0xAA], Max.airdrop);              // airdrop 2.000.000
     ...
     *  Tax[index].percent     (integer 0 - 9)
     *  Tax[index].tThreshold   (integer x * 10**18)
     *  Tax[index].eThreshold   (integer x * 10**18)
     * 
     ...
     *  @param index_ bytes1
     *  @param percent_ uint256
     *  @notice Required:
     *      - index_ must still be in effect (we are not in post burn era)
     *      - tax percentage has to be different from current
     *      - tax percentage for treasury tax has to be less or equal to 8%
     *      - total sell or burn tax has to be less or equal to 15%
     *  @notice Emits:
     ...
     *  @param eThreshold bool, if true eth threshold will be set
     *  @notice Requires:
     *      - index_ must still be in effect (we are not in post burn era)
