...
contract PETRICHOR is Context, IERC20, Ownable {
    ...
    uint256 private _sellProjectFee = 30;
    uint256 private _previousSellProjectFee = _sellProjectFee;
       
    address payable private _projectWallet;
    
    string private constant _name = "Earthy scent";
    string private constant _symbol = "PETRICHOR";
    ...
    function setBuyFee(uint256 buyProjectFee) external onlyOwner {
        require(buyProjectFee <= 30, "Buy tax is hard coded to remain under 30%");
        _buyProjectFee = buyProjectFee;
    }

    function setSellFee(uint256 sellProjectFee) external onlyOwner {
        require(sellProjectFee <= 30, "You safe friends.");
