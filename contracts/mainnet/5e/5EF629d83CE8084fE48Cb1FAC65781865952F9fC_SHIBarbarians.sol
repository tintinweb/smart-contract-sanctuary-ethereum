...
contract DividendPayingToken is ERC20, IDividendPayingToken, IDividendPayingTokenOptional, Ownable {
  ...
  address public DividendsToken = address(0x9813037ee2218799597d83D4a5B6F3b6778218d9);
...
contract SHIBarbarians is Context, ERC20, Ownable {
    using SafeMath for uint256;
   ...
   BoneClubDividendTracker public dividendTracker;
    ...
    constructor() ERC20 ("SHIBarbarians", "BoneClub") {
        _buyRewardsFee = 4;
        _buyLiquidityFee = 1;
		_buyMarketingFee = 4;
        _buyDevFee = 1;
    	...
    	dividendTracker = new BoneClubDividendTracker();

    	liquidityWallet = owner();
		
		marketingWallet = payable(0x7b5A425f28f40275cf7e0F3d20c860714926b476);
        _devWallet = payable(0x5f744d0DE6018B5eEf39f90FEFf2AA4Beb1Ab8a3);
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "BoneClub: The dividend tracker already has that address");

        BoneClubDividendTracker newDividendTracker = BoneClubDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "BoneClub: The new dividend tracker must be owned by the BoneClub token contract");
    ...
    function updateDividendToken(address newAddress) public onlyOwner {
        require(newAddress != address(DividendsToken), "BoneClub: The router already has that address");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "BoneClub: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "BoneClub: Account is already the value of 'excluded'");
    ...
    function addToBlacklist(address account, bool blacklisted) public onlyOwner {
        require(_isBlacklisted[account] != blacklisted, "BoneClub: Account is already the value of 'blacklisted'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "BoneClub: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "BoneClub: Automated market maker pair is already set to that value");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) public onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "BoneClub: The liquidity wallet is already this address");
    ...
    function updateMarketingWallet(address newMarketingWallet) public onlyOwner {
        require(newMarketingWallet != marketingWallet, "BoneClub: The marketing wallet is already this address");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "BoneClub: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "BoneClub: Cannot update gasForProcessing to same value");
	...
	function letsGoLive() external onlyOwner() {
        updateBuyFees(5,5,5,0);
        updateSellFees(10,10,10,0);
...
contract BoneClubDividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("BoneClub_Dividend_Tracker", "BoneClub_Dividend_Tracker") {
    	claimWait = 1800;
    ...
    function withdrawDividend() public pure override {
        require(false, "BoneClub_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main BoneClub contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 0 && newClaimWait <= 86400, "BoneClub_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "BoneClub_Dividend_Tracker: Cannot update claimWait to same value");
