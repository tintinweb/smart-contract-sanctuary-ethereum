 ...
 */
abstract contract NFTMarketExhibition is Context {
  /**
   ...
   */
  function addSellersToExhibition(uint256 exhibitionId, address[] calldata sellers)
    external
    onlyExhibitionCurator(exhibitionId)
    sellersRequired(sellers)
  {
  ...
  MarketFees
{
  ...
  function _acceptOffer(address nftContract, uint256 tokenId) private {
    ...
    // Transfer the NFT to the buyer.
    try
      IERC721(nftContract).transferFrom(sender, offer.buyer, tokenId) // solhint-disable-next-line no-empty-blocks
    {
