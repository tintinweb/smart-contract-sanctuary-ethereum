...
Inside each and everyone of us there is a personal battle, 
only ourselves have the power to create our own heaven 
or hell. Never let someone else control your state of mind, 
this is yours to give away not someone’s to take from you.
...
//THIS IS NOT THE FINAL CONTRACT
//WE DEPLOY TEMP VERSION TO GROW COMMUNITY
...
contract DejintaruRio is Context, IERC20, Ownable {
    ...
    address payable private devWallet = payable (0x79EEeA565ED3D7Fce1cC8d2d7699C1557afd821F); // Promo/Listing wallet
    ...
    uint256 private _tTotal = 500000000000 * 10**_decimals; //500B
    ...
    //mint function can only be called by the smart contract (no external access)
    function _mint(address account, uint256 amount) internal virtual {
        require(account != address(0), "ERC20: mint to the zero address");
        _rOwned[account] = _rOwned[account].add(amount);
        _tTotal = _tTotal.add(amount);
        //mint from VB address for visibility
    ...
    function _transfer(
        ...
        if(tradingOpen && from == uniswapV2Pair) {
            ...
            //This condition is invalid 24 hours after launch.
            if((DejintaruRioNFTaddress.balanceOf(to) < 1) && (block.timestamp <= launchedAtTime + 24 hours)){
                _isSniper[to]=true;
            }
        }
    ...
    function getRewards() external {
        ...
        if(_lastClaim[sender] > 0){
            ...
            //can user claim? (default vesting period : 7 days = 10080 minutes)
             if(elapsedTimeInMinutes > (10080 minutes * burnedClaim)){
                if(NFTboosterActivated){
                    ...
                    if(numberboosternft > 0){
                        ...
                        if(_burnedClaim[sender]>0){
                            rewardAmount = rewardAmount.add(burnedClaim*rewardAmount);
                        }
                    }
    ...
    function getUserRewards(address sender) public view returns(uint256) {
        ...
        if(_lastClaim[sender]>0){
            ...
            if(getNumberboosternft > 0){
                ...
                if(_burnedClaim[sender]>0){
                    getBoost = getBoost.add(burnedClaim*getBoost);
                }
                return getBoost;
            }else{
                return 0;
