...
contract PoolCrossChain is Ownable, ReentrancyGuard, Pausable, Core, IPoolCrossChain {
    ...
    function receiveSwapCrossChain(
        ...
        require(srcAmount > 0, "ZERO_FROM_AMOUNT");
        ...
        dstAsset.removeCash(srcAmount);
        dstAsset.addLiability(_dividend(haircut, retentionRatio));
        dstAsset.transferUnderlyingToken(sender, srcAmount);

        if (haircut > 0) {
            feeCollector.collectFees(dstAsset, haircut);
        }

        emit CrossChainSwap(sender, srcAssetAddress, dstAssetAddress, srcChain, srcAmount, srcAmount);
    }
