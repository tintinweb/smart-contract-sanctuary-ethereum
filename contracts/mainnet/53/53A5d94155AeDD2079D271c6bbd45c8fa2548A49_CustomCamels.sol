...
// File contracts/CustomCamels.sol

pragma solidity ^0.8.13;
contract CustomCamels is ERC721A, Ownable, DefaultOperatorFilterer {
    ...
    constructor(string memory initialBaseURI) ERC721A("CustomCamels", "CustomCamels") {
        setBaseTokenURI(initialBaseURI);
    }

    function ownerMint(uint256 _quantity, address _target) external onlyOwner {
        _mint(_target, _quantity);
    }

    function allowanceMint(uint256 _quantity) external payable {
        require(mintAllowance[msg.sender] >= _quantity, "Insufficient allowance");
        require(msg.value >= _quantity * price, "Not enough ether sent");
        mintAllowance[msg.sender] -= _quantity;
        _mint(msg.sender, _quantity);
    }

    function setMintAllowance(uint256 _quantity, address _target) external onlyOwner {
        mintAllowance[_target] = _quantity;
    }
