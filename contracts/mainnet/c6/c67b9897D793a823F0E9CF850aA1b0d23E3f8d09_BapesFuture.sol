...
contract BapesFuture is ERC721A, Ownable, AccessControl, ReentrancyGuard {
  ...
  mapping(address => uint256) private pcUsed;
  ...
  function pcMint(uint256 _mintAmount, bytes32[] calldata _merkleProof) external payable mintCompliance(_mintAmount) {
    uint256 minted = pcUsed[_msgSender()];
    ...
    require(balance >= _mintAmount, "Not enough PCs");
    require((minted + _mintAmount) <= balance, "Selected number of mints will exceed the amount of PCs you have.");

    bytes32 leaf = keccak256(abi.encodePacked(_msgSender()));

    require(
      MerkleProof.verify(_merkleProof, pcMerkleRoot, leaf),
      "Invalid proof, this wallet is not allowed to mint PC."
    );

    pcUsed[_msgSender()] = minted + _mintAmount;
  ...
  function updatePCRoot(bytes32 _pcRoot) external onlyRole(DEFAULT_ADMIN_ROLE) {
    pcMerkleRoot = _pcRoot;
  }
}
