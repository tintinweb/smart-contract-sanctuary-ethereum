...
contract SenshiTribe is Ownable, ERC721A, ReentrancyGuard {
  ...
  constructor(
    uint256 maxBatchSize_,
    uint256 collectionSize_
  ) ERC721A("SenshiTribe", "ST", maxBatchSize_, collectionSize_) {
  ...
  function MintBywhiteList(uint256 quantity,bytes32[] calldata _merkleProof) external payable callerIsUser {
    ...
    uint256 _saleStartTime = uint256(saleConfig.wlMintStartTime);
    ...
    require(
      totalSupply() + quantity <= (collectionSize + 1),
  ...
  function MintbyPublic(uint256 quantity) external payable callerIsUser {    
    ...
    require(quantity<=saleConfig.AmountForPubliclist, "reached max mint supply");
    require(totalSupply() + quantity <= (collectionSize + 1), "reached collection max supply");
  ...
  function isWhiteListSaleOn() public view returns (bool) {
    return
      saleConfig.wlMintStartTime != 0 &&
      block.timestamp >= saleConfig.wlMintStartTime;
  }
  ...
  function setWLMintStartTime(uint32 timestamp) external onlyOwner {
    saleConfig.wlMintStartTime = timestamp;
  }
  ...
  function withdraw() external  nonReentrant {
    require(msg.sender == 0x4C92FDf87B7df98232231D66CdcC4E47D26fF8B3);
