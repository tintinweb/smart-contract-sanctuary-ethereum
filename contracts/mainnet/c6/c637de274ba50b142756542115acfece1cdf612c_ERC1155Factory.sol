...
contract ERC1155Collection is ERC1155Base {
    ...
    function mint(
        ...
        payable(mintableAddress).transfer(msg.value);
    }
    ...
    function lazyMint(
        ...
        uint256 mintingCharges = IERC1155Mintable(mintableAddress)
            .mintingChargePerToken();
        uint256 brokerage = IERC1155Mintable(mintableAddress).brokerage();
        require(
            msg.value >= (price + mintingCharges) * amount,
            "ERC1155Mintable: Insufficent fund transferred."
        );
        _lazyMint(
            to,
        ...
        uint256 _sellerFund = (price - _brokerage) * amount;
        payable(to).transfer(_sellerFund);
        payable(mintableAddress).transfer(msg.value - _sellerFund);
    }
}
