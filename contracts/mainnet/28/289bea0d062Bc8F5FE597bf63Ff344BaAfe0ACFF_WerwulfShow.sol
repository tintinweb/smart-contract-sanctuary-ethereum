...
interface IERC1155Metadata is IERC1155 {
    ...
    function metadata() external view returns(string memory, string memory);
}
 ...
 */
contract ERC1155 is Context, ERC165Storage, IERC1155, IERC1155Metadata, IERC1155MetadataURI {
    using SafeMath for uint256;
     ...
     * @dev See {_setURI}.
     */
    constructor (string memory _name, string memory _symbol, string memory myUri) {
        name = _name;
        symbol = _symbol;
        _setURI(myUri);

        _registerInterface(type(IERC1155).interfaceId);
        _registerInterface(type(IERC1155Metadata).interfaceId);
...
contract FearNft1155 is ERC1155, AccessControl, OpenSeaCompatible, NativeMetaTransaction, ContextMixin
{
    /**
     * @dev Emitted when minting limit is set.
     */
    event MintingLimitSet(uint256 id, uint256 limit);

    bytes32 public constant MINTER_ROLE = "Minter";
    bytes32 public constant BRIDGER_ROLE = "Bridger";

    uint64 public mintingChainId;
    uint256 public minId;
    uint256 public maxId;

    mapping (uint256 => uint256) public mintedCounts;
    mapping (uint256 => uint256) public mintingLimits;

    constructor(string memory name, string memory ticker, string memory domainName, uint64 _mintingChainId, uint256 _minId, uint256 _maxId) ERC1155(name, ticker, domainName) {
        _setRoleAdmin(MINTER_ROLE, ROOT_ROLE);
        _setRoleAdmin(BRIDGER_ROLE, ROOT_ROLE);
        _initializeEIP712(domainName);
        mintingChainId = _mintingChainId;
        minId = _minId;
        maxId = _maxId;
    }
    ...
    /**
    * @dev Mints specified amount of tokens to provided address.
    ...
    * - Current chain id must be qeual to 'MINTING_CHAINID';
    * - Id must be in allowed range (if one was set);
    * - Amount to mint must not exceed the difference between minting limit and already minted amount of tokens (if limit was set).
    */
    function initialMint(address account, uint256 id, uint256 amount, bytes calldata data) external onlyRole(MINTER_ROLE) {
        require(block.chainid == mintingChainId || block.chainid == 31337, "Minting on this chain is not allowed."); // or hardhat internal (for testing)
        require(id >= minId && (id <= maxId || maxId == 0), "Id out of range");
        require(amount <= mintingLimits[id] - mintedCounts[id] || mintingLimits[id] == 0, "Amount exceeds the token minting limit.");

        mintedCounts[id] += amount;

        _mint(account, id, amount, data);
    }
    
    /**
    * @dev Sets minting limit for specific token type.
    * @param id token type to set minting limit for
    * @param limit minting limit
    * Requirements:
    * - Caller must have 'ADMIN_ROLE';
    * - Id must be in allowed range;
    * - Minting limit for this type of token must not be set before;
    * - Limit must be less or equal to amount of tokens already minted.
    */
    function setMintingLimit(uint256 id, uint256 limit) external onlyRole(ADMIN_ROLE) {
        require(id >= minId && (id <= maxId || maxId == 0), "Id out of range");
        require(mintingLimits[id] > limit || mintingLimits[id] == 0, "Minting limit for this id is already set to lower value.");
        require(mintedCounts[id] <= limit, "Cannot set the limit less than already minted.");
        mintingLimits[id] = limit;
        emit MintingLimitSet(id, limit);
    }

    /**
    * @dev Sets minting limit for all tokens.
    * @param limit minting limit
    * Requirements:
    * - Caller must have 'ADMIN_ROLE';
    * - maxId must be set;
    ...
    * - Limit must be less or equal to amount of tokens already minted.
    */
    function setGlobalMintingLimit(uint256 limit) external onlyRole(ADMIN_ROLE) {
        require(maxId > 0, "MaxId must be set");

        for (uint i = minId; i <= maxId; i++) {
            require(mintingLimits[i] > limit || mintingLimits[i] == 0, "Minting limit for this id is already set to lower value.");
            require(mintedCounts[i] <= limit, "Cannot set the limit less than already minted.");

            mintingLimits[i] = limit;
            emit MintingLimitSet(i, limit);
        }
    }
    ...
    function mint(address to, uint256 id, uint256 amount, bytes memory data) external onlyRole(BRIDGER_ROLE) {
        _mint(to, id, amount, data);
    }

    function burn(address account, uint256 id, uint256 amount) external onlyRole(BRIDGER_ROLE) {
    ...
    function isApprovedForAll(address owner, address operator) public view override(ERC1155) returns (bool) {
...
/**
 * @dev Contract of Werwulf Show (ERC-1155).
 */
contract WerwulfShow is FearNft1155
{
    constructor() FearNft1155("Werwulf Show", "WERWULFSHOW", "https://fearwolf.com", 1, 1, 160) {
    }
}