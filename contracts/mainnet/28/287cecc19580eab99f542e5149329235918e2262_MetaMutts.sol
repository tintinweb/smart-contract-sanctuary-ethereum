...
contract MetaMutts is ERC721Enumerable, Ownable {
    ...
    uint256 public constant RESERVES = 180;
    uint256 public priceInWei = 0.055 ether;
    ...
    function allowlistMint(uint256 amount, bytes32[] calldata proof) public payable {
        require(allowlistClaimed[msg.sender] + amount < allowlistLimit + 1, "Exceeds max allowlist mint amount");
        bytes32 leaf = keccak256(abi.encodePacked(msg.sender));
        require(MerkleProof.verify(proof, allowlistMerkleRoot, leaf), "Invalid Merkle Tree proof supplied.");
        require(balanceOf(msg.sender) + amount < 4, "Max Mint per wallet reached");
        require(amount * priceInWei == msg.value, "Invalid funds provided.");
        
        allowlistClaimed[_msgSender()] += amount;
    ...
    function mint(uint256 count) public payable {
        ...
        require(totalSupply + count < MAX_SUPPLY + 1, "Exceeds max supply.");
        require(count < MAX_PER_TX + 1, "Exceeds max per transaction.");
        require(balanceOf(msg.sender) + count < 4, "Max Mint per wallet reached");
