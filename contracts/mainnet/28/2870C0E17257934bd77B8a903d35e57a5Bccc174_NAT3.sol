...
──╚╝─╚═══╩╝╚═╝─╚╝─╚═══╝╚═══╝
*/
contract NAT3 is Ownable, ERC721RestrictApprove, ReentrancyGuard, MerkleProof, ERC2981, DefaultOperatorFilterer,Operable {
  //Project Settings
  ...
  uint256 public maxSupply = 198;
  uint256 public mintable = 198;
  uint256 public revealed = 198;
  ...
  function whitelistMint(uint256 _amount, bytes32[] memory proof_,string memory _passwd) external payable virtual nonReentrant {
    ...
    require(maxMintsPerWLOT >= _wlMinted[nowPhaseWl][msg.sender] + _amount, "You have no whitelistMint left");
    require(maxMintsPerWL >= _pswdMinted[_passwd] + _amount, "this account have no whitelistMint left");
