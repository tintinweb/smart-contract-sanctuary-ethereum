...
// File: contracts/biconomy-metatx-standard/lib/EIP712Base.sol
...
// File: contracts/biconomy-metatx-standard/EIP712MetaTransaction.sol
...
// File: contracts/RedeemableNFT.sol
...
contract RedeemableNFT is
    IERC721Receiver,
    EIP712MetaTransaction("RedeemableNFT", "1")
{
    /**
     * @dev Emitted when token `tokenId` in ERC721-compliant contract `tokenContract` is deposited into slot `slotId`.
     */
    event Deposit(
    ...
    /**
     * @dev Emitted when the token in `slotId` is taken out (redeemed) and transferred to `to`.
     */
    ...
    function onERC721Received(
        ...
        // Assert: tokenContract != address(0), because we should never be called by a zero address, and signing as 0x0 should not be possible.
        _requireErc721(tokenContract);
        // Parse the redeemer key from the data
        require(
            data.length == 32,
            "RedeemableNFT: Token received data must be a 32-byte public key"
        );
        // Note: abi.decode can fail and revert without providing a reason.
        address redeemerKey = abi.decode(data, (address));

        require(
            !isSlotUsed(redeemerKey),
            "RedeemableNFT: A slot with that ID/redeemer key has already been used. Generate a new key to deposit this token."
        );
        ...
        emit Deposit(redeemerKey, tokenContract, tokenId);
    ...
    function isSlotActive(address slotId) public view returns (bool) {
        ...
        return _slots[slotId].tokenContract != address(0);
    }
    ...
    function redeem(address slotId, address to) external {
        require(
            isSlotActive(slotId),
            "RedeemableNFT: Requested slot is not active."
        );
        ...
        require(
            msgSender() == slotId,
            "RedeemableNFT: This caller is not authorized to redeem this slot."
        );
        ...
        IERC721 tokenContract = IERC721(slotCopy.tokenContract);
        // safeTransferFrom can fail and revert.
        tokenContract.safeTransferFrom(address(this), to, slotCopy.tokenId);
        emit Redeem(slotId, to);
    }

    function _requireErc721(address addr) private view {
        require(
            Address.isContract(addr),
            "RedeemableNFT: Function call must come from a contract."
        );
        try IERC165(addr).supportsInterface(type(IERC721).interfaceId) returns (
            bool retval
        ) {
            require(
                retval,
                "RedeemableNFT: Token contract is not ERC721 compliant."
            );
        } catch (bytes memory reason) {
            if (reason.length == 0) {
                revert(
                    "RedeemableNFT: Token contract is not ERC165 compliant."
                );
            } else {
                assembly {
