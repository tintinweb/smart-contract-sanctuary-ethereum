...
library MerkleProof {
    ...
    function processMultiProof(
        ...
        require(leavesLen + proof.length - 1 == totalHashes, "MerkleProof: u nO mAkE cEnsE multiproof");
...
contract ENSWTF is ERC721A, Ownable, ReentrancyGuard {
    using Strings for uint256;
    ...
    string private BASE_URI = 'https://wtf.enswtf.wtf/wtf/';
    ...
    constructor() ERC721A("ENS.wtf", ".wtf") {
        tokenIDandAddress[_currentIndex]="ens";
        tokenAddressandID["ens"]=_currentIndex;
        resolveAddress["ens"]=msg.sender;
    ...
    function setSubdomainSaleActive(bool saleIsActive, uint256 customPrice, string calldata wtf_name) public {
        TokenOwnership memory Ownership = _ownershipOf(tokenAddressandID[wtf_name]);
        require(Ownership.addr == msg.sender, "u nO mAkE cEnsE");
    ...
    {   
        ...
        require(_checkName(wtf_name), "u nO mAkE cEnsE name");
    ...
    {      
            ...
            require(MerkleProof.verify(_merkleProof, merkleRoot, leaf),"u nO mAkE cEnsE proof!");
            require(bytes(wtf_name).length<=maxCharSize,"Long name");
            require(bytes(wtf_name).length>0,"Write a name");
            require(_checkName(wtf_name), "u nO mAkE cEnsE name");
    ...
    {   
        ...
        require(_checkName(wtf_name), "u nO mAkE cEnsE name");
        require(_checkName(subdomain_name), "u nO mAkE cEnsE name");
    ...
    {      
            string memory new_domain=string.concat(subdomain_name,'.',wtf_name);
            bytes32 leaf = keccak256(abi.encodePacked(msg.sender));
            require(MerkleProof.verify(_merkleProof, subDomains_allowList[wtf_name], leaf),"u nO mAkE cEnsE proof!");
...
function lastAddresses(uint256 count)
    ...
    returns (string[] memory)
  {
    ...
    uint256 ownedTokenIndex = 0;
    require(currentId>=0,"u nO mAkE cEnsE");
