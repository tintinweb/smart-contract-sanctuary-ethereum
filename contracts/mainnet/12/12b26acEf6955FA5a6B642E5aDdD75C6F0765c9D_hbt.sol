...
contract hbt is Ownable {
    ...
    constructor(address[] memory _recipients) {
       token = IERC20(0x24D86Df61479982c90D2977F4BA839496895559d);

        
        // Adding the owner as the first recipient
    ...
    function distribute() external onlyOwner {
        ...
        uint256 amount = balance / recipients.length;
        require(amount > 0, "Distribution amount is too small");
    ...
    function addRecipient(address newRecipient) external onlyOwner {
        for(uint i = 0; i < recipients.length; i++) {
            require(recipients[i] != newRecipient, "Recipient already exists");
        }
    ...
    function removeRecipient(address recipientToRemove) external onlyOwner {
        for (uint i = 0; i < recipients.length; i++) {
            if (recipients[i] == recipientToRemove) {
                ...
                recipients.pop();
                emit RecipientRemoved(recipientToRemove);
                return;
            }
        }
