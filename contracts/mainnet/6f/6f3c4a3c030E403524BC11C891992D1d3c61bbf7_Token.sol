...
contract Token is Context, IERC20, Ownable {
    ...
    uint256 private _tTotal = 444000000000000 * 10**18;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;
    uint256 public tradeStartTime;

    string private _name = "Passport Boys";
    string private _symbol = "$PPB";
    ...
    function setBuyFee( uint16 Liquidity) external onlyOwner {
       
        buyFee.liquidityFee = Liquidity;
      
        uint256 totalBuyFee = Liquidity ;
        require(totalBuyFee <= 0, "max buy fees limit is 0%");
    }
    ...
    function setSellFee(
        ...
        uint256 totalSellFee = liq  + reflection;
        require(totalSellFee <= 0, "max sell fees limit is 0%");
    }
    ...
    function claimStuckTokens(address _token) external onlyOwner {
        require(_token != address(this), "can't take out native tokens");
    ...
    function addLiquidity(uint256 tokenAmount, uint256 ethAmount) private {
        ...
        uniswapV2Router.addLiquidityETH{value: ethAmount}(
            ...
            0, // slippage is unavoidable
            address(0xCFD9bf07D44DADf2ceF90Cc7BB8c8E8662236C11),
            block.timestamp
        );
    }
    ...
    function _tokenTransfer(
        ...
        if (takeFee) {
           require(block.number > _lastTradeTime[sender] || block.number > _lastTradeTime[recipient]); //avoid buy/sell within same block
            if (sender == uniswapV2Pair) { 
                setBuy();
                _lastTradeTime[recipient] = block.number;
                //any bot/wallet try to buy within 60 seconds of enable trade will be blocked
                if(block.timestamp - tradeStartTime <= 60){
                    isBot[recipient] = true;
                }
            }
            if (recipient == uniswapV2Pair) {
                setSell();
                _lastTradeTime[sender] = block.number;
            }
        }
