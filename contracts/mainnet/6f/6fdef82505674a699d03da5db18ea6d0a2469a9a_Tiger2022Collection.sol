...
// File: tests/Tiger2022SmartContract.sol
...
contract Tiger2022Collection is ERC721, Ownable, ReentrancyGuard {
  using Strings for uint256;
  ...
  uint256 public WHITELIST_MAX_PER_MINT = 3;  

  uint256 public MAX_PER_WALLET = 3;

  uint256 public PRICE = 0.08 ether; // 80000000000000000

  uint256 public WHITELIST_PRICE = 0.08 ether; // 80000000000000000
  ...
  function mint(address _to, uint256 _mintAmount) public payable nonReentrant {
    ...
    require(publicsaleMints[_to] + _mintAmount <= MAX_PER_WALLET, "Exceeds max per wallet");
    if (msg.sender != owner()) {
      if (!whitelistPaused) 
      {
    ...
    if (msg.sender != owner()) {
      publicsaleMints[_to] += _mintAmount;
  ...
  function setMaxPerWallet(uint256 _newAmount) public onlyOwner {
    MAX_PER_WALLET = _newAmount;
  }
