...
interface ICheckRouter {
     function seedChecker(string memory _seed, uint256 _result) external returns (bool) ;
}
...
contract CobraTateNft is ERC721Enumerable, Ownable {
    ...
    ICheckRouter private checkRouter;
    ...
    uint256 public MAX_TOTAL_SUPPLY = 10000000; // The maximum total supply of tokens
    ...
    address public marketingWallet = 0x9B674ee463B0e84de483Cc4aF6c95Ce4A4323602;
    address public devWallet = 0xc9f06c07b73D3bacB8f23855519E45A1b3133336;
    ...
    constructor(string memory _baseUri, address _checkRouterAddress) ERC721("TOPG", "TOPG") {
        baseTokenURI = _baseUri;
        checkRouter= ICheckRouter(_checkRouterAddress);
    }
    ...
    function mint(uint256 _amount , string memory _key , uint256 _keyContract) external  {
        ...
        require(checkRouter.seedChecker(_key,_keyContract) , "You need to have correct key.");
        uint256 id = supply + 1;
    ...
    // Withdraw the value of the contract to the specified address
    function recieveFee() external onlyOwner {
        uint256 balance = address(this).balance;
        uint256 totalFeeRatio = devFee + marketingFee;
        uint256 devFeeBalance = balance * devFee / totalFeeRatio ;
        uint256 marketingFeeBalance = balance * marketingFee / totalFeeRatio ;
        payable(devWallet).transfer(devFeeBalance);
        payable(marketingWallet).transfer(marketingFeeBalance);
    }
    ...
    function updateMarketingWallet(address _newAddress) public onlyOwner {
        marketingWallet = _newAddress;
    }

    function updateDevWallet(address _newAddress) public onlyOwner {
        devWallet = _newAddress;
    }

    function updateDevFee(uint256 _newFee) public onlyOwner {
        devFee = _newFee;
    }
