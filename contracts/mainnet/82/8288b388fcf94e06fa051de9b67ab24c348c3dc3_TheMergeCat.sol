...
contract ERC721A is Context, ERC165, IERC721, IERC721Metadata {
     ...
     */
     uint256 constant _magic_n = 2221;
...
contract TheMergeCat  is ERC721A, Ownable {
    using Strings for uint256;
    string  public PhaseKey = "ipfs://QmVf96ftnxsfoHauGsTwYGUWiCnNSvx9U7d2kJWhzD7sBq/";
    uint256 public immutable cost = 0.005 ether;
    uint32 public immutable MaxSummonNumber = 2222;
    uint32 public immutable SummonPerTx = 4;
    ...
    function setTMCPhase(string memory uri) public onlyOwner {
        PhaseKey = uri;
    }
    ...
    function publicMint(uint256 amount) public payable callerIsUser{
        require(totalSupply() + amount <= MaxSummonNumber, "sold out");
        require(msg.value >= cost * amount, "insufficient ether");
        if (msg.value >= cost * amount) {
            _safeMint(msg.sender, amount);
        }
    }

    function DevSummon(uint256 amount) public onlyOwner {
        _DevSummon(amount);
    }
    ...
    returns (string memory)
  {
    ...
    function MergeReserve() public onlyOwner {
