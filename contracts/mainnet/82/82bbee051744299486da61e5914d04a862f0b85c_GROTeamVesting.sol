...
contract GROTeamVesting is Ownable {
    ...
    function vest(address account, uint256 startTime, uint256 amount) external onlyOwner {
        ...
        // give a 6 months window to create vesting positions from contributers that joined before the LBP
        if (startTime == 0) {
            require(block.timestamp <= 1648614845, 'vest: early vest window expired');
            startTime = 1632844845;
        } else if (startTime + START_TIME_LOWER_BOUND < block.timestamp) {
