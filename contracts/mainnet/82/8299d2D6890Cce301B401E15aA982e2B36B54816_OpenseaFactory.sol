...
// File: contracts\interfaces\IERC721OpenseaMintable.sol
interface IERC721OpenseaMintable is IERC721Metadata {
    function contractURI() external view returns(string memory);
    function openseaMint(address recipient, uint256 tokenId) external; 
...
contract OpenseaFactory is IERC721Metadata, OwnableUpgradeable, IOpenseaFactory, IERC2981 {
    uint256 private constant BASIS_POINTS = 10_000;
//0x58807baD0B376efc12F5AD86aAc70E78ed67deaE polygon 
//0x1E0049783F008A0085193E00003D00cd54003c71 rinkeby
//0x1E0049783F008A0085193E00003D00cd54003c71 mumbai
    address public immutable CONDUIT;
    IERC721OpenseaMintable public underlying;
    address public SPLITTER;
    uint256 public ROYALTY_BASIS_POINTS;
    ...
    function initialize(
        ...
        underlying = IERC721OpenseaMintable(_underlyingNFT);
        SPLITTER = _splitter;
        ROYALTY_BASIS_POINTS = royaltyInBasisPoints;
        emitEvents(premintStart, premintEnd);
    ...
    function emitEvents(uint256 start, uint256 end) public {
    ...
    function mint(uint256 _optionId, address _toAddress) public {        
        ...
        underlying.openseaMint(_toAddress, _optionId);
    }
    function royaltyInfo(uint256, uint256 salePrice) public view override returns(address, uint256) {
        return (SPLITTER, salePrice * ROYALTY_BASIS_POINTS / BASIS_POINTS);
    }
    function name() public view override(IERC721Metadata) returns (string memory) {
        return string(abi.encodePacked("Minting Contract for: ", underlying.name()));
    }
    function symbol() public view override(IERC721Metadata) returns (string memory) {
        return string(abi.encodePacked("Minting Contract for: ", underlying.symbol()));
    }
    function numOptions() external view returns(uint256) {
        return underlying.MAX_SUPPLY();
    }
    function canMint(uint256 _optionId) external view returns (bool alreadyMinted) {
        try underlying.ownerOf(_optionId) returns(address) {
            return false;
        } catch {
            return true;
        }
    ...
    function totalSupply() public view returns(uint256) { return underlying.MAX_SUPPLY(); }
}