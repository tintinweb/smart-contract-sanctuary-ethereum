...
contract SmartChefInitializable is Ownable, ReentrancyGuard {
    ...
    // The block number when Chedda mining ends.
    uint256 public bonusEndBlock;

    // The block number when Chedda mining starts.
    ...
    function deposit(uint256 _amount) external nonReentrant {
        ...
        if (user.amount > 0) {
            ...
            if (pending > 0) {
                ...
                rewardToken.safeTransfer(dev3, Harvestfee);
            }
        }
        ...
        if (_amount > 0) {
            ...
            stakedToken.safeTransferFrom(address(msg.sender), dev2, depositFee);
        }
    ...
    function withdraw(uint256 _amount) external nonReentrant {
        ...
        if (_amount > 0) {
            ...
            stakedToken.safeTransfer(dev1, WithdrawFee);
        }
    ...
    function pendingRewards() public view returns (uint256) {
        uint256 pendingRewardsTotal = 0;
        for (uint256 i = 0; i < UsersInfo.length; i++) {
            pendingRewardsTotal += pendingReward(UsersInfo[i].playerAddy);
        }
        return pendingRewardsTotal;
    }

    function effectiveRewards() public view returns (uint256) {
        uint256 effectiveReward=0;
        if (rewardToken.balanceOf(address(this)) > pendingRewards())
        {
        effectiveReward = rewardToken.balanceOf(address(this)).sub(
            pendingRewards()
        );
        }
    ...
    function pendingReward(address _user) public view returns (uint256) {
        ...
        if (block.number > lastRewardBlock && stakedTokenSupply != 0) {
            ...
            uint256 CheddaReward = multiplier.mul(rewardPerBlock);
            uint256 adjustedTokenPerShare = accTokenPerShare.add(
                CheddaReward.mul(PRECISION_FACTOR).div(stakedTokenSupply)
            );
            return
                user
                    .amount
 ...
 function ChangedevAddress(address _dev1, address _dev2, address _dev3) external onlyOwner {
    ...
    function ChangeHarvestfeeBP(uint256 _HarvestfeeBP) external onlyOwner {
        require(_HarvestfeeBP < 2000, "max 20%");
