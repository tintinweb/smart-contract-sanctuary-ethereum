...
// File: contracts/blackmarket.sol
...
contract BlackMarket is ERC721, Ownable {
    ...
    constructor(string memory initUnrevealedUriPrefix)
        ERC721("BlackMarket", "BM")
    {
        paused = true;
        revealed = false;
    ...
    function mint(uint256 amount) external payable {
        ...
        uint256 maxMintAmount = whitelisted
            ? maxMintAmountPerAddressForWL
            : maxMintAmountPerAddress;

        require(
            amount > 0 && amount <= maxMintAmountPerTX,
            "BlackMarket: invalid mint amount"
        );
        require(
            getMintsOfAddress(_msgSender(), getCurrentPhase()) + amount <=
                maxMintAmount,
            "BlackMarket: exceeded max mints for this phase"
        );
        require(
            _supply.current() + amount <= maxSupply,
            "BlackMarket: max token supply exceeded"
        );
        require(!paused, "BlackMarket: minting is paused");

        require(
            msg.value >= (whitelisted ? presaleCost : cost) * amount,
            "BlackMarket: insufficient ether"
        );
    ...
    function airDrop(address[] calldata addresses, uint8[] calldata amounts)
        external
        onlyOwner
    {
        require(
            addresses.length == amounts.length,
            "BlackMarket: invalid function arguments"
        );
        require(
            addresses.length > 0,
            "BlackMarket: invalid function arguments"
    ...
    function withdraw() external onlyOwner {
        (bool success, ) = payable(owner()).call{
            value: address(this).balance - vault()
        }("");
        require(success);
    }
    ...
    function distributeDividends() public payable onlyOwner {
        ...
        require(
            supply > 13,
            "BlackMarket: total token supply need to be above 120"
        );
    ...
    function distributeDividendsToSpecifiedAddresses(
        address[] calldata addresses
    ) public payable onlyOwner {
        require(
            addresses.length > 0,
            "BlackMarket: invalid number of addresses"
        );
    ...
    function withdrawDividend() external {
        ...
        if (withdrawableDividend > 0) {
            ...
            (bool success, ) = payable(_msgSender()).call{
                value: withdrawableDividend
            }("");
            require(
                success,
                "BlackMarket: failed to transfer withdrawable dividend"
            );
    ...
    function getMintsOfAddress(address account, uint256 phaseNumber)
        public
        view
        returns (uint256)
    {
    ...
    function setUnrevealedURIprefix(string memory newUnrevealedURI)
        external
        onlyOwner
    {
    ...
    function setMaxMintAmountPerAddressForWL(uint256 newAmount)
        external
        onlyOwner
    {
    ...
    function setWhitelistedAddresses(address[] calldata addresses)
        external
        onlyOwner
    {
