...
contract FreakyLabs is ERC721A, Ownable {
    ...
    function freakyLabExecMint(bytes32 [] calldata _merkleProof, uint _quantity) external payable {
        require(tx.origin == msg.sender);
        require(private_sale_running || public_sale_running, "Sale is not running");
        require(_quantity <= 2, "Invalid quantity queued for minting");
        ...
        if (_quantity == 1) {
            require(totalSupply() + 1 <= MAX_SUPPLY, "Not enough tokens left to mint");
        } else {
    ...
    function updateMerkleRoot(bytes32 _new_root) external onlyOwner {
