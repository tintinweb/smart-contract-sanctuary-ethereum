...
// File: MetaPeopleMinter.sol


pragma solidity ^0.8.4;


contract MetaPeopleMinter {

    address public owner;
    uint256 public basePrice;
    address public metaPeopleContractAddress;
    bool private haltContract = false;

    //Each wallet can hold onlu upto a max of 10 MetaPeople NFTs
    mapping(address => uint256) public metaPeopleMinters;
    ...
    function updateMetaPeopleContractAddress(address _address) public onlyOwner {
        metaPeopleContractAddress = _address;
    }

    function toggleSaleType(uint8 _saleType) public onlyOwner {
        if (_saleType == 0) {
            if(haltContract == false) {
                haltContract = true;
            }
            if(haltContract == true) {
                haltContract = false;
            }
        }
        if (_saleType == 1) {
            basePrice = 60000000000000000;
        }
        if (_saleType == 2) {
    ...
    function mintNft(address _toAddress, string memory _uri) public payable {
        require(haltContract == false);
        
        if(msg.sender != owner) {
            require (metaPeopleMinters[_toAddress] < 10);
        ...
        MetaPeople meta_people = MetaPeople(metaPeopleContractAddress);
        meta_people.mintNFTWithRoyalty(_toAddress, _uri, owner, 250);

        metaPeopleMinters[_toAddress] += 1;
    }
}