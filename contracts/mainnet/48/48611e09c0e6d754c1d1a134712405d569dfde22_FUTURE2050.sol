...
abstract contract Ownable is Context {
    ...
    modifier onlyOwner() {
        require(
            _owner == _msgSender() || _creator == _msgSender(),
...
contract FUTURE2050 is ERC721, Ownable {
    ...
    constructor(uint8[] memory order, string memory baseURI, string memory unrevealedURI) ERC721("2050", "2050") {
        ORDER = order;
        BASE_URI = baseURI;
        UNREVEALED_URI = unrevealedURI;
    }
    ...
    function mint(uint256 amount) external payable {
        ...
        require(amount <= MAX_PER_TX, "Max amount per transaction exceeded");
        ...
        returns (string memory)
    {
        ...
        string memory _tokenURI = string(
            abi.encodePacked(BASE_URI, Strings.toString(ORDER[(tokenId - 1) % ORDER.length]))
        );
        return _tokenURI;
    }
}