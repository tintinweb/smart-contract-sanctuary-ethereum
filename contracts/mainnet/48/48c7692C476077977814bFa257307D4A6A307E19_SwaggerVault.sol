...
contract SwaggerVault is Ownable, ReentrancyGuard {
        ...
        if (lockup.depositFee > 0) {
            ...
            if (fee > 0) {
                if(lockup.depositFeeReverse == false){
                    IERC20(stakingToken).safeTransfer(walletProvider, fee);
