...
contract EarthFundBonding is Ownable {
    ...
    struct BondPair {
        ...
        uint256 active; // Bool to see if bonds are accepting deposits
        uint256 currentPrice; // Current price of the bond
        uint256 minimumPrice; // Minimum price of the bond
        uint256 decimals; // Number of decimals to get to the smallest denomination of the base currency
    ...
    uint256 public activeBonds; //  Number of active bonds 

    mapping(address => mapping(uint256 => Bond)) public bondInfo; // stores bond information for depositors
    mapping(string => address) public erc20TokensAddresses; // stores ERC20 token addresses for additional bonds
    ...
    function bondPrice(uint256 _bondIndex) public view returns (uint256) {
        ...
        // Time in blocks since last purchase
        uint256 blocksElapsed = block.number - bond.lastDecay; // 55

        if(blocksElapsed <= bond.buffer) {
            return bond.currentPrice;
        }

        uint256 blockDelta = blocksElapsed.sub(bond.buffer);
        uint256 discount = bond.decrementRate * blockDelta;
        uint256 maxDiscount = bond.currentPrice.sub(bond.minimumPrice);

        // If the total discount is greater than the current price, then
        // we need to set it to the minimum price
        if(discount >= maxDiscount) {
            return bond.minimumPrice;
        }

        return bond.currentPrice.sub(discount);
    }
    ...
    function deposit(
        ...
        // Make sure that user is only creating a new bond
        require(bondInfo[msg.sender][_bondPairIndex].payout == 0, "Bonding: You must fully vest your previous bond before starting another");

        BondPair storage currentBondPair = bondPairs[_bondPairIndex];
        require(currentBondPair.active == 1, "Bonding: Bond is not active");
        ...
        require(payout <= maxPayout(_bondPairIndex), "Bond too large"); // size protection because there is no slippage

        // If the bonding index is 0, then it's ETH. If not, then it is a token pair.
        address bondTokenAddress = erc20TokensAddresses[currentBondPair.name];
        if (bondTokenAddress == address(0)) {
            ...
            // Token is an ERC20
            require(
                IERC20(bondTokenAddress).balanceOf(msg.sender) >=
                    _amount.div(10**currentBondPair.decimals).mul(
                        currentBondPrice
                    ),
                "Bonding: Not enough sent for bond"
            ...
            IERC20(bondTokenAddress).safeTransferFrom(
                msg.sender,
                treasury,
                _amount
            );
        ...
        bondInfo[msg.sender][_bondPairIndex] = Bond({
            payout: bondInfo[msg.sender][_bondPairIndex].payout.add(payout),
        ...
        uint256 currentPrice = currentBondPair.currentPrice;
        uint256 newBondPrice = currentPrice.add(currentBondPair.incrementRate);
        currentBondPair.currentPrice = newBondPrice;
     ...
     *  @return uint
     */
    function redeem(uint256 _bondPairIndex)
        external
        returns (bool)
    {
        // require(bondInfo[msg.sender].payout)
        Bond memory userBond = bondInfo[msg.sender][_bondPairIndex];
        uint256 percentVested = percentVestedFor(msg.sender, _bondPairIndex); // (blocks since last interaction / vesting term remaining)

        if (percentVested >= 10000) {
            // if fully vested
            delete bondInfo[msg.sender][_bondPairIndex]; // delete user info
            activeBonds--;
            
            emit BondRedeemed(msg.sender, userBond.payout, 0); // emit bond data
            return IERC20(earthFundToken).transfer(msg.sender, userBond.payout); // send payout
        } else {
            // if unfinished
            ...
            emit BondRedeemed(
                msg.sender,
                payout,
                bondInfo[msg.sender][_bondPairIndex].payout
            );
            return IERC20(earthFundToken).transfer(msg.sender, payout); // send payout
        }
    }
    ...
    function setBondTerms(
        ...
        BondPair storage bond = bondPairs[_bondPairIndex];

        if (_parameter == PARAMETER.VESTING) {
            // 0
            require(_input >= 5760, "Vesting must be longer than 24 hours");
            ...
            // 4
            bond.buffer = _input;
        } else if (_parameter == PARAMETER.ACTIVE) {
            // 5
            bond.active = _input;
        }
    }
        ...
        returns (uint256)
    {
        ...
        uint256 tokensForPayout = _value.div(price); 

        return tokensForPayout.mul(10 ** 18);
    }

    /**
        Withdraw 1EARTH tokens from contract when needed via owner
    **/
    function emergencyWithdraw() public onlyOwner {
        uint256 bondingBalance = IERC20(earthFundToken).balanceOf(address(this));
        IERC20(earthFundToken).transfer(msg.sender, bondingBalance); // send payout
    }
}