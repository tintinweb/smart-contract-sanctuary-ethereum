    ...
    ISeaDropTokenContractMetadata
{
    ...
    function tokenURI(uint tokenId) public view virtual override(ERC721A) returns (string memory) {
        ...
        return string(abi.encodePacked(_baseURI(), _toString(tokenId), _uriSuffix));
    }

    function setUnrevealedURI(string calldata unrevealedURI) external {
        require(msg.sender == owner(), "Only owner can set unrevealed URI");
        _unrevealedURI = unrevealedURI;
    }

    function setRevealStatus(bool status) external {
        require(msg.sender == owner(), "Only owner can set reveal status");
        metadataRevealed = status;
    }
    function setURISuffix(string calldata uriSuffix) external {
        require(msg.sender == owner(), "Only owner can set URI suffix");
    ...
    DefaultOperatorFilterer
{
    ...
    constructor(
        ...
        require(accounts.length == amounts.length, "ERC721PartnerSeaDrop: airdrop: accounts.length != amounts.length");
        unchecked{
        uint _totalSupply =  _totalMinted();
        for (uint i = 0; i < accounts.length; i++) {
            if(_totalSupply + amounts[i] > maxSupply()) revert MintQuantityExceedsMaxSupply(_totalSupply + amounts[i], maxSupply());
    ...
    // function setRoyalty(uint newRoyalty) external onlyOwner{
    //     ERC2981._setDefaultRoyalty(msg.sender, newRoyalty);
