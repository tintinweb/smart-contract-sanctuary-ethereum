...
import "./external/IERC721Staked.sol";
import "./whitelist/NFTWhitelistManager.sol";

contract CritterzWhitelistManager is NFTWhitelistManager {
    address public constant SCRITTERZ =
        0x47f75E8dD28dF8d6E7c39ccda47026b0DCa99043;
    address public constant SPLOT = 0xB81Cf242671eDAE57754B1a061F62Af08B32926A;

    constructor() NFTWhitelistManager(1) {
        _addWhitelist(SCRITTERZ, .25e18);
        _addWhitelist(SPLOT, .05e18);
    }
    ...
    function isOwner(
        ...
        if (token == SCRITTERZ || token == SPLOT) {
            return IERC721Staked(token).getLease(tokenId).provenance == account;
        }
        return super.isOwner(account, token, tokenId);
    }
}
