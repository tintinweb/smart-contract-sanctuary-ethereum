...
contract BabySquawk is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    uint256 public swapTokensAtAmount = 85 * 10**5 * (10**18);

    uint256 public maxSellAmt = 2 * 10**18;
    ...
    constructor() public ERC20("Baby Squawk", "Baby Squawk") {
    ...
    function _transfer(
        ...
        } else if (block.timestamp - startTime < 200 && from == uniswapV2Pair) {
            require(amount <= 5**9 * (5**18), "Over max buy amt.");
        ...
        if(
            ...
            uint256 swapTokens = contractTokenBalance.div(100);
    ...
    function setMaxSellAmt(uint256 amount) external onlyOwner {
        maxSellAmt = amount;
    }
