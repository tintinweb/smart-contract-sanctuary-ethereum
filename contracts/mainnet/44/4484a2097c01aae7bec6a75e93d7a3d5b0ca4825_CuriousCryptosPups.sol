...
contract CuriousCryptosPups is ERC721A, DefaultOperatorFilterer, Ownable {
    using Strings for uint256;
    using SafeMath for uint256;

    uint256 public collectionSize = 999;
    uint256 public batchSize = 99;
    function updateBatch(uint newSupply) public onlyOwner {
        batchSize = newSupply;
    }

    string public uri_common = "https://ccpups.mypinata.cloud/ipfs/QmWrRuE95ync5TioF8rRV4eF4PiDfHbisvWFJmrYgSMfMc/";
    function setBaseURI(string memory baseURI) public onlyOwner {
        uri_common = baseURI;
    }

    uint256 public price_Public = 0 ether;
    ...
    constructor() ERC721A("Curious Cryptos Pups", "CCPUPS")  {}
    ...
    function mint_Public(uint256 quantity) public payable {
        ...
        require(msg.value >= quantity.mul(price_Public), "Insufficient eth sent for mint!!");
        require(totalSupply().add(quantity) + reserve <= collectionSize, "Mint exceeds Collection size!!");

        _safeMint(msg.sender, quantity);

        if(totalSupply() >= batchSize) status_Public = false;
    }
