...
contract BondDepositoryCRV is Ownable {
    ...
    address public immutable BIG; // token given as payment for bond
    address public immutable principle; // token used to create bond
    address public immutable treasury; // mints BIG when receives principle
    ...
    struct Bond {
        uint payout; // BIG remaining to be paid
    ...
    constructor ( 
        ...
        DAO = _DAO;
        priceFeed = AggregatorV3Interface(0xCd627aA160A6fA45Eb793D19Ef54f5062F20f33f);
    }
    ...
    function initializeBondTerms( 
        ...
        require( _vestingTerm >= 300, "Vesting must be longer than 5 minutes" );
    ...
    function setBondTerms ( PARAMETER _parameter, uint _input ) external onlyPolicy() {
        if ( _parameter == PARAMETER.VESTING ) { // 0
            ...
            // if min bond price = 1000 $ => minimumPrice = 1000.00 = > 100000
    ...
    function deposit( 
        ...
        require( payout >= minPayout, "Bond too small" ); // must be > 0.01 BIG ( underflow protection )
        ...
        /**
            ...
            approved and
            deposited into the treasury, returning (_amount - profit) BIG
         */
        ...
        if ( fee != 0 ) { // fee is transferred to dao 
            IERC20(BIG).safeTransfer( DAO, fee );
        }
    ...
    function stakeOrSend( address _recipient, bool _stake, uint _amount ) internal returns ( uint ) {
        if ( !_stake ) { // if user does not want to stake
            IERC20( BIG ).transfer( _recipient, _amount ); // send payout
        } else { // if user wants to stake
            if ( useHelper ) { // use if staking warmup is 0
                IERC20( BIG ).approve( stakingHelper, _amount );
                IStakingHelper( stakingHelper ).stake( _amount, _recipient );
            } else {
                IERC20( BIG ).approve( staking, _amount );
    ...
    function maxPayout() public view returns ( uint ) {
        return IERC20( BIG ).totalSupply().mul( terms.maxPayout ).div( 100000 );
    }
    ...
    function payoutFor( uint _value ) public view returns ( uint ) {
        return FixedPoint.fraction( _value, bondPrice() ).decode112with18().div( 10 ** (18 - scaling) );
    }
    ...
    function bondPrice() public view returns ( uint price_ ) { 
        price_ = terms.controlVariable.mul(debtRatio()).add(1000000000) / 10 ** (9 - scaling);
    ...
    function bondPriceInUSD() public view returns ( uint price_ ) {
        price_ = bondPrice().mul( uint( assetPrice() ) ).mul(10 ** (10 - scaling));
    }


    /**
     *  @notice calculate current ratio of debt to BIG supply
     *  @return debtRatio_ uint
     */
    function debtRatio() public view returns ( uint debtRatio_ ) {   
        uint supply = IERC20( BIG ).totalSupply();
    ...
    /**
     *  @notice calculate amount of BIG available for claim by depositor
     *  @param _depositor address
    ...
    /**
     *  @notice allow anyone to send lost tokens (excluding principle or BIG) to the DAO
     *  @return bool
     */
