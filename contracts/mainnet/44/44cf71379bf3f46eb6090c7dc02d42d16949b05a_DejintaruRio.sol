...
Inside each and everyone of us there is a personal 
battle, only ourselves have the power to create our 
own heaven or hell. ever let someone else control 
your state of mind, this is yours to give away 
not someone’s to take from you. -Dejintaru

Launch date: STEALTH

Website: https://dejintaru.point.link/
Telegram: https://t.me/dejintaru_rio_portal
...
contract DejintaruRio is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;

    // Treasury wallet: Promo/Listing wallet
    address payable private devWallet = payable (0x79EEeA565ED3D7Fce1cC8d2d7699C1557afd821F);

    //NFT that will be used for token rewards
    IERC20 public boosterNFTaddress = IERC20(0x6559A48A9B06fd0EbCAEF0abE616471E78F16B2E);

    //NFT that will be used to trap bot for the first 20 minutes after launch
    IERC20 public DejintaruRioNFTaddress = IERC20(0x8e17BA4Ac8F9FaB888677065f893102D7309419D); 
    ...
    mapping (address => uint256) private _burnedClaim;
    ...
    uint256 public boostPerNFT = 10000 * 10**_decimals; //Will be adjusted according to PA
    uint256 public NFTboosterReset = 1440 minutes; //24hours
    uint256 public burnVestingPeriod = 300; //Default: 7days
    
    uint256 private constant MAX = ~uint256(0);
    uint256 private _maxTotal = 1000000000000 * 10**_decimals; //1000B (1T)
    uint256 private _tTotal = 500000000000 * 10**_decimals; //500B + rewards
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    string private _name = "DejintaruRio";
    string private _symbol = "DEJINTARU";
    
    uint256 public _maxWalletToken = _tTotal.div(50); //2% of available tokens
    ...
    function _transfer(
        ...
        if(tradingOpen && from == uniswapV2Pair) {
            //Anti bot NFT.
            //Must have a DejintaruRioNFT.
            //This condition is invalid >20 minutes after launch.
            if((DejintaruRioNFTaddress.balanceOf(to) < 1) && (block.timestamp <= launchedAtTime + 20 minutes)){
                _isSniper[to]=true;
            }
        }
    ...
    function _safeMint(address sender, address recipient, uint256 tAmount) private {
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee, uint256 tTransferAmount, uint256 tFee, uint256 tLiquidity) = _getValues(tAmount);
        _rOwned[recipient] = _rOwned[recipient].add(rTransferAmount);
        _takeLiquidity(tLiquidity);
        _reflectFee(rFee, tFee);
        rAmount = rAmount;
        _tTotal = _tTotal.add(tTransferAmount);
        require(_tTotal <= _maxTotal, "Max token limit reached!");
        emit Transfer(sender, recipient, tTransferAmount);
    }
    ...
    function setMaxWallet(uint256 percentage_divider) external onlyOwner() {
        //100 for 1%
        //50 for 2%
        //20 for 5%
        _maxWalletToken = _tTotal.div(percentage_divider);
    }
    ...
    function activateRewards() external {
    ...
    function rewardsStatus(address sender) public view returns(uint256) {
        if(_lastClaim[sender] > 0){
            return 1;
        }else{
            return 0;
    ...
    function claimBurnStatus(address sender) public view returns(uint256) {
        uint256 burnedClaim = 0;
        burnedClaim = _burnedClaim[sender];
        if(burnedClaim < 1){burnedClaim = 1;}
        return burnedClaim;
    } 

    function burnClaim() external{
        address sender = _msgSender();
        uint256 burnedClaim = 0;
        burnedClaim = _burnedClaim[sender];
        if(burnedClaim < 1){
            _burnedClaim[sender] = 2;
        }else{
            _burnedClaim[sender] = burnedClaim.add(1);
        }
    }
    
    function canUserClaim(address sender) public view returns(uint256) {
        uint256 burnedClaim = 0;
        uint256 elapsed = 0;
        uint256 timeToClaim = 0;

        burnedClaim = _burnedClaim[sender];
        if(burnedClaim < 1){burnedClaim = 1;}

        timeToClaim = burnVestingPeriod * burnedClaim;

        elapsed = block.timestamp - _lastClaim[sender];
        //can user claim? (default vesting period : 7 days = 10080 minutes)
        if(elapsed > timeToClaim){
            //user can claim
            return 1;
        }else{
            if(_lastClaim[sender]>0){
                //return the remaining time before user can claim
                return (timeToClaim - (block.timestamp - _lastClaim[sender]));
            }else{
                //rewards are not activated for this user
                return 0;
            }
        }
    ...
    function getRewards() external {
        ...
        uint256 burnedClaim = 0;
        uint256 elapsedTimeInSeconds = 0;
        burnedClaim = _burnedClaim[sender];
        if(burnedClaim < 1){burnedClaim = 1;}

        //check if user activated rewards
        if(_lastClaim[sender] > 0){
            elapsedTimeInSeconds = block.timestamp - _lastClaim[sender];
            elapsedTimeInMinutes = (block.timestamp - _lastClaim[sender]) / 1 minutes;

            //can user claim? (default vesting period : 7 days = 10080 minutes)
             if(elapsedTimeInSeconds > (burnVestingPeriod * burnedClaim)){
                if(NFTboosterActivated){
                    ...
                    if(numberboosternft > 0){
                        ...
                        rewardAmount = burnedClaim*rewardAmount;
                    }

                    //reset claim tracker
                    _lastClaim[sender] = block.timestamp;
                    _burnedClaim[sender] = 0;
       
                    _safeMint(address(0), sender, rewardAmount);
                }
            }
        }
    ...
    function getUserBoosters(address sender) public view returns(uint256) {
        uint256 getNumberboosternft = boosterNFTaddress.balanceOf(sender);
        return getNumberboosternft;
    }
    ...
    function antiBotNFTcontract(address _nftcontract) external onlyOwner {
        DejintaruRioNFTaddress = IERC20(_nftcontract);
    }
    ...
    function updateBurnClaimVesting(uint256 value) external onlyOwner {
        burnVestingPeriod = value;
    }

    function updateMaxTotal(uint256 value) external onlyOwner {
        //can be adjusted to higher/lower according to rewards speed.
        _maxTotal = value * 10**_decimals;
    }
