 ...
 */
contract DavidKassmanNFT is ERC721A, Ownable, DefaultOperatorFilterer {
    ...
    // @dev Final max supply 
    //uint public finalMaxSupply = 251;

    // @dev Price token in ether
    uint256 public price = 0.43 ether;
    ...
    modifier onlyOwnerOrPartner() {
        require(msg.sender == owner() || msg.sender == partner, "Not authorized");
        _;
    }

    function setPartner(address payable _partner) public onlyOwnerOrPartner {
        partner = _partner;
    }

    constructor(address payable team, address payable _partner) ERC721A("DavidKassmanNFT", "DK") {
        _team = team;
        partner = _partner;
        _mint(team, 1);
    }
     ...
     */
    function setPaused(bool value) public onlyOwnerOrPartner {
        paused = value;
    }
     ...
     */
    function setPrice(uint256 newPrice) external onlyOwnerOrPartner {
        price = newPrice;
    }

    function setMaxByWallet(uint newValue) external onlyOwnerOrPartner {
    ...
    /**
    * @dev Change the `maxSupply` of the token for `newMaxSupply`
    */
    function setMaxSupply(uint newMaxSupply) external onlyOwnerOrPartner {
        maxSupply = newMaxSupply;
    }
    
    /**
     * @dev Withdraw funds to the `partner` wallet
     */
