...
/**
 * @title SimpleUniRecipeETH contract for BaoFinance's Baskets Protocol (PieDAO fork)
 *
 * @author vex
 * @author fabiaz84
 */
contract SimpleUniRecipeETH is Ownable {
    ...
    IWETH constant ETH = IWETH(0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2);
    ...
    function bake(
        ...
        WETH.safeTransferFrom(msg.sender, address(this), _maxInput);
        ...
        if (remainingInputBalance > 0) {
            WETH.safeTransfer(msg.sender, remainingInputBalance);
        }
    ...
    function toBasket(
        ...
        uint256 wethBalance = ETH.balanceOf(address(this));
        if (wethBalance > 0) {
            inputAmountUsed = msg.value - wethBalance;

            ETH.withdraw(wethBalance);
            ETH.transfer(msg.sender, wethBalance);
        }
