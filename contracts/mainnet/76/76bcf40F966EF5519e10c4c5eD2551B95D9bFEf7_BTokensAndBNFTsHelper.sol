...
import {MetapolyUpgradeableProxy} from "../libraries/proxy/MetapolyUpgradeableProxy.sol";
...
contract LendPoolAddressesProvider is Ownable, ILendPoolAddressesProvider {
  ...
  bytes32 private constant METAPOLY_ORACLE = "METAPOLY_ORACLE";
  ...
  bytes32 private constant METAPOLY_DATA_PROVIDER = "METAPOLY_DATA_PROVIDER";
  ...
  /**
   * @dev Returns the id of the market to which this contracts points to
  ...
  function getProtocolDataProvider() external view override returns (address) {
    return getAddress(METAPOLY_DATA_PROVIDER);
  }

  function setProtocolDataProvider(address provider) external override onlyOwner {
    _addresses[METAPOLY_DATA_PROVIDER] = provider;
  ...
  function getImplementation(address proxyAddress) external view onlyOwner returns (address) {
    MetapolyUpgradeableProxy proxy = MetapolyUpgradeableProxy(payable(proxyAddress));
  ...
  function _updateImpl(bytes32 id, address newAddress) internal {
    ...
    if (proxyAddress == address(0)) {
      ...
      MetapolyUpgradeableProxy proxy = new MetapolyUpgradeableProxy(newAddress, address(this), params);

      _addresses[id] = address(proxy);
      emit ProxyCreated(id, address(proxy));
    } else {
      // upgrade implementation
      MetapolyUpgradeableProxy proxy = MetapolyUpgradeableProxy(proxyAddress);
...
contract LendPoolConfigurator is Initializable, ILendPoolConfigurator {
        ...
        internal
    {
        ...
        DataTypes.ReserveConfigurationMap memory currentConfig = pool_
    ...
    function _initNft(
        ...
        (address bNftProxy, ) = registry_.getBNFTAddresses(
            input.underlyingAsset
        );
     ...
     */
    function setLimits(uint256 _blockBetweenDepositWithdraw, uint256 _borrowRepayInterval) 
        external 
        onlyPoolAdmin
    {
        ILendPool cachedPool = _getLendPool();
        cachedPool.setLimits(_blockBetweenDepositWithdraw, _borrowRepayInterval);
    }

    function setNftRedeemThreshold(address asset, uint256 redeemThreshold)
        external
        onlyPoolAdmin
    {
        ILendPool cachedPool = _getLendPool();
    ...
    /**
     * @param _asset NFT address
     * @param _auctionInitialPerc Percentage of debt while starting the auction. Max
     * - Debt. (15000 is 150 % of debt)
     */
    function setAuctionInitialPerc(address _asset, uint16 _auctionInitialPerc)
        external
        onlyPoolAdmin
    {
        ILendPool cachedPool = _getLendPool();
        DataTypes.NftConfigurationMap memory currentConfig = cachedPool
            .getNftConfiguration(_asset);
        currentConfig.setMaxAuctionPerc(_auctionInitialPerc);
        cachedPool.setNftConfiguration(_asset, currentConfig.data);
    }
    ...
    function getTokenImplementation(address proxyAddress)
        ...
        returns (address)
    {
        MetapolyUpgradeableProxy proxy = MetapolyUpgradeableProxy(
    ...
    function _initTokenWithProxy(
        ...
        MetapolyUpgradeableProxy proxy = new MetapolyUpgradeableProxy(
            implementation,
            address(this),
            initParams
        );
...
contract MetapolyUpgradeableProxy is TransparentUpgradeableProxy {
  constructor(
    address _logic,
    address admin_,
    bytes memory _data
