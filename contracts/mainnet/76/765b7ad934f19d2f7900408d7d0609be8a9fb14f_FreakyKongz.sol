...
contract FreakyKongz is ERC721A, Ownable {
    using Strings for uint256;
    ...
    constructor(string memory _initBaseURI, string memory _initNotRevealedURI)
        ERC721A("FreakyKongz", "FREAKYKONGZ")
    {
    ...
    function mint(uint256 _amount) external payable {
        require(!paused, "FreakyKongz - The Contract is Paused");
        require(
            tokenCount + _amount <= maxSupply,
            "FreakyKongz - max NFT limit exceeded"
        );
        if (msg.sender != owner()) {
            require(
                _amount <= maxMintAmount,
                "FreakyKongz - max mint amount limit exceeded"
            );
            uint256 ownerMintedCount = addressMintedBalance[msg.sender];
            require(
                ownerMintedCount + _amount <= maxMintAmount,
                "FreakyKongz - max NFT per address exceeded"
            );
            require(
                msg.value >= cost * _amount,
                "FreakyKongz - insufficient ethers"
            );
        }
