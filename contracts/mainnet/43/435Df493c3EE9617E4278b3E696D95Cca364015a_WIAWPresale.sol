...
contract WIAWPresale is Ownable {
    ...
    address public wallet = payable(0x9e041125F9390aba0DE1AAdf3F61fd38163eBD30);

    // How many token units a buyer gets per ETH
    uint256 private rate = 10000000000;
    ...
    //Presale status
    bool private hasPresaleStarted = false;
    ...
    function buyTokens() public payable {
        ...
        Customer[msg.sender].totalAmount += tokens;
        tokensSold += tokens;
    ...
    function withdraw(uint amount) external onlyOwner {
        ...
        require(block.timestamp >= startTime + 38 days);
    ...
    require(hasPresaleStarted,"Presale not started");
    require(block.timestamp >= startTime, "Presale is ongoing");
    require(Customer[msg.sender].tokensBought > 0, "No tokens to be claimed");

    uint256 valueToBeClaimed;


    if (Customer[msg.sender].weekOne == false) {
        Customer[msg.sender].weekOne = true;
        valueToBeClaimed += Customer[msg.sender].totalAmount.mul(20).div(100);
    }
    

    if (block.timestamp >= startTime + 7 days){
        if (Customer[msg.sender].weekTwo == false)  {
            Customer[msg.sender].weekTwo = true;
            valueToBeClaimed += Customer[msg.sender].totalAmount.mul(20).div(100);
        }
    }

    if (block.timestamp >= startTime + 14 days) {
        if (Customer[msg.sender].weekThree == false) {
            Customer[msg.sender].weekThree = true;
            valueToBeClaimed += Customer[msg.sender].totalAmount.mul(20).div(100);
        }
    }

    if (block.timestamp >= startTime + 21 days){
        if  (Customer[msg.sender].weekFour == false){
            Customer[msg.sender].weekFour = true;
            valueToBeClaimed += Customer[msg.sender].totalAmount.mul(20).div(100);
        }
    }

    if (block.timestamp >= startTime + 28 days) {
        if (Customer[msg.sender].weekFive == false) {
            Customer[msg.sender].weekFive = true;
            valueToBeClaimed += Customer[msg.sender].totalAmount.mul(20).div(100);
        }
    }

    Customer[msg.sender].tokensBought -= valueToBeClaimed;
    token.transfer(msg.sender, valueToBeClaimed);
}
