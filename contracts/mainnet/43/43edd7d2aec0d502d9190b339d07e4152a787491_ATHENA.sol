...
/**
     ...
     █▌██▀   █▀█╥╫       ██████    ██    █▌    █▌  █▌     █▌  ▀██  ██████   █╫▌
      ...
      █▌                                                                    █╫▌
      █                                                                     █╫▌
      𓂀                     ATHENA - VIRTUES OF JUSTICE                    𓂀    
      █                                                                     █╫▌   
      █                  Teh one who deserves to survive is                 █╫▌
      █             teh one who has teh will to make it happen.             █╫▌
      █     Teh people shall make teh community and they shall see light    █╫▌           
      █                                                                     █╫▌
      █                                4% Tax                               █╫▌                       
      █                                                                     █╫▌                    
      █               ALL TAXES WILL GO TO THE LIQUIDITY POOLS              █╫▌                                    
      █                                                                     █╫▌
      █                                                             ▄  ▐ ╓ ,,
      █                                                            └x,X▄█╝  Γ
      █                                                               "█╨^└
...
contract ATHENA is Context,IERC20, Ownable{
    using Address for address;

    string private _name = "VIRTUES OF JUSTICE";
    string private _symbol = "ATHENA";
    ...
    uint256 private _totalSupply = 369369369369 * 10**_decimals; 
    ...
    address _owner;
    address payable public marketingAddress = payable(0xaBaF05AA74550EFFD72c719a1E04e4A914D0EbeA);
    ...
    function currentsellNOSfee() public view returns (uint256){
            return sellNOS;
    }
    function currentsellPOOfee() public view returns (uint256){
            return sellPOO;
    }
    ...
    function _transfer(address from, address to, uint256 amount) internal{
        ...
        if(inContractBalance >=requiredTokensToSwap && 
            ...
            uint256 POOAmount = 0; // Amount to be added to prize pool.
            ...
            if(takeFees){
                ...
                if (automatedMarketMakerPairs[to] && to != address(_router) ){
                        ...
                        totalFeeFortx = mktAmount + liqAmount + NOSAmount + POOAmount;
                }
                //Buy Fees
            ...
            _balances[to] += amount - mktAmount - NOSAmount - POOAmount - liqAmount;
    ...
    function updateNOS(address newToken)private{
        NOS = newToken;
    }
    function processNOS(uint256 ethAmount, address rewardToken) internal {
      ...
      function SwapForPOO(uint256 amount)private {
        address[] memory path = new address[](2);
		path[0] = address(this);
		path[1] = _router.WETH();
		_approve(address(this), address(_router), amount);
    ...
    function updatePOO(address newToken)private{
        POO = newToken;
    }
    function processPOO(uint256 ethAmount, address rewardToken) internal {
        // generate the uniswap pair path of weth -> eth
        address[] memory path = new address[](2);
        path[0] = _router.WETH();
        path[1] = rewardToken;
