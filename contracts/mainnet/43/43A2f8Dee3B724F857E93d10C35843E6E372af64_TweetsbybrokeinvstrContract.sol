...
//
//     __               __        _                 __      
//    / /_  _________  / /_____  (_)___ _   _______/ /______
//   / __ \/ ___/ __ \/ //_/ _ \/ / __ \ | / / ___/ __/ ___/
//  / /_/ / /  / /_/ / ,< /  __/ / / / / |/ (__  ) /_/ /    
// /_.___/_/   \____/_/|_|\___/_/_/ /_/|___/____/\__/_/     
//                                                          
//
//*********************************************************************//
//*********************************************************************//
...
abstract contract Withdrawable is Teams {
  address[] public payableAddresses = [0xe1dA9E3ea9eFc074EbFfD4D2bed209b370705188];
...
abstract contract Closeable is Teams {
  ...
  // @title An implementation of ERC-1155 with additonal context for 1:1 redemption with another ERC-1155
  ...
  abstract contract ERC1155Redemption is ERC1155, Teams {
      // @dev Emitted when someone exchanges an NFT for this contracts NFT via token redemption swap or verified Claim
      event Redemption(string indexed redemptionMethod, address indexed from, uint256 indexed tokenIdMinted, address redemptionContractUsed, uint256 foreignTokenId);

      struct RedemptionDetail {
          mapping(address => uint256) redemptionSurcharge;
          bool redemptionModeEnabled;
          bool verifiedClaimModeEnabled;
          address redemptionAddress;
          mapping(address => mapping(uint256 => bool)) redemptionContracts; // contractAddress: {ftokenId: boolean}
          mapping(address => mapping(uint256 => mapping(address => uint256))) tokenRedemptions;
          // tokenRedemptions are only enforced during verifiedClaim calls to prevent double-dips on the same token balance.
          // [foreignContractAddress] {
          //   [foreignTokenId]: {
          //      [userAddress]: [redeemedBalance] // Check this balance is never greater than foreign balanceOf(useraddress, fTokenId)
          //   }
          // }
      }

      mapping(uint256 => RedemptionDetail) internal redemptionDetail;

      function bulkSetRedemptionDetails(
          uint256 tokenId, 
          uint256 fTokenId, 
          uint256 _surchargeInWei,
          bool _redemptionModeEnabled,
          bool _verifiedClaimModeEnabled,
          address _redemptionAddress,
          address _redemptionContract
      ) public onlyTeamOrOwner {
          if(_redemptionAddress == address(0)) revert CannotBeNullAddress();
          if(_redemptionModeEnabled && _verifiedClaimModeEnabled) revert CannotEnableBothRedeemModes();

          RedemptionDetail storage details = redemptionDetail[tokenId];
          details.redemptionSurcharge[_redemptionContract] = _surchargeInWei;
          details.redemptionModeEnabled = _redemptionModeEnabled;
          details.verifiedClaimModeEnabled = _verifiedClaimModeEnabled;
          details.redemptionAddress = _redemptionAddress;
          details.redemptionContracts[_redemptionContract][fTokenId] = true;
        }

      function redemptionStatsForContract(uint256 tokenId, address _contractAddress, uint256 ftokenId) public view returns (uint256, bool, bool, bool, address) {
          RedemptionDetail storage details = redemptionDetail[tokenId];
          return (
              details.redemptionSurcharge[_contractAddress],
              details.redemptionModeEnabled,
              details.verifiedClaimModeEnabled,
              details.redemptionContracts[_contractAddress][ftokenId],
              details.redemptionAddress
          );
      }

      function redemptionBalanceForToken(uint256 tokenId, address _contractAddress, address _userAddress, uint256 ftokenId) public view returns (uint256) {
        return redemptionDetail[tokenId].tokenRedemptions[_contractAddress][ftokenId][_userAddress];
      }

      // @dev Allow owner/team to set the contract:token as eligable for redemption for this contract
      function setRedeemableContract(uint256 tokenId, address _contractAddress, bool _status, uint256 fTokenId) public onlyTeamOrOwner {
        redemptionDetail[tokenId].redemptionContracts[_contractAddress][fTokenId] = _status;
      }
  }
  
// File: contracts/TweetsbybrokeinvstrContract.sol
//SPDX-License-Identifier: MIT

pragma solidity ^0.8.2;

contract TweetsbybrokeinvstrContract is 
    ERC1155,
    Ownable,
    Teams,
    Pausable, 
    ...
    Allowlist
{
    ...
    string public name = "Tweets by brokeinvstr";
    string public symbol = "BITWT";
  ...
  */
  function mintToMultiple(address _to, uint256 _id, uint256 _qty) public payable whenNotPaused {
    ...
    * @param redemptionContract foreignContractAddress to be checked against
    * @param ftokenId the foreign token to be redeemed.
    * Emits a {Redemption} event.
    **/
    function redeem(uint256 tokenId, address redemptionContract, uint256 ftokenId) public payable {
        if (!exists(tokenId)) revert TokenDoesNotExist();
        if (!withinSupplyCap(tokenId, 1)) revert CapExceeded();
        if (redemptionContract == address(0)) revert CannotBeNullAddress();
        if (!canMintAmount(tokenId, _msgSender(), 1)) revert ExceededMints();

        RedemptionDetail storage details = redemptionDetail[tokenId];
        if(!details.redemptionModeEnabled) revert ClaimModeDisabled();
        if(msg.value != details.redemptionSurcharge[redemptionContract]) revert InvalidPayment();
        if(!details.redemptionContracts[redemptionContract][ftokenId]) revert IneligibleRedemptionContract();

        // When doing a burn mint there is no reason to do a redemption balance check
        // because if they own the tokens, they can mint and it will be removed from their wallet unless
        // they are also the burn recipient (user config issue).
        IERC1155 _targetContract = IERC1155(redemptionContract);
        uint256 currentBalance = _targetContract.balanceOf(_msgSender(), ftokenId);
        if(currentBalance == 0) revert InsufficentTokenBalance();
        if(!_targetContract.isApprovedForAll(_msgSender(), address(this))) revert InvalidApprovalForRedemption();

        // Warning: Since there is no standarized return value for transfers of ERC-1155
        ...
        // movement of the token. As an added measure we keep a mapping of tokens redemption balances to help prevent multiple single-token redemptions,
        // but the NFT may not have been sent to the redemptionAddress.
        _targetContract.safeTransferFrom(_msgSender(), details.redemptionAddress, ftokenId, 1, emptyBytes);

        emit Redemption("redeem", _msgSender(), tokenId, redemptionContract, ftokenId);
        addTokenMints(tokenId, _msgSender(), 1);
        _mint(_msgSender(), tokenId, 1, emptyBytes);
    }

    /**
    * @dev allows for verified claim mint against a tokenID. Must be owned by the owner
    ...
    * Emits a {Redemption} event.
    **/
    function verifedClaim(uint256 tokenId, address redemptionContract, uint256 ftokenId) public payable {
        if (!exists(tokenId)) revert TokenDoesNotExist();
        if (!withinSupplyCap(tokenId, 1)) revert CapExceeded();
        if (redemptionContract == address(0)) revert CannotBeNullAddress();
        if (!canMintAmount(tokenId, _msgSender(), 1)) revert ExceededMints();
        ...
        RedemptionDetail storage details = redemptionDetail[tokenId];
        if(!details.verifiedClaimModeEnabled) revert ClaimModeDisabled();
        if(msg.value != details.redemptionSurcharge[redemptionContract]) revert InvalidPayment();
        if(!details.redemptionContracts[redemptionContract][ftokenId]) revert IneligibleRedemptionContract();

        // When doing a verified mint we need to keep a tab of how many redemptions the buyer
        // has executed since the token never leaves their wallet and therefore balance remains unchanged.
        // To prevent double-claims we keep a log of what they have claimed already against each token verified.
        IERC1155 _targetContract = IERC1155(redemptionContract);
        uint256 currentBalance = _targetContract.balanceOf(_msgSender(), ftokenId);
        uint256 redeemedBalance = details.tokenRedemptions[redemptionContract][ftokenId][_msgSender()];
        if(currentBalance == 0) revert InsufficentTokenBalance();
        if(redeemedBalance >= currentBalance) revert TokenRedemptionBalanceExhausted();
        
        redemptionDetail[tokenId].tokenRedemptions[redemptionContract][ftokenId][_msgSender()] = redeemedBalance + 1;
        
        emit Redemption("verifedClaim", _msgSender(), tokenId, redemptionContract, ftokenId);
        addTokenMints(tokenId, _msgSender(), 1);
        _mint(_msgSender(), tokenId, 1, emptyBytes);
    }
  ...
  function contractURI() public pure returns (string memory) {
    return "https://metadata.mintplex.xyz/jFJlkOl6huBydIYBQDFY/contract-metadata";
  }
