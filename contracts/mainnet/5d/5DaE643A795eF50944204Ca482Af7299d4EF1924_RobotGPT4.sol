...
abstract contract DefaultOperatorFilterer is OperatorFilterer {
    ...
    contract RobotGPT4 is ERC721A, DefaultOperatorFilterer , Ownable {
    using Strings for uint256;
  ...
  constructor() ERC721A("Robot GPT4", "RGPT4") {
  }


  
 
  function mintGPT4(uint8 _mintAmount) external payable  {
  ...
  function GPT4Treasury(uint16 _mintAmount, address _receiver) external onlyOwner {
     uint16 totalSupply = uint16(totalSupply());
    require(totalSupply + _mintAmount <= maxSupply, "Exceeds max supply.");
  ...
  function  AirdropGPT4(uint8 _amountPerAddress, address[] calldata addresses) external onlyOwner {
     uint16 totalSupply = uint16(totalSupply());
...
if ( reveal == false)
{
 ...
 function setFreeMaxLimit(uint8 _limit) external onlyOwner{
  ...
  function toggleGPT4Mint() external onlyOwner {
    paused = !paused;
