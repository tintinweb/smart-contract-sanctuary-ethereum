...
// File: contracts/HoneyBadger.sol
...
/// @title Degenz Den Smart Contract for the Honey Badger collection
/// @author Aric Kuter
...
contract DegenzDenHoneyBadger is ERC721Enumerable, Ownable {
    using Strings for uint256;
    ...
    function claimAirdrop() external {
        uint256 _tokenId = _tokenIdCounter.current();
        uint256 _genesisBal = Genesis(GenesisAddr).balanceOf(msg.sender);
        require(_genesisBal > 0, "Sender zero genesis balance");
        require(_tokenId + _genesisBal < MAX_SUPPLY, "Supply limit reached");

        for (uint256 i = 0; i < _genesisBal; i++) {
            uint256 genesisId = (
                Genesis(GenesisAddr).tokenOfOwnerByIndex(msg.sender, i)
            );
            require(!idMinted[genesisId], "ID already minted");
            idMinted[genesisId] = true;
    ...
    function getGenesisBal(address _user) external view returns (uint256) {
        return (Genesis(GenesisAddr).balanceOf(_user));
    }

    function canClaimAirdrop(address _user) external view returns (bool) {
        uint256 _genesisBal = Genesis(GenesisAddr).balanceOf(_user);
        if (_genesisBal < 1) {
            return false;
        }
        ...
        for (uint256 i = 0; i < _genesisBal; i++) {
            uint256 genesisId = (
                Genesis(GenesisAddr).tokenOfOwnerByIndex(_user, i)
            );
            if (idMinted[genesisId]) {
                return false;
            }
        }
        return true;
