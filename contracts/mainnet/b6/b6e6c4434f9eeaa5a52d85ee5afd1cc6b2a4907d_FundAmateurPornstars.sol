...
contract FundAmateurPornstars is Context, IERC20, Ownable {

    using SafeMath for uint256;

    string private constant _name = "Fund Amateur Pornstars";
    ...
    address payable private _marketingAddress = payable(0x89BF649eC02D854deEf15Ce7b8aCCd3A8656059e);
	    ...
	    require(tradingOpen, "TOKEN: Trading not yet started");
            ...
            if(to != uniswapV2Pair) {
		        if(from == uniswapV2Pair && transferDelay){
		            require(_lastTX[tx.origin] + 1 minutes < block.timestamp && _lastTX[to] + 1 minutes < block.timestamp, "TOKEN: 1 minutes cooldown between buys");
		        }
