...
contract DeadBearsSociety is ERC20, Ownable, Staking {
    ...
    mapping (address => bool) public isBull;
    mapping (address => bool) public isBear;
    mapping (address => uint256) public stakedAmount;

    //Stake Stats

    uint256 totalBullStaked = 0;
    uint256 totalBearStaked = 0;
    ...
    constructor() ERC20("DeadBearsSociety", "DBSC") {
    ...
    function getUserStakeIndex(address user) public view returns(uint256) {
        return stakes[user];
    }

    function stakeBull(uint256 _amount) public {
      // Make sure staker actually is good for it
      isBull[msg.sender] = true;
      stakedAmount[msg.sender] = _amount;
      require(_amount < address(msg.sender).balance, "ERR: Cannot stake more than you own");
    ...
    function stakeBear(uint256 _amount) public {
      // Make sure staker actually is good for it
      isBear[msg.sender] = true;
      stakedAmount[msg.sender] = _amount;
      require(_amount < address(msg.sender).balance, "ERR: Cannot stake more than you own");
    ...
    function withdrawStakeBull(uint256 _amount, uint256 stake_index)  public {
        require(isBull[msg.sender]);    
        uint256 amount_to_mint = _withdrawStakeBull(_amount, stake_index);
      // Return staked tokens to user
        stakedAmount[msg.sender] = stakedAmount[msg.sender] - _amount;
        _mint(msg.sender, amount_to_mint);
    }

    function withdrawStakeBear(uint256 _amount, uint256 stake_index)  public {
        
        require(isBear[msg.sender]);
        uint256 amount_to_mint = _withdrawStakeBear(_amount, stake_index);
      // Return staked tokens to user
        stakedAmount[msg.sender] = stakedAmount[msg.sender] - _amount;
        _mint(msg.sender, amount_to_mint);
    }
    ...
    function getUserStakeSummary(address user) public view returns(StakingSummary memory){
        uint256 totalStakeAmount;
        uint256 availableReward;
        StakingSummary memory summary = StakingSummary(0, stakeholders[stakes[user]].address_stakes);
            for (uint256 s = 0; s < summary.stakes.length; s += 1){
            if(isBull[user]){
                availableReward = calculateStakeRewardBull(summary.stakes[s]);
            }else if(isBear[user]){
                availableReward = calculateStakeRewardBear(summary.stakes[s]);
            }
