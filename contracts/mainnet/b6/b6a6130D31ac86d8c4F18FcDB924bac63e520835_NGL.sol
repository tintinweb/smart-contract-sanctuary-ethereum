...
contract NGL is ERC20, Ownable {
    ...
    uint256 public buyNGLFee;
    uint256 public buyLiquidityFee;
    uint256 public buyTreasuryFee;

    uint256 private originalSellNGLFee;
    uint256 private originalSellLiquidityFee;
    uint256 private originalSellTreasuryFee;

    uint256 public sellTotalFees;
    uint256 public sellNGLFee;
    uint256 public sellLiquidityFee;
    uint256 public sellTreasuryFee;

    uint256 public tokensForTheNGL;
    ...
    event UpdatedTheNGLAddress(address indexed newWallet);
    ...
    constructor() payable ERC20("Not Gonna Lie", "NGL") {
        ...
        maxBuyAmount = (totalSupply * 10) / 1000; // 1%
        maxSellAmount = (totalSupply * 10) / 1000; // 1%
        ...
        buyLiquidityFee = 2;
        buyTreasuryFee = 0;
        buyTotalFees = buyNGLFee + buyLiquidityFee + buyTreasuryFee;

        originalSellNGLFee = 3;
        ...
        sellLiquidityFee = 2;
        sellTreasuryFee = 0;
        sellTotalFees = sellNGLFee + sellLiquidityFee + sellTreasuryFee;

        theNGLAddress = address(0xc1AB1560809e1e4D11f435de06F68e51f95A11a3);
        treasuryAddress = address(0xc1AB1560809e1e4D11f435de06F68e51f95A11a3);

        _excludeFromMaxTransaction(newOwner, true);
        _excludeFromMaxTransaction(address(this), true);
        _excludeFromMaxTransaction(address(0xdead), true);
        _excludeFromMaxTransaction(address(theNGLAddress), true);
        _excludeFromMaxTransaction(address(treasuryAddress), true);

        excludeFromFees(newOwner, true);
        excludeFromFees(address(this), true);
        excludeFromFees(address(0xdead), true);
        excludeFromFees(address(theNGLAddress), true);
    ...
    function _transfer(
        ...
        if (takeFee) {
            ...
            if (
                ...
                tokensForTheNGL += (fees * buyNGLFee) / buyTotalFees;
            ...
            // on sell
                ...
                tokensForTheNGL += (fees * sellNGLFee) / sellTotalFees;
    ...
    function swapBack() private {
        ...
        uint256 ethForTheNGL = (ethBalance * tokensForTheNGL) /
            (totalTokensToSwap - (tokensForLiquidity / 2));

        ethForLiquidity -= ethForTheNGL;
        ...
        (success, ) = address(theNGLAddress).call{
    ...
    function setTheNGLAddress(address _theNGLAddress) external onlyOwner {
        require(
            _theNGLAddress != address(0),
            "_theNGLAddress address cannot be 0"
        );
        theNGLAddress = payable(_theNGLAddress);
        emit UpdatedTheNGLAddress(_theNGLAddress);
    }
