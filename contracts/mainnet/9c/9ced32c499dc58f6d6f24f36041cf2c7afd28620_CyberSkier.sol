...
contract CyberSkier is ERC721A, ReentrancyGuard, Ownable {
    ...
    function retainMint(uint256 quantity) external onlyOwner {
        require(quantity <= MAX_RETAIN, "Exceeded maximum retain");
        uint256 supply = totalSupply();
        require(supply + quantity <= MAX_SUPPLY, "Exceeded maximum supply");

        _safeMint(msg.sender, quantity);
        MAX_RETAIN = MAX_RETAIN - quantity;
    }
