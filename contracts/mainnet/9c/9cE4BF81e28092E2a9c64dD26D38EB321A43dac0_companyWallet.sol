...
abstract contract FounderWallets is globals {
    ...
    function _createProposal(bytes32 symbol, address pFrom, address tgts, uint256 vals, bytes32 desc) 
    internal 
    virtual  {
        if(!proposalVotes[lastPID + 1].exists) {
        } 
        else require(1 == 0);
        proposalVotes[++lastPID] = FounderProposal(0, 0,
         desc, fps.Queued, symbol, pFrom, tgts, vals, true);
