...
contract FIFAUSDC is IERC20, OWNED {
	mapping(address => uint256) private _balances;
	mapping(address => mapping(address => uint256)) private _allowances;
	uint8 private constant _decimals = 9;
	uint256 private constant _totalSupply = 100_000_000 * 10**_decimals;
	string private constant _name = "FIFA USDC";
	...
	address private _treasuryWallet = address(0x9E411a4A1AcEDa03862E4d033dAcd64Dd1720195);
	...
	constructor() OWNED(msg.sender)  {
		...
		_changeLimits(15,30); //set max TX to 1.5%, max wallet 3%
	...
	function addInitialLiquidity(uint256 val) external onlyOwner {
		...
		_taxRateBuy = 10;
		_taxRateSell = 25; //anti-dump sell tax at launch
	...
	function changeTaxRates(uint8 newTaxRateBuy, uint8 newTaxRateSell) external onlyOwner {
		require( (newTaxRateBuy+newTaxRateSell) <= 25, "Max roundtrip is 25%" );
