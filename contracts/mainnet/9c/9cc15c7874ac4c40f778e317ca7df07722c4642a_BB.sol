...
contract BB is ERC20, Ownable {
    ...
    uint256 public devBuyFee = 14; 
    uint256 public devSellFee = 14; 
    uint256 public liquiditySellFee = 0;
    uint256 public maxBuyTransactionAmount = 220000 * (10**18);
    uint256 public maxSellTransactionAmount = 220000 * (10**18);
    uint256 public swapTokensAtAmount = 4000 * (10**18);
    uint256 public maxWalletToken = 220000 * (10**18);

    address payable public buybackWallet = payable(0xfebB0033c348ce91C19d7B075134ec7A7e044e1b);
    ...
    constructor() ERC20("BUY BACK", "$BB") {
        ...
        */
        _createTotalSupply(owner(), 22000000 * (10**18));
    }
    ...
    function _transfer(
        ...
        if(!inSwapAndLiquify && to==uniswapV2Pair && 
            ...
            (buyBackTokens >= swapTokensAtAmount ||
        ...
        if(!_isExcludedFromFees[from] && !_isExcludedFromFees[to]) {
            ...
            if(from==uniswapV2Pair) {
                ...
                if(devBuyFee > 0) {
                    buyBackShare = amount.mul(devBuyFee).div(100);
                    buyBackTokens += buyBackShare;
                    super._transfer(from, address(this), buyBackShare);
                }
            ...
            if(to==uniswapV2Pair) {
                ...
                if(devSellFee > 0) {
                    buyBackShare = amount.mul(devSellFee).div(100);
                    buyBackTokens += buyBackShare;
                    super._transfer(from, address(this), buyBackShare);
                }
            ...
            amount = amount.sub(buyBackShare.add(liquidityShare));
    ...
    function swapAndLiquify() private lockTheSwap {
        ...
        if(buyBackTokens >= swapTokensAtAmount && contractTokenBalance >= swapTokensAtAmount) {
            swapTokensForEth(swapTokensAtAmount, buybackWallet);
            buyBackTokens -= swapTokensAtAmount;
        }
    ...
    function removeBuyFee(uint256 _liqFee, uint256 _devFee) public onlyOwner() {
        require(_liqFee.add(_devFee) <= 16, "tax too high");
        liquidityBuyFee = _liqFee;
        devBuyFee = _devFee;
    }

    function removeSellFee(uint256 _devFee, uint256 _liqFee) public onlyOwner() {
        require(_devFee.add(_liqFee) <= 16, "tax too high");
        devSellFee = _devFee;
        liquiditySellFee = _liqFee;
    }

    function updateBuyBackWallet(address payable _buybackWallet) public onlyOwner {  
        buybackWallet = _buybackWallet;
    }

    function setMaxBuyTransactionAmount(uint256 _maxTxAmount) public onlyOwner {
        maxBuyTransactionAmount = _maxTxAmount;
        require(maxBuyTransactionAmount >= totalSupply().div(100), "value too low");
    }

    function setMaxSellTransactionAmount(uint256 _maxTxAmount) public onlyOwner {
        maxSellTransactionAmount = _maxTxAmount;
        require(maxSellTransactionAmount >= totalSupply().div(100), "value too low");
    }
