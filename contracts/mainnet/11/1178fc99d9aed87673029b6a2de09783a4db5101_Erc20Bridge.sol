 ...
 */
contract Erc20Bridge is OwnableUpgradeable {
    ...
    event FeeRatioChanged(address tokenAddress, uint256 oldFeeRatio, uint256 newFeeRatio);
    ...
    mapping(address => uint256) feeRatio;
    ...
    uint8 public constant TOKEN_LOCK_TYPE_TRANSFER_BASE_TOKEN = 3;
    ...
    function __Erc20Bridge_init(address _relayer) public initializer {
        __Ownable_init();
    ...
    function setFeeCollector(address _feeCollector) external onlyOwner {
        require(_feeCollector != address(0), "Erc20Bridge: zero address");
        address oldFeeCollector;
        feeCollector = _feeCollector;
        emit FeeCollectorChanged(oldFeeCollector, feeCollector);
    }

    function setFeeRatio(address _tokenAddress, uint256 _feeRatio) external onlyOwner {
        require(_tokenAddress != address(0), "Erc20Bridge: zero address");
        require(0 <= _feeRatio&& _feeRatio <= 10000, "Erc20Bridge: Illegal value range");
        uint256 oldFeeRatio = feeRatio[_tokenAddress];
        feeRatio[_tokenAddress] = _feeRatio;
        emit FeeRatioChanged(_tokenAddress, oldFeeRatio, feeRatio[_tokenAddress]);
    }
    ...
    function addToken(
        ...
        require(tokenInfos[tokenKey].tokenAddress == address(0), "Erc20Bridge: token already exists");
        require(
            lockType == TOKEN_LOCK_TYPE_TRANSFER || lockType == TOKEN_LOCK_TYPE_MINT_BURN || lockType == TOKEN_LOCK_TYPE_TRANSFER_BASE_TOKEN,
            "Erc20Bridge: unknown token lock type"
        );
        if(lockType != TOKEN_LOCK_TYPE_TRANSFER_BASE_TOKEN){
            require(tokenAddress != address(0), "Erc20Bridge: zero address");
        }
    ...
    function removeToken(bytes32 tokenKey) external onlyOwner {
        require(tokenInfos[tokenKey].tokenAddress != address(0), "Erc20Bridge: token does not exists");
    ...
    function addChainSupportForToken(bytes32 tokenKey, uint256 chainId) external onlyOwner {
        require(!tokenSupportedOnChain[tokenKey][chainId], "Erc20Bridge: already supported");
    ...
    function dropChainSupportForToken(bytes32 tokenKey, uint256 chainId) external onlyOwner {
        require(tokenSupportedOnChain[tokenKey][chainId], "Erc20Bridge: not supported");
    ...
    function deposit(
        ...
        require(tokenInfo.tokenAddress != address(0), "Erc20Bridge: token not found");

        require(amount > 0, "Erc20Bridge: amount must be positive");
        require(destChainId != currentChainId, "Erc20Bridge: dest must be different from src");
        require(isTokenSupportedOnChain(token, destChainId), "Erc20Bridge: token not supported on chain");
        require(recipient != 0, "Erc20Bridge: zero address");

        depositCount = depositCount + 1;
        uint256 feeAmount = amount.div(10000).mul(feeRatio[tokenInfo.tokenAddress]);

        if (feeAmount > 0 ) {
            TransferHelper.safeTransferFrom(tokenInfo.tokenAddress, msg.sender, feeCollector, feeAmount);
        }
        if (tokenInfo.lockType == TOKEN_LOCK_TYPE_TRANSFER) {
            TransferHelper.safeTransferFrom(tokenInfo.tokenAddress, msg.sender, address(this), amount.sub(feeAmount));

        } else if (tokenInfo.lockType == TOKEN_LOCK_TYPE_MINT_BURN) {
            IMintBurnToken(tokenInfo.tokenAddress).burn(msg.sender, amount.sub(feeAmount));
        } else {
            require(false, "Erc20Bridge: unknown token lock type");
        }
    ...
    function withdraw(
        ...
        require(destChainId == currentChainId, "Erc20Bridge: wrong chain");
        require(!withdrawnDeposits[srcChainId][depositId], "Erc20Bridge: already withdrawn");
        require(recipient != 0, "Erc20Bridge: zero address");
        require(amount > 0, "Erc20Bridge: amount must be positive");

        TokenInfo memory tokenInfo = tokenInfos[currency];
        if (tokenInfo.lockType != TOKEN_LOCK_TYPE_TRANSFER_BASE_TOKEN){
            require(tokenInfo.tokenAddress != address(0), "Erc20Bridge: token not found");
        }
        ...
        require(recoveredAddress == relayer, "Erc20Bridge: invalid signature");

        withdrawnDeposits[srcChainId][depositId] = true;

        address payable decodedRecipient = address(uint160(uint256(recipient)));
        ...
        } else if (tokenInfo.lockType == TOKEN_LOCK_TYPE_TRANSFER_BASE_TOKEN) {
            decodedRecipient.transfer(amount);
        } else {
            require(false, "Erc20Bridge: unknown token lock type");
        }
    ...
    function _setRelayer(address _relayer) private {
        require(_relayer != address(0), "Erc20Bridge: zero address");
        require(_relayer != relayer, "Erc20Bridge: relayer not changed");
