...
contract MappingPASS is ERC721Royalty, ERC721Enumerable, Ownable {
    ...
    enum IMAGE_TYPE{ PASS, AVATAR, MAPPING }
    ...
    string            private _mappingURI;
    string            private _avatarURI;

    mapping(uint256 => IMAGE_TYPE) private _image;
    ...
    event SetMapping(uint256 indexed tokenId, address indexed targetNFT, uint256 targetTokenId, address indexed account);
    ...
    function read(address account_) public view returns (
        ...
        baseURIs[0] = _passURI;
        baseURIs[1] = _avatarURI;
        baseURIs[2] = _mappingURI;
        baseURIs[3] = _contractURI;
    ...
    function readERC721(address nft_) public view returns (
        string memory name,
        string memory symbol
    ) {
    ...
    function getMapping(uint256 tokenId_) public view returns (
        address nft,
        uint256 tokenId,
        address owner
    ) {
    ...
    function tokenURI(uint256 tokenId_) public view override(ERC721) returns (string memory) {
        // mapping
        if (_image[tokenId_] == IMAGE_TYPE.MAPPING) {
            Mapping memory m = _mapping[tokenId_];
            if (m.nft != address(0)) {
                IERC721Metadata NFT = IERC721Metadata(m.nft);
                if (m.owner == NFT.ownerOf(m.tokenId) && 0 < bytes(_mappingURI).length) {
                    return string(abi.encodePacked(_mappingURI, tokenId_.toString()));
                }
            }
        }

        // avatar
        if (IMAGE_TYPE.AVATAR == _image[tokenId_] && 0 < bytes(_avatarURI).length) {
            return string(abi.encodePacked(_avatarURI, tokenId_.toString()));
        }
    ...
    function infoOf(uint256 tokenId_) public view returns (
        IMAGE_TYPE imageType,
        uint256 sinceBlockNumber,
        uint256 sinceTimestamp
    ) {
        imageType = _image[tokenId_];
        sinceBlockNumber = _sinceBlock[tokenId_];
        sinceTimestamp = _sinceTimestamp[tokenId_];
    }
    ...
    function setMapping(uint256 tokenId_, address targetNFT_, uint256 targetTokenId_) external payable {
        ...
        require(NFT.ownerOf(targetTokenId_) == account, "Target NFT: no permission");

        emit SetMapping(tokenId_, targetNFT_, targetTokenId_, account);
        _image[tokenId_] = IMAGE_TYPE.MAPPING;
    ...
    function setURIs(string memory pass_, string memory avatar_, string memory mapping_, string memory contract_) external onlyOwner {
        if (0 < bytes(pass_).length)     _passURI = pass_;
        if (0 < bytes(avatar_).length)   _avatarURI = avatar_;
        if (0 < bytes(mapping_).length)  _mappingURI = mapping_;
        if (0 < bytes(contract_).length) _contractURI = contract_;
    }

    function setPassURI(string memory uri_) external onlyOwner {
        _passURI = uri_;
    }

    function setAvatarURI(string memory uri_) external onlyOwner {
        _avatarURI = uri_;
    }

    function setMappingURI(string memory uri_) external onlyOwner {
        _mappingURI = uri_;
    }
    ...
    function setImageAsPASS(uint256 tokenId_) external payable onlyTokenOwner(tokenId_) {
        _image[tokenId_] = IMAGE_TYPE.PASS;
    }

    function setImageAsAvatar(uint256 tokenId_) external payable onlyTokenOwner(tokenId_) {
        _image[tokenId_] = IMAGE_TYPE.AVATAR;
    }

    function setImageAsMapping(uint256 tokenId_) external payable onlyTokenOwner(tokenId_) {
        _image[tokenId_] = IMAGE_TYPE.MAPPING;
    }
