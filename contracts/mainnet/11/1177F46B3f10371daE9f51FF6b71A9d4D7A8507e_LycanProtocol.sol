...
contract LycanProtocol is ERC721A, Ownable, ReentrancyGuard {
	...
	function _check_YourALStatus() external view returns (bool) {
		return hasALNFT(msg.sender);
	}
	...
	function _changeFWBAddress(address _FWB_ADDRESS) external onlyOwner {FWB_ADDRESS = _FWB_ADDRESS;}
	function _changeIDAddress(address _ID_ADDRESS) external onlyOwner {ID_ADDRESS = _ID_ADDRESS;}
	function _changeDENZAAddress(address _DENZA_ADDRESS) external onlyOwner {DENZA_ADDRESS = _DENZA_ADDRESS;}
	function _changeASAddress(address _ALLSTARZ_ADDRESS) external onlyOwner {ALLSTARZ_ADDRESS = _ALLSTARZ_ADDRESS;}
	function _changeMannyAddress(address _MANNY_ADDRESS) external onlyOwner {MANNY_ADDRESS = _MANNY_ADDRESS;}
	function _changeChumsAddress(address _CHUMS_ADDRESS) external onlyOwner {CHUMS_ADDRESS = _CHUMS_ADDRESS;}
	function _changePssssdAddress(address _PSSSSD_ADDRESS) external onlyOwner {PSSSSD_ADDRESS = _PSSSSD_ADDRESS;}
	function _allAddressesToOne(address _NEW_ADDRESS) external onlyOwner {
		FWB_ADDRESS = _NEW_ADDRESS;
		ID_ADDRESS = _NEW_ADDRESS;
		DENZA_ADDRESS = _NEW_ADDRESS;
		ALLSTARZ_ADDRESS = _NEW_ADDRESS;
		MANNY_ADDRESS = _NEW_ADDRESS;
		CHUMS_ADDRESS = _NEW_ADDRESS;
		PSSSSD_ADDRESS = _NEW_ADDRESS;
	}
