...
// File: contracts/HydroHomiesPass.sol
...
contract YieldingNft is ERC721A('Hydro Homies Yield Coupon', 'HHYC'), Ownable {
    address token;
   ...
   function _startTokenId() internal view override virtual returns (uint256) {
    ...
    function publicMint() external payable {
        require(msg.value >= .25 ether, "not sending .25 ether");
        uint256 count = (msg.value / (.25 ether));
    ...
    function whitelistMint() external payable {
        ...
        require(totalSupply() + 1 <= 1000);
        require(wl[msg.sender], "address not in wl");
        _safeMint(msg.sender, 1);
		Token(token).updateReward(msg.sender, address(0));
        wl[msg.sender] = false;
    }

    function mintForInvestor(address _investor) external onlyOwner {
        require(totalSupply() == 0, "already some mints");
        _safeMint(_investor, 5);
		Token(token).updateReward(_investor, address(0));    
    }
    ...
    function toggleWLFor(address user) external onlyOwner {
        wl[user] = !wl[user];
    }
