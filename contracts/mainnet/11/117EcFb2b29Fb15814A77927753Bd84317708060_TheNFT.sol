...
contract TheNFT is ERC721Enumerable, Ownable, DefaultOperatorFilterer, AccessControl {
    using Strings for uint256;

    string _baseTokenURI = ""; // base URI (used by NFT market places)
    uint256 private _price = 0.1 ether; // price of NFT
    bool public _paused = true; // if true, minting is paused (update it via pause function
    IERC20 private _the; // the contract
    uint256 public _minAmount = 50000 * 1e18; // minimum THE amount require to mint NFT
    uint256 public _maxNFTAmount = 10; // maximum amount of NFTs per wallet
    uint256 public _maxSupply = 501; //maximum supply of NFTs
    uint16 public _baseStakingBP = 41;
    bool public _isRevealed = false;
    mapping(uint256 => uint16) private _tokenBonuses;

    // withdraw addresses
    address marketingAddress = 0x2faD5732e621121921cB5d1e7989C1Fd91c8da09;
    address creatorAddress = 0xb996ECB866b8074441c5282ef9F2253645443D64;
    ...
    constructor() ERC721("TheNFT", "TheNFT") {
        marketingAddresses.push(
            MarketingAddresses(payable(address(marketingAddress)), 95)
        );
        marketingAddresses.push(
            MarketingAddresses(payable(address(creatorAddress)), 5)
        );

        _safeMint(marketingAddress, 0);

        _the = IERC20(0x0cbA60Ca5eF4D42f92A5070A8fEDD13BE93E2861);
    }
    ...
    function mint(uint256 num) public payable {
        ...
        require(
            num <= _maxNFTAmount,
            "You can mint a maximum of 3 TheNFT at the time"
        ); // check if desired number exceeds max NFT amount
        require(
            _the.balanceOf(_msgSender()) >= _minAmount,
            "Not enough THE tokens"
        ); // check if user has enough THE tokens in their wallet
        require(
        ...
        require(supply + num <= _maxSupply, "Exceeds maximum TheNFT supply"); // check if current supply exceeds max supply
    ...
    function setStakingBP(uint256[] memory tokens, uint16 bonusPoints) public onlyOwner {
        for(uint i; i < tokens.length; i++){
            uint256 tokenId = tokens[i];
            _tokenBonuses[tokenId] = bonusPoints;
        }
    }
    ...
    */
    function getStakingBP(uint256 tokenId) public view returns (uint16) {
        if (_exists(tokenId)) {
            if (_tokenBonuses[tokenId] > 0) {
                return _tokenBonuses[tokenId];
            }

            return _baseStakingBP;
        }
