    ...
    SignatureVerifier_V2
{
    ...
    mapping(bytes32 => bool) public projectWithdrawn;
    ...
    function userRefund(bytes32 projectName) external nonReentrant {
        // * refunds should be active.
        require(refundsActiveForProject[projectName],"refunds not active");

        // * get the amount
        uint256 amountInvested = investmentPerUser[projectName][msg.sender];
        require(amountInvested > 0, "no investments");

        // * reset to 0
        investmentPerUser[projectName][msg.sender] = 0;

        // * modify totalCollected
        projectsLedger[projectName].totalCollected -= amountInvested;

        // * transfer USDC.
        require(USDC.transfer(msg.sender, amountInvested), "transfer failed");
       
    }

    function toggleRefundsActive(bytes32 projectName) external onlyOwner {
        require(projectExist[projectName], "No such project");
        refundsActiveForProject[projectName] = !refundsActiveForProject[projectName];
    }
    ...
    function withdrawProjectUSDC(bytes32 projectName, address reciever) external onlyOwner {
        require(projectExist[projectName], "No such project");
        uint256 amount = projectsLedger[projectName].totalCollected;
        require(USDC.transfer(reciever, amount), "transfer failed");
        projectWithdrawn[projectName] = true;
    }
}
