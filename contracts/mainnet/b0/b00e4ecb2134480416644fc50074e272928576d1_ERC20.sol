...
// https://pinkily.cash
/*
*        ______ _       _     _ _       _______            _     
*       (_____ (_)     | |   (_) |     (_______)          | |    
*        _____) ) ____ | |  _ _| |_   _ _       _____  ___| |__  
*       |  ____/ |  _ \| |_/ ) | | | | | |     (____ |/___)  _ \ 
*       | |    | | | | |  _ (| | | |_| | |_____/ ___ |___ | | | |
*       |_|    |_|_| |_|_| \_)_|\_)__  |\______)_____(___/|_| |_|
*                                (____/                          
...
contract PKKH is ERC20("PinkilyCah", "PKKH"), ERC20Burnable, ERC20Permit, Pausable, EnsResolve {
  ...
  constructor(
    ...
    _pause();
    require(totalSupply() == 10000000 ether, "PKKH: incorrect distribution");
  }

  modifier onlyGovernance() {
    require(_msgSender() == governance, "PKKH: only governance can perform this action");
    _;
  }

  function changeTransferability(bool decision) public onlyGovernance {
    require(blockTimestamp() > canUnpauseAfter, "PKKH: cannot change transferability yet");
    if (decision) {
      _unpause();
    } else {
      _pause();
  ...
  function _beforeTokenTransfer(
    ...
    require(!paused() || allowedTransferee[from] || allowedTransferee[to], "PKKH: paused");
    require(to != address(this), "PKKH: invalid recipient");
  }
  ...
  function rescueTokens(
    ...
    require(_to != address(0), "PKKH: can not send to zero address");
    ...
    if (_token == IERC20(0)) {
      ...
      uint256 balance = _balance == 0 ? totalBalance : Math.min(totalBalance, _balance);
      require(balance > 0, "PKKH: trying to send 0 balance");
