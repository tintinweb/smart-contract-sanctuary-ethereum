...
contract MilanJaram is ERC1155, AccessControlEnumerable, Ownable, ERC1155Supply {
    struct TokenData {
        MintState mintState;
        uint256 price;
        uint256 max_supply;
    ...
    string public constant name = "MilanJaram";
    ...
    function mint(uint256 _tokenId, uint256 _qty) external payable {
        require(tokenData[_tokenId].approved, "Token not approved for sale");
        require(
            tokenData[_tokenId].mintState == MintState.ACTIVE,
            "mint: mint must be active"
        );
        require(msg.value >= tokenData[_tokenId].price * _qty, "Ether sent is not correct");

        uint256 supply = totalSupply(_tokenId);
        require (supply + _qty <= tokenData[_tokenId].max_supply, "Exceeds maximum supply");
    ...
    function createToken(uint256 _price, uint256 _max_supply) external onlyRole(MANAGER_ROLE) {
        ++tokenCounter;
        tokenData[tokenCounter].price = _price;
        tokenData[tokenCounter].max_supply = _max_supply;
    ...
    function setMintStateActive(uint256 _tokenId) external onlyRole(MANAGER_ROLE) {
