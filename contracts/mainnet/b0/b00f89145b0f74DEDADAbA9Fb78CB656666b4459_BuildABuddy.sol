...
contract BuildABuddy is Ownable, ERC721, NonblockingReceiver {
    address public _owner;
    string private baseURI;
    uint256 nextTokenId = 6801;
    uint256 MAX_MINT_SUPPLY = 9600;
    mapping(address=>uint256) minted;

    uint256 gasForDestinationLzReceive = 350000;

    constructor(string memory baseURI_, address _layerZeroEndpoint)
        ERC721("BuildABuddy", "BAB")
    {
        _owner = msg.sender;
    ...
    function mint(uint8 numTokens) external payable {
        require(numTokens < 5, "Max 4 NFTs per transaction");
        require(
            nextTokenId + numTokens <= MAX_MINT_SUPPLY,
            "Mint exceeds supply"
        );
        require(minted[msg.sender] + numTokens < 13, "Exceeds max overall mint");
        minted[msg.sender] = minted[msg.sender] + 4;
