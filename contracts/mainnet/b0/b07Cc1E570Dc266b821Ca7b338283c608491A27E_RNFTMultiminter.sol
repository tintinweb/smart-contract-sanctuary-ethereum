...
contract RNFTMultiminter is Ownable {

    RNFT public rnft;

    constructor(address rnftAddress) {
        rnft = RNFT(rnftAddress);
    }

    function multiMint(address[] calldata to, uint256[] calldata amounts) external {
        require(to.length == amounts.length, "to and amounts length mismatch");
        for (uint256 i = 0; i < to.length; i++) {
            rnft.mint(to[i], amounts[i]);
        }
    }
