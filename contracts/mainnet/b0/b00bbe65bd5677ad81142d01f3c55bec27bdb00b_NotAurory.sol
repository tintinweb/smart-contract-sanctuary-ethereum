...
contract NotAurory is ERC721A, Ownable, ReentrancyGuard {
  ...
  constructor(string memory baseURI_) ERC721A("NotAurory", "NOTA") {
    baseURI = baseURI_;
  }
  ...
  function mint(uint256 amt) external payable
  {
    ...
    require(msg.sender == tx.origin,"Are you aurory?");
    require(msg.value == amt * cost,"Please send the exact amount.");
    require(totalSupply() + amt < maxSupply + 1,"No more aurories.");
    require(mintEnabled, "Mint not live.");
    require(numberMinted(msg.sender) + amt <= maxPerWallet,"Limit exceeded.");
