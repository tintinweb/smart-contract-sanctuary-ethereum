...
import "./interfaces/ICurvePoolStETH.sol";
import "./interfaces/IConvexBooster.sol";
import "./interfaces/IConvexReward.sol";
import "./interfaces/IPrice.sol";

contract StETH is OwnableUpgradeable, ISubStrategy {
    using SafeMath for uint256;

    // Sub Strategy name
    string public constant poolName = "StETH V3";
    ...
    // WETH token id for withdraw in curve pool
    int128 public constant tokenId = 0;
    ...
    /**
        External view function of total WETH deposited in Covex Booster
     */
    ...
    /**
        Internal view function of total WETH deposited
    */
    ...
    function _deposit(uint256 _amount) internal returns (uint256) {
        ...
        ICurvePoolStETH(curvePool).add_liquidity{value: _amount}(amounts, minOutput);
    ...
    function withdraw(uint256 _amount) external override onlyController returns (uint256) {
        ...
        // Withdraw WETH from Curve Pool
        ICurvePoolStETH(curvePool).remove_liquidity_one_coin(lpWithdrawn, tokenId, minAmt);

        // Transfer withdrawn WETH to controller
    ...
    function harvest() external override onlyController {
        ...
        for (uint256 i = 0; i < rewardTokens.length; i++) {
            ...
            TransferHelper.safeTransfer(rewardTokens[i], treasury, (balance * harvestFee) / magnifier);
            TransferHelper.safeTransfer(rewardTokens[i], vault, (balance * (magnifier - harvestFee)) / magnifier);
        }
    ...
    function withdrawable(uint256 _amount) external view override returns (uint256) {
        ...
        uint256 lpAmt = (totalLP * _amount) / total;
        uint256 expectedOutput = ICurvePoolStETH(curvePool).calc_withdraw_one_coin(lpAmt, tokenId);
