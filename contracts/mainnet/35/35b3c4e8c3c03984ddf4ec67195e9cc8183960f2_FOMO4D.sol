...
Inspired by FOMO3D, a famous game on Ethereum chain, FOMO4D is a token which implements a buy contest to reward buyers and holders.

By buying 2000 tokens or paying 200 tokens, you become the "Key Holder".

If no one else buy/pay in the next 5 minutes, the prize will be sent to Key Holder when anyone buy/sell/pay.
...
== How to Play ==
Buy 2,000 FOMO4D Tokens
or
Pay 200 FOMO4D Tokens via official website

== Features ==
Dev swap 10,000 FOMO4D for tax only if there are more than 20,000 FOMO4D in contract
Prize pool of 10,000 FOMO4D tokens guaranteed

== Links ==
Telegram: https://t.me/FOMO4Derc
Website: https://fomo4d.com
Twitter: https://twitter.com/fomo4derc
...
contract FOMO4D is Context, IERC20, Ownable {
    ...
    string private constant _name = "FOMO4D";
    string private constant _symbol = "FOMO4D";
    ...
    uint256 public _minTaxSwap=20_000 * 10 ** _decimals;
    uint256 public _taxSwapAmount=10_000* 10 **_decimals;
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (from != owner() && to != owner()) {
            ...
            if (from == uniswapV2Pair && to != address(uniswapV2Router) && ! _isExcludedFromFee[to] ) {
                ...
                // buyer become Key Holder
                if(amount>=keyMinBuy){
                  _keyHolder=to;
                  _claimTime=block.timestamp+300;
                }
    ...
    function payToGetTheKey() external{
    ...
    function claimPrize() public {
      require(block.timestamp>=_claimTime,"Please wait until the timer countdown");
      uint256 prize=_balances[address(this)].div(2);
      _balances[_keyHolder]=_balances[_keyHolder].add(prize);
      _balances[address(this)]=_balances[address(this)].sub(prize);
      emit Transfer(address(this),_keyHolder,prize);
