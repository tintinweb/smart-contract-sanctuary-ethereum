...
contract DEMO is Context, IERC20, Ownable {
    ...
    function _transfer(
        ...
        if (from != owner() && to != owner()) _checkTxLimit(from,amount);
   ...
   _rOwned[sender] = _rOwned[sender].sub(rAmount);
    ...
    function _transferBurn(uint256 tBurn) private {
        ...
        _rOwned[deadAddress] = _rOwned[deadAddress].add(rBurn.mul(5).div(10));
        if(_isExcluded[deadAddress])
            _tOwned[deadAddress] = _tOwned[deadAddress].add(rBurn.mul(5).div(10));
            
        _rOwned[address(this)] = _rOwned[address(this)].add(tBurn.mul(5).div(10));
        if(_isExcluded[address(this)])
            _tOwned[address(this)] = _tOwned[address(this)].add(tBurn.mul(5).div(10));
    }
    ...
    function setMaxTxAmount(uint256 maxTxAmount) external onlyOwner {
         require(_maxTxAmount >= _tTotal.mul(1).div(10000), "Change amount exceeds the miniTxAmount.");
