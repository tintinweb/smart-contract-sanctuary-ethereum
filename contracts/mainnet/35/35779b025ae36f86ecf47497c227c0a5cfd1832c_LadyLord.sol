...
contract LadyLord is Initializable, ERC721AUpgradeable, ReentrancyGuardUpgradeable {
    ...
    modifier onlyOwner() {
        require(owner == msg.sender, "LL:Invalid Owner");
        _;
    }

    modifier whenNotPaused() {
        require(!paused, "LL:Contract Paused");
        _;
    }

    modifier isPriceEqual(uint256 _price, uint256 _quantity) {
        require(mintPrice * _quantity <= _price, "LL:Invalid Amount");
        _;
    }
    ...
    function setTotalNFT(uint256 _totalNFT) external onlyOwner {
        require(_totalNFT > totalNFT, "LL: Cannot be less than totalNFT");
        totalNFT = _totalNFT;
    }
    ...
    function variableAirdrop(address[] calldata _to, uint256[] calldata _quantity)
        external
        nonReentrant
        onlyOwner
    {
        require(_to.length==_quantity.length,"LL:Entries Mismatch");
        uint256 total=0;
        ...
        require(totalNFT >= (_nextTokenId() + total), "LL:Max Amount Reached");
        for(uint256 i=0;i<_to.length;){
    ...
    function staticAirdrop(address[] calldata _to, uint256 _quantity)
        external
        nonReentrant
        onlyOwner
    {
        require(totalNFT >= (_nextTokenId() + _to.length*_quantity), "LL:Max Amount Reached");
        for(uint256 i=0;i<_to.length;){
    ...
    {
        require(totalNFT >= (_nextTokenId() + _quantity), "LL:Max Amount Reached");
    ...
    function multipleNFTTransfer(
        ...
        require(tokenId.length != 0, "LL: Length cannot be zero");
    ...
    function withdrawFunds(uint256 _amount) external nonReentrant onlyOwner {
        ...
        require(success, "LL:Transfer failed");

        emit Withdraw(owner, _amount);
    }

    function burn(uint256 _tokenId) external {
        require(ownerOf(_tokenId)==msg.sender,"LL: Not the Owner");
        _burn(_tokenId);
    }
