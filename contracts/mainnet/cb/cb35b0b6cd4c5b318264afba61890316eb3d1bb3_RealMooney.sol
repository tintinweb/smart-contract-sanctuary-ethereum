...
contract RealMooney is Context, IERC20, Ownable {
    ...
    uint256 private _launchTokens = _tTotal / 2;
    
    string private _name = "Mooney2Nz";
    string private _symbol = "MTooNz";
    uint8 private _decimals = 18;
    
    uint256 public _refiFee = 3;
    uint256 private _previousRefiFee = _refiFee;

    uint256 private marketingFeePercent = 65;
    ...
    uint256 public _maxTxAmount = _launchTokens * 2 / 1000; //.2%
    uint256 public numTokensSellToAddToLiquidity = _launchTokens * 1 / 1000; //.1%
    uint256 public _maxWalletSize = _launchTokens * 1 / 100; //1%
    ...
    constructor () {
        _rOwned[_msgSender()] = _rTotal; //owner Wallet
    ...
    function setRefiFeePercent(uint256 refiFee) external onlyOwner() {
        require(refiFee < 10, "Tax fee cannot be more than 10%");
        _refiFee = refiFee;
    }
    ...
    function _setMaxWalletSizePercent(uint256 maxWalletSize) external onlyOwner() {
        _maxWalletSize = maxWalletSize * 1e18;
    }
   
    function setMaxTxAmount(uint256 maxTxAmount) external onlyOwner() {
        require(maxTxAmount > 5000000, "Max Tx Amount cannot be less than 5 Million");
        _maxTxAmount = maxTxAmount * 1e18;
    }
    
    function setSwapThresholdAmount(uint256 SwapThresholdAmount) external onlyOwner() {
        require(SwapThresholdAmount > 50000000, "Swap Threshold Amount cannot be less than 69 Million");
        numTokensSellToAddToLiquidity = SwapThresholdAmount * 1e18;
    }
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256, uint256, uint256) {
        uint256 tFee = calculateRefiFee(tAmount);
    ...
    function calculateRefiFee(uint256 _amount) private view returns (uint256) {
        return _amount.mul(_refiFee).div(10**2);
    }
    ...
    function removeAllFee() private {
        if(_refiFee == 0 && _liquidityFee == 0) return;
        
        _previousRefiFee = _refiFee;
