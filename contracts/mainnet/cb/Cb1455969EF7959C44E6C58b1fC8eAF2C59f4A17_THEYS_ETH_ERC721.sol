...
contract THEYS_ETH_ERC721 is ERC721Pausable, Ownable, RoyaltiesV2Impl {

    constructor() ERC721("THEYS.ETH By xObj.com", "THEYS.ETH") {
        mint();
    }
    ...
    function _baseURI() internal view virtual override returns (string memory) {
        return "https://xobj.com/theys/meta/";
    }
    ...
    function claim() external payable {
        require(_tokenIdTracker.current() < 25000, "Max tokens minted");
        require(msg.value <= 0.075 ether, "max claim 5 at once");
        require(msg.value >= 0.015 ether, "claiming costs 0.015 eth");
        uint256 mv = msg.value;
        while (mv > 0) {
            if (_tokenIdTracker.current() < 25000) {
                if (mv >= 0.015 ether) {
                    mint();
                    mv -= 0.015 ether;
                } else {
