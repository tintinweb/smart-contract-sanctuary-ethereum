...
contract DCP is ERC721A, Ownable{
    using Strings for uint256;
     uint public tokenPrice = 0.3 ether;
    uint public presalePrice = 0.2 ether;
    uint constant maxSupply = 179;
    uint constant preSale_supply = 150;
    ...
    constructor() ERC721A("Diana Clark Photography", "DCP"){}
   ...
   function presale(uint _count) public payable{
        ...
        require(totalSupply() + _count <= maxSupply, "Not enough tokens left");
        require(totalSupply() + _count <= preSale_supply, "Not enough tokens left");
  ...
  function adminMint(uint _count) external onlyOwner{
    ...
    function publicSale_status(bool temp) external onlyOwner {
     ...
     function presale_price(uint pr) external onlyOwner {
        presalePrice = pr;
    }
