...
contract SimpleGuys is ERC721URIStorage, Ownable {
    ...
    mapping(address => bool) public simpleList;
    mapping(address => uint256) public walletMints;

    event simpleListMintEnabledEvent(bool simpleListMintEnabled);
    event publicMintEnabledEvent(bool publicMintEnabled);

    constructor(
        uint256 maxPerWallet_,
        uint256 simpleListCost_,
        uint256 publicMintCost_,
        uint256 devMints_,
        ...
        publicMintCost = publicMintCost_;
        devMints = devMints_;
        baseURI = baseURI_;
    ...
    function setPublicMintCost(uint256 publicMintCost_) external onlyOwner {
        publicMintCost = publicMintCost_;
    }
        ...
        onlyOwner
    {
        ...
        emit simpleListMintEnabledEvent(simpleListMintEnabled);
    }

    function setPublicMintEnabled(bool publicMintEnabled_) external onlyOwner {
        publicMintEnabled = publicMintEnabled_;

        emit publicMintEnabledEvent(publicMintEnabled);
    }

    function setWhitelistedWallets(
        address[] calldata _wallets
    ) external onlyOwner {

        for (uint256 i = 0; i < _wallets.length; i++) {
            simpleList[_wallets[i]] = true;
        }
    }

    function transferBalanceToOwner(address to_) external onlyOwner {
    ...
    function isWalletWhitelisted() external view returns(bool){
        return simpleList[msg.sender];
    }
    ...
    function mintSimpleList(uint256 quantity_) external payable {
        ...
        require(simpleList[msg.sender], "This wallet is not simple listed");
        require(
    ...
    function publicMint(uint256 quantity_) external payable {
        ...
        require(
            msg.value * quantity_ >= publicMintCost * quantity_,
            "Not enough ether"
        );
        require(quantity_ < 4, "Too many quantity");
        mint(quantity_);
    }
}