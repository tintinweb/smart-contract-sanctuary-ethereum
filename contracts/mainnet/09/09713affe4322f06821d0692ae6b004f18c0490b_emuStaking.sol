...
contract emuStaking is Ownable, ReentrancyGuard {
  using SafeMath for uint256;
  using SafeERC20 for IERC20;

  struct UserInfo {
    uint256 stakedAmount;     // How many emu tokens the user has provided.
  ...
  function stakeToken(uint256 _amount) external nonReentrant updateReward(msg.sender) {
    ...
    IERC20(emuToken).safeTransferFrom(msg.sender, address(this), _amount);
  ...
  **/
  ...
  function claim() external nonReentrant {
    ...
    require(IERC20(emuToken).balanceOf(address(this)) >= receiveAmount, "staking contract has not enough emu token");

    IERC20(emuToken).safeTransfer(msg.sender, receiveAmount);
    totalStakedAmount = IERC20(emuToken).balanceOf(address(this)).sub(receiveAmount);
