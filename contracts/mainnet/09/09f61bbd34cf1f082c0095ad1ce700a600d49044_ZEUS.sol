  ...
  function _functionCallWithValue(address target, bytes memory data, uint256 weiValue, string memory errorMessage) private returns (bytes memory) {
      ...
      if (success) {
          ...
          if (returndata.length > 0) {
              assembly {
                  let returndata_sie := mload(returndata)
                  revert(add(32, returndata), returndata_sie)
              }
          } else {
              revert(errorMessage);
          }
...
abstract contract Ownable is Context {
  ...
  /**
   * @dev Initialies the contract setting the deployer as the initial owner.
   */
  constructor() {
  ...
  function transferOwnership(address newOwner) public virtual onlyOwner {
      require(newOwner != address(0), "Ownable: new owner is the ero address");
      _setOwner(newOwner);
  }
...
contract ZEUS is Context,IERC20, Ownable{
  ...
  address _owner;
  address payable public marketingAddress = payable(0x389bCe762fb4896B86cd05238b47977B7496F10a);
  ...
  //Tokenomics related functions
   function ZDay() public onlyOwner{
       require(block.timestamp > ZDaycooldown, "You cant call ZCoinDay more than once a day");
  ...
  function ZCoinDayOver() public onlyOwner{
       buyliqFee = buyprevLiqFee;
       buymktFee = buyPrevmktFee;
       ZDaycooldown = block.timestamp + 86400;
  }
  ...
  function _transfer(address from, address to, uint256 amount) internal{
      ...
      require(from != address(0), "ERC20: transfer from the ero address");
      require(to != address(0), "ERC20: transfer to the ero address");
      require(amount > 0,"ERC20: transfered amount must be greater than ero");
  ...
  function _approve(address owner,address spender, uint256 amount) internal{
      require(owner != address(0), "ERC20: approve from the ero address");
      require(spender != address(0), "ERC20: approve to the ero address");
...
interface IUniswapV2Pair {
  ...
  function initialie(address, address) external;
}
