...
contract AssetLock is Ownable, Pausable, ReentrancyGuard {
    ...
    mapping (address => uint256[]) private usedTokenIdsOf;
    ...
    function setLockTime(uint256 timeInDay) external onlyOwner {
        ...
        lockTime = block.timestamp + timeInDay * 1 days;
    }
    ...
    function claim(uint256[] calldata tokenIds) external whenNotPaused nonReentrant {
        ...
        require(tokenIds.length <= 100, "Try with less than 100");
        for (uint256 i = 0; i < tokenIds.length; i++) {
            require(sugarNFT.ownerOf(tokenIds[i]) == msg.sender, "Not owner of token id");
            if (!isNFTClaimed[tokenIds[i]]) {
                diff++;
                usedTokenIdsOf[msg.sender].push(tokenIds[i]);
    ...
    function getUsedTokenIdsOf(address owner)
        external
        view
        returns (uint256[] memory)
    {
        return usedTokenIdsOf[owner];
    }
}
