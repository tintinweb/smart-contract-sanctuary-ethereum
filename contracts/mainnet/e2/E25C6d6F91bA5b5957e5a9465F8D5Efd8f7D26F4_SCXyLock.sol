...
import "./ISCXyLock.sol";
import "./IUniswapV2Router02.sol";
import "./IUniswapV2Pair.sol";
import "./IUniswapV2Factory.sol";
import "./FullMath.sol";

contract SCXyLock is ISCXyLock, Ownable {
    using Address for address payable;
    using EnumerableSet for EnumerableSet.AddressSet;
    using EnumerableSet for EnumerableSet.UintSet;
    using SafeERC20 for IERC20;
    uint256 unlockTimerSeconds = 30 * 24 * 3600; //30 * 24 * 3600 - 30 days expressed in seconds
    uint256 minimumPayableEthInWei = 1 * 10**17; //lock Fee in wei
    uint256 minimumSCXHolding = 100000000 * 10**9; // minimum amount of SCX or xSCX tokens to be held to create free lock
    address public scxToken = 0xBECC0623f8e8C0db4b605135A31c681dF7deaAB2;
    address public xscxToken = 0x2ED748b5d7AE1CD122C9B2773330E2b623ed0069;
    address _dexRouter = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D; // ETH: Uniswap V2
    IUniswapV2Router02 public dexRouter = IUniswapV2Router02(_dexRouter);
    ...
    struct Lock {
        ...
        bool isPerpLock; //if perpetual lock
        string description;
    }
    ...
    event PerpUnlockStarted(uint256 lockId);
    ...
    function lock(
        ...
        if (msg.value < minimumPayableEthInWei) {
            require(
                IERC20(scxToken).balanceOf(msg.sender) > minimumSCXHolding ||
                    IERC20(xscxToken).balanceOf(msg.sender) > minimumSCXHolding,
                "Not eligible to lock!"
            );
        } else {
            buyBackTokens(msg.value);
        }
    ...
    function perpLock(
        ...
        uint256 unlockDate = block.timestamp + unlockTimerSeconds;
        id = _createLock(
            owner,
            token,
            isLpToken,
    ...
    function startPerpUnlock(uint256 lockId) external validLock(lockId) {
        Lock storage userLock = _locks[lockId];
        require(
            userLock.owner == msg.sender,
            "You are not the owner of this lock"
        );
        require(userLock.isPerpLock == true, "not a perpetual lock");
        userLock.tgeDate = block.timestamp + unlockTimerSeconds;
        emit PerpUnlockStarted(lockId);
    }
    ...
    function setMinimumPayableEthInWei(uint256 newAmountInWei)
        external
        onlyOwner
    {
        minimumPayableEthInWei = newAmountInWei;
    }

    function setMinimumSCXHolding(uint256 newAmount) external onlyOwner {
        minimumSCXHolding = newAmount * 10**9;
    }
