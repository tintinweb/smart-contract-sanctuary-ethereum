...
contract MartianClub is Ownable, ERC721A, ReentrancyGuard {
  ...
  ) ERC721A("MartianClub", "MTC", maxBatchSize_, collectionSize_) {
  ...
  function allowlistMint(uint256 quantity) external payable callerIsUser {
    ...
    require(quantity>0 && allowlist[msg.sender] >= quantity && numberMinted(msg.sender) + quantity <= maxPerAddressDuringMint, "not eligible for allowlist mint");
    ...
    external
  {
    ...
    for (uint256 i = 0; i < addresses.length; i++) {
      require(numSlots[i]<=maxPerAddressDuringMint,"can not add this many allow list mint slot");
