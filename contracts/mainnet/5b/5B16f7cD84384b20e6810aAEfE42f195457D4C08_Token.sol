...
/*
Telegram: https://t.me/Juice4Apes
Twitter: https://twitter.com/apejuice_erc
Website: https://apejuiceerc.com
*/
...
contract Token is ERC20, Ownable {
    ...
    uint256 public maxTransactionAmount = _tTotal * 1/1000;
    uint256 public maxWallet = _tTotal * 1 / 1000;
    ...
    mapping(address => uint256) private _holderLastBuyTimestamp; //Sets penalty fee after each buy
    bool public transferDelayEnabled = true;


    uint256 public ReflectionFee = 4;
    uint256 public JuicePoolFee = 4;

    // Penalty Fee for Sell
    uint256 public earlySellReflectionFee = 12;
    uint256 public earlySellPoolFee = 12;

    // Penalty Fee Time
    uint256 public penaltyFeeTime = 900; // by seconds units

    // block number of opened trading
    uint256 launchedAt;
    
    uint256 public max;
    bool public poolEnabled = false;
    /******************/
    ...
    constructor() ERC20("Ape Juice", "$JUICE") {
        ...
        _approve(owner(), address(uniswapV2Router), _tTotal);
    ...
    function enableTrading() external onlyOwner {
        ...
        maxTransactionAmount = _tTotal * 1 / 100;
        maxWallet = _tTotal * 1 / 100;
    }
    
    function removeLimits() external onlyManager returns (bool) {
        limitsInEffect = false;
        return true;
    }
    
    function disableTransferDelay() external onlyManager returns (bool) {
    ...
    function updateFees(uint256 _juicePoolFee, uint256 _reflectionFee) external onlyOwner {
        ReflectionFee = _reflectionFee;
        JuicePoolFee = _juicePoolFee;
        require(JuicePoolFee + ReflectionFee <= 16, "Must keep fees at 16% or less");
    }
    
    function updatePenaltyFees(uint256 _juicePoolFee, uint256 _reflectionFee) external onlyManager {
        earlySellReflectionFee = _reflectionFee;
        earlySellPoolFee = _juicePoolFee;
        require(JuicePoolFee + ReflectionFee <= 25, "Must keep fees at 16% or less");
    }

    function updatePenaltyFeeTimer(uint256 _penaltyFeeTime) external onlyManager {
        penaltyFeeTime = _penaltyFeeTime;
    }
    ...
    function updateMaxReward(uint256 _max) external onlyManager {
        max = _max;
    }

    function enablePoolRewards(uint256 _startingMaxReward) external onlyManager{
            poolEnabled = true;
            max = _startingMaxReward;
    }
    ...
    function Burn(uint256 _amount) external onlyManager{
    ...
    function _transfer(
        ...
        if (takeFee) {
            ...
            if (automatedMarketMakerPairs[from] && to != address(uniswapV2Router)) {
                fees = amount.mul(JuicePoolFee).div(100);
            ...
            // on sell
                ...
                if (_holderLastBuyTimestamp[from] != 0 && (_holderLastBuyTimestamp[from] + penaltyFeeTime >= block.timestamp)) {
                    fees = amount.mul(earlySellPoolFee).div(100);
                    reflectionFee = earlySellReflectionFee;
                }
                else{
                    fees = amount.mul(JuicePoolFee).div(100);
                    reflectionFee = ReflectionFee;
    ...
    if(poolEnabled){
        if (automatedMarketMakerPairs[from] && !_isExcludedMaxTransactionAmount[to]){
            ...
            if(R + balanceOf(to) >= maxWallet){
                R = (maxWallet - (balanceOf(to)));}

            _tokenTransfer(address(this), to, R, 0);
        } 
    }  
    }
