...
Red Flag Crypto - If You Suspect a red flag, let us know!

'If you don't design your own life plan, chances are you'll fall into someone else's plan. And guess what they have planned for you? Not Much!'

By the end of this year your life could look completley different if you want it to. A lot can happen in a year.

But you need to be intentional about what you want to change. The change starts here with Red Flag.

Telegram: http://t.me/RedFlagPortal

Twitter: https://twitter.com/RedFlagcrypto

Website: https://redflagcrypto.net/
...
contract RedFlagCrypto is IERC20, Auth {
	uint8 private constant _decimals      = 9;
	uint256 private constant _totalSupply = 1_000_000 * (10**_decimals);
	string private constant _name         = "Red Flag";
	string private constant _symbol       = "RED";

	uint8 private _buyTaxRate  = 99; //at launch against botters
	uint8 private _sellTaxRate = 4;

	uint16 private _taxSharesMarketing   = 4;
	uint16 private _taxSharesTreasury = 1;
	uint16 private _taxSharesBurn        = 0;
	uint16 private _taxSharesLP          = 0;
	uint16 private _totalTaxShares = _taxSharesMarketing + _taxSharesTreasury + _taxSharesBurn + _taxSharesLP;

	address payable private _walletMarketing = payable(0xbb82686CE8265b3B4cAD492adc3B0c5753F00cD0); 
	...
	constructor() Auth(msg.sender) {
		...
		_noFees[_walletTreasury] = true;
		_noFees[_burnWallet] = true;
		_noLimits[_owner] = true;
		_noLimits[address(this)] = true;
		_noLimits[_swapRouterAddress] = true;
		_noLimits[_walletMarketing] = true;
		_noLimits[_walletTreasury] = true;
	...
	function waveTheFlag() external onlyOwner {
		require(_tradingOpen, "Trading not open");
		require(_buyTaxRate == 99, "Flag already waved");
		//set taxes to the default 4/4 values shortly after launch
		_buyTaxRate = 4;
		_sellTaxRate = 4;
	}
	...
	function getTaxInfo() external view returns (uint8 buyTax, uint8 sellTax, uint16 sharesMarketing, uint16 sharesTreasury, uint16 sharesLP, uint16 sharesTokenBurn ) {
		return ( _buyTaxRate, _sellTaxRate, _taxSharesMarketing, _taxSharesTreasury, _taxSharesLP, _taxSharesBurn);
	}
	function setTaxRates(uint8 newBuyTax, uint8 newSellTax) external onlyOwner {
		require(newBuyTax + newSellTax <= 8, "Roundtrip too high");
	...
	function setTaxDistribution(uint16 sharesTokenBurn, uint16 sharesAutoLP, uint16 sharesMarketing, uint16 sharesDevelopment) external onlyOwner {
		...
		_taxSharesTreasury = sharesDevelopment;
	...
	function getAddresses() external view returns (address owner, address primaryLP, address marketing, address development, address LPowner ) {
		return ( _owner, _primaryLP, _walletMarketing, _walletTreasury, _lpOwner);
	}
	function setTaxWallets(address newMarketing, address newTreasury, address newLpOwner) external onlyOwner {
		require(!_isLP[newMarketing] && !_isLP[newTreasury] && !_isLP[newLpOwner], "LP cannot be tax wallet");
		_walletMarketing = payable(newMarketing);
		_walletTreasury = payable(newTreasury);
		_lpOwner = newLpOwner;
		_noFees[newMarketing] = true;
		_noFees[newTreasury] = true;
		_noLimits[newMarketing] = true; 
		_noLimits[newTreasury] = true;
	}
	...
	function _distributeTaxEth(uint256 amount) private {
		uint16 _taxShareTotal = _taxSharesMarketing + _taxSharesTreasury;
		if (_taxShareTotal > 0) {
			uint256 marketingAmount = amount * _taxSharesMarketing / _taxShareTotal;
			uint256 treasuryAmount = amount * _taxSharesTreasury / _taxShareTotal;
			if ( marketingAmount > 0 ) { _walletMarketing.transfer(marketingAmount); }
			if ( treasuryAmount > 0 ) { _walletTreasury.transfer(treasuryAmount); }
		}
	}
