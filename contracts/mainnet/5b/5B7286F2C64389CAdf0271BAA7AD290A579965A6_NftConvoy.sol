...
contract NftConvoy is ERC1155, Ownable, Pausable {
    uint private constant MAX_SUPPLY = 10000;
    uint private constant MAX_PER_TX = 100;
    
    // Meta
    uint private counter;
    string public name = "NftConvoy";
    ...
    function mint(uint amount) external payable whenNotPaused {
        require(amount < MAX_PER_TX + 1, "amount can't exceed 100");
        require(amount > 0, "not enough sent");
        require(msg.value == price * amount, "insufficient funds");
        require(counter + amount < MAX_SUPPLY, "none left");
