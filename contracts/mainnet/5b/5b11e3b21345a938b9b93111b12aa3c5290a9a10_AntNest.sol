...
contract AntNest is  ERC721AQueryable,Ownable {
    ...
    function setAuth(address _address) external onlyOwner {
        Auth = IAuth(_address);
    }
    ...
    function costMinting(uint256 _amount) external payable{
        require(active && block.timestamp >= openTime);
        require(_amount <= CostQuantity() && _amount <= per_costMint);
        require(_amount * price <= msg.value,"Not enough ether sent");

        costQuantity += _amount;

        _safeMint(msg.sender, _amount);
        emit CostLog(msg.sender, _amount, msg.value);
    }

    function freeMinting(uint256 _amount, string memory _sign) external {
        require(active);
        require((Auth.verifyA(msg.sender, _sign) == true  && block.timestamp >= openTime) || block.timestamp >= PublicTime,"Don't have permission to mint");
        require(_amount <= FreeQuantity() &&  _amount <= freeMint, "Insufficient quantity");

        _freeQuantity[msg.sender] += _amount;

        require(_freeQuantity[msg.sender] <= freeMint,"Exceeds max free mint");

        freeQuantity += _amount;
    ...
    function setOpenTimes(uint256 _openTime) public onlyOwner {
        openTime = _openTime;
    }
    ...
    function setFreeMint(uint256 _amount) public onlyOwner {
        freeMint = _amount;
    } 
    ...
    function setPerCostMint(uint256 _amount) public onlyOwner {
        per_costMint = _amount;
    }    

    function getFreeQuantity(address _to) public view returns(uint256){
        return _freeQuantity[_to];
    }
 
    function FreeQuantity() public view returns(uint256){
        return FREE_SUPPLY - freeQuantity;
    }

    function CostQuantity() public view returns(uint256){
        return MAX_SUPPLY - FREE_SUPPLY - costQuantity;
    }
