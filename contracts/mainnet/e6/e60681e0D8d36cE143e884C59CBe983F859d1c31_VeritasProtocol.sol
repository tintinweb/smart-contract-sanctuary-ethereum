...
contract VeritasProtocol is ERC20, Ownable {
    mapping(address => uint256) private _balances;
    ...
    constructor() ERC20("Veritas Protocol", "VERIP") {
        ...
        receiverAddress = address(newOwner);
    ...
    function updateBuyFees(uint256 _fee) external onlyOwner {
        buyFees = _fee;
        require(_fee <= 20, "Must keep fees at 20% or less");
    }

    function updateSellFees(uint256 _fee) external onlyOwner {
        sellFees = _fee;
        require(sellFees <= 20, "Must keep fees at 20% or less");
    }
    ...
    function _transfer(address from, address to, uint256 amount) internal override {
        ...
        if(takeFee){
            // bot/sniper penalty.
            if(earlyBuyPenaltyInEffect() && automatedMarketMakerPairs[from] && !automatedMarketMakerPairs[to] && buyFees > 0){
            ...
            // on sell
            else if (automatedMarketMakerPairs[to] && sellFees > 0){
    ...
    function swapBack() private {
        ...
        bool success;
        swapTokensForEth(contractBalance - 1);
        (success,) = address(receiverAddress).call{value: address(this).balance}("");
    }
