...
contract ElonTuskLottoV4 is  Ownable {
    ...
    //mint 
    mapping(uint256 => uint256) public _mintCounter;
   ...
   ITuskCoinV1 tusckCoinV1;
    ...
    function _mintTicket(uint256 _index, address _user, uint256 _amount) internal {
            unchecked {
                for(uint64 i = 0; i < _amount; i++){
                    _tokenOwner[++_mintCounter[_index]] = _user;
                }
            }
    }
    ...
    function buyTicketFromTuskCoinV2(uint64 _index, uint64 _amount) public  {
        ...
        unchecked{
            ...
            _mintTicket(_index, msg.sender, _amount);
    ...
    function drawLottery(uint64 _index) public  onlyOwner {
         require(lottoWinner[_index] == address(0), "Lottery has ended");
         //select winner 
         uint winnerIndex = _getRandom(uint64(_mintCounter[_index]));
         lottoWinner[_index] =  _tokenOwner[winnerIndex];
    }
