...
contract LL420GameItemFactory is Initializable, OwnableUpgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function buy(
        ...
        if (taken + _amount > maxPerWallet(_id) || _amount == 0) revert InvalidAmount();
    ...
    function eligibleToBuy(
        ...
        uint256 mintedAmount = mintedPerWallet[_user];
        uint256 taken = (mintedAmount >> ((_id - 1) * 4)) % 16;

        if (_amount == 0 || taken + _amount > maxPerWallet(_id)) return false;
    ...
    function maxPerWallet(uint256 id) public pure returns (uint256 max) {
        ...
        else if (id == FARMER_PASS_ID) max = 5;
        else if (id == DR_PASS_ID) max = 5;
        else max = 0;
    }
    ...
    function setSupply(uint256[] memory _caps) external onlyOwner {
        tokenCount = _caps.length - 1;
        uint256 length = _caps.length;
