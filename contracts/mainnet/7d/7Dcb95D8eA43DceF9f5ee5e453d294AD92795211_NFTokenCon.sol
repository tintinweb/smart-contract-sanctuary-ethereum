  ...
  Ownable
{
    ...
    mapping (uint8 => uint256) public counterIndex;
  ...
  constructor()
  {
    nftName = "FO";
    nftSymbol = "FO";
    baseURI = "https://ipfs.io/ipfs/QmYZQisM3RXY9xSX7EzU5qjtwzH5WqTCEKs7snFXUDevvj/";
  ...
  function mint(address addr,uint8 _type)
    public
    payable
    onlyManager
    returns (uint256 tokenId)
  {   
    require(_type>=0 && _type <=31);
    require(counterIndex[_type]<=320,"sold out");
    tokenId = _type*1000 + counterIndex[_type] + 1;
    counterIndex[_type] = counterIndex[_type] + 1;
    _mint(addr,tokenId);
    _setTokenUri(tokenId, _uint2str(tokenId));
  }
  ...
  function getUserNft(address addr) public view returns(uint256[] memory list){
    uint256 total = _getOwnerNFTCount(addr);
    list =  new uint256[](total);
    
    for(uint256 i;i<total;i++){
      uint256 _tokenId = ownerToIds[addr][i];
      list[i] = _tokenId;
    }
  }
