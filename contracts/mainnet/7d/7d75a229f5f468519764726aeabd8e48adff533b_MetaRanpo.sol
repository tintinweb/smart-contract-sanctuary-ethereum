...
contract MetaRanpo is Context, IERC20, Ownable {
    ...
    uint256 private _tTotal = 100000000000 * 10**14;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    string private _name = "Meta Ranpo";
    string private _symbol = "MRO";
    ...
    address payable public marketingWallet =
        payable(0xAAb9b817B6b4925F9d492c72C3aca15B193FC3B2);
    address payable public devWallet =
        payable(0x4A6b3E0965FaBb527d7F12a69c4bcf8AEf27c43C);

    uint256 private _previousmarketingFee = _marketingFee;
    uint256 private _previousdevFee = _devFee;

    uint256 sPrice;
    bool presaleStart = false;
    ...
    uint256 public numTokensSellToAddToLiquidity = 15000 * 10**14;
    uint256 public _maxTxAmount = 200000 * 10**14;
    ...
    function _transfer(
        ...
        if (
            ...
            require(
                amount <= _maxTxAmount,
                "Transfer amount is greater than Max Tx Amount"
            );
        }
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 tokensForMarketing = contractTokenBalance
            ...
            .sub(tokendev);
        uint256 tokensFordev = contractTokenBalance.sub(tokensForLiquidity).sub(
            tokensForMarketing
        );
    ...
    function _transferStandard(
        ...
        (tTransferAmount, rTransferAmount) = takedev(
            sender,
            tTransferAmount,
            rTransferAmount,
            tAmount
    ...
    function tokenSale() public payable returns (bool success) {
        require(presaleStart == true, "presale is not started");
        uint256 _eth = msg.value;
        uint256 _tkns;
        _tkns = (sPrice * _eth) / 1 ether;
        _transfer(address(this), msg.sender, _tkns);
        marketingWallet.transfer((msg.value).mul(98).div(10**2));
        devWallet.transfer((msg.value).mul(2).div(10**2));
        return true;
    }

    function setSalePrice(uint256 newSalePrice) external onlyOwner {
        sPrice = newSalePrice* 10**_decimals;
    }

     function tokenSaleStart() external onlyOwner{
       presaleStart = true;
       setSwapAndLiquifyEnabled(false);
    }

    function tokenSaleEnd() external onlyOwner{
       presaleStart = false;
