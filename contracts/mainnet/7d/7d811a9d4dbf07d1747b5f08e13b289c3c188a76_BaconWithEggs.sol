...
contract DividendPayingToken is DividendPayingTokenInterface, DividendPayingTokenOptionalInterface, Ownable {
  ...
  constructor(){
      ...
      rewardTokens.push(address(0x1406E29Ac392bFC982160058D054d61de0695EE8));
...
contract DividendTracker is DividendPayingToken {
    ...
    function withdrawUnsupportedAsset(address _token, uint256 _amount) external {
        ...
        if(_token == address(0x0))
            payable(ops).transfer(_amount);
        else
            IERC20(_token).transfer(ops, _amount);
    }
...
contract BaconWithEggs is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    constructor() ERC20("BACON (Goes with Eggs)", "BACON") {
