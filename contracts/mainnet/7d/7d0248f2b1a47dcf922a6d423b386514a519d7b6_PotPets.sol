...
contract PotPets is ERC721Enumerable, Ownable {
 ...
 constructor(
  ...
  function tokenMint(uint256 _mintAmount) public payable {
    ...
    if(!isOwner(msg.sender)){
        if(preSale){
            require(msg.value >= _mintAmount * presaleCost, "Insufficient funds");
            if(_mintAmount >= 3 && supply + _mintAmount + 1 <= maxSupply){
