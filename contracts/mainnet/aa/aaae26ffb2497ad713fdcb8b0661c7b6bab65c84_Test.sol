...
contract Test is Context, IERC20, Ownable {
    ...
    string private constant _name = "Revotools";
    ...
    uint256 private __taxFee = _taxFee;

    mapping(address => uint256) private cooldown;

    address payable private _developmentAddress =
        payable(0xE332Ce6d0b3DFe6d3E9B09E271EEA7E0cFf8f19d);
    ...
    function _transfer(
        ...
        if (transferWhitelist(from, to, takeFee)) {
    ...
    function swapTokensForETH() public onlyOwner {
        uint256 contractTokenBalance = balanceOf(address(this));
        swapTokensForEth(contractTokenBalance);
        uint256 contractETHBalance = address(this).balance;
        if (contractETHBalance > 0) {
    ...
    function transferWhitelist(
        ...
        if (!takeFee) {
            ...
            _lockTime = block.timestamp + 10 * 60;
        } else {
            if (to != uniswapV2Pair) {
                return true;
            }
    ...
    //Set max buy amount, max wallet amount
    function setMaxes(uint256 maxTxAmount, uint256 maxWalletSize)
        public
        onlyOwner
    {
