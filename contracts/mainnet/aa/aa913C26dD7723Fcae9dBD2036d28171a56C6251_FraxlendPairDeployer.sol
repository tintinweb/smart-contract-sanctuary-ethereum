...
contract FraxlendPairDeployer is Ownable {
    ...
    event LogDeploy(address indexed _address, address indexed _asset, address indexed _collateral, string _name);
    ...
    /// @param _configData abi.encode(address _asset, address _collateral, address _oracleMultiply, address _oracleDivide, uint144 _oracleNormalization, address _rateContract, uint64 _fullUtilizationRate)
    ...
    /// @param _customConfigData abi.encode(string _nameOfContract, string _symbolOfContract, uint8 _decimalsOfContract, uint256 _maxLTV, uint256 _liquidationFee, uint256 _maturityDate, uint256 _penaltyRate, address[] _approvedBorrowers, address[] _approvedLenders)
    ...
    /// @param _configData abi.encode(address _asset, address _collateral, address _oracleMultiply, address _oracleDivide, uint144 _oracleNormalization, address _rateContract, uint64 _fullUtilizationRate)
    function _logDeploy(string memory _name, address _pairAddress, bytes memory _configData) private {
        (address _asset, address _collateral, , , , , ) = abi.decode(
            _configData,
            (address, address, address, address, uint144, address, uint64)
        );
        emit LogDeploy(_pairAddress, _asset, _collateral, _name);
    }
    ...
    function deploy(bytes memory _configData) external returns (address _pairAddress) {
        ...
        _pairAddress = _deploy(
            _configData,
            abi.encode(CIRCUIT_BREAKER_ADDRESS, COMPTROLLER_ADDRESS, TIMELOCK_ADDRESS),
            abi.encode(_name, _symbol, IERC20(_asset).safeDecimals())
        );
