...
contract Aptosdolls is ERC721A, Ownable, Pausable, ReentrancyGuard {
    using Strings for uint256;
    ...
    constructor() ERC721A("Aptosdolls", "Adolls") {
        setBaseURI("");
	...
	require(saleActivated, "Mint: Mint process is pause");
        ...
        require(addressFreeMintedCount + _mintAmount <= maxperAddressFreeLimit, "Max Aptosdolls per address exceeded");
        require(_mintAmount > 0, "Cant mint 0 Aptosdolls");
        require(_mintAmount <= maxperAddressPublicMint, "Cant mint more Aptosdolls" );
        require(s + _mintAmount <= maxSupply, "Cant exceed Aptosdolls supply");
    ...
    function MintGenesis(uint256[] calldata quantity, address[] calldata recipient)
        external
        onlyOwner
    {
        require(
