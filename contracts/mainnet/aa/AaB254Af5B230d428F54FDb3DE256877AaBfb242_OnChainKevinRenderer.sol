...
contract OnChainKevinRenderer is Ownable {
    IOnChainKevin public onChainKevin;

    function tokenIdToHash(uint256 tokenId) external view returns (string memory) {
        return onChainKevin._tokenIdToHash(tokenId);
    }

    function hashToSVG(string memory _hash) external view returns (string memory) {
        return onChainKevin.hashToSVG(_hash);
    }

    function hashToMetadata(string memory _hash) external view returns (string memory) {
        return onChainKevin.hashToMetadata(_hash);
    }

    function setOCKContractAddress(address ockAddress) external onlyOwner {
        onChainKevin = IOnChainKevin(ockAddress);
    }
}