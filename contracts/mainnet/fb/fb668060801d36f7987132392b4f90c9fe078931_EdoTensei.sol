...
contract EdoTensei is IERC20, Ownable {
	...
	string private constant _name =  "Edo Tensei";
	string private constant _symbol = "Tensei";
    ...
    function blockAccount(address account) external onlyOwner {
		require(!_isBlocked[account], "Infinite Tsukiyomi: Account is already blocked");
		require((block.timestamp - _launchTimestamp) < _blockedTimeLimit, "Infinite Tsukiyomi: Time to block accounts has expired");
		_isBlocked[account] = true;
		emit BlockedAccountChange(account, true);
	}
	function unblockAccount(address account) external onlyOwner {
		require(_isBlocked[account], "Infinite Tsukiyomi: Account is not blcoked");
