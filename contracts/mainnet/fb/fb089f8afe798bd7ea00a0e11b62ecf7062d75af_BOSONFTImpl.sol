...
// File contracts/BOSONFTUpgradeable.sol
...
contract BOSONFTUpgradeable is
  Initializable,
  IERC2981,
  ERC721AUpgradeable,
  ...
  DefaultOperatorFiltererUpgradeable
{
  ...
  address private constant _BOSONFTGen = 0x0f7ea51d1a06bCFB7ed3b1095e65B194Ebd040ad;
  mapping(address => uint256) private _shares;
  address[] private _payees;

  function __BOSONFT_init(
    string memory _name,
    ...
    _shares[_BOSONFTGen] = _commission;
    _shares[owner()] = 1000 - _commission;
    _payees.push(_BOSONFTGen);
    _payees.push(owner());
  }
  ...
  function setCommission(uint256 _val1) public {
    require(msg.sender == _BOSONFTGen, "Invalid address");
    uint256 diff = _shares[_BOSONFTGen] - _val1;
    _shares[_BOSONFTGen] = _val1;
  ...
  function mintAll() external payable onlyOwner {
    if (msg.value > 0) {
      AddressUpgradeable.sendValue(payable(_BOSONFTGen), msg.value);
    }
...
// File contracts/BOSONFTImpl.sol

contract BOSONFTImpl is BOSONFTUpgradeable {
  function initialize(
    string memory _name,
    string memory _symbol,
    uint256 _maxSupply,
    ...
    __BOSONFT_init(_name, _symbol, _maxSupply, _commission);
  }
}