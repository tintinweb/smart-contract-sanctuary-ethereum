...
contract BoneDucks is Ownable, ERC721A, ReentrancyGuard {
    ...
    uint256 public maxToken = 6555;
    uint256 public price;

    string private _baseTokenURI;
    string public notRevealedUri = "ipfs://QmaQTwD586VYePwWccjPFSmbdWhNDnYV8fD5U6nqBj4o6J/";
    ...
    function mint(uint256 quantity, bytes32[] memory proof) external payable callerIsUser {
        ...
        if(whitelistSale){
            require(numberMinted(msg.sender) + quantity <= maxPerTxWL, "PER_WALLET_LIMIT_REACHED");
            require(verify(proof), "ADDRESS_NOT_WHITELISTED");
            require(quantity <= maxPerTxWL, "CANNOT_MINT_THAT_MANY");
        }
        if(publicSale){
            require(!publicMinted[msg.sender], "ALREADY_MINTED");
