...
contract marketfi is IERC20, OWNED {
	mapping(address => uint256) private _balances;
	mapping(address => mapping(address => uint256)) private _allowances;
	uint8 private constant _decimals = 9;
	uint256 private constant _totalSupply = 100_000_000 * 10**_decimals;
	string private constant _name = "The Last Rug";
	string private constant _symbol = "MarketFi";
	...
	address private _treasuryWallet = address(0xF9bBec5f30821160Ba36FDCD8dB4C339957ecAfD);
	...
	function addInitialLiquidity(uint256 val) external onlyOwner {
		...
		_taxRateBuy = 5;
		_taxRateSell = 5; //anti-dump sell tax at launch
	...
	function changeTaxRates(uint8 newTaxRateBuy, uint8 newTaxRateSell) external onlyOwner {
		require( (newTaxRateBuy+newTaxRateSell) <= 100, "Max roundtrip is 10%" );
