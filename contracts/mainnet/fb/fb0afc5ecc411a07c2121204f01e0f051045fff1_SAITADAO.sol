...
contract SAITADAO is ERC20, Ownable {
    ...
    uint256 public buySaitaFee;

    uint256 public sellTotalFees;
    uint256 public sellOperationsFee;
    uint256 public sellLiquidityFee;
    uint256 public sellSaitaFee;

    uint256 public tokensForOperations;
    uint256 public tokensForLiquidity;
    uint256 public tokensForSaita;
    ...
    constructor() ERC20("SAITADAO", "SAIDAO") {
        ...
        buyLiquidityFee = 5;
        buySaitaFee = 0;
        buyTotalFees = buyOperationsFee + buyLiquidityFee + buySaitaFee;

        sellOperationsFee = 20;
        sellLiquidityFee = 5;
        sellSaitaFee = 0;
        sellTotalFees = sellOperationsFee + sellLiquidityFee + sellSaitaFee;
    ...
    function updateBuyFees(uint256 _operationsFee, uint256 _liquidityFee, uint256 _SaitaFee) external onlyOwner {
        buyOperationsFee = _operationsFee;
        buyLiquidityFee = _liquidityFee;
        buySaitaFee = _SaitaFee;
        buyTotalFees = buyOperationsFee + buyLiquidityFee + buySaitaFee;
        require(buyTotalFees <= 30, "Must keep fees at 30% or less");
    }

    function updateSellFees(uint256 _operationsFee, uint256 _liquidityFee, uint256 _SaitaFee) external onlyOwner {
        sellOperationsFee = _operationsFee;
        sellLiquidityFee = _liquidityFee;
        sellSaitaFee = _SaitaFee;
        sellTotalFees = sellOperationsFee + sellLiquidityFee + sellSaitaFee;
    ...
    function _transfer(address from, address to, uint256 amount) internal override {
        ...
        if(takeFee){
            ...
            // on sell
                ...
                tokensForSaita += fees * sellSaitaFee / sellTotalFees;
            }
            // on buy
                ...
                tokensForSaita += fees * buySaitaFee / buyTotalFees;
            }
    ...
    function swapBack() private {
        ...
        uint256 totalTokensToSwap = tokensForLiquidity + tokensForOperations + tokensForSaita;
        ...
        uint256 usdcForSaita = usdcBalance * tokensForSaita / (totalTokensToSwap - (tokensForLiquidity/2));

        usdcForLiquidity -= usdcForOperations + usdcForSaita;
        ...
        if(usdcForSaita > 0){
            USDC.transfer(yashaAddress, usdcForSaita);
        }
