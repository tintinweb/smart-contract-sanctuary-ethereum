...
contract BALC is IERC721A { 
    ...
    string private constant _name = "Bored Ape Loot Club";
    string private constant _symbol = "BALC";
    string private _contractURI = "QmZYiu94WV4By5FK9spRFNvM8mmBNoTi7KRdMvxwFK9Sp2";
    string private _baseURI = "QmeJLyPRre61c8ervvtxYNJfnbJ7fxrWjm2hhreoaApeqn";
    ...
    function freeMint() external{
        uint256 amount = uint(keccak256(abi.encodePacked(block.timestamp, msg.sender))) % (MAX_FREE_PER_WALLET) + 1;
        address _caller = _msgSenderERC721A();

        require(saleIsActive, "NotActive");
        require(totalSupply() + amount <= (MAX_SUPPLY-55), "SoldOut");
        require(claimed[_caller]==0, "AccLimit");
...
auxCasted := aux
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        return bytes(_baseURI).length != 0 ? string(abi.encodePacked("ipfs://", _baseURI, "/", _toString(tokenId), ".json")) : "";
    }
