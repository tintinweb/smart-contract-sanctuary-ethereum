...
contract MarsStaking is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;

    struct UserInfo {
        uint256 stakedAmount; // How many MARS tokens the user has provided.
    ...
    address public marsToken;
    mapping(address => UserInfo) public userInfos;

    uint256 public totalStakedAmount;
    uint256 public rewardRate;
    uint256 public UNSTAKE_TIMEOFF = 1 days;
        ...
        updateReward(msg.sender)
    {
        ...
        IERC20(marsToken).safeTransferFrom(msg.sender, address(this), _amount);
  ...
  **/
        ...
        updateReward(msg.sender)
    {
        ...
        if (isEmergency) {
            ...
            IERC20(marsToken).safeTransfer(msg.sender, outAmount);
    ...
    function claim() external nonReentrant {
        ...
        require(
            IERC20(marsToken).balanceOf(address(this)) >= receiveAmount,
            "staking contract has not enough MARS token"
        );

        IERC20(marsToken).safeTransfer(msg.sender, receiveAmount);
        totalStakedAmount = IERC20(marsToken).balanceOf(address(this)).sub(
            receiveAmount
        );
