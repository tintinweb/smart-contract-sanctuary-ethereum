...
contract ItsSomething is ERC721ABurnable, Ownable {
  using ECDSA for bytes32;

  address public _itsNothingContractAddress;
  address public _systemAddress = address(0);
  uint256 public _startingSomethingId = 1;
  uint256 public constant MAX_SOMETHINGS_ID = 8888;
  uint256 public _price = 0.0 ether;

 bool public _mintPaused = true;
  ...
  */
constructor()
    ERC721A("SOMETHING", "OBJECT")
{

}

  function mintSomething(uint256[] memory tokenIds) external {
    require(msg.sender == tx.origin, "Real people only!");
    require(!_mintPaused, "Minting is paused");

    ISomething something = ISomething(_itsNothingContractAddress);

    bool success;
    for (uint256 i; i < tokenIds.length; i++) {
      uint256 nothingNftId = tokenIds[i];
      if (something.ownerOf(nothingNftId) == msg.sender && !_exists(nothingNftId)) {
        _mint(msg.sender, nothingNftId);
        success = true;
      }
    }
  }
  ...
  function pauseMint(bool val) external onlyOwner {
    _mintPaused = val;
  }
  ...
  function setNothingNftAddress(address itsNothingContractAddress) external onlyOwner {
    _itsNothingContractAddress = itsNothingContractAddress;
  }
