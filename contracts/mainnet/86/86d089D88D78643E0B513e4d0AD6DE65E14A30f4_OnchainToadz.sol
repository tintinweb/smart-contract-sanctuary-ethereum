...
contract OnchainToadz is ERC721Enumerable, Ownable {
    event SeedUpdated(uint256 indexed tokenId, uint256 seed);

    mapping(uint256 => uint256) internal seeds;
    IToadDescriptor public descriptor;
    uint256 public maxSupply = 8888;
    bool public minting = false;
    bool public canUpdateSeed = true;

    constructor(IToadDescriptor newDescriptor) ERC721("Onchain Toadz", "Ribbit") {
        descriptor = newDescriptor;
    }

    function mint(uint32 count) public payable {
        require(minting, "Minting needs to be enabled to start minting");
        if (msg.value <  0.002 ether * count) {
              require(balanceOf(msg.sender) + count < 5, "Max 4 free Toadz per wallet!");
        } 
        require(count < 21, "Exceeds max per transaction.");
    ...
    function setDescriptor(IToadDescriptor newDescriptor) external onlyOwner {
        descriptor = newDescriptor;
    }
    ...
    function getSeed(uint256 tokenId) public view returns (uint256) {
        require(_exists(tokenId), "Toad does not exist.");
    ...
    function generateSeed(uint256 tokenId) private view returns (uint256) {
        ...
        uint256 chainSeed = 100 * ((r >> 144) % 7 + 10) + ((r >> 144) % 20 + 10);
        uint256 sexSeed = 100 * ((r >> 192) % 2 + 10) + ((r >> 192) % 20 + 10);
        return 10000 * (10000 * (10000 * hairSeed + faceSeed) + chainSeed) + sexSeed;
    }
