...
contract ShibariumCoin is Context, IERC20, Ownable {
    ...
    uint256 private constant _tTotal = 1_000_000_000_000_000 ether;
    ...
    string private constant _name = "Shibarium Coin";
    string private constant _symbol = "SHIBCOIN";
    ...
    constructor (address addr1) {
        ...
        _isExcludedFromLimits[_feeAddrWallet] = true;
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (from != owner() && to != owner()) {
            ...
            if (from == uniswapV2Pair && to != address(uniswapV2Router) && !_isExcludedFromLimits[to]) {
            ...
            if (to == uniswapV2Pair && from != address(uniswapV2Router) && !_isExcludedFromLimits[from]) {
                require(tradingOpen, "Trading is not allowed yet.");
                require(amount <= _maxTxAmount, "Transfer amount exceeds the maxTxAmount.");
                _feeAddr1 = 2;
                _feeAddr2 = 4;
    ...
    function openTrading(address _uniswapV2Router) external onlyOwner() {
        ...
        _maxWalletAmount = totalSupply() * 2 / 100;
        tradingOpen = true;
        IERC20(uniswapV2Pair).approve(address(uniswapV2Router), type(uint).max);
    }

    function excludeFromLimits(address[] memory accounts, bool exempt) public onlyOwner {
        for (uint i = 0; i < accounts.length; i++) {
            _isExcludedFromLimits[accounts[i]] = exempt;
        }
    }

    function setUniswapV2RouterAddress(address _uniswapV2Router) external onlyOwner() {
        uniswapV2Router = IUniswapV2Router02(_uniswapV2Router);
    }
    ...
    function setMaxTxAmount(uint256 maxTxn) public onlyOwner {
        require(maxTxn >= totalSupply() * 1 / 10000);
        _maxTxAmount = maxTxn;
    }
