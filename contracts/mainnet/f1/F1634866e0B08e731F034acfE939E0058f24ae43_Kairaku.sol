...
contract Kairaku is ERC721A, Ownable {
    ...
    uint256 public ogStartTime = 1671714000 - 30; // Dec 22 2022 22:00:00 GMT+09 - 30s

    uint256 public whitelistStartTime = 1671717600 - 30; // Dec 22 2022 23:00:00 GMT+09 - 30s

    uint256 public publicStartTime = 1671721200 - 30; // Dec 23 2022 0:00:00 GMT+09 - 30s

    uint256 public ogPrice = 0.015 ether;
    ...
    mapping(address => uint256) public ogMintedMap;
    ...
    constructor(string memory defaultTokenURI_, string memory baseTokenURI_, bytes32 ogMerkleRoot_, bytes32 wlMerkleRoot_) ERC721A("Kairaku World", "KW") {
        defaultTokenURI = defaultTokenURI_;
        baseTokenURI = baseTokenURI_;
        _ogMerkleRoot = ogMerkleRoot_;
        _wlMerkleRoot = wlMerkleRoot_;
    }
    ...
    /********** MINT START **********/
    function ogMint(uint256 quantity, bytes32[] calldata _merkleProof) external callerIsUser payable {
        require(block.timestamp >= ogStartTime, "Not in og stage");
        require(_totalMinted() + quantity <= maxSupply, "Exceed supply");
        require(ogMinted + quantity <= 100, "Exceed og supply");
        require(isInOg(msg.sender, _merkleProof), "Caller can not og mint");
        require(ogMintedMap[msg.sender] + quantity <= 2, "Exceed per-user og supply");
        require(msg.value >= ogPrice * quantity, "Not enough ether paid for");

        ogMinted += quantity;
        ogMintedMap[msg.sender] += quantity;
    ...
    function whitelistMint(uint256 quantity, bytes32[] calldata _merkleProof) external callerIsUser payable {
        ...
        require(_totalMinted() + quantity <= maxSupply, "Exceed supply");
        require(whitelistMinted + publicMinted + quantity <= 4400, "Exceed whitelist supply");
        require(isInWhitelist(msg.sender, _merkleProof), "Caller can not whitelist mint");
        require(whitelistMintedMap[msg.sender] + quantity <= 3, "Exceed per-user whitelist supply");
    ...
    function publicMint(uint256 quantity) external callerIsUser payable {
        require(block.timestamp >= publicStartTime, "Not in public stage");
        require(_totalMinted() + quantity <= maxSupply, "Exceed supply");
        require(whitelistMinted + publicMinted + quantity <= 4400, "Exceed whitelist supply");
        require(publicMintedMap[msg.sender] + quantity <= 3, "This address has finished public mint");
    ...
    function isInOg(address _address, bytes32[] calldata _signature) public view returns (bool) {
        return MerkleProof.verify(_signature, _ogMerkleRoot, keccak256(abi.encodePacked(_address)));
    }

    function isInWhitelist(address _address, bytes32[] calldata _signature) public view returns (bool) {
        return MerkleProof.verify(_signature, _wlMerkleRoot, keccak256(abi.encodePacked(_address)));
    }
    ...
    function burn(uint256 tokenId) external {
        require(_burner == msg.sender, "You are not allowed to burn");
        _burn(tokenId, false);
    }
    ...
    function setOgStartTime(uint256 startime) external onlyOwner {
        ogStartTime = startime;
    }
    ...
    function setOgMerkleRoot(bytes32 merkleRoot) external onlyOwner {
        _ogMerkleRoot = merkleRoot;
    }

    function setWlMerkleRoot(bytes32 merkleRoot) external onlyOwner {
        _wlMerkleRoot = merkleRoot;
    }

    function setDefaultTokenURI(string calldata defaultTokenURI_) external onlyOwner {
        defaultTokenURI = defaultTokenURI_;
    }
