...
contract ZEROHAND is Context, IERC20, Ownable {
    mapping (address => uint) private _owned;
    mapping (address => mapping (address => uint)) private _allowances;
    mapping (address => bool) private _isExcludedFromFee;
    mapping (address => bool) private _isBot;
    uint private constant _totalSupply = 1000000000000 * 10**9;

    string public constant name = "ZERO HAND";
    string public constant symbol = "19";
    ...
    function _getFee(bool takefee, bool buy) private view returns (uint) {
        uint fee = 0;
        if(takefee && block.timestamp >= _launchedAt + (5 hours)) {
            uint x = (block.timestamp - _launchedAt)/ 1 hours;
            if(buy) {
                fee = _bFee;
                if(x > 0){
                  fee = _bFee - x * 1;
    ...
    function openTrading() external onlyOwner() {
        ...
        _maxBuyTokens = 20000000000 * 10**9; 
