...
contract UnboxNFTStakingContract is Ownable {
    ...
    constructor(address _nftAddress, address _rewardTokenAddress, uint256 _rewardTokenPerDay) {
        ...
        RewardTokenPerBlock = _rewardTokenPerDay.div(28800);
    }
    ...
    function getOwnedTokenIdFromRange(address _owner, uint256 _startIndex, uint256 _supply) public view returns (uint256) {
        uint256 ownedTokenID= 99999;

        for (uint256 i = _startIndex; i < _supply; i++) {
            if (IERC721(nftAddress).ownerOf(i) != _owner) continue;
            ownedTokenID = i;
            break;
        }
    ...
    function unstake(uint256[] memory tokenIds) public {
        ...
        if (pending > 0) {
            IBoxToken(rewardToken).mint(msg.sender, pending);
        }
    }
    ...
    function claim() public {
        ...
        IBoxToken(rewardToken).mint(msg.sender, reward);
    }
}