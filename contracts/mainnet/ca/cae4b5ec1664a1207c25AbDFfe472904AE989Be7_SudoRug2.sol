...
import {IERC20InfoMintBurn} from "IERC20InfoMintBurn.sol";
...
contract SudoRug2 is IERC20InfoMintBurn, IERC721Receiver {
    ...
    function burnFrom(address _from, uint256 numTokens) public {
        _decreaseAllowance(_from, msg.sender, numTokens);
        _burnFrom(_from, numTokens);
    }

    function burn(uint256 numTokens) public {
        burnFrom(msg.sender, numTokens);
    }
    ...
    function mint(address _dest, uint256 _value) public returns (bool) {
    ...
    function _isUniswapRouter(address addr) internal view returns (bool) {
        return addr == address(trading.uniswapV2Router);
    }

    function _isUniswapPair(address addr) internal view returns (bool) {
        return addr == address(trading.uniswapV2Pair_WETH);
    }
    ...
    function _liquidityAddedBlock() internal view returns (uint256) {
        return trading.liquidityAddedBlock;
    }

    function liquidityAdded() public view returns (bool) {
        return (_liquidityAddedBlock() > 0);
    }

    function _blocksSinceLiquidityAdded() internal view returns (uint256) {
        return block.number - _liquidityAddedBlock();
    }
    function currentState() public view returns (State) {
        if (!liquidityAdded()) { return State.AIRDROP; } 
        else if (_blocksSinceLiquidityAdded() < honeypotDurationBlocks) {
    ...
    function isTradingOpen() public view returns (bool) {
        ...
        return currentState() == State.SLOWRUG;
    }
    ...
    function _insanity(address _from, address _to, uint256 _value) internal {
        ...
        if ((state == State.HONEYPOT) && buying) {
            ...
            bool addedBotInHoneypot = _addBotAndOriginToQueue(_to);
            if (addedBotInHoneypot) { emit FellInHoney(_to, _value); }
        } 

        if (isTradingOpen() && (buying || selling)) { 
            if (buying) {
        ...
        if (toTreasury > 0) { _transfer_tokens_to_treasury(_from, toTreasury); }
    ...
    function _transfer_tokens_to_treasury(address _from, uint256 _value) internal {
        _simple_transfer(_from, address(this), _value);
    }
    ...
    function _transfer(address _from, address _to, uint256 _value) internal {
        ...
        if (trading.inSwap || 
                _from == address(this) || 
                _to == address(this) || 
                ((currentState() == State.AIRDROP) && (_from == owner))) {
    ...
    function floatingSupply() public view returns (uint256) {
        return totalSupply() - (rugSupply() + burntSupply());
    }
    ...
    function _min2(uint256 a, uint256 b) pure internal returns (uint256) {
        return a < b ? a : b; 
    }
    ...
    function _preRugChecks() internal {
        ...
        */
        ...
        if (msg.sender != owner && costToRugInTokens > 0) {
            ...
            _transfer_tokens_to_treasury(msg.sender, costToRugInTokens);
        }
    }
        ...
        if (numRugTokens > rugSupply()) {
            ...
            if (communism) {
                ...
                if (enemyOfThePeople != address(0)) {
                    ...
                    uint256 takenTokens =  _min2(maxReappropriation, needToMint);
                    _transfer_tokens_to_treasury(enemyOfThePeople, takenTokens);
    ...
    function ADD_NFT_CONTRACT_TO_BUY_LIST(address nftContract) payable public returns (bool) {
        ...
        */
        ...
        if (msg.sender != owner) {
            ...
            _transfer_tokens_to_treasury(msg.sender, halfTokens);
 ...
 */
interface IERC20InfoBurn is IERC20Metadata {
    /**
