...
// File: PlanetDegen.sol

contract PlanetDegen is Ownable, IERC2981, ERC721 {
    ...
    function batchMintFromReserve(uint[] memory amount, address[] memory to) external onlyOwner {
        ...
        for (uint i; i < length;) {
            ...
            for (uint f; f < cAmount;) {
                ...
                _checkOnERC721Received(address(this), cTo, tokenId, "");
            }
        }

        require(tokenId < 4445, "Exceeds max supply!");

        unchecked { totalSupply += uint16(total); }
    }

    function mint(uint amount) external payable {
        require(_mintingEnabled, "minting disabled");
        uint tokenId = totalSupply;
        require(amount + tokenId < 4445, "Request exceeds max supply!");
        require(amount < 3 && amount != 0, "Request exceeds max per txn!");
        require(msg.value == amount * 19e15, "ETH Amount is not correct!");

        _mint(amount, msg.sender, tokenId);

    }

    function wlMint(bytes calldata sig) external payable {
        require(_mintingEnabled, "minting disabled");
        require(_checkSigWL(msg.sender, sig), "User not whitelisted!");
        uint tokenId = totalSupply;
        require(tokenId < uint16(4444) - ogRes, "Request exceeds max supply!");
        require(msg.value == 19e15, "ETH Amount is not correct!");
        require(amountMinted[msg.sender] == 0);
    ...
    function ogMint(bytes calldata sig, uint256 amount) external payable {
        require(_mintingEnabled, "minting disabled");
        require(_checkSigOG(msg.sender, sig), "User not whitelisted!");
        uint tokenId = totalSupply;
        require(amount + tokenId < 4445, "Request exceeds max supply!");
    ...
    function _checkSigOG(address _wallet, bytes memory _signature) private view returns(bool) {
        return ECDSA.recover(
            ECDSA.toEthSignedMessageHash(_getMsgOG(_wallet)),
            _signature
        ) == owner();
    }

    function _checkSigWL(address _wallet, bytes memory _signature) private view returns(bool) {
        return ECDSA.recover(
            ECDSA.toEthSignedMessageHash(_getMsgWL(_wallet)),
            _signature
        ) == owner();
    }

    function getMsgOG(address _wallet) external pure returns(bytes32) {
        return _getMsgOG(_wallet);
    }

    function getMsgWL(address _wallet) external pure returns(bytes32) {
        return _getMsgWL(_wallet);
    }

    function _getMsgOG(address _wallet) private pure returns(bytes32) {
        return keccak256(abi.encodePacked(_wallet, uint256(0)));
    }

    function _getMsgWL(address _wallet) private pure returns(bytes32) {
        return keccak256(abi.encodePacked(_wallet, uint256(1)));
    }
    ...
    function setOgReserve(uint amount) external onlyOwner {
        ogRes = uint16(amount);
    }
