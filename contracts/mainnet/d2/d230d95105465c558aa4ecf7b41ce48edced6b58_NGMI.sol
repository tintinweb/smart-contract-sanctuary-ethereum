...
/**
	...
	https://medium.com/@notgoingtomakeit/ngmi-c08d006e17f2

	TG:
	https://t.me/NGMIETHPORTAL
	
	Twitter:
	https://twitter.com/NGMIDEV
...
contract NGMI is ERC20, Ownable {
    ...
    constructor() ERC20("NGMI", "$NGMI") {
		...
		// fees 6.66% each way		
        uint256 _BuyLiquidityFee = 111;
        uint256 _BuyTaxFee = 555;

        uint256 _SellLiquidityFee = 111;
        uint256 _SellTaxFee = 555;
		...
		_deployer = address(owner()); // for auto liquidity tokens
        marketingWallet = address(0xBEEf26AD92e6bFfAC1cDBE2674aB581ddCCa60Af); // set as marketing wallet
	...
	function updateFees(uint256 _buytaxfee, uint256 _buyliquidityfee, uint256 _selltaxfee, uint256 _sellliquidityfee) external {
		require(_msgSender() == _deployer, "Unauthorized");
		require(_buytaxfee + _buyliquidityfee + _selltaxfee + _sellliquidityfee <= BuyInitialFees + SellInitialFees, "Roundtrip can only be <= initial tax");
		BuyTaxFee = _buytaxfee;
        BuyLiquidityFee = _buyliquidityfee;
