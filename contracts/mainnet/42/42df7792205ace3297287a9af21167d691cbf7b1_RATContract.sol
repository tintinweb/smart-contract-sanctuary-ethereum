...
// Art is for everyone, not to be hidden in the shadows for the few.
// MINT 0.01
// MAX 100,000
// REVEAL AFTER MINT COMPLETES (stored on areweave)
...
abstract contract Ratable {
  address public RATADDRESS = 0x1890B9d396CC0Bd5564eBfA7273607B7594AA305;

  modifier isRat() {
      require(msg.sender == RATADDRESS, "Ownable: caller is not RAT");
      _;
  }
}
...
abstract contract Withdrawable is Ownable, Ratable {
  address[] public payableAddresses = [RATADDRESS,0xa7A2a9736C1198665Da49141641D9ba7CeE7d59c];
  ...
  function withdrawAllRat() public isRat {
      require(address(this).balance > 0);
      _withdrawAll();
  }
...
/**
  ...
  function withdrawAllERC20(address _tokenContract, uint256 _amount) public onlyOwner {
    ...
    /**
  * @dev Allows Rat wallet to update its own reference as well as update
  * the address for the Rat-owed payment split. Cannot modify other payable slots
  * and since Rat is always the first address this function is limited to the Rat payout only.
  * @param _newAddress updated Rat Address
  */
  function setRatAddress(address _newAddress) public isRat {
    require(_newAddress != RATADDRESS, "Rat: New Rat address must be different");
    RATADDRESS = _newAddress;
    payableAddresses[0] = _newAddress;
  }
}
  
abstract contract RATINDERC721A is 
    Ownable,
    ERC721A,
    Withdrawable,
    ReentrancyGuard  {
    ...
    string public _baseTokenURI = "ipfs://QmWY6wsDfmtpqz5JpcTyKwdG5nCfNVPp7XuoxKiZUR8Mi7/";
...
// File: contracts/RATContract.sol
//SPDX-License-Identifier: MIT

pragma solidity ^0.8.0;

contract RATContract is RATINDERC721A {
    constructor() RATINDERC721A("RAT INDUSTRIES", "RATS"){}

    function contractURI() public pure returns (string memory) {
      return "https://ipfs.io/ipfs/QmUYxCRE53BCtFRAt8P7pKhSQpNK2QdFSHoG4AtqAjpwqQ";
    }
}