...
contract BOH is ERC721A, ERC2981, DefaultOperatorFilterer, Ownable {
  ...
  ERC721A(name, symbol)
  {
    ...
    _setDefaultRoyalty(address(this), royaltyAmount);
  ...
  function getAvailablePublicSaleQuantity() public view virtual returns (uint256) {
    return _totalSupply - getPublicSaleMintedTotal();
  }

  /**
   * @notice Increase the number of minted pieces according to the sale status
   */
  function _increaseMintedCount(uint256 quantity) internal {
   ...
   */
  function airdrop(uint256 quantity, address recipient, SaleStatus countInreaseSaleStatus) external onlyOwner {
    if (_currentIndex + (quantity - 1) > _totalSupply) revert SoldOut();
    if (countInreaseSaleStatus == SaleStatus.CLOSED) revert InvalidSaleStatus();
