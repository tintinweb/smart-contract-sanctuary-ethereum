...
contract Linii is ERC721AQueryable, Ownable, ReentrancyGuard, DefaultOperatorFilterer {
  ...
  function whitelistMint(uint256 _mintAmount, bytes32[] calldata _merkleProof) public payable mintCompliance(_mintAmount) {
    ...
    //Check if the buyer have already claimed 1 token free
    if(!freeMintClaimed[_msgSender()]){
    //The free token is not claimed yet
        if(_mintAmount>1){
        //The buyer asked to mint N tokens, so he will pay N-1 tokens
            require(msg.value >= cost * (_mintAmount-1), 'Insufficient funds!');
        }
    //Ok, this call is free but only this
        freeMintClaimed[_msgSender()]=true;
    }else{
    // The buyer have already claimed a free token 
    // so he have to pay for all N tokens he asked to mint
        require(msg.value >= cost * _mintAmount, 'Insufficient funds!');
    }
  ...
  function mint(uint256 _mintAmount) public payable mintCompliance(_mintAmount) {
    ...
    //Ok, this Mint have a token free, but only this Mint
        freeMintClaimed[_msgSender()]=true;
    }else{
    // The buyer have already claimed a free token 
    // so he have to pay for all N tokens he asked to mint
