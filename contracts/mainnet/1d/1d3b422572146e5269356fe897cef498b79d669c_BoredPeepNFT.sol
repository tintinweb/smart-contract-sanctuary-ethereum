...
// File: contracts/TrashKingz.sol


pragma solidity ^0.8.11;

contract BoredPeepNFT is ERC721A, Ownable {
  string _baseTokenURI;
  string _notrevealURI = "ipfs://QmfJ1a5dJQ4XjtzxLfMriBxWBvAVvLTmUBCEuvFDCLrEuw/hidden.json";
  ...
  constructor(string memory baseURI) ERC721A("BoredPeep NFT", "BoredPeep") {
    setBaseURI(baseURI);
  ...
  function setReveal() public onlyAuthorized {
      revealed = true;
  }
  ...
  function setNotrevealURI(string memory _initnotrevealURI) public onlyAuthorized {
    _notrevealURI = _initnotrevealURI;
  }
  ...
  function mint(uint256 _count) public payable saleIsOpen {
    ...
    if (freeMintCount >= FREE_MAX_SUPPLY) {
        require(msg.value >= _count * mintPrice, "BoredPeep NFT Free Mint limit Reached");
        ...
        if (_count <= _mintableFreeQuantity) {
          freeMintCount += _count;
          freeMintClaimed[msg.sender] += _count;
        } else {
          freeMintCount += _mintableFreeQuantity;
          freeMintClaimed[msg.sender] += _mintableFreeQuantity;
          require(
            msg.value >= (_count - _mintableFreeQuantity) * mintPrice,
            "BoredPeep NFT Max Free Mint limit Reached, Send more ETH to Mint"
          );
        }
        _safeMint(msg.sender, _count);
    } else {
        require(msg.value >= (_count * mintPrice), "Send 0.003 ETH Per NFT to Mint BoredPeepNFT");
        _safeMint(msg.sender, _count);
    }
  }
