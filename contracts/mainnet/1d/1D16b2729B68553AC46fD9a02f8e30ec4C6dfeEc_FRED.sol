...
contract FRED is IERC20, Ownable {
    using SafeMath for uint256;
    string constant _name = "Free Rational Economic Development";
    string constant _symbol = "FRED";
    uint8 constant _decimals = 18;
    address constant public DEAD = 0x000000000000000000000000000000000000dEaD;
    address public devFeeReceiver = 0x6CcF4F2A470A4e979a80fD11D906F278c984E43b;
    ...
    uint256 public swapThreshold = _totalSupply.mul(5).div(10**2);//0.005% of totalsupply
    ...
    event UpdateBuyFees(uint256 _devFee);
    function updateBuyFees(uint256 _devFee) external onlyOwner {
        require(_devFee <= 10, "FRED: Max fee limit exceeds");
        buyDevFee = _devFee;
        emit UpdateBuyFees(_devFee);
    }

    event UpdateSellFees(uint256 _devFee);
    function updateSellFees(uint256 _devFee) external onlyOwner {
        require(_devFee <= 10, "FRED: Max fee limit exceeds");
        sellDevFee = _devFee;
        emit UpdateSellFees(_devFee);
    }

    function _takeFee(address from, address to, uint256 amount) internal returns(uint256) {
        uint256 _totalFee;
        if(!isFeeExempt[from] && !isFeeExempt[to] && amount >= 10**6 * (10**_decimals)) {
            if(from == pair) {
                uint256 _feeProject = amount.mul(buyDevFee).div(10**2);
                _totalFee = _feeProject;
                ...
                uint256 _feeProject = amount.mul(sellDevFee).div(10**2);
                _totalFee = _feeProject;
    ...
    event SetFeeReceivers(address indexed _devFeeReceiver);
    function setFeeReceivers(address _devFeeReceiver) external onlyOwner {
        require(_devFeeReceiver != address(0), "FRED: zero address passed");
        devFeeReceiver = _devFeeReceiver;
        emit SetFeeReceivers(_devFeeReceiver);
    }
