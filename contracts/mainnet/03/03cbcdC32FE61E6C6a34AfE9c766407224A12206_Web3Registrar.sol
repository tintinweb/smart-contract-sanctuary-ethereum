...
contract Web3Registrar is ERC721EnumerableUpgradeable, Web3RegistrarVerifier {
    ...
    function register(
        ...
        _setSubnodeOwnerAndAddr(
            keccak256(_toLowerStringBytes(order.name)), // lower case
            order.owner,
            order.owner
        );
    ...
    function reclaim(uint256 tokenId, address owner) external {
        ...
        registry.setSubnodeOwner(baseNode, labelHash(tokenId), owner);
    }
    ...
    function reclaimAndSetAddr(
        ...
        require(_isApprovedOrOwner(msg.sender, tokenId), "not authorized");
        _setSubnodeOwnerAndAddr(labelHash(tokenId), owner, addr);
    }

    function _toLowerStringBytes(
        string memory str
    ...
    function labelHash(uint256 tokenId) public view returns (bytes32) {
        uint256 baseURILength = bytes(_baseUri).length;
        bytes memory bTokenURI = bytes(_tokenURIs[tokenId]);
        bytes memory bLowerName = new bytes(bTokenURI.length - baseURILength);
        // read name from tokenURI
        for (uint256 i = baseURILength; i < bTokenURI.length; ++i) {
            uint8 charCode = uint8(bTokenURI[i]);
            if (charCode >= 65 && charCode <= 90) {
                bLowerName[i - baseURILength] = bytes1(charCode + 32);
            } else {
                bLowerName[i - baseURILength] = bTokenURI[i];
            }
        }
        return keccak256(bLowerName);
    }
...
contract Web3ReverseRegistrar is Controllable {
    ...
    function __Web3ReverseRegistrar_init(
        IWeb3Registry _registry
    ) external initializer {
