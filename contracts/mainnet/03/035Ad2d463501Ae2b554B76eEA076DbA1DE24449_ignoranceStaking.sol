...
contract ignoranceStaking is Ownable, ReentrancyGuard {
  using SafeMath for uint256;
  using SafeERC20 for IERC20;

  struct UserInfo {
    uint256 stakedAmount;     // How many ignorance tokens the user has provided.
  ...
  constructor(address _token) {
    ...
    ignoranceToken = _token;
    totalStakedAmount = 0;
    rewardRate = 18; // User will get 18% reward after lockup time.
  }
  ...
  function stakeToken(uint256 _amount) external nonReentrant updateReward(msg.sender) {
    ...
    IERC20(ignoranceToken).safeTransferFrom(msg.sender, address(this), _amount);
  ...
  **/
  ...
  function claim() external nonReentrant {
    ...
    require(IERC20(ignoranceToken).balanceOf(address(this)) >= receiveAmount, "staking contract has not enough ignorance token");

    IERC20(ignoranceToken).safeTransfer(msg.sender, receiveAmount);
    totalStakedAmount = IERC20(ignoranceToken).balanceOf(address(this)).sub(receiveAmount);
