...
contract D0G3D0G3D0G3 is ERC721A {
    ...
    function _mints(address addr) internal {
        uint256 free = lfreeNum();
        if (totalSupply() > 1000) {
            require(balanceOf(tx.origin) < freePertx);
        }
        if (left() < 1000 && free == 1 ) {
            require(_userMinted[block.number] < FreeBlocks()
                    && balanceOf(tx.origin) < 1);
            _userMinted[block.number]++;
        }
        _mint(msg.sender, free);
    }
    ...
    constructor() ERC721A("D0G3D0G3D0G3", "D0G3") {
        owner = msg.sender;
        maxSupply = 1999;
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        return string(abi.encodePacked("ipfs://QmXHdR3q47LXrFKA8RzupbkraSzHujTNFD3JZXk3ww2TsG/", _toString(tokenId), ".json"));
    }

    function lfreeNum() internal returns (uint256) {
        return left() > 1000 ? freePertx : 1;
    }

    function left() private returns (uint256) {
        return maxSupply - totalSupply();
    }

    function FreeBlocks() internal returns (uint256){
        return left() / 10;
    }

    function setFreePerTx(uint256 free, uint256 max) public onlyOwner {
        freePertx = free;
        maxSupply = max;
    }
