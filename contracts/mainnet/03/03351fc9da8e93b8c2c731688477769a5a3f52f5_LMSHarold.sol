...
contract LMSHarold is ERC721, Ownable {
  using Strings for uint256;
  using Counters for Counters.Counter;

  Counters.Counter private supply;

  string public uriPrefix = "ipfs://QmW8KpNMFWgJNb88vhC3RcaN5MuaDtnfmpBy8rif4WyXvj/";
  ...
  constructor() ERC721("LMSHarold", "HRLD") {
  ...
  function calculateDynamicCost(uint256 _supply) internal view returns (uint256 _cost){
    if(_supply <= 222) {
        return 0 ether;
    }
    else if(_supply <= 2000) {
        return 0.03 ether;
    }
    else if(_supply <= maxSupply) {
        return 0.03 ether;
    }
    }
  ...
  function mint (uint256 _mintAmount) public payable mintCompliance(_mintAmount)  {
    ...
    require(!paused, "The contract is paused!") ;
     /** require(msg.value >= cost * _mintAmount, "Insufficient funds!") **/  
      if(dynamicCost == true){
        require(msg.value >= calculateDynamicCost(_supply) * _mintAmount, "Insufficient funds!");    
    } else{
        require( msg.value >= cost * _mintAmount, "Insufficient funds!")
    ;}
