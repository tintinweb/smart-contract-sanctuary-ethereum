...
// INFINITY Finance
//
// https://infinifi.finance
// https://twitter.com/InfiniFiETH
// https://t.me/InFiPortal
//
// Infinity Finance is bringing accessible and ethical 
// DeFI to the masses through simple one click interfaces 
// monitored by fully transparent DAO governance contracts. 
//
//
//        _     _
//      /   \ /   \
...
contract InFi is Ownable, IERC20 {
    ...
    // Explicit Sales Fee
    uint256 public totalSaleFeeBPS = 1000;
    ...
    uint256 public maxTxBPS = 400;
    uint256 public maxWalletBPS = 500;

    bool isOpen = false;

    // Simple anti-bot based on blocks from isOpen
	uint256 public antiBlockNum = 1;
	bool public antiEnabled = false;
	uint256 public antiBotTimestamp;
        ...
        returns (bool)
    {
        ...
        require(
            ...
            "InFi: decreased allowance below zero"
        );
        _approve(_msgSender(), spender, currentAllowance - subtractedValue);
        return true;
    }
    ...
    function transferFrom(
        ...
        require(
            ...
            "InFi: transfer amount exceeds allowance"
        );
        _approve(sender, _msgSender(), currentAllowance - amount);
        return true;
    }
    ...
    function enableAntiBot(bool _bool) public onlyOwner {
        antiEnabled = _bool;
    }
    ...
    function _transfer(
        ...
        require(!isBlacklisted[sender], "InFi: Sender is blacklisted");
        require(!isBlacklisted[recipient], "InFi: Recipient is blacklisted");

        require(sender != address(0), "InFi: transfer from the zero address");
        require(recipient != address(0), "InFi: transfer to the zero address");
        ...
        require(
            ...
            "InFi: transfer amount exceeds balance"
        );
        ...
        if (takeFee) {
            uint256 totalFeeToTake = totalFeeBPS;

            // Is Sale 
            if (recipient == uniswapV2Pair)
            {
                totalFeeToTake = totalSaleFeeBPS;
            }

            uint256 fees = (amount * totalFeeToTake) / 10000;
            amount -= fees;
            _executeTransfer(sender, address(this), fees);
        }

         // Check antibot, blacklist from future TX if purchasing from Uniswap
    ...
    function _approve(
        ...
        require(owner != address(0), "InFi: approve from the zero address");
        require(spender != address(0), "InFi: approve to the zero address");
        _allowances[owner][spender] = amount;
        emit Approval(owner, spender, amount);
    }

    function _mint(address account, uint256 amount) private {
        require(account != address(0), "InFi: mint to the zero address");
        _totalSupply += amount;
        _balances[account] += amount;
        emit Transfer(address(0), account, amount);
    }
    ...
    function _burn(address account, uint256 amount) private {
        require(account != address(0), "InFi: burn from the zero address");
        uint256 accountBalance = _balances[account];
        require(accountBalance >= amount, "InFi: burn amount exceeds balance");
        _balances[account] = accountBalance - amount;
        _totalSupply -= amount;
        emit Transfer(account, address(0), amount);
    }
    ...
    function _executeSwap(uint256 tokens, uint256 native) private {
        ...
        uint256 swapTokensDev;
        if (address(devWallet) != address(0)) {
            swapTokensDev = (tokens * devFeeBPS) / totalFeeBPS;
        }
        ...
        uint256 tokensForLiquidity = tokens -
            swapTokensMarketing - swapTokensDev - swapTokensDividends;
        ...
        uint256 swapTokensTotal = swapTokensMarketing +
            swapTokensDividends + swapTokensDev + swapTokensLiquidity;
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(
            ...
            "InFi: account is already set to requested state"
        );
        _isExcludedFromFees[account] = excluded;
    ...
    function setAutomatedMarketMakerPair(address pair, bool value)
        ...
        onlyOwner
    {
        require(pair != uniswapV2Pair, "InFi: DEX pair can not be removed");
    ...
    function setFee(
        ...
        devFeeBPS = _devFee;
        totalFeeBPS = _treasuryFee + _liquidityFee + _dividendFee + _devFee;
    }

    function setSalesFee(
        uint256 _salesFee
    ) external onlyOwner {
        totalSaleFeeBPS = _salesFee;
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(
            automatedMarketMakerPairs[pair] != value,
            "InFi: automated market maker pair is already set to that value"
        );
        automatedMarketMakerPairs[pair] = value;
        if (value) {
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(
            ...
            "InFi: the router is already set to the new address"
        );
    ...
    function compound() public {
        require(compoundingEnabled, "InFi: compounding is not enabled");
...
contract DividendTracker is Ownable, IERC20 {
    ...
    string private _name = "InFi_DividendTracker";
    string private _symbol = "InFi_DividendTracker";
        ...
        onlyOwner
    {
        require(
            ...
            "InFi_DividendTracker: account already set to requested state"
        );
        excludedFromDividends[account] = excluded;
        if (excluded) {
            _setBalance(account, 0);
    ...
    function _mint(address account, uint256 amount) private {
        require(
            account != address(0),
            "InFi_DividendTracker: mint to the zero address"
        );
        _totalSupply += amount;
        _balances[account] += amount;
        emit Transfer(address(0), account, amount);
    ...
    function _burn(address account, uint256 amount) private {
        require(
            account != address(0),
            "InFi_DividendTracker: burn from the zero address"
        );
        uint256 accountBalance = _balances[account];
        require(
            accountBalance >= amount,
            "InFi_DividendTracker: burn amount exceeds balance"
        );
        _balances[account] = accountBalance - amount;
        _totalSupply -= amount;
        emit Transfer(account, address(0), amount);
    ...
    function transfer(address, uint256) public pure override returns (bool) {
        revert("InFi_DividendTracker: method not implemented");
    }
