...
interface IERC721 is IERC165 {
    ...
    function createCollectible(address from, string memory _tokenURI, address[] memory _royalty, uint256[] memory _royaltyFee) external returns(uint256);
}
...
contract ERC721 is Context, ERC165, IERC721, IERC721Metadata, IERC721Enumerable {
  ...
  // mapping for token creator
    ...
    */
     ...
     * @param _royalty address of the token to be minted.
     * @param _royaltyFee uint256 royalty of the token to be minted.     
     */

    function createCollectible(address from, string memory _tokenURI, address[] memory _royalty, uint256[] memory _royaltyFee) external override returns (uint256) {
    ...
    function _safeMint(address to, uint256 tokenId, address[] memory _royaltyAddress, uint256[] memory _royaltyFee, bytes memory _data) internal virtual {
        _mint(to, tokenId);
        setRoyaltyFee(tokenId, _royaltyAddress, _royaltyFee);
    ...
    function mintAndTransfer(address from, address to, address[] memory _royaltyAddress, uint256[] memory _royaltyFee, string memory _tokenURI, bytes memory data) external virtual override returns(uint256){
