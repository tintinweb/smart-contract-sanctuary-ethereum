...
contract MarketplaceV2 is Initializable, Pausable, AccessControlUpgradeable, IMarketplace {
    ...
    {
        uint256 tokenId = _createNftItem(msg.sender, _uri, _quantity, _galleryFee, _galleryAddress, _collaborators, _collaboratorsFee, _signature);
    ...
    external returns (uint256)
    {
        ...
        uint256 tokenId = _createNftItem(msg.sender, _uri, 1, _galleryFee, _galleryAddress, _collaborators, _collaboratorsFee, _signature);
    ...
    internal returns (uint256) {
        ...
        bytes32 hashWithoutPrefix = keccak256(
            abi.encode(_creator, keccak256(bytes(_uri)), _quantity, _galleryFee, _galleryAddress)
        );

        address signer = recoverSigner(prefixed(hashWithoutPrefix), _signature);

        require(
            hasRole(AUCTION_FINISHER_ROLE, signer),
            "signer must have role"
        );

        return aisNft.createNftItem(_creator, _uri, _quantity, _galleryFee, _galleryAddress, _collaborators, _collaboratorsFee);
    }
