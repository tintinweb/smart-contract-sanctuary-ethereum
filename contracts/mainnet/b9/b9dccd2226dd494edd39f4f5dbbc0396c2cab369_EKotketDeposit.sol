...
interface EKotketGatewayOracleInterface {
...
contract EKotketDeposit is EGovernanceBase, IERC721Receiver{
    ...
    modifier onlyGatewayOracleUserPermission() {
        EKotketGatewayOracleInterface kotketGatewayOracle = EKotketGatewayOracleInterface(governance.kotketGatewayOracleAddress());
        require(kotketGatewayOracle.hasRole(DEFAULT_ADMIN_ROLE, msg.sender) || kotketGatewayOracle.hasRole(SC_GATEWAY_ORACLE_ROLE, msg.sender), "Dont have Gateway Oracle permission!");
        _;
    }
    
    function transferKotketAccessToWallets() public onlyGatewayOracleUserPermission{
        address payable walletAddress = payable(governance.kotketWallet());
        address fundWalletAddress = governance.kotketFundWallet();

        EKotketTokenInterface kotkettoken = EKotketTokenInterface(governance.kotketTokenAddress());
        uint tokenBalance = kotkettoken.balanceOf(address(this));
        if (tokenBalance > 0){
            uint fundWalletAmount = tokenBalance.div(2);
            uint walletAmount = tokenBalance.sub(fundWalletAmount);
            kotkettoken.transfer(fundWalletAddress, fundWalletAmount);
            kotkettoken.transfer(walletAddress, walletAmount);
        }

        EUSDTInterface usdt = EUSDTInterface(governance.usdtAddress());
        uint usdtBalance = usdt.balanceOf(address(this));
        if (usdtBalance > 0){
            uint fundWalletUsdtAmount = usdtBalance.div(2);
            uint walletUsdtAmount = usdtBalance.sub(fundWalletUsdtAmount);
            usdt.transfer(fundWalletAddress, fundWalletUsdtAmount);
            usdt.transfer(walletAddress, walletUsdtAmount);
        }

        EKotketNFTInterface kotketNFT = EKotketNFTInterface(governance.kotketNFTAddress());
        uint256[] memory ids = kotketNFT.tokensOwned(address(this));
        if (ids.length > 0){
            for (uint256 i = 0; i < ids.length; i++) {
                uint256 tokenId = ids[i];
                kotketNFT.safeTransferFrom(address(this), walletAddress, tokenId);
            }
        }
    ...
    function transferKotketAccess(address payable _recipient) public onlyGatewayOracleUserPermission{
        _transferKotketAccess(_recipient);
    }

    function _transferKotketAccess(address payable _recipient) internal {
        EKotketTokenInterface kotkettoken = EKotketTokenInterface(governance.kotketTokenAddress());
        uint tokenBalance = kotkettoken.balanceOf(address(this));
        if (tokenBalance > 0){
            kotkettoken.transfer(_recipient, tokenBalance);
        }

        EUSDTInterface usdt = EUSDTInterface(governance.usdtAddress());
        uint usdtBalance = usdt.balanceOf(address(this));
        if (usdtBalance > 0){
            usdt.transfer(_recipient, usdtBalance);
        }
        ...
        if (ids.length > 0){
            for (uint256 i = 0; i < ids.length; i++) {
                uint256 tokenId = ids[i];
                kotketNFT.safeTransferFrom(address(this), _recipient, tokenId);
            }
        }
