...
contract BitStackerNFT is Ownable, ERC1155Supply {
    ...
    function mint(Category _category, uint256 _amount) payable public {
        ...
        require(privateSale || publicSale, "Sale is not active");

        if(privateSale){
            require(_category == Category.VIPBLACK || _category == Category.VIPBLUE, "While presale, only VIP tokens are avaialbe to mint");

            if(_category == Category.VIPBLACK){
                uint totalCost = vipBlack.price * _amount;
                require(msg.value >= totalCost, "Insufficient Funds for this purchase");
                uint totalTerraHashesMinting = vipBlack.hashRate * _amount;
                require(totalPresaleTerraHashesSold() + totalTerraHashesMinting <= THForPresale, "Not enough hashrate avaialbe");
            ...
            require(_category == Category.BLACK || _category == Category.BLUE, "While public sale, only nonVIP tokens are avaialbe to mint");

            if(_category == Category.BLACK){
                uint totalCost = black.price * _amount;
                require(msg.value >= totalCost, "Insufficient Funds for this purchase");
                uint totalTerraHashesMinting = black.hashRate * _amount;
                require(totalPublicsaleterraHashesSold() + totalTerraHashesMinting <= THForPublicsale, "Not enough hashrate avaialbe");
    ...
    function uri(uint256 _tokenid) override public view returns (string memory) {
        require(exists(_tokenid), "Not exist");
        return string(
            abi.encodePacked(
                baseURL,
    ...
    function withdrawFunds() public onlyOwner {
        ...
        require(res1, "cannot send funds to owner"); 
    }
