    ...
    IClaimCondition
{
    ...
    function withdraw(address tokenAddress) external override onlyOwner {
        ...
        if (tokenAddress == NATIVE_TOKEN) {
            ...
            tokenContract.transferFrom(address(this), _msgSender(), balance);
        }
    }
    ...
    function _transferTokens(
        ...
        if (tokenAddress == NATIVE_TOKEN) {
            ...
            require(success, "Transfer native tokens failed!");
        } else {
            // transfer ERC20 tokens
            IERC20 tokenContract = IERC20(tokenAddress);
            // in case of withdraw, it needs to execute `approve()` on the token contract to allow itself the transfer
            if (from == address(this)) tokenContract.approve(from, amount);
