...
contract WeedDoge is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;
    
    address payable public marketingAddress = payable(0x164a15F502b6d07d958C1B89d56535826b64C23f); // Marketing Address
    ...
    string private _name = "Weed Doge";
    string private _symbol = "$WOGE";
    ...
    uint256 public maxTXwallet =50000000e9;

    uint256 public airdropBal=398406374000000;
  ...
  function ExcludeLimitcheck(address _addr,bool _status) public onlyOwner() {
    ...
    function _transfer(
        ...
        if(from == uniswapV2Pair && to != address(uniswapV2Router) && !_isExcludedFromFee[to]) {
            ...
            if (block.timestamp == launchTime) {
             ...
             if(!Limtcheck[to]){ require(balanceOf(to).add(amount) <= maxTXwallet,"Tx Limit Exceed");
         }
    ...
    function setmaxTXwallet(uint256 amount) external onlyOwner() {
        maxTXwallet = amount;
    }
