...
contract uwu is ERC721, Owned {
    ...
    uint256 public reservedTokens = 41;
    ...
    function setRoot1(bytes32 newRoot) public onlyOwner {
        merkleRoot1 = newRoot;
    }

    function setRoot2(bytes32 newRoot) public onlyOwner {
        merkleRoot2 = newRoot;
    }

    function setReserved(uint256 newReserve) public onlyOwner {
    ...
    function mint1(bytes32[] calldata proof) public {
        bytes32 leaf = bytes32(uint256(uint160(msg.sender)));
        if (!MerkleProofLib.verify(proof, merkleRoot1, leaf)) revert InvalidProof();
        ...
        if (reservedTokens > 0) reservedTokens--;
        _mint(msg.sender, nextTokenId++);
    }

    function mint2(bytes32[] calldata proof) public {
        bytes32 leaf = bytes32(uint256(uint160(msg.sender)));
        if (!MerkleProofLib.verify(proof, merkleRoot2, leaf)) revert InvalidProof();

        if (nextTokenId > MAX_SUPPLY - reservedTokens) revert SoldOut();
