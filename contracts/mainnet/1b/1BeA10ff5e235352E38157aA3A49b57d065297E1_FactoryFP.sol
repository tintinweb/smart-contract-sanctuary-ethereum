...
contract FactoryFP is Ownable {
    bytes32 public constant INIT_CODE_HASH = keccak256(abi.encodePacked(type(FarmingPool).creationCode));

    uint256 private _index;

    IFungibleToken public immutable token;

    event CreatePool(uint256 id, address pool);
    ...
    function deploy(
        ...
        bytes32 salt = keccak256(abi.encodePacked(_depositToken, token));
        bytes memory bytecode = type(FarmingPool).creationCode;
        address pool = Create2.deploy(0, salt, bytecode);
        IFarmingPool(pool).initialize(_depositToken, token, _mintFromWallet, _admin, _tokensPerBlock, _startBlock);
        _index++;
    ...
    // @dev Get storage address by {_depositToken}.
    function getStorageAddress(IFungibleToken _depositToken) external view returns (address pool) {
        bytes32 salt = keccak256(abi.encodePacked(_depositToken, token));
