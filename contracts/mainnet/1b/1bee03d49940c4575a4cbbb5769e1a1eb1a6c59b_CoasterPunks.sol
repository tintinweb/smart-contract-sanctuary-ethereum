...
// File: contracts/CoasterPunks.sol


pragma solidity >=0.7.0 <0.9.0;


contract CoasterPunks is ERC721, Ownable {
  ...
  uint256 public cost = 450000000000000000; //0.45 ETH
  ...
  address PlatypusBlockchainInc = 0x894A7c92A966BD3c3D62e9AB02D9aa68500dBf8c;
  address CoasterPunksSeries1 = 0x896Bc39781ff4A061fBae7b8ac44eC18534ad3AF;
  address cpDesign = 0x04FE9F8f92AC03D463a9809b7badeB19A76457e2;
  address cpDev = 0x897be7387bcd8CD2518ce3b61b300DbD59982dee;
  ...
  constructor() ERC721("Coaster Punks", "CP") {
  ...
  function addPresaleList(address[] calldata addresses, uint8 numAllowedToMint) external onlyOwner {
    for (uint256 i = 0; i < addresses.length; i++) {
        presaleList[addresses[i]] = numAllowedToMint;
    }
  }

  function isPresaleList(address addr) external view returns (uint256) {
  ...
  function withdraw() external onlyOwner {
        ...
        uint256 balance = address(this).balance;
        require(balance > 0);
        payable(PlatypusBlockchainInc).transfer((balance * 200) / 1000);
        payable(CoasterPunksSeries1).transfer((balance * 790) / 1000);
        payable(cpDesign).transfer((balance * 5) / 1000);
        payable(cpDev).transfer((balance * 5) / 1000);
    }
