...
contract BAM is ERC721Enumerable, Ownable {
  using Strings for uint256; 
  ...
  function setLastClaimStamp(uint _tokenId) external {
    require(msg.sender == spinalFluid, "Function only callable by HonourToken contract");
  ...
  function setSpinalfluidAddress(address _address) public onlyOwner {
    spinalFluid = _address;
  }
}