...
contract TokenPaymaster is BasePaymaster {
    ...
    function getTokenToEthOutput(uint256 amountIn, address[] memory path)
        public
        view
        returns (uint256)
    {
        ...
        returns (bytes memory context, bool revertOnRecipientRevert)
    {
        ...
        if (_paymentToken == tokenAddress) {
            require(
                paymentToken.allowance(payer, target) >= _fee + amount,
                "Fee+amount: Not enough allowance"
            );
            require(
                paymentToken.balanceOf(payer) >= _fee + amount,
                "Fee+amount: Not enough balance"
            );
        } else {
            require(
                paymentToken.allowance(payer, target) >= _fee,
                "Fee: Not enough allowance"
            );
            require(
                paymentToken.balanceOf(payer) >= _fee,
                "Fee: Not enough balance"
            );

            IERC20 token = IERC20(tokenAddress);
            require(
                token.allowance(payer, target) >= amount,
                "Not enough allowance"
            );
            require(
                paymentToken.balanceOf(payer) >= amount,
                "Not enough balance"
            );
        }
        ...
        uint256 ethPrecharge = relayHub.calculateCharge(
            maxPossibleGas,
            relayData
        );
    ...
    function postRelayedCall(
        ...
        (address payer, uint256 ethPrecharge, uint256 amountOut) = abi.decode(
            context,
