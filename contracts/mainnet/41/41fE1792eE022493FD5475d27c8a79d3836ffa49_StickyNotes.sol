...
contract StickyNotes is ERC721, ERC721Enumerable, AccessControl, Ownable {
  ...
  address public bank = 0x8CF176a175418f91F019441dC49fBCDA8912EC35;
  mapping(uint256 => StickyData) public stickies;
  string public style =
    ".bodyText{font-size: 16px;font-family: 'Courier';line-height: 1.4rem;text-align: left;}";
  ...
  constructor() ERC721("Sticky Notes", "NOTES") {
  ...
  function mint(
    ...
    stickies[counter].message = message;
    stickies[counter].isSBT = isSBT;
    stickies[counter].creator = msg.sender;
    stickies[counter].color = color;
    stickies[counter].title = title;
    _mint(to, counter);
    counter++;
  ...
  function getSvg(uint256 tokenId) public view returns (string memory) {
    ...
    parts[
      4
    ] = "</style><foreignObject x='76' y='130' width='490' height='420'><div xmlns= 'http://www.w3.org/1999/xhtml' > <p id='text' class='bodyText'> stickynotes.wtf </p></div><script>var input = '";
    parts[5] = Base64.encode(abi.encodePacked(message));
    parts[
      6
    ] = "'; var test = atob(input); document.querySelector('.bodyText').innerText = test;</script></foreignObject></svg>";
    return
      Base64.encode(
        abi.encodePacked(
          parts[0],
    ...
    returns (string memory)
  {
    ...
    bool isSBT = stickies[tokenId].isSBT;
    string memory title = stickies[tokenId].title;
    string memory json = Base64.encode(
      abi.encodePacked(
        '{"description":"Send on-chain notes to any ETH wallet. StickyNotes.wtf","external_url":"https://stickynotes.wtf/","name":"',
        bytes(title).length > 0 ? title : "Sticky Note #",
