...
interface ITrackerAddon is ITokenVault {
    function tokenTrackers(address asset) external returns (IRevest.TokenTracker memory);
}
 ...
 */
contract RevestA3_3 is IRevest, RevestAccessControl, RevestReentrancyGuard {
    ...
    function _withdrawFNFT(uint fnftId, uint quantity) private {
        ...
        IRevest.TokenTracker memory tracker = ITrackerAddon(vault).tokenTrackers(config.asset);
        {
            uint lastBal = tracker.lastBalance;
            uint currentAmount;
            if(config.asset != address(0)) {
                currentAmount = IERC20(config.asset).balanceOf(vault);
            } else {
                currentAmount = lastBal;
            }
            tracker.lastMul = lastBal == 0 ? (1 ether) : (1 ether) * currentAmount / lastBal;
        }
        ...
        // Address edge-case of deposit griefing in TokenVaultV1, breaks rebase tokens
        // Solution pending deployment of TokenVaultV2, which will restore functionality
        if(config.depositMul != tracker.lastMul) {
            config.depositMul = tracker.lastMul;
            triggerRemap = true;
        }
