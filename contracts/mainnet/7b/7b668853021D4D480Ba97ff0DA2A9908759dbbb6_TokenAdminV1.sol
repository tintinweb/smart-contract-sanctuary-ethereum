...
// Custom
import "./TokenAdminStorage.sol";

contract TokenAdminV1 is TokenAdminStorageV1 {
    ...
    function initialize(address initialAdmin) external initializer {
        ...
        // Set the initial admin capable of adding and removing minters
        // This is separate from the contract owner, so it will not have upgrade privileges
    ...
    /*
    To ADD a minter, use the _admin account set in initialize() to call:
    
    grantRole(MINTER, address newMinter)

    ----------
    
    To REMOVE a minter, use the _admin account set in initialize() to call:
    
    revokeRole(MINTER, address existingMinter)
    */
    function mintERC20(
        ...
        address _owner
    ) external onlyRole(MINTER) nonReentrant returns (address) {
        bytes32 tokenId = generateTokenId(_symbol, _name);

        require(
            tokenToAddress[tokenId] == address(0),
            "Token has already been minted"
        );

        address tokenAddress = TokenFactory(tokenFactory).mintERC20(
            _name,
            _symbol,
            _initialSupply,
            _owner
        ...
        tokenToAddress[tokenId] = tokenAddress;
        tokenToIndex[tokenId] = tokens.length;
        tokens.push(tokenId);
    ...
    function removeToken(string calldata _symbol, string calldata _name)
        external
        onlyRole(MINTER)
    {
        bytes32 tokenId = generateTokenId(_symbol, _name);
        ...
        if (index < len - 1) {
            bytes32 lastToken = tokens[len - 1];
            tokenToIndex[lastToken] = index;
            tokens[index] = lastToken;
        }

        tokens.pop();
        tokenToAddress[tokenId] = address(0);
        tokenToIndex[tokenId] = 0;
    }
    ...
    function addToken(
        ...
        require(tokenToAddress[tokenId] == address(0), "Token already exists!");

        tokenToAddress[tokenId] = _tokenAddress;
        tokenToIndex[tokenId] = tokens.length;
        tokens.push(tokenId);
    }
     ...
     */
    function setTokenAdminContract(address _newTokenAdmin) external onlyOwner {
        TokenFactory(tokenFactory).setTokenAdminContract(_newTokenAdmin);
    }
    ...
    function generateTokenId(string calldata _symbol, string calldata _name)
        public
        pure
        returns (bytes32)
    {
        return keccak256(abi.encodePacked(_symbol, _name));
    }
}
...
abstract contract TokenAdminStorageV1 is
    OwnableUpgradeable,
    UUPSUpgradeable,
    ...
    AccessControlEnumerableUpgradeable
{
    ...
    mapping(bytes32 => address) public tokenToAddress;
    mapping(bytes32 => uint256) public tokenToIndex;
