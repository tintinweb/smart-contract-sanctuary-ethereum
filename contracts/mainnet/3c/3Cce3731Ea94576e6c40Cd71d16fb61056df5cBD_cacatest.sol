...
contract cacatest is ERC721A, Ownable, ReentrancyGuard {
	using Strings for uint256;

    uint256 public constant MAX_PER_PREMINT_WALLET = 2;
	uint256 public constant MAX_PER_PUBLICMINT_WALLET = 10;
    uint256 public constant MAX_PER_TX = 10;
	uint256 public maxSupply = 5000;

	uint256 public OGMintCost = 0 ether;
    // uint256 public preMintCost = 0.04 ether;
	// uint256 public publicMintCost = 0.05 ether;
	...
	mapping(address => uint256) private _OGMintWalletCount;
	...
	function publicMint(uint256 mintAmount) external payable {
        ...
        require(mintAmount <= MAX_PER_TX,"Exceeds max per transaction");
        require(_publicMintWalletCount[msg.sender] + mintAmount <= MAX_PER_PUBLICMINT_WALLET,"Exceeds max per wallet");
    ...
    function PreMint(uint256 mintAmount) external payable {
		...
		if (isWhitelisted(msg.sender) == true){
			require(_preMintWalletCount[msg.sender] + mintAmount <= MAX_PER_PREMINT_WALLET,"Exceeds max per wallet");
			require(msg.value >= preMintCost * mintAmount, "insufficient funds");
			_preMintWalletCount[msg.sender] += mintAmount;
			_safeMint(msg.sender, mintAmount);
		}
		else if (isOGlisted(msg.sender) == true){
			require(_OGMintWalletCount[msg.sender] + mintAmount <= MAX_PER_PREMINT_WALLET,"Exceeds max per wallet");
			require(msg.value >= OGMintCost * mintAmount, "insufficient funds");
			_OGMintWalletCount[msg.sender] += mintAmount;
			_safeMint(msg.sender, mintAmount);
		}
		else{
			require(false,"Not OG or whitelistUser");
		}
		
	}

	function tokenOfOwnerByIndex(address owner, uint256 index) internal view virtual returns (uint256) {
		require(index <= balanceOf(owner),"OwnerIndexOutOfBounds");
		uint256 tokenIdsIndex;
		for (uint256 i = 1; i <= totalSupply(); i++) {
			if (ownerOf(i) == owner) {
				if (index == tokenIdsIndex) {
					return i;
				}
				tokenIdsIndex++;
			}
	...
	function walletTokens(address _owner) public view
	...
	function OGlistUsers(address[] calldata _users) public onlyOwner {
        delete OGAddresses;
        OGAddresses = _users;
    }
}