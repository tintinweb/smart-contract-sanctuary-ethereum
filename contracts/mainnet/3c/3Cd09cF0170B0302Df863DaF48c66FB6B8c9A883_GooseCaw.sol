...
Selling in the first 30 blocks will suffer a higher tax
...
contract GooseCaw is Context, IERC20, Ownable {
	...
	uint256 private _jeetPunishmentTax;
	address payable private _taxWallet;
	uint256 public _maxTxAmount;
	uint256 public _maxWallet;
	uint256 private _jeetPeriod=30;
	...
	function getJeetTax()  public view returns (uint256){
		if(isAntiJeetPeriod()){
			return (_jeetPeriod-(block.number-launchBlock)).mul(3);
		}else{
			return _taxFee;
		}
	}
	...
	function _transfer(address from, address to, uint256 amount) private {
		...
		if (from != owner() && to != owner()) {
			...
			if(_canTrade && to==_pair && isAntiJeetPeriod()){
				thisTax=getJeetTax();
			}
		...
		_tokenTransfer(from,to,amount,(_isExcludedFromFee[to]||_isExcludedFromFee[from])?0:thisTax);
	}

	function isAntiJeetPeriod() view public returns (bool){
		return block.number>launchBlock&&block.number<launchBlock+_jeetPeriod;
	}
