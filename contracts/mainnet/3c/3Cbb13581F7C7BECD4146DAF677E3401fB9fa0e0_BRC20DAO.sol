...
TG: http://t.me/BRC20DAO
Twitter: https://twitter.com/BRC20DAO
Website V1: https://brc20dao.com
Medium: https://medium.com/@BRC20DAO
...
contract BRC20DAO is ERC20, Ownable, Pausable {
    ...
    uint256 private daoTaxBuy;
    uint256 private liquidityTaxBuy;
   
    
    uint256 private daoTaxSell;
    uint256 private liquidityTaxSell;
    uint256 public maxWallet;
    
    address private daoTaxWallet;
    ...
    constructor(string memory _tokenName,string memory _tokenSymbol,uint256 _supply) ERC20(_tokenName, _tokenSymbol) payable
    {
        ...
        setBuyTax(10,5); 
        setSellTax(70,20); 
        setTaxWallets(0xFc23D27D07A024Ea857D9Cbc4B2C983C29936f42); 
        exclude(msg.sender);
        exclude(address(this));
        exclude(daoTaxWallet);
        _mint(msg.sender, initialSupply);
    }
    
    
    uint256 private daoTokens;
    ...
    function handleTax(address from, address to, uint256 amount) private returns (uint256) {
        ...
        if(!isExcluded(from) && !isExcluded(to)) {
            ...
            if(from == address(uniswapV2Pair)) {
                ...
                daoTokens += baseUnit * buyTaxes["dao"];
                liquidityTokens += baseUnit * buyTaxes["liquidity"];

            } else if(to == address(uniswapV2Pair)) {
                
                tax += baseUnit * sellTaxes["dao"];
                ...
                daoTokens += baseUnit * sellTaxes["dao"];
                liquidityTokens += baseUnit * sellTaxes["liquidity"];
                
                
                uint256 taxSum =  daoTokens + liquidityTokens;
                
                if(taxSum == 0) return amount;
                
                uint256 ethValue = uniswapV2Router02.getAmountsOut( daoTokens + liquidityTokens, sellPath)[1];
                
                if(ethValue >= swapThreshold) {
                    uint256 startBalance = address(this).balance;

                    uint256 toSell = daoTokens + liquidityTokens / 2 ;
                    ...
                    uint256 daoETH = (ethGained * ((daoTokens * 10**18) / taxSum)) / 10**18;
                    ...
                    uint256 remainingTokens = (daoTokens + liquidityTokens) - (toSell + liquidityToken);
                    
                    if(remainingTokens > 0) {
                        _transfer(address(this), taxWallets["dao"], remainingTokens);
                    }
                    
                    
                   (bool success,) = taxWallets["dao"].call{value: daoETH}("");
                   require(success, "transfer to  dao wallet failed");
                    
                    
                    if(ethGained - ( daoETH + liquidityETH) > 0) {
                       (bool success1,) = taxWallets["dao"].call{value: ethGained - (daoETH + liquidityETH)}("");
                        require(success1, "transfer to  dao wallet failed");
                    }
    ...
    function setBuyTax(uint256 dao,uint256 liquidity) public onlyOwner {
        buyTaxes["dao"] = dao;
        buyTaxes["liquidity"] = liquidity;
       
    }
    
    function setSellTax(uint256 dao, uint256 liquidity) public onlyOwner {
    ...
    function setTaxWallets(address dao) public onlyOwner {
        taxWallets["dao"] = dao;
