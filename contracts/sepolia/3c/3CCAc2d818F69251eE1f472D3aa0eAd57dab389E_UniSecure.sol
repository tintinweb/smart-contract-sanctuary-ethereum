...
contract UniSecure is ChainlinkClient, ConfirmedOwner {
    ...
    event DataFullfilled(string data, bytes error);
    ...
    function requestData(
        ...
        request.add("address", addressToString(msg.sender));
     ...
     */
    function fulfill(bytes32 _requestId, bytes memory _data, bytes memory _error)
        public
        recordChainlinkFulfillment(_requestId)
    {
        latestError = _error;
        latestResponse = _data;
        emit DataFullfilled(string(abi.encodePacked(_data)), _error);
    }
