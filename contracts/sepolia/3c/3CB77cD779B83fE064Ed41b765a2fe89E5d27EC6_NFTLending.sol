...
contract NFTLending {
    ...
    event NFTAdded(address indexed owner, uint256 tokenId);
    event NFTWithdrawn(address indexed owner, uint256 tokenId);
    event NFTBorrowed(address indexed borrower, address indexed lender, uint256 tokenId);
    event NFTReturned(address indexed borrower, address indexed lender, uint256 tokenId);
    ...
    function FillTestData() public {
        registerNFT(msg.sender, msg.sender, 0x5B38Da6a701c568545dCfcB03FcB875f56beddC4, 1, 0, 0, 0, 0, true);
        registerNFT(msg.sender, msg.sender, 0x5B38Da6a701c568545dCfcB03FcB875f56beddC4, 2, 0, 0, 0, 0, true);
        registerNFT(msg.sender, msg.sender, 0x5B38Da6a701c568545dCfcB03FcB875f56beddC4, 3, 0, 0, 0, 0, true);
        registerNFT(msg.sender, msg.sender, 0x5B38Da6a701c568545dCfcB03FcB875f56beddC4, 4, 0, 0, 0, 0, true);
        registerNFT(msg.sender, msg.sender, 0x5B38Da6a701c568545dCfcB03FcB875f56beddC4, 5, 0, 0, 0, 0, true);
    }
    ...
    function purchaseNFT(address _nftContract, uint256 _tokenId) external payable {
        ...
        emit NFTBorrowed(nftData.newOwner, nftData.owner, nftData.tokenId);
    }
    ...
    function returnNFT(address _nftContract, uint256 _tokenId) external {
        ...
        emit NFTReturned(nftData.newOwner, nftData.owner, nftData.tokenId);
        deleteNFT(nftData.owner, _nftContract, _tokenId);
    }
    ...
    function withdrawAll() external {
        ...
        for (uint256 i = 0; i < nftList.length; i++) {
            ...
            if (nftData.owner == msg.sender && !nftData.isAvailable && block.timestamp >= (nftData.useTime + nftData.timestamp)) {
                ...
                emit NFTWithdrawn(_owner, nftList[i].tokenId);
