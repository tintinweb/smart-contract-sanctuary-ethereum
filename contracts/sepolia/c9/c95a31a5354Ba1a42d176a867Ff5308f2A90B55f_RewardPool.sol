...
contract RewardPool is Ownable, ReentrancyGuard {
  ...
  function setAuthorizer(address _authorizer, bool _status) external onlyOwner {
    authorizers[_authorizer] = _status;
    emit AuthorizerUpdated(_authorizer, _status);
  }
  ...
  function claimReward(address _token) external nonReentrant {
    require(_token != address(0), 'Invalid token address');
    uint256 reward = rewards[msg.sender][_token];
    require(reward > 0, 'No reward to claim');
    require(
      lastClaimed[msg.sender] + TWO_WEEKS <= block.timestamp,
      'Claim allowed only every 2 weeks'
    );
  ...
  function rewardOf(
    ...
    return rewards[_user][_token];
  }

  function isAuthorizer(address _authorizer) external view returns (bool) {
    return authorizers[_authorizer];
  }

  function getLastClaimed(address _user) external view returns (uint256) {
