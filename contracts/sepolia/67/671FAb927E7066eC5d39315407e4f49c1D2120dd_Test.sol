...
contract ECDSAAirdrop {
    address immutable private signer;
    Test immutable public token;
    ...
    function mint(
        ...
        bytes32 _msgHash = getMessageHash(account,tokenId);
        bytes32 _ethSignedMessageHash = ECDSA.toEthSignedMessageHash(_msgHash);
        address signers = ECDSA.recover(_ethSignedMessageHash,signatrue);
        require(signers == signer,"invalid signature");
        require(!mintAddress[account],"already mint");
        token.mint(account,tokenId);
        mintAddress[account] = true;
    }
