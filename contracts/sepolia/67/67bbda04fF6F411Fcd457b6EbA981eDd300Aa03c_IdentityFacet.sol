...
contract IdentityFacet {
  ...
  function transferScroll(address to, uint256 scrollId) public {
    ...
    LibIdentity.transfer(LibUtils.msgSender(), to, scrollId);
  }
  ...
  function getOwnedScrolls(address user) public view returns (uint256[] memory) {
    ...
    for (uint256 index = 0; index < idStorage.scrollSupply; index++) {
      address currentOwner = idStorage.scrolls[index].owner;

      if (currentOwner == user) ownedScrolls[scrollIdxs++] = index;
  ...
  function scrollSupply() public view returns (uint256) {
    return LibIdentity.getStorage().scrollSupply;
  }
