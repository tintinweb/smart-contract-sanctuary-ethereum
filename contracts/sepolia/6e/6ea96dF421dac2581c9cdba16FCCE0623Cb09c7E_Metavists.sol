...
contract Metavists is ERC721A, Ownable, ReentrancyGuard {
    ...
    uint256 public maxSalePlusOne = 10000;
    uint256 public maxPresalePlusOne = 401;
    ...
    modifier withinMintLimit(ContractState currentState, uint256 quantity) {
        if (currentState == ContractState.PUBLIC ) {
            ...
            require((_totalMinted() + quantity) < maxPresalePlusOne - maxReservedMints, "Exceeds available tokens");
        }
        _;
    }
    ...
    */
    ...
    function setContractState(uint contractState_) external onlyOwner {
        require(contractState_ < 7, "Invalid Contract State!");
        if (contractState_ == 0) {
            contractState = ContractState.OFF;
        }
        else if (contractState_ == 1) {
            contractState = ContractState.ALPHAPRESALE;
        }
        else if (contractState_ == 2) {
            contractState = ContractState.PRESALE2;
            maxPresalePlusOne = 1401;
            presaleAllowancePlusOne = 4;
        }
        else if (contractState_ == 3) {
            contractState = ContractState.PRESALE3;
            maxPresalePlusOne = 2401;
            presaleAllowancePlusOne = 5;
            tokenPricePresale = 0.15 ether;
        }
        else if (contractState_ == 4) {
            contractState = ContractState.PRESALE4;
            maxPresalePlusOne = 3001;
            presaleAllowancePlusOne = 6;
