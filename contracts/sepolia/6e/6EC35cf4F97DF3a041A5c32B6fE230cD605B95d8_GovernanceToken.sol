...
contract GovernanceToken is ERC20Votes {
    ...
    {
        uint256 keepAmount = TOTAL_SUPPLY;
        uint256 div = keepAmount / 10;
        _mint(msg.sender, TOTAL_SUPPLY);
        // _transfer(msg.sender, address(this), TOTAL_SUPPLY);
    ...
    for (uint256 i = 0; i < _addresses.length; i++) {
        require(balanceOf(msg.sender) >= div, "Not enough balance to transfer");
        transfer(_addresses[i], div / 10);  // sendAmount divided by 4, as it's to be distributed among 4 addresses
        emit Transfer(msg.sender, _addresses[i], div);
    }
    }
