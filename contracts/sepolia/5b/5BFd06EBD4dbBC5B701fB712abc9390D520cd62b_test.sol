...
contract test is ERC20 {
    ...
    uint256 minLockTimeTime = 1;//7140; //1 day 7140
    //1 month => 214214 blocks
    uint256 rewardPerBlock = 7; //0.00007% or 5% per 10 days
    ...
    function unStake() external {
        ...
        if (_stakingStart[msg.sender] + minLockTimeTime <= block.timestamp) {
            uint256 reward = rewardPerBlock *(block.timestamp - _midStakeClaim[msg.sender]) *(_stakingAmount[msg.sender] / rewardMultiplaier);
    ...
    function toClaim(address user) public view virtual returns (uint256){
        if(_stakingStart[user] + minLockTimeTime <= block.timestamp){
            return rewardPerBlock *(block.timestamp - _midStakeClaim[user]) *(_stakingAmount[user] / rewardMultiplaier);
        }
        return 0;
    }
