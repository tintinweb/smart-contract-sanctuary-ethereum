...
/// @title PHIAvatar
contract PhiAvatar is MultiOwner, ReentrancyGuard, ERC721Enumerable {
    /* -------------------------------------------------------------------------- */
    /*                                   CONFIG                                   */
    /* -------------------------------------------------------------------------- */
    ...
    mapping(uint256 tokenId => Avatar avatar) public allAvatar;
    mapping(uint8 faction => uint256) public factionCount;
    /* -------------------------------------------------------------------------- */
    /*                                   EVENTS                                   */
    /* -------------------------------------------------------------------------- */

    event CreatePHIAvatar(uint256 tokenId, string uri, address payable creator, uint256 maxClaimed, uint256 price);
    event LogGetPHIAvatar(address indexed sender, uint256 tokenId);
    event SetTokenURI(uint256 tokenId, string uri);
    event SetFaction(uint256 tokenId, uint8 faction);
    event SetPetId(uint256 tokenId, uint32 petId);
    event SetTrait(uint256 tokenId, Trait trait);
    event SetGene(uint256 tokenId, bytes32 gene);
    event SetbaseMetadataURI(string newBaseMetadataURI);
    /* -------------------------------------------------------------------------- */
    /*                                   ERRORS                                   */
    /* -------------------------------------------------------------------------- */
    ...
    /*                               INITIALIZATION                               */
    /* -------------------------------------------------------------------------- */
    constructor(address payable _treasuryAddress) ERC721("Phi Avatar", "Phi-AVA") {
    ...
    /*                                Utility METHOD                                */
    /* -------------------------------------------------------------------------- */
    // Function to generate a random bytes32
    function getRandomBytes32() public view returns (bytes32) {
        ...
        uint256 random =
            uint256(keccak256(abi.encodePacked(block.prevrandao, block.timestamp, msg.sender, tokenSupply)));

        // Generate a random bytes32
        bytes32 randomBytes32;
        for (uint256 i = 0; i < 32; i++) {
            uint256 bit = (random >> i) & 1;
            randomBytes32 |= bytes32(bit) << (i * 8);
        }
    ...
    /* -------------------------------------------------------------------------- */
    /* Utility Functions */
    function isCreated(uint256 tokenId) internal view {
        if (created[tokenId]) revert InvalidTokenID();
    }
    ...
    function setFaction(uint256 tokenId, uint8 faction) public virtual onlyOwner {
        allAvatar[tokenId].faction = faction;
        emit SetFaction(tokenId, faction);
    }

    function setTokenURI(uint256 tokenId, string memory tokenURI) public virtual onlyOwner {
        allAvatar[tokenId].tokenURI = tokenURI;
        emit SetTokenURI(tokenId, tokenURI);
    }

    function setPet(uint256 tokenId, uint32 petId) public virtual onlyOwner {
        allAvatar[tokenId].petId = petId;
        emit SetPetId(tokenId, petId);
    }

    function setGene(uint256 tokenId) public virtual onlyOwner {
        allAvatar[tokenId].gene = getRandomBytes32();
        emit SetGene(tokenId, allAvatar[tokenId].gene);
    }

    function setTrait(uint256 tokenId, Trait memory trait) public virtual onlyOwner {
        allAvatar[tokenId].trait = trait;
        emit SetTrait(tokenId, trait);
    }

    // /* --------------------------------- GETTER --------------------------------- */
    function getFaction(uint256 tokenId) public view returns (uint8) {
        return allAvatar[tokenId].faction;
    }

    function getTokenURI(uint256 tokenId) public view returns (string memory) {
        return allAvatar[tokenId].tokenURI;
    }

    function getAvatarInfo(uint256 tokenId) external view returns (Avatar memory) {
        return allAvatar[tokenId];
    }

    function getFactionNumber(uint8 faction) external view returns (uint256 count) {
        ...
        nonReentrant
    {
        ...
        require(!created[tokenId], "tokenId is allready created");

        // price sent in to buy should be equal to or more than the token's price
        require(msg.value >= price, "should be equal to or more than the token's price");
        (bool success1,) = payable(treasuryAddress).call{ value: price }("");
        require(success1, "cant pay mint fee");
        ...
        emit LogGetPHIAvatar(msg.sender, tokenId);
    }
}
