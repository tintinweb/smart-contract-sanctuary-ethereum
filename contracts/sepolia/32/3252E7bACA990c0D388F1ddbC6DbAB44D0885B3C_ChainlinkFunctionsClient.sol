...
import "./IChainlinkFunctionsClient.sol";
...
import "./IFunctionClient.sol";

import {IPhoenixGuard} from "./IPhoenixGuard.sol";

contract ChainlinkFunctionsClient is IChainlinkFunctionsClient {
  string public jsCodeUri;
  uint64 public subscriptionId;
  uint32 public constant MAX_GAS_LIMIT = 2000000;
  IFunctionClient private s_oracle;

  event FulfillRequest(bytes32 indexed requestId, bytes response);
  event RequestSent(bytes32 indexed requestId);

  struct BurnProofRequest {
    address user;
  ...
  mapping(bytes32 => BurnProofRequest) public burnProofRequests;

  constructor(address oracle, uint64 _subscriptionId, string memory _jsCodeUri) {
    subscriptionId = _subscriptionId;
    jsCodeUri = _jsCodeUri;

    s_oracle = IFunctionClient(oracle);
  }
  ...
  function sendChainlinkRequest(
    ...
    bytes32 requestId = s_oracle.sendRequest(subscriptionId, abi.encode(req), gasLimit, callbackContract);
  ...
  function sendBurnProofRequest(
    ...
    string[] memory reqArgs = new string[](5);
    reqArgs[0] = Strings.toHexString(_user);
    reqArgs[1] = Strings.toHexString(_burnedTokenAddress);
    reqArgs[2] = Strings.toString(_validFrom);
    reqArgs[3] = Strings.toString(_validUntil);
    reqArgs[4] = _transactionHash;

    Functions.Request memory req = Functions.Request({
      codeLocation: Functions.Location.Remote,
    ...
    bytes32 requestId = sendChainlinkRequest(req, subscriptionId, MAX_GAS_LIMIT, msg.sender);

    burnProofRequests[requestId].user = _user;
    burnProofRequests[requestId].burnedTokenAddress = _burnedTokenAddress;
  }
  ...
  function fulfillRequest(
    ...
    burnProofRequests[requestId].response = response;

    IPhoenixGuard(callbackContract).fulfillBurnProofRequest(user, 120 ether);

    emit FulfillRequest(requestId, response);
  }
}
...
interface IPhoenixGuard {
  error BurnInsuranceNotTransferable();

  function fulfillBurnProofRequest(address user, uint256 amount) external;
}
