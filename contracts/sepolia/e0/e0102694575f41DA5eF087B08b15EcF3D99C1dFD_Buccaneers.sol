...
/**
 * @title   Buccaneers.
 * @notice  Sets all fees and includes all payable functions for the BotB game.
 */
...
// CHANGE WHITE TO ALLOW
    struct Fees {
        uint allowListtMintFee;
        uint mintFee;
        uint attackFee;
        uint defendFee;
        uint trainFee;
    ...
    address payable private admin = payable(0x7689DE2dd1D9DF3d442DCcFfAACb0017F5DDcc66);  // NEW WALLET
    address payable private our = payable(0x7689DE2dd1D9DF3d442DCcFfAACb0017F5DDcc66);
    address payable private charity = payable(0x7689DE2dd1D9DF3d442DCcFfAACb0017F5DDcc66);  
    address payable private treasure = payable(0x7689DE2dd1D9DF3d442DCcFfAACb0017F5DDcc66);

    constructor() ERC721("TESTTEST", "TEST") {
        fees.allowListtMintFee = 0.066 ether; 
        fees.mintFee = 0.1 ether;   
        fees.attackFee = 0.007 ether; 
        fees.defendFee = 0.014 ether; 
        fees.trainFee = 0.007 ether;  
    }
// ----------------------------------------------------------------------------- OWNER OPERATION MODIFIERS & FUNCTIONS
    ...
    /**
    * @notice  Owner can add address to allow list.
    * @param   wl_address address to be allow listed.
    */
    function addAllowList(address wl_address) external onlyOwner {
        allowList[wl_address] = true;
    }
    
// ----------------------------------------------------------------------------- ON-RAMP FUNCTIONS
    /**
     * @notice  Payable function that mints new Buccaneer.
     */
    function mintBuccaneer() external onlyIndividual payable {
        require(_tokenIds < 1944, "All buccaneers have been minted");
        bool wl = false;
        if ((allowList[msg.sender] && mintOpen) || msg.sender == admin) {
            require(
                msg.value >= fees.allowListtMintFee,
                "You must pay 0.066 ether to mint a buccaneer"
            );
            wl = true;
            refundIfOver(fees.allowListtMintFee);
        } else {
            require(mintOpen, "Minting is not open");
            require(
        ...
        // REPLACE PLACEHOLDER
        // REPLACE PLACEHOLDER
        // REPLACE PLACEHOLDER.   the fix is in
        // REPLACE PLACEHOLDER.     white skull on black
        string memory uriExtension = "QmaHCr5Y1BrNdhhQeCVMb4fo2rwQumJwRgkCCemDVgcFva";
        _setTokenURI(
            _tokenIds,
        ...
        _tokenIds++;
        if (wl) {
            _split(fees.allowListtMintFee, 25, 25);
        } else {
            _split(fees.mintFee, 25, 25);
        }
    }
...
// ----------------------------------------------------------------------------- GAMEPLAY FUNCTIONS
     ...
     * @param   defender  ID of target.
     */
    function attackBuccaneer(uint16 attacker, uint16 defender) external onlyIndividual payable gameStatus {
        require(msg.value >= fees.attackFee, "You must pay 0.007 ether to attack another buccaneer");
    ...
    function defendBuccaneer(
        ...
        require(msg.value >= fees.defendFee, "You must pay 0.014 ether to defend your buccaneer");
    ...
    function trainBuccaneer(uint16 trainee) external onlyIndividual payable gameStatus {
        require(msg.value >= fees.trainFee, "You must pay 0.007 ether to train your buccaneer");
        refundIfOver(fees.trainFee);
        require(ownerOf(trainee) == msg.sender, "You do not own this buccaneer");
        emit ActionReceipt("train", trainee);
        _split(fees.trainFee, 40, 40);
    }
// ----------------------------------------------------------------------------- HELPER FUNCTIONS

    /**
     * @notice  Gets all token ids owned by address.
     ...
     */
    function _split(uint256 price, uint _charity, uint _treasure) internal {
        uint _amount = price / 100;
