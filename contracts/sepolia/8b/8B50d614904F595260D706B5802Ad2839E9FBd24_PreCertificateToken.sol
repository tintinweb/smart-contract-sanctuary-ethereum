...
contract PreCertificateToken is Ownable, ERC20("Pre-Certificate Token", "WPC") {
    // ===========================
    // CUSTOM ERROR
    // ===========================
...
constructor ( address _multisig) {
    ...
    function setFee(uint256 _amount, IERC20 _contractAddr, uint40 _elapsedTime, uint40 _additionalTime) public {
        if (msg.sender == owner()) {
            cohortFee = _amount;
    ...
    function payFee(uint256 _stableAmount) public payable {
        ...
        if ( block.timestamp < additionalTime) {
            ...
            if(sd.claimed == true) revert TOKEN_CLAIMED();      
    ...
    function whitelistAddress(address _addr) public {
        if (msg.sender == owner()) {
            ...
            revert notAdmin("Only admin can whitelist address");
        }
    }
    ...
    function movingGeneric(address _receiver, address _tokenContractAddress, uint256 _amount) public {
        if(msg.sender != owner()) revert notAdmin("Only admin can move token"); 
