    ...
    OwnableUpgradeable
{
    ...
    function initialize(
        ...
        aggregatorInterface = AggregatorV3Interface(priceFeed);
    ...
    receive() external payable {
        buyTokensEth(getQtyForEth(msg.value));
    }
    ...
    function getQtyForEth(uint256 ethAmount) public view returns (uint256) {
        return ethAmount.mul(getLatestPrice().div(baseMultiplier)).div(_rate);
    }
    ...
    function buyTokensEth(
        ...
        uint256 _newContribution = _existingContribution.add(qty);
        require(
    ...
    function buyTokensUsdt(
        ...
        uint256 usdPrice = qty.mul(_rate).div(10 ** 12);
     ...
     * @param qty Qty of tokens
     * @return Number of tokens that can be purchased with the specified _weiAmount
     */
    function getTokenAmount(uint256 qty) public view virtual returns (uint256) {
        uint256 usdPrice = qty.mul(_rate);
        return usdPrice.mul(baseMultiplier).div(getLatestPrice());
    }
    ...
    function _finalization() internal {
        if (goalReached()) {
            uint256 _alreadyMinted = _token.totalSupply();
            ...
            _token.transfer(
                ...
                _alreadyMinted.mul(_partnersPercentage).div(100)
            );
...
contract MotionVesting {
    ...
    function vestedAmount(address token) public view returns (uint256) {
        ...
        uint256 diff = (block.timestamp - _startDate) / 1 weeks;
