    ...
    ReentrancyGuard
{
    ...
    function _update(uint256 _balance0, uint256 _balance1, uint256 _reserve0, uint256 _reserve1) private {
        ...
        console.log("Swap Time: ", blockTimestamp);

        if (timeElapsed > 0 && _reserve0 != 0 && _reserve1 != 0) {
            // * never overflows, and + overflow is desired
            price0CumulativeLast += _reserve1 * timeElapsed * 1e18 / _reserve0;
            price1CumulativeLast += _reserve0 * timeElapsed * 1e18 / _reserve1;
        }

        reserve0 = uint256(_balance0);
        reserve1 = uint256(_balance1);
