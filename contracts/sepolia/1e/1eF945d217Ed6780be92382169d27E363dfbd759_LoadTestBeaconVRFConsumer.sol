...
contract BeaconVRFConsumer is IVRFCoordinatorConsumer {
    ...
    mapping(uint256 => mapping(VRFBeaconTypes.ConfirmationDelay => uint256)) /* block height */
    ...
    function testRequestRandomnessFulfillment(
        ...
        uint24 confDelay = VRFBeaconTypes.ConfirmationDelay.unwrap(
            confirmationDelayArg
        );
        uint256 periodOffset = ChainSpecificUtil.getBlockNumber() %
            i_beaconPeriodBlocks;
    ...
    function testRequestRandomness(
        ...
        uint256 requestID = s_coordinator.requestRandomness(
            subID,
            numWords,
            VRFBeaconTypes.ConfirmationDelay.unwrap(confirmationDelayArg),
            EMPTY_ARGS
        );
    ...
    function storeBeaconRequest(
        ...
        VRFBeaconTypes.SlotNumber slotNumber = VRFBeaconTypes.SlotNumber.wrap(
            uint32(slotNumberBig)
        );
