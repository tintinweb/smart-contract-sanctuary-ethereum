 ...
 *               - swap F-Token to WETH in Furion Swap
 *               - swap WETH to USDT in Uniswap
...
contract FurionLeverage is OwnableUpgradeable {
    ...
    function openPosition(
        ...
        uint256 price = IPriceOracle(oracle).getUnderlyingPrice(
            tokenToMoneyMarket[_cToken]
        );
    ...
    function liquidatePosition(address _user, address _fToken) external {
        ...
        console.log("collateral money market", collateralMoneyMarket);
        console.log("target money market", targetMoneyMarket);
    ...
    function _long(
        ...
        console.log("\nsupply amount usd:", parameters.supplyAmountUSD / 1e6);
        console.log("borrow amount usd:", parameters.borrowAmountUSD / 1e6);
        console.log("\n");
    ...
    function _chargePositionFee(address _cToken, uint256 _amount) internal {
        ...
        if (_cToken == WETH) console.log("Position fee is in WETH");
        console.log("Position fee: ", positionFee);
    ...
    function _short(
        ...
        if (_cToken == WETH) {
            ...
            _chargePositionFee(WETH, parameters.borrowAmountUSD);
            ...
            // console.log("\nswap fee in usd", swapFee);
            // console.log("position fee in usd", fee);
            // console.log("\n");
            ...
            console.log("\n");
            console.log("mm token balance:", mmTokenBalance);
            console.log("\n");
        }
    }
    ...
    function _toWETHAmount(
        ...
        wethAmount = (_usdValue * 1e30) / _ethPrice();
    }

    function _toFURAmount(
        uint256 _usdValue
    ) internal view returns (uint256 furAmount) {
        furAmount = (_usdValue * 1e30) / _furPrice();
    }
    ...
    function _delegateSwapWhenShortWithWETH(
        ...
        uint256 price = IPriceOracle(oracle).getUnderlyingPrice(_moneyMarket);
    ...
    function _ethPrice() internal view returns (uint256 wethPrice) {
        // TODO: fETH address (needs to be changed if new market is deployed)
        wethPrice = IPriceOracle(oracle).getUnderlyingPrice(0x9cbEf68f59571a1C9760b4265cAc587Fc9F7C34E);
        // TODO: for test, price is set to 1000
        // wethPrice = 1000e18;
    }

    function _furPrice() internal view returns (uint256 furPrice) {
        // TODO: fFUR address (needs to be changed if new market is deployed)
        furPrice = IPriceOracle(oracle).getUnderlyingPrice(0x35A1b19ab1b915B11Bf54ADF4664C142C1A3bE55);
    }
    ...
    function _closeLongPosition(
        ...
        if (position.collateralToken == WETH) {
            ...
            address[] memory uniswapPath = new address[](2);
            uniswapPath[1] = FUR;
            uniswapPath[2] = position.collateralToken;
    ...
    function _closeShortPosition(
        ...
        if (position.collateralToken == WETH) {
            ...
            uint256 neededWETHAmount = IFurionSwapV2Router(furionSwapRouter)
    ...
    ITokenBase
{
    ...
    function delegateSupplyInternal(
        ...
        console.log("supply amount: %s", _supplyAmount);
        console.log("exchange rate: %s", exchangeRate.mantissa);
