...
contract GameContract is Ownable {
	...
	function playSolo(uint256 _cardId,uint256 _score,uint8 _caracter,bytes memory _signature) external payable isCardOwner(_cardId) returns(uint256 gameID,uint256 random_score,uint8 _winner) {
	...
	function joinGame(uint256 _gameId,uint256 _cardId,uint256 _score,bytes memory _signature) external payable isCardOwner(_cardId) returns(uint256 random_score,uint8 _winner){
