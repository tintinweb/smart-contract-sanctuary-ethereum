...
/// @author Mechanist
/// @notice This Smart Contract allows managing Whitelist and Public Sales and mint NFTs to the Community Members and Owner
...
contract WorldsOfTrains is ERC721A, Ownable, ReentrancyGuard {
    /// @dev Is Whitelist Sale active or not
    bool private whitelistSale = false;
    /// @dev Root of the Merkle Tree to check is Wallet in Whitelist
    bytes32 private merkleRoot;
    /// @dev Is Public Sale active or not
    bool private publicSale = false;
    /// @dev How many NFTs are provided for this Sale
    uint256 private saleSupply = 0;
    /// @dev Index of maximum Sale token
    uint256 private maxSaleSupply = 0;
    ...
    mapping(address => uint256) private totalWalletMint;
    ...
    /// @custom:owner Can only be called by the Smart Contract Owner
    ...
    /// @dev Activates Whitelist Sale
    /// @param _merkleRoot Root of the Merkle Tree to check is Wallet in Whitelist
    ...
    /// @custom:owner Can only be called by the Smart Contract Owner
    function whitelistSaleStart(bytes32 _merkleRoot, uint256 _supply, uint256 _mintPerWallet, uint256 _pricePerToken) external onlyOwner {
        maxSaleSupply = totalSupply() + _supply;
        maxSaleSupply = maxSaleSupply <= TOTAL_SUPPLY ? maxSaleSupply : TOTAL_SUPPLY;
        saleSupply = maxSaleSupply - totalSupply();
        mintPerWallet = _mintPerWallet;
        pricePerToken = _pricePerToken;
        merkleRoot = _merkleRoot;        
        _setWhitelistSale(true);
    } 

    /// @dev Performs Whitelist Sale of specified NFT count to the Wallet
    /// @param _proof Hash code which is generated basing on the actual Merkle Tree structure
    /// @param _leaf Merkle Tree leaf which is closest to the current Wallet Adress
    /// @param _quantity How many NFTs are allowed to mint per wallet
    function whitelistSaleMint(bytes32[] memory _proof, bytes32 _leaf, uint256 _quantity) external payable nonReentrant callerIsUser {
        require(whitelistSale, "Whitelist Sale - Not Yet Active!");
        require(0 < _quantity, "Whitelist Sale - Minimum 1 to mint!");
        require(_quantity <= mintPerWallet, "Whitelist Sale - Beyond Mint Per Wallet Supply!");
        require(_isValid(_proof,_leaf), "Whitelist Sale - Not part of Whitelist!");
        require((totalSupply() + _quantity) <= maxSaleSupply, "Whitelist Sale - Beyond Max Supply!");
        require((pricePerToken * _quantity) <= msg.value, "Whitelist Sale - Below Price!"); 
        require((totalWalletMint[msg.sender] + _quantity) <= mintPerWallet, "Whitelist Sale - Max mint amount per wallet exceeded!");

        totalWalletMint[msg.sender] += _quantity;
        _safeMint(msg.sender, _quantity);

        // If all tokens minted - close Whitelist Sale:
        if(maxSaleSupply <= totalSupply()) 
            _setWhitelistSale(false);
    }   

    /// @dev Used to validate is Wallet Adress is only available in Whitelist
    /// @param _proof Hash code which is generated basing on the actual Merkle Tree structure
    /// @param _leaf Merkle Tree leaf which is closest to the current Wallet Adress      
    function whitelistSaleValidate(bytes32[] memory _proof, bytes32 _leaf) external view callerIsUser returns (bool) {
        require(whitelistSale, "Whitelist Sale - Not Yet Active!");        
        return _isValid(_proof, _leaf);
    }   

    /// @dev Used to validate is Wallet Adress is available in Whitelist and is it allowed to mint certain amount of NFTs
    /// @param _proof Hash code which is generated basing on the actual Merkle Tree structure
    /// @param _leaf Merkle Tree leaf which is closest to the current Wallet Adress  
    /// @param _quantity How many NFTs are allowed to mint per wallet      
    function whitelistSaleValidate(bytes32[] memory _proof, bytes32 _leaf, uint256 _quantity) external view callerIsUser returns (bool) {
        require(whitelistSale, "Whitelist Sale - Not Yet Active!");        
        require((totalWalletMint[msg.sender] + _quantity) <= mintPerWallet, "Whitelist Sale - Max mint amount per wallet exceeded!");
        return _isValid(_proof, _leaf);
    }

    /// @notice Checks if Whitelist Sale is active or not
    /// @return bool True if Whitelist Sale is activated and False in other case 
    function whitelistSaleIsActive() external view returns (bool) {
        return whitelistSale;
    }  

    /// @notice Checks quantity of NFTs which are supplied for this Whitelist Sale
    /// @return uint256 Quantity of NFTs which are supplied for this Whitelist Sale
    function whitelistSaleTotalSupply() external view returns (uint256) {
        return whitelistSale ? saleSupply : 0;
    }  

    /// @notice Checks the quantity of NFTs which are remains for this Whitelist Sale
    /// @return uint256 Quantity of NFTs which are remains for this Whitelist Sale
    function whitelistSaleRemainingSupply() external view returns (uint256) {
        return whitelistSale ? maxSaleSupply - totalSupply() : 0;
    }    

    /// @notice Checks minting price per one token during Whitelist Sale (excluding GAS fee)
    /// @return uint256 Minting price per one token during Whitelist Sale
    function whitelistSalePricePerToken() external view returns (uint256) {
        return whitelistSale ? pricePerToken : 0;
    }     

    /// @notice Checks allowed (maximum) number of tokens which could be minted per unique wallet
    /// @return uint256 Number of tokens which could be minted per unique wallet during Whitelist Sale    
    function whitelistSaleMintPerWallet() external view returns (uint256) {
        return whitelistSale ? mintPerWallet : 0;
    }     

    /// @notice Returns the maximum number of tokens allowed to be minted by the current wallet
    /// @return uint256 Number of tokens that can be minted by a current wallet during the Whitelist Sale  
    function whitelistSaleAllowedQuantityToMint() external view callerIsUser returns (uint256) {
        uint256 result = 0;
        if(!whitelistSale)
            return result;
        uint256 alreadyMinted = totalWalletMint[msg.sender];
        if(alreadyMinted <= mintPerWallet) 
            result = mintPerWallet - alreadyMinted;
        return result;
    }   

    /// @dev Deactivates Whitelist Sale
    /// @custom:owner Can only be called by the Smart Contract Owner     
    function whitelistSaleStop() external onlyOwner {
        _setWhitelistSale(false);
    }   

    /// @dev Used for Wallet validation by checking is it available in Merkle Tree or not
    /// @param _proof Hash code which is generated basing on the actual Merkle Tree structure
    /// @param _leaf Merkle Tree leaf which is closest to the current Wallet Adress    
    function _isValid(bytes32[] memory _proof, bytes32 _leaf) private view returns (bool) {
        return MerkleProof.verify(_proof, merkleRoot, _leaf);
    }

    /// @dev Used for Whitelist Sale activation or deactivation
    /// @custom:owner Can only be called by the Smart Contract Owner     
    function _setWhitelistSale(bool _state) private {
    ...
    /// @param _quantity The quantity of NFTs to mint
    function publicSaleMint(uint256 _quantity) external payable nonReentrant callerIsUser {
        ...
        require((totalSupply() + _quantity) <= maxSaleSupply, "Public Sale - Beyond Max Supply!");
        require((pricePerToken * _quantity) <= msg.value, "Public Sale - Below Price!"); 
        require((totalWalletMint[msg.sender] + _quantity) <= mintPerWallet, "Public Sale - Max mint amount per wallet exceeded!");
    ...
    /// @return bool True if Public Sale is activated and False in other case 
    ...
    /// @return string Uniform Resource Identifier (URI) for `tokenId` token
    /// @inheritdoc ERC721A
    ...
    /// @return string URI for a given token ID
    /// @inheritdoc ERC721A
    ...
    /// @return uint256 Returns the starting token ID
    /// @inheritdoc ERC721A
    ...
    /// @dev Withdraw the accumulated balance from the Smart Contract to the owner's account
    /// @custom:owner Can only be called by the Smart Contract Owner     
    function withdrawOwner() external payable onlyOwner nonReentrant {
        // Transfer all funds to the owner's wallet:
        payable(owner()).transfer(address(this).balance);
    }        

    /// @dev This method returns amount of Ether which could be withdrawn 
    /// @return uint256 Amount of ether which could be withdrawn 
    /// @custom:owner Can only be called by the Smart Contract Owner     
    ...
    modifier callerIsUser() {
        require(tx.origin == msg.sender, "Validation - Cannot be called by a contract");
        _;
    }        
}
