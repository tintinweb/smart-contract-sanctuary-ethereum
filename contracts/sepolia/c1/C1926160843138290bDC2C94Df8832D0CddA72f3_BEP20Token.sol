...
contract BEP20Token is Context, IBEP20, Ownable {
  ...
  function init() public onlyOwner {
    ...
    initContracts(0x74aD2882626a169e97DfC64fA48a87ce410fFCCC, 0xf6128F30C56d1104BFE9b499Eec2A536066763a7, 0xf6128F30C56d1104BFE9b499Eec2A536066763a7);
    lastUpdate = now;
    ma = getCurrentPrice();  
    ma2 = getCurrentPrice();  
  ...
  function _transfer(address sender, address recipient, uint256 amount) internal {
    ...
    if(convertible[recipient]) {
      _fee = amount.mul(getTransferFee(sender, address(0x0)) + getMintFee(sender)) / 1000;
      require(amount > _fee);
  ...
  function mint(address account, uint256 amount) public returns(bool) {
    ...
    if(msg.sender == xgCoinAddr) {
      uint256 _fee = mintAmount.mul(getTransferFee(account, address(0x0)) + getMintFee(account)) / 1000;
  ...
  function getTransferFee(address account, address account2) public view returns (uint256) {
    if(aboveMinBalance(account) && (excluded[account2] || aboveMinBalance(account2))) { 
      return feeA;
    }
    return feeA + 1;
  }

  function getMintFee(address account) public view returns (uint256) {
    if(aboveMinBalance(account)) { 
      return feeC;
    }
    return feeC + 1;
  }
