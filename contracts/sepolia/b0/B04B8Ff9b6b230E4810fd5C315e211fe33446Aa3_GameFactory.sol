...
contract GameFactory is VRFConsumerBaseV2, ConfirmedOwner, AutomationCompatibleInterface {
    ...
    function buyEntries(uint256 _gameId, uint256 _numEntries) public payable {
        ...
        if(!hasEnteredGame[msg.sender][_gameId]) {
            ...
            emit GameNumEntrantsUpdated(_gameId, games[_gameId].uniqueEntrants);
        }
        purchases[purchase.id] = purchase;
        games[_gameId].purchaseList.push(purchase.id); 
        games[_gameId].nextEntryId += _numEntries;
        nextPurchaseId += 1;
        emit UserPurchasesUpdated(msg.sender, purchase.id);
        emit GamePurchasesUpdated(_gameId, purchase.id);
    }
    ...
    event GameNumEntrantsUpdated(
        uint256 gameId,
        uint256 numEntrants
    );

    event CommunityGamesUpdated(
        address community,
        uint256 gameId
    );
    ...
    event UserPurchasesUpdated(
        address userWallet,
        uint256 purchaseId
    );

    event GamePurchasesUpdated(
        uint256 gameId,
        uint256 purchaseId
    );
    ...
    function fundGame(uint256 _id) external payable {
        ...
        emit GameFunded(_id, games[_id].jackpot);
    ...
    function createGame(
        ...
        emit GameCreated(
            ...
            game.endTime,
            game.maxEntries,
            game.minStartingBalance,
            game.jackpot
        );
        emit CommunityGamesUpdated(msg.sender, game.id);
    ...
    function startGame(uint256 _id) internal {
        ...
        games[_id].status = Status.Open;
        openGames.push(_id);
        emit GameStarted(_id, games[_id].endTime);
    }
    ...
    function closeGame(uint256 _id) external onlyOwner {
        ...
        drawGames.push(_id);
        emit GameStatusUpdated(_id, Status.Closed);
    }
    ...
    function drawNumbers() internal {
        ...
        for(uint x = 0; x < drawGames.length; x++) {
            ...
            if(s_requests[lastRequestId].fulfilled == true) {
                ...
                emit GameStatusUpdated(_id, Status.Claimable);
            }
        }
   }
   ...
   function getUserPurchases(address _owner) external view returns(uint256[] memory) {
    ...
    function setGameStatus(uint256 id, uint256 status) external onlyOwner {
        if (status == 0) {
            ...
            emit GameStatusUpdated(id, Status.Open);
        } else if (status == 1) {
            games[id].status = Status.Closed;
            emit GameStatusUpdated(id, Status.Closed);
        }
