...
contract Receipt is Context, ERC165, IERC721, IERC721Metadata, Ownable {
    ...
    // address to can mint
    mapping ( address => bool ) public isApprovedToMint;

    // base URI
    string private baseURI = "RECEIPT/";
    ...
    function setIsApprovedToMint(address user, bool canMint) external onlyOwner {
        isApprovedToMint[user] = canMint;
    }
    ...
    function mint(address to, uint256 num) external {
        require(
            isApprovedToMint[msg.sender],
            'Only Sender Can Mint'
        );
