...
// https://go.chainalysis.com/chainalysis-oracle-docs.html
ISanctionsList constant SANCTIONS_LIST = ISanctionsList(0x40C57923924B5c5c5455c48D93317139ADDaC8fb);

/// @dev helper functions for allow & deny lists
contract LagoAccess is ILagoAccess, Ownable, ISanctionsList {
    ...
    function _isAllowed(address a, address b) internal view returns (bool) {
        ...
        if (isSanctioned(a) || isSanctioned(b)) {
            return false;
        }
    ...
    function isSanctioned(address a) public view returns (bool) {
        if (a != LAGO_ACCESS_ANY && a != address(0) && useSanctionsList) {
