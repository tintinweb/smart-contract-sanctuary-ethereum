...
contract ArtTokenVault is AccessControlUpgradeable, IArtTokenVault {
    ...
    function deploySimpleCrowdsale(
        ...
        _deployArtToken();
        uint256 protocolFeeAmount = (supply * settings.protocolFeeBps()) /
            10000;
        ...
        ICrowdsale(newCrowdsale).addWhitelister(settings.protocolAdmin());
        emit ERC20CrowdsaleCreated(
            address(newCrowdsale),
            address(tokenAT),
            address(this)
        );
        tokenAT.mint(address(newCrowdsale), supply + protocolFeeAmount);
    ...
    function deployProportionalCrowdsale(
        ...
        address(newCrowdsale).functionCall(
            abi.encodeWithSignature(
                "initialize((uint256,address,address,address,uint256,uint256,uint256,uint256,uint256,uint256,uint256,uint256,address))",
                params
            )
        );
        ICrowdsale(newCrowdsale).addWhitelister(settings.protocolAdmin());
...
interface ICrowdsale is IWhitelist {
    ...
    function buyTokens(uint256 amountAT) external;
     ...
     * @param crowdsale The address of the current crowdsale contract
     * @param buyer who reserved the tokens
     * @param artTokens amount of AT tokens reserved
     */
     ...
     * @param artTokens amount of AT tokens claimed
     */
    event TokensClaimed(
        address indexed crowdsale,
        address indexed buyer,
    ...
    /**
     * @notice Emitted when a buyer receives a refund after an unsuccessful crowdsale
     * @param crowdsale The address of the current crowdsale contract
     * @param buyer who received the refund
     */
    event TokenRefunded(address indexed crowdsale, address indexed buyer);
}
    ...
    Whitelist
{
    ...
    uint256 internal targetSaleProceedsBT;
    ...
    function buyTokens(
        uint256 amountBT
    ) external virtual nonReentrant onlyRole(PARTICIPANT_ROLE) {
        ...
        emit TokensReserved(address(this), _msgSender(), amountAT);
    ...
    function claimTokens() external virtual nonReentrant returns (uint256) {
        require(
            hasRole(PARTICIPANT_ROLE, _msgSender()) ||
                hasRole(SELLER_ROLE, _msgSender()),
            "Should be a participant or a seller to claim"
        );
        require(
            status() == CrowdsaleStatus.SUCCESSFUL,
        ...
        emit TokensClaimed(address(this), _msgSender(), amountAT);
        ...
        returns (uint256)
    {
        ...
        allocationsBT[_msgSender()] = 0;
        emit TokenRefunded(address(this), _msgSender());
    ...
    function _preValidatePurchase(
        ...
        require(
            IERC20(tokenBT).balanceOf(buyer) >= amountBT,
            "Crowdsale: buyer does not have enough funds"
        );
        require(
            IERC20(tokenBT).allowance(buyer, address(this)) >= amountBT,
        ...
        require(
            getAllocationFor(buyer) <= getTokenAmount(maxPurchaseBT - amountBT),
    ...
    function _postValidatePurchase(
        ...
        assert(amountAT <= getAllocationFor(buyer));
