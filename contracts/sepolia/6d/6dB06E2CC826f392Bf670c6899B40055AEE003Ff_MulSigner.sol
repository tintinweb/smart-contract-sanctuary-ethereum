    ...
    IERCSignerManage
{
    ...
    function SignerValid(address Signer, MulTx.TxType txType) private  view returns(bool ret){
    ...
    function createMulSignerTx(
        ...
        IERCSignerManage.Signer storage Signer = Signers[msg.sender];
        ...
        for (uint i = 0; i < SignerSendTxHash[msg.sender].length; i++) {
            if (SignerSendTxHash[msg.sender][i] == mtx.txHash) {
        ...
        SignerSendTxHash[msg.sender].push(mtx.txHash);
        emit CreateMulSigner(msg.sender, SignerTx.txHash, mtx.txId);
    }
        ...
        if (Signer.isSignerFinished()) {
            ...
            if (Signer.getSignerStatus() == MulSigners.SignerStatus.Approved) {
                ...
                if (mtx.txType == MulTx.TxType.AddSigner) {
                    ...
                    emit AddSigner(mtx.to, mtx.from);
                    ...
                    SignerManage.threshold = SignerManage.threshold - 1;
                    emit RemoveSigner(mtx.to, mtx.from);
                } else {
    ...
    function setSigners(
        ...
        require(Signers[msg.sender].Signers.length == 0, "Signer is not empty");
        ...
        IERCSignerManage.Signer storage SignerManage = Signers[msg.sender];
        SignerManage.Signers = Signer;
        SignerManage.threshold = threshold;
        emit CreateSigner(Signer, threshold, msg.sender, userId);
    }
    ...
    function getSenderTxHash(address sender) external view returns (uint[] memory) {
        //返回未签名状态未完成的交易
    ...
    function getSenderTxCompleteHash(address sender) external view returns (uint[] memory) {
        //返回签名状态完成的交易
        uint[] storage txHashs = SignerSendTxHash[sender];
...
interface  IERCMulSigner {
    ...
    event ExecuteMulSigner(uint  txHash,uint  txId);
       ...
       enum SignerOp {Approve,Reject}
   //writer a createMulSigner function  param with Signers address arrary and txParams withb MulTx.Mtx struct的filed to create MulTx.Mtx and  MulSigner
    function createMulSignerTx(uint amount,uint when, address to, string memory blank, MulTx.TxType txType,uint txId) external payable;
