...
import "@openzeppelin/contracts/token/ERC1155/extensions/ERC1155URIStorage.sol";//ERC1155URIStorage
import "@openzeppelin/contracts-upgradeable/token/ERC20/IERC20Upgradeable.sol";
import "@openzeppelin/contracts/utils/cryptography/ECDSA.sol";

 
contract CrowdfundingContract is ERC1155URIStorage {
    ...
    function mint(uint _numberOfTokens) public returns (uint256[] memory tokenIds){
        ...
        bool transferSuccessful = tokenAddress.transferFrom(_msgSender(),address(this),_numberOfTokens*pricePerNft); // transfer of funds to marketplace contract
        require(transferSuccessful,"transfer of tokens unsuccessful"); //)
        commissions += (_numberOfTokens*pricePerNft*commissionPercent)/100;
        projectBalance += ((100-commissionPercent)*_numberOfTokens*pricePerNft)/100;
        if(_numberOfTokens==1){
            _mint(msg.sender, totalMinted+1, 1, bytes(string(abi.encodePacked(msg.sender," Minting ",_numberOfTokens," tokens"))));
            _setURI(totalMinted+1, ipfsMetadata);
            nftPriceArray[totalMinted+1] = pricePerNft;
            ++totalMinted;
            ...
            for (uint256 i=0; i<_numberOfTokens; i++){
                ...
                amountsArray[i] = 1;
                nftPriceArray[ totalMinted + i + 1] = pricePerNft;
                _setURI(totalMinted + i + 1, ipfsMetadata);
            }
            _mintBatch(msg.sender,tokenIdsArray, amountsArray, bytes(string(abi.encodePacked(msg.sender," Minting ",_numberOfTokens," tokens"))));
            totalMinted+=_numberOfTokens;
            return tokenIdsArray;
        }
    }
    
    function assistedMint(uint _numberOfTokens, address _ownerAddress) public payable returns (uint256[] memory tokenIds){
        require(isActive, "Cannot Mint");
        require(totalMinted + _numberOfTokens <= totalSupply, "Goal reached already");
        require(tokenAddress.allowance(_msgSender(), address(this))>=(_numberOfTokens*pricePerNft),"insufficient allowance");
        bool transferSuccessful = tokenAddress.transferFrom(_msgSender(),address(this),_numberOfTokens*pricePerNft); // transfer of funds to marketplace contract
        ...
        projectBalance += ((100-commissionPercent)*_numberOfTokens*pricePerNft)/100;
        if(_numberOfTokens==1){
            _mint(_ownerAddress, totalMinted+1, 1, bytes(string(abi.encodePacked(_ownerAddress," Minting ",_numberOfTokens," tokens"))));
            _setURI(totalMinted+1, ipfsMetadata);
            nftPriceArray[totalMinted+1] = pricePerNft;
            ++totalMinted;
            uint256[] memory tokenIdsArray = new uint256[](1);
            ...
            _mintBatch(_ownerAddress,tokenIdsArray, amountsArray, bytes(string(abi.encodePacked(_ownerAddress," Minting ",_numberOfTokens," tokens"))));
            totalMinted+=_numberOfTokens;
            return tokenIdsArray;
        }
    }
    ...
    {   require(isActive, "Cannot Mint");
        require(keccak256(abi.encodePacked(_customPricePerNft,_numberOfTokens,address(this)))==hash,"Incorrect details passed");
        require(recoverSigner(hash, signature) == projectOwnerAddress, "Address is not allowlisted");
        require(!signatureUsed[signature],"Signature is already used");
        signatureUsed[signature] = true;
        require(totalMinted + _numberOfTokens <= totalSupply, "Goal reached already");
        require(tokenAddress.allowance(_msgSender(), address(this))>=(_numberOfTokens*_customPricePerNft),"insufficient allowance");
        bool transferSuccessful = tokenAddress.transferFrom(_msgSender(),address(this),_numberOfTokens*_customPricePerNft); // transfer of funds to marketplace contract
        require(transferSuccessful,"transfer of tokens unsuccessful"); //)
        commissions += (_numberOfTokens*_customPricePerNft*commissionPercent)/100;
        projectBalance += ((100-commissionPercent)*_numberOfTokens*_customPricePerNft)/100;
        if(_numberOfTokens==1){
            _mint(msg.sender, totalMinted+1, 1, bytes(string(abi.encodePacked(msg.sender," Minting ",_numberOfTokens," tokens"))));
            _setURI(totalMinted+1, ipfsMetadata);
            nftPriceArray[totalMinted+1] = _customPricePerNft;
            ++totalMinted;
            uint256[] memory tokenIdsArray = new uint256[](1);
            tokenIdsArray[0] = totalMinted;
            return tokenIdsArray;
            ...
            for (uint256 i=0; i<_numberOfTokens; i++){
                tokenIdsArray[i] = totalMinted + i + 1;
                amountsArray[i] = 1;
                nftPriceArray[totalMinted + i + 1] = _customPricePerNft;
                _setURI(totalMinted+i+1, ipfsMetadata);
            }
            _mintBatch(msg.sender,tokenIdsArray, amountsArray, bytes(string(abi.encodePacked(msg.sender," Minting ",_numberOfTokens," tokens"))));
            totalMinted+=_numberOfTokens;
    ...
    function burn(uint256 _tokenId) public {
        ...
        require(ERC1155.balanceOf(msg.sender,_tokenId)>0, "Zero balance");
        _burn(msg.sender,_tokenId,1);
        bool transferSuccessful = tokenAddress.transfer(msg.sender, nftPriceArray[_tokenId]);// needs to be edited
        require(transferSuccessful,"transfer of tokens unsuccessful");
    }

    function burnBatch(uint256[] memory _tokenIds, uint256[] memory _amounts) public {
        require(!isActive, "Goal pending");
