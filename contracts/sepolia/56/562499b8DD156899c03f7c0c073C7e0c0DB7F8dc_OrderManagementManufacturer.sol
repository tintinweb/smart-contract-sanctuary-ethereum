...
contract OrderManagementManufacturer is Supplier {
    ...
    */
    ...
    ) public _isManufacturer(msg.sender) view returns (Types.SupplierWithMaterialID[] memory) {
        require(
    ...
    function isProducer(address _userAdd) public view returns (bool) {
        return
            registration.getStakeHolderDetails(_userAdd).role ==
            Types.StakeHolder.Producer;
    }

    function isManufacturer(address _userAdd2) public view returns (bool) {
        return
            registration.getStakeHolderDetails(_userAdd2).role ==
            Types.StakeHolder.ManuFacturer;
    }

    function isSupplier(address _userAdd3) public view returns (bool) {
        return
            registration.getStakeHolderDetails(_userAdd3).role ==
