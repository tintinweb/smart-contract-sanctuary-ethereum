...
contract Har is ERC721Enumerable, Ownable, ReentrancyGuard { 
  ...
  constructor() ERC721 ("Et", "H") {
    ...
    function random(string memory input) internal pure returns (uint256) {
     ...
     function randomtest(uint256 input) public view returns (uint256) {
        uint256 rand = ((uint256(keccak256(abi.encodePacked(input))))% img.length);
    ...
    function tokenURI(uint256 _tokenId) override public view returns(string memory ) {
      ...
      for (uint256 i = 1; i < 7; i++) {  
        ...
        for (uint256 l = 1; l < 7; l++) {  
        //finalHTML = string.concat(finalHTML, imgSart, dataURL, Strings.toString(img[getRandom(((randomData+i)*_tokenId))]+ iter ), imgEnd); 
        finalHTML = string.concat(finalHTML, imgSart, dataURL, Strings.toString(img[((uint256(keccak256(abi.encodePacked((randomData+i+l)*_tokenId))))% img.length)]+ iter ), imgEnd); 
        //((uint256(keccak256(abi.encodePacked((randomData+i)*_tokenId))))% img.length)
        iter = iter+22;
        }
    ...
    //string memory name = string.concat('Eternal Harmony #', Strings.toString(_tokenId));
      ...
      //string memory  tokenUri = Strings.toString(getRandom(_tokenId));
