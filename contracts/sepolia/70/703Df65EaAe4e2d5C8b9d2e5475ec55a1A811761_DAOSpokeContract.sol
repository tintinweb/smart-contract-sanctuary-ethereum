...
contract DAOSpokeContract {
    ...
    constructor(address payable _hubContract, IVotes _token, uint _targetSecondsPerBlock) payable {
        hubContract = _hubContract;
        token = _token;
        targetSecondsPerBlock = _targetSecondsPerBlock;
    }

    address payable public immutable hubContract;
    ...
    function receiveMessage(bytes memory _payload) public {
        require(msg.sender == hubContract, "Only messages from the hub contract can be received!");
        ...
        if (option == 0) {
            ...
            MetaHumanGovernor(hubContract).receiveMessage(votingPayload);
...
contract MetaHumanGovernor is Governor, GovernorSettings, CrossChainGovernorCountingSimple, GovernorVotes, GovernorVotesQuorumFraction, GovernorTimelockControl {
    constructor(IVotes _token, TimelockController _timelock, address[] memory _spokeContracts)
    Governor("MetaHumanGovernor")
    GovernorSettings(1 /* 1 block */, 5 /* 1 minute */, 0) //TODO in production voting period needs to be changed to some reasonable value, right now it's 5 blocks for testing
    ...
    function receiveMessage(bytes memory _payload) public {
        require(spokeContractsMapping[msg.sender] == true, "Only spoke contract can send message");
        ...
        if (option == 0) {
            onReceiveSpokeVotingData(msg.sender, _payload);
        }
    }

    function onReceiveSpokeVotingData(address srcContractAddress, bytes memory payload) internal virtual {
        (
        , // uint16 option
        uint256 _proposalId,
        uint256 _for,
        ...
        if (spokeVotes[_proposalId][srcContractAddress].initialized) {
            revert("Already initialized!");
        } else {
            // Add it to the map (while setting initialized true)
            spokeVotes[_proposalId][srcContractAddress] = SpokeProposalVote(
                _for,
                _against,
                _abstain,
                true
    ...
    function finishCollectionPhase(uint256 proposalId) public {
        ...
        for (uint16 i = 0; i < spokeContracts.length && phaseFinished; i++) {
            phaseFinished =
            phaseFinished &&
            spokeVotes[proposalId][spokeContracts[i]].initialized;
        }
    ...
    function requestCollections(uint256 proposalId) public payable {
        ...
        for (uint16 i = 0; i < spokeContracts.length; i++) {
            ...
            DAOSpokeContract(spokeContracts[i]).receiveMessage(payload);
        }
    }
...
abstract contract CrossChainGovernorCountingSimple is Governor {
    ...
    address[] public spokeContracts;
    mapping(address => bool) public spokeContractsMapping;

    constructor(address[] memory _spokeContracts) {
        updateSpokeContracts(_spokeContracts);
    }

    function updateSpokeContracts(address[] memory _spokeContracts) public {
        spokeContracts = _spokeContracts;
        for (uint16 i = 0; i < _spokeContracts.length; i++) {
            spokeContractsMapping[_spokeContracts[i]] = true;
        }
    }
    ...
    mapping(uint256 => mapping(address => SpokeProposalVote)) public spokeVotes;
    ...
    function _quorumReached(uint256 proposalId) internal view virtual override returns (bool) {
        ...
        for (uint16 i = 0; i < spokeContracts.length; i++) {
            SpokeProposalVote storage v = spokeVotes[proposalId][spokeContracts[i]];
