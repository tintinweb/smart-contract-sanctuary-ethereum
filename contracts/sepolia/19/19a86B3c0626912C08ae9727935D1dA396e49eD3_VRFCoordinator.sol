    ...
    IVRFMigration
{
    ...
    function beaconRequest(
        ...
        (
            uint48 nonce,
            ConfirmationDelay[NUM_CONF_DELAYS] memory delays
        ) = getAndUpdateRequestParams();
        RequestID requestID = computeRequestID(subID, requester, nonce);
        checkConfirmationDelays(delays, confirmationDelayArg);
    ...
    function getAndUpdateRequestParams()
        private
        returns (uint48, ConfirmationDelay[NUM_CONF_DELAYS] memory)
    {
        RequestParams memory rp = s_requestParams;
        // Ensure next request has unique nonce
        s_requestParams.nonce = rp.nonce + 1;
        return (rp.nonce, rp.confirmationDelays);
    }

    function checkConfirmationDelays(
        ConfirmationDelay[NUM_CONF_DELAYS] memory delays,
        ConfirmationDelay confirmationDelayArg
    ) private pure {
        uint256 i;
        for (i = 0; i < delays.length; i++) {
            if (
                ConfirmationDelay.unwrap(delays[i]) ==
        ...
        if (i >= delays.length) {
            revert UnknownConfirmationDelay(confirmationDelayArg, delays);
        }
    }
