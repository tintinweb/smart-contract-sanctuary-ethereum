...
contract NFTDispenser is Pausable, Ownable {
    using SafeERC20 for IERC20;

    /* Structs + Errors */

    IERC20 public fungibleToken;
    VestingNFT public nonFungibleVestedToken;

    uint256 public immutable MAX_NFT_COUNT = 10000;
    uint256 public immutable NFT_PRICE = 10000000000000000; // 0.01 ETH
    uint256 public immutable TOKEN_VESTING_AMOUNT = 1000000000000000000000000; // 1 MILLION TOKENS

    /* Constructor */

    constructor(address _fungible, address _nonFungibleVestedToken) {
        fungibleToken = IERC20(_fungible);
        nonFungibleVestedToken = VestingNFT(_nonFungibleVestedToken);
    }

    /* Public functions */

    /*
     * @dev Mints an NFT to a recipient once ETH has been paid V0.1 simple
     * @param - to - The recipient of the NFT
     * @implicit - msg.value - The amount of ETH sent as wei (1e18)
     * @calculated - amount - The amount of NFT's to mint depending on how much has been sent
     * @calculated - releaseTimestamp - The last date of tokens being vested (block.number + vestingTimestamp)
     */
    function mint(address to) public {
        uint128 vestingEndDate = getCurrentTimePlusOneYear();

        nonFungibleVestedToken.create(
            to,
            TOKEN_VESTING_AMOUNT,
            vestingEndDate,
            fungibleToken
        );
    }
    ...
    /* Internal functions */
    function getCurrentTimePlusOneYear() public view returns (uint128) {
        uint oneYearInSeconds = 31536000; // approx, doesn't account for leap years
        return uint128(block.timestamp) + uint128(oneYearInSeconds);
    }
...
contract VestingNFT is ERC5725 {
    ...
    function create(
        ...
        require(
            ...
            "release must be in future"
        );

        // TODO CHECK IF THIS IS THE DISPENSER
        ...
        _tokenIdTracker++;
        _mint(to, newTokenId);
        // Take tokens from Dispenser and locks in this contract
        IERC20(payoutToken(newTokenId)).safeTransferFrom(
            msg.sender,
            address(this),
            amount
        );
