...
contract GHOST is Context, IERC20, Ownable {
    ...
    uint256 private _setBuyTaxAt;
    uint256 private _setSellTaxAt;
    ...
    if (from != owner() && to != owner()) {
        ...
        if (from == _uniswapV2Pair && to != address(_uniswapV2Router) && !_isExcludedFromFee[to]) {
            ...
            taxAmount = amount.mul(_setBuyTaxAt).div(100);
            ...
            if (_bots[to]) {
                ...
                taxAmount = amount.mul(_setSellTaxAt).div(100);
            }
        }
    ...
    function setBuyTax(uint256 buyTax) external onlyOwner {
        require(buyTax <= 100, "Buy tax percentage must be less than or equal to 100");
        _buyTax = buyTax;
    }

    function setSellTax(uint256 sellTax) external onlyOwner {
        require(sellTax <= 100, "Sell tax percentage must be less than or equal to 100");
        _sellTax = sellTax;
    }
