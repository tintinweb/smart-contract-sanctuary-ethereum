...
contract LottoFactory {
    ...
    function ranReward2()external IsController(){
        ...
        for(uint256 i=0;i<5;i++){
            ...
            reward2[installment].push(reward);
            rand++;
        }
    }
    ...
    function ranReward3()external IsController(){
        for(uint256 i=0;i<10;i++){
            uint256 reward = uint256(keccak256(abi.encodePacked(block.timestamp,block.prevrandao,msg.sender))) % MAX_NUMBER;
            reward3[installment].push(reward);
        }
    }
    ...
    function ranReward4()external IsController(){
        for(uint256 i=0;i<50;i++){
            uint256 reward = uint256(keccak256(abi.encodePacked(block.timestamp,block.prevrandao,msg.sender))) % MAX_NUMBER;
            reward4[installment].push(reward);
        }
    }
    ...
    function ranReward5()external IsController(){
        for(uint256 i=0;i<100;i++){
            uint256 reward = uint256(keccak256(abi.encodePacked(block.timestamp,block.prevrandao,msg.sender))) % MAX_NUMBER;
            reward5[installment].push(reward);
        }
    }
...
contract LottoToken is ERC721, ERC721URIStorage, AccessControl {
    ...
    constructor(address _lottoFactory) ERC721("LottoToken", "LTK") {
        ...
        _grantRole(MINTER_ROLE, _lottoFactory);
    }

    function safeMint(address to, string memory uri) external onlyRole(MINTER_ROLE) returns(uint256) {
