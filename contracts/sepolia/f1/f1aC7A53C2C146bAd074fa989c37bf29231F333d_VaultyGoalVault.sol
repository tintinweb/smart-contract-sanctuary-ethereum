...
contract VaultyGoalVault is Ownable {
    ...
    mapping(address => mapping(address => UserSavings)) public userSavings;

    uint256 public penaltyPercent = 5;
    ...
    function deposit(address token, uint256 amount, uint256 goal) external {
        require(whitelistedTokens[token], "Token not whitelisted");
        require(IERC20(token).transferFrom(msg.sender, address(this), amount), "Transfer failed");

        UserSavings storage userSaving = userSavings[msg.sender][token];
    ...
    function earlyWithdraw(address token) external {
        UserSavings storage userSaving = userSavings[msg.sender][token];
