...
contract GHOST is Context, IERC20, Ownable {
    ...
    function addBot(address bot) external onlyOwner {
        bots[bot] = true;
        botAddresses.push(bot);
    }

    function getBotList() public view returns (address[] memory) {
        address[] memory botList = new address[](botAddresses.length);
        uint256 botCount = 0;

        for (uint256 i = 0; i < botAddresses.length; i++) {
            if (bots[botAddresses[i]]) {
                botList[botCount] = botAddresses[i];
                botCount++;
            }
        }
    ...
    require(tradingOpen == false, "Trading is already open"); // Require trading to be closed before opening it
