...
abstract contract Staking20 is ReentrancyGuard, IStaking20 {
    ...
    //Burn
    address burnToAddress = 0x000000000000000000000000000000000000dEaD;
    //Set%
...
//     uint256 burnAmount = (_amount * 10) / 100; // Calculate 10% fee
//     uint256 withdrawAmount = _amount - burnAmount; // Calculate the actual amount to withdraw
...
//         stakingToken,
//         address(this),
//         burnToAddress, // Replace with the desired recipient address
//         burnAmount,
//         nativeTokenWrapper
//     );
...
function getFeeBurnUnstakePercentage() external view returns (uint256) {
...
function _withdraw(uint256 _amount) internal virtual {
    ...
    uint256 burnAmount = (_amount * feePercentage) / 100; // Calculate fee amount based on percentage
    uint256 withdrawAmount = _amount - burnAmount; // Calculate the actual amount to withdraw
    ...
    uint256 fee = rewards / 10; // เธซเธฒเธเนเธฒเธเธต 10% เธเธฒเธ rewards
    uint256 finalRewards = rewards - fee; // rewards เธ—เธตเนเธเธฐเธกเธญเธเนเธซเนเธเธฑเธเธเธนเนเน€เธฃเธตเธขเธเน€เธเธฅเธก

    stakers[_stakeMsgSender()].timeOfLastUpdate = block.timestamp;
    stakers[_stakeMsgSender()].unclaimedRewards = 0;
    stakers[_stakeMsgSender()].conditionIdOflastUpdate = nextConditionId - 1;

    _mintRewards(_stakeMsgSender(), finalRewards);

    emit RewardsClaimed(_stakeMsgSender(), finalRewards);
}
...
contract Staking20Base is ContractMetadata, Multicall, Ownable, Staking20 {
    ...
    //** require(_rewardToken != _stakingToken,  >>> // _rewardToken == _stakingToken */

    //** Staking Token and Reward Token be same. */
    //     constructor(
    //     uint256 _timeUnit,
    //     uint256 _rewardRatioNumerator,
    //     uint256 _rewardRatioDenominator,
    ...
    //     require(_stakingToken == _rewardToken, "Staking Token and Reward Token be same.");
...
pragma solidity ^0.8.0;
contract Stake20 is Staking20Base {
    constructor(
        uint256 _timeUnit,
        uint256 _rewardRatioNumerator,
        uint256 _rewardRatioDenominator,
