...
import "./interfaces/ISwapEvent.sol";
...
contract DEINStakingView is IDEINStakingView, AbstractDependant, Initializable {
        ...
        onlyInjectorOrZero
    {
        ...
        swapEvent = ISwapEvent(_contractsRegistry.getSwapEventContract());
    ...
    {
        uint256[] memory _tokenIds = deinNFTStaking.getUserLockedNFTs(user);
        uint256 length = _tokenIds.length;

        for (uint256 i = 0; i < length; i = uncheckedInc(i)) {
            uint256 tokenID = _tokenIds[i];
            (uint256 _totalStakedM2, ) = getNFTStakedByVotingMultiplier(tokenID);
            _totalVotingPower = _totalVotingPower.add(_totalStakedM2);
        }

        _totalVotingPower = _calcVotingPower(_totalVotingPower, user);
    }
        ...
        returns (uint256 _totalVotingPower)
    {
        for (uint256 i = 0; i < _tokenIds.length; i = uncheckedInc(i)) {
            uint256 tokenID = _tokenIds[i];
            require(deinStaking.ownerOf(tokenID) == _user, "DEINStakingView: Invalid user");
            (uint256 _totalStakedM2, ) = getNFTStakedByVotingMultiplier(tokenID);
            _totalVotingPower = _totalVotingPower.add(_totalStakedM2);
        }
        _totalVotingPower = _calcVotingPower(_totalVotingPower, _user);
    }
        ...
        returns (uint256 _totalStakedM2)
    {
        for (uint256 i = 0; i < _tokenIds.length; i = uncheckedInc(i)) {
            uint256 tokenID = _tokenIds[i];
            (uint256 _stakedM2, ) = getNFTStakedByVotingMultiplier(tokenID);
            _totalStakedM2 = _totalStakedM2.add(_stakedM2);
        }
    }

    function _calcVotingPower(uint256 _totalStakedM2, address _user)
        internal
        view
        returns (uint256)
    {
        ...
        return
            Math.sqrt(
                _totalStakedM2.mul(CUSTOM_PRECISION).mul(reputationSystem.reputation(_user))
            );
    }
    ...
    {
        _totalStakedM2 = deinStaking.getFullStakedAmount(_tokenId);
        ...
        if (deinStaking.stakingHasBenefits(_tokenId)) {
            ...
            _totalStakedM2 = _totalStakedM2.mul(_voterMultiplier);
        }
    }
    ...
    {
        (uint256 locked, uint256 claimed) = IDEINStaking(address(deinStaking)).vestings(_tokenId);

        dueAmount = getDueVestedAmount(
            deinStaking.getStakerLockingDuration(_tokenId),
            locked,
            claimed
        );

        dueAmountByLiquidation = deinStaking.getAmountbyLiquidation(dueAmount);
    }
    ...
    function getDueVestedAmount(
        ...
        uint256 _locked,
        uint256 _claimed
    ) public view virtual override onlyDEINStaking returns (uint256 _claimAmount) {
        uint256 eventEndBlock = swapEvent.eventEndBlock();
        if (block.number > eventEndBlock) {
            // total blocks of the veting period
            uint256 totalBlocks =
                _lockingPeriod.uncheckedSub(CLIFF_DURATION).mul(BLOCKS_PER_MONTH);

            uint256 passedBlocks = Math.max(block.number.uncheckedSub(eventEndBlock), totalBlocks);

            _claimAmount = _locked.mul(passedBlocks).uncheckedDiv(totalBlocks).sub(_claimed);
        }
    }
...
interface IDEINStaking {
    ...
    function vestings(uint256 _tokenId) external view returns (uint256 locked, uint256 claimed);

    function tokenToVesting(address user, uint256 lockingPeriod) external view returns (uint256);
...
interface ISwapEvent {
    function eventEndBlock() external view returns (uint256);
}
