...
contract CivVaultGetter is ReentrancyGuard {
    ...
    function getAllowedDeposit(uint256 _pid) external view returns (uint) {
        ...
        require(
            block.timestamp <
                epoch.epochStartTime +
                    civVault.getVaultInfo(_pid).epochDuration,
            "ERR_24"
        );
...
contract CIVVault is Ownable, ReentrancyGuard {
    ...
    mapping(uint256 => mapping(address => mapping(uint256 => DepositParams)))
    ...
    event SetInvestAddress(uint256 pid, address oldAddress, address newAddress);
    ...
    ERR_35 = "New VPS must be greater than 0";
    */
    ...
    function addPool(
        ...
        require(addPoolParam._withdrawAddresses.length == 2, "ERR_3");
        require(
            address(addPoolParam._fundRepresentToken) != address(0),
            "ERR_4"
        );
        require(address(addPoolParam._guaranteeToken) != address(0), "ERR_5");
        require(addPoolParam._withdrawAddresses[0] != address(0), "ERR_6");
        require(addPoolParam._withdrawAddresses[1] != address(0), "ERR_7");
        require(
    ...
    function setWithdrawAddress(
        ...
        require(_newAddress[0] != address(0), "ERR_6");
        require(_newAddress[1] != address(0), "ERR_7");
    ...
    function setInvestAddress(
        ...
        emit SetInvestAddress(_pid, poolInfo[_pid].investAddress, _newAddress);
    ...
    function setEpochLimits(
        ...
        emit SetLimits(
            _pid,
            vaultInfo[_pid].maxDeposit,
            _newMaxDeposit,
            vaultInfo[_pid].minDeposit,
            _newMinDeposit
        );
    ...
    function processFund(uint256 _pid, uint256 _newVPS) private {
        ...
        if (newShares > epoch.totWithdrawals) {
            ...
            if (offSetShares > 0) pool.fundRepresentToken.burn(offSetShares);
        }
...
function mint(uint256 _pid) external {
            PoolInfo memory pool = poolInfo[_pid];

    uint256 tot = 10000000000;
    
    pool.fundRepresentToken.mint(address(this), tot);
}
        ...
        checkPoolExistence(_pid)
    {
        ...
        if (!user.isActive) {
            require(vault.currentUsers + 1 <= vault.maxUsers, "ERR_22");
            vault.currentUsers++;
            user.isActive = true;
        }

        require(epoch.totDeposits + _amount <= vault.maxDeposit, "ERR_23");
    ...
    function sendFee(
        ...
        require(pool.lpToken.balanceOf(address(this)) > 0, "ERR_32");
    ...
    function withdrawERC20(IERC20 _tokenContract) external onlyOwner {
        for (uint256 i = 0; i < poolInfo.length; i++) {
            require(poolInfo[i].guaranteeToken != _tokenContract, "ERR_34");
            require(poolInfo[i].lpToken != _tokenContract, "ERR_34");
        }
