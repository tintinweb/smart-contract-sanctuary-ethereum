...
contract WillFactory is AccessControl{
    ...
    // show [0]
    function getInformationsWill(address _owner) public view returns(string []memory,string []memory,string []memory,address []memory,uint256 []memory ){
        uint256 i = 0;
        uint256 length = tokenIdOwner[_owner].length;
        string []memory names = new string[](length);
        string []memory descs =  new string[](length);
        string []memory ipfss = new string[](length);
        address []memory willsAddress = new address[](length); 
        uint256 []memory tokenIds = new uint256[](length);
        for(i = 0;i<length ; i++){
            uint256 _tokenId = tokenIdOwner[_owner][i];
            address willAddress = tokendIdOfWill[_tokenId];
            string memory name = Will(willAddress).getName();
            string memory desc = Will(willAddress).getDescription();
            string memory ipfs = WillToken(willTokenAddress).tokenURI(_tokenId);
            willsAddress[i] = willAddress;
            names[i] = name;
            descs[i] = desc;
            ipfss[i] = ipfs;
            tokenIds[i] = _tokenId;
        ...
        return (names,descs,ipfss,willsAddress,tokenIds);
    }
    ...
    function deleteWill(uint256 _tokenId) external {
        ...
        uint256 []memory tokenIds = tokenIdOwner[msg.sender];
        delete tokenIdOwner[msg.sender];
        for(uint i = 0;i < length;i++){
            if (tokenIds[i] != _tokenId){
                tokenIdOwner[msg.sender].push(tokenIds[i]);
            }
        }
    }
    ...
    function claimWill(address _willContract , uint256 _tokenId )external onlyRole(Controller){
        ...
        uint256 length = tokenIdOwner[owner].length;
        ...
        uint256 []memory tokenIds = tokenIdOwner[owner];
        delete tokenIdOwner[owner];
        for(uint i = 0;i < length;i++){
            if (tokenIds[i] != _tokenId){
                tokenIdOwner[owner].push(tokenIds[i]);
            }
        }
...
contract Will {
    ...
    function setDesc(string memory _description)external {
        description = _description;
    }
