...
contract ALTGPTPresale is Ownable, ReentrancyGuard {
    ...
    constructor(address _dataFeed) {
        ...
        setUSDT(address(0x4C4aBd6aeaDD95d462FBC0cbdECB9fDd741e51C2));
        start();
    }
    ...
    function purchaseETH(address referrer) external payable nonReentrant() {
        ...
        address level1 = referrers[msg.sender];
        if (level1 != address(0)) {
            payable(level1).transfer(value * level1Percentage / 100);
            address level2 = referrers[level1];
            if (level2 != address(0)) {
                payable(level2).transfer(value * level2Percentage / 100);
            }
        }

    }

    function purchaseUSDT(uint256 _amount, address referrer) external {
        require(currentStage > 0, "Please start presale first");
        ...
        address level1 = referrers[msg.sender];
        if (level1 != address(0)) {
            USDT.transfer(level1, value * level1Percentage / 100);
            address level2 = referrers[level1];
            if (level2 != address(0)) {
                USDT.transfer(level2, value * level2Percentage / 100);
            }
        }
    }
