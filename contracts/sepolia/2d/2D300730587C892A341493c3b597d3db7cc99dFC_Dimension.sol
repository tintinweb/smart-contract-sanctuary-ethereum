...
contract Dimension is ERC721A, ReentrancyGuard, Ownable, DefaultOperatorFilterer{
    ...
    uint256 public maxMintPreSale = 1;
    ...
    function setMaxMintPreSale(uint256 _maxMintPreSale) public onlyOwner {
    ...
    function publicMint(uint256 _quantity) external payable callerIsUser {
        ...
        if(totalMintedPublic[msg.sender] + _quantity > maxMintPublic) revert MaxPerWalletExceeded();
        if((publicMintedAmount + _quantity) > maxSupply) revert MaxSupplyExceeded();
        if(msg.value < mintPricePublic * _quantity) revert InsufficientValue();
        refundIfOver(mintPricePublic * _quantity);
    ...
    function presaleMint(uint256 _quantity, bytes32[] memory _merkleProof) external payable callerIsUser {
        if(!preSale) revert PreSaleNotActive();
        if(totalMintedPreSale[msg.sender] + _quantity > maxMintPreSale) revert MaxPerWalletExceeded();
        if(msg.value < mintPricePreSale * _quantity) revert InsufficientValue();
        if((presaleMintedAmount + _quantity) > (reservedSupplyPresale + maxSupply)) revert MaxSupplyPreSaleExceeded();
        refundIfOver(mintPricePreSale * _quantity);
