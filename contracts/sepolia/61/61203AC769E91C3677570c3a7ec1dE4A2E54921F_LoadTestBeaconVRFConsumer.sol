...
contract LoadTestBeaconVRFConsumer is BeaconVRFConsumer {
    ...
    uint256 public s_slowestRequestID;
    mapping(uint256 => uint256[]) public s_requestIDs;
    ...
    function testRequestRandomnessFulfillmentBatch(
        ...
        uint256[] memory reqIds = new uint256[](batchSize);
        ...
        for (uint256 i = 0; i < batchSize; i++) {
            uint256 reqId = testRequestRandomnessFulfillment(
                subID,
                numWords,
    ...
    function getFulfillmentDurationByRequestID(uint256 reqID)
        external
        view
        returns (uint256)
    {
