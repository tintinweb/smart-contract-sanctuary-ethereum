    ...
    PullPaymentUpgradeable
{
    ...
    // feeTokenSymbol => feeTokenAddress
    mapping(bytes32 => address) public feeTokenAddresses;
    ...
    event FeeTokenAddressSet(bytes32 tokenSymbol, address tokenAddress);
    event Paid();
    ...
    function initialize(IVerifier verifier_, uint8 feeCountLimit_, bytes32 nativeToken_) external initializer {
    ...
    function setFeeTokenAddress(bytes32 tokenSymbol, address tokenAddress) external onlyRole(DEFAULT_ADMIN_ROLE) {
        feeTokenAddresses[tokenSymbol] = tokenAddress;
        emit FeeTokenAddressSet(tokenSymbol, tokenAddress);
    }
    ...
    function _transferFees(INFTFactory.Fee[] calldata fees, address payee) private {
        ...
        for (uint i = 0; i < feeCount; ) {
            ...
            if (fee.token == nativeToken_) {
                ...
                address tokenAddress = feeTokenAddresses[fee.token];
                if (tokenAddress == address(0)) {
                    revert UnsupportedFeeToken(fee.token);
                }
                IERC20Upgradeable(tokenAddress).safeTransferFrom(msg.sender, fee.payee, fee.amount);
            }
        ...
        if (msg.value > totalNativeTokenFee) {
            _asyncTransfer(payee, msg.value - totalNativeTokenFee);
        }
    }
