...
contract AddressProvider is IAddressProvider, AccessControlled {
    ...
    function setFactory(address factory_) public onlyAddressManager {
        if (factory_ == address(0)) revert Errors.NullAddressNotAllowed();
        _factory = factory_;
    }

    function setPaymentTokenRegistry(address paymentTokenRegistry_) public onlyAddressManager {
        if (paymentTokenRegistry_ == address(0)) revert Errors.NullAddressNotAllowed();
        _paymentTokenRegistry = paymentTokenRegistry_;
    }

    function setTreasury(address treasury_) public onlyAdmin {
        if (treasury_ == address(0)) revert Errors.NullAddressNotAllowed();
        _treasury = treasury_;
    }

    function setMetadataProvider(address metadataProvider_) public onlyAdmin {
        if (metadataProvider_ == address(0)) revert Errors.NullAddressNotAllowed();
        _metadataProvider = metadataProvider_;
    }

    function _getAddress(address address_) private pure returns (address) {
        if (address_ == address(0)) revert Errors.AddressNotSet();
        return address_;
    }
}
