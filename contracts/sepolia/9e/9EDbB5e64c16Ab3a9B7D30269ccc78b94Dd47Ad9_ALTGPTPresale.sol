...
contract ALTGPTPresale is Ownable, ReentrancyGuard {
    ...
    function setStage(uint256 _price, uint256 _countdown, uint256 _totalAmount, uint256 _currentAmount) public onlyOwner {
        ...
        require(ALTGPT.balanceOf(address(this)) >= _countdown, "Insufficient token balance");
    ...
    function setCountdown(uint256 _countdown) public onlyOwner {
        require(currentStage > 0, "Please start presale first");
        stages[currentStage].countdown = _countdown;
    }
    ...
    function purchaseETH(address referrer) external payable nonReentrant() {
        require(currentStage > 0, "Please start presale first");
        require(block.timestamp < stages[currentStage].countdown, "Presale is over");
    ...
    function start() public onlyOwner {
        ...
        currentStage++;
        require(ALTGPT.balanceOf(address(this)) >= 30000000000 * 10 ** 18, "Insufficient token balance");
        stages[currentStage].price = 50;
        stages[currentStage].countdown = block.timestamp + 15 days;
