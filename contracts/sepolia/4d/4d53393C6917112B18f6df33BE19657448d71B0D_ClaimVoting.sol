...
contract ClaimVoting is IClaimVoting, Initializable, AbstractDependant {
    ...
    address public deinStakingView;
    ...
    mapping(address => EnumerableSet.UintSet) internal _myVotes; // voter -> claimIndexes // all votes from voter
    mapping(address => EnumerableSet.UintSet) internal _myAwaitingVotes; // voter -> claimIndexes // not received votes from voter
        ...
        onlyInjectorOrZero
    {
        ...
        deinStakingView = _contractsRegistry.getDEINStakingViewContract();
    ...
    /// @notice returns the stored vote status {ANONYMOUS, EXPOSED, CLOSED}
    function getVoteStatus(uint256 voteIndex) external view returns (VoteStatus) {
        return _voteInfo[voteIndex].voteStatus;
    }

    /// @notice returns the calculated vote status {ANONYMOUS, EXPOSED, TO_RECEIVE, CLOSED}
    ...
    /// @notice returns the repartition of vote for or against, once the vote is resolved
    ...
    function getVotingPower(uint256 voteIndex) public view returns (uint256) {
        uint256 sum =
            IDEINStakingView(deinStakingView).getTotalStakedByVotingMultiplier(
                _convertEnumToArray(voteIndex)
            );
        /// @dev add custom precision to make the two numbers equal in precision
        return Math.sqrt(sum.mul(CUSTOM_PRECISION).mul(_voteInfo[voteIndex].votingReputation));
    }
    ...
    /// @dev limit = myAwaitingVotesCount()
    function getListAwaitingVotes(
        address user,
        uint256 offset,
        uint256 limit
    ) external view returns (uint256[] memory list) {
        uint256 to = (offset.tryAdd(limit)).min(_myAwaitingVotes[user].length()).max(offset);
    ...
    function getSuggestedAmount(uint256 voteIndex) external view returns (uint256) {
        return _voteInfo[voteIndex].suggestedAmount;
    }

    /// @notice return encrypted vote for expose voting
    /// @param  voteIndex the index of the voted
    function getEncryptedVote(uint256 voteIndex) external view returns (string memory) {
        return _voteInfo[voteIndex].encryptedVote;
    }

    // ********** GETTERS PROCESS ********** //
    /// @notice user must have 0 awaiting reception vote to be able to anonymously vote
    ...
    /// @notice user must not use his NFT in any awaiting calculation or reception vote to be able to unlock it
    ...
    /// @notice this function carries all condiditons on claim instance and voter to be able to vote anonymously
    ...
    /// @notice this function carries all condiditons on claim instance and voter to be able to expose vote
    ...
    /// @notice this function carries all condiditons on claim instance and voter to be able to receive vote
    ...
    // ********** VOTE PROCESS ********** //
    /// @param claimIndexes uint256[] array of claims on which user is voting
    /// @param finalHashes bytes32[] array of final hashes (got by encryption)
    /// @param encryptedVotes string[] array of encrypted votes (got by encryption)
    /// @param tokenIDs uint256[] array of locked tokens user uses to vote for each claim
    ...
    /// @param claimIndexes uint256[] array of claims on which user is voting
    /// @param suggestedAmounts uint256[] array of suggested amount for comparison with encryption
    /// @param hashedSignaturesOfClaims bytes32[] array of signatures
    /// @param isConfirmed bool[] array of bool : for each claim does user confirm or cancel his vote
    /// @dev if a vote is not expose after anonymously voted, the user will get a penalty
    function exposeVoteBatch(
        ...
        for (uint256 i = 0; i < claimIndexes.length; i = uncheckedInc(i)) {
            ...
            // remove vote encryption data , not needed after expose vote
            delete _voteInfo[voteIndex].encryptedVote;
            _voteInfo[voteIndex].finalHash = 0;
    ...
    // ********** RECEIVE PROCESS ********** //
    /// @param claimIndexes uin256[] claims user wants to receive
    function receiveVoteResultBatch(uint256[] calldata claimIndexes) external override {
        ...
        for (uint256 i = 0; i < claimIndexes.length; i = uncheckedInc(i)) {
            ...
            if (
                ...
                (uint256 deinReward, uint256 deinPenalty, uint256 newReputation) =
                    getReward(claimIndex, voteIndex, msg.sender, reputation);
    ...
    /// @notice for each claim index, user can view his reward
    /// @dev if vote is not exposed, voter gets a penalty
    /// @dev if vote is exposed, reward depend on MAJORITY / MINORITY / extreme MINORITY
    function getReward(
        uint256 claimIndex,
        uint256 voteIndex,
        address voter,
...
interface IDEINStakingView {
    ...
    function getUserVotingPower(address user) external view returns (uint256 _totalVotingPower);

    function getExpectedVotingPower(uint256[] calldata _tokenIds, address _user)
        external
        view
        returns (uint256 _totalVotingPower);

    function getTotalStakedByVotingMultiplier(uint256[] calldata _tokenIds)
        external
        view
        returns (uint256 _totalStakedM2);

    function getNFTStakedByVotingMultiplier(uint256 _tokenId)
        external
        view
        returns (uint256 _totalStakedM2, uint256 _voterMultiplier);

    function getUserStakingPower(address _user) external view returns (uint256 _totalStakingPower);

    function getDueVestedAmount(
        uint256 _lockingPeriod,
        uint256 _locked,
        uint256 _claimed
    ) external view returns (uint256 _claimAmount);
}
...
interface IStaking {
    ...
    function getAmountbyLiquidation(uint256 _amount) external view returns (uint256);
