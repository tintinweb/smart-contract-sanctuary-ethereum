...
contract DEEWPW3B is ERC20, Ownable {
    ...
    uint256 public totalFee = liquidityFee.add(operationsFee);
    
    uint256 public swapTokensAtAmount = 100000 * (10**18);

    address payable public operationsWallet = payable(0x9a65278dC3Faa9d88299A7d5c7ba27ab63Bd850b);    
    ...
    constructor() ERC20("Deepw3b", "DW3B_TEST") {
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 ethForOperations = totalEthBalance.mul(operationsFee).div(totalFee).div(2);
        
        addLiquidity(liquidityTokens, ethForLiquidity);

        emit SwapAndLiquify(liquidityTokens, ethForLiquidity, swappableTokens);

        operationsWallet.transfer(ethForOperations); 
    }
    ...
    event FeesUpdated(uint256 liqFee, uint256 operationsFee);

    function setFees(uint256 _liqFee, uint256 _operationsFee) public onlyOwner() {
        require(!taxesAreLocked, "Taxes are locked.");        
        require(_liqFee.add(_operationsFee) <= 6, "tax too high");
        liquidityFee = _liqFee;
        operationsFee = _operationsFee;
        totalFee = _liqFee.add(_operationsFee);
        emit FeesUpdated(_liqFee, _operationsFee);
    }
    ...
    function updateWallets(address payable _operationsWallet) public onlyOwner {  
        operationsWallet = _operationsWallet;        
        emit WalletsUpdated(operationsWallet);
    }
