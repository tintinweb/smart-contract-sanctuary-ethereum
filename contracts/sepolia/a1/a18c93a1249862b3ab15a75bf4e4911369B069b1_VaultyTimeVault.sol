...
contract VaultyTimeVault is TwoStepOwnable {
  using SafeERC20 for IERC20;
  using SafeMath for uint256;

  uint256 public creationFee;
  uint256 public penaltyFeePercent;
  uint256 private _vaultIdCounter;

  uint256 private creationFeePool;
  mapping(address => uint256) private penaltyFeePool;

  struct Vault {
    uint256 id;
    address depositor;
    address withdrawableBy;
    address token;
    uint256 amount;
    uint256 unlockTime;
  ...
  event PenaltyFeePercentUpdated(uint256 newFeePercent);
  ...
  event ProtocolFeesWithdrawn(address owner, uint256 amount);
  ...
  function setPenaltyFeePercent(uint256 newFeePercent) public onlyOwner {
    penaltyFeePercent = newFeePercent;
    emit PenaltyFeePercentUpdated(penaltyFeePercent);
  }
  ...
  function createVault(
    ...
    // Check the user is paying the correct creation fee
    require(
      msg.value == creationFee,
      "Incorrect creation fee. Please send the required amount."
    );
    ...
    // Add the creation fee to the creationFeePool
    creationFeePool += msg.value;

    // Transfer the tokens to this contract
    IERC20(token).safeTransferFrom(msg.sender, address(this), amount);

    // Update the vault id counter
    _vaultIdCounter++;
  ...
  function withdraw(uint256 vaultId) public {
    Vault storage userDeposit = vaultInfo[vaultId];

    // Check the user is the depositor
    require(
      userDeposit.withdrawableBy == msg.sender,
    ...
    emit Withdrawal(
      vaultId,
      msg.sender,
      userDeposit.withdrawableBy,
      userDeposit.token,
      amount
    );
  }

  function earlyWithdraw(uint256 vaultId) public {
    Vault storage userDeposit = vaultInfo[vaultId];

    // Check the deposit is emergency unlockable
    require(
      userDeposit.isWithdrawableEarly,
      "Vault is not emergency unlockable."
    );

    // Check the is the withdrawer
    require(
      userDeposit.withdrawableBy == msg.sender,
      "Only the specificed withdrawer can withdraw."
    );
    ...
    uint256 fee = amount * penaltyFeePercent / 100;
    uint256 amountToTransfer = amount - fee;

    // Transfer the tokens back to the depositor
    IERC20(userDeposit.token).safeTransfer(msg.sender, amountToTransfer);

    // Add the fee to the withdrawable fees
    penaltyFeePool[userDeposit.token] += fee;
  }
  ...
  // To withdraw fees from Vaulty penalty fees pool
  function withdrawPenaltyFees(address token) public onlyOwner {
    uint256 amount = penaltyFeePool[token];
    require(amount > 0, "No fees to withdraw.");

    // Reset the withdrawable fees
    penaltyFeePool[token] = 0;
