...
import {ChiToken, Test, TestTwo} from "src/Chi.sol";

contract AAA {
    address public immutable owner = tx.origin;
    uint256 public a;

    function test() external returns (uint, string memory) {
...
contract AAAScript is Script {
    function run() external {
        uint256 privateKey = vm.envUint("PRIVATE_KEY0");
...
contract Test {
    ...
    function test() external {
        ...
        // chi token
        assembly {
            gasSpent := add(add(21000, sub(gasStart, gas())), mul(16, calldatasize()))

            gasSpent := div(add(gasSpent, 14154), 41947)
        }

        chi.freeFromUpTo(msg.sender, gasSpent);
    }
}
