...
contract skateDonate is ERC1155, AccessControl, ERC1155Burnable,Pausable, ERC1155Supply, PaymentSplitter {
    ...
    */
    ...
    function checkForeignTokenLimits(address _wallet, uint256 _desiredAmount) internal view returns (bool) {
        // If there are no rules, it is always okay to mint
        if (foreignTokenLimits.length < 1) {
            return true;
        }

        // Sum up all foreignToken balances multiplied by the corresponding multipliers
