...
contract TraitManager is Modifiers {
  modifier layerExist(uint8 layerId) {
    require(LibTraitManager.getStorage().layers.length <= layerId, "TraitManager: Layer doesnt exist");
    _;
  }

  modifier traitExist(uint8 layerId, uint16 traitId) {
    require(LibTraitManager.getStorage().layers[layerId].traits.length <= traitId, "TraitManager: Trait doesnt exist");
    _;
  }
  ...
  function getTokenAttributeOccurrences(uint16 tokenId) public view returns (uint16[] memory) {
    ...
    unchecked {
      ...
      for (uint256 layerId = 0; layerId < layerCount; layerId++) {
        ...
        queryItems[layerId] = new string[](10000);
      }
