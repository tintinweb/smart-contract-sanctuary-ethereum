...
modifier campaignHasNotExpired(uint256 _campaignId) {
    Campaign campaign = Campaign(campaigns[_campaignId]);
    (,,uint256 activeTime,) = campaign.getDetails();
    require(
    ...
    function addBrand(
        ...
        brands[brandAddress] = Brand(name, new uint256[](0), true);
    ...
    function createCampaign(
        ...
        newCampaign.grantRole(newCampaign.DEFAULT_ADMIN_ROLE(), address(this));
    }
...
function createCampaign2(
    ...
    require(_activeTime > MIN_CAMPAIGN_ACTIVE_TIME, "Active time must be greater than minimum");
...
contract Campaign is ERC20, Ownable, AccessControl {
    // ----------------- Variables -----------------
    ...
    event AirDropCompleted(address payable[] user_rewarded , uint256[] rewards);
    ...
    constructor(
        ...
        campaignDetails.users = [address(0)];
        campaignDetails.activeTime = _activeTime;
        ...
        _mint(_brand, _initialSupply);
        _grantRole(ADMIN_ROLE, _brand);
    ...
    returns (string memory, string memory, uint256, uint256)
{
    return (
        ...
        campaignDetails.activeTime,
        campaignDetails.applicationWindowEnd
    );
}

    function getUsers() public view returns (address[] memory) {
        return campaignDetails.users;
    }
    ...
    function assign(address _user) public onlyRole(ADMIN_ROLE) {
        campaignDetails.users.push(_user);
    ...
    function doAirdrop(address payable[] calldata rewarded_users, uint256[] calldata values) onlyOwner public{
        require(rewarded_users.length == values.length, "Invalid user/value lists, tokens were not sent");
        require(campaignDetails.status== Status.Finished, "Campaign must be finished before sending tokens");
        uint256 i = 0;
        while (i < rewarded_users.length) {
            _transfer(msg.sender, rewarded_users[i], values[i]);
            i += 1;
        }
        emit AirDropCompleted(rewarded_users, values);
    }
}