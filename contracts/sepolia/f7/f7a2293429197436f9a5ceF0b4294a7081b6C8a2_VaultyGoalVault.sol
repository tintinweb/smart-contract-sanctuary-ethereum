...
contract VaultyGoalVault is TwoStepOwnable {
    ...
    uint256 public totalPenalties = 0;
    ...
    event EmergencyWithdraw(address indexed user, uint256 depositId, address indexed token, uint256 amount, uint256 penaltyAmount);
    event ProtocolFeesWithdraw(address indexed owner, uint256 amount);
    ...
    function createGoal(address token, uint256 amount, uint256 goal, bool allowEmergencyWithdrawal) external payable {
        ...
        userSaving.id = depositId;
        userSaving.goal = goal;
        userSaving.balance += amount;
        userSaving.emergencyWithdrawalEnabled = allowEmergencyWithdrawal;
    ...
    function emergencyWithdraw(uint256 depositId, address token) external {
        UserSavings storage userSaving = userSavings[msg.sender][depositId];
        require(userSaving.balance > 0, "No savings to withdraw");
        require(userSaving.emergencyWithdrawalEnabled, "Emergency withdrawal not allowed");
        ...
        emit EmergencyWithdraw(msg.sender, depositId, token, amount, penaltyAmount);
    }

    function withdrawFeesFor(address token) external onlyOwner {
        uint256 amount = totalPenalties;
        totalPenalties = 0;

        require(IERC20(token).transfer(owner(), amount), "Penalty fee transfer failed");
        emit ProtocolFeesWithdraw(owner(), amount);
    }
