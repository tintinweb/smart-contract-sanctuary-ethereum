...
import {BaseConfig, AdvancedConfig, BurnToken, MadMintStorage} from "../libraries/MadMintStorage.sol";

contract MadMintERC721AExtensionA is MadMintERC721AUpgradeable {
    ...
    function updateBaseConfig(
        ...
        require(
            _baseConfig.madMintFeeActive == state.cfg.madMintFeeActive,
            "CANNOT_UPDATE_CONSTANT_VARIABLE"
        );
    ...
    function freezePayoutAddresses() external onlyOwner {
        MadMintStorage.state().advCfg.payoutAddressesFrozen = true;
    }
    ...
    function updatePayoutAddressesAndBasisPoints(
        ...
        AdvancedConfig storage advCfg = MadMintStorage.state().advCfg;
    ...
    function setRoyaltyBasisPoints(uint16 _royaltyBps) external onlyOwner {
        MadMintStorage.state().cfg.royaltyBps = _royaltyBps;
    }
    ...
    function gift(
        ...
        if (state.cfg.madMintFeeActive) {
            uint256 madmintFee = (totalMints * madmintFeePerToken()) / 10;
            require(msg.value == madmintFee, "PAYMENT_INCORRECT");
