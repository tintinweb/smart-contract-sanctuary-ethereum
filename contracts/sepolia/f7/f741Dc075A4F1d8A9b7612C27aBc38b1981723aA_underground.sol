...
contract underground is OwnableUpgradeable, ERC721EnumerableUpgradeable, DefaultOperatorFiltererUpgradeable {
    ...
    function getLockDuration(LockDuration lockDuration) internal pure returns(uint256) {
        if (lockDuration == LockDuration.None) {
            return 0;
        } else if (lockDuration == LockDuration.SixMonths) {
            return 6 * 30 days + 3 days;
        } else if (lockDuration == LockDuration.TwelveMonths) {
    ...
    function mint(uint256 price, LockDuration lockDuration, bytes32[] calldata merkleProof) external payable {
        require(mintTimestamps[msg.sender] == 0, "You have already minted a token");
        require(price == msg.value, "Invalid price");

        bytes32 node = keccak256(abi.encode(msg.sender, price, lockDuration));
        require(MerkleProof.verify(merkleProof, merkleRoot, node), "Invalid merkle proof");

        mintTimestamps[msg.sender] = block.timestamp + getLockDuration(lockDuration);
    ...
    function sellSeasonOne(uint256 id) external {
        IERC1155(0x0000000010C38b3D8B4D642D9D065FB69BC77bC7).safeTransferFrom(msg.sender, owner(), id, 1, "");
