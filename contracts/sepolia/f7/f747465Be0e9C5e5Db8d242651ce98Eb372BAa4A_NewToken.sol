...
library Address {
     ...
     * `to`, forwarding all available gas and reverting on errors.
     *
     ...
     *
     * IMPORTANT: because control is transferred to `to`, care must be
...
contract NewToken is IBEP20, Ownable {
    ...
    uint256 private _reflectTotal = (MAX - (MAX % _totalSupply));
    uint256 private _tFeeTotal;

    string private constant _name = "NewToken";
    string private constant _symbol = "NDT";
    ...
    constructor () {
        _rOwned[_msgSender()] = _reflectTotal;
    ...
    function balanceOf(address account) public view override returns (uint256) {
        if (_isExcluded[account]) return balances[account];
    ...
    function transferFrom(
        ...
        _approve(from, _msgSender(), _allowances[from][_msgSender()].sub(amount, "BEP20: transfer amount exceeds allowance"));
        return true;
    }
    ...
    function deliver(uint256 tAmount) public {
        ...
        _rOwned[from] = _rOwned[from].sub(reflectAmount);
    ...
    function reflectionFromToken(uint256 tAmount, bool deductTransferFee) public view returns(uint256) {
        ...
        if (!deductTransferFee) {
            ...
            (,uint256 reflectTransferAmount,,,,) = _getValues(tAmount);
    ...
    function excludeFromReward(address account) public onlyOwner {
        ...
        require(_excluded.length <= 50, "Excluded list is too long!");
            if (_rOwned[account] > 0) {
                balances[account] = tokenFromReflection(_rOwned[account]);
            }
    ...
    function includeInReward(address account) external onlyOwner() {
        ...
        for (uint256 i = 0; i < _excluded.length; i++) {
            if (_excluded[i] == account) {
                ...
                uint256 currentRate = _getRate();
                _rOwned[account] = balances[account].mul(currentRate);
                balances[account] = 0;
    ...
    function _transferBothExcluded(address from, address to, uint256 tAmount) private {
        (uint256 reflectAmount, uint256 reflectTransferAmount, uint256 reflectFee, uint256 tTransferAmount, uint256 transactionFee, uint256 takeLiquidity) = _getValues(tAmount);
        balances[from] = balances[from].sub(tAmount);
        _rOwned[from] = _rOwned[from].sub(reflectAmount);
        balances[to] = balances[to].add(tTransferAmount);
        _rOwned[to] = _rOwned[to].add(reflectTransferAmount);        
        _reflectFee(reflectFee, transactionFee);
    ...
    function _reflectFee(uint256 reflectFee, uint256 transactionFee) private {
        _reflectTotal = _reflectTotal.sub(reflectFee);
        _tFeeTotal = _tFeeTotal.add(transactionFee);
    }

    function _getValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256, uint256) {
        (uint256 tTransferAmount, uint256 transactionFee, uint256 takeLiquidity) = _getTValues(tAmount);
        (uint256 reflectAmount, uint256 reflectTransferAmount, uint256 reflectFee) = _getRValues(tAmount, transactionFee, takeLiquidity, _getRate());
        return (reflectAmount, reflectTransferAmount, reflectFee, tTransferAmount, transactionFee, takeLiquidity);
    }

    function _getTValues(uint256 tAmount) private view returns (uint256, uint256, uint256) {
        uint256 transactionFee = calculateTaxFee(tAmount);
        uint256 takeLiquidity = calculateLiquidityFee(tAmount);
        uint256 tTransferAmount = tAmount.sub(transactionFee).sub(takeLiquidity);
        return (tTransferAmount, transactionFee, takeLiquidity);
    }

    function _getRValues(uint256 tAmount, uint256 transactionFee, uint256 takeLiquidity, uint256 currentRate) private pure returns (uint256, uint256, uint256) {
        uint256 reflectAmount = tAmount.mul(currentRate);
        uint256 reflectFee = transactionFee.mul(currentRate);
        uint256 rLiquidity = takeLiquidity.mul(currentRate);
        uint256 reflectTransferAmount = reflectAmount.sub(reflectFee).sub(rLiquidity);
    ...
    function _getCurrentSupply() private view returns(uint256, uint256) {
        ...
        for (uint256 i = 0; i < _excluded.length; i++) {
            if (_rOwned[_excluded[i]] > reflectSupply || balances[_excluded[i]] > tokenSupply) return (_reflectTotal, _totalSupply);
            reflectSupply = reflectSupply.sub(_rOwned[_excluded[i]]);
            tokenSupply = tokenSupply.sub(balances[_excluded[i]]);
        }
        if (reflectSupply < _reflectTotal.div(_totalSupply)) return (_reflectTotal, _totalSupply);
    ...
    function _takeLiquidity(uint256 takeLiquidity) private {
        ...
        if(_isExcluded[address(this)])
            balances[address(this)] = balances[address(this)].add(takeLiquidity);
    }
   ...
   /**
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        swapTokensForEth(half); // <- this breaks the native currency -> swap when swap+liquify is triggered
