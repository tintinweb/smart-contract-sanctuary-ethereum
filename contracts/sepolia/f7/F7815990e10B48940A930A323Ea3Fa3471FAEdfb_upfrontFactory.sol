...
contract upfrontFactory is ReentrancyGuard {
  ...
  uint256 private PRICE_DISTRIBUTION;
  uint256 private PRICE_DISTRIBUTION_PER_WALLET;
  ...
  function getTokenDistributionPrice() external view returns (uint256, uint256) {
    return (PRICE_DISTRIBUTION, PRICE_DISTRIBUTION_PER_WALLET);
  }

  function setTokenDistributionPrice(uint256 _price, uint256 _pricePerWallet) external isOwner isInitialized {
    PRICE_DISTRIBUTION = _price;
    PRICE_DISTRIBUTION_PER_WALLET = _pricePerWallet;
  }
  ...
  function tokenMultiTransfer(address token, address[] memory target, uint256[] memory amount) public payable isInitialized nonReEntrant {
    require(target.length == amount.length, "Factory: Invalid number of params.");
    ...
    unchecked {
      price = PRICE_DISTRIBUTION + (cnt * PRICE_DISTRIBUTION_PER_WALLET);
    ...
    unchecked {
      for (uint256 a; a < cnt; a++) {
        bool txTransfer = iface.transferFrom(msg.sender, target[a], amount[a]);
        require(txTransfer, "Transfer error.");
      }
    }
  }

  function tokenMultiMint(address token, address[] memory target, uint256[] memory amount, string memory method) public payable isInitialized nonReEntrant {
    require(target.length == amount.length, "Factory: Invalid number of params.");
    ...
    unchecked {
      for (uint256 a; a < cnt; a++) {
        (bool success, bytes memory result) = String.compare('mint(address,uint256)', method) ? token.delegatecall(abi.encodeWithSignature(method, target[a], amount[a])) : token.delegatecall(abi.encodeWithSignature(method, amount[a], target[a]));
