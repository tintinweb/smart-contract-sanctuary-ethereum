...
/**
 * @title EthenaLendingMarket
 * @notice Lending pools where users can deposit/withdraw collateral and borrow supported tokens.
 ...
 */
contract EthenaLendingMarket is Ownable, ReentrancyGuard, ILendingMarket
{
    ...
    /// @notice An event thats emitted when user borrows eUSD
    event Borrowed(address indexed user, uint256 eUSDAmount);
    ...
    /// @notice An event thats emitted when user repays eUSD
    event Repay(address indexed user, uint256 eUSDAmount);
    ...
    /// @notice eUSD token address
    address public immutable eUSDAddress;
    ...
    /// @notice eUSD borrow supported tokens
    mapping(address => bool) public eUSDBorrowSupportedTokens;
    /// @notice stables borrow supported tokens
    mapping(address => bool) public stablesBorrowSupportedTokens;
    /// @notice users collateral position
    mapping(address => mapping(address => Position)) public userPositions; // user => collateral token => position
    /// @notice eUSD total borrows per collateral token
     ...
     * @param _eUSD eUSD token address
     ...
     */
    function accrue() pure public {
        return; // accrual is performed in eUSD staking contract
    }
     ...
     */
    function depositForEUSD(
        address _token,
        uint256 _amount,
        address _onBehalfOf
    ) external nonReentrant {
        require(eUSDBorrowSupportedTokens[_token], "Invalid borrow eUSD deposit collateral type");
        require(
            collateralSettings[_token].status == CollateralStatus.Enabled,
            "not enabled"
        );
     ...
     * @param _token collateral token address
     * @param _eUSDAmount eUSD amount to borrow
     */
    function borrowEUSD(address _token, uint256 _eUSDAmount) external nonReentrant {
        // check if collateral is valid
        require(eUSDBorrowSupportedTokens[_token], "Invalid borrow eUSD collateral type");
        require(
            collateralSettings[_token].status == CollateralStatus.Enabled,
            "not enabled"
        );
        ...
        require(
            debtAmount + _eUSDAmount <= creditLimit,
            "insufficient collateral"
        );

        require(
            totalBorrowsPerCollateral[_token] + _eUSDAmount <=
        ...
        if (totalDebtPortion == 0) {
            totalDebtPortion = _eUSDAmount;
            position.debtPortion = _eUSDAmount;
        } else {
            uint256 plusPortion = (totalDebtPortion * _eUSDAmount) / totalDebtAmount;
            totalDebtPortion += plusPortion;
            position.debtPortion += plusPortion;
        }
        position.debtPrincipal += _eUSDAmount;
        totalDebtAmount += _eUSDAmount;
        ...
        // mint eUSD to user
        eUSD.mint(msg.sender, _eUSDAmount);

        emit Borrowed(msg.sender, _eUSDAmount);
    }
    ...
    /**
     * @notice repay position with eUSD
     * @dev user can call this function after approving his/her eUSD amount to repay
     * @param _token collateral token address
     * @param _eUSDAmount eUSD amount to repay
     */
    function repay(address _token, uint256 _eUSDAmount) external nonReentrant {
        // check if collateral is valid
        require(
            collateralSettings[_token].status != CollateralStatus.Invalid,
            "invalid token"
        ...
        _eUSDAmount = _eUSDAmount > debtAmount ? debtAmount : _eUSDAmount;

        // burn repaid eUSD
        eUSD.burnFrom(msg.sender, _eUSDAmount);

        // update user's collateral position
        uint256 paidPrincipal = _eUSDAmount > debtInterest
        ? _eUSDAmount - debtInterest
        : 0;
        uint256 minusPortion = paidPrincipal == debtPrincipal
        ? position.debtPortion
        : (totalDebtPortion * _eUSDAmount) / totalDebtAmount;
        ...
        emit Repay(msg.sender, _eUSDAmount);
    }

    /**
     * @notice deposit collateral for stable coins
     * @dev user can call this function after approving his/her collateral amount to deposit
     * @param _amount collateral amount to deposit (in eUSD)
     * @param _onBehalfOf user address to deposit
     */
    function depositEUSDForStables(
        uint256 _amount,
        address _onBehalfOf
    ) external nonReentrant {
        require(
            collateralSettings[eUSDAddress].status == CollateralStatus.Enabled,
            "not enabled"
        );

        uint256 shares = sharesFromAmount(eUSDAddress, _amount);

        // get eUSD collateral from depositor
        IERC20MetadataUpgradeable(eUSDAddress).safeTransferFrom(
            msg.sender,
            address(this),
            _amount
        );
        ...
        // update user's collateral position
        userPositions[_onBehalfOf][eUSDAddress].shares += shares;
        collateralTotalShares[eUSDAddress] += shares;

        emit Deposit(_onBehalfOf, eUSDAddress, _amount, shares);
    }

    /**
     * @notice borrow stable coins
     * @dev user can call this function after depositing his/her eUSD collateral
     * @param _stableToken stable token address, to borrow.
     * @param _stableAmount stable amount, to borrow.
     */
    function borrowStables(address _stableToken, uint256 _stableAmount) external nonReentrant {
        require(stablesBorrowSupportedTokens[_stableToken], "Invalid borrowable stablecoin");
        // check if collateral is valid
        require(
            collateralSettings[eUSDAddress].status == CollateralStatus.Enabled,
        "not enabled"
        ...
        // calculate borrow limit in USD
        uint256 creditLimit = _creditLimitUSD(msg.sender, eUSDAddress);
        // calculate debt amount in USD
        uint256 debtAmount = _debtUSD(msg.sender, eUSDAddress);
        // calculate borrow amount in USD
        uint256 amountUSD = _tokenUSD(_stableToken, _stableAmount);

        // check if additional borrow is available
        require(
            debtAmount + amountUSD <= creditLimit,
            "insufficient collateral"
        );

        require(
            totalBorrowsPerCollateral[eUSDAddress] + amountUSD <=
            collateralSettings[eUSDAddress].totalBorrowCap,
            "borrow cap reached"
        );

        // swap and transfer stables to user
        address swapper = addressProvider.getSwapper();
        IERC20(address(eUSD)).approve(swapper, _stableAmount);
        uint256 swappedAmount = ISwapper(swapper).swap(address(eUSD), _stableToken, _stableAmount, address(this));
        uint256 swappedAmountUSD = _tokenUSD(_stableToken, swappedAmount);
        IERC20MetadataUpgradeable(address(_stableToken)).safeTransfer(msg.sender, swappedAmount);

        // update user's collateral position
        Position storage position = userPositions[msg.sender][eUSDAddress];
        if (totalDebtPortion == 0) {
            totalDebtPortion = swappedAmountUSD;
            position.debtPortion = swappedAmountUSD;
        } else {
            uint256 plusPortion = (totalDebtPortion * swappedAmountUSD) /
            totalDebtAmount;
            totalDebtPortion += plusPortion;
            position.debtPortion += plusPortion;
        }
        position.debtPrincipal += swappedAmountUSD;
        totalDebtAmount += swappedAmountUSD;

        // increase total borrows of the collateral market
        totalBorrowsPerCollateral[eUSDAddress] += swappedAmountUSD;

        // check if new market user enters
        if (!marketUsers[eUSDAddress].contains(msg.sender)) {
            marketUsers[eUSDAddress].add(msg.sender);
        }

        emit Borrowed(msg.sender, swappedAmountUSD);
    }

    function repayStables(address _stableToken, uint256 _stableAmount) external nonReentrant {
        // check if collateral is valid
        require(stablesBorrowSupportedTokens[_stableToken], "Invalid borrow stable collateral type");
        require(
            collateralSettings[eUSDAddress].status != CollateralStatus.Invalid,
            "invalid token"
        );

        accrue();

        IERC20MetadataUpgradeable(_stableToken).safeTransferFrom(msg.sender, address(this), _stableAmount);

        address swapper = addressProvider.getSwapper();
        IERC20(_stableToken).approve(swapper, _stableAmount);
        uint256 _swappedAmount = ISwapper(swapper).swap(_stableToken, address(eUSD), _stableAmount, address(this));
        uint256 swappedAmountUSD = _tokenUSD(_stableToken, _stableAmount);

        require(_swappedAmount > 0, "invalid amount");

        Position storage position = userPositions[msg.sender][eUSDAddress];

        // calculate debt amount in USD
        uint256 debtAmountUSD = _debtUSD(msg.sender, eUSDAddress);
        uint256 debtPrincipal = position.debtPrincipal;
        uint256 debtInterest = debtAmountUSD - debtPrincipal;

        swappedAmountUSD = swappedAmountUSD > debtAmountUSD ? debtAmountUSD : swappedAmountUSD;

        eUSD.burnFrom(msg.sender, _swappedAmount);

        uint256 paidPrincipal = swappedAmountUSD > debtInterest
        ? swappedAmountUSD - debtInterest
        : 0;
        uint256 minusPortion = paidPrincipal == debtPrincipal
        ? position.debtPortion
        : (totalDebtPortion * swappedAmountUSD) / totalDebtAmount;
        ...
        if (position.debtPrincipal == 0) {
            // remove market user
            if (marketUsers[eUSDAddress].contains(msg.sender)) {
                marketUsers[eUSDAddress].remove(msg.sender);
            }
        }

        // decrease total borrows of the collateral market (exclude only principles)
        totalBorrowsPerCollateral[eUSDAddress] -= paidPrincipal;

        emit Repay(msg.sender, swappedAmountUSD);
    }

    function withdrawEUSD(uint256 _amount) external nonReentrant {
        // check if collateral is valid
        require(
            collateralSettings[eUSDAddress].status != CollateralStatus.Invalid,
            "invalid token"
        ...
        uint256 shares = sharesFromAmount(eUSDAddress, _amount);
        // need to consider dust here
        if (amountFromShares(eUSDAddress, shares) < _amount) {
            shares += 1;
        }
        ...
        uint256 creditLimitAfterWithdraw = (_tokenUSD(
            eUSDAddress,
            amountFromShares(eUSDAddress, position.shares - shares)
        ) * collateralSettings[eUSDAddress].creditLimitRate.numerator) /
        collateralSettings[eUSDAddress].creditLimitRate.denominator;
        // calculate debt amount in USD
        uint256 debtAmount = _debtUSD(msg.sender, eUSDAddress);
        ...
        // transfer collateral to user
        IERC20MetadataUpgradeable(eUSDAddress).safeTransfer(msg.sender, _amount);

        emit Withdraw(msg.sender, eUSDAddress, _amount, shares);
    }
    ...
    function liquidate(
        ...
        // burn eUSD from keeper
        eUSD.burnFrom(msg.sender, debtAmount);
        ...
        // swap collateral token in eUSD
        address swapper = addressProvider.getSwapper();
        IERC20(_token).approve(swapper, collateralAmountIn);
        uint256 eUSDAmountOut = ISwapper(swapper).swap(
            _token,
            address(eUSD),
            collateralAmountIn,
            address(this)
        ...
        require(eUSDAmountOut > debtAmount, "zero penalty");

        // liquidation penalty
        uint256 liquidationPenalty = eUSDAmountOut - debtAmount;

        // debtAmount + 50% of penalty => keeper
        // eUSD uses OZ ERC20 implementation, transfer function reverts on failure
        eUSD.transfer(msg.sender, debtAmount + liquidationPenalty / 2);
    ...
    function positionView(
        ...
        uint256 debtAmount = position.debtPrincipal;
        ...
        PositionView({
            ...
            debtInterest: 0, // No interest calculation
    ...
    function liquidatable(
        ...
        uint256 debtAmount = userPositions[_user][_token].debtPrincipal;
    ...
    function collateralTotalAmount(
        address _token
    ) public view returns (uint256) {
        return IERC20(_token).balanceOf(address(this));
    }

    function amountFromShares(
        address _token,
        uint256 _shares
    ) public view returns (uint256) {
    ...
    // Set a token as supported or not for eUSD borrowing
    function setSupportedEUSDBorrowToken(address token, bool isSupported) onlyOwner external {
        eUSDBorrowSupportedTokens[token] = isSupported;
    }

    // Set a token as supported or not for stable coin borrowing
    function setSupportedStablesBorrowToken(address token, bool isSupported) onlyOwner external {
        stablesBorrowSupportedTokens[token] = isSupported;
    }
}
