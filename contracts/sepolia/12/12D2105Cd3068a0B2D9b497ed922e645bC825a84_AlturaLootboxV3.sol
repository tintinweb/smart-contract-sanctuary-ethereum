...
contract AlturaLootboxV3 is UUPSUpgradeable, ERC1155Upgradeable, AccessControlEnumerableUpgradeable {
    ...
    event BoxLaunched(uint256 boxId, uint256 initialSupply);
    event BoxSupplyIncreased(uint256 boxId, uint256 supplyIncrease);
    event BoxPriceChanged(uint256 boxId, uint256 newPrice);
    ...
    function launchBox(uint256 boxId, uint256 initialSupply, address paymentToken, uint256 price) external {
        ...
        emit BoxLaunched(boxId, initialSupply);
    }
    ...
    function useKey(uint256 boxId) external {
        ...
        uint256 pseudoRandomIndex = block.prevrandao % targetBox.totalRarity;
        uint256 maxLength = targetBox.items.length;
        require(maxLength > 0, "No items in box");

        uint256 i;
        for (i; i < maxLength; ) {
            Item memory item = targetBox.items[i];

            if (pseudoRandomIndex < item.rarity && item.supply > 0 && item.rarity > 0) {
