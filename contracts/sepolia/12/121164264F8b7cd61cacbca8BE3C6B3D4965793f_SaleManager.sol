...
contract SaleManager is Modifiers {
  ...
  function mintToken(MintParams calldata params) public payable seedCheck(params.seeds.length) validateMint(params) {
    ...
    LibERC721._safeMint(LibUtils.msgSender(), params.quantity);

    unchecked {
      saleStorage.sales[params.saleId].supply += params.quantity;
      saleStorage.addressSaleMints[LibUtils.msgSender()][params.saleId] += params.quantity;
    }

    emit Minted(params.saleId, LibUtils.msgSender(), params.quantity);
  }
