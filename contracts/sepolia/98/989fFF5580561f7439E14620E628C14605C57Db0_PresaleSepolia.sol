...
// File: contracts/presaleSepolia.sol
...
contract PresaleSepolia is ReentrancyGuard, Ownable, Pausable {
    ...
    function claimTokens(uint256 amount, bytes32[] calldata merkleProof) external nonReentrant {
        require(canClaimTokens, "Tokens are not yet claimable");
        
        // Check if the user has any purchased tokens that they haven't claimed yet
        if (tokensPurchased[msg.sender] > 0) {
            uint256 purchasedAmount = tokensPurchased[msg.sender];
            tokensPurchased[msg.sender] = 0;
            token.safeTransfer(msg.sender, purchasedAmount);
            return;
        }
        
        // If the user doesn't have any purchased tokens, check the Merkle proof
    ...
    function recordPurchases(address[] memory buyers, uint256[] memory amounts) public onlyOwner {
        ...
        for (uint i = 0; i < buyers.length; i++) {
            require(tokensPurchased[buyers[i]] == 0, "Buyer already recorded");
