...
contract Vendor is Ownable {
    ...
    event buyTokensEvent(address buyer, uint256 amountOfETH, uint256 amountOfTokens);
    ...
    // function in Vendor.sol should use msg.value and tokensPerEth to calculate an amount of tokens to yourToken.transfer() to msg.sender.
    //    1 eth  = 0.5
    // 1 eth / 200

    // 📟Emit event BuyTokens(address buyer, uint256 amountOfETH, uint256 amountOfTokens) when tokens are purchased.

    // Edit deploy / 01 _deploy_vendor.js to deploy the Vendor(uncomment Vendor deploy lines).
    function buyTokens() public payable {
        require(msg.value > 0, "amount must be greater than 0!");
        uint256 numbTokens = msg.value * tokensPerEth;
        yourToken.transfer(msg.sender, numbTokens);
        emit buyTokensEvent(msg.sender, msg.value, numbTokens);
    }
...
function sellTokens(uint256 _amount) public  {
    ...
    uint256 tokensToTransfer =_amount / tokensPerEth;
