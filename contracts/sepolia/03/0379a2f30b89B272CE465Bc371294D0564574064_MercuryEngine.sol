...
import {EngineStorage} from "./storage/EngineStorage.sol";
import {IAssetDeployer} from "./interfaces/IAssetDeployer.sol";
...
import {FixinAccessControlUpgradeable} from "../../lib/fixins/FixinAccessControlUpgradeable.sol";

contract MercuryEngine is Initializable, UUPSUpgradeable, EngineStorage, FixinAccessControlUpgradeable, IUpgradeable {
    ...
    function deleteNamespace(string memory name) external onlyRole(GOVERNOR_ROLE) {
        bytes memory byteName = bytes(name);
        bytes32 nameHash = keccak256(byteName);
        require(
            _namespaceByName[nameHash] != address(0),
            "NAMESPACE_ALREADY_EXISTS"
        );
        delete _namespaceByName[nameHash];
    }

    function getNameSpaceAddr(string memory name) external view returns (address) {
        bytes memory byteName = bytes(name);
        bytes32 nameHash = keccak256(byteName);
        return _namespaceByName[nameHash];
    }

    function createNamespace(Types.CreateNamespaceParams memory params) external returns (
            address entityProxy,
            address assetBeacon
        ) {
        ...
        require(
            byteName.length <= Constant._MAX_NAME_LENGTH &&
                byteName.length > 0,
            "NAME_INVALID_LENGTH"
        );
        require(
            byteSymbol.length <= Constant._MAX_SYMBOL_LENGTH &&
                byteSymbol.length > 0,
            "SYMBOL_INVALID_LENGTH"
        );
        {
            address assetImpl = IAssetDeployer(params.addrs.assetFactory).deploy(salt);
            assetBeacon = address(
                new UpgradeableBeacon{ salt: salt }(assetImpl, address(this))
            );
            
            address entityImpl = IEntityDeployer(params.addrs.entityFactory).deployEntity(salt, assetBeacon, address(0));
            Types.CreateEntityParams memory nftParams = Types.CreateEntityParams(
                params.name,
                params.symbol,
                params.metadataURI,
                params.contractURI,
            ...
            bytes memory data = abi.encodeWithSelector(
                EntityNFT.initialize.selector,
                nftParams
            );
            entityProxy = address(
                new ERC1967Proxy{ salt: salt }(entityImpl, data)
            );
            _namespaceInfo[entityProxy].name = params.name;
            _namespaceByName[salt] = entityProxy;
        }
    }

    function mercuryNamespace_() private view returns (address namespace) {
        bytes memory byteName = bytes("MERCURY");
        bytes32 nameHash = keccak256(byteName);  
        namespace = _namespaceByName[nameHash];  
    }

    function createEntity(
        Types.EntityRegisterParams memory params,        
        bytes calldata preData,
        bytes calldata postData) external returns(uint256 id) {
        address namespace = mercuryNamespace_();   
        id = EntityNFT(namespace).registerEntity(namespace, params, preData, postData);                  
    }

    function registerAsset(Types.RegisterNFTParams memory params, Types.AssetMiddlewareEnum mwType, bytes calldata mwData) external {
        address namespace = mercuryNamespace_(); 
        require(EntityNFT(namespace).balanceOf(msg.sender) > 0 , "MUST_HAVE_ENTITY_NFT");
        require(EntityNFT(namespace).ownerOf(params.entityId) == msg.sender, "MUST_OWN_THE_ENTITY");   
        if (mwType != Types.AssetMiddlewareEnum.NONE) {
           params.nftMw = assetMwMapping[mwType];  
        }
        EntityNFT(namespace).registerAsset(params, mwData);
    }

    function collectAsset(Types.CollectNFTParams memory params, bytes calldata data) external payable {
        address namespace = mercuryNamespace_(); 
        EntityNFT(namespace).collectAsset{value: msg.value}(namespace, params, data);
    }

    function upgradeEntityBeacon(address newImpl, address namespace) external  onlyRole(GOVERNOR_ROLE) {
        bytes memory byteName = bytes(_namespaceInfo[namespace].name);
        require(byteName.length > 0, "INVALID_NAMESPACE");
        EntityNFT(namespace).upgradeTo(newImpl);
    }

    function allowAssetMiddleware(Types.AssetMiddlewareEnum mwType, address mw) external onlyRole(GOVERNOR_ROLE) {
        require(mw != address(0), "NULL_ADDRESS_DETECTED");
        assetMwMapping[mwType] = mw;
        emit MiddlewareAdded(mwType, mw);
    }

    function updateMiddleWare(Types.AssetMiddlewareEnum mwType, uint256 entityId, uint256 assetId) external onlyRole(GOVERNOR_ROLE) {
        address namespace = mercuryNamespace_(); 
        EntityNFT(namespace).updateMiddleWare(entityId, assetId, assetMwMapping[mwType]); 
        emit MiddlewareUpdated(mwType, entityId, assetId);  
    }

    function upgradeAssetBeacon(address newImpl, address namespace) external  onlyRole(GOVERNOR_ROLE) {
        bytes memory byteName = bytes(_namespaceInfo[namespace].name);
        require(byteName.length > 0, "INVALID_NAMESPACE");

        address assetBeacon = EntityNFT(namespace).AssetBeacon();
        UpgradeableBeacon(assetBeacon).upgradeTo(newImpl);
    }

    function upgradeCommodityBeacon(address newImpl, address namespace) external  onlyRole(GOVERNOR_ROLE) {
        bytes memory byteName = bytes(_namespaceInfo[namespace].name);
        require(byteName.length > 0, "INVALID_NAMESPACE");

        address assetBeacon = EntityNFT(namespace).CommodityBeacon();
        UpgradeableBeacon(assetBeacon).upgradeTo(newImpl);
    }
...
contract EntityNFT is ERC721ABase, EntityNFTStorage, UUPSUpgradeable, IEntity, IUpgradeable {
    ...
    function beaconByType(Types.AssetType _assetType) private returns(address) {
        if (_assetType == Types.AssetType.ERC721) {
            return AssetBeacon;
        } else if (_assetType == Types.AssetType.ERC1155) {
            return CommodityBeacon;
        }
        revert("NOT_SUPPORTED_ASSET_TYPE");
    }
}
...
abstract contract EngineStorage {
    uint256 constant VERSION = 18;
    mapping(bytes32 => address) internal _namespaceByName;
    mapping(address => Types.NamespaceInfo) internal _namespaceInfo;
    mapping(Types.AssetMiddlewareEnum => address) assetMwMapping;
    event MiddlewareAdded(Types.AssetMiddlewareEnum indexed mwType, address indexed mw);
    event MiddlewareUpdated(Types.AssetMiddlewareEnum indexed mwType, uint256 indexed entityId, uint256 indexed assetId);
}
