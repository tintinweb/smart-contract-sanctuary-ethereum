...
contract StableLP is Ownable, ReentrancyGuard {
    ...
    IMALP public MALP;
    address public USDT = 0x5ae1628d5519d422ea8538e7cA4f938BEd607522;
    address public USDC = 0xc9dD2C87D3eCf7F195494C862ff496667B9a8c61;
    address public DAI = 0x300e68558D97D7457121169bFb26B7bdcedDB4dC;
    ...
    function _createMALP(address _input, uint256 _amount)
        ...
        minSwap(_amount)
    {
        uint8 decimal = ERC20(_input).decimals();
        uint256 amount = _amount * 10**(18 - decimal);
        uint256 fee = (amount * feeMalp) / 10**18;
        uint256 MALPAmount = amount - fee; // it will be calculated by current liquidity ratios
    ...
    function _redeemMALP(address _output, uint256 _amount) internal 
    {
        ...
        uint8 decimal = ERC20(_output).decimals();
        uint256 outputAmount = (_amount - fee) / 10**(18 - decimal); // it will be calculated by current liquidity ratios
    ...
    function _swap(
        ...
        uint8 inputDecimal = ERC20(_input).decimals();
        uint8 outputDecimal = ERC20(_output).decimals();

        uint256 amount = _amount * 10**(18 - inputDecimal);
        ...
        require(
            IERC20(_output).balanceOf(address(this)) >= amount / 10**(18 - outputDecimal),
            "Not enough liquidity balance"
        );

        uint256 fee = calculateFees(_input, _output, amount);
        ...
        SafeERC20.safeTransfer(IERC20(_output), msg.sender, outputAmount / 10**(18 - outputDecimal));

        emit Swap(msg.sender, _input, _output, _amount, outputAmount / 10**(18 - outputDecimal));
    }
    ...
    function calculateFees(
        ...
        uint8 inputDecimal = ERC20(_input).decimals();
        uint8 outputDecimal = ERC20(_output).decimals();
        uint256 inputLiquidity = IERC20(_input).balanceOf(address(this)) * 10**(18 - inputDecimal);
        uint256 outputLiquidity = IERC20(_output).balanceOf(address(this)) * 10**(18 - outputDecimal);
    ...
    function totalBalance() public view returns (uint256) {
        uint256 usdt = IERC20(USDT).balanceOf(address(this)) * 10**12;
        uint256 usdc = IERC20(USDC).balanceOf(address(this)) * 10**12;
    ...
    function distributeProtocolOwnership() public onlyOwner {
        ...
        uint256 usdtBal = IERC20(USDT).balanceOf(address(this)) * 10**12;
        uint256 usdcBal = IERC20(USDC).balanceOf(address(this)) * 10**12;
    ...
    function payoutProtocolOwnership(
        ...
        SafeERC20.safeTransfer(IERC20(USDT), stakingRewardsAddress, shareUsdt / 10**12);
        uint256 shareUsdc = (_usdcDist * _stakePortion) / 10**18;
        SafeERC20.safeTransfer(IERC20(USDC), stakingRewardsAddress, shareUsdc / 10**12);
        ...
        SafeERC20.safeTransfer(IERC20(USDT), burnShareAddress, shareUsdt / 10**12);
        shareUsdc = (_usdcDist * _burnPortion) / 10**18;
        SafeERC20.safeTransfer(IERC20(USDC), burnShareAddress, shareUsdc / 10**12);
        ...
        SafeERC20.safeTransfer(IERC20(USDT), daoAddress, shareUsdt / 10**12);
        shareUsdc = (_usdcDist * _daoPortion) / 10**18;
        SafeERC20.safeTransfer(IERC20(USDC), daoAddress, shareUsdc / 10**12);
