...
import {NTConfig} from "./NTConfig.sol";

contract NTChampionChips is
    Initializable,
    UUPSUpgradeable,
    ERC2981Upgradeable,
...
{
    string baseURI;
    address public v1ChampionChipContract;
    NTConfig config;
    ...
    function initialize(
        ...
        __ERC721_init("Neo Tokyo Champion Chips V2", "NTCC");
        __ERC2981_init();
    ...
    function setOldContract(address _oldContract) public onlyOwner {
        v1ChampionChipContract = _oldContract;
    }
    ...
    function migrateAsset(address sender, uint256 tokenId) public nonReentrant {
        require(_msgSender() == config.migrator());

        NTChampionChips oldContract = NTChampionChips(v1ChampionChipContract);

        require(
            oldContract.ownerOf(tokenId) == sender,
            "You do not own that champion chip"
        );

        oldContract.transferFrom(sender, address(this), tokenId);
