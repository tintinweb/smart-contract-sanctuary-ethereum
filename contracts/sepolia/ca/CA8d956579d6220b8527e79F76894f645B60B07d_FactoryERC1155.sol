...
contract FactoryERC1155 is Ownable {
    address[] public idToContract; //an array that contains different ERC1155 tokens deployed
    uint public CollectionCreationFee = 0;
    ...
    function deployERC1155(
        ...
        require(msg.value >= CollectionCreationFee, "Insufficient fee");
        ERC1155Tempelate t = new ERC1155Tempelate(_contractName, _uri);
        idToContract.push(address(t));
        indexToOwner[idToContract.length - 1] = tx.origin;
        emit ERC1155Created(msg.sender, address(t));
        return address(t);
    }

    function getAllContractsCreated() public view returns (address[] memory) {
        return idToContract;
    }

    function setCollectionCreationFee(uint _newFee) external onlyOwner {
        CollectionCreationFee = _newFee;
    }
