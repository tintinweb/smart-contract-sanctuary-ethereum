...
import {BridgeStorage} from "contracts/amb/BridgeStorage.sol";

contract BridgeAccess is BridgeStorage, AccessControlUpgradeable {
    ...
    modifier onlyAdmin() {
        require(
            ...
            "BridgeRouter: only admin can call this function"
        );
        _;
    }
    ...
    modifier onlyTimelock() {
        require(
            ...
            "BridgeRouter: only timelock can call this function"
        );
        _;
    }
    ...
    modifier onlyGuardian() {
        require(
            ...
            "BridgeRouter: only guardian can call this function"
        );
        _;
    }
...
import {
    ...
    IBridgeHandler,
    IBridgeRouter
} from "contracts/amb/interfaces/IBridge.sol";
import {TargetAMB} from "contracts/amb/TargetAMB.sol";
import {SourceAMB} from "contracts/amb/SourceAMB.sol";
import {BridgeAccess} from "contracts/amb/BridgeAccess.sol";

/// @title Bridge Router
/// @author Succinct Labs
/// @notice Send and receive arbitrary messages from other chains.
contract BridgeRouter is SourceAMB, TargetAMB, BridgeAccess, UUPSUpgradeable {
...
import {ILightClient} from "contracts/lightclient/interfaces/ILightClient.sol";
import {MessageStatus} from "contracts/amb/interfaces/IBridge.sol";
...
import {Bytes32} from "contracts/libraries/Typecast.sol";
import {MessageEncoding} from "contracts/libraries/MessageEncoding.sol";
import {IBridgeRouter, Message} from "contracts/amb/interfaces/IBridge.sol";
import {BridgeAccess} from "contracts/amb/BridgeAccess.sol";
import {BridgeStorage} from "contracts/amb/BridgeStorage.sol";
...
contract SourceAMB is BridgeStorage, IBridgeRouter {
    ...
    ///         contract storage. Use `send` when interacting with Bridge to save gas.
    /// @param destinationChainId The chain id that specifies the destination chain.
    /// @param destinationAddress The contract address that will be called on the destination chain.
    /// @param data The data passed to the contract on the other chain
    /// @return bytes32 A unique identifier for a message.
...
import {StorageProof, EventProof} from "contracts/libraries/StateProofHelper.sol";
import {Address} from "contracts/libraries/Typecast.sol";
import {MessageEncoding} from "contracts/libraries/MessageEncoding.sol";

import {BridgeStorage} from "contracts/amb/BridgeStorage.sol";
import {IBridgeHandler, IBridgeReceiver, Message, MessageStatus} from "contracts/amb/interfaces/IBridge.sol";

/// @title Target Arbitrary Message Bridge
/// @author Succinct Labs
/// @notice Executes messages sent from the source chain on the destination chain.
contract TargetAMB is BridgeStorage, ReentrancyGuardUpgradeable, IBridgeReceiver {
    event Pass(uint256 pass);
    ...
    function executeMessageFromLog(
        ...
        if (!isDebugMode) {
            ...
            {
                (uint64 srcSlot, uint64 txSlot) = abi.decode(
                    srcSlotTxSlotPack,
                    (uint64, uint64)
                );
                requireLightClientDelay(srcSlot, message.sourceChainId);
                bytes32 headerRoot = lightClients[message.sourceChainId]
                    .headers(srcSlot);
...

            {
                ...
                require(
                    receiptMessageRoot == messageRoot,
        ...
        internal
    {
        ...
        bytes memory data;
        {
            bytes memory receiveCall = abi.encodeWithSelector(
                IBridgeHandler.handleBridge.selector,
                message.sourceChainId,
        ...
        if (data.length == 32) {
            ...
            implementsHandler = magic == IBridgeHandler.handleBridge.selector;
        }
...
import {Message} from "contracts/amb/interfaces/IBridge.sol";
