...
contract CHARACTER_SHOP is ReentrancyGuard, Pausable, Ownable {
    ...
    function createSmallSale(
        ...
        if (_saleType == SaleType.ZeroPriceSale && _price > 0) revert("Price should be zero on Zero price sale.");
        if (_saleType == SaleType.BuyNow && _price == 0) revert("Price should be larger than zero on Buy now sale.");
    ...
    function _setupAuction(
        ...
        nftContractAuctions[_nftContractAddress][_tokenId]
            .bidIncreaseAmount = _getBidIncreaseAmount(_nftContractAddress, _tokenId);
