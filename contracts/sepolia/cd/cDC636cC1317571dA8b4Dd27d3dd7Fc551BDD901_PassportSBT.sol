    ...
    TokenOperator
{
    ...
    bytes32 public constant MINT_TYPEHASH =
        keccak256(
            "Mint(address target,uint256 tokenId,string uri,bytes onchainMetadata,uint256 nonce,uint256 deadline)"
        );

    // mapping from nonce to bool
    mapping(uint256 => bool) public nonces;

    // mapping from token id to bool
    mapping(uint256 => bool) public isRevoked;

    // mapping from token id to bytes data
    mapping(uint256 => bytes) public onChainMetadata;

    // true value disables NFT movement
    bool public sbtMode;
    ...
    function initialize(
        ...
        __PassportSBT_init(name_, symbol_, operator_, sbtMode_);
    }

    function __PassportSBT_init(
        string memory name_,
        string memory symbol_,
        address operator_,
        bool sbtMode_
    ...
    function __PassportSBT_init_unchained(
        string memory,
        string memory,
        address,
        bool
    ...
    function mint(
        ...
        uint256 tokenId_,
        string memory uri_,
        bytes memory onchainMetadata_,
        uint256 nonce_,
        uint256 deadline_,
        bytes memory signature_
    ) external virtual {
        require(!nonces[nonce_], "PassportSBT: nonce already used");
        require(!isRevoked[tokenId_], "PassportSBT: token revoked");
        require(
            block.timestamp <= deadline_,
            "PassportSBT: expired deadline"
        );
        ...
        bytes32 structHash = keccak256(
            abi.encode(
                ...
                keccak256(bytes(onchainMetadata_)),
                nonce_,
                deadline_
            )
        );
        ...
        emit Mint({
            ...
            tokenId: tokenId_,
            uri: uri_,
            onchainMetadata: onchainMetadata_,
            nonce: nonce_,
            deadline: deadline_,
    ...
    function setURI(
        uint256 tokenId_,
        string memory uri_
    ) external virtual onlyOwnerOrOperator {
        _setTokenURI(tokenId_, uri_);
    ...
    function setOnchainMetadata(uint256 tokenId_, bytes memory metadata_) external virtual onlyOwnerOrOperator {
        onChainMetadata[tokenId_] = metadata_;

        emit SetOnchainMetadata({
            sender: _msgSender(),
            tokenId: tokenId_,
            metadata: metadata_
        });
    ...
    function forceTransfer(address from_, address to_, uint256 tokenId_) external virtual onlyOwnerOrOperator {
    ...
    function revokeMintOrBurn(uint256 tokenId_) external virtual onlyOwnerOrOperator {
        if (_exists(tokenId_)) {
            burn(tokenId_);
        } else {
            revokeMint(tokenId_);
    ...
    function revokeMint(uint256 tokenId_) public virtual onlyOwnerOrOperator {
        require(!_exists(tokenId_), "PassportSBT: token already minted");
    ...
    function burn(
        ...
        require(_exists(tokenId_), "PassportSBT: token not minted");
        ...
        override(ERC721Upgradeable, ERC721EnumerableUpgradeable)
    {
        ...
        if (sbtMode && _msgSender() != operator() && from_ != address(0)) {
            revert("PassportSBT: token cannot be transferred");
        }
...
abstract contract TokenOperator is
    OwnableUpgradeable,
    ITokenOperator
{
    ...
    modifier onlyOperator() {
        require(
            _msgSender() == operator(),
            "TokenOperator: caller is not the operator"
        );
        _;
    }
    ...
    modifier onlyOwnerOrOperator() {
        require(
            _msgSender() == operator() ||
            _msgSender() == owner(),
            "TokenOperator: caller is not the owner/operator"
        );
        _;
    }
