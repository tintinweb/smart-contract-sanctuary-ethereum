...
contract Token {
    ...
    constructor(
        ...
        swapAmount = totalSupply / 1000;
        maxWallet = (totalSupply * maxWallet_) / 100;
    ...
    function _transfer(
        ...
        if (from != deployer) require(trading);

        if (to != uniswapPair && to != deployer)
        ...
        if (from != address(this) && to != deployer) {
            uint256 taxAmount = (amount *
                (from == uniswapPair ? buyTax : sellTax)) / 100;
            amount -= taxAmount;
