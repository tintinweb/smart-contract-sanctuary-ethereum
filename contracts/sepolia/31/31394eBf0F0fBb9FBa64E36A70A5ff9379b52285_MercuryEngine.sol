...
contract MercuryEngine is Initializable, UUPSUpgradeable, EngineStorage, FixinAccessControlUpgradeable, IUpgradeable {
    ...
    function createNamespace(Types.CreateNamespaceParams memory params) external returns (
        ...
        );
        {
            ...
            address commodityImpl = IAssetDeployer(params.addrs.commodityFactory).deploy(salt);
            commodityBeacon = address(
                new UpgradeableBeacon{ salt: salt }(commodityImpl, address(this))
            );
            
            address entityImpl = IEntityDeployer(params.addrs.entityFactory).deployEntity(salt, assetBeacon, commodityBeacon);
    ...
    function updateNamespace(string memory namespace, address newNamespace) external onlyRole(GOVERNOR_ROLE) {
        bytes memory byteName = bytes(namespace);
        bytes32 nameHash = keccak256(byteName);
        _namespaceByName[nameHash] = newNamespace;
        _namespaceInfo[newNamespace].name = namespace;
    }
    ...
    function registerAsset(Types.RegisterNFTParams memory params, Types.AssetMiddlewareEnum mwType, bytes memory mwData) external {
        address namespace = mercuryNamespace_(); 
