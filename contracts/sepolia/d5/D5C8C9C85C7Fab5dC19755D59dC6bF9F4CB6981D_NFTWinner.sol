...
contract NFTWinner is ERC721A, ERC721ABurnable, ERC721AQueryable, Ownable {
  ...
  event MintReward(Winner[] winners);
...
//   }
  ...
  function mintReward(Winner[] memory winners) public onlyOwner {

    for(uint256 i = 0; i < winners.length; i++) {
        for (uint256 j = nextTokenId(); j < (nextTokenId() + winners[i].quantity); j++) {
            atributes1[j] = winners[i].atributes1;
            atributes2[j] = winners[i].atributes2;
        }
      _mint(winners[i].winnerAddress, winners[i].quantity);
    }
    emit MintReward(winners);
  }
