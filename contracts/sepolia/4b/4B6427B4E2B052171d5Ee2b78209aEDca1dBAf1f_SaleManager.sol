...
contract SaleManager is Modifiers {
  event Minted(uint256 indexed saleId, address minter, uint256 quantity);

  modifier seedCheck(uint256 seeds) {
    require(seeds >= LibTraitManager.getLayerStorage().layers.length, "SaleManager: Not enough seeds");
    _;
  }
  ...
  function mintToken(MintParams calldata params) public payable seedCheck(params.seeds.length) {
    ...
    require(currentSale.maxSupply > 0, "SaleManager: Sale doesnt exist");
...
library LibTraitManager {
  ...
  function selectRandomTrait(
    ...
    assembly {
      ...
      } lt(randomValue, sload(traitsPtr)) {

      } {
        randomValue := sub(randomValue, sload(traitsPtr))
