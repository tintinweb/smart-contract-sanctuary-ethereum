...
contract FunctionsConsumer is FunctionsClient, ConfirmedOwner, ERC20 {
  ...
  function fulfillRequest(bytes32 requestId, bytes memory response, bytes memory err) internal override {
    ...
    (uint256 amount, string memory receiverString) = abi.decode(response, (uint256, string));

    address receiverAddress = address(bytes20(bytes(receiverString)));

    super._mint(receiverAddress, amount);
