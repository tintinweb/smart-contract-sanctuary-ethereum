...
contract Bubble is WildNFT {
    // resilience of each tokenId
    // out of 1000
    uint8[100] public resilience = [
        // 100 random numbers under 100 
    ...
    string description = 'A collection of 100 Bubbles from Santiago.';
    ...
    function _beforeTokenTransfer(address from, address to, uint256 tokenId, uint256 batchSize) internal override(ERC721) {
        ...
        if (from != address(0)) {
            if (lastTransfer[tokenId] == 0) {
                ...
                if (diffHours - 24 - 216 * (1 - resilience[tokenId] / 100) < 0) {
                    isDead[tokenId] = true;
                    emit MetadataUpdate(tokenId);
                }
            }
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        ...
        if (bytes(baseURI).length > 0) {
            ...
            attributesStr = string(abi.encodePacked(',"attributes":[{"trait_type": "Alive", "value":"', phase, '"}, {"trait_type": "Resilience", "value":"', Strings.toString(resilience[tokenId]), '"}, {"trait_type": "Last Transfer Date Time (UTC)", "value":"', lastTransfer[tokenId], '"} ]'));
