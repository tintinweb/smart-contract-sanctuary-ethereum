    ...
    IERCSingerManage
{
    ...
    function singerValid(address singer, MulTx.TxType txType) private  view returns(bool ret){
        if(txType==MulTx.TxType.AddSinger){
            if(singers[msg.sender].singers.length>=10){
                revert("singer is too big");
            }
            // judge singers[msg.sender] is not contain singer
            for(uint i=0;i<singers[msg.sender].singers.length;i++){
                if(singers[msg.sender].singers[i]==singer){
                    revert("singer is exist");
                }
            }
        }
        if(txType==MulTx.TxType.RemoveSinger){
            if(singers[msg.sender].singers.length<=1){
                revert("singer is too small");
            }
            bool isContain =false;
            // judge singers[msg.sender] is  no contain singer
            for(uint i=0;i<singers[msg.sender].singers.length;i++){
                if(singers[msg.sender].singers[i]==singer){
                   isContain= true;
                }
    ...
    function createMulSingerTx(
        ...
        require(singerValid(to, txType), "singer is invalid");
    ...
    function submitMulSinger(
        uint txHash,
        IERCMulSinger.SingerOp op
    ) external override {
        MulSingers.Singer storage singer = mulSingers[txHash];
        ...
        if (op == IERCMulSinger.SingerOp.Approve) {
            singer.setSingerResult(
                msg.sender,
                MulSingers.SingerStatus.Approved
            );
        } else {
            singer.setSingerResult(
                msg.sender,
                MulSingers.SingerStatus.Rejected
            );
        }
        ...
        if (singer.isSingerFinished()) {
            ...
            if (singer.getSingerStatus() == MulSingers.SingerStatus.Approved) {
                ...
                if (mtx.txType == MulTx.TxType.AddSinger) {
                ...
                } else if (mtx.txType == MulTx.TxType.RemoveSinger) {
                    if (!singer.hasSinger(mtx.to)) {
                        revert("singer is  no exist");
                    }
    ...
    function completeExpireTx(address sender)private{
        uint[] storage txHashs = singerSendTxHash[sender];
        uint length = txHashs.length > 10 ? 10 : txHashs.length;
        for (uint i = 0; i < length; i++) {
            MulSingers.Singer storage singer = mulSingers[txHashs[i]];
            if (
                singer.status == MulSingers.SingerStatus.Created ||
                singer.status == MulSingers.SingerStatus.Pending
            ) {
                if (singer.getSingerExpire() < block.timestamp) {
                    singer.overSinger();
                }
            }
        }
    ...
    function getSenderTxHash() external view returns (uint[] memory) {
        //返回未签名状态未完成的交易
        uint[] storage txHashs = singerSendTxHash[msg.sender];
    ...
    function getSenderTxCompleteHash() external view returns (uint[] memory) {
        //返回签名状态完成的交易
        uint[] storage txHashs = singerSendTxHash[msg.sender];
        ...
        for (uint i = 1; i < length + 1; i++) {
            MulSingers.Singer storage singer = mulSingers[
                txHashs[txHashs.length - i]
            ];
            if (
                singer.status == MulSingers.SingerStatus.Approved ||
                singer.status == MulSingers.SingerStatus.Rejected ||
                singer.status == MulSingers.SingerStatus.Executed
            ) {
...
interface  IERCMulSinger {
    ...
    event CreateMulSinger(address sender, uint  txHash, uint txId);

    event ForceOverMulSinger(address sender,uint  txHash, uint txId);

    event ApproveMulSinger(address sender,uint  txHash,uint  txId);

    event StartMulSinger(address sender,uint  txHash,uint  txId);

    event RejectMulSinger(uint  txHash,uint  txId);

    event ExecuteMulSinger(uint  txHash,uint  txId);

       enum SingerOp {Approve,Reject}
   //writer a createMulSinger function  param with singers address arrary and txParams withb MulTx.Mtx struct的filed to create MulTx.Mtx and  MulSinger
    function createMulSingerTx(uint amount,uint when, address to, string memory blank, MulTx.TxType txType,uint txId) external payable;

     //force to over mul singer by TxHash
   function forceOverMulSinger(uint txHash) external;

   // singer to approve mul singer by TxHash
   function submitMulSinger(uint txHash,IERCMulSinger.SingerOp op) external;
...
//Get the number of people who have signed
    ...
    function overSinger(Singer storage self) internal {
          if(self.status==SingerStatus.Executed || self.status==SingerStatus.Rejected|| self.status==SingerStatus.Approved){
...
library MulTx {
     ...
     * 2.数字货币提现交易， blank字段暂时没用，amount为提现金额，to 提取账户地址，from为发起地址 blank字段为银行卡号
     ...
     * 6. 业务配置交易， blank字段没用，amount为业务配置的value，to 暂时没用，from为发起地址
     * 7. 业务配置删除交易， blank字段没用，amount为业务配置的value，to 暂时没用，from为发起地址
   ...
   function initialize(address from,address to,TxType txType,uint index,uint txId,uint amount,uint when,string memory blank) internal  view returns ( Mtx memory){
         if((txType==TxType.FiatWithDraw || txType==TxType.CryptoWithDraw|| txType==TxType.LimitQuota||txType==TxType.BizConfigAdd||txType==TxType.BizConfigRemove) && amount==0){
             revert("amount is zero");
         }
         if((txType==TxType.CryptoWithDraw|| txType==TxType.AddSinger|| txType==TxType.RemoveSinger) && to==address(0)){
             revert("to is zero");
         }
