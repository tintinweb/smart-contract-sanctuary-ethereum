...
contract Gateway is GatewayStorage, LockerStorage, TokenStorage, ValidatorStorage, Locker {
    ...
    uint8 constant STORAGE_VERSION = 1;
    ...
    function lock(IERC20Metadata token, uint256 amount, bytes32 foreignAccount) public onlyInitialized {
        _lock(token, _msgSender(), amount, foreignAccount);
    }

    function lockEth(bytes32 foreignAccount) public payable onlyInitialized {
        _lockEth(NATIVE_TOKEN, _msgSender(), msg.value, foreignAccount);
    }
    ...
    function release(address token, uint256 amount, address account, uint64 externalId, uint64 timestamp, Signature[] calldata signatures) public onlyInitialized {
        ...
        _release(IERC20(token), account, amount, externalId);
    }
    ...
    function addNative(uint256 maxAmount, uint8 foreignDecimals, uint64 activatedTimestamp) public onlyOwner {
        _addToken(NATIVE_TOKEN, maxAmount, foreignDecimals, activatedTimestamp, FLAG_TOKEN_NATIVE);
        emit TokenAdd(NATIVE_TOKEN, _getToken(NATIVE_TOKEN));
    }

    function addToken(address token, uint256 maxAmount, uint8 foreignDecimals, uint64 activatedTimestamp, uint184 flags) public onlyOwner {
        _addToken(token, maxAmount, foreignDecimals, activatedTimestamp, flags);
...
contract GatewayStorage is Context, Ownable, Initializable, IInitializer {
...
abstract contract ILockerStorage {
    ...
    function _markReleased(uint64 externalId) internal virtual returns (bool);
}
...
abstract contract ITokenStorage {
    ...
    uint184 constant FLAG_TOKEN_MINTABLE = 1;
    uint184 constant FLAG_TOKEN_NATIVE = 2;
    uint184 constant FLAG_MASK = FLAG_TOKEN_MINTABLE | FLAG_TOKEN_NATIVE;

    function _addToken(address account, uint256 maxAmount, uint8 foreignDecimals, uint64 activatedTimestamp, uint184 flags) internal virtual;
...
abstract contract IValidatorStorage {
...
abstract contract Locker is Context, ILockerStorage, ITokenStorage {
    ...
    function _lock(IERC20Metadata token,
                    address from,
                    uint256 amount,
                    bytes32 foreignAccount) internal returns(uint256) {
        TokenDef memory tokenDef = _getAndCheckToken(address(token));
        ...
        if (tokenDef.flags & FLAG_TOKEN_MINTABLE != 0) {
            IBridgeERC20Token(address(token)).burnFrom(from, rounded);
        }
        else {
            token.safeTransferFrom(from, address(this), rounded);
        }

        emit Lock(nextId, address(token), rounded, foreignAccount);
        return nextId;
    }

    function _lockEth(address nativeToken, address from, uint256 amount, bytes32 foreignAccount) internal returns(uint256) {
        TokenDef memory tokenDef = _getAndCheckToken(address(nativeToken));
        ...
        uint256 nextId = _incAndGet();
        emit Lock(nextId, address(nativeToken), rounded, foreignAccount);
        return nextId;
    }
    ...
    function _release(IERC20 token,
        ...
        if (tokenDef.flags & FLAG_TOKEN_NATIVE != 0) {
            ...
            IBridgeERC20Token(address(token)).mint(to, amount);
        }
        else {
    ...
    function _getAndCheckToken(address token) private view returns(TokenDef memory) {
        TokenDef memory tokenDef = _getToken(address(token));
        if (tokenDef.maxAmount == 0) {
            revert TokenNotSupported(token);
        }
        if (block.timestamp < tokenDef.activatedTimestamp) {
...
abstract contract LockerStorage is ILockerStorage {
    ...
    function _markReleased(uint64 externalId) internal override returns (bool) {
        if (_released.get(externalId)) {
            return false;
        }
        _released.setTo(externalId, true);
        return true;
    }
}
...
contract TokenStorage is ITokenStorage {
    mapping(address => TokenDef) private _tokens;

    function _addToken(address account, uint256 maxAmount, uint8 foreignDecimals, uint64 activatedTimestamp, uint184 flags) internal override {
        if (maxAmount == 0) {
            revert ZeroMaxAmount();
        }
        if (activatedTimestamp <= block.timestamp) {
            revert TokenActivationInPast();
        }
        TokenDef storage def  = _tokens[account];
        if (def.maxAmount != 0) {
            revert TokenAlreadyExists(account);
        }
        def.maxAmount = maxAmount;
        def.foreignDecimals = foreignDecimals;
        def.activatedTimestamp = activatedTimestamp;
        def.flags = flags & FLAG_MASK;
    }

    function _removeToken(address account) internal override {
        if (_tokens[account].maxAmount == 0) {
...
contract ValidatorStorage is IValidatorStorage {
    using BitMaps for BitMaps.BitMap;
    ...
    function _addValidator(address account, uint16 id) internal override returns (uint256) {
        ...
        if (_validators[account] != 0) {
            revert ValidatorAlreadyExists(account);
        }
        if (_validatorIds.get(id)) {
            revert ValidatorIdOccupied(id);
        }
        ValidatorInfo memory info = _info;
