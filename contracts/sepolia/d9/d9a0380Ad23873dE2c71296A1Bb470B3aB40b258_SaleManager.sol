...
contract SaleManager is Modifiers {
  ...
  modifier validateMint(MintParams calldata params) {
    ...
    require(LibERC721.totalSupply() + params.quantity <= appStorage.maxSupply, "SaleManager: Collection supply exceeded");
  ...
  modifier seedCheck(uint256 seeds) {
    require(seeds >= LibTokenManager.getLayerStorage().layerCount, "SaleManager: Not enough seeds");
    _;
  }
  ...
  function mintOwner(address to, uint16 quantity, uint16[] calldata seeds) public seedCheck(seeds.length) onlyOwner {
    require(LibERC721.totalSupply() + quantity <= appStorage.maxSupply, "SaleManager: Collection supply exceeded");
    require(appStorage.creatorSupply + quantity <= appStorage.creatorMaxSupply, "SaleManager: Creator supply exceeded");

    generateTokens(quantity, seeds);
    LibERC721._safeMint(to, quantity);

    unchecked {
      appStorage.creatorSupply += quantity;
    }
  }
  ...
  function generateTokens(uint16 quantity, uint16[] calldata seeds) internal {
    ...
    if (quantity > 1) {
      LibTokenManager.createMultiple(nextTokenId, quantity, seeds);
    } else if (quantity == 1) {
      LibTokenManager.createToken(nextTokenId, seeds);
    }
  }
  ...
  function createTestSale() public onlyOwner {
    ...
    require(_totalSupply <= appStorage.maxSupply, "SaleManager: Max supply exceeded");
  ...
  function setSaleMaxSupply(uint16 saleId, uint16 supply) public saleExist(saleId) onlyOwner {
    ...
    require(currentActiveSupply + supply <= appStorage.maxSupply, "SaleManager: Collection supply exceeded");
  ...
  function activeSupply() public view returns (uint256 supply) {
    ...
    unchecked {
      ...
      supply += appStorage.creatorMaxSupply;
    }
  }
