...
contract Freezable is OwnableUpgradeable {
  mapping(address account => bool frozen) public isFrozen;

  error FrozenAccount();

  event Frozen(address indexed account);
  event Thawed(address indexed account);

  modifier notFrozen(address _account) {
    if (isFrozen[_account]) revert FrozenAccount();
    _;
  }
  ...
  function thaw(address _account) public onlyOwner {
