...
import {AdvancedConfig, Data, BaseConfig, MadMintStorage} from "../libraries/MadMintStorage.sol";
import {IERC721} from "@openzeppelin/contracts/token/ERC721/IERC721.sol";

contract MadMintERC721AExtensionC is MadMintERC721AUpgradeable {
    ...
    function setTokenURIs(
        ...
        require(!MadMintStorage.state().advCfg.metadataFrozen, "NOT_ACTIVE");
        uint256 tokenIdsLength = _tokenIds.length;
        require(tokenIdsLength == _newURIs.length);
        for (uint256 i = 0; i < tokenIdsLength; i++) {
            MadMintStorage.state().data.tokenURIs[_tokenIds[i]] = _newURIs[i];
        }
    }

    function baseTokenURI() external view returns (string memory) {
        return MadMintStorage.state().cfg.uriBase;
    }
    ...
    function _publicPriceInWei() internal view returns (uint256) {
        return uint256(MadMintStorage.state().cfg.publicPrice) * 10 ** 13;
    }
    ...
    function creditCardMint(
        ...
        if (state.cfg.madMintFeeActive) {
            ...
            require(
                msg.value == publicPrice * _numTokens + madmintFee,
                "INVALID_PRICE_PAID"
            );
    ...
    function setSoulbindingState(bool _soulbindingActive) external onlyOwner {
        MadMintStorage.state().cfg.soulbindingActive = _soulbindingActive;
    }
    ...
    function setStakingState(bool _stakingState) external onlyOwner {
        MadMintStorage.state().advCfg.stakingActive = _stakingState;
    }
    ...
    function adminUnstake(uint256 _tokenId) external onlyOwner {
        Data storage data = MadMintStorage.state().data;
        require(MadMintStorage.state().data.currentTimeStaked[_tokenId] != 0);
    ...
    function currentTokenStakeTime(
        ...
        uint256 currentStakeStartTime = MadMintStorage
        .state()
        .data
    ...
    function updateMintsPerFreeClaim(
        ...
        MadMintStorage.state().advCfg.mintsPerFreeClaim = _mintsPerFreeClaim;
    }
    ...
    function freeClaim(
        ...
        if (state.cfg.madMintFeeActive) {
            uint256 madmintFee = totalMints * madmintFeePerToken();
