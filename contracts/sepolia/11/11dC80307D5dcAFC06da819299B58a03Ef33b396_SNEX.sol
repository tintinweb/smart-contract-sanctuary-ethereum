...
contract SNEX is Context, ERC721, IERC4906 {
	...
	uint256 private _mintFee = 10000000000000000; //50000000000000000, 0.05 ETH
	uint256 private _mintedTokens = 0;
	uint256 private _maxCap = 5; // ToDo: change it back to 5000.
	uint256 private _presaleCap = 2; // ToDo: change it back to 1000.
...
// ==================================================================
	...
	// ToDo: change this back to internal
	function _buildArrayHexTriplet(uint256 tokenId) public view returns(string memory) {
...
// ------------------------------------------------------------------
	...
	function mint() public payable {
		...
		if(_mintedTokens > _presaleCap) { // After presale, mintFee should be updated automatically.
...
// ------------------------------------------------------------------
	...
	/// @dev Returns contract's total supply.
...
// ------------------------------------------------------------------
	...
	/// @dev Changes mint cap.
	///	 Note: Only contract's owner can change this.
	/// @param _newCap New value.
	function __debugChangeMaxCap(uint256 _newCap) onlyAdmin public {
		_maxCap = _newCap;
	}

	/// @dev Changes presale cap.
	///	 Note: Only contract's owner can change this.
	/// @param _newCap New value.
	function __debugChangePresaleCap(uint256 _newCap) onlyAdmin public {
		_presaleCap = _newCap;
	}
}