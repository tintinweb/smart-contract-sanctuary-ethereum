...
/**
 * @title Tafect Governance contract
...
contract Governed {
    ...
    // -- Events --
    event OwnerShifted(address indexed from, address indexed to);

    /**
     * @dev Check if the caller is the governor.
     */
    modifier onlyGovernor() {
        require(msg.sender == governor, "TAF: Only Governor can call");
        _;
    }
    ...
    function transferOwnership(address _newGovernor) external onlyGovernor {
        require(_newGovernor != address(0), "TAF: Governor must be set");
    ...
    function acceptOwnership() external {
        require(pendingGovernor != address(0) && msg.sender == pendingGovernor, "TAF: Caller must be pending governor");
        ...
        emit OwnerShifted(oldGovernor, governor);
    }
}
...
/**
 * @title TafectToken contract
 * @dev This is the implementation of the ERC20 Tafect Token.
 ...
 *
 */
contract TafectToken is Governed, ERC20, ERC20Burnable {
    using SafeMath for uint256;
    ...
    bytes32 private constant DOMAIN_NAME_HASH = keccak256("Tafect Token");
    bytes32 private constant DOMAIN_VERSION_HASH = keccak256("0");
    bytes32 private constant DOMAIN_SALT = 0xc44f1f3656014cd5a975191a5689a36c1d4b6fa2b5d264ed0c53817069026afa; // Randomly generated salt
    ...
    modifier onlyMinter() {
        require(isMinter(msg.sender), "TAF: Only minter can call");
        _;
    }

    /**
     * @dev Token Contract Constructor.
     * @param _initialSupply Initial supply of TAF
     */
    constructor(uint256 _initialSupply) ERC20("Tafect Token", "TAF") {
        ...
        DOMAIN_SEPARATOR = keccak256(abi.encode(DOMAIN_TYPE_HASH, DOMAIN_NAME_HASH, DOMAIN_VERSION_HASH, _getChainID(), address(this), DOMAIN_SALT));
    }
    ...
    function permit(address _owner, address _spender, uint256 _value, uint256 _deadline, uint8 _v, bytes32 _r, bytes32 _s) external {
        ...
        require(_owner == recoveredAddress, "TAF: invalid permit");
        require(_deadline == 0 || block.timestamp <= _deadline, "TAF: expired permit");
