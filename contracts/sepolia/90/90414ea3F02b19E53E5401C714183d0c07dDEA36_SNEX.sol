...
// ==================================================================
	...
	function _buildArrayColors(uint256 tokenId) public view returns(string memory) {
		string memory _retValue = "[";
		uint _length = tokenData[tokenId].tokenColors.length;

		for(uint i = 0; i < _length; i++) {
			_retValue = string(abi.encodePacked(_retValue, tokenData[tokenId].tokenColors[i]));
			if(i < _length - 1) {
