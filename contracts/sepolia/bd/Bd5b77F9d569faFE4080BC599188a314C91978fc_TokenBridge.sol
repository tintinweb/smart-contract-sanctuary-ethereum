...
contract TokenBridge is ITokenBridge, Initializable, OwnableUpgradeable, ReentrancyGuardUpgradeable, UUPSUpgradeable {
    ...
    function sendToAptos(
        ...
        require(msg.value>=fee, "TokenBridge: insufficient fee");
        ...
        // send to aptos
        bytes memory payload = _encodeSendPayload(_token, _toAddress, amountSD);
        emit Send(_token, msg.sender, _toAddress, _amountLD, payload);
    }
    ...
    function registerToken(address _token) external onlyOwner {
        ...
        require(localDecimals >= SHARED_DECIMALS, "TokenBridge: decimals must be >= SHARED_DECIMALS");
    ...
    function withdrawFee(address _token, address _to, uint _amountLD) public onlyOwner {
        uint feeLD = accruedFeeLD(_token);
        require(_amountLD <= feeLD, "TokenBridge: fee not enough");

        IERC20Upgradeable(_token).safeTransfer(_to, _amountLD);
        emit WithdrawFee(_token, _to, _amountLD);
    }

    function withdrawTVL(address _token, address _to, uint64 _amountSD) public onlyOwner emergencyWithdrawUnlocked {
        tvlSDs[_token] -= _amountSD;

        uint amountLD = _SDtoLD(_token, _amountSD);
        IERC20Upgradeable(_token).safeTransfer(_to, amountLD);
    ...
    function msgReceive(bytes memory _payload) external onlyOwner {
        (address token, address to, uint64 amountSD) = _decodeReceivePayload(_payload);
        ...
        IERC20Upgradeable(token).safeTransfer(to, amountLD);
    ...
    function accruedFeeLD(address _token) public view returns (uint) {
        ...
        return IERC20Upgradeable(_token).balanceOf(address(this)) - tvlLD;
    }

    // ---------------------- internal functions ----------------------
    function _lockTokenFrom(address _token, address _from, uint _amountLD) internal returns (uint) {
        // support token with transfer fee
        uint balanceBefore = IERC20Upgradeable(_token).balanceOf(address(this));
        IERC20Upgradeable(_token).safeTransferFrom(_from, address(this), _amountLD);
        uint balanceAfter = IERC20Upgradeable(_token).balanceOf(address(this));
    ...
    function _payFee(uint _amountLD) internal view returns (uint _amountAfterFee, uint _fee) {
        _fee = (_amountLD * bridgeFeeBP) / BP_DENOMINATOR;
        _amountAfterFee = _amountLD - _fee;
    }
    ...
    function _encodeSendPayload(
        ...
        bytes32 tokenBytes32 = addressToBytes32(_token);
        return abi.encodePacked(uint8(PacketType.SEND_TO_APTOS), tokenBytes32, _toAddress, _amountSD);
    }

    // receive payload: packet type(1) + remote token(32) + receiver(32) + amount(8)
    function _decodeReceivePayload(
        bytes memory _payload
    ) internal pure returns (address token, address to, uint64 amountSD) {
...
interface ITokenBridge {
    ...
    function sendToAptos(address _token, bytes32 _toAddress, uint _amountLD) external payable;

    event Send(address indexed token, address indexed from, bytes32 indexed to, uint amountLD, bytes payload);
