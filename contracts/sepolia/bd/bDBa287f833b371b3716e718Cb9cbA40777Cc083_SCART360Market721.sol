...
contract SCART360Market721 is ERC721, Ownable {
    ...
    constructor() ERC721("SCART360", "SCART360") {}
    ...
    function getServiceFee() public view returns(uint96) {
     ...
     function getAuctionItemsBySeller(address seller) public view returns (AuctionItem[] memory) {
    uint256 itemCount = 0;

    // Count the number of matching auction items
    for (uint256 i = 0; i < itemsForSale.length; i++) {
        if (itemsForSale[i].seller == seller && !itemsForSale[i].isSold) {
            itemCount++;
        }
    }

    // Create a new array to store the matching auction items
    AuctionItem[] memory result = new AuctionItem[](itemCount);
    uint256 index = 0;

    // Populate the result array with the matching auction items
    for (uint256 i = 0; i < itemsForSale.length; i++) {
        if (itemsForSale[i].seller == seller && !itemsForSale[i].isSold) {
            result[index] = itemsForSale[i];
            index++;
        }
    }
    ...
    function addItemToMarket(uint256 tokenId, address tokenAddress, uint256 askingPrice,string memory collection,string memory uri) OnlyItemOwner(tokenAddress, tokenId) HasTransferApproval(tokenAddress, tokenId) external returns(uint256) {
        ...
        itemsForSale.push(AuctionItem(newItemId, tokenAddress, tokenId, payable(msg.sender), askingPrice,collection, false, uri));
