...
function mintCard() external payable {
	...
	uint256 price = getMintPriceValue();
	// console.log(price);
...
function fulfillRandomWords(uint256 requestId, uint256[] memory randomWords) internal override {
    // Generate a random index based on the first random word
    ...
    availableCards[randomIndex] = availableCards[availableCards.length - 1];
    availableCards.pop();
	// userCards[requestUsers[requestId]].push(cardId);
    // Mint the card to the msg.sender
    ...
    /// @notice Returns if the tokenId availabe or not
    /// @param tokenId The token ID
    /// @return available true if card available
    function isCardAvailabe(uint256 tokenId) external view returns (bool available) {
        for(uint256 i=0 ;i < availableCards.length;i++){
			if(availableCards[i] == tokenId)  available = true;
		}
    }
