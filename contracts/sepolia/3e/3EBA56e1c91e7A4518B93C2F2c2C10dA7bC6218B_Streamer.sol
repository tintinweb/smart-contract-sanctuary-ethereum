...
contract Streamer is Ownable {
    event Opened(address opener, uint256 fund);
    event Challenged(address challenger);
    event Withdrawn(address withdrawer, uint256 payment);
    ...
    function withdrawEarnings(Voucher calldata voucher) public onlyOwner {
        ...
        */
        ...
        console.log(balances[signer], voucher.updatedBalance);

        require(
            balances[signer] > voucher.updatedBalance,
            "Updated balance is higher than the current one!"
        );
        ...
        require(success, "Transfer back to owner failed.");
    ...
    */
    function challengeChannel() public {
        require(balances[msg.sender] != 0, "You don't have any running channel!");
    ...
    */
    function defundChannel() public {
        console.log(canCloseAt[msg.sender], block.timestamp);

        require(
            canCloseAt[msg.sender] != 0,
            "You don't have any closing channel!"
        );
        require(
            canCloseAt[msg.sender] < block.timestamp,
            "Can not close the channel yet!"
        );

        (bool success, ) = msg.sender.call{value: balances[msg.sender]}("");
        require(success, "Transfer back to fundee failed.");
        balances[msg.sender] = 0;
