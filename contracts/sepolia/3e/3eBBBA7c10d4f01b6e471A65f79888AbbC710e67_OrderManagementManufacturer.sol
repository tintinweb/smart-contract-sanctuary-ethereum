...
contract Inventroy {
    ...
    function manufEachProductDetails(address _manufAddress, bytes32 _manfProductID)
        public
        view
        returns (Types.manfItem memory)
    {
...
contract OrderManagementManufacturer is Supplier {
    ...
    */
    ...
    ) public view returns (Types.SupplierWithMaterialID[] memory) {
        require(
            registration.getStakeHolderDetails(msg.sender).IsRegistered,
            "manufcaturer not registered yet!"
        );

        Types.SupplierWithMaterialID[] memory _result = createMapping(
            materialId_
        );
        return _result;
    }
    ...
    */
    function createMapping(
        //returns all the supplier who are registered themselve via a particular material ID
        bytes32 materialId_
    ) internal view returns (Types.SupplierWithMaterialID[] memory) {
    ...
    function PurchaseRawMaterials(
        ...
        require(
            isManufacturer(),
            "only Manufacturer Can purchase Materials."
        );
        ...
        supplyItems[_supplierId][_purchaseMaterial.PId] = _newMaterial;
    ...
    function markMaterialReadyPickedUp(address _producerID, bytes32 _prodId, uint256 _quantity)
        public
    {
        require(isSupplier(), "Not a supplier!.");
        Types.Item memory product_ = inventory.getAddedMaterialDetails(
            _producerID,
            _prodId
        );
    ...
    function markMaterialDelivered(
        ...
        Types.Item memory _newMaterial = supplyItems[msg.sender][_matId];
