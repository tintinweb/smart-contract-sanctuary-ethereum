...
contract YourContract is AccessControl, ReentrancyGuard {
    ...
    function allERC20CreatorsData(
        ...
        address[] memory tokenAddresses = new address[](creatorLength * tokenLength);
        uint256[] memory caps = new uint256[](creatorLength * tokenLength);
        uint256[] memory lastWithdrawals = new uint256[](creatorLength);
        ...
        for (uint256 i = 0; i < creatorLength; ++i) {
            ...
            for (uint256 j = 0; j < tokenLength; ++j) {
                ...
                caps[k] = erc20FlowingCreators[creatorAddress].caps[tokenAddress];
                k++;
            }
            lastWithdrawals[i] = erc20FlowingCreators[creatorAddress].last;
        }
   ...
   function availableCreatorAmountERC20(
    ...
    function addCreatorERC20Flow(
        ...
        } else if (erc20ActiveCreators.length == 0) {
            erc20FlowingCreators[_creator].last = block.timestamp;
            erc20ActiveCreators.push(_creator);
            erc20CreatorIndex[_creator] = erc20ActiveCreators.length - 1;
        }
...
function addBatchERC20Creators(address[] memory _creators, address[][] memory _tokenAddresses, uint256[][] memory _caps) public onlyAdmin {
    uint256 cLength = _creators.length;
    ...
    for (uint256 i = 0; i < cLength; ++i){
        ...
        addCreatorERC20Flow(payable(_creators[i]), _tokenAddresses[i], _caps[i]);
    }
}
    ...
    // add new mapping to keep track of tokens associated with each creator
//mapping(address => address[]) public creatorTokens;
...
function removeCreatorERC20Flow(address _creator) external onlyAdmin {
    ...
    address lastCreator = erc20ActiveCreators[erc20ActiveCreators.length - 1];
        ...
        isERC20FlowActive(msg.sender, _token)
    {
        ...
        erc20FlowingCreators[msg.sender].last =
