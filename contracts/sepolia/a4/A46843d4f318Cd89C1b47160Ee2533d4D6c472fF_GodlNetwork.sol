...
contract GodlNetwork is PausableUpgradeable, OwnableUpgradeable {
        ...
        whenNotPaused
    {
        ...
        if (user[_msgSender()].rewardBalance > 0) {
            paxgToken_.safeTransfer(_msgSender(), user[_msgSender()].rewardBalance);
        }
        ...
        for (uint256 i = 0; i < 5; i++) {
            user[_newWalletAddress].referralCount[i] = user[_msgSender()].referralCount[i];
            user[_newWalletAddress].referrals[i] = user[_msgSender()].referrals[i];
        }
        user[_newWalletAddress].totalRewards = user[_msgSender()].totalRewards;
        user[_newWalletAddress].rewardBalance = 0;
        user[_newWalletAddress].registrationTime = user[_msgSender()].registrationTime;
    ...
    */
    ...
    {
        require(_uniqueId > 0 && _uniqueId <= totalUsers_, "getUserAddressFromId: Given unique id is invalid.");
    ...
    */
    ...
    {
        require(isExistingUser(_userAddress), "getUserIdFromWalletAddress: User has not yet joined the GODL Network.");
