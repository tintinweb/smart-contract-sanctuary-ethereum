...
import { LibERC721, StorageLayout, ChipData } from "../libraries/LibERC721.sol";
import { LibTraitManager, TokenMetadata } from "../libraries/LibTraitManager.sol";
...
contract ERC721Facet is IERC721, DefaultOperatorFiltererUpgradeable {
  ...
  function tokenURI(uint256 tokenId) public view returns (string memory) {
    ...
    return LibERC721.getStorage().chip[tokenId].burned ? LibTraitManager.getTokenURI(uint16(tokenId)) : "Chip svg";
  }
  ...
  function executeChip(uint256 tokenId, uint16[] calldata salt) public {
    StorageLayout storage ds = LibERC721.getStorage();
    require(LibUtils.msgSender() == ownerOf(tokenId), "ERC721Facet: Unauthorized access");
    require(salt.length >= LibTraitManager.getStorage().layers.length, "ERC721Facet: Not salty enough");

    LibTraitManager.createToken(uint16(tokenId), salt);

    unchecked {
      ds.chipsBurned++;
      ds.chip[tokenId] = ChipData({ burned: true, iteration: ds.chip[tokenId].iteration + 1 });
    }
  }

  function convertToChip(uint256 tokenId) public {
    StorageLayout storage ds = LibERC721.getStorage();
    require(LibUtils.msgSender() == ownerOf(tokenId), "ERC721Facet: Unauthorized access");
    require(ds.chip[tokenId].burned, "ERC721Facet: Chip already exist");

    LibTraitManager.destroyToken(uint16(tokenId));

    unchecked {
      ds.chipsBurned--;
      ds.chip[tokenId].burned = false;
    }
  }

  function getChipState(uint256 tokenId) public view returns (ChipData memory) {
    return LibERC721.getStorage().chip[tokenId];
  }
...
library LibTraitManager {
  ...
  function createToken(uint16 tokenId, uint16[] calldata seeds) internal {
    ...
    (uint16[] memory metadata, bytes32 dna) = generateMetadata(tokenId, seeds);
  ...
  function generateMetadata(uint tokenId, uint16[] calldata seeds) internal returns (uint16[] memory, bytes32) {
