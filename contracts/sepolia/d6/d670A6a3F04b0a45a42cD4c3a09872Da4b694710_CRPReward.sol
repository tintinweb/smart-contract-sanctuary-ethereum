    ...
    PausableUpgradeable
{
    ...
    ///@dev points for each interaction (0: min, 1: send transfer, 2: receive transfer)
    uint256[3] public interacPoints;
    ...
    mapping(uint256 => RewardCondition) private rewardConditions;

    ///@dev mapping of user address => collection address => collection index.
    mapping(address => mapping(address => uint256)) public collectionIndex; // 0 is for address no longer a collection
    ...
    mapping(address => mapping(address => uint256[])) public tokenIdsPerUser; // 0 is for token ID no longer counted
    ...
    function initialize(
        ...
        setRewardCondition(_timeUnit, _rewardsPerUnitTime);

        interacPoints = _interacPoints;
    }
    ...
    function __onlyManagers() private view {
        if (!management.managers(msg.sender)) {
            revert CRPRewardNotAllowed();
        }
    }

    ///@dev only collections created by CreatorsPRO allowed
    function __onlyCreatorsPROCollections() private view {
        if (!management.collections(msg.sender)) {
            revert CRPRewardNotAllowedCollectionAddress();
        }
    }
    ...
    function increasePoints(
        ...
        if (_interaction > 2) {
            revert CRPRewardInvalidInteraction();
        }

        if (_interaction == 0 || _interaction == 2) {
            _addToken(_user, _tokenId);
        } else {
            removeToken(_user, _tokenId, false);
        }

        if (_interaction == 1) {
            users[_user].points = users[_user].points <
                interacPoints[_interaction]
                ? 0
                : users[_user].points - interacPoints[_interaction];
        } else {
            users[_user].points += interacPoints[_interaction];
        }

        emit PointsIncreased(
            _user,
            _tokenId,
            tokenInfo[msg.sender][_tokenId].hashpower,
            _interaction
        );
    }
    ...
    function removeToken(
        ...
        uint256[] storage tokenIds = tokenIdsPerUser[_user][msg.sender];
        uint256 _hashpower = tokenInfo[msg.sender][_tokenId].hashpower;

        _updateUnclaimedRewards(_user);

        uint256 index = tokenInfo[msg.sender][_tokenId].index;

        tokenIds[index] = tokenIds[tokenIds.length - 1];
        tokenInfo[msg.sender][tokenIds[index]].index = index;
        tokenIds.pop();

        if (tokenIds.length == 1 && tokenIds[0] == 0) {
            uint256 _collectionIndex = collectionIndex[_user][msg.sender];
            user.collections[_collectionIndex] = user.collections[
                user.collections.length - 1
            ];
            user.collections.pop();
            delete collectionIndex[_user][msg.sender];
        }

        if (user.collections.length == 1 && user.collections[0] == address(0)) {
            usersArray[user.index] = usersArray[usersArray.length - 1];
            usersArray.pop();
            delete user.index;
            delete user.score;
            delete user.points;
        } else {
            user.score -= _hashpower;
        }

        delete tokenInfo[msg.sender][_tokenId].index;

        if (_emitEvent) {
            emit TokenRemoved(_user, _tokenId);
        }
    }
    ...
    function claimRewards() external override(ICRPReward) {
        ...
        if (rewards == 0) {
            revert CRPRewardNoRewards();
        }

        uint256 cretorsRoyalty = (rewards * 5) / 100; // CreatorsPRO royalty = 5%

        users[msg.sender].timeOfLastUpdate = block.timestamp;
        users[msg.sender].unclaimedRewards = 0;
        users[msg.sender].conditionIdOflastUpdate = nextConditionId - 1;
        ...
        management.tokenContract(IManagement.Coin.CREATORS_TOKEN).transfer(
            msg.sender,
            rewards - cretorsRoyalty
        );

        emit RewardsClaimed(msg.sender, rewards - cretorsRoyalty);
    }
    ...
    function setHashObject(
        ...
        if (
            ...
            revert CRPRewardInputArraysNotSameLength();
        }

        for (uint256 ii = 0; ii < _tokenId.length; ++ii) {
            tokenInfo[_collection][_tokenId[ii]].hashpower = _hashPower[ii];
            tokenInfo[_collection][_tokenId[ii]]
                .characteristId = _characteristId[ii];

            uint256 _index = tokenInfo[_collection][_tokenId[ii]].index;
            if (_index != 0) {
                address _owner = IERC721ArtHandle(_collection).ownerOf(
                    _tokenId[ii]
                );

                users[_owner].score += _hashPower[ii];
            }
        }
    ...
    function setRewardCondition(
        ...
        if (_timeUnit == 0) {
            revert CRPRewardTimeUnitZero();
        }

        uint256 conditionId = nextConditionId;
        nextConditionId += 1;

        rewardConditions[conditionId] = RewardCondition({
            timeUnit: _timeUnit,
        ...
        if (conditionId > 0) {
            rewardConditions[conditionId - 1].endTimestamp = block.timestamp;
        }

        emit NewRewardCondition(msg.sender, _timeUnit, _rewardsPerUnitTime);
    }
    ...
    /// @inheritdoc ICRPReward
    function pause() external override(ICRPReward) {
        __nonReentrant();
        __onlyManagers();
    ...
    /** @dev only managers allowed to call this function. Uses _pause internal function from PausableUpgradeable. */
    /// @inheritdoc ICRPReward
    function unpause() external override(ICRPReward) {
        __nonReentrant();
        __onlyManagers();
    ...
    function getHashObject(
        ...
        return (
            tokenInfo[_collection][_tokenId].hashpower,
            tokenInfo[_collection][_tokenId].characteristId
        );
    }
    ...
    function getTokenInfo(
        ...
        return tokenInfo[_collection][_tokenId];
    }
    ...
    function getUserUpdated(
        ...
        user.timeOfLastUpdate = block.timestamp;
        user.conditionIdOflastUpdate = nextConditionId - 1;
        return user;
    }
    ...
    {
        return getRewardCondition(nextConditionId - 1);
    }
    ...
    function getRewardCondition(
        ...
        return rewardConditions[_conditionId];
    }
    ...
    function getAllTokenIdsPerUser(
        ...
        return tokenIdsPerUser[_user][_collection];
    }
    ...
    function _updateUnclaimedRewards(address _user) internal {
        uint256 rewards = _calculateRewards(_user);
        users[_user].unclaimedRewards += rewards;
        users[_user].timeOfLastUpdate = block.timestamp;
        users[_user].conditionIdOflastUpdate = nextConditionId - 1;
    }

    /** @dev calculates rewards for given user. Also considering interaction points
        @param _user: user address
        @return _rewards uint256 value for rewards */
    function _calculateRewards(
        address _user
    ) internal view returns (uint256 _rewards) {
        User memory user = users[_user];

        uint256 _userConditionId = user.conditionIdOflastUpdate;
        uint256 _nextConditionId = nextConditionId;
        uint256 _points = user.points;
        delete user.points;

        for (uint256 i = _userConditionId; i < _nextConditionId; i += 1) {
            RewardCondition memory condition = rewardConditions[i];

            uint256 startTime = i != _userConditionId
                ? condition.startTimestamp
                : user.timeOfLastUpdate;
    ...
    function _addToken(address _user, uint256 _tokenId) internal {
        User storage user = users[_user];
        uint256[] storage tokenIds = tokenIdsPerUser[_user][msg.sender];
        uint256 _hashpower = tokenInfo[msg.sender][_tokenId].hashpower;
        ...
        if (user.index == 0) {
            ...
            user.timeOfLastUpdate = block.timestamp;
            user.conditionIdOflastUpdate = nextConditionId - 1;
            user.index = usersArray.length - 1;
            if (user.collections.length == 0) {
                user.collections.push(address(0));
            }
        } else {
            _updateUnclaimedRewards(_user);
        }

        if (collectionIndex[_user][msg.sender] == 0) {
            user.collections.push(msg.sender);
            collectionIndex[_user][msg.sender] = user.collections.length - 1;
        ...
        tokenInfo[msg.sender][_tokenId].index = tokenIds.length - 1;
    }
