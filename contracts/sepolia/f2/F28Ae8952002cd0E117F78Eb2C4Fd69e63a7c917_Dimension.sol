...
// @author:      Moiz Arfeen - twitter.com/MoizAfeen2
...
contract Dimension is ERC721A, ReentrancyGuard, Ownable, DefaultOperatorFilterer{
    ...
    error MaxSupplyPreSaleExceeded();
    ...
    uint256 public reservedSupplyPresale = 1111;
    uint256 public maxMintPerWalletPreSale = 1;
    uint256 public maxMintPerWalletPublic = 10;
    uint256 public mintPricePreSale = 0.005 ether;
    uint256 public mintPricePublic = 0.01 ether;
    uint256 private presaleMintedAmount = 0;
    ...
    mapping (address => uint256) private totalMintedPreSale;

    constructor(bytes32 _merkleRoot) ERC721A("5th Dimension", "5D") {
    ...
    function setMintPricePreSale(uint256 _mintPricePreSale) public onlyOwner {
    ...
    function setReservedSupplyPresale(uint256 _reservedSupplyPresale) public onlyOwner {
        reservedSupplyPresale = _reservedSupplyPresale;
    }
    ...
    function setMaxMintPerWalletPreSale(uint256 _maxMintPerWalletPreSale) public onlyOwner {
        maxMintPerWalletPreSale = _maxMintPerWalletPreSale;
    }
    ...
    function publicMint(uint256 _quantity) external payable callerIsUser {
        if(!isPublicMintActive) revert PublicSaleNotActive();
    ...
    // ======= PreSale Mint =======
    function presaleMint(uint256 _quantity, bytes32[] memory _merkleProof) external payable callerIsUser {
        if(!isPreSaleMintActive) revert PreSaleNotActive();
        if(totalMintedPreSale[msg.sender] + _quantity > maxMintPerWalletPreSale) revert MaxPerWalletExceeded();
        if(msg.value < mintPricePreSale * _quantity) revert InsufficientValue();
        if((totalSupply() + _quantity) > maxSupply) revert MaxSupplyExceeded();
        if((presaleMintedAmount + _quantity) > reservedSupplyPresale) revert MaxSupplyPreSaleExceeded();
        totalMintedPreSale[msg.sender] += _quantity;
        _safeMint(msg.sender, _quantity);
        presaleMintedAmount += _quantity;
