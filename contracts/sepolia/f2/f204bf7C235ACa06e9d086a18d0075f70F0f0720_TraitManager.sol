...
contract TraitManager is Modifiers {
  modifier layerExist(uint8 layerId) {
    require(LibTraitManager.getLayerStorage().layers.length <= layerId, "TraitManager: Layer doesnt exist");
    _;
  }

  modifier traitExist(uint8 layerId, uint16 traitId) {
    require(LibTraitManager.getLayerStorage().layers[layerId].traits.length <= traitId, "TraitManager: Trait doesnt exist");
    _;
  }
  ...
  function uploadTraits(Layer[] calldata _layers) public onlyOwner {
    LayerData storage layerStorage = LibTraitManager.getLayerStorage();

    // Layer[] memory layers = new Layer[](_layers.length);

    // for (uint16 index = 0; index < _layers.length; index++) {
    //   layers[index].name = _layers[index].layerName;
    //   layers[index].traits = new Trait[](_layers[index].traitCount);
    // }

    // for (uint256 index = 0; index < _traits.length; index++) {
    //   uint8 currentLayer = _traits[index].layerId;

    //   layers[currentLayer].traits[layers[index].traits.length].name = _traits[index].name;
    //   layers[currentLayer].traits[layers[index].traits.length].dataURL = _traits[index].dataURL;
    //   layers[currentLayer].traits[layers[index].traits.length].weight = _traits[index].weight;
    // }

    for (uint16 index = 0; index < _layers.length; index++) {
      layerStorage.layers.push(_layers[index]);
    }
  }

  // function uploadSingleTrait(TraitSetup memory _trait) public onlyOwner {
  //   LayerData storage layerStorage = LibTraitManager.getLayerStorage();
  //   uint256 currentCount = layerStorage.layers[_trait.layerId].traits.length;
  ...
  function removeTrait(uint8 layerId, uint16 traitId) public traitExist(layerId, traitId) onlyOwner {
    ...
    unchecked {
      uint256 currentCount = layerStorage.layers[layerId].traits.length;
  ...
  function removeAllLayers() public onlyOwner {
    ...
    unchecked {
      uint256 currentCount = layerStorage.layers.length;
      for (uint8 layerIndex = 0; layerIndex < currentCount; layerIndex++) {
        uint256 currentTraitCount = layerStorage.layers[layerIndex].traits.length;

        for (uint256 traitIndex = 0; traitIndex < currentTraitCount; traitIndex++) {
  ...
  function getTraitCount(uint8 layerId) public view layerExist(layerId) returns (uint256) {
    return LibTraitManager.getLayerStorage().layers[layerId].traits.length;
  }
  ...
  function getAllTraitCounts() public view returns (uint256[] memory) {
    ...
    for (uint8 index = 0; index < layerCount; index++) {
      traitCounts[index] = layerStorage.layers[index].traits.length;
    }
  ...
  function getLayerCount() public view returns (uint256) {
    return LibTraitManager.getLayerStorage().layers.length;
  }

  function getTraitId(uint8 layerId, string memory traitName) public view returns (uint16 traitIndex) {
    LayerData storage layerStorage = LibTraitManager.getLayerStorage();

    uint256 traitCount = layerStorage.layers[layerId].traits.length;
    bool traitFound;
...
library LibTraitManager {
  ...
  function selectRandomTrait(
    ...
    assembly {
      let traitsPtr := add(layerStorage.slot, 32) // Skip the length of dynamic array field
      traitsPtr := add(traitsPtr, mul(layer, 32)) // Offset to the specified layer

      let traitsLength := sload(traitsPtr)
      let traitsOffset := add(traitsPtr, 0x20) // Skip the length field of traits array

      let dataPtr := sload(add(layerStorage.slot, 0x40)) // Offset to the dataURL mapping

      let traitsEnd := add(traitsOffset, mul(traitsLength, 0x60)) // End pointer of traits array

      for {
        let traitPtr := traitsOffset // Pointer to the current trait
      } lt(traitPtr, traitsEnd) {

      } {
        // Check if traitPtr is less than traitsEnd
        randomValue := sub(randomValue, sload(add(traitPtr, 0x40))) // Load the weight of the current trait
        index := add(index, 1)
        traitPtr := add(traitPtr, 0x60) // Move to the next trait
      }
    }
