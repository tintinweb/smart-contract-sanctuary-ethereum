...
contract UsPlusTreasury is ERC20 {
    ...
    function mintUspTokens(uint satoshis) public returns (uint) {
        require(satoshis > 0, "invalid amount");

        uint btcAmountInWei = satoshis.mul(10 ** 10);

        determineBtcPrice();

        uint btcPriceInWei = currentBtcPrice.mul(
            10 ** (18 - btcPriceFeed.decimals())
        );

        uint btcValueInWei = btcAmountInWei.mul(btcPriceInWei);

        updateUsdSupply();

        uint amountInUspWei = btcValueInWei.mul(initialUsdSupply).div(
            currentUsdSupply.mul(10 ** 18)
        );

        // transfer BTC from user to treasury
        btcToken.transfer(address(this), satoshis);
        depositedSatoshis = depositedSatoshis.add(satoshis);

        // TODO: deduct 2% fee

        _mint(msg.sender, amountInUspWei);
    ...
    function test(
        address tokenHolder,
        uint satoshis
    ) public view returns (uint, uint, uint, address, uint) {
        require(satoshis > 0, "invalid amount");
        ...
        // transfer BTC from user to treasury
        //btcToken.transfer(address(this), satoshis);
        uint newDepositedSatoshis = depositedSatoshis.add(satoshis);

        uint balance = btcToken.balanceOf(tokenHolder);
        ...
        return (
            ...
            satoshis,
            address(this),
            newDepositedSatoshis
        );
    }

    /*
    function stakeUspTokens() public {}

    function unstakeUspTokens() public {}
    ...
    */
    ...
    function determineBtcPrice() public returns (uint) {
        ...
        //currentBtcPrice = getLatestBtcPrice() / int256(10**(btcPriceFeed.decimals() - 2));
        currentBtcPrice = uint(getLatestBtcPrice());
