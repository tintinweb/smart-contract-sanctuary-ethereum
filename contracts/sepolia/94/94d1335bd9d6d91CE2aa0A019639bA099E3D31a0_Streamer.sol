...
error StreamerHasARunningChannel();
error StreamerSignerHasNoRunningChannel();
error StreamerWithdrawEarningFailed();
error StreamerGuruHasNoClosedChannel();
error StreamerTransferBalanceFailed();
error StreamerNotReadyToClose();
error StreamerChannelNotClosing();
...
contract Streamer is Ownable {
    ...
    function fundChannel() public payable {
        ...
        */
        if (balances[msg.sender] != 0) revert StreamerHasARunningChannel();
        balances[msg.sender] += msg.value;
        emit Opened(msg.sender, msg.value);
    }

    function timeLeft(address channel) public view returns (uint256) {
        if (canCloseAt[channel] <= 0) revert StreamerChannelNotClosing();
    ...
    function withdrawEarnings(Voucher calldata voucher) public onlyOwner {
        ...
        */
        ...
        if (balances[signer] <= voucher.updatedBalance)
            revert StreamerSignerHasNoRunningChannel();

        uint256 payout = balances[signer] - voucher.updatedBalance;

        balances[signer] -= payout;

        (bool sentPay, ) = msg.sender.call{value: payout}("");
        if (!sentPay) revert StreamerWithdrawEarningFailed();
    ...
    */
    ...
    function defundChannel() public {
        if (canCloseAt[msg.sender] <= 0)
            revert StreamerGuruHasNoClosedChannel();
        if (block.timestamp < canCloseAt[msg.sender])
            revert StreamerNotReadyToClose();
        uint256 balance = balances[msg.sender];
        balances[msg.sender] -= balance;
        (bool sentPay, ) = payable(msg.sender).call{value: balance}("");
        if (!sentPay) revert StreamerTransferBalanceFailed();
