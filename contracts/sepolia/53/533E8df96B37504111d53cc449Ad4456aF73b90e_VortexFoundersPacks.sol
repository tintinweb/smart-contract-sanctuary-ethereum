...
contract VortexFoundersPacks is Ownable {
    ...
    uint256 public constant MAX_MINTS = 25;

    uint256 public totalSupply;

    PackCollection public packContract;

    constructor(PackCollection _packContract) {
    ...
    function setPackContract(PackCollection _packContract) external onlyOwner {
        packContract = _packContract;
    }
    ...
    function getMints(address _minter) external view returns (uint256) {
        return mints[_minter];
    }

    function mint(uint256 _amount) external verifyAvailableSupply(_amount) {
        ...
        if (mints[to] > MAX_MINTS) revert ExceedsWalletSupplyError();

        packContract.mint(to, 1, _amount, "");
    }
}
...
import "./interfaces/IPackBurner.sol";

contract PackCollection is
    OperatorFilterer,
    ERC1155Pausable,
    IPackBurner,
    FlagRoles,
    ERC2981
{
    ...
    function setURI(string memory _uri) external onlyRole(MODERATOR) {
        _setURI(_uri);
    }

    function pause() public virtual onlyRole(MODERATOR) {
        _pause();
    }

    function unpause() public virtual onlyRole(MODERATOR) {
        _unpause();
    }
