...
contract GameContract is Ownable {
	...
	function playSolo(uint256 _cardId,uint256 _score,uint8 _caracter,bytes memory _signature) external payable isCardOwner(_cardId) returns(uint256 gameID,uint8 _winner) {
		...
		if((_caracter == 1 && _score < random_score) ||(_caracter == 1 && _score < random_score) ) {
			_winner = 1;
			userRewards[msg.sender] = (msg.value*rewardPrc)/1000;
		}
		else _winner = 2;
	...
	function joinGame(uint256 _gameId,uint256 _cardId,uint256 _score,bytes memory _signature) external payable isCardOwner(_cardId) returns(uint8 _winner){
	    ...
	    require(msg.value >= join_price,"Invalid Amount");
		require(games[_gameId].p1_address != address(0),"Game not found");
		require(games[_gameId].winner == 0,"Game is Over");
	    bytes32 msgHash = keccak256(
		...
		uint256 win_card = getClosestValue(games[_gameId].number,_score,random_score);
		...
		_winner = (win_card == _score)? 2:1;
		games[_gameId].winner = _winner;
		address _winnerAddress = (_winner == 1)?games[_gameId].p1_address:games[_gameId].p2_address;
		userRewards[_winnerAddress] = (games[_gameId].reward*rewardPrc)/1000;
		emit PvPGame(_gameId,random_score,_winnerAddress);
		GameNft(gameNft).mintNft(_gameId,games[_gameId].p1_address,_winner==1);   
		GameNft(gameNft).mintNft(_gameId,games[_gameId].p2_address,_winner==2);   
	}
	...
	function updateArenaAddress(address _contract) external onlyOwner{
		cryptoCardArenaAddress = _contract;
    }

	function updateGameNftAddress(address _gameNft) external onlyOwner{
		gameNft = _gameNft;
    }

	function updateGameNftAddress(uint256 _join_p,uint256 _reward_prc) external onlyOwner{
		join_price = _join_p;
...
contract GameNft is ERC721{
	...
	mapping(uint256 => mapping(address => uint256)) public user_gameNfts;
	...
	function mintNft(uint256 _gameId,address _user,bool _iswinner) external onlyGameContract  returns (uint256)
    {

		_tokenIds.increment();
        uint256 newItemId = _tokenIds.current();
		contractNfts[newItemId] = ArenaGame(_gameId,_user,_iswinner);
		user_gameNfts[_gameId][_user] = newItemId;
