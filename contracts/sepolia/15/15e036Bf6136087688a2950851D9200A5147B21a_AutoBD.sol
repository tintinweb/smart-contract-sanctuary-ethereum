...
contract AutoBD {
    ...
    event Commited(
        bytes32 indexed commitedHash, uint256 stakedAmount, uint256 price
    );
    ...
    address internal constant airnode = 0x2B68407d77B044237aE7f99369AA0347Ca44B129;
    ...
    function commit(
        ...
        require(
            (
                keccak256(abi.encodePacked(price, timestamp))
                    .toEthSignedMessageHash()
            ).recover(signature) == airnode,
            "Signature mismatch"
        );
        if (timestamp + 60 <= block.timestamp) revert OutOfDatePrice();
        ...
        emit Commited(commitHash_, amount, price);
    }
