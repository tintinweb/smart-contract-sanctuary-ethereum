...
contract FeralfileVault is Ownable, FeralfileSaleData, ECDSASigner {
    ...
    function payForSale(
        ...
        bytes32 message = keccak256(
            abi.encode(msg.sender, saleData_)
        );
        require(!_paidSale[message], "FeralfileVault: paid sale");
        require(
            isValidSignature(message, r_, s_, v_),
            string(abi.encodePacked("FeralfileVault: invalid signature", block.chainid))
        );
...
contract ECDSASigner is Ownable, FeralfileSaleData {
