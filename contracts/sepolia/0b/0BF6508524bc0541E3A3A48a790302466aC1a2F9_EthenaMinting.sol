...
contract EthenaMinting is IEthenaMinting, Ownable, ReentrancyGuard, Pausable {
    ...
    function deposit(address asset, uint256 amount, address beneficiary) nonReentrant public {
    ...
    function depositWithPermit(
        ...
        ++depositIds[beneficiary];
        emit PermitDeposited(msg.sender, asset, amount);
    }
    ...
    function discharge(address asset, uint256 amount, address beneficiary) nonReentrant public {
        ...
        emit Discharged(msg.sender, asset, amount);
    }

    /// @notice discharge with permit EIP-2612
    /// @param asset address of the asset to discharge
    /// @param amount amount of eUSD to burn
    /// @param beneficiary address of the beneficiary
    /// @param deadline deadline of the permit
    /// @param v,r,s of the permit signature
    function dischargeWithPermit(
        address asset,
        uint256 amount,
        address beneficiary,
        uint256 deadline,
        ...
        dischargeIds[beneficiary]++;
        emit PermitDischarged(msg.sender, asset, amount);
    }
...
interface IEthenaMinting {
    ...
    event PermitDeposited(address indexed depositor, address indexed asset, uint256 amount);

    // @notice Event emitted when a stable is discharged
    event Discharged(address indexed depositor, address indexed asset, uint256 amount);

    // @notice Event emitted when a stable is discharged
    event PermitDischarged(address indexed depositor, address indexed asset, uint256 amount);
