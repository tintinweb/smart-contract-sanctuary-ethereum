    ...
    OwnableUpgradeable
{
    ...
    address private HEEDONG_CONTRACT_ADDRESS;
    ...
    mapping(uint256 => uint256) public tokenBlindboxClaimStatus; // token id => timestamp
    ...
    event Claim(address _address, uint256 _quantity, uint256 timestamp);
    ...
    //  Mint
    //  ============================================================
    /// @dev mints tokens to delegated wallets
    ...
    /// @param _staked array of boolean to indicate if the token is staked
    function updateStaking(
        uint256[] calldata _tokenIds,
        bool[] calldata _staked
    ) external onlyRole(DEV_ROLE) {
        for (uint256 i; i < _tokenIds.length; i++) {
            if (_staked[i]) {
    ...
    function claim(
        uint256[] calldata heedongTokenIds,
        bytes calldata signature
    )
        public
        isSignerValid(signature, heedongTokenIds)
    {
        require(heedongTokenIds.length != 0, "No tokens available for claiming Blindbox");

        uint256 timestamp = block.timestamp;
        uint256 quantity = 0;
        for (uint256 i; i < heedongTokenIds.length; i++) {
            require(IERC721(HEEDONG_CONTRACT_ADDRESS).ownerOf(heedongTokenIds[i]) == msg.sender, "Not Token Owner");

            require(tokenBlindboxClaimStatus[heedongTokenIds[i]] == 0, "Token already claimed");
            
            tokenBlindboxClaimStatus[heedongTokenIds[i]] = timestamp;
            quantity++;
        }

        require(quantity != 0, "No tokens available for claiming Blindbox");

        _mint(msg.sender, quantity);
        emit Claim(msg.sender, quantity, timestamp);
    }

    modifier isSignerValid(
        bytes calldata signature,
        uint256[] calldata heedongTokenIds
    ) {
        bytes32 messageHash = keccak256(
            abi.encodePacked(heedongTokenIds, msg.sender)
        );
    ...
    function setHeedongAddress(address _address)
        external
        onlyRole(DEV_ROLE)
    {
        HEEDONG_CONTRACT_ADDRESS = _address;
    }
