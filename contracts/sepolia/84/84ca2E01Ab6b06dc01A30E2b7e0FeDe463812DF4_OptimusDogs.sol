...
contract OptimusDogs is ERC721A, Ownable, ReentrancyGuard, ERC2981 {
    ...
    uint256 public immutable maxBatchSizeForOG;
    ...
    constructor(
        ...
        _hiddenURI = "ipfs://puturiGIF/"; // L'URI nascosto per la GIF
        _baseTokenURI = _hiddenURI; // All'inizio, l'URI di base è l'URI nascosto
        _setDefaultRoyalty(msg.sender, 500); // 5% royalties
        maxPerAddressDuringMint = maxBatchSize_;
        maxBatchSizeForOG = maxBatchSize_ * 2;
    ...
    function mintForWLorOG(uint256 amount, bytes32[] calldata merkleProof, bool isOG) external payable nonReentrant {
        if (salePhase != 1) revert SaleNotInWLOGPhase();
        bytes32 merkleRoot = isOG ? merkleRootOG : merkleRootWL;
        if (!_verifyMerkleProof(msg.sender, merkleProof, merkleRoot)) revert InvalidMerkleProof();
        uint256 maxBatchSize = isOG ? maxBatchSizeForOG : maxPerAddressDuringMint;
        if (amount > maxBatchSize) revert ExceedsMaxPerTransaction();
        if (_mintedPerAddress[msg.sender] + amount > maxBatchSize) revert ExceedsMaxPerAddress();
        if (totalSupply() + amount > AmountForPublic) revert ExceedsMaxPerPublic();

        uint256 totalPrice = isOG ? OG_PRICE * amount : WL_PRICE * amount;
    ...
    function revealBaseURI(string calldata baseURI) external onlyOwner {
