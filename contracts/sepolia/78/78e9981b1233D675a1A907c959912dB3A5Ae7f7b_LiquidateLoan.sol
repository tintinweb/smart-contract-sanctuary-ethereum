...
*
*/
contract LiquidateLoan is Ownable2Step, AccessControl {
    ...
    event LiquidateResult(address indexed user, address asset, uint256 amount, uint256 profit, address receiver);

    // intantiate lending pool addresses provider and get lending pool address
    constructor(IPoolAddressesProvider _addressProvider, IUniswapV2Router02 _uniswapV2Router) {
    ...
    function updateAddr(IPoolAddressesProvider _addressProvider, IUniswapV2Router02 _uniswapV2Router) external onlyOwner {
        provider = _addressProvider;
        lendingPool = IPool(provider.getPool());
        uniswapV2Router = _uniswapV2Router;
    }

    function updatePoolAddr() external {
        lendingPool = IPool(provider.getPool());
    }

    /**
        This function is called after your contract has received the flash loaned amount
        refer: https://github.com/aave/aave-v3-core/blob/master/contracts/flashloan/interfaces/IFlashLoanReceiver.sol
     */
        ...
        returns (bool)
    {
        ...
        //users: 0: user to liquidate 1: profit receiver
        //amountOutMin - minimum amount of asset paid when swapping collateral

        (address collateral, address[] memory users, uint256 amountOutMin, address[] memory swapPath) = abi.decode(params, (address, address[], uint256, address[]));

        //liquidate unhealthy loan
        liquidateLoan(collateral, assets[0], users[0], amounts[0], false);

        //swap collateral from liquidate back to asset from flashloan to pay it off
        swapToBorrowedAsset(collateral,assets[0],amountOutMin, swapPath);

        //Pay to owner the balance after fees
        repayFlashLoan(assets, amounts, premiums, users[0], users[1]);
        return true;
    }
    ...
    function repayFlashLoan(
        ...
        //Pay to owner the balance after fees
        (bool hasProfit, uint256 profit) = calcProfits(IERC20(assets[0]).balanceOf(address(this)),amounts[0],premiums[0]);

        require(hasProfit, "No profit");
        if(profit > 0) {
            IERC20(assets[0]).transfer(profitReceiver, profit);
        }

        uint amountOwing = amounts[0].add(premiums[0]);
        IERC20(assets[0]).approve(address(lendingPool), amountOwing);
        emit LiquidateResult(userToLiquidate, assets[0], amounts[0], profit, profitReceiver);
    }
    ...
    {
        return _balance.trySub(_loanAmount.add(_loanFee));
    }
    ...
    function swapToBorrowedAsset(address asset_from, address asset_to, uint amountOutMin, address[] memory swapPath ) public {
    ...
    * _profitReceiver - the account address who receive liquidation profit
    */
    function executeFlashLoans(address _assetToLiquidate, uint256 _flashAmt, address _collateral, address _userToLiquidate, uint256 _amountOutMin, address[] memory _swapPath, address _profitReceiver) public onlyRole(OPERATOR_ROLE) {
        ...
        users[0] = _userToLiquidate;
        users[1] = _profitReceiver;
        ...
        bytes memory params = abi.encode(_collateral, users, _amountOutMin, _swapPath);
