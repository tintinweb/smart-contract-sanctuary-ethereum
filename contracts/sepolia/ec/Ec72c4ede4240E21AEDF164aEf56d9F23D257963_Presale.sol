...
contract Vesting is Ownable, ReentrancyGuard {
    ...
    // =============================================================
    //
    function getClaim(address _beneficiary) external nonReentrant {
        require(block.timestamp > startDate, "Claiming has not begun");
    ...
    function setStartDate(uint256 epoch) external onlyOwner {
        startDate = epoch;
    }
...
contract Presale is Vesting {
    ...
    uint256 public totalSupply = 10000000000 * 10 ** 18; //do we need this, aur would mint
    ...
    function buyTokensFromEth(address beneficiary) public payable nonReentrant {
        ...
        uint256 tokens = _getTokenFromEth(amount);
        ...
        // ********************************
        emit TokensPurchasedFromEth(_msgSender(), beneficiary, amount, tokens);
    }
    ...
    function buyTokensFromBnb(
        address beneficiary,
        uint256 amount
    ) public nonReentrant {
        _preValidatePurchase(beneficiary, amount);
        require(
            _bnb.transferFrom(beneficiary, _wallet, amount),
            "BNB Transfer Failed"
        );
        // calculate token amount to be created
        uint256 tokens = _getTokenFromBnb(amount);
        _bnbRaised = _bnbRaised.add(amount);
        tokenBought[beneficiary] += tokens;
        createVestingSchedule(beneficiary, tokens);
        tokensSold = tokensSold + tokens;
        ...
        // ********************************
        emit TokensPurchasedFromBnb(_msgSender(), beneficiary, amount, tokens);
    }
    ...
    function _preValidatePurchase(
        ...
        // require(
        //     hasClaimed[beneficiary].exists = false,
    ...
    function _getTokenFromEth(
        ...
        // function receives 1 ETH = 10^18
        // 10 ** 18 = 174361940000 wei of usd     // 174361940000 received from chainlink
        uint256 ethPriceInUSD = uint256(getLatestPriceEth());
        uint256 oneWeiUsd = 10 ** 18 / ethPriceInUSD; //10 ** 18 / 174361940000; //= 5735196 wei eth
        // if 1 wei usd = 5735196 wei eth then we can say
        // 378,787,878,787,878 wei pmdg  = 5735196 wei eth , then
        // 1 wei of eth = 378,787,878,787,878 / 5735196 = 66046195 wei pmdg per wei of eth
        //so if function receives weiAmount = 1,000,000,000,000,000,000 wei of Eth
        // we just multiply value of 1 wei of eth to weiAmount and gets the number of tokens to be transferred in pmdg
        // 66046195 * 1,000,000,000,000,000,000 = 66046195000000000000000000 wei of pmdg = 66,046,195 pmdg
        uint256 PMDGperEth = _rate.div(oneWeiUsd);
        uint256 PMDGamount = PMDGperEth * weiAmount;
        return PMDGamount;
    }

    function _getTokenFromBnb(
        uint256 usdtAmount //1* 10**6
    ) internal view returns (uint256) {
        // 1 BNB = 10 ** 18
        // 10 ** 18 = 27700000000 // 1 bnb = 27700000000 usd from chainlink
        // uint256 bnbPriceInUSD = uint256(getLatestPriceBnb());
        uint256 bnbPriceInUSD = 27700000000;
        // 1 wei of usd = 10 ** 18 / 27,700,000,000 = 36101083 wei of bnb
        uint256 oneWeiUsd = 10 ** 18 / bnbPriceInUSD;
        // 378,787,878,787,878 wei pmdg  = 36101083 wei of bnb
        // then 1 wei bnb = 378,787,878,787,878 / 36101083 = 10,492,424 wei pmdg
        uint256 PMDGperBnb = _rate.div(oneWeiUsd);
        // so if function receives 10**18 wei bnb as weiAmount
        // value of 1 wei of bnb  * weiAMount = 10,492,424 * 10**18 = 10492424000000000000000000 wei pmdg = 10,492,424 PMDG
        uint256 PMDGamount = PMDGperBnb * usdtAmount;
        return PMDGamount;
    }

    function _getTokenFromUsdt(
        uint256 bnbAmount //1* 10**18
    ) internal view returns (uint256 tokenAmount) {
        //1 usdt = 10**6
        // 10**6 = 99990716 wei of usd
        // 1 wei usd = 10 ** 6 / 99990716
        uint256 usdtPriceInUSD = uint256(getLatestPriceUsdt());
        // uint256 oneWeiUsd = 10 ** 6 / usdtPriceInUSD; // would be zero
        // 378,787,878,787,878 wei pmdg  = 10 ** 6 / 99990716
        // then 1 wei of usdt
        // 1 wei of usdt = 378,787,878,787,878  * (99990716 / 10**6)
        uint256 PMDGperUSDT = _rate * (usdtPriceInUSD / 10 ** 6);
        // 1 wei of usdt = 37,875,271,212,121,133
        // so if function receives 10**6 wei usdt as weiAmount
        // value of 1 wei of usdt  * weiAMount = 37,875,271,212,121,133 * 10**6 = 37875271212121133000000 wei pmdg = 37875.271212121133 PMDG
        uint256 PMDGamount = PMDGperUSDT * bnbAmount;
        return PMDGamount;
    }
