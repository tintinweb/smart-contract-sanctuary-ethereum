...
contract OTCModule is Module, UUPSUpgradeable, FixinAccessControlUpgradeable, IMercuryModule {
    ...
    function createDesk(bytes memory deskParams, address[] memory accessptedCurrencies, address[] memory whitelist) external onlyRole(GOVERNOR_ROLE) {
        (
            address   _token,
...
contract OTCModuleFactory is IModuleDeployer {
    function deploy(bytes32 salt)
        external
        returns (address addr) {
            addr = address(new OTCModule{ salt: salt }());   
        }
}