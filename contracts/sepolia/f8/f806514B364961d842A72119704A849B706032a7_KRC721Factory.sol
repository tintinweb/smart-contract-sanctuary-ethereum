...
contract KRC721 is ERC721URIStorage {
    ...
    function buy(uint256 tokenId) public payable {
        ...
        if (isKrc20) {
            address(krc20Address).call(
                abi.encodeWithSignature("mint(address)", msg.sender)
            );
            address(krc20Address).call{value: krmDonation}(
                abi.encodeWithSignature("donation()")
            );
        }
...
contract KRC721Factory {
    address private constant KRC20_ADDR =
        0x66B51c535bdA25057b8ddB3362C5a2B8eE9d6F81;
    ...
    function createKRC721(
        ...
        bool isKrc20
    ) public onlyAuthorizedCreators returns (address) {
