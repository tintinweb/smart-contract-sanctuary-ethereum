    ...
    ReentrancyGuardUpgradeable
{
    ...
    event TaxAccountUpdated(address);
    event TaxUpdated(uint256);
    ...
    function initializeToken(
        ...
        _totalSupply = 10e9 * 10 ** 18;
        _tax = 3;
        _taxAccount = _msgSender();
    ...
    function updateTax(uint256 tax) external onlyRole(DEFAULT_ADMIN_ROLE) {
        require(tax < 25, "Tax should not be more than 25%");
        if (_tax != tax) {
            _tax = tax;
            emit TaxUpdated(tax);
        }
    ...
    function updateTaxAccount(
        ...
        if (taxAccount != _taxAccount) {
            _taxAccount = taxAccount;
            emit TaxAccountUpdated(taxAccount);
        }
    }
