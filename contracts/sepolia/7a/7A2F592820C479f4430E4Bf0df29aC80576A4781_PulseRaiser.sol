...
contract PulseRaiser is IPulseRaiser, Normalizer, ClaimTracker {
    ...
    function collate(
        ...
        tokenPerPoint = token.distributionSupply() / pointsTotal;
    ...
    function distribute() external {
        _checkOwner();
        require(tokenPerPoint > 0, "Call collate First");
        claimsEnabled = true;
        emit ClaimsEnabled();
    }
    ...
    function _encodePriceBases(
        ...
        require(bases_.length == 20, "Expected PPP for 20 Days");
        uint256 encode = bases_[19] << 10;
        for (int8 b = 18; b >= 0; b--) {
            encode |= bases_[uint8(b)];
            encode = encode << 10;
        }
