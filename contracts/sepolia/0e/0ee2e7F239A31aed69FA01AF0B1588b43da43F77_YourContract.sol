...
contract YourContract is AccessControl, ReentrancyGuard {
   ...
   function availableCreatorAmountERC20(address _creator, address _token) public view isERC20FlowActive(_creator, _token) returns (uint256) {
        ...
        require(cap > 0, "TokenNotSupported");
    ...
    require(_tokenAddresses.length == _caps.length, "MismatchedArrays");

    for (uint256 i = 0; i < _tokenAddresses.length; ++i) {
        require(_tokenAddresses[i] != address(0), "InvalidTokenAddress");
        require(_caps[i] > 0, "InvalidCap");
        require(erc20FlowingCreators[_creator].caps[_tokenAddresses[i]] == 0, "DuplicateUserNotAllowed");
...
function addBatchERC20Creators(
    ...
    require(cLength == _caps.length && cLength == _tokenAddresses.length, "MismatchedArrays");
    
    for (uint256 i = 0; i < cLength; ++i) {
        require(_tokenAddresses[i].length == _caps[i].length, "MismatchedTokenAndCapArrays");
        
        for (uint256 j = 0; j < _tokenAddresses[i].length; ++j) {
            require(_tokenAddresses[i][j] != address(0), "InvalidTokenAddress");
            require(_caps[i][j] > 0, "InvalidCap");
...
// Validate the input for acreator.
    ...
    // Update a creator's ERC20 flow cap and cycle.
      function updateCreatorERC20FlowCapCycle(
        ...
        require(_newCap > 0, "CapCannotBeZero");
   ...
   function removeCreatorERC20Flow(address _creator, address _token) external onlyAdmin isERC20FlowActive(_creator, _token) {
    require(erc20FlowingCreators[_creator].caps[_token] > 0, "TokenNotSupported");
    ...
    function erc20Withdraw(
        ...
        require(totalAmountCanWithdraw >= _amount, "InsufficientInFlow");
        ...
        require(tokenBalance >= _amount, "InsufficientFundsInContract");
