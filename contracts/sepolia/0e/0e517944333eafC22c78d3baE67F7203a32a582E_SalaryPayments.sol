...
// File: contracts/Chainlink-Functions/KeeperRegistrarInterface.sol
...
// File: contracts/Chainlink-Functions/SelfAutomation.sol
...
// File: contracts/Chainlink-Functions/UsdEthPairConverter.sol
...
// File: contracts/Chainlink-Functions/SalaryPayments.sol
    ...
    ReentrancyGuard
{
    ...
    function deleteAnEmployee(uint _deletedEmployeeIndex) public onlyOwner {
        if (_deletedEmployeeIndex >= employeeAddressList.length)
            revert Index__NotInRange();
        for (
            uint i = _deletedEmployeeIndex;
            i < employeeAddressList.length - 1;
            i++
        ) {
            employeeAddressList[i] = employeeAddressList[i + 1];
            usdAmountArray[i] = usdAmountArray[i + 1];
        }
        employeeAddressList.pop();
        usdAmountArray.pop();
        delete EmployersMapping[_deletedEmployeeIndex];
    }
    ...
    function withdrawBalance(uint _balance) public onlyOwner {
    ...
    function checkUpkeep(
        ...
        bool hasEmployees = employeeAddressList.length > 0;
