...
contract SaleManager is Modifiers {
  ...
  modifier seedCheck(uint256 seeds) {
    require(seeds >= LibTraitManager.getStorage().layerCount, "SaleManager: Not enough seeds");
    _;
  }
...
library LibTraitManager {
  bytes32 internal constant TRAIT_STORAGE_SLOT = keccak256("trait.manager.storage.layout.slot");
  ...
  function getStorage() internal pure returns (TraitStorage storage strg) {
    bytes32 slot = TRAIT_STORAGE_SLOT;
    assembly {
      strg.slot := slot
    }
  }
  ...
  function generateMetadata(TraitStorage storage layerStorage, uint tokenId, uint16[] calldata seeds) internal returns (uint16[] memory) {
    unchecked {
      uint16 iteration = 1;
  ...
  function selectRandomTrait(
    TraitStorage storage layerStorage,
    uint8 layer,
    uint tokenId,
    uint seed,
    uint i,
    ...
    assembly {
      ...
      } lt(loopRandomValue, sload(add(traitsOffset, mul(loopIndex, 32)))) {

      } {
        loopRandomValue := sub(loopRandomValue, sload(add(traitsOffset, mul(loopIndex, 32))))
  ...
  function randomize(uint tokenId, uint seed, uint iteration, uint layerWeight) internal view returns (uint256 result) {
