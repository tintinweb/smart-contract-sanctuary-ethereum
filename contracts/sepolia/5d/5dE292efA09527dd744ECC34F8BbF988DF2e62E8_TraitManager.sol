...
contract TraitManager is Modifiers {
  ...
  function sumTraitTotalWeight(uint8 layerId) internal view returns (uint256 totalWeight) {
    LayerData storage layerStorage = LibTraitManager.getLayerStorage();
    uint256 traitCount = layerStorage.layers[layerId].traits.length;
  ...
  function getTotalWeights() public view returns (uint256[] memory) {
    LayerData storage layerStorage = LibTraitManager.getLayerStorage();
...
struct LayerData {
  ...
  mapping(uint256 => uint256) totalWeights;
}
...
library LibTraitManager {
  ...
  function selectRandomTrait(
    ...
    assembly {
      let traitsPtr := add(layerStorage.slot, 32) // Skip the length of the array
      traitsPtr := add(traitsPtr, mul(layer, 96)) // Multiply by 96 bytes per Layer struct
      let traitsEnd := add(traitsPtr, mul(sload(traitsPtr), 96)) // Get the end pointer of the traits array

      for {

      } lt(randomValue, sload(add(traitsPtr, 64))) {

      } {
        randomValue := sub(randomValue, sload(add(traitsPtr, 64)))
        traitsPtr := add(traitsPtr, 96) // Move to the next Trait struct

        // Check if we reached the end of the traits array
        traitsEnd := add(traitsEnd, 96)
        if gt(traitsPtr, traitsEnd) {
          index := 0
          break
        }
        index := add(index, 1)
