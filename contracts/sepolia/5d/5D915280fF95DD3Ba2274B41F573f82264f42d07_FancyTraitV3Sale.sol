...
import "./interfaces/IFancyTraitsV2Sale.sol";

import "./Tag.sol";

contract FancyTraitV3Sale is AccessControlEnumerable, ReentrancyGuard {
    ...
    */
    ...
    IFancyTraitsV2Sale public fancyTraitsV2Sale;
    ...
    constructor(
        IFancyTraitsV2Sale _fancyTraitsV2Sale,
        ...
        fancyTraitsV2Sale = _fancyTraitsV2Sale;
    ...
    function purchaseTraitsWithHoney(
        ...
        IFancyTraitsV2Sale.LevelAttributionArgs calldata _levelAttributionArgs
    ) public {
        require(
        ...
        tokenContracts[0] = address(honeyTokenContract);

        for (uint256 i = 0; i < purchaseData.traitTokenIds.length; ) {

            (uint256 ethPrice, uint256 counter, uint256 maxSupply, bool saleActive) = fancyTraitsV2Sale.tokenSaleData(purchaseData.traitTokenIds[i]);            

            uint256[] memory prices = fancyTraitsV2Sale.getTokenPriceForTraitId(purchaseData.traitTokenIds[i], tokenContracts);
            
            require(prices.length == 1, "purchaseTraitsWithHoney: honey price not set");
            ...
            totalHoneyRequired += (prices[0]).mul(purchaseData.amountPerTrait[i]);
 
            updateTraitV2SaleData(purchaseData.traitTokenIds[i], ethPrice, counter, purchaseData.amountPerTrait[i], maxSupply, saleActive); 
    ...
    function purchaseTraitsWithETH(
        ...
        fancyTraitsV2Sale.purchaseTraitsWithETH(_traitTokenIds, _amountPerTrait, _levelAttributionArgs);
        ...
        for (; i < _traitTokenIds.length; ) {
            ...
            (uint256 ethPrice, uint256 counter, uint256 maxSupply, bool saleActive) = fancyTraitsV2Sale.tokenSaleData(_traitTokenIds[i]);            
            ...
            totalETHRequired += ethPrice.mul(
                    _amountPerTrait[i]
                );

            
            updateTraitV2SaleData(_traitTokenIds[i], ethPrice, counter, _amountPerTrait[i], maxSupply, saleActive); 
    ...
    function purchaseTraitsWithERC20(
        ...
        fancyTraitsV2Sale.purchaseTraitsWithERC20(_tokenContract, _traitTokenIds, _amountPerTrait, _levelAttributionArgs);
        ...
        address[] memory tokenContracts = new address[](1);
        tokenContracts[0] = address(_tokenContract);
        ...
        for (; i < _traitTokenIds.length; ) {
            ...
            uint256[] memory prices = fancyTraitsV2Sale.getTokenPriceForTraitId(_traitTokenIds[i], tokenContracts);
            
            require(prices.length == 1, "purchaseTraitsWithERC20: ERC20 price not set");
            ...
            totalTokensRequired +=prices[0].mul(_amountPerTrait[i]);
    ...
    function updateTraitV2SaleData(uint256 _traitId, uint256 _ethPrice, uint256 _currentCounter, uint256 _purchasedAmount, uint256 _maxSupply, bool _saleActive) internal {

        IFancyTraitsV2Sale.SaleDataUpdateArgs[] memory saleDataUpdate = new IFancyTraitsV2Sale.SaleDataUpdateArgs[](1);
        saleDataUpdate[0] = IFancyTraitsV2Sale.SaleDataUpdateArgs(_ethPrice, _currentCounter + _purchasedAmount, _maxSupply, _saleActive, new address[](0), new uint256[](0));

        uint256[] memory updateTokenIds = new uint256[](1);
        updateTokenIds[0] = _traitId;

        fancyTraitsV2Sale.updateSaleDataBulk(updateTokenIds, saleDataUpdate);
...
interface IFancyTraitsV2Sale  {
    ...
    function getTokenPriceForTraitId(uint256, address[] calldata) external view returns (uint256[] memory);
    function tokenSaleData(uint256) view external returns (uint256,uint256,uint256,bool);
    function updateSaleDataBulk(uint256[] calldata, SaleDataUpdateArgs[] calldata) external;

    function purchaseTraitsWithETH(uint256[] calldata, uint256[] calldata, LevelAttributionArgs calldata) external payable;
    function purchaseTraitsWithERC20(address, uint256[] calldata, uint256[] calldata, LevelAttributionArgs calldata) external payable;
