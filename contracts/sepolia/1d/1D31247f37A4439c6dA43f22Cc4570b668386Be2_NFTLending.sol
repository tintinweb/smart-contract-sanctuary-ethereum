...
contract NFTLending {
    ...
    // Updated function for getting NFT list without double loop
    function getAllNFTsUpdated() public view returns (Nft[] memory) {
        Nft[] memory nftList = new Nft[](nftCount);
        uint index = 0;

        address[] memory contractAddresses = new address[](ownerAddresses.length);
        uint256[] memory tokenIds = new uint256[](ownerAddresses.length);
        address _owner;
        KeyData storage keyData;

        for (uint i = 0; i < ownerAddresses.length; i++) {
            _owner = ownerAddresses[i];
            keyData = nftKeys[_owner];
            contractAddresses[i] = keyData.contractAddresses[i];
            tokenIds[i] = keyData.tokenIds[i];
        }
