...
import "./operators/DefaultOperatorFilterer.sol";
...
contract NFT is ERC721, Ownable, DefaultOperatorFilterer {
    ...
    function mint(uint256 _numTokens) external payable {
        ...
        for(uint256 i = 1; i <= _numTokens; ++i) {
            ...
            _lastClaim[curTotalSupply + i] = block.timestamp;
        }
    ...
    function claim() public {
        require(isActive, "Claim not enabled yet!");
        require(balanceOf(_msgSender()) > 0, "You do not have any NFT!");
        uint256 dividend = _getNFTsByAddress(_msgSender());
        require(dividend > 0, "Claimable amount is not enough!");
        tAddress.transfer(_msgSender(), dividend *10**18);
    }

    function retrieveClaimabe(address _owner) public returns(uint256) {
        return _getNFTsByAddress(_owner);
    }

    function _getNFTsByAddress(address _owner) internal returns (uint256) {
        uint256 dividend = 0;
        for(uint256 i = 0; i < totalSupply + 1; i++) {
            if(_owners[i] == _owner) {
                dividend += _getNFTClaimable(i);
            }
        }
        return dividend;
    }

    function _getNFTClaimable(uint256 _tokenID) internal returns (uint256) {
        uint256 timePassed = block.timestamp - _lastClaim[_tokenID];
        uint256 dividend = 0;
        if(timePassed > 300) {
            dividend = (timePassed / 60 / 60) * tokensPerClaim;
        }
    ...
    function settAddress(address _tAddress) public onlyOwner {
        tAddress = IERC20(_tAddress);
    }
