...
struct TokenInfo {
    ...
    mapping(string => TokenInfo) internal allowedCrypto;
...
*/
    ...
    function addCurrency(
        ...
        //string converted = _symbol;
        allowedCrypto[_symbol]=
            TokenInfo({
                paytoken: _paytoken,
                costvalue: _costvalue
            });
...
function getCrypto(string memory str) public view returns (TokenInfo memory) {
    return allowedCrypto[str];
}
    /**
    ...
    function mintTo(address _to, uint256 _count) public onlyOwner {
    ...
    function mint() external payable {
        ...
        if (allowList[msg.sender] > 0) {
            require(msg.value >= discountFee && msg.value < mintingFee, "Incorrect payment amount.");            
            allowList[msg.sender] -= 1;
            uint256 currentTokenId = _nextTokenId.current();
            _nextTokenId.increment();
            _mint(msg.sender, currentTokenId);
        } else {
            require(msg.value >= mintingFee * (1 + (totalSupply() / nftsPerTier)), "Insufficient funds to mint");
            uint256 currentTokenId = _nextTokenId.current();
            _nextTokenId.increment();
            _mint(msg.sender, currentTokenId);
        }
...
function mintWithErc20(uint256 _count, string memory _symbol) public payable {
        //TokenInfo storage tokens = AllowedCrypto[_pid];
        TokenInfo storage tokens = allowedCrypto[_symbol];
        IERC20 paytoken;
        ...
        require(cost > 0, "Minting with ERC20 token is Not Enabled. cost must be greater than zero.");
        require(_count > 0, "_count must be greater than zero.");
        require(_count < maxMintSize+1, "_count must be greater than maxMintSize.");
        require(totalSupply() + _count < maxSupply + 1, "Cannot Mint that many NFTs. Maximum supply reached");
        

        uint256 finalPrice = _count * (cost * (1 + (totalSupply() / nftsPerTier)));
        ...
        //require(paytoken.balanceOf(address(msg.sender)) >= finalPrice, "ERC20 Balance is not sufficient to mint this number of NFTs.");
        require(paytoken.balanceOf(msg.sender) >= finalPrice, "ERC20 Balance is not sufficient to mint this number of NFTs.");
 

        paytoken.transferFrom(msg.sender, address(this), finalPrice);
    ...
    //function withdrawTokens(uint256 tokenId) public payable onlyOwner() {
    function withdrawTokens(string memory _symbol) public payable onlyOwner() {
...
contract ChaosCube is ERC721ChaosCoinClub {
    ...
    string public _baseTokenURI = "ipfs://bafybeiguuxo53db7fnxutz5mrtd2y4ncuxsl7yh757pou2v6mfg7ga6sdy/";


    constructor(
    // mainnet
    // address(0xa5409ec958c83c3f309868babaca7c86dcb077c1), address(0x000000000000ad05ccc4f10045630fb830b95127)
)
    ERC721ChaosCoinClub("Chaos Cube", "CHAOS", address(0x36E569119af6735a6Fe4B0F9e9B8A250D7775275), address(0x36E569119af6735a6Fe4B0F9e9B8A250D7775275), 
25000000000000000, 5000000000000000, 1000, 10, 4, 20, 100){}

    
    function baseTokenURI() public override view returns (string memory) {
        //return "ipfs://bafybeiguuxo53db7fnxutz5mrtd2y4ncuxsl7yh757pou2v6mfg7ga6sdy/"; //_baseTokenURI;
    ...
    function contractURI() public view returns (string memory) {
        //return "https://nftstorage.link/ipfs/bafybeifugmip2h5do55basyzgieftmsgnju246gfqctdiugvwn5jkc45am/chaoscube"; //_contractURI;
        return _contractURI;
    }
