...
import {IEntityDeployer} from "../interfaces/IEntityDeployer.sol";
import {EntityNFT} from "../core/EntityNFT.sol";
import {Types} from  "../lib/Types.sol";

contract EntityDeployer is IEntityDeployer {
    Types.EntityDeployerParams public override entityParams;
    ...
    function deployEntity(
        ...
        entityParams.assetBeacon = assetBeacon;
        entityParams.commodityBeacon = commodityBeacon;
        addr = address(new EntityNFT{ salt: salt }());
        delete entityParams;   
    }
...
contract EntityNFT is ERC721ABase, EntityNFTStorage, UUPSUpgradeable, IEntity, IUpgradeable {
    ...
    function registerAsset(Types.RegisterNFTParams memory params, bytes calldata data) external onlyRole(GOVERNOR_ROLE) {
        ...
        (uint256 id, address assetNft) = AssetFeature.registerAsset(AssetBeacon, params, data, entitiesById, assetByIdByEntityId);
        // todo, add middleware
    ...
    function collectCommodity(
       ...
       AssetFeature.collectCommodity(params, data, commodityByIdByEntityId);                    
    }
    ...
    // todo
    // function registerCommodity(Types.RegisterNFTParams memory params, bytes calldata data) external {
    //     require(bytes(params.name).length != 0, "EMPTY_NAME");
    //     require(params.entityId != 0, "EMPTY_ENTITY");
    //     (uint256 id, address commodityNft) = AssetFeature.registerCommodity(CommodityBeacon, params, data, entitiesById, commodityByIdByEntityId);
    //     //todo, add middleware
    //     emit RegisterCommodityEvent(params.entityId, id, commodityNft, params.owner);
    // }
    ...
    function fetchCommodity(uint256 entityId, uint256 id) external view returns (Types.Commodity memory) {
        return commodityByIdByEntityId[entityId][id];
    }
    ...
    function ownerOf(uint256 tokenId) public view virtual override(IEntity, IERC721AUpgradeable, ERC721AUpgradeable) returns (address) {
        return ERC721AUpgradeable(this).ownerOf(tokenId);
    }
...
library AssetFeature {
    ...
    function _deployCommodityNFT(
        address beacon,
        uint256 entityId, 
        uint256 commodityId,
        Types.RegisterNFTParams memory nftParams,
        mapping(uint256 => mapping(uint256 => Types.Commodity)) storage entityCommodityMapping) private returns(address) {
        
        bytes memory initData = abi.encodeWithSelector(
            CommodityNFT.initialize.selector,
            nftParams
        );
        console.log("deployed commodity factory at %s", beacon);
        address commodityNFT = address(
            new BeaconProxy{ salt: bytes32(entityId)}(beacon, initData)
        );
        entityCommodityMapping[entityId][commodityId].commodityNFT = address(0);
        emit DeployedCommodityEvent(entityId, commodityId, address(0));
        return commodityNFT;
    }
    ...
    function collect(
        ...
        // uint256 entityFee = ITreasury(treasuryAddr).entityFee(params.entityId);
        // uint256 mercuryFee = ITreasury(treasuryAddr).fetchFeeByNamespace(namespace);
        // if (entityFee > 0) {
        //     require(ITreasury(treasuryAddr).isCurrencyAccepted(namespace, currency), "CURRENCY_NOT_ACCEPTED_BY_TRESURY");
        //     uint256 fee = 0;
        //     if (mercuryFee > 0) {
        //         fee = (msg.value * entityFee * mercuryFee) / (Constant._MAX_BPS * Constant._MAX_BPS); 
        //         IERC20(currency).transferFrom(
        //             msg.sender,
        //             ITreasury(treasuryAddr).fetchTreasuryByNamespace(namespace),
        //             fee
        //         );    
        //     }
        //     IERC20(currency).transferFrom(
        //         msg.sender,
        //         ITreasury(treasuryAddr).entityTreasury(params.entityId),
        //         (msg.value * entityFee) / Constant._MAX_BPS - fee
        //     ); 
        // }
     ...
     function registerCommodity(
        ...
        bytes memory data, 
        mapping(uint256 => Types.Entity) storage entitiesById,
        mapping(uint256 => mapping(uint256 => Types.Commodity)) storage entityCommodityMapping) internal returns(uint256, address) {        
        uint256 id = ++entitiesById[nftParams.entityId].totalCommodities;
        entityCommodityMapping[nftParams.entityId][id].name = nftParams.name;
        entityCommodityMapping[nftParams.entityId][id].symbol = nftParams.symbol;
        entityCommodityMapping[nftParams.entityId][id].owner = nftParams.owner;
        entityCommodityMapping[nftParams.entityId][id].middleware = nftParams.nftMw;
        if (nftParams.nftMw != address(0)) {
            // config the metadata
            IMintingMiddleware(nftParams.nftMw).setMwData(nftParams.entityId, id, data);
        }
        address commodity = _deployCommodityNFT(beacon, nftParams.entityId, id, nftParams, entityCommodityMapping);
        return (id, commodity);
    }
    ...
    function collectCommodity(
                    Types.CollectNFTParams memory params,
                    bytes calldata data, 
                    mapping(uint256 => mapping(uint256 => Types.Commodity)) storage entityCommodityMapping) internal {
        require(entityCommodityMapping[params.entityId][params.nftId].commodityNFT != address(0), "NO_COMMODITY_AVAILABLE");
        address mw = entityCommodityMapping[params.entityId][params.nftId].middleware;
        if (mw != address(0) ) {
            IMintingMiddleware(mw).preProcess{value: msg.value}(params, data);
        }
        ICommodity(entityCommodityMapping[params.entityId][params.nftId].commodityNFT).mint(params.to, params.nftId, params.quantity, "");
        if (mw != address(0) ) {
            IMintingMiddleware(mw).postProcess(params, data);
        }
        emit CollectedCommodityEvent(params.entityId, params.nftId, params.to);
    }
}
