...
contract Market {
    ...
    function mint(
        ...
        if (_isERC721(contractAddress)) {
            ...
            // );
            checkValidValue(tokenId);
            checkValidValue(quantity);

            IMarket1155(contractAddress).mint(msg.sender, tokenId, quantity);
        } else {
            notASupportInterface();
        }
    }
    ...
    function buy(
        ...
        if (_isERC721(contractAddress)) {
            ...
            checkMsgValue(saleDetail.price);
            _isApproved721(tokenId, contractAddress);
            ...
            // );
            checkMsgValue(saleDetail.price * quantity);
            _ERC1155BalanceApprov(
                saleDetail.ownerAddress,
                tokenId,
                quantity,
    ...
    function checkValidValue(uint256 value) private pure {
        require(value > 0, "Market: invalid value");
    }

    function notASupportInterface() private pure {
        revert("Market: support interface not allowed");
    }

    function checkMsgValue(uint256 price) private view {
        require(msg.value == price, "Market: invalid Amount");
    }
}
