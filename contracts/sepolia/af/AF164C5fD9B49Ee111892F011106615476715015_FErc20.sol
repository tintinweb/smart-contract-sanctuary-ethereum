...
interface IRiskManagerNew {
    function furionLeverage() external view returns (address);
}
...
contract FErc20 is TokenBase, FErc20Storage, IFErc20 {
    ...
    modifier onlyLeverage() {
        require(
            msg.sender ==
                IRiskManagerNew(address(riskManager)).furionLeverage(),
            "only leverage"
        );
        _;
    }
    ...
    ) external onlyLeverage returns (uint256) {
        // Params: supplier, supply amount
        return delegateSupplyInternal(_user, msg.sender, _supplyAmount);
    }

    // fWETH
    function supplyWithETH() external payable {
        toWETH(msg.value, msg.sender);

        // Params: supplier, supply amount
        supplyInternal(msg.sender, msg.value);
    }

    function delegateSupplyWithETH(address _user) external payable onlyLeverage returns (uint256) {
        toWETH(msg.value, msg.sender);

        // Params: supplier, supply amount
        return delegateSupplyInternal(_user, msg.sender, msg.value);
    }
    ...
    function delegateRedeem(
        ...
        // Params: redeemer, tokens supplied for redemption, amount of underlying to receive
        return delegateRedeemInternal(_user, msg.sender, _redeemTokens, 0);
    }
    ...
    function delegateRedeemWithAmount(
        ...
        // Params: redeemer, tokens supplied for redemption, amount of underlying to receive
        return delegateRedeemInternal(_user, msg.sender, 0, _redeemAmount);
    }
    ...
    ) external onlyLeverage {
        // Params: borrower, borrow amount
        delegateBorrowInternal(_user, msg.sender, _borrowAmount);
    }
    ...
    function delegateRepayBorrow(
        ...
        delegateRepayBorrowInternal(_user, _repayAmount);
    }

    // fWETH
    function repayBorrowWithETH() external payable {
        toWETH(msg.value, msg.sender);
    ...
    function delegateRepayBorrowWithETH(
        address _user
    ) external payable onlyLeverage {
        toWETH(msg.value, msg.sender);

        delegateRepayBorrowInternal(_user, msg.value);
    }
    ...
    // fWETH
    function repayBorrowBehalfWithETH(address _borrower) external payable {
        toWETH(msg.value, msg.sender);
    ...
    function delegateLiquidateBorrow(
        ...
        delegateLiquidateBorrowInternal(
            _liquidator,
            _borrower,
            _repayAmount,
            _fTokenCollateral
    ...
    // fWETH
    function delegateLiquidateBorrowWithETH(
        address _liquidator,
        address _borrower,
        address _fTokenCollateral
    ) external payable onlyLeverage {
    ...
    function liquidateBorrowWithETH(
        address _borrower,
        address _fTokenCollateral
    ) external payable {
        toWETH(msg.value, msg.sender);
    ...
    function toWETH(uint256 _amount, address _to) internal {
        IWETH(underlying).deposit{value: _amount}();
        IERC20(underlying).transfer(_to, _amount);
    }
