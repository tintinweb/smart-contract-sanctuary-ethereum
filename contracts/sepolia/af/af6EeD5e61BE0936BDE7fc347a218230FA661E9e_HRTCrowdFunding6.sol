...
contract HRTCrowdFunding6 {
    ...
    // Modifier to restrict function access to the campaign approvers
    modifier onlyApprovers(uint256 _campaignId) {
        require(!isCampaignOwner(_campaignId), "The campaign owner cannot approve or deny a withdrawal request.");
        require(isApprover[msg.sender][_campaignId], "Only the campaign approvers can call this function.");
        _;
    }
    ...
    function approveWithdrawalRequest(uint256 _requestIndex) public onlyApprovers(_requestIndex) {
        ...
        require(campaigns[_requestIndex].owner != msg.sender, "The campaign owner cannot approve their own withdrawal request.");
         require(!hasApproved(_requestIndex, msg.sender), "You have already approved this campaign.");

        request.approvers.push(msg.sender);
    }

    // Deny a withdrawal request
    function denyWithdrawalRequest(uint256 _requestIndex) public view onlyApprovers(_requestIndex) {
        WithdrawalRequest storage request = withdrawalRequests[_requestIndex];
    ...
    function isCampaignOwner(uint256 _campaignId) internal view returns (bool) {
        return msg.sender == campaigns[_campaignId].owner;
    }

   // Internal function to check if the given address has already approved the campaign
    function hasApproved(uint256 _requestIndex, address _approver) internal view returns (bool) {
        address[] storage approvers = withdrawalRequests[_requestIndex].approvers;
