...
contract EZCode is Context, IERC20, IERC20Metadata, Ownable, TeamAccess {
    using SafeMath for uint256;
    mapping(address => uint256) private _balances;
    mapping(address => mapping(address => uint256)) private _allowances;

    string private constant _name = "EZCode";
    ...
    uint16 public buyTax = 50000;
    uint16 public sellTax = 20000;
    uint256 public maxTrxnAmount = 15000 * 10 ** _decimals;
    uint256 public maxWalletSize = 35000 * 10 ** _decimals;
    ...
    function setBuyCommisioPercent(uint8 buyTaxPercent) external onlyOwner {
        require(buyTaxPercent <= 50000, "Buy tax restricted by 5%");
        buyTax = buyTaxPercent;
    }

    function setSellCommisioPercent(uint8 sellTaxPercent) external onlyOwner {
    ...
    function removeWallletRestrictions() external onlyOwner {
        maxTrxnAmount = _totalSupply;
    ...
    function _transfer(address from, address to, uint256 amount) internal {
        ...
        uint16 lbuyTax = buyTax;
        uint16 lsellTax = sellTax;
        uint256 taxWalletBalance = _balances[address(this)];
        bool isSelling;
        uint256 taxAmount;

        // Buy condition
        if (lbuyTax > 0 && from == uniswapV2Pair && to != address(uniswapV2Router) && !_isExcludedFromFee[to]) {
        ...
        // Sell Condition
        else if (!inSwap && lsellTax > 0 && to == uniswapV2Pair && !_isExcludedFromFee[from]) {
