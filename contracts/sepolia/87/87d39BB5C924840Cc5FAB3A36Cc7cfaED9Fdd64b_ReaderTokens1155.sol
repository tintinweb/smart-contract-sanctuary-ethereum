  ...
  UpdatableOperatorFiltererUpgradeable
{
  ...
  function initialize() public initializer {
    ...
    s_tokenIdToPrice[AGE_OF_VALOR_01_ARTIST_EDITION_ID] = 0.00788 ether;
    s_tokenIdToPrice[AGE_OF_VALOR_01_ID] = 0.00688 ether;
  }
  ...
  /**
   * @notice mints Age of Valor tokens
   * @param quantity number of tokens to mint
   * @dev must enable minting for Age of Valor and payment must be sufficient
   */
  function mintAgeOfValor(uint256 quantity) external payable nonReentrant {
    if (!s_tokenIdToMintEnabled[AGE_OF_VALOR_01_ID]) {
      revert ReaderTokens__NotEnabled();
    }
    if (msg.value < s_tokenIdToPrice[AGE_OF_VALOR_01_ID] * quantity) {
      revert ReaderTokens__PaymentInsufficient();
    }
    _mint(msg.sender, AGE_OF_VALOR_01_ID, quantity, "");
    emit MintedToken(AGE_OF_VALOR_01_ID, quantity, msg.sender);
  }
