...
abstract contract ERC20Spendable is ERC20, SpenderRole, Ownable {
  ...
  function spend(
    ...
    returns (bool)
  {
    require(from != address(0), "ERC20: cannot spend from the zero address");
    require(value > 0, "Token: value must be greater than zero");
    require(balanceOf(address(from)) >= value, "ERC20 Balance is too low to spend.");
...
contract ChaosCoin is ERC20, PullPayment, ERC20Spendable {
    ...
    constructor() ERC20("Chaos Coin", "CHAOSCOIN") {
        ...
        _mint(msg.sender, 1000000000000000000*unitsOneWeiCanBuy);
    }
    ...
    receive() external payable {        
        ...
        if (msg.value > tier5-1)
        {
            chaosAmount = (chaosAmount *14)/10;
        }
        else if (msg.value > tier4-1)
        {
            chaosAmount = (chaosAmount *13)/10;
        }
        else if (msg.value > tier3-1)
        {
            chaosAmount = (chaosAmount *12)/10;
        }
        else if (msg.value > tier2-1)
        {
            chaosAmount = (chaosAmount *11)/10;
        }
        else if (msg.value > tier1-1)
        {
