...
library LibTraitManager {
  ...
  function selectRandomTrait(
    ...
    assembly {
        ...
        for {} lt(rnd, sload(add(traits.slot, add(0x20, mul(0x40, traitIndex))))) {} {
            rnd := sub(rnd, sload(add(traits.slot, add(0x20, mul(0x40, traitIndex)))))
            traitIndex := add(traitIndex, 1)

            // Check if the loop has reached 2^64 iterations
            if iszero(lt(traitIndex, 0x10000000000000000)) {
                // Exit the loop and return 0 if the limit is reached
                traitIndex := 0
                rnd := 0
                break
            }
        ...
        // Continue from the last traitIndex if the previous loop reached the limit
        for {} lt(rnd, sload(add(traits.slot, add(0x20, mul(0x40, traitIndex))))) {} {
            rnd := sub(rnd, sload(add(traits.slot, add(0x20, mul(0x40, traitIndex)))))
            traitIndex := add(traitIndex, 1)
        }
