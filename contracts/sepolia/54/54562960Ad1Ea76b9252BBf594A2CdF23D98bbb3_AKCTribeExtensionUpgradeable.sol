 ...
 * Allows for a cheaper Batch Unstaking in cases
 * where a User has a large number of tribes
 */
contract AKCTribeExtensionUpgradeable is OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    event BatchUnstakeAKC(address indexed receiver, uint256[] indexed akcIds, uint256[] indexed specs);
    ...
    * @param specs List of their respective specs
    *
    * @notice This function handles the unstaking of multiple Kongs with lower gas fees, 
    * in the cases where a user has a large number of tribes.
    *
    * @custom:warning If a user is planning to also Claim his rewards, he should do so before 
    * calling this function, as it would result in a lower reward amount since 
    * the reward amount is not updated by this function, for gas saving purposes.
    */
    function batchUnstakeNew(uint256[] calldata akcIds, uint256[] calldata specs) external nonReentrant {
        require(akcIds.length > 1, "Batch stake must be more than one kong");
        require(akcIds.length == specs.length, "Kongs must match specs");
        ...
        for (uint256 i = 0; i < akcIds.length; i++) {
            ...
            //TODO delete this statement? see if is necessary
            // hasDisabledSpec[msg.sender][spec] = true;

            require(!hasUnstakedKong[msg.sender][akcId], "User has already unstaked this kong");
            hasUnstakedKong[msg.sender][akcId] = true;
        }
        emit BatchUnstakeAKC(msg.sender, akcIds, specs);
    }

    //TODO verify comment. If rewards are disabled this function can be used anytime with no repercussions
    /*
    * @param akcIds List of AKC Kong IDs 
    *
    * @notice This function withdraws Stuck Kongs from AKCMultiStakeExtension's
    * contract in a cheaper fashion. Should be used in cases where BatchUnstaking
    ...
    */
    function withdrawStuckKongs(uint256[] calldata akcIds) external nonReentrant {
        ...
        //todo fix this event if this function stays
        // emit BatchUnstakeAKC(msg.sender, akcIds);
    }
    ...
    function claimAllRewardsOfUser() external nonReentrant {
        uint256 amount = 0;
        if(!hasDisabledRewards[msg.sender]) {
            amount = getTotalClaimableReward(msg.sender);

            /* if (taxReward > 0) */
                    /*     amount += taxExtension.claimTaxShare(msg.sender, taxProof, taxReward, taxNonce); */
    ...
    function getTotalClaimableReward(address user) public view returns (uint256 amount) {
        ...
        if(!hasDisabledRewards[user]){
            ...
            // add blue banana serum capsule reward
            /* amount += blueBananaSerumStake.getTotalPendingRewardOfUser(user); */

                // rarity staking
                //TODO ligar rarity staking?
                // amount += rarityStaking.getTotalLiquidatableReward(user);
    ...
    //TODO keep these functions?
    function getKongBatchStakeSpec(uint256[] memory akcIds) public view returns (uint256[] memory specs) {
        uint256 length = akcIds.length;
        specs = new uint256[](length);
        for (uint256 i = 0; i < length; i++) {
            specs[i] = getKongStakeSpec(akcIds[i]);
        }
        return specs;
    }

    //todo keep this also?
    function getKongStakeSpec(uint256 akcId) public view returns (uint256) {
        uint256 kongStakeData = akcCoreMultiStakeExtension.kongToStaker(akcId);
...
contract IAKCCoreMultiStakeExtension {
    ...
    function getNakedRewardBySpec(address staker, uint256 targetSpec, uint256 timestamp) external view returns (uint256) {}
...
contract IAKCTribeManagerUpgradeable {
    /** STAKING */

    function userToDebt(address) public view returns (uint256) {}
}