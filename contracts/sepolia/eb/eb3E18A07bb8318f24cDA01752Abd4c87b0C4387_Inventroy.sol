...
library Types {
   ...
   enum StakeHolder {      //currently we have only 2 stakeholder so, that's why I'm using  
    ...
    // struct users {
    //     mapping(string => StakeHolder[]) manufacturerArea;  //country mapped with manufacturer
    //     mapping(string => StakeHolder[]) distributorArea;   //distributor mapped with coutry
    //     mapping(address => StakeHolder[]) distRegisteredUnderManuf;
    //     mapping(string => StakeHolder[]) retailerArea;   //retailer mapped with city
    //     mapping(address => StakeHolder[]) retRegisteredUnderDist;
    // }
...
contract Inventroy {
    ...
    mapping(address => Types.Item[]) internal producerInventor;  //public to internal
    ...
    mapping(address => mapping(bytes32 => Types.manfItem)) internal manufacturedProduct;
    ...
    function addRawMaterial(
        ...
        // require(Types.StakeHolder.Producer ==, "Only Producer Can Add Raw Materials!!");
        // require(Types.Stakeholder.IsRegistered == true, "stakeholder not registered!");
        // revert("only producer can register raw materials!!");
        require(register.getStakeHolderDetails(msg.sender).role == Types.StakeHolder.Producer,
        ...
        if(_pidHash == rawMaterials[msg.sender][_pidHash].PId){
            updateRawMaterial(_pidHash, _quantity+=_quantity, _availableDate, _expiryDate, _price);
        }
        else  {           
    ...
    function manufProductDetail(address _manufacturerAdd, bytes32 _manfProductID)
        external
        view
        returns (Types.manfItem memory)
    {
