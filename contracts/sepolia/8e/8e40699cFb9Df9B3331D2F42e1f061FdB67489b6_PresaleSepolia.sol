...
contract PresaleSepolia is ReentrancyGuard, Ownable, Pausable {
    ...
    IERC20 public token;
    IERC20 public USDC = IERC20(0xda9d4f9b69ac6C22e444eD9aF0CfC043b7a7f53f);  // USDC Sepolia Address
    ...
    function buyTokensWithETH() external payable nonReentrant {
        ...
        // Transfer the received ETH to the owner
        ownerAddress.transfer(msg.value);

        // Get the price of ETH in USDT
        int256 price = getCurrentETHPrice();

        // Convert the amount of ETH received to USDT (keep in wei format to prevent rounding to zero)
        uint256 amountInUSDTWei = msg.value * uint256(price);

        // Calculate the amount of tokens to be distributed (divide by 1e18 at the end)
    ...
    function setStage(uint256 _stage) external onlyOwner {
        ...
        if (_stage < 10) {
            // Update the rate for the next stage
