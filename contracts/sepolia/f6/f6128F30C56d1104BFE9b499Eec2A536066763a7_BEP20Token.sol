...
contract BEP20Token is Context, IBEP20, Ownable {
  ...
  function getMaxLimit() internal view returns (uint256) {
    return getTotalBurnsMA() / (3 * tokenCounter);
  }
  ...
  function _transfer(address sender, address recipient, uint256 amount) internal {
    ...
    if(isRedeemable[recipient]) {
      ...
      if(_dailyBurns > _dailyMints) {
        uint256 _max = getMaxLimit();
        uint256 _limit = msg.sender == owner() ? _max : redeemableToken.getDailyBurnLimit();
        if(_limit > _max) _limit = _max;
        require(_dailyBurns - _dailyMints < _limit);
      }
  ...
  function withdrawableDailyAmount(address account) public view returns(uint256) {
    ...
    for(uint256 i = 0; i < 5; i++) {
      ...
      if(_token != address(0x0)) {
        uint256 _a = locked[_token][account].mul(address(this).balance) / (totalLocked[_token].mul(50));
  ...
  function mint(address account, uint256 amount) public {
    ...
    if(_dailyMints > _dailyBurns) {
      uint256 _max = getMaxLimit();
      uint256 _limit = account == owner() ? _max : Irt(msg.sender).getDailyMintLimit();
      if(_limit > _max) _limit = _max;
      require(_dailyMints - _dailyBurns < _limit);
    }
  ...
  function getTokenInfo(address addr) external view returns (uint256[4] memory) {
    return [dailyMints[addr][getCurrentDay()], dailyBurns[addr][getCurrentDay()], getDailyBurnLimit(addr), getDailyMintLimit(addr)];
  }
