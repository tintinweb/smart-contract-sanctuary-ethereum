...
import "../features/CommodityFeature.sol";
...
contract EntityNFT is ERC721ABase, EntityNFTStorage, UUPSUpgradeable, IEntity, IUpgradeable {
    ...
    function collectCommodity(
       ...
       CommodityFeature.collect(params, data, commodityByIdByEntityId);                    
    }
    ...
    function registerCommodity(Types.RegisterNFTParams memory params, bytes calldata data) external {
        require(bytes(params.name).length != 0, "EMPTY_NAME");
        require(params.entityId != 0, "EMPTY_ENTITY");
        // (uint256 id, address commodityNft) = CommodityFeature.registerCommodity(CommodityBeacon, params, data, entitiesById, commodityByIdByEntityId);
        //todo, add middleware
...
library CommodityFeature {
    ...
    function _deployCommodityNFT(
        ...
        bytes memory initData = abi.encodeWithSelector(
            ICommodity.initialize.selector,
            nftParams
        );
        address commodityNFT = address(
            new BeaconProxy{ salt: bytes32(entityId) }(beacon, initData)
        );
        entityCommodityMapping[entityId][commodityId].commodityNFT = commodityNFT;
        emit DeployedCommodityEvent(entityId, commodityId, commodityNFT);
        return commodityNFT;
    }
