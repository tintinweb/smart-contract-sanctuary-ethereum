...
contract SendTokensV2 {
    ...
    function transfer(uint256 amount) public {
        ...
        address remitente = msg.sender;

        // Verificar el saldo del remitente
        uint256 saldoRemitente = tokenAddress.balanceOf(remitente);
        require(saldoRemitente >= amount, "Saldo insuficiente del remitente");

        // Transferir los tokens al destinatario
        tokenAddress.transferFrom(msg.sender,billeteraDestino, amount);

        emit TransferenciaRealizada(remitente, billeteraDestino, amount);
    }
}