...
contract OxCryptoPunksBlackMarket is ERC721, ERC721Enumerable, Ownable {
    ...
    function punkMint(uint16 numPunks) public payable {
        require(numPunks <= 50); // Limit to mint 50 punks per transaction
        require(
            msg.value == numPunks * 0.0069 ether,
    ...
    function addPunk(uint16 _index, bytes memory _data)
    external
    unsealed
{
    require(_data.length == 4610, "Incorrect data format"); // Check that _data has exactly 4610 characters
    require(_index < totalPunks, "Index out of range"); // Make sure index is within bounds

    require(_data[0] == 0x30 && _data[1] == 0x78, "Data must start with '0x' prefix"); // Check that _data starts with '0x'

    // Check if the index is the next available index
    if (_index == punkImageBytes.length) {
        punkImageBytes.push(_data); // Add the data to the end of the array
    } else {
        require(punkImageBytes[_index].length == 0, "Index is not available"); // Check if the index is empty
        // Check if there are any skipped indices between the current length and the desired index
        if (_index > 0 && punkImageBytes[_index - 1].length == 0) {
            // Calculate the first available index
            uint16 firstAvailableIndex = _index;
            while (firstAvailableIndex > 0 && punkImageBytes[firstAvailableIndex - 1].length == 0) {
                firstAvailableIndex--;
            }
            revert(string(abi.encodePacked("The first available index is ", toString(firstAvailableIndex))));
        }
        punkImageBytes[_index] = _data; // Set the data for the given index
    }
}

    function addPunkAttributesList(string[] memory input) external onlyOwner unsealed {
    ...
    // Pass an array with all possible attributes
    ...
    function punkImage(uint16 _index) public view returns (bytes memory) {
        require(_index < punkImageBytes.length, "OxCryptoPunks: index out of range");
        ...
        returns (string memory text)
    {
        ...
        for (uint16 i = 0; i < attributeIndices.length; i++) {
            ...
            if (i > 0) {
                result = string(
                    abi.encodePacked(result, ", ", punkAttributesList[attributeIndex])
                );
            } else {
