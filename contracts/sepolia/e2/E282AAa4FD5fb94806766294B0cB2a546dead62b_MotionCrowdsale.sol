    ...
    OwnableUpgradeable
{
        ...
        returns (bool)
    {
        ...
        if (_presalesAccounts[_msgSender()] == address(0)) {
            _presalesAccounts[_msgSender()] = address(
                new MotionVesting(_msgSender(), _closingTime - 1 weeks)
            );
    ...
    function addManyToWhitelist(
        ...
        for (uint256 i = 0; i < beneficiaries.length; i++) {
            _whitelist[beneficiaries[i]] = true;
        }
    }

    /**
     * @dev Removes single address from whitelist.
     * @param beneficiary Address to be removed to the whitelist
     */
    ...
    function _finalization() internal {
        if (goalReached()) {
            ...
            tokenPartnerTimeLock.initialize(
                _token,
                _partnersFund,
                _releaseTime
            );
    ...
    ReentrancyGuardUpgradeable
{
    ...
    function updateTax(uint256 tax) external onlyRole(DEFAULT_ADMIN_ROLE) {
        require(tax < 10, "Tax should not be more than 10%");
        if (_tax != tax) {
            _tax = tax;
            emit TaxUpdated(tax);
        }
