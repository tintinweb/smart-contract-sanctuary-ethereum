    ...
    EIP712Upgradeable
{
    ...
    function swapToken(
        // 0:fromTokenId, 1:toTokenId, 2:salt
        uint256[] memory tokenParams,
        // 0:fromTokenAddress, 1:toTokenAddress, 2 toUser
        address[] memory addressParams
    ) public whenNotPaused returns (uint256[] memory) {
        require(tokenParams[0] > 0, "swapToken: fromTokenId to the zero");
        require(
            addressParams[0] != address(0),
            "swapToken: fromTokenAddress to the zero address"
        );
        require(tokenParams[1] > 0, "swapToken: toTokenId to the zero");
        require(
            addressParams[1] != address(0),
            "swapToken: toTokenAddress to the zero address"
        );
        require(
            addressParams[2] != address(0),
            "swapToken: toUser to the zero address"
        );
        GenericNft _toTokenGeneric = GenericNft(addressParams[1]);
        _toTokenGeneric.safeTransferFrom(
            addressParams[2],
            _msgSender(),
            tokenParams[1]
        );
        GenericNft _fromTokenGeneric = GenericNft(addressParams[0]);
        _fromTokenGeneric.safeTransferFrom(
            _msgSender(),
            addressParams[2],
            tokenParams[0]
        );
