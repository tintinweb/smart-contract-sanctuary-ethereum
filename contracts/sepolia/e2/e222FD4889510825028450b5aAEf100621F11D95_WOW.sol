    ...
    UUPSUpgradeable
{
    ...
    mapping(uint256 => address) private coupons;
    mapping(uint256 => bool) private isInserted;
    uint256[] private couponKeys;
    ...
    modifier checksBeforeMintUSDC(uint256 quantity, uint256 _coupon) {
        ...
        if (_coupon > 0) {
            require(
                isInserted[_coupon] && coupons[_coupon] == address(0),
                "Coupon is invalid or already used"
            );
        }
        _;
    }
    ...
    modifier isBenefactor() {
        require(
            ...
            "Only benefactor is authorize to perform this transaction"
        );
        _;
    }

    modifier isBenefactorOrOwner() {
        require(
            benefactor == msg.sender || this.owner() == msg.sender,
            "Only benefactor or Owner is authorize to perform this transaction"
        );
        _;
    }
        ...
        returns (uint256 number)
    {
        ...
        if (!isInserted[randomNumber]) {
            coupons[randomNumber] = address(0);
            isInserted[randomNumber] = true;
            couponKeys.push(randomNumber);
            return randomNumber;
        } else {
            return 0;
        }
    ...
    function checkCoupan(uint256 _coupon)
        public
        view
        isBenefactorOrOwner
        returns (bool, address)
    {
        if (isInserted[_coupon]) {
            return (true, coupons[_coupon]);
        } else {
    ...
    function addRoute(address _address) public isBenefactor {
        benefactor = _address;
    } //3
    ...
    function addClaimNFTsAddressesData(
        address[] memory addresses,
        uint256[] memory amount
    ) public onlyOwner {
        require(
            addresses.length == amount.length,
            "Length of arrays doesnot match"
        );
    ...
    function getClaimNFTsAddressesData(address _address)
        public
        view
        onlyOwner
        returns (uint256)
    ...
    function mintNFTsForClaimedAddresses() public {
        require(
            ...
            "Only mint for claimed addresses"
        );
        require(
    ...
    function mintNFTwithUSDC(
        ...
        bytes32 s,
        uint256 coupon
    ) public checksBeforeMintUSDC(quantity, coupon) {
        ...
        require(
            USDCAddress.transferFrom(msg.sender, benefactor, cost),
            "USDC transfer failed"
        );
        if (coupon > 0) {
            coupons[coupon] = msg.sender;
        }
    ...
    function withdrawUSDC() public payable isBenefactor {
        require(
            USDCAddress.transfer(
                msg.sender,
                USDCAddress.balanceOf(address(this))
            ),
            "USDC not transfered"
        );
    } //2
