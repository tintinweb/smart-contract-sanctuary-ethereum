...
import { LibIdentity, IdentityLayout } from "../libraries/LibIdentity.sol";
...
contract DiamondInit {
  function init(InitParams calldata params) external {
    ...
    IdentityLayout storage ids = LibIdentity.getStorage();
    ids.maxFields = params.maxIdentityFields;

    StorageLayout storage ts = LibERC721.getStorage();
    if (ts._currentIndex < LibERC721._startTokenId) ts._currentIndex = LibERC721._startTokenId;
    ts.name = params.collectionName;
    ts.symbol = params.collectionSymbol;
    ts.maxSupply = params.maxSupply;
    ts.creatorMaxSupply = params.creatorSupply;
