...
import "../../mojito/interfaces/IRoyaltyEngineForDutch.sol";
import "../../openzeppelin/utils/structs/EnumerableSet.sol";
import {IDutchAuction} from "../../mojito/interfaces/IDutchAuction.sol";

/**
 * @title DutchAuction, enabling collectors and curators to run their own auctions
 */
...
contract DutchAuction is ReentrancyGuard, AdminControl {
    ...
    struct latestSaleDetails {
        uint256 lastBoughtPrice;
        uint256 nextBuyToken;
        uint256 noOfBuyedTokens;
        bool isEnded;
    }

    event AuctionCreatedorUpdated(string auctionId, IDutchAuction.DutchAuctionList list,string);

    event Buyed(
        string auctionId,
        uint256 totalBuyTokens,
        uint256 Quantity1155,
        uint256 buyingPrice,
        uint256 tax,
        uint256 paidAmount, // add tax in paid amount
        address buyer,
        uint256[] nftIds
    );
    event auctionEnded(
        string auctionId,
        uint256 totalnftsaled,
        uint256 finalPrice,
        uint256[] nftIds
    );
    ...
    IRoyaltyEngineForDutch public royaltySupport;
    ...
    mapping(string => EnumerableSet.AddressSet) private buyerAddress; // questionable

    mapping(string => latestSaleDetails) private latestSaleInfo;

    mapping(string => mapping(address => BuyerDetails)) private buyerInfo; // need a function only by buyer    

    constructor(IRoyaltyEngineForDutch _royaltySupport, address dutchUtilityArg) {
    ...
    function createOrUpdateDutchAuction(
        ...
        if (!usedAuctionId[dutchAuctionID]) {
            ...
            require(
                ...
                "cant update after the auction start time"
            );
            listings[dutchAuctionID] = list;
    ...
    function buy(
        ...
        IDutchAuction.DutchAuctionList memory list = listings[dutchAuctionID];

        IDutchAuction.NftContractList memory nftData = list.collectionList;

        bytes32 leaf = keccak256(abi.encodePacked(msg.sender));
        if (nftData.blacklistedBuyers != bytes32(0)) {
            require(
                !MerkleProof.verify(
                    blacklistedProof,
                    nftData.blacklistedBuyers,
                    leaf
                ),
        ...
        if (list.paymentCurrency == address(0)) {
            require(msg.value == buyingAmount+tax, "insufficient ETH amount");
        }

        // Validation
        dutchUtility.__buyValidation(
            dutchAuctionID,
            buyingAmount,
            tax,
            totalBuyTokens,
        ...
        if (nftData.saleType == IDutchAuction.typeOfSale.single_tokens_multiple_Quantity) {

            latestSaleInfo[dutchAuctionID].noOfBuyedTokens += quantity1155;
        } else if (
            nftData.saleType == IDutchAuction.typeOfSale.multiple_tokens_with_Same_Quantity ||
            nftData.saleType == IDutchAuction.typeOfSale.erc_721_nft_type
        ) {
            // tokenQuantity update
            latestSaleInfo[dutchAuctionID].noOfBuyedTokens += totalBuyTokens;
        ...
        uint256 currentPrice = dutchUtility.getCurrentDutchPrice(dutchAuctionID);

        buyerInfo[dutchAuctionID][msg.sender].boughtPrice.push(currentPrice);
        buyerInfo[dutchAuctionID][msg.sender].tax.push(tax);

        buyerInfo[dutchAuctionID][msg.sender].tokenQuantity.push(
            nftData.saleType == IDutchAuction.typeOfSale.single_tokens_multiple_Quantity
                ? quantity1155
                : totalBuyTokens
        );

        buyerInfo[dutchAuctionID][msg.sender].totalCap += nftData.saleType ==
            IDutchAuction.typeOfSale.single_tokens_multiple_Quantity
            ? quantity1155
            : totalBuyTokens;

        latestSaleInfo[dutchAuctionID].lastBoughtPrice = currentPrice;

        if (latestSaleInfo[dutchAuctionID].nextBuyToken == 0) {
            latestSaleInfo[dutchAuctionID].nextBuyToken = nftData.startTokenId;
        }
        // Payment Transfer
        if (list.paymentCurrency != address(0)) {
            if (nftData.isRebate) {
                IERC20(list.paymentCurrency).safeTransferFrom(
                    msg.sender,
                    address(this),
                    buyingAmount+tax
                );
        ...
        // NFt Transfer or mint
        if (nftData.isInstantDeliver) {
            if (!nftData.isMint) {
                (
                    tokenIds,
                    latestSaleInfo[dutchAuctionID].nextBuyToken
                ) = __tokenTransaction(
                ...
                if (
                    latestSaleInfo[dutchAuctionID].nextBuyToken == 0 &&
                    nftData.saleType ==
                    IDutchAuction.typeOfSale.single_tokens_multiple_Quantity
                ) {
                    latestSaleInfo[dutchAuctionID].nextBuyToken = tokenIds[0];
                }
            }
        }
        emit Buyed(
    ...
    function endAuction(string memory dutchAuctionID) external {
        IDutchAuction.DutchAuctionList memory list = listings[dutchAuctionID];
        IDutchAuction.NftContractList memory nftData = list.collectionList;
        // checks for auctionId is created for auction
        ...
        require(
            !latestSaleInfo[dutchAuctionID].isEnded,
            "can't end a closed auction"
        );

        require(
            latestSaleInfo[dutchAuctionID].lastBoughtPrice != 0,
            "No bids in Auction");

        uint256 totalPrice = latestSaleInfo[dutchAuctionID].lastBoughtPrice *
            latestSaleInfo[dutchAuctionID].noOfBuyedTokens;

        if (nftData.isRebate) {
            if (list.paymentCurrency == address(0)) {
                require(
                    address(this).balance >= totalPrice,
                    "does not have required balance for settlement"
                );
            } else {
                require(
                    IERC20(list.paymentCurrency).balanceOf(address(this)) >=
                        totalPrice,
                    "does not have required balance for settlement"
                );
            }
           
            uint256 totalTax = dutchUtility.computeTax(dutchAuctionID);
        ...
        if (!nftData.isInstantDeliver) {
          ...
          tokenIds = __nftTokenTransfer(dutchAuctionID,latestSaleInfo[dutchAuctionID].noOfBuyedTokens);
        }
        
        latestSaleInfo[dutchAuctionID].isEnded = true;

        emit auctionEnded(
            dutchAuctionID,
            latestSaleInfo[dutchAuctionID].noOfBuyedTokens,
            latestSaleInfo[dutchAuctionID].lastBoughtPrice,
            tokenIds
        );
    }

    function __nftTokenTransfer(
        string memory dutchAuctionID,
        uint256 noOfBuyedTokens
        ...
        uint256[] memory totalTokenIds = new uint256[](noOfBuyedTokens);
        uint256[] memory tokenIds;

        for(uint256 i = 0; i < buyerAddress[dutchAuctionID].length();i++)
        {   
            
            address buyerAdddress = buyerAddress[dutchAuctionID].at(i);
            uint256 totalBuyToken = 0;
            uint256 count=0;

            
            for(uint256 j = 0; j < buyerInfo[dutchAuctionID][buyerAdddress].tokenQuantity.length; j++){
             totalBuyToken += buyerInfo[dutchAuctionID][buyerAdddress].tokenQuantity[j];
            }
    ...
    function rebate(
        ...
        // checks for auctionId is created for auction
        require(usedAuctionId[dutchAuctionID], "unsupported auction");
        // Checks for walletAddress
        require(
            buyerAddress[dutchAuctionID].contains(walletAddress),
            "invalid Wallet address"
        );
        require(
            listings[dutchAuctionID].auctionEndTime <= uint64(block.timestamp),
            "the auction has not yet ended"
        );
        require(
            !buyerInfo[dutchAuctionID][walletAddress].isClaimed,
            "already Claimed"
        );
        ...
        (totalAmount,totaltax) = dutchUtility.claimable(dutchAuctionID, walletAddress);

        require(totalAmount > 0, "nothing to Claim");

        buyerInfo[dutchAuctionID][walletAddress].isClaimed = true;
    ...
    function paymentTransaction(
        ...
        IDutchAuction.SettlementList memory settlement = listings[_saleId].payoutList;
        ...
        if(royaltySupport!=IRoyaltyEngineForDutch(address(0))){
            (
            address payable[] memory recipients,
            uint256[] memory bps // Royalty amount denominated in basis points
        ) = royaltySupport.getCollectionRoyalty(listings[_saleId].collectionList.nftContractAddress);
        ...
        for (uint256 i = 0; i < recipients.length; ) {
            ...
            _handlePayment(msg.sender,recipients[i], _paymentToken, feeAmount,true);
            emit RoyaltyPayout(listings[_saleId].collectionList.nftContractAddress, recipients[i], feeAmount);
            ...
            // _totalAmount = _handleRoyaltyEnginePayout(
            //     listings[_saleId].collectionList.nftContractAddress,
            //     _totalAmount,
            //     _paymentToken
            // );
        }
    ...
    //         address payable[] memory recipients,
    //         uint256[] memory bps // Royalty amount denominated in basis points
    //     ) = royaltySupport.getCollectionRoyalty(_tokenContract);
    ...
    //         _handlePayment(msg.sender,recipient, _payoutCurrency, feeAmount,true);
    //         emit RoyaltyPayout(_tokenContract, recipient, feeAmount);
    ...
    function __mintTransaction(
        ...
        uint256 totalBuyTokens,
        uint256 quantity,
        IDutchAuction.typeOfSale saleType
    ) private returns (uint256[] memory) {
        uint256[] memory buyTokenIds = new uint256[](totalBuyTokens);
        if (IERC165(tokenContract).supportsInterface(ERC721_INTERFACE_ID)) {
            buyTokenIds = IERC721CreatorCore(tokenContract).mintExtensionBatch(
                buyer,
                uint16(totalBuyTokens)
            );
        } else if (
            IERC165(tokenContract).supportsInterface(ERC1155_INTERFACE_ID)
        ) {
            address[] memory to = new address[](1);
            uint256[] memory amounts = new uint256[](totalBuyTokens);
            string[] memory uris;
            to[0] = buyer;

            if (saleType == IDutchAuction.typeOfSale.multiple_tokens_with_Same_Quantity) {
                for (uint256 i = 0; i < totalBuyTokens; i++) {
                    amounts[i] = quantity;
                }

                buyTokenIds = IERC1155CreatorCore(tokenContract)
                    .mintExtensionNew(to, amounts, uris);
            } else if (saleType == IDutchAuction.typeOfSale.single_tokens_multiple_Quantity) {
                amounts[0] = quantity;
                if (nextBuyToken == 0) {
                    buyTokenIds = IERC1155CreatorCore(tokenContract)
                        .mintExtensionNew(to, amounts, uris);
    ...
    function __tokenTransaction(
        ...
        uint256[] memory buyTokenIds = new uint256[](totalBuyTokens);
        uint256 tokenId;
        tokenId = nextBuyToken;
        for (uint256 i = 0; i < totalBuyTokens; i++) {
            if (IERC165(tokenContract).supportsInterface(ERC721_INTERFACE_ID)) {
                while (IERC721(tokenContract).ownerOf(tokenId) != tokenOwner) {
                    require(
                        endTokenId >= tokenId,
                        "does not have enough token to transfer"
                    );
                    tokenId += 1;
                }
                ...
                if (saleType == IDutchAuction.typeOfSale.multiple_tokens_with_Same_Quantity) {
                    while (
                        IERC1155(tokenContract).balanceOf(tokenOwner, tokenId) <
                        quantity
                    ) {
                        require(
                            endTokenId >= tokenId,
    ...
    function cancelAuction(string memory auctionId) external adminRequired {
        ...
        require(
            listings[auctionId].startingPrice == 0,
            "auction has already deleted"
        );
        require(latestSaleInfo[auctionId].lastBoughtPrice == 0,"cant delete after the sale started");
        delete (listings[auctionId]);
        emit CancelAuction(auctionId);
    }

    //  @notice Withdraw the funds to owner
    function withdraw(address paymentCurrency) external  {
        require(owner() == msg.sender, "only owner can withdraw the funds");
        bool success;
    ...
    function UpdateEndTime(
        ...
        require(usedAuctionId[auctionId], "unsupported sale");
        require(
            listings[auctionId].auctionEndTime > block.timestamp,
            "auction has already ended"
        );
        listings[auctionId].auctionEndTime = endTime;
    }

    function updateReservePrice(string memory dutchAuctionID, uint128 reservedPrice) external {
        require(usedAuctionId[dutchAuctionID], "unsupported sale");
        require(reservedPrice < dutchUtility.getCurrentDutchPrice(dutchAuctionID), "should be less than current price");
    ...
    function updateUtilityAndRoyaltyAddress(
        ...
        require(royaltySupportArg!=address(0) && dutchUtilityArg!= address(0), "Invalid Args");
       royaltySupport = IRoyaltyEngineForDutch(royaltySupportArg);
       dutchUtility = IDutchAuction(dutchUtilityArg);
    }

     function getListings(string memory dutchAuctionId) external view returns(IDutchAuction.DutchAuctionList memory auctionist){
        return listings[dutchAuctionId];
    }
    function getLatestSaleInfo(string memory dutchAuctionId) external view returns(latestSaleDetails memory slaeDetails){
        return latestSaleInfo[dutchAuctionId];
    }
    function getBuyerInfo(string memory dutchAuctionId, address BuyerAddress) external view returns(BuyerDetails memory buyerDetails) {
        return buyerInfo[dutchAuctionId][BuyerAddress];
    }
    function getBuyerAddress(string memory dutchAuctionId) external view returns(address[] memory Buyers) {
 ...
 */
interface IRoyaltyEngineForDutch {
    /**
...
interface IDutchAuction { 
     ...
     struct NftContractList {
        ...
        uint256 startTokenId; //1   //45
        uint256 endTokenId; //10   //60
        uint256 quantity; //100
        address tokenOwner;
        uint64 noOfTokens; //100  8
        uint64 walletLimit; //10 8
        uint64 txnLimit; //5 8
        bool isMint; //f
        bool isInstantDeliver; //t
        bool isRebate; //t
        typeOfSale saleType;
    ...
    enum typeOfSale {
        erc_721_nft_type, // refer token Id
        single_tokens_multiple_Quantity, // refer token Quantity
        multiple_tokens_with_Same_Quantity //  refer token Id
    }

    struct DutchAuctionList {
        NftContractList collectionList;
        uint128 startingPrice; //100 
        uint128 reservedPrice; //10
        uint128 reducePrice; //5 16
        uint32 auctionStartTime; //4.00pm  3
        uint32 auctionEndTime; //6.00pm3
        uint32 timeForPriceDecrement; //103
        uint32 halfLifeTime; //03
        address paymentCurrency;
    ...
    function getListings(string memory dutchAuctionId) external view returns(DutchAuctionList memory auctionist);

    function getLatestSaleInfo(string memory dutchAuctionId) external view returns(latestSaleDetails memory saleDetails);
    ...
    function computeTax(string memory dutchAuctionID) external returns(uint256);
}
