...
contract DutchUtility is ReentrancyGuard, Ownable{
...
function __beforeCreateAuction(
        ...
        IDutchAuction.DutchAuctionList memory list = DutchAuction.getListings(dutchAuctionID);       
        IDutchAuction.NftContractList memory nftData = list.collectionList;
        IDutchAuction.BuyerDetails memory buyerInfo = DutchAuction.getBuyerInfo(dutchAuctionID, buyer);       
        IDutchAuction.latestSaleDetails memory latestSaleInfo = DutchAuction.getLatestSaleInfo(dutchAuctionID);
        
        require(initialized,"need to initialize the Dutch Auction");
        
        // Validating buyingAmount
        address paymentToken = list.paymentCurrency;        
        ...
        require(
            nftData.txnLimit >= totalBuyTokens,
            "tnx limit exceeds"
        );

        require(
            buyerInfo.totalCap + totalBuyTokens <=
                nftData.walletLimit,
        ...
        if (nftData.saleType == IDutchAuction.typeOfSale.single_tokens_multiple_Quantity) {
            require(
                totalBuyTokens == 1 && quantity1155 > 0,
                "can buy only single nft with multiple Quantity"
            );
            require(
                latestSaleInfo.noOfBuyedTokens + quantity1155 <=
                    nftData.quantity,
            ...
            require(
                latestSaleInfo.noOfBuyedTokens +
                    totalBuyTokens <=
                    nftData.noOfTokens,
                "maxQauntity limit reached"
            );
        ...
        if (paymentToken != address(0)) {
            ...
            require(
                IERC20(paymentToken).balanceOf(buyer) >= buyingAmount+tax,
                "Insufficient ERC20 funds"
            );
            // checks the buyer has provided approval for the contract to transfer the amount
            require(
                IERC20(paymentToken).allowance(buyer, address(DutchAuction)) >=
                    buyingAmount+tax,
        ...
        if(!nftData.isMint){
        require(
            ApprovalCheck(nftData.nftContractAddress).isApprovedForAll(
                nftData.tokenOwner,
...
function getCurrentDutchPrice(
        ...
        require(initialized,"need to initialize the Dutch Auction");
        require(
            list.auctionEndTime >= block.timestamp && list.auctionStartTime <= block.timestamp,
...
function claimable(
        ...
        IDutchAuction.DutchAuctionList memory list = DutchAuction.getListings(dutchAuctionID);
        IDutchAuction.BuyerDetails memory buyerInfo = DutchAuction.getBuyerInfo(dutchAuctionID, BuyerWallet);
        IDutchAuction.latestSaleDetails memory saleDetails = DutchAuction.getLatestSaleInfo(dutchAuctionID);
        
        require(initialized,"need to initialize the Dutch Auction");

        require(list.collectionList.isRebate," Not applicable");
          
        for (uint256 i = 0;i< buyerInfo.boughtPrice.length;i++) {
            totalClaimable +=
                (buyerInfo.boughtPrice[i]*buyerInfo.tokenQuantity[i]) -
                    (saleDetails.lastBoughtPrice * buyerInfo.tokenQuantity[i]);
                
                uint256 percentage = (buyerInfo.tax[i] * 10**18) / (buyerInfo.boughtPrice[i]*buyerInfo.tokenQuantity[i]);
                
            totalTax += (((buyerInfo.boughtPrice[i]*buyerInfo.tokenQuantity[i]) - 
                (saleDetails.lastBoughtPrice* buyerInfo.tokenQuantity[i])) * percentage) / 10**18;
        }
    } 


function computeTax(string memory dutchAuctionID) external view returns(uint256 totalTax){
        
        require(initialized,"need to initialize the Dutch Auction");

        address[] memory buyerAddress = DutchAuction.getBuyerAddress(dutchAuctionID);

       totalTax=0;
        
        for (uint256 i = 0; i < buyerAddress.length; i++) {
            address walletAddress = buyerAddress[i];
            IDutchAuction.BuyerDetails memory buyerInfo = DutchAuction.getBuyerInfo(dutchAuctionID, walletAddress);
            
            IDutchAuction.latestSaleDetails memory saleDetails = DutchAuction.getLatestSaleInfo(dutchAuctionID);       
           
            for (uint256 j = 0; j < buyerInfo.boughtPrice.length; j++) {
                
                if (buyerInfo.tax[j] != 0){                                       
                    uint256 percentage = (buyerInfo.tax[j] * 10**18) / (buyerInfo.boughtPrice[j]*buyerInfo.tokenQuantity[j]);
                    totalTax +=((saleDetails.lastBoughtPrice *buyerInfo.tokenQuantity[j])* percentage) / 10**18;
                }         
