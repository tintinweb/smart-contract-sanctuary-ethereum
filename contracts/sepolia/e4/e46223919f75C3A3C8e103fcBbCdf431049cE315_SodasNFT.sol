...
contract SodasNFT is ERC721, ERC721URIStorage, Ownable {
	...
	function isAllowedToMint() public view returns (bool) {
		return !_exists(tokenIdByAddress[msg.sender].tokenId) && tokenIdByAddress[msg.sender].tokenId == 0;
	}
	
	function isAllowedToDrink() public view returns (bool) {
		return _exists(tokenIdByAddress[msg.sender].tokenId);
	}
	
	function isAllowedToReview() public view returns (bool) {
		return !_exists(tokenIdByAddress[msg.sender].tokenId) && tokenIdByAddress[msg.sender].tokenId != 0;
	}
