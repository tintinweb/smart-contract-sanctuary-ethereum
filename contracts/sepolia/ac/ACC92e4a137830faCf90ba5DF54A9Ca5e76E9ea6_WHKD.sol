...
contract WHKD is Initializable,IERC20, IERC20Metadata,UUPSUpgradeable{
    ...
    // string private _name = "Wrapped HKD";
    // string private _symbol = "WHKD";
    string private _name;
    string private _symbol;
    uint8 private _decimals;

    // --- Auth ---
    mapping (address => uint) public Auth;
 

    function initialize() initializer public {
        //Auth init
        Auth[msg.sender] = 1;
        __ERC20_init("Wrapped HKD", "WHKD" , 18);
    ...
    //设置不同的exchange的权限
    function rely(address usr) external authorized { Auth[usr] = 1; }//要求Auth[调用者owner] = 1
    function deny(address usr) external authorized { Auth[usr] = 0; }//要求Auth[调用者owner] = 1
    
    modifier authorized {
        require(Auth[msg.sender] == 1, "USDPool/not-authorized");
        _;
    }
