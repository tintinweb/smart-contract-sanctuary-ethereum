...
import "@openzeppelin/contracts4.8.2/utils/Strings.sol";
...
contract CHARACTER_SHOP is ReentrancyGuard, Pausable, Ownable {
    ...
    mapping(address => mapping(uint256 => uint128))
        public userBuyCountInSmallSale;
    address public feeReceiver;
    uint256 failedTransferCredits;

    uint256 public smallSaleCount;
    SmallSale[] public smallSales;

    mapping(address => mapping(uint8 => BaseUrlForChapter))
        public baseUrlForChapter;
    ...
    struct Auction {
        ...
        uint32 reserved_1; // for compatibility to previous one
        uint256 auctionEnd;
        uint128 minPrice;
        uint128 buyNowPrice;
        uint128 nftHighestBid;
        // bytes32 allowListRoot; // using merkletree for whitelist addresses; remove to reduce gas fee
        address nftHighestBidder;
        address nftRecipient; // The bidder can specify a recipient for the NFT if their bid is successful.
        // string tokenURI; // tokenURI is set based on chapterId
     ...
     */
    uint128 public defaultBidIncreaseAmount;
    ...
    modifier smallSaleExist(uint256 _smallSaleId) {
        require(_smallSaleId < smallSaleCount, "SmallSaleID is invalid!");
        _;
    }
    ...
    modifier isValidChapter(address _nftContractAddress, uint8 _chapterId) {
        require(
            baseUrlForChapter[_nftContractAddress][_chapterId].isSet == true,
    ...
    constructor(address _feeReceiver) {
        defaultBidIncreaseAmount = 10 ** 16; // 0.01ETH
        defaultAuctionBidPeriod = 86400; //1 day
        feeReceiver = _feeReceiver;
    }

    // create Small Sale function
        ...
        returns (uint256)
    {
        ...
        emit SmallSaleCreated(smallSaleCount, block.timestamp);
        return smallSaleCount++;
    }

    // set base url for chapter function
    ...
    function _isAuctionOngoing(
        ...
        uint256 auctionStart = nftContractAuctions[_nftContractAddress][
            _tokenId
        ].startAt;
    ...
    function _isBuyNowPriceMetOrZeroSale(
        ...
        uint256 smallSaleId = nftContractAuctions[_nftContractAddress][_tokenId]
            .smallSaleId;
        return
            (smallSales[smallSaleId].saleType == SaleType.ZeroPriceSale) ||
            (buyNowPrice > 0 &&
    ...
    function _doesBidMeetBidRequirements(
        ...
        if (smallSales[smallSaleId].saleType == SaleType.BuyNow) {
            return msg.value >= buyNowPrice;
        } else if (
            smallSales[smallSaleId].saleType == SaleType.EnglishAuction
        ) {
            //if the NFT is up for auction, if highestBid existes return highest + increaseAmount, else return minPrice
            ...
            uint256 validBidAmount = nftHighestBidder == address(0)
                ...
                : highestBid +
                    _getBidIncreaseAmount(_nftContractAddress, _tokenId);

            return msg.value >= validBidAmount;
        } else {
            // ZeroPriceSale
            return true;
        }
    ...
    function _getBidIncreaseAmount(
        address _nftContractAddress,
        uint256 _tokenId
    ) internal view returns (uint128) {
        uint128 bidIncreaseAmount = nftContractAuctions[_nftContractAddress][
            _tokenId
        ].bidIncreaseAmount;

        if (bidIncreaseAmount == 0) {
            return defaultBidIncreaseAmount;
        } else {
    ...
    function _setupAuction(
        ...
        if (smallSale.saleType == SaleType.BuyNow) {
            nftContractAuctions[_nftContractAddress][_tokenId]
                .buyNowPrice = smallSale.price;
            nftContractAuctions[_nftContractAddress][_tokenId].minPrice = 0;
            nftContractAuctions[_nftContractAddress][_tokenId]
                .bidIncreaseAmount = 0;
        } else if (smallSale.saleType == SaleType.EnglishAuction) {
            nftContractAuctions[_nftContractAddress][_tokenId].buyNowPrice = 0;
            nftContractAuctions[_nftContractAddress][_tokenId]
                .minPrice = smallSale.price;
            nftContractAuctions[_nftContractAddress][_tokenId]
                .bidIncreaseAmount = _getBidIncreaseAmount(
                _nftContractAddress,
                _tokenId
            );
        } else {
        ...
        emit AuctionCreated(
            ...
            _auctionStart,
            _auctionEnd,
            nftContractAuctions[_nftContractAddress][_tokenId].bidIncreaseAmount
        );
    }
    ...
    function _createNewNftAuction(
        ...
        require(
            nftContractAuctions[_nftContractAddress][_tokenId]
                .nftHighestBidder == address(0),
            "Auction already has highest bidder"
        );
        _setupAuction(
            _smallSaleId,
            _nftContractAddress,
    ...
    function createDefaultNftAuction(
        ...
        address _nftContractAddress = smallSales[_smallSaleId]
            .nftContractAddress;
        try CHARACTER_V1(_nftContractAddress).ownerOf(_tokenId) returns (
            ...
            _createNewNftAuction(
                ...
                // _tokenURI,
                block.timestamp,
                block.timestamp + defaultAuctionBidPeriod
            );
        }
    }
        ...
        for (uint16 i; i < length; i++) {
            ...
            returns (address) {
                ...
                nftContractAuctions[_nftContractAddress][_tokenIds[i]]
                    .bidIncreaseAmount = _bidIncreaseAmount;
                _createNewNftAuction(
                    _smallSaleId,
                    _nftContractAddress,
                    _tokenIds[i],
    ...
    function _createSale(
        ...
        _mintNftAndTransferPayment(_nftContractAddress, _tokenId, false);
        ...
        notExceedBuyLimit(_nftContractAddress, _tokenId)
    {
        ...
        if (_isBuyNowPriceMetOrZeroSale(_nftContractAddress, _tokenId)) {
    ...
    function _resetAuction(
        ...
        // nftContractAuctions[_nftContractAddress][_tokenId]
        //     .allowListRoot = bytes32(0);
        nftContractAuctions[_nftContractAddress][_tokenId].startAt = 0;
    }
