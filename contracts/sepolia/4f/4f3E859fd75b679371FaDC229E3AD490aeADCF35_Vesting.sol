...
contract Vesting is Ownable, ReentrancyGuard{
    ...
    mapping (uint256 => address) private holdersIndexes;
    ...
    */
    function createVestingSchedule(
        ...
        holdersIndexes[_totalHolders] = _beneficiary;
        _totalHolders++;
    }
    ...
    function createVestingSchedules(
        ...
        onlyOwner{
        require(
            this.getWithdrawableAmount() >= _amount * _addresses.length,
            "TokenVesting: cannot create vesting schedule because not sufficient tokens"
        );
        require(_duration > 0, "TokenVesting: duration must be > 0");
        require(_amount > 0, "TokenVesting: amount must be > 0");
        ...
        for (uint i = 0; i < _addresses.length; i++) {
            vestingSchedules[_addresses[i]] = VestingSchedule(
                true,
                _addresses[i],
                _start,
                _duration,
            ...
            holdersIndexes[_totalHolders] = _addresses[i];
            _totalHolders++;
        }
    }
    ...
    */
    ...
    function releaseAll()
        ...
        nonReentrant {
            bool isOwner = msg.sender == owner();
            require(
                isOwner, "TokenVesting: only owner can release vested tokens"
            );

            for (uint i = 0; i < _totalHolders; i++) {
                address addr = holdersIndexes[i];
                ...
                claimedAmounts[addr] += vestedAmount;
            }
        }
    ...
    */
    ...
    function getTotalAmoutClaimed(address addr)
        public
        view
        returns(uint256){
        return claimedAmounts[addr];
    }
