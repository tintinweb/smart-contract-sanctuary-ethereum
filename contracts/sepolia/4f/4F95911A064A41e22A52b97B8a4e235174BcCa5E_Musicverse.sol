  ...
  OperatorFilterer
{
  ...
  uint private airdropTotalSupply = 0;
  ...
  constructor(
    ...
    ERC721A('ERC721PresetBase', 'ERC721PB')
    ...
    Ownable()
  {
    ...
    price[Phases.ORIGINAL_GANG] = 0.088 ether;
  ...
  function originalGangMint(
    uint64 amount,
    uint64 maxAmount,
    bytes memory signature
  ) external payable canMint(amount, Phases.ORIGINAL_GANG) nonReentrant {
    uint64 aux = _getAux(msg.sender);
    if (
      Signature.verify(maxAmount, msg.sender, signature) !=
      signer[Phases.ORIGINAL_GANG]
    ) revert InvalidSignature();
    if (aux + amount > maxAmount)
      revert ExceededMintQuota(aux + amount, maxAmount);
  ...
  function airdrop(
    ...
    if (airdropTotalSupply + amount > AIRDROP_MAX_SUPPLY)
  ...
  function setPhase(
    ...
    if (_phase == Phases.ORIGINAL_GANG && signer[_phase] == address(0))
