...
contract AnjiNFTSwap is Initializable , ReentrancyGuardUpgradeable, IERC721ReceiverUpgradeable, UUPSUpgradeable, OwnableUpgradeable{
    ...
    mapping(address => bool) private isSwappableItem;
    ...
    function setSwappable(address collection, bool isSwappable) public {
        isSwappableItem[collection] = isSwappable;
    }

    function getSwappable(address collection) public view returns (bool) {
        return isSwappableItem[collection];
    }
    ...
    function getSwapsByUser(address user) external view returns (Swap[] memory) {
        uint256 indexByUser = 0;
        Swap[] memory swapList;
        for(uint i = 0 ; i < _swapIdx.current(); i ++){
            if(_swaps[i].aAddress == user){
                swapList[indexByUser] = _swaps[i];
                indexByUser++;
            }
        }
        return swapList;
    ...
    function createSwap(NFT[] memory aNFTs, NFT[] memory expectNFTs) external payable {
        _swapIdx.increment();
        ...
        for (uint256 i = 0; i < expectNFTs.length; i++) {
            swap.expectNFTs.push(expectNFTs[i]);
        }

        swap.aEth = msg.value;
        _ethLocked += swap.aEth;

        emit SwapCreate(msg.sender, swap.bAddress, _swapIdx.current(), aNFTs, swap.aEth);
    }

    // User B init the swap
    function initSwap(uint256 id, NFT[] memory bNFTs) external payable {
        require(_swaps[id].bNFTs.length == 0 && _swaps[id].bEth == 0, "swapAlreadyInit");
        require(_swaps[id].expectNFTs.length == bNFTs.length, "bNFTs and expectNFTs are different(length).");

        for(uint i = 0 ; i < bNFTs.length; i ++){
            bool isContain = false;
            for(uint j = 0 ; j < _swaps[id].expectNFTs.length; j ++){
                if(bNFTs[i].contractAddr == _swaps[id].expectNFTs[j].contractAddr 
                && bNFTs[i].id == _swaps[id].expectNFTs[j].id){
                    isContain = true;
                }
            }
            require(isContain == true, "bNFTs and expectNFTs are different.");
        }
