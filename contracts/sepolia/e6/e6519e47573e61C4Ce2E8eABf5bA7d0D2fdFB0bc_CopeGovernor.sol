    ...
    GovernorTimelockControl
{
    ...
    function castVote(uint256 proposalId, uint8 support) public virtual override(Governor, IGovernor) returns (uint256) {
        uint256 voterBalance = getVotes(msg.sender, block.number - 1);  // Get voter's balance.
        
        if (support == 0) {
            _votes[proposalId].againstVotes += voterBalance;
        } else if (support == 1) {
            _votes[proposalId].forVotes += voterBalance;
        } else if (support == 2) {
            _votes[proposalId].abstainVotes += voterBalance;
        }
    ...
    function getVoteCounts(uint256 proposalId) public view returns (uint256, uint256, uint256) {
        return (_votes[proposalId].forVotes, _votes[proposalId].againstVotes, _votes[proposalId].abstainVotes);
    }
}
