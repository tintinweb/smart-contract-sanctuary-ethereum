...
// import "./SoRektToken.sol";

contract EthereumOrangeXNFT is ERC721, Ownable {
    using Strings for uint256;
    ...
    constructor() ERC721("EthereumOrange", "XNFTs") {
        maxSupply = 320000;
        ...
        returns (uint256)
    {
        ...
        _metadataURI[newTokenId] = _tokenURI;
        return newTokenId;
    }

    function _mintLoop(address _receiver, string[] memory _tokenURIs) internal {
        // require(_tokenURIs.length== _mintAmount,"Mint Amount should be same as uri array length");
        for (uint256 i = 0; i < _tokenURIs.length; i++) {
            addressMintedBalance[msg.sender]++;
            supply.increment();
            _metadataURI[supply.current()] = _tokenURIs[i];
            _safeMint(_receiver, supply.current());
        }
    }

    function mintForAddress(address _receiver, string memory _tokenURI)
        public
        payable
        mintCompliance(1)
        onlyOwner
    ...
    function mintForAddressCompany(
        address _receiver,
        string[] memory metadataUris
    ) public onlyOwner {
        _mintLoop(_receiver, metadataUris);
    }
    ...
    function awardItem(address tokenHolder, string[] memory metadataUris)
        public
        onlyOwner
        returns (bool)
    {
        require(
            metadataUris.length <= airDropPerAdressLimit,
            "Air drop limit exceeded"
        );
        _mintLoop(tokenHolder, metadataUris);
        return true;
    }
