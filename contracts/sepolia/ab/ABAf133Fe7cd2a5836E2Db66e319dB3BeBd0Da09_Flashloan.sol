...
contract Flashloan is FlashLoanSimpleReceiverBase{
    ...
    DEX dex = DEX(0xEe26b0fCbD5e47A8f7B1b1a45C5EB8b62622279E);
    IERC20 usdc = IERC20(0xda9d4f9b69ac6C22e444eD9aF0CfC043b7a7f53f);
    IERC20 mytoken = IERC20(0xF71be0e25209eF28F7AacfeCB574dB1f7577872B);
    ...
    function executeOperation(address asset, uint256 amount, 
                              uint256 premium, address initiator, bytes calldata params) 
                                external override returns (bool) {
                                    ...
                                    //qui vanno le operazioni di arbitraggio

                                    usdc.approve(address(dex), amount);
                                    dex.buy(amount/2);
                                    mytoken.approve(address(dex), amount);
                                    dex.sell(amount/2);
 ...
 function chiediPrestito(address _token, uint256 _amount) public {
...
contract DEX {
    ...
    function buy(uint256 amount) public {
        require(token.balanceOf(address(this)) >= amount, "pool doesn't have enough tokens");
        // take money from buyer
        uint256 buyCost = amount*buyPrice/100;
        usdc.transferFrom(msg.sender, address(this), buyCost);
        token.transfer(msg.sender,amount);
    }

    function sell(uint256 amount) public {
        uint256 sellCost = amount*sellPrice/100;
        token.transferFrom(msg.sender, address(this), amount);
        usdc.transfer(msg.sender, sellCost);
    }
    ...
    function setPrices(uint _buyPrice, uint _sellPrice) external {
