...
contract Marketplace721A is ERC721A__IERC721ReceiverUpgradeable, OwnerOperator, VerifySignature {
    ...
    function sell(
        ...
        emit CreateSellOrder(_tokenId, order.owner, order.tokenId, order.orderPrice, order.currency, order.tokenAddress);
    }

    function buy(uint256 _tokenId, address _buyer) external operatorOrOwner {
        // check order status
        Order storage order = orders[_tokenId];
        require(order.owner != address(0), 'Order does not exist or is deleted');

        IERC721AUpgradeable(order.tokenAddress).safeTransferFrom(
            address(this),
            _buyer,
            order.tokenId,
            ''
