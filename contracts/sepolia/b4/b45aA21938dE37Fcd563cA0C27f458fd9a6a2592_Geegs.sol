...
contract Geegs is ERC721A, Ownable, ReentrancyGuard, EIP712 {
    ...
    mapping(uint256 => bool) public firstTxDone;
    ...
    modifier transferrable(uint256 tokenId) {
        if (firstTxDone[tokenId])
            require(transferAllowance[tokenId] > 0, "Soul bound");
        _;
    }
    ...
    function acceptDocument(uint256 jobId, bytes memory signature) external {
        ...
        if (restrictedOperator)
            require(
                operator[msg.sender] || msg.sender == doc.talent,
                "Unauthorized"
            );
        if (usedSignature[signature]) revert InvalidSignature();
        usedSignature[signature] = true;
        bytes32 digest = keccak256(abi.encodePacked(jobId));
        bytes32 message = ECDSA.toEthSignedMessageHash(digest);
        address signer = ECDSA.recover(message, signature);
        if (signer != doc.talent) revert InvalidSignature();
        doc.status = STATUS.ACCEPTED;

        transferFrom(address(this), doc.talent, jobId);
    ...
    function updateStatus(uint256 id, STATUS status) external {
        ...
        if (status == STATUS.PENDING_RELEASE) {
            if (msg.sender != doc.talent) revert Unauthorized();
            doc.status = STATUS.PENDING_RELEASE;
            emit PendingRelease(id);
        } else if (status == STATUS.COMPLETED) {
            if (msg.sender != doc.hirer) revert Unauthorized();
            doc.status = STATUS.COMPLETED;
            payable(doc.talent).sendValue(payout);
            emit Completed(id, doc.wage);
        } else revert BadStatus();
    }
    ...
    function transferFrom(
        ...
        if (firstTxDone[tokenId]) transferAllowance[tokenId]--;
