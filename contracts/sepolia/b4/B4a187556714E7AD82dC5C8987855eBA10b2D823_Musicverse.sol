  ...
  OperatorFilterer
{
  ...
  uint private constant AIRDROP_MAX_SUPPLY = 5555;
  ...
  address[] public _payees = [
    0x6176DcbF30f3f56E51eC6e47Ce1a79093eb37070,
    0x8754471e29C50a7e087A87b561543ba0D44EDb51,
    0xFCfFbe12C877F14c6D2e7791C1B31d01fE28207d,
    0x4eBE0e733195e5867f329F0A9e09c91e9AEa8f30,
    0x30C53d45ca0cf73a7DC17CEa4Fa535937cf09980,
    0x597Eb5DcA366A2B8eF502CcF920852e5Bb3178CD,
    0xDF3Fe8FA1252071850d1a9d3640232fB012E53e5
  ];
  uint256[] private _shares = [200, 600, 70, 15, 15, 50, 50];
  ...
  modifier canMint(uint amount, Phases p) {
    ...
    if (phase[Phases.CLOSED]) revert MintNotOpened();
    if (!phase[p]) revert MintNotOpened();
    _;
  }
  ...
  constructor(
    ...
    ERC721A('Musicverse', 'musicversenft')
    ...
    Ownable()
  {
    ...
    _setDefaultRoyalty(royaltyReceiver, 750); // 750 = 7.5%
    phase[Phases.CLOSED] = true;
    price[Phases.OG] = 0.088 ether;
    price[Phases.WHITELIST] = 0.098 ether;
    price[Phases.PUBLIC] = 0.12 ether;
  }
  ...
  function ogMint(
    ...
    uint64 maxAmount,
    bytes memory signature
  ) external payable canMint(amount, Phases.OG) nonReentrant {
    uint64 aux = _getAux(msg.sender);
    if (
      Signature.verify(maxAmount, msg.sender, signature) !=
      signer[Phases.OG]
  ...
  function airdrop(
    ...
    uint256 supply = totalSupply();
    if (supply + amount > AIRDROP_MAX_SUPPLY)
  ...
  function setPhase(
    ...
    if (_phase == Phases.OG && signer[_phase] == address(0))
      revert InvalidSigner(signer[_phase]);
    if (_phase == Phases.WHITELIST && signer[_phase] == address(0))
      revert InvalidSigner(signer[_phase]);
