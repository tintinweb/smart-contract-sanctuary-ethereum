...
contract PresaleSepolia is ReentrancyGuard, Ownable, Pausable {
    ...
    function buyETHmsgvalue() external payable nonReentrant {
        require(stage <= 10, "Presale ended");
        uint256 price = getCurrentETHPrice();
        uint256 tokens = msg.value * price * getCurrentRate() * (10**18);
        tokensPurchased[msg.sender] += tokens;
        ownerAddress.transfer(msg.value);
    }

    function buyETHwei() external payable nonReentrant {
        require(stage <= 10, "Presale ended");
        uint256 price = getCurrentETHPrice();
        uint256 tokens = msg.value * price * getCurrentRate();
        tokensPurchased[msg.sender] += tokens;
        ownerAddress.transfer(msg.value);
    }
    ...
    function buyTokensWithUSDT(uint256 _usdtAmount) external nonReentrant {
        ...
        USDT.safeTransferFrom(msg.sender, ownerAddress, _usdtAmount);
        tokensPurchased[msg.sender] += tokens;
    }
