...
/// @title Trait NFT Contract
/// @notice An ERC1155-based contract for managing traits of avatars.

contract TraitNft is ERC1155, IERC4907, Ownable {
    using Strings for uint256;

    /// @notice The base URI for token metadata
    string public baseUri;

	/// @notice The base layers for the avatars
    uint256[] public baseLayers;
    ...
    mapping (uint256  => UserInfo) internal _users;//[traitId] => [UserInfo]

    /// @dev Throws if called by any account other than the marketplace contract.
    ...
    modifier onlyMainContract() {
        require(msg.sender == IMarketplace(marketplaceAddress).mainContract(), "Unauthorized Call");
        _;
    }

    /// @notice Constructs a new TraitNft contract.
    /// @param _uri The base URI for the ERC1155 metadata.
    /// @param _baseLayers The base layers for the avatars.
    constructor(string memory _uri, uint256[] memory _baseLayers) ERC1155(_uri) {
	...
	*/
    ...
    /// @notice Mints a batch of new trait tokens to a given address.
	/// @dev only the MainNft contract can call this function
    /// @param to The address to mint the new trait tokens to.
    /// @param ids The trait token IDs to mint.
    /// @param amounts The trait data for the new trait tokens.
    /// @return True if the batch minting was successful, false otherwise.
    function batchTraitMint(address to, uint256[] memory ids, uint256[] memory amounts) external onlyMainContract returns(bool) {
		_mintBatch(to, ids, amounts, "");
		return true;
	}

    /// @notice Rents avatar traits to a user.
	/// @dev only the Marketplace contract can call this function
    /// @param owner The owner of the traits.
    /// @param user The user renting the traits.
    /// @param traitIds The IDs of the traits being rented.
    /// @param expires The expiration timestamp of the rental.
    function rentAvatarTraits(address owner, address user, uint256[] memory traitIds, uint64 expires) external onlyMarketplace {
        require(
            isOwnerOfTraits(owner, traitIds),
            "ERC1155: caller is not traits owner"
        );
        require(!isTraitsAssigned(traitIds), "User already assigned");
        for (uint256 i = 0; i < traitIds.length; i++) {
            UserInfo storage info = _users[traitIds[i]];
            info.user = user;
            info.expires = expires;
            emit UpdateUser(traitIds[i], user, expires);
        }
    }

	/// @notice Sets the user and expiration timestamp for a given trait token.
	/// @dev The zero address indicates there is no user and  only the Marketplace contract can call this function
    /// Throws if `traitId` already rented
	/// @param traitId The ID of the trait token.
    /// @param user The address of the user.
    /// @param expires UNIX timestamp, The new user could use the NFT before expires
    function setUser(uint256 traitId, address user, uint64 expires) public override virtual onlyMarketplace{
        require(userOf(traitId) == address(0), "User already assigned");
        UserInfo storage info = _users[traitId];
        info.user = user;
        info.expires = expires;
        emit UpdateUser(traitId, user, expires);
    }


    /// @notice Get the user address of a trait
    /// @dev The zero address indicates that there is no user or the user is expired
    /// @param traitId The trait id to get the user address for
    /// @return The user address for this trait
	function userOf(uint256 traitId) public view override virtual returns (address) {
        if (uint256(_users[traitId].expires) >= block.timestamp) {
            return _users[traitId].user;
        }
        return address(0);
    }


    /// @notice Get the user expires of a given trait token
    /// @dev The zero value indicates that there is no user
    /// @param traitId The trait id to get the user expires for
    /// @return The user expires for this trait
    function userExpires(uint256 traitId) public view override virtual returns (uint256) {
        return _users[traitId].expires;
    }
    ...
    /// @notice Returns the URI of a given trait token.
    /// @param traitId The ID of the trait token.
    /// @return The URI of the trait token.
    function uri(uint256 traitId) public view override returns (string memory) {
        return string(abi.encodePacked(baseUri, traitId.toString(), ".json"));
    }

    // /// @notice Checks if a given trait token exists.
    // /// @param traitId The ID of the trait token.
    // /// @return True if the trait token exists, false otherwise.
    // function _exists(uint256 traitId) public view returns (bool) {
    //     return traits[traitId].layerId > 0;
    // }

    /// @notice Returns the base layers of the avatars.
    /// @return An array containing the base layers.
    function getBaseLayers() external view returns (uint256[] memory) {
        return baseLayers;
    }

    /// @notice Checks if any of the specified traits are assigned to a user.
    /// @param traitIds The IDs of the traits to check.
    /// @return True if any of the specified traits are assigned to a user, false otherwise.
    function isTraitsAssigned(uint256[] memory traitIds) public view virtual returns (bool) {
        for (uint256 i = 0; i < traitIds.length; i++) {
            if (userOf(traitIds[i]) != address(0)) return true;
        }
        return false;
    }

     /// @notice Checks if the given address is the owner of the specified traits.
    /// @param user The address to check ownership for.
    /// @param traitIds The IDs of the traits to check.
    /// @return True if the address is the owner of all specified traits, false otherwise.
    function isOwnerOfTraits(address user, uint256[] memory traitIds) internal view returns (bool) {
        for (uint256 i = 0; i < traitIds.length; i++) {
            if (balanceOf(user, traitIds[i]) == 0) return false;
        }
        return true;
    }


	/// @notice Checks if a given layer ID is a base layer.
    /// @param _layerId The layer ID to check.
    /// @return True if the given layer ID is a base layer, false otherwise.
    function isBaseLayer(uint256 _layerId) external view returns (bool) {
        for (uint256 i = 0; i < baseLayers.length; i++) {
            if (baseLayers[i] == _layerId) return true;
        }
        return false;
    }
}