...
//https://api.frank.hk/api/nft/demo/1155/marvel/{id}.json
    constructor(string memory _baseuri, address mintTo) ERC1155(_baseuri) {
        mint(mintTo, CAPTAIN_AMERICA, 10**18, "");
        mint(mintTo, THOR, 1, "");
        mint(mintTo, IRON_MAN, 5, "");
        mint(mintTo, SPIDER_MAN, 10, "");
    }
        ...
        onlyOwner
    {
        for (uint i = 0; i< ids.length; i++) {
            counts[ids[i]]  = counts[ids[i]] + amounts[ids[i]];
        }
        
        _mintBatch(to, ids, amounts, data);
    }

    function getNameByIndex(uint _index) 
    public view returns (string memory name){
        return NFTs[_index];
    }

    function getCountsByIndex(uint _index) 
    public view returns (uint256 count){
        return counts[_index];
    }
}
