...
contract AuctionHouse is IAuctionHouse, Pausable, ReentrancyGuard, Ownable {
        ...
        for (uint256 i = 0; i < _bidders.length; i++) {
            ...
            if (bid.winner == true && bid.minted == false && bid.refunded == false) {
                ...
                if (bid.rafflewinner == true) {
                    emit TokenMint(bidder, tokenId, MintType.Raffle, bid.finalprice, false, address(0), false, 0);
                } else {
                    emit TokenMint(bidder, tokenId, MintType.Auction, bid.finalprice, false, address(0), false, 0);
                }
    ...
    function promoBatchMint(address[] memory _to) external onlyAdmin {
        ...
        for (uint256 i = 0; i < numAddresses; i++) {
            _promoMint(_to[i], MintType.Promo);
        }
    ...
    */
    ...
    function _promoMint(address to, MintType mintType) internal {
        uint256 tokenId = nft.mint(to);

        emit TokenMint(to, tokenId, mintType, 0, false, address(0), false, 0);
    }
        ...
        if (group == 1) {
            ...
            for (uint256 i = 0; i < oasisCount; i++) {
                ...
                for (uint256 j = 0; j < quantityMintedWithOasis; j++) {
                    ...
                    _emitAllowListTokenMint(requester, receiver, tokenId, msgValue, true, oasisId);
            ...
            for (uint256 i = 0; i < _qty; i++) {
                ...
                _emitAllowListTokenMint(requester, receiver, tokenId, msgValue, false, 0);
    ...
    function _emitAllowListTokenMint(address requester, address receiver, uint256 tokenId, uint256 pricePaid, bool oasisUsed, uint256 oasisId) internal {
        if (requester != receiver) {
            // delegated
            emit TokenMint(receiver, tokenId, MintType.Allowlist, pricePaid, true, requester, oasisUsed, oasisId);
        } else {
            // not delegated
            emit TokenMint(receiver, tokenId, MintType.Allowlist, pricePaid, false, address(0), oasisUsed, oasisId);
        }
    }
    ...
    function getAllowlistAndRaffleInfo() public view returns (uint256 _raffleSupply, uint256 _allowListPrice, uint256 _allowListStartDateTime, uint256 _allowListEndDateTime, bool _raffleWinnersSet, bool _publicSale, uint256 _allowlistSupply, uint256 _totalMinted, uint256 _oasislistStartDateTime, uint256 _allowlistMintMax, uint256 _oasisPrice, uint256 _publicPrice) {
        return (raffleSupply, allowListPrice, allowListStartDateTime, allowListEndDateTime, raffleWinnersSet, publicSale, allowlistSupply, nft.totalSupply(), oasisListStartDateTime, allowlistMintMax, oasisPrice, publicPrice);
    }
    ...
    function publicSaleMint() public payable nonReentrant onlyUnpaused onlyUnsanctioned(msg.sender) {
        ...
        emit TokenMint(receiver, tokenId, MintType.PublicSale, msgValue, false, address(0), false, 0);
    }
    ...
    // airdrop mint
    function creditCardMint(address _to, uint256 _qty) external onlyAdmin {
        require(promoSupply >= _qty, 'Not enough promo supply');
        require(block.timestamp <= allowListEndDateTime, 'Outside promo mint window');
        ...
        for (uint256 i = 0; i < _qty; i++) {
            _promoMint(_to, MintType.CreditCard);
        }
        promoSupply -= _qty;
        auctionSupply = maxSupply - nft.totalSupply() - raffleSupply;
    }
    ...
    // airdrop batch mint; sends 1 to each address in array
    function creditCardBatchMint(address[] memory _to) external onlyAdmin {
        uint256 numAddresses = _to.length;
        ...
        for (uint256 i = 0; i < numAddresses; i++) {
            _promoMint(_to[i], MintType.CreditCard);
        }
        promoSupply -= numAddresses;
        auctionSupply = maxSupply - nft.totalSupply() - raffleSupply;
    }
...
interface IAuctionHouse {
    ...
    */
    event TokenMint(address indexed to, uint256 indexed tokenId, MintType indexed mintType, uint256 amountPaid, bool isDelegated, address delegatedVault, bool oasisUsed, uint256 oasisId);
