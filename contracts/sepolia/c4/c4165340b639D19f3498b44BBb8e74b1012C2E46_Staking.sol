...
contract Staking is ReentrancyGuard, Pausable, Ownable {
    ...
    mapping (address => uint256) public stoppingTime;
    ...
    function updateStartingTime (uint256 _startingTime) external onlyOwner returns (uint256) {
        require(_startingTime > block.timestamp, "Staking: incorrect timestamp");
        startTime = _startingTime;
        return _startingTime;
    }

    function updateStoppingTime (uint256 _stoppingTime) external onlyOwner returns (uint256) {
        require(_stoppingTime > startTime, "Staking: incorrect timestamp");
        stopTime = _stoppingTime;
        return _stoppingTime;
    }

    function calculateReward (address owner) internal view returns (uint256){
        uint256 previousRewardPaid = userRewardpaid[owner];
        uint256 stakingDuration = block.timestamp - startingTime[owner];
        uint256 rewardRate = ((_balances[owner]).div(2)).div(2592000);
        uint256 rewardForStakingDuration = stakingDuration.mul(rewardRate);
        uint256 reward = rewardForStakingDuration - previousRewardPaid;
        return reward;
    }
    ...
    function stake(uint256 amount) external nonReentrant whenNotPaused () {
        ...
        startingTime[msg.sender] = block.timestamp;
        stoppingTime[msg.sender] = block.timestamp + 60;
    ...
    function withdraw(uint256 amount) public virtual nonReentrant () {
        ...
        require(block.timestamp >= stoppingTime[msg.sender], "Staking: Cannot withraw, staking period has not elapsed");
    ...
    function getReward() public virtual nonReentrant {
        ...
        if (reward > 0) {
            ...
            userRewardpaid[msg.sender] = userRewardpaid[msg.sender] + reward;
