...
contract Inventroy {
    ...
    function getAddedMaterialDetails(bytes32 _materialID)
        external
        view
        returns (Types.Product memory)
    {
        // bytes32 _pidHash = genCn.genrateUniqueIDByProductName(_materialName);
        address currUserAddress = msg.sender;
        return rawMaterials[currUserAddress][_materialID];
    }

    function getAddedMaterialDetails(address _producerID, bytes32 _materialID)
        external
        view
        returns (Types.Product memory)
    {
        // bytes32 _pidHash = genCn.genrateUniqueIDByProductName(_materialName);
        return rawMaterials[_producerID][_materialID];
    }

    //forchecking Inventory producer can check Inventroy is added or not by passing product name.
    function IsAddedInInventory(bytes32 _materialID)
        public
        view
        returns (bool)
    {
        // bytes32 _pidHash = genCn.genrateUniqueIDByProductName(_materialName);
        return (keccak256(
            abi.encodePacked((rawMaterials[msg.sender][_materialID].PId))
        ) == keccak256(abi.encodePacked((_materialID))));
    }
