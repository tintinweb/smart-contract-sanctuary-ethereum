...
contract TenderProcess {
    ...
    modifier onlyBidManager() {
        require(msg.sender == manager, "Only BidManager can execute this");
        _;
    }
    ...
    function calculatePrequalificationScores() public onlyBidManager {
...
public {
    ...
    // 4. Calculate Score
    function calculateScore() public onlyBidManager {
        for (uint i = 0; i < bidderAddress.length; i++) {
            Bidder storage bidder = bidders[bidderAddress[i]];
    ...
    // 5. Determine Winner
    function getWinner() public view onlyBidManager returns (address _bidderAddress, uint _BidPrice, uint _TenderScore) {
    ...
    // 6. Award Contract
    function awardContract() public onlyBidManager notAwarded {
    require(bidderAddress.length > 0, "No bidders present");
    ...
    // 7. Bid Records
function BidRecords() public view onlyBidManager returns (address[] memory, uint256[] memory, uint256[] memory) {
