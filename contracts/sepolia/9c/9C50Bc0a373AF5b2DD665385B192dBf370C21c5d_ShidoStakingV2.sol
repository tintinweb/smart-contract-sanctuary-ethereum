...
contract ShidoStakingV2 is Initializable, ReentrancyGuard {
    struct Stake {
        uint128 amount;
        uint128 unlockTime;
        uint128 depositTime;
    ...
    function stake(uint128 _amount) external nonReentrant {
        ...
        if (userStakeData.amount == 0) {
            userStakeData.unlockTime = uint64(block.timestamp + lockInPeriod);
        }
    ...
    function migrateAndStake(uint128 _amount, address _userAddress) external {
        Stake memory userStakeData = userStake[_userAddress];
        if (_amount == 0) {
            revert customErrors.zeroAmount(_amount);
        }
        if (userStakeData.amount == 0) {
        ...
        uint256 reward = _calculateReward(_userAddress, userStakeData.amount);
        pendingReward[_userAddress] =
            pendingReward[_userAddress] +
            uint128(reward);
        ...
        userStake[_userAddress] = userStakeData;
        totalStakes += _amount;
        IERC20(stakeToken).transferFrom(msg.sender, address(this), _amount);
        emit events.Staked(
            _userAddress,
    ...
    function stakeShidoReward() external nonReentrant {
        ...
        userStakeData = Stake(
            userStakeData.amount + uint128(currentReward + pendingRewardAmount),
            userStakeData.unlockTime,
            uint128(block.timestamp),
            userStakeData.claimedReward
        );
    ...
    function claimReward() external nonReentrant {
        ...
        userStakeData = Stake(
            ...
            userStakeData.unlockTime,
            uint128(block.timestamp),
            userStakeData.claimedReward + uint128(reward + pendingRewardAmount)
        );
        pendingReward[msg.sender] = 0;
        userStake[msg.sender] = userStakeData;
        if (
