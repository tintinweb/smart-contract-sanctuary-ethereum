...
contract AlturaLootboxV3 is UUPSUpgradeable, ERC1155Upgradeable, AccessControlEnumerableUpgradeable {
    ...
    function useKey(uint256 boxId) external {
        ...
        uint256 i;
        for (i; i < maxLength; ) {
            Item storage item = targetBox.items[i];

            if (pseudoRandomIndex < item.rarity && item.supply > 0) {
                uint256 newSupply = item.supply - 1;
                targetBox.items[i].supply = newSupply;

                if (newSupply == 0) {
                    //Item storage lastItem = targetBox.items[maxLength - 1];
                    //targetBox.items[i] = lastItem;
                    // targetBox.items.pop();
                    targetBox.totalRarity -= item.rarity;
                }
