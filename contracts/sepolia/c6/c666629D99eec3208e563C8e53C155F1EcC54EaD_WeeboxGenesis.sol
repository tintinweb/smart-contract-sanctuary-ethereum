...
contract WeeboxGenesis is ERC721A, Ownable {
  // metadata URI
  string private _baseTokenURI;

  // premit-listed minters
  mapping(address => bool) public minters;

  uint256 private constant COLLECTION_SIZE = 1958;
  uint256 public treasuryMintedCount = 0;
  uint256 private constant MAX_TREASURY_MINT_LIMIT = 150;
  ...
  function _treasuryMint(uint256 quantity, address receiver) internal {
    ...
    emit TreasuryMint(msg.sender, receiver, quantity, indexBeforeMint);
  }
  ...
  function mint(address to, uint256 quantity) external onlyMinters {
    ...
    emit ProxyMint(msg.sender, to, quantity, indexBeforeMint);
  }
}
