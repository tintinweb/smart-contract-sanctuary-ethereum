...
contract Test is ERC721A, ERC721ABurnable, ERC721AQueryable, Ownable, DefaultOperatorFilterer {
    enum ContractStatus {
        Sale,
        Paused
    }
    ...
    address public ballerContract;

    ContractStatus public status = ContractStatus.Paused;

    uint256 public maxSupply = 5000;
    uint256 public batchSupply = 5000;
    ...
    modifier callerIsBallerContract() {
        require(msg.sender == ballerContract, "The caller is not the baller contract");
        _;
    }

    constructor(string memory contractBaseURI) ERC721A ("MOONCOURT BOOSTER", "BOOSTER") {
    ...
    function setBatchSupply(uint256 _supply) external onlyOwner {
        batchSupply = _supply;
    }
    ...
    function mint(uint256 quantity, uint256 allowedQuantity, bytes32[] calldata proof) external payable callerIsUser {
        require(status == ContractStatus.Sale, "Sale not available");
        require(msg.value >= price * quantity, "Not enough ETH sent");
        require(_totalMinted() + quantity <= batchSupply, "Currently not enough supply");
    ...
    function setBallerContract(address _ballerContract) external onlyOwner {
        ballerContract = _ballerContract;
    }
    ...
    function burn(uint256 tokenId) public override callerIsBallerContract {
        _burn(tokenId, false);
    }
