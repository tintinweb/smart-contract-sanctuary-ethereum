...
contract PepePunksMinter is Ownable {

    uint256 public unitPrice = 200 ether;
    uint256 public maxCap = 5000;
    ...
    function mint(address _to, uint _count) public {
        ...
        require(pepeToken.balanceOf(msg.sender) >= amount, 'insufficient token balance');
        require(pepeToken.transferFrom(msg.sender, address(this), amount), 'invalid pepe transfer');
        _mint(_to, _count);
    }
    ...
    function pause(bool enable) public onlyOwner {
        mintPaused = enable;
    }

    function updateNftContrcat(IPepePunks _newAddress) public onlyOwner {
        nftContract = IPepePunks(_newAddress);
    }

    function updatePepeToken(IERC20 _newAddress) public onlyOwner {
    ...
    function ownerWithdrawTokens(uint256 amount, address _to, address _tokenAddr) public onlyOwner {
