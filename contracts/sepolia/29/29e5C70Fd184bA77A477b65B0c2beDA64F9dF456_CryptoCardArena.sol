...
function mintCard() external payable {
	...
	uint256 price = getAmountInEther();
	// console.log(price);
...
function fulfillRandomWords(uint256 requestId, uint256[] memory randomWords) internal override {
   ...
   function availableCardsLength() external view returns(uint256){
	...
	function getAmountInEther() public view returns (uint256) {
        ...
        uint256 priceInEther = uint256(answer) * 1e10; // 1e10 is the price feed decimals
        uint256 amountInEther = (mintPrice * priceInEther) / 1e18;
