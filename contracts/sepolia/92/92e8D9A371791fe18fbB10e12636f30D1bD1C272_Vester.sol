...
import "src/tokens/interfaces/IEsWAR.sol";

contract Vester is IERC20, ReentrancyGuard, Ownable {
    ...
    uint256 public vestingDuration = 15778458; // 6 months
    ...
    function setPairToken(address _pairToken) external onlyOwner {
        pairToken = _pairToken;
    }

    function setClaimableToken(address _claimableToken) external onlyOwner {
        claimableToken = _claimableToken;
    }
    ...
    function transferStakeValues(address _sender, address _receiver) external nonReentrant {
        _validateHandler();
    ...
    function setTransferredAverageStakedAmounts(address _account, uint256 _amount) external nonReentrant {
        _validateHandler();
        transferredAverageStakedAmounts[_account] = _amount;
    }

    function setTransferredCumulativeRewards(address _account, uint256 _amount) external nonReentrant {
        _validateHandler();
        transferredCumulativeRewards[_account] = _amount;
    }

    function setCumulativeRewardDeductions(address _account, uint256 _amount) external nonReentrant {
        _validateHandler();
        cumulativeRewardDeductions[_account] = _amount;
    }

    function setBonusRewards(address _account, uint256 _amount) external nonReentrant {
        _validateHandler();
    ...
    function getCombinedAverageStakedAmount(address _account) public view returns (uint256) {
        uint256 cumulativeReward = IRewardTracker(rewardTracker).cumulativeRewards(_account);
        uint256 transferredCumulativeReward = transferredCumulativeRewards[_account];
    ...
    function _updateVesting(address _account) private {
        ...
        IEsWAR(esToken).burnFrom(address(this), amount);
    }
