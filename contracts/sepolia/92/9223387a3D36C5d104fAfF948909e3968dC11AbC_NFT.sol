...
contract mainPaint{
    ...
    //final work change uri:  finalSVG(); changeuri_final()
    address public initial_painter;
    string  public url_string;
    string public coordinate;
    string public color_map;
    string public coordinate_map;
    string public exchange_map;
    string public svg;
    
    
    uint256 public tokenId;
    uint256 public vote_num =0;
    uint256 public dividen_num;
    ...
    bool public final_mintsale = false;
    bool public chang_url = false;
    bool public first_vote = true;
    bool public upload = false;

    mapping (string => string) colorConvert;
    ...
    mapping (string => uint256) coordinatexy; //if coordinate is painted => 2
    mapping (uint256 => string) NFT_condition;//tokenid => url
    mapping (uint256 => string) nftcolor; //tokenid => color
    mapping (uint256 => string) nftcoordinate;//tokenid => coordinate
    mapping (string => uint256) NFT_position;//coordinate => tokenid
    mapping (address => uint256) address_vote; //if user has voted =>2
    ...
    uint256[] public own_nft;
    uint256[] public dividen_record;
    ...
    constructor() {
        ...
        // color mapping
        colorConvert["R100"] = "#d24430";
        colorConvert["R80"] = "#da6959";
        colorConvert["R60"] = "#e58c84";
        colorConvert["R40"] = "#ebb4ad";
        colorConvert["R20"] = "#f5d9d6";
        colorConvert["B100"] = "#4574e6";
        colorConvert["B80"] = "#688fea";
        colorConvert["B60"] = "#8da9f2";
        colorConvert["B40"] = "#b3c3f4";
        colorConvert["B20"] = "#d4ddfa";
    ...
    function approve_market() public {
        require(msg.sender == nft.ownerOf(2));
        nft.approveNFT(market,2);
    ...
    function paint(uint32 x, uint32 y) public can_paint{//x and y is the coordinate, your right should be 2
        require ( coordinatexy[string(abi.encodePacked("(",Strings.toString(x),".",Strings.toString(y),")"))] != 2, "this position has been painted"); //coordinate can't be same
        require(x <= 60 && y <= 32, "Please input the correct coordinate");   //the range of canvas
        
        coordinate = string(abi.encodePacked("(",Strings.toString(x),".",Strings.toString(y),")"));
        coordinatexy[coordinate] = 2;
        paint_right[msg.sender] = 3;
        
        //generate ipfs uri based color and coordinate
        if (recent_bluecolor[msg.sender] != 0){
            url_string = string(abi.encodePacked("IPFS://QmWf4iujarwgavCbSAWV9j2MphYELhs1ygjE4QvfejBM3P/", Strings.toString(x),"_", Strings.toString(y), "_B", Strings.toString(recent_bluecolor[msg.sender]),".json"));
            color_map = string(abi.encodePacked("_B", Strings.toString(recent_bluecolor[msg.sender]),".json")); 
            if (recent_bluecolor[msg.sender] == 100){
                visual_map[y][x] = "B100";
            } else if(recent_bluecolor[msg.sender] == 80){
                visual_map[y][x] = "B80";
            } else if(recent_bluecolor[msg.sender] == 60){
                visual_map[y][x] = "B60";
            }else if(recent_bluecolor[msg.sender] == 40){
                visual_map[y][x] = "B40";
            }else if(recent_bluecolor[msg.sender] == 20){
                visual_map[y][x] = "B20";
            }
                
        }else if(recent_redcolor[msg.sender] != 0){
            url_string = string(abi.encodePacked("IPFS://QmWf4iujarwgavCbSAWV9j2MphYELhs1ygjE4QvfejBM3P/", Strings.toString(x),"_", Strings.toString(y), "_R", Strings.toString(recent_redcolor[msg.sender]),".json"));
            color_map = string(abi.encodePacked("_R", Strings.toString(recent_redcolor[msg.sender]),".json"));
            if (recent_redcolor[msg.sender] == 100){
                visual_map[y][x] = "R100";
            } else if(recent_redcolor[msg.sender] == 80){
                visual_map[y][x] = "R80";
            } else if(recent_redcolor[msg.sender] == 60){
                visual_map[y][x] = "R60";
            }else if(recent_redcolor[msg.sender] == 40){
                visual_map[y][x] = "R40";
            }else if(recent_redcolor[msg.sender] == 20){
                visual_map[y][x] = "R20";
            }
        }
        ...
        NFT_condition[tokenId] = url_string;
    ...
    function swap_color(uint256 tokenId_first, uint256 tokenId_second) public {//the tokenids of your two NFT
        ...
        //new uri
        string memory url1 = string(abi.encodePacked("IPFS://QmWf4iujarwgavCbSAWV9j2MphYELhs1ygjE4QvfejBM3P/", nftcoordinate[tokenId_first], nftcolor[tokenId_first]));
        string memory url2 = string(abi.encodePacked("IPFS://QmWf4iujarwgavCbSAWV9j2MphYELhs1ygjE4QvfejBM3P/", nftcoordinate[tokenId_second], nftcolor[tokenId_second]));
        //mint same tokenid NFT
        nft.exchangeMint(msg.sender, tokenId_first,  url1);
        nft.exchangeMint(msg.sender, tokenId_second, url2);
        
        //record data
        NFT_condition[tokenId_first] = url1;
        NFT_condition[tokenId_second] = url2;
        ...
        exchange_map = visual_map[y2][x2]; 
        visual_map[y2][x2] = visual_map[y1][x1];
        visual_map[y1][x1] = exchange_map;

        emit record_swap(tokenId_first, tokenId_second);
    }

    modifier owner_nft {
        require(address_vote[msg.sender] != 2, "you have already voted");
        _;
    }
    ...
    function votetomintfinal() public owner_nft { //Vote to determine whether to mint the final artwork
        ...
        require(first_vote == true, "this step has already stopped");
        require(position_done.length> 1199,"you still should wait until 1280 pixels have been painted");// 2/3 num of pixels 1200-1    

        vote_num = vote_num + own_num;
        address_vote[msg.sender] = 2; //record vote

        if (vote_num*3 >= position_done.length*2){
            final_mint = true;
            vote_num = 0;
            first_vote = false;
            emit vote_1("The final mint start");
        } else if (vote_num*3 < position_done.length*2 ){
 ...
 //改成visualization
    function finalSVG() public returns (string memory) {//generate final artwork svg
        // require (final_mint);
        /*svg = "<svg width=\"600\" height=\"300\" style=\"background-color: white;\" viewBox=\"0 0 600 300\" xmlns=\"https://www.w3.org/2000/svg\">";
        string memory coordinate;
        string memory pixelColor;
        for (uint32 i=0; i<30; i++) {
            for (uint32 z=0; z< 60; z++){
                coordinate = string(abi.encodePacked("(",Strings.toString(z+1),".",Strings.toString(i+1),")"));
                if (coordinatexy[coordinate] == 2) {
                    pixelColor = visual_map[i+1][z+1];
                    uint32 x = 10 * z;                    
                    uint32 y = 290-10 * i;
                    svg = string(abi.encodePacked(svg, "<rect  width=\"", Strings.toString(10),"\" height=\"10\" fill=\"", colorConvert[pixelColor], "\" x=\"", Strings.toString(x), "\" y=\"", Strings.toString(y),  "\" />"));                                                  
                }               
            }
        }*/
        svg = visualization();
    ...
    function  changeuri_final() public {
        ...
        NFT_condition[1] = nft.tokenURI(1);
    ...
    modifier dividen_owner {
        require(dividen == true, "the nft has not been sale");        
        _;
    }

    function dividefinalart() public dividen_owner payable returns(uint256) {
        bool nftowner = false;
        uint256 own_num = 0; // how many nft you have
        for (uint256 i=0; i< position_done.length; i++){
        ...
        } else if (recentdividentime - startdividentime >= 60*60*24*30){ //60*60*24*30 one month
       ...
       for (uint256 i=0; i< own_nft.length; i++){
        ...
        uint256 recieve_price = dividen_num * price / 1800;  
    ...
    function visualization() public  view returns(string memory){//canvas on chain
        ...
        for (uint32 i=16; i>=1; i--) {
            for (uint32 z=1; z<= 30; z++){
                if (z == 30){
                    sum_visual = string(abi.encodePacked(sum_visual, "|.....| ", visual_map[i][z], "\n"));
                }else if (z < 30){
                    sum_visual = string(abi.encodePacked(sum_visual, "|.....| ", visual_map[i][z]));
                }
    ...
    function checkCoodinatexy(string memory coordinate) public view returns (uint256) { //if coordinate is painted => 2
        return coordinatexy[coordinate];
    }
    ...
    function check_final_url() public view returns(string memory){
        return  NFT_condition[1];
    }
    ...
    function checkvote(address sender ) public view returns(uint256){ //2 means has voted
        return address_vote[sender]; 
    }
    ...
    function checkapprove(uint256 tokenId) public view returns(address){
        return nft.getApproved(tokenId);
    }

    function checksvg() public view returns(string memory){
        return svg;
    }

    function checkpainter_length() public view returns (uint256){
        return painter_record.length;
    }

    
    function checkpainter(uint256 num) public view returns (address){
        return painter_record[num];
    }
...
contract NFT is ERC721, ERC721URIStorage, ERC721Burnable, Ownable {//删ERC721URIStorage里的supportsInterface
    uint256 public tokenId;
    ...
    constructor() ERC721("Co-painting project Test version", "A Co-painting project") {
        tokenId = 2;
    }
    ...
    function safefinalMint(address to)
        public 
        onlyOwner returns(uint256)
    {
        _safeMint(to, 1);
       _setTokenURI(1, tokenURI(1));
       emit print(tokenURI(1));
        return 1;
        
    }

    function setfinalTokenURI() public onlyOwner returns (uint256) {
        _setTokenURI(1, tokenURI(1));
       emit print(tokenURI(1));
    }
    ...
    function setSVG(string memory svg) public{
        s_highImageURI =svgToImageURI(svg);
    }
                        ...
                        Base64.encode(
                            bytes(
                                abi.encodePacked(
                                    ...
                                    '", "description":"A Co-painting project", ',
    ...
    function finalmint(uint256 tokenId, address to, string memory uri ) public onlyOwner returns(uint256){
        _safeMint(to, tokenId);
        _setTokenURI(tokenId, uri);
        return tokenId;
    }
