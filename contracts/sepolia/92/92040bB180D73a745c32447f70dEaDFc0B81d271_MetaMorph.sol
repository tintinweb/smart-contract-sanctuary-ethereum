...
contract MetaMorph is Ownable {
    function getFeeds(
        ...
        uint256[] memory tot = new uint256[](morpheus.length);
        string[] memory totS = new string[](morpheus.length);
       TotSt[] memory totSt = new TotSt[](morpheus.length);
        for (uint256 i = 0; i < IDs.length; i++) {
            ...
            if (block.timestamp - threshold < returnTimestamps) {
                tot[i] = returnPrices / 10 ** returnDecimals;
                totS[i] = returnStr;
                 for (uint j = 0; j < totSt.length; j++) {
                if (CompareStrings(totSt[j].key, returnStr)) {
                    totSt[j].value++;
                    break;
                }
                if (j == totSt.length - 1) { // if not found, add it
                    totSt[j] = TotSt({key: returnStr, value: 1});
                }
            }
            }
