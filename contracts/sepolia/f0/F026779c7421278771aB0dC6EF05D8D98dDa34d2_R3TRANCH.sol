...
contract R3TRANCH is Ownable, ReentrancyGuard, Pausable {
    ...
    function availableRewardOfStake(
        ...
        require(
            stakingBalances[account].length > stakingOrder,
            "Incorrect account or staking order"
        );
        StakeParams memory oneStake = stakingBalances[account][stakingOrder];
        RewardParams[] memory ordersReward = ordersRewards[account][
            stakingOrder
        ];
        ...
        if (tS > oneStake.start) {
            ...
            for (
                uint256 i = 0;
                i < ordersReward.length && ordersReward[i].rewardTS < tS;
                i++
            ) {
    ...
    function availableRewardOfAcount(
        ...
        for (
            ...
            stakingOrder < stakingBalances[account].length && stakingBalances[account][stakingOrder].start < tS;
            stakingOrder++
        ) {
    ...
    function calculateReward(address account) public view returns (uint256) {
        return availableRewardOfAcount(account, block.timestamp);
    }
    ...
    function calculate(
        ...
        for (
            uint256 i = activeStake[account];
            i < stakingBalances[account].length;
            i++
        ) {
    ...
    function calculateBoosts(
        ...
        for (uint256 i = 0; i < boosts[year].length; i++) {
            ...
            if (
                !oneBoost.active ||
                oneBoost.startTS >= endTS ||
                oneBoost.endTS <= countDateStart
            ) {
                continue;
            }
            countStartTS = oneBoost.startTS > countDateStart
                ? oneBoost.startTS
                : countDateStart;
            countEndTS = oneBoost.endTS < endTS ? oneBoost.endTS : endTS;
            reward +=
                ((countEndTS - countStartTS) / DAY) *
                ((stakeAmount * oneBoost.extraPercent) /
                    (100 * DECIMAL_FACTOR));
        }
        return reward;
    }
    ...
    function calculateReward(
        ...
        StakeParams memory oneStake = stakingBalances[account][stakingOrder];
        require(
            countDateStart >= oneStake.start &&
                countDateStart <= oneStake.start + FIVE_YEARS,
            "Wrong countDateStart!"
        );
        require(endTS > countDateStart, "Wrong endTS!");

        uint256 stakeEndTS = oneStake.start + FIVE_YEARS;
        uint256 countDateEnd = countDateStart +
            (((endTS - countDateStart) / DAY) * DAY);
        ...
        uint256 periodEnd;
        for (
            year = (countDateStart - oneStake.start) / YEAR + 1;
            year <= (countDateEnd - oneStake.start - 1) / YEAR;
            year++
        ) {
            periodEnd = year * YEAR + oneStake.start;
            reward +=
                (((((periodEnd - countDateStart) / DAY) * oneStake.amount) /
        ...
        reward +=
            (((((countDateEnd - countDateStart) / DAY) * oneStake.amount) /
                100) * yearPercents[year - 1]) /
            365;
        reward += calculateBoosts(
            account,
    ...
    function getReward() public nonReentrant whenNotPaused {
        ...
        require(
            reward <= r3tContract.balanceOf(address(this)),
    ...
    function withdrawRT(address account, uint256 amount) public onlyOwner {
        require(
            rtContract.balanceOf(address(this)) >= amount,
