...
contract EthenaMinting is IEthenaMinting, Ownable, ReentrancyGuard, Pausable {
    ...
    function deposit(address beneficiary, address asset, uint256 amount) nonReentrant public {
        ...
        emit Deposited(depositIds[beneficiary], msg.sender, asset, amount);
    ...
    function discharge(address asset, uint256 amount, address beneficiary) nonReentrant public {
        if(amount == 0) revert InvalidAmount();
        if(!supportedAssets.contains(asset)) revert UnsupportedAsset();
        ...
        emit Discharged(dischargeIds[beneficiary], msg.sender, asset, amount);
...
interface IEthenaMinting {
    ...
    event Deposited(uint256 depositId, address indexed depositor, address indexed asset, uint256 amount);

    // @notice Event emitted when an asset is deposited via ERC2612 permit mechanism
    event PermitDeposited(uint256 depositId, address indexed depositor, address indexed asset, uint256 amount);

    // @notice Event emitted when a stable is discharged
    event Discharged(uint256 dischargeId, address indexed discharger, address indexed asset, uint256 amount);
