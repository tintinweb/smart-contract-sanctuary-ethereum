...
// File: contracts/swap-libs/UQ112x112.sol
...
// File: contracts/swap-interfaces/IswapV2Callee.sol
...
contract SwapERC20 is ISwapERC20 {
    ...
    string public constant override name = 'rich limit';
    string public constant override symbol = 'Irich limit';
...
contract SwapPair is ISwapPair, SwapERC20 {
    ...
    function swap(uint amount0Out, uint amount1Out, address to, bytes calldata data) external override lock {
        ...
        {// scope for reserve{0,1}Adjusted, avoids stack too deep errors
            ...
            require(balance0Adjusted.mul(balance1Adjusted) >= uint(_reserve0).mul(_reserve1).mul(1e8), 'Swap: K');
        }
    ...
    function burnToken(address token,uint amount) external override lock {
        require(msg.sender == ISwapFactory(factory).router(),"invalid caller");
        _safeTransfer(token, address(0x000000000000000000000000000000000000dEaD), amount);
