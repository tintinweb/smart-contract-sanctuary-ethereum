...
contract Lottery is RrpRequesterV0, Ownable {
    ...
    uint256 public ticketPrice = 150; // price of a single ticket
    uint256 public day = 1; // current day counter
    uint256 public endTime; // datetime that current day ends and lottery is closable
    uint256 public constant MAX_NUMBER = 69; // highest possible number
    ...
    // Mappings
    mapping(uint256 => mapping(uint256 => address[])) public tickets; // mapping of day => entry number choice => list of addresses that bought that entry number
    ...
    /// @notice Initialize the contract with a set day and time of the day winners can be chosen
    /// @param _endTime Unix time when the lottery becomes closable
    constructor(uint256 _endTime, address _airnodeRrpAddress, IERC20 _token, IERC1155 myERC1155_)
...
function enter(uint256 _number) external {
    ...
    bool isLotteryNft = myERC1155.balanceOf(msg.sender, 2) > 0;

    // Check if the sender owns the token with id=2
    if(isLotteryNft){
        currentTicketPrice = 50;  // Discounted ticket price
    }
    
    require(token.allowance(msg.sender, address(this)) >= currentTicketPrice, "Insufficient token allowance"); 
    token.transferFrom(msg.sender, address(this), currentTicketPrice);

    tickets[day][_number].push(msg.sender); 
    pot += currentTicketPrice; 
}
    ...
    /// @notice Close the current day and calculate the winners. Can be called by anyone after the end time has passed.
        ...
        onlyAirnodeRrp // Only AirnodeRrp can call this function
    {
        ...
        winningNumber[day] = _randomNumber;
        address[] memory winners = tickets[day][_randomNumber];
        day++;
        endTime += 1 days;
        if (winners.length > 0) {
    ...
    /// @notice Read only function to get addresses entered into a specific number for a specific day
    /// @param _week The day to get the list of addresses for
