...
contract EventBox {
  ...
  function createEvent(
    ...
    require(events[eventCounter].currentState == EventState.Default, "Take another id!");
  ...
  function getIsFundToEvent(uint _eventId, address _userAddress) public view returns (bool) {
    if(userFunds[_userAddress][_eventId] > 0){
      return true;
    } else{
      return false;
    }
  ...
  function getFundToEvent(uint _eventId, address _userAddress) public view returns (uint) {
      return userFunds[_userAddress][_eventId];
  }
