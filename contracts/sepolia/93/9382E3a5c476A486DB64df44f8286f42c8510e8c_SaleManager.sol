...
library LibTraitManager {
  ...
  function selectRandomTrait(
    ...
    assembly {
      let layerTraits := sload(add(add(layerStorage.slot, mul(layer, 32)), 4))
      let traitsLength := sload(sub(layerTraits, 32))
      let traitsOffset := add(layerTraits, 32)

      let loopRandomValue := randomValue
      let index := 0

      for {

      } lt(index, traitsLength) {

      } {
        let traitWeight := sload(add(traitsOffset, mul(index, 32)))

        if lt(loopRandomValue, traitWeight) {
          traitIndex := index
          break
        }

        loopRandomValue := sub(loopRandomValue, traitWeight)
        traitIndex := add(index, 1)
      }
    }
