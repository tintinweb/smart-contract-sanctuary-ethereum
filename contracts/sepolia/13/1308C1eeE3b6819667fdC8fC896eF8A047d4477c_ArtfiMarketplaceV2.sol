...
import "./interfaces/Artfi-IFixedPrice.sol";
    ...
    ReentrancyGuardUpgradeable
{
    ...
    ArtfiICollectionV2 private _artfiCollection;
     ...
     *@param collection_ address of ERC721 contract
    ...
    function getConfiguration()
        external
        view
        returns (address Manager_, address collection_, address fixedPrice_)
    {
     ...
     */
    function getMakeOfferDetails(
        uint256 makeOfferId_
    ) external view returns (MakeOffer memory makeOfferData_) {
    ...
    function createSale(
        ...
        if ((_artfiManager.isBlocked(msg.sender) == true))
            revert GeneralError("AF:126");
        if (_artfiManager.isPaused() == true) revert GeneralError("AF:128");
        _offerId.increment();
        offerId_ = _offerId.current();

        _offers[offerId_] = Offer(tokenId_, offerType_, OfferState.OPEN);
    }
    ...
    function mintNft(
        ...
        // address minter;
        // if (msg.sender == address(_artfiFixedPrice)) minter = mintData_.minter;
     ...
     *@param lazyMintSellData_  contain token address, uri,seller address,buyer address, uid, creater addresses, royalties percentage, investors addresses, stakeRoyalty percentage, miPrice, purchase quantity, signature, currency.
     *@return offerId_ offerId of nft.
    ...
    function lazyMintSellNft(
        ...
        payable
        returns (
            // OnlyWhiteListed calldata onlyWhiteListed_
            uint256 offerId_,
            uint256 tokenId_
        )
    {
        ...
        if ((_artfiManager.isBlocked(msg.sender) == true))
            revert GeneralError("AF:126");
        // if (
        //   (onlyWhiteListed_.startDate <= block.timestamp) &&
        //   (_artfiCollection.isWhiteListed(msg.sender) == false)
        // ) revert GeneralError("AF:129");
        if (_artfiManager.isPaused() == true) revert GeneralError("AF:128");


        // address signerV2 = _artfiManager.verifyFixedPriceLazyMintV2(lazyMintSellData_);

        // if (lazyMintSellData_.seller != signerV2) {
        //     console.log("Signer & Seller",signerV2, lazyMintSellData_.seller);
     ...
     *@param lazyMintSellData_  contains uri,seller address,creater addresses, royalties percentage, investors addresses, stakeRoyalty percentage,purchase quantity.
     *@return offerId_ offerId of nft.
     *@return tokenId_ Id of token.
     */
    function _lazyMint(
        ...
        uint256 tokenId__ = mintNft(
            MintData(
                ...
                lazyMintSellData_.stakeRoyalty,
                1
            )
        );
        tokenId_ = tokenId__;
        ...
        _offers[offerId_] = Offer(
            tokenId_,
            OfferType.SALE,
            purchaseQuantity < 1 ? OfferState.OPEN : OfferState.ENDED
        );
        ...
        uint256[] memory paymentAmount = new uint256[](
            lazyMintSellData_.stakeRoyalty.length.add(2)
        );
        // uint256 serviceFee = _percent(
        //     (lazyMintSellData_.minPrice).mul(purchaseQuantity),
        //     _artfiManager.serviceFeePercent()
        // );
        {
            uint256 i;
            uint256 stakeRoyaltySum = 0;
            for (i = 0; i < lazyMintSellData_.stakeRoyalty.length; i++) {
                uint256 stakeRoyalty_ = _percent(
                    lazyMintSellData_.minPrice.mul(purchaseQuantity),
                    lazyMintSellData_.stakeRoyalty[i]
                );
                recipientAddresses[i] = lazyMintSellData_.investors[i];
                paymentAmount[i] = stakeRoyalty_;
                stakeRoyaltySum = stakeRoyaltySum.add(stakeRoyalty_);
            }

            // recipientAddresses[i] = _artfiManager.serviceFeeWallet();
        ...
        if (
            ...
            uint256 totalPayment = 
                (lazyMintSellData_.minPrice).mul(purchaseQuantity);
            if (
                !_artfiFixedPrice.isSaleSupportedTokens(
                    lazyMintSellData_.currency
                )
    ...
    function _transferNFT(
        ...
        (
            ...
            bool isOwner,
            uint256 quantity
        ) = _artfiManager.isOwnerOfNFT(from_, tokenId_, tokenAddress_);
        if (!isOwner) revert GeneralError("AF:103");
        if (quantity < quantity_) revert GeneralError("AF:119");
        if (
            (contractType == ContractType.ARTFI_V2 ||
