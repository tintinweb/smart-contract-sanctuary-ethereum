...
contract NFTMarketplace is Ownable, ReentrancyGuard {
    ...
    address private redemptionContract;
    ...
    event UpdateRedemptionContract(address redemptionContract);

    /// @notice Event emitted when the listing fee is updated
    event UpdateListingFee(uint256 listingFee);

    /// @notice Event emitted when the price feed address is updated
    event UpdatePriceFeedContract(address chainLink);
    ...
    function balanceCalculation(
        ...
        uint256 paymentToMarketplace = (priceInWei * listingFee) / 10_000;
    ...
    function updatePriceFeedContract(address chainlink) external onlyOwner {
        if (chainlink == address(0)) {
            revert ZeroAddress();
        }
        priceFeed = AggregatorV3Interface(chainlink);
        emit UpdatePriceFeedContract(chainlink);
    }

    /// @notice Updates the address of the redemption contract
    /// @param _redemptionContract The address of the new redemption contract
    function updateRedemptionContract(
        ...
        emit UpdateRedemptionContract(_redemptionContract);
    }
    ...
    /// @param price The price at which the NFT will be listed expressed in basis points (1 basis point = $0.01)
    function listItem(
        address nftAddress,
        uint256 tokenId,
        uint256 price
    ...
    function redeemProfit(
        ...
        if (msg.sender != redemptionContract) {
