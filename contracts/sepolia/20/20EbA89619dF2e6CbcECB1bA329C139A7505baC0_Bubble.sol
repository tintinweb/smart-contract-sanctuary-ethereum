...
// TODO
// final resilience numbers from Santiago
...
contract Bubble is WildNFT {
    ...
    // mapping from tokenId to resilience (1-999)
    mapping(uint256 => uint8) public tokenToResilience;
    
    // tokenId => dt of last transfer
    uint256[100] public lastTransfer;

    // track if token is dead or alive
    // 0 = alive, 1 = dead
    bool[100] public isDead;

    constructor(address _minter, uint256 _maxSupply, string memory _baseURI, address[] memory _payees, uint256[] memory _shares, uint96 _feeNumerator) WildNFT('Bubbles', 'BUBBLES', _minter, _maxSupply, _baseURI, _payees, _shares, _feeNumerator) {}
    ...
    function _beforeTokenTransfer(address from, address to, uint256 tokenId, uint256 batchSize) internal override(ERC721) {
        ...
        if (from == address(0)) {
            // Create hash value (id, block number, address) & save
            bytes32 mintHash = keccak256(abi.encodePacked(tokenId, blockhash(block.number - 1), to));  
            tokenToHash[tokenId] = mintHash;
            // random 3 digit number under 1000
            uint8 rand = uint8(uint256(keccak256(abi.encodePacked(block.timestamp, block.prevrandao, tokenId))) % 1000);
        ...
        if (from != address(0)) {
            ...
            } else if (isDead[tokenId] == false) {
                // ( Hours Since Last Transfer - 24 - 216 * ( 1 - R ) ) < 0 )  = DEAD
                uint diffHours = (block.timestamp - lastTransfer[tokenId]) / 60 / 60; 
                uint8 tokenResilience = tokenToResilience[tokenId];
                if (diffHours - 24 - 216 * (1 - tokenResilience / 1000) < 0) {
                    isDead[tokenId] = true;
                    emit MetadataUpdate(tokenId);
                }
            }
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        ...
        if (bytes(baseURI).length > 0) {
            ...
            uint8 tokenResilience = tokenToResilience[tokenId];
            imgUrl = string(abi.encodePacked(baseURI, Strings.toString(tokenId), '.png'));
            attributesStr = string(abi.encodePacked(',"attributes":[{"trait_type": "Alive", "value":"', phase, '"}, {"trait_type": "Resilience", "value":"', Strings.toString(tokenResilience), '"}, {"trait_type": "Hash", "value": "', tokenHash, '"}, {"trait_type": "Last Transfer Date Time (UTC)", "value":"', lastTransfer[tokenId], '"} ]'));
            animationUrl = string(abi.encodePacked(baseURI, '/viewer/index.html?tokenId', '=', Strings.toString(tokenId), '&hash=', tokenHash, '&state=', phase));
        }
        string memory json = Base64.encode(bytes(abi.encodePacked('{"name":"', name, '", "description": "', description, '", "image": "', imgUrl, '", "animation_url": "', animationUrl, '", "external_url": "', externalUrl, '"', attributesStr, '}')));
