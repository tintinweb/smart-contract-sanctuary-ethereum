    ...
    DefaultOperatorFilterer
{
    ...
    mapping(uint256 => uint256[]) public fontHash;
    ...
    /// @notice Sets the font hash for the specified token ID.
    /// @param tokenId The ID of the token.
    /// @param hash The font hash for the token.
    function setFontHash(
        uint256 tokenId,
        uint256[] calldata hash
    ) external onlyOwner {
        fontHash[tokenId] = hash;
    }

    /// @notice Retrieves the font hash for the specified token ID.
    /// @param tokenId The ID of the token.
    function getFontHash(
        uint256 tokenId
    ) public view returns (uint256[] memory) {
        return fontHash[tokenId];
    }
    ...
    function keccak256(uint8[][] memory fontHash) public pure returns (bytes32) {
        return keccak256(fontHash);
    }
}
