...
error Withdraw__AmountMustBeSmallerThanContractBalance();
    ...
    ReentrancyGuard
{
    ...
    function updateEmployeeSalary(
        ...
        if (_index >= employeeAddressList.length) revert Index__NotInRange();
        usdAmountArray[_index] = _newSalary;
    }
    ...
    function withdrawEth(uint _balance) public onlyOwner {
        if (address(this).balance < _balance) {
            revert Withdraw__AmountMustBeSmallerThanContractBalance();
        }
    ...
    function fundEthToContract() public payable {}
