...
contract Marketplace721A is ERC721A__IERC721ReceiverUpgradeable, OwnerOperator, VerifySignature {
    ...
    event PaymentTransfer(address from, address to, uint256 tokenId, string indexed paymentType,string unindexedPaymentType);
    struct Order {
    ...
    // TODO Set only owner or operator
    function setTokenAddress(address _tokenaddress) public {
        TokenAddress[_tokenaddress] = true;
    }

    function setCurrencyAddress(address _currencyaddress) public {
    ...
    function sell(
        ...
        Order memory order = Order(_tokenAddress, _tokenId, msg.sender, _orderPrice, _currency);
        orders[orderId] = order;

        IERC721AUpgradeable(_tokenAddress).safeTransferFrom(msg.sender, address(this), _tokenId, '');

        emit CreateSellOrder(orderId, order.owner, order.tokenId, order.orderPrice, order.currency, order.tokenAddress);
    }

    function resale(uint256 _orderId, address _buyer) external operatorOrOwner {
        // check order status
        require(orders[_orderId].owner != address(0), 'Order does not exist or is deleted');

        IERC721AUpgradeable(orders[_orderId].tokenAddress).safeTransferFrom(
            address(this),
            _buyer,
    ...
    function paymentTransferFrom(
        address ERC721A,
        address from,
        address to,
        uint256 tokenId,
        string memory paymentType
    ) public payable operatorOrOwner {
        IERC721AUpgradeable(ERC721A).safeTransferFrom(
            from,
            to,
            tokenId
        );

        emit PaymentTransfer(from, to, tokenId, paymentType, paymentType);
    }
    ...
    function updateOrderPrice(uint256 _orderId, uint256 _newPrice) external {
        require(orders[_orderId].owner != address(0), 'Order does not exist');
        require(orders[_orderId].owner == msg.sender, "Msg sender is not order 's owner");
        require(
        ...
        uint256 oldPrice = orders[_orderId].orderPrice;
        orders[_orderId].orderPrice = _newPrice;
...
abstract contract VerifySignature {
    ...
    function getMessageHash(
        address _to, uint256 _amount, uint256 _tokenId, uint256[] memory _token721Ids
    ) public pure returns (bytes32)
    {
        return keccak256(abi.encodePacked(_to, _amount, _tokenId, _token721Ids));
    }
    ...
    {
        bytes32 messageHash = getMessageHash(_to, _amount, _tokenId, _token721Ids);
