...
struct Layer {
  string name;
  uint16 traitCount;
  mapping(uint16 => Trait) traits; // TODO: Turn to array
}

struct LayerData {
  uint8 layerCount;
  mapping(uint8 => Layer) layers; // TODO: Turn to array
...
library LibTraitManager {
  ...
  function selectRandomTrait(
    ...
    assembly {
      let layerData := sload(layerStorage.slot)
      let traits := sload(add(layerData, 0x20))
      let traitsOffset := add(traits, mul(layer, 0x40))
      let traitsArrayLength := sload(sub(traitsOffset, 0x20))
      for {
        let j := 0
      } lt(j, traitsArrayLength) {
        j := add(j, 1)
      } {
        // Load trait weight from storage
        let traitWeight := sload(add(traitsOffset, add(0x40, mul(j, 0x20))))

        // Compare randomValue with trait weight
        if lt(randomValue, traitWeight) {
          // RandomValue is smaller, found the trait index
          traitIndex := j
          // Exit the loop
          j := traitsArrayLength
        }

        // Subtract trait weight from randomValue
        randomValue := sub(randomValue, traitWeight)
      }
    }
