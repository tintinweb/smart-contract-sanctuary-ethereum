...
contract Contra is ERC721Drop {
    ...
    // Map a contra tokenId to an NFT that lives on another collection
    mapping(uint256 => LinkedNFT) public tokenIdToLinkedNFT;

    // event to emit new set
    event NewContraCard(
        address _collection,
        uint256 _linkedNFTTokenId,
        uint256 _tokenId
    );
    ...
    function setContraCard(
        address _collection,
        uint256 _linkedNFTTokenId,
        uint256 _tokenId
    ) public {
        require(ownerOf(_tokenId) == msg.sender, "caller not owner");
        require(_collection != address(this), "invalid nft");

        // instance another collection
        IERC721 collectionContract = IERC721(_collection);

        // check token ownership
        address tokenOwner = collectionContract.ownerOf(_linkedNFTTokenId);
