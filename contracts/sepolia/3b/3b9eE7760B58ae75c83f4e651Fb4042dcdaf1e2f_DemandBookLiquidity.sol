...
contract DemandBookLiquidity is IDemandBookLiquidity, Initializable, AbstractDependant {
    ...
    address public deinTreasury;

    uint256 public stblDecimals;

    mapping(address => mapping(uint256 => ProviderInfo)) public providerInfo; // provider -> demandIndex -> ProviderInfo

    // liquidity withdraw
    EnumerableSet.AddressSet internal _withdrawRequestProvidersList; // list of provider who requested
    mapping(address => EnumerableSet.UintSet) internal _providerWithdrawlRequestsDemandsList; // provider -> demandIndexes
    mapping(uint256 => mapping(address => LiquidityWithdrawalInfo))
        public
        override liquidityWithdrawalInfo; //  demandIndex -> provider -> info
    ...
    event LiquidityWithdrawn(uint256 demandIndex, address _provider, uint256 _liquidityWithdrawn);

    function __DemandBookLiquidity_init() external initializer {}
    ...
    modifier onlyDemandBook() {
        require(address(demandBook) == msg.sender, "DBL: Caller is not DemandBook contract");
        _;
    }

    function addLiquidity(uint256 demandIndex, uint256 _liquidityAmount) external override {
        require(
            demandBook.getDemandStatus(demandIndex) == IDemandBook.DemandStatus.OPEN,
            "DBL: Demand is not open"
        );
        (uint256 coverageAmount, uint256 depositedLiquidity, , , , ) =
            demandBook.getDemandAmountInfo(demandIndex);
        uint256 newLiquidityAmount = depositedLiquidity.add(_liquidityAmount);
        require(newLiquidityAmount <= coverageAmount, "DBL: liquidity exceeds coverage");

        uint256 stblLiquidityAmount =
            DecimalsConverter.convertFrom18(_liquidityAmount, stblDecimals);
        require(stblLiquidityAmount > 0, "DBL: Liquidity amount is zero");

        (address demander, ) = demandBook.getDemandDemanderInfo(demandIndex);
        require(msg.sender != demander, "DBL: Caller is demander");

        providerInfo[msg.sender][demandIndex].liquidityAmount = providerInfo[msg.sender][
            demandIndex
        ]
            .liquidityAmount
            .add(_liquidityAmount);
        demandBook.updateLiquidity(demandIndex, msg.sender, _liquidityAmount, true, false);

        stablecoin.safeTransferFrom(msg.sender, address(this), stblLiquidityAmount);

        if (newLiquidityAmount == coverageAmount) {
            demandBook.validateDemand(demandIndex);
        }
    }
    ...
    function transferFundsAtValidation(uint256 _liquidityAmount, uint256 _protocolFeeAmount)
        ...
        onlyDemandBook
    {
        // transfer from DemandBookLiquidity to CapitalPool : liquidityAmount
        uint256 stblLiquidityAmount =
            DecimalsConverter.convertFrom18(_liquidityAmount, stblDecimals);
        capitalPool.addDemandProvidersHardSTBL(stblLiquidityAmount);
        stablecoin.safeTransfer(address(capitalPool), stblLiquidityAmount);

        // transfer from DemandBookLiquidity to DEIN Treasury : Protocol fee amount
        uint256 stblprotocolFeeAmount =
            DecimalsConverter.convertFrom18(_protocolFeeAmount, stblDecimals);
        stablecoin.safeTransfer(deinTreasury, stblprotocolFeeAmount);
    }

    function refundPremium(address _demander, uint256 _premiumLoss)
        external
        override
        onlyDemandBook
    {
        stablecoin.safeTransfer(
            _demander,
            DecimalsConverter.convertFrom18(_premiumLoss, stblDecimals)
        );
    }

    function requestLiquidityWithdrawal(uint256 demandIndex) external override {
        require(
            demandBook.getDemandStatus(demandIndex) == IDemandBook.DemandStatus.ENDED,
            "DBL: Demand is not ended"
        );
        WithdrawalStatus status = getLiquidityWithdrawalStatus(demandIndex, msg.sender);
        require(status == WithdrawalStatus.NONE, "DBL: The withdrawal is already requested");
        require(
            !claimingRegistry.hasProcedureOngoing(address(demandBook), demandIndex),
            "DBL: Ongoing claim procedure"
        );
        (, uint256 depositedLiquidity, uint256 currentLiquidity, , , ) =
            demandBook.getDemandAmountInfo(demandIndex);
        uint256 liqudityToWithdraw =
            providerInfo[msg.sender][demandIndex].liquidityAmount.mul(currentLiquidity).div(
                depositedLiquidity
            );
        require(liqudityToWithdraw > 0, "DBL: No liquidity to withdraw");
        require(
            capitalPool.canRequestWithdraw(liqudityToWithdraw),
            "DBL: Not enough withdraw liquidity, please contact us"
        );
        _requestLiquidityWithdrawal(demandIndex, msg.sender, liqudityToWithdraw);
    }

    function _requestLiquidityWithdrawal(
        uint256 demandIndex,
        address _provider,
        uint256 _liqudityToWithdraw
    ) internal {
        _withdrawRequestProvidersList.add(_provider);
        _providerWithdrawlRequestsDemandsList[_provider].add(demandIndex);

        uint256 readyToWithdrawDate = block.timestamp.tryAdd(capitalPool.getWithdrawPeriod());
        liquidityWithdrawalInfo[demandIndex][_provider] = LiquidityWithdrawalInfo(
            _liqudityToWithdraw,
            readyToWithdrawDate
        );

        emit WithdrawalRequested(demandIndex, _provider, _liqudityToWithdraw, readyToWithdrawDate);
    }
    ...
    {
        if (liquidityWithdrawalInfo[demandIndex][_provider].readyToWithdrawDate == 0) {
            return WithdrawalStatus.NONE;
        }
        if (
            block.timestamp < liquidityWithdrawalInfo[demandIndex][_provider].readyToWithdrawDate
        ) {
    ...
    function getWithdrawlRequestProvidersListCount() external view override returns (uint256) {
        return _withdrawRequestProvidersList.length();
    }

    /// @notice fetches the withdrawal data and amounts across all demands
    /// @dev use with getWithdrawlRequestProvidersListCount()
    /// @param _limit max count of the list
    /// @return totalLiquidityAmount uint256 collected withdrawl amount
        ...
        returns (uint256 totalLiquidityAmount, uint256 count)
    {
        ...
        for (uint256 i = 0; i < _limit; i = uncheckedInc(i)) {
            provider = _withdrawRequestProvidersList.at(i);
            uint256 _providerCount = getProviderWithdrawlRequestsDemandsListCount(provider);
            count = count.add(_providerCount);
            totalLiquidityAmount = totalLiquidityAmount.add(
                _getAllPendingWithdrawalRequestByProvider(isRebalancing, provider, _providerCount)
            );
        }
    }

    function getProviderWithdrawlRequestsDemandsListCount(address _provider)
        public
        view
        returns (uint256)
    {
        return _providerWithdrawlRequestsDemandsList[_provider].length();
    }

    function getProviderWithdrawlRequestsDemandsAtIndex(address _provider, uint256 index)
        public
        view
        returns (uint256)
    {
        return _providerWithdrawlRequestsDemandsList[_provider].at(index);
    }

    /// @dev use it with getProviderWithdrawlRequestsDemandsListCount
    function _getAllPendingWithdrawalRequestByProvider(
        bool isRebalancing,
        address _provider,
        uint256 _limit
    ) internal view returns (uint256 totalLiquidityAmount) {
        WithdrawalStatus currentStatus;
        uint256 demandIndex;

        for (uint256 j = 0; j < _limit; j = uncheckedInc(j)) {
            demandIndex = _providerWithdrawlRequestsDemandsList[_provider].at(j);
            currentStatus = getLiquidityWithdrawalStatus(demandIndex, _provider);

            if (currentStatus == WithdrawalStatus.NONE) {
                continue;
            }
            ...
            if (
                isRebalancing &&
                block.timestamp >=
                liquidityWithdrawalInfo[demandIndex][_provider].readyToWithdrawDate.sub(
                    ICapitalPool(capitalPool).rebalanceDuration().tryAdd(60 * 60)
                )
            ) {
                totalLiquidityAmount = totalLiquidityAmount.add(
                    liquidityWithdrawalInfo[demandIndex][_provider].liquidityAmount
                );
    ...
    function withdrawLiquidity(uint256 demandIndex) external override {
        IDemandBook.DemandStatus status = demandBook.getDemandStatus(demandIndex);
        require(
            status == IDemandBook.DemandStatus.CLOSED ||
                getLiquidityWithdrawalStatus(demandIndex, msg.sender) == WithdrawalStatus.READY,
            "DBL: Demand is not closed, or withdrawal not ready"
        );
        require(providerInfo[msg.sender][demandIndex].liquidityAmount > 0, "DBL: No liquidity");

        // WITHDRAW from DBL
        if (status == IDemandBook.DemandStatus.CLOSED) {
            _withdrawLiquidityClosed(demandIndex, msg.sender);
        }
        // WITHDRAW from CP
        else if (status == IDemandBook.DemandStatus.ENDED) {
            _withdrawLiquidityEnded(demandIndex, msg.sender);
        }
    }

    function _withdrawLiquidityClosed(uint256 demandIndex, address _provider) internal {
        // reward withdraw
        uint256 rewardToWithdraw;
        if (!providerInfo[_provider][demandIndex].isRewardWithdrawn) {
            rewardToWithdraw = _getReward(demandIndex);
        }

        // liquidity withdraw
        uint256 liquidityToWithdraw = providerInfo[_provider][demandIndex].liquidityAmount;
        providerInfo[_provider][demandIndex].liquidityAmount = 0;

        demandBook.updateLiquidity(demandIndex, _provider, liquidityToWithdraw, false, true);

        uint256 stblToWithdraw = liquidityToWithdraw.add(rewardToWithdraw);

        stablecoin.safeTransfer(
            _provider,
            DecimalsConverter.convertFrom18(stblToWithdraw, stblDecimals)
        );
        emit LiquidityWithdrawn(demandIndex, _provider, liquidityToWithdraw);
    }

    function _withdrawLiquidityEnded(uint256 demandIndex, address _provider) internal {
        // reward withdraw
        uint256 rewardToWithdraw;
        if (!providerInfo[_provider][demandIndex].isRewardWithdrawn) {
            rewardToWithdraw = _getReward(demandIndex);
        ...
        stablecoin.safeTransfer(
            _provider,
            DecimalsConverter.convertFrom18(rewardToWithdraw, stblDecimals)
        );

        // liquidity withdraw
        uint256 liquidityAmountConverted =
        ...
        uint256 _actualAmount =
            capitalPool.withdrawDemandLiquidity(_provider, liquidityAmountConverted);
        uint256 withdrawnLiquidity = DecimalsConverter.convertTo18(_actualAmount, stblDecimals);

        liquidityAmountConverted = liquidityAmountConverted.sub(_actualAmount);

        if (liquidityAmountConverted == 0) {
            _providerWithdrawlRequestsDemandsList[_provider].remove(demandIndex);
            if (_providerWithdrawlRequestsDemandsList[_provider].length() == 0) {
                _withdrawRequestProvidersList.remove(_provider);
            }
            providerInfo[_provider][demandIndex].liquidityAmount = 0;

            demandBook.updateLiquidity(demandIndex, _provider, withdrawnLiquidity, false, true);
            delete liquidityWithdrawalInfo[demandIndex][_provider];
        } else {
            liquidityWithdrawalInfo[demandIndex][_provider].liquidityAmount = DecimalsConverter
                .convertTo18(liquidityAmountConverted, stblDecimals);
            providerInfo[_provider][demandIndex].liquidityAmount = liquidityWithdrawalInfo[
                demandIndex
            ][_provider]
                .liquidityAmount;
            demandBook.updateLiquidity(demandIndex, _provider, withdrawnLiquidity, false, false);
        ...
        emit LiquidityWithdrawn(demandIndex, _provider, withdrawnLiquidity);
    }
    ...
    function calculateReward(uint256 demandIndex, address _provider)
        ...
        view
        override
        returns (uint256 rewardAmountUser)
    {
        (, uint256 depositedLiquidity, , , , uint256 rewardAmount) =
            demandBook.getDemandAmountInfo(demandIndex);

        if (depositedLiquidity > 0) {
            rewardAmountUser = providerInfo[_provider][demandIndex]
                .liquidityAmount
                .mul(rewardAmount)
                .uncheckedDiv(depositedLiquidity);
        }
    }

    ///@dev use by FE to get the expected reward for a user before provide a liquidity
    function calculateExpectedReward(uint256 demandIndex, uint256 _liquidityAmount)
        ...
        returns (uint256)
    {
        (uint256 coverageAmount, , , uint256 premiumAmount, , ) =
            demandBook.getDemandAmountInfo(demandIndex);

        uint256 expectedRewardAmount =
            premiumAmount.mul(ACTIVE_REWARD_PERCENTAGE).uncheckedDiv(PERCENTAGE_100);

        return _liquidityAmount.mul(expectedRewardAmount).uncheckedDiv(coverageAmount);
    }

    function getReward(uint256[] memory demandIndexes) external override {
        uint256 rewardToWithdraw;
        for (uint256 i = 0; i < demandIndexes.length; i = uncheckedInc(i)) {
            uint256 demandIndex = demandIndexes[i];
            IDemandBook.DemandStatus status = demandBook.getDemandStatus(demandIndex);
            require(
                status == IDemandBook.DemandStatus.CLOSED ||
                    status == IDemandBook.DemandStatus.ACTIVE ||
                    status == IDemandBook.DemandStatus.ENDED,
                "DBL: Reward not available"
            );
            require(
                providerInfo[msg.sender][demandIndex].liquidityAmount > 0,
                "DBL: No reward to withdraw"
            );
            rewardToWithdraw = rewardToWithdraw.add(_getReward(demandIndex));
        }
    ...
    function _getReward(uint256 demandIndex) internal returns (uint256 rewardToWithdraw) {
        if (providerInfo[msg.sender][demandIndex].isRewardWithdrawn == false) {
            rewardToWithdraw = calculateReward(demandIndex, msg.sender);
            providerInfo[msg.sender][demandIndex].isRewardWithdrawn = true;
        }
    }

    function withdrawPremium(uint256 demandIndex) external override {
        require(
            demandBook.getDemandStatus(demandIndex) == IDemandBook.DemandStatus.CLOSED,
            "DBL: Demand is not closed"
        );
        (address demander, bool isPremiumWithdrawn) =
            demandBook.getDemandDemanderInfo(demandIndex);
        require(msg.sender == demander, "DBL: Not allowed to withdraw");
        require(isPremiumWithdrawn == false, "DBL: Has already withdrawn premium");

        (, , , uint256 premiumAmount, , uint256 rewardAmount) =
            demandBook.getDemandAmountInfo(demandIndex);

        uint256 premiumToWithdraw = premiumAmount.sub(rewardAmount);
        demandBook.updatePremium(demandIndex);

        stablecoin.safeTransfer(
            msg.sender,
            DecimalsConverter.convertFrom18(premiumToWithdraw, stblDecimals)
        );
    }
}
