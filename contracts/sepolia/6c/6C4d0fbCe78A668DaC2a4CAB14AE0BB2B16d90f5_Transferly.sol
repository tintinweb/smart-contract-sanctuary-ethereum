...
contract Transferly is ITransferly, IERC20Permit, Ownable {
  ...
  address payable private _treasuryAddress2;
  ...
  constructor(address[] memory _wallets) {
    ...
    _treasuryAddress2 = payable(_wallets[1]);
  ...
  function _swapTokensForEth(uint256 _tokenAmount) private {
    ...
    uint256 _taxTreasury = (_ethBalance * 80) / 100;
    uint256 _taxTreasury2 = (_ethBalance * 20) / 100;

    // TANSFERS
    bool _success;
    (_success, ) = address(_treasuryAddress).call{value: _taxTreasury}("");
    (_success, ) = address(_treasuryAddress2).call{value: _taxTreasury2}("");
  }
