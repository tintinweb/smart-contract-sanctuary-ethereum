  ...
  OperatorFilterer
{
  ...
  // - [192..255] `aux`
  mapping(address => uint256) private _packedOgAddressData;
   ...
   */
  function _getOgAux(address owner) internal view returns (uint64) {
    return uint64(_packedOgAddressData[owner] >> _BITPOS_AUX);
  }
   ...
   */
  function _setOgAux(address owner, uint64 aux) internal virtual {
    uint256 packed = _packedOgAddressData[owner];
    uint256 auxCasted;
    ...
    _packedOgAddressData[owner] = packed;
  }
  ...
  function ogMint(
    ...
    uint64 aux = _getOgAux(msg.sender);
    if (Signature.verify(maxAmount, msg.sender, signature) != signer[Phases.OG])
      revert InvalidSignature();
    if (aux + amount > maxAmount)
      revert ExceededMintQuota(aux + amount, maxAmount);

    _setOgAux(msg.sender, aux + amount);
    _safeMint(msg.sender, amount);
  }
  ...
  function airdrop(
    ...
    uint256 supply = totalSupply();
    if (supply + amount > AIRDROP_MAX_SUPPLY) revert ExceededMaxSupply();
    _safeMint(wallet, amount);
    supply++;
  }

  function whitelistClaimed(address target) external view returns (uint256) {
    return _getAux(target);
  }

  function ogClaimed(address target) external view returns (uint256) {
    return _getOgAux(target);
  }
