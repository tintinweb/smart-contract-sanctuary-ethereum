...
import "./CollectorStorage.sol";
import "../interface/ICollector.sol";
import "../interface/IDexManager.sol";
import "../interface/IAddressMap.sol";
import "../library/AddressHandler.sol";
import "../library/Constant.sol";

import "hardhat/console.sol";

contract Collector is ICollector, CollectorStorage, AddressHandler, Ownable {
  using SafeERC20 for IERC20;

  constructor(IAddressMap addressMap) AddressHandler(addressMap) {}

  function convert(address tokenIn) external onlyOwner {
    uint amountIn = IERC20(tokenIn).balanceOf(address(this));
    if (amountIn > 0) {
      address dm = lookup(ROLE_DEX_MANAGER);
      IERC20(tokenIn).safeTransfer(dm, amountIn);
      uint amount = IDexManager(dm).collectorSwap(tokenIn, amountIn);
      emit Convert(tokenIn, amount);
    }
  }

  function distribute() external onlyRole(ROLE_GOVERNANCE) {
    address zdxt = lookup(TOKEN_ZDXT);
    uint balance = IERC20(zdxt).balanceOf(address(this));
    if (balance > 0) {
      uint communityFee = balance / 3;
      uint RewardFee = balance - communityFee;
      IERC20(zdxt).safeTransfer(lookup(ROLE_COMMUNITY), communityFee);
      IERC20(zdxt).safeTransfer(lookup(ROLE_GOVERNANCE), RewardFee);
    }
    emit Distribute(balance);
  }
}
...
contract CollectorStorage {}

// SPDX-License-Identifier: AGPL-3.0
pragma solidity ^0.8.17;

interface IAddressMap {
  function readAddressList(uint role) external view returns (address);
}
...
interface ICollector {
  event Convert(address tokenIn, uint amount);

  event Distribute(uint amount);

  function convert(address tokenIn) external;
...
interface IDexManager {
  // function deployPool(address asset, uint160 sqrtPriceX96) external payable returns (address pool);
  ...
  function collectorSwap(address tokenIn, uint amountIn) external returns (uint);
...
contract AddressHandler {
  IAddressMap public _addressMap;

  constructor(IAddressMap addressMap) {
    _addressMap = addressMap;
  }

  modifier onlyRole(uint256 role) {
    require(msg.sender == lookup(role), "Only Role");
    _;
  }

  function lookup(uint256 role) internal view returns (address contractAddress) {
    contractAddress = _addressMap.readAddressList(role);
    require(contractAddress != address(0), "lookup error");
  }
}
...
// uint24 constant DEFAULT_SWAP_FEE = 2000; // 0.2%
uint24 constant DEFAULT_SWAP_FEE = 500; // TEMP
uint32 constant DEFAULT_TIME_WEIGHT = 600;
uint constant DEFAULT_DEADLINE = 300;
uint constant DISTRIBUTION_INTERVAL = 60;
uint constant DEFAULT_TIME_FRAME = 60;

uint constant MAX_INT = (2 ** 255 - 1) + 2 ** 255;

uint constant ROLE_FACTORY = 0;
uint constant ROLE_LENDING = 1;
uint constant ROLE_COLLATERAL_MANAGER = 2;
uint constant ROLE_STAKING = 3;
uint constant ROLE_PRICE_ORACLE = 4;
uint constant ROLE_COLLATERAL_ORACLE = 5;
uint constant ROLE_COLLECTOR = 6;
uint constant ROLE_DEX_MANAGER = 7;
uint constant ROLE_GOVERNANCE = 8;
uint constant ROLE_COMMUNITY = 9;

uint constant TOKEN_ZDXT = 10;
uint constant TOKEN_DAI = 11;
...
library Decimal {
  ...
  // NOTE: function thant call function of this library can be tested.
...
struct ShortParams {
  ...
  // address tokenOut;
  // uint24 fee;
  // address recipient; // TODO: remove or use for require
  // uint amountIn; // TODO: remove or use for require
  // uint deadline;
...
// NOTE: it is possible to be revoked even if endPrice is not set at asset.
// REVIEW: check the usecase of end price = 0 and isRevoked = true
