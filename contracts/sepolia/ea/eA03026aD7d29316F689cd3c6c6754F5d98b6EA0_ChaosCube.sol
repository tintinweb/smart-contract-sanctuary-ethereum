...
/*
    ...
    require(_index < AllowedCrypto.length, "Index out of Array Range");
        AllowedCrypto[_index]=
            TokenInfo({
                paytoken: _paytoken,
                costvalue: _costvalue
            });
...
*/
    ...
    function mintWithChaos(uint256 _count) public {
...
/// new mint with chaos coin
    function mintWithChaos() public {
        //require(mintingWithChaos == true, "Minting with ERC20 token is Not Enabled.");
        require(totalSupply() < maxSupply, "Maximum supply reached");
        //require(mintingChaosFee > 0, "Minting with ERC20 token is Not Enabled. mintingChaosFee must be greater than zero.");
        
        tokenAddress.transferFrom(msg.sender, address(this), mintingChaosFee);
...
/*
function mintWithErc20(uint256 _count, uint256 _pid) public {   
//function mintWithErc20(uint256 _count, string memory _symbol) public payable {
 ...
 function mint(address _to, uint256 _mintAmount, uint256 _pid) public payable {
        ...
        require(_mintAmount <= maxMintSize);
...
*/
  ...
  /*  
    function withdrawTokens(uint256 _pid) public payable onlyOwner() {
    //function withdrawTokens(string memory _pid) public payable onlyOwner() {
            TokenInfo storage tokens = AllowedCrypto[_pid];
            //TokenInfo storage tokens = allowedCrypto[_symbol];
            IERC20 paytoken;
            paytoken = tokens.paytoken;
