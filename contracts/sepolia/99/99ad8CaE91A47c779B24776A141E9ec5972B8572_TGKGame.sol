...
contract TGKGame is Ownable, ERC721AQueryable {
    ...
    uint256 internal constant MINT_NUM_ON_STAKE=2;
    uint256 internal constant TEAM_MINT_NUM_ON_START_GAME=8;
    ...
    mapping(uint256 => uint256) gameMaxTotalSupply;
    ...
    function startGame(uint256 gameDuration, uint256 _gameMaxTotalSupply) external onlyOwner {
        require(!isGameLive(), "Game is live");
        require(isGameWinnersSet(gameCounter-1),"Winners not set");
        require(totalSupply()+TEAM_MINT_NUM_ON_START_GAME<=_gameMaxTotalSupply && _gameMaxTotalSupply%2==0,"Invalid max total supply");

        uint256 gameEnd = block.timestamp + gameDuration;
        gameEndTime = gameEnd;
        gameMaxTotalSupply[gameCounter] = _gameMaxTotalSupply;

        _safeMint(teamAddress,TEAM_MINT_NUM_ON_START_GAME);
        emit GameStarted(gameCounter, gameEnd, _gameMaxTotalSupply, totalSupply());
    }

    function stake(uint256 _amount) external {
        require(isGameLive(), "Game not live");
        GameBalance memory stakedBalance = stakedBalances[msg.sender];
        totalStakedAmount+=_amount; //totalStakedAmount = total staked amount
        ...
        if(stakedBalance.amount==0){
            require(totalSupply()+MINT_NUM_ON_STAKE<=gameMaxTotalSupply[gameCounter],"Max tokens minted");
        ...
        emit Staked(gameCounter, msg.sender, _amount, totalSupply());
    }
    ...
    function setWinners(address[] calldata users, uint256[] calldata multipliers) external onlyOwner {
        ...
        uint256 totalWinnersStakedAmount;

        GameWinnerInfo[] storage gameWinnerInfoList = gameWinners[gameCounter];
        for(uint i=0;i<users.length;i++){
            uint256 userStakedAmount = stakedBalances[users[i]].amount;
            winningAmount =  (userStakedAmount * multipliers[i])/10000;
            totalWinnersStakedAmount+=userStakedAmount;
            stakedBalances[users[i]].amount=0;
            claimableAmount[users[i]] += winningAmount;
            gameWinnerInfoList.push(GameWinnerInfo(users[i],userStakedAmount,winningAmount));
            totalWinningAmount+=winningAmount;
            emit GameWinner(gameCounter, users[i], userStakedAmount, winningAmount);
        }

        uint256 totalToGive = totalWinningAmount + (totalStakedAmount-totalWinnersStakedAmount)/2;
        
        require(totalStakedAmount>totalToGive,"Set winners incorrect");
        tokensToBurn = totalStakedAmount - totalToGive;
