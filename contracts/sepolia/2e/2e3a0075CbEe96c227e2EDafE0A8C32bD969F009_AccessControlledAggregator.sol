...
contract FluxAggregator is AggregatorV2V3Interface, Owned {
  ...
  mapping(uint32 => RoundDetails) internal roundDetailsMapping;
    ...
    )
  {
    ...
    if (_queriedRoundId > 0) {
      ...
      RoundDetails storage details = roundDetailsMapping[_queriedRoundId];
      return (
    ...
    private
  {
    ...
    roundDetailsMapping[_roundId] = nextDetails;
    ...
    private
  {
    ...
    delete roundDetailsMapping[_roundId];
  }
    ...
    )
  {
    ...
    if (supersedable(reportingRoundId) && shouldSupersede) {
      ...
      _paymentAmount = roundDetailsMapping[_roundId].paymentAmount;
    ...
    return (
      ...
      round.startedAt,
      roundDetailsMapping[_roundId].timeout,
      recordedFunds.available,
      oracleCount(),
      _paymentAmount
    );
  ...
  function updateRoundAnswer(uint32 _roundId)
    ...
    returns (bool, int256)
  {
    if (roundDetailsMapping[_roundId].submissions.length < roundDetailsMapping[_roundId].minSubmissions) {
      return (false, 0);
    }

    int256 newAnswer = Median.calculateInplace(roundDetailsMapping[_roundId].submissions);
  ...
  function payOracle(uint32 _roundId)
    private
  {
    uint128 payment = roundDetailsMapping[_roundId].paymentAmount;
    Funds memory funds = recordedFunds;
    ...
    private
  {
    ...
    roundDetailsMapping[_roundId].submissions.push(_submission);
  ...
  function deleteRoundDetails(uint32 _roundId)
    private
  {
    if (roundDetailsMapping[_roundId].submissions.length < roundDetailsMapping[_roundId].maxSubmissions) return;
    ...
    returns (bool)
  {
    ...
    uint32 roundTimeout = roundDetailsMapping[_roundId].timeout;
  ...
  function acceptingSubmissions(uint32 _roundId)
    ...
    returns (bool)
  {
    return roundDetailsMapping[_roundId].maxSubmissions != 0;
  }
