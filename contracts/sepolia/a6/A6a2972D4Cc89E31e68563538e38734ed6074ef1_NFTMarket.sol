...
contract NFTMarket is ReentrancyGuard {
    ...
    address napaToken;
    address usdtToken;
    CoBatchingPool coBatchingPool;
    address coBatchingContract;

    constructor(
        address _napaToken,
        address _usdtToken,
        address _coBatchingPool
    ) {
        napaToken = _napaToken;
        usdtToken = _usdtToken;
        coBatchingPool = CoBatchingPool(_coBatchingPool);
        coBatchingContract = _coBatchingPool;
    }
    ...
    function setSale(
        ...
        // if(_isCobatchable){
        //     require(_paymentMode==PaymentMode.napaToken,"co-batch only supports for NAPA tokens");
        // }
    ...
    function buyNFTToken(uint256 itemId) public payable nonReentrant {
        ...
        if (
            ...
            (
                ...
                bool active,
                uint256 resaleId
            ) = coBatchingPool.pooldata(pollId);

            // to buy a Co-batched NFT by (1. Co-batch)
            if (admin == msg.sender && active == true) {
                ...
                for (uint256 i = 0; i < memberlength; i++) {
                    ...
                    uint256 membertoken = ((idMarketItem[itemId].price *
                        totalPerscentage) / 100) / 100;
                    IERC20(napaToken).transfer(member[i], membertoken);
                }
                idMarketItem[itemId].owner = payable(coBatchingContract);
                idMarketItem[itemId].sold = true;
                _itemsSold.increment();
                coBatchingPool.setData(itemId, pollId);
            }
            // to buy a Co-batched NFT by (1. Single buy)
                ...
                idMarketItem[itemId].sold = true;
                _itemsSold.increment();
                coBatchingPool.setResaleValue(itemId);
            }
        }
        // to buy NFT by (1. Co-batch)
        else if (
            coBatchingPool.activePoll(pollId) == true &&
            paymentMode == PaymentMode.napaToken
        ) {
            (address admin, , , uint256 neededContribution, , ) = coBatchingPool
                .pooldata(pollId);
            require(neededContribution == 0, "Not get a require token");
            require(
                admin == msg.sender,
                "You are not a pool creator of this pool"
            );
            uint256 price = idMarketItem[itemId].price;
            uint256 tokenId = idMarketItem[itemId].tokenId;
            IERC721(nftContract).transferFrom(
            ...
            idMarketItem[itemId].sold = true;
            _itemsSold.increment();
            IERC20(napaToken).transfer(idMarketItem[itemId].seller, price);

            coBatchingPool.setsellData(itemId, pollId);
        }
        // to buy a single NFT Single buy
            ...
            idMarketItem[itemId].sold = true;
            _itemsSold.increment();
            feeTransfer(itemId, paymentMode);
        }
    }

    function feeTransfer(uint256 _tokenId, PaymentMode paymentMode) internal {
        if (paymentMode == PaymentMode.napaToken) {
            IERC20(napaToken).transferFrom(
                msg.sender,
                idMarketItem[_tokenId].seller,
                (idMarketItem[_tokenId].price)
            );
            ...
            require(
                ...
                "Please send sufficient ETH"
            );
            (idMarketItem[_tokenId].seller).transfer(
                idMarketItem[_tokenId].price
            );
        }
    }
    ...
    function reSellToken(
        ...
        if (idMarketItem[itemId].owner == coBatchingContract) {
            uint256 pollId = coBatchingPool.returnPollId(itemId);
            (uint256 newprice, bool active) = coBatchingPool.isResaleActive(
                pollId
            );
            require(active == true, "Your Poll is not Active For Re-sale");
            address nftContract = idMarketItem[itemId].nftContract;
            uint256 tokenid = idMarketItem[itemId].tokenId;

            idMarketItem[itemId].sold = false;
            idMarketItem[itemId].price = newprice;
            idMarketItem[itemId].seller = payable(coBatchingContract);
            ...
            idMarketItem[itemId].sold = false;
            idMarketItem[itemId].price = price * 10**18;
    ...
    function _endPool(uint256 itemId) public {
        ...
        require(admin == msg.sender, "You're not a Pool admin");
        uint256 returnPrice = totalPrice - contribution;

        bool success = IERC20(napaToken).transfer(
            coBatchingContract,
            returnPrice
        );

        if (success) {
            coBatchingPool.endPool(pollId, itemId);
        } else {
    ...
    function fetchCoBatchNFTs() public view returns (MarketItem[] memory) {
        ...
        for (uint256 i = 0; i < totalItemCount; i++) {
            if (idMarketItem[i + 1].owner == coBatchingContract) {
                itemCount += 1;
            }
        }
    ...
    function fetchPoolItems() public view returns (MarketItem[] memory) {
        uint256 currentIndex = 0;
        (uint256[] memory poolDetails) = coBatchingPool.fetchPoolDetails(msg.sender);
...
contract CoBatchingPool is Ownable {
    ...
    function fetchVotedStatus(uint256 _poolId)
        public
        view
        returns (address[] memory, bool[] memory)
    {
        address[] memory members = participantList[_poolId];
        bool[] memory votedStatus = new bool[](members.length);

        for (uint256 i = 0; i < members.length; i++) {
            address participant = members[i];
            votedStatus[i] = participants[_poolId][participant].voted;
        }
    ...
    {
        uint256 totalPools = _pollId.current();
        uint256 Count = 0;
        ...
        returns (address[] memory _participants)
    {
        ...
        for (uint256 i = 1; i <= totalPools; i++) {
            _participants = participantList[i];
            Count += 1;
        }
    }
}
