...
contract MainContract {
    ...
    TransfersAndDisputes public transfersAndDisputes;

    AggregatorV3Interface private priceFeed;
    address private propertyNftContractAddress;
    address private tenantManagerAddress;
    address private transfersAndDisputesAddress;
    ...
    mapping(uint256 => RentContract[]) propertyIdToRentHistory;
    mapping(uint256 => uint256) tenantCurrentContractId;

    constructor(address _propertyNftContractAddress, address _tenantManagerAddress, address _priceFeedAddress, address _transfersAndDisputesAddress) {
        propertyNftContractAddress = _propertyNftContractAddress;
        propertyNFT = PropertyNft(propertyNftContractAddress);
        transfersAndDisputesAddress = _transfersAndDisputesAddress;
        transfersAndDisputes = TransfersAndDisputes(transfersAndDisputesAddress);
    ...
    function acceptRentContract(uint256 _propertyNftId, uint256 _rentContractId) external onlyPropertyOwner(_propertyNftId) {
        ...
        propertyIdToRentHistory[_propertyNftId].push(rentContractIdToRentContract[_rentContractId]);
    ...
    function terminateRentContract(uint256 _propertyNftId, uint256 _rentContractId) external {
        if (
            ...
            transfersAndDisputes.createDispute(_rentContractId, "Rent Contract terminated");
        } else {
    ...
    function getTenantRentHistory(uint256 sbtTokenId) public view returns (RentContract[] memory rentContracts) {
        rentContracts = sbtTokenIdToRentHistory[sbtTokenId];
    ...
    function getPropertyRentHistory(uint256 propertyId) public view returns (RentContract[] memory rentContracts) {
        rentContracts = propertyIdToRentHistory[propertyId];
        return rentContracts;
    }
...
contract TransfersAndDisputes {
    ...
    function transferSecurityDeposit(uint256 _propertyNftId, uint256 _rentContractId, uint256 _amount) external payable {
        ...
        if (!success) {
            revert TransfersAndDisputes__TransferFailed();
        }
        rentContractIdToAllowedDepositRelease[_rentContractId] = false;
        rentContractIdToDeposit[_rentContractId] = rentContractIdToDeposit[_rentContractId] + _amount;
    ...
    function transferRent(uint256 _propertyNftId, uint256 _rentContractId, uint256 _amount) external payable {
        MainContract.RentContract memory rentContract = mainContract.getRentContract(_rentContractId);
        MainContract.Property memory property = mainContract.getProperty(_propertyNftId);
        if (rentContract.status != MainContract.RentContractStatus.Confirmed || rentContract.propertyNftId != property.propertyNftId) {
            revert MainContract__InvalidRentContract();
        ...
        nftTokenIdToBalance[_propertyNftId] = nftTokenIdToBalance[_propertyNftId] + _amount;
        rentContractIdToAmountOfPaidRent[_rentContractId] = rentContractIdToAmountOfPaidRent[_rentContractId] + _amount;
        uint256 paymentId = numberOfPayments;
        Payment storage payment = paymentIdToPayment[paymentId];
        payment.id = paymentId;
        payment.timestamp = block.timestamp;
    ...
    function allowDepositRelease(uint256 _rentContractId) external {
        ...
        if (msg.sender == owner) {
            ...
            revert MainContract__AllowDepositReleaseFailed(msg.sender, _rentContractId);
        }
    }
