...
contract OneEightyICO is ContextUpgradeable, OwnableUpgradeable {
    ...
    function depositDai(uint256 _amount) public payable {
        ...
        uint256 currentPrice = uint256(latestPrice);
        uint256 perDai = currentPrice / 100;
        uint256 _daiamount = perDai * _amount;
        uint256 perToken = tokenusdprice.mul(_daiamount);
        uint256 swapToken = perToken;
        rewardToken.transfer(msg.sender, swapToken);
    }
    ...
    function depositUsdc(uint256 _amount) public payable {
        ...
        uint256 currentPrice = uint256(latestPrice);
        uint256 perUsdc = currentPrice / 100;
        uint256 _usdcamount = perUsdc * _amount;
