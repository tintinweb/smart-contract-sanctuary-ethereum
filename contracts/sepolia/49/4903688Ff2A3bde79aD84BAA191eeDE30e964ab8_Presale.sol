...
contract Vesting is Ownable, ReentrancyGuard {
    ...
    constructor(address _tokenAddress) {
        ...
        // require(_owner != address(0), "Presale: wallet is the zero address");
    }
    ...
    //                           Get Claim
    // =============================================================
    function getClaim(bytes32[] calldata _proof, address _beneficiary) public {
        ...
        if (schedule.multipleRemainingSet > 0) {
            ...
            if (schedule.lastRevoked > 0) {
                uint weeksPassed = weeksSince(schedule.lastRevoked);
                if (weeksPassed > 1) {
                    schedule.multipleRemainingSet = weeksPassed - 1;
                } // console.log("inside last else", weeksPassed);
            }
        }
        updateSchedule(_proof, _beneficiary, schedule);
    }
    ...
    function claim(
        ...
        bytes32 _leaf = keccak256(abi.encodePacked(beneficiary, lockedAmount));
        require(
    ...
    function getRoot() external view onlyOwner returns (bytes32) {
        return _root;
    }

    function _hasClaimed(address user) external view returns (bool) {
...
contract Presale is Context, ReentrancyGuard, Ownable {
    ...
    //                            Modifier to Add
    // =============================================================
    // modifier Validate(address beneficiary, uint256 weiAmount) {
    //     require(_finalized != false, "Pre-Sale: ICO must be active");
    //     require(
    ...
    uint256 private _rate; //rate = 2, 1 usdt == 2 PMDG, 2*10**18
    ...
    constructor(
        ...
        tokenVesting = Vesting(_vestingContract);
    }
     ...
     */
    function buyTokensFromEth(
        address beneficiary,
        uint256 amount
    ) public payable nonReentrant {
        _preValidatePurchase(beneficiary, amount);
        require(
            _eth.transferFrom(beneficiary, _wallet, amount),
            "ETH Transfer Failed"
        );
        ...
        tokenVesting.createVestingSchedule(beneficiary, tokens);
        tokensSold = tokensSold.add(tokens);
        _isWhitelisted[beneficiary] = true;
        // ********************************
    ...
    function _getTokenFromEth(
        ...
        uint256 ethPriceInUSDT = uint256(getLatestPriceEth());
        uint256 rates = _rate.mul(ethPriceInUSDT.div(10 ** 8));
        uint256 conversion = weiAmount.mul(rates);
        return conversion;
    }
    ...
    function _getTokenFromUsdt(
        ...
        // uint256 usdPriceInUsdt = uint256(getLatestPriceUsdt());
        // uint256 rates = _rate.mul(usdPriceInUsdt.div(10 ** 8));
        uint256 usdPrice = usdtAmount.div(10 ** 6);
        uint256 conversion = usdPrice.mul(_rate);
        return conversion;
    }
    ...
    function _getTokenFromBnb(
        ...
        //  uint256 bnbPriceInUSDT = uint256(getLatestPriceBnb());
        uint256 bnbPriceInUSDT = 27700000000;
        uint256 rates = _rate.mul(bnbPriceInUSDT.div(10 ** 8));
        uint256 conversion = bnbAmount.mul(rates);
        return conversion;
    }
    ...
    receive() external payable {
        // undo change
        if (_finalized) {
            buyTokensFromEth(_msgSender(), msg.value);
        } else {
