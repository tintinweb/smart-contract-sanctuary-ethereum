...
//-------------------------------------------DOCUMENTS-----------------------------------------------------//
 contract STO_ERC20 is ERC20, Pausable, Ownable {
...
//-------------------------------------------DOCUMENTS-----------------------------------------------------//
  struct Doc {
    ...
    uint256 count = 1;
    event DocumentUpdated(bytes32 documentName, string uri, bytes32 documentHash);
    event Notarized(address indexed sender, bytes32 hash);
...
//--------------------------------------------MODIFIER-----------------------------------------------------//
    modifier onlyMinter() {
    require(_minter == msg.sender, "You are not the minter");
    _;
  }
//------------------------------------------CONSTRUCTOR----------------------------------------------------//
    ...
    function setControllers(address[] memory initialControllers) private onlyMinter{
    
    for (uint256 i = 0; i < initialControllers.length; i++ ){
    address _initialControllers = initialControllers[i];    
    require(_initialControllers != address(0), "invalid controller");
    require(!_isController[_initialControllers], "owner is not unique");
    _isController[_initialControllers] = true;  
    _controllers.push(_initialControllers);
    }
...
//------------------------------------TOKEN STANDARD FUNCTIONS----------------------------------------------//    
    ...
    //---------------------------------DOCUMENTS------------------------------------//
    function notarize(bytes32 hash) private onlyMinter{
        require(!notarized[hash], "Hash already notarized.");
        notarized[hash] = true;
        emit Notarized(msg.sender, hash);

        _docHashes.push(hash);

        _indexOfDocHashes[hash] = count;
        count = count+1;
    }
    ...
    function setDocument(bytes32 documentName, string calldata uri, bytes32 documentHash) external onlyMinter {
    ...
    function getHases () external view returns(bytes32[] memory) {
        return _docHashes;
    }

    function check(bytes32 hash) public view returns (bool) {
        return notarized[hash];
    }

    function calculateHashFile(bytes memory file) public pure returns (bytes32) {
    return sha256(file);
    }

    function calculateHashString(string memory _data) public pure returns(bytes32) {
