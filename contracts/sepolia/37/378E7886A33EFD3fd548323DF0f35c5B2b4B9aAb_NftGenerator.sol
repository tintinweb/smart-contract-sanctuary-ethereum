...
contract NftGenerator is ERC721Enumerable, Authorizable {
  ...
  function setTokenURI(uint256 _tokenId, string memory _tokenURI) external onlyOwnerOrController {
    require(
      _exists(_tokenId),
      "ERC721Metadata: URI query for nonexistent token."
    );
  ...
  function setTokenURIBatch(uint256[] memory _tokenIds, string[] memory _imgUris) external onlyOwnerOrController {
    require(_tokenIds.length == _imgUris.length, "Inconsistent length");
    for (uint256 i = 0; i < _tokenIds.length; i++) {
      this.setTokenURI(_tokenIds[i], _imgUris[i]);
    }
  }
}