...
contract AutoBD {
    ...
    error DataLengthNotCorrect();

    event Commited(
        bytes32 indexed commitedHash,
    ...
    // This one is for sepolia testnet
        IAPI3 internal constant iAPI3Token =
        IAPI3(0xD5c6e7676aa6753930F5576D69F809094BfA294f);

    // This one is for mainnet + local testing on mainnet fork
/*     IAPI3 internal constant iAPI3Token =
        IAPI3(0x0b38210ea11411557c13457D4dA7dC6ea731B88a); */

    address internal constant airnode =
        0xf64C92bb13a9Ac7EE3448cD45398A33cE85634F1; // Mock Airnode address
    ...
    function commit(
        ...
        if (data.length != 32) revert DataLengthNotCorrect();
        if (timestamp > block.timestamp || timestamp + 60 <= block.timestamp) {
            revert OutOfDatePrice();
        }
        if (
            (keccak256(abi.encodePacked(templateId, timestamp, data))).recover(
                signature
            ) != airnode
    ...
    function reveal(
        ...
        emit Revealed(commitHash, commit_);
    }
}
