...
contract FlexMinter is Ownable {
     ...
     * @dev The unlock time is determined by dividing the timestamp by 1 week and rounding it down to the nearest week.
     *      If the timestamp is at the start of an epoch (00:00 UTC on Thursday), the unlock time is calculated by adding
     *      the duration to the start of the epoch. Otherwise, the unlock time is calculated by adding the duration to
     *      the current epoch's end, which is rounded up to the nearest week.
    ...
    function calculateExpectedUnlock(uint256 ts) public pure returns (uint256) {
        uint256 tsEpochStart = (ts / 1 weeks) * 1 weeks;
        uint256 tsEpochEnd = ts == tsEpochStart
            ? tsEpochStart
            : tsEpochStart + 1 weeks;
        return tsEpochEnd + LOCK_DURATION;
    }
    ...
    function mintWithFlex(uint256 amountFlex) external {
        ...
        uint256 amountOpn = 100 * amountFlex;
        if (oxToken.totalSupply() + amountOpn > oxToken.MAX_MINTABLE_SUPPLY())
