...
contract Lumalee is IERC20 {
    ...
    function getContractFunctions() public pure returns (string memory) {
        return
            "name(), symbol(), decimals(), totalSupply(), balanceOf(), transfer(), transferFrom(), approve(), allowance(), increaseAllowance(), decreaseAllowance(), transferOwnership(), excludeFees(), includeFees(), isExcludedFromFees(), enableAntiBot(), disableAntiBot(), addToBlacklist(), removeFromBlacklist(), isBlacklisted(), getOwner(), getContractFunctions()";
    }
}