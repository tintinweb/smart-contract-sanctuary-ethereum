...
contract Odds is EIP712, DefaultOperatorFilterer, ERC721Royalty, Ownable, CantBeEvil(LicenseVersion.PUBLIC) {
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory result) {
        if (tokenId == 0 || tokenId > _totalMinted + _totalReserved) {
            revert();
        }
        ...
        if (spread == 1) {
            _spectrum = "Hyper Rainbow";
        } else if (steps == 200 && spread == 11 && segments == 14) {
            _spectrum = "Perfect Spectrum";
        } else if (steps == 200 && (spread == 14 || spread == 15) && (segments == 18 || segments == 19)) {
        ...
        if (bytes(_liveImagePrefix).length > 0) {
            imageTrait =
                string.concat('"image": "', _liveImagePrefix, Strings.toString(uint256(squiggleData)), '.png",');
        }
