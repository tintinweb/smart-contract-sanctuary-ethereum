...
contract LottoFactory {
    ...
    LottoToken lottoToken = new LottoToken(address(this));
    // address to number
    mapping(address => uint256[]) walletLotto;
    // address to number mapping to tokenId
    mapping (address => mapping(uint256 => uint256)) walletTokenId;

    // token id to MAX_NUMBER
    mapping(uint256 => uint256) lotto;
    ...
    function getWalletLotto(address _owner)public view returns(uint256[]memory){
        return walletLotto[_owner];
    }

    function getWalletTokenId(address _owner , uint256 _number)public view returns(uint256){
        return walletTokenId[_owner][_number];
    }

    function getLotto(uint256 _tokenId)public view returns(uint256){
        return lotto[_tokenId];
    }

    function ranReward1(uint256 _installment)external IsAdmin(){
    ...
    function ranReward2(uint256 _installment)external IsAdmin(){
    ...
    function ranReward3(uint256 _installment)external IsAdmin(){
        for(uint256 i=0;i<5;i++){
            uint256 reward = uint256(keccak256(abi.encodePacked(block.timestamp,block.prevrandao,msg.sender))) % MAX_NUMBER;
    ...
    function ranReward4(uint256 _installment)external IsAdmin(){
        for(uint256 i=0;i<5;i++){
            uint256 reward = uint256(keccak256(abi.encodePacked(block.timestamp,block.prevrandao,msg.sender))) % MAX_NUMBER;
    ...
    function ranReward5(uint256 _installment)external IsAdmin(){
        for(uint256 i=0;i<5;i++){
            uint256 reward = uint256(keccak256(abi.encodePacked(block.timestamp,block.prevrandao,msg.sender))) % MAX_NUMBER;
    ...
    function buyLotto(address _buyer, uint256 _number , string memory _uri) external payable {
        ...
        walletLotto[_buyer].push(_number);
        walletTokenId[_buyer][_number] = tokenId;
    }

    function checkReward(address _owner,uint256 _installment,uint256 _number)public view returns(string memory, bool){
        uint256 tokenId = walletTokenId[_owner][_number];
        require(IERC721(address(lottoToken)).ownerOf(tokenId) == msg.sender);
        if (_number == reward1[_installment]){
            return (rewardMessage[0], true);
        }
        for(uint256 i =0;i<reward2[_installment].length;i++){
            if (_number == reward2[_installment][i]){
                return (rewardMessage[1], true);
            }
        }
        for(uint256 i =0;i<reward3[_installment].length;i++){
            if (_number == reward3[_installment][i]){
                return (rewardMessage[2], true);
            }
        }
        for(uint256 i =0;i<reward4[_installment].length;i++){
            if (_number == reward4[_installment][i]){
                return (rewardMessage[3], true);
            }
        }
        for(uint256 i =0;i<reward5[_installment].length;i++){
            if (_number == reward5[_installment][i]){
                return (rewardMessage[4], true);
            }
        }
        return ("",false);
    }
...
contract LottoToken is ERC721, ERC721URIStorage, AccessControl {
    ...
    constructor(address LottoFactory) ERC721("LottoToken", "MTK") {
        _grantRole(DEFAULT_ADMIN_ROLE, msg.sender);
        _grantRole(MINTER_ROLE, LottoFactory);
    }
