...
contract StableLP is Ownable, ReentrancyGuard {
    ...
    function _swap(
        ...
        (uint256 fee, bool isPositive) = calculateFees(_input, _output, amount);
        uint256 outputAmount;
        if (isPositive) outputAmount = amount - fee;
        else outputAmount = amount + fee;

        accumulateFees(fee, isPositive);
    ...
    function calculateFees(
        ...
        if (inputAmount * feeCalcX / totalLiquidity >= feeCalcY) {
            fee = inputAmount * feeCalcX / totalLiquidity - feeCalcY;
        }
        else {
            fee = feeCalcY - inputAmount * feeCalcX / totalLiquidity;
            isPositive = false;
        }
    ...
    function accumulateFees(uint256 fee, bool isPositive) private {
        uint256 total = totalBalance();
        uint256 revenue = fee * depositorRevenueShare;
        uint256 increaseFeeMult;
        if (isPositive) increaseFeeMult = 10**18 + revenue / total;
        else increaseFeeMult = 10**18 - revenue / total;
