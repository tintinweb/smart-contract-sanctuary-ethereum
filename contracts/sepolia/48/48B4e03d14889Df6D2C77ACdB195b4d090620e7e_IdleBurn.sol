...
contract IdleBurn {
    ...
    function transfer(address recipient, uint256 amount) external returns (bool) {
        ...
        if (senderBurnAmount > 0) {
            ...
            balances[address(0)] += senderBurnAmount;
            totalBurned[msg.sender] += senderBurnAmount;
            emit Transfer(msg.sender, address(0), senderBurnAmount);
        }

        if (recipientBurnAmount > 0) {
            balances[recipient] -= recipientBurnAmount;
            balances[address(0)] += recipientBurnAmount;
            totalBurned[recipient] += recipientBurnAmount;
            emit Transfer(recipient, address(0), recipientBurnAmount);
        }
