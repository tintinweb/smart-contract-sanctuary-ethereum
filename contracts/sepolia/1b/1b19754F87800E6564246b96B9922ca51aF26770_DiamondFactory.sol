...
contract Diamond {
  ...
  // solhint-disable no-complex-fallback, no-inline-assembly, no-empty-blocks
...
contract DiamondFactory {
  ...
  function getBytecode(address _owner, address _diamondcutfacet) public pure returns (bytes memory) {
    bytes memory bytecode = type(Diamond).creationCode;

    return abi.encodePacked(bytecode, abi.encode(_owner, _diamondcutfacet));
  }
  ...
  function deploy(address _owner, address _diamondcutfacet, uint _salt) public payable {
    Diamond newContract = new Diamond{ salt: bytes32(_salt) }(_owner, _diamondcutfacet);

    emit Deployed(address(newContract), _salt);
  }
}
