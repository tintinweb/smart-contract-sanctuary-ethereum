...
contract DigiMonkzStaking is Ownable {
    ...
    function gen1Stake(uint16 _tokenId) external returns (bool) {
        ...
        NftInfo memory stakingNft = NftInfo(_tokenId, block.timestamp, 0);
    ...
    function claimRewardWithGen1(
        ...
        for (uint256 i = 0; i < len; i++) {
            require(genesis111.ownerOf(_tokenList[i]) == msg.sender);
            sum += artifactPerGen1Nft[_tokenList[i]];
            artifactPerGen1Nft[_tokenList[i]] = 0;
        }
    ...
    function claimRewardWithGen2(
        ...
        uint256 len = _tokenList.length;
        for (uint256 i = 0; i < len; i++) {
            require(genesis2.ownerOf(_tokenList[i]) == msg.sender);
            sum += artifactPerGen2Nft[_tokenList[i]];
            artifactPerGen2Nft[_tokenList[i]] = 0;
        }
        require(sum >= _numArtifact);
