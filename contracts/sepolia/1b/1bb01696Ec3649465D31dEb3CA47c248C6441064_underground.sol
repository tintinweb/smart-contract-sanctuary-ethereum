...
contract underground is AccessUpgradeable, ERC721EnumerableUpgradeable, DefaultOperatorFiltererUpgradeable {
    ...
    function _beforeTokenTransfer(
        ...
        if (msg.sender != owner() && from != address(0)) {
            ...
            require(block.timestamp >= mintTimestamp[from], "Transfer is restricted due to mint time constraint");
        }
    ...
    function devMint(address to, LockDuration lockDuration) external onlyOwner {
        require(mintTimestamp[to] == 0, "You have already minted a token");

        mintTimestamp[to] = block.timestamp + getLockDuration(lockDuration);

        _mint(to, totalSupply() + 1);
    }

    function mint(bool discount, bytes32[] calldata merkleProof) external payable {
        require(mintTimestamp[msg.sender] == 0, "You have already minted a token");
        ...
        mintTimestamp[msg.sender] = block.timestamp + getLockDuration(msg.value, discount);
