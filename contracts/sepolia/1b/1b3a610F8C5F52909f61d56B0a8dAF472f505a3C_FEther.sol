...
contract FEther is TokenBase {
    ...
    function repayBorrowMax() external payable {
        // Params: payer, borrower, repay amount
        repayBorrowInternal(msg.sender, msg.sender, type(uint256).max);
    }
    ...
    function repayBorrowBehalfMax(address _borrower) external payable {
        // Params: payer, borrower, repay amount
        repayBorrowInternal(msg.sender, _borrower, type(uint256).max);
    }
    ...
    function doTransferIn(address payable _from, uint256 _amount) internal override {
        ...
        if (msg.value > _amount) _from.transfer(msg.value - _amount);
