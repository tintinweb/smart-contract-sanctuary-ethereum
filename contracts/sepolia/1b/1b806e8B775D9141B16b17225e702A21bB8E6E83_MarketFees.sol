...
contract MarketFees is Ownable {
	...
	uint16 private s_marketFeeBasisPoints;
	uint256 private s_marketAccumulatedFees;
	...
	// Market fee related events
	event MarketFeeSet(uint16 marketFeeBasisPoints);
	 ...
	 */
	function contributeFees(address collection, uint256 fromAmount) external payable {
		uint256 marketFees = (fromAmount * s_marketFeeBasisPoints) /
		...
		s_marketAccumulatedFees += marketFees;
		s_accumulatedFees[collection] += royaltyFees;
	}
	...
	/**
	 * @notice Set the platform's market fees.
	 * @param feeBasisPoints The market fees in basis points.
	 */
	function setMarketFees(uint16 feeBasisPoints) external onlyOwner {
		if (feeBasisPoints > MAX_FEE_BASIS_POINTS) revert MarketFees__FeeIsOverTheLimit();

		s_marketFeeBasisPoints = feeBasisPoints;
		emit MarketFeeSet(feeBasisPoints);
	}
	...
	function withdrawMarketFees() external onlyOwner {
		uint256 amount = s_marketAccumulatedFees;
		s_marketAccumulatedFees = 0;
	...
	function withdrawRoyalties(address collection) external {
		...
		uint256 amount = s_accumulatedFees[collection];
		s_accumulatedFees[collection] = 0;
	...
	/**
	 * @notice Get the market fees in basis points.
	 * @return The market fees in basis points.
	 */
	function getMarketFeesBasisPoints() external view returns (uint16) {
		return s_marketFeeBasisPoints;
	}
	 ...
	 */
	function getFeesAmount(address collection, uint256 fromAmount) external view returns (uint256) {
		uint256 marketFees = (fromAmount * s_marketFeeBasisPoints) /
			BASIS_POINTS_PER_HUNDREED_PERCENT;
		uint256 royaltyFees = (fromAmount * getTotalRoyaltyBasisPoints(collection)) /
			BASIS_POINTS_PER_HUNDREED_PERCENT;
	 ...
	 */
	function getAccumulatedFeesForCollection(address collection) external view returns (uint256) {
		return s_accumulatedFees[collection];
	}
	...
	function getAccumulatedMarketFees() external view returns (uint256) {
		return s_marketAccumulatedFees;
	}
