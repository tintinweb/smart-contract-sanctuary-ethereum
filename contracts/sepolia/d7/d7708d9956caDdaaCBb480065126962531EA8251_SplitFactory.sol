...
/// @title Split factory contract.
/// @notice Used to deploy Split contracts.
contract SplitFactory is AccessControl {
    ...
    /// Split implementation address.
    Split public immutable contractImplementation;

    /// SplitFactory contract version.
    ...
    /// Split creation data struct.
    struct SplitCreateData {
        ...
        Split.RecipientData[] recipients;
        /// Creation id.
        bytes32 creationId;
    }

    /// Emitted when a new Split is deployed.
    event NewSplit(
    ...
    /** @notice Creates SplitFactory contract.
     * @dev Deploys current Split implementation contract
     * and sets its address to `contractImplementation`.
     */
    constructor() {
        contractImplementation = new Split();
     ...
     */
    function createSplit(SplitCreateData memory _data) external returns (Split) {
    ...
    function predictDeterministicAddress(
        SplitCreateData memory _data,
        address _deployer
    ) external view returns (Split) {
        ...
        return Split(payable(predictedAddress));
    }
...
/// @notice The main function of Split is to redistribute tokens
/// (whether they are ERC-20 or native cryptocurrency), to the participants
/// based on the percentages assigned to them.
contract Split is AccessControlEnumerableUpgradeable {
