...
contract OnlineSafetyGame is VRFConsumerBaseV2, AutomationCompatible {
    ...
    event WinnersPayed(address indexed winner1, address indexed winner2, address indexed winner3);
    event OwnerPayed(address indexed owner);
    event LuckyWinnersPayed(address payable[] luckyPlayers);
    ...
    Players_data private first;
    Players_data private second;
    Players_data private third;

    // Variable to store the end time of the game
     ...
     */
    function enterGame() external payable { /*Doesnt need public visibility */
        if (msg.value.getConversionRate() < i_entranceFee) {
            revert Game__SendMoreToEnterGame(i_entranceFee, msg.value.getConversionRate());
        }
        ...
        playersData[msg.sender] = Players_data(msg.sender, 0, 0, 0, true);
     ...
     * @param speed The speed factor to be added to the score
     */
    function updatePlayerScore(uint256 score, uint256 speed) external {
        if (!playersData[msg.sender].isPlaying) {
            revert Game__GameIsOver();
        }

        uint256 calculatedScore = (score * 1000) + speed;
        playersData[msg.sender].score =
            playersData[msg.sender].score +
            calculatedScore;
        assignRanking(calculatedScore, msg.sender);
    }
    ...
    function fulfillRandomWords(
        ...
        distributeBalance(s_winners, randomWords[0]);
    }
     ...
     * @param randomNumber A random number used for selecting lucky players
     */
    function distributeBalance(
        address payable[] memory accounts,
        uint randomNumber
    ) internal {
        require(accounts.length >= 10, "Insufficient number of accounts");

        uint256 totalBalance = address(this).balance;

        uint256 firstPlaceAmount = (totalBalance * 20) / 100;
        uint256 secondPlaceAmount = (totalBalance * 14) / 100;
        uint256 thirdPlaceAmount = (totalBalance * 8) / 100;
        uint256 ownerAmount = (totalBalance * 30) / 100;
        uint256 sharedAmount = totalBalance -
            (firstPlaceAmount +
                secondPlaceAmount +
                thirdPlaceAmount +
                ownerAmount);
        uint256 sharedAmountPerAccount = sharedAmount / 7;

        require(totalBalance > 0, "No balance to distribute");
        require(
            sharedAmountPerAccount > 0,
            "Insufficient balance for shared positions"
        );
        ...
        address payable[] memory luckyPlayers = selectRandomPlayers(randomNumber);

        bool luckyPay;
        for (uint256 i = 0; i < luckyPlayers.length; i++) {
            (luckyPay, ) = luckyPlayers[i].call{value: sharedAmountPerAccount}(
                ""
            );
            if (!luckyPay) {
        ...
        emit LuckyWinnersPayed(luckyPlayers);
    }

    /**
     * @notice Selects random players from the player list
     * @dev Requires a minimum of 7 players and selects 7 players using the provided random number
     * @param randomNumber A random number used for player selection
     * @return selectedPlayers An array of payable addresses representing the selected players
     */
    function selectRandomPlayers(
        uint256 randomNumber
    ) internal view returns (address payable[] memory) {
        require(s_players.length >= 7, "Insufficient number of players");

        address payable[] memory selectedPlayers = new address payable[](7);
        bool[] memory playerSelected = new bool[](s_players.length);

        uint256 selectedCount = 0;
        uint256 currentIndex = randomNumber % s_players.length;

        while (selectedCount < 7) {
            if (!playerSelected[currentIndex]) {
                selectedPlayers[selectedCount] = s_players[currentIndex];
                playerSelected[currentIndex] = true;
                selectedCount++;
            }

            currentIndex = (currentIndex + 1) % s_players.length;
        }
     ...
     */
    function assignRanking(uint256 score, address addr) internal {
        
        if (playersData[msg.sender].hasPlayed > 7) {
            playersData[msg.sender].isPlaying = false;
            revert Game__GameHasBeenPlayed7x();
        }

        Players_data memory currPlayer;

        if (playersData[msg.sender].hasPlayed == 0) {
            currPlayer = Players_data(addr, score, 0, 1, true);
        } else {
            currPlayer = playersData[msg.sender];
    ...
    function getAWinner(uint256 index) public view returns (address) {
