...
//
// Deployed to Sepolia 0xF35BbE7F8895298Da4229400C2b0b7e9ad0dEc96
//
// TODO:
...
contract DexzBase {
    ...
    event TradeSummary(PairKey indexed pairKey, BuySell buySell, Account indexed taker, Price price, Tokens filled, Tokens quoteTokensFilled, Tokens tokensOnOrder);
...
contract Dexz is DexzBase, ReentrancyGuard {
    ...
    function _trade(Info memory info, MoreInfo memory moreInfo) internal returns (Tokens filled, Tokens quoteTokensFilled, Tokens tokensOnOrder, OrderKey orderKey) {
                ...
                if (Unixtime.unwrap(order.expiry) == 0 || Unixtime.unwrap(order.expiry) >= block.timestamp) {
                    ...
                    if (info.buySell == BuySell.Buy) {
                        ...
                        if (_availableTokens > 0) {
                            ...
                            emit Trade(TradeInfo(moreInfo.pairKey, bestMatchingOrderKey, info.buySell, moreInfo.taker, order.maker, bestMatchingPrice, stdw.tokensToTransfer, stdw.quoteTokensToTransfer));
                        } else {
                            stdw.deleteOrder = true;
                        }
                    } else {
        ...
        if (Tokens.unwrap(filled) > 0 || Tokens.unwrap(quoteTokensFilled) > 0) {
            ...
            emit TradeSummary(moreInfo.pairKey, info.buySell, moreInfo.taker, price, filled, quoteTokensFilled, tokensOnOrder);
        }
    }
