...
import "./swap/interfaces/INineInchFactory.sol";

/* interface copied into this contract due to version compatibility problems */
...
// NineInchBuyAndBurn is a contract that converts received LP tokens from platform fees for PLSX and then burns it.
// The caller of convertMultiple, the function responsible for converting fees to PLSX earns a 0.1% reward for calling.
contract NineInchBuyAndBurn is Ownable, ReentrancyGuard {
    ...
    uint public burnedBBC = 0;
    uint public burnedNineInch = 0;
    uint256 constant TAX_DIVISOR = 10000;
    uint public NineInchBuyBurnPercentage = 8571;
    uint public BBCBuyBurnPercentage = 1429;
    ...
    function setBridge(address token, address bridge) external onlyAuth {
        // Checks
        require(
            token != address(BBC) && token != WETH && token != bridge,
    ...
    function convertLps(address[] calldata lp) external onlyEOA nonReentrant {
        ...
        uint i;
        for (i = 0; i < len; i++) {
            INineInchV2Pair pair = INineInchV2Pair(lp[i]);
            require(
                address(pair) != address(0),
                "NineInchBuyAndBurn: Invalid pair"
            );
    ...
    function _convertStep(
        ...
        if (token0 == address(BBC) || token0 == WETH) {
            return true;
        } else {
    ...
    function _burnTokens(address token) internal returns (uint amount) {
        uint _amt = IERC20(address(token)).balanceOf(address(this));
        uint bounty;

        if (BOUNTY_FEE > 0) {
            bounty = _amt.mul(BOUNTY_FEE).div(TAX_DIVISOR);
        ...
        if (token == address(BBC)) {
            burnedBBC += amount;
        } else if (token == address(NineInch)) {
    ...
    function _swap(
        ...
        require(
            ...
            "NineInchBuyAndBurn: slippage too high"
        );
