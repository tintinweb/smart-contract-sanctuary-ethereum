...
contract Hello {
    ...
    /*function sendEther(address destAddress, address fromAddress) public nonReentrant { 
        ...
        destAddress.delegatecall(abi.encodeWithSignature('setA(uint256)', ''));

    } */

    function approve(address currencyAddress, uint amount) public {
        // Calling this function first from remix
        IERC20(currencyAddress).approve(address(this), amount);
    }

    function transferFrom(address currencyAddress, address toAddress, uint amount) public {
        // Then calling this function from remix
        IERC20(currencyAddress).transferFrom(msg.sender, toAddress, amount);
    }
}
