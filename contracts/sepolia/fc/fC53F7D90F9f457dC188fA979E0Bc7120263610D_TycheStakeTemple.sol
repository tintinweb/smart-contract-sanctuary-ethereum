...
contract TycheStakeTemple is ERC721Holder, Ownable, ReentrancyGuard {
    ...
    function withdrawOne(uint256 _tokenId) external nonReentrant {
        ...
        UserInfo storage user = userInfo[msg.sender];
        require(user.tokenIds.length > 0, "You don't have any staked NFTs!");
        (bool isExist, uint256 index) = _checkTokenId(user.tokenIds, _tokenId);
        require(isExist, "The token ID is not exist.");
        user.amount = user.amount.sub(1);
        stakedSupply = stakedSupply.sub(1);
        if (user.tokenIds.length > 1 && index < user.tokenIds.length - 1) {
            for(uint256 i = index; i < user.tokenIds.length - 1; i++) {
    ...
    function _checkTokenId(uint256[] memory _ids, uint256 _id) internal pure returns(bool, uint256) {
        uint256 index = 0;
        bool isExist = false;
        for (uint256 i = 0; i < _ids.length; i++) {
            if (_ids[i] == _id) { isExist = true; index = i; break; }
        }
