...
contract NFTAuction is ReentrancyGuard, Ownable {
    ...
    uint256 public constant MIN_BID_PRICE = 0.000016 ether;
    ...
    function testPlaceBid(address[] memory users, uint256 baseAmount) public nonReentrant {
        for (uint i = 0; i < users.length; i++) {
            uint256 amount = baseAmount + (i * 0.005 ether);
            userBids[users[i]] += amount;
            emit BidPlaced(users[i], amount);
        }
    }
