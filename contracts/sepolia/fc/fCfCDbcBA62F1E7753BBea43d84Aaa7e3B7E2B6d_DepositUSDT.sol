...
contract DepositUSDT is Ownable {
    ...
    function DepositUSDTs(uint256 amount, address _from)
        public
        payable
        returns (string memory)
    {
        ...
        usdt.transferFrom(_from, address(this), amount);

        uint256 _idTransaction = TransactionCount++;
        idToListTransaction[_idTransaction].idTransaction = _idTransaction;
        idToListTransaction[_idTransaction].from = _from;
        idToListTransaction[_idTransaction].value = amount;
        idToListTransaction[_idTransaction].name = "DepositUSDT";

        idToTotalDepositUsers[_from].from = _from;
        idToTotalDepositUsers[_from].total += amount;
    ...
    function WithdrawUSDTUsers(uint256 amount) public returns (bool) {
        ...
        usdt.transfer(msg.sender, amountWithdraw);

        uint256 _idTransaction = TransactionCount++;
        idToListTransaction[_idTransaction].idTransaction = _idTransaction;
        idToListTransaction[_idTransaction].from = msg.sender;
        idToListTransaction[_idTransaction].value = amount;
        idToListTransaction[_idTransaction].name = "WithdrawUSDTs";
    ...
    function GetBalanceofUSDT() external view returns (uint256) {
        return usdt.balanceOf(address(this));
    }

    function WithdrawUSDTOwner(uint256 amount) public onlyOwner returns (bool) {
