...
contract NFTAuction {
    ...
    function createAuction(
        ...
        require(IERC721(auctionParams.assetAddress).ownerOf(auctionParams.assetRecordId) == msg.sender, "You must own the NFT to create an auction");
        IERC721(auctionParams.assetAddress).approve(address(this), auctionParams.assetRecordId);
        
        Auction storage newAuction = auctions[auctionCount];

        newAuction.auctionRecordId = auctionCount;
        newAuction.title = auctionParams.title;
        newAuction.description = auctionParams.description;
        newAuction.assetAddress = auctionParams.assetAddress;
        newAuction.assetRecordId = auctionParams.assetRecordId;
        newAuction.startingPrice = auctionParams.startingPrice;
        newAuction.reservePrice = auctionParams.reservePrice;
        newAuction.minimumIncrement = auctionParams.minimumIncrement;
        newAuction.expiryTime = block.timestamp + auctionParams.duration;
        newAuction.distributionCut = auctionParams.distributionCut;
