    ...
    IVRFMigration
{
    ...
    function processVRFOutputs(
        ...
        validateProducer
        whenNotPaused
        returns (bool containsNewOutputs)
    {
        ...
        for (uint256 i = 0; i < vrfOutputs.length; i++) {
            ...
            bool isNewOutput = processVRFOutput(
                r,
                blockHeight,
                juelsPerFeeCoin
            );
            containsNewOutputs = containsNewOutputs || isNewOutput;
    ...
    function processVRFOutput(
        ...
        uint64 blockHeight,
        uint192 /* juelsPerFeeCoin */
    ) internal returns (bool isNewOutput) {
        ...
        if (output.vrfOutput.p[0] == 0 && output.vrfOutput.p[1] == 0) {
            ...
            bytes32 currentSeed = s_seedByBlockHeight[
                getSeedByBlockheightKey(
                    output.blockHeight,
                    output.confirmationDelay
                )
        ...
        bool outputContainsNewCallbacks = processCallbacks(
            numCallbacks,
            seed,
            output
        );
        isNewOutput = isNewOutput || outputContainsNewCallbacks;
    ...
    function processCallbacks(
        ...
        bytes32 seed,
        VRFOutput memory output
    ) internal returns (bool outputContainsNewCallbacks) {
        // Struct used to avoid stack-too-deep error.
        ProcessCallbackData memory callbackData = ProcessCallbackData({
            fulfilledRequests: new uint256[](numCallbacks),
            successfulFulfillment: new bytes(numCallbacks),
            errorData: new bytes[](numCallbacks),
            errorCount: 0,
            subBalances: new uint96[](numCallbacks),
            subIDs: new uint256[](numCallbacks)
        });
        ...
        for (uint256 j = 0; j < numCallbacks; j++) {
            ...
            if (isErr) {
                callbackData.errorData[callbackData.errorCount] = errmsg;
                callbackData.errorCount++;
            } else {
                callbackData.successfulFulfillment[j] = bytes1(uint8(1)); // succeeded
            }
            outputContainsNewCallbacks =
                outputContainsNewCallbacks ||
                !unknownCallback;
            callbackData.fulfilledRequests[j] = callback.callback.requestID;
            callbackData.subBalances[j] = subBalance;
            callbackData.subIDs[j] = callback.callback.subID;
        }

        if (output.callbacks.length > 0) {
            bytes[] memory truncatedErrorData = new bytes[](
                callbackData.errorCount
            );
            for (uint256 j = 0; j < callbackData.errorCount; j++) {
                truncatedErrorData[j] = callbackData.errorData[j];
            }
            emit RandomWordsFulfilled(
                callbackData.fulfilledRequests,
                callbackData.successfulFulfillment,
                truncatedErrorData,
                callbackData.subBalances,
                callbackData.subIDs
            );
        }

        return outputContainsNewCallbacks;
    }
        ...
        )
    {
        ...
        Callback memory cb = c.callback;
        {
            ...
            if (cbCommitment != s_callbackMemo[cb.requestID]) {
                return (true, "unknown callback", sub.balance, true);
            }
        }
        BeaconRequest memory request;
        {
            // scoped to avoid stack too deep error
        ...
        if (seed == bytes32(0)) {
            return (true, bytes("unavailable randomness"), sub.balance, false);
        }
        ...
        return
            success
                ? (false, bytes(""), sub.balance, false)
                : (true, bytes("execution failed"), sub.balance, false);
    }
