...
contract Notary is Ownable, KeeperCompatibleInterface {
    ...
    function updateAssetsAndPortfolio(
        ...
        Portfolio(portfolioAddress).updateAssets(_assetsAddress, _targetWeights, _decimals, _priceFeeds);
        Vault vault = Vault(vaultAddress);
        uint256 numUsers = vault.getUsers().length;
        for (uint256 j = 0; j < numUsers; j++) {
            if (vault.getStrategy(vault.getUsers()[j]) == Portfolio.STRATEGY.DYNAMIC_MODEL) {
...
library BasketLib {
        ...
        returns (uint256 balance)
    {
        balance = self.tokenAmts[token].getConversionRate(
            self.priceFeedBasket[token], self.decimals[token], _priceFeedBenchmark
        );
    }
...
library PriceConverter {
    ...
    function getConversionRate(
        ...
        uint256 tokenAmountInUsd = (tokenPrice * tokenAmount * 1e18) / (10 ** decimals * getPrice(priceFeedEth));
        return tokenAmountInUsd;
    }

    // 1000000000
    function getConversionRateOfBasket(Basket storage self, AggregatorV3Interface priceFeedEth)
        internal
        view
        returns (uint256 price)
    {
        ...
        for (uint256 i = 0; i < length; ++i) {
            price += getConversionRate(
                self.tokenAmts[tokens[i]], self.priceFeedBasket[tokens[i]], self.decimals[tokens[i]], priceFeedEth
            );
            // or (Both will do the same thing)
...
contract Vault is Ownable {
    ...
    function addOneCollateral(address _tokenAddress, uint256 _amount, uint8 _decimal, address _priceFeed) public {
        IERC20(_tokenAddress).transferFrom(msg.sender, address(this), _amount);
        s_collateral[msg.sender].add(
            IERC20(_tokenAddress), _amount, _decimal, AggregatorV3Interface(_priceFeed), s_priceFeedBenchmark
        );
        if (s_users.length == 0) {
        ...
        returns (uint256[] memory)
    {
        ...
        for (uint256 i = 0; i < length; i++) {
            ...
            uint256 tokenPrice = PriceConverter.getPrice(s_collateral[_user].priceFeedBasket[token]);
            uint256 tokenValueInUSD = PriceConverter.getConversionRate(
                tokenBalance, s_collateral[_user].priceFeedBasket[token], decimal, s_priceFeedBenchmark
            );
            ...
            if (positiveRemainingCollateralInDecimals <= tokenValueInUSD) {
                ...
                tokenAmountstoSend[i] = (positiveRemainingCollateralInDecimals * 10 ** decimal) / tokenPrice;
                // Transfer the remaining collateral to the debtor then get out of loop
                console.log(tokenValueInUSD - positiveRemainingCollateralInDecimals);
                console.log(positiveRemainingCollateralInDecimals);
                ...
                // );
                tokenAmountstoSend[i] = (tokenValueInUSD / tokenPrice) * 10 ** decimal;
                console.log(tokenAmountstoSend[i]);
