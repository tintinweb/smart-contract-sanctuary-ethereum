...
contract Buying {
    ...
    mapping(string => uint256) public s_itemsToPrice;
    ...
    function buy(string memory _itemName) public payable {
        ...
        if (msg.value > itemPrice) {
            uint256 refundAmount = msg.value - itemPrice;
            payable(msg.sender).transfer(refundAmount);
        }
    }

    function withdraw() public OwnershipAccess {
        address[] memory sendersAddress = s_sendersAddress;

        for (uint256 i = 0; i < sendersAddress.length; i++) {
            address sender = sendersAddress[i];
            s_peopleWhoBought[sender] = 0;
        }

        s_sendersAddress = new address[](0);
