...
contract R3TRANCH is Ownable, ReentrancyGuard, Pausable {
    ...
    uint256[5] public YEAR_PERCENTS = [150, 100, 50, 50, 50];
...
/*     function findPercentIndex(uint256 date, uint256 index) internal view returns (uint256) {
    ...
    function calculateReward(address account, uint256 stakingOrder, uint256 countDateStart, uint256 endTS) internal view returns (uint256) {
        ...
        uint256 i;
        uint256 periodEnd;
        for(i = (countDateStart - oneStake.start) / YEAR + 1; i < (countDateEnd - oneStake.start) / YEAR + 1; i++) {
            periodEnd = i * YEAR + oneStake.start;
            reward += (periodEnd - countDateStart) / DAY * oneStake.amount / 100 * YEAR_PERCENTS[i - 1];
            reward += calculateBoosts(account, stakingOrder, countDateStart, periodEnd, i);
            countDateStart += YEAR;
        }
        reward += (countDateEnd - countDateStart) / DAY * oneStake.amount / 100 * YEAR_PERCENTS[i - 1];
        return reward;
