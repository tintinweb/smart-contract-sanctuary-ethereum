	...
	ReentrancyGuardUpgradeable
{
	...
	/**
	 * @notice Mapping token ID => status of token
	 */
	...
	/**
	 * @notice Emit event when set base token URI
	 */
	event SetBaseTokenURI(string oldBaseURI, string newBaseURI);
	 ...
	 */
	event SetStatus(uint256 indexed tokenId, TokenStatus indexed tokenStatus);
	...
	function _checkValidParams(
		...
		require(_to != address(0) && _params.owner != address(0), "Invalid address");
		require(
			_params.amount > 0 &&
			_params.price > 0 &&
		...
		if (_params.paymentToken == address(0)) {
			if (_msgSender() == _params.owner) {
				require(value == 0, "Invalid value");
			} else {
				require(value >= _params.amount, "Invalid price or amount");
			}
		}
		require(verifyMint(_to, verifier, _params, _signature), "Invalid signature");
	}
	...
	function setStatus(
		...
		require(_exists(_tokenId), "Nonexistent token");
		require(
			verifySetStatus(verifier, _tokenId, _status, _signature),
			"Invalid signature"
		);
		statusOf[_tokenId] = _status;
		emit SetStatus(_tokenId, _status);
	}
	...
	function setBaseURI(string memory _baseURI) external onlyOwner {
		...
		baseURI = _baseURI;
		emit SetBaseTokenURI(oldBaseURI, baseURI);
	}
	...
	function mint(
		...
		_checkValidParams(_to, _params, _signature, value);
	 ...
	 *  @param  _params.tokenURI        	Token URI
	 *  @param  _params.typeToken         	Type of token
	 *  @param  _signatures                 Signature of transaction
	 *
	 *  Emit event {Transfer(address(0), _to, tokenId)}
	 */
	function mintBatch(
		...
		for (uint256 i = 0; i < _params.length; i++) {
			_checkValidParams(_tos[i], _params[i], _signatures[i], value);
			_beforeMint(_params[i], _signatures[i]);
			_safeMint(_tos[i], lastId.current());
			_setTokenRoyalty(
				lastId.current(),
				_params[i].royaltyReceiver,
				_params[i].royaltyPercent
			);
			_handleTransfer(
				_msgSender(),
				_params[i].owner,
				_params[i].paymentToken,
				_params[i].price
			);
			if (_params[i].paymentToken != address(0)) {
				_handleTransfer(
					_msgSender(),
					address(this),
					_params[i].paymentToken,
					_params[i].amount - _params[i].price
				);
			}
		}
	}
	...
	function transfer(address _to, uint256 _tokenId) external {
		require(_exists(_tokenId), "Nonexistent token.");
		require(statusOf[_tokenId] == TokenStatus.Active, "Can not transfer this token");
	...
	function _beforeMint(
		...
		typeOf[tokenId] = _params.tokenType;
...
interface IERC721 is IERC721MetadataUpgradeable {
	...
	function mintBatch(address[] calldata _tos, MintParams[] calldata params, bytes[] calldata signatures) external;

	function setMaxBatch(uint256 _maxBatch) external;
...
contract VerifySignature {
    ...
    */
	...
	function getMessageHashSetStatus(
		uint256 _tokenId,
		TokenStatus _status
	) public pure returns (bytes32) {
		return keccak256(abi.encodePacked(_tokenId, _status));
	}
    ...
    */
	...
	function verifySetStatus(
		address _signer,
		uint256 _tokenId,
		TokenStatus _status,
		bytes memory _signature
	) public pure returns (bool) {
		bytes32 messageHash = getMessageHashSetStatus(_tokenId, _status);
		bytes32 ethSignedMessageHash = getEthSignedMessageHash(messageHash);
