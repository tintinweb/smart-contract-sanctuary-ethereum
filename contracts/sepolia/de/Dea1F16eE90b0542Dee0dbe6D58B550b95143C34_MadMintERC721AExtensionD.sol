...
import {AdvancedConfig, MadMintStorage} from "../libraries/MadMintStorage.sol";
import {MerkleProofUpgradeable} from "@openzeppelin/contracts-upgradeable/utils/cryptography/MerkleProofUpgradeable.sol";
import {IERC721} from "@openzeppelin/contracts/token/ERC721/IERC721.sol";

contract MadMintERC721AExtensionD is MadMintERC721AUpgradeable {
    ...
    // Address of the MadMint admin address
    address public constant madmintAdminAddress =
    ...
    /**
     * @notice Allows the madmintAdminAddress to set the madmint fee per token
     * @param _madmintFeePerToken The new fee per token in wei
     */
    function setMadmintFeePerToken(uint256 _madmintFeePerToken) external {
        require(msg.sender == madmintAdminAddress, "MUST_BE_MADMINT_ADMIN");
        MadMintStorage.state().data.madmintFeePerToken = _madmintFeePerToken;
    }

    // ============ MADMINT DEPOSIT TOKEN REDEMPTION ============
    ...
    function remainingDepositPaymentInWei() public view returns (uint256) {
        return
        uint256(MadMintStorage.state().advCfg.remainingDepositPayment) *
        10 ** 13;
    }
    ...
    function setDepositMerkleRoot(
        ...
        MadMintStorage.state().advCfg.depositMerkleRoot = _depositMerkleRoot;
    }

    /**
     * @notice Set the address of the MadMint deposit contract eligible for burning to mint
    ...
    function fundingTargetInWei() public view returns (uint256) {
        return uint256(MadMintStorage.state().cfg.fundingTarget) * 10 ** 16;
    }
    ...
    function setLoaningActive(bool _loaningActive) external onlyOwner {
        MadMintStorage.state().advCfg.loaningActive = _loaningActive;
    }
    ...
    function totalLoaned() public view returns (uint256) {
        return MadMintStorage.state().data.currentLoanTotal;
    }
    ...
    function loanedBalanceOf(address _owner) public view returns (uint256) {
        return MadMintStorage.state().data.totalLoanedPerAddress[_owner];
    }
    ...
    function loanedTokensByAddress(
        ...
        for (
            ...
            if (MadMintStorage.state().data.tokenOwnersOnLoan[i] == _owner) {
    ...
    function refundPriceInWei() public view returns (uint256) {
        return uint256(MadMintStorage.state().advCfg.refundPrice) * 10 ** 13;
    }
    ...
    function setRefundAddress(address _refundAddress) external onlyOwner {
        ...
        MadMintStorage.state().advCfg.refundAddress = _refundAddress;
    }
    ...
    function increaseRefundEndsAt(uint32 _newRefundEndsAt) external onlyOwner {
        ...
        MadMintStorage.state().advCfg.refundEndsAt = _newRefundEndsAt;
    }
