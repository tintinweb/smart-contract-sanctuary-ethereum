...
contract upfrontFactory is ReentrancyGuard {
  ...
  function tokenMultiTransfer(address token, address[] memory target, uint256[] memory amount) public payable isInitialized nonReEntrant {
    ...
    unchecked {
      price = PRICE_DISTRIBUTION + (target.length * PRICE_DISTRIBUTION_PER_WALLET);
  ...
  function tokenMultiMint(address token, address[] memory target, uint256[] memory amount) public payable isInitialized nonReEntrant {
    require(target.length == amount.length, "Factory: Invalid number of params.");
    ...
    unchecked {
      for (uint256 a; a < cnt; a++) { iface.mint(target[a], amount[a]); }
    }
  }
