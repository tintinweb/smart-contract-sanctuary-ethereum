...
/// @notice the next beacon output, then call `redeemRandomness` with the uint256
    ...
    IVRFMigration
{
    ...
    CallbackConfig public s_callbackConfig;

    event CallbackConfigSet(CallbackConfig newConfig);

    function setCallbackConfig(CallbackConfig memory config)
        external
        onlyOwner
    {
        s_callbackConfig = config;
        emit CallbackConfigSet(config);
    }
    ...
    {
        (
            uint256 requestID, // BeaconRequest. We do not store this, because we trust the committee
            ,
        ...
        s_callbackMemo[requestID] = keccak256(
            abi.encode(nextBeaconOutputHeight, confDelay, subID, callback)
        );
        ...
        RandomnessLogPayload memory log = RandomnessLogPayload({
            ...
            confirmationDelayArg: confDelay,
            subID: subID,
            numWords: numWords,
    ...
    mapping(uint256 => bytes32) internal s_callbackMemo;

    function getCallbackMemo(uint256 requestId) public view returns (bytes32) {
    ...
    function redeemRandomness(
        ...
        uint256 blockHeight = r.slotNumber * i_beaconPeriodBlocks;
        uint256 blockNumber = ChainSpecificUtil.getBlockNumber();
        uint256 confThreshold = blockNumber - r.confirmationDelay;
        if (blockHeight >= confThreshold) {
            revert BlockTooRecent(
                blockHeight,
                blockHeight + r.confirmationDelay + 1
            );
        }
    ...
    function getSeedByBlockheightKey(uint256 blockHeight, uint24 confDelay)
        internal
        pure
        returns (uint256)
    {
        return uint256((blockHeight << 24) | confDelay);
    }
    ...
    function processVRFOutputs(
        ...
        uint64 blockHeight
    ) external override validateProducer whenNotPaused {
    ...
    function processCallbacks(
        ...
        uint256[] memory fulfilledRequests = new uint256[](numCallbacks);
        ...
        )
    {
        ...
        uint32 slotNum = uint32(blockHeight / i_beaconPeriodBlocks);
        ...
        s_coordinatorConfig.reentrancyLock = true;
        bool success;
        ...
        s_coordinatorConfig.reentrancyLock = false;
    ...
    error RandomnessNotAvailable(uint256 requestID, uint256 requestHeight);
    ...
    /* XXX: Check that this really fits into a word. Does the compiler do the
     right thing with a custom type like uint24? */
    struct RequestParams {
        ...
        uint48 nonce;
        uint24[NUM_CONF_DELAYS] confirmationDelays;
    ...
    mapping(uint256 => BeaconRequest) public s_pendingRequests;
        ...
        returns (uint256, uint32)
    {
        ...
        if (slotNumberBig >= MAX_SLOT_NUMBER) {
    ...
    function computeRequestID(
        ...
        return
            uint256(
                keccak256(abi.encode(address(this), subID, requester, nonce))
            );
    }
        ...
        )
    {
        ...
        uint256 requestID = computeRequestID(subID, requester, nonce);
    ...
    function getConfirmationDelays()
        external
        view
        returns (uint24[NUM_CONF_DELAYS] memory)
    {
    ...
    IERC677Receiver
{
    ...
    // General configuration struct that includes billing configurations
    VRFBeaconTypes.CoordinatorConfig public s_coordinatorConfig;
    ...
    event CoordinatorConfigSet(
        VRFBeaconTypes.CoordinatorConfig coordinatorConfig
    );
    ...
    /// @dev Emitted when an invalid coordinator config update was attempted.
    error InvalidCoordinatorConfig();

    constructor(address link) OwnerIsCreator() {
        i_link = LinkTokenInterface(link);

        // Warm up s_hotBillingConfig and s_coordinatorConfig.
        // Without a manual setCoordinatorConfig, use .01 native gas token per unit link,
        ...
        s_coordinatorConfig.stalenessSeconds = type(uint32).max;
    }

    /// @notice setCoordinatorConfig updates the contract's billing config.
    function setCoordinatorConfig(
        VRFBeaconTypes.CoordinatorConfig calldata coordinatorConfig
    ) external onlyOwner {
        if (
            coordinatorConfig.unusedGasPenaltyPercent > 100 ||
            coordinatorConfig.reentrancyLock ||
            coordinatorConfig.paused
        ) {
            revert InvalidCoordinatorConfig();
        }
        s_coordinatorConfig = coordinatorConfig;
        emit CoordinatorConfigSet(coordinatorConfig);
    }
    ...
    function getWeiPerUnitLink(
        VRFBeaconTypes.CoordinatorConfig memory coordinatorConfig,
        VRFBeaconTypes.HotBillingConfig memory hotBillingConfig
    ) internal view returns (uint256) {
        uint32 stalenessSeconds = coordinatorConfig.stalenessSeconds;
        bool staleFallback = coordinatorConfig.stalenessSeconds > 1;
        ...
        if (
            ...
            weiPerUnitLink = coordinatorConfig.fallbackWeiPerUnitLink;
        }
        ...
        returns (uint256, uint96)
    {
        ...
        VRFBeaconTypes.CoordinatorConfig
            memory coordinatorConfig = s_coordinatorConfig;
    ...
    function refundCallback(
        ...
        uint256 refundWei = (((callback.gasAllowance - gasUsed) *
            callback.gasPrice) *
            (100 - s_coordinatorConfig.unusedGasPenaltyPercent)) / 100;
    ...
    function calculateRequestPriceJuelsInternal(
        ...
        uint256 baseCost = coordinatorConfig.redeemableRequestGasOverhead *
            getReasonableGasPrice(coordinatorConfig, hotBillingConfig);
        ...
        )
    {
        ...
        uint256 baseCost = (gasAllowance +
            coordinatorConfig.callbackRequestGasOverhead) *
            reasonableGasPrice +
            arguments.length *
    ...
    function getFinalCostJuels(
        ...
        uint256 premiumCost = (baseCost *
            (coordinatorConfig.premiumPercentage + 100)) / 100;
    ...
    function convertToJuels(
        ...
        uint256 weiPerUnitLink = localFeedData == 0
            ? getWeiPerUnitLink(coordinatorConfig, hotBillingConfig)
            : localFeedData;
    ...
    modifier nonReentrant() {
        if (s_coordinatorConfig.reentrancyLock) {
            revert Reentrant();
        }
        _;
    }
    ...
    function getReasonableGasPrice(
        ...
        if (
            coordinatorConfig.useReasonableGasPrice &&
            hotBillingConfig.reasonableGasPrice != 0
        ) {
            bool stalenessIsMoreThanTotalBlocks = block.number <
                coordinatorConfig.reasonableGasPriceStalenessBlocks;
            if (
    ...
    /// @dev Emitted when pause flag changes
    event PauseFlagChanged(bool paused);
    ...
    function setPauseFlag(bool pause) external onlyOwner {
        if (s_coordinatorConfig.paused == pause) {
            return;
        }
        s_coordinatorConfig.paused = pause;
        emit PauseFlagChanged(pause);
    }
}
