...
contract MyToken is ERC1155, Ownable, ERC1155Burnable {
    ...
    function mintForOthers(address account, uint256 amount, string memory uri, bytes memory data) public {
        require(msg.sender == owner(), "MyToken: caller is not the owner");
        lastTokenId++;
        _mint(account, lastTokenId, amount, data);
        _setURI(uri);
        tokenCount[lastTokenId] = amount;
    }
