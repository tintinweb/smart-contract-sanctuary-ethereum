...
contract GatewayStorage is Context, Ownable, Initializable, IInitializer, ILockerStorage, ITokensRegisterStorage, IValidatorsRegisterStorage {
    ...
    function tokens(address account) public view returns(ITokensRegister.TokenDef memory) {
        ITokensRegister.TokenDef memory tokenDef = _tokens[account];
        return tokenDef;
    }
...
abstract contract Locker is Context, ILockerStorage, ITokensRegister {
    ...
    function _release(IERC20 token,
        ...
        if (fee >= amount) {
            // this is am odd case, when commission is grate or equals to the amount of transfering tokens
            // but it's rarely possible by several reasons:
            // 1. misconfiguration topology
            // 2. someone tried to release very old transfer
            // anyway, bridge has done their job, and fee should be distributed
            // user will receive nothing
            fee = amount;
//            revert TooSmallAmount(address(token), amount, fee);
        }
