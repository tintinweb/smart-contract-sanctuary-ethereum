...
contract TokenFlexiblePool is Ownable, Pausable {
    ...
    event DepositBBC(address indexed sender, uint256 amount, uint256 shares, uint256 lastDepositedTime);
    ...
    constructor(
        ...
        // Infinite approve
        token.safeApprove(address(_bbcPool), type(uint256).max);
    }
    ...
    function deposit(uint256 _amount) public whenNotPaused {
        ...
        if (user.shares > 0) {
            if(isPureBBCPool()) {
                ...
                uint256 earnAmount = bbcPerShare * user.shares / 1 ether - userRewardDebt[msg.sender];
                if(user.shares > 0) {
    ...
    function withdraw(uint256 _shares) public {
        ...
        uint256 currentAmount = (_shares * (balanceOf() - (_isBBCPool ? currentPerformanceFee : 0))) / totalShares;
        
        if(!isPureBBCPool()) totalStakedAmount -= currentAmount;
        
        uint256 withdrawAmount = currentAmount + (_isBBCPool ? currentPerformanceFee : 0);
        if (staking) {
            // withdrawByAmount have a MIN_WITHDRAW_AMOUNT limit ,so need to withdraw more than MIN_WITHDRAW_AMOUNT.
    ...
    function claim() public {
        if(!isPureBBCPool()) {
            ...
            if(user.shares > 0) {
                ...
                payFee(currentPerformanceFee);
                emit ChargePerformanceFee(msg.sender, currentPerformanceFee, performanceFee);
                bbc.safeTransfer(msg.sender, earnAmount - currentPerformanceFee);
                userRewardPending[msg.sender] = 0;
                userRewardDebt[msg.sender] = bbcPerShare * user.shares / 1 ether;
     ...
     * @dev Only callable by the contract admin.
     */
    function setWithdrawAmountBooster(uint256 _withdrawAmountBooster) public onlyAdmin {
        require(
    ...
    function getProfit(address _user) public view returns (uint256) {
        ...
        if(user.shares==0)
            return 0;
        if(isPureBBCPool())
            return getPricePerFullShare() * user.shares / 1e18 - user.bbcAtLastUserAction;
        return bbcPool.getProfit(address(this)) * user.shares / totalShares + userRewardPending[_user] + bbcPerShare * user.shares / 1 ether - userRewardDebt[_user];
    }
