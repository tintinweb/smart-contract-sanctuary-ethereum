...
library LibTraitManager {
  ...
  function selectRandomTrait(
    ...
    assembly {
      // Start the loop
      let layerData := layerStorage.slot
      let layers := sload(layerData)
      let layerPtr := add(layerData, 0x20)
      let layerIndex := layer
      let layerLength := sload(layerPtr)
      layerPtr := add(layerPtr, 0x20)
      layerPtr := add(layerPtr, mul(layerIndex, 0x40)) // layerStorage.layers[layer]
      let trait := sload(layerPtr)
      let traitPtr := add(trait, 0x20) // layerStorage.layers[layer].traits.length
      let traitLength := mload(traitPtr)
      traitPtr := add(traitPtr, 0x20)
      let traitIndex := 0
      for {
        ...
        // Loop body
        let traitWeight := sload(add(traitPtr, mul(traitIndex, 0x60))) // layerStorage.layers[layer].traits[traitIndex].weight
