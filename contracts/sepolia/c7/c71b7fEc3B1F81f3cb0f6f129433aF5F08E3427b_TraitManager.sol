...
import { LibTraitManager, TraitManagerStorage, Trait, Layer, TokenMetadataExternal } from "../libraries/LibTraitManager.sol";

contract TraitManager is Modifiers {
  modifier layerExist(uint8 layerId) {
    require(layerId <= LibTraitManager.getStorage().layers.length, "TraitManager: Layer doesnt exist");
    _;
  }

  modifier traitExist(uint8 layerId, uint16 traitId) {
    require(traitId <= LibTraitManager.getStorage().layers[layerId].traits.length, "TraitManager: Trait doesnt exist");
    _;
  }
  ...
  //   LibTraitManager.getStorage().layers[_trait.layerId].traits[traitId] = Trait({
  //     name: _trait.name,
  ...
  function getTraitCount(uint8 layerId) public view layerExist(layerId) returns (uint256) {
    return LibTraitManager.getStorage().layers[layerId].traits.length;
  }
  ...
  function getLayerCount() public view returns (uint256) {
    return LibTraitManager.getStorage().layers.length;
  }
  ...
  function queryForDuplicateTokens() external view returns (uint16) {
    TraitManagerStorage storage tokenData = LibTraitManager.getStorage();
    uint16 tokensCreated = uint16(LibERC721.totalSupply());
    uint16 duplicateCount = 0;

    for (uint16 tokenId = 0; tokenId < tokensCreated; tokenId++) {
      if (tokenData.tokens[tokenId].attributes.length > 0) {
        TokenMetadataExternal memory currentMetadata = getTokenMetadata(tokenId);

        // Check for duplicates among previous tokens
        for (uint16 i = 0; i < tokenId; i++) {
          if (tokenData.tokens[i].attributes.length > 0) {
            TokenMetadataExternal memory previousMetadata = getTokenMetadata(i);

            // Compare the attributes of the current and previous tokens
            if (compareTokenAttributes(currentMetadata, previousMetadata)) {
              duplicateCount++;
              break;
            }
          }
  ...
  function compareTokenAttributes(TokenMetadataExternal memory metadata1, TokenMetadataExternal memory metadata2) internal pure returns (bool) {
    if (!LibUtils.compareStrings(metadata1.name, metadata2.name)) return false;

    if (metadata1.attributes.length != metadata2.attributes.length) return false;

    for (uint i = 0; i < metadata1.attributes.length; i++) {
      if (metadata1.attributes[i].traitId != metadata2.attributes[i].traitId) return false;
    }
