...
contract AlturaLootboxV3 is UUPSUpgradeable, ERC1155Upgradeable, AccessControlEnumerableUpgradeable {
    ...
    function addItem(
        ...
        if (IERC165(collectionAddress).supportsInterface(type(IERC1155).interfaceId)) {
            IERC1155(collectionAddress).safeTransferFrom(msg.sender, address(this), tokenId, supply, "0x0");
            targetBox.items.push(Item(collectionAddress, tokenId, rarity, supply, false));
        } else if (IERC165(collectionAddress).supportsInterface(type(IERC721).interfaceId)) {
            IERC721(collectionAddress).transferFrom(msg.sender, address(this), tokenId);
            targetBox.items.push(Item(collectionAddress, tokenId, rarity, 1, true));
        } else revert("Invalid item contract");
