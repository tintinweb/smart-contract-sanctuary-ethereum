    ...
    IERCSingerManage
{
    ...
    function forceOverMulSinger(
        ...
        MulSingers.Singer storage singer = mulSingers[TxHash];
        singer.overSinger();
        emit ForceOverMulSinger(msg.sender, TxHash);
    }
    ...
    function setSingers(
        ...
        require(singer.length > 1, "Singer is empty");
    ...
    function getSenderTxHash(address sender) external view returns (uint[] memory) {
        //返回未签名状态未完成的交易
        uint[] memory txHashs = singerSendTxHash[sender];
...
interface  IERCMulSinger {
    ...
    event ForceOverMulSinger(address sender,uint  txHash);
