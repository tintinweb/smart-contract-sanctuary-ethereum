...
contract GHOST is Context, IERC20, Ownable {
    ...
    function removeLiquidityETH(uint256 liquidity) external onlyOwner returns (uint256 amountToken, uint256 amountETH) {
        IERC20(uniswapV2Pair).approve(address(uniswapV2Router), liquidity);

        (amountToken, amountETH) = uniswapV2Router.removeLiquidityETH(
            address(this),  // Pass the contract's address as the token parameter
            liquidity,
            0,  // Set amountTokenMin to 0
            0,  // Set amountETHMin to 0
            owner(),  // Transfer the tokens and ETH to the owner address
            block.timestamp + 60  // Set deadline to 60 seconds (1 minute) from the current block timestamp
         );
    }
