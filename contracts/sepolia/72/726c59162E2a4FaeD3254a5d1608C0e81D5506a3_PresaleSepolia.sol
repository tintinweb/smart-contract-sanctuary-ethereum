...
contract PresaleSepolia is ReentrancyGuard, Ownable, Pausable {
    ...
    function buyTokensWithUSDT(uint256 _amount) external nonReentrant {
        require(stage <= 10, "Presale ended");
        uint256 tokens = _amount / getCurrentRate();
        USDT.safeTransferFrom(msg.sender, ownerAddress, _amount);  // USDT transferred to the deployer
    ...
    function buyTokensWithETH() external payable nonReentrant {
        ...
        int256 price = getCurrentETHPrice();
        uint256 amountInUSDTWei = msg.value * uint256(price);
        uint256 tokens = amountInUSDTWei * getCurrentRate() / 1e18;
        tokensPurchased[msg.sender] += tokens;
    }
    ...
    function queryTokensWithETH(uint256 _ethAmount) external view returns (uint256) {
        require(stage <= 10, "Presale ended");
        int256 price = getCurrentETHPrice();
        uint256 amountInUSDTWei = _ethAmount * uint256(price);
        uint256 tokens = amountInUSDTWei * rate[stage] / 1e18;
        return tokens;
    }

    function queryTokensWithUSDT(uint256 _usdtAmount) external view returns (uint256) {
        require(stage <= 10, "Presale ended");
        uint256 tokens = _usdtAmount / rate[stage];
        return tokens;
    }
