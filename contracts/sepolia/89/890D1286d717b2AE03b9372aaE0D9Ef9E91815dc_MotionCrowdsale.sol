    ...
    OwnableUpgradeable
{
    ...
    // Token Distribution
    uint256 private _tokenSalePercentage;
    uint256 private _partnersPercentage;

    // Caps
    uint256 private _investorMinCap;
    uint256 private _investorHardCap;

    // Token reserve funds
    address private _partnersFund;
    address private _partnersTimelock;
    mapping(address => address) private vestingBalances;
    address[] private _supportedStableTokens;
    mapping(address => address) private _presalesAccounts;
     ...
     * @param crowdSalerate Number of token units a buyer gets per wei
     ...
     * @param saleOpeningTime Crowdsale opening time
     * @param saleClosingTime Crowdsale closing time
     * @param saleGoal Funding goal
     */
    ...
    function initialize(
        ...
        _tokenSalePercentage = 90;
        _partnersPercentage = 10;
        _investorMinCap = 1;
        _investorHardCap = 50000;
        ...
        returns (bool)
    {
        ...
        require(
            _newContribution >= _investorMinCap &&
                _newContribution <= _investorHardCap,
            "Your Qty is less than minimum cap or contributions are above hard cap"
        );
        uint256 usdPrice = qty * _rate;
        uint256 ethAmount = (usdPrice * baseMultiplier) / getLatestPrice();
        require(msg.value >= ethAmount * baseMultiplier, "Less payment");

        uint256 excess = msg.value - (ethAmount * baseMultiplier);

        if (_presalesAccounts[_msgSender()] == address(0)) {
            _presalesAccounts[_msgSender()] = address(
                new MotionVesting(_msgSender(), _closingTime - 2 days)
            );
            _token.addToWhitelist(_presalesAccounts[_msgSender()]);
        }
        if (_nftHolders[_msgSender()] != 0) {
            qty = qty + ((qty * _nftHolders[_msgSender()]) / 100);
        }
        IERC20Upgradeable(_token).transfer(
            _presalesAccounts[_msgSender()],
            qty * baseMultiplier
        );
        _contributions[_msgSender()] = _newContribution;
        _qtySold = _qtySold + qty;
        sendValue(_wallet, ethAmount * baseMultiplier);
        if (excess > 0) sendValue(payable(_msgSender()), excess);
        emit TokensPurchased(_msgSender(), ethAmount, qty);
        return true;
    }
    ...
    function _finalization() internal onlyOwner {
        require(
            ...
            "Cannot Finalize Crowdsale before Closing Time"
        );
        TokenTimelockUpgradeable tokenPartnerTimeLock = new TokenTimelockUpgradeable();

        tokenPartnerTimeLock.initialize(_token, _partnersFund, _releaseTime);
        _partnersTimelock = address(tokenPartnerTimeLock);
        _token.addToWhitelist(_partnersTimelock);
        _token.transfer(
            address(tokenPartnerTimeLock),
            (_goal * _partnersPercentage) / 100
        );
        _token.transfer(_wallet, _token.balanceOf(address(this)));
        _token.grantRole(_token.DEFAULT_ADMIN_ROLE(), _wallet);
        _token.renounceRole(_token.DEFAULT_ADMIN_ROLE(), address(this));
        _token.renounceRole(_token.MINTER_ROLE(), address(this));
    }
    ...
    function changeTokenOwnership(
        ...
        require(
            _token.hasRole(_token.DEFAULT_ADMIN_ROLE(), address(this)),
            "Can not change owner as It is already changed"
        );
        _token.grantRole(_token.DEFAULT_ADMIN_ROLE(), ownerAddress);
        _token.renounceRole(_token.DEFAULT_ADMIN_ROLE(), address(this));
    }
    ...
    function airdropTokens(
        ...
        _token.airdropTokens(recipients, amounts);
        return true;
    }

    function getVestedAccount() public view returns (address) {
        return _presalesAccounts[msg.sender];
    }
 ...
 */
contract MotionVesting {
    address private immutable _beneficiary;
    uint256 private immutable _startDate;
    mapping(address => uint256) private _erc20Released;
    event ERC20Released(address indexed token, uint256 amount);

    constructor(address to, uint256 startDate) {
        require(to != address(0), "Address zero can't be associated");
        _beneficiary = to;
     ...
     * @dev Reverts if not in crowdsale time range.
     */
    modifier afterVestingBegin() {
        require(block.timestamp > _startDate, "Vesting not begun yet");
        _;
    }

    function vestedAmount(address token) public view returns (uint256) {
        uint256 balance = IERC20(token).balanceOf(address(this));
        if (_startDate > block.timestamp || balance < 0) {
            return 0;
        }
        uint256 diff = (block.timestamp - _startDate) / 1 days;
        return (((balance + _erc20Released[token]) / 20) *
            (diff < 20 ? diff : 20));
    }

    function withdrawnAmount(address token) public view returns (uint256) {
    ...
    ) public virtual afterVestingBegin {
        require(
            _beneficiary == msg.sender,
            "Amount can only be withdrawn by the benificiary"
        );
        require(
            IERC20(token).balanceOf(address(this)) > 0,
            "Amount already withdrawn"
        );
        uint256 amount = vestedAmount(token);

        require(
            amountToWitdraw <= amount - _erc20Released[token],
            "Can't withdraw more than vested amount check vested amount"
        );

        IERC20(token).transfer(_beneficiary, amount);

        _erc20Released[token] = _erc20Released[token] + amountToWitdraw;
...
contract TokenTimelockUpgradeable is Initializable {
    ...
    function initialize(
        ...
        uint256 releaseTimeSec
    ) public virtual initializer {
        __TokenTimelock_init(tokenContract, beneficiaryAddress, releaseTimeSec);
    }
