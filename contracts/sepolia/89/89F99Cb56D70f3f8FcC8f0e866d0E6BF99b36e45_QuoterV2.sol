...
import '@zdx-finance/dex-periphery/contracts/base/PeripheryImmutableState.sol';
import '@zdx-finance/dex-core/contracts/libraries/SafeCast.sol';
import '@zdx-finance/dex-core/contracts/libraries/TickMath.sol';
import '@zdx-finance/dex-core/contracts/libraries/TickBitmap.sol';
import '@zdx-finance/dex-core/contracts/interfaces/IZdxPool.sol';
import '@zdx-finance/dex-core/contracts/interfaces/callback/IZdxSwapCallback.sol';
import '@zdx-finance/dex-periphery/contracts/libraries/Path.sol';
...
contract QuoterV2 is IQuoterV2, IZdxSwapCallback, PeripheryImmutableState {
    using Path for bytes;
    using SafeCast for uint256;
    using PoolTicksCounter for IZdxPool;
    ...
    /// @inheritdoc IZdxSwapCallback
    function zdxSwapCallback(int256 amount0Delta, int256 amount1Delta, bytes memory path) external view override {
        ...
        IZdxPool pool = SmartRouterHelper.getPool(deployer, tokenIn, tokenOut, fee);
        ...
        returns (uint256 amountOut, uint160 sqrtPriceX96After, uint32 initializedTicksCrossed, uint256 gasEstimate)
    {
        ...
        IZdxPool pool = SmartRouterHelper.getPool(deployer, params.tokenIn, params.tokenOut, params.fee);
...
/// @title Callback for IZdxPoolActions#swap
/// @notice Any contract that calls IZdxPoolActions#swap must implement this interface
interface IZdxSwapCallback {
    /// @notice Called to `msg.sender` after executing a swap via IZdxPool#swap.
    /// @dev In the implementation you must pay the pool tokens owed for the swap.
    /// The caller of this method must be checked to be a ZdxPool deployed by the canonical ZdxFactory.
    /// amount0Delta and amount1Delta can both be 0 if no tokens were swapped.
    /// @param amount0Delta The amount of token0 that was sent (negative) or must be received (positive) by the pool by
    /// the end of the swap. If positive, the callback must send that amount of token0 to the pool.
    /// @param amount1Delta The amount of token1 that was sent (negative) or must be received (positive) by the pool by
    /// the end of the swap. If positive, the callback must send that amount of token1 to the pool.
    /// @param data Any data passed through by the caller via the IZdxPoolActions#swap call
    function zdxSwapCallback(int256 amount0Delta, int256 amount1Delta, bytes calldata data) external;
}

// SPDX-License-Identifier: GPL-2.0-or-later
pragma solidity >=0.5.0;

import './pool/IZdxPoolImmutables.sol';
import './pool/IZdxPoolState.sol';
import './pool/IZdxPoolDerivedState.sol';
import './pool/IZdxPoolActions.sol';
import './pool/IZdxPoolOwnerActions.sol';
import './pool/IZdxPoolEvents.sol';

interface IZdxPool is
    IZdxPoolImmutables,
    IZdxPoolState,
    IZdxPoolDerivedState,
    IZdxPoolActions,
    IZdxPoolOwnerActions,
    IZdxPoolEvents
{}
...
interface IZdxPoolActions {
    ...
    /// @dev The caller of this method receives a callback in the form of IZdxMintCallback#zdxMintCallback
    ...
    /// @dev The caller of this method receives a callback in the form of IZdxSwapCallback#zdxSwapCallback
    ...
    /// @dev The caller of this method receives a callback in the form of IZdxFlashCallback#zdxFlashCallback
...
interface IZdxPoolDerivedState {
...
interface IZdxPoolImmutables {
    /// @notice The contract that deployed the pool, which must adhere to the IZdxFactory interface
...
interface IZdxPoolOwnerActions {
...
import '@zdx-finance/dex-core/contracts/libraries/LowGasSafeMath.sol';
import '@zdx-finance/dex-core/contracts/interfaces/IZdxPool.sol';
...
library SmartRouterHelper {
    ...
    function getPool(address deployer, address tokenA, address tokenB, uint24 fee) public pure returns (IZdxPool) {
        return IZdxPool(computeAddress(deployer, getPoolKey(tokenA, tokenB, fee)));
    }

    /// @notice Returns the address of a valid Pool
    /// @param deployer The contract address of the deployer
    ...
    function verifyCallback(
        ...
        address tokenB,
        uint24 fee
    ) public view returns (IZdxPool pool) {
    ...
    /// @return pool The V3 pool contract address
    function verifyCallback(address deployer, PoolKey memory poolKey) public view returns (IZdxPool pool) {
        pool = IZdxPool(computeAddress(deployer, poolKey));
