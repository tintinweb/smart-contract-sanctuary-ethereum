...
contract ERC721Facet is IERC721, DefaultOperatorFiltererUpgradeable {
  ...
  function ownerOf(uint256 tokenId) public view returns (address) {
    return address(uint160(LibERC721._packedOwnershipOf(tokenId)));
  }
  ...
  function transferFrom(address from, address to, uint256 tokenId) public payable onlyAllowedOperatorApproval(from) {
    StorageLayout storage ds = LibERC721.getStorage();
    uint256 prevOwnershipPacked = LibERC721._packedOwnershipOf(tokenId);

    if (address(uint160(prevOwnershipPacked)) != from) revert TransferFromIncorrectOwner();

    (uint256 approvedAddressSlot, address approvedAddress) = LibERC721._getApprovedSlotAndAddress(tokenId);

    // The nested ifs save around 20+ gas over a compound boolean condition.
    if (!LibERC721._isSenderApprovedOrOwner(approvedAddress, from, LibUtils.msgSender()))
      if (!isApprovedForAll(from, LibUtils.msgSender())) revert TransferCallerNotOwnerNorApproved();

    if (to == address(0)) revert TransferToZeroAddress();

    LibERC721._beforeTokenTransfers(from, to, tokenId, 1);
    ...
    unchecked {
      ...
      ds._packedOwnerships[tokenId] = LibERC721._packOwnershipData(
        to,
        LibERC721._BITMASK_NEXT_INITIALIZED | LibERC721._nextExtraData(from, to, prevOwnershipPacked)
      );

      if (prevOwnershipPacked & LibERC721._BITMASK_NEXT_INITIALIZED == 0) {
    ...
    LibERC721._afterTokenTransfers(from, to, tokenId, 1);
  }
  ...
  function safeTransferFrom(address from, address to, uint256 tokenId, bytes memory _data) public payable onlyAllowedOperatorApproval(from) {
    ...
    if (to.code.length != 0)
      if (!LibERC721._checkContractOnERC721Received(from, to, tokenId, _data)) {
