...
contract FurionLeverage is OwnableUpgradeable {
    ...
    function _long(
        ...
        if (_cToken == WETH) {
            ...
            console.log("WETH borrowed: ", wethAmount);
    ...
    function _delegateSwapWhenShortWithWETH(
        ...
        uint256 price = _getPrice(IFErc20(_moneyMarket).getUnderlying());
    ...
    function _calculateAmountsWhenLongWithWETH(
        ...
        console.log("WETH needed ", wethAmountToSwap);
        ...
        // +1 to match with +1 in getAmountIn in swap router
        IERC20(WETH).transferFrom(
            msg.sender,
            address(this),
            _toWETHAmount(_collateral) + fee + 1
        );
        console.log("Transferred WETH ", _toWETHAmount(_collateral) + fee);
    ...
    function _closeLongPosition(
        ...
        if (position.collateralToken == WETH) {
            ...
            if (amounts[2] > borrowDebtToRepay)
                IERC20(WETH).transfer(msg.sender, amounts[2] - borrowDebtToRepay);
        } else {
            address[] memory path = new address[](2);
            path[0] = position.targetToken;
            path[1] = FUR;
