...
/// @notice Add Payment Splitter pattern
    ...
    PaymentSplitterUpgradeable
{
    ...
    /// @notice Array of ticket types not logical
    error TicketCountInvalid();
    /// @notice Array of ticket types not logical
    error TicketTypesInvalid();
    /// @notice Array of ticket prices not logical
    error TicketPricesInvalid();
    /// @notice Token amount to mint in the contract has exceeded limit
    error MaxTokenPerUserExceeded();
    /// @notice User mint limit is invalid 
    error MintLimitInvalid();
    ...
    /// @notice If the ETH amount sent is wrong
    error WrongEthAmount();
    /// @notice Token amount to mint in the contract has exceeded limit
    error MaxTokenExceeded();
    /// @notice User that is not whitelisted for mint 
    error NotWhitelistedUser();
    /// @notice Invalid nft price
    error InvalidNFTPrice();
    /// @notice if the phase is invalid
    error InvalidPhase(); 
    /// @notice if the base uri is invalid
    error InvalidBaseURI();
    /// @notice If the merkle root is invalid
    error InvalidMerkleRoot();
    /// @notice If the token does not exist
    error TokenDoesNotExist();
    /// @notice If the token type does not exist
    error TokenTypeDoesNotExist();
    /// @notice If the contract is the base contract then it should fail
    error BaseContract();
    /// @notice If the owner sent too many prices with respect to the number of tokenIds
    error TooManyPrices();
    /// @notice If the owner sent too many types with respect to the number of tokenIds
    error TooManyTypes();
    /// @notice If the address beign chaged is the same as before
    error SameAddressError();
    /// @notice If the admin has not set the approval for releasing the funds
    error ReleaseFundsError();
    /// @notice If the user is fetching a URI for a nonexistent token
    ...
    function initialize( 
        ...
        if (isBase == true) revert BaseContract();

        if(defaultPlatformAdmin == address(0)) revert ZeroAddress();

        // Checks if the minting limit is negative or greater than the max supply
        if(mintingLimit_ >= ticketRanges_[ticketRanges_.length - 1] || mintingLimit_ < 0) revert MintLimitInvalid();
        ...
        if(ticketPrices_.length != ticketRanges_.length) revert TooManyPrices();

        if(ticketRanges_.length < 0 || ticketRanges_.length >= 256) revert TicketTypesInvalid();
        if(ticketPrices_.length < 0 || ticketPrices_.length >= 256) revert TicketPricesInvalid();
        ...
        for(uint256 i = 0; i<ticketPrices_.length; i++) {
            ...
            if(ticketRange <= 0) revert TicketCountInvalid();

            startTokenId = endTokenId + 1;
            endTokenId = ticketRange;

            uint256 price = ticketPrices_[i];
            if(price < 0) revert TicketPricesInvalid();
        ...
        whenNotPaused
    {
        ...
        if(ticketTypeCounts.length != ticketTypes.length) revert TooManyTypes();
        ...
        for(uint256 i = 0; i < ticketTypeCounts.length; i++) {
            ...
            if(ticketTypeCount > ticketTypes[i].endTokenId - ticketTypes[i].startTokenId) revert MaxTokenExceeded();
        ...
        if(received != totalPrice) revert WrongEthAmount();
    ...
    {
        if(to == address(0)) revert InvalidAddress();
        if(amount.length != ticketTypes.length) revert TooManyTypes();
    ...
    function setPhase(
        ...
        for(uint256 i = 0; i < mintCost.length; i++){
            if(mintCost[i]<0) revert InvalidNFTPrice();
        ...
        // If the minting limit is equal to zero set the userMintingLimit state becomes false where the user can mint as many as they would like 
        if(mintingLimit_ == 0) {useMintingLimit = false;}
        mintingLimit = mintingLimit_;
    ...
    function changeBaseURI(string memory newBaseURI)
        ...
        onlyOwner
    {
        if(keccak256(abi.encodePacked((_baseTokenURI))) == keccak256(abi.encodePacked((newBaseURI)))) revert InvalidBaseURI();
    ...
    **/
    function changeMerkleRoot(bytes32 merkleRoot)
        ...
        onlyOwner
    {
        if(merkleRoot == _merkleRoot) revert InvalidMerkleRoot();
    ...
    function checkUserLimit(address to, uint256 amount) internal view returns (bool) {
        if(mintsPerUser[to]+amount>mintingLimit) revert MaxTokenPerUserExceeded();
        return true;
    }
    ...
    function checkMaxSupplyPerTicketType(uint256[] memory amount) public view returns (bool) {
        for(uint256 i = 0; i<amount.length; i++){
            if(amount[i]>ticketTypes[i].endTokenId - ticketTypes[i].currentTokenId) revert MaxTokenExceeded();
        }
        return true;
    }
    ...
    function isAllowedToMint(bytes32[] memory proof_)
        ...
        returns (bool)
    {
        if(!(MerkleProofUpgradeable.verify(proof_, _merkleRoot, keccak256(abi.encodePacked((msg.sender)))))) revert NotWhitelistedUser();
        return true;
    }
    ...
    function addTicketTypes(uint256[] memory ticketRanges_, uint256[] memory ticketPrices_) public onlyOwner {
        // Put the intial checks that check for array size and equivilance
        if(ticketPrices_.length != ticketRanges_.length) revert TooManyPrices();

        if(ticketRanges_.length < 0 || ticketRanges_.length >= 256) revert TicketTypesInvalid();
        if(ticketPrices_.length < 0 || ticketPrices_.length >= 256) revert TicketPricesInvalid();

        // Make sure that the ranges array is bigger in each index and that the first index has an index bigger than the last index in the ticketType array
        if(ticketRanges_[0]<ticketTypes[ticketTypes.length-1].endTokenId) revert TicketTypesInvalid();
    ...
    function getNextTicketId(uint256 ticketTypeId) public view returns (uint256) {
        if(ticketTypeId >= ticketTypes.length) revert TicketTypesInvalid();
    ...
    function changeTicketPrice(uint256 tokenTypeId, uint256 price) public onlyOwner {
        if(tokenTypeId < 0 || tokenTypeId > ticketTypes.length) revert TokenTypeDoesNotExist();
        ticketTypes[tokenTypeId].price = price;
        tokenPrices[tokenTypeId] = price;
    ...
    function changeTicketPlatformAdmin(address newAdmin) public onlyTicketPlatformAdmin {
        if(newAdmin == ticketPlatformAdmin) revert SameAddressError();
    ...
    function changeMintingLimit(uint32 mintingLimit_) public onlyOwner {
        if(mintingLimit_ >= ticketTypes[ticketTypes.length - 1].endTokenId || mintingLimit_ < 0) revert MintLimitInvalid();
        if(mintingLimit_ == 0) {useMintingLimit = false;}
        mintingLimit = mintingLimit_;
    }
    ...
    function release(address payable account) public override {
        if(!isReleased) revert ReleaseFundsError();
