...
contract CrowdSale is Crowdsale, FinalizableCrowdsale, UUPSUpgradeable {
    ...
    bool public referalStatus;

    mapping(address => uint256) public totalReferralAmount;

    mapping(address => bool) public isReferredBy;

    uint256 public tire1MaxPurchaseLimit;
    uint256 public tire2MaxPurchaseLimit;
    ...
    event TierUpdates(uint256 tier1, uint256 tier2, uint256 tier3);
    event AddReferral(address indexed _beneficiary, address indexed referredTo);
    event UpdateReferralStatus(bool isReferalStatusOn);
    ...
    event TirePercentageForReferral(
        uint256 _tire1MaxPurchaseLimit,
        uint256 _tire2MaxPurchaseLimit
    );
    ...
    function initialize(
        ...
        minPurchaseAmount = 20000000000000000000;
        ...
        bonusPercentage = 1;
        referalStatus = true;
        tire1MaxPurchaseLimit = 10000;
        tire2MaxPurchaseLimit = 20000;
    ...
    function buyToken() external payable onlyWhileOpen whenNotPaused {
        ...
        require(
            ...
            "Maximum purchase Limit existed"
        );
        ...
        if (
            ...
            referralAmount = calculateReferral(token_amount);
            totalReferralAmount[referredBy] += referralAmount;
            token.transfer(referredBy, referralAmount);
        }
    ...
    function buyToken(
        ...
        if (isReferred[_beneficiary] == true && referalStatus) {
            bonus = calculateBonus(token_amount);
            isReferred[_beneficiary] = false;
            emit Bonus(bonus);
        }
    ...
    function updateTiers(
        ...
        _tier2 = tier2;
        _tier3 = tier3;
        emit TierUpdates(tier1, tier2, tier3);
    }
    ...
    function calculateReferral(uint256 tokens) internal view returns (uint256) {
        uint256 tire2MinPurchaseLimit = tire1MaxPurchaseLimit + 1;
        if (tokens <= (tire1MaxPurchaseLimit * 10 ** 18)) {
            uint256 referralAmount = (tokens * _tier1) / 100;
            return referralAmount;
        } else if (
            tokens >= (tire2MinPurchaseLimit * 10 ** 18) &&
            tokens <= (tire2MaxPurchaseLimit * 10 ** 18)
        ) {
            uint256 referralAmount = (tokens * _tier2) / 100;
            return referralAmount;
        } else {
            uint256 referralAmount = (tokens * _tier3) / 100;
            return referralAmount;
        }
    }
    ...
    function addReferral(
        ...
        require(totalPurchaseAmount[msg.sender] == 0,
     ...
     * @param isReferalStatusOn - get the boolean value
     */

    function updateReferralStatus(bool isReferalStatusOn) external onlyOwner {
        referalStatus = isReferalStatusOn;
        emit UpdateReferralStatus(isReferalStatusOn);
    }
    ...
    * @param _tire1MaxPurchaseLimit and _tire2MaxPurchaseLimit
     */

    function updateTirePercentage(
        uint256 _tire1MaxPurchaseLimit,
        uint256 _tire2MaxPurchaseLimit
    ) external onlyOwner {
        tire1MaxPurchaseLimit = _tire1MaxPurchaseLimit;
        tire2MaxPurchaseLimit = _tire2MaxPurchaseLimit;
        emit TirePercentageForReferral(
            _tire1MaxPurchaseLimit,
            _tire2MaxPurchaseLimit
        );
    }
}
