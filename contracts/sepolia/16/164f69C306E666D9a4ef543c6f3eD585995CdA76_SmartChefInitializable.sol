...
contract SmartChefInitializable is Ownable, ReentrancyGuard {
    ...
    function deposit(uint256 _amount) external nonReentrant {
            ...
            if (_amount > 0) {
                ...
                user.amount = user.amount.add(balanceDiff);
            }

            user.rewardDebt = user.rewardDebt.add(balanceDiff.mul(accTokenPerShare).div(PRECISION_FACTOR));


            emit Deposit(msg.sender, balanceDiff);
