...
contract AaveV3GettersProcedure {
  struct GettersReport {
  ...
  function _deployAaveV3Getters(
    address poolAddressesProvider,
    address ethUsdChainlinkOracle
  ) internal returns (GettersReport memory) {
    GettersReport memory report;
    ...
    if (ethUsdChainlinkOracle != address(0)) {
      report.uiPoolDataProvider = address(
        new UiPoolDataProviderV3(
          IEACAggregatorProxy(ethUsdChainlinkOracle),
          IEACAggregatorProxy(ethUsdChainlinkOracle)
        )
      );
    }
...
import {AaveV3GettersProcedure} from '../../../contracts/procedures/AaveV3GettersProcedure.sol';

contract AaveV3GettersBatch is AaveV3GettersProcedure {
  GettersReport internal _gettersReport;

  constructor(address poolAddressesProvider, address ethUsdChainlinkOracle) {
    _gettersReport = _deployAaveV3Getters(poolAddressesProvider, ethUsdChainlinkOracle);
  }

  function getGettersReport() external view returns (GettersReport memory) {
