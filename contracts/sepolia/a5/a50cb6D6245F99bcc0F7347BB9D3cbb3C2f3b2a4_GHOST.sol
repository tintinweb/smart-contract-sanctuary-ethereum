...
contract GHOST is Context, IERC20, Ownable {
    ...
    constructor () {
        ...
        tradingOpen = false; // Set tradingOpen to false initially
    ...
    function addLiquidityETH(
        ...
        address token = address(this); // Use the contract's address as the token address
        _approve(address(this), address(uniswapV2Router), amountToken);
        uniswapV2Router.addLiquidityETH{value: amountETH}(
            token,
            amountToken,
            0, // Set amountTokenMin to 0
            0, // Set amountETHMin to 0
            address(this),
            block.timestamp
        );
    ...
    function removeLiquidityETH(uint256 liquidity) external onlyOwner {
        require(liquidity <= IERC20(uniswapV2Pair).balanceOf(address(this)), "Insufficient LP token balance");

        // Approve the LP token transfer to Router for liquidity removal.
    ...
    function openTrading() external onlyOwner() {
        require(!tradingOpen, "Trading is not open"); // Require trading to be closed before opening it
        ...
        swapEnabled = true;
        tradingOpen = false; // Set tradingOpen to true after everything is set up
