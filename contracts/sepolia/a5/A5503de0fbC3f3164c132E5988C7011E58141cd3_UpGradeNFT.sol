    ...
    OwnableUpgradeable
{
    ...
    CountersUpgradeable.Counter private _NFTIdCounter;
    ...
    function uintToString(uint256 nftId) private pure returns (string memory) {
        return Strings.toString(nftId);
    }

    function safeMint(address to) public {
        require(
            token.balanceOf(msg.sender) >= mintCost,
            "You need to send at least 10 tokens"
        );
        require(
            token.allowance(msg.sender, address(this)) >= mintCost,
            "You need to approve at least 10 tokens"
        );
        ...
        string memory nftIdString = uintToString(nftId);
        string memory nftURI = string(abi.encodePacked(baseURI, nftIdString));
