...
contract SKY is ERC20Permit, ERC165, Ownable {
    using SafeERC20 for IERC20;

    /// @notice The minimum amount that can be purchased is set at 1 SKY
    uint256 public constant MIN_PURCHASE_AMOUNT = 1e18;

    /// @notice The rate at which tokens are sold is set at 0.1 USDT
    uint256 public constant SALE_RATE = 0.1e18;

    /// @notice Total supply of tokens is set at 100 million SKY
    ...
    error InsufficientTokenToBuy();

    /// @notice When there are not enough tokens available to fulfill a sell
    error InsufficientTokenToSell();

    /// @notice Emitted when tokens are successfully sold tokens
    event Sold(address indexed selller, uint256 amount);

    /// @notice Emitted when tokens are successfully purchased tokens
     ...
     * @dev Mints the total supply of SKY tokens to the contract's address
     */
    constructor(IERC20 paymentToken, address sellerWallet) ERC20("SKY", "SKY") ERC20Permit("SKY") {
    ...
    /**
     * @notice Allows a user to sell tokens by specifying the selling amount
    ...
    function sell(uint256 amount) external {
        ...
        // Checks if the user has enough tokens to sell
        if (balanceOf(address(msg.sender)) < amount) {
            revert InsufficientTokenToSell();
        }

        // Calculates the sell price based on the selling amount and sale rate
        uint256 price = amount * SALE_RATE / _MULTIPLIER;

        // Transfers the payment from the buyer to the seller's wallet
        _paymentToken.safeTransferFrom(address(this), msg.sender, price);
     ...
     * @param amount The amount of tokens to be purchased
     * @param recipient The address where the purchased tokens will be transferred
     * @dev Emits a Purchased event to indicate the successful token purchase
     */
    function _buy(uint256 amount, address recipient) private {
        ...
        // Checks if the contract has enough tokens to purchase
        if (balanceOf(address(this)) < amount) {
            revert InsufficientTokenToBuy();
        }
        ...
        emit Purchased(msg.sender, recipient, amount);
    }
}