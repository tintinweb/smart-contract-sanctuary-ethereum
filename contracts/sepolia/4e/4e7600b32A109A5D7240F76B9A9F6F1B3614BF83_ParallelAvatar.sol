    ...
    ParallelOperatorFilterer
{
    ...
    ) ParallelAvatarInvoke(name, symbol) {
    ...
    function adminMintSeaDrop(
        ...
        // Mint the quantity of tokens to the receiver.
    ...
    ReentrancyGuard
{
    ...
    function invoke(
        ...
        if (_primeValue != 0) {
            bool primeSent = _prime.transferFrom(
                msg.sender,
                routerEndpoint.primeReceiver,
                _primeValue
            );
            if (!primeSent) {
                revert PrimeTransferFailed();
            }
        }
...
interface IParallelAvatarInvoke is IERC2981 {
    ...
    /**
     * @dev Revert if Invoke prime receiver call did not succeed.
     */
    error PrimeTransferFailed();
