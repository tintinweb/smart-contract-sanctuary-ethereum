...
contract SaleManager is Modifiers {
  ...
  modifier seedCheck(uint256 seeds) {
    require(seeds >= LibTraitManager.getStorage().layers.length, "SaleManager: Not enough seeds");
    _;
  }
  ...
  function mint(address owner, uint256 quantity) internal {
    ...
    if (quantity > 1) {
      for (uint i = 0; i < quantity; i++) {
        ...
        console.log("scroll minted: ", ids.scrollSupply);
...
library LibTraitManager {
  ...
  function createToken(uint16 tokenId, uint16[] calldata seeds) internal {
    TraitManagerStorage storage tokenData = getStorage();
  ...
  function generateMetadata(uint tokenId, uint16[] calldata seeds) internal returns (uint16[] memory) {
    ...
    unchecked {
      ...
      uint16[] memory metadata = new uint16[](layerCount);
  ...
  function selectRandomTrait(
    TraitManagerStorage storage layerStorage,
    uint256 layer,
    uint tokenId,
    uint seed,
    uint i,
