...
contract QuadraticVotingDAO {
    ...
    function decreaseVote(uint256 _proposalId, uint256 _voteCount) external {
        require(_proposalId > 0 && _proposalId <= proposalCount, "Invalid proposal ID");

        Proposal storage proposal = proposals[_proposalId];
        require(proposal.status == Status.Active, "Proposal is not active");

        // Get the number of votes already cast by the user for this proposal
        uint256 voteCount = proposalVotes[_proposalId][msg.sender];

        // Check if the user has enough votes to decrease
        require(_voteCount <= voteCount, "Cannot decrease more votes than previously cast");

        // Subtract the previous votes from the total votes
        proposal.totalVotes -= voteCount;

        // Update the user's votes for the proposal
        proposalVotes[_proposalId][msg.sender] = voteCount - _voteCount;

        // Add the new votes to the total votes
        proposal.totalVotes += voteCount - _voteCount;

        emit Voted(_proposalId, msg.sender, voteCount - _voteCount);
}
