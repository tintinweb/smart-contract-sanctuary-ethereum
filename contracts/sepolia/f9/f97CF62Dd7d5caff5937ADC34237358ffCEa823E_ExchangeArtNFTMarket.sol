  ...
  NFTMarketFees
{
  ...
  function buy(
    ...
    _process_buy_now(nftContract, tokenId, isPrimarySale);
  }
...
import "../../IExchangeArtRoyaltiesUpgradeable.sol";
import "../../libraries/EAERC165Checker.sol";
...
abstract contract NFTMarketFees is ExchangeArtTreasuryNode,  NFTMarketSharedCore, SendValueWithFallbackWithdraw  { 
  using Address for address;
  using EAERC165Checker for address;
    ...
    )
  {
    ...
    // Pay the seller
    if (sellerRev > 0){ // Unlikely scenario in which someone does a primary sale but is not in the creators array
      _sendValueWithFallbackWithdraw(seller, sellerRev, SEND_VALUE_GAS_LIMIT_SINGLE_RECIPIENT);
    }

    // Pay the protocol fee
    _sendValueWithFallbackWithdraw(getExchangeArtTreasury(), exchangeArtFees, SEND_VALUE_GAS_LIMIT_SINGLE_RECIPIENT);
    ...
    )
  {
    ...
    if (isPrimarySale){
      unchecked {
        exchangeArtFees = price * EXCHANGE_ART_PRIMARY_FEE;
        creatorsPayout = price - exchangeArtFees;
      }
      // If there are no creators defined, all revenue goes to seller;
      if (creatorRecipients.length != 0 || totalShares > BASIS_POINTS - EXCHANGE_ART_PRIMARY_FEE)  {
      ...
      unchecked {
        exchangeArtFees = price * EXCHANGE_ART_SECONDARY_FEE;
      }
      if (totalShares > BASIS_POINTS - EXCHANGE_ART_PRIMARY_FEE) {
        creatorsPayout = 0;
      } else {
         creatorsPayout = (price - exchangeArtFees) * totalShares / BASIS_POINTS;
      }
...
interface IExchangeArtRoyaltiesUpgradeable is IERC165Upgradeable {
    /**
     * @dev Get royalites of a token. Returns list of receivers and basis points.
     */
    function getRoyalties(uint256 tokenId) external view returns (address payable[] memory, uint256[] memory);

    /**
     * @dev Support for EIP2981. Returns receiver and the amount, depending on the tokenId and the sale price.
     * This only works for NFTs with a single creator, and will error otherwise.
     */
