...
import '@zdx-finance/dex-periphery/contracts/base/SelfPermit.sol';
...
interface IPeripheryPayments {
    ...
    /// that use ether for the input amount. And in the Router, this would be called
...
import '@zdx-finance/dex-core/contracts/libraries/FullMath.sol';
...
library OracleLibrary {
    ...
    function consult(
        ...
        (int56[] memory tickCumulatives, uint160[] memory secondsPerLiquidityCumulativeX128s) = IZdxPool(pool).observe(
            secondsAgos
        );
    ...
    function getOldestObservationSecondsAgo(address pool) internal view returns (uint32 secondsAgo) {
        (, , uint16 observationIndex, uint16 observationCardinality, , , ) = IZdxPool(pool).slot0();
        require(observationCardinality > 0, 'NI');

        (uint32 observationTimestamp, , , bool initialized) = IZdxPool(pool).observations(
        ...
        if (!initialized) {
            (observationTimestamp, , , ) = IZdxPool(pool).observations(0);
        }
    ...
    function getBlockStartingTickAndLiquidity(address pool) internal view returns (int24, uint128) {
        (, int24 tick, uint16 observationIndex, uint16 observationCardinality, , , ) = IZdxPool(pool).slot0();
        ...
        (uint32 observationTimestamp, int56 tickCumulative, uint160 secondsPerLiquidityCumulativeX128, ) = IZdxPool(
            pool
        ).observations(observationIndex);
        if (observationTimestamp != uint32(block.timestamp)) {
            return (tick, IZdxPool(pool).liquidity());
        }
        ...
        ) = IZdxPool(pool).observations(prevIndex);
...
library PoolAddress {
    bytes32 internal constant POOL_INIT_CODE_HASH = 0x9ed8c786376c7026f4956539299f34b3b6fe5ac7d309929a98e4b107532a01df;
...
import '@openzeppelin/contracts/token/ERC20/IERC20.sol';
import '@zdx-finance/dex-periphery/contracts/interfaces/INonfungiblePositionManager.sol';
...
import '@zdx-finance/dex-periphery/contracts/base/Multicall.sol';
...
import '@zdx-finance/dex-periphery/contracts/base/PeripheryImmutableState.sol';
import '@zdx-finance/dex-periphery/contracts/base/BlockTimestamp.sol';
import '@zdx-finance/dex-periphery/contracts/libraries/Path.sol';
import '@zdx-finance/dex-periphery/contracts/libraries/PoolAddress.sol';
import '@zdx-finance/dex-core/contracts/interfaces/IZdxPool.sol';
import '@zdx-finance/dex-periphery/contracts/libraries/OracleLibrary.sol';
...
abstract contract OracleSlippage is IOracleSlippage, PeripheryImmutableState, BlockTimestamp {
    ...
    function getPoolAddress(address tokenA, address tokenB, uint24 fee) internal view virtual returns (IZdxPool pool) {
        pool = IZdxPool(PoolAddress.computeAddress(deployer, PoolAddress.getPoolKey(tokenA, tokenB, fee)));
    }
    ...
    function getSyntheticTicks(
        ...
        for (uint256 i = 0; i < numPools; i++) {
            ...
            IZdxPool pool = getPoolAddress(tokenIn, tokenOut, fee);
            ...
            if (secondsAgo == 0) {
                ...
                (, currentTick, , , , , ) = IZdxPool(pool).slot0();
            }
...
import '@zdx-finance/dex-periphery/contracts/base/PeripheryPayments.sol';
import '@zdx-finance/dex-periphery/contracts/libraries/TransferHelper.sol';
...
import '@zdx-finance/dex-periphery/contracts/base/PeripheryPaymentsWithFee.sol';
...
import '@zdx-finance/dex-periphery/contracts/base/PeripheryValidation.sol';
...
interface IImmutableState {
    ...
    /// @return Returns the address of NFT position manager
...
import '@zdx-finance/dex-periphery/contracts/interfaces/IMulticall.sol';
...
import '@zdx-finance/dex-periphery/contracts/interfaces/IPeripheryPayments.sol';
...
import '@zdx-finance/dex-periphery/contracts/interfaces/IPeripheryPaymentsWithFee.sol';
...
import '@zdx-finance/dex-periphery/contracts/interfaces/ISelfPermit.sol';
...
interface IV3SwapRouter is IZdxSwapCallback {
...
abstract contract V3SwapRouter is IV3SwapRouter, PeripheryPaymentsWithFeeExtended, OracleSlippage, ReentrancyGuard {
    ...
    function zdxSwapCallback(int256 amount0Delta, int256 amount1Delta, bytes calldata _data) external override {
