...
import "@openzeppelin/contracts/token/ERC721/IERC721.sol"; // This interface provides the getApproved function for NFT transaction approval from owner
import "@openzeppelin/contracts/security/ReentrancyGuard.sol"; // This guards against fallback functions from another contract continuously calling withdraw. We lock the withdraw with a mutex.
...
contract NftMarketplace is ReentrancyGuard {
    ...
    function listItem(address nftAddress, uint256 tokenId, uint256 price) external notListed(nftAddress, tokenId, msg.sender) isOwner(nftAddress, tokenId, msg.sender) {
        ...
        IERC721 nft = IERC721(nftAddress);
        // tokenId is the NFT we're trying to list -> != address(this) means we never approve txn
