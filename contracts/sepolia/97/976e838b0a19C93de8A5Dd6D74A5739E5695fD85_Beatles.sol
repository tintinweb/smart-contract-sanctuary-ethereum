...
contract SalesActivation is Ownable {
    ...
    function setPublicSalesTime(uint256 _startTime, uint256 _endTime) external onlyOwner {
        require(_endTime >= _startTime, "Public sales: End time should be later than start time");
    ...
    modifier isPublicSalesActive() {
        require(isPublicSalesActivated(), "Public sales: Sale is not activated");
        _;
    }
    ...
    function setPreSalesTime(uint256 _startTime, uint256 _endTime) external onlyOwner {
        require(_endTime >= _startTime, "Pre sales: End time should be later than start time");
    ...
    modifier isPreSalesActive() {
        require(isPreSalesActivated(), "Pre sales: Sale is not activated");
        _;
    }
...
// Beatles
contract Beatles is Ownable, ERC721Enumerable, SalesActivation, Whitelist {
    // ------------------------------------------- const
    // total sales
    ...
    // pre mint
    function preMint(uint256 _mintQty) external payable isPreSalesActive callerIsUser {
        require(isInWhitelist(msg.sender), "Not in whitelist yet!");
        require(publicSalesMintedQty + preSalesMintedQty + _mintQty <= SALES_MAX_QTY, "Exceed sales max limit!");
        require(preSalesMinterToTokenQty[msg.sender] + _mintQty <= max_qty_per_minter_in_presales, "Exceed max mint per minter!");
        require(msg.value >= _mintQty * PRE_SALES_PRICE, "Insufficient ETH!");
    ...
    // mint for public
    function mint(uint256 _mintQty) external payable isPublicSalesActive callerIsUser {
        require(publicSalesMintedQty + preSalesMintedQty + _mintQty <= SALES_MAX_QTY, "Exceed sales max limit!");
        require(publicSalesMinterToTokenQty[msg.sender] + _mintQty <= max_qty_per_minter_in_public_sales, "Exceed max mint per minter!");
        require(msg.value >= _mintQty * PUBLIC_SALES_PRICE, "Insufficient ETH");
    ...
    function gift(address[] calldata receivers) external onlyOwner {
        require(giftedQty + receivers.length <= GIFT_MAX_QTY, "Exceed gift max limit");
    ...
    function privateMint(address account, uint256 amount) external onlyOwner {
