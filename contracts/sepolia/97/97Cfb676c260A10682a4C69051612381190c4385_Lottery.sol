        ...
        for (uint i = 0; i < prizes.length; i++) {
            while (true) {
                ...
                if (winnersMap[winner]) {
                    emit Log(string.concat("SKIP DUPLICATED WINNER: ", uint2str(winnerIdx)));
                    continue;
                }
                winners[i] = winner;
                winnersMap[winner] = true;
                emit Log(string.concat("WINNER: ", uint2str(winnerIdx)));
                break;
            }
        }

        emit Log(string.concat("WINNERS COUNT: ", uint2str(winners.length)));
    }
