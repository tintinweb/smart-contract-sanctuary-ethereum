...
contract MyToken is BaseToken, Ownable {
    ...
    constructor(string memory name, string memory symbol, uint8 decimals) BaseToken(name, symbol, decimals) {}

    modifier checkContractInWhitelist(address from, address to) {
        if(address(0) != from) {
            require(isMember(to) || !isContract(to));
        }
        _;
    }
    ...
    function _beforeTokenTransfer(
        ...
        address to,
        uint256 amount
    ) internal override checkContractInWhitelist(from, to){
