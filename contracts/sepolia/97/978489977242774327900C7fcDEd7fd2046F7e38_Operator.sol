...
contract Operator is AccessControl, ReentrancyGuard {
    ...
    event ClaimDurationChanged(uint256 claimDuration); // [email protected] indexed
    event MinimumUSD(uint256 minimumUSD); // [email protected] indexed
    ...
    uint256 public minimumUSD = 300; // [email protected] decimal?
    ...
    uint256 public claimDuration = 7;
    ...
    function setClaimPeriodInDays(
        ...
        return true; //[email protected] true?
    }
    ...
    function mintNft(
        ...
        verifyOwnerSign(msg.sender, msg.value, ethPerUSDPrice, _tokenURI, sign);
        (bool success, ) = owner.call{value: msg.value}("");
        require(success, "Failed to send Ether");
        uint256 tokenID = NFT.safeMint(msg.sender, _tokenURI);
    ...
    function claimTokens(uint256 claimID) external nonReentrant {
        ...
        require(
            block.timestamp >=
                users[claimID].initialTime + (claimDuration * 1 days),
            "time not reached"
        );
    ...
    function claimTokensOwners() external onlyRole("ADMIN_ROLE") {
        ...
        for (uint8 i; i < length0; ) {
            uint256 data = ((tokenDistribution[WalletType(0)].reservedAmt *
        ...
        for (uint8 i = 0; i < length1; ) {
            uint256 data = ((tokenDistribution[WalletType(1)].reservedAmt *
    ...
    function distributeToken(uint256 tokenID) internal {
        ...
        uint256 minterRewardAmt = (rewardAmount * communityMintingPer) / 100; // [email protected] reduce the variable size
    ...
    function verifyOwnerSign(
        ...
        bytes32 hash = keccak256(
            abi.encodePacked(to, amount, perUSDPrice, tokenURI, sign.nonce)
        );
