    ...
    OwnableUpgradeable
{
    ...
    event Claim(address _address, uint256 _heedongTokenId);
    ...
    ) external isSignerValid(signature, heedongTokenIds) {
        require(heedongTokenIds.length != 0, "No tokens available");
        for (uint256 i; i < heedongTokenIds.length; i++) {
            require(
                IERC721(HEEDONG_CONTRACT_ADDRESS).ownerOf(heedongTokenIds[i]) ==
                    msg.sender,
                "Not Token Owner"
            );
            require(
                tokenBlindboxClaimStatus[heedongTokenIds[i]] == 0,
                "Token already claimed"
            );
            tokenBlindboxClaimStatus[heedongTokenIds[i]] = block.timestamp;
            emit Claim(msg.sender, heedongTokenIds[i]);
        }
        _mint(msg.sender, heedongTokenIds.length);
    }
    ...
    function setHeedongAddress(address _address) external onlyRole(DEV_ROLE) {
        HEEDONG_CONTRACT_ADDRESS = _address;
    }

    function setSigner(address _address) external onlyRole(DEV_ROLE) {
        SIGNER = _address;
    }
}
