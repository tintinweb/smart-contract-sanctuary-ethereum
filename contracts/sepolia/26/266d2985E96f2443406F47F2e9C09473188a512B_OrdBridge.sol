...
contract OrdBridge is Initializable, OwnableUpgradeable, ReentrancyGuardUpgradeable, PausableUpgradeable , UUPSUpgradeable {
    ...
    string[] public brcTickers;
    mapping(string => bool) brctokenTickerMap;
    ...
    event BurnForBRCEntryProcessed(uint256 id);
    // Pause/Unpause
    event Unpause();
    event Pause();
    ...
    function getTickersAndTokenAddresses() external view returns (string[] memory, address[] memory) {
        address[] memory brcAddress = new address[](brcTickers.length);
    
        for (uint256 i = 0; i < brcTickers.length; i++) {
            brcAddress[i] = tokenContracts[brcTickers[i]];
        }
        return (brcTickers, brcAddress);
    }

    function getBRCTickersCount() external view returns (uint256) {
        return brcTickers.length;
    }

    function getBurnForBRCEntriesCount() external view returns (uint256) {
        return burnForBRCEntries.length;
    }
    ...
    function getBurnForBRCEntriesCountToProcess() public view returns (uint256 count) {
        for (uint256 index = 0; index < burnForBRCEntries.length; index += 1) {
            if (!burnForBRCEntries[index].processed) {
                count += 1;
            }
        }
    }
//TODO
function checkPendingERCToClaimForWallet(address wallet) external view returns (string[] memory, uint256[] memory) {
    string[] memory pendingTickers = new string[](brcTickers.length);
    uint256[] memory pendingCounts = new uint256[](brcTickers.length);
    uint256 pendingIndex = 0;

    for (uint256 i = 0; i < brcTickers.length; i++) {
        if (mintableERCTokens[brcTickers[i]][wallet] > 0) {
            pendingTickers[pendingIndex] = brcTickers[i];
            pendingCounts[pendingIndex] = mintableERCTokens[brcTickers[i]][wallet];
            pendingIndex++;
        }
    }
    ...
    function getBurnForBRCEntriesToProcess(
        ...
        for (uint256 index = 0; index < burnForBRCEntries.length && entryIndex < entriesCount; index += 1) {
            if (!burnForBRCEntries[index].processed) {
                entries[entryIndex].id = burnForBRCEntries[index].id;
                entries[entryIndex].ticker = burnForBRCEntries[index].ticker;
                entries[entryIndex].user = burnForBRCEntries[index].user;
                entries[entryIndex].amount = burnForBRCEntries[index].amount;
                entries[entryIndex].btcAddress = burnForBRCEntries[index].btcAddress;
                entries[entryIndex].meta = burnForBRCEntries[index].meta;
                entries[entryIndex].processed = burnForBRCEntries[index].processed;
    ...
    function markBurnForBRCEntriesAsProcessed(uint256[] calldata ids) external onlyOwner {
        for (uint256 index = 0; index < ids.length; index++) {
            require(ids[index] < burnForBRCEntries.length, "Invalid ids");
            require(!burnForBRCEntries[ids[index]].processed, "Already processed");
            burnForBRCEntries[ids[index]].processed = true;
            emit BurnForBRCEntryProcessed(ids[index]);
        }
    }
     ...
     */
    function updateHandlingFeesInTokenPercent(uint256 _TOKEN_FEE_PERCENT) external onlyOwner {
        require(_TOKEN_FEE_PERCENT < 100, "Invalid _TOKEN_FEE_PERCENT");
