...
contract Factory {
    ...
    function create(uint256[] calldata salts) external {
        ...
        address[] memory addresses = new address[](saltsLength);
        ...
        assembly {
            ...
            for {let i := 0} lt(i, saltsLength) {i := add(i, 1)} {
                let salt := mload(add(saltLookup, mul(i, 0x20)))
                let cloneAddress := create2(0, clone, 0x37, salt)
                mstore(add(addresses, add(0x20, mul(i, 0x20))), cloneAddress)
            }
        }
        for (uint256 i = 0; i < saltsLength; i++) {
            receiversMap[saltLookup[i]] = Receiver(addresses[i]);
        }
    }
