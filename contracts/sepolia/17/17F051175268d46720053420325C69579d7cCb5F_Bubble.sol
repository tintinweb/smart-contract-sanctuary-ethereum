...
contract Bubble is WildNFT {
    ...
    uint256[200] public lastTransfer;

    // track if token is dead or alive
    // 0 = alive, 1 = dead
    bool[200] public isDead;

    uint256 public DAY_SECONDS = 86400;
    uint256 public TEN_DAYS_SECONDS = 864000;
    uint256 public SIX_HOURS_SECONDS = 21600;

    uint256 public mintDayEODUnixTime = 0;

    string public revealedImageURI = 'https://metadata2-rvt3y3bmha-ue.a.run.app/capture/1131/';
    ...
    function setRevealedImageURI(string memory _revealedImageURI) public onlyOwner {
        revealedImageURI = _revealedImageURI;
    }

    function setMintDayEndUnixTime(uint256 _mintDayEODUnixTime) public onlyOwner {
        mintDayEODUnixTime = _mintDayEODUnixTime;
    }
    ...
    function _beforeTokenTransfer(address from, address to, uint256 tokenId, uint256 batchSize) internal override(ERC721) {
        ...
        if (from == address(0)) {
            ...
            // only process if the safety net period has ended (minting EOD + 3 days)
            if (block.timestamp > (mintDayEODUnixTime + SIX_HOURS_SECONDS)) {
    ...
    function phaseStrLowercase(uint256 tokenId) public view returns (string memory) {
        if (isDead[tokenId] == false) {
            return 'alive';
        } 
        return 'dead';
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        ...
        if (bytes(baseURI).length > 0) {
            bytes32 tokenHash = tokenToHash[tokenId];
            {
                ...
                string memory tokenResilienceStr = Strings.toString(tokenToResilience[tokenId]);
                attributesStr = string(abi.encodePacked(',"attributes":[{"trait_type": "Alive", "value":"', phase, '"}, {"trait_type": "Resilience", "value":"', tokenResilienceStr, '"}, {"trait_type": "Hash", "value": "', tokenHash, '"}, {"trait_type": "Last Transfer Date Time (UTC)", "value":"', lastTransferStr, '"} ]'));
            }
            {
                string memory phaseLowercase = phaseStrLowercase(tokenId);
                imgUrl = string(abi.encodePacked(revealedImageURI, tokenHash, '?state=', phaseLowercase));
                animationUrl = string(abi.encodePacked(baseURI, '?tokenId', '=', Strings.toString(tokenId), '&hash=', tokenHash, '&state=', phaseLowercase));
            }
        }

        string memory json;
        {
            string memory name = string(abi.encodePacked('Bubble #', Strings.toString(tokenId)));
            ...
            string memory externalUrl = string.concat('https://wild.xyz/santiago-grau/bubble/', Strings.toString(tokenId));
