...
// OxCryptoPunks is 
contract Ox111t is ERC721, ERC721Enumerable, Ownable {
    using Counters for Counters.Counter;
    Counters.Counter private _tokenIdCounter;
    using Strings for uint256;
    using SafeMath for uint256;
    constructor() ERC721("OxCryptoPunks", "Ox\u03FE") {}
    ...
    function safeMint(address to, uint256 tokenId) public onlyOwner {
        require(totalSupply() < totalPunks, "All punks minted");
        _safeMint(to, tokenId);
    }

    function punkMint(uint16 numPunks) public payable {
        require(numPunks <= 50); // Limit to mint 50 punks per transaction
        require(msg.value == numPunks * 0.02 ether, "Incorrect amount of funds");
    ...
    bytes[] private punkData; //punks storage
    string[] private acessories; // all accessories storage
    // string[] private punkAttributes; //attributes list for each punk storage
    mapping(uint256 => uint8[]) private punkAttributes;
    ...
    function addPunk(uint16 _index, bytes memory _data) external onlyOwner unsealed {
        require(_index < totalPunks, "Index out of range"); // Make sure index is within bounds
        if (_index >= punkData.length) {
            // If the index is greater than or equal to the length of the array, we need to add elements to the array
            uint256 elementsToAdd = (_index - punkData.length) + 1;
            for (uint256 i = 0; i < elementsToAdd; i++) {
                punkData.push("");
            }
        }
        punkData[_index] = _data; // Set the data for the given index
    }

    function addAcessories(string[] memory input) external onlyOwner unsealed {
    for (uint256 i = 0; i < input.length; i++) {
        acessories.push(input[i]);
        }
    }
   ...
   // Helper function to split a string by a delimiter
    function split(string memory input, string memory delimiter) internal pure returns (string[] memory) {
        ...
        for (uint16 i = 0; i < inputBytes.length - delimiterBytes.length + 1; i++) {
            bytes memory currentBytes;
            for (uint16 j = 0; j < delimiterBytes.length; j++) {
                currentBytes = abi.encodePacked(currentBytes, inputBytes[i + j]);
            }
    ...
    function substring(string memory str, uint16 startIndex, uint16 endIndex) internal pure returns (string memory) {
    ...
    function populatepunkAttributes(uint16 index, uint8[] memory attributeIndices) external onlyOwner unsealed {
        punkAttributes[index] = attributeIndices;
        if (index >= punkAttributesCount) {
            punkAttributesCount = uint16(index) + 1;
        }
    }

    function getPunkAttributes(uint16 index) public view returns (string memory) {
        require(index < punkAttributesCount, "Invalid index");

        uint8[] memory attributeIndices = punkAttributes[index];
        string memory result;

        for (uint16 i = 0; i < attributeIndices.length; i++) {
            uint8 attributeIndex = attributeIndices[i];
            require(attributeIndex < acessories.length, "Invalid attribute index");
            if (i > 0) {
                result = string(abi.encodePacked(result, ", ", acessories[attributeIndex]));
            } else {
                result = acessories[attributeIndex];
            }
        }
...
//---------------------------------------------------------------------------------------
    ...
    // Returns punk image in bytes
    function punkImage(uint16 _index) public view returns (bytes memory) {
        require(_index < punkData.length, "testPunks: index out of range");
        return punkData[_index];
    }
