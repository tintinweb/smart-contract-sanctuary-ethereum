...
contract MIRLCampaignV2 is AccessControl {
    ...
    function createCampaign(
        ...
        emit CampaignCreated(campaignId, rewardForHolders, rewardForNonHolders);
    }

    /// @notice Change campaign's reward.
    /// @param campaignId campaignId.
    /// @param rewardForHolders Set to 0 if no change.
    /// @param rewardForNonHolders Set to 0 if no change.
    function updateReward(
        ...
        // Effects
        if (rewardForHolders > 0) {
            _campaigns[campaignId].rewardForHolders = rewardForHolders;
        }
        if (rewardForNonHolders > 0) {
            _campaigns[campaignId].rewardForNonHolders = rewardForNonHolders;
        }

        emit CampaignRewardModified(
            campaignId,
            rewardForHolders,
            rewardForNonHolders
        );
    ...
    function setWinner(
        ...
        require(
            ...
            "Holder winner address is required"
        );
        require(
            nonHolderAddresses.length > 0,
            "Non-holder winner address is required"
        );
        require(
        ...
        {
            _campaigns[campaignId].holderWinners = holderAddresses;
            uint256 totalHolderAllocationPoints = _reduce(
                holderRewardAllocationPoints
            );
            for (uint256 i = 0; i < holderAddresses.length; i++) {
                address currentWinner = holderAddresses[i];
                uint256 balance = currency.balanceOf(currentWinner);
                require(balance > 0, "Address is not MIRL holder");
                winnersBalances[currentWinner] +=
                    (holderRewardAllocationPoints[i] *
                        campaign.rewardForHolders *
                        MIRL_PRECISION_MULTIPLIER) /
                    totalHolderAllocationPoints;
            }
        }
        {
            _campaigns[campaignId].nonHolderWinners = nonHolderAddresses;
            uint256 totalNonHolderAllocationPoints = _reduce(
                holderRewardAllocationPoints
            );
            for (uint256 i = 0; i < nonHolderAddresses.length; i++) {
                winnersBalances[nonHolderAddresses[i]] +=
                    (nonHolderRewardAllocationPoints[i] *
                        campaign.rewardForNonHolders *
                        MIRL_PRECISION_MULTIPLIER) /
                    totalNonHolderAllocationPoints;
            }
        }
    ...
    function balanceOfWinner(address to) public view returns (uint256) {
        return winnersBalances[to] / MIRL_PRECISION_MULTIPLIER;
    }
