...
/// @title Registry contract factory for spinning up new officail registries
...
contract ValidityRegistryFactory is Initializable, OwnableUpgradeable {
    // Array to keep track of deployed contracts
    ValidityRegistry[] public registries;

    // Event to be emitted when a new ValidityRegistry is deployed
    event ValidityRegistryCreated(
        address owner,
        address registryAddress,
        string registryName
    );

    // Initialize the contract and make the msg.sender as the owner
    function initialize() public initializer {
        __Ownable_init();
    }

    // Function to create a new ValidityRegistry
    function createValidityRegistry(
        string memory name_,
        address owner_
    ) public onlyOwner {
        ValidityRegistry newRegistry = new ValidityRegistry(name_, owner_);
        registries.push(newRegistry);

        emit ValidityRegistryCreated(msg.sender, address(newRegistry), name_);
    }

    // Function to get the deployed official registries
    function getDeployedRegistries()
        public
        view
        returns (ValidityRegistry[] memory)
    {
        return registries;
    }
}
...
contract ValidityRegistry is IRegistry {
    ...
    constructor(string memory name_, address owner_) {
        _name = name_;
        _owner = owner_;
        _salt = keccak256(abi.encodePacked(msg.sender));
    }
    ...
    function _revoke(uint256 studentId) internal virtual {
        _records[studentId].status = Status.Pending;
    }
