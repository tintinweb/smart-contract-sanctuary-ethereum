...
/// @title ERC4907 Standard
/// @dev Refers to : https://github.com/ethereum/EIPs/blob/master/assets/eip-4907/contracts/ERC4907.sol
...
interface IMarketplace {
        ...
        function traitContract() external view returns(address);
        function mainContract() external view returns(address);
        function validateMinting(address,uint256,string memory,bytes memory,uint256[] memory,uint256[] memory) external view;
...
/// @title MainNFT
/// @notice A smart contract that mints and manages NFTs with customizable traits.
...
import {Trait,RentAvatar,AvatarTrait,OrderParams,AvatarUri} from "./Structs.sol";
import {Helpers} from "./Helpers.sol";

// import "hardhat/console.sol";

contract MainNft is ERC4907, Ownable {
    
    using Strings for uint256;
 
    /// @notice The mint price for a single NFT
    uint256 public MINT_PRICE = 5000;//wei
	
    /// @notice The price per trait
    uint256 private PRICE_PER_TRAIT = 100;//wei
    ...
    mapping(uint256 => AvatarUri) private _tokenURIs;

    /// @notice Mapping to check if an NFT is a temporal NFT
    mapping(uint256 => bool) public temporalAvatars;

    /// @notice Mapping of NFT traits
    mapping(uint256 => uint256[]) public avatarTraits; //[nftId => traitsIds]

    /// @notice Modifier to allow only the marketplace contract to execute certain functions
    modifier onlyMarketplace(){
         require(msg.sender == marketplaceAddress, "Only the Marketplace contract can execute this action");
         _;
    }

    /// @notice Constructor for the MainNFT contract
	...
	* Requirements:
    * - `_marketplace` should not be NullAddress
	*/

    function setMarketplace(address _marketplace) external onlyOwner {
        require(_marketplace != address(0), "ADDRESS(0)");
        marketplaceAddress = _marketplace;
    }

	 /// @notice Updates the MINT_PRICE variable
    /// @param _newPrice The new mint price
	/// @dev only the contract owner can call this function
    function updateMintPrice(uint256 _newPrice) external onlyOwner {
        require(_newPrice > 0, "ZERO VAL");
        MINT_PRICE = _newPrice;
    }

    /// @notice Updates the PRICE_PER_TRAIT variable
    /// @param _newPrice The new price per trait
	/// @dev only the contract owner can call this function
    function updatePricePerTrait(uint256 _newPrice) external onlyOwner {
        require(_newPrice > 0, "ZERO VAL");
        PRICE_PER_TRAIT = _newPrice;
    }
    ...
    function _setTokenURI(uint256 tokenId, string memory _tokenURI) internal {
        _requireMinted(tokenId);
        _tokenURIs[tokenId].avatarUri = _tokenURI;
    }
    ...
    /**
    * @notice Mints an NFT with traits
    * @param avatarId The ID of the avatar to be minted
    * @param avatarUri The URI of the avatar
    * @param signature The server signature for the minting process
    * @param traits The array of traits to be added to the NFT
	* Requirements:
    * - `signature` should not be valid
	*/
    function mintNFT(uint256 avatarId,string memory avatarUri,bytes calldata signature,Trait[] calldata traits) external payable {
		uint256 totalPrice = calculateAvatarPrice(traits.length);
        require(msg.value >= totalPrice, "Insufficient funds to mint NFT");
		uint256[] memory traitIds;
		uint256[] memory layerIds;
		(traitIds,layerIds) = Helpers._extractTraitLayerIds(traits);
        IMarketplace(marketplaceAddress).validateMinting(msg.sender,avatarId,avatarUri,signature,traitIds,layerIds);
        _safeMint(msg.sender, avatarId);
        _setTokenURI(avatarId,avatarUri);
		avatarTraits[avatarId] = traitIds;
		uint256[] memory amounts = Helpers._initArray(avatarTraits[avatarId].length, 1);
        ITraitNft(IMarketplace(marketplaceAddress).traitContract()).batchTraitMint(msg.sender,traitIds,amounts);
    }

      /// @notice Withdraw all contract balance
    ...
    /// @notice set the user and expires of a NFT
    /// @dev The zero address indicates there is no user and  only the Marketplace contract can call this function
    /// Throws if `tokenId` already rented
    /// @param user  The new user of the NFT
    /// @param expires  UNIX timestamp, The new user could use the NFT before expires
    function setUser(uint256 avatarId, address user, uint64 expires) public override virtual onlyMarketplace {
        require(userOf(avatarId) == address(0), "User already assigned");
        UserInfo storage info = _users[avatarId];
        info.user = user;
        info.expires = expires;
        emit UpdateUser(avatarId, user, expires);
    }

    /// @notice Performs actions before the token transfer, including trait transfers
    ...
    function _beforeTokenTransfer(
        ...
        super._beforeTokenTransfer(from, to, avatarId, batchSize);
        if (from != address(0) && to != address(0)) {
            uint256[] memory amounts = Helpers._initArray(avatarTraits[avatarId].length, 1);
            address traitAddress = IMarketplace(marketplaceAddress).traitContract();
            ITraitNft(traitAddress).safeBatchTransferFrom(from, to, avatarTraits[avatarId], amounts, "");
        }
	}

    /// @notice Updates the avatar traits for two avatars
    /// @param item1 The first avatar's ID and URI, and the item ID
    /// @param item2 The second avatar's ID and URI, and the item ID
    function updateAvatarTraits(AvatarTrait calldata item1, AvatarTrait calldata item2) external onlyMarketplace {
        _tokenURIs[item1.avatarId].avatarUri = item1.uri;
        _tokenURIs[item2.avatarId].avatarUri = item2.uri;
        (bool status1, uint256 index1) = getAvatarTraitIndex(item1.avatarId, item1.itemId);
        (bool status2, uint256 index2) = getAvatarTraitIndex(item1.avatarId, item1.itemId);
        require(status1 && status2, "Invalid Avatar Traits");
        avatarTraits[item1.avatarId][index1] = item2.itemId;
        avatarTraits[item2.avatarId][index2] = item1.itemId;
    }

    /// @notice Updates an avatar with the specified parameters
    /// @param data The order parameters
    /// @param traitId The trait ID
    function updateAvatar(OrderParams calldata data, uint256 traitId) external onlyMarketplace {
        _setTokenURI(data.fromAvatar, data.fromUri);
        _setTokenURI(data.toAvatar, data.toUri);
        (bool status, uint256 index) = getAvatarTraitIndex(data.fromAvatar, traitId);
        require(status, "Invalid Avatar Traits");
        delete avatarTraits[data.fromAvatar][index];
        avatarTraits[data.toAvatar].push(traitId);
    }

	/// @notice Mints a temporary avatar with specified parameters
    /// @param user The user address
    /// @param data The order parameters
    /// @param traitId The trait ID
    /// @param expires The expiration timestamp
    function mintTempAvatar(address user, OrderParams calldata data, uint256 traitId, uint64 expires) external onlyMarketplace {
        _tokenURIs[data.fromAvatar] = AvatarUri(tokenURI(data.fromAvatar), data.fromUri, expires);
        _safeMint(user, data.toAvatar);
        _setTokenURI(data.toAvatar, data.toUri);
        temporalAvatars[data.toAvatar] = true;
        avatarTraits[data.toAvatar].push(traitId);
        _users[data.toAvatar] = UserInfo(user, expires);
        emit UpdateUser(data.toAvatar, user, expires);
    }

    /// @notice Removes a temporal avatar with the specified avatar ID
	/// @dev burns the temporal avatar and removes the tokenUri and his traits also
    /// @param avatarId The avatar ID
    function removeTemporalAvatar(uint256 avatarId) external onlyMarketplace {
        delete temporalAvatars[avatarId];
        delete _tokenURIs[avatarId];
		delete avatarTraits[avatarId];
        _burn(avatarId);
    }
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        _requireMinted(tokenId);
        AvatarUri memory avaUri = _tokenURIs[tokenId];
        string memory _tokenURI = (avaUri.expires < block.timestamp) ? avaUri.avatarUri : avaUri.tmpUri;
        if (bytes(_tokenURI).length == 0) {
            return bytes(baseURI).length > 0 ? string(abi.encodePacked(baseURI, tokenId.toString())) : "";
        } else {
            return bytes(baseURI).length > 0 ? string(abi.encodePacked(baseURI, _tokenURI)) : _tokenURI;
        }
    }


	/// @notice Calculates the total price for an NFT based on the traits number
    /// @param traitsCount The number of traits
    /// @return The total price for minting this avatar
    function calculateAvatarPrice(uint256 traitsCount) public view returns (uint256) {
        return MINT_PRICE + (traitsCount * PRICE_PER_TRAIT);
    }

   

	 /// @notice Retrieves the traits of a given avatar ID
    /// @param avatarId The avatar ID
    /// @return An array of trait IDs
    function getAvatarTraits(uint256 avatarId) public view returns (uint256[] memory) {
        return avatarTraits[avatarId];
    }

    /// @notice Retrieves the index of a given trait ID in the avatarTraits array of a specified avatar ID
    /// @param avatarId The avatar ID
    /// @param traitId The trait ID
    /// @return A tuple containing a boolean flag indicating if the trait exists and the index of the trait
    function getAvatarTraitIndex(uint256 avatarId, uint256 traitId) public view returns (bool, uint256) {
        for (uint256 i = 0; i < avatarTraits[avatarId].length; i++) {
            if (avatarTraits[avatarId][i] == traitId) return (true, i);
        }
        return (false, 0);
    }
