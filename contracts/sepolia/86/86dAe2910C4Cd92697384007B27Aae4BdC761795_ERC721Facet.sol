...
library LibTraitManager {
  ...
  function selectRandomTrait(
    ...
    assembly {
      ...
      traitsPtr := add(traitsPtr, mul(layer, 64)) // Multiply by 64 bytes per Layer struct
      let traitsEnd := add(traitsPtr, mul(sload(traitsPtr), 64)) // Get the end pointer of the traits array

      for {

      } lt(randomValue, mload(add(traitsPtr, 32))) {

      } {
        randomValue := sub(randomValue, mload(add(traitsPtr, 32)))
        traitsPtr := add(traitsPtr, 64) // Move to the next Trait struct

        // Check if we reached the end of the traits array
        traitsEnd := add(traitsEnd, 64)
