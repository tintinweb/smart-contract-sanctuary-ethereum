...
library LibTraitManager {
  ...
  function selectRandomTrait(
    ...
    assembly {
      // Start the loop
      let trait := sload(add(add(layerStorage.slot, 0x20), mul(layer, 0x40))) // layerStorage.layers[layer]
      let traitLength := mload(add(trait, 0x20)) // layerStorage.layers[layer].traits.length
      let traitIndex := 0
      for {

      } lt(traitIndex, traitLength) {

      } {
        // Loop body
        let traitPtr := add(add(trait, 0x40), mul(traitIndex, 0x60)) // layerStorage.layers[layer].traits[traitIndex]
        let traitWeight := sload(traitPtr) // layerStorage.layers[layer].traits[traitIndex].weight
        let randomValueCopy := randomValue
        randomValueCopy := sub(randomValueCopy, traitWeight)
        randomValue := randomValueCopy

        switch lt(randomValue, 0)
        case 1 {
          index := traitIndex
          randomValue := 0
        }
        ...
        traitIndex := add(traitIndex, 1) // Increment traitIndex
      }
    }
