...
contract SmokleySdCommerceWTF is ERC1155, ERC1155Supply, ERC1155URIStorage, Ownable {
    ...
    event shippingClaimed(address indexed to, uint256 id, uint256 amount);
    event shippingClaimedBulk(address indexed to, uint256[] ids, uint256[] amounts);

    constructor() ERC1155("") {
        name = "SmokleyS dCommerce";
        symbol = "SmokleyS.shop";
    ...
    function buyBulk(address to, uint256[] calldata ids, uint256[] calldata amounts)
        external
        payable
    {
        uint256 totalAmount;
    ...
    function claimShipping(address account, uint256 id, uint256 amount)
        external
    {
        require(balanceOf(account, id) > claimed[id][account], "no shipping claims left");
        require(amount > 0 && amount <= (balanceOf(account, id) - claimed[id][account]),"cannot claim more than your remaining items");
        claimed[id][account] += amount;
        emit shippingClaimed(account, id, amount);
    } 

    function claimShippingBulk(address account, uint256[] calldata ids, uint256[] calldata amounts)
        external
    {
        for (uint i = 0; i < ids.length; i++) {
            require(balanceOf(account, ids[i]) > claimed[ids[i]][account], "no shipping claims left for at least one product");
            require(amounts[i] > 0 && amounts[i] <= (balanceOf(account, ids[i]) - claimed[ids[i]][account]),"cannot claim more than your remaining items");
            claimed[ids[i]][account] += amounts[i];
        }
        
        emit shippingClaimedBulk(account, ids, amounts);
    } 
