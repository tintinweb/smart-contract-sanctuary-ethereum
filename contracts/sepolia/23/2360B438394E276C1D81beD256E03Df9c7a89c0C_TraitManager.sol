...
import { LibTraitManager, TraitStorage, Layer, Trait, LayerSetup, TraitSetup, TokenMetadataExternal, TraitExternal } from "../libraries/LibTraitManager.sol";

contract TraitManager is Modifiers {
  modifier layerExist(uint8 layerId) {
    require(layerId <= LibTraitManager.getStorage().layerCount, "TraitManager: Layer doesnt exist");
    _;
  }

  modifier traitExist(uint8 layerId, uint16 traitId) {
    require(traitId <= LibTraitManager.getStorage().layers[layerId].traitCount, "TraitManager: Trait doesnt exist");
    _;
  }
  ...
  function uploadTraits(LayerSetup[] memory _layers, TraitSetup[] memory _traits) public onlyOwner {
    TraitStorage storage layerStorage = LibTraitManager.getStorage();
  ...
  function getTraitCount(uint8 layerId) public view layerExist(layerId) returns (uint16) {
    return LibTraitManager.getStorage().layers[layerId].traitCount;
  }
  ...
  function getLayerCount() public view returns (uint8) {
    return LibTraitManager.getStorage().layerCount;
  }
  ...
  function queryForDuplicates() public view returns (uint16 duplicateCount) {
    uint16 totalTokens = uint16(LibERC721.totalSupply());

    for (uint16 i = 0; i < totalTokens - 1; i++) {
      TokenMetadataExternal memory token1 = LibTraitManager.getTokenMetadata(i);

      for (uint16 j = i + 1; j < totalTokens; j++) {
        TokenMetadataExternal memory token2 = LibTraitManager.getTokenMetadata(j);

        if (compareTokenMetadata(token1, token2)) {
          duplicateCount++;
        }
      }
    }
  ...
  function compareTokenMetadata(TokenMetadataExternal memory token1, TokenMetadataExternal memory token2) internal pure returns (bool) {
    if (token1.attributes.length == token2.attributes.length) {
      for (uint16 i = 0; i < token1.attributes.length; i++) {
        TraitExternal memory trait1 = token1.attributes[i];
        TraitExternal memory trait2 = token2.attributes[i];

        if (!LibUtils.compareStrings(trait1.traitName, trait2.traitName)) {
          return false;
        }
      }
      return true;
