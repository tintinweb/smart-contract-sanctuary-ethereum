    ...
    UUPSUpgradeable
{
    ...
    error WalletLimitReached(uint limit);
    error InvalidEtherAmount(uint provided, uint required);
    ...
    function reserve(uint amount) external payable whenNotPaused {
        ...
        if (totalReserved + amount > availableReserves) {
            revert ReservationClosed();
        }

        if (reserves[msg.sender] + amount > maxPerWallet) {
            revert WalletLimitReached(maxPerWallet);
        }

        if (msg.value != amount * reservePrice) {
            revert InvalidEtherAmount(msg.value, amount * reservePrice);
        }
    ...
    function stats() external view returns (uint, uint) {
        return (reserves[msg.sender], availableReserves - totalReserved);
    }
}
