...
contract IdleBurn {
    ...
    function calculateBurnAmount(address account) private view returns (uint256) {
        ...
        uint256 burnPercentage = inactiveIntervals * 1; // 1% per 3-minute interval

        // Check if the address is active (less than 3 minutes of inactivity)
        if (inactiveIntervals == 0) {
            return 0; // No burn for active addresses
        }
