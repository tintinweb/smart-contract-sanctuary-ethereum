...
contract NFTAuction is ReentrancyGuard, Ownable {
    ...
    function testPlaceBid(address[] memory users, uint256 amount) public nonReentrant {
        for (uint i = 0; i < users.length; i++) {
            userBids[users[i]] += amount + (i * 10000000000000000);
            emit BidPlaced(users[i], amount + (i * 10000000000000000));
        }
    }
