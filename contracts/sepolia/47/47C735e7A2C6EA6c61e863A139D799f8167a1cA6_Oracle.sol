...
contract Oracle is AccessControl, IOracle {
    ...
    //TODO: Should we do it like this or deploy the Loan manually and then add it?
    function requestLoan(address _pool, address _rampAddress, uint256 _amount, uint256 _interestRate, uint256 _duration) external onlyRole(ORACLE_MANAGER_ROLE) {
        //Define this based on the current cycle remaining time
        //require(_duration < pool.CYCLE_DURATION, "Loan duration cannot exceed cycle duration");

        new Loan(brzToken, _rampAddress, _amount, _interestRate, _duration, address(this));
    }

    function approveLoan(address _loan, address _pool) public onlyRole(ORACLE_MANAGER_ROLE) {
        loans[_loan] = _pool;
    }

    function commitStandByCapital(address _pool, uint256 _total) external onlyRole(ORACLE_MANAGER_ROLE) {
        uint currentCycle = IPool(_pool).getCurrentCycleNumber();
    ...
    function requestFunding(address _loan) external onlyRole(ORACLE_MANAGER_ROLE) {
        require(loans[_loan] != address(0), "Not approved");
        
        address pool = loans[_loan]; 
        IPool(pool).requestFunding(_loan);
        
        address rampAddress = ILoan(_loan).getRampAddress();
        uint256 loanAmount = ILoan(_loan).getAmount(); 

        uint currentCycle = IPool(pool).getCurrentCycleNumber();

        emit LoanWithdrawn(_loan, pool, currentCycle, rampAddress, loanAmount);
    }
    ...
    function repay(uint256 _total) external onlyLoan {
        ...
        emit LoanRepaid(msg.sender, pool, currentCycle, _total);
    }
}
...
interface ILoan {
   ...
   function getRampAddress() external view returns(address);
...
interface IOracle {
    ...
    event LoanWithdrawn(address indexed loan, address indexed pool, uint256 indexed cycle, address rampAddress, uint256 total);
    event LoanRepaid(address indexed loan, address indexed pool, uint256 indexed cycle, uint256 total);
...
interface IPool {
    ...
    event StakerVoted(address indexed staker, address indexed oracle, uint256 cycle, uint256 totalVotes);
    ...
    event LoanRepaid(address indexed loan, address indexed oracle, uint256 indexed cycle, uint256 total);
    event FundsAddedAfterDistribution(uint256 indexed cycle, address indexed oracle, uint256 total);
    event NewDepositRequest(address indexed liquidityProvider, uint256 total);
    event NewWithdrawRequest(address indexed liquidityProvider, uint256 total);
    event NewRewardDepositRequest(address indexed liquidityProvider, uint256 total);
    ...
    function processLPsRequests(address user) external;
    function requestAddOracle(address _oracle) external;
    function requestRemoveOracle(address _oracle) external;
    function requestFunding(address _loan) external;
    function processOraclesRequests() external;
    function assignCapitalAfterVoting(address _oracle) external;
    ...
    function requestRemove() external;
    function getBRZForShares(uint shares) external view returns (uint);
    function calculateCostsOfCapital(address _oracle) external;
    function acceptCapitalAssigned(address _oracle, uint256 _total) external;
    ...
    function vote(address _oracle, uint256 _percentage) external;
    ...
    function requestCreateRewardDeposit(address _beneficiary, uint256 _total) external;
}
...
contract Loan is ILoan {
    ...
    function setIsFunded() external {
        ...
        emit Withdrawn(rampAddress, amount);
    } 
}