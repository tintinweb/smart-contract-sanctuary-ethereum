...
library LibTraitManager {
  ...
  function selectRandomTrait(
    ...
    assembly {
      let layersPtr := add(layerStorage.slot, 32) // Skip the length of the dynamic array field
      let layerPtr := add(layersPtr, mul(layer, 96)) // Offset to the specified layer

      let traitsPtr := add(layerPtr, 32) // Skip the length of the traits dynamic array field
      let traitsLength := sload(traitsPtr)

      let traitsOffset := add(traitsPtr, 0x20) // Skip the length field of traits array
      let traitsEnd := add(traitsOffset, mul(traitsLength, 96)) // End pointer of traits array
      ...
      for {
        ...
        let traitWeight := shr(240, sload(add(traitPtr, 0x40))) // Load the weight of the current trait

        randomValue := sub(randomValue, traitWeight)
        index := add(index, 1)
        traitPtr := add(traitPtr, 96) // Move to the next trait (96 bytes)
      }
    }
