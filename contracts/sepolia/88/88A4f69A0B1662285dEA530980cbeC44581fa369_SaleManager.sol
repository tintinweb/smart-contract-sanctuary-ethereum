...
library LibTraitManager {
  ...
  function selectRandomTrait(
    ...
    assembly {
      ...
      } lt(traitIndex, traitsLength) {

      } {
        let trait := add(traitsOffset, mul(96, traitIndex))

        let traitWeight := and(mload(add(trait, 0x40)), 0xFFFF) // Load the weight of the current trait
