...
/* TEST1 - TEST1 - TEST1 - TEST1 - TEST1 - TEST - TEST - TEST - TEST - TEST - TEST - TEST - TEST - TEST - TEST - TEST
 */
...
contract OxCryptoPunksBlackMarket is ERC721, ERC721Enumerable, Ownable {
    ...
    string internal nftSymbol = unicode"ꙨxϾ";
    ...
    uint128 public constant punkPrice = 700000000000000; // 0.007 ether
    ...
    function devMint(address to, uint16 numPunks) public onlyOwner {
    ...
    bytes[] private punkImageBytes; // The punk image storage
    string[] private punkAttributesList; // The punk attributes storage
    mapping(uint256 => uint8[]) private punkAttributesMap; // The punk attributes base
    ...
    // Emergency function which can override punk image data at any index: dev only
    function addPunkDev(uint16 _index, bytes memory _data)
        external
        onlyOwner
        unsealed
    {
        require(_index < totalPunks, "Index out of range"); // Make sure index is within bounds
        if (_index >= punkImageBytes.length) {
            // If the index is greater than or equal to the length of the array, we need to add elements to the array
            uint256 elementsToAdd = (_index - punkImageBytes.length) + 1;
            for (uint256 i = 0; i < elementsToAdd; i++) {
                punkImageBytes.push("");
            }
        }
        punkImageBytes[_index] = _data; // Set the data for the given index
    }

    function addPunk(uint16 _index, bytes memory _data) external unsealed {
        require(_index < totalPunks, "Index out of range"); // Make sure index is within bounds
        require(punkImageBytes.length <= _index, "Index is not available"); // Check if the index is already taken

        if (punkImageBytes.length < _index) {
            // If there are skipped indices between the current length and the desired index
            uint16 firstAvailableIndex = uint16(punkImageBytes.length);
            revert(
                string(
                    abi.encodePacked(
                        "The next available index is ",
                        toString(firstAvailableIndex)
                    )
                )
            );
        }

        punkImageBytes.push(_data); // Add the data to the end of the array
    }
    ...
    function addToPunkAttributesMap(
        ...
        for (uint16 i = 0; i < indexes.length; i++) {
            ...
            require(
                punkAttributesMap[index].length == 0,
                "Index is not available"
            );

            punkAttributesMap[index] = indices;
    ...
    function populatePunkAttributesDev(
        uint16[] memory indexes,
        uint8[][] memory attributeIndices
    ...
    function punkImage(uint16 _index) public view returns (bytes memory) {
        require(
            _index < punkImageBytes.length,
            "OxCryptoPunks: index out of range"
        );
        return punkImageBytes[_index];
    }
        ...
        returns (string memory text)
    {
        ...
        uint8[] memory attributeIndices = punkAttributesMap[index];
