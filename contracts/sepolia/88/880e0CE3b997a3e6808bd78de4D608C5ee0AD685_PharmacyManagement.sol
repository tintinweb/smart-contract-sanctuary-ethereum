...
contract PharmacyManagement {
    ...
    require(
        users[msg.sender].role == Role.Pharmacist || users[msg.sender].role == Role.Warehouse,
        "Unauthorized"
    );

    orders[Role.Driver][orderCount] = Order(
        email,
        name,
        medicalId,
        productId,
        false,
        orderCount, // Assign the correct orderId value here
        0,
        block.timestamp,
        driverAddress
    );

    emit OrderPlaced(orderCount, email, Role.Driver, driverAddress);
