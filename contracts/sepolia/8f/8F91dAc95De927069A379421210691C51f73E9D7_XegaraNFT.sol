    ...
    EIP712Upgradeable
{
    ...
    function verifyNFTListing(
        ...
        bytes32 signed_data = _hashTypedDataV4(
            keccak256(
                abi.encode(
                    keccak256(
                        "NFTListing(uint256 pricePerShare,uint256 tokenId,address owner)"
                    ),
    ...
    function buyToken(
        ...
        require(_currency != address(0), "EZA"); //ERC20 Token address can't be zero address
        require(_nftListing.owner != address(0), "NOCZA"); //NFT owner can't be zero address
        require(_amountToBuy > 0, "QZ"); //Quantity to buy should not be zero
        require(verifyNFTListing(_nftListing) == _nftListing.owner, "IS"); // Invalid Signer

        uint256 finalPrice = _nftListing.pricePerShare * _amountToBuy;
    ...
    function safeMint(
        ...
        require(msg.sender == admin, "CNA"); //Caller is not authorized
        supply[id] += amount;
    ...
    function safeBurn(
        ...
        require(msg.sender == admin, "OA"); // Only Admin;
...
interface IFactory {
    function initialize(address _NFT, address _Dividend) external;
