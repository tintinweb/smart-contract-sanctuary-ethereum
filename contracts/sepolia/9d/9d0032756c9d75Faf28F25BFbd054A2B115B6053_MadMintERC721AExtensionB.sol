...
import {MadMintERC721AUpgradeable} from "./MadMintERC721AUpgradeable.sol";
import {MadMintStorage, BaseConfig, BurnToken} from "../libraries/MadMintStorage.sol";
import {ECDSAUpgradeable} from "@openzeppelin/contracts-upgradeable/utils/cryptography/ECDSAUpgradeable.sol";
import {IERC721} from "@openzeppelin/contracts/token/ERC721/IERC721.sol";
import {IERC1155} from "@openzeppelin/contracts/token/ERC1155/IERC1155.sol";

contract MadMintERC721AExtensionB is MadMintERC721AUpgradeable {
    using MadMintStorage for MadMintStorage.State;
    ...
    function presalePriceInWei() public view returns (uint256) {
        return uint256(MadMintStorage.state().cfg.presalePrice) * 10 ** 13;
    }
    ...
    function setPresaleState(bool _saleActiveState) external onlyOwner {
        MadMintStorage.state().cfg.presaleActive = _saleActiveState;
    }
    ...
    function setPresalePrice(uint32 _presalePrice) external onlyOwner {
        MadMintStorage.state().cfg.presalePrice = _presalePrice;
    }
    ...
    function reducePresaleMaxSupply(
        ...
        BaseConfig storage cfg = MadMintStorage.state().cfg;
        require(
    ...
    function setPresaleSignerAddress(
        ...
        MadMintStorage.state().cfg.presaleSignerAddress = _presaleSignerAddress;
    }
    ...
    function setPresaleStartTime(uint32 _presaleStartTime) external onlyOwner {
        MadMintStorage.state().cfg.presaleStartTime = _presaleStartTime;
    }
    ...
    function setPresaleEndTime(uint32 _presaleEndTime) external onlyOwner {
        ...
        MadMintStorage.state().cfg.presaleEndTime = _presaleEndTime;
    }
    ...
    function setUsePresaleTimes(bool _usePresaleTimes) external onlyOwner {
        MadMintStorage.state().cfg.usePresaleTimes = _usePresaleTimes;
    }
    ...
    function verifySignerAddress(
        ...
        return
        MadMintStorage.state().cfg.presaleSignerAddress ==
    ...
    function presaleMint(
        ...
        if (cfg.madMintFeeActive) {
            uint256 madmintFee = _numTokens * madmintFeePerToken();
            require(
                msg.value == presalePrice * _numTokens + madmintFee,
                "INVALID_PRICE_PAID"
            );
            (bool success, ) = madmintPayoutAddress.call{value: madmintFee}("");
        ...
        if (cfg.fundingEndsAt > 0) {
            ...
            for (uint256 i = 0; i < _numTokens; i++) {
                MadMintStorage.state().data.pricePaid[
    ...
    function burnPaymentInWei() public view returns (uint256) {
        return uint256(MadMintStorage.state().advCfg.burnPayment) * 10 ** 13;
    }
    ...
    function setBurnClaimState(bool _burnClaimActive) external onlyOwner {
        MadMintStorage.State storage state = MadMintStorage.state();
        if (_burnClaimActive) {
    ...
    function updateBurnTokens(
        ...
        BurnToken[] storage burnTokens = MadMintStorage.state().burnTokens;
    ...
    function updateMintsPerBurn(uint8 _mintsPerBurn) external onlyOwner {
        MadMintStorage.state().advCfg.mintsPerBurn = _mintsPerBurn;
    }
    ...
    function updatePaymentPerBurn(uint32 _burnPayment) external onlyOwner {
        MadMintStorage.state().advCfg.burnPayment = _burnPayment;
    }
    ...
    function burnToMint(
        ...
        if (state.cfg.madMintFeeActive) {
            uint256 madmintFee = _tokensToMint * madmintFeePerToken();
            require(
                msg.value == burnPaymentTotal + madmintFee,
                "INVALID_PRICE_PAID"
            );
            (bool success, ) = madmintPayoutAddress.call{value: madmintFee}("");
            require(success, "TRANSFER_FAILED");
...
import {Data, MadMintStorage} from "../libraries/MadMintStorage.sol";
...
RevokableOperatorFiltererUpgradeable
{
    ...
    uint256 public constant defaultMadmintFeePerToken = 0.0007 ether;
    address public constant madmintPayoutAddress =
    ...
    /**
     * @notice Returns the MadMint fee per token. If the fee is 0, the default fee is returned
     */
    function madmintFeePerToken() public view returns (uint256) {
        uint256 fee = MadMintStorage.state().data.madmintFeePerToken;
        return fee == 0 ? defaultMadmintFeePerToken : fee;
    }
    ...
    {
        require(
            !MadMintStorage.state().cfg.soulbindingActive,
            "TOKEN_IS_SOULBOUND"
        );
    ...
    function _generateRandomHash(uint256 _tokenId) internal {
        Data storage data = MadMintStorage.state().data;
...
pragma solidity 0.8.18;
    ...
    struct BaseConfig {
        ...
        // If true, MadMint fees will be charged for minting tokens
        bool madMintFeeActive;
        ...
        uint32 presalePrice;
        // Used to create a default MadMint Launchpad URI for token metadata to save gas over setting a custom URI and increase fetch reliability
        uint24 projectId;
    ...
    struct Data {
        // ============ BASE FUNCTIONALITY ============
        // MadMint fee to be paid per minted token (if not set, defaults to defaultMadmintFeePerToken)
        uint256 madmintFeePerToken;
...
library MadMintStorage {
    ...
    keccak256("madmint.launchpad.storage.erc721a");
