...
contract LibesBetting is Ownable {
    ...
    event CreateTournament(uint256 _TournamentId, string indexed _Name);
    event LibesEntry(
        uint256 _BetId,
    ...
    verifySignature(nonce, _tournamentId, signature) 
    returns (uint256 _betId) {
        ...
        better[_betId] = bet;
        addHistory(_tokenId);
        emit LibesEntry(_betId, _tournamentId, msg.sender, _tokenId);
    }
    ...
    function cancel( uint256 _betId, uint256[] memory _listToken, uint256 nonce, bytes memory signature) external 
    verifySignature(nonce, _betId, signature)
     {
        require(better[_betId].Owner == msg.sender, "YOU ARE NOT THE BETTOR");
        require(
            tournaments[better[_betId]._TournamentId].status != true,
            "THE TOURNAMENT IS FINISHED YET"
        );

        NFTBetting(NFTBettingAddress).unlockToken(_listToken);

        for(uint256 i = 0; i < _listToken.length; i++) {
               History[_listToken[i]] = false;
        }

        emit Cancel(_betId, _listToken);
    }
