...
contract TicketContract is ERC1155, Ownable {
    ...
    struct Event {
        ...
        uint256 startTime; // 活動開始時間（Unix 時間戳記）
        uint256 endTime; // 活動結束時間（Unix 時間戳記）
    }
    ...
    mapping(address => mapping(uint256 => uint256)) public addressHasPurchased; // 地址是否已購買特定活動的票
    ...
    function mintTicket(uint256 eventId, uint256 quantity)
        ...
        returns (bool)
    {
        Event memory selectedEvent = events[eventId];
        require(
            tokenIdCounter.current() + quantity <= MAX_SUPPLY,
            "Exceeds maximum supply"
        );
        require(!hasEventEnded(eventId), "Event has already ended");
        require(
            !selectedEvent.oneTicketPerPerson ||
                addressHasPurchased[msg.sender][eventId] == 0,
            "One ticket per person rule violated"
        );
        require(quantity <= selectedEvent.capacity, "Exceeds event capacity");
        require(
            msg.value >= (quantity * events[eventId].price) / 10**18,
            "not enough ETH"
        );

        uint256 tokenId = tokenIdCounter.current();

        addressHasPurchased[msg.sender][eventId] += 1;
    ...
    function hasEventEnded(uint256 eventId) internal view returns (bool) {
        return block.timestamp > events[eventId].endTime;
    }
    ...
    function refund(uint256 tokenId) external {
        ...
        require(
            !tickets[tokenId].refunded,
            "The ticket has already been refunded."
        );
        require(!tickets[tokenId].isUsed, "The ticket has already been used.");
        require(
            block.timestamp < events[tickets[tokenId].eventId].startTime,
            "Refund period has ended"
        );

        uint256 refundAmount = (tickets[tokenId].price *
            tickets[tokenId].quantity *
            REFUND_PERCENT) / 100;
        ...
        emit TicketTransferred(tokenId, msg.sender, address(0));
    }
    ...
    function getUserTickets() public view returns (Ticket[] memory) {
        Ticket[] memory allTickets = new Ticket[](tokenIdCounter.current());
        for (uint256 i = 0; i < tokenIdCounter.current(); i++) {
