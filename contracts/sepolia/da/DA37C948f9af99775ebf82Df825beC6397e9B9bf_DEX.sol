...
contract DEX {
    ...
    mapping(address => uint256) liquidity; //allows anyone to swap between the assests
    ...
    function init(uint256 tokens) public payable returns (uint256) {
        ...
        // Fund with ETH
        // (bool ethFundedOK, ) = payable(address(this)).call{value: msg.value}(
        //     ""
        // );
        // require(ethFundedOK, "Failed to fund with ETH");
        // Transfer tokens
        bool transferOK = token.transferFrom(msg.sender, address(this), tokens);
        require(transferOK, "Failed to transfer tokens");
    ...
    function tokenToEth(uint256 tokenInput) public returns (uint256 ethOutput) {
        require(tokenInput > 0, "cannot swap 0 BAL");
    ...
    function deposit() public payable returns (uint256 tokensDeposited) {
        ...
        uint256 ethReserve = address(this).balance.sub(msg.value);
        uint256 tokenReserve = token.balanceOf(address(this));
        uint256 tokenDeposit = (msg.value.mul(tokenReserve) / ethReserve).add(
            1
        );
