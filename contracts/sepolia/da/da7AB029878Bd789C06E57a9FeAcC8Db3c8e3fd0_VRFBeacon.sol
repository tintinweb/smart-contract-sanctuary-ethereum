...
contract VRFBeaconOCR is VRFBeaconReport {
    ...
    function transmit(
...

        {
            ...
            if (seenReports[reportHash]) {
                // Note that a report can only legitimately be processed once.
                revert SeenReport(reportHash); // reject already-processed reports
            }
            seenReports[reportHash] = true; // record hash to avoid later repeat
        }
        ...
        returns (uint256)
    {
        ...
        // s_hotVars.observationPayment <= 2**64
        ...
        ) * uint256(s_hotVars.observationPayment);
    ...
    function _totalLinkDue() internal view returns (uint256 linkDue) {
        ...
        linkDue *= uint256(s_hotVars.observationPayment);
        for (uint256 i = 0; i < n; i++) {
...
contract VRFBeaconReport is VRFBeaconTypes, OwnerIsCreator {
    ...
    function _report(
        ...
        i_coordinator.setReasonableGasPrice(report.reasonableGasPrice);
