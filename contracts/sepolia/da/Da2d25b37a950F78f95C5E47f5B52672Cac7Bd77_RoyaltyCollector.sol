...
interface IRoyaltyCollector {
    ...
    function collectRoyalties(NFTItem[] calldata nftItems) external;
}
...
contract RoyaltyCollector is IRoyaltyCollector, Initializable, AccessControlUpgradeable {
    ...
    function collectRoyalties(NFTItem[] calldata nftItems) external onlyRole(COLLECTOR_ROLE) {
        ...
        for (uint256 i = 0; i < nftItemCount; ) {
            ...
            IRoyaltyForwarder royaltyForwarder = IRoyaltyForwarder(
                royaltySplitter_.computeRoyaltyForwarderAddress(nftItem.collection, nftItem.tokenId)
            );
