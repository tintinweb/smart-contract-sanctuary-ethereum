...
/*
  Features:
  _% Reflections to Rewards Pool (LP-Farming/Staking/etc...?)
  _% Auto LP
  _% Marketing/Team
  _% Gov
  _% Burn
...
contract Token is ERC20Snapshot, Ownable {
    ...
    constructor(
        ...
        uint256 _totalSupply = 420_000_000 ether;
    ...
    function setPresaleAddress(address _presaleAddress) external onlyOwner {
        require(presaleAddress == address(0), "Presale Address Already Set");
        canAddLiquidityBeforeLaunch[_presaleAddress] = true;
        isFeeExempt[_presaleAddress] = true;
        presaleAddress = _presaleAddress;
    }

    function initializePair() external onlyPresaleContract {
        require(!initialized, "Already initialized");
        require(
            msg.sender == presaleAddress,
            "You are not the presale or owner address"
        );
    ...
    function transfer(
        ...
        return _customTransfer(_msgSender(), to, amount);
    }
    ...
    function swapBack() internal swapping {
        ...
        uint256 amountTokenBurn = (taxAmount * burnFee) / (totalFee);
        uint256 amountTokenLp = (taxAmount * liquidityFee) / (totalFee);
        uint256 amountTokenBonus = (taxAmount * rewardsFee) / (totalFee);
        taxAmount -= amountTokenBurn;
        taxAmount -= amountTokenLp;
        taxAmount -= amountTokenBonus;
        ...
        _transfer(address(this), DEAD, amountTokenBurn);
        _approve(address(this), address(rewardsWallet), amountTokenBonus);
        IRewardsPool(rewardsWallet).injectRewards(amountTokenBonus);

        uint256 amountBackToken = WETH.balanceOf(address(this)) - balanceBefore;
        uint256 WETHTotalFee = totalFee - burnFee - liquidityFee - rewardsFee;
        uint256 amountBackTokenGov = (amountBackToken * govFee) /
            (WETHTotalFee);
        uint256 amountBackTokenDev = amountBackToken - amountBackTokenGov;

        WETH.transfer(govWallet, amountBackTokenGov);
        WETH.transfer(devWallet, amountBackTokenDev);
    ...
    /*** ADMIN FUNCTIONS ***/
    function launch() external onlyPresaleContract {
    ...
    function setBuyFees(
        ...
        devFeeBuy = _devFee;
        burnFeeBuy = _burnFee;
        totalFeeBuy = _liquidityFee + _rewardsFee + _devFee + _burnFee;
        require(totalFeeBuy <= 500, "Too fucking high bro");
    }
    ...
    function setSellFees(
        ...
        totalFeeSell = _liquidityFee + _rewardsFee + _devFee + _burnFee;
        require(totalFeeBuy <= 500, "Too fucking high bro");
    }
