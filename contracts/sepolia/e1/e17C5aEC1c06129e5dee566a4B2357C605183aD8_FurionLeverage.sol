...
contract FurionLeverage is OwnableUpgradeable {
    ...
    function _delegateSwapWhenShortWithWETH(
        ...
        uint256 wethAmount = _toWETHAmount(_extraSupplyAmountUSD);

        // Swap exact {actualBorrowed} F-Token to WETH
        uint256[] memory amounts = IFurionSwapV2Router(furionSwapRouter)
            .swapExactTokensForTokens(
                actualBorrowed,
                (wethAmount * SLIPPAGE) / 100,
                _path,
                address(this),
                block.timestamp
            );
    ...
    function _delegateSwapWhenShort(
        ...
        uint256 wethPrice = _ethPrice();
        uint256 wethAmount = (_extraSupplyAmountUSD * 1e30) / wethPrice;
    ...
    function _calculateAmountsWhenLongWithWETH(
        ...
        // How much WETH is needed to buy {actualSupply} F-Token
        uint256[] memory amounts = IFurionSwapV2Router(furionSwapRouter)
            .getAmountsIn(actualSupply, _path);

        uint256 wethAmountToSwap = amounts[0];
        ...
        )
    {
        ...
        wethAmountToSwap = amounts[0];

        // How much USDT is needed to swap that amount of WETH
        address[] memory uniswapPath = new address[](3);
        uniswapPath[0] = _cToken;
        uniswapPath[1] = FUR;
        uniswapPath[2] = WETH;
        uint256[] memory amountsInUniswap = IFurionSwapV2Router(uniswapRouter)
            .getAmountsIn(wethAmountToSwap, uniswapPath);
    ...
    function _ethPrice() internal pure returns (uint256 wethPrice) {
        // wethPrice = IPriceOracle(oracle).getUnderlyingPrice(WETH);
