...
library LibTraitManager {
  ...
  function selectRandomTrait(
    ...
    assembly {
      ...
      let layers := sload(add(layerData, 0x20))
      let traits := sload(add(layerData, 0x40))
      ...
      } lt(rnd, sload(add(add(traitsOffset, mul(0x20, traitIndex)), 0x20))) {

      } {
        rnd := sub(rnd, sload(add(add(traitsOffset, mul(0x20, traitIndex)), 0x20)))
