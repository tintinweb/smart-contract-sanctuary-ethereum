...
import {ForjERC1155} from "src/ForjERC1155.sol";
import {ForjTierLogic} from "src/ForjTierLogic.sol";
...
contract ERC1155SelfMinterTiered is ForjERC1155, ForjTierLogic {
    ...
    function initialize(
        ...
        _paymentInitializer(_mintFee, _paymentToken);
    ...
    function tierMint(
        ...
        (uint256[] memory _confirmedIds, uint256[] memory _confirmedAmounts) = _tierChecks(to, tierNo, ids, amounts, merkleProof);
        if(_confirmedIds.length == 0 || _confirmedAmounts.length == 0) revert NoConfirmedIds();
        
        uint256 amount = amountCalc(amounts);
        _mintFeeCheck(msg.sender, amount, token);

        for(uint256 i = 0; i < _confirmedIds.length; ++i) {
            supplyPerId[_confirmedIds[i]].total += _confirmedAmounts[i];
        }
        mintedPerAddress[to] += amount;

        _mintBatch(to, _confirmedIds, _confirmedAmounts, "");

        emit TierMint(to, tierNo, _confirmedIds, _confirmedAmounts);
    }
}
...
import {ForjTreasuryLogic} from "src/ForjTreasuryLogic.sol";
...
import {ForjERC1155} from "src/ForjERC1155.sol";
import {ForjPaymentLogic} from "src/ForjPaymentLogic.sol";
...
contract ForjTierLogic is ForjPaymentLogic{
    ...
    mapping(uint256 => uint256[]) public availableIdsForTierNumber;
    ...
    event TierMint(address indexed user, uint256 _tier, uint256[] indexed _ids, uint256[] indexed _amounts);
    ...
    function _tierLogicInitialize(
        ...
        for (uint256 i = 0; i < tiersNumbers.length; i++) {
            ...
            for(uint256 j = 0; j < idsAvailable[i].length; ++j){
                availableIdsForTierNumber[tiersNumbers[i]].push(idsAvailable[i][j]);
            }
        }
    ...
    function getAvailableIdsForTierNumber(uint256 _tierNumber) public view returns (uint256[] memory) {
        return availableIdsForTierNumber[_tierNumber];
    }
    ...
    function amountCalc(uint256[] memory _amounts) public view returns (uint256){
        uint256 amount;
    ...
    function _idCheck(
        uint256[] memory idsAllowed, 
        uint256[] memory idsDesired,
        uint256[] memory amountsDesired
    ) internal pure returns (uint256[] memory, uint256[] memory){

        uint256[] memory confirmedIds = new uint256[](idsAllowed.length);
        uint256[] memory confirmedAmounts = new uint256[](idsAllowed.length);

        for(uint256 i = 0; i < idsAllowed.length; ++i){
            for(uint256 j = 0; j < idsDesired.length; ++j){
                if(idsAllowed[i] == idsDesired[j]){
                    confirmedIds[i] = idsDesired[j];
                    confirmedAmounts[i] = amountsDesired[j];
                }
            }
        }
        return (confirmedIds, confirmedAmounts);
    }
    ...
    function _tierChecks(
        ...
        uint256[] memory _idsAllowed = getAvailableIdsForTierNumber(tierNo);
        return _idCheck(_idsAllowed, ids, amounts);
    }
}
...
import {ForjModifiers} from "src/ForjModifiers.sol";
...
contract ForjPaymentLogic is ForjTreasuryLogic{
    ...
    mapping(address => bool) public isPaymentTokenAccepted;

    function _paymentInitializer(uint256 _mintFee, address _paymentToken) internal {
        if(paymentInitialized) revert AlreadyInitialized();
        mintFee = _mintFee;
        acceptedTokenPrice[_paymentToken] = _mintFee;
        isPaymentTokenAccepted[_paymentToken] = true;
...
import {Ownable} from "lib/openzeppelin-contracts/contracts/access/Ownable.sol";
import {ForjCustomErrors} from "src/ForjCustomErrors.sol";
