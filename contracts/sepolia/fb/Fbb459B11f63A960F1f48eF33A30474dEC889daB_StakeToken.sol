...
contract StakeToken is Ownable{
    ...
    function showMyBalance(address _myAddres) public view returns(uint256){
      return myBalance[_myAddres] + myBalance[_myAddres] / 100 * APR / 365 /60 * (block.timestamp - myStartTime[_myAddres]) / 60;
    ...
    function showEarned(address _myAddres) public view returns(uint256){
        return myBalance[_myAddres] / 100 * APR / 365 /60 * (block.timestamp - myStartTime[_myAddres]) / 60 ;
    }
    ...
    function withdraw(uint256 _amount) public{
         require (_amount > 0 , "amount < 0");
         require (_amount <= myBalance[msg.sender] + (myBalance[msg.sender] / 100 * APR / 365 /60 * (block.timestamp - myStartTime[msg.sender]) / 60) , "insufficient funds");
         myStartTime[msg.sender] = block.timestamp;
         myBalance[msg.sender] -= _amount + myBalance[msg.sender] / 100 * APR / 365 /60 * (block.timestamp - myStartTime[msg.sender]) / 60;
