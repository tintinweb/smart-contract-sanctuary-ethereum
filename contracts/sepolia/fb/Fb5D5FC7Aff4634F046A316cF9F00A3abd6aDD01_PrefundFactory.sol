...
// This factory contract allows for the creation of crowdfunding campaigns.
contract PrefundFactory {
    ...
    // The collector of the fees from campaigns
    address public feeCollector;

    // Relative fee that applies to the funds raised by a campaign (0.5% = 500 per-mille)
    uint256 public relativeFee;

    // Fixed fee for the creation of a campaign
    uint256 public fixedFee;

    // Price for boosting a campaign
    uint256 public boostPrice;

    // Address of the NFT contract
    address public NFTContractAddress;

    // Number of free boosts granted per NFT
    uint256 public freeBoostsPerNFT;

    // Array of all campaign addresses
    ...
    // 
    // --------------------------------------------------------
    event CampaignCreated(address campaingAddress);
    ...
    event FreeBoostsPerNFTUpdated(uint256 freeBoostsPerNFT);
    ...
    constructor(
        ...
        freeBoostsPerNFT = _freeBoostsPerNFT;
    }
    ...
    function createCampaign (
        ...
        require(msg.value == 0 || msg.value == fixedFee, "Amount of ETH sent is incorrect");
        ...
        PrefundCampaign newCampaign = new PrefundCampaign(
        ...
        // Transfer the ETH sent to the method to the feeCollector
        if (msg.value > 0) {
    ...
    function updateFreeBoostsPerNFT(uint256 _freeBoostsPerNFT) external onlyOwner {
        freeBoostsPerNFT = _freeBoostsPerNFT;
        emit FreeBoostsPerNFTUpdated(_freeBoostsPerNFT);
    }
    ...
    function getConfig() external view returns (address, address, uint256, uint256, uint256, address, uint256) {
        return (owner, feeCollector, relativeFee, fixedFee, boostPrice, NFTContractAddress, freeBoostsPerNFT);
    }
    ...
    function getCampaigns(uint256 page, uint256 limit) public view returns (address[] memory) {
        require(page > 0, "Page must be at least 1");
        require(limit > 0, "Limit must be at least 1");
        require(page <= (campaigns.length / limit) + 1, "Page out of range");
