...
// import {IDEDIndex} from "./interfaces/IDEDIndex.sol";

/// @title A contract to manage listings and profiles of decentralized education platform (DED)
/// @author <@quigela> <@pynchmeister>

contract DEDIndex is Ownable, ReentrancyGuard {
    ...
    function submitListing(string memory subject1, string memory topic1, 
        string memory objectives1, uint256 postAmount1) payable nonReentrant public returns (DEDListing newListing){
        ...
        // Transfer ether to the new listing contract
        payable(address(newListing)).transfer(msg.value);
        
        generateProfile();

        // Emit event after interactions
    ...
    function generateProfile() internal nonReentrant {
        // Checks
        require(address(profiles[msg.sender]) == address(0x0), "Profile already exists");
        ...
        // Interactions
        newProfile.transferOwnership(msg.sender);
    }
}
