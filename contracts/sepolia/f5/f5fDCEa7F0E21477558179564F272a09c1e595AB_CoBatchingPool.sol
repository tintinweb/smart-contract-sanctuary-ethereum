...
contract CoBatchingPool is Ownable {
        ...
        )
    {
        ...
        uint256[] memory _nftIds = new uint256[](totalPools);
        address[] memory _nftContractAddresses = new address[](totalPools);

        for (uint256 i = 0; i < totalPools; i++) {
            if (pools[i + 1].admin == msg.sender) {
                Pool storage pool = pools[i + 1];
                _itemIds[i] = pool.itemId;
                _nftIds[i] = pool.nftId;
                _nftContractAddresses[i] = pool.nftContract;
                Count += 1;
            }
        }
        return (_itemIds, _nftIds, _nftContractAddresses);
    }
