...
contract NFTAuction is ReentrancyGuard, Ownable {
    ...
    function setPositions(address[] memory positions, uint base) external nonReentrant onlyOwner {
        if (block.timestamp < endTime)
            revert AuctionIsStillOngoing();
    ...
    function withdrawAdminFunds() external nonReentrant onlyOwner {
        ...
        for (uint256 i = 0; i < users.length; i++) {
            if (userPositions[users[i]] > 0)
                amount += userBids[users[i]];
        }
    ...
    function testPlaceBid(address user, uint256 amount) public nonReentrant {
        if (userBids[user] == 0)
            users.push(user);

        userBids[user] += amount;
    }
