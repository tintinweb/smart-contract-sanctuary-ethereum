...
contract Crowdfunding {
    ...
    /**
     * @dev Withdraws the funds raised by a project.
     * @param _projectId The project ID.
     */
    function withdrawFunds(uint256 _projectId) external onlyOwner {
        Project storage project = projects[_projectId];
        require(project.closed, "The project is not closed yet");
    ...
    /**
     * @dev Returns the details of a project.
     * @param _projectId The project ID.
     */
    function getProjectContributions(uint256 _projectId) external view returns (uint256) {
        return projects[_projectId].contributions[msg.sender];
    }


    function getProjectReward(uint256 _projectId) external view returns (bool) {
        return projects[_projectId].rewardClaimed[msg.sender];
    }
}