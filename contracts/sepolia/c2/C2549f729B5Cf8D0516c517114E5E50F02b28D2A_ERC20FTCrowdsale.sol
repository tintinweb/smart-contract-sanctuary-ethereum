    ...
    Whitelist
{
    ...
    ILisaSettings private settings;
     ...
     * @param protocolFeeAmount Total amount of AT tokens additionally minted during this crowdsale
    ...
    function initialize(
        ...
        amountLeftAT = crowdsaleAmount - sellerRetainedAmount;
     ...
     */
    function getAllocationFor(address buyer) public view returns (uint256) {
        return allocationsAT[buyer];
    }
    ...
    function claimProtocolFee() external {
        ...
        token.safeTransfer(settings.protocolFeeWallet(), protocolFeeAmount);
    }
    ...
    function claimSaleProceeds() public virtual nonReentrant returns (uint256) {
        ...
        collectedBT = 0;
        tokenBT.safeTransfer(seller, amountBT);
        return amountBT;
    }
    ...
    function claimTokens() public virtual nonReentrant returns (uint256) {
        ...
        token.safeTransfer(_msgSender(), amountAT);
        return amountAT;
    }
        ...
        returns (uint256)
    {
        ...
        tokenBT.safeTransfer(_msgSender(), refundBT);
        return refundBT;
    }
    ...
    function _preValidatePurchase(
        ...
        require(
            tokenBT.balanceOf(buyer) >= amountBT,
            "Crowdsale: buyer does not have enough funds"
        );
        require(
            tokenBT.allowance(buyer, address(this)) >= amountBT,
    ...
    function _postValidatePurchase(
        ...
        assert(
            costBT(totalForSaleAT - amountLeftAT - allocationsAT[seller]) ==
                collectedBT
        );
        assert(amountAT <= getAllocationFor(buyer));
        assert(getAllocationFor(buyer) <= getTokenAmount(maxPurchaseBT));
        assert(collectedBT <= tokenBT.balanceOf(address(this)));
    }
    ...
    function _processPurchase(
        ...
        tokenBT.safeTransferFrom(buyer, address(this), amountBT);
    }
    ...
    function _updatePurchasingState(
        ...
        amountLeftAT = amountLeftAT.sub(amountAT);
...
contract ArtTokenVault is AccessControlUpgradeable, IArtTokenVault {
    ...
    function _deployCrowdsale(
        ...
        address newCrowdsale = Clones.clone(
            settings.getLogic(keccak256("CrowdsaleV1"))
        );
        ICrowdsale(newCrowdsale).initialize(
            rate,
            seller,
            tokenAT,
            baseToken,
        ...
        return ICrowdsale(newCrowdsale);
    }
     ...
     * @param   minBaseTokenParticipation  .
     * @param   maxBaseTokenParticipation  .
    ...
    function deployBuyout(uint256 amountBT) public returns (address) {
        ...
        IArtBuyout newBuyout = IArtBuyout(
            Clones.clone(settings.getLogic(keccak256("BuyoutV1")))
        );
