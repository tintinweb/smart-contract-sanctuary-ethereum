...
contract SupplychainManagement{
    ...
    function generateProductHash(string memory _name,string memory description, uint256 productId) pure internal returns(bytes32){
       return keccak256(abi.encode(_name,description,productId));
    }

    function registerProduct(string memory _name,string memory description, uint256 productId,string memory productSerialNumber) external returns(bytes32){
    ...
    function addSupplyChainDetails(bytes32 productHash, string memory _name, uint256 productId,string memory productSerialNumber,string memory source, string memory destination, string memory remarks,string memory status) external returns(ProductSupply memory){
