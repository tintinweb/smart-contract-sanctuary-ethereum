...
contract AlturaLootboxV3 is UUPSUpgradeable, ERC1155Upgradeable, AccessControlEnumerableUpgradeable {
    ...
    event BoxLaunched(uint256 boxId, uint256 initialSupply, address paymentToken, uint256 price);
    event BoxSupplyIncreased(uint256 boxId, uint256 supplyIncrease);
    event BoxPriceChanged(uint256 boxId, address paymentToken, uint256 price);
    event BoxPaused(uint256 boxId, bool isPaused);
    event ItemAdded(uint256 boxId, address collectionAddress, uint256 tokenId, uint256 rarity, uint256 supply);
    event KeyPurchased(uint256 boxId, address buyer);
    event KeyUsed(uint256 boxId, address user, address collectionAddress, uint256 tokenId);
    ...
    function launchBox(uint256 boxId, uint256 initialSupply, address paymentToken, uint256 price) external {
    ...
    function increaseBoxSupply(uint256 boxId, uint256 supplyIncrease) external {
        Box storage targetBox = _getBoxInternal(boxId);
        require(targetBox.creator == msg.sender, "Only creator");
    ...
    function addItem(
        ...
        targetBox.items.push(Item(collectionAddress, tokenId, rarity, !isERC721 ? supply : 1, isERC721));
        targetBox.totalRarity += rarity;

        emit ItemAdded(boxId, collectionAddress, tokenId, rarity, supply);
    }
    ...
    function useKey(uint256 boxId) external {
        ...
        while (j < maxLength) {
            ...
            if (pseudoRandom < item.rarity && item.supply > 0 && item.rarity > 0) {
                ...
                emit KeyUsed(boxId, sender, item.collectionAddress, item.tokenId);
                return;
            }
