...
library LibTraitManager {
  ...
  function selectRandomTrait(
    ...
    assembly {
      ...
      let traitWeightPtr := add(traitsOffset, 0x20) // Offset to the weight field of the first trait
      let traitWeight := sload(traitWeightPtr)

      for {

      } lt(randomValue, traitWeight) {

      } {
        randomValue := sub(randomValue, traitWeight)
        index := add(index, 1)
        traitWeightPtr := add(traitWeightPtr, 0x60) // Offset to the weight field of the next trait
        traitWeight := sload(traitWeightPtr)
      }
    }
