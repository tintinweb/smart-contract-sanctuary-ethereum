...
contract MoonboyzToken is ERC20, AccessControl {
  ...
  constructor(
    ...
    _grantRole(EXEMPT_ROLE, msg.sender);
    _grantRole(EXEMPT_ROLE, address(this));
  }

  function claimMBZ(uint256[] memory tokenIds, bool claimDark) external {
    ...
    if (tokenIds.length > 0) {
      for (uint i = 0; i < tokenIds.length; i++) {
        ...
        require(tokenClaim.totalClaimTime <= earnPeriod, "Token already claimed");
    ...
    uint256 total = darkClaimableMbz + totalEarned / 1 days;
    if (total > 0)
  ...
  function _getMbzFromDark(uint256 darkAmount) internal view returns (uint256) {
    return darkAmount / darkConversionRate;
  }
