...
contract SafuuArb {
    ...
    event Withdrawn(address token, address sender, uint256 amount);
    ...
    function stake(address _token, uint256 amount) external payable {
        ...
        if (_token == weth) {
            ...
            IWETH wethToken = IWETH(_token);
            wethToken.deposit{value: msg.value}();
            stakedAmountPerToken[_token] = stakedAmountPerToken[_token].add(
                msg.value
            );
            userData[sender].userBalance[_token] = userData[sender]
                .userBalance[_token]
                .add(amount);
            emit Staked(_token, sender, msg.value);
            if (!isUser[sender]) {
                users.push(sender);
                isUser[sender] = true;
            }
    ...
    function withdraw(address token, uint256 amount) external {
        ...
        if (token == weth) {
            ...
            require(
                payable(msg.sender).send(amountInEther),
                "ETH transfer failed"
            );
        } else {
            require(
