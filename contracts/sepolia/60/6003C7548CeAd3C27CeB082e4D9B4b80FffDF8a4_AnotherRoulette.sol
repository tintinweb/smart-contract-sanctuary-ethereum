...
contract AnotherRoulette is VRFV2WrapperConsumerBase {
    uint64 constant MAX_AMOUNT_ALLOWED_IN_THE_BANK = 2 ether;
    ...
    function placeBet(uint8 _number, uint8 _betType) external payable {
        ...
        require(_betType <= 5 && _number <= numberRange[_betType], "Incorrect bet type or value.");
        require(gamesCount == 0 || games[gamesCount-1].fulfilled == true, "Last game not finished");
    ...
    function spin() external returns (uint256) {
        require(players[msg.sender].isExists || msg.sender == owner, "Only the player or owner can spin.");
        require(games[gamesCount].started, "Nobody in the game.");
        require(
            games[gamesCount].startAt + TIMER < block.timestamp,
            "Timer has not expired."
        );
        require(!games[gamesCount].isActive, "Game active");
    ...
    function allWithdraw() external onlyOwner {
        require(address(this).balance > 0, "No money on the contract's balance.");
