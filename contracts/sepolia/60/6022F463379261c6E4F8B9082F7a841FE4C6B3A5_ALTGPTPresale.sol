...
contract ALTGPTPresale is Ownable, ReentrancyGuard {
    ...
    constructor(address _dataFeed) {
        ...
        // test
        setALTGPT(address(0x68d705f31e53a255E8079af0DB247a71A4Dfaafc));
        setALTGPT(address(0x4C4aBd6aeaDD95d462FBC0cbdECB9fDd741e51C2));
        start();
    }
    ...
    function setUSDT(address _tokenAddress) public onlyOwner {
    ...
    function purchaseETH() external payable nonReentrant() {
        require(currentStage > 0, "Please start presale first");
        require(msg.value > 0, "Insufficient ETH");
        uint256 amount = calculateETH2Amount(msg.value);
        ...
        ALTGPT.transfer(msg.sender, amount);
    }

    function purchaseUSDT(uint256 _amount) external {
        require(currentStage > 0, "Please start presale first");
        require(_amount > 0, "Insufficient USDT");
        uint256 amount = _amount / stages[currentStage].price * (10 ** 18);
        require(amount <= ALTGPT.balanceOf(address(this)), "Insufficient balance");
        require(amount <= stages[currentStage].totalAmount - stages[currentStage].currentAmount, "Presale quantity beyond this stage.");
        stages[currentStage].currentAmount += amount;
