...
import {ERC721AUpgradeable} from "./core/ERC721/ERC721AUpgradeable.sol";
import {ERC2981Upgradeable} from "./core/ERC2981/ERC2981Upgradeable.sol";
import {IERC721ProjectApproveTransferManager} from "./managers/ERC721/IERC721ProjectApproveTransferManager.sol";
import {IERC721ProjectBurnableManager} from "./managers/ERC721/IERC721ProjectBurnableManager.sol";
import {IProjectTokenURIManager} from "./managers/project-token-uri-manager/IProjectTokenURIManager.sol";

/// added transferByPermit
contract TRLabERC721AImpl is
    Initializable,
    ...
    UUPSUpgradeable
{
    ...
    error NotManager(address operator);
    error ManagerNotImplementApproveTransferManager();
    error PermitExpired();
    error InvalidPermitSig();
    error BurnCallerIsNotOwnerNorApproved();
    error ManagerApprovalFailure();
    ...
    mapping(uint256 => bool) internal _adminMinted;
    ...
    function initialize(string memory _name, string memory _symbol, string memory _contractUri) public initializer {
    ...
    function setManager(address _manager) external onlyOwner {
        ...
        if (oldManager != _manager) {
            ...
            if (managerApproveTransfer) {
                ...
                emit LogManagerApproveTransferUpdated(_msgSender(), false);
            }
        }
    }
    ...
    function managerSetApproveTransfer(bool enabled) external onlyManager {
        if (enabled && !ERC165CheckerUpgradeable.supportsInterface( _msgSender(), type(IERC721ProjectApproveTransferManager).interfaceId)) {
            revert ManagerNotImplementApproveTransferManager();
        }
        if (managerApproveTransfer != enabled) {
            managerApproveTransfer = enabled;
            emit LogManagerApproveTransferUpdated(_msgSender(), enabled);
        }
    }
    ...
    function transferByPermit(address to, uint256 tokenId, uint256 deadline, bytes calldata sig) external {
        ...
        bytes32 digest = ECDSAUpgradeable.toTypedDataHash(
            DOMAIN_SEPARATOR, keccak256(abi.encode(PERMIT_TYPEHASH, to, tokenId, nonces[tokenId]++, deadline))
        );
        ...
        // bypass the onlyAllowedOperator modifier
    ...
    function adminMint(address to, uint256 quantity) external onlyOwner {
        ...
        for (uint256 i = nextId; i < nextId + quantity;) {
            _adminMinted[i] = true;
            unchecked {
                i++;
            }
    ...
    function burn(uint256 tokenId) public nonReentrant {
        ...
        bool isApprovedOrOwner =
            (_msgSender() == owner || isApprovedForAll(owner, _msgSender()) || approvedAddress == _msgSender());
        if (!isApprovedOrOwner) {
            revert BurnCallerIsNotOwnerNorApproved();
        }
        _burn(tokenId);
    ...
    function _approveTransfer(address from, address to, uint256 tokenId) internal {
        if (managerApproveTransfer) {
            if (!IERC721ProjectApproveTransferManager(manager).approveTransfer(from, to, tokenId)) {
                revert ManagerApprovalFailure();
            }
        }
    }
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        if (
            !_adminMinted[tokenId]
                && ERC165CheckerUpgradeable.supportsInterface(manager, type(IProjectTokenURIManager).interfaceId)
        ) {
...
contract ERC721AUpgradeable is Initializable, ContextUpgradeable, IERC721AUpgradeable {
    ...
    function _transfer(address from, address to, uint256 tokenId) private {
        ...
        bool isApprovedOrOwner =
            (_msgSender() == from || isApprovedForAll(from, _msgSender()) || approvedAddress == _msgSender());
        ...
        unchecked {
            ...
            _packedOwnerships[tokenId] =
                _packOwnershipData(to, BITMASK_NEXT_INITIALIZED | _nextExtraData(from, to, prevOwnershipPacked));
...
abstract contract ERC2981Upgradeable is Initializable, IERC2981Upgradeable, ERC165Upgradeable {
    ...
    function _setTokenRoyalty(uint256 _id, address _receiver, uint256 _royaltyBPs)
        internal
    ...
    function supportsInterface(bytes4 interfaceId)
        ...
        returns (bool)
    {
        return interfaceId == type(IERC2981Upgradeable).interfaceId || interfaceId == _INTERFACE_ID_ERC2981
