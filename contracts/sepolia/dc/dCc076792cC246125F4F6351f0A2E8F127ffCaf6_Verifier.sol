 ...
 * @author Knoct
 * @notice verifier management of credential i.e creating new templete , generating templete DID
 */

contract Verifier is NativeMetaTransaction {
    /*state variables */
    ...
    // mapping of verifierDID to list of templeteDID created by verifier
    //verifierDID --> templeteDID[]
    mapping(string => string[]) issuedTempletes;

    /*Constructor */

    constructor(address authAddress, address holderContractAddress) {
    ...
    /* Generate templeteDID from verifierDID, block.timestamp and block.diffuculty*/
    function generateTempleteDID(
        string memory verifierDID
    ) private view returns (string memory) {
        // Generating hash for templete DID
        ...
        string memory did = "did:templete:"; // replace with  DID method
        // packed encoding and converting into string
        did = string(abi.encodePacked(did, hash));
        return did;
    }

    // Verifier function to create a new Templete of given(input) dataTypes type
    // and pushing templeteDID to issuedTemplates

    function createNewTemplete(string[] calldata dataTypes) external {
        // get the VerifierDID from getDID function of DataLib library
        string memory VerifierDID = DataLib.getDID();
        // Generating templeteDID
        string memory templeteDID = generateTempleteDID(VerifierDID);
        // calling creatNewTemplete method of authority to create new templete corresponding to given datatypes
        authority.createNewTemplete(templeteDID, dataTypes);
        // pushing templeteDID to list of issued Templetes
        issuedTempletes[VerifierDID].push(templeteDID);
    }

    // Get all templetes issued by verifier
    function getMyTempletes() external view returns (string[] memory) {
        return authority.getMyTempletes();
    }

    // Get Templete information i.e verifierDID and dataTypes from templeteDID
    function getTempleteById(
        string calldata templeteDID
    ) external view returns (string memory, string[] memory) {
        return authority.getTemplete(templeteDID);
    }

    // Get verified HolderDIDs corresponding to given tempeleteData
    // tempeleteData= templeteDID
    function getVerifiedHolders(
        string calldata templeteData
    ) external view returns (string[] memory) {
        return holderContract.getVerifiedHoldersToATemplete(templeteData);
    }

    function getTempleteDID() external view returns(string memory){
        string memory VerifierDID = DataLib.getDID();
          return generateTempleteDID(VerifierDID);
    }
}
