...
contract MarketFees is Ownable {
	...
	mapping(address => uint256) private s_accumulatedRoyalties;
	...
	function contributeFees(
		...
		uint256 royaltyFees;
		for (uint256 i = 0; i < s_royaltyConfigs[collection].length; i++) {
			address reciever = s_royaltyConfigs[collection][i].receiver;
			uint256 ethShare = (fromAmount * s_royaltyConfigs[collection][i].share) /
				BASIS_POINTS_PER_HUNDREED_PERCENT;

			s_accumulatedRoyalties[reciever] += ethShare;
			royaltyFees += ethShare;
		}
	...
	/**
	 * @notice Withdraw accumulated royalties for a wallet.
	 * @param wallet The address of the wallet to withdraw royalties for.
	 */
	function withdrawRoyalties(address payable wallet) external {
		uint256 amount = s_accumulatedRoyalties[wallet];
		s_accumulatedRoyalties[wallet] = 0;

		emit RoyaltiesWithdrawn(wallet, amount);

		_transferFunds(amount, wallet);
	}
	...
	/**
	 * @notice Get the accumulated royalties for a specific wallet.
	 * @param wallet The address of the wallet.
	 * @return The accumulated royalties for the wallet.
	 */
	function getAccumulatedRoyalties(address wallet) external view returns (uint256) {
		return s_accumulatedRoyalties[wallet];
	}
