...
import "./QSignFaucetExchange.sol";

contract QSignFaucetExchangeWrapper is QSignFaucetExchange {
    uint256 internal defaultGasPrice = 50000000000; // 50 GWEI
    ...
    function submitTransactionToFuji(uint256 _gasPrice) external payable {
        _gasPrice = adjustGasPrice(_gasPrice);
        submitTransaction(fujiChainId, 0, msg.sender, _gasPrice, gasLimit);
    }

    function submitTransactionToFuji(
        address _to,
        uint256 _gasPrice
    ) external payable {
        _gasPrice = adjustGasPrice(_gasPrice);
        submitTransaction(fujiChainId, 0, _to, _gasPrice, gasLimit);
    }

    function submitTransactionToMumbai(uint256 _gasPrice) external payable {
        _gasPrice = adjustGasPrice(_gasPrice);
        submitTransaction(mumbaiChainId, 0, msg.sender, _gasPrice, gasLimit);
    }

    function submitTransactionToMumbai(
        address _to,
        uint256 _gasPrice
    ) external payable {
        _gasPrice = adjustGasPrice(_gasPrice);
        submitTransaction(mumbaiChainId, 0, _to, defaultGasPrice, gasLimit);
    }

    function adjustGasPrice(uint256 _gasPrice) internal view returns (uint256) {
        if (_gasPrice == 0) {
...
// Interface for the QSignFaucetExchange contract
interface IQSignFaucetExchange {
    function configExecutor(bytes32 chainId, address payable executor) external;
...
import "./interfaces/IQSignFaucetExchange.sol";

contract QSignFaucetExchange is QSignConnect, IQSignFaucetExchange {
    using StringToAddress for string;

    mapping(bytes32 => address payable) internal executors;
    mapping(bytes32 => uint256) internal nonces;

    mapping(bytes32 => mapping(uint256 => Transaction)) internal transactions;
    ...
    function configExecutor(bytes32 chainId, address payable executor) external {
        require(executors[chainId] == address(0), "executor already configured");
        executors[chainId] = executor;
    }
    ...
    function submitTransaction(
        ...
        require(getEVMWallet(_fromKeyIndex).stringToAddress() != address(0), "incorrect from key index");
        bytes memory data = rlpEncodeData(abi.encodePacked(block.chainid));

        uint256 transactionFee = _gasPrice * _gasLimit;
        require(
        ...
        uint256 nonce = nonces[_dstChain];

        Transaction storage t = transactions[_dstChain][nonce];
        ...
        uint256 collateralAmount = msg.value-_qSignFee;
        executors[_dstChain].transfer(collateralAmount);
    ...
    function speedUpTransasction(
        ...
        bytes memory data = rlpEncodeData(abi.encodePacked(block.chainid));
        Transaction storage t = transactions[_dstChain][_nonce];
        require(msg.sender == t.initiator, "only initiator");
        uint256 oldFee = t.fee;
        uint256 newFee = _gasPrice * _gasLimit;
    ...
    function getTransaction(bytes32 _dstChain, uint256 _nonce)
        external
        view
        returns (Transaction memory)
    {
        return transactions[_dstChain][_nonce];

    }

    function getNonce(bytes32 _dstChain) external view returns (uint256) {
        return nonces[_dstChain];
    }

    function getExecutor(bytes32 chainId) external view returns (address payable) {
        return executors[chainId];
    }
}