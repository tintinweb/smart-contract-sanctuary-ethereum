 ...
 * The accepted token to buy credits is LINK (Chainlink ERC 677), the amount of LINK is used to fund the keeper and execute the order, a percentage of that amount will be used to buy and burn NINEINCH.
 */
    ...
    Ownable
{
    ...
    //---------- Contracts ----------//
    KeeperRegistryInterface private immutable keeperRegistery; //Keeper registry contract.
    INineInchRouter02 private immutable nineInchRouter; // JamonSwap Router contract.
    IERC20 private immutable link; // LINK (Chainlink ERC 677) contract.

    //---------- Variables ----------//
    uint256 public baseFee; // Price of credit.
    uint256 public burnFee; // Fee to buy and burn NINEINCH.
    ...
    //---------- Constructor ----------//
    constructor() {
        keeperRegistery = KeeperRegistryInterface(
            0xE16Df59B887e3Caa439E0b29B42bA2e7976FD8b2
        );
        nineInchRouter = INineInchRouter02(
            0x722f0bD8810863Fa840A1d3d663A230a04A9F468
        );
    ...
    modifier onlyKeeper() {
        require(
            _msgSender() == address(keeperRegistery),
    ...
    modifier onlyOnKeeperActive() {
        (address target, , , , , , ) = keeperRegistery.getUpkeep(keeperId);
        require(target == address(this), "Uninitialized keeper");
        _;
    }
    ...
    /**
     * @notice Show the amount of LINK needed to buy a credit.
     * @return The amount of LINK.
     */
    function creditPrice() public view returns (uint256) {
        address[] memory path = _getPath(
            address(nineInchRouter.WETH()),
            address(link)
        );
        (, uint256 _creditPrice) = getPrice(baseFee, path);
        return _creditPrice;
    }
    ...
    /**
     * @notice Buy credits and fund keepers, also send a percentage to buy and burn NINEINCH.
     * @param amount The amount of credits to buy.
     */
    function buyCredits(
        uint256 amount
    ) external whenNotPaused nonReentrant onlyOnKeeperActive {
        require(amount > 0, "Zero amount");
        uint256 _creditPrice = creditPrice().mul(amount);
        require(
            link.transferFrom(_msgSender(), address(this), _creditPrice),
            "Transfer LINK failed"
        );
        uint256 toBurn = _creditPrice.mul(burnFee).div(10000);
        /*
        require(
            link.transfer(burnContract, toBurn),
            "Transfer LINK burn failed"
        );
        */
        require(link.transfer(owner(), toBurn), "Transfer LINK burn failed");
        uint256 funds = _creditPrice.sub(toBurn);
        if (link.allowance(address(this), address(keeperRegistery)) == 0) {
            link.approve(address(keeperRegistery), ~uint256(0));
        }
        keeperRegistery.addFunds(keeperId, funds.toUint96());

        unchecked {
            credits[_msgSender()] += amount;
        }
        emit BoughtCredit(_msgSender(), amount);
    }

    /**
     * @notice Transfer credits to another wallet.
    ...
    function transferCredits(
        ...
        require(amount > 0 && to != address(0x0), "Invalid inputs");
        require(to != _msgSender(), "Invalid recipient");
        require(credits[_msgSender()] >= amount, "Insufficient credit");

        unchecked {
            credits[_msgSender()] -= amount;
            credits[to] += amount;
        }
        emit TransferCredit(_msgSender(), to, amount);
    }
    ...
    function createOrder(
        ...
        require(credits[_msgSender()] >= credits_, "No credits available");
        ...
        if (tokenIn == address(nineInchRouter.WETH())) {
            require(msg.value == prices[2], "Insufficient balance");
        }
    ...
    /**
     * @notice Add credits to wallet.
     * @param wallet_ Wallet to add the credits.
     * @param credits_ Amount of credits to add.
     */
    function addCredits(address wallet_, uint256 credits_) external onlyOwner {
        credits[wallet_] += credits_;
    }
    ...
    /**
     * @notice Set the base fee and the percentage to buy and burn NINEINCH.
     * @param baseFee_ The base fee in MATIC to calculate the credit price.
     * @param burnFee_ The percentage of the base fee to buy and burn.
     */
    function setFees(uint256 baseFee_, uint256 burnFee_) external onlyOwner {
        require(baseFee_ != 0 && burnFee_ != 0, "Zero fee");
        baseFee = baseFee_;
        burnFee = burnFee_;
    }
