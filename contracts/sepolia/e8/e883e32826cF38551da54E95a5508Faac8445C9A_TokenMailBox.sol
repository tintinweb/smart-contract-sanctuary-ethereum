...
contract TokenMailer is Ownable {
    ...
    mapping(Token => address) public Tokens;

    function updateTokenInfo(Token _tokenName,address _tokenAddress) external onlyOwner() {
        Tokens[_tokenName] = _tokenAddress;
    }

    function sendToken(uint32 _destinationChainId, address _destinationTokenBox,Token _tokenName,uint256 _tokenAmount) external {
        IERC20(Tokens[_tokenName]).safeTransferFrom(msg.sender, address(this), _tokenAmount);
        ...
        bridgeRouter.sendViaStorage(_destinationChainId, _destinationTokenBox,message);
    }
}
...
contract TokenMailBox is BridgeHandler, Ownable{
    ...
    Info[] public messages;

    event MessageReceived(uint32 indexed sourceChainId, address indexed sourceAddress, Info message);

    constructor(address _bridgeRouter) BridgeHandler(_bridgeRouter) {}

    function setContractApprove(address tokenAddress) external onlyOwner(){
        IERC20(tokenAddress).approve(bridgeRouter,type(uint256).max);
    }

    function handleBridgeImpl(uint32 _sourceChainId, address _sourceAddress, bytes memory _message)
        internal
        override
    {
        Info memory info = decodeInfo(_message);
        IERC20(info.tokenAddress).safeTransferFrom(address(this),info.sender, info.tokenAmount);
        
        messages.push(info);
        emit MessageReceived(_sourceChainId, _sourceAddress, info);
    }

    function decodeInfo(bytes memory _message) public pure returns(Info memory info){
        address tokenAddress; // 20 bytes
        uint256 tokenAmount; // 256 / 8 = 32
        address sender; // 20 bytes
        assembly {
            tokenAddress:= mload(add(_message, 20))
            tokenAmount:= mload(add(_message, 52))
            sender:= mload(add(_message, 72))
        }
