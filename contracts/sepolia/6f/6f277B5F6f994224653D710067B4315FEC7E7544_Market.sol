...
contract Market is IMarket {
    ...
    function _contractAddress(address contractAddress, address seller)
        private
        view
    {
    ...
    function sell(
        ...
        if (isERC721(contractAddress)) {
            ...
            _saleOrAuction(tokenId, contractAddress, address(0), amount);
        ...
        saleInfo[contractAddress][tokenId][sellBy] = MarketInfo(
            msg.sender,
            tokenId,
            amount,
            price
    ...
    function buy(
        ...
        amountOfToken(contractAddress, amount);
        if (isERC721(contractAddress)) {
            notOnSell(saleInfo[contractAddress][id][address(0)].owner);
            from = address(0);

            address owner = saleInfo[contractAddress][id][address(0)].owner;
            ...
            // );
            inputPrice(saleInfo[contractAddress][id][address(0)].price);
    ...
    function cancelSell(address contractAddress, uint256 id) external {
        ...
        notOnSell(saleInfo[contractAddress][id][cancelBy].owner);
        delete saleInfo[contractAddress][id][cancelBy];
    }
    ...
    function setAuction(
        ...
        if (isERC721(contractAddress)) {
            ...
            _saleOrAuction(id, contractAddress, address(0), amount);
        ...
        auctionInfo[contractAddress][id][auctionFrom] = AuctionInfo(
            msg.sender,
            id,
            amount,
            basePrice,
    ...
    function bidInAuction(
        ...
        _contractAddress(contractAddress, seller);
        require(
    ...
    function claimToken(
        ...
        if (isERC721(contractAddress)) {
            ...
            erc721Transfer(
                ...
                id,
                owner,
                auctionInfo[contractAddress][id][address(0)].maxAddress
            );
            seller = address(0);
        } else if (isERC1155(contractAddress)) {
        ...
        emit ClaimToken(contractAddress, id, __owner);
    }
    ...
    function cancelAuction(address contractAddress, uint256 id) external {
        ...
        notOnSell(auctionInfo[contractAddress][id][cancelFrom].owner);
        require(
        (block.timestamp < auctionInfo[contractAddress][id][cancelFrom].auctionTime)|| (auctionInfo[contractAddress][id][cancelFrom].maxAddress == address(0)),"cancel not allowed" );

        for (
            uint256 index;
            index < bidInfo[contractAddress][id][cancelFrom].bids.length;
            index++
        ) {
        ...
        delete auctionInfo[contractAddress][id][cancelFrom];
    }
