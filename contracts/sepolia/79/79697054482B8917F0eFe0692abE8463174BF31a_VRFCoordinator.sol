    ...
    IVRFMigration
{
        ...
        returns (uint256)
    {
        ...
        ) = beaconRequest(subID, requester, numWords, confDelay);
        uint256 costJuels = billSubscriberForRequest(requester, subID); // throws on failure
        ...
        returns (uint256)
    {
        ...
        if (arguments.length > s_config.maxCallbackArgumentsLength) {
            revert CallbackArgumentsLengthTooBig(
                uint32(arguments.length),
                s_config.maxCallbackArgumentsLength
            );
        }
        (
        ...
        (
            ...
            callback.gasPrice,
            balance
        ) = billSubscriberForCallback(callback);
        ...
        RandomnessLogPayload memory log = RandomnessLogPayload({
            ...
            callback: callback,
            arguments: arguments,
            costJuels: costJuels, // throws on failure
            balance: balance
        });
        ...
        returns (uint256[] memory randomness)
    {
        ...
        if (r.requester != sender) {
            revert ResponseMustBeRetrievedByRequester(r.requester, sender);
        }
        ...
        emit RandomnessRedeemed(requestID, sender, subID);
    ...
    function getSeedByBlockheightKey(
        ...
        return uint256((blockHeight << 24) | ConfirmationDelay.unwrap(confDelay));
    }
    ...
    function processCallback(
    ...
    ) internal returns (bool isErr, bytes memory errmsg, uint96 subBalance) {
        ...
        IVRFRouterConsumer consumer = IVRFRouterConsumer(request.requester);
        ...
        uint256 gasBefore = gasleft();
        {
            // scoped to avoid stack too deep error
            bool sufficientGas;
            (success, sufficientGas) = i_router
    ...
    constructor(
        ...
        address linkEthFeed,
        address router
    ) VRFBeaconBilling(linkToken, linkEthFeed, router) {
    ...
    /// @dev blocks currently require an answer. (NewHead is used to invalidateProducer
    ...
    function beaconRequest(
        ...
        uint16 numWords,
        ConfirmationDelay confirmationDelayArg
    ) internal returns (RequestID, BeaconRequest memory, uint64) {
        ...
        RequestID requestID = computeRequestID(subID, requester, rp.nonce);
        // Ensure next request has unique nonce
        s_requestParams.nonce = rp.nonce + 1;
    ...
    function transferLink(
        address recipient,
        uint256 juelsAmount
    ) public override validateProducer {
    ...
    /// @inheritdoc IVRFCoordinatorProducerAPI
    function setReasonableGasPrice(
        uint64 gasPrice
    ) external override validateProducer {
        if (s_billingConfig.useReasonableGasPrice) {
            s_gasPriceData = GasPriceData({
                blockNumber: uint64(block.number),
                reasonableGasPrice: gasPrice
            });
        }
    }
    ...
    function getFee(
        ...
        return uint256(calculateRequestPriceJuels());
    }
    ...
    function getFulfillmentFee(
        ...
        (uint96 price, , ) = calculateRequestPriceCallbackJuels(
            callbackGasLimit,
            arguments
        );
    ...
    /// @notice emitted when migrate() is called when given coordinator is not registered on the router
    ...
    function migrate(
        ...
        if (!i_router.isCoordinatorRegistered(address(newCoordinator))) {
        ...
        if (i_router.getRoute(request.subID) != address(this)) {
            revert InvalidSubscription(request.subID);
        }
     ...
     * @inheritdoc IVRFMigration
     */
    function onMigration(bytes calldata /* encodedData */) external pure {
    ...
    Pausable
{
    ...
    AggregatorV3Interface public immutable i_link_eth_feed; // Address of LINK/ETH feed
    VRFRouter public immutable i_router; // Address of VRF router
    ...
    GasPriceData internal s_gasPriceData;
    ...
    constructor(
        ...
        i_link_eth_feed = AggregatorV3Interface(linkEthFeed);
        i_router = VRFRouter(router);
    }
    ...
    function getFeedData() internal view returns (uint256) {
        int256 weiPerUnitLink = s_billingConfig.fallbackWeiPerUnitLink;
        if (weiPerUnitLink == 0) {
            weiPerUnitLink = 50000000000000000;
        }
        ...
        )
    {
        ...
        (
            ...
            uint256 weiPerUnitLink,
            uint256 gasPrice
        ) = calculateRequestPriceCallbackJuels(
        ...
        sub.pendingFulfillments++;
        uint96 newSubBalance = sub.balance - uint96(costJuels);
    ...
    function calculateRequestPriceJuels() public view returns (uint96) {
        uint256 baseCost = s_billingConfig.redeemableRequestGasOverhead *
            getReasonableGasPrice();
        (uint96 finalCost, ) = getFinalCostJuels(baseCost, 0);
        return finalCost;
    }
    ...
    function calculateRequestPriceCallbackJuels(
        uint96 gasAllowance,
        bytes memory arguments
    ) public view returns (uint96, uint256, uint256) {
        ...
        uint256 baseCost = (gasAllowance +
            s_billingConfig.callbackRequestGasOverhead) *
            reasonableGasPrice +
            arguments.length *
    ...
    function addConsumer(
        uint256 subId,
        address consumer
    ) external override onlySubOwner(subId) nonReentrant whenNotPaused {
        ...
        nonReentrant
    {
        ...
        // only delete route if coordinator is registered on the router
        // if coordinator is deprecated from the router, subscription should
        // be cancelled without restting any routes on the router.
        if (i_router.isCoordinatorRegistered(address(this))) {
            i_router.resetRoute(subId);
        }
    ...
    function getReasonableGasPrice() internal view returns (uint64) {
        if (s_gasPriceData.reasonableGasPrice != 0) {
            bool stalenessIsMoreThanTotalBlocks = block.number <
                s_billingConfig.reasonableGasPriceStalenessBlocks;
            if (
                stalenessIsMoreThanTotalBlocks || // prevents underflow
                (s_gasPriceData.blockNumber >=
                    block.number -
                        s_billingConfig.reasonableGasPriceStalenessBlocks)
            ) {
                return s_gasPriceData.reasonableGasPrice;
            }
        }
...
abstract contract IVRFRouterConsumer {
    VRFRouter immutable i_router;

    constructor(address _router) {
        i_router = VRFRouter(_router);
    }
    ...
    function rawFulfillRandomWords(
        ...
        if (address(i_router) != msg.sender) {
            revert MustBeRouter();
        }
...
contract VRFRouter is IVRFRouter, TypeAndVersionInterface, OwnerIsCreator {
    ...
    function requestRandomness(
        ...
        uint256 requestID = coordinator.requestRandomness{value: msg.value}(
            msg.sender,
            subID,
            numWords,
            confDelay,
    ...
    function requestRandomnessFulfillment(
        ...
        return
            coordinator.requestRandomnessFulfillment{value: msg.value}(
                msg.sender,
                subID,
                numWords,
                confDelay,
