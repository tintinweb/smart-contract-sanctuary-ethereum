...
} from "./interfaces/IRandomNumberGenerator.sol";

/**
 * @notice This contract uses ERC721PartnerSeaDrop,
 *         an ERC721A token contract that is compatible with SeaDrop.
 */
contract WilliamShatnerCosmicExplorer is ERC721PartnerSeaDrop {
    // A token receiver
    struct Receiver {
        address to;
        uint256 quantity;
    ...
    string private _termsURI;

    /// @notice The address of the presale contract.
    address private _presaleAdmin;

    /// @notice The VRF admin for randomization.
    ...
    /// @notice An event emitted when the presale admin is updated.
    event PresaleAdminUpdated(address newAddress);

    /// @notice An event emitted when the VRF coordinator is updated.
    event VRFCoordinatorUpdated(address contractAddress);

    /// @notice An event emitted when the VRF administrator is updated.
    event VRFAdministratorUpdated(address newWallet);

    /// @notice Revert with error if address not owner or presale administrator.
    error OnlyOwnerOrPresaleAdministrator();

    modifier onlyOwnerOrPresaleAdministrator() virtual {
        if (msg.sender != owner()) {
            if (msg.sender != _presaleAdmin) {
                revert OnlyOwnerOrPresaleAdministrator();
            }
        }
        _;
    }
    ...
    /**
     * @notice Sets the Presale contract which will call airdrop functions
     *         from the presale.
     *
     * @param newAdmin The new terms URI.
     */
    function setPresaleAdmin(address newAdmin) external onlyOwner {
        // Set the new admin.
        _presaleAdmin = newAdmin;

        // Emit an event with the update.
        emit PresaleAdminUpdated(newAdmin);
    }
    ...
    /**
     * @notice Set the VRF Coordinator contract address.
     *
    ...
    function setVRFCoordinatorAddress(
        ...
        _randomNumberGenerator = IRandomNumberGenerator(contractAddress);

        emit VRFCoordinatorUpdated(contractAddress);
    }

    /**
     * @notice Returns the Presale Admin Address
     *
     * @return address The admin address
     */
    function presaleAdmin() external view returns (address) {
        return _presaleAdmin;
    }
    ...
    ) external onlyOwnerOrPresaleAdministrator {
        uint256 quantity = _totalQuantity(receivers);
        uint256 quantityAfter = totalSupply() + quantity;

        if (quantityAfter > maxSupply()) {
            revert MintQuantityExceedsMaxSupply(quantityAfter, maxSupply());
        }

        for (uint256 i = 0; i < receivers.length; ) {
            // Mint the quantity of tokens to the minter.
            _safeMint(receivers[i].to, receivers[i].quantity);
    ...
    /**
     * @notice Request random words from Chainlink VRF consumer.
     *
     * @param totalEntries The total number of entries.
     * @param totalSelections The total number of selections.
     * @param title The title of random request.
     */
...
interface IRandomNumberGenerator {
     ...
     * @notice Request random words from Chainlink VRF consumer.
     *
     * @param partnerContract The partner contract address.
     * @param totalEntries The total number of entries.
     * @param totalSelections The total number of selections.
     * @param title The title of random request.
     */
 ...
 *         an ERC721A token contract that is compatible with SeaDrop.
 */
contract TheRemarkables is ERC721PartnerSeaDrop {
    // A token receiver
    struct Receiver {
        address to;
        uint256 quantity;
...
interface ERC721Airdroppable {
    /// @notice A token receiver
    struct Receiver {
        address to;
        uint256 quantity;
    }

    function airDrop(Receiver[] memory receivers) external;
}

/**
 * @notice A contract for accepting crypto in exchange for a future airdrop.
 */
contract CometPrePay is Ownable, ReentrancyGuard {
    /// @notice Reservation struct
    struct Reservation {
        address to;
        uint256 quantity;
    }

    /// @notice Sale Config struct
    struct SaleConfig {
        uint256 startTime;
        uint256 endTime;
        uint256 price;
    ...
    /// @notice The address of the ERC721 token to sell.
    address private _tokenAddress;

    /// @notice The address of the beneficiary to get paid.
    address private _beneficiary;

    /// @notice The owner address of the tokens that will be sold.
    uint256 private _maxSupply;

    /// @notice The owner address of the tokens that will be sold.
    uint256 private _totalSupply;

    /// @notice The pointer to the current index of reservations.
    uint256 private _redeemPointer = 0;

    /// @notice Reservations array.
    Reservation[] private _reservations;

    /// @notice The sale configuration.
    SaleConfig private _saleConfig;

    /// @notice The interface to the ERC721 token.
    ERC721Airdroppable private _token;

    event BeneficiaryUpdated(address newBeneficiary);
    event MaxSupplyUpdated(uint256 newMaxSupply);
    event PrePayCompleted(uint256 quantity, address sender);
    event PriceUpdated(uint256 newPrice);
    event Redeem(uint256 reservations, uint256 redeemPointer, uint256 tokens);
    event SaleConfigUpdated(SaleConfig newSaleConfig);
    event SaleIsOpenUpdated(bool isOpen);
    event TokenAddressUpdated(address newTokenAddress);

    error BeneficiaryCannotBeNullAddress();
    error IncorrectPayment(uint256 received, uint256 needs);
    error MaxSupplyExceeded();
    error PreSaleNotOpen();
    error TokenCannotBeNullAddress();
     ...
     */
    constructor(uint256 initialMaxSupply, uint256 price) {
        _maxSupply = initialMaxSupply;
        _saleConfig.price = price;
    ...
    function _checkPayment(uint256 quantity) private view {
        ...
        if (msg.value != quantity * _saleConfig.price) {
            revert IncorrectPayment(msg.value, quantity * _saleConfig.price);
        }
    }

    /**
     * @notice Revert if the max supply is exceeded.
     *
     * @param quantity  The quantity of tokens to mint.
     */
    function _checkMaxSupply(uint256 quantity) private view {
        // Revert if max supply is exceeded
        if (_totalSupply + quantity > _maxSupply) {
     ...
     */
    function setBeneficiary(address wallet) external onlyOwner {
        // Revert if the new address is the null address.
        if (wallet == address(0)) {
            revert BeneficiaryCannotBeNullAddress();
        }

        _beneficiary = wallet;

        // Emit updated event
        emit BeneficiaryUpdated(wallet);
    }

    /**
     * @notice Set the max supply of the prePay sale.
     *
    ...
    /**
     * @notice Settting the status to closed forces closure
     *
     * @param value The is-open status.
     */
    function setIsOpen(bool value) external onlyOwner {
        _saleConfig.isOpen = value;

        emit SaleIsOpenUpdated(value);
    }

    /**
     * @notice Set the price of the prePay sale.
     *
    ...
    /**
     * @notice Set the sale config
     *
     * @param config  The sale config.
     */
    function setSaleConfig(SaleConfig memory config) external onlyOwner {
        _saleConfig = config;

        emit SaleConfigUpdated(config);
    }

    /**
     * @notice Set the ERC721 token address that reseverations will redeem.
     *
     * @param newTokenAddress  The ERC721 token address.
     */
    function setTokenAddress(address newTokenAddress) external onlyOwner {
        _tokenAddress = newTokenAddress;
        _token = ERC721Airdroppable(newTokenAddress);

        emit TokenAddressUpdated(newTokenAddress);
    }

    /**
     * @notice Redeem reservations by dropping tokens.
     *
     * @param quantity The number of reservations to redeem.
     */
    function redeem(uint256 quantity) external onlyOwner {
        // Revert if the new address is the null address.
        if (_tokenAddress == address(0)) {
            revert TokenCannotBeNullAddress();
        }

        uint256 tokens = 0;
        uint256 remaining = _totalSupply - _redeemPointer;

        if (remaining < quantity) {
            quantity = remaining;
        }

        ERC721Airdroppable.Receiver[]
            memory receivers = new ERC721Airdroppable.Receiver[](quantity);

        for (uint256 i = 0; i < quantity; i++) {
            Reservation memory reservation = _reservations[_redeemPointer + i];
            receivers[i] = ERC721Airdroppable.Receiver(
                reservation.to,
                reservation.quantity
            );

            tokens += reservation.quantity;
        }

        _token.airDrop(receivers);

        _redeemPointer += quantity;

        emit Redeem(quantity, _redeemPointer, tokens);
    }
    ...
    function dispose() external onlyOwner {
        address payable addr = payable(address(_beneficiary));
        selfdestruct(addr);
    }

    /**
     * @notice Get's the max supply
     *
    ...
    /**
     * @notice Get the position of the redeem pointer
     *
     * @return uint256 the redeem pointer
     */
    function redeemPointer() external view returns (uint256) {
        return _redeemPointer;
    }

    /**
     * @notice Get's the current sale config.
     *
     * @return SaleConfig the config
     */
    function saleConfig() external view returns (SaleConfig memory) {
        return _saleConfig;
    }

    /**
     * @notice Get's the total supply
     *
    ...
    /**
     * @notice Pre Pay! Get a reservation for your NFT
     *
     * @param quantity the amount to buy (reserve)
     */
    function prePay(uint256 quantity) external payable nonReentrant {
        // Is the sale open??
        if (!isOpen()) {
        ...
        _reservations.push(Reservation(msg.sender, quantity));
        ...
        emit PrePayCompleted(quantity, msg.sender);
    }
    ...
    function isOpen() public view returns (bool) {
        // if the sale config isOpen is set to false then that takes precedence
        if (!_saleConfig.isOpen) {
            return false;
        }

        // otherwise check if the sale is open using the block timestamp
        if (
            block.timestamp >= _saleConfig.startTime &&
            block.timestamp < _saleConfig.endTime
        ) {
            return true;
        }
...
contract CometSale is Ownable, ReentrancyGuard {
    /// @notice The address of the ERC721 token to sell.
    address private _tokenAddress;
    ...
    /// @notice The current pointer tokenId of where to start selling tokens.
    uint256 private _currentTokenId = 1;

    /// @notice The price of each token.
    uint256 private _price = 0.1 ether;

    /// @notice The ERC721 contract being sold.
    ERC721 private _token;

    event CurrentTokenIdReset(uint256 newTokenId);
    event PriceUpdated(uint256 newPrice);
    event TokenAddressUpdated(address newTokenAddress);
    event TokenPurchased(uint256 tokenId, address buyer);
    event TokensPurchased(uint256 quanity, address buyer);
    ...
    function _checkPayment(uint256 quantity) private view {
        ...
        if (msg.value != quantity * _price) {
            revert IncorrectPayment(msg.value, quantity * _price);
        }
    }

    function getNextTokenId() public view returns (uint256) {
        for (uint256 i = 0; i < 100; i++) {
            if (_token.ownerOf(_currentTokenId + i) == _ownerAddress) {
                return _currentTokenId + i;
            }
        }
    ...
    function setCurrentTokenId(uint256 tokenId) external onlyOwner {
        ...
        emit CurrentTokenIdReset(tokenId);
    }
    ...
    function setToken(address newAddress) external onlyOwner {
        ...
        _token = ERC721(newAddress);

        emit TokenAddressUpdated(newAddress);
    }

    function purchaseToken() external payable nonReentrant {
        // Get the next available token ID
        uint256 tokenId = this.getNextTokenId();

        // TODO: checkPayment
        _checkPayment(1);

        _token.safeTransferFrom(_ownerAddress, msg.sender, tokenId);

        emit TokenPurchased(tokenId, msg.sender);

        // Increment the token after a safe transfer
        _currentTokenId = this.getNextTokenId();
    }

    function purchaseMany(uint256 quantity) external payable nonReentrant {
        // TODO: checkPayment
        _checkPayment(quantity);
        ...
        emit TokensPurchased(quantity, msg.sender);
    }
    ...
    function getBalance() public view returns (uint256) {
        return _token.balanceOf(_ownerAddress);
    }
}
...
abstract contract ContractFilter {
    // using Address for address;
    // using Strings for uint256;

    address[] private _contracts;

    constructor(address[] memory initialContracts) {
        _contracts = initialContracts;
    }
