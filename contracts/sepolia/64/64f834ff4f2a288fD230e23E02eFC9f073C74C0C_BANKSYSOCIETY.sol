...
contract BANKSYSOCIETY is ERC721AQueryable, ERC2981, Ownable, Pausable {
    ...
    function presaleMint(uint256 quantity) public payable {
        ...
        if (!whitelist[msg.sender] && !collectionWhitelist[msg.sender]) revert NotAuthorised();
     ...
     * @dev allows an admin to add an address to the whitelist
     * @param addressToAdd the address to add to the whitelist
     */
    function addToWhitelist(address addressToAdd) public onlyAdmin {
        whitelist[addressToAdd] = true;
    }

    /**
     * Remove address from whitelist
     * @dev allows an admin to remove an address from the whitelist
    ...
    /**
     * Add collection address to whitelist
     * @dev allows an admin to add a collection address to the collection whitelist
     * @param collectionAddressToAdd the collection address to add to the collection whitelist
     */
    function addToCollectionWhitelist(address collectionAddressToAdd) public onlyAdmin {
        collectionWhitelist[collectionAddressToAdd] = true;
    }

    /**
     * Remove collection address from whitelist
     * @dev allows an admin to remove a collection address from the collection whitelist
     * @param collectionAddressToRemove the collection address to remove from the collection whitelist
     */
    function removeFromCollectionWhitelist(address collectionAddressToRemove) public onlyAdmin {
        collectionWhitelist[collectionAddressToRemove] = false;
    }
    ...
    function isWhitelisted(address addressToCheck) public view returns (bool) {
        return whitelist[addressToCheck] || collectionWhitelist[addressToCheck];
    }

    /**
     * Check if an address is whitelisted for collection
     * @param collectionAddress the collection address to check
     * @return true if the collection address is whitelisted, false otherwise
     */
    function isCollectionWhitelisted(address collectionAddress) public view returns (bool) {
        return collectionWhitelist[collectionAddress];
    }

    /**
     * Include token holders in the whitelist
     * @dev allows anyone who holds a token in their wallet to be automatically included in the whitelist
     */
    function includeTokenHoldersInWhitelist() public {
