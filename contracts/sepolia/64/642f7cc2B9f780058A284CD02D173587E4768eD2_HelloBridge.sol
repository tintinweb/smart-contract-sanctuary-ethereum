...
contract HelloBridge is Ownable {
    ...
    //User -> depositNonce -> Amount
    address public withdrawSigner;
    //User -> Chain Id -> Amount
    ...
    function sendToChain(uint256 amount, uint256 chainId) external {
        ...
        totalCrossChainDeposits[msg.sender][chainId] += amount;
    }

    function claimFromChain(uint256 totalDepositedOnOtherChain, uint256 otherChainId, bytes memory signature)
        external
    {
        //No need to check if chain is supported as the user would not have been able to deposit if it was not supported

        //This can never undeflow as the user would not have been able to deposit if they had no balance
        uint256 amountToWithdraw = totalDepositedOnOtherChain - totalCrossChainWithdrawals[msg.sender][otherChainId];
        ...
        if (hash.toEthSignedMessageHash().recover(signature) != withdrawSigner) {
            _revert(SignerNotWithdrawSigner.selector);
        }

        totalCrossChainWithdrawals[msg.sender][otherChainId] = totalDepositedOnOtherChain;
    ...
    function setSupportedChains(uint256[] memory chainIds, bool isSupported) external onlyOwner {
