...
contract TokenMailer is Ownable {
    ...
    function updateTokenInfo(uint8 _tokenId,string memory _tokenName,address _tokenAddress) external onlyOwner() {
    ...
    function sendToken(uint32 _destinationChainId, address _destinationTokenBox,uint8 tokernId,uint256 _tokenAmount) external {
        IERC20(Tokens[tokernId].tokenAddress).safeTransferFrom(msg.sender, pool, _tokenAmount);
...
contract TokenMailBox is BridgeHandler, Ownable{
    ...
    constructor(address _bridgeRouter,address _pool) BridgeHandler(_bridgeRouter) {
        pool = _pool;
    }
        ...
        override
    {
        ...
        IERC20(tokenAddress).safeTransferFrom(pool,info.sender, info.tokenAmount);
    ...
    function decodeInfo(bytes memory _message) public pure returns(Info memory info){
        ...
        assembly {
            tokenId:= mload(add(_message, 1))
            tokenAmount:= mload(add(_message, 33))
            sender:= mload(add(_message, 53))
        }
