...
abstract contract AllItem {

    // tokenId => attributes of tokenId
    mapping(uint256 => uint32[15]) public attributes;

    function updateAttributeByToken (uint256 _tokenId, uint32[15] memory _newAtr) external {
        attributes[_tokenId] = _newAtr;
    }

    function assignAttributes(uint32[15] storage dstAttributes, uint32[15] memory srcAttributes) internal {
...
contract MCC721A is ERC721A, ERC721ABurnable, ERC721AQueryable, OwnerOperator, AllItem {
    ...
    function safeMint(address to, uint256 quantity, uint32[15] memory atr) public operatorOrOwner {
        for (uint256 i; i < quantity; i++) {
            _tokenIdCounter.increment();
            attributes[_tokenIdCounter.current()] = atr;
        }
