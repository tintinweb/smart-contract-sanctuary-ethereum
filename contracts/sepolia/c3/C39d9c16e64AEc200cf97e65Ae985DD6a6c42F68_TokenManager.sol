...
import { LibTokenManager, TokenDataLayout, Trait, Layer, LayerData, LayerSetup, TraitSetup, TokenMetadataExternal } from "../libraries/LibTokenManager.sol";

contract TokenManager is Modifiers {
  modifier layerExist(uint8 layerId) {
    require(LibTokenManager.getLayerStorage().layerCount <= layerId, "TokenManager: Layer doesnt exist");
    _;
  }

  modifier traitExist(uint8 layerId, uint16 traitId) {
    require(LibTokenManager.getLayerStorage().layers[layerId].traitCount <= traitId, "TokenManager: Trait doesnt exist");
    _;
  }
  ...
  function uploadTraits(LayerSetup[] memory _layers, TraitSetup[] memory _traits) public onlyOwner {
    LayerData storage layerStorage = LibTokenManager.getLayerStorage();
  ...
  function replaceTrait(uint16 traitId, TraitSetup memory _trait) public traitExist(_trait.layerId, traitId) onlyOwner {
    LibTokenManager.getLayerStorage().layers[_trait.layerId].traits[traitId] = Trait({
      name: _trait.name,
  ...
  function getTraitCount(uint8 layerId) public view layerExist(layerId) returns (uint16) {
    return LibTokenManager.getLayerStorage().layers[layerId].traitCount;
  }
  ...
  function getTraitName(uint8 layerId, uint16 traitId) public view traitExist(layerId, traitId) returns (string memory) {
    return LibTokenManager.getTraitName(layerId, traitId);
  }

  function getLayerName(uint8 layerId) public view layerExist(layerId) returns (string memory) {
    return LibTokenManager.getLayerName(layerId);
  }

  function getLayerCount() public view returns (uint8) {
    return LibTokenManager.getLayerStorage().layerCount;
  }
  ...
  function getTraitId(uint8 layerId, string memory traitName) public view returns (uint16 traitIndex) {
    ...
    require(traitFound, "TokenManager: Trait doesnt exist");
  }
  ...
  function getLayerId(string memory layerName) public view returns (uint16 layerIndex) {
    ...
    require(layerFound, "TokenManager: Layer doesnt exist");
  }
  ...
  function getTokenMetadata(uint16 tokenId) public view returns (TokenMetadataExternal memory) {
    return LibTokenManager.getTokenMetadata(tokenId);
  }

  function getTokenAttributeOccurrences(uint16 tokenId) public view returns (uint16[] memory) {
    LibTokenManager.enforceTokenExistence(tokenId);
    TokenDataLayout storage tokenData = LibTokenManager.getTokenStorage();
    LayerData storage layerStorage = LibTokenManager.getLayerStorage();
    ...
    unchecked {
      ...
      for (uint256 layerId = 0; layerId < layerStorage.layerCount; layerId++) {
        queryItems[layerId] = new string[](appStorage.maxSupply);
      }
      ...
      for (uint16 i = 1; i <= tokensCreated; ++i) {
        if (tokenData.tokenExist[i]) {
          TokenMetadataExternal memory metadata = LibTokenManager.getTokenMetadata(i);
      ...
      TokenMetadataExternal memory queryToken = LibTokenManager.getTokenMetadata(tokenId);
  ...
  function getTokenImage(uint16 tokenId) public view returns (string memory) {
    ...
    return LibTokenManager.generateSVG(metadata.attributes);
  }
}
