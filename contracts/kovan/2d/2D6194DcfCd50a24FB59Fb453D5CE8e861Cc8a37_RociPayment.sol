    ...
    ERC20PaymentStandard
{
    ...
    returns (uint256 principal, address borrower)
  {
    ...
    LoanValueLimitsById[_id] = 
      LoanValueLimits({
          ltv: ltv,
          lt: oracle.LT(ln.ERC20Address, score.creditScore)
        });
  ...
  * Sometimes it shouldn't revert, instead we just have it return 0
    ...
    override
  {
    ...
    if (_ln.status == Loan.Status.NEW) {
      ...
      // if loan was closed or liquidated and not found in the array that's an error
      revert(
        "Error: Loan is marked Closed or Liquidated, yet is not in the user's active loan list"
      );
    }
  }
}
