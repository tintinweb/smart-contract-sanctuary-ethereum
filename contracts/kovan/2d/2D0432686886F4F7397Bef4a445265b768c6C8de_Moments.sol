...
contract Moments is ERC721, AccessControl, ContextMixin, NativeMetaTransaction, ERC721Holder, ERC721URIStorage {
...
constructor() ERC721("moments", "MOMs") {
...
//caps max token Id - this may need adjustments 
...
function mint(address to, uint256[] calldata tokenIds, uint32 momentsToMint, string[] memory uri)
    //string memory uri)
        public
        onlyRole(DEFAULT_ADMIN_ROLE)
    {

 for (uint i = 0; i < momentsToMint; i++) {
            _mint(address(this), tokenIds[i]);
            _setTokenURI(tokenIds[i], uri[i]);
    }
}



///ends mint functionality
