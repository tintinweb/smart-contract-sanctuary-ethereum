...
{   
     ...
     */
    function readNFTOwnedTokenIDs(address Wallet, address NFTAddress, uint[] calldata TokenIDs) public view returns (uint[] memory)
    {
        uint[] memory ids = new uint[](TokenIDs.length);
        uint counter;
        for (uint i = 0; i < TokenIDs.length; i++) 
        {
            if(IERC721(NFTAddress).ownerOf(TokenIDs[i]) == Wallet)
            {
                  ids[counter] = TokenIDs[i];
                  counter++;
            }
        }
        return ids;
    ...
    /**
     * @dev Batch Returns Multiple TokenID NFTs
     */
    function readNFTsOwnedTokenIDs(
        ...
        uint[][] memory OwnedIDs = new uint[][](TokenIDs.length);
        for(uint x; x < NFTAddresses.length; x++)
        {
            uint counter;
            for(uint y; y < TokenIDs.length; y++) 
            {
                if(IERC721(NFTAddresses[x]).ownerOf(TokenIDs[y]) == Wallet)
                {
                    OwnedIDs[x][counter] = TokenIDs[y];
                    counter++;
                }   
            }
        }
