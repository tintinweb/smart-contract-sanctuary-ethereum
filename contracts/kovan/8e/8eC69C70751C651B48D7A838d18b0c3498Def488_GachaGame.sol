...
contract GachaGame {
    ...
    // event
    event DepositToDev(address indexed userAddress,uint256 amount);
    event ShowUserPull(address indexed userAddress,string rare);
    event UserRegister(address indexed userAddress,string name);
    ...
    function register(string memory newName)public{
        ...
        // new event 
        emit UserRegister(msg.sender,newName);
    ...
    function onePullGacha()public payable{ 
        ...
        emit ShowUserPull(msg.sender,rollInfo.rare);
        emit DepositToDev(msg.sender,msg.value);
