...
contract PoolComptroller is Comptroller {
    ...
    function initialize(PriceOracle _oracle, uint _closeFactorMantissa, uint _liquidationIncentiveMantissa, address _deployer, address _supervisor) public initializer{
    ...
    function hasOwnerRights() public override view returns(bool) {
        return msg.sender == owner() || msg.sender == supervisor;
    }
    ...
    function deployMarket(address underlying, address interestRateModel, uint reserveFactorMantissa, uint collateralFactorMantissa) public onlyOwner{
        ...
        bytes memory payload = abi.encodeWithSignature("createMarket(address,address,address,uint256)", address(this), underlying, interestRateModel, reserveFactorMantissa);
        bytes memory result = deployer.functionCall(payload);
        address marketToken = abi.decode(result,(address));
    ...
    function deployDistributionerManager() public onlyOwner {
        ...
        bytes memory payload = abi.encodeWithSignature("createDistributionerManager(address,address)", msg.sender, address(this));
        bytes memory result = deployer.functionCall(payload);
