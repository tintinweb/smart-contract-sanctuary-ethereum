...
contract BookBondingCalculator is IBondingCalculator {
        ...
        returns (uint256 _value)
    {
        ...
        _value = totalValue
            .mul(
                FixedPoint.decode112with18(
                    FixedPoint.fraction(amount_, totalSupply)
                )
            )
            .div(1e18);
    }
    ...
    function markdown(address _pair) external view returns (uint256) {
        ...
        uint256 reserve;
        if (IUniswapV2Pair(_pair).token0() == BOOK) {
            reserve = reserve1;
        } else {
            reserve = reserve0;
        }
        return
            reserve.mul(2 * (10**IERC20(BOOK).decimals())).div(
                getTotalValue(_pair)
            );
    }
}