...
// File: contracts\interfaces\IERC721EnumerableExt.sol
...
// File: contracts\libs\AdvancedString.sol
...
// File: contracts\interfaces\IERC721URIExtensible.sol
...
abstract contract IERC721URIExtensible is ERC721URIStorage {
    ...
    string constant _extension = ".json";
    string private uri;
        ...
        returns (string memory)
    {
        ...
        if (split[0].length() == _uri.length()) {
            return string(abi.encodePacked(_uri, _extension));
        }
        return string(abi.encodePacked(split[0], _extension, "?", split[1]));
    }
...
contract Module is
    ...
    Ownable
{
    constructor(address owner) ERC721("Module", "MDL") {
        require(owner != address(0), "Empty address");
        super.transferOwnership(msg.sender);
        super._addAdmin(owner);
    }

    using SafeMath for uint256;
    using Counters for Counters.Counter;
    Counters.Counter private moduleIds;
     ...
     */
    function burn(uint256 tokenId) public override isAdmin {
        super._burn(tokenId);
    }
    ...
    /**
     * @dev ERC721 function used for inheritance
     */
    ...
    {
        moduleIds.increment();
        super._mint(to, moduleIds.current());
        return moduleIds.current();
    }
}