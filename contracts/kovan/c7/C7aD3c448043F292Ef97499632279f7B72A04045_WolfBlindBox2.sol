...
contract WolfBlindBox2 is AdminTransfer,ReentrancyGuard,EIP712,Multicall {
    ...
    event BoxOpen(uint indexed requestId,uint indexed oid,address to,uint level);
    ...
    function open(uint256 deadline,uint256 oid,uint level,address to,bytes memory signature) public nonReentrant {
        ...
        emit BoxOpen(requestId,oid,to,level);
    }
    ...
    function _open1(address to,uint randomX128,uint,uint) internal {
        uint i;
    ...
    function _open2(address to,uint randomX128,uint wolfKindRandomX128,uint wolfRandomX128) internal {
        uint i;
    ...
    function _open(address to,uint level,uint randomX128,uint wolfKindRandomX128,uint wolfRandomX128) internal {
        if(level==1) {
            _open1(to,randomX128,wolfKindRandomX128,wolfRandomX128);
        } else if(level==2) {
            _open2(to,randomX128,wolfKindRandomX128,wolfRandomX128);
        }
    }
    ...
    function fulfillRandomWords(
        ...
        if(to!=address(0)) {
            _open(to,level,(randomWords[0]&((uint(1)<<128)-1)),(randomWords[1]&((uint(1)<<128)-1)),(randomWords[2]&((uint(1)<<128)-1)));
