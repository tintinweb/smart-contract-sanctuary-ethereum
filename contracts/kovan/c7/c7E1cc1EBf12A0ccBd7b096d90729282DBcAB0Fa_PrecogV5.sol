...
contract PrecogV5 is IPrecogV5 { 
    ...
    mapping(address => mapping(address => Investment)) public override investmentOf; ///// investmentOf[token][account] = Investment()
    ...
    function updateProfitCycleToCurrentCycle(address token) public override {
        ...
        while (uint48(block.timestamp) >= lastCycle.endTime) {
            newCycleStartTime = lastCycle.endTime;
            if(newCycleStartTime < lastChangeProfitCycle && (newCycleStartTime + duration) >= lastChangeProfitCycle) {
                duration = lastCycle.endTime - lastCycle.startTime;    
            } else {
                duration = profitCycle;
            }
    ...
    function _increaseInvestment(address token, address account, uint256 amount) internal {
        ...
        Investment memory investment = investmentOf[token][account];
        Cycle[] memory _profitCycles = profitCycles[token];
        uint256 unit = (_profitCycles[_profitCycles.length - 1].endTime - uint48(block.timestamp)) * amount;
        uint256 updatedUnit;
        ...
        if(!isNotFirstIncreaseInvestment[token][account]) {
            investment.lastProfitCycleId = uint16(_profitCycles.length - 1);
        ...
        if (investment.lastProfitCycleId < updatedLatestCycle) {
            ...
            // Looping though stable cycles to calculate profit from last to current profit cycle id
            {
            ...
            investmentOf[token][account] = Investment(investment.amount + amount, updatedUnit, updatedLatestCycle);
        } 
        else {
            updatedUnit = investment.unit + unit;                
            investmentOf[token][account] = Investment(investment.amount + amount, updatedUnit, investment.lastProfitCycleId); 
        }
