...
contract Cube_Finance is Context, IERC20, Ownable {
  ...
  string private _name = "Cube Finance";
  ...
  function sendETHToTeam(uint256 amount) private {
    _TreasuryWalletAddress.call{ value: amount.div(2) }('');
  ...
  function _setTaxFee(uint256 taxFee) external onlyOwner {
    require(taxFee <= 30, 'taxFee should be in 0 - 30');
    _taxFee = taxFee;
  }
  function _setTeamFee(uint256 teamFee) external onlyOwner {
    require(teamFee <= 30, 'teamFee should be in 0 - 30');
    _teamFee = teamFee;
  }
  function _setSellTaxMultiplier(uint8 mult) external onlyOwner {
    require(mult >= 1 && mult <= 5, 'multiplier should be in 1 - 5');
