...
contract Uchu {
    ...
    **/
    function deposit(address user) public payable returns (bool) {
        if (productId > 0) {
            balanceOf[user][curLenderPro.name] += msg.value;
            allowance[curLenderPro.name] += msg.value;
            maxBalance[user][getCurLender(user).proIndex] = MaxProduct(msg.value, curLenderPro, block.timestamp, 0);
            for(uint128 i=0; i<lenderId; i++) {
            ...
            emit Log(user, "deposit", curLenderPro.name, msg.value);
            emit Log(user, "deposit", "maxBalance", getCurLender(user).proIndex);
            return true;
        } else {
            return false;
        }
