...
// File: my/wolf/game/WolfAttackCow.sol
...
contract WolfAttackCow is WolfGameStorage {
    event AttackApply(uint indexed requestId,uint wolfId,uint cowId);
    event Attack(uint indexed requestId,uint wolfId,uint cowId,bool success,uint stealValue,uint cowDownLevel,uint blood);

    function attack(uint wolfId,uint cowId) public nonReentrant {
        require(wolf.ownerOf(wolfId)==msg.sender,"need owner of wolf");
        require(ICowGame(cowGame).cowLevel(cowId)>15,"cow level must more than 15");
        vitalityInfo(wolfId);
        require(vitalityX64[wolfId]>=MathX64.oneX64,"Game: wolf VIT not enough");
        vitalityX64[wolfId]-=MathX64.oneX64;
        uint requestId=linkRequest();
        attackCowId[requestId]=cowId;
        attackWolfId[requestId]=wolfId;
        emit AttackApply(requestId,wolfId,cowId);
    }

    function linkRequest() internal returns(uint s_requestId) {
        address link=paramAddress["link"];
        require(link!=address(0),"no link");
        s_requestId = VRFCoordinatorV2Interface(link).requestRandomWords(
    ...
    function _attack(AttackParam memory attackParam) internal {
        IWolfAttackCowParam param=IWolfAttackCowParam(paramAddress["wolfAttackCowParam"]);
        attackParam.wolfGrade=wolf.uintProperty(attackParam.wolfId,"grade");
        ICowGame game=ICowGame(cowGame);
        (uint incomeValue,,uint life)=game.cowInfo(attackParam.cowId);
        attackParam.cowLevel=ICowGame(cowGame).cowLevel(attackParam.cowId);
        INewCowParam cowParam=INewCowParam(game.param());
        attackParam.cowLevelLife=cowParam.lifecycle(attackParam.cowLevel);
        if(!param.attackSuccess(attackParam.randomX128,attackParam.cowLevel,_cowGrade(attackParam.cowId),attackParam.wolfGrade,life,attackParam.cowLevelLife)){
            emit Attack(attackParam.requestId,attackParam.wolfId,attackParam.cowId,false,0,0,0);
            return;
        }
        uint stealValue=param.calcSteal(incomeValue,game.stolen(attackParam.cowId));
        game.steal(attackParam.cowId,stealValue,wolf.ownerOf(attackParam.wolfId));
        uint cowDownLevel=0;
        if(param.levelDown(attackParam.random2X128)){
            game.levelDown(attackParam.cowId,1);
            cowDownLevel=1;
        }
        uint blood=param.blood(attackParam.random3X128,attackParam.cowLevelLife);
        if(blood!=0) {
            if(!game.blood(attackParam.cowId,attackParam.wolfId,blood)){
                blood=0;
            }
        }
        emit Attack(attackParam.requestId,attackParam.wolfId,attackParam.cowId,true,stealValue,cowDownLevel,blood);
    }
    ...
    function fulfillRandomWords(
        ...
        uint cowId=attackCowId[requestId];
        uint wolfId=attackWolfId[requestId];
        if(cowId!=0 && wolfId!=0) {
            AttackParam memory attackParam= AttackParam({
            requestId:requestId,wolfId:wolfId,cowId:cowId,
            randomX128:(randomWords[0]&((uint(1)<<128)-1)),
            random2X128:(randomWords[1]&((uint(1)<<128)-1)),
            random3X128:(randomWords[2]&((uint(1)<<128)-1)),
            wolfGrade:0,
            cowLevel:0,
            cowLevelLife:0
            });
            _attack(attackParam);
            attackCowId[requestId]=0;
            attackWolfId[requestId]=0;
        }
    }
