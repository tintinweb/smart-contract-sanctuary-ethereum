...
contract ComeFundMe is Ownable, Pausable {
    struct Campaign {
        bool isAlive; //if campaign is active or not
        address initiator; //whoever initiates campaign, has power to end it whenever.
        uint256 fundsRaised;
        string title;
        string description;
    }
    ...
    event CampaignStarted(bytes32 campaignId); //emmited when a campaign is started
    event CampaignEnded(bytes32 campaignId, uint256 fundsRaised);
    event CampaignDonationReceived(
        //keeps track of who gave how much
        bytes32 campaignId,
        address donor,
        uint256 amount
    );
    ...
    function togglePause() external onlyOwner {
        //allows owner to pause or unpause the contract
        paused() ? _unpause() : _pause(); //first, the value of paused is evaluated. if true evaluate first one, if false, do second one
    }
    ...
    function startCampaign(string calldata title, string calldata description)
        ...
        whenNotPaused
    {
        //the moment the contract is deployed, the storage of the map have been initialized to nothing
        //this is the same for a struct.
        bytes32 campaignId = getCampaignId(_msgSender(), title, description); //is the hash of the three inputs
        Campaign storage c = _campaigns[campaignId];
        //checking if the initilizer address's is 0 because that makes it so that it is the first campaign being made
        //making sure whatever we're trying to write into doesnt already exist
        require(c.initiator == address(0), "Campaign exists");
