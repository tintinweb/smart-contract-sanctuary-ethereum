...
contract TPCore is AccessControl {
    ...
    uint256 public ownerPercentage = 95; //(this means we hold 5%) make sure when changing it to keep this formatting
    ...
    function stake(uint256 amount_) public {
        ...
        liquidityAmount =  stakingcoinAmount * liquidityToken.totalSupply() / (totalReward);
    ...
    function unstake(uint256 amount_) public {
        ...
        require(liquidityAmount >= amount_, "Not enough liquidity tokens");
        liquidityToken.burn(_msgSender(), amount_);
        
        uint256 estimatedRewards = estimateReward(_msgSender());
        if (0 != estimatedRewards) {
            _takeReward(estimatedRewards); 
        }
        stakingcoin.safeTransfer(_msgSender(), estimatedRewards*ownerPercentage/100);

        uint256 estimatedTPYRewards = estimateTPYReward(_msgSender());
        if (0 != estimatedTPYRewards) {
            _takeTPYReward(estimatedTPYRewards); 
        }
       

        //todo do the same for TPY as well
...
//todo withdraw from this contract function for erc20s
    ...
    function estimateTPYReward(address lpProvider) public view returns (uint256) {
        ...
        uint256 totalReward = (block.number - startingBlock) * blockReward; 
        
        
        uint256 userReward = (totalReward * liquidityToken.balanceOf(lpProvider) / liquidityToken.totalSupply() ); //todo fix this proportion
        return userReward;
    }

   function _takeTPYReward(uint256 amount_) private {
        if (0 != amount_) {

        tpy.safeTransfer(_msgSender(), amount_);
    }
    }
