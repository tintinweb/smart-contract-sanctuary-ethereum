...
contract Faucet is Ownable, ReentrancyGuard {
  ...
  mapping(address => uint256) public nextFaucet;
  ...
  function faucet(address token, uint amount) external nonReentrant {
    ...
    require(nextFaucet[_msgSender()] <= block.timestamp, "must wait util the end of next faucet time");
    ...
    nextFaucet[_msgSender()] = block.timestamp + lockPeriod;
