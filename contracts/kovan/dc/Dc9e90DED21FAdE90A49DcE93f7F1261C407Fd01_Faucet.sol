...
contract Faucet is Ownable, ReentrancyGuard {
  ...
  function addToken (address _newToken, uint256 _faucetAmount) external onlySponsor {
    require(_faucetAmount > 0, "zero faucet amount");
    require(faucetAmount[_newToken] == 0, "token added already");
    _supportedTokens.push(IERC20(_newToken));
    faucetAmount[_newToken] == _faucetAmount;
  }
  ...
  function removeToken (IERC20 _token) external onlySponsor {
    ...
    for (uint256 i = 0; i < length; i++){
      if (tokens[i] == _token){
        _supportedTokens[i] = tokens[length-1];
  ...
  function faucet() external nonReentrant {
    ...
    for (uint256 i; i < tokens.length; i++){
      ...
      uint256 amount = faucetAmount[address(token)];
      if (token.balanceOf(_sponsorAddress) >= amount){
        token.safeTransferFrom(_sponsorAddress, _msgSender(), amount);
      }
    }
  }
