...
contract HyperDexRouter is GasStationRecipient, Ownable {
    ...
    ITokensApprover private _approver;
    ...
    constructor(IHyperDex hyperDex, address payable allowanceTarget, address payable beneficiary, address gasStation, ITokensApprover approver) {
        _hyperDex = hyperDex;
        _allowanceTarget = allowanceTarget;
        _beneficiary = beneficiary;
        _setGasStation(gasStation);
        _approver = approver;
    }
    ...
    function multiRouteWithPermit(
        ...
        return this.multiRoute(msgData, feeToken, inputToken, inputAmount, outputToken, fee);
    }
    ...
    function setGasStation(address newGasStation) external onlyOwner {
        _setGasStation(newGasStation);
    }

    function setApprover(ITokensApprover approver) external onlyOwner {
        _approver = approver;
    }
    ...
    function getApprover() public view returns (ITokensApprover) {
        return _approver;
    }

    function _permit(address token, bytes calldata approvalData) internal {
        if (approvalData.length > 0 && _approver.hasConfigured(token)) {
            (bool success,) = _approver.callPermit(token, approvalData);
    ...
    function _payFees(address token, uint256 amount) private {
        ...
        return _sendERC20(IERC20(token), _msgSender(), _beneficiary, amount);
    }
