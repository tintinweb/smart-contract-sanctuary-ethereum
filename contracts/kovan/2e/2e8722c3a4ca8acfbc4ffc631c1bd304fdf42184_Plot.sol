...
contract AlternativeSea is ERC20, ERC20Permit, ERC20Votes {
    address admin;
    address plots;
    address gov;

    constructor() ERC20("AlternativeSea", "CREATE") ERC20Permit("AlternativeSea") {
        admin = msg.sender;
    }
...
contract Plot is ERC721URIStorage, ERC721Enumerable, ERC721Royalty {
    ...
    constructor() ERC721("AlternativeSea Plot", "PLOT") {
        admin = msg.sender;
        owner = msg.sender;
    ...
    function editPlot(uint256 tokenId, string memory cid) public payable returns (uint256) {
        if (_exists(tokenId) && ownerOf(tokenId) == msg.sender) {
            ...
            require(msg.value == 0.1 ether); // Minting costs 0.1 Eth
    ...
    function setCreateTokenContract(address tokenContract) external {
        require(msg.sender == gov || msg.sender == admin);
        token = AlternativeSea(tokenContract);
    }
