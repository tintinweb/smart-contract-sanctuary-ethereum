...
contract DataCompressor is IDataCompressor {
    ...
    /// @dev Returns all credit managers data + hasOpendAccount flag for borrower
        ...
        returns (CreditManagerData memory result)
    {
        ...
        // } else {
        //     (result.minAmount, result.maxAmount) = creditManagerV2.limits();
        // }
        ...
        uint256 allowedContractsCount = (ver == 1)
            ? creditFilter.allowedContractsCount()
            : creditConfigurator.allowedContractsCount();
        ...
        for (uint256 i = 0; i < allowedContractsCount; ) {
            ...
            adapter.allowedContract = (ver == 1)
                ? creditFilter.allowedContracts(i)
                : creditConfigurator.allowedContracts(i);
            adapter.adapter = (ver == 1)
                ? creditFilter.contractToAdapter(adapter.allowedContract)
                : creditManagerV2.contractToAdapter(adapter.allowedContract);
    ...
    /// @dev Returns PoolData for particular pool
...
struct Slot0 {
    ...
    uint16 liquidationDiscount;
    // Liquidation fee protocol charges on expired accounts
    uint16 feeLiquidationExpired;
    // Multiplier for the amount the liquidator has to pay when closing an expired account
    uint16 liquidationDiscountExpired;
...
contract CreditManager is ICreditManagerV2, ACLTrait, ReentrancyGuard {
        ...
        nonReentrant
    {
        ...
        uint256 cumulativeDrop = WAD -
            ((amountOutCollateral * WAD) / amountInCollateral) +
        ...
        if (cumulativeDrop <= slot0.feeLiquidation * WAD / PERCENTAGE_FACTOR) {
        ...
        )
    {
        ...
        if (closureActionType == ClosureAction.LIQUIDATE_ACCOUNT) {
            ...
            uint256 totalFunds = (totalValue * slot0.liquidationDiscountExpired) /
                PERCENTAGE_FACTOR; 

            amountToPool +=
                (totalValue * slot0.feeLiquidationExpired) /
                PERCENTAGE_FACTOR; 
        ...
        )
    {
        ...
        feeLiquidationExpired = slot0.feeLiquidationExpired; // F:[CM-51]
        liquidationDiscountExpired = slot0.liquidationDiscountExpired; // F:[CM-51]
    }
        ...
        creditConfiguratorOnly // F:[CM-4]
    {
        ...
        slot0.feeLiquidationExpired = _feeLiquidationExpired; // F:[CM-51]
        slot0.liquidationDiscountExpired = _liquidationDiscountExpired; // F:[CM-51]
    }
...
contract CreditFacade is ICreditFacade, ReentrancyGuard {
        ...
        returns (uint256 hf)
    {
        ...
        hf = (twv * PERCENTAGE_FACTOR) / borrowAmountWithInterest; // F:[FA-42]
    }
