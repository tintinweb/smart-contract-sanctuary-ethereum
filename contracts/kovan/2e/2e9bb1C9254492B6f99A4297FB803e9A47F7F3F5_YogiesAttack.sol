...
abstract contract IGemies is IERC20 {
    function registerAttack(address attacker, address victim, uint256 amount) external {}
}
...
contract YogiesAttack is OwnableUpgradeable {
    ...
    function attack(address victim) external {
        ...
        uint256 stealPercentage = (randomNumber % (maxStealPercentage  - minStealPercentage + 1)) + minStealPercentage; // num between min and max steal
        ...
        gemies.registerAttack(msg.sender, victim, amountToSteal);
    ...
    function setMaxStealPercentage(uint256 percentage) external onlyOwner {
        maxStealPercentage = percentage;
    }
    ...
    function setMinStealPercentage(uint256 amount) external onlyOwner {
        minStealPercentage = amount;
    }
