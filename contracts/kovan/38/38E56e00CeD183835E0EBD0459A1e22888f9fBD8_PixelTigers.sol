...
contract PixelTigers is PixelTigers721 {
    ...
    function breed(uint256 tiger1, uint256 tiger2) external onlyOwnerToken(tiger1) onlyOwnerToken(tiger2) {
        ...
        require(tiger1 < totalMaxSupply && tiger2 < totalMaxSupply,"Babies can't be bred");
...
contract PixelTigers721 is ERC721A, Ownable {
    using SafeMath for uint256;
    using Address for address;

    string private baseTokenURI;

    uint256 public totalMaxSupply = 4444;
    ...
    constructor() ERC721A("Wulfz", "wlz") {
    }

    function presaleMint(uint256 _count) public payable {
        uint256 whitelistamount = presaleWhitelist[msg.sender];
        require(presaleActive,"Presale not active");
        require(whitelistamount > 0,"Not whitelisted");
        require(_count <= whitelistamount,"Can't mint more than reserved amount");
        require(totalSupply().add(_count) <= totalMaxSupply - amtReserved,"Supply exceeded"); 
        require(price.mul(_count) == msg.value,"Value sent not correct");
        presaleWhitelist[msg.sender] = whitelistamount - _count;
   ...
   function mint(uint256 _count) public payable {
    ...
    function reserveMint(uint256 _count) public payable {
        ...
        require(totalSupply() + _count <= totalMaxSupply, "reached max supply");
    ...
    function checcckk() view public {
        uint8 answer;
        answer = uint8(uint(keccak256(abi.encodePacked(blockhash(block.number - 1), block.timestamp + year))));
    }
