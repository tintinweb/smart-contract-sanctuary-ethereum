...
abstract contract TimedFee {
    ...
    function getTimeInterval() private view returns (uint256) {
        return MIN_TIMER + (_random() % (MAX_TIMER - MIN_TIMER));
    }

    function getFeeLapsedTime(address account)
        public
        view
        returns (uint256)
    {
    ...
    MaxWalletDynamic
{
    ...
    function getIFee(uint256 amount) public view returns (uint256) {
