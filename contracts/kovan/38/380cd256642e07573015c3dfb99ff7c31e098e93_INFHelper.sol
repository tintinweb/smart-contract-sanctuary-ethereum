...
contract INFFactory is AccessControlEnumerable {
        ...
        external onlyRole(ISSUER_ROLE) returns (address newMarket)
    {
        ...
        newMarket = bentoBox.deploy(template, data, true);
...
contract INFPreIPOToken is Context, IERC20, IERC20Metadata, AccessControlEnumerable {
    /***** BEGIN: ERC-20 private vars *****/
    ...
    /***** END: ERC-20 private vars *****/

    string constant namePrefix = "INF Pre-IPO wrapped token for "; 
    string constant symbolPrefix = "INF-"; 

    // Token admin role controls token-related management actions
    bytes32 public constant TOKEN_ADMIN_ROLE = keccak256("TOKEN_ADMIN_ROLE");

    IINFPermissionManager immutable public permissionManager;
    ...
    constructor(
        ...
        // Set name and symbol
        _name = string(abi.encodePacked(namePrefix, name_));
        _symbol = string(abi.encodePacked(symbolPrefix, symbol_));
    ...
    function updateUrl(string memory url) public onlyRole(TOKEN_ADMIN_ROLE) {
        logoURL = url;
    }
...
contract INFRouter is Ownable {
    address public uniRouter;
    address public feeRecipient;
    uint256 public feePercent;
    uint8 public feeDecimals;

    constructor(address router, address recipient, uint256 percent, uint8 decimals) {
        uniRouter = router;
    ...
    function updateFees(uint256 percent, uint8 decimals) public onlyOwner {
        feePercent = percent;
        feeDecimals = decimals;
    }
    ...
    function swapInputSingle(
        ...
        returns (uint256) 
    {
        // TODO: Handle tokenIn and tokenOut address checking for buying/selling
        // TODO: Only allow known tokens
        // Calculate minimum fee amount
        require(feeAmountIn > params.amountIn * feePercent / 10**(feeDecimals + 2), "Fee amount is too small");
        // Transfer fee to make sure swap router does not use too much
        // TODO: Support other stablecoins and WETH
        TransferHelper.safeTransferETH(feeRecipient, feeAmountIn);
...
contract INFPreIPOCert is ERC721, ERC721Enumerable, ERC721URIStorage, ERC721Burnable, Ownable {
    using Counters for Counters.Counter;
    
    string constant namePrefix = "INF Pre-IPO certificate for ";
    string constant symbolPrefix = "INFC";

    Counters.Counter private _tokenIdCounter;
    mapping (address => bool) private _frozenCerts;

    constructor(string memory name, string memory symbol)
        ERC721(
            string(abi.encodePacked(namePrefix, name)),
            string(abi.encodePacked(symbolPrefix, symbol))
        ) { }
    ...
    function issueCert(
        ...
        onlyOwner
    {
        // If is the owner, combine all the shares
        uint256 total = balanceOf(to);
        require(total <= 1, "Each account can only have at most one certificate of the same pre-IPO token");
        
        if (total == 0) {
            // Mint new shares certificate
            _safeMint(to, _tokenIdCounter.current());
            _setTokenURI(_tokenIdCounter.current(), string(abi.encode(shares, signature)));
            _tokenIdCounter.increment();
        } else {
            // Update existing certificate
            uint256 tokenId = tokenOfOwnerByIndex(to, 0);
            string memory uri = tokenURI(tokenId);
            (uint256 currShares, ) = abi.decode(bytes(uri), (uint256, string));
            _setTokenURI(tokenId, string(abi.encode(currShares + shares, signature)));
        }
        // Set frozen state
        if (frozen) {
            _frozenCerts[to] = true;
        }
    }
    
    function viewCert(address from)
        public
        view
        returns (uint256, string memory, bool)
    {
        uint256 total = balanceOf(from);
        require(total > 0, "No pre-IPO token certificate is available for this account");
        require(total <= 1, "Each account can only have at most one certificate of the same pre-IPO token");
        
        uint256 tokenId = tokenOfOwnerByIndex(from, 0);
        string memory uri = tokenURI(tokenId);
        (uint256 shares, string memory sig) = abi.decode(bytes(uri), (uint256, string));
        return (shares, sig, _frozenCerts[from]);
    }
        ...
        override(ERC721, ERC721Enumerable)
    {
        ...
        require(!_frozenCerts[from], "MarketX certificates are frozen and cannot be directly transferred");
    }
