...
contract Staking is AccessControl {
    ...
    function depositOrWLFromOwner(
        ...
        for (uint256 i = 0; i < len; i += 1) {
            ...
            require(timestamp >= wl.enteredAt && wl.enteredAt >= previousEnteredAt_,"5");
        ...
        nonReentrant
    {
        ...
        if (whiteList_.amount == amount && whiteList_.lockUpWL == lockUp && timestamp - deployTime / 60 <= 30 /*14 days <= 2*/) {
        ...
        returns (uint256 amount, uint256 months)
    {
        ...
        if (amount >= 3)
            months = (block.timestamp - (start + (3 * YEAR))) / 50;// days;
    }
