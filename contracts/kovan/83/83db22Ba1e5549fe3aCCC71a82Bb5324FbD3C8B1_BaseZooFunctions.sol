...
contract BaseZooFunctions is Ownable, VRFConsumerBase
{
	...
	function init(address _nftBattleArena, address owner) external onlyOwner {
	...
	function getRandomNumber() external onlyArena
	{
		require(randomResult == 0 && battles.getCurrentStage() == Stage.FifthStage, "Random was resseted");

		if (address(LINK) == address(0) || LINK.balanceOf(address(this)) < chainLinkFee)
		{
	...
	function _computePseudoRandom() internal view returns(uint)
	{
...
contract NftBattleArena is Ownable
{
	...
	function liquidateVotingPosition(uint256 votingPositionId, address voter, address beneficiary, uint256 lastEpoch, uint256 stakingPositionId) internal
	{
	...
	function withdrawDaiFromVoting(uint256 votingPositionId, address voter, uint256 daiNumber, address beneficiary) external only(nftVotingPosition)
	{
		...
		{
			liquidateVotingPosition(votingPositionId, voter, beneficiary, lastEpoch, stakingPositionId);
			return;
		}
		...
		vault.withdraw(shares, beneficiary);
