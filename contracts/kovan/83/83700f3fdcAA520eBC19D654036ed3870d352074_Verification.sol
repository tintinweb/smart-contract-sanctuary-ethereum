...
contract Verification is Initializable, IVerification, OwnableUpgradeable {
    ...
    modifier onlyVerifier() {
        require(verifiers[msg.sender], 'OV1');
        _;
    }
    ...
    function registerMasterAddress(address _masterAddress, bool _isMasterLinked) external override onlyVerifier {
        require(masterAddresses[_masterAddress][msg.sender] == 0, 'RMA1');
    ...
    function unregisterMasterAddress(address _masterAddress, address _verifier) external override {
        if (msg.sender != super.owner()) {
            require(masterAddresses[_masterAddress][msg.sender] != 0, 'UMA1');
        }
    ...
    function requestAddressLinking(address _linkedAddress) external {
        require(linkedAddresses[_linkedAddress].masterAddress == address(0), 'RAL1');
    ...
    function cancelAddressLinkingRequest(address _linkedAddress) external {
        require(pendingLinkAddresses[_linkedAddress][msg.sender], 'CALR1');
    ...
    function linkAddress(address _masterAddress) external {
        require(_masterAddress != address(0), 'LA1');
        require(linkedAddresses[msg.sender].masterAddress == address(0), 'LA2');
        require(pendingLinkAddresses[msg.sender][_masterAddress], 'LA3');
    ...
    function unlinkAddress(address _linkedAddress) external {
        ...
        require(_linkedTo == msg.sender, 'UA1');
    ...
    function addVerifier(address _verifier) external onlyOwner {
        require(_verifier != address(0), 'AV1');
        require(!verifiers[_verifier], 'AV2');
    ...
    function removeVerifier(address _verifier) external onlyOwner {
        require(verifiers[_verifier], 'RV1');
