...
contract PlayerStoxToken is Context, IERC20, Ownable {
    ...
    function allowTransfer(address sender_, bool value_) external onlyOwner {
        require(transferEnabled != value_);
...
interface ICricStoxMaster {
    ...
    function buyExactStoxForTokens(address stox_, address token_, address user_, uint amountMax_, uint256 quantity_) external returns (bool);
    function buyStoxForExactTokens(address stox_, address token_, address user_, uint256 amount_, uint256 quantityMin_) external returns (bool);
    function sellExactStoxForTokens(address stox_, address token_, address user_, uint256 amountMin_, uint256 quantity_) external returns (bool);
    function sellStoxForExactTokens(address stox_, address token_, address user_, uint256 amount_, uint256 quantityMax_) external returns (bool);
}

// 
interface ICricStoxContractPool {
...
contract CricStoxFactory is Ownable {
    ...
    function createPlayerStox(string memory name_, string memory symbol_, uint8 decimals_, address buyToken_, address[] memory contractPools_, uint256[] memory contractPoolShares_, address[] memory addressPools_, uint256[] memory addressPoolShares_) external onlyOwner {
        require(contractPools_.length == contractPoolShares_.length);
        require(addressPools_.length == addressPoolShares_.length);
        ...
        for(uint8 i=0; i<contractPoolShares_.length; i++) {
            totalShares = totalShares.add(contractPoolShares_[i]);
        }
        for(uint8 i=0; i<addressPoolShares_.length; i++) {
            totalShares = totalShares.add(addressPoolShares_[i]);
        }
        ...
        bool success = ICricStoxMaster(cricStoxMasterAddress).buyExactStoxForTokens(address(playerStox), buyToken_, adminWallet, amount, totalShares);
        require(success, "Couldn't buy stox tokens");
        for(uint8 i=0; i<contractPools_.length; i++) {
            IERC20(address(playerStox)).transfer(contractPools_[i], contractPoolShares_[i]);
            ICricStoxContractPool(contractPools_[i]).addToPool(address(playerStox), contractPoolShares_[i]);
        }
        for(uint8 i=0; i<addressPools_.length; i++) {
            IERC20(address(playerStox)).transfer(addressPools_[i], addressPoolShares_[i]);
        }
