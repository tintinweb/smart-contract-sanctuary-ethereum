...
contract SwanToken is Context, IERC20, Ownable {
    ...
    uint256 public _elseFee = marketFee+boardFee+fundFee+liquidFee+returnBackFee; //other total 15%
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount, bool takeFee) private {
        ...
        uint256 _marketFee=tElseFee.mul(marketFee).div(_totalFeeRatio);
        _takeInviterFee(sender, recipient, _marketFee); 
        uint256 _liquidFee=tElseFee.mul(liquidFee).div(_totalFeeRatio);
        _takeLiquidity(sender, _liquidFee); 
        uint256 _boardFee=tElseFee.mul(boardFee).div(_totalFeeRatio);
        _takeBoard(sender, _boardFee);
         uint256 _fundFee=tElseFee.mul(fundFee).div(_totalFeeRatio);
    ...
    function calculateTaxFee(uint256 _amount) private view returns (uint256) {
        return _amount.mul(_taxFee).div(_totalFeeRatio);
    }

    function calculateElseFee(uint256 _amount) private view returns (uint256) {
        return _amount.mul(_elseFee).div(_totalFeeRatio);
    }
