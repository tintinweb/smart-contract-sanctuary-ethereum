...
contract PrecogCore is IPrecogCore {
    ...
    modifier onlyAdmin() {
        require(msg.sender == admin, "PrecogCore: NOT_ADMIN_ADDRESS");
        _;
    }
    ...
    mapping(address => uint256) public minInvestment;
    ...
    function setCycleConfiguration(CycleConfiguration memory config) external override onlyAdmin {
        ...
        address[] memory tokens = IPrecogV5(precog).getExistingTokens();
        for(uint i = 0; i < tokens.length; i++) {
            IPrecogV5(precog).updateProfitCycleToCurrentCycle(tokens[i]);
        }
    ...
    function collectFee(address token) external override onlyAdmin {
    ...
    function setMinInvestment(address token, uint256 amount) external onlyAdmin {
        minInvestment[token] = amount;
    }
