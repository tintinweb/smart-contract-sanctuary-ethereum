...
contract NFTStakingPoolFunctions is AccessControl {
    ...
    uint256 public constant RAINI_REWARD_DECIMALS = 1000000000;
    ...
    function initLevels(uint8[] memory levels) external onlyOwner {
        ...
        for (uint256 i; i < levels.length; i++) {
            ...
            nftStakingPool.setStaminaEvent(
                uint24(_staminaEventCount),
                INFTStakingPool.StaminaEvent({
    ...
    function calculateRewardPerToken(
        ...
        uint56 rainiRewardRate,
        uint32 _totalSupply
    ) private pure returns (uint56 rewardPerToken) {
        if (_totalSupply == 0) {
            return 0;
        }
    ...
    function getRewardPerToken(uint24 _eventId, uint24 _fromEvent)
        public
        view
        returns (INFTStakingPool.StaminaEvent memory se)
    {
        uint24 _currentId = _fromEvent;
        INFTStakingPool.StaminaEvent memory _se = nftStakingPool.staminaEvents(
            _currentId
        );
        ...
        uint56 rainiRewardRate;
        uint32 rewardPeriodFinish = nftStakingPool
            .generalRewardVars(_se.level)
            .periodFinish;
        uint256 _finalTimeApplicable = _se.timeStamp < rewardPeriodFinish
            ? Math.min(block.timestamp, rewardPeriodFinish)
            : block.timestamp;
        ...
        while (true) {
            ...
            INFTStakingPool.StaminaEvent memory _seNext = nftStakingPool
                .staminaEvents(_se.next);

            if (
                _eventId != _currentId &&
                (_se.next == 0 || _seNext.timeStamp > _finalTimeApplicable)
            ) {
                require(
                    _eventId == 0 && _se.timeStamp <= _finalTimeApplicable,
                    "invalid start 2"
                );
                _currentId = 0;
            ...
            if (
                _eventId == _currentId || _se.timeStamp >= _finalTimeApplicable
            ) {
                if (_eventId == 0) {
                    if (_se.timeStamp != lastTime) {
                        rainiRewardPerTokenStored += calculateRewardPerToken(
                            _se.timeStamp - lastTime,
                            rainiRewardRate,
                            _totalSupply
                        );
                    }
    ...
    function balanceUpdate(
        address _owner,
        uint24[] memory _eventIds,
        uint32[] memory nftOrder,
        int256 _dataUpdate,
        uint256 _level
    ) internal {
        INFTStakingPool.AccountVars memory _accountVars = nftStakingPool
            .accountVars(_owner);
        INFTStakingPool.AccountRewardVars memory _rewardVars = nftStakingPool
            .accountRewardVars(_owner, _level);

        uint256 lastDate = _rewardVars.lastUpdated;

        uint24[] memory _stakedNFTs = nftStakingPool.getStakedNfts(
            _owner,
            _level
        );
        ...
        for (uint256 n = 0; n <= nftOrder.length; n++) {
            ...
            if (n < nftOrder.length) {
                ...
                _nftEvent = INFTStakingPool.StaminaEvent(
                    0,
                    0,
                    0,
            ...
            require(
                _nftEvent.timeStamp <= _rewardVars.lastUpdated ||
            ...
            if (
                n == nftOrder.length ||
                (_nftEvent.timeStamp > _rewardVars.lastUpdated &&
                    _nftEvent.timeStamp <= block.timestamp)
            ) {
                ...
                if (n == nftOrder.length || _nftEvent.timeStamp != lastDate) {
                    ...
                    require(_se.level == _level, "bad level");
                    _accountVars.rainiRewards += uint104(
                        (uint256(_rewardVars.staked) *
                            (_se.rainiRewardPerTokenStored -
                                _rewardVars.rainiRewardPerTokenPaid))
                    );
                    _rewardVars.rainiRewardPerTokenPaid = _se
                        .rainiRewardPerTokenStored;
                    _accountVars.pointsBalance += calculateReward(
                        _rewardVars.staked,
                        _nftEvent.timeStamp - lastDate,
                        _level
                    );
                }
                ...
                if (n == nftOrder.length) {
                    if (_owner == address(0)) {
                        _se.rainiRewardRate = uint56(uint256(_dataUpdate));
                    } else {
                        //update the total supply depending on whether the user is staking or withdrawing
                        require(
                            int256(uint256(_se.totalSupply)) + _dataUpdate >= 0,
                            "totalSupply err"
                        );
                        _se.totalSupply = uint32(
                            uint256(
                                int256(uint256(_se.totalSupply)) + _dataUpdate
                            )
                        );
                        //_se.supplyDelta = int16(_dataUpdate);

                        require(
                            int256(uint256(_rewardVars.staked)) + _dataUpdate >=
                                0,
                            "staked err"
                        );
                        _rewardVars.staked = uint32(
                            uint256(
                                int256(uint256(_rewardVars.staked)) +
                                    _dataUpdate
                            )
                        );
    ...
    function stake(
        ...
        uint24[] memory _staminaEventId,
        uint24[][] memory _eventIds,
        uint32[][] memory _nftOrder
    ) external {
        ...
        for (uint256 i = 0; i < _tokenId.length; i++) {
            uint256 _contractId = nftStakingPool.approvedTokenContracts(
                _contractAddress[i]
            );
            ...
            _locals.nftId = nftStakingPool.rainiNftIdMap(
                _contractAddress[i],
                _tokenId[i]
            );
            ...
            if (_locals.nftId == 0) {
                ...
                nftStakingPool.setRainiNft(
                    uint24(_locals.rainiNftCount),
                    INFTStakingPool.RainiNft({
                        id: uint24(_locals.rainiNftCount),
                        lastStamina: uint32(_locals.stamina),
                        maxStamina: _cl.maxStamina,
                        level: _tv.level,
                        lastUpdated: uint32(block.timestamp),
                        staminaEventId: uint24(
                            nftStakingPool.staminaEventCount() + 1
                        ),
                        tokenId: _tokenId[i],
                ...
                nftStakingPool.setRainiNftIdMap(
                    _contractAddress[i],
                    _tokenId[i],
                    _locals.nftId
                );
            } else {
                INFTStakingPool.RainiNft memory _nft = nftStakingPool.rainiNfts(
                    _locals.nftId
                );
                _locals.stamina = _nft.lastStamina;
                _nft.isStaked = true;
                _nft.lastUpdated = uint32(block.timestamp);
                _nft.staminaEventId = _locals.stamina > 1
                    ? uint24(nftStakingPool.staminaEventCount() + 1)
                    : 0;
            ...
            if (_locals.stamina > 1) {
                uint256 endTime = block.timestamp +
                    (_locals.stamina * nftStakingPool.staminaDuration()) /
                    _cl.maxStamina /
                    STAMINA_DECIMALS;

                INFTStakingPool.StaminaEvent memory _se = INFTStakingPool
                    .StaminaEvent({
                        id: 0,
                        next: 0,
                ...
                nftStakingPool.insertStaminaEvent(
                    _staminaEventId[i],
                    _se,
                    _tv.level
                );
            ...
            nftStakingPool.addStakedNft(
                _msgSender(),
                _tv.level,
                uint24(_locals.nftId)
            );
        }
        ...
        for (uint256 i = 0; i < _eventIds.length; i++) {
            ...
            if (_locals.supplyAdded[i] > 0) {
                ...
                require(
                    _eventIds.length >= i && _eventIds[i].length > 0,
                    "Invalid fields"
                );
                balanceUpdate(
                    _msgSender(),
    ...
    function withdrawTokens(
        ...
        INFTStakingPool.RainiNft[]
            memory _nfts = new INFTStakingPool.RainiNft[](_tokenId.length);

        for (uint256 i = 0; i < _tokenId.length; i++) {
            uint256 _nftId = nftStakingPool.rainiNftIdMap(
                _contractAddress[i],
                _tokenId[i]
            );
            ...
            require(
                _eventIds.length >= _nfts[i].level + 1 &&
                    _eventIds[_nfts[i].level].length > 0,
                "Invalid fields"
            );
            ...
            if (
                nftStakingPool
                    .staminaEvents(_nfts[i].staminaEventId)
        ...
        for (uint256 i = 0; i < _tokenId.length; i++) {
            bool ownsNFT = nftStakingPool.removeStakedNft(
                _msgSender(),
                _nfts[i].level,
                _nfts[i].id
            );
            ...
            nftStakingPool.removeStaminaEvent(
                _nfts[i].staminaEventId,
                _prevEventId[i]
            );

            _nfts[i].lastStamina = nftStakingPool.getTokenStaminaTotal(
                _tokenId[i],
                _contractAddress[i]
            );
    ...
    function calculateReward(
        ...
        return uint128(_duration * nftStakingPool.rewardRate(_level) * _amount);
    }

    // RAINI rewards

    function addRainiRewardPool(
        uint256 _amount,
        uint256 _duration,
        uint32 _level,
        uint24[] memory _eventIds
    ) external onlyOwner {
        INFTStakingPool.GeneralRewardVars
            memory _generalRewardVars = nftStakingPool.generalRewardVars(
                _level
            );
        ...
        nftStakingPool.rainiToken().safeTransferFrom(
            _msgSender(),
            address(nftStakingPool),
            _amount
        );
        _generalRewardVars.rainiRewardRate = uint56(
            _amount / _duration / RAINI_REWARD_DECIMALS
        );
        ...
        balanceUpdate(
            ...
            _eventIds,
            new uint32[](0),
            int256(uint256(_generalRewardVars.rainiRewardRate)),
            _level
        );
    }

    function abortRainiRewardPool(uint32 _level, uint24[] memory _eventIds)
        external
        onlyOwner
    {
        INFTStakingPool.GeneralRewardVars
        ...
        uint256 remainingAmount = timeRemaining *
            _generalRewardVars.rainiRewardRate;
    ...
    function withdrawReward(
        ...
        INFTStakingPool.AccountVars memory _accountVars = nftStakingPool
            .accountVars(_msgSender());

        require(_accountVars.rainiRewards > 0, "no reward to withdraw");
        nftStakingPool.transferRaini(
            _msgSender(),
            _accountVars.rainiRewards * RAINI_REWARD_DECIMALS
        );
    ...
    function getRewardBalance(
        ...
        uint32[] memory nftOrder,
        uint256 _level
    ) external view returns (INFTStakingPool.AccountVars memory rewards) {
        INFTStakingPool.AccountVars memory _accountVars = nftStakingPool
            .accountVars(_owner);
        INFTStakingPool.AccountRewardVars memory _rewardVars = nftStakingPool
            .accountRewardVars(_owner, _level);
...
interface INFTStakingPool {
    ...
    function generalRewardVars(uint256 _level)
        external
        view
        returns (GeneralRewardVars memory);

    function accountRewardVars(address _account, uint256 _level)
        external
        view
        returns (AccountRewardVars memory);

    function accountVars(address _account)
        external
        view
        returns (AccountVars memory);

    function staminaEvents(uint24 _id)
        external
        view
        returns (StaminaEvent memory);
    ...
    function stakedNFTs(address _account, uint256 _level)
        external
        view
    ...
    function rainiNftIdMap(address _contract, uint256 _tokenId)
        external
        view
        returns (uint256);
    ...
    function approvedTokenContracts(address _contractAddress)
        external
        view
        returns (uint256);
    ...
    function getStakedNfts(address _account, uint256 _level)
        external
        view
        returns (uint24[] memory _stakedNfts);

    //Setters

    function setGeneralRewardVars(
        uint256 _level,
        GeneralRewardVars memory _generalRewardVars
    ) external;

    function setAccountRewardVars(
        address _account,
        uint256 _level,
        AccountRewardVars memory _accountRewardVars
    ) external;
    ...
    function addStakedNft(
        address _account,
        uint256 _level,
        uint24 _nftId
    ) external;

    function removeStakedNft(
        address _account,
        uint256 _level,
        uint24 _nftId
    ...
    function insertStaminaEvent(
        uint24 _currentId,
        StaminaEvent memory _newEvent,
        uint256 _level
    ) external;
    ...
    function initNft(
        ...
        uint32 _stamina,
        bool _isStaked,
        uint24 _staminaEventId
    ) external returns (uint32 maxStamina);

    function removeStaminaEvent(uint24 _removeId, uint24 _prevId)
        external
        returns (bool wasRemoved);
