...
/**
 * @title Chai's price oracle, rely on various chainlink price feed
...
contract ChainlinkOracle is Ownable {
    ...
    mapping(address => TokenConfig) public getTokenConfigByChToken;

    event TokenConfigChanged(address indexed token, PriceSource source, uint256 fixedPrice, address chainlinkFeed);

    function getUnderlyingPrice(address _chToken) external view returns (uint256) {
        TokenConfig storage config = getTokenConfigByChToken[address(_chToken)];
    ...
    function configToken(
        ...
        getTokenConfigByChToken[_chToken] = config;
        emit TokenConfigChanged(_chToken, _source, _fixedPrice, _chainlinkPriceFeed);
    }
