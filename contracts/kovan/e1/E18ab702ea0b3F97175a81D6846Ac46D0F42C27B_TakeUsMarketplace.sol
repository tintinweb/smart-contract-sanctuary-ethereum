...
contract TakeUsMarketplace is Ownable {
  ...
  modifier eligibleListingERC721(IERC721 _nftAddress, uint256 _tokenId) {
    // Check that require. May be needed to write a separe function that can process throw
    require(
  ...
  function listERC721(
    ...
    if (lenderFee > 0) {
      uint256 fee = (listingERC721[_nftAddress][_tokenId].price * lenderFee) /
        100;
      transfer(
        msg.sender,
    ...
    eligibleListingERC721(_nftAddress, _tokenId)
  {
    ...
    emit Canceled(address(_nftAddress), _tokenId, 0);
  }
  ...
  function listERC1155(
    ...
    if (lenderFee > 0) {
      uint256 fee = (listingERC1155[_nftAddress][_tokenId].price * lenderFee) /
        100;
      transfer(
        msg.sender,
        addressRegistry.royaltyCollector(),
    ...
    eligibleListingERC1155(_nftAddress, _tokenId)
  {
    ...
    emit Canceled(address(_nftAddress), _tokenId, 1);
  }
