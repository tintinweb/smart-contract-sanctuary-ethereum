...
import {VaultLifecycleMatching as VaultLifecycle} from "../../libraries/VaultLifecycleMatching.sol";
import {ShareMath} from "../../libraries/ShareMath.sol";
import {RibbonVault} from "./base/RibbonVault.sol";
import {IMatchingSwap} from "../../interfaces/IMatchingSwap.sol";
 ...
 */
contract MatchingThetaVault is RibbonVault, RibbonThetaVaultStorage {
    ...
    function commitAndClose(
        ...
        address otokenAddress = VaultLifecycle.commitAndClose(closeParams, vaultParams, strikePrice);

        emit NewOptionStrikeSelected(strikePrice, 0);
    ...
    function rollToNextOption() external onlyKeeper nonReentrant {
        ...
        if (optionState.currentOption != address(0)) {
            uint256 optionAmount = IERC20(optionState.currentOption).balanceOf(address(this));
            IERC20(optionState.currentOption).approve(SWAP, optionAmount);
            IMatchingSwap(SWAP).swap(optionState.currentOption, optionAmount, vaultParams.asset, currentOtokenPremium);
        }
    }
    ...
    /**
     * @notice Sets the new swap contract
     * @param _swap is the address of the new swap
     */
...
library VaultLifecycleMatching {
    using SafeMath for uint256;
        ...
        )
    {
        ...
        {
            (performanceFeeInAsset, , totalVaultFee) = VaultLifecycleMatching
                .getVaultFees(
                balanceForVaultFees,
...
interface IMatchingSwap {
    function swap(address option, uint256 optionAmount, address premium, uint256 premiumAmount) external;
}
