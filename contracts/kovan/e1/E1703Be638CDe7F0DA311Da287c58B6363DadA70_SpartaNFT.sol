...
struct TransactionData {
    address buyer;
    uint256 dateBought;
    uint256 numberBought;
}

contract SpartaNFT is
    ERC721,
    ReentrancyGuard,
    Ownable,
    ERC2981PerTokenRoyalties
{
    ...
    Counters.Counter private winnerCount;
    Winner[] public winnerLog;
    ...
    uint256 public constant TOTAL_WALLET_LIMIT = 15;
    uint256 public MAX_MINT_AMOUNT_TX = 5;
    uint256 public MAX_SUPPLY = 20;
    uint256 private contractRoyalties = 500; //5%
    ...
    // Presale is 8 hours
    uint256 public preSaleDuration = 8;
    ...
    constructor() ERC721("SpartaNodes", "SNNFT") {
    ...
    modifier mintCompliance(uint256 _mintAmount) {
        ...
        require(
            _mintAmount > 0 && _mintAmount <= MAX_MINT_AMOUNT_TX,
            "Invalid mint amount!"
        );
        require(totalSupply() < MAX_SUPPLY, "SOLD OUT");
        require(
            supply.current() + _mintAmount <= MAX_SUPPLY,
            "Not enough left to purchase"
        );
        require(preSaleStart || publicSaleStart, "You must wait for launch");
        _;
    }
        ...
        mintCompliance(_mintAmount)
    {
        ...
        require(_mintAmount > 0, "Cannot buy 0");
        require(
            transactionsForAddress[msg.sender].length < TOTAL_WALLET_LIMIT,
            "This wallet minted the max amount!"
        );
        require(
        ...
        if (
            preSaleStart &&
            !publicSaleStart &&
            (block.timestamp <= preSaleStartTime + (preSaleDuration * 1 hours))
        ) {
            // Presale has started but not ended

            if (isWhitelisted[msg.sender]) {
                if (
                    (transactionsForAddress[msg.sender].length + _mintAmount) <=
                    PRESALE_LIMIT
                ) {
            ...
            if (publicSaleStart) {
                ...
                revert("Pre or Public sale has not started");
            }
        }
    }
    ...
    function _mintLoop(address _receiver, uint256 _mintAmount) internal {
        for (uint256 i = 0; i < _mintAmount; i++) {
            ...
            transactionsForAddress[_receiver].push(transaction);
            supply.increment();
            _safeMint(_receiver, supply.current());
            _setTokenRoyalty(supply.current(), owner(), contractRoyalties);
        }
    }

    //public views
    function getTransactions() public view returns (uint256) {
        return transactionsForAddress[msg.sender].length;
    }
    ...
    function setMaxMintAmountPerTx(uint256 _maxMintAmountPerTx)
        ...
        onlyOwner
    {
        MAX_MINT_AMOUNT_TX = _maxMintAmountPerTx;
    }
    ...
    //whitelist functions
    //one address
    function addWhitelistAddress(address _wlAddress) public onlyOwner {
        isWhitelisted[_wlAddress] = true;
    }

    //many addresses
    function addWhitelistAddresses(address[] calldata _wlAddresses)
        public
        onlyOwner
    {
        for (uint256 i = 0; i < _wlAddresses.length; i++) {
            addWhitelistAddress(_wlAddresses[i]);
        }
    }

    //remove addresses
    function removeWhitelistAddresses(address[] calldata _rmAddresses)
        public
        onlyOwner
    {
        for (uint256 i = 0; i < _rmAddresses.length; i++) {
            isWhitelisted[_rmAddresses[i]] = false;
        }
    }
    ...
    function produceWinner(uint256 _token) public onlyOwner {
        Winner memory winner;
        winner.date = block.timestamp;
        winner.token = _token;
        winner.waddress = ERC721.ownerOf(_token);
    ...
    function getLatestWinner() public view returns (address) {
        return winnerLog[winnerCount.current() - 1].waddress;
    }
