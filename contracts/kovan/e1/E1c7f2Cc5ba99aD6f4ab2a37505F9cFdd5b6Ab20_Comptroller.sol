...
interface DistributionerInterface {
    ...
    function rewardSupplierIndex(address marketToken, address account) external view returns(uint);
    function rewardBorrowerIndex(address marketToken, address account) external view returns(uint);
...
contract Comptroller is AbstractComptroller {
    ...
    function _setMarketMintPaused(MarketTokenInterface marketToken, bool state) public returns (bool) {
       _checkWhenSetPaused(marketToken, state);
    ...
    function _checkWhenSetPaused(MarketTokenInterface marketToken, bool state) internal view{
        ...
        require(hasOwnerRights() || state == true, "only owner can unpause");
    }
