...
contract BixosverseGame is AccessControl {
        ...
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        ...
        emit DefaultMinTitleLengthUpdated(defaultMaxGameStartTime);
    }
        ...
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        ...
        emit DefaultMinTitleLengthUpdated(defaultMinTitleLength);
    }
        ...
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        ...
        emit DefaultMaxTitleLengthUpdated(defaultMaxTitleLength);
    }
        ...
        onlyValidPlayerCount(_maxPlayers)
    {
        ...
        uint256 max = now_ + defaultMaxGameStartTime;

        if (_startTime < now_ || _startTime > max)
            revert InvalidGameStartTime({ min: now_, max: max });
    ...
    function joinGame(uint256 nftId) public onlyIslandExists(nftId) {
        ...
        emit GameJoined(nftId, msg.sender, game.pool);
    }
        ...
        onlyRole(ORACLE_ROLE)
    {
        ...
        for (uint256 i = 0; i < game.joinedPlayersCount; i++) {
        ...
        onlyRole(ORACLE_ROLE)
    {
        ...
        _ubxs.safeTransfer(
            game.winner,
            (game.pool * game.percentages.winner) / 100
        );
        _ubxs.safeTransfer(
     ...
     */
    function updateFeeWallet(address feeWalletAddress)
        public
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        _feeWallet = feeWalletAddress;
    ...
    {
        require(
            gameId < _islands[islandId].games.length,
            "Game does not exist"
        );
