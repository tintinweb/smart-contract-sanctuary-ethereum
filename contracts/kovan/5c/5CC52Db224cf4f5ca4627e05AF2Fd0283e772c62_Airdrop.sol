...
contract Airdrop is Ownable {
	...
	/**
     * @notice Event emitted when user claim the XANA token
     */
    event claimEvent(address user, uint256 amount);

	/**
     * @notice Event emitted when owner set next airdrop
     */
    event setNextAirdropEvent(uint256 setNextAirdropEvent);
	...
	/**
     * @notice number of xana
     */
    uint256 public numberOfXana;

	/**
     * @notice period of airdrop
     */
    uint256 public period = 5 hours;

	/**
     * @notice start time of airdrop
     */
	...
	/**
     * @notice claim availabe users
     */
    mapping(uint256 => mapping(address => bool)) userLists;

    /**
     * @notice Method for getting the xana token
    ...
    /**
     * @notice Method for claim
     */
    function claim() public {
        require(
            block.timestamp <= startAt.add(period),
            "claim error: airdrop is expired"
        );
        require(
            userLists[currentNumber][msg.sender] == false,
            "claim error: you have already claimed XANA token"
        );
        require(
            ZoraToken.balanceOf(msg.sender) > 0,
            "claim error: you are not zora holder"
        );
        require(
            numberOfXana > 0,
            "claim error: currently airdrop is not available"
        );
        uint256 amount = numberOfXana.mul(ZoraToken.balanceOf(msg.sender)).div(
            ZoraToken.totalSupply()
        );
        require(
            XanaToken.balanceOf(XanaTokenSender) > amount,
            "claim error: balance of sender is not enough"
        );
        XanaToken.safeTransferFrom(XanaTokenSender, msg.sender, amount);
        userLists[currentNumber][msg.sender] = true;
     ...
     */
    function getXanaForUser(address _holder) external view returns (uint256) {
        uint256 amount;
        if (userLists[currentNumber][_holder] == true) {
            amount = 0;
        } else {
            amount = numberOfXana.mul(ZoraToken.balanceOf(_holder)).div(
                ZoraToken.totalSupply()
            );
        }
        return amount;
    ...
    /**
     * @notice Method for user validator
     * @param _holder address
     */
    function validator(address _holder) external view returns (bool) {
        return userLists[currentNumber][_holder];
    }

    /**
     * @notice Method for setting sender address
     * @dev Only admin
     * @param _newSenderAddr address
     */
    function setSenderAddr(address _newSenderAddr) external onlyOwner {
        XanaTokenSender = _newSenderAddr;
        emit changeSenderAddr(_newSenderAddr);
    }

    /**
     * @notice Method for setting next airdrop
     * @dev Only admin
     */
    function setNextAirdrop() external onlyOwner {
        startAt = block.timestamp;
        currentNumber += 1;
        emit setNextAirdropEvent(currentNumber);
    }

    /**
     * @notice Method for setting XANA amount
     * @dev Only admin
     */
    function setXanaAmount(uint256 _amount) external onlyOwner {
        numberOfXana = _amount;
    }

    /**
     * @notice Method for setting Zora token address
     * @dev Only admin
     * @param _newZoraToken address
     */
    function setZoraTokenAddr(address _newZoraToken) external onlyOwner {
     ...
     * @dev Only admin
     * @param _newXanaToken address
     */
    function setXanaTokenAddr(address _newXanaToken) external onlyOwner {
