...
contract TokenFarm is Ownable{
    ...
    function getUserTotalValue(address _user) public view returns (uint256){
        ...
        for(uint i_allowed_tokens; i_allowed_tokens < allowedTokens.length; i_allowed_tokens++){
            totalValue = totalValue + getUserSingleTokenValue(_user, allowedTokens[i_allowed_tokens]);
        }
    ...
    function issueTokens() public onlyOwner {
        for(uint256 i_staker = 0;i_staker<stakers.length; i_staker++){
            address recipient = stakers[i_staker];
    ...
    function removeStaker(address _user) private {      
          
        bool staker_found = false;
        for(uint256 i=0; i<stakers.length; i++){
            if(stakers[i] == _user) staker_found = true;
            if(staker_found){
