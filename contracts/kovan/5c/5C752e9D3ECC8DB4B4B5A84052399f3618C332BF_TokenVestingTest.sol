...
contract TokenVestingTest is TokenVesting {
    uint256 public currentTimestamp;

    constructor(IERC20 _token, address agent) TokenVesting(_token, agent) {}

    function currentTime() public view virtual override returns (uint256) {
        return currentTimestamp;
    }

    function setCurrentTime(uint256 _timestamp) external onlyOwner {
        currentTimestamp = _timestamp > 0 ? _timestamp : block.timestamp;
    }
}