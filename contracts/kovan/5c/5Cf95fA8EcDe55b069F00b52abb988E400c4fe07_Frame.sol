...
contract Frame is Ownable {
    ...
    event StakedToken(address wallet, uint256 tokenId, uint256 timestamp);
    ...
    function stake(uint256[] memory tokens) public isStakingAvailable isNotContract {
        ...
        for (uint256 x = 0;x < tokens.length;x++) {
            ...
            emit StakedToken(msg.sender, _token, tokenYield[_token]);
        }
    ...
    function getLockedInTime(address wallet) public view isNotContract returns (uint256) {
