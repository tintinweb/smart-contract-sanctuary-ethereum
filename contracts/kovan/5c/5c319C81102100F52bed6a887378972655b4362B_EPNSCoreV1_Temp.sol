...
contract EPNSCoreV1 is Initializable, EPNSCoreStorageV1 {
    using SafeMath for uint256;
...
// import "hardhat/console.sol";
// num channeels: 70 => `10` => 7
contract TempStore{
    mapping(address=>bool) _isChannelUpdated;

    function isChannelUpdated(address addrs) external returns(bool) {
        return _isChannelUpdated[addrs];
    }

    function setChannelUpdated(address addrs)external {
        _isChannelUpdated[addrs] = true;
    }    
}

contract EPNSCoreV1_Temp is Initializable, EPNSCoreStorageV1_5, PausableUpgradeable,TempStore {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;
     ...
     */
     function adjustChannelPoolContributions(
       ...
       address tempStore
        ...
        for (uint256 i = _startIndex; i < _endIndex; i++) {
          if(channels[_channelAddresses[i]].channelState == 0 || tempStore.isChannelUpdated(_channelAddresses[i])){
            //   channels[_channelAddresses[i]].channelVersion == 2){ // instead of this do extenal call
                continue;
              } else{
                ...
                // channels[_channelAddresses[i]].channelVersion = 2; // update to true
                tempStore.setChannelUpdated(_channelAddresses[i]);
