...
contract FileStorage {
    ...
    mapping(address => bool) public logged;
    ...
    // function balanceFunc() public view returns(uint) {
    //     Token thyme = Token(addressThyme);
    //     uint result = thyme.balanceOf[msg.sender];
    //     return result;
    // }

    // methods for the account specified in the parameter
    // function balanceOf_aUser(address userAccoount) public view returns(uint) {
    //     Token thyme = Token(addressThyme);
    //     uint result = thyme.balanceOf[userAccoount];
    //     return result;
    // }

    // methods for the account selected in metamask at the moment of transaction
    function isMemberFunc() public view returns(bool) {
        return isMember[msg.sender];
    }

    function isMember_aUser(address userAccoount) public view returns(bool)  {
        return isMember[userAccoount];
    }
    ...
    function uploadFiles(address recipient, uint fileSize) external { //diskSpace in GB
      ...
      Token thyme = Token(addressThyme);
       ...
       // uint charge = fileSize * 0.75;
        ...
        require(isMember[msg.sender] == true, 'user is not a member'); 
    ...
    //before using this method compile tyme contract deploy then in the approve method enter this file storage address 
    //and amount approved
    function uploadFiles_usingTransferFrom(address recipient, uint fileSize) external { //diskSpace in GB
