...
contract FundsProvider is Ownable {
    ...
    function push(address token, uint256 amt) external payable returns (uint256 _amt) {
        ...
        if (token != Configure.ETH_ADDRESS) {
            _amt = _amt == type(uint256).max ? IERC20(token).balanceOf(msg.sender) : _amt;
    ...
    function pullInternal(
        ...
        if (token == Configure.ETH_ADDRESS) {
            ...
            require(retCall != false, "BYDEFI: pull ETH from funds provider fail");
        } else {
            _amt = _amt == type(uint256).max ? IERC20(token).balanceOf(address(this)) : _amt;
