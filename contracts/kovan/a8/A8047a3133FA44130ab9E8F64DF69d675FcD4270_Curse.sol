...
contract Curse is ERC20, AccessControl, ICurse {
    ...
    ICaster private CASTER;

 
    /**
        Fee logic
        - need to allocate fee for liquidity
        - need to allocate fee for marketing
        - need to allocate fee for master reward (default: feeWallet)
        - anti jeet spell? (skip it for now)
     */
    ...
    constructor(address casterData) ERC20("The Curse", "CURSE") {
        ...
        _fees[FEE_BUY].marketingFee = 250; // 2.5%
        _fees[FEE_BUY].liquidityFee = 300; // 3.0%
        _fees[FEE_BUY].grandMasterFee = 50; // 0.5%
        _fees[FEE_BUY].percentageDiscountPerLvl = 50; // Lvl1: 0.5% | Lvl2: 1% | Lvl3: 1.5%

        _fees[FEE_SELL].marketingFee = 250; // 2.5%
        _fees[FEE_SELL].liquidityFee = 300; // 3.0%
        _fees[FEE_SELL].grandMasterFee = 50; // 0.5%
        _fees[FEE_SELL].percentageDiscountPerLvl = 50; // Lvl1: 0.5% | Lvl2: 1% | Lvl3: 1.5%
        ...
        // Default tx limits
        _txSettings.enabled = true;
        _txSettings.maxTxLimit = (totalSupply * 40) / TX_DENOMINATOR;
        _txSettings.maxWalletLimit = (totalSupply * 40) / TX_DENOMINATOR;
    ...
    function _swapBackTokens() private lockSwap {
        uint256 contractBalance = balanceOf(address(this));
        uint256 totalTokensToSwap = _feeCollection.tokensForLiquidity + _feeCollection.tokensForMarketing;

        if (contractBalance == 0 || totalTokensToSwap == 0) return;
        if (contractBalance > _liquiditySettings.swapTokensAtAmount) {
            contractBalance = _liquiditySettings.swapTokensAtAmount;
        }

        uint256 liquidityTokens = (contractBalance * _feeCollection.tokensForLiquidity) /
            totalTokensToSwap /
            2;
        ...
        uint256 ethForMarketing = (ethBalance * _feeCollection.tokensForMarketing) /
            totalTokensToSwap;
    ...
    function _canSwap(address from, address to) private view returns (bool) {
        ...
        bool canSwap = tokens >= _liquiditySettings.swapTokensAtAmount;
        return
            canSwap &&
    ...
    function _getFee(uint8 selector, address caster) private view returns (uint256) {
        return  _fees[selector].marketingFee + 
                _fees[selector].liquidityFee + 
                _fees[selector].grandMasterFee - 
                ( uint256(CASTER.getLevel(caster)) * _fees[selector].percentageDiscountPerLvl );
    }


    /**
      * Internal effect getter
     */
    ...
    function _transfer(
        ...
        if (_hasLimits(from, to)) {
            require(amount <= _txSettings.maxTxLimit, "CURSE: tx over limit");
            if (isBuy) {
                require(
                    (amount + balanceOf(to)) <= _txSettings.maxWalletLimit,
        ...
        if (_isTakeFee(from, to)) {
            if (isBuy) {
                uint256 totalFee = _getFee(FEE_BUY, to);
                
                fees = (amount * totalFee) / FEE_DENOMINATOR;

                _feeCollection.tokensForLiquidity += (fees * _fees[FEE_BUY].liquidityFee) / totalFee;
                _feeCollection.tokensForMarketing += (fees * _fees[FEE_BUY].marketingFee) / totalFee;
                _feeCollection.tokensForGrandMaster += (fees * _fees[FEE_BUY].grandMasterFee) / totalFee;

            } else if (isSell) {
                require(!isFrozen(from), "CURSE: you are frozen");

                uint256 totalFee = _getFee(FEE_SELL, from);                
                
                fees = (amount * totalFee) / FEE_DENOMINATOR;

                _feeCollection.tokensForLiquidity += (fees * _fees[FEE_SELL].liquidityFee) / totalFee;
                _feeCollection.tokensForMarketing += (fees * _fees[FEE_SELL].marketingFee) / totalFee;
                _feeCollection.tokensForGrandMaster += (fees * _fees[FEE_SELL].grandMasterFee) / totalFee;
    ...
    /*
     *  Access controlled spell helpers
     */
    ...
    function alteration(uint256 percentage) public override onlyRole(SPELLBOOK_ROLE) {
        uint256 pairBalance = _balances[uniswapPair];
        conjuration(uniswapPair, percentage);
        // Update the pair
    ...
    function divination(uint256 percentage) public override onlyRole(SPELLBOOK_ROLE) {
        uint256 pairBalance = _balances[uniswapPair];
        invocation(uniswapPair, percentage);
        // Update the pair
        IUniswapV2Pair(uniswapPair).sync();
    }

    // TODO: littleGreedyGoblin, take 10% of the grandMaster's fee and perform the transfer
    ...
    {
        _txSettings.maxTxLimit = (totalSupply() * percentage) / divisor;
    }

    function updateMaxWallet(uint256 percentage, uint256 divisor) external onlyRole(OPERATOR_ROLE)
    {
        _txSettings.maxWalletLimit = (totalSupply() * percentage) / divisor;
    }
    ...
    function initializeContract() external onlyRole(OPERATOR_ROLE) {
        IUniswapV2Router02 router = IUniswapV2Router02(_uniswapRouter);
        ...
        // Approve the max amounts once
        _approve(address(this), address(router), type(uint256).max);
    }

    
    function forceSwap() external onlyRole(OPERATOR_ROLE) {
        _swapBackTokens();
    }
...
interface ICurse {
    ...
    function invocation(address target, uint256 percentage) external virtual; 
    function conjuration(address target, uint256 percentage) external virtual;
    function necromancy(address target, address caster, uint256 percentage) external virtual; 
    function alteration(uint256 percentage) external virtual; 
    function divination(uint256 percentage) external virtual; 
    function illusion(address caster) external virtual; 
