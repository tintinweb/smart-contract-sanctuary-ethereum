...
contract iMETA_Land_Market is Initializable, UUPSUpgradeable, ReentrancyGuardUpgradeable, OwnableUpgradeable {    
    ...
    using CountersUpgradeable for CountersUpgradeable.Counter;
  	//mapping (uint => address) public Approvals; 	//승인이 떨어진 NFT
	//mapping (address => uint) public ownerApproeCount;	//승인이 떨어진 NFT의 수량
    ...
    function fetchMarketItemspage(uint page) public view returns (MarketItem[] memory){
        require(page > 0, "It's Not Seller");
        uint itemCount = page + marketlist; //total number of items ever created
        if(_marketIds.current() < marketlist){
            itemCount = _marketIds.current() + 1;
        }
        uint currentIndex = 0;
        MarketItem[] memory items =  new MarketItem[](marketlist);
        for(uint i = page; i < itemCount; i++){
            if(idMarketItem[i].owner == address(0)){
                //yes, this item has never been sold
                uint currentId = idMarketItem[i].marketId;
