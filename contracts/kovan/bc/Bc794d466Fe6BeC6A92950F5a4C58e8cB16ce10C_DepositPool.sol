...
contract DepositPool is Initializable, UUPSUpgradeable, ERC721HolderUpgradeable, ERC20Upgradeable {
    ...
    function initialize() public initializer {
        ...
        dao = msg.sender;
        assets.push(0xa36085F69e2889c224210F603D836748e7dC0088);
    }
    ...
    function isAssetPriceSupported(address erc20) public view returns (bool) {
    ...
    function currentPrice() public view returns (uint256) {
        ...
        for (uint256 i = 0; i < assets.length; i++) {
            (, int price,,,) = registry.latestRoundData(assets[i], Denominations.ETH);
            if (price > 0) {
                total += uint256(price) * IERC20Metadata(assets[i]).balanceOf(address(this));
            }
        }
        return total;
    }
