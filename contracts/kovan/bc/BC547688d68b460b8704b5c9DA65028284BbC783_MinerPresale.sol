...
contract MinerPresale is Ownable {
    ...
    function buyMiner(uint256 _amount) external isWhitelisted(msg.sender) {
        require(maxSupply >= soldTokens + _amount, "Max Limit Reached!");

        if (userContribution[msg.sender] == 0) {
            totalParticipants += 1;
        }
        uint256 _usdtPrice = (_amount * tokenPrice) / 10**miner.decimals();
        usdt.transferFrom(msg.sender, treasury, _usdtPrice);
        miner.transferFrom(owner(), msg.sender, _amount);
    ...
    function updateUSDT(address _token) external onlyOwner {
        usdt = IERC20(_token);
    }
