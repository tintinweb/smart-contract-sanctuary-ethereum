...
contract MarketPlaceScientist is IAdmin, Ownable, IMarketPlaceScientist {
        ...
        override
    {
        ...
        require(index != type(uint256).max, "Token is not on sale");
    ...
    {
        address[] memory scientistsOwners = new address[](
            sellingScientists.length
        );
        for (uint256 i = 0; i < sellingScientists.length; i++) {
            scientistsOwners[i] = IERC721(scientistToken).ownerOf(
                sellingScientists[i]
            );
        }
    ...
    function withdrawMDMATokens(address token, address to) external isAdmin {
