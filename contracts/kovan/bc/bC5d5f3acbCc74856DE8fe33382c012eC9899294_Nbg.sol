...
// the peg token
contract Nbg is ERC20Burnable, Operator {
    using SafeMath8 for uint8;
    using SafeMath for uint256;

    // Initial distribution for the first 24h genesis pools
    uint256 public constant INITIAL_GENESIS_POOL_DISTRIBUTION = 11000 ether;
    // Initial distribution for the day 2-5 NBG-WETH LP -> NBG pool
    uint256 public constant INITIAL_NBG_POOL_DISTRIBUTION = 140000 ether;
    ...
    /**
     * @notice Constructs the NBG ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("NBG", "NBG") {
    ...
    function _getNBGPrice() internal view returns (uint256 _NBGPrice) {
        try IOracle(NBGOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("NBG: failed to fetch NBG price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _NBGPrice) internal returns (uint256) {
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_NBGPrice >= taxTiersTwaps[tierId]) {
    ...
    function setNBGOracle(address _NBGOracle) public onlyOperatorOrTaxOffice {
        require(_NBGOracle != address(0), "oracle address cannot be 0 address");
        NBGOracle = _NBGOracle;
    }
    ...
    /**
     * @notice Operator mints NBG to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of NBG to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentNBGPrice = _getNBGPrice();
            currentTaxRate = _updateTaxRate(currentNBGPrice);
            if (currentNBGPrice < burnThreshold) {
                burnTax = true;
            }
        }
    ...
    function distributeReward(
        ...
        require(_NBGPool != address(0), "!_NBGPool");
        require(_airdropWallet != address(0), "!_airdropWallet");
        rewardPoolDistributed = true;
        _mint(_genesisPool, INITIAL_GENESIS_POOL_DISTRIBUTION);
        _mint(_NBGPool, INITIAL_NBG_POOL_DISTRIBUTION);
