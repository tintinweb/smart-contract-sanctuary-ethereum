...
contract PlayerStoxToken is Context, IERC20, Ownable {
     ...
     */
    constructor (string memory name_, string memory symbol_, uint8 decimals_, address cricStoxMasterAddress_, address adminWallet_) {
        _name = name_;
        _symbol = symbol_;
        _decimals = decimals_;
...
contract CricStoxFactory is Ownable {
    ...
    /**
        ...
        */
    function createPlayerStox(string memory name_, string memory symbol_, uint8 decimals_, address[] memory pools_, uint256[] memory poolShares_) external onlyOwner {
        require(pools_.length == poolShares_.length);
        PlayerStoxToken playerStox = new PlayerStoxToken(name_, symbol_, decimals_, cricStoxMasterAddress, adminWallet);
        ...
        if (pools_.length > 0) {
            ...
            uint256 quote = ICricStoxMaster(cricStoxMasterAddress).quote(address(playerStox), totalShares);
            uint256 amount = totalShares.mul(quote);
            if (amount > 0) {
                bool success = ICricStoxMaster(cricStoxMasterAddress).buyExactStoxForTokens(address(playerStox), baseCurrency, amount, totalShares);
