...
contract OpenStarterSaleFactory {
    ...
    function createSale(
        ...
        sale.setStringInfo(
            _saleInfo.projectName,
            _saleInfo.artwork,
            _saleInfo.kycInformation,
            _saleInfo.description,
            _saleInfo.projectStage,
            _saleInfo.cover
        );

        sale.setCopyright(_saleInfo.copyright, _saleInfo.utility);
        ...
        try IOpenStarterNft(_nft).setMinter(address(sale)) {
            emit SaleCreated(saleId, _nft, msg.sender, true);
        } catch Error(string memory reason) {
            emit SaleCreated(saleId, _nft, msg.sender, false);
