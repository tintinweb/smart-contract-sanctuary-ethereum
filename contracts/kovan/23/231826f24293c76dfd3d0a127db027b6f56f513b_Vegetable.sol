...
contract Vegetable is Ownable{
    ...
    function hatchEggs() public{
        ...
        for(uint i=0;i<5;i++){
            ...
            emit referralAwardLog(msg.sender,refer,realEggsUsed,i);
    ...
    function buyEggs(uint256 amount , address ref) public{
        ...
        for(uint i=0;i<5;i++){
            ...
            TransferHelper.safeTransfer(cake,refer,SafeMath.div(SafeMath.mul(amount,rebate[i]),100));
            emit referralAwardLog(msg.sender,refer,SafeMath.div(SafeMath.mul(amount,rebate[i]),100),i);
            refer = referrals[refer];
