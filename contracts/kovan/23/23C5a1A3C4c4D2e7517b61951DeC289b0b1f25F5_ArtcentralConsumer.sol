...
/**
 * @title A consumer contract for Artcentral API.
 * @author LinkPool.
 * @dev Uses @chainlink/contracts 0.4.0.
 */
contract ArtcentralConsumer is ChainlinkClient {
    ...
    function fulfillTami(bytes32 _requestId, uint256 _price) external recordChainlinkFulfillment(_requestId) {
        requestIdResult[_requestId] = _price;
    }

    /**
     * @notice Returns the TAMI in WEI for an NFT collection.
     * @dev Result format is a uint256.
     * @param _specId the jobID.
     * @param _payment the LINK amount in Juels (i.e. 10^18 aka 1 LINK).
     * @param _assetAddress the collections address for which we want to request the TAMI.
     */
    function requestTami(
        ...
        Chainlink.Request memory req = buildOperatorRequest(_specId, this.fulfillTami.selector);
