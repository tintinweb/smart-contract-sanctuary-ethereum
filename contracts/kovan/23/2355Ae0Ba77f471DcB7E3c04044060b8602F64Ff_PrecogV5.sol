...
contract PrecogV5 {
    ...
    mapping(address => mapping(address => Investment)) public investmentOf;  ///// investmentOf[token][user] = Investment()
    mapping(address => mapping(address => uint256)) profitOf; /////
    mapping(address => Cycle[]) profitCycles; //

    mapping(address => uint16) public currentInvestmentCycleId;
    mapping(address => uint16) public currentWithdrawalCycleId;
    mapping(address => uint16) public currentProfitCycleId;

    mapping(address => uint256[]) totalInvestmentUnits; /////
    address[] public existingTokens;
    mapping(address => bool) public isExistingToken;
    mapping(address => address) public tokenConvert;
    mapping(address => uint256) public liquidity;
    mapping(address => uint256[]) public profit;
    mapping(address => mapping(address => uint256)) public requestedWithdrawals; // requestedWithdrawals[token][user] = 3000
    mapping(address => uint256) availableWithdrawals; // for calculating actual (USDC, USDT, DAI, ...) balance of Precog
    mapping(address => uint256) totalRequestedWithdrawal; // totalRequestedWithdrawal[token] = 1000000000
    ...
    function isLiquidityToken(address liqudityToken) public view returns (bool) {
    ...
    function getTotalInvestmentUnits(address token) public view returns (uint256[] memory) {
        return totalInvestmentUnits[token];
    }

    function availableWithdrawal(address token, address user) public view returns (uint256) {
    ...
    function setMiddlewareExchange(IMiddlewareExchange newMiddlewareExchange) external onlyAdmin {
    ...
    function addLiquidityPool(address token) external onlyAdmin {
        ...
        // Deploy new IPCOG
        address liquidityToken = factory.create(IERC20(token).decimals());
    ...
    function takeInvestment(address token) public onlyMiddleware {
    ...
    function takeInvestmentsOfAllPools() external onlyMiddleware {
        for(uint256 i = 0; i < existingTokens.length; i++) {
            if(liquidity[existingTokens[i]] == 0) continue;
            takeInvestment(existingTokens[i]);
        }
    }

    // for function "buyPCOG"
    

    function sendProfit(ProfitInfo memory profitInfo, uint256 deadline) public onlyMiddleware {
    ...
    function sendMultipleProfits(ProfitInfo[] memory profitsInfo, uint256 deadline) external onlyMiddleware {
        for(uint256 i = 0; i < profitsInfo.length; i++) {
            if(profitsInfo[i].amount == 0) continue;
            sendProfit(profitsInfo[i], deadline);
        }
    }

    function sendWithdrawalRequestTokens(address token) public onlyMiddleware {
        require(isExistingToken[token], "PrecogV5: TOKEN_WAS_NOT_ADDED_TO_POOL");
    ...
    function sendAllWithdrawalRequestTokens(address[] memory tokens) external onlyMiddleware {
        for(uint256 i = 0; i < tokens.length; i++) {
            if(totalRequestedWithdrawal[tokenConvert[tokens[i]]] > 0) continue;
            sendWithdrawalRequestTokens(tokens[i]);
        }
    }
    ...
    function isInCurrentCycle(address token) public view returns (bool) {
    ...
    function _decreaseInvestment(address token, address account, uint256 amount) internal {
        ...
        if (investment.lastProfitCycleId < updatedLatestCycle) {
            ...
            // Update investment of user
            if (profitCycles[token][updatedLatestCycle].endTime > uint48(block.timestamp)){
                updatedUnit = investment.amount * (_profitCycles[updatedLatestCycle].endTime - _profitCycles[updatedLatestCycle].startTime) - unit;
            } else {
                updatedUnit = investment.amount * core.getCycleConfiguration().profitCycle - unit;
            }
    ...
    function deposit(address token, uint256 amount) external {
        ...
        IERC20(liquidityToken).mint(msg.sender, actualDepositAmount);
    ...
    function withdraw(address to, address token, uint256 amount) external {
        ...
        IERC20(tokenConvert[token]).burnFrom(msg.sender, amount);
...
interface IPrecogCore {
    ...
    function feeDecimalBase() external returns (uint8);
}
