...
contract AirdropSubscription is Ownable, KeeperCompatibleInterface {
  ...
  function getClaimableTokens(address _subscriber, uint256 _airdropIndex) public view returns (uint256) {
    ...
    if (airdrop.remainingSupply == 0) return 0; // airdrop has been cleaned up
  ...
  function createAirdrop(address _token, uint256 _initialSupply, uint256 _start, uint256 _end) external {
    ...
    require(_initialSupply > 0, "Invalid initialSupply");
  ...
  function cleanupAirdrop(uint256 _airdropIndex) external {
    ...
    airdrops[_airdropIndex].remainingSupply = 0;
  ...
  function getAirdrops() public view returns (
    ...
    uint[] memory initialSupplies,
    uint[] memory remainingSupplies,
    uint[] memory starts,
    uint[] memory ends
  ) {
    ...
    initialSupplies = new uint[](airdrops.length);
    remainingSupplies = new uint[](airdrops.length);
    ...
    for (uint i; i < airdrops.length; i++) {
      ...
      initialSupplies[i] = airdrops[i].initialSupply;
      remainingSupplies[i] = airdrops[i].remainingSupply;
