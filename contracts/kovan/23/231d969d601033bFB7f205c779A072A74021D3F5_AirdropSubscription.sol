...
contract AirdropSubscription is Ownable {
  ...
  constructor(
    ...
    // Store weth to usdToken path for reuse
    wethUsdPath = new address[](2);
    wethUsdPath[0] = uniswapV2Router.WETH();
    wethUsdPath[1] = address(usdToken);
  }
  ...
  function subscribeBNB(address _subscriber, uint8 _tier, address _discountToken) public payable {
    require(1 <= _tier && _tier <= 3);
    require(!isSubscribed[_subscriber]);
    require(msg.value >= getPaymentBNB(_subscriber, _tier, _discountToken));

    // Pay in BNB
    uniswapV2Router.swapExactETHForTokens{value:msg.value}(
      getPaymentBUSD(_subscriber, _tier, _discountToken),
      wethUsdPath,
      address(this),
      block.timestamp+10
    );
  ...
  function subscribeBUSD(address _subscriber, uint8 _tier, address _discountToken, uint256 _payment) public {
    require(1 <= _tier && _tier <= 3);
    require(!isSubscribed[_subscriber]);
    require(_payment >= getPaymentBUSD(_subscriber, _tier, _discountToken));

    // Pay in BUSD
    usdToken.transferFrom(msg.sender, address(this), _payment);
    _subscribe(_subscriber, _tier);
  }

  function getPaymentBNB(address _subscriber, uint8 _tier, address _discountToken) public view returns (uint256) {
    uint256 amountOut = getPaymentBUSD(_subscriber, _tier, _discountToken);
    uint256 amountInMin = uniswapV2Router.getAmountsIn(amountOut, wethUsdPath)[0];
  ...
  function getPaymentBUSD(address _subscriber, uint8 _tier, address _discountToken) public view returns (uint256) {
    require(1 <= _tier && _tier <= 3);
    uint256 payment;
    if (_tier == 1) payment = basePrice * 100 / 100; // 1 x basePrice w/ 0% discount
    if (_tier == 2) payment = basePrice * 180 / 100; // 2 x basePrice w/ 10% discount
    if (_tier == 3) payment = basePrice * 240 / 100; // 3 x basePrice w/ 20% discount
    if (_discountToken != address(0)) {
      ...
      address[] memory discountWethPath = new address[](2);
      discountWethPath[0] = discountToken.token;
      discountWethPath[1] = uniswapV2Router.WETH();
      uint256 balance = IERC20(discountToken.token).balanceOf(_subscriber);
      uint256 wethOut = uniswapV2Router.getAmountsOut(balance, discountWethPath)[1];
      if (wethOut >= discountToken.minWethValue)
  ...
  function payoutBUSD() external {
    uint256 usdTokenAmount = usdToken.balanceOf(address(this));
    usdToken.transfer(lead, (usdTokenAmount*70)/100);
    usdToken.transfer(dev1, (usdTokenAmount*15)/100);
    usdToken.transfer(dev2, (usdTokenAmount*15)/100);
  }
