...
{   
    ...
    function readBatchMetadata(
        ...
        string[][] memory Metadata = new string[][](ContractAddresses.length);
        for(uint ProjectID; ProjectID < TokenIDs.length; ProjectID++)
        {
            for(uint TokenID; TokenID < TokenIDs[ProjectID].length; TokenID++)
            {
                Metadata[ProjectID][TokenID] = ERC721(ContractAddresses[ProjectID]).tokenURI(
                    TokenIDs[ProjectID][TokenID]
                );
            }
        }
        return Metadata;
