...
contract ZooNftFaucet is ERC721Burnable, Ownable {
	...
	function mintNft(string memory imageUri) external returns (uint256 newId)
	{
	...
	function multiMint(address recipient, string memory batchUri) external
	{
		require(whiteList[msg.sender] == true, "not whitelisted");
	...
	function batchMint(address[] calldata recipients, uint256 nftAmount) external onlyOwner
	{
