    ...
    UUPSUpgradeable
{
    ...
    /**
     * proposalIDs : ProposalId is a counter to track the proposals
     * stakingAddress:To store the address of staking smartContract and used its functions.

     * artistList: artistList is an structure of addresses for storing multiple unique artist
     * userProposer: UserProposer is a struct to store details of proposal applied by user.

     * adminResponse: enum adminResponse to set multiple responses
     
     * isArtist:a modifier to check that caller should belong to artist category 
     * isUser:a modifier to check that caller should belong to user category 
    ...
    modifier isArtist() {
        require(
            artistExist(msg.sender),
            "Artist can create proposal"
        );
        _;
    }
    ...
    /**
     * @dev Initializes the contract by setting a '_token` ,'_timelock and '_stakingContract' to the
      GovernorVotes, GovernorTimelockControl and staking_address and set a owner
     */
    function initialize(
        ...
        require(
            ...
            "initializing with zero address"
        );
     ...
     */
    function addOrRemoveArtist(address _artistAddress, bool _status)
        public
        onlyOwner
    {
    ...
    function acceptOrRejectProposal(uint256 _proposalID, bool _status) public {
        ...
        if (_status) {
            ...
            _userProposer.status = adminResponse.Approved;
        } else {
            _userProposer.status == adminResponse.Reject;
        }
    }
     ...
     * Overrided function from IGovernorUpgradeable and GovernorSettingsUpgradeable
     */
     ...
     * Overrided function from IGovernorUpgradeable and GovernorVotesQuorumFractionUpgradeable
     */
     ...
     * Overrided function from IGovernorUpgradeable and GovernorVotesUpgradeable
     */
     ...
     * Overrided function from IGovernorUpgradeable , GovernorTimelockControlUpgradeable and GovernorUpgradeable
     */
    ...
    function userProposal(
        ...
        require(
            IStaking(stakingAddress).hasStaked(msg.sender) &&
                artistExist(_proposer),
            "TokenId not staked by caller or invalid proposer"
        );
        proposalIDs.increment();
        userProposer storage _userProposer = proposerDetails[
            proposalIDs.current()
        ];
        ...
        _userProposer.status = adminResponse.Pending;
        return proposalIDs.current();
    }

    /**
     * @dev Create a new proposal for artist. Vote start {IGovernor-votingDelay} blocks after the proposal is created and ends
     * {IGovernor-votingPeriod} blocks after the voting starts.
     *
     * Emits a {ProposalCreated} event.
     *Overrided function from  GovernorCompatibilityBravoUpgradeable
     */
    function artistProposal(
        ...
        uint256 proposalId = GovernorCompatibilityBravoUpgradeable.propose(
            targets,
            values,
            signatures,
            calldatas,
     ...
     *
     * Emits a {ProposalCreated} event.
     *Overrided function from IGovernorUpgradeable, GovernorCompatibilityBravoUpgradeable and GovernorUpgradeable
     */
    function propose(
        address[] memory targets,
        uint256[] memory values,
     ...
     * Overrided function from GovernorUpgradeable and GovernorSettingsUpgradeable
     */
     ...
     *Override function from GovernorUpgradeable and GovernorTimelockControlUpgradeable
     */
    function _execute(
        uint256 proposalId,
        address[] memory targets,
     ...
     *Overrided function from GovernorUpgradeable and GovernorTimelockControlUpgradeable
     */
    function _cancel(
        address[] memory targets,
        uint256[] memory values,
     ...
     *Override function from GovernorUpgradeable,IERC165Upgradeable and GovernorTimelockControlUpgradeable
     */
