...
contract ChefIncentivesController is Ownable {
    ...
    function addPool(address _token, uint256 _allocPoint) external {
        require(msg.sender == poolConfigurator, 'msg.sender is not a pool configurator');
...
contract LendingPoolConfigurator is VersionedInitializable, ILendingPoolConfigurator {
  ...
  function addLiquidator(address liquidator) public onlyPoolAdmin {
    pool.addLiquidator(liquidator);
  }

  function removeLiquidator(address liquidator) public onlyPoolAdmin {
    pool.removeLiquidator(liquidator);
  }
  ...
  IAToken
{
   ...
   * @param aTokenName The name of the aToken
   * @param aTokenSymbol The symbol of the aToken
   * @param params Abi encoded (treasury and reservePercent)
   */
  function initialize(
    ILendingPool pool,
    address distributor,
    ...
    (_treasury, _reservePercent) = abi.decode(params, (address, uint16));
  ...
  function mintToTreasury(uint256 amount, uint256 index) external override onlyLendingPool {
    ...
    // Percentage is calculated based on two decimals mask, so 10000 = 100.00%
    uint256 splitAmount = amount.rayDiv(index).mul(_reservePercent).div(10000);

    // The part of the fees goes to treasury
    _mint(treasury, splitAmount);

    // The other part of the fees goes to distributor
    _mint(distributor, splitAmount);
  ...
  /**
   * @dev Returns the address of the Nereus treasury, receiving the part of the fees on this aToken
   **/
  ...
  /**
   * @dev Returns the percent of the fees in this aToken that goes to the distributor
   **/
  function RESERVE_PERCENT() public view returns (uint16) {
    return _reservePercent;
  }
...
contract LendingPool is VersionedInitializable, ILendingPool, LendingPoolStorage {
  ...
  function _onlyLiquidator() internal view {
    require(
      ...
      "Must be a liquidator"
    );
  }

  function addLiquidator(address liquidator) override public onlyLendingPoolConfigurator {
    liquidators[liquidator] = true;
  }

  function removeLiquidator(address liquidator) override public onlyLendingPoolConfigurator {
  ...
  function liquidationCall(
    ...
    uint256 debtToCover,
    bool receiveAToken
  ) external override whenNotPaused onlyLiquidator {
