...
contract AirdropRewarder is Initializable, OwnableUpgradeable {
    ...
    function claim(
        ...
        uint256[] calldata _totalClaimableAmount,
        bytes32[][] calldata _merkleProofs
    ) external {
        uint256 rewardTokenLength = rewardTokens.length;
        require(
            rewardTokenLength == _merkleProofs.length && rewardTokenLength == _totalClaimableAmount.length,
        ...
        for (uint256 i = 0; i < rewardTokenLength; i++) {
            ...
            emit RewardsClaimed(_user, rewardTokens, _totalClaimableAmount);
        }
    }
