...
contract MoonwalkersA is ERC721A, Pausable, Ownable, ReentrancyGuard {
  ...
  /// @dev Mapping of the amount of NFT minted by an elite address during the presale.
  ...
  constructor() ERC721A("Moonwalkers", "MW") {
    ...
    saleConfig.maxEliteMintPerWallet = 7;
    saleConfig.maxPublicMintPerWallet = 10;
    revealed = false;
    saleState = true;
    merkleRootWhitelist = 0x432eef6147ed482431347e58b9bf66304557b103e6c4ee0e4124dee4adf62484;
    merkleRootElite = 0x432eef6147ed482431347e58b9bf66304557b103e6c4ee0e4124dee4adf62484;
  }
  ...
  /// @dev Modifier used in the mint functions in order to avoid routed calls.
  ...
  /// @dev Presale mint function. Only the whitelisted and elite addresses are able to mint during the presale.
  function allowlistMint(uint256 _mintAmount, bytes32[] calldata _merkleProof) external payable whenNotLocked callerIsUser {
    SaleConfig memory config = saleConfig;
    require(block.timestamp > config.presaleStartTime, "Presale has not stared yet.");
    require(_totalMinted().add(_mintAmount) < maxSupply.add(1), "Max supply reached.");
  ...
  /// @dev Presale mint function for elite addresses. Only the whitelisted and elite addresses are able to mint during the presale.
  function eliteMint(uint256 _mintAmount, bytes32[] calldata _merkleProof) external payable whenNotLocked callerIsUser {
    SaleConfig memory config = saleConfig;
    require(block.timestamp > config.presaleStartTime, "Presale has not stared yet.");
    require(_totalMinted().add(_mintAmount) < maxSupply.add(1), "Max supply reached.");
    require(_mintAmount > 0 && _mintAmount < config.maxEliteMintPerWallet.add(1), "Max mint per tr. exceeded.");
    require(msg.value == config.presaleMintPrice.mul(_mintAmount), "Tr. value did not equal the mint price.");
    require(amountMintedWhitelist[msg.sender].add(_mintAmount) < config.maxEliteMintPerWallet.add(1), "Max presale mint per address exceeded.");
    bytes32 leaf = keccak256(abi.encodePacked(msg.sender));
    require(MerkleProof.verify(_merkleProof, merkleRootElite, leaf), "Invalid Merkle Proof.");
  ...
  /// @dev Sets the max presale mint per elite wallet allowed.
  function setMaxEliteMintPerWallet(uint256 _maxEliteMintPerWallet) external onlyOwner {
    saleConfig.maxEliteMintPerWallet = _maxEliteMintPerWallet;
  }
