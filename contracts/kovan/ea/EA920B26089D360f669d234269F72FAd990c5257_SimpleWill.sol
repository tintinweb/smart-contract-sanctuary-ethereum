...
contract SimpleWill is Ownable {
    ...
    function releaseERC20(address tokenERC20Address) external {
        require(block.timestamp >= releaseTime,  "Current time is before release time");
        IERC20 tokenERC20 = IERC20(tokenERC20Address);
        ...
        emit ReleasedERC20(owner(), beneficiary, tokenERC20Address);
    }


    function releaseERC721(address tokenERC721Address, uint256[] calldata tokenIdList) external {
        require(block.timestamp >= releaseTime,  "Current time is before release time");
        IERC721 tokenERC721 = IERC721(tokenERC721Address);
        for (uint i = 0; i < tokenIdList.length; i++) {
            tokenERC721.safeTransferFrom(owner(), beneficiary, tokenIdList[i]);
            }
        emit ReleasedERC721(owner(), beneficiary, tokenERC721Address);
    }


    function releaseERC1155(address tokenERC1155Address, uint256[] calldata tokenIdList, uint256[] calldata value) external {
        require(block.timestamp >= releaseTime,  "Current time is before release time");
        IERC1155 tokenERC1155 = IERC1155(tokenERC1155Address);
        tokenERC1155.safeBatchTransferFrom(owner(), beneficiary, tokenIdList, value, bytes(''));
        emit ReleasedERC1155(owner(), beneficiary, tokenERC1155Address);
    }
}
