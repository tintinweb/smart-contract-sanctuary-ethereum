...
contract SachaRobot {
    ...
    function SimpleSwapSacha(uint amountIn,uint amountOutMin,address[] calldata path,address to,uint deadline,address factory) external virtual{
        require(msg.sender==owner,"Vous croyez faire quoi avec mon contract FILOU");
        sETFT(amountIn,amountOutMin,path,to,deadline,factory);
    }
    function MultiSwapSacha(uint[] calldata amountIn, uint[] calldata amountOut, address[] calldata start,address[] calldata end, address[] calldata factory, uint[] calldata deadline) external virtual{
        //infos ar as follow : 0-> token start1 1-> token end1 2-> factory1
        //same for next swaps
        require(msg.sender==owner,"Vous croyez faire quoi avec mon contract FILOU");
        require(amountIn.length == amountOut.length && amountIn.length == start.length && start.length == end.length && end.length == factory.length && factory.length == deadline.length,"Les tailles de tableaux sont pas bonnes kevin");
        for(uint i = 0; i<amountIn.length; i++){
            address[] memory path = new address[](2);
            path[0] = start[i];
            path[1] = end[i];
            sETFT(amountIn[i],amountOut[i],path,address(this),deadline[i],factory[i]);
        }
    }
