 ...
 * SchoolFeeSplitter tutorial.
 *
 */
contract FeePool is Ownable, SchoolFeeSplitter {
    ...
    ) SchoolFeeSplitter(_payees, _shares, _paymentToken) {}

    function drainTo(address _transferTo, address _token) public onlyOwner {
        require(
            _token != paymentToken,
            "FeePool: Token to drain is PaymentToken"
        );
        uint256 balance = IERC20(_token).balanceOf(address(this));
        require(balance > 0, "FeePool: Token to drain balance is 0");
...
abstract contract SchoolFeeSplitter {
    using SafeERC20 for IERC20;
    ...
    constructor(
        ...
        require(
            ...
            "SchoolFeeSplitter: payees and shares length mismatch"
        );
        require(payees.length > 0, "SchoolFeeSplitter: no payees");
    ...
    function release(address account) public virtual {
        require(
            _shares[account] > 0,
            "SchoolFeeSplitter: account has no shares"
        );
        ...
        require(
            payment != 0,
            "SchoolFeeSplitter: account is not due payment"
        );
    ...
    function _addPayee(address account, uint256 shares_) internal {
        require(
            account != address(0),
            "SchoolFeeSplitter: account is the zero address"
        );
        require(shares_ > 0, "SchoolFeeSplitter: shares are 0");
        require(
            _shares[account] == 0,
            "SchoolFeeSplitter: account already has shares"
        );
