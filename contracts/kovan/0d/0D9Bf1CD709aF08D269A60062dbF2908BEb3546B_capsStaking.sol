...
contract capsStaking is Ownable {
    ...
    struct User {
        ...
        mapping(uint => uint) distributedRewardPerBet;
    ...
    uint private availableReward;
    uint maxDataFetchSizeLimit = 20;
    ...
    function stake(uint amount) external returns (bool) {
        ...
        tempUserData.stackedAt[curStakCount] = curTime + 0;
        if (tempUserData.claimFrom == 0) {
            tempUserData.claimFrom = 1;
            tempUserData.isActiveUser = true;
        }

        if (isActiveUser(msgSender) == false) {
            activeStakers.push(msgSender);
        }

        if (availableReward != 0) {
            distributeReward(availableReward);
        }
    ...
    function isActiveUser(address userAddress) public view returns (bool) {
        uint totalActiveUsers = activeStakers.length;
        for (uint i; i < totalActiveUsers; ) {
            if (userAddress == activeStakers[i]) {
                return true;
            }
            unchecked {
                i++;
    ...
    function distributeReward(uint rewardAmount) private returns (bool) {
        address[] memory _activeStackerData = activeStakers;
        uint totalActiveStackers = _activeStackerData.length;
        for (uint i = 0; i < totalActiveStackers; i++) {
            address curActiveUser = _activeStackerData[i];
            uint _userTotalStackedAmount = getUserTotalStackedAmount(
                curActiveUser
            );
            if (_userTotalStackedAmount != 0) {
                uint userRewardAmount = uint(
                    (((rewardAmount * (1e18)) / totalStakedAmount) *
                        (_userTotalStackedAmount)) / (1e18)
                );
                usersData[curActiveUser].distributedRewardPerBet[
                        usersData[curActiveUser].totalStakes
                    ] += userRewardAmount;
            }
        }
        availableReward = 0;
    ...
    function claimRewards() external {
        ...
        User storage tempUserData = usersData[msgSender];
        (
            uint _claimableDistributedReward,
    ...
    function withdrawReward(uint amount) external {
        address msgSender = msg.sender;
        require(
            amount <= usersData[msgSender].totalReward,
            "Entered Amount Is More Than Your Balance"
        );
        usersData[msgSender].totalReward -= amount;
    ...
    function notifyReward() external payable {
        ...
        if (msgValue > 0) {
            availableReward = msgValue;
            if (activeStakers.length != 0 && totalStakedAmount != 0) {
                distributeReward(msgValue);
            }
        }
    }
    ...
    function getActiveStacker(uint index) external view returns (address) {
        return activeStakers[index];
    }

    function getActiveStackers_Arr(uint from, uint to)
        external
        view
        returns (address[] memory)
    {
        require(
            (to - from) <= maxDataFetchSizeLimit,
            "You Cannot Fetch Day More Than Maximum Size"
        );
        require(
            activeStakers.length > to,
            "Index Out Of Bound Or To Should Less Than Array Size"
        );
        address[] memory activeStackersData = new address[](to - from);
        uint256 j;
        for (uint256 i = from; i < to; ) {
            activeStackersData[i] = activeStakers[i];
            unchecked {
                i++;
                j++;
            }
        }
        return activeStackersData;
    }
        ...
        )
    {
        ...
        if (tempUserData.totalStakes > 0) {
            for (
                ...
                if (tempUserData.stackedAt[i] < block.timestamp) {
                    withdrawableReward += tempUserData.distributedRewardPerBet[
                        i
                    ];
        ...
        returns (uint)
    {
        ...
        for (
            ...
            _totalReward += tempUserData.distributedRewardPerBet[i];
            unchecked {
                i++;
            }
        }
