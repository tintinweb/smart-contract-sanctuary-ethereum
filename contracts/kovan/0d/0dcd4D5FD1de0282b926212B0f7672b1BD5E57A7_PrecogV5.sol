...
contract PrecogV5 is IPrecogV5 {
    ...
    modifier _isExistingToken(address token) {
        require(isExistingToken[token], "3");
        _;
    }
    ...
    function addLiquidityPool(address token) external onlyAdmin {
        ...
        require(tokenConvert[token] == address(0) && token != core.PCOG() && token != address(0), "5");
        // Deploy new IPCOG
        ...
        if (!isNotFirstInvestmentCycle[token]) {
            ...
            cyclesChangedInfo[token].fundingDuration = core.getCycleConfiguration().fundingCycle;
            cyclesChangedInfo[token].defundingDuration = core.getCycleConfiguration().defundingCycle;
    ...
    function removeLiquidityPool(address token) external onlyAdmin _isExistingToken(token) {
        ...
        require(IERC20(liquidityToken).totalSupply() == 0, "6");
    ...
    function sendProfit(address token, uint profitAmount) external override onlyMiddleware _isExistingToken(token) {
        ...
        require(_currentProfitId < tradingCycles[token][lastIndexProfitCycle].id, "7");
    ...
    function _updateAdjustment(address _token) internal {
        ...
        }
        {
            ...
            uint48 _nextDefundingTime = PrecogV5Library._nextDefundingTime(_defundingApplyTime, _defundingDuration, 0);
    ...
    function _decreaseInvestment(
        ...
        require(!isBeforeFundingTime(_token, _account), "8");
        Investment[] memory _investmentsOf = investmentsOf[_token][_account];
        uint48 _nextDefundingTime;
        Cycle memory _futureTradingCycle;
        {
            uint48 _duration = isFirstRequestWithdrawal ? core.getCycleConfiguration().firstDefundingCycle : 0;
            CyclesChangedInfo memory _cycleChangedInfo = cyclesChangedInfo[_token];
        ...
        ) 
    {
        ...
        lastFundingStartTime = nextFundingStartTime - cycleChangedInfo.fundingDuration;
    }

    function deposit(address token, uint amount) external override {
        require(amount >= core.minFunding(token), "9");
        _updateCurrentTradingCycle(token);
    ...
    function _registerWithdrawal(address _token, address _account, uint _amount, bool _isFirstRequestWithdrawal) internal {
        ...
        uint48 _locktime = _nextDefundingTime - uint48(block.timestamp);
        withdrawalRegister.registerWithdrawal(_token, _account, _amount, _locktime);
        if(_isFirstRequestWithdrawal == true) {
            accountTradingInfo[_token][_account].isNotFirstRequestWithdrawal = true;
        }
    }

    function requestWithdrawal(address token, uint amount) external override {
        require(amount > core.minDefunding(token), "10");
        _updateCurrentTradingCycle(token);
        ...
        require(remainingAmount == 0 || remainingAmount >= core.minFunding(token), "11");
