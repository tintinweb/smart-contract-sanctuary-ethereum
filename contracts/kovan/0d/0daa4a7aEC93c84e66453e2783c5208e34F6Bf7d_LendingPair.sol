...
contract LendingPair is IBSLendingPair, Exponential, Initializable {
        ...
        whenNotPaused(Actions.Deposit)
    {
        ...
        // mintTokens = vaultShareAmount / exchangeRate
        uint256 mintTokens =
    ...
    function borrow(uint256 _amountToBorrow, address _debtOwner)
        public
    ...
    function repay(uint256 _repayAmount, address _beneficiary) public {
        ...
        require(vault.balanceOf(asset, msg.sender) >= repayAmountInShares, "NOT_ENOUGH_BALANCE");
        ...
        emit Repay(address(this), address(asset), _beneficiary, msg.sender, repayAmount);
    }
    ...
    function redeem(address _to, uint256 _amount)
        public
        override
        returns (uint256 amountOfSharesToRedeem)
    {
        ...
        require(
            vault.balanceOf(asset, address(this)) >= amountOfSharesToRedeem,
            "NOT_ENOUGH_BALANCE"
        );
    ...
    function warp(uint8[] calldata actions, bytes[] calldata data)
        external
    ...
    function select(int256 paramInput, uint256 returnInput) internal pure returns (uint256 value) {
        value = paramInput >= 0 ? uint256(paramInput) : (paramInput == USE_RETURN_INPUT)
            ? returnInput
            : uint256(paramInput);
    }
    ...
    **/
    ...
    function withdrawFees(uint256 _toWithdraw) external override {
        ...
        uint256 shareAmount = vault.toShare(asset, _toWithdraw, false);
    ...
    function getMaxWithdrawAllowed(address _account)
        public
        override
        returns (uint256 maxWithdrawAllowed)
    {
        // save on sload
        ...
        maxWithdrawAllowed = vault.toShare(
            collateralAsset,
            leftoverCollateral / currentCollateralValueInUSD,
            false
        );
    }
    ...
    function borrowRatePerBlock() external view returns (uint256) {
        return interestRate.getBorrowRate(getCashPrior(), debtToken.totalSupply(), totalReserves);
    }
