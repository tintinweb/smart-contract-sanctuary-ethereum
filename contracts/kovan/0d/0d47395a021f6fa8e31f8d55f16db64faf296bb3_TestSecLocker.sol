...
contract TestSecLocker is Owned {
    ...
    event BalanceCheck(uint256 _testSecAmount, uint256 balance);
    event AllowanceCheck(uint256 _testSecAmount, uint256 allowance);
    event ApprovalCheck(address _holder, uint256 _testSecAmount, uint256 testSecBalance, uint256 allowance, uint256 allowanceCurrentWallet);
    ...
    function checkApproval(address _holder, uint256 _testSecAmount) onlyOwner public{
        uint256 testSecBalance = uint256(_testSecAmount.div(TRUNCATE_SCALE));
        _testSecAmount = uint256(testSecBalance.mul(TRUNCATE_SCALE));  

        uint256 balance = testSecToken.balanceOf(_holder);
        emit BalanceCheck(_testSecAmount, balance);

        uint256 allowance = testSecToken.allowance(_holder, address(this));
        emit AllowanceCheck(_testSecAmount, allowance);

        uint256 allowanceCurrentWallet = testSecToken.allowance(msg.sender, _holder);

        require(testSecToken.approve(address(this),_testSecAmount),"Unable toapprove spender");
        require(testSecToken.increaseAllowance(address(this), _testSecAmount),"Unable to increase allowance of spender");
        allowance = testSecToken.allowance(_holder, address(this));
        emit ApprovalCheck(_holder, _testSecAmount, testSecBalance, allowance, allowanceCurrentWallet);
