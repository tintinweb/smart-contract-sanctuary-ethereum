...
{   
    ...
    address public _ERC20_BRT_TokenAddress = 0xaa6e5Db9977dB33d33f7351b80F2DF6B3979d603;             // BRT ERC-20 Contract Address   
    ...
    address public _GoldenTokenAddress = 0x4e15845FC4B9C6eC9A2774ebD483331d14d7A6c0;                 // Golden Token Contract Address KOVAN
    ...
    mapping(uint => mapping(uint => uint)) public SelectedTimeSlotArtistMintPass;           // Returns Selected TimeSlot Corresponding To Artist Mint Pass
     ...
     **/
    event LiveMintComplete(address indexed Redeemer, uint TicketID, uint CitizenTokenID);

    /**
     * @dev Emitted When `Redeemer` IRL-Mints Artist Corresponding To `ArtistID`
     */
    event LiveMintCompleteArtist(address indexed Redeemer, uint TicketID, uint ArtistID);

    /**
     * @dev Emitted When `Redeemer` IRL-Mints ArtBlocks Artist Corresponding To `ArtistID`
     */
    event LiveMintCompleteArtistArtBlocks(address indexed Redeemer, uint ArtistID, uint TicketID, uint TokenID);

    /**
     * @dev Emitted When `Redeemer` Redeems Golden Token Corresponding To `TicketID` 
     **/
    event GoldenTokenRedeemed(address indexed Redeemer, uint TicketID, uint Slot);

    /**
     * @dev Emitted When `Redeemer` Redeems Artist Mint Pass Corresponding To `TicketID`
     */
    event ArtistMintPassRedeemed(address indexed Redeemer, uint  ArtistID, uint TicketID, uint Slot);
    ...
    { 
        ...
        Role[msg.sender] = _MINTER_ROLE; //testnet
     ...
     **/
    function RedeemGT(uint TicketID, uint TimeSlot) public nonReentrant whenNotPaused
    {
        ...
        emit GoldenTokenRedeemed(msg.sender, TicketID, TimeSlot);
    }

    /**
     * @dev Redeems Artist Mint Pass & BrightLists Address To Receive Artist Mint
     **/
    function RedeemArtistPass(uint ArtistID, uint TicketID, uint TimeSlot) public nonReentrant whenNotPaused
    {
        require(BookingsAvailable[_CurrentCityIndex][TimeSlot] > 0, "TimeSlot Fully Booked");
        ...
        BookingsAvailable[_CurrentCityIndex][TimeSlot]--;
        SelectedTimeSlotArtistMintPass[_CurrentCityIndex][TimeSlot] = TimeSlot;
        emit ArtistMintPassRedeemed(msg.sender, ArtistID, TicketID, TimeSlot);
    }
    ...
    {
        address to = readBrightListCitizen(TicketID);
        require(to != address(0), "LiveMint: Golden Token Entered Is Not Brightlisted");
        ...
        emit LiveMintComplete(to, TicketID, CitizenTokenID);
    }
    ...
    function _LiveMintArtist(uint ArtistID, uint TicketID) public onlyMinter whenNotPaused 
    {
        ...
        if(ArtBlocksMint)
        {
            uint TokenID = IArtBlocks(ArtistContractAddress).purchase(ArtistID);
            IERC721(_ArtBlocksCoreContractAddress).transferFrom(address(this), Recipient, TokenID);
            emit LiveMintCompleteArtistArtBlocks(Recipient, ArtistID, TicketID, TokenID);
        }
        else 
        { 
            IRandomEdition(ArtistContractAddress)._Mint(Recipient, 1); 
            emit LiveMintCompleteArtist(Recipient, ArtistIndex[_CurrentCityIndex][ArtistID], ArtistID);
        }
     ...
     **/
    function __approveERC20(address contractAddress, uint amount) external onlyOwner 
    { 
     ...
     **/
    function readBrightListCitizen(uint TicketID) public view returns(address) { return BrightListCitizen[_CurrentCityIndex][TicketID]; }

    /**
     * @dev Returns BrightListed Address Corresponding to Mint Pass `ArtistID` & `TicketID`
     */
    function readBrightListArtist(uint ArtistID, uint TicketID) public view returns(address) { return BrightListArtist[_CurrentCityIndex][ArtistID][TicketID]; }
     ...
     */
    function resolveArtistMint(uint ArtistID) public view returns(bool) { return MintType[_CurrentCityIndex][ArtistID]; }

    /**
     * @dev Returns Booked TimeSlot For Golden Ticket Corresponding `TicketID`
     */
    function readTimeSlotGoldenTicket(uint TicketID) public view returns(uint) { return SelectedTimeSlotGoldenTicket[_CurrentCityIndex][TicketID]; }

    /**
     * @dev Returns Booked TimeSlot For Artist Mint Pass Corresponding To `TicketID`
     */
    function readTimeSlotArtistMintPass(uint TicketID) public view returns(uint) { return SelectedTimeSlotArtistMintPass[_CurrentCityIndex][TicketID]; }
