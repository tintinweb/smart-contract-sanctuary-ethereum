...
contract Yogies is ERC721Y, Ownable {
    ...
    /** === Staking interface === */
    function stakeYogie(uint256 yogie) external onlyOperator {
        _stakeYogie(yogie);
    }

    function unstakeYogie(uint256 yogie) external onlyOperator {
        _unstakeYogie(yogie);
    }

    /** === Minting === */
    function freeMint(bytes32[] calldata proof) external onlyOperator {
    ...
    function mintTo(uint256 amount, address _to) external onlyOwner {
    ...
    function lockYogiesInVault() external onlyOwner {
        ...
        _safeMint(address(this), left);
    }

    function unlockYogieFromVault() external {
        require(vaultOpened, "Vault closed");
        require(getYogiesLeftInVault() > 0, "Vault empty");
        require(gemmies.getEcoSystemBalance(msg.sender) >= vaultPriceYogie, "Not enough gemmies");

        vaultReleasedCounter++;
        gemmies.spendEcosystemBalance(vaultPriceYogie, msg.sender);
    ...
    function unlockVIYFromVault() external {
        ...
        require(gemmies.getEcoSystemBalance(msg.sender) >= vaultPriceVIY, "Not enough gemmies");

        viyReleaseCounter++;
        gemmies.spendEcosystemBalance(vaultPriceVIY, msg.sender);
