...
import "./ReforgeBridgeUpgradeable.sol";
import "./eEUR_AccessControlUpgradeable.sol";

/// @custom:security-contact [email protected]
contract TestContractBridgeVersion is
    Initializable,
    ERC20Upgradeable,
    ...
    ReforgeBridgeUpgradeable
{
    ...
    function initialize() public initializer {
        multiSigTreasury = 0xbE35035bbC968cFC28Cf7F6359d3223f78375445; //Dima's test acc

        __ERC20_init("TestContractBridgeVersion", "TCv2");
        __ERC20Burnable_init();
        __Pausable_init();
        __AccessControl_init();
        __UUPSUpgradeable_init();
        __WhitelistUpgradeable_init(multiSigTreasury); // add default account for whitelist
        __ReforgeBridgeUpgradeable_init(
            2000000000000000,
            payable(multiSigTreasury)
        ); // 0.002 ether, bridgeOwner by default
...
contract WhitelistUpgradeable is Initializable, AccessControlUpgradeable {
    ...
    /**
     * @dev Emitted when addToWhitelist is triggered for `account`.
     */
    event Whitelisted(address acoount);
    /**
     * @dev Emitted when removeFromWhitelist is triggered for `account`.
     */
    event Unwhitelisted(address account);

    function __WhitelistUpgradeable_init(address account)
        internal
        onlyInitializing
    {
        __WhitelistUpgradeable_init_unchained(account);
    }

    function __WhitelistUpgradeable_init_unchained(address account)
        internal
        onlyInitializing
    {
...
contract ReforgeBridgeUpgradeable is
    Initializable,
    ERC20Upgradeable,
    ERC20BurnableUpgradeable,
    AccessControlUpgradeable
{
    ...
    /**
     * @dev Bridge transactions for prevent recurrence of the transaction
     */
    mapping(string => bool) private _transactions;
    /**
    ...
    function __ReforgeBridgeUpgradeable_init(
        uint256 fee,
        address payable bridgeOwner
    ) internal onlyInitializing {
        __ReforgeBridgeUpgradeable_init_unchained(fee, bridgeOwner);
    }

    function __ReforgeBridgeUpgradeable_init_unchained(
        uint256 fee,
        address payable bridgeOwner
    ) internal onlyInitializing {
        _bridgePaused = false;
     ...
     */
    function setBridgeOwner(address payable account)
        public
        onlyRole(ADMIN_eEUR)
    {
     ...
     */
    function sendToChain(uint256 amount, uint256 destinationChainId)
        public
        payable
    {
        require(_bridgePaused == false, "Bridge paused");
 ...
 *
 * The admin role for all roles is `ADMIN_eEUR`, which means
 * that only accounts with this role will be able to grant or revoke other
 * roles. More complex role relationships can be created by using
 * {_setRoleAdmin}.
 *
 * WARNING: The `ADMIN_eEUR` is also its own admin: it has permission to
...
abstract contract AccessControlUpgradeable is Initializable, ContextUpgradeable, IAccessControlUpgradeable, ERC165Upgradeable {
    ...
    bytes32 public constant ADMIN_eEUR = 0x00;
