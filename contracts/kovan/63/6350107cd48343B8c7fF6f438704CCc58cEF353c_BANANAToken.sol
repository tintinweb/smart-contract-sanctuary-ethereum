...
contract NODERewardManagement is Ownable {
    ...
    uint256 public DetaforDay = 60 * 10;

    address public token;

    uint256 public totalNodesCreated = 0;
    uint256 public maximumTotalNodes = 1000000;
    ...
    function setInitialLaunchTime() external onlyOwner {
        ...
        returns (uint256)
    {
        ...
        for (uint256 i = 0; i < nodesCount; i++) {
            if (block.timestamp - launchTime < 8400) {
                if ((block.timestamp - nodes[i].creationTime) < 4200) {
                    temp =
        ...
        returns (uint256)
    {
        ...
        if (block.timestamp - launchTime < 8400) {
            if ((block.timestamp - node.creationTime) < 4200) {
                rewardOfNode =
        ...
        if ((block.timestamp - node.creationTime) < 4200) {
            rewardOfNode =
                ((node.rewardAvailable +
                    ...
                    (1000 * DetaforDay)) * 70) /
                100;
        } else if ((block.timestamp - node.creationTime) < 8400) {
            rewardOfNode =
                ((node.rewardAvailable +
                    ((block.timestamp - node.lastClaimedTime) *
                        node.tokenValue *
