...
contract RewardDistribution is IRewardDistribution, Ownable, GovernancePool, KeeperCompatibleInterface  {
    ...
    constructor(
     ...
     function geTestArray()
        external
        view
        returns (uint256[] memory _testArray)
    {
        uint256 _testArrayLength = test.length;
        _testArray = new uint256[](_testArrayLength);
        for (uint256 i = 0; i < _testArrayLength; i++) {
            _testArray[i] = test[i];
        }
        return _testArray;
    }
