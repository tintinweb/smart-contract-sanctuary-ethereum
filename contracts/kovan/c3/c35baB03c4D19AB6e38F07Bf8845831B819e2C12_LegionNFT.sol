...
contract LegionNFT is Context, ILegionNFT, Ownable {
    ...
    function mint(
        string memory name,
        uint256[] memory beastIds,
        uint256[] memory warriorIds
    ) external {
        require(
            bytes(name).length > 0,
            "mint:  you should mint with legion name"
        );
        require(
            beastIds.length <= maxBeasts,
            "mint: you should not overflow max beast number"
        );
        ...
        uint256 bloodstoneAmount = itemPrice
            .mul(warriorCount + beastCount)
            .mul(10**18)
            .div(denominator);
        require(
            bloodstone.balanceOf(msg.sender) >= bloodstoneAmount,
            "Insufficient payment"
        );
        uint256 capacity = 0;
        for (uint256 i = 0; i < beastCount; i++) {
            require(
                beast.ownerOf(beastIds[i]) == msg.sender,
                "mint : you should own these beast tokens"
            );
        ...
        require(
            maxCapacity >= warriorCount,
            "mint : You can not mint more warriors than max capacity"
        );
        uint256 ap = 0;
        for (uint256 i = 0; i < warriorCount; i++) {
            require(
                warrior.ownerOf(warriorIds[i]) == msg.sender,
                "mint : you should own these warrior tokens"
            );
        ...
        tokenData[_tokenIds.current()] = Legion(
            name,
            beastIds,
            warriorIds,
            0,
    ...
    function canHuntMonster(uint256 tokenId)
        external
        view
        override
        returns (uint256)
    ...
    function hunt(uint256 tokenId, uint8 monsterId)
        external
        returns (
            bool,
            uint256,
        ...
        )
    {
        require(
            ...
            "Hunt: you should own this token first"
        );
        require(canHunt(tokenId) == true, "Hunt: can not hunt");
        (, uint256 percent, uint256 ap, uint256 reward) = monster
            .getMonsterInfo(monsterId);
        require(
            tokenData[tokenId].attack_power >= ap,
            "This legion should have more strong power to hunt this monster"
        );
        uint256 randnum = genRand(100);
        bool retval = false;

        if (monsterId <= 20) {
            uint256 bonusPercent = (tokenData[tokenId].attack_power - ap) /
                2000 /
                100;
        ...
        if (randnum < percent) {
            if (rewardpool.getUnclaimedUSD(msg.sender) == 0)
                taxStartDay[msg.sender] =
                    block.timestamp -
    ...
    function decreaseAttackPower(uint256 _tokenId) internal {
        uint256 damamgePercent = feehandler.getFee(2);
        tokenData[_tokenId].attack_power = tokenData[_tokenId].attack_power.sub(
            tokenData[_tokenId].attack_power.mul(damamgePercent).div(10000)
        );
    }

    function claimReward() external {
        require(
            rewardpool.getUnclaimedUSD(msg.sender) > 0,
            "You don't have any uncliamed reward"
        );
        rewardpool.claimReward(msg.sender, taxStartDay[msg.sender]);
        taxStartDay[msg.sender] = 0;
    }

    function getTaxLeftDays(address _address) external view returns (uint256) {
        if (taxStartDay[_address] == 0) return 0;
        uint256 diffDays = (block.timestamp - taxStartDay[_address]) / 1 days;
    ...
    function getLegion(uint256 tokenId)
        external
        view
        override
        returns (
    ...
    function addBeasts(uint256 tokenId, uint256[] memory beastIds) internal {
        ...
        uint256 beastLength = tokenData[tokenId].beast_ids.length;
        require(
            beastLength + beastIds.length <= maxBeasts,
            "You could not overflow the max beasts number"
        );
        for (uint256 i = 0; i < beastCount; i++) {
            require(
                beast.ownerOf(beastIds[i]) == msg.sender,
                "You should own these beasts"
            );
            tokenData[tokenId].beast_ids.push(beastIds[i]);
            beast.burn(beastIds[i]);
        }
        uint256 unit = 10**18;
        uint256 bloodstoneAmount = itemPrice.mul(beastCount).mul(unit).div(
            denominator
        );
        require(
            bloodstone.balanceOf(msg.sender) >= bloodstoneAmount,
    ...
    function addWarriors(uint256 tokenId, uint256[] memory warriorIds)
        internal
    {
        require(ownerOf(tokenId) == msg.sender, "You should own this legion");
        uint256 warriorCount = warriorIds.length;
        ...
        require(
            maxCapacity >=
                tokenData[tokenId].warrior_ids.length + warriorIds.length,
            "Warrior count should not exceed the max capacity"
        );
        uint256 ap = 0;
        for (uint256 i = 0; i < warriorCount; i++) {
            require(
                warrior.ownerOf(warriorIds[i]) == msg.sender,
                "You should own these warriors"
            );
            (, , ap) = warrior.getWarrior(warriorIds[i]);
            tokenData[tokenId].warrior_ids.push(warriorIds[i]);
            tokenData[tokenId].attack_power = tokenData[tokenId]
                .attack_power
                .add(ap * 100);
            warrior.burn(warriorIds[i]);
        }
        uint256 unit = 10**18;
        uint256 bloodstoneAmount = tokenData[tokenId]
            .supplies
            .mul(supplyPrice)
            .mul(unit)
            .mul(warriorCount) +
            itemPrice.mul(warriorCount).mul(unit).div(denominator);
        require(
            bloodstone.balanceOf(msg.sender) >= bloodstoneAmount,
            "Insufficient balance"
        );
    ...
    function addSupply(
        ...
        uint256 bloodstoneAmount = warriorCount.mul(supply).mul(supplyPrice) *
            10**18;
        require(
            bloodstone.balanceOf(msg.sender) >= bloodstoneAmount,
            "Insufficient balance"
    ...
    function execute(uint256 _tokenId, bool isbeast) public {
        if (isbeast) {
            require(
                beast.ownerOf(_tokenId) == msg.sender,
                "Execute : you should own this beast"
            );
            uint256 mintingPrice = beast.getMintingPrice();
            rewardpool.returnTokenToPlayer(
                msg.sender,
                mintingPrice.mul(2000).div(10000) * 10**18
            );
            beast.burn(_tokenId);
        } else {
            require(
                warrior.ownerOf(_tokenId) == msg.sender,
                "Execute : you should own this warrior"
            );
    ...
    function getAvailableLegionsCount(address _address)
        external
        view
        returns (uint256)
    {
        uint256 ret = 0;
        for (uint256 i = 0; i < addressTokenIds[_address].length; i++) {
            if (canHunt(addressTokenIds[_address][i])) ret++;
        }
        return ret;
    }

    function getMaxAttackPower(address _address)
        external
        view
        returns (uint256)
    {
        uint256 ret = 0;
        for (uint256 i = 0; i < addressTokenIds[_address].length; i++) {
            if (tokenData[addressTokenIds[_address][i]].attack_power > ret)
                ret = tokenData[addressTokenIds[_address][i]].attack_power;
        }
        return ret;
    }

    function getCapacityOfLegion(uint256 tokenId)
        public
        view
        returns (uint256)
    {
        uint256 maxCapacity = 0;
        uint256 capacity = 0;
        for (uint256 i = 0; i < tokenData[tokenId].beast_ids.length; i++) {
    ...
    function setItemPrice(uint256 _itemPrice, uint256 _denominator)
        external
        onlyOwner
    {
    ...
    function setFee(uint256 _fee, uint8 _index) external onlyOwner {
