...
contract HypnoWorld is ERC721A, Pausable, Ownable, ReentrancyGuard {
    ...
    uint64 constant MINT_FEE = .001 ether;

    address private immutable WETH = 0xd0A1E359811322d97991E03f863a0C30C2cF029C;
    address private immutable treasury = address(0);
    ...
    constructor() ERC721A("HypnoWorld", "IRL")
    {
        // The initial baseURI serves as the default metadata until collection
        // is minted IRL, at the launch party
        _baseURIData = "";
    ...
    function mint(uint256 quantity) external payable nonReentrant whenNotPaused whenMintStage(MintStage.LIVE) 
    {
        ...
        _safeTransferETHWithFallback(treasury, feeAmount);
        if(msg.value > feeAmount) 
        {
            uint256 refund = msg.value - feeAmount;
            _safeTransferETHWithFallback(_msgSender(), refund);
        }
        _safeMint(_msgSender(), quantity);
    }

    function mintChanged(uint256 quantity) external payable nonReentrant whenNotPaused whenMintStage(MintStage.LIVE) 
    {
        if(quantity > MAX_MINTS_PER_BATCH) revert MaxMintPerBatchExceeded();
        uint256 feeAmount = quantity * MINT_FEE;
        if(msg.value != feeAmount) revert InvalidFee();
    ...
    /**
     * Internal function used to transfer ETH to treasury and refund to caller
     * Users WETH as a fallback
     */
     ...
     */
    function advanceStage() public onlyOwner whenNotPaused {
    ...
    function finalizeCollection() public onlyOwner whenMintStage(MintStage.FINISHED) whenNotPaused {
        if (!collectionRevealed) { revert CollectionNotRevealed(); }
