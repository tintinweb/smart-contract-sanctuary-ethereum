...
contract DCPToken is Context, IERC20, Ownable {
 ...
 event takeShare (address indexed from,address to,uint id,uint value);
    function _takeShare(uint tShare,address from,address user) private {
        ...
        uint len = farthers.length;
         emit takeShare(from,user,uint(-1),len);
        uint sended = 0;
        for( uint i = 0; i < len; i++ ){
            ...
            emit takeShare(from,parent,i,tv);
        ...
        if( tShare > sended ){
            _take(tShare - sended,from,rootAddress);
        }
    }
