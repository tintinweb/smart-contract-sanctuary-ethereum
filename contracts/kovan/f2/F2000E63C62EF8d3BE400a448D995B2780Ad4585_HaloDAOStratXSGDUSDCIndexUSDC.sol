...
contract HaloDAOStratXSGDUSDCIndexUSDC is Ownable {
    ...
    uint256 public constant BASIS_POINTS = 10**5;
    ...
    function withdraw(
        ...
        removeLiquidity(tokenAddress, deadline);
    ...
    function addLiquidity(
        ...
        IERC20(tokenAddress).safeApprove(addresses.ammZap, 0);
        IERC20(tokenAddress).safeApprove(addresses.ammZap, amount);

        uint256 rate = 0;
        if (tokenAddress == addresses.xsgd) {
            rate = IBaseToUsdAssimilator(addresses.assimXsgd).getRate();
        } else {
            rate = IBaseToUsdAssimilator(addresses.assimUsdc).getRate();
        }
        uint256 numeraire = amount.mul(rate).div(1e8);
        uint256 minLpTokens = numeraire.sub(
            numeraire.mul(slippage).div(BASIS_POINTS)
        );

        IZap(addresses.ammZap).zap(
            addresses.ammCurve,
            amount,
            deadline,
            minLpTokens,
    ...
    function depositAndBorrow() internal {
        ...
        (uint256 totalCollateralETH, , , , , ) = ILendingPool(
            addresses.lendingPool
        ).getUserAccountData(msg.sender);
        console.log("[StratContract] totalCollateralETH: ", totalCollateralETH);
        uint256 maxBorrowInETH = totalCollateralETH
            .mul(collateralizationRatio)
            .div(BASIS_POINTS);
        console.log("[StratContract] maxBorrowInETH: ", maxBorrowInETH);
        uint256 usdcPriceInETH = IAaveOracle(addresses.lendingOracle)
            .getAssetPrice(addresses.usdc);
        console.log("[StratContract] usdcPriceInETH: ", usdcPriceInETH);
        uint256 usdcToBorrow = maxBorrowInETH.mul(1e6).div(usdcPriceInETH);
        console.log("[StratContract] usdcToBorrow: ", usdcToBorrow);

        uint256 interestRateMode = 2; // 1 = stable, 2 = variable
    ...
    function swap(
        ...
        console.log("[StratContract] === Swapping... ===");
        console.log("[StratContract] originToken balance:", amount);
        ...
        (int24 tick, ) = OracleLibrary.consult(addresses.uniswapPool, 10);
        ...
        console.log("[StratContract] quoted targetToken:", amountOut);
        uint256 minAmountOut = amountOut.sub(
            amountOut.mul(slippage).div(BASIS_POINTS)
        );
        console.log("[StratContract] amountOutMinimum:", minAmountOut);
        ...
        console.log("[StratContract] targetToken received:", amountReceived);
    ...
    function repayAndWithdraw() internal {
        uint256 usdcBalance = IERC20(addresses.usdc).balanceOf(address(this));
        console.log(
            "[StratContract] USDC got from swapping indexUSDC:",
            usdcBalance
        );
        ...
        console.log("[StratContract] === before repay ===");
        console.log(
            "[StratContract] totalDebtETHBeforeRepay:",
        ...
        console.log("[StratContract] === after repay ===");
        console.log(
            "[StratContract] totalDebtETHAfterRepay:",
            totalDebtETHAfterRepay
        );
        console.log("[StratContract] totalCollateralETH:", totalCollateralETH);
        uint256 repayPercentage = uint256(1 ether).sub(
            totalDebtETHAfterRepay.mul(1e18).div(totalDebtETHBeforeRepay)
        );
        console.log("[StratContract] repayPercentage:", repayPercentage);
        uint256 maxWithdrawInETH = totalCollateralETH.mul(repayPercentage).div(
            1e18
        );
        console.log("[StratContract] maxWithdrawInETH:", maxWithdrawInETH);
        uint256 hlpPriceInETH = IAaveOracle(addresses.lendingOracle)
            .getAssetPrice(addresses.ammCurve);
        console.log("[StratContract] hlpPriceInETH:", hlpPriceInETH);
        uint256 withdrawAmount = (totalDebtETHAfterRepay == 0)
            ? type(uint256).max // withdraw everything if debt is totally paid
            : maxWithdrawInETH.mul(1e18).div(hlpPriceInETH);
        console.log("[StratContract] hlpToWithdraw:", withdrawAmount);
    ...
    function removeLiquidity(address tokenAddress, uint256 deadline) internal {
        uint256 hlpBalance = IERC20(addresses.ammCurve).balanceOf(
            address(this)
        );
        ...
        console.log(
            "[StratContract] withdrawn from pool:",
            withdrawals[0],
            withdrawals[1]
        );
        ...
        if (tokenAddress == addresses.xsgd) {
            ...
            console.log("[StratContract] target amount: ", targetAmount);
    ...
    function transferBalances() internal {
        ...
        console.log(
            "[StratContract] indexUSDC balance left: ",
            indexUsdcBalanceLeft
        );
        console.log("[StratContract] XSGD balance left: ", xsgdBalanceLeft);
        console.log("[StratContract] USDC balance left: ", usdcBalanceLeft);
        console.log("[StratContract] hToken balance left: ", hTokenBalanceLeft);
...
interface IZap {
    ...
    function calcSwapAmountForZapFromBase(address _curve, uint256 _zapAmount)
        external
        view
        returns (uint256);

    function calcSwapAmountForZapFromQuote(address _curve, uint256 _zapAmount)
        external
        view
        returns (uint256);
