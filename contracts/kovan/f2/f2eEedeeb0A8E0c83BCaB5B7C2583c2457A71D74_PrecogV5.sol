...
contract PrecogV5 is IPrecogV5 {
    ...
    function _updateCurrentTradingCycle(address _token, bool _isLimitedTradingCycles, uint _limitTradingCycles) internal {
        ...
        while (uint48(block.timestamp) >= _lastTradingCycle.endTime) {
            ...
            _lastTradingCycle = Cycle(_lastTradingCycle.id + 1, _newCycleStartTime, _newCycleStartTime + _duration);
    ...
    function updateCurrentTradingCycle(address token, uint limitTradingCycles) override external {
    ...
    function addLiquidityPool(address token) external onlyAdmin {
        ...
        if (!isNotFirstInvestmentCycle[token]) {
            tradingCycles[token].push(
                Cycle(
                    ...
                    uint48(block.timestamp) + cycleConfig.firstTradingCycle
                )
            );
        ...
        if (isRemoved[token] == true) {
            ...
            if (block.timestamp >= lastTradingCycle.endTime) {
                Cycle memory newCycle = Cycle(
                    ...
                    uint48(block.timestamp),
                    uint48(block.timestamp) + cycleConfig.tradingCycle
                );
        ...
        cyclesChangedInfo[token] = CyclesChangedInfo(lastTradingCycle.endTime, uint48(block.timestamp), uint48(block.timestamp), cycleConfig.fundingCycle, cycleConfig.firstDefundingCycle, cycleConfig.defundingCycle);
    }
    ...
    function _updateAdjustment(address _token) internal {
        ...
        uint48 _nextFundingTime = PrecogV5Library._nextFundingTime(_cycleChangedInfo.fundingApplyTime, _cycleChangedInfo.fundingDuration);
        uint48 _nextTradingTime = _nextFundingTime;
        
        uint48 _nextDefundingTime = PrecogV5Library._nextDefundingTime(_cycleChangedInfo.defundingApplyTime, _cycleChangedInfo.defundingDuration, _cycleChangedInfo.firstDefundingDuration);
        ...
        _nextDefundingTime = PrecogV5Library._nextDefundingTime(_cycleChangedInfo.defundingApplyTime, _cycleChangedInfo.defundingDuration, 0);
        cyclesChangedInfo[_token] = CyclesChangedInfo(_cycleChangedInfo.tradingApplyTime, _nextFundingTime, _nextDefundingTime, _cycleChangedInfo.fundingDuration, _cycleChangedInfo.firstDefundingDuration, _cycleChangedInfo.defundingDuration);
    }

    function updateAdjustment(address token) external override onlyPrecogCore {
        _updateAdjustment(token);
    }

    function _updateFundingDuration(address _token) internal {
        if(PrecogV5Library._isAppliedChangedCycle(cyclesChangedInfo[_token].fundingApplyTime)) {
    ...
    function _updateDefundingDuration(address _token) internal {
        if(PrecogV5Library._isAppliedChangedCycle(cyclesChangedInfo[_token].defundingApplyTime)) {
        ...
        Cycle memory _futureTradingCycle;
        {
            ...
            if(isFirstRequestWithdrawal == true) {
                if(withdrawalRegister.isInDeadline(_token, _account)) {
                    ...
                    _nextDefundingTime = PrecogV5Library._nextDefundingTime(_cycleChangedInfo.defundingApplyTime, _cycleChangedInfo.defundingDuration, _cycleChangedInfo.firstDefundingDuration);
                }
            } else {
                _nextDefundingTime = PrecogV5Library._nextDefundingTime(_cycleChangedInfo.defundingApplyTime, _cycleChangedInfo.defundingDuration, 0);
            }
    ...
    function _registerWithdrawal(address _token, address _account, uint _amount, bool _isFirstRequestWithdrawal) internal {
        ...
        uint48 _nextDefundingTime = PrecogV5Library._nextDefundingTime(_cycleChangedInfo.defundingApplyTime, _cycleChangedInfo.defundingDuration, _duration);
        uint48 _locktime;
        if(withdrawalRegister.isInDeadline(_token, _account)) {
    ...
    function _withdrawBeforeFundingTime(address _account, address _to, address _token, uint _amount, uint _fee) internal {
        ...
        require(availableWithdrawalAmount >= _amount && IERC20(_token).balanceOf(address(this)) >= _amount);
