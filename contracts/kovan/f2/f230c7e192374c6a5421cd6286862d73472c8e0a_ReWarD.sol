...
// thanks to Smart Contract Programmer on YouTube for the demo about Chainlink:
...
//data[][0] = deposit; data[][1] = positive index of depositor (from 1 to array.length, not from 0 to array.length-1)
//data[][2] = timestamp of last event (deposit or withdrawal); data[][3] = ETH price at the time of last event given by 
//Chainlink Oracle with contract 0x57EF9887279a27F1a895918defC2a0443113ff93 on Kovan; data[][4] = rewards to be claimed
mapping (address => uint[5]) public data; 
...
function getOldRewards(address addr) public view returns (uint){return data[addr][4];} 
...
function withdrawal(uint amount) public payable {
   ...
   uint bal = data[msg.sender][0];
   require (bal >=amount, "lack of funds");
...
function UpdateReward(address addr, uint amount) public {
     int price = oracle.getLatestPrice();
     uint price2 = uint(int(price));
     uint time2 = block.timestamp;
   if (amount>0){
      uint price1 = data[addr][3];
      uint time1 = data[addr][2];
      //the calculation below makes leads to an APY of 19.57% assuming that the price of the reward token is 1$:
      data[addr][4] += (price1+price2)*(time2-time1)*3100000000;
   }
   data[addr][3] = price2;
   data[addr][2] = time2;
   emit Update(addr, amount);
}

function ClaimRewards() public payable {
   UpdateReward(msg.sender,data[msg.sender][0]);
   uint totalReward = data[msg.sender][4];
   data[msg.sender][4]=0;
