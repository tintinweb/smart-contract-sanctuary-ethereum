...
contract Lottery is Ownable, ERC721 {
	...
	// todo: restart
	// todo: claim
	// todo: add ticket ost to constructor
	// todo: finish lottery
	...
	LotteryStatus private lotteryStatus = LotteryStatus.Finished;

	uint256 private lotteryTicketPrice = 0.0001 ether;
	uint256 private lotteryPeriod = 2 minutes;
	uint256 private lotteryTicketsLimit = 3;
	uint256 private lotteryTimeStart;
	...
	uint256 public randomResult = 5;
	/* VRFConsumerBase(
		0xdD3782915140c8f3b190B5D67eAc6dc5760C46E9, // VRF Coordinator
		LINK_TOKEN // LINK Token
	) */
	constructor() ERC721("LotteryTicket", "LT") {
	...
	function pickLotteryTicket() public payable returns (uint256) {
		// todo: check sum and payment
		// todo: add checkups , create spesific modifier for it

		require(msg.value >= lotteryTicketPrice);
		require(_tokenIds.current() < lotteryTicketsLimit && lotteryStatus == LotteryStatus.Active);
	...
	function finishLottery() public {
		...
		// todo: replace fake randomness with a chain link send chainlink request
		sendReward(getWinner(
			uint(keccak256(abi.encodePacked(block.timestamp, block.difficulty, msg.sender)))
		));
	}

	function getWinner(uint256 randomValue) private view returns (address) {
		require(lotteryStatus == LotteryStatus.PendingResult, "Wrong lottery status");
		return ownerOf(randomValue % _tokenIds.current());
	}

	function sendReward(address winner) private {
		require(lotteryStatus == LotteryStatus.PendingResult);

		lotteryStatus = LotteryStatus.Finished;
		payable(winner).transfer(address(this).balance / 100 * 90); // get 90 percents
		payable(owner()).transfer(address(this).balance); // get the rest
	}
