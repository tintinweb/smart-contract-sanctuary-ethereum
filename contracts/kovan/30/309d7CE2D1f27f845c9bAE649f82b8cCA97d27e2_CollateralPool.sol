...
contract CollateralPool is Ownable, CheckContract, ICollateralPool {
    ...
    function setExternalStaker(address _address) external onlyOwner {
        maticStaker = IMockMATICStaker(_address);
    ...
    function _transferDepositedMATIC(uint _amount) internal {
        ...
        require(success, "Collateral pool: matic transfer to buffer Pool failed");

        (success, ) = payable(externalStakingAddress).call{ value: stakingAmount }("");
        require(success, "Collateral pool: matic transfer to external staking address failed");
    }

    function swapoMATICtoMATIC(uint _amount) external override payable {
        require( oMATICToken.balanceOf(msg.sender) >= _amount );
        require(address(bufferPool).balance >= _amount, "not enough matic available in buffer pool");
        require(MATIC_depoisted >= _amount, "cannot swap more oMATIC than deposited MATIC");

        MATIC_depoisted -= _amount;
        oMATIC_minted -= _amount;

        oMATICToken.burn(msg.sender, _amount);

        if( _amount <= address(bufferPool).balance){
            bool success = bufferPool.sendMATIC(msg.sender, _amount);
            require(success, "Collateral pool: MATIC transfer failed from BufferPool");
        }else{
            uint amount_matic = maticStaker.transferMATIC(_amount, msg.sender);
            require(amount_matic == _amount , "Collateral pool: MATIC transfer failed form MATIC Staker");
        }

        emit oMaticSwappedToMatic(address(this), msg.sender, _amount);
    }
    ...
    receive() external payable {
        ...
        oMATICToken.mint(msg.sender, msg.value);
        _transferDepositedMATIC(msg.value);
        emit OMATICTokenMintedTo(msg.sender, msg.value);
    }
}