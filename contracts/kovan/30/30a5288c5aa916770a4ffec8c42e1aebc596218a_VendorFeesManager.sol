...
contract VendorFeesManager is
    ...
    OwnableUpgradeable
{
    event ChangeFee(address _pool, uint256 _feeRate, uint256 _type);
    ...
    function getFee(
        ...
        if (rateFunction[_pool] == 2) {
            return getLinearFee(pool, _rawPayoutAmount);
        }
    ...
    function getLinearFee(
        ...
        uint256 rate = getCurrentRate(address(_pool));
        return (_rawPayoutAmount * rate) / 1000000;
    }
    ...
    function getCurrentRate(
        ...
        if (rateFunction[_pool] == 2) {
            return (feeRates[_pool] * (ILendingPool(_pool).expiry() - block.timestamp)) / SECONDS_IN_YEAR;
        } 
