...
contract Betting {
    ...
    function addGame(
        ...
        game.id = gameCounter;
        game.name = _name;
        game.game_state = GameState.UPCOMING;
        game.bet_state = BetState.NONE;
    ...
    function openBidding(uint256 _gameIndex) public onlyOwner {
        ...
        require(
            game.bet_state != BetState.BIDDING_STARTED,
    ...
    function closeBidding(uint256 _gameIndex) public onlyOwner {
        Game storage game = games[_gameIndex];
        require(
            game.bet_state != BetState.BIDDING_CLOSED,
    ...
    function getActiveGames() public view returns (ActiveGame[] memory) {
        ...
        for (uint256 i = 0; i < activeGameIds.length; i++) {
            ...
            active_gems[i].game_state = game.game_state;
    ...
    //     games[_gameIndex].game_state = GameState.LISTED;
    //     activeGameId = 0;
    //     anyGameActive = false;
    // }
    ...
    function declareResult(uint256 gameIndex, Winner winner) public onlyOwner {
        ...
        game.bet_state = BetState.RESULT_DECLARED;
