...
// https://docs.chain.link/docs/google-weather/
...
contract Weather is ChainlinkClient {
    ...
    event Rain(uint256 _result);
    event Hail(uint256 _result);
    event Generic(uint256 _result);
    ...
    function requestRain(string memory _from, string memory _to) external {
        Chainlink.Request memory req = buildChainlinkRequest(
            rainJobId,
            address(this),
            this.fulfillRain.selector
        );
        req.add("dateFrom", _from);
        req.add("dateTo", _to);
        req.add("method", "SUM");
    ...
    function fulfillRain(bytes32 _requestId, uint256 _result)
        external
        recordChainlinkFulfillment(_requestId)
    {
        rain = _result;
    ...
    function fulfillGeneric(bytes32 _requestId, uint256 _result)
        external
        recordChainlinkFulfillment(_requestId)
    {
        emit Generic(_result);
    }
}