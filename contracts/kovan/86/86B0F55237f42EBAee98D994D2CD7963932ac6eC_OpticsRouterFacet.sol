...
contract OpticsRouterFacet {
    ...
    struct Storage {
        /// @dev Home contract (https://docs.celo.org/celo-codebase/protocol/optics#home)
        Home home;
        /// Approved routers by domain
        mapping(uint32 => bytes32) routers;
    }

    /* ========== Events ========== */

    event RemoteCall(uint32 domain, address indexed callee);
    event LocalCall(uint32 origin, address indexed callee);
    ...
    function initOpticsRouter(Home home) external {
    ...
    function remoteCall(
        ...
        s.home.dispatch(domain, s.routers[domain], abi.encodePacked(callee, callData));

        emit RemoteCall(domain, callee);
    }
    ...
    /// @notice Batch registers the addressed of Router contracts on a remote chain
