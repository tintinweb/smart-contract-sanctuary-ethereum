    ...
    Version
{
    ...
    event TokenMinted(uint256 timestamp, address indexed _recipient, uint256 indexed _tokenId, address[] _addressBundle);
        ...
        returns (address[] memory)
    {
        require(
            _bundleNonce[tokenId],
            Errors.NFCS_TOKEN_HAS_NOT_BUNDLE
        );
  ...
  */
   ...
   */
    ...
    function getLimits() external override view returns(uint128 dailyLimit, uint128 globalLimit,uint128 userDailyLimit, uint128 userGlobalLimit){
    ...
    function getNFCSLimits(uint _nfcsId) external override view returns(uint128 dailyLimit, uint128 globalLimit) {
        dailyLimit = 0;
        globalLimit = addressBook.globalNFCSLimit(_nfcsId);
    }


    function getTotalOutstanding(uint _nfcsId) public override view returns(uint gloablOutstanding,uint userOutstanding,uint nfcsOutstanding){
        ...
        for(uint i = 0; i < investors.length; i++){
            ...
            userOutstanding += 
                    IERC20PaymentStandard(investors[i])
                        .getUserTotalOutstanding(_nfcsId);
            gloablOutstanding += 
                    IERC20PaymentStandard(investors[i])
                        .getTotalOutstanding();
            nfcsOutstanding += 
                    IERC20PaymentStandard(investors[i])
                        .getNFCSTotalOutstanding(_nfcsId);
        }
    }
