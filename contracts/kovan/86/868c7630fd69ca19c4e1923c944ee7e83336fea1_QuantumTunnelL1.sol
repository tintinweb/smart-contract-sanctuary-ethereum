...
contract QuantumTunnelL1 is Ownable {
    ...
    function createTokenOnL2(
        ...
        bytes4 selector = bytes4(
            keccak256("mintToken(address, address, uint256, uint256)")
        );
        ...
        //emit UpdateInitiated(unusedAsset, newValue, msg.sender);
    }
    ...
    function removeStake(ERC721 token, uint256 tokenId) external {
        ...
        require(
            originalTokenOwner[address(token)][tokenId] == msg.sender,
            "Sender is not original owner of token"
        );
        require(
            block.timestamp > lockExpires[address(token)][tokenId],
            "The token lock is not expired yet"
        );
    ...
    function renewStake(
        ...
        // call L2
        bytes4 selector = bytes4(
            keccak256("updateExpiry(address, uint256, uint256)")
        );
        bytes memory callData = abi.encodeWithSelector(
            selector,
            address(token),
