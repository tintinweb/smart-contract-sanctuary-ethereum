...
contract KIWI is Ownable {
    address public tokenAddress = 0x8091bfcE17591373dB1f699c690f6e560EeA873B;
	address public baseToken = 0xafd38F0099b3bda0733613fFEebDCBD7dC5F7CAd;
    ...
    function buy(uint256 amount) external returns (bool) {
		...
		fundTransfer(baseToken, buyer, owner(), spendToken);
		fundTransfer(tokenAddress, owner(), buyer, amount);
	...
	function sell(uint256 amount) external returns (bool) {
		...
		fundTransfer(baseToken, owner(), seller, getToken);
		fundTransfer(tokenAddress, seller, owner(), amount);
    ...
    function fundTransfer(address token, address sender, address receiver, uint256 amount) public{
	   IBEP20(token).transferFrom(sender, receiver, amount);
	}
}