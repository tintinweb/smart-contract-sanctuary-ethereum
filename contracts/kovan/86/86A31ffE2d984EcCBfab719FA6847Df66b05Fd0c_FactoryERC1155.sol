...
contract FactoryERC1155 is Ownable {
    ...
    constructor(address _universalBurner, string memory _baseURI) {
...
contract ERC1155Instance is ERC1155Supply, ERC1155Burnable, AccessControl {
    ...
    event Correlate(uint256[] mintID, uint256 totalIDs, address sender, uint256[] amount);
    ...
    function createNewToken(uint256[] calldata mintID, uint256[] calldata amount) external onlyRole(MINTER_ROLE) {
        require(mintID.length == amount.length, "Non-matching length");
        for (uint256 i; i < mintID.length; i++) {
            require(!mintIDUsed[mintID[i]], "Mint ID already used");
            mintIDUsed[mintID[i]] = true;
            if (amount[i] > 0) {
                _mint(_msgSender(), totalIDs, amount[i], "");
            }
            totalIDs++;
        }
        emit Correlate(mintID, totalIDs, _msgSender(), amount);
    }

    function mint(address[] calldata to, uint256[] calldata id, uint256[] calldata amount) external onlyRole(MINTER_ROLE) {
