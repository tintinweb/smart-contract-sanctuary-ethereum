...
contract NFTMarket is Ownable, ReentrancyGuard, IERC721Receiver {
    ...
    function addSupportedNFTContract(address nftContract)
        public
        payable
        nonReentrant
        onlyOwner
    ...
    function removeSupportedNFTContract(address nftContract)
        public
        payable
        nonReentrant
        onlyOwner
    {
        ...
        // delete _supportedNFTContract[totalContract - 1];
        // _supportedNFTContract.length--;
    }
    ...
    function getMyItems() public view returns (MarketItem[] memory) {
        ...
        for (uint256 i = 0; i < totalItemCount; i++) {
            if (
                _itemIdToMarketItem[i + 1].seller == msg.sender &&
        ...
        for (uint256 i = 0; i < totalItemCount; i++) {
            uint256 currentId = i + 1;
            if (
                _itemIdToMarketItem[currentId].seller == msg.sender &&
