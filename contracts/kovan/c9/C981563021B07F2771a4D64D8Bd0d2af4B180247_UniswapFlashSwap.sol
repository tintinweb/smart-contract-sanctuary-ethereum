...
contract UniswapFlashSwap is IUniswapV2Callee, Withdrawable {
    ...
    function uniswapV2Call(
        ...
        emit LogAddress("borrowed token", tokenBorrow);

        // swap tokens on sushi - by me
        uint256 min_amount_out = getAmountOutMin(token0, token1, _amount0);
        emit Log("minimum amount out is:", min_amount_out);
        require(
            min_amount_out > amountToRepay,
            "the Amount Out is not enough!"
        );
        // swap on sushi
        swap(token1, token0, _amount1, amountToRepay);
