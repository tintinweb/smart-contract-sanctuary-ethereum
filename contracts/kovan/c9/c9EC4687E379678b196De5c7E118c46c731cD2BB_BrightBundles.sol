...
contract BrightBundles is Ownable, ReentrancyGuard
{
    ...
    {
        string Name; //0 
        uint PriceBrightList; //1
        uint PricePublic; //2
        uint[] StartingIndexes; //3
        uint[] EndingIndexes; //4
        uint PurchaseableAmount; //5
        uint PurchaseableAmountBrightList; //6
        uint PurchaseableAmountPublic; //7
        address[] ContractAddresses; //8
        address Operator; //9
        bytes32 Root; //10
        bool ActivePublic; //11
        bool ActiveBrightList; //12
        bool AllowMultiplePurchases; //13
    }
    ...
    /**
     * @dev Bundle Purchase Public Bundle
     */
    ...
    { 
        ...
        require(Bundles[BundleIndex].EndingIndexes[0] - Bundles[BundleIndex].StartingIndexes[0] > 0, "BrightBundles: BundleIndex Sold Out");
        require(
