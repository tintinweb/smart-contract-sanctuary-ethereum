...
contract InvestorClaimV2 is Ownable {
    ...
    function claimableFor( address _address ) public view returns (uint) {
        ...
        uint calculatedMax = circulatingSupply().mul( info.percent ).div( 1e6 );
        if ( calculatedMax > info.max ) {
            calculatedMax = info.max;
        }

        return calculatedMax.sub( claimed( _address ) ).mul( 1e9 );
    }
