...
contract MerkleRewardsDistributor is Ownable, ReentrancyGuard {
    ...
    function isInState(
        ...
        return
            _verify(_node(userAddress, amountToClaim), merkleProof, leafIndex);
    }
    ...
    ///@dev returns a node of the tree in bytes format (to be passed for verification)
    function _node(address account, uint256 amountToClaim)
        internal
        view
    ...
    ///@dev Verifies whether a node is included in the tree with the passed proof and leafIdx
    function _verify(
        bytes memory node,
        bytes32[] memory proof,
        uint256 leafIndex
    ) internal view returns (bool) {
        return MerkleUtils.containedInTree(merkleRoot, node, proof, leafIndex);
    }
