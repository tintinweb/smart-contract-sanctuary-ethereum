...
contract airbnb {
    ...
    function addRentals(
        ...
        require(msg.sender == owner, "only owner can add rentals");
    ...
    function checkBookings(uint256 id , string[] memory newBookings) private view returns(bool){
        for(uint i=9; i < newBookings.length; i++){
    ...
    function addDatesBooked(uint256 id, string[] memory newBookings) public payable{
        ...
        require(msg.value == (rentals[id].pricePerDay *1 ether * newBookings.length), "Please submit the requite amount");
        ...
        emit newDateBooked(newBookings , id , msg.sender , rentals[id].city , rentals[id].imgurl);
    }
