...
contract Uchu {
    ...
    **/
    ...
    function singleProBalance(Product memory product) private view returns (uint256) {
        ...
        //uint b = 3600 * 24;
        // emit Log(msg.sender, "singleProBalance", getPoint().createPositionTime);
        // emit Log(msg.sender, "singleProBalance", block.timestamp);
        return amount * ((1 + product.yeildNum/product.totalNum) ** 2);
    }
    ...
    function getBalance(address client, string memory name) public payable returns (uint256) {
        ...
        emit Log(msg.sender, "getBalance", finalBalance[client][name]);
