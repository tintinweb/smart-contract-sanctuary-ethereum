...
contract ToucanCrosschainMessenger is ToucanCrosschainMessengerStorage, Router {
     ...
     * //note to pause the bridge for a specific home token, map it to zero address for the that domain
     */
    function addTokenPair(
        ...
        // reason why remote token address can be zero,
        // mapping a home token to zero address can act like restricting any home tokens to be bridged
        // as home tokens mapped to zero address cannot be bridged
        // this is done to prevent using another mapping or more storage space in tracking
        // which tokens can be bridged and which cannot be
    ...
    function handle(
        ...
        (
            ...
            uint32 toDomain,
            bytes32 requestHash
        ) = abi.decode(_message, (uint8, address, address, uint256, uint32, bytes32));
        require(
    ...
    function sendMessage(
        ...
        requests[requestHash] = BridgeRequest(false, BridgeRequestType.SENT, MessageTypes.MINT);
...
contract ToucanCrosschainMessengerStorage {
    ...
    mapping(address => mapping(uint32 => address)) public remoteTokens;
}
