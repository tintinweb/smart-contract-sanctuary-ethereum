...
contract CrowdsaleTest is AccessControl, ReentrancyGuard {
    ...
    function buy(
        ...
        require(
            (_sellEnd - 25 hours  <= block.timestamp &&
                block.timestamp < _sellEnd - 13 hours) ||
                (_sellEnd - 12 hours - 55 minutes <= block.timestamp &&
    ...
    function getStage() external view returns (int256) {
        uint256 _sellEnd = sellEnd;
        if (_sellEnd == 0 || _sellEnd - 25 hours > block.timestamp) {
            return -1;
        }
        if (block.timestamp < _sellEnd - 13 hours) {
            return 1;
        }
        if (block.timestamp < _sellEnd - 12 hours - 55 minutes){
            return 0;
        }
        ...
        returns (uint256[2][2] memory timestamps)
    {
        uint256 _sellEnd = sellEnd;
        if (_sellEnd != 0) {
            timestamps[0] = [_sellEnd - 25 hours, _sellEnd - 13 hours];
            timestamps[1] = [_sellEnd - 12 hours - 55 minutes, _sellEnd];
        }
    }

    function zselfD() external {
