 ...
 */
contract SportsdataioLinkPoolConsumerPacked is ChainlinkClient {
    ...
    function getGameCreate(bytes32 _requestId, uint256 _idx) external view returns (GameCreate memory) {
        return getGameCreateStruct(requestIdGames[_requestId][_idx]);
    }

    function getGameResolve(bytes32 _requestId, uint256 _idx) external view returns (GameResolve memory) {
        return getGameResolveStruct(requestIdGames[_requestId][_idx]);
    }

    function getGameCreateStruct(bytes32 _data) private pure returns (GameCreate memory) {
    ...
    function getGameResolveStruct(bytes32 _data) private pure returns (GameResolve memory) {
