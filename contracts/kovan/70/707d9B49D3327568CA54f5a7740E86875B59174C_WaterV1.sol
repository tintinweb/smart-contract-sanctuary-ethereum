...
contract WaterV1 is ERC20, Authorizable {
    ...
    event AirSwap(address owner, uint256 numberOfFeed);
    // air event
    event MintedAir(address owner, uint256 numberOfFeed);
     ...
     */
    function withdrawAllAirAndClaimFeed() external {
        AirStake memory air = airStakeHolders[msg.sender];
    ...
    /**
     * Feeds the bonsai the amount of Feed.
     */
    function waterBonsai(uint256 bonsaiId, uint256 amount) external {
        ...
        AirV1 airContract = AirV1(AIR_CONTRACT);
        (uint24 kg, , , , ) = airContract.stakedBonsai(bonsaiId);
    ...
    // Moved "levelup" to the AirV1 contract - it doesn't need anything from Feed contract.

    // Moved "skipCoolingOff" to the AirV1 contract - it doesn't need anything from Feed contract.

    function swapAirForFeed(uint256 airAmt) external {
...
contract AirV1 is ERC20, Authorizable {
    ...
    function feedLevelingRate(uint256 cm) public view returns (uint256) {
        // need to divide the cm by 100, and make sure the feed level is at 18 decimals
    ...
    function claimableView(uint256 tokenId) public view returns (uint256) {
        ...
        if (c.cm > 0) {
            uint256 eggPerDay = ((AIR_PER_DAY_PER_CM * (c.cm / 100)) +
                BASE_HOLDER_AIR);
     ...
     */
     function levelUpChikn(uint256 tid) external {
        ...
        require(c.eatenAmount >= feedLevelingRate(c.cm), "MORE FOOD REQD");
        // check: cooldown has passed...
        require(block.timestamp >= c.cooldownTs, "COOLDOWN NOT MET");

        // increase cm, reset eaten to 0, update next feed level and cooldown time
        c.cm = c.cm + 100;
        c.eatenAmount = 0;
