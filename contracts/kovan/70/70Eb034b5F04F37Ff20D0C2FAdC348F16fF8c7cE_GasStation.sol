...
import "./utils/ExchangeV2.sol";
//import "./utils/ExchangeV3.sol";

contract GasStation is Ownable, FeePayerGuard, LondonTxSupport, ExchangeV2 {
    ...
    constructor(address _router, address _feeTokensStore, address _approver, address _feePayer, uint256 _txRelayFeePercent) {
        _setRouter(_router);
    ...
    function sendTransaction(TxRequest calldata _tx, TxFee calldata _fee, bytes calldata _sign) external onlyFeePayer {
        ...
        _verify(_tx, _sign);
        require(feeTokensStore.isAllowedToken(_fee.token), "Fee token not supported");
        ...
        uint256 estimatedGasUsed = callGasUsed.add(_getEstimatedPostCallGas(_fee.token));
        ...
        uint256 maxFeeInTokens = _toTokensAmount(maxFeeInEth, address(token));
        require(token.transferFrom(_tx.from, address(this), maxFeeInTokens), "Transfer fee failed");
        ...
        uint256 spentTokens = _swapTokensToEth(totalFeeInEth, maxFeeInTokens, address(token), msg.sender);
        ...
        emit TransactionExecuted(_tx.from, _tx.to, _fee.token, spentTokens, txRelayFeeInEth);

        // We check the gas consumption, and save it for calculation in the following transactions
        {
            uint256 postCallGasUsed = initialGas.sub(gasleft()).sub(callGasUsed);
...
abstract contract ExchangeV2 {

    ISwapV2Router public router;

    function _setRouter(address _routerAddress) internal {
        router = ISwapV2Router(_routerAddress);
    }

    function _toTokensAmount(uint256 _ethAmount, address token) internal returns (uint256) {
        return router.getAmountsIn(_ethAmount, _getSwapPair(address(token)))[0];
    }

    function _swapTokensToEth(uint256 _ethAmount, uint256 tokensMaxSpend, address token, address receiver) internal returns (uint256) {
        return router.swapTokensForExactETH(_ethAmount, tokensMaxSpend, _getSwapPair(address(token)), receiver, block.timestamp)[0];
    }
