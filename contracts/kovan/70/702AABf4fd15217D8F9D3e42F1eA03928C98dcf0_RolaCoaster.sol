...
contract RolaCoaster is AbstractRolaCoaster{
    ...
    uint256 immutable ROLA_CAP;

    uint256 public totalSupplyRola = 0;
    ...
    uint16[] public percentagePublicSale;
    uint16[] public daysPublicSale;
    ...
    constructor(
        ...
        uint16[] memory publicSalePercentages,
        uint16[] memory publicSaleDays,
        ...
        ROLA_CAP = rolaCap * (10 ** decimalsRola);
        rolaRatePerUSDC = rolaTokenRatePerUSDC;
        percentagePublicSale = publicSalePercentages;
        daysPublicSale = publicSaleDays;
    ...
    function mintRolaToTreasury(uint256 amountRola) external override onlyTreasury whenNotPaused nonReentrant {
        ...
        totalSupplyRola += amountRola;
        treasurySupplyRola += amountRola;
        _mint(treasury, ROLA, amountRola, '');
    ...
    function mintRolaforPublicSale(uint256 amountUSDC) external override whenNotPaused nonReentrant whenNotBlackListedUser(_msgSender()) {
        ...
        uint256 amountRola = (amountUSDC * rolaRatePerUSDC * ( 10 ** rolaDecimals)) / (10 ** DECIMALS_USDC);
        _checkPhaseSupply(amountRola);
        USDCContract.transferFrom(_msgSender(), treasury, amountUSDC);
        totalSupplyRola += amountRola;
        publicSaleSupplyRola += amountRola;
        _mint(_msgSender(), ROLA, amountRola, '');
        emit RolaMinted(_msgSender(), amountRola);
    }

    function _checkPhaseSupply(uint256 amountRola) internal view returns (bool) {
        uint256 totalPublicSaleAmount = (ROLA_CAP * PUBLIC_SALE_PERCENTAGE)/HUNDRED_PERCENTAGE;
        uint256 daysDifference;
        if(currentStage == PublicSaleStages.phase1){
            require((totalPublicSaleAmount * percentagePublicSale[0])/HUNDRED_PERCENTAGE >= publicSaleSupplyRola + amountRola, "RolaCoaster: Public mint maximum supply reached.");
        }
        else if(currentStage == PublicSaleStages.phase2){
            uint256 lowerSupplyLimit = (totalPublicSaleAmount * percentagePublicSale[0])/HUNDRED_PERCENTAGE;
            uint256 higherSupplyLimit = (totalPublicSaleAmount * percentagePublicSale[1])/HUNDRED_PERCENTAGE;
            uint256 differenceSupplyLimit = higherSupplyLimit - lowerSupplyLimit;
            uint256 differenceDayLimit = daysPublicSale[2] - daysPublicSale[1];
            uint256 currentDayPassed = (block.timestamp - startTime) / 60 / 60 / 24;
            unchecked {
                int256 signedDaysDifference = int256(currentDayPassed - daysPublicSale[1]);
                if(signedDaysDifference < 0)
                    daysDifference = 1;
                else
                    daysDifference = ((currentDayPassed - daysPublicSale[1]) / 10) + 1;
            }
            uint256 salePerTenDays = (differenceSupplyLimit * 10) / differenceDayLimit;
            uint256 maxCurrentSupplyCap = lowerSupplyLimit + (daysDifference * salePerTenDays);
            // emit RolaMinted(_msgSender(), maxCurrentSupplyCap);
            require(maxCurrentSupplyCap >= publicSaleSupplyRola + amountRola, "RolaCoaster: Public mint maximum supply reached.");
        }
        else if(currentStage == PublicSaleStages.phase3){
            require((totalPublicSaleAmount * percentagePublicSale[1])/HUNDRED_PERCENTAGE >= publicSaleSupplyRola + amountRola, "RolaCoaster: Public mint maximum supply reached.");
        }
        else if(currentStage == PublicSaleStages.phase4){
            uint256 lowerSupplyLimit = (totalPublicSaleAmount * percentagePublicSale[1])/HUNDRED_PERCENTAGE;
            uint256 higherSupplyLimit = (totalPublicSaleAmount * percentagePublicSale[2])/HUNDRED_PERCENTAGE;
            uint256 differenceSupplyLimit = higherSupplyLimit - lowerSupplyLimit;
            uint256 differenceDayLimit = daysPublicSale[4] - daysPublicSale[3];
            uint256 currentDayPassed = (block.timestamp - startTime) / 60 / 60 / 24;
            unchecked {
                int256 signedDaysDifference = int256(currentDayPassed - daysPublicSale[3]);
                if(signedDaysDifference < 0)
                    daysDifference = 1;
                else
                    daysDifference = ((currentDayPassed - daysPublicSale[3]) / 10) + 1;
            }
            uint256 salePerTenDays = (differenceSupplyLimit * 10) / differenceDayLimit;
            uint256 maxCurrentSupplyCap = lowerSupplyLimit + (daysDifference * salePerTenDays);
            // emit RolaMinted(_msgSender(), maxCurrentSupplyCap);
        ...
        else if(currentStage == PublicSaleStages.phase5){
            require((totalPublicSaleAmount * percentagePublicSale[2])/HUNDRED_PERCENTAGE >= publicSaleSupplyRola + amountRola, "RolaCoaster: Public mint maximum supply reached.");
        }
        return true;
    }

    function changePhase() public onlyOwner {
        require(currentStage != PublicSaleStages.phase5, "RolaCoaster: Already set to highest phase.");
        if(currentStage == PublicSaleStages.phase1){
            currentStage = PublicSaleStages.phase2;
            emit PhaseChanged(uint8(PublicSaleStages.phase2));
        } else if(currentStage == PublicSaleStages.phase2){
            currentStage = PublicSaleStages.phase3;
            emit PhaseChanged(uint8(PublicSaleStages.phase3));
        } else if(currentStage == PublicSaleStages.phase3){
            currentStage = PublicSaleStages.phase4;
            emit PhaseChanged(uint8(PublicSaleStages.phase4));
        } else{
            currentStage = PublicSaleStages.phase5;
            emit PhaseChanged(uint8(PublicSaleStages.phase5));
        }
    }
    ...
    function initializeMintRolaGram(uint256 amountRola) external override whenNotPaused nonReentrant {
        require(amountRola > 0, "RolaCoaster: ROLA amount should be greater than zero.");
        safeTransferFrom(_msgSender(), treasury, ROLA, amountRola, '');
    ...
    function airdropRola(address[] memory assigned, uint256[] memory quantity) external override onlyOwner whenNotPaused nonReentrant {
        ...
        for (uint8 index = 0; index < assigned.length; index++) {
            if(!_isBlackListUser(assigned[index])){
                totalSupplyRola += quantity[index];
    ...
    function updateTreasuryAddress(address newTreasuryAddress) external override onlyTreasury whenNotPaused {
        ...
        uint256 amount = super.balanceOf(treasury, ROLA);
        super.safeTransferFrom(treasury, newTreasuryAddress, ROLA, amount, '');
    ...
    /// @dev This is the getRolaDecimals function. It is used to get the rola decimals in the contract.

    function getRolaDecimals() external view override returns (uint256) {
        return rolaDecimals;
    }
    ...
    /// @dev This function would send the amount to `to` account
    /// @param from The account to be send from
    /// @param to The account to be send to
    /// @param amount The account to be send
    /// @param data The data to be send

    function safeTransferRola(address from, address to, uint256 amount, bytes memory data) external whenNotPaused whenNotBlackListedUser(to) override {
        super.safeTransferFrom(from, to, ROLA, amount, data);
    }

    /// @dev This function would get the balance of the account
    /// @param account The account to get balance

    function balanceRola(address account) external view whenNotPaused override returns (uint256 balance) {
        return super.balanceOf(account, ROLA);
    }
...
interface IRolaCoaster {
    ...
    /**
     * @dev Get the Rola decimals of the contract
     */
    function getRolaDecimals() external view returns (uint256);
    ...
    /**
     * @dev Transfer the ROLA tokens
     */
    function safeTransferRola(address from, address to, uint256 amount, bytes memory data) external;

    /**
     * @dev Balance of the ROLA tokens
     */
    function balanceRola(address account) external returns (uint256);
