...
contract FearWolfDistributor is AccessControl, Pausable, Withdrawable, ReentrancyGuard {
    ...
    */
    ...
    function setBatchMintingLimit(uint256 _batchMintingLimit) external onlyRole(MANAGER_ROLE) {
        require(block.timestamp < saleStart, "Too late, sale is already active");
        require(presaleWolvesPerAccountLimit <= batchMintingLimit, "Cannot set batchMintingLimit less than presaleWolvesPerAccountLimit");
    ...
    function forceClaimWolves() external nonReentrant onlyRole(ADMIN_ROLE) {
        require(DEBUG_SALE_ACTIVE || block.timestamp >= saleStart, "Public sale is not active yet");
