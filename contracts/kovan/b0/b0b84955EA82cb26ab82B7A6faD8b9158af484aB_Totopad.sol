...
// TODO ограничения перевести на произвольное количество decimals
...
contract Totopad is AccessControl, ReentrancyGuard {
    ...
    struct PoolInfo {
        ...
        uint256 targetRaise;
        uint256 totalStaked;
        uint256 fundPercentOfTotalRaise;
    ...
    constructor(IERC20 _stakingToken, IERC20 _fundingToken) {
        ...
        require(address(_fundingToken) != address(0), "constructor: _fundingToken must not be zero address");
     ...
     * @param _withUpdate make multiple update or not
     */
    function add(
        IERC20 _rewardToken,
        ...
        require(address(_rewardToken) != address(0), "add: _rewardToken is zero address");
        require(block.timestamp <= _allocStartTime, "add: tokAlloc must be > 0");
        require(_allocStartTime < _stakingEndTime, "add: _allocStartTime must be before staking end");
        require(_stakingEndTime < _fundingEndTime, "add: stakendEndTime must be before fundingEndTime");
        require(_fundingEndTime < _rewardStopTime);
        ...
        uint256 rewardDuration = _rewardStopTime - _fundingEndTime;

        poolInfo.push(PoolInfo({
            lastAllocTime: _allocStartTime,
            maxStakingAmountPerUser: 250 * 1e18,
            accPercentPerShare: 0,
            rewardPerBlock: _rewardAmounts[0] / rewardDuration,
            targetRaise: _targetRaises[0],
            totalStaked: 0,
            fundPercentOfTotalRaise: 0,
            lastRewardTime: 0,
            accRewardPerShare: 0
        }));
        ...
        poolInfo.push(PoolInfo({
            ...
            accPercentPerShare: 0,
            rewardPerBlock: _rewardAmounts[1] / rewardDuration,
        ...
        poolInfo.push(PoolInfo({
            ...
            accPercentPerShare: 0,
            rewardPerBlock: _rewardAmounts[2] / rewardDuration,
    ...
    function pledge(uint256 _pid, uint256 _amount) external nonReentrant {
        ...
        require(user.amount + _amount <= pool.maxStakingAmountPerUser, "pledge: can not exceed max staking amount per user");
        require(block.timestamp <= project.stakingEndTime, "pledge: Staking no longer permitted");

        updatePool(_pid);

        user.amount += _amount;
        user.tokenAllocDebt = user.tokenAllocDebt + (_amount * pool.accPercentPerShare / 1e18);
    ...
    function fundPledge(uint256 _pid) external nonReentrant {
        ...
        pool.fundPercentOfTotalRaise += userPercentAlloc;
    ...
    function withdrawReward(uint256 _pid) external nonReentrant onlyRole(DEFAULT_ADMIN_ROLE) {
        ...
        uint256 rewardAmount = pool.rewardPerBlock * rewardDuration;
    ...
    function claimReward(uint256 _pid) external nonReentrant {
        ...
        uint256 userPercentAlloc = user.amount * pool.accPercentPerShare / 1e18 - user.tokenAllocDebt;
        uint256 pending = userPercentAlloc * pool.accRewardPerShare / 1e18 - user.rewardDebt;

        if (pending > 0) {
            user.rewardDebt = userPercentAlloc * pool.accRewardPerShare / 1e18;
    ...
    function updatePool(uint256 _pid) public {
        ...
        if (block.timestamp < project.fundingEndTime) {
            return;
        }

        uint256 endRewardTime = block.timestamp <= project.rewardStopTime ? block.timestamp : project.rewardStopTime;
        uint256 timeSinceReward = getMultiplier(pool.lastRewardTime, endRewardTime);

        if (timeSinceReward > 0) {
            (uint256 accRewardPerShare, uint256 lastRewardTime) = getAccRewardPerShare(_pid);
    ...
    function pendingRewards(uint256 _pid, address _user) public view returns (uint256) {
        ...
        (uint256 accPercentPerShare,) = getAccPerShareAlloc(_pid);
        (uint256 accRewardPerShare,) = getAccRewardPerShare(_pid);

        uint256 userPercentAlloc = user.amount * accPercentPerShare / 1e18 - user.tokenAllocDebt;
        return userPercentAlloc * accRewardPerShare / 1e18 - user.rewardDebt;
    }

    function getAccRewardPerShare(uint256 _pid) internal view returns (uint256, uint256) {
        ProjectInfo memory project = projectInfo[_pid / 3];
        PoolInfo memory pool = poolInfo[_pid];

        uint256 endRewardTime = block.timestamp <= project.rewardStopTime ? block.timestamp : project.rewardStopTime;
        uint256 multiplier = getMultiplier(pool.lastRewardTime, endRewardTime);
        uint256 reward = multiplier * pool.rewardPerBlock;

        return (
            pool.accRewardPerShare + reward * 1e18 / pool.fundPercentOfTotalRaise,
            endRewardTime
        );
