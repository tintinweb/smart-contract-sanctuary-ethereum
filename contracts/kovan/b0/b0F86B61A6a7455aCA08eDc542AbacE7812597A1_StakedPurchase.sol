...
contract StakedPurchase is Ownable {
  ...
  function publishAsset(
    ...
    Asset memory asset = Asset(maxAssetId, name, uri, price, payable(msg.sender));
  ...
  function assetOfOwner(address _ownerAddress) external view returns (uint256[] memory) {
    uint256[] memory assetIds;
    uint256 currentIndex = 0;
    for (uint256 i = 1; i <= maxAssetId; i++) {
      if (assetMaping[i].seller == _ownerAddress) {
        assetIds[currentIndex] = i;
        currentIndex += 1;
      }
    }
    return assetIds;
