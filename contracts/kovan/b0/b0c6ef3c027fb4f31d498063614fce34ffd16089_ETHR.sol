...
contract ETHR is ERC20, LockToken {
    using SafeMath for uint256;
    ...
    ETHRDividendTracker public dividendTracker;

    address public deadWallet = 0x000000000000000000000000000000000000dEaD;

    address public immutable USDT = address(0xdAC17F958D2ee523a2206206994597C13D831ec7); //USDT

    uint256 public _totalSupply = 77000000000 * (10**18);
    ...
    address payable public _buybackWalletAddress = payable(0x8402ce27b4095802f0ef53F0F5ED3331b31ffdbC);
    ...
    constructor() ERC20("ETH Rewards", "ETHR") 
    {
        dividendTracker = new ETHRDividendTracker();
    ...
    {
        require(newAddress != address(dividendTracker), "ETHR: The dividend tracker already has that address");
        ETHRDividendTracker newDividendTracker = ETHRDividendTracker(payable(newAddress));
        require(newDividendTracker.owner() == address(this), "ETHR: The new dividend tracker must be owned by the ETHR token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "ETHR: The router already has that address");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "ETHR: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "ETHR: The Uniswap pair cannot be removed from automatedMarketMakerPairs");
        _setAutomatedMarketMakerPair(pair, value);
    }
    

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "ETHR: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue != gasForProcessing, "ETHR: Cannot update gasForProcessing to same value");
    ...
    function _transfer(address from, address to, uint256 amount) internal open(from, to) override 
    {
        ...
        !automatedMarketMakerPairs[from] && from != owner()) 
        {
            ...
            uint256 swapableFees = totalFees.sub(USDTRewardsFee); // marketingFee+buybackFee+liquidityFee.
...
contract ETHRDividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("ETHR_Dividend_Tracker", "ETHR_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 1000000 * (10**18); //must hold 1 Millions+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "ETHR_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "ETHR_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main ETHR contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "ETHR_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "ETHR_Dividend_Tracker: Cannot update claimWait to same value");
