...
contract CricStoxFactory is Ownable {
    ...
    function createPlayerStox(string memory name_, string memory symbol_, uint8 decimals_, address[] memory pools_, uint256[] memory poolShares_) external onlyOwner {
        require(pools_.length == poolShares_.length, 'Mismatched array lengths for pools and pool shares');
        ...
        PlayerStoxToken playerStox = new PlayerStoxToken(name_, symbol_, decimals_, cricStoxMasterAddress, adminWallet, transferAllowances);
