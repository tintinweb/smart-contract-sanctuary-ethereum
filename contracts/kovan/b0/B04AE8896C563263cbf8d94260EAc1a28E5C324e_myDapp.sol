...
contract myDapp is ERC20 {
    constructor() public ERC20("jagan's token", "jaganToken") {
...
contract tokenFarm is Ownable {
    uint256 public entryToken = 100;
    uint256 public winTokens=10;
    uint256 public loseTokens;
    uint256 randNonce = 0;
    ...
    function storeTokenAddress(address _tokenaddress) public onlyOwner{
        dappToken=IERC20(_tokenaddress);
    }
    function getToken(address _user) public {
        for (uint256 usersIndex = 0; usersIndex > users.length; usersIndex++) {
            require(users[usersIndex] != _user, "Already registered");
        }
        users.push(_user);
        dappToken.transfer(_user, entryToken);
        tokensUserHas[_user] = tokensUserHas[_user] + entryToken;
    }
    ...
    function enterGame(uint256 _guessedNumberFromUser, address _user)
        public
        payable
        returns (uint256, string memory)
    {
        ...
        if (guessedNumberFromUser == randomNumberGenerated) {
            tokensUserWon[_user]=tokensUserWon[_user]+winTokens;
            return (randomNumberGenerated, "you won");
        } else {
            tokensUserWon[_user]=tokensUserWon[_user]-loseTokens;
    ...
    function addGameTokensToUsers() public onlyOwner {
        for (uint256 usersIndex = 0; usersIndex > users.length; usersIndex++) {
