...
contract UsdtSwap is ReentrancyGuard {
    using SafeMath for uint256;
    ...
    uint256 public usdpRate;
    
    constructor(address _usdt, address _usdp) {
        admin = msg.sender;
        usdt = ERC20(_usdt);
        usdp = ERC20(_usdp);
        usdpRate = 99;
    ...
    function setUSDTRatePerMimatic(uint256 _rate) public {
        require(admin==msg.sender, "ser pls no hack");
        usdtRate = _rate;
    }

    function setUSDPRatePerUSDT(uint256 _rate) public {
        require(admin==msg.sender, "ser pls no hack");
        usdpRate = _rate;
    }
    ...
    function getReserves() public view returns(uint256, uint256) {
    	return ( usdp.balanceOf(address(this)), usdt.balanceOf(address(this)) );
    }
    ...
    function swapFrom(uint256 amount) public nonReentrant {
    	...
    	require(usdp.balanceOf(address(this))!=0, "swapFrom: Not enough miMatic in reserves");

	    // for every 1.02 usdt we get 1.00 mimatic
	    	// 1020000 we get 1000000000000000000

	    uint256 amountToSend = amount.mul(100000000000000).div(usdtRate);

	    require(usdp.balanceOf(address(this)) >= amountToSend, "swapFrom: Not enough miMatic in reserves");

	    // Transfer usdt to contract
	    usdt.transferFrom(msg.sender, address(this), amount);
	    // Transfer miMatic to sender
	    usdp.transfer(msg.sender, amountToSend);
	}
    ...
    function swapTo(uint256 amount) public nonReentrant {
	    ...
	    // for every 1.00 mimatic we get 0.98 usdt
	    	// 1000000000000000000 we get 980000 (bc decimals)
	    uint256 amountToSend = amount.mul(usdpRate).div(100000000000000);
