    ...
    AccessControlUpgradeable
{
    ...
    function initialize(IVempire _vempireNFT, uint256 _phaseOneStartTime, uint256 _phaseOneDuration, uint256 _phaseTwoStartTime, uint256 _phaseTwoDuration, uint256 _phaseThreeStartTime) public initializer  {
     ...
     */
    function validatePhaseSpecificPurchase(uint256 amount, uint256 phase, bytes32[] calldata proof) internal {
        ...
        if (phase == 1) {
            ...
            require(msg.value >= phaseOnePrice * amount, "ETHER SENT NOT CORRECT");
            require(merkleRootPhaseOne != "", "PERMISSIONED SALE CLOSED");
            require(MerkleProofUpgradeable.verify(proof, merkleRootPhaseOne, leaf), "INVALID PROOF");        
        } else if (phase == 2) {
            ...
            require(msg.value >= phaseTwoPrice * amount, "ETHER SENT NOT CORRECT");
            require(merkleRootPhaseTwo != "", "PERMISSIONED SALE CLOSED");
            require(MerkleProofUpgradeable.verify(proof, merkleRootPhaseOne, leaf), "INVALID PROOF");
        } else if (phase == 3) {
            require(block.timestamp >= phaseThreeStartTime, "PHASE ONE SALE HASN'T STARTED YET"); 
            require(msg.value >= phaseThreePrice * amount, "ETHER SENT NOT CORRECT");
        } else {
     ...
     */
    function buy(uint256 amount, uint256 phase, bytes32[] calldata proof) 
        external 
        payable {

        /// @dev Verify that user can perform permissioned sale based on the provided parameters.
        
        require(address(vempireNFT) != address(0), "JUNIOR NFT SMART CONTRACT NOT SET");
        require(phase > 0 && phase < 4, "INCORRECT PHASE SUPPLIED");
        
        require(amount > 0, "HAVE TO BUY AT LEAST 1");
        require(addressToTicketsPermissioned[msg.sender] + amount <= limit, "BUY AMOUNT EXCEEDS MAX FOR USER");            
        require(bought + amount <= maxSupply, "BUY AMOUNT GOES OVER MAX SUPPLY");
    ...
    /**
     * @notice Change the merkleRootPhaseOne of the sale.
     *
    ...
    /**
     * @notice Change the merkleRootPhaseTwo of the sale.
     *
    ...
    /**
     * @notice Change duration of the Phase Two permissioned sale.
     *
     * @param newDuration. The new duration.
     */
