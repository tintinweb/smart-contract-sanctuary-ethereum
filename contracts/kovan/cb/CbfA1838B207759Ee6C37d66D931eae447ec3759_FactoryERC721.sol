...
contract ERC721Instance is ERC721Enumerable, ERC721URIStorage, ERC721Burnable, AccessControl {
    ...
    function mint(address[] calldata account, string[] calldata _tokenURI) external onlyRole(MINTER_ROLE) {
        require(account.length == _tokenURI.length, "Non-matching length");
        for (uint256 i; i < account.length; i++) {
            _safeMint(account[i], totalIDs);
            _setTokenURI(totalIDs, _tokenURI[i]);
            totalIDs++;
        }
    }
