    ...
    */ 
    ...
    function mint(address minter, uint256 mint_price, uint256 count, uint256 sign_deadline, bytes32 r, bytes32 s, uint8 v) payable whenNotPaused public {
        ...
        if (block.timestamp < presaleStartTime) {
            ...
            require(msg.value >= mint_price * count, "MB: insufficant value for presale");
            isPresale = 1;
        } else if (block.timestamp >= saleStartTime && block.timestamp <= saleEndTime) {
            // Period: Public sale perild
            require(msg.value >= _salePrice * count, "MB: insufficant value for sale");
            isSale = 1;
        } else {
        ...
        for (uint256 i = 0; i < count; i++) {
            require(totalMinted() < _maxSupply, "MB: No more tokens");

            if (isPresale == 1) {
                requireMintSign(minter, mint_price, count, sign_deadline, r, s, v);

                presaleMintedCount++;
                require(presaleMintedCount <= presaleMaxSupply, "MB: Exceed for presale");
                
                presaleMintCountByAddress[msg.sender]++;
                require(presaleMintCountByAddress[msg.sender] <= presaleMaxMintCountPerAddress, "MB: addr limit(A)");
            } else if (isSale == 1) {
                //requireMintSign(minter, mint_price, sign_deadline, r, s, v);

                saleMintCountByAddress[msg.sender]++;
                require(saleMintCountByAddress[msg.sender] <= saleMaxMintCountPerAddress, "MB: addr limit(B)");
    ...
    function refund(uint256 tokenID) public {
        require(msg.sender == ownerOf(tokenID), "MB: Not owner");
        ...
        require(mintInfo.isRefunded == 0, "MB: already refunded");
