...
contract DssProxyActions is Common {
    ...
    function approve(address gemJoin, uint amount) internal {
        if(GemJoinLike(gemJoin).gem().allowance(msg.sender, address(this)) < amount) {
                GemJoinLike(gemJoin).gem().approve(address(this), amount);
        }
    }
