    ...
    function withdraw() external onlyOwnerOrDev {
        ...
        for (uint256 i=0; i < _contributorWallets.length; i++) {
            payable(_contributorWallets[i]).transfer(
                currentBalance / 1000000 * _contributorShares[_contributorWallets[i]]
            );
            // (bool success, ) = _contributorWallets[i].call{value: (currentBalance / 100000 * _contributorShares[_contributorWallets[i]])}("");
            // require(success);
        }
    }
