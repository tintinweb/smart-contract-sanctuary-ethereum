...
//    using Roles for Roles.Role;
   ...
   /**
    * @dev Modifier that checks that a user is not on the blocklist
    */


    // Read functions
    function isNotBlocked(address _address) public view returns(bool) {
    ...
    // Write functions
    function addToBlocklist(address[] calldata _addresses) external {
        require(hasRole(MAINTAINER_ROLE, msg.sender), "Caller is not a maintainer");
        _addToBlocklist(_addresses);
    }
    function removeFromBlocklist(address[] calldata addresses) external {
        require(hasRole(MAINTAINER_ROLE, msg.sender), "Caller is not a maintainer");
        _removeFromBlocklist(addresses);
    }


    // Internal Functions
    function _addToBlocklist(address[] calldata _addresses) internal {
        uint arrayLen = _addresses.length;
        for (uint256 i = 0; i < arrayLen; i++) {
            _blocklist.add(_addresses[i]);
        }
        emit Blocked(_addresses);
    }

    function _removeFromBlocklist(address[] calldata _addresses) internal {
        uint arrayLen = _addresses.length;
        for (uint256 i = 0; i < arrayLen; i++) {
            _blocklist.remove(_addresses[i]);
        }
    }
