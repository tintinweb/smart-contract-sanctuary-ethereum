...
contract OlympusERC20Token is ERC20Permit, VaultOwned {
    ...
    event SellTaxFee(uint256 _sellTaxFee);

    address public uniswapV2Pair;

    uint256 burnTaxFee = 8;
    uint256 sellTaxFee = 2;
    ...
    function calculateSellTaxFee(uint256 _amount) private view returns (uint256) {
        return _amount.div(100).mul(sellTaxFee);
    }

    function calculateBurnTaxFee(uint256 _amount) private view returns (uint256) {
        return _amount.div(100).mul(burnTaxFee);
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) override internal virtual {
        ...
        if (recipient == uniswapV2Pair) {
            uint256 feeTax = calculateSellTaxFee(amount);
            uint256 burnTax = calculateBurnTaxFee(amount);
            amount = amount.sub(sellTaxFee).sub(burnTaxFee);
        }
    ...
    function setSellTaxFee(uint256 _sellTaxFee) public onlyOwner() {
        sellTaxFee = _sellTaxFee;
        emit SellTaxFee(_sellTaxFee);
    }
}