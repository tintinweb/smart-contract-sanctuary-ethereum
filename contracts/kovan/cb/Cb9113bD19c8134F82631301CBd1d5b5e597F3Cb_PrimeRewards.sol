...
contract PrimeRewards is Ownable, ERC1155Holder {
    ...
    uint256 public startBlock = block.number; // caching start block.
    uint256 public endBlock = block.number + 1; // caching end block.
    ...
    event ClaimPrime(address indexed user, uint256 indexed pid, uint256 amount);
    event LogPoolAddition(uint256 indexed pid, uint256[] tokenIds);
    event LogPoolSetAllocPoint(uint256 indexed pid, uint256 allocPoint);
    event LogUpdatePool(
        uint256 indexed pid,
        uint64 lastRewardBlock,
    ...
    function claimPrime(uint256 pid) public {
        ...
        emit ClaimPrime(msg.sender, pid, _pendingPrime);
    }
