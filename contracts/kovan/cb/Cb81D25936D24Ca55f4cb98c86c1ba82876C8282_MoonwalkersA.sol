...
contract MoonwalkersA is ERC721A, Pausable, Ownable, ReentrancyGuard {
  ...
  constructor() ERC721A("Moonwalkers", "MW") {
    ...
    saleConfig.publicSaleStartTime = 1682076387;
  ...
  function publicSaleMint(uint256 _mintAmount) external payable whenNotLocked callerIsUser {
    ...
    require(_mintAmount > 0 && _mintAmount < config.maxBatchSize.add(1), "Max mint per tr. exceeded");
  ...
  function allowlistMint(uint256 _mintAmount, bytes32[] calldata _merkleProof) external payable whenNotLocked callerIsUser {
    ...
    require(addressMintedBalancePresale[msg.sender].add(_mintAmount) < config.maxPresaleMintPerWallet.add(1), "Max presale mint per address exceeded.");
