...
contract PoolMarketTokenERC20 is MarketToken {
   ...
   uint cashPrior;
    ...
    function mint(uint mintAmount) external returns (uint) {
        ...
        IERC20(underlying).safeTransfer(address(fundManager), mintAmount);
        fundManager.mint(msg.sender, mintAmount);
    ...
    function repayBorrow(uint repayAmount) external returns (uint) {
        ...
        IERC20(underlying).safeTransfer(address(fundManager), repayAmount);
        fundManager.repayBorrow(msg.sender, repayAmount);
    ...
    function repayBorrowBehalf(address borrower, uint repayAmount) external returns (uint) {
        ...
        IERC20(underlying).safeTransfer(address(fundManager), repayAmount);
        fundManager.repayBorrow(borrower, repayAmount);
    ...
    function liquidateBorrow(address borrower, uint repayAmount, MarketToken marketTokenCollateral) external returns (uint) {
        ...
        IERC20(underlying).safeTransfer(address(fundManager), repayAmount);
        fundManager.liquidateBorrow(borrower, repayAmount);
