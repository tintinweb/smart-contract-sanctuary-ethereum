...
interface IReferrals {
    ...
    function queryReferrerByAddress(address)
        external
        view
        returns (
            uint256,
...
contract BASCollection is ERC721Enumerable, Ownable, VRFConsumerBase {
     ...
     */
    uint256 internal link_fee = 0.1 * 10**18 wei; // 0.1 LINK (Rinkeby)
    address internal _link = 0x01BE23585060835E02B77ef475b0Cc51aA1e0709;
    ...
    mapping(uint256 => mapping(uint256 => uint256)) public typeCounts;
    // Represents a mask of the significant bits for a token value
    uint256 public Combinations = 0;
    Trait[] public TokenTraits;

    /**
     * Ensures that a given list of traits is valid
     */
    modifier ValidTraits(uint256[] memory traits) {
     ...
     */
    modifier ValidMint(uint256 mintNumber) {
        if (saleIsActive) {
            require(
                BASC_FEE.mul(mintNumber) == msg.value,
                "Ether value sent is not correct"
            );
            require(
                mintNumber <= maxBASCPurchase,
                "Can only mint 10 tokens at a time during presale"
            );
            require(
     ...
     */
    function traitIsValid(uint256 traitTypes)
        internal
        pure
    ...
    {
        require(
            keccak256(abi.encodePacked((tokenTraitNames[0]))) ==
                keccak256(abi.encodePacked(("Gender"))),
            "First Attribute must be a gender"
        );
        ...
        for (uint256 i = 0; i < tokenTraitNames.length; i++) {
            uint256 types = tokenTraits[i];
            uint256 mask = types - 1;
            uint256 masksize = 1;
     ...
     */
    function reveal(bytes32 requestId, uint256 revealAmount) public onlyOwner {
        require(
            requestIdToRandomness[requestId] != 0,
            "Randomness has not been fullfilled for this requestId yet."
        );
        require(
            (revealAmount.add(lastNFTRevealed)) <= MAX_BASC,
            "Reveal amount indexes past max number of tokens."
        );
        require(
            (revealAmount.add(lastNFTRevealed)) <= totalSupply(),
            "Reveal amount indexs past current total supply."
        );
        ...
        uint256 nextMintId = lastNFTRevealed + revealAmount;
        for (; lastNFTRevealed < nextMintId; lastNFTRevealed++) {
            uint256 randomValue = uint256(
                keccak256(abi.encode(randomness, lastNFTRevealed))
            );
            if (tokenRevealed[lastNFTRevealed] == false) {
                //Safty check to make sure token random values are not overwritten
     ...
     */
    function updateCounts(uint256 token) internal {
        uint256 gender = getGender(token);
        for (uint256 i = 0; i < TokenTraits.length; i++) {
            Trait memory trait = TokenTraits[i];
            uint256 traitType = getTrait(token, i);
            uint256 traitTypeIndex = uint256(
                keccak256(abi.encode(trait.name, traitType))
            );
     ...
     */
    function mintBASC(uint256 numberOfTokens, uint256 _refID)
        public
        payable
        ValidMint(numberOfTokens)
        returns (uint256 minterRefId)
    {
        require(
            totalSupply().add(numberOfTokens) <= MAX_BASC,
            "Purchase would exceed max supply of the BAS collection"
        );
        ...
        if (msg.sender != owner()) {
            ...
            minterRefId = referral.payReferrerAndRegister{
                value: referralFee.mul(numberOfTokens)
            }(_refID);
        }
    }
     ...
     */
    function getCollectable(uint256 token)
        public
        view
        returns (
     ...
     */
    function getTrait(uint256 token, uint256 trait_n)
        public
        view
        returns (uint256 traitType)
    {
        require(tokenRevealed[token], "Token has not yet been revealed.");
        require(trait_n < TokenTraits.length, "Index out of bounds.");
        uint256 randomValue = tokenValues[token];

        uint256 ops = trait_n;
        uint256 index = 0;
        // while there are operations remaining shift right the randomValue by the size of each traits mask_size
        while (ops > 0) {
            uint256 traitMaskSize = TokenTraits[index].mask_size;
            randomValue = randomValue >> traitMaskSize;
            ops--;
            index++;
        }

        uint256 maskedValue = randomValue & TokenTraits[trait_n].mask;
     ...
     */
    function getGender(uint256 token) internal view returns (uint256 gender) {
        require(tokenRevealed[token], "Token has not yet been revealed.");
    ...
    function getTraitCount(
        ...
        uint256 type_n,
        uint256 gender
    ) public view returns (uint256 type_count) {
        require(trait_n < TokenTraits.length, "Index out of bounds.");
        require(
            type_n < TokenTraits[trait_n].uniqueTypes,
            "Index out of bounds."
        );
        require(gender < 2, "Gender out of bounds");
        string memory traitName = TokenTraits[trait_n].name;
        type_count = typeCounts[gender][
            uint256(keccak256(abi.encode(traitName, type_n)))
        ];
    }
     ...
     */
    function getTokenTraitsLength() public view returns (uint256 length) {
     ...
     */
    function getAllTraits(uint256 token)
        public
        view
        returns (uint256[] memory)
    {
        uint256[] memory token_traits = new uint256[](TokenTraits.length);
        for (uint256 i = 0; i < TokenTraits.length; i++) {
        ...
        returns (uint256 rarity)
    {
        ...
        for (uint256 i = 0; i < TokenTraits.length; i++) {
            ...
            uint256 traitCount = TokenTraits[i].uniqueTypes;
            uint256 traitTypeCount = typeCounts[gender][
                uint256(keccak256(abi.encode(traitName, traitType)))
            ];
