...
contract AmazingPandaverse is Ownable, ERC721, VRFConsumerBase {
  ...
  event FulfillRandomness(address _sender, uint256 _tokenId);
  ...
  function fulfillRandomness(bytes32 requestId, uint256 randomness) internal override(VRFConsumerBase) {
      ...
      require(left > 0, "fulfillRandomness: left == 0");
      ...
      emit FulfillRandomness(to, tokenId);
  }
  ...
  function checkWhitelisting(
    ...
    uint256 approvedLimit) external view returns (bool) {
    bytes32 digest = keccak256(abi.encode(TYPEHASH, buyer, approvedLimit));
  ...
  function buyPresale(
    ...
    bytes32 digest = keccak256(abi.encode(TYPEHASH, msg.sender, approvedLimit));
    address signer = digest.toEthSignedMessageHash().recover(signature);
    require(
  ...
  function mint(address to, uint256 numberOfTokens) private {
    ...
    for (uint256 i = 0; i < numberOfTokens; i++) {
      total += 1;
      requestIdToAddress[requestRandomness(keyHash, fee)] = to;
    }
