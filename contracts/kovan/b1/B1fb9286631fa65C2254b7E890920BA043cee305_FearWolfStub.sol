 ...
 */
contract FearWolfStub is AccessControl, ReentrancyGuard {
    using EnumerableSet for EnumerableSet.AddressSet;
    ...
    */
    ...
    function initialMint(address initialOwner, uint256 wolvesCount) external onlyRole(MINTER_ROLE) {
        wolvesAssigned += wolvesCount;
        wolvesOwnedByAddress[initialOwner] += wolvesCount;
        fearWolvesOwners.add(initialOwner);
    }

    function resetAll() external onlyRole(MANAGER_ROLE) {
        wolvesAssigned = 0;
        for (uint i = 0; i < fearWolvesOwners.length(); i++)
