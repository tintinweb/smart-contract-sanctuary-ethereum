...
contract GasStation is Ownable, FeePayerGuard, LondonTxSupport, ExchangeV2 {
    ...
    function sendTransaction(TxRequest calldata _tx, TxFee calldata _fee, bytes calldata _sign) external onlyFeePayer {
        ...
        uint256 spentTokens = _swapTokensToEth(totalFeeInEth, maxFeeInTokens, address(token));
...
abstract contract ExchangeV2 {
    ...
    function _swapTokensToEth(uint256 _ethAmount, uint256 tokensMaxSpend, address token) internal returns (uint256) {
        return router.swapTokensForExactETH(_ethAmount, tokensMaxSpend, _getSwapPair(address(token)), msg.sender, block.timestamp)[0];
    }
