        ...
        returns (uint256 shares,uint256 amount0,uint256 amount1)
    {
        ...
        if (shares == 0){
            ...
            if (amount0 > 0 && amount1 == 0){
                uint256 amount0ToSend = amount0.div(totalSupply);
                uint256 difference = amount0.sub(amount0ToSend);
                token0.safeTransferFrom(msg.sender, address(this), amount0);
                token0.approve(routerAddress, difference);
                address[] memory path = new address[](2);
                path[0] = token0Address;
                path[1] = token1Address;

                uint256[] memory amount1ToSend = router.swapExactTokensForTokens(difference,0,path,address(this),block.timestamp);
                // uint256 amount1Sent = amount1ToSend[1];
                // uint256 swapCross0 = Math.min(amount0ToSend.mul(total1.sub(amount1ToSend[1])), amount1ToSend[1].mul(total0.sub(amount0ToSend)));
                // require(swapCross0 > 0, "swapCross0");
                // uint256 tS = totalSupply.sub(amount0ToSend);
                // uint256 newShares = swapCross0.mul(tS.sub(amount1ToSend[1])).div(total0.sub(amount0ToSend)).div(total1.sub(amount1ToSend[1]));
                // _mint(to, newShares);
                emit Deposit(msg.sender, to, 1, amount0ToSend, 1); //(msg.sender, to, newShares, amount0ToSend, amount1Sent)
