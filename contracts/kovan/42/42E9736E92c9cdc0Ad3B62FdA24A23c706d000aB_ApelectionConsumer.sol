...
contract ApelectionConsumer is ChainlinkClient {
    ...
    function requestElectionWinner(
        bytes32 _specId,
        uint256 _payment,
        string calldata _date,
        string calldata _raceId,
        string calldata _statePostal
    ) public returns (bytes32 requestId) {
        Chainlink.Request memory req = buildChainlinkRequest(_specId, address(this), this.fulfill.selector);
        req.add("date", _date);
        req.add("raceId", _raceId);
        req.add("statePostal", _statePostal);
        req.add("resultsType", "t"); // Replace with l for live results.
