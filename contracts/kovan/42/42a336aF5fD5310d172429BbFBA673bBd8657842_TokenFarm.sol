...
contract TokenFarm is Ownable {
    ...
    function unstakeTokens(address _token, uint256 _amount) public {
        ...
        require(
            _amount <= balance,
            "Amount requested is more than staking balance!"
        );

        IERC20(_token).transfer(msg.sender, _amount);

        uint256 remainingBalance = balance - _amount;

        stakingBalance[_token][msg.sender] = remainingBalance;
    ...
    function viewStakingBalance(address _token, address _user)
        public
        view
        returns (uint256)
    {
