    ...
    UUPSUpgradeable
{
        ...
        returns (uint256)
    {
        ...
        require(_gaming[_msgSender()][game_].gameType == 3);
            //Base
            ...
            if(gameResult[_gaming[_msgSender()][game_].fixtureID].status.equal(hex"66696e6973686564"))
                {
                    if ( 
            ...
            else if(sliceInterruptedBytes(_gaming[_msgSender()][game_].fixtureID)
                        .equal(hex"696e746572727570746564")) {
                reward = amount;
            }
    ...
    { 
        ...
        if (gameType == 1) {
            ...
            for(uint256 i = 0; i < _gamingSlip[_msgSender()][game_].slips.length; i++ ){
                ...
                if(gameResult[_gamingSlip[_msgSender()][game_].slips[i].fixtureID].status.equal(hex"66696e6973686564"))
                {
                    ...
                    //Compensate Other Winners Flexibly with 20%
                    else {
                        reward = (2 * amount).div(10);
                        break;
                    }
                } else if(sliceInterruptedBytes(_gamingSlip[_msgSender()][game_].slips[i].fixtureID)
                        .equal(hex"696e746572727570746564")) {
  ...
  //Convert String to Uint
    ...
    function sliceFinishedBytes(uint256 _fixtureID, bytes memory _bytes) internal {
    ...
    function sliceInterruptedBytes(uint256 _fixtureID) internal view returns(bytes memory) {
        IScorefamConsumer sc = IScorefamConsumer(consumer);
        return sc.requestIdData(sc.gameScoreRequestID(_fixtureID)).slice(2,11);
    }
    ...
    function updateGameResult(uint256 fixtureID) internal {
        IScorefamConsumer sc = IScorefamConsumer(consumer);
        sliceFinishedBytes(fixtureID, sc.requestIdData(sc.gameScoreRequestID(fixtureID)));
    }
    ...
    function claimGameReward(uint256 game_) external returns (bool){
        require(block.timestamp > _gaming[_msgSender()][game_].gameStartTime
                .add(_gameTypeOptions[_gaming[_msgSender()][game_].gameType].gameLockTime));
        ...
        if(!gameResult[fixtureID].retrieved){
            updateGameResult(fixtureID);
        }
        
        require(gameResult[fixtureID].retrieved);
        require(gameResult[fixtureID]
                        .status
                        .equal(hex"66696e6973686564") 
                        || 
                        sliceInterruptedBytes(fixtureID)
                        .equal(hex"696e746572727570746564"));
    ...
    function updateSlipScore(uint256 game_) external {
        ...
        for(uint256 i = 0; i < _gamingSlip[_msgSender()][game_].slips.length; i++){
            ...
            result = IScorefamConsumer(consumer).requestIdData(IScorefamConsumer(consumer).gameScoreRequestID(fixtureID));
            if(result.length == 0 || result.slice(2,10).equal(hex"6e6f7473746172746564")){
                //Update Game Status, Home and Away Scores from the Oracle
                bytes32 JOB_ID = bytes32("7a1ade57c05f49f1bd5c25241acdb107");       
    ...
    function claimSlipReward(uint256 game_) external returns (bool){
        ...
        for(uint256 i = 0; i < _gamingSlip[_msgSender()][game_].slips.length; i++){
            if(!gameResult[_gamingSlip[_msgSender()][game_].slips[i].fixtureID].retrieved){
                updateGameResult(_gamingSlip[_msgSender()][game_].slips[i].fixtureID);
            }
            require(gameResult[_gamingSlip[_msgSender()][game_].slips[i].fixtureID].retrieved);
            require(gameResult[_gamingSlip[_msgSender()][game_].slips[i]
                        .fixtureID]
                        .status
                        .equal(hex"66696e6973686564") 
                        || 
                        sliceInterruptedBytes(_gamingSlip[_msgSender()][game_].slips[i].fixtureID)
                        .equal(hex"696e746572727570746564"));        
            }
