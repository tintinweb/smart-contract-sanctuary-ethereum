...
import "ShambaGeoParametricInsurance.sol";

contract OracleFacingParametricInsuranceProvider is InsuranceProvider {}
...
contract InsuranceContract is ChainlinkClient  {
    ...
    uint public constant DAY_IN_SECONDS = 60; //How many seconds in a day. 60 for testing, 86400 for Production
    uint public constant DROUGHT_DAYS_THRESDHOLD = 3 ;  //Number of consecutive days without geostats data to be defined as a drought
    ...
    int256[2] public currentGeostatsList;
    ...
    constructor(address payable _client, uint _duration, uint _premium, uint _payoutValue, 
        ...
        oracles[1] = 0xe2edEB50A3248256A3c56f782f6BB4157B08549c;
        jobIds[0] = 'a8c9590bae904f328eb155f10d4ac841';
        jobIds[1] = '2f92a2bc36b34aaa9316ee1b3ce84f23';
   ...
   /**
    ...
    function updateContract(
        ...
        if (contractActive) {
            ...
            checkGeostats(oracles[0], jobIds[0], agg_x, dataset_code, selected_band, image_scale, start_date, end_date, geometry);
            checkGeostats(oracles[1], jobIds[1], agg_x, dataset_code, selected_band, image_scale, start_date, end_date, geometry);    
        }
     ...
     * @dev Calls out to an Oracle to obtain weather data
     */ 
    function checkGeostats(address _oracle, bytes32 _jobId, 
        string memory agg_x,
        string memory dataset_code,
        string memory selected_band,
        string memory image_scale,
        ...
        string memory end_date,
        Geometry[] memory geometry
    ) private onContractActive() returns (bytes32 requestId)   {
        ...
        Chainlink.Request memory req = buildChainlinkRequest(_jobId, address(this), this.checkGeostatsCallBack.selector);
     ...
     * The function will take the geostats given by the Oracle and updated the Inusrance Contract state
     */ 
    function checkGeostatsCallBack(bytes32 _requestId, int256 _geostats, string calldata cidValue) public recordChainlinkFulfillment(_requestId) onContractActive() callFrequencyOncePerDay()  {
       ...
       currentGeostatsList[dataRequestsSent] = _geostats; 
       dataRequestsSent = dataRequestsSent + 1;
       
       //set current geostats to average of both values
       if (dataRequestsSent > 1) {
          currentGeostats = median(currentGeostatsList, currentGeostatsList.length);
          currentGeostatsDateChecked =  block.timestamp;
          requestCount +=1;
          cid = cidValue;
          ...
          if (daysWithoutGeostats >= DROUGHT_DAYS_THRESDHOLD) {  // day threshold has been met
              //need to pay client out insurance amount
              payOutContract();
          }
       }
       
       emit dataReceived(_geostats);
