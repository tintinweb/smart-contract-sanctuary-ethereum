...
contract TokenFactoryNFT is TokenFactoryNFTStorage, ITokenFactoryNFT, AccessControlEnumerableUpgradeable/*, OwnableUpgradeable */{
    ...
    // modifier onlySuperFactory() {
    //     require(superFactoryAddress == msg.sender, "Not a super factory");
    //     _;
    // }

    // modifier onlyOwnerOrSuperFactory() {
    //     require(_owner == msg.sender || superFactoryAddress == msg.sender, "Not an owner or a super factory");
    //     _;
    // }

    // function setNewSuperFactory(address _superFactoryAddress) external override onlyAdmin {
    //     require(_superFactoryAddress != address(0));
    //     superFactoryAddress = _superFactoryAddress;
    // }

    // /**
    // * @dev check if a token contract address was deployed
    // * @param _tokenAddress, address to check
    // * @return true if token address was deployed, otherwise false
    // */
    // function isTokenDeployed(address _tokenAddress) external view returns (bool) {
    //     return tokenDeployed[_tokenAddress];
    // }

    // /**
    // * @dev get deployed asset contract counter
    // * @return number of deployed asset contracts
    // */
    // function getDeployedTokenCounter() external view returns (uint256) {
    //     return tokenCounter;
    // }
    ...
    // * @return address of idx-th asset contract address
    // */
    ...
    // /**
    // * @dev check if a token WL contract address was deployed
    // * @param _tokenWLAddress address to check
    // * @return true if token WL address was deployed, otherwise false
    // */
    // function isTokenWLDeployed(address _tokenWLAddress) external view returns (bool) {
    //     return tokenWLDeployed[_tokenWLAddress];
    // }

    // /**
    // * @dev get deployed token WL contract address as an item of an array
    // * @param _tokenWLAddress, token address
    // */
    // function getDeployedTokenWLAddress(address _tokenWLAddress) external view returns(address) {
    //     return whitelists[_tokenWLAddress];
    // }
    ...
    */
    ...
    /**
    * @dev deploy a new token contract, add its address to internal variables and change the ownership to new owner address
    ...
    * @param _creatorCollectorAddress creator fees collector address where fees are goint to be collected
    * @param _biPercent, BI fees percentage
    ...
    */
    function deployTokenNFT(string memory _name, 
            ...
            address _creatorCollectorAddress,
            uint256 _biPercent,
            uint256 _creatorPercent,
            uint256 _baseCollectionPrice,
            bool _waitToTransferOwner) public override onlyAdmin returns (address) {
        TokenNFT newToken = new TokenNFT(); //(_name, _symbol, _baseTokenURI, _creatorAddress, _creatorCollectorAddress, _biPercent, _creatorPercent, _baseCollectionPrice);
        newToken.initialize(_name, _symbol, _baseTokenURI, _creatorAddress, 
                _creatorCollectorAddress, _biPercent, _creatorPercent, _baseCollectionPrice);
        addTokenContractAddress(address(newToken));
        if (!_waitToTransferOwner)
            newToken.transferOwnership(_creatorAddress);
        emit TokenCreatedNFT(address(newToken), _name, _symbol, _baseTokenURI, _creatorAddress);
        return address(newToken);
    }

    /**
    * @dev deploy a new token WL contract, add its address to internal variables and change the ownership to new owner address
    * @param _tokenAddress, token address
    * @param _creator, creator address
    * @return address of the deployed token WL contract
    */
    function deployTokenWL(address _tokenAddress, address _creator) public override onlyAdmin returns (address) {
        require(tokenDeployed[_tokenAddress], "Tank index not allowed");
        ...
        emit TokenNFTWLCreated(_tokenAddress, address(newTokenWL), _creator);
        return address(newTokenWL);
    }

    /**
    * @dev deploy a new token and a new token WL contract calling internal functions
    * @param _name, name of the asset to be deployed
    * @param _symbol, symbol of the asset to be deployed
    * @param _baseTokenURI, base token URI
    * @param _creatorAddress, creator address 
    ...
    */
    function deployTokenWithWL(string memory _name, 
            ...
            uint256 _biPercent,
            uint256 _creatorPercent,
            uint256 _baseCollectionPrice) external override onlyAdmin returns (address, address) {
        address newTokenAddress = deployTokenNFT(_name, _symbol, _baseTokenURI, _creatorAddress, 
                _creatorCollectorAddress, _biPercent, _creatorPercent, _baseCollectionPrice, true);
        address newTokenWLaddress = deployTokenWL(newTokenAddress, _creatorAddress);
        TokenNFT(newTokenAddress).transferOwnership(_creatorAddress);
...
interface ITokenNFT {
    ...
    function getBIFeePercent() external view returns (uint256);
...
interface ITokenFactoryNFT {
    ...
    function deployTokenNFT(string calldata _name, 
            ...
            uint256 _creatorPercent,
            uint256 _baseCollectionPrice,
            bool _waitToTrancsferOwner) external returns (address);
    function deployTokenWithWL(string calldata _name, 
            ...
            uint256 _biPercent,
            uint256 _creatorPercent,
            uint256 _baseCollectionPrice) external returns (address, address);
    // function setNewSuperFactory(address _superFactoryAddress) external;

    event TokenCreatedNFT(
...
import "./TokenWLStorage.sol";

contract TokenWhitelist is TokenWLStorage, OwnableUpgradeable, ITokenWhitelist {
    using SafeMathUpgradeable for uint256;
     ...
     */
    function addToWhitelist(address _subscriber) external override onlyWLOrOwner {
        require(_subscriber != address(0), "_subscriber is zero");
        require(!_whitelist[_subscriber], "already whitelisted");

        _whitelist[_subscriber] = true;
        _whitelistLength++;
     ...
     */
    function removeFromWhitelist(address _subscriber) external override onlyWLOrOwner {
        require(_subscriber != address(0), "_subscriber is zero");
        require(_whitelist[_subscriber], "not whitelisted");

        _whitelist[_subscriber] = false;
        _whitelistLength--;
    ...
    function addToWhitelistMassive(address[] calldata _subscriber) external override onlyWLOrOwner returns (bool _success) {
        ...
        for (uint8 i = 0; i < _subscriber.length; i++) {
            ...
            require(!_whitelist[_subscriber[i]], "already whitelisted");
...
contract TokenNFTStorage {
    ...
    address public creatorCollectorAddress;
...
contract TokenNFT is TokenNFTStorage, OwnableUpgradeable, AccessControlEnumerableUpgradeable, 
                    ERC721EnumerableUpgradeable, ERC721PausableUpgradeable, ERC2981Upgradeable, ITokenNFT/*, ContextMixin*/ {
    ...
    function initialize(string memory _name, 
            ...
            uint256 _biPercent,
            uint256 _creatorPercent,
            uint256 _baseCollectionPrice) public initializer {
        OwnableUpgradeable.__Ownable_init();
        __ERC721_init(_name, _symbol);
        baseTokenURI = _baseTokenURI;
        creatorAddress = _creatorAddress;
        creatorCollectorAddress = _creatorCollectorAddress;
        biPercent = _biPercent;
        creatorPercent = _creatorPercent;
        nftFactoryAddress = msg.sender;

        baseCollectionPrice = _baseCollectionPrice;

        _setupRole(DEFAULT_ADMIN_ROLE, creatorAddress);
        _setupRole(MINTER_ROLE, creatorAddress);
        _setupRole(PAUSER_ROLE, creatorAddress);
        _setupRole(URI_SETTER_ROLE, creatorAddress);
    }
    ...
    modifier factoryOrOwnerOnly() {
        require(nftFactoryAddress == msg.sender || owner() == msg.sender, "Not owner or factory");
        _;
    }

    function setTokenWLAddress(address _tokenWLAddr) external factoryOrOwnerOnly {
        require(_tokenWLAddr != address(0), "WL address not allowed");
        tokenWLAddress = _tokenWLAddr;
    }

    //  function getTokenWLAddress() external view returns (address) {
    //     return tokenWLAddress;
    // }

    function getBIFeePercent() external view override returns (uint256) {
        return biPercent;
    }
    ...
    */
    function setURIBase(string memory _uriBase) public onlyRole(URI_SETTER_ROLE) {
        baseTokenURI = _uriBase;
    }
     ...
     */
    function tokenURI(uint256 tokenId) public view override(ERC721Upgradeable, ITokenNFT) returns (string memory) {
    ...
    */
    function mint(address to) public payable whenNotPaused {
        require(tokenWLAddress == address(0) || (tokenWLAddress != address(0) && ITokenWhitelist(tokenWLAddress).isWhitelisted(to)), "TokenNFT: recipient not whitelisted");
    ...
    */
    function mintTokenID(address to, uint256 tokenId) public payable whenNotPaused onlyRole(MINTER_ROLE) {
        require(tokenWLAddress == address(0) || (tokenWLAddress != address(0) && ITokenWhitelist(tokenWLAddress).isWhitelisted(to)), "TokenNFT: recipient not whitelisted");
        require(hasRole(MINTER_ROLE, _msgSender()), "TokenNFT: must have minter role to mint");
        require(msg.value >= baseCollectionPrice, "TokenNFT: not enough eth to mint");
...
/*
    ...
    */
/*    function mintComplete(address to, uint256 tokenId, string memory _tokenURI) public whenNotPaused onlyRole(MINTER_ROLE) {
        require(tokenWLAddress == address(0) || (tokenWLAddress != address(0) && ITokenWhitelist(tokenWLAddress).isWhitelisted(to)), "TokenNFT: recipient not whitelisted");
        require(hasRole(MINTER_ROLE, _msgSender()), "TokenNFT: must have minter role to mint");
...
*/
    ...
    */
    ...
    function supportsInterface(bytes4 interfaceId) public view 
            override(ERC721Upgradeable, ERC721EnumerableUpgradeable, AccessControlEnumerableUpgradeable, ERC2981Upgradeable) returns (bool) {
        return (
    ...
    function _setRoyalties(address newRecipient) internal {
        require(newRecipient != address(0), "Token1155: new recipient is the zero address");
        creatorCollectorAddress = newRecipient;
    }

    function setRoyalties(address newRecipient) external creatorOnly {
    ...
    {
        return (creatorCollectorAddress, (_salePrice * 1000) / 10000);
    }
...
contract TokenFactoryNFTStorage {
    // address public superFactoryAddress;
