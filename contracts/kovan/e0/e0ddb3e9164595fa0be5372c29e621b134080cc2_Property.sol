...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////
   ...
   //设置链图片
   function setChainImgUrl(string memory imgUrl) public {
       chainImgUrl = imgUrl;
   }
    ...
    stableListPar[] public stableList;
    function setStableList(address addr,string memory name,string memory imgUrl) public {
        for (uint i=0;i<stableList.length;i++){
            require(stableList[i].addr != addr,"DONOT_SAME");
        }
        stableListPar memory m;
        m.addr = addr;
        m.name = name;
        m.imgUrl = imgUrl;
        m.decimals = getTokenDecimals(addr);
        m.ctime = block.timestamp;
        stableList.push(m);
    }

    function updateStableList(uint num,address addr,string memory name,string memory imgUrl,uint state) public {
    ...
    function setColla(
        ...
        collaList[collaAddr].imgUrl = imgUrl;
    ...
    function withdraw(address USDAddr,address stableAddr,uint amount)public {
        ...
        bool isStable = false;
        uint stableDecimals;
        for (uint j=0;j<stableList.length;j++){
            if (stableList[j].addr == stableAddr && stableList[j].state==0){
                isStable = true;
                stableDecimals = stableList[j].decimals;
                break;
            }
        }
        require(isStable,"stableAddr_error");
        uint assetAmount;
        uint num;
   ...
   function getUserAsset(address userAddr) public view returns(string memory){
    ...
    function getUserCont(address userAddr) public view returns(string memory){
        ...
        for (uint i=0;i<lengthNum;i++){
            ...
            conList = string(abi.encodePacked(conList,'"collaName":"'));
            conList = string(abi.encodePacked(conList,collaList[collaAddr].name));
            conList = string(abi.encodePacked(conList,'",'));

            conList = string(abi.encodePacked(conList,'"collaDecimals":'));
            conList = string(abi.encodePacked(conList,uint2str(collaDecimals)));
            conList = string(abi.encodePacked(conList,','));
            ...
            conList = string(abi.encodePacked(conList,'"USDName":"'));
            conList = string(abi.encodePacked(conList,USDAddrList[contList[contId].USDAddr].name));
            conList = string(abi.encodePacked(conList,'",'));

            conList = string(abi.encodePacked(conList,'"USDDecimals":'));
            conList = string(abi.encodePacked(conList,uint2str(USDDecimals)));
            conList = string(abi.encodePacked(conList,','));

            //图片url
            conList = string(abi.encodePacked(conList,'"imgUrl":"'));
            conList = string(abi.encodePacked(conList,collaList[collaAddr].imgUrl));
            conList = string(abi.encodePacked(conList,'",'));

            conList = string(abi.encodePacked(conList,'"chainImgUrl":"'));
            conList = string(abi.encodePacked(conList,chainImgUrl));
            conList = string(abi.encodePacked(conList,'",'));
    ...
    //提币列表
    function getStableList() public view returns(string memory){
        uint lengthNum = stableList.length;
        string memory asset = '[';
        for (uint i=0; i<lengthNum; i++){
            if (stableList[i].state == 0){
                asset = string(abi.encodePacked(asset,"{"));

                asset = string(abi.encodePacked(asset,'"address":"'));
                asset = string(abi.encodePacked(asset,addressToSting(stableList[i].addr)));
                asset = string(abi.encodePacked(asset,'",'));

                asset = string(abi.encodePacked(asset,'"name":"'));
                asset = string(abi.encodePacked(asset,stableList[i].name));
                asset = string(abi.encodePacked(asset,'",'));

                asset = string(abi.encodePacked(asset,'"imgUrl":"'));
                asset = string(abi.encodePacked(asset,stableList[i].imgUrl));
                asset = string(abi.encodePacked(asset,'",'));
            }
