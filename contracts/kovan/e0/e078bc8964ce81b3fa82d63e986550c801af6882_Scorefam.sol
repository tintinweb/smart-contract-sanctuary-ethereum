    ...
    UUPSUpgradeable
{
    ...
    mapping(address => uint256) private _totalGamesClaimed;
    ...
    function getGameRemainingLockTime(uint256 game_) public view returns(uint256) {
    ...
    function getGames() external view returns (uint256[7][] memory) {
        ...
        for (uint256 i = 1; i <= _gamesCount[_msgSender()]; i++) {
            ...
            // tempStakeList[i][7] = getGameRemainingLockTime(i);
        }
    ...
    function getSlips() external view returns (uint256[5][] memory) {
        ...
        for (uint256 i = 1; i <= _slipsCount[_msgSender()]; i++) {
            ...
            // temp[i][5] = getGameRemainingLockTime(i);
