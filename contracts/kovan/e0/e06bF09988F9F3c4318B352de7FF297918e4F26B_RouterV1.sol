...
contract RouterV1 {
    ...
    function trancheCount(IBondController bond) public view returns (uint256) {
        return bond.getTrancheData().trancheCount;
    }

    function tranche(IBondController bond, uint256 index) public view returns (ITranche) {
        return bond.getTrancheData().tranches[index];
    }

    function trancheRatio(IBondController bond, uint256 index) public view returns (uint256) {
        return bond.getTrancheData().trancheRatios[index];
    }
