...
contract EtermonSale is Ownable {
    ...
    uint256 private constant _remainingDistroPercentage = 93;
    ...
    // 7% at TGE, 93% released daily over 540 Days, no Cliff
        ...
        returns (uint256 availablePercentage)
    {
        ...
        // 93 days duaration 
        // x * (x *.93) / int(days elapsed/30)  = every period released
        ...
        if (currentTimeStamp > _initialTimestamp) {
            if (currentTimeStamp <= oneDays) {
                ...
                return (uint256(7) * 1e18) + currentUnlockedPercentage;
            } else {
