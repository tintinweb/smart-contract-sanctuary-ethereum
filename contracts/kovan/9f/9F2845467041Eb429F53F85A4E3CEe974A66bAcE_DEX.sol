...
contract DEX {
    ...
    event Log(uint256 EthReserve, uint256 EthValue, uint256 TokenReserve);
    ...
    function withdraw(uint256 liqAmount) public returns (uint256, uint256) {
        require(
            liquidity[msg.sender] >= liqAmount,
            "withdraw: sender does not have enough liquidity to withdraw."
        );
        uint256 ethReserve = address(this).balance;
        uint256 tokenReserve = token.balanceOf(address(this));
        uint256 ethWithdrawn;
        ethWithdrawn = liqAmount.mul(ethReserve) / totalLiquidity;
        uint256 tokenAmount = liqAmount.mul(tokenReserve) / totalLiquidity;

        liquidity[msg.sender] = liquidity[msg.sender].sub(liqAmount);
        totalLiquidity = totalLiquidity.sub(liqAmount);
        (bool sent, ) = payable(msg.sender).call{value: ethWithdrawn}("");
        require(sent, "withdraw(): revert in transferring eth to you!");
        require(token.transfer(msg.sender, tokenAmount));
        emit LiquidityRemoved(msg.sender, liqAmount, ethWithdrawn, tokenAmount);
