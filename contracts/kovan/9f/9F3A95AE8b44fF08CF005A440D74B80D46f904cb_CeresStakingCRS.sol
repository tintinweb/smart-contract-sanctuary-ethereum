...
import "../interface/IMintReceiver.sol";
import "../interface/IReinvestReceiver.sol";
import "../common/Ownable.sol";

contract CeresStakingCRS is CeresStakingRewards, IMintReceiver, IReinvestReceiver, Ownable {
    ...
    function stake(uint256 amount) public override nonReentrant updateReward(msg.sender) updateMint(msg.sender) {
        require(amount > 0, "StakingCRS: cannot stake 0");

        SafeERC20.safeTransferFrom(crs, msg.sender, address(this), amount);
        _stake(msg.sender, amount);
    }

    function withdraw(uint256 shareAmount) public override nonReentrant updateReward(msg.sender) updateMint(msg.sender) {
        require(shareAmount > 0 && shareAmount <= unlockedShareBalanceOf(msg.sender), "StakingCRS: your share balance is not enough!");
        ...
        SafeERC20.safeTransfer(crs, msg.sender, withdrawAmount);
    ...
    function withdrawAll(uint256 shareAmount) external {
        ...
        require(shareAmount <= unlockedShareBalance, "StakingCRS: Unlocked share balance not enough!");
    ...
    function claimMintWithPercent(uint256 percent) public override nonReentrant updateMint(msg.sender) {
        require(percent > 0 && percent <= 100, "StakingCRS: percent wrong");
        uint256 mint = mints[msg.sender] * percent / 100;
        if (mint > 0) {
            mints[msg.sender] -= mint;
            SafeERC20.safeTransfer(asc, msg.sender, mint);
            emit MintPaid(msg.sender, mint);
        }
    }
    ...
    function reinvestMint() public override nonReentrant updateMint(msg.sender) {
        ...
        if (mint > 0) {
            ...
            SafeERC20.safeTransfer(asc, stakingASC, mint);
            IReinvestReceiver(stakingASC).notifyReinvest(msg.sender, mint);
        }
    }
    ...
    /* ---------- Public Func: Notifies ---------- */

    function notifyMint(uint256 amountMint, uint256 amountBurn) public override onlyStakings {

        uint256 leftStaking = _totalStaking - amountBurn;
        require(crs.balanceOf(address(this)) >= leftStaking, "StakingCRS: Staking left balance is not enough!");
    ...
    function notifyReinvest(address account, uint256 amount) external override updateMint(account) updateReward(account) onlyStakings {
        require(_totalStaking + amount <= crs.balanceOf(address(this)), "StakingCRS: Notify amount exceeds balance!");
