    ...
    AccessControl
{
    ...
    event Rerange(address poolAddress, int24 baseLower, int24 baseUpper);
        ...
        )
    {
        ...
        if (operationMode == 2) 
            require(NFT_TOKEN.ownersToIds(sender) != 0, "invalid invitation");
    ...
    function rerange(int24 _tickRange, uint8 _poolId) external onlyStrategy {
        ...
        emit Rerange(address(currentPool.pool), _baseLower, _baseUpper);
        ...
        emit Snapshot(address(currentPool.pool), _tick, balance0, balance1, totalSupply());
