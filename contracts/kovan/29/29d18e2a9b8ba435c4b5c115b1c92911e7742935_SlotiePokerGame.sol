...
contract SlotiePokerGame is WattsBurnerUpgradable {
    ...
    function ReleasePrizePoolToWinner(address winner, uint256 amountFromPrizePool) external onlyRole(GameAdminRole) {
        _releaseWatts(winner, amountFromPrizePool);
    }

    function BurnRemainingWattsInPrizePool() external onlyRole(GameAdminRole) {
        _burnRemainingWatts();
    }
}
...
contract WattsBurnerUpgradable is AccessControlUpgradeable {
    ...
    function _burnRemainingWatts() internal {
        ...
        if (balance > 0) {
            watts.burn(address(this), balance);
        }
    }
