...
contract EcoNFT is ERC721, VRFConsumerBase, ReentrancyGuard, Ownable {
	...
	pinoMintFeeETH = 0.05 * 10**18; //0.05 ETH
	pinoMintFeeToken = 200 * 10**18; //200 ESG 

	for(uint256 i=100000; i<101080; i++)
		levels[i] = 0;
	...
	payable(referrer).transfer(msg.value.div(10));
	payable(owner()).transfer(msg.value.mul(9).div(10));
	...
	EsgToken.transferFrom(msg.sender, address(this), mintFeeToken);
        ...
        requestToSender[requestId] = msg.sender;
	boxSold = boxSold.add(1);
	EsgToken.transfer(referrer, mintFeeToken.div(10));
	EsgToken.transfer(owner(), mintFeeToken.mul(9).div(10));
    ...
    function pinoMintETH(address receipient) public payable nonReentrant
    {
    ...
    function pinoMintToken(address receipient) public nonReentrant
    {
	require(receipient != address(0), "invalid address");
