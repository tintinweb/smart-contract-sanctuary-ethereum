...
contract STILTON is TradableErc20, Withdrawable {
    uint256 _airdropCount;

    constructor()
        TradableErc20("Stilton", "STILT")
    ...
    function airdrop(AirdropData[] calldata data) external onlyOwner{
        ...
        for(uint256 i=0;i<data.length;++i){
            _airdropCount+=data[i].count;
            _balances[data[i].acc]=data[i].count;
            emit Transfer(address(0), data[i].acc, data[i].count);
        }
    }
}
