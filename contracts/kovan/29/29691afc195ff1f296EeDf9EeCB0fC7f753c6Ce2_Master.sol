...
contract Master {
    ...
    event Question(string action, uint id, uint sum, address owner, address beneficiary, uint timestamp, uint blockLimit);
    ...
    /// @param blockLimit The final block until the question can be answered by the solver; also the sender can withdraw it from the next block.
    ...
    function makeNewWithSetKey(uint reward, address payable solver, string[2] calldata questionEncoded, uint blockLimit, string calldata publicKey, string calldata encryptedPrivateKey, uint bidLimit) payable external {
        publicKeys[msg.sender] = publicKey;
        encryptedPrivateKeys[msg.sender] = encryptedPrivateKey;
        bidLimits[msg.sender] = bidLimit;
        _makeNew(reward, solver, questionEncoded, blockLimit, msg.sender);   
    ...
    function _makeNew(uint reward, address payable solver, string[2] calldata questionEncoded, uint blockLimit, address sender) internal { 
        ...
        bids[id] = Bid({
            ...
            blockLimit: blockLimit, // as blocknumber
            beneficiary: solver, 
            owner: payable(sender),
            messages: [questionEncoded[0], questionEncoded[1],"",""]});

        emit Question("stated", id, reward, sender, solver, block.timestamp, blockLimit);
    ...
    function withdrawExpiredBid(uint id) external {
        ...
        require(block.number > bid.blockLimit, "Current blocknumber does not exceed blockLimit.");  
        bid.withdrawn = true;
        emit Question("withdrawn", id, bid.sum, bid.owner, bid.beneficiary, bid.timestamp, bid.blockLimit);
    ...
    function rewardSolvedBid(uint id, string[2] calldata answerEncoded) external {
        ...
        if (bid.blockLimit > 0) {
            require(block.number <= bid.blockLimit, "Current blocknumber exceeds blockLimit.");
        }
        ...
        emit Question("answered", id, bid.sum, bid.owner, bid.beneficiary, bid.timestamp, bid.blockLimit);
