...
// imports other contracts
import "./aave/FlashLoanReceiverBaseV2.sol";
import "./aave/ILendingPoolAddressesProviderV2.sol";
import "./aave/ILendingPoolV2.sol";
import "hardhat/console.sol";

// creates the contract and inherits FlashLoanReceiverBase
contract Flasher is FlashLoanReceiverBaseV2, Withdrawable {

    // uses SafeMath
    using SafeMath for uint256;

    // constructor function runs on creation
    constructor(address _addressProvider) FlashLoanReceiverBaseV2(_addressProvider) {
        console.log("Deploying the Flasher contract");
    }
    ...
    function _flashloan(address[] memory assets, uint256[] memory amounts) internal {
        ...
        console.log("address(this) is: %s, and msg.sender is %s", address(this), msg.sender);
        bytes memory params = "";
        ...
        LENDING_POOL.flashLoan(
            receiverAddress, // address to recieve the loaned assets
            assets, // asset(s) to be loaned
            amounts, // amount(s) to recieve of the asset(s)
            modes, // 0 is flash loan
            onBehalfOf, // on behalf of this address
            params, // n/a
            referralCode // n/a
        );
    }
    ...
    function flashloan(address _asset) public onlyOwner {
        // bytes memory data = "";
        // amount of _asset you want to borrow
        uint256 amount = 100000000000000000;

        // adds _asset (token you want to borrow) to the assets array
        address[] memory assets = new address[](1);
        assets[0] = _asset;

        // adds amount (amount of _asset you want to borrow) to the amounts array
        uint256[] memory amounts = new uint256[](1);
        amounts[0] = amount;


        // takes out the flashloan
        console.log("Flashloan taken out for %s %s", amount, _asset);
        _flashloan(assets, amounts);
    }

    //
    // This function is called by the lending pool contract
    // It is called after your contract has received the flash loaned amount
    //  
    function executeOperation(address[] calldata assets, uint256[] calldata amounts, uint256[] calldata premiums, address initiator, bytes calldata params) external override returns (bool) {
        ...
        //
        console.log("Trading logic occurring");
        initiator = initiator;
        params = params;
        //
        // Your logic goes here.
        ...
        console.log("Flashloan paid back");
        return true;
    }
}
