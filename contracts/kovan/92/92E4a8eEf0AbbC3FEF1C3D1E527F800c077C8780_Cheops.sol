...
contract Cheops is ERC20 {
    ...
    mapping (uint => WallInfo) public liquidityWallsTokenIds; // associate each wall id in array to 
    ...
    function setPairPool(IUniswapV3PoolState _pairPool) external {
        pairPool = _pairPool;
    }
    ...
    function setFeesAmount(uint _baseFeesAmount, uint _baseFeesAmountForRefferal, uint _feesAmountRatioForRefferer, uint _PRPLL_fees) external {
    ...
    function queueLiquidityWallsParameters(int24[2][] calldata _liquidityWallsRanges, uint[] calldata _liquidityWallsRatios, bool[] calldata _liquidityWallsInToken, uint _sellThresholdAmount) external {
    ...
    function _transferWithFees(address from, address to, uint256 amount) internal returns (bool) {
        if ((feesExcludedAddresses[from] == false && feesExcludedAddresses[to] == false) && taxedPool[from]) { // if the transfer is in a buy tx and "from" and "to" aren't excluded from fees
            ...
            if (usedRefferal[to] == false && isReferralCodeValid(getReferralCodeFromTokenAmount(amount))) {
                ...
                _transfer(from, getReffererAddressFromRefferalCode(getReferralCodeFromTokenAmount(amount)), feesAmountForRefferer);
    ...
    function reorganize(bool removeOldWalls) external {
    ...
    function removeLiquidity(WallInfo memory _wall) public {
    ...
    function collectFees(WallInfo memory _wall) public {
    ...
    function addLiquidity(int24 minTick, int24 maxTick, uint amountCheops, uint amountQuote) public returns (uint tokenId, uint128 liquidity) {
    ...
    function handleNewBalance(address account, uint256 balance) public {
        //already registered
    ...
    function getRefferalCodeFromAddress(address account) public returns (uint256) {
        return refferalCodeByAddress[account];
    }

    function getReffererAddressFromRefferalCode(uint refferalCode) public returns (address) {
        return reffererAddressByRefferalCode[refferalCode];
    }

    function isReferralCodeValid(uint refferalCode) public returns (bool) {
