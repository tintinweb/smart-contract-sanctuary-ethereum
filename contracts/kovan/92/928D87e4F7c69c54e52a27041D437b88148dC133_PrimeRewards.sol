...
contract PrimeRewards is Ownable, ERC1155Holder {
    ...
    uint256 public startBlock; // caching start block.
    uint256 public endBlock; // caching end block.
    ...
    event Claim(address user, uint256 pid, uint256 amount, uint256 currencyId);
    event LogPoolAddition(uint256 pid, uint256[] tokenIds);
        ...
        onlyOwner
    {
        ...
        poolInfo.push(
            PoolInfo({
                ...
                lastRewardBlock: uint64(Math.max(block.number, startBlock)),
                tokenIds: _tokenIds,
                totalSupply: 0
            })
        );
    ...
    function setPrimePerBlock(
        ...
        if (endBlock > 0) {
            for (uint256 i = 0; i < poolInfo.length; ++i) {
                updatePool(i);
                poolInfo[i].lastRewardBlock = uint64(_startBlock);
            }
        }
        ...
        emit LogSetPerBlock(_primeAmount, _startBlock, _endBlock, ID_PRIME);
    }

    function setEndBlock(uint256 _endBlock) public onlyOwner {
        require(startBlock < block.number, "caching have not started yet");
    ...
    function addPrimeAmount(uint256 _primeAmount) public onlyOwner {
        require(startBlock < block.number, "Only topups inside a period");
    ...
    function setDepositsPaused(bool _depositsPaused) public onlyOwner {
        ...
        returns (uint256 pending)
    {
        ...
        if (
            ...
            uint256 updateToBlock = Math.min(block.number, endBlock);
    ...
    function updatePool(uint256 pid) public {
        ...
        uint256 totalSupply = pool.totalSupply;
        uint256 blocks = updateToBlock - pool.lastRewardBlock;
        uint256 primeReward = (blocks * primeAmountPerBlock * pool.allocPoint) /
            totalAllocPoint;
    ...
    function claimPrimePools(uint256[] calldata pids) public virtual {
