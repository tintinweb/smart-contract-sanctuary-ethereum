...
contract MCRCompute {
    ...
    function compute(address config_, address project_, uint256 pay_, bool add_) external returns(uint256) {
        uint256 _projectValue =  _updatePolicyValue(config_, project_, pay_);
    ...
    function _updatePolicyValue(address config_, address project_, uint256 pay_) internal returns (uint256) {
        uint256 _leverage = IInsureConfig(config_).leverageFactor(project_);
        uint256 _projectValue = pay_ * _leverage;
        projectValues[project_] = _projectValue;
