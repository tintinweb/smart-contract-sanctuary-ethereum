...
contract Compute {
    ...
    function add(address policy_, uint256 pay_) external {
        ...
        uint256 _length = policyTypes[_typeID].length;
        for (uint256 _pid = 0; _pid < _length; ++_pid) {
            address _policy = policyTypes[_typeID][_pid];
            ...
            correlation.value = uint240(_afterValue);
            _square = _square + _afterValue - _beforeValue;
        }
        square = _square;
