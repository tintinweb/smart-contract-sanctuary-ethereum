...
contract SellTaxHandler is ITaxHandler,Auth {
    ...
    function process() external override {
        ...
        if(amount>0&&shouldDistribute()) {
            ...
            uint256 amountToLiquidity = busdAmount.mul(taxToLiquidity).div(taxToLiquidity+taxToDev);
            uint256 amountToDev = busdAmount.sub(amountToLiquidity);
            busdToken.transfer(liquidityPool, amountToLiquidity);
            busdToken.transfer(devPool, amountToDev);
        }
    }
