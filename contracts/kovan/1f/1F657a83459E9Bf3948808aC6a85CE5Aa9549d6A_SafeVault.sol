...
interface IVaultManager {
  function lending(address, uint256) external view returns (address, uint256);
}

/// @title SafeVault - A multisignature wallet with support for confirmations using signed messages based on ERC191.
  ...
  Executor
{
  ...
  function onERC721Received(
    ...
    console.log("marketplace %s", addressRegistry.marketplace());

    if (_from == addressRegistry.marketplace()) {
      lockedNFTS[nftAddress][_tokenId] = _from;
    }

    console.log("nftAddress: %s", nftAddress);
    console.log("operator: %s", _operator);
  ...
  function setAddressRegistry(address _addressRegistry) public {
    addressRegistry = ITakeUsAddressRegistry(_addressRegistry);
  }

  function checkIfNFTisLocked(address _nftAddress, bytes calldata _data)
    private
  {
    (, address transferTo, uint256 tokenId) = abi.decode(
      _data[4:],
      (address, address, uint256)
    );

    // safeTransferFrom bytes (MAY BE YES MAY BE NO CHECK PLS)
    // data[:4] = 0x49403944

    (address lender, uint256 endTimestamp) = vaultManager.lending(
      _nftAddress,
      tokenId
    );
    ...
    if (lender != address(0)) {
      require(
        lender == transferTo,
        "Cannot send locked NFTs different to NFT owner address"
      );
      require(
        endTimestamp <= block.timestamp,
        "Cannot send locked NFTs before lending duration ends"
      );
      delete lockedNFTS[_nftAddress][tokenId];
    }
  }
