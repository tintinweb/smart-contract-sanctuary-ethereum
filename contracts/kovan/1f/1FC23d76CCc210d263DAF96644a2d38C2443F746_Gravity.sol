...
contract Gravity is KeeperCompatibleInterface {
    ...
    function performUpkeep(bytes calldata /* performData */) external override {
        ...
        if (_toPurchase > 0) {
            ...
            // update each account's scheduledBalance and targetBalance
            for(uint i = 0; i < purchaseOrders[_nextSlot].length; i++) {
                // account's share of sourceAsset
                // uint _shareOf = purchaseOrders[_nextSlot][i].purchaseAmount / _toPurchase * 100;
                accounts[purchaseOrders[_nextSlot][i].user].scheduledBalance -= purchaseOrders[_nextSlot][i].purchaseAmount;
                accounts[purchaseOrders[_nextSlot][i].user].targetBalance += purchaseOrders[_nextSlot][i].purchaseAmount * (_targetPurchased * 10000) / _toPurchase; // stored as N * 10 ** 2
            }
