...
// File: contracts/nft/MelodyBoxNFT.sol
...
contract MelodyBoxNFT is Initializable, ERC721Upgradeable, UUPSUpgradeable {

    address public presellAddress;
    uint public total;
    address public owner;
    event preSaleBox(uint indexed tokenId, address indexed from);

    modifier presell() {
        require(presellAddress!=address(0),"no presell");
        require(msg.sender==presellAddress,"must presell call");
        _;
    }
    ...
    function initialize() initializer public {
        __ERC721_init("MelodyBoxNFT", "MBNFT");
        owner = _msgSender();
    }
    ...
    function presellBox(address to) external virtual presell returns(uint tokenId) {
        tokenId=_mintBox(to);
        emit preSaleBox(tokenId, to);
    }

    function _mintBox(address to) virtual internal returns(uint tokenId) {
        total=total+1;
        tokenId=total;
        _safeMint(to, tokenId);
    }
    ...
    function setPresaleAddress(address _presaleAddress) public onlyOwner {
        presellAddress = _presaleAddress;
    }

    // function tokenURI(uint256 tokenId) override public view returns (string memory){
    //     return string(abi.encodePacked("https://bscscan.com/images/favicon.ico"));
    // }
}
...
// File: contracts/nft/MelodyBoxPreSale.sol
...
contract MelodyBoxPreSale is ReentrancyGuard {
    ...
    function confifOnSale(bool _on_sale) external onlyOwner {
        on_sale = _on_sale;
    }
    ...
    function preSaleBnb(address to) payable external nonReentrant {
        require(on_sale, "preSale: not on sale");
        require(msg.value >= getBnbPrice(to), "preSale: need more bnb");
        uint tokenId = melodyBoxNFT.mintNFT(to);
        emit preSaleBox(tokenId, to);
    }

    function preSaleUsdt(address to) external nonReentrant {
        require(on_sale, "preSale: not on sale");
    ...
    function preSaleBusd(address to) external nonReentrant {
        require(on_sale, "preSale: not on sale");
