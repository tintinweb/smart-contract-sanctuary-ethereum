...
contract NFTMarket is Initializable,UUPSUpgradeable,ContextUpgradeable,ReentrancyGuardUpgradeable,OwnableUpgradeable,ERC1155HolderUpgradeable{
  using CountersUpgradeable for CountersUpgradeable.Counter;
  IERC20Upgradeable public tokenAddress;

  CountersUpgradeable.Counter private _itemIds;
  CountersUpgradeable.Counter private _itemsSold; 
  uint256 public royalties ; // percent of sale going to royalties
  uint256 public royalty_firstOwner; // percent of royalty going to first owner
  uint256 public royalty_lastOwner; // percent of royalty going to the seller /*implicit royalty_intermediaries = 30;percent of royalty going to the the intermediaries*/
  ...
  mapping (address => uint256) pullableAmount;
  ...
  function initialize () public initializer{
      royalties = 100;
      royalty_firstOwner = 500;
      royalty_lastOwner = 200;
  ...
  function setTokenAddress(address _tokenaddress) external onlyOwner{
    console.log("token address recieved",_tokenaddress);
    tokenAddress = IERC20Upgradeable(_tokenaddress);
  }

  function GetUserTokenBalance() public view returns(uint256){ 
       return tokenAddress.balanceOf(_msgSender());// balancdOf function is already declared in ERC20 token function
   }
  function GetContractBalance() public view returns(uint256){
     return tokenAddress.balanceOf(address(this));
  }
  function GetAllowance() public view returns(uint256){
       return tokenAddress.allowance(_msgSender(), address(this));
  }
  ...
  function createMarketItem(
    ...
    require(price > 1, "Price must be at least 1 USDC");
    console.log("the base price is ", price);
    
    //nft contract transfers from msg.sendet to contract
    //contract becomes owner

    IERC1155Upgradeable(nftContract).safeTransferFrom(_msgSender(), address(this), tokenId, 1, "");
    ...
    owners[itemId][0] = payable(_msgSender());
  ...
  function CreateBid(uint256 itemId, uint256 amount) public payable nonReentrant{ // bid sends money in msg.value
    ...
    //transfer money from msg.sender to address(this)
    ...
    idToAuctionItem[itemId].highestBidder = payable(_msgSender());
  ...
  function endAuction(uint256 itemId) external 
  {
        ...
        if (idToAuctionItem[itemId].highestBidder == address(0)) {  //in case there are no bids for the item
            IERC1155Upgradeable(idToMarketItem[itemId].nftContract).safeTransferFrom(address(this), idToMarketItem[itemId].owner, idToMarketItem[itemId].tokenId,1 ,"");
            idToMarketItem[itemId].isOnMarket = false;
        } else {
            distributionOfFundsAndAssets(idToAuctionItem[itemId].highestBid, idToAuctionItem[itemId].highestBidder, itemId);
        }
        //resetting
  ...
  function distributionOfFundsAndAssets(uint price, address buyer, uint itemId) internal {
    ...
    //giving commission to marketplace first:
    pullableAmount[deployer]+=((price * commissionPercent)/1000);
    //allocation of sale price and royalties:
    if(saleCount==1){
      //if it is the first sale, the seller gets all the money
      pullableAmount[idToMarketItem[itemId].owner]+=(((1000-commissionPercent)*price)/1000);
    } else if (saleCount==2){
      //if it is the second sale
      //first owner gets royalty
      pullableAmount[owners[itemId][0]]+=((price*royalties)/1000);
      //seller gets the sale price ( - royalty - commission)
      pullableAmount[owners[itemId][idToMarketItem[itemId].saleCount-1]]+=((price*(1000-royalties-commissionPercent))/1000);
    } else { // this condition is hit when saleCount>2
      //first owner gets royalty
      pullableAmount[owners[itemId][0]]+=((price*royalties*royalty_firstOwner)/1000000);
      // intermediaries get royalty
      for (uint i=1;i< idToMarketItem[itemId].saleCount-2; i++){ //for all indermediaries, commission =  (msg.value*r*b/i)
        pullableAmount[owners[itemId][i]]+=((price*royalties*(1000-royalty_firstOwner-royalty_lastOwner))/1000000)/(idToMarketItem[itemId].saleCount-3);
      }
      // royalty to last seller
      pullableAmount[owners[itemId][idToMarketItem[itemId].saleCount-2]]+=((price*royalties*royalty_lastOwner)/1000000);
      // selling price - commission given to seller
      pullableAmount[owners[itemId][idToMarketItem[itemId].saleCount-1]]+=((price*(1000 - royalties - commissionPercent))/1000);
    }

    IERC1155Upgradeable(idToMarketItem[itemId].nftContract).safeTransferFrom(address(this), buyer, idToMarketItem[itemId].tokenId, 1, "");  //Ownership transfer
    owners[itemId][idToMarketItem[itemId].saleCount]=payable(buyer); // the new owner is _msgSender()
    idToMarketItem[itemId].isOnMarket = false;
    idToMarketItem[itemId].owner = payable(buyer);
    _itemsSold.increment();
  }

  function directMarketSale(
    uint256 itemId
    ) public payable nonReentrant {
      //maybe add require idToMarketItem[itemId].sold == false;
      //0.97 multiplication of prices (p1+p2)
    require(!idToMarketItem[itemId].isOnAuction,"Sale type is auction");  
    require(idToMarketItem[itemId].isOnMarket, "Currently not on sale");
    tokenAddress.transferFrom(_msgSender(),address(this),idToMarketItem[itemId].price);
    distributionOfFundsAndAssets(idToMarketItem[itemId].price, _msgSender(), itemId);
  }
  ...
  function resellItem(
    ...
    IERC1155Upgradeable(idToMarketItem[itemId].nftContract).safeTransferFrom(_msgSender(), address(this), idToMarketItem[itemId].tokenId, 1, "");
  ...
  */
  ...
  function withdrawFunds(address payee) public {
    require(pullableAmount[payee]>0,"No balance to withdraw");
    uint256 currentBalance = pullableAmount[payee];
    pullableAmount[payee]=0;
    tokenAddress.transfer(payable(payee),currentBalance);
  }

  function checkBalance(address payee) public view returns(uint256){
