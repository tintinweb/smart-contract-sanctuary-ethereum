...
contract PresaleContract is AccessControlEnumerable, PresaleInterface {
    ...
    uint private _blockStart = 10;                      // TO-DO: Setup on which block presale starts
    uint private _maxContributionLimitCutoff = 20;      // TO-DO: Setup on which block maxContribution is lifted
    uint private _firstStageParticipantCount = 2;     // TO-DO: Setup how many participants needs to be to lift maxcontribution
    uint private _blockEnd = 40;                        // TO-DO: Setup on which block presale ends
    uint private _presaleCap = 10 * 10 ** 18;           // TO-DO: Setup what is presale cap

    uint private _maxContribution = 2 * 10**18;         // TO-DO: Setup what is maximum contribution in first stage
    ...
    // ************* PUBLIC REGION **************
    // ******************************************
    /// @notice Executes the code for participating in the crowdsale
    ...
    function contribute(address tokenInvested_, uint amountInvested_, bytes memory signature_) public override returns (bool) {
        require(getPresaleActive(), "Presale is not active!");
        require(getTokenAllowed(tokenInvested_), "Token is not allowed!");

        uint allowedContribution = getParticipantsMaxContribution(msg.sender);
        ...
        _dataInstance.creditParticipation(msg.sender, allowedContribution);
    ...
    // ************* MANAGER REGION **************
    // *******************************************
    /// @notice Sets new block number on which presale should start
    /// @dev This method should only be used in ongoing presale if something goes wrong
    /// @param blockStart_ The block number on which crowdsale will start
    /// @return default return True after everything is processed
    function setBlockStart(uint blockStart_) public onlyRole(MODERATOR_ROLE) override returns (bool) {
        uint oldValue = _blockStart;
        require(oldValue != blockStart_, "Value is already set!");
        _blockStart = blockStart_;
        emit BlockStartChanged(oldValue, blockStart_);
        return true;
    }
    /// @notice Sets new block on which maxContribution limit is waived
    /// @dev This method should only be used in ongoing presale if something goes wrong
    /// @param limitCutoff_ The block number on which maxContribution limit is waived
    /// @return default return True after everything is processed
    function setMaxContributionLimitCutoff(uint limitCutoff_) public onlyRole(MODERATOR_ROLE) override returns (bool) {
        uint oldValue = _maxContributionLimitCutoff;
        require(oldValue != limitCutoff_, "Value is already set!");
        _maxContributionLimitCutoff = limitCutoff_;
        emit LimitCutoffChanged(oldValue, limitCutoff_);
        return true;
    }
    /// @notice Sets the amount of contributors when maxContribution is lifted
    /// @dev This method should only be used in ongoing presale if something goes wrong
    /// @param participantCount_ The amount of contributors
    /// @return default return True after everything is processed
    function setFirstStageParticipantCount(uint participantCount_) public onlyRole(MODERATOR_ROLE) override returns (bool) {
        uint oldValue = _firstStageParticipantCount;
        require(oldValue != participantCount_, "Value is already set!");
        _firstStageParticipantCount = participantCount_;
        emit FirstStageContributorCountChanged(oldValue, participantCount_);
        return true;
    }
    /// @notice Sets new block on which presale will end
    /// @dev This method should only be used in ongoing presale if something goes wrong
    /// @param blockEnd_ The block number on which presale will end
    /// @return default return True after everything is processed
    function setBlockEnd(uint blockEnd_) public onlyRole(MODERATOR_ROLE) override returns (bool) {
        uint oldValue = _blockEnd;
        require(oldValue != blockEnd_, "Value is already set!");
        _blockEnd = blockEnd_;
        emit BlockEndChanged(oldValue, blockEnd_);
        return true;
    }
    ...
    /// @dev This method should only be used in ongoing presale if something goes wrong
    /// @param maxContribution_ The new cap on users contribution on first phase
    /// @return default return True after everything is processed
    function setMaxContribution(uint maxContribution_) public onlyRole(MODERATOR_ROLE) override returns (bool) {
        uint oldValue = _maxContribution;
        require(oldValue != maxContribution_, "Value is already set!");
        _maxContribution = maxContribution_;
        emit MaxContributionChanged(oldValue, maxContribution_);
        return true;
    }
    ...
    function getPresaleActive() public view override returns (bool) {
        if (block.number < _blockStart) { return false; }
        if (block.number > _blockEnd) { return false; }
    ...
    /// @notice Returns the block on which presale will start
    function getBlockStart() public view override returns (uint) {
        return _blockStart;
    }
    /// @notice Returns the block on which maxContribution limitation will be lifted
    function getMaxContributionLimitCutoff() public view override returns (uint) {
        return _maxContributionLimitCutoff;
    }
    /// @notice Returns the amount of participants when max contribution limit is lifted
    function getFirstStageParticipantCount() public view override returns (uint) {
        return _firstStageParticipantCount;
    }
    /// @notice Returns the block on which presale will end
    function getBlockEnd() public view override returns (uint) {
        return _blockEnd;
    }
    /// @notice Returns the amount of tokens participant can contribute in first phase
    function getMaxContribution() public view override returns (uint) {
        return _maxContribution;
    }
    /// @notice Returns the amount that participant can contribute depending on the state of presale
    function getParticipantsMaxContribution(address participant_) public view override returns (uint) {
        if (block.number >= getMaxContributionLimitCutoff() || _firstStageParticipantCount <= _dataInstance.getParticipantCount()) {
            return getPresaleCap() - _dataInstance.getCombinedContributions();
        } else {
            uint maxContribution = getMaxContribution() - _dataInstance.getParticipantContributions(participant_);
            if (maxContribution + _dataInstance.getCombinedContributions() > getPresaleCap()) {
                return getPresaleCap() - _dataInstance.getCombinedContributions();
            }
...
interface PresaleInterface is IAccessControlEnumerable {
    ...
    // ************* PUBLIC REGION **************
    // ******************************************
    function contribute(address tokenInvested_, uint amountInvested_, bytes memory signature_) external returns (bool);
    ...
    function setBlockStart(uint blockStart_) external returns (bool);
    function setMaxContributionLimitCutoff(uint limitCutoff_) external returns (bool);
    function setFirstStageParticipantCount(uint participantCount_) external returns (bool);
    function setBlockEnd(uint blockEnd_) external returns (bool);
    function setPresaleCap(uint presaleCap_) external returns (bool);

    function setMaxContribution(uint maxContribution_) external returns (bool);
    ...
    function getBlockStart() external view returns (uint);
    function getMaxContributionLimitCutoff() external view returns (uint);
    function getFirstStageParticipantCount() external view returns (uint);
    function getBlockEnd() external view returns (uint);
    function getMaxContribution() external view returns (uint);
    function getParticipantsMaxContribution(address participant_) external view returns (uint);
    ...
    // ************* EVENTS REGION **************
    // ******************************************
    event BlockStartChanged(uint oldValue, uint newValue_);
    event LimitCutoffChanged(uint oldValue, uint newValue_);
    event FirstStageContributorCountChanged(uint oldValue, uint newValue_);

    event BlockEndChanged(uint oldValue, uint newValue_);
    event PresaleCapChanged(uint oldValue, uint newValue_);

    event MaxContributionChanged(uint oldValue, uint newValue_);
    event TokenAllowedChanged(bool oldValue, bool newValue_);

    event DataAddressChanged(address oldValue, address newValue_);
    event CorporateAddressChanged(address oldValue, address newValue_);
