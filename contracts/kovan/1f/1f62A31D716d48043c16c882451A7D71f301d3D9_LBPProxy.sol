...
import "./interfaces/ILBPFactory.sol";
...
contract LBPProxy is Ownable {
    ...
    function createAuction(PoolConfig memory poolConfig) external returns (address) {
        ...
        TransferHelper.safeApprove(poolConfig.tokens[0], _vault, poolConfig.amounts[0]);
        TransferHelper.safeApprove(poolConfig.tokens[1], _vault, poolConfig.amounts[1]);

        // 2: pool creation
        address pool = ILBPFactory(_LBPFactoryAddress).create(
            poolConfig.name,
            poolConfig.symbol,
            poolConfig.tokens,
            poolConfig.weights,
        ...
        IVault(_vault).joinPool(
            ...
            IVault.JoinPoolRequest(poolConfig.tokens, poolConfig.amounts, poolConfig.userData, false)
        );
        emit JoinedPool(pool, poolConfig.tokens, poolConfig.amounts, poolConfig.userData);

        // 5: configure weights
        ILBP(pool).updateWeightsGradually(poolConfig.startTime, poolConfig.endTime, poolConfig.endWeights);
    ...
    function exitPool(address pool, uint256[] calldata minAmountsOut, uint256 maxBPTTokenOut) external onlyPoolOwner(pool) {
        // 1. Get pool data
        bytes32 poolId = ILBP(pool).getPoolId();
        (address[] memory poolTokens, , ) = IVault(_vault).getPoolTokens(poolId);
        ...
        IVault.ExitPoolRequest memory exitRequest = IVault.ExitPoolRequest(poolTokens, minAmountsOut, userData, false);

        // 3. Exit pool and keep tokens in contract
        IVault(_vault).exitPool(poolId, address(this), payable(address(this)), exitRequest);
