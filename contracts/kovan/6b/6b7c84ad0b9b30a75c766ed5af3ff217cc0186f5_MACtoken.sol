...
contract MACtoken is IBEP20, Ownable {
    using SafeMath for uint256;
    ...
    uint256 public maxTxAmount = _totalSupply.div(100); // 1%
    ...
    constructor(
        ...
        isExcludedFromMaxWalletLimit[msg.sender] = true;
        isExcludedFromMaxWalletLimit[address(this)] = true;
        isExcludedFromMaxWalletLimit[address(DEAD)] = true; 
    ...
    function _transferFrom(
        ...
        if (!isTxLimitExempt[sender] && !isTxLimitExempt[recipient]) {
            ...
            if (!isExcludedFromMaxWalletLimit[recipient] && sender == pair) {
                require(
    ...
    function swapBack() internal swapping {
        ...
        uint256 amountETHBuyback = amountETH.mul(devFee).div(totalETHFee);

        try distributor.deposit{value: amountETHReflection}() {} catch {}
        payable(marketingFeeReceiver).transfer(amountETHMarketing);
        payable(devFeeReceiver).transfer(amountETHBuyback);
    ...
    function ExcludeAddressesFromMaxWalletLimit(address account, bool _state)
        ...
        onlyOwner
    {
        isExcludedFromMaxWalletLimit[account] = _state;
    }
    ...
    function setFees(
        ...
        totalFee = _liquidityFee.add(_devFee).add(_reflectionFee).add(
            _marketingFee
        );
        feeDenominator = _feeDenominator;
        require(totalFee < feeDenominator / 4);
        require(totalFee <= 200, "Can not exceed than 20%");
    }
