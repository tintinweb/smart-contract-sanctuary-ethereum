...
contract Chikn is ERC721Upgradeable {
    // Royalty
    ...
    // Toggle sale on and off for both GB and Public
    function toggleOpenForGB(bool status) external {
    ...
    function hasGB(address user) public view returns (bool) {
        ERC20 instance = ERC20(GB_erc20_contract);
        uint256 bal = instance.balanceOf(user);
        // ERC20 GB token has 9 decimals places (more than 900)
        bool result = bal >= 900000000000;
        // emit GBERC20Checked(user, bal, result);
        return result;
    }

    function gbHolderMint() public {
        require(openForGB == true, "not open");
        require(hasGB(msg.sender), "Not valid GB");
        require(_allTokens.length < maxSupply, "max supply");
        require(_gbTokenSupply.length < gbHoldersMaxMint);
        require(gbholders[msg.sender] == false, "already minted");
        _tokenIds.increment();
        uint256 newItemId = _tokenIds.current();
        _safeMint(msg.sender, newItemId);
        ChickenRun memory newChickenRun = ChickenRun(
        ...
        // add the token id to the allChickenRun
        allChickenRun[newItemId] = newChickenRun;
        // add GB holder to the list
        gbholders[msg.sender] = true;
        _gbTokenSupply.push(newItemId);
    }
