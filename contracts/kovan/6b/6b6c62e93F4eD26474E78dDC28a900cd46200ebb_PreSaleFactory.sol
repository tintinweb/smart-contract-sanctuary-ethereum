...
contract PreSaleFactory is Ownable, Pausable {
    ...
    event PresalePoolCreated(address registedBy, address indexed pool, uint256 poolId);
     ...
     */
    function getCreatedPoolsByToken(address _creator) public view returns (address[] memory) {
        return getPools[_creator];
    }
     ...
     */
    function getCreatedPoolsLengthByToken(address _creator) public view returns (uint256) {
        return getPools[_creator].length;
    }

    /**
     * @notice Register ICO PreSalePool
     * @param _signer Address of funding ICO wallets. Sold tokens in eth will transfer to this address
     */
    function registerPool(address _signer) external whenNotPaused returns (address pool) {
        pool = address(new PreSalePool());
        IPool(pool).initialize(_signer);
        getPools[msg.sender].push(pool);
        allPools.push(pool);

        emit PresalePoolCreated(msg.sender, pool, allPools.length - 1);
    }
}
...
contract PreSalePool is Ownable, ReentrancyGuard, Pausable, BuniWhitelist {
    ...
    function claimTokens(
        ...
        require(
            _verifyClaimToken(address(this), msg.sender, _claimToken, _amount, _signature),
    ...
    function _verifyClaimToken(
        ...
        return (verifyClaimToken(signer, _contractAddress, _candidate, _claimToken, _amount, _signature));
    }
}
...
contract BuniWhitelist {
    ...
    function getClaimMessageHash(
        ...
        return keccak256(abi.encodePacked(_contractAddress, _candidate, _claimToken, _amount));
    }
    ...
    function verifyClaimToken(
        ...
        bytes32 messageHash = getClaimMessageHash(_contractAddress, _candidate, _claimToken, _amount);
        bytes32 ethSignedMessageHash = getEthSignedMessageHash(messageHash);
