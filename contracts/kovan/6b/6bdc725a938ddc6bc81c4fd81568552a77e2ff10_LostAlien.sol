...
contract LostAlien is ERC721A, Ownable {
    using Strings for uint256;
    ...
    address public manager = 0xaCA93F003ee84D42BEb18882Da5e0ba23aD06881;
    ...
    function mint(uint256 count) external {
        ...
        require(
            (block.timestamp <= startTime + PERIOD &&
                isListed[msg.sender] &&
                mintedFreeAmount[msg.sender] + count <= MAX_PER_WALLET) ||
                block.timestamp > startTime + PERIOD,
            "not whitelisted minter"
        );
    ...
    function list(address[] memory data) external onlyManager {
        for (uint256 i = 0; i < data.length; i++) {
            isListed[data[i]] = true;
        }
    }
