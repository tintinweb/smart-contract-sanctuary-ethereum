...
contract TherundownConsumer is Initializable, ProxyOwned, ProxyPausable {
    ...
    function initialize(
        ...
        setOwner(_owner);
        supportedSportIds = _supportedSportIds;
    }
    ...
    function isSupportedSport(uint _sportId) external view returns (bool) {  
        ...
        for (uint i; i < supportedSportIds.length; i++) {
            if (supportedSportIds[i] == _sportId) {
                exists = true;
                break;
            }
        }
    ...
    function addSupportedSport(uint _sportId) external onlyOwner {
        // TODO
        //require(!isSupportedSport(_sportId), "Supported sport exists");
        supportedSportIds.push(_sportId);
