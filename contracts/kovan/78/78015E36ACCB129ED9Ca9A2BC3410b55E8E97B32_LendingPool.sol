    ...
    ReentrancyGuardUpgradeable
{
    ...
    ///@param _rate  The user expected rate should be larger than 
        ...
        nonReentrant
    {
        ...
        onlyNotPaused();
        if (undercollateralized == 0 && !isValidPrice()) revert NotValidPrice();
    ...
    function rollOver(address _newPool) external nonReentrant {
        ...
        if (!newPool.isValidPrice()) revert NotValidPrice();
        ...
        if (newPool.mintRatio() <= mintRatio) {
            ...
            newPool.borrowOnBehalfOf(msg.sender, 
                                    userReport.colAmount,
        ...
        nonReentrant
    {
        ...
        uint256 colReturnAmount = (repayRemainder * 1e18 * (10**colToken.decimals())) /
    ...
    function collect() external nonReentrant {
        ...
        _safeTransfer(lendToken, treasury, (totalFees * protocolFee) / 1000000);
        totalFees = 0;
        _safeTransfer(
            colToken,
            treasury,
    ...
    ///@param _mintRatio useful on rollover payout calculation
    ...
    ///@param from Wallet that will send the tokens to Vendor
