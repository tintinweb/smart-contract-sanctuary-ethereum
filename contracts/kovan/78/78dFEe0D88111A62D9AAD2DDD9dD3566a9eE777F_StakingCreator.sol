...
contract StakingCreator is IStakingCreator, Ownable {
    ...
    modifier onlyCeresCreator {
        require(msg.sender == ceresCreator, "StakingCreator: Only CeresCreator!");
        _;
    }
    ...
    /* ---------- Functions ---------- */
    function createStaking(address ceresFactory, address asc, address crs, address token) external override
        onlyCeresCreator returns (address){
        return address(new CeresStaking(ceresFactory, asc, crs, token, stakerBook));
    }
