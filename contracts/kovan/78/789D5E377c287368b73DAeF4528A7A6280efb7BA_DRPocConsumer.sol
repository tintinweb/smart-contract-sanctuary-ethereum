...
contract DRPocConsumer is ChainlinkClient {
    ...
    // );
    // Short event
    event OracleRequest(bytes32 indexed specId, address requester, uint256 payment);
    // Pepe event
    event Pepe(
        bytes32 indexed specId,
        address requester,
    ...
    // No specId, no payment
    event OracleRequestC(
        address requester,
        bytes32 requestId,
        address callbackAddr,
        bytes4 callbackFunctionId,
    ...
    // Another event
    event NewRoundA(address indexed to);

    error FailedTransferLINK(address to, uint256 amount);
    error UnsupportedTestCase(TestCase _case);
    ...
    function requestTest(bytes32 _specId, uint256 _payment) external {
        ...
        //     req.buf.buf
        // );
        emit OracleRequest(_specId, msg.sender, _payment);
    }
    ...
    function requestTestCase(
        ...
        if (_case == TestCase.A) {
            ...
            revert UnsupportedTestCase(_case);
        }
    }

    function requestNewRound(address _address, TestCase _case) external {
        if (_case == TestCase.A) {
            emit NewRoundA(_address);
        } else {
            revert UnsupportedTestCase(_case);
        }
    }
