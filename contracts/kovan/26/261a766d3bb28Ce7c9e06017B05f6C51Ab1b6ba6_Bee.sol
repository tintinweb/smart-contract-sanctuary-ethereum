...
contract Bee is ERC721A, SimpleAccess {
     ...
     * get your 5% back and start the counter again from 0.
     *
    ...
    struct BeeSpec {
        ...
        uint88 powerCycleStart; /// @dev records start time of power cycle which decrease 5% every weeks
        uint80 powerCycleStored; /// @dev stores the power cycle left of the bee when unstaked
    }
    struct UserBeeSpec {
        uint256 beeId;
        bool isAlreadyStaked;
        // Add BEE remaining boost power
    }
     ...
     * 1 (1 week or 5%).
     *
     * Once we know this target lost periods amount we set the powerCycleStart such that the difference
     ...
     * start and current time is between 1 week and less than 2 weeks.
     */
    function _getPowerReductionPeriods(uint256 tokenId) internal view returns (uint256) {
        BeeSpec memory bee = beeSpecs[tokenId];
        uint256 reductionPeriods = block.timestamp - bee.powerCycleStart / powerCycleBasePeriod;
