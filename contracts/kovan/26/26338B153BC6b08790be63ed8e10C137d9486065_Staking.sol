...
contract Staking is AccessControl {
    ...
    function setLocks(uint256[] memory lockPeriod, uint256[] memory index) private { // 44
        require((block.timestamp - deployTime) / /*60 < 120*/ 1 days < 3, "0");
        uint256 len = index.length;
        for (uint256 i = 0; i < len; i += 1) {
            require(allDeposits[index[i]].sumInLock > 0, "00");
