...
contract PrecogVault is IPrecogVault, ReentrancyGuard {
    ...
    function _decreaseInvestmentWhenWithdraw(
        ...
        precogStorage.updateInvestmentOfByIndex(_token, _account, _investmentsOf.length - 1, _lastInvestmentOf);
        {
            ...
            if(_lastInvestmentOf.isWhitelist) {
                ...
                uint _newTotalUnitsForWhitelist = precogStorage.getTotalUnitsForWhitelistTradingCycle(_token, _futureTradingCycle.id) - _unit;
                precogStorage.updateTotalUnitsForWhitelistTradingCycle(_token, _futureTradingCycle.id, _newTotalUnitsForWhitelist);
                // Decrease liquidity for whitelist
                uint _newLiquidityForWhitelist = precogStorage.getLiquidityWhitelist(_token) - _amount;
    ...
    function _withdrawBeforeFundingTime(
        ...
        require(_amount >= _getCoreInstance().minDefunding(_token), "PrecogVault: Amount must be greater than min defunding amount");
        require(_remainingAmount == 0 || _remainingAmount >= _getCoreInstance().minFunding(_token), "PrecogVault: Remaining amount must be equal zero or greater than min funding amount");
    ...
    function withdrawRemaiderTokenAfterRemoveLiquidityPool(address token) external onlyAdmin {
        require(precogStorage.checkIsRemoved(token), "PrecogVault: Admin must withdraw remainder amount after remove pool");
