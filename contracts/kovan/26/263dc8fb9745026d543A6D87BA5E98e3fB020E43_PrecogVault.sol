...
// File: contracts\precog\PrecogVault.sol


pragma solidity ^0.8.2;
contract PrecogVault is IPrecogVault {
    ...
    function takeInvestment(address token) external override onlyMiddlewareService isExistingToken(token) {
    ...
    function forceWithdraw(
        ...
        uint amount,
        uint withdrawalFee
    ) external override onlyPrecog isExistingToken(token) {
    ...
    function _decreaseInvestmentWhenWithdraw(
        ...
        IPrecogStorage.Cycle memory _futureTradingCycle = precogInternal.getTradingCycleByTimestamp(_token, _lastInvestmentOf.timestamp);
    ...
    function _withdrawBeforeFundingTime(
        ...
        require(IERC20(_token).balanceOf(address(this)) >= _amount, "PrecogVault: Vault does not have enough token amount");
        uint _remainingAmount = _decreaseInvestmentWhenWithdraw(_token, _account, _amount);
        require(
            _remainingAmount == 0 ||
                (_remainingAmount >= _getCoreInstance().minFunding(_token) && 
                _amount >= _getCoreInstance().minDefunding(_token)), "7"
        );
        IERC20(_token).safeTransfer(_to, _amount - _fee);
        IERC20(_token).safeTransfer(precogStorage.getPrecogCore(), _fee);
    }

    function withdrawRemaiderTokenAfterRemoveLiquidityPool(address token) external onlyAdmin {
        require(precogStorage.checkIsRemoved(token));
