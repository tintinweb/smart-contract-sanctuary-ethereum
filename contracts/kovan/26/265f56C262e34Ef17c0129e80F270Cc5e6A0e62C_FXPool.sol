...
contract FXPool is IMinimalSwapInfoPool, BalancerPoolToken, Ownable, Storage, ReentrancyGuard, Pausable {
    ...
    function onExitPool(
        ...
        // minting protocol fees before decreasingthe value of totalSupply
        _mintProtocolFees();
    ...
    function getLpPrice() public view returns (uint256) {
        ...
        uint256 oGLiqUInt = ABDKMath64x64.toUInt(_oGLiq);

        return (oGLiqUInt.mul(1e18)).mul(1e18).div(curve.totalSupply);
    }
    ...
    function _calculateAndStoreUnclaimedProtocolFee(int128 fees) private {
        uint256 feesToAdd = ABDKMath64x64.toUInt(fees);
        console.log('Fees to add: ', feesToAdd);

        // added 1e18 to convert to wei
        feesToAdd = feesToAdd.mul(1e18).mul(protocolPercentFee).div(1e2);
    ...
    function _mintProtocolFees() private {
        if (collectorAddress != address(0) && curve.totalSupply > 0) {
            uint256 currentLpPrice = getLpPrice();
            console.log('Current LP Price: ', currentLpPrice);
            // div 1e18 since we padded that in the lp price for more precisions
            uint256 lpTokenFeeAmount = (totalUnclaimedFeesInNumeraire.mul(currentLpPrice)).div(1e18);

            console.log('LP Token to Mint: ', lpTokenFeeAmount);
...
library FXSwaps {
    ...
    function viewOriginSwap(
        ...
        console.log('Origin amount: ', _originAmount);
        if (_o.ix == _t.ix)
        ...
        console.log('Input numeraire amount: ', ABDKMath64x64.toUInt(inputNumeraireAmount));
        console.log('amt in abs value: ', ABDKMath64x64.toUInt(_amt.abs()));

        accruedFees_ = inputNumeraireAmount.sub(_amt.abs());
        console.log('Accrued Fees: ', ABDKMath64x64.toUInt(accruedFees_));
    ...
    function viewTargetSwap(
        ...
        int128 inputNumeraireAmount = _amt;
        console.log('Input numeraire: ', ABDKMath64x64.toUInt(_amt));
        ...
        accruedFees_ = _amt.abs().sub(inputNumeraireAmount);
