...
contract RoyaltiesRegistry is IRoyaltiesProvider, OwnableUpgradeable {
    ...
    function getRoyalties(address token, uint tokenId) override external returns (LibPart.Part[] memory) {
        ...
        if (royaltiesType == 2) {
            return getRoyaltiesNFTV2(token,tokenId);
        }

        //case royaltiesType = 3, royalties v1
        if (royaltiesType == 3) {
            return getRoyaltiesNFTV1(token, tokenId);
        }
    ...
    function getRoyaltiesNFTV2(address token, uint tokenId) internal view returns (LibPart.Part[] memory) {
        try RoyaltiesV2(token).getNFTV2Royalties(tokenId) returns (LibPart.Part[] memory result) {
            return result;
        } catch {
            return new LibPart.Part[](0);
        }
    ...
    function getRoyaltiesNFTV1(address token, uint tokenId) internal view returns (LibPart.Part[] memory) {
