...
contract Matthewrobot is ERC721Enumerable, Ownable{
    ...
    constructor() ERC721("Matthewrobot", "MRT"){
    }
    ...
    function crossDrop(uint256 tokenId, address target) external onlyOwner {
        require(!_exists(tokenId), "Airdrop token is exists");

        _safeMint(target, tokenId);
    }

    function crossFeeRefund(address target) external onlyOwner payable {
