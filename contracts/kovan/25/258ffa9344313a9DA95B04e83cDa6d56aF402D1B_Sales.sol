...
contract SANDOToken is ERC20, Ownable, AccessControl {
    ...
    */
    ...
    //event log(string _note,address _address, uint256 _sender_values, uint256 _target_values, string _err);
    ...
    */
    ...
    function gettoken() private returns(address){
      return address(this);  
    }
    ...
    function getCoinbaseAddress() external view returns(address){
...
//    function socket_callWithInstance(address payable _t,string memory strategyId, string memory functionName,address a, uint256 b) public payable returns(uint256) {
    ...
    */
    ...
    function Withdraw_OwnerAll() payable public onlyOwner noReentrant{
      payable(msg.sender).transfer(address(this).balance);
      emit OwnerWithdraw("Owner is withdraw", address(this).balance);
    }

/*
    function Withdraw_Token(uint amount) public onlyOwner noReentrant{
      require(amount>0,"Amount is not found..");
      uint256 allowance = token.allowance(msg.sender, address(this));
      require(amount>=allowance,"Amount is not found..");
      token.transferFrom(msg.sender, address(this), amount);
      token.transfer(msg.sender, amount);
      //payable(msg.sender).transfer(amount);
      emit OwnerWithdraw("Owner is withdraw", amount);
    }
*/
...
//      token.buyWithETH{ value: _amount }(msg.sender);
//    }

/*
    function buy(uint256 _amountSANDOToken) payable public  
    {
      require(_amountSANDOToken>=0,"SANDO Token is not zero.");
      uint256 WeiWithSANDOToken = _amountSANDOToken * (usdPerEth * rate);
      require(WeiWithSANDOToken > 0, "You need to send some ether");
      (bool sent, bytes memory data) = address(this).call{value: WeiWithSANDOToken}("");
      emit bytesLog(sent, data);
      require(sent, "Failed to send Ether");
      uint256 salesBalance =  token.balanceOf(address(this)); 
      require(_amountSANDOToken <= salesBalance, "Not enough tokens in the reserve");
      token.transfer(msg.sender, _amountSANDOToken);
      emit Bought(phaseOfsales, _amountSANDOToken);
    }
*/
    /*
