...
contract Staking is AccessControl, ReentrancyGuard {
        ...
        onlyRole(SPONSOR)
    {
        ...
        for (uint256 j = 0; j < 4; j += 1) {
            require(block.timestamp == lastUpdate[j], "8");
        }
        ...
        creator(index)
    {
        ...
        if (
            ...
            (stake.lockUp < 2 && (stake.pool >= stake.lockUp + 1 || year <= stake.pool)) || // конец 1 и 2 лет или еще рано пересчитывать для 1 и 2 лет
            (stake.lockUp == 2 && ((year < 3 && year <= stake.pool) || stake.pool == 4)) // конец 3 лет или еще рано пересчитывать для 3 лет < 3 (если третий локап больше 3 лет, то сюда даже не зайдет, тк выкинется выше на месяцах)
        ) {
            vars.rewardsKON += stake.varKoeff.unreceivedRewardKON;
            vars.rewardsUSDT += stake.varKoeff.unreceivedRewardUSDT;
            if(stake.lockUp == 2 && year >= 3) reward.variableRewardTaken = months + 1;   
        } else {
            ...
            if(stake.lockUp < 2 || (stake.lockUp == 2 && months > 11)) stake.pool += 1;  
            if(stake.lockUp == 2 && year >= 3) reward.variableRewardTaken = months + 1;   
        }
        if (
            stake.lockUp < 2 &&
        ...
        creator(index)
    {
        ...
        if (
            (stake.lockUp < 2 && (year <= stake.lockUp)) ||
            (stake.lockUp == 2 && year < 3)
        ) {
        ...
        } else if ((stake.lockUp < 2 && !stake.gotFixed) || (stake.lockUp == 2 && months >= reward3Info[user][index].variableRewardTaken)) {
            harvest(index);
        }

        if ((stake.lockUp == 2 && stake.pool != 4) || (stake.lockUp < 2 && stake.pool < stake.lockUp + 1))
            updateTotalStaked(stake.sumInLock, stake.lockUp, index);
        ...
        returns (uint256 amount, uint256 months)
    {
        ...
        if(amount >= 3) months = (block.timestamp - (start + (3 * YEAR))) / 30 days;
    }
 ...
 function withdraw(uint256 index)
        ...
        nonReentrant
    {
        ...
        if (
            ...
            updateTotalStaked(stake.sumInLock, stake.lockUp, index);
        } else if (stake.lockUp < 2) {
            // если не ранний для локапа до двух лет
            if (!stake.gotFixed) {
                vars.rewardsKON += fixedPart(index);
            }
            if (stake.pool >= stake.lockUp + 1)
                inactiveTokensInPool -= stake.sumInLock; // если был пройден апдейт пул
            else {
                //если пул не апдейтнул этот депозит
                (vars.amountKON, vars.amountUSDT) = varPart(
                    stake.sumInLock,
                    stake.varKoeff.koeffBeforeDepositKON,
                    stake.varKoeff.koeffBeforeDepositUSDT,
                    stake.varKoeff.receivedRewardKON,
        ...
        } else if (stake.lockUp == 2 && year >= 3) {
            ///виздро для трехлетнего локапа без штрафа
