...
contract PredictionMarket is AdminManaged,ReentrancyGuard {
   ...
   constructor (
    ...
    function settleMarket(uint _id, bool _result)
        public
        onlyTrustedSource
        marketInSettled(_id)
        returns(bool success)
    {
        ...
        emit MarketSettled(_id, _result,block.timestamp);
        return true;
    }

    // Public functions

    function predictMarket(uint _id, bool _result, uint amount)
    	public
        marketLive(_id)
        returns (bool success)
    {
        ...
        require(markets[_id].startTime >= block.timestamp,"Market is not accepting prediction anymore");
        require(markets[_id].predictionStatus == PredictionStatus.Live,"Market is not accepting prediction anymore");
        ...
        emit Predict(_id, msg.sender, amount, _result);
        return true;
    }
    ...
    {
    	require(isMarket(_id),"Invalid market id");
        require(markets[_id].predictionStatus == PredictionStatus.Settled,"Market is still active");
