...
contract PrecogV5 is IPrecogV5 {
    ...
    {
        (currentCycleId, currentCycleStartTime, currentCycleEndTime) = _getCurrentProfitCycleByTimestamp(token, block.timestamp);
    }
    ...
    function _chooseLastAvailableProfitId(
        Investment[] memory _investments,
    ...
    {
        _nextInvestment = Investment(0, 0, 0, 0);
        ...
        returns (uint _profitAtCycle) 
    {
        if (_totalInvestmentUnit > 0) {
            if (_lastProfitIdOf == _investment.idChanged) {
                ...
                Cycle memory lastCycle = _profitCycles[_lastProfitIdOf];
        ...
        )
    {
        ...
        if (_lastProfitIdOf < _updatedLatestCycle) {
            for (_investmentId; _investmentId < _investments.length; _investmentId++) {
                ...
                (_nextInvestment, _lastAvailableProfitId) = _chooseLastAvailableProfitId(_investments, _investmentId, _updatedLatestCycle);
                for (_lastProfitIdOf; _lastProfitIdOf < _lastAvailableProfitId; _lastProfitIdOf++) {
                    _profitOfAccount += _calculateProfitAtCycle(_profitCycles, _investments[_investmentId], totalInvestmentUnits[_token][_lastProfitIdOf], _profits[_lastProfitIdOf], _lastProfitIdOf);
                }
    ...
    function sendProfit(address token, uint profitAmount) external override onlyMiddleware {
        ...
        _buyPCOG(token, profitAmount, _currentProfitCycleId);
    ...
    function _getFutureProfitCycle(address _token, uint _futureTimestamp) internal view returns (Cycle memory _futureProfitCycle) {
        (uint _futureProfitCycleId, uint _futureStartTime, uint _futureEndTime) = _getCurrentProfitCycleByTimestamp(_token, _futureTimestamp);
        _futureProfitCycle = Cycle(uint16(_futureProfitCycleId), uint48(_futureStartTime), uint48(_futureEndTime));
    }

    function _isAfterLatestDepositedTime(address _token) internal view returns (bool) {
        return block.timestamp > currentDepositedTimestamp[_token];
    }

    function _defineDurationInvestmentCycle(address _token, Cycle memory _lastProfitCycle) internal view returns (uint48 _duration) {
        if(_isAfterLatestDepositedTime(_token)) {
            _duration = uint48(core.getCycleConfiguration().investmentCycle);
        } else {
            _duration = _lastProfitCycle.endTime - _lastProfitCycle.startTime;
        }
    }

    function _calculateNextInvestmentTimestamp(Cycle memory _firstProfitCycle, uint48 _duration) internal view returns (uint48 _nextInvestmentTimestamp) {
    ...
    function _increaseInvestment(
        ...
        uint48 _duration = _defineDurationInvestmentCycle(_token, _lastProfitCycle);
        uint48 _nextInvestmentTimestamp = _calculateNextInvestmentTimestamp(_profitCycles[0], _duration);
        Cycle memory _futureProfitCycle = _getFutureProfitCycle(_token, _nextInvestmentTimestamp);
    ...
    function _decreaseInvestmentWhenWithdraw(address _token, address _account, uint _amount) internal {
        Investment[] memory _investmentsOf = investmentsOf[_token][_account];
        Investment memory _lastInvestmentOf = _investmentsOf[_investmentsOf.length - 1];
        Cycle memory _futureProfitCycle = _getFutureProfitCycle(_token, _lastInvestmentOf.timestamp);
        require(_futureProfitCycle.endTime >= uint48(block.timestamp), "PrecogV5: INVALID_TIMESTAMP");
