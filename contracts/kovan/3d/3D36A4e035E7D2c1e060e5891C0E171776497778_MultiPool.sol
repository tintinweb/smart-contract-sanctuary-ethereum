...
contract MultiPool is Ownable {
    ...
    // Return the value held in this MultiPool
    function multiPoolValue() public view returns(uint) {
        uint totalValue;
        for (uint i=0; i<pools.length; i++) {
            PoolInfo memory pool = pools[i];
            totalValue += IPoolV2(pool.poolAddress).portfolioValue(address(this));
        }
    ...
    function portfolioValue(address _addr) public view returns(uint) {
        ...
        for (uint i=0; i<pools.length; i++) {
            PoolInfo memory pool = pools[i];
            totalValue += IPoolV2(pool.poolAddress).portfolioValue(_addr);
        }
   ...
   function withdrawLP(uint256 lpAmount) external {
    ...
    // Returns the MultiPool LP tokens representing the % of the value of this deposit 
    ...
    function lpTokensForDeposit(uint amount) internal view returns (uint) {
        ...
        if (lpToken.totalSupply() == 0) {
            ...
            uint precision = 10 ** uint(lpToken.decimals());
            uint depositPercentage = precision * amount / multiPoolValue();
