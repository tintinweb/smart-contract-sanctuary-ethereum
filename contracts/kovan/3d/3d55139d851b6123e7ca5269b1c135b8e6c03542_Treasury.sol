    ...
    ReentrancyGuardUpgradeable
{
    ...
    function validatePayout() external {
        ...
        if(balance > 0) {
            ...
            if(IGovernance(dao).nextInvesteeFund()<IGovernance(dao).nextInvestee() && investee._fundAmount <= getRvltAmountOneETH[1]){
    ...
    function fundInvestee(uint256 totalAmount) internal nonReentrant{
        ...
        if(investee._fundAmount <= totalAmount) {
            ...
            IURevolt(uRvlt).updateCultMandatorsReward(investee._fundAmount.mul(5).div(100));
        }
    }
}
