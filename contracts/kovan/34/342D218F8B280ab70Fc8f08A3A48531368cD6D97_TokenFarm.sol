...
contract TokenFarm is Ownable {
    ...
    mapping(address => uint256) public uniqueTokenStaked; //該用戶存了多少種代幣
    mapping(address => address) public tokenPriceFeedMapping;
    IERC20 public meowToken;

    constructor(address _meowTokenAddress) {
        meowToken = IERC20(_meowTokenAddress);
    }
    ...
    function issueTokens() public onlyOwner {
        for (
            ...
            meowToken.transfer(recipient, userTotalValue);
        }
    }
    ...
    function stakeToken(uint256 _amount, address _token) public {
        ...
        //tranfer:只有代幣所有者可以用 transferfrom大家都可以用
        IERC20(_token).transferFrom(msg.sender, address(this), _amount);
        updateUniqueTokenUserStaked(msg.sender, _token);
    ...
    function unstackedToken(address _token) public {
        ...
        require(balance > 0, "Stacking balance cannot be zero");
    ...
    function updateUniqueTokenUserStaked(address _user, address _token)
        internal
    {
        if (stakingBalance[_token][_user] <= 0) {
            uniqueTokenStaked[_user] = uniqueTokenStaked[_user] + 1; //該用戶存了多少種代幣
        }
    }
