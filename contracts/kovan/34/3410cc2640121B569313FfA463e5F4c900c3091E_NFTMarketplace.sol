...
contract NFTMarketplace is AccessControl, ReentrancyGuard {
    ...
    function fillOffers(uint _amount, string memory referCode) public inProgress() nonReentrant {
        require(_amount >= minPurchase, "Amount must >= minPurchase");
        uint level = isWhitelist(msg.sender);
        if (level > 0) {
            if (level == 1) {
                require(!requireWhitelist || _amount <= highLevelWhitelist[msg.sender], "Insufficient claimable quota");
            } else {
                require(!requireWhitelist || _amount <= lowLevelWhitelist[msg.sender], "Insufficient claimable quota");
            }
        }
