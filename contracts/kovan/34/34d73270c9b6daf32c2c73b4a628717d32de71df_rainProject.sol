...
// File: contracts/weather721a.sol
...
contract rainProject is ERC721A, Ownable {
    using Strings for uint256;
    ...
    uint8 public precipitationType;
    ...
    constructor() ERC721A("rain project", "RP", 5){
   ...
   // }
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        //if it's raining in HK (precipitationType is equal to one) show the Buri
        if (precipitationType == 1) {
            return bytes(Buri).length > 0
            ? string(abi.encodePacked(Buri, trueId.toString(), ".json"))
            : "";
        }else{

            //if it's not raining in HK (precipitationType is equal to zero,two,three and four) show the Auri
            return bytes(Auri).length > 0
                ? string(abi.encodePacked(Auri, trueId.toString(), ".json"))
                : "";
        }
