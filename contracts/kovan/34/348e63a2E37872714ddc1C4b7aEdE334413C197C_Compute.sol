...
contract Compute {
    ...
    function addPool(uint256 tpyeID_, address[] memory policyTypes_, uint256[] memory leverages_, uint256[] memory corrs_) external {
    ...
    function add(address policy_, uint256 pay_) external {
        ...
        uint256 _policyPay = policyInfo.pay + pay_;
        uint256 _policyValue = _policyPay * policyInfo.leverage;
        ...
        for (uint256 _pid = 0; _pid < _length; ++_pid) {
            ...
            uint256 _afterValue = correlation.corr * _value * _policyValue;
