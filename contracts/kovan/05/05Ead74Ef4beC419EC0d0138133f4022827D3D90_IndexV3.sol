...
contract IndexV3 is Ownable {
   ...
   function withdrawLP(uint256 lpAmount) external {
    ...
    function feesForWithdraw(uint lpAmount, address account) public view returns (uint) {
        uint amount = lpAmount == 0 ? lpToken.balanceOf(account) : lpAmount;

        if (amount == 0 || lpToken.totalSupply() == 0) return 0;

        require(amount <= lpToken.balanceOf(account), "LP balance exceeded");
        ...
        // sum up expected fees across all pools in the multipool
        uint total;
        for (uint i=0; i<pools.length; i++) {
            PoolInfo memory pool = pools[i];
            if (pool.poolAddress != address(0x0) && pool.lpTokenAddress != address(0x0)) {
                ...
                total += IPoolV3(pool.poolAddress).feesForWithdraw(withdrawAmount, address(this));
            }
        }
