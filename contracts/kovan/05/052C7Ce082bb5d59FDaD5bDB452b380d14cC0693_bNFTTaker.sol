...
contract bNFTTaker is bNFTBase {
    function initialize(
        string memory name,
        ...
        __bNFTBase__init(name, symbol, _token, _config);
    }
    ...
    function _exists(uint tokenId)
        ...
        returns (bool)
    {
        return IMarketPositionTransfer(market).existsTakerPosition(tokenId);
    }

    function ownerOf(uint tokenId) public view override returns (address) {
        return IMarketPositionTransfer(market).ownerOfTakerPosition(tokenId);
    }
    ...
    function _transfer(
        ...
        IMarketPositionTransfer(market).safeTransferTakerPosition(
            from,
            to,
            tokenId
        );
...
/// @notice NFT token for taker position
abstract contract bNFTBase is ContextUpgradeable, ERC165Upgradeable, IERC721Upgradeable, IERC721MetadataUpgradeable, IPositionNFT {
    ...
    function _transfer(
        ...
        console.log(" _balances[from] %s",  _balances[from] );
        console.log(" _balances[to] %s",  _balances[to] );
    ...
    /// @notice mint new NFT for position
    function safeMint(address to, uint tokenId) external override onlyMarket {
        _safeMint(to, tokenId);
    }

    /// @notice burn existing NFT when position is going to be close
    function safeBurn(uint tokenId) external override onlyMarket {
        _burn(tokenId);
    } 
}
