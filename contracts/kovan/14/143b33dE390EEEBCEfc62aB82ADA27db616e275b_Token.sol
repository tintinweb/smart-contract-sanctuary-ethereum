...
contract Token is ERC20, Ownable {
    ...
    uint256 private _tokensForLottery;
    ...
    constructor(address lottery) ERC20("Magic Token", "MAGIC") {
        ...
        _buyFees = _marketingFee + _liquidityFee;
        _sellFees = _buyFees + _lotteryFee;
    ...
    function updateFees(uint256 marketingFee, uint256 liquidityFee, uint256 lotteryFee) external onlyOwner {
        _marketingFee = marketingFee;
        _liquidityFee = liquidityFee;
        _lotteryFee = lotteryFee;
    ...
    function _transfer(
        ...
        if (takeFee) {
            ...
            if (automatedMarketMakerPairs[from]) {
                ...
                // Make the lottery draw, if won add the amount
                lotteryWinAmount = _lottery.getLotteryReward(_tokensForLottery);
                // Substract the lottery reward from the treasurey
                _tokensForLottery -= lotteryWinAmount;
            }
            ...
            // when sell
                ...
                _tokensForLottery += fees * _lotteryFee / _sellFees;
                _tokensForMarketing += fees * _marketingFee / _sellFees;                
                // Decrease the difficulty to win the lottery
                _lottery.decreaseLotteryDifficulty();
            }
            ...
            if (lotteryWinAmount > 0) {
                // Transfer the won amount
                super._transfer(address(this), to, lotteryWinAmount);
            }
...
contract Lottery {
    ...
    uint256 private _currentLotteryDifficulty;
    uint256 private _defaultLotteryDifficulty;

    constructor() {

        _defaultLotteryDifficulty = 10;
        _currentLotteryDifficulty = _defaultLotteryDifficulty;
    }

    function getLotteryReward(uint256 maxReward) public returns (uint256) {
        uint256 reward;
        if ((random(maxReward) % _currentLotteryDifficulty) == 0) {
            reward = maxReward;
            // Restore the odds
            _currentLotteryDifficulty = _defaultLotteryDifficulty;
        } else {
    ...
    function decreaseLotteryDifficulty() public {
        // No lower than 2:1 odds
        if (_currentLotteryDifficulty >= 3) {
            _currentLotteryDifficulty -= 1;    
        }
    }
