...
contract Metadata {
    ...
    function count2() public view returns (uint256) {
        return _coordinates.length / 3;
    }


    bytes public _coordinates;

    function setPointsAsBytes(bytes memory pairs) public {
        // bytes memory b = new bytes(200);
        _coordinates = pairs;
    }

    function hydrate(uint start) public view returns (bytes1, bytes memory) {

        require(_coordinates.length > start + 2, "Not enough data");
        bytes1 n = _coordinates[start];

        bytes memory z_bytes = new bytes(2);
        z_bytes[0] = _coordinates[start + 1];
        z_bytes[1] = _coordinates[start + 2];

        return (n, z_bytes);

        // z = z_bytes;

        // for (uint i = 0; i < _cords.length; i++)
        //     b[i] = byte(uint8(i));
