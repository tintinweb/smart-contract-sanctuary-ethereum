...
contract WolfAirDrop is AdminTransfer,EIP712,ReentrancyGuard,Multicall {
    address public signerAddress;
    INFTAdmin public nftAdmin;
    mapping(uint=>bool) public oidUsed;

    event AirDrop(uint256 indexed oid,address indexed user,uint indexed tokenId,uint256 grade,uint256 kind,address coin,uint256 amount);

    function airDrop(uint256 deadline,uint256 oid,uint256 kind,uint256 grade,uint256 number,address to,address coin,uint256 amount,bytes memory signature) public nonReentrant {
        require(block.timestamp <= deadline, "expired");
        require(check(signerAddress,hashData(deadline,oid,kind,grade,number,to,coin,amount),signature), "sign error");
        require(!oidUsed[oid],"already submit");
        require(kind>0 && kind<5 && grade>0 &&grade<6,"param error");
        oidUsed[oid]=true;
        if(amount>0) {
        ...
        for(uint i=0;i<number;i++) {
            ...
            nftAdmin.setUintProperty(tokenId,"grade",grade);
            nftAdmin.setUintProperty(tokenId,"kind",kind);
            emit AirDrop(oid,to,tokenId,grade,kind,coin,amount);
        }
    }

    constructor(address _signerAddress,INFTAdmin _nftAdmin) EIP712("WolfAirDrop", "1") {
    ...
    function hashData(uint256 deadline,uint256 oid,uint256 kind,uint256 grade,uint256 number,address to,address coin,uint256 amount) public view returns (bytes32) {
        return _hashTypedDataV4(keccak256(abi.encode(
            keccak256("WolfAirDropRequest(uint256 deadline,uint256 oid,uint256 kind,uint256 grade,uint256 number,address to,address coin,uint256 amount)"),
            deadline,
            oid,
            kind,
            grade,
