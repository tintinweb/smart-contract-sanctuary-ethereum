...
contract CadgerRegistry {
        ...
        returns (VaultData memory vaultData)
    {
        ...
        vaultData.rewardTokens = new address[](balanceOfRewards.length);
        for (uint256 i = 0; i < balanceOfRewards.length; i++) {
            vaultData.rewardTokens[i] = balanceOfRewards[i].token;
        }
    ...
    function calculateAPR(
        ...
        for (uint256 i = 0; i < _simulatedYields.length; i++) {
            yieldsInUSD += amountInUSD(_simulatedYields[i]);
        }
...
interface IStrategy {
    ...
    function initialize(address vault, address want, address governance) external;
