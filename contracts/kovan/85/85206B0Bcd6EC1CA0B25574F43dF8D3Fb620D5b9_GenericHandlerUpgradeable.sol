 ...
 */
contract GenericHandlerUpgradeable is Initializable, EIP712Upgradeable, ContextUpgradeable, AccessControlUpgradeable {
    ...
    function __GenericHandlerUpgradeable_init(address _bridge, bytes32 _resourceID) internal initializer {
        ...
        __EIP712_init("RouterSync", "0.0.1");
    ...
    function MapContract(RouterLinker calldata linker, bytes memory _sign) external {
        ...
        address linkedAddress = _verify(linker, _sign);
        require(
            linkedAddress == crossTalk.fetchLinkSetter(),
            "Router Generichandler : Signature Mismatch for Linked Address to contract"
        );
        require(linker.linkerType == 1, "Router Generichandler : Linker Type 1 required for linking contracts");
    ...
    function UnMapContract(RouterLinker calldata linker, bytes memory _sign) external {
        ...
        require(linker.linkerType == 2, "Router Generichandler : Linker Type 2 required for unlinking contracts");
        crossTalk.Unlink(linker._chainID);
    }

    function _hash(RouterLinker calldata linker) internal view returns (bytes32) {
        return
            _hashTypedDataV4(
                keccak256(
                    abi.encode(
    ...
    function _verify(RouterLinker calldata linker, bytes memory signature) internal view returns (address) {
    ...
    function genericDeposit(
        ...
        require(destAddress != address(0), "Router Generichandler : Destination Address not linked to contract");
        // TODO : Add Fee logic in here
    ...
    function fetchDepositRecord(uint8 _srcChainID, uint64 _nonce) external view returns (DepositRecord memory) {
        return _depositRecords[_srcChainID][_nonce];
    }
    ...
    function executeProposal(bytes calldata _data) external onlyRole(BRIDGE_ROLE) returns (bool) {
        ...
        iRouterCrossTalk crossTalk = iRouterCrossTalk(depositData._destAddress);
        (bool success, bytes memory callback) = crossTalk.routerSync(
    ...
    function decodeData(bytes calldata _data) internal pure returns (DepositRecord memory) {
        ...
        (
            ...
            depositData.data,
            depositData.hash
        ) = abi.decode(_data, (uint8, uint64, address, address, bytes4, bytes, bytes32));
        return depositData;
    }
