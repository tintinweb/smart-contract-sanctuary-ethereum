...
contract MasterContractManager is BoringOwnable, BoringFactory {
    ...
    function _calculateDomainSeparator(uint256 chainId) private view returns (bytes32) {
        return keccak256(abi.encode(DOMAIN_SEPARATOR_SIGNATURE_HASH, keccak256("BreadBox V1"), chainId, address(this)));
    }
    ...
    /// @notice Other contracts need to register with this master contract so that users can approve them for the BreadBox.
                    ...
                    abi.encodePacked(
                        ...
                        keccak256(
                            abi.encode(
                                ...
                                approved
                                    ? keccak256("Give FULL access to funds in (and approved to) BreadBox?")
                                    : keccak256("Revoke access to BreadBox?"),
                                user,
                                masterContract,
                                approved,
                                nonces[user]++
...
// File contracts/BreadBox.sol
// License-Identifier: UNLICENSED

/// @title BreadBox
/// @author BoringCrypto, Keno
/// @notice The BreadBox is a vault for tokens. The stored tokens can be flash loaned and used in strategies.
/// Yield from this will go to the token depositors.
/// Rebasing tokens ARE NOT supported and WILL cause loss of funds.
/// Any funds transfered directly onto the BreadBox will be lost, use the deposit function instead.
contract BreadBoxV1 is MasterContractManager, BoringBatchable {
    using BoringMath for uint256;
    using BoringMath128 for uint128;
    ...
    struct StrategyData {
        ...
        uint128 balance; // the balance of the strategy that BreadBox thinks is in there
    }
    ...
    /// If 'from' is the BreadBox itself, it's allowed. Any ETH, token balances (above the known balances) or BreadBox balances
    ...
    modifier allowed(address from) {
        if (from != msg.sender && from != address(this)) {
            ...
            require(masterContract != address(0), "BreadBox: no masterContract");
            require(masterContractApproved[masterContract][from], "BreadBox: Transfer not approved");
        }
        _;
    }
    ...
    function deposit(
        ...
        // Checks
        require(to != address(0), "BreadBox: to not set"); // To avoid a bad UI from burning funds
        ...
        require(total.elastic != 0 || token.totalSupply() > 0, "BreadBox: No tokens");
        if (share == 0) {
    ...
    function withdraw(
        ...
        require(total.base >= MINIMUM_SHARE_BALANCE || total.base == 0, "BreadBox: cannot empty");
        totals[token] = total;
        ...
        if (token_ == USE_ETHEREUM) {
            ...
            require(success, "BreadBox: ETH transfer failed");
        } else {
    ...
    function transferMultiple(
        ...
        // Checks
        require(tos[0] != address(0), "BreadBox: to[0] not set"); // To avoid a bad UI from burning funds
    ...
    function flashLoan(
        ...
        require(_tokenBalanceOf(token) >= totals[token].addElastic(fee.to128()), "BreadBox: Wrong amount");
    ...
    function batchFlashLoan(
        ...
        for (uint256 i = 0; i < len; i++) {
            ...
            require(_tokenBalanceOf(token) >= totals[token].addElastic(fees[i].to128()), "BreadBox: Wrong amount");
    ...
    // F5 - Checks-Effects-Interactions pattern followed? (SWC-107)
    // F5: Total amount is updated AFTER interaction. But strategy is under our control.
    // F5: Not followed to prevent reentrancy issues with flashloans and BreadBox skims?
    function harvest(
        IERC20 token,
        bool balance,
        ...
        if (balanceChange > 0) {
            ...
            // But tokens with balances that large are not supported by the BreadBox.
            uint256 sub = uint256(-balanceChange);
