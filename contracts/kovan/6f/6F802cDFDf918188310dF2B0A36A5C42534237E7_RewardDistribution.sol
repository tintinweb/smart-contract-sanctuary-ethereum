...
contract RewardDistribution is IRewardDistribution, Ownable, GovernancePool, KeeperCompatibleInterface  {
    ...
    function setWeekDays(uint8 _day) external {
        day = _day;
    }

    function setIntervalForCheck(uint256 _interval) external {
        interval = _interval;
    }
    ...
    /// @dev Allowed to be called only by authorized game contracts.
    /// Should be called every Sunday by BE
    function _checkGovernanceReward() internal {
        // // check if the last check were at leat 7 days before
        // require(
        //     block.timestamp >= (7 days) + lastTimeGovRewardCheck,
        //     "INCORRECT_TIME"
        // );
