...
contract PlasmaGasStation is Ownable, LondonTxSupport {
    ...
    uint256 private txRelayFeePercent;
    ...
    constructor(address _router, address _feePayer, uint256 _txRelayFeePercent) {
    ...
    function sendTransaction(TxRequest calldata _tx, TxFee calldata _fee, bytes calldata _sign) external onlyFeePayer {
        ...
        (uint256 totalFeeInEth, uint256 txRelayFee) = _calculateCharge(estimatedGasUsed, txRelayFeePercent, _tx);
        uint256 spentTokens = router.swapTokensForExactETH(totalFeeInEth, maxFeeInTokens, _getSwapPair(address(token)), msg.sender, block.timestamp)[0];
        ...
        emit TransactionExecuted(_tx.from, _tx.to, _fee.token, spentTokens, txRelayFee);
