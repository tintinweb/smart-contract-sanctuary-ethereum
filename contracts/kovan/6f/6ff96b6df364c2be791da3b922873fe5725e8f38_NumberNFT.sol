...
contract NumberNFT is ERC721A, Ownable {
  enum Status {
    Waiting,
    Started,
    Finished
  ...
  uint16[1326] private randomArray;
  // mapping(uint256 => address) private randomMap;

  constructor(string memory initBaseURI, uint16[1326] memory _randomArray) ERC721A("TOM TEST", "TOMTEST") {
    baseURI = initBaseURI;
    status = Status.Started;
    // randomArray = new uint[](1326);
    randomArray = _randomArray;
    // for(uint256 i = 0; i < 1326; i++) {
    //   randomArray[i] = i;  
    // }

    // for (uint256 i = 0; i < randomArray.length; i++) {
    //     uint256 n = i + uint256(keccak256(abi.encodePacked(block.timestamp))) % (randomArray.length - i);
    //     uint256 temp = randomArray[n];
  ...
  function getRandomArray() external view onlyOwner returns (uint16[1326] memory) {
    return randomArray;
  }
  ...
  function mint(uint256 _mintAmount) external payable {
    require(status == Status.Started, "POKER : HAVEN'T STARTED YET");
    ...
    require(
      ...
      "POKER : EXCEED THE MAX QUNATITY PER WALLET"
    );
  ...
  function refundIfOver(uint256 price) private {
    require(msg.value >= price, "POKER: REFUND ETH.");
    if (msg.value > price) {
