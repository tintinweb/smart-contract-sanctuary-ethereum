...
contract IDOTest is DynamicIdo {
    ...
    function testGetMaxReward(address _user) external view returns (uint256) {
        return maxReward[_user];
    }
...
contract DynamicIdo is IDO {
    ...
    function getMaxReward() public view override returns (uint256) {
        return maxReward[msg.sender];
    }
...
contract IDO is Claim, Whitelisted {
    ...
    function getMaxReward() public view virtual returns (uint256) {
        return _maxReward;
    }
        ...
        onlyWhitelisted(msg.sender)
    {
        ...
        require(totalReward <= getMaxReward(), "More then max amount");
