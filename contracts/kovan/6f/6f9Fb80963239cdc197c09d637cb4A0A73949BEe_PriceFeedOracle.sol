...
contract PriceFeedOracle is PriceOracle, AccessControl {
    ...
    mapping(address => PriceFeed) public priceFeedsByUnderlyingAddress;
    ...
    function getCTokenPrice(CToken _cToken) external view returns (uint256) {
        address underlyingAddress = _getUnderlyingAddress(_cToken);

        return getUnderlyingPrice(underlyingAddress);
    }

    function getUnderlyingPrice(address _underlyingAddress)
        public
        view
        returns (uint256)
    {
        // Reported prices come before price feeds
        if (reportedPricesByUnderlyingAddress[_underlyingAddress] != 0) {
            return reportedPricesByUnderlyingAddress[_underlyingAddress];
        }

        PriceFeed memory priceFeed = priceFeedsByUnderlyingAddress[
            _underlyingAddress
        ];

        (, int256 price, , , ) = priceFeed.aggregator.latestRoundData();

        // Compound protocol does not handle negative prices, we return 0 instead which means the price is unavailable
        if (price < 0) {
            return 0;
        }

        // Compound expects oracle's dollar values to have 18 decimals precision
        // Based on the price feed decimals we adjust the value as needed
        if (priceFeed.decimals > 18) {
            return uint256(price) / 10**(priceFeed.decimals - 18);
        }
    ...
    /// @param _priceMantissa The price of the asset using 18 decimals of precision
    ...
    function setPriceFeed(
        ...
        emit SetPriceFeed(
            _underlyingAddress,
            address(
                priceFeedsByUnderlyingAddress[_underlyingAddress].aggregator
            ),
            _priceFeed
        );

        priceFeedsByUnderlyingAddress[_underlyingAddress] = PriceFeed(
            _decimals,
...
contract PriceOracle {
     ...
     */
    function getCTokenPrice(CToken cToken) external view returns (uint256);
}
