...
contract UniswapV3Locker is Ownable, ReentrancyGuard {
  using EnumerableSet for EnumerableSet.AddressSet;
  using EnumerableSet for EnumerableSet.UintSet;

  IUniV3LPFactory public uniswapFactory;
  ...
  constructor(IUniV3LPFactory _uniswapFactory, ICountryList _countryList) {
   ...
   * @param _lpToken the univ3 token address
  ...
  function lockLPToken (address _lpToken, uint256 _amount, uint256 _unlock_date, address payable _referral, bool _fee_in_eth, address payable _withdrawer, uint16 _countryCode) external payable nonReentrant {
    ...
    // ensure the lp token is a valid Unicrypt Uniswap V3 LP token querying the factory
    require(uniswapFactory.isLPToken(_lpToken), 'NOT UNI V3');
