...
contract DepositAave{
    ...
    // for other tokens apart from ETH
    function depositTokenTest(uint256  _value)  external payable onlyOwner{
        
        uint16 referralCode = 0; // referralCode 0 is like none
        address onBehalfOf = msg.sender;
        uint256 amountToDeposit = _value  ;
        address daiAddress = 0xFf795577d9AC8bD7D90Ee22b6C1703490b6512FD;

        // uint priceToken = priceOracle.getAssetPrice(daiAddress);
        // uint256 amountToDeposit = (msg.value * 1e18)/(priceToken * 100);
 
        //Approve LendingPool contract to move your Asset
        // IERC20(daiAddress).approve(address(this), amountToDeposit);

        IERC20(daiAddress).transferFrom(onBehalfOf,address(this),_value);

        // require(IERC20(daiAddress).transferFrom(msg.sender,addressLendingPool, amountToDeposit));
        // // require(IERC20(batAddress).transfer(address(this), amountToDeposit));

        // lendingPool.deposit(daiAddress, amountToDeposit,msg.sender, referralCode);
