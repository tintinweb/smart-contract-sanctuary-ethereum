...
contract TokenFarm is Ownable {
    ...
    function issueTokens() public onlyOwner {
        ...
        for (stakersIndex = 0; stakersIndex < stakers.length; stakersIndex++) {
            address recipient = stakers[stakersIndex];
            // send them token reward
            uint256 userTotalValue = getUserTotalValue(recipient);
            // base on their total value locked
        ...
        returns (uint256)
    {
        ...
        uint256 price;
        uint256 decimals;
        (price, decimals) = getTokenValue(_token);
    ...
    function stakeTokens(uint256 _amount, address _token) public {
        // what toke can they stake?
        // how much can they stake/
        require(_amount > 0, "Amount must be more than 0");
        require(tokenIsAllowed(_token), "Token is currently not allow");
