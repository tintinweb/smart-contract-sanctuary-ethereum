...
//the respective prices
     function stakeSACX(address _synth, uint256 amount) external  {
        ...
        uint256 TotalSynthPrice=synthToken1.synthToUsd(amount);//price of 10 synth e.g 10 sETH = 1000$
        uint256 collatteralPrice = TotalSynthPrice.mul(CollatteralRatio.div(100));//price of the collateral we need to stake
        uint256 sacxPrice=GetSacxPrice();//price of 1 SACX
        uint256 collatteralToStake=collatteralPrice.div(sacxPrice);//amount of SACX that will be staked according to price
...
//function unstakes SACX for a given amount of synth provided by user
   function UnstakeSACX(address _synth, uint256 amount) external
   {
      ...
      uint256 usdPrice=synthToken1.synthToUsd(amount);
...
/*function _unstake(address _recipient,uint256 _amount)public
   ...
   {
     uint256 _TotalSacx=SacxToken.balanceOf(address(this));
     return _TotalSacx;
...
//by adding total supply of each synth
   function totalsynths()public view returns(uint256){
     uint256 totaltokens;
for (uint256 i = 0; i < SynthsAddr.length; i++){
        ISynth synthToken1 = getSynthinfo(SynthsAddr[i]);
...
//function gets the synthetic asset addresses and info to use in the system
    function getSynthinfo(address SynthAddress) public view returns(ISynth){
      ...
      address Saddress;
        for (uint256 i = 0; i < SynthsAddr.length; i++){
         if(SynthsAddr[i] == SynthAddress)
         {
           Saddress=address(SynthAddress);
           break;
         }
        }
       return ISynth(Saddress);
    }
