...
contract DepositAndWithdraw is Ownable {
    ...
    function balanceOfToken(address _depositorAddress, address _tokenAddress)
        public
        view
        returns (uint256)
    {
        return IERC20(_tokenAddress).balanceOf(_depositorAddress);
    }
    ...
    function deposit(
        ...
        require(
            balanceOfToken(_depositorAddress, _token) >= _amount,
            "insufficient tokens available in your wallet"
        );
        require(tokenIsAllowed(_token), "token is not allowed to be deposited");
        IERC20(_token).transferFrom(_depositorAddress, address(this), _amount);
        if (alreadyUser[_depositorAddress]) {
            for (uint256 i = 0; i < users.length; i++) {
                if (users[i].userAddress == _depositorAddress) {
                    if (
                        users[i].tokenBalances[_token][_depositorAddress] <= 0
                    ) {
                        users[i].uniqueTokensDeposited[_token] += 1;
                        users[i].tokenBalances[_token][
                            _depositorAddress
                        ] += _amount;
                        break;
                    }
        ...
        u.userAddress = _depositorAddress;
        u.uniqueTokensDeposited[_token] += 1;
        u.tokenBalances[_token][_depositorAddress] += _amount;
        alreadyUser[_depositorAddress] = true;
    }
