...
contract LQDFarm is Ownable, Pausable {
    ...
    struct UserInfo {
        ...
        uint256 boostedShare; // boostedShare = amount + amount * boost /100 or (amount* (1 + boost/100))
    ...
    struct PoolInfo {
        ...
        uint256 boostedShares;      // this is the number of all users's shares userShare = stakedAmount + stakedAmount*boost/100;
    }

    // Address of the ERC20 Token contract.
    IERC20 public LQDToken;
    ...
    // when boost is updated 
    mapping(uint256 => mapping(address => uint256)) public usersReserves;

    // boosts
    mapping(address => uint256) public usersBoosts;

    address public booster;
    function setBooster(address boosterAddress_) external onlyOwner {
        booster = boosterAddress_;
    }

    function setUserBoost(address user_,uint256 boost_) external onlyBooster {
        // take care of the boost
        usersBoosts[user_] = boost_;
        for(uint256 i = 0 ; i < poolInfo.length; i++){
            _updateBoostedAmount(i , user_ );
        }
    }

    function _updateBoostedAmount(uint256 pid_,address user_) internal {
        PoolInfo storage pool = poolInfo[pid_];
        UserInfo storage user = userInfo[pid_][user_];

        if (user.amount == 0 || pool.allocPoint == 0) return; // no need to update

        updatePool(pid_);

        usersReserves[pid_][user_] += user.boostedShare.mul(pool.accERC20PerShare).div(1e36).sub(user.rewardDebt);
        
        pool.boostedShares = pool.stakedAmount.add(pool.stakedAmount.mul(usersBoosts[user_]).div(100));

        user.boostedShare = user.amount.add(user.amount.mul(usersBoosts[user_]).div(100));
        user.rewardDebt = user.boostedShare.mul(pool.accERC20PerShare).div(1e36);
    }
    ...
    constructor(IERC20 _rewardToken, uint256 _rewardPerBlock, uint256 _startBlock, address _feeWalletAddr) {
        LQDToken = _rewardToken;
    ...
    function fund(uint256 _amount) external onlyOwner onlyAfterInit {
        // in case user took too long to fund it prevent it from expiring
        if(startBlock == endBlock && block.number >= endBlock && startBlock != 0){
        ...
        uint256 balanceBefore = LQDToken.balanceOf(address(this));

        LQDToken.transferFrom(address(msg.sender), address(this), _amount.sub(leftOver));

        uint256 diff = LQDToken.balanceOf(address(this)) - balanceBefore;
    ...
    function addStakingPool(uint256 _allocPoint, IERC20 _lpToken, uint256 _withdrawFee, uint256 _claimFee) external onlyOwner onlyAfterInit {
        massUpdatePools();
    ...
    function updateAllocationPoints(uint256 _pid, uint256 _allocPoint) external onlyOwner onlyAfterInit {
        massUpdatePools();
    ...
    // View function to see boosted deposit for a user.
    function boostedDeposited(uint256 _pid, address _user) external view returns (uint256) {
        UserInfo storage user = userInfo[_pid][_user];
        return user.boostedShare;
    }
    ...
    function pending(uint256 _pid, address _user) external view returns (uint256) {
        ...
        uint256 totalShares = pool.boostedShares;
        ...
        if (block.number > pool.lastRewardBlock && totalShares != 0) {
            ...
            accERC20PerShare = accERC20PerShare.add(erc20Reward.mul(1e36).div(totalShares));
        }

        return user.boostedShare.mul(accERC20PerShare).div(1e36).sub(user.rewardDebt).add(usersReserves[_pid][msg.sender]);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        pool.accERC20PerShare = pool.accERC20PerShare.add(erc20Reward.mul(1e36).div(totalShares));
        pool.lastRewardBlock = lastBlock; 
    }

    // Deposit LP tokens to Farm for ERC20 allocation.
    function stake(uint256 _pid, uint256 _amount) external onlyAfterInit whenNotPaused{
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        updatePool(_pid);
        if (user.boostedShare > 0) {
            uint256 pendingAmount = user.boostedShare.mul(pool.accERC20PerShare).div(1e36).sub(user.rewardDebt).add(usersReserves[_pid][msg.sender]);
            uint256 feeWalletAmount = pendingAmount * pool.claimFee / 1000;
            if (feeWalletAmount > 0)
                _transferRewardsToken(feeWallet, feeWalletAmount, true);
            _transferRewardsToken(msg.sender, pendingAmount - feeWalletAmount, true);
            usersReserves[_pid][msg.sender] = 0;
        }
        ...
        user.boostedShare += netDeposit.add(netDeposit.mul(usersBoosts[msg.sender]).div(100));
        pool.boostedShares += netDeposit.add(netDeposit.mul(usersBoosts[msg.sender]).div(100));
        user.rewardDebt = user.boostedShare.mul(pool.accERC20PerShare).div(1e36);
        emit Stake(msg.sender, _pid, netDeposit);
    }
    ...
    function unstake(uint256 _pid, uint256 _amount) external whenNotPaused {
        ...
        if (feeRewardAmount > 0)
            _transferRewardsToken(feeWallet, feeRewardAmount, true);
        _transferRewardsToken(msg.sender, pendingAmount - feeRewardAmount, true);
        usersReserves[_pid][msg.sender] = 0;

        user.amount = user.amount.sub(_amount);
        user.boostedShare = user.amount.add(user.amount.mul(usersBoosts[msg.sender]).div(100));
        user.rewardDebt = user.boostedShare.mul(pool.accERC20PerShare).div(1e36);
        ...
        pool.boostedShares -= (_amount).add(_amount.mul(usersBoosts[msg.sender]).div(100));
    ...
    function emergencyWithdraw(uint256 _pid) external whenPaused {
        ...
        pool.boostedShares -= user.amount.add(user.amount.mul(usersBoosts[msg.sender]).div(100));
        delete userInfo[_pid][msg.sender];
    }

    // Transfer ERC20 and update the required ERC20 to payout all rewards
    function _transferRewardsToken(address _to, uint256 _amount, bool _isPayout) internal {
        LQDToken.transfer(_to, _amount);
        if (_isPayout)
            paidOut += _amount;
    }
    ...
    */
    ...
    function changeRewardPerBlock(uint256 _rewardPerBlock) external onlyOwner onlyAfterInit {
        ...
        if (leftoverRewards > 0) {
            ...
            _transferRewardsToken(feeWallet, leftoverRewards, false);
