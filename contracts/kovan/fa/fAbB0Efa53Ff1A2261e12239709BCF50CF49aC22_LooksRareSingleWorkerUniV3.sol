...
import "../../../../apis/UniswapV3/ISwapRouter.sol";
import "../../../interfaces/IStrategy.sol";
import "../../../interfaces/IWorker02UniV3.sol";
import "../../../../utils/SafeToken.sol";
import "../../../interfaces/IVault.sol";
import "../../../interfaces/IVaultConfig.sol";
import "../../../interfaces/ITwapOracle.sol";
import "../../../../looksrare/interfaces/IFeeSharingSystem.sol";
import "../../../../looksrare/interfaces/ITokenDistributor.sol";

contract LooksRareSingleWorkerUniV3 is
  Ownable,
  ReentrancyGuard,
  IWorker02UniV3
{
  ...
  event SetReinvestConfig(address indexed caller, uint256 reinvestBountyBps);

  /// @notice Configuration variables
  IUniswapV3Factory public factory;
  ISwapRouter public router;
  IUniswapV3Pool public override lpToken;
  IVaultConfig public config;
  ITwapOracle public oracle;
  IFeeSharingSystem public feeSharing;
  ...
  /// @notice Configuration varaibles for Uniswap fee
  uint256 public fee;
  ...
  constructor(
    ...
    uint24 _poolFee,
    ITwapOracle _oracle,
    IFeeSharingSystem _feeSharing
  ) public {
    // 1. Assign dependency contracts
    operator = _operator;
    wNative = _router.WETH9();
    router = _router;
    factory = IUniswapV3Factory(_router.factory());
    config = IVaultConfig(IVault(_operator).config());
    oracle = ITwapOracle(_oracle);
    feeSharing = _feeSharing;

    // 2. Assign tokens state variables
    baseToken = _baseToken;
    farmingToken = feeSharing.looksRareToken();
    rewardToken = feeSharing.rewardToken();

    // 3. Assign critical strategy contracts
    addStrat = _addStrat;
    liqStrat = _liqStrat;
    okStrats[address(addStrat)] = true;
    okStrats[address(liqStrat)] = true;

    // 4. Assign re-invest parameters
    reinvestBountyBps = _reinvestBountyBps;
    maxReinvestBountyBps = 2000;

    // 5. Assign Path parameters
    path = _path;

    // 6. Setup Uniswap swap fee
    fee = 9970;
    feeDenom = 10000;
    poolFee = _poolFee;
    ...
    require(
      path.length >= 2,
      "LooksRareSingleWorkerUniV3::constructor:: path length must be >= 2"
    );
    require(
      path[0] == baseToken && path[path.length - 1] == farmingToken,
      "LooksRareSingleWorkerUniV3::constructor:: path must start with base token and end with farming token"
    );
    require(
      reinvestBountyBps <= maxReinvestBountyBps,
      "LooksRareSingleWorkerUniV3::constructor:: reinvestBountyBps exceeded maxReinvestBountyBps"
    );
  }
  ...
  modifier onlyEOA() {
    require(
      ...
      "LooksRareSingleWorkerUniV3::onlyEOA:: not eoa"
    );
    _;
  }
  ...
  modifier onlyOperator() {
    require(
      msg.sender == operator,
      "LooksRareSingleWorkerUniV3::onlyOperator:: not operator"
    );
    _;
  }
  ...
  function getShares() public view returns (uint256 _shares) {
    (_shares, , ) = feeSharing.userInfo(address(this));
  }

  function calculateShares(uint256 amount) public view returns (uint256) {
    // Retrieve total amount staked
    (uint256 totalAmountStaked, ) = ITokenDistributor(
      feeSharing.tokenDistributor()
    ).userInfo(address(feeSharing));
    uint256 currentShares;
    uint256 totalShares = feeSharing.totalShares();
  ...
  function shareToBalance(uint256 share) public view returns (uint256) {
    ...
    uint256 totalBalance = getShares();
  ...
  function balanceToShare(uint256 balance) public view returns (uint256) {
    if (totalShare == 0) return calculateShares(balance); // When there's no share, 1 share = 1 balance.
    uint256 totalBalance = getShares();
    return calculateShares(balance).mul(totalShare).div(totalBalance);
  }
  ...
  function reinvest() external override onlyEOA onlyReinvestor nonReentrant {
    require(canReinvest(), "can not reinvest yet");
    // 1. Withdraw all the rewards.
    uint256 balanceBefore = rewardToken.myBalance();
    feeSharing.harvest();
    uint256 balanceAfter = rewardToken.myBalance();
    uint256 reward = balanceAfter.sub(balanceBefore);
    // 2. Send the reward bounty to the caller.
    uint256 bounty = reward.mul(reinvestBountyBps).div(10000);
    if (bounty > 0) rewardToken.safeTransfer(msg.sender, bounty);
    // 3. Convert all the remaining rewards to FarmingToken.
    address(rewardToken).safeApprove(address(router), type(uint256).max);
    ISwapRouter.ExactInputSingleParams memory params = ISwapRouter
      .ExactInputSingleParams({
        tokenIn: rewardToken,
        tokenOut: farmingToken,
        fee: poolFee,
        recipient: address(this),
        deadline: block.timestamp,
        amountIn: reward.sub(bounty),
        amountOutMinimum: 0,
        sqrtPriceLimitX96: 0
      });
    router.exactInputSingle(params);
    address(rewardToken).safeApprove(address(router), 0);
    uint256 balance = actualFarmingTokenBalance();
    // 4. Deposit all farmingToken to FeeSharingSystem.
    address(farmingToken).safeApprove(address(feeSharing), type(uint256).max);
    feeSharing.deposit(balance, false);
    address(farmingToken).safeApprove(address(feeSharing), 0);
    emit Reinvest(msg.sender, reward, bounty);
  }

  function canReinvest() public view returns (bool) {
    uint256 reward = feeSharing.calculatePendingRewards(address(this));
    uint256 bounty = reward.mul(reinvestBountyBps).div(10000);
    uint256 convertAmt = oracle.getAmountOut(
      rewardToken,
      reward.sub(bounty),
      farmingToken,
      poolFee
    );
    return convertAmt > 1.1 ether;
  }
  ...
  function work(
    ...
    require(
      okStrats[strat],
      "LooksRareSingleWorkerUniV3::work:: unapproved work strategy"
    );
  ...
  function health(uint256 id) external view override returns (uint256) {
    ...
    uint256 _shares = shareToBalance(shares[id]);
    amount[0] = _shares.mul(feeSharing.calculateSharePriceInLOOKS()).div(
      1 ether
    );

    for (uint256 i = 1; i < reversedPath.length; i++) {
      amount[i] = oracle.getAmountOut(
        reversedPath[i - 1],
        amount[i - 1],
        reversedPath[i],
        poolFee
  ...
  function _addShare(uint256 id) internal {
    ...
    if (shareBalance > 0) {
      ...
      // 4. Deposit balance to FeeSharingSystem
      feeSharing.deposit(shareBalance, false);
  ...
  /// so it needs to return the current reward balance to be excluded from the farming token balance
  function _removeShare(uint256 id) internal {
    uint256 share = shares[id];
    if (share > 0) {
      uint256 looksShare = shareToBalance(share);
      totalShare = totalShare.sub(share);
      shares[id] = 0;
      feeSharing.withdraw(looksShare, false);
  ...
  function setReinvestConfig(uint256 _reinvestBountyBps) external onlyOwner {
    require(
      _reinvestBountyBps <= maxReinvestBountyBps,
      "LooksRareSingleWorkerUniV3::setReinvestConfig:: _reinvestBountyBps exceeded maxReinvestBountyBps"
    );

    reinvestBountyBps = _reinvestBountyBps;

    emit SetReinvestConfig(msg.sender, _reinvestBountyBps);
  }
  ...
  function setMaxReinvestBountyBps(uint256 _maxReinvestBountyBps)
    external
    onlyOwner
  {
    require(
      _maxReinvestBountyBps >= reinvestBountyBps,
      "LooksRareSingleWorkerUniV3::setMaxReinvestBountyBps:: _maxReinvestBountyBps lower than reinvestBountyBps"
    );
    require(
      _maxReinvestBountyBps <= 3000,
      "LooksRareSingleWorkerUniV3::setMaxReinvestBountyBps:: _maxReinvestBountyBps exceeded 30%"
    );
  ...
  /// @param isOk Whether to approve or unapprove the given addresses.
  function setReinvestorOk(address[] calldata reinvestors, bool isOk)
    external
    override
    onlyOwner
  {
  ...
  function setPath(address[] calldata _path) external onlyOwner {
    require(
      _path.length >= 2,
      "LooksRareSingleWorkerUniV3::setPath:: path length must be >= 2"
    );
    require(
      _path[0] == baseToken && _path[_path.length - 1] == farmingToken,
      "LooksRareSingleWorkerUniV3::setPath:: path must start with base token and end with farming token"
    );
  ...
  function setCriticalStrategies(IStrategy _addStrat, IStrategy _liqStrat)
    external
    onlyOwner
  {
    addStrat = _addStrat;
...
import "./IWorkerUniV3.sol";

interface IWorker02UniV3 is IWorkerUniV3 {
...
interface IVault {
  ...
  /// Return owner of the given position.
...
interface ITokenDistributor {
  ...
  function stakingPeriod(uint256)
    external
    view
    returns (
...
interface IWorkerUniV3 {
  ...
  function lpToken() external view returns (IUniswapV3Pool);
