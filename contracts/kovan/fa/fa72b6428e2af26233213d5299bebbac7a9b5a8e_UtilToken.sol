...
contract UtilToken is ERC721AUpgradeable, BlackListUpgradeable,  PausableUpgradeable, OwnableUpgradeable, ReentrancyGuardUpgradeable, IUtilToken{
    ...
    function mintSanityCheck(address callerAddress, bytes32[] calldata _merkleProof) internal {
        require(batchCollection[batchCollectionArrayLength()].isSaleActive, "Sale must be active to mint NFT");
        require(batchCollection[batchCollectionArrayLength()].mintPrice <= msg.value, "Insufficient balance");
        if(block.timestamp < batchCollection[batchCollectionArrayLength()].endTime && block.timestamp > batchCollection[batchCollectionArrayLength()].startTime){
            bytes32 leaf = keccak256(abi.encodePacked(callerAddress));
            require(
                MerkleProofUpgradeable.verify(_merkleProof, batchCollection[batchCollectionArrayLength()].merkleHash, leaf),
                "MerkleDistributor: Invalid proof."
            );
            require(!whitelistClaimed[batchCollectionArrayLength()][msg.sender], "Whitelisted user already minted for current batch");
            whitelistClaimed[batchCollectionArrayLength()][msg.sender] = true;
        }
    }
    ...
    function airDrop(address[] memory assigned, uint256[] memory quantity) public override onlyOwner whenNotPaused {
        ...
        for (uint256 index = 0; index < assigned.length; index++) {
            if(!_isBlackListUser(assigned[index])){
                _safeAirdrop(assigned[index], quantity[index]);
            }
        }
    }

    function addBatch(uint256 _startTime, uint256 _endTime, uint256 _mintPrice, bytes32 _merkleHash, bool _publicMint) public onlyOwner whenNotPaused {
        require(block.timestamp < _endTime, "New Batch end time should be greater than current time");
        require(batchCollection[batchCollectionArrayLength()].endTime < _startTime, "New Batch start should be after previous batch end time");
        require(_mintPrice > 0, "New Batch mint price should be greater than zero");
        Batch memory tempBatch = Batch(_startTime, _endTime, _mintPrice, _merkleHash, _publicMint);
        batchCollection.push(tempBatch);
    }

    function updateCurrentBatch(uint256 _endTime,bool isActive) public onlyOwner whenNotPaused {
        uint256 batchArrayLength = batchCollectionArrayLength();
        require(batchCollection[batchArrayLength].endTime < block.timestamp, "Batch time already passed");
        batchCollection[batchArrayLength].endTime = _endTime;
        batchCollection[batchArrayLength].isSaleActive = isActive;
    }

    function batchCollectionArrayLength() public view returns(uint256){
        return batchCollection.length.sub(1);
    }
    ...
    function isAirDropToken(uint256 index) public view override returns (bool){
        return _airDroppedTokens[index];
    }
    ...
    modifier airDropCheck(uint256 startTokenId, uint256 quantity) {
        for (uint256 i = 0; i < quantity; i++) {
            require(
                !_airDroppedTokens[startTokenId.add(i)],
...
interface IUtilToken {
     ...
     */
    function airDrop(address[] memory assigned, uint256[] memory quantity) external;
    ...
    function isAirDropToken(uint256 index) external view returns (bool);
}
    ...
    IERC721EnumerableUpgradeable
{
    ...
    // Mapping from airdropped token ID to boolean
    mapping(uint256 => bool) internal _airDroppedTokens;
    ...
    function _safeAirdrop(address to, uint256 quantity) internal {
    ...
    /**
     * @dev Airdrops `quantity` tokens and transfers them to `to`.
     *
     * Requirements:
     *
     * - `to` cannot be the zero address.
    ...
    function _safeAirdrop(
        ...
        for (uint256 i = 0; i < quantity; i++) {
            ...
            _airDroppedTokens[updatedIndex] = true;
            updatedIndex++;
        }
