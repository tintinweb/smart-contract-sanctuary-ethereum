...
interface IRestrictedSafeProxyFactory {
    function isWalletApproved(address proxy) external view returns (bytes4);
}

abstract contract Gated is Context {
    // bytes4(keccak256("isWalletApproved(address)")
    bytes4 internal constant APPROVED_WALLET_MAGIC_VALUE = 0x3657e851;
    IRestrictedSafeProxyFactory internal _proxyFactory;

    constructor(address proxyFactory) {
        _proxyFactory = IRestrictedSafeProxyFactory(proxyFactory);
    }

    function isRestrictedWallet(address wallet) internal view returns (bytes4) {
        return _proxyFactory.isWalletApproved(wallet);
    }

    modifier onlyRestrictedWallet() {
        require(isRestrictedWallet(_msgSender()) == APPROVED_WALLET_MAGIC_VALUE, "Sender not approved to make the request");
        _;
    }
}
...
LOOKSRARELOOKSRARELOOKSRLOOKSRARELOOKSRARELOOKSRARELOOKSRARELOOKSRLOOKSRARELOOKSRARELOOKSR
 */
contract LooksRareExchange is ILooksRareExchange, ReentrancyGuard, Ownable, Gated {
    ...
    ) external payable override nonReentrant onlyRestrictedWallet {
        ...
        nonReentrant
    {
        ...
        //     msg.sender,
        //     makerBid.signer,
        //     block.timestamp + makerBid.duration,
        //     takerAsk.price,
        //     takerAsk.currency
        // );
    ...
    //     address currency
    // ) internal {
    //     require(lender != address(0) && borrower != address(0), "invalid addresses");
    //     IRestrictedSafe(borrower).createBorrow(
    //         makerOrderHash,
    //         collection,
    //         tokenId,
    //         lender,
    ...
    function _validateOrder(OrderTypes.MakerOrder calldata makerOrder, bytes32 orderHash) internal view {
        ...
        if (!makerOrder.isOrderAsk) {
            require(isRestrictedWallet(makerOrder.signer) == APPROVED_WALLET_MAGIC_VALUE, "Order: Invalid signer (non-approved wallet)");
        }
 ...
 */
library OrderTypes {
    // keccak256("MakerOrder(bool isOrderAsk,address signer,address collection,uint256 price,uint256 tokenId,uint256 amount,address strategy,address currency,uint256 nonce,uint256 startTime,uint256 endTime,uint256 minPercentageToAsk,bytes params,uint256 duration)")
    bytes32 internal constant MAKER_ORDER_HASH = 0xb4f9c13a195e6cd9d30e046a976a0826f6ec3f2748300c8f1b80ab84d5859781;
