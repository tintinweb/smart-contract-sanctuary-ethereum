...
error SpecEntryLibrary__SpecEntryNotInserted(bytes32 key);

library SpecEntryLibrary {
    struct Map {
        bytes32[] keys;
    ...
    function getSpecEntry(Map storage _map, bytes32 _key) internal view returns (SpecEntry memory) {
    ...
    function remove(Map storage _map, bytes32 _key) internal {
        if (!_map.inserted[_key]) {
            revert SpecEntryLibrary__SpecEntryNotInserted(_key);
        }
