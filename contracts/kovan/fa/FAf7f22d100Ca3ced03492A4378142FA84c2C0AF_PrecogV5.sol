...
contract PrecogV5 is IPrecogV5 {
    ...
    function getCurrentProfitCycleByTimestamp(address token, uint timestamp)
        public
        view
        returns (
            uint currentCycleId,
        ...
        )
    {
        ...
        while (uint48(timestamp) >= lastCycle.endTime) {
            newCycleStartTime = lastCycle.endTime;
            if (
                lastCycle.endTime < lastChangeProfitCycle &&
                (newCycleStartTime + duration) >= lastChangeProfitCycle
        ...
        if (lastProfitOfId < updatedLatestCycle) {
            for (investmentId; investmentId < investment.length; investmentId++) {
                ...
                for (lastProfitOfId; lastProfitOfId < lastAvailableProfitId; lastProfitOfId++) {
                    uint totalInvestmentUnit = totalInvestmentUnits[token][lastProfitOfId];
    ...
    function _increaseInvestment(
        ...
        uint48 duration = uint48(core.getCycleConfiguration().profitCycle);
        uint48 nextInvestmentTimestamp = ((uint48(block.timestamp) - _profitCycles[0].startTime) / duration + 1) * duration + _profitCycles[0].startTime;
        Cycle memory futureProfitCycle; 
        {
            (uint futureProfitCycleId, uint futureStartTime, uint futureEndTime) = getCurrentProfitCycleByTimestamp(token, nextInvestmentTimestamp);
            futureProfitCycle = Cycle(uint16(futureProfitCycleId), uint48(futureStartTime), uint48(futureEndTime));
            require(futureProfitCycle.endTime >= uint48(block.timestamp), "PrecogV5: INVALID_TIMESTAMP");
        }
        uint unit = amount * (futureProfitCycle.endTime - nextInvestmentTimestamp);
        uint updatedUnit;
        if (!isNotFirstIncreaseInvestment[token][account]) {
            lastProfitCycleId[token][account] = uint16(futureProfitCycle.id);
        ...
        if (investmentsOf.length > 0) {
            ...
            if (lastInvestmentOf.idChanged < futureProfitCycle.id) {
                updatedUnit = lastInvestmentOf.amount * (futureProfitCycle.endTime - futureProfitCycle.startTime) + unit;
                Investment memory newInvestmentOf = Investment(
                    lastInvestmentOf.amount + amount,
                    updatedUnit,
                    futureProfitCycle.id
                );
            ...
            Investment memory newInvestmentOf = Investment(amount, unit, futureProfitCycle.id);
            investmentOf[token][account].push(newInvestmentOf);
        }
        totalInvestmentUnits[token][futureProfitCycle.id] += unit;
    ...
    function _decreaseInvestment(
        ...
        totalInvestmentUnits[token][lastProfitCycle.id] -= unit;
