...
contract BrightBundles is Ownable, ReentrancyGuard
{
     ...
     */
    event BundleEnded(uint BundleIndex);
    ...
    event BundleChangedActiveState(uint indexed BundleIndex, bool OldState, bool NewState);
    event BundleChangedActiveStateBrightList(uint indexed BundleIndex, bool OldState, bool NewState);
    ...
    { 
        require(Bundles[BundleIndex].ActivePublic, "Requested Bundle Is Not Available For Public Purchases");
        require(Bundles[BundleIndex].EndingIndexes[0] - Bundles[BundleIndex].StartingIndexes[0] > 0, "Sold Out");
        require(
        ...
        if(!Bundles[BundleIndex].AllowMultiplePurchases) { require(!BundlePurchased[BundleIndex][msg.sender], "User Has Already Purchased This Bundle Index"); }
        require(msg.value == Bundles[BundleIndex].PricePublic * Amount && Amount > 0, "Incorrect Ether Amount Or Token Amount Sent For Purchase");
    ...
    {
        require(Bundles[BundleIndex].ActiveBrightList, "Requested Bundle Is Not Available For BrightList Purchases");
        require(Bundles[BundleIndex].EndingIndexes[0] - Bundles[BundleIndex].StartingIndexes[0] > 0, "Sold Out");
        require(msg.value == Bundles[BundleIndex].PriceBrightList * Amount && Amount > 0, "Incorrect Ether Amount Or Token Amount Sent For Purchase");
        require(viewBrightListAllocation(msg.sender, BundleIndex, Proof), "User Is Not On BrightList");
        require(
    ...
    /**
     * @dev Ends Bundle At Index `BundleIndex`
     */
    function _EndBundle(uint BundleIndex) external onlyBRTOperator 
    { 
        Bundles[BundleIndex].ActivePublic = false; 
        Bundles[BundleIndex].ActiveBrightList = false;
        emit BundleEnded(BundleIndex);
    }
