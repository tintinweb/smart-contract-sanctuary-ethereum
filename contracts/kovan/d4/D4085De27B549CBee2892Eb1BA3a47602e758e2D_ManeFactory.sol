...
contract ManeBase is ERC721AM, Ownable {
    ...
    */ 
    constructor(string memory name_, string memory symbol_, uint256[] memory u256s,
        ...
        require(u256s[0] + u256s[2] <= u256s[1], "ManeB: small maxSupply");
        ...
        require(totalShareRatios <= 1 * 1000 * 1000, "ManeB: shareRatios overflow");

        /// 4. Setting refund times
        require(refundTimes_.length == refundRatios_.length, "ManeB: length mismatch");
        ...
        for( uint256 i = 0; i < refundTimes_.length; i++) {
            ...
            require(t.endTime > oldEndTime, "ManeB: refundTimes invalid");
            require(t.ratioPPM < oldRatio, "ManeB: refundRatio invalid");
    ...
    function mint(address minter, uint256 mint_price, uint256 count, uint256 sign_deadline, bytes32 r, bytes32 s, uint8 v) payable whenNotPaused public {
        ...
        if (block.timestamp < presaleStartTime) {
            ...
            revert("ManeB: Not started");
        } else if (block.timestamp >= presaleStartTime && block.timestamp < presaleEndTime) {
            // Period: Pre-sale period
            require(msg.value >= mint_price * count, "ManeB: insufficant value for presale");
            isPresale = 1;
        } else if (block.timestamp >= saleStartTime && block.timestamp <= saleEndTime) {
            // Period: Public sale perild
            require(msg.value >= _salePrice * count, "ManeB: insufficant value for sale");
            isSale = 1;
        } else {
            revert("ManeB: Invalid period");
        }

        /// Mint `count` number of tokens
        for (uint256 i = 0; i < count; i++) {
            require(totalMinted() < _maxSupply, "ManeB: No more tokens");
            ...
            // require(actualPlatformFee >= expectPlatformFee, "ManeB: insufficant platform fee");

            if (isPresale == 1) {
                requireMintSign(minter, mint_price, count, sign_deadline, r, s, v);

                presaleMintedCount++;
                require(presaleMintedCount <= presaleMaxSupply, "ManeB: Exceed for presale");
                
                presaleMintCountByAddress[msg.sender]++;
                require(presaleMintCountByAddress[msg.sender] <= presaleMaxMintCountPerAddress, "ManeB: addr limit(A)");
            } else if (isSale == 1) {
                //requireMintSign(minter, mint_price, sign_deadline, r, s, v);

                saleMintCountByAddress[msg.sender]++;
                require(saleMintCountByAddress[msg.sender] <= saleMaxMintCountPerAddress, "ManeB: addr limit(B)");
                
            } else {
                revert("ManeB: Not sale period");
            }
    ...
    function refund(uint256 tokenID) public {
        require(msg.sender == ownerOf(tokenID), "ManeB: Not owner");
        ...
        require(refundRatioPPM > 0, "ManeB: refund not avail");

        /// 2. Get mint info and check if this token is refundable
        TMintInfo storage mintInfo = _mintInfo[tokenID];
        
        require(mintInfo.isRefunded == 0, "ManeB: already refunded");
    ...
    function requireCollectable() view internal {
        for (uint256 i = 0; i < _refundTimeList.length; i++) {
            require(block.timestamp > _refundTimeList[i].endTime, "ManeB: Not allow before refund deadline");
        }
    }
    ...
    function requireMintSign(address minter, uint256 price, uint256 count, uint256 deadline, bytes32 r, bytes32 s, uint8 v)  internal view {
        ...
        //require(msg.sender == minter, "ManeB: Invalid sender in signature");

        //require(hash_address == owner(), "ManeB: Invalid signature");
        require(hash_address == ManeFactory(factory).signerAddress(), "ManeB: Invalid sign");
    }
    ...
    //     require(newMaxSupply >= totalMinted(), "ManeB: maxSupply too small");
    ...
    //     require(preSaleStartTime_ <= saleStartTime_, "ManeB: preSaleStartTime must be smaller than saleStartTime");
    ...
    function getCollectorTotalRatioPPM() internal view returns (uint256) {
        ...
        require(ratioPPM <= 1e6, "ManeB: ratio overflow");
    ...
    function destroy() public onlyOwner {
        //require(mintedAmount == 0, "ManeB: not allow after mint");
        require(_currentIndex == _reserveQuantity, "ManeB: not allow");
...
contract ManeFactory is SignAndOwnable {
    ...
    function deploy(string memory name_, string memory symbol_,  uint256[] memory u256s, address[] memory shareAddresses_, uint256[] memory shareRatios_, uint256[] memory refundEndTimes_, uint256[] memory refundRatios_, uint8 v, bytes32 r, bytes32 s) public returns (address) {
        ...
        require(_usedNonces[u256s[12]] == false, "ManeF: dup nonce");
        _usedNonces[u256s[12]] = true;

        require(verifySignature(user_hash, v, r, s) == true, "ManeF: invalid sign");
    ...
    function requireOriginIsOwner() view public {
        require(tx.origin == owner(), "ManeF: Not owner");
    }
}
