...
contract MooneryLottery is Lottery, VRFConsumerBase {
    ...
    constructor() VRFConsumerBase(vrfCoordinator, linkToken) Lottery(25) {
    ...
    function fulfillRandomness(bytes32 _requestId, uint256 _randomness) internal override {
        ...
        Ticket memory winnerTicket = _determineRandomWinnerTicket(_randomness);
    ...
    function _determineRandomWinnerTicket(uint256 randomness) internal view returns (Ticket memory) {
        ...
        // throw error, if no ticket cloud be determined. Should not be reached.
        revert("No winner ticket could be determined.");
    }
}
...
abstract contract Lottery {
    ...
    uint8 lotteryBlockDuration;

    event LotteryEndBlockNrDetermined(uint256 blockNr);

    constructor(uint8 _lotteryBlockDuration) {
        lotteryBlockDuration = _lotteryBlockDuration;
