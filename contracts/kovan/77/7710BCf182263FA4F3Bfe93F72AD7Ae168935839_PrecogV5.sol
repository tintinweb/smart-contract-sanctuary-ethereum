...
contract PrecogV5 is IPrecogV5 {
    ...
    modifier onlyPrecogCore() {
        require(msg.sender == precogStorage.getCore());
        _;
    }
    ...
    function _getVaultInstance() internal view returns (IPrecogVault _vault) {
        _vault = IPrecogVault(precogStorage.getVault());
    }
    ...
    function deposit(address token, uint amount) external override isExistingToken(token) {
        ...
        // Transfer tokens
        IERC20(token).safeTransferFrom(msg.sender, precogStorage.getVault(), actualDepositAmount);
        IERC20(token).safeTransferFrom(msg.sender, precogStorage.getCore(), feeDeposit);
        IIPCOG(precogStorage.getTokenConvert(token)).mint(msg.sender, actualDepositAmount);
        precogInternal.updateDepositInfo(token, msg.sender, actualDepositAmount);
