...
contract FilthyRichCryptoKids is ERC721, Ownable, VRFConsumerBase{
    ...
    uint maxVipSupply = 1000;
    // Index for regular mints
    uint minted = maxVipSupply;
    // Index fo VIP mints
    uint vipMinted = 0;

    string private _customBaseURI = "";

    mapping(address => bool) isSaleContract;

    address vaulablesContract;
    ...
    modifier onlySeller () {
        require(isSaleContract[address(msg.sender)], "Caller is not a seller");
        _;
    }
    ...
    function getRandomNumber() public onlyOwner {
        ...
        require(randomnessRequested == false, "Randomness has already been called");
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory){
        ...
        if(tokenId < maxVipSupply){
            ...
            uint tokenId_n = tokenId.add(randomness & n) % n; // 1 + 0 
    ...
    function setVaulablesContract(address _vaulablesContract) external onlyOwner {
        vaulablesContract = _vaulablesContract;
    }

    function getValuables(uint tokenId) external view returns(string[] memory traits, uint[] memory types) {
        return IFRCKVaulables(vaulablesContract).getState(tokenId);
    }
 
    function batchMint(uint _amount, address _recipient) external onlySeller {
        require(minted.add(_amount) < maxSupply, "Purchase would exceed max supply");
    ...
    function mintVip(uint _amount, address _recipient) external onlySeller {
        require(vipMinted.add(_amount) < maxVipSupply, "Purchase would exceed max vip supply");
         for(uint i = 0; i < _amount; i++) {
