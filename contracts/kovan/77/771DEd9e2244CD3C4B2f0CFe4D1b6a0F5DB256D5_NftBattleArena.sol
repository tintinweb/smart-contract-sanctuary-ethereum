...
contract NftBattleArena is Ownable
{
	...
	event ClaimedRewardFromVoting(uint256 indexed currentEpoch, address indexed voter, uint256 indexed stakingPositionId, address beneficiary, uint256 reward, uint256 votingPositionId);

	/// @notice Event about claimed reward from staking.        FirstStage
	event ClaimedRewardFromStaking(uint256 indexed currentEpoch, address indexed staker, uint256 indexed stakingPositionId, address beneficiary, uint256 reward);
	...
	function createVotingPosition(uint256 stakingPositionId, address voter, uint256 amount) external only(nftVotingPosition) returns (uint256 votes, uint256 votingPositionId)
	{
		...
		require(stakingPositionsValues[stakingPositionId].startDate != 0, "Staking doesn't exist"); // Requires to exist.
	...
	function withdrawDaiFromVoting(uint256 votingPositionId, address voter, uint256 daiNumber, address beneficiary) external only(nftVotingPosition)
	{
		...
		rewardsForEpoch[stakingPositionId][currentEpoch].yTokens = yTokens * (daInvested - daiNumber) / daInvested;
		
		vault.withdraw(yTokens * (daInvested - daiNumber) / daInvested, beneficiary);
	...
	function claimRewardFromVoting(uint256 votingPositionId, address voter, address beneficiary) external only(nftVotingPosition) returns (uint256 rewardDaiAmount)
	{
		...
		emit ClaimedRewardFromVoting(currentEpoch, voter, stakingPositionId, beneficiary, yTokens, votingPositionId);
	...
	function claimRewardFromStaking(uint256 stakingPositionId, address staker, address beneficiary) public only(nftStakingPosition)
	{
		...
		updateInfo(stakingPositionId);
		(uint256 stakerReward, uint256 end) = getPendingStakerReward(stakingPositionId);
		...
		emit ClaimedRewardFromStaking(currentEpoch, staker, stakingPositionId, beneficiary, stakerReward);
	}
