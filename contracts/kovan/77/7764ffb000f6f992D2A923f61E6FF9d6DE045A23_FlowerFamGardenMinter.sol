...
contract FlowerFamGardenMinter is OwnableUpgradeable {
    ...
    function initialize(
       ...
       flowerFamGarden = IFlowerFamGarden(_flowerFamGardenNFT);
        ...
        startTimeClaim = 1656139802;
        endTimeClaim = 1656139802 + 100 minutes;

        startTimePublic = 1655967002;
    }


    receive() external payable {}

    function _merkleProofMint(bytes32[] calldata proof, uint256 amount) internal {
        ...
        require(
            flowerFamGarden.totalSupply() + amount <= maxSupply,
            "Flower Fam Garden NFT is sold out"
        );
        require(addressToClaim[msg.sender] + amount <= amount);

        addressToClaim[msg.sender] += amount;

        flowerFamGarden.mint(msg.sender, amount);
    }

    function claimMint(bytes32[] calldata proof, uint256 amount) external payable {
        require(block.timestamp >= startTimeClaim, "Claim mint not started");
        require(block.timestamp < endTimeClaim, "Claim mint closed");

        _merkleProofMint(proof, amount);
    }
