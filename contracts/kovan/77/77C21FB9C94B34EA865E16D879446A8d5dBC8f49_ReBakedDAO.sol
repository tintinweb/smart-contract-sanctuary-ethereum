...
/**
	...
	*/
		...
		function getRejectedPayment(bytes32 _projectId, bytes32 _packageId,address _collaborator) external view onlyOwner returns(uint256,uint256) {
...
/**
	...
	*/
		...
		returns(bytes32 packageId_)
	{
		...
		IERC20(_token).safeTransferFrom(msg.sender,treasury,total*5*10e15);
	...
	*/
	...
	function canclePackage (
	 ...
	 onlyInitiator(projectId_)
	{
		for(uint i = 0; i < collaborator_.length; i++) {
			payMgp(projectId_,packageId_,collaborator_[i]);
		}
		for(uint i = 0; i < observer_.length; i++){
			payObserverFee(projectId_,packageId_,observer_[i]);
		}
			_revertPackageBudget(projectId_,toBeReverted_);
			_canclePackage(projectId_,packageId_);
	}
		...
		public
	{
		...
		uint amount_ = collaboratorData[projectId_][packageId_][collaborator_].mgp;
	...
	*/
	...
	function removeCollaborator(
		...
		external
	{
	  address initiator_ = projectData[projectId_].initiator;	
		if(packageStatus == true && msg.sender == initiator_){
		...
		external
	{
		for(uint i = 0; i < packageId_.length; i++) {
					address initiator_ = projectData[projectId_].initiator;		
			if(packageStatus[i] == true && msg.sender == initiator_){
	...
	function payObserverFee(
		...
		public
	{
		uint concernedFee = uint(packageData[projectId_][packageId_].budgetObservers/packageData[projectId_][packageId_].totalObservers);
	...
	*/
		...
		returns (uint256 budgetLeft_)
	{
		...
		budgetLeft_ = packageData[projectId_][packageId_].budget - packageData[projectId_][packageId_].budgetAllocated; 
...
interface IReBakedDAO {
	...
	*/
	function createPackage(
        bytes32 projectId_, uint256 budget_, uint256 bonus_, uint256 observerBudget_, uint256 budgetWithDeduction_) external returns(bytes32 packageId_);
	...
	*/
	...
	function canclePackage(
		bytes32 projectId_, bytes32 packageId_,address[] calldata collaborator_,address[] calldata observer_,uint256 toBeReverted_
	) external ;
	...
	*/
	...
	function removeCollaborator(bytes32 projectId_, bytes32 packageId_, address collaborator_,bool packageStatus) external;

	function removeObserver(bytes32 projectId_, bytes32[] calldata packageId_, address observer_,bool[] calldata packageStatus) external;
	...
	* @param projectId_ Id of the project 
	*/
	function finishPackage( bytes32 projectId_, bytes32 packageId_) external returns (uint256 budgetLeft_);
...
contract Packages is Observers, Collaborators {
	...
	*/
	...
	function _canclePackage(
		bytes32 projectId_,
		bytes32 packageId_
	)
		internal
	{
		packageData[projectId_][packageId_]._canclePackage();
	}
...
library PackageLibrary {
	...
	modifier activePackage(Package storage package_) {
		require(package_.isActive == true,"already cancled!");
		_;
	}
	...
	*/
		...
		activePackage(package_)
	{
		...
		package_.timeCancled = block.timestamp;
