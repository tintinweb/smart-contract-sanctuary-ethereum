...
{   
    ...
    address public immutable _BRTMULTISIG = 0x5778B0B140Fa7a62B96c193cC8621e6E96c088A5;                 // BRT Multisig Address
    address public _ERC20_BRT_Token = 0x08ebADbc44F0f76CAFC6FD49e53247Cc9798eddD;                       // BRT ERC-20 Contract Address   
    address public _ArtBlocksMintingContractCitizens = address(0);                                      // ArtBlocks Minting Contract Citizens
    address public _ArtBlocksMintingContractArtists = 0xa07f47c30C262adcC263A4D44595972c50e04db7;       // ArtBlocks Minting Contract Artists
    address public _ArtBlocksCoreContractCitizens = 0xdAe755c2944Ec125a0D8D5CB082c22837593441a;         // KOVAN ArtBlocks Citizen NFT Collection Contract Address
    address public _ArtBlocksCoreContractArtists = 0xdAe755c2944Ec125a0D8D5CB082c22837593441a;          // ArtBlocks Artist NFT Collection Contract Address 
    address public _GoldenToken = 0xc22266Fa49E15BbE2f9622B8EEF652C5f8628A29;                           // Golden Token Contract Address
    uint public _ArtBlocksProjectID;                                                                    // ArtBlocks Project ID
    uint public _CurrentCityIndex;                                                                      // Current City Index
    bytes32 public Root;                                                                                // Current City Index

    /*-------------------*/
    /*     MAPPINGS      */
    /*-------------------*/

    mapping(uint => mapping(address => bool)) public Redeemed;                                      // Returns If User Has BrightList Minted NFT
    ...
    mapping(uint=>uint) public _QRIndex;                                                            // QR Code Index
    ...
    { 
        ...
        BookingsAvailable[_CurrentCityIndex][20220604] = 200; // 
        BookingsAvailable[_CurrentCityIndex][20220804] = 200; // 
        BookingsAvailable[_CurrentCityIndex][20220904] = 200; // 
        BookingsAvailable[_CurrentCityIndex][20221304] = 200; // 
        BookingsAvailable[_CurrentCityIndex][20221504] = 200; // 
        BookingsAvailable[_CurrentCityIndex][20221604] = 200; // 
        BookingsAvailable[_CurrentCityIndex][20221704] = 200; // 
        BookingsAvailable[_CurrentCityIndex][20222004] = 200; // 
        BookingsAvailable[_CurrentCityIndex][20222204] = 200; // 
        BookingsAvailable[_CurrentCityIndex][20222304] = 200; // 
        _QRIndex[_CurrentCityIndex] = 667; // First QR To Be Redeemed
    ...
    {
        require(BookingsAvailable[_CurrentCityIndex][TimeSlot] > 0, "LiveMint: TimeSlot Fully Booked");
        require(!Redeemed[_CurrentCityIndex][msg.sender], "User Has Already Redeemed");
        require(readBrightListMerkle(msg.sender, Proof), "User Is Not On BrightList");
        BookingsAvailable[_CurrentCityIndex][TimeSlot]--;
        SelectedTimeSlotGoldenTicket[_CurrentCityIndex][_QRIndex[_CurrentCityIndex]] = TimeSlot;
        BrightListCitizen[_CurrentCityIndex][_QRIndex[_CurrentCityIndex]] = msg.sender;
        emit QRRedeemed(msg.sender, _QRIndex[_CurrentCityIndex], TimeSlot);
        _QRIndex[_CurrentCityIndex]++;
    }

    // /**
    //  * @dev Redeems Spot For IRL Minting
    //  */
    // function RedeemQRArtist(uint ArtistID, uint TicketID, uint TimeSlot, bool Delegate, bytes32[] calldata Proof) public nonReentrant whenNotPaused
    // {
    //     require(BookingsAvailable[_CurrentCityIndex][TimeSlot] > 0, "LiveMint: TimeSlot Fully Booked");
    //     require(!Redeemed[_CurrentCityIndex][msg.sender], "User Has Already Redeemed");
    //     require(readBrightListMerkle(msg.sender, Proof), "User Is Not On BrightList");
    //     Redeemed[_CurrentCityIndex][msg.sender] = true;
    //     BookingsAvailable[_CurrentCityIndex][TimeSlot]--;
    //     SelectedTimeSlotGoldenTicket[_CurrentCityIndex][TicketID] = TimeSlot;
    //     BrightListCitizen[_CurrentCityIndex][TicketID] = msg.sender;
    //     DelegateStatusCitizen[_CurrentCityIndex][TicketID] = Delegate;
    //     emit QRRedeemed(msg.sender, TicketID, TimeSlot, Delegate);
    // }
