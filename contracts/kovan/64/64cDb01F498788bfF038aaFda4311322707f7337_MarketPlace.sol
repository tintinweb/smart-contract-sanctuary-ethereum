...
contract MarketPlace is Initializable, IERC721Receiver, Auction, Sale, Minting {
    ...
    */
    function setFee (uint8 newFee) external onlyManager returns (bool) {
        _fee = newFee;
        return true;
    }
    ...
    */
    function setPaymentToken (address paymentToken) external onlyManager returns (bool) {
        require(paymentToken != address(0), 'Payment token address should not be zero');
        _paymentToken = IERC20(paymentToken);
        return true;
    }
    ...
    */
    function setNftToken (address nftToken) external onlyManager returns (bool) {
        require(nftToken != address(0), 'Nft token address should not be zero');
        _nftToken = IERC721Token(nftToken);
        return true;
    }
    ...
    */
    function setFeeReceiver (address feeReceiver) external onlyManager returns (bool) {
...
contract Minting is Storage {
    ...
    */
    function acceptMintingRecord (
        ...
        uint256 tokenId = _nftToken.totalSupply() + 1;
        _mintingRecords[_mintingRecordsNumber].tokenId = tokenId;
    ...
    */
    function getNft (uint256 mintingRecordId) external returns (bool) {
        ...
        _nftToken.mint(
            msg.sender,
            _mintingRecords[mintingRecordId].tokenId,
            _mintingRecords[mintingRecordId].tokenUri
        );
        uint256 feeAmount = amount * _fee / 100;
        uint256 paymentAmount = amount - feeAmount;
    ...
    */
    ...
    function mintingRecordIsActive (uint256 mintingRecordId) public view returns (bool) {
        return _mintingRecords[mintingRecordId].completed == 0 &&
            _mintingRecords[mintingRecordId].tokenId > 0 &&
    ...
    function tokenUriInUse (
        ...
        return _nftToken.tokenIdByTokenUri(tokenUri) > 0
        || _tokenUriInUse[keccak256(abi.encode(tokenUri))];
    }
}
...
contract Storage is AccessControl {
    ...
    struct MintingRecord { // fixed price record
        uint256 tokenId; // ERC721 token id for the current record, is set automatically
        // when admin accepts record
        uint256 price; // ERC721 token price
