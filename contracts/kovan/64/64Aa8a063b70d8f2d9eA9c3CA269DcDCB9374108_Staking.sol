...
contract Staking is AccessControl {
    ...
    uint256 public constant YEAR = 10 minutes;//365 days;
    ...
    function depositOrWLFromOwner(
        ...
        uint256 timestamp = block.timestamp;
        require((timestamp - deployTime) / 60 < 20 /*1 days < 3*/, "0");
        ...
        nonReentrant
    {
        ...
        if (
            ...
            (timestamp - deployTime) / 60 < 30/*14 days < 1*/
        ) {
        ...
        returns (uint256 amount, uint256 months)
    {
        ...
        if (amount >= 3)
            months = (block.timestamp - (start + (3 * YEAR))) / 50;//30 days;
    }
