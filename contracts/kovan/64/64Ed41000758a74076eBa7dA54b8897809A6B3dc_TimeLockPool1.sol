...
contract TimeLockPool1 is BasePool, ITimeLockPool {
    ...
    // Input must be sorted by deposit id
...
abstract contract BasePool is AbstractRewards, IBasePool, TokenSaver {
    ...
    constructor(
        ...
        AbstractRewards(
            ...
            IVoteAnzenToken(_veAnzenToken).totalSupply
        )
    {
        require(
    ...
    function _mint(address _account, uint256 _amount) internal virtual {
        ...
        totalLocked += _amount;
        amountPerUser[_account] += _amount;
    ...
    function distributeRewards(uint256 _amount) external override {
        ...
        _distributeRewards(_amount, totalLocked);
    }

    function claimRewards(address _receiver) public {
...
abstract contract AbstractRewards is IAbstractRewards {
    ...
    function(address) external view returns (uint256) private getSharesOf;
    function() external view returns (uint256) private getTotalShares;
    ...
    constructor(
        ...
        function() external view returns (uint256) getTotalShares_
    ) {
    ...
    {
        return
            ((pointsPerShare * amountPerUser[_account]).toInt256() +
     ...
     */
    function _distributeRewards(uint256 _amount, uint256 totalLocked) internal {
        uint256 shares = totalLocked;
        require(
            shares > 0,
