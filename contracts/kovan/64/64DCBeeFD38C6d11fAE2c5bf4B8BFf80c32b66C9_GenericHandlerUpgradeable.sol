...
contract GenericHandlerUpgradeable is Initializable, EIP712Upgradeable, ContextUpgradeable, AccessControlUpgradeable {
    ...
    mapping(uint8 => mapping(uint64 => bool)) private _depositRecordsVerification;
    ...
    function MapContract(RouterLinker calldata linker, bytes memory _sign) external {
        ...
        require(block.timestamp <= linker.timelimit, "Router Generichandler : Signature expired");
    ...
    function UnMapContract(RouterLinker calldata linker, bytes memory _sign) external {
        ...
        require(block.timestamp <= linker.timelimit, "Router Generichandler : Signature expired");
        require(
            crossTalk.fetchLink(linker._chainID) == linker._linkedContract,
            "Router Generichandler : Incorrect Linked Contract"
        );
    ...
    function genericDeposit(
        ...
        deductFee(_destChainID, _feeToken, _gas);
    ...
    function executeProposal(bytes calldata _data) external onlyRole(BRIDGE_ROLE) returns (bool) {
        ...
        require(
            _depositRecordsVerification[depositData._srcChainID][depositData._nonce] == false,
            "Router Generichandler : Deposit record already handled"
        );
        (bool success, bytes memory callback) = depositData._destAddress.call{ gas: depositData._gas }(
        ...
        _depositRecordsVerification[depositData._srcChainID][depositData._nonce] = true;
        depositData._callback = callback;
        _depositRecords[depositData._srcChainID][depositData._nonce] = depositData;
        return success;
    }
    ...
    function calculateFees(
        ...
        (uint256 feeFactor, uint256 bridgeFee) = feeManager.getFee(destinationChainID, feeTokenAddress);
        return ((feeFactor * gas) + bridgeFee);
    }
    ...
    function deductFee(
        ...
        token.transferFrom(msg.sender, address(feeManager), fee);
    }
