    ...
    function executeOperation(
        ...
        if (s_platformFee != 0) {
            for (uint256 i = 0; i < assets.length; i++) {
                uint256 amountOwingToPlatform = calculatePlatformFee(amounts[i]);
                emit amount(amountOwingToPlatform);
                // IERC20(assets[i]).transfer(s_owner, amountOwingToPlatform);
            }
        }
