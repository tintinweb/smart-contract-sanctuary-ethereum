...
contract PerpetualTranche is ERC20, Initializable, Ownable, IPerpetualTranche {
        ...
        returns (uint256 burnAmt, int256 burnFee)
    {
        ...
        (uint256 trancheOutAmt, uint256 remainder) = perpsToCoveredTranches(
            trancheOut,
            perpAmountRequested,
            type(uint256).max
        );
    ...
    /// @inheritdoc IPerpetualTranche
    // @dev Set `trancheAmtCovered` to max(uint256) to use the current reserve balance.
    function perpsToCoveredTranches(
        ...
        return
            _perpsToCoveredTranches(
                tranche,
                perpAmountRequested,
                Math.min(trancheAmtCovered, tranche.balanceOf(_self())),
