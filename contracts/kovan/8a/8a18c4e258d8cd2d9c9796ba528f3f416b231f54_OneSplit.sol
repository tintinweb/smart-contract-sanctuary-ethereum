...
contract OneSplitRoot {
    ...
    IWETH internal constant wnative = IWETH(0xd0A1E359811322d97991E03f863a0C30C2cF029C);
    ...
    ) public view returns (int256 returnAmount, uint256[] memory distribution) {
...
contract OneSplitView is Initializable, IOneSplitView, OneSplitRoot, UUPSUpgradeable, AccessControlUpgradeable {
        ...
        )
    {
        ...
        (, distribution) = findBestDistribution(parts, matrix);
    ...
    {
        return [
            flags.check(FLAG_DISABLE_UNISWAP_V2_ALL) ? calculateUniswapV2 : _calculateNoReturn,
            flags.check(FLAG_DISABLE_DFYN) ? calculateUniswapV2 : _calculateNoReturn,
            flags.check(FLAG_DISABLE_PANCAKESWAP) ? calculatePancakeSwap : _calculateNoReturn,
            flags.check(FLAG_DISABLE_QUICKSWAP) ? calculateQuickSwap : _calculateNoReturn,
            flags.check(FLAG_DISABLE_SUSHISWAP) ? calculateSushiSwap : _calculateNoReturn
        ];
    }
