...
contract Shiburai is ERC20, Ownable {
    ...
    uint256 public projectFeeForSelling = 3;
    uint256 public liquidityFeeForSelling = 2;
    uint256 public projectFeeForBuying = 3;
    uint256 public liquidityFeeForBuying = 2;
    uint256 public projectFeeForTransfering = 3;
    ...
    uint256 public totalFeesForSelling =
        projectFeeForSelling.add(liquidityFeeForSelling);
    uint256 public totalFeesForBuying =
        projectFeeForBuying.add(liquidityFeeForBuying);
    uint256 public totalFeesForTransfering =
        projectFeeForTransfering.add(liquidityFeeForTransfering);
    ...
    function checkTransactionParameters(
        ...
        if (dailyCoolDown && automatedMarketMakerPairs[to]) {
            require(
                _lastTransfer[from] + 2 minutes <= block.timestamp,
                "One sell per day is allowed"
            );
        }

        if (automatedMarketMakerPairs[to]) {
            if (_lastTransfer[from] + 2 minutes >= block.timestamp) {
        ...
        if (cooldown) {
            if (from != address(this) && to != address(this)) {
                ...
                if (
                    ...
                    require(
                        block.timestamp >= (_lastTX[from] + 30),
                        "Cooldown in effect"
                    );
