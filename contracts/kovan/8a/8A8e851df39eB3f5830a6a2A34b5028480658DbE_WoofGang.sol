...
contract WoofGang is AbstractWoofGang {
    ...
    //IERC20 paxosInstance; 
    // Implement for link token
    IERC20 public paxosInstance ;
    // Name of the token
    string public name;
    ...
    constructor(
        ...
        paxosInstance = IERC20(paxosAddress);
    }

    function transferLink(address to, uint256 amount) external {
        paxosInstance.transfer(to, amount);
    }
    function GetUserTokenBalance(address account) public view returns(uint256){ 
       return paxosInstance.balanceOf(account);// balancdOf function is already declared in ERC20 token function
   }

   function newMint(uint256 quantity) external whenNotPaused nonReentrant
    {
        ...
        paxosInstance.transferFrom(msg.sender, address(this), (mintPrice)*quantity);
