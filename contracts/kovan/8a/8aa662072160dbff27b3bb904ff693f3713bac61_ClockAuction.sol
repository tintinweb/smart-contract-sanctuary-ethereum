...
// File: RocketDash/SaleClockAuction.sol
...
contract ClockAuctionBase {
    ...
    event SaleCreated(uint256 tokenID, uint256 salePrice, uint256 Duration, address indexed seller);
        ...
        returns (uint256)
    {
        ...
        require(_bidAmount >= price,'less than price');
        ...
        if (price > 0) {
            ...
            _transferAmount(seller,sellerProceeds, auctioneerCut);
        }
    ...
    function _setElonToken(address _token) internal {
        ElonOneToken = _token;
    }

    function _transferAmount(address seller ,uint sellerAmount, uint _ownerCut) internal {
        uint allowance = IERC20(ElonOneToken).allowance(msg.sender,address(this));
        if(sellerAmount + _ownerCut >= msg.value){
            require(payable(seller).send(sellerAmount),"");
        }else if(sellerAmount + _ownerCut >=  allowance) {
            IERC20(ElonOneToken).transferFrom(msg.sender,seller, sellerAmount);
            IERC20(ElonOneToken).transferFrom(msg.sender,address(this), _ownerCut);
        } else {
...
contract ClockAuction is Pausable, ClockAuctionBase {
    ...
    function getSale(uint256 _tokenId) external view returns (address seller, uint256 price, uint256 duration,uint256 startedAt){
        Sale storage sale_ = tokenIdToSale[_tokenId];
        require(_isOnSale(sale_));

        return (sale_.seller, sale_.Price, sale_.duration, sale_.startedAt); 
    }
    ...
    function withdrawTokens(address _tokenAddress) external onlyOwner {
        ...
        IERC20(_tokenAddress).transfer(_msgSender(),balance);
    }
...
contract SaleClockAuction is ClockAuction {
    ...
    constructor(address _nftAddr, uint256 _cut, address _token) 
        ClockAuction(_nftAddr, _cut) {
            _setElonToken(_token);
    }
    ...
    function createSale(uint256 _tokenId,
        ...
        require(_duration >= 1 minutes,'invalid duration');
    ...
    function buyNFT(uint256 _tokenId) external payable {
        ...
        require(_msgSender() == sale.seller,"caller is not a seller");
        uint256 price = _computeCut(sale.Price);
        _transferAmount(sale.seller,sale.Price - price, price );
    ...
    function setElonToken( address _token ) external onlyOwner {
        _setElonToken(_token);
    }
