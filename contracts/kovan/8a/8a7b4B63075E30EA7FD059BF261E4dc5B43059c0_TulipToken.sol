...
interface ITulipToken {
    ...
    function setTokenWinner(address _to, uint256 _randomNumber) external returns (uint256);
...
contract TulipToken is Ownable, ERC721, ITulipToken {
    ...
    mapping(uint256 => address) public tokensToBeMinted;
    mapping(uint256 => uint256[]) public idsToTraits;

    event ContollerUpdated(address indexed minter, bool role);
    event BaseURIChanged(string baseURI);

    /// @notice Initializes the contract with base variables.
    /// @param _numberOfTraits: the number of traits a token will have.
    /// @param _upperBound: the upper bound number of the value of a trait.
    /// @param _lowerBound: the lower bound number of the value of a trait.
    constructor(
        string memory _uri,
        ...
        require(
            ...
            "TulipToken/upperBound-lower-than-lowerBound"
        );
    ...
    /// @notice Sets the token winner and the token id as well as generates
    /// the values for the traits based on the given random number.
    /// @param _winner: address of the token winner.
    /// @param _randomNumber: number that was generated for the win.
    function setTokenWinner(address _winner, uint256 _randomNumber)
        external
        override
        onlyController
        returns (uint256)
    {
        ...
        tokensToBeMinted[_id] = _winner;
        _generateTraits(_id, _randomNumber);

        return _id;
    }

    /// @notice This can only be minted by the winner of the token.
    /// @param _id: ID of the token to be minted.
    function mint(uint256 _id) external override {
        require(
            tokensToBeMinted[_id] == msg.sender,
            "TulipToken/user-not-a-winner"
        );
    ...
    /// @notice Returns a list of traits generated for a token. The token must
    /// have been set by the controller. If the token isn't eligible to be minted then
    /// it has no traits.
    /// @return array of numbers which correspond to traits.
    function getTraitsForId(uint256 _id)
        public
        view
        returns (uint256[] memory)
    {
        return idsToTraits[_id];
    }

    /// @return address of the token winner if it was set.
    function tokenWinner(uint256 _id) public view returns (address) {
        return tokensToBeMinted[_id];
    }

    /// https://docs.chain.link/docs/chainlink-vrf-best-practices/#getting-multiple-random-numbers
    /// @notice An empty array of traits is created with the required size. A random number
    /// is then generated inclusively between upper and lower bounds. This random number
    /// is then stored in the array and the array is stored in a mapping of id -> traits.
    /// @param _id: token identifier.
    /// @param _randomNumber: random number that has been derived from CL VRF.
    function _generateTraits(uint256 _id, uint256 _randomNumber) internal {
        uint256[] memory generatedTraits = new uint256[](numberOfTraits);
        ...
        idsToTraits[_id] = generatedTraits;
    }
