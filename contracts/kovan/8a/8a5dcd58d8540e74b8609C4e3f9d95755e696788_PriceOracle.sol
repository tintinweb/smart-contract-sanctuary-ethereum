...
import {IPriceFeedAddress} from "../interfaces/IPriceFeedAddress.sol";
...
uint256 constant DEPENDS_ON_ADDRESS_FLAG = 1 << 160;
...
contract PriceOracle is ACLTrait, IPriceOracleV2, PriceFeedChecker {
    ...
    // token => priceFeed mapping
    // priceFeed keeps address and flag that priceFeed depends on address
    // [0000 . flag . <address>]
    ...
    /// @dev Sets price feed if it doesn't exist, otherwise updates it
    /// @param token Address of token
    /// @param priceFeed Address of chainlink price feed token => USD
    ...
    function _addPriceFeed(address token, address priceFeed) internal {
        ...
        try ERC20(token).decimals() returns (uint8 _decimals) {
            ...
            decimals[token] = 10**_decimals; // F:[PO-3]
        } catch {
        ...
        try IPriceFeedType(priceFeed).dependsOnAddress() returns (
            bool property
        ) {
            dependsOnAddress = property; // F:[PO-2]
        } catch {}
        ...
        if (dependsOnAddress) {
            try
                IPriceFeedAddress(priceFeed).latestRoundData(address(this))
            returns (
                uint80 roundID,
                int256 price,
                uint256,
        ...
        _setPriceFeedWithFlags(token, priceFeed, dependsOnAddress, skipCheck);
        ...
        returns (uint256)
    {
        (
            ...
            bool dependsOnAddress,
            bool skipCheck
        ) = priceFeedsWithFlags(token); //

        console.log("priceFeed");
        console.log(priceFeed);

        if (dependsOnAddress && creditAccount == address(0))
            revert PriceFeedRequiresAddressException(); // F:[PO-4]
        ...
        (
            ...
            ? IPriceFeedAddress(priceFeed).latestRoundData(creditAccount) // F:[PO-6]
            : AggregatorV3Interface(priceFeed).latestRoundData(); // F:[PO-6]

        // Checks if SKIP_PRICE_CHECK_FLAG is not set
        if (!skipCheck)
            _checkAnswer(roundID, price, updatedAt, answeredInRound); // F:[PO-5]

        return uint256(price); // F:[PO-6]
    }
    ...
    function convertToUSD(
        ...
        return (amount * getPrice(creditAccount, token)) / decimals[token]; // F:[PO-7]
    }
    ...
    function convertFromUSD(
        ...
        return (amount * decimals[token]) / getPrice(creditAccount, token); // F:[PO-7]
    }
    ...
    function convert(
        ...
        return
            convertFromUSD(
                creditAccount,
                convertToUSD(creditAccount, amount, tokenFrom),
                tokenTo
            ); // F:[PO-8]
    }

    /// @dev Computes collaterals in USD for non-address depended tokens
    /// @param amountFrom Amount to convert
    /// @param tokenFrom Token address converts from
    /// @param amountTo Amount to convert
    /// @param tokenTo Token address - converts to
    function fastCheck(
        uint256 amountFrom,
        address tokenFrom,
    ...
    {
        collateralFrom = convertToUSD(address(0), amountFrom, tokenFrom); // F:[PO-9]
        collateralTo = convertToUSD(address(0), amountTo, tokenTo); // F:[PO-9]
    }
        ...
        )
    {
        ...
        dependsOnAddress = pf & DEPENDS_ON_ADDRESS_FLAG != 0; // F:[PO-3]
    ...
    function _setPriceFeedWithFlags(
        ...
        if (dependsOnAddress) value |= DEPENDS_ON_ADDRESS_FLAG; // F:[PO-3]
        if (skipCheck) value |= SKIP_PRICE_CHECK_FLAG; // F:[PO-3]

        _priceFeeds[token] = value; // F:[PO-3]
    }
}
