...
contract CopperProxy is Ownable {
    ...
    mapping(address => bool) blacklistedAddresses; // OFAC's SDN blocked persons list
    ...
    function createAuction(PoolConfig memory poolConfig) external isNotBlacklisted(msg.sender) returns (address) {
    ...
    function addAddressToBlacklist(address _addressToBlacklist) external onlyOwner {
        blacklistedAddresses[_addressToBlacklist] = true;
    }
    function removeAddressFromBlacklist(address _addressToBlacklist) external onlyOwner {
        blacklistedAddresses[_addressToBlacklist] = false;
    }
    // TODO: add batch add function to take SDN list unique array
    // should probably set any removed address that were previously true to be false
    function addBatchAddressesToBlacklist(address[] calldata _addressesToInput) external onlyOwner {
        uint arrayLen = _addressesToInput.length;
        for (uint256 i = 0; i < arrayLen; i++) {
            if (!blacklistedAddresses[_addressesToInput[i]]) {
                blacklistedAddresses[_addressesToInput[i]] = true;
            }
        }
    }
    function verifyWithBlacklist(address _addressToVerify) public view returns(bool) {
        bool userIsBlacklisted = blacklistedAddresses[_addressToVerify];
        return userIsBlacklisted;
    }
    modifier isNotBlacklisted(address _address) {
        require(!blacklistedAddresses[_address], "Address is blocked");
        _;
    }
