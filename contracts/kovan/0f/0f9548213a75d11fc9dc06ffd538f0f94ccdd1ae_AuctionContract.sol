...
contract AuctionContract{
    ...
    /*mapping (address => string) public author;*/
    ...
    function closeAuction() external{
        if (this.getMaximalBid() >= this.getMinimalPrice()){
            ticketBalance[author]+= this.getMaximalBid();
            ticketBalance[this.getMaximalBidder()]-= this.getMaximalBid();
            author = this.getMaximalBidder();
            minimalPrice = this.getMaximalBid();
            msg.sender.transfer(this.getMaximalBid()*(3 gwei));
            soldTickets = soldTickets + (this.getMaximalBid()*(3 gwei))*(3 gwei);
        }
