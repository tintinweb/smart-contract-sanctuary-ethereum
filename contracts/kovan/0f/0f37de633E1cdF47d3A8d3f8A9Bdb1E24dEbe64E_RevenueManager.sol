...
contract RevenueManager is PaymentSplitter{
    ...
    * @dev for investors to request funds (for balancing)
    ...
    function requestFunds(address _token, uint _amount) external{
        require(acceptedCollateral[msg.sender], "only verified investors may request funds");
        IERC20 t = IERC20(_token);
        require(balanceAvailable(msg.sender, _token) >= _amount, "There are not enough funds to complete this request");
    ...
    * @dev  returns the balance available for a caller to request
     */
    function balanceAvailable(address _caller, address _token) public view returns(uint){
        if(acceptedCollateral[_caller]){
...
import "../libraries/RoundedDivMath.sol";
...
abstract contract PaymentSplitter is IRevenueManager, Manager{
    using SafeERC20 for IERC20;
    using RoundedDivMath for uint256;
    ...
    function payment(address _tokenContract, uint _amount) public virtual{
        ...
        for(uint i = 0; i < shares.length; i++){
            ...
            if(s.payee != address(this)){
                token.safeTransferFrom(address(this), s.payee, (_amount * s.share).roundedDiv(totalShares));
                //  token.transferFrom(address(this), s.payee, _amount * s.share/totalShares);
            }
        }
    }

    function withdraw(address recipient,uint256 amount) public payable onlyRole(Role.admin) {
