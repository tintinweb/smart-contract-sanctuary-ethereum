...
contract EventConsumer {
    ...
    uint[] public eventIndexes;
    MatchEvent[]public allEventMatches;
    address owner;
    ...
    function setMatchEvent(
        ...
        eventIndexes.push(event_id);
        matchevents[event_id] = MatchEvent(event_id, home_id, home_name, away_id, away_name, home_score, away_score, game_status);
        emit MatchEventAdded(event_id, MatchEvent(event_id, home_id, home_name, away_id, away_name, home_score, away_score, game_status));
        return true;
    }
    ...
    function getAllMatchEvent() public returns (MatchEvent[] memory) {
        for (uint i = 0; i < eventIndexes.length; i++) {
            allEventMatches[i] = matchevents[eventIndexes[i]];
        // ...  
        }
