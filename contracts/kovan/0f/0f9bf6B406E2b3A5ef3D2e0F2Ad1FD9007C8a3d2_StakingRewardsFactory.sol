  ...
  KeeperCompatibleInterface
{
    ...
    updateReward(msg.sender)
  {
    ...
    if (_userLockPeriod[msg.sender] < 1) {
      _userLockPeriod[msg.sender] = block.timestamp + 5 minutes; // for withdraw locking
      _userRewardLockPeriod[msg.sender] = block.timestamp + 5 minutes; // for reward locking
    }
    ...
    updateReward(uaddress)
  {
    ...
    if (earned(uaddress) > 0) {
      ...
      if (_userLockPeriod[uaddress] < 1) {
        _userLockPeriod[uaddress] = block.timestamp + 5 minutes; // for withdraw locking
        _userRewardLockPeriod[uaddress] = block.timestamp + 5 minutes; // for reward locking
      }
  ...
  function getReward() public override nonReentrant updateReward(msg.sender) {
    ...
    if (reward > 0) {
      ...
      _userRewardLockPeriod[msg.sender] = block.timestamp + 5 minutes; // reward lock timestamp
