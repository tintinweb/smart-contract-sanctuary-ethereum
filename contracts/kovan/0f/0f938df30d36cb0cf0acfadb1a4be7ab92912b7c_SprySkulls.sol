...
contract BaseContract is ERC721Enumerable, Ownable {
  ...
  uint256 public whitlistSpots = 500;
...
//     if (msg.sender != owner()) {
//         uint256 costToUse = cost;
//         if(onlyWhitelisted == true) {
//             costToUse = whitelistCost;
//             require(isWhitelisted(msg.sender), "user is not whitelisted");
//             uint256 ownerMintedCount = addressMintedBalance[msg.sender];
//             require(ownerMintedCount + _mintAmount <= nftPerAddressLimit, "max NFT per address exceeded");
//         }
//         require(msg.value >= costToUse * _mintAmount, "insufficient funds");
//     }
...
//   }
    ...
    function mintIMX(address user, uint256 id) public payable {
    require(!paused, "the contract is paused");
    uint256 supply = totalSupply();
    ...
    if (msg.sender != owner()) {
        ...
        if(onlyWhitelisted == true) {
            ...
            uint256 ownerMintedCount = addressMintedBalance[user];
            require(ownerMintedCount + 1 <= nftPerAddressLimit, "max NFT per address exceeded");
        }
        require(msg.value >= costToUse, "insufficient funds");
    }
  ...
  function setWhitelistSpots(uint256 _whitlistSpots) public onlyOwner {
    whitlistSpots = _whitlistSpots;
  }
...
// File: contracts/SprySkull_IMX.sol
...
contract SprySkulls is BaseContract, Mintable {
    ...
    BaseContract(_name, _symbol, _initBaseURI, _initNotRevealedUri)
