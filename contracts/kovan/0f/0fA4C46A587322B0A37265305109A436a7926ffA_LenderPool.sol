...
/** 
 *  SourceUnit: /Users/prateekyammanuru/work/sublime/dev/contracts/sublime-v1/contracts/PooledCreditLine/LenderPool.sol
*/
...
contract LenderPool is ERC1155Upgradeable, ReentrancyGuard {
    ...
    // poolId -> lender -> LenderInfo
    mapping(uint256 => LendingInfo) public creditLines;

    // Since ERC1155 tokens donot support the totalSupply function, we will keep a record of it manually.
    ...
    constructor(address _pooledCreditLine, address _savingsAccount, address _verification) {
    ...
    function create(
        ...
        creditLines[_id].startTime = block.timestamp.add(_collectionPeriod);
        creditLines[_id].token = _token;
        creditLines[_id].borrowLimit = _borrowLimit;
        creditLines[_id].minBorrowAmount = _minBorrowAmount;
        creditLines[_id].verifier = _verifier;
        creditLines[_id].strategy = _strategy;
        creditLines[_id].areTokensTransferable = _areTokensTransferable;

        uint256 allowance = SAVINGS_ACCOUNT.allowance(address(this), _token, address(POOLED_CREDIT_LINE));
        if (allowance != type(uint256).max) {
            SAVINGS_ACCOUNT.approve(_token, address(POOLED_CREDIT_LINE), type(uint256).max);
        }
    }

    function start(uint256 _id) external nonReentrant {
        require(creditLines[_id].borrowLimit != 0, 'S1');
        require(block.timestamp >= creditLines[_id].startTime, 'S2');
        uint256 _totalLent = totalSupply[_id];
        require(_totalLent >= creditLines[_id].minBorrowAmount, 'S3');

        _accept(_id, _totalLent);
    }

    function _accept(uint256 _id, uint256 _amount) internal {
        address _token = creditLines[_id].token;
        address _strategy = creditLines[_id].strategy;
        IERC20(_token).safeApprove(_strategy, _amount);
        creditLines[_id].sharesHeld = SAVINGS_ACCOUNT.deposit(_token, _strategy, address(this), _amount);

        POOLED_CREDIT_LINE.accept(_id, _amount);

        emit PooledCreditLineAccepted(_id, _amount);

        delete creditLines[_id].startTime;
        delete creditLines[_id].borrowLimit;
        delete creditLines[_id].minBorrowAmount;
    }
    ...
    function requestCancelled(uint256 _id) external onlyPooledCreditLine {
        ...
        // After this, we cannot delete *creditLines[_id]*, else we risk getting stuck with some of the lenders'
    ...
    function lend(uint256 _id, uint256 _amount) external nonReentrant {
        require(VERIFICATION.isUser(msg.sender, creditLines[_id].verifier), 'L1');
        require(block.timestamp < creditLines[_id].startTime, 'L2');

        uint256 _totalLent = totalSupply[_id];
        uint256 _maxLent = creditLines[_id].borrowLimit;
        ...
        IERC20(_token).safeTransferFrom(msg.sender, address(this), _amountToLend);
        _mint(msg.sender, _id, _amountToLend, '');

        emit Lend(_id, msg.sender, _amount);

        // If borrowLimit not reached, return
    ...
    function borrowed(uint256 _id, uint256 _sharesBorrowed) external onlyPooledCreditLine nonReentrant {
        creditLines[_id].sharesHeld = creditLines[_id].sharesHeld.sub(_sharesBorrowed);
    }
    ...
    function repaid(
        ...
        creditLines[_id].sharesHeld = creditLines[_id].sharesHeld.add(_sharesRepaid);
        creditLines[_id].borrowerInterestShares = creditLines[_id].borrowerInterestShares.add(_interestShares);
    }

    function calculatePrincipleWithdrawable(uint256 _id, address _lender) public view returns (uint256) {
        uint256 _creditLineStatus = uint256(POOLED_CREDIT_LINE.getCreditLineStatus(_id));
        if (_creditLineStatus == uint256(CreditLineStatus.CLOSED) || _creditLineStatus == uint256(CreditLineStatus.LIQUIDATED)) {
            return _calculatePrincipleWithdrawable(_id, _lender);
        } else if (_creditLineStatus == uint256(CreditLineStatus.REQUESTED) && block.timestamp > creditLines[_id].startTime) {
    ...
    function _calculatePrincipleWithdrawable(uint256 _id, address _lender) internal view returns (uint256) {
        uint256 _currentSupply = totalSupply[_id];
        uint256 _totalLiquidityWithdrawable = creditLines[_id].borrowLimit.sub(POOLED_CREDIT_LINE.getPrincipal(_id));
        uint256 _principalWithdrawable = _totalLiquidityWithdrawable.mul(balanceOf(_lender, _id)).div(_currentSupply);
        return _principalWithdrawable;
    }

    // Liquidity can be withdrawn when the pooled credit line is no longer running. Various such scenarios are
    ...
    function _withdrawLiquidity(uint256 _id, bool _isLiquidationWithdrawn) internal {
        uint256 _creditLineStatus = POOLED_CREDIT_LINE.getCreditLineStatus(_id);

        uint256 _liquidityProvided = balanceOf(msg.sender, _id);
        _burn(msg.sender, _id, _liquidityProvided);

        if (
            (_creditLineStatus == uint256(CreditLineStatus.CANCELLED)) ||
            (_creditLineStatus == uint256(CreditLineStatus.REQUESTED) && block.timestamp > creditLines[_id].startTime)
        ) {
            // Case 0:
            ...
            address lentToken = creditLines[_id].token;
            IERC20(lentToken).safeTransfer(msg.sender, _liquidityProvided);
            emit WithdrawLiquidityOnCancel(_id, msg.sender, _liquidityProvided);
        } else if (_creditLineStatus == uint256(CreditLineStatus.CLOSED) || _creditLineStatus == uint256(CreditLineStatus.LIQUIDATED)) {
            if (_creditLineStatus == uint256(CreditLineStatus.LIQUIDATED)) {
                require(_isLiquidationWithdrawn, 'IWL1');
            }
            // all other cases distribute the sharesHeld proportional to their poolToken balances
            address _strategy = creditLines[_id].strategy;
            address _lentToken = creditLines[_id].token;
            uint256 _principalWithdrawable = _calculatePrincipleWithdrawable(_id, msg.sender);
            (uint256 _interestWithdrawable, ) = _calculateInterestToWithdraw(_id, msg.sender, _strategy, _lentToken);
            creditLines[_id].sharesHeld = creditLines[_id].sharesHeld.sub(_principalWithdrawable).sub(_interestWithdrawable);
            uint256 _sharesToWithdraw = _principalWithdrawable.add(_interestWithdrawable);

            uint256 _amountToWithdraw = IYield(_strategy).getTokensForShares(_sharesToWithdraw, _lentToken);
            SAVINGS_ACCOUNT.withdrawShares(_lentToken, _strategy, msg.sender, _amountToWithdraw, false);
    ...
    // 1. Tokens lent are locked till end of Pooled Credit line
    //    Any interest paid by borrower can be withdrawn by lenders proportional to their token balances for that pooled credit line
    // 2. Also, we are not allowing the users to withdraw their interest partially.
    //    Whenever they call the `withdrawInterest` function they will get the entire amount of interest that is owed to them by that time
    ...
    function _withdrawInterest(uint256 _id, address _lender) internal {
        ...
        (uint256 _interestToWithdraw, uint256 _interestSharesToWithdraw) = _calculateInterestToWithdraw(_id, _lender, _strategy, _lentToken);

        if (_interestToWithdraw != 0) {
            SAVINGS_ACCOUNT.withdraw(_lentToken, _strategy, _lender, _interestToWithdraw, false);
        }
    ...
    function _calculateInterestToWithdraw(
        ...
        {
            uint256 _notBorrowed = creditLines[_id].borrowLimit.sub(POOLED_CREDIT_LINE.getPrincipal(_id));
            uint256 _notBorrowedInShares = IYield(_strategy).getSharesForTokens(_notBorrowed, _lentToken);
            _totalInterestInShares = creditLines[_id].sharesHeld.sub(_notBorrowedInShares);
        }

        uint256 _borrowerInterestShares = creditLines[_id].borrowerInterestShares;
        ...
        uint256 _borrowerInterestForLender = (_borrowerInterestShares.mul(_lenderBalance).div(_totalSupply)).sub(
            creditLines[_id].lenders[_lender].borrowerInterestSharesWithdrawn
        );

        uint256 _yieldInterestForLender;
        {
            uint256 _totalYieldInterest = _totalInterestInShares.sub(_borrowerInterestShares).add(
                creditLines[_id].yieldInterestWithdrawnShares
            );

            _yieldInterestForLender = (_totalYieldInterest.mul(_lenderBalance).div(_totalSupply)).sub(
                creditLines[_id].lenders[_lender].yieldInterestWithdrawnShares
            );
        }

        creditLines[_id].lenders[_lender].yieldInterestWithdrawnShares = creditLines[_id].lenders[_lender].yieldInterestWithdrawnShares.add(
            _yieldInterestForLender
        );
        creditLines[_id].lenders[_lender].borrowerInterestSharesWithdrawn = creditLines[_id]
            .lenders[_lender]
            .borrowerInterestSharesWithdrawn
            .add(_borrowerInterestForLender);
        creditLines[_id].borrowerInterestSharesWithdrawn = creditLines[_id].borrowerInterestSharesWithdrawn.add(_borrowerInterestForLender);
        creditLines[_id].yieldInterestWithdrawnShares = creditLines[_id].yieldInterestWithdrawnShares.add(_yieldInterestForLender);
        creditLines[_id].sharesHeld = creditLines[_id].sharesHeld.sub(_yieldInterestForLender.add(_borrowerInterestForLender));
    ...
    function _withdrawLiquidation(
        ...
        uint256 _collateralLiquidated = creditLines[_id].collateralHeld;
        uint256 _currentSupply = totalSupply[_id];

        uint256 _lenderCollateralShare = _balance.mul(_collateralLiquidated).div(_currentSupply);

        if (_lenderCollateralShare != 0) {
            creditLines[_id].collateralHeld = creditLines[_id].collateralHeld.sub(_lenderCollateralShare);

            IERC20(creditLines[_id].collateralToken).safeTransfer(_lender, _lenderCollateralShare);
        }
    ...
    function liquidate(uint256 _id, bool _withdraw) external nonReentrant {
        ...
        (address _collateralToken, uint256 _collateralLiquidated) = POOLED_CREDIT_LINE.liquidate(_id);
        creditLines[_id].collateralToken = _collateralToken;
        creditLines[_id].collateralHeld = _collateralLiquidated;
    ...
    function _rebalanceInterestWithdrawn(
        ...
        uint256 yieldInterestOnTransferAmount = creditLines[id].lenders[from].yieldInterestWithdrawnShares.mul(amount).div(fromBalance);
        uint256 borrowerInterestOnTransferAmount = creditLines[id].lenders[from].borrowerInterestSharesWithdrawn.mul(amount).div(
            fromBalance
        );

        if (borrowerInterestOnTransferAmount != 0) {
            creditLines[id].lenders[from].borrowerInterestSharesWithdrawn = creditLines[id]
                .lenders[from]
                .borrowerInterestSharesWithdrawn
        ...
        if (yieldInterestOnTransferAmount != 0) {
            creditLines[id].lenders[from].yieldInterestWithdrawnShares = creditLines[id].lenders[from].yieldInterestWithdrawnShares.sub(
                yieldInterestOnTransferAmount
            );
        }

        if (to != address(0)) {
            if (borrowerInterestOnTransferAmount != 0) {
                creditLines[id].lenders[to].borrowerInterestSharesWithdrawn = creditLines[id]
                    .lenders[to]
                    .borrowerInterestSharesWithdrawn
                    .add(borrowerInterestOnTransferAmount);
            }
            if (yieldInterestOnTransferAmount != 0) {
                creditLines[id].lenders[to].yieldInterestWithdrawnShares = creditLines[id].lenders[to].yieldInterestWithdrawnShares.add(
                    yieldInterestOnTransferAmount
                );
            }
        }
    ...
    function _beforeTokenTransfer(
        ...
        for (uint256 i; i < ids.length; ++i) {
            uint256 id = ids[i];
            require(
                VERIFICATION.isUser(msg.sender, creditLines[id].verifier)
                || to == address(0), 
                'T2'
            );
            ...
            if (from == address(0)) {
                ...
                require(creditLines[id].areTokensTransferable, 'T4');
            }
