...
contract OneSplitView is Initializable, IOneSplitView, OneSplitRoot, UUPSUpgradeable, AccessControlUpgradeable {
    ...
    {
        return [
            flags.check(FLAG_DISABLE_UNISWAP_V2_ALL) ? _calculateNoReturn : calculateUniswapV2,
            flags.check(FLAG_DISABLE_DFYN) ? _calculateNoReturn : calculateUniswapV2,
            flags.check(FLAG_DISABLE_PANCAKESWAP) ? _calculateNoReturn : calculatePancakeSwap,
            flags.check(FLAG_DISABLE_QUICKSWAP) ? _calculateNoReturn : calculateQuickSwap,
            flags.check(FLAG_DISABLE_SUSHISWAP) ? _calculateNoReturn : calculateSushiSwap
        ];
    }
