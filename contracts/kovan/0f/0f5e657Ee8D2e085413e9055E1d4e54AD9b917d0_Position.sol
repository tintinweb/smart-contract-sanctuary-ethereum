...
contract Position is ERC721Upgradeable, IPosition {
  ...
  function updateRate(uint128 tokenId, uint128 newRate) external override {
    ...
    (uint128 newAmount, uint128 newBondsIssuanceIndex) = pools.updateRate(
      _positions[tokenId].adjustedBalance,
      _positions[tokenId].poolHash,
      oldRate,
      newRate,
    ...
    emit UpdateRate(_msgSender(), tokenId, newAmount, newRate, _positions[tokenId].poolHash);
  }
...
interface IBorrowerPools {
   ...
   * @param platformFee The amount of fee paid to atlendis
   ...
   **/
  event TickPendingDeposit(bytes32 poolHash, uint128 rate, uint128 adjustedPendingAmount);
   ...
   * @param poolHash The identifier of the pool
   * @param rate The tick's bidding rate
   * @param liquidityRewards the amount of liquidityRewards added to the tick
   * @param addedAccruedFees Increase in accrued fees for that tick
   **/
  event CollectFeesForTick(bytes32 poolHash, uint128 rate, uint128 liquidityRewards, uint128 addedAccruedFees);
   ...
   * @return totalBorrowed Amount borrowed
   * @return normalizedAvailableDeposits Amount of tokens deposited before bond
   * issuance, available for the borrower and currently on third party yield provider
  ...
  function getPoolState(bytes32 poolHash)
    ...
    view
    returns (
      uint128 weightedAverageLendingRate,
      uint128 totalBorrowed,
      uint128 normalizedAvailableDeposits,
      uint128 adjustedPendingDeposits
    );
   ...
   * amount, late repay fees and platform fees
   * @param poolHash The identifier of the pool
   ...
   * @return platformFees Normalized fee amount paid to the protocol
   **/
  ...
  ) external returns (uint128 newAdjustedAmount, uint128 newBondsIssuanceIndex);
...
interface IPosition {
   ...
   * @param lender The address of the lender depositing token on the platform
   * @param tokenId The tokenId of the position
...
library Errors {
  ...
  string public constant PSM_NOT_ENOUGH_PLATFORM_FEES = "25"; // "Not enough registered platform fees to withdraw"
...
library Types {
  ...
  struct PoolState {
    ...
    uint128 nextLoanMinStart;
    uint128 remainingLiquidityRewardsReserve;
