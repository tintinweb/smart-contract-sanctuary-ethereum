...
contract ToucanCrosschainMessenger is ToucanCrosschainMessengerStorage, Router {
    ...
    event BridgeRequestSent(uint32 remoteDomain, bytes message);
    ...
    function handle(
        ...
        (address bridger, address token, uint256 amount, uint32 toDomain) = abi
            .decode(_message, (address, address, uint256, uint32));
        emit BridgeRequestReceived(_origin, toDomain, bridger, token, amount);
    }
    ...
    function sendMessage(
        ...
        emit BridgeRequestSent(_destinationDomain, _outboundMessage);
    }
}
