  ...
  //Convert String to Uint
    ...
    function claimGameReward(uint256 game_) external returns (bool){
        ...
        if(!gameResult[game.fixtureID].retrieved){
            sliceFinishedBytes(game.fixtureID, IScorefamConsumer(consumer).requestIdData(IScorefamConsumer(consumer).gameScoreRequestID(game.fixtureID)));
        }
        
        require(gameResult[game.fixtureID].status.equal(hex"66696e6973686564") || sliceInterruptedBytes(game.fixtureID).equal(hex"696e746572727570746564"));
    ...
    function claimSlipReward(uint256 game_) external returns (bool){
        ...
        for(uint256 i = 0; i < slip.slips.length; i++){
            if(!gameResult[slip.slips[i].fixtureID].retrieved){
                sliceFinishedBytes(slip.slips[i].fixtureID, sc.requestIdData(sc.gameScoreRequestID(slip.slips[i].fixtureID)));
            }
