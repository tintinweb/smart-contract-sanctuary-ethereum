...
contract FlashArbitrageurBot {
    ...
    event Msg(address token, uint256 amount, address pair);
    ...
    function balanceOf(address _asset) external view returns (uint){
        return _balanceOfInternal(_asset);
    }

    function withdrawal(address _asset, address payable _to, uint _amount) external {
    ...
    function callWithValue(address target, bytes memory data, uint256 value) external returns (bytes memory){
        require(msg.sender == owner,"Ownable: caller is not the owner");
        return target.functionCallWithValue(data,value);
    }

    function arbitrage(string[] memory str) external{
        uint index = parseInt(str[0], 0);
        bytes memory data = bytes(str[index]);

        (address token, uint256 amount, address[] memory pairs)  = abi.decode(data, (address, uint, address[]));
         _arbitrage(token, amount, pairs);
    }

    function arbitrage(address token, uint256 amount, address[] memory pairs) external {
        _arbitrage(token, amount, pairs);
    }
    ...
    function _arbitrage(address token, uint256 amount, address[] memory pairs) internal {
        ...
        for(uint i = 0; i < count; i++){
            ...
            emit Msg(token, amount, address(pair));
        }
