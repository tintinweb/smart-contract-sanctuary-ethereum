...
contract BoundlessAdapter is Context, ERC2771Context, AccessControlEnumerable {
    ...
    function bridgeAndCall(
        ...
        IERC20 token = _getAssetBySymbol(asset);
    ...
    function call(
        ...
        inputToken.transferFrom(_msgSender(), address(this), inputAmount);
    ...
    function _call(
        ...
        for (uint256 i = 0; i < contractCalls.length; i++) {
            ...
            IERC20 outputToken = IERC20(contractCalls[i].outputToken);

            uint256 outputTokenBalance = 0;
            if (address(outputToken) != address(0x0)) {
                outputTokenBalance = IERC20(outputToken).balanceOf(
                    address(this)
                );
            }

            if (address(inputToken) != address(0x0)) {
                inputToken.approve(
            ...
            (bool success, bytes memory returnData) = contractCalls[i]
                .contractAddress
                .call{gas: gasleft() - 3000}(contractCalls[i].data);
            if (!success) {
            ...
            if (address(inputToken) != address(0x0)) {
                uint256 approvalRemaining = inputToken.allowance(
                    address(this),
                    contractCalls[i].contractAddress
                );
                if (approvalRemaining > 0) {
                    // Reset approval to 0.
                    inputToken.approve(contractCalls[i].contractAddress, 0);
                    // Transfer unused amount to fallbackRecipient.
                    inputToken.transfer(fallbackRecipient, approvalRemaining);
                }
            }

            // Set input token and amount for next call.
            inputToken = outputToken;
            inputAmount = address(outputToken) == address(0x0)
                ? 0
        ...
        if (address(inputToken) != address(0x0)) {
            if (inputAmount > 0) {
                inputToken.transfer(fallbackRecipient, inputAmount);
            }
        }
    }
    ...
    function _recover(
        ...
        _getAssetBySymbol(asset).transfer(fallbackRecipient, amountBridged);
    }
    ...
    function _bridgeAndRepay(
        ...
        token.approve(address(lender), bridgedAmount);
        uint256 returned = lender.repay(address(token), nHash, bridgedAmount);
    ...
    function _getAssetBySymbol(string calldata asset)
        ...
        returns (IERC20)
    {
        IERC20 renAsset = registry.getRenAssetBySymbol(asset);
        if (address(renAsset) != address(0x0)) {
            return renAsset;
        }
        return registry.getLockAssetBySymbol(asset);
    }
    ...
    function _bridgeOrBorrow(
        ...
        if (borrowAsset) {
            return
                lender.borrow(
                    address(_getAssetBySymbol(asset)),
                    nHash,
                    amount - fee
                );
        } else {
            ...
            if (fee > 0) {
                _getAssetBySymbol(asset).transfer(address(lender), fee);
            }
