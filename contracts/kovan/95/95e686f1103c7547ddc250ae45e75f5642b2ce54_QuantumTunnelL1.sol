...
contract QuantumTunnelL1 is Ownable {
    ...
    event Created(address, address, uint256, uint256);
    event Destroyed(address, address, uint256);
    ...
    function createTokenOnL2(
        ...
        emit Created(msg.sender, address(token), tokenId, lockExpiresAt);
    }
    ...
    function removeStake(ERC721 token, uint256 tokenId) external {
        ...
        bytes memory callData = abi.encodeWithSelector(
            QuantumTunnelL2(receiverOnL2[tokenInDomain]).burnToken.selector,
            address(token),
            tokenId
        );
        ...
        emit Destroyed(msg.sender, address(token), tokenId);
    }
    ...
    function renewStake(
        ...
        // call L2
        bytes memory callData = abi.encodeWithSelector(
            QuantumTunnelL2(receiverOnL2[tokenInDomain]).updateExpiry.selector,
            address(token),
            tokenId,
            lockExpiresAt
        );
        ...
        emit Renewed(msg.sender, address(token), tokenId, lockExpiresAt);
    }
...
contract QuantumTunnelL2 is Ownable {
    ...
    constructor(uint32 _originDomain, address payable _connext) Ownable() {
        originDomain = _originDomain;
    ...
    // Permissioned function
    function burnToken(address tokenAddress, uint256 tokenId)
        external
        onlyExecutor
    {
