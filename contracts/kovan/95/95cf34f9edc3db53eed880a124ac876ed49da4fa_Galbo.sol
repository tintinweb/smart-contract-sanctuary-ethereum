...
contract Galbo is ERC721A,ReentrancyGuard,Ownable,ERC721AOwnersExplicit {
    ...
    GalboConfig public galboConfig;
    ...
    constructor(
        ...
        bytes32 _whitelistRoot) ERC721A("Galbo", "GALBO", maxBatchSize_, collectionSize_) {
    ...
    function auctionMint(uint256 quantity) external payable callerIsUser {
        uint256 _saleStartTime = uint256(galboConfig.auctionSaleStartTime);
        require(
    ...
    function whitelistMint(bytes32[] memory proof) external payable callerIsUser {
        uint256 price = uint256(galboConfig.whitelistPrice);
        require(price != 0, "Whitelist sale hasn't begun yet");
        require(isWhitelisted(msg.sender,proof), "User not is not on whitelist");
    ...
    function publicMint(uint256 quantity, uint256 callerPubMintKey) external payable callerIsUser {
        GalboConfig memory config = galboConfig;
        uint pubSaleKey = uint256(config.publicSaleKey);
        uint256 pubPrice = uint256(config.publicPrice);
        uint256 pubSaleStartTime = uint256(config.publicSaleStartTime);
        require(pubSaleKey == callerPubMintKey, "Incorrect public mint key");
        require(isPublicSaleOn(pubPrice,pubSaleKey,pubSaleStartTime), "Sale hasn't started yet");
        require(totalSupply() + quantity <= collectionSize, "Max Supply Reached");
        require(addressMintedBalance[msg.sender] + quantity <= maxPerAddressDuringMint, "Too Many to mint");
        _safeMint(msg.sender, quantity);
    }
    ...
    function endAuctionStartPublicSale(
        uint64 mintlistPriceWei,
        uint64 publicPriceWei,
        uint32 publicSaleStartTime
    ) external onlyOwner {
        galboConfig = GalboConfig(
            0, 
            publicSaleStartTime,
            mintlistPriceWei,
            publicPriceWei,
            galboConfig.publicSaleKey
        );
    }

    function setAuctionSaleStartTime(uint32 timestamp) external onlyOwner {
        galboConfig.auctionSaleStartTime = timestamp;
    }
    ...
    function refundIfOver(uint256 price) private {
        require(msg.value >= price, "Need to send more Ether.");
        if (msg.value > price) {
