...
contract CivFarm is Ownable, ReentrancyGuard {
    ...
    struct PoolInfo {
        ...
        IERC20 representToken; // Represent Token contract, for us xSomething
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) public nonReentrant {
        ...
        if (user.amount > 0) {
            uint256 pending = (user.amount * (pool.accTokensPerShare)) /
                ...
                (user.rewardDebt);
            if (pending > 0) {
                stone.safeTransfer(msg.sender, pending);
        ...
        if (_amount > 0) {
            ...
            pool.representToken.safeTransfer(msg.sender, _amount);
        }
    ...
    function enterStaking(uint256 _amount) public nonReentrant {
        ...
        if (user.amount > 0) {
            ...
            if (pending > 0) {
                stone.safeTransfer(msg.sender, pending);
                emit YieldWithdraw(msg.sender, 0, pending);
            }
        }
        if (_amount > 0) {
    ...
    function leaveStaking(uint256 _amount) public nonReentrant {
        ...
        require(
            lpBalance >= _amount,
            "Smart contract hasn't got enough LP token"
        );
        require(
            xCIVBalanceOfUser >= _amount,
            "You should have enough proof xtokens for unstaking"
        );
        require(allowanceAmount >= _amount, "You should approve first");
    ...
    function withdrawYield(uint256 _pid) public {
        ...
        stone.safeTransfer(msg.sender, toTransfer);
