...
contract OlympusERC20Token is ERC20Permit, VaultOwned {
    ...
    function _transfer(address sender, address recipient, uint256 amount) override internal virtual {
        ...
        // uint256 amountToSend = amount.sub(msigTax).sub(burnTax);
    ...
    function setBurnTaxFee(uint256 _burnTaxFee) public onlyOwner() {
        burnTaxFee = _burnTaxFee;
        emit BurnTaxFee(_burnTaxFee);
    }

    function setMsigTaxFee(uint256 _msigTaxFee) public onlyOwner() {
        msigTaxFee = _msigTaxFee;
        emit MsigTaxFee(_msigTaxFee);
    }

    function activateTaxes(bool _disable) public onlyOwner() {
        taxesDisabled = _disable;
        emit TaxesDisabled(_disable);
    }
}