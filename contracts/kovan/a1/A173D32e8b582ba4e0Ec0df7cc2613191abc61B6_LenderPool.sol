...
contract LenderPool is ERC1155Upgradeable, ReentrancyGuard, OwnableUpgradeable {
    ...
    /**
     * @notice Struct that stores the info of a specific credit line
     * @param lenders mapping that stores lender specific info for the credit line
     * @param startTime Timestamp at which credit line starts
     ...
     * @param strategy address of strategy to deposit lent tokens to
     ...
     * @param areTokensTransferable boolean that represents if pool tokens for credit line are transferable
     */
    struct LendingInfo {
    ...
    /**
     * @notice Mapping that stores data related to creditLine against the creditLineId
     */
    ...
    /**
     * @notice function to get start time of the pooled credit line
     */
    function getCreditLineStartTime(uint256 creditLineId) public view returns (uint256) {
        return creditLines[creditLineId].startTime;
    }
     ...
     * @param _strategy address of strategy to deposit lent tokens to
    ...
    function create(
        ...
        creditLines[_id].borrowAsset = _borrowAsset;
    ...
    function start(uint256 _id, address _to) external nonReentrant {
        ...
        address _borrowAsset = creditLines[_id].borrowAsset;
    ...
    function _accept(uint256 _id, uint256 _amount) internal {
        ...
        creditLines[_id].sharesHeld = SAVINGS_ACCOUNT.deposit(_borrowAsset, _strategy, address(this), _amount);
    ...
    function _withdrawLiquidity(uint256 _id, bool _isLiquidationWithdrawn) internal {
        ...
        if (
            ...
            uint256 _amountToWithdraw = IYield(_strategy).getTokensForShares(_sharesToWithdraw, _borrowAsset);
            SAVINGS_ACCOUNT.withdrawShares(_borrowAsset, _strategy, msg.sender, _amountToWithdraw, false);
    ...
    function _withdrawLiquidation(
        ...
        if (_lenderCollateralShare != 0) {
            ...
            IERC20(creditLines[_id].collateralAsset).safeTransfer(_lender, _lenderCollateralShare);
        }
    ...
    function liquidate(uint256 _id, bool _withdraw) external nonReentrant {
        ...
        creditLines[_id].collateralAsset = _collateralAsset;
    ...
    function _beforeTokenTransfer(
        ...
        for (uint256 i; i < ids.length; ++i) {
            uint256 id = ids[i];
            require(VERIFICATION.isUser(msg.sender, creditLines[id].verifier) || to == address(0), 'T2');
