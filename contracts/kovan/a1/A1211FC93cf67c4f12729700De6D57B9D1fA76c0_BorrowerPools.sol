...
contract BorrowerPools is PoolsController, IBorrowerPools {
  ...
  function initialize(address governance) public initializer {
    ...
    _setRoleAdmin(POSITION_ROLE, GOVERNANCE_ROLE);
  }
  ...
  function withdraw(
    ...
    uint128 bondsIssuanceIndex,
    address owner
  ) public override whenNotPaused onlyRole(POSITION_ROLE) returns (uint128 normalizedDepositedAmountToWithdraw) {
    Types.Pool storage pool = pools[poolHash];
   ...
   **/
  function borrow(address to, uint128 loanAmount) external override whenNotPaused onlyRole(BORROWER_ROLE) {
    ...
    console.log(normalizedBorrowedAmount.scaleFromWad(pool.parameters.TOKEN_DECIMALS));
    pool.parameters.YIELD_PROVIDER.withdraw(
      pool.parameters.UNDERLYING_TOKEN,
   ...
   **/
  function topUpLiquidityRewards(uint128 amount) external override whenNotPaused onlyRole(BORROWER_ROLE) {
...
abstract contract PoolsController is AccessControlUpgradeable, PausableUpgradeable, IPoolsController {
   ...
   **/
  function setRepaymentFeeRate(uint128 repaymentFeeRate, bytes32 poolHash) external override onlyRole(GOVERNANCE_ROLE) {
    if (!pools[poolHash].state.active) {
      revert Errors.PC_POOL_NOT_ACTIVE();
    }
