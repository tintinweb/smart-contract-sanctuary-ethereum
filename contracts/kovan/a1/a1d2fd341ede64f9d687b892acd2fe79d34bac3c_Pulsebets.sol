...
library CBORChainlink {
    ...
    function encodeBigNum(BufferChainlink.buffer memory buf, uint256 value)
        internal
        pure
    {
...
// import "./ChainlinkRequestInterface.sol"; done already
...
abstract contract ChainlinkClient {
     ...
     * @return requestId The request ID
     */
    function sendOperatorRequest(Chainlink.Request memory req, uint256 payment)
        internal
        returns (bytes32)
    {
...
contract Pulsebets is ChainlinkClient {
    using Chainlink for Chainlink.Request;
    using CBORChainlink for BufferChainlink.buffer;
    ...
    function getGameCreateMlb(bytes32 _requestId, uint256 _idx)
        external
        view
        returns (GameCreateMlb memory)
    {
        return _getGameCreateMlbStruct(requestIdGames[_requestId][_idx]);
    }

    function getGameResolveMlb(bytes32 _requestId, uint256 _idx)
        external
        view
        returns (GameResolveMlb memory)
    {
    ...
    function _getGameCreateMlbStruct(bytes32 _data)
        private
        pure
        returns (GameCreateMlb memory)
    {
    ...
    function _getGameResolveMlbStruct(bytes32 _data)
        private
        pure
        returns (GameResolveMlb memory)
    {
