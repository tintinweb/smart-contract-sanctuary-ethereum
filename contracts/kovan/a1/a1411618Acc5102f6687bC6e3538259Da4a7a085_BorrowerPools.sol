    ...
    returns (uint128 weightedAverageLendingRate, uint128 adjustedPendingDeposits)
  {
    ...
    for (rate; rate != parameters.MAX_RATE + parameters.RATE_SPACING; rate += parameters.RATE_SPACING) {
      if (pool.state.currentMaturity == 0) {
        ...
        totalAmount += pool.ticks[rate].adjustedRemainingAmount.wadRayMul(atlendisLiquidityRatio);
        if (totalAmount >= pool.parameters.MAX_BORROWABLE_AMOUNT) {
          uint128 delta = totalAmount - pool.parameters.MAX_BORROWABLE_AMOUNT;
          amountWeightedRate += (pool.ticks[rate].adjustedRemainingAmount.wadRayMul(atlendisLiquidityRatio) - delta)
            .wadMul(rate);
          totalAmount -= delta;
          break;
        }
        amountWeightedRate += pool.ticks[rate].adjustedRemainingAmount.wadRayMul(atlendisLiquidityRatio).wadMul(rate);
      } else {
...
import "./interfaces/IProtocolManagement.sol";
 ...
 * @notice Overall management of the protocol
 **/
