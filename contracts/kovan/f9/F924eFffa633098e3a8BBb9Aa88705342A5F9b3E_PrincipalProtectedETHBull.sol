...
contract BaseVault is IVault {
     ...
     */
    function previewShares(uint256 assets)
        public
        view
        virtual
        returns (uint256)
     ...
     */
    function processQueuedDeposits(uint256 startIndex, uint256 endIndex)
        public
    {
        ...
        for (uint256 i = startIndex; i < endIndex; i++) {
            DepositQueueLib.DepositEntry memory depositEntry = depositQueue.get(
                i
            );
            _mintShares(
                depositEntry.owner,
    ...
    {
        if (shares > userShares[owner])
            revert IVault__CallerHasNotEnoughShares();
        claimableUnderlying = userShares[owner].mulDivDown(
            totalAssets(),
            totalShares
        );
        userShares[owner] -= shares;
