    ...
    ERC165Upgradeable
{
    ...
    IERC1155Upgradeable public rewardToken;
    ...
    function initialize(
        ...
        IERC1155Upgradeable _rewardToken,
    ...
    function stopExchange() external onlyOwner {
        delete rewardQuantities;
        delete rewardTotalQuantities;
        delete timestampStartClaims;
        delete claimDurations;
        delete rewardTokenIds;
    }

    function exchange(uint256 _vialAmount, uint256 _rewardTokenId) external {
        bool found = false;
        uint256 index;
        for (uint256 i = 0; i < rewardTokenIds.length; i++) {
            if (rewardTokenIds[i] == _rewardTokenId) {
                found = true;
                index = i;
                break;
            }
        ...
        require(found, "invalid reward token id");

        require(timestampStartClaims[index] != 0, "not start yet");
        require(
            timestampStartClaims[index] <= block.timestamp &&
                block.timestamp <=
                timestampStartClaims[index] + claimDurations[index],
            "not start yet"
        );

        require(rewardQuantities[index] > 0, "rewards drained");
        rewardQuantities[index]--;
        ...
        returns (uint256[] memory tokenIds, uint256[] memory balances)
    {
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            balances[i] = rewardToken.balanceOf(address(this), tokenIds[i]);
        }
    }
