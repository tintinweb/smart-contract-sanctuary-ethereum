...
contract BundleProvider is Ownable, ReentrancyGuard
{
    /***************************/
    /********** STRUCT *********/
    /***************************/
     ...
     */
    event BundlesStarted(uint StartingBundleIndex, uint EndingBundleIndex);

    /**
     * @dev Emitted When Various State Variables Are Modified
     */
    event BundleEnded(uint BundleIndex);
    event BundleChangedName(uint indexed BundleIndex, string OldName, string NewName);
    ...
    event BundleChangedAllocationPublic(uint indexed BundleIndex, uint OldAllocation, uint NewAllocation);
    event BundleChangedAllocationBrightList(uint indexed BundleIndex, uint OldAllocation, uint NewAllocation);
    event BundleChangedPriceBrightList(uint indexed BundleIndex, uint OldPrice, uint NewPrice);
    event BundleChangedPricePublic(uint indexed BundleIndex, uint OldPrice, uint NewPrice); 
    ...
    event BundleChangedRoot(uint indexed BundleIndex, bytes32 OldRoot, bytes32 NewRoot);
    event BundleChangedOperator(uint indexed BundleIndex, address OldOperator, address NewOperator);
    ...
    { 
        ...
        require(Bundles[BundleIndex].PurchaseableAmount > 0, "Bundle Sold Out");
        require(
            PurchasedAmountPublic[BundleIndex][msg.sender] + Amount <= Bundles[BundleIndex].PurchaseableAmountPublic,
            "BundleProvider: User Has Used Up All Of Public Allocation For This Bundle Index"
        );
        ...
        Bundles[BundleIndex].PurchaseableAmount -= Amount;
        for(uint x; x < Amount; x++)
        {
            for(uint y; y < Bundles[BundleIndex].ContractAddresses.length; y++)
            {
                IERC721(Bundles[BundleIndex].ContractAddresses[x]).transferFrom(
                    Bundles[BundleIndex].Operator, 
                    msg.sender, 
                    Bundles[BundleIndex].StartingIndexes[x]
                );
                Bundles[BundleIndex].StartingIndexes[x]++;
            }
        }
    ...
    function BundlePurchaseBrightList(uint BundleIndex, uint Amount, bytes32[] calldata Proof) public payable nonReentrant
    {
        ...
        require(Bundles[BundleIndex].PurchaseableAmount > 0, "Sold Out");
        ...
        require(
            ...
            "User Has Used Up All BrightList Allocation For This Bundle Index"
        );
        if(!Bundles[BundleIndex].AllowMultiplePurchases) { require(!BundlePurchased[BundleIndex][msg.sender], "User Has Already Purchased This Bundle Index"); }
        if(!BundlePurchased[BundleIndex][msg.sender]) { BundlePurchased[BundleIndex][msg.sender] = true; }
        ...
        {
            for(uint j; j < Bundles[BundleIndex].ContractAddresses.length; j++)
            {
                IERC721(Bundles[BundleIndex].ContractAddresses[i]).transferFrom(
                    Bundles[BundleIndex].Operator, 
                    msg.sender, 
                    Bundles[BundleIndex].StartingIndexes[i]
                );
                Bundles[BundleIndex].StartingIndexes[i]++;
            }
        }
     ...
     */
    function _StartBundles(Bundle[] memory NewBundleInstance) external onlyBRTOperator   
    {
        uint StartingBundleIndex = UniqueBundleIndex;
        for(uint i; i < NewBundleInstance.length; i++)
        {  
            //Auto Increments Index    
            UniqueBundleIndex++; 

            //Assigns State Variables To The New Bundle Instance
            Bundles[UniqueBundleIndex].Name = NewBundleInstance[i].Name;
            Bundles[UniqueBundleIndex].PriceBrightList = NewBundleInstance[i].PriceBrightList;
            Bundles[UniqueBundleIndex].PricePublic = NewBundleInstance[i].PricePublic;
            Bundles[UniqueBundleIndex].PurchaseableAmount = NewBundleInstance[i].PurchaseableAmount;
            Bundles[UniqueBundleIndex].PurchaseableAmountBrightList = NewBundleInstance[i].PurchaseableAmountBrightList;
            Bundles[UniqueBundleIndex].PurchaseableAmountPublic = NewBundleInstance[i].PurchaseableAmountPublic;
            Bundles[UniqueBundleIndex].ContractAddresses = NewBundleInstance[i].ContractAddresses;
            Bundles[UniqueBundleIndex].Operator = NewBundleInstance[i].Operator;
            Bundles[UniqueBundleIndex].Root = NewBundleInstance[i].Root;
            Bundles[UniqueBundleIndex].StartingIndexes = NewBundleInstance[i].StartingIndexes;
            Bundles[UniqueBundleIndex].EndingIndexes = NewBundleInstance[i].EndingIndexes;
            Bundles[UniqueBundleIndex].ActivePublic = NewBundleInstance[i].ActivePublic;
            Bundles[UniqueBundleIndex].ActiveBrightList = NewBundleInstance[i].ActiveBrightList;
            Bundles[UniqueBundleIndex].AllowMultiplePurchases = NewBundleInstance[i].AllowMultiplePurchases;
        }
        emit BundlesStarted(StartingBundleIndex, UniqueBundleIndex);
    }
     ...
     */
    function _ChangeBundleName(uint BundleIndex, string memory Name) external onlyBRTOperator 
    { 
        string memory OldName = Bundles[BundleIndex].Name;
        Bundles[BundleIndex].Name = Name; 
        emit BundleChangedName(BundleIndex, OldName, Name);
    }
     ...
     */
    function _ChangeBundleAllocationPublic(uint BundleIndex, uint Amount) external onlyBRTOperator 
    {
        uint OldAllocation = Bundles[BundleIndex].PurchaseableAmountBrightList; 
        Bundles[BundleIndex].PurchaseableAmountPublic = Amount; 
        emit BundleChangedAllocationPublic(BundleIndex, OldAllocation, Amount);
    }
     ...
     */
    function _ChangeBundleAllocationBrightList(uint BundleIndex, uint Amount) external onlyBRTOperator 
    {
        uint OldAllocation = Bundles[BundleIndex].PurchaseableAmountBrightList; 
        Bundles[BundleIndex].PurchaseableAmountBrightList = Amount; 
        emit BundleChangedAllocationBrightList(BundleIndex, OldAllocation, Amount);
    }
     ...
     */
    function _ChangeBundlePricePublic(uint BundleIndex, uint Price) external onlyBRTOperator 
    { 
        uint OldPrice = Bundles[BundleIndex].PriceBrightList; 
        Bundles[BundleIndex].PricePublic = Price;
        emit BundleChangedPricePublic(BundleIndex, OldPrice, Price);
    }
     ...
     * note: This Is Input In WEI Not In Ether. 1 ETH = 1000000000000000000 WEI
     */
    function _ChangeBundlePriceBrightList(uint BundleIndex, uint Price) external onlyBRTOperator 
    {
        uint OldPrice = Bundles[BundleIndex].PriceBrightList; 
        Bundles[BundleIndex].PriceBrightList = Price; 
        emit BundleChangedPriceBrightList(BundleIndex, OldPrice, Price);
    }
     ...
     */
    function _ChangeBundleContract(uint BundleIndex, address[] calldata Contracts) external onlyBRTOperator 
    { 
        address[] memory OldContract = Bundles[BundleIndex].ContractAddresses;
        Bundles[BundleIndex].ContractAddresses = Contracts; 
        emit BundleChangedContracts(BundleIndex, OldContract, Contracts);
    }
     ...
     */
    function _ChangeBundleOperator(uint BundleIndex, address Operator) external onlyBRTOperator 
    { 
        address OldOperator = Bundles[BundleIndex].Operator;
        Bundles[BundleIndex].Operator = Operator; 
        emit BundleChangedOperator(BundleIndex, OldOperator, Operator);
    }
     ...
     */
    function _ChangeBundleRoot(uint BundleIndex, bytes32 RootHash) external onlyBRTOperator 
    { 
        bytes32 OldRoot = Bundles[BundleIndex].Root;
        Bundles[BundleIndex].Root = RootHash; 
        emit BundleChangedRoot(BundleIndex, OldRoot, RootHash);
    }
     ...
     */
    function _ChangeBundleActiveStatePublic(uint BundleIndex, bool State) external onlyBRTOperator 
    { 
        bool OldState = Bundles[BundleIndex].ActivePublic;
        Bundles[BundleIndex].ActivePublic = State; 
        emit BundleChangedActiveState(BundleIndex, OldState, State);
    }

    /**
     * @dev Changes BrightList Bundle State `State` At Index `BundleIndex`
     * note: Possible Inputs are `true` or `false`
     */
    function _ChangeBundleActiveStateBrightList(uint BundleIndex, bool State) external onlyBRTOperator 
    { 
        bool OldState = Bundles[BundleIndex].ActiveBrightList;
        Bundles[BundleIndex].ActiveBrightList = State; 
        emit BundleChangedActiveStateBrightList(BundleIndex, OldState, State);
    }
    ...
    /**
     * @dev Returns Multiple Bundle Instances Of `BundleIndex`
     * note: `0. Name`
     * note: `1. Price BrightList`
     * note: `2. Price Public`
     * note: `3. StartingIndexes`
