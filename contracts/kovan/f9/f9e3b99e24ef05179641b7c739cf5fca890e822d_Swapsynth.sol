...
contract Swapsynth {
    ...
    event  Buy(string symbol, uint val);
    event  Sell(string symbol, uint val);
    ...
    constructor() {
        ...
        addAsset("BTC", address(0), 0x6135b13325bfC4B00278B4abC5e20bbce2D6580e);
        addAsset("ETH", address(0), 0x9326BFA02ADD2366b30bacB125260Af641031331);
        addAsset("LTC", address(0), 0xCeE03CF92C7fFC1Bad8EAA572d69a4b61b6D4640);

        addAsset("UNI", address(0), 0xDA5904BdBfB4EF12a3955aEcA103F51dc87c7C39);
        addAsset("LINK", address(0), 0x396c5E36DD0a0F5a5D33dae44368D4193f69a1F0);
        addAsset("COMP", address(0), 0xECF93D14d25E02bA2C13698eeDca9aA98348EFb6);

        addAsset("CHF", address(0), 0xed0616BeF04D374969f302a34AE4A63882490A8C);
        addAsset("EUR", address(0), 0x0c15Ab9A0DB086e062194c273CC79f41597Bbf13);
        addAsset("GBP", address(0), 0x28b0061f44E6A9780224AA61BEc8C3Fcb0d37de9);

        addAsset("XAU", address(0), 0xc8fb5684f2707C82f28595dEaC017Bfdf44EE9c5);
        addAsset("XAG", address(0), 0x4594051c018Ac096222b5077C3351d523F93a963);
        addAsset("OIL", address(0), 0x48c9FF5bFD7D12e3C511022A6E54fB1c5b8DC3Ea);
    }
    ...
    function addAsset(string memory _symbol, address _token, address _oracle) public {
    ...
    function editAsset(uint _id, string calldata _symbol, address _token, address _oracle) external {
        require(owners[msg.sender] == true);
        assets[_symbol] = Asset(_id, _symbol, _token, _oracle);
    }

    function buyAsset(string calldata _symbol, uint _size, bool _bal) external {
        require(control());
        ...
        if (_bal == true) {
            require(balances[msg.sender]["VSDC"] >= val, "No VSDC");

            balances[msg.sender]["VSDC"] -= val;
    ...
    function sellAsset(string calldata symbol, uint size, bool bal) external {
        require(control());
    ...
    function deposit(uint _size) public {
        ...
        balances[msg.sender]["VSDC"] += _size;
    ...
    function withdraw(uint _size) public {
        require(control());
        require(_size > 0, "No size");
        require(balances[msg.sender]["VSDC"] >= _size, "No VSDC");
