...
contract USMTemplate is IUSM, Oracle, ERC20Permit, WithOptOut, Delegable {
    ...
    event PriceChanged(uint256 timestamp, uint256 price);

    uint256 public constant WAD = 10**18;
    uint256 public constant HALF_WAD = WAD / 2;
    uint256 public constant BILLION = 10**9;
    uint256 public constant HALF_BILLION = BILLION / 2;
    uint256 public constant MAX_DEBT_RATIO = (WAD * 8) / 10; // 80%
    uint256 public constant MIN_FUM_BUY_PRICE_HALF_LIFE = 1 days; // Solidity for 1 * 24 * 60 * 60
    uint256 public constant BID_ASK_ADJUSTMENT_HALF_LIFE = 1 minutes; // Solidity for 1 * 60
    ...
    struct LoadedState {
        ...
        uint256 ethUsdPriceTimestamp;
        uint256 ethUsdPrice; // This one is in WADs, not billionths
        uint256 bidAskAdjustmentTimestamp;
        uint256 bidAskAdjustment; // WADs, not billionths
        uint256 ethPool;
        uint256 usmTotalSupply;
    }

    StoredState public storedState =
        StoredState({
            timeSystemWentUnderwater: 0,
            ethUsdPriceTimestamp: 0,
            ethUsdPrice: 0,
            bidAskAdjustmentTimestamp: 0,
            bidAskAdjustment: uint80(BILLION) // Initialize adjustment to 1.0 (scaled by 1b)
        });

    constructor(
        Oracle oracle_,
        address[] memory optedOut_,
        string memory name,
        string memory symbol,
        string memory fundingName,
        string memory fundingSymbol
    ) ERC20Permit(name, symbol) WithOptOut(optedOut_) {
        oracle = oracle_;
    ...
    modifier onlyHolderOrDelegateOrFUM(address owner, string memory errorMessage) {
        require(msg.sender == owner || delegated[owner][msg.sender] || msg.sender == address(fum), errorMessage);
        _;
    }
     ...
     */
    function mint(address to, uint256 minUsmOut) external payable override returns (uint256 usmOut) {
    ...
    function burn(
        ...
        uint256 usmToBurn,
        uint256 minEthOut
    ) external override onlyHolderOrDelegate(from, 'Only holder or delegate') returns (uint256 ethOut) {
     ...
     */
    function fund(address to, uint256 minFumOut) external payable override returns (uint256 fumOut) {
    ...
    function defund(
        ...
        uint256 fumToBurn,
        uint256 minEthOut
    ) external override onlyHolderOrDelegateOrFUM(from, 'Only holder or delegate or FUM') returns (uint256 ethOut) {
    ...
    function refreshPrice() public virtual override(IUSM, Oracle) returns (uint256 price, uint256 updateTime) {
    ...
    function _transfer(
        ...
        address recipient,
        uint256 amount
    ) internal override noOptOut(recipient) returns (bool) {
    ...
    function _mintUsm(address to, uint256 minUsmOut) internal trialPeriod returns (uint256 usmOut) {
    ...
    function _burnUsm(
        ...
        uint256 newDebtRatio = debtRatio(ls.ethUsdPrice, ls.ethPool - ethOut, ls.usmTotalSupply - usmToBurn);
    ...
    function _fundFum(address to, uint256 minFumOut) internal trialPeriod returns (uint256 fumOut) {
        // 1. Load the stored state:
        LoadedState memory ls = loadState();
        ls.ethPool -= msg.value; // Backing out the ETH just received, which our calculations should ignore
        ...
        (ls.timeSystemWentUnderwater, ls.usmTotalSupply) = checkIfUnderwater(
            ls.usmTotalSupply,
            ls.ethPool,
            ls.ethUsdPrice,
        ...
        // 4. Calculate fumOut:
        uint256 fumSupply = fum.totalSupply();
        uint256 adjGrowthFactor;
    ...
    function _defundFum(
        ...
        uint256 newDebtRatio = debtRatio(ls.ethUsdPrice, ls.ethPool - ethOut, ls.usmTotalSupply);
        ...
        )
    {
        ...
        if (!priceChanged) {
            // If the price isn't fresher than our old one, scrap it and stick to the old one
            (price, updateTime) = (ls.ethUsdPrice, ls.ethUsdPriceTimestamp);
        } else if (ls.ethUsdPrice != 0) {
            // If the old price is 0, don't try to use it to adjust the bidAskAdjustment...
            /**
    ...
    function _storeState(LoadedState memory ls) internal {
        ...
        (
            ...
            storedState.ethUsdPrice,
            storedState.bidAskAdjustmentTimestamp,
            storedState.bidAskAdjustment
        ) = (
            uint32(ls.timeSystemWentUnderwater),
            uint32(ls.ethUsdPriceTimestamp),
            uint80(priceToStore),
            uint32(ls.bidAskAdjustmentTimestamp),
            uint80(adjustmentToStore)
        );
    }

    // ____________________ Public Oracle view functions ____________________

    function latestPrice() public view virtual override(IUSM, Oracle) returns (uint256 price, uint256 updateTime) {
    ...
    function latestOraclePrice() public view virtual override returns (uint256 price, uint256 updateTime) {
     ...
     */
    function ethPool() public view override returns (uint256 pool) {
        pool = address(this).balance;
    }

    function fumTotalSupply() public view override returns (uint256 supply) {
     ...
     */
    function bidAskAdjustment() public view override returns (uint256 adjustment) {
        adjustment = loadState().bidAskAdjustment; // Not just from storedState, b/c need to update it - see loadState()
    }

    function timeSystemWentUnderwater() public view override returns (uint256 timestamp) {
    ...
    function loadState() public view returns (LoadedState memory ls) {
        ...
        ls.bidAskAdjustment = bidAskAdjustment(
            storedState.bidAskAdjustmentTimestamp,
    ...
    function ethBuffer(
    ...
    ) public pure override returns (int256 buffer) {
        ...
        buffer = int256(ethInPool) - int256(usmToEth(ethUsdPrice, usmSupply, downOrUp));
        require(buffer <= int256(ethInPool), 'Underflow error');
    }
    ...
    function debtRatio(
        ...
        uint256 ethInPool,
        uint256 usmSupply
    ) public pure override returns (uint256 ratio) {
        uint256 ethPoolValueInUsd = ethInPool.wadMulDown(ethUsdPrice);
    ...
    function ethToUsm(
        ...
        uint256 ethAmount,
        WadMath.Round upOrDown
    ) public pure override returns (uint256 usmOut) {
    ...
    function usmToEth(
        ...
        uint256 usmAmount,
        WadMath.Round upOrDown
    ) public pure override returns (uint256 ethOut) {
    ...
    function fumPrice(
        ...
        if (fumSupply == 0) {
            ...
            int256 buffer = ethBuffer(ethUsdPrice, ethInPool, usmEffectiveSupply, upOrDown);
            price = (buffer <= 0 ? 0 : uint256(buffer).wadDiv(fumSupply, upOrDown));
    ...
    function checkIfUnderwater(
        ...
        uint256 currentTime
    ) public pure override returns (uint256 timeSystemWentUnderwater_, uint256 usmSupplyForFumBuys) {
        uint256 debtRatio_ = debtRatio(ethUsdPrice, ethPool_, usmActualSupply);
        if (debtRatio_ <= MAX_DEBT_RATIO) {
            // We're not underwater, so leave timeSystemWentUnderwater_ as 0
            ...
            uint256 maxEffectiveDebtRatio = debtRatio_.wadMin(WAD); // min(actual debt ratio, 100%)
            uint256 numHalvings = (currentTime - timeSystemWentUnderwater_).wadDivDown(MIN_FUM_BUY_PRICE_HALF_LIFE);
            uint256 decayFactor = numHalvings.wadHalfExp();
            uint256 effectiveDebtRatio = maxEffectiveDebtRatio -
                decayFactor.wadMulUp(maxEffectiveDebtRatio - MAX_DEBT_RATIO);
     ...
     */
    function bidAskAdjustment(
        uint256 storedTime,
        uint256 storedAdjustment,
        uint256 currentTime
    ) public pure returns (uint256 adjustment) {
        uint256 numHalvings = (currentTime - storedTime).wadDivDown(BID_ASK_ADJUSTMENT_HALF_LIFE);
     ...
     */
    function usmFromMint(LoadedState memory ls, uint256 ethIn)
        public
        pure
        returns (uint256 usmOut, uint256 adjShrinkFactor)
    {
        ...
        uint256 usmBuyPrice0 = usmPrice(IUSM.Side.Buy, ls.ethUsdPrice, ls.bidAskAdjustment);
        uint256 ethPool1 = ls.ethPool + ethIn;
        ...
        int256 log = ethPool1.wadDivDown(ls.ethPool).wadLog();
        require(log >= 0, 'log underflow');
        usmOut = ls.ethPool.wadDivDown(usmBuyPrice0).wadMulDown(uint256(log));
    }
     ...
     */
    function ethFromBurn(LoadedState memory ls, uint256 usmIn)
        public
        pure
        returns (uint256 ethOut, uint256 adjGrowthFactor)
    {
        ...
        uint256 usmSellPrice0 = usmPrice(IUSM.Side.Sell, ls.ethUsdPrice, ls.bidAskAdjustment);

        // The integral - calculating the amount of ETH yielded by burning the USM at our sliding-down USM price:
        uint256 exponent = usmIn.wadMulDown(usmSellPrice0).wadDivDown(ls.ethPool);
        uint256 ethPool1 = ls.ethPool.wadDivUp(exponent.wadExp());
     ...
     */
    function fumFromFund(
        LoadedState memory ls,
        uint256 fumSupply,
        uint256 ethIn
    ) public pure returns (uint256 fumOut, uint256 adjGrowthFactor) {
        if (ls.ethPool == 0) {
            // No ETH in the system yet, which breaks our adjGrowthFactor calculation below - skip sliding-prices this time:
            adjGrowthFactor = WAD;
            uint256 fumBuyPrice0 = fumPrice(
                IUSM.Side.Buy,
                ls.ethUsdPrice,
                ls.ethPool,
                ls.usmTotalSupply,
            ...
            uint256 effectiveDebtRatio = debtRatio(ls.ethUsdPrice, ls.ethPool, ls.usmTotalSupply);
            uint256 effectiveFumDelta = WAD.wadDivUp(WAD - effectiveDebtRatio);
            ...
            uint256 ethUsdPrice1 = ls.ethUsdPrice.wadMulUp(adjGrowthFactor);
            uint256 avgFumBuyPrice = ls.bidAskAdjustment.wadMulUp(
     ...
     * @return ethOut The amount of ETH to receive in exchange
     */
    function ethFromDefund(
        LoadedState memory ls,
        uint256 fumSupply,
        uint256 fumIn
    ) public pure returns (uint256 ethOut, uint256 adjShrinkFactor) {
        ...
        uint256 fumSellPrice0 = fumPrice(
            IUSM.Side.Sell,
            ls.ethUsdPrice,
            ls.ethPool,
            ls.usmTotalSupply,
        ...
        uint256 fumDelta = WAD.wadDivUp(WAD - debtRatio(ls.ethUsdPrice, ls.ethPool, ls.usmTotalSupply));
        ...
        uint256 lowerBoundAdjShrinkFactor = (ls.ethPool - fumIn.wadMulUp(fumSellPrice0)).wadDivDown(ls.ethPool).wadExp(
            fumDelta / 2
        );

        // 5. From the "pessimistic" lowerBoundAdjShrinkFactor, we can easily infer a lower bound on what the ETH mid price
        // can end up at:
        uint256 lowerBoundEthUsdPrice1 = ls.ethUsdPrice.wadMulDown(lowerBoundAdjShrinkFactor);
        ...
        uint256 avgFumSellPrice = fumPrice(
            IUSM.Side.Sell,
            lowerBoundEthUsdPrice1,
            ls.ethPool,
            ls.usmTotalSupply,
            fumSupply,
            ls.bidAskAdjustment
...
abstract contract IUSM is IERC20 {
    ...
    function mint(address to, uint256 minUsmOut) external payable virtual returns (uint256);
    ...
    function fund(address to, uint256 minFumOut) external payable virtual returns (uint256);
    ...
    function refreshPrice() public virtual returns (uint256 price, uint256 updateTime);

    function latestPrice() public view virtual returns (uint256 price, uint256 updateTime);

    function latestOraclePrice() public view virtual returns (uint256 price, uint256 updateTime);

    function ethPool() public view virtual returns (uint256 pool);

    function fumTotalSupply() public view virtual returns (uint256 supply);

    function bidAskAdjustment() public view virtual returns (uint256 adjustment);

    function timeSystemWentUnderwater() public view virtual returns (uint256 timestamp);
    ...
    function ethBuffer(
        ...
        uint256 usmSupply,
        WadMath.Round upOrDown
    ) public pure virtual returns (int256 buffer);

    function debtRatio(
        uint256 ethUsdPrice,
        uint256 ethInPool,
        uint256 usmSupply
    ) public pure virtual returns (uint256 ratio);

    function ethToUsm(
        uint256 ethUsdPrice,
        uint256 ethAmount,
        WadMath.Round upOrDown
    ) public pure virtual returns (uint256 usmOut);

    function usmToEth(
        uint256 ethUsdPrice,
        uint256 usmAmount,
        WadMath.Round upOrDown
    ) public pure virtual returns (uint256 ethOut);
    ...
    function checkIfUnderwater(
        ...
        uint256 oldTimeUnderwater,
        uint256 currentTime
    ) public pure virtual returns (uint256 timeSystemWentUnderwater_, uint256 usmSupplyForFumBuys);
}
