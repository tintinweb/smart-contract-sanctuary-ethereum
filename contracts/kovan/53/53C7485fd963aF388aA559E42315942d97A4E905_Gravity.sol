...
contract Gravity is KeeperCompatibleInterface {
    ...
    function reconstructSchedule(address _account) public view returns (uint256[] memory, uint256[] memory) {
        ...
        // uint _unixSlot = _accountStart - (_accountStart % upKeepInterval) + 2 * upKeepInterval;

        uint _unixSlot = _accountStart - (_accountStart % upKeepInterval) + 2 * upKeepInterval;
        uint _nearestInterval = block.timestamp - (block.timestamp % upKeepInterval) + 2 * upKeepInterval;
        if(_nearestInterval > _unixSlot && _nearestInterval < (_unixSlot + (_interval * _purchasesRemaining))) {
            _unixSlot = _nearestInterval;
        }

        uint _unixInterval = _interval * upKeepInterval;
        for(uint i = 1; i <= _purchasesRemaining; i++) {
            uint _nextUnixPurchaseDate = _unixSlot + (_unixInterval * i);
