...
contract ZkBobAccounting {
    ...
    /**
     * @dev Returns currently configured limits and remaining quotas for the given user as of current block timestamp.
     * @param _user user for which to retrieve limits.
     * @return res array of 9 values (denominated in zkBOB units = 1e-9 BOB):
     * - tvl cap
     * - current tvl
     * - daily deposit cap
     * - today daily deposit cap usage
     * - daily withdrawal cap
     * - today withdrawal deposit cap usage
     * - daily user deposit cap
     * - today user daily deposit cap usage
     * - single deposit cap
     */
    function getLimitsFor(address _user) external view returns (uint256[9] memory res) {
        ...
        res[0] = s2.tvlCap * PRECISION;
        res[1] = s1.tvl;
        res[2] = s2.dailyDepositCap * PRECISION;
        res[3] = (s0.headSlot / DAY_SLOTS == today) ? s1.dailyDeposit * PRECISION : 0;
        res[4] = s2.dailyWithdrawalCap * PRECISION;
        res[5] = (s0.headSlot / DAY_SLOTS == today) ? s1.dailyWithdrawal * PRECISION : 0;
        res[6] = s2.dailyUserDepositCap * PRECISION;
        res[7] = (us.day == today) ? us.dailyDeposit : 0;
        res[8] = s2.depositCap * PRECISION;
    }
