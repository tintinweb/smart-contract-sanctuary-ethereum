...
contract MerkleRewardsDistributor is Ownable, ReentrancyGuard {
    ...
    function claim(
        ...
        require(
            isInState(userAddress, amountToClaim, leafIndex, merkleProof),
    ...
    function isInState(
        ...
        return
            _verify(_leaf(userAddress, amountToClaim, leafIndex), merkleProof);
    }
    ...
    function _leaf(
        ...
        console.logBytes32(
            keccak256(abi.encodePacked(account, amountToClaim, leafIndex))
        );
        console.log("On-chain encodePacked");
        console.logBytes(abi.encodePacked(account, amountToClaim, leafIndex));
        // bytes memory data = ConvertUtils.addressToBytesString(
        //     address(account)
        // );
        return keccak256(abi.encodePacked(bytes20(account), bytes32(amountToClaim), bytes32(leafIndex)));
    }
