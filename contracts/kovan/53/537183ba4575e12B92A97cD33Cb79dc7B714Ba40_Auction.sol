...
contract Auction is Ownable, ReentrancyGuard, ERC721Holder, ERC1155Holder,AccessControl {
    ...
    mapping(address => bytes4) public whitelistedCollections;
    ...
    event Bided(bytes32 collectibleHash, address bidder, uint256 price);
    ...
    function removeWhitelistedToken(address collection,uint256 tokenId) external onlyRole(DEFAULT_ADMIN_ROLE) {
        require(allowTokens[collection][tokenId] == true, "Collection or tokenId is not whitelisted");

        delete allowTokens[collection][tokenId];
        emit WhitelistedTokenRemoved(collection,tokenId);
    }

    function addCollectible(address collection, uint256 tokenId, uint256 amount, address currency, uint256 price, uint256 opening, uint256 finish) external nonReentrant {
        require(allowTokens[collection][tokenId] == true, "Collection or tokenId is not whitelisted");
    ...
    function bid(bytes32 collectibleHash, uint256 price) payable external nonReentrant {
        ...
        emit Bided(collectibleHash, _msgSender(), price + paymentAmount);
    }
