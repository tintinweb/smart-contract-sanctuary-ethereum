...
// File: contracts\market-place\MarketPlace.sol
...
contract MarketPlace is IAdmin, IMarketPlace, IModuleBox, Ownable {
    ...
    constructor(
        ...
        mdmaToken = _mdmaToken;
        moduleToken = _moduleToken;
        feeWallet = msg.sender;
        mdmaTokenPrice = 100000000;
    }
    ...
    function setMDMAToken(address _address) external override isAdmin {
        ...
        emit TokenAdressChanged(mdmaToken, _address);
        mdmaToken = _address;
    }
    ...
    function withdrawMDMATokens(address token) external override isAdmin {
    ...
    function setMDMATokenPrice(uint256 _price) external override isAdmin {
        emit TokenPriceChanged(mdmaTokenPrice, _price);
    ...
    function buyMDMAToken(uint256 _amount) external payable override {
        ...
        IMintable(mdmaToken).mint(msg.sender, _amount);
        ...
        override
    {
        ...
        cell.onSale = false;
        ICellRepository(metaCellToken).updateMetaCell(cell, cell.user);
    ...
    function sellMetaCell(uint256 _tokenId, uint256 _price) external override {
        ...
        metaCellsOwners.push(IERC721(metaCellToken).ownerOf(_tokenId));
    ...
    function buyModule(uint256 id) external override {
        ...
        require(
            IERC20(mdmaToken).balanceOf(msg.sender) > price,
            "Not enough funds"
        );
        IERC20(mdmaToken).transferFrom(msg.sender, address(this), price);
        uint256 feeAmount = getFeeAmount(price);
        IERC20(mdmaToken).approve(address(this), price.sub(feeAmount));
        IERC20(mdmaToken).transferFrom(
            address(this),
            oldOwnerOf,
            price.sub(feeAmount)
        );
    ...
    function buyBox(address owner) external override {
        require(
            IERC20(mdmaToken).balanceOf(msg.sender) >= pricePerBox,
            "Not enough funds"
        );

        //TODO: ideally we should send mdma directly to feeWallet
        IERC20(mdmaToken).transferFrom(msg.sender, address(this), pricePerBox);
    ...
    function openBox(uint256 id) external override {
        ...
        IMintable(moduleToken).mint(msg.sender, 1);
    }
