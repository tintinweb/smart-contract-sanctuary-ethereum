...
contract StakingPool
{
	...
	uint256 public totalRewardPerSecond;               // Total reward per time second.
	Epoch[] public epochs;                             // Array of epochs.
	uint256 public lastRewardTimestamp;                // Date of staking end.

	bool internal wasCalled = false;

	// position id => Position struct
	mapping (address => Position) public positions;    // Records Into position struct.
	...
	{
		require(wasCalled == false, "already filled");
		wasCalled = true;                                                       // Contract is for one fullfill only.

		rewardToken.transferFrom(msg.sender, address(this), value);             // Transfers tokens to contract.
		lastRewardTimestamp = block.timestamp + value / totalRewardPerSecond;   // Sets date of end of staking.
	}
	...
	{
		stakedToken.transferFrom(msg.sender, address(this), value);             // Еransfers tokens from sender to this contract.
		...
		if (block.timestamp >= lastRewardTimestamp)                             // If staking time has ended.
		{
	...
	function harvest(address who) public
	{
		...
		uint256 duration = block.timestamp < lastRewardTimestamp ? block.timestamp : lastRewardTimestamp;
