    ...
    UUPSUpgradeable
{
    ...
    mapping(address => uint256) private _totalAmountClaimed;
  ...
  //Convert String to Uint
   ...
   //WITHDRAWALS
     ...
     */
    //FOR GAMES
    function getGameScoreRequestStatus(uint256 fixtureID) external view returns(bool){
        bytes memory result = IScorefamConsumer(consumer).requestIdData(IScorefamConsumer(consumer).gameScoreRequestID(fixtureID));
        if(result.length == 0 || result.slice(2,10).equal(hex"6e6f7473746172746564"))
            return true;
        return false;
    }

    function updateGameScore(uint256 fixtureID) external {
    ...
    function claimGameReward(uint256 game_) external returns (bool){
        Game memory game = _gaming[_msgSender()][game_];

        require(game.gameActive);

        if(!gameResult[game.fixtureID].retrieved){
            sliceBytes(game.fixtureID, IScorefamConsumer(consumer).requestIdData(IScorefamConsumer(consumer).gameScoreRequestID(game.fixtureID)));
        }
        
        

        require(gameResult[game.fixtureID].retrieved);
        uint256 amount_ = calculateGameReward(game_);
        uint256 onePercent = amount_.div(100);
        _totalStakedSupply -= amount_;
        _gameTypeOptions[game.gameType]
    ...
    function getSlipScoreRequestStatus(uint256 game_) external view returns(bool){
        ...
        bytes memory result;
        for(uint256 i = 0; i < _gamingSlip[_msgSender()][game_].slips.length; i++){
            result = IScorefamConsumer(consumer).requestIdData(IScorefamConsumer(consumer).gameScoreRequestID(_gamingSlip[_msgSender()][game_].slips[i].fixtureID));
            if(result.length == 0 || result.slice(2,10).equal(hex"6e6f7473746172746564")){
                outcome = true;
                break;
            }else {
    ...
    function updateSlipScore(uint256 game_) external {
        //Update Game Status, Home and Away Scores from the Oracle
        Slip memory slip = _gamingSlip[_msgSender()][game_];
        IScorefamConsumer sc = IScorefamConsumer(consumer);
        bytes memory result;

        for(uint256 i = 0; i < slip.slips.length; i++){
            result = sc.requestIdData(sc.gameScoreRequestID(slip.slips[i].fixtureID));
            if(result.length == 0 || result.slice(2,10).equal(hex"6e6f7473746172746564")){
                //Update Game Status, Home and Away Scores from the Oracle
                bytes32 JOB_ID = bytes32("7a1ade57c05f49f1bd5c25241acdb107");       
                sc.requestGameScore(JOB_ID, slip.slips[i].fixtureID); 
            }else {
                continue;
            }        
        }
    ...
    function claimSlipReward(uint256 game_) external returns (bool){
        Slip memory slip = _gamingSlip[_msgSender()][game_];
        require(slip.gameActive);
        IScorefamConsumer sc = IScorefamConsumer(consumer);
        
        for(uint256 i = 0; i < slip.slips.length; i++){
            if(!gameResult[slip.slips[i].fixtureID].retrieved){
                sliceBytes(slip.slips[i].fixtureID, sc.requestIdData(sc.gameScoreRequestID(slip.slips[i].fixtureID)));
            }
            require(gameResult[slip.slips[i].fixtureID].retrieved);
        }
        
        uint256 amount_ = calculateSlipReward(game_);
        uint256 onePercent = amount_.div(100);
        _totalStakedSupply -= amount_;
        _gameTypeOptions[slip.gameType].gameTypeStakedPool -= amount_;
        slip.gameActive = false;
        _totalAmountClaimed[_msgSender()] += amount_;
        _totalGamesClaimed[_msgSender()]++;
        _mint(fee, onePercent); //Charging 1% Fee
        _mint(_msgSender(), amount_.sub(onePercent));
