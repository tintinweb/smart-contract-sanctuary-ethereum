...
contract SecondaryNftBridgePool is IERC721Receiver, IERC1155Receiver, AccessControl, ReentrancyGuard {
  ...
  function bulkWithdrawNfts(uint232[] memory _contractId, address[] memory _recipient, uint256[] memory _cardId, uint256[] memory _cardLevel, uint256[] memory _amount, bytes1[] memory _mintedContractChar, uint256[] memory _number, uint256[] memory _requestsId, uint32[] memory _stamina, bytes[] calldata _state) 
    external onlyMinter {
      for (uint256 i; i < _contractId.length; i++) {
        uint256 mainBridgeCount = mainNftBridge.findHeldToken(_contractId[i], _cardId[i], _cardLevel[i], _mintedContractChar[i], _number[i]);
        uint256 mainBridgeAmount = 0;
        if (mainBridgeCount > 0) {
          if (_amount[i] > mainBridgeCount) {
            mainBridgeAmount = mainBridgeCount;
          } else {
            mainBridgeAmount = _amount[i];
          }
          mainNftBridge.withdrawNft(_contractId[i], _recipient[i], _cardId[i], _cardLevel[i], mainBridgeAmount, _mintedContractChar[i], _number[i], _requestsId[i], _stamina[i], _state[i]);
        }
        if (_amount[i] > mainBridgeCount) {
          withdrawNft(_contractId[i], _recipient[i], _cardId[i], _cardLevel[i], _amount[i] - mainBridgeCount, _mintedContractChar[i], _number[i], _requestsId[i], _stamina[i], _state[i]);
        }
