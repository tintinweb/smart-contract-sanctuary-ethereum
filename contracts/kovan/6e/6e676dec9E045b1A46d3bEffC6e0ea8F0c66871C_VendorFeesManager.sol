...
contract VendorFeesManager {
    event ChangeFee(address pool, uint256[] feeRates);
    ...
    function setPoolFees(address _pool, uint256[] calldata _feeRates) external {
        if (
            feeRates[_pool].length != 0 &&
            _feeRates.length != feeRates[_pool].length
        ) revert InvalidArray();
        if (msg.sender == address(factory) || _pool == msg.sender) {
            feeRates[_pool] = _feeRates;
            rateFunction[_pool] = _feeRates.length == 2;
            emit ChangeFee(_pool, _feeRates);
        } else {
