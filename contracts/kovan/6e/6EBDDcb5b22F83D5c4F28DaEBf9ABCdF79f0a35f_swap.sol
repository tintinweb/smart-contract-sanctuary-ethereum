...
contract swap{
    ...
    function dex(
        ...
        address [] memory Token_In_Out,
        uint[] memory DexRoute,   
        uint[] memory Uniswap_amount,  
        int[] memory options,           
        address[] memory path,
           ...
           // uint[] memory Uni_amountIn = Uniswap_amountIn;
            ...
            for(uint i=0; i<DexRoute.length; i++){
                ...
                if(DexRoute[i] == 0)
                {
                    ...
                    amountin = Uniswap_amount[amountIndex];
                    amountOut = Uniswap_amount[amountIndex+1];
            ...
            { 
                ...
                if(address(singleswap.assetIn) == ETH){
                    ...
                    Ibal(balancerVault).swap { value: msg.value }(singleswap, funds, limit, time);
                }
                ...
                Ibal(balancerVault).swap(singleswap, funds, limit, time);
                }
            }  
            ...
            if(DexRoute[i] == 2)
            {
                if(address(assets[0]) == ETH){
                    Ibal(balancerVault).batchSwap { value: msg.value }(kind, swaps, assets, funds, limits, time);
                }

                else{

                IERC20(address(assets[0])).transferFrom(msg.sender, address(this),  swaps[0].amount);
                IERC20(address(assets[0])).approve(balancerVault,  swaps[0].amount);
                Ibal(balancerVault).batchSwap(kind, swaps, assets, funds, limits, time);
                
                }
            }
        }
        
        //for getting AmountIn & TokenIn for Emitting Event 
        if(DexRoute[0] == 0){
            eventEmit(Uniswap_amount[0], UserBalanceBeforeSwap, path[0], Tokenout, msg.sender);
        }

        else if(DexRoute[0] == 1){
            eventEmit(singleswap.amount, UserBalanceBeforeSwap, address(singleswap.assetIn), Tokenout, msg.sender);
        }

        else{
            eventEmit(swaps[0].amount, UserBalanceBeforeSwap, address(assets[0]), Tokenout, msg.sender);
        }
    ...
    function eventEmit(uint amountIn, uint UserBalanceBeforeSwap, address TokenIn, address TokenOut, address sender) internal{
        uint256 amountRecevied = IERC20(TokenOut).balanceOf(sender) - UserBalanceBeforeSwap;
        console.log("AMount REc", amountRecevied);
        emit BeeSwap(sender, TokenIn, TokenOut, amountIn, amountRecevied, block.timestamp);
    }
