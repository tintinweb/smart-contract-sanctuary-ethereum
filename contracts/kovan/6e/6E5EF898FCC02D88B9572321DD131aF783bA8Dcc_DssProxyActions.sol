...
contract DssProxyActions is Common {
    ...
    address public constant config = 0xBFC8BAA17D6f7AF5d330576E7db0844F4aeaD3BB;
    ...
    function draw(
        ...
        bytes32 ilk = ManagerLike(manager).ilks(cdp);
        uint _fee = wad * getFee(ilk, Config.Fees.BORROW);
    ...
    function wipe(
        ...
        address urn = ManagerLike(manager).urns(cdp);
        bytes32 ilk = ManagerLike(manager).ilks(cdp);
        uint _fee = wad * getFee(ilk, Config.Fees.PAYBACK);
        if(_fee > 0) {
            USBJoinLike(USBJoin).USB().transferFrom(msg.sender, Config(config).getTreasury(), _fee);
            wad -= _fee;
        }
    ...
    function wipeAll(
        ...
        bytes32 ilk = ManagerLike(manager).ilks(cdp);
        (, uint art) = VatLike(vat).urns(ilk, urn);
        uint _fee = art * getFee(ilk, Config.Fees.PAYBACK);
        if(_fee > 0) {
            USBJoinLike(USBJoin).USB().transferFrom(msg.sender, Config(config).getTreasury(), _fee);
            art -= _fee;
        }
        ...
        if (own == address(this) || ManagerLike(manager).cdpCan(own, cdp, address(this)) == 1) {
            // Joins USB amount into the vat
            USBJoin_join(ilk, USBJoin, urn, _getWipeAllWad(vat, urn, urn, ilk));
            // Paybacks debt to the CDP
            frob(manager, cdp, 0, -int(art));
        } else {
            // Joins USB amount into the vat
            USBJoin_join(ilk, USBJoin, address(this), _getWipeAllWad(vat, address(this), urn, ilk));
            // Paybacks debt to the CDP
            VatLike(vat).frob(
                ilk,
                urn,
    ...
    function wipeAllAndFreeETH(
        ...
        bytes32 ilk = ManagerLike(manager).ilks(cdp);
        (, uint art) = VatLike(vat).urns(ilk, urn);
        uint _feeRepay = art * getFee(ilk, Config.Fees.PAYBACK);
        if(_feeRepay > 0) {
            USBJoinLike(USBJoin).USB().transferFrom(msg.sender, Config(config).getTreasury(), _feeRepay);
            art -= _feeRepay;
        }
