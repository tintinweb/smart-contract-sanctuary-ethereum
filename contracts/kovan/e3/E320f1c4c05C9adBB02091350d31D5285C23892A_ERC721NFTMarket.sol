...
contract ERC721NFTMarket is ERC721Holder, Ownable, ReentrancyGuard {
    ...
    constructor(
        ...
        require(_CSKAddress != address(0), "Operations: CSK address cannot be zero");
    ...
    /**
     * @notice Buy token with CSK by matching the price of an existing ask order
     * @param _collection: contract address of the NFT
     * @param _tokenId: tokenId of the NFT purchased
     * @param _price: price (must be equal to the askPrice set by the seller)
     */
    function buyTokenUsingCSK(
        address _collection,
        uint256 _tokenId,
        uint256 _price,
        string memory _hash
        ...
        IERC20(CSK).safeTransferFrom(address(msg.sender), address(this), amounts[0]);
        IERC20(CSK).approve(PancakeRouter, amounts[0]);
