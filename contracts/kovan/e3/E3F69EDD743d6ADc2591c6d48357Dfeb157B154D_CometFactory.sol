...
contract Comet is CometCore {
    ...
    constructor(Configuration memory config) {
        ...
        require(decimals_ <= MAX_BASE_DECIMALS, "");
        require(config.assetConfigs.length <= MAX_ASSETS, "");
        require(config.baseMinForRewards > 0, "");
        require(AggregatorV3Interface(config.baseTokenPriceFeed).decimals() == PRICE_FEED_DECIMALS, "");
    ...
    function initializeStorage() public {
        require(totalsBasic.lastAccrualTime == 0, "");
    ...
    function _getPackedAsset(AssetConfig[] memory assetConfigs, uint i) internal view returns (uint256, uint256) {
        ...
        require(AggregatorV3Interface(priceFeed).decimals() == PRICE_FEED_DECIMALS, "");
        require(ERC20(asset).decimals() == decimals_, "");

        // Ensure collateral factors are within range
        require(assetConfig.borrowCollateralFactor < assetConfig.liquidateCollateralFactor, "");
        require(assetConfig.liquidateCollateralFactor <= MAX_COLLATERAL_FACTOR, "");
        ...
        require(borrowCollateralFactor < liquidateCollateralFactor, "");
