...
contract StakedPurchase {
  ...
  /**
   * @dev Throws if Asset ID is valid
   * @param _assetId Asset ID
   */
  modifier validAsset(uint256 _assetId) {
    require(maxAssetId >= _assetId, "Asset ID is invalid");
    _;
  }
   ...
   * @return Stake The amount staked and the time since when it's staked.
   */
  function allIntendedStakesOf(uint256 _assetId)
    public
    view
    validAsset(_assetId)
    returns (uint256)
  ...
  function publishAsset(
    ...
    require(_price > 0, "price must be larger than 0");
   ...
   * @return Asset
   */
  function assetOfId(uint256 _assetId) external view validAsset(_assetId) returns (Asset memory) {
   ...
   * @param _assetId Asset ID
   */
  function stake(uint256 _assetId) external payable validAsset(_assetId) {
   ...
   */
  function addIntendedStakeholder(uint256 _assetId, address payable _stakeholder)
    internal
    validAsset(_assetId)
  {
   ...
   */
  function finalize(uint256 _assetId)
    external
    onlySeller(_assetId)
    validIntendedStakes(_assetId)
    validAsset(_assetId)
  {
    ...
    for (uint256 i = 0; i < intendedStakeholders[_assetId].length; i++) {
      ...
      if (remainingStakes > stake.amount) {
        ...
        remainingStakes = remainingStakes.sub(stake.amount);
  ...
  function getAllIntendedStakes(uint256 _assetId)
    internal
    view
    validAsset(_assetId)
    returns (uint256)
