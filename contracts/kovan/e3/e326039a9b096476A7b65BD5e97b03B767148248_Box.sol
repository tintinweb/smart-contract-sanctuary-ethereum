...
contract Box is Initializable, ERC721AUpgradeable, ERC721ABurnableUpgradeable, OwnableUpgradeable{
    ...
    event minted(string _str,address _address, uint256 _quantity);
    event boxBurn(address _address, uint256 _quantity);
    event balWidthdraw(uint256 _amount); 
    ...
    function mint(uint256 _quantity) external payable callerIsUser{
        ...
        // emit event
        emit minted("publicMinted",msg.sender, _quantity);
    ...
    function burnBatch(uint256[] memory tokenIds)  external callerIsUser{
        ...
        require(jack_address != address(0), "BOX :: Jack address is not set yet");
        require((balanceOf(msg.sender)) >= (_quantity), "Box :: Cannot burned beyond minted tokens");
        for (uint256 i = 0; i < _quantity; i++) {
            _burn(tokenIds[i], true);
        }
        emit boxBurn(msg.sender, _quantity);
        mintJack(_quantity);
    }
    ...
    function withdraw() external onlyOwner{
        emit balWidthdraw(address(this).balance);
...
contract Jack is Initializable, ERC721AUpgradeable, OwnableUpgradeable{
    ...
    function mint(address addr,uint256 _quantity) external callerIsBox{
        require(box_address != address(0), "Jack :: Box address is not set yet");
