...
contract FamNFT is ERC721, Ownable {
    ...
    constructor(address _authorizer) ERC721("FamNFT", "FCT") {
        authorizer = _authorizer;
    }

    function safeMint(uint256 nonce, bytes memory signature) public {
        require(!usedNonces[nonce], "You cannot mint it twice.");
        usedNonces[nonce] = true;

        address recoveredAddr = recoverAuthorizer(msg.sender, nonce, signature);
    ...
    function recoverAuthorizer(address recipient, uint256 nonce, bytes memory signature) 
        public 
        view 
        returns(address)
    {
        bytes32 message = prefixed(keccak256(abi.encodePacked(recipient, nonce, this)));
