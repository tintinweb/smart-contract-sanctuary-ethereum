...
contract Challenger {
    ...
    function submitData(
        ...
        if (manager.hasUnlockedChallenge(challengeId, msg.sender) == true) {
            require(
            ...
            uint256 keyPrice = manager.getKeyPrice(challengeId);
            uint256 gymnasiaFee = msg.value - keyPrice;

            manager.addToChallengePot{value: keyPrice}(challengeId);
...
contract ChallengeManager is Ownable {
    ...
    mapping(uint256 => uint256) public challengePot;
    ...
    function addToChallengePot(uint256 id) external payable onlyChallenger {
        require(
            msg.value >= getKeyPrice(id),
            "ChallengeManager: entered Challengefee too low"
        );
        challengePot[id] += (msg.value);
    }

    function withdraw(uint256 id, address winner) public onlyChallenger {
        bool sent = payable(winner).send(challengePot[id]);
