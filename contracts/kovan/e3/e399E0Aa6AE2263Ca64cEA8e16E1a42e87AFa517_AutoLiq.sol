...
//    FlashLoanReceiverBase()
    ...
    function beginLiquidate(uint repayAmount,address _borrower, address aave_tokenBorrow, address _tokenBorrow, address _cTokenBorrow, address _cTokenCollateral)
    public
    onlyOwner
    {

        IERC20(aave_tokenBorrow).transferFrom(msg.sender, address(this), repayAmount);



        bytes memory by = abi.encode(_borrower, _tokenBorrow, _cTokenBorrow, _cTokenCollateral);


        flashLoan(by, aave_tokenBorrow, repayAmount);
    ...
    function flashLoan(bytes memory by, address _tokenBorrow, uint _repayAmount) internal {
    ...
    function executeOperation(
        ...
        (address _borrower, address _tokenBorrow,address _cTokenBorrow, address _cTokenCollateral) = abi.decode(params,(address, address, address, address));
        ...
        liquidate(amounts[0], _borrower, _tokenBorrow, _cTokenBorrow, _cTokenCollateral);

        uint collateralAmount = CErc20(_cTokenCollateral).balanceOf(address(this));
        emit Log("collateralAmount", collateralAmount);

        this.swapExactInputSingle(_cTokenCollateral, _tokenBorrow, collateralAmount);
...
//        uint tokenBalance = IERC20(tokenBorrow).balanceOf(address(this));
//        //        CErc20(_cTokenCollateral).redeem(amount);
//        uint transferAmount = tokenBalance - amountOwing;
//
//        bool success = IERC20(tokenBorrow).transfer(tx.origin, transferAmount);
//
//        require(
//            success == true,
//            "transfer failed"
//        );
    ...
    function liquidate(
        ...
        address _cTokenBorrow,
        address _cTokenCollateral
    ) onlyOwner internal returns (uint code){

        IERC20(_tokenBorrow).transferFrom(tx.origin, address(this), _repayAmount);
        //授权cTokenBorrow可划转tokenBorrow的额度为repayAmount
        IERC20(_tokenBorrow).approve(_cTokenBorrow, _repayAmount);

        code = CErc20(_cTokenBorrow).liquidateBorrow(_borrower, _repayAmount, _cTokenCollateral);
    ...
    function transferOut(address _cTokenCollateral) onlyOwner internal {
        uint amount = IERC20(_cTokenCollateral).balanceOf(address(this));
//        CErc20(_cTokenCollateral).redeem(amount);

        bool success = IERC20(_cTokenCollateral).transfer(tx.origin, amount);
    ...
    function swapExactInputSingle(address tokenIn, address tokenOut, uint256 amountIn) external returns (uint256 amountOut) {
        ...
        ISwapRouter.ExactInputParams memory params =
            ISwapRouter.ExactInputParams({
                path: abi.encodePacked(tokenIn, poolFee, 0xd0A1E359811322d97991E03f863a0C30C2cF029C, poolFee, tokenOut),
...
//
        // Executes the swap.
        try swapRouter.exactInput(params) returns (uint amount){
            emit Log("swapAmount", amount);
            return (amount);
        }catch Error(string memory reason) {
            emit Log("swapAmounterror", reason);
            return (0);
        } catch (bytes memory lowLevelData) {
            // This is executed in case revert() was used。
            emit Log("swapAmounterrorlow", string(lowLevelData));
            return (2);
        }
