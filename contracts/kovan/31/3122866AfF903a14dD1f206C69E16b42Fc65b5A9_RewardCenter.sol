...
contract RewardCenter is Ownable {
    ...
    event ShowMerkel1(uint256 round,uint256 amount, bytes32 root, bytes32 leaf);
    event ShowMerkel2(uint256 round,uint256 amount, bytes32 root, bytes32 leaf);
    ...
    function claimPositionReward(
        ...
        emit ShowMerkel1(epochRound,amount,merkelRoots[epochRound],keccak256(leafData));
        emit ShowMerkel2(epochRound,amount,merkelRoots[epochRound],sha256(leafData));  
        require(MerkleProof.verify(proof,merkelRoots[epochRound],keccak256(leafData)) == true ,"CHFRY: MerkleProof Fail");
