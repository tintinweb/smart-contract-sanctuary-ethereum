...
contract RouterV1 {
    ...
    function previewTranche(IPerpetualTranche perp, uint256 collateralAmount)
        external
        returns (IBondController bond, ITranche[] memory tranches, uint256[] memory trancheAmts)
    {
    ...
    {
        require(perp.getDepositBond() == IBondController(trancheIn.bond()), "Expected to tranche deposit bond");
        ...
        )
    {
        ...
        r.remainingTrancheOutBalance = trancheOut.balanceOf(perp.reserve());

        uint256 rolloverAmtRemainder;
        (r.trancheOutAmt, rolloverAmtRemainder) = perp.perpsToCoveredTranches(trancheOut, r.requestedRolloverAmt);
