...
contract ERC20Upgradeable is Initializable, ContextUpgradeable, IERC20Upgradeable, OwnableUpgradeable {
    ...
    uint256 private _minBuy = 100; // 1 euro, change to 10000 in main network
    ...
    function getRate() public pure returns (int256) {
        return 300000000000;
    ...
    receive() external payable {
        address admin = ITrustate(owner()).owner();
        if (_msgSender() == admin) {
            ...
            require(tokenAmount <= balanceOf(address(this)), "Insufficient token amount on contract.");
...
contract Trustate is Ownable {
    ...
    function claim(uint256 _id) external {
        require(tokenContracts[_id] != address(0), "Invalid project ID");
        uint256 tokenBalance = ITokenContract(tokenContracts[_id]).balanceOf(_msgSender());
        ...
        ITokenContract(tokenContracts[_id]).claim(_msgSender());
    }
