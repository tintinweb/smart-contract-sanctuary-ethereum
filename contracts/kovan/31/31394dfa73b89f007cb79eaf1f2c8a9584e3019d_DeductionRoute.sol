...
// File: my/new_cow/DeductionRoute.sol
...
contract DeductionRoute is EIP712,ReentrancyGuard,AdminTransfer {
    address public signerAddress;
    IUnifiedApprove public unifiedApprove;
    mapping(uint=>bool) public oidUsed;

    event Deduction(uint indexed oid,uint biz,address coin,uint amount,address pay_user);

    constructor(address _signerAddress,IUnifiedApprove _unifiedApprove) EIP712("DeductionRoute", "1") {
        signerAddress=_signerAddress;
        unifiedApprove=_unifiedApprove;
    }

    function deduction(uint256 deadline,uint256 oid,uint256 biz,address coin,uint256 amount,bytes calldata signature) external {
        require(block.timestamp <= deadline, "expired");
        require(check(signerAddress,hashData(deadline,oid,biz,coin,amount),signature), "sign error");
        require(!oidUsed[oid],"already submit");
        oidUsed[oid]=true;
        unifiedApprove.transferCoin(IERC20(coin),msg.sender,address(this),amount);
        emit Deduction(oid,biz,coin,amount,msg.sender);
    }
    ...
    function hashData(uint256 deadline,uint256 oid,uint256 biz,address coin,uint256 amount) public view returns (bytes32) {
        return _hashTypedDataV4(keccak256(abi.encode(
            keccak256("DeductionRouteRequest(uint256 deadline,uint256 oid,uint256 biz,address coin,uint256 amount)"),
            deadline,
            oid,
            biz,
            coin,
