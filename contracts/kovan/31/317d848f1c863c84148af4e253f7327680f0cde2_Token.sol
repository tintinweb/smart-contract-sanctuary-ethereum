...
// SPDX-License-Identifier: MIT
 ...
 contract Token {
     ...
     mapping(address => mapping(address => uint256)) public allowance;          // keeps track of our account, as well as other addresses that are allowed to spend our CRYE

     // Events - fire events on state changes etc
     event Transfer(address indexed from, address indexed to, uint256 value);                   // from owner, debit _to, credit value
     ...
     function approve(address _spender, uint256 _value) external returns (bool) {       // to let a certain address to spend the crypto, hence the approve function
     ...
     // Allow _spender to spend up to _value on your behalf
     function transferFrom(address _from, address _to, uint256 _value) external returns (bool) {        // this function is for DEX to read. basically to help us transfer FROM, TO address, VALUE of token
