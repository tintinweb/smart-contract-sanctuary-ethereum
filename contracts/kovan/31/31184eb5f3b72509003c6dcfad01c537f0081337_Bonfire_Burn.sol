...
contract Bonfire_Burn is ERC721A, Ownable {
    bytes32 public root;
    ...
    function RAND_BURN() public {
        //Check for fully minted
        require(_totalMinted() >= maxSupply, "Cannot burn. Total supply not fully minted.");
        //Randomness Burn
        uint randomHash = uint(keccak256(abi.encodePacked(msg.sender, block.timestamp)));
        uint256 tokenId = randomHash % _totalMinted();
        
        _burn(tokenId);
    }

    /*function RAND_BURN() internal virtual {
        bool approvalCheck = false;
        //Randomness Mint
        uint randomHash = uint(keccak256(abi.encodePacked(msg.sender, block.timestamp)));
        uint256 tokenId = randomHash % 10000;
        ...
        // Clear approvals from the previous owner
        approve(address(0), tokenId, from);
