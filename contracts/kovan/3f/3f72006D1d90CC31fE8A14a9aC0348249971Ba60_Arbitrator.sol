...
contract Arbitrator is IArbitrator {
    ...
    error NotOwner(address expected, address found);
    ...
    modifier requireArbitrationFee(bytes memory _extraData) {
        if(msg.value < arbitrationCost(_extraData)){
            revert InsufficientPayment(msg.value,arbitrationCost(_extraData));
        }
        _;
    }
    ...
    modifier onlyEscrow(){
        ...
        {
            revert NotOwner(address(escrow), msg.sender);
        }
        _;
    } 

    modifier requireAppealFee(uint _disputeID, bytes memory _extraData) {
        if(msg.value < appealCost(_disputeID, _extraData)){
            revert InsufficientPayment(msg.value,appealCost(_disputeID, _extraData));
        }
        _;
    }
     ...
     */
    function createDispute(uint _choices, bytes memory _extraData) public override requireArbitrationFee(_extraData) onlyEscrow payable returns(uint disputeID) {
        disputes.push(
