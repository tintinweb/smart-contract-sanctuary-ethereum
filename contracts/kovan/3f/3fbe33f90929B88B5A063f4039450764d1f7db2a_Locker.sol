...
contract Locker is ReentrancyGuard {
  ...
  event LockAmountUpdated(uint256 amount);
  event UnlockedAmount(uint256 amount);
  event NextUnlockIndex(uint256 unlockIndex);
  event Relocking(address token, address user, uint256 unlocked, uint256 duration, uint256 nextUnlockIndex);
  event WithdrawAllWithPenalty(address token, address user, uint256 amount, uint256 penalty, uint256 nextUnlockIndex);
  ...
  function deposit(address token, address user, uint256 amount, uint256 duration) public {
    
    require(amount > 0, "Amount cannot be 0");
    require(duration > 0, "Duration cannot be 0");
    require(getBalance(user, token) >= amount, "Not enough tokens");

    IERC20(token).transferFrom(user, address(this), amount);
    lockedBalance[token][user].locked += amount;

    Deposit memory newDeposit = Deposit({
      id: deposits[token][user].length,
      user: user,
      token: token,
      amount: amount,
      duration: duration, 
    ...
    deposits[token][user].push(newDeposit);

  }

  function withdrawAllWithPenalty (address _token) public nonReentrant {
    ...
    for (uint i = nextUnlockIndex; i < _deposits.length; i++){
      ...
      withdrawalAmount += _deposit.amount;
      nextUnlockIndex++;
    }

    uint256 penalty = calculateExitPenalty(withdrawalAmount);

    lockedBalance[_token][msg.sender].locked = 0; // set locked back to 0 since we withdraw everything

    IERC20(_token).transfer(msg.sender, withdrawalAmount - penalty);

    emit WithdrawAllWithPenalty (_token, msg.sender, withdrawalAmount, penalty, nextUnlockIndex);
  }

  function withdrawAllUnlocked (address _token, bool _relock, uint256 duration) public nonReentrant {
    ...
    if (_relock) {
      emit Relocking(_token, msg.sender, unlocked, duration, nextUnlockIndex);
      deposit(_token, msg.sender, unlocked, duration);

      // calling deposit again during relock essentially doubles the users locked amount
      // so we subtract the previously unlocked amount to correct for that

      // TODO: see how CVX locker handles this
  ...
  function calculateExitPenalty (uint256 _amount) public view returns (uint256) {
    return _amount * earlyExitPenalty / 100; 
  }
  ...
  function withdrawWithPenalty (address _token, uint256 _amount, uint256 _depositId) public nonReentrant {
    ...
    uint256 penalty = calculateExitPenalty(_amount);
