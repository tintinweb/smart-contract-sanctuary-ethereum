...
import "RapidTokenUpgradeable.sol";
...
//    mapping(address => uint256) public stableCoinStaked;
    IERC20 public stableCoin;
//    IERC20Upgradeable public rapidBondzToken;
    RapidTokenUpgradeable public rapidBondzContract;
    ...
    constructor(address __stableCoin, address __rapidBondzContract) {
        stableCoin = IERC20(__stableCoin);
        rapidBondzContract = RapidTokenUpgradeable(__rapidBondzContract);
    }

//    function addStableCoin(
//        address __stableCoin,
//        mapping(address => uint32) storage __pledgedAmountsDict
//    ) private {
//        stableCoin = IERC20(__stableCoin);
//    }
//
//    function addRapidBondzToken(
//        address __rapidBondzToken
//    ) private {
//         rapidBondzToken = RapidTokenUpgradeable(__rapidBondzToken);
//    }
...
//        stableCoin.transferFrom(_investor, address(this), _amount);

        emit Pledge(msg.sender, _amount);
    }

//    function create_rapid_tokens() {
//        rapidBondzToken.mint(address(this), _amount);
//    }

//    event PledgedTotal(IssuingData _initialOffering);
    event PledgedTotal(uint32 pledged);

    function getPledgedAmount() public {
        emit PledgedTotal(initialOffering.pledged);
    }

    function retrieve() public view returns (uint256) {

        return (initialOffering.pledged);
    }
    ...
    function settlement() external {
        ...
        rapidBondzContract.mint(address(this), initialOffering.pledged);
...
import "DataStorageModule.sol";
import "EnforcementModule.sol";
import "PauseModule.sol";
import "ValidationModule.sol";
import "IRuleEngine.sol";


contract RapidTokenUpgradeable is Initializable, ERC20Upgradeable, ERC20BurnableUpgradeable,
AccessControlUpgradeable, ERC20PermitUpgradeable, UUPSUpgradeable, ERC20SnapshotUpgradeable, ValidationModule,
EnforcementModule, PauseModule, DataStorageModule {
//DataStorageModule, SnapshotModule {
...
//    address __rapidBondzToken;
    ...
    function initialize(
        BaseData memory __baseData,
        BondData memory __bondData,
        CalendarData memory __calendarData
//        address __initialOffering
//        address __stableCoin
//        address __rapidBondzToken

    ) initializer public {
        __DataStorage_init_unchained(
            __baseData, __bondData, __calendarData
        );
        __ERC20_init(__baseData.name, __baseData.symbol);
        __ERC20Burnable_init();
//        __Snapshot_init_unchained();
        __ERC20Snapshot_init();
        __AccessControl_init();
        __Pausable_init();
        __ERC20Permit_init(__baseData.name);
        __UUPSUpgradeable_init();

        _grantRole(DEFAULT_ADMIN_ROLE, msg.sender);
        _grantRole(SNAPSHOT_ROLE, msg.sender);
        _grantRole(PAUSER_ROLE, msg.sender);
//        _mint(msg.sender, __bondData.maximal_amount * 10 ** decimals());
//        _mint(msg.sender, __bondData.maximal_amount * 10 ** decimals());
//        _grantRole(MINTER_ROLE, msg.sender);
        _grantRole(UPGRADER_ROLE, msg.sender);
//        __InitialOffering_init_unchained(__stableCoin);
    }

    function assignMinterRole(address __initialOffering) public onlyRole(DEFAULT_ADMIN_ROLE) {
        _grantRole(MINTER_ROLE, __initialOffering);
    }

    // kill will destroy contract
    ...
    function snapshot() public onlyRole(SNAPSHOT_ROLE) {
        ...
        // todo extend snapshot module
//    function scheduleSnapshot(uint256 time) public onlyRole(SNAPSHOT_ROLE) returns (uint256) {
//        return _scheduleSnapshot(time);
//    }
//
//    function rescheduleSnapshot(uint256 oldTime, uint256 newTime) public onlyRole(SNAPSHOT_ROLE) returns (uint256) {
//        return _rescheduleSnapshot(oldTime, newTime);
//    }
//
//    function unscheduleSnapshot(uint256 time) public onlyRole(SNAPSHOT_ROLE) returns (uint256) {
...
//        require(!paused(), "RapidBondz: token transfer attempt while paused");
//        require(!frozen(from), "RapidBondz: token transfer attempt while frozen");
//
//        super._beforeTokenTransfer(from, to, amount);
//
//        if (address(ruleEngine) != address(0)) {
//            require(_validateTransfer(from, to, amount), "RapidBondz: transfer rejected by validation module");
//        }
//    }
...
abstract contract DataStorageModule is Initializable, ERC20Upgradeable {
    struct BaseData {
        string name;
        string symbol;
        string tokenId;
    ...
    struct BondData {
        string guarantor_identifier;
        string bondholder_representative_identifier;
        uint256 maturity_date;
        uint256 interest_rate;
        string currency;
        string interest_schedule_format;
        uint256 interest_payment_date;
        uint256 par_value;
    ...
    struct CalendarData {
        ...
        string business_day_convention;
        string public_holidays_calendar;
    }

    BaseData public baseData;
    BondData public bondData;
    CalendarData public calendarData;


    event IssuedBase(string _name, string _symbol, string _tokenId, string _terms);
    event IssuedBond(
        string _guarantor_identifier, string _bondholder_representative_identifier, uint256 _maturity_date,
        uint256 _interest_rate, string _currency, string _interest_schedule_format, uint256 _interest_payment_date,
        uint256 _par_value
    );

    event IssueCalendar(
        string _day_count_convention, string _business_day_convention, string _public_holidays_calendar
    );

    function _storeBaseData(
        BaseData memory baseData_
    ) public {
        baseData = baseData_;
        emit IssuedBase(baseData_.name, baseData_.symbol, baseData_.tokenId, baseData_.terms);
    }

    function _storeBondData(
        BondData memory bondData_
    ) public {
        bondData = bondData_;
        emit IssuedBond(bondData_.guarantor_identifier, bondData_.bondholder_representative_identifier,
            bondData_.maturity_date, bondData_.interest_rate, bondData_.currency, bondData_.interest_schedule_format,
            bondData_.interest_payment_date, bondData_.par_value);
    }

    function _storeCalendarData(
        CalendarData memory calendarData_
    ) public {
        calendarData = calendarData_;
        emit IssueCalendar(calendarData_.day_count_convention, calendarData_.business_day_convention, calendarData_.public_holidays_calendar);
    }

    function __DataStorage_init() internal onlyInitializing {
    }

    function __DataStorage_init_unchained(
        BaseData memory baseData__,
        BondData memory bondData__,
        CalendarData memory calendarData__
    ) internal onlyInitializing {
        _storeBaseData(baseData__);
        _storeBondData(bondData__);
        _storeCalendarData(calendarData__);
    }
}
