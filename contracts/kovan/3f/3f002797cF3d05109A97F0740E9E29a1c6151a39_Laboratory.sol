...
// File: contracts\interfaces\ILaboratory.sol
...
interface ILaboratory is ISeed {
     ...
     */
    function createNewMetaCell(address user, bytes calldata depositData)
        external;
...
// File: contracts\MetaCell.sol


pragma solidity ^0.8.0;
contract MetaCell is ERC721, ICellRepository, IAdmin, Ownable {
    using Counters for Counters.Counter;
        ...
        ERC721("MetaCell", "CLL")
    {
        ...
        _addAdmin(_childChainManagerProxy);
        deployer = _owner;
    ...
    function withdraw(uint256 tokenId) external {
        require(msg.sender == ownerOf(tokenId), "Invalid token owner");
        _burn(tokenId);
    }
    ...
    Ownable
{
    ...
    function _processSplit(CellData.Cell memory cell) private {
        if (CellData.Class(cell.class) == CellData.Class.SPLITTABLE_NANO) {
            uint256 id = IMintable(nanoCell).mint(msg.sender, 0);
            emit NewCellCreated("NanoCell", id);
        } else if (
    ...
    function mutate(uint256 _cellId, uint256 _nftId) external override {
        ...
        ICellRepository(metaCell).removeMetaCell(_cellId, msg.sender);
        ICellRepository(metaCell).addMetaCell(newCell);
    ...
    function getSeed() external view override isAdmin returns (uint256) {
    ...
    function createNewMetaCell(address user, bytes calldata depositData)
        ...
        onlyOwner
    {
        _mintMetaCell(user, depositData);
    }

    function _mintMetaCell(address _user, bytes calldata _depositData) private {
        MetaCell(metaCell).deposit(_user, _depositData);

        emit NewMetaCellCreated();
    }
    ...
    function getMetaCellByID(uint256 tokenId, address tokenOwner)
        ...
        returns (CellData.Cell memory)
    {
        return ICellRepository(metaCell).getMetaCell(tokenOwner, tokenId);
    }
}