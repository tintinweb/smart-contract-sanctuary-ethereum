...
contract IdenticalTokenOracleAdapter is AccessControl, IOracleAdapter {
    ...
    address[] internal _identicalTokens;

    // identical token => underlying token
    ...
    function getIdenticalTokens() external view returns (address[] memory) {
        return _identicalTokens;
    }

    function getIdenticalToken(uint256 index) external view returns (address) {
        return _identicalTokens[index];
    }
    ...
    function addPair(address identicalToken, address underlyingToken) external onlyAdmin {
        require(identicalToken != address(0), "IOA0a");
        require(underlyingToken != address(0), "IOA0b");
        require(getUnderlyingToken(identicalToken) == address(0), "IOA0c");
        require(getUnderlyingToken(underlyingToken) != identicalToken, "IOA0d"); // Prevent infinite loops when call getPrice

        _identicalTokens.push(identicalToken);
        _underlyingTokens[identicalToken] = underlyingToken;
    }

    function removePair(address identicalToken, address underlyingToken) external onlyAdmin {
        require(identicalToken != address(0), "IOA1a");
        require(getUnderlyingToken(identicalToken) == underlyingToken, "IOA1b");

        _identicalTokens.quickRemoveItem(identicalToken);
        delete _underlyingTokens[identicalToken];
    }

    // ==================== Public functions ====================

    function getUnderlyingToken(address identicalToken) public view returns (address) {
        return _underlyingTokens[identicalToken];
    }

    // ==================== Private functions ====================

    function _onlyAdmin() private view {
        require(hasRole(ADMIN_ROLE, _msgSender()), "IOA2");
    }
}
