...
interface IMarketplace {
    ...
    event OrderCancelled(bytes32 id, address seller);
...
contract GPSmarketplace is Ownable, Pausable, FeeManager, IMarketplace, ERC721Holder {
    ...
    mapping(uint256 => address[]) public contributers;

    event versionPublished(address  nftOwner, uint256 _assetID, uint256 nftVersionNO ,string  ipfshash);

    event LicenseBought(address buyer,address seller, address indexed _nftAddress, uint256 _assetId, uint256 _priceInWei, string ipfsTermsHash);

    event rewardDistributed(uint256 _amount,address _nftAddress, uint256 _assetId,address _contributer);
     ...
     */
    function createOrder(address _nftAddress, uint256 _assetId, uint256 _priceInWei, uint256 _expiresAt , string memory IpfsTermsHash, address userAddress) public whenNotPaused {
        
        _createOrder(_nftAddress, _assetId, _priceInWei, _expiresAt,IpfsTermsHash,userAddress);
    }

    //  function Contribute(address _contributer,address _nftAddress,uint256 _assetId,string memory _text) public whenNotPaused{
    //    suggestion = _text;
    //    contributers[_assetId].push(address(_contributer));
    //}

    function rewardContributer(uint256 _amount,address _nftAddress, uint256 _assetId,address _contributer) public whenNotPaused{
        
        acceptedToken.transferFrom(msg.sender,_contributer,_amount);

        emit rewardDistributed(_amount,_nftAddress,_assetId,_contributer);
    }
     ...
     * @param _assetId - ID of the published NFT
     */
    function cancelOrder(address _nftAddress, uint256 _assetId,address userAddress) public whenNotPaused {
        Order memory order = orderByAssetId[_nftAddress][_assetId][userAddress];
      ...
      //  if (bid.id != 0) {
       ...
       // }
        ...
        // Cancel order.
        _cancelOrder(order.id, _nftAddress, _assetId, msg.sender,userAddress);
    }
     ...
     * @param _assetId - ID of the published NFT
     */
    function updateOrder(address _nftAddress, uint256 _assetId, uint256 _priceInWei, uint256 _expiresAt , string memory IpfsTermsHash, address userAddress)
        public whenNotPaused {
        Order storage order = orderByAssetId[_nftAddress][_assetId][userAddress];
        ...
        emit OrderUpdated(order.id, _priceInWei, _expiresAt,IpfsTermsHash,userAddress);
    }

    function publishVersion(address _nftAddress, uint256 _assetId, uint256 versionNo, string memory ipfshasH,address userAddress) public  returns(address,uint256,uint256,string memory) {
             
         Order memory order = _getValidOrder(_nftAddress, _assetId,userAddress);
         require (order.seller == msg.sender, "GPS : only nft owner can publish");
         nftOwner = msg.sender;
         nftID = _assetId;
         nftVersionNO = versionNo;
         ipfshash = ipfshasH;
         emit versionPublished(_nftAddress,_assetId,versionNo,ipfshasH);
         return(nftOwner,nftID,nftVersionNO,ipfshash);
     ...
     */
    function BuyLicense(address _nftAddress, uint256 _assetId, uint256 _priceInWei , string memory _ipfsTermsHash,address userAddress) public  whenNotPaused {
        // Get the current valid order for the asset or fail
        Order memory order = _getValidOrder(_nftAddress, _assetId,userAddress);

        require(msg.sender == userAddress,"Only provided address can buy");
        ...
        LicenseHolders[_assetId].push((address(msg.sender))); 

        emit LicenseBought(msg.sender,order.seller,_nftAddress,_assetId,_priceInWei,_ipfsTermsHash);
     ...
     * @param _assetId - ID of the published NFT
     */
    function _getValidOrder(address _nftAddress, uint256 _assetId,address userAddress) internal view returns (Order memory order) {
        order = orderByAssetId[_nftAddress][_assetId][userAddress];
     ...
     */
    function _createOrder(address _nftAddress, uint256 _assetId, uint256 _priceInWei, uint256 _expiresAt , string memory _IpfsTermsHash ,address _userAddress) internal {
        // Check nft registry
       ...
       // nftRegistry.safeTransferFrom(assetOwner, address(this), _assetId);
        ...
        // save order
        orderByAssetId[_nftAddress][_assetId][_userAddress] = Order({
            id: orderId,
            seller: assetOwner,
            nftAddress: _nftAddress,
            price: _priceInWei,
            expiresAt: _expiresAt,
            ipfsTermsHash: _IpfsTermsHash,
            useraddress: _userAddress
        });

        LicenseOwner[_assetId] = msg.sender;

        emit OrderCreated(orderId, assetOwner, _nftAddress, _assetId, _priceInWei, _expiresAt , _IpfsTermsHash);
    }
     ...
     */
    function _cancelOrder(bytes32 _orderId, address _nftAddress, uint256 _assetId, address _seller,address userAddress) internal {
        delete orderByAssetId[_nftAddress][_assetId][userAddress];

        /// send asset back to seller
        //IERC721(_nftAddress).safeTransferFrom(address(this), _seller, _assetId);

        emit OrderCancelled(_orderId , _seller);
    }
