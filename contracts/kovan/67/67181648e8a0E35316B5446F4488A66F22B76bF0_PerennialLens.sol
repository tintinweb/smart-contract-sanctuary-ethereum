...
contract PerennialLens {
  ...
  function position(IProduct product) public settle(product) returns (Position memory) {
    uint256 latestVersion = product.latestVersion();
    return product.positionAtVersion(latestVersion);
  }

  function payoff(IProduct product) external settle(product) returns (Fixed18) {
    IOracle.OracleVersion memory currentVersion = product.provider().currentVersion();
    return product.provider().payoff(currentVersion).price;
  }

  function rate(IProduct product) external settle(product) returns (Fixed18) {
    Position memory position_ = position(product);
    return product.provider().rate(position_);
  }
