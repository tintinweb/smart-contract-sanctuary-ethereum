...
// File: contracts\ipcog\IPCOG.sol
...
contract IPCOG is Context, IIPCOG, Ownable {
    ...
    function burn(uint256 amount) external virtual override onlyBurner {
        _burn(_msgSender(), amount);
    }

    function burnFrom(address account, uint256 amount) external virtual override onlyBurner {
    ...
    function _processTransferIPCOG(
        address sender,
        address recipient, 
        uint256 amount
    ) internal {
        IPrecogStorage precogStorage = IPrecogV5(owner()).precogStorage();
        IPrecogInternal precogInternal = IPrecogV5(owner()).precogInternal();
        IWithdrawalRegister withdrawalRegiter = IWithdrawalRegister(precogStorage.getWithdrawalRegister());
        address token = precogStorage.getTokenConvert(address(this));
        require(
            !withdrawalRegiter.isInDeadline(token, sender) || 
            !withdrawalRegiter.isInDeadline(token, recipient), 
            "IPCOG: Can not transfer when sender or recipient is in deadline"
        );
        require(
            precogInternal.availableDepositedAmount(token, sender) == 0 || 
            precogInternal.availableDepositedAmount(token, recipient) == 0, 
            "IPCOG: Can not transfer when sender or recipient is not availble"
        );

        
        precogInternal.updateCurrentTradingCycle(token, false, 2**255);
        precogInternal.decreaseInvestment(token, sender, amount, uint48(block.timestamp));
        precogInternal.increaseInvestment(token, recipient, amount, uint48(block.timestamp));
    }
    ...
    function _transfer(
        ...
        _processTransferIPCOG(sender, recipient, amount);
...
// File: contracts\precog\IPCOGFactory.sol
