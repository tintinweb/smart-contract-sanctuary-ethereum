...
import "../interfaces/IFactVoting.sol";

contract FactMedia is ERC721, ERC721URIStorage, Ownable {
    IFactManager FactManager;
    IFactVoting FactVoting;

    uint256 public tokenIdCounter;

    constructor() ERC721("FactMedia", "FactMD") {}
    ...
    function setFactVotingContract(address _addressFactVoting)
        external
        onlyOwner
    {
        FactVoting = IFactVoting(_addressFactVoting);
    }

    function safeMint(string memory uri) public {
        require(
            msg.sender == address(FactVoting),
            "This function can call only Voting contract!"
        );
        _safeMint(address(FactManager), tokenIdCounter);
