...
contract UniswapFlash is 
    ...
    function initUniFlashSwap(
        ...
        uint24 fee = swapRouterInfos[UNISWAP_V3_ROUTER_ID].poolFee;
        flashPool = IUniswapV3Pool(PoolAddress.computeAddress(factory, poolKey));
        // address flashPoolAddr = IUniswapV3Factory(factory).getPool(
        //     loanAssets[0],
        //     loanAssets[1],
        //     fee
        // );
        // flashPool = IUniswapV3Pool(flashPoolAddr);
    ...
    function uniswapV3FlashCallback(
        ...
        loanAsset = decoded.amount0 > 0 ? decoded.poolKey.token0 : decoded.poolKey.token1;
        loanAmount = decoded.amount0 > 0 ? decoded.amount0 : decoded.amount1;
        uint256 fee = decoded.amount0 > 0 ? fee0 : fee1;
        tradeDexes = tradeDexs;
        emit AssetBorrowed(loanAsset, loanAmount, fee);

        // compute amount to pay back to pool 
        // (amount loaned) + fee
        uint256 amountOwed = LowGasSafeMath.add(loanAmount, fee);

        uniswapV2Router = IUniswapV2Router02(swapRouterInfos[tradeDexes[0]].router);
        uint256 amountOut = uniV2InputTokenForToken(
            Helpers.getPaths(loanAsset, tradeAssets[0]),
            loanAmount,
            0
        );
        uniswapV3Router = ISwapRouter(swapRouterInfos[tradeDexes[1]].router);
        amountOut = uniV3ExactInputSingle(
            Helpers.getPaths(tradeAssets[0], tradeAssets[1]),
            amountOut,
            0,
            swapRouterInfos[UNISWAP_V3_ROUTER_ID].poolFee
        );
        // uint256 amountOut = tradeExecute(loanAsset, loanAmount, tradeAssets, tradeDexs);
        emit AssetSwaped(tradeAssets[1], amountOut);
        ...
        if (amountOwed > 0) {
            ...
            emit RepayedAssetToPool(loanAsset, amountOwed);
        }
        ...
        if (amountOut > amountOwed) {
            ...
            emit TransferedProfitToWallet(loanAsset, profit);
        }
    }
