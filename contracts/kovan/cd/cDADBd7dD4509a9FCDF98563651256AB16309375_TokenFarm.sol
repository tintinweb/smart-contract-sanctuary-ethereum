...
contract TokenFarm is Ownable {
    ...
    constructor(address accTokenAddress) public {
        accToken = IERC20(accTokenAddress);
    }
    ...
    function issueTokens() public onlyOwner {
        for (
            ...
            uint256 totalUserValue = getUserTotalValue(recipent);
            accToken.transfer(recipent, totalUserValue);
        }
    }
    ...
    function setPriceFeedContract(address token, address tokenPriceFeed)
        public
        onlyOwner
    {
        tokenPriceFeedMapping[token] = tokenPriceFeed;
    }

    function stakeTokens(address token, uint256 amount) public {
    ...
    function tokenIsAllowed(address token) public view returns (bool) {
        for (
            ...
            if (token == allowedTokens[allowedTokensIndex]) {
                return true;
            }
        }
        return false;
