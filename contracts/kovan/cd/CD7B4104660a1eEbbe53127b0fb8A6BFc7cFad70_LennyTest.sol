...
contract LennyTest is ERC721A, Ownable {
    ...
    modifier mintConditions(uint256 count) {
        ...
        require(count > 0 && count <= maxMintPerTx, "Invalid mint amount!");
        require(totalSupply() + count <= maxSupply, "Max supply exceeded!");
        require(mintPrice(count) == msg.value, "Wrong ethers value");
        _;
    }

    function mintPrice(uint256 count) public view returns (uint256) {
        uint256 freeMints = defaultFreeMints;
        uint256 minted = mintedNFTs[_msgSender()];
        uint256 remainingFreeMints = freeMints > minted
            ? freeMints - minted
            : 0;
        return
            remainingFreeMints >= count
                ? 0
                : (count - remainingFreeMints) * cost;
    }

    function mint(uint256 count) public payable mintConditions(count) {
        require(!paused, "The contract is paused!");
        mintedNFTs[msg.sender] += count;
