...
interface IMoltlyPriceFeed {
    function wETH() external view returns (address);

    function getPrice(address) external view returns (int256, uint8);
}

contract MoltlyMarketplace is OwnableUpgradeable, ReentrancyGuardUpgradeable {
    using SafeMath for uint256;
    ...
    function createOffer(
        ...
        IMoltlyAuction auction = IMoltlyAuction(addressRegistry.auction());
    ...
    function registerRoyalty(
        ...
        require(_isMoltlyNFT(_nftAddress), "invalid nft address");
    ...
    function _isMoltlyNFT(address _nftAddress) internal view returns (bool) {
        return
            addressRegistry.moltly() == _nftAddress ||
            IMoltlyNFTFactory(addressRegistry.factory()).exists(_nftAddress) ||
            IMoltlyNFTFactory(addressRegistry.privateFactory()).exists(
                _nftAddress
            ) ||
            IMoltlyNFTFactory(addressRegistry.artFactory()).exists(
                _nftAddress
            ) ||
            IMoltlyNFTFactory(addressRegistry.privateArtFactory()).exists(
                _nftAddress
            );
    }
    ...
    function getPrice(address _payToken) public view returns (int256) {
        int256 unitPrice;
        uint8 decimals;
        IMoltlyPriceFeed priceFeed = IMoltlyPriceFeed(
