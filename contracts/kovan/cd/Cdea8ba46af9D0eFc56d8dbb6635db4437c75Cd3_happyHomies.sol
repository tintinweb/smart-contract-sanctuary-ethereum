...
contract happyHomies is ERC721A, Ownable, ReentrancyGuard {
    ...
    // Constructor //
    constructor() ERC721A("Happy Homies", "HH") {
        ...
        nonReentrant
    {
        ...
        require(
            publicsaleAddressMinted[msg.sender] + quantity <= maxMint,
            "Can only mint 2 per wallet"
        );
        require(
        ...
        nonReentrant
    {
        require(
            ...
            "Address is not on 1 MINT Allowlist"
        );
        require(price * quantity == msg.value, "Wrong amout of ETH sent");
        require(
            _tokensMintedByAddress[msg.sender] + quantity == maxPresaleMint,
            "Can only mint 1 token during PreSale"
        );
        require(
            totalSupply() + quantity < MAX_SUPPLY,
            "Can not mint more than max supply"
    ...
    function mintContest(uint256 quantity, bytes32[] calldata proof)
        external
        payable
        onlyPresaleActive
        nonReentrant
    {
        require(
            ...
            "Address is not on 2 MINT Allowlist"
        );
        require(price * quantity == msg.value, "Wrong amout of ETH sent");
        require(
            _tokensMintedByAddress[msg.sender] + quantity <= maxContestMint,
