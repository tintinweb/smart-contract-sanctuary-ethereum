...
contract NFTContract is ERC721, ERC721URIStorage, Ownable {
    ...
    uint256 public publicSale_mintPrice; // initail price to mint NFT
    uint256 public publicSale_time; // initail price to mint NFT
    uint256 public preSale_mintPrice; // initail price to mint NFT
    ...
    event payment_Sent(bool payment_Sent);
     ...
     constructor( ) ERC721("Demo","Dem")
     {
         ...
         setMaxSupply(10);
         set_PublicSaleMintPrice( 1 ether );
         set_PreSaleMintPrice( 0 ether );
         set_preSale_time( block.timestamp + 60 );
         set_publicSale_time( block.timestamp + 120 );
     }
    ...
    function set_PublicSaleMintPrice(uint _price) public onlyAdmin {
        publicSale_mintPrice = _price;   
    }

    function set_PreSaleMintPrice(uint _price) public onlyAdmin {
        preSale_mintPrice = _price;   
    }
    
    function set_preSale_time(uint _time) public onlyAdmin {
        preSale_time = _time;   
    }

    function set_publicSale_time(uint256 _time) public onlyAdmin {
        publicSale_time = _time;
    }
    ...
    {
        require(block.timestamp >= preSale_time, "Sale is not on yet");
        ...
        if (block.timestamp>=preSale_time)
            mintPrice = preSale_mintPrice;

        if (block.timestamp>=publicSale_time)
            mintPrice = publicSale_mintPrice;
