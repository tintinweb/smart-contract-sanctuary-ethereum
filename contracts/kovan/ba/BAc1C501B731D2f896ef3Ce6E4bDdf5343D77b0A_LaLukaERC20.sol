...
contract LaLukaERC20 is ERC20PausableUpgradeable, IBurnableMintableERC677Token, Blacklistable, Rescuable {
    ...
    event Beneficiary(address indexed newBeficiary);
    event Fee(uint32 newFee);

    modifier onlyOwnerOrBridge() {
        require( (bridgeContractAddr == _msgSender() || owner() == _msgSender() ), "Caller is not owner or the bridge");
        _;
    }
    ...
    function initialize(address beneficiary_, uint32 fee_) public initializer {
        __ERC20_init("La Luka COP", "COPL", beneficiary_, fee_);
        __Ownable_init();
    }
     ...
     */
    function setBeneficiary(address newBeficiary) public onlyOwner {
        _beneficiary = newBeficiary;
        emit Beneficiary(newBeficiary);
    }

    /**
     * @dev Set fee
     */
    function setFee(uint32 newFee) public onlyOwner {
        _fee = newFee;
        emit Fee(newFee);
    }
...
contract ERC20Upgradeable  is Initializable, ContextUpgradeable, IERC20Upgradeable, IERC20MetadataUpgradeable {
    ...
    uint32 public _fee; // max value 4,294,967,295 or 42,949,672.95 COPL
     ...
     * construction.
     */
    function __ERC20_init(string memory name_, string memory symbol_, address beneficiary_, uint32 fee_) internal initializer {
        __Context_init_unchained();
        __ERC20_init_unchained(name_, symbol_, beneficiary_, fee_);
    }

    function __ERC20_init_unchained(string memory name_, string memory symbol_, address beneficiary_, uint32 fee_) internal initializer {
        _name = name_;
        _symbol = symbol_;
    ...
    function _transfer(
        ...
        uint256 transferAmount = amount - _fee;
        ...
        _balances[_beneficiary] += _fee;

        emit Transfer(sender, recipient, transferAmount);
        emit Transfer(sender, _beneficiary, _fee);
...
import { OwnableUpgradeable } from "./Ownable.sol";
...
contract Rescuable is OwnableUpgradeable {
    ...
    function rescueEther(address payable to) external onlyOwner {
        require(to != address(0), "LaLuka: transfer to the zero address");
