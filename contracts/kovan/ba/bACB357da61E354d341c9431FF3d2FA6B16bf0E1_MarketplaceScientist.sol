...
// File: contracts\interfaces\IMarketPlaceScientist.sol


pragma solidity ^0.8.3;
interface IMarketPlaceScientist {
...
// File: contracts\MarketplaceScientist.sol
...
contract MarketplaceScientist is IAdmin, Ownable, IMarketPlaceScientist {
     ...
     * @param id of the scientist sold
     */
    event ScientistSold(uint256 id);

    /**
     * @dev Event emits when user successful added scientist to marketplace
     * @param _tokenId id of the scientist
     */
    event ScientistAddedToMarketplace(uint256 _tokenId);

    /**
     * @dev Event emits when user successful added scientist to marketplace
     * @param _tokenId id of the scientist
     */
    event ScientistRemovedFromMarketPlace(uint256 _tokenId);
        ...
        override
    {
        ...
        ScientistData.Scientist memory scientist = IScientistRepository(scientistTokenAddress)
            .getScientist(_oldOwner, _tokenId);

        require(msg.value == scientist.price, "Incorrect price");

        IScientistRepository(scientistTokenAddress).removeScientist(_tokenId, _oldOwner);
        ...
        IScientistRepository(scientistTokenAddress).addScientist(scientist);
    ...
    function updateScientistPrice(uint _tokenId, uint _newPrice) external override {
        ScientistData.Scientist memory scientist = IScientistRepository(scientistTokenAddress)
            .getScientist(msg.sender, _tokenId);
    ...
    function removeScientistFromSale(uint256 _tokenId) external override {
        ...
        emit ScientistRemovedFromMarketPlace(_tokenId);
    }
    ...
    function sellScientist(uint _tokenId, uint _price) external override {
        ...
        emit ScientistAddedToMarketplace(_tokenId);
    }
