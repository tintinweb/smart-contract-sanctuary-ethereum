...
contract Uchu {
    ...
    */
    ...
    function withdraw(uint amont) public returns(uint) {
        require(balanceOf[msg.sender][curLenderPro.name] >= amont);
        allowance[curLenderPro.name] -= amont;
        return balanceOf[msg.sender][curLenderPro.name] -= amont;
    }
}