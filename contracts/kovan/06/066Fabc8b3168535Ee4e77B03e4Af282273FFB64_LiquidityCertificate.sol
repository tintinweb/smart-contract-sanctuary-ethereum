...
// File: LiquidityCertificate.sol

contract LiquidityCertificate is ERC721, AccessControl, Ownable {

    bytes32 public constant LIQUIDITY_POOL_ROLE = keccak256("LIQUIDITY_POOL");
    ...
    mapping(uint256 => CertificateData) internal _certificateData;

    constructor(string memory _name, string memory _symbol) ERC721(_name, _symbol) public {
        _setupRole(DEFAULT_ADMIN_ROLE, msg.sender);
        _setRoleAdmin(LIQUIDITY_POOL_ROLE, DEFAULT_ADMIN_ROLE);
    }

    modifier onlyLiquidityPool {
        require(hasRole(LIQUIDITY_POOL_ROLE, msg.sender), "only LiquidityPool");
        _;
    }

    function addLiquidityPoolRole(address liquidityPool) public virtual onlyOwner {
        grantRole(LIQUIDITY_POOL_ROLE, liquidityPool);
    }

    function certificates(address owner) external view returns (uint256[] memory) {
    ...
    */
    function certificateData(uint256 certificateId)
        external
        view
        returns (address liquidityPool, uint256 roundId, uint256 liquidityAmount, uint256 enteredAt, uint256 burnableAt)
    {
        require(_certificateData[certificateId].liquidityAmount != 0, "certificate does not exist");
        return (
            _certificateData[certificateId].liquidityPool,
            _certificateData[certificateId].roundId,
            _certificateData[certificateId].liquidityAmount,
            _certificateData[certificateId].enteredAt,
            _certificateData[certificateId].burnableAt
        );
    }
    ...
    function mint(
        ...
        uint256 enteredAt,
        uint256 burnableAt
    ) external onlyLiquidityPool returns (uint256) {
        // require(liquidityAmount >= MIN_LIQUIDITY, "liquidity value of certificate must be >= 1");

        uint256 certificateId = nextId++;
        _certificateData[certificateId] = CertificateData(msg.sender, roundId, liquidityAmount, enteredAt, burnableAt);
        _mint(owner, certificateId);

        emit CertificateDataModified(certificateId, msg.sender, roundId, liquidityAmount, enteredAt, burnableAt);
    ...
    function burn(address spender, uint256 certificateId) external onlyLiquidityPool {
    ...
    */
    event CertificateDataModified(uint256 indexed certificateId, address liquidityPool, uint256 roundId, uint256 liquidityAmount, uint256 enteredAt, uint256 burnableAt);
