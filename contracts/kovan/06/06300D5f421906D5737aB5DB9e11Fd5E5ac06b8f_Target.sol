...
interface IXApp {
	...
	event XAppRegistered(uint32 indexed _chainDomain, address _xAppContract);
	event XCallSent(
		uint32 indexed _destinationDomain,
		address indexed _destinationContract,
		bytes payload
	);
...
contract XApp is IXApp, Ownable {
	...
	//Chain Domain -> XApp
	mapping(uint32 => address) internal xCallApps;
	mapping(uint32 => uint256) internal nonces;
	mapping(uint32 => mapping(uint256 => FailedCall)) internal failedXCalls;
	...
	function xCallReceived(
		...
		if (!success) {
			...
			failedXCalls[_domain][nonce] = FailedCall(
				_to,
				_payloadWithSignature
			);
		...
		emit XCallSent(_domain, _to, _payloadWithSignature);
	}
...
contract Target is Ownable {
	...
	uint256 public raceValue;

	function changeUselessValue(uint256 _value) external {
		uselessValue = _value;
	}

	function changeRaceValue(uint256 _value) external {
		raceValue = _value;
	}
