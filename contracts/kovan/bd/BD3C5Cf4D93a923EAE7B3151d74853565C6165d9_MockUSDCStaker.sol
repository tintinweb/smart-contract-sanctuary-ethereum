...
contract MockUSDCStaker is IMockUSDCStaker, Ownable{
    ...
    function getTotalUSDCDeposited() external view override returns(uint) {
        return usdc_token.balanceOf(address(this));
    }

    function transferUSDC(uint _amount, address _to) external override returns (uint) {
        bool success = usdc_token.transfer(_to, _amount);
        require( success , "MockUSDCStaker: USDC transfer failed");
        return _amount;
    }
}
