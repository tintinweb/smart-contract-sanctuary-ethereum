...
contract ImpossibleStaking is Ownable {
    ...
    function getMultiplier(uint256 _from, uint256 _to)
        ...
        returns (uint256)
    {
        uint256 _endTimestamp = endTimestamp; // gas savings
        if (_from >= _endTimestamp) {
            return 0;
        } else {
            return
                _to >= _endTimestamp
                    ? _endTimestamp.sub(_from)
                    : _to.sub(_from);
        }
    }
    ...
    function removeIFBal(uint256 amount) external onlyOwner {
        require(
            block.timestamp > endTimestamp,
            'only can withdraw IF after endTimestamp'
        );
    ...
    function emergencyTokenRetrieve(address token) external onlyOwner {
        ...
        for (i = 0; i < poolInfo.length; i++) {
            require(
                token != address(poolInfo[i].lpToken),
                'Cannot withdraw LP tokens'
            );
        }
