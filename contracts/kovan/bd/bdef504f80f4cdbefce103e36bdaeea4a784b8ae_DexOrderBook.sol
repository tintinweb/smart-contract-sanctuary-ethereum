...
contract DexOrderBook is OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    // user =>  OrderBook
    mapping(address => OrderBook) public orders;
    address private _serverKey;
    ...
    modifier isAuthorized() {
        require(msg.sender == _serverKey, "Unauthorized Access!");
        _;
    }
    ...
    function placeOrder(
        ...
        require(_baseToken != address(0), "Base token must be valid");
        require(_quoteToken != address(0), "Quote token must be valid");
        require(msg.value >= orderFee, "Insufficient fee");

        (bool isPaid, ) = payable(_serverKey).call{value: msg.value}("");
        ...
        Order memory _orderDetail = Order({
            price: _price,
            amount: _amount,
            orderType: _isLimitOrder ? "LIMIT" : "MARKET",
    ...
    function cancelOrder(uint256 _orderId) external nonReentrant {
        ...
        IERC20Upgradeable(_currentOrder.baseToken).transferFrom(
            address(this),
            msg.sender,
    ...
    // ================================= Internal Functions =================================
    function proceedOrder(uint256 _orderId) internal isAuthorized {
        Order storage _currentOrder = orders[msg.sender].orders[_orderId];
