...
// TODO [start, stop] or [start,stop) or how?
// TODO сделать админа проектов, а не общего (кто проводит транзу создания владелец платформы или проекта)
...
contract Totopad is AccessControl, ReentrancyGuard {
    ...
    function fundPledge(uint256 _pid) external nonReentrant {
        ...
        require(getPledgeFundingAmount(_pid, _msgSender()) > 0, "Cant fund");

        uint256 fundingAmount = getPledgeFundingAmount(_pid, _msgSender());

        user.fundingAmount = fundingAmount;
        project.totalRaised += fundingAmount;

        fundingToken.safeTransferFrom( _msgSender(), address(this), fundingAmount);
        stakingToken.safeTransfer(_msgSender(), user.amount);

        emit PledgeFunded(_msgSender(), _pid, fundingAmount);
    }
     ...
     * @param _pid pool id
     */
    function withdraw(uint256 _pid) external nonReentrant { // TODO check delete calls
        ...
        withdrawToken.safeTransfer(_msgSender(), withdrawAmount);
    ...
    function withdrawReward(uint256 _pid) external nonReentrant onlyRole(DEFAULT_ADMIN_ROLE) {
        ...
        project.rewardToken.safeTransfer(_msgSender(), rewardAmount);

        emit RewardWithdraw(_msgSender(), _pid, rewardAmount);
    }
    ...
    function claimReward(uint256 _pid) external nonReentrant {
        ...
        uint256 pending = pendingRewards(_pid, _msgSender());
        
        if (pending > 0) {
            user.rewardDebt += pending;

            project.rewardToken.safeTransfer(_msgSender(), pending);
        }
     ...
     * @param _pid pool id
     */
    function claimFundRaising(uint256 _pid) external nonReentrant onlyRole(DEFAULT_ADMIN_ROLE) {
        require(_pid < poolInfo.length, "Invalid pid");
        ...
        emit FundClaimed(_msgSender(), _pid, project.totalRaised);
    }
    ...
    function getPledgeFundingAmount(uint256 _pid, address _user) public view returns (uint256) {
        ...
        return userPercentAlloc * pool.targetRaise / TOTAL_TOKEN_ALLOCATION_POINTS;
    }
    ...
    function pendingRewards(uint256 _pid, address _user) public view returns (uint256) {
        ...
        return pool.rewardAmount * user.fundingAmount * timePass / rewardDuration / pool.targetRaise - user.rewardDebt;
    }
