...
contract Uchu is Initializable {
    ...
    */
    ...
    function getProducts() public returns (Product[] memory) {
        for(uint8 i=0; i<product_list.length; i++) {
            product_list[i].debt = setProductDebt(i, 0, 0, false);
        }
        return product_list;
    }

    function getProduct(uint8 productId) public returns (Product memory) {
    ...
    **/
    function lenderShare(address user, uint8 productId, uint index, uint deposit, uint withdraw, bool isIn) private {
        ...
        if (product.debt == 0) {
            ...
            position_map[user][index].share += (deposit - withdraw) / lendingPrice(productId);
        }
    }
    ...
    **/
    function borrowerShare(address user, uint8 productId, uint index, uint borrow, uint repay, bool isIn) private {
        ...
        if (product.debt == 0) {
            ...
            loan_map[user][index].share += (borrow - repay) / borrowingPrice(productId);
        }
    }
    ...
    **/
    function lenderEarning(uint8 productId) private returns (uint) {
        ...
        uint amount = product.balance + realDebt;
        uint16 pow = getDays(product.lastest_change_time, block.timestamp);
        int128 multiple = 2 ** 64;
        int128 sum = multiple + int128(int256(realDebt / (1000 * amount))) * multiple;
        return realDebt * uint(int(ABDKMath64x64.pow(sum, pow))) / uint(int(multiple));
    }

    /**
    * return lending share price.
    **/
    function lendingPrice(uint8 productId) private returns (uint) {
        Product memory product = product_list[productId];
        uint totalAmount = product.balance + lenderEarning(productId);
        return totalAmount / product.lending_share;
    }

    /**
    * return borrowing share price.
    **/
    function borrowingPrice(uint8 productId) private returns (uint) {
        Product memory product = product_list[productId];
    ...
    **/
    function setProductDebt(uint8 productId, uint borrow, uint repay, bool isChange) private returns (uint) {
        ...
        int128 multiple = 2 ** 64;
        int128 sum = multiple + int128(int256((product.borrow_rate_factor * amount + 100 * product.debt) / (100000 * amount))) * multiple;
        if (!isChange) {
            uint16 day = getDays(product.lastest_change_time, block.timestamp);
            if(day == 0) {
                return realDebt;
            }
            uint16 pow = day + 1;
