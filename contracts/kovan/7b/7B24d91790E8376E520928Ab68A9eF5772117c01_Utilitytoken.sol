...
contract MythWhiteList {
    ...
    function getAffiliatePercent(address _address)
        ...
        returns (bool)
    {
        if (partnerAddress[affiliateAddress[msg.sender]]) {
            return true;
        } else {
            return false;
        }
...
contract MythCoinFlip {
    ...
    function placeBet(uint8 _side, bool _pvp) external payable {
        require(msg.value >= minBet, "Too little of bet");
        require(msg.value <= maxBet, "Too Big of a bet");
    ...
    function resolveBet(uint256 _betId, bytes32 _resolutionSeed) external {
        require(msg.sender == owner, "Only the owner can resolve Bets");
        placedBets[_betId].resolutionSeed = _resolutionSeed;
        placedBets[_betId].betState = BetState.RESOLVED;
        placedBets[_betId].betterWinner =
            uint8(uint256(_resolutionSeed) % 2) ==
            placedBets[_betId].betterSide;

        if (placedBets[_betId].betterWinner) {
            rewardBalances[placedBets[_betId].better] +=
                placedBets[_betId].amount *
                2;
        } else if (placedBets[_betId].caller != address(0)) {
            rewardBalances[placedBets[_betId].caller] +=
                placedBets[_betId].amount *
                2;
        }
        mintMyth(owner, placedBets[_betId].amount / 200);
        ...
        if (placedBets[_betId].caller != address(0)) {
