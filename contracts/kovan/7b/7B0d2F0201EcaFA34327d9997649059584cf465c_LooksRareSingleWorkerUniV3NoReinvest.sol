...
import "../../../interfaces/IWorker02UniV3NoReinvest.sol";
...
contract LooksRareSingleWorkerUniV3NoReinvest is
  Ownable,
  ReentrancyGuard,
  IWorker02UniV3NoReinvest
{
  /// @notice Libraries
  ...
  constructor(
    ...
    // 4. Assign Path parameters
    path = _path;

    // 5. Setup Uniswap swap fee
    fee = 9970;
    feeDenom = 10000;
    poolFee = _poolFee;
  ...
  function health(uint256 id) external view override returns (uint256) {
    ...
    if (baseToken == rewardToken) {
      reward = getPositionReward(id);
    } else {
      reward = oracle.getAmountOut(
        rewardToken,
        getPositionReward(id),
        baseToken,
        poolFee
      );
    }
    ...
    return amount[amount.length - 1].add(reward);
  }
  ...
  function rewardToBase(uint256 rewardAmt) public {
  ...
  function actualBaseTokenBalance() internal view returns (uint256) {
    if (baseToken == rewardToken) {
      return baseToken.myBalance().sub(rewardBalance);
    } else {
  ...
  function _removeShare(uint256 id) internal {
    ...
    if (share > 0) {
      ...
      feeSharing.withdraw(looksShare, true);
      ...
      uint256 positionReward = share.mul(rewardBalance).div(total);
      if (positionReward > 0) {
        if (rewardToken != baseToken) {
          rewardToBase(positionReward);
        }
        rewardBalance = rewardBalance.sub(positionReward);
      }
      emit RemoveShare(id, share);
    }
  }

  function getPositionOwner(uint256 id) public view returns (address) {
    return IVault(operator).positionOwner(id);
  }

  function getTotalReward() public view returns (uint256) {
    return rewardBalance.add(feeSharing.calculatePendingRewards(address(this)));
  }

  function getPositionReward(uint256 id) public view returns (uint256) {
    uint256 share = shares[id];
    uint256 totalReward = getTotalReward();
    return share.mul(totalReward).div(totalShare);
  }
...
import "./IWorkerUniV3NoReinvest.sol";

interface IWorker02UniV3NoReinvest is IWorkerUniV3NoReinvest {
...
interface IWorkerUniV3NoReinvest {
