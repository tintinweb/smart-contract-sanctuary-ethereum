...
contract Snap_OTC is Context, AccessControlEnumerable, Pausable {
    ...
    */
    ...
    /**
    @dev token treasury contract address
     */
    SnapUSD public SUSD_TREASURY;
    ...
    */
    ...
    // fires when treasury wallet address has been updated
    event treasuryWalletUpdated(address new_wallet);

    // fires when treasury contract address has been updated
    event treasuryContractUpdated(address new_contract);
    ...
    @param _treasury_address is the contract address of the treasury contract
    @param _treasury_wallet is the address of the treasury wallet which gets the withdrawed funds
     */
    constructor(
        ...
        USDT = _USDT;
        USDC = _USDC;
        SUSD_TREASURY = SnapUSD(_treasury_address);
    ...
    /**
    @dev this modifier make sure the role of the caller is admin and passed parameter is not address 0
     */
    modifier AdminOnly() {
    ...
    /**
    @dev this modifier makes sure the address is not address 0
     */
    ...
    /**
    @dev withdraw token balance of the contract to the treasury wallet
    @param _token contract address which we want to withdraw
    */
    function withdraw(address _token) public AdminOnly {
        if (_token != address(0)) {
            ERC20(_token).transfer(
                treasury_wallet,
                ERC20(_token).balanceOf(address(this))
            );
            emit withdraw_event(_token);
        }
    ...
    // ==================================== minting logic - USDT ====================================
    /**
    @dev sending USDT and minting SUSD
    Note: this will consider referral code as 1 and can be called without referral code
    @param _amount of USDT to send. user has to approve this contract to spend _amount of USDT before calling this function.
     */
    function mint_by_USDT(uint256 _amount) public {
        address token = USDT;
        if (ERC20(token).allowance(_msgSender(), address(this)) < _amount) {
            revert AllowanceIsLow();
        }

        ERC20(token).transferFrom(_msgSender(), address(this), _amount);
        uint256 _mint_amount = (USDT_USD_price() * _amount);
        SUSD_TREASURY.mint(_msgSender(), _mint_amount);
        referrals_amount[1] += _amount;
        emit mint_event(token, _mint_amount, 1);
    }
     ...
     */
    function mint_by_USDT(uint256 _amount, uint256 _referral) public {
        address token = USDT;
        if (ERC20(token).allowance(_msgSender(), address(this)) < _amount) {
            revert AllowanceIsLow();
        }
        ...
        emit mint_event(token, _mint_amount, _referral);
    }
    ...
    */
    function mint_swap_USDT_view(uint256 _amount_in)
        public
        view
        returns (uint256)
    {
    ...
    function mint_by_USDC(uint256 _amount) public {
        ...
        ERC20(token).transferFrom(_msgSender(), address(this), _amount);
        uint256 _mint_amount = (USDC_USD_price() * _amount);
        SUSD_TREASURY.mint(_msgSender(), _mint_amount);
        referrals_amount[1] += _amount;
        emit mint_event(token, _mint_amount, 1);
    }
    ...
    @param _referral code 
     */
    function mint_by_USDC(uint256 _amount, uint256 _referral) public {
        address token = USDC;
        if (ERC20(token).allowance(_msgSender(), address(this)) < _amount) {
            revert AllowanceIsLow();
        }
    ...
    */
    function mint_swap_USDC_view(uint256 _amount_in)
        public
        view
        returns (uint256)
    {
    ...
    * NOTE: false is USDC, true is USDT
    ...
    /**
    @dev updating the treasury wallet 
    * Note: only ADMIN role can call this
    @param _newWallet to be replaced with the old one
     */
    function updateTreasuryWallet(address _newWallet)
        public
        AdminOnly
        NoAddress0(_newWallet)
    {
        treasury_wallet = _newWallet;
        emit treasuryWalletUpdated(_newWallet);
    }

    /**
    @dev updating the treasury contract
    * Note: only ADMIN role can call this
    @param _newTreasuryContract is the new Treasury contract address
     */
    function updateTreasuryContract(address _newTreasuryContract)
        public
        AdminOnly
        NoAddress0(_newTreasuryContract)
    {
        SUSD_TREASURY = SnapUSD(_newTreasuryContract);
        emit treasuryContractUpdated(_newTreasuryContract);
    }
...
contract SnapUSD is ERC20PresetMinterPauser {
    ...
    constructor(uint256 _initialSupply)
        ERC20PresetMinterPauser("Snap USD", "SUSD")
    {
    ...
    /**
     * @dev returns the amount of Allowance a minter has
     */
        ...
        notBlacklisted(_minter)
    {
        require(
            ...
            "ERC20PresetMinterPauser: must have ADMIN role to set Allowance"
        );
        require(
    ...
    /**
     * @dev Checks if _account is blacklisted
