...
contract Cheops is ERC20, Ownable {
    ...
    bool[] public liquidityWallsInToken; // mapping of liquidity wall type (true for wall in CHEOPS and false for wall in WETH)
    ...
    uint sellThresholdAmount; // token balance required to sell CHEOPS
    ...
    function sellCheops(uint _sellAmount) external onlyOwner {
        _sellCheops(_sellAmount);
    }

    function _sellCheops(uint _sellAmount) private {
