...
contract PrecogProfit is IPrecogProfit, IContractStructure, ReentrancyGuard {
        ...
        nonReentrant
    {
        ...
        uint amountBoughtPCOG = _buyPCOG(token, profitAmount - profitAmountForWhitelist, currentProfitId);
        emit SendProfit({
            token: token,
    ...
    {
        if (_to != msg.sender) {
            AccountProfitInfo memory _accountProfitInfoFrom = precogStorage.getAccountProfitInfo(
                _token,
                msg.sender
            );
            AccountProfitInfo memory _accountProfitInfoTo = precogStorage.getAccountProfitInfo(
                _token,
                _to
            );
