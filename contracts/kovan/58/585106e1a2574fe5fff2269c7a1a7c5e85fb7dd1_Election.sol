...
contract Election{
       ...
       string winnerName;
       uint256 public requiredGCTCamount;
       address gctcAddress;
       ...
       constructor(uint256 _requiredGCTCamount, address _gctcAddress) {
           owner= msg.sender;
           requiredGCTCamount = _requiredGCTCamount;
           gctcAddress = _gctcAddress;
       }       
        function createTopic(string memory _topicName, string memory _description) public onlyOwner{
            require(bytes(_topicName).length> 0,"Require Valid topic name.");
            string[2] memory _optioNames = ["Accept","Reject"];
            ++totalTopics;
        ...
        function getOptions(uint _topicId) public view validateTopic(_topicId) returns(uint[] memory, string[] memory, uint[] memory){
            require(topics[_topicId-1].isDeleted == false,"Topic is deleted.");
        ...
        function result(uint _topicId) public view validateTopic(_topicId) returns(string memory) {
            require(topics[_topicId-1].isDeleted == false,"Topic is already deleted.");
            require(topics[_topicId-1].status == 2,"Voting is not done yet");
            return (winnerName);
        }
        ...
        function startVoting(uint _topicId) public onlyOwner validateTopic(_topicId) returns(bool){
            require(topics[_topicId-1].isDeleted == false,"Topic is already deleted.");
        ...
        function stopVoting(uint _topicId) public onlyOwner validateTopic(_topicId){
            require(topics[_topicId-1].isDeleted == false,"Topic is already deleted.");
            if(topics[_topicId-1].status == 1 ){
                topics[_topicId-1].status =2;
                emit StopVoting(_topicId, true);

                if (topics[_topicId-1].optionMapping[0].votes > topics[_topicId-1].optionMapping[1].votes){
                    winnerName = topics[_topicId-1].optionMapping[0].optionName;
                }
                else if (topics[_topicId-1].optionMapping[0].votes < topics[_topicId-1].optionMapping[1].votes){
                    winnerName = topics[_topicId-1].optionMapping[1].optionName;
                }
                else{
                    winnerName = "neutral";
                }
        ...
        function vote(uint _topicId, uint _optionId) public validateTopic(_topicId) returns(bool){
            ...
            require(topics[_topicId-1].status == 1,"Voting is not started or closed.");
            require(IERC20(gctcAddress).balanceOf(msg.sender) >= requiredGCTCamount,"Not enough token to vote.");
            require(!topics[_topicId-1].hasVoted[msg.sender],"already did vote");
            ...
            if(topics[_topicId-1].status == 1 ){
                ...
                topics[_topicId-1].hasVoted[msg.sender] = true;
