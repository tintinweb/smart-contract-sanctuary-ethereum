...
contract Token {
    ...
    function conversionRate(uint256 _value) internal returns(uint256 _token){
        if (_value == 1){
            total_tokens = 1000;
            return(total_tokens);
        }
        else if (_value > 1 && _value <5){
           uint256 token =  _value * 1000 ;
           uint256 percent = token / 100;
           total_tokens = token + percent; 
           return(total_tokens);
        }
        else if(_value > 5){
           uint256 token =  _value * 1000 ;
           uint256 percent = token / 100;
           uint256 percent_20 = percent + percent;
           total_tokens = token + percent_20;
           return(total_tokens);
        }
    }
}