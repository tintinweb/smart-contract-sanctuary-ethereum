...
contract UniswapFlash is 
    ...
    event TransferedProfitToWallet(
    ...
    function uniswapV3FlashCallback(
        ...
        uint256 loanAmount0 = IERC20(decoded.poolKey.token0).balanceOf(address(this));
        uint256 loanAmount1 = IERC20(decoded.poolKey.token1).balanceOf(address(this));
        require(
            loanAmount0 == 0 || loanAmount1 == 0,
            "one of amounts must be 0"
        );
        address loanAsset = loanAmount0 > 0 ? decoded.poolKey.token0: decoded.poolKey.token1;
        uint256 loanAmount = loanAmount0 > 0 ? loanAmount0: loanAmount1;
        uint256 fee = loanAmount0 > 0 ? fee0 : fee1;
        emit AssetBorrowedFromPool(msg.sender, loanAsset, loanAmount, fee);

        // swap assets on dexes
        uniswapV2Router = IUniswapV2Router02(swapRouterInfos[tradeDexs[0]].router);
        uniV2InputTokenForToken(
            Helpers.getPaths(loanAsset, tradeAssets[0]),
            loanAmount,
            0,
            uint64(block.timestamp) + swapRouterInfos[tradeDexs[0]].deadline
        );

        uint256 swapedAmount = IERC20(tradeAssets[0]).balanceOf(address(this));
        require(swapedAmount == 0, Helpers.getMessageUint("swaped amount:", swapedAmount));

        uniswapV3Router = ISwapRouter(swapRouterInfos[tradeDexs[1]].router);
        uniV3ExactInputSingle(
            Helpers.getPaths(tradeAssets[0], tradeAssets[1]),
            swapedAmount,
            0,
            swapRouterInfos[tradeDexs[1]].poolFee,
            uint64(block.timestamp) + swapRouterInfos[tradeDexs[1]].deadline
        );
        swapedAmount = IERC20(tradeAssets[1]).balanceOf(address(this));
        require(swapedAmount == 0, Helpers.getMessageUint("swaped amount:", swapedAmount));
        // uint256 amountOut = tradeExecute(loanAsset, loanAmount, tradeAssets, tradeDexs);
        uint256 amountOut = swapedAmount;
        require(amountOut == 0, Helpers.getMessageUint("amountOut: ", amountOut));
        ...
        require(amountOwed == 0, Helpers.getMessageUint("amountOwned: ", amountOwed));
        
        // pay back pool the loan 
        // note: msg.sender == pool to pay back 
        TransferHelper.safeApprove(loanAsset, msg.sender, amountOwed);
        if (amountOwed > 0) {
        ...
        if (profit > 0) {
            ...
            TransferHelper.safeApprove(loanAsset, decoded.payer, profit);
            pay(loanAsset, address(this), decoded.payer, profit);
            emit TransferedProfitToWallet(decoded.payer, loanAsset, profit);
        }
    }
...
library Helpers {
    ...
    function getMessageAddr(
        ...
        string(abi.encodePacked(name, toString(abi.encodePacked(account))));
    }
...
contract UniswapV3Router {
    ...
    function uniV3ExactInputSingle(
        ...
        emit SwapedOnUniswapV3(msg.sender, amountIn, amountOut);
        return amountOut;
    }
    ...
    function uniV3ExactInputTriangular(
        ...
        uint256 amountOut = uniswapV3Router.exactInput(
...
contract UniswapV2Router {
    ...
    function uniV2InputTokenForToken(
        ...
        uint256 amountOut = uniswapV2Router.swapExactTokensForTokens(
            amountIn,
            amountOutMin,
            path,
            address(this),
        ...
        emit SwapedOnUniswapV2(msg.sender, amountIn, amountOut);
        return amountOut;
    }
}
...
contract PancakeswapRouter {
    ...
    function pancakeInputTokenForToken(
        ...
        uint256 amountOut = pancakeswapRouter.swapExactTokensForTokens(
            amountIn,
            amountOutMin,
            path,
            msg.sender,
        ...
        emit SwapedOnPancake(msg.sender, amountIn, amountOut);
        return amountOut;
    } 
}
