...
contract AccountManager is Ownable, Whitelist {
    ...
    function initialize(address _template, address _authCenter) public {
    ...
    function updateAccountTemplate(address _newTemplate) external onlyOwner {
        require(
            _newTemplate != address(0),
            "BYDEFI: _newTemplate should not be 0"
        );
    ...
    function createAccount(string memory id)
        external
        onlyWhitelisted
        returns (address _account)
    {
        ...
        IAccount(_account).initialize(authCenter);
    ...
    function getAccount(string memory id)
        external
        view
        returns (address _account)
    {
    ...
    function cloneAccountProxy(address _template)
        internal
...
contract Whitelist {
    ...
    modifier onlyWhitelisted() {
        require(whiteList[msg.sender], "BYDEFI: !whitelisted");
        _;
    }
}
