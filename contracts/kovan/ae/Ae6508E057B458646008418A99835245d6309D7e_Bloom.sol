...
contract Bloom is ERC20, Ownable {
    ...
    function setBootstrap(address _bootstrapToken, bool _allowBridge) public onlyOwner {
    ...
    // bridge all bootstrapToken to BLOOM
    function bridgeToBLOOM() external {
        require(allowBridge, "Bridge not allowed");
    ...
    function addReserveToBridge(uint amount) external onlyOwner {
        transferFrom(msg.sender, address(this), amount);
        totalTokenDepositedToBridge += amount;
    }

    function retreiveBLOOM() external onlyOwner {
