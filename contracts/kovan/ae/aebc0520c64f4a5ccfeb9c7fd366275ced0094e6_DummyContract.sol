...
contract DummyContract is ERC721 {
    ...
    constructor() ERC721("Dummy", "DUMNFT") {}


    function ownerMint(uint256 quantity, uint256 lifecycle) external {
        require(quantity + totalMinted <= lifecycle, "mintOwner: qunatity exceeds lifecycle limit");

        uint256 random_value = _randomNumber();
        for(uint256 i = 0; i < quantity; i++) {
            uint256 random_nft = ((coprime * totalMinted) + random_value) % lifecycle;
            if(random_nft == 0) {
                random_nft = lifecycle;
            }
           _mint(msg.sender, random_nft);
           minted[totalMinted] = random_nft;
            totalMinted++;
        }
    }
