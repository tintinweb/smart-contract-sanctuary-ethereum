    ...
    INFTXVault
{
    ...
    function getRatio(uint256 quality) public override view returns(uint256){
        return ratio[quality] == 0 ? ratio[0] : ratio[quality];
    }
    ...
    function setAmountRedeem(uint256 tokenId) internal {
        if (is1155) {
            if (mintedNFT[msg.sender][tokenId] > 0) {
                mintedNFT[msg.sender][tokenId] -= 1;
            } else {
                allowAllWithdrawals[tokenId] -= 1;
            }
    ...
    function mintTo(
        ...
        emit Minted(tokenIds, amounts, to, allowAllWithdrawal);
        return count;
    }
...
interface INFTXVault {
    ...
    event Minted(uint256[] nftIds, uint256[] amounts, address to, bool allowAllWithdrawal);
...
interface INFTXVaultFactory is IBeacon {
  ...
  event NewVault(uint256 indexed vaultId, string vaultName, address vaultAddress, address assetAddress, uint256[] levels, uint256[] ratios);
