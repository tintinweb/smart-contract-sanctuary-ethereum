...
contract UniswapFlash1Inch is 
    ...
    function uniswapV3FlashCallback(
        ...
        for (uint i; i < tradeDatas.length; i++) {
            ...
            (bool success, ) = routers[i].call(tradeDatas[i]);
            require(success, "Swap Failue!");

        }
        uint256 amountOut = IERC20(callback.poolKey.token0).balanceOf(address(this));
        uint256 amountOwed = callback.amount0.add(fee0);
        
        if (amountOut >= amountOwed) {
            pay(callback.poolKey.token0, address(this), msg.sender, amountOwed);
        }
        uint256 profit = amountOut.sub(amountOwed);
        if (profit > 0) {
            pay(callback.poolKey.token0, address(this), callback.payer, profit);
        }
    }
