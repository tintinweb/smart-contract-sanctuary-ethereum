...
contract ETPToken is Context, IERC20, Ownable {
    ...
    uint256 public etcSparePoolPrepaidCount; //etc spare pool prepaid count
   ...
   //============ update ===============
    ...
    uint256 public _maxTxAmount = 100e18;
    ...
    function _transfer(
        ...
        if( 
            ...
            etpFee2UsdtCount += beforeBalance.sub(afeterBalance);
         ...
         if( takeFee ){           
           if( ammPairs[from]){  // buy or removeLiquidity
                ...
                if(etpFeeValue > 0 ){
                    ...
                    if(usdtFeeValue > 0){
                        ...
                        if(!_isContract(to)){
                            TransferHelper.safeTransferFrom(busd,usdtSparePoolManager,etcPoolAddress,usdtFeeValue.mul(6).div(8));
                            etcSparePoolPrepaidCount += usdtFeeValue.mul(6).div(8);
                            ETC(etcPoolAddress).casting(usdtFeeValue.mul(6).div(8),to);
                        }

                        IERC20(busd).transfer(daoAddress,usdtFeeValue.mul(1).div(8));
                        IERC20(busd).transfer(lpPoolAddress,usdtFeeValue.mul(1).div(8));
           ...
           if( ammPairs[to]){//sell or addLiquidity
                ...
                if(usdtFeeValue > 0 ){
                    usdtSparePoolPrepaidCount += usdtFeeValue;
                    if(feeRate == 3){
                        TransferHelper.safeTransferFrom(busd,usdtSparePoolManager,etcPoolAddress,usdtFeeValue);
                    }else{
                        if(!_isContract(to)){
                            TransferHelper.safeTransferFrom(busd,usdtSparePoolManager,etcPoolAddress,usdtFeeValue.mul(6).div(9));
                            ETC(etcPoolAddress).casting(usdtFeeValue.mul(6).div(9),to);
