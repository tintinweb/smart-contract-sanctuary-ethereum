...
contract MiddlewareExchange is IMiddlewareExchange {
    ...
    function buyToken(address tokenIn, address tokenOut, uint amountIn) external override returns (uint amountOut) {
        IERC20(tokenIn).safeTransferFrom(msg.sender, address(this), amountIn);
        if (IERC20(tokenIn).allowance(address(this), exchange) < amountIn) {
            IERC20(tokenIn).safeApprove(exchange, 2**255);
        }
        uint estimatedPCOG = IExchangeRouter(exchange).getAmountsOut(amountIn, getPath(tokenIn, tokenOut))[1];
