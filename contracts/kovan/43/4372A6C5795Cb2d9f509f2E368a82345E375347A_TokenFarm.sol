...
contract TokenFarm is ChainlinkClient, Ownable {
    string public name = "Data Life Token Farm";
    IERC20 public dlToken;

    // staker addresses' list
    address[] public stakers;
    //allowed token addresses' list
    address[] public allowedTokens;
    // mapping of staker address to user's staked tokens
    mapping(address => uint256) public uniqueTokensStaked;
    // nested mapping: token address to (staker address to staker amount)
    mapping(address => mapping(address => uint256)) public stakingBal;
    // mapping of token address to its relevant price feed address
    mapping(address => address) public tokenPriceFeedMapping;

    constructor(address dlTokenAddress) public {
        dlToken = IERC20(dlTokenAddress);
    }
    ...
    function unstakeTokens(address token) public {
     ...
     // This is vulnerable to a reentrancy attack!!!
        ...
        uint256 balance = stakingBal[token][msg.sender];
        require(balance > 0, "Staking balance cannot be 0");
        IERC20(token).transfer(msg.sender, balance);
        stakingBal[token][msg.sender] = 0;
    ...
    function issueTokens() public onlyOwner {
        ...
        for (
            ...
            dlToken.transfer(recipient, getUserTotalValue(recipient));
            // send token award to recipient address based on recipient's TVL
        }
    }
    ...
    function getUserTotalValue(address user) public view returns (uint256) {
        ...
        for (uint256 allowedTokensIndex = 0; 
            ...
            allowedTokensIndex++) 
        {
            totalValue = totalValue + getUserTokenStakingBalanceValue(
                user, allowedTokens[allowedTokensIndex]);
        }
        return totalValue;
    }

    function getUserTokenStakingBalanceValue(address user, address token)
        public
        view
        returns (uint256)
    {
        ...
        (uint256 price, uint8 decimals) = getTokenPrice(token);
        // 10 ETH (ETH/USD) -> in Wei 10000000000000000000
        // figures out the decimals needed from (price) feed to convert the bal properly 
        // 10 ETH (stakingBal[token][user]) * $1000/1ETH (price) / 10^decimals = $10,000 = 10
        return (stakingBal[token][user] * price) / (10 ** uint256(decimals));
    }
