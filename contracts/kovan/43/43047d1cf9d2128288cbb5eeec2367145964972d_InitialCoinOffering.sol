...
contract InitialCoinOffering is Ownable, ReentrancyGuard, ERC721Holder, Pausable {    
    ...
    constructor(address collarQuest_, uint startTimestamp) {
        require(startTimestamp > block.timestamp, "constructor : startTimestamp must be");
        collarQuest = collarQuest_;

        timestamp[uint(Sale.preSale)] = Timestamp( // setting presale durations
            uint64(startTimestamp),
            uint64(startTimestamp + 1 days)
        );

        timestamp[uint(Sale.publicSale)] = Timestamp( // setting public sale durations
            uint64(startTimestamp + 1 days),
            uint64(startTimestamp + 2 days)
        );
    }
