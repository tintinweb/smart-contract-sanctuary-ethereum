...
contract CompoundStakingReward is ReentrancyGuard, AccessControl, IStakeFor {
    ...
    address penaltyAddress;          // penalty goes to this address
     ...
     */
    function getPenalty(uint256 _amount) view public returns (uint256) {
        ...
        for (uint i = 0; i < penaltyPeriods.length; i++) {
            ...
            if (block.number < endBlock) {
                return _amount.div(penaltyRates[i]).mul(100);
            }
        }
        return 0;
    }
    ...
    function _withdraw(uint256 shares) internal {
        ...
        // check penalty 
        uint256 penaltyAmount = getPenalty(currentAmount);

        stateToken.safeTransfer(penaltyAddress, penaltyAmount);
        ...
        emit Withdraw(msg.sender, currentAmount, 0);
    }
}
