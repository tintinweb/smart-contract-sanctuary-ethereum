...
contract Escrow is Ownable, ReentrancyGuard {
    ...
    function createContest(bytes calldata _signature, Token[][] calldata _prize) external nonReentrant {
        uint256 i;
        uint256 j;
        uint256 totalEth;
        ...
        address contestOwner = ECDSA.recover(message, _signature);
    ...
    function getAddress(bytes calldata _signature, Token[][] calldata _prize) external pure returns(address) {
        uint256 i;
        uint256 j;
        bytes memory signature;
