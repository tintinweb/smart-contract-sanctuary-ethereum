...
/**
 * @title DEMOSTBXTWO | Stobox Technologies Inc.
 * @author Stobox Technologies Inc.
 * @dev DEMOSTBXTWO ERC20 Token | This contract is opt for digital securities management.
 */
 
contract DEMOSTBXTWOToken is IERC20, Roles {
    ...
    modifier onlyWhitelisted(address _account) {
        if (_isDisabledWitelist) {
            require(
                ...
                "DEMOSTBXTWO: not whitelisted address."
            );
        }
        _;
    }
    ...
    modifier onlyWithUnfrozenFunds(address _account) {
        require(
            ...
            "DEMOSTBXTWO: funds are frozen."
        );
        _;
    }

    /**
     * @notice DEMOSTBXTWOToken simply implements a ERC20 token.
     */
    ...
    function burn(address _account, uint256 _amount) external onlySuperAdmin {
        require(balanceOf(_account) >= _amount, "DEMOSTBXTWO: balance too low");
    ...
    function transferFunds(
        ...
        require(balanceOf(_from) >= _amount, "DEMOSTBXTWO: not enough tokens");
    ...
    function splitOrMerge(uint256 _x, uint256 _y) external onlySuperAdmin {
        require(_x != _y, "DEMOSTBXTWO: _x must not be equal to _y");
        require(_x > 0, "DEMOSTBXTWO: _x must be greater than 0");
        require(_y > 0, "DEMOSTBXTWO: _y must be greater than 0");
    ...
    */
        ...
        returns (bool)
    {
        ...
        _approve(
            ...
            _allowances[_sender][msg.sender].sub(
                ...
                "DEMOSTBXTWO: transfer amount exceeds allowance"
            )
        );
        ...
        returns (bool)
    {
        _approve(
            ...
            _allowances[msg.sender][_spender].sub(
                ...
                "DEMOSTBXTWO: decreased allowance below zero"
            )
        );
    ...
    function _transfer(
        ...
        require(_sender != address(0), "DEMOSTBXTWO: transfer from the zero address");
        require(_recipient != address(0), "DEMOSTBXTWO: transfer to the zero address");

        _beforeTokenTransfer(_sender, _recipient, _amount);

        _balances[_sender] = _balances[_sender].sub(
            _amount,
            "DEMOSTBXTWO: transfer amount exceeds balance"
        );
    ...
    function _mint(address _account, uint256 _amount) internal virtual {
        require(_account != address(0), "DEMOSTBXTWO: mint to the zero address");
    ...
    function _burn(address _account, uint256 _amount) internal virtual {
        require(_account != address(0), "DEMOSTBXTWO: burn from the zero address");

        _beforeTokenTransfer(_account, address(0), _amount);

        _balances[_account] = _balances[_account].sub(
            _amount,
            "DEMOSTBXTWO: burn amount exceeds balance"
        );
    ...
    function _approve(
        ...
        require(_owner != address(0), "DEMOSTBXTWO: approve from the zero address");
        require(_spender != address(0), "DEMOSTBXTWO: approve to the zero address");
    ...
    function _updateTransferLimit(address _account, uint256 _amount) private {
        ...
        _transferLimits[_account].allowedToTransfer = _transferLimits[_account]
            .allowedToTransfer
            .sub(_amount, "DEMOSTBXTWO: transfer exceeds your transfer limit");
    }
    ...
    function _updateTransactionCountLimit(address _account) private {
        if (_isEnabledTransactionCount) {
            ...
            _transactionCountLimits[_account]
                ...
                .sub(1, "DEMOSTBXTWO: transfer exceeds your transaction count limit");
        }
    }
}