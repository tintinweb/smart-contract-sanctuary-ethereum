...
contract ImprovedFarm is Ownable {
    struct StakerStruct {
        uint256 index;
        uint256 uniqueStakedTokenCound;
        mapping(address => uint256) stakedBalance; //token to balance
    }
        ...
        returns (uint256 index)
    {
        ...
        //do uniqueStakedTokenCound-1 on unstake
    ...
    // // SEND REW 2T FOR USERS WHO DEPOSITED 2T
    // // ex. deposit 2t 1coin, sends 2t reward with 2coin (each tx)
    // function sendRewards() public onlyOwner {
    //     // stakers contains the same addr 2 times
