...
contract Token is ERC20Permit {
    ...
    uint256 public immutable TOKEN_CLAIM_END;
    address public immutable TOKEN_BURN_ADDRESS;
     ...
     */
    constructor()
        ERC20("Test New Token", "TEST2")
        ERC20Permit("Test New Token")
    {
        TOKEN = IERC20(0x94FB893a5599c378E2b58dA2F8496D2D983Da26d);
        TOKEN_CLAIM_END = _getNow().add(1814400); // 3*7*24*60*60 = 1814400
        TOKEN_BURN_ADDRESS = 0x000000000000000000000000000000000000dEaD;

        // 3500000-2432000 = 1068000 (max supply - tokens to claim)
        _mint(_msgSender(), 1068000 ether);
    }
    ...
    function claim(uint256 amount) external inClaimPeriod {
        ...
        TOKEN.safeTransferFrom(_msgSender(), TOKEN_BURN_ADDRESS, amount);
        emit TokensClaimed(_msgSender(), amount);
    }

    modifier inClaimPeriod() {
        require(_getNow() < TOKEN_CLAIM_END, "CLAIM_PERIOD_ENDED");
        _;
    }
}