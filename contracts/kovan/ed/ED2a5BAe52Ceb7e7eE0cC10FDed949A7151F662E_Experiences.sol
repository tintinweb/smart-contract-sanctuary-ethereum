...
contract Experiences is ERC721Enumerable, VRFConsumerBase, Ownable {
    ...
    uint256 public constant NUM_TRAITS = 2;

    uint256 public constant TRAIT_INDEX_CATEGORY = 0;
    uint256 public constant TRAIT_INDEX_LOCATION = 1;

    uint256[NUM_TRAITS] private traitSizes;
    uint256[NUM_TRAITS] private traitCounts;
    uint256[NUM_TRAITS] private traitRemaining;

    bytes32[NUM_TRAITS] public traitProperties;
    ...
    uint256 internal price = 1 * 10**16;

    bool public isSaleActive = false;

    uint256 public numSold = 0; // Mainnet results
    ...
    uint256 internal linkFee = 1 * 10**17; // 0.1 LINK
    // 2 * 10**18; // 2 LINK

    address public mintableToken = 0xA0b86991c6218b36c1d19D4a2e9Eb0cE3606eB48;

    address public treasury;

    constructor()
        ERC721("Experiences", "EXPERIENCE")
        VRFConsumerBase(VRFCoordinator, linkToken)
    {
        traitSizes = [4, 9];

        uint256[] memory categoryDistribution = new uint256[](
            traitSizes[TRAIT_INDEX_CATEGORY]
        );
        uint256[] memory locationDistribution = new uint256[](
            traitSizes[TRAIT_INDEX_LOCATION]
        );

        traitCounts[TRAIT_INDEX_CATEGORY] = UIntArrays.packedUintFromArray(
            categoryDistribution
        );
        traitCounts[TRAIT_INDEX_LOCATION] = UIntArrays.packedUintFromArray(
            locationDistribution
        );

        traitRemaining[TRAIT_INDEX_CATEGORY] = UIntArrays.packedUintFromArray(
            categoryDistribution
        );
        traitRemaining[TRAIT_INDEX_LOCATION] = UIntArrays.packedUintFromArray(
            locationDistribution
        );

        traitProperties = [bytes32("Category"), bytes32("Location")];

        traitNames[TRAIT_INDEX_CATEGORY] = [
            bytes32("Parties"),
            bytes32("Housing"),
            bytes32("Cars"),
            bytes32("Companions")
        ];
        traitNames[TRAIT_INDEX_LOCATION] = [
            bytes32("Tahoe"),
            bytes32("Monterey"),
            bytes32("Pebble Beach"),
            bytes32("Los Angeles"),
            bytes32("Costa Mesa"),
            bytes32("Carlsbad"),
            bytes32("Scottsdale"),
    ...
    function setExperience(uint256 _tokenId, uint256[] memory _traits)
        internal
    {
        uint256 experience = _traits[0];
        experience |= _traits[1] << 8;

        experiences[_tokenId] = experience;
    }

    function getExperience(uint256 _tokenId)
        internal
        view
        returns (Experience memory _experience)
    {
        uint256 experience = experiences[_tokenId];
        _experience.category = uint256(uint8(experience));
        _experience.location = uint256(uint8(experience >> 8));
    }
    ...
    function experienceHash(uint256[] memory _traits)
        public
        pure
        returns (bytes32)
    {
    ...
    function isAvailable(uint256 _claimId) public view returns (bool) {
        return experiences[_claimId] == 0;
    }
    ...
    function tokenIdFromTraits(uint256[] memory _traits)
        ...
        returns (uint256)
    {
        return hashToTokenId[experienceHash(_traits)];
    }

    function traitsForTokenId(uint256 _tokenId)
        public
        view
        returns (uint256[] memory _traits)
    {
        (_traits) = traitsFromExperience(getExperience(_tokenId));
    }

    function traitsFromExperience(Experience memory _experience)
        internal
        pure
        returns (uint256[] memory _traits)
    {
        _traits = new uint256[](NUM_TRAITS);
        _traits[TRAIT_INDEX_CATEGORY] = _experience.category;
        _traits[TRAIT_INDEX_LOCATION] = _experience.location;
    }
    ...
    function _mintExperience(address _sendTo, uint256 _tokenId) internal {
        // 1. Try to get random traits from remaining
        // uint256 dna = uint256(keccak256(abi.encodePacked(msg.sender, _tokenId, block.difficulty, block.timestamp)));
        getRandomNumber();
        uint dna = randomResult;
        uint256[] memory traits = randomTraits(dna);

        hashToTokenId[experienceHash(traits)] = _tokenId;
        setExperience(_tokenId, traits);
        ...
        emit ExperienceMinted(_sendTo, _tokenId, traits);
    }

    function randomTraits(uint256 _dna)
        internal
        view
        ...
        returns (string memory resultString)
    {
        ...
        resultString = "{";
        uint256[] memory traits = traitsForTokenId(_tokenId);
        ...
        for (uint256 i = 0; i < traits.length; i++) {
            ...
            resultString = strConcat(
                resultString,
                bytes32ToString(traitProperties[i])
            );
            resultString = strConcat(resultString, '": "');
            resultString = strConcat(
                resultString,
    ...
    function buy(uint256 _numBuy) external {
        uint256 startIndex = numSold;
        require(isSaleActive, "Sale is not active");
        require(startIndex + _numBuy < TOKEN_LIMIT, "Exceeded 20000 limit");
        require(_numBuy < 11, "You can buy maximum 10 experiences");

        ITreasury(treasury).deposit(mintableToken, price * _numBuy);

        for (uint256 i = 0; i < _numBuy; i++) {
            _mintExperience(msg.sender, startIndex + i);
        }
        numSold += _numBuy;
    }
    ...
    function airdrop(address _sendTo, uint256 _numAirdrop) public onlyOwner {
        ...
        for (uint256 i = 0; i < _numAirdrop; i++) {
            _mintExperience(_sendTo, startIndex + i);
        }
        numSold += _numAirdrop;
    }

    // Note: This should only be used for minting existing experiences on Ethereum Mainnet
    function mintSpecificExperience(
        address _sendTo,
        uint256 _tokenId,
        uint256[] memory _traits
    ) public onlyOwner {
        hashToTokenId[experienceHash(_traits)] = _tokenId;
        setExperience(_tokenId, _traits);
        ...
        _safeMint(_sendTo, _tokenId);
        emit ExperienceMinted(_sendTo, _tokenId, _traits);
    }
    ...
    function reserveMint(address _sendTo, uint256 _tokenId) public {
        ...
        require(isAvailable(_tokenId), "Invalid token id");
        _mintExperience(_sendTo, _tokenId);
    }
}
