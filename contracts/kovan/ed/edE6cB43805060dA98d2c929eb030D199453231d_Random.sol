...
contract Random is IRandom, ISeed, Ownable {
    ...
    function randomRateSplitMadToken() external view override returns (uint amount) {
        uint _random = random();
        uint _randomByDifficulty = randomByDifficulty();
        uint typeAmount = _random % HUNDRED_PERCENT;
        if(typeAmount <= 8999) {
            amount =  (_randomByDifficulty - typeAmount * HUNDRED_PERCENT) % 91 + 10;
        } else if (typeAmount >= 9000 && typeAmount <= 9699) {
            amount = (_randomByDifficulty - typeAmount * HUNDRED_PERCENT) % 901 + 100;
        } else {
            amount = (_randomByDifficulty - typeAmount * HUNDRED_PERCENT) % 9001 + 1000;
        }
    }

    function randomEnhancerId(uint limit) external view override returns (uint randomId) {
