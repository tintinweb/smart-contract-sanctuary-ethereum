...
contract SmartWalletFactory is Ownable, WhitelistConsumer {
    using Clones for address;

    event SmartWalletCreated(address _smartWalletAddress);
    event TemplateChanged(address _templateAddress);
    ...
    function setTemplateAddress(address _templateAddress) public onlyOwner {
        ...
        emit TemplateChanged(_templateAddress);
    }

    function createSmartWallet(bytes32 _salt, address _whitelistAddress)
        external
        isWhitelisted
        returns (address smartWalletAddress)
    {
        smartWalletAddress = templateAddress.cloneDeterministic(_salt);
        ISmartWallet(smartWalletAddress).init(_whitelistAddress);

        emit SmartWalletCreated(smartWalletAddress);
    }

    function predictSmartWalletAddress(bytes32 _salt)
        public
        view
        returns (address)
    {
