...
contract TokenLock is AccessControlEnumerable {
    ...
    function updateTokenPenaltyFees(
        ...
        require(_burnFee + _treasuryFee < 100_00, "Invalid fee");
    ...
    function _returnWithPenalty(LockPosition memory lockPosition) private {
        ...
        uint256 burnFee = (lockPosition._amount * tokenPenalty.burn) / DIVIDER;
    ...
    function _removePosition(address _beneficiary, uint256 _positionIndex)
        private
    {
        LockPosition[] storage positions = lockPositions[_beneficiary];
        uint256 positionsLength = positions.length;
        LockPosition memory lastLockPosition = positions[positionsLength - 1];

        lastLockPosition._index = _positionIndex;
        positions[_positionIndex] = lastLockPosition;
        positions.pop();
    }
