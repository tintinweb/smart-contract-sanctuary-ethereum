...
contract UtilToken is ERC721AUpgradeable, BlackListUpgradeable,  PausableUpgradeable, OwnableUpgradeable, ReentrancyGuardUpgradeable, IUtilToken{
    ...
    function whiteListCheck(address callerAddress, bytes32[] calldata _merkleProof, Batch memory currentBatch, uint256 batchArrayIndex) internal view returns (bool) {
    ...
    function addBatch(uint256 _startTime, uint256 _endTime, uint256 _mintPrice, bytes32 _merkleHash) external override onlyOwner whenNotPaused {
        ...
        emit NewBatch(batchCollectionLength());
    }
    ...
    function updateCurrentBatch(uint256 _endTime) external override onlyOwner whenNotPaused {
        Batch memory tempBatch = latestBatch();
        require(tempBatch.startTime < _endTime, "Batch start time should be less than end time");
        require(block.timestamp < _endTime, "Batch end time should be greater than current time");
        require(block.timestamp < tempBatch.endTime, "Batch time already passed");
        uint256 batchArrayIndex = batchCollectionLength().sub(1);
        batchCollection[batchArrayIndex].endTime = _endTime;
    }

    /// @dev The internal function for getting the index of the batch array

    function latestBatch() internal view returns(Batch memory){
        uint256 batchCollectionArrayIndex = batchCollection.length.sub(1);
        return batchCollection[batchCollectionArrayIndex];
    }

    /// @dev The internal function for getting the length of the batch array

    function batchCollectionLength() internal view returns(uint256){
    ...
    function isTokenAirDropped(uint256 index) public view override returns (bool){
...
interface IUtilToken {
    ...
    event NewBatch (uint batchIndex);
...
// event name change
// get batch function
// Insufficient balance change standard
// sanity check change name and things
