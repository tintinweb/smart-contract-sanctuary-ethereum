...
contract Multiswap {
    ...
    function addEthereumLiquidity(uint256 amount) public payable {
        require(amount > 0, "Cannot create a pool with 0 ethers");
    }
    ...
    function addLiquidity(
        ...
        _token2.transferFrom(msg.sender, address(this), (amount2 * 10**18));

        liquidityPools[eth][symbol2].push(amount1 * 10**18);
        liquidityPools[eth][symbol2].push(amount2 * 10**18);

        liquidityPools[symbol2][eth].push(amount2 * 10**18);
        liquidityPools[symbol2][eth].push(amount1 * 10**18);
    ...
    function checkTokensYouGetForSingleSwap(
        ...
        if (
            ...
            if (
                ...
                liquidityPools[symbol1][eth][0] += (amount1 * 10**18);
                liquidityPools[symbol2][eth][0] -= (symbol2_to_be_given);

                liquidityPools[eth][symbol1][1] += (amount1 * 10**18);
                ...
                uint256 eth_to_be_given = (symbol2_supply * 10**18 * amount1) /
                    (symbol1_supply);
            ...
            uint256 symbol2_to_be_given = (symbol2_supply * amount1 * 10**18) /
                (symbol1_supply);

            liquidityPools[eth][symbol2][0] += (amount1 * 10**18);
            liquidityPools[eth][symbol2][1] -= symbol2_to_be_given;

            liquidityPools[symbol2][eth][0] -= symbol2_to_be_given;
            liquidityPools[symbol2][eth][1] += (amount1 * 10**18);
    ...
    function checkTokensYouGetForMultipleSwap(
        ...
        if (
            ...
            liquidityPools[eth][symbol2][0] += ((amount1 * 10**18 * per1) /
                100);
            liquidityPools[eth][symbol2][1] -= symbol2_to_be_given;

            liquidityPools[symbol2][eth][0] -= symbol2_to_be_given;
            liquidityPools[symbol2][eth][1] += ((amount1 * 10**18 * per1) /
                100);

            liquidityPools[eth][symbol3][0] += ((amount1 * 10**18 * per2) /
                100);
            liquidityPools[eth][symbol3][1] -= symbol3_to_be_given;

            liquidityPools[symbol3][eth][0] -= symbol3_to_be_given;
            liquidityPools[symbol3][eth][1] += ((amount1 * 10**18 * per2) /
                100);
            ...
            if (
                ...
                uint256 eth_to_be_given = (local.eth1_supply *
                    per1 *
                    10**18 *
                    amount1) / (100 * local.symbol1_supply);
                uint256 symbol3_to_be_given = (local.eth1_supply *
                    local.symbol3_supply *
                    per2 *
                    amount1 *
                    10**18) / (100 * local.symbol1_supply * local.eth2_supply);
                ...
                uint256 symbol2_to_be_given = (local.eth1_supply *
                    ...
                    per1 *
                    amount1 *
                    10**18) / (100 * local.symbol1_supply * eth2_supply);
    ...
    function payEthereum(uint256 amount) public returns (bool) {
        (bool succ, ) = payable(msg.sender).call{value: amount}("");

        return succ;
    }

    function pay(uint256 amount, string memory symbol) public {
        IERC20 _token = IERC20(tokens[symbol]);
