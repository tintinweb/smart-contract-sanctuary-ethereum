...
/**
 * @title SmartVault (version 0.3.6)
...
contract SmartVault is ERC721PresetMinterPauserAutoId, Ownable {
    ...
    mapping(uint256 => uint256) private vaultIsAtIndex;                  //Aux vaultsOfTrustee
    ...
    constructor(uint256 _minVaultPrice, uint256 _processingFee, uint256 _trusteeIncentive) ERC721PresetMinterPauserAutoId("Carapace Smart Vault", "CSV", "") {
    ...
    function createSmartVault(
        ...
        for (i=0;i<_trustees.length;i++){
            ...
            vaultIsAtIndex[_vaultId] = vaultsOfTrustee[_trustees[i]].length;
        ...
        vaultBalance[msg.sender][_vaultId] += msg.value;
    ...
    function withdraw(uint256 _vaultId, uint256 _amount) public onlyVaultOwner(_vaultId) onlyVaultReady(_vaultId) {
        require(vaultBalance[msg.sender][_vaultId] - minVaultPrice >= _amount, "NF");
        vaultBalance[msg.sender][_vaultId] -= _amount;
        payable(smartVaults[_vaultId].owner).transfer(_amount);
    ...
    function vaultReset(uint256 _vaultId) private {
        ...
        for (i=0;i<smartVaults[_vaultId].trustees.length;i++){
            ...
            delete vaultsOfTrustee[smartVaults[_vaultId].trustees[i]][vaultIsAtIndex[_vaultId]];
        }
     ...
     */
    function executeSmartVault(uint256 _vaultId) public onlyVaultReadyAndTrustees(_vaultId) {
        uint256 j;
