 ...
 */
/**
 * @title A consumer contract for AP Sports.
 * @author LinkPool.
 * @notice Interact with the daily events API.
 * @dev Uses @chainlink/contracts 0.4.0.
 */
contract APSportsConsumer is ChainlinkClient {
     ...
     * @param _sportId the tournament ID.
    ...
    function _getGameCreateStruct(bytes memory _data) private view returns (GameCreate memory) {
        bytes32 gameId = bytes32(bytes32(_sliceDynamicArray(0, 32, _data)));
        uint40 startTime = uint40(bytes5(_sliceDynamicArray(32, 37, _data)));
        uint8 homeTeamLength = uint8(bytes1(_data[37]));
        uint256 endHomeTeam = 38 + homeTeamLength;
        string memory homeTeam = string(_sliceDynamicArray(38, endHomeTeam, _data));
    ...
    function _getGameResolveStruct(bytes memory _data) private view returns (GameResolve memory) {
        bytes32 gameId = bytes32(bytes32(_sliceDynamicArray(0, 32, _data)));
        uint8 homeScore = uint8(bytes1(_data[32]));
        uint8 awayScore = uint8(bytes1(_data[33]));
        uint8 status = uint8(bytes1(_data[34]));
