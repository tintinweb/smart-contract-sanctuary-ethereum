...
contract PrecogProfit is IPrecogProfit {
    ...
    function _buyPCOG(
        ...
        if (_profitAmount > 0) {
            ...
            IERC20(_token).safeTransfer(precogStorage.getCore(), _feeTrading);
    ...
    {
        uint _currentProfitId = precogStorage.getCurrentProfitId(token);
        if (precogStorage.getTotalUnitsTradingCycle(token, _currentProfitId) == 0) {
            profitAmount = 0;
        }
        precogInternal.updateCurrentTradingCycle(token, false, 0);
        IERC20(token).safeTransferFrom(msg.sender, address(this), profitAmount);
        require(_currentProfitId < precogStorage.getLastTradingCycle(token).id);
