...
contract BixosverseGame is AccessControl {
     ...
     */
    event GameReward(uint256 nftId, address winner, uint256 amount);
    ...
    function sendReward(
        ...
        uint256 gameId,
        address winner
    ) external onlyIslandExists(nftId) {
        require(
        ...
        require(GameState.Ended == game.state, "Game state must be ended");
        require(game.winner == winner, "User must be winner of the game");
        ...
        emit GameReward({ nftId: nftId, winner: winner, amount: reward });
    }
