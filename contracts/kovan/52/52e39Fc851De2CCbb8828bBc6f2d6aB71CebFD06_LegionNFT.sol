...
contract LegionNFT is Context, ILegionNFT, Ownable {
    ...
    function mint(string memory name, uint256[] memory beastIds, uint256[] memory warriorIds) external {
        ...
        tokenData[_tokenIds.current()] = Legion(name, beastIds, warriorIds, 0, attack_power, block.timestamp);
    ...
    function claimReward() external {
        ...
        taxStartDay[msg.sender] = block.timestamp;
    }
    ...
    function getLegion(uint256 tokenId) external view override returns(string memory, uint256) {
        return (
...
interface ILegionNFT is IERC721 {
    function getLegion(uint256 tokenId) external  view returns(string memory, uint256);
...
contract FeeHandler is IFeeHandler {
    uint[8] fees = [1500,250,200,800,200,20,12,16];
    address legion;
    modifier onlyLegion() {
...
contract RewardPool is IRewardPool, Ownable {
    ...
    function claimReward(address _address, uint256 taxStartDay) external override onlyLegion {
        ...
        uint amountToClaim = (100-2*(20-diffDays)).mul(unclaimedUSD[_address]).div(100);
