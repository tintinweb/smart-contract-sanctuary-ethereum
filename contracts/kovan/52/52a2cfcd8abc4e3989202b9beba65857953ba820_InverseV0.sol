...
// File: contracts/interfaces/IOracleWrapper.sol
...
// File: contracts/libraries/UserLibrary.sol
...
// File: contracts/interfaces/InverseV0Events.sol
...
contract InverseV0 is InverseV0Events, CustomOwnable, ReentrancyGuard {
    ...
    uint256 public miniStakeAmount ;   // Min amount of token that user can stake 1000000000000000000000000000
    uint256 public maxStakeAmount ;   // Max amount of token that user can stake 
    uint256 public betFactorLP;               // 10000000000000000000000000 this is the ratio according to which users can bet considering the amount staked..
    uint256 public miniBetAmount;             // 1000000000000000000000000 min amount that user can bet on.
    uint256 public maxBetAmount;            // 1000000000000000000000000000 max amount that user can bet on.
    ...
    modifier onlyAdmin() {
        require(_msgSender() == adminAddress, "Invalid");
        _;
    }
    ...
    modifier planValidation(uint256 coinType, uint256 planType) {                   //REDUNDANT
    ...
    function initialize(address _owner, address _admin, address _XIVAddress, address _revokeComissionAddress, uint _miniStakeAmount, uint _betFactorLP, uint _miniBetAmount, uint _maxBetAmount, address _oracle, address _usdt) public {
    ...
    function saveBetDetailsOne(address user, uint256 amount, address _coinAddress, address betToken, bool _isInverse) internal {
        ...
        if (betToken != address(XIV)) {
            ...
            // check this condition------ideally ---> (XIV.balanceOf(address(this)) = globalPool)
    ...
    function resolveBet(uint[] memory index, address[] memory user, bool timeCheck) external onlyAdmin nonReentrant {
    ...
    function claimBets() external nonReentrant {
        ...
        for(uint256 i = 0; i < betCounter[_msgSender()]; i++) {
    ...
    function getBetRewards(address user) public view returns (uint256) {
        ...
        for(uint256 i = 0; i < betCounter[user]; i++) {
