...
contract ScoreDB is ScoreConfigs, ChainlinkClient, Pausable, ScoreDBInterface {
  struct LoanRequest {
    uint256 tokenId;
    uint256 loanId;
    bytes32 loanHash; // has of signed encoded investorcontract + loanId
    bytes signature;
    address investor;
  }
  ...
  function fulfill(bytes32 _requestId, uint16 _score)
    ...
    whenNotPaused
  {
    uint256 tokenId = loanRequestTracker[_requestId].tokenId;
    // Emit ScoreRecieved event with tokenId and score
    emit ScoreReceived(tokenId, _score);
    // Revert with particular messages, if score is out of range (currently for scores of 0 and 1000)
    require(_score != NOT_GENERATED, "ScoreDB: Score not yet generated.");
    require(_score != GENERATION_ERROR, "ScoreDB: Error generating score.");
    require(_score < MAX_SCORE && _score > MIN_SCORE, "ScoreDB: Unknown error fetching score.");
    // Update the score cache
    scoreCache[tokenId] = _score;
    // Borrowfulfill logic; not executed in testing mode
    if(testing == false) {
      LoanRequest memory currentLoanRequest = loanRequestTracker[_requestId];
      if (currentLoanRequest.loanId != 0) {
        IInvestor(currentLoanRequest.investor).borrowFulfill(currentLoanRequest.loanId, currentLoanRequest.loanHash, currentLoanRequest.signature);
     }
    }
  }
  ...
  function setTesting(bool _status) public override onlyOwner whenPaused {
    testing = _status;
  }

  function getTesting() public view override returns (bool testingStatus){
    return testing;
  }
   ...
   */
  function setBaseUri(string memory newBaseUri) public override whenPaused onlyOwner {
    baseURI = newBaseUri;
  }

  function setOracle (address _oracle, bytes32 _jobId, uint256 _fee) public override whenPaused onlyOwner {
    oracle = _oracle;
    jobId = _jobId;
    fee = _fee;
  }

  function getOracleDetails() public view override returns (address, bytes32, uint256) {
...
/// @dev Remember to feed LINK to ScoreDB; 0.1 LINK per getLatestScore() call!

interface ScoreDBInterface is IScoreConfigs{
    // Returns the current scored for the token from the on-chain storage.
    function getCurrentScore(uint256 tokenId) external view returns (uint16);
    // Called by the lending contract, initiates logic to update score and fulfill loan.
    function requestUpdatedScoreBorrow(uint256 tokenId, uint256 loanId, bytes32 _hash, bytes memory signature) external;
    // Sets the baseURI of the off-chain API. [OWNER] 
    function setBaseUri(string memory newBaseUri) external;
    // Sets the testing status; if true, borrowFulfill() not called in fulfill() [OWNER]
    function setTesting(bool _status) external;
    // Gets the current testing status
    function getTesting() external view returns (bool testingStatus);
    // Sets a new oracle
    function setOracle (address _oracle, bytes32 _jobId, uint256 _fee) external;
    // Returns the oracle details
    function getOracleDetails() external view returns (address, bytes32, uint256);
    // Pauses the contract [OWNER]
 ...
 */
abstract contract ScoreConfigs is IScoreConfigs, Ownable{
    ...
    */
    function setConfig(address[] memory _tokens, uint16[] memory _scores, uint256[] memory _LTVs, uint256[] memory _LTs) external onlyOwner {
        require(_tokens.length == _scores.length && 
