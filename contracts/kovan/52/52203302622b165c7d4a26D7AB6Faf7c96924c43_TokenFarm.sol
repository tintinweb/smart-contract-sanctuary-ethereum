...
contract TokenFarm is Ownable {
    ...
    function stakeTokens(uint256 _amount, address _token) public {
        // what tokens an they stake ?
        //  how much can they stake ?
        require(_amount > 0, "amount must be more than zero");
        // token is allowed they stake ??
        require(
            tokenisallowed(_token),
            "token is not allowed in the current time"
        );
    ...
    function unstakeTokens(address _token) public {
        ...
        require(balance > 0, "Stacking balance can't be 0");
    ...
    function tokenisallowed(address _token) public returns (bool) {
        for (
            ...
            allowedToken < allowedTokens.length;
            allowedToken++
        ) {
            if (allowedTokens[allowedToken] == _token) {
                return true;
            }
        }
        return false;
