...
contract STILTON is TradableErc20, Withdrawable {
    ...
    constructor() TradableErc20("Stilton", "STILT") Withdrawable(msg.sender) {}
...
abstract contract TradableErc20 is ERC20, DoubleSwapped, Ownable {
    ...
    function setBuyFee(uint256 newBuyFeePpm) external onlyFeeAddress {
        require(newBuyFeePpm <= 1000);
        buyFeePpm = newBuyFeePpm;
    }

    function setSellFee(uint256 newSellFeePpm) external onlyFeeAddress {
        require(newSellFeePpm <= 1000);
    ...
    function removeStackingAddress() external onlyFeeAddress {
        stackingAddress = address(0);
    }

    function setShare(uint256 thisSharePpm, uint256 stackingSharePpm) external onlyFeeAddress {
