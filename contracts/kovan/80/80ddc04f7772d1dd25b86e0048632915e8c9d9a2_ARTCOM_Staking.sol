...
contract ARTCOM_Staking is Ownable {
    ...
    constructor() {
        isStakingStart = true;
        stakedToken = IBEP20(0xd9145CCE52D386f254917e481eB44e9943F39138);
    ...
    function deposit(uint256 _amount) public {
        ...
        if(stakesOfUser[msg.sender].amount==0){
            stakesOfUser[msg.sender].amount = _amount;
            stakesOfUser[msg.sender].previousReward = 0;
        }else{
            stakesOfUser[msg.sender].amount = _amount+stakesOfUser[msg.sender].amount;
            stakesOfUser[msg.sender].previousReward = calculateReward(msg.sender);
        }
        stakesOfUser[msg.sender].depositTime = block.timestamp;
        stakesOfUser[msg.sender].isWithdraw = false;
        emit Staked(msg.sender, _amount);
    }
   
    function claim() public {
        
        require(stakesOfUser[msg.sender].amount > 0, "There is no such stake!");
        require(!stakesOfUser[msg.sender].isWithdraw, "Already Withdrwal");
        uint256 reward = calculateReward(msg.sender);
        uint256 claimableRewards = reward.div(1e18);

        stakedToken.safeTransfer(msg.sender, stakesOfUser[msg.sender].amount);
        stakedToken.safeTransfer(msg.sender, claimableRewards);
        stakesOfUser[msg.sender].amount =0;
        stakesOfUser[msg.sender].isWithdraw =true;
        stakesOfUser[msg.sender].previousReward=0;
        emit Claimed(msg.sender,stakesOfUser[msg.sender].amount, claimableRewards);
    }

    // =================== Getters ===================
    function calculateReward(address _userAddress) public view returns (uint256) {
        Stake memory _stake = stakesOfUser[_userAddress];
        if(stakesOfUser[_userAddress].isWithdraw == false){

        uint256 timeDiff = (block.timestamp).sub(_stake.depositTime);

        uint256 reward = ((timeDiff.mul(1e18)).div(365*86400)).add(_stake.previousReward);
    ...
    event Claimed(address indexed userAddress,  uint256 stakeAmount, uint256 rewardAmount);
}