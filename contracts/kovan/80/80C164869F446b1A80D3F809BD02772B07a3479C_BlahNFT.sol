...
contract BlahNFT is BlahNFTScore {
    ...
    function uploadMetadata(BlahNFTTypes.CityWithId[] memory _cities, BlahNFTTypes.CountryToScore[] memory _countryToScore) external onlyOwner {
        ...
        for(uint i = 0; i < _cities.length; i++) {
            ...
            if(idToCities[_cities[i].id].points == 0 && _exists(_cities[i].id)) {
                idToCities[_cities[i].id] = BlahNFTTypes.City(_cities[i].city, _cities[i].country, _cities[i].points, _cities[i].cityFaction);
                addressToFaction[ownerOf(_cities[i].id)] = _cities[i].cityFaction;
            }
        }
        for(uint i = 0; i < _countryToScore.length; i++) {
            countryToScore[_countryToScore[i].country] = BlahNFTTypes.CountryScore(_countryToScore[i].red, _countryToScore[i].green, _countryToScore[i].blue);
        }
    }
    ...
    function _transfer(
        ...
        if(addressToFaction[from] != addressToFaction[to]) {
            ...
            BlahNFTTypes.CountryScore storage countryScore = countryToScore[city.country];
            uint8 winningFaction = getWinningFaction(countryScore.red, countryScore.green, countryScore.blue);//countryToFaction[city.country];
    ...
    function tokenURI(uint256 _tokenId) public view virtual override returns (string memory) {
        ...
        string memory attributes = string(
            abi.encodePacked(
                ...
                '"value": "', factionToColorName(countryToFaction[city.country]),
                '"}'
            )
        );
        ...
        string memory imageString = getImage(city);//"https://gateway.pinata.cloud/ipfs/Qmf5gV47UyV3j6D3oY49vC6t4J8ArhWBCYJ3RoUzgLjV9u";
                ...
                string(
                    abi.encodePacked(
                        '{',
                            ...
                            ', "image": ', '"', imageString,
                            '", "description": "BlahNFT is a NFT game with 3 factions all on chain.",',
                            '"attributes": [',
                                attributes,
                            ']',
                        '}'
    ...
    function getImage(BlahNFTTypes.City memory city) public view returns (string memory) {
        string memory imageBytes = toHex(
            keccak256(
                abi.encodePacked(
                    uintToByteString(city.points, 2),
                    city.city,
                    city.country
                )
            )
        ...
        return string(
            abi.encodePacked(
                ...
                "-",
                uintToByteString(countryToFaction[city.country], 1),
                ".png"
            )
        );
    }
...
import "./BlahNFTNFT.sol";

contract BlahNFTScore is BlahNFTNFT {
    
    string[] countries = ["Japan","United States","South Korea","France","United Kingdom","Russia","China","Mexico","India","Argentina","Brazil","Canada","Australia","Spain","Italy","Singapore","Indonesia","Taiwan","Turkey","Philippines","Peru","Germany","Saudi Arabia","Chile","Pakistan","Thailand","Kuwait","Colombia","Sweden","Bangladesh","Israel","Netherlands","Denmark","Austria","Poland","Egypt","Belgium","Ireland","United Arab Emirates","Vietnam","Switzerland","Nigeria","Greece","Finland","South Africa","New Zealand","Venezuela","Czech Republic","Norway","Romania","Portugal","Malaysia","Hungary","Uruguay","Ukraine","Morocco","Kazakhstan","Bulgaria","C\u00f4te D\u2019Ivoire","Lithuania","Luxembourg","Tanzania","Belarus","Serbia","Estonia","Kenya","Latvia","Afghanistan","Georgia","Iraq","Zambia","Iran","Ecuador","Mali","Paraguay","DR Congo","Republic of the Congo","Syria","Mozambique","Ghana","Malawi","Honduras","Myanmar","Guinea","Bolivia","Uganda","Burkina Faso","Oman","Mongolia","Qatar","Azerbaijan","Uzbekistan","Cameroon","Guatemala","Cuba","North Korea","Sudan","Yemen","Ethiopia","Algeria","Jordan","Dominican Republic","Armenia","Somalia","Madagascar","Angola","Cambodia","Zimbabwe","Mauritania","Togo","Niger","Gabon","Haiti","Nepal","Eritrea","Sierra Leone","Central African Republic","Panama","Nicaragua","Kyrgyzstan","Turkmenistan","Moldova","Rwanda","Senegal","Libya","Chad","Benin","Laos","Sri Lanka","Burundi","North Macedonia","Jamaica","Tunisia","El Salvador","Djibouti","Tajikistan","Croatia","Liberia","Lebanon","Kosovo","Cyprus","Lesotho","Equatorial Guinea","Guyana","Timor-Leste","Suriname","Botswana","Bahamas","Bosnia and Herzegovina","Slovenia","Montenegro","Costa Rica","Namibia","Slovakia","Mauritius","Papua New Guinea","South Sudan","Guinea-Bissau","Albania","Gambia","Cabo Verde","Iceland","Maldives","Vanuatu","Comoros","Barbados","Bhutan","Bahrain","Eswatini","Sao Tome and Principe","St. Lucia","Solomon Islands","Fiji","Kiribati","Tonga","Antigua and Barbuda","St. Vincent and The Grenadines","Seychelles","Marshall Islands","Andorra","Liechtenstein","Trinidad and Tobago","Samoa","Monaco","St. Kitts and Nevis","Brunei","Bermuda","Malta","Vatican City","Belize","Dominica","Micronesia","San Marino","Grenada","Tuvalu"];
    
    function hasUploadedMetadata(BlahNFTTypes.City memory city) internal pure returns (bool) {
    ...
    function getOverallLosingFaction() public view returns (uint8) {
        (uint redScore, uint greenScore, uint blueScore) = calculateScores();
        return getLosingFaction(redScore, greenScore, blueScore);
    }

    function calculateCountryScore(string memory _country) public view returns (uint, uint, uint) {
        uint redPoints = 0;
        uint greenPoints = 0;
        uint bluePoints = 0;

        for(uint i = 0; i < getTokenIds(); i++) {
            uint id = i + 1;
            BlahNFTTypes.City memory city = idToCities[id];
            if(!hasUploadedMetadata(city)) {        //only count cities that have their metadata
                continue;
            }
            if(keccak256(abi.encodePacked(city.country)) == keccak256(abi.encodePacked(_country))) {
                if(city.cityFaction == 1) {
                    redPoints += city.points;
                } else if(city.cityFaction == 2) {
                    greenPoints += city.points;
                } else if(city.cityFaction == 3) {
                    bluePoints += city.points;
                }
            }
        }
        for(uint i = MAX_CITIES - FOUNDERS_RESERVE_AMOUNT; i < MAX_CITIES - FOUNDERS_RESERVE_AMOUNT + getReservedTokenIds(); i++) {
            uint id = i + 1;
            BlahNFTTypes.City memory city = idToCities[id];
            if(!hasUploadedMetadata(city)) {        //only count cities that have their metadata
                continue;
        ...
        return (redPoints, greenPoints, bluePoints);        //RGB
    }

    function calculateScores() public view returns (uint, uint, uint) {
        uint redScore = 0;
        uint greenScore = 0;
        uint blueScore = 0;

        //TODO: check for end index issue here
        for(uint i = 0; i < countries.length; i++) {
            string memory country = countries[i];
            (uint redPoints, uint greenPoints, uint bluePoints) = calculateCountryScore(country);
            uint8 winningFaction = getWinningFaction(redPoints, greenPoints, bluePoints);
            if(winningFaction == 1) {
                redScore += (redPoints + greenPoints + bluePoints);
            } else if(winningFaction == 2) {
                greenScore += (redPoints + greenPoints + bluePoints);
            } else if(winningFaction == 3) {
                blueScore += (redPoints + greenPoints + bluePoints);
            }
        }
        return (redScore, greenScore, blueScore);           //RGB
    }
}
...
import "./BlahNFTTypes.sol";
import { Base64 } from "./libraries/Base64.sol";

contract BlahNFTNFT is ERC721, Ownable, ReentrancyGuard {
    mapping(uint256 => BlahNFTTypes.City) public idToCities;
    mapping(address => uint8) public addressToFaction;
    mapping(string => uint8) public countryToFaction; //1 red, 2 green, 3 blue
    mapping(string => BlahNFTTypes.CountryScore) public countryToScore;

    event GeneratedCity(uint256 indexed tokenId, uint256 dna);
    using Counters for Counters.Counter;
    Counters.Counter private _tokenIds;
    Counters.Counter private _reservedTokenIds;

    uint256 internal constant MAX_CITIES = 3000;
    uint256 internal constant FOUNDERS_RESERVE_AMOUNT = 25;
    ...
    string placeHolderImage = "someimage.com";
    string baseUrl = "";
    ...
    constructor() ERC721("BlahNFT", "Unite factions!") {
    ...
    function removeFromEarlyAccessList(address[] memory _toRemove) external onlyOwner {
        for (uint256 i = 0; i < _toRemove.length; i++) {
            isOnEarlyAccessList[_toRemove[i]] = false;
        }
    }

    function addToFaction(BlahNFTTypes.AddressToFaction[] memory _toFactions) external onlyOwner {
