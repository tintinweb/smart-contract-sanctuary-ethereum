...
contract uRevolt is Initializable, UUPSUpgradeable, ERC20Upgradeable, ERC20PermitUpgradeable, ERC20VotesUpgradeable, OwnableUpgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable{
    ...
    function randomSelectUsers(uint16 numberOfUsers, uint256[] memory ids) external randomGenerationInProgress nonReentrant{
            for(uint256 i=0; i< numberOfUsers; i++) {
                is_mandator[lastFulfilledId][stakerId[ids[i]]] = true;
                selectedIndexes.push(ids[i]);
                UserInfo storage user = userInfo[0][stakerId[ids[i]]];
                _deposit(lastFulfilledId, stakerId[ids[i]], user.amount);
                }
    ...
    function rand(uint256[] memory ids) external {
