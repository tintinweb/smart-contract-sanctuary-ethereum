...
contract PrimaryContract {
  ...
  function purchaseTokens(uint256 amount, bytes32[] calldata proof) public returns (bool success) {
    require(!checkAirdropClaimed(msg.sender), "Airdrop claimed. Ineligible for purchase");
    require(verifyMerkleProof(msg.sender, amount, proof));
    require(amount >= baseTier, "Insufficient tokens to participate in purchase"); // 100 whole unit requirement
    require(amount <= proTier, "Ineligible to participate in purchase. See airdrop instead"); // 2000 whole unit maximum

    uint256 allocation;
    uint256 usdcCost;

    if (amount >= 3500000) { // < 2000 due to the above require case, >= 350 tokens
      // entitled to 2000 - amount
      allocation = remainingAllocation(msg.sender, amount, proof);
  ...
  // @param participant (address)
  // @return bool
  function checkAirdropEligibility(address participant, uint256 amount, bytes32[] calldata proof) public view returns (bool) {
    require(verifyMerkleProof(participant, amount, proof));
  ...
  // @param participant (address)
  // @return bool
  function checkPurchaseEligibility(address participant, uint256 amount, bytes32[] calldata proof) public view returns (bool) {
  ...
  function initialAllocation(address participant, uint256 amount, bytes32[] calldata proof) public view returns (uint256 allocation) {
    require(verifyMerkleProof(participant, amount, proof));
  ...
  // @param participant (address) 
  // @return allocation (uint256)
  function remainingAllocation(address participant, uint256 amount, bytes32[] calldata proof) public view returns (uint256 allocation) {
    require(verifyMerkleProof(participant, amount, proof));
  ...
  function verifyMerkleProof(address _address, uint256 _amount, bytes32[] calldata _proof) internal view returns (bool valid) {
