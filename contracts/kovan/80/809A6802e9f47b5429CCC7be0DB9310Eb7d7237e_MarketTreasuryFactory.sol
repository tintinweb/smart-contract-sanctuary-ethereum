...
import "../market_treasury/MarketTreasury.sol";

interface ITreasury {
    function initialize(address client) external;
}

contract MarketTreasuryFactory {
    bytes32 public constant TREASURY_CONTRACT_HASH = keccak256(abi.encodePacked(type(MarketTreasury).creationCode));

    address[] public allContracts;

    event TreasuryContractCreated(
    ...
    function createContract(address client) external returns (address contractCreated) {
        // bytes memory bytecode = type(MarketTreasury).creationCode;
        // bytes32 salt = keccak256(abi.encodePacked(client));
        // assembly {
        //     contractCreated := create2(0, add(bytecode, 32), mload(bytecode), salt)
        // }

        // ITreasury(contractCreated).initialize(client);

        contractCreated = address(new MarketTreasury());
        ITreasury(contractCreated).initialize(client);

        allContracts.push(contractCreated);

        emit TreasuryContractCreated(contractCreated, client);
    }
}
...
/**
 * @notice The Market treasury which collects fees generated by the market.
