...
contract Master {
    ...
    function getBid(uint id) external view returns(bool, bool, uint, uint, uint, uint, address payable, address payable, string[4] memory) {
        Bid storage bid = bids[id];
        require(bid.owner == msg.sender || bid.beneficiary == msg.sender);
        return(bid.answered, bid.withdrawn, bid.timestamp, bid.deltaBlockNumber, bid.value, bid.sum, bid.owner, bid.beneficiary, bid.messages);
    }
