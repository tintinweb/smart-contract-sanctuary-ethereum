...
contract TPCore is AccessControl {
    ...
    function estimateTPYReward(address addr) public view returns(uint136) {
        ...
        if(lastCompleteInterval >= staking.intervalNumber) {
            ...
            if((staking.pendingAmount == 0 && staking.pendingAmount2 == 0 ) || staking.intervalNumber == lastCompleteInterval) {
            ...
            else if(staking.pendingAmount > 0 && staking.intervalNumber < lastCompleteInterval) {

                if(staking.pendingAmount2 > 0 && staking.intervalNumber + 1 < lastCompleteInterval) {

                    totalstakingAmount = staking.amount + staking.pendingAmount;
                    uint256 i = staking.intervalNumber + 1;
                    stakedUntilInterval += intervalStakes[i];
                    stakedUntilInterval -= intervalUnstakes[i];
                    estimatedTPYs += totalstakingAmount * getTPYIntervalReward(i) / stakedUntilInterval;

                    totalstakingAmount = staking.amount + staking.pendingAmount + staking.pendingAmount2;
                    for(i = staking.intervalNumber + 2; i <= lastCompleteInterval; i++) {
                        stakedUntilInterval += intervalStakes[i];
                        stakedUntilInterval -= intervalUnstakes[i];
                        if(stakedUntilInterval == 0) {
                        continue;
            ...
            else if(staking.pendingAmount2 > 0 && staking.intervalNumber < lastCompleteInterval) {
                uint256 i = staking.intervalNumber + 1;
                stakedUntilInterval += intervalStakes[i];
                stakedUntilInterval -= intervalUnstakes[i];
                estimatedTPYs += staking.amount * getTPYIntervalReward(i) / stakedUntilInterval;

                totalstakingAmount = staking.amount + staking.pendingAmount2;
                for( i = staking.intervalNumber + 2; i <= lastCompleteInterval; i++) {
                    stakedUntilInterval += intervalStakes[i];
                    stakedUntilInterval -= intervalUnstakes[i];
                    if(stakedUntilInterval == 0) {
    ...
    function registerTPY(address staker, uint104 stakingCoinAmount) private {
        ...
        if(userStaking[staker].intervalNumber > 0) {// exists
            ...
            else if(staking.intervalNumber + 1 >= currentIntervalNumber) {
                staking.pendingAmount2 += stakingCoinAmount;
            }
            else {
                uint136 currentReward = estimateTPYReward(staker);
                staking.amount += uint104(staking.pendingAmount);
                staking.pendingAmount = staking.pendingAmount2;
                staking.pendingAmount2 = 0;
                staking.intervalNumber++;
                if(staking.intervalNumber + 1 >= currentIntervalNumber) {
                    staking.pendingAmount2 += stakingCoinAmount;
                }
                else {
                    staking.amount += uint104(staking.pendingAmount);
            ...
            userStaking[staker] = Staking(nextIntervalNumber, stakingCoinAmount, 0, 0, 0);
        }
