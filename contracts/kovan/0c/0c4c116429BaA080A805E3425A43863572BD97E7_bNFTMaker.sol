...
contract bNFTMaker is bNFTBase {
    function initialize(
        string memory name,
    ...
    function _exists(uint tokenId)
        ...
        returns (bool)
    {
        return IMarketPositionTransfer(market).existsMakerPosition(tokenId);
    }

    function ownerOf(uint tokenId) public view override returns (address) {
        return IMarketPositionTransfer(market).ownerOfMakerPosition(tokenId);
    }
    ...
    function _transfer(
        ...
        IMarketPositionTransfer(market).safeTransferMakerPosition(
            from,
            to,
            tokenId
        );
