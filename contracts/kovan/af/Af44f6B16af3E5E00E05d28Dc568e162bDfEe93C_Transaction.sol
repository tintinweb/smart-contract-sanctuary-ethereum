...
contract Transaction is ERC2771Context, Ownable {
    ...
    function transfer(uint256 amount) payable public {
        require(_msgSender().balance > amount, "Balance doesn't enough for transaction");
        emit Bought(amount);
    }

    function refund(uint256 amount) payable public {
        require(address(this).balance > amount, "Contract's balance doesn't enough for transaction");
        emit Sold(amount);
    }
