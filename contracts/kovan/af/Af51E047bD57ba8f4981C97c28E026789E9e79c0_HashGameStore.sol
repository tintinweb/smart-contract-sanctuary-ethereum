...
contract HashGameStore {
    ...
    */
    ...
    function getGamePrice(uint256 gameID) public view returns (uint256) {
    ...
    // Get lowest key price for a specific game
    /* NOTE: (CHANGE LATER)
        This function should instead return the lowest key instead of 
        the price.

        It should not be possible to buy specific keys, only 'buyLowestPriceResoldKey'
        should be possible.
     */
        ...
        returns (address)
    {
        ...
        for (uint256 i = 0; i < registeredAddresses.length; i++) {
            ...
            if (
                addressToGameIDToKeysForSale[a][gameID] > 0 && // There are any keys on sale
                addressToGameIDToPrice[a][gameID] < lowestPrice // Price is lower than currently tracked one
            ) {
                lowestPrice = addressToGameIDToPrice[a][gameID]; // Update lowest tracked price
