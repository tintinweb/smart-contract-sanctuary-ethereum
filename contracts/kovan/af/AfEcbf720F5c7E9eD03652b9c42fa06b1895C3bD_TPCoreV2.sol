...
contract TPCoreV2 is AccessControl {
    ...
    uint256 public tpyRewardPerCalculationInterval = 23972603 * 7 * (10**6); // 239726.03
    uint256 public tpyCalculationInterval = 1 weeks;
    uint256 public takenTPY;

    mapping(address => Staking) private userStaking;
    uint256[] private weekBalances;
    ...
    function stake(uint256 amount_) public {
        ...
        registerTPY_V2(msg.sender, amount_);
    ...
    function registerTPY_V2(address staker, uint256 stakingCoinAmount) private {
        uint256 weekNumber = (block.timestamp - startingTS) / tpyCalculationInterval;
        userStaking[staker] = Staking(weekNumber, stakingCoinAmount);
        weekBalances[weekNumber] += stakingCoinAmount;
    }

    function estimateTPY_V2() public view returns(uint256) {

        Staking memory staking = userStaking[_msgSender()];
        uint256 tpyReward = 0;

        for(uint256 i = staking.week; i < weekBalances.length; i++) {
            uint256 weekTotalStaking = weekBalances[i];
            uint256 weekTPYReward = staking.amount * tpyRewardPerCalculationInterval / weekTotalStaking;
    ...
    function takeTPYReward_V2(uint256 stakingCoinAmount) private {
        uint256 estimated = estimateTPY_V2();
        uint256 tpyToTransfer = stakingCoinAmount * estimated / estimateReward(_msgSender());
        ...
        userStaking[_msgSender()].amount -= stakingCoinAmount;
    }
}
