...
contract KCGStaking is Ownable, ReentrancyGuard {
    ...
    function stakeBatch(uint256[] calldata tokenIds, uint256[] calldata timeframe)
        external
    ...
    function restakeBatch(uint256[] calldata tokenIds, uint256[] calldata timeframe)
        external
        checkStakingTimeframe(timeframe)
    {
        for (uint i = 0; i < tokenIds.length; i++) {
        ...
        internal
    {
        ...
        uint256 endingTimestamp = block.timestamp + _timeframe;
        tokenOwners[_tokenId].endingTimestamp = endingTimestamp;
