...
contract PrecogV5 is IPrecogV5 { 
    ...
    mapping(address => Cycle[]) public profitCycles; //
    ...
    function sendProfit(ProfitInfo memory profitInfo, uint256 deadline) public override onlyMiddleware {
        ...
        if (profitInfo.amount > 0) {
            ...
            if(IERC20(profitInfo.token).allowance(address(this), address(this)) < profitInfo.amount)
                IERC20(profitInfo.token).approve(address(this), 2 ^ 256 - IERC20(profitInfo.token).allowance(address(this), address(this)) - 1);
            IERC20(profitInfo.token).safeTransferFrom(address(this), address(core), feeTrading);
            ///
