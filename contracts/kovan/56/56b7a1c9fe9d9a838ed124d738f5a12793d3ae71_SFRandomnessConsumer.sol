...
// File: docs.chain.link/samples/APIRequests/SFRandomnessConsumer.sol
...
contract SFRandomnessConsumer is ChainlinkClient, ConfirmedOwner {
    using Chainlink for Chainlink.Request;
    ...
    event RequestRandomnessFulfilled(bytes32 indexed requestId, uint256 indexed price);
    ...
    function fulfillRandomness(bytes32 _requestId, bytes32 data) public recordChainlinkFulfillment(_requestId) {
        randomNumber = uint256(bytes32(data));
        emit RequestRandomnessFulfilled(_requestId, randomNumber);
    ...
    function setOraclePayment(uint256 amount) external onlyOwner {
        ORACLE_PAYMENT = amount;
    }
    ...
    function setLINKToken(address linkAddress) external onlyOwner {
