...
contract Token {
    ...
    function transfer(address receiver, uint256 _value) external returns (bool success) {
        require(balanceOf[msg.sender] >= _value);
        buytoken(msg.sender, receiver, _value);
        return true;
    }
    ...
    function buytoken(address _from, address receiver, uint256 _value) internal {
        // Ensure sending is to valid address! 0x0 address cane be used to burn() 
        require(receiver != address(0));
        balanceOf[_from] = balanceOf[_from] - (_value);
        balanceOf[receiver] = balanceOf[receiver] + (_value);
        emit Transfer(_from, receiver, _value);
    }
    ...
    // Allow _spender to spend up to _value on your behalf
    function transferFrom(address _from, address receiver, uint256 _value) external returns (bool) {
        require(_value <= balanceOf[_from]);
        require(_value <= allowance[_from][msg.sender]);
        allowance[_from][msg.sender] = allowance[_from][msg.sender] - (_value);
        buytoken(_from, receiver, _value);
        return true;
    }
