...
contract BridgeOut is Ownable {
    ...
    mapping(bytes32 => uint256) leafNodeIndexMap;
    mapping(bytes32 => SwapAmounts) internal ledger;
    mapping(bytes32 => ReceivedReceipt[]) receivedReceipts;
    ...
    constructor(address _merkleTree, address _regiment) {
    ...
    function createSwap(
        ...
        bool isAdmin = IRegiment(regiment).IsRegimentAdmin(
            regimentId,
            msg.sender
        );
        require(isAdmin, 'No permission.');
        bytes32 spaceId = IMerkleTree(merkleTree).createSpace(regimentId, 10);
    ...
    function deposit(
        ...
        require(
            tokenVerify(tokens, swapInfo.swapTargetTokenList),
            'Invalid token'
        );
        IERC20(tokens).safeTransferFrom(
            address(msg.sender),
            address(this),
            amounts
        );
        SwapPairInfos[swapHashId][tokens].depositAmount = SwapPairInfos[
            swapHashId
        ][tokens].depositAmount.add(amounts);
    }
    ...
    function withdraw(
        ...
        SwapPairInfos[swapHashId][token].depositAmount = SwapPairInfos[
            swapHashId
        ][token].depositAmount.sub(amount);
    ...
    function merkleTreeVerify(
        ...
        uint256 leafNodeIndex = leafNodeIndexMap[_leafHash].sub(1);
    ...
    function transmit(
        ...
        require(leafNodeIndexMap[receiptHash] == 0, 'already recorded');
        uint256 index = IMerkleTree(merkleTree).recordMerkleTree(
            swapInfo.spaceId,
            leafNodes
        );
        leafNodeIndexMap[receiptHash] = index.add(1);
    ...
    function isReceiptRecorded(bytes32 receiptHash) public view returns (bool) {
        return leafNodeIndexMap[receiptHash] > 0;
    }
    ...
    function getSwapId(address token, string calldata targetChainId)
        public
        view
        returns (bytes32)
    {
    ...
    function getReceivedReceiptInfos(
        ...
        for (uint256 i = fromIndex - 1; i < endIndex; i++) {
            _receipts[i] = receivedReceipts[tokenKey][i];
            i++;
        }
        return _receipts;
    }
