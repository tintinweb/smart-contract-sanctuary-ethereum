...
contract Escrow is Ownable, ReentrancyGuard {
    ...
    /// @dev Only contest owner can end this contest.
    /// @param _contestId Contest id to end
    /// @param _winners Array of winner address
    function endContest(uint256 _contestId, address[] memory _winners) external {
        ContestInfo storage contest = contests[_contestId];

        if (msg.sender != contest.contestOwner) {
            revert EscrowError(EscrowErrorCodes.NOT_CONTEST_OWNER);
        }
