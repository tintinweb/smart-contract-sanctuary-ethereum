...
interface Config {
    function getFeeDeposit() external view returns (uint);

    function getFeeDepositGem() external view returns (uint);

    function getFeeWithdraw() external view returns (uint);

    function getFeeWithdrawGem() external view returns (uint);
...
contract DssProxyActions is Common {
    ...
    function getFee(address token, uint percentDeposit) public view returns (uint percent) {
        percent = !Config(config).checkToken(token) ? percentDeposit : 0;
    }
    ...
    function lockETH(
        ...
        address token = address(GemJoinLike(ethJoin).gem());
        uint _caculatedFee = getFee(token, Config(config).getFeeDeposit());
    ...
    function lockGem(
        ...
        address token = address(GemJoinLike(gemJoin).gem());
        uint _caculatedFee = getFee(token, Config(config).getFeeDepositGem());
    ...
    function freeETH(
        ...
        address token = address(GemJoinLike(ethJoin).gem());
        uint _caculatedFee = getFee(token, Config(config).getFeeWithdraw());
    ...
    function lockETHAndDraw(
        ...
        bytes32 ilk = ManagerLike(manager).ilks(cdp);
        address token = address(GemJoinLike(ethJoin).gem());
        uint _fee = msg.value * getFee(token, Config(config).getFeeDeposit()) / decimal;
        // Receives ETH amount, converts it to WETH and joins it into the vat
    ...
    function lockGemAndDraw(
        ...
        bytes32 ilk = ManagerLike(manager).ilks(cdp);
        address token = address(GemJoinLike(gemJoin).gem());
        uint _fee = amtC * getFee(token, Config(config).getFeeDepositGem()) / decimal;
        // Takes token amount from user's wallet and joins into the vat
    ...
    function wipeAndFreeGem(
        ...
        // Exits token amount to the user's wallet as a token
        address token = address(GemJoinLike(gemJoin).gem());
        uint _caculatedFee = getFee(token, Config(config).getFeeWithdrawGem());
        uint _fee = wadC * _caculatedFee / decimal;
        if (_fee > 0) {
            GemJoinLike(gemJoin).gem().transferFrom(msg.sender, Config(config).getTreasury(), _fee);
        }
