    ...
    IStructs
{
    ...
    function deployPool(
        ...
        if (_feeRate > 1000000) revert FeeTooLarge();

        address lendingPool = address(
            new ERC1967Proxy(poolImplementationAddress, "")
        );

        uint48 discountValue = licenseDiscount();
        ...
        ILendingPool(lendingPool).initialize(
            Data(
                ...
                _expiry,
                _borrowers,
                (protocolFee * (1000000 - discountValue)) / 1000000,
                (protocolColFee * (1000000 - discountValue)) / 1000000,
                address(feesManager),
                oracle
            )
        );
        ...
        feesManager.setPoolFees(lendingPool, _feeRate, _type);
    ...
    function setOracle(address _oracle) external whenNotPaused {
        onlyOwner();
        oracle = _oracle;
    }
    ...
    function pause() public {
        if (msg.sender != firstResponder) revert NotFirstResponder();
        _pause();
    }
    ...
    /// @notice Check if sender has a license
    /// @return the largest discount across all lenders licenses
    function licenseDiscount() private returns (uint48) {
        // Call the licence engine and see if user has balance else return 0
        uint256 bal = licenseEngine.balanceOf(msg.sender);
        uint48 maxDiscount = 0;
        uint256 licId = 0;
        if (bal <= 0) {
            return maxDiscount;
        } else {
            for (uint256 index = 0; index < bal; index++) {
                ...
                (uint256 maxCount, uint256 curCount, uint48 discount, uint48 expiry, ) = licenseEngine.licenses(id);
                if (block.timestamp < expiry && curCount + 1 <= maxCount && discount >= maxDiscount){
                    maxDiscount = discount;
                    licId = id;
                }
            }
        }
        if (maxDiscount < 0 || maxDiscount > 1000000) revert DiscountTooLarge();
        if (maxDiscount > 0) {
            licenseEngine.incrementCurrentPoolCount(licId);   
        }
        return maxDiscount;
    }
...
interface ILendingPool is IStructs {
    ...
    function borrowOnBehalfOf(address _borrower, uint256 _colDepositAmount, uint256 _rate)
        external;
...
interface ILicenseEngine is IERC721, IERC721Enumerable {
    struct LicenseInfo {
        ...
        uint48 discount; // Discount applies to both collateral and lend tokens
