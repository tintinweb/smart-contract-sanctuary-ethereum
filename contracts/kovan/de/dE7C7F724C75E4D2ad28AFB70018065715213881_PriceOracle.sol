...
import "./oracles/ChainlinkPriceOracleV3.sol";
import "./oracles/UniswapV3TwapPriceOracle.sol";

/**
 * @title PriceOracle
 * @notice Returns prices from ChainlinkPriceOracleV3, UniswapV3TwapPriceOracle, UniswapV2TwapPriceOracle (in this particular order).
 * @dev Implements `BasePriceOracle`.
 */
contract PriceOracle is BasePriceOracle {
    /**
     * @dev `ChainlinkPriceOracleV3`.
     */
    ChainlinkPriceOracleV3 public chainlinkOracle;

    /**
     * @dev  `UniswapV3TwapPriceOracle`.
     */
    UniswapV3TwapPriceOracle public uniswapV3Oracle;

    /**
     * @dev Constructor to set the oracles.
     */
    constructor(
        ChainlinkPriceOracleV3 _chainLinkOracle,
        UniswapV3TwapPriceOracle _uniswapV3Oracle
    ) public {
        require(
            address(_chainLinkOracle) != address(0),
            "ChainlinkPriceOracleV3 not set."
        );
        require(
            address(_uniswapV3Oracle) != address(0),
            "UniswapV3TwapPriceOracle not set."
        );
     ...
     * @return Price denominated in ETH (scaled by 1e18). Returns 0 for no price
     */
    ...
    /**
     * @notice Fetches the token/USD price, with 18 decimals of precision.
     * @param token The token address for which to get the price.
     * @return Price denominated in USD (scaled by 1e18)
     */
    function priceinUSD(address token) external view returns (uint256) {
        uint256 priceinETH = _price(token);
        uint256 ethUSDPrice = (uint256)(chainlinkOracle.getEthUsdPrice());
        return priceinETH * ethUSDPrice;
    }

    /**
     * @dev Internal function returning the price in ETH of `token`. Returns 0 for no price
     */
    function _price(address token) internal view returns (uint256) {
        // Try to get Chainlink price
        uint256 chainLinkPrice = chainlinkOracle.price(token);
        if (chainLinkPrice != 0) return chainLinkPrice;

        // Try to get UniswapV3 price
        uint256 uniswapV3Price = uniswapV3Oracle.price(token);
        if (uniswapV3Price != 0) return uniswapV3Price;
