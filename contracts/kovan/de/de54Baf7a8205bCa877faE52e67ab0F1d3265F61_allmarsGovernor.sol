...
contract allmarsGovernor is ERC721Holder, Ownable {
    using Address for address;
    ...
    uint256 public minTerm = 5 * 60;
    uint256 public maxTerm = 10 * 60;
...
//    uint256 public minTerm = 24 * 60 * 60;
    ...
    modifier isInitiated() {
        ...
        require(bpoolContract != address(0), "Balancer Pool contract is not set.");
        _;
    }
    ...
    function setBPOOL(address account) public onlyOwner {
        require((account != address(0) && account.isContract()), "Invalid contract address");
        bpoolContract = account;
    }
    ...
    function getExchangeRate() public view returns(uint256) {
        uint256 result = IBPool(bpoolContract).calcInGivenOut(
                        IBPool(bpoolContract).getBalance(erc20Contract),
                        IBPool(bpoolContract).getNormalizedWeight(erc20Contract),
                        IBPool(bpoolContract).getBalance(wethContract),
                        IBPool(bpoolContract).getNormalizedWeight(wethContract),
                        wethRate,
                        IBPool(bpoolContract).getSwapFee()
                );
        return result;
    }
    ...
    function claimNFT(uint256 tokenId, address token) public isInitiated {
        ...
        if (_deposits[tokenId].timestamp + minTerm > block.timestamp) {
            ...
            if (token == wethContract) {
                ...
                sendNFT(_msgSender(), tokenId);
                IERC20(wethContract).safeTransfer(bpoolContract, wethRate * BALANCER_SHARE / 100);
                IBPool(bpoolContract).gulp(wethContract);
            } else  {
                uint256 tokenAmount = getExchangeRate();
