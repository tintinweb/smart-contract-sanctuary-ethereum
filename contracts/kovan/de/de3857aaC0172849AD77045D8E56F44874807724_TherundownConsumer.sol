...
contract TherundownConsumer is Initializable, ProxyOwned, ProxyPausable {
    ...
    function fulfillGamesCreated(
        ...
        for (uint i = 0; i < _games.length; i++) {
            ...
            _createGameFulfill(game, _sportId);
        }
    }
    ...
    function fulfillGamesResolved(
        ...
        for (uint i = 0; i < _games.length; i++) {
            ...
            _resolveGameFulfill(game, _sportId);
        }
    }

    function createMarketForGame(
        bytes32 _gameId
    ) public {
        // TODO require
    ...
    function _createGameFulfill(GameCreate memory _game, uint _sportId) internal {
        ...
        emit GameCreted(_sportId, _game.gameId, _game);
    }

    function _resolveGameFulfill(GameResolve memory _game, uint _sportId) internal {
        gameResolved[_game.gameId] = _game;
        sportsIdPerGame[_game.gameId] = _sportId;
        emit GameResolved(_sportId, _game.gameId, _game);
    }
    ...
    event GameCreted(uint _sportId, bytes32 _id, GameCreate _game);
    event GameResolved(uint _sportId, bytes32 _id, GameResolve _game);
