...
contract Mteam {
    ...
    event Deposit(address indexed _user, uint _value, uint _oldBalance);
    event Withdraw(address indexed _user, uint _value);
    event CalcDistrCoefEvent(address indexed _user, uint _factor, uint _scale);
    event CalcIntrstCoefEvent(address indexed _user, uint _factor, uint _scale);
    ...
    function depositTokens() public payable {
        ...
        emit Deposit(msg.sender, msg.value, userInfo[msg.sender].balance);
    }
    ...
    function withdrawTokens() public {
        ...
        if(userCount == 0){
            distributionCoefficient = 1 * scale;
            interestCoefficient = 1 * scale;
            emit CalcIntrstCoefEvent(msg.sender, 1 * scale, scale);
            emit CalcDistrCoefEvent(msg.sender, 1 * scale, scale);
        }
        else{
            distributionCoefficient = distributionCoefficient * (totalBalance - withdraw)  /  tempBalance; 
            emit CalcDistrCoefEvent(msg.sender, (totalBalance - withdraw) * scale / tempBalance, scale);
        }
    ...
    function updateAAVEData() private {
        ...
        if(prevBalance == 0){
            ...
            emit CalcIntrstCoefEvent(msg.sender,totalBalance * scale/prevBalance, scale);
        }
    }
