...
{   
    ...
    address public _ERC20_BRT_TokenAddress = 0x08ebADbc44F0f76CAFC6FD49e53247Cc9798eddD;                // BRT ERC-20 Contract Address   
    address public _ArtBlocksMintingContractAddress = 0xa07f47c30C262adcC263A4D44595972c50e04db7;       // ArtBlocks Minting Contract Address
    address public _ArtBlocksCoreContractAddressCitizens = 0xdAe755c2944Ec125a0D8D5CB082c22837593441a;  // KOVAN ArtBlocks Citizen NFT Collection Contract Address
    address public _ArtBlocksCoreContractAddressArtists = 0xdAe755c2944Ec125a0D8D5CB082c22837593441a;   // ArtBlocks Artist NFT Collection Contract Address 
    address public _GoldenTokenAddress = 0x4e15845FC4B9C6eC9A2774ebD483331d14d7A6c0;                    // Golden Token Contract Address
    ...
    { 
        ...
        Role[0xfd0cA120bcd30a700165294Aa9a89642732C1583] = _MINTER_ROLE; // BRT Minter #2
        ...
        IERC20(_ERC20_BRT_TokenAddress).approve(_ArtBlocksMintingContractAddress, 1000000000000000000000);
    ...
    {
        require(BookingsAvailable[_CurrentCityIndex][TimeSlot] > 0, "TimeSlot Fully Booked");
        require(IERC721(_GoldenTokenAddress).ownerOf(TicketID) == msg.sender, "ERC721: Sender Does Not Own Token With The Input Token ID");
        IERC721(_GoldenTokenAddress).transferFrom(msg.sender, _BRTMULTISIG, TicketID);
        require(IERC721(_GoldenTokenAddress).ownerOf(TicketID) == _BRTMULTISIG, "ERC721: Golden Token Redemption Failed");
    ...
    function _LiveMint(uint TicketID) public onlyMinter whenNotPaused 
    {
        ...
        uint TokenID = IArtBlocks(_ArtBlocksMintingContractAddress).purchase(_ArtBlocksProjectID);
        IERC721(_ArtBlocksCoreContractAddressCitizens).transferFrom(address(this), Recipient, TokenID);
    ...
    function _LiveMintArtist(uint ArtistID, uint TicketID) public onlyMinter whenNotPaused 
    {
        ...
        if(ArtBlocksMint)
        {
            ...
            IERC721(_ArtBlocksCoreContractAddressArtists).transferFrom(address(this), Recipient, TokenID);
    ...
    {
        require(ArtistIDs.length == TicketIDs.length, "Arrays Must Be Of Equal Length");
        for(uint i; i < TicketIDs.length; i++)
        {
     ...
     **/
    function __ApproveERC20(address BRT, address Operator, uint Amount) external onlyOwner 
    { 
        IERC20(BRT).approve(Operator, Amount); 
    }
    ...
    {
        require(ERC20s.length == Operators.length && Operators.length == Amounts.length, "Arrays Must Be Equal Length");
    ...
    {
        address OldMintingAddress = _ArtBlocksCoreContractAddressCitizens;
        _ArtBlocksCoreContractAddressCitizens = NewMintingAddress;
        emit ArtBlocksCoreAddressChanged(OldMintingAddress, _ArtBlocksCoreContractAddressCitizens);
    }

    /**
     * @dev Modifies ArtBlocks Core Address For Artist Mints
     */
    function __ChangeArtBlocksCoreAddressArtist(address NewMintingAddress) external onlyOwner
    {
        address OldMintingAddress = _ArtBlocksCoreContractAddressArtists;
        _ArtBlocksCoreContractAddressArtists = NewMintingAddress;
        emit ArtBlocksCoreAddressChangedArtist(OldMintingAddress, _ArtBlocksCoreContractAddressArtists);
    }
    ...
    external onlyOwner 
    {
        ...
        _ArtBlocksCoreContractAddressCitizens = ArtBlocksCoreContractAddress;
        _ArtBlocksCoreContractAddressArtists = ArtBlocksCoreContractAddressArtist;
        ...
        emit NewCityStarted(
            ...
            _ArtBlocksCoreContractAddressCitizens, 
            _ArtBlocksCoreContractAddressArtists,
            _GoldenTokenAddress, 
            _ArtBlocksProjectID, 
            _CurrentCityIndex
        );
    ...
    function __NewArtists(
        ...
        require(
            ...
            "Array Lengths Must Be Of Equal Value"
        );
    ...
    {
        return IERC721(_GoldenTokenAddress).ownerOf(TicketID);
    }
    ...
    function readMetadataArtist(uint ArtistID, uint TokenID) public view returns(string memory)
    {
        ...
        {
            return "Artist NFT Corresponding To `TicketID` Not Minted";
        }
    }
    ...
    {
        return IArtBlocks(_ArtBlocksCoreContractAddressCitizens).tokenURI(TokenID);
    }
