...
interface ISTRebalancer {
    ...
    function readTest() external view returns (string memory);
}

contract Wallet {
    address public STRebalancerAddress;
    ...
    constructor(address _STRebalancer) {
        STRebalancerAddress = address(_STRebalancer);
    }
    ...
    function testIssue() public returns (string[] memory) {
        ISTRebalancer STRebalancer = ISTRebalancer(STRebalancerAddress);
        return STRebalancer.buyList(address(this));
    }

    function readFromRebalancer2(address _walletAddress)
        public
        view
        returns (string memory)
    {
        ISTRebalancer STRebalancer2 = ISTRebalancer(_walletAddress);
        return STRebalancer2.readTest();
    }

    function readFromRebalancer3() public view returns (string memory) {
        ISTRebalancer STRebalancer3 = ISTRebalancer(STRebalancerAddress);
        return STRebalancer3.readTest();
    }
