...
contract ETPToken is Context, IERC20, Ownable {
   ...
   //============ update ===============
    ...
    constructor (
        ...
        usdtSparePoolManager  = 0x65518F078Ea391fdFFa3ebe916778b70440116D7; // usdt spare pool,this must approve address(this) contract;
    ...
    function  etpFeeToUsdt() public {
        require(balanceOf(address(this)) > 0,"etp isn't enough");
         ...
         if( takeFee ){           
           ...
           if( !ammPairs[from] && !ammPairs[to]){
                ...
                if(balanceOf(address(0)) < _tTotal.mul(99).div(100)){
                    ...
                    emit burn(from,address(0),amount.mul(9).div(100));
                }    
           }
        }else{
