...
contract Lottery is VRFConsumerBase, KeeperCompatibleInterface, Ownable {
    ...
    function checkUpkeep(bytes calldata /* checkData */) external view override returns (bool upkeepNeeded, bytes memory /* performData */) {
        upkeepNeeded = lotteryState == LOTTERY_STATE.OPEN && (block.timestamp - lastTimeStamp) > duration;
    }

    function performUpkeep(bytes calldata /* performData */) external override {
        if (lotteryState == LOTTERY_STATE.OPEN && (block.timestamp - lastTimeStamp) > duration) {
            endLotteryInternal();
        }
    }
}
