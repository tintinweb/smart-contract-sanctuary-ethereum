...
contract NodeManager is Ownable, IERC721, IERC721Metadata {
    ...
    function getPrice(uint8 levelID) view external returns (uint16) {
        return _nodeLevels[levelID].price;
    }

    function createNode(address account, string memory nodeName, uint8 levelIndex) onlyOwner external {
        ...
        totalShares = totalShares + ((_nodeLevels[levelIndex].price / 10) * (_nodeLevels[levelIndex].weight / 10));
    ...
    function getSharesForNode(uint8 levelIndex) public view returns (uint) {
        return ((_nodeLevels[levelIndex].price / 10) * (_nodeLevels[levelIndex].weight / 10));
    }
