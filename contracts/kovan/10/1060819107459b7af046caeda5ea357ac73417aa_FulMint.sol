...
contract FulMint is Ownable{
    ...
    function buyPackOfNFT( bytes32[] memory _leaf, bytes32[][] memory _proof, uint _noOfPack,uint256[] memory nftIDs,string[] memory nftName, string[] memory tokenUri, string[] memory nftPosition,string[] memory nftRarity)public isWhitelisted{
        ...
        require(verifyPacks(rootOfPack[_noOfPack], _leaf,_proof) == true,"Either proof,leaf rootOfPack is not Correct");
    ...
    function burnNFTId(uint256[]memory NftID, bytes32  _leaf, bytes32[] memory _proof, uint256 nftId, string memory nftName,string memory tokenUri,string memory nftPosition, string memory nftRarity)public {
        require(verify(rootOfPack[14], _leaf,_proof) == true," Leaf is not geeting Verified");
        for(uint i = 0; i<=2; i++){
