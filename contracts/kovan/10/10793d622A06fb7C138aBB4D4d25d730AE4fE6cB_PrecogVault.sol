...
contract PrecogVault is IPrecogVault, ReentrancyGuard {
    ...
    modifier onlyAdmin() {
        require(
            ...
            "PrecogVault: Caller is not admin"
        );
        _;
    }
    ...
    modifier onlyPrecog() {
        require(
            msg.sender == precogStorage.getPrecog(),
            "PrecogVault: Caller is not accessible"
        );
        _;
    }
    ...
    modifier onlyMiddlewareService() {
        require(
            ...
            "PrecogVault: Caller is not middleware"
        );
        _;
    }
    ...
    modifier isExistingToken(address token) {
        require(
            ...
            "PrecogVault: Token is not in trading"
        );
        _;
    }
    ...
    function takeInvestment(address token)
        external
        override
        ...
        nonReentrant
    {
        ...
        if (idealRemainBalance < actualBalance) {
            ...
            IERC20(token).safeTransferFrom(
                msg.sender,
                address(this),
                idealRemainBalance - actualBalance
            );
        }
    }
    ...
    function _decreaseInvestmentWhenWithdraw(
        ...
        IPrecogStorage.Investment[] memory _investmentsOf = precogStorage
            .getInvestmentsOf(_token, _account);
        IPrecogStorage.Investment memory _lastInvestmentOf = _investmentsOf[
            _investmentsOf.length - 1
        ];
        IPrecogStorage.Cycle memory _futureTradingCycle = precogInternal
            .getTradingCycleByTimestamp(_token, _lastInvestmentOf.timestamp);
        // Update investment of
        _lastInvestmentOf.amount -= _amount;
        _remainingAmount = _lastInvestmentOf.amount;
        uint _unit = _amount *
            (_futureTradingCycle.endTime - _lastInvestmentOf.timestamp);
        ...
        );
        {
            ...
            precogStorage.updateTotalUnitsTradingCycle(
                _token,
                _futureTradingCycle.id,
                _newUnit
            );
            ...
            if (_lastInvestmentOf.isWhitelist) {
                ...
                uint _newTotalUnitsForWhitelist = precogStorage
                    .getTotalUnitsForWhitelistTradingCycle(
                        _token,
                        _futureTradingCycle.id
                    ) - _unit;
                ...
                uint _newLiquidityForWhitelist = precogStorage.getLiquidityWhitelist(
                    _token
                ) - _amount;
        ...
        }
        {
            ...
            IPrecogStorage.AccountTradingInfo memory _accountTradingInfo = precogStorage
                .getAccountTradingInfo(_token, _account);
    ...
    function _withdrawBeforeFundingTime(
        ...
        uint _availableWithdrawalAmount = precogInternal.availableDepositedAmount(
            _token,
            _account
        );
        require(
            _availableWithdrawalAmount >= _amount,
            "PrecogVault: Not available amount can withdraw"
        );
        require(
            IERC20(_token).balanceOf(address(this)) >= _amount,
            "PrecogVault: Vault does not have enough token amount"
        );
        uint _remainingAmount = _decreaseInvestmentWhenWithdraw(
            _token,
            _account,
            _amount
        );
    ...
    function withdrawRemainderTokenAfterRemoveLiquidityPool(address token)
        external
        override
        onlyAdmin
    {
