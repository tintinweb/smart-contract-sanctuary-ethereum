...
contract PrivateEscrowAccount is ReentrancyGuard {
    ...
    uint public tokenAmountAlessFee;
    uint public tokenAmountBlessFee;
    uint public tokenAFee;
    uint public tokenBFee;
    ...
    owner = msg.sender;
    feeCollector =  0xaEd3BFdC4E2f356C5C47cDb2670C4085000F65BB; // fee collector address EOA or Multisig
    }
    ...
    function partyADeposit() external nonReentrant {
        ...
        tokenAmountAlessFee = tokenAmountA * 1/10 /100;
        tokenAFee = tokenAmountA - tokenAmountAlessFee;
    ...
    function partyBDeposit() external nonReentrant {
        ...
        tokenAmountBlessFee = tokenAmountB * 1/10 /100;
        tokenBFee = tokenAmountB - tokenAmountBlessFee;
    ...
    function executeTrade() external onlyParties nonReentrant { 
        ...
        require(tokenContractB.transferFrom(address(this), partyA, tokenAmountBlessFee)); 
        require(tokenContractA.transferFrom(address(this), partyB, tokenAmountAlessFee)); 
        require(tokenContractB.transferFrom(address(this), feeCollector, tokenBFee)); 
        require(tokenContractA.transferFrom(address(this), feeCollector, tokenAFee));
        depositedValueTokenA = 0;
        depositedValueTokenB = 0;
        tokenAmountAlessFee = 0;
        tokenAmountBlessFee =0;
        tokenAFee = 0;
        tokenBFee = 0;
        partyADeposited = false;
        partyBDeposited = false;
        emit tradeComplete(partyA, tokenContractB, tokenAmountBlessFee);
        emit tradeComplete(partyB, tokenContractA, tokenAmountAlessFee);
    }
...
contract EscrowFactory {
    ...
    // need to log contracts in an array for the gui to index
