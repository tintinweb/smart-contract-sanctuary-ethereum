...
contract UbxsStake is Ownable {
    ...
    function reinvest() external {
        ...
        user.deposits.push(deposit);
        user.totalInvestment += totalWithdrawable;
    ...
    function getlastDeposits(address userAddress)
        public
        view
        returns (Deposit[] memory)
    {
        Deposit[] memory lastDeposits = new Deposit[](5);
        Deposit[] memory deposits = users[userAddress].deposits;
        uint256 len = deposits.length;
        uint256 j = 0;
        for (uint256 i = len - 5; i < len; i++) {
            lastDeposits[j++] = deposits[i];
        }
        return lastDeposits;
    }
