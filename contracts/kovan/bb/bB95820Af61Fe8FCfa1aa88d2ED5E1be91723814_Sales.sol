...
contract Sales is Ownable, AccessControl
{
    IERC20 public token;
    uint256 rate;
    ...
    constructor(IERC20 _tokenAddress,uint _initTokenPerWeiRate) public onlyOwner
    {
        require(_initTokenPerWeiRate > 0,"Rate not found..");
        token = _tokenAddress; //new SANDOToken("SANDO","SANDO");
        rate = _initTokenPerWeiRate; //1200000000000000; //0.0012;
    }
    ...
    function buy() payable public // send ether and get tokens in exchange; 1 token == 1 ether
    {
      ...
      token.transfer(msg.sender, amountTobuy*rate);
      emit Bought(phaseOfsales, amountTobuy*rate);
    }
