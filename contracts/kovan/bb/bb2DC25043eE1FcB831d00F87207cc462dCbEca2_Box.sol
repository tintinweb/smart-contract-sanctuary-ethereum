...
contract Box is ERC721 {
    ...
    */ 
    ...
    // Rarity types: 0-Gold; 1-Silver; 2-Bronze; 

    enum RarityTypes{Gold, Silver, Bronze}

    /**
    * mapping that save token id => token metadata 
    */
    mapping(uint256 => TokenMeta) private tokenMetadata;

    // max ammount of tokens that can have 1 address
    uint256 private maxAmmountOfTokens = 15;

    // address of the Controller contract
    IController private controller;

    // address of the CubeManager contract
    CubeManager private cubeManager;

    constructor(address _controller) ERC721("Box", "BOX") {
        controller = IController(_controller);
        require(controller.isSuperAdmin(msg.sender), "Box: Only super admin can create a contract.");
    ...
    */
    function _initializeSC(address _cubeManager, address _controller) external {
        require(controller.isSuperAdmin(msg.sender), "Controller: Only Super Admin change address");
        cubeManager = CubeManager(_cubeManager);
        controller = IController(_controller);
    }
    ...
    function mint(address to, uint256 tokenId) public {
        ...
        require(msg.sender == controller.getSellerContract() || controller.isNFTManager(msg.sender), "Access denied: sender is not a minter.");

        require(balanceOf(to) < maxAmmountOfTokens, "An address cannot have more than 15 tokens");
    ...
    function burn(uint256 tokenId) public {
        require(controller.isNFTManager(msg.sender), "Access denied: sender is not a burner.");
        _burn(tokenId);
    }
    ...
    * Requirements:
    * sender must have a NFTManager role or be the Seller contract.
    */
    function setMetadata(string memory _name, string memory _description, uint256 _Id, string memory _URL, RarityTypes _rarity, bool _isPanno) public {
        require(msg.sender == controller.getSellerContract() || controller.isNFTManager(msg.sender), "Access denied: sender is not a minter.");
        TokenMeta memory tokMeta = TokenMeta(_name, _description, _Id, _URL, _rarity, _isPanno);
        tokenMetadata[_Id] = tokMeta;
    }

    // Returns the URI of the token.
    // Requirements: token must exist.
    ...
    // Requirements: 'to' can have only 15 tokens.
    ...
    function setIsPanno(uint256 tokenId, bool _isPanno) public {
        require(msg.sender == address(cubeManager), "Access denied: sender is not a manager.");
        tokenMetadata[tokenId].isPanno = _isPanno;
    }
    ...
    function getDescription(uint256 tokenId) public view returns(string memory){
        return tokenMetadata[tokenId].Description;
    }

    // Returns the rarity of the token. 
    function getRarity(uint256 tokenId) public view returns(RarityTypes) {
        return tokenMetadata[tokenId].Rarity;
    }

    // Returns the isPanno of the token. 
    function getIsPanno(uint256 tokenId) public view returns(bool) {
        return tokenMetadata[tokenId].isPanno;
    }
}
