...
contract Single is ERC20, ERC20Burnable, Ownable {
    ...
    uint public chainsCount;
    mapping(address => uint256) public userUnlockTime;
    mapping(address => bool) public minter;
    mapping(string => address) public chainContract;

    modifier onlyOwnerOf() {
        require(currentOwner == msg.sender);
        _;
    }
    constructor(string memory _name, string memory _symbol, address _receiver, address _admin, uint _amount, bool _isMulti, string[] memory _chains) ERC20(_name, _symbol) {
        currentOwner = _receiver;
        admin = _admin;
        minter[_admin] = true;
        minter[_receiver] = true;
        isMulti = _isMulti;
        chains = _chains;
        chainsCount = _chains.length;
    ...
    function setChainContract(string memory _chain, address _chainContract) public {
        require(msg.sender == admin);
        chainContract[_chain] = _chainContract;
    }
    
    function setMinter(address _minter, bool _value) public onlyOwnerOf {
        minter[_minter] = _value;
    }
    /**
