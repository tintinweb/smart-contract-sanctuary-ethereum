...
contract WarriorNFT is Context, IWarriorNFT, Ownable {
    ...
    function sendToMarketplace(uint256 tokenId, uint256 price) external {
        require(ownerOf(tokenId)==msg.sender, "You should own this warrrior first");
        require(price>0, "Token price should not be zero");
        tokenData[tokenId].onMarket = true;
        tokenPrice[tokenId] = price;
        tokensOnMarket.push(tokenId);
        tokenIndex[tokenId] = tokensOnMarket.length-1;
    }

    function buyToken(uint256 tokenId) external {
        require(msg.sender!=ownerOf(tokenId), "You can not buy your warrior");
        require(tokenData[tokenId].onMarket, "This warrior is not on Market");
        uint256 blstAmount = tokenPrice[tokenId]*10**18;
        require(bloodstone.balanceOf(msg.sender)>=blstAmount, "Insufficient bloodstone balance");
        uint marketplaceTax = feehandler.getFee(0);
        uint256 amountToRewardpool = blstAmount.mul(marketplaceTax).div(10000);
        uint256 amountToOwner = blstAmount.sub(amountToRewardpool);
        bloodstone.transferFrom(msg.sender, ownerOf(tokenId), amountToOwner);
        bloodstone.transferFrom(msg.sender, rewardPoolAddr, amountToRewardpool);
        transferFrom(ownerOf(tokenId), msg.sender, tokenId);
        removeTokenFromMarketplace(tokenId);
    }

    function removeTokenFromMarketplace(uint256 _tokenId) internal {
        //Removing tokenId from tokensOnMarket
        tokenData[_tokenId].onMarket = false;
        tokensOnMarket[tokenIndex[_tokenId]] = tokensOnMarket[tokensOnMarket.length-1];
        tokenIndex[tokensOnMarket[tokensOnMarket.length-1]] = tokenIndex[_tokenId];
        delete tokensOnMarket[tokensOnMarket.length-1];
    }
    ...
    function getTokenIds(address _address) external view returns (uint256[] memory) {
        uint256[] memory retvals;
        uint k=0;
        for(uint i=0;i<addressTokenIds[_address].length;i++) {
            if(!tokenData[addressTokenIds[_address][i]].onMarket) {retvals[k]= addressTokenIds[_address][i]; k++;}
        }
        return retvals;
    }
