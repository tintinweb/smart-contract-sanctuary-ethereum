...
contract FulMint is Ownable{
    ...
    function setRootOfPack(uint256[]memory packNumber, bytes32[]memory rootHash) public onlyOwner{
        require(packNumber.length <=13, "only 13 packs are allowed " );
        for(uint i=0; i<=12;i++){
        rootOfPack[packNumber[i]] = rootHash[i];
        }
    }
