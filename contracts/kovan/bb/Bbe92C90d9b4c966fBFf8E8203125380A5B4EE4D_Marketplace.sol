...
contract Marketplace {
    ...
    struct BuyOffer {
        address buyer;
        uint256 price; // make changes to uint size
        uint256 createTime; // make changes to uint size to save storage
    }
    ...
    address public _tokenContractAddress; // made changes
        ...
        returns (uint256 netSaleAmount)
    {
        ...
        if (royaltiesAmount > 0) {
            (bool success, ) = royaltiesReceiver.call{value: royaltiesAmount}(  // made changes here
                ""
            );
            require(
                success,
        ...
        tokenOwnerForbidden(tokenId)
    {
        ...
        (bool success, ) = activeSellOffers[tokenId].seller.call{value: saleValue}(""); // made changes here
        ...
        tokenOwnerForbidden(tokenId)
    {
        ...
        if (refundBuyOfferAmount > 0) {
            (bool success, ) = payable(previousBuyOfferOwner).call{ // made changes here
                value: refundBuyOfferAmount
            }("");
        ...
        lastBuyOfferExpired(tokenId)
    {
        ...
        if (refundBuyOfferAmount > 0) {
            (bool success, )  = msg.sender.call{value: refundBuyOfferAmount}(""); // made changes here
            require(success, "Address: unable to send value, recipient may have reverted");
        }
        // Broadcast offer withdrawal
        ...
        tokenOwnerOnly(tokenId)
    {
        ...
        // Transfer funds to the seller
        (bool success, ) = msg.sender.call{value: netSaleValue}("");  // made changes here
