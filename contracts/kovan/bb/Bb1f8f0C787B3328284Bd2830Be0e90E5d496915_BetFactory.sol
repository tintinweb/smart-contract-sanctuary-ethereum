...
interface IUpshotConsumer {
  ...
  function requestCapAndFloor(
      bytes32 _specId,
      uint256 _payment,
      address _assetAddress
  ) external returns(bytes32);

  function requestIdCapAndFloor(bytes32 _specId)
   external
   view
  returns(CapAndFloor memory);
}
...
contract Bet is ReentrancyGuard {
  ...
  // Oracle
  IUpshotConsumer public consumer;
  // Bet finish unlockTime
  uint public unlockTime;

  // ids for Orcale requests
  bytes32 public initializedId;
  bytes32 public finilizedId;

  // NFT collection for bet
  address public NFTCollection;

  // Oracle platform token
  IERC20 public LINK;

  uint128 private _initialPrice;
  bytes32 private _specId;
    ...
    )
  {
    ...
    consumer = IUpshotConsumer(_upshot);
  ...
  // set initial floor for NFT
  function setInitialPrice(uint256 _payment) external {
    require(initializedId == bytes32(0), "initial price initialized");
    initializedId = requestPrice(_specId, _payment);
  }

  // set final floor for NFT
  function setFinalPrice(uint256 _payment) external {
    require(initializedId != bytes32(0), "require initial price");
    require(finilizedId == bytes32(0), "final price initialized");
    require(block.timestamp > unlockTime, "locked yet");
    finilizedId = requestPrice(_specId, _payment);
  }
  ...
  function getReward() external returns(uint128 initialPrice, uint128 finalPrice){
    require(initializedId != bytes32(0), "require initial price");
    require(finilizedId != bytes32(0), "require final price");

    initialPrice = getPrice(initializedId);
    finalPrice = getPrice(finilizedId);

    // bet Yes win final price more than initial
    if(finalPrice > initialPrice){
      _withdraw(
        betYesToken,
        IERC20(betYesToken).balanceOf(msg.sender),
        betYesDeposiedWei + betNoDeposiedWei
      );
    }
    // bet No win final price less than initial
    else{
      _withdraw(
        betNoToken,
        IERC20(betNoToken).balanceOf(msg.sender),
        betYesDeposiedWei + betNoDeposiedWei
      );
    }
  }
  ...
  // helper for get price by id
  function getPrice(bytes32 _id) public view returns(uint128){
    IUpshotConsumer.CapAndFloor memory data = consumer.requestIdCapAndFloor(_id);
  ...
  // helper for request price from Oracle
  // NOTE: price will be set in callback in next tx
  function requestPrice(bytes32 _specId, uint256 _payment) internal returns(bytes32 id){
    LINK.safeTransferFrom(msg.sender, address(this), _payment);
    LINK.safeTransfer(address(consumer), _payment);

    id = consumer.requestCapAndFloor(
      _specId,
      _payment,
      NFTCollection
    );
  ...
  function _withdraw(address betShareToken, uint sharesAmount, uint balance) internal nonReentrant returns (uint withdrawed){
...
contract BetFactory {
    ...
    function createBet(
      ...
      returns (address)
    {
      address bet = address(new Bet(_NFTCollection, upshot, _LINK, _specialId, _lockTime, _name));
