...
// creates a token based on a artwork tokens stay on contract until owner does domething with them
...
contract SolidStateToken is ERC20 {
    ...
    address private usdEthPriceFeed;
    ...
    function getContractPriceInETH() public returns (uint256) {
        (uint256 price, uint256 decimals) = getUSDETHValue();

        return (contractSalePrice * (10**decimals)) / (price / (10**decimals));
    }
    ...
    function setPriceFeed(address _usdEthPriceFeed) public onlyOwner {
        usdEthPriceFeed = _usdEthPriceFeed;
    }

    function getPriceFeed() public view onlyOwner returns (address) {
        return usdEthPriceFeed;
    }

    function getUSDETHValue() public returns (uint256, uint256) {
        priceFeed = AggregatorV3Interface(usdEthPriceFeed);
