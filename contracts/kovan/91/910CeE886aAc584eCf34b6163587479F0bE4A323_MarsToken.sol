...
contract MarsToken is IERC1404, ERC20, Ownable {
    uint8 SUCCESS_CODE = 1;
    uint8 FAILURE_CODE = 0;
    uint8 NON_ACCRED_USER = 0;
    uint8 ELIGIBLE_USER = 1;
    uint8 ELIGIBLE_USER_2 = 2;
    uint8 ACCRED_USER = 3;
    uint8 NOT_APPLICABLE_USER = 4;
    ...
    mapping(uint8 => uint256) private _investorTypeLimits;

    // These addresses can control addresses that can manage whitelisting of investor or in otherwords can call modifyKYCUnitData (address _address) or modifyKYCMultiData (address[] _addresses)
    ...
    constructor(
        ...
        _investorTypeLimits[NON_ACCRED_USER] = 10000 * 10**18;
        _investorTypeLimits[ELIGIBLE_USER] = 30000 * 10**18;
        _investorTypeLimits[ELIGIBLE_USER_2] = 100000 * 10**18;
        _investorTypeLimits[ACCRED_USER] = 0;
        _investorTypeLimits[NOT_APPLICABLE_USER] = 0;
        ...
        returns (bool)
    {
        if (_allowedInvestors != 0)
            require(
                ...
                "Allowed Investors cannot be less than Current holders"
            );
    ...
    function flipTradingStatus() external onlyOwner returns (bool) {
    ...
    // Set buy and sell restrictions on investor
    function modifyKYCUnitData(
        address user,
        ...
        require(
            _whitelistControlAuthority[msg.sender] == true,
            "Not Whitelist Authority"
        );
        modifyKYC(user, buyRestriction, sellRestriction, _type);
    }

    function modifyKYCMultiData(
        ...
        for (uint256 i = 0; i < users.length; i++) {
            modifyKYC(users[i], buyRestriction, sellRestriction, _type);
        }
        return true;
    }
    ...
    function modifyKYC(
        ...
        if (_type > 4) {
            _investorType[user] = 0;
        } else {
            _investorType[user] = _type;
        }
    ...
    function detectTransferRestriction(
        ...
        // Validate whitelist restrictions
        require(
            _sellRestriction[_from] != 0 && _buyRestriction[_to] != 0,
            "Not Whitelisted - Seller/Buyer"
        );

        require(
            _sellRestriction[_from] <= block.timestamp &&
                _buyRestriction[_to] <= block.timestamp,
            "KYC Time Restriction"
        );
        ...
        if (allowedInvestors == 0) return SUCCESS_CODE;

        // Validates the current balance of receiver against their limit
        uint256 investorLimit = _investorTypeLimits[_investorType[_to]];
        bool isNoLimit = investorLimit == 0;
        bool isError = _to != owner() &&
            !isNoLimit &&
            (value + super.balanceOf(_to)) > investorLimit;

        require(!isError, "You have exceeded your investment limit.");

        if (super.balanceOf(_to) > 0 || _to == owner())
        ...
        // Validates investor count limit
        if (currentTotalInvestors < allowedInvestors)
            ...
            if (
                super.balanceOf(_from) == value &&
                super.balanceOf(_to) == 0 &&
                _from != owner()
            ) return SUCCESS_CODE;
            else return FAILURE_CODE;
        }
    }
    ...
    {
        if (restrictionCode == 1) message = "Whitelisted";
        else message = "Not Whitelisted";
    }
    ...
    function transfer(address to, uint256 value)
        ...
        returns (bool)
    {
        updateInvestorCount(msg.sender, to, value);
    ...
    function transferFrom(
        ...
        address to,
        uint256 value
    ) public override notRestricted(from, to, value) returns (bool) {
        updateInvestorCount(msg.sender, to, value);
    ...
    function updateInvestorCount(
        ...
        if (super.balanceOf(sender) == value && sender != owner())
            currentTotalInvestors = currentTotalInvestors - 1;

        if (super.balanceOf(recipient) == 0 && recipient != owner())
            currentTotalInvestors = currentTotalInvestors + 1;
    }
