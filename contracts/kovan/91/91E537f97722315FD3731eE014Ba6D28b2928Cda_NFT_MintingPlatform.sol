...
contract NFT_MintingPlatform {
  ...
  constructor(address SignerAddress,address OwnerAddr, address _ERC721,address _USDToken) {
    ...
    function mintNFT(Order memory _Order, Sig memory sig,uint USDTokenAmount,address reffererAddress)public payable{
        require(reffererAddress != _Order.toAddr && reffererAddress != address(0),"Refferrer Address can't be the caller and Refferrer address can't be zero address!!!");
        ...
        if(MintedNFTsUserDetails[_Order.toAddr].reffererAddress == address(0)){
            MintedNFTsUserDetails[_Order.toAddr].reffererAddress = reffererAddress;
        }
        require(MintedNFTsUserDetails[_Order.toAddr].reffererAddress == reffererAddress,"Refferer address must be same as previous!!!");
        if(_Order.assetType == AssetType.ETHER){
                ...
                require(USDTokenAmount > 0,"USDT amount invalid or should not give ether while exchanging asset with USDT!!!");
