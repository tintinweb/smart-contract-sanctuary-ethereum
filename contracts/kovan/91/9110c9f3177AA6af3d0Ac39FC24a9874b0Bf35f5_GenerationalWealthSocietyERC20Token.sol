...
contract GenerationalWealthSocietyERC20Token is ERC20Permit, VaultOwned {
    ...
    function _transfer(address sender, address recipient, uint256 amount) override internal virtual {
        ...
        if(
            ...
            _balances[teamTaxFeeAddress] = _balances[teamTaxFeeAddress].add(teamTaxFee);
            emit Transfer(sender, teamTaxFeeAddress, teamTaxFee);
            // Burn tax fee
            uint256 burnTaxFee = calculateBurnTaxFee(amount);
            _balances[burnTaxFeeAddress] = _balances[burnTaxFeeAddress].add(burnTaxFee);
            emit Transfer(sender, burnTaxFeeAddress, burnTaxFee);

            receiveAmount = receiveAmount.sub(burnTaxFee).sub(teamTaxFee);
        }
