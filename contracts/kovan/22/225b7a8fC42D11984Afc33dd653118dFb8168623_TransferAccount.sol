    ...
    function executeOperation(
        ...
        for(uint i=0; i<_DebtTokenBalance.length; i++) {
            ...
            if (_DebtTokenBalance[i].stableDebtTokenBalance != 0) {
                ...
                LENDING_POOL.repay(_DebtTokenBalance[i].tokenAddress, _DebtTokenBalance[i].stableDebtTokenBalance, 1, initiator);
            }
            // Repay VariableDebt
            if (_DebtTokenBalance[i].variableDebtTokenBalance != 0) {
                ...
                LENDING_POOL.repay(_DebtTokenBalance[i].tokenAddress, _DebtTokenBalance[i].variableDebtTokenBalance, 2, initiator);
            }
        }

        // Transfer all aTokens from Account 1 to Account 2
        for (uint i=0; i<_aTokenBalance.length; i++) {
            IERC20(_aTokenBalance[i].aTokenAddress).transferFrom(initiator, _recipient, _aTokenBalance[i].aTokenBalance);
        }
