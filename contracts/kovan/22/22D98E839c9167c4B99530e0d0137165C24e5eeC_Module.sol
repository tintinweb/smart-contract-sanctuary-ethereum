...
contract Module is IModule, ERC721URIStorage, IERC721EnumerableExt, Ownable {
    ...
    /**
     * @dev Randomizer nonce
     */
    uint256 internal nonce = 0;

    /**
     * @dev Actual tokens store
     */
    uint256[MAX_TOKENS] internal indices;

    mapping(address => EnumerableSet.UintSet) private boxIds;
    mapping(uint256 => Modules.ModuleBox) private boxes;

    function setPricePerBox(uint256 price) external override {
        pricePerBox = price;
    }
    ...
    function buyBox(address owner) external payable override {
        require(msg.value == pricePerBox, "Insufficient amount");
        ...
        emit ModuleCreated(box.id, box.expired);
    }

    function openBox(uint256 id) external override returns (uint256) {
        ...
        require(!boxes[id].onSale, "Remove module from Market place");

        EnumerableSet.remove(boxIds[msg.sender], id);
        delete boxes[id];

        return _internalMint(msg.sender);
    }
    ...
    function changeModuleSaleState(
        ...
        require(id > 0, "Incorect Box Id");
        require(owner == boxes[id].owner, "You do not own of the module");
        Modules.ModuleBox memory box = boxes[id];
        box.onSale = onSale;
        boxes[id] = box;
    }
