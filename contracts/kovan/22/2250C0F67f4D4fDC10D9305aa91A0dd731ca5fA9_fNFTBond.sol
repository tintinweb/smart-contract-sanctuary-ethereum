...
contract fNFTBond is ERC721, Ownable {
    ...
    function getBondShares(uint256 bondID) external view onlyIfExists(bondID) returns (uint256 unweightedShares, uint256 weightedShares) {
        uint256 x = (bondManager.index() + 10 ** 18) - bonds[bondID].index;
        unweightedShares = toFixed(bondManager.getBondLevel(bonds[bondID].levelID).price * x / 10 ** 18, 5);
        weightedShares = toFixed(bondManager.getBondLevel(bonds[bondID].levelID).price * x * bondManager.getBondLevel(bonds[bondID].levelID).weight / 10 ** 18 / 100, 5);
    }  
...
/*
        ...
        */
        ...
        uint x = (length - ((length - precision) + n));
