...
contract TinyExplorersRenderer is Ownable, ReentrancyGuard {
    ...
    uint256 private constant NUM_LAYERS = 9; // 5 + 5 ? // Chainrunners = 13;
    uint256 private constant NUM_PALETTES = 10; //10;
    ...
    */
    ...
    constructor() {
        ...
        // Masks - 1
        WEIGHTS[5] = [129, 129, 129, 129, 29, 129, 129, 7389];
        // Goggles - 1
        WEIGHTS[6] = [143, 143, 143, 143, 143, 143, 143, 143, 143, 143, 143, 143, 143, 143, 143, 143, 143, 143, 143, 143, 143, 143, 143, 143, 143, 143, 143, 4331];
        // Hats - 1
        WEIGHTS[7] = [83, 83, 83, 83, 83, 83, 83, 83, 83, 83, 83, 83, 83, 83, 83, 83, 83, 83, 83, 83, 83, 83, 83, 83, 83, 83, 83, 83, 83, 83, 83, 83, 83, 83, 83, 83, 83, 83, 83, 83, 83, 83, 83, 83, 83, 83, 83, 83, 83, 83, 83, 3959];
        // Face Accessories - 1
        WEIGHTS[8] = [203, 203, 203, 203, 203, 203, 203, 203, 203, 203, 203, 203, 203, 203, 203, 203, 203, 4741];
        // Backgrounds

        BACKGROUND_COLORS = [
            0xfaf2e5f6f4edfcefdffff8e7,
            0xf0f0e8f6f1ecf1f0f0f3eded, 
            0x261c2c37263b1f1d36041c32
        ];
    ...
    function generateHashNew(uint256 tokenId) public view returns (uint256,bool){
        uint256 hash;
        bool isPirate= false;
        hash= uint256(
            keccak256(
    ...
    function tokenURI(uint256 tokenId,TinyExplorersTypes.TinyExplorer memory explorerdata ) public view returns (string memory) {
       string memory json= Base64.encode(
           bytes(string(abi.encodePacked(
               '{"name": "Tiny Explorer #',
               uintToByteString(tokenId, 6),
               '", "description": "Fully on-chain, randomly generated tiny explorers.",',
       ...
       json=string(abi.encodePacked(
           ...
           json,
           Base64.encode(bytes(tokenSVG(explorerdata.dna))),
           Base64.encode(bytes(tokenAttributes(explorerdata.dna)))));
    //   console.log(json);
       return json;
        
       }

    function tokenAttributes(uint256 _dna) internal view returns (string memory) {
       ...
       attributes = string(abi.encodePacked('","attributes":',attributes,'{"trait_type":"Explorer","value":"True"}'));
       return attributes;
    }
    ...
    */
    ...
    function getTokenData(uint256 _dna) internal view returns (Layer [NUM_LAYERS] memory tokenLayers, Color [NUM_COLORS][NUM_LAYERS] memory tokenPalettes, uint8 numTokenLayers, string [NUM_LAYERS] memory traitTypes) {
        ...
        bool hasMask = dna[5] < (8192 - WEIGHTS[5][7]); //false
        bool hasGoggles = dna[6] < (8192 - WEIGHTS[6][27]);
        bool hasHat = dna[7] < (8192 - WEIGHTS[7][51]); 
        // 8 - face accessory
                ...
                */
                if (isPirateAndHasKingdom) {
                    ...
                    if ( (i == 4 && hasGoggles && !hasMask) || (i == 8 && !hasMask ) || (i == 6 && !hasMask) || (i == 3 && !hasHat) || (i < 3 || i == 5 || i == 7) ) {
                        ...
                        // traitTypes[numTokenLayers] = ["QmFja2dyb3VuZA", "U2hvdWxkZXJz", "RmFjZQ","SGFpcg", "RmFjaWFsIEhhaXI", "TWFzaw", "R29nZ2xl", "SGF0" ,"RmFjZSBBY2Nlc3Nvcnk"][i];
    ...
    function palette(uint8 index, uint8 idx, uint16 _dna) internal view returns (Color [NUM_COLORS] memory) {
        ...
        for (uint16 i = 0; i < NUM_COLORS; i++) {
            if (idx == 0) {
                colors[i].hexString=Base64.encode(bytes(allColors(BACKGROUND_COLORS[uint8(_dna % 3)], true)[uint8(_dna % 4)]));
            } else {
