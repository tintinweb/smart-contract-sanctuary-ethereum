...
contract DaiBondDepository is Ownable {
    ...
    address public immutable MINER; // token given as payment for bond
    address public immutable treasury; // mints MINER when receives principle
    ...
    struct Bond {
        uint256 payout; // MINER remaining to be paid
    ...
    function setPrinciple(address _addr) external onlyPolicy {
    ...
    function deposit(
        ...
        require(payout >= 10000000, "Bond too small"); // must be > 0.01 MINER ( underflow protection )
        ...
        /**
            ...
            approved and
            deposited into the treasury, returning (_amount - profit) MINER
         */
        ...
        if (fee != 0) {
            ...
            IERC20(MINER).safeTransfer(DAO, fee);
        }
    ...
    function stakeOrSend(
        ...
        if (!_stake) {
            ...
            IERC20(MINER).transfer(_recipient, _amount); // send payout
        } else {
            IERC20(MINER).approve(staking, _amount);
            IStaking(staking).stake(_recipient, _amount, true, true);
        }
        return _amount;
    }
    ...
    function maxPayout() public view returns (uint256) {
        return IERC20(MINER).totalSupply().mul(terms.maxPayout).div(100000);
    }
    ...
    /**
     *  @notice calculate current ratio of debt to MINER supply
     *  @return debtRatio_ uint
     */
    function debtRatio() public view returns (uint256 debtRatio_) {
        uint256 supply = IERC20(MINER).totalSupply();
    ...
    /**
     *  @notice calculate amount of MINER available for claim by depositor
     *  @param _depositor address
    ...
    /**
     *  @notice allow anyone to send lost tokens (excluding principle or MINER) to the DAO
     *  @return bool
     */
    function recoverLostToken(address _token) external returns (bool) {
        require(_token != MINER);
