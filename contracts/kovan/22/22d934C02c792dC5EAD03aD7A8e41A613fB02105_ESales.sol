...
contract ESales is ERC721Holder, ERC1155Holder, Ownable, ReentrancyGuard, TokensHandler {
    ...
    function cancel(uint saleId) external isAccesible(saleId) {
    ...
    function buy(uint saleId) external payable isAccesible(saleId) nonReentrant {
        require(saleId < id, 'Sale is no longer accessible');
        require(sales[saleId].seller != msg.sender, 'You can not purchse your own items');
        ...
        uint bmarketFee = sales[saleId].price / fee / discount;
        ...
        transferTokens(sales[saleId].seller,msg.sender,sales[saleId].nftAddresses,sales[saleId].nftIds,sales[saleId].nftTypes);
