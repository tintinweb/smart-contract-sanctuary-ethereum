...
contract PooledCreditLine is ReentrancyGuard, OwnableUpgradeable, IPooledCreditLine {
    ...
    * @param gracePenaltyRate rate at which penalty is levied during grace period
     ...
     */
    event PooledCreditLineLiquidated(uint256 indexed id, address indexed liquidator);
    ...
    function request(Request calldata _request) external returns (uint256) {
        ...
        _limitBorrowedInUSD(_request.borrowAsset, _request.borrowLimit);
     ...
     * @param _id identifier for the pooled credit line
     */
    function accept(uint256 _id, uint256 _amount) external override onlyLenderPool {
        ...
        pooledCreditLineConstants[_id].borrowLimit = uint128(_amount);
    ...
    * @param _toSavingsAccount if true, tokens are transferred from savingsAccount 
    ...
    function calculateInterestAccrued(uint256 _id) public view returns (uint256) {
        ...
        if (_lastPrincipalUpdateTime <= _endTime && block.timestamp > _endTime) {
            _penalityInterest = calculateInterest(_principal, _penaltyRate, block.timestamp.sub(_endTime));
        } else if (_lastPrincipalUpdateTime > _endTime) {
            _penalityInterest = calculateInterest(_principal, _penaltyRate, block.timestamp.sub(_lastPrincipalUpdateTime));
        }
        uint256 _interestAccrued = calculateInterest(_principal, _borrowRate, _timeElapsed);
    ...
    function liquidate(uint256 _id) external override nonReentrant onlyLenderPool returns (address, uint256) {
        ...
        if (currentCollateralRatio < pooledCreditLineConstants[_id].idealCollateralRatio) {
            _collateralToLiquidate = _totalCollateral;
        } else if (block.timestamp >= pooledCreditLineConstants[_id].defaultsAt) {
        ...
        emit PooledCreditLineLiquidated(_id, msg.sender);
