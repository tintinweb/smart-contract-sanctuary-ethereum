...
/** 
 ...
 - Fee is charged only when buying eggs.
 - You can't buy a fractional number of eggs. Fractional parts trancated, capitalized on
the contract and go to feed all the chickens.
 - Eggs you buoght immediately hatch.
...
contract ChickenFarm {
    ...
    uint256 private immutable OneEggInitialCost = 0.004 ether; //for final version should be 0.04 ether (0.04 MATIK)
    ...
    function buyEggs(address ref) external payable {
        ...
        require(
            marketEggs >= eggsBought,
            "All eggs are sold! Come back tomorrow!"
        );
        ...
        Chickens[msg.sender] =
            Chickens[msg.sender] +
            (eggsBought * (100 - ReferralsFeePercent)) /
            100; //10% for ref
        ...
        Chickens[referrals[msg.sender]] =
            Chickens[referrals[msg.sender]] +
            (eggsBought * ReferralsFeePercent) /
            100;
    ...
    function getMyEggsForSale(address adr) public view returns (uint256) {
        // 100 chickens produce 4 eggs per day
        uint256 amount = (Chickens[adr] *
            DailyEggsPersent *
            (block.timestamp - lastHarvest[adr])) / (DAY_IN_SECONDS * 100);
        return amount;
    }
