...
contract GasTank {
    ...
    bytes32 public constant TRANSACTION_TYPEHASH = keccak256('Transaction(bytes32 sigHash,bytes32 gasTankKey,address token,uint256 fee)');
    mapping (bytes32 => mapping(address=>uint256)) public gasTankMapping;

    event Pay(bytes32 sigHash,address solver,uint256 value);
    event Deposit(bytes32 gasTankKey,address token,address addr,uint256 value);
    event Withdraw(bytes32 gasTankKey,address token,address addr,uint256 value);

    /*
        1. gasTankKey还需要对应到代币名称
        mapping(bytes32 => (address=>uint256)) 
    */

    constructor(address payable _factory,address _signer) {
        signer = _signer;
        factory = _factory;
        DOMAIN_SEPARATOR = DeforFactoryV2(_factory).DOMAIN_SEPARATOR();
    }

    function pay(bytes32 _sigHash,bytes32 _gasTankKey,address _token,uint256 _fee,bytes memory _signature) public onlyFactory{
        address from = verifyGasTank(_sigHash,_gasTankKey,_token,_fee,_signature);
        require(from == signer,"From is error");
        require(gasTankMapping[_gasTankKey][_token] >= _fee,"gasTank balance is not enough");
        if(_token == ETH_ADDRESS){
            payable(tx.origin).transfer(_fee);
        }else{
            IERC20(_token).transfer(tx.origin,_fee);
        }
        gasTankMapping[_gasTankKey][_token] -= _fee;
        emit Pay(_sigHash,tx.origin,_fee);
    }

    function verifyGasTank(bytes32 _sigHash,bytes32 _gasTankKey,address _token,uint256 _fee,bytes memory _signature) public view returns(address){
    ...
    function getEip712Data(bytes32 _sigHash,bytes32 _gasTankKey,address _token,uint256 _fee) public view returns(bytes memory){
    ...
    function deposit(bytes32 _gasTankKey,address _token,uint256 _amount) public{
        IERC20(_token).transferFrom(msg.sender,address(this),_amount);
        gasTankMapping[_gasTankKey][_token] += _amount;
        emit Deposit(_gasTankKey,_token,msg.sender,_amount);
    }

    function depositETH(bytes32 _gasTankKey) public payable{
        gasTankMapping[_gasTankKey][ETH_ADDRESS] += msg.value;
        emit Deposit(_gasTankKey,ETH_ADDRESS,msg.sender,msg.value);
    }

    function withdraw(bytes32 _gasTankKey,address _token,address _receive,uint256 _amount) public onlyOwner{
        gasTankMapping[_gasTankKey][_token] -= _amount;
        IERC20(_token).transfer(_receive,_amount);
        emit Withdraw(_gasTankKey,_token,_receive,_amount);
    }

    function withdrawETH(bytes32 _gasTankKey,address payable _receive,uint256 _amount) public onlyOwner{
        gasTankMapping[_gasTankKey][ETH_ADDRESS] -= _amount;
        _receive.transfer(_amount);
        emit Withdraw(_gasTankKey,ETH_ADDRESS,_receive,_amount);
    }

    function updateFactory(address payable _factory) public onlyOwner{
        factory = _factory;
    }
    ...
    function getStatus(bytes32 _gasTankKey,address _token,uint256 _fee) public view returns(bool){
        uint256 balance = gasTankMapping[_gasTankKey][_token];
        if(balance >= _fee){
            return true;
        }
        return false;
    ...
    modifier onlyOwner(){
        require(DeforFactoryV2(factory).owner() == msg.sender, 'Unauthorized');
        _;
    }
}

contract T{
    bytes32 public constant TRANSACTION_TYPEHASH = keccak256('Data(uint256 value)');

    function getData(uint256 _value) public pure returns(bytes memory){
        return abi.encodePacked(
            '\x19\x01',
            TRANSACTION_TYPEHASH,
            _value
    ...
    function getData2(uint256 _value) public pure returns(bytes memory){
        return abi.encode(
            '\x19\x01',
            TRANSACTION_TYPEHASH,
            _value
    ...
    function getSha3(bytes memory _data) public pure returns(bytes32){
