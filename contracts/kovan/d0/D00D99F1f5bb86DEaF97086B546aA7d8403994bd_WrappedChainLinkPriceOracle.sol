...
// File contracts/oracle/dependencies/chainlink/AggregatorV3Interface.sol
...
// File contracts/oracle/WrappedChainLinkPriceOracle.sol
...
contract WrappedChainLinkPriceOracle is OwnableUpgradeable{
    ...
    function initialize() initializer public {
     ...
     function getUnderlyingPrice(MarketTokenInterface marketToken) external view returns (uint){
        ...
        TokenConfig memory tokenConfig = tokenConfigs[address(marketToken)];
        if (tokenConfig.fixedUsd) {//if true,will return 1*e36/baseUnit
            return uint(1).mul(1e36).div(tokenConfig.baseUnit);
        }

        if(tokenConfig.sources.length == 1){  // for XXX-USD

            AggregatorV3Interface source = tokenConfig.sources[0];

            (,int _price,,,) = source.latestRoundData();
            return uint(_price).mul(1e28).div(tokenConfig.baseUnit);

        }else if(tokenConfig.sources.length == 2){ // for XXX-ETH-USD

            AggregatorV3Interface source1 = tokenConfig.sources[0];
            AggregatorV3Interface source2 = tokenConfig.sources[0];

            (,int _price1,,,) = source1.latestRoundData();
            (,int _price2,,,) = source2.latestRoundData();
            return uint(_price1).mul(uint(_price2)).mul(1e10).div(tokenConfig.baseUnit);
    ...
    function _setTokenConfig(MarketTokenInterface marketToken, AggregatorV3Interface[] memory sources,bool fixedUsd) public onlyOwner{

        TokenConfig storage tokenConfig = tokenConfigs[address(marketToken)];
        tokenConfig.underlying = marketToken.underlying();
        tokenConfig.marketToken = address(marketToken);
        tokenConfig.fixedUsd = fixedUsd;
        tokenConfig.sources = sources;
    
        if(tokenConfig.underlying == 0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE){
            tokenConfig.baseUnit = 1e18;
        }else{
            IERC20 erc20 = IERC20(tokenConfig.underlying);
        
            uint decimals = erc20.decimals();
            if(decimals == 6){
                tokenConfig.baseUnit = 1e6;
            }else if(decimals == 8){
                tokenConfig.baseUnit = 1e8;
            }else if(decimals == 18){
                tokenConfig.baseUnit = 1e18;
            }else{
                tokenConfig.baseUnit = 10 ** decimals;
            }
