...
/// @title Miner Bond Depository V2
/// @author Zeus, Indigo
/// Review by: JeffX

contract MinerBondDepositoryV2 is IBondDepository, NoteKeeper {
    /* ======== DEPENDENCIES ======== */
    ...
    constructor(
        IMinerAuthority _authority,
        IERC20 _miner,
        IgMINER _gminer,
        IStaking _staking,
        ITreasury _treasury
    ) NoteKeeper(_authority, _miner, _gminer, _staking, _treasury) {
        // save gas for users by bulk approving stake() transactions
        _miner.approve(address(_staking), 1e45);
    }
     ...
     * @return payout_     the amount of gMINER due
        ...
        )
    {
         ...
         * price = quote tokens : miner (i.e. 42069 DAI : MINER)
         *
         * 1e18 = MINER decimals (9) + price decimals (9)
         */
         ...
         *
         * this is either the number of MINER that the market can sell
        ...
        // purchased, and how much MINER has been sold
    ...
    function _tune(uint256 _id, uint48 _time) internal {
        ...
        if (_time >= meta.lastTune + meta.tuneInterval) {
            ...
            // miner decimals (9) + price decimals (9)
             ...
             * is 10,000 MINER. max payout would be 1,000 MINER (10,000 * 1 / 10).
             */
     ...
     * @param _market      [capacity (in MINER or quote), initial price / MINER (9 decimals), debt buffer (3 decimals)]
    ...
    function create(
         ...
         *
         * 1e18 = miner decimals (9) + initial price decimals (9)
         */
        uint64 targetDebt = uint64(_booleans[0] ? ((_market[0] * 1e18) / _market[1]) / 10**decimals : _market[0]);

        /*
         * max payout is the amount of capacity that should be utilized in a deposit
         * interval. for example, if capacity is 1,000 MINER, there are 10 days to conclusion,
         * and the preferred deposit interval is 1 day, max payout would be 100 MINER.
         */
        ...
        emit CreateMarket(id_, address(miner), address(_quoteToken), _market[1]);
    }
     ...
     * @param _id          ID of market
     * @return             price for market in MINER decimals
     *
     ...
     * @param _id          ID of market
     * @return             amount of MINER to be paid in MINER decimals
     *
     * @dev 1e18 = miner decimals (9) + market price decimals (9)
     */
     ...
     * @param _id          ID of market
     * @return             current debt for market in MINER decimals
     */
     ...
     * @param _id          ID of market
     * @return             control variable for market in MINER decimals
     */
...
interface IBondDepository {
    ...
    struct Market {
        ...
        bool capacityInQuote; // capacity limit is in payment token (true) or in MINER (false, default)
    ...
    struct Terms {
        ...
        uint64 maxDebt; // 9 decimal debt maximum in MINER
    }
...
interface INoteKeeper {
    ...
    struct Note {
        uint256 payout; // gMINER remaining to be paid
    ...
    function redeemAll(address _user, bool _sendgMINER) external returns (uint256);
...
abstract contract FrontEndRewarder is MinerAccessControlled {
    /* ========= STATE VARIABLES ========== */
    ...
    IERC20 internal immutable miner; // reward token

    constructor(IMinerAuthority _authority, IERC20 _miner) MinerAccessControlled(_authority) {
        miner = _miner;
    }
    ...
    function getReward() external {
        ...
        miner.transfer(msg.sender, reward);
    }
...
abstract contract NoteKeeper is INoteKeeper, FrontEndRewarder {
    ...
    IgMINER internal immutable gMINER;
    ...
    constructor(
        ...
        IStaking _staking,
        ITreasury _treasury
    ) FrontEndRewarder(_authority, _miner) {
        gMINER = _gminer;
        staking = _staking;
        treasury = _treasury;
    }
     ...
     * @param _payout      the amount of MINER due to the user
    ...
    function addNote(
        ...
        notes[_user].push(
            Note({
                payout: gMINER.balanceTo(_payout),
        ...
        // note that only the payout gets staked (front end rewards are in MINER)
     ...
     * @param _sendgMINER    send payout as gMINER or vMINER
     * @return payout_     sum of payout sent, in gMINER
     */
    function redeem(
        address _user,
        uint256[] memory _indexes,
        ...
        if (_sendgMINER) {
            gMINER.transfer(_user, payout_); // send payout as gMINER
        } else {
            staking.unwrap(_user, payout_); // unwrap and send payout as vMINER
        }
    }
     ...
     * @param _sendgMINER    send payout as gMINER or vMINER
     * @return             sum of payout sent, in gMINER
     */
    function redeemAll(address _user, bool _sendgMINER) external override returns (uint256) {
        return redeem(_user, indexesFor(_user), _sendgMINER);
    }
     ...
     * @return payout_     the payout due, in gMINER
