...
contract YearnMock is ERC20, VaultAPI, Ownable {
	...
	// 	}
	// 	else {
	// 		// price = dai.balanceOf(address(this)) * (10 ** 18) / totalSupply();
	// 		price = dai.balanceOf(address(this))*(10**uint256(decimals()))/totalSupply();
	// 	}
	// 	return price;
	// }
	...
	function _sharesForValue(uint256 amount) public view returns (uint256)
	{
		...
		{
			// return totalSupply().mul(amount).div(totalBalance);
	...
	// 	if (totalShares > 0) {
	// 		return
	// 			dai.balanceOf(address(this)).mul(numShares).div(totalShares);
	// 	} else {
	// 		return numShares;
	// 	}
	// }
	...
	// {
	// 	uint256 yTokens;
	// 	uint256 deposited = dai.balanceOf(address(this));
	...
	// 	else 
	// 	{
	// 		yTokens = totalSupply() * amount / (dai.balanceOf(address(this)) - amount);
	// 		// yTokens = ((deposited + amount)*(totalSupply()) / deposited) - totalSupply();
	// 	}
	// 	_mint(msg.sender, yTokens);

	// 	dai.transferFrom(msg.sender, address(this), amount);
	// 	return (yTokens);
	// }

	// /// @param amount - amount of yTokens.
	// function withdraw(uint256 amount, address receiver) external override returns (uint256)
	// {
	// 	// uint256 returnAmount = amount * pricePerShare();
	// 	uint256 returnAmount = _shareValue(amount);
