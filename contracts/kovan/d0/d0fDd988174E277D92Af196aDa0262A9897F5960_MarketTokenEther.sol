...
abstract contract AbstractMarketToken is Exponential, TokenErrorReporter {
...
abstract contract MarketToken is AbstractMarketToken, ReentrancyGuardUpgradeable{
    ...
    function init(address underlying_,
        ...
        require(err == NO_ERROR, "setting reserve factor failed");
    ...
    function accrueInterest() public returns(uint){
        ...
        (MathError mathErr, uint timeDelta) = subUInt(currentBlockTimestamp, accrualBlockTimestampPrior);
        require(mathErr == MathError.NO_ERROR, "could not calculate time delta");
        ...
        (mathErr, simpleInterestFactor) = mulScalar(Exp({mantissa: borrowRateMantissa}), timeDelta);
    ...
    function mintFresh(address minter, uint mintAmount) internal returns (uint, uint) {
        ...
        if (vars.mathErr != MathError.NO_ERROR) {
            revert ExchangeRateReadFailed(uint(vars.mathErr));
        }
        ...
        return (NO_ERROR, vars.actualMintAmount);
    }
    ...
    function redeemFresh(address payable redeemer, uint redeemTokensIn, uint redeemAmountIn) internal returns (uint) {
        ...
        if (redeemTokensIn > 0) {
            ...
            if (vars.mathErr != MathError.NO_ERROR) {
                revert RedeemExchangeTokenCalculationFailed(uint(vars.mathErr));
            }
        } else {
            /*
            ...
            if (vars.mathErr != MathError.NO_ERROR) {
                revert RedeemExchangeAmountCalculationFailed(uint(vars.mathErr));
            }
    ...
    function repayBorrowFresh(address payer, address borrower, uint repayAmount) internal returns (uint, uint) {
        ...
        return (NO_ERROR, vars.actualRepayAmount);
    }
     ...
     * @return (uint, uint) An error code (0=success, otherwise a failure, see ErrorReporter.sol), and the actual repayment amount.
     */
    function liquidateBorrowInternal(address borrower, uint repayAmount, MarketToken marketTokenCollateral) internal nonReentrant returns (uint, uint) {
        accrueInterest();
     ...
     * @return (uint, uint) An error code (0=success, otherwise a failure, see ErrorReporter.sol), and the actual repayment amount.
     */
    function liquidateBorrowFresh(address liquidator, address borrower, uint repayAmount, MarketToken marketTokenCollateral) internal returns (uint, uint) {
        ...
        if (repayBorrowError != NO_ERROR) {
            revert LiquidateRepayBorrowFreshFailed(uint(repayBorrowError));
        }
        ...
        require(seizeError == NO_ERROR, "token seizure failed");
    ...
    function seizeInternal(address seizerToken, address liquidator, address borrower, uint seizeTokens) internal returns (uint) {
        ...
        require(vars.mathErr == MathError.NO_ERROR,"LIQUIDATE_SEIZE_BALANCE_DECREMENT_FAILED");
        ...
        require(vars.mathErr == MathError.NO_ERROR,"LIQUIDATE_SEIZE_BALANCE_INCREMENT_FAILED");
    ...
    function _addReservesInternal(uint addAmount) internal nonReentrant returns (uint) {
        ...
        (uint err, ) = _addReservesFresh(addAmount);
        return err;
    }
    ...
    function sweepToken(IERC20 token) external {
        ...
        require(msg.sender == admin,"MarketToken::sweepToken: only admin can call this");
...
// File contracts/market/MarketTokenEther.sol


pragma solidity ^0.8.0;

contract MarketTokenEther is MarketToken {
    ...
    function mint() external payable {
        ...
        require(err == NO_ERROR, "mint failed");
    }
    ...
    function repayBorrow() external payable {
        ...
        require(err == NO_ERROR, "repayBorrow failed");
    }
    ...
    function repayBorrowBehalf(address borrower) external payable {
        ...
        require(err == NO_ERROR, "repayBorrowBehalf failed");
    }
    ...
    function liquidateBorrow(address borrower, MarketToken marketTokenCollateral) external payable {
        ...
        require(err == NO_ERROR, "liquidateBorrow failed");
    }
