...
contract PerpetualTranche is ERC20, Initializable, Ownable, IPerpetualTranche {
    ...
    function rollover(
        ...
        require(isAcceptableRollover(trancheIn, trancheOut), "Expected rolling over tranches into the queue");
    ...
    /// @inheritdoc IPerpetualTranche
    function isAcceptableRollover(ITranche trancheIn, ITranche trancheOut) public override returns (bool) {
        IBondController bondIn = IBondController(trancheIn.bond());
        IBondController bondOut = IBondController(trancheOut.bond());
        return (bondIn != bondOut && // Expected bondIn and bondOut NOT be the same
            bondIn == getDepositBond() && // Expected trancheIn to be of deposit bond
