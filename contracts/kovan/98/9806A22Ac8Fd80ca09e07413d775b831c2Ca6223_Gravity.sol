...
contract Gravity is KeeperCompatibleInterface {
    ...
    constructor(address _sourceToken, address _targetToken, uint _upKeepInterval, ISwapRouter _swapRouter) {
    ...
    function initiateNewStrategy(address _sourceAsset, address _targetAsset, uint _sourceBalance, uint _interval, uint _purchaseAmount) public {
        ...
        // populate purchaseOrders mapping
        uint _unixNextSlot = _accountStart - (_accountStart % upKeepInterval) + upKeepInterval;
    ...
    */
    ...
    function checkUpkeep(bytes calldata /* checkData */) external override returns (bool upkeepNeeded, bytes memory /* performData */) {
        ...
        if((block.timestamp - lastTimeStamp) > upKeepInterval) {
            ...
            uint256 _nextSlot = _now - (_now % upKeepInterval) + (upKeepInterval*2);
            lastTimeStamp = _nextSlot;
    ...
    function performUpkeep(bytes calldata /* performData */) external override {
        ...
        uint256 _now = block.timestamp;
        uint256 _nextSlot = _now - (_now % upKeepInterval) + (upKeepInterval*2);
        emit InsideUpkeep(block.timestamp);
        uint256 _total = accumulatePurchaseOrders(_nextSlot);
        if (_total > 0) {
            //swapExactInputSingle(address _tokenIn, address _tokenOut, uint256 amountIn,0)
    ...
    */
    ...
    function swapExactInputSingle(address _tokenIn, address _tokenOut, uint256 amountIn, uint256 _amountOutMin) external returns (uint256 amountOut) {
        ...
        // TO DO Naively set amountOutMinimum to 0. In production, use an oracle or other data source to choose a safer value for amountOutMinimum.
        // TO DO We also set the sqrtPriceLimitx96 to be 0 to ensure we swap our exact input amount.
