...
contract assetTokenisation is ERC1155, Ownable {
    ...
    uint256 Id;
    IERC20 USDC = IERC20(0x2519f6FC924BE4ac916152059C46E1bFeB5CfFcC);   //For Kovan Testnet
    address adminWallet;
    ...
    function addProperty(string memory _userId, address _user, string memory _propertyId, uint256 _propertyPrice,
        ...
        _mint(userDetails[_userId].user, propertyDetails[_propertyId].propertyTokenId, _investmentPortion, "" );    //Create Property specific ERC20 tokens based on ERC1155 standard
    ...
    function transferDividend(string memory _propertyId) public onlyOwner { 
        ...
        USDC.transferFrom(userDetails[propertyOwner[_propertyId]].user,adminWallet,_maintenanceFee);
        ...
        for(uint256 i = 0; i< investors[_propertyId].length; i++) {
            if(isInvestor[_propertyId][investors[_propertyId][i]]) {
                ...
                uint256 currentTime = block.timestamp;   //Convert seconds in days
                uint256 recordedTime = investmentTime[investors[_propertyId][i]][_propertyId] - currentTime;
                uint256 investedTime = recordedTime / 86400 ;
                uint256 _dividendAmount = investedTime * dividendAmount;    //Should this be divided by 30
