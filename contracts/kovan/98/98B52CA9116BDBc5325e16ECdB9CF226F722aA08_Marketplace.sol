...
contract Marketplace is Ownable, ReentrancyGuard
{
    ...
    function _StartSales(Sale[] memory NewSaleInstance) external onlyBRTOperator   
    {
        ...
        for(uint i; i < NewSaleInstance.length; i++)
        {
            ...
            Sales[UniqueSaleIndex].ActivePublic = bool(NewSaleInstance[i].ActivePublic);
            Sales[UniqueSaleIndex].ActiveBrightList = bool(NewSaleInstance[i].ActiveBrightList);
            Sales[UniqueSaleIndex].AllowMultiplePurchases = bool(NewSaleInstance[i].AllowMultiplePurchases);
        }
