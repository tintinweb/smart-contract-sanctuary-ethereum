...
contract MultiFeeDistribution is ReentrancyGuard, Ownable {
    ...
    IFund public stakingTokenFund;
    address[] public rewardTokens;
    mapping(address => Reward) public rewardData;

    // Duration that rewards are streamed over
    uint256 public constant rewardsDuration = 60;//86400 * 7; // 1 week

    // Duration of lock/earned penalty period
    uint256 public constant lockDuration = rewardsDuration * 30; // 13 weeks
    ...
    constructor(address _stakingToken, address _stakingTokenFund) Ownable() {
        stakingToken = IERC20(_stakingToken);
        stakingTokenFund = IFund(_stakingTokenFund);
    ...
    function removeMinter(address _minter) external onlyOwner {
        require(minters[_minter], "minter not existed");
    ...
    function approveRewardDistributor(
        ...
        require(rewardData[_rewardsToken].lastUpdateTime > 0, "reward token not found");
        rewardDistributors[_rewardsToken][_distributor] = _approved;
    }

    /* ========== VIEWS ========== */

    /// @dev return reward per token with REWARD_PRECISION to prevent rounded error
    ...
    function _earned(
        ...
        return
            _balance
                .mul(_rewardPerToken(_rewardsToken, supply)
                .sub(userRewardPerTokenPaid[_user][_rewardsToken]))
                .div(1e18)
    ...
    function mint(address user, uint256 amount) external updateReward(user) {
        require(minters[msg.sender], "Only minters allowed");
        totalSupply = totalSupply.add(amount);
        Balances storage bal = balances[user];
        bal.total = bal.total.add(amount);
        ...
        stakingTokenFund.transferTo(address(stakingToken), address(this), amount);
    ...
    function notifyRewardAmount(address _rewardsToken, uint256 reward) external updateReward(address(0)) {
        require(rewardDistributors[_rewardsToken][msg.sender], "Only reward distributors allowed");
