...
contract MarketPlace is MarketPlaceStorage, AccessControl, IERC1155Receiver, Ownable {
    ...
    function cancelSell721(uint _index) external {
        ...
        uint last = sellOrdersByCollection[sell.collection].length - 1;
    ...
    function sell1155(address _collection, uint _tokenId, uint _amount, uint _price, address _paymentToken) external {
        ...
        require (IERC1155(_collection).balanceOf(msg.sender, _tokenId) > 0, "Does not have items");
        require (whitelistedPaymentTokens[_paymentToken], "Invalid payment token");
     ...
     * @param _is721 True if the collections is an ERC721, false if it is an ERC1155
     */
    function removeCollectionToWhitelist(uint _index, bool _is721) external onlyRole(DEFAULT_ADMIN_ROLE) {
        address token;
     ...
     */
    function changeTreasury(address _treasury) external onlyRole(DEFAULT_ADMIN_ROLE) {
        treasury = _treasury;
    }
    ...
    // INTERNAL METHODS
    function processNativePayment(address _collection, uint _price, address _seller, address _creator, uint256 _royalties) internal {
        require (msg.value >= _price, "Not enough funds");

        // Platform Fees
        uint biPercentFee = ITokenNFT(_collection).getBIFeePercent();
        uint platformFee = _price * biPercentFee / PERCENT_DIVIDER;
        ...
        // transfer payment
        uint sellerAmount = (_price.sub(platformFee)).sub(_royalties);
    ...
    function processPayment(address _collection, address _token, uint _price, address _seller, address _creator, uint256 _royalties) internal {
        ...
        // process fee payment
        IERC20(_token).safeTransfer(treasury, platformFee);

        // royalty fees to creator
        IERC20(_token).safeTransfer(_creator, _royalties);
