...
contract LL420RollOGList is VRFConsumerBase {
    ...
    constructor (address _vrfCoordinator, address _linkToken, bytes32 _keyHash, uint256 _fee, uint _spotLimit) VRFConsumerBase(_vrfCoordinator, _linkToken) {
        keyHash = _keyHash;
        fee = _fee;
    ...
    function batchRoll(uint _count) public {
    ...
    function setSpotLimit(uint _newLimit) external onlyOwner {
    ...
    function random() internal returns (uint) {
        totalRuns += 1;
        return uint(keccak256(abi.encodePacked(randomNumber, totalRuns)));
    }
