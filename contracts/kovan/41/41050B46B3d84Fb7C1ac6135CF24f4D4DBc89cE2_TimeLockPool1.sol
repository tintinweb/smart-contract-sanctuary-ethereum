...
contract TimeLockPool1 is BasePool, ITimeLockPool {
    ...
    mapping(uint256 => mapping(address => uint256))
        public initialBalanceWhenPaused;

    mapping(uint256 => mapping(address => uint256))
        public userWithdrawalsWhenPaused;
    ...
    function withdraw(
        ...
        if (withdrawalsPaused == true) {
            if (initialBalanceWhenPaused[pauseId][_msgSender()] == 0) {
                initialBalanceWhenPaused[pauseId][_msgSender()] = userDeposit
                    .amount;
            }
            uint256 maxWithdrawableAmount = initialBalanceWhenPaused[pauseId][
                _msgSender()
            ] -
                (initialBalanceWhenPaused[pauseId][_msgSender()] *
                    pauseWithdrawalsPercent) /
                BP;
        ...
        // remove ITimeLockPool.Deposit if user withdraws everything
        if (_amount == userDeposit.amount) {
            ...
            userDeposit.amount = userDeposit.amount - _amount;
        }
        ...
        emit Withdrawn(_depositId, _receiver, _msgSender(), _amount);
    }
...
interface IVoteAnzenToken is IERC20, IERC20Permit {
...
abstract contract BasePool is AbstractRewards, IBasePool, TokenSaver {
    using SafeERC20 for IERC20;
    using SafeCast for uint256;
    using SafeCast for int256;

    IVoteAnzenToken public immutable veAnzenToken;
    ...
    // balanceOf => getSharesOf
    // totalSupply => getTotalShares
    constructor(
        address _depositToken,
        address _rewardToken,
        address _veAnzenToken,
        ...
        AbstractRewards(
            IVoteAnzenToken(_veAnzenToken).balanceOf,
            IERC20(_rewardToken).balanceOf
        )
    {
        require(
            _veAnzenToken != address(0),
            "BasePool.constructor: veAnzen token must be set"
        );
        require(
            _escrowPortion <= 1e18,
    ...
    function _mint(address _account, uint256 _amount) internal virtual {
        veAnzenToken.mint(_account, _amount);
        _correctPoints(_account, -(_amount.toInt256()));
    }

    function _burn(address _account, uint256 _amount) internal virtual {
        veAnzenToken.burn(_account, _amount);
        _correctPoints(_account, _amount.toInt256());
    }

    function distributeRewards(uint256 _amount) external override {
        // Brings rewards to this contract
        rewardToken.safeTransferFrom(_msgSender(), address(this), _amount);
        // Calculates the pointsPerShare
...
abstract contract AbstractRewards is IAbstractRewards {
    ...
    function(address) external view returns (uint256) private getSharesOf; // VeANZ balanceOf
    // function() external view returns (uint256) private getTotalShares; // VeANZ getTotalShares
    function(address) external view returns (uint256) private rewardBalanceOf;
    ...
    constructor(
        function(address) external view returns (uint256) getSharesOf_,
        function(address) external view returns (uint256) rewardBalanceOf_
    ) {
        getSharesOf = getSharesOf_;
        rewardBalanceOf = rewardBalanceOf_;
    }
    ...
    function withdrawableRewardsOf(address _account)
        ...
        returns (uint256)
    {
        // Becuase less rewards are distributed as time goes on, this subtraction will revert the second time
    ...
    function cumulativeRewardsOf(address _account)
        ...
        returns (uint256)
    {
        // Gives les rewards when it gets closer to the deposit end
        // (200/1000) * X + -20
        // (200/1000) * 200 + -20 = 20 rewards
        // (200/1000) * 198 + -20 = 19,6 rewards
        // (200/1000) * 100 + -20 = 0 rewards
        return
    ...
    function _distributeRewards(uint256 _amount) internal {
        // uint256 shares = getTotalShares(); // VeANZ total supply tokens 
        uint256 shares = rewardBalanceOf(address(this)); // ANZ pool balance
        require(
            shares > 0,
