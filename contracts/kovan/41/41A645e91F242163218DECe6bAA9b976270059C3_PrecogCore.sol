...
contract PrecogCore is IPrecogCore {
    ...
    function _updateAdjustments() internal {
        IPrecogStorage.TokenPair[] memory pairs = IPrecogStorage(precogStorage).getExistingTokensPair();
        for (uint i = 0; i < pairs.length; i++) {
            
            precogInternal.updateCurrentTradingCycle(pairs[i].token, false, 2**255);

            CyclesChangedInfo memory _cycleChangedInfo = cyclesChangedInfo[pairs[i].token];
            ...
            IPrecogStorage.Cycle memory _futureTradingcycle = precogInternal.getTradingCycle(
                pairs[i].token,
                _nextTradingTime
            );
            ...
            cyclesChangedInfo[pairs[i].token] = CyclesChangedInfo({
