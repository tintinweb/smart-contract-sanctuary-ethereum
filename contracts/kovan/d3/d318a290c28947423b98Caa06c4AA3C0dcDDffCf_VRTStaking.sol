...
contract VRTStaking  {
	...
	constructor(VRT _vrt) public {
        vrt = _vrt;
        owner = msg.sender;
        plans.push(Plan(15, 10));
        plans.push(Plan(30, 20));
        plans.push(Plan(90, 90));
        plans.push(Plan(180, 200));
        plans.push(Plan(365, 500));
	}

	function invest(address referrer, uint8 plan, uint256 stakedAmount) public  {
		...
		if (user.referrer != address(0)) {
			...
			for (uint256 i = 0; i < 3; i++) {
				if (upline != address(0)) {
					uint256 amount = stakedAmount.mul(REFERRAL_PERCENTS[i]).div(PERCENTS_DIVIDER);
		...
		(uint256 percent, uint256 profit, uint256 finish) = getResult(plan, stakedAmount);

		user.deposits.push(Deposit(plan, percent, stakedAmount, profit, block.timestamp, finish));

		totalStaked = totalStaked.add(stakedAmount);
        vrt.transferFrom(msg.sender, address(this), stakedAmount);
		emit NewDeposit(msg.sender, plan, percent, stakedAmount, profit, block.timestamp, finish);
	}
	...
	function withdraw() public {
		...
		uint256 contractBalance = vrt.balanceOf(address(this));
		...
		vrt.approve(msg.sender, totalAmount);
        vrt.transferFrom(address(this), msg.sender, totalAmount);
		emit Withdrawn(msg.sender, totalAmount);

	}

	function getContractBalance() public view returns (uint256) {
		return vrt.balanceOf(address(this));
	}
