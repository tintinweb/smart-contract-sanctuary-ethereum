...
contract DssProxyActions is Common, DSAuth {

    uint256[50] _gap;
    uint public feeDeposit;
    uint public feeWithdraw;
    uint public feeDepositGem;
    uint public feeWithdrawGem;
    address public treasury;
    uint decimal = 10000;
    mapping(address => bool) public listStableTokens;

    function setFee(uint _feeDepositETH, uint _feeWithdrawETH, uint _feeDepositGem, uint _feeWthdrawGem) public auth {
    ...
    function setTreasury(address _treasury) public auth {
        require( _treasury != address(0), "invalid address");
        treasury = _treasury;
    }

    function addListToken(address _token) public auth {
        require( _token != address(0), "invalid address");
        listStableTokens[_token] = true;
    }

    function removeListToken(address _token) public auth {
        require(listStableTokens[_token] = true, "token is not on the white list");
        listStableTokens[_token] = false;
    }
    // Internal functions
    ...
    function ethJoin_join(address apt, address urn, uint _feeDeposit) public payable {
        if (_feeDeposit > 0) {
            payable(treasury).transfer(_feeDeposit);
        }
        // Wraps ETH in WETH
    ...
    function gemJoin_join(address apt, address urn, uint amt, bool transferFrom, uint _feeDepositGem) public {
        ...
        if (transferFrom) {
            if (_feeDepositGem > 0) {
                GemJoinLike(apt).gem().transferFrom(msg.sender, treasury, _feeDepositGem);
            }
    ...
    function getFee(address _gemJoin, uint percent, uint value) internal returns (uint fee) {
        address token = address(GemJoinLike(_gemJoin).gem());
        fee = !listStableTokens[token] ? value * percent/decimal : 0;
    }
    ...
    function lockETH(
        ...
        uint _fee = getFee(ethJoin, feeDeposit, msg.value);
    ...
    function lockGem(
        ...
        uint _fee = getFee(gemJoin, feeDepositGem, amt);
    ...
    function freeETH(
        ...
        uint _fee = getFee(ethJoin, feeWithdraw, wad);
        transferFeeWithdrawETH(msg.sender, treasury, wad, _fee);
    }
    ...
    function freeGem(
        ...
        uint _fee = getFee(gemJoin, feeWithdrawGem, amt);
        ...
        if (_fee > 0) {
            GemJoinLike(gemJoin).gem().transferFrom(msg.sender, treasury, _fee);
        }
    ...
    function lockGemAndDraw(
        ...
        uint _fee = getFee(gemJoin, feeDepositGem, amtC);
        // Takes token amount from user's wallet and joins into the vat
    ...
    function wipeAndFreeETH(
        ...
        // Sends ETH back to the user's wallet
        uint _fee = getFee(ethJoin, feeWithdraw, wadC);
        transferFeeWithdrawETH(msg.sender, treasury, wadC, _fee);
    }
    ...
    function wipeAndFreeGem(
        ...
        flux(manager, cdp, address(this), wadC);
        // Exits token amount to the user's wallet as a token
        uint _fee = getFee(gemJoin, feeWithdrawGem, wadC);
        if (_fee > 0) {
            GemJoinLike(gemJoin).gem().transferFrom(msg.sender, treasury, _fee);
        }
