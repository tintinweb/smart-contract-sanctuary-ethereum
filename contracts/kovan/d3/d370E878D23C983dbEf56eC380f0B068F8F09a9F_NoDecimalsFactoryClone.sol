...
contract NoDecimalsFactoryClone {
    ...
    address public immutable _vaultAddress;
    address public immutable _lbpCreateProxyAddress;
    address public immutable _instantMintSwapProxyAddress;
    ...
    constructor(
        ...
        _lbpCreateProxyAddress = lbpCreateProxyAddress;
        _instantMintSwapProxyAddress = instantMintSwapProxyAddress;
    ...
    function createToken(
        ...
        NoDecimalsERC20Alpha(clone).initialize(
            ...
            initialSupply,
            _vaultAddress,
            _lbpCreateProxyAddress,
            _instantMintSwapProxyAddress,
            nftAddress
        );
...
contract NoDecimalsERC20Alpha is Initializable, ERC20Upgradeable, ERC20BurnableUpgradeable {
    ...
    address public _lbpCreateProxyAddress;
    address public _instantMintSwapProxyAddress;
    address public _nftAddress; // ERC721 or ERC1155
    ...
    function initialize(
        ...
        _mint(mintTo, initialAmount + 3 * (10**decimals()));
    }
        ...
        require(
            (totalSupply() == 0 ||
                (_vaultAddress != address(0) &&
                    _instantMintSwapProxyAddress != address(0) &&
                    _nftAddress != address(0))),
            "Burn to mint: Vault, swap proxy, or nft contract address not set"
        );
        require(
            totalSupply() == 0 ||
                (to == _lbpCreateProxyAddress) || // deposit into copper proxy
                (from == _lbpCreateProxyAddress) || // withdraw from copper proxy or create lbp
                (from == _vaultAddress) || // buy from vault (can only send to Vault through create proxy)
                (to == _nftAddress) || // send to NFT for mint
                (from == _nftAddress && to == address(0)), // burn to mint
            "Burn to mint: Specified transfer not supported within flow"
        );
        bool isLessThanMaximum = totalSupply() == 0 || totalSupply() - amount >= (3 * (10**decimals()));
        require(
            isLessThanMaximum || from == _lbpCreateProxyAddress || to == _lbpCreateProxyAddress,
