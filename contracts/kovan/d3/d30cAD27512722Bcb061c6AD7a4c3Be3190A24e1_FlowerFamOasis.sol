...
contract FlowerFamOasis is SimpleAccessUpgradable {
        ...
        external
    {
        ...
        if (walletAmount > 0) {
            honeyCoinReserve.releaseFunds(walletAmount, msg.sender);
        }
    ...
    function getTotalProductionOfUser(address user) external view returns (uint256) {
    ...
    function getUserLands(address user) external view returns (UserLandInfo[] memory) {
    ...
    function setShares(uint256 _marketing, uint256 _liquidity) external onlyAuthorized {
    ...
    function shouldTriggerMarketing() internal view returns (bool) {
        return (taxActionCounter + marketingActionTriggerOffset) % taxActionTriggerMark == 0 && allocMarketing > 0 && actionActive;
    }

    function shouldTriggerLiquidity() internal view returns (bool) {
        return (taxActionCounter + liquidityActionTriggerOffset) % taxActionTriggerMark == 0 && allocLiquidity > 0 && actionActive;
    }
    ...
    function _getStakedAssetReward(address user) internal view returns (uint256) {
    ...
    function _getLastClaimedBonusPercentage(address user) internal view returns (uint256) {
    ...
    function _getTotalProductionPerDay(address user) internal view returns (uint256) {
        uint256 amount = 0;
    ...
    function _getTotalProductionOfUser(address user) internal view returns (uint256) {
        ...
        uint256 claimAmount = _getNotAccumulatedProduction(
            ...
            userOasisProduction.totalClaimed;
    ...
    function _getUserLandYield(uint256 landId, address user) internal view returns (uint256) {
        uint256 count = userToLandSpots[user][landId];
