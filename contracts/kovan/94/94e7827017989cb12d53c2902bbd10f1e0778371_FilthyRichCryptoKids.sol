...
contract FilthyRichCryptoKids is ERC721Enumerable, Ownable, VRFConsumerBase{
     ...
     */
    uint256 internal link_fee = 0.1 * 10 ** 18 wei; // 0.1 LINK (KOVAN)
    address internal _link = 0xa36085F69e2889c224210F603D836748e7dC0088;
    address internal _vrfCoordinator = 0xdD3782915140c8f3b190B5D67eAc6dc5760C46E9;
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory){
        ...
        if(tokenId < maxVipSupply){
            ...
            // Need to get a random number between maxVip and maxSupply, 10000 - 1000 = 9000
            // Shift it by randomness that is got, so its between 1000 - 9999
            uint n = maxSupply.sub(maxVipSupply);
            uint tokenId_n = tokenId.add(randomness) % n;
            tokenId_n = tokenId_n.add(maxVipSupply);
            return bytes(baseURI).length > 0 ? string(abi.encodePacked(baseURI, Strings.toString(tokenId_n))) : ""; 
        }   
    }
    ...
    function totalSupply() public view override returns (uint256){
        return (minted.sub(maxVipSupply)) + vipMinted;
    }
    ...
    function getValuables(uint tokenId) external view returns(string[] memory traits, uint[] memory types) {
        require(valuablesContract != address(0), "FRCK: Rare items contract not yet added.");
    ...
    /**
        Overiddes the transfer function to limit trading until enabled
     */
    function _transfer(
        address from,
        address to,
