    ...
    ERC721URIStorage
{
    ...
    event packSaleStateChanged(bool val);
    ...
    /** Pack sale inital state */
    bool public packSaleActive = true;
...
//boolean function to call when a season is closed 
/// @notice Sets packs sales to a closing state state
/// @param val The new value
    function setPackSaleState(bool val) 
    external 
    onlyRole(MINTER_ROLE) 
    {
        packSaleActive = val;
        emit packSaleStateChanged(val);
    }
        ...
        onlyRole(MINTER_ROLE)
    {
        ...
        for (uint i = 0; i < externalContracts.length; i++) {
            IERC721(externalContracts[i]).safeTransferFrom(
                sweetAdminWallet,
                ownerOfPack,
                momentsSerial[i]
            );
