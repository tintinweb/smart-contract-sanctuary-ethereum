...
// ----------------------------------------------------------------------------
// Send MATIC to Redeem Your Fedora Gold Tokens - FEDG
...
// Decimals    : 18
// Price : enter price while creating contract or set later (this feature is not used)
// Details of Contract : Users will be able to Redeem FEDG tokens by sending > 1 Matic amount to contract.
// Users will have to enter Fedora Gold Coin Transaction Txn ID and Proof Of Purchase id after sending FED coins to Burn address.
...
// Rule : One ERC 20 address can only redeem once.
// Rule : One transaction hash can only be used once.
// ------------------------------------------------------------------------------------------------------------
...
contract FedoraGoldTokenSale {
   ...
   function redeem() public payable {
      ...
      uint tokensToGet;
      uint etherUsed = msg.value;
      tokensToGet = p.amountcoins.mul(1).mul(10**18);
