...
contract ERC20Voting is MajorityVoting {
    ...
    event StartVoteToSetRatio(uint voteId, uint[] quality, uint[] price);
    event StartVoteToSetLevel(uint voteId, uint[] idToken, uint[] quality);
    ...
    function initialize(
        ...
        __MajorityVoting_init(_dao, _participationRequiredPct, _supportRequiredPct, _minDuration);
    ...
    function newVote(
        ...
        uint64 _endDate,
        VoterState _choice
    ) external override returns (uint voteId) {}
    ...
    function _vote(
        ...
        uint voterStake = votingToken.getPastVotes(_voter, vote_.snapshotBlock);
    ...
    function newVoteToCreateVault(
        IDAO.Action[] calldata _actions,
        uint64 _startDate,
        uint64 _endDate,
        VoterState _choice,
        string[] memory vaultInfo,
        uint[] memory quality,
        uint[] memory price,
        address assetAddress
    ) external returns (uint voteId) {
        uint64 snapshotBlock = getBlockNumber64() - 1;

        uint votingPower = votingToken.getPastTotalSupply(snapshotBlock);
        ...
        emit StartVote(voteId, _msgSender());
        emit StartVoteToCreateVault(voteId, vaultInfo, assetAddress, quality, price);

        if (_choice != VoterState.None && _canVote(voteId, _msgSender())) {
            _vote(voteId, _choice, _msgSender());
        }
    }

    function newVoteToSetRatio(
        IDAO.Action[] calldata _actions,
        uint64 _startDate,
        uint64 _endDate,
        VoterState _choice,
        ...
        emit StartVote(voteId, _msgSender());
        emit StartVoteToSetRatio(voteId, quality, price);
    ...
    function newVoteToSetLevel(
        ...
        emit StartVote(voteId, _msgSender());
        emit StartVoteToSetLevel(voteId, idToken, quality);
    ...
    function _canVote(uint _voteId, address _voter) internal view override returns (bool) {
        Vote storage vote_ = votes[_voteId];
...
import "./../../dao/component/MetaTxComponent.sol";
...
abstract contract MajorityVoting is IMajorityVoting, MetaTxComponent, TimeHelpers {
    ...
    function newVote(
        ...
        uint64 _endDate,
        VoterState _choice
    ) external virtual returns (uint voteId);

    /// @inheritdoc IMajorityVoting
    function vote(uint _voteId, VoterState _choice) external {
        if (_choice != VoterState.None && !_canVote(_voteId, _msgSender()))
            revert VoteCastForbidden(_voteId, _msgSender());
        _vote(_voteId, _choice, _msgSender());
    }
    ...
    /// @inheritdoc IMajorityVoting
    function getVoterState(uint _voteId, address _voter) public view returns (VoterState) {
        return votes[_voteId].voters[_voter];
    }

    /// @inheritdoc IMajorityVoting
    function canVote(uint _voteId, address _voter) public view returns (bool) {
        return _canVote(_voteId, _voter);
    }

    /// @inheritdoc IMajorityVoting
    function getVote(uint _voteId)
        public
        view
        returns (
            bool open,
    ...
    function _execute(uint _voteId) internal virtual {
    ...
    function _canVote(uint _voteId, address _voter) internal view virtual returns (bool);

    /// @dev Internal function to check if a vote can be executed. It assumes the queried vote exists.
    /// @param _voteId vote id
    /// @return True if the given vote can be executed, false otherwise
    function _canExecute(uint _voteId) internal view virtual returns (bool) {
        Vote storage vote_ = votes[_voteId];
    ...
    function _isValuePct(
        ...
        uint computedPct = (_value * PCT_BASE) / _total;
...
interface IMajorityVoting {
    ...
    error VoteCastForbidden(uint voteId, address sender);
    error VoteExecutionForbidden(uint voteId);
    error VotePowerZero();

    event StartVote(uint indexed voteId, address indexed creator);
    event CastVote(uint indexed voteId, address indexed voter, uint8 voterState, uint voterWeight);
    event ExecuteVote(uint indexed voteId, bytes[] execResults);
    ...
    function canVote(uint _voteId, address _voter) external view returns (bool);

    function execute(uint _voteId) external;

    function getVoterState(uint _voteId, address _voter) external view returns (VoterState);
...
abstract contract IDAO {
    ...
    struct Action {
        ...
        uint value; // Value to be sent with the call. for example (ETH)
    ...
    function execute(uint callId, Action[] memory _actions) external virtual returns (bytes[] memory);

    event Executed(address indexed actor, uint callId, Action[] actions, bytes[] execResults);
    ...
    event Deposited(address indexed sender, address indexed token, uint amount, string _reference);

    event ETHDeposited(address sender, uint amount);
    ...
    event Withdrawn(address indexed token, address indexed to, uint amount, string _reference);
