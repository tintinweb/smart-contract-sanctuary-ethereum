...
contract PrecogCore is IPrecogCore {
    ...
    uint32 firstInvestmentCycle = 604800; // 7 days
    uint32 firstWithdrawalCycle = 604800; // 7days
    uint32 investmentCycle = 86400; // 1 day
    uint32 withdrawalCycle = 172800; // 2 days
    uint32 profitCycle = 86400; // 1 day
    uint32 public lastChangeProfitCycle;
    ...
    function setCycleConfiguration(CycleConfiguration memory config) external override onlyAdmin {
        ...
        lastChangeProfitCycle = uint32(block.timestamp);
    ...
    function collectFee(address token) public override onlyAdmin {
        ...
        emit CollectFee(admin, token, IERC20(token).balanceOf(address(this)));
    }
