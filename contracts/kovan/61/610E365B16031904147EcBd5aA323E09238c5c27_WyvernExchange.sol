...
contract WyvernExchange is Exchange {
     ...
     */
    constructor (ProxyRegistry registryAddress, TokenTransferProxy tokenTransferProxyAddress, ERC20 tokenAddress, address protocolFeeAddress, uint8 _chainID) public {
...
contract ExchangeCore is ReentrancyGuarded, Ownable {
    ...
    // constant by chainID -> after set circuitBreaker, this variable could be public and setter function.
    ...
    function setChainID(uint8 _chainID) 
        public
        onlyOwner
    {
        chainID = _chainID;
        ...
        returns (bytes32 hash)
    {
        ...
        index = ArrayUtils.unsafeWriteUint8Word(index, chainID);
        ...
        returns (uint)
    {
        ...
        // This is new version of feeManager in BMall
        IFeeManager feeManager = IFeeManager(protocolFeeRecipient);
            ...
            if (sell.feeMethod == FeeMethod.SplitFee) {
                ...
                if (sell.makerProtocolFee > 0) {
                    ...
                    if (sell.paymentToken == address(0)) {
                        ...
                        feeManager.communityFeeUpdate(sell.target, address(0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE), makerProtocolFee);
                    } else {
                        transferTokens(sell.paymentToken, sell.maker, protocolFeeRecipient, makerProtocolFee);
                        feeManager.communityFeeUpdate(sell.target, sell.paymentToken, makerProtocolFee);
                    }
                }
                ...
                if (sell.takerProtocolFee > 0) {
                    ...
                    if (sell.paymentToken == address(0)) {
                        ...
                        feeManager.communityFeeUpdate(sell.target, address(0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE), takerProtocolFee);
                    } else {
                        transferTokens(sell.paymentToken, buy.maker, protocolFeeRecipient, takerProtocolFee);
                        feeManager.communityFeeUpdate(sell.target, sell.paymentToken, takerProtocolFee);
                    }
                }
...
interface IFeeManager {
    ...
    function feeClaim(address nftAddr, uint256[] tokenID, address[]  tokenAddrs) external;
