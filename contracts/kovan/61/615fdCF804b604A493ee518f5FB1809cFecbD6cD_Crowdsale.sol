...
// TODO minAllowance, getStage = 3 if end
...
contract Crowdsale is AccessControl, ReentrancyGuard {
    ...
    /**
     * @return alowable - array with 2 numbers 
     * alowable[0] - min amount 0gas, that user can buy
     * alowable[1] - how many 0gas user can buy, till reach max purchase
     */
    function getAllowable(address user)
        external
        view
        returns (uint256[2] memory alowable)
    {
        uint256 bought = userToBalance[user][3];
        alowable[0] = bought >= minPurchase ? 1 : minPurchase - bought;
        alowable[1] = maxPurchase - bought;
    }
