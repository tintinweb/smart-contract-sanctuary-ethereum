...
contract VamosCasar {
    ...
    function Casar(string memory data) public {
        require (msg.sender == oNoivo.conta || msg.sender == aNoiva.conta,unicode"So os noivos podem casar.");
        require (bytes(data).length > 0 && bytes(data).length < 99999999, unicode"Inserir data:");
        ...
        function Votos() public view returns(string memory dizNoiva, string memory dizNoivo) {
