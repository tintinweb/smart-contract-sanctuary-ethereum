...
contract AKCCoreMultiStakeExtension is Ownable {
    ...
    function _getBonus(address staker, uint256 spec)
        ...
        returns (uint256) {
            ...
            uint256 pendingReward = akcCore.getAllRewardsByTimestampAndSpec(staker, lastTimeStamp, spec);
    ...
    function stake(address staker, uint256 spec, uint256 kong)
        external
        onlyManager {
            require(spec < akcCore.getTribeSpecAmount(), "Invalid spec");
            require(kongToStaker[kong] == 0, "Kong already staked");
            require(akcCore.getTotalTribesByspec(staker, spec) > 0, "User has no items in spec");
            require(akc.ownerOf(kong) == address(this), "Kong not in custody");

            kongToStaker[kong] = _getKongStakeData(staker, spec);

            uint256 stakeData = userToStakeData[staker][spec];
            uint256 currentAmount = stakeData == 0 ? 0 : _getStakeAmountFromStakeData(stakeData);            
            uint256 accumulatedBonus = stakeData == 0 ? 0 : _getStakePendingBonusFromStakeData(stakeData);
    ...
    function unstake(address staker, uint256 spec, uint256 kong)
        ...
        onlyManager {
            ...
            userToStakeData[staker][spec] = currentAmount - 1 == 0 ? 0 : _getUpdatedStakeData(block.timestamp, currentAmount - 1, accumulatedBonus + pendingBonus);
        }
