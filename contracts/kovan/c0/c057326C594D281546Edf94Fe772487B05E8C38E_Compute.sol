...
contract Compute {
    ...
    function add(address policy_, uint256 pay_) external {
        ...
        uint112 _policyPay = policyInfo.pay;
        uint112 _leverage = policyInfo.leverage;
        uint256 _typeID = policyInfo.tpyeID;

        _policyPay =  _policyPay + uint112(pay_);
        policyInfo.pay = _policyPay;
        uint120 _policyValue = _policyPay * _leverage;
        ...
        for (uint256 _pid = 0; _pid < _length; ++_pid) {
            ...
            uint120 _value = policyInfos[_policy].value;
            ...
            uint240 _beforeValue = correlation.value;
            uint16 _corr = correlation.corr;
            uint240 _afterValue = _corr * _value * _policyValue;
