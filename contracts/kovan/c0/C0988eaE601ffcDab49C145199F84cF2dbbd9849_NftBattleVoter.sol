...
contract NftBattleVoter is Ownable, ERC721
{
	...
	NftBattleStaker public battleStaker;
	...
	event VotingLiquidated(address indexed owner, address beneficiary, uint256 indexed votingPositionId, uint256 indexed stakingPositionId, uint256 zooReturned, uint256 daiReceived, uint256 currentEpoch);
	...
	uint256 public mintVoterPositionId;
	...
	constructor (
		...
		address _battleStaker
		) Ownable() ERC721("ZooVoter", "ZV")
	{
		...
		battleStaker = NftBattleStaker(_battleStaker);
	...
	{
		require(uint256(battleStaker.getCurrentStage()) == 1, "Wrong stage!");            // Requires to be at second stage of battle epoch.
		require(battleStaker.ownerOf(stakingPositionId) != address(0));                          // Requires for id to exist.

		(,,,,,uint256 endEpoch,,) = battleStaker.stakingPositionsValues(stakingPositionId);
		require(endEpoch == 0, "dai voting error");// Requires to be staker type and currently staked.

		uint256 currentEpoch = battleStaker.currentEpoch();

		battleStaker.updateInfo(stakingPositionId);
		...
		_safeMint(msg.sender, mintVoterPositionId);                                   // Mints Zoo battle nft-position for voter.

		votingPositions[mintVoterPositionId].stakingPositionId = stakingPositionId;   // Records staker position Id voted for.
		votingPositions[mintVoterPositionId].startDate = block.timestamp;
		votingPositions[mintVoterPositionId].daiInvested = amount;                    // Records amount of dai invested.
		votingPositions[mintVoterPositionId].yTokensNumber = yTokensNumber;           // Records amount of yTokens got from yearn vault.
		totalDaiInvested += amount;

		votingPositions[mintVoterPositionId].daiVotes = votes;                        // Records computed amount of votes to daiVotes.
		votingPositions[mintVoterPositionId].votes = votes;                           // Records computed amount of votes to total votes.
		votingPositions[mintVoterPositionId].startEpoch = currentEpoch;               // Records epoch when position created.
		votingPositions[mintVoterPositionId].lastRewardedEpoch = currentEpoch;        // Sets starting point for reward to current epoch.
		...
		battleStaker.votesToStakedNft(stakingPositionId, votes);  // Adds votes for staker position for this epoch.
		battleStaker.yTokensToStakedNft(stakingPositionId, yTokensNumber);// Adds yTokens for this staker position for this epoch.

		emit CreatedVotingPosition(msg.sender, stakingPositionId, amount, votes, mintVoterPositionId, currentEpoch);
	...
	{
		require(uint256(battleStaker.getCurrentStage()) == 3, "Wrong stage!");             // Requires to be at 4th stage.
		...
		uint256 currentEpoch = battleStaker.currentEpoch(); // todo: decide if we need it.
	...
	function liquidateVotingPosition(uint256 votingPositionId, address beneficiary) public
	{
		...
		(,,,,uint256 endDate,,,) = battleStaker.stakingPositionsValues(stakingPositionId);
		require(uint256(battleStaker.getCurrentStage()) == 0 || endDate != 0, "Wrong stage!"); // Requires correct stage or nft to be unstaked.
		...
		uint256 startEpoch = votingPositions[votingPositionId].startEpoch;
		uint256 lastEpoch = computeLastEpoch(votingPositionId);

		uint256 actualYtokens = battleStaker.subYtokensUntilLastEpoch(stakingPositionId, daiNumber, yTokens, startEpoch, lastEpoch);

		vault.withdraw(actualYtokens, beneficiary);
		...
		battleStaker.nftwithZeroVotesNegativeSwap(stakingPositionId, votes);
	...
	{
		require(uint256(battleStaker.getCurrentStage()) == 0, "Wrong stage!");            // Requires to be at first stage.
		require(ownerOf(votingPositionId) == msg.sender, "wrong position");// Requires to be owner of position.
		...
		uint256 totalYTokens = yTokens + yTokensIns + yTokensGas + yTokensTeam;
		battleStaker.subYtokensFromStakedNft(stakingPositionId, totalYTokens); // Subtracts yTokens for this position.
	...
	{
		uint256 stakingPositionId = votingPositions[votingPositionId].stakingPositionId;  // Gets staker position id from voter position.
		(,,,,,uint256 lastEpochOfStaking,,) = battleStaker.stakingPositionsValues(stakingPositionId);
		...
		{
			lastEpochNumber = battleStaker.currentEpoch();
		}
	}
	...
	uint256 lastEpochNumber)
	{
		...
		{
			(int256 saldo,,,,,) = battleStaker.rewardsForEpochFromStaker(stakingPositionId, i);// Gets saldo from staker position.
			(,uint256 _totalVotes,,,,) = (battleStaker.rewardsForEpochFromStaker(stakingPositionId, i));// Gets total votes from staker position.
			int256 totalVotes = int256(_totalVotes);

			if (saldo > 0)
			{ //todo: calculate out of loop?
				...
				yTokens += (saldo * 94 / 100) * votes / totalVotes;                              // Calculates yTokens amount for voter.(94%, and the last 2% goes for staker).
			}
		}
	...
	function recomputeDaiVotes(uint256 votingPositionId) external
	{
		...
		require(reward == 0, "reward not claimed");                             // Requires to claim reward before recompute.
		...
		uint256 deltaVotes = newVotes - votes;
		battleStaker.votesToStakedNft(stakingPositionId, deltaVotes);// Increases rewards for staker position for added amount of votes in this epoch.
	...
	function recomputeZooVotes(uint256 votingPositionId) external
	{
		...
		uint256 oldZooVotes = votingPositions[votingPositionId].votes - votingPositions[votingPositionId].daiVotes;
		require(newZooVotes > oldZooVotes, "recompute to lower value");             // Requires for new votes amount to be bigger than before.

		uint256 stakingPositionId = votingPositions[votingPositionId].stakingPositionId;
		battleStaker.updateInfo(stakingPositionId);

		uint256 deltaVotes = newZooVotes + votingPositions[votingPositionId].daiVotes - votingPositions[votingPositionId].votes; // Gets amount of recently added zoo votes.
		battleStaker.votesToStakedNft(stakingPositionId, deltaVotes);// Adds amount of recently added votes to reward for staker position for current epoch.
		votingPositions[votingPositionId].votes += deltaVotes;                           // Add amount of recently added votes to total votes in voting position.
...
/* todo: update.
	...
	function addDaiToPosition(uint256 votingPositionId, uint256 amount) external returns (uint256 votes)
	{
		...
		stakingPositionsValues[stakingPositionId].rewards[currentEpoch].votes += votes;          // Adds votes to staker position for current epoch.
		stakingPositionsValues[stakingPositionId].rewards[currentEpoch].yTokens += yTokensNumber;// Adds yTokens to rewards from staker position for current epoch.
	...
	function addZooToPosition(uint256 votingPositionId, uint256 amount) external returns (uint256 votes)
	{
		...
		uint256 stakingPositionId = votingPositions[votingPositionId].stakingPositionId;// Gets id of staker position.
		updateInfo(stakingPositionId);
		stakingPositionsValues[stakingPositionId].rewards[currentEpoch].votes += votes;   // Adds votes for staker position.
	...
	// functions todo:
	function withdrawDaiFromPosition(uint256 votingPositionId, uint256 amount) external returns (uint256)
	{
	}

	function withdrawZooFromPosition(uint256 votingPositionId, uint256 amount) external returns (uint256)
	{
	}
