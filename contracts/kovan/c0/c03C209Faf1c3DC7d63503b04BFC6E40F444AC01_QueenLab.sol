...
contract QueenLab is IQueenLab, QueenLabBase {
    ...
    function rarityLottery(
        ...
        if (_queeneId == 1) {
            ...
            if (_traitId == 1 || _traitId % 2 == 0)
                ...
                return
                    raritiesList[
                        (pseudoRandomNumber % 2) + (raritiesList.length - 2)
                    ].id;
        ...
        returns (RoyalLibrary.sQUEEN memory)
    {
        ...
        uint256 checkers;
        while (checkers < 3) {
            //if (checkRepeated(uint256(keccak256(abi.encode(_dna))), _queeneContract)) {
            if (
                queenPalace.QueenE().dnaMapped(
                    uint256(keccak256(abi.encode(_dna)))
                )
            ) {
        ...
        return
            RoyalLibrary.sQUEEN({
                queeneId: _queeneId,
                description: getQueenEDescriptionIdx(
                    _queeneId,
                    getQueenRarity(_dna)
                ),
                finalArt: "",
        ...
        returns (string memory rarityName)
    {
        ...
        if (rarityId == uint256(RoyalLibrary.queeneRarity.LEGENDARY))
            ...
            return "Super-Rare";
        else if (rarityId == uint256(RoyalLibrary.queeneRarity.RARE))
            return "Rare";
        else return "Common";
    }
    ...
    function getQueeneAttributes(
        ...
        for (uint256 idx = 0; idx < _blood.length; idx++) {
            if (idx > 0)
                attribute = string(
                    abi.encodePacked(attribute, ',{ "trait_type":"')
                );
            else
                attribute = string(
                    abi.encodePacked(attribute, '{ "trait_type":"')
            ...
            attribute = string(
                abi.encodePacked(
                    ...
                    queenPalace
                        .QueenTraits()
                        .getTrait(_blood[idx].traitId)
                        .traitName
                )
            );
            attribute = string(abi.encodePacked(attribute, '", "value": "'));
            attribute = string(
                abi.encodePacked(attribute, _blood[idx].artName)
            );
        ...
        if (_rarityId == RoyalLibrary.queeneRarity.COMMON) {
            attribute = string(
                abi.encodePacked(
                    attribute,
                    ', {"trait_type":"Rarity Level","value":1}'
                )
            );
            ...
            attribute = string(
                abi.encodePacked(
                    attribute,
                    ', {"trait_type":"Rarity Level","value":2}'
                )
            );
            ...
            attribute = string(
                abi.encodePacked(
                    attribute,
                    ', {"trait_type":"Rarity Level","value":3}'
                )
            );
        } else {
            attribute = string(
                abi.encodePacked(
                    attribute,
                    ', {"trait_type":"Rarity Level","value":4}'
                )
            );
        }
