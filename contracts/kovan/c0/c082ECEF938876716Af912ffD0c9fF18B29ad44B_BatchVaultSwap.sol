...
contract BatchVaultSwap {
  VaultSwap public vaultSwap;

  // solhint-disable-next-line func-visibility
  constructor(address _vaultSwapAddress) {
    vaultSwap = VaultSwap(payable(_vaultSwapAddress));
  }

  function batchSwapIntent(
    VaultSwap.SwapIntent memory _swapIntent,
    VaultSwap.SwapStruct[] memory _nftsOne,
    VaultSwap.SwapStruct[] memory _nftsTwo,
    ...
    uint256 i;
    for (i = 0; i < _times; i++) {
      vaultSwap.createSwapIntent(_swapIntent, _nftsOne, _nftsTwo, merkleProofOne, merkleProofTwo);
    }
  }
}
