...
contract FrankTreasury is Ownable {
    ...
    function _addPoolAndFarmLiquidity5(uint256 _amount, address _pool) public returns (uint256) {
        //harvest();
        ...
        uint256 amountOutOther = TraderJoeRouter.swapExactTokensForTokens(safeAmount, (TraderJoeRouter.getAmountsOut(safeAmount, path)[1]), path, address(this), block.timestamp + 2000)[1];
        ...
        if(amountInJoe + safeAmount < _amount) {
            emit Test(_amount - (amountInJoe + safeAmount));
            return _amount - (amountInJoe + safeAmount);
        }
        emit Test(0);
        return 0;
    ...
    function _addPoolAndFarmLiquidity4(uint256 _amount, address _pool) public returns (uint256) {
        //harvest();
        ...
        address tokenB = pair.token0() == address(JOE) ? pair.token1() : pair.token0();
        ...
        uint256 minAmountOut = TraderJoeRouter.getAmountsOut(safeAmount, path)[1];
        uint256 amountOut = TraderJoeRouter.swapExactTokensForTokens(safeAmount, minAmountOut, path, address(this), block.timestamp + 2000)[1];
        ...
        (, uint256 amountOutA, ) = TraderJoeRouter.addLiquidity(tokenB, tokenA, amountOut, x, 0, 0, address(this), block.timestamp + 1000);

        if(amountOutA + safeAmount < _amount) {
            return _amount - (amountOutA + safeAmount);
        }
