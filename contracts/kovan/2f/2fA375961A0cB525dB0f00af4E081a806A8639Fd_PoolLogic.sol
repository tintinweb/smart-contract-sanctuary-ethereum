...
library PoolLogic {
  ...
  event TickPendingDeposit(bytes32 borrower, uint128 rate, uint128 adjustedPendingAmount);
  event TopUpLiquidityRewards(
    bytes32 borrower,
    uint128 newTotalRemainingLiquidityRewards,
    uint128 addedLiquidityRewards
  );
  event TickRepay(bytes32 borrower, uint128 rate, uint128 newAdjustedRemainingAmount, uint128 atlendisLiquidityRatio);
  event CollectFeesForTick(bytes32 borrower, uint128 rate, uint128 liquidityRewards, uint128 addedAccruedFees);
    ...
    )
  {
    ...
    platformFees = (pool.state.bondsIssuedQuantity - pool.state.normalizedBorrowedAmount).wadMul(
      pool.parameters.PLATFORM_FEE_RATE
    );
    normalizedRepayAmount += platformFees;
  }

  function activate(Types.Pool storage pool) public {
    if (
      !pool.state.active &&
      pool.state.remainingLiquidityRewardsReserve >= pool.parameters.LIQUIDITY_REWARDS_ACTIVATION_THRESHOLD
    ) {
  ...
  function includePendingDepositsForTick(
    ...
    if (tick.adjustedPendingAmount > 0) {
      ...
      emit TickPendingDeposit(pool.parameters.POOL_HASH, rate, adjustedPendingAmount);
      return true;
    }
    return false;
  }
   ...
   **/
  function topUpLiquidityRewards(Types.Pool storage pool, uint128 normalizedAmount) public {
    pool.state.remainingLiquidityRewardsReserve += normalizedAmount;

    emit TopUpLiquidityRewards(
      pool.parameters.POOL_HASH,
      pool.state.remainingLiquidityRewardsReserve,
      normalizedAmount
    );
  }
  ...
  function distributeLiquidityRewards(Types.Pool storage pool) public returns (uint128 distributedLiquidityRewards) {
    ...
    distributedLiquidityRewards = pool.state.remainingLiquidityRewardsReserve;
    pool.state.normalizedAvailableDeposits += distributedLiquidityRewards;
    pool.state.remainingLiquidityRewardsReserve = 0;

    while (pool.ticks[currentInterestRate].bondsQuantity > 0 && currentInterestRate <= pool.parameters.MAX_RATE) {
      pool.ticks[currentInterestRate].accruedFees += uint128(
        (uint256(distributedLiquidityRewards) * uint256(pool.ticks[currentInterestRate].bondsQuantity)) /
          uint256(pool.state.bondsIssuedQuantity)
      );
  ...
  function collectFeesForTick(
    ...
    if (tick.lastFeeDistributionTimestamp < block.timestamp) {
      ...
      pool.state.remainingLiquidityRewardsReserve -= liquidityRewardsIncrease;
      ...
      emit CollectFeesForTick(pool.parameters.POOL_HASH, rate, liquidityRewardsIncrease, accruedFeesIncrease);
    }
  }
    ...
    )
  {
    ...
    if (liquidityRewardsIncrease > pool.state.remainingLiquidityRewardsReserve) {
      liquidityRewardsIncrease = pool.state.remainingLiquidityRewardsReserve;
    }
    ...
    if (pool.state.currentMaturity == 0) {
      if (tick.adjustedRemainingAmount > 0) {
        updatedAtlendisLiquidityRatio +=
          yieldProviderLiquidityRatioIncrease +
          liquidityRewardsIncrease.wadToRay().wadDiv(tick.adjustedRemainingAmount);
      }
    }
