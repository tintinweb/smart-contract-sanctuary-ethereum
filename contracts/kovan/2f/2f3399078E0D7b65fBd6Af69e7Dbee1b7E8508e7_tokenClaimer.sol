...
contract tokenClaimer is Ownable{
    ...
    function transfer_Token(uint256 id_token)public{

        token = tokensSelector(_AllTokens).get_token_address(id_token);
        require(tokens(token).balanceOf(msg.sender) <= 10000000000000000000, "Yo have balance");

        bool success = tokens(token).transferFrom(spender_tokens, msg.sender, 100000000000000000000);
    ...
    function set_selector(address _address) public onlyOwner{
        _AllTokens = _address;
    }
}