...
contract DRewards is Ownable, IDPairing {
   ...
   function createNewContest(uint256 contestID, uint256 rewardAmount) public onlyOwner {
      require(contestID > 0);
      require(rewardAmount > 0);
      contest[contestID].contestOwner = msg.sender;
      contest[contestID].contestID = contestID;
      contest[contestID].rewardAmount = rewardAmount;
   }
   
   function deliverCandicates(uint256 contestID, uint256[] memory candicatesIDs) public onlyOwner {
      require(contestID > 0);
      require(contest[contestID].rewardAmount > 0);
      require(candicatesIDs.length > 0);
      contest[contestID].candicatesIDs = candicatesIDs;
      emit candicatesDelivered(contest[contestID]);
   }
   
   function getRewardAmount(uint256 contestID) public view onlyValidContest(contestID) returns(uint256)
   {
   ...
   function triggerRewardDistrobution(uint256 contestID) 
      ...
      returns(uint256)
   {
      require(contest[contestID].rewardsDone == 0);
      require(contest[contestID].randomRequestDone == true);
      ...
      return contest[contestID].candicatesIDs[winnerIndex];
   }
   
   // VRF
   // Assumes the subscription is funded sufficiently.
   function requestRandomSeed(uint256 contestID) external onlyValidContest(contestID) onlyOwner {
      require(contest[contestID].rewardsDone == 0);
      require(contest[contestID].randomRequestDone == false);
      
      contest[contestID].randomSeed = uint256(keccak256(abi.encodePacked(block.timestamp,block.difficulty, msg.sender)));
      contest[contestID].randomRequestDone = true;
      /* VRF have problems.
   ...
   ) internal  
   {
      ...
      contest[contestRandomTable[requestID]].randomRequestDone = true;
   }
...
contract IDPairing is Ownable {
    ...
    event contestIDUpdated(address indexed _from, uint256 ID);
    
    function setContestID(address _address, uint256 ID)
        public
        onlyOwner
    {
        require(ID > 0);
        adressToContestID[_address] = ID;
        emit contestIDUpdated(_address, ID);
    }
