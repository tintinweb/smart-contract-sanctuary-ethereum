    ...
    Ownable
{
    ...
    function setScientistSalary(address _scientistSalary) external onlyOwner {
        require(_scientistSalary != address(0), "Address should not be empty");
        scientistSalary = _scientistSalary;
    }
    ...
    function _processSplit(CellData.Cell memory cell) private {
        if (CellData.Class(cell.class) == CellData.Class.SPLITTABLE_NANO) {
            ...
            IScientistSalary(scientistSalary).registerNanoCellForMetaCell(cell.tokenId, tokenId);
            ...
            emit SplitMADToken(msg.sender, amount, block.timestamp);
        } else if (
    ...
    function addNft(
        ...
        require(
            EnumerableSet.contains(nftWhitelist, _nftAddress),
    ...
    function createMetaCell(uint256 positionId) external override {
        ...
        IScientistSalary(scientistSalary).registerMetaCellForScientist(positionId, metaCellId);
    ...
    function addNFTWhitelist(address nft) external onlyOwner {
        require(
            !EnumerableSet.contains(nftWhitelist, nft),
            "This address is current whitelist"
        );
        EnumerableSet.add(nftWhitelist, nft);
    }

    function removeNFTWhitelist(address nft) external onlyOwner {
        require(
            EnumerableSet.contains(nftWhitelist, nft),
            "This address is not current whitelist"
        );
        EnumerableSet.remove(nftWhitelist, nft);
    }

    function getNFTWhitelist() external view returns (address[] memory) {
        return EnumerableSet.values(nftWhitelist);
    }
}
...
interface IScientistSalary {
    ...
    function genesisBasedSalary() external view returns (uint256);
...
interface IMetaCellCreator {
    function create(address to) external returns (uint tokenId);
}
...
interface ILaboratoryEvent {
    ...
    /**
     * @dev Emits when user evolve MetaCell and split MAD token
     * @param receiver is receiver address
     * @param amount is MAD token amount that split for user
     * @param timestamp is is the time that event emitted
     */
    event SplitMADToken(
        address indexed receiver,
...
abstract contract IExternalNftRepository {
    ...
    Counters.Counter public nftLatestId;
