    ...
    function licenseDiscount() private returns (uint256) {
        ...
        if (bal <= 0) {
            ...
            for (uint256 index = 0; index < bal; index++) {
                ...
                (uint256 maxCount, uint256 curCount, uint256 discount, uint256 expiry, ) = licenseEngine.licenses(id);
...
interface ILicenseEngine is IERC721, IERC721Enumerable {
    struct LicenseInfo {
        ...
        uint256 discount; // Discount applies to both collateral and lend tokens
        uint256 expiry; // Date when the license expires
        bool exists;
    }
