...
contract AdminVerifier is Initializable, IVerifier, OwnableUpgradeable, EIP712Upgradeable {
    ...
    function registerSelf(
        ...
        require(bytes(userData[msg.sender]).length == 0, 'RS1');
        require(block.timestamp < _timestamp + signValidity, 'RS2');
        ...
        require(hashAddressMap[digest] == address(0), 'RS3');

        bytes32 hash = _hashTypedDataV4(digest);
        address signer = ECDSA.recover(hash, _v, _r, _s);
        require(signer == signerAddress, 'RS5');
    ...
    function unregisterSelf() external {
        ...
        require(bytes(_userdata).length != 0, 'URS1');
    ...
    function unregisterUser(address _user) external onlyOwner {
        ...
        require(bytes(_userdata).length != 0, 'UU1');
    ...
    function _updateSignValidity(uint256 _signValidity) internal {
