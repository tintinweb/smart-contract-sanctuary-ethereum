...
contract DToken is ERC20 {
    ...
    **/
    ...
    constructor(
        ...
        vault = IDeepWatersVault(_vaultAddress);
    }
    ...
    **/
    function changeDeepWatersContracts(address _newLendingContractAddress, address payable _newVaultAddress) external onlyVault {
        ...
        vault = IDeepWatersVault(_newVaultAddress);
    }
    ...
    **/
    function mint(address _user, uint256 _amount) external onlyLendingContract {
        // mint an relative amount of dTokens
        _mint(_user, _amount * HUNDRED_PERCENT / vault.getAssetCumulatedLiquidityIndex(basicAsset));
    ...
    **/
    function burn(address _user, uint256 _amount) external onlyLendingContract {
        // burn an relative amount of dTokens
        _burn(_user, _amount * HUNDRED_PERCENT / vault.getAssetCumulatedLiquidityIndex(basicAsset));
    ...
    **/
    function balanceOf(address _user) override public view returns(uint256) {
        return relativeDTokenBalanceOf(_user) * 
            vault.getAssetCumulatedLiquidityIndex(basicAsset) /
            HUNDRED_PERCENT;
    }

    /**
    * @dev returns the relative dToken balance of the user.
    * @return the relative dToken balance of the user
    **/
    function relativeDTokenBalanceOf(address _user) internal view returns(uint256) {
