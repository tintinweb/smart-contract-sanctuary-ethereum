...
contract AKCCore is Ownable, AccessControl, IERC721Receiver {
    ...
    uint256 public akcStakeBoost = 5;
     ...
     * @dev EVENTS
     */
    event TribeCreated(address indexed owner, uint256 indexed tribeSpec, uint256 indexed ascii);
    ...
    event SetAddBoostsEvent(bool indexed shouldAdd);
    ...
    function createSingleTribe(address newOwner, uint256 spec, uint256 asciiData) 
        ...
        onlyRole(CREATOR_ROLE) {
            ...
            require(asciiData != 0, "IDENTIFIER CANNOT BE ZERO");
            ...
            emit TribeCreated(newOwner, spec, asciiData);
    }

    function createManyTribes(address[] calldata newOwners, uint256[] calldata specs, uint256[] calldata asciiDatas)
        external 
        onlyRole(CREATOR_ROLE) {
            require(newOwners.length == specs.length, "NEWOWNERS MUST MATCH SPEC");
            require(newOwners.length == asciiDatas.length, "NEWOWNERS MUST MATCH ASCII DATAS");
            ...
            for (uint i = 0; i < newOwners.length; i++) {
                ...
                uint256 asciiData = asciiDatas[i];
    ...
    function claimRewardOfTribeByIndex(address tribeOwner, uint256 tribeIndex) 
        ...
        returns(uint256) {
            ...
            newTribe |= spec << 64;
            newTribe |= getAsciiFromTribe(tribe) << 72;
    ...
    function stakeAKC(address staker, uint256 akcId, uint256 spec)
        external
        onlyRole(CLAIMER_ROLE) {
            require(spec < tribeSpecs.length, "SPEC OUT OF BOUNDS");
    ...
    function getAsciiFromTribe(uint256 tribe)
        public
        pure
        returns(uint256) {
            return uint256(uint184(tribe >> 72));
        }
    ...
    function getTribeStructFromTribe(uint256 tribe) 
        ...
        returns (Tribe memory _tribe) {
            ...
            _tribe.identifier = getAsciiFromTribe(tribe);
        }
    ...
    function setAddBoosts(bool shouldAdd)
        external 
        onlyRole(MODIFIER_ROLE) {
            addBoosts = shouldAdd;

            emit SetAddBoostsEvent(shouldAdd);
        }
