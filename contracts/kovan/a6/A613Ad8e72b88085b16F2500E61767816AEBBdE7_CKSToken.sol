...
// CKSToken with Governance.
contract CKSToken is ERC20Upgradeable, OwnableUpgradeable {
    uint constant public MAX_SUPPLY = 100_000_000 ether;
    // Burn address
    address public BURN_ADDRESS;
    address public CKSTokenStrategy;
    ...
    event SetCKSTokenStrategy(address _by, address _CKSTokenStrategy);
    ...
    modifier antiWhale(address sender, address recipient, uint amount) {
        if (maxTransferAmount() > 0) {
            if (
                ...
                require(amount <= maxTransferAmount(), "CKS::antiWhale: Transfer amount exceeds the maxTransferAmount");
            }
        }
        _;
    }
    ...
    constructor(address _CKSTokenStrategy) {
        CKSTokenStrategy = _CKSTokenStrategy;
    }
    ...
    function initialize() external virtual initializer {
        ...
        __ERC20_init("Crypto Kill Shark","CKS");
        ...
        _excludedFromTax[CKSTokenStrategy] = true;
        _excludedFromTax[address(this)] = true;
    }
    function setPercent(uint16 _percent2burn) external onlyOwner {
        require(_percent2burn < 50, "CKS::setPercent: Invalid percent.");
        burnRate = _percent2burn;
    }

    function setCKSTokenStrategy(address _CKSTokenStrategy) external onlyOperator {
        CKSTokenStrategy = _CKSTokenStrategy;
        emit SetCKSTokenStrategy(_msgSender(), _CKSTokenStrategy);
    }
    function taxBaseOnAmount(uint _transferAmount) public view returns(uint) {
        return _transferAmount * transferTaxRate / 10000;
    }
    /// @dev overrides transfer function to meet tokenomics of CKS
    function _transfer(address sender, address recipient, uint amount) internal virtual override antiWhale(sender, recipient, amount) {
        if (_excludedFromTax[sender] || recipient == BURN_ADDRESS || transferTaxRate == 0) {
            ...
            uint remaintaxAmount = taxAmount - burnAmount;
            uint sendAmount = amount - taxAmount;

            if(remaintaxAmount > 0) super._transfer(sender, CKSTokenStrategy, remaintaxAmount);
    ...
    function updateTransferTaxRate(uint16 _transferTaxRate) external onlyOperator {
        require(_transferTaxRate <= MAXIMUM_TRANSFER_TAX_RATE, "CKS::updateTransferTaxRate: Transfer tax rate must not exceed the maximum rate.");
    ...
    function updateMaxTransferAmountRate(uint16 _maxTransferAmountRate) external onlyOperator {
        require(_maxTransferAmountRate <= 10000, "CKS::updateMaxTransferAmountRate: Max transfer amount rate must not exceed the maximum rate.");
    ...
    function transferOperator(address newOperator) external onlyOperator {
        require(newOperator != address(0), "CKS::transferOperator: new operator is the zero address");
    ...
    */
    ...
    external
    {
        ...
        require(block.timestamp <= expiry, "CKS::delegateBySig: signature expired");
    ...
    internal
    {
        ...
        uint delegatorBalance = balanceOf(delegator); // balance of underlying CKS (not scaled);
    ...
    function inCaseTokensGetStuck(IERC20Upgradeable _token) external onlyOwner {
