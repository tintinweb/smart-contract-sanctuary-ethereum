...
contract MFLIXSale is Ownable {
  ...
  function presaleBuyToken (uint256 amount) public onlyWhitelisted{  
      ...
      require(presaleAmount >= tokenAmount * 10**uint(6),"Presale amount token is over."); 
      _totalSold += tokenAmount;
      _preTotalSold += tokenAmount;
      IERC20(mcubeAddress).transfer(msg.sender, tokenAmount * 10**uint(6));
    }
    ...
    function buyToken (uint256 amount) public onlyWhitelisted{  
      ...
      require(icosaleAmount >= tokenAmount * 10**uint(18),"Presale amount token is over."); 
      _totalSold += tokenAmount;
