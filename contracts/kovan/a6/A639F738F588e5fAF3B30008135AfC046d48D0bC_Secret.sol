...
/// @dev The contract is deployed on several networks: rinkeby, kovan, goerli
contract Secret {
    ...
    /// @notice The goal is to emit the Success event wuth your contacts
    function applyToMellow(string calldata contacts, bytes32 password1, bytes32 password2, bytes32 password3) external {
        uint256 res;
    ...
    function secret1(bytes32 password_) external {
        ...
        address f = factory;
        uint256 n = nonce;
        bytes memory data = abi.encodePacked("0xf14f0155", uint256(3000));
        bytes memory buffer = new bytes(32);
        assembly {
            secret := sload(0x0)
            secret := mulmod(secret, 200, 2)
            pop(staticcall(gas(), f, add(data, 0x20), 0x24, buffer, 0x20))
            let y := mload(buffer)
            mstore(add(buffer, 0x20), add(add(secret, y), n))
            secret := keccak256(buffer, 0x20)           
        }
