...
contract TPCore is AccessControl {
    ...
    function pending(address lpProvider) external view returns(uint104) {
        return userStaking[lpProvider].pendingAmount;
    }

    function staking(address lpProvider) external view returns(uint104) {
    ...
    function estimateTPYReward(address addr) public view returns(uint136) {
        ...
        uint intervalNumber = staking.intervalNumber;
        // if(staking.lastActionIsUnstake) {
        //     if(intervalNumber == currentInterval()) {
        //         return staking.calculatedTPYs;
        //     }
        //     else {
        //         intervalNumber++;
        ...
        if(staking.pendingAmount == 0){
            ...
            if(lastCompleteInterval >= intervalNumber) {
                ...
                if(stakedUntilInterval > 0 ) {
                    ...
                    estimatedTPYs += staking.amount * getTPYIntervalReward(intervalNumber) / stakedUntilInterval;

                    //console.log(staking.amount, getTPYIntervalReward(intervalNumber), stakedUntilInterval);
                    //console.log(estimatedTPYs);

                }
                for(uint256 i = intervalNumber + 1; i <= lastCompleteInterval; i++) {
            ...
            if(intervalNumber -1 <= lastCompleteInterval && !staking.lastActionIsUnstake) {
                //console.log("if 2");
                //stakedUntilInterval = totalStakedUntilInterval(intervalNumber - 1);

                //console.log(staking.amount, getTPYIntervalReward(intervalNumber - 1), stakedUntilInterval);

                estimatedTPYs += staking.amount * getTPYIntervalReward(intervalNumber - 1) / stakedUntilInterval;
                //console.log(estimatedTPYs);
            }
            ...
            for(uint256 i = intervalNumber; i <= lastCompleteInterval; i++) {
                ...
                //console.log("bazuk",(staking.amount + staking.pendingAmount) , getTPYIntervalReward(i) , stakedUntilInterval);
                //console.log("bazuk",estimatedTPYs);
            }
        }

        //console.log("final tpy", estimatedTPYs);
    ...
    function registerTPY(address staker, uint104 stakingCoinAmount) private {
        ...
        if(userStaking[staker].amount > 0) {
            ...
            userStaking[staker] = Staking(nextIntervalNumber, stakingCoinAmount, 0, 0, false);
        }
    ...
    function takeTPYReward(uint104 unstakeCoinAmount) private {
        ...
        if(staking.intervalNumber > currentIntervalNumber) {
            if(pending >= unstakeCoinAmount || pending == 0) {
                intervalUnstakes[currentIntervalNumber + 1] += unstakeCoinAmount;
            }
            else {
                intervalUnstakes[currentIntervalNumber + 1] += pending;
                intervalUnstakes[currentIntervalNumber] += unstakeCoinAmount - pending;
            }
        }
        else {
