...
contract FantasticChef is Ownable {
    ...
    modifier onlyOwnerOrController() {
        require(msg.sender == owner() || msg.sender == emissionController, "NOT_ALLOWED");
        _;
    }
    ...
    function setRewardPerSecond(uint256 _rewardPerSecond) public onlyOwnerOrController {
        require(
    ...
    function setEmissionController(address _controller) public onlyOwner {
        require(_controller != address(0), "FantasticChef::setEmissionController: INVALID ADDRESS");
        emissionController = _controller;
        emit EmissionControllerSet(_controller);
    }
    ...
    function setRewardMinter(address _rewardMinter) external onlyOwner {
        require(_rewardMinter != address(0), "FantasticChef::setRewardMinter: Invalid address");
        rewardMinter = IFantasticStaking(_rewardMinter);
    }
    ...
    event EmissionControllerSet(address indexed _controller);
}
