...
contract TokenFarm is Ownable {
    ...
    returns (uint256){
        ...
        // prie of the token * stakin balance
    ...
    function stakeToken(uint256 _amount, address _token) public {
        // Howm much token we want to stake??
        // WHAT TOKEN CAN STAKE?
        require(_amount > 0, "Amount must be greater than 0");
        // require if token is allowed
        require(tokenIsAllowed(_token), "Token is currently not allowed");
        IERC20(_token).transferFrom(msg.sender, address(this), _amount);
        updateUniqueTokenStake(msg.sender, _token);
        // mapping token adress to staker adress to the amount
    ...
    function updateUniqueTokenStake(address user, address _token) internal {
