...
contract Uchu {
    ...
    event Log(address from, string operation, uint256 amount);
    ...
    mapping (address => mapping(string => uint256)) public finalBalance;
    ...
    */
    ...
    function deposit() public payable returns (bool) {
        if (productId > 0) {
            ...
            if(getPoint().createPositionTime <= 1) {
    ...
    **/
    ...
    function singleBalance(address user) public view returns (uint256) {
        uint256 amount = balanceOf[user][curLenderPro.name];
        uint pow = caculateTimeGap(block.timestamp, getPoint().createPositionTime, 60 * 2);
        return amount / (100 ** pow) * ((curLenderPro.yeildNum + curLenderPro.totalNum) ** pow);
    }
