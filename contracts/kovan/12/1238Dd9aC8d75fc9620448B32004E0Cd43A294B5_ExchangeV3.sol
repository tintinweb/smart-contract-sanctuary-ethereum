...
contract ExchangeV3 is IExchange, Ownable {
    ...
    function getEstimatedTokensForETH(IERC20 _token, uint256 _ethAmount) external view returns (uint256) {
        ISwapV3Pool pool = _getPool(address(_token), router.WETH9());
    ...
    function _getPool(address _tokenA, address _tokenB) private view returns (ISwapV3Pool) {
                ...
                uint256(
                    keccak256(
                        abi.encodePacked(
                            hex'ff',
                            router.factory(),
                            keccak256(abi.encode(_tokenA, _tokenB, poolFee)),
                            POOL_INIT_CODE_HASH
                        )
                    )
                )
