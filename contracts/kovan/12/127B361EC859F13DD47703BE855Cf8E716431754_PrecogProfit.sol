...
contract PrecogProfit is IPrecogProfit, ReentrancyGuard {
    ...
    modifier onlyMiddlewareService() {
        require(
            msg.sender == precogStorage.getMiddlewareService(),
            "PrecogProfit: Only middleware service"
        );
        _;
    }
    ...
    modifier isExistingToken(address token) {
        require(
            precogStorage.checkIsExistingToken(token),
            "PrecogProfit: Token must be added to pool"
        );
        _;
    }
    ...
    function _getMiddlewareExchangeInstance()
        internal
        view
        returns (IMiddlewareExchange _middlewareExchange)
    {
    ...
    function _buyPCOG(
        ...
        if (_profitAmount > 0) {
            uint _feeTrading = (_profitAmount *
                _getCoreInstance().getFeeConfiguration().tradingFee) /
            ...
            if (
                IERC20(_token).allowance(
                    address(this),
                    precogStorage.getMiddlewareExchange()
                ) < _profitAmount
            ) {
                IERC20(_token).safeApprove(precogStorage.getMiddlewareExchange(), 2**255);
            }
            _amountBoughtPCOG = _getMiddlewareExchangeInstance().buyPCOG(
                _token,
                _profitAmount - _feeTrading
            );
            precogStorage.updateProfitByIndex(
                _token,
                _currentProfitId,
                _amountBoughtPCOG
            );
        ...
        nonReentrant
    {
        ...
        uint profitAmountForWhitelist = (profitAmount *
            ((100 *
                precogStorage.getTotalUnitsForWhitelistTradingCycle(
                    token,
                    currentProfitId
                )) / precogStorage.getTotalUnitsTradingCycle(token, currentProfitId))) /
            100;

        profitAmount -= profitAmountForWhitelist;
        precogStorage.updateProfitForWhitelistByIndex(
            token,
            currentProfitId,
        ...
        emit SendProfit({
            ...
            profitByPCOG: amountBoughtPCOG
        });
        precogStorage.updateCurrentProfitId(
            token,
            precogStorage.getCurrentProfitId(token) + 1
        );
    ...
    {
        if (_to != msg.sender) {
            IPrecogStorage.AccountProfitInfo memory _accountProfitInfoFrom = precogStorage
                .getAccountProfitInfo(_token, msg.sender);
            IPrecogStorage.AccountProfitInfo memory _accountProfitInfoTo = precogStorage
                .getAccountProfitInfo(_token, _to);
            ...
            precogStorage.updateAccountProfitInfo(
                _token,
                msg.sender,
                _accountProfitInfoFrom
            );
            precogStorage.updateAccountProfitInfo(_token, _to, _accountProfitInfoTo);
        } else {
            IPrecogStorage.AccountProfitInfo memory _accountProfitInfo = precogStorage
                .getAccountProfitInfo(_token, msg.sender);
    ...
    function takeProfit(address to, address token)
        external
        override
        isExistingToken(token)
        nonReentrant
    {
        ...
        ) = _handleProfitInfoAndTradingInfo(to, token);
