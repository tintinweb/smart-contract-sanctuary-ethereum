...
contract ERC721Enumerable is Context, ERC165, ERC721Metadata, IERC721Enumerable {
...
contract shareOracle {
    ...
    constructor(address _priceSource, address _underlying, address _shares) {
	...
	function latestRoundData() public view
		...
		return(_roundId, int256(newPrice), _startedAt, _updatedAt, _answeredInRound);
	}
...
contract erc20Stablecoin is ReentrancyGuard, VaultNFTv3 {
    ...
    modifier notBlackList(address owner) {
        require(!blackList[owner] && !blackList[msg.sender], "not a black list");
        _;
    }

    function getDebtCeiling() public view returns (uint256){
        return usdp.balanceOf(address(this));
    }
    ...
    function calculateCollateralProperties(uint256 _collateral, uint256 debt) private view returns (uint256, uint256) {
    ...
    function createVault() external returns (uint256) {
        ...
        require(freeze[msg.sender][uint8(FreezeType.VAULT)] <= block.timestamp, "freeze time is not ended");
    ...
    function depositCollateral(uint256 vaultID, uint256 amount) external notBlackList(ownerOf(vaultID)) {
        ...
        require(freeze[owner][uint8(FreezeType.COLLATERAL)] <= block.timestamp, "freeze time is not ended");
    ...
    function withdrawCollateral(uint256 vaultID, uint256 amount) external onlyVaultOwner(vaultID) notBlackList(ownerOf(vaultID)) nonReentrant {
    ...
    function borrowToken(uint256 vaultID, uint256 amount) external onlyVaultOwner(vaultID) notBlackList(ownerOf(vaultID)) {
        require(amount > 0, "Must borrow non-zero amount");
        require(amount >= minDebtCeiling, "amount greater than min debt ceiling");
        ...
        require(newDebt <= maxVaultCeiling, "newDebt less max vault ceiling");
        require(isValidCollateral(vaultCollateral[vaultID], newDebt), "Borrow would put vault below minimum collateral percentage");
        require(freeze[owner][uint8(FreezeType.DEBT)] <= block.timestamp, "freeze time is not ended");
    ...
    function payBackToken(uint256 vaultID, uint256 amount) external notBlackList(ownerOf(vaultID)) {
        require(usdp.balanceOf(msg.sender) >= amount, "Token balance too low");
        require(vaultDebt[vaultID] >= amount, "Vault debt less than amount to pay back");
        require(freeze[ownerOf(vaultID)][uint8(FreezeType.REPAY)] <= block.timestamp, "freeze time is not ended");
    ...
    function liquidateVault(uint256 vaultID) external {
        ...
        require(usdp.balanceOf(msg.sender) >= halfDebt, "Token balance too low to pay off outstanding debt");

        usdp.safeTransferFrom(msg.sender, address(this), halfDebt);
...
contract erc20TokenStablecoin is erc20Stablecoin, Ownable {
    ...
    function setBlackList(address account, bool status) external onlyOwner() {
        require(status != blackList[account], "status has already changed");
    ...
    function setMinDebtCeiling(uint minimumDebtCeiling) external onlyOwner() {
        require(minimumDebtCeiling < maxDebtCeiling, "minimumDebtCeiling less than maxDebtCeiling");
        minDebtCeiling = minimumDebtCeiling;
    }

    function setMaxDebtCeiling(uint maximumDebtCeiling) external onlyOwner() {
        require(maximumDebtCeiling > minDebtCeiling, "maximumDebtCeiling greater than minDebtCeiling");
        maxDebtCeiling = maximumDebtCeiling;
    }

    function setMaxVaultCeiling(uint maximumVaultCeiling) external onlyOwner() {
        require(maximumVaultCeiling <= 1e6, "maximumVaultCeiling less than 1000000");
    ...
    function transferToken(uint256 amountToken) public onlyOwner() {
        // Transfer reserve tokens back to main usdp contract
        usdp.transfer(address(usdp), amountToken);
    }
