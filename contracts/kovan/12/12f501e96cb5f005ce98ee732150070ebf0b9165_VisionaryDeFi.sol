...
contract VisionaryDeFi is Context, IERC20, Ownable {
    ...
    address payable public devAddress = payable(0xa96B9779c87e5f6a7dCa9C2e507bD8a3d4f3359D);
    ...
    function totalFees() internal view returns (uint256) {
        return _taxFee.add(_liquidityFee).add(_burnFee).add(_marketingFee).add(_foundationFee).add(_devFee);
    }
    ...
    function _getValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256, uint256, uint256, uint256) {
        ...
        {
            (uint256 tTransferAmount, uint256 tFee, uint256 tDev, uint256 tBurn, uint256 tLiquidity) = _getTValues(tAmount);
            tArray[0] = tTransferAmount;
            tArray[1] = tFee;
            tArray[2] = tDev;
            tArray[3] = tBurn;
            tArray[4] = tLiquidity;
        }
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee) = _getRValues(tAmount, tArray[1], tArray[3], tArray[2], tArray[4], _getRate());
        return (rAmount, rTransferAmount, rFee, tArray[0], tArray[1], tArray[3], tArray[2], tArray[4]);
    }

    function _getTValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256) {
        uint256[] memory arr;
        arr[0] = calculateTaxFee(tAmount); // tFee
        arr[1] = calculateBurnFee(tAmount); // tBurn
        arr[2] = calculateDevFee(tAmount); // tDev
        arr[3] = calculateLiquidityFee(tAmount); // tLiquidity
        arr[4] = tAmount.sub(arr[0]).sub(arr[1]).sub(arr[2]).sub(arr[3]); // tTransferAmount
        return (arr[4], arr[0], arr[1], arr[2], arr[3]);
    }

    function _getRValues(uint256 tAmount, uint256 tFee, uint256 tBurn, uint256 tDev, uint256 tLiquidity, uint256 currentRate) private pure returns (uint256, uint256, uint256) {
        uint256 rAmount = tAmount.mul(currentRate);
        uint256 rFee = tFee.mul(currentRate);
        uint256 rBurn = tBurn.mul(currentRate);
        uint256 rDev = tDev.mul(currentRate);
        uint256 rLiquidity = tLiquidity.mul(currentRate);
        uint256 totalTax = rFee.add(rBurn).add(rDev).add(rLiquidity);
    ...
    function _takeDev(address sender, uint256 tDev) private returns(uint256){
      ...
      _rOwned[sender] = _rOwned[sender].sub(rDev);
      emit Transfer(sender, address(this), rDev);
         if(_isExcludedFromReward[address(this)])
    ...
    function removeAllFee() private {
        if(_taxFee == 0 && _burnFee == 0 &&_devFee == 0 && _liquidityFee == 0) return;
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) private {
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee, uint256 tTransferAmount, uint256 tFee, uint256 tDev, uint256 tBurn, uint256 tLiquidity) = _getValues(tAmount);
        _rOwned[sender] = _rOwned[sender].sub(rAmount);
    ...
    function _transferToExcluded(address sender, address recipient, uint256 tAmount) private {
        (uint256 rAmount, uint256 rTransferAmount, uint256 rFee, uint256 tTransferAmount, uint256 tFee, uint256 tBurn, uint256 tDev, uint256 tLiquidity) = _getValues(tAmount);
        _rOwned[sender] = _rOwned[sender].sub(rAmount);
