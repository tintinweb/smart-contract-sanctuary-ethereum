...
contract FanLink is ERC1155, Ownable, ERC1155Supply {
    mapping(string => uint256) public idMapping; //"id" needs to be uint256 so i need a mapping of artist ID to ERC1155 id.
    ...
    function balanceOfID(address account, string memory id)
        public
        view
        returns (uint256)
    {
        ...
        if (idMapping[id] <= 0) {
            return 0;
        } else return this.balanceOf(account, idMapping[id]);
    }
    ...
    function mintBatch(string[] memory ids) public {
        ...
        for (uint256 index = 0; index < ids.length; index++) {
            if (idMapping[ids[index]] <= 0) {
                //check new artist to push
                idMapping[ids[index]] = artistCount + 1;
                artistCount++;
            }
            internalIds[index] = idMapping[ids[index]]; //translate external id to internal id
            amounts[index] = 1;
        }
