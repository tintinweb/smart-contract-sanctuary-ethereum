    ...
    **/
    ...
    function updateProduct(uint8 productId, uint deposit, uint withdraw, uint borrow, uint repay) private {
        if (product_list[productId].debt == 0) {
            ...
            if (deposit != 0) {
                product_list[productId].debt = setProductDebt(productId, 0, 0);
            } else if (withdraw != 0) {
    ...
    function updatePosition(address user, uint8 productId, uint deposit, uint withdraw) private {
        ...
        if (size == 0) {
            lenderShare(user, productId, 0, deposit, withdraw);
        } else {
            for (uint i=0; i<size; i++) {
                //in the array
                if (position_map[user][i].product_id == productId) {
                    lenderShare(user, productId, i, deposit, withdraw);
                //not in the array
                } else {
                    lenderShare(user, productId, size, deposit, withdraw);
                }
            }
        }
    }

    function lenderShare(address user, uint8 productId, uint index, uint deposit, uint withdraw) private {
        Product memory product = product_list[productId];
        if (product.debt == 0) {
            position_map[user][index].share += deposit;
        } else {
            position_map[user][index].share += (deposit - withdraw) / lenderPrice();
        }
    }
    ...
    function updateLoan(address user, uint8 productId, uint borrow, uint repay) private {
        ...
        if (size == 0) {
            borrowerShare(user, productId, 0, borrow, repay);
        } else {
            for (uint i=0; i<size; i++) {
                //in the array
                if (position_map[user][i].product_id == productId) {
                    borrowerShare(user, productId, i, borrow, repay);
                //not in the array
                } else {
                    borrowerShare(user, productId, size, borrow, repay);
                }
            }
        }
    }

    function borrowerShare(address user, uint8 productId, uint index, uint borrow, uint repay) private {
        Product memory product = product_list[productId];
        if (product.debt == 0) {
            loan_map[user][index].share += borrow;
        } else {
            loan_map[user][index].share += (borrow - repay) / borrowerPrice();
        }
    }

    /**
    * return earning in debt.
    **/
    function lenderEarning(uint8 productId, uint borrow, uint repay) private returns (uint) {
        Product memory product = product_list[productId];
