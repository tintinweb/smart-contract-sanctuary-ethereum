...
contract WithdrawalPool is IWithdrawalPool, Ownable, CheckContract {
    ...
    function withdrawUSDC(uint _id) external override {
        _requireCallerIsStakingPool();

        (uint USDCAmount,,,uint USDCFee,uint withdrawalTime) = stakingPool.getNFTVariables(_id);
        
        require(withdrawalTime <= block.timestamp, "WithdrawalPool: Withdrawal can only be made after the mentioned time");
        require(usdc_token.balanceOf(address(this)) >= USDCAmount, "WithdrawalPool: Insufficient USDC in WithdrawalPool");

        _transferUSDC(USDCAmount - USDCFee, tx.origin);
        _transferUSDC(USDCFee, treasuryAddress);
        
    }

    function _transferUSDC(uint _amount, address _to) internal {
        usdc_token.transfer(_to, _amount);
    }

    function _requireCallerIsStakingPool() internal {
        require(msg.sender == address(stakingPool), "WithdrawalPool: Caller is not stakingPool");
    }
