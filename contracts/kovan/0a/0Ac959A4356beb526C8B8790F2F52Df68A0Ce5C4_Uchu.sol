...
contract Uchu {
    ...
    **/
    function getProductReturn(address user, uint8 proId) public returns (uint256) {
        uint8 length = getCurLender(user).poIndex;
        Product memory pro = getProduct(proId);
        uint256 result;
        emit Log(user, "getProductReturn", "length", length);
        ...
        emit Log(user, "getProductReturn", "result", result);
        return result;
    }
