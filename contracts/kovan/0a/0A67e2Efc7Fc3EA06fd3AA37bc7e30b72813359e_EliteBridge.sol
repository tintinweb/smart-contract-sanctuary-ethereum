...
abstract contract Context {
    ...
    function _msgData() internal view virtual returns (bytes memory) {
        this; // silence state mutability warning without generating bytecode - see https://github.com/ERCereum/solidity/issues/2691
        return msg.data;
    }
}
...
library ECDSA {
     ...
     * be too long), and then calling {toERCSignedMessageHash} on it.
     */
    function recover(bytes32 hash, bytes memory signature) internal pure returns (address) {
        ...
        // unique. Appendix F in the ERCereum Yellow paper (https://ERCereum.github.io/yellowpaper/paper.pdf), defines
    ...
    /**
     * @dev Returns an ERCereum Signed Message, created from a `hash`. This
     * replicates the behavior of the
     * https://github.com/ERCereum/wiki/wiki/JSON-RPC#ERC_sign[`ERC_sign`]
     * JSON-RPC mERCod.
     *
     * See {recover}.
     */
    function toERCSignedMessageHash(bytes32 hash) internal pure returns (bytes32) {
        // 32 is the length in bytes of hash,
        // enforced by the type signature above
        return keccak256(abi.encodePacked("\x19ERCereum Signed Message:\n32", hash));
    }
}
...
contract EliteBridge is Ownable {
    ...
    */
    ...
    /**
    * @dev ERCereum Signed Message, created from `hash`
    ...
    */
    ...
    {
        bytes32 hash = ECDSA.toERCSignedMessageHash(_messageHash);
