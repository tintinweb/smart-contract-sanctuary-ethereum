...
contract FairXYZDeployer is ERC721xyz, Pausable{
    ...
    function view_signer() public view returns(address){
        address return_signer = IFairXYZWallets(interface_address).view_signer(); 
        return(return_signer);
    }

    // Fair.XYZ wallet address
    function view_withdraw() public view returns(address){
        address return_withdraw = IFairXYZWallets(interface_address).view_withdraw(); 
        return(return_withdraw);
    }

    // Initialize Creator contract
    function initialize(address _owner, uint256 max_, uint256 price_, string memory name_, string memory symbol_,
                        bool burn_, uint256 mints, address int_add) external {
        ...
        _symbol = symbol_;
        burnable = burn_; 
        Max_mints_per_wallet = mints;
        interface_address = int_add;
        isInitialized = true;
    }
    ...
    // Modify sale price
    function change_NFT_price(uint256 new_price) public onlyOwner returns(uint256)
    {
        NFT_price = new_price;
        return(NFT_price);
    }
    ...
    function remainingMints(address minterAddress) public view returns(uint256) {
        ...
        uint256 mints_left = Max_mints_per_wallet - mintsPerWallet[minterAddress];
        ...
        onlyOwner
    {
        ...
        payable(IFairXYZWallets(interface_address).view_withdraw()).transfer(bal_*3/50);
        payable(msg.sender).transfer(bal_*47/50);
    }
