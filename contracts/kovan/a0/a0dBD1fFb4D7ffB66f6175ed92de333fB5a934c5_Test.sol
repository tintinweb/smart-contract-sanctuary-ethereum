...
library MCR {
    function compute(uint256[] storage policy, uint256[] storage leverage, uint256[] storage corr) internal view returns (uint256 square) {
        square = 0;
        uint256 length = policy.length;
        if (length == 0) {
            return square;
        }
        require(length == leverage.length, "!rf.length");
        ...
        for (uint256 id1 = 0; id1 < length - 1; ++id1) {
            for (uint256 id2 = id1 + 1 ; id2 < length; ++id2) {
                square += 2 * leverPolicy[id1] * leverPolicy[id2] * corr[++id3]  / 1000000;
            }
        }
    }
}
...
contract Test {
    ...
    function Square() public {
        square = MCR.compute(policy, leverage, corr);
    }
