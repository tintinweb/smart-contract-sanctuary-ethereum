...
interface IFactory {
    ...
    function initialize(ICollateral collateral_, IIncentivizer incentivizer_, IBeacon productBeacon_, address treasury_, address pauser_) external;
...
contract PerennialLens {
    ...
    function userCollateral(address account, IProduct product)
    external settleAccount(account, product) returns (UFixed18) {
        return collateral.collateral(account, product);
    }

    function productCollateral(IProduct product) external settle(product) returns (UFixed18) {
        return collateral.collateral(product);
    }

    function productShortfall(IProduct product) external settle(product) returns (UFixed18) {
        return collateral.shortfall(product);
    }

    function maintenance(address account, IProduct product)
    external settleAccount(account, product) returns (UFixed18) {
        return product.maintenance(account);
    }
    ...
    function latestVersion(IProduct product) internal view returns (IOracle.OracleVersion memory) {
