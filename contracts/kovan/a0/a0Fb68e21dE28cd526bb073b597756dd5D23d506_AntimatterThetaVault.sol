...
contract AntimatterThetaVault is RibbonVault, RibbonThetaVaultStorage {
     ...
     */
    function withdrawInstantly(uint256 amount) external nonReentrant checkTime {
     ...
     */
    function completeWithdraw() external nonReentrant checkTime {
    ...
    function getOtokenMintInfo() public view returns (
        ...
        if (vaultParams.isPut && optionState.nextOption == address(0)) {
            return (0, 0, 0);
        }
        ...
        if (vaultParams.isPut) {
            IOtoken oToken = IOtoken(optionState.nextOption);
            uint256 collateralDecimals = uint256(IERC20Detailed(oToken.collateralAsset()).decimals());
            ...
            uint256 collateralDecimals = uint256(IERC20Detailed(vaultParams.asset).decimals());
    ...
    ERC20Upgradeable
{
    ...
    modifier checkTime() {
        // Disallow action when time from UTC 8:00 to 12:00 per Friday
        if (getDayOfWeek(block.timestamp) == 5) { // Friday
            uint256 hour = getHour(block.timestamp);
            require(hour >= 8 && hour < 12, "disallow action");
        }
        _;
    }
    ...
    function getDayOfWeek(uint timestamp) internal pure returns (uint dayOfWeek) {
        uint _days = timestamp / 86400;
        dayOfWeek = (_days + 3) % 7 + 1;
    }

    function getHour(uint timestamp) internal pure returns (uint hour) {
        uint secs = timestamp % 86400;
        hour = secs / 3600;
    }
     ...
     */
    function depositETH() external payable nonReentrant checkTime {
     ...
     */
    function deposit(uint256 amount) external nonReentrant checkTime {
        require(amount > 0, "!amount");
     ...
     */
    function initiateWithdraw(uint256 numShares) external nonReentrant checkTime {
