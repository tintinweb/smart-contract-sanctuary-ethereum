...
contract DataContract is AccessControlEnumerable, DataInterface {
    ...
    uint private _tokenConversion = 10;     // TO-DO: TokenConversion should be setup
    ...
    function manuallyCreditParticipant(address participant_, uint tokenAmount_) public onlyRole(MODERATOR_ROLE) override {
        require(_isParticipantWhitelisted[participant_], "Participant is not whitelisted!");
        _participantContributions[participant_] += tokenAmount_;
        _combinedContributions += tokenAmount_;
        emit ContributionMade(msg.sender, participant_, tokenAmount_, tokenAmount_ * _tokenConversion);
    }
    ...
    function creditParticipation(address participant_, uint tokenAmount_) public onlyRole(SALECONTRACT_ROLE) override {
        _participantContributions[participant_] += tokenAmount_;
        _combinedContributions += tokenAmount_;
        emit ContributionMade(msg.sender, participant_, tokenAmount_, tokenAmount_ * _tokenConversion);
    }
...
interface DataInterface is IAccessControlEnumerable{
    ...
    function manuallyCreditParticipant(address participant_, uint tokenAmount_) external;
    ...
    function creditParticipation(address participant_, uint tokenAmount_) external;
    ...
    // ************* EVENTS REGION **************
    // ******************************************
    event ContributionMade(address indexed contractAddress, address indexed participant_, uint baseAmount_, uint tokensIssued_);
