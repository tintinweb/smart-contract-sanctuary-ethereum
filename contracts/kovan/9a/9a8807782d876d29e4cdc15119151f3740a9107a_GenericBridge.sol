    ...
    ChainIdHolding
{
    ...
    /*
     * cross-chain liquidity token assets
     */
    mapping(uint256 => mapping(address => address)) public crossChainMapping;
    uint256 public crossChainIndex;
    mapping(address => TokenInfo) public crossChainMappingReverse;
    ...
    function _mintTokenOrTransferLiquidity(
        uint256 _amount,
        address _originToken,
        uint256[] memory _chainIdsIndex,
        bytes32 _txHash,
        ...
        address _crossTokenOnThisChain = crossChainMapping[_chainIdsIndex[0]][_originToken];
        if (_crossTokenOnThisChain == address(0)) {
            //simply mint bridge token
            _mintTokenForUser(_amount, _originToken, _chainIdsIndex, _txHash, _toAddr);
        } else {
            uint256 availableLiquid = IERC20Upgradeable(_crossTokenOnThisChain).balanceOf(address(this));
            if (_amount <= availableLiquid) {
                safeTransferOut(_crossTokenOnThisChain, _toAddr, _amount, 0);
            } else {
                //transfer the available liquid and mint bridge token for the rest
                safeTransferOut(_crossTokenOnThisChain, _toAddr, availableLiquid, 0);
                _mintTokenForUser(_amount - availableLiquid, _originToken, _chainIdsIndex, _txHash, _toAddr);
            }
        }
    }
    ...
    function addLiquidity(
        ...
        require(
            crossChainMapping[_originChainId][_originToken] ==
                _tokenOnThisChain,
            "unsupported liquidity token on this chain"
        );
        safeTransferIn(_tokenOnThisChain, msg.sender, _amount);
        //minting wrap token with amount _amount
        uint256[] memory _chainIdsIndex = new uint256[](4);
        _chainIdsIndex[0] = _originChainId;
        _chainIdsIndex[1] = _originChainId;
        _chainIdsIndex[2] = chainId;
        _chainIdsIndex[3] = crossChainIndex;
        crossChainIndex++;
        bytes32 h = keccak256(
            abi.encode(
                _originToken,
        ...
        _mintTokenForUser(_amount, _originToken, _chainIdsIndex, h, _recipient);
        emit AddLiquidity(
            _recipient,
            _originToken,
            _originChainId,
    ...
    function removeLiquidity(
        ...
        require(
            crossChainMapping[_originChainId][_originToken] ==
                _tokenOnThisChain,
            "invalid liquidity token"
        );
        ERC20BurnableUpgradeable(tokenMap[_originChainId][_originToken])
            .burnFrom(msg.sender, _amount);
        safeTransferOut(_tokenOnThisChain, _recipient, _amount, 0);
    ...
    //this function must be approved by validators
    function setCrossChainMapping(
        address _originToken,
        uint256 _originChainId,
        address _tokenOnThisChain
        // uint256 _signatureValidTil,
        // bytes32[] memory r,
        // bytes32[] memory s,
        // uint8[] memory v
    )
        ...
        onlyGovernance
    {
        ...
        //     )
        // );
        // require(verifySignatures(r, s, v, _msg), "invalid signatures");

        address bridgeToken = tokenMap[_originChainId][_originToken];
        require(isBridgeToken(bridgeToken), "invalid origin token");
        //verify decimals
        if (_tokenOnThisChain == NATIVE_TOKEN_ADDRESS) {
            require(
                IDecimals(bridgeToken).decimals() == 18,
                "invalid decimals"
            );
        } else {
            require(
                IDecimals(bridgeToken).decimals() ==
                    IDecimals(_tokenOnThisChain).decimals(),
                "invalid decimals"
            );
        }
        crossChainMapping[_originChainId][_originToken] = _tokenOnThisChain;
        crossChainMappingReverse[_tokenOnThisChain] = TokenInfo({
            addr: _originToken,
