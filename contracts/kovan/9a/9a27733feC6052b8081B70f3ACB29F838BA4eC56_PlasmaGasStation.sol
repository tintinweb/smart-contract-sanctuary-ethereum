...
import "./utils/TrustedFeePayer.sol";
import "./utils/LondonTxSupport.sol";
// import "./utils/LegacyTxSupport.sol";

contract PlasmaGasStation is Ownable, LondonTxSupport, TrustedFeePayer {
    ...
    // The maximum available amount of gas, if you need to call the permit method
    uint256 private txMaxPermitGasUsage = 60000;
    // The gas used before performing the user transaction, excluding permit method call
    uint256 private txPreCallGasUsage = 150000;
    // The gas used after the user's transaction is completed
    uint256 private txPostCallGasUsage = 120000;

    constructor(address _router, address _feePayer) {
        router = IUniswapV2Router02(_router);
        _addTrustedFeePayer(_feePayer);
    ...
    function removeTrustedFeePayer(address _feePayer) external onlyOwner {
        _removeTrustedFeePayer(_feePayer);
    }

    function setGasLimits(uint256 _txMaxPermitGasUsage, uint256 _txPreCallGasUsage, uint256 _txPostCallGasUsage) external onlyOwner {
        txMaxPermitGasUsage = _txMaxPermitGasUsage;
        txPreCallGasUsage = _txPreCallGasUsage;
        txPostCallGasUsage = _txPostCallGasUsage;
    }

    function getGasLimits() external view returns (uint256, uint256, uint256) {
        return (txMaxPermitGasUsage, txPreCallGasUsage, txPostCallGasUsage);
    }
    ...
    function sendTransaction(TxRequest calldata _tx, TxFee calldata _fee, bytes calldata _sign) external onlyFeePayer {
        ...
        uint256 gasUsed = initialGas.sub(gasleft()).add(txPostCallGasUsage);
        uint256 feeInEth = _calculateCharge(gasUsed, _tx);
        ...
        emit TransactionExecuted(_tx.from, _tx.to, address(token), spentTokens);

        // This ensures that transfer has not caused gas overruns.
        require(gasUsed > gasleft(), "Gas overspending");
    }
...
abstract contract TrustedFeePayer {
    // Trusted fee payers address
    ...
    function _addTrustedFeePayer(address _trustedFeePayer) internal {
        feePayers[_trustedFeePayer] = true;
    }

    function _removeTrustedFeePayer(address _trustedFeePayer) internal {
        feePayers[_trustedFeePayer] = false;
    }
}
