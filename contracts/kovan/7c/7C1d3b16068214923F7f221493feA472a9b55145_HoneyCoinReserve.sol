...
contract HoneyCoinReserve is SimpleAccessUpgradable {
    IERC20 public honeyCoin;
    ...
    function initialize(
        ...
        honeyCoin = IERC20(_honey);

        marketingWallet = _marketing;
        MAX_INT = ~uint256(0);
        honeyCoin.approve(address(router), MAX_INT);
    }
    ...
    function sendToMarketing(uint256 amount) external onlyAuthorized {
        if (amount > 0) {
            swapTokensForEth(amount, marketingWallet);
        }
    }

    function sendToLiquidity(uint256 amount) external onlyAuthorized {
        if (amount > 0) {
            uint256 toEth = amount / 2;
            uint256 toAKC = amount - toEth;
    ...
    function releaseFunds(uint256 amount, address to) external onlyAuthorized {
        require(address(honeyCoin) != address(0), "Honey Coin NOT SET");
        honeyCoin.transfer(to, amount);
    }

    function reserveBalance() external view returns (uint256) {
        return honeyCoin.balanceOf(address(this));
    }

    function setHoneyCoin(address hc) external onlyOwner {
        honeyCoin = IERC20(hc);
    }

    function swapTokensForEth(uint256 tokenAmount, address recipient) internal {
        address[] memory path = new address[](2);
        path[0] = address(honeyCoin);
        path[1] = router.WETH();

        if (honeyCoin.allowance(address(this), address(router)) < tokenAmount) {
            honeyCoin.approve(address(router), MAX_INT);
        }
