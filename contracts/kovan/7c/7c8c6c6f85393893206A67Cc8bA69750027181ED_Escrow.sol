...
contract Escrow {
    ...
    //  state events
    event newSale(address indexed seller, uint indexed gotchi);
    event concludeSale(address indexed buyer, uint indexed gotchi);
    event abortSale(address indexed seller, uint indexed gotchi);
    ...
    function withdrawERC721(uint _tokenId) public onlyAdmin {
    ...
    ) private
    {
        // add sale to seller
        uint id = getNewId();
        sellers[_seller].push(GotchiSale(_id, _gotchi, _price, _buyer));
        // add reference to buyer
        buyers[_buyer].push(SaleRef(_seller, _id));
    }

    function getOffer(address _buyer, uint _index) public view returns (
        address,
        uint
    ) {
    ...
    function getSale(address _seller, uint _index) public view returns (
        uint,
        uint,
        uint,
        address
    ...
    function getSaleIndex(address _seller, uint _id) public view returns (uint) {
        require(isSeller(_seller), "Cannot get sale: No sales found");

        uint index;
        
        // for each sales of the seller
    ...
    function removeSale(address _seller, uint _index ) private {
        ...
        uint buyer_sales = buyers[buyer].length;
        ...
        for(uint i=0; i < buyer_sales; i++) {
            ...
            if (buyers[buyer][i].seller == _seller) {
                // get the sale id
                uint sale_id = buyers[buyer][i].id;
                // check if it is for that particular id 
                if (sale_id == sellers[_seller][_index].id) {
                    // if so remove offer
        ...
        uint length = sellers[_seller].length;
    ...
    function getBuyerSalesCount(address _buyer) external view returns (uint) {
        require(isBuyer(_buyer), "No sales found");
        return buyers[_buyer].length;
    }

    function getSellerSalesCount(address _seller) external view returns (uint) {
    ...
    {
        return bytes4(
            keccak256("onERC721Received(address,address,uint,bytes)")
        );
    }
