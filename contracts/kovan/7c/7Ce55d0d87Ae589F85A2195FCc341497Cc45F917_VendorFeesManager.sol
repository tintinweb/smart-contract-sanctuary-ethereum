    ...
    OwnableUpgradeable
{
    ...
    function setPoolFees(address _pool, uint48 _feeRate, uint256 _type) external {
        ...
        if (rateFunction[_pool] != 0 && rateFunction[_pool] != _type) revert InvalidType();
        if (msg.sender == address(factory) || ILendingPool(_pool).owner() == msg.sender) {
    ...
    function getFee(
        ...
        if (rateFunction[_pool] == 2) {
            return (_rawPayoutAmount * getCurrentRate(address(_pool))) / 1000000;
        }
    ...
    function getCurrentRate(
        ...
        if (rateFunction[_pool] == 2) {
            return (feeRates[_pool] * uint48((ILendingPool(_pool).expiry() - block.timestamp))) / SECONDS_IN_YEAR;
        } 
