...
// File: contracts/tool/ManualMint.sol
...
contract ManualMint {
    ...
    constructor(BoxNFT _boxNFT, IERC20 _sns, IERC20 _sgs){
        boxNFT=_boxNFT;
        sns = _sns;
        sgs = _sgs;        
    }

    function mintBox(address to, uint number) public {
        for(uint i=0;i<number;i++) {
            boxNFT.mintNFT(to);
        }
    }

    function mintSns(address to, uint amount) public {
        TransferLib.transferFrom(sns,address(this),to,amount);
    }

    function mintSgs(address to, uint amount) public {
        TransferLib.transferFrom(sgs,address(this),to,amount);
    }
}