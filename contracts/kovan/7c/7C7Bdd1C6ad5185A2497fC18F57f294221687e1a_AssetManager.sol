...
contract AssetManager is IAssetManager, Ownable {
    ...
    CreatePoolConfig public configCreatePool;

    event PoolCreated(address pool, IAssetPool.Pool info);
    ...
    function createPool(IAssetPool.Pool memory pool) external payable {
        CreatePoolConfig memory config = configCreatePool;
        ...
        if (config.token == address(0)) value = msg.value;
            ...
            IERC20(config.token).transferFrom(msg.sender, address(this), config.initAmount);
            IERC20(config.token).approve(address(assetPool), type(uint).max);
        }
        assetPool.deposit{value: value}(config.token, config.initAmount);
        emit PoolCreated(address(assetPool), pool);
    }

    function _deployPool(IAssetPool.Pool memory pool) internal virtual returns (IAssetPool) {
        BeaconProxy assetPool = new BeaconProxy(
            beacon,
            abi.encodeWithSignature("initialize((address,bool,uint16,uint256,uint256))", pool)
        );
        return IAssetPool(address(assetPool));
    }
    ...
    function getUsdBalance(address account) external view override returns (uint usdBalance) {
        ...
        for (uint i = 0; i < tokensSupported.length; i++) {
            ...
            uint balance = token.getBalance(account);
