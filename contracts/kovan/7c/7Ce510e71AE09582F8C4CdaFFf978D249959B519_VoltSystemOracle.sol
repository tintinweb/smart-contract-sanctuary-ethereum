...
contract VoltSystemOracle is IVoltSystemOracle {
    ...
    function getCurrentOraclePrice() public view  returns (uint256) {
        ...
        // K: time delta capped on range of 1 - TIMEFRAME
        uint256 pricePercentageChange = cachedOraclePrice * monthlyChangeRateBasisPoints / Constants.BASIS_POINTS_GRANULARITY;
        // K: price percentage change is the total amount of monthly change
        uint256 priceDelta = pricePercentageChange * timeDelta / TIMEFRAME;
        // K: price delta is capped on a range of nearly 0 - 1 times the total monthly change
    ...
    function compoundInterest() external {
        // assumption: the time period will not be out of date due to the keepers
        uint256 periodEndTime = periodStartTime + TIMEFRAME; /// save a single warm SLOAD when writing to periodStartTime
        // K: periodStartTime and TIMEFRAME are both in the same denomination
        require(
            block.timestamp >= periodEndTime,
            "VoltSystemOracle: not past end time"
        );
        // K: Even if block timestamp was manipulated at this point, the time would be in a valid state

        /// first set Oracle Price to interpolated value
        oraclePrice = getCurrentOraclePrice(); // K: This function has no external calls and cannot be a reentrant vector
        ...
        // K: because the time will not be out of date, this period will not be more than one TIMEFRAME out of date
