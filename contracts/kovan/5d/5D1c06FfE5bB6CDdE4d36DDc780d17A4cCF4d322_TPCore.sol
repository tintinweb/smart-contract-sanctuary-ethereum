...
contract TPCore is AccessControl {
    ...
    uint256 public blockReward = 39 * (10**8);
    ...
    function unstake(uint256 amount_) public {
        ...
        uint256 rewardToTake = lToBurn * totalReward / liquidityToken.totalSupply(); // Saqo
    ...
    function TPYPerShare() public view returns (uint256) {
        if(liquidityToken.totalSupply() == 0) {
            return 0;
        }

        return totalTPYAvail() / liquidityToken.totalSupply(); 
    }
    ...
    function _takeTPYReward(uint256 lToBurn) private {
        ...
        if (tpy.balanceOf(address(this)) < tpyReward) {
            return;
        }
