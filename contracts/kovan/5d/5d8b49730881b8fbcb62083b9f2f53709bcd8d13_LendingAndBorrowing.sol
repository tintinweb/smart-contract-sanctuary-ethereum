...
abstract contract Controller {
    ...
    constructor(address _controllerAdmin) {
        tempAdmin = _controllerAdmin;
    }

    
    function deactivateController() public {
        require(controllerActivated==true && msg.sender == tempAdmin,"Controller is not activated");
        controllerActivated=false;
    }
}

// Set up you contract to convert DAI to USD or ETH to USD
contract LendingAndBorrowing is Ownable, Controller {
    ...
    modifier onlyPlatformAdmin() {
        if(controllerActivated) {
            require(msg.sender == tempAdmin,"Only Solvendo Controller Admin can call this function");
        }
        else {
    ...
    constructor(address _token, address solvendoControllerAdmin) Controller(solvendoControllerAdmin) {
