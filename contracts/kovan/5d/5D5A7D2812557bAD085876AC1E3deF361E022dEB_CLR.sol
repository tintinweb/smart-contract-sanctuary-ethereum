    ...
    StakingRewards
{
    ...
    function withdraw(uint256 amount) public {
        ...
        // Unstake rewards
        unstakeRewards(proRataBalance, msg.sender);
    ...
    /**
     * @dev Check how much CLR tokens will be minted on mint
    ...
    function mintInitial(
        ...
        uint256 mintAmount = _mintInternal(liquidity);
        // Stake CLR tokens
...
contract StakingRewards is IStakingRewards, ReentrancyGuard {
    ...
    mapping(address => uint256) public lastUpdateTime; // last time the rewards have been updated

    // Reward token mapping to information for each token
    // Reward token address => RewardInformation
    mapping(address => RewardInformation) public rewardInfo;

    bool public override rewardsAreEscrowed; // True if rewards are escrowed in RewardEscrow after unstaking

    IRewardEscrow public rewardEscrow; // Vesting / Escrow contract address
    ...
    struct RewardInformation {
        ...
        uint256 totalRewardAmount; // total amount of rewards sent to contract
    ...
    function claimRewardForSingleToken(address token) private {
        ...
        if (
            rewardAmount > 0 &&
            rewardInfo[token].remainingRewardAmount >= rewardAmount
        ) {
        ...
        override
    {
        ...
        rewardTokenInfo.remainingRewardAmount = rewardAmount;
...
interface IRewardEscrow {
    ...
    function addRewardsToken(address rewardToken) external;
    ...
    function removeRewardsToken(address rewardToken) external;

    function renounceOwnership() external;

    function rewardTokens(uint256) external view returns (address);

    function rewardsTokenVestingPeriod(address) external view returns (uint256);

    function setCLRPoolVestingPeriod(address rewardToken, uint256 vestingPeriod)
        external;
