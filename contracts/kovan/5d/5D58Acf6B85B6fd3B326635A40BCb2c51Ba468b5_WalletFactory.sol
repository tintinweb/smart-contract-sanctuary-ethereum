...
contract Wallet {
    ...
    address public rebalancerOwner;
    ...
    uint256 internal rebalancePeriods = 5 * 60; //30 * 86400;
    ...
    constructor(
        ...
        oracleAddress = address(_oracleAddress);
        ...
        returns (uint256, uint256)
    {
        ...
        uint256 tokenPrice = IPolybitOracle(oracleAddress).getTokenAddressPrice(
            _tokenAddress
        );
    ...
    // Update targetAssets with the latest targets from the PolybitOracle
    function getTargetAssets() public view returns (address[] memory) {
        (address[] memory targetAssets, ) = IPolybitOracle(oracleAddress)
            .getTargetAssets();
        return targetAssets;
    }
    ...
    function checkForDeposits() public onlyRebalancerOwner {
        ...
        emit returnBalance("ETH", _ethBalance);
        if (_ethBalance > 0) {
