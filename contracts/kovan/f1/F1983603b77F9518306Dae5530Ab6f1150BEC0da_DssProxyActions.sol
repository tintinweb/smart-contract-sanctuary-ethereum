...
contract DssProxyActions is Common, DSAuth {
    ...
    function getFee(address token, uint percent, uint value) public view returns (uint fee) {
    ...
    function lockETH(
        ...
        uint _fee = getFee(token, feeDeposit, msg.value);
    ...
    function lockGem(
        ...
        uint _fee = getFee(token, feeDepositGem, amt);
    ...
    function freeGem(
        ...
        address token = address(GemJoinLike(gemJoin).gem());
        uint _fee = getFee(token, feeWithdrawGem, amt);
    ...
    function lockGemAndDraw(
        ...
        uint _fee = getFee(token, feeDepositGem, amtC);
        // Takes token amount from user's wallet and joins into the vat
    ...
    function wipeAndFreeETH(
        ...
        // Sends ETH back to the user's wallet
        address token = address(GemJoinLike(ethJoin).gem());
        uint _fee = getFee(token, feeWithdraw, wadC);
    ...
    function wipeAndFreeGem(
        ...
        // Exits token amount to the user's wallet as a token
        address token = address(GemJoinLike(gemJoin).gem());
        uint _fee = getFee(token, feeWithdrawGem, wadC);
        if (_fee > 0) {
            GemJoinLike(gemJoin).gem().transferFrom(msg.sender, treasury, _fee);
        }
