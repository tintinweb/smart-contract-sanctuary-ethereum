...
contract SteamToken is ERC20Reward{
    ...
    /// @notice Constructor
    /// @param admins_ Addresses that will be granted the DEFAULT_ADMIN_ROLE.
    /// @param holder_ Address that will hold the initial supply of this token.
    constructor(address[] memory admins_, address holder_)
        ERC20Reward("STEAM", "STM", admins_)
    {
        _mint(holder_, D_INITIAL_STM_SUPPLY);
    }
...
uint256 constant D_INITIAL_H2O_SUPPLY = 1000000e18;
uint256 constant D_INITIAL_STM_SUPPLY = 1000000e18;

// Token initial Prices in H2O/

//todo decide initial pool size
uint256 constant D_INITIAL_ICE_POOL_H2O_SIZE = 100000e18;
uint256 constant D_INITIAL_ICE_PRICE = 25e18;

//todo decide initial pool size
uint256 constant D_INITIAL_STM_POOL_H2O_SIZE = 100000e18;
uint256 constant D_INITIAL_STM_PRICE = 5e18;

// The relationship between the error and the condensation rate
int256 constant D_CONDENSATION_ERROR_FACTOR = 5e7; //.00000000005

// The relationship between the accumulated error and the condensation rate
int256 constant D_CONDENSATION_ACCUM_FACTOR = 2e6 / int256(1 days); //0.000000000002 / SecondsPerDay

// The relationships between the error and the steam price adjustment
int256 constant D_STEAM_PRICE_FACTOR = 5e16;

// How long it takes for the current steam price to adjust to the target steam price
uint256 constant I_STM_PRICE_CHANGE_PERIOD = 1 days;
...
uint256 constant I_ICE_PRICE_CHANGE_PERIOD = uint256(50 days);
...
abstract contract ERC20Reward is ERC20MintableBurnable {
    ...
    ) ERC20MintableBurnable(name_, symbol_, admins_) {
    }
    ...
    function claimReward(address account)
        external onlyRole(CLAIMER_ROLE) returns (uint256)
    {
...
abstract contract ERC20MintableBurnable is ERC20, AccessControlEnumerable {
    ...
    constructor(
        ...
        // Initialize the admin role with the entries of the admins array.
        for (uint i = 0; i < admins_.length; ++i) {
            _setupRole(DEFAULT_ADMIN_ROLE, admins_[i]);
        }
    }
