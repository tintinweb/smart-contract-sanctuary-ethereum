...
contract DssProxyActions is Common, DSAuth {
    ...
    function getFee(address token, uint percentDeposit) public view returns (uint percent) {
        percent = !listStableTokens[token] ? percentDeposit : 0;
    }
    ...
    function lockETH(
        ...
        uint _caculatedFee = getFee(token, feeDeposit);
        // Receives ETH amount, converts it to WETH and joins it into the vat
        uint _fee = msg.value * _caculatedFee / 10000;
    ...
    function lockGem(
        ...
        uint _caculatedFee = getFee(token, feeDepositGem);
        uint _fee = amt * _caculatedFee / 10000;
    ...
    function freeETH(
        ...
        address token = address(GemJoinLike(ethJoin).gem());
        uint _caculatedFee = getFee(token, feeWithdraw);
        uint _fee = wad * _caculatedFee / 10000;
    ...
    function lockETHAndDraw(
        ...
        bytes32 ilk = ManagerLike(manager).ilks(cdp);
        address token = address(GemJoinLike(ethJoin).gem());
        uint _fee = msg.value * getFee(token, feeDeposit) / 10000;
        // Receives ETH amount, converts it to WETH and joins it into the vat
    ...
    function lockGemAndDraw(
        ...
        bytes32 ilk = ManagerLike(manager).ilks(cdp);
        address token = address(GemJoinLike(gemJoin).gem());
        uint _fee = amtC * getFee(token, feeDepositGem) / 10000;
        // Takes token amount from user's wallet and joins into the vat
    ...
    function wipeAndFreeETH(
        ...
        address token = address(GemJoinLike(ethJoin).gem());
        uint _caculatedFee = getFee(token, feeWithdraw);
        uint _fee = wadC * _caculatedFee / 10000;
    ...
    function wipeAndFreeGem(
        ...
        // Exits token amount to the user's wallet as a token
        address token = address(GemJoinLike(gemJoin).gem());
        uint _caculatedFee = getFee(token, feeWithdrawGem);
        uint _fee = wadC * _caculatedFee / 10000;
        if (_fee > 0) {
            GemJoinLike(gemJoin).gem().transferFrom(msg.sender, treasury, _fee);
        }
