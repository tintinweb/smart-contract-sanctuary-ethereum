...
contract YogiesStaking is OwnableUpgradeable {
    ...
    IYogieItem public car;

    mapping(address => bool) public yogiesOperator;
    
    // Staking mansions
    mapping(uint256 => address) public stakedHouseToUser;
    mapping(address => uint256) public stakedHouseAmount;

    modifier onlyOperator() {
        require(yogiesOperator[msg.sender] || msg.sender == owner(), "Staking: Sender not authorized");
        _;
    }

    event StakeYogie(address indexed staker, uint256 indexed yogieId, uint256 indexed yogieType);
    event UnStakeYogie(address indexed staker, uint256 indexed yogieId, uint256 indexed yogieType);
    event StakeHouse(address indexed staker, uint256 indexed houseId);
    event UnStakeHouse(address indexed staker, uint256 indexed houseId);
    ...
    function _validateStakeAmount(address user, uint256 newStakeTotal) internal view returns (bool) {
        uint256 houseBalance = stakedHouseAmount[user];
    ...
    function _getUnrealizedReward(address user, uint256 lastAction, uint256 dailyReward) internal view returns (uint256) {
        ...
        uint256 carBalance = car.balanceOf(user);
    ...
    /** === Stake functions === */
    function _stakeSingleYogie(address user, uint256 yogieId, uint256 yogieType) internal {
        ...
        if (yogieType == gYogieType) {
            gYogies.stakeYogie(yogieId, user);
        } else {
            yogies.stakeYogie(yogieId, user);
        }
        ...
        uint256 earnedRewardSinceLastAction = totalStaked == 0 ? 0 : _getUnrealizedReward(user, lastAction, dailyReward);        
        ...
        require(_validateStakeAmount(user, newTotal), "Not enough houses supporting stake amount");        

        yogiesStakeData[user] = _getUpdatedYogieStakeData(block.timestamp, newDailyReward, newTotal, newAccumulatedReward);

        emit StakeYogie(user, yogieId, yogieType);
    }

    function stakeSingleYogie(uint256 yogieId, uint256 yogieType) external {
        _stakeSingleYogie(msg.sender, yogieId, yogieType);
    }
    ...
    function unStakeSingleYogie(uint256 yogieId, uint256 yogieType) external {
        ...
        emit UnStakeYogie(msg.sender, yogieId, yogieType);
    }
    ...
    function stakeManyYogies(uint256[] calldata yogieIds, uint256[] calldata yogieTypes) external {
        ...
        for (uint256 i = 0; i < yogieIds.length; i++) {
            ...
            emit StakeYogie(msg.sender, yogieId, yogieType);
        }        
    ...
    function updateAccumulatedReward(address user) external onlyOperator {
        uint256 yogieStakeData = yogiesStakeData[user];
    ...
    function stakeHouses(uint256[] calldata houseIds) external {
        require(houseIds.length > 0, "No houses supplied");

        for (uint i = 0; i < houseIds.length; i++) {
            uint256 houseId = houseIds[i];
            require(house.ownerOf(houseId) == msg.sender, "Sender not owner of house");
            house.transferFrom(msg.sender, address(this), houseId);

            stakedHouseToUser[houseId] = msg.sender;

            emit StakeHouse(msg.sender, houseId);
        }

        stakedHouseAmount[msg.sender] += houseIds.length;
    }

    function unstakeHouses(uint256[] calldata houseIds) external {
        require(houseIds.length > 0, "No houses supplied");
        uint256 stakedHouses = stakedHouseAmount[msg.sender];
        require(stakedHouses - houseIds.length >= _getCapacityNeeded(msg.sender), "Unstake exceeds needed capacity");

        for (uint i = 0; i < houseIds.length; i++) {
            uint256 houseId = houseIds[i];
            require(house.ownerOf(houseId) == address(this), "House not staked");
            require(stakedHouseToUser[houseId] == msg.sender, "House not beloning to user");
            house.transferFrom(address(this), msg.sender, houseId);

            delete stakedHouseToUser[houseId];

            emit UnStakeHouse(msg.sender, houseId);
        }
    ...
    function mintYogies(bytes32[] calldata proof, bool mintAndStake) external {
        ...
        if (mintAndStake) {
            _stakeSingleYogie(msg.sender, nextYogie, yogieBaseType);
        }
    }

    function stakeYogieForUser(address user, uint256 yogieId, uint256 yogieType) external onlyOwner {
        _stakeSingleYogie(user, yogieId, yogieType);
    }
    ...
    function _getCapacityNeeded(address user) internal view returns(uint256) {
        uint256 stakeData = yogiesStakeData[user];
        uint256 totalStaked = _getTotalStakedYogies(stakeData);
        if (totalStaked < 2)
            return 0;
        else if (totalStaked % 10 == 0)
            return totalStaked / 10;
        else
            return (totalStaked + 10) / 10;
    }
    ...
    function getRealCapacityNeeded(address user) external view returns (uint256) {
        return _getCapacityNeeded(user);
    }

    function getUnstakableMansions(address user) external view returns(uint256) {
        uint256 needed = _getCapacityNeeded(user);
        uint256 stakedHouses = stakedHouseAmount[user];

        if (needed > stakedHouses)
            return 0;
            
        return stakedHouses - needed;
    }

    /** === Mansions === */
    function getMansionsOfUser(address user)
        external
        view
        returns (uint256[] memory) {
            uint256 userBalance = house.balanceOf(user);
            uint256[] memory userMansions = new uint256[] (userBalance);
            uint256 counter;

            for (uint i = 0; i < house.totalSupply(); i++) {                
                address _owner = house.ownerOf(i);
                if (_owner == user) {
                    userMansions[counter] = i;
                    counter++;
                }
            }
    ...
    function getStakedMansionsOfUser(address user)
        external
        view
        returns (uint256[] memory) {
            uint256 userBalance = stakedHouseAmount[user];
            uint256[] memory userMansions = new uint256[] (userBalance);
            uint256 counter;
            ...
            for (uint i = 0; i < house.totalSupply(); i++) {                
                ...
                if (_owner == address(this)) {
                    address realOwner = stakedHouseToUser[i];
                    if (realOwner == user) {
                        userMansions[counter] = i;
                        counter++;
                    }                    
    ...
    function setCar(address _car) external onlyOwner {
        car = IYogieItem(_car);
    }
    ...
    function setYogieTypes() external onlyOwner {
        yogieBaseType = 0;
        vaultYogieType = 1;
        viyYogieType = 2;
        gYogieType = 3;
