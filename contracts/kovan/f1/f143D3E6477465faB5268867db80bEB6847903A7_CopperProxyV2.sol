...
contract CopperProxyV2 is Ownable {
    ...
    EnumerableSet.AddressSet feeRecipients;
    ...
    function getFeeRecipients() external view returns (address[] memory recipients) {
        return feeRecipients.values();
    }

    function getRecipientShareBPS(address recipient) external view returns (uint256 shareSize) {
        for (uint256 i = 0; i < feeRecipients.length(); i++) {
            if (recipient == feeRecipients.at(i)) {
                return shareBPS[i];
            }
        }
    }
    ...
    function _distributeTokens(
        ...
        if (fundTokenBalance > initialFund) { 
            uint256 totalPlatformAccessFeeAmount = ((fundTokenBalance - initialFund) * platformAccessFeeBPS) / 10_000;
            remainingFundBalance = fundTokenBalance - totalPlatformAccessFeeAmount;

            if (isStandardFee == true && feeRecipients.length() > 0 ) {
                _distributePlatformAccessFee(pool, fundToken, totalPlatformAccessFeeAmount);
            }// else keep fee in contract in case of error, and manually send fee to recipients
        }
    ...
    /**
     * Distribute fee between recipients
     */
    function _distributePlatformAccessFee(address pool, address fundToken, uint256 totalFeeAmount) private {
        for (uint256 i = 0; i < feeRecipients.length(); i++) {
            uint256 proportionalAmount = (totalFeeAmount * shareBPS[i]) / 10_000;
            TransferHelper.safeTransfer(fundToken, feeRecipients.at(i), proportionalAmount);
            emit TransferredFee(pool, fundToken, feeRecipients.at(i), proportionalAmount);
        }
    }

    /**
     * Updates recipients and share.
    ...
    function updateRecipients(
        ...
        require(recipients.length == recipientShareBPS.length, "recipients and shareBPS must have same length");
        uint256 sumBPS = 0;
        for (uint256 i = 0; i < recipientShareBPS.length; i++) {
            require(recipientShareBPS[i] > uint256(0), "share size must be grader or equal than 0");
            sumBPS += recipientShareBPS[i];
            feeRecipients.add(recipients[i]);
        }
        require(sumBPS == 10_000, "invalid sum");
        shareBPS = recipientShareBPS;
        require(feeRecipients.length() == shareBPS.length,"only unique addresses are supported");
        // emit event
        emit RecipientsUpdated(feeRecipients.values(), shareBPS);
    }
