...
// File: contracts\abstracts\AScientistRepository.sol
...
abstract contract AScientistRepository is IScientistRepository, Multicall {
    ...
    function addScientist(ScientistData.Scientist memory _scientist) external override {
    ...
    function removeScientist(uint256 _tokenId, address _owner) external override {
...
contract Scientist is ERC721Tradable, AScientistRepository, ReentrancyGuard {
    ...
    function _burn(uint tokenId) internal override {
        ...
        require(ownerOf(tokenId) == msg.sender, "You are not owner of Scientist");
        super._burn(tokenId);
    }
    ...
    function addPoint(uint tokenId, uint pointAmount, ScientistData.Attribute attribute) external {
        require(ownerOf(tokenId) == msg.sender, "You are not owner of Scientist");
        ScientistData.Scientist memory scientist = _getScientist(tokenId);
        require(pointAmount <= scientist.point.remaining, "Scientist has not points enough to add");

        if(attribute == ScientistData.Attribute.PHYSICS) {
            scientist.point.physics += pointAmount;
        } else if (attribute == ScientistData.Attribute.CHEMISTRY) {
            scientist.point.chemistry += pointAmount;
        } else if (attribute == ScientistData.Attribute.BIOLOGY) {
            scientist.point.biology += pointAmount;
        } else if (attribute == ScientistData.Attribute.SOCIOLOGY) {
            scientist.point.sociology += pointAmount;
        } else if (attribute == ScientistData.Attribute.MATHEMATICS) {
            scientist.point.mathematics += pointAmount;
        } else {
            revert("Invalid type of point");
        }
        scientist.point.remaining -= pointAmount;
        _updateScientist(scientist, msg.sender);
    }
