...
contract Farm {
    ...
    function stake(uint256 amount) public {
        require(
            amount > 0 &&
            IERC20(pair).balanceOf(msg.sender) >= amount, 
    ...
    function unstake() public {
        require(
            isStaking[msg.sender] = true &&
            startTime[msg.sender] + timeLock <= block.timestamp, 
            "Tokens is locked"
        );

        claim();
        uint256 balTransfer = stakingBalance[msg.sender];
        stakingBalance[msg.sender] -= balTransfer;
        IERC20(pair).transfer(msg.sender, balTransfer);
    ...
    function claim() public {
        require(isStaking[msg.sender] = true, "You dont have tokens in stack");
        uint256 time = block.timestamp;
        uint256 timeFarm = time - startTime[msg.sender];
        uint256 reward = (timeFarm / timeLock) * (stakingBalance[msg.sender] * proccentRewars / 100);
