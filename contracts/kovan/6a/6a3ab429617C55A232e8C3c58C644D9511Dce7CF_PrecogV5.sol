...
contract PrecogV5 is IPrecogV5 { 
   ...
   function _increaseInvestment(address token, address account, uint256 amount) internal {
        require(isExistingToken[token], "PrecogV5: INVALID_TOKEN");
        require(isIncreasedInvesmentInCycle[token][account][profitCycles[token].length - 1] == false, "PrecogV5: CAN_NOT_DEPOSIT_AGAIN_AT_THIS_TIME");
        ...
        if (investment.lastProfitCycleId < updatedLatestCycle) {
            ...
            investmentOf[token][account] = Investment(investment.amount + amount, updatedUnit, previousUnit, updatedLatestCycle);
        } 
        else {
            updatedUnit = investment.unit + unit;                
            previousUnit = investment.previousUnit;
            investmentOf[token][account] = Investment(investment.amount + amount, updatedUnit, previousUnit, investment.lastProfitCycleId); 
        }
    ...
    function _decreaseInvestment(address token, address account, uint256 amount) internal {
        ...
        if (investment.lastProfitCycleId < updatedLatestCycle) {
            ...
            updatedUnit = investment.unit - unit;                
            previousUnit = investment.previousUnit;
            investmentOf[token][account] = Investment(updatedAmount, updatedUnit, previousUnit, investment.lastProfitCycleId); 
        }
    ...
    function takeProfit(address to, address token) external override {
        updateProfitCycleToCurrentCycle(token);
        _decreaseInvestment(token, msg.sender, 0);
    ...
    function withdraw(address to, address token, uint256 amount) external override {
        ...
        withdrawalRegister.claimWithdrawal(token, to, actualAmount);
        withdrawalRegister.claimWithdrawal(token, address(core), withdrawalFee);
