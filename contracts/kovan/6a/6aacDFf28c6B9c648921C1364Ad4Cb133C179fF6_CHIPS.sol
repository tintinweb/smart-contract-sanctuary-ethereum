...
contract CHIPS is Initializable, ERC20Upgradeable, OwnableUpgradeable{
    ...
    address internal constant UNISWAP_ROUTER_ADDRESS   = 0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D; // change it with mainnet router address
    address internal constant UNISWAP_FACTORY_ADDRESS  = 0x5C69bEe701ef814a2B6a3EDD4B1652CB9cc5aA6f; // change it with mainnet factory address
    ...
    // events
    event _sold(uint256 _amount, uint256 _taxAmount);
    event _bought(uint256 _amount, uint256 _taxAmount);
    event _minted(address _address, uint256 _quantity);

    event _currentBalane(uint curBal);
    event _etherWidthdraw(uint256 _amount); 
    event _distributeTaxAmount(uint256 _quantity);
    event _buyBack(address _from, uint256 _amount);
    ...
    function distribute() external payable onlyOwner{
        ...
        emit _minted(address(this), (TOTAL_SUPPLY.mul(15).div(1000)) * 10 ** decimals());          // Initial Liquidity => 1.5%
        emit _minted(teamAddress, (TOTAL_SUPPLY.mul(100).div(1000)) * 10 ** decimals());           // Team => 10%
        emit _minted(treasuryAddress, (TOTAL_SUPPLY.mul(205).div(1000)) * 10 ** decimals() );      // Treasury => 20.5%
        emit _minted(stakeRewardAddress, (TOTAL_SUPPLY.mul(680).div(1000)) * 10 ** decimals());    // Staking Pool Rewards  => 68%
    ...
    function addLiquidity() internal returns(bool){
        emit _currentBalane(address(this).balance);
    ...
    function distributeTaxAmount(uint256 amount) internal {
        ...
        emit _distributeTaxAmount(amount);
    }
    ...
    function buyBack(uint256 amount) public onlyOwner {
        ...
        emit _buyBack(msg.sender, amount);
    }
