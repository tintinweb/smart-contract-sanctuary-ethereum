...
contract PoolInvestor is IpoolInvestor, Investor, ERC20, Ownable {
    ...
    * @dev Returns debt token price
    ...
    function getDebtTokensToMintAmount(uint _amount) public view returns (uint256 toMint) {
        uint supplyAdd = 0;     // meant to relect if tokens will be minted to balance starting liquidity
        IRevenueManager revManager = IRevenueManager(lookup(Role.revManager));
        if(totalSupply() == 0){
            supplyAdd = revManager.balanceAvailable(address(this), address(token));
        }

        toMint =
        _amount.getDepositAmount(
            revManager.balanceAvailable(address(this), address(token)),
            totalSupply() + supplyAdd,
            reserveRate
        );
    }
    ...
    function getWithdrawalTokenReturnAmount(uint _amount) public view returns(uint256 toReturn){
        ...
        toReturn = _amount.getWithdrawalAmount(
            revManager.balanceAvailable(address(this), address(token)),
            totalSupply(),
            reserveRate
        );
    ...
    * NOTE ideally should send stright to revManager, but user would need to approve it
    ...
    function depositPool(uint _amount, string memory version) 
        ...
        // The starting ratio of liquidity to debt token supply must be 1
        // so we mint any debt tokens necessary to match starting liquidity
        // must do this minting before the getDebtTokensToMintAmount calculation
        if(totalSupply() == 0){
            IRevenueManager revManager = IRevenueManager(lookup(Role.revManager));
            _sendAllToRevManager();     // remember to send any sitting funds first to ensure they don't get sent later and mess up the calcuation
            _mint(lookup(Role.revManager), revManager.balanceAvailable(address(this), address(token)));
        }
