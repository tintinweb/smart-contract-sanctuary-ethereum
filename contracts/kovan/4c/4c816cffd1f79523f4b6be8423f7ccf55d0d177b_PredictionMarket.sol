...
contract PredictionMarket is AdminManaged,ReentrancyGuard {
    ...
    event Predict(uint id, address sender, uint amountBetted, bool answer);
    ...
    function settleMarket(uint id, bool _answer)
        public
        onlyTrustedSource
        returns(bool success)
    {
    	...
    	returns (bool success)
    {
        require(msg.value>0);
        require(isMarket(id));
        require(markets[id].betStatus == BetStatus.Pending);
        ...
        returns(bool success)
    {
        ...
        bool qAnswer = markets[qId].answer;
        uint reward;
        uint ratio;
