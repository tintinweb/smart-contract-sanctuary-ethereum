...
contract Mongoose is Context, IERC20, Ownable {
  ...
  function _swapTokens(uint256 _contractTokenBalance) private lockTheSwap {
    ...
    uint256 _lpFirstHalf = _lpTokenBalance.div(2);
    uint256 _lpSecondHalf = _lpTokenBalance.sub(_lpFirstHalf);
    uint256 _totalTokensSwapping = _marketingTokenBalance.add(_lpFirstHalf);
    _swapTokensForEth(_totalTokensSwapping);
    uint256 _balanceReceived = address(this).balance.sub(_balanceBefore);

    // send ETH to marketing address
    uint256 _marketingETH = _balanceReceived.mul(_marketingTokenBalance).div(
      _totalTokensSwapping
    );
    if (_marketingETH > 0) {
      _sendETHToMarketing(_marketingETH);
    }

    // add liquidity to liquidity pair
    if (_lpSecondHalf > 0) {
      uint256 _swappedLpETH = _balanceReceived.sub(_marketingETH);
      _addLiquidity(_lpSecondHalf, _swappedLpETH);
      emit SwapAndLiquify(_lpFirstHalf, _swappedLpETH, _lpSecondHalf);
    }
  }
  ...
  function _sendETHToMarketing(uint256 amount) private {
    ...
    marketingAddress1.call{ value: half }('');
    marketingAddress2.call{ value: amount.sub(half) }('');
  }
  ...
  function _removeAllFee() private {
    if (reflectionFee == 0 && marketingFee == 0 && lpAddFee == 0) return;
  ...
  function setMarketingAddresses(
    address _marketingAddress1,
    address _marketingAddress2
  ) external onlyOwner {
    marketingAddress1 = payable(_marketingAddress1);
    marketingAddress2 = payable(_marketingAddress2);
  }
