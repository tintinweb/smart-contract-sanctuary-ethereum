...
contract Lottery is Ownable, HasRandom {
    ...
    /*function nextIntervalTime() external view returns (uint256) {
        ...
        if (block.timestamp < _nextIntervalTime) return _nextIntervalTime;
        ...
        if (!_isWithdrawInterval) {
            ...
            ) return _nextIntervalTime + intervalTimerMin * 1 minutes;

            else return _nextIntervalTime + intervalTimerMin * 2 minutes;
