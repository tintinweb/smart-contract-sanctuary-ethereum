...
/*  Imports - Openzeppelin and TaxToken */
...
contract TaxVendorPol is Ownable {
    ...
    /*  State variables */
    IERC20 immutable taxToken; // Assigned in constructor
    IERC20 immutable usdc;     // Assigned in constructor
    ...
    /*  Struct */
    struct User {
        uint totalTokensClearedByUser; // User counter for amount of cleared tokencontracts.
    ...
    constructor(address taxTokenAddr) {
        usdcFee = 25 * 10 ** 18; // Initial fee set to 25 USDC.
        usdc = IERC20(0xd8f2c30Fab55F464cA8d7e7e1525F96ab6907928); // USDC on Polygon network.
    ...
    /*   
        ...
        executed at any time, as long as the fee is paid and the token is cleared and approved)
    */
    function payUsdcFeeAndClearToken(address clearToken) public returns (address) {
        uint fee = usdcFee;
        bool checkTokenCleared = false; 
         
        // Check approvals, allowances, fee and that the tokencontract is _NOT_ cleared.
        // If the tokencontract already is cleared, there is no need to clear it again.
        ...
        require(allowance >= fee, "You need to approve USDC");
        uint balance = usdc.balanceOf(msg.sender);
        require (balance >= fee, "Your balance of USDC is not enough to pay the fee");
        
        // Sends USDC fee from user to contract, and clears the tokencontract for swap.
        bool receivedFee = usdc.transferFrom(msg.sender,address(this), fee);
        require (receivedFee, "USDC fee needs to be paid");
        bool tokenAddrToTrue = _setTokenClearance(msg.sender, clearToken);
        require (tokenAddrToTrue, "Clearence for the token could not be set to 'true'");
    ...
    */
    function swapTokens(address yourTokenAddress, uint yourTokenAmount) public returns (bool swapComplete) {
        ...
        // Check balances, allowances and that the users tokencontract is cleared.
        ...
        // Sets tokenclearence back to 'false'(not cleared). Adding +1 to the TAX tokens sent out.         
        numberOfTaxTokenSent += 1;
        bool  tokenAddrToFalse = _clearTokenClearance(msg.sender, yourTokenAddress);
        require(tokenAddrToFalse, "Clearence for the token could not be set back to 'false'");
        ...
        require(receivedTokenFromUser, "Tokens could not be sent from the user using the standard 'transferFrom' ERC20 function");          
    ...
    */
    function resetNumberOfTaxTokenSent() public onlyOwner {
    ...
    */
    function checkTokenClearanceString(address user, address tokenAddress) public view returns (string memory) {
        string memory cleared = "The token is cleared for swap";
        string memory notCleared = "The token is NOT cleared for swap";
