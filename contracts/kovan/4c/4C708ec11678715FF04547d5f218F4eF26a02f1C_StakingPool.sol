...
contract StakingPool
{
	...
	function stake(uint256 value) public
	{
		...
		emit Staked(msg.sender, value);                         // Emits event.
	}
	...
	function unstake() public
	{
		...
		stakedToken.transfer(msg.sender, positions[msg.sender].value);          // Transfers staked tokens to msg.sender.
	...
	{
		uint256 reward = computeReward(msg.sender, epochs.length - 1);          // Computes reward for position.
		rewardToken.transfer(who, reward);                                      // Transfers reward to recipient.
		positions[msg.sender].startEpochId = epochs.length - 1;
	}
	...
	{
		uint256 balance = positions[msg.sender].value;                          // Gets value amount from position.
	...
	function viewReward(uint256 timestamp) external view returns (uint256)
	{
		uint256 endEpochId = epochs.length - 1;                                 // Gets id of last epoch.
		uint256 reward = computeReward(msg.sender, endEpochId);                 // Computes reward for this position and epoch.
		uint256 value = positions[msg.sender].value;                            // Gets amount of tokens staked.
		...
		return reward + positions[msg.sender].accumulatedReward;
	}

	function deposit(uint value) external
	{
		if (positions[msg.sender].value == 0)
		{
			stake(value);
		}
		else
		{
			addLpToPosition(value);
		}
	}
