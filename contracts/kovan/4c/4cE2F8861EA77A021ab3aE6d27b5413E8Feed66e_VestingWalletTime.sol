...
/**
 * Vesting wallet which vests linearly based on a given vesting period
 * Every *vestingPeriod()* seconds a given amount is vested
 */
contract VestingWalletTime is VestingWallet {
     ...
     * @dev Vesting period indicates the amount of time
     * @dev before the next vesting amount can be received
     */
    ...
    function _vestingSchedule(uint256 totalAllocation, uint64 timestamp) internal view override returns (uint256) {
        if (timestamp < start()) {
            ...
            uint256 vestingPeriodsPassed = timePassed / vestingPeriod();
            uint256 amountPerSecond = totalAllocation / duration();
            uint256 amountPerVestingPeriod = amountPerSecond * vestingPeriod();
            return (amountPerVestingPeriod * vestingPeriodsPassed);
        }
    }
}
