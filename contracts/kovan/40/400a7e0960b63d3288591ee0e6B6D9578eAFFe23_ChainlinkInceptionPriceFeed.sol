...
contract ChainlinkInceptionPriceFeed is IInceptionVaultPriceFeed, Initializable {
  ...
  ERC20 public override inceptionCollateral;

  AggregatorV3Interface public override assetOracle;
  ...
  function initialize(
    ...
    a = _addresses;
    inceptionCollateral = ERC20(_inceptionCollateral);
  ...
  function getAssetPrice() public view override returns (uint256 price) {
    ...
    (, int256 answer, , uint256 assetUpdatedAt, ) = assetOracle.latestRoundData();
   ...
   */
  function convertFrom(uint256 _amount) public view override returns (uint256) {
    uint256 price = getAssetPrice();
    uint8 collateralDecimals = ERC20(inceptionCollateral).decimals();
   ...
   */
  function convertTo(uint256 _amount) public view override returns (uint256) {
    uint256 price = getAssetPrice();
    uint8 collateralDecimals = ERC20(inceptionCollateral).decimals();
    uint8 parDecimals = ERC20(address(a.stablex())).decimals(); // Needs re-casting because ISTABLEX does not expose decimals()
    uint8 oracleDecimals = assetOracle.decimals();
