...
contract Cheops is ERC20 {
    ...
    function setLiquidityParameters(int24[2][] calldata _liquidityWallsRanges, uint[] calldata _liquidityWallsRatios, bool[] calldata _liquidityWallsInToken, uint _sellThresholdAmount) external {
    ...
    function reorganize() external {
        ...
        for (uint i=0; i < liquidityWallsRanges.length; i++) {
            if (liquidityWallsInToken[i]) {
                ...
                if (liquidityAmount > 0) addLiquidity(currentTick + liquidityWallsRanges[i][0], currentTick + liquidityWallsRanges[i][1], liquidityAmount, 0);
            }
            else {
                liquidityAmount = (liquidityWallsRatios[i] * quoteAmount) / 10000;

                if (liquidityAmount > 0) addLiquidity(currentTick + liquidityWallsRanges[i][0], currentTick + liquidityWallsRanges[i][1], 0, liquidityAmount);
            }
        }
    }
