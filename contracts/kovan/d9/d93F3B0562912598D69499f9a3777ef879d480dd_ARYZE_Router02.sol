...
contract ARYZE_Router02 is IARYZE_Router02 {
        ...
        )
    {
        ...
        address pair = ARYZE_Library.pairFor(factory, token, WETH);
    ...
    function swapExactETHForTokens(
        ...
        require(path[0] == WETH, 'ARYZE_Router: INVALID_PATH');
        amounts = ARYZE_Library.getAmountsOut(factory, msg.value, path);
        require(amounts[amounts.length - 1] >= amountOutMin, 'ARYZE_Router: INSUFFICIENT_OUTPUT_AMOUNT');
        IWETH(WETH).deposit{value: amounts[0]}();
        assert(IWETH(WETH).transfer(ARYZE_Library.pairFor(factory, path[0], path[1]), amounts[0]));
        _swap(amounts, path, to);
    }
    ...
    function swapTokensForExactETH(
        ...
        require(path[path.length - 1] == WETH, 'ARYZE_Router: INVALID_PATH');
        amounts = ARYZE_Library.getAmountsIn(factory, amountOut, path);
        require(amounts[0] <= amountInMax, 'ARYZE_Router: EXCESSIVE_INPUT_AMOUNT');
        TransferHelper.safeTransferFrom(
            path[0],
    ...
    function swapExactETHForTokensSupportingFeeOnTransferTokens(
        ...
        assert(IWETH(WETH).transfer(ARYZE_Library.pairFor(factory, path[0], path[1]), amountIn));
        uint256 balanceBefore = IERC20(path[path.length - 1]).balanceOf(to);
        _swapSupportingFeeOnTransferTokens(path, to);
        require(
            IERC20(path[path.length - 1]).balanceOf(to).sub(balanceBefore) >= amountOutMin,
