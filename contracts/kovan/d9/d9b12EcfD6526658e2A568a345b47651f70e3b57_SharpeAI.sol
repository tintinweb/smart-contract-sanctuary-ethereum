...
contract SharpeAI is IVault,IUniswapV3MintCallback,IUniswapV3SwapCallback,ERC20,ReentrancyGuard{
    ...
    IUniswapV2Router public immutable router; // Uniswap router
        ...
        returns (uint256 shares,uint256 amount0,uint256 amount1)
    {
        ...
        if (totalSupply == 0) {
            ...
            if (amount0Desired > 0 && amount1Desired == 0){
                require(amount0Desired > 0, "need amount0Desired");
                uint256 amount0ToSend = amount0Desired.div(totalSupply);
                uint256 difference = amount0Desired.sub(amount0ToSend);
                ...
                uint256[] memory amount1ToSend = router.swapExactTokensForTokens(
                    difference,
                    0,
                    path,
                    address(this),
                ...
                shares = swapCross0.mul(tS.sub(amount1ToSend[1])).div(total0.sub(amount0ToSend)).div(total1.sub(amount1ToSend[1]));

            }
            else if( amount1Desired > 0 && amount0Desired == 0 ){
                require(amount1Desired > 0, "need amount1Desired");
