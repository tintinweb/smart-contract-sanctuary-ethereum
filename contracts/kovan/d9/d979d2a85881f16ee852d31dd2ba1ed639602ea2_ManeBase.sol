...
contract ManeBase is ERC721A, Ownable, Pausable {
    ...
    // Total amount of minted (include refunded token)
    ...
    */ 
    ...
    constructor(string memory name_, string memory symbol_, uint256[] memory u256s,
        ...
        clubId = u256s[3];
        if (_clubIds[clubId] == true) {
            revert("ERC721(ManeBase): Duplicated clubId");
        }
        _clubIds[clubId] = true;
    ...
    function mint(address minter, uint256 mint_price, uint256 sign_deadline, bytes32 r, bytes32 s, uint8 v) payable whenNotPaused public returns (uint256) {
        ...
        uint256 expectPlatformFee = mint_price * ManeFactory(factory).platformFeePPM() / 10e6 ;
        ...
        ManeFactory(factory).wallet().transfer(actualPlatformFee);
    ...
    function setMintPrice(uint256 pre_sale_price, uint256 sale_price) public onlyOwner {
        require(pre_sale_price < sale_price, "ERC721(ManeBase): preSalePirce must be smaller than salePrice");
    ...
    function collect(uint256 val) public {
        ...
        require(to != address(0), "ERC721(ManeBase): You are not in the collector list");
...
contract ManeFactory is Ownable {
    ...
    //function deploy(string memory name_, string memory symbol_, uint256 reserveQuantity, uint256 totalSupply, uint256 presaleTotalSupply, uint256 clubId, uint256 presaleStartTime_, uint256 saleStartTime_, uint256 presalePrice_, uint256 salePrice_, address[] memory shareAddresses_, uint256[] memory shareRatios_, uint256[] memory refundEndTimes_, uint256[] memory refundRatios_) public returns (address) {
    function deploy(string memory name_, string memory symbol_,  uint256[] memory u256s, address[] memory shareAddresses_, uint256[] memory shareRatios_, uint256[] memory refundEndTimes_, uint256[] memory refundRatios_) public returns (address) {
        //ManeBase c = new ManeBase(name_, symbol_, reserveQuantity, totalSupply, presaleTotalSupply, clubId, presaleStartTime_, saleStartTime_, presalePrice_, salePrice_, shareAddresses_, shareRatios_, refundEndTimes_, refundRatios_);
