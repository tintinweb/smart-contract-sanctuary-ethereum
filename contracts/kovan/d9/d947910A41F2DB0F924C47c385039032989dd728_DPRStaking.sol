...
contract DPRStaking {
    ...
    constructor(IERC20 _usdt, address _submitter) public {
        usdt = _usdt;
    ...
    function stake(string calldata DPRAddress, uint256 amount) external whenNotPaused returns(bool){
       ...
       usdt.safeTransferFrom(msg.sender, address(this), staking_amount);
    ...
    function transferUserInfo(address oldUser, address newUser) external onlyOwner returns(bool){
        ...
        user_staking_time[newUser] = user_staking_time[oldUser];
        //clear account
    ...
    function setSubmitter(address newSubmitter) external onlyOwner returns(bool){
        submitter = newSubmitter;
        return true;
    }

    function addDPRAmount(address user, uint256 amount) external OnlySubmitter returns(bool){
        user_staking_amount[user] = user_staking_amount[msg.sender].add(amount);
        return true;
    }

    function decreaceDPRAmount(address user, uint256 amount) external OnlySubmitter returns(bool){
        require(user_staking_amount[msg.sender] >= amount, "Too large");
        user_staking_amount[user] = user_staking_amount[msg.sender].sub(amount);
        return true;
    }
    ...
    function adminWithdrawUserFund(address user) external onlyOwner returns(bool){
        ...
        usdt.safeTransfer(user, user_usdt_staking_amount[user]);
    ...
    function migrate() external returns(bool){
        ...
        usdt.approve(address(migrate_address), uint256(-1));
