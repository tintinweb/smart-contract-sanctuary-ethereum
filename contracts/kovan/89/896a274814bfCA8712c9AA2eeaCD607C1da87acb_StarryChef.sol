...
import "../interfaces/IStarryStaking.sol";
import "../interfaces/IRewarder.sol";

contract StarryChef is Ownable {
    ...
    IStarryStaking public rewardMinter;
    ...
    uint256 private constant MAX_REWARD_PER_SECOND = 1e19; // 1 token per second
    ...
    function deposit(
        ...
        if (pool.depositFee > 0) {
            depositFee = (amount * pool.depositFee) / PRECISION;
        }
        user.amount = user.amount + amount - depositFee;
        user.rewardDebt += int256(((amount - depositFee) * pool.accRewardPerShare) / ACC_REWARD_PRECISION);
        ...
        if (depositFee > 0 && feeReserve != address(0)) {
            lpToken[pid].safeTransfer(feeReserve, depositFee);
        }
    ...
    function add(
        ...
        poolInfo.push(PoolInfo({allocPoint: allocPoint, lastRewardTime: block.timestamp, accRewardPerShare: 0, depositFee: _fee}));
        require(poolInfo.length <= MAX_NUM_OF_POOLS, "StarryChef::add: > MAX_NUM_OF_POOLS");
        emit LogPoolAddition(lpToken.length - 1, allocPoint, _lpToken, _rewarder, _fee);
    }
    ...
    function setRewardPerSecond(uint256 _rewardPerSecond) public onlyOwner {
        require(_rewardPerSecond <= MAX_REWARD_PER_SECOND, "StarryChef::setRewardPerSecond: > MAX_REWARD_PER_SECOND");
        massUpdatePools();
    ...
    function setRewardMinter(IStarryStaking _rewardMinter) external {
        require(address(rewardMinter) == address(0), "StarryChef::setRewardMinter: Cannot redefine rewardMinter");
        rewardMinter = _rewardMinter;
    }

    // retrieve tokens erroneously sent in to this address
    function emergencyTokenRetrieve(
        address token,
        uint256 amount,
        address addr
    ) external onlyOwner {
    ...
    function setFeeReserve(address _feeReserve) external {
        require(_feeReserve != address(0), "StarryChef::setFeeReserve: not allowed");
    ...
    event LogPoolAddition(uint256 indexed pid, uint256 allocPoint, IERC20 indexed lpToken, IRewarder indexed rewarder, uint16 fee);
