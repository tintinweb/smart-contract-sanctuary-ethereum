...
contract EventCenter is Ownable {
    ...
    function startEpoch(uint256 _rewardAmount) external notInEpoch {
        ...
        emit StartEpoch(msg.sender, epochRound, epochStart, epochEnd, _rewardAmount);
    }
        ...
        onlyAccountDSA
    {
        ...
        emit UseFlashLoanForLeverage(EOA, account, token, amount, epochRound);
        if (__isInRewardEpoch() == true) {
            emit AddFlashLoanScore(EOA, account, token, amount, epochRound);
        }
    }
