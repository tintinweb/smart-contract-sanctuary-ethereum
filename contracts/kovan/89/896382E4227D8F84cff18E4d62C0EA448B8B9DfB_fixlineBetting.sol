...
contract fixlineBetting is Ownable, ReentrancyGuard {
    ...
    function makerLoss(
        ...
        usersData[makerAddress].withdrawableBalance +=
            ((totalBetAmount * 2) * 5) /
            100;
        payable(owner()).transfer(((totalBetAmount * 2) * 3) / 100);
        capsStakingContract_Ins.notifyReward{
            value: ((totalBetAmount * 2) * 2) / 100
        }();
    ...
    function takerLoss(
        ...
        usersData[tempBetData.maker].withdrawableBalance += (((tempBetData
            .basePrice * 2) * 90) / 100);
        payable(owner()).transfer(((tempBetData.basePrice * 2) * 3) / 100);
        capsStakingContract_Ins.notifyReward{
            value: ((tempBetData.basePrice * 2) * 2) / 100
        }();
