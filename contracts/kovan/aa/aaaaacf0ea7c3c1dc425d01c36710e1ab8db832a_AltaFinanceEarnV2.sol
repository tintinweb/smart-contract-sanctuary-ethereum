...
contract AltaFinanceEarnV2 is EarnBase {
        ...
        whenNotPaused
    {
        ...
        ALTA.transferFrom(msg.sender, address(this), _amount);
    ...
    function acceptBid(uint256 _bidId) external whenNotPaused {
        ...
        if (fee > 0) {
            ALTA.transfer(loanAddress, fee);
            bid.amount = bid.amount - fee;
        }
        ALTA.transfer(bid.to, bid.amount);
            ...
            for (uint256 i = length; i > 0; i--) {
                ...
                if (_bids[bidId].earnContractId == _earnContractId) {
                    if (!_bids[bidId].accepted) {
                        ALTA.transfer(_bids[bidId].bidder, _bids[bidId].amount);
                    }
                    _removeBid(bidId);
                }
            }
    ...
    function removeBid(uint256 _bidId) external {
        ...
        ALTA.transfer(bid.bidder, bid.amount);
