...
contract GameHub {
    mapping(uint16 => Monaco) public getGameByPin;

    function createGame(uint16 pin) public returns (Monaco) {
        require(address(getGameByPin[pin]) == address(0));

        getGameByPin[pin] = new Monaco();
