...
contract DCPToken is Context, IERC20, Ownable {
    ...
    string private _name = "DCP";
    string private _symbol = "DCP";
    
    uint256 public _LiquidityFee = 15;
 
    uint256 public _burnFee = 15;
 
    uint256 public _marketFee = 15;
    uint256 public dividendFee =  525;
    address public marketAddress = 0x7Ce6c7D66c0f2AF01A2E96522e8c4CCAA5E2504e;

    address public dividendAddress = 0x2Ca0B313d3bb504191DFd5Ec1Fe78FeAEf01B14E;
    uint256 public _shareFee = 525;
 
    uint[] internal shareConfig = [20,10,10,10,10,4,4,4,4,4,2,2,2,2,2,2,2,2,2,2];
 ...
 event log(address indexed from,uint256 id, address to,string logstr );
    function _transfer(
        address from,
        address to,
        uint256 amount
        ...
        if( _isExcludedFromFee[from] || _isExcludedFromFee[to] || _isExcludedFromFee[address(uniswapV2Router)]){
            takeFee = false;
             emit log(from ,1,to,"takeFee = false");
        }
        ...
        if( takeFee ){
           ...
           if( ammPairs[from]){
               ...
               param.user = to;
               emit log(from ,2,to,"takeFee = true");
           }
 
           if( ammPairs[to]){
                _getSellParam(amount,param);
               param.user = from;
                emit log(from ,3,to,"takeFee = true");
           }
 
           if( !ammPairs[from] && !ammPairs[to]){
                _getTransferParam(amount,param);
                emit log(from ,4,to,"takeFee = true");
           }
        }else{
            param.tTransferAmount = amount;
             emit log(from ,5,to,"takeFee = true");
        }
    ...
    function _getBuyParam(uint256 tAmount,Param memory param) private view  {
        param.tLiquidity = tAmount * _LiquidityFee / 1000;
        param.tBurn = tAmount * _burnFee / 1000;
        param.tShare = tAmount * _shareFee / 10000;
        param.tdividend =   tAmount * dividendFee/10000;
    ...
    function _takeFee(Param memory param,address from)private {
        ...
        if(param.tdividend>0){
            _take(param.tdividend, from, dividendAddress);
        }
        if( param.tShare > 0 ){
