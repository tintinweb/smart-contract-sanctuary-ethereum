...
contract FrankTreasury is Ownable {
    ...
    function _addPoolAndFarmLiquidity3(uint256 _amount, address _pool) public {
        ...
        uint256 minOut = TraderJoeRouter.getAmountsOut(_div, path)[1];
        uint256 out = TraderJoeRouter.swapExactTokensForTokens(_div, minOut, path, address(this), block.timestamp + 1000)[1];

        IERC20(tokenA).approve(address(TraderJoeRouter),_div);
        IERC20(tokenB).approve(address(TraderJoeRouter), out);

        (, , uint256 liquidity) = TraderJoeRouter.addLiquidity(tokenA, tokenB, _div, out, 0, 0, address(this), block.timestamp + 1000);
