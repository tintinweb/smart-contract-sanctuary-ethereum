...
// File: TokenSaleV3.sol

/// @author Jones DAO
/// @title Jones token sale contract V3
contract TokenSaleV3 is ReentrancyGuard {
    ...
    // Keeps track of Naboo deposited during whitelist phase
    uint256 public weiDepositedWhitelist;

    // Keeps track of Naboo deposited
    ...
    uint256 public jonesForWl = 0;

    // amount of jones tokens allocated to public sale
    uint256 public jonesForPublic = 0;

    // total wei deposited during the public sale
    uint256 public totalDepoPublic = 0;

    // Naboo raised in eccess
    uint256 eccessNaboo = 0;
    ...
    // Max Naboo that can be deposited by whitelisted addresses
    ...
    /// @param amount of Naboo deposited
    event TokenDeposit(
    ...
    /// @param claimer contract caller claiming on behalf of beneficiary
    /// @param beneficiary receives the tokens they claimed
    /// @param amount Naboo amount beneficiary claimed
    event NabooRefundClaim(
        address indexed claimer,
        address indexed beneficiary,
        uint256 amount
    );

    /// Emits on Naboo withdraw
    /// @param amount amount of Naboo that was withdrawn
    event WithdrawNaboo(uint256 amount);
    ...
    constructor(
        ...
        require(_naboo != address(0), "invalid naboo address");
        ...
        require(_saleStart > _saleWhitelistStart, "invalid saleStart");
    ...
    function depositForWhitelistedAddress(
        ...
        require(amount > 0, "must deposit greater than 0");
        require(
            amount <= depositableLeftWhitelist(beneficiary),
            "user whitelist allocation used up"
        );
        require(
            (weiDepositedWhitelist + amount) <= maxDepositsWhitelist,
        ...
        naboo.safeTransferFrom(msg.sender, address(this), amount);
        ...
        weiDepositedWhitelist = weiDepositedWhitelist.add(amount);
        weiDeposited = weiDeposited.add(amount);
        ...
        nonReentrant
    {
        ...
        deposits[beneficiary] = deposits[beneficiary].add(amount);
        weiDeposited = weiDeposited.add(amount);
        emit TokenDeposit(
            msg.sender,
            beneficiary,
        ...
        returns (uint256)
    {
        ...
        // total Jones allocated * user share in the Naboo deposited
        uint256 beneficiaryClaim = claimAmountJones(beneficiary);
        uint256 beneficiaryClaimNaboo = claimAmountNaboo(beneficiary);
        depositsWhitelist[beneficiary] = 0;
        deposits[beneficiary] = 0;

        jones.safeTransfer(beneficiary, beneficiaryClaim);

        if (beneficiaryClaimNaboo > 0) {
            naboo.safeTransfer(beneficiary, beneficiaryClaimNaboo);
        }

        emit TokenClaim(msg.sender, beneficiary, beneficiaryClaim);
        emit NabooRefundClaim(msg.sender, beneficiary, beneficiaryClaimNaboo);

        return beneficiaryClaim;
    }

    /// @dev Withdraws Naboo deposited into the contract. Only owner can call this.
    function withdraw() external {
        require(owner == msg.sender, "caller is not the owner");

        uint256 balance = naboo.balanceOf(address(this));
        naboo.safeTransfer(msg.sender, balance);

        emit WithdrawNaboo(balance);
    }
    ...
    function setMaxDeposits(uint256 _maxDeposits) external {
        ...
        // amount of jones tokens allocated to whitelist sale
        jonesForWl = jonesTokensAllocatedWhitelist.sub(unsoldWlJones);

        // amount of jones tokens allocated to public sale
        jonesForPublic = jonesTokensAllocated.sub(jonesForWl);

        // total wei deposited during the public sale
        totalDepoPublic = weiDeposited.sub(weiDepositedWhitelist);

        if (weiDeposited > maxDeposits) {
            eccessNaboo = weiDeposited.sub(maxDeposits);
        }
    ...
    /// View beneficiary's claimable Naboo amount
    /// @param beneficiary address to view claimable Naboo amount of
    function claimAmountNaboo(address beneficiary) public view returns (uint256) {
        // wei deposited during public sale by beneficiary
        uint256 userDepoPub = deposits[beneficiary];

        // if user has not depoisted during the public sale OR the sale did not reach the max deposit cap
        if (userDepoPub == 0 || eccessNaboo == 0) {
            return 0;
        }

        return eccessNaboo.mul(userDepoPub).div(totalDepoPublic);
    }

    /// View leftover depositable Naboo for whitelisted user
