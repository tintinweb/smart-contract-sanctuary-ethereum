    ...
    DarknodeRegistryStateV2
{
        ...
        returns (uint256)
    {
        ...
        if (penalty > 0) {
            ...
            require(
                ren.transfer(msg.sender, penalty),
                "DarknodeRegistry: REN transfer to slasher failed"
            );
        }
...
contract _DarknodeRegistryV1ToV2 is Ownable {
    RenProxyAdmin proxyAdmin;
    DarknodeRegistryLogicV2 dnrV2Logic;
    DarknodeRegistryLogicV1 dnrProxy;
    Slasher slasher;
    ...
    constructor(
        RenProxyAdmin _proxyAdmin,
        DarknodeRegistryLogicV2 _dnrV2Logic,
        DarknodeRegistryLogicV1 _dnrProxy,
        Slasher _slasher
    ) public {
        Ownable.initialize(msg.sender);
        proxyAdmin = _proxyAdmin;
        dnrV2Logic = _dnrV2Logic;
        dnrProxy = _dnrProxy;
        slasher = _slasher;

        proxyAdminOwner = proxyAdmin.owner();
        dnrOwner = _dnrProxy.owner();
    }

    function upgrade() public onlyOwner {
        dnrProxy.claimOwnership();
        // Start two-step process to return DNR, in case `returnDNROwner` can't
        // be called.
        dnrProxy.transferOwnership(dnrOwner);

        // Pre-checks
        uint256 numDarknodes = dnrProxy.numDarknodes();
        uint256 numDarknodesNextEpoch = dnrProxy.numDarknodesNextEpoch();
        uint256 numDarknodesPreviousEpoch = dnrProxy
            .numDarknodesPreviousEpoch();
        uint256 minimumBond = dnrProxy.minimumBond();
        uint256 minimumPodSize = dnrProxy.minimumPodSize();
        uint256 minimumEpochInterval = dnrProxy.minimumEpochInterval();
        uint256 deregistrationInterval = dnrProxy.deregistrationInterval();
        RenToken ren = dnrProxy.ren();
        DarknodeRegistryStore store = dnrProxy.store();
        IDarknodePayment darknodePayment = dnrProxy.darknodePayment();

        proxyAdmin.upgrade(
            // Cast dnrProxy to payable address and then to Proxy
            AdminUpgradeabilityProxy(address(uint160(address(dnrProxy)))),
            address(dnrV2Logic)
        );

        // Variables are not corrupted:

        // Post-checks
        require(
            numDarknodes == dnrProxy.numDarknodes(),
            "Migrator: expected 'numDarknodes' not to change"
        );
        require(
            numDarknodesNextEpoch == dnrProxy.numDarknodesNextEpoch(),
            "Migrator: expected 'numDarknodesNextEpoch' not to change"
        );
        require(
            numDarknodesPreviousEpoch == dnrProxy.numDarknodesPreviousEpoch(),
            "Migrator: expected 'numDarknodesPreviousEpoch' not to change"
        );
        require(
            minimumBond == dnrProxy.minimumBond(),
            "Migrator: expected 'minimumBond' not to change"
        );
        require(
            minimumPodSize == dnrProxy.minimumPodSize(),
            "Migrator: expected 'minimumPodSize' not to change"
        );
        require(
            minimumEpochInterval == dnrProxy.minimumEpochInterval(),
            "Migrator: expected 'minimumEpochInterval' not to change"
        );
        require(
            deregistrationInterval == dnrProxy.deregistrationInterval(),
        ...
        require(
            store == dnrProxy.store(),
            "Migrator: expected 'store' not to change"
        );
        require(
            darknodePayment == dnrProxy.darknodePayment(),
        ...
        require(
            DarknodeRegistryLogicV2(address(dnrProxy)).subnets(address(0x0)) ==
                0,
            "Upgrader: failed subnets() check"
        );
    ...
    function returnDNROwner() public onlyOwner {
        dnrProxy._directTransferOwnership(dnrOwner);
    }

    // NO_COMMIT - remove for mainnet
    function _returnProxyAdminTmp(address nextOwner) public onlyOwner {
        proxyAdmin.transferOwnership(nextOwner);
    }
}
...
interface IRenVMSignatureVerifier {
    /// @notice isValidSignature checks the the provided signature matches the
...
contract Slasher {
    ...
    event Challenged(
        address indexed _challenger,
        bytes32 indexed _epochHash,
        uint32 indexed _subnetID
    );
    ...
    function slash(
        ...
        for (uint256 i = 0; i < _darknodes.length; i++) {
            ...
            uint256 slashedAmount = dnr.slash(
                _subnetID,
                darknode,
                _challenger,
                _percentages[i]
            );
            slashedTotal = slashedTotal.add(slashedAmount);
            slashed[_epochHash][darknode] = _challenger;
        }

        // The challenger only gets half of the slashed bonds - this is to
        // disincentivize an attacked from slashing themselves in order to
        // prevent others from slashing them first.
        uint256 challengerReward = slashedTotal.div(2);

        // In a future version, this burnt amount may get paid to all other
        // darknodes. Currently it is just left in the Slasher contract.
        // uint256 burnAmount = slashedTotal.sub(challengerReward);

        require(
            dnr.ren().transfer(
                _challenger,
                challengerReward.add(challengeBond)
            ),
            "Slasher: reward transfer failed"
        );
    }
}
