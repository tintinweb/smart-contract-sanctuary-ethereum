...
//   address public chainLinkETHUSDAddress = 0x9326BFA02ADD2366b30bacB125260Af641031331;
  ...
  function initVariables(address _daiAddress,  address _wethAddress) public{
    // setting address
    ...
    quoter          = IQuoter(uinswapV3QuoterAddress);
    uniswapRouter   = IUniswapRouter (uinswapV3RouterAddress);
  }
  constructor() {
  ...
  function safeTransferWithApprove(uint256 amountIn, address routerAddress)
    ...
    //  selling Dai and getting ether
    function sellChips(uint256 _amountIn)
        external
        returns (uint256 _amountOut)
    {
        daiToken.approve(address(this), _amountIn);
    ...
    function buyChips(uint amountUSD) internal {
        uint256 deadline = block.timestamp + 15;
        uint24 fee = 3000;
        address recipient = address(this);
