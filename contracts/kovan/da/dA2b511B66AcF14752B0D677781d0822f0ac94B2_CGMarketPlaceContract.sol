...
contract CGMarketPlaceContract is IMarket, AccessControl {
    ...
    */
    ...
    function completeOrder(uint256 _orderId)
        ...
        require(token.balanceOf(_msgSender()) >=  order.askAmount, "CG: Not Enough Funds");
        require(
        ...
        override
    {
        ...
        require(token.balanceOf(_msgSender()) >= _bidAmount, "CG: Not Enough Funds for Bid");
        require(
            token.allowance(_msgSender(), address(this)) >= _bidAmount,
            "CG: Please Approve Tokens Before You Bid"
        );
    ...
    function acceptBid(uint256 _bidId)
        ...
        require(token.balanceOf(bid.sender) >= bid.bidAmount, "CG: Bidder don't have Enough Funds");
        require(
            token.allowance(bid.sender, address(this)) >= bid.bidAmount,
            "CG: Bidder has not Approved Tokens"
        );
