...
contract MultiFeeDistribution is ReentrancyGuard, Ownable {
    ...
    IShareReserve public stakingTokenReserve;
    ...
    constructor(
        ...
        require(address(_stakingTokenReserve) != address(0), "MultiFeeDistribution::constructor:: invalid address");
        stakingToken = IERC20(_stakingToken);
        stakingTokenReserve = IShareReserve(_stakingTokenReserve);
    ...
    function addReward(address rewardsToken, address distributor) public onlyOwner {
        require(rewardData[rewardsToken].lastUpdateTime == 0, "MultiFeeDistribution::addReward: Invalid");
    ...
    function approveRewardDistributor(
        ...
        require(rewardData[rewardsToken].lastUpdateTime > 0, "MultiFeeDistribution::approveRewardDistributor: Invalid");
        rewardDistributors[rewardsToken][distributor] = approved;
    }

    /*===================== VIEWS =====================*/

    function _rewardPerToken(address rewardsToken, uint256 supply) internal view returns (uint256) {
        if (supply == 0) {
            return rewardData[rewardsToken].rewardPerTokenStored;
        }
        return
            rewardData[rewardsToken].rewardPerTokenStored.add(
                lastTimeRewardApplicable(rewardsToken).sub(rewardData[rewardsToken].lastUpdateTime).mul(rewardData[rewardsToken].rewardRate).mul(1e18).div(supply)
            );
    }
    ...
    function _earned(
        ...
        return balance.mul(_rewardPerToken(rewardsToken, supply).sub(userRewardPerTokenPaid[user][rewardsToken])).div(1e18).add(rewards[user][rewardsToken]);
    }

    function lastTimeRewardApplicable(address rewardsToken) public view returns (uint256) {
    ...
    function rewardPerToken(address rewardsToken) external view returns (uint256) {
        uint256 supply = rewardsToken == address(stakingToken) ? lockedSupply : totalSupply;
        return _rewardPerToken(rewardsToken, supply);
    }

    function getRewardForDuration(address rewardsToken) external view returns (uint256) {
        return rewardData[rewardsToken].rewardRate.mul(rewardsDuration);
    }
    ...
    function _notifyReward(address rewardsToken, uint256 reward) internal {
        if (block.timestamp >= rewardData[rewardsToken].periodFinish) {
            rewardData[rewardsToken].rewardRate = reward.div(rewardsDuration);
        } else {
            uint256 remaining = rewardData[rewardsToken].periodFinish.sub(block.timestamp);
            uint256 leftover = remaining.mul(rewardData[rewardsToken].rewardRate);
            rewardData[rewardsToken].rewardRate = reward.add(leftover).div(rewardsDuration);
        }

        rewardData[rewardsToken].lastUpdateTime = block.timestamp;
        rewardData[rewardsToken].periodFinish = block.timestamp.add(rewardsDuration);
    }

    function notifyRewardAmount(address rewardsToken, uint256 reward) external updateReward(address(0)) {
        require(rewardDistributors[rewardsToken][msg.sender], "MultiFeeDistribution::notifyRewardAmount: Only reward distributors allowed");
    ...
    /// fallback for payable -> required to unwrap WETH
