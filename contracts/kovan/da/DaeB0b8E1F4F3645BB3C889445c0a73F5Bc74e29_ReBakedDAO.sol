...
contract ReBakedDAO is IReBakedDAO, Ownable, ReentrancyGuard, Projects {
	...
	*/
		...
		returns(bytes32 packageId_)
	{
		...
		//uint temp = feeObservers * 10*10*10 / PCT_PRECISION;
		_createPackage(projectId_, packageId_, budget_, budget_ * 5, bonus_);
	...
	*/
		...
		returns(bytes32[] memory packageIds_)
	{
		...
		for (uint256 i = 0; i < budgets_.length; i++) {
			...
			_createPackage(projectId_, packageIds_[i], budgets_[i], budgets_[i] * feeObservers * 5, bonuses_[i]);
		}
	...
	*/
		...
		returns(uint256 amount_)
	{
		...
		_payObserver(projectId_, amount_);
...
contract Projects is Packages {
	...
	*/
	...
	function _payObserver(
		bytes32 projectId_,
		uint256 amount_
	)
		internal
	{
		projectData[projectId_]._payObserver(amount_);
	}
...
library ProjectLibrary {
	...
	*/
	...
	function _payObserver(
	 ...
	 onlyExistingProject(project_)
	 {
		 IERC20(project_.token).safeTransfer(msg.sender, amount_*10e14);
		project_.budgetPaid += amount_;
	 }
