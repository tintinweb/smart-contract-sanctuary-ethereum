...
contract BondIssuer is IBondIssuer {
    ...
    /// @inheritdoc IBondIssuer
    // @dev Lazily issues a new bond when the time is right.
    function totalIssued() external override returns (uint256) {
        issue();
    ...
    /// @inheritdoc IBondIssuer
    // @dev Lazily issues a new bond when the time is right.
    function issuedBondAt(uint256 index) external override returns (IBondController) {
        issue();
...
interface IBondIssuer {
    ...
    function totalIssued() external returns (uint256);

    // @notice The bond address from the issued list by index.
    // @return Address of the bond.
    function issuedBondAt(uint256 index) external returns (IBondController);
}
