...
contract Claim is Ownable {
    ...
    function pendingAll(address user) public view returns(uint,uint,uint,uint){
        uint farmIPending = 0;
        uint farmIIPending = 0;
        uint distributionIPending = 0;
        uint distributionIIPending = 0;

        bool success;
        bytes memory retuan_data;

        bytes memory farmPayload = abi.encodeWithSignature("pending(address)", user);
        ( success, retuan_data) = address(farmI).staticcall(farmPayload);
        if(success){
            farmIPending =abi.decode(retuan_data,(uint256));
        }
        ( success,retuan_data) = address(farmII).staticcall(farmPayload);
        if(success){
            farmIIPending =abi.decode(retuan_data,(uint256));
        }

        bytes memory distributionIPayload = abi.encodeWithSignature("pendingRewardAccruedI(address,bool,bool)", user,true,true);
        ( success, retuan_data) = address(distributionI).staticcall(distributionIPayload);
        if(success){
            distributionIPending =abi.decode(retuan_data,(uint256));
        }
        bytes memory distributionIIPayload = abi.encodeWithSignature("pendingRewardAccruedII(address,bool,bool)", user,true,true);
        ( success, retuan_data) = address(distributionII).staticcall(distributionIIPayload);
        if(success){
            distributionIIPending =abi.decode(retuan_data,(uint256));
        }

        return (farmIPending,farmIIPending,distributionIPending,distributionIIPending);
    }
