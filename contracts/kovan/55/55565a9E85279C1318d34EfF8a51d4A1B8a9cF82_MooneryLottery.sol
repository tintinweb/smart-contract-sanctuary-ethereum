...
contract MooneryLottery is Lottery, VRFConsumerBase {
    ...
    /// Method to select winner and restart lottery.
    function endAndRestartLottery() public override 
                requireOpenState(currentLottery.state) 
                requireLotteryEndBlockNr(currentLottery.endBlockNr) 
                requireAtLeastOneTicket() {
        ...
        if (tickets.length == 1) {
            // Prevent unnecessary spending of LINK for VRF call if only one ticket
            _restartLotteryWithWinner(0); // first ticket is winner
        } else {
    ...
    function fulfillRandomness(bytes32 _requestId, uint256 _randomness) internal override {
        ...
        if (winnerIndex != -1) {
            _restartLotteryWithWinner(uint256(winnerIndex));
        }
    }

    /// Algorithm to determine winner ticket.
    function _restartLotteryWithWinner(uint256 ticketIndex) internal {
        Ticket memory winnerTicket = tickets[ticketIndex];
...
abstract contract Lottery {
    ...
    modifier requireAtLeastOneTicket() {
        require(tickets.length > 0, "At least one lottery ticket expected.");
        _;
    }
    ...
    function _restartLottery() internal {
      ...
      delete tickets;
      currentLottery = _createLottery();
      emit LotteryRestarted(previousLottery.winner, previousLottery.pot);
    }
