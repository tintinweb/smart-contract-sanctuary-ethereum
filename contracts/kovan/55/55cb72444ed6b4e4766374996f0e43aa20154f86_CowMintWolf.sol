...
contract CowMintWolf is ReentrancyGuard {
    ...
    constructor(INFTAdmin _wolfAdmin,ICowActivity _cowAddress,VRFCoordinatorV2Interface _link,ICowGame _cowGame) {
        cowGame=_cowGame;
    ...
    function _open(uint256 requestId,address to,uint allPower,uint randomX128,uint wolfKindRandomX128,uint wolfRandomX128) internal {
        ...
        if(allPower>=89999) {
            success=true;
        } else if(allPower>=49999) {
            if(randomX128>=MathX128.toX128(50,0)) {
                success=true;
            }
        } else if(allPower>=9999) {
            if(randomX128>=MathX128.toX128(90,0)) {
                success=true;
            }
        }
        if(success) {
