    ...
    IStructs
{
    ...
    function pause() public {
        if (msg.sender != firstResponder) revert NotFristResponder();
        _pause();
    }
    ...
    function licenseDiscount() private returns (uint256) {
        ...
        if (bal <= 0) {
            ...
            for (uint256 index = 0; index < bal; index++) {
                ...
                (uint256 maxCount, uint256 curCount, uint256 discount, , ) = licenseEngine.licenses(id);
                if (curCount + 1 <= maxCount && discount >= maxDiscount){
