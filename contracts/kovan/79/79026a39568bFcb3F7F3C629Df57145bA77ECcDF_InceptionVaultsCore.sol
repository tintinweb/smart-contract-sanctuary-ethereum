...
contract InceptionVaultsCore is IInceptionVaultsCore, BoringOwnable, ReentrancyGuard, Initializable {
  ...
  IAdminInceptionVault public adminInceptionVault;
  IInceptionVaultsDataProvider public inceptionVaultsData;
  IERC20 public inceptionCollateral;
  IAddressProvider public a;
  IInceptionVaultPriceFeed public inceptionPriceFeed;

  modifier onlyVaultOwner(uint256 _vaultId) {
    require(inceptionVaultsData.vaultOwner(_vaultId) == msg.sender);
    _;
  }
  ...
  function initialize(
    ...
    vaultConfig.liquidationRatio = _vaultConfig.liquidationRatio;
    vaultConfig.minCollateralRatio = _vaultConfig.minCollateralRatio;
    vaultConfig.borrowRate = _vaultConfig.borrowRate;
    vaultConfig.originationFee = _vaultConfig.originationFee;
    vaultConfig.liquidationBonus = _vaultConfig.liquidationBonus;
    vaultConfig.liquidationFee = _vaultConfig.liquidationFee;
    inceptionCollateral = _inceptionCollateral;
    lastRefresh = block.timestamp;
    _cumulativeRate = WadRayMath.ray();
    a = _addressProvider;
    adminInceptionVault = _adminInceptionVault;
    inceptionVaultsData = _inceptionVaultsDataProvider;
    owner = _owner;
    inceptionPriceFeed = _inceptionPriceFeed;
  }
  ...
  **/
  function deposit(uint256 _amount) public override {
    require(_amount > 0);
    inceptionCollateral.safeTransferFrom(msg.sender, address(this), _amount);
  ...
  **/
  function depositAndBorrow(uint256 _depositAmount, uint256 _borrowAmount) public override {
    deposit(_depositAmount);
    uint256 vaultId = inceptionVaultsData.vaultId(msg.sender);
  ...
  function _addCollateralToVault(uint256 _amount) internal {
    uint256 vaultId = inceptionVaultsData.vaultId(msg.sender);
    if (vaultId == 0) {
      vaultId = inceptionVaultsData.createVault(msg.sender);
    }
    _addCollateralToVaultById(vaultId, _amount);
  }

  function _addCollateralToVaultById(uint256 _vaultId, uint256 _amount) internal {
    IInceptionVaultsDataProvider.InceptionVault memory v = inceptionVaultsData.vaults(_vaultId);
    inceptionVaultsData.setCollateralBalance(_vaultId, v.collateralBalance.add(_amount));
  ...
  **/
  function withdraw(uint256 _vaultId, uint256 _amount) public override onlyVaultOwner(_vaultId) nonReentrant {
    ...
    inceptionCollateral.safeTransfer(msg.sender, _amount);
  }
  ...
  function _removeCollateralFromVault(uint256 _vaultId, uint256 _amount) internal {
    ...
    inceptionVaultsData.setCollateralBalance(_vaultId, newCollateralBalance);
    if (v.baseDebt > 0) {
      _refreshCumulativeRate();
      uint256 newCollateralValue = inceptionPriceFeed.convertFrom(newCollateralBalance);
      require(
        a.liquidationManager().isHealthy(
          newCollateralValue,
          inceptionVaultsData.vaults(_vaultId).baseDebt,
          vaultConfig.minCollateralRatio
        )
      );
    }
  ...
  function _refreshCumulativeRate() internal {
    ...
    uint256 _timeElapsed = block.timestamp.sub(lastRefresh);
    _cumulativeRate = a.ratesManager().calculateCumulativeRate(vaultConfig.borrowRate, _cumulativeRate, _timeElapsed);
  ...
  **/
  function borrow(uint256 _vaultId, uint256 _amount) public override onlyVaultOwner(_vaultId) nonReentrant {
    ...
    require(_amount <= stablex.balanceOf(address(adminInceptionVault)));
    IInceptionVaultsDataProvider.InceptionVault memory v = inceptionVaultsData.vaults(_vaultId);
    ...
    if (vaultConfig.originationFee > 0) {
      newDebt = newDebt.add(_amount.wadMul(vaultConfig.originationFee));
    }

    // Increment vault borrow balance

    uint256 newBaseDebt = a.ratesManager().calculateBaseDebt(newDebt, _cumulativeRate);

    inceptionVaultsData.setBaseDebt(_vaultId, v.baseDebt.add(newBaseDebt));

    uint256 collateralValue = inceptionPriceFeed.convertFrom(v.collateralBalance);
    uint256 newVaultDebt = inceptionVaultsData.vaultDebt(_vaultId);

    bool isHealthy = a.liquidationManager().isHealthy(collateralValue, newVaultDebt, vaultConfig.minCollateralRatio);

    require(isHealthy);

    adminInceptionVault.lendPAR(_amount, msg.sender);
  ...
  **/
  function repay(uint256 _vaultId, uint256 _amount) public override nonReentrant onlyVaultOwner(_vaultId) {
    // Make sure current rate is up to date
    _refreshCumulativeRate();

    uint256 currentVaultDebt = inceptionVaultsData.vaultDebt(_vaultId);
    ...
    IERC20 stablex = IERC20(a.stablex());
    stablex.safeTransferFrom(msg.sender, address(adminInceptionVault), _amount);
  ...
  **/
  function _reduceVaultDebt(uint256 _vaultId, uint256 _amount) internal {
    ...
    if (remainder == 0) {
      inceptionVaultsData.setBaseDebt(_vaultId, 0);
    } else {
      uint256 newBaseDebt = a.ratesManager().calculateBaseDebt(remainder, _cumulativeRate);
      inceptionVaultsData.setBaseDebt(_vaultId, newBaseDebt);
    }
  }
  ...
  **/
  function liquidatePartial(uint256 _vaultId, uint256 _amount) public override nonReentrant {
    ...
    require(!a.liquidationManager().isHealthy(collateralValue, currentVaultDebt, vaultConfig.liquidationRatio));

    uint256 repaymentAfterLiquidationFeeRatio = WadRayMath.wad().sub(vaultConfig.liquidationFee);
    ...
    uint256 collateralValueToReceive = _amount.add(_amount.wadMul(vaultConfig.liquidationBonus));
    ...
    if (collateralValueToReceive >= collateralValue) {
      ...
      uint256 discountedCollateralValue = collateralValue.wadDiv(vaultConfig.liquidationBonus.add(WadRayMath.wad()));
      if (currentVaultDebt > discountedCollateralValue) {
        ...
        require(a.stablex().balanceOf(address(adminInceptionVault)) >= insuranceAmount);
      }
    ...
    stablex.safeTransfer(address(adminInceptionVault), _amount);

    // Send the claimed collateral to the liquidator
    uint256 collateralToReceive = inceptionPriceFeed.convertTo(collateralValueToReceive);
    inceptionVaultsData.setCollateralBalance(_vaultId, v.collateralBalance.sub(collateralToReceive));
    inceptionCollateral.safeTransfer(msg.sender, collateralToReceive);
  ...
  /**
    ...
    backwards compatibility with the InceptionVaultsDataProvider.
  **/
  function getCumulativeRate() public view override returns (uint256) {
