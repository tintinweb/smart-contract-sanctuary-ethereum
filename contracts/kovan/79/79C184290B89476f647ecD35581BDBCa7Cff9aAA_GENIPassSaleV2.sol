...
contract GENIPassSaleV2 is IGENIPassSaleV2, IERC721ReceiverUpgradeable, PermissionGroupUpgradeable, ReentrancyGuardUpgradeable, PausableUpgradeable {
    ...
    function withdrawNftEmergency(uint[] memory tokenIds, address recipient) external onlyOperator {
        for (uint i = 0; i < tokenIds.length; i++) {
            require(geniPass.ownerOf( tokenIds[i]) == address(this), "GENIPassSale: not for sale");

            geniPass.transferFrom(address(this), recipient, tokenIds[i]);
            emit WithdrawGeniPass(msg.sender, recipient, tokenIds[i]);

            delete saleInfos[tokenIds[i]];
        }
    }
