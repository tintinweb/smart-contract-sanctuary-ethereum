...
contract PooledCreditLine is ReentrancyGuardUpgradeable, OwnableUpgradeable, IPooledCreditLine {
    ...
    * @param gracePenaltyRate rate at which penalty is levied during grace period
    ...
    modifier onlyCreditLineBorrower(uint256 _id) {
        require(pooledCreditLineConstants[_id].borrower == msg.sender, 'OCLB1');
        _;
    }
     ...
     */
    event PooledCreditLineRequested(uint256 indexed id, address indexed borrower);
     ...
     * @param id identifier for the pooled credit line
     */
    event PooledCreditLineCancelled(uint256 indexed id);
    ...
    function _limitBorrowedInUSD(
        ...
        require(_minBorrowLimitInUSD >= _borrowLimitMin, 'ILB3');
    }
    ...
    function updateCollectionPeriodLimits(uint256 _min, uint256 _max) external onlyOwner {
        require(_min < _max || _min.mul(_max) == 0, 'UCPL1');
    ...
    function updateDurationLimits(uint256 _min, uint256 _max) external onlyOwner {
        require(_min < _max || _min.mul(_max) == 0, 'UDL1');
    ...
    function updateDefaultGracePeriodLimits(uint256 _min, uint256 _max) external onlyOwner {
        require(_min < _max || _min.mul(_max) == 0, 'UDGPL1');
    ...
    function updateGracePenaltyRateLimits(uint256 _min, uint256 _max) external onlyOwner {
        require(_min < _max || _min.mul(_max) == 0, 'UGPRL1');
    ...
    function updateProtocolFeeFraction(uint256 _protocolFeeFraction) external onlyOwner {
        require(protocolFeeFraction != _protocolFeeFraction, 'UPFF1');
    ...
    function updateVerification(address _verification) external onlyOwner {
        require(verification != _verification, 'UV1');
     ...
     */
    constructor(address _lenderPool, address _usdc) {
    ...
    function request(Request calldata _request) external nonReentrant returns (uint256) {
        require(IVerification(verification).isUser(msg.sender, _request.borrowerVerifier), 'R1');
        ...
        require(IStrategyRegistry(strategyRegistry).registry(_request.borrowAssetStrategy) != 0, 'R5');
        ...
        emit PooledCreditLineRequested(_id, msg.sender);
        return _id;
    }
    ...
    function accept(uint256 _id, uint256 _amount) external override nonReentrant onlyLenderPool {
        require(pooledCreditLineVariables[_id].status == PooledCreditLineStatus.REQUESTED, 'A1');
    ...
    function depositCollateral(
        ...
        PooledCreditLineStatus _status = getStatus(_id);
        require(_status == PooledCreditLineStatus.ACTIVE || _status == PooledCreditLineStatus.EXPIRED, 'DC1');
    ...
    * @param _toSavingsAccount if true, tokens are transferred from savingsAccount 
    ...
    function _withdrawCollateral(uint256 _id, bool _toSavingsAccount) internal {
        ...
        _transferCollateral(_id, pooledCreditLineConstants[_id].collateralAsset, _withdrawableCollateral, _toSavingsAccount);
    ...
    function withdrawableCollateral(uint256 _id) public returns (uint256) {
        if (getStatus(_id) == PooledCreditLineStatus.EXPIRED) {
            return 0;
        }
    ...
    function _transferCollateral(
        ...
        if (_toSavingsAccount) {
            _amountReceived = _savingsAccount.transferShares(_amountInShares, _asset, _strategy, msg.sender);
        } else {
            _amountReceived = _savingsAccount.withdrawShares(_asset, _strategy, msg.sender, _amountInShares, false);
        }
    ...
    function _borrow(uint256 _id, uint256 _amount) internal {
        ...
        require(block.timestamp >= pooledCreditLineConstants[_id].startsAt, 'B2');
        ...
        ILenderPool(lenderPool).borrowed(_id, _sharesWithdrawn);
    ...
    function calculateBorrowableAmount(uint256 _id) public returns (uint256) {
        ...
        return Math.min(_borrowLimit - _principal, _maxPossible - _currentDebt);
    }
    ...
    function _withdrawBorrowAmount(
        ...
        _savingsAccount.withdrawFrom(_asset, _strategy, lenderPool, address(this), _amountInTokens, false);
        return _shares;
    }
    ...
    function repay(uint256 _id, uint256 _amount) external nonReentrant {
        PooledCreditLineStatus currentStatus = getStatus(_id);
        require(currentStatus == PooledCreditLineStatus.ACTIVE || currentStatus == PooledCreditLineStatus.EXPIRED, 'R1');
        ...
        require(_totalCurrentDebt != 0, 'R2');
    ...
    function _repay(uint256 _id, uint256 _amount) internal returns (uint256) {
        ...
        _savingsAccount.deposit(_borrowAsset, _strategy, lenderPool, _amount);
    ...
    function calculateInterestAccrued(uint256 _id) public view returns (uint256) {
        ...
        uint256 _penalityInterest;
        if (_lastPrincipalUpdateTime <= _endTime && block.timestamp > _endTime) {
            _penalityInterest = _calculateInterest(_principal, _penaltyRate, block.timestamp.sub(_endTime));
        } else if (_lastPrincipalUpdateTime > _endTime) {
            _penalityInterest = _calculateInterest(_principal, _penaltyRate, block.timestamp.sub(_lastPrincipalUpdateTime));
        }
    ...
    function liquidate(uint256 _id) external override nonReentrant onlyLenderPool returns (address, uint256) {
        PooledCreditLineStatus currentStatus = getStatus(_id);
        require(pooledCreditLineVariables[_id].principal != 0, 'L1');
        require(currentStatus == PooledCreditLineStatus.ACTIVE || currentStatus == PooledCreditLineStatus.EXPIRED, 'L2');
        address _collateralAsset = pooledCreditLineConstants[_id].collateralAsset;
        ...
        uint256 _collateralReceived = _transferCollateral(_id, _collateralAsset, _collateralToLiquidate, false);
    ...
    function close(uint256 _id) external nonReentrant onlyCreditLineBorrower(_id) {
        require(pooledCreditLineVariables[_id].status == PooledCreditLineStatus.ACTIVE, 'C1');
    ...
    function cancelRequest(uint256 _id) external nonReentrant onlyCreditLineBorrower(_id) {
        require(pooledCreditLineVariables[_id].status == PooledCreditLineStatus.REQUESTED, 'CR1');
        require(block.timestamp < pooledCreditLineConstants[_id].startsAt, 'CR2');
        ...
        emit PooledCreditLineCancelled(_id);
    }
    ...
    function terminate(uint256 _id) external nonReentrant onlyOwner {
        ...
        delete pooledCreditLineVariables[_id];
        delete pooledCreditLineConstants[_id];
        ILenderPool(lenderPool).terminate(_id, msg.sender);
    ...
    function cancelRequestOnLowCollection(uint256 _id) external override nonReentrant onlyLenderPool {
        require(pooledCreditLineVariables[_id].status == PooledCreditLineStatus.REQUESTED, 'CRLC1');
        require(block.timestamp >= pooledCreditLineConstants[_id].startsAt, 'CRLC2');
        delete pooledCreditLineVariables[_id];
        delete pooledCreditLineConstants[_id];
        pooledCreditLineVariables[_id].status = PooledCreditLineStatus.CANCELLED;
        emit PooledCreditLineCancelled(_id);
     ...
     */
    function getStatus(uint256 _id) public override returns (PooledCreditLineStatus) {
...
library SavingsAccountUtil {
    ...
    function depositFromSavingsAccount(
        ...
        require(_token != address(0), 'DFSA1');
        if (_toSavingsAccount) {
