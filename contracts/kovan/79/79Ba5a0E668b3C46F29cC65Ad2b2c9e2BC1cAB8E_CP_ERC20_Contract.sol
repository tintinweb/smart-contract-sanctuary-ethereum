...
contract CP_ERC20_Contract is ERC2771Context, Ownable {
    ...
    function transfer(address from, address to, uint amount)public returns(bool){
        require(balances[from] >= amount, "Balance of buyer must be enough!");
        balances[from] -= amount;
        balances[to] += amount;
        return true;
    }

    function withdraw(address receiver, uint amount) external {
        require(balances[receiver] <= amount, "Balance of requester must be enough!");
