...
contract ShineMineStake is Initializable, OwnableUpgradeable, UUPSUpgradeable {
    ...
    mapping(uint256 => uint256) private rewardsPerPointEpoch; //this keeps tally of the expected rewards per point for each epoch
    ...
    uint256 public silverReward;
    uint256 public goldReward;
    uint256 public diamondReward;
    uint256 private rewardDenominator;

    uint256 public epoch;

    uint256 public silverMinTime;
    uint256 public goldMinTime;
    uint256 public diamondMinTime;
    ...
    function initialize() initializer public {
        ...
        goldReward = 3;
        diamondReward = 4;
        silverMinTime = 14 days;
        goldMinTime = 90 days;
        diamondMinTime = 180 days;
        epoch = 0;
        DIAMOND = "DIAMOND";
        GOLD = "GOLD";
        SILVER = "SILVER";
...
*/
    ...
    function stake(string calldata pool, uint256 amount) public {
        updatePoints(msg.sender);
    ...
    function unstake(string calldata pool, uint256 amount) public canUnstake(pool) { 
        updatePoints(msg.sender);
        if (keccak256(abi.encodePacked(pool)) == keccak256(abi.encodePacked(SILVER))) {
            userStake[msg.sender].silverAmount -= amount;
            totalSilverStaked -= amount;
            ...
            Shine.transferFrom(address(this), msg.sender, amount);
        }
        else if (keccak256(abi.encodePacked(pool)) == keccak256(abi.encodePacked(GOLD))) {
            userStake[msg.sender].goldAmount -= amount;
            totalGoldStaked -= amount;
...
*/
    ...
    function getTotalPointsEpoch(uint256 _epoch) public view returns (uint256) { //RETURNS THE TOTAL REWARD POINTS FOR A GIVEN EPOCH. VALUE SHOULD INCREASE UNTIL THE EPOCH ENDS.
        return epochPoints[_epoch] + getPendingRewardPointsEpoch(_epoch);
    }

    function getRewardsPerPointEpoch(uint256 _epoch) public view returns (uint256) { //RETURNS THE TOTAL REWARD POINTS FOR A GIVEN EPOCH. VALUE SHOULD INCREASE UNTIL THE EPOCH ENDS.
        return rewardsPerPointEpoch[_epoch];
    }
