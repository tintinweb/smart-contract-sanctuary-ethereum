...
contract NFT721Bridge is
    ...
    function checkSignatures(
        ...
        require(minApprovers >= 2, "invalid min approvers");
        require(bridgeApprovers.length >= minApprovers, "invalid min approvers");
        uint256 successSigner = 0;
        if (
            r.length == s.length &&
            s.length == v.length &&
    ...
    function setUnClaim(bytes32 _claimId) public onlyGovernance {
        alreadyClaims[_claimId] = false;
    }
...
//        string memory _tokenURI
        ...
        if (!(bridgeNFT721Tokens[_originToken] && chainId == _chainIdsIndex[0]) && tokenMap[_chainIdsIndex[0]][_originToken] == address(0)){
            //create bridge token
    ...
    function _mintMultiNFT721ForUser(
        ...
        if (bridgeNFT721Tokens[_originToken] && chainId == _chainIdsIndex[0]) {
            for (uint256 i = 0; i < _tokenIds.length; i++) {
                IERC721Upgradeable(_originToken).safeTransferFrom(address(this), _toAddr, _tokenIds[i]);
            }
        } else {
    ...
    function emergencyWithdrawNFT(address tokenAddress, address receiver, uint256 tokenId) public onlyGovernance {
