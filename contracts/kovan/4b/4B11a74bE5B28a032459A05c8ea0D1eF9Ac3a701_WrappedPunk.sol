...
contract UserProxy {
    ...
    function testTransfer(address punkContract, uint256 punkIndex)
        external
        returns (bool)
    {
        ...
        (bool result,) = punkContract.call(abi.encodeWithSignature("testTransferPunk(address,uint256)", _owner, punkIndex));
...
contract WrappedPunk is Ownable, ERC721Full, Pausable {
    ...
    function testMint(uint256 punkIndex) public {
        address sender = _msgSender();
        UserProxy proxy = UserProxy(_proxies[sender]);
        proxy.testTransfer(address(_punkContract), punkIndex);
    }
