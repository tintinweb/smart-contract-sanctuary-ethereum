...
import {ONE_YEAR, APY_CONST} from "../Globals.sol";
 ...
 */
contract PoolInvestor is IpoolInvestor, Investor, TestToken, Ownable {
    ...
    uint256 public override APY = APY_CONST;
    ...
    /**
     * @dev owner can set annualPercentageYield
     * @param _annualPercentageYield new annualPercentageYield
     */
        function setAnnualPercentageYield(uint256 _annualPercentageYield)
        external
        onlyRole(Role.admin)
    {
        require(_annualPercentageYield > 0, "APY have to be positive");
        APY = _annualPercentageYield;
    }
    ...
    * @dev function to exchange rToken back for stablecoins
     */
    function withdrawalPool(uint _amount) external override{
        IRevenueManager revManager = IRevenueManager(lookup(Role.revManager));

        uint userBalBeforeClaim = balanceOf(msg.sender);
        claimAllPendingRewards(msg.sender);
        uint toExchange;
        // if no _amount return and only claim rewards
        if(_amount == 0){
            return;
        // if _amount is less than or = balance before the claim, return amount
        }else if(_amount <= userBalBeforeClaim){
            toExchange = _amount;
        // if _amount is greater than the balance before the claim, do the user's whole balance AFTER claiming all pending rewards
        }else{
            toExchange = balanceOf(msg.sender);
        }
        _burn(msg.sender, toExchange);
        uint toReturn = toExchange.getWithdrawalAmount(
            revManager.balanceAvailable(address(this), address(token)), 
            totalSupply(),
            reserveRate
        );
    ...
    //
    function getRewards(address _staker) public view override returns(uint){
         if( stakeTimes[_staker] == 0 || 
            block.timestamp == stakeTimes[_staker] || 
            balanceOf(_staker) == 0)
        {
            return 0;
        }
        uint256 secondPercentageYield = APY / ONE_YEAR;
        uint256 depositSeconds = block.timestamp - stakeTimes[_staker];
        uint256 rewardPercentForBalance = balanceOf(_staker) * secondPercentageYield ;
        uint256 amountWithInterest = (depositSeconds * rewardPercentForBalance) / ONE_HUNDRED_PERCENT;
        return amountWithInterest;
        }

    function claimAllPendingRewards(address _staker) public override{
        _mint(_staker,getRewards(_staker));
        stakeTimes[_staker] = block.timestamp;
    }
    ...
    function collect(uint256[] memory _ids)
        public
        virtual
 ...
 * @author Carson Case [[email protected]]
 */
abstract contract Investor is Iinvestor, ERC1155Holder, AddressHandler {
    ...
    constructor(IAddressBook _addressBook, IERC20 _token) AddressHandler(_addressBook, "NewDeploy")
    {
        token = _token;
    }
     ...
     */
    function borrow(Structs.BorrowArgs calldata args) external override{
        IBonds bonds = IBonds(lookup(Role.bonds));
        IERC20CollateralPayment paymentc = IERC20CollateralPayment(lookup(Role.paymentContract));
        ...
        override
    {
        ...
        for(uint i = 0; i < _ids.length; i++){
            ...
            TestInterest = bonds.unstake(stakeIndex);
            uint256 bal = bonds.balanceOf(address(this), _id);
            // withdrawal profit
            uint256 awaitingCollection = paymentc
                .loanLookup(_id)
                .awaitingCollection;
...
library PoolRateCalculator {
    ...
    * @dev function with the uniswap bonding curve logic but with the reserve ratio logic thrown in
    ...
    function _getAmountOut(uint amountIn, uint reserveIn, uint reserveOut, uint _reserveRatio, bool purchaseIn) private pure returns(uint){
        ...
        uint denominator = ((reserveIn) + ((ONE_HUNDRED_PERCENT - _reserveRatio) * amountInWithFee));
        return purchaseIn ? 
 ...
 * @dev TestToken will eventually be replaced with ERC-20
 */
interface IpoolInvestor is Iinvestor, IERC20 {
    ...
    * @dev function to exchange rToken back for stablecoins
     */
    function withdrawalPool(uint _amount) external;

    function getRewards(address _staker) external view returns(uint amountWithInterest);

    function claimAllPendingRewards(address _staker) external;
...
interface Iinvestor {
    ...
    function borrow(Structs.BorrowArgs calldata) external;

    /**
     * @dev collects an array of loan id's payments to this
     * @param _ids to collect on
     */
    function collect(uint256[] memory _ids) external;
...
library Loan{
    ...
    enum Status{UNISSUED, NEW, APPROVED, PAID, PAIDPART, CLOSED, PAIDLATE, LIQUIDATED}
    struct loan {
        Status status;
