...
contract AaveV2WrapV2Adapter {
    ...
    function getWrappedTokens(address[] memory underlyingTokens) external view returns (address[] memory aTokens, string[] memory symbols) {
        aTokens = new address[](underlyingTokens.length);
        symbols = new string[](underlyingTokens.length);

        for (uint256 i = 0; i < underlyingTokens.length; i++) {
            (aTokens[i], symbols[i]) = getWrappedToken(underlyingTokens[i]);
        }
    }

    function getUnderlyingTokens(address[] memory wrappedTokens) external view returns (address[] memory tokens, string[] memory symbols) {
        tokens = new address[](wrappedTokens.length);
        symbols = new string[](wrappedTokens.length);

        for (uint256 i = 0; i < wrappedTokens.length; i++) {
            (tokens[i], symbols[i]) = getUnderlyingToken(wrappedTokens[i]);
        }
    }
    ...
    function getWrappedToken(address underlyingToken) public view returns (address aToken, string memory symbol) {
    ...
    function getUnderlyingToken(address wrappedToken) public view returns (address token, string memory symbol) {
