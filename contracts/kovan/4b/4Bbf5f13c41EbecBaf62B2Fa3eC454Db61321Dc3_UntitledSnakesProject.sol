 ...
 * @notice An NFT collection of 5000 Snakes slithering on the Ethereum Blockchain.
 * All profits from mints by addresses registered on Proof Of Humanity are transferred
 * directly to the UBIBurner contract, which buys UBI tokens and then burns them.
...
contract UntitledSnakesProject is ERC721, Ownable {
    ...
    /// ProofOfHumanity contract address.
    /// On Mainnet: 0xC5E9dDebb09Cd64DfaCab4011A0D5cEDaf7c9BDb
    ...
    /**
     * @dev Instantiate contract and initialize variables.
     * @param _initialBaseURI string - initial base URI for all tokens.
     * @param _price uint256 - price to mint one token.
     * @param _pohAddress address - ProofOfHumanity contract address.
     * @param _ubiBurnerAddress address - UBIBurner contract address.
     */
    constructor(
     ...
     * If caller is a registered PoH address, transfer ether value to the UBIBurner contract.
     * @dev Requirements:
     * - The sale must be active.
     * - `_amount` must be greater than zero and less than or equal to {MAX_MINT_PER_TX}.
     * - The current supply plus `_amount` must be less than or equal to {MAX_SUPPLY}.
    ...
    function mint(uint256 _amount) public payable {
        ...
        require(supply + _amount <= MAX_SUPPLY, "Would exceed max supply");
        require(price * _amount <= msg.value, "Ether value sent is not enough");

        bool isRegistered = IProofOfHumanity(POH).isRegistered(_msgSender());
        if (isRegistered) {
            (bool success, ) = UBI_BURNER.call{value: msg.value}("");
     ...
     * Will be used to update the initial {baseURI} with the final IPFS {baseURI} after minting.
    ...
    /**
     * @notice Set the provenance hash of all tokens images.
     * @dev Can only be called by the contract owner.
     * Will be used to set the final provenance hash once it's calculated.
     * @param _provenanceHash string - provenance hash of all tokens images.
     */
     ...
     * @param _to address - account that will receive the token.
     */
 ...
 * This will allow us to verify if an address is registered in Proof of Humanity.
 */
