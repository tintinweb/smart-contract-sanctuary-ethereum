...
{   
    ...
    address public _ArtBlocksCoreContractAddressArtists = address(0);                                   // ArtBlocks Artist NFT Collection Contract Address
     ...
     */
    event LiveMintCompleteArtist(address indexed Redeemer, uint ArtistID, uint TicketID, uint MintID);
    ...
    function _LiveMint(uint TicketID) public onlyMinter whenNotPaused 
    {
        ...
        IERC721(_ArtBlocksCoreContractAddressCitizens).transferFrom(address(this), to, CitizenTokenID);
    ...
    /**
     * @dev Returns IPFS String Of Asset
     */
    function readIPFSString(uint ArtistID, uint TicketID, uint TokenID) public view returns(string memory)
    {
        if(MintedArtist[_CurrentCityIndex][ArtistID][TicketID] && !resolveArtistMint(ArtistID))
        {
            return IRandomEdition(resolveArtistContract(ArtistID)).tokenURI(TokenID);
        }
        else if(MintedArtist[_CurrentCityIndex][ArtistID][TicketID] && resolveArtistMint(ArtistID))
        {
            return IArtBlocks(resolveArtistContract(ArtistID)).tokenURI(TokenID);
        }
        else
        {
            return "TicketID Not Minted Or Is Not Random Edition";
        }
    }
