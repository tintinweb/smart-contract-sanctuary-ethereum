...
contract InceptionVaultFactory is IInceptionVaultFactory, BoringOwnable {
  ...
  IWETH public WETH;
  IERC20 public mimo;
  address public adminInceptionVaultBase;
  address public inceptionVaultsCoreBase;
  address public inceptionVaultsDataProviderBase;

  uint256 public inceptionVaultCount;
  uint16 public priceFeedCount;

  mapping(uint256 => InceptionVault) public inceptionVaults;
  mapping(uint16 => address) public priceFeeds;
  mapping(address => uint16) public priceFeedIds;
  ...
  constructor(
    ...
    require(_adminInceptionVaultBase != address(0));
    require(_inceptionVaultsCoreBase != address(0));
    require(_inceptionVaultsDataProviderBase != address(0));
    require(address(_addressProvider) != address(0));
  ...
  @dev contract inittilizations are done with low level calls for gas efficiency.
  ...
  */
  function cloneInceptionVault(
    ...
    require(address(_inceptionCollateral) != address(0));
    require(_inceptionVaultPriceFeed != address(0));
    ...
    if (priceFeedIds[_inceptionVaultPriceFeed] != 0) {
      require(address(_assetOracle) != address(0));
    ...
    uint256 newId = ++inceptionVaultCount;
  ...
  function addPriceFeed(address _address) external override onlyOwner {
    ...
    require(priceFeedIds[_address] == 0);
    uint16 newId = ++priceFeedCount;
    priceFeeds[newId] = _address;
    priceFeedIds[_address] = newId;
