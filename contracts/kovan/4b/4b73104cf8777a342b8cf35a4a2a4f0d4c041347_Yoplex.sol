...
contract Yoplex is BRC20, BRC20Burnable, Ownable, ChainlinkClient{
	...
	function viewUserDetails(address _user) public view returns(uint, uint, uint, uint) {
        return (users[_user].id, users[_user].ROI_taken_USD, users[_user].ROI_taken_BNB, users[_user].ROI_taken_time);
    }
