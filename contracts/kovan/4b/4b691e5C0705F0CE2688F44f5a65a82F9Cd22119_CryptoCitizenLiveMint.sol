...
{   
    ...
    /**
     * @dev Delegate Mints Artist Collection
     */
    function _DelegateMintArtist(uint ArtistID, uint TicketID) public onlyMinter whenNotPaused
    {

    }

    /**
     * @dev Delegate Mints CryptoCitizen
     */
    function _DelegateMintCitizen(uint TicketID) public onlyMinter whenNotPaused
    {
    ...
    function readNFTOwnedTokenIDs(
        ...
        {
            try NFT.ownerOf(TokenIDs[i]) 
            {
                if(NFT.ownerOf(TokenIDs[i]) == Wallet)
                {
                    OwnedIDs[counter] = TokenIDs[i];
                    counter++;
                }
            } catch { }
        }
    ...
    function readNFTsOwnedTokenIDs(
        ...
        for(uint x; x < NFTAddresses.length; x++)
        {
            ...
            uint[] memory temp = new uint[](TokenIDs.length);
            uint counter;
            for(uint y; y < TokenIDs.length; y++) 
            {
                try NFT.ownerOf(TokenIDs[y]) 
                {
                    if(NFT.ownerOf(TokenIDs[y]) == Wallet)
                    {
                        temp[counter] = TokenIDs[y];
                        counter++;   
                    }
                } catch { }
            }
