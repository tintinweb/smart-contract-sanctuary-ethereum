  ...
  Ownable
{
   ...
   */
  uint256 public constant UID =
    0xf50fd26e20f9246eddc285a279c52fb31d309ece04cfa1006506f82aeb885941;

  uint256 public constant maxSupply = 4896;
  uint256 private m_MaxMintPerAddress = 5;
  bool private m_IsMintable = false; // false
  mapping(address => uint256) user_mint_count;
  uint256 private m_MintPrice = 0.08 ether; // 0.1Eth
  ...
  function setMintEnabled(bool _enabled) external onlyRole(DEFAULT_ADMIN_ROLE) {
  ...
  function setPreMintPrice(uint256 _mintPrice)
    external
    onlyRole(DEFAULT_ADMIN_ROLE)
  {
    m_MintPrice = _mintPrice;
  }
  ...
  function encodePackedData(address _to, uint256 _tokenId)
    public
    view
    returns (bytes32)
  {
    return keccak256(abi.encodePacked(this, _to, address(this), _tokenId));
  }
    ...
    return
      ecrecover(
        toEthSignedMessageHash(
          keccak256(abi.encodePacked(this, _to, address(this), _tokenId))
        ),
        v,
        r,
        s
  ...
  function mint(
    ...
    require(
      user_mint_count[_to] < m_MaxMintPerAddress,
      "exceed max mint number"
    );
    require(msg.value >= m_MintPrice);
    user_mint_count[_to] = user_mint_count[_to] + 1;
    // Delegate to internal OpenZeppelin function
    _mint(_to, _tokenId);
  }
