...
// File: contracts\meta-cell\abstracts\ACellRepository.sol
...
abstract contract ACellRepository is ICellRepository, Multicall {
    ...
    function _removeMetaCell(address _user, uint256 _tokenId) internal {
        require(
            _getMetaCell(_tokenId).user != address(0),
            "Token not exists"
        );
        ...
        addressToMap[_user][_tokenId] = CellData.Cell({
            tokenId: 0,
            user: address(0),
...
// File: contracts\meta-cell\MetaCell.sol


pragma solidity ^0.8.3;
contract MetaCell is ERC721, ACellRepository, IMetaCellCreator, IAdmin, Ownable {
    using Counters for Counters.Counter;
    ...
    function create(address account)
        external override isAdmin returns (uint tokenId)
    {
        tokenIdCount.increment();
        _mint(account, tokenIdCount.current());
    ...
    function burn(uint tokenId) external {
        ...
        require(ownerOf(tokenId) == msg.sender, "You are not owner of MetaCell");
    ...
    function ownerOf(uint256 tokenId) public view virtual override(ERC721, ACellRepository) returns (address) {
        return ERC721.ownerOf(tokenId);
    }

    function addMetaCell(CellData.Cell memory _cell) external override isAdmin {
        _addMetaCell(_cell);
    }

    function removeMetaCell(uint256 _tokenId, address _owner) external override isAdmin {
