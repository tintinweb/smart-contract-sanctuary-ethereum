...
// File: my/new_cow/grade/CowAttack.sol
...
contract CowAttack is EIP712,ReentrancyGuard,AdminTransfer{
    address public signerAddress;

    function cowAttack(uint deadline,uint256 wolfId,uint256 cowId,bytes calldata signature) view external {
        require(block.timestamp <= deadline, "expired");
        require(check(signerAddress,hashData(deadline,wolfId,cowId),signature), "sign error");
    }

    constructor(address _signerAddress) EIP712("CowAttack", "1") {
    ...
    function hashData(uint deadline,uint256 wolfId,uint256 cowId) public view returns (bytes32) {
        return _hashTypedDataV4(keccak256(abi.encode(
            keccak256("CowExploreRequest(uint256 deadline,uint256 wolfId,uint256 cowId)"),
            deadline,
            wolfId,
            cowId
        )));
