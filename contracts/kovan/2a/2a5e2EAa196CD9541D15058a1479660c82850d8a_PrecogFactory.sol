...
contract PrecogFactory is IPrecogFactory {
    ...
    modifier onlyAdmin() {
        require(
            ...
            "PrecogFactory: Caller is not admin"
        );
        _;
    }
    ...
    modifier isExistingToken(address token) {
        require(
            ...
            "PrecogFactory: Token is not in trading"
        );
        _;
    }
    ...
    function _updateConvertTokenWhenAddLiquidityPool(address _tokenA, address _tokenB)
        internal
    {
    ...
    function _handleAddLiquidityPoolFromRemovedPool(
        ...
        if (precogStorage.checkIsRemoved(_token) == true) {
            ...
            if (block.timestamp >= _lastTradingCycle.endTime) {
                ...
                precogStorage.pushProfit(_token, 0);
                precogStorage.pushProfitForWhitelist(_token, 0);
                precogStorage.updateIsUpdateUnitTradingCycle(
                    _token,
    ...
    function addLiquidityPool(address token) external override onlyAdmin {
        require(
            !precogStorage.checkIsExistingToken(token),
            "PrecogFactory: Token is existed"
        );
        require(
        ...
        IPrecogCore.CycleConfiguration memory cycleConfig = _getCoreInstance()
        ...
        currentTradingCycle = _handleAddLiquidityPoolFromRemovedPool(
            token,
            cycleConfig,
            currentTradingCycle
        );
    ...
    function _handleRemoveLiquidityPool(address _token) internal {
        ...
        precogStorage.swapExistingTokensByIndex(
            indexOfToken,
            precogStorage.getExistingTokens().length - 1
        );
    ...
    function _updateTokenConvertWhenRemoveLiquidityPool(address _tokenA, address _tokenB)
        internal
    {
        ...
        isExistingToken(token)
    {
        ...
        require(
            IERC20(liquidityToken).totalSupply() == 0,
            "PrecogFactory: Token is still in trading"
        );
