...
contract Staking is AccessControl, ReentrancyGuard {
    ...
    function depositOrWLFromOwner(
        ...
        for (uint256 i = 0; i < len; i += 1) {
            wl = WhiteListInfo(allDeposits.length, enteredAt[i], amount[i], lockUp[i]);
            user = addresses[i];
            require(
                wl.amount > 0 &&
                    wl.lockUpWL <= 2 &&
                    user != address(0) &&
                    wl.enteredAt >= previousEnteredAt_,
                "2"
            );
        ...
        returns (uint256 inKON, uint256 inUSDT)
    {
        ...
        } else return(stake.varKoeff.unreceivedRewardKON, stake.varKoeff.unreceivedRewardUSDT);
    }
