...
contract DcrFactory is IDcrFactory {

    Parameters public override parameters;

    address public deployer;

    mapping(address => mapping(address => uint256)) private _poolCount;
    mapping(address => mapping(address => mapping(uint256 => DcrPool))) private _pools;
    ...
    event SetDeployer(address indexed deployer);
    ...
    function deployPool(
        ...
        int24 priceLower,
        int24 priceUpper
    ) external onlyDeployer returns (DcrPool dcrPool, uint256 poolIndex) {

        require(token0 < token1, "Wrong token order");

        poolIndex = poolCount(token0, token1);

        parameters = Parameters(token0, token1, controller, feeTo, swapFee, protocolFee, priceLower, priceUpper);

        dcrPool = new DcrPool{salt: keccak256(abi.encode(token0, token1, poolIndex))}();

        _poolCount[token0][token1]++;

        _pools[token0][token1][poolIndex] = dcrPool;
    ...
    function setDeployer(address _deployer) external onlyDeployer {
        deployer = _deployer;
        emit SetDeployer(_deployer);
    }

    function poolCount(address tokenA, address tokenB) public view returns (uint256) {
        if (tokenA < tokenB) return _poolCount[tokenA][tokenB];
        return _poolCount[tokenB][tokenB];
    }

    function pool(address tokenA, address tokenB, uint256 i) public view returns (DcrPool) {
        if (tokenA < tokenB) return _pools[tokenA][tokenB][i];
        return _pools[tokenB][tokenB][i];
    }
