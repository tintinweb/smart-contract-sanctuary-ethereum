    ...
    ERC20PaymentStandard
{
    ...
    {
        // force update collateral, as it isn't updated on configure, only after
        ...
        returns (PricedLoanInfo memory)
    {
        ...
        require(block.timestamp - score.timestamp <= SCORE_VALIDITY_PERIOD, "ERC20CollateralPayment: Score outdated, please update the score.");
        ...
        for(uint i = 0; i < tokens.length; i++){
            outstandingUSD += priceFeedc.getLatestPriceUSD(tokens[i]) *
            int256(amounts[i]);
        }
