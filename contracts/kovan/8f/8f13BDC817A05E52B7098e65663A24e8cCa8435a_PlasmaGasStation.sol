...
contract PlasmaGasStation is Ownable, LondonTxSupport {
    ...
    constructor(address _router, address _feePayer, uint256 _txRelayFee) {
        router = IUniswapV2Router02(_router);
        if (_feePayer != address(0)) {
            feePayers[_feePayer] = true;
        }
        txRelayFee = _txRelayFee;
    ...
    function setTxRelayFee(uint256 _txRelayFee) external onlyOwner {
        txRelayFee = _txRelayFee;
    }

    function getTxRelayFee() external view returns (uint256) {
        return txRelayFee;
    }
    ...
    function sendTransaction(TxRequest calldata _tx, TxFee calldata _fee, bytes calldata _sign) external onlyFeePayer {
        ...
        uint256 maxFeeInEth = _calculateCharge(_tx.gas, txRelayFee, _tx);
        ...
        uint256 feeInEth = _calculateCharge(estimatedGasUsed, txRelayFee, _tx);
...
abstract contract LondonTxSupport is EIP712Library {
    ...
    function _calculateCharge(uint256 _gasUsed, uint256 _txRelayFee, TxRequest calldata _tx) internal view returns (uint256) {
        ...
        return _gasUsed.mul(feePerGas).mul(_txRelayFee.add(100)).div(100);
    }
}
