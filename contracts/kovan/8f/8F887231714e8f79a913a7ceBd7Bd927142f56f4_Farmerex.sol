...
contract Farmerex is Ownable {
	...
	mapping(uint256=>uint256) public totalDailyLpUsdt;
	mapping(address=>mapping(uint256 =>uint256)) public userDailyProfit;
	...
	function invest(address referrer,uint256 usdt_ ) public  {
		...
		totalDailyLpUsdt[getDays()]  += usdt_.mul(2);
		...
		for(uint256 i = 0;i<=9;i++){
			if (upline != address(0)) {
					if(getReferLeval(upline)> (i)) {
						...
						userDailyProfit[upline][getDays()] += amount;
	...
	function getUserLpDrawInfo(address userAddress_) public view returns(uint256 todayLp,uint256 totalLp,uint256 userTodayProfit,uint256 userTotalProfit,uint256 userDeposit){
		todayLp = totalDailyLpUsdt[getDays()];
		totalLp = totalInvested;
		userTodayProfit =  userDailyProfit[userAddress_][getDays()];
		userTotalProfit = userWithdrawn[userAddress_].add(getUserAvailable(userAddress_));
		userDeposit =  getUserTotalDeposits(userAddress_);
	}
