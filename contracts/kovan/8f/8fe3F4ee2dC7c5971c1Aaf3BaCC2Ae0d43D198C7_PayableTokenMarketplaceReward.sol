...
interface IMarketplace {
    ...
    struct Order {
        ...
        address currency;
        // reward (in wei) for the published item
        uint256 reward;
    }
...
contract PayableTokenMarketplaceReward is Pausable, FeeManager, IMarketplace, ERC721Holder, ReentrancyGuard {
    ...
    constructor(address _rewardToken) public {
        require(_rewardToken.isContract(),"The accepted token address must be a deployed contract");
    ...
    /**
     * @dev Set reward token as reward for nft. Can only be called by owner
     * @param _rewardToken - ERC20 contract address
     */
    function setReward(address _rewardToken) external onlyOwner {
        require(_rewardToken.isContract(),"The accepted rewardToken address must be a deployed contract");
         rewardToken = IERC20(_rewardToken);
    }
     ...
     * @param _currency - Duration of the order (in hours)
     * @param _reward - Duration of the order (in hours)
     */
    function createOrder(address _nftAddress, uint256 _assetId, uint256 _priceInWei, uint256 _expiresAt, address _currency, uint256 _reward) external whenNotPaused {
        _createOrder(_nftAddress, _assetId, _priceInWei, _expiresAt, _currency, _reward);
     ...
     * @param _assetId - ID of the published NFT
     */
    function updateOrder(address _nftAddress, uint256 _assetId, uint256 _priceInWei, uint256 _expiresAt, uint256 _reward)
        ...
        order.reward = _reward;


        emit OrderUpdated(order.id, _priceInWei, _expiresAt, _reward);
    }
    ...
    function safeExecuteOrder(address _nftAddress, uint256 _assetId, uint256 _priceInWei) external payable whenNotPaused {
        ...
        order.currency == MARKETPLACE_ETHER ? 
            require(order.price == msg.value, "Marketplace: invalid price")
            :
            require(order.price == _priceInWei, "Marketplace: invalid price2");
        ...
        if (cutPerMillion > 0) {
            ...
            :
            saleShareAmount = (order.price ).mul(cutPerMillion).div(1e6);
        ...
        _executeOrder(order.id, msg.sender, _nftAddress, _assetId, order.price , order.reward);
    }
     ...
     */
    function acceptBid(address _nftAddress, uint256 _assetId, uint256 _priceInWei, uint256 _reward) external payable whenNotPaused {
       ...
       // Calculate sale share
            ...
            saleShareAmount = (msg.value).mul(cutPerMillion).div(1e6)
            :
            saleShareAmount = (bid.price).mul(cutPerMillion).div(1e6);

            // Transfer share amount for marketplace Owner
            order.currency == MARKETPLACE_ETHER ? 
                payable(owner()).transfer(saleShareAmount)
                :
                 IERC20(order.currency).safeTransferFrom(msg.sender, order.seller, bid.price.sub(saleShareAmount));
        }
        ...
        _executeOrder(order.id, bid.bidder, _nftAddress, _assetId, _priceInWei, _reward);
    }
     ...
     */
    function _executeOrder(bytes32 _orderId, address _buyer, address _nftAddress, uint256 _assetId, uint256 _priceInWei, uint256 _reward) internal {
         Order memory order = _getValidOrder(_nftAddress, _assetId);  
        // Transfer NFT asset
        IERC721(_nftAddress).safeTransferFrom(address(this), _buyer, _assetId);

        if (order.reward !=0){ 
          rewardToken.safeTransfer(_buyer, order.reward);
        }
        // remove order
        delete orderByAssetId[_nftAddress][_assetId];
        // Notify ..
        emit OrderSuccessful(_orderId, _buyer, _priceInWei, _reward);
    }
    /**
     * @dev Creates a new order
     * @param _nftAddress - Non fungible registry address
     ...
     */
    function _createOrder(address _nftAddress, uint256 _assetId, uint256 _priceInWei, uint256 _expiresAt, address _currency, uint256 _reward) internal {
        // Check nft registry
        ...
        require(_priceInWei > 0, "Marketplace: Price should be bigger than 0");
            // (_reward >= 0, "Marketplace: reward should be bigger than 0");
        ...
        rewardToken.safeTransferFrom(assetOwner, address(this), _reward);
        ...
        emit OrderCreated(orderId, assetOwner, _nftAddress, _assetId, _priceInWei, _expiresAt, _currency, _reward);
    }
