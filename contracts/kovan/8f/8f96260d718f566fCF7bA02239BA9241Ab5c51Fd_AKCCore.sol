...
contract AKCCore is Ownable, AccessControl, IERC721Receiver {
    ...
    // uint256 private totalEarningsTaxAmount = 0;
    ...
    // function transferTaxAmountToOwner(address ownerAddress) internal returns(uint256) {
    //     if(totalEarningsTaxAmount > 0) {
    //         // akcCore.releaseFunds(bonus, affiliate);
    //     }
    // }
    ...
    function claimRewardOfTribeByIndex(address tribeOwner, uint256 tribeIndex) 
        ...
        returns(uint256) {
            ...
            // uint256 rewardPerDay =  reward / intervalDays;
            // uint256 taxPercentage = rewardPerDay <= 9 ? 1: 3;

            // totalEarningsTaxAmount += (rewardPerDay * tribeSpec.price) * (taxPercentage / 100);

            // Need to check if taxPercentage need to reduce from user earnings?
