    ...
    OwnableUpgradeable
{
    ...
    function createFullTrack(
        ...
        fullTrackIdToFullTrack[_fullTrackIds.current()] = FullTrack(
            supply,
            0,
            startTime,
            price,
        ...
        whenNotPaused
    {
        ...
        address signer = _verify(
            signature,
            fullTrackId,
            fullTrackIdToFullTrack[fullTrackId].sold + 1
        );
        require(
            hasRole(MINTER_ROLE, signer),
            "Artist: Signature invalid or unauthorized"
        );
        for (uint256 i = 0; i < royaltiesReceivers[fullTrackId].length; i++) {
            uint256 royalty = royalties[fullTrackId][
                royaltiesReceivers[fullTrackId][i]
            ];
            (bool success, ) = payable(royaltiesReceivers[fullTrackId][i]).call{
        ...
        bytes32 digest = keccak256(
            abi.encodePacked(
                ...
                keccak256(
                    abi.encode(
                        keccak256(
                            "SignedData(string name,uint256 fullTrackId,uint256 copyId)"
                        ),
