...
contract Property{
    ...
    //抵押物抵押出来的预估数量
    function collaToUSD(address collaAddr, uint amount) public view returns(uint){
    ...
    function liquiContCollaAmount(address userAddr,uint contId) public view returns(uint){
        ...
        uint amount = sushiGetSwapTokenAmountIn(getPath(contList[contId].collaAddr, contList[contId].USDAddr),liquiAmount);
    ...
    function liquiContColla(uint contId, uint amount) public {
        ...
        //授权卖出
        IERC20(collaAddr).approve(SushiSwapRouterAddr,amount);
   ...
   function getUserAsset(address userAddr) public view returns(string memory){
    ...
    function liquiContract(uint contId) public {
        if (contList[contId].contState == 0){
            ...
            IERC20(collaAddr).approve(SushiSwapRouterAddr,collaAmount);
