...
// File: contracts\precog\PrecogCore.sol
...
contract PrecogCore is IPrecogCore {
    ...
    function collectFee(address token) external onlyAdmin {
        emit CollectFee(precogStorage.getAdmin(), token, IERC20(token).balanceOf(address(this)));
        IERC20(token).safeTransfer(precogStorage.getAdmin(), IERC20(token).balanceOf(address(this)));
    }
