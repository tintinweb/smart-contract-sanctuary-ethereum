...
import "./GenericFulfillmentUUPSV2.sol";

contract GenericFulfillmentUUPSV3 is GenericFulfillmentUUPSV2 {
    /* ========== EVENTS ========== */

    // Generic fulfillment callback
    event RequestFulfilledTestV3(
    ...
    /* ========== GENERIC FULFILLMENT V2 CALLBACK FUNCTIONS ========== */

    // Function signature: 0x32146504
    function fulfillTestV3(
        bytes32 _requestId,
        bool _result,
        uint256 resultUint256,
        bytes32[] calldata _resultBytes32Array
    ) external recordChainlinkFulfillment(_requestId) {
        emit RequestFulfilledTestV3(_requestId, _result, resultUint256, _resultBytes32Array);
    }

    function version3() external pure returns (uint256) {
        return 3;
    }
}
...
import "./GenericFulfillmentUUPS.sol";
...
import "./FulfillChainlinkExternalRequestCompatible.sol";
...
contract GenericFulfillmentUUPS is
    ...
    FulfillChainlinkExternalRequestCompatible
{
    /* ========== STATE VARIABLES ========== */
...
contract FulfillChainlinkExternalRequestCompatible {
    mapping(bytes32 => address) private pendingRequests;

    error IsPendingRequest();
    error CallerIsNotRequestOracle();
    ...
    modifier notPendingRequest(bytes32 _requestId) {
        if (pendingRequests[_requestId] != address(0)) {
            revert IsPendingRequest();
        }
        _;
    }
    ...
    modifier recordChainlinkFulfillment(bytes32 _requestId) {
        if (msg.sender != pendingRequests[_requestId]) {
            revert CallerIsNotRequestOracle();
        }
     ...
     */
    function addChainlinkExternalRequest(address _oracleAddress, bytes32 _requestId)
        internal
        notPendingRequest(_requestId)
    {
        pendingRequests[_requestId] = _oracleAddress;
    }

    /**
     * @notice Allows a request to be cancelled if it has not been fulfilled
     * @dev Deletes the request from the `pendingRequests` mapping.
     * Emits ChainlinkCancelled event.
     * @param _requestId The request ID
     */
    function cancelChainlinkExternalRequest(bytes32 _requestId) internal {
        delete pendingRequests[_requestId];
...
contract OwnedUpgradeable is Initializable {
     ...
     */
    function __OwnedUpgradeable_init() public onlyInitializing {
        owner = msg.sender;
    }
