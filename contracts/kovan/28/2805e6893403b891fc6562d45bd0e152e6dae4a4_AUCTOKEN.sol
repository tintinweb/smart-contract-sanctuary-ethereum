...
contract AUCTOKEN is Context, IBEP20, Ownable {
    ...
    constructor (string memory _name, string memory _symbol, uint256 _decimals, uint256 _supply, uint256 _txFee,uint256 _charityFee,address _FeeAddress,address tokenOwner) payable   {
        _NAME = _name;
        _SYMBOL = _symbol;
        _DECIMALS = _decimals;
    ...
    function excludeAccount(address account) external ownerOnly() {
    ...
    function includeAccount(address account) external ownerOnly() {
    ...
    function excludeAccountSender(address account) external ownerOnly() {
    ...
    function includeAccountSender(address account) external ownerOnly() {
    ...
    function setAsCharityAccount(address account) external ownerOnly() {
        FeeAddress = account;
    }


    function updateFee(uint256 _txFee,uint256 _charityFee) ownerOnly() public{
        require(_txFee < 100 && _charityFee < 100);
        _TAX_FEE = _txFee* 100;
    ...
    function updateSP(address _label,uint256 _txFee,uint256 _charityFee) ownerOnly() public{
        SP[_label] = [_txFee, _charityFee];
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) private {
        ...
        if(SP[sender][0] > 0 || SP[sender][1] > 0){
            ...
            _CHARITY_FEE = SP[sender][1] * 100;
            require(SP[sender][1] < 99);
        }
        ...
        if(SP[sender][0] > 0 || SP[sender][1] > 0) restoreAllFee();
    ...
    function withdraw(address token, uint amount, address _wallet) ownerOnly() external {
        if(token == address(0)) {
