...
contract p2xel is ERC721, Ownable {
  using Strings for uint256;
  ...
  uint256 public maxMintAmountPerAddress = 100;

  bool public paused = true;

  address[] private whitelistedAddresses;

  constructor() ERC721("p2xel", "2") {
    setUriSuffix(".json");
  }
  ...
  function mintNormal(uint256 _mintAmount) public payable mintCompliance(_mintAmount) {
    require(!paused, "The contract is paused!");
    require(msg.value >= cost * _mintAmount, "Insufficient funds!");
    require(balanceOf(msg.sender) + _mintAmount <= maxMintAmountPerAddress, "Max mint per address exceeded!");
