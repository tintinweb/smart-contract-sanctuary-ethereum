...
contract CoreBank is Ownable {
    ...
    // event - user
    event DepositMode(address indexed accountAddress,uint256 amount);
    event WithdrawMode(address indexed accountAddress,uint256 amount);
    // event - user
    event SystemDepositMode(address indexed admin,uint256 amount);
    event SystemWithdrawMode(address indexed admin,uint256 amount);
    event PayDividendMode(address indexed admin,uint256 totalInterest);
    ...
    function systemWithdraw(uint256 withdrawAmount) public onlyOwner returns(uint256) {
        ...
        //event
        emit SystemWithdrawMode(msg.sender,withdrawAmount);
    ...
    function payDividendsPerYear() public payable onlyOwner{
        ...
        require(msg.value == totalInterest,"No enough to pay");

          //event
        emit PayDividendMode(msg.sender,totalInterest);
    }
}