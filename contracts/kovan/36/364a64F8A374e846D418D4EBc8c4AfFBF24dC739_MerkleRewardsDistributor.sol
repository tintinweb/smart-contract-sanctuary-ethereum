...
contract MerkleRewardsDistributor is Ownable, ReentrancyGuard {
    ...
    function claim(
        ...
        require(
            ...
            isInState(userAddress, amountToClaim, merkleProof),
    ...
    function isInState(
        ...
        return
            _verify(_leaf(userAddress, amountToClaim), merkleProof);
    }
    ...
    function _leaf(
        ...
        console.logBytes32(
            keccak256(abi.encodePacked(account, amountToClaim))
        );
        console.log("On-chain encodePacked");
        console.logBytes(abi.encodePacked(account, amountToClaim));
        // bytes memory data = ConvertUtils.addressToBytesString(
        //     address(account)
        // );
        string memory data = Strings.toHexString(uint256(uint160(account)), 20);
        //return keccak256(abi.encodePacked(account, amountToClaim));
        return keccak256(abi.encodePacked(data, ":", Strings.toHexString(amountToClaim)));
    }
