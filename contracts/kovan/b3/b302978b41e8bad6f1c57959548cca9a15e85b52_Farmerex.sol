...
contract Farmerex is Ownable {
	...
	uint256 constant public INVEST_MIN_AMOUNT = 300e18;
	...
	uint256 public effecTotalUsers;
	...
	event RefBonus(address indexed referrer, address  preUser, uint256  level, uint256 amount);
	event ExchangeEtp(address indexed user,uint256 usdtIncome,uint256 etpIncome);
	event Regist(address indexed referrer,address usder);
	...
	//==========update==================
	//1:aprove ETPAssetsManager for address(this) of  etp token
	...
	function regist(address referrer) public {
		require(!isContract(referrer),"referrer isn't legitimate") ;
		...
		if ( user.referrer == address(0) && referrer != msg.sender) {
			...
			for (uint256 i = 0; i < 10; i++) {
				if (upline != address(0)) {				
					users[upline].invalidReferCount = users[upline].invalidReferCount+1;
			...
			emit Regist(referrer,msg.sender);
		}

	}

	function invest(uint256 usdt_ )public{
		
		address refer =  getUserReferrer(msg.sender);
		if(refer != address(0)){
			invest(refer,usdt_);
			totalUsers += 1;
			
		}else{
			refer = getDaoOrIeoRefer(msg.sender);
			if(refer != address(0)){
				invest(refer,usdt_);
			}else{
				totalUsers += 1;
				invest(0x98d54875E45eACa500300FAd1CF81770e2C9678f,usdt_);
			}
		}
		effecTotalUsers+=1;
	...
	function invest(address referrer,uint256 usdt_ ) public  {
		require(!isContract(referrer),"referrer isn't legitimate") ;
		require(usdt_ >= INVEST_MIN_AMOUNT,"usdt  value must emt 300e18");	
		...
		if (user.deposits.length == 0) {
			...
			effecTotalUsers = effecTotalUsers.add(1);

			users[user.referrer].directNumber ++;
		}
	...
	function withdraw() public {
		...
		uint256 burnDividends = totalAmount;
		uint j = 0;
		...
		for(uint256 i = 0;i<=9;i++){
			if (upline != address(0)) {
				if(getReferLeval(upline)> (j)) {
					uint256 preDeposit_ =  getUserTotalDeposits(preUser);
					uint256 currentDeposit =  getUserTotalDeposits(upline);
					
					if(preDeposit_ != 0){
						preDeposit = preDeposit_;
					}
					...
					uint256 amount = burnDividends.mul(REFERRAL_PERCENTS[j]).div(100);
					users[upline].bonus = users[upline].bonus.add(amount); 
					emit RefBonus(upline, preUser, j, amount);						
				}		
				preUser = upline;
				upline = users[upline].referrer;			
			} else break;
