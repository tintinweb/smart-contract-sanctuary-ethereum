...
contract Flashloaner is FlashLoanReceiverBase {
 ...
 event DaiinExecution (
        ...
        external override
    {
        ...
        address usdtAddress = 0x07de306FF27a2B630B1141956844eB1552B956B5;
        address routerAddress = 0x1b02dA8Cb0d097eB8D57A175b88c7D8b47997506;
        address factoryAddress = 0xc35DADB65012eC5796536bD9864eD8773aBc74C4;
       ...
       uint amountOutMin = routerInstance.getAmountOut(daiBalance- 1 ether-100000, daiBalance- 1 ether -10000000, daiBalance- 1 ether -1000000);
    //    address getPair = factoryInstance.getPair(daiAddress, usdtAddress);
       address[] memory path = new address[](2);
       path[0] =daiAddress;
       path[1] =usdtAddress;

       routerInstance.swapExactTokensForTokens(daiBalance- 1 ether-100000 , amountOutMin, path, address(this), block.timestamp+1 hours);
// ----------------------------------------------------------------
       ...
       uint amountOutMin1 = routerInstance.getAmountOut(usdtBalance, usdtBalance-100, usdtBalance-10);
    //    address getPair = factoryInstance.getPair(usdtAddress, daiAddress);
         address[] memory path1 = new address[](2);
       path[0] =usdtAddress;
       path[1] =daiAddress;
       routerInstance.swapExactTokensForTokens(usdtBalance , amountOutMin1,path1, address(this), block.timestamp+1 hours);

        uint daiBalance1 = daiToken.balanceOf(address(this));
         emit DaiinExecution(daiBalance1);

// ----------------------------------------------------------------
        // Time to transfer the funds back
        uint totalDebt = fixstackdeepamount.add(fixstackdeepfee);
