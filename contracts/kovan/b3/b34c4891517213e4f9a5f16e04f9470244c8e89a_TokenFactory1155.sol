...
contract TokenFactory1155 is TokenFactory1155Storage, ITokenFactory1155{
    using SafeMath for uint256;

    address public immutable _owner;

    constructor() {
    // function _TF1155_init(address _biCollector) initializer {
        _owner = msg.sender;
    }
    ...
    modifier onlyOwnerOrSuperFactory() {
        require(_owner == msg.sender || superFactoryAddress == msg.sender, "Not a super factory");
        _;
    }

    function setNewSuperFactory(address _superFactoryAddress) external override onlyOwnerOrSuperFactory {
    ...
    */
    function deployToken1155(string memory _baseTokenURI,
            address _creatorAddress,
            address _creatorCollectorAddress,
            uint256 _biPercent,
            uint256 _creatorPercent) public override onlyOwnerOrSuperFactory returns (address) {
    ...
    */
    function deployTokenWL(address _tokenAddress, address _creator) public override onlyOwnerOrSuperFactory returns (address) {
        require(tokenDeployed[_tokenAddress], "Tank index not allowed");
    ...
    */
    function deployTokenWithWL(string memory _baseTokenURI,
            address _creatorAddress,
            address _creatorCollectorAddress,
            uint256 _biPercent,
            uint256 _creatorPercent) external override onlyOwnerOrSuperFactory returns (address, address) {
        address newTokenAddress = deployToken1155(_baseTokenURI, _creatorAddress, _creatorCollectorAddress, _biPercent, _creatorPercent);
...
interface ITokenFactory1155 {
    ...
    function deployToken1155(string calldata _baseTokenURI,
            address _creatorAddress,
            address _creatorCollectorAddress,
            uint256 _biPercent,
            uint256 _creatorPercent) external returns (address);
    function deployTokenWithWL(string calldata _baseTokenURI,
            address _creatorAddress,
            address _creatorCollectorAddress,
            uint256 _biPercent,
...
contract TokenWhitelist is TokenWLStorage, Ownable, ITokenWhitelist {
    using SafeMath for uint256;
...
contract Token1155Storage {
    ...
    // id => creator collectors
    mapping (uint256 => address) public creatorCollectors;
    // creator => whitelist
    mapping (address => address) public creatorWhitelists;

    uint256 public creatorsCounter;
...
contract Token1155 is Token1155Storage, Ownable, AccessControlEnumerable, Pausable, ERC1155, ERC2981, IToken1155/*, ContextMixin*/ {

    constructor(string memory _uri,
            address _creatorAddress,
            address _creatorCollectorAddress,
            uint256 _biPercent,
            uint256 _creatorPercent) ERC1155(_uri) {
     ...
     */
    function supportsInterface(bytes4 interfaceId) public view override(AccessControlEnumerable, ERC1155, ERC2981) returns (bool) {
        return (
    ...
    function getTokenWLAddress() external view override returns (address) {
    ...
    function mint(address to,
            uint256 id,
            uint256 amount,
            bytes memory data) external whenNotPaused {
        require(tokenWLAddress == address(0) || (tokenWLAddress != address(0) && ITokenWhitelist(tokenWLAddress).isWhitelisted(to)), "Token1155: recipient not whitelisted");
        require(hasRole(MINTER_ROLE, _msgSender()), "Token1155: must have minter role to mint");
    ...
    function tokenOfOwnerById(address owner, uint256 id) external view override returns (uint256) {
        // require(index < ERC1155.balanceOf(owner, id), "ERC721Enumerable: owner index out of bounds");
    ...
    function setURI(string calldata _uri, uint256 _id) external creatorOnly {
        // string(abi.encodePacked(baseMetadataURI, _id, ".json"));
        // _setURI(_uri);
        emit URI(_uri, _id);
    }
