...
contract AKCCore is Ownable, AccessControl, IERC721Receiver {
    ...
    function addBoostsToReward(
        ...
        view
        returns(uint256) {
            /// @dev Staked Kong BOOST
            uint256 kongBoost;
            if (userToAKC[tribeOwner][spec] != 0) {
                kongBoost = reward * akcStakeBoost / 100;
            }

            /// @dev Total tribes BOOST
            uint256 totalTribesBoost;
            if (totalTribes >= 100) {
                totalTribesBoost = reward * 35 / 100;
            } else if (totalTribes >= 60) {
                totalTribesBoost = reward * 25 / 100;
            } else if (totalTribes >= 40) {
                totalTribesBoost = reward * 22 / 100;
            } else if (totalTribes >= 20) {
                totalTribesBoost = reward * 16 / 100;
            } else if (totalTribes >= 12) {
                totalTribesBoost = reward * 10 / 100;
            } else if (totalTribes >= 8) {
                totalTribesBoost = reward * 8 / 100;
            } else if (totalTribes >= 4) {
                totalTribesBoost = reward * 4 / 100;
            } else if (totalTribes >= 2) {
                totalTribesBoost = reward * 2 / 100;
            }
            ...
            return reward + kongBoost + totalTribesBoost + timeBoost;
        }
    ...
    function claimRewardOfTribeByIndex(address tribeOwner, uint256 tribeIndex) 
        ...
        returns(uint256) {
            uint256 totalTribes = userToTribes[tribeOwner].length;
            require(totalTribes > tribeIndex, "TRIBE INDEX OUT OF BOUNDS");
            ...
            uint256 rewardInterval = (block.timestamp - lastClaimedTimeStamp);
            require(rewardInterval > claimDelay, "CLAIM TOO FAST");

            tribe |= uint256(uint32(block.timestamp)) << 32;
            uint256 reward = tribeSpec.rps * rewardInterval / 86400;

            if (addBoosts)
                reward = addBoostsToReward(tribeOwner, spec, reward, rewardInterval, totalTribes);
