...
contract aggregator{
    ...
    function swapAggregatedRoute(
        ...
        external payable{
            //console.log("sender eth balanace before",msg.sender.balance);
            ...
            expectedAmountOut = amountInOut[1];
            ...
            { 
                if(address(singleswap.assetIn) == ETH){
                    ...
                    balOutput= Ibal(balancerVault).swap{value: amountIn}(singleswap, funds, limit, deadline);    
                }
            ...
            {            
                if(address(assets[0]) == ETH){
                    ...
                    Ibal(balancerVault).batchSwap { value: amountIn }(kind, swaps, assets, funds, limits, deadline);
                    amountIn=IERC20(address(assets[assets.length-1])).balanceOf(address(this));
                }
                    ...
                    if(address(assets[assets.length-1])==ETH){
        ...
        transfer(amountInOut[0],expectedAmountOut,tokenIn,tokenOut,msg.sender);
        optionIndex = 0;
        pathindex = 0;
        amountOut=0;
    ...
    function transfer(uint amountIn0, uint expectedAmountOut0, address TokenIn, address TokenOut, address msgSender) internal{
        uint amountReceived = TokenOut==ETH?address(this).balance:IERC20(TokenOut).balanceOf(address(this));
            if(amountReceived<expectedAmountOut0){
        ...
        console.log(TokenIn, TokenOut, amountIn0, amountReceived);
