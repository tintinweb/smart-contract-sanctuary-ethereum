...
contract Pair {
    ...
    function addNewLiquidity(address _tokenA, address _tokenB, uint _amountA, uint _amountB)   external returns(uint256, uint256, uint256){
        ...
        IERC20(_tokenA).approve(router , _amountA*10);
        IERC20(_tokenB).approve(router , _amountB*10);


        (uint256 amountA, uint256 amountB, uint256 liquidity) = IUniswapV2Router02(router).addLiquidity(_tokenA, _tokenB, _amountA, _amountB, _amountA, _amountB, msg.sender, block.timestamp+100);
