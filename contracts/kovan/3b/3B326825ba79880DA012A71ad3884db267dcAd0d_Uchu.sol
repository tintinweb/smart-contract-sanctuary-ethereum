...
contract Uchu is Initializable {
    ...
    **/
    function deposit(address user, uint8 proId) public payable returns (bool) {
        if (productId > 0) {
            ...
            createDopositPosition(proId, OPEN, user, msg.value,  block.timestamp, 0);
            return true;
        } else {
    ...
    **/
    function createDopositPosition(uint8 proId, uint8 status, address user, uint256 amount, uint256 depositTime, uint256 withdrawTime) private {
    ...
    /**
    *  modify record by current lender's positionIndex
    **/
    function createWitdrawPosition(address user, uint16 posIndex) private {
        positions[user][posIndex].status = CLOSED;
        positions[user][posIndex].closeTime = block.timestamp;
    }
    ...
    **/
    ...
    function withdraw(address user, uint256 amount, uint8 proId, uint16 posIndex) public returns (bool) {
        Product memory pro = getProduct(proId);
        if (balanceOf[user][pro.name] >= amount) {
            allowance[pro.name] -= amount;
            balanceOf[user][pro.name] -= amount;
            createWitdrawPosition(user, posIndex);
