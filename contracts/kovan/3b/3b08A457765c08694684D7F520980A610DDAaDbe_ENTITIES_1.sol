...
// File: contracts/EntitiesCollection.sol
...
contract ENTITIES_1 is ERC721, ReentrancyGuard{
    ...
    uint256 public ownerBPS = 750;
    uint256 public managerBPS = 9250;

    //whitelist
    bytes32 public ogMerkleRoot;

    //addresses
    address public owner = 0x92458488e7Edd18cf5b7a9Ad75b5a623314d2c07;
     ...
     */
    function teamMint(uint256 amount) public onlyManager {
        require(
     ...
     */
    function setMintPrice(uint256 val) external onlyManager {
        price = val;
    }
    /**
     * @dev Sets a new mints per tx maximum (Max: 100)
     */
    function setMaxMintTx(uint256 val) external onlyManager {
        require(
            val < 100 + 1,
            "Maximum value is 100 mints per tx"
        );
        MAX_MINT = val;
    }
     ...
     */
    function toggleMintState() public onlyManager {
     ...
     */
    function toggleMintMintPass() public onlyManager {
     ...
     */
    function toggleMintWhiteListState() public onlyManager {
        mintWhiteListActive = !mintWhiteListActive;
    }

    function setProvenanceHash(string memory provenanceHash) public onlyManager {
        PROVENANCE = provenanceHash;
    }

    function setOgMerkleRoot(bytes32 _ogMerkleRoot) external onlyManager {
        ogMerkleRoot = _ogMerkleRoot;
    }
    ...
    /**
     * @dev Sets the opensea contractURI
     */
    ...
    function withdrawOwner() public payable onlyOwner {
        uint256 sale1 = (address(this).balance * ownerBPS) / 10000;
        uint256 sale2 = (address(this).balance * managerBPS) / 10000;
        require(payable(owner).send(sale1));
        require(payable(manager).send(sale2));
    }
    function withdrawManager() public payable onlyManager {
        uint256 sale1 = (address(this).balance * ownerBPS) / 10000;
        uint256 sale2 = (address(this).balance * managerBPS) / 10000;
        require(payable(owner).send(sale1));
        require(payable(manager).send(sale2));
