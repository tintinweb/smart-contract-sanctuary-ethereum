...
contract TransferAccount is FlashLoanReceiverBaseV2, Withdrawable {
     ...
     * @param params The byte array containing, in this case, its contains the params of transferAccount
     */
    function executeOperation(
        ...
        (address _recipient, DebtTokenBalance[] memory _DebtTokenBalance, aTokenBalance[] memory _aTokenBalance) = abi.decode(params, (address, DebtTokenBalance[], aTokenBalance[]));
        ...
        for(uint i=0; i<_DebtTokenBalance.length; i++) {
            ...
            if (_DebtTokenBalance[i].stableDebtTokenBalance != 0) {
                ...
                LENDING_POOL.repay(_DebtTokenBalance[i].tokenAddress, _DebtTokenBalance[i].stableDebtTokenBalance, 1, 0x2c915A8E2fae1241a79235259FFd960c47c4A51A);
            }
            // Repay VariableDebt
            if (_DebtTokenBalance[i].variableDebtTokenBalance != 0) {
                ...
                LENDING_POOL.repay(_DebtTokenBalance[i].tokenAddress, _DebtTokenBalance[i].variableDebtTokenBalance, 2, 0x2c915A8E2fae1241a79235259FFd960c47c4A51A);
            }
        }

        // Transfer all aTokens from Account 1 to Account 2
        for (uint i=0; i<_aTokenBalance.length; i++) {
            IERC20(_aTokenBalance[i].aTokenAddress).transferFrom(0x2c915A8E2fae1241a79235259FFd960c47c4A51A, _recipient, _aTokenBalance[i].aTokenBalance);
        }
    ...
    function transferAccount(
        ...
        bytes memory params = abi.encode(_recipientAccount, _DebtTokenBalance, _aTokenBalance);
