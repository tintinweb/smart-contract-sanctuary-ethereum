...
// get eth value
    ...
    // For each token address, we need to keep track of the amount of that token that each staker has staked
    ...
    function getUserSingleTokenValue(address _user, address _token) public
    view 
    returns (uint256) {
        if (uniqueTokensStaked[_user] <= 0) {
            return 0;
        }
        // price of the token * staking balance of token of user
