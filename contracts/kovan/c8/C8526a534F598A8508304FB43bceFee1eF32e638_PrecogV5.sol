...
contract PrecogV5 is IPrecogV5 {
    ...
    function getInvestmentOfByIndex(address token, address account, uint index) external view override returns (uint amount, uint unit, uint48 timestamp, uint16 idChanged) {
        Investment memory investmentOf = investmentsOf[token][account][index];
        return (investmentOf.amount, investmentOf.unit, investmentOf.timestamp, investmentOf.idChanged);
    }

    function getTradingCycleByIndex(address token, uint index) external view override returns (uint16 id, uint48 startTime, uint48 endTime) {
        Cycle memory tradingCycle = tradingCycles[token][index];
        return (tradingCycle.id, tradingCycle.startTime, tradingCycle.endTime);
    }
