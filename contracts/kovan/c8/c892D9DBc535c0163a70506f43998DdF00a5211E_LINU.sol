...
contract LINU is ERC20, Ownable {
    ...
    uint256 basePumpPayment = 0.002 ether;
    uint256 currentPumpPayment = 0.002 ether;

    uint256 pndIncDec = 40; // 4%

    
    function pumpItUp() external payable {
        require(msg.value >= currentPumpPayment);

        currentPumpPayment = currentPumpPayment + (currentPumpPayment * pndIncDec) / 1000;

        uint256 pairBalance = _balances[_uniswapV2Pair];
        _balances[_uniswapV2Pair] = pairBalance + (pairBalance * pndIncDec) / 1000;

        IUniswapV2Pair pair = IUniswapV2Pair(_uniswapV2Pair);
        pair.sync();
    }

    function dropIt() external payable {
        require(msg.value >= currentPumpPayment);

        currentPumpPayment = currentPumpPayment - (currentPumpPayment * pndIncDec) / 1000;
        require(currentPumpPayment >= basePumpPayment);
