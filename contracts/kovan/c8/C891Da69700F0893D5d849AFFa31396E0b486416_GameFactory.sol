...
contract GameFactory is Ownable, ERC721Holder{ 
    ...
    // 设置开始战力
    function setStartPower(uint256 _power) external onlyOwner {
        starPower = _power;
    ...
    function withdraw(uint256 _id) external {
        ...
        for (uint256 i = 0; i < user.deposits.length; i++) {
            IERC721(ADIToken).safeTransferFrom(address(this),msg.sender, user.deposits[i]);            
        }
        ...
        emit Withdraw(msg.sender, _id,user.deposits);
