...
contract DcaKeeperFacet is KeeperCompatibleInterface {
    ...
    function checkUpkeep(
        ...
        uint256 total = amountToSwap();
        upkeepNeeded = total > 0;
    }

    function performUpkeep(bytes calldata performData) external override {
        // Check if we need to perform dca for any of the accounts
        /*if (performData > 0) {
            swapExactInputSingle(performData, s.daiAddress, s.wEthAddress);
        }*/
    }

    function amountToSwap() internal view returns (uint256) {
        uint256 total = 0;
        /*for (uint256 i = 0; i < s.dcaSettings.length; i++) {
            if (
                block.timestamp - s.dcaSettings[i].lastDcaTimestamp >
                s.dcaSettings[i].period
            ) {
                s.dcaSettings[i].lastDcaTimestamp = block.timestamp;
                // If the dca is not valid anymore, we can remove it from the dcaSettings
                total += s.dcaSettings[i].amount;
            }
        }*/
        return total;
    }
