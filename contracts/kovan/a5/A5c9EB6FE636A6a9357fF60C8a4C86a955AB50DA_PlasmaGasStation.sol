...
contract PlasmaGasStation is Ownable, LondonTxSupport {
    ...
    IFeeTokensStore private feeTokenStore;
    ...
    constructor(address _router, address _feeTokenStore, address _feePayer, uint256 _txRelayFeePercent) {
        router = IUniswapV2Router02(_router);
        feeTokenStore = IFeeTokensStore(_feeTokenStore);
    ...
    function getEstimateGas(address _token) external view returns (uint256) {
        require(feeTokenStore.isAllowedToken(_token), "Fee token not supported");
    ...
    function sendTransaction(TxRequest calldata _tx, TxFee calldata _fee, bytes calldata _sign) external onlyFeePayer {
        ...
        require(feeTokenStore.isAllowedToken(_fee.token), "Fee token not supported");
