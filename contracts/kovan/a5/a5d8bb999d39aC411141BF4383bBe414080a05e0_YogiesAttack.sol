...
abstract contract IYogiesItems {
    function getHouseBalance(address owner) external view returns (uint256) {}
...
contract YogiesAttack is Ownable {
    ...
    uint256 public maxStealPercentage = 25;
    ...
    function attack(address victim) external {
        ...
        require(!yogiesItems.hasPet(victim), "Victim is protected");

        if (!yogiesItems.hasHouse(msg.sender)) {
            require(!yogiesItems.hasHouse(victim), "Cannot attack victim with house");
        }

        uint256 randomNumber = randomizer.getRandomNumber(msg.sender, victim);
        uint256 stealPercentage = randomNumber % (maxStealPercentage  - 5) + 5; // generate number between 0 and 20 and add 5
        ...
        userToCooldown[victim] = block.timestamp + (1 hours * stealPercentage);
        userToAttackCooldown[msg.sender] = block.timestamp + 1 days;
    ...
    function setYogiesItems(address _addr) external onlyOwner {
        yogiesItems = IYogiesItems(_addr);
    }
