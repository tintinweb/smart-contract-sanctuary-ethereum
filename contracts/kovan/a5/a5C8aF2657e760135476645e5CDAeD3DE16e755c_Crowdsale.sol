...
contract Crowdsale is AccessControl, ReentrancyGuard {
    ...
    function initiate(uint128 _sellStart) external onlyRole(0x0) {
        require(sellEnd == 0, "Already initiated");
        require(_sellStart > block.timestamp, "Sell start before now");
        require(
            IERC20(zeroGas).balanceOf(address(this)) >= hardcap,
            "Transfer tokens before init"
        );
        sellEnd = _sellStart + 35 days;
    }
    ...
    function buy(
        ...
        require(
            (sellEnd - 35 days <= block.timestamp &&
                block.timestamp < sellEnd - 21 days) ||
                (sellEnd - 14 days <= block.timestamp &&
                    block.timestamp < sellEnd),
            "Buy is not available"
        );
        require(expirationTime >= block.timestamp, "Signature out of time");
        require(payed > 0 && buyed > 0, "Zero amount");
        require(
            hasRole(
                SIGNER_ROLE,
                ECDSA.recover(
        ...
        totalBuyed += buyed;
        require(hardcap >= totalBuyed, "Hardcap exceeded");
        userToBalance[_msgSender()][3] += buyed;
        ...
        emit Buy(buyed, payed);
    }

    function claim() external nonReentrant {
        require(
            block.timestamp > sellEnd && totalBuyed >= softcap,
    ...
    function refund() external nonReentrant {
        ...
        require(
            balance[0] > 0 || balance[1] > 0 || balance[2] > 0,
            "Nothing to withdraw"
        );
        delete userToBalance[_msgSender()];
        if (balance[0] > 0) {
