...
contract Staking is Ownable, ReentrancyGuard {
  ...
  */
  function deposit(uint _amount) external notZero(_amount) lockable nonReentrant {
    ...
    _deposit(_amount);
    if (getBonus(msg.sender) != 0 && getRewards(msg.sender) != 0) {
  ...
  */
  function getBonus(address _user) public view returns(uint) {
    ...
    return currentBalance.amount * (block.timestamp - currentBalance.time) * bonusMultiplier / bonusDivisor - currentBalance.writeOffedBonus;
  }
  ...
  */
  function getRewards(address _user) public view returns(uint){
    Balance memory currentBalance = balances[_user];
    return currentBalance.amount * (block.timestamp - currentBalance.time) * rewardsMultiplier / rewardsDivisor - currentBalance.withdrawnRewards;
  }
  ...
  */
  ...
  function _writeOffBonus(uint _amount) private {
    require(getBonus(msg.sender) - _amount >= 0, "Insufficient bonus");
  ...
  function _withdrawRewards(uint _amount) private {
    require(getRewards(msg.sender) - _amount >= 0, "Insufficient rewards");
