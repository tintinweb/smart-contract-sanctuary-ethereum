...
contract FanLink is ERC1155, Ownable, ERC1155Supply {
    ...
    string[] public idMapAry; //"id" needs to be uint256 so i need an ary of artist ID to ERC1155 id to do translations back-and-forth
    uint256 public artistCount; //count of total unique artists for use with mapping
    address[] public ownershipAry; //This is an array of unique owners since ERC1155 doesn't track this in an easy-to-use way
    ...
    function isFanLinkOwner(address account) internal view returns (bool) {
        //is a given account an owner of these NFTs?
    ...
    {
        //grabs unique owners of a given token ID to return as address array
        for (uint256 id = 0; id < artistCount; id++) {
