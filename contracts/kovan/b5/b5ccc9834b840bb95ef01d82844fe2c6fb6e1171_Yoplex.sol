...
contract Yoplex is BRC20, BRC20Burnable, Ownable, ChainlinkClient{
    ...
    function investmentDetails(address _user, uint _index) public view returns(uint, uint, uint, uint, uint){
        return (investment[_user][_index].time, investment[_user][_index].amount_USD, investment[_user][_index].amount_BNB, investment[_user][_index].amount_YPLX, investment[_user][_index].YPLX_USD) ;
    }

    function withdrawalDetails(address _user, uint _index) public view returns(uint, uint, uint){
        return (withdawal[_user][_index].time, withdawal[_user][_index].amount_USD, withdawal[_user][_index].amount_YPLX) ;
    }
