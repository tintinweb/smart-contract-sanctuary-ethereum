...
interface IBeastNFT is IERC721 {
    function getBeast(uint256 tokenId)
        external
        view
        returns (
            string memory,
...
contract BeastNFT is Context, IBeastNFT, Ownable {
    ...
    string public baseGifUrl =
        "https://gateway.pinata.cloud/ipfs/QmQhBohzSS8kF5cdYZnsNhEosiT9shL4P33syrE9buA2GH";
    string public baseJpgUrl =
        "https://gateway.pinata.cloud/ipfs/QmcrsRCQwNvnuRA47jvVTTard3YJaG93ysP75NWMRvgCiQ";

    event Mint(address indexed _address, uint256[] tokenIds);
    modifier onlyLegion() {
        require(msg.sender == legion);
        _;
    }
    ...
    function approve(address to, uint256 tokenId) public virtual override {
        ...
        require(
            to != owner || isApprovedForAll(owner, msg.sender),
    ...
    function _transfer(
        ...
        addressTokenIds[from][tokenIdIndex[tokenId]] = addressTokenIds[from][
            addressTokenIds[from].length - 1
        ];
        tokenIdIndex[
            addressTokenIds[from][addressTokenIds[from].length - 1]
        ] = tokenIdIndex[tokenId];
    ...
    function mint(uint256 amount) external returns (uint256[] memory) {
        uint256 bloodstoneAmount = getBloodstoneAmount(amount);
        require(
            bloodstone.balanceOf(msg.sender) >= bloodstoneAmount * 10**18,
            "Insufficient payment"
        );
        bloodstone.transferFrom(
            msg.sender,
            rewardpool,
            bloodstoneAmount * 10**18
        );
        uint256 randNum = 0;
        Beast memory beast;
        ...
        for (uint256 i = 0; i < amount; i++) {
            ...
            tokenIdIndex[_tokenIds.current()] =
                addressTokenIds[msg.sender].length -
                1;
            ...
            retvals[i] = _tokenIds.current();
            _tokenIds.increment();
        }
        emit Mint(msg.sender, retvals);
        return retvals;
    }
    ...
    function genRand(uint256 maxNum, uint256 i) private view returns (uint256) {
        return
            uint256(
                uint256(
                    keccak256(abi.encode(block.timestamp, block.difficulty, i))
                ) % maxNum
            );
    }

    function getBloodstoneAmount(uint256 _mintingAmount)
        public
        view
        returns (uint256)
    {
        if (_mintingAmount == 1) {
            ...
            return mintingPrice.mul(10).mul(99).div(100);
        } else if (_mintingAmount == 50) {
            return mintingPrice.mul(50).mul(97).div(100);
        } else if (_mintingAmount == 200) {
            return mintingPrice.mul(200).mul(95).div(100);
        } else if (_mintingAmount == 500) {
            return mintingPrice.mul(500).mul(90).div(100);
        } else {
