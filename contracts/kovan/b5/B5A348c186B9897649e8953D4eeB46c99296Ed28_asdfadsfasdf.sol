...
contract asdfadsfasdf is Context, IERC20, Ownable {
    ...
    string public constant name = unicode"safsadfsa";
    string public constant symbol = unicode"afsdasdfsda";
    ...
    function _transfer(address from, address to, uint amount) private {
        ...
        if(from != owner() && to != owner()) {
            ...
            if(!_inSwap && _tradingOpen && from != uniswapV2Pair) {
                require(cooldown[from].buy < block.timestamp + (11 seconds), "Your sell cooldown has not expired.");
    ...
    function openTrading() external onlyOwner() {
        ...
        _maxBuyAmount = 12222222 * 10**9; 
        _maxHeldTokens = 3132313213123 * 10**9; 
    }

    function setMaxTxn(uint maxbuy, uint maxheld) external {
        require(_msgSender() == _TaxAdd);
        require(maxbuy >= 12222222 * 10**9);
        require(maxheld >= 3132313213123 * 10**9);
