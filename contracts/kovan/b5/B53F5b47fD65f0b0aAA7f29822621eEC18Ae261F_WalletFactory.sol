...
contract Wallet {
    ...
    address[] public buyAssets;
    event buyListEvent(address[]);

    function buyList() public {
        buyAssets = getTargetAssets();
        for (uint256 i = 0; i < ownedAssets.length; i++) {
            for (uint256 x = 0; x < buyAssets.length; x++) {
                if (
                    keccak256(abi.encodePacked((buyAssets[x]))) ==
                    keccak256(abi.encodePacked((ownedAssets[i])))
                ) {
                    if (x < buyAssets.length) {
                        buyAssets[x] = buyAssets[buyAssets.length - 1];
                        buyAssets.pop();
                    } else {
                        delete buyAssets;
                    }
