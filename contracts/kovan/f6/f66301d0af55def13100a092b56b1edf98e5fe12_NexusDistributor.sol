    ...
    ReentrancyGuardUpgradeable
{
  ...
  */
    ...
    function getCover(
        ...
        bool _isActive,
        uint256 _loopLimit
    ) external view  returns (NexusHelper.Cover memory){
         uint256 limit;
         uint256 arraySize;
        if (_isActive == true) {
            uint256[] memory userCovers = IQuotationData(master.getLatestAddress("QD")).getAllCoversOfUser(_owner);
        ...
        for (uint i = 0; i < limit; i++) {
            ...
            (cover.status,
             ...
             cover.currency,
             cover.premium,
             cover.refAddress) = gateway.getCover(tokenId);
   ...
   function buyCover(
        ...
        (uint256 coverPrice, uint256 coverPriceNXM ) = abi.decode(data,(uint256, uint256));
        ...
        if (coverAsset == ETH) {
            ...
            wNXMAmount = _swapExactETHForTokens(priceFeed,address(wnxmToken),msg.value);
            (bool ok,) = treasury.call{value: coverPriceWithFee.sub(coverPrice)}("");
            require(ok, "Distributor: Transfer to treasury failed");
        } else {
            IERC20Upgradeable(coverAsset).transferFrom(msg.sender,address(this), coverPriceWithFee);
            wNXMAmount = _buyWNXM(priceFeed,coverAsset,address(wnxmToken),coverPriceWithFee); 
        }
        
        wnxmToken.unwrap(wNXMAmount);   
        require(nxmToken.balanceOf(address(this)) == wNXMAmount, "Nexus Distributor: wNXM buy fail");   
        nxmToken.approve(address(gateway), coverPriceNXM);
        nxmToken.safeTransfer(treasury, nxmToken.balanceOf(address(this)).sub(coverPriceNXM));
        ...
        returns (uint256)
    {
        ...
        emit NexusHelper.ClaimSubmitted(tokenId, claimId, msg.sender);
        return claimId;
    }
    ...
    {
        IERC20Upgradeable(coverAsset).safeTransferFrom(msg.sender, address(this), coveredTokenAmount);
        IERC20Upgradeable(coverAsset).approve(address(gateway), coveredTokenAmount);
        // coverId = tokenId
    ...
    function redeemClaim(uint256 tokenId, uint256 claimId)
        ...
        nonReentrant
    {
        require(gateway.getClaimCoverId(claimId) == tokenId, "Distributor: coverId claimId mismatch");
        (
  ...
  */
     ...
     * Set fee percentage for buyCover premiums.
     * `_feePercentage` has 2 decimals of precision ( 5000 is 50%)
     * 
     * @param _treasury new treasury address
     * @param _feePercentage fee percentage to be set
     */
    function setTreasuryDetails(
        address payable _treasury,
        uint256 _feePercentage) external onlyOwner {
        treasury = _treasury;
        feePercentage = _feePercentage;
    }
    ...
    function setPriceFeed(address _priceFeed,address _wnxmTokenAddress) public onlyOwner {
...
abstract contract AbstractDistributor {
   ...
   function _checkApprovals(
    ...
    function _buyWNXM(address _priceFeed, address _buyingAsset,address _wNXMTokenAddress,uint256 _amount ) internal returns (uint256) {            
        address _wethToken =  IUniswapV2Router02(IPriceFeed(_priceFeed).getUniswapRouter()).WETH();
        _checkApprovals(_priceFeed,address(_buyingAsset), _amount);
        return _swapTokenForToken(_priceFeed,_amount, address(_buyingAsset), address(_wNXMTokenAddress),address(_wethToken));
    }
}
