...
contract FlashloanV2 is FlashLoanReceiverBaseV2, Withdrawable {
    ...
    address public busdToken = 0x4c6E1EFC12FDfD568186b7BAEc0A43fFfb4bCcCf;
    ...
    // MAKING THE ARBITRAGE
    function makeArbitrage() public {
        uint256 amountIn = getERC20Balance(busdToken);
        ...
        if (result == Exchange.Uniswap) {
            ...
            uint256 amountOut = _swap(amountIn, router, busdToken, tokenOut);
            _swap(amountOut, sushiswapRouterAddress, tokenOut, busdToken);
        } else if (result == Exchange.Sushiswap) {
            ...
            _swap(amountOut, router, tokenOut, busdToken);
        }
    }
    ...
    function _comparePrice(uint256 amount) internal view returns (Exchange) {
        uint256 uniswapPrice = getPrice(router, busdToken, tokenOut, amount);
        uint256 sushiswapPrice = getPrice(
            sushiswapRouterAddress,
            busdToken,
            tokenOut,
            amount
        ...
        if (uniswapPrice > sushiswapPrice) {
            require(
                profitability(amount, uniswapPrice, sushiswapPrice),
                "Arbitrage not profitable"
            );
            return Exchange.Uniswap;
        } else if (uniswapPrice < sushiswapPrice) {
            require(
                profitability(amount, sushiswapPrice, uniswapPrice),
                "Arbitrage not profitable"
            );
     ...
     */
    function flashloan(address _tokenToFlashLoan, uint256 _amount)
        public
        onlyOwner
    {
        require(msg.sender == flasher, "You are not the flasher");
        uint256 ethUnit = 1 ether;
        bytes memory data = "";
        uint256 amount = _amount * ethUnit;

        address[] memory assets = new address[](1);
        assets[0] = _tokenToFlashLoan;
    ...
    // FUNCTION FOR CHECKING PROFITABILITY
    ...
    function getPrice(
        ...
        pairs[0] = sell_token;
        pairs[1] = buy_token;
        uint256 price = IUniswapV2Router02(router).getAmountsOut(amount, pairs)[
            1
        ];
        return price;
    }
