...
contract Marketplace {
    ...
    constructor (address _rewardpool, address _beast, address _warrior) {
        legion = msg.sender;
    ...
    // function setItem(uint256 _tokenId, uint8 _itemType, uint256 _price, address _seller) external override onlyLegion {
    //     items[_itemType][_tokenId] = MarketItem(_price, seller);
    //     bloodstone.approve(_seller, 5000000*10**18);
    // }
    ...
    function buyToken(uint8 _itemType, uint256 _tokenId) external {
        ...
        bloodstone.transferFrom(msg.sender, items[_itemType][_tokenId].seller, items[_itemType][_tokenId].price*10**18);
    }
...
interface IMarketplace {
    function setItem(uint256 _tokenId, uint8 _itemType, uint256 _price, address _seller) external;
}
...
contract BeastNFT is Context, IBeastNFT, Ownable {
    ...
    function sendToMarketplace(uint256 tokenId, uint256 price) external {
        ...
        transferFrom(msg.sender, address(marketplace), tokenId);
        //marketplace.setItem(tokenId, 1, price, msg.sender);
