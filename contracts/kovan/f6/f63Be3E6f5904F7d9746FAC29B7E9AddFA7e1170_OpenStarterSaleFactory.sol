...
contract OpenStarterSale {
    ...
    uint256 public saleType = 0; // 0 : DAO Sale, 1 : Certified Sale
    ...
    function addStageInfo(
        ...
        _newInfo.isPublicMode = _isPublicMode;
        _newInfo.isLotteryMode = _isLotteryMode;
    ...
    function addWhitelistedAddresses(
        ...
        uint256 whitelistLen = _whitelistedAddresses.length;
    ...
    function isVotePassed() public view returns (bool) {
        return
            yesVotes >=
    ...
    function getCurrentStage() public view returns (uint256) {
        uint256 stagesLen = stages.length;
        uint256 stageIndex = stagesLen;
        for (uint256 i = 0; i < stagesLen; i++) {
            if (
    ...
    function getRemainingCounts(uint256 stageIndex)
        public
        view
        returns (uint256)
    {
        return stages[stageIndex].quantity.sub(stages[stageIndex].sold);
    }
    ...
    function invest(uint256 _buyCount) public payable {
        ...
        // In Whitelist Mode - Whitelists are required.
        ...
        // In Whitelist / Public Mode - Staking Required In here.

        if (stage.isWhitelistedMode || stage.isPublicMode) {
            require(userTier == tierIndex && userTier > 0, "6");
        }
        ...
        // In Whitelist/Public Mode - Maximum Counts = Tier Percentage of Stage Quantity
        ...
        // In Lottery Mode - Maximum Counts = Whitelist Remaining + Public Remaining + Lottery Quantity.

        if (stage.isLotteryMode) {
            uint256 lotteryCounts = stage
                .quantity
                .add(getRemainingCounts(0))
                .add(getRemainingCounts(1)); // Whitelist/Public Stage Remaining Nfts Counts
            require(stage.sold <= lotteryCounts, "9");
        }

        // Sending Funds from Account to Contract

        uint256 investAmount = stage.pricePerNft.mul(_buyCount);
        if (address(fundingTokenAddress) == openStarterLibrary.getWETH()) {
            require(msg.value >= investAmount, "3");
        }
...
contract OpenStarterSaleFactory {
    ...
    function createSale(
        ...
        uint256 _saleType, // 0: DAO Sale, 1: Certified
        ...
        uint256 stagesLen = _stages.length;
        for (uint256 i = 0; i < stagesLen; i++) {
            sale.addStageInfo(
                _stages[i].stageType,
                _stages[i].openTime,
                ...
                _stages[i].isPublicMode,
                _stages[i].isLotteryMode
            );
        }
