...
interface ILaboratory is ISeed {
     ...
     */
    event NewEvolutionCompleted(string methodName, CellData.Cell cell, uint timestamp);

    event SplitMDMAToken(address receiver, uint amount, uint timestamp);
     ...
     */
    event EvolutionTimeReduced(string name, CellData.Cell cell, uint timestamp);

    /**
     *  Event to show that price for boost is changed
     *   @dev has to be emited in `setBoostPerBlockPrice`
     */
    event BoostPricePerBlockChanged(uint256 _price, uint timestamp);
    ...
    event NewMetaCellCreated(address ownerOf, uint tokenId, uint timestamp);
}
...
interface INanoCellCreator {
    function create(address account) external;
}
    ...
    Ownable
{
    ...
    function _processSplit(CellData.Cell memory cell) private {
        if (CellData.Class(cell.class) == CellData.Class.SPLITTABLE_NANO) {
            INanoCellCreator(nanoCell).create(msg.sender);
        } else if (
            CellData.Class(cell.class) == CellData.Class.SPLITTABLE_MAD
        ) {
            
            uint amount = IRandom(random).randomRateSplitMadToken();
            IMintable(madToken).mint(msg.sender, amount * 10**IERC20Metadata(madToken).decimals());
            emit SplitMDMAToken(msg.sender, amount, block.timestamp);
        } else if (
            CellData.Class(cell.class) == CellData.Class.SPLITTABLE_ENHANCER
        ) {
            uint randomEnhancerId = IRandom(random).randomEnhancerId(IMarketPlace(marketPlace).numberOfEnhancersType());
    ...
    function isCanCreateMetaCell(uint256 positionId) external view returns (bool) {
        return !EnumerableSet.contains(scientistsUsed, positionId);
    }

    function createMetaCell(uint256 positionId)
        external
        override
    {
        require(IERC721(scientistToken).ownerOf(positionId) == msg.sender, "Caller is not ower of Scientist");
        require(
            !EnumerableSet.contains(scientistsUsed, positionId),
            "Already used scientist"
        );
        EnumerableSet.add(scientistsUsed, positionId);
        uint metaCellId = IMetaCellCreator(metaCell).create(msg.sender);
    ...
    function withdrawFee() external onlyOwner {
        IERC20(madToken).transfer(msg.sender, IERC20(madToken).balanceOf(address(this)));
    }
}