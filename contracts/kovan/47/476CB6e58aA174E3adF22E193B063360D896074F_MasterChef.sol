...
contract MasterChef is Ownable {
    ...
    function pendingReward(uint256 _pid, address _user) external view returns (uint256 pending) {
        ...
        if (block.timestamp > pool.lastRewardTime && lpSupply != 0) {
            ...
            uint256 rewardAmount = time.mul(rewardPerSecond).mul(pool.allocPoint).div(totalAllocPoint);
            accRewardPerShare = accRewardPerShare.add(rewardAmount.mul(ACC_REWARD_PRECISION).div(lpSupply));
        }
        pending = uint256(int256(user.amount.mul(accRewardPerShare).div(ACC_REWARD_PRECISION)).sub(user.rewardDebt));
    }
    ...
    function harvest(uint256 pid, address to) public {
        ...
        uint256 _pendingReward = uint256(accumulatedReward.sub(user.rewardDebt));
    ...
    function withdrawAndHarvest(
        ...
        // Effects
        user.rewardDebt = accumulatedReward.sub(int256(amount.mul(pool.accRewardPerShare).div(ACC_REWARD_PRECISION)));
        user.amount = user.amount.sub(amount);
