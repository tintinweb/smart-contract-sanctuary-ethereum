...
contract Packs is ERC721, AccessControl, ContextMixin, NativeMetaTransaction, ERC721URIStorage {
    bytes32 public constant MINTER_ROLE = keccak256("MINTER_ROLE");
    using ECDSA for bytes32;


 //opensea proxy address for gasless transactions
address private constant OPENSEA_PROXY = 0x58807baD0B376efc12F5AD86aAc70E78ed67deaE;
...
function _baseURI() internal pure override returns (string memory) {
 ...
 // OpenSea metadata initialization
    function contractURI() public pure returns (string memory) {
        return "https://example.com/opensea/contract_metadata.json";
    }

uint256 public globalTransferNonce = 0;

//constructor to set admin

constructor(string memory name) ERC721(name, "PACK") {
...
//open pack
event PackOpened(address indexed ownerOfPack, uint256 indexed tokenId);

//moments sent
event MomentsReceived(address indexed ownerOfPack, uint256[] momentsSerial);
   ...
   function delegatedSafeSignedTransferFrom(
        ...
        require(userSignedApprovalForTransfer(from, to, tokenId, signature), "Signature did not match");
    ...
    function userSignedApprovalForTransfer(
        ...
        userHash = keccak256(abi.encodePacked(address(this), globalTransferNonce, from, to, _msgSender(), tokenId));
...
function safeMint(address to, uint256 tokenId, string memory uri)
    ...
    {
        require(_exists(tokenId) == false, "someone minted that");
 //       require(tokenId <= (_maxSupply - 1), "packs are sold out");
 //mint occurs
...
function userSignedApprovalForPackOpen(address ownerOfPack, 
uint256 tokenId, 
uint256[] memory momentsSerial, 
address[] memory externalContracts, 
address sweetAdminWallet, 
bytes memory signature) 
public view 
returns (bool) {
    bytes32 userHash;
    userHash = keccak256(abi.encodePacked(address(this), 
    globalTransferNonce, 
    ownerOfPack, 
    tokenId, 
    momentsSerial, 
    ...
    sweetAdminWallet));
    address messageSigner = userHash.toEthSignedMessageHash().recover(signature);
    require((messageSigner == ownerOfPack && messageSigner == this.ownerOf(tokenId)), "This token, is not yours.");
    return true;
}

//token id nuumber uint number of tokens and then the [] address to external mint
//signiture get which calls burn pack
//safeTransferFrom(address from, address to, uint256 tokenId)
//admin wallet needs to call the function
function delegatedExchangeToken(address ownerOfPack, 
uint256 tokenId, 
uint256[] memory momentsSerial, 
address[] memory externalContracts, 
address sweetAdminWallet, 
...
public 
onlyRole(MINTER_ROLE) {
    require(userSignedApprovalForPackOpen(ownerOfPack, 
    tokenId, 
    momentsSerial, 
    externalContracts, sweetAdminWallet, signature), "Signature did not match");
    _safeTransfer(ownerOfPack, address(0x0000000000000000000000000000000000000000), tokenId, "");
    emit PackOpened(ownerOfPack, tokenId);
    globalTransferNonce += 1;
    for (uint i = 0; i < externalContracts.length; i++) {
        //ADMIN
       IERC721(externalContracts[i]).transferFrom(sweetAdminWallet, ownerOfPack, momentsSerial[i]);
       emit MomentsReceived(ownerOfPack, momentsSerial);
  }
}
    /**
...
//sets opensea proxy contract for gasless transactions
