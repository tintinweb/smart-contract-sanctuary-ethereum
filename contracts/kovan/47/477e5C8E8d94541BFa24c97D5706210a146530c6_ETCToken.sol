...
contract ETCToken is Context, IERC20, IERC20Metadata ,  Ownable{
    ...
    function casting(uint256 taxes,address _caller)  external   returns(bool) {
            ...
            _mint(_caller,etcPrice.mul(taxes.mul(1).div(3)).div(1e18));        
           
            uint256 usdtBalance =  usdtAddress.balanceOf(address(this));
           
            etcPrice =  _totalSupply.mul(1e18).div(usdtBalance);
            usdtValue += taxes;
    ...
    function sellEtc(uint256 amount) external lockTheSwap returns(bool){
        ...
        uint256 usdtIncome = amount.mul(80).div(100).mul(1e18).div(etcPrice);
        uint256 daoUsdtIn =  amount.mul(1e18).div(etcPrice).mul(2).div(100);
        uint256 lpUsdtIn =  amount.mul(1e18).div(etcPrice).mul(2).div(100);
