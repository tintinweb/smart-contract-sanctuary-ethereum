...
contract ERC20NFTBond is NFTBond {
    ...
    external whenNotPaused returns (uint256) {
        return super._buyBond(tokenUri, beneficiary, maturity, principal);
    }
...
abstract contract NFTBond is NFT, CollateralizedBondGranter, LiquidityRequester, PausableUpgradeable {
    ...
    function requestLiquidity(address destination, uint256 amount) public override whenNotPaused returns (uint256) {
        ...
        _afterRequestLiquidity(destination);
        return totalBorrowed;
    }
    ...
    internal returns (uint256) {
        ...
        uint256 tokenId = _safeMint(beneficiary, tokenUri);
    ...
    function _redeemBond(uint256 tokenId) internal virtual override returns (uint256) {
        ...
        _beforeBondRedeemed(tokenId, amount);
...
abstract contract CollateralizedBondGranter is BondGranter {
    ...
    function _issueBond(
        ...
        require(_hasCollateral(principal), "CollateralizedLoanGranter::Not enough collateral to cover the bond");
        super._issueBond(tokenId, maturity, principal);
    ...
    function _calculateCollateralBondAmount(uint256 principal) internal view returns (uint256) {
        uint256 bondCollateral = principal * collateralMultiplier;
        return bondCollateral;
    }

    function _hasCollateral(uint256 principal) internal view returns (bool) {
        if (_collateralToken.balanceOf(address(this)) >= principal * collateralMultiplier) {
            return true;
        }
        return false;
    }
...
abstract contract BondGranter is Initializable, InterestParameters, InterestCalculator {
    ...
    function _issueBond(uint256 tokenId, uint256 maturity, uint256 principal) internal virtual {
        require(principal > 0, "BondGranter::Principal can't be 0");
    ...
    function _redeemBond(uint256 tokenId) internal virtual returns (uint256) {
        ...
        require(!bond.redeemed, "BondGranter::Bond already redeemed");
    ...
    function _bondValue(uint256 tokenId) internal view virtual returns (uint256) {
        Bond memory bond = bonds[tokenId];
        return bond.principal + bond.principal.mul(super.simpleInterest(bond.interest, bond.maturity, bond.mintingDate) / 100);
    }
...
abstract contract InterestCalculator is IInterestCalculator {
  function _getElapsedSeconds(uint256 timestamp, uint256 mintingDate) internal pure virtual returns (uint256) {
    return timestamp - mintingDate;
  }

  function _simpleInterest(uint256 interest, uint256 maturity, uint256 mintingDate) internal view virtual returns (uint256) {
    uint256 elapsedSeconds = _getElapsedSeconds(block.timestamp, mintingDate);

    if (elapsedSeconds < maturity) {
      return elapsedSeconds * interest;
    }
    else {
  ...
  function simpleInterest(uint256 interest, uint256 maturity, uint256 mintingDate) public view virtual override returns (uint256) {
    return _simpleInterest(interest, maturity, mintingDate);
  }
}
...
abstract contract InterestParameters is Initializable, IInterestParameters, AccessManagedUpgradeable {
    ...
    internal {
        require(_interests.length <= maxParametersLength, "InterestParameters::Interest parameters is major than max parameters");
    ...
    /**
     * @dev Checks the interest correspondant to the maturity. Needs at least 1 maturity / interest pair
    ...
    function _getInterestForMaturity(uint256 maturity) internal view returns (uint256) {
        ...
        return interests[interests.length - 1];
    }
...
interface IInterestCalculator {
   ...
   * @param mintingDate timestamp
   * @return simple interest per second that is int 1e18
   */
  function simpleInterest(uint256 interest, uint256 maturity, uint256 mintingDate) external view returns (uint256);
}
