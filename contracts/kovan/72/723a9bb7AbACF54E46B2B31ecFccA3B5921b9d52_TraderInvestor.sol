...
contract TraderInvestor is Investor {
    ...
    constructor(
        ...
        token.approve(address(trader), uint256(2**256 - 1));
        _approve(address(this), address(trader), uint256(2**256 - 1));
    }
     ...
     * @param _erc20Amount is amount to pay off
     */
    function payment(uint256 _id, uint256 _erc20Amount) public override {
        super.payment(_id, _erc20Amount);
    ...
    function _checkAvailable(uint256 _amount) internal override {
        require(
            revManager.balanceAvailable(address(this), address(token)) >=
                _amount,
    ...
    * @dev returns the expected output of investor tokens if tradeAmount
    ...
    function _sell() private returns (bool) {
        _mint(address(this), _getTradeAmount());
 ...
 * @dev TestToken will eventually be replaced with ERC-20
 */
contract Investor is ERC20, ERC20CollateralPayment {
    address public farmMinterRole;
    ...
    /**
     * @dev for farm to mint
     * @param to is who the tokens are minted to
     * @param amount is the amount to mint
     */
    function mint(address to, uint256 amount) external {
        require(
            msg.sender == farmMinterRole,
            "Only the farm may call this function"
        );
        _mint(to, amount);
    }

    /**
     * @dev for owner to set the minter role
     * @param _new is the new address
     */
    function setFarmMinterRole(address _new) external onlyOwner {
        farmMinterRole = _new;
    }
    ...
    function borrow(
        ...
        ScoreDBInterface(creditOracle).requestUpdatedScoreBorrow(
            _NCFSID,
            id,
            _hash,
            _signature
     ...
     * @param _erc20Amount is amount to pay off
     */
    function payment(uint256 _id, uint256 _erc20Amount)
        public
        virtual
        override
    {
    ...
    function borrowFulfill(
        ...
        // fulfill loan
        _sendFunds(loanLookup[_id].borrower, amount);
    }
...
contract ERC20CollateralPayment is ERC20PaymentStandard, Ownable {
    ...
    constructor(address _bonds, address _oracle, address _ncfs)
        ERC20PaymentStandard(_bonds, _ncfs)
        Ownable()
    {
        creditOracle = _oracle;
    }
        ...
        )
    {
        ...
        uint16 score = ScoreDBInterface(creditOracle).getCurrentScore(
            ln.ncfsID
        );

        uint256 ltv = ScoreDBInterface(creditOracle).LTV(ln.ERC20Address,score);
        uint256 lt = ScoreDBInterface(creditOracle).LT(ln.ERC20Address, score);
        ...
        return (collateralUSD, outstandingUSD, score, ltv, lt, availableCollateral);
    }
    ...
    function _isWithinLT(uint256 _id) internal view returns(bool){
        ...
        ((outstandingUSD * int256(ONE_HUNDRED_PERCENT)) / collateralUSD <= int256(lt));
    }
    ...
    function _isWithinLTV(uint256 _id) internal view returns(bool){
        ...
        return collateralUSD == 0 ? false : 
        ((outstandingUSD * int256(ONE_HUNDRED_PERCENT)) / collateralUSD <= int256(ltv));
    }
...
contract ERC20PaymentStandard is ERC1155Holder {
     ...
     */
    constructor(address _bonds, address _ncfs) {
        bondContract = _bonds;
        NCFS = _ncfs;
    }
     ...
     * @param _amm the amount of interest to add
     ...
     */
    function getLoanInfo(uint256 _id)
        public
        view
        virtual
    ...
    function configureNew(
        ...
        require(
            IERC721(NCFS).ownerOf(_NCFSID) == _borrower,
            "NCFS ID must bellong to the borrower"
        );
        ...
        incomplete(_id)
    {
        loan memory ln = loanLookup[_id];
        require(
            _erc20Amount >= ln.minPayment || //Payment must be more than min payment
        ...
        loanLookup[_id].awaitingCollection += _erc20Amount;
        loanLookup[_id].paymentDueDate = block.timestamp + ln.paymentPeriod; //Reset paymentTimer;
        loanLookup[_id].paymentComplete += _erc20Amount; //Increase paymentComplete
    }
