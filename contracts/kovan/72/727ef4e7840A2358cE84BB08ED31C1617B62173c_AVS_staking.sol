...
// File: contracts/stakin_contract.sol
...
contract AVS_staking is Ownable , ReentrancyGuard
{
    ...
    uint256 constant public MAX_NUM_DAYS = 195;
    ...
    {
        require(
            block.timestamp >= zeroDayStartTime,
    ...
    constructor (IERC20 _AVSAddress, uint256 _zeroDayStartTime, uint256 _dayDurationSec) ReentrancyGuard() {
    ...
    {
        require(
            numDaysStake > 0 && numDaysStake <= MAX_NUM_DAYS && (numDaysStake%15 == 0 || numDaysStake == 181),
    ...
    function stakeEnd(uint256 stakeIndex, uint256 stakeId) external onlyWhenOpen nonReentrant
    {
        ...
        if (isWhitelisted(sender)) {
            ...
            if (servedNumOfDays < st.numDaysStake){
                ...
                uint256 remainingAVSTokens = st.freezedRewardAVSTokens.sub((avsTokensToReturn - st.stakedAVS).mul(98).div(100));
                unfreezedAVSTokens = unfreezedAVSTokens.add(remainingAVSTokens).add(st.stakedAVS.mul(2).div(100));
                freezedAVSTokens = freezedAVSTokens.sub(st.freezedRewardAVSTokens);
                emit TokenUnfreezed(sender, st.freezedRewardAVSTokens, currDay);
                
                allAVSTokens = allAVSTokens.sub((avsTokensToReturn.sub(st.stakedAVS)).mul(98).div(100)); // dividends
                allAVSTokens = allAVSTokens.add(st.stakedAVS.mul(2).div(100)); // stake

                avsAddress.transfer(sender, avsTokensToReturn.mul(98).div(100));
                emit AVSTokenOutcome(sender, avsTokensToReturn.mul(98).div(100), currDay);

                emit StakeEnd(
                    sender,
                    st.stakeId,
                    (avsTokensToReturn - st.stakedAVS).mul(98).div(100),
                    servedNumOfDays,
                    currDay
                );
                totalStakedAVS = totalStakedAVS.sub(st.stakedAVS);
    ...
    {
        return block.timestamp.sub(zeroDayStartTime).div(dayDurationSec);
    }
