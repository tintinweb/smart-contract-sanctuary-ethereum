...
contract BlahNFT is BlahNFTScore {
    ...
    function uploadMetadata(BlahNFTTypes.CityWithId[] memory _cities) external onlyOwner {
        ...
        for(uint i = 0; i < _cities.length; i++) {
            ...
            if(idToCities[_cities[i].id].points == 0 && _exists(_cities[i].id)) {
                idToCities[_cities[i].id] = BlahNFTTypes.City(_cities[i].city, _cities[i].country, _cities[i].points, _cities[i].cityFaction, 0);
                addressToFaction[ownerOf(_cities[i].id)] = _cities[i].cityFaction;
            }
        }
    }

    function uploadScore(BlahNFTTypes.CountryToScore[] memory _countryToScore,
                        uint256 red,
                        uint256 green,
...
contract BlahNFTScore is BlahNFTNFT {
    ...
    function getCountryFaction(string memory country) public view returns (uint8) {
        BlahNFTTypes.CountryScore memory countryScore = countryToScore[country];
...
contract BlahNFTNFT is ERC721, Ownable, ReentrancyGuard {
    ...
    uint256 public constant FOUNDERS_RESERVE_AMOUNT = 25;
