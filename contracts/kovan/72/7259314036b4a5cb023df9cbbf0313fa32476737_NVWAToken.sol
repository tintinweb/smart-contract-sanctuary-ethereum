...
contract NVWAToken is Context, IERC20, Ownable {
    ...
    mapping(uint256 =>mapping(uint256 => address)) public totalUserAddres;
    ...
    constructor () public {
        ...
        // uint time = block.timestamp / 1 days * 1 days;
         startime = block.timestamp;
        totalUserAddres[0][userTotal] = _holder;
    ...
    function addRelationEx(address recommer,address user) internal {
        if( 
            ...
            && _recommerMapping[recommer] != address(0x0) ){
                ...
                uint time = (block.timestamp -startime) / 1 days * 1 days;
                totalUserAddres[time][userTotal] = user;
        }       
    }
