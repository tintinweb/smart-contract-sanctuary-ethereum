...
contract PrecogV5 is IPrecogV5 {
    ...
    function _updateCurrentTradingCycle(address _token, bool _isLimitedTradingCycles, uint _limitTradingCycles) internal {
        ...
        while (uint48(block.timestamp) >= _lastTradingCycle.endTime) {
            if(_isLimitedTradingCycles == true) {
    ...
    function _updateAdjustment(address _token) internal {
        ...
        if(_cycleChangedInfo.tradingApplyTime <= block.timestamp) {
        ...
        cyclesChangedInfo[_token] = CyclesChangedInfo(_cycleChangedInfo.tradingApplyTime, _nextFundingTime, _nextDefundingTime, _fundingDuration, _firstDefundingDuration, _defundingDuration);
    }
