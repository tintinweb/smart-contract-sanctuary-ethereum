...
contract Bridge is IBridge, Ownable {
    ...
    constructor(ValidatorWithPower[] memory validators) {
        _updateValidatorPowers(validators);
    }

    /// Update validator powers by owner.
    /// @param validators The changed set of BandChain validators.
    function _updateValidatorPowers(ValidatorWithPower[] memory validators)
        private
    {
        ...
        for (uint256 idx = 0; idx < validatorsCount; ++idx) {
            require(
                validators[idx].addr > lastValidator,
                "INVALID_VALIDATOR_ORDER"
            );
