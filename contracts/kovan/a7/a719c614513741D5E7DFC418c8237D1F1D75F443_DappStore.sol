...
contract DappStore is ERC721, IDappStore {
  ...
  /**
   * @dev See {IDappStore-getDappsInRangeLastVersion}
   */
  function getDappsInRangeLastVersion(uint256 beginId, uint256 endId)
    external
    view
    override
    returns (DappLastVersion[] memory)
  {
    uint256 size = endId.sub(beginId).add(1);
    DappLastVersion[] memory _dapps = new DappLastVersion[](size);

    uint256 index = 0;
    for (uint256 i = beginId; i <= endId; i++) {
      if (_exists(i) && dapps[i].versions.length > 0) {
        _dapps[index].dappId = i;
        _dapps[index].owner = ownerOf(i);
        _dapps[index].name = dapps[i].name;
        _dapps[index].lastVersion = dapps[i].versions[
  ...
  /**
   * @dev See {IDappStore-getOwnersInRange}
   */
...
interface IDappStore is ITypes {
  ...
  /**
   * @notice Get dapp's owners & lastVersion in range
   * @dev the range is inclusive
   *
   * @param beginId fetch starting range
   * @param endId  fecch ending range
   *
   * @return dtos array of dapp's dto (owner ,name ,lastVersion)
   */
  function getDappsInRangeLastVersion(uint256 beginId, uint256 endId)
    external
    view
    returns (DappLastVersion[] memory dtos);

  /**
   * @notice Get dapp's owners in range
   * @dev the range is inclusive
   *
   * @param beginId fetch starting range
   * @param endId  fecch ending range
   *
   * @return owners array of dapp's owners
   */
  function getOwnersInRange(uint256 beginId, uint256 endId)
    external
    view
...
interface ITypes {
  ...
  /**
   * @dev DappLastVersion is usual dto (nothing special)
   */
  struct DappLastVersion {
    uint256 dappId;
    address owner;
    string name;
