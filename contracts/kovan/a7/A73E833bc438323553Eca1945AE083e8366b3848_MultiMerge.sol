...
interface IRainiNft1155 is IERC1155 {
    ...
    function numberMintedByAddress(address _address, uint256 _cardID)
        external
        view
        returns (uint256);
    ...
    function getPathUri(uint256 _cardId) external view returns (string memory);
...
contract MultiMerge is AccessControl, ReentrancyGuard {
    ...
    function setFees(uint256[] memory _mergeFees) external onlyOwner {
        for (uint256 i = 0; i < _mergeFees.length; i++) {
    ...
    function setMergeAmounts(
        uint256[] memory _mintedCardId,
        uint256[] memory _mintedCardLevel,
        uint256[][] memory _mergedId,
        ...
        for (uint256 i = 0; i < _mergedId.length; i++) {
            ...
            for (uint256 j = 0; j < _mergedId[i].length; j++) {
                mergeAmounts[_mintedCardId[i]][_mintedCardLevel[i]].push(
                    CardAmount({
    ...
    function merge(
        ...
        _locals.willCallSubContract =
            _cardLevel.tokenId == 0 &&
            nftContract.cards(_cardId).subContract != address(0);
        if (_locals.willCallPool || _locals.willCallSubContract) {
            ...
            if (_locals.willCallSubContract) {
                subContract = IRainiCustomNFT(
                    nftContract.cards(_cardId).subContract
                );
            }
        }
        ...
        for (uint256 i = 0; i < _tokenIds.length; i++) {
            ...
            require(
                _tempTokenVars.cardId == cost[_locals.currentCard].cardId,
                "card mismatch"
            );
            ...
            if (_tempTokenVars.level == 0) {
                ...
                cost[_locals.currentCard].amount -= uint32(
                    _burnAmounts[i] * _tempCardLevel.conversionRate
                );
            }
            nftContract.burn(
                _msgSender(),
                ...
                if (_locals.isFinished) {
                    if (_locals.willCallPool) {
                        nftStakingPool.mergeTokens(
                            nftContract.maxTokenId() + 1,
                            mergedTokensIds,
                            address(nftContract)
                        );
                    }
                    if (_locals.willCallSubContract) {
                        subContract.onMerged(
                            nftContract.maxTokenId() + 1,
                            mergedTokensIds,
                            address(nftContract),
                            _data
        ...
        (_locals.success, ) = _msgSender().call{
            value: msg.value - mergeFees[_level]
        }(""); // refund excess Eth
