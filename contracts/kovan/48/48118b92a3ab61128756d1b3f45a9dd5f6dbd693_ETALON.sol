...
abstract contract RewardableErc20 is ERC20, MaxWalletDynamic {
    ...
    function getRewardForBalance(uint256 balance)
        public
        view
        returns (uint256)
    {
        return
            (rewardIntervalStartRewards * balance) /
    ...
    function getRewardCount(address account) external view returns (uint256) {
        ...
        return getRewardForBalance(_balances[account]);
    }
    ...
    function _claimReward(address account) private {
        ...
        uint256 reward = getRewardForBalance(_balances[account]);
...
abstract contract TradableErc20 is RewardableErc20, Ownable {
    ...
    function getLiquidityImpactFee(uint256 amount)
        ...
        returns (uint256)
    {
        uint256 count = (_balances[uniswapV2Pair] * 25) / 3; // 8.33% i.e. 30% fee is for 5% liquidity
