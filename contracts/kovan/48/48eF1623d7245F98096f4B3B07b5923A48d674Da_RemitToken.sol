...
contract RemitToken is ERC20Capped, Ownable, ReentrancyGuard {
    ...
    address public immutable i_circulationSupplyExchangeAddress;
    address public immutable i_stakingAndFarmingContractAddress;
    ...
    uint256 public constant TEAM_AND_ADVISORS_MONTHLY_AMOUNT = TEAM_AND_ADVISORS_AMOUNT / 12;
    uint256 public constant DEVELOPMENT_FUND_MONTHLY_AMOUNT = (DEVELOPMENT_FUND_AMOUNT * 20) /100;

    /**@dev Creates the ERC20 token REMIT.
     * @param _circulationSupply address of the contract for swapping pool in Uniswap.
     * @param _stakingAndFarmingContract address of contract for staking and farming.
     ...
     */
    constructor(
        address _circulationSupply,
        address _stakingAndFarmingContract,
        ...
        if (!_circulationSupply.isContract()) {
            revert NotContract(_circulationSupply);
        }
        if (!_stakingAndFarmingContract.isContract()) {
            revert NotContract(_stakingAndFarmingContract);
        }
        ...
        (
            i_owner,
            i_circulationSupplyExchangeAddress,
            i_stakingAndFarmingContractAddress,
            i_teamAndAdvisors,
            i_developmentFund,
            i_marketingFund,
            i_reserveFund
     ...
     */
    function issueToken() public onlyOwner nonReentrant{
        ...
        if(mod12Value == 0) {
            _mint(i_circulationSupplyExchangeAddress, CIRCULATION_SUPPLY_AMOUNT);
            _mint(i_stakingAndFarmingContractAddress, STAKING_AND_FARMING_AMOUNT);
        ...
        _mint(i_teamAndAdvisors, TEAM_AND_ADVISORS_MONTHLY_AMOUNT);
     ...
     */
    function burnToken(uint256 _amount) public nonReentrant{
