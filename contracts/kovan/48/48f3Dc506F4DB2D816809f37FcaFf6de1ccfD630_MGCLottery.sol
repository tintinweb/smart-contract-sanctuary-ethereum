...
contract MGCLottery is Ownable {
    ...
    constructor(address _treasuryAddress, address _asset2BuyAdress) {
        require(_treasuryAddress != address(0), "MGCLottery:: _treasuryAddress invalid");
        treasuryAddress = _treasuryAddress;
        asset2BuyAdress = _asset2BuyAdress;
        addPool(1 ether, 1000 ether, 100, 1, 3, 1 days);
        addPool(1 ether, 1000 ether, 1000, 1, 3, 10 days);
        addPool(1 ether, 1000 ether, 10000, 1, 3, 100 days);
    }
    function setAsset2BuyAdress(address _asset2BuyAdress) internal onlyOwner {
        asset2BuyAdress = _asset2BuyAdress;
    }
    function getUserSpins(address _user, uint _limit, uint skip) external view returns(Spin[] memory list, uint totalItem) {
        uint limit = _limit < users[_user].spins.length ? _limit : users[_user].spins.length;
        for(uint i = skip; i < limit; i++) {
            list[i] = users[_user].spins[i];
        }
    ...
    function setTop10Winner(uint _winAmount) internal {
        users[msg.sender].totalWin += _winAmount;
    ...
    function spin(uint _pid, uint nonce, uint number, uint amount) public {
        ...
        setTop10Player(amount);
        bool result = random(nonce, p.decimal) == number;
        user.spins.push(Spin(number, amount, result));
        if(result) {
            uint reward = amount * (p.decimal - 1);
            user.totalWin += reward;
            user.rewards.push(block.timestamp);
            user.rewardInfo[block.timestamp] = RewardIno(reward, block.timestamp + p.lockTime);
            setTop10Winner(reward);
        }
    }
    function addPool(uint _mintTokenAmount, uint _maxTokenAmount, uint _decimal, uint _referCommission, uint _holderReward, uint _lockTime) public onlyOwner {
        require(_mintTokenAmount > 0 && _maxTokenAmount > _mintTokenAmount, "MGCLottery::addPool _price invalid");
        pools.push(Pool(_mintTokenAmount, _maxTokenAmount, _decimal, _referCommission, _holderReward, _lockTime, true));
    }
