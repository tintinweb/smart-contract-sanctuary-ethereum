...
contract ASATOMO is ERC721AUpgradeable, AccessControlUpgradeable {
  ...
  // change constructor to initialize
  ...
  function mint(uint256 _mintAmount) public onlyRole(MINTER_ROLE) {
    require(totalSupply() + _mintAmount <= maxSupply, "Max supply exceeded!");
    _safeMint(_msgSender(), _mintAmount);
  }

  function adminMint(address to, uint256 qty) external onlyRole(MINTER_ROLE) {
        require(totalSupply() + qty <= maxSupply, "Max supply exceeded!");
        _safeMint(to, qty);
  }

  function bulkMint(address[] calldata _to, uint256[] calldata _id) public onlyRole(MINTER_ROLE) {
