...
contract PrecogV5 is IPrecogV5 {
    ...
    function updateCurrentTradingCycle(address token) public override {
        ...
        while (uint48(block.timestamp) >= lastTradingCycle.endTime) {
            ...
            if(isUpdateUnitTradingCycle[token][lastTradingCycle.id] == false) {
    ...
    function _updateAdjustment(address _token) internal {
        ...
        {
            uint48 _nextFundingTimestamp = PrecogV5Library._calculateNextFundingTimestamp(_firstTradingCycle, lastFundingDuration[_token]);
        ...
        {
            uint48 _nextDefundingTimestamp = PrecogV5Library._calculateNextDefundingTimestamp(_firstTradingCycle, lastDefundingDuration[_token]);
    ...
    function _increaseInvestment(
        ...
        if(isUpdateUnitTradingCycle[_token][_futureTradingCycle.id] == false) {
            totalUnitsTradingCycle[_token][_futureTradingCycle.id] += (liquidity[_token] * (_futureTradingCycle.endTime - _futureTradingCycle.startTime));
    ...
    function _decreaseInvestment(
        ...
        Cycle memory _futureTradingCycle;
        {
            ...
            _nextDefundingTimestamp = PrecogV5Library._calculateNextDefundingTimestamp(_firstTradingCycle, lastDefundingDuration[_token]);
        ...
        if(isUpdateUnitTradingCycle[_token][_futureTradingCycle.id] == false && _nextDefundingTimestamp > _futureTradingCycle.startTime) {
            totalUnitsTradingCycle[_token][_futureTradingCycle.id] += (liquidity[_token] * (_futureTradingCycle.endTime - _futureTradingCycle.startTime));
            isUpdateUnitTradingCycle[_token][_futureTradingCycle.id] = true;      
        }
        if (_lastInvestmentOf.idChanged < _futureTradingCycle.id && _nextDefundingTimestamp > _futureTradingCycle.startTime) {
            _newInvestmentOf.unit = _lastInvestmentOf.amount * (_futureTradingCycle.endTime - _futureTradingCycle.startTime) - _unit;
            investmentsOf[_token][_account].push(_newInvestmentOf);
        } else {
            if(_nextDefundingTimestamp == _futureTradingCycle.startTime) {
                _unit = 0;
            } 
            _newInvestmentOf.unit = _lastInvestmentOf.unit - _unit;
            investmentsOf[_token][_account][_investmentsOf.length - 1] = _newInvestmentOf;
    ...
    function _withdrawBeforeFundingTime(address _account, address _to, address _token, uint _amount, uint _fee) internal {
        require(_amount <= availableDepositedAmount(_token, _account), "PrecogV5: INVESTMENT_HAS_ALREADY_STARTED");
        require(_amount >= core.minDefunding(_token), "PrecogV5: MUST_WITHDRAW_GREATER_THAN_MIN_DEFUNDING_AMOUNT");
