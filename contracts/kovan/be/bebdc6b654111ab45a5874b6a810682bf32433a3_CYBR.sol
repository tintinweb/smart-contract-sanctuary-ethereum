...
/**
    ...
    bots or contracts. If the Cyber contract detects the use of a unapproved bot, via its smart contract bot 
    mitigation functions, it will result in a blacklist and consequently a loss of funds. If your address has 
    been falsely blacklisted, please contact the team within 7 days. Depending on the specific situation, the 
    revision to unblacklist may occur, in which case the funds will be returned. However we do charge a fee 
    for processing this and interacting with the smart contract. You can check and see if your contract is 
    supported by using the checkProtectedAddress or showProtectedAdresses function.
 */
...
/**
    ...
    We want to thank the CYBR community who have helped us embark on this journey throughout the course of
    the last 7 months. 
...
contract CYBR is Context, IERC20, IERC20Metadata, Ownable {
    ...
    uint256 private _robinHoodProtectionTime = 300;            // 7 days
    ...
    function setRobinHoodProtectionTime(uint256 _time) external onlyOwner {
        require(_time >= 300, "Cannot be set to less than 7 days(604800 seconds).");
