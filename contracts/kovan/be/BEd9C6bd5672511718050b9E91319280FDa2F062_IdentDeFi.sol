...
contract IdentDeFi is ERC721URIStorage, Ownable, ChainlinkClient {
  ...
  function revoke(uint256 tokenId) public onlyOwnerOrHolder(tokenId) {
  ...
  function tokenBalance(address _tokenContract) onlyOwner view external returns (uint256) {
  ...
  function withdrawToken(address _tokenContract, uint256 _amount) onlyOwner external {
    IERC20 token = IERC20(_tokenContract);
    require(token.transfer(msg.sender, _amount), "Error: withdrawing token");
  }
...
abstract contract ERC721 is IERC721 {
  ...
  function _transfer(
    ...
    revert('IdentDeFI: Cannot transfer assigned tokens');
