    ...
    **/
    function lenderShare(address user, uint8 productId, uint index, uint deposit, uint withdraw, bool isIn) private {
        ...
        if (product.debt == 0) {
            ...
            if (deposit == 0) {
                position_map[user][index].share -= withdraw / lendingPrice(productId);
            } else if (withdraw == 0) {
                position_map[user][index].share += deposit / lendingPrice(productId);
            }
        }
    }
    ...
    **/
    function borrowerShare(address user, uint8 productId, uint index, uint borrow, uint repay, bool isIn) private {
        ...
        if (product.debt == 0) {
            ...
            if (borrow ==0) {
                loan_map[user][index].share -= repay / borrowingPrice(productId);
            } else if (repay ==0) {
                loan_map[user][index].share += borrow / borrowingPrice(productId);
            }
        }
    }
