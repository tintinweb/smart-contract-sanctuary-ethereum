...
contract Token is ERC20BurnableUpgradeSafe {
    ...
    event SetStartBlock(uint256 startTime);
    ...
    function setStartBlock(uint256 _startTime) external onlyOwner{
        startTime = _startTime;
        emit SetStartBlock(_startTime);
    }
    ...
    function verifyBuySellConditions(address from, address to, uint256 amount) internal view{
        if(pairs[from] || pairs[to]){
            require(startTime < block.number, "Token not available for trade");
            if(endBlock>block.number){
                require(amount <= block.number.sub(startTime).mul(tgeAmount).div(blocks), "Trade amount reached");
            }
        }
...
contract ERC20UpgradeSafe is Initializable, ContextUpgradeSafe, IERC20, Ownable {
    ...
    function pauseToken() external returns (bool){
        ...
        require(!paused, "pauseToken: Already paused");
        paused = true;
        return true;
    }
     /**
     ...
     */
     function unpauseToken() external returns (bool){
        ...
        require(paused, "unpauseToken: Already unpaused");
        paused = false;
        return true;
    }
    ...
    */
    function mintNewTokens(address recipient, uint256 amount) external virtual returns (bool) {
        require(minter[msg.sender], "Call must come from Timelock");
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual {
        ...
        require(!paused,"Transfer: Token is paused");
