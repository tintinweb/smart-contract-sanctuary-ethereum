...
contract PoolInvestor is IpoolInvestor, Investor, ERC20, Ownable {
    ...
    constructor(
        ...
        "Roci Debt Token",
        string(
            abi.encodePacked(_prefix, IERC20Metadata(_token).symbol(), _postfix)
        )
    )
    Ownable()
    {
    ...
    function revenueManager() private view returns (IRevenueManager) {
        return IRevenueManager(lookup(Role.revManager));
    }    
    ...
    * send all funds to RevenuManager after
        ...
        checkVersion(version)
    {
        ...
        for (uint256 i = 0; i < _ids.length; i++) {
            ...
            stakingIndexes[_id] = bonds.stake(_id, bal);
        }
    ...
    *   also ensures we can track funds lost
    ...
    function _checkAvailable(uint256 _amount) internal override {
        ...
        uint available = revManager.balanceAvailable(address(this));
    ...
    function _sendAllToRevManager() internal {
        ...
        revenueManager().payment(address(token), bal);
    }
  ...
  Version
{
  ...
  constructor(IAddressBook _addressBook, IERC20Metadata _token)
  ...
  {
    IBonds bonds = IBonds(lookup(Role.bonds));
    IRociPayment paymentc = IRociPayment(
      lookup(Role.paymentContract)
    );

    require(
      paymentc.hasSameScoreForActiveLoans(msg.sender, args._NFCSID),
      Errors.INVESTOR_BORROW_WITH_ANOTHER_SCORE
    );

    // set up loan config here
    _checkAvailable(args._amount);
    uint256 accrualPeriod = 60 * 60;
    uint256 periodsInYear = 12 * 24 * 30;
    ...
    bonds.newLoan(lookup(Role.paymentContract), id, args._hash, args._signature);

    require(
      bonds.balanceOf(address(this), id) == args._amount,
      Errors.INVESTOR_ISSUE_BONDS
    );
