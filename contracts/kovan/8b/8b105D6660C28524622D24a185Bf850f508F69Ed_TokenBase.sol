...
contract TokenBase is Ownable {
    ...
    function isStationEnrolled(address _station) public returns (bool) {
        for (
            uint256 allowedTokenCounter = 0;
            allowedTokenCounter < stationList.length;
            allowedTokenCounter++
        ) {
            if (stationList[allowedTokenCounter] == _station) return true;
        }
        return false;
    }
