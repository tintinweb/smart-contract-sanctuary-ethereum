    ...
    UUPSUpgradeable
{
        ...
        returns (uint256)
    {
        ...
        require(_gaming[_msgSender()][game_].gameType == 3, "Scorefam: Invalid Game Type");
            //Base
  ...
  //Convert String to Uint
    ...
    function sliceInterruptedBytes(uint256 _fixtureID) internal view returns(bytes memory) {
        return IScorefamConsumer(consumer).requestIdData(IScorefamConsumer(consumer).gameScoreRequestID(_fixtureID)).slice(2,11);
    }
    ...
    function claimGameReward(uint256 game_) external returns (bool){
        ...
        if(!gameResult[fixtureID].retrieved){
            sliceFinishedBytes(fixtureID, IScorefamConsumer(consumer).requestIdData(IScorefamConsumer(consumer).gameScoreRequestID(fixtureID)));
        }
        ...
        _mint(fee, amount_.div(100)); //Charging 1% Fee
        _mint(_msgSender(), amount_.sub(amount_.div(100)));
    ...
    function claimSlipReward(uint256 game_) external returns (bool){
        ...
        for(uint256 i = 0; i < _gamingSlip[_msgSender()][game_].slips.length; i++){
            if(!gameResult[_gamingSlip[_msgSender()][game_].slips[i].fixtureID].retrieved){
                sliceFinishedBytes(
                        _gamingSlip[_msgSender()][game_].slips[i].fixtureID, 
                        IScorefamConsumer(consumer).requestIdData(IScorefamConsumer(consumer).gameScoreRequestID(_gamingSlip[_msgSender()][game_].slips[i].fixtureID))
                        );
            }
