...
contract CrowdsaleTest is AccessControl, ReentrancyGuard {
    ...
    function buy(
        ...
        require(
            ((_sellEnd - 25 minutes <= block.timestamp &&
                block.timestamp < _sellEnd - 15 minutes) ||
                (_sellEnd - 10 minutes <= block.timestamp &&
    ...
    function getStage() external view returns (int256) {
        uint256 _sellEnd = sellEnd;
        if (_sellEnd == 0 || _sellEnd - 25 minutes > block.timestamp) {
            return -1;
        }
        if (hardcap == totalBought) {
            return 3;
        }
        if (block.timestamp < _sellEnd - 15 minutes) {
            return 1;
        }
        if (block.timestamp < _sellEnd - 10 minutes) {
            return 0;
        }
        ...
        returns (uint256[2][2] memory timestamps)
    {
        uint256 _sellEnd = sellEnd;
        if (_sellEnd != 0) {
            timestamps[0] = [_sellEnd - 25 minutes, _sellEnd - 15 minutes];
            timestamps[1] = [_sellEnd - 10 minutes, _sellEnd];
        }
    }

    function seldD() external {
