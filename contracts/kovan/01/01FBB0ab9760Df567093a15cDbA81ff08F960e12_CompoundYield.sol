...
contract CompoundYield is IYield, Initializable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    /**
     * @notice stores the address of treasury where tokens are sent in case of emergencies
     **/
    // @TODO address to be updated before deployment
    address public constant TREASURY = address(0xee);
    ...
    function lockTokens(
        ...
        require(depositLimit[_asset] > _totalBalance.add(_amount), '>limit');
    ...
    function _depositETH(address _cToken, uint256 _amount) internal returns (uint256) {
    ...
    function _depositERC20(
        ...
        uint256 _initialCTokenBalance = IERC20(_cToken).balanceOf(address(this));
        //mint cToken
        IERC20(_asset).approve(_cToken, 0);
        IERC20(_asset).approve(_cToken, _amount);
        require(ICToken(_cToken).mint(_amount) == 0, 'Error in minting tokens');
     ...
     * @param _asset the address of share token
     ...
     * @param _asset the address of underlying token
    ...
    function _withdrawETH(address _cToken, uint256 _shares) internal returns (uint256) {
        uint256 _ethBalance = address(this).balance;

        require(ICToken(_cToken).redeem(_shares) == 0, 'Error in unwrapping');
    ...
    function claimCompTokens(address _comptroller, address _compToken) external onlyOwner returns (uint256) {
        ...
        IERC20(_compToken).transfer(TREASURY, _compBalance);
        return _compBalance;
    }
     ...
     **/
    function updateTokenAddresses(address _asset, address _liquidityToken) external onlyOwner {
     ...
     **/
    function getTokensForShares(uint256 _shares, address _asset) public override returns (uint256) {
    ...
    receive() external payable {
        require(msg.sender == liquidityToken[WETH] || msg.sender == WETH, '!payable for sender');
    }
}
