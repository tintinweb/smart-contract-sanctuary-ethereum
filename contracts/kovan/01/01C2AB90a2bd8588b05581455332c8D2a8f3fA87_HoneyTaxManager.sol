...
contract HoneyTaxManager is OwnableUpgradeable {
    ...
    uint256 private teamAlloc;
    uint256 private liqAlloc;
    uint256 private burnAlloc;
    ...
    uint256 public burnSellPtg;
    bool public burnSellFee;
    ...
    function shouldDoSendToTeam() internal view returns (bool) {
        return taxActionCounter + teamActionOffset % taxActionTrigger == 0 && teamAlloc > 0;
    }

    function shouldDoSendToLiquidity() internal view returns (bool) {
        return taxActionCounter + liqActionOffset % taxActionTrigger == 0 && liqAlloc > 0;
    }
    ...
    function getBuyTax(address user, uint256 amount) external returns (uint256) {
        ...
        return amount * (buyTaxPtgBase - buyTaxLiqPtg - buyTaxTeamPtg) / buyTaxPtgBase;
    }
    ...
    function getSellTax(address user, uint256 amount) external returns (uint256) {
        ...
        if (burnSellFee) {
             if (burnSellPtg != 0)
                burnAlloc += amount * burnSellPtg / sellTaxPtgBase;

            return amount * (sellTaxPtgBase - burnSellPtg) / sellTaxPtgBase;
        } else {
            if (sellTaxTeamPtg != 0)
            ...
            return amount * (sellTaxPtgBase - sellTaxLiqPtg - sellTaxTeamPtg) / sellTaxPtgBase;
        }
    }

    function getSpendTax(address user, uint256 amount) external returns (uint128) {
        return 0;
        if (shouldDoSendToTeam())
            sendToTeam();
        if (shouldDoSendToLiquidity())
        ...
        return uint128(amount * (spendTaxPtgBase - spendTaxLiqPtg - spendTaxTeamPtg) / spendTaxPtgBase);
    }
    ...
    function setSellTax(uint256 team, uint256 liq, uint256 base) external onlyOwner {
        sellTaxTeamPtg = team;
        sellTaxLiqPtg = liq;
        sellTaxPtgBase = base;
    }
    ...
    function setSellTaxBurn(uint256 ptg, bool burn) external onlyOwner {
        burnSellPtg = ptg;
        burnSellFee = burn;
    }
}
...
interface IHoney {
    function spendEcoSystemBalance(address user, uint128 amount, uint256[] memory flowersWithBees) external;
