...
contract Categories is ICategories, Ownable {
    ...
    mapping(uint8 => mapping(bytes32 => uint256)) public _excellentIndex;

    mapping(uint8 => bytes32[])private _excellent;

    event AddedExcellent(uint8 carbonType,bytes32 collectionHash);
    event RemovedExcellent(uint8 carbonType,uint256 index);

    constructor(address member_, address sharedToken_) {
    ...
    function addExcellent(uint8 carbonType ,bytes32 collectionHash) external {
        ...
        require(_excellentIndex[carbonType][collectionHash] <= 0,"Collectionhash used");
        _excellent[carbonType].push(collectionHash);
        _excellentIndex[carbonType][collectionHash] = _excellent[carbonType].length;

        emit AddedExcellent(carbonType,collectionHash);
    }

    function removeExcellent(uint8 carbonType,uint256 index) external {
        require(index < _excellent[carbonType].length, "Index exceeds maximum");
        bytes32 collectionHash = _excellent[carbonType][index];

        require(_excellentIndex[carbonType][collectionHash] > 0, "Index does not exist");

        delete _excellentIndex[carbonType][collectionHash];
        delete _excellent[carbonType][index];
