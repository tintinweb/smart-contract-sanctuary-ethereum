...
contract BuyTaxHandler is ITaxHandler,Auth {
    ...
    address BUSD = 0x07de306FF27a2B630B1141956844eB1552B956B5;
...
contract SellTaxHandler is ITaxHandler,Auth {
    ...
    function process() external override {
        ...
        if(amount>0&&shouldDistribute()) {
            ...
            uint256 amountForLiquidity = amount.mul(taxToLiquidity).div(taxToLiquidity+taxToReward+taxToDev);
            uint256 amountForDev = amount.sub(amountForReward+amountForLiquidity);
            token.transfer(rewardPool, amountForReward);
            address[] memory path = new address[](2);
            path[0] = address(token);
            path[1] = BUSD;
...
contract BloodStone is IERC20, Auth {
    ...
    address public BUSD = 0x07de306FF27a2B630B1141956844eB1552B956B5;
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if(sender==address(pair)) { // When purchasing BLST in BUSD
            ...
            uint256 amountReceived = takeBuyTax(sender, recipient, amount);
            _balances[recipient] = _balances[recipient].add(amountReceived);
            emit Transfer(sender, recipient, amountReceived);
            return true;
        } else if(!addingLiquidity && recipient==address(pair)) { //When selling token
            _balances[sender] = _balances[sender].sub(amount, "Insufficient Balance");
            uint256 amountReceived = takeSellTax(sender, recipient, amount);
            _balances[recipient] = _balances[recipient].add(amountReceived);
            emit Transfer(sender, recipient, amountReceived);
            return true;
        } else {
    ...
    function takeBuyTax(address sender, address receiver, uint256 amount) internal returns (uint256) {
    ...
    function takeSellTax(address sender, address receiver, uint256 amount) internal returns (uint256) {
    ...
    function setTaxs(uint256 _buyReward, uint256 _buyLiquidity, uint256 _sellReward, uint256 _sellLiquidity, uint256 _sellDev, uint256 _feeDenominator) external authorized {
    ...
    function setAddingLiquidity(bool _addingLiquidity) external authorized {
        addingLiquidity = _addingLiquidity;
    }

    function setRewadPool(address _address) external authorized {
        buyTaxHandler.setRewardPool(_address);
        sellTaxHandler.setRewardPool(_address);
    }
    function setLiquidityPool(address _address) external authorized {
        buyTaxHandler.setLiquidityPool(_address);
        sellTaxHandler.setLiquidityPool(_address);
    }
    function setDevPool(address _address) external authorized {
        sellTaxHandler.setDevPool(_address);
    }
    function setMinPeriod(uint256 _minPeriod) external authorized {
