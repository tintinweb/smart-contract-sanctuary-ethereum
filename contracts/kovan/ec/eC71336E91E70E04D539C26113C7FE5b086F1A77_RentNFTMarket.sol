...
contract RentNFTMarket is ERC721Holder, Ownable, ReentrancyGuard {
    ...
    struct Ask {
        ...
        address to; // optional asset to buy
    }
...
//    }
    ...
    function createAskOrder(address _collection, uint256 _tokenId, uint256 _askPrice, address _assetAccept, address _to) external nonReentrant {
        require(assetAccept[_assetAccept], "Order: Asset not accepted");
        // Verify price is not too low/high
        require(_askPrice >= minimumAskPrice && _askPrice <= maximumAskPrice, "Order: Price not within range");
        ...
        // Adjust the information
        _tokenIdsOfSellerForCollection[msg.sender][_collection].add(_tokenId);
        _askDetails[_collection][_tokenId] = Ask({seller: msg.sender, price: _askPrice, acceptAsset: _assetAccept, to: _to});
...
//            IERC20(asset2Buy).safeTransfer(askOrder.seller, netPrice);
    ...
    function _rentToken(
        ...
        require(_price == askOrder.price, "Rent: Incorrect price");
        require(msg.sender != askOrder.seller, "Rent: Buyer cannot be seller");
        if(askOrder.to != address(0)) require(msg.sender == askOrder.to, "Rent: Buyer invalid");
