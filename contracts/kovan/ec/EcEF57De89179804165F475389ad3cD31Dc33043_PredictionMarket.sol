...
contract PredictionMarket is AdminManaged,ReentrancyGuard {
    ...
    event UpdateMaket(uint id, string state) ;
   ...
   constructor (
    ...
    function updateMarket(uint _id)
        public
        onlyTrustedSource
        marketLive(_id)
        returns(bool success)
    {
        require(isMarket(_id),"Invalid market id");
        markets[_id].predictionStatus = PredictionStatus.InSettlement;
        emit UpdateMaket(_id,"Insettlement");
        return true;
    }
