...
contract Uchu {
    ...
    **/
    ...
    function singleBalance(Product memory product) private view returns (uint256) {
        ...
        uint b = 60;
        uint pow = a/b;
        return amount / (100 ** pow) * ((product.yeildNum + product.totalNum) ** pow);
    }

    function getBalance(address client, string memory name) public payable returns (uint256) {
        finalBalance[client][name] = singleBalance(curLenderPro);
