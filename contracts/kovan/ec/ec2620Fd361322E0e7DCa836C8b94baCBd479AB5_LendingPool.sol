...
contract LendingPool is ILendingPool, Ownable {
    ...
    /// @dev Initialize the pool with all the user provided settings
    /// @param _owner Deployer of the pool
    /// @param _mintRatio Mint ratio in wei
    /// @param _colToken Address of the collateral token
    /// @param _lendToken Address of the token that will be lend out
    /// @param _oracle Address of the Vendor oracle. Borrowing is not allowed when price of col. is bellow mint ratio
    /// @param _feeRate Fee that will go to the lender in this pool. Set by pool deployer and owner
    /// @param _protocolFee Fee that goes to Vendor, set by the pool factory
    /// @param _expiry Epoch timestamp of the expiry. In seconds
    /// @param _treasury Address of the Vendor treasury that all the protocol fees will be transfered to
    function initialize(
        address _owner,
        uint256 _mintRatio,
        address _colToken,
    ...
    /* ========== PUBLIC FUNCTIONS ========== */
    ///@dev Deposit the funds you would like to lend. Prior approval of lend token is required
    ///@param _depositAmount Amount of lend token to deposit into the pool
    function deposit(uint256 _depositAmount) external onlyOwner {
        _pullTokensFrom(msg.sender, lendToken, _depositAmount);
        emit Deposit(_depositAmount);
    }

    ///@dev Withdraw the lend token from the pool
    ///@param _amount Amount of lend token to withdrwa from the pool
    function withdraw(uint256 _amount) public onlyOwner {
        if (
            _amount >
            _sub(
                lendToken.balanceOf(address(this)),
                ((totalFees * protocolFee) / 10000)
            )
    ...
    ///@dev Shortcut for a simple borrow
    ...
    ///@dev Borrow on behalf of a wallet. Usefull on rollovers between pools deployed by two different lenders
    ...
    ///@dev Register a users debt in from the old pool
    ///@param _borrower User who's loan is migrating into this pool
    ///@param _colDepositAmount Amount of collateral that is migrating
    ///@param _debtAmount Amount of debt that is migrating
    function rollIn(
        address _borrower,
        ...
        if (!factory.pools(msg.sender)) revert NotAPool();
        if (!_isValidPrice()) revert NotValidPrice();
        if (block.timestamp > expiry) revert PoolClosed();
    ...
    ///@dev Rollover loan into a pool that has been deployed by the same lender as the original one
    ...
    ///@dev Rollover availiable lent funds into a new pool
    ///@param _newPool Address of the destination pool
    function lenderRollOver(address _newPool) public onlyOwner {
    ...
    ///@dev Repay the loan of msg.sender convenience shortcut
    ///@param _repayAmount Amount of lend tokens to repay
    function repay(uint256 _repayAmount) external {
        repayOnBehalfOf(msg.sender, _repayAmount);
    }

    ///@dev Repay the loan on behalf of a different wallet
    ...
    ///@dev Collect the interest, defaulted collateral and pay vendor fee
    ...
    /* ========== PRIVATE FUNCTIONS ========== */
    ///@dev Make sure new pool can be rolled into
    function _validateNewPool(ILendingPool _pool) private view {
        ...
        if (address(_pool.colToken()) != address(colToken))
            revert DifferentColToken();

        if (address(IOwnable(address(_pool)).owner()) != address(owner()))
            revert DifferentPoolOwner();

        if (_pool.expiry() <= expiry) revert ExpiryNotLongEnough();
    }

    ///@dev Pay the outstanding vendor fee. Triggered on lender rollover and collect
    ...
    ///@dev Compute the amount of lend tokesn to send given collateral deposited.
    ...
    ///@dev Compute the amount of collateral to return given repayment amount. Does not account fees.
    ...
    ///@dev Check if the borrowing is enables in this pool based on the oracle
    ...
    /* ========== SETTERS ========== */
    ///@dev Update the fee rate for the new coming borrowers
    ///@param _newFeeRate in basis points 1% - 100
    ...
    ///@dev Pull the tokens from wallet and ensure balance increase
    ...
    ///@dev Transfer tokens with overflow protection
    ...
    ///@dev Subtract with 0 floor ex:5-3=2; 3-5=0
    ///@param a Number to subtract from
    ///@param b Number to subtract
    ///@return Flored difference: ex:5-3=2; 3-5=0
