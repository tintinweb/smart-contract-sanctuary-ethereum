...
contract Staker {
  ...
  modifier notCompleted() {
    require(block.timestamp >= deadline && !completed, 'already completed or deadline not yet passed');
    _;
  }
  ...
  function stake() external payable stakingOpen {
    require(block.timestamp < deadline, 'deadline already passed');
    balances[msg.sender] += msg.value;
    emit Stake(msg.sender, msg.value);
  }

  receive() external payable stakingOpen {
    require(block.timestamp < deadline, 'deadline already passed');
    this.stake();
  }
