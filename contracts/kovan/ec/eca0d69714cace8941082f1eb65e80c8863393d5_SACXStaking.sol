...
contract SACXStaking{
    ...
    constructor(ISACXToken tokenaddress) public{
     ...
     function MintSynths(address staker,string calldata _synth, uint256 amount) external {
        ...
        uint256 totalsynthprice=synthPriceinUSD.mul(amount);
        uint256 collatteralToStake = (totalsynthprice.mul((collatteralRatio).div(100)));
        require(SACXToken.balanceOf(staker) >= collatteralToStake,"User does not have sufficient SACX to Mint synths");
        SACXToken.transferFrom(staker,address(SACXToken),collatteralToStake);
        StakerInfo[staker].TotalSACXStakedasCollateral= StakerInfo[staker].TotalSACXStakedasCollateral.add(collatteralToStake);
        emit Transfer(staker,address(SACXToken),collatteralToStake);
        synthToken.mint(amount,staker);
