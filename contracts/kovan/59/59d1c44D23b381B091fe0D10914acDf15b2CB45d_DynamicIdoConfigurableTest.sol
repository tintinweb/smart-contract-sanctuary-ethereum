...
import "../ido/DynamicIdoConfigurable.sol";

// solhint-disable not-rely-on-time
contract DynamicIdoConfigurableTest is DynamicIdoConfigurable {
    constructor(
        uint256 _tokenPrice,
    ...
    function testSetInitialUnlock(uint256 _initialUnlock) external {
        initialUnlock = _initialUnlock;
    }

    function testSetCliff(uint256 _cliff) external {
        cliff = _cliff;
    }

    function testSetVesting(uint256 _vesting) external {
        vesting = _vesting;
    }

    function testSetVestingInterval(uint256 _vestingInterval) external {
