...
contract AutoLiq is FlashLoanReceiverBase {
    ...
    address public cTokenBorrow;
    address public cTokenCollateral;
    address public borrower;
    address public aaveToken;
    mapping(bytes => mapping(string => address)) public liquidations;
...
//        require(msg.sender == owner, "only owner can call this");
    ...
    constructor(address _aavePool, address _compoundComptroller, address _uniswapRouter,address _borrower, address aave_tokenBorrow, address _tokenBorrow, address _cTokenBorrow, address _cTokenCollateral)
    public
...
//    FlashLoanReceiverBase()
    {
        ...
        borrower = _borrower;
        aaveToken = aave_tokenBorrow;
        tokenBorrow = _tokenBorrow;
        cTokenBorrow = _cTokenBorrow;
        cTokenCollateral = _cTokenCollateral;
    }

    function beginLiquidate()
    public
    onlyOwner
    returns (uint)
    {

        uint repayAmount = this.getRepayAmount(borrower, cTokenBorrow);
//        IERC20(aave_tokenBorrow).transferFrom(msg.sender, address(this), repayAmount);

//        bytes memory by = addressToBytes(_borrower);
//
//        liquidations[by]["borrower"] = _borrower;
//        liquidations[by]["cTokenBorrow"] = _cTokenBorrow;
//        liquidations[by]["cTokenCollateral"] = _cTokenCollateral;
//        liquidations[by]["tokenBorrow"] = _tokenBorrow;
//
//        borrower = _borrower;
//        cTokenBorrow = _cTokenBorrow;
//        cTokenCollateral = _cTokenCollateral;
//        tokenBorrow = _tokenBorrow;
        ...
        flashLoan("", aaveToken, repayAmount);
        return repayAmount;
    }
...
//        address _borrower = liquidations[params]["borrower"];
//        address _cTokenBorrow = liquidations[params]["cTokenBorrow"];
//        address _cTokenCollateral = liquidations[params]["cTokenCollateral"];
//        address _tokenBorrow = liquidations[params]["tokenBorrow"];

        emit Log("borrower", borrower);
        emit Log("cTokenBorrow", cTokenBorrow);
        emit Log("cTokenCollateral", cTokenCollateral);
        emit Log("tokenBorrow", tokenBorrow);

        liquidate(amounts[0], borrower, tokenBorrow, cTokenBorrow, cTokenCollateral);
...
//        IERC20(_tokenBorrow).transferFrom(msg.sender, address(this), _repayAmount);
