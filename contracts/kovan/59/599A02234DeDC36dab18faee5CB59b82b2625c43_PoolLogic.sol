...
library PoolLogic {
  ...
  function depositToYieldProvider(
    ...
    IERC20Upgradeable(pool.parameters.UNDERLYING_TOKEN).safeIncreaseAllowance(address(yieldProvider), normalizedAmount);
    IERC20Upgradeable(underlyingToken).safeTransferFrom(from, address(this), normalizedAmount);
