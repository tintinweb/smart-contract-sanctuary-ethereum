...
contract TinyExplorers is ERC721Enumerable, ReentrancyGuard, Ownable{
    ...
    function claimWithTinyKingdoms(uint256 tinyKingdomsId) public nonReentrant ownerOfTinyKingdom(tinyKingdomsId) tokenAvailable(tinyKingdomsId){
        
        if (tinyKingdomsId> 0 && tinyKingdomsId < 4097) revert TokenIdInvalid();
    ...
    // }
     ...
     function findfirstExplorer() public view returns (uint256){
        ...
        for (uint256 i=1; i< TOTAL_SUPPLY; i++){
            if (explorerExists(i)==false) return i;
        }
    }
