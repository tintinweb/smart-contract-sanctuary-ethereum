...
interface IWithdrawable {
    ...
    */
    function withdraw(address to, string calldata reason) external;
...
abstract contract Withdrawable is AccessControl, RequirementsChecker, ERC721Holder, ERC1155Holder, IWithdrawable {
    ...
    * Emits a {Withdrawal} event.
    */
    function withdraw(address to, string calldata reason) external onlyRole(ADMIN_ROLE) {
 ...
 */
interface IFearWolf is IAccessControl, IRoleContainerMinter {
    function initialMint(address initialOwner, uint256 wolvesCount) external;
...
contract FearWolfDistributor is AccessControl, Pausable, Withdrawable, ReentrancyGuard {
    ...
    */
    ...
    // Timestamps
    uint256 public presaleStart = 1648810800; // 2022-Apr-01 11:00:00 UTC
    uint256 public saleStart = 1649156400; // 2022-Apr-15 11:00:00 UTC
    ...
    */
    ...
    /**
    * @dev FEAR contract address.
    */
    address public fearContractAddress;
    ...
    constructor(address _fearContractAddress) {
        fearContractAddress = _fearContractAddress;
    ...
    function _beforeUnpause() internal view virtual override {
        ...
        require(fearWolfContract.hasRole(fearWolfContract.MINTER_ROLE(), address(this)), "Minter role missing");
    }

    function _beforeWithdrawalERC20(address asset, address) internal virtual override {
        require(asset != fearContractAddress, "FEAR tokens can be withdrawn only by stakers via unstake() function call");
    }
    ...
    function _buyWolf(uint256 wolvesCount, uint256 price) private {
        ...
        fearWolfContract.initialMint(caller, wolvesCount);
    ...
    */
    ...
    /**
    * @dev Returns whitelisted (available to reserve a FEAR Wolf without staking on presale) status for the caller.
    * @return True if the caller is whitelisted, false - otherwise.
    */
    function getMyWhitelistedStatus() external view returns (bool) {
        return whitelistedWithoutStaking[_msgSender()];
    }
    ...
    */
    function reserveWolf(uint256 wolvesCount) external payable nonReentrant {
        ...
        if (!whitelistedWithoutStaking[caller]) {
            ...
            require(fearContract.balanceOf(caller) >= stakeAmount, "You do not have enough FEAR in your wallet to stake");
            require(fearContract.allowance(caller, address(this)) >= stakeAmount, "Contract allowance is less than stake amount");
            ...
            fearContract.transferFrom(caller, address(this), stakeAmount);
    ...
    */
    function unstake() external nonReentrant {
        ...
        fearContract.transfer(caller, stakeAmount);
        emit StakeUpdated(caller, 0);
    }

    /**
    * @dev Allows caller to claim reserved FEAR Wolves.
    * Requirements:
    * - Contract must not be paused;
    ...
    */
    function claimWolves() external whenNotPaused nonReentrant {
        ...
        require(wolvesCount > 0, "You don't have any unclaimed wolf");
        
        wolvesReserved[caller] = 0;
        fearWolfContract.initialMint(caller, wolvesCount);
        emit WolvesBought(caller, presalePrice, wolvesCount);
    }
    ...
    */
    ...
    function setSaleStart(uint256 _presaleStart, uint256 _saleStart) external onlyRole(MANAGER_ROLE) {
    ...
    function setStakePerWolf(uint256 _stakePerWolf) external onlyRole(MANAGER_ROLE) {
    ...
    function setStakeLockPeriod(uint16 _days) external onlyRole(MANAGER_ROLE) {
        require(block.timestamp < presaleStart, "Too late, presale is already active");
    ...
    function giftWolves(address _address, uint256 wolvesCount) external nonReentrant onlyRole(ADMIN_ROLE) {
        require(giftWolvesCount >= wolvesCount, "Not enough wolves left to gift");
        giftWolvesCount -= wolvesCount;
        fearWolfContract.initialMint(_address, wolvesCount);
        emit WolvesBought(_address, 0, wolvesCount);
    }
    ...
    function forceClaimWolf() external nonReentrant onlyRole(ADMIN_ROLE) {
        require(DEBUG_SALE_ACTIVE || block.timestamp >= saleStart, "Public sale is not active yet");
        ...
        for(uint i = 0; i < fearWolvesReservers.length(); i++) {
            ...
            if (wolvesCount > 0) {
                ...
                fearWolfContract.initialMint(user, wolvesCount);
                emit WolvesBought(user, presalePrice, wolvesCount);
            }
        }
    }
    ...
    function resetAll() external onlyRole(ADMIN_ROLE) {
        ...
        stakeLockPeriod = 30;
        presaleWolvesPerAccountLimit = 5;
        batchMintingLimit = 10;
        presaleStart = 1648810800; // 2022-Apr-01 11:00:00 UTC
        saleStart = 1649156400; // 2022-Apr-15 11:00:00 UTC
