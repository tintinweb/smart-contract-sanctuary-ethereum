...
contract ConnextBridgeToken is IBridgeToken, Ownable {
    ...
    mapping(uint128 => address) public assetStable;
    mapping(uint256 => address) public controllerAddress;
    ...
    ) IBridgeToken(_controller) {
        controllerAddress[_chainId] = _controller;
        connext = _connext;
    ...
    function addStable(uint128 _chainId, address _address) external onlyOwner {
        assetStable[_chainId] = _address;
    }

    function callController(uint256 num) external {
        value = num;
    }
    ...
    function _bridgeConnext(
        ...
        (, , destinationChainId) = abi.decode(
            header,
            (address, address, uint128)
        );

        IERC20(assetStable[chainId]).transferFrom(
            address(controller),
            address(this),
            amount
        );
        IERC20(assetStable[chainId]).approve(address(connext), amount);
        //  callData: abi.encodeWithSelector(
        //         ConnextBridgeToken(address(this)).callController.selector,
        //         payload
        //     ),

        bytes4 selector = bytes4(keccak256("callController(uint256)"));
        bytes memory callData = abi.encodeWithSelector(selector, 4);

        CallParams memory callParams = CallParams({
            to: BridgeInfoData[destinationChainId].bridgeAddress,
            callData: callData,
            originDomain: BridgeInfoData[chainId].chainDomainId,
            destinationDomain: BridgeInfoData[destinationChainId].chainDomainId,
            recovery: BridgeInfoData[chainId].bridgeAddress,
        ...
        XCallArgs memory xcallArgs = XCallArgs({
            params: callParams,
            transactingAssetId: assetStable[chainId],
            amount: amount,
            relayerFee: 0
        });
    ...
    function _bridge(
        ...
        _bridgeConnext(amount, chainId, header, payload);
    }
}
...
abstract contract IBridgeToken {
    ...
    function bridge(
        ...
        _bridge(amount, chainId, header, payload);
    }
}
...
contract BridgeController is Ownable {
    ...
    function swap(
        ...
        bytes calldata header //Flexibal for use another services : destinationAssetAddress, destinationChainId
    ) public {
        ...
        _bridge(amount, _chainId, header);
    }
    ...
    function swap2(
        ...
        address destnationAssetAddress;
        address reciever;
        (destnationAssetAddress, reciever, ) = abi.decode(
            header,
            (address, address, uint128)
        );

        _swap2(
            destnationAssetAddress,
            reciever,
            _amount,
            swapContract[_chainId],
            _chainId
    ...
    function _swap(
        ...
        IERC20(_token1).approve(_swapContract, _amount);
        require(
            IERC20(_token1).allowance(address(this), _swapContract) > 0,
            "Not Approve"
        );
    ...
    function _swap2(
        ...
        IERC20(stableCoin[_chainId]).approve(swapContract[_chainId], _amount);
        require(
            IERC20(stableCoin[_chainId]).allowance(
                address(this),
                swapContract[_chainId]
            ) > 0,
            "Not Approve"
        ...
        amount = ISwapTokenUniswapV2(_swapContract)
            .swapExactTokensForTokensDesstination(
                stableCoin[_chainId],
                _tokenOut,
                _amount,
                _destinationAddress,
    ...
    function _bridge(
        ...
        IBridgeToken(registeredBridge[_chainId]).bridge(
            amount,
            _chainId,
            header,
            abi.encodeWithSelector(
                BridgeController(address(this)).swap2.selector,
                amount,
                destinationChainId,
                header
            )
