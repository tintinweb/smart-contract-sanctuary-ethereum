    ...
    IStructs
{
    ...
    /// @notice Error for discount that is over 100%
    ...
    /// @param _oracle Oracle address that will be referenced by pools when deciding on wheather to allow borrowing.
    ...
    /// @param _treasury Vendot fees will be sent to this address
    ...
    function initialize(
        ...
        __Ownable_init();
        __UUPSUpgradeable_init(); //Not required, OCD
        __Pausable_init();
    ...
    /// @param _feeRates Interest that will be due on expiry by borrowers.
    ...
    function deployPool(
        ...
        uint256 discountValue = licenseDiscount();
        ...
        ILendingPool(lendingPool).initialize(
            Data(
                ...
                _expiry,
                _borrowers,
                protocolFee - discountValue,
                protocolColFee - discountValue,
                address(feesManager),
                oracle
            )
        );
        ...
        feesManager.setPoolFees(lendingPool, _feeRates);
        pools[lendingPool] = true;
        return lendingPool;
    }
    ...
    /// @notice Update treasury address
    function setFeeManager(address _feesManager)
        external
        onlyOwner
        whenNotPaused
    {
    ...
    function setProtcolFee(uint256 _protocolFee)
        external
        onlyOwner
        whenNotPaused
    {
    ...
    function flipCollateralAllowList(address _col)
        public
        onlyOwner
        whenNotPaused
    {
        allowList[_col] = !allowList[_col];
    }
    ...
    /// @notice Check if sender has a license, adjust Vendor fee accordingly
    /// @return Vendor fee adjusted for license
    function licenseDiscount() private view returns (uint256) {
        // Call the licence engine and see if user has balance else return 0
        uint256 bal = licenseEngine.balanceOf(msg.sender);
        uint256 maxDiscount = 0;
        if (bal <= 0) {
            return maxDiscount;
        } else {
            for (uint256 index = 0; index < bal; index++) {
                //Find the largest discout and apply it.
                uint256 id = licenseEngine.tokenOfOwnerByIndex(
                    msg.sender,
                    index
                );
                (, , uint256 discout, , ) = licenseEngine.licenses(id);
                maxDiscount = maxDiscount >= discout ? maxDiscount : discout;
            }
        }
        if (maxDiscount < 0 || maxDiscount > 10000) revert DiscountTooLarge();
        return maxDiscount;
    }
    ...
    function flipFullStop() external onlyOwner {
        fullStop = !fullStop;
    }
    ...
    function flipPoolStop(address _pool) external onlyOwner {
        pausedPools[_pool] = !pausedPools[_pool];
    }
