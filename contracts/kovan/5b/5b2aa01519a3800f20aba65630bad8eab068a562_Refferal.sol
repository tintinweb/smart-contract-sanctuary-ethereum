...
contract Refferal is Ownable {
    ...
    function getTotalUserByUp(address _refferrer, uint _limit, uint _skip) public view returns(Refer[] memory list, uint totalItem) {
        uint limit = _limit <= referreds[_refferrer].length - _skip ? _limit + _skip : referreds[_refferrer].length;
        uint lengthReturn = _limit <= referreds[_refferrer].length - _skip ? _limit : referreds[_refferrer].length - _skip;
        list = new Refer[](lengthReturn);
        for(uint i = _skip; i < limit; i++) {
            list[i-_skip] = userInfos[referreds[_refferrer][i].user];
        }
    ...
    function _register(address _refferrer) internal {
        Refer memory refer = Refer(_msgSender(), _refferrer, block.timestamp, 0, false);
