...
// import "../beachside/OwnableUpgradeable.sol";
...
// import "../beachside/OwnableUpgradeable.sol";
// import "../beachside/PausableUpgradeable.sol";
// import "../beachside/ERC20Upgradeable.sol";
...
contract IndexToken is ERC20Upgradeable, OwnableUpgradeable, PausableUpgradeable {
    ...
    uint256 public inflationRate; //per second
    ...
    modifier onlyMethodologist() {
        require(msg.sender == methodologist);
        _;
    }
    ...
    function burn(address from, uint256 amount) external whenNotPaused onlyOwner {
        mintToFeeReceiver();
        _burn(from, amount);
    }
    ...
    function mintToFeeReceiver() public {
        if (timestamp < block.timestamp && feeReceiver != address(0)) {
            uint256 amount = (totalSupply() * (inflationRate * (block.timestamp - timestamp))) / 1e20;
    ...
    function setFeeRate(uint256 _rateBP) external onlyOwner {
        mintToFeeReceiver();

        uint256 scaled = (_rateBP * 1e20) / 10000;
        inflationRate = scaled / 31536000; //per second
    }
...
interface ControllerInterface {
    ...
    function getToken() external view returns (IndexToken);

    function tokenPause(bool pause) external returns (bool);

    function factoryPause(bool pause) external returns (bool);

    function pauseAll(bool pause) external returns (bool);
...
contract Controller is ControllerInterface, OwnableUpgradeable {
    ...
    function setFeeReceiver(address feeReceiver) external onlyOwner returns (bool) {
        token.setFeeReceiver(feeReceiver);
        emit FeeReceiverSet(feeReceiver);
        return true;
    }

    function setFeeRate(uint256 feeRate) external onlyOwner returns (bool) {
        token.setFeeRate(feeRate);
        emit FeeRateSet(feeRate);
        return true;
    }

    function setMethodologist(address methodologist) external onlyOwner returns (bool) {
        token.setMethodologist(methodologist);
    ...
    function tokenPause(bool pause) public onlyOwner returns (bool) {
        if (pause) {
            token.pause();
        } else {
            token.unpause();
        }
        emit TokenPause(pause);
        return true;
    }

    function factoryPause(bool pause) public onlyOwner returns (bool) {
        if (pause) {
            factory.pause();
        } else {
            factory.unpause();
    ...
    function pauseAll(bool pause) external onlyOwner returns (bool) {
        tokenPause(pause);
        factoryPause(pause);
        return true;
    }
    ...
    function burn(address from, uint256 value) external onlyFactory returns (bool) {
...
contract Factory is Ownable, Pausable {
    ...
    function setMerchantDepositAddress(string memory depositAddress) external onlyMerchant returns (bool) {
        require(!isEmptyString(depositAddress), "invalid asset deposit address");
    ...
    ) external onlyMerchant whenNotPaused returns (bool) {
        require(!isEmptyString(depositAddress), "invalid asset deposit address");
    ...
    function cancelMintRequest(bytes32 requestHash) external onlyMerchant whenNotPaused returns (bool) {
        uint256 nonce;
    ...
    function confirmMintRequest(bytes32 requestHash) external onlyCustodian whenNotPaused returns (bool) {
        uint256 nonce;
        Request memory request;
    ...
    function rejectMintRequest(bytes32 requestHash) external onlyCustodian whenNotPaused returns (bool) {
        uint256 nonce;
        Request memory request;
    ...
    function burn(uint256 amount) external onlyMerchant whenNotPaused returns (bool) {
...
contract InitialDeploy {
    ...
    constructor(address multisig) {
        ...
        Members members = new Members(multisig);
        ...
        token = IndexToken(address(tokenProxy));
        token.initialize("Alongside Crypto Market Index", "AMKT");
        ...
        Factory factory = new Factory(address(controller));
        controller.initialize(address(tokenProxy));
        controller.setMembers(address(members));
        controller.setFactory(address(factory));
        controller.setFeeReceiver(multisig);
        controller.setFeeRate(100);
        controller.setMethodologist(multisig);
        controller.transferOwnership(address(multisig));
