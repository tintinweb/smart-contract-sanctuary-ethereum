...
contract CopperProxy is Ownable {
    ...
    mapping(address => uint256) public _partnerShare;
    ...
    constructor(
        ...
        // The 3 of the fee recipients should sum up 10_000
    ...
    function getPercentageShare(address onwerAddress) external view returns (uint256 share) {
    ...
    /**
     * In case of emergency, transfer funds to tertiary recipient and manually distribute fees.
     */
    function safeFeeDistribution(address pool, address fundToken) internal {
        ...
        if (fundTokenBalance > uint256(0)) {
            ...
            TransferHelper.safeTransfer(fundToken, _tertiaryFeeRecipient, feeAmount);
            emit TransferedFee(pool, fundToken, _tertiaryFeeRecipient, feeAmount);
        }
    }
    ...
    function distributeFee(address pool, address fundToken) internal {
        ...
        if (fundTokenBalance > uint256(0)) {
            uint256 totalFeeAmount = ((fundTokenBalance * _platformAccessFee) / 10_000);
            // primary recipient
            sendFee(pool, totalFeeAmount, _primaryFeeRecipient, fundToken);
            // secondary recipient
            sendFee(pool, totalFeeAmount, _secondaryFeeRecipient, fundToken);
            // tertiary recipient 
            sendFee(pool, totalFeeAmount, _tertiaryFeeRecipient, fundToken);
        }
    }

    /**
    *  Send share of fee to recipient to partner
    */
    function sendFee(
        address pool, uint256 totalFeeAmount,
        address feeRecipient, address fundToken ) internal {
        uint256 partnerProportionalAmount = (totalFeeAmount * _partnerShare[feeRecipient]) / 10_000;
        TransferHelper.safeTransfer(fundToken, feeRecipient, partnerProportionalAmount);
        emit TransferedFee(pool, fundToken, feeRecipient, partnerProportionalAmount);
    }

    function changePrimaryFeeRecipient(address newRecipient, uint256 shareSize) external onlyOwner {
        require(shareSize >= uint256(0), "shareSize must be grader or equal to 0");
        require(shareSize <= uint256(10_000), "shareSize must be less or equal to 10_000"); // less than 100%
        require(newRecipient != _secondaryFeeRecipient, "newRecipient must be different than _secondaryFeeRecipient");
        require(newRecipient != _tertiaryFeeRecipient, "newRecipient must be different than _tertiaryFeeRecipient");
    ...
    function changeSecondaryFeeRecipient(address newRecipient, uint256 shareSize) external onlyOwner {
        require(shareSize >= uint256(0), "shareSize must be grader or equal to 0");
        require(shareSize <= uint256(10_000), "shareSize must be less or equal to 10_000"); // less than 100%
        require(newRecipient != _primaryFeeRecipient, "newRecipient must be different than _primaryFeeRecipient");
        require(newRecipient != _tertiaryFeeRecipient, "newRecipient must be different than _tertiaryFeeRecipient");
    ...
    function changeTertiaryFeeRecipient(address newRecipient, uint256 shareSize) external onlyOwner {
        require(shareSize >= uint256(0), "shareSize must be grader or equal to 0");
        require(shareSize <= uint256(10_000), "shareSize must be less or equal to 10_000"); // less than 100%
        require(newRecipient != _primaryFeeRecipient, "newRecipient must be different than _primaryFeeRecipient");
        require(newRecipient != _secondaryFeeRecipient, "newRecipient must be different than _secondaryFeeRecipient");
