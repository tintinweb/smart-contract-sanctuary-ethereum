...
contract BeastNFT is Context, IBeastNFT, Ownable {
    ...
    function sendToMarketplace(uint256 tokenId, uint256 price) external {
        require(ownerOf(tokenId)==msg.sender && tokenData[tokenId].onMarket==false, "You are not owner of this token or it's already on market");
    ...
    function buyToken(uint256 tokenId) external {
        require(msg.sender!=ownerOf(tokenId), "You can not buy your beast");
