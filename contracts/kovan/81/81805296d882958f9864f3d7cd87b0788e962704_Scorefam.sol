    ...
    UUPSUpgradeable
{
    ...
    function getGameRemainingLockTime(uint256 game_) internal view returns(uint256) {
     ...
     */
    function getGames() external view returns (uint256[8][] memory) {
        uint256[8][] memory tempStakeList = new uint256[8][](_gamesCount[_msgSender()] + 1);
        for (uint256 i = 1; i <= _gamesCount[_msgSender()]; i++) {
    ...
    function getSlips() external view returns (uint256[6][] memory) {
        uint256[6][] memory temp = new uint256[6][](_slipsCount[_msgSender()] + 1);
  ...
  //Convert String to Uint
    ...
    function getGameScoreRequestID(uint256 fixtureID) public returns(bytes32){
        return IScorefamConsumer(consumer).gameScoreRequestID(fixtureID);
    }
