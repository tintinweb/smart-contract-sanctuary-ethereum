...
library MegafiswapV2Library {
    ...
    function pairFor(address factory, address tokenA, address tokenB) internal pure returns (address pair) {
        ...
        pair = address(uint(keccak256(abi.encodePacked(
                ...
                hex'b68912df698027f165e81e05d9380615da83ba9cbd2f08cd9ff38f97dc8bd246' // init code hash
            ))));
    }
...
contract MegafiAggregator{
    ...
    function addLiquidity(
        ...
        IERC20Megafiswap(tokenA).transferFrom(msg.sender, address(this), amountADesired);
        IERC20Megafiswap(tokenB).transferFrom(msg.sender, address(this), amountBDesired);
        ...
        // } else 
        if (IMegafiswapV2Factory(factory).getPair(tokenA, tokenB) != address(0)) {
    ...
    function addLiquidityETH(
        ...
        IERC20Megafiswap(token).transferFrom(msg.sender, address(this), amountTokenDesired);
        ...
        // } else 
        if (IMegafiswapV2Factory(factory).getPair(token, weth) != address(0)) {
    ...
    function removeLiquidity(
        ...
        if (IMegafiswapV2Factory(megaFactory).getPair(tokenA, tokenB) != address(0)) {
            ...
            IERC20Megafiswap(pair).transferFrom(msg.sender, address(this), liquidity);
    ...
    function swapExactTokensForTokens(
        ...
        IERC20Megafiswap(path[0]).transferFrom(msg.sender, address(this), amountIn);
    ...
    function swapTokensForExactTokens(
        ...
        IERC20Megafiswap(path[0]).transferFrom(msg.sender, address(this), amountInMax);
    ...
    function _approve(address _token, address _receiver, uint amount) private {
        IERC20Megafiswap(_token).approve(address(_receiver), amount);
    }
