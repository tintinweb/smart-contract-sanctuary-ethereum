...
contract Breeding is BreedingCore {
    ...
    function breed( uint _matronId, uint _sireId, uint _gene, uint _deadLine, bytes memory _signature) external notBreedWhen {
       ...
       uint eggID = breedInfo.length - 1;

       uint _sparceID = collarQuest.spawnSparce(breedInfo[eggID].eggGene, breedInfo[eggID].receiver);
       sparceInfo[_sparceID].eggID = eggID;
       
       emit Breed( _sireId,  _matronId, eggID);
    }
