...
//easier than importing ERCURIstorage to set
...
function delegatedExchangeToken(address ownerOfPack, uint256 tokenId, uint256[] memory momentsSerial, address[] memory externalContracts, address sweetAdminWallet, bytes memory signature)
public virtual onlyRole(MINTER_ROLE) {
        bytes32 userHash;
         userHash = keccak256(abi.encodePacked(address(this), globalTransferNonce, ownerOfPack, tokenId, momentsSerial, externalContracts, sweetAdminWallet, signature));
        address messageSigner = userHash.toEthSignedMessageHash().recover(signature);
        require((messageSigner == ownerOfPack && messageSigner == this.ownerOf(tokenId)), "Transfer signer is not authorized signer");
    _safeTransfer(ownerOfPack, address(0x00000000000000000000), tokenId, signature);
    for (uint i = 0; i < externalContracts.length; i++) {
        //ADMIN
       IERC721(externalContracts[i]).safeTransferFrom(sweetAdminWallet, ownerOfPack, momentsSerial[i]);
    }
  }
    /**
