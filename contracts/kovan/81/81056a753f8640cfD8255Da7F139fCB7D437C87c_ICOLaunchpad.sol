...
// File: contracts/Tasks/ICO LaunchPad/Launch.sol
...
contract ICOLaunchpad {
    ...
    function listProject(address _projectAdd,uint noTokens,uint _rate,uint _Tokens) external {
        Project  storage newP = projects[_projectAdd];
        newP.projectAdd = _projectAdd;
        newP.owner = payable(msg.sender);
        newP.Date = block.timestamp;
        // newP.startDate = sD;
        // newP.endDate = eD;
    ...
    function caculate(uint amountTokens,address _projectAdd) public {
         Project  storage p = projects[_projectAdd];
