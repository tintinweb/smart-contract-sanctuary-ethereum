...
contract Sale1155 is ERC1155Holder, Ownable {
  ...
  function setAsk(uint256 _tokenId, uint256 _price, uint256 _quatity, address _owner) internal {
    AskEntry storage askEntry = asks[_tokenId];

    askEntry.tokenId = _tokenId;
    askEntry.price = _price;
    askEntry.quatity = _quatity;
    askEntry.owner = _owner;
    askEntry.exist = true;
  ...
  function addNewToken(uint256 _tokenId, uint256 _price, uint256 _quatity) external {
    require(_price > 0, 'price must be granter than zero');
    require(_quatity > 0, 'quatity must be granter than zero');
    require(!asks[_tokenId].exist, 'already exist');

    nft.safeTransferFrom(address(msg.sender), address(this), _tokenId, _quatity, '0x');
    setAsk(_tokenId, _price, _quatity, msg.sender);
  }

  function updateTokenQty(uint256 _tokenId, uint256 _quatity) external onlyCreator(msg.sender) {
    require(_quatity > 0, 'quatity must be granter than zero');
    AskEntry storage askEntry = asks[_tokenId];
    askEntry.quatity = _quatity;
  }

  function updateTokenPrice(uint256 _tokenId, uint256 _price) external onlyCreator(msg.sender) {
    require(_price > 0, 'price must be granter than zero');
    AskEntry storage askEntry = asks[_tokenId];
    askEntry.quatity = _price;
  }

  function cancelSellToken(uint256 _tokenId) external onlyCreator(msg.sender) {
    AskEntry storage askEntry = asks[_tokenId];
    require(askEntry.exist, 'not in sell book');
    require(askEntry.quatity > 0, 'no token exist');

    nft.safeTransferFrom(address(this), askEntry.owner, _tokenId, askEntry.quatity, '0x');
    askEntry.quatity = 0;
  }
  ...
  function buyToken(uint256 _tokenId) public payable {
    ...
    AskEntry storage askEntry = asks[_tokenId];
    require(askEntry.quatity >= 1, 'token sale out');
    require(msg.value == askEntry.price, 'balance too low');

    (bool sent, ) = payable(askEntry.owner).call{value: msg.value}("");
