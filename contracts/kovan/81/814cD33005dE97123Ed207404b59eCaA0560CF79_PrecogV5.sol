...
contract PrecogV5 is IPrecogV5 { 
    ...
    function updateProfitCycleToCurrentCycle(address token) public override {
        ...
        while (uint48(block.timestamp) >= lastCycle.endTime) {
            ...
            if(lastCycle.endTime < core.lastChangeProfitCycle() && (newCycleStartTime + duration) >= core.lastChangeProfitCycle()) {
                ...
                newCycleEndTime = newCycleStartTime + core.getCycleConfiguration().profitCycle;
                profitCycles[token].push(Cycle(lastCycle.id + 1, newCycleStartTime, newCycleEndTime));
                totalInvestmentUnits[token].push(liquidity[token] * core.getCycleConfiguration().profitCycle);
            }
