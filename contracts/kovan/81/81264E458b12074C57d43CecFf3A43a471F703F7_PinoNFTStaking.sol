...
/**
 * @title Pino NFT's Staking Contract
 * @author ESG Team
 */
contract PinoNFTStaking is IERC721Receiver, ReentrancyGuard, Ownable {
	...
	uint256 level = IEcoNFT(nft).getLevel(tokenId);
        require(level == 0, "Only PINO NFT accepted.");
	...
	if(cp.isUsed == true)
	{
		...
		if(level == 0 && cp.isBonusNFTStaked == false)
		{
			totalHashRate = totalHashRate.add(cp.hashRate.div(10).add(hash_rate.mul(110).div(100)));
			cp.hashRate = cp.hashRate.add(hash_rate).mul(110).div(100);
			cp.isBonusNFTStaked = true;
		}else if(cp.isBonusNFTStaked == true)
		{
			totalHashRate = totalHashRate.add(hash_rate.mul(110).div(100));
			cp.hashRate = cp.hashRate.add(hash_rate.mul(110).div(100));
		}
		else if(cp.isBonusNFTStaked == false)
		{
	...
	}else
	{
		...
		{
			totalHashRate = totalHashRate.add(hash_rate.mul(110).div(100));
			stakings[nftOwner] = Checkpoint(hash_rate.mul(110).div(100), rewardDebt, 0, 0, true, true);
		}
		else
		{
			totalHashRate = totalHashRate.add(hash_rate);
			stakings[nftOwner] = Checkpoint(hash_rate, rewardDebt, 0, 0, false, true);
		}
	}
	lastStakeBlock = block.number;	
	...
	uint256 amt = getBonusNFTStaked(msg.sender);
	
	if(amt == 1)
	{
		//restore all staked NFT hashrate to 100%
		totalHashRate = totalHashRate.sub(cp.hashRate.div(10)).sub(hash_rate);
		cp.hashRate = cp.hashRate.sub(hash_rate.mul(110).div(100));
	}else if(amt > 1)
	{
		//just subtract the NFT unstaked hash_rate*110%
		totalHashRate = totalHashRate.sub(hash_rate.mul(110).div(100));
		cp.hashRate = cp.hashRate.sub(hash_rate.mul(110).div(100));
	}else 
	{
		//just subtract the NFT unstaked hash_rate
    ...
    /**
     * @notice Returns amount of bonus NFT staked
     * @param account The address 
     * @return Number of bonus NFT staked
     */
    function getBonusNFTStaked(address account) internal view returns (uint256) {
	uint256 amt = 0;
	for(uint256 i=0; i<EnumerableSet.length(userTokenIds[account]); i++)
	{
		uint256 tokenId = EnumerableSet.at(userTokenIds[account], i);	
		uint256 level = IEcoNFT(nft).getLevel(tokenId);
		if(level == 0)
			amt = amt.add(1);
	}
