...
contract ArtistTools is VRFConsumerBase {
    address private constant FanLinkAddress =
        0x81826b25831E7ECd3cAf8553fe19C2BC100eaDF0;
    //Price Feed stuff
    ...
    event err(uint256 msgVal, uint256 amountRequired);
    ...
    function buyTicketsForFansOfArtist(
        ...
        uint256 amount = (ticketPrice * 10**8) / (getLatestPrice()); //convert USD to Eth amount based on pricefeed.
        emit err(msg.value / (10**18), amount * fanAry.length);
        require(
            (msg.value / (10**18)) > (amount * fanAry.length),
            "Not enough Eth sent in transaction: "
        );
        // transfer the required amount of ether to each one of the fans
        for (uint256 i = 0; i < fanAry.length; i++) {
            payable(fanAry[i]).transfer(amount);
        }
    }

    function seeTicketPriceTotals(uint256 ticketPrice, uint256 count)
        public
        returns (uint256)
    {
        uint256 amount = (ticketPrice * 10**8) / (getLatestPrice()); //convert USD to Eth amount based on pricefeed.
