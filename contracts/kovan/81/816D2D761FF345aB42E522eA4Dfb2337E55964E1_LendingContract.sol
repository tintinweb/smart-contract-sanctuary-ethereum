...
contract LendingContract is Ownable {
    ...
    uint256 private interestDecimal = 10**18;
    ...
    event LoanCreated(uint256 indexed id, address[] agreements);
    event Repayment(uint256 indexed id, uint256 amount);
    event LoanIncreased(uint256 indexed id, uint256 amount);
    ...
    modifier onlyWhitelisted() {
        require(isWhitelisted(msg.sender), "Sender must be whitelisted");
        _;
    }

    constructor(IERC20 _trackedToken) {
    ...
    function approveLoan(
        ...
        require(borrower != address(0), "Transfer to the zero address");
        require(amount > 0, "Amount is zero");
        require(interestRate > 0, "Interest rate is zero");
        require(interestRate < interestDecimal, "Interest rate is too big"); // 100% per day is too big
        require(agreements.length > 0, "No agreements");
        for (uint256 i = 0; i < agreements.length; i++) {
            require(agreements[i] != address(0), "Invalid agreement address");
            // cannot be used as collateral until debt is repaid
            require(!isCollateral(borrower, agreements[i]), "Repay first");
        }
        lastLoanId++;
        ...
        emit LoanCreated(lastLoanId, agreements);
    }

    function approveIncreaseLoan(uint256 loanId, uint256 amount)
        public
        onlyWhitelisted
    {
        require(msg.sender == loans[loanId].lender, "Only lender can approve");
        require(loans[loanId].borrower != address(0), "Loan does not exists");
    ...
    function getOutstandingDebt(uint256 loanId)
        public
        view
        returns (
            uint256 debtSum,
