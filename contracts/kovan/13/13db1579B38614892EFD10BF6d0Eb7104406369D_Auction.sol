...
contract Auction is Ownable, ReentrancyGuard, ERC721Holder, ERC1155Holder,AccessControl {
    ...
    function bid(bytes32 collectibleHash, uint256 price) payable external nonReentrant {
        ...
        if (paymentAmount > 0){
            _executeFundsTransfer(collectible.currency, _msgSender(), paymentAmount);
            emit Withdrawn(collectibleHash, _msgSender(), paymentAmount);
        }

        if (bids[collectibleHash][_msgSender()] == 0) {
            bidders[collectibleHash].push(_msgSender());
        }
        bids[collectibleHash][_msgSender()] = price;

        emit Bided(collectibleHash, _msgSender(), price);
    }
