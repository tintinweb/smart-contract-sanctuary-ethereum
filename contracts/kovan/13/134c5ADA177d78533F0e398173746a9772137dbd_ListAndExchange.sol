...
contract ListAndExchange{
    ...
    function PositiveIndexOfCollectionC (address add, address contrAdd) public view returns (uint) {
        uint i = collectionsC[add].length; uint j; bool Done;
        while (i > 0) {
            if (address(collectionsC[add][i-1]) == contrAdd) {
            Done = true; j=i; i=1;
            } 
            i--;
        }
        if (Done == true) {return j;} else {return 0;} 
    }
    ...
    function PositiveIndexOfCollectionB (address add, address contrAdd) public view returns (uint) {
        uint i = collectionsB[add].length; uint j; bool Done;
        while (i > 0) {
            if (address(collectionsB[add][i-1]) == contrAdd) {
            Done = true; j=i; i=1;
            } 
            i--;
        }
    ...
    function ListNFT(address collectionAddress, uint256 Id ) public {
        ...
        if (!(contrat.balanceOf(creator, Id)==0)) {           
              uint index = PositiveIndexOfCollectionB(creator, address(contrat));
    ...
    function BuyNFT(uint exchangeId, uint sellerIndex, uint amount) external payable {
        ...
        uint index = PositiveIndexOfCollectionB(msg.sender, address(contrat));
        //if the buyer don't have the collection in his list, we add it
        ...
        if (contrat.AllBalancesOf(seller)==0 ) {
            index = PositiveIndexOfCollectionB(seller, address(contrat));
