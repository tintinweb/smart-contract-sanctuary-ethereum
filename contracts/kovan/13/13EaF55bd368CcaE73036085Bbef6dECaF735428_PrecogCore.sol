...
contract PrecogCore is IPrecogCore {
    ...
    uint48 public override lastChangeCyclesTimestamp;
    address public PCOG;
    ...
    function setCycleConfiguration(CycleConfiguration memory _cycleConfiguration) external onlyAdmin {
        ...
        lastChangeCyclesTimestamp = uint48(block.timestamp);
    ...
    function _updateAdjustments() internal {
        ...
        for(uint i = 0; i < tokens.length; i++) {
            IPrecogV5(precog).updateCurrentTradingCycle(tokens[i]);
            IPrecogV5(precog).updateAdjustment(tokens[i]);
        }
    }
