...
contract MarsToken is IERC1404, ERC20, Ownable {
    uint8 constant SUCCESS_CODE = 1;
    uint8 constant FAILURE_CODE = 0;
    enum investorTypeCode {
        NON_ACCRED_USER,
        ELIGIBLE_USER,
        ELIGIBLE_USER_2,
        ACCRED_USER,
    ...
    // Sets initial redemption amount
    mapping(address => uint256) public redemptionLimit;
    ...
    constructor(
        ...
        _investorTypeLimits[uint8(investorTypeCode.NON_ACCRED_USER)] =
            10000 *
            10**18;
        _investorTypeLimits[uint8(investorTypeCode.ELIGIBLE_USER)] =
            30000 *
            10**18;
        _investorTypeLimits[uint8(investorTypeCode.ELIGIBLE_USER_2)] =
            100000 *
            10**18;
        _investorTypeLimits[uint8(investorTypeCode.ACCRED_USER)] = 0;
        _investorTypeLimits[uint8(investorTypeCode.NOT_APPLICABLE_USER)] = 0;
    ...
    function updateProtocolAddress(address _protocolAddress)
        external
        onlyOwner
        returns (bool)
    {
    ...
    function updateRedemptionLimit(address investor, uint256 amount)
        external
        onlyOwner
        returns (bool)
    {
        _updateRedemptionLimit(investor, amount);
        return true;
    }

    function _updateRedemptionLimit(address investor, uint256 amount)
        internal
        returns (bool)
    {
        redemptionLimit[investor] = amount;
    }
    ...
    modifier validateInitialRedeem(
        address from,
        address to,
        uint256 value
    ) {
        if (from == protocolAddress) {
            require(
                redemptionLimit[to] >= value,
                "Redemption value exceeds balance."
            );
            redemptionLimit[to] -= value;
        }
        _;
    ...
    function transferFrom(
        ...
        validateInitialRedeem(msg.sender, to, value)
        returns (bool)
    {
        updateInvestorCount(msg.sender, to, value);
