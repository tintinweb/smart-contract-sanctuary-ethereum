...
/**
 * @title A consumer contract for Wavebridge.
 * @author LinkPool.
 * @notice Interact with the WavebridgeIndex API v3.
 * @dev Uses @chainlink/contracts 0.3.1.
 */
contract WavebridgeConsumer is ChainlinkClient {
    using Chainlink for Chainlink.Request;

    /* ========== CONSUMER STATE VARIABLES ========== */

    // Maps
    mapping(bytes32 => int256) public requestIdIndex;
    ...
    /**
     * @notice Requests CMX Index at yesterday's close, KIMP Index at yesterday's close or the KIMP Realtime Index.
     * @param _specId the jobID.
     * @param _payment the LINK amount in Juels (i.e. 10^18 aka 1 LINK).
     * @param _endpoint the EA endpoint to request ('cmx-daily', 'kimp-daily' or 'kimp-realtime').
     */
    function requestIndex(
        ...
        Chainlink.Request memory req = buildChainlinkRequest(_specId, address(this), this.fulfillIndex.selector);
    ...
    /**
     * @notice Requests either CMX Index by date or KIMP Index by date.
     * @param _specId the jobID.
     * @param _payment the LINK amount in Juels (i.e. 10^18 aka 1 LINK).
     * @param _endpoint the EA endpoint to request ('cmx-daily' or 'kimp-daily').
     * @param _date the date of the index (YYYY-MM-DD).
     */
    function requestIndexDailyByDate(
        bytes32 _specId,
        uint256 _payment,
        string calldata _endpoint,
    ...
    /**
     * @notice Stores the index.
     * @dev The index has been multiplied by 10^6.
     * @param _requestId the request ID for fulfillment.
     * @param _result the Index value.
     */
    function fulfillIndex(bytes32 _requestId, int256 _result) public recordChainlinkFulfillment(_requestId) {
        requestIdIndex[_requestId] = _result;
    }
