...
contract PrecogCore is IPrecogCore {
    ...
    function setCoreConfiguration(CoreConfiguration memory _coreConfiguration) external override onlyAdmin {
    ...
    function setCycleConfiguration(CycleConfiguration memory _cycleConfiguration) external override onlyAdmin {
        require(
            _cycleConfiguration.firstTradingCycle > 0 &&
            _cycleConfiguration.firstDefundingCycle > 0 &&
            _cycleConfiguration.fundingCycle > 0 &&
            _cycleConfiguration.defundingCycle > 0 &&
            _cycleConfiguration.tradingCycle > 0
        );
        ...
        emit SetCycleConfiguration(
            ...
            _cycleConfiguration.firstTradingCycle,
            _cycleConfiguration.firstDefundingCycle,
            _cycleConfiguration.fundingCycle,
            _cycleConfiguration.defundingCycle,
            _cycleConfiguration.tradingCycle
        );
    }

    function setFeeConfiguration(FeeConfiguration memory _feeConfiguration) external override onlyAdmin {
        feeConfiguration = _feeConfiguration;
        emit SetFeeConfiguration(coreConfiguration.admin, _feeConfiguration.depositFee, _feeConfiguration.withdrawalFee, _feeConfiguration.tradingFee, _feeConfiguration.lendingFee);
    }
...
interface IPrecogV5 {
    ...
    function tradingCycles(address token, uint id) external view returns (uint16, uint48, uint48);
    function currentFundingId(address token) external view returns (uint16);
    function currentProfitId(address token) external view returns (uint16);
    function lastProfitIdOf(address token, address account) external view returns (uint16);
