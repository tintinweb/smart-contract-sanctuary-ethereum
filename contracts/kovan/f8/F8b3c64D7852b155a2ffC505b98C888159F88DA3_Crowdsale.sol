...
contract Crowdsale is AccessControl, ReentrancyGuard {
    ...
    mapping(address => bool) public userGetCode;
    ...
    event CreatedCode(bytes32 code, address user);
    ...
    function createCode() external {
       ...
       // require(!userGetCode[owner], "Crowdsale: user has code");
        bytes32 result;
        ...
        codes[result] = owner;
        userGetCode[owner] = true;
        emit CreatedCode(result, owner);
    }
    ...
    function buyTokens(
        ...
        if (codes[code] != address(0) && codes[code] != user) {
            uint256 toCodeOwner;
            ...
            if (toCodeOwner != 0) {
                ...
                TransferHelper.safeTransfer(ARA, codes[code], toCodeOwner);
            }
        } else {
            require(
