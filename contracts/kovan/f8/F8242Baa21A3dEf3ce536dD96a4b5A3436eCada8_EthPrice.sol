...
contract EthPrice is ChainlinkClient {
    ...
    onlyOwner
  {
    ...
    string memory requestURL = string(abi.encodePacked("https://api.coingecko.com/api/v3/simple/price?ids=", "bitcoin", "&vs_currencies=usd"));
    req.add("get", requestURL);

    string memory path = string(abi.encodePacked("bitcoin", ".usd"));
    req.add("path", path);
