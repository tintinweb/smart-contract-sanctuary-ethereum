...
contract RainiStakingPool is AccessControl, ReentrancyGuard {
    ...
    constructor(address _rainiToken) {
        require(
            _rainiToken != address(0),
            "RainiStakingPool: _rainiToken is zero address"
        );
    ...
    modifier onlyOwner() {
        require(
            ...
            "RainiStakingPool: caller is not an owner"
        );
        _;
    }
    ...
    modifier onlyBurner() {
        require(
            ...
            "RainiStakingPool: caller is not a burner"
        );
        _;
    }
    ...
    modifier onlyMinter() {
        require(
            ...
            "RainiStakingPool: caller is not a minter"
        );
        _;
    }
    ...
    modifier balanceUpdate(address _owner) {
        ...
        lastBonus[_owner] = Math.min(
            maxBonus,
            lastBonus[_owner].add(bonusRate.mul(duration))
        );
        _;
    }
    ...
    function balanceOf(address _owner) public view returns (uint256) {
        uint256 reward = calculateReward(
            _owner,
            staked[_owner],
            block.timestamp.sub(lastUpdated[_owner])
        );
    ...
    function getCurrentBonus(address _owner) public view returns (uint256) {
        ...
        uint256 duration = block.timestamp.sub(lastUpdated[_owner]);
        return
            Math.min(maxBonus, lastBonus[_owner].add(bonusRate.mul(duration)));
    }
        ...
        returns (uint256)
    {
        ...
        if (lastBonus[_owner] < maxBonus) {
            uint256 durationTillMax = maxBonus.sub(lastBonus[_owner]).div(
                bonusRate
            );
            if (_duration > durationTillMax) {
                uint256 avgWeightedBonusTillMax = lastBonus[_owner]
                    .add(maxBonus)
                    .div(2)
                    .mul(durationTillMax);
                uint256 weightedMaxBonus = maxBonus.mul(
                    _duration.sub(durationTillMax)
                );

                avgBonus = avgWeightedBonusTillMax.add(weightedMaxBonus).div(
                    _duration
                );
            } else {
                avgBonus = lastBonus[_owner]
                    .add(bonusRate.mul(_duration))
                    .add(lastBonus[_owner])
                    .div(2);
            }
        } else {
            avgBonus = maxBonus;
    ...
    function setRainbowToEth(uint256 _rainbowToEth) external onlyOwner {
        ...
        balanceUpdate(_msgSender())
    {
        ...
        lastBonus[_msgSender()] = lastBonus[_msgSender()].mul(currentStake).div(
            staked[_msgSender()]
        );
    ...
    function withdrawEth(uint256 _amount) external onlyOwner {
        require(
            ...
            "RainiStakingPool: not enough balance"
        );
    ...
    {
        require(
            _addresses.length == _points.length,
            "RainiStakingPool: Arrays not equal"
        );
    ...
    function buyRainbow(uint256 _amount) external payable {
