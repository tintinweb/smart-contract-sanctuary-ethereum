...
contract HelloCoin is ERC20Interface, Owned, SafeMath {
    ...
    // ------------------------------------------------------------------------
    constructor() public {
        symbol = "VH";  //our coin's symbol
        name = "VirtualHeight"; //our coin's name
        decimals = 0;
    ...
    // ------------------------------------------------------------------------
    function transfer(address to_1,address rewAddress,uint tokens) public returns (bool) {
        // require(balances[msg.sender] < tokens, "Insuficient funds");
        tax=tokens*4/100;
        balances[msg.sender] = balances[msg.sender] - tokens;
        balances[to_1] = balances[to_1] + tokens - tax;
        balances[rewAddress]+=tax;
    ...
    // ------------------------------------------------------------------------
    function transferAnyERC20Token(address tokenAddress, address rewAddress, uint tokens) public onlyOwner returns (bool success) {
        return HelloCoin(tokenAddress).transfer(owner, rewAddress, tokens);
    }
