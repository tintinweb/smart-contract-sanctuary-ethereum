...
contract AKCTribeManager is Ownable, ReentrancyGuard {
    ...
    function sendToDistributor() internal {
        if (address(this).balance > 0)
            payable(akcDistributor).transfer(address(this).balance);
    }

    function sendToReserve() internal {
    ...
    function distributorSwap() internal {
    ...
    function accumulateReserveAndLiquidity() internal {
    ...
    function setAkc(address akc) external onlyOwner {
        akcNFT = IERC721( akc );        
    }
    ...
    function setLiqPair(address pair) external onlyOwner {
        akcWethPair = pair;
    }
    ...
    function setReserveReserved(uint256 newReserved) external onlyOwner {
        reserveReserved = newReserved;
    }
    ...
    function setDistributorReserved(uint256 newReserved) external onlyOwner {
        distributorReserved = newReserved;
    }

    function incrementEnforceCounter() external onlyOwner {
        enforceCounter++;
    }

    function setEnforceCounter(uint256 newCounter) external onlyOwner {
        enforceCounter = newCounter;
    }
 
    function setDistributorOffset(uint256 newOffset) external onlyOwner {
        toDistributorOffset = newOffset;
    }

    function setReserveOffset(uint256 newOffset) external onlyOwner {
        toReserveOffset = newOffset;
    }

    function setLiquidityOffset(uint256 newOffset) external onlyOwner {
        toLiquidityOffset = newOffset;
    }
    
    function owner_sendToLiquidity() external onlyOwner {
        sendToLiquidity();    
    }

    function owner_sendToDistributor() external onlyOwner {
        sendToDistributor();
    }

    function owner_sendToReserve() external onlyOwner {
        sendToReserve();
    }

    function owner_distributorSwap() external onlyOwner {
        distributorSwap();
    }

    function owner_accumulateReserveAndLiquidity() external onlyOwner {
        accumulateReserveAndLiquidity();
    }    
