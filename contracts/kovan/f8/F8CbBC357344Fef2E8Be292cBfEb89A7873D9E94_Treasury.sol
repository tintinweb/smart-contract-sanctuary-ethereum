...
contract Treasury is Ownable, Initializable {
    ...
    uint256 public ratioStep = 2500; // = 0.0025 or 0.25%
    uint256 public priceTarget = 1e18; // = 1
    /// @dev CR will be tune if price go out this band
    uint256 public priceBand = 5e16; // = 0.05
    ...
    function initialize(IXftm _xftm, IFantasmOracle _fantasmOracle) external initializer {
        fantasmOracle = _fantasmOracle;
        xftm = _xftm;
    }
    ...
    function refreshCollateralRatio() public {
        ...
        require(block.timestamp - lastRefreshCrTimestamp >= refreshCooldown, "Must wait for the refresh cooldown since last refresh");

        uint256 _xftmPrice = IFantasmOracle(fantasmOracle).getXftmTWAP();
    ...
    function setFees(uint256 _mintingFee, uint256 _redemptionFee) public onlyOwner {
        require(_mintingFee <= MINTING_FEE_MAX, ">MINTING_FEE_MAX");
        require(_redemptionFee <= REDEMPTION_FEE_MAX, ">REDEMPTION_FEE_MAX");
    ...
    function setFeeDistributor(address _feeDistributor) external onlyOwner {
        require(_feeDistributor != address(0), "Cannot set to zero address");
