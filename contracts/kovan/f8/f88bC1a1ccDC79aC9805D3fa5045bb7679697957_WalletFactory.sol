...
contract WalletFactory {
    ...
    function createWallet(
        ...
        //
        Wallet PolybitWallet = new Wallet(
            _oracleAddress,
            _riskWeighting,
            _walletOwner,
            wethAddress,
        ...
        walletArray.push(PolybitWallet);
        walletAddressList.push(address(PolybitWallet));
        walletsOfAddress[msg.sender].push(address(PolybitWallet));
...
contract Wallet {
    ...
    event buyAssetsEvent(uint256, uint256, uint256);
    event addressList(address[]);

    function buyAssets() public {
        address[] memory targetAssets = getTargetAssets();
        emit addressList(targetAssets);
        if (targetAssets.length > 0) {
            for (uint256 x = 0; x < targetAssets.length; x++) {
                emit buyAssetsEvent(100, 100, 100);
            }
        }
    }
    ...
    function launchStrategy() public onlyWalletOwner {
        ...
        // Get target assets
        // BUY assets
        // Update owned assets after purchase
    }
    ...
    function transfer(address _address, uint256 _amount)
        ...
        onlyWalletOwner
    {
        //manually transfer a single token or native token
        //require wallet not locked
    }

    function transferToClose() public onlyWalletOwner {
        //transfer all tokens or native tokens
        //require wallet not locked
    }

    event closeWalletEvent(string msg, uint256 ref);

    function sellToClose() public onlyWalletOwner {
