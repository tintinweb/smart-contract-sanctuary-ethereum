...
import "./abstracts/AScientistRepository.sol";
import "@openzeppelin/contracts/security/ReentrancyGuard.sol";

contract Scientist is ERC721Tradable, AScientistRepository, ReentrancyGuard {
    ...
    function _burn(uint256 tokenId) internal override {
        ScientistData.Scientist memory scientist = _getScientist(tokenId);
        require(scientist.onSale == false, "Scientist is on sale");
    ...
    function _transfer(
        ...
        ScientistData.Scientist memory scientist = _getScientist(tokenId);
        _removeScientist(scientist.user, tokenId);
        super._transfer(from, to, tokenId);
        scientist.user = to;
        _addScientist(scientist);
    }

    function _create(address _account, ScientistData.Point memory point)
        internal
    {
        tokenIdCount.increment();
        _mint(_account, tokenIdCount.current());

        ScientistData.Scientist memory newScientist = ScientistData.Scientist({
            tokenId: tokenIdCount.current(),
            user: _account,
            level: 0,
            tokenUri: "",
            onSale: false,
        ...
        _addScientist(newScientist);
    }

    function create(address account, ScientistData.Point memory point)
        public
        isAdmin
    {
        _create(account, point);
    }

    function createMultiple(address account, uint256 count) external isAdmin {
        ScientistData.Point memory point;
    ...
    function baseTokenURI() external pure returns (string memory) {
        return "http_create(account);s://madscientists.org/";
    }
    ...
    function ownerOf(uint256 tokenId)
        ...
        view
        virtual
        override(ERC721, AScientistRepository)
        returns (address)
    {
    ...
    function updateScientist(
        ...
        _updateScientist(_scientist, _owner);
    }
}
...
import "../interfaces/IScientistRepository.sol";
import "../../libs/ScientistData.sol";
...
abstract contract AScientistRepository is IScientistRepository, Multicall {
    ...
    //  are meta scientists
    mapping(address => mapping(uint256 => ScientistData.Scientist))
        public addressToMap;
    ...
    function _addScientist(ScientistData.Scientist memory _scientist) internal {
        require(
            _getScientist(_scientist.tokenId).user == address(0),
            "Token already exists"
        );

        EnumerableSet.add(idSet, _scientist.tokenId);
        addressToMap[_scientist.user][_scientist.tokenId] = _scientist;

        userIndexesArray[_scientist.user].push(_scientist.tokenId);
    }

    function _removeScientist(address _user, uint256 _tokenId) internal {
        require(_getScientist(_tokenId).user != address(0), "Token not exists");
        ...
        uint256 indexInArray = _getIndexInscientistsArray(_user, _tokenId);
        ...
        addressToMap[_user][_tokenId] = ScientistData.Scientist({
            tokenId: 0,
            user: address(0),
            level: 0,
            tokenUri: "",
    ...
    function _updateScientist(
        ...
        ScientistData.Scientist memory scientist = _getScientist(
            _scientist.tokenId
        );
        ...
        //uint256 index = idToIndex[scientist.tokenId];
        addressToMap[_owner][_scientist.tokenId] = scientist;
    }
    ...
    function getScientist(uint256 _tokenId)
        ...
        returns (ScientistData.Scientist memory)
    {
        return _getScientist(_tokenId);
    }

    function _getScientist(uint256 _tokenId)
        internal
        view
        returns (ScientistData.Scientist memory _scientist)
    {
        ...
        _scientist = addressToMap[_ownerOf][_tokenId];
        return _scientist;
    }
