...
contract Faucet is Ownable {
    ...
    uint256 public constant AIRDROP_INTERVAL = 1 days;
    mapping(address => uint256) public lastClaimed;
    mapping(address => bool) public executors;

    //  Sends 0.05 ETH to the sender when the faucet has enough funds
    //  Only allows one withdrawal every day
    function withdraw(address _recipient) external {
        require(executors[msg.sender], "Only executors");
        require(
            address(this).balance >= AIRDROP_AMOUNT,
            "This faucet is empty. Please check back later."
        );
        require(
            lastClaimed[_recipient] <= block.timestamp - AIRDROP_INTERVAL,
            "You can only withdraw once every day. Please check back later."
        );

        Address.sendValue(payable(_recipient), AIRDROP_AMOUNT);
