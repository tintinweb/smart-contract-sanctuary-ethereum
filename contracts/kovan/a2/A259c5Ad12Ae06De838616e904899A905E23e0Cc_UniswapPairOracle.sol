...
contract UniswapPairOracle is Ownable {
    ...
    function twap(address token, uint256 pricePrecision) external view returns (uint256 amountOut) {
        ...
        if (token == token0) {
            amountOut =
                price0Average.mul(pricePrecision).decode144() * (10**_token1MissingDecimals) / 10**_token0MissingDecimals;
        } else {
            require(token == token1, "PairOracle: INVALID_TOKEN");
            amountOut =
                price1Average.mul(pricePrecision).decode144() * (10**_token0MissingDecimals) / 10**_token1MissingDecimals;
        }
    }
    ...
    function spot(address token, uint256 pricePrecision) external view returns (uint256 amountOut) {
        ...
        uint256 _price = 0;
        if (token == _token0) {
            _price = (_reserve1 * (10**_token1MissingDecimals) * pricePrecision) / _reserve0 / 10**_token0MissingDecimals;
        } else {
            _price = (_reserve0 * (10**_token0MissingDecimals) * pricePrecision) / _reserve1 / 10**_token1MissingDecimals;
        }
        return _price;
    }
