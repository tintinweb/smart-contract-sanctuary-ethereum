...
contract TestSecLocker is Owned {
    ...
    function controllerLock(string calldata _meshAddress, uint256 _testSecAmount) onlyOwner public {
        _lock(_meshAddress, _testSecAmount);
    }
    ...
    function _lock(string memory _meshAddress, uint256 _testSecAmount) internal {      
    ...
    function unlock(string calldata _meshAddress, uint256 _testSecAmount) public {
        _unlock(_meshAddress, msg.sender, _testSecAmount);
    }

    function burn(string calldata _meshAddress, uint256 _testSecAmount) public { 
        _unlock(_meshAddress, address(0x0000000000000000000000000000000000000000), _testSecAmount);
    }
    ...
    function _unlock(string memory _meshAddress, address _holder, uint256 _testSecAmount) internal {
        ...
        uint256 testSecBalance = _testSecAmount / TRUNCATE_SCALE;
        _testSecAmount = testSecBalance * TRUNCATE_SCALE;
        require(testSecToken.transfer(_holder,_testSecAmount), "Unable to complete transfer");
        emit TestSecUnlocked( _holder, _meshAddress, _testSecAmount);
    }
