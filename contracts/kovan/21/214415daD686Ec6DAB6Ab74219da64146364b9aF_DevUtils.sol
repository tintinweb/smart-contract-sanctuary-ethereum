    ...
    AssetBalance
{
    ...
    function getOrderRelevantState(LibOrder.Order memory order, bytes memory signature, address exchangeAddr)
        public
        returns (
        ...
        )
    {
        ...
        orderInfo = IExchange(exchangeAddr).getOrderInfo(order);

        // Validate the maker's signature
        address makerAddress = order.makerAddress;
        isValidSignature = IExchange(exchangeAddr).isValidOrderSignature(
            order,
            signature
        );
    ...
    function getOrderRelevantStates(LibOrder.Order[] memory orders, bytes[] memory signatures, address[] memory exchangeAddrs)
        public
        returns (
