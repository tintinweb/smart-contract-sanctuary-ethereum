...
contract swap{
    ...
    function dex(
        ...
        external payable{
            ...
            for(uint i=0; i<dexRoute.length; i++){
                ...
                if(dexRoute[i] == 0)
                {
                    ...
                    console.log("Path0",updPath[0]);
                    console.log("Path1",updPath[1]);

                    if(options[optionIndex] == 0 && dexRoute[0] == 0){
                    ...
                    else if (options[optionIndex] == 1 && dexRoute[0] == 0) { 
                        ...
                        IERC20(updPath[0]).approve(UNISWAP_ROUTER_ADDRESS, amountin);
                        IUniswapV2Router02(UNISWAP_ROUTER_ADDRESS).swapExactTokensForETH(amountin,amountOut,updPath,payable(address(this)),deadline);
                        optionIndex++;
                    }
            ...
            { 
                ...
                if(address(singleswap.assetIn) == ETH){
                    console.log("RECEVING ETH",address(this).balance);
        ...
        if(dexRoute[0] == 0){
            eventEmit(uniswapAmountIn[0], tokenIn, tokenOut, sender);
        }

        else if(dexRoute[0] == 1){
            eventEmit(singleswap.amount, tokenIn, tokenOut, sender);
        }

        else{
            eventEmit(swaps[0].amount, tokenIn, tokenOut, sender);
        }
    ...
    function eventEmit(uint amountIn, address TokenIn, address TokenOut, address msgSender) internal{
        console.log("***************************after swap", IERC20(address(TokenOut)).balanceOf(address(this)));
        uint amountRecevied;
        amountRecevied = IERC20(TokenOut).balanceOf(address(this));
