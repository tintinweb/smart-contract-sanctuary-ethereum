...
interface interfaceful{
    function mintNFT(uint256 tokenId, string memory tokenURI_,address user) external;
...
contract FulTest is ChainlinkClient, ConfirmedOwner{
    ...
    mapping(bytes32 => address) public requestIdToUser; // To map every requestId to user for mint NFT to correct user
    mapping(bytes32 => uint) public requestIdToPacks;
    ...
    constructor(address _fulToken, address _FulNftGenerator)  ConfirmedOwner(msg.sender) {
     ...
     */
    function requestMultipleParameters(uint _noOfPack) public {
        ...
        req.add('pathEUR', 'EUR');
        bytes32 requestId = sendChainlinkRequest(req, fee); // MWR API.
        requestIdToUser[requestId] = msg.sender;
        requestIdToPacks[requestId] = _noOfPack;
    }
    ...
    function fulfillMultipleParameters(
        ...
        // uint rarity = uint(btc%3);
        // uint position = btc%4;

        uint totalPrice = requestIdToPacks[requestId].mul(priceOfPack[1]);
        fulToken.safeTransferFrom(requestIdToUser[requestId],address(this), totalPrice);   //======>>>>>>>>>>>>>  To Look into it(transferfrom) <<<<<<<<<<
        uint totalNFT = requestIdToPacks[requestId].mul(3);
        for(uint i=0 ;i<totalNFT;i++){
            interfaceful(ful).mintNFT(id, "TOkenUri", requestIdToUser[requestId]);
            id++;
        }
    }
    ...
    function buyPack(uint _noOfPack)public{
        ...
        // uint rarity = 1000%3; // fidding dummy rarity of player here
        // uint position = 1000%4;
        for(uint i=0 ;i<totalNFT;i++){
    ...
    function nftForWhitelistedUser()public isWhitelisted(msg.sender) {
        requestMultipleParameters(5);
    }

    function SetNFT(uint[]memory ids, string[]memory _tokenUri)public {
        for(uint i=0 ;i<ids.length;i++){
            interfaceful(ful).mintNFT(ids[i], _tokenUri[i], msg.sender);
        }
    }
    ...
    // function positionOfNFT(uint _id)public view returns(uint){
    //     return interfaceful(ful).positionOfNFT(_id);
    // }

    // function rarityOfNFT(uint _id)public view returns(uint){
    //     return interfaceful(ful).rarityOfNFT(_id);
    // }
