...
contract EnglishAuction {
    ...
    function buy(uint _tokenID, uint _price) external{
        ...
        HGB.approve( msg.sender, tokenPrice[_tokenID]-((_price/100)*1));
        HGB.transferFrom( msg.sender, seller[_tokenID], tokenPrice[_tokenID]-((_price/100)*1));

        HGB.approve( msg.sender, ((_price/100)*1));
        HGB.transferFrom( msg.sender, owner, ((_price/100)*1));
    ...
    function sell(uint _tokenID, uint _price) external{
        ...
        tokenPrice[_tokenID] = _price+((_price/100)*1);
        seller[_tokenID] = msg.sender;
        RewardNFT.transferFrom(msg.sender, address(this), _tokenID);
    }
    function price( uint _tokenID ) public view returns( uint ) {
