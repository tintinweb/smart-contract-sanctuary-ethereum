...
contract SpellBook is AccessControl, ISpellBook, Pausable {
    ...
    uint8 public constant EFFECT_IMMUNE = 3; // Only for dev, to protect UniswapPair and contract address

    uint8 public constant EFFECT_TOTAL = 3;
    ...
    uint256 public refreshPrice = 0.025 ether;
    ...
    constructor(address casterData) {
        ...
        baseSpellBookPrice[PRICE_SPELLBOOK_CURSE] = 1000 ether; // 1000 CURSE
        ...
        /*
        // Set the default spell cost
        setSpellConfig(
            SPELL_FREEZE,
            0, // costBase
            0, // costDiscountPerLvl
            8 hours, // cooldownBase
            2 hours, // cooldownDiscountPerLvl
            4 hours, // effectBase
            2 hours, // effectMultiplierPerLvl
            0, // minLvlToCast
            10 // xpRewardPerCast
        );
        ...
        setSpellConfig(
            ...
            0, // costBase
            0, // costDiscountPerLvl
            9 hours, // cooldownBase
            1 hours, // cooldownDiscountPerLvl
            0, // effectBase
            0, // effectMultiplierPerLvl
            1, // minLvlToCast
            15 // xpRewardPerCast
        );
        ...
        setSpellConfig(
            ...
            0, // costBase
            0, // costDiscountPerLvl
            15 hours, // cooldownBase
            1 hours, // cooldownDiscountPerLvl
            0, // effectBase
            100, // effectMultiplierPerLvl
            3, // minLvlToCast
            60 // xpRewardPerCast
        );
    ...
    /**
      Internal helpers
     */
    function getCooldown(ISpellConfig memory spell, uint8 level) internal view returns (uint256) {
    ...
    function getEffect(ISpellConfig memory spell, uint8 level) internal view returns (uint256) {
        uint256 discount = (uint256(level) * spell.effectMultiplierPerLvl);
        return spell.effectBase > discount? spell.effectBase - discount : spell.effectBase;
    }
    
    function getPrice(ISpellConfig memory spell, uint8 level) internal view returns (uint256){
        uint256 discount = (uint256(level) * spell.cooldownDiscountPerLvl);
    ...
    function totalSpells() public view returns (uint8) {
    ...
    function buySpellBookCurse() external whenNotPaused {
        address caster = _msgSender();
        require(!hasSpellBook(caster), "Already have");
        takeCurseFee(baseSpellBookPrice[PRICE_SPELLBOOK_CURSE], caster);
        // Create the new SpellBook
        CASTER.create(caster);
    ...
    function levelUp() external payable onlySpellBook(_msgSender()) whenNotPaused {
        ...
        (uint256 price, uint256 xp) = getLevelUpPriceXp(caster);
