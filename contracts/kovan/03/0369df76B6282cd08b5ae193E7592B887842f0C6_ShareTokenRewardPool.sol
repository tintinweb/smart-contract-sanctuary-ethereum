...
contract ShareTokenRewardPool {
    ...
    struct PoolInfo {
        ...
        uint256 accShareTokenPerShare; // Accumulated tSHAREs per share, times 1e18. See below.
    ...
    uint256 public tSharePerSecond = 0.00186122 ether; // 59500 shareToken / (370 days * 24h * 60min * 60s)
    ...
    function pendingShare(uint256 _pid, address _user) external view returns (uint256) {
        ...
        return user.amount.mul(accShareTokenPerShare).div(1e18).sub(user.rewardDebt);
    }
    ...
    // Safe shareToken transfer function, just in case if rounding error causes pool to not have enough tSHAREs.
    function safeShareTokenTransfer(address _to, uint256 _amount) internal {
        uint256 _shareTokenBal = shareToken.balanceOf(address(this));
        if (_shareTokenBal > 0) {
            if (_amount > _shareTokenBal) {
                shareToken.safeTransfer(_to, _shareTokenBal);
            } else {
