...
contract LmPoolV3 is Ownable, ReentrancyGuardUpgradeable, IERC721Receiver {
    ...
    // Lp pool info
    mapping(address => mapping(address => uint256)) public lpTokensPool;
    // Info of each pool.
    ...
    */
    function poolLength() external view returns (uint256) {
        ...
        /**
    @notice Add a factory for uniswap v3.
    @param  _factory: factory contract address
    @dev  This functiona set the factory address for 
    */
    function setFactoryAddress(address _factory) external onlyOwner {
        require(address(_factory) != address(0),"Zero Address");
        factory = _factory;
    }
    ...
    @dev    this function definition is marked "external" because this fuction is called only from outside the contract.
    */
    function add(IERC721 _erc721Token,address _token0, address _token1) external onlyOwner {
        require(address(_erc721Token) != address(0),"Zero Address");
        require(lpTokensStatus[_token0][_token1] != true, "LP token already added");
    ...
    */
    function deposit(
        ...
        (,,,,,,,_amount,,,,) = pool.lpToken.positions(_tokenId);
        if (_amount > 0) {
