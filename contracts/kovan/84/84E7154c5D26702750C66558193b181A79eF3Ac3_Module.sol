...
contract Module is
    ...
    Ownable
{
    constructor(address _timelock) ERC721("Module", "MDL") {
        timelock = _timelock;
    }
    ...
    function burn(uint256 tokenId) public override {
        require(ownerOf(tokenId) == msg.sender, "You are not owner of Module");
        super._burn(tokenId);
    }
    ...
    function setBaseURI(string calldata newUri) external override onlyTimelock {
    ...
    function _create(address _to) internal returns (uint256 _tokenId) {
        moduleIds.increment();
        super._mint(_to, moduleIds.current());
        return moduleIds.current();
    }
}
