...
contract USDPairOracle {
    // Address token want to change to price usd
    address public token;

    //UniswapPairOracle
    IPairOracle public tokensPair;

    //Address data feed from chainlink to get price usd
    ...
    constructor(
        ...
        require(address(_tokensPair) != address(0), "USDPairOracle::Tokens pair address is invalid");
    ...
    /// @notice get usd price spot of token from uniswap pair and chainlink data feed
    /// @param _pricePrecision: precision for price, usual 1e18
    /// @return spot price
    function spot(uint256 _pricePrecision) public view returns (uint256) {
    ...
    /// @notice get usd price twap of token from uniswap pair and chainlink data feed
    /// @param _pricePrecision: precision for price, usual 1e18
    function twap(uint256 _pricePrecision) public view returns (uint256) {
        (, int256 price, , , ) = chainLinkDataFeed.latestRoundData();
        uint8 _decimals = chainLinkDataFeed.decimals();
