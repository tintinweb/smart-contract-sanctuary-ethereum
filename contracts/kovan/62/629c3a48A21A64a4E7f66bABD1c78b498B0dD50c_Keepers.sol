...
contract Keepers is KeeperCompatibleInterface, Initializable {

    address public owner;

    address public sale;
    uint128 public timestamps;
    ...
    function initialize(address _sale, uint128 _timestamps) public initializer {
        sale = _sale;
    ...
    function setKeepers(address _sale, uint128 _timestamps) public onlyOwner{
        require(!isLive, "isLive");
        sale = _sale;
        timestamps = _timestamps;
        isLive = true;
    }
    ...
    function checkUpkeep(bytes calldata checkData) external view override returns (bool upkeepNeeded, bytes memory performData ) {
        upkeepNeeded = PublicGovSale(sale).totalStaked() == 0 && block.timestamp >= PublicGovSale(sale).calculation();
    ...
    function performUpkeep(bytes calldata performData) external override {
        ...
        if(PublicGovSale(sale).totalStaked() == 0 && block.timestamp >= PublicGovSale(sale).calculation()){
            PublicGovSale(sale).setTotalStaked();
            isLive = false;
        }
        performData;
