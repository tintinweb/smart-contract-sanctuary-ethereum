    ...
    function claimDividend(
        ...
        for (uint256 i = 0; i < stox_.length; i++) {
            ...
            if (availablePools[stox_[i]] >= transferrableAmount[i]) {
                ...
                availablePools[stox_[i]] = availablePools[stox_[i]].sub(transferrableAmount[i]);
                // update the claimedDividends for the user (add transferrable amount to the current)
                claimedDividends[user_][stox_[i]] = claimedDividends[user_][stox_[i]].add(transferrableAmount[i]);
            }
        }
    }
}