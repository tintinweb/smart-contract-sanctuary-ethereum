...
// File: my/new_cow/grade/CowExplore.sol
...
contract CowExplore is EIP712,ReentrancyGuard,AdminTransfer{
    address public signerAddress;

    function cowExplore(uint deadline,uint256 wolfId,address fram,bytes calldata signature) view public {
        require(block.timestamp <= deadline, "expired");
        require(check(signerAddress,hashData(deadline,wolfId,fram),signature), "sign error");
    }

    constructor(address _signerAddress) EIP712("CowExplore", "1") {
    ...
    function hashData(uint deadline,uint256 wolfId,address fram) public view returns (bytes32) {
        return _hashTypedDataV4(keccak256(abi.encode(
            keccak256("CowExploreRequest(uint256 deadline,uint256 wolfId,address fram)"),
            deadline,
            wolfId,
            fram
        )));
