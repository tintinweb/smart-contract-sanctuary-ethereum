...
contract Oracle is IOracle, Ownable {
    ...
    function getPriceData(string memory baseSymbol, string memory quoteSymbol) public view returns (PriceData memory) {
        uint256 basePrice = 1e9;
        uint256 quotePrice = 1e9;
        if (keccak256(abi.encodePacked(baseSymbol)) != keccak256(abi.encodePacked("USD"))) {
            basePrice = pricesMapping[baseSymbol];
        }
        if (keccak256(abi.encodePacked(quoteSymbol)) != keccak256(abi.encodePacked("USD"))) {
            quotePrice = pricesMapping[quoteSymbol];
        }
        return PriceData({
