...
contract FlowerFamEcoSystem is SimpleAccessUpgradable {
    ...
    function initialize(        
        ...
        speciesToHoneyProduction[0] = 5 ether;
        speciesToHoneyProduction[1] = 7 ether;
        speciesToHoneyProduction[2] = 12 ether;
        speciesToHoneyProduction[3] = 20 ether;
        speciesToHoneyProduction[4] = 35 ether;
        newGenHoneyProduction = 25 ether / 10; /// @dev 2.5 honey

        beeProductionBonus = 5; /// @dev 5%
        upgradeProductionBonus = 5; /// @dev 5%        
    }
    ...
    function _getTotalNotAccumulatedProductionOfUser(address user, uint256[] memory flowersWithBees) internal view returns (uint256) {
        ...
        for (uint i = 0; i < flowersWithBees.length; i++) {
            if (flowerToBee[i].beeId != 0)
                unAccumulated += _getProductionFromBee(i, true, flowerToBee[i].beeId);
            if (newGenFlowerToBee[i].beeId != 0)
                unAccumulated += _getProductionFromBee(i, false, newGenFlowerToBee[i].beeId);     
        }
    ...
    function _getProductionFromBee(uint256 flowerId, bool isFam, uint256 beeId) internal view returns(uint256) {
        uint256 flowerBaseProduction = isFam ? 
            speciesToHoneyProduction[flowerFamNFT.getSpeciesOfFlower(flowerId)] :
        ...
        for (uint i = 0; i <= reductions - reductionsStart; i++) {
            ...
            if (i == 0) {
                ...
                uint256 timeSpentAtFirstReduction = block.timestamp < nextReductionAfterStart ? 
                    block.timestamp - beeLastInteraction : 
                    nextReductionAfterStart - beeLastInteraction;
                totalEarned += flowerBaseProduction * (100 - reductionsStart * beeProductionBonus) * timeSpentAtFirstReduction / 1 days;
            
            /// @dev Here we just calculate one week worth of rewards at that level
            } else if (i < reductions - reductionsStart) {
                totalEarned += flowerBaseProduction * (100 - (reductionsStart + i) * beeProductionBonus) * powerCycleBasePeriod / 1 days;
            ...
            /// @dev At last reduction we add period from last reduction until now with that reduction rate as reward.
                ...
                totalEarned += flowerBaseProduction * (100 - reductions * beeProductionBonus) * timeSpentAtLastReduction / 1 days;
            }
        }
    ...
    function stakeFlowerFamFlower(uint256 flowerFamId, uint256 species, bytes32[] calldata proof) external {
        require(flowerFamNFT.validateSpecies(flowerFamId, species, proof), "Provided species is not correct for provided flower ID");
    ...
    function unstakeFlowerFamFlower(uint256 flowerFamId, uint256 species, bytes32[] calldata proof) external {
        require(flowerFamNFT.validateSpecies(flowerFamId, species, proof), "Provided species is not correct for provided flower ID");
    ...
    function batchStakeFlowerFamFlowers(uint256[] calldata flowerFamIds, uint256[] calldata species, bytes32[][] calldata proofs) external {
        require(flowerFamIds.length > 0, "No fams provided");
        require(species.length == flowerFamIds.length, "Each fam needs a species provided");
        require(proofs.length == flowerFamIds.length, "Each fam needs a proof provided");

        for (uint i = 0; i < flowerFamIds.length; i++) {
            require(flowerFamNFT.validateSpecies(flowerFamIds[i], species[i], proofs[i]), "Provided species is not correct for provided flower ID");
        }

        uint256 additionalHoneyProduction;
        for (uint i = 0; i < flowerFamIds.length; i++) {
            uint256 flowerProduction = speciesToHoneyProduction[species[i]];
            flowerProduction += _getProductionFromUpgrade(additionalHoneyProduction, flowerFamIds[i]);
        ...
        for (uint i = 0; i < flowerFamIds.length; i++)
            flowerFamNFT.stake(msg.sender, flowerFamIds[i]);

        emit UpdateTotalProductionPerDay(msg.sender, userHoneyProduction.totalProductionPerDay);
    }

    function batchUnstakeFlowerFamFlower(uint256[] calldata flowerFamIds, uint256[] calldata species, bytes32[][] calldata proofs) external {
        require(flowerFamIds.length > 0, "No fams provided");
        require(species.length == flowerFamIds.length, "Each fam needs a species provided");
        require(proofs.length == flowerFamIds.length, "Each fam needs a proof provided");
        ...
        for (uint i = 0; i < flowerFamIds.length; i++) {
            if (flowerToBee[flowerFamIds[i]].beeId != 0) {
                releaseBeeFromFlower(flowerFamIds[i], flowerToBee[flowerFamIds[i]].beeId);
            }

            uint256 flowerProduction = speciesToHoneyProduction[species[i]];
            flowerProduction += _getProductionFromUpgrade(reducedHoneyProduction, flowerFamIds[i]);
        ...
        for (uint i = 0; i < flowerFamIds.length; i++)
            flowerFamNFT.unstake(msg.sender, flowerFamIds[i]);
    ...
    function upgradeFlower(address user, uint256 flowerFamId) external onlyAuthorized {
        ...
        uint256 additionalHoneyProduction = speciesToHoneyProduction[flowerFamNFT.getSpeciesOfFlower(flowerFamId)];
    ...
    function restorePowerOfBee(address user, uint256 flowerId, bool isFam, uint256 beeId, uint256 restorePeriods) external {    
        if (isFam) {
    ...
    function claimTotalNotAccumulatedProductionOfUser(address user, uint256[] memory flowersWithBees) external {
        UserHoneyProduction storage userHoneyProduction = userToProductionInfo[user];
         
        uint256 unAccumulated =  _getTotalNotAccumulatedProductionOfUser(user, flowersWithBees);
    ...
    function getAttachedFlowerOfBee(uint256 beeId) external view returns (uint256) {
        uint256 flowerId = 0;
        for (uint i = 1; i < flowerFamNFT.totalSupply(); i++) {
    ...
    function getAttachedNewGenFlowerOfBee(uint256 beeId) external view returns (uint256) {
        uint256 flowerId = 0;
        for (uint i = 1; i < flowerFamNewGenNFT.totalSupply(); i++) {
...
interface IFlowerFam {
    ...
    function getSpeciesOfFlower(uint256 tokenId) external view returns (uint8);
    ...
    function validateSpecies(uint256 tokenId, uint256 species, bytes32[] calldata proof) external view returns (bool);
}
