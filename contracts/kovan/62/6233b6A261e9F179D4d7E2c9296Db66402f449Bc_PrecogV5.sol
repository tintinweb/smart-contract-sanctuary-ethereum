...
contract PrecogV5 is IPrecogV5 { 
    ...
    modifier isFirstTimeRequestedWithdrawal() {
    ...
    mapping(address => mapping(address => mapping(uint256 => bool))) isIncreasedInvesmentInCycle; ///// isDepositedInCycle[token][user][0] = false
    ...
    function updateProfitCycleToCurrentCycle(address token) public override {
        ...
        uint48 newCycleStartTime = lastCycle.endTime;
        ...
        while (uint48(block.timestamp) >= lastCycle.endTime) {
            ...
            profitCycles[token].push(Cycle(lastCycle.id + 1, newCycleStartTime, newCycleEndTime));
            totalInvestmentUnits[token].push(liquidity[token] * (duration));
            lastCycle = profitCycles[token][profitCycles[token].length - 1];
        }
    }
    ...
    function sendProfit(address token, uint256 profitAmount) external override onlyMiddleware {
        require(isExistingToken[token], "PrecogV5: TOKEN_WAS_NOT_ADDED_TO_POOL");
        if(liquidity[token] == 0) profitAmount = 0;
        updateProfitCycleToCurrentCycle(token);
    ...
    function sendWithdrawalRequestTokens(address token) external override onlyMiddleware {
        require(isExistingToken[token], "PrecogV5: TOKEN_WAS_NOT_ADDED_TO_POOL");
        ...
        uint actualBalance = getActualBalance(token);
        if(actualBalance < requestedAmount) {
            IERC20(token).safeTransferFrom(msg.sender, address(this), requestedAmount - actualBalance);
        }
    ...
    function isInCurrentCycle(address token) public view override returns (bool) {
        return profitCycles[token][currentProfitCycleId[token]].endTime > block.timestamp;
    }
   ...
   function _increaseInvestment(address token, address account, uint256 amount) internal {
        require(isExistingToken[token], "PrecogV5: INVALID_TOKEN");
        require(isIncreasedInvesmentInCycle[token][account][profitCycles[token].length - 1] == false);
        ...
        unit = (_profitCycles[_profitCycles.length - 1].endTime - uint48(block.timestamp)) * amount;
        ...
        if (investment.lastProfitCycleId < updatedLatestCycle) {
            ...
            // Calculate profit in last transaction
            if(investment.lastProfitCycleId > 0) {
                if (totalInvestments[investment.lastProfitCycleId - 1] > 0) {
    ...
    function withdraw(address to, address token, uint256 amount) external override {
        ...
        IERC20(token).safeTransfer(address(core), withdrawalFee);
