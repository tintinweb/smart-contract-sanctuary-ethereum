...
using SafeERC20 for IERC20;
    ...
    /// @notice price of Box in USD (Box/USD)    
    ...
    */
    function buyForETH(uint256 _BoxID) public payable returns (bool) {        
        require (controller.isAddressWhitelisted(msg.sender), "Seller: You are not whitelisted, you can't buy Box");  
     ...
     * @param _BoxID ID of Box to buy  
    */
    function buyForToken(address _tokenToPay, uint8 _decimals, uint256 _BoxID) public payable returns (bool) {        
        require (controller.isAddressWhitelisted(msg.sender), "Seller: You are not whitelisted, you can't buy Box");        
        
        uint256 amountOfToken = getTokenPrice(_tokenToPay, _decimals);  
        address payable treasury = payable(controller.getTreasureContract());
        require (IERC20(_tokenToPay).balanceOf(msg.sender) >= amountOfToken, "Insufficient balance to buy Box");
...
interface IBox {
    ...
    */
     ...
     * - sender must be the Seller contract.
