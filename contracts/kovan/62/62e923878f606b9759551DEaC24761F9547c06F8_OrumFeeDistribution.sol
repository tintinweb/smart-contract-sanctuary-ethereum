...
// File: OrumFeeDistribution.sol

contract OrumFeeDistribution is IOrumRevenue{
    ...
    event OrumFeeDistributionoMATICBalanceUpdated(uint _amount);
    ...
    function setAddresses(address _borrowerOpsAddress, address _activePoolAddress, address _rewardsPoolAddress, address _oMATIC) external {
        ...
        oMATICToken = IoMATICToken(_oMATIC);
    }
    ...
    function _checkpoint_token() internal {
        uint token_balance = oMATICToken.balanceOf(address(this));
    ...
    function claim(uint _tokenIndex) external returns (uint) {
        ...
        if (amount != 0) {
            oMATICToken.transfer(msg.sender, amount);
    ...
    receive() external payable{
        _requireCallerIsBorrowerOpsOrActivePoolOrRewardsPool();
        _checkpoint_token();
        emit OrumFeeDistributionoMATICBalanceUpdated(msg.value);
    }

    function _requireCallerIsBorrowerOpsOrActivePoolOrRewardsPool() internal view {
        require(msg.sender == borrowerOpsAddress || msg.sender == activePoolAddress || msg.sender == rewardsPoolAddress, "FeeDistribution: caller not BO or ActivePool");
    }
}