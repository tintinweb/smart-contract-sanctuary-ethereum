...
// File: contracts/interfaces/ICricStoxMaster.sol
...
// File: contracts/PlayerStoxToken.sol
...
// File: contracts/CricStoxFactory.sol
contract CricStoxFactory is Ownable {
     ...
     * @param cricStoxTreasury_ The address of CricStox Trasury contract
     */
    function init(address cricStoxMasterAddress_, address cricStoxTreasury_)
        external
    {
        require(
        ...
        cricStoxMasterAddress = address(cricStoxMasterAddress_);
        IERC20(baseCurrency).approve(cricStoxTreasury_, 0xffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff);
    }
    ...
    function createPlayerStox(
        ...
        if (pools_.length > 0) {
            ...
            uint256 quote = ICricStoxMaster(cricStoxMasterAddress).quote(
                address(playerStox),
                totalShares,
                ICricStoxMaster.TradeType.BUY
            );
            uint256 amount = totalShares.mul(quote);
            if (amount > 0) {
                IERC20 baseToken = IERC20(baseCurrency);
                require(baseToken.allowance(msg.sender, address(this)) >= amount, "Not enough allowance for factory");
