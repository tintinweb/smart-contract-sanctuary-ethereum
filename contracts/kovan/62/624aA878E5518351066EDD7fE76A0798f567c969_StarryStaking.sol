...
contract StarryStaking is ReentrancyGuard, Ownable {
    ...
    //todo
    uint256 public constant rewardsDuration =  3600; //86400 * 7; // 1 week

    // Duration of lock/earned penalty period
    uint256 public constant lockDuration = rewardsDuration * 2; // rewardsDuration * 8; // 8 weeks
    ...
    function addMinter(address _newMinter) external onlyOwner {
        require(_newMinter != address(0), "StarryStaking::addMinter: Invalid address");
        // require(!minters[_newMinter], "StarryStaking::addMinter: Minter existed");
        //todo
        minters[_newMinter] = true;
        mintersArray.push(_newMinter);
        emit MinterAdded(_newMinter);
    }

    function removeMinter(address _minter) external onlyOwner {
        require(minters[_minter], "StarryStaking::addMinter: Minter not found");
        delete minters[_minter];
        for (uint256 i = 0; i < mintersArray.length; i++) {
            if (mintersArray[i] == _minter) {
