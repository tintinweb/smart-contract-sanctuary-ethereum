...
contract RewardCenter is Ownable {
    ...
    event SetOpenAccountReward(
        address indexed owner,
        uint256 amountPerAccout
    );
    ...
    event ClaimPositionReward(address indexed EOA, uint256 epochRound, uint256 amount);

    event ClaimOpenAccountReward(address indexed EOA, address indexed account, uint256 amount);

    // Open Account Reward
    function setEventCenter(address _eventCenter) public onlyOwner {
        require(_eventCenter != address(0), "CHFRY: EventCenter address should not be 0");
    ...
    function claimOpenAccountReward(address EOA, address account) public {
        ...
        emit ClaimOpenAccountReward(EOA, account, rewardAmountPerAccoutOpen);
    }


    // Postition Reward
    function startNewPositionRewardEpoch( uint256 rewardAmount) public onlyOwner {
        require(EventCenterLeveragePositionInterface(eventCenter).isInRewardEpoch()==false,
        "CHFRY: already in reward epoch");
        EventCenterLeveragePositionInterface(eventCenter).startEpoch(rewardAmount);
    }

    function releasePositionReward(
        uint256 epochRound,
        bytes32 merkelRoot
    ) public onlyOwner {
        require(merkelRoot != bytes32(0),"CHFRY: merkelRoot should not be 0");
        uint256 round = EventCenterLeveragePositionInterface(eventCenter).epochRound();
    ...
    function claimPositionReward(
        ...
        require(merkelRoots[epochRound] != bytes32(0), "CHFRY: this round reward is not released");
        bytes memory leafData = abi.encodePacked(msg.sender,amount);
        require(MerkleProof.verify(proof,merkelRoots[epochRound],sha256(leafData)) == true ,"CHFRY: MerkleProof Fail");
        IERC20(rewardToken).transfer(msg.sender, amount);
        emit ClaimPositionReward(msg.sender,epochRound,amount);
    }

    function drainReward(uint256 amount, address to) public onlyOwner{
        require(to != address(0), "CHFRY: should not drain reward token to address(0)");
        IERC20(rewardToken).transfer(to, amount);
    }

    function cleanReward(address to) public onlyOwner{
        require(to != address(0), "CHFRY: should not drain reward token to address(0)");
    ...
    function inEpoch() public view returns(bool){
        return EventCenterLeveragePositionInterface(eventCenter).isInRewardEpoch();
    } 
}
