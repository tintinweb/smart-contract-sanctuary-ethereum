...
contract honeyPotChecker{
    ...
    //receive() external payable {
        //assert(msg.sender == WETH); // only accept ETH via fallback from the WETH contract
    //}
    function honeyTest(address[] calldata path,address[] calldata factory,uint[] calldata afterFees,address payable router) external virtual returns(uint){
        uint isSafe = 0;

            //sETFT(amountIn,0,path,address(this),factory[i]);
            isSafe = 1;
            try trades(router).sETFT(ERC20(path[0]).balanceOf(router),0,path,router,factory,afterFees){
                isSafe = 2;
                ...
                if(ERC20(path[1]).balanceOf(router)>0){
                    isSafe = 3;
                    try trades(router).sETFT(ERC20(path[1]).balanceOf(router),0,path,router,factory,afterFees){
                        isSafe = 4;
