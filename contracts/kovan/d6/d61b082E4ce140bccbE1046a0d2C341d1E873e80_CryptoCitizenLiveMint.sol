...
{   
    ...
    function readNFTOwnedTokenIDs(
        ...
        uint index;
        for(uint z; z < OwnedIDs.length; z++)
        {
            if(OwnedIDs[z] != 0)
            {
                FormattedOwnedIDs[index] = OwnedIDs[z];
                index++;
            }
        }
    ...
    function readNFTsOwnedTokenIDs(
        ...
        {
            try IERC721(NFTAddresses[x]).ownerOf(TokenIDs[0]) //checks if valid ERC721
            {
