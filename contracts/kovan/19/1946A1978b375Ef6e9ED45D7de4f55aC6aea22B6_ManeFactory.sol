...
contract ManeBase is ERC721A, Ownable {
    ...
    mapping(address => uint256[]) tokenIDByHolder;

    // Platform fee ratio in PPM
    uint256 public platformFeePPM = 0;

    uint256 isRefundable = 0;
    ...
    */ 
    constructor(string memory name_, string memory symbol_, uint256[] memory u256s,
        ...
        for( uint256 i = 0; i < refundTimes_.length; i++) {
            ...
            require(t.endTime > oldEndTime, "ManeBase: refundTimes order invalid");
            require(t.ratioPPM > oldRatio, "ManeBase: refundRatio order invalid");
        ...
        require(totalRefundRatios <= 1 * 1000 * 1000, "ManeBase: refundRatios overflow");
    ...
    function getAll() public view returns (uint256[12] memory) {
        uint256[12] memory u;
    ...
    function mint(address minter, uint256 mint_price, uint256 sign_deadline, bytes32 r, bytes32 s, uint8 v) payable whenNotPaused public returns (uint256) {
        ...
        if (block.timestamp < presaleStartTime) {
            ...
            require(msg.value >= mint_price, "ManeBase: insufficant value for presale");
            isPresale = 1;
        } else if (block.timestamp >= saleStartTime && block.timestamp <= saleEndTime) {
            // Period: Public sale perild
            require(msg.value >= _salePrice, "ManeBase: insufficant value for sale");
            isSale = true;
        } else {
        ...
        if (isPresale == 1) {
            ...
            require(presaleMintedCount < presaleMaxSupply, "ManeBase: No more tokens could be mint at presale period");
            presaleMintedCount++;

            require(presaleMintCountByAddress[msg.sender] < presaleMaxMintCountPerAddress, "ManeBase: Address mint limit exceed(A)");
            presaleMintCountByAddress[msg.sender]++;
        } else if (isSale == true) {
            //requireMintSign(minter, mint_price, sign_deadline, r, s, v);

            require(saleMintCountByAddress[msg.sender] < saleMaxMintCountPerAddress, "ManeBase: Address mint limit exceed(B)");
            saleMintCountByAddress[msg.sender]++;
        } else {
            revert("ManeBase: No in sale time period");
        }
        ...
        uint256 platformGot = mint_price * platformFeePPM / 10e6;
        uint256 collectorGot = (mint_price - platformGot) * getCollectorTotalRatioPPM() / 10e6;
    ...
    function refund(uint256 tokenID) public {
        require(isRefundable != 0);
        require(msg.sender == ownerOf(tokenID), "ManeBase: Not token owner");
        ...
        require(refundRatioPPM > 0, "ManeBase: refund is not avail");
        ...
        uint256 refundValue = mintInfo.price * refundRatioPPM / 10e6; 

        /// 4. Do refund
        mintedAmount -= refundValue;

        uint256 platformReturn = refundValue * platformFeePPM / 10e6;
        uint256 collectorReturn = (refundValue - platformReturn) * getCollectorTotalRatioPPM() / 10e6;
    ...
    // Share owners collect their shares
    function shareCollect() public {
        ...
        for (uint256 i = 0; i < _shareList.length; i++) {
            uint256 collectValue = b * _shareList[i].ratioPPM / 10e6;
            collectorBalance -= collectValue;
            payable(_shareList[i].owner).transfer(collectValue);
        }
    }

    // Contract owner (artist) collect his/her shares
    ...
    function platformCollect() public {
        requireCollectable();
    ...
    function requireCollectable() view private {
        for (uint256 i = 0; i < _refundTimeList.length; i++) {
    ...
    function requireMintSign(address minter, uint256 price, uint256 deadline, bytes32 r, bytes32 s, uint8 v)  private view {
        bytes memory prefix = "\x19Ethereum Signed Message:\n32";
        bytes32 userHash = encodeMint(minter, price, deadline);
    ...
    function encodeMint( address minter, uint256 price, uint256 deadline) public pure returns (bytes32) {
        return keccak256(abi.encodePacked(minter, price, deadline));
    }
    ...
    function getCollectorTotalRatioPPM() private view returns (uint256) {
        uint256 ratioPPM = 0;
        for (uint256 i =0; i < _shareList.length; i++) {
            ratioPPM += _shareList[i].ratioPPM;
        }

        require(ratioPPM <= 10e6, "ManeBase: Collector ratio overflow");
    ...
    function getTokenIDsByHolder(address holder, uint256 limit, uint256 offset) public view returns (uint256[] memory) {
        uint256[] memory ret;
