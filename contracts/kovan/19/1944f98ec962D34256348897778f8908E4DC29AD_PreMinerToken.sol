...
contract PreMinerToken is ERC20PresetFixedSupply, AccessControl {
    ...
    constructor(uint256 _maxSupply) ERC20PresetFixedSupply("PreMiner (POS)", "pMINER", 0 * 1e18, msg.sender) {
    ...
    function allowOpenTrading() external onlyRole(DEFAULT_ADMIN_ROLE) returns (bool) {
        requireSellerApproval = false;
        return requireSellerApproval;
    }

    function disableMinting() external onlyRole(DEFAULT_ADMIN_ROLE) returns (bool) {
        allowMinting = false;
        return allowMinting;
    }
    ...
    function addApprovedSeller(address approvedSeller_) external onlyRole(DEFAULT_ADMIN_ROLE) returns (bool) {
    ...
    function removeApprovedSeller(address disapprovedSeller_) external onlyRole(DEFAULT_ADMIN_ROLE) returns (bool) {
    ...
    function mint(address recipient_, uint256 amount_) public virtual onlyRole(MINTER_ROLE) {
