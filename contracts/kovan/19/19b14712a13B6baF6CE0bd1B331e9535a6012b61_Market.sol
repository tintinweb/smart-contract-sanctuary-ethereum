    ...
    TransferableMarketBase
{
    ...
    function depositWithAutoBonding(
        uint amount,
        uint16 risk,
        uint16 term,
        bool autorenew,
        ...
        autoBonding(msg.sender, bondAmount);
        return deposit(amount, risk, term, autorenew);
    }

    function protectWithAutoBonding(
        uint amount,
        uint16 risk,
        uint16 term,
        bool autorenew,
    ...
    function depositWithPermitWithAutoBondingPermit(
        ...
        bytes memory permitBond
    ) external returns (uint id) {
        autoBondingWithPermit(msg.sender, bondAmount, permitBond);
    ...
    function protect(
        ...
        (uint bumpAmount, , uint reduceAmount) = bond.bondTakerPosition(
            msg.sender,
            _token,
            amount,
            risk,
    ...
    function deposit(
        ...
        address _token = address(token);
        // lock BUMP tokens using Bond contract
        (uint bumpAmount, , uint reduceAmount) = bond.bondMakerPosition(
            msg.sender,
            _token,
            amount,
            risk,
        ...
        IPositionNFT(config.getNFTMaker(address(token))).safeMint(
            msg.sender,
            id
        );
    ...
    function autoBonding(address bondTo, uint amount) internal {
        bump.safeTransferFrom(msg.sender, address(this), amount);
        bond.deposit(bondTo, amount);
    }

    function autoBondingWithPermit(
        address bondTo,
        uint amount,
        uint deadline,
        uint8 v,
        ...
        autoBonding(bondTo, amount);
    }

    function autoBondingWithPermit(
        address bondTo,
        uint amount,
        bytes memory permitEncoded
    ) internal {
        (uint deadline, uint8 v, bytes32 r, bytes32 s) = _decodePermit(
            permitEncoded
        );
        autoBondingWithPermit(bondTo, amount, deadline, v, r, s);
    }
...
interface IBond {
    ...
    function bondTakerPositionPermit(
        address recipient,
        address token, 
        uint amount, 
        uint16 risk, 
    ...
    function bondMakerPositionPermit(
        address recipient,
        address token, 
        uint amount, 
        uint16 risk, 
    ...
    function lock(address marketToken, address addr, uint amount) external;
    ...
    function transferLocked(address token, address from, address to, uint amount) external; 
}
