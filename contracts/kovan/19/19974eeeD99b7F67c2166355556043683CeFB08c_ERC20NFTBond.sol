...
contract ERC20NFTBond is NFTBond {
    ...
    external initializer {
        ...
        __AAVECreditDelegator_init(_poolAddressesProvider);
    }
    ...
    /**
     * @dev External function to buy a bond with credit delegation and returns the tokenId of the bond
     * when the contract is active
     * @param tokenUri string
     * @param beneficiary address
     * @param maturity uint256
     ...
     * @param nonce uint256
     * @param signature bytes
     * @param interestRateMode uint256
     */
    function buyBondWithAAVECreditDelegator(
        string calldata tokenUri,
        address beneficiary,
        uint256 maturity,
        uint256 principal,
    ...
    external whenNotPaused returns (uint256) {
        return super._buyBondWithAAVECreditDelegator(tokenUri, beneficiary, maturity, principal,  nftHash, setApprove, nonce, signature, interestRateMode);
    }
    ...
    /**
     * @dev External function to redeem a bond with credit delegation and returns the amount of the bond
     */
    function redeemBondWithAAVECreditDelegator(uint256 tokenId, uint256 interestRateMode) external returns (uint256) {
        return super._redeemBondWithAAVECreditDelegator(tokenId, interestRateMode);
    }
    ...
    function _principalTokenAddress() internal view override returns (address) {
...
import "../aave-credit-delegator/AAVECreditDelegator.sol";
 ...
 * @author Ethichub
 */
abstract contract NFTBond is NFT, CollateralizedBondGranter, LockedBondGranter, LiquidityRequester, AAVECreditDelegator, PausableUpgradeable {
    ...
    function _buyBondWithAAVECreditDelegator(
        string calldata tokenUri,
        address beneficiary,
        uint256 maturity,
        uint256 principal,
    ...
    internal returns (uint256) {
        _borrow(principal, interestRateMode, _principalTokenAddress());
        uint256 tokenId = _buyBond(tokenUri, beneficiary, maturity, principal, nftHash,setApprove, nonce, signature);
        return tokenId;
    }
    ...
    function _redeemBondWithAAVECreditDelegator(
        uint256 tokenId,
        uint256 interestRateMode
    )
    internal returns (uint256) {
        ...
        _repay(principal, interestRateMode, _principalTokenAddress());
        uint256 amount = _redeemBond(tokenId);
        return amount;
    }
    ...
    function _principalTokenAddress() internal virtual returns (address) {}
...
abstract contract AAVECreditDelegator is Initializable {
    ...
    function __AAVECreditDelegator_init(
      address _poolAddressesProvider
    )
    internal initializer {
