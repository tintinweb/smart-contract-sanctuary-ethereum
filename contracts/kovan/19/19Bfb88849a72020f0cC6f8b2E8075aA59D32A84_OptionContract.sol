...
// File contracts/OptionMakerV2.sol
...
contract OptionContract {
    ...
    // @dev USDC is an ERC20 token we created

    address public USDC = 0x62851D3F60C25c426A753b8Ce46930D7e9fBa9aC;
    ...
    mapping(address => uint[]) public listMappingOwner;
    ...
    function buyCallOption(uint amount, uint  currentPrice, uint strikePrice, uint timeDays) external payable returns (bool) {
        ...
        // @dev deposit amount user can later withdraw if contract is not in the money
        uint deposit;
        ...
        // @dev 
        // @dev this is where the function is currently failing
    ...
    function checkERC20Balance(address token) public view returns (uint) {
        uint balance;
    ...
    function buyAtStrike(uint ID) public payable returns (bool) {
        ...
        require(CallOptions[msg.sender][ID].filled != true, "Option order already filled");
        ...
        // @dev user buys ETH for specified strike price
        require(IERC20(USDC).transferFrom(sender, address(this), payment));

        // @dev smart contract transfers the ETH to user
        payable(msg.sender).transfer(amount);


        // @dev setting CallOptions filled = true 
        CallOptions[msg.sender][ID].filled = true;

        // @dev since we paid off potential debt, we subtract
        potentialDebtObligation -= amount;
    ...
    // withdraw deposit if Option Contract is not in the money
    function withdrawDeposit(uint ID) public returns (uint) {
        ...
        uint deposit;
        deposit = CallOptions[msg.sender][ID].deposit;

        // @dev sending USDC deposit to user (we still earned C from black scholes)
        IERC20(USDC).transferFrom(address(this), msg.sender, deposit);
    ...
    // #######################
    // @dev MATHEMATICAL FUNCTIONS


    // @dev Normal Distribution is always from 0 - 1
    // @dev in the future we would create a chainlink 
    ...
    // @dev Calculate optimal price of options contract using Black Scholes formula
    ...
    // calculate 1st part of Black Scholes 
    // @dev calculate d1
    ...
    function func_d1(uint currentPrice, uint strikePrice, uint expirationBlock) public view returns (uint) {
        ...
        // @dev we will be able to update risk in the future currently at 0.08
        uint riskTimesTime;
    ...
    // @dev calculate 2nd part of Black Scholes
    // @dev calculate d2
    // d2 = d1 - sd * sqrt(time)
    ...
    // ####################
    // @dev calculating Natural Logarithm
    ...
    // @dev Taylor series for exponential 
