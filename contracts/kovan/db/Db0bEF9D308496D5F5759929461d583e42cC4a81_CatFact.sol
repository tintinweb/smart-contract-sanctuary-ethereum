...
contract CatFact is ChainlinkClient, ConfirmedOwner {
   ...
   *
   * Please note that you SHOULD NOT CALL THIS METHOD DIRECTLY, but rather call
   * the transferAndCall() function on LINK's contract (see here the addresses
   * https://docs.chain.link/docs/link-token-contracts/)) with sufficient
   * tokens as noted above (currently 0.11 LINK).
   */
  function onTokenTransfer(
    ...
    require(_value >= (catFactFee + chainlinkFee), "Not enough LINK");

    //send back change if too much LINK was transferred
    uint256 change = _value - (catFactFee + chainlinkFee);
    if (change > 0) link.transfer(msg.sender, change);

    // check that the contract has sufficient LINK to make a request
    require(link.balanceOf(address(this)) >= (catFactFee + chainlinkFee), "No enough LINK to request a new cat fact");
