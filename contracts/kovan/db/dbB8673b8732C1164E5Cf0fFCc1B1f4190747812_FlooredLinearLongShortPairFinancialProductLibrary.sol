 ...
 * depending on where the settlement price lands within a price range between an upperBound and a lowerBound with
 ...
 * that will be sent to the long and short side. If the price is at or higher than the upperBound then
 * expiryPercentLong = 1. If the price is at or lower than the lowerBound then expiryPercentLong = floorPercentage. For
 * example, consider a linear LSP on the price of ETH collateralized in USDC with an upperBound = 4000,
 * lowerBound = 2000 and floorPercentage = 20% with a collateralPerPair of 1000 (i.e each pair of long and shorts is
 * worth 1000 USDC). At settlement the expiryPercentLong would equal 1 (each long worth 1000 and short worth 0) if ETH
 * price was > 4000 and it would equal 0.2 if < 2000 (each long is worth minimum 200 and each short is worth maximum
 * 800). If between the two (say 3500) then expiryPercentLong = (3500 - 2000) / (4000 - 2000) * (1 - 0.2) + 0.2 = 0.8.
 * Therefore each long is worth 800 and each short is worth 200.
 */
