...
contract StakingPool is ERC721
{
	...
	uint256 public positionNumber;                     // Position Id.

	Epoch[] public epochs;                             // Array of epochs.

	// position id => Position struct
	mapping (uint256 => Position) public positions;    // Records id of positions.

	event Staked(address indexed staker, uint256 positionId, uint256 value);
	...
	constructor(string memory name, string memory symbol, address _stakedToken, address _rewardToken, uint256 _rewardPerSecond) public ERC721(name, symbol)
	{
	...
	/// @return positionId - stakers' position id.
	function deposit(uint256 value) external returns (uint256 positionId)
	{
		stakedToken.transferFrom(msg.sender, address(this), value);             // transfers tokens from sender to this contract.
		_safeMint(msg.sender, positionNumber);                                  // Mints nft-staking-position with incremented id.
		...
		positions[positionNumber].startEpochId = len;                           // Records start epoch for position.
		positions[positionNumber].value = value;                                // Records new value for position.

		emit Staked(msg.sender, positionNumber, value);                         // Emits event.

		return positionNumber++;                                                // Returns position Id.
	}

	/// @notice Function to withdraw tokens.
	/// @param positionId - id of staker's position.
	function withdraw(uint256 positionId) external
	{
		require(ownerOf(positionId) == msg.sender, "not the owner");            // Requires to be owner of position.

		uint256 reward = computeReward(positionId, epochs.length - 1);          // Computes reward for this position.

		stakedToken.transfer(msg.sender, positions[positionId].value);          // Transfers staked tokens to msg.sender.
		rewardToken.transfer(msg.sender, reward);                               // Transfers reward tokens to msg.sender.

		uint256 value = positions[positionId].value;                            // Get value from position.
		...
		_burn(positionId);                                                      // Burns position.
	}

	/// @notice Function to claim reward from staking.
	/// @param positionId - id of staking position.
	/// @param who -- address of recipient.
	function claimReward(uint256 positionId, address who) external
	{
		require(ownerOf(positionId) == msg.sender, "not the owner");            // Requires to be owner of position.

		uint256 reward = computeReward(positionId, epochs.length - 1);          // Computes reward for position.
		rewardToken.transfer(who, reward);                                      // Transfers reward to recipient.
	}

	/// @notice Function to compute reward amount.
	/// @param positionId - id of staker position.
	/// @return - amount of reward.
	function computeReward(uint256 positionId, uint256 endEpochId) public view returns (uint256)
	{
		uint256 reward = 0;                                                     // Rewards starts from zero.
		uint256 value = positions[positionId].value;                            // Gets value from position.
		for (uint256 i = positions[positionId].startEpochId; i < endEpochId; i++)
		{
		...
		return reward;                                                          // Gets actual reward amount for position.
	}

	/// @notice Function to add liquidity to position.
	/// @param positionId - id of position.
	/// @param value - amount of tokens to add.
	function addLpToPosition(uint256 positionId, uint256 value) external
	{
		require(ownerOf(positionId) == msg.sender, "not the owner");            // Requires to be owner of position.
		...
		positions[positionId].value += value;                                   // Adds value to position.
	}

	/// @notice Function to withdraw liquidity from position.
	/// @param positionId - id of position.
	/// @param value - amount of tokens to withdraw.
	function decreaseLpFromPosition(uint256 positionId, uint256 value) external
	{
		require(ownerOf(positionId) == msg.sender, "not the owner");            // Requires to be owner of position.
		
		uint256 balance = positions[positionId].value;                          // Gets value amount from position.
		...
		uint256 len = epochs.length;                                            // Gets amount of epochs.
		uint256 newTvl = epochs[len - 1].tvl - value;                           // Gets new tvl.
		epochs.push(Epoch(block.timestamp, newTvl));                            // Pushes new date and new tvl to array.

		positions[positionId].value -= value;                                   // Withdraws value from position.
	}

	/// @notice Function to view lenght of epochs array.
	function numberOfEpochs() public view returns (uint)
	{
	...
	function viewReward(uint256 positionId, uint256 timestamp) external view returns (uint256)
	{
		uint256 endEpochId = epochs.length - 1;                                 // Gets id of last epoch.
		uint256 reward = computeReward(positionId, endEpochId);                 // Computes reward for this position and epoch.
		uint256 value = positions[positionId].value;                            // Gets amount of tokens staked.
