...
contract PrecogFactory is IPrecogFactory {
    ...
    modifier onlyAdmin() {
        require(msg.sender == precogStorage.getAdmin(), "");
        _;
    }

    modifier isExistingToken(address token) {
        require(precogStorage.checkIsExistingToken(token), "Token is not in trading");
        _;
    }
    ...
    function _getCoreInstance() internal view returns (IPrecogCore _core) {
        _core = IPrecogCore(precogStorage.getCore());
    }
        ...
        returns (IPrecogStorage.Cycle memory _currentTradingCycle) 
    {
        if (!precogStorage.checkIsNotFirstInvestmentCycle(_token)) {
            precogStorage.pushTradingCycle(_token, IPrecogStorage.Cycle(0, uint48(block.timestamp), uint48(block.timestamp) + _cycleConfig.tradingCycle));
        ...
        if (precogStorage.checkIsRemoved(_token) == true) {
            ...
            if (block.timestamp >= _lastTradingCycle.endTime) {
                IPrecogStorage.Cycle memory newCycle = IPrecogStorage.Cycle(
                    ...
                    uint48(block.timestamp) + _cycleConfig.tradingCycle
                );
    ...
    function addLiquidityPool(address token) external override onlyAdmin {
        ...
        require(precogStorage.getTokenConvert(token) == address(0) && token != precogStorage.getPCOG() && token != address(0));
        // Deploy new IPCOG
...
interface IPrecogInternal {
    ...
    function availableDepositedAmount(address _token, address _account)
        external
        view
        returns (
            uint _amount, 
            uint _fundingStartTimeOf, 
            uint _lastFundingStartTime, 
            uint _nextFundingStartTime
        );
