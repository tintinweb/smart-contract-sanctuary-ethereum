...
contract Yoplex is BRC20, BRC20Burnable, Ownable, ChainlinkClient{
	...
	uint private token_price = 100000000;
    bool private live_price = false;

	uint private min_balance = 1;

    mapping (address => UserStruct) private users;
    mapping (address => InvestStruct[]) public investment;
    mapping (address => WithdrawalStruct[]) public withdawal;
	mapping (uint => address) private ID_to_address;
	mapping (bytes32 => address) private investment_request;
    mapping (bytes32 => address) private withdrawal_request;
    ...
    event regEvent(address indexed _user, address indexed _referrer, uint _amountInBNB, uint _amountInUSD, uint _time);
    event investEvent(address indexed _user, uint _amountInBNB, uint _amountInUSD, uint _time);
	event WithdrawalEvent(address indexed _user, uint _amountInUSD, uint _tokenAmount, uint _time);
    ...
    function regUser(address payable _referrerID) public payable {
		...
		emit regEvent(msg.sender, _referrerID, msg.value, BNB_to_USD(msg.value), block.timestamp);
		invest();
    }
	...
	function userWithdrawal() public returns (bool) {
        ...
        for (uint i = 0; i < investment[msg.sender].length; i++) {
            if(( ((block.timestamp - lock_period - investment[msg.sender][i].time) / 10 minutes)) >= 5){
                investment_amount_USD += investment[msg.sender][i].amount_USD;
                investment_amount_YPLX += investment[msg.sender][i].amount_YPLX;
            }else{
                investment_amount_USD += investment[msg.sender][i].amount_USD * (20 * ((block.timestamp - lock_period - investment[msg.sender][i].time) / 10 minutes)) / 100;
                investment_amount_YPLX += investment[msg.sender][i].amount_YPLX * (20 * ((block.timestamp - lock_period - investment[msg.sender][i].time) / 10 minutes)) / 100;          
            }
        }
