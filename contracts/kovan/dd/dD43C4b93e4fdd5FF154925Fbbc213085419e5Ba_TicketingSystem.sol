...
contract TicketingSystem  is ERC721URIStorage{
    ...
    uint256 public projectLimit = 10000;
    ...
    mapping(address => uint) public blacklist;
     ...
     constructor() ERC721("CBQA Ticketing System", "CTS") {
        priceFeed = AggregatorV3Interface(0x9326BFA02ADD2366b30bacB125260Af641031331);
                contractOwner = msg.sender;
        categoryDetails[1] = Pass({
            id: 1,
            uri: "https://gateway.pinata.cloud/ipfs/QmU9za71hprey1o8rv89EdvYJoenivCG5eZjRARNoQyRXh",
            price: 5
        });
        categoryDetails[2] = Pass({
            id: 2,
            uri: "https://gateway.pinata.cloud/ipfs/QmUH4y55d2TmfyfF1heHDK8D95XckBrrhvEjPcyshR4Gn4",
            price: 10
        });
        categoryDetails[3] = Pass({
            id: 3,
            uri: "https://gateway.pinata.cloud/ipfs/QmZDNK6jmbgGZfvSeuPiLupZLDRj2B7VNeei2Fi7gSK1AR",
            price: 20
        });
    }
   ...
   function getTokenPrice(uint tokenId) public view returns (uint256) {
    ...
    function setPrice(int regularPrice, int platinumPrice, int vipPrice) external onlyOwner{
        categoryDetails[1].price = regularPrice;
        categoryDetails[2].price = platinumPrice;
        categoryDetails[3].price = vipPrice;
    }
...
function tokensOfOwner(address _owner) external view returns(uint256[] memory ownerTokens) {
    ...
    if (balance == 0) {
        ...
        for (passId = 1; passId <= total; passId++) {
            if (ownerOf(passId) == _owner) {
                result[resultIndex] = passId;
                resultIndex++;
            }
        }
        return result;
