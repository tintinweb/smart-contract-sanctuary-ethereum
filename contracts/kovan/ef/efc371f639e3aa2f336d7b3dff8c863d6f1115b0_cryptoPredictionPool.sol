...
contract cryptoPredictionPool is ReentrancyGuard {
    ...
    struct Rebase {
        ...
        mapping(address => bool) winnersStatus; 
        uint user_id;
        address winner;
    }
    ...
    constructor(address _pool_address,address _mega_pool) {
        owner = msg.sender;
    ...
    function setRebaseStatus() public returns (uint) {
        ...
        if(tempStatus_ > lastTotalSupply) {
            status_ = 1;
            isRebaseNegative[poolId] = false;
        } else if(tempStatus_ == lastTotalSupply) {
            status_ = 2;
        } else {
            status_ = 3;
            isRebaseNegative[poolId] = true;
        }
    ...
    function changeRebaseCaller(address _rebaseCaller) public {
        require(msg.sender == owner, "Not Owner");
        rebaseCaller = _rebaseCaller;
    }
    ...
    /* Function to be called by admin to update if the rebase is Negative */
    ...
    function registerforMegaPool() public {
        require(rebasePool[rebasePoolId].winnersStatus[msg.sender] == false, "Already registered");
        //IMegaPool(MegaPool).register();        
        // winners.push(msg.sender);
        // userId[num] = msg.sender;
        // num ++;
        // registered[msg.sender] = true;
        rebasePool[rebasePoolId].winnersStatus[msg.sender] = true;
        uint temp_ = rebasePool[rebasePoolId].user_id;
        rebasePool[rebasePoolId].winners[temp_] = msg.sender;
        rebasePool[rebasePoolId].user_id += 1;
    }


    /* A random winner among the registered users will be announced calling this function */

    function RollDice() public returns (address) {
        require(msg.sender == owner, "Not Owner");
        //IMegaPool(MegaPool).random(); 
        uint _id = uint(keccak256(abi.encodePacked(block.difficulty, block.timestamp)))%rebasePool[rebasePoolId].user_id;
        //WinnerAddress = userId[_id];
        rebasePool[rebasePoolId].winner = rebasePool[rebasePoolId].winners[_id];
        uint temp_ = IERC20(MegaPool).balanceOf(MegaPool);
        IERC20(MegaPool).transferFrom(MegaPool,rebasePool[rebasePoolId].winners[_id],temp_);
        return rebasePool[rebasePoolId].winners[_id];
    }

    // function claimFromMegaPool() public {
    ...
    function PlaceBet(uint256 index,uint256 _prices,uint256 _percent,uint256 _poolId,uint256 _amount) public returns(bool){
        ...
        //require(block.timestamp <= lastcreated + 15 minutes,"Stacking Locked!");
    ...
    function updatebal(address _user,uint256 _poolId,uint256 _reward,bool _isPositive) public returns(bool){
        ...
        if(_isPositive == true){
            ...
            bool status_ = isRebaseNegative[_poolId];
            if(!status_){
                uint temp_ = SafeMath.mul(us.freebal,12);
                uint add_ = SafeMath.add(us.freebal,temp_);
                us.freebal = SafeMath.div(us.freebal,1e2);
            } else {
                uint temp_ = SafeMath.mul(us.freebal,12);
                uint sub_ = SafeMath.sub(us.freebal,temp_);
                us.freebal = SafeMath.div(us.freebal,1e2);
            }
    ...
    /* create new rebase count after 30 days */
     ...
     function createRebase() public returns(bool) {
        ...
        if(rebasePool[rebasePoolId].numberOfNegativeDates >= 15) {
            uint balance_ = IERC20(pool_address).balanceOf(address(this));
            IERC20(pool_address).transfer(rebaseCaller,balance_);
        }
    ...
    /* Create a new pool after 1 days */
    
    function createPool(uint256[15] memory _prices) public returns(bool){
        require(msg.sender == owner,'Not Owner');
        require( block.timestamp > lastpoolcreated +  30 minutes,'Cannot Create');
        ...
        lastcreated = block.timestamp;
        b.endtime = SafeMath.add(block.timestamp,30 minutes);
    ...
    function fetchPool(uint256 _poolId) public view returns(uint256[15] memory _prices,uint256 _start,uint256 _end,uint256 _staking_ends){
        Pool storage b = pool[_poolId];
    ...
    function fetchUserBets(address _user, uint256 _poolId) public view returns(uint256[15] memory _pools,uint256[15] memory _prices,uint256[15] memory _amounts,uint256 balance,uint256 totalbet){
        UserBets storage u = bets[_user][_poolId];
    ...
    function fetchRebaseData(uint _rebaseId) public view returns (uint,uint,uint,uint,uint,address) {
        Rebase storage r = rebasePool[_rebaseId];
        return (r.NumberOfOccurances,r.numberOfNegativeDates,r.createdTime,r.endingTime,r.user_id,r.winner);
    }
