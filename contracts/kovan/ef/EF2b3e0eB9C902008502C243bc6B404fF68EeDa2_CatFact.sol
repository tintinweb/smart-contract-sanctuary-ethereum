...
contract CatFact is ChainlinkClient, ConfirmedOwner {
   ...
   *
   *This contract enables a caller to request a new cat fact from the https://catfact.ninja external API, with the help of the Chainlink client.
   *
   ...
   *
   *Enough explanations, how do I get a cat fact?
   *
   *- The caller must send a transaction to LINK's contract transferAndCall()
   ...
   *      _value: 0.11 LINK (can change over time, see more details below)
   *      _data: "" (can be left empty)
   *
   *- The LINK contract will then transfer the LINK to this contract
  ...
  constructor() ConfirmedOwner(msg.sender) {
    ...
    // 10% of the Chainlink fee i.e aiming for a caller to pay 0.11 LINK, where 0.1 goes to Chainlink for the API call and 0.1 LINK remains in profit for this contract
    catFactFee = (1 * LINK_DIVISIBILITY) / 100; // 0,01 * 10**18 (Varies by network and job)
  }

  /**
   * Callback function from chainlink oracle network, updates the state of
  ...
  /**
   * Provide cat fact from the contract's memory storage
   *
   */
   ...
   * contract from LINK's contract via the transferAndCall() function.
   *
   * Please note that this function is meant to be called ONLY BY THE LINK
   * contract (it will revert otherwise). Rather call the transferAndCall()
   * function on LINK's contract (see here the addresses
   * https://docs.chain.link/docs/link-token-contracts/)) with sufficient
   * tokens as noted above (currently min. 0.11 LINK).
   */
  function onTokenTransfer(
    ...
    // There is no good way I can think of to limit the LINK that can be sent to this contract. So if the caller sends more than 0.11, they still get just 1 API call => more profit for the contract but not great for the caller.
    requestCatFact();
  }
   ...
   *
   * Function is set to private as the contract requires payment to retrieve
   * the data
   */
