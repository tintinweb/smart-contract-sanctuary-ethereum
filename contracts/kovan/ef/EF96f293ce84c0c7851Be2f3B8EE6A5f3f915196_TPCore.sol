...
contract TPCore is AccessControl {
    ...
    function estimateRewardDetails(address lpProvider) public view returns (uint104 gross, uint104 net, uint104 profit, uint104 fee) {
        ...
        uint104 userStakingAmount = userStaking[lpProvider].amount + uint104(userStaking[lpProvider].pendingAmount);
    ...
    function estimateTPYReward(address addr) public view returns(uint136) {
        ...
        if(lastCompleteInterval + 1 >= staking.intervalNumber) {
            ...
            uint136 TotalstakingAmount;
            if(staking.pendingAmount > 0 && staking.intervalNumber < lastCompleteInterval) {
                TotalstakingAmount = staking.amount + staking.pendingAmount;
            }
            else {
                TotalstakingAmount = staking.amount;
            }
            for(uint256 i = staking.intervalNumber + 1; i <= lastCompleteInterval; i++) {
                ...
                estimatedTPYs += TotalstakingAmount * getTPYIntervalReward(i) / stakedUntilInterval;
            }
        }
        ...
        if(userStaking[staker].intervalNumber > 0) {// exists
            ...
            if(staking.intervalNumber >= currentIntervalNumber){
                ...
                if(staking.pendingAmount > 0) {
                    staking.amount += uint104(staking.pendingAmount);
                    staking.pendingAmount = 0;
                    staking.intervalNumber++;
                }
    ...
    function takeTPYReward(uint104 unstakeCoinAmount) private {
        ...
        if(unstakeCoinAmount > staking.amount + staking.pendingAmount) {
            unstakeCoinAmount = staking.amount + uint104(staking.pendingAmount);
        }
        ...
        if(unstakeCoinAmount  <= staking.pendingAmount) {
            ...
            staking.amount -= unstakeCoinAmount - uint104(staking.pendingAmount);
            staking.pendingAmount = 0;
        }
