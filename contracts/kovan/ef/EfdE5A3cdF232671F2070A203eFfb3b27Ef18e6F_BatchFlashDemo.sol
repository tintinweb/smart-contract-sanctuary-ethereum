...
contract BatchFlashDemo is FlashLoanReceiverBase, Ownable {
    ...
    */
    ...
    function swapExactInputSingle(uint256 amountIn,  address[] calldata addresses) external returns (uint256 amountOut) {
        ...
        // TransferHelper.safeApprove(kovanLINK, address(swapRouter), amountIn);
        // TransferHelper.safeApprove(kovanWETH, address(swapRouter), amountIn);
        // TransferHelper.safeApprove(kovanCOMP, address(swapRouter), amountIn);


        for( uint i=0; i+1 < addresses.length; i++){
            TransferHelper.safeApprove(addresses[i], address(swapRouter), amountIn);
             // Naively set amountOutMinimum to 0. In production, use an oracle or other data source to choose a safer value for amountOutMinimum.
            // We also set the sqrtPriceLimitx96 to be 0 to ensure we swap our exact input amount.
            
            address tokenIn = addresses[i];
            address tokenOut = addresses[i+1];
