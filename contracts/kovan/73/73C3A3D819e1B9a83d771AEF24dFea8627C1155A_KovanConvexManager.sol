...
contract Booster{
    ...
    function addPool(address _lpToken, uint256 _pid) external returns(address){
        ...
        address token = IConvexTokenFactory(tokenFactory).deployPoolToken(name, symbol, 18);
...
import {BaseRewardPool} from "./ConvexBaseRewardPoolK.sol";
import {VirtualBalanceRewardPool} from "./ConvexVirtualBalanceRewardPoolK.sol";
...
contract ConvexPoolFactory is IConvexPoolFactory {
    ...
    function deployBasePool(
      ...
      return address(new BaseRewardPool(_pid, _stakingToken, _rewardToken, _operator, _manager));
    }
    ...
    function deployExtraPool(
      ...
      return address(new VirtualBalanceRewardPool(_basePool, _rewardToken, _operator, _manager));
    }
}
...
contract ConvexTokenFactory is IConvexTokenFactory {
    ...
    function deployPoolToken(
      ...
      address token = address(new ERC20Kovan(name, symbol, decimals));
