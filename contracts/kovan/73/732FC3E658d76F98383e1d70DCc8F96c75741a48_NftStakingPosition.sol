...
contract NftStakingPosition is ERC721, ListingList
{
	struct Nft
	{
		address token;
	...
	function stakeNft(address token, uint256 id) external isAllowedToStake(token)
	{
		IERC721(token).transferFrom(msg.sender, address(this), id);                // Sends NFT token to this contract.

		uint256 index = nftBattleArena.createStakerPosition(msg.sender);
...
contract ListingList is Ownable {
	...
	function allowNewContractForStaking(address token) external onlyOwner
	{
		...
		emit NewContractAllowed(token);                              // Emits event that new contract are allowed.
	}

	modifier isAllowedToStake(address token)
	{
		require(allowedForStaking[token], "NFT collection is not allowed"); // Requires for nft-token to be from allowed contract.
		_;
	}
}
...
contract NftBattleArena is Ownable
{
	...
	function liquidateVotingPosition(uint256 votingPositionId, address voter, address beneficiary, uint256 lastEpoch, uint256 stakingPositionId, uint256 daiNumber) internal
	{
		...
		emit LiquidatedVotingPosition(currentEpoch, voter, stakingPositionId, beneficiary, votingPositionId, daiNumber, zooReturned);
	}
	...
	function withdrawDaiFromVoting(uint256 votingPositionId, address voter, uint256 daiNumber, address beneficiary) external only(nftVotingPosition)
	{
		...
		{
			liquidateVotingPosition(votingPositionId, voter, beneficiary, lastEpoch, stakingPositionId, daiNumber);
			return;
		}
	...
	function chooseWinnerInPair(uint256 pairIndex) external
	{
		...
		if(battleRandom == 0)
		{
			battleRandom =  uint256(keccak256(abi.encodePacked(blockhash(block.number - 1))));
		}
	...
	function calculateWithdrawForZoo(uint256 zooAmount, address beneficiary) internal
	{
