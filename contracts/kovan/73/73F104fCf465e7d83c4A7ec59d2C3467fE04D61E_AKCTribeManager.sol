...
contract AKCTribeManager is Ownable, ReentrancyGuard {
    ...
    uint256 public minimumEnforceBalance = 10 ether;

    uint256 public enforceCounter;
    uint256 public enforceInterval = 20;

    /**
     * @dev ENFORCE VARS
     */
    uint256 public reserveReserved;
    ...
    /// @dev Core Events
    event PurchaseTribeEvent(address indexed purchaser, uint256 indexed spec, uint256 indexed identifier);
    ...
    event UnstakeAKC(address indexed receiver, uint256 indexed akc, uint256 indexed spec);

    /// @dev Operations Events
    event SwapAKCForEth(uint256 indexed akcAmount);
    event LiquidityAdded(uint256 indexed akcCoin, uint256 indexed eth);
    event SendToReserveEvent(uint256 indexed amount);
    ...
    constructor(
        ...
        akcCoin.approve(address(router), MAX_INT);

        akcNFT = IERC721(_akc);        
    }

    /** DEX HELPER METHODS */
    function swapTokensForEthSupportingFeeOnTransferTokens(uint256 tokenAmount) internal {
        // generate the pancake pair path of token -> weth
        address[] memory path = new address[](2);
        path[0] = address(akcCoin);
        path[1] = router.WETH();

        if (akcCoin.allowance(address(this), address(router)) < tokenAmount) {
            akcCoin.approve(address(router), MAX_INT);
        }
        ...
        emit SwapAKCForEth(tokenAmount);
    }
    ...
    /** RESERVE MANAGEMENT */
    function sendToLiquidity() public onlyOwner {
        require(liquidityReserved > 0, "NO AKC RESERVED FOR LIQUIDITY");

        uint256 toEth = liquidityReserved / 2;
        uint256 toAKC = liquidityReserved - toEth;
    ...
    function sendToDistributor() public onlyOwner {
        payable(akcDistributor).transfer(address(this).balance);
    }

    function sendToReserve() public onlyOwner {
        require(reserveReserved > 0, "NO RESERVE FOR RESERVE");
        akcCoin.transfer(address(akcReserve), reserveReserved);

        emit SendToReserveEvent(reserveReserved);
        reserveReserved = 0;        
    }

    function liquifyAKC() public onlyOwner {
        uint256 contractBalance = akcCoin.balanceOf(address(this));
        uint256 toSubstract = liquidityReserved + reserveReserved;
        uint256 usableBalance = contractBalance >= toSubstract ? contractBalance - toSubstract : contractBalance;
        bool minimumReached = usableBalance >= minimumEnforceBalance;

        if (enforceTerms && minimumReached && msg.sender != owner()) {
            uint256 toReserve = usableBalance * reserveAllocation / 100;
            uint256 toLiquidity = usableBalance * liquidityAllocation / 100;
            
            reserveReserved += toReserve;
            liquidityReserved += toLiquidity;

            uint256 toEth = usableBalance - toReserve - toLiquidity;
            swapTokensForEth(toEth);
        }
    ...
    /** PURCHASING TRIBES */
    function applyDiscount(uint256 price, uint256 tribeAmount) internal pure returns(uint256) {
        uint256 discount;
        if (tribeAmount == 2) {
            discount = 2;
        } else if (tribeAmount == 3) {
            discount = 3;
        }
    ...
    function purchaseTribe(uint256 spec, uint256 identifier)
        external
        nonReentrant { 
            require(spec < akcCore.getTribeSpecAmount(), "SPEC OUT OF BOUNDS");

            (uint256 price,,) = akcCore.tribeSpecs(spec);
            uint256 tribeAmount = akcCore.getTribeAmount(msg.sender);

            price = applyDiscount(price, tribeAmount);
            ...
            if (enforceCounter % enforceInterval == 0) {
                uint256 contractBalance = akcCoin.balanceOf(address(this));
                uint256 toSubstract = liquidityReserved + reserveReserved;
                uint256 usableBalance = contractBalance >= toSubstract ? contractBalance - toSubstract : contractBalance;
                bool minimumReached = usableBalance >= minimumEnforceBalance;
    ...
    /*function claimRewardOfTribe(uint256 tribeIndex)
        ...
        nonReentrant {
            ...
            if (enforceTerms && distributorAllocation > 0 && distributorAllocation < 100 && usableBalance >= toDistributor) {
    ...
    /** STAKING */
    function safeStakeAKC(uint256 akcId, uint256 spec)
        external
        nonReentrant {
            require(akcNFT.ownerOf(akcId) == msg.sender, "SENDER NOT OWNER OF AKC");
            require(akcNFT.getApproved(akcId) == address(this), "MANAGER NOT APPROVED");
            
            akcNFT.safeTransferFrom(msg.sender, address(akcCore), akcId);
            akcCore.stakeAKC(msg.sender, akcId, spec);
        }
    
    function safeUnstakeAKC(uint256 akcId, uint256 spec)
        external
        nonReentrant {
            require(akcNFT.ownerOf(akcId) == address(akcCore), "AKC CORE NOT OWNER OF NFT");
            require(akcNFT.isApprovedForAll(address(akcCore), address(this)), "MANAGER NOT APPROVED FOR CORE");
            
            akcNFT.safeTransferFrom(address(akcCore), msg.sender, akcId);
            akcCore.unstakeAKC(msg.sender, akcId, spec);
        }

    function stakeAKC(uint256 akcId, uint256 spec)
        external
        nonReentrant {
            require(akcNFT.ownerOf(akcId) == msg.sender, "SENDER NOT OWNER OF AKC");
            require(akcNFT.getApproved(akcId) == address(this), "MANAGER NOT APPROVED");
            
            akcNFT.transferFrom(msg.sender, address(akcCore), akcId);
            akcCore.stakeAKC(msg.sender, akcId, spec);
        }
    
    function unstakeAKC(uint256 akcId, uint256 spec)
        external
        nonReentrant {
            require(akcNFT.ownerOf(akcId) == address(akcCore), "AKC CORE NOT OWNER OF NFT");
            require(akcNFT.isApprovedForAll(address(akcCore), address(this)), "MANAGER NOT APPROVED FOR CORE");
            
            akcNFT.transferFrom(address(akcCore), msg.sender, akcId);
            akcCore.unstakeAKC(msg.sender, akcId, spec);
        }
    ...
    function setLiquidityReserved(uint256 liqReserved) external onlyOwner {
        liquidityReserved = liqReserved;
    }

    function setEnforceInterval(uint256 newInterval) external onlyOwner {
        enforceInterval = newInterval;
    }
...
contract IAKCCore {
    ...
    /** CREATING */
    function createSingleTribe(address newOwner, uint256 spec, uint256 asciiData) 
        external {}
