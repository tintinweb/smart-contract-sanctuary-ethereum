...
contract CopperProxy is Ownable {
    ...
    function transferPoolOwnership(address pool, address newOwner) external onlyPoolOwner(pool) {
        require(blacklistedAddresses[newOwner] != true, "newOwner address is blocked!");
    ...
    function safeFeeDistribution(address pool, address fundToken) internal {
        ...
        if (fundTokenBalance > uint256(0)) {
            ...
            emit TransferedFee(pool, fundToken, feeRecipients[0], feeAmount);
        }
    }
    ...
    function distributeFee(address pool, address fundToken) internal {
        ...
        if (fundTokenBalance > uint256(0)) {
            ...
            for (uint256 i = 0; i < feeRecipients.length; i++) {
                ...
                emit TransferedFee(pool, fundToken, feeRecipients[i], proportionalAmount);
            }
        }
    }
    ...
    function verifyWithBlacklist(address _addressToVerify) public view returns(bool) {
        bool userIsBlacklisted = blacklistedAddresses[_addressToVerify] == true;
        return userIsBlacklisted;
    }
    modifier isNotBlacklisted(address _address) {
        require(blacklistedAddresses[_address] != true, "Address is blocked!");
        _;
    }
