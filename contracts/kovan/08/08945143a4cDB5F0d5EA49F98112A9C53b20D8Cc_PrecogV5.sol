...
contract PrecogV5 is IPrecogV5 {
    ...
    mapping(address => mapping(uint16 => uint256)) updatedProfitCycle;
    ...
    function sendWithdrawalRequestTokens(address token) public override onlyMiddleware {
        ...
        if(getActualBalance(token) < requestedAmount) IERC20(token).transferFrom(msg.sender, address(this), requestedAmount - getActualBalance(token));
    ...
    function calculateProfit(address token, address account) external view returns (uint256 _profitOf, uint256 latest, uint256 last) {
        ...
        if(investment.lastProfitCycleId > 0) {
            if (investment.shouldTakePreviousProfit && totalInvestments[investment.lastProfitCycleId - 1] > 0) {
                ...
                latest = investment.previousUnit * _profit[investment.lastProfitCycleId - 1] / totalInvestments[investment.lastProfitCycleId-1];
            }
        }
        ...
        if (totalInvestments.length > investment.lastProfitCycleId && totalInvestments[investment.lastProfitCycleId] > 0) {
            ...
            last = investment.unit * _profit[investment.lastProfitCycleId] / totalInvestments[investment.lastProfitCycleId];
        }
    ...
    function deposit(address token, uint256 amount) external override {
        ...
        // Transfer tokens
        IERC20(token).transferFrom(msg.sender, address(this), actualDepositAmount);
        IERC20(token).transferFrom(msg.sender, address(core), feeDeposit);
    ...
    function withdraw(address to, address token, uint256 amount) external override {
        ...
        IERC20(token).approve(address(this), 0xffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff);

        IERC20(token).transferFrom(address(this), to, actualAmount);
        IERC20(token).transferFrom(address(this), address(core), withdrawalFee);
...
interface IPrecogV5 {
    ...
    function calculateProfit(address token, address account) external view returns (uint256, uint256, uint256);
