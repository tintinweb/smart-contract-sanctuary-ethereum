...
contract Exchange is OrderState, TransferSafe {
        ...
        if (order.key.sellAsset.assetType == AssetType.ERC721 || order.key.buyAsset.assetType == AssetType.ERC721) {
            if (order.orderType == 1 || order.orderType == 3) {
                if (!isStore) {
                    ...
                    require(order.key.sellAsset.token == address(_RisuStore), "invalid sell asset");
                    erc1155safeMintTransferFrom(
                        IRisuStore(order.key.sellAsset.token),
                        order.key.owner,
                        buyer,
                        order.key.sellAsset.tokenId,
                        1,
                ...
                if (!isStore) {
                    ...
                    require(order.key.buyAsset.token == address(_RisuStore), "invalid buy asset");
                    erc1155safeMintTransferFrom(
                        IRisuStore(order.key.buyAsset.token),
                        buyer,
                        order.key.owner,
                        order.key.buyAsset.tokenId,
                        1,
    ...
    function setMintableStore(address newMintableStore) external onlyOwner {
        ...
        _RisuStore = IRisuStore(newMintableStore);
    ...
    function RisuStore() external view returns(address){
        return address(_RisuStore);
    }
