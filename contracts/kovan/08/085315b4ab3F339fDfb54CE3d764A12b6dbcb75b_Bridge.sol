...
contract Bridge is IBridge, Ownable {
    ...
    function getValidatorsWithPower() external view returns(ValidatorWithPower[] memory) {
        ValidatorWithPower[] memory tmp = new ValidatorWithPower[](validatorsCount);
        for (uint256 idx = 0; idx < validatorsCount; ++idx) {
            tmp[idx] = validatorPowers[idx];
        }
        return tmp;
    }
    ...
    function relayBlock(
        ...
        for ((uint256 idx, uint256 jdx) = (0, 0); idx < signers.length && jdx < validatorsCount;) {
            ...
            if (signer == validatorAddress) {
                sumVotingPower += validatorPowers[idx].power;
                idx++;
                jdx++;
