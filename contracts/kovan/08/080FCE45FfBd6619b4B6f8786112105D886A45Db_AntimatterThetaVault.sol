...
import {VaultLifecycleAntimatter as VaultLifecycle} from "../../libraries/VaultLifecycleAntimatter.sol";
import {ShareMath} from "../../libraries/ShareMath.sol";
import {RibbonVault} from "./base/RibbonVault.sol";
import {IAntimatterSwap} from "../../interfaces/IAntimatterSwap.sol";
 ...
 */
contract AntimatterThetaVault is RibbonVault, RibbonThetaVaultStorage {
     ...
     */
    function commitAndClose(uint256 strikePrice) external nonReentrant {
    ...
    function newProduct(
        ...
        require(products[productId] == address(0), "Product exists");
        require(premiumPrice > 0, "!premiumPrice");

        nextProductId = productId;
        products[nextProductId] = optionState.currentOption;

        ShareMath.assertUint104(premiumPrice);
        currentOtokenPremium = uint104(premiumPrice);

        emit NewProduct(nextProductId, products[nextProductId], premiumPrice);
    }
    ...
    function getOtokenMintInfo() public view returns (
        uint256 lockedBalance,
        uint256 totalVaultFee,
        uint256 mintAmount
    ) {
        require(optionState.nextOption != address(0), "!optionState.nextOption");
        address oTokenAddress = optionState.nextOption;
        ...
        uint256 depositAmount = lockedBalance;

        // An otoken's collateralAsset is the vault's `asset`
        // So in the context of performing Opyn short operations we call them collateralAsset
        IOtoken oToken = IOtoken(oTokenAddress);
        address collateralAsset = oToken.collateralAsset();
        uint256 collateralDecimals = uint256(IERC20Detailed(collateralAsset).decimals());
        if (oToken.isPut()) {
        ...
        return (lockedBalance, totalVaultFee, mintAmount);
    }
