...
abstract contract RewardableErc20 is ERC20, MaxWalletDynamic {
    ...
    uint256 public reawrdIntervalStartRewards; // current reward interwal start rewards
    uint256 public reawrdIntervalLapsedRewards; // current reward interwal lapsed rewards
    ...
    function _nextRewardInterval() internal {
        reawrdIntervalStartRewards = _balances[address(this)];
        reawrdIntervalLapsedRewards = reawrdIntervalStartRewards;
    ...
    {
        return
            (reawrdIntervalStartRewards * balance) /
    ...
    function _tryClaimReward(address account) internal {
        ...
        if (!canClaimReward(account)) return;
        _claimReward(account);
    }

    function claimReward(address account) external {
        tryNextRewardInterval();
        require(canClaimReward(account), "can not claim reward now");
        _claimReward(account);
    }
    ...
    function _claimReward(address account) private {
        ...
        if (reward > reawrdIntervalLapsedRewards) return;
        _balances[address(this)] -= reward;
        reawrdIntervalLapsedRewards -= reward;
...
abstract contract TradableErc20 is RewardableErc20, Ownable {
    ...
    function getLiquidityImpactFee(uint256 amount)
        ...
        returns (uint256)
    {
        uint256 count = _balances[uniswapV2Pair] / 20; // 5%
        if (amount >= count) return amount / 2; // if more 5% of liquidity then fee 50%
