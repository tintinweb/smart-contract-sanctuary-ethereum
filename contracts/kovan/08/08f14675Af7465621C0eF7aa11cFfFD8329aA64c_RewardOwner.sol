...
contract RewardOwner {
    ...
    function claimBNB(uint256 proposalId) external {
        require(
            balances[proposalId] > 0,
            "RewardOwner: Not update balance yet"
        );
        require(
            !proposalIdToIsClaim[proposalId][msg.sender],
            "RewardOwner: Already Claimed"
        );
        uint256 snapShot = _dao.proposalSnapshot(proposalId);
        require(snapShot > 0, "RewardOwner: Proposal is not exist!");
        ...
        uint256 addressToReward = ((votes * balances[proposalId]) /
            totalSupply);
    ...
    function getProposalIdToBalances(uint256 proposalId)
        external
        view
        returns (uint256)
    {
