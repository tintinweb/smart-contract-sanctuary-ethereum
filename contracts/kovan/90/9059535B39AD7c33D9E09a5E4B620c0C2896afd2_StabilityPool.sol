...
contract StabilityPool is IStabilityPool, Ownable {
    ...
    function withdrawFromStabilityPool(uint _amount) external override {
        ...
        _sendUSDCToDepositor(externalStakingAddress, msg.sender, USDCAmount);
    ...
    function _sendUSDCToDepositor(address _from, address _depositor, uint USDCWithdrawal) internal {
        if (USDCWithdrawal == 0) {return;}

        usdc_token.transferFrom(_from, _depositor, USDCWithdrawal);
    }
