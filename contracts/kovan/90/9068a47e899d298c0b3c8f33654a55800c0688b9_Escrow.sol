...
contract Escrow is Ownable {
    ...
    */
    function initialize(
        address blinkToken, //blink token address
    ...
    */
    function addFounder(address[] calldata accounts, uint[] calldata allocations) external onlyOwner {
        ...
        for(uint i=0;i<accounts.length;i++) {
            ...
            foundersInfo.totalBlinks += allocations[i];
        }
    }
    ...
    function removeFounder(address[] calldata accounts) external onlyOwner {
        ...
        for(uint i=0;i<accounts.length;i++) {
            if(founderInfo[accounts[i]].isActive) {
                ...
                foundersInfo.totalBlinks -= founderInfo[accounts[i]].allocated;
                founderInfo[accounts[i]].allocated = 0;
                //remove founder
                address last = founders[founders.length];
    ...
    */
    function addWallets(address[] calldata accounts, uint[] calldata allocations) external onlyOwner {
        ...
        for(uint i=0;i<accounts.length;i++) {
            ...
            walletsInfo.totalBlinks += allocations[i];
        }
    }
    ...
    function removeWallet(address[] calldata accounts) external onlyOwner {
        ...
        for(uint i=0;i<accounts.length;i++) {
            if(walletInfo[accounts[i]].isActive) {
                ...
                walletsInfo.totalBlinks -= walletInfo[accounts[i]].allocated;
    ...
    */
    function pendingClaim(address account) public view returns (uint claimable, uint lastClaim) {
        ...
        uint blockRewards = ((founder.allocated * foundersInfo.issuance) / 10000) * pendingBlocks;
    ...
    */
    function _founder(address account) private {
        ...
        if(claimable > 0){
            ...
            _sendBlink(account, claimable,"founders");
        }
    }
    ...
    */
    function _launchPad(address account) private {
        ...
        require(launchpadInfo.totalBlinks > 0,"Escrow:: _launchPad: has claimed all issuance");
        
        uint amount = launchpadInfo.totalBlinks;
        launchpadInfo.totalBlinks = 0;
        _sendBlink(account, amount,"launchpad");
    }
    ...
    */
    function _wallets(address account) private {
        ...
        info.allocated = 0;
        _sendBlink(account, amount,"wallets");
    }

    /**
    * @dev send tokens to founder and launch pad.
    */
    function _sendBlink(address account, uint amount, string memory claimType) private {
