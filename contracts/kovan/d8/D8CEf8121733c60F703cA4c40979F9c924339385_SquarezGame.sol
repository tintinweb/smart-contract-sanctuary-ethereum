...
contract SquarezGame is Ownable, SquarezNFT, RandomGameBoard, SportsDataAPIConsumer, AdAuction {
    ...
    mapping(uint256 => uint256) public payoutPercentagePerQuarter;

    // constructor(address _tokenContract)
    constructor()
    SportsDataAPIConsumer(0xc57B33452b4F7BB189bB5AfaE9cc4aBa1f7a4FD8)
    AdAuction("Atlas Corp",0x5B38Da6a701c568545dCfcB03FcB875f56beddC4)
    {
        // 
        payoutPercentagePerQuarter[1] = 25;
        payoutPercentagePerQuarter[2] = 25;
        payoutPercentagePerQuarter[3] = 25;
        payoutPercentagePerQuarter[4] = 25;
    }

    function createGameBoard() public override(RandomGameBoard) {
    ...
    function simulateFulfillRandomness(uint256 randomness) public onlyOwner {
    ...
    function getGameScores() public onlyOwner {
    ...
    function claimPrize(uint256 _quarter) public {
        ...
        // add payment from smart contract balance
    ...
    function setBaseURI(string memory _URI) external override(SquarezNFT) onlyOwner {
        baseURI = _URI;
    }

    function startSale() external override(SquarezNFT) onlyOwner {
        require(saleState == SaleState.Off, "Sale already started and/or completed");
...
contract SquarezNFT is ERC721, ERC721Enumerable  {
    ...
    function stopSale() private {
        saleState = SaleState.SoldOut;
    }

    function setBaseURI(string memory _URI) external virtual {
        baseURI = _URI;
    }
    ...
    function getTileOwners(uint256 _lower, uint256 _upper) public view returns (address[] memory) {
        require(_upper <= 100, "Upper cannot exceed 100");
        ...
        for(uint256 i = 1; i <= _upper - _lower; i++ ){
            if(tilePurchased[i]){
                tileOwners[i] = ownerOf(i);
            }
            else{
...
contract RandomGameBoard is VRFConsumerBase {
    ...
    function createGameBoard() public virtual {
        require(LINK.balanceOf(address(this)) >= VRFfee, "Not enough LINK - fill contract with faucet");
        VRFRequestId = requestRandomness(keyHash, VRFfee);
    }
    ...
    function fulfillRandomness(bytes32 requestId, uint256 randomness) internal override {
        require(requestId == VRFRequestId, "Request ID does not match");
        require(gameState == GameState.PreReveal,"Game must be in initial state");
        gameState = GameState.GettingVRF;
        assignRandomness(randomness);
    }
    ...
    function assignGameBoard() private {
...
contract AdAuction {
    ...
    constructor(string memory _initialDiamondSponsorAd, address _initialDiamondSponsor){
        ...
        lastAmountPaid = 0;
        minimumAirtimeInBlocks = 10;

        platinumSponsorshipRate = 5 ether;
        goldSponsorshipRate = 2 ether;
        silverSponsorshipRate = 1 ether;
    }

    function diamondSponsorship(string calldata _ad) public payable {
        require(bytes(_ad).length<=20,"Ad cannot be greater than 20 characters");
    ...
    function stopAdSale() public virtual {
        require(adSaleActive, "Ad sale already stopped");
        adSaleActive = false;
    }
