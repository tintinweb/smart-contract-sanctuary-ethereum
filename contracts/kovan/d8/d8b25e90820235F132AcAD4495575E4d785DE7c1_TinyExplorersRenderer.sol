...
// based on the solid concepts + foundations set by the greatest: Anonymice, Blitmap, Chainrunners, fiveoutofnine
...
contract TinyExplorersRenderer is Ownable, ReentrancyGuard {
    ...
    */
    ...
    function getTokenData(TinyExplorersTypes.TinyExplorer memory explorerData) internal view returns (Layer [NUM_LAYERS] memory tokenLayers, Color [NUM_COLORS][NUM_LAYERS] memory tokenPalettes, uint8 numTokenLayers, string [NUM_LAYERS] memory traitTypes, string memory backColor) {
        ...
        bool hasGoggles = dna[5] < ITEMS[5][2];
        bool hasHat = dna[6] < ITEMS[6][2]; 
        ...
        for (uint8 i; i < NUM_LAYERS; i ++) {
            ...
            if (layer.hexString.length > 0) {
                ...
                if ( (( i == 6 ||  i == 5) && isPirateAndHasKingdom) || (i == 3 && hasGoggles && !hasMask) || (i == 7 && !hasMask ) || (i == 5 && !hasMask) || (i == 2 && !hasHat) || (i < 2 || i == 4 || i == 6) ) {
