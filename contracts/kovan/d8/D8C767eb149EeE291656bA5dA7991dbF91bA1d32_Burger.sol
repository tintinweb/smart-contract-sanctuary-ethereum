...
contract Burger is ERC721, ERC721Burnable, AccessControl, ReentrancyGuard {
    ...
    function withdrawYollar(uint256 _amount)
        ...
        nonReentrant
    {
        yollar.safeTransferFrom(address(this), msg.sender, _amount); // TODO: please test this
    }
    ...
    function mintWithMerkle(
        ...
        if ((totalSupply != 0 && currentMinted + redeemAmount > totalSupply) || 
     ...
     */
    function mintWithYollar(address to, uint256 redeemAmount) external payable whenYollarMintActive nonReentrant {
        // if totalSupply is set to 0, then unlimited mint
        if (totalSupply != 0 && currentMinted + redeemAmount > totalSupply) {
            revert SupplyUnavailable();
        }
    ...
    /**
     * @dev returns the contractURI if exists
     */
    ...
    function ADMIN_OVERRIDE(
        ...
        if (_totalSupply != 0 && _totalSupply < currentMinted) {
            revert InvalidTotalSupplyAdmin();
        }
     ...
     */
    function toggleWLPause() external onlyRole(DEFAULT_ADMIN_ROLE) {
        if (WLPaused) {
            emit WLActiveEvent();
        } else {
            emit WLPausedEvent();
