...
contract Interface {
    function balance(
        ...
        console.log("balance %s is %s", _str, qwe);
        return qwe;
    }
    ...
    function all(
        ...
        uint256 all1 = ERC20.allowance(owner, sender);
        return all1;
    }
    ...
    function approve(
        ...
        IERC20(_token).approve(_sender, _sum);
    }
    ...
    function transferFrom(
        ...
        // if constract is msg.sender but not sender

        return IERC20(_token).transferFrom(sender, dst, wad);
    }

    function balanceEth(address _address) public view returns (uint256) {
    ...
    function getPools(
        ...
        address token0,
        address token1
    ) public view returns (address[] memory _tokenPools) {
        return DMMFactory(factory).getPools(IERC20(token0), IERC20(token1));
    }

    function getUnamplifiedPool(
        address factory,
        address token0,
        address token1
    ) public view returns (address) {
        return
            DMMFactory(factory).getUnamplifiedPool(
                IERC20(token0),
                IERC20(token1)
            );
    }
    ...
    function swapExactTokensForTokens(
        ...
        address[] memory _tokenPools = DMMFactory(factory).getPools(
            IERC20(token0),
            IERC20(token1)
        );
        IERC20[] memory path = new IERC20[](2);
        path[0] = IERC20(token0);
        path[1] = IERC20(token1);
        IERC20(token0).approve(router, amount0);
        uint256[] memory amounts = DMMRouter(router).swapExactTokensForTokens(
            amount0,
            amountOutMin,
            _tokenPools,
            path,
