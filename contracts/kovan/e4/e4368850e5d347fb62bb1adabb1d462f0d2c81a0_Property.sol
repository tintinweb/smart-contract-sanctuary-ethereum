...
contract Property{
    ...
    function liquiContCollaAmount(address userAddr,uint contId) public view returns(uint){
        ...
        return amount * 105 /100 * deno / collaList[contList[contId].collaAddr].decimals;
    }

    //抵押清算
    function liquiContColla(uint contId, uint amount) public {
        require(contList[contId].contState == 0,"CONTID_ERROR");
        require(contList[contId].userAddr == msg.sender,"CONTID_ERROR");
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////
   uint public deno;
    ...
    function setStableList(address addr,string memory name,uint feePer, uint feeMin, uint withdrawMin, uint state) public {
        ...
        stableList[addr].feeMin = feeMin * (10 ** decimals) / deno;
        stableList[addr].withdrawMin = withdrawMin * (10 ** decimals) / deno;
    ...
    function withdraw(address USDAddr,address stableAddr,uint USDAmount)public {
        ...
        require(USDAmount >= stableList[stableAddr].withdrawMin,"withdrawMin_too_low");
        ...
        require(assetAmount - collaAmount >= USDAmount,"AMOUNT_INSUFFICIENT");

        uint fee = USDAmount * stableList[stableAddr].feePer / deno;
        if (fee < stableList[stableAddr].feeMin / deno){
            fee = stableList[stableAddr].feeMin / deno;
        }
