...
contract OrumFeeDistribution is IOrumRevenue{
    ...
    event OrumFeeDistributionoETHBalanceUpdated(uint _amount);
    ...
    function setAddresses(address _borrowerOpsAddress, address _activePoolAddress, address _rewardsPoolAddress, address _oETH) external {
    ...
    function _checkpoint_token() internal {
        uint token_balance = oETHToken.balanceOf(address(this));
    ...
    function claim(uint _tokenIndex) external returns (uint) {
        ...
        if (amount != 0) {
            oETHToken.transfer(msg.sender, amount);
    ...
    receive() external payable{
        ...
        _checkpoint_token();
        emit OrumFeeDistributionoETHBalanceUpdated(msg.value);
    }
