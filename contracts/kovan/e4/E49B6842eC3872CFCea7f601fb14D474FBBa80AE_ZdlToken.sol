...
contract ZdlToken is Context, IERC20, Ownable {
    ...
    string private _name = "Zdl Finance Community Token";
    string private _symbol = "Zdl";
    ...
    address public oldZdl = address(0x1676BDdf162A800cCb5f9e81b0F402A33c51e06c);
    ...
    function mappingInviter(address[] calldata _sender) public {
        ...
        for(uint i=0;i<_sender.length;i++){
            address account=IZdl(oldZdl).inviter(_sender[i]);
    ...
    function transferAddress(address[] calldata _newAddress) public {
        ...
        for(uint i=0;i<_newAddress.length;i++){
          uint256 value=IERC20(oldZdl).balanceOf(_newAddress[i]);
