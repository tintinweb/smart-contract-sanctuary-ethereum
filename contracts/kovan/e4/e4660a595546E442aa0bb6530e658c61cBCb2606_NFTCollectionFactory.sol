...
contract NFTCollectionFactory {
    ...
    // IDCard Contract address on kovan
    IDCardInterface immutable IDCard = IDCardInterface(0x44eEF5D302eCd4610872e28eEe2284f892E25dea);
    ...
    function newNFTCollection(
        ...
        NFTCollection(collectionAddr).initialize(IDCard.userId(creator), name, symbol);
        emit NewNFTCollection(collectionAddr, creator, name, symbol);
    }
}
...
interface IDCardInterface {
    ...
    function userId(string memory _username) external view returns(uint256 Id);
}
...
contract NFTCollection is Initializable, ERC721Upgradeable, ERC721URIStorageUpgradeable, IDCardOwnable {
...
contract IDCardOwnable is Initializable {
    // IDCard Contract address on kovan
    IDCardInterface immutable IDCard = IDCardInterface(0x44eEF5D302eCd4610872e28eEe2284f892E25dea);

    uint256 private _ownerID;

    function __Ownable_init(uint256 ownerID_) internal onlyInitializing {
        _ownerID = ownerID_;
    }
