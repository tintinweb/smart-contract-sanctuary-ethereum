...
contract Lottery is Ownable, HasRandom {
    ...
    uint256 public intervalTimerMin = 1440;
    address public cAASBankAddress;
    uint256 cAASBankAddressPercent = 50;
    uint256 public jackpot;
    uint256 public lotteryFeePercent = 5;
    uint256 jackpotN = 100;
    uint256 nextJackpotN = 100;
    bool public isOpened;

    event OnJackpot(uint256 count, uint256 roundNumber);
    ...
    function setOpenLottery(bool opened) external onlyOwner {
        isOpened = opened;
        nextIntervalTime = block.timestamp + intervalTimerMin * 1 minutes;
    }
    ...
    function setCAASBankAddress(address cAASBankAddress_)
        external
        onlyOwner
    {
    ...
    function getTicketsCount(address account) public view returns (uint256) {
        if (
            ...
            (
                (!_isWithdrawInterval &&
                    block.timestamp >= nextIntervalTime + intervalTimerMin * 1 minutes &&
                    playersCount > 1)
            )
        ) return 0;
    ...
    function intervalLapsedTime() public view returns (uint256) {
        ...
        // if timer
        if (block.timestamp < nextIntervalTime)
            return nextIntervalTime - block.timestamp;
    ...
    function isWithdrawInterval() external view returns (bool) {
        // if timer
        if (block.timestamp < nextIntervalTime) return _isWithdrawInterval;
        // now withdraw interval (skipping withdraq interval)
        if (
            !_isWithdrawInterval &&
            block.timestamp >= nextIntervalTime + intervalTimerMin * 1 minutes
