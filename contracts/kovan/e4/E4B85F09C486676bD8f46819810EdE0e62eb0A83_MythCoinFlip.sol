...
contract MythWhiteList {
    ...
    event betCancelled(address gameAddress, uint256 betId);
    ...
    function emitBetResolved(
        ...
        emit betResolved(msg.sender, resolutionSeed, betterWinner, betId);
        return true;
    }

    function emitBetCancelled(uint256 betId) external returns (bool) {
        require(
            whitelist[msg.sender],
            "Only whitelisted addresses can emit events"
        );
...
contract MythCoinFlip {
    ...
    function placeBet(bool _pvp, uint8 _side) external payable {
        ...
        if (!_pvp) {
            ...
            whitelist.emitBetCalled(address(0), block.number, betCount);
    ...
    function cancelBet(uint256 _betId) external payable {
        ...
        pendingBets--;
        whitelist.emitBetCancelled(_betId);
    }
    ...
    function callBet(uint256 _betId) external payable {
        ...
        betCountsPerBlock[block.number] += 1;
        pendingBets--;
        whitelist.emitBetCalled(msg.sender, block.number, _betId);
    ...
    function callBob(uint256 _betId) external {
        ...
        betCountsPerBlock[block.number] += 1;
        pendingBets--;
        whitelist.emitBetCalled(address(0), block.number, _betId);
