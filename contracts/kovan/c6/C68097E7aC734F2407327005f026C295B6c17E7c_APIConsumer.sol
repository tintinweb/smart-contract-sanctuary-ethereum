...
contract APIConsumer is ChainlinkClient {
    ...
    mapping (bytes32 => uint128) private typeIdrequested;

    mapping (uint128 => int256) public playerGWSCore;
    ...
    function getKeeperScore(uint128 playerId_) private returns(bytes32 requestId){
        Chainlink.Request memory request = buildChainlinkRequest(jobId, address(this), this.fulfill.selector);
        request.add("get", "http://localhost:8000/goal/");

        bytes memory id = abi.encodePacked(playerId_);
        
        request.add("path", string(id));
        requestId = sendChainlinkRequestTo(oracle, request, fee);
        typeIdrequested[requestId] = playerId_;
    }

    function getDefenderScore(uint128 playerId_) private returns (bytes32 requestId){
        Chainlink.Request memory request = buildChainlinkRequest(jobId, address(this), this.fulfill.selector);
        request.add("get", "http://localhost:8000/defender/");
        bytes memory id = abi.encodePacked(playerId_);
    ...
    function getMidfielderScore(uint128 playerId_) private returns(bytes32 requestId){
        Chainlink.Request memory request = buildChainlinkRequest(jobId, address(this), this.fulfill.selector);
        request.add("get", "http://localhost:8000/midfielder/");
        bytes memory id = abi.encodePacked(playerId_);
    ...
    function getForwardScore(uint128 playerId_) private returns(bytes32 requestId){
        Chainlink.Request memory request = buildChainlinkRequest(jobId, address(this), this.fulfill.selector);
        request.add("get", "http://localhost:8000/forward");
        bytes memory id = abi.encodePacked(playerId_);
     ...
     */ 
    function fulfill(bytes32 _requestId, int256 _score) public recordChainlinkFulfillment(_requestId)
    {
        uint128 _playerId = typeIdrequested[_requestId];
        playerGWSCore[_playerId] = _score;
    }
