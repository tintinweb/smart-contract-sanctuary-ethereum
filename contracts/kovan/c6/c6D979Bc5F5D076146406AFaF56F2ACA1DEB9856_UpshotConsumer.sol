...
contract UpshotConsumer is ChainlinkClient {
    ...
    /**
     * @notice Returns the floor price in GWEI for an NFT collection.
     * @dev Result format is a uint256.
     * @param _specId the jobID.
     * @param _payment the LINK amount in Juels (i.e. 10^18 aka 1 LINK).
     * @param _assetId the ID of the collection for which we want the floor price.
     */
    ...
    function fulfillPrice(bytes32 _requestId, uint256 _price) public recordChainlinkFulfillment(_requestId) {
        requestIdResult[_requestId] = _price;
    }
