...
contract UniswapFlash is 
        ...
        PoolAddress.PoolKey memory poolKey = PoolAddress.PoolKey(
            {
                token0: loanAssets[0],
                token1: loanAssets[1],
                fee: swapRouterInfos[UNISWAP_V3_ROUTER_ID].poolFee
            }
        );
    ...
    function uniswapV3FlashCallback(
        ...
        require(msg.sender == address(flashPool), "Only pool contract can call");
        (
        ...
        require(
            ...
            "one of token amounts should be 0"
        );
        ...
        uint256 amountOut = tradeExecute(loanAsset, loanAmount, tradeAssets, tradeDexs);
     
        // pay back pool the loan 
        // note: msg.sender == pool to pay back 
        if (amountOwed > 0) {
            TransferHelper.safeApprove(loanAsset, address(this), amountOwed);
        ...
        if (amountOut > amountOwed) {
            ...
            TransferHelper.safeApprove(loanAsset, address(this), profit);
            pay(loanAsset, address(this), decoded.payer, profit);
        }
    }
}
...
library Helpers {
    ...
    // function logBalances(address[] memory assets) private view  {
    //     for (uint i = 0; i < assets.length; i++) {
    //         uint balance = IERC20Metadata(assets[i]).balanceOf(address(this));
    //         string memory symbol = IERC20Metadata(assets[i]).symbol();
    //         console.log("Balance of %s ---> %s", symbol, balance); 
    //     }
    // }
...
contract SwapAssets is 
    ...
    function tradeExecute(
        ...
        amountOut = swapAsset(Helpers.getPaths(loanedAssest, tradeAssets[0]), loanedAmount, tradeDexes[0]);
        for (uint i = 1; i < tradeAssets.length; i++) {
            amountOut = swapAsset(Helpers.getPaths(tradeAssets[i - 1], tradeAssets[i]), amountOut, tradeDexes[i]);
        }
    }
    ...
    function swapAsset(
        ...
        if (dexId == UNISWAP_V3_ROUTER_ID) {
            ...
            amountOut = uniV3ExactInputSingle(
                path,
                amountIn,
                0,
            ...
            swapV2Router = IUniswapV2Router02(swapRouterInfos[dexId].router);
            amountOut = uniV2InputTokenForToken(
                path,
                amountIn,
                0
            );
            ...
            pancakeRouter = IPancakeRouter02(swapRouterInfos[dexId].router);
            amountOut = pancakeInputTokenForToken(
                path,
                amountIn,
                0
            );
...
contract UniswapV3Router {
    ...
    //     uniswapV3Router = ISwapRouter(_routerAddress);
    // }

    function uniV3ExactInputSingle(
        address[] memory path,
        uint256 amountIn,
    ...
    /// @notice uniswapV3ExactOutputSingle swaps a minimum possible amount of tokenIn for a fixed amount of WETH.
    /// @dev The calling address must approve this contract to spend its tokenIn for this function to succeed. As the amount of input token1 is variable,
    /// the calling address will need to approve for a slightly higher amount, anticipating some variance.
    /// @param amountOut The exact amount of tokenOut to receive from the swap.
    /// @param amountInMax The amount of tokenIn we are willing to spend to receive the specified amount of tokenOut.
    /// @return amountIn The amount of tokenIn actually spent in the swap.
    function uniV3ExactOutputSingle(
        address[] memory path,
        uint256 amountOut,
        uint256 amountInMax,
        uint24 poolFee
        ...
        // Approve the router to spend the specifed `amountInMax` of tokenIn.
        // In production, you should choose the maximum amount to spend based on oracles or other data sources to acheive a better swap.
        TransferHelper.safeApprove(path[0], address(uniswapV3Router), amountInMax);

        amountIn = uniswapV3Router.exactOutputSingle(
    ...
    function uniV3ExactInputTriangular(
        address[] memory path,
        uint256 amountIn,
        uint256 amountOutMin,
    ...
    /// @notice uniswapV3ExactOutputTriangular swaps a minimum possible amount of token1 for a fixed amount of WETH through an intermediary pool.
    /// For this example, we want to swap token1 for token3 through a token2 pool but we specify the desired amountOut of token3. Notice how the path encoding is slightly different in for exact output swaps.
    /// @dev The calling address must approve this contract to spend its token1 for this function to succeed. As the amount of input token1 is variable,
    /// the calling address will need to approve for a slightly higher amount, anticipating some variance.
    /// @param amountOut The desired amount of token3.
    /// @param amountInMax The maximum amount of token1 willing to be swapped for the specified amountOut of token3.
    /// @return amountIn The amountIn of token1 actually spent to receive the desired amountOut.
    function uniV3ExactOutputTriangular(
        address[] memory path,
        uint256 amountOut,
        uint256 amountInMax,
        uint24[] memory poolFee
        ...
        require(path.length == 3, "Invaild triangular trade");
        require(poolFee.length == 2, "Invaild pool fee");
        // Approve the router to spend  `amountInMax`.
        TransferHelper.safeApprove(path[0], address(uniswapV3Router), amountInMax);
        bytes memory datas = abi.encodePacked(path[2], poolFee[1], path[1], poolFee[0], path[0]);
        // The parameter path is encoded as (tokenOut, fee, tokenIn/tokenOut, fee, tokenIn)
        // The tokenIn/tokenOut field is the shared token between the two pools used in the multiple pool swap. In this case token2 is the "shared" token.
        // For an exactOutput swap, the first swap that occurs is the swap which returns the eventual desired token.
        // In this case, our desired output token is token3 so that swap happpens first, and is encoded in the path accordingly.
...
contract UniswapV2Router {
    ...
    function uniV2InputTokenForToken(
        address[] memory path,
        uint256 amountIn,
        uint256 amountOutMin
    ) internal returns (uint256 amountOut) {
        ...
        TransferHelper.safeApprove(path[0], address(swapV2Router), amountIn);
    
        amountOut = swapV2Router.swapExactTokensForTokens(
            amountIn,
            amountOutMin,
            path,
            msg.sender,
    ...
    function uniV2TokenForOutToken(
        address[] memory path,
        uint256 amountOut,
        uint256 amountInMax
    ) internal returns (uint256 amountIn) {
        ...
        amountIn = swapV2Router.swapTokensForExactTokens(
            amountOut,
            amountInMax,
            path,
            msg.sender,
...
contract PancakeswapRouter {
    ...
    function pancakeInputTokenForToken(
        address[] memory path,
        uint256 amountIn,
        uint256 amountOutMin
    ) internal returns (uint256 amountOut) {
        // Approve the router to spend token.
        TransferHelper.safeApprove(path[0], address(pancakeRouter), amountIn);
    ...
    function pancakeTokenForOutToken(
        address[] memory path,
        uint256 amountOut,
        uint256 amountInMax
    ) internal returns (uint256 amountIn) {
        ...
        amountIn = pancakeRouter.swapTokensForExactTokens(
            amountOut,
            amountInMax,
            path,
            msg.sender,
