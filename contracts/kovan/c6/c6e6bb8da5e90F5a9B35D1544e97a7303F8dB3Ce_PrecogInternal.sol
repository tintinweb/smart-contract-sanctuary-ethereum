...
contract PrecogInternal is IPrecogInternal {
    ...
    modifier onlyPrecog() {
        require(
            ...
            "PrecogInternal: Caller is not accessible"
        );
        _;
    }
    ...
    modifier isExistingToken(address token) {
        require(
            ...
            "PrecogInternal: Token is not in trading"
        );
        _;
    }
    ...
    {
        IPrecogStorage.Cycle[] memory _tradingCycles = precogStorage.getTradingCycles(
            _token
        );
        IPrecogStorage.Cycle memory _lastTradingCycle = _tradingCycles[
            _tradingCycles.length - 1
        ];
        if (precogStorage.checkIsRemoved(_token)) {
            return _lastTradingCycle;
        }
        _currentTradingCycleByTimestamp = PrecogV5Library
            ._calculateTradingCycleByTimestamp(
                _lastTradingCycle,
        ...
        returns (IPrecogStorage.AccountProfitInfo memory _accountProfitInfo)
    {
        ...
        IPrecogStorage.Investment[] memory _investments = precogStorage.getInvestmentsOf(
            _token,
            _account
        );
        IPrecogStorage.Cycle[] memory _tradingCycles = precogStorage.getTradingCycles(
        ...
        if (_accountProfitInfo.lastProfitId < _updatedLatestCycle) {
            ...
            for (
                ...
                if (
                    ...
                    _updatedLatestCycle ==
                    _investments[_accountProfitInfo.lastInvestmentId].idChanged
                ) {
                    return _accountProfitInfo;
                }
                // Get next investment of account and available profit id for the loop of profitId
                (_nextInvestment, _lastAvailableProfitId) = PrecogV5Library
                    ._chooseLastAvailableTradingId(
                        _investments,
                        _accountProfitInfo.lastInvestmentId,
                        _updatedLatestCycle
                    );
                ...
                for (
                    ...
                    if (_investments[_accountProfitInfo.lastInvestmentId].isWhitelist) {
                        ...
                        uint totalUnitsForWhitelist = precogStorage
                            .getTotalUnitsForWhitelistTradingCycle(
                                _token,
                                _accountProfitInfo.lastProfitId
                            );
                        // Calculate profit for account at trading cycle
                        _accountProfitInfo.profitForWhitelist += PrecogV5Library
                            ._calculateProfitAtCycle(
                    ...
                    // Calcuclate profit for normal account at trading cycle
                        ...
                        uint totalUnits = (precogStorage.getTotalUnitsTradingCycle(
                            _token,
                            _accountProfitInfo.lastProfitId
                        ) -
                        ...
                        // Calculate profit for account at trading cycle
                        _accountProfitInfo.profit += PrecogV5Library
                            ._calculateProfitAtCycle(
                                _tradingCycles[_accountProfitInfo.lastProfitId],
                                _investments[_accountProfitInfo.lastInvestmentId],
                                totalUnits,
    ...
    function _processFirstTimeIncreaseInvestment(
        ...
        IPrecogStorage.AccountProfitInfo memory _accountProfitInfo = precogStorage
            .getAccountProfitInfo(_token, _account);
        if (
            !precogStorage
                .getAccountTradingInfo(_token, _account)
                .isNotFirstIncreaseInvestment
        ) {
    ...
    function _updateTotalUnitsForFirstTimeOfTradingCycle(
        ...
        if (
            !precogStorage.checkIsUpdateUnitTradingCycle(_token, _futureTradingCycle.id)
        ) {
            uint _duration = _futureTradingCycle.endTime - _futureTradingCycle.startTime;

            // Update total units for trading cycle
            uint newTotalUnits = precogStorage.getTotalUnitsTradingCycle(
                _token,
            ...
            uint newTotalUnitsForWhitelist = precogStorage
                .getTotalUnitsForWhitelistTradingCycle(_token, _futureTradingCycle.id) +
                (precogStorage.getLiquidityWhitelist(_token) * _duration);
            precogStorage.updateTotalUnitsForWhitelistTradingCycle(
                _token,
                _futureTradingCycle.id,
                newTotalUnitsForWhitelist
            );
    ...
    function _processIncreaseInvestment(
        ...
        if (_investmentsOf.length > 0) {
            ...
            IPrecogStorage.Investment memory _newInvestmentOf = IPrecogStorage
                .Investment({
            ...
            uint newTotalUnits = precogStorage.getTotalUnitsForWhitelistTradingCycle(
                _token,
                _futureTradingCycle.id
            );
    ...
    /**
     * @dev Calculates and updates investment amount, unit, trading cycle id 
     * at current investment or push new investment into array
     * @param _token is token address
     * @param _account is account address
     * @param _amount is token amount that is used to increase unit in trading cycle
     * @param _timestamp is timestamp that is the next funding cycle with deposit or block.timestamp
     */
    function _increaseInvestment(
        ...
        IPrecogStorage.Cycle memory _futureTradingCycle = _getTradingCycleByTimestamp(
            _token,
            _timestamp
        );
    ...
    function _updateCurrentTradingCycle(
        ...
        IPrecogStorage.Cycle memory _lastTradingCycle = precogStorage.getLastTradingCycle(
            _token
        );
