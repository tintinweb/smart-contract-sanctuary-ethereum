...
contract Astrobot is
    ...
    OwnableUpgradeable
{
    bytes32 public constant AIRDROP_MINT_ROLE = keccak256("AIRDROP_MINT_ROLE");

    uint256 public airdropSupply;
    uint256 public maxSupply;
    uint256 public auctionSupply;
    uint256 public dutchMinted;
    ...
    event DutchMinted(address indexed _user, uint256 indexed _quantity);
    event AirdropMinted(address indexed _user, uint256 indexed _quantity);
    ...
    function initialize(
        ...
        _grantRole(AIRDROP_MINT_ROLE, msg.sender);
        airdropSupply = _airdropSupply;
        maxSupply = _maxSupply;
        auctionSupply = maxSupply - airdropSupply;
    }
     ...
     *
     * @dev Update dutch contract address. Only owner can call this function.
     * @param _auctionContract Dutch auction contract address.
     */
    ...
    function updateAuctionContract(address _auctionContract)
        ...
        onlyOwner
    {
        require(_auctionContract != address(0), "Zero address");
     ...
     * @notice Mint Tokens
     *
     * @dev Mint new token. Only dutch contract can call this function.
    ...
    function mint(address _user, uint256 quantity) external whenNotPaused {
        require(
            auctionSupply >= dutchMinted + quantity,
            "mint: Auction completed"
        );
        require(
            msg.sender == auctionContract,
            "mint: Caller is not auction contract"
        );
        dutchMinted = dutchMinted + quantity;
        _mint(_user, quantity);
        emit DutchMinted(_user, quantity);
    }

    /**
     * @notice Mint Airdrop Tokens
     *
     * @dev Mint new token for airdrop. Only user with access can call this function.
     * @param _user Address to mint token.
     * @param quantity Number of tokens to mint.
     */

    function airdropMint(address _user, uint256 quantity)
        external
        onlyRole(AIRDROP_MINT_ROLE)
        whenNotPaused
    {
        require(airdropSupply >= airdropMinted + quantity, "mint: All minted");
        airdropMinted = airdropMinted + quantity;
        _mint(_user, quantity);
        emit AirdropMinted(_user, quantity);
    }
