...
contract FearWolfDistributor is AccessControl, Pausable, Withdrawable, ReentrancyGuard {
    ...
    */
    ...
    function setBatchMintingLimit(uint256 _batchMintingLimit) external onlyRole(ADMIN_ROLE) {
        require(block.timestamp < saleStart, "Too late, sale is already active");
    ...
    function giftWolves(address _address, uint256 _wolvesCount) external onlyRole(ADMIN_ROLE) {
        require(giftWolvesCount >= _wolvesCount, "Not enough wolves left to gift");
        giftWolvesCount -= _wolvesCount;
        fearWolfContract.initialMint(_address, _wolvesCount);
        emit WolvesBought(_address, 0, _wolvesCount);
    }
