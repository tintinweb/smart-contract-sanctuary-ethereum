...
contract PlasmaGasStation is Ownable, LondonTxSupport {
    ...
    uint256 private txRelayFeeMultiplier;
    ...
    constructor(address _router, address _feePayer, uint256 _txRelayFee) {
        ...
        txRelayFeeMultiplier = _txRelayFee.add(100).div(100);
    ...
    function getTxRelayFee() external view returns (uint256) {
        return txRelayFeeMultiplier.mul(100).sub(100);
    }
    ...
    function sendTransaction(TxRequest calldata _tx, TxFee calldata _fee, bytes calldata _sign) external onlyFeePayer {
        ...
        uint256 maxFeeInEth = _calculateCharge(_tx.gas, txRelayFeeMultiplier, _tx);
        ...
        uint256 feeInEth = _calculateCharge(estimatedGasUsed, txRelayFeeMultiplier, _tx);
...
abstract contract LondonTxSupport is EIP712Library {
    ...
    function _calculateCharge(uint256 _gasUsed, uint256 _txRelayFeeMultiplier, TxRequest calldata _tx) internal view returns (uint256) {
        ...
        return _gasUsed.mul(feePerGas).mul(_txRelayFeeMultiplier);
    }
}
