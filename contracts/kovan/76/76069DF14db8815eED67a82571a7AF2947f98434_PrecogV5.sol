...
contract PrecogV5 is IPrecogV5 {
    ...
    function addLiquidityPool(address token) external onlyAdmin {
        require(!isExistingToken[token], "4");
        require(tokenConvert[token] == address(0) && token != core.PCOG() && token != address(0), "4");
        // Deploy new IPCOG
        ...
        if (!isNotFirstInvestmentCycle[token]) {
            tradingCycles[token].push(Cycle(0, currentTime, currentTime + cycleConfig.firstTradingCycle));
    ...
    function removeLiquidityPool(address token) external onlyAdmin _isExistingToken(token) {
        ...
        require(IERC20(tokenConvert[token]).totalSupply() == 0, "5");
    ...
    function _decreaseInvestment(
        ...
        require(!isBeforeFundingTime(_token, _account), "04");
        Investment[] memory _investmentsOf = investmentsOf[_token][_account];
    ...
    function requestWithdrawal(address token, uint amount) external override {
        require(amount >= core.minDefunding(token), "01");
        _updateCurrentTradingCycle(token, false, 0);
        ...
        require(remainingAmount == 0 || remainingAmount >= core.minFunding(token), "02");
    ...
    function _withdrawBeforeFundingTime(
        ...
        require(availableWithdrawalAmount >= _amount && IERC20(_token).balanceOf(address(this)) >= _amount, "03");
        uint remainingAmount = _decreaseInvestmentWhenWithdraw(_token, _account, _amount);
        require(
            remainingAmount == 0 ||
                (remainingAmount >= core.minFunding(_token) && _amount >= core.minDefunding(_token)), "02"
        );
