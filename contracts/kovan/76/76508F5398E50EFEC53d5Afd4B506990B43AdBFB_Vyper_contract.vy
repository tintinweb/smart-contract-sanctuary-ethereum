...
def __init__(
    ...
    for i in range(BASE_N_COINS):
        _base_coin: address = Curve(_3pool).coins(convert(i, uint256))
        self.BASE_COINS[i] = _base_coin

    self.BASE_POOL = _3pool
...
def withdraw_admin_fees():
    ...
    if amount > 0:
        receiver: address = Factory(factory).fee_receiver(self.BASE_POOL)
