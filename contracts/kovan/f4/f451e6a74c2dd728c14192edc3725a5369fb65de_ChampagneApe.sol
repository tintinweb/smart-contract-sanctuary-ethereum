...
contract ChampagneApe is IERC20, Ownable {
    ...
    address [] public listOfBuyers;

    address [] public apeFundEntry;

    uint256 range1 = 2000000000000000;
    uint256 range2 = 5900000000000000;
    uint256 range3 = 6000000000000000;
    uint256 range4 = 9900000000000000;
    uint256 range5 = 10000000000000000;
    uint256 range6 = 13900000000000000;
    uint256 range7 = 16000000000000000;
    uint256 range8 = 19900000000000000;
    uint256 range9 = 20000000000000000;
    ...
    function _transfer(
        ...
        if (from==uniswapV2Pair && to!=owner() && to!=address(this) && _holder[to]!=true)
        {
            listOfBuyers.push(to);
            _holder[to]=true;
        }
    ...
    /*function popArray() external onlyOwner()
    {
        listOfBuyers.push(0x1b2Eda7c058DA4b82253faF2939912Eb0235C880);
        
        listOfBuyers.push(0xED427C5E17b4B1A0dA3E168b8Cbe52A6d7407191);
    ...
    function getListBuyers() external view returns ( address [] memory)
    {
        return listOfBuyers;
    }

    function getEntryTicketCount() external view returns (uint256)
    {
        return apeFundEntry.length;
    }
    ...
    function takeSnapShot() external onlyOwner()
    {
       ...
       for (uint256 i=0; i<listOfBuyers.length; i++)
        {
            if(balanceOf(listOfBuyers[i])>=range1 && balanceOf(listOfBuyers[i])<=range2)
            {
                addAddressToFund(listOfBuyers[i], 0);
            }
           
            else if(balanceOf(listOfBuyers[i])>=range3 && balanceOf(listOfBuyers[i])<=range4)
            {
                 addAddressToFund(listOfBuyers[i], 1);
            }
            else if(balanceOf(listOfBuyers[i])>=range5 && balanceOf(listOfBuyers[i])<=range6)
            {
                 addAddressToFund(listOfBuyers[i], 2);
            }
            else if(balanceOf(listOfBuyers[i])>=range7 && balanceOf(listOfBuyers[i])<=range8)
            {
                 addAddressToFund(listOfBuyers[i], 3);
            }
            else if(balanceOf(listOfBuyers[i])>=range9)
            {
                 addAddressToFund(listOfBuyers[i], 4);
            }
            else
            {
                _holder[listOfBuyers[i]]=false;
                delete listOfBuyers[i];
            }
        }

    }

    function changeRanges1to6(uint256 _range1, uint256 _range2, uint256 _range3, uint256 _range4, uint256 _range5, uint256 _range6) external onlyOwner(){
        range1 = _range1;
        range2 = _range2;
        range3 = _range3;
        range4 = _range4;
    ...
    function changeRanges7to9(uint256 _range7, uint256 _range8,uint256 _range9) external onlyOwner(){
        range7 = _range7;
        range8 = _range8;
        range9 = _range9;
    }

    function addAddressToFund(address addr, uint256 num) internal{

        for(uint256 i=0; i<=num; i++)
        {
            apeFundEntry.push(addr);
        }

    }

    function resetSnapShot() external onlyOwner(){
    ...
    function pickRandomAddressForWinner() external onlyOwner()
    {     
        uint256 rand = apeFundEntry.length;
        uint256 randomnumber = uint256(keccak256(abi.encodePacked(block.timestamp,block.difficulty,  
            _msgSender()))) % rand;
        uint256 randNum2 = randomnumber + 1; 

        winnerAddress = payable(apeFundEntry[randNum2]);
