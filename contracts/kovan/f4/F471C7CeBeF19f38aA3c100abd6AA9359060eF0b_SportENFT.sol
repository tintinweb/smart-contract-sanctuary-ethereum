...
contract SportENFT is ERC721Burnable, AccessControlEnumerable, EIP712 {
    ...
    function equip(uint256 tokenId, bytes calldata signature) external {
        ...
        require(hasRole(SIGNER_ROLE, _signer), "SportE: signature invalid");

        require(
            ownerOf(tokenId) == msg.sender,
            "SportE: equip caller is not owner nor approved"
        );
        require(!_idFreeze[tokenId], "SportE: nft equipped");
    ...
    function redeem(
        ...
        require(_exists(tokenId), "SportE: signature invalid");
        require(hasRole(SIGNER_ROLE, _signer), "SportE: tokenId is not exists");

        require(
            msg.sender == owner || isApprovedForAll(owner, msg.sender),
            "SportE: Must be approve"
        );
