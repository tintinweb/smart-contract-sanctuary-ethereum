...
contract P4TCHwxyz is ERC721, ERC721Enumerable, Ownable {
    ...
    function whitelistMint() external payable {
        ...
        if (totalSupply() < MAX_W_ID) {            //WHITELISTED [W] SALE

        } else if (totalSupply() < MAX_X_ID) {     //WHITELISTED [X] SALE
            require(!isWPublic, "[W] is still active");

        } else if (totalSupply() < MAX_Y_ID) {     //WHITELISTED [Y] SALE
            require(!isXPublic, "[X] is still active");

        } else if (totalSupply() < MAX_Z_ID) {     //WHITELISTED [Z] SALE
    ...
    function publicMint(uint256 numOfPatches) external payable {
        ...
        require(msg.value == _price * numOfPatches, "Ether value sent is not correct");
        //require(msg.sender == tx.origin, "We like real users");

        if (totalSupply() + numOfPatches <= MAX_W_ID) {          //PUBLIC [W] SALE
            require(isWPublic, "[W] public sale must be active to mint patches");
            require(numOfPatches <= 2, "Exceeded max numOfPatches");

        } else if (totalSupply() + numOfPatches <= MAX_X_ID) {   //PUBLIC [X] SALE
            require(isXPublic, "[X] public sale must be active to mint patches");
            require(numOfPatches <= 3, "Exceeded max numOfPatches");

        } else if (totalSupply() + numOfPatches <= MAX_Y_ID) {   //PUBLIC [Y] SALE
            require(isYPublic, "[Y] public sale must be active to mint patches");
            require(numOfPatches <= 4, "Exceeded max numOfPatches");

        } else if (totalSupply() + numOfPatches <= MAX_Z_ID) {   //PUBLIC [Z] SALE
            require(isZPublic, "[Z] public sale must be active to mint patches");
            require(numOfPatches <= 5, "Exceeded max numOfPatches");

        } else {
            revert SoldOut();
        }
        for (uint256 i = 0; i < numOfPatches; i++) {
            uint256 mintIndex = nextPatchId;
            nextPatchId++;
...
/* W */ if (_tokenId <= 111) {                   // 1[X], 2[Y], 3[Z] Claimable
            ...
            if (totalSupply() < MAX_X_ID) {

                require(!isClaimed(_tokenId + 10000), "1[X] P4TCH claimed already");
                _setClaimed(_tokenId + 10000);

            } else if (totalSupply() < MAX_Y_ID) {
...
/* X */ } else if (_tokenId - MAX_W_ID <= 333) {      // 1[Y], 1[Z] Claimable
            ...
            if (totalSupply() < MAX_Y_ID) {
                require(!isClaimed(_tokenId + 70000), "[Y] P4TCH claimed already");
                _setClaimed(_tokenId + 70000);

            } else if (totalSupply() < MAX_Z_ID) {
                require(!isClaimed(_tokenId + 80000), "[Z] P4TCH claimed already");
...
/* Y */ } else if (_tokenId - MAX_X_ID <= 888) {           // 1[Z] Claimable
            require(totalSupply() < MAX_Z_ID);
            require(!isClaimed(_tokenId + 90000), "[Z] P4TCH claimed already");
    ...
    function devMint(uint256 numOfPatches) external onlyOwner {
        ...
        if (totalSupply() + numOfPatches <= MAX_W_ID) {
            require(devMinted + numOfPatches <= 11, "Dev minted 11 [W] already");

        } else if (totalSupply() + numOfPatches <= MAX_X_ID) {
            require(totalSupply() >= MAX_W_ID, "The last W ID has not been minted yet");
            require(devMinted + numOfPatches <= 44, "Dev minted 33 [X] already");

        } else if (totalSupply() + numOfPatches <= MAX_Y_ID) {
            require(totalSupply() >= MAX_X_ID, "The last X ID has not been minted yet");
            require(devMinted + numOfPatches <= 132, "Dev minted 88 [Y] already");

        } else if (totalSupply() + numOfPatches <= MAX_Z_ID) {
            require(totalSupply() >= MAX_Y_ID, "The last Y ID has not been minted yet");
    ...
    function addAddresses(address[] memory _address) external onlyOwner {
        for (uint i=0; i<_address.length; i++) {
            addWL(_address[i]);
        }
    }

    function removeAddresses(address[] memory _address) external onlyOwner {
        for (uint i=0; i<_address.length; i++) {
            removeWL(_address[i]);
        }
    }
    ...
    /**
     * P4TCH Ids Of Owner/Address Function
     */
