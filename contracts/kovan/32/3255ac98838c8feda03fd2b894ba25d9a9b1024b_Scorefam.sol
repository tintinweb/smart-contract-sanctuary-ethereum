    ...
    UUPSUpgradeable
{
    ...
    address private consumer;
     ...
     //Bytes Manipulation
    bytes private status;
    uint256 private homeScore;
    uint256 private awayScore;
    ...
    function sliceBytes(bytes memory _bytes) internal {
        ...
        returns (uint256)
    {
        ...
        require(gameType == 3, "Scorefam: Invalid Game Type");
            //Base
            if (amount < 74900000000) {
                reward = ((getHalvedReward(amount) * amount) / 100) + amount;
            } 
            //Bronze
            else if (amount >= 75000000000 && amount < 149900000000) {
                reward = ((getHalvedReward(amount) * amount) / 100) + amount;
            }
            //Gold
            else if (amount >= 150000000000 && amount < 249900000000) {
                reward = ((getHalvedReward(amount) * amount) / 100) + amount;
            }
            //VIP
            else if (amount >= 250000000000 ) {
                reward = ((getHalvedReward(amount) * amount) / 100) + amount;
            }
            else {
                reward = amount;
    ...
    { 
        ...
        uint256 reward;
         ...
         // Outcome Types {
            // home: 1; draw: 2; away: 3; bts: 4; odd: 5; even: 6;
            // over1: 7; under1: 8; 
            // over2: 9; under2: 10;
            // over3: 11; under3: 12;
            // over4: 13; under4: 14;
    ...
    function claimGameReward(uint256 game_) public returns (bool){
        ...
        if(sc.requestIdData(sc.gameScoreRequestID(_gaming[_msgSender()][game_].fixtureID)).length > 5){
            sliceBytes(sc.requestIdData(sc.gameScoreRequestID(_gaming[_msgSender()][game_].fixtureID)));
        } else {
        ...
        require(status.equal(hex"66696e6973686564"), "Scorefam: Game Not Ended");
    ...
    function claimSlipReward(uint256 game_) public returns (bool){
        ...
        for(uint256 i = 0; i <= _slipsCount[_msgSender()]; i++){
            ...
            "Scorefam: Reward not yet available");
            //Update Game Status, Home and Away Scores from the Oracle
            if(sc.requestIdData(sc.gameScoreRequestID(_gamingSlip[_msgSender()][game_].slips[i].fixtureID)).length > 5){
                sliceBytes(sc.requestIdData(sc.gameScoreRequestID(_gamingSlip[_msgSender()][game_].slips[i].fixtureID)));
            } else {
                bytes32 JOB_ID = bytes32("8bf0b32bca0f41afab51577f8d156cf2");
                sc.requestGameScore(JOB_ID, _gamingSlip[_msgSender()][game_].slips[i].fixtureID);
                sliceBytes(sc.requestIdData(sc.gameScoreRequestID(_gamingSlip[_msgSender()][game_].slips[i].fixtureID)));
            }
        }
