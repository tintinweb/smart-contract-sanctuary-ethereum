    ...
    AccessControlUpgradeable
{
    ...
    function validatePhaseSpecificPurchase(uint256 amount, uint256 phase, bytes32[] calldata proof) internal {
        ...
        if (phase == 1) {
            ...
            require(bought + amount <= maxSupplyPhaseOne, "BUY AMOUNT GOES OVER MAX SUPPLY FOR PHASE");        
        } else if (phase == 2) {
            ...
            require(MerkleProofUpgradeable.verify(proof, merkleRootPhaseTwo, leaf), "INVALID PROOF");
            require(bought + amount <= maxSupplyPhaseTwo, "BUY AMOUNT GOES OVER MAX SUPPLY FOR PHASE");
        } else if (phase == 3) {
            require(block.timestamp >= phaseThreeStartTime, "PHASE THREE SALE HASN'T STARTED YET"); 
    ...
    /**
     * @notice Change the maximum supply of tickets that are for sale.
     *
    ...
    /**
     * @notice Change the maximum supply of tickets that are for sale in phase one of sale.
     *
     * @param newMaxSupplyPhaseOne. The new max supply.
     */
    function setMaxSupplyPhaseOne(uint256 newMaxSupplyPhaseOne) external onlyOwner {
        maxSupplyPhaseOne = newMaxSupplyPhaseOne;
        emit setMaxSupplyPhaseOneEvent(newMaxSupplyPhaseOne);
    }

    /**
     * @notice Change the maximum supply of tickets that are for sale in phase two of sale.
     *
     * @param newMaxSupplyPhaseTwo. The new max supply.
     */
    function setMaxSupplyPhaseTwo(uint256 newMaxSupplyPhaseTwo) external onlyOwner {
        maxSupplyPhaseTwo = newMaxSupplyPhaseTwo;
        emit setMaxSupplyPhaseTwoEvent(newMaxSupplyPhaseTwo);
    }
