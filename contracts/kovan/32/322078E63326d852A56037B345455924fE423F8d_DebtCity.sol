...
*/
    ...
    constructor() ERC721("DebtCity", "BANKERS") {
        _owner = msg.sender;
        // TODO: change this address or test without it
        payableWallet = payable(address(0x3811866A85b0e42B437B36BB4FD2f05e4c3b16f8));
    ...
    function hash(
        ...
        for (uint8 i = 0; i < 15; i++) {
            ...
            uint8 traitVal = applyBankerRules(i, bstyle, randDigit);
    ...
    function mintInternal(uint8 quantity) internal {
        ...
        //require(msg.value == (ethPrice * quantity); // TODO uncomment this
        require(
    ...
    function mintBanker(uint8 quantity) public {
    ...
    function mintAllowList(uint8 quantity) external payable {
        ...
        require(ethPrice * quantity <= msg.value, "Ether value sent is not correct");
    ...
    function applyBankerRules(uint8 i, uint8 bstyle, uint8 randDigit) pure internal returns (uint8) {

        if (i == 6 && randDigit > 5) randDigit = 0;
        else if (i == 7 || i == 8) {
            if ((bstyle == 5 && randDigit > 5) || bstyle < 5) randDigit = 0;
        } 
        else if (i == 9) {
            if (bstyle  == 5 || bstyle == 4 || (bstyle == 2 && randDigit > 5)) randDigit = 0;
            if (bstyle == 3 && randDigit == 0) randDigit = 1;
        }
        else if (i == 10) {
            if (bstyle < 4 || randDigit > 5) randDigit = 0;
        }
        else if (i == 11) {
            if (bstyle > 2) randDigit = 0;
        }
        else if (i == 12 && bstyle > 1) randDigit = 0;
        else if (i == 13 && randDigit > 7) randDigit = 0;
        else if(i == 14 && (randDigit > 5 || bstyle == 5)) randDigit = 0; 
    ...
    function getBankersRemaining() public view returns (uint256) {
        return (REMAINING_BOARD_MEMBERS + REMAINING_VPS + REMAINING_ASSOCIATES + REMAINING_ANALYSTS); 
    }
