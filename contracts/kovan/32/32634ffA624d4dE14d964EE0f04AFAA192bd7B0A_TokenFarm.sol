...
contract TokenFarm is Ownable {
    ...
    constructor(address kolTokenAddress) {
        kolToken = IERC20(kolTokenAddress);
    }
    ...
    function setPriceFeedContract(address token, address priceFeedAddress)
        ...
        onlyOwner
    {
        tokenPriceFeedAddress[token] = priceFeedAddress;
    }
    ...
    function issueTokens() public onlyOwner {
        for (
            ...
            uint256 totalValue = getUserTotalValue(recipent);
            kolToken.transfer(recipent, totalValue);
        }
    }
    ...
    function getUserSingleTokenValue(address user, address token)
        ...
        returns (uint256)
    {
        (uint256 price, uint256 decimals) = getSingleTokenPrice(token);
        return ((stakingBalance[token][user] * price) / (10**decimals));
    }

    function getSingleTokenPrice(address token)
        public
        view
        returns (uint256, uint256)
    {
        AggregatorV3Interface priceFeed = AggregatorV3Interface(
            tokenPriceFeedAddress[token]
        );
