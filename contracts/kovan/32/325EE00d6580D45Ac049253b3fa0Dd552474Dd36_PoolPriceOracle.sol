...
// File contracts/misc/isolation/PoolPriceOracle.sol
...
contract PoolPriceOracle is OwnableUpgradeable, PriceOracle{
    ...
    PriceSource public defaultSource;
    mapping(address => TokenConfig) public tokenConfigs;

    event NewDefaultSource(address oldSource, address newSource);
    event NewSource(address underlying, address oldSource, address newSource);

    function initialize(PriceSource _defaultSource) public initializer {
        defaultSource = _defaultSource;
        OwnableUpgradeable.__Ownable_init();
    }

    function getUnderlyingPrice(MarketTokenInterface marketToken) external override view returns (uint price){

        address underlying = marketToken.underlying();
        price = getPrice(underlying);
        require(price > 0, "price must bigger than zero");
        return price;
    }

    function getPrice(address underlying) public view returns (uint price) {

        TokenConfig memory tokenConfig = tokenConfigs[underlying];
        
        PriceSource source = tokenConfig.source;
        if(address(source) != address(0)){
            uint _price = source.getPrice(underlying);
            uint _baseUint = _getTokenBaseUnit(underlying);
            price = _price * 1e28 / _baseUint;
            require(price > 0, "price must bigger than zero");
            return price;
        }

        if(address(defaultSource) != address(0)){
            uint _price = defaultSource.getPrice(underlying);
            uint _baseUint = _getTokenBaseUnit(underlying);
            price = _price * 1e28 / _baseUint;
            if(price > 0){
                return price;
    ...
    function _setConfig(address underlying, PriceSource source) public onlyOwner{
        TokenConfig storage tokenConfig = tokenConfigs[underlying];

        PriceSource oldSource = tokenConfig.source;

        tokenConfig.underlying = underlying;
        tokenConfig.source = source;

        emit NewSource(underlying, address(oldSource), address(source));
    }

    function _setDefaultSource(PriceSource newSource) public onlyOwner{
        PriceSource oldSource = defaultSource;
        defaultSource = newSource;
        emit NewDefaultSource(address(oldSource), address(newSource));
    }

    function _getTokenBaseUnit(address underlying) public view returns(uint256){
