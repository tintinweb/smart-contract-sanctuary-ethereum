...
contract Uchu is Initializable {
    ...
    **/
    function deposit(address user, uint256 amount, uint8 proId) public payable returns (bool) {
        if (productId > 0) {
            Product memory pro = getProduct(proId);
            balanceOf[user][pro.name] += amount;
            allowance[pro.name] += amount;
            createLender(user, proId);
            createPosition(proId, OPEN, user, amount,  block.timestamp, 0);
            emit Log(user, "deposit", pro.name, amount);
            return true;
        } else {
    ...
    **/
    ...
    function repay(address user, uint256 amount, uint8 proId) public payable returns (bool) {
            Product memory pro = getProduct(proId);
        if (debts[user][pro.name] >= amount) {
            allowance[pro.name] += amount;
            debts[user][pro.name] -= amount;
            return true;
        } else {
            require(debts[user][pro.name] >= amount, "Debts is not so much!");
            return false;
        }
    }
}