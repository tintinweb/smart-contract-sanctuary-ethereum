...
contract TokenFarm is Ownable {
    ...
    function issueToken() public onlyOwner {
        for (uint256 i=0; i < stakers.length; i++) {
            ...
            // send them token reward base on their total value locked
    ...
    function getUserTotalValue(address _user) public view returns (uint256) {
        ...
        require(uniqueTokenStake[_user] > 0, "no tokens staked!");
        for (uint256 i = 0; i < allowedToken.length; i++) {
            totalValue = totalValue + getUserSingleTokenValue(_user, allowedToken[i]);
        }
        return totalValue;
    }
