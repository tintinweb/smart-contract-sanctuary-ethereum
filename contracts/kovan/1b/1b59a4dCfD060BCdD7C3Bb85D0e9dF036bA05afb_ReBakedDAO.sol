...
/**
	...
	*/
	 ...
	 onlyInitiator(projectId_)
	{
		...
		for(uint i = 0; i < observer_.length; i++){
			...
			uint256 amount = packageData[projectId_][packageId_].budget - packageData[projectId_][packageId_].budgetAllocated;
			uint256 temp = uint(amount*5/100);
			_revertPackageBudget(projectId_,amount-temp);
	...
	*/
		...
		external
	{
		address initiator_ = projectData[projectId_].initiator;		
		if(packageStatus == true || msg.sender == initiator_){
			payObserverFee(projectId_,packageId_,observer_);
			observerData[projectId_][packageId_][observer_].isRemoved = true;
		} else{
			observerData[projectId_][packageId_][observer_].timeCreated = 0;
			packageData[projectId_][packageId_].totalObservers -= 1;
		}
	}
...
interface IReBakedDAO {
	...
	*/
	...
	function canclePackage(
		bytes32 projectId_, bytes32 packageId_,address[] calldata collaborator_,address[] calldata observer_
	) external ;
	...
	*/
	...
	function removeObserver(bytes32 projectId_, bytes32 packageId_, address observer_,bool packageStatus) external;
