...
// File: contracts/FixedPointPriceMarketplace.sol
...
contract NFTFixedPoint is ERC721URIStorage {
    ...
    // enum  Auction_State  {Start,Runing,Ended,Cancelled} 
    ...
    function createNFT(address to,string memory _tokenURI) public payable returns(uint256 id){
    ...
    // }
     ...
     modifier onlyOwner(uint256 tokenId) {
        require(ownerOf(tokenId) == msg.sender,"only NFT owner set put on sale");
        _;
    }
    
    //Put on sale minting NFT
    function putOnSale(uint256 _tokenId) onlyOwner(_tokenId) public returns(bool) {
            // require(onSale[_tokenId] == _tokenId,"Created NFt should be on the Put on sale");
    ...
    function buyFixedPriceMarketNFT(uint256 _id) public payable returns(bool) {
        ...
        require(msg.value >= onSale[_id].price,"send fixed price amount of NFT");
      ...
      function onFixedPrice(uint256 _id)public returns(bool){
        ...
        require(onSale[_id].buyer!=(address(0)),"Buyer address should not 0 & should be correct");
    ...
    // modifier started(){
    //     require(block.number > openingTime);
    //     _;
    // }

    // modifier beforeClosing(){
    //     require(block.number < closingTime);
    //     _;
    // }
    ...
    // function onBid() public payable notOwner started beforeClosing {
        
    //       auctionState = Auction_State.Runing;
    //       require(msg.value > 1 ether );

    //       uint256 currentBid;
    //       currentBid = bids[msg.sender] + bidInc;
