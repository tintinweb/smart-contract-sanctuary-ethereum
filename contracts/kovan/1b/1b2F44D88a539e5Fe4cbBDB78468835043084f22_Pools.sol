...
contract Pools is KayiAave {
    ...
    function getUserBalance(address _user, address _token)
        ...
        returns (uint256)
    {
        return _userBalance[_token][_user];
    }
    ...
    function _depositTokens(address _token, uint256 _amount) internal {
        ...
        _userBalance[_token][msg.sender] += _amount;
        emit updatedUserBalance(
            msg.sender,
            _token,
            _userBalance[_token][msg.sender]
        );
    }
...
contract KayiAave {
    ...
    function getAaveLiquidityRate(address _token) public view returns (uint256) {
        IProtocolDataProvider protocolDataProvider = _getProtocolDataProvider();
        uint256 liquidityRate;
        (, , , liquidityRate, , , , , , ) = protocolDataProvider.getReserveData(
            _token
        );
    ...
    function _getProtocolDataProvider()
        private
        view
        returns (IProtocolDataProvider)
    {
        return
            IProtocolDataProvider(
                _lendingPoolAddressProvider.getAddress(
                    keccak256(abi.encodePacked(uint256(0x1)))
                )
            );
    }
}
