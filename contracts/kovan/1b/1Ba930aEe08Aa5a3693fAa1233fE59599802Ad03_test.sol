...
contract test is FlashLoanReceiverBaseV1 {
    ...
    function executeArbitrage(address fromToken, uint256 amountIn, address[] calldata pools) public override returns (uint256){
        require(amountIn <= getBalanceInternal(address(this), fromToken), "Invalid balance, was the flashLoan successful?");

        uint256 fee = 3;
        uint256 one = 1000;
        uint256 _fee = fee.div(one);
        uint totalDebt = amountIn.add(_fee);





        transferFundsBackToPoolInternal(fromToken, totalDebt);
...
abstract contract FlashLoanReceiverBaseV1 is Withdrawable,IPulsarArbitrage {
