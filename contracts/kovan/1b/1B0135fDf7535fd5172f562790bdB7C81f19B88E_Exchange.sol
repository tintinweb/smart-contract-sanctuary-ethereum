    ...
    MixinSignatureValidator
{
        ...
        internal
    {
        ...
        if (feesManager != address(0)) {
            uint256 assetId = _getMakerAssetId(order.makerAssetData);
            address tokenAddress = _getTokenAddress(order.takerAssetData);
            IFeesManager(feesManager).receiveFees(assetId, order.takerAddress, tokenAddress, fillResults.takerFeePaid);
        }
