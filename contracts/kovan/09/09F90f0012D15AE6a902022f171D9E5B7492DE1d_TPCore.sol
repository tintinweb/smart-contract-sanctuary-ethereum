...
contract TPCore is AccessControl {
    ...
    // 48 bit minimum
    uint48 private startingTpyRewardPerInterval = 10 * (10**8);//1027 * 7 * 4 * (10**8); // per day * 4 week
    
    // 32 (max 1 year)
    uint32 public tpyInterval = 5 minutes;
    ...
    function emergencyUnstake() external {
        ...
        intervalUnstakes[nextInterval()] += staking.amount + staking.pendingAmount;
        staking.amount = 0;
    ...
    function stakingAmount(address lpProvider) external view returns(uint104) {
        return userStaking[lpProvider].amount + userStaking[lpProvider].pendingAmount;
    }
    ...
    function estimateRewardDetails(address lpProvider) public view returns (uint104 gross, uint104 net, uint104 profit, uint104 fee) {
        ...
        uint104 userStakingAmount = userStaking[lpProvider].amount + userStaking[lpProvider].pendingAmount;
    ...
    function totalStakedUntilInterval(uint256 finalInterval) public view returns(uint256) {
        uint256 total;
        for(uint256 i = 1; i <= finalInterval; i++){
    ...
    function estimateTPYReward(address addr) public view returns(uint136) {
        ...
        estimatedTPYs += staking.calculatedTPYs;
            ...
            if(staking.pendingAmount == 0){
                if(lastCompleteInterval >= staking.intervalNumber) {
                    stakedUntilInterval = totalStakedUntilInterval(staking.intervalNumber);
                ...
                if(staking.intervalNumber -1 <= lastCompleteInterval) {
                    stakedUntilInterval = totalStakedUntilInterval(staking.intervalNumber -1);
                    estimatedTPYs += staking.amount * getTPYIntervalReward(staking.intervalNumber - 1) / stakedUntilInterval;
                }
                ...
                for(uint256 i = staking.intervalNumber; i <= lastCompleteInterval; i++) {
                    ...
                    estimatedTPYs += (staking.amount + staking.pendingAmount) * getTPYIntervalReward(i) / stakedUntilInterval;
                }
            }
    ...
    function registerTPY(address staker, uint104 stakingCoinAmount) private {
        ...
        if(userStaking[staker].intervalNumber > 0) {
            ...
            userStaking[staker] = Staking(nextIntervalNumber, stakingCoinAmount, 0, 0);
        }
    ...
    function takeTPYReward(uint104 unstakeCoinAmount) private {
        ...
        uint136 tpyToTransfer = uint136(unstakeCoinAmount * tpyRewards / (staking.amount + staking.pendingAmount));
        ...
        if( (staking.intervalNumber <= currentIntervalNumber) || (pending > 0 && staking.pendingAmount == 0) ) {
