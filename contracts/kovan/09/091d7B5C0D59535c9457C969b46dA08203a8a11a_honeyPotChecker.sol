...
contract honeyPotChecker{
    ...
    //receive() external payable {
        //assert(msg.sender == WETH); // only accept ETH via fallback from the WETH contract
    //}
    function honeyTestComplex(address[] calldata path,address[] calldata factory,address payable router) external virtual returns(bool,string memory){
        bool isSafe = false;
        string memory raison = "";
            ...
            try trades(router).swapExactTokensForTokensSupportingFeeOnTransferTokens(ERC20(path[0]).balanceOf(router),0,path,factory){
                ...
                if(ERC20(path[path.length-1]).balanceOf(router)>0){
