...
contract Flopper {
    ...
    struct Bid {
        ...
        uint256  tic;  // bid expiry time         [unix epoch time]
        uint256  end;  // auction expiry time     [unix epoch time]
    }
    ...
    // uint256   public   ttl = 3 hours;  // 3 hours bid duration         [seconds]
    uint256   public   ttl = 30 seconds;  // 3 hours bid duration         [seconds]
    // uint256   public   tau = 2 days;   // 2 days total auction length  [seconds]
    uint256   public   tau = 2 minutes;   // 2 days total auction length  [seconds]
    ...
    function file(bytes32 what, uint data) external auth {
        ...
        else if (what == "ttl") ttl = uint256(data);
        else if (what == "tau") tau = uint256(data);
    ...
    function kick(address gal, uint lot, uint bid) external auth returns (uint id) {
        ...
        bids[id].lot = lot;
        bids[id].guy = gal;
        bids[id].end = add(uint256(block.timestamp), tau);
    ...
    function dent(uint id, uint lot, uint bid) external {
        ...
        bids[id].lot = lot;
        bids[id].tic = add(uint256(block.timestamp), ttl);
    }
