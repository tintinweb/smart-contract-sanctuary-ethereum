...
contract FixlineLossCap{
	...
	uint256 capDiv = 1;                                 // 1% to CAPs stakers
	uint256 fxlDiv = 1;                                 // 1% to FXL
	...
	address public CAPs_staking_Pool_address;            // Put CAPsStakingPool contract address
    ...
    */
    ...
    // function DepositAndMakeBothSidesBet(uint32 apiID, uint256 betID, bool fave, uint256 betAmt, uint256 toWinFave, uint256 toWinDog, uint256 numSpots, uint256 expiry) public payable
	function DepositAndMakeBothSidesBet(uint32 apiID, uint256 betID, bool fave, uint256 betAmt, uint256 numSpots, uint256 expiry) public payable {
		...
		expireTime[betID] = block.timestamp + expiry;
	...
	// 	require(msg.sender == admin, "You are not the admin");
    //     require(block.timestamp > cliff,"You cannot call this function within 10 days");
    //     IPool(CAPs_staking_Pool_address).notifyReward{value : accumulatedCapsFees}();
	...
	function creditMyProfile(uint256[] memory betIds) public {   //Transfers amount to maker if no. of takers is less than the selected spot
		for(uint i=0; i < betIds.length; i++) {
			require(spotsTaken[betIds[i]] < spots[betIds[i]], "Spots are filled");
			require(block.timestamp > expireTime[betIds[i]], "Bet is not expired yet");
			require(msg.sender == maker[betIds[i]], "You are not the maker of the bet");

			balances[msg.sender] = (balances[maker[betIds[i]]] - (amount[betIds[i]] * spotsTaken[betIds[i]]));
		}
	}
