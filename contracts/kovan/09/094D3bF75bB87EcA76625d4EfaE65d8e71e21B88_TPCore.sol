...
contract TPCore is AccessControl {
    ...
    // 48 bit minimum
    uint48 private startingTpyRewardPerInterval = 12 * (10**8);//1027 * 7 * 4 * (10**8); // per day * 4 week
    
    // 32 (max 1 year)
    uint32 public tpyInterval = 1 minutes;
    ...
    function emergencyUnstake() external {
        ...
        intervalUnstakes[nextInterval()] += staking.amount;
        staking.amount = 0;
    ...
    function totalStakedForTPYInterval() external view returns(uint256) {
        return intervalStakes[nextInterval()];
    }
    ...
    function estimateRewardDetails(address lpProvider) public view returns (uint104 gross, uint104 net, uint104 profit, uint104 fee) {
        ...
        uint104 userStakingAmount = userStaking[lpProvider].amount;
        require(gross >= userStakingAmount, "gross must be greater than amount");
    ...
    function currentIntervalTPYs(address lpProvider) external view returns (uint256) {
        ...
        uint256 intervalTotalStakingAmount = totalStakedUntilInterval(interval);

        if (intervalTotalStakingAmount > 0){
            uint256 intervalTotalReward = getTPYIntervalReward(interval);
            return userStaking[lpProvider].amount * intervalTotalReward / intervalTotalStakingAmount;
        }
        else {
            return 0;
        }
    ...
    function takeTPYReward(uint104 unstakeCoinAmount) private {
        ...
        intervalUnstakes[nextInterval()] += unstakeCoinAmount;
    ...
    function totalStakedUntilInterval(uint256 finalInterval) public view returns(uint256) {
        ...
        for(uint256 i = 0; i <= finalInterval; i++){
            total += intervalStakes[i];
            total -= intervalUnstakes[i];
        }
        return total;
    }
    ...
    function estimateTPYReward(address addr) public view returns(uint136) {
        ...
        uint256 stakedUntilInterval = totalStakedUntilInterval(staking.intervalNumber);
        if(stakedUntilInterval > 0) {
                estimatedTPYs += staking.amount * getTPYIntervalReward(staking.intervalNumber) / stakedUntilInterval;
            }
        
        for(uint256 i = staking.intervalNumber + 1; i <= lastCompleteInterval; i++) {
