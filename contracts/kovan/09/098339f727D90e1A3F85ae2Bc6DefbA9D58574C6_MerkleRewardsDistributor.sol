...
contract MerkleRewardsDistributor is Ownable, ReentrancyGuard {
    ...
    event MerkleProofURIUpdated(bytes32 _newMerkleURI);
    ...
    function setRoot(bytes32 _merkleRoot, bytes32 _newMerkleURI)
        external
        onlyOwner
    {
        merkleRoot = _merkleRoot;
        emit MerkleProofURIUpdated(_newMerkleURI);
    }
    ...
    function endEpoch(
        uint256 etherAttributed,
        address[] calldata tokensAttributed,
        uint256[] calldata amountsAttributed
    ) external onlyOwner {
        require(etherAttributed >= 0);
        if (etherAttributed > 0) {
            _currentEtherToSwap = etherAttributed;
        }
    ...
    function swapMultiple(
        ...
        uint256 amountsOutMinLen = _amountsOutMin.length;

        require(
            msg.value > 0
                ? tokensToBeSwappedLen == amountsOutMinLen - 1
                : tokensToBeSwappedLen == amountsOutMinLen,
            "Number of input tokens should be the same as number of output amount of tokens"
        );

        uint256 outCounter = 0;

        if (msg.value > 0) {
            // require(msg.value >= _currentEtherToSwap, "msg.value not enough");
            address[] memory wethPath = new address[](2);

            wethPath[0] = _wethContractAddress;
            wethPath[1] = _xyzContractAddress;
            uint256[] memory amounts = routerContract.swapExactETHForTokens{
                value: msg.value
            }(_amountsOutMin[0], wethPath, address(this), block.timestamp + 15);
        ...
        for (uint256 i = 0; i < tokensToBeSwappedLen; ++i) {
            address currentTokenToSwap = _tokensIn[i];
            path[0] = currentTokenToSwap;

            uint256 amountToSwap = totalTokensAttributed[currentTokenToSwap];

            require(
                amountToSwap >= 0,
                "Not enough amount attributed"
            );
            IERC20(currentTokenToSwap).approve(address(routerContract), amountToSwap);
            _swap(
                amountToSwap,
                _amountsOutMin[outCounter],
                path
            );
            ++outCounter;
        }
