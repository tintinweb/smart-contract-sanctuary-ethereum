...
contract JobFactory is ProxyFactory {
     ...
     * @param client    the address of the client (should be swapped with msg.sender?)
     * @return          success  
     * TODO:         
            Should require that applicants have created an account. 
            Import a IProfile interface and validate they have a user
    */
    function createJob(address logic, address client, string calldata ipfsHash) external {

        bytes memory payload = abi.encodeWithSignature("initialize(address, ipfsHash)", client, ipfsHash);

        // Deploy minimal proxy
        address jobAddress = deployMinimal(logic, payload);
        getJob[jobAddress] = client;
