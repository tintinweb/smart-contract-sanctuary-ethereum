...
contract HRF is ERC721, ERC721Metadata, ERC173, String {
    ...
    function tokenURI(uint256 _tokenId) public view virtual override returns (string memory) {
        if (_reveal == true) {
            if (_baseLevel[_tokenId] == true) {
                return string(abi.encodePacked(_baseUri(), _extendedBaseUri, "/", toString(_tokenId), ".", _fileExtension));
            } else {
                return string(abi.encodePacked(_baseUri(), _level[_tokenId]));
            }
        } else {
            return string(abi.encodePacked(_baseUri(), "bafybeieuawcxjiesamn6p776hpaxo6nzzmwsm6lblveamqvtr4emauxl6e/prereveal.gif"));
        }
    }
    ...
    function lockRevealBase() public ownership {
        require(_baseSet == true, "HRF: Reveal base not set");
        _baseLock = true;
    }

    function setRevealBase(string memory _cid, string memory _fileType) public ownership {
        require(_baseLock == false, "HRF: Already revealed");
        _extendedBaseUri = _cid;
        _fileExtension = _fileType;
        _baseSet = true;
    }
