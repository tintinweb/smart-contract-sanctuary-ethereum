...
/// @title $TKD Staking Contract
/// @notice The contract is used to manage staking $tkd tokens.
contract tkdStaking is Ownable {
    ...
    uint256[3] public durations = [300 seconds, 600 seconds, 900 seconds]; // lock durations
    uint256[3] public apyBonus = [180000, 240000, 300000];
    ...
    function withdraw(uint256 index) public {
        ...
        _token.transfer(msg.sender, userStakeInfo.amount);
        _token.transferFrom(owner(), msg.sender, remaingReward);
        ...
        returns (uint256 reward)
    {
        ...
        if (block.timestamp < userStakeInfo.unlockTime) {
            uint256 lockQuarterTS = userStakeInfo.stakeTime.add(
                durations[userStakeInfo.plan].div(4)
            );

            uint256 rewardDurationInDays = block.timestamp > lockQuarterTS
                ? block.timestamp.sub(lockQuarterTS).div(DAY)
                : 0;
    ...
    function realtimeReward(address user)
        public
        view
        returns (uint256 realReward)
    {
        ...
        for (uint256 i = 1; i <= _user.stakeCount; i++) {
            if (!_user.stakingInfo[i].unstaked) {
                ...
                if (currentReward < _user.stakingInfo[i].reward) {
                    realReward += currentReward;
                } else {
                    realReward += _user.stakingInfo[i].reward;
                }
            }
        }
    }
    ...
    function getUserInfo(address _usr, uint256 index)
        public
        view
        returns (uint256[5] memory arrData, bool unstaked)
    {
        arrData = [
