...
contract ERC20NFTBond is NFTBond {
    ...
    external initializer {
        ...
        __CreditDelegatorBondGranter_init(_poolAddressesProvider);
    }
    ...
    external whenNotPaused returns (uint256) {
        return super._buyBond(tokenUri, beneficiary, maturity, principal, nftHash, setApprove, nonce, signature, typeOfBond);
    }
    ...
    internal override {
        super._beforeBondPurchased(tokenUri, beneficiary, maturity, principal, typeOfBond);
        if (typeOfBond == 1) {
            _borrow(principal, 1, _principalTokenAddress());
        } else if (typeOfBond == 2) {
            _borrow(principal, 2, _principalTokenAddress());
        } else {
    ...
    internal override {
        ...
        if (principalToken.balanceOf(address(this)) < amount) {
            ...
            if (bond.typeOfBond == 1) {
                _repay(principalToken.balanceOf(address(this)), 1, _principalTokenAddress());
            } else if (bond.typeOfBond == 2) {
                _repay(principalToken.balanceOf(address(this)), 2, _principalTokenAddress());
            } else {
            ...
            if (bond.typeOfBond == 1) {
                _repay(amount, 1, _principalTokenAddress());
            } else if (bond.typeOfBond == 2) {
                _repay(amount, 2, _principalTokenAddress());
            } else {
...
import "../bond/CreditDelegatorBondGranter.sol";
 ...
 * @author Ethichub
 */
abstract contract NFTBond is NFT, CollateralizedBondGranter, LockedBondGranter, CreditDelegatorBondGranter, LiquidityRequester, PausableUpgradeable {
    ...
    internal returns (uint256) {
        ...
        _beforeBondPurchased(tokenUri, beneficiary, maturity, principal, typeOfBond);
        uint256 tokenId = _safeMintBySig(tokenUri, beneficiary, nftHash, setApprove, nonce, signature);
        super._issueBond(tokenId, maturity, principal, typeOfBond);
    ...
    function _issueBond(
        ...
        uint256 principal,
        uint8 typeOfBond
    ) internal virtual override (BondGranter, CollateralizedBondGranter, CreditDelegatorBondGranter) {}

    /**
     * @dev Returns the amount that corresponds to the bond
     */
    function _redeemBond(uint256 tokenId) internal virtual override (BondGranter, CollateralizedBondGranter, LockedBondGranter) returns (uint256) {
...
abstract contract CollateralizedBondGranter is BondGranter {
     ...
     * @param typeOfBond uint8 (0: normal, 1: credit delegation, ...)
     *
     * Requirement:
     *
...
abstract contract CreditDelegatorBondGranter is BondGranter {
    ...
    event RepayedCredit(address indexed asset, address indexed delegator, uint256 amount, uint256 interestRateMode);
    ...
    function __CreditDelegatorBondGranter_init(
      address _poolAddressesProvider
    )
    internal initializer {
    ...
    function getAmountDelegated(address stableDebtTokenAddress, address delegator, address delegatee)
      external view returns (uint256 amount) {
      return ICreditDelegationToken(stableDebtTokenAddress).borrowAllowance(delegator, delegatee);
    }

    function _borrow(uint256 amount, uint256 interestRateMode, address assetToBorrow) correctInterestRateMode(interestRateMode) internal {
      pool.borrow(assetToBorrow, amount, interestRateMode, REFERRAL_CODE, msg.sender);
      emit DelegatedCredit(assetToBorrow, msg.sender, amount, interestRateMode);
    }   

    function _repay(uint256 amount, uint256 interestRateMode, address assetToRepay) 
      correctInterestRateMode(interestRateMode)
      internal {
      pool.repay(assetToRepay, amount, interestRateMode, msg.sender);
      emit RepayedCredit(assetToRepay, msg.sender, amount, interestRateMode);
    }
    ...
    function _issueBond(
        ...
        // TODO: mirar si tiene el suficiente supply para cubrir ese principal
        super._issueBond(tokenId, maturity, principal, typeOfBond);
    }
...
abstract contract BondGranter is Initializable, InterestParameters, InterestCalculator {
     ...
     * @param maturity uint256 seconds
     * @param principal uint256 in wei
     * @param typeOfBond uint8 (0: normal, 1: stable debt credit delegation, 2: variable debt credit delegation, ...)
     *
     * Requirements:
     *
     ...
     */
    function _issueBond(uint256 tokenId, uint256 maturity, uint256 principal, uint8 typeOfBond) internal virtual {
        ...
        bonds[tokenId] = Bond(block.timestamp, maturity, principal, interest, false, typeOfBond);
