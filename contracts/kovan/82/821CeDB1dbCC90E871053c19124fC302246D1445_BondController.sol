...
contract BondController is IBondController, OwnableUpgradeable, KeeperCompatibleInterface {
    ...
    uint256 public constant RISK_FACTOR_GRANULARITY = 1000;
    ...
    uint256 public constant MAX_RISK_FACTOR = 1000000;
    ...
    uint256 public override riskFactor;
    ...
    function init(
        ...
        for (uint256 i = 0; i < _tranches.length; i++) {
            ...
            address iouTokenAddress = IIouFactory(_iouFactory).createIou(
                string(abi.encodePacked("HourGlass [", IERC20Metadata(address(_tranches[i].token)).name(), "]-PRIME")),
                string(abi.encodePacked(IERC20Metadata(address(_tranches[i].token)).symbol(), "-PRIME")),
                address(_tranches[i].token)
            );
        ...
        feeBps = MAX_FEE_BPS;
        riskFactor = RISK_FACTOR_GRANULARITY;
    }
    ...
    function mature() external override {
        ...
        interestRate = INTEREST_RATE_GRANULARITY * RISK_FACTOR_GRANULARITY * tokenSupplyAtCreation;
        uint256 tokenSupplyAtMaturity = IERC20(token).totalSupply();
        if (tokenSupplyAtMaturity < tokenSupplyAtCreation) {
          interestRate /= RISK_FACTOR_GRANULARITY * tokenSupplyAtCreation + riskFactor * (tokenSupplyAtCreation - tokenSupplyAtMaturity);
        } else {
          interestRate /= RISK_FACTOR_GRANULARITY * tokenSupplyAtCreation + riskFactor * (tokenSupplyAtMaturity - tokenSupplyAtCreation);
        }
     ...
     * @inheritdoc IBondController
     */
    function setRiskFactor(uint256 newRiskFactor) external override onlyOwner {
        require(!isMature, "BondController: Invalid call to setRiskFactor");
        require(newRiskFactor <= MAX_RISK_FACTOR, "BondController: New risk factor too high");
        riskFactor = newRiskFactor;

        emit RiskFactorUpdate(newRiskFactor);
    }
}
...
interface IBondController {
    ...
    event RiskFactorUpdate(uint256 newRiskFactor);
    ...
    function riskFactor() external view returns (uint256 fee);
    ...
    /**
     * @dev Updates the risk factor to the given new risk factor
     *
     * Requirements
     * - `msg.sender` has admin role
     * - `newRiskFactor` is in range [0, MAX_RISK_FACTOR]
     */
    function setRiskFactor(uint256 newRiskFactor) external;
}
