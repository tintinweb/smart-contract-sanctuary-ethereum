...
contract FlowerFamOasis is SimpleAccessUpgradable {
        ...
        external
    {
        ...
        uint256 canClaimAmount = _getTotalProductionOfUser();
        uint256 lastClaimedBonus = _getLastClaimedBonusPercentage();
        ...
        // if (toWallet) {
        //     honeyCoinReserve.releaseFunds(claimAmount, msg.sender);
        // } else {
        //     flowerFamEcoSystem.updateTotalAccumulatedProductionOfUser(
        //         msg.sender,
        //         claimAmount
    ...
    //function cliamUnclaimedReward(bool toWallet) external {
    //    uint256 claimAmount = _getTotalProductionOfUser();
    //    uint256 lastClaimedBonus = _getLastClaimedBonusPercentage();
//
    //    UserOasisProduction storage userOasisProduction = userToProductionInfo[
    //        msg.sender
...
//
    ...
    //    userOasisProduction.lastAction = uint32(block.timestamp);
//
    //    claimAmount += (claimAmount * lastClaimedBonus) / 100;
//
    //    if (toWallet) {
    //        honeyCoinReserve.releaseFunds(claimAmount, msg.sender);
    //    } else {
    ...
    //    }
//
    //    emit ClaimUnclaimedAmount(msg.sender, claimAmount, lastClaimedBonus, toWallet);
    //}

    function getUserLandsCount() external view returns (uint256) {
        uint256 count = 0;
        for (uint256 i = 0; i < oasisLandIds.length; i++) {
            count += userToLandSpots[msg.sender][oasisLandIds[i]];
        }
        return count;
    }

    function getUserLandDetail(uint256 landId)
        external
        view
        returns (UserLandInfo memory)
    {
        ...
        userLand.landYield = _getUserLandYield(landId);
        userLand.landEarnings = _getUserLandEarnings(landId);
        userLand.landCount = userToLandSpots[msg.sender][landId];

        return userLand;
    }

    function getTotalProductionOfUser() external view returns (uint256) {
        uint256 claimAmount = _getTotalProductionOfUser();        
        uint256 lastClaimedBonus = _getLastClaimedBonusPercentage();
        return claimAmount + (claimAmount * lastClaimedBonus) / 100;
    }

    function getUserLands() external view returns (UserLandInfo[] memory) {
        UserLandInfo[] memory lands = new UserLandInfo[](oasisLandIds.length);

        for (uint256 i = 0; i < oasisLandIds.length; i++) {
            uint256 landId = oasisLandIds[i];
            uint256 count = userToLandSpots[msg.sender][landId];
            if (count > 0) {
                lands[i] = userLands[landId];
                lands[i].landCount = count;
                lands[i].landYield = _getUserLandYield(landId);
                lands[i].landEarnings = _getUserLandEarnings(landId);
            }
        }
    ...
    function _getStakedAssetReward() internal view returns (uint256) {
        uint256 count = userAssetsCount[msg.sender];
        if (count == 0) return 0;
    ...
    function _getLastClaimedBonusPercentage() internal view returns (uint256) {
        uint256 userLastInteractionDiff = (block.timestamp -
            userToLastClaimInteraction[msg.sender]) / 1 days;
    ...
    function _getTotalProductionPerDay() internal view returns (uint256) {
        uint256 amount = 0;
        ...
        for (uint256 i = 0; i < oasisLandIds.length; i++) {
            uint256 landId = oasisLandIds[i];
            uint256 landCount = userToLandSpots[msg.sender][landId];
            if (landCount > 0) {
        ...
        if (userAssetsCount[msg.sender] > 0) {
            // Adding staked asset reward
            rewardAmount += (amount * _getStakedAssetReward()) / 1000;
        }
    ...
    function _getTotalProductionOfUser() internal view returns (uint256) {
    ...
    function _updateUserOasisProduction() internal {
        UserOasisProduction storage userOasisProduction = userToProductionInfo[
            msg.sender
        ];
        ...
        userOasisProduction.totalProductionPerDay = uint112(
            _getTotalProductionPerDay()
        );
    }

    // VIEW functions

    function _getUserLandYield(uint256 landId) internal view returns (uint256) {
        uint256 count = userToLandSpots[msg.sender][landId];
        return uint256(count * oasisLands[landId].reward);
    }

    function _getUserLandEarnings(uint256 landId)
        internal
        view
        returns (uint256)
    {
        uint256 totalReward = 0;
        uint256 yield = _getUserLandYield(landId);
        uint256 count = userToLandSpots[msg.sender][landId];

        totalReward += (yield * _getStakedAssetReward()) / 1000;
