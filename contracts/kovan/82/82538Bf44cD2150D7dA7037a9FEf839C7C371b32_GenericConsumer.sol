...
import { SpecEntry, SpecEntryLibrary } from "./SpecEntryLibrary.sol";
    ...
    KeeperCompatible
{
    ...
    using SpecEntryLibrary for SpecEntryLibrary.Map;
    ...
    SpecEntryLibrary.Map private s_specEntries;
    ...
    event ExternalRequestFulfillFailed(
        address indexed callbackAddr,
        bytes32 indexed requestId,
        bytes4 indexed callbackFunctionSignature,
        bytes data
    ...
    fallback() external whenNotPaused nonReentrant {
        ...
        if (callbackAddr == address(0)) {
            emit RequestFulfilled(requestId, callbackFunctionSignature, data);
        } else {
    ...
    function getSpecEntriesNextRoundPayment() external view returns (uint256) {
        ...
        for (uint256 i = 0; i < specEntriesLength; ) {
            ...
            if (!_isScheduled(specEntry.startAt, specEntry.interval, s_lastRequestTimestamps[key], blockTimestamp))
                continue;
        ...
        whenNotPaused
    {
        ...
        _requireEqualLength(keysLength, _specEntries.length);
    ...
    function performUpkeep(bytes calldata _performData) external override whenNotPaused nonReentrant {
        ...
        for (uint256 i = 0; i < specEntryKeysLength; ) {
            ...
            _requireSpecEntryScheduled(key, specEntry.startAt, specEntry.interval, blockTimestamp);
            ...
            s_lastRequestTimestamps[key] = blockTimestamp;
            emit SpecEntryRequested(roundId, key, requestId, blockTimestamp);
        }
    ...
    function withdraw() external onlyOwner {
        withdraw(payable(msg.sender), LINK.balanceOf(address(this)));
    }
    ...
    function _fulfillExternalRequest(
        ...
        if (!success) {
            emit ExternalRequestFulfillFailed(_callbackAddr, _requestId, _callbackFunctionSignature, _data);
        } else {
    ...
    function _requireSpecEntryScheduled(
        ...
        if (!_isScheduled(_startAt, _interval, lastRequestTimestamp, _blockTimestamp)) {
