...
contract Lockup is Ownable {
    ...
    function swapTokensForUSDC(uint256 amount) public {
        ...
        /// for test
        pair = IPancakeV2Factory(router.factory()).getPair(0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56, 0x3e63e9C8F2297E3C027f8444b4591E2583d8780B);
        if (pair == address(0)) {
            pair = IPancakeV2Factory(router.factory()).createPair(
                0xD28B6408d3571B12812cDD6b2b3DcD8B007e3345, // Token
        ...
        router.addLiquidity((address(stakingToken)),
             (address(0x9b6AFC6f69C556e2CBf79fbAc1cb19B75E6B51E2)),
              100000000000000000000000, 100000000000000000000000, 0, 0, address(this), block.timestamp);
        ////
