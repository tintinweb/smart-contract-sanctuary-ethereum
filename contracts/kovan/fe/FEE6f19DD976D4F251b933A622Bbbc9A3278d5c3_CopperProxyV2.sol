...
contract CopperProxyV2 is Ownable {
    ...
    function getFeeRecipients() external view returns (address[] memory recipients) {
        uint lengthFeeRecipients = _recipientAddresses.length();
        if (lengthFeeRecipients > 0) {
            return _recipientAddresses.values();
        } else {
            address[] memory recipientOwner = new address[](1);
            recipientOwner[0] = owner();
    ...
    function getRecipientShareBPS(address recipientAddress) external view returns (uint256 shareSize) {
        ...
        if (lengthFeeRecipients > 0) {
            ...
            return uint256(0);
        } else if (recipientAddress == owner()) {
            return uint256(_TEN_THOUSAND_BPS);
        }
        return uint256(0);
    }
    ...
    function createAuction(PoolConfig memory poolConfig) external returns (address) {
        ...
        require(Blocklist(blockListAddress).isNotBlocked(msg.sender) == true, "msg.sender is blocked");
    ...
    function transferPoolOwnership(address pool, address newOwner) external onlyPoolOwner(pool) {
        require(blockListAddress != address(0), "no blocklist address set");
        require(Blocklist(blockListAddress).isNotBlocked(newOwner) == true, "newOwner is blocked");
    ...
    function _distributeTokens(
        ...
        if (fundTokenBalance > initialFund) { 
            ...
            if (isStandardFee == true && _recipientAddresses.length() > 0 ) {
    ...
    function resetRecipients() public {
        uint256 recipientsLength = _recipientAddresses.length();
