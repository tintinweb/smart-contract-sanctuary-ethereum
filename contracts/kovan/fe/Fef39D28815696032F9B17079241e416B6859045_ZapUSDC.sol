...
/*
    Zap is a ZapperFi's simplified version of zapper contract which will:
    1. use USDC to swap to target token
    2. make LP between USDC and target token
    3. add into MasterChef farm
*/

contract ZapUSDC is Ownable, ReentrancyGuard {
    ...
    address public constant usdcAddress = 0xa70005f5a46e693cfbEB0cd51Cd6a949cf5A85bF;
    /*================= VARIABLES ==================*/
    ...
    constructor(address _masterChef, address _swapRouter) {
        require(_masterChef != address(0), "Zap::constructor: Invalid masterChef address");
        require(_swapRouter != address(0), "Zap::constructor: Invalid swapRouter address");
    ...
    /// @notice Zap function to receive USDC, swap for opposing token,
    ...
    function zap(
        ...
        uint256 _usdcIn = _amount;

        address _lp = address(masterChef.lpToken(_info.pid));
        require(_lp != address(0), "Zap::zap: Invalid LP");

        // STEP 1: Swap or Mint token
        (uint256 _usdcAmtToSwap, uint256 _tokenAmtToAddLP) = swap(_lp, _usdcIn, address(_info.token));

        // STEP 2: Add liquditiy
        uint256 _usdcAmtToAddLP = _usdcIn - _usdcAmtToSwap;
        approveToken(address(_info.token), address(swapRouter), _tokenAmtToAddLP);
        (uint256 _tokenAmtUsedInLp, uint256 _usdcAmtUsedInLp, uint256 _liquidity) = swapRouter.addLiquidityETH{
            value: _usdcAmtToAddLP
        ...
        approveToken(_lp, address(masterChef), _liquidity);
        masterChef.deposit(_info.pid, _liquidity, msg.sender);
        ...
        if (_transferResidual) {
            ...
            if (_usdcAmtToAddLP > _usdcAmtUsedInLp) {
                Address.sendValue(payable(msg.sender), _usdcAmtToAddLP - _usdcAmtUsedInLp);
            }
        }

        emit Zapped(_zapId, _usdcIn, _liquidity);
    }

    /// @notice Swap internal function to swap from USDC to target Token with calculation
    /// for the appropriate amount of USDC will be used to swap in order to minimize dust
    /// @param _lp: address of the LP pair between USDC and Token
    ...
    function swap(
        ...
        uint256 _usdcIn,
        address _token
    ) internal returns (uint256 _usdcAmtToSwap, uint256 _tokenAmtReceived) {
        address _token0 = IUniswapV2Pair(_lp).token0();
        (uint256 _res0, uint256 _res1, ) = IUniswapV2Pair(_lp).getReserves();

        if (_token == _token0) {
            _usdcAmtToSwap = calculateSwapInAmount(_res1, _usdcIn);
        } else {
            _usdcAmtToSwap = calculateSwapInAmount(_res0, _usdcIn);
        }

        if (_usdcAmtToSwap <= 0) _usdcAmtToSwap = _usdcIn / 2;
        _tokenAmtReceived = doSwapUSDC(_token, _usdcAmtToSwap);
    }

    /// @notice Swap internal function to swap from ETH to target Token
    /// @param _toToken: address of target Token
    /// @param _usdcAmt: Amount of USDC input
    function doSwapUSDC(address _toToken, uint256 _usdcAmt) internal returns (uint256 _tokenReceived) {
        address[] memory _path = new address[](2);
        _path[0] = usdcAddress;
        _path[1] = _toToken;

        _tokenReceived = swapRouter.swapExactTokensForTokens(_usdcAmt, 1, _path, address(this), block.timestamp)[
            _path.length - 1
        ];

        require(_tokenReceived > 0, "Zap::doSwapUSDC: Error Swapping Tokens 2");
    }
    ...
    /// copied from zapper.fi contract. Assuming 0.3% swap fee
    ...
    function calculateSwapInAmount(uint256 _reserveIn, uint256 _tokenIn) internal pure returns (uint256) {
        return
            (Babylonian.sqrt(_reserveIn * ((_tokenIn * 3988000) + (_reserveIn * 3988009))) - (_reserveIn * 1997)) /
            1994;
    }
