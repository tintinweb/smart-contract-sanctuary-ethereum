...
contract Account {
    ...
    function push(address token, uint256 amt) external payable returns (uint256 _amt) {
        _amt = amt;

        if (token != Configure.ETH_ADDRESS) {
            IERC20 tokenContract = IERC20(token);
            _amt = _amt == type(uint256).max ? tokenContract.balanceOf(msg.sender) : _amt;
            tokenContract.safeTransferFrom(msg.sender, address(this), _amt);
        } else {
            require(msg.value == _amt, "BYDEFI: Invalid Ether Amount");
            _amt = msg.value;
        }
        emit Push(token, _amt);
    }
    ...
    function pull(
        ...
        IAuthCenter(authCenter).ensureAccountAccess(msg.sender);
        _amt = amt;
        bool retCall;
        if (token == Configure.ETH_ADDRESS) {
