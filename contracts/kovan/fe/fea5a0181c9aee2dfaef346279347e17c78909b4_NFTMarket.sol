...
contract NFTMarket is ReentrancyGuard {
    ...
    function createMarketItem(
      ...
      /// @notice fetch list of NFTS owned/bought by this user
        function fetchsoldItems(address _user ) public  view returns (MarketItem[] memory){
            uint itemCount = _itemIds.current(); //total number of items ever created
            ...
            for(uint i = 0; i < itemCount; i++){
                ...
                if(idMarketItem[i+1].seller == _user){
                    uint currentId = idMarketItem[i + 1].itemId;
            ...
            return (items); //return array of all sold items
