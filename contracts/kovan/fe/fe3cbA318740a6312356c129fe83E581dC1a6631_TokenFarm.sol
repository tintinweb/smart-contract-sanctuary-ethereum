...
contract TokenFarm is Ownable {
    ...
    function unStakeTokens(address _token, uint256 _amount) public {
        ...
        require(balance > 0, "Staking balance need to be more than 0");
        require(
            _amount <= balance,
            "Amount requested is bigger than staking balance!"
        );
    ...
    function issueTokens() public onlyOwner {
        // issue Dapp Token to all stakers
        for (
            uint256 stakersIndex = 0;
            stakersIndex < stakers.length;
            stakersIndex++
    ...
    function stakeTokens(address _token, uint256 _amount) public {
        require(_amount > 0);
        require(tokenIsAllowed(_token), "Token is not allow!");
