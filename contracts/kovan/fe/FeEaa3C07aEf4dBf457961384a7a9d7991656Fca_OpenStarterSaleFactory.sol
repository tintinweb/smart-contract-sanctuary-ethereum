...
import "./interfaces/IOpenStarterNft.sol";
import "./interfaces/IOpenStarterLibrary.sol";
...
contract OpenStarterSale {
    ...
    struct StageInfo {
        ...
        bool isLotteryMode;
        bool isPublicMode;
        bool isWhitelisted; // any stage can support whitelist, not only whitelisted mode
    ...
    mapping(uint256 => uint256[15]) public mintedCount; // minted nfts per stage
    ...
    string public externalNftInfo; // external nft info link
    ...
    modifier onlyDevOrCreatorOrFactory() {
        require(
    ...
    modifier onlyFactoryOrOwner() {
        require(
            factoryAddress == msg.sender ||
                openStarterLibrary.owner() == msg.sender
        );
        _;
    }

    function setSaleCreator(address _saleCreator) external onlyFactoryOrOwner {
        saleCreatorAddress = payable(_saleCreator);
    }

    function setAddressInfo(address _tokenAddress, address _fundingTokenAddress)
        external
        onlyDevOrFactory
    {
    ...
    ) external onlyDevOrCreatorOrFactory {
    ...
    function setCopyright(
        ...
        string calldata _externalNftInfo
    ) external onlyDevOrCreatorOrFactory {
        utility = _utility;
        copyright = _copyright;
        externalNftInfo = _externalNftInfo;
    }
    ...
    function addStageInfo(
        ...
        bool _isPublicMode,
        bool _isLotteryMode
    ) external onlyDevOrFactory returns (uint256) {
    ...
    function addWhitelistedAddresses(
        ...
        stages[_stageIndex].isWhitelisted = true;
    }

    function disableStageWhitelist(uint256 _stageIndex)
        external
        onlyDevOrCreatorOrFactory
    {
        stages[_stageIndex].isWhitelisted = false;
    }
    ...
    function invest(uint256 _buyCount) public payable {
        ...
        require(stageIndex < stages.length, "1");

        StageInfo memory stage = stages[stageIndex];

        // In Whitelist Mode, Whitelists are required
        // In Other Modes, Whitelists are optional

        if (stage.isWhitelistedMode || stage.isWhitelisted) {
            require(stages[stageIndex].whitelists[msg.sender], "2");
        }
        ...
        stages[stageIndex].sold = stage.sold.add(_buyCount);
        require(mintedNfts[msg.sender].length <= stage.maxPerWallet, "4");

        // In Whitelist or Public Mode, Maximum Counts = Tier Percentage of Stage Quantity

        if (stage.isWhitelistedMode || stage.isPublicMode) {
            mintedCount[stageIndex][tierIndex] = mintedCount[stageIndex][
                tierIndex
            ].add(_buyCount);
        ...
        if (stage.isLotteryMode) {
            ...
            require(stage.sold <= lotteryCounts, "5");
        }

        if (stage.isFreeMintingMode) {
            require(stage.sold <= stage.quantity, "6");
        } else {
            ...
            if (address(fundingTokenAddress) == openStarterLibrary.getWETH()) {
                require(msg.value == investAmount, "7");
            } else {
    ...
    function collectFundsRaised() external onlyDevOrCreatorOrFactory {
...
contract OpenStarterSaleFactory {
    ...
    function setLibraryAddress(address _newLibrary) public {
        require(openStarterLibrary.owner() == msg.sender, "1");
    ...
    struct SaleInfo {
        ...
        string copyright;
        string cover;
        string externalNftInfo; // only for nft contracts created by outside of OpenStarterNFT
    }
    ...
    function createSale(
        ...
        sale.setSaleCreator(_owner);
        sale.setAddressInfo(_nft, _fundingTokenAddress);
        sale.setGeneralInfo(
