...
contract PresalePublic is Context, ReentrancyGuard, VRFConsumerBase {
    ...
    function invest(uint256 payAmount) external payable timing nonReentrant initialized {
    ...
    function addLiquidity() external nonReentrant onlyPresaleCreator initialized {
        uint256 currentTime = block.timestamp;
        require(
    ...
    function withdrawTokens() external nonReentrant onlyPresaleCreator initialized {
        require(
            block.timestamp > generalInfo.closeTime &&
                intermediate.lpAddress == address(0) &&
                intermediate.raisedAmount < generalInfo.softCap
        ...
        uint256 amount = IERC20(generalInfo.tokenAddress).balanceOf(address(this));
        require(amount > 0);
        TransferHelper.safeTransfer(generalInfo.tokenAddress, generalInfo.creator, amount);
    }
    //-------------------------------------------------------------
