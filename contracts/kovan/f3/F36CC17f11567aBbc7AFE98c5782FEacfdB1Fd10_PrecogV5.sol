...
contract PrecogV5 is IPrecogV5 {
    ...
    mapping(address => Cycle[]) public override tradingCycles; //
    mapping(address => Cycle[]) public fundingCycles;
    ...
    mapping(address => mapping(uint16 => uint)) public totalUnitsTradingCycle;
    ...
    function getExistingTokens() external view override returns (address[] memory) {
        return existingTokens;
    }

    function getExistingTokenByIndex(uint index) external view override returns (address) {
    ...
    function updateCurrentTradingCycle(address token) public override {
        ...
        uint48 duration;
        uint48 profitCycle = core.getCycleConfiguration().tradingCycle;
    ...
    function calculateProfit(address token, address account) 
        ...
        returns (
            ...
            uint16 newInvestmentId,
            uint16 newProfitIdOf
        )
    {
        (profitOfAccount, newInvestmentId, newProfitIdOf) = _calculateProfit(token, account);
    }
        ...
        )
    {
        ...
        _lastProfitIdOf = lastProfitIdOf[_token][_account];
        ...
        if (_lastProfitIdOf < _updatedLatestCycle) {
            for (_investmentId; _investmentId < _investments.length; _investmentId++) {
                ...
                if (_lastProfitIdOf == _updatedLatestCycle && _updatedLatestCycle == _investments[_investmentId].idChanged) 
                    return (_profitOf, _investmentId, _lastProfitIdOf);
                (_nextInvestment, _lastAvailableProfitId) = PrecogV5Library._chooseLastAvailableTradingId(_investments, _investmentId, _updatedLatestCycle);
                for (_lastProfitIdOf; _lastProfitIdOf < _lastAvailableProfitId; _lastProfitIdOf++) {
                    _profitOf += PrecogV5Library._calculateProfitAtCycle(_tradingCycles[_lastProfitIdOf], _investments[_investmentId], totalUnitsTradingCycle[_token][_lastProfitIdOf], _profits[_lastProfitIdOf], _lastProfitIdOf);
                }
    ...
    function addLiquidityPool(address token) external onlyAdmin {
        ...
        lastFundingDuration[token] = core.getCycleConfiguration().fundingCycle;
        lastDefundingDuration[token] = core.getCycleConfiguration().defundingCycle;
    ...
    function _getTradingCycleByTimestamp(
        ...
        internal
        view
        returns (Cycle memory _currentProfitCycle)
    {
        Cycle[] memory _tradingCycles = tradingCycles[_token];
        ...
        _currentProfitCycle = _lastTradingCycle;
        return _currentProfitCycle;
    }

    

    function updateAdjustment(address token) external onlyPrecogCore {
        Cycle memory firstTradingCycle = tradingCycles[token][0];
        uint48 fundingDuration;
        uint48 defundingDuration;
        uint48 nextTimestamp;
        {
            uint48 nextFundingTimestamp = PrecogV5Library._calculateNextFundingTimestamp(firstTradingCycle, fundingDuration);
            timestampApplyNewFundingCycle[token] = nextFundingTimestamp;
            lastFundingDuration[token] = core.getCycleConfiguration().fundingCycle;
            nextTimestamp = nextFundingTimestamp;
        }
        {
            uint48 nextDefundingTimestamp = PrecogV5Library._calculateNextDefundingTimestamp(firstTradingCycle, defundingDuration);
            timestampApplyNewDefundingCycle[token] = nextDefundingTimestamp;
            lastDefundingDuration[token] = core.getCycleConfiguration().defundingCycle;
            if(nextTimestamp < nextDefundingTimestamp) {
                nextTimestamp = nextDefundingTimestamp;
            }
        }
        Cycle memory futureTradingcycle = _getTradingCycleByTimestamp(token, nextTimestamp);
        timestampApplyNewTradingCycle[token] = futureTradingcycle.endTime;
    }
    ...
    function withdraw(
        ...
        if(isBeforeFundingTime(token, msg.sender)) {
...
interface IPrecogV5 {
    ...
    function tradingCycles(address token, uint index) external view returns (uint16 id, uint48 startTime, uint48 endTime);
