...
contract ToucanCrosschainMessenger is ToucanCrosschainMessengerStorage, Router {
    ...
    constructor(address _xAppConnectionManager, uint256 _cooldownPeriod) {
     ...
     * @dev only owner can update the cooldown period in seconds
    ...
    function revertBridgeRequest(
        ...
        BridgeRequest memory request = requests[requestHash];
        require(request.requestType == BridgeRequestType.SENT, '!Message_Sent');
        // in future ToucanCrosschainMessenger can be enabled to bridge more requests type,
        // below check is added to make sure only 'MINT' type bridge request are reverted
        require(
            request.messageType == MessageTypes.MINT,
            '!Mint_Type_operation'
        );
        require(!request.isReverted, 'Already_Reverted');
        require(
            block.timestamp >= request.timestamp + cooldownPeriod,
            'cooldown period not over'
        );
        requests[requestHash].isReverted = true;
        IBridgeableToken(_token).bridgeMint(_bridger, _amount);
        emit BridgeRequestReverted(_bridger, requestHash);
    }
