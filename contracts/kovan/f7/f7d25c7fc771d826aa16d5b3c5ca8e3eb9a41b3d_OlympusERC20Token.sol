...
contract OlympusERC20Token is ERC20Permit, VaultOwned {
    ...
    event TreasuryTaxFee(uint256 _treasuryTaxFee);

    address public uniswapV2Pair;

    uint256 treasuryTaxFee = 2;
    ...
    function calculateTreasuryTaxFee(uint256 _amount) private view returns (uint256) {
        return _amount.div(100).mul(treasuryTaxFee);
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) override internal virtual {
        ...
        if (recipient == uniswapV2Pair) {
            uint256 treasuryTax = calculateTreasuryTaxFee(amount);
            uint256 burnTax = calculateBurnTaxFee(amount);
            amount = amount.sub(treasuryTax).sub(burnTax);
        }
    ...
    function setTreasuryTaxFee(uint256 _treasuryTaxFee) public onlyOwner() {
        treasuryTaxFee = _treasuryTaxFee;
        emit TreasuryTaxFee(_treasuryTaxFee);
    }
}