...
//Initializable, ContextUpgradeable, BaseModule, AuthorizationModule, PauseModule, MintModule, BurnModule, EnforcementModule, ValidationModule, MetaTxModule, SnapshotModule
//contract RapidTokenUpgradeable is Initializable, ERC20Upgradeable, ERC20BurnableUpgradeable, ERC20SnapshotUpgradeable, AccessControlUpgradeable, PausableUpgradeable, ERC20PermitUpgradeable, UUPSUpgradeable, ValidationModule {
contract RapidTokenUpgradeable is Initializable, ERC20Upgradeable, BaseModule, ERC20BurnableUpgradeable, ERC20SnapshotUpgradeable, AccessControlUpgradeable, ERC20PermitUpgradeable, UUPSUpgradeable, ValidationModule, EnforcementModule, PauseModule {
    ...
    /* INITIALIZE RAPID BOND TOKEN MODULE */
    /**
     * @dev initialize issuance of RapidToken and dependant functions
     *
     */
...
//        CalendarData memory calendarData
    ) initializer public {
        //        __ERC20_init(baseData.name, baseData.symbol);
        __Base_init(baseData.name, baseData.symbol, baseData.tokenId, baseData.terms, baseData.par_value,
            bondData.guarantor_identifier, bondData.bondholder_representative_identifier, bondData.maturity_date,
            bondData.interest_rate, bondData.currency, bondData.interest_schedule_format,
            bondData.interest_payment_date);
...
//    }
    ...
    function __Base_init(
        ...
        uint256 par_value_,
        string memory guarantor_identifier_,
        string memory bondholder_representative_identifier_,
        uint256 maturity_date_,
        uint256 interest_rate_,
        string memory currency_,
        string memory interest_schedule_format_,
        uint256 interest_payment_date_
//        string memory day_count_convention_,
//        string memory business_day_convention_,
//        string memory public_holidays_calendar_

    ) internal initializer {
        __ERC20_init(name_, symbol_);
        _storeBaseData(name_, symbol_, tokenId_, terms_, par_value_);
        _storeBondData(guarantor_identifier_, bondholder_representative_identifier_, maturity_date_, interest_rate_,
            currency_, interest_schedule_format_, interest_payment_date_);
//        _storeCalendarData(day_count_convention_, business_day_convention_, public_holidays_calendar_);
    }
...
//    }
    ...
    function approve(address spender, uint256 amount, uint256 currentAllowance) public virtual returns (bool) {
        require(allowance(_msgSender(), spender) == currentAllowance, "RapidBondz: current allowance is not right");
