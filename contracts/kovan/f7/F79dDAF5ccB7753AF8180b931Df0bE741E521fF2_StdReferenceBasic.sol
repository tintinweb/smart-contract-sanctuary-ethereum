...
contract StdReferenceBasic is AccessControl, StdReferenceBase {
    ...
    function relay(
        ...
        for (uint256 idx = 0; idx < symbols.length; idx++) {
            RefData memory ref = refs[symbols[idx]];
    ...
    /// @notice Reset ref data of symbols in the contract
    /// @param symbols A list of symbols whose ref data is being cleared in this function call
    function clearRefDataBulk(string[] memory symbols) external {
        require(hasRole(RELAYER_ROLE, msg.sender), "NOTARELAYER");
        for (uint256 idx = 0; idx < symbols.length; idx++) {
            refs[symbols[idx]] = RefData({rate: 0, resolveTime: 0, requestID: 0});
        }
    }
