...
contract ManeBase is ERC721A, Ownable, Pausable {
    ...
    uint256 public _reserveQuantity;
    ...
    */ 
    ...
    function getAll() public view returns (uint256[12] memory, TShare[] memory, TRefundTime[] memory) {
        uint256[12] memory u;
        ...
        return (u, _shareList, _refundTimeList);
    }
    ...
    function mint(address minter, uint256 mint_price, uint256 sign_deadline, bytes32 r, bytes32 s, uint8 v) payable whenNotPaused public returns (uint256) {
        ...
        if (block.timestamp < presaleStartTime) {
            ...
            revert("ERC721(ManeBase): Sale not started");
            ...
            require(msg.value >= 0, "ERC721(ManeBase): insufficant value for presale");
            isPresale = true;
        } else if (block.timestamp >= saleStartTime && block.timestamp <= saleEndTime) {
            // Period: Public sale perild
            require(msg.value >= _salePrice, "ERC721(ManeBase): insufficant value for sale");
            isSale = true;
        } else {
            revert("ERC721(ManeBase): Invalid _period");
        }
        
        require(totalMinted() < _maxSupply, "ERC721(ManeBase): No more tokens");
        ...
        if (isPresale == true) {
            ...
            require(presaleMintCountByAddress[msg.sender] < presaleMaxMintCountPerAddress, "ERC721(ManeBase): Address mint limit exceed");
            presaleMintCountByAddress[msg.sender]++;
        } else if (isSale == true) {
            //requireMintSign(minter, mint_price, sign_deadline, r, s, v);

            require(saleMintCountByAddress[msg.sender] < saleMaxMintCountPerAddress, "ERC721(ManeBase): Address mint limit exceed");
        ...
        uint256 platformGot = mint_price * platformFeePPM;
    ...
    function refund(uint256 tokenID) public {
        ...
        uint256 platformReturn = refundValue * platformFeePPM;
    ...
    function collect() public {
        ...
        for (uint256 i = 0; i < _refundTimeList.length; i++) {
            require(block.timestamp > _refundTimeList[i].endTime, "ERC721(ManeBase): Unable to collect before refund deadline");
        }
    ...
    function setMaxSupply(uint256 newMaxSupply) public onlyOwner {
        require(newMaxSupply >= totalMinted(), "ERC721(ManeBase): maxSupply too small");
    ...
    function getCollectorTotalRatioPPM() public view returns (uint256) {
        uint256 ratioPPM = 0;
        for (uint256 i =0; i < _shareList.length; i++) {
            ratioPPM += _shareList[i].ratioPPM;
        }

        require(ratioPPM <= 10e6, "ERC721(ManeBase): Collector ratio overflow");
