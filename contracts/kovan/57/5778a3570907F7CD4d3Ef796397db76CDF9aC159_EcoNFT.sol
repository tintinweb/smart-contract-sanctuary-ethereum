...
contract EcoNFT is ERC721, VRFConsumerBase, ReentrancyGuard, Ownable {
    ...
    uint256 public adminStartIndex; //inclusive
    uint256 public adminNftMinted; 
	...
	startIndex = 0;
	endIndex = 100;
	adminStartIndex = 100000;
	adminNftMinted = 0;
	mintFeeETH = 0.05 * 10**18; //0.05 ETH
	mintFeeToken = 100 * 10**18; //100 ESG 

	for(uint256 i=100000; i<20; i++)
		levels[i] = 0;
	for(uint256 i=0; i<41; i++)
		levels[i] = 1;
	for(uint256 i=41; i<74; i++)
		levels[i] = 2;
	for(uint256 i=74; i<91; i++)
		levels[i] = 3;
	for(uint256 i=91; i<99; i++)
		levels[i] = 4;
	levels[99] = 5;
    }

    //buy a lucky box
    function requestNewRandomNFTETH() public payable nonReentrant returns (bytes32) {
        require(
    ...
    function requestNewRandomNFTToken() public nonReentrant returns (bytes32) {
        require(
            LinkToken.balanceOf(address(this)) >= fee,
            "Not enough LINK"
        );
	...
	EsgToken.transferFrom(msg.sender, address(this), mintFeeToken);
        ...
        requestToSender[requestId] = msg.sender;
	boxSold = boxSold.add(1);
	EsgToken.transfer(owner(), mintFeeToken);
    ...
    function adminMint(address receipient)
	external
	onlyOwner
    {
	require(adminNftMinted <= 1080, "PINO limit is 1080");
	for(uint256 i=0; i < 20; i++)
	{
		uint256 newId =  adminStartIndex.add(i);
		require(!_exists(newId), "NFT minted.");
		nftMinted = nftMinted.add(1);
		adminNftMinted = adminNftMinted.add(1);
   ...
   function _withdrawLinkToken() public onlyOwner {
