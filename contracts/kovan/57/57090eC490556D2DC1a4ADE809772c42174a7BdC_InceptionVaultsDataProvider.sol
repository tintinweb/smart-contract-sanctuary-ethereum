...
contract InceptionVaultsDataProvider is IInceptionVaultsDataProvider, Initializable {
  ...
  IInceptionVaultsCore public inceptionVaultsCore;
  IAddressProvider public a;

  uint256 public inceptionVaultCount;

  uint256 public baseDebt;

  mapping(uint256 => InceptionVault) private _vaults;
  mapping(address => uint256) private _vaultOwners;

  modifier onlyInceptionCore() {
    require(msg.sender == address(inceptionVaultsCore));
    _;
  }

  function initialize(IInceptionVaultsCore _inceptionVaultsCore, IAddressProvider _addressProvider)
    external
    override
    initializer
  {
    inceptionVaultsCore = _inceptionVaultsCore;
    a = _addressProvider;
  }
  ...
  */
  function createVault(address _owner) public override onlyInceptionCore returns (uint256) {
  ...
  */
  function setCollateralBalance(uint256 _vaultId, uint256 _balance) public override onlyInceptionCore {
    require(vaultExists(_vaultId));
  ...
  */
  function setBaseDebt(uint256 _id, uint256 _newBaseDebt) public override onlyInceptionCore {
    ...
    if (_newBaseDebt > _vault.baseDebt) {
      ...
      baseDebt = baseDebt.add(increase);
    } else {
      uint256 decrease = _vault.baseDebt.sub(_newBaseDebt);
      baseDebt = baseDebt.sub(decrease);
    }
  ...
  */
  function vaults(uint256 _id) public view override returns (InceptionVault memory) {
...
@param _id the ID of the vault
  ...
  */
  function vaultId(address _owner) public view override returns (uint256) {
  ...
  */
  function vaultDebt(uint256 _vaultId) public view override returns (uint256) {
    ...
    return a.ratesManager().calculateDebt(v.baseDebt, inceptionVaultsCore.getCumulativeRate());
  }
}
