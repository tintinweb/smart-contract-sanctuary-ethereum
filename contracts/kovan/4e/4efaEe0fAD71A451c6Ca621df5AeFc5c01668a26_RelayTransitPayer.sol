...
/* This contract pays all the tx with Eth. This is done ONLY for testnets */
contract RelayTransitPayer {
    using GelatoBytes for bytes;
    address payable public immutable gelato;

    event LogExecSuccess(bytes32 indexed _taskId);

    modifier onlyGelatoWithEthPayment(address _token) {
        require(msg.sender == gelato, "Only gelato");
        _;

        if (_token != ETH) {
            (bool success, ) = gelato.call{value: 1}("");
            require(
                success,
                "RelayTransitPayer.onlyGelatoWithEthPayment: Gelato ETH transfer failed"
            );
        }
    }
    ...
    function _transferToGelato(address _token, uint256 _amount) private {
        ...
        if (_token == ETH) {
            ...
            require(
                success,
                "RelayTransitPayer._transferGelato: Gelato ETH transfer failed"
            );
        } else {
