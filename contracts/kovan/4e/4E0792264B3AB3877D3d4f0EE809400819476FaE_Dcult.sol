...
contract Dcult is Initializable, UUPSUpgradeable, ERC20Upgradeable, ERC20PermitUpgradeable, ERC20VotesUpgradeable, OwnableUpgradeable, PausableUpgradeable {
    ...
    // Amount hold by last top staker.
    uint256 public lastTopStakerAmount;
    // Address of last top staker.
    address public lastTopStakerAddress;
    ...
    */
    function addHighestStakedUser(
        ...
        uint256 i;
        uint256 nullAddressPosition = topStakerNumber;
        uint256 lastTopAddressPosition;
        ...
        for (i = 0; i < highestStaker.length; i++) {
            if (highestStaker[i].addr == user) {
                ...
                updateLastUser();
                return;
            }else if (highestStaker[i].addr == address(0)){
                nullAddressPosition = i;
            } else if(highestStaker[i].addr == lastTopStakerAddress){
                lastTopAddressPosition = i;
            }
        }

        // This condition will be called if one of top staker withraw and removed from guardian list.
        if(nullAddressPosition != topStakerNumber){
                highestStaker[nullAddressPosition].addr = user;
                highestStaker[nullAddressPosition].deposited = _amount;
                lastTopStakerAddress = user;
                lastTopStakerAmount = _amount;
                updateLastUser();
                return;
        }
        ...
        if (highestStaker.length < topStakerNumber) {
            ...
            if (lastTopStakerAmount < _amount) {
                // If the last staker deposited amount is less than new then we put the greater one in the array.
                highestStaker[lastTopAddressPosition].addr = user;
                highestStaker[lastTopAddressPosition].deposited = _amount;
                lastTopStakerAddress = user;
                lastTopStakerAmount = _amount;
                updateLastUser();
            }
    ...
    */
    ...
    function removeHighestStakedUser(uint256 _pid, uint256 _amount, address user) private {
        ...
        for (uint256 i = 0; i < highestStaker.length; i++) {
            if (highestStaker[i].addr == user) {
                ...
                if(_amount > lastTopStakerAmount){
                    highestStaker[i].deposited = _amount;
                    updateLastUser();
                } else {
                    delete highestStaker[i];
                    lastTopStakerAddress = address(0);
                    lastTopStakerAmount = 0;
                }
                return;
            }
        }
    ...
    function updateLastUser() internal {
        HighestAstaStaker[] storage highestStaker = highestStakerInPool[0];
        //for loop to check if the staking address exist in array
        for (uint256 i = 0; i < highestStaker.length; i++) {
            if (lastTopStakerAmount > highestStaker[i].deposited) {
                lastTopStakerAddress = highestStaker[i].addr;
                lastTopStakerAmount = highestStaker[i].deposited;
                // Called the function for sorting the array in ascending order.
                // quickSort(_pid, 0, highestStaker.length - 1);
                return;
            }
    ...
    function updateLastStakerDetails(uint256 stakerID) external onlyOwner {
        HighestAstaStaker[] storage highestStaker = highestStakerInPool[0];
        lastTopStakerAddress = highestStaker[stakerID].addr;
        lastTopStakerAmount = highestStaker[stakerID].deposited;
    }
