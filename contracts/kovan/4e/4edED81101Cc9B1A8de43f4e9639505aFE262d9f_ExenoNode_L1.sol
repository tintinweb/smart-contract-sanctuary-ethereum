...
// File: @connext/nxtp-contracts/contracts/nomad-core/libs/TypedMemView.sol
...
// File: @connext/nxtp-contracts/contracts/nomad-xapps/contracts/connext/ConnextMessage.sol
...
// File: @connext/nxtp-contracts/contracts/interfaces/IConnextHandler.sol
...
abstract contract ExenoNode is ERC1363Payable, Ownable {
    
    uint32 public immutable domain;
    address public immutable asset;
    IConnextHandler public immutable handler;
    mapping(uint32 => address) public nodes;

    event Deposited(address beneficiary, uint256 amount);
    event Withdrawn(address beneficiary, uint256 amount);
    event Reverted(address beneficiary, uint256 amount);

    modifier onlyExecutor() {
        require(
            msg.sender == handler.getExecutor(),
            "Only executor is allowed to call this method"
        );
        uint32 origin = IExecutor(msg.sender).origin();
        require(nodes[origin] != address(0), "Expected origin domain");
        require(
            IExecutor(msg.sender).originSender() == nodes[origin],
    ...
    {
        if (_processWithdraw(beneficiary, amount)) {
            emit Withdrawn(beneficiary, amount);
        } else {
            _deposit(IExecutor(msg.sender).origin(), beneficiary, amount);
            emit Reverted(beneficiary, amount);
        }
    }
    ...
    function _transferReceived(
        ...
        internal override
    {
        uint32 destination = abi.decode(data, (uint32));
        _deposit(destination, beneficiary, amount);
    }
    ...
    function _deposit(
        ...
        require(nodes[destination] != address(0), "Uknown destination address");

        bytes4 selector = bytes4(keccak256("withdraw(address,uint256)"));
        bytes memory callData = abi.encodeWithSelector(selector, beneficiary, amount);
        ...
        handler.xcall(xcallArgs);
        _processDeposit(amount);
        emit Deposited(beneficiary, amount);
    }

    function _processWithdraw(address beneficiary, uint256 amount) internal virtual returns(bool) {}
    function _processDeposit(uint256 amount) internal virtual {}
}

// File: contracts/ExenoNode_L1.sol
...
contract ExenoNode_L1 is ExenoNode {
    ...
    constructor(
        ...
        IConnextHandler _handler,
        IERC1363 _token
    ) ExenoNode(_domain, _asset, _handler, _token) {
        token = _token;
    }

    function _processWithdraw(address beneficiary, uint256 amount)
