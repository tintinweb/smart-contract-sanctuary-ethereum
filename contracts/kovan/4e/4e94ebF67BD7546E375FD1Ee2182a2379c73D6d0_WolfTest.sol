...
contract WolfTest is Ownable {
    INFTAdmin public wolfAdmin;

    constructor(INFTAdmin _wolfAdmin) {
        wolfAdmin=_wolfAdmin;
    }

    function getWolf(uint grade,uint kind) external onlyOwner returns(uint tokenId) {
        require(kind>=1&& kind<=4 &&grade>=1 && grade<=5,"param error");
        tokenId=wolfAdmin.mintNFT(msg.sender);
