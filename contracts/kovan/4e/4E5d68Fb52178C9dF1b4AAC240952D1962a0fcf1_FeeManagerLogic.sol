...
contract FeeManagerLogic {
  ...
  // accumulatedCommunityFee[nftAddr][tokenAddra] = unifiedAmount
  ...
  function batchFeeClaim(address[] memory nftAddr, uint256[][] memory tokenID, address[][] memory tokenAddrs) external {
  ...
  function adminWithdraw(address tokenAddr, uint256 tokenAmount) external onlyOwner {
    if(tokenAddr == NATIVECOINADDR){
      ...
      IERC20(tokenAddr).transfer(owner, tokenAmount);
    }
  ...
  function _feeClaim(address nftAddr, uint256[] memory tokenID, address[] memory tokenAddrs) internal {
    ...
    cummunityFeeParams.totalSupply = nft.totalSupply() * UNIFIEDPOINT;
    ...
    for(uint256 tokenIndex=0; tokenIndex < tokenAddrs.length; tokenIndex++){
      if(tokenAddrs[tokenIndex] == NATIVECOINADDR){
        ...
        uint256 underlyingAmount = rewardAmount[tokenIndex].unifiedToUnderlyingAmount(underlyingDecimal);
  ...
  function _getRewardPerNFT(address _nftAddr, address _tokenAddr) internal view returns (uint256) {
    ...
    uint256 totalSupply = nft.totalSupply() * UNIFIEDPOINT;
