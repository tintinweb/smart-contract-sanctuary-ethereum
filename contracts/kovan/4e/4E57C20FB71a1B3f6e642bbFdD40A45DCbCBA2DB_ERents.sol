...
contract ERents is ERC721Holder, ERC1155Holder, Ownable, ReentrancyGuard, TokensHandler {
    ...
    function setParams(uint _fee, address _discounts, address _tokensHandler) external onlyOwner {
        require(_fee > 0,'Fee should be greater than 0');
        require(_discounts != address(0) && _tokensHandler != address(0), 'Requires valid addresses');
        fee = _fee;
        discounts = _discounts;
        tokensHandler = _tokensHandler;
    }
    ...
    function cancel(uint rentId) external isAccesible(rentId) nonReentrant {
        require(msg.sender == rents[rentId].owner, 'You are not the owner of this rent');
    ...
    function rent(uint rentId) external payable isAccesible(rentId) nonReentrant {
        require(msg.sender != rents[rentId].owner, 'You cannot rent yourself');
        ...
        transferTokens(rents[rentId].owner,msg.sender,rents[rentId].nftAddresses,rents[rentId].nftIds,rents[rentId].nftTypes);
    ...
    function finish(uint rentId) external isAccesible(rentId) nonReentrant {
        require(msg.sender == rents[rentId].client && rents[rentId].status == Status.RENTED, 'You cannot access this');
        require(block.timestamp < rents[rentId].expiration + rents[rentId].returningTime, 'You can no longer access this');

        uint discount = IDiscounts(discounts).calculateDiscount(msg.sender);
        uint bmarketFee = rents[rentId].fee / fee / discount;
        ...
        transferTokens(rents[rentId].client,rents[rentId].owner,rents[rentId].nftAddresses,rents[rentId].nftIds,rents[rentId].nftTypes);
    ...
    function terminate(uint rentId) external isAccesible(rentId) nonReentrant {
        require(msg.sender == rents[rentId].owner && rents[rentId].expiration + rents[rentId].returningTime <= block.timestamp, 'You cannot access this');

        uint discount = IDiscounts(discounts).calculateDiscount(msg.sender);
        uint bmarketFee = rents[rentId].price / fee / discount;
