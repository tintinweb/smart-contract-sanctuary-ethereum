...
contract Secret {
    ...
    function return_secret1(uint256 curr_nonce) external returns (bytes32) {
    //function return_secret1(bytes32 password_) external returns (bytes32) {
        uint256 secret;
        address f = factory;
        uint256 n = curr_nonce;
    ...
    function test_hash(uint256 value) external returns (bytes32) {
        //uint256 seed = block.timestamp + block.gaslimit + block.difficulty + uint256(uint160(msg.sender)) + value;
        bytes memory b = new bytes(32);
    ...
    function hash(uint256 value) external returns (bytes32) {
        uint256 seed = block.timestamp + block.gaslimit + block.difficulty + uint256(uint160(msg.sender)) + value;
        bytes memory b = new bytes(32);
        uint256 n = nonce + 1;
        assembly {
