...
contract LinearVesting is ReentrancyGuard {
    ...
    uint256 public TGEDistibutePercentage = 2500;
     ...
     */
    constructor(IERC20 _token, uint256 _start, uint256 _end, uint256 _cliffDurationInSecs, uint256 _tgedistributionPercentage)  {
        ...
        TGEDistibutePercentage = _tgedistributionPercentage;
    }
...
contract ShavoVestingFactory{
    ...
    struct Vesting{
        ...
        uint256 end;
        uint256 cliffTime;
        uint256 TGEDistibutePercentage;
    }

    mapping(address => Vesting) public vestingData;
    address[] public vestingAddress;

    function createVestingContract(IERC20 _tokenAddress, uint256 _start, uint256 _end, uint256 _cliffTime, uint256 _tgedistributionPercentage) public returns(address) {
        LinearVesting vesting = new LinearVesting(_tokenAddress, _start, _end, _cliffTime, _tgedistributionPercentage);
        vestingData[address(vesting)].vestingAddress = address(vesting); 
        vestingData[address(vesting)].tokenAddress = _tokenAddress;
        vestingData[address(vesting)].start = _start;
        vestingData[address(vesting)].end = _end;
        vestingData[address(vesting)].cliffTime = _cliffTime;
        vestingData[address(vesting)].TGEDistibutePercentage = _tgedistributionPercentage;
        vestingAddress.push(address(vesting));
        return address(vesting);
    }

    function getVestingAddresses() public view returns(address[] memory){
