...
contract BatchSwap is Ownable, Pausable, IERC721Receiver, IERC1155Receiver {
  ...
  function createSwapIntent(
    ...
    for (i = 0; i < nftsOne[_swapIntent.id].length; i++) {
      ...
      if (nftsOne[_swapIntent.id][i].typeStd == assetType.ERC20) {
        ...
        ERC721Interface(nftsOne[_swapIntent.id][i].dapp).safeTransferFrom(
          _swapIntent.addressOne,
          address(this),
          nftsOne[_swapIntent.id][i].tokenId[0],
          nftsOne[_swapIntent.id][i].data
        );
      } else if (nftsOne[_swapIntent.id][i].typeStd == assetType.ERC1155) {
        ERC1155Interface(nftsOne[_swapIntent.id][i].dapp).safeBatchTransferFrom(
            _swapIntent.addressOne,
            address(this),
  ...
  function cancelSwapIntent(uint256 _swapId) public {
    ...
    require(
      swapList[msg.sender][swapMatch[_swapId]].status == swapStatus.Pending,
      "Swap Status is not opened"
    );
    //Rollback
    ...
    swapList[msg.sender][swapMatch[_swapId]].status = swapStatus.Canceled;
    emit swapEvent(
      msg.sender,
      block.timestamp,
      swapStatus.Canceled,
