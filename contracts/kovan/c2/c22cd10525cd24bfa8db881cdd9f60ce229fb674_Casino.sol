...
contract Casino is Ownable {
    ...
    function topUpBalance(uint256 count) external {
        ...
        erc20.transferFrom(msg.sender, address(this), user + jp);
        if (caas > 0) erc20.transferFrom(msg.sender, caasbank, caas);
        addBalanveInternal(msg.sender, user);
        addBalanveInternal(address(0), jp);
    }

    function topUpBalanceInternal(address account, uint256 count) internal {}
    ...
    function giveJackpot(address account) external onlyAdmin {
        // give erc20
        uint256 count = jackpot();
        erc20Balances[address(0)] = 0;
    ...
    function addBalance(address account, uint256 count) public onlyAdmin {
        addBalanveInternal(account, count);
    }

    function addBalanveInternal(address account, uint256 count) internal {
        erc20Balances[account] += count;
    }
    ...
    function applyChanges(BalanceChange[] calldata changes) external onlyAdmin {
        for (uint256 i = 0; i < changes.length; ++i) {
            ...
            if (change > 0)
                addBalanveInternal(changes[i].account, uint256(change));
    ...
    function withdrawNft(address account) external onlyAdmin {
        uint256 nftBalance = nft.balanceOf(address(this));
