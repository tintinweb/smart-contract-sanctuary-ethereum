...
// File contracts/tokens/IRainiCustomNFT.sol
...
// File contracts/tokens/RainiChildContractv1.sol
...
contract RainiChildContractv1 is AccessControl, IRainiCustomNFT {
    ...
    function onMinted(
        ...
        if (_data.length > 0) {
            IRainiNft1155.TokenVars memory _tokenVars = nftContract.tokenVars(
                _tokenId
            );
    ...
    function getTokenState(uint256 id)
        public
        view
        override
    ...
    function setTokenStates(uint256[] memory id, bytes[] memory state)
        external
        override
        onlyEditor
    {
        ...
        for (uint256 i; i < id.length; i++) {
            if (!isEditor) {
                IRainiNft1155.TokenVars memory _tokenVars = nftContract
                    .tokenVars(id[i]);
                require(
                    _tokenVars.number > 0 &&
                        ownerCanEdit[_tokenVars.cardId] &&
                        nftContract.balanceOf(_msgSender(), id[i]) == 1,
                    "can't edit"
                );
            }
    ...
    function uri(uint256 id) external view override returns (string memory) {
        ...
        for (uint256 i; i < tokenData[id].length; i++) {
            query = string(
                abi.encodePacked(
                    ...
                    Strings.toString(i),
                    "=",
                    Strings.toString(tokenData[id][i]),
                    ""
                )
            );
        }
        ...
        return
            string(
                abi.encodePacked(
                    baseUri,
                    pathUris[_tokenVars.cardId],
                    "?c=",
                    _tokenVars.mintedContractChar,
                    "&l=",
                    Strings.toString(_tokenVars.level),
                    "&n=",
                    Strings.toString(_tokenVars.number),
                    query
                )
            );
    }
