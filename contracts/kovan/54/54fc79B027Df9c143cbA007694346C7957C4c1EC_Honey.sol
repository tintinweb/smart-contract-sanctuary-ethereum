...
contract Honey is ERC20, SimpleAccess {
    ...
    function getEcoSystemBalance(address user, uint256[] memory flowersWithBees) public view returns (uint256) {
        ...
        return ecoSystem.getTotalProductionOfUser(user, flowersWithBees) + ecoSystemBalance.deposit - ecoSystemBalance.spent;
    }
    ...
    function spendEcoSystemBalance(address user, uint128 amount, uint256[] memory flowersWithBees) external onlyAuthorized {
        require(getEcoSystemBalance(user, flowersWithBees) >= amount, "Eco system balance too low");

        uint256 taxAmount = taxManager.getSpendTax(user, amount);
        uint256 toSpend = amount - taxAmount;

        EcoSystemBalance storage ecoSystemBalance = userEcoSystemBalance[user];
        ecoSystemBalance.spent += toSpend;
    ...
    function _beforeTokenTransfer(
        ...
        require(!blacklisted[from] && !blacklisted[to], "FROM OR TO IS BANNED");
    }
    ...
    function mint(uint256 amount) external {
        require(msg.sender == address(taxManager), "Only tax manager can mint");
        _mint(msg.sender, amount);
    }
