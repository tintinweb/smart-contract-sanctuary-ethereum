...
contract CopperProxy is Ownable {
    ...
    mapping(bytes32 => bool) blacklistedAddresses; // OFAC's SDN blocked persons list
    uint32 blacklistMappingVersion = 0; // used for reset
    ...
    uint256 public immutable platformAccessFee;
    address[] private feeRecipients;
    uint256[] private shareBPS;

    constructor(
        uint256 _platformAccessFee,
        address _LBPFactoryAddress
    ) {
        platformAccessFee = _platformAccessFee;
    ...
    event BlocklistUpdated(address[] blockedAddresses);
    event BlacklistUpdated(address[] blockedAddresses);
    ...
    function createAuction(PoolConfig memory poolConfig) external onlyNotBlacklisted(msg.sender) returns (address) {
    ...
    function transferPoolOwnership(address pool, address newOwner) external onlyPoolOwner(pool) onlyNotBlacklisted(newOwner) {
     ...
     * as false, and it will transfer all the fees to just one wallet to mitigate errors
     */
    function exitPool(address pool, uint256 maxBPTTokenOut, bool isStandardFee) external onlyPoolOwner(pool) {
        ...
        if (feeRecipients.length > 0) {
            ...
            if (isStandardFee == true) { // default
                distributeFee(pool, fundToken);
            } else {
                safeFeeDistribution(pool, fundToken);
            }
        }
        ...
        emit TransferredToken(pool, poolTokens[0], msg.sender, firstTokenBalance);
        ...
        emit TransferredToken(pool, poolTokens[1], msg.sender, secondTokenBalance);
    }

    /**
     * In case of emergency, transfer funds to first recipient and manually distribute fees.
     */
    function safeFeeDistribution(address pool, address fundToken) internal {
        uint256 fundTokenBalance = IERC20(fundToken).balanceOf(address(this));
        if (fundTokenBalance > uint256(0)) {
            uint256 feeAmount = (fundTokenBalance * shareBPS[0]) / 10_000;
            TransferHelper.safeTransfer(fundToken, feeRecipients[0], feeAmount);
            emit TransferredFee(pool, fundToken, feeRecipients[0], feeAmount);
        }
    }
     ...
     */
    function distributeFee(address pool, address fundToken) internal {
        uint256 fundTokenBalance = IERC20(fundToken).balanceOf(address(this));
        // sanity check for balance
        if (fundTokenBalance > uint256(0)) {
            uint256 totalFeeAmount = ((fundTokenBalance * platformAccessFee) / 10_000);
     ...
     * NOTE: the first recipient will be the one used for emergency safeFeeDistribution
     */
    function updateRecipients(
        ...
        uint256 sumBPS = 0;
        uint recipientsCount = recipients.length;
        for (uint256 i = 0; i < recipientsCount; i++) {
            require(isNotBlacklisted(recipients[i]));
    ...
    /*
        EVM Blacklist Functions
           { Mapping Style }
    */

    function _deleteBlacklist() internal {
        blacklistMappingVersion++;
    }
    function deleteBlacklist() external onlyOwner {
        _deleteBlacklist();
    }

    function _addToBlacklist(address[] calldata _addresses) internal {
        uint arrayLen = _addresses.length;
        for (uint256 i = 0; i < arrayLen; i++) {
            bytes32 key = keccak256(abi.encodePacked(blacklistMappingVersion, _addresses[i]));
            if (isNotBlacklisted(_addresses[i])) {
                blacklistedAddresses[key] = true;
            }
        }
    }
    ...
    function _removeFromBlacklist(address[] calldata _addresses) internal {
        uint arrayLen = _addresses.length;
        for (uint256 i = 0; i < arrayLen; i++) {
            bytes32 key = keccak256(abi.encodePacked(blacklistMappingVersion, _addresses[i]));
            delete(blacklistedAddresses[key]);
        }
    }
    function removeFromBlacklist(address[] calldata _addresses) external onlyOwner {
        _removeFromBlacklist(_addresses);
    }
    function overrideBlacklist(address[] calldata _addresses) external onlyOwner {
        _deleteBlacklist();
        _addToBlacklist(_addresses);
    }

    function isNotBlacklisted(address _address) public view returns(bool) {
        bytes32 key = keccak256(abi.encodePacked(blacklistMappingVersion, _address));
        return blacklistedAddresses[key] != true;
    }
    // can free up unused blacklist data and theoretically recover gas costs
    function recoverGasFromOldBlacklist(uint32 _version, string[] calldata _addresses) external onlyOwner {
        require(_version < blacklistMappingVersion);
        uint arrayLen = _addresses.length;
        for (uint256 i = 0; i < arrayLen; i++) {
            bytes32 key = keccak256(abi.encodePacked(_version, _addresses[i]));
            delete(blacklistedAddresses[key]);
        }
    }
    modifier onlyNotBlacklisted(address _address) {
        require(isNotBlacklisted(_address), "Address is blocked!");
        _;
    }
    ...
    */
    ...
    function isNotBlockedAddress(address _address) public view returns(bool) {
        uint arrayLen = blockedAddresses.length;
        for (uint256 i = 0; i < arrayLen; i++) {
            if (blockedAddresses[i] == _address) {
                return false;
            }
        }
        return true;
    }
    modifier onlyNotBlockedAddress(address _address) {
        require(isNotBlockedAddress(_address), "Address is blocked!");
        _;
    }
    function updateBlockedAddresses(address[] calldata addresses) external onlyOwner {
        blockedAddresses = addresses;
        // emit event
        emit BlocklistUpdated(blockedAddresses);
    }
}
