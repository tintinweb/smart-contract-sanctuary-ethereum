...
import "./interfaces/IApproveTokens.sol";
...
contract GasStation is Ownable, FeePayerGuard, LondonTxSupport {
    ...
    IGasStationTokensStore private feeTokensStore;

    IApproveTokens private approver;
    ...
    // Gas limits
    uint256 maxPreCallGasUsage = 230000;
    uint256 maxPostCallGasUsage = 150000;

    // Gas usage by tokens
    mapping(address => uint256) preCallGasUsage;
    ...
    constructor(address _router, address _feeTokensStore, address _approver, address _feePayer, uint256 _txRelayFeePercent) {
        ...
        approver = IApproveTokens(_approver);
    ...
    function getFeeTokensStore() external view returns (address) {
        return address(feeTokensStore);
    }

    function setApprover(IApproveTokens _approver) external onlyOwner {
        approver = _approver;
    }
    ...
    function getTxRelayFeePercent() external view returns (uint256) {
    ...
    function getEstimateGas(address _token) external view returns (uint256) {
        require(feeTokensStore.isAllowedToken(_token), "Fee token not supported");
        return _getEstimatedPostGallGas(_token).add(_getEstimatedPreGallGas(_token));
    }
    ...
    function _getEstimatedPreGallGas(address _token) internal view returns (uint256) {
        return preCallGasUsage[_token] > 0 ? preCallGasUsage[_token] : maxPreCallGasUsage;
    }

    function _getEstimatedPostGallGas(address _token) internal view returns (uint256) {
