...
contract PropertyVaultUUPS is AccessControlUpgradeable, UUPSUpgradeable, IPropertyVaultUUPS {
  ...
  function getMerkleRoot() external override view returns (bytes32) {
    return _merkleRoot;
  }
  ...
  function claim(
    ...
    for (uint256 i = 0; i < tokens.length; i++) {
      ...
      require(preclaimed < cumulativeAmounts[i], "CMD: Nothing to claim");
  ...
  function withdraw(address account, address[] calldata tokens, uint256[] calldata amounts) external override onlyRole(DEFAULT_ADMIN_ROLE) {
    for (uint256 i = 0; i < tokens.length; i++) {
      IERC20Upgradeable(tokens[i]).safeTransfer(account, amounts[i]);
      emit Withdrawed(account, tokens[i], amounts[i]);
    }
  }
...
interface IPropertyVaultUUPS {
  ...
  event Withdrawed(address indexed account, address indexed token, uint256 indexed amount);
  // Returns the total amount that the address already claimed. 
  function totalClaimedAmount(address token, address account) external view returns (uint256);
  ...
  function withdraw(address account, address[] calldata tokens, uint256[] calldata amounts) external;
}
