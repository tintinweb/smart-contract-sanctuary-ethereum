...
This contract will:
    ...
    - issueTokens: Issue tokens rewards
    - addAllowedTokens: Allow a new token to be staked in the contract
    - getEthValue: Get the value in ETH of our tokens
*/
...
contract TokenFarm is Ownable {
    ...
    function issueTokens() public onlyOwner {
        for (
            ...
            dappToken.transfer(recipient, totalValue * 10**18);
        }
    }
    ...
    function getUserTotalValue(address _user) public view returns (uint256) {
        ...
        require(
            ...
            "The user does not have tokens staked!"
        );
        for (
    ...
    function getUserSingleTokenValue(address _user, address _token)
        ...
        returns (uint256)
    {
        uint256 userStakingBalance = stakingBalances[_token][_user];
        if (userStakingBalance <= 0) {
            return 0;
        }
        (uint256 price, uint256 decimals) = getTokenValue(_token);
        return (price * userStakingBalance) / 10**decimals;
    }
    ...
    function stakeTokens(uint256 _amount, address _token) public {
        ...
        require(tokenIsAllowed(_token), "The token is not allowed!");
        IERC20(_token).transferFrom(msg.sender, address(this), _amount);
        updateUniqueTokenStakedAndStakers(msg.sender, _token);
        stakingBalances[_token][msg.sender] += _amount;
    }

    function unstakeTokens(address _token) public {
        uint256 balance = stakingBalances[_token][msg.sender];
        require(balance > 0, "Cannot unstake because you have 0 tokens");
        IERC20(_token).transfer(msg.sender, balance);
        stakingBalances[_token][msg.sender] = 0;
        uniqueTokenStaked[msg.sender] -= 1;
    }

    function updateUniqueTokenStakedAndStakers(address _user, address _token)
        internal
    {
        if (stakingBalances[_token][_user] <= 0) {
            uniqueTokenStaked[_user] += 1;
            if (uniqueTokenStaked[_user] == 1) {
                stakers.push(_user);
            }
        }
    }
