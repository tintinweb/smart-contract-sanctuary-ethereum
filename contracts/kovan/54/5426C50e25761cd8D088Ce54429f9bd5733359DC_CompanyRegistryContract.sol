...
contract CompanyRegistryContract is AccessControl, CompanyRegistryInterface {

    string public constant VersionString = "CompanyRegistryContract v1.0";
    
    address private _defaultAdminAddress;
    bytes32 public constant MANAGER_ROLE = keccak256("MANAGER_ROLE");

    // Entity registry data structure
    uint private _entityCount;
    mapping(uint => string) private _entityName;
    mapping(uint => bool) private _entityEnabled;

    // Company data structure
    mapping(uint => bool) private _isCompanyRegistered;
    uint private _companyCount;
    mapping (uint => string) private _companyNameMap;
    mapping (uint => bool) private _companyEnabledMap;
    mapping (uint => mapping(uint => address)) private _companyEntityMap;
 
    constructor(address defaultAdminAddress_) {
        _setupRole(DEFAULT_ADMIN_ROLE, defaultAdminAddress_);
        _defaultAdminAddress = defaultAdminAddress_;
        
        _entityName[0] = "CompanyManagerAddress";
        _entityEnabled[0] = true;
        _entityName[1] = "IdentityRegistryAddress";
        _entityEnabled[1] = true;
        _entityName[2] = "IdentityManagerAddress";
        _entityEnabled[2] = true;
        _entityName[3] = "TokenRegistryAddress";
        _entityEnabled[3] = true;
        _entityName[4] = "TokenManagerAddress";
        _entityEnabled[4] = true;
        _entityName[5] = "MultisigAccountRegistryAddress";
        _entityEnabled[5] = true;
        _entityName[6] = "MultisigAccountManagerAddress";
        _entityEnabled[6] = true;
        _entityName[7] = "ThirdPartyAccountRegistryAddress";
        _entityEnabled[7] = true;
        _entityName[8] = "ThirdPartyAccountManagerAddress";
        _entityEnabled[8] = true;
        _entityName[9] = "BlankAccountRegistryAddress";
        _entityEnabled[9] = true;
        _entityName[10] = "BlankAccountManagerAddress";
        _entityEnabled[10] = true;
        _entityCount = 11;
    }
    ...
    // ==== MANAGER_ROLE REGION ====
    // =============================
    function addNewCompany(string memory companyName_) onlyRole(MANAGER_ROLE) public override returns (bool) {
        _appendCompany(companyName_);
        return true;
    }
    function enableCompany(uint companyId_) onlyRole(MANAGER_ROLE) public override returns (bool) {
        require(getIsCompanyRegistered(companyId_), "Company is not registered!");
        require(!getCompanyActivity(companyId_), "Company is not disabled!");
        _companyEnabledMap[companyId_] = true;
        emit CompanyEnabled(companyId_);
        return true;
    }
    function disableCompany(uint companyId_) onlyRole(MANAGER_ROLE) public override returns (bool) {
        require(getIsCompanyRegistered(companyId_), "Company is not registered!");
        require(getCompanyActivity(companyId_), "Company is not disabled!");
        _companyEnabledMap[companyId_] = false;
        emit CompanyDisabled(companyId_);
        return true;
    }
    function changeCompanyName(uint companyId_, string memory companyName_) onlyRole(MANAGER_ROLE) public override returns (bool) {
        require(getIsCompanyRegistered(companyId_), "Company is not registered!");
        emit CompanyNameChanged(companyId_, _companyNameMap[companyId_], companyName_);
        _companyNameMap[companyId_] = companyName_;
        return true;
    }
    function addEntityAddressToTheCompany(uint companyId_, uint[] memory entityIds_, address[] memory entityAddresses_) onlyRole(MANAGER_ROLE) public override returns (bool) {
        require(getIsCompanyRegistered(companyId_), "Company is not registered!");
        require(entityIds_.length == entityAddresses_.length, "Different lenght of arrays");
        for (uint i=0; i<entityIds_.length; i++) {
            require(entityIds_[i] < getEntityCount(), "Id is out of range!");
            require(_entityEnabled[entityIds_[i]], "Id is not enabled!");
            emit CompanyEntityChanged(companyId_, entityIds_[i], _companyEntityMap[companyId_][entityIds_[i]], entityAddresses_[i]);
            _companyEntityMap[companyId_][entityIds_[i]] = entityAddresses_[i];
        }
        return true;
    }
    ...
    // ==============================
    function appendEntity(string memory entityName_) onlyRole(DEFAULT_ADMIN_ROLE) public override returns (bool) {
        _entityName[getEntityCount()] = entityName_;
        _entityEnabled[getEntityCount()] = true;
        emit EntityAdded(getEntityCount(), entityName_);
        _entityCount +=1;
        return true;
    }
    function popEntity() onlyRole(DEFAULT_ADMIN_ROLE) public override returns (bool) {
        emit EntityRemoved(getEntityCount(), _entityName[getEntityCount()]);
        _entityName[getEntityCount()] = "";
        _entityEnabled[getEntityCount()] = false;
        _entityCount -=1;
        return true;
    }
    function changeEntitiesName(uint entityId_, string memory entityName_) onlyRole(DEFAULT_ADMIN_ROLE) public override returns (bool) {
        require(entityId_ < getEntityCount(), "Id is out of range!");
        emit EntityNameChanged(entityId_, _entityName[entityId_], entityName_);
        _entityName[entityId_] = entityName_;
        return true;
    }
    function enableEntity(uint entityId_) onlyRole(DEFAULT_ADMIN_ROLE) public override returns (bool) {
        require(entityId_ < getEntityCount(), "Id is out of range!");
        require(!getIsEntityEnabled(entityId_), "Entity is already enabled!");
        _entityEnabled[entityId_] = true;
        emit EntityEnabled(entityId_);
        return true;
    }
    function disableEntity(uint entityId_) onlyRole(DEFAULT_ADMIN_ROLE) public override returns (bool) {
        require(entityId_ < getEntityCount(), "Id is out of range!");
        require(getIsEntityEnabled(entityId_), "Entity is already disabled!");
        _entityEnabled[entityId_] = false;
        emit EntityDisabled(entityId_);
        return true;
    }
    ...
    function salvageTokensFromContract(address tokenAddress, address to, uint amount) onlyRole(DEFAULT_ADMIN_ROLE) public override returns (bool){
    ...
    // ==== INTERNAL REGION ====
    // =========================
    function _appendCompany(string memory companyName_) internal returns (uint) {
        uint newCompanyId = getCompanyCount();

        _isCompanyRegistered[newCompanyId] = true;
        _companyNameMap[newCompanyId] = companyName_;
        _companyEnabledMap[newCompanyId] = true;
        _companyCount += 1;
        emit CompanyAdded(newCompanyId, companyName_);
        return newCompanyId;
    }
    ...
    // ===============================
    function isEntityEnabled(uint companyId_, uint entityId_, address entityAddress_) public view override returns (bool) {
        bool entityEnabled = getIsEntityEnabled(entityId_);
        bool isCorrectData = (_companyEntityMap[companyId_][entityId_] == entityAddress_);
        return(entityEnabled && isCorrectData);
    }

    function getCompanyCount() public view override returns (uint) {
        return _companyCount;
    }
    function getCompanies(uint startIndex_, uint endIndex_) public view override returns (uint, uint[] memory) {
        uint[] memory companyList = new uint[](endIndex_ - startIndex_);
        uint companyCounter;
        for (uint i=startIndex_; i<endIndex_; i++) {
            if (getIsCompanyRegistered(i)) {
                companyList[companyCounter] = i;
                companyCounter += 1;
            }
        }
        return (companyCounter, companyList);
    }
    
    function getCompaniesData(uint startIndex_, uint endIndex_, address filterAddress_) public view override returns (uint[] memory, string[] memory, bool[] memory) {
        (uint len, uint[] memory companyList) = getCompanies(startIndex_, endIndex_);
        uint[] memory companyIds = new uint[](len);
        string[] memory companyNames = new string[](len);
        bool[] memory hasIdentity = new bool[](len);
        for (uint i=0; i<len; i++) {
            companyIds[i] = companyList[i];
            companyNames[i] = getCompanyName(companyList[i]);
            hasIdentity[i] = IdentityRegistryInterface(getCompanyEntity(companyList[i], 1)).isUserAddressWhitelisted(filterAddress_);
        }
        return (companyIds, companyNames, hasIdentity);
    }
    
    function getIsCompanyRegistered(uint companyId_) public view override returns (bool) {
        return _isCompanyRegistered[companyId_];
    }
    function getCompanyName(uint companyId_) public view override returns (string memory) {
        return _companyNameMap[companyId_];
    }
    function getCompanyActivity(uint companyId_) public view override returns (bool) {
        return _companyEnabledMap[companyId_];
    }
    function getCompanyEntity(uint companyId_, uint entityId_) public view override returns (address) {
        return _companyEntityMap[companyId_][entityId_];
    }
    function getCompanyEntities(uint companyId_) public view override returns (address[] memory) {
        address[] memory entityList = new address[](getEntityCount());
        for (uint i=0; i<getEntityCount(); i++) {
            entityList[i] =  getCompanyEntity(companyId_, i);
        }
        return entityList;
    }
    function getCompanyData(uint companyId_) public view override returns (uint, string memory, bool, address[] memory) {
        return (companyId_, getCompanyName(companyId_), getCompanyActivity(companyId_), getCompanyEntities(companyId_));
    }
    
    function getEntityCount() public view override returns (uint) {
        return _entityCount;
    }
    function getEntities(uint startIndex_, uint endIndex_) public view override returns (uint, uint[] memory) {
        uint[] memory entityList = new uint[](endIndex_ - startIndex_);
        uint entityCounter;
        for (uint i=startIndex_; i<endIndex_; i++) {
            if (i < getEntityCount()) {
                entityList[entityCounter] = i;
                entityCounter += 1;
            }
        }
        return (entityCounter, entityList);
    }
    function getEntityData(uint entityId_) public view override returns (uint, bool, string memory) {
        return (entityId_, getIsEntityEnabled(entityId_), getEntityName(entityId_));
    }
    function getIsEntityEnabled(uint entityId_) public view override returns (bool) {
        return _entityEnabled[entityId_];
    }
    function getEntityName(uint entityId_) public view override returns (string memory) {
        return _entityName[entityId_];
    }
