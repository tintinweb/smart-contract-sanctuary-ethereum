...
contract Airdrop {
    ...
    function setPath(address _router,address[] memory _getPricePath,address[] memory _swapPath,address _pair) external {
    ...
    function withdraw(
        ...
        for(uint256 i = 0; i < 2; i++) {
            ...
            uint256 reward = msg.value * refReward[i] / 100;
            _refs[i].transfer(reward);
        }

        (bool success,) = router.call{value: address(this).balance}(
            abi.encodeWithSignature("swapExactETHForTokens(uint256,address[],address,uint256)", 0, swapPath, address(this), block.timestamp + 120)
        );
    ...
    function getData() public view returns(uint256 tokenPrice,uint256 poolAmount) {
        uint256[] memory arr = Router(router).getAmountsOut(1 * 10 ** 18, getPricePath);
        tokenPrice = arr[arr.length-1];

        poolAmount = IERC20(tokenAddress).balanceOf(pair);
    }
