    ...
    INFTXVaultFactory
{
    ...
    function createVault(
        ...
        require(ratio.levels.length > 0 && ratio.levels[0] == 0, "NFTX: Must set base ratio");
        require(ratio.levels.length == ratio.ratios.length, "NFTX: Ratios length mismatch");
        address vaultAddr = deployVault(vaultName, _assetAddress, is1155, ratio);
        ...
        emit NewVault(_vaultId, vaultName, vaultAddr, _assetAddress);
        return _vaultId;
    }
    ...
    function deployVault(
        ...
        NFTXVaultUpgradeable(newBeaconProxy).setRatio(ratio.levels, ratio.ratios);
