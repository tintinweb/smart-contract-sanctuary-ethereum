...
contract ManeBase is ERC721A, Ownable, Pausable {
    ...
    uint256 public mintedAmountNotRefunded;

    // Total amount of minted (exclude platform fee)
    uint256 public mintedAmountForUser;
    uint256 public mintedAmountForUserNotRefunded;
    ...
    mapping(address => uint256) presaleMintCountByAddress;
    uint256 public presaleMaxMintCountPerAddress;

    mapping(address => uint256) saleMintCountByAddress;
    uint256 public saleMaxMintCountPerAddress;

    // Platform fee ratio in PPM
    uint256 platformFeePPM = 0;
    ...
    */ 
    constructor(string memory name_, string memory symbol_, uint256[] memory u256s,
        ...
        emit ContractDeployed(tx.origin, address(this), u256s[0], clubId);
        ...
        require(refundTimes_.length == refundRatios_.length, "ERC721(ManeBase): refundTimes and refundRatios length mismatch");
        uint256 totalRefundRatios = 0;
        ...
        for( uint256 i = 0; i < refundTimes_.length; i++) {
            ...
            require(t.endTime > oldEndTime, "ERC721(ManeBase): refundTimes must be in ascending order");
            require(t.ratioPPM > oldRatio, "ERC721(ManeBase): refundRatio must be in ascending order");
            ...
            totalRefundRatios += t.ratioPPM;
        }
        require(totalRefundRatios <= 1 * 1000 * 1000, "ERC721(ManeBase): Sum of refundRatios bigger than 1");
    ...
    // mint_price is salePrice or presalePrice. 
    // sign_deadline, r, s, v is only require at presale perioid. These parameters are server-side signature data.
    function mint(address minter, uint256 mint_price, uint256 sign_deadline, bytes32 r, bytes32 s, uint8 v) payable whenNotPaused public returns (uint256) {
        ...
        if (block.timestamp < presaleStartTime) {
            ...
            revert("ERC721(ManeBase): Not in sale or presale period");
        } else if (block.timestamp >= presaleStartTime && block.timestamp < presaleEndTime) {
            // Period: Pre-sale period
            //require(msg.value >= _preSalePrice, "ERC721(ManeBase): insufficant value for presale period");
            require(msg.value >= 0, "ERC721(ManeBase): insufficant value for presale period");
            isPresale = true;
        } else if (block.timestamp >= saleStartTime && block.timestamp <= saleEndTime) {
            // Period: Public sale perild
            require(msg.value >= _salePrice, "ERC721(ManeBase): insufficant value for sale peroid");
            isSale = true;
        } else {
            revert("ERC721(ManeBase): Invalid _period");
        }
        
        require(totalMinted() < _maxSupply, "ERC721(ManeBase): No more tokens could be mint");

        uint256 expectPlatformFee = mint_price * platformFeePPM / 10e6 ;
        uint256 userGotAmount = mint_price - expectPlatformFee;
        // uint256 actualPlatformFee = msg.value - mint_price;
        // require(actualPlatformFee >= expectPlatformFee, "ERC721(ManeBase): insufficant platform fee");

        if (isPresale == true) {
            requireMintSign(minter, mint_price, sign_deadline, r, s, v);

            require(presaleMintedCount < presaleMaxSupply, "ERC721(ManeBase): No more tokens could be mint at presale period");
            presaleMintedCount++;

            require(presaleMintCountByAddress[msg.sender] < presaleMaxMintCountPerAddress, "ERC721(ManeBase): No more tokens could be mint at presale period by this address");
            presaleMintCountByAddress[msg.sender]++;
        } else if (isSale == true) {
            //requireMintSign(minter, mint_price, sign_deadline, r, s, v);

            require(saleMintCountByAddress[msg.sender] < saleMaxMintCountPerAddress, "ERC721(ManeBase): No more tokens could be mint at sale period by this address");
            saleMintCountByAddress[msg.sender]++;
        } else {
            revert("ERC721(ManeBase): No in sale time period");
        }
        ...
        mintedAmountForUser += userGotAmount;
        mintedAmountForUserNotRefunded += userGotAmount;


        // 4. Log events and other data
        TMintInfo memory info;
        info.price = mint_price;
        info.isRefunded = false;
        info.isPreMint = isPresale;
        info.minter = minter;
        _mintInfo[currentIndex] = info;
    ...
    function requireMintSign(address minter, uint256 price, uint256 deadline, bytes32 r, bytes32 s, uint8 v)  public view {
        bytes memory prefix = "\x19Ethereum Signed Message:\n32";
        bytes32 userHash = encodeMint(deadline, minter, price);
        bytes32 prefixHash = keccak256(abi.encodePacked(prefix, userHash));

        address hash_address = ecrecover(prefixHash, v, r, s);

        //require(msg.sender == minter, "LoveWall: Invalid sender in signature");

        require(hash_address == owner(), "LoveWall: Invalid signature");
    }


    function encodeMint(uint256 deadline, address minter, uint256 price) public pure returns (bytes32) {
        return keccak256(abi.encodePacked(deadline, minter, price));
    }
    ...
    function setMaxSupply(uint256 newMaxSupply) public onlyOwner {
        require(newMaxSupply >= totalMinted(), "ERC721(ManeBase): maxSupply must bigger than totalMinted()");
    ...
    //     require(preSaleStartTime_ <= saleStartTime_, "ERC721(ManeBase): preSaleStartTime must be smaller than saleStartTime");
    ...
    function adminSetRefund(bool is_refundable_) public onlyFactoryOwner {
        isRefundable = is_refundable_;
    }

    /// User request to refund
    function refund(uint256 tokenID) public {
        require(isRefundable == true);
        require(msg.sender == ownerOf(tokenID), "ERC721(ManeBase): You are not the owner of the token");
        ...
        require(refundRatioPPM > 0, "ERC721(ManeBase): Unable to refund, refund is not available now or refund time is exceeded");

        /// 2. Get mint info and check if this token is refundable
        TMintInfo storage mintInfo = _mintInfo[tokenID];
        
        require(mintInfo.isRefunded == false, "ERC721(ManeBase): This token is already refunded");

        /// 3. Caculate the refundable value
        uint256 refundValue = mintInfo.price * refundRatioPPM / 1000 / 1000; 

        /// 4. Do refund
        mintedAmount -= refundValue;
        mintedAmountForUser -= refundValue;

        transferFrom(msg.sender, this.owner(), tokenID);
        
        mintInfo.isRefunded = true;
    ...
    function collect(uint256 val) public {
        /// 1. Find the collector
        TShare storage collector = _shareList[0];
        address to = address(0);

        for (uint256 i = 0; i < _shareList.length; i++) {
            if (msg.sender == _shareList[i].owner) {
                to = msg.sender;
        ...
        require(to != address(0), "ERC721(ManeBase): Not a collector");
        require(isRefundable == false, "ERC721(ManeBase): Refunding in progress, unable to collect");

        uint256 availAmount = availableCollectAmount();

        require(val <= availAmount, "ERC721(ManeBase): Request collect amount exceed");
        require(collector.owner == msg.sender);

        collector.collectAmount = collector.collectAmount + val;

        payable(collector.owner).transfer(val);
    ...
    function availableCollectAmount() view public returns (uint256) {
    ...
    function destory() public onlyOwner {
        require(mintedAmount == 0, "ERC721(ManeBase): selfdestruct is not allow after mint");
...
contract ManeFactory is SignAndOwnable {
    ...
    string public factoryBaseURI = "https://manestudio.com/nft/";
    ...
    function deploy(string memory name_, string memory symbol_,  uint256[] memory u256s, address[] memory shareAddresses_, uint256[] memory shareRatios_, uint256[] memory refundEndTimes_, uint256[] memory refundRatios_, uint8 v, bytes32 r, bytes32 s) public returns (address) {
        ...
        require(_usedNonces[u256s[12]] == false, "ManeFactory: Signature nonce is used");
