...
contract PrecogV5 is IPrecogV5, ReentrancyGuard {
    ...
    modifier onlyPrecogCore() {
        require(
            msg.sender == precogStorage.getPrecogCore(),
            "PrecogV5: Only Precog Core"
        );
        _;
    }
    ...
    modifier isExistingToken(address token) {
        require(
            ...
            "PrecogV5: Token must be added to pool"
        );
        _;
    }
    ...
    function _getWithdrawalRegisterInstance()
        internal
        view
        returns (IWithdrawalRegister _register)
    {
    ...
    function _processHandleIncreaseInvestment(
        ...
        if (precogStorage.getInvestmentsOf(_token, _account).length == 0) {
            ...
            IPrecogStorage.Investment memory _lastInvestmentOf = precogStorage
                .getLastInvestmentOf(_token, _account);
            IPrecogStorage.Cycle memory _futureTradingCycle = precogInternal
                .getTradingCycleByTimestamp(_token, _nextFundingTime);
            ...
            if (_futureTradingCycle.id >= _lastInvestmentOf.idChanged) {
                ...
                IWithdrawalRegister.Register
                    memory _register = _getWithdrawalRegisterInstance().getRegister(
                        _token,
                        _account
                    );
                if (_amount < _register.amount) {
                    ...
                    for (
                        uint i = _futureTradingCycle.id + 1;
                        i <= _lastInvestmentOf.idChanged;
                        i++
                    ) {
    ...
    function _increaseLiquidity(address _token, uint _amount) internal {
        precogStorage.updateLiquidity(
            _token,
            precogStorage.getLiquidity(_token) + _amount
        );
        if (precogStorage.checkIsInWhitelist(_token, msg.sender)) {
            precogStorage.updateLiquidityWhitelist(
                _token,
                precogStorage.getLiquidityWhitelist(_token) + _amount
            );
        }
    }
    ...
    {
        require(
            amount >= _getCoreInstance().minFunding(token),
            "PrecogV5: Amount must be greater than min funding amount"
        );
        precogInternal.updateCurrentTradingCycle(token, false, 0);
        // Calculate fees and actual deposit amount
        uint feeDeposit = (amount * _getCoreInstance().getFeeConfiguration().depositFee) /
            10**_getCoreInstance().feeDecimalBase();
        uint actualDepositAmount = amount - feeDeposit;
        uint depositAmount = _processHandleIncreaseInvestment(
            token,
            msg.sender,
            actualDepositAmount
        );
        uint amountInvestment = precogStorage
            .getLastInvestmentOf(token, msg.sender)
            .amount;
        require(
            amountInvestment <= _getCoreInstance().maxFunding(token),
        ...
        require(
            precogStorage.getLiquidity(token) <= _getCoreInstance().maxFundingPool(token),
            "PrecogV5: Liquidity amount must be less than limitation of pool"
        );
        // Transfer token amount and fees
        IERC20(token).safeTransferFrom(
            msg.sender,
            precogStorage.getPrecogVault(),
            depositAmount
        );
        IERC20(token).safeTransferFrom(
            msg.sender,
            precogStorage.getPrecogCore(),
            feeDeposit
        );
    ...
    function _processHandleDecreaseInvestment(
        ...
        IPrecogCore.CyclesChangedInfo memory _cycleChangedInfo = _getCoreInstance()
            .getCyclesChangedInfo(_token);
        ...
        if (_isFirstRequestWithdrawal) {
            ...
            if (_getWithdrawalRegisterInstance().isInDeadline(_token, msg.sender)) {
                _nextDefundingTime = uint48(
                    _getWithdrawalRegisterInstance()
                        .getRegister(_token, msg.sender)
                        .deadline
                );
            } else {
        ...
        _remainingAmount = precogInternal.decreaseInvestment(
            _token,
            msg.sender,
            _amount,
            _nextDefundingTime
    ...
    {
        require(
            amount >= _getCoreInstance().minDefunding(token),
            "PrecogV5: Amount must be greater than min defunding amount"
        );
        precogInternal.updateCurrentTradingCycle(token, false, 0);
        bool isFirstRequestWithdrawal = _getWithdrawalRegisterInstance().isFirstWithdraw(
            token,
            msg.sender
        );
        uint remainingAmount = _processHandleDecreaseInvestment(
            token,
            amount,
            isFirstRequestWithdrawal
        );
        require(
            remainingAmount == 0 ||
                remainingAmount >= _getCoreInstance().minFunding(token),
    ...
    function _registerWithdrawal(
        ...
        IPrecogCore.CyclesChangedInfo memory _cycleChangedInfo = _getCoreInstance()
            .getCyclesChangedInfo(_token);
        uint48 _duration = _isFirstRequestWithdrawal
            ? _getCoreInstance().getCycleConfiguration().firstDefundingCycle
            : 0;
        ...
        uint48 _locktime = _getWithdrawalRegisterInstance().isInDeadline(_token, _account)
            ? uint48(
                _getWithdrawalRegisterInstance().getRegister(_token, _account).deadline
            )
            : _nextDefundingTime;

        _getWithdrawalRegisterInstance().registerWithdrawal(
            _token,
            _account,
            _amount,
            _locktime
    ...
    function withdraw(
        ...
        uint withdrawalFee = (amount *
            _getCoreInstance().getFeeConfiguration().withdrawalFee) /
            10**_getCoreInstance().feeDecimalBase();
        IIPCOG(precogStorage.getTokenConvert(token)).burnFrom(msg.sender, amount);
        if (isEmergency) {
            _getVaultInstance().forceWithdraw(
                token,
                msg.sender,
                to,
                amount,
            ...
            _getWithdrawalRegisterInstance().claimWithdrawal(
                token,
                msg.sender,
                to,
                amount,
    ...
    function turnOnWhitelist(address token, address account) external {
        require(
            !precogStorage.checkIsInWhitelist(token, account),
            "This account is current whitelist"
        );
        ...
        precogStorage.updateLiquidityWhitelist(
            token,
            precogStorage.getLiquidityWhitelist(token) + amount
        );
    }
    ...
    function turnOffWhitelist(address token, address account) external {
        require(
            precogStorage.checkIsInWhitelist(token, account),
            "This account is not current whitelist"
        );
