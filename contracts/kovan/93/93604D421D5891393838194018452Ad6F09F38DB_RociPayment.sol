...
import {ONE_HUNDRED_PERCENT, CONTRACT_DECIMALS} from "../Globals.sol";
import {ScoreDBInterface} from "../interfaces/ScoreDBInterface.sol";
  ...
  ERC20PaymentStandard
{
    ...
    returns (uint256 principal, address borrower)
  {
    ...
    (uint256 collateral, uint256 collateralLTV,) = getCollateralLTVandLTAbsoluteValues(borrower);
    require(collateralLTV != 0 && collateral >= collateralLTV, Errors.PAYMENT_NOT_ENOUGH_COLLATERAL);
  }

  function getCollateralData(address user) public view returns (uint256, uint8, int256, uint8) {
    (address collateralContract, uint256 collateral) = collateralManager()
      .getCollateralLookup(address(this), user);
    (int256 priceCollateral, uint8 feederDecimalsCollateral) = _safeGetPriceOf(collateralContract);
    return (
      collateral,
  ...
  function claimCollateral(
    ...
    require(getBalanceOfCollateral(msg.sender) >= int256(_amount), Errors.PAYMENT_CLAIM_COLLATERAL);
  ...
  function getMaxWithdrawableCollateral()
    ...
    returns (uint256)
  {
    int256 amount = getBalanceOfCollateral(msg.sender);
    return amount < 0 ? 0 : uint256(amount);
  }

  function getBalanceOfCollateral(address user)
    internal
    view
    whenNotPaused
    returns (int256) 
  {
    ...
    return collateralLTV == 0 ? 
      int256(collateral) :
      int256(collateral) - int256(collateralLTV);
  }

  function calculateCollateralFromOutStanding(int256 outstandingUSD, uint256 parameter)
    internal
    pure
    returns (int256) 
  {
    return parameter == 0 ?
      int256(0) :
      outstandingUSD * int256(ONE_HUNDRED_PERCENT) /
      int256(parameter);
  }
    ...
    returns (uint256, uint256, uint256)
  {
    ...
    int256 collateralLTV = fromFeedPriceToToken(
      calculateCollateralFromOutStanding(outstandingUSD, ltvMean),
      collateralDecimals, 
      feedPrice, 
      feedDecimals
    );
    int256 collateralLT = fromFeedPriceToToken(
      calculateCollateralFromOutStanding(outstandingUSD, ltMean),
      collateralDecimals, 
      feedPrice, 
      feedDecimals
    );
    return (collateral, uint256(collateralLTV), uint256(collateralLT));
  }
    ...
    returns (bool)
  {
    ...
    (uint256 collateral, , uint256 collateralLT) = getCollateralLTVandLTAbsoluteValues(_loanLookup[_id].borrower);
    ...
    returns (int256, uint256, uint256)
  {
    ...
    int256 outstandingUSD;
    uint256 ltvAggregated;
    uint256 ltAggregated;
    uint256 ltvMean;
    uint256 ltMean;
    if (activeLoans.length > 0) {
      ...
      for (uint256 i = 0; i < activeLoans.length; i++) {
        ...
        outstandingUSD += int256(loan.getOutstanding());
        ltvAggregated += loan.ltv;
        ltAggregated += loan.lt;
      }

      (int256 priceLoan, uint8 feederDecimalsLoan) = _safeGetPriceOf(erc20Address);
      ...
      ltvMean = ltvAggregated / activeLoans.length;
      ltMean = ltAggregated / activeLoans.length;
    }
    return (outstandingUSD, ltvMean, ltMean);
  }

  /**
   * @dev converts from token value to price feed value
   * @param value native value of token
   * @param decimals native decimals of token
   * @param price price of token from priceFeed
   * @param decimals decimals from priceFeed
   * @return int256 price feed value
   */
  function fromTokenToFeedPrice(
    ...
    int256 converted = (price * value) / int256(10**feedDecimals);

    int8 decimalsDifference = int8(CONTRACT_DECIMALS) - int8(decimals);

    if (decimalsDifference != 0) {
      converted *= int256(10**uint8(decimalsDifference));
    }
   ...
   * @param decimals native decimals of token
   * @param price price of token from priceFeed
   * @param feedDecimals decimals from priceFeed
   * @return int256 native token value
   */
  function fromFeedPriceToToken(
    ...
    int256 converted = (int256(10**feedDecimals) * value) / price;
  ...
  Version
{
   ...
   */
  function missedPayment(uint256 _id)
    public
    view
    virtual
    override
