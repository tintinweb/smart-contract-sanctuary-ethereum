...
contract UtilToken is ERC721AUpgradeable, BlackListUpgradeable, PausableUpgradeable, OwnableUpgradeable, ReentrancyGuardUpgradeable, IUtilToken{
    ...
    function mint(bytes32[] calldata _merkleProof) external payable override whenNotBlackListedUser(_msgSender()) nonReentrant whenNotPaused {
        ...
        Batch memory currentBatch = latestBatch();
        uint256 batchArrayIndex = batchCount().sub(1);
        bool privateRound;
    ...
    function addBatch(uint256 _startTime, uint256 _endTime, uint256 _mintPrice, bytes32 _merkleHash) external override onlyOwner whenNotPaused {
        ...
        if(batchCount() > 0)
            require(latestBatch().endTime < _startTime, "New Batch start time should be after previous batch end time");
        ...
        emit NewBatchCreated(batchCount());
    }
    ...
    function updateCurrentBatch(uint256 _endTime) external override onlyOwner whenNotPaused {
        Batch storage currentBatch = latestBatch();
    ...
    /// @dev The internal function for getting the index of the batch array

    function latestBatch() internal view returns(Batch storage){
        require(batches.length > 0, "No batch has been started");
        uint256 batchIndex = batches.length.sub(1);
        return batches[batchIndex];
    }

    /// @dev The internal function for getting the length of the batch array

    function batchCount() internal view returns(uint256){
    ...
    function withdrawToAddress(address recipient) external override onlyOwner nonReentrant whenNotPaused{
        uint256 contractBalance = address(this).balance;
    ...
    function isTokenAirDropped(uint256 tokenID) public view override returns (bool){
        return _airDroppedTokens[tokenID];
    }
    ...
    /// @dev Modifier to check whether a token is airdropped or not
    ...
    IERC721EnumerableUpgradeable
{
    ...
    /**
     * @dev Burns `tokenId` token and transfers them to zero address.
     *
     * Requirements:
     *
     * - `quantity` must be greater than 0.
    ...
    function _burn(uint256 tokenId) internal virtual {
        ...
        // Clear approvals
        _approve(address(0), tokenId, _msgSender());
