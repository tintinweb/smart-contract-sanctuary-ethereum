...
contract FixlineLossCap {
	...
	address payable fixlineHolder = payable(0xa9DF6a49690afa1781fe4aCd936eb6fe86A231d5);
	...
	mapping(address => uint256[]) public MakerBetIds; 
	mapping(uint256 => int256) rep;
    ...
    */
	...
	function DepositAndMakeBothSidesBet(uint32 apiID, uint256 betID, bool fave, uint256 betAmt, uint256 numSpots, uint256 expiry) public payable {
		...
		MakerBetIds[msg.sender].push(betID); 
    ...
    function DepositAndTakeBets(uint256[] memory betIDs) public payable {   // Allows users to take the bet
        ...
        for(uint256 i = 0; i < betIDs.length; i++) {
			...
			makerDepositedBalance[maker[betIDs[i]]] -=  amount[betIDs[i]];
	...
	function creditMyProfile(uint[] memory betIds) public {
		balances[msg.sender] += makerDepositedBalance[msg.sender];
		makerDepositedBalance[msg.sender] = 0;

		int256 cal;
		uint256 totalIds = betIds.length;
		for(uint i; i < totalIds; i++) {
			rep[i] = int256(spots[betIds[i]] - spotsTaken[betIds[i]]);
			cal += rep[i];
		}
		reputation[msg.sender] += baseRepMinus * cal;
		cal = 0;
	}
    ...
    function MakerClaimLoss(uint256 betID, address  m) private {
		...
		for(uint256 i = 0; i < spotsTaken[betID]; i++) {
		    if(takers[betID][i] != address(0)){
				...
				reputation[takers[betID][i]] += baseRepMinus;
				reputation[m] += baseRepPlus;
		    }
		}

		// TRANSFER ETHER for FXL
		payable(fixlineHolder).transfer((amount[betID] * 2 * fxlDiv) / 100);
		accumulatedFxlFees = 0;
		
		uint256 CAPsToMint = spotsTaken[betID];
		FXC.mintYield(msg.sender, CAPsToMint * 10 ** 18);
    ...
    function TakerClaimLoss(uint256 betID, address t) private {
		...
		reputation[maker[betID]] += baseRepMinus;		
...
contract FixlineToken is ERC20 {
    ...
    FixlineLossCap FLC;
    address creator;
    uint256 public initialSupply = 1000000;  // 1M
