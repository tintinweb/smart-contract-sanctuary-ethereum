...
contract TokenStaking is Ownable, Pausable {
    ...
    constructor (address _wallet, address penky, address elite) {
        ...
        planDetails[2] = PlanInfo({planID: 2, planDays: 182 * 86400, planReward: 40, planReferalReward: 20, planFee: 1 * 1e16 });
        planDetails[3] = PlanInfo({planID: 3, planDays: 365 * 86400, planReward: 100, planReferalReward: 50, planFee: 2 * 1e16 });
    }
    ...
    function withdraw(uint256 _stakeID) external payable whenNotPaused {
        ...
        IERC20(user.stakeToken).transfer(msg.sender, user.amount);
