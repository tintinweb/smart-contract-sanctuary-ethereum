...
contract FrankTreasury is Ownable {
    ...
    function _addPoolAndFarmLiquidity2(uint256 _amount, address _pool) public {
        ...
        uint256 minAmountOut = ((TraderJoeRouter.getAmountsOut((_amount / 2), path)[1] * 95) / 100);
        uint256 amountOutB = (TraderJoeRouter.swapExactTokensForTokens((_amount / 2), minAmountOut, path, address(this), (block.timestamp + 1000)))[1];

        IERC20(tokenA).approve(address(TraderJoeRouter), _amount / 2);
        IERC20(token1).approve(address(TraderJoeRouter), amountOutB);
        
        uint y = ((_amount / 2) * 95 / 100);

        TraderJoeRouter.addLiquidity(tokenA, tokenB, _amount / 2, amountOutB, y, amountOutB, address(this), block.timestamp + 1000);
        ...
        if(token1 != address(JOE)) {
            JOE.approve(address(TraderJoeRouter), _amount / 2); 
            path[1] = token1;
            TraderJoeRouter.swapTokensForExactTokens(amountBOptimal, _amount / 2, path, address(this), (block.timestamp + 1000));
        }
   /*   
