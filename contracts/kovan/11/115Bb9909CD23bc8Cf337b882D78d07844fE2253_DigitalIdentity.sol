...
contract DigitalIdentity is Ownable {
    ...
    function getDirectoryNames() public view returns (string[] memory) {
        string[] memory ret = new string[](directory.length);

        console.log("GDN ", directory.length);

        for (uint256 i = 0; i < directory.length; i++) {
            console.log("Add ", identities[directory[i]].name);
            ret[i] = identities[directory[i]].name;
        }
        return ret;
    }
