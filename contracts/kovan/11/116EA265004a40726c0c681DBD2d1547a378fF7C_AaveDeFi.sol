...
import './IPriceOracle.sol';
 ...
 contract AaveDeFi {
    ...
    // mapping(address => mapping(address => uint256)) public totalTokenBorrows;

    // Keep track latest DAI/ETH price
    // uint256 public tokenEthprice;
    mapping(address => uint256) public tokenEthprice;
    ...
    event DepositBorrow(
        ...
        uint256 priceToken, 
        uint256 safeMaxTokenBorrow 
        // uint256 totalTokenBorrows
        );
    ...
    function borrowTokenAgainstETH(address _tokenAddress) external payable {
        ...
        address tokenAddress = _tokenAddress;  //kovan token
        ...
        uint priceToken = priceOracle.getAssetPrice(tokenAddress);
        tokenEthprice[tokenAddress] = priceToken;

        // Calculate the maximum safe DAI value you can borrow
        assert(priceToken != 0);
        uint safeMaxTokenBorrow = ltv * msg.value * (10**18) / (priceToken * 100); // remember scaling in front end
        
        uint256 userToken = safeMaxTokenBorrow*99/100;
        // uint256 contractDAI = safeMaxDAIBorrow*1/100;
        // Borrow the safeMaxDAIBorrow amount from protocol
        // function borrow(address asset, uint256 amount, uint256 interestRateMode, uint16 referralCode, address onBehalfOf)
        lendingPool.borrow(tokenAddress, safeMaxTokenBorrow, variableRate, referralCode, address(this));

        // Send the borrowed DAI to borrower
        // require(IERC20(daiAddress).transfer(msg.sender, safeMaxDAIBorrow));
        require(IERC20(tokenAddress).transfer(msg.sender, userToken));

        // require(IERC20(daiAddress).transferFrom(address(this), msg.sender,safeMaxDAIBorrow));


        // Update daiBorrowBalances
        // totalTokenBorrows[msg.sender][tokenAddress] = totalTokenBorrows[msg.sender][tokenAddress] + safeMaxTokenBorrow;
        ...
        emit DepositBorrow(
            ...
            priceToken, 
            safeMaxTokenBorrow
            // totalTokenBorrows[msg.sender][tokenAddress]     
        );
    }
