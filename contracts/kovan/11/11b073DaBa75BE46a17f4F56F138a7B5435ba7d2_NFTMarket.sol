...
contract NFTMarket is Ownable, ReentrancyGuard, IERC721Receiver {
    ...
    mapping(address => mapping(uint256 => uint256)) contractTokenIdToItemId;
    ...
    function updateListingFee(uint256 newListingFee)
        public
        payable
        nonReentrant
        onlyOwner
    ...
    function addItemToMarket(
        ...
        uint256 itemId = 0;
        MarketItem storage marketItem = itemIdToMarketItem[
            contractTokenIdToItemId[nftContract][tokenId]
        ];
        if (marketItem.nftContract == nftContract) {
            ...
            contractTokenIdToItemId[nftContract][tokenId] = itemId;
        }
        ...
        nonReentrant
    {
        ...
        IERC721(itemIdToMarketItem[itemId].nftContract).transferFrom(
            address(this),
            msg.sender,
            tokenId
        );
        ...
        priceHistory[itemId].push(price);
    }

    function removeItemFromMarket(uint256 itemId) public payable nonReentrant {
        require(
            itemIdToMarketItem[itemId].owner == address(this),
            "The item is not selling"
        );
        require(
            itemIdToMarketItem[itemId].seller == msg.sender,
            "You are not the owner of this item"
        );
        IERC721(itemIdToMarketItem[itemId].nftContract).transferFrom(
            address(this),
            msg.sender,
            itemIdToMarketItem[itemId].tokenId
        );
        itemIdToMarketItem[itemId].owner = payable(msg.sender);
    ...
    function getMarketItemByContractTokenId(
        ...
        return
            itemIdToMarketItem[contractTokenIdToItemId[nftContract][tokenId]];
    }
    ...
    function getMarketItems() public view returns (MarketItem[] memory) {
        ...
        for (uint256 i = 0; i < itemCount; i++) {
            ...
            if (itemIdToMarketItem[currentId].owner == address(this)) {
                MarketItem memory currentItem = itemIdToMarketItem[currentId];
    ...
    function getMyItems() public view returns (MarketItem[] memory) {
        ...
        for (uint256 i = 0; i < totalItemCount; i++) {
            if (
                itemIdToMarketItem[i + 1].seller == msg.sender &&
                itemIdToMarketItem[i + 1].owner == address(this)
            ) {
                itemCount += 1;
            }
        }
        ...
        for (uint256 i = 0; i < totalItemCount; i++) {
            uint256 currentId = i + 1;
            if (
                itemIdToMarketItem[currentId].seller == msg.sender &&
                itemIdToMarketItem[currentId].owner == address(this)
            ) {
                items[currentIndex] = itemIdToMarketItem[currentId];
                currentIndex += 1;
            }
        }
