...
contract MetaMech is ERC721AOwnersExplicit, Ownable {
    ...
    string public baseURIHash;
    uint256 public preSaleStartTime;
    uint256 public preSaleEndTime;
    mapping(address => bool) private whiteList;
    mapping(address => bool) private whiteListMinted;
    uint256 public mintedAmt; // minted amount
    address public foundAddr;
    event PresaleTime(uint256 now, uint256 start, uint256 end);
        ...
        ERC721A(_name, _symbol)
    {
        ...
        //TODO: Does it need to change this?
    ...
    function setPreSaleTime(uint256 _preSaleStartTime, uint256 _preSaleEndTime)
        ...
        onlyOwner
    {
        emit PresaleTime(block.timestamp, _preSaleStartTime, _preSaleEndTime);
     ...
     */
    function mintTo(address[] calldata _tos, uint256 amount) public onlyOwner {
        uint256 _mintedAmt = mintedAmt;
        uint256 mintAmt = _tos.length * amount;
        require(
            _mintedAmt + mintAmt <= MAX_LIMIT,
    ...
    function mint(uint256 num) public payable {
        uint256 _mintedAmt = mintedAmt;
        require(
            _mintedAmt + num <= MAX_SALE_LIMIT,
            "MetaMech: reached max limit at public sale"
        );
        require(
            preSaleStartTime > 0 && preSaleEndTime > 0,
            "MetaMech: preSaleStartTime and preSaleEndTime not set"
        );
        require(
        ...
        if (
            ...
            require(whiteList[msg.sender], "MetaMech: invalid whitelist user.");
            require(
    ...
    function setBaseURIHash(string calldata _baseURIHash) public onlyOwner {
...
abstract contract ERC721AOwnersExplicit is ERC721A {
    ...
    function _setOwnersExplicit(uint256 quantity) internal {
        ...
        if (_nextOwnerToExplicitlySet >= currentIndex)
