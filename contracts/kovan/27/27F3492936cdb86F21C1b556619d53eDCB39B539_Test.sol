...
library MCR {
    function compute(uint256 length, uint256[] memory policy, uint256[] memory leverage, uint256[] memory corr) internal returns (uint256 square) {
        square = 0;
        if (length == 0) {
            return square;
        }
...
contract Test {
    ...
    function computeMCR(uint256 length) external  {
        ...
        uint256 _square = MCR.compute(length, policy, leverage, corr);
