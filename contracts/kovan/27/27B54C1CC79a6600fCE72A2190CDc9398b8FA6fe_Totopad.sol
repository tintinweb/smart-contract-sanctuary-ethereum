    ...
    function changeProjectStatus(uint256 projectId, bool isAccept) external nonReentrant onlyRole(DEFAULT_ADMIN_ROLE) {
        ...
        if (isAccept) {
            ...
            if (block.timestamp > project.allocStartTime) {
                ...
                project.stakingEndTime = newStartTime + (project.stakingEndTime - oldStartTime);
                project.fundingEndTime = newStartTime + (project.fundingEndTime - oldStartTime);
                project.rewardStopTime = newStartTime + (project.rewardStopTime - oldStartTime);
