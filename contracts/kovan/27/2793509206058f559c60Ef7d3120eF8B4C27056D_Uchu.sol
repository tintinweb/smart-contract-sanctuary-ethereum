...
contract Uchu is Initializable {
    ...
    * ALL_BORROWED = 30. All amounts in the position are withdrawn by the borrower.
    * ALL_REPAID = 50. All amounts including interest are deposited by the borrower. 
    ...
    **/
    function deposit(address user, uint8 proId) public payable returns (bool) {
        if (productId > 0) {
            ...
            createLenderPosition(proId, OPEN, user, msg.value,  block.timestamp, 0);
            return true;
        } else {
    ...
    **/
    function createLenderPosition(uint8 proId, uint8 status, address user, uint256 amount, uint256 depositTime, uint256 withdrawTime) private {
    ...
    /**
    *  insert record by current borrower's positionIndex
    **/
    function createBorrowerPosition(uint8 proId, uint8 status, address user, uint256 amount, uint256 depositTime, uint256 withdrawTime) private {
        Borrower memory borrower = getCurBorrower(user);
        positions[msg.sender][borrower.posIndex] = Position(proId, status, 0, borrower.id, amount, depositTime, withdrawTime);
            for(uint32 i=0; i<borrowerId; i++) {
                if(msg.sender == borrowers[i].borrower) {
                    borrowers[i].posIndex++;
                    break;
                }
            }
    }
            ...
            if(positions[user][i].proId == proId) {
                ...
                if (pow >= 1) {
                    ...
                    if (10000 <= total && total < 100000)
                        percent = 10000; 
                    result += amount / (percent ** pow) * (total ** pow);
                } else {
                    result += amount;
                }
            }
    ...
    **/
    ...
    /**
    * Get the transaction order history.
    **/
    ...
    function withdraw(address user, uint256 amount, uint8 proId) public returns (bool) {
        ...
        if (balanceOf[user][pro.name] >= amount) {
            ...
            createLenderPosition(proId, CLOSED, user, amount, 0, block.timestamp);
    ...
    function borrow(address user, uint256 amount, uint8 proId) public returns (bool) {
        ...
        if (allowance[pro.name] >= amount) {
            ...
            createBorrowerPosition(proId, ALL_BORROWED, user, amount, block.timestamp, 0);
            payable(user).transfer(amount);
            return true;
        } else {
    ...
    **/
    function getCurBorrower(address user) private view returns (Borrower memory borrower) {
        for(uint32 i=0; i<borrowerId; i++) {
            if(user == borrowers[i].borrower) {
    ...
    function repay(address user, uint8 proId) public payable returns (bool) {
        ...
        if (debts[user][pro.name] >= msg.value) {
            ...
            createBorrowerPosition(proId, ALL_REPAID, user, msg.value, 0, block.timestamp);
            return true;
        } else {
