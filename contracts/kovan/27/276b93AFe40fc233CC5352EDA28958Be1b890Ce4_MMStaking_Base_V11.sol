...
/**
    ...
    @author ^M
 */
contract MMStaking_Base_V11 is
    ...
    ReentrancyGuard
{
    ...
    */
     ...
     */
    uint256 public interval = 18000; // default interval is each 5 hours
    ...
    /**
        @dev set the block timestamp to current time and counter to zero
    */
    constructor() {
    ...
    */
    function unstake(uint256 _stakeId) public nonReentrant {
        StakeInfo[] memory tempStakes = allStakes;
        if (
            _stakeId >= tempStakes.length ||
            ((tempStakes[_stakeId].holder != _msgSender()) &&
                _msgSender() != owner())
        ) {
            revert PermissionDenied();
        }

        StakeInfo memory _stake = tempStakes[_stakeId];
    ...
    */
    ...
    /**
        @dev setting a new interval for upkeep
        @param newInterval is the new interval to be set in seconds
     */
    function setInterval(uint256 newInterval) public onlyOwner nonReentrant {
    ...
    */
    ...
    {
        upkeepNeeded = false;
        StakeInfo[] memory tempAllStake = allStakes;
        for (uint256 i = 0; i < tempAllStake.length; i++) {
            StakeInfo memory _stake = tempAllStake[i];
            if (_stake.stakedDate + _stake.lockingPeriod < block.timestamp) {
                upkeepNeeded = true;
                break;
            }
        }
    ...
    function performUpkeep(bytes calldata performData) external override {
        ...
        if ((block.timestamp - lastTimeStamp) > interval) {
            lastTimeStamp = block.timestamp;
            // to reduce gas fee
            StakeInfo[] memory tempSakes = allStakes;

            for (uint256 i = 0; i < tempSakes.length; i++) {
                StakeInfo memory _stake = tempSakes[i];
                if (
                    _stake.stakedDate + _stake.lockingPeriod < block.timestamp
                ) {
                    uint256 j = 0;
                    // deleting it from holders mapping
                    uint256[] memory _holders = holders[_stake.holder];
                    for (j = 0; j < _holders.length; j++) {
                        if (_holders[j] == _stake.id) {
                            delete holders[_stake.holder][j];
                            break;
                        }
                    }
                    // deleting it from players mapping
                    uint256[] memory _players = players[_stake.holder];
                    for (j = 0; j < _players.length; j++) {
                        if (_players[j] == _stake.id) {
                            delete players[_stake.player][j];
                            break;
                        }
                    }

                    delete allStakes[_stake.id];
                    IERC721(_stake.collection).transferFrom(
                        address(this),
                        _stake.holder,
                        _stake.tokenId
