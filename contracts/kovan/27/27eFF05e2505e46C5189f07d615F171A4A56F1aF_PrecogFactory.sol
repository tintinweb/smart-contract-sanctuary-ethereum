...
// File: contracts\common\interfaces\IOwnable.sol
...
// File: contracts\precog\interfaces\IPrecogFactory.sol
...
// File: contracts\precog\PrecogFactory.sol


pragma solidity ^0.8.2;
contract PrecogFactory is IPrecogFactory {
    ...
    constructor(IPrecogStorage _precogStorage, IPrecogInternal _precogInternal, IIPCOGFactory _IPCOGfactory) {
    ...
    function _handleFirstTimeAddLiquidityPool(
        address _token, 
        IPrecogCore.CycleConfiguration memory _cycleConfig
    ) 
        internal 
        returns (IPrecogStorage.Cycle memory _currentTradingCycle) 
    {
        if (!precogStorage.checkIsNotFirstInvestmentCycle(_token)) {
            precogStorage.pushTradingCycle(_token, 
                IPrecogStorage.Cycle({
                    id: 0, 
        ...
        returns (IPrecogStorage.Cycle memory _currentTradingCycle) 
    {
        if (precogStorage.checkIsRemoved(_token) == true) {
            ...
            if (block.timestamp >= _lastTradingCycle.endTime) {
                IPrecogStorage.Cycle memory newCycle = IPrecogStorage.Cycle({
    ...
    function addLiquidityPool(address token) external override onlyAdmin {
        ...
        require(
            ...
            token != precogStorage.getPCOG() && token != address(0), 
        ...
        IPrecogCore.CycleConfiguration memory cycleConfig = _getCoreInstance().getCycleConfiguration();
        IPrecogStorage.Cycle memory currentTradingCycle;
        ...
        emit AddLiquidity(token, liquidityToken);
    }
    ...
    function removeLiquidityPool(address token) external override onlyAdmin isExistingToken(token) {
        ...
        emit RemoveLiquidity(token, liquidityToken);
