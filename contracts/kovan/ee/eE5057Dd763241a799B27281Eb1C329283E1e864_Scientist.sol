...
interface IScientistRepository {
    ...
    function getScientist(address _owner, uint256 _tokenId)
        external
        view
...
abstract contract AScientistRepository is IScientistRepository, Multicall {
    ...
    function _removeScientist(address _user, uint256 _tokenId) internal {
        require(
            _getScientist(_user, _tokenId).user != address(0),
            "Token not exists"
        );
        ...
        returns (ScientistData.Scientist memory)
    {
        ...
        require(
            addressToMap[_owner][_tokenId].user == _owner,
            "User is not the owner"
        );

        scientist = addressToMap[_owner][_tokenId];
        return scientist;
    }
}
    ...
    Ownable
{
    ...
    {
        tokenId.increment();
        _mint(account, tokenId.current());

        ScientistData.Scientist memory newCell = ScientistData.Scientist({
