...
contract RociPayment is ERC20CollateralPayment, IRociPayment{
    ...
    function hasSameScoreForActiveLoans(address user, uint256 nfcsId)
        external
        view
        override
        returns (bool)
    {
        ...
        uint16 score = ScoreDBInterface(lookup(Role.oracle)).getScore(nfcsId).creditScore;
        return score == _loanLookup[loans[0]].score;
    }
...
function getDataFromOracle(
    ...
    {
      ScoreDBInterface oracle = ScoreDBInterface(lookup(Role.oracle));
    ...
    returns (uint256 principal, address borrower)
  {
    ...
    PricedLoanInfo memory infoHolder = pricedLoanInfo(_id, false);

    require(
      // isWithinLimit args
      // total value of collateral (in USD) deposited by user across all loans in the protocol
      // total value of loans (in USD) yet to be repaid by user across all loans in the protocol
      // LTV for the user's NFCS token
      isWithinLimit(
        infoHolder.collateralUSD,
        infoHolder.outstandingUSD,
        infoHolder.ltv
      ),
  ...
  function claimCollateral(
    ...
    // Send _amount to the borrower (the msg.sender)
    ...
    for (uint256 i = 0; i < usersActiveLoans[msg.sender].length; i++) {
      // If a loan has not been deleted...
      if (
        _loanLookup[usersActiveLoans[msg.sender][i]].ERC20Address != address(0)
      ) {
        // Require that said deleted loan was not delinquent
        require(
          !isDelinquent(usersActiveLoans[msg.sender][i]),
          Errors.PAYMENT_CLAIM_COLLATERAL
        );
      }
    }
    // At this point, if any loan did become delinquent due to this withdrawal of collateral, the call will revert due to the require above
  ...
  // So, the amount right before _isDelinquent returns true for one of the user's loans
  function getMaxWithdrawableCollateral()
    ...
    returns (int256)
  {
    uint256[] memory activeLoans = usersActiveLoans[msg.sender];
    if (activeLoans.length == 0) {
      return 0;
    }
    // Fetch totalCollateral, collateralizedBy and collateralizedByUSD from pricedLoanInfo
    PricedLoanInfo memory infoHolder = pricedLoanInfo(
      // Send any loanId since pricedLoanInfo returns global details
      activeLoans[0],
      true
    );
    return int256(infoHolder.collateral) - int256(infoHolder.collateralizedBy);
  }
  ...
  function isDelinquent(uint256 _id)
    ...
    returns (bool)
  {
    // This isDelinquent checks if there are any missed/ late payments for this loan i.e., if it is delinquent
    if (super.isDelinquent(_id)) {
      return true;
    }
    // Fetch LT from the Loan object instead of the pricedLoanInfo as we need LT value for when this loan was taken
    PricedLoanInfo memory infoHolder = pricedLoanInfo(_id, true);
    // Now, check if global borrowed amount has crossed LT
    // If the LT has not been crossed i.e., TotalBorrowedAmount / TotalDepositedCollateral is not greater than LT, return true; otherwise return false
   ...
   * @param _id loan ID
   * @param _existing bool for whether caller wants info for an existing loan or a new loan
   ...
   */
  function pricedLoanInfo(uint256 _id, bool _existing)
    private
    view
    returns (PricedLoanInfo memory)
  {
    ...
    Loan.loan memory ln = _loanLookup[_id];
    // Create a var to store Info
    PricedLoanInfo memory infoHolder = PricedLoanInfo(0,0,0,0,0,0,0);
    // Fetch the ContractAddress and Amount of currently deposited collateral token for this loan and payment contract
    (address erc20Contract, uint256 collateral) = collateralManager()
      .getCollateralLookup(address(this), ln.borrower);
    infoHolder.collateral = collateral;
    // If isDelinquent or claimCollateral called, fetch LT and LTV from the Loan it self
    if (_existing) {
      infoHolder.ltv = ln.ltv;
      infoHolder.lt = ln.lt;
      infoHolder.score = ln.score;
    } else {
      // Fetch LT, LTV and NFCS from ScoreDB
      (infoHolder.ltv, infoHolder.lt, infoHolder.score) = getDataFromOracle(
        ln.nfcsID,
        ln.ERC20Address,
        true
      );
    ...
    // Fetch the price and decimals for the Collateral token Contract
    (int256 price, uint8 feederDecimals) = _safeGetPriceOf(erc20Contract);
    // Calculate the diff, if any, bw the decimals of the CollateralToken contract and the BorrowedToken contract
    int8 decimalsDifference = int8(IERC20Metadata(erc20Contract).decimals()) -
      int8(IERC20Metadata(ln.ERC20Address).decimals());
    // Calculate the collateral amount in USD using collateral amount, its price from oracles, deci difference if any, decimals of feeder
    infoHolder.collateralUSD = calculateCollateralUSD(
      collateral,
      price,
      decimalsDifference,
      feederDecimals
    ...
    uint256 sumOfLTVs = 0;
    uint256[] memory activeLoans = usersActiveLoans[ln.borrower];
    for (uint256 i = 0; i < activeLoans.length; i++) {
      // Fetch the loan
      Loan.loan memory loan = _loanLookup[activeLoans[i]];
      sumOfLTVs += loan.ltv;
      // Fetch price and decimals for the token which was borrowed
      (int256 _price, uint8 _feederDecimals) = _safeGetPriceOf(
        loan.ERC20Address
      );
      infoHolder.outstandingUSD +=
        // Fetching outstanding for each loanId
        (_price * int256(loan.getOutstanding())) /
        int256(10**uint256(_feederDecimals));
    }
    uint256 collateralizedByUSD =
        uint256(infoHolder.outstandingUSD * int256(ONE_HUNDRED_PERCENT)) /
        (sumOfLTVs / activeLoans.length);
    infoHolder.collateralizedBy = calculateCollateralFromUSD(
      collateralizedByUSD, 
      price, 
      decimalsDifference, 
      feederDecimals
   ...
   */
  function calculateCollateralUSD(
    uint256 collateralValue,
    int256 price,
    int8 decimalsDifference,
    uint8 feederDecimals
  ) internal pure returns (int256) {
    int256 collateralUSD = ((price * int256(collateralValue)) /
      int256(10**uint256(feederDecimals)));
  ...
  function calculateCollateralFromUSD(
    uint256 collateralUSD,
    int256 price,
    int8 decimalsDifference,
    uint8 feederDecimals
  ) internal pure returns (uint256) {
    uint256 collateral = (10**uint256(feederDecimals) * collateralUSD) /
      uint256(price);

    if (decimalsDifference > 0) {
      collateral *= 10**uint8(decimalsDifference);
    } else if (decimalsDifference < 0) {
      collateral /= 10**uint8(-decimalsDifference);
    }
    return collateral;
  ...
  function _liquidate(uint256 _id, address _receiver) internal virtual {
    (, uint256 amount) = collateralManager().getCollateralLookup(
      address(this),
  ...
  function isWithinLimit(
    // Total collateral amount deposited by this NFCS (user)
    int256 collateralUSD,
    // Amount already borrowed by this NFCS (user)
    int256 outstandingUSD,
    // LTV or LT for this NFCS
    uint256 parameter
  ) internal pure returns (bool) {
    return
      collateralUSD == 0
        ? false // (Already borrowed amount x 10^18) / collateral being deposited for this loan should be less or equal to the LTV
        : ((outstandingUSD * int256(ONE_HUNDRED_PERCENT)) / collateralUSD <=
          int256(parameter));
  }
    ...
    if (_ln.status == Loan.Status.NEW) {
      ...
      for (uint256 i = 0; i < usersActiveLoans[_ln.borrower].length; i++) {
        ...
        if (usersActiveLoans[_ln.borrower][i] == _id) {
            usersActiveLoans[_ln.borrower][i] = usersActiveLoans[_ln.borrower][usersActiveLoans[_ln.borrower].length - 1];
...
interface IRociPayment is IERC20CollateralPayment {
    ...
    function hasSameScoreForActiveLoans(address user, uint256 nfcsId) external returns (bool); 
}
  ...
  Version
{
  ...
  function configureNew(
    ...
    uint LTV = oracle.LTV(_erc20, score);
    uint LT = oracle.LT(_erc20, score);
