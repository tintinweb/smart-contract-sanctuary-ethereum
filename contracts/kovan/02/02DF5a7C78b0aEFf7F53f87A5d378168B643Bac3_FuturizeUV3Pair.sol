...
contract FuturizeUV3Pair is BasePool {
	...
	function _updatePos(
		...
		// console.log('to borrow value: %s collateral*leverage', borrow, collateral * leverage);
...
library UniswapV3SwapHelper {
	...
	function initializeSwapHelper(
		...
		require(address(pool) != address(0), 'SwapHelper: uniswap pool not found for pair');
	...
	function getToken0Price(address pool) public view returns (uint256 price) {
		...
		return mulDiv(uint256(sqrtPriceX96)**2, 1e18, 2**192);
	}

	function getToken1Price(address pool) public view returns (uint256 price) {
		(uint160 sqrtPriceX96, , , , , , ) = IUniswapV3Pool(pool).slot0();
		return mulDiv(2**192, 1e18, uint256(sqrtPriceX96)**2);
	}
	...
	{
		token0Price = mulDiv(uint256(sqrtPriceX96)**2, 1e18, 2**192);

		token1Price = mulDiv(2**192, 1e18, uint256(sqrtPriceX96)**2);
	}
