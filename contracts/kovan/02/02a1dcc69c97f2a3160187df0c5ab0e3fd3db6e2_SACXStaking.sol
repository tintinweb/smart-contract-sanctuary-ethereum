...
//the respective prices
     function stakeSACX(string memory _synth, uint256 amount) external  {
...
//function unstakes SACX for a given amount of synth provided by user
   function UnstakeSACX(string memory _synth, uint256 amount) external
   {
...
//by adding total supply of each synth
 ...
 //function stores the synthetic asset addresses and info to use in the system
    function SetSynthInfo(string  memory name,ISynth addr) public {
...
//function stores the synthetic asset addresses and info to use in the system
    function GetSynthInfo(string  memory name) public view returns(ISynth,string memory){
        
        bytes32 sname = stringToBytes32(name);
        string memory sym=getSynthAddress[sname].symbol;
        ISynth saddr=getSynthAddress[sname]._contractAddress;
        return (saddr,sym);
    }
