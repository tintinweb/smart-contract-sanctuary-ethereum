...
// File: interfaces/IRootChainManager.sol
...
abstract contract IAllowance {
    event AllowanceAdded(address);
    event AllowanceRemoved(address);
    mapping(address => bool) private allowedOperators;

    modifier allowedOperator() {
        require(allowedOperators[msg.sender], "Caller is not allowed");
        _;
    }
...
interface IFactoryERC721 {
    ...
    event MetaCellMintSucceed(address _toAddress);
    ...
    event NewScientistCreated(ScientistData.Scientist);
...
// File: interfaces/IRepository.sol
 ...
 */
abstract contract IRepository is Multicall {
    event ScientistAdded(ScientistData.Scientist);
    event ScientistUpdated(ScientistData.Scientist);
    event ScientistRemoved(uint256);

    using SafeMath for uint256;
    using Counters for Counters.Counter;

    mapping(address => mapping(uint256 => ScientistData.Scientist))
        public s_addressToMap;
    ...
    function _addScientist(ScientistData.Scientist memory _scientist) internal {
        require(
            _getScientist(_scientist.user, _scientist.tokenId).user ==
                address(0),
        ...
        s_userIndexesArray[_scientist.user].push(_scientist.tokenId);
        emit ScientistAdded(_scientist);
    }
    ...
    function _removeScientist(address _user, uint256 _tokenId) internal {
        ...
        require(
            s_addressToMap[_user][index].user == _user,
        ...
        emit ScientistRemoved(_tokenId);
    }
     ...
     */
    function getUserScientistsIndexes(address _user)
        external
        view
    ...
    function _updateScientist(
        ...
        emit ScientistUpdated(_scientist);
    }
        ...
        returns (ScientistData.Scientist memory)
    {
        ...
        require(
            s_addressToMap[_owner][index].user == _owner,
    ...
    Ownable
{
     ...
     */
    function mintTo(address _to, uint256 _optionId) external onlyOwner {
        _mint(_to, _optionId);
    }

    function baseTokenURI() external pure virtual returns (string memory);
...
contract ScientistsFactory is IFactoryERC721, Ownable {
    event RepositoryAddressChanged(address);
    event MarketplaceAddressChanged(address);
    ...
    address private metaCellAddress;
    address private predicateAddress;
    address private rootChainManager;

    /*
     * Enforce the existence of only 10k scientists; for tests 1k
     */
    uint256 SCIENTISTS_NUM_OPTIONS = 1000;
    uint256 lastIndex;

    constructor(address _proxyRegistryAddress, address _scientistNftAddress) {
        require(
            _proxyRegistryAddress != address(0),
            "Empty address of proxyRegister"
        );
        require(
            _scientistNftAddress != address(0),
            "Empty address of scientist"
        );
        proxyRegistryAddress = _proxyRegistryAddress;
    ...
    function fireTransferEvents(address _from, address _to) private {
        uint256 _scientistAmount = SCIENTISTS_NUM_OPTIONS;
        for (uint256 i = 0; i < _scientistAmount; i++) {
            /*
             * This event needed for OpenSea marketplace,
             * the platform uses it to display NFTs
             */
            emit Transfer(_from, _to, i);
        }
        lastIndex = _scientistAmount - 1;
    }

    //Function to add new tokens to the existing collection
    function addTokens(uint256 _amount) external onlyOwner {
        SCIENTISTS_NUM_OPTIONS = SCIENTISTS_NUM_OPTIONS + _amount;
        uint256 _scientistAmount = SCIENTISTS_NUM_OPTIONS;
        for (uint256 i = lastIndex; i < _scientistAmount; i++) {
    ...
    function setRepository(address _repository) external onlyOwner {
        ...
        emit RepositoryAddressChanged(repository);
    }
    ...
    function setMarketplace(address _marketplace) external onlyOwner {
        ...
        emit MarketplaceAddressChanged(marketplace);
    }
    ...
    function setToken(address _token) external onlyOwner {
        require(_token != address(0), "Empty address");
        scientistNftAddress = _token;
        emit TokenAddressChanged(repository);
    }

    /**
     * @dev Sets MetaCell token address
     * can be called only by contract owner
     */
    function setMetaCellToken(address _token) external onlyOwner {
        metaCellAddress = _token;
    }

    /**
     * @dev Sets Predicate address for using in approve functions
     * Predicate is using in Ethereum-Polygon bridge
     */
    function setPredicate(address _predicate) external onlyOwner {
        predicateAddress = _predicate;
    }

    /**
     * @dev Sets ProxyRegistry address
     * used for OpenSea whitelisting
     */
    function setProxyRegistry(address _proxy) external onlyOwner {
        proxyRegistryAddress = _proxy;
    }

    /**
     * @dev Sets RootChainManager address
     * RootChainManager using in Ethereum-Polygon bridge
     */
    function setRootChainManager(address _rcm) external onlyOwner {
        rootChainManager = _rcm;
    }

    /**
     * @dev returns scientist info by it's id
    ...
    function getScientistByID(uint256 _id, address _owner)
        ...
        returns (ScientistData.Scientist memory)
    {
        ScientistData.Scientist memory scientist = IRepository(repository)
            .getScientist(_id, _owner);
        require(scientist.user != address(0), "Token not exists");
    ...
    function getUri(uint256 _optionId) internal pure returns (string memory) {
        return
            string(
                abi.encodePacked(
    ...
    function createScientist(address _toAddress, uint256 _optionId)
        internal
        returns (ScientistData.Scientist memory)
    {
        ...
        IRepository(repository).addScientist(newScientist);

        return newScientist;
    }

    // mint Scientist token
    function mint(uint256 _optionId, address _toAddress)
        public
        override
        returns (uint256)
    {
        ...
        require(canMint(_optionId), "You cannot mint right now");

        Scientist(scientistNftAddress).mintTo(_toAddress, _optionId);

        // Bridge functions:
        // 1. mint MetaCell for using in bridge
        MetaCell(metaCellAddress).mint(address(this), _optionId);

        // 2. approve token
        MetaCell(metaCellAddress).approve(predicateAddress, _optionId);

        // 3. deposit
        bytes memory depositData = abi.encode(_optionId);
        IRootChainManager(rootChainManager).depositFor(
            _toAddress,
            metaCellAddress,
            depositData
        );

        ScientistData.Scientist memory scientist = createScientist(
            _toAddress,
            _optionId
        );
        emit MintSucceed(_toAddress);

        emit NewScientistCreated(scientist);

        emit MetaCellMintSucceed(_toAddress);
