...
contract wCROW is IERC20, Auth, ReentrancyGuard {
    ...
    address CROW = 0xe8414389498E18345545C0A94AC1D9B09fF86ce1;
    IERC20 crow = IERC20(CROW);

    string constant _name = "Wrapped Crow Bot";
    string constant _symbol = "wCROW";
    ...
    function mint(
        ...
        require(crow.allowance(msg.sender, address(this)) >= _amount, "wCROW allowance is too low");
        require(maxSupply >= _amount, "The amount exceeds the supply");
        bool _transfer = crow.transferFrom(msg.sender, address(this), _amount);
    ...
    function redeem(
        ...
        require(!hasEmergencySweeped, "This contract is no longer in use");
        require(!paused, "This contract is paused");
        require(_balances[msg.sender] >= _amount, "Your wCROW balance is too low");
        require(_totalSupply >= _amount, "The contract CROW balance is too low");
        bool _transfer = crow.transfer(msg.sender, _amount);
    ...
    function burn(
        ...
        require(_balances[msg.sender] >= _amount, "Your wCROW balance is too low");
        require(_totalSupply >= _amount, "The contract CROW balance is too low");
        bool _transfer = crow.transferFrom(address(this), BURN, _amount);
        require(_transfer, "Failed to send tokens");
        _balances[msg.sender] -= _amount;
        _totalSupply -= _amount;
        emit Transfer(msg.sender, address(this), _amount);
    ...
    function emergencySweepFull(
        ...
        require(!hasEmergencySweeped, "This contract is no longer in use");
        require(canEmergencySweep, "Emergency sweeping has been disabled");
        crow.transfer(_to, crow.balanceOf(address(this)));
        hasEmergencySweeped = true;
    }
