...
contract PriceFeedOracle is PriceOracle, AccessControl {
    bytes32 public constant REPORTER_ROLE = keccak256("REPORTER_ROLE");

    mapping(address => uint256) public reportedPricesByUnderlyingAddress;
    mapping(address => AggregatorV3Interface)
        public priceFeedsByUnderlyingAddress;

    event ReportedPrice(
        address underlyingAddress,
        uint256 oldPriceMantissa,
    ...
    function _getUnderlyingAddress(CToken _cToken)
        private
        view
        returns (address)
    {
        if (
            _compareStrings(_cToken.symbol(), "cETH") ||
            _compareStrings(_cToken.symbol(), "cMATIC")
        ) {
            // Hardcoded underlying address for native tokens
            return 0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE;
        }

        return address(CErc20(address(_cToken)).underlying());
    }

    function getUnderlyingPrice(CToken _cToken)
        external
        view
        returns (uint256)
    {
        address underlyingAddress = _getUnderlyingAddress(_cToken);

        // Reported prices come before price feeds
        if (reportedPricesByUnderlyingAddress[underlyingAddress] != 0) {
            return reportedPricesByUnderlyingAddress[underlyingAddress];
        }

        (, int256 price, , , ) = priceFeedsByUnderlyingAddress[
            underlyingAddress
        ].latestRoundData();

        // Compound protocol does not handle negative prices, we return 0 instead to make it return an error
        if (price < 0) {
            return 0;
        }
    ...
    function reportUnderlyingPrice(
        ...
        emit ReportedPrice(
            _underlyingAddress,
            reportedPricesByUnderlyingAddress[_underlyingAddress],
            _priceMantissa
        );

        reportedPricesByUnderlyingAddress[_underlyingAddress] = _priceMantissa;
    }

    function setPriceFeed(address _underlyingAddress, address _priceFeed)
        external
    {
        require(hasRole(REPORTER_ROLE, msg.sender), "NOT_REPORTER");
        ...
        emit SetPriceFeed(
            _underlyingAddress,
            address(priceFeedsByUnderlyingAddress[_underlyingAddress]),
            _priceFeed
        );

        priceFeedsByUnderlyingAddress[
            _underlyingAddress
    ...
    ERC1155Holder
{
    ...
    function initialize(
        ...
        err = _setPermissionManager(permissionManager_);
        require(
            err == uint256(Error.NO_ERROR),
            "setting permission manager failed"
        );
    ...
    /**
     * @notice Checks a given account address has tier 2 assigned, is not suspended, and is not rejected
     * @param account The address of the account
     * @return Whether or not the account address satifies the condition
     */
    function matchIsTier2AndNotSuspendedAndNotRejected(address account)
        internal
        view
        returns (bool)
    {
        bool hasTier2 = permissionManager.hasTier2(account);
        bool isSuspended = permissionManager.isSuspended(account);
        bool isRejected = permissionManager.isRejected(account);

        return hasTier2 && !isSuspended && !isRejected;
    }

    /**
     * @notice Checks a given account address has tier 2 assigned, and is not suspended
     * @param account The address of the account
     * @return Whether or not the account address satifies the condition
     */
    function matchIsTier2AndNotSuspended(address account)
        internal
        view
        returns (bool)
    {
    ...
    function transferTokens(
        ...
        if (matchIsTier2AndNotSuspendedAndNotRejected(spender) == false) {
            return
                fail(
                    Error.UNAUTHORIZED,
                    FailureInfo.PERMISSION_MANAGER_UNAUTHORIZED
                );
        }
    ...
    function mintFresh(address minter, uint256 mintAmount)
        ...
        returns (uint256, uint256)
    {
        if (matchIsTier2AndNotSuspendedAndNotRejected(minter) == false) {
            return (
                fail(
                    Error.UNAUTHORIZED,
                    FailureInfo.PERMISSION_MANAGER_UNAUTHORIZED
    ...
    function redeemFresh(
        ...
        if (matchIsTier2AndNotSuspended(redeemer) == false) {
            return
                fail(
                    Error.UNAUTHORIZED,
                    FailureInfo.PERMISSION_MANAGER_UNAUTHORIZED
    ...
    function borrowFresh(address payable borrower, uint256 borrowAmount)
        ...
        returns (uint256)
    {
        if (matchIsTier2AndNotSuspendedAndNotRejected(borrower) == false) {
            return
                fail(
                    Error.UNAUTHORIZED,
                    FailureInfo.PERMISSION_MANAGER_UNAUTHORIZED
    ...
    function repayBorrowFresh(
        ...
        if (matchIsTier2AndNotSuspended(payer) == false) {
            return (
                fail(
                    Error.UNAUTHORIZED,
                    FailureInfo.PERMISSION_MANAGER_UNAUTHORIZED
    ...
    function liquidateBorrowFresh(
        ...
        if (matchIsTier2AndNotSuspendedAndNotRejected(liquidator) == false) {
            return (
                fail(
                    Error.UNAUTHORIZED,
                    FailureInfo.PERMISSION_MANAGER_UNAUTHORIZED
    ...
    /**
     * @notice Sets a new permission manager
     * @dev Admin function to set a new permission manager
     * @return uint 0=success, otherwise a failure (see ErrorReporter.sol for details)
     */
    function _setPermissionManager(IPermissionManager newPermissionManager)
        public
        returns (uint256)
    {
        // Check caller is admin
        ...
        IPermissionManager oldPermissionManager = permissionManager;

        permissionManager = newPermissionManager;

        emit NewPermissionManager(oldPermissionManager, newPermissionManager);
...
contract ERC1155Receiver is ERC165, IERC1155Receiver {
    function supportsInterface(bytes4 interfaceId) public view returns (bool) {
        return
            interfaceId == 0x4e2312e0 || super.supportsInterface(interfaceId);
    }
}
