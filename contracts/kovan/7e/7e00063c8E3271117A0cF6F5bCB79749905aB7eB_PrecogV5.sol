...
contract PrecogV5 is IPrecogV5 {
    ...
    function addLiquidityPool(address token) external onlyAdmin {
        ...
        if (!isNotFirstInvestmentCycle[token]) {
            tradingCycles[token].push(
                Cycle(
                    0,
                    _now,
                    _now + cycleConfig.firstTradingCycle
                )
            );
            Cycle memory currentTradingCycle = tradingCycles[token][0];
            cyclesChangedInfo[token] = CyclesChangedInfo(currentTradingCycle.endTime, _now, _now, cycleConfig.fundingCycle, cycleConfig.firstDefundingCycle, cycleConfig.defundingCycle);
        ...
        if (isRemoved[token] == true) {
            _updateFundingDuration(token);
            _updateDefundingDuration(token);
    ...
    function _updateAdjustment(address _token) internal {
        ...
        Cycle memory _currentTradingCycle = tradingCycles[_token][tradingCycles[_token].length - 1];
        Cycle memory _futureTradingcycle = _getTradingCycleByTimestamp(_token, _nextTradingTime);


        require(_currentTradingCycle.id != 0 || _futureTradingcycle.id != 1);
    ...
    function _increaseInvestment(
        ...
        {
            _nextFundingTime = PrecogV5Library._nextFundingTime(cyclesChangedInfo[_token].fundingApplyTime, cyclesChangedInfo[_token].fundingDuration);
    ...
    function _decreaseInvestment(
        ...
        require(!isBeforeFundingTime(_token, _account), "3");
        Investment[] memory _investmentsOf = investmentsOf[_token][_account];
        ...
        Cycle memory _futureTradingCycle;
        {
            ...
            if(isFirstRequestWithdrawal == true) {
                if(withdrawalRegister.isInDeadline(_token, _account)) {
                    ...
                    uint48 _defundingApplyTime = _cycleChangedInfo.defundingApplyTime;
                    uint48 _defundingDuration = _cycleChangedInfo.defundingDuration;
                    _nextDefundingTime = PrecogV5Library._nextDefundingTime(_defundingApplyTime, _defundingDuration, core.getCycleConfiguration().firstDefundingCycle);
                    _futureTradingCycle = _getTradingCycleByTimestamp(_token, _nextDefundingTime);
                }
    ...
    function deposit(address token, uint amount) external override {
        require(amount >= core.minFunding(token), "0");
        _updateCurrentTradingCycle(token, false, 0);
    ...
    function requestWithdrawal(address token, uint amount) external override {
        require(amount >= core.minDefunding(token), "2");
        _updateCurrentTradingCycle(token, false, 0);
        ...
        require(remainingAmount == 0 || remainingAmount >= core.minFunding(token), "2");
    ...
    function _withdrawBeforeFundingTime(address _account, address _to, address _token, uint _amount, uint _fee) internal {
        ...
        require(availableWithdrawalAmount >= _amount && IERC20(_token).balanceOf(address(this)) >= _amount, "1");
        uint remainingAmount = _decreaseInvestmentWhenWithdraw(_token, _account, _amount);
        require(remainingAmount == 0 || (remainingAmount >= core.minFunding(_token) && _amount >= core.minDefunding(_token)), "2");
        IERC20(_token).safeTransfer(_to, _amount - _fee);
        IERC20(_token).safeTransfer(address(core), _fee);
    }

    function isBeforeFundingTime(address token, address account) public view returns (bool isBeforeInvestmentCycle) {
