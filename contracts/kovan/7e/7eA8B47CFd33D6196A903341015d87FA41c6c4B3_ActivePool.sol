...
contract ActivePool is Ownable, CheckContract, IActivePool {
    ...
    address public externalStakingUSDCAddress;
    ...
    function setAddresses(
        ...
        address _externalStakingUSDCAddress,
        address _externalStakingMATICAddress,
        ...
        onlyOwner
    {
        ...
        externalStakingUSDCAddress = _externalStakingUSDCAddress;
        maticStaker = IMockMATICStaker(_externalStakingMATICAddress);
    ...
    */
    ...
    function _swapMATICToUSDC(uint _amount) internal {
        address[] memory path = new address[](2);
        path[0] = uniswapV2Router02.WETH(); 
        path[1] = address(usdcToken);

        uniswapV2Router02.swapExactETHForTokens{ value:  _amount }(0, path, address(this), block.timestamp+300);
        usdcToken.transfer(externalStakingUSDCAddress, usdcToken.balanceOf(address(this)));
    }

    function offsetLiquidation(uint _collAmount) external override {
        _requireCallerIsVaultM();
        oMATIC -= _collAmount;
        oMATICToken.burn(address(this), _collAmount);

        if( _collAmount <= address(bufferPool).balance){
            bool success = bufferPool.sendMATIC(address(this), _collAmount);
            require(success, "Active pool: MATIC transfer failed from BufferPool");
        }else{
            uint amount_matic = maticStaker.transferMATIC(_collAmount, address(this));
            require(amount_matic == _collAmount , "Active pool: MATIC transfer failed form MATIC Staker");
        }
        _swapMATICToUSDC(address(this).balance);
    }
