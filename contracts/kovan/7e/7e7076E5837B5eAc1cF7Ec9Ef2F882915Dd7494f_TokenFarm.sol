...
contract TokenFarm is Ownable {
    ...
    // Mapping: User address ==> Count of unique token staked by a user
    mapping(address => uint256) public uniqueTokensStaked;
    // Mapping: token address to their priceFeed addresses
    ...
    function issueToken() public onlyOwner {
        for (uint256 i = 0; i < stakers.length; i++) {
            ...
            recepient = stakers[i];
            userTotalValue = getUserTotalValue(recepient);
    ...
    function stakeTokens(uint256 _amount, address _token) public {
        ...
        require(isTokenAllowed(_token), "Token is not allwoed to stake");
    ...
    function unstakeToken(address _token) public {
        ...
        require(balance > 0, "Staking balance can't be 0 for unstaking");
    ...
    function isTokenAllowed(address _token) public returns (bool) {
        for (uint256 i = 0; i < allowedTokens.length; i++) {
            if (_token == allowedTokens[i]) return true;
        }
        return false;
    }
