...
contract AdminInceptionVault is IAdminInceptionVault, BoringOwnable, Initializable {
  ...
  IWETH public WETH;
  IERC20 public mimo;
  IInceptionVaultsCore public inceptionCore;

  uint8 public collateralCount;
  mapping(uint8 => address) public collaterals;
  mapping(address => uint8) public collateralId;

  uint256 private _balance;

  modifier onlyInceptionVaultsCore() {
    require(address(inceptionCore) == msg.sender, "IV031");
    _;
  }
  ...
  function initialize(
    ...
    WETH = _WETH;
    mimo = _mimo;
    inceptionCore = _inceptionVaultsCore;
    owner = _owner;
  }
  ...
  **/
  function depositETH() public payable override onlyOwner {
  ...
  **/
  function depositETHAndBorrow(uint256 _borrowAmount) public payable override onlyOwner {
    a.core().depositETHAndBorrow{ value: msg.value }(_borrowAmount);
  }
  ...
  **/
  function deposit(address _collateralType, uint256 _amount) public override onlyOwner {
    ...
    asset.safeIncreaseAllowance(address(a.core()), _amount);
    a.core().deposit(_collateralType, _amount);
  }
  ...
  **/
  function depositAndBorrow(
    ...
    asset.safeIncreaseAllowance(address(a.core()), _depositAmount);
    a.core().depositAndBorrow(_collateralType, _depositAmount, _borrowAmount);
    if (collateralId[_collateralType] == 0) {
      uint8 newId = ++collateralCount;
      collateralId[_collateralType] = newId;
      collaterals[newId] = _collateralType;
    }
  }
  ...
  **/
  function withdraw(uint256 _vaultId, uint256 _amount) external override onlyOwner {
    a.core().withdraw(_vaultId, _amount);
    IERC20 asset = IERC20(a.vaultsData().vaultCollateralType(_vaultId));
  ...
  **/
  function borrow(uint256 _vaultId, uint256 _amount) public override onlyOwner {
    a.core().borrow(_vaultId, _amount);
    IVaultsDataProvider vaultsData = IVaultsDataProvider(a.vaultsData());
    address vaultCollateral = vaultsData.vaultCollateralType(_vaultId);
    if (collateralId[vaultCollateral] == 0) {
      uint8 newId = ++collateralCount;
      collateralId[vaultCollateral] = newId;
      collaterals[newId] = vaultCollateral;
    }
  }
  ...
  */
  function claimMimo() external override onlyOwner {
    for (uint8 i = 1; i < collateralCount + 1; i++) {
      IGenericMiner supplyMiner = IGenericMiner(address(debtNotifier.collateralSupplyMinerMapping(collaterals[i])));
  ...
  */
  function transferPar(uint256 _amount, address _to) external override onlyOwner {
    IERC20 par = IERC20(address(a.stablex()));
