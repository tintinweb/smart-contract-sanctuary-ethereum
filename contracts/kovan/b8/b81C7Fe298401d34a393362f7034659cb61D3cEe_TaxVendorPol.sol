...
contract TaxVendorPol is Ownable {
    ...
    address public usdcAddress = 0xd8f2c30Fab55F464cA8d7e7e1525F96ab6907928;
    ...
    //  Struct
    struct User {
        uint totalTokensClearedByUser; // User specific counter
    ...
    constructor(address taxTokenAddr) {
        usdcFee = 25 * 10 ** 18; // Initial fee of 25 USDC
    ...
    //  executed at any time, as long as the fee is paid and the token is cleared).
    function payUsdcFeeAndClearToken(address clearToken) public returns (address) {
        ...
        require (vendorBalance >= tokensToTransfer, "The vendor dont hold enough tokens to make a swap possible at the moment. Please check again late");
    ...
    //  Lets the owner set a new USDC fee (ownlyOwner)
    ...
    function withdrawFunds() public payable onlyOwner returns (uint) {
    ...
    function withdrawTokens(address token, uint amount) public onlyOwner returns (uint) {
    ...
    function swapTokens(address yourTokenAddress, uint yourTokenAmount) public returns (bool swapComplete){
        ...
        uint vendorBalance = taxToken.balanceOf(address(this));
        require (vendorBalance >= tokensToTransfer, "The vendor dont hold enough tokens to make a swap possible at the moment. Please check again later");  

        // Sets tokenclearence back to 'false' 
    ...
    //  Checks if a specific token is cleared for the user. (Returns string)
