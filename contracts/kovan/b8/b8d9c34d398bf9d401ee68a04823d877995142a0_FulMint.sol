...
contract FulMint is Ownable{
    ...
    function updateRoot(uint256 packNumber, bytes32 root_)public onlyOwner{
        rootOfPack[packNumber] = root_;
    }

    function setRootOfPack(uint256[]memory sizeOfPack, bytes32[]memory rootHash) public onlyOwner{
        require(sizeOfPack.length <=13, "only 13 packs are allowed " );
        for(uint i=0; i<=12;i++){
    ...
    // function nftForWhitelistedUser()public isWhitelisted {
    //     buyPackOfNFT(5);
    // }
