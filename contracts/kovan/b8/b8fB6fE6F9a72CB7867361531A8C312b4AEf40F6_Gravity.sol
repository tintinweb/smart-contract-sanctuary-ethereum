...
contract Gravity is KeeperCompatibleInterface {
    ...
    // testing keepers automation
    uint public testTimeInterval = 0;
    ...
    */
    ...
    function initiateNewStrategy(address _sourceAsset, address _targetAsset, uint _sourceBalance, uint _interval, uint _purchaseAmount) public {
        ...
        require(_interval == 1, "Unsupported interval");
        ...
        // schedule purchases by populating purchaseOrders mapping
        for(uint i = 0; i < _purchasesRemaining; i++) {
            purchaseOrders[i].push(PurchaseOrder(msg.sender, _purchaseAmount));
        }
    ...
    function checkUpkeep(bytes calldata /* checkData */) external override returns (bool upkeepNeeded, bytes memory /* performData */) {
        ...
        */
        if((block.timestamp - lastTimeStamp) > upKeepInterval) {
            // if total PO > 0
            uint _total = accumulatePurchaseOrders(testTimeInterval);
            if(_total > 0) {
                upkeepNeeded = true;
            }
        }
    ...
    function performUpkeep(bytes calldata /* performData */) external override {
        ...
        if (_total > 0) {
            ...
            */
            testTimeInterval = testTimeInterval + 1;
            emit PurchaseExecuted(block.timestamp);
        }
        emit PurchaseNotExecuted(block.timestamp);
    }
    ...
    */
    ...
    //         purchaseOrders[_nextUnixPurchaseDate](PurchaseOrder(msg.sender, _purchaseAmount));
    //     }
