...
pragma solidity 0.8.7;
contract Oxmose is ERC721A, ERC2981ContractWideRoyalties, Ownable, Whitelist {

    string public baseURI = "https://gateway.pinata.cloud/ipfs/QmfJGhAZWpjiuuYzSBSSo6uj94EbYihSpUE8ZEV81Zdpmm/"; 
    string public arweaveHash;
 
    // Price and supply definition
    uint constant public OXMOSE_PRICE = 0.5 ether; 
    uint constant public OXMOSE_MAX_SUPPLY = 4000;
    uint constant public OXMOSE_AIRDROP_LIMIT = 20;
  
    //  EIP 2981 Standard Implementation
    address constant public ROYALTY_RECIPIENT = 0x583031D1113aD414F02576BD6afaBfb302140225;
    uint constant public ROYALTY_PERCENTAGE = 1000; // value percentage (using 2 decimals - 10000 = 100, 0 = 0)
    ...
    constructor (address _payments, bytes32 _whitelistMerkle) 
        ERC721A("Oxmose Season 1", "Oxmose S1", 10) 
        Whitelist(_whitelistMerkle) 
    {
        ...
        returns (string memory)
    {
        ...
        if(workflow != WorkflowStatus.Reveal) {
            return _baseURI();
        }
 
        //if(bytes(arweaveHash).length != 0) {
        //    string(abi.encodePacked(_baseURI(), arweaveHash, "/", Strings.toString(tokenId)));
        //}
    ...
    // More gas efficient base URI system
    ...
    function setBaseURI(string memory _newUri) public onlyOwner {
        //require(workflow != WorkflowStatus.Reveal, "Already revealed");
        baseURI = _newUri;
        workflow = WorkflowStatus.Reveal;
    }
    ...
    // Allows to change the workflow to Presale
    function setUpPreSale() public onlyOwner {
        require(workflow == WorkflowStatus.Before);
        workflow = WorkflowStatus.PreSale;
    }

    // Allows to change the workflow to PublicSale
    function setUpPublicSale() public onlyOwner {
    ...
    function mint(uint256 quantity, bytes32[] calldata proof, uint256[] calldata positions) public payable {
        ...
        //require(workflow == WorkflowStatus.PreSale || workflow == WorkflowStatus.PublicSale, "Sale is not started yet");
