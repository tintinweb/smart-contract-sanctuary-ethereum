...
contract cryptoPredictionPool is ReentrancyGuard {
    ...
    function checkRebase() public view returns(uint) {
    ...
    function updatebal(address _user,uint256 _poolId,uint256 _reward,bool _isPositive) public returns(bool){
        ...
        require(us.balance >= u.totalbet,"Error Ocuured!!! Subtraction Will Not Be Not Done...");
        if(_isPositive == true){
    ...
    /* create new rebase count after 30 days */
     ...
     function createRebase() public returns(bool) {
        require(msg.sender == owner,'Not Owner');
        require(block.timestamp > lastRebaseCreated +  5 minutes, "cannot create new one before 30 days cycle");
    ...
    function createPool(uint256[15] memory _prices) public returns(bool){
        require(msg.sender == owner,'Not Owner');
        require( block.timestamp > lastpoolcreated +  5 minutes,'Cannot Create');
