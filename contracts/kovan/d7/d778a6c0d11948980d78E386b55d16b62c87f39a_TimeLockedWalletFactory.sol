...
contract TimeLockedWalletFactory {
        ...
        returns (address)
    {
        ...
        listOfWallets.push(address(wallet));
        ...
        uint256 index = walletOwners.length;

        if (msg.sender == _owner) {
            bool found1 = false;
            for (index = 0; index < walletOwners.length; ) {
                if (walletOwners[index] == msg.sender) {
                    found1 = true;
                }
                unchecked {
                    ++index;
    ...
    function destroy(address _walletAddres) external {
        TimeLockedWallet walletToDestroy = TimeLockedWallet(
            payable(address(_walletAddres))
        );

        require(msg.sender == walletToDestroy.owner(), "You are not the owner");

        address[] memory ownerWalletAddres = wallets[msg.sender];
        uint8 indexWallet;

        for (uint8 index = 0; index < ownerWalletAddres.length; index++) {
            if (ownerWalletAddres[index] == _walletAddres) {
                indexWallet = index;
            }
        }
        delete wallets[msg.sender];
        delete ownerWalletAddres[indexWallet];
        wallets[msg.sender] = ownerWalletAddres;
        address(walletToDestroy).delegatecall(
            abi.encodeWithSignature("eraseWallet()")
        );
    }
...
contract TimeLockedWallet {
    ...
    /**
    Modifier to permit only Owner to call function */
    modifier onlyOwner() {
    ...
    function eraseWallet() public onlyOwner {
