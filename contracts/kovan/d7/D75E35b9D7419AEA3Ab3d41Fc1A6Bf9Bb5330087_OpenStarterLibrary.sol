...
contract OpenStarterLibrary is Ownable {
    ...
    uint256[5][10] private tiers; // tiers[0][] is APE; tiers[1][] is START;
    ...
    uint256 private minVoterBalance = 10 * 1e18; // min APE needed to vote
    uint256 private minYesVotesThreshold = 10000000 * 1e18; // min YES votes needed to pass
    ...
    function getStaked(address _sender) external view returns (uint256) {
        ...
        if (
            ...
            voteTokenIndex == externalTokenIndex // only include outside bal if its the votetoken
        ) {
