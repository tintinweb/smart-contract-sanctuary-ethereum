...
contract MGCLottery is Ownable {
    ...
    function spin(uint _pid, uint nonce, uint number, uint amount) public {
        ...
        setTop10Player();
        bool result = random(nonce, p.decimal) == number;
        user.spins.push(Spin(number, amount, result, block.number));
        if(result) {
            p.poolReward -= reward;
