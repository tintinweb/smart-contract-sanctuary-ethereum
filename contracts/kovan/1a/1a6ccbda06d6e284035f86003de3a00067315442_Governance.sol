...
contract Governance is IGovernance, Timelock, Initializable {
    ...
    function initialize(
        ...
        uint256 _proposalMaxActions,
        uint256 _creationThresholdPercentage,
        address daoStakingAddr
    ) public initializer {
        require(_acceptanceThreshold > 50, "invalid acceptance threshold");
        require(_acceptanceThreshold <= 100, "invalid acceptance threshold");
        require(_creationThresholdPercentage <= 100, "invalid creation threshold");
        require(daoStakingAddr != address(0), "DAOStaking must not be 0x0");

        warmUpDuration = _warmUpDuration;
        ...
        proposalMaxActions = _proposalMaxActions;
        creationThresholdPercentage = _creationThresholdPercentage;
