...
contract Dog {
    ...
    function caclDart(uint256 art, uint256 room, uint256 rate, uint256 chop_) public pure returns (uint){
        uint dart = min(art, mul(room, WAD) / rate / chop_);
        return dart;
    }

    function caclRoom(uint256 Hole_, uint256 Dirt_, uint256 hole, uint256 dirt) public pure returns (uint){
        uint256 room = min(Hole_ - Dirt_, hole - dirt);
        return room;
    }
