...
contract INFLending is ERC721, Domain, BoringOwnable {
    ...
    function calculateOutstandingInterest (PaymentData memory data, LoanDetails memory details) internal returns (uint256 outstandingInterest) {
        uint256 delta = block.timestamp - uint256(data.beginTimeOrExpiry);
        outstandingInterest = data.loanAmount * delta * data.interestPerYear / SECONDS_PER_YEAR / BPS - uint256(details.amountRepaid);
    }

    function calculateCurrentlyOutstanding(uint256 id) external returns (uint256 outstanding) {
    ...
    function calculateRequiredAmountToBePaid (PaymentData memory data, LoanDetails memory details) internal returns (uint256 amountOutstanding) {
