...
contract ATestnetConsumer is ChainlinkClient, ConfirmedOwner {
    ...
    event RequestNetsScoreFulfilled(
        bytes32 indexed requestId,
        uint256 indexed price
    );
    ...
    ////////////////////////////////
    function requestNetsScore(address _oracle, string memory _jobId)
        public
        onlyOwner
    {
        Chainlink.Request memory req = buildChainlinkRequest(
            stringToBytes32(_jobId),
            address(this),
            this.fulfillNetsScore.selector
        );
        req.add(
            "get",
            "https://api.sportsdata.io/v3/nba/stats/json/BoxScore/16692?key=07cfd7f2c2664647a804bf614664a341"
        );
        string[] memory path = new string[](2);
        path[0] = "Game";
        path[1] = "AwayTeamScore";
        //path[2] = "awayScore";
        //path[3] = "LASTMARKET";
        req.addStringArray("path", path);
        sendChainlinkRequestTo(_oracle, req, ORACLE_PAYMENT);
    ...
    function fulfillNetsScore(bytes32 _requestId, uint256 _score)
        public
        recordChainlinkFulfillment(_requestId)
    {
        emit RequestNetsScoreFulfilled(_requestId, _score);
        endScore = _score;
    }
