...
contract TPCore is AccessControl {
    ...
    // 48 bit minimum
    uint48 private startingTpyRewardPerInterval = 1200 * (10**8);//1027 * 7 * 4 * (10**8); // per day * 4 week
    
    // 32 (max 1 year)
    uint32 public tpyInterval = 10 minutes;
    uint256 public takenTPY;

    mapping(address => Staking) private userStaking;
    uint256[1000] public intervalBalances;
    uint256[1000] public intervalNegativeBalances;
    ...
    /// Constructor
    constructor(address stakingcoinAddress, address tpyTokenAddress, address initialStrategyAddress) {
    ...
    function transferLTAdminRole(address newAdmin) external onlyRole(DEFAULT_ADMIN_ROLE) {
        require(newAdmin != address(0), "newAdmin address can not be zero address.");
        liquidityToken.transferAdminRole(newAdmin);
    }

    function transferLTMinterRole(address newMinter) external onlyRole(DEFAULT_ADMIN_ROLE) {
        require(newMinter != address(0), "newMinter address can not be zero address.");
        liquidityToken.transferMinterRole(newMinter);
    }

    function withdraw(address tokenAddress) external onlyRole(DEFAULT_ADMIN_ROLE) {
        ERC20 token = ERC20(tokenAddress);
        uint256 balance = token.balanceOf(address(this));
        token.transfer(_msgSender(), balance);
    }

    function changeOwnerPercentage(uint16 percentage) external onlyRole(DEFAULT_ADMIN_ROLE) {
        require(percentage <= 10000, "Percentage must be from 0 to 10000 (10000 means 100%)");
        ownerPercentage = percentage;
    }

    function changeTPYPerIntervalReward(uint256 value) external onlyRole(DEFAULT_ADMIN_ROLE) {
        tpyIntervalRewards[nextInterval()] = value;
    }

    function changeStrategy(address newStrategyAddress) external onlyRole(DEFAULT_ADMIN_ROLE) {
        require(address(0x0) != newStrategyAddress, "Strategy cannot be null");
        strategy = IStrategy(newStrategyAddress);
    }

    function stake(uint104 amount) external {
        require(amount > 0, "The amount must be greater than 0.");
        uint256 allowance = stakingcoin.allowance(_msgSender(), address(this));
        require(amount <= allowance, "There is no as much allowance for staking coin.");
        ...
        (bool success, bytes memory result) = address(strategy).delegatecall(abi.encodeWithSignature(
                        "farm(address,uint256)", stakingcoin, amount));
    ...
    function unstake(uint104 amount) external {
        ...
        require(amount <= net, "Dont be greedy (the amount must be less than or equal to net rewards).");

        uint104 wantedGross = amount * gross / net;

        uint256 liquidityAmount = liquidityToken.balanceOf(_msgSender());

        uint256 lToBurn = wantedGross * liquidityToken.totalSupply() / totalReward;
        if (liquidityAmount < lToBurn) {
            lToBurn = liquidityAmount;
        }

        takeReward(wantedGross);// yearn => TPCore
        stakingcoin.safeTransfer(_msgSender(), amount);

        takeTPYReward(wantedGross);
    ...
    function stakingAmount(address lpProvider) external view returns(uint104) {
        return userStaking[lpProvider].amount;
    }

    function totalStakedForTPYInterval() external view returns(uint256) {
        return intervalBalances[nextInterval()];
    }

    function rewardForTPYInterval() external view returns(uint256) {
        return tpyIntervalRewards[nextInterval()];
    }

    function estimateNetReward(address lpProvider) external view returns (uint104) {
        uint104 grossReward = estimateGrossReward(lpProvider);
        uint104 stakingAmountPart = userStaking[lpProvider].amount;
    ...
    function estimateRewardDetails(address lpProvider) public view returns (uint104 gross, uint104 net, uint104 profit, uint104 fee) {
        gross = estimateGrossReward(lpProvider);
        uint104 stakingAmount = userStaking[lpProvider].amount;
        require(gross >= stakingAmount, "gross must be greater than amount");
        profit = gross - stakingAmount;
        fee = (10000 - ownerPercentage) * profit / 10000;
        net = gross - fee;
    }

    function totalTPYEarned() external view returns (uint256) {
    ...
    function currentIntervalTPYs(address lpProvider) external view returns (uint256) {
        uint256 interval = nextInterval();
        uint256 intervalTotalReward = getTPYIntervalReward(interval);
        uint256 intervalTotalStakingAmount = intervalBalances[interval];
        uint256 intervalUserStakingAmount = userStaking[lpProvider].amount;

        return intervalUserStakingAmount * intervalTotalReward / intervalTotalStakingAmount;
    }
    ...
    // function estimateTPYReward(address lpProvider) public view returns(uint136) {
    ...
    function registerTPY(address staker, uint104 stakingCoinAmount) private {
        ...
        intervalBalances[nextIntervalNumber] += stakingCoinAmount;
    }
    ...
    function takeTPYReward(uint104 unstakeCoinAmount) private {
        ...
        uint136 tpyToTransfer = uint136(unstakeCoinAmount * tpyRewards / staking.amount);
        staking.calculatedTPYs = uint136(tpyRewards);
        staking.amount -= unstakeCoinAmount;
        staking.intervalNumber = nextInterval();

        //intervalBalances[nextInterval()] -= unstakeCoinAmount;
        intervalNegativeBalances[nextInterval()] += unstakeCoinAmount;

        if(tpyToTransfer != 0 && tpy.balanceOf(address(this)) >= tpyToTransfer) { // Not enough tpy balance for TPCore !!!
    ...
    function getTPYIntervalReward(uint256 intervalNumber) private view returns(uint256) {
        if(intervalNumber == 0){
            return 0;
        }
    ...
    function totalStakedUntilInterval(uint256 finalInterval) public view returns(uint256) {
        uint256 total;
        for(uint256 i = 0; i <= finalInterval; i++){
            total += intervalBalances[i];
            total -= intervalNegativeBalances[i];
        }
        return total;
    }
    ...
    function estimateTPYReward(address addr) public view returns(uint136) {
        ...
        uint256 lastCompleteInterval = currentInterval();
        uint256 estimatedTPYs = staking.calculatedTPYs;
        for(uint256 i = staking.intervalNumber; i <= lastCompleteInterval; i++) {
            uint256 stakedUntilInterval = totalStakedUntilInterval(i);
            if(stakedUntilInterval == 0) {
                continue;
            }
            estimatedTPYs += staking.amount * getTPYIntervalReward(i) / stakedUntilInterval;
        }
