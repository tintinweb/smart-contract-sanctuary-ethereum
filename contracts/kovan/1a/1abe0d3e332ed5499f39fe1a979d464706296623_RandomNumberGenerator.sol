    ...
    ReentrancyGuardUpgradeable
{
    ...
    bool public isAvailableforDeposit;

    /**
     * @dev Throws if called by any account other than the uRevolt contract.
     */
    modifier eligibleForEnableDepositWithdraw() {
        require(uRevolt == _msgSender(), "Ownable: caller is not the uRevolt");
        _;
    }
    ...
    modifier eligibleForRequest() {
        require(
            ...
            "Ownable: caller is not the uRevolt"
        );
        _;
    }

    event RequestRandomNumber(uint256 indexed _requestId);
    event FulfilledRequest(uint256 indexed _requestId, uint256 indexed _fulfilledIndex, uint256 firstNumber, uint256 secondNumber);
    ...
    function enableDepositOnceRandomNumberFulfilled()
        external
        eligibleForEnableDepositWithdraw
    {
        isAvailableforDeposit = true;
    }
    ...
    function fulfillRandomWords(
        ...
        if (block.timestamp > lastRequestTime.add(randomNumberInterval)) {
            ...
            isAvailableforDeposit = false;
            emit FulfilledRequest(_requestId, _requestCounter.current(), _randomWords[0], _randomWords[1]);
        }
    }
