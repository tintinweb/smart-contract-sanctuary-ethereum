...
contract RolaCoaster is ERC20, Ownable, Pausable, ReentrancyGuard, BlackList, Vesting, IRolaCoaster{
    ...
    mapping(uint256 => ParticipationDetails) public userToParticipation;
    ...
    function mintRolaforPublicSale(uint256 amountUSDC) external override whenNotPaused nonReentrant whenNotBlackListedUser(_msgSender()) {
        ...
        uint256 currentAvailableRola = getPublicSupplyForDays(amountRola, 0);
        ParticipationDetails memory participationDetails = ParticipationDetails({
            participationId: totalParticipation,
            amountBought: amountRola,
            amountUSDCPaid: amountUSDC,
            timeParticipated: block.timestamp - (totalParticipation*10000000),
            amountReceived: currentAvailableRola
        });
        userToParticipation[totalParticipation] = participationDetails;
        totalParticipation++;
        publicSaleSupplyRola += amountRola;
        _mint(_msgSender(), currentAvailableRola);
        emit RolaMinted(_msgSender(), currentAvailableRola);
    }

    function bulkMintRolaforPublicSale() external whenNotPaused nonReentrant onlyMaintainer {
        for(uint256 participantIndex = 0; participantIndex < totalParticipation; participantIndex++){
            ParticipationDetails storage participationDetails = userToParticipation[participantIndex];
            ...
            _mint(_msgSender(), allowableMint);
        }
    }
...
contract Vesting {
    ...
    function getAdvisorSupplyForDays(uint256 daysPassed) internal pure returns(uint256 supply){
        supply = 50000;
    ...
    function getTeamSupplyForDays(uint256 daysPassed) internal pure returns(uint256 supply){
        supply = 36000;
        ...
        else if(daysPassed > 650){
            supply += 84000;
        }
    }

    function getPublicSupplyForDays(uint256 amount, uint256 daysPassed) internal pure returns(uint256 supply){
