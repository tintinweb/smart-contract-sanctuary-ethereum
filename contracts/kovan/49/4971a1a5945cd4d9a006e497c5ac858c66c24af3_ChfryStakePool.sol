...
// File: contracts/avalanche/ChfryStakePool.sol
...
contract ChfryStakePool is UpgradableProduct, ReentrancyGuard {
	...
	event SetChfryFactory(address indexed factory);
	event SetChfryToken(address indexed token);
	...
	ChfryToken public token;
	ChfryFactory public chfryFactory;
	...
	constructor(address chfryFactory_, address token_) public {
		chfryFactory = ChfryFactory(chfryFactory_);
		token = ChfryToken(token_);
	...
	function setChfryFactory(address chfryFactory_) external requireImpl {
		chfryFactory = ChfryFactory(chfryFactory_);
		emit SetChfryFactory(chfryFactory_);
	}

	function setChfryToken(address token_) external requireImpl {
		token = ChfryToken(token_);
		emit SetChfryToken(token_);
	}
	...
	modifier updateAllPoolRewardPerShare() {
		if (totalWeight > 0 && block.timestamp > lastBlockTimeStamp) {
			...
			require(_reward == chfryFactory.poolMint(ConfigNames.STAKE), 'pool mint error');
		}
		_;
	}
	...
	function currentAllPoolRewardShare() public view virtual returns (uint256 _reward, uint256 _perShare) {
		_reward = chfryFactory.prePoolMint(ConfigNames.STAKE);
