...
interface IMuiGlobals {
    ...
    function mui() external view returns (address);

    function MuiTreasury() external view returns (address);
    ...
    function setMuiTreasury(address) external;
...
library SafeERC20 {
    ...
    function _callOptionalReturn(IERC20 token, bytes memory data) private {
        ...
        // we're imuiementing it ourselves. We use {Address.functionCall} to perform this call, which verifies that
...
/// @title PoolLib is a Preprocessor Dirctory of utility Methods used by our own Pool.
library PoolLib {
    ...
    uint8   public constant DL_FACTORY  = 1;         // MUI Factory_type of our MUI_DebtLockerFactory

    event         LoanFunded(address indexed loan, address debtLocker, uint256 amountFunded);
    event DepositDateUpdated(address indexed liquidityProvider, uint256 depositDate);

    /***************************************/
    /*** Pool Delegate Utility Methods ***/
    /***************************************/

    /** 
        @dev   Prechecks for MUI_Pools in the constructor.
        @param globals        Instance of a MuiGlobals.
    ...
    */
    ...
    /**
        @dev   Funds a Loan for an amount, utilizing the supplied MUI_DebtLockerFactory for DebtLockers.
        @dev   It emits a `LoanFunded` event for Logging in Transactions.
        @param debtLockers     Mapping contains the DebtLocker contract address corresponding to the DebtLockerFactory and Loan.
        @param superFactory    Address of MUI_PoolFactory.
        @param liquidityLocker Address of  MUI_LiquidityLocker contract attached with this MUI_Pool.
        @param loan            Address of  Loan to fund.
        @param dlFactory       MUI_DebtLockerFactory to utilize.
    ...
    */
    function fundLoan(
        ...
        IMuiGlobals globals = IMuiGlobals(ILoanFactory(superFactory).globals());
        ...
        // Instantiate MUI_DebtLocker if it doesn't exist withing this factory
    ...
    /**
        @dev    Helper function used by MUI_Pool `claim` function, for when if a default has occurred.
        @param  liquidityAsset                  IERC20 of Liquidity Asset.
        @param  stakeLocker                     Address of StakeLocker.
        @param  stakeAsset                      Address of BPTs.
        @param  defaultSuffered                 Total_Amount of shortfall in defaulted Loan after liquidation.
        @return bptsBurned                      Total_Amount of BPTs burned to cover shortfall.
        @return postBurnBptBal                  Total_Amount of BPTs returned to StakeLocker after burn.
        @return liquidityAssetRecoveredFromBurn Total_Amount of Liquidity Asset recovered from burn.
    */
    ...
    /**
        @dev    Calculates portions of Redeemed from DebtLocker to be used by Pool `Redeemed` function.
        @param  claimInfo           [0] = Total Redeemed
                                    [1] = Interest Redeemed
                                    [2] = Principal Redeemed
                                    [3] = Fee Redeemed
                                    [4] = Excess Returned Redeemed
                                    [5] = Amount Recovered (from Liquidation)
                                    [6] = Default Suffered
        @param  delegateFee         Portion of Dividends (basis points) that goes to the Pool Delegate.
        @param  stakingFee          Portion of Dividends (basis points) that goes to the StakeLocker.
        ...
        @return principalClaim      Total principal Redeemed.
        @return interestClaim       Total Dividends Redeemed.
    */
    ...
    /**
        ...
        @param globals        Instance of a MuiGlobals.
        @param principalOut   Total_Amount of funds that are already funded to Loans.
        @param liquidityAsset Liquidity Asset of the Pool.
    */
    function validateDeactivation(IMuiGlobals globals, uint256 principalOut, address liquidityAsset) external view {
        require(principalOut <= _convertFromUsd(globals, liquidityAsset, 100), "P:PRINCIPAL_OUTSTANDING");
    }

    /********************************************/
    /*** Liquidity Provider Utility Methods ***/
    /********************************************/
    ...
    /**
        ...
        @dev   It emits a `DepositDateUpdated` event for Logs.
    ...
    */
    function updateDepositDate(mapping(address => uint256) storage depositDate, uint256 balance, uint256 amt, address account) internal {
        ...
        // NOTE: prevDate = 0 imuiies balance = 0, and equation reduces to now
    ...
    /**
        @dev Performs all necessary checks for a `transferByCustodian` calls of Methods.
    ...
    */
    ...
    /**
        @dev Performs all necessary checks for an `increaseCustodyAllowance` call of Methods.
    */
    ...
    /**********************************/
    /*** Governor Utility Methods ***/
    /**********************************/

    /**
        @dev   Transfers any locked funds to the Governor. Only the Governor can call this Method.
        @param token          Address of the token to be reclaimed.
        @param liquidityAsset Address of Liquidity Asset that is supported by the MUI_Pool.
        @param globals        Instance of a MuiGlobals.
    */
    function reclaimERC20(address token, address liquidityAsset, IMuiGlobals globals) external {
    ...
    */
    ...
    /**
        @dev    Computes the value of BPT in units of Liquidity Asset.
    ...
    */
    function getInitialStakeRequirements(IMuiGlobals globals, address balancerPool, address liquidityAsset, address poolDelegate, address stakeLocker) external view returns (
    ...
    */
    function _convertFromUsd(IMuiGlobals globals, address liquidityAsset, uint256 usdAmount) internal view returns (uint256) {
        return usdAmount
...
/**
 * @dev Imuiementation of the {IERC20} interface.
 *
 * This imuiementation is agnostic to the way tokens are created. This means
 * that a supply mechanism has to be added in a derived contract using {_mint}.
 * For a generic mechanism see {ERC20PresetMinterPauser}.
 *
 * TIP: For a detailed writeup see our guide
 * https://forum.zeppelin.solutions/t/how-to-imuiement-erc20-supply-mechanisms/226[How
 * to imuiement supply mechanisms].
 *
 ...
 * by listening to said events. Other imuiementations of the EIP may not emit
...
contract ERC20 is Context, IERC20 {
     ...
     * For examuie, if `decimals` equals `2`, a balance of `505` tokens should
     ...
     * e.g. imuiement automatic token fees, slashing mechanisms, etc.
     *
     * Emits a {Transfer} event.
     *
     * Requirements:
...
/// @title BasicFDT imuiements base level FDT functionality for accounting for revenues.
...
/// @title ExtendedFDT imuiements FDT functionality for accounting for losses.
...
contract Pool is PoolFDT {
    ...
    */
    ...
    /**
        @dev   Liquidates a Loan. The Pool Delegate could liquidate the Loan only when the Loan comuietes its grace period.
    ...
    */
    ...
    /**
        ...
        @return claimInfo The claim details.
                    claimInfo [0] = Total amount Redeemed
                    claimInfo [1] = Interest  portion Redeemed
                    claimInfo [2] = Principal portion Redeemed
                    claimInfo [3] = Fee       portion Redeemed
                    claimInfo [4] = Excess    portion Redeemed
                    claimInfo [5] = Recovered portion Redeemed (from liquidations)
    ...
    */
    ...
    /**
        ...
        @dev   It emits a `LockupPeriodSet` event for Logging.
    ...
    */
    ...
    /**
        ...
        @dev   It emits a `DepositDateUpdated` event for Logging.
        @dev   It emits a `BalanceUpdated` event for Logging.
        @dev   It emits a `Cooldown` event for Logging.
    ...
    */
    ...
    /**
        ...
        @dev   `from` and `to` should always be equal in this imuiementation.
    ...
    */
    ...
    /**
        @dev Returns the MuiGlobals instance.
    */
    function _globals(address poolFactory) internal view returns (IMuiGlobals) {
        return IMuiGlobals(IPoolFactory(poolFactory).globals());
    }
...
/**
 * @dev Contract module which allows children to imuiement an emergency stop
 * mechanism that can be triggered by an authorized account.
 *
 * This module is used through inheritance. It will make available the
 * modifiers `whenNotPaused` and `whenPaused`, which can be applied to
 * the functions of your contract. Note that they will not be pausable by
 * simuiy including this module, only once the modifiers are put in place.
 */
...
contract PoolFactory is Pausable {
    ...
    IMuiGlobals public globals;  // A MuiGlobals instance.
    ...
    constructor(address _globals) public {
        globals = IMuiGlobals(_globals);
    }

    /**
        @dev   Sets MuiGlobals instance. Only the Governor can call this function.
        @param newGlobals Address of new MuiGlobals.
    */
    function setGlobals(address newGlobals) external {
        _isValidGovernor();
        globals = IMuiGlobals(newGlobals);
    }
    ...
    */
    function createPool(
        ...
        string memory name   = "Mui Pool Token";
        string memory symbol = "MUI-LP";
