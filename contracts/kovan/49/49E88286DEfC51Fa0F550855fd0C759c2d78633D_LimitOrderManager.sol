...
contract LimitOrderManager is
    ...
    event LimitOrderCreated(address indexed owner, uint256 indexed tokenId, uint256 amount0, uint256 amount1);
    ...
    function processLimitOrder(
        ...
        (bool underfunded, uint256 _serviceFee,uint256 _monitorFee) = canProcess(_tokenId, tx.gasprice);
        if (underfunded) {
            return (false, 0);
        }
        ...
        _transferTokenTo(address(KROM), _serviceFeePaid.sub(_monitorFee), feeAddress);
    ...
    function tokensLength() external view returns(
        uint256
    ) {
        return nextId;
    }
