   ...
   function getUserAsset(address userAddr) public view returns(string memory){
    ...
    function getContract(address userAddr) public view returns(string memory){
        ...
        for (uint i=0;i<lengthNum;i++){
            ...
            conList = string(abi.encodePacked(conList,uint2str(contList[contId].mortAmount)));
            conList = string(abi.encodePacked(conList,'",'));

            uint decimal = collaList[contList[contId].mortAddr].decimal;
            conList = string(abi.encodePacked(conList,'"mortDecimal":'));
            conList = string(abi.encodePacked(conList,uint2str(decimal)));
            conList = string(abi.encodePacked(conList,','));

            conList = string(abi.encodePacked(conList,'"outAmount":'));
            conList = string(abi.encodePacked(conList,uint2str(contList[contId].tokenUSDAmount)));
            conList = string(abi.encodePacked(conList,','));

            uint outdecimal = tokenUSDList[contList[contId].tokenUSDAddr].tokenUSDAddrOutDecimal;
            conList = string(abi.encodePacked(conList,'"outDecimal":'));
            conList = string(abi.encodePacked(conList,uint2str(outdecimal)));
            conList = string(abi.encodePacked(conList,''));
