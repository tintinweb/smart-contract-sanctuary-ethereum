...
contract DeepWatersLending is ReentrancyGuard {
    ...
    **/
    ...
    /**
    * @dev emitted on switch deposit state
    * @param _asset the address of the deposit asset
    * @param _user the address of the user
    * @param _newStateDepositUsedAsCollateral the true if the user wants to use the deposit as collateral (new state)
    * @param _timestamp the timestamp of the action
    **/
    event SwitchDepositState(
        address indexed _asset,
        address indexed _user,
        bool _newStateDepositUsedAsCollateral,
        uint256 _timestamp
    );
    
    /**
    * @dev emitted on switch between stable and variable borrow rate modes
    * @param _asset the address of the borrowed asset
    * @param _user the address of the user
    * @param newIsStableRateBorrow the true for stable mode and the false for variable mode
    * @param _timestamp the timestamp of the action
    **/
    event SwitchBorrowRateMode(
        address indexed _asset,
        address indexed _user,
        bool newIsStableRateBorrow,
        uint256 _timestamp
    );
    ...
    **/
    ...
    /**
    * @dev allows user to switch the state of their deposit (use as collateral or not)
    * @param _asset the address of the deposit asset
    **/
    function switchDepositState(address _asset)
        external
    {
        bool depositUsedAsCollateral = vault.getDepositUsedAsCollateral(_asset, msg.sender);
        
        if (depositUsedAsCollateral) {
            checkNewCRatio(_asset, msg.sender, vault.getUserAssetBalance(_asset, msg.sender));
        }
        
        vault.setDepositUsedAsCollateral(_asset, msg.sender, !depositUsedAsCollateral);
        vault.updateLog2CumulatedIndexes(_asset);
        
        emit SwitchDepositState(_asset, msg.sender, !depositUsedAsCollateral, block.timestamp);
    }
    ...
    **/
    function switchBorrowRateMode(address _asset)
        ...
        nonReentrant
    {
        require(msg.sender != liquidator, "Liquidator cannot switch borrow rate mode");
        require(vault.getAssetIsActive(_asset), "Action requires an active asset");
        ...
        vault.updateLog2CumulatedIndexes(_asset);
        vault.updateBorrowBalance(_asset, msg.sender, 0);
        vault.setBorrowRateMode(_asset, msg.sender, newIsStableRateBorrow);
        
        if (newIsStableRateBorrow) {
            vault.setAverageStableInterestRate(_asset, msg.sender, vault.getAssetCurrentStableInterestRate(_asset));
        }
        
        vault.updateBorrowBalance(_asset, msg.sender, borrowBalance);
        
        emit SwitchBorrowRateMode(
            _asset,
            msg.sender,
            newIsStableRateBorrow,
            block.timestamp
        );
    }
