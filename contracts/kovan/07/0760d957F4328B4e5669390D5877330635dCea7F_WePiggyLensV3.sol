...
contract WePiggyLensV3 {

    using SafeMath for uint256;

    ComptrollerLensInterface public comptroller;
    PiggyDistributionInterface[] public distributions;
    string  public pNativeToken;
    string public nativeToken;
    string public nativeName;
    address public owner;

    constructor(ComptrollerLensInterface _comptroller, PiggyDistributionInterface[] memory _distributions,
    ...
    function updateProperties(ComptrollerLensInterface _comptroller, PiggyDistributionInterface[] memory _distributions,
        string  memory _pNativeToken, string  memory _nativeToken, string memory _nativeName) public {
    ...
    function getWpcSpeed(PTokenLensInterface pToken) public view returns (PTokenWpcSpeed memory){
        ...
        uint length = distributions.length;
        uint[] memory speeds = new uint[](length);
        for(uint i = 0; i < length; i++){
            PiggyDistributionInterface d = PiggyDistributionInterface(distributions[i]);
            uint wpcSpeed = d.wpcSpeeds(address(pToken));
            speeds[i] = wpcSpeed;
        }
