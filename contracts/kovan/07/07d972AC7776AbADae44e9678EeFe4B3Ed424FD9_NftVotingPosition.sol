...
contract NftVotingPosition is ERC721
{
	...
	{
		require(ownerOf(votingPositionId) == msg.sender);
		dai.transferFrom(msg.sender, address(nftBattleArena), amount);                        // Transfers DAI to arena contract for vote.
	...
	{
		require(ownerOf(votingPositionId) == msg.sender);
		nftBattleArena.withdrawZooFromVoting(votingPositionId, zooNumber, beneficiary);
	}
...
contract NftBattleArena is Ownable
{
	...
	event CreatedStakerPosition(uint256 indexed currentEpoch, address indexed staker, uint256 stakingPositionId);
	...
	event LiquidatedVotingPosition(uint256 indexed currentEpoch, address indexed voter, address beneficiary, uint256 votingPositionId, uint256 indexed stakingPositionId, uint256 zooReturned, uint256 daiReceived);


	/// @notice Event about recomputing votes from dai.         SecondStage
	event RecomputedDaiVotes(uint256 indexed currentEpoch, address indexed voter, uint256 indexed votingPositionId, uint256 newVotes, uint256 oldVotes);

	/// @notice Event about recomputing votes from zoo.         FourthStage
	event RecomputedZooVotes(uint256 indexed currentEpoch, address indexed voter, uint256 indexed votingPositionId, uint256 newVotes, uint256 oldVotes);


	/// @notice Event about adding dai to voter position.       SecondStage
	event AddedDaiToVoting(uint256 indexed currentEpoch, address indexed voter, uint256 indexed votingPositionId, uint256 stakingPositionId, uint256 amount, uint256 votes);

	/// @notice Event about adding zoo to voter position.       FourthStage
	event AddedZooToVoting(uint256 indexed currentEpoch, address indexed voter, uint256 votingPositionId, uint256 indexed stakingPositionId, uint256 amount, uint256 votes);


	/// @notice Event about withdraw dai from voter position.   FirstStage
	event WithdrawedDaiFromVoting(uint256 indexed currentEpoch, address indexed voter, uint256 stakingPositionId, uint256 indexed votingPositionId, uint256 daiNumber, address beneficiary);

	/// @notice Event about withdraw zoo from voter position.   FirstStage
	event WithdrawedZooFromVoting(uint256 indexed currentEpoch, uint256 indexed stakingPositionId, uint256 votingPositionId, uint256 zooNumber, address indexed beneficiary);


	/// @notice Event about claimed reward from voting.         FirstStage
	event ClaimedRewardFromVoting(uint256 indexed currentEpoch, address indexed voter, address beneficiary, uint256 reward, uint256 indexed votingPositionId);

	/// @notice Event about claimed reward from staking.        FirstStage
	event ClaimedRewardFromStaking(uint256 indexed currentEpoch, address indexed staker, address beneficiary, uint256 reward, uint256 indexed stakingPositionId);


	/// @notice Event about paired nfts.                        ThirdStage
	event PairedNft(uint256 indexed currentEpoch, uint256 fighter1, uint256 fighter2, uint256 indexed pairIndex);

	/// @notice Event about winners in battles.                 FifthStage
	event ChosenWinner(uint256 indexed currentEpoch, uint256 fighter1, uint256 fighter2, bool winner, uint256 indexed pairIndex, uint256 playedPairsAmount);
	...
	function removeStakerPosition(uint256 stakingPositionId, address staker) public only(nftStakingPosition)
	{
		...
		emit RemovedStakerPosition(currentEpoch, msg.sender, stakingPositionId);   // Emits UnstakedNft event.
	}
	...
	function createVotingPosition(uint256 stakingPositionId, address voter, uint256 amount) external only(nftVotingPosition) returns (uint256 votes, uint256 votingPositionId)
	{
		...
		require(stakingPositionsValues[stakingPositionId].endEpoch == 0, "Nft unstaked");// Requires to be staked.
		...
		// TODO: Here posible need to change at method daiAddition
	...
	function liquidateVotingPosition(uint256 votingPositionId, address voter, address beneficiary) public
	{
		uint256 stakingPositionId = votingPositionsValues[votingPositionId].stakingPositionId;  // Gets id of staker position from this voting position.

		StakerPosition storage stakingPosition = stakingPositionsValues[stakingPositionId];

		require(getCurrentStage() == Stage.FirstStage || stakingPosition.endDate != 0, "Wrong stage!"); // Requires correct stage or nft to be unstaked.
		require(votingPositionsValues[votingPositionId].endEpoch == 0);                         // Requires to be not liquidated yet.

		uint256 daiNumber = votingPositionsValues[votingPositionId].daiInvested;
		uint256 yTokens = votingPositionsValues[votingPositionId].yTokensNumber;
		uint256 zooReturned = votingPositionsValues[votingPositionId].zooInvested * 995 / 1000; // Calculates amount of zoo to withdraw.
		uint256 zooToBurn = votingPositionsValues[votingPositionId].zooInvested * 5 / 1000;     // Calculates amount of zoo to burn.
		...
		zoo.transfer(address(0), zooToBurn);                                                              // Burns zoo for 0.5%.
		...
		if (rewardsForEpoch[stakingPositionId][currentEpoch].votes == 0 && stakingPosition.endDate == 0)
		{
			for(uint256 i = 0; i < activeStakerPositions.length; i++)
			{
				if (activeStakerPositions[i] == stakingPositionId)
		...
		emit LiquidatedVotingPosition(currentEpoch, msg.sender, beneficiary, votingPositionId, stakingPositionId, daiNumber, zooReturned);
	}
	...
	function recomputeDaiVotes(uint256 votingPositionId) public
	{
		...
		emit RecomputedDaiVotes(currentEpoch, msg.sender, votingPositionId, newVotes, votes);
	}
	...
	function recomputeZooVotes(uint256 votingPositionId) public
	{
		...
		emit RecomputedZooVotes(currentEpoch, msg.sender, votingPositionId, newZooVotes, oldZooVotes);
	}
	...
	function addDaiToVoting(uint256 votingPositionId, address voter, uint256 amount) external only(nftVotingPosition) returns (uint256 votes)
	{
		...
		//battleStaker.nftWithZeroVotesPositiveSwap(stakingPositionId);  // If votes before were zero, moves position to eligible for pairing.
		...
		emit AddedDaiToVoting(currentEpoch, voter, votingPositionId, stakingPositionId, amount, votes);
	}
	...
	function addZooToVoting(uint256 votingPositionId, address voter, uint256 amount) external only(nftVotingPosition) returns (uint256 votes)
	{
		...
		emit AddedZooToVoting(currentEpoch, voter, votingPositionId, stakingPositionId, amount, votes);
	}
	...
	function withdrawDaiFromVoting(uint256 votingPositionId, address voter, uint256 daiNumber, address beneficiary) external only(nftVotingPosition)
	{
		...
		require(votingPosition.endEpoch == 0, "already liquidated");                         // Requires to be not liquidated yet.
		require(daiNumber <= votingPosition.daiInvested, "withdraw amount exceed limit");

		if (daiNumber == votingPosition.daiInvested)
		{
			liquidateVotingPosition(votingPositionId, voter, beneficiary);
			return;
		}

		uint256 daInvested = votingPosition.daiInvested;
		uint256 daiLeft = daInvested - daiNumber; // dai left in position

		// withdraw extra zoo module
		...
		if (votingPosition.zooInvested > votingPosition.daiInvested) // If zooInvested more than daiInvested left in position.
		{
			...
			calculateWithdrawForZoo(zooDelta, beneficiary);
		...
		for (uint256 i = votingPosition.startEpoch; i < lastEpoch; i++)
		{
			if (rewardsForEpoch[stakingPositionId][i].pricePerShareCoef != 0)
			{
				yTokens -= daInvested / rewardsForEpoch[stakingPositionId][i].pricePerShareCoef;
			}
		}

		votingPosition.startEpoch = currentEpoch;

		rewardsForEpoch[stakingPositionId][currentEpoch].yTokens = yTokens * daiLeft / daInvested;
		
		vault.withdraw(yTokens * daiLeft / daInvested, beneficiary);

		uint256 deltaVotes = votingPosition.daiVotes * daiNumber / daInvested; 
	...
	/// @param beneficiary - address of recipient.
	function withdrawZooFromVoting(uint256 votingPositionId, uint256 zooNumber, address beneficiary) external only(nftVotingPosition)
	{
		VotingPosition storage votingPosition = votingPositionsValues[votingPositionId];
		...
		calculateWithdrawForZoo(zooNumber, beneficiary);
		...
		emit WithdrawedZooFromVoting(currentEpoch, stakingPositionId, votingPositionId, zooNumber, beneficiary);
	}

	/// @notice Function to claim reward in yTokens from voting.
	/// @param votingPositionId - id of voting position.
	/// @param beneficiary - address of recipient of reward.
	function claimRewardFromVoting(uint256 votingPositionId, address voter, address beneficiary) external only(nftVotingPosition) returns (uint256 rewardDaiAmount)
	{
		...
		uint256 lastEpoch = computeLastEpoch(votingPositionId);
		uint256 startEpoch = votingPosition.lastRewardedEpoch;
		uint256 yTokens = getPendingVoterReward(votingPositionId, startEpoch, lastEpoch); // Calculates amount of reward.
		yTokens += votingPosition.yTokensRewardDebt;
		votingPosition.yTokensRewardDebt = 0;

		// todo: maybe one withdraw and 4 transfers better?
		vault.withdraw(yTokens * 2 / 100, treasury);                                            // Get treasury fee from yearn.
		vault.withdraw(yTokens * 1 / 100, gasPool);                                              // Get Gas fee from yearn.
		vault.withdraw(yTokens * 1 / 100, team);                                                // Get Team fee from yearn.

		rewardsForEpoch[stakingPositionId][currentEpoch].yTokens -= yTokens * 98 / 100; // Subtracts yTokens for this position.
		votingPosition.lastRewardedEpoch = lastEpoch;            // Records epoch of last reward claimed.

		emit ClaimedRewardFromVoting(currentEpoch, voter, beneficiary, yTokens, votingPositionId);

		return vault.withdraw(yTokens * 94 / 100, beneficiary);                                             // Get reward from yearn.
	}
	...
	function getPendingVoterReward(uint256 votingPositionId, uint256 startEpoch, uint256 endEpoch) public view returns (uint256 rewardAmount)
	{
	...
	/// @param stakingPositionId - id of staker position.
	/// @param beneficiary - address of recipient.
	function claimRewardFromStaking(uint256 stakingPositionId, address staker, address beneficiary) public only(nftStakingPosition)
	{
		require(getCurrentStage() == Stage.FirstStage, "Wrong stage!");             // Requires to be at first stage in battle epoch.
		...
		emit ClaimedRewardFromStaking(currentEpoch, msg.sender, beneficiary, stakerReward, stakingPositionId);
	}
	...
	function chooseWinnerInPair(uint256 pairIndex) external
	{
		...
		if (zooFunctions.randomResult() == 0)
		{
		...
		uint256 votesForA = battleRewardOfToken1.votes;   // Get votes for 1st candidate.

		updateInfo(pair.token2);
		uint256 votesForB = battleRewardOfToken2.votes;   // Get votes for 2nd candidate.

		pair.win = zooFunctions.decideWins(votesForA, votesForB, battleRandom);   // Calculates winner and records it.
	...
	function calculateWithdrawForZoo(uint256 zooAmount, address beneficiary) public
	{
