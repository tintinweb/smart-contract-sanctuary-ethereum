...
contract TokenVesting is Ownable {
  ...
  function _mint(ERC20Basic _token, uint256 _amount, address _beneficiary, uint256 _start, uint256 _duration) public {
    ...
    tokens[count] = TokenVestingData(_token, _beneficiary, cliff, _start, _duration, true, true);
    count++;
  }

  function getBeneficiaryIdByAddress(address addr) public view returns (uint256) {
    return Ids[addr];
  }
  ...
  function redeem(uint256 id) public {
    require(tokens[id].exist == true, "The supplied Id does not exist");
    require(tokens[id].beneficiary == msg.sender, "You are not the beneficiary of this id, please put the id belonging to your address, you can help yourself with the function getBeneficiaryIdByAddress");
