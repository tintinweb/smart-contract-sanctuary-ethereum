...
contract Yoplex is Ownable, ChainlinkClient{
    ...
    struct WithdrawalRequestStruct {
        address user;
		uint amount_USD;
        uint amount_YPLX;
    }
    ...
    struct UserStruct {
        uint id;
        address[] referral;  
        uint curr_investment_USD;
        uint curr_investment_YPLX;   		
		uint ROI_taken_USD;
        uint ROI_taken_BNB;
		uint ROI_taken_time;
    }
	...
	uint private lock_period = 10 minutes;
    ...
    mapping (bytes32 => WithdrawalRequestStruct) private withdrawal_request;
    mapping (bytes32 => uint) private investment_index;
    mapping (address => bytes32) public address_to_withdraw_request;
    ...
    function regUser(address payable _referrerID) public payable {
        ...
        userStruct = UserStruct({
            ...
            curr_investment_USD: 0,
            curr_investment_YPLX: 0,      
            ROI_taken_USD: 0,
			ROI_taken_BNB: 0,
    ...
    function invest() public payable {
        ...
        users[msg.sender].curr_investment_USD += BNB_to_USD(msg.value);
        users[msg.sender].curr_investment_YPLX += USD_to_token(BNB_to_USD(msg.value));
	...
	function YPLX_Withdrawal() public returns (bool) {
        ...
        WithdrawalRequestStruct memory withdrawalRequestStruct;
        withdrawalRequestStruct = WithdrawalRequestStruct({
            user: msg.sender,
        ...
        request.add("get", string(abi.encodePacked(base_URL, "withdrawYPLX/?address=0x", toAsciiString(msg.sender))));
        request.add("path", "data");
        bytes32 request_id = sendChainlinkRequestTo(oracle, request, fee);
        withdrawal_request[request_id] = withdrawalRequestStruct;       
        address_to_withdraw_request[msg.sender] = request_id;
        return true;
    } 

    function fulfill2(bytes32 _requestId, uint256 _volume) public recordChainlinkFulfillment(_requestId){
        WithdrawalRequestStruct memory withdrawInfo = withdrawal_request[_requestId];
		users[withdrawInfo.user].curr_investment_USD -= withdrawInfo.amount_USD;
        users[withdrawInfo.user].curr_investment_YPLX -= withdrawInfo.amount_YPLX;
        ...
        withdrawalStruct = WithdrawalStruct({
            id: withdrawalID,
            time: block.timestamp,
            amount_USD: withdrawInfo.amount_USD,
            amount_YPLX: withdrawInfo.amount_YPLX         
        });

        withdawal[withdrawInfo.user].push(withdrawalStruct);
		emit WithdrawalEvent(withdrawInfo.user, withdrawInfo.amount_USD, withdrawInfo.amount_YPLX, block.timestamp);      
    } 
