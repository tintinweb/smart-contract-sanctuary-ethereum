...
contract Vault is Initializable, PositionManager {
    ...
    function initialize(
        ...
        PositionManager__initialize(_positionFee, _liquidationFee, _interestRate, _accrualInterval);
    }
    ...
    function setFee(
        ...
        fee.setFee(_positionFee, _liquidationFee);
    }
...
import {Position, PositionUtils, LONG, SHORT, IncreasePositionResult, DecreasePositionResult} from "./Position.sol";
...
contract PositionManager is AssetManager {
    ...
    function increasePosition(
        ...
        uint256 feeAmount = result.feeValue / collateralPrice;
        poolAssets[_collateralToken].increaseReserve(result.reserveAdded, feeAmount);
    ...
    function decreasePosition(
        ...
        poolAssets[_collateralToken].decreaseReserve(result.reserveReduced, feeAmount);
        ...
        _doTransferOut(_indexToken, _account, result.payout);
    }
    ...
    function getCollateralToken(uint256 _side, address _indexToken) internal view returns (address) {
    ...
    function getPositionKey(
        ...
        return keccak256(abi.encodePacked(_account, _collateralToken, _market, side));
    }
    ...
    function _doTransferOut(
        ...
        if (_amount > 0) {
            IERC20(_token).transferTo(_to, _amount);
        }
    }
...
library FeeUtils {
    ...
    function calcInterest(
        ...
        return
            (size *
                (self.cumulativeInterestRate - entryCumulativeInterestRate)) /
            INTEREST_RATE_PRECISION;
    }

    function calcPositionFee(Fee memory self, uint256 sizeChanged)
        internal
        pure
        returns (uint256)
    {
    ...
    function calcLiquidationFee(Fee memory self, uint256 size)
        internal
        pure
        returns (uint256)
    {
    ...
    function cumulativeInterest(Fee storage self) internal {
        ...
        uint256 nInterval = (_now - self.lastAccrualTimestamp) /
            self.accrualInterval;
        self.cumulativeInterestRate += nInterval * self.interestRate;
        self.lastAccrualTimestamp = nInterval * self.accrualInterval;
    }

    function setInterestRate(Fee storage self, uint256 interestRatePerYear, uint accrualInterval)
        internal
    {
        self.accrualInterval = accrualInterval;
        self.interestRate = interestRatePerYear * accrualInterval / SECONDS_PER_YEAR;
    }

    function setFee(Fee storage self, uint256 positionFee, uint256 liquidationFee) internal {
...
library PositionUtils {
    ...
    function decreaseUnchecked(
        ...
        position.collateralValue = collateral.isNeg() ? 0 : collateral.abs;
        position.size = position.size - sizeChanged;
        position.entryInterestRate = fee.cumulativeInterestRate;
    ...
    function calcAveragePrice(
        ...
        SignedInt memory divisor = side == LONG ? nextSize.add(pnl) : nextSize.sub(pnl);
    ...
    function validatePosition(Position storage position, bool isIncrease) internal view {
        if (isIncrease) {
            require(position.size >= 0, "Position: invalid size");
        }
        require(position.size >= position.collateralValue, "Position: invalid leverage");
        require(position.size <= position.collateralValue * MAX_LEVERAGE, "POSITION: max leverage exceeded");
    }
...
import {PositionUtils, SHORT} from "../position/Position.sol";
import {SignedInt, SignedIntOps} from "../lib/SignedInt.sol";
...
library PoolAssetImpl {
    ...
    function increaseLongPosition(
        ...
        self.poolAmount = self.poolAmount + collateralAmountIn - feeAmount;
        // ajust guaranteed
    ...
    function increaseShortPosition(
        ...
        self.averageShortPrice = PositionUtils.calcAveragePrice(SHORT, lastSize, sizeChanged, entryPrice, indexPrice);
    ...
    function decreaseLongPosition(
        ...
        self.poolAmount -= payoutAmount + feeAmount;
    }
    ...
    function calcManagedValue(PoolAsset storage self, uint256 price) internal view returns (SignedInt memory aum) {
        ...
        SignedInt memory longPnl = SignedIntOps.wrap(self.poolAmount).sub(self.reservedAmount).mul(price).add(
            self.guaranteedValue
        );

        console.log("calc aum", self.poolAmount, price);
        console.log("calc aum", longPnl.abs);
        console.log("calc aum", shortPnl.abs);
        aum = longPnl.sub(shortPnl);
    }
...
uint constant VALUE_PRECISION = 1e24;
...
abstract contract AssetManager is Ownable, ReentrancyGuard {
    ...
    uint constant LP_INITIAL_PRICE = 1e6;
    ...
    /// @dev since oracle return price in precision of 10 ^ (24 - token decimals)
    /// this function will returns dollar value with decimals of 24
    ...
    function _calcLpAmount(address token, uint256 amount) internal view returns (uint256) {
        ...
        uint256 lpAmount = (amount * tokenPrice * poolValue) / lpSupply / VALUE_PRECISION;
        return lpAmount;
    }
    ...
    function _getPoolValue() internal view returns (uint256 sum) {
        ...
        console2.log("getPoolValue", aum.sig, aum.abs);
