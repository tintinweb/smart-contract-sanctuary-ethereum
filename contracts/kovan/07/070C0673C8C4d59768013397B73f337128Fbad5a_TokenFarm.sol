...
contract TokenFarm is Ownable {
    ...
    // Mapping of address to unique tokens staked
    ...
    function issueTokens() public onlyOwner {
        ...
        for (
            ...
            // Send recipient token reward based on total value staked
    ...
    function unstakeToken(address _token) public {
        ...
        //     // Should remove sender from stakers list
    ...
    function getRecipientTotalValue(address _user)
        public
        view
        returns (uint256)
    {
        ...
        // Loop through each token to get total value
        for (
            uint256 allowedTokensIndex = 0;
            allowedTokensIndex < allowedTokens.length;
            allowedTokensIndex++
        ...
        returns (uint256)
    {
        if (uniqueTokensStaked[_user] <= 0) {
            ...
            // Get price of token * stakingBalance[_token][user]
            (uint256 price, uint256 decimals) = getTokenValue(_token);
            uint256 modifiedPrice = price / (10**decimals);
            return stakingBalance[_token][_user] * modifiedPrice;
        }
    }
