...
contract RevShareDistributor is
    Initializable,
    OwnableUpgradeable,
    KeeperCompatible
{
    ...
    function distribute() public onlyOwner {
        (address[] memory users, uint256[] memory userLevels) = xpContract
            .getXPLevels();
        LevelAllocation[] memory levelAllocations = _getLevelAllocations(
            users,
            userLevels
        );
    ...
    function _getLevelAllocations(
        address[] memory users,
        uint256[] memory userLevels
    ) internal view returns (LevelAllocation[] memory) {
        LevelAllocation[] memory levels = new LevelAllocation[](
            xpLevelDistributions.length
        );
        uint256 epochRevenue = gOhmContract.balanceOf(address(this)) -
            prevBalance +
            epochClaimedAmount;
        for (uint256 i = 0; i < users.length; i++) {
        ...
        for (uint256 i = 0; i < levels.length; i++) {
            uint256 xpAllocatedForLevel = (epochRevenue *
                xpLevelDistributions[i]) / 100;
    ...
    function setXpLevelDistributions(uint256[] memory _xpLevelDistributions)
        public
        onlyOwner
    {
        require(
            _xpLevelDistributions[0] == 0,
            "First xp level must be default (0 allocation)"
        );
        require(
            sumValues(_xpLevelDistributions) == 100,
            "Total xp level distribution must equal 100"
        );
        xpLevelDistributions = _xpLevelDistributions;
    }

    function sumValues(uint256[] memory _values)
        internal
        pure
        returns (uint256)
    {
    ...
    function getUserLevelOrMax(uint256 userLevel)
        public
        view
        returns (uint256)
    {
