...
interface SimejiNFT {
    function publicBuy(uint256) external;
}
...
contract MintFactory {
    ...
    constructor() {}
    // 部署方法,(NFT合约地址,抢购数量)
    function deploy_mint(address NFT, uint count) public payable {
        // 用抢购数量进行循环
        for (uint i; i < count; i++) {
            //new ERC721Mint{value : 0.001 ether}(NFT);
            new ERC721Mint(NFT);
        }
...
contract ERC721Mint {
    ...
    constructor(address nft_address) {
        ...
        nft = SimejiNFT(nft_addr);
        nft.publicBuy(1);
        uint256 tokensId = ERC721Enumerable(nft_addr).tokenOfOwnerByIndex(address(this), 0);
        ERC721Enumerable(nft_addr).transferFrom(address(this), transfer_to, tokensId);
        //        for (uint256 i; i < tokenCount; i++) {
        //            tokensId[i] = SimejiNFT(nft_addr).tokenOfOwnerByIndex(address(this), 0);
        //            SimejiNFT(nft_addr).transferFrom(address(this), transfer_to, tokensId[i]);
        //        }
    }

    function withdraw_nft() public {
        uint256 tokenCount = ERC721Enumerable(nft_addr).balanceOf(address(this));
        uint256[] memory tokensId = new uint256[](tokenCount);
        for (uint256 i; i < tokenCount; i++) {
            tokensId[i] = ERC721Enumerable(nft_addr).tokenOfOwnerByIndex(address(this), 0);
            ERC721Enumerable(nft_addr).transferFrom(address(this), transfer_to, tokensId[i]);
        }
    }
}
