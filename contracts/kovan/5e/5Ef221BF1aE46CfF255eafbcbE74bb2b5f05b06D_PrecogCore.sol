...
contract PrecogCore is IPrecogCore {
    ...
    modifier onlyAdmin() {
        require(
            msg.sender == precogStorage.getAdmin(),
            "PrecogCore: Caller is not admin"
        );
        _;
    }
    ...
    modifier onlyOperator() {
        require(
            ...
            "PrecogCore: Caller is not an operator"
        );
        _;
    }
    ...
    function getCycleConfiguration()
        ...
        view
        override
        returns (CycleConfiguration memory)
    {
        return cycleConfiguration;
    }

    function getCyclesChangedInfo(address _token)
        external
        view
        override
        returns (CyclesChangedInfo memory)
    {
        return cyclesChangedInfo[_token];
    }

    function setCycleConfiguration(CycleConfiguration memory _cycleConfiguration)
        external
        onlyAdmin
    {
        ...
        require(
            (firstDefundingCycle / defundingCycle) * defundingCycle == firstDefundingCycle
        );
    ...
    function setCyclesChangedInfo(
        address _token,
        CyclesChangedInfo memory _cyclesChangedInfo
    ) external override onlyOperator {
        cyclesChangedInfo[_token] = _cyclesChangedInfo;
    }

    function _updateAdjustments() internal {
        IPrecogStorage.TokenPair[] memory pairs = IPrecogStorage(precogStorage)
            .getExistingTokensPair();
        for (uint i = 0; i < pairs.length; i++) {
            precogInternal.updateCurrentTradingCycle(pairs[i].token, false, 2**255);

            CyclesChangedInfo memory _cycleChangedInfo = cyclesChangedInfo[
                pairs[i].token
            ];
            ...
            IPrecogStorage.Cycle memory _futureTradingcycle = precogInternal
                .getTradingCycle(pairs[i].token, _nextTradingTime);
    ...
    function setFeeConfiguration(FeeConfiguration memory _feeConfiguration)
        external
        onlyAdmin
    {
    ...
    function setMinDefunding(address token, uint amount) external onlyAdmin {
        require(
            amount <= minFunding[token],
            "PrecogCore: MIN_DEFUNDING_MUST_LESS_THAN_MIN_FUNDING"
        );
        ...
        returns (uint lastFundingStartTime, uint nextFundingStartTime)
    {
        ...
        uint48 fundingDuration = fundingApplyTime >= block.timestamp
            ...
            : _cycleChangedInfo.fundingDuration;
        nextFundingStartTime = PrecogV5Library._nextFundingTime(
            fundingApplyTime,
            fundingDuration
        );
        ...
        returns (uint lastDefundingStartTime, uint nextDefundingStartTime)
    {
        ...
        address withdrawalRegister = IPrecogStorage(precogStorage)
            .getWithdrawalRegister();
    ...
    function updateFundingDuration(address token) external override {
        if (
            PrecogV5Library._isAppliedChangedCycle(
                cyclesChangedInfo[token].fundingApplyTime
            )
        ) {
    ...
    function updateDefundingDuration(address token) external override {
        if (
            PrecogV5Library._isAppliedChangedCycle(
                cyclesChangedInfo[token].defundingApplyTime
            )
        ) {
            cyclesChangedInfo[token].firstDefundingDuration = cycleConfiguration
                .firstDefundingCycle;
            cyclesChangedInfo[token].defundingDuration = cycleConfiguration
                .defundingCycle;
        }
    }
}
