...
contract SafeVaultProxyFactory {
  event ProxyCreation(GnosisSafeProxy proxy, address singleton);
  GnosisSafeProxy[] public safeVaults;
  ...
  function createProxyWithCallback(
    ...
    proxy = createProxyWithNonce(initializer, saltNonceWithCallback);
    if (address(callback) != address(0))
      callback.proxyCreated(proxy, singleton, initializer, saltNonce);
  }
