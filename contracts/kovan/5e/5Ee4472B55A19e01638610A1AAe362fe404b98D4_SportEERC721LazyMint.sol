...
import "./SportEERC721Enumerable.sol";
import "../lib/utils/cryptography/draft-EIP712.sol";

contract SportEERC721LazyMint is SportEERC721Enumerable, EIP712 {
    mapping(address => bool) public signers;

    string private constant SIGNING_DOMAIN = "SPORTENFT";
    ...
    function equip(uint256 tokenId, bytes calldata signature) external {
        ...
        returns (bytes32)
    {
        return
            _hashTypedDataV4(
                keccak256(
                    abi.encode(
                        keccak256("SPORTENFT(address owner, uint256 tokenId)"),
                        owner,
                        tokenId
                    )
                )
...
import "./SportEERC721.sol";
import "../lib/token/ERC721/extensions/ERC721Enumerable.sol";

contract SportEERC721Enumerable is SportEERC721, ERC721Enumerable {
    /**
     * @dev Create a new SportEERC721Enumerable contract.
     *
     */
    constructor() SportEERC721() {}
    ...
    function tokenURI(uint256 tokenId)
        public
        view
        override(ERC721, SportEERC721)
        returns (string memory)
    {
        return SportEERC721.tokenURI(tokenId);
    }
    ...
    function _beforeTokenTransfer(
        ...
        address to,
        uint256 tokenId
    ) internal virtual override(SportEERC721, ERC721Enumerable) {
        SportEERC721._beforeTokenTransfer(from, to, tokenId);
...
contract SportEERC721 is ERC721Pausable, Ownable {
    ...
    function _transferable(uint256 tokenId)
        internal
        view
        virtual
        returns (bool)
    ...
    /**
     * @dev Create a new SportEERC721 contract.
     *
     */
    constructor() ERC721("SPORTE ERC721", "SPORTE") Ownable() Pausable() {
        __baseURI = "https://metadata.sporte.io/";
    }
    ...
    function equip(uint256 tokenId) internal virtual {
        require(
            ...
            "SportEERC721: equip caller is not owner nor approved"
        );
        require(!_idFreeze[tokenId], "SportEERC721: nft equipped");
        _idFreeze[tokenId] = true;
        emit Equip(msg.sender, tokenId);
    }

    function redeem(address redeemer, uint256 tokenId)
        internal
        virtual
        returns (uint256)
    {
        if (_exists(tokenId)) {
            //require(ownerOf(tokenId) == msg.sender, "SportEERC721: redeemer caller is not owner nor approved");
            //require(_idFreeze[tokenId], "SportEERC721: nft redeemed");
    ...
    function isFreeze(uint256 tokenId) public view virtual returns (bool) {
        require(
            _exists(tokenId),
            "ERC721Metadata: Freeze query for nonexistent token"
        );
        return _idFreeze[tokenId];
    }
    ...
    function _beforeTokenTransfer(
        ...
        require(
            _transferable(tokenId),
            "SportEERC721: token transfer while equipped"
        );
    }
}
