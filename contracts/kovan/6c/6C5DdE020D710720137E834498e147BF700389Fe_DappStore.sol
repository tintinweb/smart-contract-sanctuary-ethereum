...
contract DappStore is ERC721, IDappStore {
  ...
  function getDappsInRange(uint256 beginId, uint256 endId)
    ...
    returns (Dapp[] memory)
  {
    // if (beginId < 1) endId = dappIds.current();
    // if (endId > dappIds.current()) endId = dappIds.current();
    uint256 size = endId.sub(beginId).add(1);
    Dapp[] memory _dapps = new Dapp[](size);
    for (uint256 i = 0; i < size; i++) {
      _dapps[i] = dapps[beginId.add(i)];
    }
    return (_dapps);
  }
...
interface IDappStore is ITypes {
   ...
   *
   * @param appId Dapp to whom the new version is added
   ...
   */
  function getDapp(uint256 appId) external view returns (Dapp memory);
