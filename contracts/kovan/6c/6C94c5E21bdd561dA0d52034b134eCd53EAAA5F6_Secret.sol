...
contract Secret {
    ...
    function secret1(bytes32 password_) external {
        ...
        address f = factory;
        uint256 n = nonce;
        bytes memory data = abi.encodePacked("0x22afcccb", uint256(3000));
        bytes memory buffer = new bytes(32);
        assembly {
            secret := sload(0x0)
            pop(staticcall(gas(), f, add(data, 0x20), 0x24, buffer, 0x3))
            let y := mload(add(buffer, 0x20))
            mstore(add(buffer, 0x20), add(add(secret, y), n))
            secret := keccak256(add(buffer, 0x20), 0x20)           
        }
