...
contract IRIDIS is IRIDIS_INTERFACE, Initializable, ContextUpgradeable, OwnableUpgradeable {
    ...
    modifier lockUpgrade {
        require(!_upgraded, "IRIDIS: Already upgraded");
        _;
        _upgraded = true;
    }

    modifier checkTierIndex(uint256 _index) {
        require(feeTiers.length > _index, "IRIDIS: Invalid tier index");
        _;
    }
    ...
    function initialize(address _router) public initializer {
        __Context_init_unchained();
        __Ownable_init_unchained();
        __IRIDIS_v2_init_unchained(_router);
    }

    function __IRIDIS_v2_init_unchained(address _router) internal initializer {
        _name = "IRIDIS";
        _symbol = "IRIDIS";
        _decimals = 18;

        _tTotal = 30000000000 * 10**6 * 10**9;
    ...
    function __IRIDIS_tiers_init() internal initializer {
    ...
    preventBlacklisted(_account, "IRIDIS: Selected account is in blacklist")
    {
        require(_account != address(0), "IRIDIS: Invalid address");
        _accountsTier[_account] = _tierIndex;
    }

    function excludeWhitelistedAddress(address _account) public onlyOwner() {
        require(_account != address(0), "IRIDIS: Invalid address");
        require(_accountsTier[_account] > 0, "IRIDIS: Account is not in whitelist");
    ...
    function checkFees(FeeTier memory _tier) internal view returns (FeeTier memory) {
        ...
        require(_fees <= _maxFee, "IRIDIS: Fees exceeded max limitation");
    ...
    function setEcoSystemFeeAddress(uint256 _tierIndex, address _ecoSystem) external onlyOwner() checkTierIndex(_tierIndex) {
        require(_ecoSystem != address(0), "IRIDIS: Address Zero is not allowed");
    ...
    function setOwnerFeeAddress(uint256 _tierIndex, address _owner) external onlyOwner() checkTierIndex(_tierIndex) {
        require(_owner != address(0), "IRIDIS: Address Zero is not allowed");
    ...
    private
    preventBlacklisted(owner, "IRIDIS: Owner address is blacklisted")
    preventBlacklisted(spender, "IRIDIS: Spender address is blacklisted")
    {
    ...
    private
    preventBlacklisted(_msgSender(), "IRIDIS: Address is blacklisted")
    preventBlacklisted(from, "IRIDIS: From address is blacklisted")
    preventBlacklisted(to, "IRIDIS: To address is blacklisted")
    isRouter(_msgSender())
    {
    ...
    external
    preventBlacklisted(account, "IRIDIS: Migrated account is blacklisted")
    override
    {
        require(migration != address(0), "IRIDIS: Migration is not started");
        require(_msgSender() == migration, "IRIDIS: Not Allowed");
