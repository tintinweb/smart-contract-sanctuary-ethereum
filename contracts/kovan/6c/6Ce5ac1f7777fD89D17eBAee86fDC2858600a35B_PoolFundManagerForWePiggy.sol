...
contract PoolFundManagerForWePiggy is PoolFundManagerInterface,OwnableUpgradeable  {
    ...
    enum ActionType { DepositAction, RedeemAction, BorrowAction, RepayAction, AddReservesAction, ReduceReservesAction }
    ...
    function getTotalDepositStore(address token) public view returns(uint) {
        // totalWePiggy[token] = C   totalLoans[token] = U   totalReserve[token] = R
    ...
    function updateTotalReserve(address token, uint amount, ActionType action) internal returns(uint256 wepiggyAmount){
        ...
        if(action == ActionType.DepositAction || action == ActionType.RepayAction || action == ActionType.AddReservesAction){
            // update totalAmount when deposit or update totalBorrows when repay
            if(action == ActionType.DepositAction){
                totalAmount = totalAmount + amount;
            }else if(action == ActionType.RepayAction){
                totalBorrows[token] = totalBorrows[token] < amount ? 0 : totalBorrows[token] - amount;
            }
        ...
        }else if(action == ActionType.RedeemAction || action == ActionType.BorrowAction || action == ActionType.ReduceReservesAction){
            ...
            if(action == ActionType.RedeemAction){
                totalAmount = totalAmount < amount ? 0 : totalAmount - amount;
            }else if(action == ActionType.BorrowAction){
    ...
    function redeem(address redeemer, uint amount) external onlyMarketListed(msg.sender){
        ...
        uint wepiggyAmount = update(token, amount, ActionType.RedeemAction);
        if(wepiggyAmount > 0){
    ...
    function borrow(address borrower, uint amount) external onlyMarketListed(msg.sender){
        ...
        uint wepiggyAmount = update(token, amount, ActionType.BorrowAction);
        if(wepiggyAmount > 0){
            fromWePiggy(token, wepiggyAmount);
        }
    ...
    function repayBorrow(address borrower, uint amount) external payable onlyMarketListed(msg.sender){
        ...
        uint wepiggyAmount = update(token, amount, ActionType.RepayAction);
        if(wepiggyAmount > 0){
            toWePiggy(token, wepiggyAmount);
        }
    ...
    function addReserves(uint amount) external payable onlyMarketListed(msg.sender){
        require(amount != 0,"Amount is zero");
        address token = MarketTokenInterface(msg.sender).underlying();

        uint wepiggyAmount = update(token, amount, ActionType.AddReservesAction);
        if(wepiggyAmount > 0){
            toWePiggy(token, wepiggyAmount);
        }

    }
    function reduceReserves(uint amount) external payable onlyMarketListed(msg.sender){
        require(amount != 0,"Amount is zero");
        address token = MarketTokenInterface(msg.sender).underlying();

        uint wepiggyAmount = update(token, amount, ActionType.ReduceReservesAction);
        if(wepiggyAmount > 0){
            fromWePiggy(token, wepiggyAmount);
        }
    }
    ...
    function withdrwalInterest(MarketTokenInterface poolMarket, uint amount, address payable _beneficiary) external onlyOwner{
        
        uint exchangeRage = poolMarket.exchangeRateCurrent();
        uint totalSupply = poolMarket.totalSupply();
        uint requiredAmount = exchangeRage * totalSupply / 1e18;

        updateTotalWePiggy(poolMarket.underlying());
        uint interest = getTotalDepositStore(poolMarket.underlying()) - requiredAmount;
        require(interest > amount, "error amount");

        if(totalReserve[poolMarket.underlying()] < amount){
            fromWePiggy(poolMarket.underlying(), amount);
        }

        transferInternal(poolMarket.underlying(), _beneficiary , amount);
    }
