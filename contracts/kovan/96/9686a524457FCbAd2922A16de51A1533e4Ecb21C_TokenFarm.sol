...
contract TokenFarm is Ownable {
    ...
    mapping(address => uint256) public uniqueTokenStaked; /* nb of different token stacked by a user */
    mapping(address => address) public tokenPriceFeedMapping;
    IERC20 public dappToken;
    event TokenIssue(address _user, uint256 amount);
    ...
    function stakeToken(uint256 _amount, address _token) public {
        ...
        updateUniqueTokenStacked(_token, msg.sender);
    ...
    function unstackedToken(address _token) public {
    ...
    function updateUniqueTokenStacked(address _token, address _user) internal {
        if (stakingBalance[_token][_user] <= 0) {
            uniqueTokenStaked[_user] = uniqueTokenStaked[_user] + 1;
        }
    }

    // For every 1 ETH, we give one dapp token
    function issueTokens() public onlyOwner {
        for (
            ...
            // send them dapp token reward based on their total value locked.
            uint256 userTotalvalue = getUserTotalValue(recipient);
            emit TokenIssue(recipient, userTotalvalue);
    ...
    function getUserTotalValue(address _user) public view returns (uint256) {
        ...
        require(uniqueTokenStaked[_user] > 0, "No token stacked!");
        for (
