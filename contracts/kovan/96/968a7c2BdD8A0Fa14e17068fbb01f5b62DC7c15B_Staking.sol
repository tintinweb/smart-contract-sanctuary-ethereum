...
contract Staking is AccessControl {
    ...
    mapping(uint256 => uint256) public lockOwnersDeposits; //44
    ...
    function depositOrWLFromOwner(
        ...
        uint256 timestamp = block.timestamp;
        require((timestamp - deployTime) / 60 < 30 /*1 days < 3*/, "0");
    ...
    function setLocks(uint256[] memory lockPeriod, uint256[] memory index) external onlyRole(DEFAULT_ADMIN_ROLE)  { // 44
        require((block.timestamp - deployTime) / 60 < 30 /*1 days < 3*/ , "0");
        ...
        nonReentrant
    {
        ...
        if (
            ...
            (timestamp - deployTime) / 60 < 10 /*14 days < 1*/
        ) {
