...
contract Repayments is Initializable, IRepayment, ReentrancyGuardUpgradeable {
    ...
    modifier isPoolInitialized(address _poolID) {
        require(repayConstants[_poolID].numberOfTotalRepayments != 0, 'IPI1');
        _;
    }
    ...
    modifier onlyValidPool() {
        require(poolFactory.poolRegistry(msg.sender) != 0, 'OVP1');
        _;
    }
     ...
     */
    modifier onlyOwner() {
        require(msg.sender == poolFactory.owner(), 'OO1');
        _;
    }
    ...
    function _updatePoolFactory(address _poolFactory) internal {
        require(_poolFactory != address(0), 'IUPF1');
    ...
    function _repayInterest(
        ...
        require((_amount < _interestLeft) != _isLastRepayment, 'IRI1');
    ...
    function _repay(
        ...
        require(_loanStatus == uint256(IPool.LoanStatus.ACTIVE), 'IR1');

        uint256 _initialAmount = _amount;

        // pay off grace penality
        uint256 _gracePenaltyDue = _repayGracePenalty(_poolID);
        _amount = _amount.sub(_gracePenaltyDue, 'IR2');

        // pay interest
        uint256 _interestRepaid = _repayInterest(_poolID, _amount, _isLastRepayment);
        _amount = _amount.sub(_interestRepaid, 'IR4');

        return _updateRepaidAmount(_poolID, _initialAmount.sub(_amount, 'IR5'));
    }
    ...
    function repayPrincipal(address _poolID) external nonReentrant isPoolInitialized(_poolID) {
        ...
        require(repayConstants[_poolID].loanDuration == repayVariables[_poolID].loanDurationCovered, 'RP2');
    ...
    function getInterestPerSecond(address _poolID) public view returns (uint256) {
        ...
        uint256 _interestPerSecond = _activePrincipal.mul(repayConstants[_poolID].borrowRate).div(YEAR_IN_SECONDS).div(SCALING_FACTOR);
    ...
    function _getInterestDueTillInstalmentDeadline(address _poolID, uint256 _nextInstalmentDeadline) public view returns (uint256) {
        uint256 _loanDurationCovered = repayVariables[_poolID].loanDurationCovered;
    ...
    function _isGracePenaltyApplicable(address _poolID, uint256 _nextInstalmentDeadline) internal view returns (bool) {
        uint256 _repaymentInterval = repayConstants[_poolID].repaymentInterval;
        uint256 _currentTime = block.timestamp.mul(SCALING_FACTOR);
        ...
        require(_currentTime <= _gracePeriodDeadline, 'IGPA1');
