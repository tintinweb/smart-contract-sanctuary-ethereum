...
contract PrecogV5 is IPrecogV5 {
    ...
    function getExistingTokens() external override view returns (address[] memory) {
        return existingTokens;
    }

    function getExistingTokenByIndex(uint index) external override view returns (address) {
        ...
        )
    {
        ...
        while (uint48(block.timestamp) >= lastCycle.endTime) {
            ...
            if (lastCycle.endTime < lastChangeProfitCycle) {
    ...
    function sendProfit(address token, uint profitAmount) external override onlyMiddleware {
        ...
        Cycle memory lastProfitCycle = profitCycles[token][profitCycles[token].length - 1];
        require(
            _currentProfitCycle < lastProfitCycle.id,
        ...
        emit SendProfit(token, _currentProfitCycle, lastProfitCycle.startTime, lastProfitCycle.endTime, liquidity[token], profitAmount, amountBoughtPCOG);
...
interface IPrecogV5 {
    ...
    function investmentOf(address token, address account, uint index) external view returns (
        uint amount, 
        uint unit,
        uint16 idChanged
    );
    ...
    function profit(address token, uint index) external view returns (uint);
