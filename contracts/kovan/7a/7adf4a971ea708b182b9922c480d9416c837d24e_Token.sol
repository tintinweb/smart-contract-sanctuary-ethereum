...
contract Token {
    ...
    // Tal comment: my account address mapped to all the other accounts that allow to spend on my behalf
    // keeping track of the address allowing to sepnd on my behalf (the Uniswap pool takes the tokens and transfer it on my behalf) 
    // this function mapped by the approve() function down below.
    mapping(address => mapping(address => uint256)) public allowance;

    // Events - fire events on state changes etc
    event Transfer(address indexed from, address indexed to, uint256 value);
    // tal: approval event, metamask will ask you to approve this 
