...
contract dMONO is ERC20("dMONO", "dMONO"), Ownable {
    using SafeMath for uint256;
    IERC20 public mono;

    uint INITIAL_SUPPLY = 31*10**24;
    ...
    // Locks Mono and mints dMONO
    // function enter(uint256 _amount) public {
    //     // Gets the amount of Mono locked in the contract
    //     uint256 totalMono = mono.balanceOf(address(this));
    //     // Gets the amount of dMONO in existence
    //     uint256 totalShares = totalSupply();
    //     // If no dMONO exists, mint it 1:1 to the amount put in
    //     if (totalShares == 0 || totalMono == 0) {
    //         _mint(msg.sender, _amount);
    //     } 
    //     // Calculate and mint the amount of dMONO the Mono is worth. The ratio will change overtime, as dMONO is burned/minted and Mono deposited + gained from fees / withdrawn.
    //     else {
    ...
    // Unlocks the staked + gained Mono and burns dMONO
    function leave(uint256 _share) public {
        // Gets the amount of dMONO in existence
        uint256 totalShares = totalSupply();
        // Calculates the amount of Mono the dMONO is worth
