...
import "../precog/libraries/PrecogV5Library.sol";
...
contract PrecogCore is IPrecogCore {
    ...
    modifier onlyAdmin() {
        require(msg.sender == coreConfiguration.admin, "PrecogCore: NOT_ADMIN_ADDRESS");
        _;
    }
    ...
    mapping(address => uint256) public override minFunding;
    mapping(address => uint256) public override minDefunding;


    constructor(address _admin, address _middleware, address _PCOG, address _exchange) {
        PCOG = _PCOG;
        coreConfiguration = CoreConfiguration(_admin, _middleware, _exchange);
        feeConfiguration = FeeConfiguration(1000000000000000, 1000000000000000, 1000000000000000, 1000000000000000);
        cycleConfiguration = CycleConfiguration(604800, 604800, 86400, 172800, 86400);
    }

    //

    function getCoreConfiguration() external view override returns (CoreConfiguration memory _coreConfiguration) {
        _coreConfiguration = coreConfiguration;
    }

    function getFeeConfiguration() external view override returns (FeeConfiguration memory _feeConfiguration) {
        _feeConfiguration = feeConfiguration;
    }

    function getCycleConfiguration() external view override returns (CycleConfiguration memory _cycleConfiguration) {
        _cycleConfiguration = cycleConfiguration;
    }

    function setCoreConfiguration(CoreConfiguration memory _coreConfiguration) external onlyAdmin {
        emit SetCoreConfiguration(coreConfiguration.admin, _coreConfiguration.admin, _coreConfiguration.middleware, _coreConfiguration.exchange);
        coreConfiguration = _coreConfiguration;
    }

    function setCycleConfiguration(CycleConfiguration memory _cycleConfiguration) external onlyAdmin {
        uint32 firstTradingCycle = _cycleConfiguration.firstTradingCycle;
        uint32 firstDefundingCycle = _cycleConfiguration.firstDefundingCycle;
        uint32 fundingCycle = _cycleConfiguration.fundingCycle;
        uint32 defundingCycle = _cycleConfiguration.defundingCycle;
        uint32 tradingCycle = _cycleConfiguration.tradingCycle;

        require(firstDefundingCycle > 0 && defundingCycle > 0 && fundingCycle > 0);
        require(fundingCycle <= firstTradingCycle && fundingCycle <= tradingCycle);
        require(firstTradingCycle >= firstDefundingCycle && firstTradingCycle >= defundingCycle);
        ...
        cycleConfiguration = _cycleConfiguration;
        emit SetCycleConfiguration(
            coreConfiguration.admin,
            firstTradingCycle,
            firstDefundingCycle,
            fundingCycle,
            defundingCycle,
    ...
    function _updateAdjustments() internal {
        IPrecogV5.TokenPair[] memory pairs = IPrecogV5(precog).getExistingTokensPair();
        for(uint i = 0; i < pairs.length; i++) {
            IPrecogV5(precog).updateCurrentTradingCycle(pairs[i].token, 2**255);
            IPrecogV5(precog).updateAdjustment(pairs[i].token);
        }
    }
    ...
    function setFeeConfiguration(FeeConfiguration memory _feeConfiguration) external onlyAdmin {
        ...
        emit SetFeeConfiguration(
            coreConfiguration.admin, 
            _feeConfiguration.depositFee, 
            _feeConfiguration.withdrawalFee,
            _feeConfiguration.tradingFee,
            _feeConfiguration.lendingFee
        );
    }

    function collectFee(address token) external onlyAdmin {
        emit CollectFee(coreConfiguration.admin, token, IERC20(token).balanceOf(address(this)));
        IERC20(token).safeTransfer(coreConfiguration.admin, IERC20(token).balanceOf(address(this)));
    }
    ...
    function setPrecog(address newPrecog) external onlyAdmin {
        precog = newPrecog;
    }


    function getCurrentFundingCycle(address token) external view override returns (uint lastFundingStartTime, uint nextFundingStartTime) {
        IPrecogV5.CyclesChangedInfo memory cycleChangedInfo = IPrecogV5(precog).getCycleChangedInfo(token);
        uint48 fundingApplyTime = cycleChangedInfo.fundingApplyTime;
        uint48 fundingDuration = fundingApplyTime >= block.timestamp ? cycleConfiguration.fundingCycle : cycleChangedInfo.fundingDuration;
    ...
    function getCurrentDefundingCycle(address token) external view returns (uint lastDefundingStartTime, uint nextDefundingStartTime) {
        IPrecogV5.CyclesChangedInfo memory cycleChangedInfo = IPrecogV5(precog).getCycleChangedInfo(token);
        ...
        (, ,address withdrawalRegister, ) = IPrecogV5(precog).getConfig();
        if(IWithdrawalRegister(withdrawalRegister).isFirstWithdraw(token, msg.sender)) {
            if(IWithdrawalRegister(withdrawalRegister).isInDeadline(token, msg.sender)) {
                nextDefundingStartTime = IWithdrawalRegister(withdrawalRegister).getRegister(token, msg.sender).deadline;
            } else {
                nextDefundingStartTime = PrecogV5Library._nextDefundingTime(defundingApplyTime, defundingDuration, firstDefundingDuration);
            }
        } else {
            nextDefundingStartTime = PrecogV5Library._nextDefundingTime(defundingApplyTime, defundingDuration, 0);
        }
        lastDefundingStartTime = PrecogV5Library._nextDefundingTime(defundingApplyTime, defundingDuration, 0) - defundingDuration;
    }
...
interface IPrecogCore {
    ...
    function getCurrentFundingCycle(address token) external view returns (uint lastFundingStartTime, uint nextFundingStartTime);
