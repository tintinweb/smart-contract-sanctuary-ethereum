    ...
    ReentrancyGuardUpgradeable
{
    ...
    IVendorOracle private priceFeed;
    IPoolFactory private factory;
    IFeesManager private feeManager;
    ...
    function withdraw(uint256 _amount) public nonReentrant {
        ...
        if (
            ...
            _amount + ((totalFees * protocolFee) / 10000)
    ...
    function _payVendorFees() private {
        uint256 feeDue = (totalFees * protocolFee) / 10000;
        _safeTransfer(lendToken, treasury, feeDue);
        _safeTransfer(
            colToken,
            treasury,
            (colToken.balanceOf(address(this)) * protocolColFee) / 10000
        );
        totalFees = 0;
    }
    ...
    {
        return
            (_repayAmount * colToken.balanceOf(address(this))) / totalBorrowed;
    }
    ...
    ///@param _feeRates Array for fees of the pool
    function setFeeRates(uint256[] calldata _feeRates) external {
        onlyOwner();
        feeManager.setPoolFees(address(this), _feeRates);
    }

    function extendExpiry(uint256 _newDate) external {
        onlyOwner();
        if (_newDate <= expiry || _newDate >= block.timestamp + 4 weeks)
