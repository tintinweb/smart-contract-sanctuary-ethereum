...
contract Gravity is KeeperCompatibleInterface {
    ...
    event PerformUpkeepSucceeded(uint now, uint lastTimeStamp, uint targetPurchased);
    event PerformUpkeepFailed(uint now, uint lastTimeStamp, uint toPurchase);
    ...
    constructor(address _sourceToken, address _targetToken, uint _upKeepInterval) {
        ...
        lastTimeStamp = block.timestamp - (block.timestamp % _upKeepInterval);// + _upKeepInterval;
    ...
    function initiateNewStrategy(address _sourceAsset, address _targetAsset, uint _sourceBalance, uint _interval, uint _purchaseAmount) public {
        ...
        uint _accountStart = lastTimeStamp; //  [V2 removed 2 *]
    ...
    function checkUpkeep(bytes calldata /* checkData */) external view override returns (bool upkeepNeeded, bytes memory /* performData */) {
        ...
        if((_now - lastTimeStamp) > upKeepInterval) {
            uint _toPurchase = accumulatePurchaseOrders(lastTimeStamp);
    ...
    function performUpkeep(bytes calldata /* performData */) external override {
        ...
        uint _now = block.timestamp; // [V2] get block.timestamp once
        ...
        if((_now - lastTimeStamp) > upKeepInterval) { // [V2]
            ...
            if (_toPurchase > 0) {
                ...
                for(uint i = 0; i < purchaseOrders[lastTimeStamp].length; i++) {
                    accounts[purchaseOrders[lastTimeStamp][i].user].scheduledBalance -= purchaseOrders[lastTimeStamp][i].purchaseAmount;
                    accounts[purchaseOrders[lastTimeStamp][i].user].purchasesRemaining -= 1;
                    accounts[purchaseOrders[lastTimeStamp][i].user].targetBalance += purchaseOrders[lastTimeStamp][i].purchaseAmount * _targetPurchased / _toPurchase;
                    accounts[purchaseOrders[lastTimeStamp][i].user].accountStart = lastTimeStamp;
                }
                
                // delete purchaseOrder post swap
                delete purchaseOrders[lastTimeStamp];
                 // increment only if purchase executed
                lastTimeStamp += upKeepInterval; // [V2]
                emit PerformUpkeepSucceeded(_now, lastTimeStamp, _targetPurchased);
            } else {
                emit PerformUpkeepFailed(_now, lastTimeStamp, _toPurchase);
            }
        }
    }
