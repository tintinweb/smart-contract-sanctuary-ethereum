...
contract LegionNFT is Context, IERC721, Ownable {
    ...
    function mint(string memory name, uint256[] memory beastIds, uint256[] memory warriorIds) external {
        ...
        tokenData[_tokenIds.current()] = Legion(name, "QmTyQ3Mab2711GWGMyvXqMVCQpK49WH1KQRzhUBJS96Q7W", beastIds, warriorIds, 0, attack_power, block.timestamp, false);
        lastHuntTime[_tokenIds.current()] = 0;
        _tokenIds.increment();
    }

    function canHunt(uint256 tokenId) internal returns (bool) {
        return ownerOf(tokenId)==msg.sender
            && block.timestamp >= lastHuntTime[tokenId] + 1 days
            && tokenData[tokenId].supplies > 0
            && !tokenData[tokenId].onMarket;
    }

    function hunt(uint256 tokenId) external returns(uint256, uint256, uint256){
        if(canHunt(tokenId)) {
            ...
            return (percent, ap, reward);
        }
        return (0,0,0);
    }

    function getLegion(uint256 tokenId) external view 
        returns(string memory, string memory, uint256[] memory, uint256[] memory, uint256, uint256, bool) {
        return (
            tokenData[tokenId].name,
            tokenData[tokenId].imgUrl,
    ...
    function addWarriors(uint256 tokenId, uint256[] memory warriorIds) external {
        ...
        uint256 unit = 10**18;
        uint256 bloodstoneAmount = tokenData[tokenId].supplies.mul(supplyPrice).mul(unit) + itemPrice.mul(warriorCount).mul(unit).div(denominator);
        require(bloodstone.balanceOf(msg.sender) >= bloodstoneAmount, "Insufficient balance");
        bloodstone.transferFrom(msg.sender, rewardPoolAddr, bloodstoneAmount);
    }
...
interface IMonster {
    function getMonsterInfo(uint256 monsterId) external returns(uint256, uint256, uint256);
    function getMonsterToHunt(uint256 ap) external returns (uint256);
}
...
contract Monster is IMonster {
    ...
    mapping (uint256 => Monster) rewardTable;
    modifier onlyLegion() {
        require(msg.sender == legion); _;
    }
    ...
    function initiateRewardTable() internal {
        rewardTable[1].percent = 85;
        rewardTable[1].attack_power = 2000;
        rewardTable[1].reward = 65;
        rewardTable[2].percent = 82;
        rewardTable[2].attack_power = 5000;
        rewardTable[2].reward = 160;
        rewardTable[3].percent = 78;
        rewardTable[3].attack_power = 8000;
        rewardTable[3].reward = 260;
        rewardTable[4].percent = 75;
        rewardTable[4].attack_power = 10000;
        rewardTable[4].reward = 325;
        rewardTable[5].percent = 72;
        rewardTable[5].attack_power = 13000;
        rewardTable[5].reward = 440;
        rewardTable[6].percent = 68;
        rewardTable[6].attack_power = 17000;
        rewardTable[6].reward = 605;
        rewardTable[7].percent = 65;
        rewardTable[7].attack_power = 20000;
        rewardTable[7].reward = 740;
        rewardTable[8].percent = 62;
        rewardTable[8].attack_power = 22000;
        rewardTable[8].reward = 850;
        rewardTable[9].percent = 59;
        rewardTable[9].attack_power = 25000;
        rewardTable[9].reward = 1010;
        rewardTable[10].percent = 55;
        rewardTable[10].attack_power = 28000;
        rewardTable[10].reward = 1210;
        rewardTable[11].percent = 52;
        rewardTable[11].attack_power = 31000;
        rewardTable[11].reward = 1410;
        rewardTable[12].percent = 49;
        rewardTable[12].attack_power = 34000;
        rewardTable[12].reward = 1620;
        rewardTable[13].percent = 45;
        rewardTable[13].attack_power = 37000;
        rewardTable[13].reward = 1900;
        rewardTable[14].percent = 42;
        rewardTable[14].attack_power = 40000;
        rewardTable[14].reward = 2150;
        rewardTable[15].percent = 41;
        rewardTable[15].attack_power = 42000;
        rewardTable[15].reward = 2450;
        rewardTable[16].percent = 41;
        rewardTable[16].attack_power = 47000;
        rewardTable[16].reward = 2950;
        rewardTable[17].percent = 41;
        rewardTable[17].attack_power = 50000;
        rewardTable[17].reward = 3250;
        rewardTable[18].percent = 39;
        rewardTable[18].attack_power = 53000;
        rewardTable[18].reward = 3800;
        rewardTable[19].percent = 39;
        rewardTable[19].attack_power = 56000;
        rewardTable[19].reward = 4300;
        rewardTable[20].percent = 39;
        rewardTable[20].attack_power = 60000;
        rewardTable[20].reward = 4900;
        rewardTable[21].percent = 35;
        rewardTable[21].attack_power = 250000;
        rewardTable[21].reward = 23000;
        rewardTable[22].percent = 30;
        rewardTable[22].attack_power = 300000;
        rewardTable[22].reward = 33000;
    }
    
    function getMonsterInfo(uint256 monsterId) external override returns(uint256, uint256, uint256) {
        require(monsterId>0&&monsterId<23, "Monster is not registered");
        return (rewardTable[monsterId].percent, rewardTable[monsterId].attack_power, rewardTable[monsterId].reward);
    }

    function getMonsterToHunt(uint256 ap) external override onlyLegion returns (uint256) {
        uint256 retVal = 0;
        for(uint i=1;i<23;i++) {
            if(ap<=rewardTable[i].attack_power) continue;
            retVal = i-1;
            break;
        }
        return retVal;
