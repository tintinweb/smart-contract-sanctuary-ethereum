...
contract FlowerFamMinter is Ownable {
    ...
    uint256 public startTimeWL = 1653148800; /// @dev unix timestamp of start time of whitelist mint (first round).
    ...
    mapping(uint256 => uint256) public roundToMaxSupply; /// @dev max supply per round
    mapping(uint256 => uint256) public roundToMints; /// @dev number of flowers minted per round
    ...
    constructor(
        ...
        roundToMaxSupply[whitelistRound] = 2000;
        roundToMaxSupply[giveawaysRound] = 4550;
    ...
    function _roundToMaxSupply(uint256 round) internal view returns (uint256) {
        if (round < raffleRound) {
            return roundToMaxSupply[round];
        } else if (round == raffleRound) {
            uint256 mintedFromWl = roundToMints[whitelistRound];
            uint256 mintedFromGiveaway = roundToMints[giveawaysRound];
            uint256 mintedTotal = mintedFromWl + mintedFromGiveaway;

             /// @dev should not be possible
            if (mintedTotal > maxSupply)
                return 0;
            else 
                return maxSupply - mintedTotal;
        } else if (round == publicRound) {
            uint256 mintedFromWl = roundToMints[whitelistRound];
            uint256 mintedFromGiveaway = roundToMints[giveawaysRound];
            uint256 mintedFromRaffle = roundToMints[raffleRound];

            uint256 mintedTotal = mintedFromWl + mintedFromGiveaway + mintedFromRaffle;
            ...
            revert("Incorrect round provided");
    }
    ...
    function _merkleProofMint(address sender, uint256 amount, bytes32[] calldata proof, uint256 round) internal {
        ...
        require(roundToMinted[sender][round] + amount <= roundToMintLimits[round], "Sender acceeds max mint limit of this round");
        require(roundToMints[round] + amount <= _roundToMaxSupply(round), "Current mint round is sold out");

        roundToMinted[sender][round] += amount;
        roundToMints[round] += amount;
    ...
    function _mintFromMintPass(address sender, uint256 amount) internal {
        require(amount > 0, "Invalid mint amount");
        require(roundToMints[whitelistRound] + amount <= _roundToMaxSupply(whitelistRound), "Current mint round is sold out");

        roundToMints[whitelistRound] += amount;
    ...
    function publicMint(uint256 amount, bool stake) external payable {
        ...
        require(block.timestamp < startTimePublic + mintDuration, "Public mint closed");
        require(roundToMints[publicRound] + amount <= _roundToMaxSupply(publicRound), "Current mint round is sold out");
        require(msg.value >= amount * price, "Not enough eth sent");

        roundToMints[publicRound] += amount;
        flowerFam.mint(msg.sender, amount);  
    ...
    function maxSupplyOfRound(uint256 round) external view returns (uint256) {
        return _roundToMaxSupply(round);
    }
    ...
    function getSupplyLeft() external view returns (uint256) {
        uint256 totalMinted = roundToMints[whitelistRound] + 
                              roundToMints[giveawaysRound] + 
                              roundToMints[raffleRound] + 
                              roundToMints[publicRound];

        return maxSupply - totalMinted;
    }

    function getMintedAtRound(uint256 round) external view returns (uint256) {
        return _roundToMaxSupply(round) - roundToMints[round];
    }
    ...
    function setMaxSupplyOfRound(uint256 round, uint256 newMaxSupply) external onlyOwner {
        roundToMaxSupply[round] = newMaxSupply;
    }
