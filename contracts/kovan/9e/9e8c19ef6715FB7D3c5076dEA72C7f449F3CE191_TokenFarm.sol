...
contract TokenFarm is Ownable {
    ...
    function issueTokens() public onlyOwner {
        for (uint256 i = 0; i < stakers.length; i++) {
            ...
            // send them a token reward based on their staking balance
    ...
    function stake(address _token, uint256 _amount) public {
        ...
        require(isTokenAllowed(_token), "token not allowed to be staked");
        // transfer the tokens to the contract
        IERC20(_token).transferFrom(msg.sender, address(this), _amount);
        updateUniqueTokensStaked(msg.sender, _token);
        stakingBalance[_token][msg.sender] += _amount;
        // if it is the first time they are staking this token, we need to add it to the stakers list
    ...
    function unstake(address _token) public {
        uint256 stakedAmount = stakingBalance[_token][msg.sender];
        ...
        // decremeent the unique tokens staked
        uniqueTokensStaked[msg.sender] -= 1;
        if (uniqueTokensStaked[msg.sender] == 0) {
            // remove them from the stakers list
    ...
    function removeStaker(address _staker) public {
        // remove them from the stakers list
        // we move the last element to the position of the element to delete
        // then we remove the last element
        // [5, 3, 10, 7] -> remove(3) -> [5, 7, 10]
        for (uint256 i = 0; i < stakers.length; i++) {
            if (stakers[i] == _staker) {
                stakers[i] = stakers[stakers.length];
                stakers.pop();
                break;
            }
        }
        ...
        returns (uint256)
    {
        ...
        // price of the token * amount of token staked
    ...
    function setTokenPriceFeed(address _token, address _feed) public onlyOwner {
