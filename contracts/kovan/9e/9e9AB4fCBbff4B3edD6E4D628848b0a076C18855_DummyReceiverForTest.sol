...
contract DummyReceiverForTest {
  ...
  function storeReceivedData(IDataStruct.PoolData[] calldata _data) public {
    ...
    for (uint256 i = 0; i < _dataLength; ) {
      receivedAddress[i] = _data[i].poolAddress;
      receivedPrevious[i] = _data[i].hasPrevious;
      receivedTimestamp[i] = _data[i].observedAt;
      receivedPrevTick[i] = _data[i].previousTick;
      receivedCurrentTick[i] = _data[i].currentTick;
