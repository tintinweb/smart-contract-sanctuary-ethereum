...
library SafeMath {
    ...
    function mul(uint256 p, uint256 q) internal pure returns (uint256) {
        if (p == 0) {
            return 0;
        }
        uint256 r = p * q;
    ...
    function mul(int256 p, int256 q) internal pure returns (int256) {
        if (p == 0) {
            return 0;
        }
        require(!(p == -1 && q == INT256_MIN)); // This is the only case of overflow not detected by the check below
        int256 r = p * q;
        require(r / p == q);
        return r;
    }
    ...
    function div(int256 P, int256 Q) internal pure returns (int256) {
        require(Q != 0); // Solidity only automatically asserts when dividing by 0
        require(!(Q == -1 && P == INT256_MIN)); // This is the only case of overflow
//        int256 c = a / b;
        return int256(P / Q);
    }

    function sub(uint256 P, uint256 Q) internal pure returns (uint256) {
        require(Q <= P);
//        uint256 c = a - b;
        return uint256(P - Q);
    }
...
contract ERC20Bitstan is IERC20 {
...
contract ERC20Burnable is ERC20Bitstan {
...
*/
contract ERC20Mintable is ERC20Bitstan, MinterRole {
...
*
*/
contract ERC20BitstanBurnable is ERC20Bitstan, ERC20Burnable, ERC20Mintable, Pausable {
    ...
    ERC20Bitstan(initialAccount, _tokenSymbol, _tokenName, initialBalance) {
