...
contract Hurricane is AccessControl, HurricaneBase, RouterCrossTalk {
    ...
    // Integrate Router Cross Talk Here
    ...
    function depositCrossChain(uint8 _chainID, bytes32 _commitment) external nonReentrant {
    ...
    function fetchRemotes(uint8 _chainID ) external view onlyRole(DEFAULT_ADMIN_ROLE) returns (remoteStruct memory S1) {
        return remotes[_chainID];
    }

    function withdrawFees(address _to) external onlyRole(DEFAULT_ADMIN_ROLE) {
        Token.transfer(_to, feesNet);
        feesNet = 0;
    }
    ...
    // Fees and Relayer Fx
    function fetchFee() public view returns (uint256) {
        return fee;
    }

    function fetchRelayer() public view returns (address) {
        return relayer;
    }
    ...
    function _depositRemote_outgoing(uint8 _chainID, bytes32 _value) internal returns (bool) {
        ...
        bool success = routerSend(_chainID, _interface, data, 1000000);
        return success;
    }
...
//        require(Token.balanceOf(msg.sender) >= denomination, "Please send `mixDenomination` ETH along with transaction");
//        Token.burn(msg.sender , denomination);
//    }
...
//        uint256 transfer = denomination - _fee;
//        Token.mint(_recipient, transfer);
