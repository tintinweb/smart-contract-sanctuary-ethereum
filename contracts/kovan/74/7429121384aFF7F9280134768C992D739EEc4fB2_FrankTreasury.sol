...
contract FrankTreasury is Ownable {
    ...
    function _addPoolAndFarmLiquidity4(uint256 _amount, address _pool) public {
        //harvest();
        ...
        uint256 minAmountOut = TraderJoeRouter.getAmountsOut(_div, path)[1];
        uint256 amountOut = TraderJoeRouter.swapExactTokensForTokens(_div, minAmountOut, path, address(this), block.timestamp + 2000)[1];
        ...
        uint x = TraderJoeRouter.quote(amountOut, reserveB, reserveA);

        IERC20(tokenA).approve(address(TraderJoeRouter),x);
        IERC20(tokenB).approve(address(TraderJoeRouter), amountOut);

        (, , uint256 liquidity) = TraderJoeRouter.addLiquidity(tokenB, tokenA, amountOut, x, 0, 0, address(this), block.timestamp + 1000);
