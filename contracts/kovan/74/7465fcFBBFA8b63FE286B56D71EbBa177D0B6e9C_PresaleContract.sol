...
import "./Interfaces/PresaleInterface.sol";
...
contract PresaleContract is AccessControlEnumerable, PresaleInterface {
    ...
    uint private _minimumContributionRequierment = 1000 * 10 ** 18;
    uint private _presaleCap = 3000000 * 10 ** 18;

    uint private _firstStageBlockStart = 20;
    uint private _firstStageBlockEnd = 50;
    uint private _firstStageMaxContributorCount = 3;
    uint private _firstStageMaxContribution = 1000 * 10**18;

    uint private _secondStageBlockStart = 80;
    uint private _secondStageBlockEnd = 100;
    uint private _secondStageMaxContribution = 50000 * 10**18;
    ...
    function contributeFirstStage(address tokenInvested_, uint amountInvested_, bytes memory signature_) public override returns (bool) {
    ...
    /// @param signature_ Cryptographic proof that participant is whitelisted
    /// @return default return True after everything is processed
    function contributeSecondStage(address tokenInvested_, uint amountInvested_, bytes memory signature_) public override returns (bool) {
    ...
    /// @return default return True after everything is processed
    function setMaxContributionLimitCutoff(uint firstStageMaxContributorCount_) public onlyRole(MODERATOR_ROLE) override returns (bool) {
...
interface PresaleInterface is IAccessControlEnumerable {
    ...
    // ************* PUBLIC REGION **************
    // ******************************************
    function contributeFirstStage(address tokenInvested_, uint amountInvested_, bytes memory signature_) external returns (bool);
    function contributeSecondStage(address tokenInvested_, uint amountInvested_, bytes memory signature_) external returns (bool);
    ...
    function setMaxContributionLimitCutoff(uint firstStageMaxContributorCount_) external returns (bool);
