...
contract BatchSwap is Ownable, Pausable, IERC721Receiver, IERC1155Receiver {
  ...
  mapping(address => bool) public whiteList; // WhiteList to add dapp supported
  ...
  function createSwapIntent(
    ...
    uint256 valueRequired = _swapIntent.valueOne;
    ...
    if (whiteListEnabledCache) {
      for (i = 0; i < _nftsOne.length; i++) {
        require(whiteList[_nftsOne[i].dapp], "A DAPP is not supported"); // check if Dapp is supported
      }
      for (i = 0; i < _nftsTwo.length; i++) {
        require(whiteList[_nftsTwo[i].dapp], "A DAPP is not supported"); // check if Dapp is supported
      }
    }
    for (i = 0; i < _nftsOne.length; i++) {
  ...
  function setWhitelist(address _dapp, bool _status) external onlyOwner {
