  ...
  OwnableUpgradeable
{
  ...
  function initialize(
    ...
    require(
      _erc20Token != address(0) && _erc721Token != address(0),
      "MErn_DAO: initializing with zero address"
    );
    __Ownable_init();
  ...
  function deposit(uint256[] calldata tokenIds) external {
    ...
    rewardDebt[msg.sender] += tokenIds.length;
    IERC20(ERC20_CONTRACT).mint(msg.sender, tokenIds.length);
  ...
  function withdraw(uint256[] calldata tokenIds) external {
    ...
    rewardDebt[msg.sender] -= tokenIds.length;
    IERC20(ERC20_CONTRACT).burnFrom(msg.sender, tokenIds.length);
   ...
   */
  function hasStaked(address account) public view returns (bool) {
    uint256 stakedNFTLength = _deposits[account].length();
    if (stakedNFTLength != 0) return true;
    else return false;
  }
