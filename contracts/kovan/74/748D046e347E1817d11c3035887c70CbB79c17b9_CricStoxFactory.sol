...
contract PlayerStoxToken is Context, IERC20 {
    ...
    constructor (string memory name_, string memory symbol_, uint8 decimals_, address cricStoxMasterAddress_, address cricStoxDividendsAddress_, uint256 dividendsPool_) {
        ...
        // TODO: Replace with false and add logic for handling Dividends, refer n earn contracts only
...
contract CricStoxFactory is Ownable {
    ...
    function createPlayerStox(string memory name_, string memory symbol_, uint8 decimals_, address cricStoxDividendsAddress_, uint256 dividendsPool_) external onlyOwner {
        PlayerStoxToken playerStox = new PlayerStoxToken(name_, symbol_, decimals_, cricStoxMasterAddress, cricStoxDividendsAddress_, dividendsPool_);
        playerStoxs.add(address(playerStox));
        ICricStoxDividends01(cricStoxDividendsAddress_).addToPool(address(playerStox), dividendsPool_);
