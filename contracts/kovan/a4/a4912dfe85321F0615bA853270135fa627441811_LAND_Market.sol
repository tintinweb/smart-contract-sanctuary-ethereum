...
contract LAND_Market is UserInfo, Initializable, UUPSUpgradeable, ReentrancyGuardUpgradeable, OwnableUpgradeable {
    using CountersUpgradeable for CountersUpgradeable.Counter;
  	mapping (uint => address) public Approvals; 	//승인이 떨어진 함선
	mapping (address => uint) public ownerApproeCount;	//승인이 떨어진 함선의 수량
	mapping (uint => address) public saleToOwner;		//판매하는 물건의 소유자
	mapping (address => uint) ownersaleCount;			//주인이 판매하는 수량

    CountersUpgradeable.Counter private _itemIds; //total number of items ever created
...
*/
    ...
    function tokenBalance(address _tokenContract) public view returns (uint256) {
	...
	function receive_user_reward_token(address _tokenContract, address _address) public {
		IERC20Upgradeable tokenContract = IERC20Upgradeable(_tokenContract);
		tokenContract.transfer(_address, UserReward[_address]);
		UserReward[_address] = 0;
	}
    ...
    function createMarketItem(address _nftContract, uint256 _tokenId, uint256 _price) public payable nonReentrant{
        ...
        //MarketItems.push(MarketItem(itemId, _nftContract, _tokenId, payable(msg.sender), payable(address(0)), _price, true));
        idMarketItem[itemId] = MarketItem(itemId, _nftContract, _tokenId, payable(msg.sender), payable(address(0)), _price, false);
        IERC721Upgradeable(_nftContract).transferFrom(msg.sender, address(this), _tokenId);
        emit MarketItemCreated(itemId, _nftContract, _tokenId, msg.sender, msg.sender, _price, false);
    }


    /// @notice function to create a sale
    function createMarketSale(address _nftContract, uint256 _itemId, uint256 _price) public payable nonReentrant{
        ...
        IERC721Upgradeable(_nftContract).transferFrom(address(this), msg.sender, tokenId);

        idMarketItem[_itemId].seller = payable(address(0)); //mark buyer as new owner
    ...
    function cancleMarketSale(address _nftContract, uint256 _itemId) public nonReentrant{
        uint tokenId = idMarketItem[_itemId].tokenId;
        IERC721Upgradeable(_nftContract).transferFrom(address(this), msg.sender, tokenId);
    ...
    function changePrice(uint256 _itemId, uint256 _price) public payable nonReentrant{
...
//contract UserFactory is OwnableContract {
contract UserInfo{
	...
	function get_User_reward(address _address) public view is_User() returns (uint256){
		return UserReward[_address];
	}

    function set_User_reward(address _address, uint256 _value) public is_User() returns (uint256){
        return UserReward[_address] = UserReward[_address] + _value;
    }

	modifier is_User(){
        require(User[msg.sender] == true, "User: User is not exist");
        _;
	}
