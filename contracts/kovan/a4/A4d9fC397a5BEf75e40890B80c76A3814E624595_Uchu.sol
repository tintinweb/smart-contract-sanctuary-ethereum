...
contract Uchu is Initializable {
    ...
    */
    ...
    mapping (uint32 => uint256) private allowance;
    mapping (address => mapping(uint32 => uint256)) private debts;
    mapping (address => mapping(uint32 => uint256)) private balanceOf;
    mapping (address => mapping(uint32 => Position)) private positions;

    function initialize() public payable initializer {
        owner = payable(msg.sender);
    }

    function createProduct(uint8 dpyYield, uint16 dpyBase, uint16 lockPeriod, uint16 asset, uint16 name) public {
        products[productId] = Product(productId, dpyYield, dpyBase, lockPeriod, asset, name, owner, block.timestamp);
    ...
    **/
    function deposit(address user, uint8 proId) public payable returns (bool) {
        if (productId > 0) {
            Product memory pro = getProduct(proId);
            balanceOf[user][pro.name] += msg.value;
            allowance[pro.name] += msg.value;
            createLender(user, proId);
            createPosition(proId, OPEN, user, msg.value,  block.timestamp, 0);
            emit Log(user, "deposit", pro.name, msg.value);
            return true;
        } else {
            require(productId > 0, "No product!");
            return false;
        }
    }
    ...
    */
    ...
    function getDeposit(uint16 name) public view returns (uint256) {
        return allowance[name];
    }

    function getDeposit(address user, uint16 name) public view returns (uint256) {
    ...
    **/
    function getProductReturn(address user, uint8 proId) public view returns (uint256) {
        uint16 length = getCurLender(user).posIndex;
        Product memory pro = getProduct(proId);
        uint256 result;
        for(uint8 i=0; i<length; i++) {
            if(positions[user][i].proId == proId) {
                ...
                uint256 total = pro.dpyYield + pro.dpyBase;
                if (pow > 1) {
    ...
    **/
    ...
    function withdraw(address user, uint256 amount, uint8 proId) public returns (bool) {
        ...
        if (balanceOf[user][pro.name] >= amount) {
            ...
            require(balanceOf[user][pro.name] >= amount, "BalanceOf is not enough!");
            return false;
        }
    }
    ...
    function borrow(address user, uint256 amount, uint8 proId) public returns (bool) {
        ...
        if (allowance[pro.name] >= amount) {
            ...
            require(allowance[pro.name] >= amount, "Allowance is not enough!");
            return false;
        }
    }
    ...
    function repay(address user, uint8 proId) public payable returns (bool) {
        ...
        if (debts[user][pro.name] >= msg.value) {
            ...
            require(debts[user][pro.name] >= msg.value, "Debts is not so much!");
            return false;
        }
    }
}