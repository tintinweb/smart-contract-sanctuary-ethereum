...
contract DEMOTKN is Initializable,UUPSUpgradeable,PausableUpgradeable, ERC721AUpgradeable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    uint256 public whitelistMintMaxSupply;
    uint256 public maxItemsPerWhiteListedWallet;
    ...
    // Constructor(
    // uint256 mintPrice_, uint256 collectionSize_,
    // uint256 whitelistMintMaxSupply_, uint256 maxItemsPerWhiteListedWallet_,
    // uint256 maxItemsPerTx_, bool whitelistMintPaused_,
    // bool publicMintPaused_) initializer {
    //     intPrice = mintPrice_;
    //     collectionSize = collectionSize_;
    //     whitelistMintMaxSupply = whitelistMintMaxSupply_;
    //     maxItemsPerWhiteListedWallet = maxItemsPerWhiteListedWallet_;
    //     maxItemsPerTx = maxItemsPerTx_;
    //     whitelistMintPaused = whitelistMintPaused_;
    //     publicMintPaused = publicMintPaused_;
    // }
...
/// @custom:oz-upgrades-unsafe-allow constructor
    ...
    function initialize(string memory baseTokenURI_,string memory name_ , string memory symbol_,
    uint256 mintPrice_, uint256 collectionSize_,
    uint256 whitelistMintMaxSupply_, uint256 maxItemsPerWhiteListedWallet_,
    uint256 maxItemsPerTx_, bool whitelistMintPaused_,
    bool publicMintPaused_) external initializer {
    ...
    function ownerMint(address to, uint256 amount) external onlySender onlyOwner {
        require((totalSupply() + amount) <= collectionSize, 'Sold out!');
        _safeMint(to, amount);
    }

    function ownerMintInBatch(BatchMint[] memory batchMint) external onlySender onlyOwner {
        for (uint256 i = 0; i < batchMint.length; i++) {
            require((totalSupply() + batchMint[i].amount) <= collectionSize, 'Sold out!');
            _safeMint(batchMint[i].to, batchMint[i].amount);
        }
    }

    // function whitelistMint(bytes32[] memory proof, uint256 amount) external onlySender nonReentrant {

    function whitelistMint(bytes32[] memory proof) external payable onlySender nonReentrant {
    ...
    function setmaxItemsPerWhiteListedWallet(uint256 _maxItemsPerWhiteListedWallet) external onlyOwner {
