...
contract WithdrawalRegister is IWithdrawalRegister {
    ...
    modifier onlyPrecog() {
        require(msg.sender == precog, "CALLER_IS_NOT_PRECOG");
        _;
    }

    address public override precog;
    address public override precogCore;
    mapping(address => mapping(address => Register)) public register;
    ...
    function isInDeadline(address token, address account) public view returns (bool) {
        Register memory lastRegister = register[token][account];
        return lastRegister.deadline >= block.timestamp;
    }

    function registerWithdrawal(address token, address account, uint256 amount, uint256 deadline) onlyPrecog external {
        Register memory lastRegister = register[token][account];
        require(block.timestamp + deadline >= lastRegister.deadline, "WithdrawalRegister: DEADLINE_IS_INVALID");
        require(isInDeadline(token, account) || lastRegister.amount == 0, "WithdrawalRegister: MUST_CLAIM_TOKEN_BEFORE_REGISTER_AGAIN");
        register[token][account] = Register(lastRegister.amount + amount, block.timestamp + deadline);   
    }

    function claimWithdrawal(address token, address account, address to, uint256 amount, uint256 fee) onlyPrecog external { 
        Register memory lastRegister = register[token][account];
