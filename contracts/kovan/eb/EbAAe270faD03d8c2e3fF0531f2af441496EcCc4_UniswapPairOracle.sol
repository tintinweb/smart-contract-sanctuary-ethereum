...
contract UniswapPairOracle is Ownable {
    ...
    function twap(address token, uint256 pricePrecision) external view returns (uint256 amountOut) {
        require(token == token0 || token == token1, "UniswapPairOracle::twap: Invalid pair");
        ...
        if (token == token0) {
            ...
            amountOut = amountOut.mul(10**_token1MissingDecimals);
        } else {
            amountOut = price1Average.mul(pricePrecision).decode144();
            amountOut = amountOut.mul(10**_token0MissingDecimals);
        }
    }
    ...
    function spot(address token, uint256 pricePrecision) external view returns (uint256) {
        ...
        uint256 _price = 0;
        if (token == token0) {
            _price = _reserve1.mul(10**_token1MissingDecimals).mul(pricePrecision).div(_reserve0).div(
                10**_token0MissingDecimals
            );
        } else {
            _price = _reserve0.mul(10**_token0MissingDecimals).mul(pricePrecision).div(_reserve1).div(
