...
contract BorrowerOps is  OrumBase, Ownable, CheckContract, IBorrowerOps {
    ...
    function openVault(uint _maxFeePercentage, uint _USDCAmount, uint _colloMATICamount , address _upperHint, address _lowerHint) external payable override {
        ...
        require(usdcToken.balanceOf(address(stabilityPool)) >= _USDCAmount + (USDC_GAS_COMPENSATION), "BorrowerOps: SP balance is lower than requested borrow amount");
        ...
        if (!isRecoveryMode) {
            ...
            // send the oMatic fee to the staking contract
        ...
        oMaticToken.sendToPool(msg.sender, address(activePool), _colloMATICamount);
        activePool.receiveoMATIC(_colloMATICamount);
        ...
        if (!isRecoveryMode) {
            if (vars.oMATICFee > 0) {
                activePool.sendoMATIC(orumFeeDistributionAddress, vars.oMATICFee);
            }
        }
    }
    ...
    // Send oMATIC as collateral to a vault. Called by borrowers.
    function moveoMATICGainToVault(address _upperHint, address _lowerHint) external payable override {
    ...
    function moveoMATICGainToVaultFromStabilityPool(address _borrower, uint depositorROSEGain, address _upperHint, address _lowerHint) external payable override {
    ...
    */
    ...
    function closeVault() external override {
        ...
        activePoolCached.sendoMATIC(msg.sender, coll);
    }
    ...
    function _triggerBorrowingFee(IVaultManager _vaultManager, uint _USDCAmount, uint _maxFeePercentage, uint _price) internal returns (uint) {
        ...
        uint oMATICFee = (USDCFee * DECIMAL_PRECISION * (DECIMAL_PRECISION / USDC_DECIMAL_PRECISION))/ _price; // oMATIC fee should be in 1e18 precision
        return oMATICFee;
    }
        ...
        internal
    {
        if (_isDebtIncrease ) {
            ...
            _activePool.sendoMATIC(orumFeeDistributionAddress, _borrowFee);
        ...
        if (_isCollIncrease) {
            ...
            _activePool.sendoMATIC(_borrower, _collChange); 
        }
    }
    ...
    function _withdrawUSDC(IActivePool _activePool, IUSDCToken _usdcToken, address _account, uint _USDCAmount, uint _netDebtIncrease) internal {
        ...
        stabilityPool.sendUSDCtoBorrower(_account, _USDCAmount);
    }
