...
contract MyGovernor is Governor, GovernorCountingSimple, GovernorVotes, GovernorTimelockControl {
    ...
    function queueAnkit(address[] memory targets, uint256[] memory values, bytes[] memory calldatas, string memory description)
        public 
        returns(uint256)    
        {
        bytes32 hashvalue = keccak256(bytes(description));
        return super.queue(targets,values,calldatas,hashvalue);
    }  

      function executeAnkit(uint256 proposalId, address[] memory targets, uint256[] memory values, bytes[] memory calldatas, string memory description)
        public 
           
        {
        bytes32 hashvalue = keccak256(bytes(description));
         super._execute(proposalId,targets,values,calldatas,hashvalue);
    } 
