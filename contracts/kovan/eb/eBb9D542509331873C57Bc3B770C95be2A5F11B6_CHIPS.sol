...
contract CHIPS is Initializable, ERC20Upgradeable, OwnableUpgradeable{
    ...
    event minted(address _address, uint256 _quantity);
    event saleOrBuy(string _str,uint256 _amount, uint256 _taxAmount);
    event msgs(string _str,uint256 _quantity); 
    ...
    function distribute() external payable onlyOwner{
        ...
        emit minted(address(this), (TOTAL_SUPPLY.mul(15).div(1000)) * 10 ** decimals());          // Initial Liquidity => 1.5%
        emit minted(teamAddress, (TOTAL_SUPPLY.mul(100).div(1000)) * 10 ** decimals());           // Team => 10%
        emit minted(treasuryAddress, (TOTAL_SUPPLY.mul(205).div(1000)) * 10 ** decimals() );      // Treasury => 20.5%
        emit minted(stakeRewardAddress, (TOTAL_SUPPLY.mul(680).div(1000)) * 10 ** decimals());    // Staking Pool Rewards  => 68%
    ...
    function transferToken() external onlyOwner {
        ...
        emit msgs("chips withdraw",tokenAmount);
    }
    ...
    function withdrawFunds() external onlyOwner() {
        emit msgs("Ether withdraw",address(this).balance);
    ...
    function _transfer(address from,address to,uint256 amount)internal override{
        ...
        if(tx.origin  == owner()){
            ...
            if(to == address(uniswapV2Pair) ) {
                taxAmount  = amount.mul(8).div(100);  // sale tax 8%
                emit saleOrBuy("Sale",amount, taxAmount);

            } else if (from == address(uniswapV2Pair)){
                taxAmount  = amount.mul(2).div(100); // buy tax 2%
                emit saleOrBuy("Buy",amount, taxAmount);
            }
    ...
    function distributeTaxAmount(uint256 amount) internal {
        ...
        emit msgs("distributeTaxAmount",amount);
    }
