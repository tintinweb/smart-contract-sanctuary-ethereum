...
contract CopperProxy is Ownable {
    ...
    function _distributePlatformAccessFee(address pool, address fundToken, uint256 totalFeeAmount) private {
        for (uint256 i = 0; i < feeRecipients.length; i++) {
            ...
            TransferHelper.safeTransfer(fundToken, feeRecipients[i], proportionalAmount);
            emit TransferredFee(pool, fundToken, feeRecipients[i], proportionalAmount);
        }
    }
    ...
    function updateRecipients(
        ...
        // emit event
        emit RecipientsUpdated(feeRecipients, shareBPS);
    }
