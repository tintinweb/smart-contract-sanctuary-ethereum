...
contract FarmREMIT {
    string constant farmContractName = "FarmREMIT";
    address immutable owner;
    RemitToken immutable remitToken;

    constructor(address _tokenAddress) {
        owner = msg.sender;
        remitToken = RemitToken(_tokenAddress);
    }
    ...
    function transferToOwner(uint256 _amount) public onlyOwner {
        ...
        require(
            remitToken.balanceOf(address(this)) >= _amount,
            "Insufficient contract balance"
        );
        require(remitToken.transfer(msg.sender, _amount));
    }
}
