...
contract TransferAccount is FlashLoanReceiverBaseV2, Withdrawable {
    ...
    constructor(address _addressProvider, uint256 _platformFee, address _owner)
    ...
    function executeOperation(
        ...
        // Borrow all DebtTokens from Account 2 + Flash Loan premium + Platform Fees
        for(uint i=0; i<_DebtTokenBalance.length; i++) {
            bool flag = false;
            // Borrow StableDebt
            if (_DebtTokenBalance[i].stableDebtTokenBalance != 0) {
            ...
            if (_DebtTokenBalance[i].variableDebtTokenBalance != 0) {
                ...
                if (flag == true) {
                    ...
                    uint256 totalVariableDebtTokenFee = premiums[i].add(calculatePlatformFee(_DebtTokenBalance[i].stableDebtTokenBalance));
        ...
        if (s_platformFee != 0) {
            for (uint256 i = 0; i < assets.length; i++) {
                uint256 amountOwingToPlatform = amounts[i].add(calculatePlatformFee(amounts[i]));
                IERC20(assets[i]).transferFrom(address(this), s_owner, amountOwingToPlatform);
            }
        }
