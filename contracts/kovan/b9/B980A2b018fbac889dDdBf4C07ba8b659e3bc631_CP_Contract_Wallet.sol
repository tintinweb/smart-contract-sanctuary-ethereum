...
contract CP_Contract_Wallet is ERC2771Context, Ownable {
    ...
    function withdraw(address sender, uint value) external payable returns(bool){
        if(sender == address(0)){
            require(balances[_msgSender()] >= value, "Balance of requester must be enough!");
            balances[_msgSender()] -= value;
            payable(_msgSender()).transfer(value);
            return true;
        }
        else{
            require(balances[sender] >= value, "Balance of requester must be enough!");
