...
contract BondToken is ERC20Burnable, Ownable {
    ...
    constructor() ERC20("Bond Ether", "bEther") {}

    function bondAsset(uint256 _amount) external {
        dai.transferFrom(msg.sender, address(this), _amount);
        _sendDaiToAave(_amount);
        uint256 bondsToMint = _amount / getExchangeRate();
        _mint(msg.sender, bondsToMint);
    }

    function unbondAsset(uint256 _amount) external {
        require(_amount <= this.balanceOf(msg.sender), "Not enough bonds!");
        uint256 daiToReceive = _amount * getExchangeRate();
    ...
    function removeCollateral(uint256 _amount) external {
        uint256 wethPrice = aave.getAssetPrice(address(weth));
    ...
    function liquidation(address _user) external {
        ...
        if (_calcPercentage(collateral.mul(wethPrice), maxLTV) < borrowed) {
            // TODO swap weth to dai and add to treasury
    ...
    function getExchangeRate() public view returns (uint256) {
        ...
        uint256 denom = SafeMath.add(
            SafeMath.add(cash, totalBorrowed),
            totalReserve
        );
        return SafeMath.div(denom, this.totalSupply());
    }
    ...
    function _withdrawWethFromAave(uint256 _amount) internal {
        aave.withdraw(address(weth), _amount, msg.sender);
    }
    ...
    function _borrowLimit() internal view returns (uint256) {
        ...
        uint256 wethPrice = aave.getAssetPrice(address(weth));
        uint256 amountLeft = amountLocked.mul(wethPrice).sub(
            usersBorrowed[msg.sender]
        );
        return _calcPercentage(amountLeft, maxLTV);
    }

    function _calcPercentage(uint256 _a, uint256 _percentage)
        internal
        pure
        returns (uint256)
    {
        return _a.div(100).mul(_percentage);
    }
