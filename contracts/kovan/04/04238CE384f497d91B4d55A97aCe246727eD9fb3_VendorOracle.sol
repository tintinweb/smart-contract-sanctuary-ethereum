...
contract VendorOracle is IVendorOracle, Ownable {
    ...
    mapping(address => address) public pegs;
    ...
    constructor(
        ...
        pegs[_wNativeAddress] = _NativeAddress;
    }
    ...
    function getPriceUSD(address _base) public view returns (int256) {
        address token = pegs[_base] == address(0) ? _base : pegs[_base]; //Convert wrapped native token to native
    ...
    /// @dev After setPeg(A, B) When the price for token A is requested price for token B is returned
    /// @param _base Address of the token you would like to get the price for
    /// @param _peg Address of the token you would receive the price of
    function setPeg(address _base, address _peg) external onlyOwner {
        pegs[_base] = _peg;
    }
        ...
        returns (int256)
    {
        if (_base == gOHM) {
            ...
            return (getPriceUSD(OHM) * SafeCast.toInt256(index)) / 1e9;
        } //gOHM
        return NOT_FOUND;
    }
