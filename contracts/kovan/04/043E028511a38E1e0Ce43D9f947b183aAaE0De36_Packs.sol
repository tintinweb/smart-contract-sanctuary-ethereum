    ...
    ERC721URIStorage
{
    ...
    // string public contractURI = "https://example.com/opensea/contract_metadata.json";
    ...
    // open pack
    event PackOpened(
        address indexed ownerOfPack,
        uint256 indexed tokenId
    );

    // moments sent
    event MomentsReceived(
        address indexed ownerOfPack,
        uint256[] momentsSerial
    );
...
//admin
    function userSignedApprovalForTransfer( // requires signature setup
        address from,
        address to,
        uint256 tokenId,
        bytes memory signature
        ...
        returns (bool)
    {
        ...
        require(
            ...
            "Transfer signer is not authorized signer"
        );
        ...
        onlyRole(MINTER_ROLE)
    {
        ...
        //equire(tokenId <= (_maxSupply - 1), "packs are sold out");
        //mint occurs#
    ...
    function userSignedApprovalForPackOpen(
        address ownerOfPack,
        uint256 tokenId,
        ...
        returns (bool)
    {
        ...
        require(
            messageSigner == ownerOfPack,
            "Packs: INVALID_CALLER"
        );

        require(
            messageSigner == this.ownerOf(tokenId),
            "Packs: NOT_YOUR_TOKEN"
        );

        return true;
    }

    // token id number uint number of tokens and then the [] address to external mint
    // signiture get which calls burn pack
    // safeTransferFrom(address from, address to, uint256 tokenId)
    // admin wallet needs to call the function
    function delegatedExchangeToken(
        address ownerOfPack,
        uint256 tokenId,
        uint256[] memory momentsSerial,
        address[] memory externalContracts,
        ...
        onlyRole(MINTER_ROLE)
    {
        ...
        for (uint i = 0; i < externalContracts.length; i++) {
            IERC721(externalContracts[i]).transferFrom(
                sweetAdminWallet,
                ownerOfPack,
                momentsSerial[i]
            );
