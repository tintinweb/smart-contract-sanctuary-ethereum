...
contract Pool is Ownable, ReentrancyGuard {
    ...
    constructor(
        ...
        require(_controller != address(0), "Pool::initialize: invalidAddress");
        xToken = IXToken(_xToken);
        yToken = IYToken(_yToken);
        yTokenReserve = IYTokenReserve(_yTokenReserve);
    }

    function updateData(address _controller) public {
    ...
    function swap(uint256 _minXTokenOut) external payable nonReentrant {
        ...
        require(
            _xTokenOut + xToken.totalSupply() <= IController(controller).maxXTokenSupply(),
            "Pool::mint: > Max mintable total supply"
        );
...
interface IController {
    function maxXTokenSupply() external pure returns (uint256);
}
...
library WethUtils {
    ...
    // test WETH
    IWETH public constant weth = IWETH(0xd0A1E359811322d97991E03f863a0C30C2cF029C); // WETH
    // IWETH public constant weth = IWETH(0x21be370D5312f44cB42ce377BC9b8a0cEF1A4C83); // WFTM
    // IWETH public constant weth = IWETH(0xB31f66AA3C1e785363F0875A1B74E27b85FD66c7); // WAVAX
