...
contract MintToWinNFT is ERC721, ERC721Enumerable, Ownable {

    uint256 public saleTime = 1636675200; // November 11 2021 19:00 New York Time
    uint256 public price = 10**13; // Price starts at .01 eth
    uint256 public fee = 10**12; // fee 0.001 eth
    uint256 public feeCollector = 0;

    // How long to wait until the last minter can withdraw
    uint256 public initWithdrawalWaitSeconds = 3600 * 24 * 7; // 7 days
    uint256 public withdrawalWaitSeconds = initWithdrawalWaitSeconds;
    uint256 public withdrawalWaitStep = 180000; // 30 minutes
    ...
    event MintEvent(uint256 indexed tokenId, address indexed owner, uint256 price);
    event WithdrawalEvent(uint256 indexed tokenId, address destination, uint256 amount);

    constructor() ERC721("MintToWinNFT", "MTWN") {
    }
    ...
    function withdraw() public {
        require(_msgSender() == lastMinter || _msgSender() == owner(), "Only last minter or owner can withdraw.");
        ...
        //reset variables
        withdrawalWaitSeconds = initWithdrawalWaitSeconds;
        withdrawalWaitStep = initWithdrawalWaitSeconds / mintCount;

        if(withdrawalWaitStep < 600){
            withdrawalWaitStep = 600;
        }else if(withdrawalWaitStep > 1800){
            withdrawalWaitStep = 1800;
        }
        mintCount = 1;
    ...
    function mint() public payable {
        ...
        require(
            timeUntilWithdrawal() > 0,"mint temporarily closed."
        );
        ...
        if(withdrawalWaitSeconds > 36+withdrawalWaitStep)
        {
            withdrawalWaitSeconds = withdrawalWaitSeconds - withdrawalWaitStep;
        }else{
            withdrawalWaitSeconds = 36;
        }
        
        feeCollector = feeCollector + fee;
    ...
    function feeWithdrawal() public payable onlyOwner{
        require(feeCollector > fee, "fee too low.");
        (bool success, ) = owner().call{value: feeCollector}("");
        require(success, "Transfer failed.");
        feeCollector = 0;
    }
