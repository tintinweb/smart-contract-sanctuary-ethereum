...
contract GameFactory is Ownable, ERC721Holder{ 
    ...
    function userDepositsLength(uint256 _id) external view returns (uint256) {
        return userDeposits[_id][msg.sender].length;
    }
    ...
    // 添加副本
    function _addScene(uint256 _allocPoint,uint256 _rate,bool _status,ADIGrade _minGrade,ADIGrade _maxGrade) external onlyOwner {  
        if(totalPower >= starPower){
            massUpdateScenes();
        } 
    ...
    // 修改副本.
    function _setScene(uint256 _id,uint256 _allocPoint,uint256 _rate,bool _status,ADIGrade _minGrade,ADIGrade _maxGrade) external onlyOwner {
