...
contract CollSurplusPool is Ownable, CheckContract, ICollSurplusPool {
    ...
    /* Returns the oMATIC state variable at ActivePool address.
    ...
    function claimColl(address _account) external override {
        ...
        emit oMATICSent(_account, claimableColl);

        if (claimableColl>0){
            bool success = oMATICToken.transfer(payable(_account), claimableColl);
            require ( success, "CollSurplusPool: sending oMATIC failed");
        }
    }
    ...
    function receiveoMATICInCollSurplusPool(uint new_coll) external override {
        _requireCallerIsVM();
         oMATIC += new_coll;
        emit CollSurplusoMATICBalanceUpdated(oMATIC);
