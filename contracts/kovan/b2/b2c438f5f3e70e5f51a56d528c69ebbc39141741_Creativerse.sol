...
contract Plot is ERC721URIStorage, ERC721Enumerable, ERC721Royalty {
    ...
    mapping(uint256 => LockInfo) public tokenLockInfo; // Locking info associated with plot. After locking ends, plot owner should claim tokens before transferring plot, or they may lose their claim.

    mapping(uint256 => bool) locked;
    uint256 rate = 64602;
    uint256 thirtyDays = 2592000; // 30 days in blocks
    uint96 royaltyPercent = 10;

    constructor(address create, address govContract) ERC721("Creativerse Plot", "PLOT") {
        token = Creativerse(create);
        gov = govContract;
        _setDefaultRoyalty(gov, royaltyPercent);
    }
    ...
    function claimable(uint256 tokenId) public view returns (uint256) {
        ...
        return (tokensPerBlock*deltaCurrent) - tokenLockInfo[tokenId].claimed;
    }
    ...
    function setRoyalty(uint96 percent) external {
        require(msg.sender == gov);
        _setDefaultRoyalty(gov, percent);
    }
...
contract Creativerse is ERC20, ERC20Permit, ERC20Votes {
    ...
    // One time function that sets plots as address for plots nft contract
    function initialize(address plotsContract, address govContract) public {
