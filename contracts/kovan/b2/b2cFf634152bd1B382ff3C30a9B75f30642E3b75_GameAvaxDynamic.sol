...
contract GameAvaxDynamic is ChainlinkClient {
    using Chainlink for Chainlink.Request;

    uint256 public playerScore;
    mapping(address => uint) public playerScoreReqId;
    mapping(uint => uint) public playerScoreDict;
     ...
     */
    function requestPlayerScoreData(string memory _address) public returns (bytes32 requestId) 
    {
        require(!players[msg.sender].isOracle);
        string memory _path = concatenate(_address,".score");

        Chainlink.Request memory request = buildChainlinkRequest(jobId, address(this), this.fulfill.selector);
        
        // Set the URL to perform the GET request on
        request.add("get", "https://avaxgames-d1861-default-rtdb.europe-west1.firebasedatabase.app/2048.json");
        
        // Set the path to find the desired data in the API response, where the response format is:
        request.add("path", _path);

        //return playerScoreReqId[msg.sender] = uint(sendChainlinkRequestTo(oracle, request, fee));
    ...
    {
        playerScore = _score;
        playerScoreDict[uint(_requestId)] = _score;
    }

    function approvePlayerScore(string memory _path) public {
        playerScoreReqId[msg.sender] = uint(requestPlayerScoreData(_path));
    }

    function setPlayerScore() public returns(uint){
        score = playerScoreDict[playerScoreReqId[msg.sender]];
        players[msg.sender].score = playerScoreDict[playerScoreReqId[msg.sender]];
        delete playerScoreDict[playerScoreReqId[msg.sender]];
        return score;
    }
    ...
    function enter() public {
        if(players[msg.sender].playerAddress == msg.sender){
            ...
            players[msg.sender].isOracle = false;
        } else {
    ...
    function reward(address _address) public {
        rewardPool = address(this).balance;
        if(rewardPool > 0.01 ether && score>=scoreTarget && statusPauseReward==false && players[_address].isPlaying){
            payable(_address).transfer(0.01 ether);
            score=0;
        }
        score=0;
