...
error NotContract(address _account);
error NotEOAAddress(address _contract);
error ContractsNotAssigned();
error ContractsCanNotBeChanged();
error ZeroAddress();
 ...
 */
contract RemitToken is ERC20Capped, Ownable, ReentrancyGuard {
    ...
    uint256 public constant CIRCULATION_SUPPLY_AMOUNT = (YEARLY_SUPPLY * 37) / 100;
    uint256 public constant STAKING_AND_FARMING_AMOUNT = (YEARLY_SUPPLY * 35) / 100;
    uint256 public constant STAKING_AND_FARMING_INDIVIDUAL_AMOUNT = STAKING_AND_FARMING_AMOUNT / 2;
    uint256 public constant TEAM_AND_ADVISORS_AMOUNT = (YEARLY_SUPPLY * 10) / 100;
    uint256 public constant DEVELOPMENT_FUND_AMOUNT = (YEARLY_SUPPLY * 12) / 100;
    uint256 public constant MARKETING_FUND_AMOUNT = (YEARLY_SUPPLY * 5) / 100;
    uint256 public constant RESERVED_FUND_AMOUNT = (YEARLY_SUPPLY * 1) / 100;
    uint256 public constant TEAM_AND_ADVISORS_NORMAL_MONTHLY_AMOUNT = (TEAM_AND_ADVISORS_AMOUNT * 8) / 100;
    uint256 public constant TEAM_AND_ADVISORS_SPECIAL_MONTHLY_AMOUNT = TEAM_AND_ADVISORS_AMOUNT - (
        TEAM_AND_ADVISORS_NORMAL_MONTHLY_AMOUNT * 12
    );
    uint256 public constant DEVELOPMENT_FUND_MONTHLY_AMOUNT = (DEVELOPMENT_FUND_AMOUNT * 20) /100;
    ...
    modifier onContractsNotAvailable() {
        if (doesContractsAssigned) {revert ContractsCanNotBeChanged();}
        _;
    }
    ...
    modifier onContractsAvailable() {
        if (!doesContractsAssigned) {revert ContractsNotAssigned();}
        _;
    }
    ...
    constructor(
        ...
        if (msg.sender.isContract()) {
            revert NotEOAAddress(msg.sender);
        }
        if (_teamAndAdvisors.isContract()) {
            revert NotEOAAddress(_teamAndAdvisors);
        }
        if (_developmentFund.isContract()) {
            revert NotEOAAddress(_developmentFund);
        }
        if (_marketingFund.isContract()) {
            revert NotEOAAddress(_marketingFund);
        }
        if (_reserveFund.isContract()) {
            revert NotEOAAddress(_reserveFund);
        }
    ...
    function enterContractAddresses(
        ...
        if(_remitExchangeContractAddress == address(0)) {
            revert ZeroAddress();
        }
        if(_stakingContractAddress == address(0)) {
            revert ZeroAddress();
        }
        if(_farmingContractAddress == address(0)) {
            revert ZeroAddress();
        }
        if(!_remitExchangeContractAddress.isContract()) {
            revert NotContract(_remitExchangeContractAddress);
        }
        if(!_stakingContractAddress.isContract()) {
            revert NotContract(_stakingContractAddress);
        }
        if(!_farmingContractAddress.isContract()) {
            revert NotContract(_farmingContractAddress);
        }
        (
    ...
    function getNextTokenAllocationTime() public view returns(uint256) {
        return getLastTokenIssuanceTime() + 5 minutes;
    }
