...
contract TokenFarm {
    using SafeMath for uint256;
    string  public name = "VRT Staking";
    VRT  public vrt;
    address public owner;
    uint256 public dayToSecond = 86400; 

    uint8[]                     public stakeRate = [1,2,9,20,50];
    uint256[]                   public stakeDay  = [15,30,90,180,365];
    mapping(address=>uint256)   public stakingBalance;
    mapping(address=>uint8)     public stakingPlan;
    mapping(address=>bool)      public isStakedAddress;
    mapping(address => uint256) public stakedTime;
    address[]                   public staker;
    mapping (address => uint8)  public stakedPlan;

    constructor (VRT _vrt) public{
    ...
    function stakeToken(uint256 _amount, uint8 _plan) public {
        require(_amount>0,"amount need to be more than 0");         
        vrt.transferFrom(msg.sender, address(this), _amount); 
        isStakedAddress [msg.sender]  = true;
        stakingBalance[msg.sender] = _amount;
        stakedTime[msg.sender]  = block.timestamp;
        stakedPlan[msg.sender]  = _plan;

    }
       
    function unstakeToken() public {
        require(isStakedAddress[msg.sender] == true, "You have nothing to unstake.");
        require(stakedTime[msg.sender] + dayToSecond.mul(stakeRate[stakedPlan[msg.sender]]) < block.timestamp, "you have to unstake token after 4 hours from stake time");
        uint256 rewardAmount;
        uint256 balance = stakingBalance[msg.sender];

        rewardAmount = balance.div(100);
        rewardAmount = rewardAmount.mul(stakeRate[stakedPlan[msg.sender]]);

        stakingBalance[msg.sender] = 0;
        vrt.transfer(msg.sender, balance + rewardAmount);
        isStakedAddress[msg.sender] = false;
    }

    function stakedInfo  (address _address)  public view returns (bool, uint8, uint256, uint256, uint256) {
        if (isStakedAddress[_address]){
             return (isStakedAddress[_address], stakedPlan[_address], stakingBalance[_address], stakedTime[_address], stakedTime[_address] +  dayToSecond.mul(stakeRate[stakedPlan[msg.sender]]) ); 
        }
        else {
            return (isStakedAddress[_address], 6, 0,0,0);
        }
    }
    ...
    function setStakeRate (uint8 _index, uint8 _rate)  public  {
        require(msg.sender == owner,"you are not a owner");
        stakeRate[_index] = _rate;
    }

    function setStakeTime (uint8 _index, uint256 _time) public {
        require(msg.sender == owner,"you are not a owner");
        stakeDay[_index] = _time;
    }
}