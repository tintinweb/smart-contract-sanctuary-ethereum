...
contract EmblemUpgrade is AdminTransfer,ReentrancyGuard {
    ...
    event CompleteUpgrade(uint indexed tokenId,address to,uint level,uint category);
    ...
    function completeUpgrade(address to,uint tokenId0,uint tokenId1) external nonReentrant {
        ...
        emit CompleteUpgrade(tokenId,to,2,category);
    }

    function _jumpUpgrade(uint requestId,address to,uint level,uint randomX128,uint kindRandomX128) internal {
    ...
    function fulfillRandomWords(
        ...
        if(to!=address(0)) {
            if(requestLevel[requestId]==0){
                _jumpUpgrade(requestId,to,level,(randomWords[0]&((uint(1)<<128)-1)),(randomWords[1]&((uint(1)<<128)-1)));
            } else {
