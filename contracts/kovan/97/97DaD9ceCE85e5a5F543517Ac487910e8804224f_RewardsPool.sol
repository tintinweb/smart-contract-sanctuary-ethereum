...
contract RewardsPool is IRewardsPool, Ownable{
    ...
    function _transferRewards(uint _rewards) internal {
        if(stability_pool.getUSDCDeposits() > 0) {
            ...
            swapMaticToUSDC(amount);
            uint USDCRewards = usdc_token.balanceOf(address(this));
            _transferUSDCRewards(USDCRewards);
            totalUSDCRewards += USDCRewards;

            // oMATIC rewards to borrower's + treasury
            (bool success, ) = payable(address(collateral_pool)).call{ value: address(this).balance }("");
            require(success, "RewardsPool: MATIC transfer to CP failed");
            uint omatic_rewards = omatic_token.balanceOf(address(this));
            uint omatic_rewards_scaled = (omatic_rewards * 100) / (rewardsRatios.Borrowers + rewardsRatios.Treasury);
            totaloMATICRewards += omatic_rewards;

            // Borrower rewards
            if (sortedVaults.getSize() > 0 && rewardsRatios.Borrowers > 0 ){
                uint b_rewards = (omatic_rewards_scaled * rewardsRatios.Borrowers) / 100;
                omatic_token.transfer(address(borrowers_rewards_pool), b_rewards);
                borrowers_rewards_pool.receiveoMATICBorrowersRewardsPool(b_rewards);
            }
            else {
                uint b_rewards = (omatic_rewards_scaled * rewardsRatios.Borrowers) / 100;
                omatic_token.transfer(address(orum_revenue), b_rewards);
                orum_revenue.checkpoint_token();
            }

            // Treasury rewards
            uint t_rewards = (omatic_rewards_scaled * rewardsRatios.Treasury) / 100;
            omatic_token.transfer(address(orum_revenue), t_rewards);
            orum_revenue.checkpoint_token();
        }
        else {
            // oMATIC rewards to borrower's + treasury
            ...
            // Borrower rewards
            if (sortedVaults.getSize() > 0 && rewardsRatios.Borrowers > 0 ){
                uint b_rewards = (omatic_rewards * rewardsRatios.Borrowers) / 100;
                omatic_token.transfer(address(borrowers_rewards_pool), b_rewards);
                borrowers_rewards_pool.receiveoMATICBorrowersRewardsPool(b_rewards);
            }
            else {
            ...
            // Treasury rewards
            uint t_rewards = (omatic_rewards * (rewardsRatios.Treasury + rewardsRatios.StabilityPool)) / 100;
            omatic_token.transfer(address(orum_revenue), t_rewards);
            orum_revenue.checkpoint_token();
        }
    }
