...
contract DemoNFT is ERC721, Ownable, Verification {
    ...
    constructor(string memory _name, string memory _symbol, address _provider)
        ERC721(_name, _symbol)
        Verification(_provider)
    {
    }
    ...
    function lock(uint256 _id) internal {
        locked[_id] = true;
        lockedForOwner[ownerOf(_id)].push(_id);
    }

    // Unlock all tokens of the caller.
    function unlock() public {
        address owner = msg.sender;
        require(lockedForOwner[owner].length > 0, "No locked tokens");
    ...
    // Callback from IdentityProvider on verification: unlock the tokens
    function onVerified(address _owner) internal override {
...
abstract contract Verification is IVerification {
    IdentityProvider private identityProvider;
    mapping(address => bool) private verified;

    constructor(address _provider) {
        identityProvider = IdentityProvider(_provider);
    }

    function verify(address _addr) internal returns (bool) {
        if (verified[_addr]) {
            return true;
        }

        identityProvider.query(_addr);
        return false;
    }

    function onVerification(address _addr, bool _approved) external {
        require(msg.sender == address(identityProvider), "Caller is not the identity provider");
    ...
    function onVerified(address _addr) internal virtual;
}
...
contract IdentityProvider is Ownable, ChainlinkClient {
    ...
    mapping(bytes32 => Entry) private requests;

    mapping(address => mapping(address => bool)) private pending;

    string private url;
    bytes32 private chainlinkJobId;
    uint256 private chainlinkFee;

    event Register(address indexed wallet);
    event Revoke(address indexed wallet);

    function setup(address _token, address _oracle, string calldata _url, bytes32 _jobId, uint256 _fee) public onlyOwner {
    ...
    function register(address _wallet) public onlyOwner {
        wallets[_wallet] = true;
        emit Register(_wallet);
    }
    ...
    function isKnown(address _wallet) public view returns (bool) {
        return wallets[_wallet];
    }


    // ChainLink AnyApi call
    function query(address _addr) public onlyVerifier {
        require(isKnown(_addr), "unable to verify unknown wallet");
        if (pending[msg.sender][_addr]) return; // Verification already pending

        Entry memory entry = Entry(IVerification(msg.sender), _addr);
        ...
        request.add("get", url);
        request.add("extPath", toHexString(entry.wallet));
        request.add("queryParams", string(abi.encodePacked("contract=", toHexString(address(entry.verifier)))));
        request.add("path", "approved");

        bytes32 requestId = sendChainlinkRequest(request, chainlinkFee);

        requests[requestId] = entry;
        pending[address(entry.verifier)][entry.wallet] = true;
    }

    // ChainLink AnyApi return
    function fulfill(bytes32 _requestId, bool _approved) public recordChainlinkFulfillment(_requestId) {
        require(requests[_requestId].wallet != address(0), "unknown request id");
        Entry memory entry = requests[_requestId];

        entry.verifier.onVerification(entry.wallet, _approved);

        delete pending[address(entry.verifier)][entry.wallet];
        delete requests[_requestId];
    }

    function isPending(address _verifier, address _addr) external view returns (bool) {
        return pending[_verifier][_addr];
    }
