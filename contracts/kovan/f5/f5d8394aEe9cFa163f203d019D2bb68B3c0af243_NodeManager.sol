...
contract NodeManager is Ownable, IERC721, IERC721Metadata {
    ...
    mapping(uint8 => NodeLevel) public _nodeLevels;

    uint8 private _nodeLevelIndexMax = 3;
    ...
    //KOVAN USDC -> CHANGE TO WHATEVER
    address public purchaseCurrency = 0xd0A1E359811322d97991E03f863a0C30C2cF029C;
    address public treasury = 0xc25147fCE9Ca996400A18b8da4d6E98A6Ce4BF7d;

    uint256 public nodePrice = 1 * (10 ** 10);

    constructor() {
        _nodeLevels[0] = NodeLevel({
            index: 0,
            name: "Level 1",
            price: 1 * (10 ** 10),
            weight: 10
    ...
    function createNode(address account, string memory nodeName, uint8 levelIndex) external {
        //require(keccak256(bytes(nodeName)) == keccak256(bytes("V1 NODE")), "MANAGER: V1 NODE is reserved name");

        require(levelIndex <= _nodeLevelIndexMax, "MANAGER: Node level index out of bounds.");
        require(IERC20(purchaseCurrency).allowance(msg.sender, address(this)) > _nodeLevels[levelIndex].price, "MANAGER: Token has not been approved.");
        require(IERC20(purchaseCurrency).transferFrom(msg.sender, treasury, _nodeLevels[levelIndex].price), "MANAGER: Failed transfering token to treasury.");
