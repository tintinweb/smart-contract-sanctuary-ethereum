...
interface IRainiNft1155 is IERC1155 {
    ...
    function cardPathUri(uint256 _cardId) external view returns (string memory);
...
contract RainiEvolvingNFTv2Ftm is AccessControl, IRainiCustomNFT {
    ...
    function onTransfered(
        ...
        if (
            cards[tokenVars[id].cardId].resetsOnTransfer &&
            !tokenVars[id].isGrouped
        ) {
    ...
    function getState(uint256 _tokenId, uint256 _timeStamp)
        public
        view
        returns (uint256)
    {
    ...
    function uri(uint256 id) external view override returns (string memory) {
        ...
        return
            string(
                abi.encodePacked(
                    baseUri,
                    nftContract.cardPathUri(_tokenVars.cardId),
                    "/",
                    (_tokenVars.number == 0)
                    ...
                    "s",
                    Strings.toString(getState(id, 0)),
                    ".json"
                )
            );
    }
