...
contract TPCore is AccessControl {
    ...
    // 48 bit minimum
    uint48 private startingTpyRewardPerInterval = 1027 * 7 * 4 * (10**8); // per day * 4 week
    
    // 32 (max 1 year)
    uint32 public tpyInterval = 1 hours;
    ...
    constructor(address stakingcoinAddress, address tpyTokenAddress, address initialStrategyAddress) {
        ...
        tpyIntervalRewards[0] = startingTpyRewardPerInterval;
    ...
    function estimateTPYReward(address addr) public view returns(uint136) {
        ...
        for(uint256 i = staking.intervalNumber; i <= lastCompleteInterval; i++) {
            estimatedTPYs += staking.amount * getTPYIntervalReward(i) / totalStakedUntilInterval(i);
        }
