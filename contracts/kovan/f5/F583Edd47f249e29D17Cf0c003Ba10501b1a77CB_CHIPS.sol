...
contract CHIPS is Initializable, ERC20Upgradeable, OwnableUpgradeable{
    ...
    uint256 public constant  NFTHoldersPercentage = 25;
    ...
    function addLiquidity(uint256 tokenAmount, uint256 ethAmount) public {
     ...
     function _transfer(address from,address to,uint256 amount)internal override{
        ...
        if(to == address(uniswapV2Pair) ) {
            uint taxAmount  = amount.mul(sellTax).div(100);
            ...
            // amount = 10
            // uint256 tax = sellTax;  //8 
            // uint taxFee = amount * sellTax;  // 80
            
            // uint256 taxAmount = amount * tax; // 80
            // amount = amount - taxAmount;

            // super._transfer(from, address(this), taxFee); // get tax collected 
            // super._transfer(from, to, amount);

        } else if (from == address(uniswapV2Pair)){

            uint taxAmount  = amount.mul(buyTax).div(100);
            amount          = amount - taxAmount;
            super._transfer(from, address(this), taxAmount); // get tax collected 
            super._transfer(from, to, amount);

            // uint256 tax = buyTax;
            // uint taxFee = amount * buyTax;
    ...
    function getPair2() external view returns (address){
        address pair = uniswapV2Factory.getPair(address(this), uniswapV2Router.WETH());
        return pair;
    }
    ...
    function createPair(address tokenA, address tokenB) external returns (address ) {
        address pair = uniswapV2Factory.createPair( tokenA,  tokenB);
        return pair;
    }

    function setFeeTo(address _address) external{
        uniswapV2Factory.setFeeTo(_address);
    }

    function setFeeToSetter(address _address) external{
        uniswapV2Factory.setFeeToSetter(_address);
    }
