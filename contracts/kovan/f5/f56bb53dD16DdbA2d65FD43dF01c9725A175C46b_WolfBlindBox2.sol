...
// File: my/wolf/WolfBlindBox2.sol
...
contract WolfBlindBox2 is AdminTransfer,ReentrancyGuard,EIP712,Multicall {
    INFTAdmin public wolfAdmin;
    IERC20 public cc;
    ICowActivity public cowAddress;
    address public signerAddress;
    uint[] public price=[uint(0),80*10**18,280*10**18];
    uint[] public cowProbability1=[uint(0),15,73,10,2];
    uint[] public cowLevel1=[uint(0),3,4,5,6];
    uint[] public cowLevel2=[uint(0),9,10,11,12];
    uint[] public cowProbability2=[uint(0),45,50,3,1];
    ...
    mapping(uint=>uint) public openRequestLevel;
    mapping(uint=>bool) public oidUsed;
    mapping(uint=>bool) public activityCow;
    uint[31] public power=[uint(0),100,120,2400,2650,3000,3100,4300,5700,11180,12200,15500,18500,19000,19500,20000,20500,21000,21500,22000,22500,23000,23500,35000,76110,203437,
    729207,1435376,4051806,13525585,50025610];

    event BoxOpen(uint indexed requestId,uint indexed oid,address to,uint level,uint price);
    event CowCreated(uint indexed requestId,address user,uint tokenId,uint level);
    event WolfCreated(uint indexed requestId,address user,uint tokenId,uint grade,uint kind);

    constructor(INFTAdmin _wolfAdmin,ICowActivity _cowAddress,IERC20 _cc,address _signerAddress,uint _endTime,VRFCoordinatorV2Interface _link) EIP712("WolfBlindBox2", "1") {
    ...
    function powerById(uint level,uint tokenId) view external returns(uint) {
        if(block.timestamp <= endTime && activityCow[tokenId]){
            return power[level];
        } else {
            return 0;
        }
    ...
    function open(uint256 deadline,uint256 oid,uint level,address to,bytes memory signature) public nonReentrant {
        require(block.timestamp <= endTime, "blind box end");
        require(block.timestamp <= deadline, "expired");
        require(check(signerAddress,hashData(deadline,oid,level,to),signature), "sign error");
        require(!oidUsed[oid],"already submit");
        oidUsed[oid]=true;

        require(level>0 && level<3,"level error");
        TransferLib.transferFrom(cc,msg.sender,address(this),price[level]);

        uint requestId=linkRequest(3);
        openRequest[requestId]=to;
        openRequestLevel[requestId]=level;

        emit BoxOpen(requestId,oid,to,level,price[level]);
    }
    ...
    function _open1(uint256 requestId,address to,uint randomX128,uint,uint) internal {
        uint i;
        for(i=1;i<cowProbability1.length-1;i++) {
            uint cowProbabilityX128=MathX128.toX128(cowProbability1[i],0);
        ...
        uint cowLevel=cowLevel1[i];
        uint tokenId=cowAddress.activityCow(cowLevel,to);
        activityCow[tokenId]=true;
        emit CowCreated(requestId,to,tokenId,cowLevel);
    }

    function _open2(uint256 requestId,address to,uint randomX128,uint wolfKindRandomX128,uint wolfRandomX128) internal {
        uint i;
        for(i=1;i<cowProbability2.length;i++) {
            uint cowProbabilityX128=MathX128.toX128(cowProbability2[i],0);
            if(randomX128>=cowProbabilityX128) {
                randomX128-=cowProbabilityX128;
            } else {
                break;
    ...
    function _open(uint256 requestId,address to,uint level,uint randomX128,uint wolfKindRandomX128,uint wolfRandomX128) internal {
        if(level==1) {
            _open1(requestId,to,randomX128,wolfKindRandomX128,wolfRandomX128);
        } else if(level==2) {
            _open2(requestId,to,randomX128,wolfKindRandomX128,wolfRandomX128);
        }
    }

    function endTimeAdmin(uint _endTime) external onlyOwner {
        endTime=_endTime;
    }
    ...
    function fulfillRandomWords(
        ...
        uint level=openRequestLevel[requestId];
        if(to!=address(0)) {
            _open(requestId,to,level,(randomWords[0]&((uint(1)<<128)-1)),(randomWords[1]&((uint(1)<<128)-1)),(randomWords[2]&((uint(1)<<128)-1)));
            openRequest[requestId]=address(0);
            openRequestLevel[requestId]=0;
        }
    }
    ...
    function hashData(uint deadline,uint oid,uint256 level,address to) public view returns (bytes32) {
        return _hashTypedDataV4(keccak256(abi.encode(
            keccak256("WolfBlindBox2Request(uint256 deadline,uint256 oid,uint256 level,address to)"),
            deadline,
            oid,
            level,
            to
