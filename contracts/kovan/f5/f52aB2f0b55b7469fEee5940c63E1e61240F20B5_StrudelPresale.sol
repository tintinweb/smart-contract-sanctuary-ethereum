...
contract StrudelPresale is ReentrancyGuard, Context, Ownable {
    ...
    event TokenAddress(IERC20 token);
    ...
    modifier icoNotActive() {
        require(endICO < block.number, 'ICO is not active');
        _;
    }
    ...
    constructor (
        ...
        endICO = block.number + 864000; // 30 Days in eth 
        rate = 1e9; // 1 (1000000000)
    ...
    */
    function buyTokens(address _beneficiary) public nonReentrant icoActive payable {
        uint256 ftmPurchaseInGwei = msg.value;
        uint256 tokensPurchase = getTokenAmount(ftmPurchaseInGwei);

        require(_beneficiary != address(0), "Pre-Sale: Beneficiary is the zero address");
        require(tokensPurchase != 0, "Pre-Sale: Value is 0");
        require(tokensPurchase >= minPurchase, 'Pre-Sale: Have to send at least: minPurchase');
        require((distributed[_beneficiary].amount + tokensPurchase) <= maxPurchase, 'Pre-Sale: Max purchase has been reached');

        // Amount of FTM that has been raised
        ftmRaised = ftmRaised + tokensPurchase;
    ...
    */
    function setDistributedAmount(address _wallet, uint256 _amountInGwei) public onlyOwner {
    ...
    */
    function setRewardTokenCount(uint256 _rewardTokenCount) public onlyOwner {
        rewardTokenCount = _rewardTokenCount;
    ...
    * @param _gweiAmount The amount of tokens in wei
    */
    function getTokenAmount(uint256 _gweiAmount) internal view returns (uint256) {
        return (_gweiAmount * rewardTokenCount) / rate;
    }

    /**
    * @notice Checks the amount the beneficiary can withdraw
    * @param _beneficiary the wallet address for distributed amount
    */
    function checkWithdrawAmount(address _beneficiary) public view returns(uint256 amount) {
    ...
    */
    function withdrawReward() public icoNotActive() {
        require(distributed[_msgSender()].amount != 0, "Pre-Sale: Haven't bought any tokens");

        distributed[_msgSender()].amount = 0;

        token.transfer(_msgSender(), distributed[_msgSender()].amount);

        emit WithdrawRewards(_msgSender(), distributed[_msgSender()].amount);
    }
}