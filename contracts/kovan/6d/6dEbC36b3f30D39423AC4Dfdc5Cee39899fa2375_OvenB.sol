...
contract OvenB is Context, Convert {
	...
	function migrateFunds(address migrateTo) external onlyGov {
		...
		uint256 totalSupplyfUSDToFunds = convertTokenAmount(friesToken, token, totalFunds);
		...
		OvenB(migrateTo).distribute(address(this), migratableFunds);
...
contract HarvestVaultAdapter is IHarvestVaultAdapter {
    ...
    require(_newTreasury != address(0), 'HarvestVaultAdapter: invalid address!');
