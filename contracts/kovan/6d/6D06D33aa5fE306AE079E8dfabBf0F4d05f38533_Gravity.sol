...
contract Gravity is KeeperCompatibleInterface {
    ...
    event Withdrawn(address to, uint256 targetWithdrawn);
        ...
        if((block.timestamp - lastTimeStamp) > upKeepInterval) {
            ...
            if (_toPurchase > 0) {
                ...
                for(uint i = 0; i < purchaseOrders[_nextSlot].length; i++) {
                    ...
                    accounts[purchaseOrders[_nextSlot][i].user].targetBalance += purchaseOrders[_nextSlot][i].purchaseAmount * (_targetPurchased /* * 10000 */) / _toPurchase;
                }
