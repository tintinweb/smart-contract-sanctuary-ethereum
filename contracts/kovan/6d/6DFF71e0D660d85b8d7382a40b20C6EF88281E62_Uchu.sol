...
contract Uchu {
    ...
    function createProduct(uint8 yeildNum, uint8 totalNum, uint8 lockPeriod, string memory asset, string memory name) public {
        ...
        products[productId] = Product(productId, yeildNum, totalNum, lockPeriod, owner, asset, name, 0, 0);
        insertBorrowProduct(owner, productId);
        productId++;
    }
    ...
    */
    function insertLendProduct(address user, uint8 index) private {
        ...
        lenders[lenderId] = Lender(lenderId, pro.id, 0, user, 0, 0);
        lenderId++;
    }
    ...
    */
    function insertBorrowProduct(address user, uint8 index) private {
        ...
        borrowers[borrowerId] = Borrower(borrowerId, pro.id, 0, user, "888", 2022);
        borrowerId++;
    }
    ...
    **/
    function deposit(uint8 proId) public payable returns (bool) {
        if (productId > 0) {
            ...
            insertLendProduct(msg.sender, proId);
