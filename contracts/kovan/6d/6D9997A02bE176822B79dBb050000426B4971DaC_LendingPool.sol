...
contract LendingPool {
    ...
    function depositAave(uint256 amount) external {
        uint256 allowance = IERC20(A_WETH).allowance(msg.sender, address(this));
        require(
            amount <= allowance,
            "amount must be less than or equal to allowance"
        );

        A_WETH.transferFrom(msg.sender, address(this), amount);

        A_WETH.approve(address(AAVE), amount);

        AAVE.deposit(address(A_WETH), amount, msg.sender, 0);
    }
    ...
    function withdrawAave(uint256 amount) external {
        ...
        AAVE.withdraw(address(A_WETH), amount, msg.sender);
    }
    ...
    function borrowCompound(uint256 amount) public returns (bool) {
        (bool success, ) = address(C_DAI).call(
