...
contract SimpleInsureConfig is IInsureConfig {
  ...
  uint16 public globalClassID = 0;
    ...
    function projectClass(uint256 classID_) external view returns (address[] memory) {
        require(classID_ <=  globalClassID, "!classID_");
        return projectClasses[classID_];
    }

    function project(uint256 classID_, uint256 ID_) external view returns (address) {
        require(classID_ <=  globalClassID, "!classID_ project");
        return projectClasses[classID_][ID_];
    }

    function projectConfig(address project_) external view returns (uint256, uint256, uint256, uint256, uint256) {
        require(1 == projects[project_].accept, "!project classID");
        return (projects[project_].classID, projects[project_].projectID, projects[project_].leverageFactor, projects[project_].coverageFactor, projects[project_].policyPrice);
    }
    ...
    function updateCoverageFactor(address project_, uint16 coverageFactor_) external {
        require(msg.sender == governance, "!governance");
        require(coverageFactor_ <=  maxCoverageFactor, "!coverageFactor_");
        projects[project_].coverageFactor = coverageFactor_;
    }

    function updateLeverageFactor(address project_, uint16 leverageFactor_) external {
        require(msg.sender == governance, "!governance");
        require(leverageFactor_ <=  maxLeverageFactor, "!leverage_");
        projects[project_].leverageFactor = leverageFactor_;
    }
    function updatePolicyPrice(address project_, uint96 policyPrice_) external {
        require(msg.sender == governance, "!governance");
        require(1 == projects[project_].accept, "!project price");
        projects[project_].policyPrice = policyPrice_;
    }

    function project(uint16 classID_, address project_,  uint16 leverage_, uint96 price_) external {
        require(msg.sender == governance, "!governance");
        if (0 == classID_ ){
...
interface IInsureConfig {
    ...
    function policyPrice(address project_) external view returns (uint256);
    function projectConfig(address project_) external view returns (uint256 classID, uint256 projectID, uint256 leverageFactor, uint256 coverageFactor, uint256 policyPrice);
}