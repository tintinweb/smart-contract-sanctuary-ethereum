...
contract Lottery {
  ...
  constructor() public {
    token = IERC20(0xa36085F69e2889c224210F603D836748e7dC0088);
    owner = msg.sender;
    active = true;
  }
  ...
  function guess(uint number) public onlyplayer returns(bool) {
  ...
  function stop() public onlyowner returns(bool) {
    ...
    if (guesses[luckyNumber].length == 0) {
      ...
      for (uint i = 0; i < guesses[luckyNumber].length; i++) {
        ...
        token.transferFrom(address(this), winner, ((token.balanceOf(address(this)) * 90) / 100) / guesses[luckyNumber].length);
      }

      token.transferFrom(address(this), owner, (token.balanceOf(address(this)) * 10) / 100);
    }
