...
contract PrecogV5 is IPrecogV5 {
    ...
    function addLiquidityPool(address token) external onlyAdmin {
        ...
        if (!isNotFirstInvestmentCycle[token]) {
            ...
            // cycleChangedInfo.fundingDuration = core.getCycleConfiguration().fundingCycle;
            // cycleChangedInfo.firstDefundingDuration = core.getCycleConfiguration().firstDefundingCycle;
            // cycleChangedInfo.defundingDuration = core.getCycleConfiguration().defundingCycle;
            // cycleChangedInfo.fundingApplyTime = uint48(block.timestamp);
            // cycleChangedInfo.defundingApplyTime = uint48(block.timestamp);
            uint48 fundingDuration = core.getCycleConfiguration().fundingCycle;
            uint48 firstDefundingDuration = core.getCycleConfiguration().firstDefundingCycle;
            uint48 defundingDuration = core.getCycleConfiguration().defundingCycle;
            uint48 _now = uint48(block.timestamp);
            cyclesChangedInfo[token] = CyclesChangedInfo(_now, _now, _now, fundingDuration, firstDefundingDuration, defundingDuration);
    ...
    function _updateAdjustment(address _token) internal {
        ...
        uint48 _nextTradingTime;
        {
            ...
            _cycleChangedInfo.fundingApplyTime = _nextFundingTime;
            _cycleChangedInfo.fundingDuration = core.getCycleConfiguration().fundingCycle;
        ...
        }
        {
            ...
            _cycleChangedInfo.defundingApplyTime = _nextDefundingTime;
            _cycleChangedInfo.defundingDuration = core.getCycleConfiguration().defundingCycle;
            _cycleChangedInfo.firstDefundingDuration = core.getCycleConfiguration().firstDefundingCycle;
            if(_nextTradingTime < _nextDefundingTime) {
    ...
    //     if(withdrawalRegister.isFirstWithdraw(token, msg.sender)) {
    //         if(withdrawalRegister.isInDeadline(token, msg.sender)) {
    //             nextDefundingStartTime = withdrawalRegister.getRegister(token, msg.sender).deadline;
    //         } else {
    ...
    function requestWithdrawal(address token, uint amount) external override {
        ...
        _updateCurrentTradingCycle(token);
        AccountTradingInfo memory _accountTradingInfo = accountTradingInfo[token][msg.sender];
        bool isFirstRequestWithdrawal = !_accountTradingInfo.isNotFirstRequestWithdrawal;
