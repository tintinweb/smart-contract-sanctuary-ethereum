...
contract MockExchange is IExchange {
    ...
    function swap(
        ...
        uint amount2 = (amount1 * liquidity[token2][token1]) / liquidity[token1][token2];
        if (token1 != address(0)) {
            MockToken(token1).transferFrom(msg.sender, address(this), amount1);
        }
        if (token2 == address(0)) {
            payable(msg.sender).transfer(amount2);
        } else {
            MockToken(token2).transfer(msg.sender, amount2);
        }
    }
