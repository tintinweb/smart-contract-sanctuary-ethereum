...
contract DeFundFactory is /*ChainlinkClient, KeeperCompatibleInterface,*/ Ownable {
    ...
    mapping(uint => mapping(address => DeFund)) public s_fundraisersTest;
    ...
    function createFundraiser(
        ...
        s_fundraisersTest[fundraiserId][address(fundraiser)] = fundraiser;
    ...
    function listFundraisersByStatus(FundraiserStatus _status) public view returns (DeFund[] memory filteredFundraisers) {
        uint resultCount;
        ...
        filteredFundraisers = new DeFund[](resultCount);
        // new address[](resultCount);
        uint resultIdx;

        for (uint i = 0; i < s_counter; i++) {
            if (DeFund(s_fundraisers[i]).s_status() == _status) {
                filteredFundraisers[resultIdx] = DeFund(s_fundraisers[i]);
                resultIdx++;
            }
        }
