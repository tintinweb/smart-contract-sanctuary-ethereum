...
contract GsnRelay is Ownable, LondonTxSupport {
    ...
    function sendTransaction(TxRequest calldata _tx, TxFee calldata _fee, bytes calldata _sign) external onlyFeePayer {
        ...
        bytes4 methodSelector = _getMethodSelector(_tx.data);
        ...
        emit TransactionExecuted(_tx.from, _tx.to, methodSelector, address(token), spentTokens);
    ...
    function _getMethodSelector(bytes memory data) internal pure returns (bytes4 result) {
        if (data.length < 4) {
            return bytes4(0);
        }
        // Read the bytes4 from array memory
        assembly {
            result := mload(add(data, add(0, 32)))
