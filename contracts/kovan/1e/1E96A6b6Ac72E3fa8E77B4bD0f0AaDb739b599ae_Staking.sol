...
contract Staking is AccessControl, ReentrancyGuard {
    ...
    function depositOrWLFromOwner(
        ...
        for (uint256 i = 0; i < len; i += 1) {
            ...
            require(
                amount_ > 0 &&
                    lockUp_ <= 2 &&
                    user != address(0) &&
                    enteredAt_ >= previousEnteredAt_,
                "2"
            );
            previousEnteredAt_ = enteredAt_;
        ...
        KON.safeTransferFrom(_msgSender(), address(this), debt);
    }
        ...
        nonReentrant
    {
        ...
        if (whiteList_.amount == amount && whiteList_.lockUpWL == lockUp) {
            require(whiteList_.enteredAt <= timestamp, "4");
        ...
        depositeIndex(index)
    {
        ...
        address user = allDeposits[index].user;
        ...
        depositeIndex(index)
    {
        ...
        if (year <= stake.lockUp) {
            ...
            if (
                ...
                vars.rewardsUSDT = stake.varKoeff.unreceivedRewardUSDT; //текущие переменные по юздт
                rewKON = stake.varKoeff.unreceivedRewardKON; //текущие переменные по кон
            ...
            vars.rewardsKON += currentFixedPart(index); // текущая часть фиксы
            vars.amountPenalty = fixedPart(index) / 2; // пенальти
            ...
            (!stake.gotFixed && (stake.lockUp < 2 || months >= reward3Info[index].variableRewardTaken))
        ) {
     ...
     * @param index index of deposit
     */
    function fixedPart(uint256 index)
        public
        view
        depositeIndex(index)
        returns (uint256 amount)
        ...
        returns (uint256 inKON, uint256 inUSDT)
    {
        ...
        if((stake.lockUp < 2 && stake.pool != stake.lockUp + 1) || (stake.lockUp == 2 && stake.pool != 4)) {
            ...
            inKON = stake.varKoeff.unreceivedRewardKON;
            inUSDT = stake.varKoeff.unreceivedRewardUSDT;
        }
    }
    ...
    function _harvest(uint256 index) private returns(uint256, uint256) {
        ...
        if (stake.lockUp == 2 && year >= 3) {
            ...
            if (reward.variableRewardTaken < 6) {
                if(months > 5) {
                    vars.amountKON = reward.part * (6 - reward.variableRewardTaken);
                } else vars.amountKON = reward.part * (months + 1 - reward.variableRewardTaken);
            }
