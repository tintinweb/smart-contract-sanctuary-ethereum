...
contract BatchSwap is Ownable, Pausable, IERC721Receiver, IERC1155Receiver {
  ...
  function createSwapIntent(
    ...
    uint256 i;
    bool whiteListEnabled_cache = whiteListEnabled;

    for (i = 0; i < _nftsOne.length; i++) {
      if (whiteListEnabled_cache)
        require(
          whiteList[_nftsOne[i].dapp],
    ...
    for (i = 0; i < _nftsOne.length; i++) {
      ...
      if (_nftsOne[i].typeStd == assetType.ERC20) {
        ...
        CustomInterface(dappRelations[_nftsOne[i].dapp])
  ...
  function closeSwapIntent(address _swapCreator, uint256 _swapId)
    ...
    whenNotPaused
  {
    swapIntent memory swapIntent_cache = swapList[_swapCreator][
      swapMatch[_swapId]
    ];
    require(
      swapIntent_cache.status == swapStatus.Pending,
      "Swap Status is not opened"
    );
    require(
      swapIntent_cache.addressTwo == msg.sender,
      "You're not the interested counterpart"
    );
    if (payment.status) {
      if (ERC721Interface(TRADESQUAD).balanceOf(msg.sender) == 0) {
        require(
          msg.value >= payment.value.add(swapIntent_cache.valueTwo),
          "Not enought WEI for handle the transaction"
        );
        // Move the fees to the vault
        if (payment.value.add(swapIntent_cache.swapFee) > 0)
          VAULT.transfer(payment.value.add(swapIntent_cache.swapFee));
      } else {
        require(
          msg.value >= swapIntent_cache.valueTwo,
          "Not enought WEI for handle the transaction"
        );
        if (swapIntent_cache.swapFee > 0)
          VAULT.transfer(swapIntent_cache.swapFee);
      }
    } else
      require(
        msg.value >= swapIntent_cache.valueTwo,
    ...
    swapIntent_cache.addressTwo = payable(msg.sender); // to make address payable
    swapIntent_cache.swapEnd = block.timestamp; // set time of swap closing (TODO maybe move in the end)
    swapIntent_cache.status = swapStatus.Completed; // (TODO maybe move in the end)

    swapList[_swapCreator][swapMatch[_swapId]] = swapIntent_cache;
    ...
    if (swapIntent_cache.valueOne > 0)
      swapIntent_cache.addressTwo.transfer(swapIntent_cache.valueOne);
    ...
    if (swapIntent_cache.valueTwo > 0)
      swapIntent_cache.addressOne.transfer(swapIntent_cache.valueTwo);
  ...
  // Cancel Swap
  function cancelSwapIntent(address _swapCreator, uint256 _swapId) public {
    swapIntent memory swapIntent_cache = swapList[_swapCreator][
      swapMatch[_swapId]
    ];

    require(
      msg.sender == swapIntent_cache.addressOne ||
        msg.sender == swapIntent_cache.addressTwo,
      "You're not the interested counterpart"
    );
    require(
      swapIntent_cache.status == swapStatus.Pending,
    ...
    //Rollback
    if (swapIntent_cache.swapFee > 0)
      payable(msg.sender).transfer(swapIntent_cache.swapFee);

    swapIntent_cache.swapEnd = block.timestamp;
    swapIntent_cache.status = swapStatus.Canceled;
    swapList[_swapCreator][swapMatch[_swapId]] = swapIntent_cache;
    ...
    for (i = 0; i < nftsOne[_swapId].length; i++) {
      if (nftsOne[_swapId][i].typeStd == assetType.ERC20) {
        ...
        ERC1155Interface(nftsOne[_swapId][i].dapp).safeBatchTransferFrom(
          address(this),
          swapIntent_cache.addressOne,
    ...
    if (swapIntent_cache.valueOne > 0)
      swapIntent_cache.addressOne.transfer(swapIntent_cache.valueOne);
  ...
  function setWhitelist(address _dapp, bool _status) public onlyOwner {
    ...
    emit whiteListChange(_dapp, _status);
  }
  ...
  function editCounterPart(uint256 _swapId, address payable _counterPart)
    public
  {
    swapIntent memory swapIntent_cache = swapList[msg.sender][
      swapMatch[_swapId]
    ];
    require(
      swapIntent_cache.id == _swapId &&
        msg.sender == swapIntent_cache.addressOne,
