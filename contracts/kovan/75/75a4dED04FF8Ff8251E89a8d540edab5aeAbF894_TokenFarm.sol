...
contract TokenFarm is Ownable {
    // mapping tokenAddress -> userAddress -> amount staked
    ...
    function issueTokens() public onlyOwner {
        for (uint256 i = 0; i < stakers.length; i++) {
            ...
            // send them a token reward based on
    ...
    function getUserTotalValue(address _user) public view returns (uint256) {
        require(uniqueTokensStaked[_user] > 0, "User with no tokens staked");
        uint256 totalValue = 0;
        for (uint256 i = 0; i < allowedTokens.length; i++) {
            totalValue =
                totalValue +
                getUserSingleValue(_user, allowedTokens[i]);
        }
        return totalValue;
    }
    ...
    function getUserSingleValue(address _user, address _token)
        ...
        returns (uint256)
    {
        // Current price of the token * amount staked
    ...
    function unstakeTokens(address _token) public {
        ...
        require(tokenBalance > 0, "Nothing staked to unstake!");
