...
contract StakingPool
{
	...
	function stake(uint256 value) public
	{
		...
		uint256 len = epochs.length;                                            // Gets lenght of epochs array.
	...
	function harvest(address who) public
	{
		...
		uint256 accumulatedReward = positions[user].accumulatedReward;
		positions[user].accumulatedReward = 0;

		if (value == 0)
		{

			rewardToken.transfer(who, accumulatedReward);
			emit Harvest(msg.sender, accumulatedReward);
			return;
		}

		uint256 endEpochId = epochs.length - 1;
		uint256 duration = block.timestamp < lastRewardTimestamp ? block.timestamp : lastRewardTimestamp; // Gets duration from current date or last reward date.
		
		uint256 reward = computeReward(user, endEpochId) + accumulatedReward; // Computes reward for this position.

		uint256 additionReward;
		if (reward > 0)                                                                       // Main block for non-zero rewards.
		{
			duration -= epochs[endEpochId].start;                                             // Substract date of start staking.
			additionReward = duration * totalRewardPerSecond * value / epochs[endEpochId].tvl;// Calculates added amount of reward.
			reward += additionReward;                                                         // Adds reward.
			reward -= positions[user].debtReward;                                             // Substract previous rewards.
			positions[user].debtReward = additionReward;                                      // Records added reward as previous rewards.
			positions[user].startEpochId = endEpochId;                                        // Records start epoch of staking for current epoch.
		}
		else                                                                                  // If rewards are zero.
		{
			uint256 lastHarvestTimestamp = positions[user].lastHarvestTimestamp;              // Gets timestamp of last harvest.
			if (lastHarvestTimestamp > epochs[endEpochId].start)                              // If timestamp more than last epoch start timestamp.
			{
				duration -= lastHarvestTimestamp;                                             // Substracts timestamp of last harvest.
				additionReward = duration * totalRewardPerSecond * value / epochs[endEpochId].tvl;// Calculates amount of reward.
				reward = additionReward;                                                      // Records amount of reward.
				positions[user].debtReward += additionReward;                                 // Records reward as previous rewards.

			}
			else
			{
				duration -= epochs[endEpochId].start;                                         // Substracts date of last epoch start.
	...
	function viewReward(address user, uint256 timestamp) external view returns (uint256)
	{
		...
		if (value == 0)
			return positions[user].accumulatedReward;
