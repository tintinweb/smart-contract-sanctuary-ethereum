...
contract PrimaryContract {
  ...
  mapping (address => uint256) private _addressToSAMPBalance;
  ...
  constructor(address[] memory _addresses, uint256[] memory _sampleBalances, address _usdcTokenContract, address _sampleTokenContract) {
    require(_addresses.length == _sampleBalances.length, "Mismatching number of constructor arguments");
    ...
    for (uint256 i = 0; i < _addresses.length; i++) {
      // populate _sampleBalances one by one
      _addressToSAMPBalance[_addresses[i]] = _sampleBalances[i];
    }
  }
  ...
  function claimAirdrop() public returns (bool success){
    ...
    require(_addressToSAMPBalance[msg.sender] >= proTier, "Ineligible for airdrop");
  ...
  function purchaseTokens(uint256 amount) public returns (bool success) {
    uint256 tokenBalance = _addressToSAMPBalance[msg.sender];
    
    require(tokenBalance >= baseTier, "Insufficient tokens to participate in purchase"); // 100 whole unit requirement
    require(tokenBalance <= proTier, "Ineligible to participate in purchase. See airdrop instead"); // 2000 whole unit maximum

    uint256 allocation;
    uint256 usdcCost;

    if (tokenBalance >= 3500000) { // < 2000 due to the above require case, >= 350 tokens
      // entitled to 2000 - tokenBalance
      allocation = remainingAllocation(msg.sender);
      require(amount <= allocation, "Attempting to buy more than allocation");

      usdcCost = amount * rate;
      conductTrade(msg.sender, usdcCost, amount);

    } else if (tokenBalance >= baseTier) { // < 300, >= 100 tokens
      // entitled to 700 - tokenBalance
      allocation = remainingAllocation(msg.sender);
      require(amount <= allocation, "Attempting to buy more than allocation");
  ...
  // @param participant (address)
  // @return bool
  function checkAirdropEligibility(address participant) public view returns (bool) {
    return _addressToSAMPBalance[participant] >= proTier;
  }
  
  // @dev Check if participant is eligible for discounted token sale
  // @param participant (address)
  // @return bool
  function checkPurchaseEligibility(address participant) public view returns (bool) {
    return initialAllocation(participant) > 0;
  }
  ...
  function initialAllocation(address participant) public view returns (uint256 allocation) {
    uint256 tokenBalance = _addressToSAMPBalance[participant];

    if (tokenBalance > proTier) { // > 2000
      // no allocation; eligible for airdrop instead
      allocation = 0;

    } else if (tokenBalance >= 3500000) { // <= 2000, >= 350 tokens
      // entitled to 2000 - tokenBalance
      allocation = proTier - tokenBalance;

    } else if (tokenBalance >= baseTier) { // < 300, >= 100 tokens
      // entitled to 700 - tokenBalance
      allocation = midTier - tokenBalance;
  ...
  // @param participant (address) 
  // @return allocation (uint256)
  function remainingAllocation(address participant) public view returns (uint256 allocation) {
    uint256 tokenBalance = _addressToSAMPBalance[participant];
    uint256 purchasedAmount = _purchasedAmount[participant];

    if (tokenBalance > proTier) { // > 2000
      // no allocation to begin with; eligible for airdrop instead
      allocation = 0;

    } else if (tokenBalance >= 3500000) { // <= 2000, >= 350 tokens
      // entitled to 2000 - tokenBalance
      allocation = proTier - tokenBalance - purchasedAmount;

    } else if (tokenBalance >= baseTier) { // < 300, >= 100 tokens
      // entitled to 700 - tokenBalance
      allocation = midTier - tokenBalance - purchasedAmount;

    } else { // < 100 tokens
      // we shouldn't reach this case given we have that require statement above
      allocation = 0; // we can also return -1 if that better indicates that the address wasn't eligible in the first place
    }
  }
