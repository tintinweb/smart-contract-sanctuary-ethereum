...
contract Presale is ReentrancyGuard {
    ...
    function userDeposit( uint256 BUSD_amount) public nonReentrant {
        ...
        uint256 busdAllowance = IERC20(busdAddr).allowance(address(msg.sender),address(this));
        require(busdAllowance >= BUSD_amount, "BUSD not approved");
        uint256 userBUSDBalance = IERC20(busdAddr).balanceOf(address(msg.sender));
    ...
    function userWithdrawTokens() public nonReentrant {
        ...
        require(block.timestamp >= presale_info.presale_end, "Must be after pre-sale end date.");
    ...
    function refundRemainedTokens() public onlyOwner {
        require(PreSaleStatus() == 0, "Not succeeded"); // Success
    
        uint256 refundTokenAmount = IERC20(presale_info.sale_token).balanceOf(address(this)) - status.sold_amount;
    ...
    function withdrawcoins() public nonReentrant onlyOwner {
        require(PreSaleStatus() == 0, "Not succeeded"); // Success
        TransferHelper.safeTransfer(address(busdAddr),address(owner),IERC20(busdAddr).balanceOf(address(this)));
    }
    // set lock period
