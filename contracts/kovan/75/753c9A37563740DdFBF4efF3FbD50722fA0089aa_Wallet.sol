...
contract Wallet {
    ...
    address public wbnbAddress =
        address(0xd0A1E359811322d97991E03f863a0C30C2cF029C); //Kovan testnet
    ...
    */
        ...
        internal
    {
        ...
        for (uint256 x = 0; x < _buyList.length; x++) {
            if (
                ...
                IERC20Detailed assetToken = IERC20Detailed(
                    ISpaceTimeOracle(oracleAddress).getSymbolAddress(
                        _buyList[x]
                    )
                );
                uint256 tokenAmountOut = wbnbAmountIn /
                    (ISpaceTimeOracle(oracleAddress).getSymbolPrice(
                        _buyList[x]
                ...
                sellWbnb(tokenAddress, wbnbAmountIn, tokenAmountOut);
                (
                    uint256 tokenBalance,
                    uint256 tokenBalanceInWbnb
    ...
    function rebalance() public {
        ...
        uint256 bnbBalance = getBnbBalance();
    ...
    function sellWbnb(
        ...
        uint24 fee = 10000;
        address recipient = address(this);
        uint256 amountOutMinimum = ((100 - 5) * _tokenAmountOut) / 100;
        uint160 sqrtPriceLimitX96 = 0;
        require(
