...
contract EnglishAuction {
    ...
    mapping(address => mapping(uint => mapping(uint => uint))) public _bidPerPerson;
        // for Auction
    ...
    function bid(uint _price) external {
        // console.log(grant[msg.sender]);
        require(grant[msg.sender] == true, "you don't have permission to bid");
    ...
    function withdrawReward(uint _auctionNo,uint bidId) external {
        ...
        if(auction[_auctionNo].bidders == bidId){
            // Winner
            HGB.approve( address(this), _bidPerPerson[msg.sender][_auctionNo][bidId] );
            HGB.transferFrom( address(this), owner, _bidPerPerson[msg.sender][_auctionNo][bidId] );
