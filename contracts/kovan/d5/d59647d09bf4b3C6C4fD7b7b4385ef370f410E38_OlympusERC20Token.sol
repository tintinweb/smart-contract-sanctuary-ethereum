...
contract OlympusERC20Token is ERC20Permit, VaultOwned {
    ...
    event MultiSigAddress(address _msigAddress);
    event UniswapPair(address _uniswapV2Pair);
    event BurnTaxFee(uint256 _burnTaxFee);
    event MsigTaxFee(uint256 _msigTaxFee);
    event TaxesDisabled(bool _taxesDisabled);

    address public uniswapV2Pair;
    address public msigAddress;

    uint256 msigTaxFee = 2;
    uint256 burnTaxFee = 8;

    bool taxesDisabled = false;
    ...
    function calculateMsigTaxFee(uint256 _amount) private view returns (uint256) {
          return _amount.mul(msigTaxFee).div(
            10**2
        );
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) override internal virtual {
        ...
        if (
            !taxesDisabled &&
            msigAddress != address(0) && 
            uniswapV2Pair != address(0) && 
            recipient == uniswapV2Pair
        ) {
            // // Send fees to the msig
            // msigTax = calculateMsigTaxFee(amount);
            // _balances[msigAddress] = _balances[msigAddress].add(msigTax);
            // emit Transfer(sender, msigAddress, msigTax);

            // uint256 balanceAfterMsigTax = amount.sub(msigTax);
            // _balances[sender] = _balances[sender].sub(balanceAfterMsigTax, "ERC20: transfer amount exceeds balance");
            
            // // Burn the amount
            // burnTax = calculateBurnTaxFee(amount);
            // burn(burnTax);

            // The recipient amount
            receiveAmount = receiveAmount.sub(msigTax).sub(burnTax);
            // _balances[recipient] = _balances[recipient].add(receiveAmount);
            // emit Transfer(sender, recipient, receiveAmount);

            _balances[sender] = _balances[sender].sub(amount.sub(msigTax).sub(burnTax), "ERC20: transfer amount exceeds balance");
            _balances[recipient] = _balances[recipient].add(receiveAmount);
            emit Transfer(sender, recipient, receiveAmount);
        } else {
            _balances[sender] = _balances[sender].sub(amount, "ERC20: transfer amount exceeds balance");
    ...
    function setUniswapPair(address _uniswapV2Pair) public onlyOwner() {
        ...
        emit UniswapPair(_uniswapV2Pair);
    }

    function setMsigAddress(address _msigAddress) public onlyOwner() {
        msigAddress = _msigAddress;
        emit MultiSigAddress(_msigAddress);
    }

    function setTaxes(uint256 _burnTaxFee, uint256 _msigTaxFee, bool _disableTaxes) public onlyOwner() {
        burnTaxFee = _burnTaxFee;
        msigTaxFee = _msigTaxFee;
        taxesDisabled = _disableTaxes;
        
        emit MsigTaxFee(_msigTaxFee);
        emit BurnTaxFee(_burnTaxFee);
        emit TaxesDisabled(_disableTaxes);
    }
}