...
contract RewardCenter is Ownable {
    ...
    event SetAaveStakedTokenIncentivesController(
        address indexed owner,
        address indexed addr
    );

    event SetAaveLendingPool(address indexed owner, address indexed addr);
    ...
    function setAaveStakedTokenIncentivesController(address addr)
        public
        onlyOwner
    {
        require(addr != address(0), "CHFRY: address should not be 0");
        aaveStakedTokenIncentivesController = addr;
        emit SetAaveStakedTokenIncentivesController(msg.sender, addr);
    }

    function setAaveLendingPool(address addr) public onlyOwner {
        require(addr != address(0), "CHFRY: address should not be 0");
        aaveLendingPool = addr;
        emit SetAaveLendingPool(msg.sender, addr);
    }
        ...
        onlyOwner
    {
        ...
        // emit ReleasePositionReward(msg.sender, epochRound, merkelRoot);
    }
    ...
    function checkAaveStakeReward() public view returns (uint256) {}
}
