...
contract UbxsStake is Ownable {
    ...
    uint256 public minUbxsAmount = 100e6;
    ...
    function setAlias(string memory newAlias) public {
        //check alias is taken
        if (aliases[newAlias] != address(0)) {
            revert AliasTaken();
        }
    ...
    function invest(address refererAddress, uint256 amount) external {
        ...
        if (user.referer == address(0)) {
            if (referer.referer == address(0)) revert InvalidReferer();
                ...
                referer.referals.push(msg.sender);
            }
        }
        ...
        uint256 totalBonus;
        uint256 counter;
        User storage ref = users[refererAddress];
        address tempAddress = refererAddress;
        do {
            uint8 percent = REFERENCE_INCOME_PERCENTAGES[counter++];
            uint256 bonus = (amount * percent) / 1000;

            totalBonus += !isBlacklist[refererAddress]
                ? ref.refBonus += bonus
                : 0;
        ...
        emit Invested(msg.sender, user.referer, amount);
    }

    // solhint-disable not-rely-on-time
    function calculateTotalWithdrawable() internal returns (uint256) {
        User storage user = users[msg.sender];
        Deposit[] memory deposits = user.deposits;
        uint256 len = deposits.length;

        if (len == 0) revert NoInvestment();

        uint256 dividen;
        for (uint256 i = 0; i < len; i++) {
            if (user.deposits[i].amount == 0) continue;

            uint256 withdrawTime = block.timestamp;
            uint256 endTime = deposits[i].start +
                user.deposits[i].plan.investingDays *
                1 days;
            // if staking period is completed investor can withdraw their stake
            if (block.timestamp >= endTime) {
                ...
                if (deposits[i].amount > 0) {
                    dividen += deposits[i].amount;
                    user.deposits[i].amount = 0;
                }
            }

            uint256 passedTime;
            deposits[i].start > user.checkpoint
                ? passedTime = withdrawTime - deposits[i].start
                : passedTime = withdrawTime - user.checkpoint;
        ...
        returns (uint256)
    {
        ...
        for (uint256 i = 0; i < len; i++) {
            ...
            block.timestamp >= endTime ? to = endTime : to = block.timestamp;

            deposits[i].start > user.checkpoint
                ? from = deposits[i].start
                : from = user.checkpoint;

            uint256 dividen = (deposits[i].plan.investingNominator *
                deposits[i].amount *
                (to - from)) / (DENOMINATOR * 1 days);
    ...
    function withdraw() external {
        uint256 totalBonus = calculateTotalWithdrawable();
        ubxs.transfer(msg.sender, totalBonus);

        // solhint-disable-next-line not-rely-on-time
        users[msg.sender].actions.push(Action(2, totalBonus, block.timestamp));

        emit Withdrawn(msg.sender, totalBonus);
    }
    ...
    function reinvest() external {
        ...
        user.actions.push(Action(1, totalBonus, block.timestamp));
        // solhint-enable not-rely-on-time

        user.deposits.push(deposit);
        user.totalInvestment += totalBonus;
        ...
        ubxsAmount -= amountToBeReduced;
        emit Reinvested(msg.sender, totalBonus);
    }
    ...
    function getRefCount(address userAddress) public view returns (uint256) {
