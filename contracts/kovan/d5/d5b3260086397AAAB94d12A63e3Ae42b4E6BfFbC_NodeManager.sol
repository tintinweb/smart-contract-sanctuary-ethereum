...
contract NodeManager is Ownable, IERC721, IERC721Metadata {
    ...
    function createNode(address account, string memory nodeName, uint8 levelIndex) onlyOwner external {
        ...
        totalShares = totalShares + (_nodeLevels[levelIndex].price  * _nodeLevels[levelIndex].weight);
    ...
    function getSharesForNodeLevel(uint8 levelIndex) public view returns (uint256) {
        return (_nodeLevels[levelIndex].price * _nodeLevels[levelIndex].weight);
    }

    function getSharesForNode(uint64 nodeIndex) public view returns (uint256) {
        return getSharesForNodeLevel(_nodes[nodeIndex].levelID);
    }

    function testShareSystem(uint8 levelIndex) public view returns (uint256) {
        uint256 totalPool = 10000000000;
        return ((getSharesForNodeLevel(levelIndex) / totalShares) * totalPool);
    }
