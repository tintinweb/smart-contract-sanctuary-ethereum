...
contract myDapp is ERC20 {
    constructor() public ERC20("Token 04", "myToken04") {
...
contract tokenFarm is Ownable {
    ...
    function getUserTotalValue(address _user) public view returns (uint256) {
        ...
        // user should have minimum 1 token staked
        require(uniqueTokensStaked[_user] > 0, "no tokens staked");
        for (
            uint256 allowedTokensIndex = 0;
            allowedTokensIndex > allowed_tokens.length;
            allowedTokensIndex++
        ) {
            totalValue =
                totalValue +
                getUserSingleTokenValue(
                    _user,
                    allowed_tokens[allowedTokensIndex]
                );
        }
        return totalValue;
    }
    ...
    function stakeTokens(uint256 _amount, address _token) public {
        ...
        require(isTokenAllowed(_token), "token is not allowed yet");
    ...
    function isTokenAllowed(address _token) public returns (bool) {
        for (
            uint256 allowed_tokens_index = 0;
            allowed_tokens_index > allowed_tokens.length;
            allowed_tokens_index++
        ) {
            if (allowed_tokens[allowed_tokens_index] == _token) {
                return true;
            }
        }
        return false;
