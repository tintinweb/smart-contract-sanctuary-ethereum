...
/**
 * @dev A stub for FearWolf contract.
 */
contract FearWolfStub is ERC721Enumerable, ERC721Holder, AccessControl, ReentrancyGuard {
    ...
    */
    ...
    /**
    * @dev Distributor role identifier.
    */
    bytes32 public constant DISTRIBUTOR_ROLE = "Distributor";

    uint256 public totalWolvesCount = 6666;
    uint256 public mintedWolvesCount;
    uint256 public distributedWolvesCount;

    uint256 private nextTokenIdToMint = 1;
    uint256 private nextTokenIdToGive = 1;

    constructor() ERC721("FEAR Wolf Stub", "FWSTUB")  {
    ...
    function initialMint(uint256 wolvesCount) external onlyRole(MINTER_ROLE) {
        require(mintedWolvesCount + wolvesCount <= totalWolvesCount, "Too much wolves");
        require(wolvesCount > 0, "Cannot mint zero wolves");

        mintedWolvesCount += wolvesCount;
        for (uint i = 0; i < wolvesCount; i++) {
            _safeMint(address(this), nextTokenIdToMint);
            nextTokenIdToMint++;
        }
    }
    ...
    function setInitialOwner(address initialOwner, uint256 wolvesCount) external nonReentrant onlyRole(DISTRIBUTOR_ROLE) {
        ...
        require(distributedWolvesCount + wolvesCount <= mintedWolvesCount, "Not enough wolves minted");

        for (uint i = 0; i < wolvesCount; i++) {
            _safeTransfer(address(this), initialOwner, nextTokenIdToGive, "");
            nextTokenIdToGive++;
        }
    }
}