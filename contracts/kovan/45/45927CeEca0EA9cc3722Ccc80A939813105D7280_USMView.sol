...
contract USMView {
     ...
     */
    function ethBuffer(WadMath.Round upOrDown) external view returns (int256 buffer) {
        (uint256 price, ) = usm.latestPrice();
     ...
     */
    function ethToUsm(uint256 ethAmount, WadMath.Round upOrDown) external view returns (uint256 usmOut) {
        (uint256 price, ) = usm.latestPrice();
     ...
     */
    function usmToEth(uint256 usmAmount, WadMath.Round upOrDown) external view returns (uint256 ethOut) {
        (uint256 price, ) = usm.latestPrice();
    ...
    function debtRatio() external view returns (uint256 ratio) {
        (uint256 price, ) = usm.latestOraclePrice();
     ...
     */
    function usmPrice(IUSM.Side side) external view returns (uint256 price) {
        (uint256 ethUsdPrice, ) = usm.latestPrice();
     ...
     */
    function fumPrice(IUSM.Side side) external view returns (uint256 price) {
        (uint256 ethUsdPrice, ) = usm.latestPrice();
        uint256 ethPool = usm.ethPool();
        uint256 usmSupply = usm.totalSupply();
        uint256 oldTimeUnderwater = usm.timeSystemWentUnderwater();
