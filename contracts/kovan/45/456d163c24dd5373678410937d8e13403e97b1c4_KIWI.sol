...
contract KIWI is Ownable {
    IBEP20 public tokenAddress = IBEP20(0x8091bfcE17591373dB1f699c690f6e560EeA873B);
	IBEP20 public baseToken = IBEP20(0xafd38F0099b3bda0733613fFEebDCBD7dC5F7CAd);
	...
	KiwiToken public immutable KTOKEN = KiwiToken(0x8091bfcE17591373dB1f699c690f6e560EeA873B);
	
    event TokensBuy(address indexed buyer, uint256 amount);
	event TokensSell(address indexed seller, uint256 amount);
	
	function calulateBuy(uint256 amount) external view returns (uint256) {
    ...
    function buy(uint256 amount) external returns (bool) {
		...
		baseToken.safeTransferFrom(buyer, owner(), spendToken);
		tokenAddress.safeTransferFrom(owner(), buyer, amount);
		
		users[buyer].investment = users[buyer].investment + spendToken;
		users[buyer].token = users[buyer].token + amount;
	...
	function sell(uint256 amount) external returns (bool) {
		...
		baseToken.safeTransferFrom(owner(), seller, getToken);
		tokenAddress.safeTransferFrom(seller, owner(), amount);
    ...
    function setTokenAddress(IBEP20 newAddress) external onlyOwner{
        tokenAddress = newAddress;
    }
	
	function setBaseToken(IBEP20 newToken) external onlyOwner{
        baseToken = newToken;
    }
