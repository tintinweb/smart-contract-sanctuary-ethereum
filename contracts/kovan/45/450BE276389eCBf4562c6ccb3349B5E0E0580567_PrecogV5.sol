...
contract PrecogV5 is IPrecogV5 {
    ...
    function calculateProfit(address token, address account) external view returns (uint256 _profitOf, uint256 updatedLatestCycle, uint256 lastProfitCycle) {
        require(isExistingToken[token], "PrecogV5: INVALID_TOKEN");
        Investment memory investment = investmentOf[token][account];
        updatedLatestCycle = currentProfitCycleId[token];
        
        Cycle[] memory _profitCycles = profitCycles[token];
        _profitOf = profitOf[token][account];

        lastProfitCycle = investment.lastProfitCycleId;
