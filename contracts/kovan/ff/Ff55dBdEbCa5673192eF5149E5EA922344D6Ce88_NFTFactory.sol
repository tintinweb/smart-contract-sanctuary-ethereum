...
contract NFT is ERC721Upgradeable, ERC721RoyaltyUpgradeable {
    ...
    function initialize(
        ...
        bool isBurnable_,
        uint16 fragmentsCount_, // including the full art NFT
        ...
        _supply = fragmentsCount_;

        _mintFragments(fragmentsCount_, creator_);
    ...
    function _mintFragments(uint256 nftsCount, address to) internal {
        // One nft for the full art (id 0) and at least two fragments
        require(nftsCount > 2 && nftsCount <= 500, "Invalid NFTs count");
        for (uint256 index = 0; index < nftsCount; index++) {
            safeMint(to);
        }
    }
    ...
    function _beforeTokenTransfer(
        ...
        if (isArtTransfer(from, to, tokenId)) {
            require(isEligibleForArtOwner(to), "Invalid Art transfer!");
        }
    }
    ...
    function _afterTokenTransfer(
        ...
        if (isFragmentTransfer(from, to, tokenId)) {
            redistributeArt(to);
        }
    }

    function isArtTransfer(
        address from,
        address to,
        uint256 tokenId
    ) private view returns (bool) {
        return isTransfer(from, to) && tokenId == artId;
    }

    function isFragmentTransfer(
        address from,
        address to,
        uint256 tokenId
    ) private view returns (bool) {
        return tokenId != artId && isTransfer(from, to);
    }

    function isTransfer(address from, address to) private pure returns (bool) {
        return from != address(0) && to != address(0) && from != to;
    }

    function isEligibleForArtOwner(address user) private view returns (bool) {
        if (isAllFragmentsHolder(user)) {
            return true;
        }

        if (user == creator && !isArtOwnerValid()) {
            return true;
        }
    ...
    function isArtOwnerValid() private view returns (bool) {
        return balanceOf(ownerOf(artId)) == _supply;
    }

    function isAllFragmentsHolder(address user) private view returns (bool) {
        //returns true also when fragments are burned
        return balanceOf(user) == _supply - 1 && ownerOf(artId) != user;
    }

    function redistributeArt(address to) private {
        address artOwner = ownerOf(artId);
        if (isEligibleForArtOwner(to)) {
            _transfer(artOwner, to, artId);
            return;
        }

        if (artOwner != creator) {
            _transfer(artOwner, creator, artId);
        }
    }
}
