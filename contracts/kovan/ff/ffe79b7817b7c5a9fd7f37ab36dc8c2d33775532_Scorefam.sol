    ...
    UUPSUpgradeable
{
    ...
    function playSlip(
        ...
        uint256 amount_, uint256 gameType_, 
        uint256 accOddPoint_
    ) external returns (bool){
        require(amount_ > 0);
        require(gameType_ == 1 || gameType_ == 2);
        require(accOddPoint_ > 0);
        require(accOddPoint_.mul(amount_).div(100) <= 50000000000000,"Limit Exceeded");
        ...
        if(gameType_ == 1){
            ...
            for(uint256 i = 0; i < _slipCuts.length; i++){
                ...
                require(_slipCuts[i].outcomeType > 0 && _slipCuts[i].outcomeType < 29);
  ...
  //Convert String to Uint
    ...
    function claimSlipReward(uint256 game_) external returns (bool){
        ...
        _mint(fee, 100000000000); //Charging 1% Fee
        // _mint(_msgSender(), amount_.sub(onePercent));
        emit Claimed(_msgSender(), amount_);
