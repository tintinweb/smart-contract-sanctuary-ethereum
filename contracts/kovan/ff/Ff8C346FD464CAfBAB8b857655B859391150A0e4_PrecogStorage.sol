...
contract PrecogStorage is IPrecogStorage {
    ...
    function updateLiquidityWhitelist(address token, uint newValue) external override {
    ...
    function updateIsInWhitelist(address token, address account, bool newValue) external override onlyOperator {
    ...
    function updateProfitForWhitelistByIndex(address token, uint index, uint newValue) external override onlyOperator {
    ...
    function updateTotalUnitsForWhitelistTradingCycle(address token, uint index, uint newValue) external override onlyOperator {
    ...
    function getUnitInTradingCycle(
        ...
        for (uint investmentId = 0; investmentId < investments.length; investmentId++) {
            IPrecogStorage.Investment memory nextInvestment = IPrecogStorage.Investment({
                amount: 0, 
                unit: 0, 
                timestamp: 0, 
                idChanged: 0, 
