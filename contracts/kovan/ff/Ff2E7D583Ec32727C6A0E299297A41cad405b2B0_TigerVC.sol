...
interface IHeroicAge {
    function customizeMint(address to, string memory uri, uint256 tokenId) external;
}
...
contract TigerVC is AccessControl, ReentrancyGuard {
    ...
    mapping(address => uint256[]) public userStakedAssets;
    ...
    function setHero(address _hero) public onlyRole(MANAGER_ROLE) inPause() {
    ...
    function pause() public onlyRole(MANAGER_ROLE) inProgress() {
        paused = true;
        emit Paused();
    }

    function unpause() public onlyRole(MANAGER_ROLE) inPause() {
    ...
    function createRequest(string memory description, uint256 min, uint256 max, uint256 level) public onlyRole(MANAGER_ROLE) {
    ...
    function _safeMint() internal {
        ...
        uint256 randomNum = rnd1(1232132132131, 10000);
        for (uint i = 0; i < arrayLength; i++) {
            if (randomNum > items[i].min && randomNum <= items[i].max) {
                items[i].level = items[i].level + 1;
                nftToken = items[i].level;
                uri = items[i].ipfsUrl;
            }
        }

        hero.customizeMint(msg.sender, uri, nftToken);
