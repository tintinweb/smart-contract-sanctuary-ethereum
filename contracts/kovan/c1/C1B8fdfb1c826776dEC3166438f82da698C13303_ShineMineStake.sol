...
contract ShineMineStake is Initializable, OwnableUpgradeable, UUPSUpgradeable, ReentrancyGuardUpgradeable {
...
*/
    ...
    function unstake(string calldata pool, uint256 amount) public canUnstake(pool) nonReentrant { 
        updatePoints(msg.sender);
        if (keccak256(abi.encodePacked(pool)) == keccak256(abi.encodePacked(SILVER))) {
            userStake[msg.sender].silverAmount -= amount;
            totalSilverStaked -= amount;
