...
/**
 * @dev initially we will denote the species through
 * merkle proofs, which the user should provide at staking,
 * unstaking, boosting a bee and upgrading their flower.
 */
contract FlowerFam is ERC721A, SimpleAccess {
    ...
    struct FlowerSpec {
        ...
        uint8 species; /// @dev records the type of flower (prodigy, seedling, ancestor, elder, pioneer)
    }
    ...
    bytes32 public speciesMerkleRoot = 0x24b71d3bfb379a5af6a7b72e46cf8628d7a5a70620e813b768215fe808a59d60;
    ...
    function validateSpecies(uint256 tokenId, uint256 species, bytes32[] calldata proof) public view returns (bool) {
        bytes32 leaf = keccak256(abi.encodePacked(tokenId, species));
        return MerkleProof.verify(proof, speciesMerkleRoot, leaf);
    }
    ...
    function setSpecies(uint256 tokenId, uint256 species, bytes32[] calldata proof) external onlyOwner {
        require(validateSpecies(tokenId, species, proof), "Species incorrect");
        FlowerSpec storage flower = flowerSpecs[tokenId];
        flower.species = uint8(species);
    }
