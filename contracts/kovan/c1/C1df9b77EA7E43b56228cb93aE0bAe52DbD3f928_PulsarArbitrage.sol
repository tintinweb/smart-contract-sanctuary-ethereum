...
contract PulsarArbitrage is FlashLoanReceiverBaseV1, IPulsarArbitrage {
    ...
    function executeArbitrage(address fromToken, uint256 amountIn, address[] calldata pools) external override returns (uint256){
        require(pools.length > 0,"No pools provided");
        require(amountIn > 0,"Amount must be greater than 0");

        uint256 amountOut = 0;
        // for (uint256 i = 0; i < pools.length; i++) {
        //     IPulsarArbitrage arbitrage = IPulsarArbitrage(pools[i]);
        //     uint256 amount = arbitrage.executeArbitrage(fromToken, amountIn);
        //     amountOut += amount;
        // }
