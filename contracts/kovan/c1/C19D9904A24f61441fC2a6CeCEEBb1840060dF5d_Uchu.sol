...
contract Uchu is Initializable {
    ...
    */
    ...
    uint8 private loanCount;
    uint8 private positionCount;
    ...
    **/
    ...
    function withdraw(address user, uint8 productId, uint amount) public returns (bool) {
        if(product_list[productId].balance >= amount) {
            ...
            updatePosition(user, productId, amount);
    ...
    function borrow(address user, uint amount, uint8 productId) public returns (bool) {
        if (product_list[productId].debt >= amount) {
            ...
            updateLoan(user, productId, amount);
            payable(user).transfer(amount);
            return true;
        } else {
        ...
        for (uint8 i=0; i<proCount; i++) {
            if (product_list[i].product_id == productId) {
                if (product_list[i].debt == 0) {
                    if (deposit != 0) {
                        product_list[i].lending_share += deposit;
                        product_list[i].balance += deposit;
                    } else if (withdraw != 0) {

                    } else if (borrow != 0) {
                        product_list[i].borrowing_share += borrow;
                        product_list[i].debt = productDebt(productId, borrow, 0);
                        product_list[i].balance -= borrow;
                        product_list[i].balance += product_list[i].debt;
                    } else if (repay != 0) {
    ...
    function productDebt(uint8 productId, uint borrow, uint repay) private returns (uint) {
        Product memory product = getProduct(productId);
        uint base = product.debt + borrow - repay;
        return base;
    }
    ...
    function updateLoan(address user, uint8 productId, uint amount) private {
        Product memory product = getProduct(productId);
        Loan memory loan = loan_map[user][productId];
        if (loan.loan_id == 0) {
            loan_map[user][productId] = Loan(++loanCount, amount);
        } else {
            if (product.debt == 0) {
                loan_map[user][productId].share += amount;
            } else {
