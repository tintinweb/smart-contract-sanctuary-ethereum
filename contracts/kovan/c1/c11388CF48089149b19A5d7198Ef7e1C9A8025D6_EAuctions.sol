...
contract EAuctions is ERC721Holder, ERC1155Holder, Ownable, ReentrancyGuard, TokensHandler {
    ...
    function setParams(uint _fee, address _discounts) external onlyOwner {
        require(_fee > 0,'Fee should be greater than 0');
        require(_discounts != address(0), 'Requires a valid discounts address');
        fee = _fee;
    ...
    function cancel(uint auctionId) external isAccesible(auctionId) nonReentrant {
        require(msg.sender == auctions[auctionId].owner, 'You are not the owner of this auction');
    ...
    function bid(uint auctionId) external payable isAccesible(auctionId) nonReentrant {
        require(auctions[auctionId].winner != msg.sender, 'You are the highest bid already');
    ...
    function finish(uint auctionId) external nonReentrant {
        ...
        require(auctions[auctionId].owner == msg.sender, 'You are not the owner of this auction');

        uint discount = IDiscounts(discounts).calculateDiscount(msg.sender);
        uint bmarketFee = auctions[auctionId].price / fee / discount;

        if ( auctions[auctionId].currency != address(0) ) {
            require(IERC20(auctions[auctionId].currency).transferFrom(address(this), auctions[auctionId].owner, auctions[auctionId].price - bmarketFee), 'Sending auction funds failed');
            require(IERC20(auctions[auctionId].currency).transferFrom(msg.sender,owner(),bmarketFee), 'Transfer of tokens to BMarket failed');
        } else {
            require(payable(auctions[auctionId].owner).send(auctions[auctionId].price - bmarketFee),'Sending auction ETH failed');
            require(payable(owner()).send(bmarketFee),'Transfer of ETH to BMarket failed');
        }
        
        transferTokens(auctions[auctionId].owner,auctions[auctionId].winner,auctions[auctionId].nftAddresses,auctions[auctionId].nftIds,auctions[auctionId].nftTypes);
...
contract TokensHandler {
    ...
    function transferTokens(
        ...
        require(length == nftIds.length && nftTypes.length == length, 'Items information provided is invalid');
        for(uint i = 0; i < length; ++i) {
            if ( nftTypes[i] == 0 ) {
                IERC721(nftAddresses[i]).safeTransferFrom(from,to,nftIds[i]);
            } else {
                IERC1155(nftAddresses[i]).safeTransferFrom(from,to,nftIds[i],1,'0x00');
            }
        }
    }
