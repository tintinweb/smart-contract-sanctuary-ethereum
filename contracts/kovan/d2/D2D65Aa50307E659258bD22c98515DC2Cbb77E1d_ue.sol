...
contract ue{
    ...
    function main(
        ...
        address token2,
        uint112 amm,
        address [] memory path_a,
        address [] memory path_b)public{
        
        uint112 e = t0t1(amm,path_a,one_c);
        bytes4 token1_Id = bytes4(keccak256("transfer(address,uint256)"));
        token1.call(abi.encodeWithSelector(token1_Id,one_lp,amm));
        if (one_t0t1 == 0){
            bytes4 buyId = bytes4(keccak256("swap(uint256,uint256,address,bytes)"));
            one_lp.call(abi.encodeWithSelector(buyId,0,e,address(this),empty));        
        }
        if (one_t0t1 == 1){
            bytes4 buyId = bytes4(keccak256("swap(uint256,uint256,address,bytes)"));
            one_lp.call(abi.encodeWithSelector(buyId,e,0,address(this),empty));           
        }
        uint112 u = t0t1(e,path_b,two_c);
        bytes4 token2_Id = bytes4(keccak256("transfer(address,uint256)"));
        token2.call(abi.encodeWithSelector(token2_Id,two_lp,e));
        if (two_t0t1 == 0){
            bytes4 sellId = bytes4(keccak256("swap(uint256,uint256,address,bytes)"));
            one_lp.call(abi.encodeWithSelector(sellId,0,u,address(this),empty));        
        }
        if (two_t0t1 == 1){
            bytes4 sellId = bytes4(keccak256("swap(uint256,uint256,address,bytes)"));
            one_lp.call(abi.encodeWithSelector(sellId,u,0,address(this),empty));           
        }


    }
    function t0t1(uint256 am,address [] memory path,address ff) private returns(uint112){
