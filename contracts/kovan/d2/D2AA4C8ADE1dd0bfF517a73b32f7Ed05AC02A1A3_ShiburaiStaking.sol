...
contract ShiburaiStaking is Ownable {
    ...
    uint256[3] public durations = [30 minutes, 90 days, 180 days]; // lock durations
    uint256[3] public wpyBonus = [2500, 3750, 5000]; // wpy = apy / 52
    ...
    function stake(uint256 amount, uint256 planIndex) public {
        ...
        userStakeInfo.rewardPerDay = userStakeInfo.rewardPerWeek.div(7);
        ...
        returns (uint256 reward)
    {
        ...
        if (block.timestamp < userStakeInfo.unlockTime) {
            ...
            uint256 rewardDurationInDays = block.timestamp.sub(lockHalfTS).div(
                DAY
            );
