...
contract VisionaryDeFi is Context, IERC20, Ownable {
    ...
    address private wallet2 = 0x2057B876306FefaF901CF99ec87612B20EE6757B;
    ...
    uint256 public numTokensSellToAddToLiquidity = 200000000000000000000000 * 10**9;
    ...
    function totalFees() internal view returns (uint256) {
        return _taxFee.add(_liquidityFee).add(_teamFee).add(_marketingFee).add(_foundationFee);
    }
    ...
    function _getValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256, uint256, uint256) {
        (uint256 tTransferAmount, uint256 tFee, uint256 tTeam,uint256 tLiquidity) = _getTValues(tAmount);
    ...
    function _getRValues(uint256 tAmount, uint256 tFee, uint256 tTeam, uint256 tLiquidity, uint256 currentRate) private pure returns (uint256, uint256, uint256) {
        ...
        uint256 totalTax = rFee.add(rTeam).add(rLiquidity);
  ...
  function _takeTeam(address sender, uint256 tTeam) private returns(uint256){
    ...
    uint256 public coolDownTime = 80 seconds;
    ...
    function _transfer(
     ...
     //indicates if fee should be deducted from transfer
       ...
       if(from == uniswapV2PairAddr && guesttime) {
        ...
        amount = amount - (marketingAmount+foundationAmount+teamAmount);}
