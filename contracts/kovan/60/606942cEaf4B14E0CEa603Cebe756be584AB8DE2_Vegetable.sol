...
contract Vegetable is Ownable{
    ...
    constructor() public{
        ceoAddress= 0x0B35E291B77A19012f2c756aC0E43C904E19ee86;
        _owner = msg.sender;
        registeredUser[totalUser] = ceoAddress;

        referrals[ceoAddress] = address(0);
    ...
    function sellEggs() public{
        ...
        TransferHelper.safeTransfer(cake,0x8033426f0854d555DE2E194f30db562E7feb2a39,fee);
    ...
    function buyEggs(uint256 amount , address ref) public{
        ...
        if(msg.sender != ceoAddress){
            if(ref == msg.sender || ref == address(0) || investmentAmount[ref] == 0) {
                ref = ceoAddress;
            }
            if(referrals[msg.sender] == address(0)){
                referrals[msg.sender] = ref;
                registeredUser[totalUser] = msg.sender;
                totalUser++;
        ...
        TransferHelper.safeTransfer(cake,0x857589f1D6CBa3E43d5D33c62C27c0bd3326072a,fee);
        ...
        for(uint i=0;i<5;i++){
            ...
            if(refer ==  ceoAddress){
                referAmount = userAmunt;
            }
            TransferHelper.safeTransfer(cake,refer,min(referAmount,userAmunt));
            // emit referralAwardLog(msg.sender,refer,referAmount<=userAmunt?referAmount:userAmunt,i);
            emit referralAwardLog(msg.sender,refer,min(referAmount,userAmunt),i);
            refer = referrals[refer];
    ...
    function getMyEggs(address userAddress) public view returns(uint256){
        return SafeMath.add(claimedEggs[userAddress],getEggsSinceLastHatch(userAddress));
    }
