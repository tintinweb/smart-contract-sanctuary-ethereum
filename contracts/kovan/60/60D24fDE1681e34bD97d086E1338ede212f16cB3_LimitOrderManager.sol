...
contract LimitOrderManager is
    ...
    /// @dev whitelisted relayers
    mapping (address => bool) private whitelistedRelayers;

    /// @dev noonce
    mapping (address => uint256) public relayedNonce;
    ...
    function initialize(
        ...
        nativeTransfer = true;
        whitelistedRelayers[address(this)] = true;
    ...
    function multicall(bytes[] calldata data) public payable returns (bytes[] memory results) {
        return _multicall(address(this), data);
    }

    function relayedCall(address target,
        bytes[] calldata data,
        bytes calldata signature,
        address _owner,
        uint256 _nonce) public payable returns (bytes[] memory results) {

        require(whitelistedRelayers[target]);
        ...
        // do the call
        // check the
        results = _multicall(target, data);

        _gasUsed = _gasUsed - gasleft();
        _chargeRelayerFee(_gasUsed, tx.gasprice, _owner, msg.sender);
    }

    function _multicall(address target, bytes[] calldata data) internal returns (bytes[] memory results) {
    ...
    function _checkSignature(bytes32 dataHash, address _owner, bytes memory _signature) internal pure {
        ...
        require(recoveredAddress == _owner);
    }
    ...
    function _chargeRelayerFee(
        ...
        uint256 _relayerFee = utils.quoteKROM(factory, address(WETH), address(KROM), _gasUsed.mul(_txPrice));

        uint256 _totalRelayFee = _relayerFee
        ...
        balance = balance.sub(_totalRelayFee);
        funding[_owner] = balance;

        uint256 fees = _totalRelayFee.sub(_relayerFee);
    ...
    function processLimitOrder(
        ...
        balance = balance.sub(_serviceFee);
        funding[_owner] = balance;
        ...
        uint256 fees = _serviceFee.sub(_monitorFee);
        // send fees to monitor and protocol
        _transferTokenTo(address(KROM), _monitorFee, msg.sender);
        _transferTokenTo(address(KROM), fees, feeAddress);

        emit LimitOrderProcessed(msg.sender, _tokenId, _serviceFee);

        return (true, _monitorFee);
    }
    ...
    function setProtocolFee(uint32 _protocolFee) external {
        ...
        require(_protocolFee <= PROTOCOL_FEE_MULTIPLIER);
        protocolFee = _protocolFee;
        emit ProtocolFeeChanged(msg.sender, _protocolFee);
    }
