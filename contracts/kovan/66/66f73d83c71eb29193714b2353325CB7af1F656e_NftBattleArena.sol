...
contract NftBattleArena is Ownable
{
	...
	function liquidateVotingPosition(uint256 votingPositionId, address voter, address beneficiary, uint256 lastEpoch, uint256 stakingPositionId, uint256 daiNumber) internal
	{
		...
		emit LiquidatedVotingPosition(currentEpoch, voter, beneficiary, votingPositionId, stakingPositionId, daiNumber, zooReturned);
	}
	...
	function claimRewardFromStaking(uint256 stakingPositionId, address staker, address beneficiary) public only(nftStakingPosition)
	{
		...
		emit ClaimedRewardFromStaking(currentEpoch, staker, beneficiary, stakerReward, stakingPositionId);
	}
