...
interface ISocialRecoveryGroup is IGuardianHolder {
    ...
    event VoteSuccess(address oldOwenr, address newOwner, string newPublicKey);
    event VoteFail(address oldOwner, address newOwner, string newPublicKey);
}
...
interface ISocialRecoveryGroupFactory is IGuardianHolder {
    ...
    function createGroup(
        ...
        string calldata ownerPublicKey,
        uint8 quorum,
...
contract SocialRecoveryGroup is ISocialRecoveryGroup {
    ...
    string private name;
    event F(bool sevent);
    event Added(address addresGuardian, string publicKey);
    event CurrentVotesAndQuorum(uint8 votes, uint8 quorumNumber);
    ...
    function addGuardian(Guardian calldata guardian) external override onlyOwner {
        ...
        emit Added(guardian.guardianAddress, guardian.publicKey);
    }
    ...
    function changeOwner(
        ...
        emit CurrentVotesAndQuorum(validVotes, quorum);
        if (validVotes < quorum) {
            emit VoteFail(_owner, msg.sender, newOwnerPK);
        } else {
            ...
            if (
                ...
                ownerPK = newOwnerPK;
                // _owner = msg.sender;
                emit VoteSuccess(oldOwner, msg.sender, newOwnerPK);
            } else {
                _owner = oldOwner;
                emit VoteFail(oldOwner, msg.sender, newOwnerPK);
                emit F(true);
            }
        }
    }
