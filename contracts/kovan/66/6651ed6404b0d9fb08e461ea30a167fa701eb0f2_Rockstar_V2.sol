 ...
 */
contract Rockstar_V2 is ERC721URIStorage, Ownable{
     ...
     */
    constructor() ERC721("Rockstars of EPNS V2", "ROCKSTAR V2") Ownable() {}
    ...
    function safeMint (address recipient, string memory metadata) public onlyOwner returns (bool){
        require( hashCheck[metadata] != 1, "Rockstar V2::safeMint: hash already in use");
    ...
    function burn(uint256 tokenId) public returns (bool)  {
        require(_exists(tokenId), "Rockstar V2::burn: burn of nonexistent token");
        require(_isApprovedOrOwner(_msgSender(), tokenId), "Rockstar V2::burn: caller is not owner nor approved");
        _burn(tokenId);
        return true;
    }
}