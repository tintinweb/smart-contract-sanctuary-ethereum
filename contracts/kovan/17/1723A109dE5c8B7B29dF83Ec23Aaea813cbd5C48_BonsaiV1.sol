...
contract BonsaiV1 is ERC721Upgradeable {
    // Royalty
    ...
    //seed HOLDERS EARLY ACCESS TOKENS
    uint256 public constant seedHoldersMaxMint = 0;
    mapping(address => bool) public seedholders;

    address public SEED_erc20_contract;
    ...
    // enable flag for public and seed
    bool public openForSEED;
    bool public openForPublic;

    // define Bonsai struct
    struct Bonsai {
        uint256 tokenId;
        // string tokenURI;
        address mintedBy;
    ...
    // map id to Bonsai obj
    mapping(uint256 => Bonsai) public allBonsai;
    ...
    //================================= Events ================================= //
    event seedERC20Checked(address indexed _holder, uint256 _bal, bool _result);

    event SaleToggle(uint256 seedNumber, bool isForSale, uint256 price);
    event PurchaseEvent(uint256 seedNumber, address from, address to, uint256 price);
    ...
    function initialize(
        ...
        address _seedContractAddress,
        bool _openForSEED,
        bool _openForPublic
    ) public initializer {
        // __ERC721_init("swng", "swng");
        __ERC721_init("Bonsai", "Bonsai");
        ...
        baseURL = _baseURL;
        SEED_erc20_contract = _seedContractAddress;
        openForSEED = _openForSEED;
    ...
    function mint(uint256 numberOfToken) public payable {
        ...
        for (uint256 i = 1; i <= numberOfToken; i++) {
            ...
            Bonsai memory newBonsai = Bonsai(
                newItemId,
                msg.sender,
                msg.sender,
                0,
            ...
            // add the token id to the allBonsai
            allBonsai[newItemId] = newBonsai;
        }
    }
    ...
    // seed holder related functions
    function seedTotalSupply() public view returns (uint256) {
        return _seedTotalSupply.length;
    }
    ...
    function setPriceForSale(
        ...
        Bonsai memory bonsai = allBonsai[_tokenId];
        bonsai.price = _newPrice;
        bonsai.forSale = isForSale;
        allBonsai[_tokenId] = bonsai;
    ...
    function getAllSaleTokens() public view returns (uint256[] memory) {
        ...
        for (uint256 i = 1; i <= _totalSupply; i++) {
            if (allBonsai[i].forSale == true) {
                _tokenForSales[counter] = allBonsai[i].tokenId;
                counter++;
            }
        }
    ...
    function buyToken(uint256 _tokenId) public payable {
        ...
        // get that token from all Bonsai mapping and create a memory of it defined as (struct => Bonsai)
        Bonsai memory bonsai = allBonsai[_tokenId];
        // price sent in to buy should be equal to or more than the token's price
        require(msg.value >= bonsai.price);
        // token should be for sale
        require(bonsai.forSale);
        ...
        payable(bonsai.currentOwner).transfer(payOwnerAmount);
        bonsai.previousPrice = bonsai.price;
        allBonsai[_tokenId] = bonsai;
        _transfer(tokenOwner, msg.sender, _tokenId);
        emit PurchaseEvent(_tokenId, bonsai.currentOwner, msg.sender, bonsai.price);
    }
    ...
    function _beforeTokenTransfer(
        ...
        Bonsai memory bonsai = allBonsai[tokenId];
        bonsai.currentOwner = to;
        bonsai.numberOfTransfers += 1;
        bonsai.forSale = false;
        allBonsai[tokenId] = bonsai;
        if (from == address(0)) {
    ...
    function setCm(uint256 _tokenId, uint256 _newCm) external onlyAuthorized {
        Bonsai memory bonsai = allBonsai[_tokenId];
        bonsai.cm = _newCm;
