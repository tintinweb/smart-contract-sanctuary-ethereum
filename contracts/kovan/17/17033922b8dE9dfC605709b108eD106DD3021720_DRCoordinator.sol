...
import { IExternalFulfillmentDRC } from "./interfaces/IExternalFulfillmentDRC.sol";
...
contract DRCoordinator is ConfirmedOwner, Pausable, ReentrancyGuard, TypeAndVersionInterface, IDRCoordinator {
    ...
    error DRCoordinator__CallerIsNotRequestOperator();
    error DRCoordinator__ConsumerIsNotAuthorizedToRequestSpec(address operatorAddr, bytes32 specId);
    error DRCoordinator__ConsumersArrayIsEmpty();
    ...
    error DRCoordinator__OperatorAddrIsNotAContract();
    ...
    event DRCoordinator__AuthorizedConsumersAdded(bytes32 indexed key, address[] consumers);
    event DRCoordinator__AuthorizedConsumersRemoved(bytes32 indexed key, address[] consumers);
    ...
    event DRCoordinator__Requested(bytes32 indexed id);
    event DRCoordinator__RequestCancelled(bytes32 indexed id);
    ...
    function addSpecAuthorizedConsumers(bytes32 _key, address[] calldata _consumers) external onlyOwner whenNotPaused {
        _addSpecAuthorizedConsumers(_key, _consumers);
    }

    function addSpecsAuthorizedConsumers(bytes32[] calldata _keys, address[][] calldata _specConsumers)
        external
        onlyOwner
        whenNotPaused
    {
        ...
        _requireEqualLength(keysLength, _specConsumers.length);
        for (uint256 i = 0; i < keysLength; ) {
            _addSpecAuthorizedConsumers(_keys[i], _specConsumers[i]);
            unchecked {
                ++i;
            }
        }
    ...
    function fulfillData(bytes32 _requestId, bytes calldata _data) external whenNotPaused nonReentrant {
        ...
        _requireRequestOperator(s_pendingRequests[_requestId]);
    ...
    function removeSpecAuthorizedConsumers(bytes32 _key, address[] calldata _consumers)
        external
        onlyOwner
        whenNotPaused
    {
        AuthorizedConsumerLibrary.Map storage s_authorizedConsumerMap = s_keyToAuthorizedConsumerMap[_key];
        _removeSpecAuthorizedConsumers(_key, _consumers, s_authorizedConsumerMap, false);
    }

    function removeSpecsAuthorizedConsumers(bytes32[] calldata _keys, address[][] calldata _specConsumers)
        external
        onlyOwner
        whenNotPaused
    {
        ...
        for (uint256 i = 0; i < keysLength; ) {
            ...
            _removeSpecAuthorizedConsumers(key, _specConsumers[i], s_authorizedConsumerMap, false);
            unchecked {
                ++i;
            }
        }
    ...
    function requestData(
        ...
        _requireOperatorAddr(_operatorAddr);
        _requireSpecId(specId);
        _requireCallbackAddr(callbackAddr);
        bytes32 key = _generateSpecKey(_operatorAddr, specId);
        _requireSpecIsInserted(key, s_keyToSpec._isInserted(key));
        // Validate consumer (requester) is authorized to request the Spec
        AuthorizedConsumerLibrary.Map storage s_authorizedConsumerMap = s_keyToAuthorizedConsumerMap[key];
        _requireAuthorizedConsumer(
            s_authorizedConsumerMap._size(),
            s_authorizedConsumerMap._isInserted(msg.sender),
            _operatorAddr,
            specId
        );
        ...
        bytes32 requestId = _sendOperatorRequestTo(_operatorAddr, _req, spec.payment);
        ...
        if (callbackAddr != msg.sender) {
            // TODO: address IExternalFulfillmentDRC -> to be inherited by Consumer & Fulfillment
            IExternalFulfillmentDRC fulfillmentContract = IExternalFulfillmentDRC(callbackAddr);
    ...
    function removeSpec(bytes32 _key) external onlyOwner whenNotPaused {
        ...
        if (s_authorizedConsumerMap._size() > 0) {
            _removeSpecAuthorizedConsumers(_key, s_authorizedConsumerMap.keys, s_authorizedConsumerMap, true);
        }
        _removeSpec(_key);
    }
    ...
    function removeSpecs(bytes32[] calldata _keys) external onlyOwner whenNotPaused {
        ...
        for (uint256 i = 0; i < keysLength; ) {
            ...
            AuthorizedConsumerLibrary.Map storage s_authorizedConsumerMap = s_keyToAuthorizedConsumerMap[key];
            if (s_authorizedConsumerMap._size() > 0) {
                _removeSpecAuthorizedConsumers(key, s_authorizedConsumerMap.keys, s_authorizedConsumerMap, true);
            }
            _removeSpec(key);
            unchecked {
                ++i;
    ...
    // TODO: improve cancelling experience by pulling expiration from fulfillConfig, or s_requestIdToExpiration
    function cancelRequest(bytes32 _requestId, uint256 _expiration) external {
        IDRCoordinator.FulfillConfig memory fulfillConfig = s_requestIdToFulfillConfig[_requestId];
        _requireRequestNotPending(fulfillConfig.msgSender);
        _requireRequestOperator(fulfillConfig.msgSender);
        s_consumerToLinkBalance[msg.sender] += fulfillConfig.payment;
        OperatorInterface operator = OperatorInterface(s_pendingRequests[_requestId]);
        delete s_pendingRequests[_requestId];
        emit DRCoordinator__RequestCancelled(_requestId);
        operator.cancelOracleRequest(_requestId, fulfillConfig.payment, FULFILL_DATA_SELECTOR, _expiration);
    }
    ...
    function getSpec(bytes32 _key) external view returns (Spec memory) {
        _requireSpecIsInserted(_key, s_keyToSpec._isInserted(_key));
    ...
    function _addSpecAuthorizedConsumers(bytes32 _key, address[] calldata _consumers) private {
        _requireSpecIsInserted(_key, s_keyToSpec._isInserted(_key));
        if (_consumers.length == 0) {
            revert DRCoordinator__ConsumersArrayIsEmpty();
        }
        AuthorizedConsumerLibrary.Map storage s_authorizedConsumerMap = s_keyToAuthorizedConsumerMap[_key];
        uint256 consumersLength = _consumers.length;
        for (uint256 i = 0; i < consumersLength; ) {
            s_authorizedConsumerMap._add(_consumers[i]);
            unchecked {
                ++i;
            }
        }
        emit DRCoordinator__AuthorizedConsumersAdded(_key, _consumers);
    }
    ...
    function _removeSpecAuthorizedConsumers(
        ...
        bool _isRemoveSpecCase
    ) private {
        if (!_isRemoveSpecCase) {
            _requireSpecIsInserted(_key, _s_authorizedConsumerMap._size() > 0);
            if (_consumers.length == 0) {
                revert DRCoordinator__ConsumersArrayIsEmpty();
            }
        }
        uint256 consumersLength = _consumers.length;
        for (uint256 i = 0; i < consumersLength; ) {
            _s_authorizedConsumerMap._remove(_consumers[i]);
            unchecked {
                ++i;
            }
        }
        emit DRCoordinator__AuthorizedConsumersRemoved(_key, _consumers);
    }
    ...
    function _sendOperatorRequestTo(
        ...
        emit DRCoordinator__Requested(requestId);
        _requireLinkTransferAndCall(
            LINK.transferAndCall(_operatorAddr, _payment, encodedRequest),
            _operatorAddr,
            _payment,
            encodedRequest
        );
    ...
    function _setSpec(bytes32 _key, Spec calldata _spec) private {
        ...
        _requireOperatorAddr(_spec.operator);
    ...
    function _requireOperatorAddr(address _operatorAddr) private view {
        if (!_operatorAddr.isContract()) {
            revert DRCoordinator__OperatorAddrIsNotAContract();
        }
    }

    function _requireRequestOperator(address _msgSender) private view {
        if (_msgSender != msg.sender) {
            revert DRCoordinator__CallerIsNotRequestOperator();
        }
    }
    ...
    function _requireAuthorizedConsumer(
        uint256 _numberOfAuthorizedConsumers,
        bool _isInserted,
        address _operatorAddr,
        bytes32 _specId
    ) private pure {
        if (_numberOfAuthorizedConsumers > 0 && !_isInserted) {
            revert DRCoordinator__ConsumerIsNotAuthorizedToRequestSpec(_operatorAddr, _specId);
        }
    }
    ...
    function _requireLinkTransferAndCall(
        bool _success,
        address _to,
        uint96 _amount,
        bytes memory _encodedRequest
    ) private pure {
        if (!_success) {
            revert DRCoordinator__LinkTransferAndCallFailed(_to, _amount, _encodedRequest);
        }
    }
...
interface IDRCoordinator {
    struct FulfillConfig {
        ...
        // TODO: consider adding expiration = (block.timestamp + 5 minutes)
    }
...
interface IExternalFulfillmentDRC {
...
// variables for dynamic LINK payments, e.g. fulfillmentFee, feeType.
struct Spec {
 ...
 * Operator directrequest TOML job spec. It also includes specific variables for dynamic LINK payments, e.g.
 * fulfillmentFee, feeType.
 */
library SpecLibrary {
     ...
     * @param _map The reference to this iterable mapping.
     * @param _key The Spec key.
     */
    function _remove(Map storage _map, bytes32 _key) internal {
     ...
     * @return The Spec.
     */
    function _getSpec(Map storage _map, bytes32 _key) internal view returns (Spec memory) {
     ...
     * @return The Spec key.
     */
    function _getKeyAtIndex(Map storage _map, uint256 _index) internal view returns (bytes32) {
     ...
     */
    function _isInserted(Map storage _map, bytes32 _key) internal view returns (bool) {
     ...
     */
    function _size(Map storage _map) internal view returns (uint256) {
...
library InsertedAddressLibrary {
     ...
     */
    function _remove(Map storage _map, address _key) internal {
        if (!_map.inserted[_key]) {
     ...
     */
    function _add(Map storage _map, address _key) internal {
        if (_map.inserted[_key]) {
    ...
    /**
     * @notice Returns the address (key) at the given index.
     * @param _map The reference to this iterable mapping.
     * @param _index The index of the keys array.
     * @return The address (key).
     */
    function _getKeyAtIndex(Map storage _map, uint256 _index) internal view returns (address) {
        return _map.keys[_index];
    }
     ...
     */
    function _isInserted(Map storage _map, address _key) internal view returns (bool) {
        return _map.inserted[_key];
    }
