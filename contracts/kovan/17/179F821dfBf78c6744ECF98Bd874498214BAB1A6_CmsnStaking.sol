...
contract CmsnStaking is Ownable {
        ...
        poolExists(_poolId)
    {
        ...
        require(pool.endsAt < block.timestamp, "pool ended");

        PoolType memory poolType = poolTypes[pool.poolTypeId];
        require(
            pool.startedAt + poolType.timeAccepting > block.timestamp,
    ...
    function withdraw(uint256 _poolId) external poolExists(_poolId) {
        ...
        if (pool.endsAt <= block.timestamp) {
    ...
    {
        return
            pools[_poolId].endsAt >= block.timestamp &&
            pools[_poolId].startedAt <= block.timestamp;
    }
