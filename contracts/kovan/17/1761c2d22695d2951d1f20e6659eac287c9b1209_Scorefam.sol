...
interface IScorefamConsumer {
    ...
    function requestGameScore(bytes32 _jobId, uint256 _fixtureID) external view returns (bytes32);
}
    ...
    UUPSUpgradeable
{
    ...
    //Game Outcome Types for Flexible and Multi-flex Game Types
    // struct OutcomeTypes {
    //     uint256 home;
    //     uint256 draw;
    //     uint256 away;
    ...
    function playSlip(SlipCut[] memory _slipCuts, uint256 amount_, uint256 gameType_, uint256 oddPoint_) public returns (bool) {
        require(amount_ > 0, "Scorefam: No Stake Amount Provided");
  ...
  //Convert String to Uint
    ...
    function sliceBytes(uint256 _fixtureID, bytes memory _bytes) public {
   ...
   function iGameScoreRequestID(uint256 fixtureID) public view returns(bytes32) {
       IScorefamConsumer sc = IScorefamConsumer(consumer);

        return sc.gameScoreRequestID(fixtureID);
   }

    function iRequestIdData(bytes32 requestID) public view returns(bytes memory) {
        IScorefamConsumer sc = IScorefamConsumer(consumer);

        return sc.requestIdData(requestID);
   }
     ...
     */
    function getGameResult(uint256 game_) public {
        //Update Game Status, Home and Away Scores from the Oracle

        uint256 fixtureID = _gaming[_msgSender()][game_].fixtureID;
    ...
    function claimGameReward(uint256 game_) public returns (bool){
        require(
            block.timestamp >
    ...
    function getSlipResult(uint256 game_) internal {
        //Update Game Status, Home and Away Scores from the Oracle
    ...
    function claimSlipReward(uint256 game_) public returns (bool){
