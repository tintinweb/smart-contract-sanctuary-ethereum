...
contract SushiWallet is Ownable {
    ...
    function checkIfPairExists(address tokenA, address tokenB) public view onlyOwner returns (address pair) {
        ...
        console.log(SushiSwapRouter(SushiRouterAddress).factory());
        pair = SushiSwapV2Factory(address(SushiSwapRouter(SushiRouterAddress).factory())).getPair(token0, token1);
        console.log(pair);
        return pair;
    }

    //FUNCIONES DE PRUEBA
    function getSushiSwapRouterFactory() public view onlyOwner returns (address factoryAddress) {
        return SushiSwapRouter(SushiRouterAddress).factory();
    }
