...
contract FrankTreasury is Ownable {
    ...
    function _addPoolAndFarmLiquidity2(uint256 _amount, address _pool) public {
        ...
        uint256 amountBOptimal = UniswapV2Library.quote(amountOutA, reserveA, reserveB);

        if(token1 != address(JOE)) {
            JOE.approve(address(TraderJoeRouter), 999999999999999999999999999999999999999999); 
            path[1] = token1;
            TraderJoeRouter.swapTokensForExactTokens(amountBOptimal, 999999999999999999999999999999999999999999, path, address(this), (block.timestamp + 1000));
        }
        
        IERC20(token0).approve(address(TraderJoeRouter), amountOutA);
        IERC20(token1).approve(address(TraderJoeRouter), amountBOptimal);

        TraderJoeRouter.addLiquidity(token0, token1, amountOutA, amountBOptimal, amountOutA, amountBOptimal, address(this), block.timestamp + 1000);
