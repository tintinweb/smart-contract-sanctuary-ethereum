...
contract DappStore is ERC721, IDappStore {
    ...
    returns (Dapp[] memory)
  {
    ...
    Dapp[] memory _dapps = new Dapp[](size);

    uint256 index = 0;
    for (uint256 i = beginId; i < size; i++) {
      if (_exists(i)) {
        _dapps[index] = dapps[i];
      } else {
        require(_dapps.length > 0, 'DappStore: Array size fell over zero');
   ...
   * @dev See {IDappStore-getDappsInRange}
   */
  function getDappsInRangeLastVersion(uint256 beginId, uint256 endId)
    external
    view
    override
  ...
  {
    uint256 size = endId.sub(beginId).add(1);
    DappDto[] memory _dtos = new DappDto[](size);

    uint256 index = 0;
    for (uint256 i = beginId; i <= endId; i++) {
      if (_exists(i) && dapps[i].versions.length > 0) {
        _dtos[index].dappId = i;
        _dtos[index].owner = ownerOf(i);
        _dtos[index].name = dapps[i].name;
        _dtos[index].lastVersion = dapps[i].versions[
          dapps[i].versions.length.sub(1)
        ];
        index++;
      } else {
        require(_dtos.length > 0, 'DappStore: Array size fell over zero');

        //solhint-disable-next-line no-inline-assembly
        assembly {
          mstore(_dtos, sub(mload(_dtos), 1))
        }
      }
    }
    return (_dtos);
   ...
   * @dev See {IDappStore-getDappsInRange}
   */
  function getOwnersInRange(uint256 beginId, uint256 endId)
    external
    view
    override
  ...
  {
    uint256 size = endId.sub(beginId).add(1);
    address[] memory _owners = new address[](size);

    uint256 index = 0;
    for (uint256 i = beginId; i <= endId; i++) {
      if (_exists(i)) {
        _owners[index] = ownerOf(i);
        index++;
      } else {
        require(_owners.length > 0, 'DappStore: Array size fell over zero');

        //solhint-disable-next-line no-inline-assembly
        assembly {
          mstore(_owners, sub(mload(_owners), 1))
        }
      }
    }
    return (_owners);
  ...
  function pushNewVersion(
    ...
    dapps[_dappId].versions.push(newVersion);
...
interface IDappStore is ITypes {
   ...
   * @param _nodeIds A hash tp identify the location of the root folder
   * @param _paddingSize A hash tp identify the location of the partKey files
   * @param _encryptionKey A hash tp identify the location of the root folder
   * @param _manifestFileKey A hash tp identify the location of the manifest file
   * @param _manifestpartKey A hash tp identify the location of the partKey files
   */
  function pushNewVersion(
    uint256 _dappId,
    uint256[] memory _nodeIds,
   ...
   * @return dapp dapp data (name, versions)
   */
   ...
   * @return dapps array of dapps data (name, versions)
   */
  function getDappsInRange(uint256 beginId, uint256 endId)
    external
    view
    returns (Dapp[] memory dapps);
