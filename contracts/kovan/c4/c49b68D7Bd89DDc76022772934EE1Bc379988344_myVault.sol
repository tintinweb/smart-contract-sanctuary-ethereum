...
contract myVault {
  ...
  function safeTransferWithApprove(uint256 amountIn, address routerAddress)
        internal
    {
        ...
        TransferHelper.safeApprove(daiAddress, routerAddress, amountIn);
    }


    function swapExactInputSingle(uint256 _amountIn)
        external
        returns (uint256 _amountOut)
    {
        daiToken.approve(msg.sender, _amountIn);
        safeTransferWithApprove(_amountIn, address(uniswapRouter));

        uint256 deadline = block.timestamp + 15;
        uint24 fee = 3000;
        address recipient = address(this);
        uint256 amountIn = _amountIn; // includes 18 decimals
        ...
        ISwapRouter.ExactInputSingleParams memory params = ISwapRouter.ExactInputSingleParams(
         ...
         _amountOut =  uniswapRouter.exactInputSingle(params);
