...
contract Billionaire is
  Ownable,
  PaymentSplitter,
  ERC721A("Fresh Balance", "FB")
{
  ...
  //Price of one NFT in presales
  uint256 public pricePresale = 0.0000025 ether;
  //Price of one NFT in sale
  uint256 public priceSale = 0.000003 ether;
  ...
  function changeTeam(address[] memory _teaminput) external onlyOwner {
    _team = _teaminput;
  }

  function changeShare(uint256[] memory _teamSharesinput) external onlyOwner {
    _teamShares = _teamSharesinput;
  }
   ...
   *I need to dot the follwing: 
   Read the base URI
   If baseURI = spaceArray 
   for(var i = 0 ; i< address.lenght();i ++){
       batchtransfer(add[i],i,1);
    ...
    payable
  {
    ...
    require(msg.value >= pricePresale * _ammount, "Not enought funds.");
    bytes32 leaf = keccak256(abi.encodePacked(msg.sender));
    require(_proof.verify(merkleRoot, leaf), "Not on the whitelist");
  ...
  function saleMint(uint256 _ammount) external payable {
    // require(sellingStep == Steps.Sale, "Sorry, the Sale has not started yet.");
    require(msg.value >= priceSale * _ammount, "Not enought funds.");
    require(
