...
contract KIWI is Ownable {
	...
	function calulateSell(uint256 amount) external view returns (uint256) {
    ...
    function buy(uint256 amount) external returns (bool) {
		...
		require(
           IBEP20(baseToken).balanceOf(buyer) >= spendToken,
	...
	function sell(uint256 amount) external returns (bool) {
		...
		require(
           IBEP20(tokenAddress).balanceOf(seller) >= amount,
           "Seller does not have sufficient token balance"
        );
