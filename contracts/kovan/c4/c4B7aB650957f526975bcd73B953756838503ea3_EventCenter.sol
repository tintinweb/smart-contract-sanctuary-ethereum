...
contract EventCenter is Ownable {
    ...
    event StartEpoch(address indexed owner, uint256 epochRound,uint256 rewardAmount);
    ...
    function setRewardCenter(address _rewardCenter) public onlyOwner {
        require(_rewardCenter != address(0), "CHFRY: EventCenter address should not be 0");
    ...
    function startEpoch(uint256 _rewardAmount) external notInEpoch {
        require(msg.sender == rewardCenter, "CHFRY: only Reward Center could start new Epoch");
        epochRound = epochRound + 1;
        epochStart = block.timestamp;
        epochEnd = epochStart + epochInterval;
        rewardAmount[epochRound] = _rewardAmount;
        emit StartEpoch(msg.sender, epochRound, _rewardAmount);
    }
