...
contract NFTStakingPool is AccessControl, ERC1155Holder {
    ...
    // account => level =>
    mapping(address => mapping(uint256 => AccountRewardVars))
    ...
    function setRainiNftCount(uint256 _rainiNftCount) external onlyEditor {
    ...
    function setReward(uint256 _rewardRate, uint256 _level) external onlyOwner {
        rewardRate[_level] = _rewardRate;
    }

    function setStaminaDuration(uint256 _staminaDuration) external onlyOwner {
    ...
    function withdrawNft(
        ...
        IRainiNft1155(_contractAddress).safeTransferFrom(
            address(this),
            _owner,
            _tokenId,
            1,
    ...
    function _initNft(
    ...
    ) internal returns (uint32 maxStamina) {
        ...
        rainiNfts[rainiNftCount] = RainiNft({
            ...
            lastStamina: _stamina == 0
                ? uint32(_cl.maxStamina * STAMINA_DECIMALS)
                : _stamina,
    ...
    function initNft(
        ...
        bool _isStaked,
        uint24 _staminaEventId
    ) public onlyEditor returns (uint32 maxStamina) {
        return
            _initNft(
                _nftContractAddress,
                _tokenId,
        ...
        onlyMinter
    {
        for (uint256 i = 0; i < _addresses.length; i++) {
            accountVars[_addresses[i]].pointsBalance = uint128(
                accountVars[_addresses[i]].pointsBalance + _points[i]
            );
        }
    }

    function burn(address _owner, uint256 _amount) external onlyBurner {
        accountVars[_owner].pointsBalance = uint128(
            accountVars[_owner].pointsBalance - _amount
        );
    }
        ...
        returns (uint32 stamina)
    {
        ...
        if (_rainiNft.isStaked) {
            uint256 degraded = (_rainiNft.maxStamina *
                STAMINA_DECIMALS *
                (block.timestamp - _rainiNft.lastUpdated)) / staminaDuration;
    ...
    function _setTokenStaminaTotal(
        uint32 _stamina,
        uint256 _tokenId,
        address _nftContractAddress
    ) internal {
    ...
    function recoverNfts(
        ...
        for (uint256 i; i < _tokenId.length; i++) {
            ...
            IRainiNft1155(_contractAddress[i]).safeTransferFrom(
                address(this),
                owner[i],
                _tokenId[i],
                1,
    ...
    function transferRaini(address _recipient, uint256 _amount)
        external
        onlyEditor
    {
    ...
    function findStaminaEventBefore(
        uint256 _eventId,
        uint256 _timeStamp,
        uint256 _start,
        uint256 _maxSteps,
        ...
        for (uint256 i = 0; i < _maxSteps; i++) {
            ...
            if (
                nextId == 0 ||
                _eventId == nextId ||
                (_timeStamp > 0 && _se.timeStamp > _timeStamp)
            ) {
                require(
                    lastTime != 0 || _eventId == nextId,
                    "starting event in future"
                );
                require(!(_eventId > 0 && nextId == 0), "event not found");
                if (_stateSet) {
                    return int256(lastStateId);
                } else {
                    return int256(lastId);
                }
            } else {
                lastTime = _se.timeStamp;
                lastId = nextId;
    ...
    function getBalanceUpdateEventIds(
        address _account,
        uint256 _level,
        ...
        AccountRewardVars memory _rewardVars = accountRewardVars[_account][
            _level
        ];
        ...
        for (uint256 n = 0; n <= nftOrder.length; n++) {
            ...
            if (
                ...
                int256 eventIdBefore = findStaminaEventBefore(
                    nft.staminaEventId,
                    (n == nftOrder.length) ? block.timestamp : 0,
                    _startId,
                    _maxSteps,
                    true
                );
    ...
    function getNftEventTimes(address _account, uint256 _level)
        public
        view
        returns (uint32[] memory eventTimes)
    {
        uint24[] memory _stakedNFTs = stakedNFTs[_account][_level];
    ...
    function getStakedNftData(address _account, uint256 _level)
        public
        view
        returns (RainiNft[] memory _nfts)
    {
        uint24[] memory _stakedNFTs = stakedNFTs[_account][_level];
    ...
    function getNftData(
        ...
        for (uint256 n = 0; n < _tokenId.length; n++) {
            _nftData[n] = rainiNfts[
                rainiNftIdMap[_contractAddress[n]][_tokenId[n]]
            ];
        }
        return _nftData;
    }
