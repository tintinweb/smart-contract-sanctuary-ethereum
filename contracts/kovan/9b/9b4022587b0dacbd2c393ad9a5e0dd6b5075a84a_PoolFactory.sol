    ...
    IStructs
{
    ...
    function deployPool(
        ...
        if (_undercollateralized < 0 || _undercollateralized > 1) revert InvalidType();
        
        return _initializePool(
            UserPoolData(
                _mintRatio,
                _colToken,
                _lendToken,
    ...
    function _initializePool(
        UserPoolData memory poolData
    ) private returns (address) {
        address lendingPool = address(
            new ERC1967Proxy(poolImplementationAddress, "")
        );

       (uint48 discountValue, uint48 dicountColValue) = licenseDiscount(poolData._licenseId);
        ...
        ILendingPool(lendingPool).initialize(
            Data(
                ...
                (protocolColFee * (hundredPercent - dicountColValue)) / hundredPercent,
                address(feesManager),
                oracle,
                address(this),
        ...
        pools[lendingPool] = true; // Register the pool before setting the fee, since FeeManager is checking if pool cretaed by Facotry
    ...
    function setRollbackImplementation(address _implementation)
        external
    {
        onlyOwner();
        rollBackImplementation = _implementation;
    ...
    /// @notice Update treasury address
    function setFeesManager(address _feesManager)
        external
    {
        onlyOwner();
        feesManager = IFeesManager(_feesManager);
    ...
    /// @notice Update treasury address
    function setLicenseEngine(address _licenseEngine)
        external
    {
        onlyOwner();
        licenseEngine = ILicenseEngine(_licenseEngine);
    ...
    function setProtcolFee(uint48 _protocolFee)
        external
    {
        onlyOwner();
        protocolFee = _protocolFee;
    ...
    function setCollateralAllowList(address _col, bool _allowed)
        public
    {
        onlyOwner();
    ...
    /// @notice This function will now accept the licence id that pool deployer has provided and will validate it.
    /// @notice If license is valid and has a valid discout associated with it, then such discount will be returned. 
    /// @return the discount amounts for a given license (lendDiscout, colDiscount)
    function licenseDiscount(uint256 _licenceId) private returns (uint48, uint48) {
        if (!licenseEngine.exists(_licenceId) || msg.sender != licenseEngine.ownerOf(_licenceId)) {
            return (0, 0);
        } else {
            (uint256 maxCount, uint256 curCount, uint48 discount, uint48 colDiscount, uint48 expiry) = licenseEngine.licenses(_licenceId);
            if (block.timestamp < expiry && curCount + 1 <= maxCount){
                if (discount > HUNDRED_PERCENT || colDiscount > HUNDRED_PERCENT) revert DiscountTooLarge();
                licenseEngine.incrementCurrentPoolCount(_licenceId);   
    ...
    function onlyOwnerORFirstResponder() private view {
        if (msg.sender != firstResponder && msg.sender != owner) revert NotAuthorized();
    }
