...
contract NodeERC1155 is INodeERC1155, ERC1155, Ownable, ReentrancyGuard {
    ...
    address public corkAddr;
    uint256 private _percentRate = 10**8;
    uint256 private _currentTokenID = 0;
    uint256 private _rewardInterval = 1 days;
    uint256 private _period = 30 days;
    ...
    function setCorkAddr(address _corkAddr) public onlyOwner {
        corkAddr = _corkAddr;
    }
    ...
    function mint(
        ...
        ICorkToken corkToken = ICorkToken(corkAddr);
        require(
        ...
        require(
            corkToken.balanceOf(_receiver) >= collection[_nodeType].price,
            "receiver's balance is less than node price"
        );

        if (collection[_nodeType].currentSupply == 5)
            collection[_nodeType].price = 100 ether;
        uint256 _id = _getNextTokenID();
        _incrementTokenID();
        nodeState[_id].purchaser = _receiver;
        ...
        _mint(_receiver, _nodeType, 1, "");

        ownedNodes[_receiver].push(_id);
    }
    ...
    function claim() external payable nonReentrant {
        ...
        // require(
        //     corkToken.balanceOf(address(this)) >= claimableCork,
        //     "Your claimable Cork is more than pool"
        // );
        corkToken.mint(msg.sender, claimableCork);
    }

    function getClaimableCork() public view returns (uint256) {
        require(ownedNodes[msg.sender].length > 0, "No have a node");
        uint256 claimableCork;
    ...
    function calculateMainAmount(
        ...
        if (_lastedMainDays > _periodDays) {
            ...
            return _lastedMainDays.div(_rewardInterval) * _firstRun;
        }
    }
    ...
    function calculateSnowballAmount(
        ...
        if (_lastedSnowballDays < _periodDays) {
            ...
            for (
                uint256 i = 0;
                i <= _lastedSnowballDays.div(_rewardInterval);
                i++
            ) {
                totalRates += i;
            }
    ...
    function sellableCork(address from)
        ...
        returns (uint256)
    {
        require(ownedNodes[from].length > 0, "Need a node at least");
    ...
    function getNodeState(uint id) public view returns (NodeStruct memory) {
        return nodeState[id];
    }
