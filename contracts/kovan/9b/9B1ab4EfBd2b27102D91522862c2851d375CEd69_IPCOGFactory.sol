...
interface IPrecogV5 {
    ...
    function getInvestmentOfByIndex(address token, address account, uint index) external view returns (
        uint amount, 
        uint unit,
        uint48 timestamp,
        uint16 idChanged
    );
    function getTradingCycleByIndex(address token, uint index) external view returns (uint16 id, uint48 startTime, uint48 endTime);
