...
contract BorrowerPools is PoolsSettingsManager, IBorrowerPools {
    ...
    )
  {
    ...
    for (rate; rate != parameters.MAX_RATE + parameters.RATE_SPACING; rate += parameters.RATE_SPACING) {
      if (pool.state.currentMaturity == 0) {
        ...
        totalAmount += uint128(uint256(pool.ticks[rate].adjustedRemainingAmount) * uint256(atlendisLiquidityRatio));
        if (totalAmount >= pool.parameters.MAX_BORROWABLE_AMOUNT) {
          uint128 delta = totalAmount - pool.parameters.MAX_BORROWABLE_AMOUNT;
          amountWeightedRate +=
            (uint128(uint256(pool.ticks[rate].adjustedRemainingAmount) * uint256(atlendisLiquidityRatio)) - delta) *
            rate;
          totalAmount -= delta;
          break;
        }
        amountWeightedRate +=
          (uint128(uint256(pool.ticks[rate].adjustedRemainingAmount) * uint256(atlendisLiquidityRatio))) *
          rate;
      } else {
    ...
    weightedAverageLendingRate = amountWeightedRate / totalAmount;
  }
  ...
  ) external view override returns (uint128 bondsQuantity, uint128 normalizedDepositedAmount) {
    Types.Pool storage pool = pools[poolHash];
    uint128 yieldProviderLiquidityRatio = uint128(
      pool.parameters.YIELD_PROVIDER.getReserveNormalizedIncome(address(pool.parameters.UNDERLYING_TOKEN))
    );
  ...
  function withdraw(
    ...
    yieldProvider.withdraw(pool.parameters.UNDERLYING_TOKEN, normalisedDepositedAmountToWithdraw, owner);
  }
  ...
  function updateRate(
    ...
    returns (uint128 newAdjustedAmount, uint128 newBondsIssuanceIndex)
  {
    Types.Pool storage pool = pools[poolHash];
    ...
    require(
      (oldBondsIssuanceIndex > pool.state.currentBondsIssuanceIndex) || (pool.state.currentMaturity == 0),
    ...
    uint128 normalizedAmount = pool.withdrawDepositedAmountForTick(
      oldRate,
      adjustedBondIndexAmount,
      oldBondsIssuanceIndex
    );
  ...
  {
    bytes32 poolHash = borrowerAuthorizedAddresses[_msgSender()];
    Types.Pool storage pool = pools[poolHash];
    require(!pool.state.closed, Errors.PSM_POOL_CLOSED);
    require(!pool.state.defaulted, Errors.PSM_POOL_DEFAULTED);
    require(pool.state.active, Errors.PSM_POOL_NOT_ACTIVE);
    ...
    pool.collectFees();
    uint128 remainingAmount = normalizedBorrowedAmount;
  ...
  function repay() external override whenNotPaused onlyRole(BORROWER_ROLE) {
    ...
    (uint128 normalizedRepayAmount, uint128 lateRepayFeePerBond, uint128 platformFee) = getRepayAmounts(poolHash);
    uint128 lateRepayFee = lateRepayFeePerBond.wadMul(pool.state.bondsIssuedQuantity);
    platformFees[poolHash] += platformFee;
  ...
  function topUpLiquidityRewards(uint128 normalizedAmount) external override whenNotPaused onlyRole(BORROWER_ROLE) {
    ...
    pool.depositToYieldProvider(_msgSender(), normalizedAmount, yieldProvider);

    pool.topUpLiquidityRewards(normalizedAmount);

    pool.activate();

    emit TopUpLiquidityRewards(borrowerAuthorizedAddresses[_msgSender()], normalizedAmount);
  }
...
import "./interfaces/IPlatformManagement.sol";
import "./interfaces/IPoolsParametersManagement.sol";

abstract contract PoolsSettingsManager is
  ...
  IPoolsParametersManagement
{
  ...
  function getPlatformFees(bytes32 poolHash) public view returns (uint128) {
    return platformFees[poolHash];
  }

  /**
   * @notice Withdraws platform fees to a target address
   * @param poolHash The identifier of the pool
  ...
  function claimPlatformFees(
    ...
    require(normalizedAmount <= platformFees[poolHash], Errors.PSM_NOT_ENOUGH_PLATFORM_FEES);
    yieldProvider.withdraw(pools[poolHash].parameters.UNDERLYING_TOKEN, normalizedAmount, to);
    platformFees[poolHash] -= normalizedAmount;

    emit ClaimPlatformFees(poolHash, normalizedAmount, to);
  }
  ...
  function createNewPool(PoolCreationParams calldata params) external override onlyRole(GOVERNANCE_ROLE) {
    ...
    pools[params.poolHash].parameters = Types.PoolParameters({
      ...
      PLATFORM_FEE_RATE: params.platformFeeRate,
    ...
    pools[params.poolHash].activate();
  }
  ...
  function allow(address borrowerAddress, bytes32 poolHash) external override onlyRole(GOVERNANCE_ROLE) {
    ...
    borrowerAuthorizedAddresses[borrowerAddress] = poolHash;
  ...
  function disallow(address borrowerAddress, bytes32 poolHash) external override onlyRole(GOVERNANCE_ROLE) {
    ...
    require(borrowerAddress != address(0), Errors.PSM_ZERO_ADDRESS);
    require(pools[poolHash].parameters.POOL_HASH == poolHash, Errors.PSM_POOL_NOT_ACTIVE);
    require(borrowerAuthorizedAddresses[borrowerAddress] == poolHash, Errors.PSM_DISALLOW_UNMATCHED_BORROWER);
  ...
  function closePool(bytes32 poolHash, address to) external override onlyRole(GOVERNANCE_ROLE) {
    ...
    uint128 collectedLiquidityRewards = pool.state.remainingLiquidityRewardsReserve;
    pool.state.remainingLiquidityRewardsReserve = 0;
    if (collectedLiquidityRewards > 0) {
      yieldProvider.withdraw(pools[poolHash].parameters.UNDERLYING_TOKEN, collectedLiquidityRewards, to);
    }
    emit PoolClosed(poolHash, collectedLiquidityRewards);
  }
  ...
  /**
   * @notice Set the pool platform fee
   **/
  function setPlatformFeeRate(uint128 platformFeeRate, bytes32 poolHash) external override onlyRole(GOVERNANCE_ROLE) {
    require(pools[poolHash].state.active, Errors.PSM_POOL_NOT_ACTIVE);

    pools[poolHash].parameters.PLATFORM_FEE_RATE = platformFeeRate;

    emit SetPlatformFeeRate(platformFeeRate, poolHash);
  }
}
...
/**
 * @title IBorrowerManagement
 * @notice Management of a pool lifecycle
 **/
interface IBorrowerManagement {
   ...
   * @param platformFeeRate Fees paid to Atlendis at repay time
...
/**
 * @title IPlatformManagement
 * @notice Overall management of the platform
 **/
interface IPlatformManagement {
   ...
   * @param to The address receiving the fees
   **/
  event ClaimPlatformFees(bytes32 poolHash, uint128 normalizedAmount, address to);
...
/**
 * @title IPoolsParametersManagement
 * @notice Management of a single pool
 **/
interface IPoolsParametersManagement {
  ...
  /**
   * @notice Emitted after governance sets the platform fee for a pool
   **/
  event SetPlatformFeeRate(uint128 platformFeeRate, bytes32 poolHash);
   ...
   * @notice Set the pool platform fee
   **/
  function setPlatformFeeRate(uint128 platformFeeRate, bytes32 poolHash) external;
}