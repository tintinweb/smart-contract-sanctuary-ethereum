...
contract Bridge is ValidatorSet {
     ...
     */
    function unlock(address payable to, address token, uint64 amount, uint[] calldata signers, uint8[] calldata v, bytes32[] calldata r, bytes32[] calldata s) external returns (bool) {
    ...
    function newEdgewareERC20(string calldata name, uint decimals, uint[] calldata signers, uint8[] calldata v, bytes32[] calldata r, bytes32[] calldata s) external returns (address addr) {
        ...
        edgewareTokens[name] = address(newToken);
        edgewareTokenAddresses[address(newToken)] = true;

        emit NewEdgewareERC20(name, address(newToken));
