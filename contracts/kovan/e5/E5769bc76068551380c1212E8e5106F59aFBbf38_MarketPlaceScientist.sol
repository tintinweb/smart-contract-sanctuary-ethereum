...
import "./interfaces/IMarketPlaceScientist.sol";
import "../helpers/timelock-access/TimelockAccess.sol";

contract MarketPlaceScientist is TimelockAccess, Ownable, IMarketPlaceScientist, Initializable {
    ...
    function initialize(address _timelock, address _scientistToken) external initializer {
        timelock = _timelock;
    ...
    function setScientistToken(address _address) external override onlyTimelock {
        require(_address != address(0), "Address should not be empty");
        emit TokenAdressChanged(scientistToken, _address);
        scientistToken = _address;
    }

    function setFeeWallet(address payable _feeWallet) external onlyTimelock {
        require(_feeWallet != address(0), "Address should not be empty");
        emit WalletAddressChanged(feeWallet, _feeWallet);
        feeWallet = _feeWallet;
    }

    function setFeeQuote(uint256 _feeQuote) external onlyTimelock {
        feeQuote = _feeQuote;
    }
        ...
        override
    {
        ...
        ScientistData.Scientist memory scientist = IScientistRepository(
            scientistToken
        ...
        IScientistRepository(scientistToken).updateScientist(
            scientist,
            _oldOwner
        );
        ...
        IERC721(scientistToken).safeTransferFrom(
            _oldOwner,
            msg.sender,
            _tokenId,
            ""
        ...
        returns (uint256)
    {
        ...
        revert("Token is not on sale");
    }

    function updateScientistPrice(uint256 _tokenId, uint256 _newPrice)
        external
        override
    {
        ScientistData.Scientist memory scientist = IScientistRepository(
        ...
        require(msg.sender == IERC721(scientistToken).ownerOf(_tokenId), "You are not the owner");
    ...
    function removeScientistFromSale(uint256 _tokenId) external override {
        ...
        emit ScientistRemovedFromMarketPlace(
            msg.sender,
            _tokenId,
            block.timestamp
        );
    ...
    function sellScientist(uint256 _tokenId, uint256 _price) external override {
        ...
        emit ScientistAddedToMarketplace(
            msg.sender,
            _tokenId,
            _price,
            block.timestamp
    ...
    function getOnSaleScientists()
        ...
        view
        override
        returns (address[] memory owners, uint256[] memory scientists)
    {

        scientists = sellingScientists;
        owners = new address[](
            scientists.length
        );

        for (uint256 i = 0; i < scientists.length; i++) {
            owners[i] = IERC721(scientistToken).ownerOf(
                scientists[i]
            );
        }
        return (owners, scientists);
    }
    ...
    function withdrawTokens(address token, address to) external onlyTimelock {
...
import "./IMarketPlaceScientistEvent.sol";
