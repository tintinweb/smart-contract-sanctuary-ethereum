...
contract Nft{
    ...
    function setBlue(uint256 vote) public {
        value(vote);
    ...
    function setGold(uint256 vote) public  {
        value(vote);
        userGold[msg.sender]+=vote;
        totalGold+vote;
    }
 
    function value(uint256 vote) public payable  {
     ...
     function value2(uint256 vote) public  returns(uint256)  {
        passingTime = block.timestamp-mmutable_start_time;
        timeDifference = passingTime/STEP;
        return price = (price + (timeDifference*1 ether/100000))*vote;
    }  
