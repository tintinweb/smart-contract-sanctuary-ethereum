...
contract Staking is Ownable, ReentrancyGuard, IStaking {
    ...
    uint256[4] private lockType = [
        60 * 60 * 24 * 7,
        60 * 60 * 24 * 7 * 2,
        60 * 60 * 24 * 30,
        60 * 60 * 24 * 90
    ];
    uint256[4] private apy = [0, 8, 13, 28];
        ...
        nonReentrant
    {
        ...
        uint256 toTransfer = allRewars(user);

        if (toTransfer > 0) QUACK.safeTransfer(user, toTransfer);

        stake.totalStakedForUser += amount;
        totalStaked += amount;

        updateLevel(user, lockUp);

        if (lockUp < 2)
            require(stake.level <= 3 + (3 * lockUp), "uncorrect level");
        ...
        if (amount > 0) {
            if (
                (lockUp < 3 &&
                    ...
                    block.timestamp) ||
                (lockUp == 3 &&
                    stake.enteredAt[lockUp] + lockType[lockUp] >
                    block.timestamp)
            ) {
            ...
            stake.sumInLock[lockUp] -= amount;
            totalStaked -= amount;
            if (stake.sumInLock[lockUp] == 0) {//сделать else'ом от 179
            ...
            if (stake.totalStakedForUser == 0) {
                ...
                if (stake.sumInLock[lockUp] > 0)
                    stake.rewardTaken[lockUp] = calculateReward(user, lockUp);
                updateLevel(user, 0);//вынести в else от 152?
            }
        }

        if (toTransfer > 0) {
            require(
                QUACK.balanceOf(address(this)) - totalStaked >= toTransfer,//добавить feesCollected
    ...
    {
        return
            calculateReward(user, lockUp) - stakeInfo[user].rewardTaken[lockUp];//развернуть полностью, удалить calculateReward()
    }

    function allRewars(address user) private returns (uint256 toTransfer) {
        UserInfo storage stake = stakeInfo[user];
        if (stake.totalStakedForUser > 0) {
            uint256 rew;
    ...
    function calculateReward(address _investor, uint256 lockUp)
        private
        view
        returns (uint256 rewardAmount)
    {
        rewardAmount =
            (stakeInfo[_investor].sumInLock[lockUp] *
                apy[lockUp] *
                (block.timestamp - stakeInfo[_investor].enteredAt[lockUp])) /
            (BASE * YEAR);
    }

    function updateLevel(address user, uint256 lockUp) private {//аргумент lockUp не нужен?
        require(lockUp <= 3, "Uncorrect lockUp");
        UserInfo storage stake = stakeInfo[user];

        if (stake.totalStakedForUser >= levels[8]) {
            stake.level = 9;
        } else if (stake.totalStakedForUser <= levels[0]) {
            stake.level = 0;
        } else {
            for (uint256 i; i < 8; i += 1) {
                if (
                    stake.totalStakedForUser >= levels[i] &&
                    stake.totalStakedForUser < levels[i + 1]
                ) stake.level = i + 1;
            }
        }
    }
