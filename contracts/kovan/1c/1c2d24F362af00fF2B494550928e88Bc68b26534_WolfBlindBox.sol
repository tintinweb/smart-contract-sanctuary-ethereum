...
// File: @openzeppelin/contrac[email protected]/utils/Address.sol
...
contract WolfBlindBox is Random,AdminTransfer,ReentrancyGuard {
    ...
    event WolfCreated(address indexed user,uint indexed tokenId,uint level,uint kind);

    constructor(IERC20 _cc,INFTAdmin _wolfAdmin,ICowActivity _cowAddress,uint _startDay) {
        cc=_cc;
    ...
    function adminStartDay(uint _startDay) public onlyOwner {
        startDay=_startDay;
    }
    ...
    function open(address to) public nonReentrant {
        ...
        uint nowDay=block.timestamp / 1 days;
        require(nowDay>=startDay,"WolfBlindBox: not open");
        uint index=(nowDay-startDay)+1;
        ...
        uint randomX128=_random(keccak256(abi.encode(cc,boxPrice)));
        ...
        if(i>=cowProbability.length) {//wolf
            wolf=wolf+1;
            uint kind=MathX128.mulX128(_random(keccak256(abi.encode(cc,i))),4)+1;
            uint wolfRandomX128=_random(keccak256(abi.encode(kind,i)));
            uint j;
            for(j=1;j<wolfProbability.length;j++){
                uint wolfProbabilityX128=MathX128.toX128(wolfProbability[i],0);
            ...
            uint level=j;
            require(level>=1 && level<=5,"WolfBlindBox: level error");
            uint tokenId=wolfAdmin.mintNFT(to);
            wolfAdmin.setUintProperty(tokenId,"level",level);
            wolfAdmin.setUintProperty(tokenId,"kind",kind);
            emit WolfCreated(to,tokenId,level,kind);
        } else {
            uint cowLevel=i+5;
