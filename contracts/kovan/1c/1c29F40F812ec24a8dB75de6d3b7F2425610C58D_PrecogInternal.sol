...
contract PrecogInternal is IPrecogInternal {
    ...
    function _calculateProfit(address _token, address _account)
        ...
        view
        returns (
            IPrecogStorage.AccountProfitInfo memory _accountProfitInfo
        )
    {
        ...
        if (_accountProfitInfo.lastProfitId < _updatedLatestCycle) {
            ...
            for (
                    ...
                    _accountProfitInfo.lastInvestmentId < _investments.length; 
                    _accountProfitInfo.lastInvestmentId++
                ) 
            {
                ...
                (
                    _nextInvestment, 
                    _lastAvailableProfitId
                ) = PrecogV5Library._chooseLastAvailableTradingId(
                    _investments,
                    _accountProfitInfo.lastInvestmentId,
                    _updatedLatestCycle
                );
                ...
                for (
                        ...
                        _accountProfitInfo.lastProfitId < _lastAvailableProfitId; 
                        _accountProfitInfo.lastProfitId++
                    ) 
                {
                    ...
                    if(_investments[_accountProfitInfo.lastInvestmentId].isWhitelist) {
                        ...
                        uint totalUnitsForWhitelist = precogStorage.getTotalUnitsForWhitelistTradingCycle(_token, _accountProfitInfo.lastProfitId);
                    ...
                    // Calcuclate profit for normal account at trading cycle 
                        ...
                        uint totalUnits = (
                            precogStorage.getTotalUnitsTradingCycle(_token, _accountProfitInfo.lastProfitId) - 
                            precogStorage.getTotalUnitsForWhitelistTradingCycle(_token, _accountProfitInfo.lastProfitId)
                        );
                        // Calculate profit for account at trading cycle
    ...
    function calculateProfit(address token, address account)
        ...
        override
        returns (
            IPrecogStorage.AccountProfitInfo memory accountProfitInfo
        )
    {
    ...
    function _updateProfit(address _token, address _account) internal {
        precogStorage.updateAccountProfitInfo(
            _token,
            _account,
            _calculateProfit(_token,_account)
        );
    }
    ...
    function _updateTotalUnitsForFirstTimeOfTradingCycle(
        ...
        if (!precogStorage.checkIsUpdateUnitTradingCycle(_token, _futureTradingCycle.id)) {
            uint _duration = _futureTradingCycle.endTime - _futureTradingCycle.startTime;
            ...
            uint newTotalUnitsForWhitelist = precogStorage.getTotalUnitsForWhitelistTradingCycle(_token, _futureTradingCycle.id) +
                (precogStorage.getLiquidityWhitelist(_token) * _duration);
            precogStorage.updateTotalUnitsForWhitelistTradingCycle(_token, _futureTradingCycle.id, newTotalUnitsForWhitelist);
    ...
    function _processIncreaseInvestment(
        ...
        if (_investmentsOf.length > 0) {
            ...
            if (_lastInvestmentOf.idChanged < _futureTradingCycle.id) {
                ...
                precogStorage.updateInvestmentOfByIndex(
                    _token,
                    _account,
                    _investmentsOf.length - 1,
                    _newInvestmentOf
                );
            }
            ...
            if (_lastInvestmentOf.isWhitelist != _newInvestmentOf.isWhitelist) {
                ...
                } else { // Current investment is not whitelist
                    newTotalUnits -= _newInvestmentOf.unit;
                }
            } else {
                if (_newInvestmentOf.isWhitelist) {
    ...
    function increaseInvestment(
        ...
        require(
            precogStorage.isOperator(msg.sender) || msg.sender == precogStorage.getTokenConvert(token),
    ...
    function _decreaseInvestment(
        ...
        require(!_isBeforeFundingTime(_token, _account), "PrecogInternal: Cannot request withdrawal before funding time");

        IPrecogStorage.Cycle memory _futureTradingCycle = _getTradingCycleByTimestamp(_token, _timestamp);
        _updateTotalUnitsForFirstTimeOfTradingCycle(_token, _futureTradingCycle);
        _remainingAmount = _processDecreaseInvestment(
            _token,
            _account,
            _amount,
            _timestamp,
    ...
    function _updateDepositInfo(
        ...
        if (block.timestamp >= _accountTradingInfo.depositedTimestampOf) {
            ...
            _accountTradingInfo.depositedTimestampOf = precogStorage
    ...
    function _updateCurrentTradingCycle(
        ...
        uint _timestampApplyNewTradingCycle = _getCoreInstance()
    ...
    function getTradingCycle(address token, uint48 tradingTime)
        ...
        view
        override
        returns (IPrecogStorage.Cycle memory)
    {
