    ...
    UUPSUpgradeable
{
    ...
    function setFeeAddress(address _fee) external onlyRole(MINTER_ROLE) {
        fee = _fee;
    }

    function setConsumerAddress(address _consumer) external onlyRole(MINTER_ROLE) {
    ...
    function playGame(
        ...
        require(outcomeType_ > 0 && outcomeType_ < 17);
    ...
    function playSlip(SlipCut[] memory _slipCuts, uint256 amount_, uint256 gameType_, uint256 oddPoint_) external returns (bool) {
        ...
        if(gameType_ == 1){
            ...
            for(uint256 i = 0; i < _slipCuts.length; i++){
                ...
                require(_slipCuts[i].outcomeType > 0 && _slipCuts[i].outcomeType < 17);
  ...
  //Convert String to Uint
    ...
    function sliceInterruptedBytes(uint256 _fixtureID) internal view returns(bool) {
        if(sc.requestIdData(sc.gameScoreRequestID(_fixtureID)).slice(2,11).equal(hex"696e746572727570746564") || 
            sc.requestIdData(sc.gameScoreRequestID(_fixtureID)).slice(2,8).equal(hex"66696e6973686564")){
           return true; 
        }
        else{
            return false;
    ...
    function getGameScoreRequestStatus(uint256 game_) public view returns(bool){
        bytes memory result = sc.requestIdData(sc.gameScoreRequestID(_gaming[_msgSender()][game_].fixtureID));
        if(result.length == 0 || result.slice(2,10).equal(hex"6e6f7473746172746564"))
            return true;
        return false;
    }

    function getGameScoreRequest(uint256 fixtureID) external view returns(bytes memory){
       return sc.requestIdData(sc.gameScoreRequestID(fixtureID));    
    }
    ...
    function claimGameReward(uint256 game_) external returns (bool){
        ...
        if(!gameResult[_gaming[_msgSender()][game_].fixtureID].retrieved){
            sliceBytes(_gaming[_msgSender()][game_].fixtureID, sc.requestIdData(sc.gameScoreRequestID(_gaming[_msgSender()][game_].fixtureID)));
        }
        
        require(gameResult[_gaming[_msgSender()][game_].fixtureID].status.equal(hex"66696e6973686564") 
                ||
                sc.requestIdData(sc.gameScoreRequestID(_gaming[_msgSender()][game_].fixtureID)).slice(2,11).equal(hex"696e746572727570746564"));
    ...
    //FOR SLIPS
    // function getSlipScoreRequestStatus(uint256 game_) external view returns(bool){
    //     bool outcome = false;
    //     bytes memory result;
    //     for(uint256 i = 0; i < _gamingSlip[_msgSender()][game_].slips.length; i++){
    //         result = sc.requestIdData(sc.gameScoreRequestID(_gamingSlip[_msgSender()][game_].slips[i].fixtureID));
    //         if(result.length == 0 || result.slice(2,10).equal(hex"6e6f7473746172746564")){
    //             outcome = true;
    //             break;
    //         }else {
    ...
    //         result = sc.requestIdData(sc.gameScoreRequestID(fixtureID));
    //         if(result.length == 0 || result.slice(2,10).equal(hex"6e6f7473746172746564")){
    //             //Update Game Status, Home and Away Scores from the Oracle
    //             bytes32 JOB_ID = bytes32("7a1ade57c05f49f1bd5c25241acdb107");       
    ...
    function claimSlipReward(uint256 game_) external returns (bool){
        ...
        for(uint256 i = 0; i < _gamingSlip[_msgSender()][game_].slips.length; i++){
            if(!gameResult[_gamingSlip[_msgSender()][game_].slips[i].fixtureID].retrieved){
                sliceBytes(_gamingSlip[_msgSender()][game_].slips[i].fixtureID, sc.requestIdData(sc.gameScoreRequestID(_gamingSlip[_msgSender()][game_].slips[i].fixtureID)));
            }
            require(gameResult[_gamingSlip[_msgSender()][game_].slips[i].fixtureID].retrieved);
            require(gameResult[_gamingSlip[_msgSender()][game_].slips[i].fixtureID].status.equal(hex"66696e6973686564"));
        }
