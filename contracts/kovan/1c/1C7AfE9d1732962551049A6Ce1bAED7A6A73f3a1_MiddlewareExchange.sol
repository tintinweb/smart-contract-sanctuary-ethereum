...
contract MiddlewareExchange is IMiddlewareExchange {
    ...
    function buyPCOG(address token, uint256 amount) override external returns (uint256 boughtAmount) {
        ...
        boughtAmount = IExchangeRouter(exchange).swapExactTokensForTokens(amount, estimatedPCOG, getPath(token), address(this), block.timestamp + 1000)[1];
        IERC20(PCOG).safeTransfer(msg.sender, boughtAmount);
    }
}
