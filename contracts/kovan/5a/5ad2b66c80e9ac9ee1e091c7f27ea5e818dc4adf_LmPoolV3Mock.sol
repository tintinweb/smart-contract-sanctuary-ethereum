...
contract LmPoolV3Mock is Ownable, ReentrancyGuardUpgradeable, IERC721Receiver {
    ...
    */
    ...
    @dev    this function definition is marked "external" because this fuction is called only from outside the contract.
    */
    function add(IERC721 _erc721Token,address _token0, address _token1, uint24 fee) external onlyOwner {
        ...
        require(IUniswapV3Factory(_erc721Token.factory()).getPool(_token0,_token1,fee) != address(0), "Pair not created");
        ...
        lpTokensPool[_token0][_token1] = poolInfo.length.sub(1);
        lpTokensPool[_token1][_token0] = poolInfo.length.sub(1);
    }
    ...
    */
    function deposit(
        ...
        (,,_token0,_token1,,,,_amount,,,,) = pool.lpToken.positions(_tokenId);
        require(lpTokensStatus[_token0][_token1], "LP token not added");
        require(lpTokensPool[_token0][_token1] == _pid, "LP token wrong pool Id");
        if (_amount > 0) {
    ...
    function checkEligibleAmount(
        ...
        for (uint256 i = 1; i <= countofstake; i++) {
            ...
            if (stkInfo.deposit == true) {
                ...
                if (block.timestamp >= timeaftervaultmonth) {
                    eligibleAmount = eligibleAmount.add(stkInfo.amount);
                    pool.lpToken.safeTransferFrom(address(this), address(msg.sender), stkInfo.tokenId);
                    if (_withUpdate) {
    ...
    */
    ...
    @param _pid : pool id
    */
    function viewRewardInfo(uint256 _pid, address _userAddress) external view returns (uint256) {
        ...
        uint256 userBaseMul =
            currentUser.amount.mul(getRewardMultiplier(_userAddress));
        uint256 totalBlockReward = blockReward.add(currentUser.preBlockReward);
