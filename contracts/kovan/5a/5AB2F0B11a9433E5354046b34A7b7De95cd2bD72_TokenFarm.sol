...
contract TokenFarm is Ownable {
    ...
    function stakeTokens(uint256 _amount, address _token) public {
        ...
        require(isTokenAllowed(_token), "The token is currently not allowed.!");
    ...
    function issueTokens() public onlyOwner {
        for (uint256 index = 0; index < stakers.length; index++) {
            ...
            // the `transfer` and the `transferFrom` methods do the same thing
            // but the `transferFrom` method transfers token with a "from" parameter
            // whereas the `transfer` method transfers the token a the caller contract
            // for example the following statement can be call like that:
            // dappToken.transferFrom(address(this), recipient, totalValue)
            // (note: I am not sure about the above statement, but I guess it is something like that)
