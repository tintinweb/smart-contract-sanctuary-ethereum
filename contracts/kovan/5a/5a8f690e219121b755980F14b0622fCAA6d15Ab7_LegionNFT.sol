...
contract LegionNFT is Context, ILegionNFT, Ownable {
    ...
    event Hunted(uint256 roll, uint256 percent, bool success);
    ...
    function hunt(uint256 tokenId, uint8 monsterId) external returns(bool, uint256, uint256) {
        ...
        emit Hunted(randnum+1, percent, retval);
        return (retval, randnum+1, percent);
    }
    ...
    */
    ...
    function getSummoningPrice(uint256 _amount) external view override returns(uint256) {
        ...
        if(_amount==1) {
            ...
            amountIn = UsdValue*10*99/100;
        } else if(_amount==50) {
            amountIn = UsdValue*50*97/100;
        } else if(_amount==200) {
            amountIn = UsdValue*200*95/100;
        } else if(_amount==500) {
            amountIn = UsdValue*500*90/100;
        } else {
            amountIn = UsdValue*_amount;
        }
    ...
    function getTrainingCost(uint256 _count) external view override returns(uint256) {
        ...
        path[0] = BLST;
        path[1] = BUSD;
        return router.getAmountsIn(_count*(10**6)/2, path)[1];
    }
    ...
    function getSupplyCostInUSD(uint256 _warriorCount, uint256 _supply) external view override returns(uint256) {
        if(_supply==7) {
            return 7*_warriorCount;
        } else if(_supply==14) {
            return fees[4]*_warriorCount;
        } else if (_supply==28) {
            return fees[5]*_warriorCount;
        } else {
            return _supply*_warriorCount;
        }
    }
    ...
    function getCostForAddingWarrior(uint256 _warriorCount, uint256 _remainingHunts) external view override returns(uint256) {
        ...
        path[0] = BLST;
        path[1] = BUSD;
        uint256 cost = router.getAmountsIn((_remainingHunts*_warriorCount)*10**6+_warriorCount*10**6/2, path)[1];
        return cost;
    }
    ...
    function getBLSTAmount(uint256 _usd) internal view returns(uint256) {
        ...
        path[0] = BLST;
        path[1] = BUSD;
        return router.getAmountsIn(_usd*10**6, path)[1];
    }
    function getBLSTReward(uint256 _reward) external view override returns(uint256) {
        address[] memory path = new address[](2);
        path[0] = BLST;
        path[1] = BUSD;
        return router.getAmountsIn(_reward*10**2, path)[1];
    }
    function getExecuteAmount() external view override returns(uint256) {
        address[] memory path = new address[](2);
        path[0] = BLST;
        path[1] = BUSD;
        return router.getAmountsIn(fees[3]*10**6*2/10, path)[1]; // 20% will return back to player
    }
}
...
contract RewardPool is IRewardPool, Ownable {
    ...
    function addReward(address _winner, uint256 _monster_id, uint256 _reward) external override onlyLegion {
        ...
        userRewards[_winner].push(Reward(_monster_id, block.timestamp, realReward));
        unclaimedUSD[_winner] += realReward;
        totalHuntedCount--;
        emit RewardChanged(_winner, unclaimedUSD[_winner]);
    }

    function subReward(address _address, uint256 _reward) external override onlyLegion {
