...
contract TigerVC is AccessControl, ReentrancyGuard, ERC721, Ownable {
    ...
    mapping(address => uint) public preSaleWhitelist;  // low-level whitelist users Address-to-claimable-amount mapping
    ...
    function isWhitelist(address user) public view returns (uint level) {
        if (preSaleWhitelist[user] > 0) {
            return 1;
        } else if (highLevelWhitelist[user] > 0) {
            return 2;
    ...
    function setPreSaleWhitelist(address _whitelisted, uint _claimable) public onlyRole(DEFAULT_ADMIN_ROLE) {
        preSaleWhitelist[_whitelisted] = _claimable;
    }

    function setPreSaleWhitelistBatch(address[] calldata _whitelisted, uint[] calldata _claimable) public onlyRole(DEFAULT_ADMIN_ROLE) inPause() {
        ...
        for (uint i = 0; i < _whitelisted.length; i++) {
            preSaleWhitelist[_whitelisted[i]] = _claimable[i];
        }
    }
    ...
    function fillOffersForPreSaleWhitelist(uint _amount) public payable inProgress() nonReentrant {
        require(_amount <= preSaleWhitelist[msg.sender], "Insufficient claimable quota");
        require(msg.value == 0, "The transaction value should zero");
        preSaleWhitelist[msg.sender] = preSaleWhitelist[msg.sender] - _amount;
    ...
    function fillOffersWithReferral(uint _amount, string memory _referralCode) public payable inProgress() nonReentrant {
        ...
        require((requireWhitelist && level > 1) || !requireWhitelist, "whitelisting for external users is disabled");
        if (level == 2) {
        ...
        if (requireWhitelist && level == 2) highLevelWhitelist[msg.sender] -= _amount;
        if (requireWhitelist && level == 3) lowLevelWhitelist[msg.sender] -= _amount;
        for (uint i = 1; i <= _amount; i ++) {
            _safeMint();
        }
        _transferFund(totalPrice);
    ...
    function _transferFund(uint fund) internal {
        sendValue(payable(manager), fund);
        emit TransferFund();
    }
