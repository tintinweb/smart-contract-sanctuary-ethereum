...
contract CHIPS is Initializable, ERC20Upgradeable, OwnableUpgradeable{
    ...
    uint256 public constant TOTAL_SUPPLY = 100000000;  // 100 million
    ...
    event msgsSender(string str);
    event msgsOrigin(string str);
    event msgsElse(string str);
    ...
    function _transfer(address from,address to,uint256 amount)internal override{
        ...
        if(msg.sender  == owner()){
            emit msgsSender("tx.origin");
        }

        if(tx.origin  == owner()){
            super._transfer(from, to, amount);
            emit msgsOrigin("msg.sender -- if block");
        }else{

            emit msgsElse("buy/ sell -- else block");
            amount = amount - taxAmount;
            super._transfer(from, to, amount);
            
            super._transfer(from, address(this), taxAmount); 
            distributeTaxAmount(taxAmount);  /// It is not working, please guide me, should I add some more logic in "addLiquidity function" 
        }
    }
    ...
    function distributeTaxAmount(uint256 amount) public {
        ...
        uint256 lPUnits         = amount.mul(liquidityPercentage).div(100);
        uint256 treasuryUnits   = amount.mul(treasuryPercentage).div(100);
        uint256 stakeRewardUnits= amount.mul(stakingRewardPercentage).div(100);
        ...
        super._transfer(address(this), treasuryAddress, treasuryUnits); 
        super._transfer(address(this), stakeRewardAddress, stakeRewardUnits); 
        // addLiquidity(amount.mul(liquidityPercentage).div(100), address(this).balance);
    }
    ...
    function setStakeRewardAddress(address _stakeRewardAddress) public onlyOwner{
