...
contract MoltlyBundleMarketplace is
    OwnableUpgradeable,
    ReentrancyGuardUpgradeable
{
    ...
    IMoltlyAddressRegistry public addressRegistry;
        ...
        require(
            _payToken == address(0) ||
                (tokenRegistry != address(0) &&
                    IMoltlyTokenRegistry(tokenRegistry).enabled(_payToken)),
            "invalid pay token"
        );
    ...
    function _buyItem(string memory _bundleID, address _payToken) private {
        ...
        if (_payToken == address(0)) {
            ...
            require(
                feeTransferSuccess,
                "MoltlyMarketplace: Fee transfer failed"
            );
            ...
            require(
                ...
                "MoltlyMarketplace: Owner transfer failed"
            );
        } else {
        ...
        for (uint256 i; i < listing.nfts.length; i++) {
            ...
            IMoltlyMarketplace(addressRegistry.marketplace()).validateItemSold(
                listing.nfts[i],
                listing.tokenIds[i],
                owner,
                _msgSender()
        ...
        emit ItemSold(
            ...
            _bundleID,
            _payToken,
            IMoltlyMarketplace(addressRegistry.marketplace()).getPrice(_payToken),
            price
        );
        ...
        nonReentrant
    {
        ...
        emit ItemSold(
            ...
            IMoltlyMarketplace(addressRegistry.marketplace()).getPrice(address(offer.payToken)),
            offer.price
        );
    ...
    /**
     @notice Update MoltlyAddressRegistry contract
     @dev Only admin
     */
    function updateAddressRegistry(address _registry) external onlyOwner {
        addressRegistry = IMoltlyAddressRegistry(_registry);
    }
