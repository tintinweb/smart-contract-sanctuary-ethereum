...
contract MasterEthStaking is MasterPrimeStaking {
    ...
    /// @notice Eth amount distributed for given period. ethAmountPerBlock = ethAmount / (ethEndBlock - ethStartBlock)
    uint256 public ethStartBlock; // staking start block.
    uint256 public ethEndBlock; // staking end block.
    uint256 public ethAmount; // the amount of ETH to give out as rewards.
    uint256 public ethAmountPerBlock; // the amount of ETH to give out as rewards per block.
    ...
    mapping(uint256 => mapping(address => EthDepositInfo)) public ethDepositInfo;
    ...
    event LogPoolSetEthAllocPoint(uint256 indexed pid, uint256 allocPoint);
    event LogSetEthPerBlock(
        uint256 ethAmount,
        uint256 startBlock,
        uint256 endBlock,
        uint256 ethAmountPerBlock
    );

    constructor(IERC20 _prime, IERC1155 _parallelAlpha)
        MasterPrimeStaking(_prime, _parallelAlpha)
    {}
        ...
        onlyOwner
    {
        ...
        ethPoolInfo.push(
            EthPoolInfo({
                ...
                lastRewardBlock: uint64(ethStartBlock)
            })
        );
        MasterPrimeStaking.addPool(_allocPoint, _tokenIds);
    }

    // Set remaining eth to distribute between ethEndBlock-ethStartBlock
    function setEthPerBlock(
        uint256 _ethStartBlock,
        uint256 _ethEndBlock,
        uint256 _ethAmount
    ) public payable onlyOwner {
        require(
            _ethStartBlock < _ethEndBlock,
            "endBlock cant be less than startBlock"
        );
        require(ethEndBlock < block.number, "Only updates after initial ethEndBlock");
        require(address(this).balance + msg.value >= _ethAmount, "Insufficient ETH");
        // Update all ethPools before proceeding, ensure rewards calculated up to this block
        for (uint256 i = 0; i < ethPoolInfo.length; ++i) {
            updateEthPool(i);
            ethPoolInfo[i].lastRewardBlock = uint64(_ethStartBlock);
        }
        ethAmount = _ethAmount;
        ethStartBlock = _ethStartBlock;
        ethEndBlock = _ethEndBlock;
        ethAmountPerBlock = _ethAmount / (_ethEndBlock - _ethStartBlock);
        emit LogSetEthPerBlock(
            _ethAmount,
            _ethStartBlock,
            _ethEndBlock,
            ethAmountPerBlock
    ...
    function setEthEndBlock(uint256 _ethEndBlock) public onlyOwner {
        require(ethStartBlock < block.number, "staking have not started yet");
        require(ethEndBlock < _ethEndBlock, "invalid end block");
        for (uint256 i = 0; i < ethPoolInfo.length; ++i) {
            updateEthPool(i);
        }
        // Update ethPool stuffs
        ethEndBlock = _ethEndBlock;
        ethAmountPerBlock = ethAmount / (ethEndBlock - ethStartBlock);
    }

    function addEthAmount(uint256 _ethAmount) public payable onlyOwner {
        require(
            ethStartBlock < block.number && block.number < ethEndBlock,
        ...
        ethAmount += _ethAmount;
        ethAmountPerBlock = ethAmount / (ethEndBlock - block.number);
    }
        ...
        onlyOwner
    {
        ...
        emit LogPoolSetEthAllocPoint(_pid, _allocPoint);
    }
        ...
        returns (uint256 pending)
    {
        ...
        if (
            ethStartBlock <= block.number &&
            ethPool.lastRewardBlock < block.number &&
            totalSupply > 0
        ) {
            uint256 updateToBlock = Math.min(block.number, ethEndBlock);
            uint256 blocks = updateToBlock - ethPool.lastRewardBlock;
            uint256 ethReward = blocks *
                ethAmountPerBlock *
                ethPool.allocPoint / ethTotalAllocPoint;
            accEthPerShare += ethReward / totalSupply;
        }
        pending = uint256(
            int256(ethDeposit.amount * accEthPerShare) - ethDeposit.rewardDebt
        );
    }
    ...
    function updateEthPool(uint256 pid) public {
        ...
        uint256 totalSupply = pool.totalSupply;
        if (ethStartBlock > block.number || ethPool.lastRewardBlock >= block.number) {
            return;
        }
        ...
        ethPool.lastRewardBlock = uint64(updateToBlock);
        emit LogUpdatePool(
            pid,
            ethPool.lastRewardBlock,
            totalSupply,
    ...
    function deposit(uint256 pid, uint256 amount) public override virtual {
        require(amount > 0, "Specify valid paraset amount to deposit");
        updateEthPool(pid);
        EthDepositInfo storage ethDeposit = ethDepositInfo[pid][msg.sender];

        // Effects
        ethDeposit.amount += amount;
        ethDeposit.rewardDebt += int256(amount * ethPoolInfo[pid].accEthPerShare);

        MasterPrimeStaking.deposit(pid, amount);
    }

    /// @notice Withdraw parasets.
    /// @param pid The index of the ethPool. See `ethPoolInfo`.
    /// @param amount amounts to withdraw for the paraset
    function withdraw(uint256 pid, uint256 amount) public override virtual {
        updateEthPool(pid);
        EthDepositInfo storage ethDeposit = ethDepositInfo[pid][msg.sender];
        ...
        MasterPrimeStaking.withdraw(pid, amount);
    }

    /// @notice Harvest proceeds for transaction sender to msg.sender.
    /// @param pid The index of the ethPool. See `ethPoolInfo`.
    function harvestEth(uint256 pid) public {
        updateEthPool(pid);
        EthDepositInfo storage ethDeposit = ethDepositInfo[pid][msg.sender];
        int256 accumulatedEth = int256(
            ethDeposit.amount * ethPoolInfo[pid].accEthPerShare
        );
        ...
        emit HarvestEth(msg.sender, pid, _pendingEth);
    }

    /// @notice HarvestPrime and HarvestETH a pool
    /// @param pid Pool IDs of all to be harvested
    function harvestEthAndPrime(uint256 pid) public virtual {
      MasterPrimeStaking.harvestPrime(pid);
      harvestEth(pid);
    }

    /// @notice HarvestPrime multiple ethPools
    /// @param pids Pool IDs of all to be harvested
    function harvestPools(uint256[] calldata pids) public virtual {
        for (uint256 i = 0; i < pids.length; ++i) {
            harvestEthAndPrime(pids[i]);
        }
    }
    ...
    function withdrawAndHarvestEthAndPrime(uint256 pid, uint256 amount)
        public
        virtual
    {
        // Harvest ETH
        ...
        // Withdraw and harvest PRIME
        MasterPrimeStaking.withdrawAndHarvestPrime(pid, amount);
        emit HarvestEth(msg.sender, pid, _pendingEth);
    }
}
...
/// rewards distributed evenly across all stakers per block.
contract MasterPrimeStaking is Ownable, ERC1155Holder {
    using SafeERC20 for IERC20;
    ...
    /// @param pids Pool IDs of all to be harvested
    function harvestPrimePools(uint256[] calldata pids) public virtual {
        for (uint256 i = 0; i < pids.length; ++i) {
            harvestPrime(pids[i]);
        }
    }
