...
contract YourContract {
  ...
  function checkUpkeep(bytes calldata checkData) external returns (bool upkeepNeeded, bytes memory performData) {
      ...
      //  emit CheckUpkeep(block.timestamp, lastTimeStamp);
      ...
      for (uint i = 0; i < rentalDatas.length; i++) {
        ...
        emit CheckEndRent(rentalDatas[i].owner, rentalDatas[i].renter, rentalDatas[i].tokenId, 
          rentalDatas[i].contractAddress, rentalDatas[i].startDate, rentalDatas[i].endDate);
        
        if(block.timestamp > rentalDatas[i].endDate) {
          
          emit NeedEndRent(rentalDatas[i].owner, rentalDatas[i].renter, rentalDatas[i].tokenId, 
            rentalDatas[i].contractAddress, rentalDatas[i].startDate, rentalDatas[i].endDate);
          upkeepNeeded = true;
        } 
      } 
   ...
   function performUpkeep(bytes calldata performData) external {
      ...
      for (uint i = 0; i < rentalDatas.length; i++) {
        if(block.timestamp > rentalDatas[i].endDate) {
          (bool success, bytes memory data) = rentalDatas[i].contractAddress.delegatecall(
            abi.encodeWithSignature("transferFrom(address, address, uint256)", rentalDatas[i].renter, 
              rentalDatas[i].owner, rentalDatas[i].tokenId)
          );
        } 
      } 
   ...
   function addRentData(address owner, address renter, uint256 tokenId, 
    address contractAddress, uint startDate,  uint endDate) external {
       emit AddRentalData(owner, renter, tokenId, contractAddress, startDate, endDate);
       rentalDatas.push(RentalData(owner, renter, tokenId, contractAddress, startDate, endDate));
   }

    function getRentData() external {
      // new code
      currentState = "nice";
      // for (uint i = 0; i < rentalDatas.length; i++) {
      //   currentState = string(abi.encodePacked(currentState, ";;;;", rentalDatas[i].owner, "/", rentalDatas[i].renter));
      // } 
   }
