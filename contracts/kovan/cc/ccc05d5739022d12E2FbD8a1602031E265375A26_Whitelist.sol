...
contract Whitelist is Ownable, IWhiteList {
    ...
    function addValidator(address newValidator) public onlyOwner {
        ...
        require(!validator[newValidator], "ALREADY EXIST");
        validator[newValidator] = true;
        emit AddValidator(newValidator);
    }

    function removeValidator(address oldValidator) public onlyOwner {
        require(validator[oldValidator], "NOT EXIST");
        validator[oldValidator] = false;
        emit RemoveValidator(oldValidator);
    }

    function checkValidator(address _validator)
        external
        view
        override
        returns (bool)
