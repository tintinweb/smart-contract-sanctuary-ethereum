...
contract TPCoreV2 is AccessControl {
    ...
    // 16
    uint256 public ownerPercentage = 9950; //(this means we hold 0.5%) make sure when changing it to keep this formatting
    ...
    // 48 bit minimum
    //uint256 public tpyRewardPerInterval = 23972603 * 7 * (10**6); // 239726.03
    uint256 public tpyRewardPerInterval = 1_000;// todo test
    
    // 32 (max 1 year)
    //uint256 public tpyInterval = 1 weeks; 
    uint256 public tpyInterval = 30 seconds;//todo test
    ...
    function registerTPY(address staker, uint256 stakingCoinAmount) private {
        uint256 nextIntervalNumber = nextInterval();
    ...
    function currentInterval() private view returns (uint256) {
        return (block.timestamp - startingTS) / tpyInterval;
    }

    function nextInterval() private view returns (uint256) {
