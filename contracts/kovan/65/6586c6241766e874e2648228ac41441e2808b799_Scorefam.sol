    ...
    UUPSUpgradeable
{
    ...
    { 
        ...
        if (gameType == 1) 
        {
            ...
            for( uint256 i = 0; i < _gamingSlip[_msgSender()][game_].slips.length; i++ ){
                ...
                require(isGameResultResolved(fixtureID));
    ...
    function playSlip(
        ...
        //Multi-flex Game
            ...
            IBEP20.transfer(address(this), amount_);
    ...
    //FOR GAMES
    function isOracleGameResolved(uint256 _fixtureID, uint256 _leagueId, uint256 _date) external view returns(bool){
        bool resolved = false;
        for(uint8 i = 0; i < sc.getRequestIdGamesLength(sc.requestId(_leagueId,_date)); i++){
            if(sc.getGameResolve(sc.requestId(_leagueId,_date),i).gameId == _fixtureID){
                resolved = true;
                break;
            } else {
                continue;
    ...
    function requestOracleSchedule(uint256 _leagueId, uint256 _date) external {
        sc.requestSchedule(0x6532343762366132633332373431366138393030646432373030356436626334, 100000000000000000, 1, _leagueId, _date);
    }
    ...
    function claimSlipReward(uint256 game_) external returns (bool){
        ...
        if(_gamingSlip[_msgSender()][game_].gameType == 2){
            IBEP20.transfer(_msgSender(), amount_.sub(onePercent));
        } else {
            _mint(fee, onePercent); //Charging 1% Fee
            _mint(_msgSender(), amount_.sub(onePercent));
        }
