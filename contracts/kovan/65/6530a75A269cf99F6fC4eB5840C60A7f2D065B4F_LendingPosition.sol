...
contract LendingPosition is Ownable {
    ...
    // could use struct to easily manage
    //

    uint256 public pid;
    uint256 public amount; //ETH
    uint256 public borrowAmount; //DAI
    uint256 public positionPrice; // price @ borrow
    address public tokenToBorrow; // DAI token
    uint256 public interestRateMode;
    uint16 public referralCode;
    uint256 public leveragePercentage; // <= 80 %
    ...
    constructor(
        ...
        uniswapRouter = IUniswapV2Router02(
            LendingFactory(factory).uniswap_address() //should read from constructor
        );
    ...
    function getTotalDebtETH() public view returns (uint256) {
        //redundant. could use getPositionData
        (, uint256 totalDebtETH, , , , ) = getPositionData();
        return totalDebtETH;
    }
...
// can add more utility to manage position and update factory state
// ex. ClosePosition -> mark position as remove from openedPosition,
...
contract LendingFactory is Ownable {
    ...
    address public dai_address; // actually don't need this _tokenToBorrow will do
    address public weth_address;
    address public uniswap_address; // don't need this
    address public lendingPoolAddressProvider;

    IWETH public wethToken; //don't need this
    ...
    function getAddressFromPositionId(
        uint256 positionId // could read directly
    ) public view returns (address) {
        return positionAddress[positionId];
    }

    function getUserOpenedPosition(
        address _user // could read directly
