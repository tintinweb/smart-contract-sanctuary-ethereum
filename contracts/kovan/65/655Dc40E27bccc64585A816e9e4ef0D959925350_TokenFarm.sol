...
contract TokenFarm is Ownable{
    ...
    constructor (address _dappTokenAddress) public {
        dappToken = IERC20(_dappTokenAddress); //defined the token
    }
    ...
    function stakeTokens(uint256 _amount, address _token) public {
        ...
        require(tokenIsAllowed(_token)  , "The token entered is not allowed");

        //transfer ERC20 used when perso owns the wallet
        //transferFrom  used when somebody else owns te wallet 
        // we need ABI foe the token se using interface
    ...
    function issueToken() public onlyOwner{
        ...
        for (uint256 Stakerindex = 0; Stakerindex < stakers.length; Stakerindex++) {
            ...
            //send them a total reward
            //based on their total value locked
            // dappToken.transfer(recepient, ?? how much)
            dappToken.transfer(recepient, userTotalValue);
            //transfer the dapp token similar to amount the user has locked on the platform
        }
    ...
    function getUserSingleTokenValue(address _user, address _token) public view returns (uint256){
        ...
        //price of that token * stakingBalance of user
        (uint256 price, uint256 decimals) = getTokenValue(_token);
        return (stakingBalance[_token][_user] * price / (10**decimals));
    } 

    function getTokenValue(address _token) public view returns (uint256, uint256){
        // price feed address => map token 
