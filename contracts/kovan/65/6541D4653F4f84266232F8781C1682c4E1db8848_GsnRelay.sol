...
import "./versions/LondonTxSupport.sol";
// import "./versions/LegacyTxSupport.sol";

contract GsnRelay is Ownable, LondonTxSupport {
    ...
    // The gas used to send the user transaction, excluding gas, the user transaction itself.
    uint256 private txRelayGasUsage = 50000;
    // The amount of gas to swap and send tokens back to the user
    uint256 private txPostRelayGasUsage = 30000;
    ...
    function sendTransaction(TxRequest calldata _tx, TxFee calldata _fee, bytes calldata _sign) external onlyFeePayer {
        ...
        _verify(_tx, _sign);
        require(_tx.gas > txRelayGasUsage, "GsnRelay: Not enough gas to execute the transaction");
        ...
        uint256 maxFeeInTokens = router.getAmountsIn(maxFeeInEth, _swapPair(address(token)))[0];
        ...
        require(token.approve(address(router), maxFeeInTokens), "GsnRelay: Approve router failed");


        uint256 gasUsed = initialGas.sub(gasleft()).add(txPostRelayGasUsage);
        uint256 feeInEth = _calculateCharge(gasUsed, _tx);
        router.swapTokensForExactETH(feeInEth, maxFeeInTokens, _swapPair(address(token)), txFeePayer, block.timestamp)[0];
...
//        }
    ...
    function _call(address from, address to, uint256 value, bytes calldata data) internal {
        uint256 gas = gasleft() - txRelayGasUsage;

        bytes memory callData = abi.encodePacked(data, from);
        (bool success,) = to.call{gas : gas, value : value}(callData);

        require(success, "GsnRelay: Transaction Call Error");
    }
    ...
    function _verify(TxRequest calldata _tx, bytes calldata _sign) internal {
        require(_tx.deadline == 0 || _tx.deadline > block.timestamp, "GsnRelay: Request expired");
        require(nonces[_tx.from]++ == _tx.nonce, "GsnRelay: Nonce mismatch");

        address signer = _getSigner(DOMAIN_SEPARATOR, _tx, _sign);

        require(signer != address(0) && signer == _tx.from, 'GsnRelay: Invalid signature');
    }
}
