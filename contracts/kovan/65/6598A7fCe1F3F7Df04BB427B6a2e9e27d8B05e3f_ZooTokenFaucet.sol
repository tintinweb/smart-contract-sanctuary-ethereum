...
contract ZooTokenFaucet is ERC721 {
	...
	event tokensGiven(address indexed user, uint256 id1, uint256 id2);

	constructor (string memory name, string memory symbol, address _zoo, address _dai) ERC721(name, symbol) 
	{
		zoo = IERC20(_zoo);
		dai = IERC20(_dai);
	...
	function getTokens() external 
	{
		...
		_tokenIdTracker.increment();
		uint256 newId1 = _tokenIdTracker.current();
		_safeMint(msg.sender, newId1);

		attemptAmount[msg.sender] += 1;

		emit tokensGiven(msg.sender, newId, newId1);
	}
	...
	function mintNft() external onlyWhiteList
	{
		_tokenIdTracker.increment();
		uint256 newId = _tokenIdTracker.current();
		_safeMint(msg.sender, newId);
