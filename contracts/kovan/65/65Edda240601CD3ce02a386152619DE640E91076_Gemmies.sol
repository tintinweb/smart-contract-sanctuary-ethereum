...
contract Gemmies is ERC20, Ownable {
    ...
    function _getEcoSystemBalance(address user) 
        ...
        returns (uint256) {
            ...
            uint256 positiveBalance = stakingProvider.getAccumulatedGemmies(user) + deposit;
    ...
    function spendEcosystemBalance(uint256 amount, address user) external onlyOperator {
        require(_getEcoSystemBalance(user) >= amount, "Cannot transfer more than available");
    ...
    function registerAttack(address attacker, address victim, uint256 amount) external onlyOperator {
        uint256 ecoSystemDataSender = ecoSystemBalance[attacker];
        ...
        ecoSystemBalance[attacker] = _getUpdatedEcoSystemBalance(depositedSender, spentSender + amount);
        ecoSystemBalance[victim] = _getUpdatedEcoSystemBalance(depositedReceiver + amount, spentReceiver);
    }
