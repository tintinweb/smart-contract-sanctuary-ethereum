...
abstract contract AbstractComptroller is OwnableUpgradeable{
    ...
    mapping(address => bool)  public repayBorrowGuardianPaused;
    mapping(address => bool)  public transferGuardianPaused;
    mapping(address => bool)  public seizeGuardianPaused;
    ...
    /**
     * @notice Supply caps enforced by mintAllowed for each pToken address. Defaults to zero which corresponds to unlimited minting.
     */
...
contract Comptroller is AbstractComptroller, ComptrollerErrorReporter, ExponentialNoError {
     ...
     */
    function addToMarketInternal(MarketTokenInterface marketToken, address borrower) internal returns (Error) {
    ...
    function mintAllowed(address marketToken, address minter, uint mintAmount) external returns (uint) {
        ...
        require(!mintGuardianPaused[marketToken], "mint is paused");
        ...
        if (!markets[marketToken].accountMembership[minter]) {
            ...
            Error err = addToMarketInternal(MarketTokenInterface(msg.sender), minter);
    ...
    function redeemAllowed(address marketToken, address redeemer, uint redeemTokens) external returns (uint) {
        // Pausing is a very serious situation - we revert to sound the alarms
        require(!redeemGuardianPaused[marketToken], "redeem is paused");
    ...
    function redeemAllowedInternal(address marketToken, address redeemer, uint redeemTokens) internal view returns (uint) {
        ...
        (Error err, , uint shortfall) = getHypotheticalAccountLiquidityInternal(redeemer, MarketTokenInterface(marketToken), redeemTokens, 0);
        if (err != Error.NO_ERROR) {
            return uint(err);
        }
        if (shortfall > 0) {
    ...
    function borrowAllowed(address marketToken, address borrower, uint borrowAmount) external returns (uint) {
        // Pausing is a very serious situation - we revert to sound the alarms
        require(!borrowGuardianPaused[marketToken], "borrow is paused");
        ...
        if (!markets[marketToken].accountMembership[borrower]) {
            ...
            Error _err = addToMarketInternal(MarketTokenInterface(msg.sender), borrower);
        ...
        (Error err, , uint shortfall) = getHypotheticalAccountLiquidityInternal(borrower, MarketTokenInterface(marketToken), 0, borrowAmount);
        if (err != Error.NO_ERROR) {
            return uint(err);
        }
        if (shortfall > 0) {
    ...
    function repayBorrowAllowed(
        ...
        // Pausing is a very serious situation - we revert to sound the alarms
        require(!repayBorrowGuardianPaused[marketToken], "repayborrow is paused");
    ...
    function seizeAllowed(
        ...
        // Pausing is a very serious situation - we revert to sound the alarms
        require(!seizeGuardianPaused[marketTokenCollateral] && !seizeGuardianPaused[marketTokenBorrowed] , "seize is paused");
    ...
    function transferAllowed(address marketToken, address src, address dst, uint transferTokens) external returns (uint) {
        // Pausing is a very serious situation - we revert to sound the alarms
        require(!transferGuardianPaused[marketToken], "transfer is paused");
        ...
        if (!markets[marketToken].accountMembership[dst]) {
            require(msg.sender == marketToken, "sender must be marketToken");
            Error err = addToMarketInternal(MarketTokenInterface(msg.sender), dst);
            if (err != Error.NO_ERROR) {
                return uint(err);
            }
    ...
    function getAccountLiquidity(address account) public view returns (uint, uint, uint) {
        (Error err, uint liquidity, uint shortfall) = getHypotheticalAccountLiquidityInternal(account, MarketTokenInterface(address(0)), 0, 0);
    ...
    function getHypotheticalAccountLiquidity(
        ...
        (Error err, uint liquidity, uint shortfall) = getHypotheticalAccountLiquidityInternal(account, MarketTokenInterface(marketTokenModify), redeemTokens, borrowAmount);
        return (uint(err), liquidity, shortfall);
    }
    ...
    function _setCloseFactor(uint newCloseFactorMantissa) external returns (uint) {
        // Check caller is owner
    	require(msg.sender == owner(), "only owner can set close factor");
    ...
    /**
      ...
      */
    function _setCollateralFactor(MarketTokenInterface marketToken, uint newCollateralFactorMantissa) external returns (uint) {
        // Check caller is owner
        if (msg.sender != owner()) {
    ...
    /**
      ...
      */
    function _supportMarket(MarketTokenInterface marketToken) external returns (uint) {
        if (msg.sender != owner()) {
    ...
    function _setDistributioner(address newDistributioner) public returns (uint) {
        ...
        require(msg.sender == owner(),"only owner can set distributioner");
    ...
    function _setMintPaused(MarketTokenInterface marketToken, bool state) public returns (bool) {
        ...
        mintGuardianPaused[address(marketToken)] = state;
        emit ActionPaused(marketToken, "Mint", state);
        return state;
    }

    function _setRedeemPaused(MarketTokenInterface marketToken, bool state) public returns (bool) {
        require(markets[address(marketToken)].isListed, "cannot pause a market that is not listed");
        require(msg.sender == pauseGuardian || msg.sender == owner(), "only pause guardian and owner can pause");

        redeemGuardianPaused[address(marketToken)] = state;
        emit ActionPaused(marketToken, "Redeem", state);
        return state;
    }

    function _setBorrowPaused(MarketTokenInterface marketToken, bool state) public returns (bool) {
        require(markets[address(marketToken)].isListed, "cannot pause a market that is not listed");
        require(msg.sender == pauseGuardian || msg.sender == owner(), "only pause guardian and owner can pause");

        borrowGuardianPaused[address(marketToken)] = state;
        emit ActionPaused(marketToken, "Borrow", state);
        return state;
    }

    function _setRepayBorrowPaused(MarketTokenInterface marketToken, bool state) public returns (bool) {
        require(markets[address(marketToken)].isListed, "cannot pause a market that is not listed");
        require(msg.sender == pauseGuardian || msg.sender == owner(), "only pause guardian and owner can pause");

        repayBorrowGuardianPaused[address(marketToken)] = state;
        emit ActionPaused(marketToken, "RepayBorrow", state);
        return state;
    }
    
    function _setTransferPaused(MarketTokenInterface marketToken, bool state) public returns (bool) {
         require(markets[address(marketToken)].isListed, "cannot pause a market that is not listed");
        require(msg.sender == pauseGuardian || msg.sender == owner(), "only pause guardian and owner can pause");


        transferGuardianPaused[address(marketToken)] = state;
        emit ActionPaused(marketToken, "Transfer", state);
        return state;
    }

    function _setSeizePaused(MarketTokenInterface marketToken, bool state) public returns (bool) {
         require(markets[address(marketToken)].isListed, "cannot pause a market that is not listed");
        require(msg.sender == pauseGuardian || msg.sender == owner(), "only pause guardian and owner can pause");


        seizeGuardianPaused[address(marketToken)]  = state;
    ...
    function isDeprecated(MarketTokenInterface marketToken) public view returns (bool) {
        return
            ...
            borrowGuardianPaused[address(marketToken)] == true && 
    ...
    function _setLiquidateWhiteAddresses(address[] memory _liquidateWhiteAddresses) public  {
        ...
        require(msg.sender == owner(),"only owner can set liquidateWhiteAddresses");
