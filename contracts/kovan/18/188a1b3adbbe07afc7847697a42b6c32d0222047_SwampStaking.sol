...
/// @author @ryeshrimp
/// @notice By staking tokens to this contract, you'll be rewarded with ERC20 on a per day basis.
...
contract SwampStaking is Initializable, ERC721Holder, ReentrancyGuardUpgradeable, OwnableUpgradeable, PausableUpgradeable {
    ...
    // a mapping from an address to whether or not it pause NFTs
    ...
    mapping(uint256 => uint256) public swampersPausedUntil;
    mapping(uint256 => uint256) public creaturesPausedUntil;
    ...
    function initialize(
      ...
      swamperRewardPerDay = 5;
      creatureRewardPerDay = 15;
    ...
    function withdrawSwampers(uint256[] memory tokenIds) external nonReentrant updateReward(msg.sender) {
        ...
        for (uint256 i; i < tokenIds.length;) {
          ...
          require(block.timestamp > swampersPausedUntil[tokenIds[i]], "SwampStaking: Token is paused");
          ...
          delete swampersPausedUntil[tokenIds[i]];
          unchecked { i++; }
        }
    ...
    function withdrawCreatures(uint256[] memory tokenIds) external nonReentrant updateReward(msg.sender) {
        ...
        for (uint256 i; i < tokenIds.length;) {
          ...
          require(block.timestamp > creaturesPausedUntil[tokenIds[i]], "SwampStaking: Token is paused");
          // Transfer NFTs back to the owner
          ...
          delete creaturesPausedUntil[tokenIds[i]];
          unchecked { i++; }
        }
