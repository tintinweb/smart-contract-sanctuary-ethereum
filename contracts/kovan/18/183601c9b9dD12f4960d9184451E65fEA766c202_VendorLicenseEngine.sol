    ...
    ILicenseEngine
{
    ...
    function safeMint(
        ...
        if (_discount < 0 || _discount > 1000000) revert InvalidDiscount();
    ...
    function setDiscount(uint256 _lic, uint256 _discount) external onlyOwner {
        if (!licenses[_lic].exists) revert LicenseNotFound();
        if (_discount < 0 || _discount > 1000000) revert InvalidDiscount();
        licenses[_lic].discount = _discount;
    }

    function setExpiry(uint256 _lic, uint256 _expiry) external onlyOwner {
        if (!licenses[_lic].exists) revert LicenseNotFound();
        if (_expiry <= block.timestamp) revert InvalidDiscount();
    ...
    function incrementCurrentPoolCount(uint256 _lic)
        external
    {
        if (msg.sender != factory) revert NotFacotry();
        if (!licenses[_lic].exists) revert LicenseNotFound();
