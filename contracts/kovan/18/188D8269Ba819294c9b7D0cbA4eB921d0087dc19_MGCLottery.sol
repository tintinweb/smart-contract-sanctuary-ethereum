...
contract MGCLottery is Ownable {
    ...
    struct User {
        ...
        uint[] rewards;
        Spin[] spins;
        mapping(uint => RewardIno) rewardInfo;
    }
    ...
    function getUserReward(address _user) external view returns(RewardIno[] memory list) {
        uint length = users[_user].rewards.length;
        list = new RewardIno[](length);
    ...
    function spin(uint _pid, uint nonce, uint number, uint amount) public {
        ...
        if(result) {
            ...
            user.rewardInfo[block.timestamp] = RewardIno(reward, block.timestamp + p.lockTime, false);
            setTop10Winner();
        }
    }
