...
contract UtilToken is ERC721AUpgradeable, BlackListUpgradeable,  PausableUpgradeable, OwnableUpgradeable, ReentrancyGuardUpgradeable, IUtilToken{
    ...
    function mintSanityCheck(address callerAddress, bytes32[] calldata _merkleProof) internal {
        require(block.timestamp < batchCollection[batchCollectionArrayLength()].endTime && block.timestamp > batchCollection[batchCollectionArrayLength()].startTime, "Currently no batch is active");
        ...
        if(!batchCollection[batchCollectionArrayLength()].isSaleActive){
    ...
    function batchCollectionLength() public view returns(uint256){
