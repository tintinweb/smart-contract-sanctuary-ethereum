...
/**
 * @title Drippie
 * @notice Drippie goes brrr.
 */
contract Drippie is RetroReceiver {
    /**
    ...
    function drip(string memory _name) public {
        ...
        require(
            state.config.dripcheck.check(state.config.recipient, state.config.params),
            "Drippie: dripcheck failed so drip is not yet ready to be triggered"
        );

        state.last = block.timestamp;
        (bool success, ) = state.config.recipient.call{ value: state.config.value }(
            state.config.action
        );
