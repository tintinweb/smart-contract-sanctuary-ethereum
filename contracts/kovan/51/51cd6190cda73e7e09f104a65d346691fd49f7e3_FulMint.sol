...
interface interfaceful{
    ...
    function toLockId(uint256 _id, uint256 duration)external;
    function transferFrom(address from,address to,uint256 tokenId) external;
    function safeTransferFrom(address from,address to,uint256 tokenId) external;
    function safeTransferFrom(address from,address to,uint256 tokenId,bytes memory _data) external;
    function burnNFT(uint256 tokenId)external;
    function getNFTCount() external view returns (uint256);
    function ownerOfNft(uint256 _id)external view returns(address);
    function rarityOfNFT(uint256 _id)external view returns(uint256);
    function positionOfNFT(uint256 _id)external view returns(uint256);
...
contract FulMint is Ownable{
    ...
    mapping(uint256 => uint256) public priceOfPack;
    mapping(address => uint256) noOfBurnedNFT;
    ...
    function buyPackOfNFT( bytes32[] memory _leaf, bytes32[][] memory _proof, uint256 _noOfPack,uint256[] memory nftIDs,string[] memory nftName, string[] memory tokenUri, string[] memory nftPosition,string[] memory nftRarity)public isWhitelisted{
        ...
        // uint256 amount = _noOfPack.mul(priceOfPack[1]);
        // fulToken.safeTransferFrom(msg.sender,address(this), amount);

        //uint256 totalNFT = _noOfPack.mul(3);
    ...
    function removeWhiteListedUser(address[]memory _addressToRemove)public onlyOwner{
      for(uint256 i=0;i<_addressToRemove.length;i++){
    ...
    function addUserToBlackList(address[] memory _addressToBlackList) public onlyOwner {
      for(uint256 i=0;i<_addressToBlackList.length;i++){
    ...
    // function buyPack(uint256 _noOfPack)public{
    //     require(_noOfPack <= 13, "You can't buy more than 13 packs");
    //     uint256 amount = _noOfPack.mul(priceOfPack[1]);
    //     fulToken.safeTransferFrom(msg.sender,address(this), amount);
    ...
    function toLockNFT(uint256[] memory _id, uint256 _duration)public { 
    ...
    function tansferNFT(address to, uint256 _id)public  {
