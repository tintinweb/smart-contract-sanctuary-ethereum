...
contract LegionNFT is Context, IERC721, Ownable {
    ...
    function hunt(uint256 tokenId, uint8 monsterId) external returns(bool) {
    ...
    function sendToMarketplace(uint256 tokenId, uint256 price) external {
        ...
        require(canHunt(tokenId), "You can sell only available legion to hunt");
...
contract BeastNFT is Context, IBeastNFT, Ownable {
    ...
    mapping (address => uint256[]) tokensOnMarket;
    ...
    function sendToMarketplace(uint256 tokenId, uint256 price) external {
        ...
        tokensOnMarket[msg.sender].push(tokenId);
    }

    function getTokenIdsOnMarket(address _address) external view returns(uint256[] memory) {
        return tokensOnMarket[_address];
    }
