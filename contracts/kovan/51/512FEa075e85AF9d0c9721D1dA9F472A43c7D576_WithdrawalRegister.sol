...
contract WithdrawalRegister is IWithdrawalRegister {
    ...
    function claimWithdrawal(address token, address account, uint256 amount) onlyPrecog external {
        Register memory lastRegister = register[token][account];
...
interface IWithdrawalRegister {
    ...
    function claimWithdrawal(address token, address account, uint256 amount) external;
}
