...
contract Stablecoin is ERC20, ReentrancyGuard {
    ...
    uint256 public maxDebtCeiling;
    uint256 public minDebtCeiling;
    uint256 public maxVaultCeiling;
    ...
    mapping(uint8 => uint) public freezeTime;
    ...
    mapping(address => mapping(uint8 => uint64)) public freeze;
    ...
    constructor(
        ...
        uint256 maximumDebtCeiling,
        uint256 minimumDebtCeiling,
        uint256 maximumVaultCeiling,
        string memory name,
        ...
        require(maximumDebtCeiling <= totalDebtCeiling);
        require(minimumDebtCeiling <= maximumDebtCeiling);

        debtCeiling = totalDebtCeiling;
        maxDebtCeiling = maximumDebtCeiling;
        maxVaultCeiling = maximumDebtCeiling * maximumVaultCeiling / 1e6;
        minDebtCeiling = minimumDebtCeiling;

        freezeTime[uint8(FreezeType.VAULT)] = 3 days;
        freezeTime[uint8(FreezeType.COLLATERAL)] = 0;
        freezeTime[uint8(FreezeType.DEBT)] = 1 days;
        freezeTime[uint8(FreezeType.REPAY)] = 3 days;
    ...
    function createVault() external returns (uint256) {
        ...
        require(freeze[msg.sender][uint8(FreezeType.VAULT)] <= block.timestamp);

        vaultExistence[id] = true;
        vaultOwner[id] = msg.sender;
        freeze[msg.sender][uint8(FreezeType.VAULT)] = uint64(block.timestamp + freezeTime[uint8(FreezeType.VAULT)]);
    ...
    function depositCollateral(uint256 vaultID) external payable onlyVaultOwner(vaultID) {
        ...
        require(freeze[owner][uint8(FreezeType.COLLATERAL)] <= block.timestamp);

        freeze[owner][uint8(FreezeType.COLLATERAL)] = uint64(block.timestamp + freezeTime[uint8(FreezeType.COLLATERAL)]);
    ...
    function borrowToken(uint256 vaultID, uint256 amount) external onlyVaultOwner(vaultID) {
        ...
        require(newDebt <= maxVaultCeiling);
        require(isValidCollateral(vaultCollateral[vaultID], newDebt), "Borrow would put vault below minimum collateral percentage");
        require(freeze[owner][uint8(FreezeType.DEBT)] <= block.timestamp);
        
        freeze[owner][uint8(FreezeType.DEBT)] = uint64(block.timestamp + freezeTime[uint8(FreezeType.DEBT)]);
        freeze[owner][uint8(FreezeType.REPAY)] = uint64(block.timestamp + freezeTime[uint8(FreezeType.REPAY)]);

        uint256 _openingFee = (amount.mul(openingFee).mul(getTokenPriceSource())).div(getEthPriceSource().mul(10000));        

        vaultDebt[vaultID] = newDebt;
        vaultCollateral[vaultID]=vaultCollateral[vaultID].sub(_openingFee);
        vaultCollateral[treasury]=vaultCollateral[treasury].add(_openingFee);
    ...
    function payBackToken(uint256 vaultID, uint256 amount) external onlyVaultOwner(vaultID) {
        ...
        require(freeze[vaultOwner[vaultID]][uint8(FreezeType.REPAY)] <= block.timestamp);
...
contract USDP is Stablecoin, Ownable {
    constructor(
        address ethPriceSourceAddress,
        uint256 minimumCollateralPercentage,
        uint256 totalDebtCeiling,
    ...
    function setMinDebtCeiling(uint minimumDebtCeiling) external onlyOwner() {
        require(minimumDebtCeiling < maxDebtCeiling);
        minDebtCeiling = minimumDebtCeiling;
    }

    function setMaxDebtCeiling(uint maximumDebtCeiling) external onlyOwner() {
        require(maximumDebtCeiling > minDebtCeiling);
        maxDebtCeiling = maximumDebtCeiling;
    }

    function setMaxVaultCeiling(uint maximumVaultCeiling) external onlyOwner() {
        require(maximumVaultCeiling <= 1e6);
        maxVaultCeiling = maxDebtCeiling * maximumVaultCeiling / 1e6;
    }
