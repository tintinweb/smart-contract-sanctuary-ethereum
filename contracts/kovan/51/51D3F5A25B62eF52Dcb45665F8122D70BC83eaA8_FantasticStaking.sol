...
contract FantasticStaking is ReentrancyGuard, Ownable {
    ...
    uint256 public constant rewardsDuration = 86400; // 86400 * 7; // 1 week
    ...
    constructor(address _stakingToken, address _stakingTokenReserve) Ownable() {
    ...
    function addReward(address _rewardsToken, address _distributor) public onlyOwner {
        require(
            ...
            "FantasticStaking::addReward: Invalid"
        );
    ...
    function approveRewardDistributor(
        ...
        require(
            rewardData[_rewardsToken].lastUpdateTime > 0,
            "FantasticStaking::approveRewardDistributor: Invalid"
        );
    ...
    function stake(uint256 amount, bool lock) external nonReentrant updateReward(msg.sender) {
        require(amount > 0, "FantasticStaking::stake: Cannot stake 0");
    ...
    function mint(address user, uint256 amount) external updateReward(user) {
        require(minters[msg.sender], "FantasticStaking::mint: Only minters allowed");
        totalSupply = totalSupply.add(amount);
        Balances storage bal = balances[user];
        bal.total = bal.total.add(amount);
    ...
    function withdraw(uint256 amount) public nonReentrant updateReward(msg.sender) {
        require(amount > 0, "FantasticStaking::withdraw: Cannot withdraw 0");
        Balances storage bal = balances[msg.sender];
        ...
        if (amount <= bal.unlocked) {
            ...
            require(
                ...
                "FantasticStaking::withdraw: Insufficient unlocked balance"
            );
            bal.unlocked = 0;
            ...
            for (uint256 i = 0; ; i++) {
                ...
                if (
                    ...
                    require(
                        bal.earned >= remaining,
                        "FantasticStaking::withdraw: Insufficient balance after penalty"
                    );
                    bal.earned = bal.earned.sub(remaining);
                    if (bal.earned == 0) {
        ...
        updateReward(address(0))
    {
        require(
            ...
            "FantasticStaking::notifyRewardAmount: Only reward distributors allowed"
        );
        require(reward > 0, "FantasticStaking::notifyRewardAmount: No reward");
    ...
    function recoverERC20(address tokenAddress, uint256 tokenAmount) external onlyOwner {
        require(
            ...
            "FantasticStaking::recoverERC20: Cannot withdraw staking token"
        );
        require(
            rewardData[tokenAddress].lastUpdateTime == 0,
            "FantasticStaking::recoverERC20: Cannot withdraw reward token"
        );
    ...
    function addMinter(address _newMinter) external onlyOwner {
        require(_newMinter != address(0), "FantasticStaking::addMinter: Invalid address");
        require(!minters[_newMinter], "FantasticStaking::addMinter: Minter existed");
    ...
    function removeMinter(address _minter) external onlyOwner {
        require(minters[_minter], "FantasticStaking::addMinter: Minter not found");
