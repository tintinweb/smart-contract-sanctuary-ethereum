...
contract Votes is Ownable {
  ...
  // constants
  uint minRewardAmount = 5e18;
  uint minLockAmount = 1e18;
  uint feePercent = 3;
  // for bsc
//  address public lockAsset = 0xEd8c8Aa8299C10f067496BB66f8cC7Fb338A3405;
  // fot bsctestnet
  // address public lockAsset = 0x55d398326f99059fF775485246999027B3197955;
  // for kovan
  address public lockAsset = 0x8CC5d395F3Dbe44d0D28D0abd5078a1D1ae058e6;
  ...
  event NewSurvey(uint eventId, uint creationTimestamp, address sender, string nftUrl, uint start, uint end, uint minVotes, uint maxVotes, uint rewardAmount, uint surveyType);
  ...
  function createSurvey (string memory nftUrl, uint startTimestamp, uint endTimestamp, uint minVotes, uint maxVotes, uint rewardAmount, uint surveyType)
    ...
    fees = fees.add(fee);
    {
      Survey memory newSurvey = Survey(msg.sender, nftUrl, startTimestamp, endTimestamp, minVotes, maxVotes, rewardAmount.sub(rewardAmount.mul(feePercent).div(100)), surveyType);
