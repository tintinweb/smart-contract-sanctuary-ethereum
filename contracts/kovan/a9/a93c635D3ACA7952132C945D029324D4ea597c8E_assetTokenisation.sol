...
contract assetTokenisation is ERC1155, Ownable {
    ...
    uint256 Id;
    IERC20 USDC = IERC20(0xABf205c2E99A4B65389E82661aF82A327B09858f);   //For Kovan Testnet   //0x2519f6FC924BE4ac916152059C46E1bFeB5CfFcC
    ...
    // function approval(string memory _userId) public hasProperty(_userId) returns(bool)  {
    //     require(msg.sender == userDetails[_userId].user,"");
    //     uint256 _amount = USDC.balanceOf(msg.sender);
    //     USDC.approve(address(this), _amount);     //owner()
    //     return true;
    // }
    ...
    function addProperty(string memory _userId, address _user, string memory _propertyId, uint256 _propertyPrice,
        ...
        _mint(owner(), propertyDetails[_propertyId].propertyTokenId, _investmentPortion, "" );    //Create Property specific ERC20 tokens based on ERC1155 standard
    ...
    function transferDividend(string memory _propertyId) public onlyOwner { 
        require(USDC.balanceOf(userDetails[propertyOwner[_propertyId]].user) >= propertyDetails[_propertyId].rentAmount,"Insufficient Property Owner balance");
        require(USDC.allowance(userDetails[propertyOwner[_propertyId]].user,address(this))>=propertyDetails[_propertyId].rentAmount,"Insufficient Approved Balance");
        ...
        USDC.transferFrom(userDetails[propertyOwner[_propertyId]].user,owner(),_maintenanceFee);
        ...
        for(uint256 i = 0; i< investors[_propertyId].length; i++) {
            if(isInvestor[_propertyId][investors[_propertyId][i]]) {
                uint256 investment = propertywiseInvestment[investors[_propertyId][i]][_propertyId];
                uint256 dividendAmount =  (_rentAmount* investment)/_investmentPortion  ;
                uint256 recordedTime = currentTime - investmentTime[investors[_propertyId][i]][_propertyId];
                uint256 _dividendAmount = (recordedTime * dividendAmount)/ epochTime;
                investmentTime[investors[_propertyId][i]][_propertyId] = block.timestamp;

                USDC.transferFrom(userDetails[propertyOwner[_propertyId]].user,userDetails[investors[_propertyId][i]].user , _dividendAmount);
            }
        }
    ...
    function approval(string memory _userId) public {
        require(msg.sender == userDetails[_userId].user,"Caller is not the Investor");
        setApprovalForAll(owner(), true);
    }

    function withdrawInvestment(string memory _investoruserId, string memory _propertyId, uint256 _amount) public onlyOwner{
