...
contract PrecogV5 is IPrecogV5 {
    ...
    function availableDepositedAmount(address token, address account) 
        ...
        amount = depositedTimestampOf[token][account] > block.timestamp ? availableAmount[token][account] : 0;
        Cycle memory _firstTradingCycle = tradingCycles[token][0];
        nextFundingStartTime = PrecogV5Library._calculateNextFundingTimestamp(_firstTradingCycle, lastFundingDuration[token]);
        lastFundingStartTime = nextFundingStartTime - lastFundingDuration[token];
    }
    ...
    function _withdrawBeforeFundingTime(address _account, address _to, address _token, uint _amount, uint _fee) internal {
        ...
        require(_amount <= availableWithdrawalAmount, "PrecogV5: INVESTMENT_HAS_ALREADY_STARTED");
