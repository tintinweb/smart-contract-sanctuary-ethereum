...
contract Token is Context, IERC20, Ownable {
    ...
    address public _marketingWallet = 0x665884b278Cc3420E1721F2142098620100e1D15; //change
    address public  _treasuryWallet = 0x665884b278Cc3420E1721F2142098620100e1D15; //change
    address public _lpWallet = 0x665884b278Cc3420E1721F2142098620100e1D15; //change

    uint8 public constant _maxTotalFee = 10;
    uint8 public _totalFee = 10;
    uint8 public _liquidityFee = 4;
    uint8 public _marketingFee = 4;
    uint8 public _treasuryFee = 2;

    //address constant public routerAddress = 0x10ED43C718714eb63d5aA57B78B54704E256024E; //pancake
    ...
    // logic for first 24hours
    uint256 private _firstBlock;

    //5minutes 0.5% tx limit
    uint256 private _limitDuration1 = 5 * 60;
    uint256 private constant _maxTx1 = 5000000000 * 10**18; // fees
    //24hours  1.5% tx limit
    uint256 private _limitDuration2 = 24 * 60 * 60;
    uint256 private constant _maxTx2 = 15000000000 * 10**18; // fees
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual {
        ...
        //first 24hour tx restrictions
        if(block.timestamp < _firstBlock + _limitDuration2){
            if(recipient != uniswapV2Pair && recipient != address(this) && recipient != address(uniswapV2Router)){
                require(amount <= _maxTx2);      
            }

            if(block.timestamp < _firstBlock + _limitDuration1){
                if(recipient != uniswapV2Pair && recipient != address(this) && recipient != address(uniswapV2Router)){
                require(amount <= _maxTx1);      
                }
            }
        }
    ...
    function setAllFees(uint8 newMarketingFee, uint8 newTreasuryFee, uint8 newLiquidityFee) external onlyOwner {
        // we can't set fees higher than 10% (_maxTotalFee is a constant).
