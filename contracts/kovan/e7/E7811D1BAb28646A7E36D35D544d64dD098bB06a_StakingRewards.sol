...
contract StakingRewards is Initializable {
  ...
  uint128 public earmarkIncentive;

  /// @notice operation fee
  uint128 public operationFee;
  ...
  function initialize(
    ...
    earmarkIncentive = 100; // 100 means 1%
    operationFee = 1900; 

    DepositToken dtoken = new DepositToken(address(this));
    DEPOSIT_TOKEN_ADDR = address(dtoken);
    BALRewardPool pool = new BALRewardPool(address(dtoken), address(BAL), address(this)); 
  ...
  **/  
  ...
  function balTokenBalance() external view returns (uint256) {
    return IERC20Upgradeable(BAL).balanceOf(address(this));
  }
