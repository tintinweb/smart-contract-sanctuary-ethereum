 ...
 * @author MikhilMC
 */
contract StakeREMIT is ReentrancyGuard {
    ...
    uint256 public constant DAILY_APR = 2;              // 0.2%
    uint256 public constant STAKING_FEE = 15;           // 1.5%
    uint256 public constant UNSTAKING_FEE = 5;          // 0.5%
    uint256 public constant STAKING_REWARDS_FEE = 5;    // 5%
    ...
    uint256 public totalTokensStakedInContract;
    ...
    function stakeAmount(uint256 _amount) public nonReentrant onlyAfterStakingStarted {
        require(
            !msg.sender.isContract(),
            "ERROR: You can not stake in this smart contract from another contract"
        );
        ...
        uint256 userStakingAmount = _amount.sub(userStakingFee);
        ...
        if(!_isAccountAvailable(msg.sender)) {
            ...
            accountDatas[msg.sender].totalDepositedAmount =
                accountDatas[msg.sender].totalDepositedAmount.add(
                    _amount
                );
            accountDatas[msg.sender].totalStakedAmount =
                accountDatas[msg.sender].totalStakedAmount.add(
                    userStakingAmount
                );
            accountDatas[msg.sender].totalStakingFee =
                accountDatas[msg.sender].totalStakingFee.add(
                    userStakingFee
                );
        }
        ...
        if(_isUserStaking(msg.sender)) {
            ...
            uint256 rewardGained = rewardAmount.sub(rewardFee);

            stakingDatas[msg.sender].rewardGained = rewardGained;
            stakingDatas[msg.sender].rewardFee = rewardFee;

            stakingDatas[msg.sender].depositAmount = 
                stakingDatas[msg.sender].depositAmount.add(_amount);
            stakingDatas[msg.sender].stakingAmount = 
                stakingDatas[msg.sender].stakingAmount.add(userStakingAmount);
            stakingDatas[msg.sender].stakingFee = 
                stakingDatas[msg.sender].stakingFee.add(userStakingFee);
        ...
        stakingDatas[msg.sender].numberOfStakings = 
            stakingDatas[msg.sender].numberOfStakings.add(1);

        totalTokensStakedInContract = totalTokensStakedInContract.add(
            userStakingAmount
        );
     ...
     * - This function can only be executed on or after the unlock time.
     *
     * - 0.5% of the current staked amount will be taken as unstaking fee.
     * - The remaining amount will be sent to the user.
     * - Every day user will get 0.2% of the initial staking amount as reward.
     * - 5% of the total reward gained will be taken as reward fee.
     * - The remaining reward will be sent to the user.
     */
    function unstakeAmount() public nonReentrant onlyAfterStakingStarted {
        require(
            !msg.sender.isContract(),
            "ERROR: You can not unstake from this smart contract from another contract"
        );
        ...
        uint256 availableStakedAmount = stakedAmount.sub(userUnstakingFee);
        ...
        uint256 totalAmountAvailable = availableStakedAmount.add(
            stakingDatas[msg.sender].rewardGained
        );
        ...
        accountDatas[msg.sender].totalUnstakingFee = 
            accountDatas[msg.sender].totalUnstakingFee.add(
                userUnstakingFee
            );
        accountDatas[msg.sender].totalRewardAmount = 
            accountDatas[msg.sender].totalRewardAmount.add(
                rewardGained
            );
        accountDatas[msg.sender].totalRewardFee = 
            accountDatas[msg.sender].totalRewardFee.add(
                rewardFee
            );

        totalTokensStakedInContract = totalTokensStakedInContract.sub(
            stakedAmount
        );
     ...
     * - The account must be staking currently.
     */
    function emergencyWithdrawal() nonReentrant public onlyAfterStakingStarted {
        require(
            !msg.sender.isContract(),
            "ERROR: You can not withdraw token of this smart contract from another contract"
        );
        ...
        uint256 rewardFee = _getStakingRewardsFee(rewardAmount);
        uint256 rewardGained = rewardAmount.sub(rewardFee);
        uint256 totalAmount = stakedAmount.add(rewardGained);
    ...
    */
    function isUserStaking(address _account) external view returns(bool) {
        return _isUserStaking(_account);
    }

    /**@dev Function to get the current deposited amount by a user.
     *
     * @param _account address of a user.
    */
    function getDepositAmount(address _account) external view returns(uint256) {
        return stakingDatas[_account].depositAmount;
    }

    /**@dev Function to get the current staking amount by a user.
     *
     * @param _account address of a user.
    */
    function getStakingAmount(address _account) external view returns(uint256) {
        return stakingDatas[_account].stakingAmount;
    }

    /**@dev Function to get the current staking fee amount paid by a user.
     *
     * @param _account address of a user.
    */
    function getStakingFee(address _account) external view returns(uint256) {
        return stakingDatas[_account].stakingFee;
    }

    /**@dev Function to get the number of stakings done by a user currently before unstaking.
     *
     * @param _account address of a user.
    */
    function getNumberOfStakings(address _account) external view returns(uint256) {
        return stakingDatas[_account].numberOfStakings;
    }

    /**@dev Function to get the current unstaking amount paid by a user.
     *
     * @param _account address of a user.
    */
    function getUnstakingFee(address _account) external view returns(uint256) {
        return stakingDatas[_account].unstakingFee;
    }

    /**@dev Function to get the current gained reward amount by a user.
     *
     * @param _account address of a user.
    */
    function getRewardGained(address _account) external view returns(uint256) {
        return stakingDatas[_account].rewardGained;
    }

    /**@dev Function to get the current reward fee paid by a user.
     *
     * @param _account address of a user.
    */
    function getRewardFee(address _account) external view returns(uint256) {
        return stakingDatas[_account].rewardFee;
    }

    /**@dev Function to get the current staking starting time by a user.
     *
     * @param _account address of a user.
    */
    function getStakingStartingTime(address _account) external view returns(uint256) {
        return stakingDatas[_account].stakingStartingTime;
    }

    /**@dev Function to get the current unstaking unlock time by a user.
     *
     * @param _account address of a user.
    */
    function getUnlockingTime(address _account) external view returns(uint256) {
        return stakingDatas[_account].unlockingTime;
    }

    /**@dev Function to get the current staking ending time by a user.
     *
     * @param _account address of a user.
    */
    function getStakingEndingTime(address _account) external view returns(uint256) {
        return stakingDatas[_account].stakingEndingTime;
    }

    /**@dev Function to get the current staking duration in days by a user.
     *
     * @param _account address of a user.
    */
    function getStakingDurationInDays(address _account) external view returns(uint256) {
        return stakingDatas[_account].stakingDurationInDays;
    }
     ...
     * @param _account address of a user.
    */
    function isAccountAvailable(address _account) external view returns(bool) {
        return _isAccountAvailable(_account);
    }

    /**@dev Function to get the total deposited amount by a user.
     *
     * @param _account address of a user.
    */
    function getTotalDepositedAmount(address _account) external view returns(uint256) {
        return accountDatas[_account].totalDepositedAmount;
    }

    /**@dev Function to get the total staked amount by a user.
     *
     * @param _account address of a user.
    */
    function getTotalStakedAmount(address _account) external view returns(uint256) {
        return accountDatas[_account].totalStakedAmount;
    }

    /**@dev Function to get the total staking fee amount paid by a user.
     *
     * @param _account address of a user.
    */
    function getTotalStakingFee(address _account) external view returns(uint256) {
        return accountDatas[_account].totalStakingFee;
    }

    /**@dev Function to get the total unstaking fee amount paid by a user.
     *
     * @param _account address of a user.
    */
    function getTotalUnstakingFee(address _account) external view returns(uint256) {
        return accountDatas[_account].totalUnstakingFee;
    }

    /**@dev Function to get the total reward gained by a user.
     *
     * @param _account address of a user.
    */
    function getTotalRewardAmount(address _account) external view returns(uint256) {
        return accountDatas[_account].totalRewardAmount;
    }

    /**@dev Function to get the total reward fee amount paid by a user.
     *
     * @param _account address of a user.
    */
    function getTotalRewardFee(address _account) external view returns(uint256) {
        return accountDatas[_account].totalRewardFee;
    }
    ...
    */
    function _setUnlockingTime(uint256 _time) private pure returns(uint256) {
        return _time.add(MINIMUM_LOCK_TIME);
    }
    ...
    */
    function _getStakingFee(uint256 _amount) private pure returns(uint256) {
        return _amount.mul(STAKING_FEE).div(DENOMINATOR).div(PERCENTAGE);
    }
    ...
    */
    function _getUnstakingFee(uint256 _amount) private pure returns(uint256) {
        return _amount.mul(UNSTAKING_FEE).div(DENOMINATOR).div(PERCENTAGE);
    }
    ...
    */
    function _getStakingRewardsFee(uint256 _amount) private pure returns(uint256) {
        return _amount.mul(STAKING_REWARDS_FEE).div(PERCENTAGE);
    }
