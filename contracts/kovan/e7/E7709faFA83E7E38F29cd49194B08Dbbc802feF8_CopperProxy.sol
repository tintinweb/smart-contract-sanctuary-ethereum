...
contract CopperProxy is Ownable {
    ...
    function exitPool(address pool, uint256 maxBPTTokenOut, bool isStandardFee) external onlyPoolOwner(pool) {
        ...
        distributeTokens(pool, mainToken, fundToken, isStandardFee);
    }

    function distributeTokens(address pool, address mainToken, address fundToken, bool isStandardFee) internal {
        ...
        uint256 totalPlatformAccessFeeAmount = ((fundTokenBalance * platformAccessFeeBPS) / 10_000);
        uint256 remainingFundBalance = fundTokenBalance - totalPlatformAccessFeeAmount; // todo safe substraction ?
