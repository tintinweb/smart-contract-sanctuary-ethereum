...
contract NODERewardManagement {
    ...
    function setRemoveNodeBonus(uint256 _price) external onlySentry {
        removeNodeBonus = _price;
    }
...
contract VOCToken is ERC20, Ownable {
    ...
    constructor(address uniV2Router, address _usdcAddres)
        ERC20("VOCYield.io", "VOC")
    {
    ...
    function setRemoveNodePrice(uint256 _price) external onlyOwner {
        ...
        require(nodePrice > _price, "removePrice is bigger than NodePrice");
        removeNodePrice = _price;
        // set removeNodeBonus
        uint256 _removeNodeBonus = nodePrice - removeNodePrice;
        nodeRewardManager.setRemoveNodeBonus(_removeNodeBonus);
    }
    ...
    function swapTokensForUSDC(address destination, uint256 tokenAmount)
        private
    {
    ...
    function changeNodePrice(uint256 newNodePrice) public onlyOwner {
        require(
            newNodePrice > removeNodePrice,
            "RemovePrice is bigger than NodePrice"
        );
        nodeRewardManager._changeNodePrice(newNodePrice);
        uint256 _removeNodeBonus = newNodePrice - removeNodePrice;
        nodeRewardManager.setRemoveNodeBonus(_removeNodeBonus);
    }
