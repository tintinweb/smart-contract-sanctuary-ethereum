...
// File: contracts/avalanche/ChfryToken.sol
...
contract ChfryToken is ERC20, UpgradableProduct {
	using SafeMath for uint256;
...
// File: contracts/avalanche/ChfryFactory.sol
...
//an=a1+(n-1)*d
//Sn=n*a1+(n(n-1)*d)/2
contract ChfryFactory is UpgradableProduct, ReentrancyGuard {
	using SafeMath for uint256;
	...
	constructor(address token_, uint256 weekTimestamp_) public {
		...
		token = ChfryToken(token_);
	}

	function setChfryToken(address token_) external requireImpl {
		token = ChfryToken(token_);
	}
    ...
    uint256 minted,
		...
		// weight
		poolInfo[ConfigNames.STAKE] = Pool(stake_, TOTAL_WEIGHT, minted);
		initialized = true;
