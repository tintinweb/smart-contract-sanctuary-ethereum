...
contract Masterchef {
    ...
    uint256 public rewardPerBlock = 10000; // thưởng mỗi block
    ...
    function withdraw(uint256 _amount) public {
        UserInfo storage user = userInfo[msg.sender];
        require(user.amount > 0 && _amount <= user.amount, "reject");
        updatePool();
           if (user.amount > 0) {
            uint256 pending = user.amount * accRewardPerShare - user.rewardDebt;
            if (pending > 0) {
    ...
    function claimRdx(uint256 _amount) public {
        ...
        require((user.reward + pending) >= _amount, "reject");

        RDX.transfer(msg.sender, _amount);
        user.reward -= _amount - pending;
