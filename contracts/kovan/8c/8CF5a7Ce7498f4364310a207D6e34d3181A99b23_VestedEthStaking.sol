...
/// @notice The Vested ETH staking contract. It allows for a fixed PRIME token
/// rewards distributed evenly across all stakers per block.
contract VestedEthStaking is MasterPrimeStaking, ReentrancyGuard {
    struct VestingInfo {
    ...
    function pendingEth(uint256 pid, address user)
        external
        view
        returns (uint256 pending)
    {
        DepositInfo storage _deposit = depositInfo[pid][user];
        EthPoolInfo storage _ethPoolInfo = ethPoolInfo[pid];
        VestingInfo storage _vesting = vestingInfo[pid][user];
    ...
    function deposit(uint256 pid, uint256 amount) public override {
        VestingInfo storage _vesting = vestingInfo[pid][msg.sender];
    ...
    function harvestEth(uint256 pid) public nonReentrant {
        DepositInfo memory _deposit = depositInfo[pid][msg.sender];
        EthPoolInfo storage _ethPoolInfo = ethPoolInfo[pid];
        VestingInfo memory _vesting = vestingInfo[pid][msg.sender];
        require(
    ...
    function harvestPrimeAndEth(uint256 pid) public {
        harvestPrime(pid);
        harvestEth(pid);
    }
    ...
    function harvestPoolsPrimeAndEth(uint256[] calldata pids) public {
        for (uint256 i = 0; i < pids.length; ++i) {
    ...
    function withdrawAndHarvestEth(uint256 pid, uint256 amount) public {
        harvestEth(pid);
        withdraw(pid, amount);
    }
    ...
    /// @param pid Token id to withdraw.
    function withdrawAndHarvestPrimeAndEth(uint256 pid, uint256 amount) public {
        harvestEth(pid);
