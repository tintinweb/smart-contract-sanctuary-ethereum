...
contract BondManager is Ownable, BondDiscountable {
    ...
    constructor(address _bond, address _baseToken, address _treasury) {
        ...
        addBondLevelAtIndex("Level I", SafeMath.mul(10, GLOBAL_PRECISION), 100, 0, totalActiveBondLevels);
        addBondLevelAtIndex("Level II", SafeMath.mul(100, GLOBAL_PRECISION), 105, 0, totalActiveBondLevels);
        addBondLevelAtIndex("Level III", SafeMath.mul(1000, GLOBAL_PRECISION), 110, 0, totalActiveBondLevels);
        addBondLevelAtIndex("Level IV", SafeMath.mul(5000, GLOBAL_PRECISION), 115, 0, totalActiveBondLevels);
    }
    ...
    function addBondLevelAtIndex(string memory _name, uint256 _price, uint16 _weight, uint32 _sellableAmount, uint16 _index) public onlyOwner returns (bytes4) {
    ...
    function addBondLevel(string memory _name, uint256 _price, uint16 _weight, uint32 _sellableAmount) external onlyOwner returns (bytes4) {
        return addBondLevelAtIndex(_name, _price, _weight, _sellableAmount, totalActiveBondLevels);
    }
    ...
    /// We disregard unbounded gas cost possible error as the contract is designed to store a "concise"
    /// amount of Bond levels: 10.
    function activateBondLevel(bytes4 levelID, uint16 _index) public onlyOwner {
    ...
    function createMultipleBondsWithTokens(bytes4 levelID, uint16 _amount) public {
        ...
        //bond.mintBonds(sender, levelID, uint8(_amount), unweightedShares, weightedShares);
    }
    ...
    function _claim2(uint256 _bondID, address sender) internal {
        (uint256 claimableShares, uint256 claimableRewards) = getClaimableAmounts(_bondID);
        require((claimableShares != 0 || claimableRewards != 0));
