...
contract AssetPool is IAssetPool, OwnableUpgradeable {
    ...
    function deposit(address token, uint amount) public payable override {
        ...
        if (msg.sender != info.manager) require(info.isOpen, "Not open");
        require(amount > 0, "Invalid amount");
        token.transferToThis(amount);

        (uint shareMinted, uint feeTotal, bool isPhaseChanged) = _getMintShareAndFeeData(token, amount);
        _updateFee(feeTotal);
    ...
    function _getMintShareAndFeeData(address token, uint amount)
        internal
        view
        returns (
            uint shareMinted,
    ...
    function withdraw(address token, uint amount) external override {
        ...
        (uint shareBurnt, uint fee, uint feeTotal) = _getBurnShareAndFeeData(token, amount);
        require(amount <= token.getBalance(address(this)) - fee, "Have no balance to withdraw");
    ...
    function _getBurnShareAndFeeData(address token, uint amount)
        internal
        view
        returns (
            uint shareBurnt,
    ...
    function setPoolInfo(Pool memory pool) external onlyManager {
        ...
        require(
            pool.manager == msg.sender || userShare(msg.sender) == 0,
            "Cannot change manager role before withdraw all shares"
        );
        poolInfo = pool;
    }
}
