...
contract GenerationalWealthSocietyERC20Token is ERC20Permit, VaultOwned {
    ...
    event UpdateTeamTaxFee(uint256 teamTaxFee_);
    event UpdateTeamTaxFeeAddress(address teamTaxFeeAddress_);
    event UpdateBurnTaxFee(uint256 burnTaxFee_);
    ...
    event UpdateIsExcludedMaxTransactionAmount(address addr_, bool isExcl_);
    ...
    uint256 maxTxAmount = 1000 * 10**9;
    uint256 teamTaxFee = 2;
    ...
    constructor() ERC20("GWS", "GWS", 9) {
        ...
        isExcludedMaxTransactionAmount[teamTaxFeeAddress] = true;
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) override internal virtual {
        ...
        // when transfer
        if(!isExcludedMaxTransactionAmount[sender] && !isExcludedMaxTransactionAmount[recipient]) {
        ...
        if(
            ...
            uint256 teamTaxFee = calculateTeamTaxFee(amount);
            uint256 burnTaxFee = calculateBurnTaxFee(amount);
            uint256 totalTax = teamTaxFee.add(burnTaxFee);

            super._transfer(sender, teamTaxFeeAddress, teamTaxFee);
            super._transfer(sender, burnTaxFeeAddress, burnTaxFee);
    ...
    function setTaxes(uint256 teamTaxFee_, uint256 burnTaxFee_) public onlyOwner() {
        teamTaxFee = teamTaxFee_;
        burnTaxFee = burnTaxFee_;
        require((teamTaxFee_.add(burnTaxFee_)) <= 20, 'Must keep sell taxes below 20%');
    ...
    function setAutomatedMarketMakerPair(address pair_, bool value_) public onlyOwner {
        ...
        excludeFromMaxTransaction(pair_, value_);
    }

    function excludeFromMaxTransaction(address addr_, bool isExcl_) public onlyOwner {
        isExcludedMaxTransactionAmount[addr_] = isExcl_;
        emit UpdateIsExcludedMaxTransactionAmount(addr_, isExcl_);
    }
