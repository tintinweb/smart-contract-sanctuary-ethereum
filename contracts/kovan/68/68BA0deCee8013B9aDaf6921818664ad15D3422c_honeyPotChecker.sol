...
contract trades{
    ...
    function swapExactTokensForTokensSupportingFeeOnTransferTokens(
        ...
        TransferHelper.safeTransfer(
            path[0], UniswapV2Library.pairFor(factorys[0], path[0], path[1]), amountIn
        );
...
contract honeyPotChecker{
    ...
    //receive() external payable {
        //assert(msg.sender == WETH); // only accept ETH via fallback from the WETH contract
    //}
    function honeyTestComplex(address[] calldata path,address[] calldata factory,address payable router) external virtual returns(bool isSafe,string memory raison, uint startingAmount, uint endingAmount){
        require(path.length == factory.length+1,"Les tailles de tableaux ne sont pas bonnes");
        startingAmount = ERC20(path[0]).balanceOf(router);
        try trades(router).swapExactTokensForTokensSupportingFeeOnTransferTokens(startingAmount,0,path,factory){
            endingAmount = ERC20(path[path.length-1]).balanceOf(router);
            if(endingAmount>0){
                //la transaction a marché, c'est parfait
                    isSafe = true;
            }
        ...
        return (isSafe, raison, startingAmount,endingAmount);
    }
