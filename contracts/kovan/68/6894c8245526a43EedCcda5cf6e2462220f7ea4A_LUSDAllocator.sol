...
contract LUSDAllocator is MinerAccessControlled {
    /* ======== DEPENDENCIES ======== */
    ...
    ITreasury public treasury; // Miner Treasury
    ...
    function withdraw(address token, uint256 amount) external onlyGuardian {
        ...
        if (token == lusdTokenAddress) {
            ...
            treasury.deposit(balance, token, value); // deposit using value as profit so no MINER is minted
        } else {
    ...
    /**
    Helper method copying MinerTreasury::_tokenValue(), whose name was 'valueOf()' in v1
