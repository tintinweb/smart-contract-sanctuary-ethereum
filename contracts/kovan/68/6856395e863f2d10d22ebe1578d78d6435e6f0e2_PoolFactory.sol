    ...
    PausableUpgradeable
{
    ...
    mapping(address => address[]) public lenders;
    mapping(address => address[]) public borrowers;
    // Collateral => Lend Token => Pool
    mapping(address => mapping(address => address[])) public migrateRegistry;
    ...
    function deployPool(
        ...
        address lendingPool = Clones.clone(poolImplementationAddress);
        ...
        lenders[msg.sender].push(lendingPool);
        migrateRegistry[_colToken][_lendToken].push(lendingPool);
        emit DeployPool(
            lendingPool,
            _mintRatio,
            _colToken,
    ...
    function addBorrowerRecord(address _borrower) external onlyPool {
        borrowers[_borrower].push(msg.sender);
    }
    ...
    function getLenderPools(address _lender) external view returns(address[] memory){
        return lenders[_lender];
    }

    function getMigrationPools(address _colToken, address _lendToken) external view returns(address[] memory){
        return migrateRegistry[_colToken][_lendToken];
    }
