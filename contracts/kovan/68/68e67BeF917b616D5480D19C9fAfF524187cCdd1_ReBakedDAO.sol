...
contract ReBakedDAO is IReBakedDAO, Ownable, ReentrancyGuard, Projects {
	...
	*/
    ...
    function approvePayment(bytes32 _projectId, bytes32 _packageId, address _collaborator) external onlyOwner {
        ...
        _paidBonusForApprovedPayment(_projectId, _packageId,_collaborator);
				...
				collaboratorData[_projectId][_packageId][_collaborator].isDisputeRaised = false;
    }
	...
	*/
		...
		returns(bytes32 packageId_)
	{
		...
		_createPackage(projectId_, packageId_, budget_, observerBudget_, bonus_);
	...
	*/
	function addObserver(
		...
		onlyInitiator(projectId_)
	{
		for(uint i = 0; i<packageId_.length;i++){
	...
	*/
		...
		returns(uint256 amount_)
	{
		require(!isDispute[collaborator_][packageId_]); 
				if(collaboratorData[projectId_][packageId_][collaborator_].approvedMGPForDispute == true){
	...
	*/
		...
		returns(uint256 amount_)
	{
		...
		if(collaboratorData[projectId_][packageId_][collaborator_].approvedBonusForDispute == true){
...
interface IReBakedDAO {
	...
	*/
	function createPackage(
        bytes32 projectId_, uint256 budget_, uint256 bonus_, uint256 observerBudget_) external returns(bytes32 packageId_);
	...
	* @param observer_ observer addresses
	*/
	function addObserver(bytes32 projectId_, bytes32[] calldata packageId_, address observer_) external;
	...
	*/
		...
		onlyExistingProject(project_)
	{
		...
		if(project_.isOwnToken)
			...
			IERC20(project_.token).safeTransferFrom(msg.sender, address(this), project_.budget*10e17);
		else
	...
	*/
		...
		onlyExistingProject(project_)
	{
		...
		if(project_.timeStarted != 0 && budgetLeft_ != 0){
			if(project_.isOwnToken)
				IERC20(project_.token).safeTransfer(project_.initiator, budgetLeft_*10e17);
			else
				IIOUToken(address(project_.token)).burn(budgetLeft_*10e17);
		}
	}
...
contract Packages is Observers, Collaborators {
	...
	*/
	...
	{
		return (packageData[projectId_][packageId_],uint(packageData[projectId_][packageId_].budgetObservers/packageData[projectId_][packageId_].totalObservers));
	}
...
contract Collaborators {
	...
	*/
	...
	function _getMgpForApprovedPayment(
		...
		returns(uint256)
	{
		return collaboratorData[projectId_][packageId_][collaborator_]._getMgpForApproved();
	}
	/**
	...
	*/
	...
	function _paidBonusForApprovedPayment(
		...
		internal
	{
		collaboratorData[projectId_][packageId_][collaborator_]._paidBonusForApproved();
	}
...
library CollaboratorLibrary {
	...
	*/
	...
	function _paidBonusForApproved(
		Collaborator storage collaborator_
	)
		internal
		onlyExistingCollaborator(collaborator_)
	{
		require(collaborator_.approvedBonusForDispute == true || collaborator_.isBonusPaid == false, "bonus already approved");
