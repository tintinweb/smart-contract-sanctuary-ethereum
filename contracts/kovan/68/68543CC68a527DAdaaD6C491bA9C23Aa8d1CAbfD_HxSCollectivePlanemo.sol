...
contract HxSCollectivePlanemo is ERC721, Ownable {
  ...
  constructor(string memory uri) ERC721('H x S Collective - Planemo', 'HxSCollectivePlanemo') {
    ...
    for (uint256 tokenId = 1; tokenId <= collectionSize; ) {
      ...
      // 2.1 We don't need the autochecks as we're already checking the variable in the for function
      unchecked {
        tokenId++;
      }
    }
  ...
  */
  ...
  function withdraw() external onlyOwner {
    ...
    require(sent, 'HxSCollectivePlanemo: Failed to witdraw the balance of the contract');
  }
