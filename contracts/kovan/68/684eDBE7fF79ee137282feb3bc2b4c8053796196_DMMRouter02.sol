...
contract DMMRouter02 is IDMMRouter02 {
    ...
    function _addLiquidity(
        ...
        ) = DMMLibrary.getTradeInfo(pool, tokenA, tokenB);
        if (reserveA == 0 && reserveB == 0) {
            (amountA, amountB) = (amountADesired, amountBDesired);
        } else {
            uint256 amountBOptimal = DMMLibrary.quote(
                amountADesired,
                reserveA,
                reserveB
            );
            if (amountBOptimal <= amountBDesired) {
                ...
                uint256 amountAOptimal = DMMLibrary.quote(
                    amountBDesired,
                    reserveB,
                    reserveA
                );
    ...
    function verifyPoolAddress(
        ...
        require(
            IDMMFactory(factory).isPool(tokenA, tokenB, pool),
