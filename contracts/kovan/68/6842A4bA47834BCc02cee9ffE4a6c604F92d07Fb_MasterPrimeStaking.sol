...
contract MasterPrimeStaking is Ownable, ERC1155Holder {
    ...
    function addPool(uint256 _allocPoint, uint256[] memory _tokenIds)
        ...
        onlyOwner
    {
        require(_tokenIds.length > 0, "Paraset TokenIds cannot be empty");
        ...
        returns (uint256 pending)
    {
        ...
        if (
            ...
            accPrimePerShare = accPrimePerShare + (primeReward / totalSupply);
        }
        pending = uint256(
    ...
    function updatePool(uint256 pid) public {
        ...
        if (totalSupply > 0) {
            ...
            pool.accPrimePerShare = uint128(
                uint256(pool.accPrimePerShare) + (primeReward / totalSupply)
            );
        }
