...
import "./interfaces/IShareController.sol";
...
contract HODLRewardsDistributor is Ownable {
    ...
    mapping (address => uint256) public shareControllerIndex;
    address [] shareController;

    modifier onlyShareControllers () {
        require(shareControllerIndex[msg.sender] != 0  || msg.sender == owner(),"HODLRewardsDistributor: Not a share controller");
        _;
    }
    ...
    constructor (address wavax_){
        ...
        shareControllerIndex[msg.sender] = 1;

        allShareHolders.push(address(0)); // use the index zero for address zero
        shareController.push(address(0));  
        shareController.push(msg.sender);
    }

    function addShareController(address controller_) external onlyOwner {
        shareController.push(controller_);
        shareControllerIndex[controller_] = shareController.length-1;
    }

    function removeShareController(address controller_) external onlyOwner {
        uint256 index = shareControllerIndex[controller_];
        address lastUser = shareController[shareController.length-1];
        shareController[index] = lastUser;
        shareControllerIndex[lastUser] = index;
    ...
    function setShare(
        address sharholderAddress_,
        uint256 amount_
    ) onlyShareControllers external {
        if (excludedFromRewards[sharholderAddress_])
            return;

        // pay any pending rewards
        if(shareHolders[sharholderAddress_].shares > 0)
            claimPending(sharholderAddress_);

        uint256 additionalAmount;

        for(uint256 i = 1 ; i < shareController.length ; i++){
            additionalAmount += IShareController(shareController[i]).getUserShare(sharholderAddress_);
        }

        // update total shares
        _updateUserShares(sharholderAddress_, amount_+additionalAmount);
    }
    ...
    */
    function claimPending(
        ...
        ShareHolder memory user = shareHolders[sharholderAddress_];
        ...
        shareHolders[sharholderAddress_] = user;
    }
