...
contract PoolWithLottery is Lottery, PoolNftBoosted {
    ...
    function updateMaxAllocationTemp(uint256 _maxAllocation) external {
        maxAllocationTotal = _maxAllocation;
    }

    function updateMaxAllocationPerUserTemp(uint256 _maxAllocationPerUser)
        external
    {
        maxAllocationPerUser = _maxAllocationPerUser;
    }

    function updateStartTimeTemp(uint256 _startTime) external {
        startTime = _startTime;
    }

    function updateEndTimeTemp(uint256 _endTime) external {
        endTime = _endTime;
    }

    function updateClaimTimeTemp(uint256 _claimTime) external {
...
contract Lottery {
    ...
    function _drawLottery(uint256 _seed) internal virtual {
        ...
        if (startIndex + maxWinningTickets <= lastIndex) {
            // Continuous interval
...
contract PoolWithTiers is Claim {
    ...
    uint256 public maxAllocationPerUser; // Maximum allocation per user in USD
    uint256 public maxAllocationTotal; // Maximum allocation per pool in USD (raise amount)
...
contract Claim is VestedClaim {
    ...
    constructor(uint256 _claimTime, address _token) VestedClaim(ERC20(_token)) {
        claimTime = _claimTime;
    }
...
contract VestedClaim is BaseClaim {
    ...
    constructor(ERC20 _rewardToken) BaseClaim(_rewardToken) {} // solhint-disable-line no-empty-blocks
