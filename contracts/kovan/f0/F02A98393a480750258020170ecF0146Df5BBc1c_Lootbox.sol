...
// Modified copyright OpenZeppelin Contracts v4.4.0
...
contract LootboxConfiguration is ILootboxConfiguration, ILootboxEvents, Ownable, LootboxStorage {
    function updateConfig(LootboxConfig calldata config, string calldata uri) external override isOwner {
        if (keccak256(abi.encode(_config, _baseURI)) == keccak256(abi.encode(config, uri))) revert LootboxErrors.SameConfig();
        _config = config;
        _baseURI = uri;
        emit ConfigUpdated();
    }
