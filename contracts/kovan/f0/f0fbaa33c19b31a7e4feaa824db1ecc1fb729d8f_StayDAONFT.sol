    ...
    UUPSUpgradeable
{
    ...
    uint8 public constant ARTIST_ROYALTY_SHARE = 10;
    uint8 public constant STAYDAO_ROYALTY_SHARE = 65;
    ...
    uint16 public constant ROYALTY_DECIMALS = 1000;
    ...
    address payable public stayDAO;
    uint256 public artistRoyalty;
    uint256 public stayDAORoyalty;
    ...
    function setAddresses(address payable artist_, address payable stayDAO_)
        external
        onlyOwner
    {
        artist = artist_;
    ...
    function withdrawRoyalty() external whenNotPaused nonReentrant {
        uint256 amount;
        if (_msgSender() == artist) {
            amount = artistRoyalty;
            artistRoyalty = 0;
            // solhint-disable-next-line avoid-low-level-calls
            (bool success, bytes memory data) = artist.call{value: amount}("");
            require(success, string(data));
        } else if (_msgSender() == stayDAO) {
            amount = stayDAORoyalty;
            stayDAORoyalty = 0;
            // solhint-disable-next-line avoid-low-level-calls
            (bool success, bytes memory data) = stayDAO.call{value: amount}("");
            require(success, string(data));
        } else {
            revert("Unauthorized");
        }
    ...
    function _wrappedSafeMint(address to, uint256 quantity) internal {
        ...
        artistRoyalty += (msg.value * ARTIST_ROYALTY_SHARE) / ROYALTY_DECIMALS;
        stayDAORoyalty +=
            (msg.value * STAYDAO_ROYALTY_SHARE) /
            ROYALTY_DECIMALS;
