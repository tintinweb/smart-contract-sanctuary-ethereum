...
contract Staking_Pool is Ownable {
    ...
    uint public per30days;
    uint public per30daysReward;
    ...
    function set_Reward(uint _daily, uint _30days, uint _60days) public onlyOwner {
        _Daily_Reward = _daily * R_token_decimal;
        _30Days_Reward = _30days * R_token_decimal;
    ...
    function set_deduction(uint _onperDaily, uint _onperDailyReward, uint _onper30days, uint _onper30daysReward, uint _onper60days, uint _onper60daysReward ) public onlyOwner{
        perDaily = _onperDaily;
        perDailyReward = _onperDailyReward;
    
        per30days = _onper30days;
        per30daysReward = _onper30daysReward;
    ...
    function Stake(uint _pid,uint256 _amount) public { //1=daily, 2=30days, 3=60days
        ...
        if(_pid == 2) {
            uint256 time = block.timestamp + 30 days; //30 days;
            user_rtime[_person] = time;
        ...
        for(uint i = 0 ; i < _userdata.length ; i++) {
            ...
            if(_userdata[i].person == msg.sender){
                ...
                if(_userdata[i].id == 2) { //30 days
                    ...
                    Stake_Token.transfer(msg.sender,getCal(_userdata[i].amount,per30days));
                    Reward_Token.transfer(msg.sender, getCal(_30Days_Reward, per30daysReward));
