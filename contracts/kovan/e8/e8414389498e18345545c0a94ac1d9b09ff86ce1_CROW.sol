...
/**
 *
 * CROW is an ERC-20 utility token on Ethereum mainnet.
 *
 * It enforces the scarcity of "Crow Bot", which is a bot for escrowing cryptocurrency transactions.
 *
 * In short, Crow Bot allows users to deposit funds into an escrow contract.
 * Those funds will be automatically dispensed to a winner or group of winners if a certain condition or set of conditions is met.
 * Use cases of this technology include automatically managing and verifying the integrity of cryptocurrency giveaways, competitions, and more.
 *
 * The possibilities might be endless, but as a fixed-supply and non-inflationary token, CROW's supply is limited.
 * And to use Crow Bot, you must set aside a certain amount of CROW tokens.
 * This mechanism ensures sustainability and exclusivity.
 *
*/
...
contract DividendDistributor is IDividendDistributor, Auth {
    ...
    IERC20 BASE = IERC20(0xd0A1E359811322d97991E03f863a0C30C2cF029C);
    ...
    address public DEFAULT_TOKEN = 0xd0A1E359811322d97991E03f863a0C30C2cF029C;
    ...
    function setShare(
        ...
        if (shares[shareholder].amount > 0) {
            if (allowed[choice[shareholder]]) {
                ...
                distributeDividend(shareholder, DEFAULT_TOKEN);
            }
        }
    ...
    function changeDEFAULT_TOKEN(
        address _DEFAULT_TOKEN
    ) external onlyOwner {
...
contract CROW is IERC20, Auth {
    using SafeMath for uint256;

    uint256 public constant MASK = type(uint128).max;
    address BASE = 0xd0A1E359811322d97991E03f863a0C30C2cF029C;
    address public WETH = 0xd0A1E359811322d97991E03f863a0C30C2cF029C;
    address DEAD = 0x000000000000000000000000000000000000dEaD;
    address ZERO = 0x0000000000000000000000000000000000000000;
    address DEAD_NON_CHECKSUM = 0x000000000000000000000000000000000000dEaD;

    string constant _name = "Crow Bot";
    ...
    uint256 buyLiqFee = 0;
    uint256 buyReflectionFee = 150;
    uint256 buyDevFee = 750;
    uint256 buyGiveawayFee = 100;
    uint256 buyTotalFee = 1000;

    uint256 sellLiqFee = 150;
    uint256 sellReflectionFee = 250;
    uint256 sellDevFee = 600;
    uint256 sellGiveawayFee = 0;
    uint256 sellTotalFee = 1000;

    uint256 feeDenominator = 10000;

    uint256 _liqTokensToSwap;
    uint256 _reflectionTokensToSwap;
    uint256 _devTokensToSwap;
    uint256 _giveawayTokensToSwap;

    address public liqFeeReceiver = 0x9B006037682Fbaa1bb30Cb78C280F325aE541002; // CrowBotLiquidity.eth
    address public devFeeReceiver = 0xB5F54A1FfdDAfb437422159235046a130aCc292C; // CrowBotDevelopers.eth
    address public giveawayFeeReceiver = 0x3d597949fa41143503F13a84548b1b6f6264E42b; // CrowBotGiveaways.eth
    ...
    function takeFee(
        ...
        if (totalFee > 0) {
            if (isSell) {
                if (sellLiqFee > 0) {
                    _liqTokensToSwap += feeAmount * sellLiqFee / totalFee;
                }
                if (sellDevFee > 0) {
                    _devTokensToSwap += feeAmount * sellDevFee / totalFee;
                }
                if (sellReflectionFee > 0) {
                    _reflectionTokensToSwap += feeAmount * sellReflectionFee / totalFee;
                }
                if (sellGiveawayFee > 0) {
                    _giveawayTokensToSwap += feeAmount * sellGiveawayFee / totalFee;
                }
            } else {
                if (buyLiqFee > 0) {
                    _liqTokensToSwap += feeAmount * buyLiqFee / totalFee;
                }
                if (buyDevFee > 0) {
                    _devTokensToSwap += feeAmount * buyDevFee / totalFee;
                }
                if (buyReflectionFee > 0) {
                    _reflectionTokensToSwap += feeAmount * buyReflectionFee / totalFee;
                }
                if (buyGiveawayFee > 0) {
                    _giveawayTokensToSwap += feeAmount * buyGiveawayFee / totalFee;
                }
            }
        }
    ...
    function swapBack() internal swapping {
        ...
        uint256 totalTokensToSwap = _liqTokensToSwap.add(_devTokensToSwap).add(_reflectionTokensToSwap).add(_giveawayTokensToSwap);
        
        uint256 tokensForLiq = _liqTokensToSwap.div(2);
        uint256 amountToSwap = contractBalance.sub(tokensForLiq);
        ...
        uint256 amountETHLiq = amountETH.mul(_liqTokensToSwap).div(totalTokensToSwap).div(2);
        uint256 amountETHReflection = amountETH.mul(_reflectionTokensToSwap).div(totalTokensToSwap);
        uint256 amountETHDev = amountETH.mul(_devTokensToSwap).div(totalTokensToSwap);
        uint256 amountETHGiveaway = amountETH.mul(_giveawayTokensToSwap).div(totalTokensToSwap);
        
        _liqTokensToSwap = 0;
        _devTokensToSwap = 0;
        _reflectionTokensToSwap = 0;
        _giveawayTokensToSwap = 0;
        ...
        if (amountETHGiveaway > 0) {
            payable(giveawayFeeReceiver).transfer(amountETHGiveaway);
        }

        if (tokensForLiq > 0) {
            router.addLiquidityETH{value: amountETHLiq}(
                address(this),
                tokensForLiq,
                0,
                0,
            ...
            emit AutoLiquify(amountETHLiq, tokensForLiq);
        }
    }
    ...
    function setFees(
        ...
        uint256 _buyDevFee,
        uint256 _buyGiveawayFee,
        uint256 _sellLiqFee,
        uint256 _sellReflectionFee,
        uint256 _sellDevFee,
        uint256 _sellGiveawayFee,
        uint256 _feeDenominator
    ) external authorized {
        buyLiqFee = _buyLiqFee;
        buyReflectionFee = _buyReflectionFee;
        buyDevFee = _buyDevFee;
        buyGiveawayFee = _buyGiveawayFee;
        buyTotalFee = _buyLiqFee.add(_buyReflectionFee).add(_buyDevFee).add(_buyGiveawayFee);

        sellLiqFee = _sellLiqFee;
        sellReflectionFee = _sellReflectionFee;
        sellDevFee = _sellDevFee;
        sellGiveawayFee = _sellGiveawayFee;
        sellTotalFee = _sellLiqFee.add(_sellReflectionFee).add(_sellDevFee).add(_sellGiveawayFee);

        feeDenominator = _feeDenominator;
    }

    function setFeeReceivers(
        address _liqFeeReceiver,
        address _devFeeReceiver,
        address _giveawayFeeReceiver
    ) external authorized {
        liqFeeReceiver = _liqFeeReceiver;
