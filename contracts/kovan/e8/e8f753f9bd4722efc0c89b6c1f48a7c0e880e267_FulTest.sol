...
interface interfaceful{
    function mintNFT(uint256 tokenId, string memory tokenURI_,address user,uint _rarity,uint position ) external;
...
contract FulTest is ChainlinkClient, ConfirmedOwner{
    ...
    function fulfillMultipleParameters(
        ...
        fulToken.transferFrom(requestIdToUser[requestId],address(this), totalPrice);
        uint totalNFT = requestIdToPacks[requestId].mul(3);
        for(uint i=0 ;i<totalNFT;i++){
            interfaceful(ful).mintNFT(id, "TOkenUri", requestIdToUser[requestId],rarity, position);
            id++;
        }
    }
    ...
    function buyPack(uint _noOfPack)public{
        ...
        for(uint i=0 ;i<totalNFT;i++){
            interfaceful(ful).mintNFT(id, "TOkenUri", msg.sender, rarity, position);
            id++;
        }

    }

    function SetNFT(uint[]memory ids, string[]memory _tokenUri,uint _rarity,uint _position)public {
        for(uint i=0 ;i<ids.length;i++){
            interfaceful(ful).mintNFT(ids[i], _tokenUri[i], msg.sender,_rarity, _position);
        }
    }
    ...
    function toLockNFT(uint[] memory _id, uint _duration)public { 
        uint sizeOfArray = 15;
        require(_id.length == sizeOfArray,"Array of Ids should be length of 15");
