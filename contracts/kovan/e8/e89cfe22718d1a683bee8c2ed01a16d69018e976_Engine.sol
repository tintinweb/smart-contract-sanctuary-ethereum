...
interface DaiToken {
    ...
    function balanceOf(address source) external view returns (uint);
}

contract Engine is VRFConsumerBase {
    ...
    uint internal fuel;        // Oracle fee to get a random number
    ...
    event participantEntry(address indexed participant, uint indexed amount, uint indexed time);
    event prizeTransfer(address indexed winner, uint indexed amount, uint indexed time);
    event remainingContractFuel(uint indexed remainingFuel);
    ...
    function enter() public payable {
        require(msg.value > 0, "amount should be one DAI");
        // require(amount < 2, "no more than one DAI");
        
        // Transfer the amount to this contract's balance
    ...
    function payWinner() public onlyOwner {
        ...
        uint prize = (address(this).balance / 100) * 96; // 96% of the reward
        uint fee = address(this).balance - prize; // 4% of the reward
    ...
    // this function generates a random number using Chainlink oracle
    function getRandomNumber() public returns (bytes32 requestId) {
        require(LINK.balanceOf(address(this)) >= fuel, "Not enough LINK in contract");
        return requestRandomness(keyHash, fuel);
    }

    // this function checks the validity of the random number and is required by Chainlink
