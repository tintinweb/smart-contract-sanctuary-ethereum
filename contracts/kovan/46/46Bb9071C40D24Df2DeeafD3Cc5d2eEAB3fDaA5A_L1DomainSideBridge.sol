...
contract L1DomainSideBridge {
    ...
    mapping(bytes32 => bool) sentHashesOnions;
    
    address public governance;//this is the only address that can add sDTokenAddresspair 
    event Deb(string transferDataHash);
    ...
    function declareNewHashChainHead(bytes32 _hashOnion) public{
        sentHashesOnions[_hashOnion] = true;
    }
    /*
    /// @notice getLPFee function
    /// @notice calculates the liquidity provider fee.
    function declareNewHashChainHead(bytes32 _hashOnion) external{
        ICrossDomainMessenger l1cdm = ICrossDomainMessenger(Proxy_OVM_L1CrossDomainMessenger);
        l1cdm.sendMessage(
            sourceSideAddress,
            abi.encodeWithSignature(
                "addNewKnownHashOnion(byte32)",
                _hashOnion
            ),
