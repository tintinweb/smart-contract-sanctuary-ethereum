 ...
 */
contract OpenStarterNft is ERC721, Ownable {
    ...
    modifier onlyMinter() {
        require(
            owner == msg.sender || creator == msg.sender || minters[msg.sender],
            "1"
        );
        _;
    }
    ...
    function _incrementTokenId() private {
        require(_currentTokenId < maxQuantityMinted, "sold out");
        _currentTokenId++;
    }
...
interface IOpenStarterLibrary {
    ...
    function getSaleFactoryAddress() external returns (address);

    function addNfts(address _nftAddress) external returns (uint256);
}

contract OpenStarterNftFactory {
    using SafeMath for uint256;
    ...
    function createNft(
        ...
        Attributes[] memory _levelInfo
    ) external returns (uint256) {
        OpenStarterNft nft = new OpenStarterNft(
            _info.name,
            _info.symbol,
            _info.maxQuantityMinted
        );
        nft.setAddressInfo(
            _creator,
            address(this),
            openStarterLibrary.getSaleFactoryAddress(),
            _owner
        );
        nft.setPreviewInfo(
            _info.logo,
        ...
        uint256 levelLen = _levelInfo.length;
        for (uint256 i = 0; i < levelLen; i++) {
            nft.setLevelInfo(
                i,
                _levelInfo[i].sName,
                _levelInfo[i].uMin,
        ...
        nft.setBaseUrl(_info.baseUrl);
        uint256 nftId = openStarterLibrary.addNfts(address(nft));
