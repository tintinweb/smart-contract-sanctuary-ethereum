...
contract CGMarketPlaceContract is IMarket, AccessControl {
    ...
    address public _adminAddress = 0x2E96A99eA61CB5c80547aBf44aB510ee2d6dBA55; // CG VAULT ADDRESS For Commission

    bytes32 public constant ADMIN_SIGNATURE = keccak256("ADMIN_SIGNATURE");
    ...
    */
    ...
    function signatureWallet(uint256 _orderId, uint256 commissionPercent, uint256 _timestamp, bytes memory _signature) public pure returns (address){
        return ECDSA.recover(keccak256(abi.encode(_orderId, commissionPercent, _timestamp)), _signature);
    }

    /**
     * @dev See {IMarket}
     */
    function completeOrder(uint256 _orderId, uint256 _commissionPercent, uint256 _timestamp, bytes memory _signature)
        external
        override
        returns (bool) {
        ...
        address signerOwner = signatureWallet(_orderId, _commissionPercent, _timestamp, _signature);
        require(hasRole(ADMIN_SIGNATURE, signerOwner), "CG Market: Not authorized to complete order");
     ...
     * @dev See {IMarket}
     */
    function acceptBid(uint256 _bidId, uint256 _commissionPercent, uint256 _timestamp, bytes memory _signature)
        external
        override
        returns (bool) {
        ...
        address signerOwner = signatureWallet(_bidId, _commissionPercent, _timestamp, _signature);
        require(hasRole(ADMIN_SIGNATURE, signerOwner), "CG Market: Not authorized to complete order");
