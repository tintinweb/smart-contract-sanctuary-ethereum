...
contract EmpressKYC is Ownable {
...
contract StudentOptInOut is Ownable, EmpressKYC {

    mapping(address => string) optInOutFlag;
    mapping(address => address) processorChoice;
    mapping(address => uint32) numStudents;
    ...
    function setProcesorChoice(address _addr1, address _addr2) public onlyOwner {
        ...
        require(kycCompleted(_addr2), 'KYC needs to be completed');
        processorChoice[_addr1] = _addr2;
        numStudents[_addr2] += 1;
    ...
    function getNumberStudents(address _address) public view returns(uint32) {
        return numStudents[_address];
    }
