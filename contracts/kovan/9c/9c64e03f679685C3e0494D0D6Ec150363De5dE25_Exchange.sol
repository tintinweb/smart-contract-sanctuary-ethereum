    ...
    IMatchOrders
{
        ...
        internal
    {
        ...
        if (leftFeeRecipientAddress != address(0)) {
            require(leftFeeRecipientAddress == feesManager, "Fee recipient must be fees manager");
        }

        if (rightFeeRecipientAddress != address(0)) {
            require(rightFeeRecipientAddress == feesManager, "Fee recipient must be fees manager");
        }
        ...
        if (feesManager != address(0)) {
            uint256 assetId = _getMakerAssetId(rightOrder.makerAssetData);
            address tokenAddress = _getTokenAddress(rightOrder.takerAssetData);
            IFeesManager(feesManager).receiveFees(assetId, rightOrder.makerAddress,
                tokenAddress, matchedFillResults.right.makerFeePaid);

            assetId = _getMakerAssetId(leftOrder.makerAssetData);
            tokenAddress = _getTokenAddress(leftOrder.takerAssetData);
            IFeesManager(feesManager).receiveFees(assetId, leftOrder.makerAddress,
                tokenAddress, matchedFillResults.left.makerFeePaid);
        }
    }
}
    ...
    MixinSignatureValidator
{
    ...
    /// @dev Set 4K fees manager
    /// @param newFeesManager Fees manager
        ...
        internal
    {
        ...
        if (order.feeRecipientAddress != address(0)) {
            require(order.feeRecipientAddress == feesManager, "Fee recipient must be fees manager");
        }
        ...
        if (feesManager != address(0)) {
            // default is the NFT holder filling the bid order (during bid acceptance)
            bytes memory lhsAssetData = order.takerAssetData;
            bytes memory rhsAssetData = order.makerAssetData;
            address feeOwner = order.makerAddress;
            uint256 feePaid = fillResults.makerFeePaid;

            // If the maker asset data contains erc1155 data, the length will be more than 36
            // the length for erc20 data is 36 bytes
            if(order.makerAssetData.length > 36){
                lhsAssetData = order.makerAssetData;
                rhsAssetData = order.takerAssetData;
                feeOwner = order.takerAddress;
                feePaid = fillResults.takerFeePaid;
            }

            uint256 assetId = _getMakerAssetId(lhsAssetData);
            address tokenAddress = _getTokenAddress(rhsAssetData);
            IFeesManager(feesManager).receiveFees(assetId, feeOwner, tokenAddress, feePaid);
        }
