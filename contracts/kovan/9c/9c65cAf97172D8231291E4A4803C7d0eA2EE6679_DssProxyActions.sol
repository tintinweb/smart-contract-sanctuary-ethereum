...
interface Config {
    ...
    function getFee(Fees feeType) external view returns (uint);
}
...
contract DssProxyActions is Common {
    ...
    address public constant config = 0xEE29985A0BF2Cf06F1E2F6052C8528384Fbf9AA8;
    ...
    function toIntEx(uint x) public pure returns (int y) {
        y = int(x);
        require(y >= 0, "int-overflow");
    }
    ...
    function ethJoin_join(address apt, address urn, uint _feeDeposit) public payable {
        address _treasury = Config(config).getTreasury();
        if (_feeDeposit > 0) {
            payable(_treasury).transfer(_feeDeposit);
        }
        // Wraps ETH in WETH
        GemJoinLike(apt).gem().deposit{value:(msg.value - _feeDeposit)}();
        // Approves adapter to take the WETH amount
        GemJoinLike(apt).gem().approve(address(apt), msg.value - _feeDeposit);
        // Joins WETH collateral into the vat
        GemJoinLike(apt).join(urn, msg.value - _feeDeposit);
    }

    function gemJoin_join(address apt, address urn, uint amt, bool transferFrom, uint _feeDepositGem) public {
        // Only executes for tokens that have approval/transferFrom implementation
        address _treasury = Config(config).getTreasury();
        if (transferFrom) {
            if (_feeDepositGem > 0) {
                GemJoinLike(apt).gem().transferFrom(msg.sender, _treasury, _feeDepositGem);
            }
            // Gets token from the user's wallet
            GemJoinLike(apt).gem().transferFrom(msg.sender, address(this), amt - _feeDepositGem);
            // Approves adapter to take the token amount
            GemJoinLike(apt).gem().approve(apt, amt - _feeDepositGem);
        }
        // Joins token collateral into the vat
        GemJoinLike(apt).join(urn, amt - _feeDepositGem);        
    }
    ...
    function getFee(bytes32 token, Config.Fees feeType) public view returns (uint percent) {
        percent = !Config(config).checkToken(token) ? Config(config).getFee(feeType) : 0;
    }

    function transferFeeWithdrawETH(address user, address _treasury, uint value, uint _fee) internal {
        if (_fee > 0) {
            payable(_treasury).transfer(_fee);
        }
        payable(user).transfer(value - _fee);
    }
    ...
    function lockETH(
        ...
        uint _caculatedFee = getFee(ilk, Config.Fees.DEPOSIT);
        // Receives ETH amount, converts it to WETH and joins it into the vat
        uint _fee = msg.value * _caculatedFee / decimal;
        ethJoin_join(ethJoin, address(this), _fee);
    ...
    function lockGem(
        ...
        bytes32 ilk = ManagerLike(manager).ilks(cdp);
        uint _caculatedFee = getFee(ilk, Config.Fees.DEPOSIT_GEM);
        uint _fee = amt * _caculatedFee / decimal;
        gemJoin_join(gemJoin, address(this), amt, transferFrom, _fee);
        ...
        VatLike(ManagerLike(manager).vat()).frob(
            ...
            address(this),
            address(this),
            toInt(convertTo18(gemJoin, amt - _fee)),
            0
        );
    }
    ...
    function freeETH(
        ...
        bytes32 ilk = ManagerLike(manager).ilks(cdp);
        uint _caculatedFee = getFee(ilk, Config.Fees.WITHDRAW);
        uint _fee = wad * _caculatedFee / decimal;
        transferFeeWithdrawETH(msg.sender, Config(config).getTreasury(), wad, _fee);
    }
    ...
    function freeGem(
        ...
        bytes32 ilk = ManagerLike(manager).ilks(cdp);
        uint _caculatedFee = getFee(ilk, Config.Fees.WITHDRAW_GEM);
        uint _fee = amt * _caculatedFee / decimal;
        ...
        if (_fee > 0) {
            GemJoinLike(gemJoin).gem().transferFrom(msg.sender, Config(config).getTreasury(), _fee);
        }
        GemJoinLike(gemJoin).exit(msg.sender, wad - _fee);
    }
    ...
    function draw(
        ...
        bytes32 ilk = ManagerLike(manager).ilks(cdp);
        uint _fee = wad * getFee(ilk, Config.Fees.BORROW) / decimal;
        ...
        if(_fee > 0) {
            USBJoinLike(USBJoin).USB().transferFrom(msg.sender, Config(config).getTreasury(), _fee);
        }
    }
    ...
    function wipe(
        ...
        address urn = ManagerLike(manager).urns(cdp);
        bytes32 ilk = ManagerLike(manager).ilks(cdp);
        uint _fee = wad * getFee(ilk, Config.Fees.PAYBACK) / decimal;
        if(_fee > 0) {
            USBJoinLike(USBJoin).USB().transferFrom(msg.sender, Config(config).getTreasury(), _fee);
            wad -= _fee;
        }
        ...
        if (own == address(this) || ManagerLike(manager).cdpCan(own, cdp, address(this)) == 1) {
            ...
            USBJoin_join(ilk, USBJoin, urn, wad);
            // Paybacks debt to the CDP
            frob(manager, cdp, 0, _getWipeDart(vat, VatLike(vat).USB(urn), urn, ilk));
        } else {
             // Joins USB amount into the vat
            USBJoin_join(ilk, USBJoin, address(this), wad);
            // Paybacks debt to the CDP
            VatLike(vat).frob(
                ilk,
                urn,
    ...
    function wipeAll(
        ...
        bytes32 ilk = ManagerLike(manager).ilks(cdp);
        (, uint art) = VatLike(vat).urns(ilk, urn);
        uint _fee = art * getFee(ilk, Config.Fees.PAYBACK) / decimal;
        if(_fee > 0) {
            USBJoinLike(USBJoin).USB().transferFrom(msg.sender, Config(config).getTreasury(), _fee);
            art -= _fee;
        }
        ...
        if (own == address(this) || ManagerLike(manager).cdpCan(own, cdp, address(this)) == 1) {
            // Joins USB amount into the vat
            USBJoin_join(ilk, USBJoin, urn, _getWipeAllWad(vat, urn, urn, ilk, _fee));
            // Paybacks debt to the CDP
            frob(manager, cdp, 0, -int(art));
        } else {
            // Joins USB amount into the vat
            USBJoin_join(ilk, USBJoin, address(this), _getWipeAllWad(vat, address(this), urn, ilk, _fee));
            // Paybacks debt to the CDP
            VatLike(vat).frob(
                ilk,
                urn,
    ...
    function lockETHAndDraw(
        ...
        address vat = ManagerLike(manager).vat();
        bytes32 ilk = ManagerLike(manager).ilks(cdp);
        uint _fee = msg.value * getFee(ilk, Config.Fees.DEPOSIT) / decimal;
        uint _feeBorrow = wadD * getFee(ilk, Config.Fees.BORROW) / decimal;
        // Receives ETH amount, converts it to WETH and joins it into the vat
        ethJoin_join(ethJoin, urn, _fee);
        // Locks WETH amount into the CDP and generates debt
        frob(manager, cdp, toInt(msg.value - _fee), _getDrawDart(vat, jug, urn, ilk, wadD));
        // Moves the USB amount (balance in the vat in rad) to proxy's address
        ...
        if(_fee > 0) {
            USBJoinLike(USBJoin).USB().transferFrom(msg.sender, Config(config).getTreasury(), _feeBorrow);
        }
    }
    ...
    function lockGemAndDraw(
        ...
        address vat = ManagerLike(manager).vat();
        bytes32 ilk = ManagerLike(manager).ilks(cdp);
        uint _fee = amtC * getFee(ilk, Config.Fees.DEPOSIT_GEM) / decimal;
        uint _feeBorrow = wadD * getFee(ilk, Config.Fees.BORROW) / decimal;
        // Takes token amount from user's wallet and joins into the vat
        gemJoin_join(gemJoin, urn, amtC, transferFrom, _fee);
        // Locks token amount into the CDP and generates debt
        int lockAmount = toInt(convertTo18(gemJoin, amtC - _fee));
    ...
    function wipeAndFreeETH(
        ...
        address urn = ManagerLike(manager).urns(cdp);
        bytes32 ilk = ManagerLike(manager).ilks(cdp);
        uint _feeRepay = wadD * getFee(ilk, Config.Fees.PAYBACK);
        if(_feeRepay > 0) {
            USBJoinLike(USBJoin).USB().transferFrom(msg.sender, Config(config).getTreasury(), _feeRepay);
            wadD -= _feeRepay;
        }
        // Joins USB amount into the vat
        USBJoin_join(ilk, USBJoin, urn, wadD);
        // Paybacks debt to the CDP and unlocks WETH amount from it
        int dart = _getWipeDart(
            ManagerLike(manager).vat(), 
            VatLike(ManagerLike(manager).vat()).USB(urn), 
            urn, 
            ManagerLike(manager).ilks(cdp)
        );
        ...
        // Sends ETH back to the user's wallet
        uint _caculatedFee = getFee(ilk, Config.Fees.WITHDRAW);
        uint _fee = wadC * _caculatedFee / decimal;
        transferFeeWithdrawETH(msg.sender, Config(config).getTreasury(), wadC, _fee);
    }
    ...
    function wipeAllAndFreeETH(
        ...
        bytes32 ilk = ManagerLike(manager).ilks(cdp);
        (, uint art) = VatLike(vat).urns(ilk, urn);
        uint _feeRepay = art * getFee(ilk, Config.Fees.PAYBACK) / decimal;
        if(_feeRepay > 0) {
            USBJoinLike(USBJoin).USB().transferFrom(msg.sender, Config(config).getTreasury(), _feeRepay);
            art -= _feeRepay;
        }
        ...
        // Joins USB amount into the vat
        USBJoin_join(ilk, USBJoin, urn, _getWipeAllWad(vat, urn, urn, ilk, _feeRepay));
        // Paybacks debt to the CDP and unlocks WETH amount from it
        frob(
            manager,
            cdp,
    ...
    function wipeAndFreeGem(
        ...
        address urn = ManagerLike(manager).urns(cdp);
        bytes32 ilk = ManagerLike(manager).ilks(cdp);
        uint _feeRepay = wadD * getFee(ilk, Config.Fees.PAYBACK) / decimal;
        if(_feeRepay > 0) {
            USBJoinLike(USBJoin).USB().transferFrom(msg.sender, Config(config).getTreasury(), _feeRepay);
            wadD -= _feeRepay;
        }
        ...
        GemJoinLike(gemJoin).exit(msg.sender, wadC - _fee);
    }
