...
contract Cheops is ERC20, Ownable {
    ...
    uint baseFeesAmount; // base 10000
    uint baseFeesAmountForRefferal; // base 10000
    uint feesAmountRatioForRefferer; // base 10000
    uint PRPLL_fees; // base 10000
    uint sellThresholdAmount;
    ...
    bool[] public liquidityWallsInToken;
    ...
    QueuedWallsUpdate public queuedWalls;
    ...
    uint public tokensNeededForRefferalNumber;
    ...
    function updateExternalAddressAndPoolFee(INonfungiblePositionManager _nonfungiblePositionManager, ISwapRouter _swapRouter, IUniswapV3PoolState _pairPool, uint24 _mainPoolFee) external onlyOwner {
        nonfungiblePositionManager = _nonfungiblePositionManager;
        swapRouter = _swapRouter;
        pairPool = _pairPool;
    ...
    function setFeesAmount(uint _baseFeesAmount, uint _baseFeesAmountForRefferal, uint _feesAmountRatioForRefferer, uint _PRPLL_fees) external onlyOwner {
        require(_baseFeesAmount <= 3000);
        require(_baseFeesAmountForRefferal <= 3000);
    ...
    function _transferWithFees(address from, address to, uint256 amount) internal returns (bool) {
        if ((feesExcludedAddresses[from] == false && feesExcludedAddresses[to] == false) && taxedPool[from] && swapping == false) { // if the transfer is in a buy tx and "from" and "to" aren't excluded from fees
            uint feesAmount;
            ...
            _transfer(from, operator, feesForDevelopement);
    ...
    function init() external onlyOwner {
        ...
        uint cheopsAmount = balanceOf(address(this));
        ...
        for (uint i=0; i < liquidityWallsRanges.length; i++) {
            if (liquidityWallsInToken[i]) {
                liquidityAmount = (liquidityWallsRatios[i] * cheopsAmount) / 10000;
    ...
    function sellAllCheops() private {
    ...
    function addLiquidity(int24 minTick, int24 maxTick, uint amountCheops, uint amountQuote) private returns (uint tokenId, uint128 liquidity) {
        ...
        if (address(this) < pairToken) {
            token0 = address(this);
            token1 = pairToken;
            token0Amount = amountCheops;
            token1Amount = amountQuote;
        } else {
            token0 = pairToken;
            token1 = address(this);
            token0Amount = amountQuote;
            token1Amount = amountCheops;
        }
