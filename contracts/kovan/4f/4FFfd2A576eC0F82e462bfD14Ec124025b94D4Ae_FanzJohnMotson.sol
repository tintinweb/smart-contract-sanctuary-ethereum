...
contract FanzJohnMotson is Ownable, ERC721A, ReentrancyGuard, Pausable {
    ...
    bytes32 public constant PRESALE_TYPEHASH =
        keccak256("PrivateSale(address buyer,uint256 maxMint)");

    mapping(address => uint256) public privateSaleHasMinted;
    ...
    function _validateMint(
        ...
        require(
            msg.value >= (price_ * quantity_),
            "FanzJohnMotson: ETH sent is incorrect"
        );
    }
    ...
    function _validatePresaleSignature(
        uint256 maxMint_,
        ...
        bytes32 digest = keccak256(
            abi.encodePacked(
                "\x19\x01",
                DOMAIN_SEPARATOR,
                keccak256(abi.encode(PRESALE_TYPEHASH, _msgSender(), maxMint_))
            )
        );
    ...
    function privateSaleMint(
        ...
        require(
            (privateSaleHasMinted[_msgSender()] + quantity_) <= maxMint_,
            "FanzJohnMotson: Reached max mint"
        );
        _validateMint(privateSaleActive, privateSalePrice, quantity_);
        _validatePresaleSignature(maxMint_, signature_);

        privateSaleHasMinted[_msgSender()] += quantity_;
