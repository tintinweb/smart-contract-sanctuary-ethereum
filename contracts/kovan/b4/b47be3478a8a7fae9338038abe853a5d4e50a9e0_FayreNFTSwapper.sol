...
contract FayreNFTSwapper is OwnableUpgradeable, IERC721ReceiverUpgradeable, IERC1155ReceiverUpgradeable {
    /**
        E#1: only the seller can create the swap
        E#2: swap already finalized
        E#3: only the bidder or the seller can conclude the swap
        E#4: not enough liquidity provided for asset
        E#5: seller and bidder cannot be the same address
        E#6: unable to transfer liquidity
        E#7: wrong liquidity provided to finalize swap
    ...
    */
    ...
    event ExpireSwap(uint256 indexed swapId, address indexed seller, address indexed bidder);
    ...
    uint256 public swapExpirationDeltaTime;
    ...
    function onERC1155BatchReceived(address operator, address from, uint256[] calldata id, uint256[] calldata value, bytes calldata data) external pure returns (bytes4) {
    ...
    function setSwapExpirationDeltaTime(uint256 newSwapExpirationDeltaTime) external onlyOwner {
        swapExpirationDeltaTime = newSwapExpirationDeltaTime;
    }

    function initialize() public initializer {
        __Ownable_init();

        swapExpirationDeltaTime = 259200;
    }

    function createSwap(SwapRequest calldata swapRequest) public payable {
        require(swapRequest.seller == msg.sender, "E#1");
        require(swapRequest.seller != swapRequest.bidder, "E#5");

        for (uint256 i = 0; i < swapRequest.sellerAssetData.length; i++) {
            if (swapRequest.sellerAssetData[i].assetType == SwapAssetType.LIQUIDITY) {
                require(msg.value == swapRequest.sellerAssetData[i].amounts[0], "E#4");
            }
            else if (swapRequest.sellerAssetData[i].assetType == SwapAssetType.ERC20) {
                require(IERC20Upgradeable(swapRequest.sellerAssetData[i].contractAddress).transferFrom(swapRequest.seller, address(this), swapRequest.sellerAssetData[i].amounts[0]), "E#8");
            }
            else if (swapRequest.sellerAssetData[i].assetType == SwapAssetType.ERC721) {
                IERC721Upgradeable(swapRequest.sellerAssetData[i].contractAddress).safeTransferFrom(swapRequest.seller, address(this), swapRequest.sellerAssetData[i].ids[0], swapRequest.sellerAssetData[i].additionalData);
            }
            else if (swapRequest.sellerAssetData[i].assetType == SwapAssetType.ERC1155) {
                IERC1155Upgradeable(swapRequest.sellerAssetData[i].contractAddress).safeBatchTransferFrom(swapRequest.seller, address(this), swapRequest.sellerAssetData[i].ids, swapRequest.sellerAssetData[i].amounts, swapRequest.sellerAssetData[i].additionalData);
            }
        }

        swapsData[_currentSwapId].swapRequest = swapRequest;
        swapsData[_currentSwapId].start = block.timestamp;
        swapsData[_currentSwapId].expiration = swapsData[_currentSwapId].start + swapExpirationDeltaTime;
    ...
    function finalizeSwap(uint256 swapId, bool rejectSwap) public payable {
        SwapData storage swapData = swapsData[swapId];

        require(swapData.expiration > 0, "E#2");

        if (swapData.expiration < block.timestamp) {
            _cancelSwap(swapData);

            emit ExpireSwap(swapId, swapData.swapRequest.seller, swapData.swapRequest.bidder);

            return;
        }

        require(swapData.swapRequest.bidder == msg.sender || swapData.swapRequest.seller == msg.sender, "E#3");
        ...
        for (uint256 i = 0; i < swapData.swapRequest.bidderAssetData.length; i++) {
            if (swapData.swapRequest.bidderAssetData[i].assetType == SwapAssetType.LIQUIDITY) {
                require(msg.value == swapData.swapRequest.bidderAssetData[i].amounts[0], "E#7");
            }
        }

        swapData.expiration = 0;
