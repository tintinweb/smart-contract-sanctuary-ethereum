...
contract Wallet {
    ...
    event buyAssetsEvent(address, uint256, uint256);

    function buyAssets(uint256 _amount) public {
        address[] memory targetAssets = getTargetAssets();
        if (targetAssets.length > 0) {
            for (uint256 x = 0; x < targetAssets.length; x++) {
                uint256 targetPercentage = IPolybitOracle(oracleAddress)
                    .getTokenAddressTargetPercentage(
                        targetAssets[x],
                        riskWeighting
                    );
                emit buyAssetsEvent(
                    targetAssets[x],
                    targetPercentage,
                    (targetPercentage / 10000) * _amount
                );
            }
        }
    }
