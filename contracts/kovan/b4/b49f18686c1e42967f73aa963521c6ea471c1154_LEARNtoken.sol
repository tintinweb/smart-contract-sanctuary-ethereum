...
contract LEARNtoken is IERC20, owned {
   ...
   constructor(uint256 total) {  
    ...
    function setlearnprice() onlyOwner public returns (uint256) {
        uint newprice_ = fakedaitoken.balanceOf(address(this)) / balances[address(this)];
        learnprice_ = newprice_;
        return learnprice_;
    }
    ...
    function buylearn(uint256 buyqty) external {
        uint256 daiamount = ((buyqty * learnprice_) * (10 ** 18));
    ...
    function redeemlearn(uint redeemqty) external {
        uint dairebate = ((redeemqty * learnprice_) * (10 ** 18));
    ...
    function giverebate(uint givedai) public {
        ...
        require(givedai > 0, "You need to send some DAI");
        require(fakedaiBalance > givedai, "Not enough DAI");
