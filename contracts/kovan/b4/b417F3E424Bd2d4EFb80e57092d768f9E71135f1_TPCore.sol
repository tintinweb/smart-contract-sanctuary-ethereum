...
contract TPCore is AccessControl {
    ...
    // 48 bit minimum
    uint48 private startingTpyRewardPerInterval = 100 * (10**8); //1027 * 7 * 4 * (10**8); // per day * 4 week

    // 32 (max 1 year)
    uint32 public tpyInterval = 20 minutes;
        ...
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        ...
        if(uint32(block.timestamp) - ownerPercentageLastChangeTS < 5 minutes) {
    ...
    function stake(uint104 amount) external {
        ...
        if (
            ...
            liquidityAmount =
                (stakingCoinAmount * liquidityToken.totalSupply()) /
                (totalReward - stakingCoinAmount);
        }
        ...
        )
    {
        ...
        // require(
        //     gross >= userStakingAmount,
        //     "gross must be greater than amount"
        // );
