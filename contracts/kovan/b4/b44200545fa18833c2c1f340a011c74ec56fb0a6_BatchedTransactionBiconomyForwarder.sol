...
/**
 *
 * @title BatchedTransactionBiconomyForwarder
 *
 ...
 *
 */
contract BatchedTransactionBiconomyForwarder is ERC20ForwardRequestTypes,Ownable{
    ...
    function executeBatchEIP712(
        ERC20ForwardRequest[] calldata reqs,
        bytes32[] memory domainSeparators,
        bytes[] calldata sigs
    ) external returns (bytes[] memory results) {
        results = new bytes[](reqs.length);
        for(uint256 i = 0; i < reqs.length; i++) {
            _verifySigEIP712(reqs[i],domainSeparators[i],sigs[i]);
            _updateNonce(reqs[i]);
            (bool success, bytes memory result) = reqs[i].to.call{gas: reqs[i].txGas}(abi.encodePacked(reqs[i].data, reqs[i].from));
            assert(gasleft() > reqs[i].txGas / 63);
            _verifyCallResult(success,result,"Forwarded call to destination did not succeed");
            results[i] = result;
        }
    }
    ...
    function executeBatchPersonalSign(ERC20ForwardRequest[] calldata reqs,bytes[] calldata sigs)
    external 
    returns(bytes[] memory results){
        results = new bytes[](reqs.length);
        for(uint256 i = 0; i < reqs.length; i++) {
            _verifySigPersonalSign(reqs[i], sigs[i]);
            _updateNonce(reqs[i]);
            (bool success, bytes memory result) = reqs[i].to.call{gas : reqs[i].txGas}(abi.encodePacked(reqs[i].data, reqs[i].from));
            // Validate that the relayer has sent enough gas for the call.
            // See https://ronan.eth.link/blog/ethereum-gas-dangers/
