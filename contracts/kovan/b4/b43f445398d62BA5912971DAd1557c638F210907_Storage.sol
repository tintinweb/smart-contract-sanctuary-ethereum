...
contract Storage is IStorage, PermissionsController {
        ...
        returns (bool)
    {
        ...
        emit ShipmentDocumentAdded(shipmentId);
        return true;
    }
    ...
    function addTripHistory(uint256 shipmentId, TripHistory memory history) external onlyOperator returns (bool) {
        ...
        emit TripHistoryAdded(shipmentId);
        return true;
    }
    ...
    function createShipment(
        ...
        _shipmentUserRole[_shipmentCount][members.shipment] = Roles.SHIPMENT;
...
interface IStorage {
    ...
    event ShipmentDocumentAdded(uint256 indexed shipmentId);
    event ShipmentStatusUpdated(uint256 shipmentId, Status status);
    event TripHistoryAdded(uint256 indexed shipmentId);
}