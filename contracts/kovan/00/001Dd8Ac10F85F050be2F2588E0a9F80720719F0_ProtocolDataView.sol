...
contract ProtocolDataView {
    ...
    constructor(string memory _nativeSymbol, string memory _nativeName,ComptrollerInterface _comptroller, DistributionerInterface _distributioner) {
        comptroller = _comptroller;
        distributioner = _distributioner;
    ...
    function marketTokenMetadata(MarketTokenInterface marketToken) public returns (MarketMetadata memory){
        ...
        return MarketMetadata({
            ...
            supplySpeed: distributioner.rewardSupplySpeeds(address(marketToken)),
            borrowSpeed: distributioner.rewardBorrowSpeeds(address(marketToken)),
    ...
    function allMarketTokenMetadata(MarketTokenInterface[] memory marketTokens) public returns (MarketMetadata[] memory){
    ...
    function accountAllMarkets(address payable account) public view returns(MarketTokenInterface[] memory){
        MarketTokenInterface[] memory markets = getAllMarkets();
    ...
    function allMarketInfo() external returns (MarketMetadata[] memory, InterestRateModel[] memory){

        MarketTokenInterface[] memory markets = getAllMarkets();
        MarketMetadata[] memory metaData = allMarketTokenMetadata(markets);
    ...
    function accountAssetsInMarketInfo(address payable account) external returns (MarketMetadata[] memory, AccountLimits memory, MarketTokenBalances[] memory){

        AccountLimits memory accountLimits = getAccountLimits(account);
        MarketMetadata[] memory metaData = allMarketTokenMetadata(accountLimits.markets);
    ...
    function accountAllMarketInfo(address payable account) external returns (AccountLimits memory, MarketTokenBalances[] memory, MarketMetadata[] memory){
        MarketTokenInterface[] memory accountMarkets = accountAllMarkets(account);
    
        AccountLimits memory accountLimits = getAccountLimits(account);
        MarketTokenBalances[] memory balances = allMarketTokenBalances(accountMarkets, account);
        MarketMetadata[] memory metaData = allMarketTokenMetadata(accountMarkets);

        return (accountLimits, balances, metaData);
    }


    function pendingReward(address account) external returns (uint){
