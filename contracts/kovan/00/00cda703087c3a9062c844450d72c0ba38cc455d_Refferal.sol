...
contract Refferal is Ownable {
    ...
    function getUsers(uint _limit, uint skip) external view returns(Refer[] memory list, uint totalItem) {
        uint limit = _limit < users.length ? _limit : users.length;
        for(uint i = skip; i < limit; i++) {
            list[i] = users[i];
        }
        totalItem = users.length;
    }
    function getTotalUserByUp(address _refferrer, uint _limit, uint skip) public view returns(Refer[] memory list, uint totalItem) {
        uint limit = _limit < referreds[_refferrer].length ? _limit : referreds[_refferrer].length;
        for(uint i = skip; i < limit; i++) {
            list[i] = referreds[_refferrer][i];
        }
    ...
    function register(address _refferrer) external {
        ...
        referreds[_refferrer][referreds[_refferrer].length] = Refer(_msgSender(), _refferrer, block.timestamp);

        users[users.length] = Refer(_msgSender(), _refferrer, block.timestamp);
        isReferrer[_msgSender()] = true;
    }
