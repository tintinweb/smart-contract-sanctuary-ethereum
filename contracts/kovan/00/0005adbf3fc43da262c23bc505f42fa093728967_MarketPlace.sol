...
contract MarketPlace is MarketPlaceStorage, AccessControlUpgradeable, IERC1155ReceiverUpgradeable, OwnableUpgradeable {
    ...
    function cancelSell721(uint _index) external {
        ...
        uint last = (sellOrdersByCollection[sell.collection].length).sub(1);
    ...
    function removeCollectionToWhitelist(uint _index, bool _is721) external onlyRole(DEFAULT_ADMIN_ROLE) {
        ...
        if (_is721) {
            ...
            if (whitelisted721Collections.length == 1) {
                ...
                uint last = (whitelisted721Collections.length).sub(1);
            ...
            if (whitelisted1155Collections.length == 1) {
                ...
                uint last = (whitelisted1155Collections.length).sub(1);
    ...
    function processNativePayment(address _collection, uint _price, address _seller, address _creator, uint256 _royalties) internal {
        ...
        // Platform Fees
        uint biPercentFee = ITokenNFT(_collection).getFeesPercent();
        uint platformFee = _price.mul(biPercentFee).div(PERCENT_DIVIDER);
