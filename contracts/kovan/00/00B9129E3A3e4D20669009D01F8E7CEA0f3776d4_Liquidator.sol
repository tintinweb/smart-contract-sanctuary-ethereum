...
contract Liquidator is ILiquidator, Multicall, Manager {
    ...
    address immutable tokenTransferProxy;
    uint256 constant ACTIVE_MASK = 0xFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFEFFFFFFFFFFFFFF; // prettier-ignore

    constructor(address poolAddress, address swapperAddress, address TokenTransferProxy) {
        lendingPoolAddressProvider = poolAddress;
        augustusSwapper = swapperAddress;
    ...
    function swapOnUniswap(uint256 amountIn, uint256 amountOutMin, address[] calldata path)
    override
    payable
    public
    onlyManager {
        IAugustusSwapper swapper = IAugustusSwapper(augustusSwapper);
        IERC20 inputToken = IERC20(path[0]);
        if (inputToken.allowance(address(this), tokenTransferProxy) < amountIn) {
            inputToken.approve(tokenTransferProxy, 0);
            inputToken.approve(tokenTransferProxy, type(uint256).max);
        }
        swapper.swapOnUniswap(amountIn, amountOutMin, path);
    }
...
interface ILiquidator {
  ...
  function getBatchUserData(address[] calldata users) external view returns (UserData[] memory);
  function batchBalanceOf(address[] calldata users, address[] calldata tokens) external view returns (UserBalances[] memory);
  function balanceOf(address user, address token) external view returns (Balance memory);
  function swapOnUniswap(uint256 amountIn, uint256 amountOutMin, address[] calldata path) external payable;
  function liquidate(address _collateral, address _reserve, address _user, uint256 _purchaseAmount, bool _receiveAToken) external payable returns (address, address);
}
