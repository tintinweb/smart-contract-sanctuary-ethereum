...
import "./NFCSInterface.sol";
import "hardhat/console.sol";

import {IAddressBook} from "../interfaces/IAddressBook.sol";
import {IERC20PaymentStandard} from "../interfaces/IERC20PaymentStandard.sol";
    ...
    NFCSInterface
{
     ...
     * @ This function MUST be inclueded in each iteration of this contract, otherwise upgradeability will be lost!
     */
    ...
    event TokenMinted(address indexed _recipient, uint256 indexed _tokenId, address[] _addressBundle);
    // Event emitted when a token is reminted
    event TokenReMinted(address indexed _recipient, uint256 indexed _tokenId, address[] _addressBundle);

    uint public MAXIMUM_BORROW_LIMIT;
    IAddressBook public addressBook;
    string public investorsAddressLookupCategory;

    /// @dev sets the string used to get the list of investors from addressBook
    function setInvestorLookupCategory(string memory _newCateogry) external onlyOwner{
        investorsAddressLookupCategory = _newCateogry;
    }
    ...
    function mintToken(
        ...
        emit TokenMinted(msg.sender, tokenId, bundle);
    ...
    function storeBundle(uint256 tokenId, address[] memory bundle) private {
        ...
        require(
            _exists(tokenId),
            "NFCS: Cannot store bundle for nonexistent token"
        );
        require(
            !_bundleNonce[tokenId],
            "NFCS: This token already has an associated bundle"
        );
        ...
        returns (address[] memory)
    {
        require(
            _exists(tokenId),
            "NFCS: cannot get bundle of nonexistent token"
        );
        require(
            _bundleNonce[tokenId],
     ...
     */
    function getToken(address tokenOwner)
        public
        view
        override
        returns (uint256)
  ...
  */
    function _isApprovedOrOwner(address spender, uint256 tokenId)
        ...
        returns (bool)
    {
        require(_exists(tokenId), "NFCS: query for nonexistent token");
        address owner = ERC721Upgradeable.ownerOf(tokenId);
        return (spender == owner);
    }

    /**
     * @notice burns the token and removes bundle when reminting a token
     */
    function burnToken() internal {
        // Require that sender owns a token
        require(_mintedNonce[msg.sender], "ERC721: no token owned.");
        // Set status of all addresses in bundle to false once again
        for (
            uint256 i = 0;
            i < _tokenBundle[tokenOfOwnerByIndex(msg.sender, 0)].length;
            i++
        ) {
            _addressNonce[
                _tokenBundle[tokenOfOwnerByIndex(msg.sender, 0)][i]
            ] = false;
        }
        // Change to bundle Nonce to false; no bundle is stored against the token anymore.
        _bundleNonce[tokenOfOwnerByIndex(msg.sender, 0)] = false;
        // Remove the bundle stored against that token Id
        delete _tokenBundle[tokenOfOwnerByIndex(msg.sender, 0)];
        // Burn the token
        _burn(tokenOfOwnerByIndex(msg.sender, 0));
        // Change the addressNonce to false; address owns no token anymore.
        _mintedNonce[msg.sender] = false;
    }

    /**
     * @notice Mints a new token for recipient with given bundle after buring the existing one
     */
    function remintToken(
        ...
        // Require that the caller owns a token
        require(_mintedNonce[msg.sender], "NFCS: no token owned.");
        // Burn the existing token (sets address bundle status to false, then deletes the address bundle and the token )
        // Msg.sender is preserved due to internal function call.
        burnToken();
        // Mint a new token with the given bundle and send to caller
        require(!_mintedNonce[msg.sender], "Token already minted.");
        for (uint256 i = 0; i < bundle.length; i++) {
            // Check if any address is already part of a bundle
            require(!_addressNonce[bundle[i]], "Address already bundled.");
        ...
        _mintedNonce[msg.sender] = true;
        // Emit TokenMinted event
        emit TokenReMinted(msg.sender, tokenId, bundle);
        // Increase the token counter
        _tokenIdCounter.increment();
    }
   ...
   */
    ...
    function isUserIdAtBorrowLimit(uint _userId) external view override returns(bool){
        return(getUserTotalOustanding(_userId) >= MAXIMUM_BORROW_LIMIT);
    }

    function getUserTotalOustanding(uint _userId) public view returns(uint){
        address[] memory investors = addressBook.addressList(investorsAddressLookupCategory);
        address[] storage usersAddresses = _tokenBundle[_userId];
        uint oustandingBalance = 0;
        // for each investor
        for(uint i = 0; i < investors.length; i++){
            // for each of the user's addresses
            for(uint j = 0; j < usersAddresses.length; j++){
                //update outstanding global balance of that user at that investor
                oustandingBalance += 
                    IERC20PaymentStandard(investors[i])
                        .getUserGlobalOutstanding(usersAddresses[j]);
            }
        }
...
interface NFCSInterface {
    ...
    // Remits the token, receives the same params as mintToken()
    function remintToken(
        address[] memory bundle,
        bytes[] memory signatures,
        string memory _message,
    ...
    // Simply returns the current address bundle
    function version() external pure returns (string memory);

    // returns if user is at their borrow limit
    function isUserIdAtBorrowLimit(uint _userId) external view returns(bool);
}
...
interface IERC20PaymentStandard {
    ...
    function getUserGlobalOutstanding(address _user) external view returns(uint);
     ...
     */
    function getLoanInfo(uint256) external view returns (uint256, uint256);
