...
contract MarketV2 is Pausable {
    ...
    uint public validDuration; // in seconds
    ...
    function getTokenPrice(address token) internal view returns (uint) {
        ...
        (uint price, uint lastUpdate) = IOracle(oracle).getTokenPrice(token);
        require(lastUpdate + validDuration >= block.timestamp, "StalePrice");
        return price;
    }
    ...
    function batch(Actions[] calldata actions, bytes[] calldata data) public payable whenNotPaused {
        require(actions.length == data.length, "InvalidInput");
        for (uint i = 0; i < actions.length; i++) {
            if (actions[i] == Actions.Create) {
                (address lockedToken, address borrowedToken) = abi.decode(data[i], (address, address));
                createPosition(lockedToken, borrowedToken);
            } else if (actions[i] == Actions.Lock) {
                (uint pid, uint amount) = abi.decode(data[i], (uint, uint));
                lock(pid, amount);
            } else if (actions[i] == Actions.Unlock) {
                (uint pid, uint amount) = abi.decode(data[i], (uint, uint));
                unlock(pid, amount);
            } else if (actions[i] == Actions.Borrow) {
                (uint pid, uint amount) = abi.decode(data[i], (uint, uint));
                borrow(pid, amount);
            } else if (actions[i] == Actions.Repay) {
                (uint pid, uint amount) = abi.decode(data[i], (uint, uint));
                repay(pid, amount);
            } else if (actions[i] == Actions.Liquidate) {
                (address user, uint pid, uint amount) = abi.decode(data[i], (address, uint, uint));
                liquidate(user, pid, amount);
            } else {
                revert("InvalidInput");
            }
        }
    ...
    function setValidDuration(uint _validDuration) public onlyOwner {
