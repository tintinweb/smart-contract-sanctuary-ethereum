...
function swapMaticToUSDCTEST(uint _amount) public payable returns (uint) {
        address[] memory path = new address[](2);
        path[0] = uniswapV2Router02.WETH(); 
        path[1] = address(usdc_token);
...
function swapMaticToUSDCTEST2() public payable  {
        address[] memory path = new address[](2);
        path[0] = uniswapV2Router02.WETH(); 
        path[1] = address(usdc_token);

        uniswapV2Router02.swapExactETHForTokens{ value:  msg.value }(0, path, msg.sender, block.timestamp+300);
    }

function swapMaticToDai() public payable  {
    address[] memory path = new address[](2);
        path[0] = uniswapV2Router02.WETH(); 
        path[1] = 0x9DC7B33C3B63fc00ed5472fBD7813eDDa6a64752;
