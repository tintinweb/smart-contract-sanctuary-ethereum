...
contract WLNFT_MintPass is ERC721A, ChainlinkClient, Ownable, ContextMixin, NativeMetaTransaction {
    ...
    bytes32 private oracleJob = "bc746611ebee40a3989bbe49e12a02b9";
    address private oracleAddress = 0xc57B33452b4F7BB189bB5AfaE9cc4aBa1f7a4FD8;
    uint256 private oraclePayment = 0.1 * 10 ** 18;
    ...
    function getPrice() public view returns (uint256) {
        if (totalSupply() >= 6001) {
            return basePrice * 4;
        } else if (totalSupply() >= 3001) {
            return basePrice * 3;
        } else if (totalSupply() >= 1001) {
            return basePrice * 2;
        } else {
            return basePrice;
        }
    ...
    function fulfill(bytes32 _requestId, bool _isWhitelisted) public recordChainlinkFulfillment(_requestId) {
        if (_isWhitelisted) {
            ...
            payable(requests[_requestId].addr).transfer(requests[_requestId].value);
            delete requests[_requestId];
            require(_isWhitelisted, "This address was not on the whitelist.");
        }
    }
