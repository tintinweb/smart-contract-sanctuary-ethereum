...
interface ComptrollerInterface {
    ...
    function redeemGuardianPaused(address market) external view returns(bool);
    function borrowGuardianPaused(address market) external view returns(bool);
    function repayBorrowGuardianPaused(address market) external view returns(bool);
    function transferGuardianPaused(address market) external view returns(bool);
    function seizeGuardianPaused(address market) external view returns(bool);
...
// File contracts/distribution/DistributionerInterface.sol
...
// File contracts/ratemodel/JumpRateModelInterface.sol
...
contract ProtocolDataView {
    ...
    function marketTokenMetadata(ComptrollerInterface comptroller, MarketTokenInterface marketToken) public returns (MarketMetadata memory){
        ...
        (uint supplySpeed, uint borrowSpeed) = _getRewardSpeeds(comptroller, address(marketToken));
        ...
        return MarketMetadata({
            ...
            mintPaused: comptroller.mintGuardianPaused(address(marketToken)),
            redeemPaused: comptroller.redeemGuardianPaused(address(marketToken)),
            borrowPaused: comptroller.borrowGuardianPaused(address(marketToken)),
            repayBorrowPaused: comptroller.repayBorrowGuardianPaused(address(marketToken)),
            transferPaused: comptroller.transferGuardianPaused(address(marketToken)),
            seizePaused: comptroller.seizeGuardianPaused(address(marketToken)),
            isListed: isListed,
    ...
    function accountAssetsInMarketInfo(ComptrollerInterface comptroller, address payable account) external returns (MarketMetadata[] memory, AccountLimits memory, MarketTokenBalances[] memory){
    ...
    function pendingReward(ComptrollerInterface comptroller, address account) external returns (uint){

        DistributionerInterface distributioner = DistributionerInterface(comptroller.distributioner());

        IERC20 rewardToken = IERC20(distributioner.rewardToken());
    ...
    function _getRewardSpeeds(ComptrollerInterface comptroller, address marketToken) internal view returns(uint supplySpeed,uint borrowSpeed) {

         DistributionerInterface distributioner = DistributionerInterface(comptroller.distributioner());

        try distributioner.rewardSupplySpeeds(marketToken) returns(uint _supplySpeed){
            supplySpeed = _supplySpeed;
        }catch {
            supplySpeed = 0;
        }

        try distributioner.rewardBorrowSpeeds(marketToken) returns(uint _borrowSpeed){
            borrowSpeed = _borrowSpeed;
        }catch {
            borrowSpeed = 0;
        }
