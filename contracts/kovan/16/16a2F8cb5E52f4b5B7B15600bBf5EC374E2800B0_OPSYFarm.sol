...
// farm supports optional fee fee on both the staked token and the acc-rewards

contract OPSYFarm is Ownable, Pausable {
    ...
    struct PoolInfo {
        ...
        uint256 withdrawFee;        // Fee of amount which will go to fee's wallet when users unstake
        uint256 claimFee;           // Fee of amount which will go to fee's wallet when users claim
        uint256 stakedAmount;       // Amount of @lpToken staked in this pool
    }

    // Address of the ERC20 Token contract.
    IERC20 public opsyERC20;
    // amount of OPSY deposited
    IHODLRewardDistributor distibutor;
    bool enableDistributor;

    function updateDistributor() external onlyOwner{
        distibutor = IOPSY(address(opsyERC20)).hodlRewardDistributor();
    }

    function setShareControlEnabled(bool shareControlerEnabled) external onlyOwner {
        enableDistributor = shareControlerEnabled;
    }

    mapping (address => uint256) opsyShare;
    ...
    constructor(IERC20 _erc20, uint256 _rewardPerBlock, uint256 _startBlock, address _feeWalletAddr) {
        opsyERC20 = _erc20;
        ...
        //transferOwnership(0xA0BC90c020DF37029f6a5535F6d571a5736e3e69);
    }
    ...
    function initializeFarm(
        ...
        init = _rewardToken != IERC20(address(0)) &&
        endBlock != 0 && _startBlock != 0;
    }
    ...
    function changeFeeWallet(address _newFeeWallet) external onlyOwner {
        require(_newFeeWallet != address(0), "changeFeeWallet: can't be zero address");
    ...
    function fund(uint256 _amount) external onlyOwner onlyAfterInit {
        ...
        uint256 balanceBefore = opsyERC20.balanceOf(address(this));

        opsyERC20.transferFrom(address(msg.sender), address(this), _amount.sub(leftOver));

        uint256 diff = opsyERC20.balanceOf(address(this)) - balanceBefore;
    ...
    function deposit(uint256 _pid, uint256 _amount) external onlyAfterInit whenNotPaused{
        ...
        if (userAmount > 0) {
            ...
            uint256 feeWalletAmount = pendingAmount * poolInfo[_pid].claimFee / 1000;
            if (feeWalletAmount > 0)
                erc20Transfer(feeWallet, feeWalletAmount, true);
            erc20Transfer(msg.sender, pendingAmount - feeWalletAmount, true);
        }
        ...
        if(lpToken == opsyERC20 && enableDistributor){
            opsyShare[msg.sender] += userAmount;
            distibutor.setShare(msg.sender, opsyERC20.balanceOf(msg.sender));
        }
    ...
    function withdraw(uint256 _pid, uint256 _amount) external whenNotPaused {
        ...
        uint256 feeRewardAmount = pendingAmount.mul(poolInfo[_pid].claimFee).div(1000);
        if (feeRewardAmount > 0)
            erc20Transfer(feeWallet, feeRewardAmount, true);
        erc20Transfer(msg.sender, pendingAmount - feeRewardAmount, true);

        userAmount = userAmount.sub(_amount);
        userInfo[_pid][msg.sender].rewardDebt = userAmount.mul(accERC20PerShare).div(1e36);
        userInfo[_pid][msg.sender].amount = userAmount;

        uint256 feeWalletAmount = _amount * poolInfo[_pid].withdrawFee / 1000;
        if (feeWalletAmount > 0)
            lpToken.safeTransfer(feeWallet, feeWalletAmount);
        lpToken.safeTransfer(address(msg.sender), _amount - feeWalletAmount);
        poolInfo[_pid].stakedAmount -= _amount;

        if(lpToken == opsyERC20 && enableDistributor){
            opsyShare[msg.sender] -= _amount;
            IHODLRewardDistributor(address(opsyERC20)).setShare(msg.sender, opsyERC20.balanceOf(msg.sender));
        }
    ...
    function emergencyWithdraw(uint256 _pid) external whenPaused {
        ...
        if(pool.lpToken == opsyERC20 && enableDistributor){
            opsyShare[msg.sender] -= userAmount;
            IHODLRewardDistributor(address(opsyERC20)).setShare(msg.sender, opsyERC20.balanceOf(msg.sender));
        }
    }
    ...
    function erc20Transfer(address _to, uint256 _amount, bool _isPayout) internal {
        opsyERC20.transfer(_to, _amount);
        if (_isPayout)
            paidOut += _amount;
    }
    ...
    */
    ...
    function changeRewardPerBlock(uint256 _rewardPerBlock) external onlyOwner onlyAfterInit {
        ...
        if (leftoverRewards > 0) {
            ...
            erc20Transfer(feeWallet, leftoverRewards, false);
