...
pragma solidity ^0.8.3;
contract Scientist is ERC721Tradable, AScientistRepository {
    using Counters for Counters.Counter;
    ...
    function _burn(uint tokenId) internal override {
        ScientistData.Scientist memory scientist = _getScientist(msg.sender, tokenId);
    ...
    function _transfer(address from, address to, uint tokenId) internal override {
        ScientistData.Scientist memory scientist = _getScientist(from, tokenId);        
        require(scientist.onSale == false, "Scientist is on sale");
