...
// contract SinglePlayerFlipKing is VRFConsumerBase {
...
contract FlipKing {
    ...
    function withdrawWinnings() external payable {
        ...
        whitelist.recieve{value: rewardPerCent * 3}();
        (bool successUser, ) = msg.sender.call{value: (rewardPerCent * 97)}("");
        require(successUser, "Transfer to user failed");
        totalClaimedRewardsAddress[msg.sender] += rewardPerCent * 97;
    }
    ...
    function placeBet(uint256 _side) external payable {
        ...
        queueSize += 1;
        address tempAff = whitelist.affiliateAddress(msg.sender);
        uint256 affiliateReward = (msg.value - (msg.value % 100000)) / 2000;
        if (tempAff != address(0)) {
            myth.mint(tempAff, affiliateReward);
            myth.mint(
                msg.sender,
