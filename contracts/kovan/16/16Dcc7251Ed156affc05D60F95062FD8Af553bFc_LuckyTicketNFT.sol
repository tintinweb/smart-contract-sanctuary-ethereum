    ...
    VRFConsumerBase
{
    ...
    function getListOwnerCrownNFT() external view returns (uint256[] memory) {
        uint256[] memory listCrown;
        uint256 counter = 0;
        uint256[] memory ownerCrown = ownerWinningCrownNFT[msg.sender];
        for (uint256 i = 0; i < ownerCrown.length; i++) {
            if (ownerCrown[i] != 0) {
                listCrown[counter] = ownerWinningCrownNFT[msg.sender][i];
                counter++;
            }
        }
        return listCrown;
