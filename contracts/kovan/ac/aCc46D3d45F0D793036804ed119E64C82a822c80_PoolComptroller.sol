...
// File contracts/misc/isolation/PoolComptroller.sol


pragma solidity ^0.8.0;


contract PoolComptroller is Comptroller {

    using AddressUpgradeable for address;

    DeployFactoryForPoolInterface public deployFactory;
    PoolManagerInterface public poolManager;
    address[] public allowAddresses;

    function initialize(PriceOracle _oracle, uint _closeFactorMantissa, uint _liquidationIncentiveMantissa, DeployFactoryForPoolInterface _deployFactory, PoolManagerInterface _poolManager) public initializer{
        Comptroller.initialize(_oracle, _closeFactorMantissa, _liquidationIncentiveMantissa);
        deployFactory = _deployFactory;
        poolManager = _poolManager;
    }

    function hasOwnerRights() public override view returns(bool) {
        return msg.sender == owner() || msg.sender == address(poolManager);
    }

    function deployMarket(address underlying, address interestRateModel, uint reserveFactorMantissa, uint collateralFactorMantissa) public onlyOwner {

        (uint256 poolId, string memory poolName, address poolCreator, address fundManage) = poolManager.getPoolBaseInfoByComptroller(address(this));
        address marketToken = deployFactory.createMarket(address(this), underlying, interestRateModel, reserveFactorMantissa, poolId, poolName, poolCreator, fundManage);

        _supportMarket(MarketTokenInterface(marketToken));
        _setCollateralFactor(MarketTokenInterface(marketToken), collateralFactorMantissa);
    }

    function deployDistributionerManager() public onlyOwner {
        address _distributioner = deployFactory.createDistributionerManager(msg.sender, address(this));
        _setDistributioner(_distributioner);
    }

    function transferVerify(address marketToken, address src, address dst, uint transferTokens) public override {
        super.transferVerify(marketToken, src, dst, transferTokens);
        poolManager.setAccountPools(dst);
        poolManager.setAccountPools(src);
    }

    function setAllowAddresses(address[] memory _allowAddresses) public {
        require(hasOwnerRights(), "only owner can setAllowAddresses");
        allowAddresses = _allowAddresses;
    }

    function isInAllowAddresses(address account) public view returns(bool) {
        uint length = allowAddresses.length;
        if(length > 0){
            for(uint i = 0; i < length; i++){
                if(allowAddresses[i] == account){
                    return true;
                }
            }
            return false;
