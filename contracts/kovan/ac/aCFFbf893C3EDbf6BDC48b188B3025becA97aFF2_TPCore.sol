...
contract TPCore is AccessControl {
    ...
    function registerTPY(address staker, uint104 stakingCoinAmount) private {
        ...
        if(userStaking[staker].amount > 0) {
            Staking storage stake = userStaking[staker];
            if(stake.intervalNumber == nextIntervalNumber){ 
                if(stake.pendingAmount == 0) {
                    stake.amount += stakingCoinAmount;
                }
                else {
                    stake.pendingAmount += stakingCoinAmount;
                } 
            }
            else{
                uint136 currentReward = estimateTPYReward(staker);
                stake.calculatedTPYs = currentReward;
                stake.amount += stake.pendingAmount;
                stake.pendingAmount = stakingCoinAmount;
                stake.intervalNumber = nextIntervalNumber;
            }
