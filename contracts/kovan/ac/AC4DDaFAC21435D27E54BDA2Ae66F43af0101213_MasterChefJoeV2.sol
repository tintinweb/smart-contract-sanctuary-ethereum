...
contract MasterChefJoeV2 is Ownable {
    ...
    uint256 public devPercent; //权重 duke
    ...
    // 更新矿池，pid如果没有可以自动创建，但是没有LP地址啊，这个需要校验一下才行。 duke
    function set(
        uint256 _pid,
        uint256 _allocPoint,
        IRewarder _rewarder,
        ...
        )
    {
        ...
        if (address(pool.rewarder) != address(0)) { //显示后三个字段。
            (bonusTokenAddress, bonusTokenSymbol) = rewarderBonusTokenInfo(_pid);
            //这说明rewarder得是一个farm合约啊 duke
    ...
    function updatePool(uint256 _pid) public {
        PoolInfo storage pool = poolInfo[_pid];
        if (block.timestamp <= pool.lastRewardTimestamp) { //说明还没开始。duke
            return;
        }
        uint256 lpSupply = pool.lpToken.balanceOf(address(this)); //不存在的话直接失败了。需要确认一下。
        if (lpSupply == 0) {
            pool.lastRewardTimestamp = block.timestamp;
            return;
        }

        //每一个pool都会对所有的参与方（dev，treasury，investor，当前lp合约）等进行分配joe奖励。
        uint256 multiplier = block.timestamp.sub(pool.lastRewardTimestamp);
