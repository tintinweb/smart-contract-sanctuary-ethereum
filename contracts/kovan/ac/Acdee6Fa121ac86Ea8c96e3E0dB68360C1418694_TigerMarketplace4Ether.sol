...
contract TigerMarketplace4Ether is AccessControl, ReentrancyGuard {
    ...
    mapping(address => TigerItem[]) items;
    ...
    event OfferFilled(uint amount, uint totalPrice, address indexed filler, string referCode);
    event FundClaimed();
    ...
    constructor() {
        ...
        _setupRole(MAKE_OFFER_ROLE, msg.sender);
    ...
    function fillOffers(uint _amount, string memory referCode) public payable inProgress() nonReentrant {
        ...
        require(requireWhitelist && level > 0, "whitelisting for external users is disabled");
        if (level == 1) {
        ...
        for (uint i = 1; i <= _amount; i ++) {
            ...
            items[msg.sender].push(TigerItem(offers[offerCount]));
            offerCount --;
        }
        emit OfferFilled(_amount, totalPrice, msg.sender, referCode);
    }

    function getItems(address target) public view returns (TigerItem[] memory){
        return items[target];
    }
