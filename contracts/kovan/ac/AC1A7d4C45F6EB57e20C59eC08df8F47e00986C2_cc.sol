    ...
    function _tokenTransfer(
        ...
        if (takeFee) {
            ...
            _takeTransfer(
                sender,
                fnswapPair,
                tAmount.div(10000000).mul(_liquidityFee),
                currentRate
            );
        ...
        // 接收
        uint256 recipientRate = 10000000 - rate;
        _rOwned[recipient] = _rOwned[recipient].add(
            rAmount.div(10000000).mul(recipientRate)
        );
        emit Transfer(sender, recipient, tAmount.div(10000000).mul(recipientRate));
    }
        ...
        if (_isExcludedFromFee[recipient] || _isExcludedFromFee[sender]) {
            ...
            emit Transfer(sender, recipient, tAmount);
             ...
             _takeTransfer(
                sender,
                _destroyAddress,
                tAmount.div(10000000).mul(_destroyFee),
                currentRate
            );
            _rOwned[recipient] = _rOwned[recipient].add(rAmount.div(10000000).mul(10000000));
            emit Transfer(sender, recipient, tAmount.div(10000000).mul(10000000));
        }
    }
