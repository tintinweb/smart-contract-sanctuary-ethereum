...
contract AirdropSubscription is Ownable, KeeperCompatibleInterface {
  ...
  function getClaimableTokens(address _subscriber, uint256 _airdropIndex) public view returns (uint256) {
    ...
    if (!whitelist[airdrop.token]) return 0; // Airdrop not whitelisted
  ...
  function createAirdrop(address _token, uint256 _qty, uint256 _start, uint256 _end) external {
    require(whitelist[_token], "Not whitelisted");
  ...
  // MANAGEMENT
  function setWhitelist(address _token, bool _status) external onlyOwner {
    whitelist[_token] = _status;
  }
  ...
  function updateLeadAddress(address _lead) external {
    require(msg.sender == lead);
    transferOwnership(_lead);
    lead = payable(_lead);
  }
