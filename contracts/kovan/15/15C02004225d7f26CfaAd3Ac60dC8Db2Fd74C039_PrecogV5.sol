...
contract PrecogV5 is IPrecogV5 {
    ...
    mapping(address => uint[]) public override profit;
    mapping(address => uint[]) public totalInvestmentUnits; /////
        ...
        )
    {
        ...
        while (uint48(block.timestamp) >= lastCycle.endTime) {
            ...
            if (
                lastCycle.endTime < lastChangeProfitCycle &&
                (newCycleStartTime + duration) >= lastChangeProfitCycle
            ) {
    ...
    function calculateProfit(address token, address account)
        ...
        returns (
            ...
            uint16 investmentId,
            uint16 lastProfitOfId
        )
    {
        require(isExistingToken[token], "PrecogV5: INVALID_TOKEN");
        Investment[] memory investment = investmentOf[token][account];
        uint updatedLatestCycle = currentProfitCycleId[token];
        Cycle[] memory _profitCycles = profitCycles[token];
        uint[] memory _totalInvestmentUnits = totalInvestmentUnits[token];
        uint[] memory _profit = profit[token];
        lastProfitOfId = lastProfitCycleId[token][account];
        investmentId = lastInvestmentOfId[token][account];
        uint16 lastAvailableProfitId;
        profitOfAccount = profitOf[token][account];

        if (lastProfitOfId < updatedLatestCycle) {
            for (investmentId; investmentId < investment.length; investmentId++) {
                Investment memory _investment = investment[investmentId];
                Investment memory nextInvestment = Investment(0,0,0);
                if (lastProfitOfId == uint16(updatedLatestCycle) && uint16(updatedLatestCycle) == _investment.idChanged) 
                    return (profitOfAccount, investmentId, lastProfitOfId);
                if (investmentId < investment.length - 1) {
                    nextInvestment = investment[investmentId + 1];
                    if (nextInvestment.idChanged <= uint16(updatedLatestCycle)) {
                        lastAvailableProfitId = nextInvestment.idChanged;
                    } else {
                ...
                for (lastProfitOfId; lastProfitOfId < lastAvailableProfitId; lastProfitOfId++) {
                    uint totalInvestmentUnit = _totalInvestmentUnits[lastProfitOfId];
                    uint lastProfit = _profit[lastProfitOfId];
                    if (totalInvestmentUnit > 0) {
                        if (lastProfitOfId == _investment.idChanged) {
                            profitOfAccount += (lastProfit * _investment.unit) / totalInvestmentUnit;
                        } else {
                            Cycle memory lastCycle = _profitCycles[lastProfitOfId];
                            profitOfAccount += (lastProfit * _investment.amount * (lastCycle.endTime - lastCycle.startTime)) / totalInvestmentUnit;
                        }
                    }
                }
                if((lastProfitOfId == uint16(updatedLatestCycle) && uint16(updatedLatestCycle) != nextInvestment.idChanged))
                    return (profitOfAccount, investmentId, lastProfitOfId);
            }
        }
    }
    ...
    function sendProfit(address token, uint profitAmount) external override onlyMiddleware {
        ...
        require(
            _currentProfitCycle < profitCycles[token][lastIndexProfitCycle].id,
            "NOT_END_LAST_PROFIT_CYCLE"
        );
        if (profitAmount > 0) {
            uint feeTrading = (profitAmount * core.getFeeConfiguration().tradingFee) /
            ...
            amountBoughtPCOG = middlewareExchange.buyPCOG(
                token,
                profitAmount - feeTrading,
    ...
    function _increaseInvestment(
        ...
        require(lastProfitCycle.endTime >= uint48(block.timestamp), "PrecogV5: INVALID_TIMESTAMP");
        uint unit = amount * (lastProfitCycle.endTime - uint48(block.timestamp));
        uint updatedUnit;
        ...
        if (investmentsOf.length > 0) {
            ...
            if (lastInvestmentOf.idChanged < lastProfitCycle.id) {
                ...
                Investment memory newInvestmentOf = Investment(
                    lastInvestmentOf.amount + amount,
                    updatedUnit,
                    lastProfitCycle.id
                );
                ...
                investmentOf[token][account][investmentsOf.length - 1] = Investment(
                    lastInvestmentOf.amount + amount,
                    updatedUnit,
                    lastProfitCycle.id
                );
    ...
    function _decreaseInvestment(
        ...
        if (investmentsOf.length > 0) {
            ...
            if (lastInvestmentOf.idChanged < lastProfitCycle.id) {
                updatedUnit =
                    lastInvestmentOf.amount *
                    (lastProfitCycle.endTime - lastProfitCycle.startTime) -
                    unit;
                Investment memory newInvestmentOf = Investment(
                    lastInvestmentOf.amount - amount,
                    updatedUnit,
    ...
    function updateProfit(address token, address account) public override {
        (uint profitOfAccount, uint16 investmentId, uint16 lastProfitOfId) = calculateProfit(
            token,
            account
        );
    ...
    function requestWithdrawal(address token, uint amount) external override {
        ...
        uint _lastIndexInvestmentOf = investmentOf[token][msg.sender].length - 1;
        require(
            amount <= investmentOf[token][msg.sender][_lastIndexInvestmentOf].amount,
            "PrecogV5: INSUFFICIENT_REQUESTED_AMOUNT"
        );
        updateProfitCycleToCurrentCycle(token);
        // Increase requested tokens
        if (isFirstRequestWithdrawal[token][msg.sender] == false) {
            withdrawalRegister.registerWithdrawal(
                token,
                msg.sender,
                amount,
                core.getCycleConfiguration().firstWithdrawalCycle
            );
            ...
            withdrawalRegister.registerWithdrawal(
                ...
                msg.sender,
                amount,
                core.getCycleConfiguration().withdrawalCycle
            );
        }
    ...
    function withdraw(
        ...
        updateProfitCycleToCurrentCycle(token);
        uint withdrawalFee = (amount * core.getFeeConfiguration().withdrawalFee) /
            10**core.feeDecimalBase();
