...
contract StrudelTreasury is Ownable {
    ...
    enum MANAGING { RESERVEDEPOSITOR, RESERVESPENDER, RESERVETOKEN, RESERVEMANAGER, LIQUIDITYDEPOSITOR, LIQUIDITYTOKEN, LIQUIDITYMANAGER, DEBTOR, REWARDMANAGER, SORKAN }
    ...
    uint public sORKANQueue; // Delays change to sORKAN address
    ...
    /**
        @notice allow approved address to deposit an asset for ORKAN
        @param _amount uint
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        // mint ORKAN needed and store amount of rewards for distribution
        send_ = value.sub( _profit );
        IERC20Mintable( ORKAN ).mint( msg.sender, send_ );
    ...
    /**
        @notice allow approved address to burn ORKAN for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOf( _token, _amount );
        IORKANERC20( ORKAN ).burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( sORKAN ).balanceOf( msg.sender ); // Can only borrow against sORKAN held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with ORKAN
        @param _amount uint
     */
    function repayDebtWithORKAN( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved" );

        IORKANERC20( ORKAN ).burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, ORKAN, _amount, _amount );
    }
    ...
    function mintRewards( address _recipient, uint _amount ) external {
        ...
        IERC20Mintable( ORKAN ).mint( _recipient, _amount );
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( IERC20( ORKAN ).totalSupply().sub( totalDebt ) );
    }
    ...
    /**
        @notice returns ORKAN valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match ORKAN decimals
            value_ = _amount.mul( 10 ** IERC20( ORKAN ).decimals() ).div( 10 ** IERC20( _token ).decimals() );
    ...
    function queue( MANAGING _managing, address _address ) external onlyManager() returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.SORKAN ) { // 9
            sORKANQueue = block.number.add( blocksNeededForQueue );
        } else return false;
