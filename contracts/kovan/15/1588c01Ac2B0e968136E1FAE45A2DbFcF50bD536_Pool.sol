...
contract Pool is Ownable, ReentrancyGuard, Initializable {
    ...
    mapping(address => uint256) public mintXftmBalances;
    mapping(address => uint256) public redeemFantasmBalances;
    mapping(address => uint256) public redeemFtmBalances;
    ...
    function initialize(
        ...
        require(_xftm != address(0), "invalidAddress");
        require(_fantasm != address(0), "invalidAddress");
    ...
    {
        return (
            unclaimedXftm, // unclaimed amount of Xftm
            unclaimedFtm, // unclaimed amount of FTM
            unclaimedFantasm, // unclaimed amount of Fantasm
            mintPaused,
            redeemPaused
        );
    }
        ...
        )
    {
        ...
        require(_fantasmPrice > 0, "Invalid Fantasm price");
    ...
    function calcExcessFtmBalance() public view returns (uint256 _excessFtmBalance) {
        ...
        if (_usableFtmBal > _requiredFtmBal) {
            _excessFtmBalance = _usableFtmBal - _requiredFtmBal;
        }
    }
    ...
    function mint(uint256 _fantasmIn, uint256 _minXftmOut) external payable nonReentrant {
        ...
        require(_minXftmOut <= _xftmOut, "slippage");
        require(_minFantasmIn <= _fantasmIn, "Not enough Fantasm input");
        require(treasury.maxXftmSupply() >= xftm.totalSupply() + _xftmOut, "> Xftm supply limit");

        WethUtils.wrap(_ftmIn);
        lastMinted[_minter] = block.number;

        if (_xftmOut > 0) {
            mintXftmBalances[_minter] = mintXftmBalances[_minter] + _xftmOut;
    ...
    function zap(uint256 _minXftmOut) external payable nonReentrant {
        ...
        require(_xftmOut >= _minXftmOut, "slippage");

        WethUtils.wrap(_ftmIn);
        if (_fantasmOut > 0 && _ftmSwapIn > 0) {
            uint256 _minFantasmOut = (_fantasmOut * (PRECISION - helper.swapSlippage())) / PRECISION;
            WethUtils.weth.safeIncreaseAllowance(address(helper), _ftmSwapIn);
            helper.zap(_ftmSwapIn, _minFantasmOut);
        }
        ...
        emit ZapMint(_minter, _xftmOut, _ftmIn, _fee);
    }
    ...
    function redeem(
        ...
        require(_ftmOut <= usableFtmBalance(), "> FTM balance");
        require(_minFtmOut <= _ftmOut && _minFantasmOut <= _fantasmOut, ">slippage");

        if (_ftmOut > 0) {
            redeemFtmBalances[_redeemer] = redeemFtmBalances[_redeemer] + _ftmOut;
            unclaimedFtm = unclaimedFtm + _ftmOut;
        }

        if (_fantasmOut > 0) {
            redeemFantasmBalances[_redeemer] = redeemFantasmBalances[_redeemer] + _fantasmOut;
            unclaimedFantasm = unclaimedFantasm + _fantasmOut;
        }

        lastRedeemed[_redeemer] = block.number;

        // Move all external functions to the end
        IXftm(xftm).burn(_redeemer, _xftmIn);
        if (_fee > 0) {
            WethUtils.transfer(address(treasury), _fee);
        }

        emit Redeem(_redeemer, _xftmIn, _ftmOut, _fantasmOut, _fee);
    }
    ...
    function collect() external nonReentrant {
        ...
        require(lastMinted[_sender] < block.number, "<minting_delay");
        require(lastRedeemed[_sender] < block.number, "<redeem_delay");
        ...
        if (mintXftmBalances[_sender] > 0) {
            _xftmAmount = mintXftmBalances[_sender];
            mintXftmBalances[_sender] = 0;
            unclaimedXftm = unclaimedXftm - _xftmAmount;
            _sendXftm = true;
        }

        if (redeemFantasmBalances[_sender] > 0) {
            _fantasmAmount = redeemFantasmBalances[_sender];
            redeemFantasmBalances[_sender] = 0;
            unclaimedFantasm = unclaimedFantasm - _fantasmAmount;
            _sendFantasm = true;
        }

        if (redeemFtmBalances[_sender] > 0) {
            _ftmAmount = redeemFtmBalances[_sender];
            redeemFtmBalances[_sender] = 0;
    ...
    function toggle(bool _mintPaused, bool _redeemPaused) external onlyOwner {
        ...
        emit UpdateMintAndRedeem(_mintPaused, _redeemPaused);
    }

    function reduceExcessFtm(uint256 _amount) external onlyOwner {
        uint256 _excessFtmBal = calcExcessFtmBalance();
        require(_amount <= _excessFtmBal, "The amount is too large");
        require(address(treasury) != address(0), "Invalid treasury");
        WethUtils.transfer(address(treasury), _amount);
    }

    function setOracle(IFantasmOracle _oracle) external onlyOwner {
    ...
    event UpdateMintAndRedeem(bool _mintPaused, bool _redeemPaused);
...
interface IPoolHelper {
    function swapSlippage() external pure returns (uint256);

    function zap(uint256 _ftmIn, uint256 _minFantasmOut) external returns (uint256);
}
