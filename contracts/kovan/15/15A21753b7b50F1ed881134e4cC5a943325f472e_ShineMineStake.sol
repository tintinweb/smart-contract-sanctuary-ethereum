...
contract ShineMineStake is Initializable, OwnableUpgradeable, UUPSUpgradeable, ReentrancyGuardUpgradeable, PausableUpgradeable {
...
*/
    ...
    function stake(string calldata pool, uint256 amount) public nonReentrant whenNotPaused {
        updatePoints(msg.sender);
    ...
    function unstake(string calldata pool, uint256 amount) public canUnstake(pool) whenNotPaused nonReentrant { 
        updatePoints(msg.sender);
        if (keccak256(abi.encodePacked(pool)) == keccak256(abi.encodePacked(SILVER))) {
            userStake[msg.sender].silverAmount -= amount;
            totalSilverStaked -= amount;
...
*/
    ...
    function setEpochRewardPerPoint(uint256 reward) internal {
        if (reward == 0) rewardsPerPointEpoch[epoch] = 0;
        else rewardsPerPointEpoch[epoch] = reward / epochPoints[epoch];
    }
    ...
    function getUserPendingPointsEpoch(address user, uint256 _epoch) internal view returns (uint256) {
        ...
        if (lastChangeTime[user] >= epochEndTime && epochEndTime != 0) return 0; 
            ...
            uint256 delta = (end - start)/ 3 hours;
    ...
    function getPendingRewardPointsEpoch(uint256 _epoch) internal view returns (uint256) {
        if (epoch != _epoch) return 0;
            ...
            uint256 delta = (block.timestamp - lastUpdateTime) / 3 hours;
...
*/
    ...
    function updateRewardPerPointEpoch(uint256 _epoch, uint256 reward) public onlyOwner {
        rewardsPerPointEpoch[_epoch] = reward;
    }

    function updateRewardDenominator(uint256 num) public onlyOwner {
        rewardDenominator = num;
    }
