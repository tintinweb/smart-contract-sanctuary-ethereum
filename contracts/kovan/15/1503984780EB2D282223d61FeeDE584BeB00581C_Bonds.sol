...
import {BONDS_VERSION} from "../ContractVersions.sol";
 ...
 */
contract Bonds is ERC1155, IBonds, Version {
    ...
    ) external override checkVersion(version) {
     ...
     */
    function stake(uint256 _id, uint256 _amm, string memory version)
        external
        override
     ...
     */
    function unstake(uint256 _index, string memory version)
        external
        override
        checkVersion(version)
        returns (bool)
        ...
        returns (uint256)
    {
        ...
        IERC20PaymentStandard pc = IERC20PaymentStandard(paymentContract);
        uint256 accrualPeriod = pc.loanLookup(iou.ID).interestRate;
...
interface IBonds is IERC1155, IVersion {
    ...
    function newLoan(address, uint256, bytes32, bytes memory, string memory) external;
    
    function stake(uint256, uint256, string memory) external returns(uint);

    function unstake(uint256, string memory) external returns(bool);    
...
abstract contract Version is IVersion {
    ...
    /**
     * @notice controls the call of mutating methods in versioned contract
     */
...
string constant BONDS_VERSION = "7.0.0";
string constant INVESTOR_VERSION = "1.0.0";
...
interface IVersion {
    ...
    /**
     * @notice converts string into bytes
     */
