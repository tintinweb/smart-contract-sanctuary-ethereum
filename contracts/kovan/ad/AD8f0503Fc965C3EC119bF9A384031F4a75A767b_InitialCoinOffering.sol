...
contract InitialCoinOffering is Ownable, ReentrancyGuard, ERC721Receiver, Pausable {    
    ...
    uint[2] public discount = [5000,2500];
     ...
     */
    function addNftToList( uint nftId, uint price) external onlyOwner {
    ...
    function buyOffer( uint nftId) external payable onlyWhitelister whenNotPaused nonReentrant {
        ...
        _buyInternal(_msgSender(), nftId, value, uint8(Sale.preSale));
    }
    ...
    function buy( uint nftId) external payable whenNotPaused nonReentrant {
        ...
        require(msg.value == value,"InitialCoinOffering :: buy : invalid price value");

        listedOnSale[nftId].isActive = false;
        _buyInternal(_msgSender(), nftId, value, uint8(Sale.publicSale));
    }
     ...
     * @dev Internal buy
     */
    function _buyInternal( address buyer, uint nftId, uint value, uint8 saleType) private {
    ...
    function getDiscountPrice(uint nftId, uint sale) public view returns (uint discountPrice) {
        if(discount[sale] == 0) {
            return listedOnSale[nftId].price;
        }
