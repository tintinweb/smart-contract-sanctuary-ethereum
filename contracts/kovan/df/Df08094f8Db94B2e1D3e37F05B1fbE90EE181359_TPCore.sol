...
contract TPCore is AccessControl {
    ...
    // external
    function transferLTAdminRole(address newAdmin)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        require(
            newAdmin != address(0),
            "newAdmin address can not be zero address."
        );
        liquidityToken.transferAdminRole(newAdmin);
    }

    function transferLTMinterRole(address newMinter)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        require(
            newMinter != address(0),
            "newMinter address can not be zero address."
        );
    ...
    // function changeStrategy(address newStrategyAddress)
    //     external
    //     onlyRole(DEFAULT_ADMIN_ROLE)
    // {
