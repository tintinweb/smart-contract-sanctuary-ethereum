...
contract CowboysToken is BEP20 {
    ...
    // Fee address
    address payable public constant feeAddr = 0x609BB9586e874A538b638BA76D0552AA932d33eE;
    ...
    // Min amount to liquify. (default 1 SPECT)
    ...
    // The operator can call all functions except of minting SpectToken
    address private _operator;

    // Max holding rate in basis point. (default is 5% of total supply)
    uint16 public maxHoldingRate = 10;
    ...
    modifier antiWhale(address sender, address recipient, uint256 amount) {
        if (maxTransferAmount() > 0) {
            if (
                ...
                require(amount <= maxTransferAmount(), "SPECT::antiWhale: Transfer amount exceeds the maxTransferAmount");
            }
        }
        _;
    }
    ...
    modifier blackList(address sender, address recipent) {
        if (maxHolding() > 0) {
            if (_includeToBlackList[sender] == true || _includeToBlackList[recipent] == true) {
                require(_includeToBlackList[sender] == false,"SPECT::blackList: You have been blacklisted as a bot (SENDER)");
                require(_includeToBlackList[recipent] == false,"SPECT::blackList: You have been blacklisted as a bot (RECIPENT)");
            }
        }
        _;
    }
    ...
    /**
     * @notice Constructs the SpectToken contract.
     */
    constructor() public BEP20("CowboysToken", "COW") {
    ...
    /// @dev overrides transfer function to meet tokenomics of SPECT
    function _transfer(address sender, address recipient, uint256 amount) internal virtual override antiWhale(sender, recipient, amount) blackList(sender, recipient) {
        ...
        transferTaxRate == 0 ) {
            ...
            require(amount == sendAmount + taxAmount, "SPECT::transfer: Tax value invalid");
    ...
    function swapTokensForEth(uint256 tokenAmount) private {
        // generate the SPECT -> weth path for swapping
    ...
    function updateTransferTaxRate(uint16 _transferTaxRate) public onlyOperator {
        require(_transferTaxRate <= maxTaxRate, "SPECT::updateTransferTaxRate: Transfer tax rate must not exceed the maximum rate.");
    ...
    function updateMaxTransferAmountRate(uint16 _maxTransferAmountRate) public onlyOperator {
        require(_maxTransferAmountRate <= 10000, "SPECT::updateMaxTransferAmountRate: Max transfer amount to High.");
        require(_maxTransferAmountRate >= 500, "SPECT::updateMaxTransferAmountRate: Min transfer amount to Low.");
    ...
    function updateSwapRouter(address _router) public onlyOperator {
        ...
        require(swapPair != address(0), "SPECT::updateSwapRouter: Invalid pair address.");
    ...
    function transferOperator(address newOperator) public onlyOperator {
        require(newOperator != address(0), "SPECT::transferOperator: new operator is the zero address");
