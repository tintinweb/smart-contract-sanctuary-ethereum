...
abstract contract WLNFT_Partner is ERC721A, Ownable {
    ...
    function mintPhase2(uint256 _quantity, uint256 _maxMint) public {
        ...
        require(minters[msg.sender].hasMinted.add(_quantity) <= _maxMint, "Exceeds per wallet presale limit.");
        if (minters[msg.sender].exists) {
            if (minters[msg.sender].hasMinted == 0) {
                whitelist[contracts[idx]].usedSpots = whitelist[contracts[idx]].usedSpots.add(1);
            }
            minters[msg.sender].hasMinted = minters[msg.sender].hasMinted.add(
                _quantity
            );
        } else {
            minters[msg.sender].exists = true;
            minters[msg.sender].hasMinted = _quantity;
            whitelist[contracts[idx]].usedSpots = whitelist[contracts[idx]].usedSpots.add(1);
        }
        ...
        returns (bool, uint256)
    {
        for (uint256 i = 0; i < contracts.length; i += 1) {
            if (
                whitelist[contracts[i]].nft.balanceOf(_address) > 0 &&
                whitelist[contracts[i]].usedSpots < whitelist[contracts[i]].availSpots
            ) {
                return (true, i);
            }
        }
        ...
        returns (bool)
    {
        ...
        whitelist[_address].nft = NFT(_address);
        whitelist[_address].availSpots = _availSpots;
    ...
    {
        for (uint256 i = 0; i < contracts.length; i += 1) {
            if (_address == contracts[i] && whitelist[_address].exists) return (true, i);
        }
        return (false, 0);
    }
...
abstract contract WLNFT_Oracle is ChainlinkClient, ERC721A, Ownable {
    using Chainlink for Chainlink.Request;
    using SafeMath for uint256;

    string private apiUrl;
    bytes32 private oracleJob;
    ...
    function mintPhase1(uint256 _quantity, uint256 _maxMint) public returns (bytes32 requestId) {
        require(minters[msg.sender].hasMinted.add(_quantity) <= _maxMint, "Exceeds per wallet presale limit.");
    ...
    function fulfill(bytes32 _requestId, bool _isWhitelisted) public recordChainlinkFulfillment(_requestId) {
        ...
        minters[addr].hasMinted = minters[addr].hasMinted.add(quantity);
...
contract WLNFT_MintPass is ERC721A, Ownable {
    ...
    WLNFT_Oracle oracle;
    WLNFT_Partner partner;

    constructor() ERC721A("WhitelistNFT MintPass", "WxMINT", 100) {
        oracle = WLNFT_Oracle(address(this));
        partner = WLNFT_Partner(address(this));
    }
    ...
    function mint(uint _quantity) public payable {
        ...
        if(preSaleIsActive) {
            ...
            if (preSalePhase == 1) {
                oracle.mintPhase1(_quantity, maxPerWallet);
            }
            if (preSalePhase == 2) {
                partner.mintPhase2(_quantity, maxPerWallet);
            }
        } else {
            require(getPrice().mul(_quantity) <= msg.value, "ETH sent is incorrect.");
