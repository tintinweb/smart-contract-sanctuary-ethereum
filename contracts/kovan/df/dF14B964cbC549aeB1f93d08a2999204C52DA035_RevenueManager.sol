...
contract RevenueManager is PaymentSplitter{
    ...
    // Events 
    event BalancingFundsRequested(uint256 timestamp, address indexed _token, uint256 indexed _amount);

    /**
    * @dev for investors to request funds (for balancing)
     */
    function requestFunds(address _token, uint _amount) external whenNotPaused verifyInvestor {
    ...
    * @dev  returns the balance available for a caller to request
     */
    function balanceAvailable(address _poolAddress) public view returns(uint){
...
abstract contract PaymentSplitter is IRevenueManager, Manager{
    ...
    * @param _shares is the arrya of share amounts. Must be the same size as Payees
     */
    function addShares(address[] calldata _payees, uint[] calldata _shares) external onlyRole(Role.admin){
    ...
    * @param _indecies is the array of indecies to remove from Shares array
     */
    function removeShares(uint[] calldata _indecies) external onlyRole(Role.admin){
        require(Functions.isUnique(_indecies), Errors.REVENUE_UNIQUE_INDEXES);
        Functions.quickSortDESC(_indecies, int(0), int(_indecies.length - 1));
    ...
    * @param _amount to transfer
     */
    function payment(address _tokenContract, uint _amount) public virtual whenNotPaused verifyInvestor{
        ...
        for(uint i = 0; i < shares.length; i++){
            Structs.Share memory s = shares[i];
            /// NOTE do not do a transfer if sending the this to save gas
            if(s.payee != address(this)){
                token.safeTransfer(s.payee, Functions.roundedDiv((_amount * s.share), totalShares));
            }
        }
    }

    function withdrawToken(address token,address recipient) public onlyRole(Role.admin) {
