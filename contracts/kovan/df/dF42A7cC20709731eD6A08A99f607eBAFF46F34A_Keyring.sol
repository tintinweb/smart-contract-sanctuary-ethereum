...
contract Keyring is Base, ChainlinkClient, IKeyring {
    ...
    mapping(bytes32 => address) private revalidationRequest;
    ...
    string public constant CREATE_ENDPOINT = "/create?address=";
    string public constant VALIDATE_ENDPOINT = "/validate?address=";
    string public constant DELETE_ENDPOINT = "/delete?address=";
    ...
    constructor(
        ...
        oracle = _oracle;
        jobId = "4ff328c1870548bfb5f9a27cfcad0a12"; /* @todo replace with _jobId */
        fee = _linkFee;
        validity = 1 minutes;
    }

    function test() external view override returns (string memory, string memory) {
        return ( (msg.sender).addressToString(),  _getURL(BASE_URL, (msg.sender).addressToString(), CREATE_ENDPOINT) );
    }

    function create() external override {
        Chainlink.Request memory req = buildChainlinkRequest(jobId, address(this), this.createCallback.selector);
    ...
    function createCallback(bytes32 requestId, bool valid) public recordChainlinkFulfillment(requestId) {
        address user = mintingRequest[requestId];
        delete mintingRequest[requestId];

        if (!valid) revert Keyring__Invalid(user);

        _mint(user);
    }

    function revalidate(address person) external override {
        Chainlink.Request memory req = buildChainlinkRequest(jobId, address(this), this.revalidateCallback.selector);
        req.add("get", _getURL(BASE_URL, person.addressToString(), VALIDATE_ENDPOINT));
        req.add("path", "result");
        bytes32 requestId = sendChainlinkRequestTo(oracle, req, fee);
        revalidationRequest[requestId] = person;
    }

    function revalidateCallback(bytes32 requestId, bool valid) public recordChainlinkFulfillment(requestId) {
        address user = revalidationRequest[requestId];
        delete revalidationRequest[requestId];

        if (valid) timestamp[user] = block.timestamp + validity;
        else revert Keyring__Invalid(user);
    }
    ...
    function revoke(address user) external override onlyOwner {
        _burnRequest(user);
    }
    ...
    function _mint(address user) internal {
        ...
        _safeMint(user, id);
        timestamp[user] = block.timestamp;
...
interface IKeyring {
    function test() external view returns (string memory, string memory);

    function create() external;

    function revalidate(address user) external;
    ...
    event IdentityTokenRevoked(address indexed user);
