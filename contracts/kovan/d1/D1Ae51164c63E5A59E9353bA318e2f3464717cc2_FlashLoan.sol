...
contract FlashLoan{
    ...
    constructor( address _USDTETH, address _USDT, address _USDC, address _WETH, address _uniV2 ) {
        USDTETH = _USDTETH;
        USDT = _USDT;
        USDC = _USDC;
        WETH = _WETH;
        uniV2 = _uniV2; 
        safeApprove(WETH,uniV2,type(uint256).max);
        safeApprove(USDT,uniV2,type(uint128).max);
        safeApprove(USDC,uniV2,type(uint128).max);
    }
