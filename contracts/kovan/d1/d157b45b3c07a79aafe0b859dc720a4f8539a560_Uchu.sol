...
contract Uchu {
    ...
    **/
    ...
    function singleProBalance(Product memory product) private view returns (uint256) {
        uint amount = balanceOf[msg.sender][product.name];
        uint startTime = getPoint().createPositionTime;
        uint a = block.timestamp - startTime;
        uint b = 3600 * 24;
        return amount * ((1 + product.yeildNum/product.totalNum) ** a/b);
    }

    function getBalance(address client, string memory name) public payable returns (uint256) {
        finalBalance[client][name] = singleProBalance(curLenderPro);
        return finalBalance[client][name];
    }
