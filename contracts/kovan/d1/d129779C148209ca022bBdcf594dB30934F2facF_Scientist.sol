...
// File: contracts\scientist\ERC721Tradable.sol
...
abstract contract AScientistRepository is IScientistRepository, Multicall {
    ...
    //  are meta scientists
    mapping(address => mapping(uint256 => ScientistData.Scientist)) public addressToMap;
    ...
    function _removeScientist(address _user, uint256 _tokenId) internal {
        require(
            _getScientist(_tokenId).user != address(0),
            "Token not exists"
        );
    ...
    function _updateScientist(ScientistData.Scientist memory _scientist, address _owner)
        internal
    {
        ScientistData.Scientist memory scientist = _getScientist(_scientist.tokenId);
...
contract Scientist is ERC721Tradable, AScientistRepository, ReentrancyGuard {
    ...
    function ownerOf(uint256 tokenId) public view virtual override(ERC721, AScientistRepository) returns (address) {
        return ERC721.ownerOf(tokenId);
    }

    function addScientist(ScientistData.Scientist memory _scientist) external override isAdmin {
        _addScientist(_scientist);
    }

    function removeScientist(uint256 _tokenId, address _owner) external override isAdmin {
