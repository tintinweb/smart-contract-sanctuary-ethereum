...
contract TPCore is AccessControl {
    ...
    function estimateTPYReward(address addr) public view returns(uint136) {
        ...
        uint256 lastCompleteInterval = currentInterval() - 1;
    ...
    function registerTPY(address staker, uint104 stakingCoinAmount) private {
        ...
        if(userStaking[staker].intervalNumber > 0) {// exists
            ...
            if(staking.intervalNumber >= currentInterval()){
                staking.meetIntervalAmount += stakingCoinAmount;
            }
            else {
                if(staking.meetIntervalAmount > 0) {
                    staking.amount += uint104(staking.meetIntervalAmount);
                    staking.meetIntervalAmount = 0;
