...
contract Comm is Ownable {
    ...
    constructor(IRefferal _refer, IERC20 _MGC) {
        refer = _refer;
        MGC = _MGC;
    }
    ...
    function getUserSpins(address _from, IMGCLottery lottery) external view returns(Spin[] memory _list) {
        (_list,) = lottery.getUserSpins(_from, 1, 0);
    }
    function handleComm(address _fromUser, uint _amount, IMGCLottery _lottery) external {
        ...
        for(uint i = 0; i < 7; i++) {
            ...
            if(_user == _refferBy) {
                ...
                Spin[] memory _list;
                (_list,) = _lottery.getUserSpins(from, 1, 0);
