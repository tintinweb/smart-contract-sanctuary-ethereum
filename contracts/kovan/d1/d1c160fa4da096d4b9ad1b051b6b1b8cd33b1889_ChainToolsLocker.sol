...
contract ChainToolsLocker is Ownable, ReentrancyGuard,EternalStorage {
  ...
  function setLp(address payable _lpAddr) public onlyOwner {
    lpAddr = _lpAddr;
  }
   ...
   */
  function lock (address _lpToken, uint256 _amount, uint256 _unlockDate, address payable _withdrawer) external payable nonReentrant {
    ...
    uint256 devFee = gFees.ethFee;
    require(msg.value >= devFee, 'FEE NOT ENOUGH');
    devAddr.transfer(msg.value);
  ...
  function getLockOfToken (address _lpToken) external view 
  returns (address[] memory) {
    address[] memory addeList = lockedTokens[_lpToken];
    return addeList;
  }
  
  function getLockOfUser (address _user) external view 
  returns (address[] memory) {
    address[] memory addeList = lockedUser[_user];
    return addeList;
  }
  
  function getUserLockOfToken (address _user, address _lpToken) external view 
