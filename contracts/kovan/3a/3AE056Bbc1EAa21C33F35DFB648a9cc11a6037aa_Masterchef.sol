   ...
   // mapping(address => uint256) public balances;
    ...
    function updatePool() private {
        if (A.balanceOf(address(this)) == 0) {
            ...
            accRewardPerShare += // 10000 * 5 / 119000
                (rewardPerBlock * (block.number - lastRewardBlock))*1e12 /
    ...
    function claimRdx(uint256 _amount) public {
        ...
        uint256 _accRewardPerShare = accRewardPerShare +
            ((rewardPerBlock * (block.number - lastRewardBlock)) *1e12/
                A.balanceOf(address(this)));
        uint256 pending = user.amount * _accRewardPerShare - user.rewardDebt*1e12;

        require((user.reward*1e12 + pending) >= _amount*1e12, "reject amount > reward");
        RDX.transfer(msg.sender, _amount);
        user.reward = user.reward + pending/1e12 - _amount;
