...
contract Contract_SLP is Ownable {
    ...
    uint public effect_factor = 1;    //change to 10 when need half life to 0.1
    ...
    constructor() {
        ...
        T_WBTC = IERC20(0xfB237dF8e64e3cA51EEF9c80F4F1DFD9946Bf1e8);    //8 decimal
    ...
    function burn_Lp(uint _pid) external view {
        require(block.timestamp == Pool_END_Time, "You can't burn at the moment!");
    ...
    function add_coin_liquidity() public payable {
        ...
        require(amount != 0 ,"Need some Coin!");

        _LP_User[person][Coin] += amount;     //userdata rec

        uint256 decimal = 1 * 10 ** 18;   //change with coin decimal
        uint256 e = amount * decimal;  //ad
        uint256 lt = LT_ETH();    //get 10k worth price of eth and weth
        uint256 cl = address(this).balance;   //pool balance
        uint256 equation = ( lt ) * (cl + decimal);
        uint256 re = equation / decimal;
           
        uint256 LP_Pool_2 = (e  / re ) * hl;

        LP_Pool_2 = LP_Pool_2 / effect_factor;

        BhETH.transfer(person, decimal);    ////
        Pool_2.transferFrom(contr_pool2,person,LP_Pool_2);
        
        _LP_Rec[person][bheth] += decimal;
        _LP_Rec[person][pool] += LP_Pool_2;

     }
    
    
    function add_token_liquidity(uint _pid , uint256 _amount) public {
        ...
        if(_pid == 1) {   //BhETH--WETH

            _LP_User[person][token_1] += amount;     //userdata rec

            uint256 decimal = 1 * 10 ** 18;   //change with coin decimal
            uint256 e = amount * decimal;  //ad
            uint256 lt = LT_ETH();    //get 10k worth price of eth and weth
            uint256 cl = T_WETH.balanceOf(address(this));   //pool balance
            uint256 equation = ( lt ) * (cl + decimal);
            uint256 re = equation / decimal;
            ...
            BhWETH.transfer(person, decimal);    ////
            Pool_2.transferFrom(contr_pool2,person,LP_Pool_2);
            
            _LP_Rec[person][bhweth] += decimal;
            _LP_Rec[person][pool] += LP_Pool_2;

        }

        if(_pid == 2) {     ///bhwbtc---wbtc

            _LP_User[person][token_2] += amount;     //userdata rec

            uint256 decimal = 1 * 10 ** 18;   //change with coin decimal
            uint256 e = amount * decimal;  //ad
            uint256 lt = LT_WBTC();    //get 10k worth price of eth and weth
            uint256 cl = T_WBTC.balanceOf(address(this));   //pool balance
            uint256 equation = ( lt ) * (cl + decimal);
            uint256 re = equation / decimal;
            ...
            BhWBTC.transfer(person, decimal);    ////
            Pool_2.transferFrom(contr_pool2,person,LP_Pool_2);
            
            _LP_Rec[person][bhwbtc] += decimal;
            _LP_Rec[person][pool] += LP_Pool_2;

        }
        else {
            revert("Select the Correct Option!!");
        }
    ...
    //initially hl = 10 , factor = 1
    function update_hl(uint _hl, uint _fact) public onlyOwner{
        hl = _hl;
        effect_factor = _fact;
    }

    function User_LP_Balance(uint _pid) external view returns (uint256){
        if(_pid == 1 ) return BhETH.balanceOf(msg.sender);
        if(_pid == 2 ) return BhWETH.balanceOf(msg.sender);
        if(_pid == 3 ) return BhWBTC.balanceOf(msg.sender);
        if(_pid == 4 ) return Pool_2.balanceOf(msg.sender);
        else{
            revert("Select Correct Option!!");
        }
    }

    function Check_Allowance(uint _pid) external view returns (uint256){
    ...
    function extract_balance(uint _pid) external onlyOwner view returns (uint256){
