...
contract PrecogV5 is IPrecogV5, ReentrancyGuard {
    ...
    function _processHandleIncreaseInvestment(
        ...
        _nextFundingTime = PrecogV5Library._nextFundingTime(
            _getCoreInstance().getCyclesChangedInfo(_token).fundingApplyTime,
            _getCoreInstance().getCyclesChangedInfo(_token).fundingDuration
        );
        ...
        if (precogStorage.getInvestmentsOf(_token, _account).length == 0) {
            ...
            IPrecogStorage.Investment memory _lastInvestmentOf = precogStorage.getLastInvestmentOf(_token, _account);
            IPrecogStorage.Cycle memory _futureTradingCycle = precogInternal.getTradingCycleByTimestamp(
                _token,
                _nextFundingTime
            );
    ...
    function _registerWithdrawal(
        ...
        IWithdrawalRegister _withdrawalRegister = _getWithdrawalRegisterInstance();
        IPrecogCore.CyclesChangedInfo memory _cycleChangedInfo = _core.getCyclesChangedInfo(_token);
    ...
    function turnOnWhitelist(address token, address account) external isExistingToken(token) onlyAdmin {
        ...
        uint timestamp;
        {
            ...
            uint fundingTime = PrecogV5Library._nextFundingTime(
                _core.getCyclesChangedInfo(token).fundingApplyTime,
                _core.getCyclesChangedInfo(token).fundingDuration
            );
            ...
            timestamp = futureIdForNextFunding = precogInternal.getTradingCycleByTimestamp(token, fundingTime).id;
        ...
        for (uint i = futureIdForNextFunding + 1; i <= futureIdforNextFirstDefunding; i++) {
            if (precogStorage.checkIsUpdateUnitTradingCycle(token, i)) {
                uint duration;
                {
                    IPrecogStorage.Cycle memory futureTradingCycle = precogInternal.getTradingCycleByTimestamp(
                        token,
                        timestamp
                    );
...
library PrecogV5Library {
    ...
    function _chooseLastAvailableTradingId(
        ...
        uint _investmentId,
        uint _value
    ) internal pure returns (IPrecogStorage.Investment memory _nextInvestment, uint _lastAvailableProfitId) {
        unchecked {
            _nextInvestment = IPrecogStorage.Investment({
                amount: 0,
                unit: 0,
                timestamp: 0,
                idChanged: 0,
    ...
    function _calculateTradingCycleByTimestamp(
        ...
        uint timestamp
    ) internal pure returns (IPrecogStorage.Cycle memory currentCycle) {
        unchecked {
...
interface IPrecogStorage {
    ...
    event SetMiddlewareService();
    event SetMiddlewareExchange();
    event SetPrecogInternal();
    event SetPrecogCore();
    event SetPrecogFactory();
    event SetPrecogVault();
    event SetPrecogProfit();
    event SetPrecog();
    event SetWithdrawalRegister();
...
interface IPrecogInternal {
    ...
    function calculateProfit(address _token, address _account)
        external
        view
        returns (IPrecogStorage.AccountProfitInfo memory accountProfitInfo);
