...
contract MetaNFTEcommerse is Ownable,Pausable,ReentrancyGuard {
     address public Owner;
     using SafeERC20 for IERC20;
     IERC20 public Token ;
     nftbase public Metanftaddress;
     address public signerAddress;
     using SignatureChecker for address;

    constructor(address _signerAddress,nftbase nftaddress,address _tokenaddress){
        Owner = msg.sender;
        signerAddress = _signerAddress; 
        Metanftaddress = nftaddress; 
    ...
    function buy(string memory hash,uint256 nftId,uint256 amount,bytes memory signature,uint256 _flag)external payable whenNotPaused nonReentrant{
        ...
        if (_flag == 0) {
            Token.safeTransferFrom(user,Owner,amount);
        }
        else {
        ...
        require(signCheck(toSignedMessageHash(hash_),signature), "Sign Error");
         ...
         Metanftaddress.safeMints(user,nftId,hash);
