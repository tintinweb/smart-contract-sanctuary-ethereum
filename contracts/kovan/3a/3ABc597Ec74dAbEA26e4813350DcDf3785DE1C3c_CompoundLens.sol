...
interface CompoundLensInterface {
    ...
    function getAssetsIn(address) external view returns (MToken[] memory);
...
contract CompoundLens {
    ...
    function cTokenMetadata(MToken mToken) public returns (MTokenMetadata memory) {
        uint exchangeRateCurrent = mToken.exchangeRateCurrent();
        CompoundLensInterface comptroller = CompoundLensInterface(address(mToken.comptroller()));
        (bool isListed, uint collateralFactorMantissa) = comptroller.markets(address(mToken));
        address underlyingAssetAddress;
        uint underlyingDecimals;
        ...
        if (compareStrings(mToken.symbol(), "cETH")) {
            ...
            MErc20 cErc20 = MErc20(address(mToken));
        ...
        return MTokenMetadata({
            ...
            supplyRatePerBlock: mToken.supplyRatePerBlock(),
            borrowRatePerBlock: mToken.borrowRatePerBlock(),
            reserveFactorMantissa: mToken.reserveFactorMantissa(),
            totalBorrows: mToken.totalBorrows(),
            totalReserves: mToken.totalReserves(),
            totalSupply: mToken.totalSupply(),
            totalCash: mToken.getCash(),
            isListed: isListed,
            collateralFactorMantissa: collateralFactorMantissa,
            underlyingAssetAddress: underlyingAssetAddress,
            cTokenDecimals: mToken.decimals(),
            underlyingDecimals: underlyingDecimals,
            liquidationThreshold: comptroller.liquidationThreshold(address(mToken)),
            liquidationIncentive: comptroller.liquidationIncentive(address(mToken))
        });
    }

    function cTokenMetadataAll(MToken[] calldata cTokens) external returns (MTokenMetadata[] memory) {
        uint cTokenCount = cTokens.length;
        MTokenMetadata[] memory res = new MTokenMetadata[](cTokenCount);
    ...
    function cTokenBalances(MToken mToken, address payable account) public returns (MTokenBalances memory) {
        uint balanceOf = mToken.balanceOf(account);
        uint borrowBalanceCurrent = mToken.borrowBalanceCurrent(account);
        uint balanceOfUnderlying = mToken.balanceOfUnderlying(account);
        uint tokenBalance;
        uint tokenAllowance;
        ...
        if (compareStrings(mToken.symbol(), "cETH")) {
            ...
            tokenAllowance = underlying.allowance(account, address(mToken));
        }
    ...
    function cTokenBalancesAll(MToken[] calldata cTokens, address payable account) external returns (MTokenBalances[] memory) {
        uint cTokenCount = cTokens.length;
        MTokenBalances[] memory res = new MTokenBalances[](cTokenCount);
        for (uint i = 0; i < cTokenCount; i++) {
    ...
    struct MTokenUnderlyingPrice {
        address mToken;
        uint underlyingPrice;
    }

    function cTokenUnderlyingPrice(MToken mToken) public view returns (MTokenUnderlyingPrice memory) {
        CompoundLensInterface comptroller = CompoundLensInterface(address(mToken.comptroller()));
        PriceOracle priceOracle = comptroller.oracle();

        return MTokenUnderlyingPrice({
            mToken: address(mToken),
            underlyingPrice: priceOracle.getUnderlyingPrice(mToken)
        });
    }

    function cTokenUnderlyingPriceAll(MToken[] calldata cTokens) external view returns (MTokenUnderlyingPrice[] memory) {
        uint cTokenCount = cTokens.length;
        MTokenUnderlyingPrice[] memory res = new MTokenUnderlyingPrice[](cTokenCount);
        for (uint i = 0; i < cTokenCount; i++) {
