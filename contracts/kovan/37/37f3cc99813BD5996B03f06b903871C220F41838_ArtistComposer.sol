    ...
    OwnableUpgradeable
{
    ...
    function buyFullTrack(
        ...
        address signer = _verify(signatureData);
        require(
    ...
    function _verify(SignatureData calldata signatureData)
        internal
        view
        returns (address)
    {
        bytes32 digest = keccak256(
            abi.encodePacked(
                ...
                keccak256(
                    abi.encode(
                        keccak256("SignatureData(string unique)"),
                        keccak256(bytes(signatureData.unique))
                    )
                )
            )
        );
        return ECDSAUpgradeable.recover(digest, signatureData.signature);
    }
