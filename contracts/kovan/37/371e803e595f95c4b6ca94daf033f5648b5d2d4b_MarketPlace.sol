...
contract MarketPlace is IAdmin, IMarketPlace, IModuleBox, Ownable {
    ...
    function setMDMAToken(address _address) external override isAdmin {
        ...
        emit TokenAdressChanged(mdmaTokenAddress, _address);
    ...
    function setMetaCellToken(address _address) external override isAdmin {
        require(_address != address(0), "Address should not be empty");
        emit TokenAdressChanged(metaCellTokenAddress, _address);
        metaCellTokenAddress = _address;
    }

    function setNanoCellToken(address _address) external override isAdmin {
        require(_address != address(0), "Address should not be empty");
        emit TokenAdressChanged(nanoCellTokenAddress, _address);
    ...
    function setModulesAddress(address _address) external override isAdmin {
        require(_address != address(0), "Address should not be empty");
        emit ModuleAddressChanged(module, _address);
        module = _address;
    }
        ...
        override
    {
        ...
        emit EnhancerBought(msg.sender, enhancerId, amount, msg.value, block.timestamp);
    }
    ...
    function buyEnhancerForToken(
        ...
        _buyEnhancer(enhancerId, amount);
        emit EnhancerBought(msg.sender, enhancerId, amount, enhancer.basePrice.mul(amount), block.timestamp);
    }
    ...
    function modifyEnhancer(
        ...
        CellEnhancer.Enhancer memory enhancer = IEnhancerRepository(laboratory).getEnhancerInfo(_enhancer.id);
        ...
        isAdmin
    {
        ...
        emit EnhancersAmountIncreased(msg.sender, _id, enhancersAmount[_id], block.timestamp);
    }
        ...
        override
    {
        ...
        cell.onSale = false;
        ICellRepository(metaCellTokenAddress).updateMetaCell(cell, cell.user);
        ...
        emit MetaCellSold(_oldOwner, _tokenId, msg.sender, cell.price, feeAmount, block.timestamp);
    }
        ...
        override
    {
        ...
        emit MetaCellPriceUpdated(cell.user, _tokenId, _newPrice, block.timestamp);
    }
    ...
    function removeMetaCellFromSale(uint256 _tokenId) external override {
        ...
        emit MetaCellRemovedFromMarketPlace(msg.sender, _tokenId, block.timestamp);
    }
    ...
    function sellMetaCell(uint256 _tokenId, uint256 _price) external override {
        ...
        CellData.Cell memory cell = ICellRepository(metaCellTokenAddress)
            .getMetaCell(msg.sender, _tokenId);
        require(msg.sender == IERC721(metaCellTokenAddress).ownerOf(_tokenId), "You are not the owner");
        ...
        metaCellsOwners.push(IERC721(metaCellTokenAddress).ownerOf(_tokenId));

        emit MetaCellAddedToMarketplace(msg.sender, _tokenId, _price, block.timestamp);
    }

    function buyModule(uint256 id) external override {
        address oldOwnerOf = IERC721(module).ownerOf(id);
        uint256 price = modulesOnSale[id];
        require(modulesOnSale[id] != 0, "Empty price");
        require(oldOwnerOf != msg.sender, "You are the owner");
        require(
            IERC20(mdmaTokenAddress).balanceOf(msg.sender) > price,
            "Not enough funds"
        );
        ...
        emit ModuleSold(oldOwnerOf, id, msg.sender, price, feeAmount, block.timestamp);
    }
    ...
    function buyNanoCell(uint256 id) external override {
        address oldOwnerOf = IERC721(nanoCellTokenAddress).ownerOf(id);
        uint256 price = nanoCellOnSale[id];
        require(price != 0, "Empty price");
        require(
            oldOwnerOf != msg.sender,
            "You are the owner"
        );
        require(
            IERC20(mdmaTokenAddress).balanceOf(msg.sender) > price,
        ...
        emit NanoCellSold(oldOwnerOf, id, msg.sender, price, feeAmount, block.timestamp);
    }
