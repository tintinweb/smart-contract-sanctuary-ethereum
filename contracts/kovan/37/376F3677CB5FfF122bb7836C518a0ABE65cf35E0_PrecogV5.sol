...
contract PrecogV5 is IPrecogV5 {
    ...
    mapping(address => uint16) public override currentFundingId;
    mapping(address => uint16) public override currentProfitId;
    mapping(address => mapping(address => uint16)) public override lastProfitIdOf;
        ...
        )
    {
        ...
        currentTradingCycle = PrecogV5Library._getCurrentTradingCycleByTimestamp(lastTradingCycle, block.timestamp, _currentDepositedTimestamp, tradingCycle, isRemoved[token]);
    }
        ...
        if (_lastProfitIdOf < _updatedLatestCycle) {
            for (_investmentId; _investmentId < _investments.length; _investmentId++) {
                ...
                for (_lastProfitIdOf; _lastProfitIdOf < _lastAvailableProfitId; _lastProfitIdOf++) {
                    _profitOfAccount += PrecogV5Library._calculateProfitAtCycle(_tradingCycles[_lastProfitIdOf], _investments[_investmentId], totalUnitsTradingCycle[_token][_lastProfitIdOf], _profits[_lastProfitIdOf], _lastProfitIdOf);
                }
    ...
    function _getFutureTradingCycle(
        address _token,
        uint _futureTimestamp,
        uint48 _tradingDuration
        ) 
            internal 
            view 
            returns (Cycle memory _futureTradingCycle) 
    {
        Cycle memory _lastTradingCycle = tradingCycles[_token][tradingCycles[_token].length - 1];
        _futureTradingCycle = 
            PrecogV5Library._getCurrentTradingCycleByTimestamp(_lastTradingCycle, _futureTimestamp, currentDepositedTimestamp[_token], _tradingDuration, isRemoved[_token]);
    }
    ...
    function _increaseInvestment(
        ...
        {
            _duration = PrecogV5Library._defineDurationFundingCycle(_lastTradingCycle, currentDepositedTimestamp[_token], core.getCycleConfiguration().fundingCycle);
            _nextInvestmentTimestamp = PrecogV5Library._calculateNextFundingTimestamp(_tradingCycles[0], _duration);
            _futureTradingCycle = _getFutureTradingCycle(_token, _nextInvestmentTimestamp, _duration);
        }
         
        require(_futureTradingCycle.endTime >= uint48(block.timestamp), "PrecogV5: INVALID_TIMESTAMP");
        uint _unit = _amount * (_futureTradingCycle.endTime - _nextInvestmentTimestamp);
        uint _updatedUnit;
        ...
        if (_investmentsOf.length > 0) {
            ...
            Investment memory _newInvestmentOf = Investment(_lastInvestmentOf.amount + _amount, _updatedUnit, _nextInvestmentTimestamp, _futureTradingCycle.id);
            ...
            Investment memory _newInvestmentOf = Investment(_amount, _unit, _nextInvestmentTimestamp, _futureTradingCycle.id);
            investmentsOf[_token][_account].push(_newInvestmentOf);
        }
    ...
    function _decreaseInvestment(
        ...
        Cycle[] memory _tradingCycles = tradingCycles[_token];
        Cycle memory _lastTradingCycle = _tradingCycles[_tradingCycles.length - 1];
        require(_lastTradingCycle.endTime >= uint48(block.timestamp), "PrecogV5: INVALID_TIMESTAMP");
        if (_investmentsOf.length > 0) {
            Investment memory _lastInvestmentOf = _investmentsOf[_investmentsOf.length - 1];
            Investment memory _newInvestmentOf = Investment(_lastInvestmentOf.amount - _amount, _lastInvestmentOf.unit, _lastInvestmentOf.timestamp, _lastTradingCycle.id);
            if (_lastInvestmentOf.idChanged < _lastTradingCycle.id) {
                _newInvestmentOf.unit = _lastInvestmentOf.amount * (_lastTradingCycle.endTime - _lastTradingCycle.startTime);
    ...
    function updateProfit(address token, address account) public override {
        (uint profitOfAccount, uint16 newInvestmentId, uint16 newProfitIdOf) = _calculateProfit(token,account);
    ...
    function _registerWithdrawal(address _token, address _account, uint _amount) internal {
        ...
        if (isFirstRequestWithdrawal[_token][_account] == false) {
            _locktime = core.getCycleConfiguration().firstDefundingCycle;
            isFirstRequestWithdrawal[_token][_account] = true;
        } else {
            _locktime = core.getCycleConfiguration().defundingCycle;
        }
    ...
    function _decreaseInvestmentWhenWithdraw(address _token, address _account, uint _amount) internal {
        ...
        uint48 _duration = PrecogV5Library._defineDurationFundingCycle(_lastTradingCycle, _currentDepositedTimestamp, core.getCycleConfiguration().fundingCycle);
        Cycle memory _futureTradingCycle = _getFutureTradingCycle(_token, _lastInvestmentOf.timestamp, _duration);
        require(_futureTradingCycle.endTime >= uint48(block.timestamp), "PrecogV5: INVALID_TIMESTAMP");
...
library PrecogV5Library {
    ...
    function _getCurrentTradingCycleByTimestamp( 
        IPrecogV5.Cycle memory _lastTradingCycle,
        uint _timestamp, 
        ...
        internal
        pure
        returns (IPrecogV5.Cycle memory _currentProfitCycle)
    {
        ...
        while (uint48(_timestamp) >= _lastTradingCycle.endTime) {
            ...
            if (_lastTradingCycle.endTime < _currentDepositedTimestamp) {
    ...
    function _defineDurationFundingCycle(
    ...
    function _calculateNextFundingTimestamp(IPrecogV5.Cycle memory _firstProfitCycle, uint48 _duration) internal view returns (uint48 _nextInvestmentTimestamp) {
