...
contract aggregator{
    ...
    address private constant BKWETH = 0xdFCeA9088c8A88A76FF74892C1457C17dfeef9C1;
  ...
  // get facet from function selector
    ...
    function dex(
                ...
                if(dexRoute1[i] == 0)
                {
                    ...
                    if(i==0){
                        ...
                        if(options[optionIndex]==2){
                            ...
                            //console.log("optionIndex",options[optionIndex]);
                            revert('invalid option');
                        }
                    }
                    else {
        ...
        eventEmit(amountin,tokenIn,tokenOut,msg.sender);
    ...
    function eventEmit(uint amountIn0, address TokenIn, address TokenOut, address msgSender) internal{
        //if(dexRoute1[dexLength])
        //uint a = dexRoute.length;
        if(TokenOut==ETH)
        {   
            console.log("USers ETH**********TokenOut ETH",msgSender.balance);
            console.log("Address(this).balance ETH",address(this).balance);
            uint amountReceived = msgSender.balance - ethBalBefore;
            
            payable(msgSender).transfer(address(this).balance);
            console.log("USers ETH**********TokenOut ETH",msgSender.balance);
            console.log("Address(this).balance ETH after transferring to users",address(this).balance);
        ...
        emit BeeSwap(msgSender, TokenIn, TokenOut, amountIn0, amountRecevied, block.timestamp);
