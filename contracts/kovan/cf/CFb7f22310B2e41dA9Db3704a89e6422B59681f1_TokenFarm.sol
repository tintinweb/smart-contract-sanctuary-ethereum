...
contract TokenFarm is Ownable {
    // Stake, unstake, issue, add allowed tokens, getEth value
    ...
    function stakeTokens(uint256 _amount, address _token) public {
        // what tokens and how many can be staked
        require(_amount > 0, "Amount must be more than 0");
        require(tokenIsAllowed(_token), "Token is currently not allowed");

        // ERC20 has two transfer functions -> transfer(): if we own the tokens we are transferring -> transferFrom(): if we don't own the token
