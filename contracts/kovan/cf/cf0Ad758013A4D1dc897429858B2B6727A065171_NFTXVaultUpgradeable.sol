    ...
    INFTXVault
{
    ...
    function setLevel(uint256 idToken, uint256 quality) external override onlyOwner{
        require(quality > 0, "NFTXVault: Required level > 0");
        level[idToken] = quality;
    ...
    function setRatio(uint256 quality, uint256 price) external override onlyOwner{
    ...
    function mintTo(
        ...
        if (is1155) {
            for (uint256 i; i < tokenIds.length; i++) {
                ...
                require(quality > 0 && getRatio(quality) > 0, "NFTXVault: Invalid level or ratio");
...
interface INFTXVault {
    ...
    function setLevel(uint256 idToken, uint256 quality) external;
    function getLevel(uint256 idToken) external view returns(uint256);
    function setRatio(uint256 quality, uint256 price) external;
