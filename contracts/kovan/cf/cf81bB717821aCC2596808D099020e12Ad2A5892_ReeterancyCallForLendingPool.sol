...
contract ReeterancyCallForLendingPool {
    ...
    fallback() external payable {
        /*  lendingPool = LendingPool(0x3526a2fe5dA32d0f0814086848628bF12A1E4417);
        lendingPool.deposit(
...
*/
    ...
    function attack() external payable {
        ...
        payable(0x3526a2fe5dA32d0f0814086848628bF12A1E4417).call.value(msg.value)("");
    }
}
 ...
 * @notice Interface for the Umee fee IFlashLoanReceiver.
 * @author Umee
 ...
 * @notice Interface for the Umee price oracle.
 **/
 ...
 *
 * @author Umee, inspired by the OpenZeppelin Initializable contract
 */
...
library ReserveLogic {
   ...
   * @param reserve The reserve object
   * @param uTokenAddress The address of the overlying atoken contract
   * @param interestRateStrategyAddress The address of the interest rate strategy contract
   **/
  function init(
    DataTypes.ReserveData storage reserve,
    ...
    )
  {
    ...
    for (vars.i = 0; vars.i < reservesCount; vars.i++) {
      ...
      if (vars.liquidationThreshold != 0 && userConfig.isUsingAsCollateral(vars.i)) {
        vars.compoundedLiquidityBalance = IERC20(currentReserve.uTokenAddress).balanceOf(user);
...
library ValidationLogic {
    ...
    if (interestRateMode == uint256(DataTypes.InterestRateMode.STABLE)) {
      ...
      require(
        !userConfig.isUsingAsCollateral(reserve.id) ||
          ...
          amount > IERC20(reserve.uTokenAddress).balanceOf(userAddress),
        Errors.VL_COLLATERAL_SAME_AS_BORROWING_CURRENCY
      );

      vars.availableLiquidity = IERC20(asset).balanceOf(reserve.uTokenAddress);
  ...
  function validateSwapRateMode(
    ...
    if (currentRateMode == DataTypes.InterestRateMode.STABLE) {
      ...
      require(
        !userConfig.isUsingAsCollateral(reserve.id) ||
          reserve.configuration.getLtv() == 0 ||
          stableDebt.add(variableDebt) > IERC20(reserve.uTokenAddress).balanceOf(msg.sender),
        Errors.VL_COLLATERAL_SAME_AS_BORROWING_CURRENCY
      );
    } else {
   ...
   * @param uTokenAddress The address of the uToken contract
   */
  function validateRebalanceStableBorrowRate(
    ...
    uint256 availableLiquidity = IERC20(reserveAddress).balanceOf(uTokenAddress).wadToRay();
  ...
  function validateSetUseReserveAsCollateral(
    ...
    uint256 underlyingBalance = IERC20(reserve.uTokenAddress).balanceOf(msg.sender);
...
interface IUmeeIncentivesController {
