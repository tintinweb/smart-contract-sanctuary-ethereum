...
abstract contract ExenoNode is Ownable, ERC1363Payable, ICallback {
    ...
    IConnextHandler public handler;
    ...
    modifier onlyExecutor() {
        require(msg.sender == address(handler.executor()),
            "ExenoNode: unexpected executor");
        uint32 origin = IExecutor(msg.sender).origin();
        require(nodes[origin].deployment != address(0),
            "ExenoNode: unexpected origin domain");
        require(IExecutor(msg.sender).originSender() == nodes[origin].deployment,
            "ExenoNode: unexpected origin contract");
        _;
    }
    ...
    function addNode(
        ...
        external onlyOwner
    {
        nodes[origin] = Node(deployment, fee);
    }
        ...
        external onlyOwner
    {
        ...
        require(balance > 0,
            "ExenoNode: there is no cash to release");
    ...
    function deposit(
        ...
        external payable
    {
        require(acceptedToken().allowance(msg.sender, address(this)) >= amount,
            "ExenoNode: user must approve amount");
    ...
    {
        uint256 fee = feeAlreadyPaid ? 0 : tokenFee;
        if (_processWithdraw(beneficiary, amount, fee)) {
            emit Withdrawn(msg.sender, beneficiary, amount, fee);
        } else {
            _deposit(IExecutor(msg.sender).origin(), beneficiary, amount);
            emit Reverted(msg.sender, beneficiary, amount);
        }
    }
    ...
    function callback(
        ...
        external override
    {
        (address beneficiary, uint256 amount, bool feeAlreadyPaid) = abi.decode(data, (address, uint256, bool));
        emit Callback(transferId, success, beneficiary, amount, feeAlreadyPaid);
    }
        ...
        internal
    {
        ...
        require(nodes[destination].deployment != address(0),
            "ExenoNode: unknown destination address");

        require(minimumAmount == 0 || amount >= minimumAmount,
            "ExenoNode: amount is below the required minimum");
        ...
        if (msg.value > 0) {
            ...
            fee = nodes[destination].fee;
            feeAlreadyPaid = false;
        }

        require(address(this).balance >= fee,
            "ExenoNode: not enough cash to cover xcall fee");

        bytes4 selector = bytes4(keccak256("withdraw(address,uint256,bool)"));
        bytes memory callData = abi.encodeWithSelector(selector, beneficiary, amount, feeAlreadyPaid);

        CallParams memory callParams = CallParams({
            to: nodes[destination].deployment,
            callData: callData,
            originDomain: domain,
            destinationDomain: destination,
            recovery: nodes[destination].deployment,
        ...
        bytes32 transferId = handler.xcall{value: fee}(xcallArgs);

        _processDeposit(amount);
        emit Deposited(transferId, msg.sender, beneficiary, amount, fee, feeAlreadyPaid);
    }
...
contract ExenoNode_L2 is ExenoNode {

    IERC1363Mintable public immutable token;
    ...
    constructor(
        ...
        uint256 _tokenFee,
        IConnextHandler _handler,
        IERC1363Mintable _token
    )
        ExenoNode(_domain, _asset, _wallet, _minimumAmount, _tokenFee, _handler, _token)
    {
        token = _token;
    }
    ...
    {
        if (token.manager() == address(this)) {
            if (fee > 0) {
                token.mint(beneficiary, amount - fee);
                token.mint(wallet, fee);
            } else {
                token.mint(beneficiary, amount);
            }
            return true;
        }
        emit MintError(msg.sender, beneficiary, amount);
        return false;
    }

    function _processDeposit(uint256 amount)
        internal override
    {
        token.burn(amount);
    }
...
interface IERC1363Mintable is IERC1363 {
