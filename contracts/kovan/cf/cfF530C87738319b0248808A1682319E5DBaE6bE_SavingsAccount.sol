...
contract SavingsAccount is ISavingsAccount, Initializable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function deposit(
        ...
        require(_to != address(0), 'D1');
        require(_amount != 0, 'D2');
        require(IStrategyRegistry(strategyRegistry).registry(_strategy) != 0, 'D3');
    ...
    function switchStrategy(
        ...
        require(_currentStrategy != _newStrategy, 'SS1');
        IStrategyRegistry _registry = IStrategyRegistry(strategyRegistry);
        require(_registry.registry(_newStrategy) != 0, 'SS2');
        require(_registry.isValidStrategy(_currentStrategy), 'SS3');
        require(_amount != 0, 'SS4');

        IYield currentStrategy = IYield(_currentStrategy);
        _amount = currentStrategy.getSharesForTokens(_amount, _token);

        balanceInShares[msg.sender][_token][_currentStrategy] = balanceInShares[msg.sender][_token][_currentStrategy].sub(_amount, 'SS5');
    ...
    function transfer(
        ...
        require(_amount != 0, 'T1');
        require(_to != address(0), 'T2');
        require(IStrategyRegistry(strategyRegistry).registry(_strategy) != 0, 'T3');

        _amount = IYield(_strategy).getSharesForTokens(_amount, _token);

        balanceInShares[msg.sender][_token][_strategy] = balanceInShares[msg.sender][_token][_strategy].sub(_amount, 'T4');
    ...
    function transferShares(
        ...
        require(_shares != 0, 'TS1');
        require(_to != address(0), 'T2');
        require(IStrategyRegistry(strategyRegistry).registry(_strategy) != 0, 'T3');

        balanceInShares[msg.sender][_token][_strategy] = balanceInShares[msg.sender][_token][_strategy].sub(_shares, 'TS2');
    ...
    function transferFrom(
        ...
        require(_amount != 0, 'TF1');
        require(_from != address(0), 'TF2');
        require(_to != address(0), 'TF3');
        require(IStrategyRegistry(strategyRegistry).registry(_strategy) != 0, 'TF4');

        //update allowance
        uint256 _currentAllowance = allowance[_from][_token][msg.sender];
        if (_currentAllowance != type(uint256).max) {
            allowance[_from][_token][msg.sender] = _currentAllowance.sub(_amount, 'TF5');
        }

        _amount = IYield(_strategy).getSharesForTokens(_amount, _token);

        //reduce sender's balance
        balanceInShares[_from][_token][_strategy] = balanceInShares[_from][_token][_strategy].sub(_amount, 'TF6');
    ...
    function transferSharesFrom(
        ...
        uint256 _currentAllowance = allowance[_from][_token][msg.sender];
        if (_currentAllowance != type(uint256).max) {
            allowance[_from][_token][msg.sender] = _currentAllowance.sub(_amount, 'TFS2');
        }

        balanceInShares[_from][_token][_strategy] = balanceInShares[_from][_token][_strategy].sub(_shares, 'TFS3');
    ...
    function withdraw(
        ...
        require(_to != address(0), 'W2');
        require(IStrategyRegistry(strategyRegistry).isValidStrategy(_strategy), 'W3');

        _amount = IYield(_strategy).getSharesForTokens(_amount, _token);

        balanceInShares[msg.sender][_token][_strategy] = balanceInShares[msg.sender][_token][_strategy].sub(_amount, 'W4');

        uint256 _amountReceived = _withdraw(_token, _strategy, _to, _amount, _receiveShares);

        emit Withdrawn(msg.sender, _to, _amount, _token, _strategy, _receiveShares);
    ...
    function withdrawShares(
        ...
        require(_shares != 0, 'WS1');
        require(_to != address(0), 'W2');
        require(IStrategyRegistry(strategyRegistry).isValidStrategy(_strategy), 'W3');

        balanceInShares[msg.sender][_token][_strategy] = balanceInShares[msg.sender][_token][_strategy].sub(_shares, 'WS2');
    ...
    function withdrawFrom(
        ...
        require(_amount != 0, 'WF1');
        require(_from != address(0), 'WF2');
        require(_to != address(0), 'WF3');
        require(IStrategyRegistry(strategyRegistry).isValidStrategy(_strategy), 'WF4');

        uint256 _currentAllowance = allowance[_from][_token][msg.sender];
        if (_currentAllowance != type(uint256).max) {
            allowance[_from][_token][msg.sender] = _currentAllowance.sub(_amount, 'WF5');
        }

        _amount = IYield(_strategy).getSharesForTokens(_amount, _token);

        balanceInShares[_from][_token][_strategy] = balanceInShares[_from][_token][_strategy].sub(_amount, 'WF6');
        uint256 _amountReceived = _withdraw(_token, _strategy, _to, _amount, _receiveShares);
        emit Withdrawn(_from, msg.sender, _amount, _token, _strategy, _receiveShares);
        return _amountReceived;
    }
    ...
    function withdrawSharesFrom(
        ...
        allowance[_from][_token][msg.sender] = allowance[_from][_token][msg.sender].sub(_amount, 'WSF2');

        balanceInShares[_from][_token][_strategy] = balanceInShares[_from][_token][_strategy].sub(_amount, 'WSF3');
        uint256 _amountReceived = _withdraw(_token, _strategy, _to, _shares, _receiveShares);
        emit Withdrawn(_from, msg.sender, _shares, _token, _strategy, _receiveShares);
        return _amountReceived;
    }
    ...
    function _withdraw(
        ...
        if (_receiveShares) {
            _tokenReceived = _strategyContract.liquidityToken(_token);
            require(_tokenReceived != address(0), 'IW1');
            _amountReceived = _strategyContract.unlockShares(_tokenReceived, _to, _amount);
        } else {
            _tokenReceived = _token;
            _amountReceived = _strategyContract.unlockTokens(_token, _to, _amount);
        }
        return _amountReceived;
    }
    ...
    function withdrawAll(address _token, address _strategy) external override nonReentrant returns (uint256) {
        require(IStrategyRegistry(strategyRegistry).isValidStrategy(_strategy), 'WA1');
