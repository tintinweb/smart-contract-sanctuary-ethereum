...
contract SugarPretzels is ERC721, ChainlinkClient, ERC2771Context, Ownable {
    ...
    // The tokenId of the last token
    uint256 internal _currentIndex;

    mapping(address => uint256) public tokensMintedPerWallet;
    uint256 public maxMintablePerAddress = 2;
    ...
    function setMaxMintablePerAddress(uint256 _maxMintablePerAddress)
        external
        onlyOwner
    {
        maxMintablePerAddress = _maxMintablePerAddress;
    }
        ...
        returns (uint256[] memory)
    {
        ...
        for (uint256 i = 1; i < NUM_WORDS; i++) {
            randomWords[0] = uint256(keccak256(abi.encode(randomWords[i - 1])));
        }
    ...
    function mint() external {
        require(
            tokensMintedPerWallet[_msgSender()] < maxMintablePerAddress,
            "Maximum number of mints reached"
        );

        handleMint(_msgSender(), generatePretzel(getRandomWords(_msgSender())));
    }

    function handleMint(address minter, Pretzel memory pretzel) private {
        unchecked {
            _currentIndex++;
            tokensMintedPerWallet[_msgSender()]++;
        }
