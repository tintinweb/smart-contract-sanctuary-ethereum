...
contract MoonwalkersA is ERC721A, Pausable, Ownable, ReentrancyGuard {
  ...
  /// @dev Constructor.
  constructor(bytes32 _merkleRootWhitelist, bytes32 _merkleRootElite) ERC721A("Moonwalkers", "MW") {
    baseTokenURI = "";
  ...
  function allowlistMint(uint256 _mintAmount, bytes32[] calldata _merkleProof) external payable whenNotLocked callerIsUser {
    SaleConfig memory config = saleConfig;
    require(block.timestamp > config.presaleStartTime && block.timestamp < config.publicSaleStartTime, "Presale is not active yet.");
    require(_totalMinted().add(_mintAmount) < maxSupply.add(1), "Max supply reached.");
