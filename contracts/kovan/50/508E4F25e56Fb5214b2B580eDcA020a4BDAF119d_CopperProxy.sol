...
contract CopperProxy is Ownable {
    ...
    function exitPool(address pool, uint256 maxBPTTokenOut, bool isStandardFee) external onlyPoolOwner(pool) {
        ...
        _distributeTokens(pool, mainToken, fundToken, isStandardFee);
    }

    function getFund(address token) external view returns (uint256){
        return IERC20(address(token)).balanceOf(address(this));
    }

    function _distributeTokens(address pool, address mainToken, address fundToken, bool isStandardFee) internal {
        ...
        uint256 totalPlatformAccessFeeAmount = uint256(2000001); //((fundTokenBalance * platformAccessFeeBPS) / 10_000);
