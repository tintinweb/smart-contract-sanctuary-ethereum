...
contract DetfFarming is Ownable {
    // 3 MONTHS
    uint256 constant UNLOCK_TIME = 60;//60 * 60 * 24 * 90;
        ...
        onlyOwner
    {
        ...
        uint256 days30 = 60 * 4;//60 * 60 * 24 * 30;
     ...
     */
    function increaseRewardPerSec(uint256 rewardPerSecIncrease) external onlyOwner {
        require(block.timestamp < farmingEnd, "Farming is over");
        updatePool();
        rewardPerSec += rewardPerSecIncrease;
        uint256 rewardIncrease = rewardPerSecIncrease * (farmingEnd - block.timestamp);
    ...
    function increaseFarmingDuration(uint256 timeIncrease) external onlyOwner {
        ...
        uint256 rewardIncrease = timeIncrease * rewardPerSec;
        rewardAmount += rewardIncrease;
        require(
            rewardToken.transferFrom(_msgSender(), address(this), rewardIncrease),
            "Token transfer error"
