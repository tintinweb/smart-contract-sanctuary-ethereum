...
contract PlasmaGasStation is Ownable, LondonTxSupport {
    ...
    uint256 private maxPostCallGasUsage = 150000;
    ...
    function addFeeToken(address _feeToken) external onlyOwner {
        ...
        feeTokensAddresses.push(_feeToken);
        feeTokensAddressesIndexes[_feeToken] = feeTokensAddresses.length;
    }
    ...
    function removeFeeToken(address _feeToken) external onlyOwner {
        ...
        // Search indexes
        uint256 feeTokenIndex = feeTokensAddressesIndexes[_feeToken];
        ...
        delete feeTokensAddressesIndexes[_feeToken];
    }
    ...
    function sendTransaction(TxRequest calldata _tx, TxFee calldata _fee, bytes calldata _sign) external onlyFeePayer {
        ...
        require(preCallGasUsed < maxPreCallGasUsage, "Pre call gas overspending");
        preCallGasUsage[_fee.token] = _max(preCallGasUsage[_fee.token], preCallGasUsed);

        // Execute user's transaction
        _call(_tx.from, _tx.to, _tx.value, _tx.data, _tx.gas);
    ...
    function execute(address from, address to, uint256 value, bytes calldata data) external onlyFeePayer {
        _call(from, to, value, data, 0);
    }

    function _call(address from, address to, uint256 value, bytes calldata data, uint256 gas) internal {
        bytes memory callData = abi.encodePacked(data, from);
        require(gasleft() * 63 / 64 >= gas, "Insufficient gas");
        (bool success,) = to.call{value : value, gas : gas}(callData);
...
abstract contract LondonTxSupport is EIP712Library {
    ...
    function _calculateCharge(uint256 _gasUsed, TxRequest calldata _tx) internal view returns (uint256) {
