...
contract RewardCenter is Ownable {
    ...
    event SetAaveResolvers(address indexed owner, address indexed addr);
    ...
    function setAaveResolvers(address addr) public onlyOwner {
        require(addr != address(0), "CHFRY: address should not be 0");
        aaveResolver = addr;
        emit SetAaveResolvers(msg.sender, addr);
    }
    ...
    function checkAaveStakeReward() public view returns (address[] memory collateralToken) {
        // AaveV2Resolver(aaveResolver).getConfiguration()
        address[] memory reservesList = AaveV2Resolver(aaveResolver).getReservesList();

        collateralToken = new address[](reservesList.length);

        address dsa = AccountCenterInterface(accountCenter).getAccount(1);

        (bool[] memory collateral,)=AaveV2Resolver(aaveResolver).getConfiguration(dsa);

        for (uint256 i = 0; i < reservesList.length; i++) {
            if(collateral[i]==true){
                collateralToken[i] = reservesList[i];
            }
        }  
    }
}
