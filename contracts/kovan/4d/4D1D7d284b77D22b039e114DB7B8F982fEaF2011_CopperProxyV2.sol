...
contract CopperProxyV2 is Ownable {
    ...
    function exitPool(address pool, uint256 maxBPTTokenOut, bool isStandardFee) external onlyPoolOwner(pool) {
        ...
        // 4. Distribute tokens and fees
        _distributeTokens(pool, poolTokens, poolData.isCorrectOrder, poolData.fundTokenInputAmount, isStandardFee);
    }
    ...
    function _distributeTokens(
        ...
        address mainToken = poolTokens[isCorrectOrder ? 1 : 0];
        address fundToken = poolTokens[isCorrectOrder ? 0 : 1];
        ...
        uint256 remainingFundBalance = fundTokenBalance;

        if (fundTokenBalance > initialFund) { 
            uint256 totalPlatformAccessFeeAmount = ((fundTokenBalance - initialFund) * platformAccessFeeBPS)
                / _TEN_THOUSAND_BPS;
            remainingFundBalance = fundTokenBalance - totalPlatformAccessFeeAmount;
