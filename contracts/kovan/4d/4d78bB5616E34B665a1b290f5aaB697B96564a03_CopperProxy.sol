...
contract CopperProxy is Ownable {
    ...
    uint32 blacklistMappingVersion; // used for reset
    ...
    */
    ...
    function _removeFromBlacklist(address[] calldata _addresses) internal {
        ...
        for (uint256 i = 0; i < arrayLen; i++) {
            bytes32 key = keccak256(abi.encodePacked(blacklistMappingVersion, _addresses[i]));
            if (!isNotBlacklisted(_addresses[i])) {
                blacklistedAddresses[key] = false;
            }
        }
    }
    ...
    function recoverGas(uint32 _version, string calldata _address) external onlyOwner {
        require(_version < blacklistMappingVersion);
        bytes32 key = keccak256(abi.encodePacked(_version, _address));
