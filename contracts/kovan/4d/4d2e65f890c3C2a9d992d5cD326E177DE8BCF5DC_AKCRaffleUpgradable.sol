...
contract AKCRaffleUpgradable is OwnableUpgradeable {
    IAKC public akc;
    IERC721 public nft;
    ...
    function reserveRaffleProjectSpot(uint256 projectId) external {
        ...
        tribeManager.transferAkcWithoutApproval(
            msg.sender,
            reserve,
            project.price
        );
    ...
    function transfer(uint256 projectId) internal {
        RaffleProject memory project = raffleProjects[projectId];

        require(project.nftTokenId != 0, "No nft token found for project");
        require(project.nftProject != address(0), "No project address found for project");
        require(project.winner != address(0), "Winner not choosen for project");
        ...
        IERC721(project.nftProject).transferFrom(address(this), project.winner, project.nftTokenId);
    }
    ...
    function addOrModifyRaffleProject(
        ...
        if (isModify) {            
            ...
            if (nftId != project.nftTokenId) {
                ...
                IERC721(nftProject).transferFrom(address(this), msg.sender, project.nftTokenId);
            ...
            emit RaffleAddProject(projectId, price, deadline, supply, nftId, nftProject);
        }                
    }
