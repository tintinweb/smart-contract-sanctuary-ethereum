...
contract MockOneInch is Context {
    function calculateDstAmount(
        IERC20 srcToken,
        IERC20 dstToken,
        uint256 srcAmount
    ) public view returns (uint256) {
        uint256 srcTokenBalance = IERC20(srcToken).balanceOf(address(this));
        uint256 dstTokenBalance = IERC20(dstToken).balanceOf(address(this));
        if (srcTokenBalance == 0) {
            return srcAmount;
        }
        uint256 denominator = (srcAmount + srcTokenBalance);
        return ((dstTokenBalance * (denominator - srcTokenBalance)) /
            denominator);
    }
    ...
    function swap(
        ...
        // MockOneInch just returns the same amount

        uint256 dstAmount = calculateDstAmount(
            desc.srcToken,
            desc.dstToken,
            desc.amount
        );

        desc.srcToken.transferFrom(_msgSender(), address(this), desc.amount);
        desc.dstToken.transfer(desc.dstReceiver, dstAmount);
        return dstAmount;
    }
