...
contract RewardsPool is IRewardsPool, Ownable{
    ...
    function _transferUSDCRewards(uint256 _rewards) internal {
        ...
        uint256 spRewardsN = (spRewards+treasuryRewards) * 8e17/1e18 ;
        uint256 treasuryRewardsN = (spRewards+treasuryRewards) * 2e17/1e18 ;

        // 80%(initial) of block rewards to the stability pool
        if(stability_pool.getUSDCDeposits() > 0) {
            bool success_spRewards = usdc_token.transfer(address(stability_pool), spRewardsN);
            require(success_spRewards, "RewardsPool: sending SP rewards failed");
        }
        else {
            bool success_spRewards = usdc_token.transfer(address(treasury), spRewardsN);
            require(success_spRewards, "RewardsPool: sending SP rewards to Treasury failed");
        }

        // 20%(initial) of block rewards to the treasury
        bool success_treasuryRewards = usdc_token.transfer(address(treasury), treasuryRewardsN);
    ...
    receive() external payable {
        ...
        uint usdc_distributed = usdc_token.balanceOf(address(this));
        _transferUSDCRewards(usdc_distributed);
        totalUSDCRewards += usdc_distributed;
        (bool success, ) = payable(address(collateral_pool)).call{ value: address(this).balance }("");
        uint borrower_omatic = omatic_token.balanceOf(address(this));
        omatic_token.transfer(address(borrowers_rewards_pool), borrower_omatic);
        borrowers_rewards_pool.receiveoMATICBorrowersRewardsPool(borrower_omatic);
