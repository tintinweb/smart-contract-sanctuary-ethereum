...
contract BookTreasury is Policy {
    ...
    address public sBOOK;
    uint256 public sBOOKQueue; // Delays change to sBOOK address
    ...
    constructor(
        address _BOOK,
        address _DAI, //address _MIM,//address _FRAX, //address _BOOKFRAX, //address _bondCalculator, //address _DAO,
        ...
        //isLiquidityToken[_BOOKFRAX] = true;
        //liquidityTokens.push(_BOOKFRAX);
        //bondCalculator[_BOOKFRAX] = _bondCalculator;
    ...
    /**
        @notice allow approved address to deposit an asset for BOOK
        @param _amount uint
    ...
    function deposit(
        ...
        // mint BOOK needed and store amount of rewards for distribution
        send_ = value.sub(_profit);
        IBOOKERC20(BOOK).mint(msg.sender, send_);
    ...
    /**
        @notice allow approved address to burn BOOK for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw(uint256 _amount, address _token) external {
        ...
        uint256 value = valueOf(_token, _amount);
        IBOOKERC20(BOOK).burnFrom(msg.sender, value);
    ...
    function incurDebt(uint256 _amount, address _token) external {
        ...
        uint256 maximumDebt = IERC20(sBOOK).balanceOf(msg.sender); // Can only borrow against sBOOK held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with BOOK
        @param _amount uint
     */
    function repayDebtWithROME(uint256 _amount) external {
        require(isDebtor[msg.sender], "Not approved");

        IBOOKERC20(BOOK).burnFrom(msg.sender, _amount);

        debtorBalance[msg.sender] = debtorBalance[msg.sender].sub(_amount);
        totalDebt = totalDebt.sub(_amount);

        emit RepayDebt(msg.sender, BOOK, _amount, _amount);
    }
    ...
    function mintRewards(address _recipient, uint256 _amount) external {
        ...
        IBOOKERC20(BOOK).mint(_recipient, _amount);
    ...
    function excessReserves() public view returns (uint256) {
        return totalReserves.sub(IERC20(BOOK).totalSupply().sub(totalDebt));
    }
    ...
    /**
        @notice returns BOOK valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    ...
    {
        if (isReserveToken[_token]) {
            // convert amount to match BOOK decimals
            value_ = _amount.mul(10**IERC20Metadata(BOOK).decimals()).div(
        ...
        returns (bool)
    {
        ...
        } else if (_managing == MANAGING.SROME) {
            // 9
            sBOOKQueue = block.number.add(blocksNeededForQueue);
        } else return false;
