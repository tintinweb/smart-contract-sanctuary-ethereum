...
contract HaloDAOStratXSGDUSDCIndexUSDC {
    using SafeERC20 for IERC20;
    using SafeMath for uint256;

    address private constant AMM_ZAP_ADDRESS =
        0xE950eC7Fc508dd86fD9B36671f6B1602007D5B72;
    address private constant XSGD_ADDRESS =
        0x70e8dE73cE538DA2bEEd35d14187F6959a8ecA96;
    address private constant USDC_ADDRESS =
        0xA0b86991c6218b36c1d19D4a2e9Eb0cE3606eB48;
    address private constant INDEX_USDC_ADDRESS =
        0x075dA7e9EFEA6813aB0B2680423df75150120d12;
    address private constant XSGD_USDC_CURVE_ADDRESS =
        0x64DCbDeb83e39f152B7Faf83E5E5673faCA0D42A;
    address private constant LENDING_POOL_ADDRESS =
        0xC73b2c6ab14F25e1EAd3DE75b4F6879DEde3968E;
    address private constant HLP_H_TOKEN_ADDRESS =
        0xA8A04EcBBCc5f1B05773A34cE8495507aD6CcA22;
    address private constant UNISWAP_ROUTER_ADDRESS =
        0x68b3465833fb72A70ecDF485E0e4C7bD8665Fc45;
    address private constant LENDING_ORACLE_ADDRESS =
        0x50FDeD029612F6417e9c9Cb9a42848EEc772b9cC;
    address private constant XSGD_USD_ASSIMILATOR_ADDRESS =
        0xCaE2502093413290bc0E5c2CfA1039C661103bf1;
    address private constant USDC_USD_ASSIMILATOR_ADDRESS =
        0x311FDdE361e6258e9730c6147aAf584aC0F9c59A;
    address private constant USDC_INDEX_USDC_POOL_ADDRESS =
        0x203D789bFD07CA0C9675D9b5FFA04D658f1467Fe;

    uint24 public constant POOL_FEE = 3000; // 0.3%, for uniswap trades

    uint256 public constant BASIS_POINTS = 10**4; // applicable for this contract only
    uint256 public constant COLLATERALIZATION_RATIO = 7000; // 70%, for LM loans
    ...
    function zap(
        ...
        require(
            tokenAddress == XSGD_ADDRESS || tokenAddress == USDC_ADDRESS,
            "Unsupported token"
        );
    ...
    function withdraw(
        ...
        IERC20(INDEX_USDC_ADDRESS).safeTransferFrom(
            msg.sender,
            address(this),
            amount
        );

        swap(INDEX_USDC_ADDRESS, USDC_ADDRESS, address(this), amount, slippage);
    ...
    function addLiquidity(
        ...
        IERC20(tokenAddress).safeApprove(AMM_ZAP_ADDRESS, 0);
        IERC20(tokenAddress).safeApprove(AMM_ZAP_ADDRESS, amount);

        uint256 rate = 0;
        if (tokenAddress == XSGD_ADDRESS) {
            rate = IBaseToUsdAssimilator(XSGD_USD_ASSIMILATOR_ADDRESS).getRate();
        } else {
            rate = IBaseToUsdAssimilator(USDC_USD_ASSIMILATOR_ADDRESS).getRate();
        }
        ...
        IZap(AMM_ZAP_ADDRESS).zap(
            XSGD_USDC_CURVE_ADDRESS,
            amount,
            deadline,
            minLpTokens,
    ...
    function depositAndBorrow() internal {
        uint256 hlpBalance = IERC20(XSGD_USDC_CURVE_ADDRESS).balanceOf(
            address(this)
        );

        IERC20(XSGD_USDC_CURVE_ADDRESS).safeApprove(LENDING_POOL_ADDRESS, 0);
        IERC20(XSGD_USDC_CURVE_ADDRESS).safeApprove(
        ...
        uint256 maxBorrowInETH = totalCollateralETH
            .mul(COLLATERALIZATION_RATIO)
            .div(BASIS_POINTS);
        console.log("[StratContract] maxBorrowInETH: ", maxBorrowInETH);
        uint256 usdcPriceInETH = IAaveOracle(LENDING_ORACLE_ADDRESS)
        ...
        ILendingPool(LENDING_POOL_ADDRESS).borrow(
            USDC_ADDRESS,
            usdcToBorrow,
            interestRateMode,
            0, // referral code is now inactive, for future use
    ...
    function swap(
        ...
        IERC20(originTokenAddress).safeApprove(UNISWAP_ROUTER_ADDRESS, 0);
        IERC20(originTokenAddress).safeApprove(UNISWAP_ROUTER_ADDRESS, amount);

        (int24 tick, ) = OracleLibrary.consult(
            USDC_INDEX_USDC_POOL_ADDRESS,
            10
        );
        ...
        uint256 amountReceived = ISwapRouter02(UNISWAP_ROUTER_ADDRESS)
    ...
    function repayAndWithdraw() internal {
        ...
        IERC20(USDC_ADDRESS).safeApprove(LENDING_POOL_ADDRESS, 0);
        IERC20(USDC_ADDRESS).safeApprove(LENDING_POOL_ADDRESS, usdcBalance);
        ...
        ILendingPool(LENDING_POOL_ADDRESS).repay(
            USDC_ADDRESS,
            usdcBalance,
        ...
        ) = ILendingPool(LENDING_POOL_ADDRESS).getUserAccountData(msg.sender);
        ...
        uint256 hlpPriceInETH = IAaveOracle(LENDING_ORACLE_ADDRESS)
            .getAssetPrice(XSGD_USDC_CURVE_ADDRESS);
        ...
        IERC20(HLP_H_TOKEN_ADDRESS).safeTransferFrom(
            msg.sender,
            address(this),
            withdrawAmount
        );
    ...
    function removeLiquidity(address tokenAddress, uint256 deadline) internal {
        ...
        uint256[] memory withdrawals = ICurve(XSGD_USDC_CURVE_ADDRESS).withdraw(
            hlpBalance,
            deadline
        );
        console.log(
        ...
        if (tokenAddress == XSGD_ADDRESS) {
            IERC20(USDC_ADDRESS).safeApprove(XSGD_USDC_CURVE_ADDRESS, 0);
            IERC20(USDC_ADDRESS).safeApprove(
                XSGD_USDC_CURVE_ADDRESS,
                withdrawals[1]
            );

            uint256 targetAmount = ICurve(XSGD_USDC_CURVE_ADDRESS)
                .viewOriginSwap(USDC_ADDRESS, XSGD_ADDRESS, withdrawals[1]);
            console.log("[StratContract] target amount: ", targetAmount);

            ICurve(XSGD_USDC_CURVE_ADDRESS).originSwap(
                USDC_ADDRESS,
                XSGD_ADDRESS,
                withdrawals[1],
                targetAmount,
            ...
            IERC20(XSGD_ADDRESS).safeApprove(XSGD_USDC_CURVE_ADDRESS, 0);
            IERC20(XSGD_ADDRESS).safeApprove(
                XSGD_USDC_CURVE_ADDRESS,
                withdrawals[0]
            );

            uint256 targetAmount = ICurve(XSGD_USDC_CURVE_ADDRESS)
                .viewOriginSwap(XSGD_ADDRESS, USDC_ADDRESS, withdrawals[0]);
            console.log("[StratContract] target amount: ", targetAmount);
    ...
    function transferBalances() internal {
        uint256 indexUsdcBalanceLeft = IERC20(INDEX_USDC_ADDRESS).balanceOf(
            address(this)
        );
        uint256 xsgdBalanceLeft = IERC20(XSGD_ADDRESS).balanceOf(address(this));
        uint256 usdcBalanceLeft = IERC20(USDC_ADDRESS).balanceOf(address(this));
        uint256 hTokenBalanceLeft = IERC20(HLP_H_TOKEN_ADDRESS).balanceOf(
            address(this)
        );
        console.log(
        ...
        if (indexUsdcBalanceLeft > 0) {
            IERC20(INDEX_USDC_ADDRESS).safeTransfer(
                msg.sender,
                indexUsdcBalanceLeft
            );
        }
        if (xsgdBalanceLeft > 0) {
            IERC20(XSGD_ADDRESS).safeTransfer(msg.sender, xsgdBalanceLeft);
        }
        if (usdcBalanceLeft > 0) {
            IERC20(USDC_ADDRESS).safeTransfer(msg.sender, usdcBalanceLeft);
        }
        if (hTokenBalanceLeft > 0) {
            IERC20(HLP_H_TOKEN_ADDRESS).safeTransfer(
                msg.sender,
                hTokenBalanceLeft
            );
        }
