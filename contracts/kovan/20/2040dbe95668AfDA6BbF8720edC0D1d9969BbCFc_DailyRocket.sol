...
contract DailyRocket is Ownable, KeeperCompatibleInterface {
        ...
        )
    {
        ...
        dayCount = 1;
        dayCloseTime[1] = block.timestamp + 84600;
    }
    ...
    {   
        require(getTime() <= dayCloseTime[dayCount] + 64800 seconds);//@dev: After this time, one cannot predict    
    ...
    {
        upkeepNeeded = dayCloseTime[dayCount] >= getTime();
        
    }
    

    function performUpkeep(bytes calldata performData) external override {
        if (dayCloseTime[dayCount] >= getTime()) {
            dayCloseTime[dayCount + 1] = block.timestamp + 86400 seconds;
            setNewClosingPrice();
