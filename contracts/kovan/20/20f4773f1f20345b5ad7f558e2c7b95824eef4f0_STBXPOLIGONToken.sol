...
/**
 * @title STBXPOLIGON | Stobox Technologies Inc.
 * @author Stobox Technologies Inc.
 * @dev STBXPOLIGON ERC20 Token | This contract is opt for digital securities management.
 */
...
// File: contracts/STBXPOLIGON.sol
...
contract STBXPOLIGONToken is IERC20, Roles {
    ...
    modifier onlyWhitelisted(address _account) {
        if (_isDisabledWitelist) {
            require(
                ...
                "STBXPOLIGON: not whitelisted address."
            );
        }
        _;
    }
    ...
    modifier onlyWithUnfrozenFunds(address _account) {
        require(
            ...
            "STBXPOLIGON: funds are frozen."
        );
        _;
    }

    /**
     * @notice STBXPOLIGONToken simply implements a ERC20 token.
     */
    constructor() public Roles(msg.sender) {
        _name = "Stobox Poligon Security Token";
        _symbol = "STBXPOLIGON";
        _decimals = 0;
        _kDecimals = 18;
        _k = 10**_kDecimals;
    ...
    function burn(address _account, uint256 _amount) external onlySuperAdmin {
        require(balanceOf(_account) >= _amount, "STBXPOLIGON: balance too low");
    ...
    function transferFunds(
        ...
        require(balanceOf(_from) >= _amount, "STBXPOLIGON: not enough tokens");
    ...
    function splitOrMerge(uint256 _x, uint256 _y) external onlySuperAdmin {
        require(_x != _y, "STBXPOLIGON: _x must not be equal to _y");
        require(_x > 0, "STBXPOLIGON: _x must be greater than 0");
        require(_y > 0, "STBXPOLIGON: _y must be greater than 0");
    ...
    */
        ...
        returns (bool)
    {
        ...
        _approve(
            ...
            _allowances[_sender][msg.sender].sub(
                ...
                "STBXPOLIGON: transfer amount exceeds allowance"
            )
        );
        ...
        returns (bool)
    {
        _approve(
            ...
            _allowances[msg.sender][_spender].sub(
                ...
                "STBXPOLIGON: decreased allowance below zero"
            )
        );
    ...
    function _transfer(
        ...
        require(_sender != address(0), "STBXPOLIGON: transfer from the zero address");
        require(_recipient != address(0), "STBXPOLIGON: transfer to the zero address");

        _beforeTokenTransfer(_sender, _recipient, _amount);

        _balances[_sender] = _balances[_sender].sub(
            _amount,
            "STBXPOLIGON: transfer amount exceeds balance"
        );
    ...
    function _mint(address _account, uint256 _amount) internal virtual {
        require(_account != address(0), "STBXPOLIGON: mint to the zero address");
    ...
    function _burn(address _account, uint256 _amount) internal virtual {
        require(_account != address(0), "STBXPOLIGON: burn from the zero address");

        _beforeTokenTransfer(_account, address(0), _amount);

        _balances[_account] = _balances[_account].sub(
            _amount,
            "STBXPOLIGON: burn amount exceeds balance"
        );
    ...
    function _approve(
        ...
        require(_owner != address(0), "STBXPOLIGON: approve from the zero address");
        require(_spender != address(0), "STBXPOLIGON: approve to the zero address");
    ...
    function _updateTransferLimit(address _account, uint256 _amount) private {
        ...
        _transferLimits[_account].allowedToTransfer = _transferLimits[_account]
            .allowedToTransfer
            .sub(_amount, "STBXPOLIGON: transfer exceeds your transfer limit");
    }
    ...
    function _updateTransactionCountLimit(address _account) private {
        if (_isEnabledTransactionCount) {
            ...
            _transactionCountLimits[_account]
                ...
                .sub(1, "STBXPOLIGON: transfer exceeds your transaction count limit");
        }
    }
}