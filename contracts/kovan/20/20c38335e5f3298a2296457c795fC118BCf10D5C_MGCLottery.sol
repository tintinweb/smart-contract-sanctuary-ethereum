...
contract MGCLottery is Ownable {
    ...
    function spin(uint _pid, uint nonce, uint number, uint amount) public {
        ...
        require(amount >= p.minTokenAmount && amount < p.maxTokenAmount, "MGCLottery::spin: amount invalid");
        ...
        holderRewardTreasury.reflect(holderReward);
        ...
        if(result) {
            ...
            user.rewardInfo[block.timestamp] = RewardInfo(_pid, reward, block.timestamp + p.lockTime, resultNumber, false);
            setTop10Winner();
        }
    }
