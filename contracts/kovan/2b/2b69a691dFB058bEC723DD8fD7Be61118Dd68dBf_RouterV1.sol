...
contract RouterV1 {
        ...
        )
    {
        ...
        uint256 rolloverAmtRemainder = r.trancheOutAmt > 0
            ? (r.requestedRolloverAmt * (trancheOutAmtForRequested - r.trancheOutAmt)).ceilDiv(trancheOutAmtForRequested)
    ...
    // @notice Rolls over given set of tranchesIn and amounts for set of tranchesOut.
    // @param perp Address of the perpetual tranche contract.
    // @param tranchesIn List of tranche tokens deposited.
    // @param tranchesOut List of tranche tokens requested to be redeemed.
    // @param trancheInAmts List of trancheIn token amounts deposited.
    function rollover(
        IPerpetualTranche perp,
        ITranche[] memory tranchesIn,
        ITranche[] memory tranchesOut,
        uint256[] memory trancheInAmts,
        uint256 fee
    ) external {
        require(tranchesIn.length == tranchesOut.length, "Expect tranche out for tranche out");
        require(tranchesIn.length == trancheInAmts.length, "Expected tranche amount for every tranche in");
        ...
        // approve fees to be spent by perp
        _checkAndApproveMax(feeToken, address(perp), fee);

        for (uint256 i = 0; i < tranchesIn.length; i++) {
            // transfer trancheIn to router
            tranchesIn[i].safeTransferFrom(msg.sender, self, trancheInAmts[i]);

            // approve trancheIn to be spent by perp
            _checkAndApproveMax(tranchesIn[i], address(perp), trancheInAmts[i]);

            // perform rollover
            perp.rollover(tranchesIn[i], tranchesOut[i], trancheInAmts[i]);

            // transfer trancheOut tokens back to the user
            tranchesOut[i].safeTransfer(msg.sender, tranchesOut[i].balanceOf(self));
        }
    ...
    // @dev Pokes the perp contract to update the time dependent queue state.
    //      Used by "preview" functions to bring the state up to date.
    function _updateTrancheQueue(IPerpetualTranche perp) private {
        perp.getDepositBond();
        perp.getRedemptionTranche();        
    }
}
