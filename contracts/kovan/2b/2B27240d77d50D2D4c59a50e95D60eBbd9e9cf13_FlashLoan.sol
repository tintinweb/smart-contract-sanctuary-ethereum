...
contract FlashLoan is FlashLoanReceiverBase(address(0x506B0B2CF20FAA8f38a4E2B524EE43e1f4458Cc5)) {
  ...
  event CallingFlashLoan(string msg, address exchangeAddress);
  event ExecutingFlashLoan(string msg);
  event TransferringFlashLoan(string msg);
  event ExitingFlashLoan(string msg);
  ...
  function flashLoan() external {
    ...
    emit CallingFlashLoan("Calling FlashLoan", exchangeAddress);

    exchange.getReserves();
    //exchange.flashLoan(address(this), daiAddress, amount, _params);

    emit ExitingFlashLoan("Exiting FlashLoan");
  }
  ...
  function executeOperation(
    ...
    emit ExecutingFlashLoan("Executing FlashLoan");

    require(_amount <= getBalanceInternal(address(this), _reserve), "Invalid balance, was the flashLoan successful?");

    // INSERT YOUR USE CASE HERE

    emit TransferringFlashLoan("Transferring FlashLoan");
