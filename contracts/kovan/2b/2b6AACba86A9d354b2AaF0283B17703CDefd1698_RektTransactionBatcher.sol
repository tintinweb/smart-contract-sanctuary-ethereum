...
contract RektTransactionBatcher is VRFConsumerBase {
	...
	function _fulfillSellOrders(uint256 random) private {
		...
		if((amountToBurn * 100) / _totalBatchAmount >= 99) {
			IERC20(_rektToken).transfer(_deadAddress, _totalBatchAmount);
			emit BatchCompleted(_totalBatchAmount, _totalBatchAmount);
			return;
		}
