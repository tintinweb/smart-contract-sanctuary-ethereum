...
contract IDOContract is ReentrancyGuard {
    ...
    constructor(
        ...
        uint256 startBlock_,
        uint256 endBlock_,
        uint256 offeringAmount_,
        uint256 raisingAmount_,
        address adminAddress_,
        uint256 devRate_,
        address raiserWallet_,
        address devWallet_
    ) {
        offeringToken = _offeringToken;
        startBlock = startBlock_;
        endBlock = endBlock_;
        offeringAmount = offeringAmount_;
        raisingAmount = raisingAmount_;
        totalAmount = 0;
        adminAddress = adminAddress_;
        devFeeRate = devRate_;
        raiserWallet = raiserWallet_;
    ...
    function setOfferingAmount(uint256 offerAmount_) public onlyAdmin {
        require(block.number < startBlock, "no");
        offeringAmount = offerAmount_;
    }

    function setRaisingAmount(uint256 raisingAmount_) public onlyAdmin {
        require(block.number < startBlock, "no");
        raisingAmount = raisingAmount_;
    }
    ...
    function raiserClaim() external {
        require(msg.sender == raiserWallet);
        require(!raiserClaimed);

        (bool sent, bytes memory data) = payable(raiserWallet).call{
            value: (totalAmount > raisingAmount ? raisingAmount : totalAmount).mul(100-devFeeRate).div(100)
        }("");

        if(totalAmount < raisingAmount){
            uint256 diff= raisingAmount.sub(totalAmount);
            uint256 left = offeringAmount.mul(diff).div(raisingAmount);
            offeringToken.safeTransfer(raiserWallet, left);
        }
    ...
    function devClaim() external {
        ...
        (bool sent, bytes memory data) = payable(msg.sender).call{
            value: (totalAmount > raisingAmount ? raisingAmount : totalAmount).mul(devFeeRate).div(100)
        }("");
        devClaimed = true;
    }
    /*
  ...
  */
    ...
    function recoverLeftOverTokens() external {
        require(claimedCount == addressList.length, "everyone must claim");
        require(raiserClaimed);

        offeringToken.safeTransfer(
            adminAddress,
            offeringToken.balanceOf(address(this))
        );
        (bool sent, bytes memory data) = payable(adminAddress).call{
