...
contract TokenFarm is Ownable {
    ...
    function issueTokens() public onlyOwner {
        for (uint256 idx = 0; idx < stakers.length; idx++) {
            ...
            // based on their total locked value
        }
    }
    ...
    function getUserTotalValue(address _user) public view returns (uint256) {
        ...
        for (uint256 idx = 0; idx < allowedTokens.length; idx++) {
            totalValue += getUserSingleTokenValue(_user, allowedTokens[idx]);
        }
        return totalValue;
    }
        ...
        returns (uint256)
    {
        ...
        // price of the token*stakingbalnce[_token][_user]
    ...
    function tokenIsAllowed(address _token) internal returns (bool) {
        for (uint256 idx = 0; idx < allowedTokens.length; idx++) {
            if (allowedTokens[idx] == _token) return true;
        }
        return false;
    }
}