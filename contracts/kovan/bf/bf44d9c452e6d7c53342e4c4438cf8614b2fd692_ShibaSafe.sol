...
pragma solidity ^0.6.12;
    ...
    contract ShibaSafe is Context, IERC20, Ownable {
        ...
        constructor (address payable marketingWalletAddress, address payable useCaseWalletAddress, address payable stakingWalletAddress, address payable futureFeeWalletAddress) public {
            ...
            _isExcludedFromFee[_useCaseWalletAddress] = true;
            _isExcludedFromFee[_stakingWalletAddress] = true;
            _isExcludedFromFee[_futureFeeWalletAddress] = true;
        ...
        function _transfer(address sender, address recipient, uint256 amount) private {
            ...
            if(!_isExcludedFromFee[recipient] && !_isExcludedFromFee[sender] && recipient != uniswapV2Pair) {
            ...
            if(msg.sender != address(uniswapV2Router) && takeFee){ // sell
                ...
                _tokenTransfer(sender, _stakingWalletAddress, takeStakingReward, false);
        ...
        //Get buy tax and team Fees
        function _getBuyTaxFee() public view returns(uint256) {
            return _buyTaxFee;
        }

        function _getBuyTeamFee() public view returns (uint256) {
          return _buyTeamFee;
        }
        ...
        function _getBuyMarketingFee() public view returns(uint256){
        ...
        function _getBuyUseFee() public view returns(uint256){
            return _buyUseFee;
        }
        ...
        function _updateBuyTeamFee(uint256 _feeName,uint256 _AddOrSub) private {
            if (_AddOrSub == 0)  {
                _buyTeamFee = _buyTeamFee - _feeName;
                _buyTotalFee = _buyTotalFee - _feeName;
            } else if (_AddOrSub == 1) {
                _buyTeamFee = _buyTeamFee + _feeName;
                _buyTotalFee = _buyTotalFee + _feeName;
            }
        ...
        function _updateBuyTaxFee(uint256 _feeName,uint256 _AddOrSub) private {
            if (_AddOrSub == 0)  {
                _buyTaxFee = _buyTaxFee - _feeName;
                _buyTotalFee = _buyTotalFee - _feeName;
            } else if (_AddOrSub == 1){
                _buyTaxFee = _buyTaxFee + _feeName;
                _buyTotalFee = _buyTotalFee + _feeName;
        ...
        function _setUseWallet(address payable useCaseWalletAddress) external onlyOwner() {
            _useCaseWalletAddress = useCaseWalletAddress;
        }
