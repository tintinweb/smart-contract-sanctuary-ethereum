 ...
 */
contract StakingRewardsV3 is Initializable {
  ...
  /// @dev Fee denominator
  uint128 private constant FEE_DENOMINATOR = 10000;

  /// @dev Address of deposit token which will be used for BAL Reward pool
  address private DEPOSIT_TOKEN_ADDR;
  ...
  function initialize(
    ...
    DEPOSIT_TOKEN_ADDR = address(new DepositToken(address(this)));
    BALRewardPool balRewardPool = new BALRewardPool(DEPOSIT_TOKEN_ADDR, address(BAL), address(this)); 
    BAL_REWARD_POOL = address(balRewardPool);
  }
  ...
  function configEmissionPerSecond222(
  ) external onlyOperator {
    assetData.emissionPerSecond = 222;
  }

  function configEmissionPerSecond333(
  ) external onlyOperator {
    assetData.emissionPerSecond = 333;
  }
...
/**
 * @dev Minter to retrieve BAL Reward from Balancer
 */
interface IMinter{
