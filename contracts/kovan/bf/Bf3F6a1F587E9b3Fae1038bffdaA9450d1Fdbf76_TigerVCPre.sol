...
contract TigerVCPre is AccessControl, ReentrancyGuard {
    ...
    mapping(address => User) public whitelist;  // high-level whitelist users Address-to-claimable-amount mapping
    uint public fund;                           // Payment tokens collected
    uint public minPurchase = 1;                // Minimum NFT to buy per purchase
    uint public maxPurchase = 10;                // Maximum NFT to buy per purchase
    ...
    function setWhitelist(address _whitelisted) public onlyRole(DEFAULT_ADMIN_ROLE) {
        whitelist[_whitelisted] = User(0, 0);
        emit WhitelistAdded();
    }

    function setWhitelistBatch(address[] calldata _whitelisted) public onlyRole(DEFAULT_ADMIN_ROLE) inPause() {
        for (uint i = 0; i < _whitelisted.length; i++) {
            whitelist[_whitelisted[i]] = User(0, 0);
        }
        emit WhitelistBatchAdded();
    }

    function preSale() public payable inProgress() nonReentrant {
        require((requireWhitelist && whitelist[msg.sender].amount >= 0) || !requireWhitelist, "whitelisting for external users is disabled");
        require(msg.value / unitPrice > minPurchase, "Amount must >= minPurchase");
        uint amount = msg.value / unitPrice;
        uint total = amount + whitelist[msg.sender].amount;
        require(total < maxPurchase, "Reached MaxPurchase!");
        whitelist[msg.sender].amount = whitelist[msg.sender].amount + amount;
        whitelist[msg.sender].balance = whitelist[msg.sender].balance + msg.value;
        fund += msg.value;
    }

    function claimFund() public inProgress() nonReentrant {
        require(requireWhitelist && whitelist[msg.sender].amount >= 1, "whitelisting for external users is disabled");
        uint toTransfer = whitelist[msg.sender].balance;
        delete whitelist[msg.sender];
        emit DeleteUser(msg.sender);
        sendValue(payable(msg.sender), toTransfer);
        emit UserFundClaimed(msg.sender, fund);
    }
    ...
    function claimTotalFund() public onlyRole(CLAIM_FUND_ROLE) inPause() nonReentrant {
