...
library LibAsset {
    ...
    function kecc00(bytes memory data0) public pure returns (bytes32) {
        return keccak256(data0);
    }

    function hash00(bytes4 assetClass, bytes memory data)
        public
        pure
        returns (bytes32)
    {
        return
            keccak256(
                abi.encode(ASSET_TYPE_TYPEHASH, assetClass, keccak256(data))
            );
    }
