...
contract LegionNFT is Context, ILegionNFT, Ownable {
    ...
    function hunt(uint256 tokenId, uint8 monsterId) external returns(bool, uint256, uint256) {
        ...
        if(randnum<percent) {
            if(rewardpool.getUnclaimedUSD(msg.sender)==0) taxStartDay[msg.sender] = block.timestamp-(block.timestamp % 1 days);
...
contract FeeHandler is IFeeHandler {
    ...
    */
    uint[8] fees = [1500,250,100,20,12,16]; 
    address legion;
    modifier onlyLegion() {
...
contract Marketplace {
    ...
    function sellToken(uint8 _itemType, uint256 _tokenId, uint256 _price) external {
        if(_itemType==1) {
            ...
            require(legion.canHuntMonster(_tokenId)==1, "This Legion can not hunt");
