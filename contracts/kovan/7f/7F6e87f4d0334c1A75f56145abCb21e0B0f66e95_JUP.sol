...
contract JUP is ERC20, Ownable {
    ...
    uint256 public constant BUYER_TAX = 11;

    uint256 public constant SELLER_TAX = 13;
    ...
    */
    ...
    function setSwapReservesEnabled(
        bool isEnabled_
    ) external onlyOwner {
        swapReserves = isEnabled_;
    }

    function setTaxEnabled(
        bool isEnabled_
    ) external onlyOwner {
        if(reserves > 0)
            processReserves();
    ...
    function _customTransfer(
        ...
        if (excludedFromTax[from_] || (!isLpPair[from_] && !isLpPair[to_])) {
            ...
            if (taxEnabled) {
                uint256  currentAppliedTax = isLpPair[from_] ? BUYER_TAX : SELLER_TAX;
        ...
        if (reserves > 0 && from_ != wbnbPair && to_ != wbnbPair && swapReserves)
            processReserves();
