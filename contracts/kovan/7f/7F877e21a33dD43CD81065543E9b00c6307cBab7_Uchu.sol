...
contract Uchu is Initializable {
    ...
    function updateProduct(uint8 productId, uint deposit, uint withdraw, uint borrow, uint repay) private {
        if (product_list[productId].debt == 0) {
            if (deposit != 0) {
                ...
                product_list[productId].debt = setProductDebt(productId, borrow, 0, false);
            ...
            if (deposit != 0) {
                ...
                product_list[productId].balance += deposit;
                product_list[productId].lending_share = setProductLendShare(productId, deposit, withdraw);
    ...
    **/
    function lenderShare(address user, uint8 productId, uint index, uint deposit, uint withdraw, bool isIn) private {
        ...
        if (product.debt == 0) {
            ...
            position_map[user][index].share += (deposit - withdraw) / lenderPrice(productId, deposit, withdraw);
        }
    }
    ...
    **/
    function lenderPrice(uint8 productId, uint deposit, uint withdraw) private returns (uint) {
        Product memory product = product_list[productId];
        uint totalAmount = product.balance + lenderEarning(productId);
        return totalAmount / setProductLendShare(productId, deposit, withdraw);
    }
