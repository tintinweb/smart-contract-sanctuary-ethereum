...
import "./interfaces/IWorkerUniV3.sol";
...
contract Vault is IVault, ERC20, ReentrancyGuard, Ownable {
  ...
  /// debtToken - just a simple ERC20 token .
  ...
  function positionInfo(uint256 id) external view returns (uint256, uint256) {
    ...
    return (IWorkerUniV3(pos.worker).health(id), debtShareToVal(pos.debtShare));
  }

  /// @dev Return owner of the given position.
  /// @param id The position ID to query.
  function positionOwner(uint256 id) public view override returns (address) {
    Position storage pos = positions[id];
    return pos.owner;
  }
  ...
  function _deposit(uint256 amountToken) internal {
    ...
    require(
      totalSupply() > 10**uint256(decimals() - 1),
  ...
  function _safeUnwrap(address to, uint256 amount) internal {
    if (token == config.getWrappedNativeAddr()) {
      ...
      IWNativeRelayer(payable(config.getWNativeRelayer())).withdraw(amount);
    ...
    nonReentrant
  {
    ...
    uint256 back;
    {
      ...
      IWorkerUniV3(worker).work(id, msg.sender, debt, data);
    ...
    if (debt > 0) {
      ...
      uint256 health = IWorkerUniV3(worker).health(id);
    ...
    nonReentrant
  {
    ...
    uint256 health = IWorkerUniV3(pos.worker).health(id);
    ...
    IWorkerUniV3(pos.worker).liquidate(id);
