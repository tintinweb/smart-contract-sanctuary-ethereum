...
contract RewardCenter is Ownable {
    ...
    uint256 public lastRewardReleasedEpochRound;

    event SetRewardToken(address indexed owner, address indexed token);

    event SettleOpenAccountReward(
    ...
    event ReleasePositionReward(
        address indexed owner,
        uint256 epochRoundUpdateTo,
        uint256 rewardAmount,
        bytes32 merkelRoot
    );

    event ClaimPositionReward(address EOA, uint256 amount);

    event ClaimOpenAccountReward(address EOA, address account, uint256 amount);
    ...
    function settleOpenAccountReward(uint256 _rewardAmountPerAccoutOpen)
        public
        onlyOwner
    {
        rewardAmountPerAccoutOpen = _rewardAmountPerAccoutOpen;
        emit SettleOpenAccountReward(msg.sender, rewardAmountPerAccoutOpen);
    }

    function sendCheeseTo(address EOA, address account) public {
        require(
    ...
    function claimPositionReward(
        ...
        emit ClaimPositionReward(msg.sender,amount);
    }
}
