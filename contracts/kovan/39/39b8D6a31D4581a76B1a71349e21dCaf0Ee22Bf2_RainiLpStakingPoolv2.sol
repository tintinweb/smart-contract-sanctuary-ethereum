...
contract RainiLpStakingPoolv2 is AccessControl, ReentrancyGuard {
    ...
    uint256 public constant RAINI_REWARD_DECIMALS = 10000000000000;

    uint256 public unicornToFTM;
    ...
    event UnicornToFTMSet(uint256 unicornToFTM);
    ...
    constructor(address _rainiLpToken, address _rainiToken) {
        require(
            _rainiLpToken != address(0),
    ...
    modifier onlyBurner() {
        require(
            ...
            "RainiLpStakingPool: caller is not a burner"
        );
        _;
    }
    ...
    modifier onlyMinter() {
        require(
            ...
            "RainiLpStakingPool: caller is not a minter"
        );
        _;
    }
    ...
    modifier balanceUpdate(address _owner) {
        ...
        // Raini rewards
        _generalRewardVars.rainiRewardPerTokenStored = uint64(
            rainiRewardPerToken()
        );
        ...
        if (_owner != address(0)) {
            ...
            uint128 reward = calculateReward(
                _owner,
                _accountVars.staked,
                duration
            );
            ...
            _accountRewardVars.lastBonus = uint64(
                Math.min(
                    maxBonus,
                    _accountRewardVars.lastBonus + bonusRate * duration
                )
            );

            _accountRewardVars.rainiRewards = uint96(rainiEarned(_owner));
            _accountRewardVars.rainiRewardPerTokenPaid = _generalRewardVars
    ...
    function balanceOf(address _owner) public view returns (uint256) {
        uint256 reward = calculateReward(
            _owner,
            accountVars[_owner].staked,
            block.timestamp - accountRewardVars[_owner].lastUpdated
        );
        ...
        returns (uint256)
    {
        ...
        if (_accountRewardVars.lastBonus < maxBonus) {
            uint256 durationTillMax = (maxBonus -
                _accountRewardVars.lastBonus) / bonusRate;
            if (_duration > durationTillMax) {
                uint256 avgWeightedBonusTillMax = ((_accountRewardVars
                    .lastBonus + maxBonus) * durationTillMax) / 2;
                uint256 weightedMaxBonus = maxBonus *
    ...
    function setUnicornToFTM(uint256 _unicornToFTM) external onlyOwner {
        unicornToFTM = _unicornToFTM;

        emit UnicornToFTMSet(_unicornToFTM);
    }
        ...
        balanceUpdate(_msgSender())
    {
        ...
        accountRewardVars[_msgSender()].lastBonus = uint64(
            (accountRewardVars[_msgSender()].lastBonus * currentStake) /
    ...
    function withdrawFTM(uint256 _amount) external onlyOwner {
        require(
            ...
            "RainiLpStakingPool: not enough balance"
        );
        ...
        onlyMinter
    {
        require(
            ...
            "RainiLpv3StakingPool: Arrays not equal"
        );

        for (uint256 i = 0; i < _addresses.length; i++) {
            accountVars[_addresses[i]].unicornBalance = uint128(
                accountVars[_addresses[i]].unicornBalance + _points[i]
            );
    ...
    function buyUnicorn(uint256 _amount) external payable {
        require(_amount > 0, "RainiLpv3StakingPool: _amount is zero");
        require(
            msg.value * unicornToFTM >= _amount,
            "RainiLpv3StakingPool: not enougth eth"
        );

        accountVars[_msgSender()].unicornBalance = uint128(
            accountVars[_msgSender()].unicornBalance + _amount
        );

        uint256 refund = msg.value - (_amount / unicornToFTM);
        if (refund > 0) {
    ...
    {
        accountVars[_owner].unicornBalance = uint128(
            accountVars[_owner].unicornBalance - _amount
        );
    ...
    function calculateReward(
        ...
        uint256 reward = (_duration * rewardRate * _amount) /
            (REWARD_DECIMALS * minRewardStake);
    ...
    function rainiEarned(address account) public view returns (uint256) {
        ...
        uint256 calculatedEarned = (uint256(_accountVars.staked) *
            (rainiRewardPerToken() -
                _accountRewardVars.rainiRewardPerTokenPaid)) /
            RAINI_REWARD_DECIMALS +
            _accountRewardVars.rainiRewards;
    ...
    function recoverRaini(uint256 _amount) external onlyOwner nonReentrant {
        require(
            ...
            "Raini cannot be recovered while reward pool active."
        );
