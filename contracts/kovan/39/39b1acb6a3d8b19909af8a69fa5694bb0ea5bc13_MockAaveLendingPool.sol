...
import "../../interfaces/aaveV2/IAaveFlashLoanReceiverV2.sol";
...
contract MockAaveLendingPool is IAaveLendingPoolV2 {
    ...
    constructor(uint256 _flashLoanPremiumTotal) {
    ...
    function flashLoan(
        ...
        for (index = 0; index < assets.length; index++) {
            premiums[index] = amounts[index] + (amounts[index] * flashLoanPremiumTotal) / 10000;

            MockToken(assets[index]).mint(address(this), amounts[index]);

            // transfer tokens to receiver
            IERC20(assets[index]).transfer(receiverAddress, amounts[index]);
        }

        require(
            IAaveFlashLoanReceiverV2(receiverAddress).executeOperation(
                assets,
                amounts,
                premiums,
                msg.sender,
        ...
        for (index = 0; index < assets.length; index++) {
            // transfer tokens from receive again with fee
            IERC20(assets[index]).safeTransferFrom(receiverAddress, address(this), premiums[index]);
        }
    }
}
