...
// this contract needs to be able to:
// 1. stake tokens
// 2. unstake tokens
// 3. issue token for rewards
// 4. add allowed tokens
// 5. get the value of the underlying staked tokens
    ...
    //how many tokens does each address have staked || wallet address => number of unique tokens staked
...
// 100 ETH 1:1 for every 1 ETH, we give 1 RWRD
// 50 ETH and 50 DAI staked, we want to give a reward of 1 DAPP / 1 DAI
    ...
    function issueTokens() public onlyOwner {
        ...
        for (uint256 stakersIndex = 0; stakersIndex < stakers.length; stakersIndex++) {
            ...
            //send them a token reward based on TVL
    ...
    function stakeTokens(uint256 _amount, address _tokenAddress) public {
        ...
        require(
            ...
            "This token is currently not approved for staking"
        );

        IERC20(_tokenAddress).transferFrom(msg.sender, address(this), _amount);

        updateUniqueTokensStaked(msg.sender, _tokenAddress);
        // this interface ^^ wraps the token and gives us the abi which allows us to interact with the contract

        stakingBalance[_tokenAddress][msg.sender] += _amount;

        // if this is their first token to stake, the value will be 1
    ...
    function unstakeTokens(address _token) public {
        ...
        stakingBalance[_token][msg.sender] = 0; // vulnerable to re-entrancy attacks? I think yes, because staking balance is set to 0 after the transfer has occurred
    ...
    function addAllowedToken(address _tokenAddress) public {
