...
import "./lib/PreSalesActivation.sol";
import "./lib/PublicSalesActivation.sol";
    ...
    Withdrawable
{
    ...
    uint256 public constant TEAM_MAX_QTY = 200;
    uint256 public constant PRESALES_MAX_QTY = 7500;
    uint256 public constant SALES_MAX_QTY = TOTAL_MAX_QTY - TEAM_MAX_QTY;
    uint256 public constant MAX_QTY_PER_MINTER = 3;
    uint256 public constant PRE_SALES_PRICE = 0.0002 ether;
    uint256 public constant PUBLIC_SALES_PRICE = 0.0005 ether;
    ...
    function preSalesMint(uint256 _mintQty, bytes calldata _signature)
        external
        payable
        isPreSalesActive
    ...
    function teamMint(address[] calldata receivers) external onlyOwner {
        require(
            teamQty + receivers.length <= TEAM_MAX_QTY,
            "Exceed team max limit"
        );

        teamQty += receivers.length;
    ...
    function setTokenBaseURI(string memory tokenBaseURI) external onlyOwner {
        _tokenBaseURI = tokenBaseURI;
    }

    function setRevealBlock(uint256 _revealBlock) external onlyOwner {
    ...
    function getMetadata(uint256 tokenId) public view returns (string memory) {
        ...
        uint256[] memory metadata = new uint256[](TOTAL_MAX_QTY + 1);

        for (uint256 i = 1; i <= TOTAL_MAX_QTY; i += 1) {
            metadata[i] = i;
        }

        for (uint256 i = 2; i <= TOTAL_MAX_QTY; i += 1) {
            uint256 j = (uint256(keccak256(abi.encode(seed, i))) %
                (TOTAL_MAX_QTY)) + 1;

            if (j >= 2 && j <= TOTAL_MAX_QTY) {
