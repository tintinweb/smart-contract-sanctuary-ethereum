...
contract Stablecoin is ERC20, ReentrancyGuard {
    ...
    constructor(
        ...
        require(ethPriceSourceAddress != address(0), "USDP:: ethPriceSourceAddress not zero");
        require(minimumCollateralPercentage != 0, "USDP:: minimumCollateralPercentage not zero");
        require(maximumDebtCeiling <= totalDebtCeiling, "USDP:: maximumDebtCeiling less than totalDebtCeiling");
        require(minimumDebtCeiling <= maximumDebtCeiling, "USDP:: minimumDebtCeiling less than maximumDebtCeiling");
    ...
    function calculateCollateralProperties(uint256 collateral, uint256 debt) private view returns (uint256, uint256) {
        require(getEthPriceSource() != 0, "USDP:: getEthPriceSource not zero");
        require(getTokenPriceSource() != 0, "USDP:: getTokenPriceSource not zero");
        ...
        require(debtValue >= debt, "USDP:: debtValue greater than debt");
    ...
    function createVault() external returns (uint256) {
        ...
        require(freeze[msg.sender][uint8(FreezeType.VAULT)] <= block.timestamp, "USDP:: freeze time is not ended");
    ...
    function depositCollateral(uint256 vaultID) external payable onlyVaultOwner(vaultID) {
        ...
        require(owner != address(0), "USDP:: owner not ");
        require(freeze[owner][uint8(FreezeType.COLLATERAL)] <= block.timestamp, "USDP:: freeze time is not ended");
    ...
    function borrowToken(uint256 vaultID, uint256 amount) external onlyVaultOwner(vaultID) {
        require(amount > 0, "USDP::Must borrow non-zero amount");
        require(amount >= minDebtCeiling, "USDP:: amount greater than min debt ceiling");
        require(totalSupply().add(amount) <= debtCeiling, "USDP: Cannot mint over totalSupply.");

        uint256 newDebt = vaultDebt[vaultID].add(amount);
        address owner = vaultOwner[vaultID];

        assert(newDebt > vaultDebt[vaultID]);
        require(newDebt <= maxVaultCeiling, "USDP:: newDebt less max vault ceiling");
        require(isValidCollateral(vaultCollateral[vaultID], newDebt), "USDP:: Borrow would put vault below minimum collateral percentage");
        require(freeze[owner][uint8(FreezeType.DEBT)] <= block.timestamp, "USDP:: freeze time is not ended");
    ...
    function payBackToken(uint256 vaultID, uint256 amount) external onlyVaultOwner(vaultID) {
        require(balanceOf(msg.sender) >= amount, "USDP::Token balance too low");
        require(vaultDebt[vaultID] >= amount, "USDP::Vault debt less than amount to pay back");
        require(freeze[vaultOwner[vaultID]][uint8(FreezeType.REPAY)] <= block.timestamp, "USDP:: freeze time is not ended");
...
contract USDP is Stablecoin, Ownable {
    ...
    function setMinDebtCeiling(uint minimumDebtCeiling) external onlyOwner() {
        require(minimumDebtCeiling < maxDebtCeiling, "USDP:: minimumDebtCeiling less than maxDebtCeiling");
        minDebtCeiling = minimumDebtCeiling;
    }

    function setMaxDebtCeiling(uint maximumDebtCeiling) external onlyOwner() {
        require(maximumDebtCeiling > minDebtCeiling, "USDP:: maximumDebtCeiling greater than minDebtCeiling");
        maxDebtCeiling = maximumDebtCeiling;
    }

    function setMaxVaultCeiling(uint maximumVaultCeiling) external onlyOwner() {
        require(maximumVaultCeiling <= 1e6, "USDP::maximumVaultCeiling less than 1000000");
