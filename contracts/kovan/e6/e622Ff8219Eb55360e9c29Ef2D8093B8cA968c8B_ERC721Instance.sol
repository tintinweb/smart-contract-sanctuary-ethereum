...
contract ERC721Instance is ERC721Enumerable, ERC721Burnable, AccessControl {
    ...
    mapping(uint256 => bool) claimIDUsed;
    ...
    function mint(uint256[] calldata mintID, address[] calldata account) external onlyRole(MINTER_ROLE) {
    ...
    function claim(uint256 claimID, uint256 tokenID, uint256 deadline, bytes calldata signature) external {
        ...
        require(factory.signer() == ECDSA.recover(ECDSA.toEthSignedMessageHash(keccak256(abi.encodePacked(claimID, _msgSender(), address(this), tokenID, deadline))), signature), "Invalid signature");
    ...
    function _baseURI() internal view override returns (string memory) {
        return string.concat(factory.baseURI(), Strings.toHexString(uint256(uint160(address(this))), 20), "/");
    }
