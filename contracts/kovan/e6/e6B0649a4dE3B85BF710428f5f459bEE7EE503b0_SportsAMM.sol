...
contract SportsAMM is Initializable, ProxyOwned, PausableUpgradeable, ProxyReentrancyGuard {
    ...
    function _buyPriceImpact(
        ...
        (uint balancePosition, uint balanceOtherSide, ) = _balanceOfPositionsOnMarket(market, position);        uint balancePositionAfter = balancePosition > amount ? balancePosition.sub(amount) : 0;
    ...
    function _balanceOfPositionsOnMarket(address market, Position position) internal view returns (uint, uint, uint) {
        (IPosition home, IPosition away, IPosition draw) = ISportPositionalMarket(market).getOptions();
        uint balance = position == Position.Home ? home.getBalanceOf(address(this)) : away.getBalanceOf(address(this));
        ...
        if (ISportPositionalMarket(market).optionsCount() == 3) {
            ...
            if (position == Position.Home) {
                ...
                // balanceOtherSideMax = awayBalance < drawBalance ? awayBalance : drawBalance;
            } else if (position == Position.Away) {
                balance = awayBalance;
