...
contract PrecogProfit is IPrecogProfit, ReentrancyGuard {
        ...
        nonReentrant
    {
        ...
        if(totalUnit > 0) {
            profitAmountForWhitelist = (profitAmount *
            ((100 * precogStorage.getTotalUnitsForWhitelistTradingCycle(token, currentProfitId)) /
                totalUnit)) / 100;
        }
    ...
    {
        if (_to != msg.sender) {
            IPrecogStorage.AccountProfitInfo memory _accountProfitInfoFrom = precogStorage.getAccountProfitInfo(
                _token,
                msg.sender
            );
            IPrecogStorage.AccountProfitInfo memory _accountProfitInfoTo = precogStorage.getAccountProfitInfo(
                _token,
                _to
            );
            ...
            IPrecogStorage.AccountProfitInfo memory _accountProfitInfo = precogStorage.getAccountProfitInfo(
                _token,
                msg.sender
            );
