...
contract StakingRewards is ReentrancyGuard, Pausable {
  ...
  /// @notice Address to pull the rewards for this contract and treasury for BAL Rewards.
  ...
  /// @notice Emission Per Sec initial value
  uint128 private constant EMISSION_PER_SEC = 0.01929 * 10e18; 
  ...
  /// @notice Operator address who can manage configurations of this contract.
  ...
  address public immutable balancerMinter;

  /// @notice incentive to users who spend gas to make earmark calls to harvest BAL rewards
  uint128 public earmarkIncentive = 100; // 100 means 1%

  /// @notice operation fee
  uint128 public operationFee = 1900; 
  ...
  // BPT Gauge: https://etherscan.io/address/0xbec2d02008dc64a6ad519471048cf3d3af5ca0c5#code
  address public DEPOSIT_TOKEN_ADDR; // can be private
  address public BAL_REWARD_POOL; // can be private

  /* ========== EVENTS ========== */
  event FeesUpdated(uint128, uint128);
  event EmissionPerSecondUpdated(uint128);  
  event CooldownSecondAndUnstakeWindowUpdated(uint128, uint128);  
  ...
  event EarmarkRewards(uint256 _balReward);  
  event RewardsVaultChanged(address _rewardsVault);
  ...
  constructor(
    ...
    _stakedToken.safeApprove(address(_balancerGauge), type(uint256).max);

    assetData.emissionPerSecond = EMISSION_PER_SEC;

    DepositToken dtoken = new DepositToken(address(this));
    DEPOSIT_TOKEN_ADDR = address(dtoken);
    BALRewardPool pool = new BALRewardPool(address(dtoken), address(_balToken), address(this)); 
    BAL_REWARD_POOL = address(pool);
  }

  function configEmissionPerSecond(uint128 _emissionPerSecond) external {
  ...
  function configCooldownSecondAndUnstakeWindow(uint128 _cooldownSeconds, uint128 _unstakeWindow) external {
    require(msg.sender == OPERATOR, 'ONLY_OPERATOR');
  ...
  function configFees(uint128 _earmarkIncentive, uint128 _operationFee) external {
    require(msg.sender == OPERATOR, 'ONLY_OPERATOR');
    require(_earmarkIncentive.add(_operationFee) <= earmarkIncentiveMax.add(operationFeeMax), ">MaxFees");
  ...
  function changeRewardsVault(address _rewardsVault) external {
    require(msg.sender == OPERATOR, 'ONLY_OPERATOR');
    REWARDS_VAULT = _rewardsVault;
    emit RewardsVaultChanged(_rewardsVault);
  }

  function changeOperator(address _to) external {
    require(msg.sender == OPERATOR, 'ONLY_OPERATOR');
    OPERATOR = _to;
  ...
  function stake(uint256 _amount) external {
    ...
    IERC20(STAKED_TOKEN).safeTransferFrom(msg.sender, address(this), _amount);
    emit Staked(msg.sender, _amount);

    /// stake deposit token to the Balancer Gauge.
    balancerGauge.deposit(_amount);
    emit Balancer_Gauge_Staked(msg.sender, _amount);

    /// stake to Bal Reward Pool
    IDepositToken(DEPOSIT_TOKEN_ADDR).mint(address(this), _amount);
    IERC20(DEPOSIT_TOKEN_ADDR).safeApprove(BAL_REWARD_POOL, _amount);
  ...
  function withdraw(uint256 _amount) public returns (bool) {
    ...
    // https://github.com/aurafinance/convex-platform/blob/cdcd6f33a64e6fb9f971c90ecce79ea71af02bbb/contracts/contracts/Booster.sol#L485
    ...
    // transfer staked token to the message sender
    IERC20(STAKED_TOKEN).safeTransfer(msg.sender, amountToWithdraw);
  ...
  // claim ALL: WNCG and BAL Rewards
  ...
  /**
    ...
    */
  function earmarkRewards() external whenNotPaused returns(bool){
    _earmarkRewards();
    return true;
  }
  
  /// https://github.com/aurafinance/convex-platform/blob/cdcd6f33a64e6fb9f971c90ecce79ea71af02bbb/contracts/contracts/Booster.sol#L634
  /**
    * @notice Basically a hugely pivotal function.
    *         Repsonsible for collecting the crv from gauge, and then redistributing to the correct place.
    *         Pays the caller a fee to process this.
  ...
  function _earmarkRewards() internal {    
    IMinter(balancerMinter).mint(address(balancerGauge));
    
    //require (balance check)
    uint256 balReward = IERC20(BAL).balanceOf(address(this));
    ...
    if (balReward > 0) {
      ...
      // send oepration fee to rewards vault
      IERC20(BAL).safeTransfer(REWARDS_VAULT, fee);

      // remove incentives from balance
      balReward = balReward.sub(_callIncentive);

      // send incentives to user
      IERC20(BAL).safeTransfer(msg.sender, _callIncentive);

      // send BAL to reward contract
      IERC20(BAL).safeTransfer(BAL_REWARD_POOL, balReward);
      IBALRewardPool(BAL_REWARD_POOL).queueNewRewards(balReward);
    }

    emit EarmarkRewards(balReward);
  }

  /* ========== VIEWS ========== */
  function earnedWNCG() external view returns (uint256) {
    return rewardsToClaim[msg.sender].add(getUnclaimedRewards(msg.sender));
  }

  function earnedBAL() external view returns (uint256) {
    return IBALRewardPool(BAL_REWARD_POOL).earned(msg.sender);
  }
  ...
  function stakedTokenBalance(address account) external view returns (uint256) {
  ...
  /* ========== FOR DEBUGGING ========== */
  function balTokenBalance() external view returns (uint256) {
    return IERC20(BAL).balanceOf(address(this));
  }

  function getCooldownEndTimestamp() external view returns (uint256) {    
    return cooldowns[msg.sender].add(COOLDOWN_SECONDS);
  }

  function getWithdrawEndTimestamp() external view returns (uint256) {    
    return cooldowns[msg.sender].add(COOLDOWN_SECONDS).add(UNSTAKE_WINDOW);
  }
...
interface IBalancerGauge is IERC20 {
    /**
     * @notice Deposit `_value` LP tokens. eg WNCG BPT.
     * @param _value Number of tokens to deposit
     */
    function deposit(uint256 _value) external;

    /**
     * @notice Withdraw `_value` LP tokens. eg WNCG BPT.
...
contract DepositToken is ERC20 {
    ...
    constructor(address _operator) ERC20("DepositToken", "depWNCGBPT") {
        operator =  _operator;
    }
...
contract BALRewardPool is ReentrancyGuard {
    ...
    function stakeFor(address _for, uint256 _amount) public nonReentrant returns (bool) {
    ...
    function withdrawFor(address _for, uint256 amount) public nonReentrant updateReward(_for) returns(bool) {
     ...
     */
    function getReward(address _account) public nonReentrant updateReward(_account) returns(bool){
