...
contract Product is IProduct, UInitializable, UControllerProvider, UReentrancyGuard {
    ...
    function settleAccountInternal(address account, IOracleProvider.OracleVersion memory currentOracleVersion) internal {
        ...
        // Get latest oracle version
        uint256 _latestVersion = latestVersion(account);
        if (_latestVersion == currentOracleVersion.version) return; // short circuit entirely if a == c
        ...
        // value a->b
        accumulated = accumulated.add(_accumulators[account].syncTo(_accumulator, _positions[account], settleOracleVersion.version).sum());

        // sync incentivizer before position update
        _controller.incentivizer().syncAccount(account);
        ...
        if (settleOracleVersion.version != currentOracleVersion.version) {
            ...
            // value b->c
            accumulated = accumulated.add(_accumulators[account].syncTo(_accumulator, _positions[account], currentOracleVersion.version).sum());
        }
