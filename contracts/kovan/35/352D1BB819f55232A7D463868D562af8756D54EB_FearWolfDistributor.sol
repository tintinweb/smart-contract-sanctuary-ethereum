...
contract FearWolfDistributor is AccessControl, Pausable, Withdrawable, ReentrancyGuard {
    ...
    */
    event DatesChanged(uint256 whitelistedPresaleStart, uint256 affiliatedPresaleStart, uint256 publicPresaleStart, uint256 saleStart, uint256 claimingStart);
    /**
    ...
    */
    ...
    /**
    * @dev Total numbers of wolves reserved during presale.
    */
    uint256 public totalWolvesReserved;
    /**
    * @dev Total numbers of wolves bought during sale.
    */
    uint256 public totalWolvesBought;

    // Timestamps
    uint256 public whitelistedPresaleStart;
    uint256 public affiliatedPresaleStart;
    ...
    */
    ...
    // address => wolves reserved on presale
    ...
    // address => true for affiliated
    mapping (address => bool) private affiliated;


    constructor() {
        whitelistedPresaleStart = DateTimeHelper.getDateOnly(block.timestamp) + 7 days;
        affiliatedPresaleStart = whitelistedPresaleStart + 2 days;
    ...
    function _buyWolves(uint256 wolvesCount, uint256 price) private {
    ...
    */
    ...
    /**
    * @dev Returns affiliated status for the caller.
    * @return True if the caller is affiliated, false - otherwise.
    */
    function getMyAffiliatedStatus() external view returns (bool) {
        return affiliated[_msgSender()];
    }
    ...
    */
    function getPublicSaleWolvesAvailable() external view returns (uint256) {
        if (block.timestamp < saleStart)
            ...
            return presaleWolvesCount + saleWolvesCount; // unsold presale wolves are added to wolves available for sale
    }
    ...
    * - Unless public presale is active, user must be whitelisted or affiliated to reserve a wolf;
    ...
    */
    function reserveWolves(uint256 wolvesCount) external payable nonReentrant {
        address caller = _msgSender();

        require(block.timestamp >= whitelistedPresaleStart, "Presale phase is not active yet");
        require(block.timestamp < saleStart, "Presale phase is over");
        require(block.timestamp >= affiliatedPresaleStart || whitelisted[caller], "Only whitelisted users can reserve a wolf now");
        require(block.timestamp >= publicPresaleStart || whitelisted[caller] || affiliated[caller], "Only whitelisted or affiliated users can reserve a wolf now");
    ...
    */
    function buyWolves(uint256 wolvesCount) external payable whenNotPaused nonReentrant {
        ...
        _buyWolves(wolvesCount, price);
    }


    function setDates(uint256 _whitelistedPresaleStart, uint256 _affiliatedPresaleStart, uint256 _publicPresaleStart, uint256 _saleStart, uint256 _claimingStart) external onlyRole(MANAGER_ROLE) {
        require(block.timestamp < whitelistedPresaleStart, "Too late, presale is already active");
        require(block.timestamp < _whitelistedPresaleStart 
                && block.timestamp < _affiliatedPresaleStart
                && block.timestamp < _publicPresaleStart
                && block.timestamp < _saleStart
                && block.timestamp < _claimingStart, "Cannot set timestamps to the past");
        require(_whitelistedPresaleStart < _affiliatedPresaleStart
                && _affiliatedPresaleStart < _publicPresaleStart
        ...
        affiliatedPresaleStart = _affiliatedPresaleStart;
        publicPresaleStart = _publicPresaleStart;
        saleStart = _saleStart;
        claimingStart = _claimingStart;
        emit DatesChanged(whitelistedPresaleStart, affiliatedPresaleStart, publicPresaleStart, saleStart, claimingStart);
    }
    ...
    function addAffiliatedAddress(address _address) external onlyRole(ADMIN_ROLE) {
        affiliated[_address] = true;
    }
    ...
    function addAffiliatedAddresses(address[] calldata _addresses) external onlyRole(ADMIN_ROLE) {
        for (uint i = 0; i < _addresses.length; i++)
            affiliated[_addresses[i]] = true;
    }
    ...
    function forceClaimWolves() external whenNotPaused nonReentrant onlyRole(ADMIN_ROLE) {
        require(block.timestamp >= saleStart, "Presale is still active");
        
        for(uint i = 0; i < fearWolvesReservers.length(); i++) {
            address user = fearWolvesReservers.at(i);
            uint256 wolvesCount = wolvesReserved[user];

            if (!claimed[user]) {
                claimed[user] = true;
                fearWolfContract.setInitialOwner(user, wolvesCount);
                emit WolvesClaimed(user, wolvesCount);
            }
        }
    }
}