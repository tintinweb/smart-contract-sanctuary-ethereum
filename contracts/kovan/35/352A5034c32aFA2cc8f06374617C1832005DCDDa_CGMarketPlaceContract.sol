...
contract CGMarketPlaceContract is IMarket, AccessControl {
    using SafeERC20 for IERC20;
    using Counters for Counters.Counter;
    Counters.Counter private _orderIds;
    Counters.Counter private _bidIds;

    address public _adminAddress = 0xaA73564c53a3a86eAa4871dF82772c88fB36af6a; // CG VAULT ADDRESS For Commission
    ...
    */
    function setAdminAddress(address _newAdminWallet) external override returns (bool) {
        require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "CG: Caller is not a admin");
        require(_newAdminWallet != address(0), "CG: Admin Wallet cannot be empty address");
        ...
        override
    {
        ...
        require(approvedNfts[_nftContract], "CG: NFT is not approved by admin");
        require(approvedCurrency[_currency], "CG: Currency is not approved by admin");
        require(_askAmount > 0, "CG: Ask Amount Cannot be Zero");
        require(_expiryTime > block.timestamp, "CG: Expiry Time cannot be in Past");
        ERC721 nftContract = ERC721(_nftContract);
        require(nftContract.ownerOf(_tokenId) == msg.sender, "CG: You are not owner of Token Id");
        ...
        require((isAllTokenApproved || approvedSpenderOfToken == address(this)), "CG: Market Contract is not allowed to manage this Token ID");
    ...
    function cancelOrder(uint256 _orderId)
        ...
        require(order.sender != address(0), "CG: Invalid Order Id");
        require(order.orderStatus == IUtils.OrderStatus.OPEN, "CG: Order status is not opened");
        bool hasAdminRole = hasRole(DEFAULT_ADMIN_ROLE, _msgSender());
        require(order.sender == _msgSender() || hasAdminRole, "CG: You Don't have right to cancel order");
    ...
    function completeOrder(uint256 _orderId)
        ...
        require(
            ...
            "CG: Please Approve Tokens Before You Buy"
        );
        ...
        override
    {
        ...
        require(approvedNfts[_nftContract], "CG: NFT is not approved by admin");
        require(approvedCurrency[_currency], "CG: Currency is not approved by admin");
        require(_bidAmount > 0, "CG: Bid Amount Cannot be Zero");
        require(_expiryTime > block.timestamp, "CG: Expiry Time cannot be in Past");
        ERC721 nftContract = ERC721(_nftContract);
    ...
    function cancelBid(uint256 _bidId)
        ...
        require(bid.sender != address(0), "CG: Invalid Bid Id");
        require(bid.bidStatus == IUtils.OrderStatus.OPEN, "CG: Bid status is not opened");
        bool hasAdminRole = hasRole(DEFAULT_ADMIN_ROLE, _msgSender());
        require(bid.sender == _msgSender() || hasAdminRole, "CG: You Don't have right to cancel order");
    ...
    function acceptBid(uint256 _bidId)
        ...
        require(bid.sender != address(0), "CG: Invalid Order Id");
        require(bid.bidStatus == IUtils.OrderStatus.OPEN, "CG: Bid status is not opened");
        IERC20 token = IERC20(bid.currency);
        ERC721 nft = ERC721(bid.nftContract);
        require(nft.ownerOf(bid.tokenId) == msg.sender, "CG: You are not owner of Token Id");

        require(
            token.allowance(bid.sender, address(this)) >= bid.bidAmount,
            "CG: Bidder Approve Tokens Are not working."
        );
