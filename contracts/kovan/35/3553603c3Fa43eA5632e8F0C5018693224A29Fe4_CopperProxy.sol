...
contract CopperProxy is Ownable {
    ...
    uint256 public immutable _platformAccessFee;
    address public _primaryFeeRecipient;
    address public _secondaryFeeRecipient;
    address public _tertiaryFeeRecipient;

    mapping(address => uint256) private _partnerShare;

    constructor(
        uint256 platformAccessFee,
        address primaryFeeRecipient,
        address secondaryFeeRecipient,
        address tertiaryFeeRecipient,
        address LBPFactoryAddress
    ) {
        _platformAccessFee = platformAccessFee;
        _LBPFactoryAddress = LBPFactoryAddress;
        _primaryFeeRecipient = primaryFeeRecipient;
        _secondaryFeeRecipient = secondaryFeeRecipient;
        _tertiaryFeeRecipient = tertiaryFeeRecipient;

        _partnerShare[_primaryFeeRecipient] = 5_000; // 50% default
        _partnerShare[_secondaryFeeRecipient] = 3_000; // 30% default
        _partnerShare[_tertiaryFeeRecipient] = 2_000; // 20% default
    }
    ...
    event ChangedFeeRecipient(address previousRecipient, address newRecipient, uint256 shareSize);

    event ChangedPartnerShare(address feeRecipient, uint256 newPercentage);
    ...
    function getShare(address onwerAddress) external view returns (uint256 share) {
        return _partnerShare[onwerAddress];
    }
    ...
    function exitPool(address pool, uint256[] calldata minAmountsOut, uint256 maxBPTTokenOut) external onlyPoolOwner(pool) {
        ...
        if (fundTokenBalance > uint256(0)) {
            uint256 feeAmount = (fundTokenBalance * _platformAccessFee) / 10_000;
            TransferHelper.safeTransfer(fundToken, _primaryFeeRecipient, feeAmount);
            emit TransferedFee(pool, fundToken, _primaryFeeRecipient, feeAmount);
        }
    ...
    function changePrimaryFeeRecipient(address newRecipient, uint shareSize) external onlyOwner {
        require(shareSize >= uint256(0), "shareSize must be grader or equal to 0");
        require(shareSize <= uint256(10_000), "shareSize must be less or equal to 10_000");

        address previousFeeRecipient = _primaryFeeRecipient;
        // remove old record from mapping
        delete _partnerShare[previousFeeRecipient];
        // add address and %share to mapping
        _partnerShare[newRecipient] = shareSize;
        // update _primaryFeeRecipient with new address
        _primaryFeeRecipient = newRecipient;

        emit ChangedFeeRecipient(previousFeeRecipient, newRecipient, shareSize);
    }

    function changeSecondaryFeeRecipient(address newRecipient, uint shareSize) external onlyOwner {
        require(shareSize >= uint256(0), "shareSize must be grader or equal to 0");
        require(shareSize <= uint256(10_000), "shareSize must be less or equal to 10_000");

        address previousFeeRecipient = _secondaryFeeRecipient;
        // remove old record from mapping
        delete _partnerShare[previousFeeRecipient];
        // add address and %share to mapping
        _partnerShare[newRecipient] = shareSize;
        // update _secondaryFeeRecipient with new address
        _secondaryFeeRecipient = newRecipient;

        emit ChangedFeeRecipient(previousFeeRecipient, newRecipient, shareSize);
    }

    function changeTertiaryFeeRecipient(address newRecipient, uint shareSize) external onlyOwner {
        require(shareSize >= uint256(0), "shareSize must be grader or equal to 0");
        require(shareSize <= uint256(10_000), "shareSize must be less or equal to 10_000");

        address previousFeeRecipient = _tertiaryFeeRecipient;
        // remove old record from mapping
        delete _partnerShare[previousFeeRecipient];
        // add address and %share to mapping
        _partnerShare[newRecipient] = shareSize;
        // update _tertiaryFeeRecipient with new address
        _tertiaryFeeRecipient = newRecipient;
