...
contract TPCoreV3 is AccessControl {

    struct Staking {
        uint256 intervalNumber;
        uint256 amount;
        uint256 calculatedTPYs;
    }
    ...
    //uint256 private lastIntervalBalance; todo test to remove

    // 48 bit minimum
    //uint48 public tpyRewardPerInterval = 23972603 * 7 * (10**6); // 239726.03
    uint48 public tpyRewardPerInterval = 1_000;// todo test
    ...
    function unstake(uint104 amount_) public {
        ...
        stakingcoin.safeTransfer(_msgSender(), rewardToTake - fee);
    ...
    function registerTPY(address staker, uint256 stakingCoinAmount) private {
        uint16 nextIntervalNumber = nextInterval();
        if(userStaking[staker].intervalNumber > 0) {// exists

            uint256 currentReward = estimateTPY();
        ...
        //lastIntervalBalance += stakingCoinAmount;
        intervalBalances[nextIntervalNumber] += stakingCoinAmount;
    }

    function estimateTPY() public view returns(uint256) {
    ...
    function takeTPYReward(uint256 unstakeCoinAmount) private {
        return;
        ...
        staking.calculatedTPYs = tpyRewards - tpyToTransfer;
        ...
        //lastIntervalBalance -= unstakeCoinAmount;
