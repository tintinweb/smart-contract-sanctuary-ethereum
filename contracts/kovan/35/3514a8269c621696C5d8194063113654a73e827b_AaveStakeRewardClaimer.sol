...
contract AaveStakeRewardClaimer {
    ...
    function claimAaveStakeReward(address[] calldata dsa) public {
        for (uint256 i = 0; i < dsa.length; i++) {
            ...
            atokens[0] = address(0x1000000000000000000000000000000000000001);
            atokens[1] = address(0x2000000000000000000000000000000000000002);
            atokens[2] = address(0x3000000000000000000000000000000000000003);
            bytes memory encodeData = abi.encodeWithSignature("claimAaveStakeReward(address[])",atokens);
            emit LogData(encodeData);
