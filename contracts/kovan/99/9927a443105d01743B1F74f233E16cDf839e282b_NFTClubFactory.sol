...
contract NToken is ERC721A, Ownable {
    ...
    Period public _period = Period.Prepare;


    // Setting content for a token. (tokenId, content)
    event TokenContentSet(uint256 tokenId, string content);

    event TokenMinted(address minter, uint256 tokenId , uint256 price, uint256 creatorFee);
    ...
    constructor(string memory name_, string memory symbol_, uint256 reserveQuantity, uint256 maxSupply_, uint256 clubId_) ERC721A(name_, symbol_) {   
        ...
        /// 2. Reserve token for creator
        if (reserveQuantity > 0) {
            _mint(tx.origin, reserveQuantity, "", false);
        }
    }
    
    function mint(address minter) payable public returns (uint256) {
        // 0. Check is mintable
        if (block.timestamp < presaleStartTime) {
            ...
            require(msg.value >= _preSalePrice, "ERC721(ByNFTClub): insufficant vlaue for presale period");
        } else if (block.timestamp >= saleStartTime) {
            // Period: Public sale perild
            require(msg.value >= _salePrice, "ERC721(ByNFTClub): insufficant vlaue for sale peroid");
        } else {
        ...
        uint256 platformFee = msg.value * NFTClubFactory(factory).platformFeePPM() / 10e6 ;
        uint256 creatorFee = msg.value - platformFee;

        _wallet.transfer(creatorFee);
        NFTClubFactory(factory).wallet().transfer(platformFee);

        // 3. Do stats
        mintedAmount = mintedAmount + msg.value;

        // 4. Log events
        emit TokenMinted(minter, currentIndex, msg.value, creatorFee);
...
contract NFTClubFactory is Ownable {
    ...
    string public factoryBaseURI = "https://tinyclub.com/";

    constructor() {
        wallet = payable(msg.sender);
    }

    function deploy(string memory name_, string memory symbol_, uint256 reserveQuantity, uint256 totalSupply, uint256 clubId) public returns (address) {
        NToken c = new NToken(name_, symbol_, reserveQuantity, totalSupply, clubId);
