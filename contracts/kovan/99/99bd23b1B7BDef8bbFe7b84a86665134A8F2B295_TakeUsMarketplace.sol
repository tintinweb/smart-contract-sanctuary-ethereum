...
contract TakeUsMarketplace is Ownable {
  ...
  //nftAddress => tokenId
  mapping(IERC721 => mapping(uint256 => Lending)) public listing;
  ...
  event Canceled(IERC721 nftAddress, uint256 tokenId);
  event Borrowed(
    IERC721 nftAddress,
    uint256 tokenId,
    uint256 price,
  ...
  modifier onlyLender(IERC721 _nftAddress, uint256 _tokenId) {
    // require(
  ...
  modifier eligibleListing(IERC721 _nftAddress, uint256 _tokenId) {
    // Check that require. May be needed to write a separe function that can process throw
    // require(
    //   listing[_nftAddress][_tokenId].lender != address(0),
  ...
  function borrow(
    ...
    IERC721 _nftAddress,
    uint256 _tokenId
  ) public eligibleListing(_nftAddress, _tokenId) {
    // require(
    ...
    // if (borrowerFee > 0) {
    //   uint256 fee = (listing[_nftAddress][_tokenId].price * borrowerFee) / 100;
    //   transfer(
    //     msg.sender,
    //     addressRegistry.royaltyCollector(),
    //     fee,
    //     listing[_nftAddress][_tokenId].paymentToken
    //   );
    // }

    // transfer(
    //   msg.sender,
    //   listing[_nftAddress][_tokenId].lender,
    //   listing[_nftAddress][_tokenId].price,
    //   listing[_nftAddress][_tokenId].paymentToken
    // );
    // vaultManager.setLending(
    //   _borrower,
    //   listing[_nftAddress][_tokenId].duration,
    //   listing[_nftAddress][_tokenId].lender,
    //   _nftAddress,
    //   _tokenId
    // );
  ...
  function list(
  ...
  ) public onlyLender(_nftAddress, _tokenId) {
    // require(
    //   IERC721(_nftAddress).getApproved(_tokenId) == address(this),
    //   "Marketplace is missing approve"
    // );
    // if (lenderFee > 0) {
    //   uint256 fee = (listing[_nftAddress][_tokenId].price * lenderFee) / 100;
    //   transfer(
    //     msg.sender,
    //     addressRegistry.royaltyCollector(),
    //     fee,
    ...
    listing[_nftAddress][_tokenId] = Lending({
      duration: _duration,
      price: _price,
      lender: msg.sender,
  ...
  function update(
    ...
    public
    onlyLender(_nftAddress, _tokenId)
    eligibleListing(_nftAddress, _tokenId)
  {
    listing[_nftAddress][_tokenId] = Lending({
      duration: _duration,
      price: _price,
    ...
    emit Updated(_nftAddress, _tokenId, _price, _duration);
  }

  function cancel(IERC721 _nftAddress, uint256 _tokenId)
    public
    onlyLender(_nftAddress, _tokenId)
  {
    // require(
