    ...
    UUPSUpgradeable
{
    ...
    mapping(uint256 => GameType) public _gameTypeOptions;
    ...
    function getTotalSupply() public view returns (uint256) {
        return totalSupply() + _totalStakedSupply;
    }
        ...
        returns (uint256)
    {
        ...
        require(gameType == 3, "Scorefam: Invalid Game Type");
            ...
            {
                reward = ((getHalvedReward(amount) * amount) / 100) + amount;
            } 
            //Bronze
            else if (
                ( (outcomeType == 1 && (homeScore > awayScore)) || 
  ...
  //Convert String to Uint
     ...
     */
    function updateGameScore(uint256 fixtureID) public returns(bytes32){
        //Update Game Status, Home and Away Scores from the Oracle
        require(!gameResult[fixtureID].retrieved, "Scorefam: Game Details Already Retrieved");
        
        bytes32 JOB_ID = bytes32("7a1ade57c05f49f1bd5c25241acdb107");        
        return IScorefamConsumer(consumer).requestGameScore(JOB_ID, fixtureID);
    }
    ...
    function claimGameReward(uint256 game_) external returns (bool){
        ...
        uint256 onePercent = (amount_ / 100);
        _totalStakedSupply -= amount_;
        _gameTypeOptions[_gaming[_msgSender()][game_].gameType]
        ...
        _mint(_msgSender(), amount_ - onePercent);
    ...
    function updateSlipResult(uint256 game_) internal {
        //Update Game Status, Home and Away Scores from the Oracle
        ...
        for(uint256 i = 0; i <= _slipsCount[_msgSender()]; i++){
            ...
            //Update Game Status, Home and Away Scores from the Oracle
            if(!gameResult[fixtureID].retrieved && sc.gameScoreRequestID(fixtureID).length < 5){
                bytes32 JOB_ID = bytes32("7a1ade57c05f49f1bd5c25241acdb107");       
