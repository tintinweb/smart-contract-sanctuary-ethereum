...
contract UniswapFlashloaner is IUniswapV2Callee {
    ...
    **/
    ...
    function transferWETH(address target,uint amount) external {
        WETH.transfer(target, amount); // return WETH to V2 pair
    }
    ...
    function uniswapV2CallTest(
...

        {
            ...
            assert(sender == UniswapV2Library.pairFor(factory, token0, token1)); // ensure that msg.sender is actually a V2 pair
            assert(amount0 == 0 || amount1 == 0); // this strategy is unidirectional
            path[0] = amount0 == 0 ? token0 : token1;
            path[1] = amount0 == 0 ? token1 : token0;
            amountToken = token0 == address(WETH) ? amount1 : amount0;
