...
contract placePixel is ERC721URIStorage {
    ...
    event lockRequest(bytes32 questionID, bytes32 packedCoordinates);
    ...
    function lock(bytes32 packedCoordinates) external payable returns(bytes32 quesitonID){
        ...
        emit lockRequest(questionID, packedCoordinates);
        return questionID;
    }
    ...
    function executeRuling(bytes32 questionID, bytes32 packedCoordinates, address requester, uint256 timestamp) external{
        ...
        bytes32 question_id_calculated = keccak256(abi.encodePacked(content_hash, arbitrator, challengePeriod, uint256(0), reality, requester, timestamp));
    ...
    function burn(bytes32 questionID, bytes32 packedCoordinates, uint256 timestamp) external{
        ...
        // templateID = 54 for mainnet, 20 for kovan
        bytes32 content_hash = keccak256(abi.encodePacked(uint256(20), uint256(0), question));
        bytes32 question_id_calculated = keccak256(abi.encodePacked(content_hash, arbitrator, challengePeriod, uint256(0), reality, msg.sender, timestamp));
        if(questionID != question_id_calculated) { revert AuthenticationFailure();}
