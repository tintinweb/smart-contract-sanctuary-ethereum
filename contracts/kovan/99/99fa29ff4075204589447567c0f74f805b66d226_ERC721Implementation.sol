...
contract ERC721Implementation is ERC721Upgradeable {
    function mint (
        address to,
        uint256 tokenId
...
contract FactoryClone {
    ...
    constructor() {
        tokenImplementation = address(new ERC721Implementation());
    }

    function createToken(string calldata name, string calldata symbol) external returns (address) {
        address clone = Clones.clone(tokenImplementation);
        ERC721Implementation(clone).initialize(name, symbol);
        return clone;
    }
}
