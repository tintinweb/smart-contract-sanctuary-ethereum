    ...
    Ownable
{
    ...
    function _processSplit(CellData.Cell memory cell) private {
        if (CellData.Class(cell.class) == CellData.Class.SPLITTABLE_NANO) {
            uint tokenId = INanoCellCreator(nanoCell).create(msg.sender);
            ...
            IMintable(madToken).mint(
                msg.sender,
                amount * 10**IERC20Metadata(madToken).decimals()
            );
        ...
        override
    {
        ...
        emit AddNFT(_owner, _metadataUri, block.timestamp);
    }
    ...
    function setMarketPlace(address newMarketPlace) external override onlyOwner {
