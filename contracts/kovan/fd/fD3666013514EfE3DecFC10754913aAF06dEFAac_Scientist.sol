    ...
    ReentrancyGuardUpgradeable
{
    ...
    // Events
    event SetGiftMerkleRootAnPrice(bytes32 merkleTree, uint256 price, uint256 timestamp);
    event MintForGift(address caller, address to, uint256 tokenId, uint256 timestamp);
    ...
    modifier isValidMerkleProof(bytes32[] calldata merkleProof, bytes32 root) {
        ...
        require(
            !isClaimed[msg.sender] && !isClaimedByLeaf[leaf],
    ...
    function getProxyRegistryAddress() public virtual override view returns (address) {
        return proxyRegistryAddress;
    }

    function setProxyRegistryAddress(address newProxyRegistryAddress) external onlyTimelock {
        ...
        onlyTimelock
    {
        ...
        price = newPrice;
        emit SetGiftMerkleRootAnPrice(merkleRoot, newPrice, block.timestamp);
    }

    function getLeaf(address account)
        external
        pure
        returns (bytes32)
    {
