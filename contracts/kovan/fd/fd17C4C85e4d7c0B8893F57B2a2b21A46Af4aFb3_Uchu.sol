...
contract Uchu is Initializable {
    ...
    */
    ...
    mapping (address => uint8) private position_size;
    mapping (address => uint8) private loan_size;
    ...
    **/
    ...
    function updatePosition(address user, uint8 productId, uint deposit, uint withdraw) private {
        ...
        if (pos.position_id == 0) {
            position_map[user][productId] = Position(++position_size[user], deposit);
        } else {
            if (product.debt == 0) {
    ...
    function updateLoan(address user, uint8 productId, uint borrow, uint repay) private {
        ...
        if (loan.loan_id == 0) {
            loan_map[user][productId] = Loan(++loan_size[user], borrow);
        } else {
            if (product.debt == 0) {
