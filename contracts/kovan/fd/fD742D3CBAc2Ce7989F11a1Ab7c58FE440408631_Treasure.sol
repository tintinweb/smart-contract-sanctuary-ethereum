...
*/
    ...
    bytes32 secretHash = 0x56c71d4bea07c5a7b7b2294a3103ca63ff403898238bad1d800c13e203be3952;
    ...
    mapping(uint256 => uint256) internal crewIdToCaptainId;
    ...
    function setCapCode(bytes32 newSecret) public onlyOwner {
        ...
        returns (uint256)
    {
        ...
        // TODO randomly determine the winner from the battleSum?
        uint256 winner = battle.firstCaptain;

        if (battleSum % 2 == 0) winner = battle.secondCaptain;

        // return the captain id of the winner
    ...
    function stakeCaptain(uint256 captainId, bool isHunt) internal {
    ...
    function stakeCrew(uint256 crewId, uint256 captainId) internal {
        ...
        crewIdToCaptainId[crewId] = captainId;
    ...
    function unstakeCaptain(uint256 captainId) internal returns (uint256) {

        uint256 rewardTime = block.timestamp - captainIdToTimeStamp[captainId];
        ...
        captainIdToStaker[captainId] = nullAddress;
        captainIdToBattleId[captainId] = 0;

        return captainReward;


    }


    function unstakeCrew(uint256 crewId) internal returns (uint256) {
        ...
        uint256 stakeTime = block.timestamp - crewIdToTimeStamp[crewId];
    ...
    function huntUnstake(uint256 captainId) internal returns (uint256) {
        ...
        uint256 huntPeriodSeconds = 50;


        require(
            block.timestamp > (huntTime + huntPeriodSeconds),
            "Captain must have completed full treasure hunt period"
        );
        ...
        */
        ...
        return unstakeCaptain(captainId);
    }
    ...
    function unstakeBattleByIds(uint256 captainId, uint256[] memory crewIds) public {
        ...
        require (captainIdToBattleId[captainId] != 0, "Sike this captain token is not in a battle");
        ...
        uint256 battlePeriodSeconds = 50;


        require(
            block.timestamp > (bt + battlePeriodSeconds),
            "Captain must have completed full battle period"
        );


        uint256 battleWinner = determineBattleOutcome(captainId);
        

        unstakeCaptain(captainId);


        // unstakes each crew member

        for (uint256 i = 0; i < crewIds.length; i++) {
            uint256 crewId = crewIds[i];

            require (crewIdToCaptainId[crewId] == captainId, "Sike this crew member was staked with a different captain");
            

            uint256 crewReward = unstakeCrew(crewId);
            totalRewards += crewReward;
        }


        // if this captain won then they get a crew member from the captain they battled
    ...
    function unstakeCrewsByIds(uint256 captainId, uint256[] memory crewIds) public {
        ...
        //uint256 totalRewards = unstakeCaptain(captainId);

        uint256 totalRewards = huntUnstake(captainId);
    ...
    function unstakeCaptainsByIds(uint256[] memory captainIds) public {
        ...
        for (uint256 i = 0; i < captainIds.length; i++) {
            ...
            //uint256 captainReward = unstakeCaptain(captainId);
            uint256 captainReward = huntUnstake(captainId);
    ...
    function claimByCaptainId(uint256 captainId, string calldata secret) public {
        require(
            captainIdToStaker[captainId] == msg.sender,
            "Captain is not claimable by you!"
        );

        require (captainIdToBattleId[captainId] == 0, "Treasure for captain cannot be claimed during battle");
        

        uint256 codeMult = 1;

        require(keccak256(abi.encodePacked(secret, "")) == secretHash, "sike missed secret code");
        if (keccak256(abi.encodePacked(secret, "")) == secretHash) codeMult = 2;
        ...
        */
        ...
        uint256 totalRewards = rewardDays * pay_rate * codeMult;
