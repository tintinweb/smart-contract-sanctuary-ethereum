...
error PandaNft__NotPendingState();
...
contract PandaNft is ERC721, Ownable {
    ...
    // guo mint event
    event GuoMint(address recipient);

    // presale mint event
    event PresaleMint(address recipient);

    // public sale mint event
    event Mint(address recipient);
    ...
    // guo mint count
    uint256 public immutable guoMintCount;
    ...
    constructor(
        uint256 _maxSupply,
        uint256 _guoMinCount,
        uint256 _maxPerAccount,
        uint256 _maxFreeCount,
        uint256 _price,
        string memory _uri
    ) ERC721("LedgerMobile", "LM") {
        maxSupply = _maxSupply;
        guoMintCount = _guoMinCount;
    ...
    // guo mint
    function guoMint(address recipient) public onlyOwner { 
        if (state != State.Pending) {
            revert PandaNft__NotPendingState();
        }

        myMint(recipient, guoMintCount);

        emit GuoMint(recipient);
    }

    // my mint
    function myMint(address recipient, uint256 count) internal {
