...
contract CreepFinance is ReentrancyGuard, RrpRequesterV0 {
    ...
    address public teamWallet = 0xEaC458B2F78b8cb37c9471A9A0723b4Aa6b4c62D;
    address public treasuryWallet = 0xEaC458B2F78b8cb37c9471A9A0723b4Aa6b4c62D;
        ...
        returns (address[] memory)
    {
        ...
        for (uint256 i = 0; i < half; i++) {
            ...
            uint256 swap = (uint256(
                keccak256(
                    abi.encodePacked(
                        randoms[round + pools[i].createdIndex - 1].value,
                        i,
                        players
                    )
                )
    ...
    function winners(uint256 index, uint256 round)
        ...
        returns (address[] memory)
    {
        address[] memory players = getWinners(index, round);
        uint256 half = (players.length - (players.length % 2)) / 2;
