...
contract RockGuyContract is Ownable, ERC721A, ReentrancyGuard {
 ...
 constructor(
  ...
  function Mint(uint256 mintamount, bytes32[] memory proof) external payable {
    uint256 cost = saleprice*mintamount;
    bool bFree = (MintPerWallet[msg.sender] + mintamount <= TotalFreeTokenPerwallet);
       
    if (bFree) {
        cost = saleprice*(mintamount-TotalFreeTokenPerwallet);
    }
