...
contract CryptoTests is ERC721, Ownable {
    ...
    constructor(string memory baseURI, string memory baseContract) ERC721("CryptoTests","CT") {
        setBaseURI(baseURI);
    ...
    function MintTest(uint256 quantity, bytes32 hash, bytes memory signature, address secretWallet) public payable {
        mintTest(quantity, msg.sender, hash, signature, secretWallet);
    }
    
    function mintTest(uint256 quantity, address receiver, bytes32 hash, bytes memory signature, address secretWallet) public payable {
        require(secretWallet == verify(hash, signature), "Not authorized");
