...
contract WallNutsGrooveERC721A is Ownable, ReentrancyGuard, ERC721A, PaymentSplitter {
    ...
    uint private constant MAX_PUBLIC = 4000;
    uint private constant MAX_RESERVED = 40;

    // Sale price presets
    uint public whitelistSalePrice = 0.0008 ether;
    uint public publicSalePrice = 0.001 ether;

    // June 23th 2022, 14:00 GMT
    uint public saleStartTime = 1655985600;
    ...
    // Payment splitter params
    uint private teamLength;

    address[] private _team = [
        0x70997970C51812dc3A010C7d01b50e0d17dc79C8, //  JS
        0x3C44CdDdB6a900fa2b585dd299e03d12FA4293BC, //  NW
        0x90F79bf6EB2c4f870365E785982E1f101E93b906, //  SM
        0x15d34AAf54267DB7D7c367839AAf71A00a2C6A65, //  MG
        0x9965507D1a55bcC2695C58ba16FB37d819B0A4dc  //  Treasure
    ];
    ...
    // Reserved minted boolean
    bool public isReservedMinted;

    //Constructor
    constructor(bytes32 _merkleRoot, string memory _baseURI)

    ERC721A("Wall Nuts Groove", "WNG")
    ...
    **/
    function whitelistMint(address _account, uint _quantity, bytes32[] calldata _proof) external payable {
        require(!isPaused, "The sale is paused.");

        require(currentTime() >= saleStartTime, "Sale has not started yet!");
        require(currentTime() < saleStartTime + 12 hours, "Whitelist sale is finished.");

        uint price = whitelistSalePrice;
        require(price != 0, "Price is not defined.");

        require(currentStep == Step.WhitelistSale, "Whitelist sale is not activated.");

        require(msg.sender != address(0x0), "Public address is not correct");
        require(isWhitelisted(msg.sender, _proof), "Your account is not whitelisted.");
        require(amountNFTperWalletWhitelistSale[msg.sender] + _quantity <= maxPerAddressDuringWhitelistMint, "This address has already claimed 1 NFT.");
        require(totalSupply() + _quantity <= MAX_WHITELIST, "Max supply for whiteList sale reached.");
    ...
    **/
    function publicMint(address _account, uint _quantity) external payable {
        ...
        require(currentStep == Step.PublicSale, "Public sale is not activated.");

        require(msg.sender != address(0x0), "Public address is not correct");
        require(amountNFTperWalletPublicSale[msg.sender] + _quantity <= maxPerAddressDuringPublicMint, "You cannot mint more than 10 NFTs per account.");
    ...
    /**
    * @notice Allows the owner to mint team reserved NFTs
    **/
    function reservedMint() external onlyOwner {
        require(!isReservedMinted, "Reserved NFTs already minted.");
        for(uint i = 0 ; i < teamLength-1 ; i++) {
            _safeMint(payee(i), 10);
        }
        isReservedMinted = true;
    }

    /**
    * @notice Allows the owner to mint unsold NFTs (Gift or Burn)
    *
    * @param _to The address of the receiver
    * @param _quantity Amount of NFTs the owner wants to mint
    **/
    ...
    *
    * @param _whitelistSalePrice The new price of one NFT during the whitelist sale
    */
    function setWhitelistSalePrice(uint _whitelistSalePrice) external onlyOwner {
    ...
    */
    ...
    /**
    * @notice Change the step of the contract
    *
    * @param _step The new step of the contract
    */
    ...
    */
    ...
    /**
    * @notice Revert renounceOwnership function
    */
    function renounceOwnership() public view override onlyOwner {
        revert("This function is disabled."); //Not possible with this smart contract
    }
