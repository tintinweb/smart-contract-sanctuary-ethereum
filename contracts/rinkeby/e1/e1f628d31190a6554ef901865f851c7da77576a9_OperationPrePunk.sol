...
contract OperationPrePunk is Ownable, ERC721A {
    ...
    //Enums
    enum SaleStatus { CLOSED, WHITELIST, PUBLIC }
    ...
    uint256 constant MAX_PUBLIC = 972;
    uint256 publicMinted = 0;

    uint256 private amountToDAO = 900;
    uint256 daoMinted = 0;
    ...
    mapping(address => mapping(uint256 => bool)) private usedWhitelist;
    ...
    constructor() ERC721A("Operation Pre-Punk", "OPP") {
        claimPeriodEnd = block.timestamp + 45 days;
    }
    ...
    function withinClaimPeriod() internal view returns (bool) {
    ...
    /// @notice Public mint for an avatar
    /// @param shapes shapes of the avatar
    /// @param colorsPrimary primary colors of the avatar
    /// @param colorsSecondary secondary colors of the avatar
    /// @param positions positions of the shapes (Not used for this collection)
    function publicMint(
        ...
        require(saleStatus == SaleStatus.PUBLIC, "Public closed");
        require(!addressMinted[msg.sender], "Wallet already minted");
        require(withinClaimPeriod(), "Claim period over");
        ...
        require(publicMinted <= MAX_PUBLIC, "Public is minted out");

        _mintAvatar(shapes, colorsPrimary, colorsSecondary, positions);
    }

    /// @notice Whitelist mint for an avatar
    /// @param shapes shapes of the avatar
    /// @param colorsPrimary primary colors of the avatar
    /// @param colorsSecondary secondary colors of the avatar
    /// @param positions positions of the shapes (Not used for this collection)
    /// @param tokenId tokenId of the illuminati NFT used for whitelist
    ...
    function whitelistMint(
        ...
        require(saleStatus != SaleStatus.CLOSED, "Whitelist closed");
        ...
        require(!usedWhitelist[requiredNFTAddress][tokenId], "Already used token");
        require(withinClaimPeriod(), "Claim period over");
        
        usedWhitelist[requiredNFTAddress][tokenId] = true;
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        (shapes, colorsPrimary, colorsSecondary, positions) = avatars.get(avatarHash);

        return string(abi.encodePacked(URI, shapes, "-", colorsPrimary, "-", colorsSecondary, "-", positions));
    }
    ...
    function giveLeftoverMintsToDAO() public onlyOwner {
        require(!withinClaimPeriod(), "Claim period not over");
        uint256 leftover = MAX_SUPPLY - amountMinted;

        amountToDAO = leftover;
    }

    function daoMintMany(
        string[] calldata shapes,
        string[] calldata colorsPrimary,
        string[] calldata colorsSecondary,
        string[] calldata positions,
        ...
        for(uint i = 0; i < shapes.length; i++)
            _daoMint(shapes[i], colorsPrimary[i], colorsSecondary[i], positions[i], signature[i]);
    }

    /// @notice Dev mint for the DAO
    /// @param shapes shapes of the avatar
    /// @param colorsPrimary primary colors of the avatar
    /// @param colorsSecondary secondary colors of the avatar
    /// @param positions positions of the shapes (Not used for this collection)
