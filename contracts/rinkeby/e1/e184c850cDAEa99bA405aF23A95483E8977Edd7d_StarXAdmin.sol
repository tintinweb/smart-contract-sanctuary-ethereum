...
contract StarXAdmin is OwnableUpgradeable, StarXManaged {
    //---- member variables - start ----
    uint256 m_celebCount;
    mapping(address => uint256) m_celebToIndex;
    mapping(uint256 => address) m_celebToIndexRev;
    //---- member variables - finish ----
    ...
    //---- modifiers - start ----
    modifier modCelebContractExists(address _celebContract) {
        require(
            m_celebToIndex[_celebContract] > 0,
            "DNE: celebrity contract address"
        );
        _;
    }
    ...
    function getCelebCount() external view returns (uint256) {
        return m_celebCount;
    }
    ...
    function deployNewCelebContract(
    ...
    ) external starXOnly returns (address, uint256) {
        // check: valid celeb wallet
        require(_celebWallet != address(0), "invalid arg: _celebWallet");
        // check: valid address for StarXAddressRegistry
        require(
            _starXAddressRegistry != address(0),
            "invalid arg: _starXAddressRegistry"
        );
        ...
        // effect: update mappings for new contract
        m_celebCount++;
        m_celebToIndex[address(celeb)] = m_celebCount;
        m_celebToIndexRev[m_celebCount] = address(celeb);
        ...
        // return
        return (address(celeb), m_celebCount);
    }

    // TODO: callable by starx only or starx admin
    // TODO: fn doc
    function deployNewAssetContractForCelebContract(
        ...
        public
        starXOnly
        modCelebContractExists(_celebrityContractAddress)
        returns (address newAssetContractAddress)
    {
        // check: valid custody wallet
        require(
            _req.custodyWallet != address(0),
            "invalid address for arg: _starXCustodyWalletForAsset"
        );
        // effect: deploy new asset contract for specified celebrity contract
        newAssetContractAddress = IStarXCelebrity(_celebrityContractAddress)
    ...
    function mintNewSecurityForCelebAndAssetContract(
        ...
        public
        starXOnly
        modCelebContractExists(_celebContract)
    ...
    ) external starXOnly modCelebContractExists(_celebContract) {
        // effect: DEPLOY and obtain new asset contract address
        address newAssetContractAddress = deployNewAssetContractForCelebContract(
                _celebContract,
                _deployReq
            );
        // effect: MINT
...
contract StarXCelebrity is IStarXCelebrity {
    ...
    modifier modOnlyAdmin() {
        // halt function execution if neither of those conditions are true
        require(
            IStarXAddressRegistry(m_starXAddressRegistry).adminNftContracts(
                _msgSender()
            ),
            "[StarXCelebrity] caller must be registered StarX admin"
        );
        _;
    }
    ...
    modifier modAssetContractExists(address _assetContract) {
        require(
            ...
            "[StarXCelebrity] DNE: asset contract address"
        );
        _;
    }
    ...
    // TODO: fn doc
    // TODO: dev doc - can only initialized by StarXAdmin
    // -> its presence must be known by StarXAdmin (and its maps) and this is how we enforce that
        ...
        modOnlyAdmin
    {
        require(
            ...
            "[StarXCelebrity] updateStarXAddressRegistry - invalid arg = zero-address"
        );
        // emit event
    ...
    function deployFinancialSecurityAssetContract(
        IRequest.DeployAssetContractRequest calldata _req
    ) external virtual override modOnlyAdmin returns (address) {
        // check: valid custody wallet
        require(
            _req.custodyWallet != address(0),
            "[StarXCelebrity] deploy asset - invalid arg: _req.custodyWallet"
        );
        // init new contract
        ...
        // add newly-deployed contract to local lookup
        m_assetLookup[m_deployedAssetCount] = address(security);
        m_assetLookupRev[address(security)] = m_deployedAssetCount;
        ...
        returns (uint256 newTokenId)
    {
        ...
        require(
            ...
            "[StarXCelebrity] mintNewSecurity - invalid arg for: _securityContract"
        );
        ...
        modAssetContractExists(_assetContract)
    {
        ...
        require(
            m_assetLookupRev[_assetContract] > 0,
            "[StarXCelebrity] updateCustodyWalletForAsset - invalid arg for: _assetContract"
        );
...
contract StarXFinancialSecurityAsset is ERC1155Burnable {
    // basic attributes which define a FinancialSecurityAsset
    // -> defined per unique token id
    struct AssetInfo {
        bool isListed;
        address creator;
    ...
    // TODO: dev doc - mention that this is immutable
    ...
    modifier modTokenIdExists(uint256 _tokenId) {
        require(
            ...
            "[StarXFinancialSecurityAsset] DNE: token id"
        );
        _;
    }
    ...
    // TODO: should only be celeb?
    modifier modOnlyApproved() {
        // caller = deployer?
        bool isDeployerOrAdmin = _msgSender() == m_deployingStarXCelebrity;

        // caller = registered StarXAdmin?
        if (!isDeployerOrAdmin) {
            address starXAddressRegistry = IStarXCelebrity(
                m_deployingStarXCelebrity
            ).getStarXAddressRegistry();
            isDeployerOrAdmin = IStarXAddressRegistry(starXAddressRegistry)
                .adminNftContracts(_msgSender());
        }
        ...
        require(
            isDeployerOrAdmin,
            "caller must be either contract deployer/owner or StarXAdmin"
        );
        _;
    }
    ...
    constructor(IRequest.DeployAssetContractRequest memory _req) ERC1155("") {
        require(
            ...
            "contract deployer must be of contract type: StarXCelebrity"
        );
        ...
        modOnlyApproved
    {
        require(
            ...
            "invalid address arg for new custody wallet"
        );
