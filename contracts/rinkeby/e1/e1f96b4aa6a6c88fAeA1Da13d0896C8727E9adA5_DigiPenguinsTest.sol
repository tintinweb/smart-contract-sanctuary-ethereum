	...
	ERC721PausableUpgradeable
{
	...
	mapping(uint256 => uint256) private freeMintedAmountPerDay;
	...
	uint256 public mutationTokenIdTracker;

	// Reserved mint
	uint256 constant public reservedMintMutationPrice = 10 ether / 100;				// 0.1 ETH

    // event Invitation(address indexed referrer, address indexed minter, uint256 indexed tokenId);
	...
	function publicMint(uint16[] calldata evolutionTokenIds, uint8 mutationTokenAmount) external payable {
		...
		if (mutationTokenAmount > 0) {
			for (uint8 i = 0; i < evolutionTokenIds.length; i++){
				...
				_mint(_msgSender(), mutationTokenIdTracker);
				mutationTokenIdTracker += 1;
