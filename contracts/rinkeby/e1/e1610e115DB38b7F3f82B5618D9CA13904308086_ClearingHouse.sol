...
contract ClearingHouse is IClearingHouse, IncreOwnable, Pausable, ReentrancyGuard {
    ...
    function liquidate(
        ...
        uint256 positiveTraderOpenNotional = _getTraderPosition(idx, liquidatee).openNotional.abs().toUint256();

        uint256 positiveLpOpenNotional = _getLpPosition(idx, liquidatee).openNotional.abs().toUint256(); // TODO: replace by nathan getter
        uint256 positiveActiveLpOpenNotional = _getLpPositionAfterWithdrawal(idx, liquidatee)
            .openNotional
            .abs()
            .toUint256();

        if (positiveTraderOpenNotional + positiveLpOpenNotional == 0) revert ClearingHouse_LiquidateInvalidPosition();
        ...
        int256 pnl = positiveTraderOpenNotional != 0 ? _liquidateTrader(idx, liquidatee, proposedAmount) : int256(0);
        pnl += positiveLpOpenNotional != 0 ? _liquidateLp(idx, liquidatee, proposedAmount) : int256(0);

        // take fee from liquidatee for liquidator
        uint256 liquidationRewardAmount = (positiveActiveLpOpenNotional + positiveTraderOpenNotional).wadMul(
            liquidationReward
        );
        ...
        emit LiquidationCall(idx, liquidatee, liquidator, positiveActiveLpOpenNotional + positiveTraderOpenNotional);
    }
    ...
    function provideLiquidity(
        ...
        uint256 amountUSDValue = vault.deposit(msg.sender, amount, token).toUint256();
    ...
    function _marginRatio(address account) internal view returns (int256) {
        ...
        for (uint256 i = 0; i < perpLength; i++) {
            (int256 traderUnrealizedPositionPnl, int256 traderOpenNotional) = perpetuals[i].getTraderPositionHealth(
                account
            );

            (int256 lpUnrealizedPositionPnl, int256 lpOpenNotional) = perpetuals[i].getLpPositionHealth(account);

            openNotional += traderOpenNotional + lpOpenNotional;
        ...
        int256 collateral = _getReserveValue(account);

        return _computeMarginRatio(collateral, unrealizedPositionPnl, openNotional);
    }

    function _getReserveValue(address account) internal view returns (int256) {
        return vault.getReserveValue(account);
    }
