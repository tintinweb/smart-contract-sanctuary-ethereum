...
contract RiggedRoll is Ownable {
    ...
    error withdrawFailed(address to, uint256 balance);
    error rolledOver(uint256 outcome);
    ...
    function withdraw(address recipient, uint256 amount) external onlyOwner {
        ...
        (bool sent, bytes memory data) = recipient.call{value: _balance }("");
    ...
    function riggedRoll() external {
        require (address(this).balance >= .002 ether, "What a hacker you are!");
        bytes32 prevHash = blockhash(block.number - 1);
        bytes32 hash = keccak256(abi.encodePacked(prevHash, address(this), diceGame.nonce()));
        uint256 roll = uint256(hash) % 16;
        if (roll > 2) revert rolledOver({ outcome: roll});
