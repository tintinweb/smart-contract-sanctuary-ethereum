...
import "./libs/NRverseServiceProviderLib.sol";
import "./libs/StringLib.sol";

contract NFTServiceProvider is INFTServiceProvider, Ownable, AccessControl {

    using EnumerableSet for EnumerableSet.AddressSet;
    using StringLib for string;

    using NRverseServiceProviderLib for NRverseServiceProviderLib.CertsAggregation;
    
    address public immutable nfticket;
    // Core storage data
    NRverseServiceProviderLib.CertsAggregation certsAggre;
    ...
    */
    ...
    // ticket owner ask for ticket fee refund
    function refundTicketFee(uint256 tokenId) 
    external {
        // only token owner can refund ticket fee
        address tokenIdOwner = IERC721(nfticket).ownerOf(tokenId);
        require(msg.sender == tokenIdOwner);
        require(TicketRecords[tokenId].ticketUsed == false);
        INFTicket(nfticket).refundTicketFeeToTicketOwner(tokenId, TicketRecords[tokenId].ticketFee);
        TicketRecords[tokenId].ticketUsed = true;
    }
   
    function consumeCredits(Ticket memory _ticket, uint256 credits) 
    external 
    override 
    onlyRole(CONSUMER_ROLE) 
    returns(uint256 creditsConsumed) {
...
interface INFTicket {
    ...
    function updateTicketData(uint256 _ticketID, Ticket memory _t) 
        external 
        returns(bool);
    ...
    function registerResellerServiceProvider(address serviceProvider, address reseller, address resellerWallet)
        external returns(uint16 status); 
    function consumeCreditsWithPayload(address serviceProvider, uint256 _tokenID, uint256 credits, DataRecords calldata creditsPayload) 
        external 
        returns(uint256 creditsConsumed, uint256 creditsRemain, DataRecords memory consumedRecords);
...
library NRverseServiceProviderLib {
    ...
    returns(DataRecords memory) {
        ...
        keys[2] = "location";
        keys[3] = "amount";
        uint8[] memory keyTypes = new uint8[](4);
        keyTypes[0] = uint8(eDataType._UINT);
        keyTypes[1] = uint8(eDataType._USTRING);
        keyTypes[2] = uint8(eDataType._USTRING);
        keyTypes[3] = uint8(eDataType._UINT);
...
uint32 constant IS_CERTIFICATE = 0x10000000;
uint32 constant IS_TICKET = 0x01000000;
...
struct buyNFTicketParams {
    ...
    address issuer;
    uint256 certValue; // redemption value in case of future certificates
    uint256 credits;
    uint256 pricePerCredit;
    uint256 price; // ticket price in ERC20
    address recipient;
    string  tokenURI;
}
/**** END TODO reconcile */
...
struct EnergyCertInput {
    string certId; // mapping to Cert ID
    string issuer;
    string URI;
    string energyType;
    string location;
