...
interface ISign {
    function updateRandomIndex() external;
    function verifyTokenForAddress(string calldata _salt, bytes calldata _token, address _address) external view returns (bool);
}
...
contract PartyUnicorn is ERC721Enumerable, Ownable {
    ...
    string private _apiURI = "https://api.partyunicorn.acedemos.com/";
    ...
    constructor() ERC721("PartyUnicorn", "PartyUnicorn"){
        ...
        phasePrice[0] = 0.01 ether;
        phasePrice[1] = 0.01 ether;
        phasePrice[2] = 0.01 ether;
        phasePrice[3] = 0.01 ether;
        phasePrice[4] = 0.01 ether;
        phasePrice[5] = 0.01 ether;
        phasePrice[6] = 0.01 ether;
    }
    ...
    function whiteListMint(string calldata _salt, bytes calldata _token) public payable whenNotPaused {
        require(sign.verifyTokenForAddress(_salt, _token, msg.sender), "Invalid payment amount");
        mint(1);
    }

    function specialMint(uint _amount) internal{
    ...
    function mint(uint _amount) public payable whenNotPaused{
        require(tx.origin == msg.sender, "Only EOA");
        require(tokensMinted + _amount <= MAX_TOKENS, "All tokens minted");
        require(_amount > 0 && _amount <= MINT_PER_TX_LIMIT, "Invalid mint amount");
        require(_availableTokens.length > 0, "All tokens for this Phase are already sold");
    ...
    //     if (_msgSender() != address(partyUnicoinBarn))
