...
contract TheCryptomasksProject is ERC721, IERC2981, ReentrancyGuard, Ownable {
    ...
    address private proxyAddr = 0xF57B2c51dED3A29e6891aba85459d600256Cf317;
    ...
    function claim(uint256 tokenId, bytes32[] calldata proof) public payable canMint() {
        require(phase1, "phase 1 not started");
        require(!claimed[msg.sender], "already claimed");
        require(_verify(_leaf(msg.sender, tokenId), proof), "invalid proof");
        _mint(msg.sender, tokenId);
        _updateCounter(tokenId);
        claimed[msg.sender] = true;
    }

    function presaleMint(uint256 tokenId, bytes32[] calldata proof) public payable canMint() {
    ...
    function publicMint() public payable canMint() {
        ...
        require(minted[msg.sender] < limit, "minted max");
    ...
    function getPrice(uint256 tokenId) public view returns (uint256 priceIs) {
        if (0 < tokenId && tokenId < 5) { return priceMythic; }
        if (4 < tokenId && tokenId < 30) { return priceLegendary; }
        if (29 < tokenId && tokenId < 115) { return priceEpic; }
        if (114 < tokenId && tokenId < 270) { return priceRare; }
        if (269 < tokenId && tokenId < 556) { return priceCommon; }
    }
    ...
    function _getTokenID() internal view returns (uint256 newToken) {
