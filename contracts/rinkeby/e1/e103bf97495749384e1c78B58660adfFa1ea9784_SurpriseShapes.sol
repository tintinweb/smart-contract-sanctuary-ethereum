...
contract SurpriseShapes is ERC721, Ownable, ReentrancyGuard {
  using Counters for Counters.Counter;
  using Strings for uint256;

  Counters.Counter private supply;

  uint256 private maxSupplyTotal = 27;
  uint256 private maxSupplyPrivate = 20;
  ...
  constructor(string memory _hiddenMetadataURI, address _paymentsContract) ERC721("Surpise Shapes FT1", "SSFT1") {
  ...
  function mint() public payable {
    ...
    require((totalMinted + maxPerMint) <= maxSupplyPrivate, "Not enough NFTs.");
    require(msg.value >= price, "Insufficient balance to mint.");
    require(
      mintedWallets[msg.sender] < maxPerWallet,
      "The number of minted NFTs will exceed the maximum amount of allowed per wallet."
    );
  ...
  function reserve(uint256 _count) public onlyOwner {
    ...
    require((totalMinted + _count) <= maxSupplyTotal, "Not enough NFTs.");
  ...
  function withdraw() public onlyOwner nonReentrant {
    (bool success, ) = payable(paymentsContract).call{value: address(this).balance}("");
