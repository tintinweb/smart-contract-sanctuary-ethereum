...
contract Test is ERC721Enumerable, Ownable {
    ...
    event WhitelistMinted(uint256 token);
    event ReserveListMinted(uint256 token);
    event NFTsReserved(uint256 startingId, uint256 endingId);
    event WhitelistAdded(uint256 numAdded);
    event ReserveListAdded(uint256 numAdded);
    ...
    constructor(string memory baseURI) ERC721("Yo", "Whatsgood") {
        setBaseURI(baseURI);
    }
    ...
    function reserveNFTs(uint numTokens) public onlyOwner {
        ...
        emit NFTsReserved(tokensMinted + 1, tokensMinted.add(numTokens));
    }
    ...
    function mintWhitelist() public {
        ...
        emit WhitelistMinted(tokensMinted + 1);
    }
    ...
    function mintReserveList() public {
        ...
        _mintToken();
        mintedWallets[msg.sender] = true;
        emit ReserveListMinted(tokensMinted + 1);
    }
    ...
    function reserveListAdd(address[] memory addys) public onlyOwner {
        ...
        emit ReserveListAdded(addys.length);
    }
    ...
    function whitelistAdd(address[] memory addys) public onlyOwner {
        ...
        emit WhitelistAdded(addys.length);
    }
