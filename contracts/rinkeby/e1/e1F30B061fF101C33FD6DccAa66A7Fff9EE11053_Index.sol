...
contract Index is Ownable {
  uint public  thresholdamount= 20*1e18;
  uint public totaldeposit;
  uint public minimumdeposit=1e16;
...
function depositEth() public payable {
     ...
     require(msg.value>minimumdeposit,"amount should be greater than 0.01 eth");
   ...
   uint n= indexinfo.tokens.length;
IndexInfo memory indexa=indexinfo;
uint[] memory balancearray= new uint[](n);
...
require(address(this).balance>=thresholdamount,"Threshold amount is not reached");
uint numoftokens= indexinfo.tokens.length;
IndexInfo memory indexa=indexinfo;
...
IDexAggregator(dex).bestrateswap(weth,indexa.tokens[i],(totaldeposit*indexa.percentages[i])/100,0,address(this));
