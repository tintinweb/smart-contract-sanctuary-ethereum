...
.----------------.  .----------------.  .----------------.  .----------------.  .----------------.  .----------------.  .----------------.
| .--------------. || .--------------. || .--------------. || .--------------. || .--------------. || .--------------. || .--------------. |
| |    _______   | || |     ____     | || |   _____      | || |      __      | || |  _______     | || |     _____    | || |    _______   | |
| |   /  ___  |  | || |   .'    `.   | || |  |_   _|     | || |     /  \     | || | |_   __ \    | || |    |_   _|   | || |   /  ___  |  | |
| |  |  (__ \_|  | || |  /  .--.  \  | || |    | |       | || |    / /\ \    | || |   | |__) |   | || |      | |     | || |  |  (__ \_|  | |
| |   '.___`-.   | || |  | |    | |  | || |    | |   _   | || |   / ____ \   | || |   |  __ /    | || |      | |     | || |   '.___`-.   | |
| |  |`\____) |  | || |  \  `--'  /  | || |   _| |__/ |  | || | _/ /    \ \_ | || |  _| |  \ \_  | || |     _| |_    | || |  |`\____) |  | |
| |  |_______.'  | || |   `.____.'   | || |  |________|  | || ||____|  |____|| || | |____| |___| | || |    |_____|   | || |  |_______.'  | |
| |              | || |              | || |              | || |              | || |              | || |              | || |              | |
| '--------------' || '--------------' || '--------------' || '--------------' || '--------------' || '--------------' || '--------------' |
'----------------'  '----------------'  '----------------'  '----------------'  '----------------'  '----------------'  '----------------'
*/
...
contract Solaris is ERC721, ERC2981, AccessControl, ERC721Burnable {
    ...
    // Swich to pause the minting process by public user
    ...
    constructor(address royaltyAddress, uint96 feeNumerator ) ERC721("Solaris", "SOL") {
    ...
    modifier whilteListCompliance(uint256 _whiteListSize) {
        require(_whiteListSize <= 1000 , "The size of the whitelist array is exceeded");
        _;
    }
    ...
    function publicMint(uint256 _mintAmount) external payable mintCompliance(_mintAmount) {
        ...
        // Atleast one NFT should be minted at a time
        ...
        // Sender must have sufficicent fund to mint the number of tokens
     ...
     * @param _mintAmount The number of tokens to be minted
     */
    function whiteListMint(uint256 _mintAmount) external payable mintCompliance(_mintAmount) {
        // Contract must have enabled for public minting
        require(whitelistEnabled, "The contract is paused for whiteList minting!");
        // If the contract is enabled to whitelist users, the sender's address should be whitelisted
     ...
     */
    function mintManyForAddress(uint256 _mintAmount, address _receiver) public mintCompliance(_mintAmount) onlyRole(MINTER_ROLE) {
        require(_mintAmount <= maxNFTMintedByMinter, "The maximum number of NFTs to be minted exceeds the limit");
     ...
     * @param _receiver The receiver's address
     */
    function mintOneForAddress(address _receiver) public mintCompliance(1) onlyRole(MINTER_ROLE) {
        _mintOne(_receiver);
    }
    ...
    function tokenURI(uint256 _tokenId) public view virtual override returns (string memory){
        ...
        // if revealed is enabled, it returns the toeknURI
     ...
     */
    function setMaxMintAmountPerTx(uint256 _maxMintAmountPerTx) public onlyRole(ADMIN) {
     ...
     */
    function setMaxMintAmountPerWt(uint256 _maxMintAmountPerWt) public onlyRole(ADMIN) {
     ...
     */
    function setMaxNFTMintedByMinter(uint256 _maxNFTMintedByMinter) public onlyRole(ADMIN) {
     ...
     */
    function setHiddenMetadataUri(string memory _hiddenMetadataUri) public onlyRole(ADMIN) {
     ...
     * @param _state True or False value to enable or disable whitelist the addreses
     */
    ...
    /** 
     * @dev Allow admin to whitelist the addresses. The size of the whiltelist addresses array should not be exceeded the max value
     * @param newAddresses The array of addresses to be whitelisted
     */
    function setWhitelist(address[] calldata newAddresses) external whilteListCompliance(newAddresses.length) onlyRole(ADMIN) {
     ...
     */
    function removeWhitelist(address[] calldata currentAddresses) external whilteListCompliance(currentAddresses.length) onlyRole(ADMIN) {
     ...
     */
    function bulkMintOneByOne(address[] calldata receiverAddresses) external mintCompliance(receiverAddresses.length) onlyRole(MINTER_ROLE) {
        require(receiverAddresses.length >=0, "Address array should not be empty");
        // The size of receivers' addressses array should not be exceeded to 100 
        require(receiverAddresses.length <= 100, "The size of Receiver addresses array is exceeded");
