...
contract NastyNFT is ERC721A, Ownable, SignedAllowance {
    /* ========== STORAGE ========== */
    ...
    mapping(bytes => uint256) mintedAmount;
    ...
    modifier mintCompliance(uint256 amount){
        require(amount > 0, "Bad mints");
        require(totalSupply() + amount <= MAX_SUPPLY, "Exceeds max supply");
        require(amount <= maxPerTx, "Exceeds max amount");
        _;
    }
    ...
    /// @param state the state in int accordingly
    ...
    function withdraw() external onlyOwner {
        ...
        require(success, "Unable to withdraw payments");
    }
    ...
    function mint(uint256 amount) external payable notPaused mintCompliance(amount) {
    ...
    /// @param amount amount of token to mint
    function presaleMint(uint256 nonce, bytes memory signature, uint256 amount) 
        external 
        payable 
        notPaused 
        ...
        require(mintedAmount[signature] + amount <= maxPerWl, "Insufficient allowance");
        require(msg.value >= amount * price, "Insufficient fund");

        mintedAmount[signature] += amount;
        _safeMint(msg.sender, amount);
    }
}
