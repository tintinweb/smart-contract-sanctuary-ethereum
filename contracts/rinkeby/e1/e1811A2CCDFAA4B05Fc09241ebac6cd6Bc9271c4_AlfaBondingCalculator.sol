...
contract AlfaBondingCalculator is IBondingCalculator {
    using FixedPoint for *;
    using SafeMath for uint256;

    IERC20 internal immutable AFA;

    constructor(address _AFA) {
    ...
    function markdown(address _pair) external view override returns (uint256) {
        ...
        uint256 reserve;
        if (IUniswapV2Pair(_pair).token0() == address(AFA)) {
            reserve = reserve1;
        } else {
            require(IUniswapV2Pair(_pair).token1() == address(AFA), 'Invalid pair');
            reserve = reserve0;
        }
        return reserve.mul(2 * (10**IERC20Metadata(address(AFA)).decimals())).div(getTotalValue(_pair));
    }
}
