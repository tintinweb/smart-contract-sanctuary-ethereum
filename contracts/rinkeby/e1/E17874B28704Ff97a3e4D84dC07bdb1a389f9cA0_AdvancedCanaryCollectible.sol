...
import "VRFv2RandomnessManager.sol";

contract AdvancedCanaryCollectible is ERC721 {
    ...
    mapping(uint256 => CanaryBreed) tokenIdToCanaryBreed; //Store the Breed identified for one tokenId @todo : might be redundant with tokenIdToCanaryRandomBreed
    mapping(uint256 => uint256) tokenIdToCanaryRandomBreed; // Store teh random mumber identified per tokenId
    ...
    VRFv2RandomnessManager internal vrfv2RandomnessManager;

    address internal s_owner;

    constructor(VRFv2RandomnessManager _vrfv2RandomnessManager)
        ERC721(nftName, nftSymbol)
    {
        ...
        vrfv2RandomnessManager = _vrfv2RandomnessManager;
        s_owner = msg.sender;
    }

    /* 
    args : none
    Returns @param 1 : tokenId that has just been minted.
            @param 2 : requestId for the random number request
    */
    function mintCollection() public onlyOwner returns (uint256) {
        ...
        uint256 requestID = vrfv2RandomnessManager.requestRandomWords();
    ...
    function revealTokenId(uint256 tokenId) public onlyOwner {
        ...
        require(
            vrfv2RandomnessManager.s_randomWords(0) > 0,
            "The random number for the tokenID your are trying to reveal hasn't yet been provided by chainlink"
        );

        // @todo : geenrate random breed for canari
        tokenIdToCanaryRandomBreed[tokenId] =
            vrfv2RandomnessManager.s_randomWords(0) %
            10;
    }
...
contract VRFv2RandomnessManager is VRFConsumerBaseV2 {
