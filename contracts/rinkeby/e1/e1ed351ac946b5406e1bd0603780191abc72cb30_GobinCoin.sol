...
contract GobinCoin is ERC721A, Ownable, ReentrancyGuard {
  ...
  uint256 public coppperPrice = 0.00021 ether;
  uint256 public silverPrice = 0.00022 ether;
  uint256 public goldPrice = 0.0004 ether;
  uint256 public copperSupply = 4;
  uint256 public silverSupply = 6;
  uint256 public goldSupply = 8;
  ...
  function mint(uint256 _mintAmount) public payable mintCompliance(_mintAmount) mintPriceCompliance(_mintAmount) {
    ...
    else if(totalSupply() + _mintAmount < copperSupply + 1){
        cost = coppperPrice;
        require(numberMinted(msg.sender) + _mintAmount <= maxPerWallet,"Too many per wallet!");
    }
    else if(totalSupply() + _mintAmount < silverSupply + coppperPrice + 1){
        cost = silverSupply;
        require(numberMinted(msg.sender) + _mintAmount <= maxPerWallet,"Too many per wallet!");
    }
    else if (totalSupply() + _mintAmount < goldSupply + silverSupply + coppperPrice + 1){
        cost = goldSupply;
        require(numberMinted(msg.sender) + _mintAmount <= maxPerWallet,"Too many per wallet!");
    }
    require(numberMinted(msg.sender) + _mintAmount <= maxPerWallet,"Too many per wallet!");
