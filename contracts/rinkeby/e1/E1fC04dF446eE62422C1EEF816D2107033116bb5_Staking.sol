...
contract Staking is ERC721, ReentrancyGuard, Ownable {
    ...
    function updateBaseRewards(uint256 _rewardsPerSecond) external onlyOwner {
        BaseRewards storage baseRewards = baseRewardsData[baseRewardsIndex];
        baseRewards.rewardsPerSecond = _rewardsPerSecond; 
		currentBaseReward = _rewardsPerSecond;
    ...
    function calculateReward(uint256 bundleId) public view returns(uint256) {
        if (ownerOf(bundleId) == address(this)) {
            ...
            for(currentIndex; currentIndex < baseRewardsIndex - 1; currentIndex++)   {
				...
				totalRewardsTemp = totalRewardsTemp.add(((nextTimestamp - previousTimestamp).div(180)).mul(baseRewardsData[currentIndex].rewardsPerSecond));
                previousTimestamp = nextTimestamp;
            }
        
            //Add rewards for current index
            totalRewardsTemp = totalRewardsTemp.add(((block.timestamp - previousTimestamp).div(180)).mul(baseRewardsData[currentIndex].rewardsPerSecond));
    ...
    function claimReward(uint256 bundleId) public {
        require(msg.sender == ownerOf(bundleId),"Caller not eligible to claim rewards");
		require((block.timestamp - bundles[bundleId].lastUpdateTimestamp) > 540, "Min. stake duration not met");
