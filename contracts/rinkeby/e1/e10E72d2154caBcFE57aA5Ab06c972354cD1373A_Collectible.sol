...
contract Collectible is ERC721MULTI {   
  using Strings for uint256;
    ...
    constructor() {
        owner = _msgSender();
        baseUri = "https://gateway.pinata.cloud/ipfs/QmWH5HQg8A9bUpUcVd56BpwY42wrdo1oExR7xNDsnh4Q5X/";
    }
...
contract ERC721MULTI is Context {
    ...
    struct CollectionState{
        uint16 maxMintsPerUser;
        uint120 collectionSize;
        uint120 tokenCounter;
    }


    string public name = "TestoE";
    string public symbol = "TEST";
    CollectionState public state = CollectionState(5, 25, 0);
    
    mapping(address => mapping(address => bool)) public isApprovedForAll;
    mapping(uint256=>address) internal ownings;
    mapping(uint256=>address) internal approvements;
    mapping(address => uint256) internal balances;


    function _safeMint(address to, uint256 quantity) internal{
        require(to != address(0), "zero address cant reseive");

        uint120 _tokenCounter = state.tokenCounter;

        unchecked {
                balances[to]+=quantity;
                ownings[_tokenCounter] = to;
        }

        for(uint256 i = 0; i < quantity; i++) {
             //       console.log("mint", _tokenCounter+i);
            emit Transfer(address(0), to, _tokenCounter+i);
        }
        
        unchecked {
            state.tokenCounter = _tokenCounter+uint120(quantity);
        }
    }
    ...
    function transferFrom(
        ...
        require(from == ownings[id], "token is already minted");
        require(to != address(0), "zero address cant reseive");
        require(
            _msgSender() == from || _msgSender() == approvements[id] || isApprovedForAll[from][_msgSender()],
            "NOT_AUTHORIZED"
        );
    ...
    function safeTransferFrom(
        ...
        require(calledByUser() || _checkOnERC721Received(from, to, ""), "address cant reseive");
    }
    ...
    function safeTransferFrom(
        ...
        transferFrom(from, to, id);
        require(calledByUser() || _checkOnERC721Received(from, to, data), "address cant reseive");
    }

    function approve(address spender, uint256 id) public {
        address owner = ownings[id];

        require(_msgSender() == owner || isApprovedForAll[owner][_msgSender()], "NOT_AUTHORIZED");

        approvements[id] = spender;
    ...
    function ownerOf(uint256 tokenId) external view returns (address){
        if(tokenId>=state.tokenCounter)revert("token not minted");
    uint256 lowestTokenToCheck;
    if (tokenId >= state.maxMintsPerUser) {
      lowestTokenToCheck = state.tokenCounter - state.maxMintsPerUser + 1;
    }

    for (uint256 curr = tokenId; curr >= lowestTokenToCheck; curr--) {
      address ownership = ownings[curr];
    ...
    function _checkOnERC721Received(
        ...
        if (!calledByUser()) {
            try IERC721Receiver(to).onERC721Received(_msgSender(), from, state.tokenCounter, _data) returns (bytes4 retval) {
