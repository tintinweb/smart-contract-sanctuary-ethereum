...
contract PawtocolStake is Ownable, ReentrancyGuard {
    ...
    struct PoolInfo {
        ...
        uint256 poolDays;
        uint256 earlyMaturityDays;
        uint256 fullMaturityTime;
        uint256 earlyMaturityTime;
        uint256 poolLimit;
        uint256 poolStaked;
        uint256 minStake;
        uint256 maxStake;
    ...
    constructor(
        ...
        poolInfo.push(
            PoolInfo({
                ...
                poolDays: _poolDays,
                earlyMaturityDays: _earlyMaturityDays,
                fullMaturityTime: _poolDays.mul(86400),
                earlyMaturityTime: _earlyMaturityDays.mul(86400),
    ...
    function claimPool() external nonReentrant returns (bool) {
        ...
        if (
            block.timestamp < (user.pool_deposit_time + pool.earlyMaturityTime)
        ) {
            calculatedRewards = 0;
        } else if (
            block.timestamp >
            (user.pool_deposit_time + pool.earlyMaturityTime) &&
            block.timestamp < (user.pool_deposit_time + pool.fullMaturityTime)
        ) {
            uint256 fullReward = (((user.poolBal * pool.poolRewardPercent) /
                1000) / 360) * pool.poolDays;
            calculatedRewards = ((fullReward) * pool.poolPenaltyPercent) / 1000;
        } else {
        ...
        uint256 principalStakeBalance = user.poolBal;
        user.poolBal = 0;
        user.pool_deposit_time = 0;
        user.pool_payouts += amount;

        pool.stakeToken.safeTransfer(address(msg.sender), principalStakeBalance);
        if(totalReward>0){
            pool.rewardToken.safeTransfer(address(msg.sender), totalReward);
        }
    ...
    function earlyMaturityReward(address _userAdd)
        public
        view
        returns (uint256)
    {
        ...
        uint256 fullReward = (((user.poolBal * pool.poolRewardPercent) / 1000) /
            360) * pool.poolDays;
        uint256 calculatedRewards = ((fullReward) * pool.poolPenaltyPercent) /
            1000;
