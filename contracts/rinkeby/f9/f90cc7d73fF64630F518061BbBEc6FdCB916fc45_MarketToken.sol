    ...
    IMarketToken
{
    ...
    function _setRoyalties(uint256 _tokenId) internal virtual {
        for (uint256 i = 0; i < _payees.length; i++) {
            ...
            _royalties[0].value = uint96(_shares[_payees[i]]);
            _royalties[0].account = payable(address(_payees[i]));
        ...
        returns (address receiver, uint256 royaltyAmount)
    {
        ...
        if (_royalties.length > 0) {
            ...
            for (uint256 i = 0; i < _royalties.length; i++) {
                _royaltyAmount += (_salePrice * _royalties[i].value);
            }

            return (_royaltyRecipient, _royaltyAmount / 10000);
        }
        return (address(0), 0);
    }
