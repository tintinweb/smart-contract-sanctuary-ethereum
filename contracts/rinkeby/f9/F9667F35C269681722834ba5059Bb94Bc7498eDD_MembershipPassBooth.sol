...
contract MembershipPass is IMembershipPass, ERC1155, Ownable {
    ...
    constructor(
        ...
        for (uint256 i = 0; i < _tierFee.length; i++) {
            if (_tierCapacity[i] == 0) revert("MembershipPassBooth: Tier capacity at least 1");
            if (_tierFee[i] > 10) revert("MembershipPassBooth: Tier fee less that 10%");
        }
    ...
    function batchMintPassForMember(
        ...
        for (uint256 i = 0; i < _tiers.length; i++) {
            if (_tiers[i] > tierCapacity.length)
                revert("MembershipPassBooth::mintPassForMember: Tier not exist");
            if (_amounts[i] > getRemainingAmount(_tiers[i]))
                revert("MembershipPassBooth::batchMintPassForMember: Insufficient balance");
            supplyByTier[_tiers[i]] = supplyByTier[_tiers[i]].add(_amounts[i]);
        }
        _mintBatch(_account, _tiers, _amounts, "");
...
contract MembershipPassBooth is IMembershipPassBooth, TerminalUtility {
    ...
    function issue(
        ...
        tierSizeOf[_projectId] = _tierCapacity.length;
        _membershipPass = address(membershipPass);

        emit Issue(_projectId, _uri, _membershipPass, _tierFee, _tierCapacity);
    }

    /**
        @notice For the contribution that user need to deposit the fund to the pool.
        @param _fundingCycleId The funding cycle ID
        @param _from The wallet address of the contributor.
        @param _payInfo The payment information for this transaction.
     */
    function stake(
        uint256 _projectId,
        ...
        for (uint256 i = 0; i < _payInfo.length; i++) {
            uint256 baseWeight = _payInfo[i].amount.mul(_payInfo[i].weight);
            uint256 sqrtedWeight = _sqrt(baseWeight);
            totalSqrtWeightBy[_fundingCycleId][i] = totalSqrtWeightBy[_fundingCycleId][i].add(
                sqrtedWeight
            );
            WeightInfo memory _weightByTier = depositedWeightBy[_from][_fundingCycleId][i];
            depositedWeightBy[_from][_fundingCycleId][i] = WeightInfo({
                amount: _payInfo[i].amount,
                baseWeight: _weightByTier.baseWeight.add(baseWeight),
                sqrtWeight: _weightByTier.sqrtWeight.add(sqrtedWeight)
            });
        }
    }
    ...
    /**
     * @notice Get allocations by funding cycle
    ...
    /**
     * @notice Get estimated allocations by funding cycle
     *  allocation 100000 means 0.1(10%), 1 meanss 0.000001(0.0001%), 1000000 means 1(100%)
    ...
    function getEstimatingUserAllocation(
        ...
        for (uint256 i = 0; i < tierSizeOf[_projectId]; i++) {
            uint256 sqrtedWeight = _sqrt(_weights[i]);
            _allocations[i] = sqrtedWeight
                .mul(1e12)
                .div(totalSqrtWeightBy[_fundingCycleId][i] + sqrtedWeight)
                .div(1e6);
        }
    }
