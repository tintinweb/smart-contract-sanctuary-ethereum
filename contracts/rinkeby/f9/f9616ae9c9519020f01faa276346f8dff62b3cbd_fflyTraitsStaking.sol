...
contract fflyTraitsStaking is Ownable, ReentrancyGuard {
    address public tokenAddress = 0xd9145CCE52D386f254917e481eB44e9943F39138; // Token address
    address public NFTaddress = 0xd8b934580fcE35a11B58C6D73aDeE468a2833fa8; // NFT address
    ...
    string rewardingTraitName = "Background";
    string rewardingTraitValue = "Purple";

    bool stakeStatus = true;

    mapping (uint256 => uint256) public totalClaimedByTokenId;
    ...
    function getUnclaimedRewards(uint256[] memory _claimableTokenIds) public view returns (uint256, uint256, uint256, uint256, uint256){
        ...
        for(uint256 i=0; i < _claimableTokenIds.length; i++){
            if((block.timestamp - lastClaimedByTokenId[_claimableTokenIds[i]]) > 7 days){
                rewardMultiplier++;
            }
        }
        ...
        for(uint256 i=0; i < ownedTokenIds.length; i++){
            if((block.timestamp - lastClaimedByTokenId[ownedTokenIds[i]]) > 7 days){
                rewardMultiplier++;
            }
        }
        */
        ...
        if(rewardMultiplier > 0) {
            /*
            if(lastClaimedByTokenId[tokenId] == 0) {
                timeLapsed = block.timestamp - launchTime;
            } else {
                timeLapsed = block.timestamp - lastClaimedByTokenId[tokenId];
            }
            */
            //uint256 unclaimedDuration = timeLapsed * rewardDuration;
            uint256 unclaimedRewards = tokensPerNFT *  rewardMultiplier;
            //return (unclaimedRewards);
            return (tokensPerNFT, timeLapsed, rewardDuration, unclaimedRewards, rewardMultiplier);
        }
    ...
    */
    ...
    function claimRewards(uint256[] memory _claimableTokenIds) public nonReentrant {
        ...
        uint256 tokenId;
        require(((block.timestamp - lastClaimedByTokenId[tokenId]) > 1 days), "You already have claimed today");
        ...
        totalClaimedByTokenId[tokenId] += payableRewards;
        lastClaimedByTokenId[tokenId] = block.timestamp;
