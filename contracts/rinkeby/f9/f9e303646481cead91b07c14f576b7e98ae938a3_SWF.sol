...
contract SWF is ISWF, ERC721Enumerable, Ownable, Pausable {
    ...
    event HumanMinted(uint256 indexed tokenId);
    event VariantMinted(uint256 indexed tokenId);
    event HumanStolen(uint256 indexed tokenId);
    event VariantStolen(uint256 indexed tokenId);
    event HumanBurned(uint256 indexed tokenId);
    event VariantBurned(uint256 indexed tokenId);
    ...
    mapping(uint256 => HumansVariants) private tokenTraits;
    ...
    // 0 - 9 are associated with Human, 10 - 18 are associated with Variants
    uint8[][20] public aliases;

    // reference to the Safezone contract to allow transfers to it without approval
    ...
    constructor(uint256 _maxTokens) ERC721("SafeZone Game", "SWF") {
        ...
        // eyes
        aliases[13] = [0, 1, 2, 3, 4];
        // neck
        aliases[14] = [0, 1, 2, 3, 4];


        // Variants
        // body
        aliases[15] =  [0, 1, 2, 3, 4];
        // head
        aliases[16] = [0, 1, 2, 3, 4];
        // spell
        aliases[17] = [0, 1, 2, 3, 4];
        // eyes
        aliases[18] = [0, 1, 2, 3, 4];
        // neck
        aliases[19] = [0, 1, 2, 3, 4];
    ...
    modifier requireContractsSet() {
        require(address(traits) != address(0) && address(safezone) != address(0) && address(randomizer) != address(0), "Contracts not set");
        _;
    }
    ...
    function setContracts(address _traits, address _safezone, address _rand) external onlyOwner {
    ...
    */
    function mint(address recipient, uint256 seed) external override whenNotPaused {
        ...
        if(tx.origin != recipient && recipient != address(safezone)) {
            // Stolen!
            if(tokenTraits[minted].isHuman) {
                emit HumanStolen(minted);
            }
            else {
                emit VariantStolen(minted);
            }
        }
    ...
    */
    function burn(uint256 tokenId) external override whenNotPaused {
        ...
        if(tokenTraits[tokenId].isHuman) {
            emit HumanBurned(tokenId);
        }
        else {
            emit VariantBurned(tokenId);
        }
        _burn(tokenId);
    }
    ...
    */
    function generate(uint256 tokenId, uint256 seed, LastWrite memory lw) internal returns (HumansVariants memory t) {
        t = selectTraits(seed);
        if (existingCombinations[structToHash(t)] == 0) {
            tokenTraits[tokenId] = t;
            existingCombinations[structToHash(t)] = tokenId;
            if(t.isHuman) {
                emit HumanMinted(tokenId);
            }
            else {
                emit VariantMinted(tokenId);
            }
            return t;
        }
    ...
    */
    function selectTrait(uint16 seed, uint8 traitType) internal view returns (uint8) {
        // uint8 trait = uint8(seed) % uint8(12);
        ...
        uint8 trait = 0;
        uint nonce = 0;
		uint randomnumber = uint256(keccak256(abi.encodePacked(block.timestamp, msg.sender, nonce))) % 5;
		trait = uint8(randomnumber);
    ...
    */
    function selectTraits(uint256 seed) internal view returns (HumansVariants memory t) {    
        t.isHuman = (seed & 0xFFFF) % 10 != 0;
        ...
        if (t.isHuman) {
            // choose a number between 0 and 2
            uint nonce = 0;
		    uint randomnumber = uint256(keccak256(abi.encodePacked(block.timestamp, msg.sender, nonce))) % 3;
			shift = uint8(randomnumber) *5;
    ...
    */
    function structToHash(HumansVariants memory s) internal pure returns (uint256) {
    ...
    /**
    * checks if a token is a Humans
    * @param tokenId the ID of the token to check
    * @return human - whether or not a token is a Humans
    */
    function isHuman(uint256 tokenId) external view override blockIfChangingToken(tokenId) returns (bool) {
        // Sneaky variants will be slain if they try to peep this after mint. Nice try.
        ISWF.HumansVariants memory s = tokenTraits[tokenId];
        return s.isHuman;
    }
    ...
    */
    ...
    function getTokenTraits(uint256 tokenId) external view override blockIfChangingAddress blockIfChangingToken(tokenId) returns (HumansVariants memory) {
