...
contract Phases is Ownable, ReentrancyGuard, ERC721A {
    ...
    modifier mintCompliance(uint256 _quantity) {
        ...
        require(maxSupply >= totalSupply() + _quantity, "Minted Out");
        _;
    }
    ...
    ) external mintCompliance(_quantity) nonReentrant {
        require(_numberMinted(msg.sender) + _quantity <= maxWL, "Exceeds users WL mints");
        require(MerkleProof.verify(proof, _wlMerkleRoot, _leaf(msg.sender)), "Not WL");
        _safeMint(msg.sender, _quantity);
    }

    function publicMint() external mintCompliance(1) nonReentrant {
