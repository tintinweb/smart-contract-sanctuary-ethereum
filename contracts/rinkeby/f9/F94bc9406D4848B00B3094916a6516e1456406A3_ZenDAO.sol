...
contract ZenDAO is Ownable, ERC721, IERC721Metadata {
  ...
  constructor(OSProxyRegistry osProxyRegistry, string memory baseURI, string memory uriExtension) {
    _osProxyRegistry = osProxyRegistry;
    setBaseURI(baseURI);
    setURIExtension(uriExtension);
    _tokenIdTracker.increment();
  }

  function mintWhitelist(uint8 numberOfTokens) external payable {
    if (whitelistIsActive != true) revert WhitelistNotActive();
    if (numberOfTokens > _whitelist[_msgSender()])
      revert ExceedsWhitelistAllowance();
  ...
  function mint(uint256 numberOfTokens) external payable {
    ...
    if (saleIsActive != true) revert SaleNotActive();
  ...
  function setSaleActive(bool saleIsActive_) external onlyOwner {
    saleIsActive = saleIsActive_;
  }

  function setWhitelist(address[] memory addresses, uint8 numAllowedToMint)
    external
    onlyOwner
  {
  ...
  function setWhitelistIsActive(bool whitelistIsActive_) external onlyOwner {
    whitelistIsActive = whitelistIsActive_;
  }
    ...
    returns (string memory)
  {
    ...
    return (bytes(_baseURI).length > 0 && bytes(_uriExtension).length > 0)
