...
contract RewardsWallet is Ownable {
    ...
    */
    ...
    function setNewEGCaddress(address _EGC) external onlyOwner {
        EGC = IBEP20(_EGC);
    }

    function setNewRouterAddress(address _router) external onlyOwner {
    ...
    */
    function withdrawDividends(address _msgS, uint256 _amount) external {
    ...
    Ownable 
{
    ...
    modifier distributionDayCheck {
        if (distributionPeriod < block.timestamp) {
            if (rewardsWallet.getDistributionDay() == distributionDay) {
                distributionDay++;
                distributionPeriod = block.timestamp + 10 minutes;
    ...
    */
    ...
    <for dividends>
    */
    function checkDividendsPerDayClaimed() external view returns(bool) {
        return dividendsPerDayClaimed[_msgSender()][distributionDay];
    }

    function checkWhenSold() external view returns(uint256) {
        return soldInDay[_msgSender()];
    }
    ...
    */
    ...
    function claimDividends(address _beneficiary) public returns(bool) {
        ...
        if (dividendsTotal == 0) {
            ...
            totalUserDividends[_beneficiary] += dividendsTotal;
            return true;
        }
    }
