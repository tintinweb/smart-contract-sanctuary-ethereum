...
contract CrowdFunding {
    ...
    */
    ...
    event search( address funder, uint256 amount);
    function showFunder(uint256 campaignID,uint256 funder_index) public returns(address funder, uint256 amount){ 
        Campaign storage c = campaigns[campaignID];
        funder = c.funders[funder_index].addr;
        amount = c.funders[funder_index].amount;
        emit search(funder,amount);
    }
}