...
/**
 * @title ERC721 Contract for B.Hive Collection 
*/

contract TheBHive is ERC721, ERC721Enumerable, Ownable, ERC721Burnable, ReentrancyGuard {
    
    using Strings for uint256;

    // Provenance Hash of the Bees
    string public constant BEES_PROVENANCE = "";  //Decide to keep or not 
    ...
    uint32 public _reserved;
   
    // Sets the first mint Token ID 
    uint32 public _currentTokenId;
 
    // Max token by wallet
    uint32 public _PresaleWalletLimit;
    uint32 public _PublicSaleWalletLimit;

     // Price token in ether
    uint256 public _price; 
    
    // mapping to keep count of how many tokens an address has bought during each phase
    mapping(address => uint256) private _PresaleWalletCounter;
    mapping(address => uint256) private _PublicPresaleCounter;
     ...
     * _revealedURI : URI of the revealed NFT
     * _unrevealedURI : URI of the non revealed NFT
     */
     ...
     */
    constructor ( string memory unrevealedURI,
                    address payable beneficiary,
                    string memory revealedURI,
                    address signerAddress
                    ) ERC721("The B.Hive", "TBH"){      
        _maxSupply = 5000;
        _reserved = 250;
        _price = 0.02 ether;
        _currentTokenId = 1;
        _PresaleWalletLimit = 2;
        _PublicSaleWalletLimit = 5;
    ...
    function whitelistMinting(uint amountToMint, bytes memory WhitelistSignature ) external payable {
        ...
        require(_PresaleWalletCounter[msg.sender] != 100, "Wallet Address is blacklisted");
        require(_PresaleWalletCounter[msg.sender] + amountToMint <= _PresaleWalletLimit, "max NFT per address exceeded");
        ...
        _PresaleWalletCounter[msg.sender] += amountToMint;
    }
    ...
    */
    ...
    function updatePresalesWalletLimit (uint32 PresaleWalletLimit) external onlyOwner{
         _PresaleWalletLimit = PresaleWalletLimit;
    }

    function updatePublicSaleWalletLimit (uint32 PublicSaleWalletLimit ) external onlyOwner{
         _PublicSaleWalletLimit = PublicSaleWalletLimit;
    }
    ...
    function publicMint(uint amountToMint) external payable{
        ...
        require(_PublicPresaleCounter[msg.sender] + amountToMint <= _PublicSaleWalletLimit, "max NFT per address exceeded");
        require (amountToMint > 0, "Error in the amount requested");
        require (amountToMint + totalSupply() <= _maxSupply - _reserved, "Request superior max Supply");
        require (msg.value >= _price*amountToMint, "Insuficient funds");
        _mintNFT(amountToMint);        

        _PublicPresaleCounter[msg.sender] += amountToMint;
    }
    ...
    **/
    function changeBeneficiary(address beneficiary) public onlyOwner {
        require(beneficiary != address(0), "Beneficiary address cannot be address 0");
        require(beneficiary != _beneficiary, "Beneficiary cannot be same as previous");
        _beneficiary = payable(beneficiary);
    }

    /**
     * @dev Blacklist Address so it cannot mint during presale
    */
    function _blackList(address blacklistedAddress) internal onlyOwner {
        _PresaleWalletCounter[blacklistedAddress] = 100;
    }
    ...
    function getNbrOfMintedToken(address wallet, bool inPresales) external view returns (uint256) {
        if(inPresales){
            return _PresaleWalletCounter[wallet];
        } else {
            return _PublicPresaleCounter[wallet];
        }
    }
    
    /**
     * @dev Exposes the _baseURI
     */
    function revealNFT(bool _andLock) public onlyOwner {
        _revealed = true;
        if(_andLock){
            lockMetadata();
        }
    ...
    */
     ...
     */
    function setBaseURI(string memory newRevealedURI) public onlyOwner { 
        require (!_isLocked, "Metadata is locked!");
        _revealedURI = newRevealedURI;
    }

    /**
    * @dev Set the _unrevealedURI just in case
    *+
    */
    ...
    *
    */
    function decreaseSupply(uint32 newSupply) public onlyOwner {
        require(newSupply < _maxSupply,"Maximum supply can only be decreased");
        require(newSupply >= totalSupply(), "Maximum supply cannot be lower than the current supply");
        _maxSupply = newSupply;
    }

    /**
    * @dev Functions Overload
    *
    */
    ...
    /************
     * @dev presale Whitelist handling 
     ************/
