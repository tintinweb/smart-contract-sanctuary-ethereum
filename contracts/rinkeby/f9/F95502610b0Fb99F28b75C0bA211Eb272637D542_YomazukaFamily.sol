...
contract YomazukaFamily is ERC721A, Ownable {
  ...
  uint256 public WHITELIST_SALE_PRICE = 0.001 ether;
  ...
  function mint(uint256 _quantity) external payable callerIsUser {
    ...
    if ((totalSupply() + _quantity) > NUMBER_OF_UNITS_DROP_1) {
      require(drop1, 'Yomazuka Family :: Drop 1 beyond Max Supply');
    }

    if ((totalSupply() + _quantity) > NUMBER_OF_UNITS_DROP_2) {
      require(drop2, 'Yomazuka Family :: Drop 2 beyond Max Supply');
    }
  ...
  function setDropOneAmount(uint256 dropOneAmount) external onlyOwner {
    NUMBER_OF_UNITS_DROP_1 = dropOneAmount;
  }

  function setDropTwoAmount(uint256 dropTwoAmount) external onlyOwner {
  ...
  function toggleDrop1() external onlyOwner {
    drop1 = !drop1;
  }
