...
contract Yokai2 is ERC721A, Ownable, ReentrancyGuard {
    string private _baseURIextended = "https://yokailabs.net/metadata/";
    uint256 private MAX_SUPPLY = 4444;
    address yokaiBoxContractAddress =
        address(0xc4345dE1b8C101a37c504b53A79C68F2fB85b40f);

    constructor() ERC721A("Yokai Labs", "YOKAI") {}

    function Mint(uint256[] memory tokenList) public {
        YokaiBox yContract = YokaiBox(yokaiBoxContractAddress);
        for (uint256 i = 0; i < tokenList.length; i++) {
            yContract.safeTransferFrom(msg.sender, address(0), tokenList[i]);
        }
