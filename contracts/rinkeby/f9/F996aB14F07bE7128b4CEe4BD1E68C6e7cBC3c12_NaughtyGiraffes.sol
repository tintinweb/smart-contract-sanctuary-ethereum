...
contract NaughtyGiraffes is ERC721Enumerable, Ownable {
    ...
    enum saleType{ PRE, EARLY, PUBLIC }

    // mapping(address => uint[]) private totalNoOfTokensMintedByAddress;
    mapping(address => mapping(uint256 => uint)) public totalNoOfTokensMintedByAddress;    
        ...
        isAddressOnWhitelist(_merkleproof)
    {
        ...
        uint256 totalTokensMintedByAddress;
        uint currentSale;
        ...
        if(block.timestamp >= _publicStartTime) {
            ...
            currentSale = uint(saleType.PUBLIC);
            ...
            currentSale = uint(saleType.EARLY);
        }else if(block.timestamp >= (_publicStartTime - 172800)){
            maxMintPerAddress = maxMintPerAddressInPresale;
            mintPrice = preSalePrice;
            maxMintsPerCategory = maxPresaleSupply;
            currentSale = uint(saleType.PRE);
        }

        totalTokensMintedByAddress = totalNoOfTokensMintedByAddress[msg.sender][currentSale];
        ...
        totalNoOfTokensMintedByAddress[msg.sender][currentSale] = totalTokensMintedByAddress + _numOfTokensToMint;
    }
        ...
        returns (string memory)
    {
        ...
        return
            bytes(_tokenRevealedBaseURI).length > 0
                ? string(
                    abi.encodePacked(_tokenRevealedBaseURI, _tokenId.toString(),baseExtension)
                )
                : _tokenBaseURI;
    }
}
