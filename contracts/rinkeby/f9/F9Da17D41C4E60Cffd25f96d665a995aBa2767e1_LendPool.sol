...
LendPoolStorageExt
{
    ...
    //TODO
    IWETH constant internal WETH = IWETH(0xb49dBe8e2A5a140b3b810c33ac2ba4907A3CA95e);
    /**
    ...
    //TODO
    function getWETH() public pure returns (IWETH){
        return WETH;
    }
    ...
    function liquidate(
        ...
        amount : amount
            ...
            _liquidationThresholdNow
        );
    }

    function liquidateByInsurance(
        address nftAsset,
        uint256 nftTokenId
    ) external override nonReentrant whenNotPaused {
        LiquidateLogic.executeLiquidateByInsurance(_addressesProvider,
            _reserves,
            _nfts,
            nftAsset,
            nftTokenId,
    ...
    struct GetLiquidationPriceLocalVars {
        ...
        uint256 liquidationThresholdNow;
    }
    ...
    view
    override
    returns (uint256 liquidatePrice, uint256 paybackAmount, uint256 liquidationThresholdNow)
    {
        ...
        (vars.paybackAmount, vars.thresholdPrice, vars.liquidatePrice, vars.liquidationThresholdNow,) = GenericLogic.calculateLoanLiquidatePrice(
            loanData.reserveAsset,
            reserveData,
            loanData.nftAsset,
            nftData,
            DataTypes.CalcLiquidatePriceParam({
        loanId : vars.loanId,
        loanAddress : vars.poolLoan,
        reserveOracle : _addressesProvider.getReserveOracle(),
        nftOracle : _addressesProvider.getNFTOracle(),
        liquidationThresholdNow : _liquidationThresholdNow
        })
        );
        ...
        return (vars.liquidatePrice, vars.paybackAmount, vars.liquidationThresholdNow);
    }
    ...
    function setLiquidationThresholdNow(uint256 val) external override onlyLendPoolConfigurator {
        _liquidationThresholdNow = val;
    }
    ...
    function initReserve(
        ...
        _reserves[asset].init(bTokenAddress, debtTokenAddress, interestRateAddress, initExchangeRate);
...
interface ILendPoolLoan {
    ...
    event LoanLiquidatedByInsurance(
        address indexed user,
        uint256 indexed loanId,
        address nftAsset,
        uint256 nftTokenId,
    ...
    function liquidateLoanByInsurance(
        address initiator,
        uint256 loanId,
        address bNftAddress,
        uint256 borrowAmount,
...
interface ILendPool {
    ...
    function setLiquidationThresholdNow(uint256 val) external;
    /**
     * @dev Returns if the LendPool is paused
     */
...
interface ILendPoolAddressesProvider {
    ...
    event InsurancePoolUpdated(address indexed newAddress, bytes encodedCallData);
    ...
    event WINGOracleUpdated(address indexed newAddress);
    ...
    event WingUpdated(address indexed newAddress);
    ...
    function setInsurancePoolImpl(address pool, bytes memory encodedCallData) external;
    ...
    function setWingAddress(address newAddress) external;

    function getWingAddress() external view returns (address);
...
library Errors {
  ...
  string public constant LPC_INVALIED_EMERGERNCY_ADDRESS = "706";
...
library GenericLogic {
    ...
    function calculateLoanLiquidatePrice(
        ...
        address nftAsset,
        DataTypes.NftData storage nftData,
        DataTypes.CalcLiquidatePriceParam memory param
    )
    internal
    view
    returns (
    ...
    )
    {
        ...
        (, vars.borrowAmount) = ILendPoolLoan(param.loanAddress).getLoanReserveBorrowAmount(param.loanId);

        (vars.ltv, vars.liquidationThreshold, vars.liquidationBonus) = nftData.configuration.getCollateralParams();

        vars.nftPriceInETH = INFTOracleGetter(param.nftOracle).getAssetPrice(nftAsset);

        vars.reservePriceInETH = IReserveOracleGetter(param.reserveOracle).getAssetPrice(reserveAsset);
        ...
        vars.liquidatePriceNow = vars.nftPriceInReserve.percentMul(param.liquidationThresholdNow);

        vars.liquidatePrice = vars.nftPriceInReserve.percentMul(PercentageMath.PERCENTAGE_FACTOR - vars.liquidationBonus);

        return (vars.borrowAmount, vars.thresholdPrice, vars.liquidatePrice, vars.liquidatePriceNow, vars.nftPriceInETH);
    }

    function calculateLoanLiquidatePriceByInsurance(
        uint256 loanId,
        address reserveAsset,
        DataTypes.ReserveData storage reserveData,
        address nftAsset,
    ...
    )
    {
        ...
        uint wingPriceInETH = IReserveOracleGetter(reserveOracle).getAssetPrice(wingAddress);

        uint needWingAmount = (vars.borrowAmount * (10 ** 9) * vars.reservePriceInETH) / (wingPriceInETH * 10 ** vars.reserveDecimals);

        vars.nftPriceInETH = INFTOracleGetter(nftOracle).getAssetPrice(nftAsset);

        vars.nftPriceInReserve = ((10 ** vars.reserveDecimals) * vars.nftPriceInETH) / vars.reservePriceInETH;

        vars.liquidatePriceNow = vars.nftPriceInReserve.percentMul(liquidationThresholdNow);

        return (vars.borrowAmount, needWingAmount, vars.liquidatePriceNow, vars.nftPriceInETH);
    }
...
library ReserveLogic {
    ...
    function getNormalizedIncome(DataTypes.ReserveData storage reserve) internal view returns (uint256) {
        if (reserve.lastUpdateTimestamp == block.timestamp) {
            return getExchangeRate(reserve, reserve.variableBorrowIndex, 0);
        } else {
            uint256 scaledVariableDebt = IDebtToken(reserve.debtTokenAddress).scaledTotalSupply();
            uint128 newBorrowIndex = getNewBorrowIndex(reserve, scaledVariableDebt, reserve.variableBorrowIndex, uint40(block.timestamp));
            uint deltaReserve = getReserveAmount(reserve, scaledVariableDebt, reserve.variableBorrowIndex, newBorrowIndex);
            return getExchangeRate(reserve, newBorrowIndex, deltaReserve);
        }
    }

    function getExchangeRate(DataTypes.ReserveData storage reserve, uint borrowIndex, uint deltaReserve) internal view returns (uint) {
        // fToken amount
        uint totalSupply = IBToken(reserve.bTokenAddress).scaledTotalSupply();
        if (totalSupply == 0) {
            return reserve.initExchangeRate;
        }
        address underlying = IBToken(reserve.bTokenAddress).UNDERLYING_ASSET_ADDRESS();
        uint totalCash = IERC20Upgradeable(underlying).balanceOf(reserve.bTokenAddress);
        uint256 totalBorrowFToken = IDebtToken(reserve.debtTokenAddress).scaledTotalSupply();
        uint totalBorrow = totalBorrowFToken.rayMul(borrowIndex);
        return uint128((totalCash + totalBorrow - reserve.totalReserve - deltaReserve).rayDiv(totalSupply));
    }

    function getReserveAmount(DataTypes.ReserveData storage reserve,
        uint256 scaledVariableDebt, uint256 previousVariableBorrowIndex,
        uint256 newVariableBorrowIndex) internal view returns (uint) {
        MintToTreasuryLocalVars memory vars;

        vars.reserveFactor = reserve.configuration.getReserveFactor();

        if (vars.reserveFactor == 0 || previousVariableBorrowIndex == newVariableBorrowIndex) {
            return 0;
        }
    ...
    function updateState(DataTypes.ReserveData storage reserve) internal {
        ...
        uint256 newVariableBorrowIndex = _updateIndexes(
            reserve,
            scaledVariableDebt,
    ...
    function init(
        ...
        reserve.initExchangeRate = initExchangeRate;
    }
    ...
    function _mintToTreasury(
        ...
        reserve.totalReserve += getReserveAmount(reserve, scaledVariableDebt, previousVariableBorrowIndex,
            newVariableBorrowIndex);
    }
    ...
    function _updateIndexes(
        ...
        reserve.variableBorrowIndex = getNewBorrowIndex(reserve, scaledVariableDebt, variableBorrowIndex, timestamp);
        //solium-disable-next-line
        reserve.lastUpdateTimestamp = uint40(block.timestamp);
        reserve.liquidityIndex = uint128(getExchangeRate(reserve, reserve.variableBorrowIndex, 0));
        return reserve.variableBorrowIndex;
    }

    function getNewBorrowIndex(DataTypes.ReserveData storage reserve,
        uint256 scaledVariableDebt, uint256 variableBorrowIndex, uint40 timestamp) internal view returns (uint128) {
        if (scaledVariableDebt != 0) {
            uint256 cumulatedVariableBorrowInterest = MathUtils.calculateCompoundedInterest(
                reserve.currentVariableBorrowRate,
                timestamp
            );
            uint newVariableBorrowIndex = cumulatedVariableBorrowInterest.rayMul(variableBorrowIndex);
            require(newVariableBorrowIndex <= type(uint128).max, Errors.RL_VARIABLE_BORROW_INDEX_OVERFLOW);
            return uint128(newVariableBorrowIndex);
        } else {
            return uint128(variableBorrowIndex);
        }
    }
}
...
library ValidationLogic {
    ...
    function validateLiquidateByInsurance(
        DataTypes.ReserveData storage reserveData,
        DataTypes.NftData storage nftData,
        DataTypes.LoanData memory loanData
    ) internal view {
...
library SupplyLogic {
    ...
    function withdrawTreasury(
        ...
        reserve.updateState();
        reserve.updateInterestRates(asset, bToken, 0, 0);
        if (amount > reserve.totalReserve) {
            amount = reserve.totalReserve;
        }
        IBToken(asset).transferUnderlyingToTreasury(amount);
        reserve.totalReserve -= amount;

        emit WithdrawTreasury(asset, amount);
    }
...
library LiquidateLogic {
    ...
    function executeAuction(
        ...
        (vars.borrowAmount, vars.thresholdPrice, vars.liquidatePrice,,) = GenericLogic.calculateLoanLiquidatePrice(
            ...
            DataTypes.CalcLiquidatePriceParam({
        loanId : vars.loanId,
        loanAddress : vars.loanAddress,
        reserveOracle : vars.reserveOracle,
        nftOracle : vars.nftOracle,
        liquidationThresholdNow : 0
        })
        );
    ...
    function enterAuction(
        ...
        ValidationLogic.validateEnterAuction(reserveData, nftData, loanData);
        ...
        ILendPoolLoan(vars.loanAddress).enterAuction(
            vars.loanId,
            loanData.nftAsset,
            loanData.nftTokenId
        );
    ...
    //todo
    event Test(uint i, address reserveAsset, address weth);
    ...
    function exitAuction(
        ...
        ValidationLogic.validateExitAuction(reserveData, nftData, loanData);
        ...
        ILendPoolLoan(vars.loanAddress).exitAuction(
            vars.loanId,
            loanData.nftAsset,
            loanData.nftTokenId
        );

        emit Test(1, loanData.reserveAsset, address(weth));
        if (loanData.bidderAddress != address(0)) {
            if (loanData.reserveAsset == address(weth)) {
                weth.withdraw(loanData.bidPrice);
                payable(loanData.bidderAddress).transfer(loanData.bidPrice);
            } else {
                IERC20Upgradeable(loanData.reserveAsset).safeTransfer(loanData.bidderAddress, loanData.bidPrice);
            }
        }
    ...
    function executeRedeem(
        ...
        if (loanData.bidderAddress != address(0)) {
            ...
            if (loanData.reserveAsset == address(weth)) {
                ...
                weth.withdraw(loanData.bidPrice + vars.bidFine);
                payable(loanData.bidderAddress).transfer(loanData.bidPrice);
                payable(loanData.firstBidderAddress).transfer(vars.bidFine);
            } else {
                IERC20Upgradeable(loanData.reserveAsset).safeTransfer(loanData.bidderAddress, loanData.bidPrice);
    ...
    struct LiquidateLocalVars {
        ...
        uint256 thresholdPrice;
        uint256 liquidatePrice;
        uint256 liquidationThresholdNowPrice;
        uint256 needWingAmount;
        uint256 nftPriceInETH;
    ...
    function executeLiquidate(
        ...
        (vars.borrowAmount, vars.thresholdPrice, vars.liquidatePrice, vars.liquidationThresholdNowPrice, vars.nftPriceInETH) = GenericLogic.calculateLoanLiquidatePrice(
            loanData.reserveAsset,
            reserveData,
            loanData.nftAsset,
            nftData,
        ...
        reserveOracle : vars.reserveOracle,
        nftOracle : vars.nftOracle,
        liquidationThresholdNow : liquidationThresholdNow
        })
        );

        //If it is in the clearing protection period, it shall be 24 hours. If it reaches the direct clearing line, it is not subject to this time limit
        require(block.timestamp > vars.auctionEndTimestamp || (vars.borrowAmount >= vars.liquidationThresholdNowPrice && vars.liquidationThresholdNowPrice != 0), Errors.LPL_BID_AUCTION_DURATION_NOT_END);
    ...
    function executeLiquidateByInsurance(
        ILendPoolAddressesProvider addressesProvider,
        mapping(address => DataTypes.ReserveData) storage reservesData,
        mapping(address => DataTypes.NftData) storage nftsData,
        address nftAsset,
        ...
        vars.reserveOracle = addressesProvider.getReserveOracle();
        vars.nftOracle = addressesProvider.getNFTOracle();
        vars.emergencyAdmin = addressesProvider.getEmergencyAdmin();
        require(vars.emergencyAdmin != address(0), Errors.LPC_INVALIED_EMERGERNCY_ADDRESS);
        vars.loanId = ILendPoolLoan(vars.poolLoan).getCollateralLoanId(nftAsset, nftTokenId);
        require(vars.loanId != 0, Errors.LP_NFT_IS_NOT_USED_AS_COLLATERAL);
        ...
        ValidationLogic.validateLiquidateByInsurance(reserveData, nftData, loanData);

        // TODO 主网要修改成 day
        vars.auctionEndTimestamp = loanData.bidStartTimestamp + (nftData.configuration.getAuctionDuration() * 600);


        // update state MUST BEFORE get borrow amount which is depent on latest borrow index
        reserveData.updateState();
        address wingAddress = addressesProvider.getWingAddress();
        (vars.borrowAmount, vars.needWingAmount, vars.liquidationThresholdNowPrice, vars.nftPriceInETH) = GenericLogic.calculateLoanLiquidatePriceByInsurance(
            vars.loanId,
            loanData.reserveAsset,
            reserveData,
            nftAsset,
            ...
            vars.nftOracle,
            wingAddress,
            liquidationThresholdNow
        );
        require(block.timestamp > vars.auctionEndTimestamp || (vars.borrowAmount >= vars.liquidationThresholdNowPrice && vars.liquidationThresholdNowPrice != 0), Errors.LPL_BID_AUCTION_DURATION_NOT_END);
        ILendPoolLoan(vars.poolLoan).liquidateLoanByInsurance(
            loanData.bidderAddress,
            vars.loanId,
            nftData.bNftAddress,
            vars.borrowAmount,
        ...
        // update interest rate according latest borrow amount (utilizaton)
        reserveData.updateInterestRates(loanData.reserveAsset, reserveData.bTokenAddress, 0, 0);
        address incentivesController = addressesProvider.getIncentivesController();

        IIncentivesController(incentivesController).liquidateByInsurance(
            reserveData.bTokenAddress, IBToken(reserveData.bTokenAddress).scaledTotalSupply(), vars.needWingAmount);

        // transfer extra borrow amount from liquidator to lend pool
        IERC20Upgradeable(wingAddress).safeTransferFrom(addressesProvider.getInsurancePool(), incentivesController, vars.needWingAmount);

        // transfer erc721 to bidder
        IERC721Upgradeable(loanData.nftAsset).safeTransferFrom(address(this), vars.emergencyAdmin, nftTokenId);

        emit LiquidateByInsurance(
            vars.initiator,
            loanData.reserveAsset,
            vars.borrowAmount,
            vars.remainAmount,
...
library DataTypes {
    ...
    struct CalcLiquidatePriceParam {
        uint256 loanId;
        address loanAddress;
        address reserveOracle;
        address nftOracle;
...
contract LendPoolStorage {
  ...
  uint256 internal _liquidationThresholdNow; // Direct liquidate line
...
interface IBToken is IScaledBalanceToken, IERC20Upgradeable, IERC20MetadataUpgradeable {
  ...
  function transferUnderlyingToTreasury(uint256 amount) external returns (uint256);
