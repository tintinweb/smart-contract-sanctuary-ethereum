...
contract Vendor is Ownable {
  ...
  function buyTokens() payable public{
    ...
    uint256 transferAmount = msg.value * tokensPerEth;
      yourToken.transfer(msg.sender, transferAmount);
      emit BuyTokens(msg.sender, msg.value, transferAmount);
  }
  // ToDo: create a withdraw() function that lets the owner withdraw ETH
function withdraw() public payable onlyOwner{
  uint256 ownerBalance = address(this).balance;
  require(ownerBalance>0, "There are no GLD in your balance");
  ...
  function sellTokens(uint256 _amount) public{
    ...
    uint256 amountToSell = _amount / tokensPerEth;

    uint256 ownerBalance = address(this).balance;

    require(amountToSell <= ownerBalance, "Owner has insufficient amount of eth");
