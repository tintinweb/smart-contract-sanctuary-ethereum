...
contract LilChimpsU is AuthorizableU, ERC721AU, ReentrancyGuardUpgradeable {
    ...
    string private s_staticTokenURI;

    uint256 private s_startTimestamp;
    uint256 private s_preSaleDuration; // 4hours
    uint256 private s_preRevealDuration; // 24hours
    ...
    function initialize() public virtual initializer {
        __ERC721AU_init("Lil Chimpls", "LILCHIMPS");
        __Authorizable_init();
        __ReentrancyGuard_init();
        s_baseTokenURI = "";
        s_maxMintable = 5555;
        s_preSaleDuration = 14400;
        s_preRevealDuration = 86400;
    ...
    function setStaticURI(string calldata _statckTokenURI) external onlyOwner {
        s_staticTokenURI = _statckTokenURI;
    }
    ...
    function setStartTimestamp(uint256 _startTimestamp) external onlyOwner {
        s_startTimestamp = _startTimestamp;
    }
    ...
    function setPreRevealDuration(uint256 _preRevealDuration) external onlyOwner {
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        if (block.timestamp < s_startTimestamp + s_preRevealDuration) {
            return s_staticTokenURI;
        } else {
 ...
 */
contract ERC721AU is ContextUpgradeable, ERC165Upgradeable, IERC721Upgradeable, IERC721MetadataUpgradeable {
    ...
    function __ERC721AU_init(string memory name_, string memory symbol_) internal virtual initializer {
        _name = name_;
        _symbol = symbol_;
    ...
    function approve(address to, uint256 tokenId) public override {
        address owner = ERC721AU.ownerOf(tokenId);
