...
contract Voting  {
 ...
 modifier onlyOwner{
    ...
    function addCandidate(string memory _name,string memory _category,string memory _uri,string memory _description) public onlyOwner {
        ...
        //counter = counter + 1;  //counter should be start from zero
        uint256 _uniqueId = counter;
        Project memory newProject =  Project(_uniqueId,_name,_category,_uri,_description,0,0,new address[](0));
        projects[_uniqueId] = newProject;
        projectCollect.push(newProject);
        counter += 1;
