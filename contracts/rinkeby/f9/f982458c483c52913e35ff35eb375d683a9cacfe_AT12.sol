...
contract AT12 is Ownable, ERC721A, ReentrancyGuard {
  ...
  constructor(
  ) ERC721A("AT12", "AT12", mintLimit, _totalSupply) {
     tokenCount = 0;
  }
  ...
  function socondPsMint(uint256 quantity) public payable nonReentrant {
    require(mintLimit >= quantity, "limit over");
    require(mintLimit >= secondPsMinted[msg.sender] + quantity, "You have no Mint left");
    require(msg.value == mintPrice * quantity, "Value sent is not correct");
    require((quantity + tokenCount) <= (currentSupply), "Sorry. No more NFTs");
