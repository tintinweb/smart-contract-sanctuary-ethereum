...
contract TokenTwo is ERC721, Ownable, recovery {
        ...
        returns (string memory)
    {
        ...
        if (tokenId < _next) {
            ...
            // ( id - start + offset .. mod supply - start ) + start

            newTokenId = (((tokenId - START) + _rand) % (_next - START)) + START;

            console.log("(tokenId - START)", (tokenId - START));
            console.log("(tokenId - START)+ _rand", (tokenId - START) + _rand);
            console.log("(_next - START)", (_next - START));
