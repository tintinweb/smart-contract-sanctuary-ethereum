...
contract Launchpad {
    ...
    constructor(){}
    function deployNewCommissionContract(address _reservedAddress, address _recipientAddress) public {
        commissionAddress = new Commission(_reservedAddress, _recipientAddress); 
    }

    function checkBalance(address commissionContractAddress, address tokenAddress) public returns(uint256){
        require(tokenAddress != address(0), "tokenAddress: can not be zero Address");
        require(commissionContractAddress != address(0), "commissionContractAddress: can not be zero Address");
        commissionAddress = Commission(commissionContractAddress);
        balance = commissionAddress.checkMyTokenBalance(tokenAddress);
        return balance;
    }

    function withdrawBalance(address commissionContractAddress, address tokenAddress) public{
        require(tokenAddress != address(0), "tokenAddress: can not be zero Address");
        require(commissionContractAddress != address(0), "commissionContractAddress: can not be zero Address");
        commissionAddress = Commission(commissionContractAddress);
        commissionAddress.withdraw(tokenAddress);
    }

    function getDecimal(address commissionContractAddress,address token)public returns(uint256){
        commissionAddress = Commission(commissionContractAddress);
        return commissionAddress.getTokenDecimals(token);
    }
}
...
contract Commission is Ownable, ReentrancyGuard{
    ...
    address public ownerCommission;
    ...
    constructor(address _reservedAddress, address _recipientAddress){
        ...
        ownerCommission  =  tx.origin;
    }

    function withdraw(address token) external nonReentrant onlyOwner {
        require(ownerCommission == tx.origin, "You are not the owner");
    ...
    function split(uint256 percentageForOwner, address token) internal view returns(uint256) {
        ...
        console.log(percentageForOwner);
        uint256 percentageBalance = uint256( (percentageForOwner * tokenBalance) / 10000 ); 
    ...
    function checkMyTokenBalance(address token) public view onlyOwner returns(uint256){
