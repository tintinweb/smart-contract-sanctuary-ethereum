...
abstract contract LoanAgentEvents {
    event BorrowSent(address user, address loanAgent, uint256 amount);
    /**
    ...
    event RepaySent(address payer, address borrower, address loanAgent, uint256 repayAmount);
...
abstract contract LoanAgentMessageHandler is
    ILoanAgent,
    ILoanAgentInternals,
    LoanAgentModifiers,
    LoanAgentEvents
{
    ...
    function _sendBorrow(
        ...
        emit BorrowSent(user, address(this), amount);
    }
    ...
    function borrowApproved(
        ...
        if (!ecc.preProcessingValidation(abi.encode(params), metadata)) {
            emit MessageFailed(abi.encode(0x0500, params, metadata));
            return;
        }

        if (!ecc.flagMsgValidated(abi.encode(params), metadata)) {
            emit MessageFailed(abi.encode(0x0501, params, metadata));
            return;
        }
    ...
    function _repayBorrowFresh(
        ...
        emit RepaySent(payer, borrower, address(this), repayAmount);
