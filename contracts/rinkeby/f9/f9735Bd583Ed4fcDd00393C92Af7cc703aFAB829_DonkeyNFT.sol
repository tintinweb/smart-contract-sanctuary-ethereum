...
contract DataLayout is LibraryLock {
    ...
    // 0x0[0] = white list class one
    // 0x0[0] = white list class two
    // 0x0[0] = white list class three
    mapping(address => mapping(uint => uint)) whiteListMintCounts;

    bytes32 public merkleRootOne = 0x84bf2e45e4b23bb29373da83131ac7774bb0a76c96ffdcbd159a41ed652646b7;
    bytes32 public merkleRootTwo = 0xb8a2a15886fd6495cf67297c5e429d8c3d72c1ea8835bceec15e138920867def;
    bytes32 public merkleRootThree = 0xb4069c60c73a82a9308f56ce13b32425ecb3120b774102c315d3643f20b77cb1;
...
contract DonkeyNFT is ERC721Enumerable, DataLayout, Proxiable, Ownable {
    ...
    function createNFTWhitelist(uint amount, address claimer, bool ownerMint, bytes32[] calldata _merkleProof) public payable delegatedOnly {
        require(!isMintingPause, "Minting is currently paused");
        require(inWhitelistMode, "Whitelist only");
        bytes32 leaf =  keccak256(abi.encodePacked(msg.sender));
        uint currentMintCount;
        uint mintBound;
        uint mappingIndex;

        if (MerkleProof.verify(_merkleProof, merkleRootOne, leaf)) {
            mappingIndex = 0;
            currentMintCount = whiteListMintCounts[msg.sender][mappingIndex];
            mintBound = 10;
            ...
            revert("Msg.sender not whitelisted");
        }

        if (!ownerMint) {
            require(amount > 0 && amount <= mintBound, "Invalid mint amount");
            require(currentMintCount + amount <= mintBound, "Invalid mint amount");
        ...
        whiteListMintCounts[msg.sender][mappingIndex] = currentMintCount + amount;
    }
