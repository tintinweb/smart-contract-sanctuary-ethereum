...
contract L7LImplementation is BaseNFT {
    constructor(ICRS _crs, bytes32 _baseNode) ERC721("Community public record","CRS") {
        crs = _crs;
        baseNode = _baseNode;
    }
}
...
abstract contract BaseNFT is ERC721, IBaseNFT {
    ...
    bytes4 constant public TOKEN_URI_SELECTOR = bytes4(keccak256("tokenURI(uint256)"));
    ...
    /**
     * @dev Sets the royalties associated with an base node.
     * @param _beneficiary The associated beneficiary to recieve royalties.
     * @param _amount The associated royalty amount.
     * @param _token ERC20 token to charge royalites, address(0) to use gas coin.
     * @param _forAddress Address which gets this exact 
     */
    function setRoyalties(address _beneficiary, uint256 _amount, address _token, address _forAddress) external onlyOwner {
        bytes32 _node = baseNode;
        IRoyaltiesResolver(crs.resolver(_node)).setRoyalties(_node, _beneficiary, _amount, _token, _forAddress);
    }
     ...
     */
    function tokenURI(uint256 _id) public override(ERC721) view returns (string memory) {
        address _proxy = IProxyConfigResolver(crs.resolver(baseNode)).proxyConfig(baseNode, address(this), TOKEN_URI_SELECTOR);
        return ITokenURIProxy(_proxy).tokenURI(_id);
    }

    /**
     * @dev Change NFT metadata by controller.
     * @param _proxy Address which implements proxy interface.
     */
    function changeTokenURIProxy(address _proxy) external override(IBaseNFT) onlyController {
        IProxyConfigResolver(crs.resolver(baseNode)).setProxyConfig(baseNode, address(this), TOKEN_URI_SELECTOR, _proxy);
    }
