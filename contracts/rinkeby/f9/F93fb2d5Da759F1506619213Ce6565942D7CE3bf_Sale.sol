...
contract Sale is ReentrancyGuardUpgradeable, ERC721HolderUpgradeable, ERC1155HolderUpgradeable, OwnableUpgradeable {
    ...
    function _sharing(ProjectInfo memory _project, SaleInfo memory _sale, uint256 _amount) private {
        ...
        if (_project.isSingle) {
            if (_project.isCreatedByAdmin) {
                emit BidSingleProjectAdmin(_msgSender(), _sale.id, _sale.tokenId, royaltyFeeAble, remainingProfit);
            } else {
                emit BidSingleProjectUser(_msgSender(), _sale.id, _sale.tokenId, percentAdminFee, adminFee, royaltyFeeAble, remainingProfit);
            }

            if (_project.isCreatedByAdmin) {
                emit BidMultiProjectAdmin(_msgSender(),_sale.id, _sale.tokenId, _amount, royaltyFeeAble, remainingProfit);
            } else {
                emit BidMultiProjectUser(_msgSender(), _sale.id, _sale.tokenId, _amount, percentAdminFee, adminFee, royaltyFeeAble, remainingProfit);
            }
        }
    }
