...
contract VestingMinter is AccessControl, Pausable {
    ...
    mapping(address => uint256) public benVested;       // total tokens vested
    ...
    function revoke(address _addr)
        ...
        onlyRole(GRANTOR_ROLE)
    {
        uint256 amount = _mintableAmount(_addr).sub(benClaimed[_addr]);
    ...
    /// @notice Calculate mintable amount for a beneficiary
    /// @param _addr beneficiary address
        ...
        returns (uint256 sum)
    {
        ...
        for (uint256 i = 0; i < timelocks[_addr].length; i++) {
            ...
            if (block.timestamp >= timelock.releaseTimestamp) {
                ...
                uint256 iTimeStamp = timelock.releaseTimestamp.sub(timelock.duration);
    ...
    function _mint(address grantee)
        private
    {
        uint256 amount = _mintableAmount(grantee).sub(benClaimed[grantee]);
        if (amount > 0) {
            require(amount.add(benClaimed[grantee]) <= benTotal[grantee], "Cannot withdraw more than unclaimed amount");

            benClaimed[grantee] = benClaimed[grantee].add(amount);
            totalClaimedAmount = totalClaimedAmount.add(amount);
