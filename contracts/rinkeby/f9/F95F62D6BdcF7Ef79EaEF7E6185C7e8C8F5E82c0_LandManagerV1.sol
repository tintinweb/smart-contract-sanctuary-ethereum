...
contract LandManagerV1 is Ownable {
    ...
    constructor(
        ...
        TerraLand = _terraLand;
        arraySize = _arraySize;
        addLandTiers(_landTiers, _tierCaps, _tierPrices);
    }

    modifier onlyOwnerOrTerraLand() {
        require(
            msg.sender == owner() || msg.sender == address(TerraLand),
            "Ownable: Only Owner and Authorized Contract can perform this action"
        );
        _;
    }
    ...
    function getTerraLand() public view returns (IERC721) {
        return TerraLand;
    }

    function setTerraLand(IERC721 _terraLand) public onlyOwner {
        TerraLand = _terraLand;
    }
    ...
    function afterTokenBurn(uint256 _tokenId) external onlyOwnerOrTerraLand {
        string memory _landTierName = tierByTokenId[_tokenId];
        LandTierInfo memory _tierInfo = landTierInfo[_landTierName];
        landTierInfo[_landTierName] = LandTierInfo(
            --_tierInfo.cap,
            _tierInfo.price,
        ...
        uint256 _landId = landIdByTokenId[_tokenId];
        delete tierByLandId[_landId];
        landIdsByTier[_landTierName]--;
        delete landIdByTokenId[_tokenId];

        emit TokenBurned(_landTierName, _tokenId, _landId, block.timestamp);
    }
