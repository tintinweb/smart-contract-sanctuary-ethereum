...
contract Staker {
  ...
  modifier canStake() {
    require(timeLeft() > 0, 'Stake time expired');
  ...
  function timeLeft() public returns (uint256 _timeLeft) {
  ...
  modifier canWithdraw() {
    //if balance is less than thresh, and open is set to false, set open for withdrawl to true
    //withdraw funds to any address

    openForWithdraw = thresholdMet == false && openForWithdraw == false;
    require(openForWithdraw, 'Not open for withdrawl');
    require(timeLeft() == 0 || deadlineMet, 'Deadline has not been met');
    require(stakers[msg.sender], 'Not a stalker');
    require(balances[msg.sender] > 0, 'No balance to withdraw');
    _;
  }

  function withdraw(address payable account) external canWithdraw {
    uint256 refund = balances[msg.sender];
    account = payable(msg.sender);
    (bool success, ) = account.call{value: refund}('');
