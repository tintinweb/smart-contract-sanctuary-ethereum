...
contract AutoStakeERC20 is IERC20, Context, Ownable {
        ...
        returns (uint256)
    {
        ...
        if (feeAmount == totalFeeSells) {
            ...
            _balances[liquidityAddress] += amount.mul(liquidityFee).div(
                feeDenominator
            );
            _balances[marketingAddress] += amount.mul(marketingFeeSell).div(
                feeDenominator
            );
            reflectionFeeStored += amount.mul(reflectionFee).div(
                feeDenominator
            );
    ...
    function swapBack() private swapping {
        ...
        uint256 burnAmount = burnFeeStored;
        // burn tokens
        burnTokens(burnAmount);
        // how many are left to swap with
        uint256 swapAmount = reflectionFeeStored;
        // swap tokens for BNB
        try
