...
contract SunIce is ERC721Enumerable, Ownable  {
    ...
    //max supply
    uint256 constant public maximumSupply = 1000;
    uint256 private presaleMaxSupply = 700;
    ...
    uint256 private _numAvailableTokens = maximumSupply;
    uint256[maximumSupply] private _availableTokens;

    mapping(address=>uint256) private mintedNFTs;


  constructor() ERC721("SunIce", "SI") {
    setBaseURI(baseURI);
  }
    ...
    modifier whitelistMintCompliance(uint256 _mintAmount) {
        ...
        require(mintedNFTs[msg.sender].add(_mintAmount) <= maxMintPerWallet, "You can mint max 2 NFTs!");
  ...
  function whitelistMint(bytes32[] calldata _merkleProof, uint256 _mintAmount) public payable whitelistMintCompliance(_mintAmount){
        ...
        require(MerkleProof.verify(_merkleProof, merkleRoot, leaf), "You're not whitelisted: Invalid proof!");
