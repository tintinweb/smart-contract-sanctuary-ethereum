...
function walletOfOwnerNameSubdomain(address _owner,string calldata subdomain)
    ...
    returns (string[] memory)
  {
    ...
    uint256 currentTokenId = 1;
    uint256 ownedTokenIndex = 0;
    bytes memory whatBytes = bytes (string.concat('.',subdomain));

    while (ownedTokenIndex < ownerTokenCount) {
      address currentTokenOwner = ownerOf(currentTokenId);

      if (currentTokenOwner == _owner) {
        bytes memory whereBytes = bytes (tokenIDandAddress[currentTokenId]);
        if (whereBytes.length >= whatBytes.length) {
        uint i = whereBytes.length - whatBytes.length;
        bool flag = true;
        uint last_j=0;
