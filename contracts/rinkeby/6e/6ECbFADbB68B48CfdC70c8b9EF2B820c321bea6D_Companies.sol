...
contract Companies {
    ...
    uint private version = 10;
    uint constant BASEPAY = 100;
    uint constant DECIMALS = 18;
    uint constant PAY_DURATION = 60 seconds; // sped up for testing
    ...
    // Events
    event CreatedCompany(uint[] members, string companyName, uint  companyId, address   _wallet);
    event LeftCompany(uint member, string  companyName, uint  companyId, address  _wallet);
    event JoinedCompany(uint[] members, string companyName, uint  companyId, address  _wallet);
    event CreateOffer(uint members, uint[]  candidate, string  companyName, uint  companyId, address  _wallet);
    event AcceptOffer(uint member, string  companyName, uint  companyId, address  _wallet);
    event ClaimAll(address member, uint amount, address  _wallet);
    ...
    constructor() {
        ...
        // a1[1] = 9341;
        // a1[2] = 2315;
        // createCompany("McDonalds",a1);

        // console.log("Attempting to leave company..");
        // uint [] memory a2 = new uint[](2);
        // a2[0] = 8037;
        // a2[1] = 9341;
        // leaveCompany(a2);
    }
...
// Create Company
    function createCompany(string memory _companyName, uint[] memory _members) public {
        ...
        require(!companyNameExists(_companyName), "Company name already exists");
        Company memory _company;
        _company.name = _companyName; // should we make sure this is unique?
        _company.created = block.timestamp; 
        companies.push(_company);  
        ...
        emit CreatedCompany(_members, _companyName, _companyId, msg.sender);
    }
    ...
    function joinCompany(uint _companyId, uint[] memory _newMembers) private {
        ...
        addCompanyEvent(_companyId);
        emit JoinedCompany(_newMembers, companies[_companyId].name, _companyId, msg.sender);
    }
...
// Leave Company
    ...
    function leaveCompany(uint[] memory _members) public {
        ...
        // iterate through each array element, and move to new temp array if found in _members
        uint[] memory _tempMembers = new uint[](companies[_companyId].members.length - _members.length);
        uint _count = 0;
        for (uint j = 0; j < companies[_companyId].members.length; j++) { // iterate through all members of company
            for (uint i = 0; i < _members.length; i++) { // iterate through all members to delete
                if (_members[i] == companies[_companyId].members[j]){
                    companyIds[_members[i]] = 0;
                    emit LeftCompany(_members[i], companies[_companyId].name,_companyId, msg.sender);
                } else {
                    _tempMembers[_count] = companies[_companyId].members[j];
                    _count++;
                }
            }
        }
        companies[_companyId].members = _tempMembers;
    ...
    function createJobOffer(uint _member, uint _companyId, uint[] memory _candidates) public  { 
        ...
        for (uint i = 0; i < _candidates.length; i++){
            ...
            if (ownerOfNFT(_member) == ownerOfNFT(_candidates[i])){ 
                ...
                // emit CreateOffer(_member, _candidates, companies[_companyId].name,_companyId, msg.sender);
                emit CreateOffer(_member, _candidates, companies[_companyId].name,_companyId, msg.sender);
            }
        }
    }
       ...
       for (uint m = 0; m < _members.length; m++) {
            ...
            for (uint i = 0; i < offers[_members[m]].length ; i++) {
                ...
                if (offers[_members[m]][i] ==_companyId){ 
                    ...
                    emit AcceptOffer(_members[m], companies[companyIds[_members[m]]].name,companyIds[_members[m]], msg.sender);
                    break; 
                }
            }
       }
...
// view functions
    ...
    function allNFTsByAddress(address _address) public view returns(uint[] memory){
    ...
    function getCompaniesByRange(uint _start, uint _end) public view returns (Company[] memory) {
        if (_end > companies.length -1)
            _end = companies.length - 1;
        Company[] memory _companies = new Company[](_end - _start + 1);
        for (uint i = 0; i < _companies.length;i++){
            _companies[i] = companies[_start + i];
        }
        return _companies;
    }

    function getTotalSalaries(uint _companyId) public view returns (uint) {
        uint total = 0;
        for (uint i=0;i<companies[_companyId].members.length;i++){
            total += currentSalary(companies[_companyId].members[i]);
        }
        return total;
    }
    ...
    function getCreated(uint _companyId) public view returns (uint) {
        return companies[_companyId].created;
    }

    function companyNameExists(string memory _name) private view returns (bool) {
        for (uint i;i < companies.length;i++){
            string memory _companyName = companies[i].name;
            if ( keccak256(abi.encodePacked(_companyName)) == keccak256(abi.encodePacked(_name))) // compare strings
                return true;
        }
        return false;
    }
...
// Payments
    ...
    function newHireSalary(uint _companyId) public view returns (uint) {
        uint companySize = companies[_companyId].members.length;
        return paycenter.payRate(companySize, companySize + 1);
    }


    // function newHireSalary(uint _hireNumber, uint _companySize) public view returns (uint) {
    //     return paycenter.payRate(_hireNumber, _companySize);
    // }
    ...
    function claimAll() public {
        ...
        RegularsERC20Contract(regularsERC20_Address).mint(msg.sender, totalPay * 10**DECIMALS);
        emit ClaimAll(msg.sender, totalPay  * 10**DECIMALS, msg.sender);
    }
}
