...
/// @title The Farmers staking contract
...
import "./interfaces/IFarmersToken.sol";
import "./base/ERC721Checkpointable.sol";
import "@openzeppelin/contracts/interfaces/IERC2981.sol";

contract FarmersToken is IFarmersToken, Ownable, ERC721Checkpointable, IERC2981 {

    // The farmers DAO address (creators org)
    address public farmersDAO;

    // 10% royalty
    uint256 public royaltiesAmount = 1000;

    // An address who has permissions to mint Farmers
    address public minter;

    // Whether the minter can be updated
    bool public isMinterLocked;

    // The internal farmer ID tracker
    uint256 private _currentFarmerId;
    ...
    /**
     * @notice Require that the sender is the farmers DAO.
     */
    modifier onlyFarmersDAO() {
        require(msg.sender == farmersDAO, 'Sender is not the farmers DAO');
        _;
    }
    ...
    ) ERC721('Farmers', 'FARMER') {
        farmersDAO = _farmersDAO;
        minter = _minter;
    ...
    * @notice Override isApprovedForAll to whitelist user's OpenSea proxy accounts to enable gas-less listings.
    ...
    /**
     * @notice Mint a Farmer to the minter, along with a possible farmers reward
     * Farmers reward are minted every 10 Farmers, starting at 0,
     * @dev Call _mintTo with the to address(es).
     */
    function mint() public override onlyMinter returns (uint256) {
        if (_currentFarmerId <= 10_000 && _currentFarmerId % 10 == 0) {
            _mintTo(farmersDAO, _currentFarmerId++);
        }
        return _mintTo(minter, _currentFarmerId++);
    }

    /**
     * @notice Burn a farmer.
     */
    function burn(uint256 tokenId) public override onlyMinter {
        _burn(tokenId);
        emit FarmerBurned(tokenId);
    }
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        require(_exists(tokenId), 'FarmersToken: URI query for nonexistent token');
        return super.tokenURI(tokenId);
    }

    /**
     * @notice Set the farmers DAO.
     * @dev Only callable by the farmers DAO when not locked.
     */
    function setFarmersDAO(address _farmersDAO) external override onlyFarmersDAO {
        farmersDAO = _farmersDAO;

        emit DAOUpdated(_farmersDAO);
    }
...
/// @title Interface for FarmersToken
...
interface IFarmersToken is IERC721 {

    event FarmerCreated(uint256 indexed tokenId);

    event FarmerBurned(uint256 indexed tokenId);

    event DAOUpdated(address farmersDAO);
    ...
    function setFarmersDAO(address dao) external;
