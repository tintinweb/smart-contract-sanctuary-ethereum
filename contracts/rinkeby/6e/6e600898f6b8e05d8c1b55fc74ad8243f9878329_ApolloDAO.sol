...
contract ApolloDAO is Context {
    ...
    event cycleWinner(address winner, uint256 reward, string summary);
    ...
    uint256 public constant votingCyleLength = 1800;
    uint256 public currentVotingCycleEnd;
    uint256 public previousVotingCycleEnd;
    ...
    function isAlreadyNominated(address _address)
        public
        view
        returns(bool, uint256)
    {
        for (uint256 s = 0; s < votingData[currentVotingCycleEnd].length; s += 1){
            if (_address == votingData[currentVotingCycleEnd][s].nomiantion) return (true, s);
        }
        return (false, 0);
    }
    ...
    function vote(address candidate) external {
        ...
        hasVoted[_msgSender()][currentVotingCycleEnd]=true;
        votingData[currentVotingCycleEnd].push(VoteData(candidate,msg.sender,apolloToken.balanceOf(msg.sender),currentVotingCycleEnd)); 
    }
    ...
    function completeCycle(address _candidate,uint256 _voteCount,string memory voteSummary) public  onlyOwner{
        require(block.timestamp > currentVotingCycleEnd, "Voting Cycle has not ended");
            ...
            previousVotingCycleEnd=currentVotingCycleEnd;
            ...
            if(minContractBalance < votesToAward){
                ...
                emit cycleWinner(leadVoteRecipient.candidate, minContractBalance - daoOwnerTake, voteSummary);

            } else {
                daoOwnerTake = votesToAward * daoOwnerTax / 100;
                _winnings[leadVoteRecipient.candidate] += (votesToAward - daoOwnerTake);
                emit cycleWinner(leadVoteRecipient.candidate, votesToAward - daoOwnerTake, voteSummary);
            }
    ...
    function hasVotedForCreator(address voter,uint256 _cycleId) external view returns (bool) {
        return hasVoted[voter][_cycleId];
    }
