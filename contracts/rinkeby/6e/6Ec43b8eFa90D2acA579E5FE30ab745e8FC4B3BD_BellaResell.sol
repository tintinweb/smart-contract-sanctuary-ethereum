...
contract BellaResell is Initializable, UUPSUpgradeable, ReentrancyGuardUpgradeable, IERC165Upgradeable, AccessControlUpgradeable {
    ...
    // BR-ERR-17        Seller has still NFT to sell
    // BR-ERR-18        Item already closed
        ...
        onlyRole(BACKEND_ROLE) 
    {
        ...
        if(item.supply > 0) {
            if(item.tokenType == TokenType.ERC721) {
                if(IERC721Upgradeable(item.nftContract).ownerOf(item.tokenId) == item.seller) {
                    revert("BR-ERR-17");
                }
            } else {
                if(IERC1155Upgradeable(item.nftContract).balanceOf(item.seller, item.tokenId) >= item.supply) {
                    revert("BR-ERR-17");
                }
            }
        } else {
