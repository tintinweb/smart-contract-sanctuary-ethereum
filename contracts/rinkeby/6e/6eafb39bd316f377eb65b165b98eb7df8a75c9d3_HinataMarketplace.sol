    ...
    ReentrancyGuardUpgradeable
{
    ...
    modifier canBeStoredWith128Bits(uint256 value) {
        require(value < 340282366920938463463374607431768211455);
        _;
    }
    ...
    function initialize(
        ...
        require(hinataStorage_ != address(0), "HinataMarket: INVALID_HINATA_STORAGE");
        require(marketFee_ <= 10000, "HinataMarket: INVALID_FEE");

        __ReentrancyGuard_init();
        __UUPSUpgradeable_init();

        hinataStorage = IHinataStorage(hinataStorage_);
        treasury = treasury_;
    ...
    function createListing(Listing memory listing) external canBeStoredWith128Bits(listing.price) {
        ...
        lastListingId += 1;
        listings[lastListingId] = listing;
        hinataStorage.safeBatchTransferFrom(
            msg.sender,
            address(this),
            listing.tokenIds,
        ...
        emit ListingCreated(lastListingId, msg.sender, listing.listingType);
    }
    ...
    function purchaseListing(uint256 listingId) external nonReentrant {
        ...
        if (fee > 0) {
            IERC20Upgradeable(listing.payToken).safeTransferFrom(msg.sender, treasury, fee);
        }
    ...
    function bid(uint256 listingId, uint256 bidAmount) external nonReentrant {
        ...
        if (bidding.bidder == msg.sender) bidAmount -= bidding.bidAmount;
    ...
    function completeAuction(uint256 listingId) external nonReentrant {
        ...
        uint256 price = bidding.bidAmount;
        uint256 fee = (price * marketFee) / 10000;
        if (fee > 0) {
            IERC20Upgradeable(listing.payToken).safeTransfer(treasury, fee);
        }
        IERC20Upgradeable(listing.payToken).safeTransfer(msg.sender, price - fee);
