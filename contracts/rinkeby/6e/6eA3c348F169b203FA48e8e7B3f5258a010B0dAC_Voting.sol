...
contract Voting {
	...
	/**
	* @notice is member Voted Before 
	*/
	function isMemberVotedBefore(uint256 pollId) internal returns(bool) {
		PollVote[] memory pollsVote = _pollVotes[pollId];
		bool isVotedBefore = false;
		for (uint256 index = 0; index < pollsVote.length; index++) {
			PollVote memory pollVote =  pollsVote[index];
			if(pollVote.voter == msg.sender){
				isVotedBefore = true;
			}
		}
		return isVotedBefore;
	}
	...
	*/
	function communityVoteCast(uint256 pollId, bool vote) external {
		bool checkVoteExistBefore = isMemberVotedBefore(pollId);
		require( checkVoteExistBefore == false, "You have already Voted Before");
