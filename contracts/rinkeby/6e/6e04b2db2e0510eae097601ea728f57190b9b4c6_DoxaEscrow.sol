...
contract DoxaEscrow is Ownable {
   ...
   // mapping(string => address) public eventHostRecords;
    ...
    event DoxaEscrowPayment(address indexed sender, EscrowInfo indexed escrow);
    event ETHEscrowPayment(address indexed sender, EscrowInfo indexed escrow);
    event AppWalletEscrowPayment(address indexed sender, EscrowInfo indexed escrow);
    event Refund(address indexed account, EscrowInfo indexed escrow);
    event USDTEscrowPayment(address indexed sender, EscrowInfo indexed escrow);
    event EscrowPaymentRelease(EscrowInfo indexed escrow);
    ...
    function escrowPayETH(string memory id, address seller) public payable {
        escrowPayment(msg.sender, seller, msg.value, id, 0);
    }

    function escrowPayment(address _buyer, address _seller, uint _amount, string memory id, uint paymentType) public {
        ...
        if(paymentType == 0) {
            emit ETHEscrowPayment(msg.sender, escrow);
            return;
        }
        ...
        if(paymentType == 1) {
            ...
            emit DoxaEscrowPayment(_buyer, escrow);
            return;
        }
        ...
        if(paymentType == 2) {
            ...
            emit AppWalletEscrowPayment(_buyer, escrow);
            return;
        }
        ...
        if(paymentType == 3) {
            ...
            emit USDTEscrowPayment(_buyer, escrow);
            return;
        }
    ...
    function releaseEscrowPayment(string memory id, uint releaseTo) public notDisputed(id) {
        ...
        if(paymentType == 0) {
            ...
            emit EscrowPaymentRelease(escrow);
            return;
        }
    ...
    function releaseDisputePayment(string memory id, uint releaseTo) public onlyOwner {
        require(EscrowRecords[id].isDisputed, "Escrow not in disputed state");
        EscrowRecords[id].isDisputed = false;
        releaseEscrowPayment(id, releaseTo);
    }
