...
function toggleSaleParameters(uint256 MintPrice_, uint256 Event_ID_, uint256 MaxSupply_) external onlyOwner {
...
//User
 function getTicket(uint256 Ticketid) external view 
    returns (
        bool Ticket_Minted,
        uint256 price, 
        bool forSale,
    ...
    {   
        ...
        Ticket_Minted = true;
        price = uint256(tickets[Ticketid].price);
        forSale = bool(tickets[Ticketid].forSale);
        used = bool(tickets[Ticketid].used);
    }
...
function mintTicket() external payable {
   ...
   // require(msg.value == MintPrice, 'Trying to pay a wrong ticket price!');
    ...
    uint256 Ticket_ID = tickets.length -1; 
    _mint(msg.sender, Event_ID*1000 + Ticket_ID);
}
