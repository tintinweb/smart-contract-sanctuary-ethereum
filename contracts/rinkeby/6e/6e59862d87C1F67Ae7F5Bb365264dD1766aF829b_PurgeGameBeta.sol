...
contract PurgeGameBeta is ERC721, ERC721Enumerable, Pausable, Ownable
{
    ...
    address private purgeCoinContract;
    ...
    function setShitcoinAddress(address _purgeCoinContract) public onlyOwner
    {
        purgeCoinContract = _purgeCoinContract;
    }
    ...
    function purge(uint256[] calldata _tokenIds) public  
    {
        ...
        PurgeCoinInterface(purgeCoinContract).mintFromPurge(msg.sender, _tokenIds.length * cost * 1000);
    }
    ...
    function devPurge() public onlyOwner 
    {
        ...
        PurgeCoinInterface(purgeCoinContract).mintFromPurge(tokenowner, cost * 1000);
        _burn(_randomToken);
    ...
    {
        require (PurgeCoinInterface(purgeCoinContract).balanceOf(msg.sender) >= _number * cost * 10000, "Not enough $PURGED");
    }
    ...
    function mintAndPurge(uint256 _number) external payable 
    {
        ...
        RequireSale(_number);
        PurgeCoinInterface(purgeCoinContract).mintFromPurge(msg.sender, _number * cost * 1000);
    ...
    function coinMintAndPurge(uint256 _number) external 
    {
        ...
        PurgeCoinInterface(purgeCoinContract).burnToMint(msg.sender, _number * cost * 9000);
        addToPrizePool(_number);
        emit MintAndPurge("mintandpurge",_number,msg.sender);
    }
    ...
    function coinMint(uint256 _number) external
    {
        ...
        RequireTenMax(_number);
        RequireCoinFunds(_number);
        PurgeCoinInterface(purgeCoinContract).burnToMint(msg.sender, _number * cost * 10000);
        _mintToken(_number);
        addToPrizePool(_number);
    }
