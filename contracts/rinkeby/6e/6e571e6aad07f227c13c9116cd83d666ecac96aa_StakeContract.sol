...
library SafeMath {
   ...
   contract StakeContract {
        ...
        uint256[4] public APY = [800000,900000,1000000,1100000];
        uint256[4] public durations = [4 minutes, 8 minutes, 16 minutes, 90 minutes];
   ...
   constructor(address wolve, address amd,address amdStaking) {
       ...
       minStake = 45522400000000;
        ...
        for(uint256 i ; i < percentages.length;i++){
            percentages[i] = APYtoPercentage(APY[i], durations[i].div(1 minutes));
        }
   }
   ...
   function withdrawInWolve(uint256 count) public {
    ...
    function extendStake(uint256 count,uint256 newplan) public {
       ...
       User storage user = users[msg.sender];
        ...
        uint256 beforeDays = (currentTime  - timeBefore).div(1 minutes);
    ...
    function checkAfterTimeBonus(address userAddress,uint count) public {
        ...
        uint256 currentTime = block.timestamp;
        uint256 nextDays = (currentTime  - timeBefore).div(1 minutes);
    ...
    function checkEndingTime(address userAddress, uint256 count, uint plan) public returns(uint256){
        ...
        endingTime = (block.timestamp - user.stakerecord[count].stakeTime).div(1 minutes);
        stakeTimeForEndind = durations[plan].div(1 minutes);
        return endingTime;
    }
    ...
    function calculaateNewPercentage(uint256 _newDuration,uint256 plan) public {
        ...
        newPercentage = APYtoPercentage(APY[plan], newDuration.div(1 minutes));
