...
contract NewFutureThings is ERC721A, Ownable {
    ...
    KaboomBoxMP public kaboomBox;
    string private _contractURI;
    uint public supply = 7000;
    ...
    function mint(uint quantity) external onlyOwner {
        ...
        uint kaboomCount;
        kaboomCount = ((minted + quantity) / 7) - (minted / 7 );
        if(kaboomCount > 0) {
            kaboomBox.mint(msg.sender, kaboomCount);
        }
    }
    ...
    function setKaboomBox(address addr) external onlyOwner {
        kaboomBox = KaboomBoxMP(addr);
    }
