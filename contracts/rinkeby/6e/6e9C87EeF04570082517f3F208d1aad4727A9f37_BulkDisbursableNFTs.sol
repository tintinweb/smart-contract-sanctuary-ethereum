 ...
 ERC1155BurnableUpgradeable, ERC1155SupplyUpgradeable, UUPSUpgradeable
{
  ...
  function hasRole(
    ...
    returns (bool has)
  {
    // bits that can be set, but don't affect matching
    uint256 unnecessary = USE_ONCE | INTERNAL_MASK;
    uint256 gate = roleToken(role) & ~unnecessary;
    console.log(user);
    ...
    returns (uint256 id)
  {
    ...
    uint256 tokenId = VANILLA_TYPE | tokenNum;
