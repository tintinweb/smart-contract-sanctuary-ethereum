 ...
 * https://github.com/Uniswap/v2-core/blob/master/contracts/UniswapV2Factory.sol#L32
  ...
  _uintArgs[4]: _shopxFee
  _uintArgs[5]: _platformFee

  _addressArgs[0]: _royaltyRecipient
  _addressArgs[1]: _shopxAddress
  _addressArgs[2]: _platformAddress
...
contract ShopXReserveFactory is Pausable {
  ...
  address public shopxTokenAddress=0x7BEF710a5759d197EC0Bf621c3Df802C2D60D848;
  uint256 public shopxRequired;
  ...
  event ShopxAddressUpdated(address indexed _shopxAddress);
  event PlatformAddressUpdated(address indexed _platformAddress);
  ...
  function createReserveNFT(
    ...
    require(IERC20(shopxTokenAddress).balanceOf(msg.sender) >= shopxRequired);
    require(isReserveX[computeAddress(_name,_symbol,_brand)] == false);
    require(_beneficiaryFees.length == _beneficiaryAddresses.length);
    require(shopxFee + platformFee < 10000);
 ...
 * @dev Throws if called by any account other than a shopxAdmin
  ...
  function setShopxAddress (address _shopxAddress) onlyShopxAdmin external {
    require(_shopxAddress != address(0));
    shopxAddress = _shopxAddress;
    emit ShopxAddressUpdated(_shopxAddress);
  }
  ...
  function setShopxRequired (uint256 _shopxRequired) onlyShopxAdmin external {
    shopxRequired = _shopxRequired;
  }
  ...
  function setPlatformFee (uint256 _platformFee) onlyShopxAdmin external {
    platformFee = _platformFee;
  }

  function setPlatformAddress (address _platformAddress) onlyShopxAdmin external {
...
contract ShopXReserveNFT is ERC721A, Ownable, ERC2981ContractWideRoyalties, AccessControl, Pausable, ReentrancyGuard {
  ...
  */
  ...
  function initialize(
    ...
    // value percentage (using 2 decimals: 10000 = 100.00%, 0 = 0.00%)
    // shopxFee value (between 0 and 10000)
    shopxFee = _uintArgs[4];
    shopxAddress = _addressArgs[1];

    // Platform Settings
    platformFee = _uintArgs[5];
    platformAddress = _addressArgs[2];
...
* @dev Mints a token to the msg.sender. It consumes whitelisted supplies.
  ...
  * @dev Only allowed for CrossMint. Mints a token to an address but also checks if the destination address is in the allowList.
    * @param to address of the future owner of the token.
    */
  function crossMint(bytes32[] calldata merkleProof, address to, uint256 quantity) payable external whenNotPaused {
...
interface IShopXReserveNFT {
    ...
    function grantRole(bytes32 role, address account) external;
    function hasRole(bytes32 role, address account) external view returns (bool);
    function initialize(address _owner, string memory _baseURI, uint256[6] memory _uintArgs, address[3] memory _addressArgs, uint256[] memory _beneficiaryFees, address[] memory _beneficiaryAddresses, address[] memory _brandAdmins, address[] memory _saleAdmins, address[] memory _shopxAdmins) external;
