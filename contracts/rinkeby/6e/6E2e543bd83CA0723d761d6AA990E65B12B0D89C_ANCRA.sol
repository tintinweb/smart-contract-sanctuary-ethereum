...
contract ANCRA is Ownable, ERC721A, EIP712Whitelisting, PaymentSplitter, VRFConsumerBaseV2{
    ...
    function publicMint(uint256 amount) external payable callerIsUser {
        ...
        require(totalSupply() + airdropOffset() + amount <= collectionSize, "Reached max supply");
    ...
    function airdropOffset() public view returns (uint256) {
        if (airdropped >= reservedAirdrops) return 0;
        else return (reservedAirdrops - airdropped);
    }

    function getOwnedIDs(address address_) public view returns(uint256[] memory){
