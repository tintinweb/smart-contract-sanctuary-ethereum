...
contract RandomBandName is ERC721Optimized, Ownable, ReentrancyGuard {
    using Strings for uint256;

    uint256 public constant MAX_RBN = 10000;
    uint256 public maxRBNPurchase = 10;
    uint256 public RBNPrice = 0.0042 ether;
    string public _baseRBNURI;

    event RBNMinted(address indexed mintAddress, uint256 indexed tokenId);
    event PermanentURI(string _value, uint256 indexed _id);

    constructor(string memory baseURI)
        ERC721Optimized("RandomBandName", "RBN")
    {
        _baseRBNURI = baseURI;
    }
    ...
    function mintRBN(uint256 numberOfTokens) public payable nonReentrant {
        require(
            (RBNPrice * numberOfTokens) <= msg.value,
            "Ether value sent is not correct"
        );
        require(
            numberOfTokens <= maxRBNPurchase,
            "You can mint max 10 RBN s per transaction"
        );
        require(
            (totalSupply() + numberOfTokens) <= MAX_RBN,
            "Purchase would exceed max supply of RBNs"
        );
    ...
    function createCollectible(address mintAddress) private {
        ...
        if (mintIndex < MAX_RBN) {
            _safeMint(mintAddress, mintIndex);
            emit RBNMinted(mintAddress, mintIndex);
        }
    }
