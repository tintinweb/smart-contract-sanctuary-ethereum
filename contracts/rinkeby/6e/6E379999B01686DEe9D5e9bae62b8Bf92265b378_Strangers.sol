...
//FIXME optimization
//FIXME script testing
//FIXME think of using Pausable.sol
//FIXME think of using OpenZeppelin's Base64
...
contract Strangers is ERC721A, Ownable {
  ...
  string[] private personalities = ["INTJ","INTP","ENTJ","ENTP","INFJ","INFP","ENFJ","ENFP", "ISTJ", "ISFJ", "ESTJ", "ESFJ", "ISTP", "ISFP", "ESTP", "ESFP"];
  mapping (string => string) traitNames;
  ...
  uint128 public constant supply = 4444;
  ...
  modifier enoughSupply(uint256 _amountToMint) {
    require(supply >= _currentIndex.add(_amountToMint),"NES");
    _;
  }
  ...
  function getStrangerAttr(uint256 token) internal view returns (Stranger memory) {
    ...
    for (uint256 index = 0; index < 11; index++) dnaDigits[index] = (dna / 10**((index + 1) * 2)) % 10**2; 
  ...
  function tokenURI(uint256 token) public view virtual override returns (string memory) {
    require(_exists(token), "ERC721Metadata: URI query for nonexistent token");
    return string(abi.encodePacked(
      "data:application/json;base64,",
      Base64.encode(bytes(
        string(abi.encodePacked('{"name": "Stranger #',Strings.toString(token),'", "image": "data:image/svg+xml;base64,',Base64.encode(bytes(image(token))),'",',attributes(token), '}'
        ))
      ))
    ));
  }
