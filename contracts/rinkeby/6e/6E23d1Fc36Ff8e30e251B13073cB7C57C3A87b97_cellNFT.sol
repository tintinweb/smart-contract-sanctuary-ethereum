...
contract cellNFT is ERC721A, Ownable, ReentrancyGuard, svgManip {
    ...
    function _add_trait(bytes memory json, string memory trait_type, string memory trait_value) internal pure returns (bytes memory) {
    ...
    function tokenURI(uint256 tokenId) override public view returns (string memory) {
        ...
        output.json = (_get_cell_core_color(tokenId_str) == true) ? _add_trait(output.json, "Core Color", "Dark"): output.json = _add_trait(output.json, "Core Color", "Light");

        if (cell.eukaryote == 1) {
            output.json = _add_trait(output.json, "Nucleus?", "YES");
            output.json = _add_trait(output.json, "Cell Type", cell_types[cell._type]);
        } else {
            output.json = _add_trait(output.json, "Nucleus?", "no");
        }

        if (tokenId >= TOKENS_MINTED) {
            output.json = _add_trait(output.json, "Found yet?", "No");
            output.json = _add_trait(output.json, "In a better place", "No");
        } else {
            output.json = _add_trait(output.json, "Found yet?", "Yes");
            output.json = (ownerOf(tokenId) == address(0)) ? _add_trait(output.json, "In a better place", "Yes") : _add_trait(output.json, "In a better place", "No");
        }
...
contract svgManip {
    ...
    function _get_cell_core_color(string memory tokenId) internal view returns (bool){
        return random(abi.encodePacked(_seed_phrase, "CELL CORE COLOR", tokenId)) % 10 == 9;
    }
