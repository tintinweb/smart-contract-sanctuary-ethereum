...
contract BookLibrary is Ownable {
    ...
    uint256 borrowPrice = 100000000000000000; // 0.1 LIB

    mapping(bytes32 => Book) books; // bytes32 will be used for the book name (ID)
    mapping(address => mapping(bytes32 => bool)) borrowedBooks; // map that saves the address of the customers and books borrowed
    ...
    constructor(address libTokenAddress, address libWrapperAddress) {
	   LIBToken = libTokenAddress;
       wrapperContract = LIBWrapper(payable(libWrapperAddress));
    }

    event AddBook(bytes32 indexed bookId, uint availableCopies);
    event BorrowBook(bytes32 indexed bookId, uint32 availableCopies, address customerAddr);
    event ReturnBook(bytes32 indexed bookId, uint32 availableCopies, address customerAddr);
    ...
    modifier bookNotBorrowedByUser(string memory _bookName) {
        bytes32 bookId = keccak256(abi.encodePacked(_bookName));
        require(borrowedBooks[msg.sender][bookId] == false, "A book can be borrowed only once.");
        _;
    }

    modifier bookOwnedByUser(string memory _bookName) {
        bytes32 bookId = keccak256(abi.encodePacked(_bookName));
        require(borrowedBooks[msg.sender][bookId] == true, "The book is not borrowed by the customer.");
        _;
    }
    ...
    function getBook(string memory _bookName) public view returns (bytes32, uint32) {
        bytes32 bookId = keccak256(abi.encodePacked(_bookName));
        return (books[bookId].bookId, books[bookId].numOfCopies);
    }

    // Method used to borrow a book by its id
    function borrowBook(string memory _bookName, uint256 value, uint256 deadline, uint8 v, bytes32 r, bytes32 s) public bookAvailable(_bookName) bookNotBorrowedByUser(_bookName) {
        bytes32 bookId = keccak256(abi.encodePacked(_bookName));
        Book storage book = books[bookId];

        LIB(LIBToken).permit(msg.sender, address(this), value, deadline, v, r, s);
        LIB(LIBToken).transferFrom(msg.sender, address(this), borrowPrice);

        book.customers.push(msg.sender);
        borrowedBooks[msg.sender][book.bookId] = true;
        book.numOfCopies--;

        emit BorrowBook(bookId, books[bookId].numOfCopies, msg.sender);
    }
    ...
    function returnBook(string memory _bookName) public bookOwnedByUser(_bookName) {
        ...
        emit ReturnBook(bookId, books[bookId].numOfCopies, msg.sender);
    }
    ...
    function isBookOwnedByCustomer(string memory _bookName) public view returns (bool) {
        bytes32 bookId = keccak256(abi.encodePacked(_bookName));
        return borrowedBooks[msg.sender][bookId] == true;
    }
    ...
    function unwrapToken(address wrapperAddress) public onlyOwner {
        LIB(LIBToken).approve(wrapperAddress, borrowPrice);
...
contract LIBWrapper {
    ...
    function wrap() public payable {
		...
		WLIBToken.mint(msg.sender, msg.value);
