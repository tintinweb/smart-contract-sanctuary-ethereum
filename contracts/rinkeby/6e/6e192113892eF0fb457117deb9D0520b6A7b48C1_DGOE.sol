...
contract DGOE is AdminControl, ICreatorExtensionTokenURI, ILazyDelivery, ILazyDeliveryMetadata {
    address _creator;
    ...
    function configure(uint listingId, address marketplace, address creator, uint tokenIdToMint) public adminRequired {
    ...
    function deliver(uint40 listingId, address to, uint256, uint24 payableCount, uint256, address, uint256) external override returns(uint256) {
        ...
        require(minters[to] + payableCount <= 2, "You can only mint twice.");

        minters[to] += uint8(payableCount);
        ...
        numToSend[0] = payableCount;

        if (IERC1155CreatorCore(_creator).totalSupply(_tokenIdToMint) < 1) {
