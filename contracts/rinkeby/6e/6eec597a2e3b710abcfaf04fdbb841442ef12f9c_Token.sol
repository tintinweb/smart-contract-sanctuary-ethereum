...
contract Token is ERC20, ERC20Burnable {
    ...
    address public vestingContractAddress; // to remain transparent, we declare a public vesting contract address that is deployed within the scope of the current contract 

    constructor() ERC20("TokenName - TestName", "CCOIN") {
        address[] memory beneficiaries = new address[](2); // the array should be initiatized as per the number of beneficiaries we want to declare
        uint256[] memory tokenAmounts = new uint256[](2); // the array should be initiatized as per the number of beneficiaries we want to declare
        uint256[] memory releaseTimes = new uint256[](2); // the array should be initiatized as per the number of vesting periods we want
        uint256[] memory releasePercentages = new uint256[](2); // the array should be initiatized as per the number of vesting periods we want

        beneficiaries[0] = 0xcA918d355Bfb9b554B1D513fa1b0D4Df97A4B96a; // using dummy javascript VM addresses for test purposes, !change this before deployment
        beneficiaries[1] = 0x0387eB62A48dbE63c074d455367A93EEF248956c; // using dummy javascript VM addresses for test purposes, !change this before deployment

        tokenAmounts[0] = 16000000000000000000000000; // using dummy token amounts for test purposes, !change this before deployment
        tokenAmounts[1] = 24000000000000000000000000; // using dummy token amounts for test purposes, !change this before deployment

        releaseTimes[0] = block.timestamp + 1200; // using dummy release times for test purposes, !change this before deployment
        releaseTimes[1] = block.timestamp + 18000; // using dummy release times for test purposes, !change this before deployment

        releasePercentages[0] = 10; // using dummy release percentages for test purposes, !change this before deployment
        releasePercentages[1] = 90; // using dummy release percentages for test purposes, !change this before deployment

        // deploy the vesting contract with the above parameters
        vestingContractAddress = address(new TokenVestor(address(this), 
            beneficiaries, 
            tokenAmounts, 
            releaseTimes,
            releasePercentages));

        // mint the total token amount required to the vesting contract address
        _mint(vestingContractAddress, 16000000000000000000000000 + 24000000000000000000000000);
        // _mint(xxxxxxxxxxxxxxxxxx,  4000000000000000000000000);

   
        // Liquidity and tokens for DEX and Exchange Listing
        _mint(0x6fA7c4125704Fd1F52106d268316782eD7147257, 10000000000000000000000000);    
        // Founders and Partners Wallet
        _mint(0x363e356D765596a13Cbb654dfB23D15807f73D19, 20000000000000000000000000 );
    }
}
...
contract TokenVestor {
    ...
    function release() public virtual {
        ...
        tokenAmounts[msg.sender] = tokenAmounts[msg.sender].sub(amount); // subtract the withdrawn tokens from the users balance in the vesting contract
