...
contract DCCSwapFeeCallBack is IDCCSwapFeeCallBack, Ownable {
    ...
    //缓存区块价格的长度
    uint256 public blockPriceLength = 120;
    ...
    address public propToSetter ;//可以设置属性的用户
    ...
    uint256 public buyBackAmountRateInAccount = 20; //回购当前账户资金比例可以设置 ，按照当前用户剩余资金的20% 来回购
    ...
    function AfterSwapFeeSupportingFeeOnTransferTokensCallBack(
        ...
        require(blackList[xOrigin]!=true,'DCCSwapFeeCallBack::in black list::FORBIDDEN'
        );
        require(
    ...
    function doAssaignCakeReward(
        ...
        if (cakePrice > 0) {
            ...
            if (whetherPriceDown() && buyBackCakeAmountPool > 0) {
                ...
                if(buyBackAmountRateInAccount > 0 && dccBalance > 0){
                    // 当前账号里的20%用来回购
                    buyBackDccBalance = dccBalance.mul(buyBackAmountRateInAccount).div(100);
                }
    ...
    function setBuyBackAmountRateInAccount(uint256 _buyBackAmountRateInAccount) external  {
        require(msg.sender == propToSetter, "DCCSwapFactory::setBuyBackAmountRateInAccount::FORBIDDEN");
        buyBackAmountRateInAccount = _buyBackAmountRateInAccount;
    }

    function setBuyBackPriceDownRate(uint256 _buyBackPriceDownRate) external  {
        require(msg.sender == propToSetter, "DCCSwapFactory::setBuyBackPriceDownRate::FORBIDDEN");
        buyBackPriceDownRate = _buyBackPriceDownRate;
    }

    function setMinDCCValueAllow(uint256 _minDCCValueAllow) external  {
        require(msg.sender == propToSetter, "DCCSwapFactory::setMinDCCValueAllow::FORBIDDEN");
    ...
    function setPropToSetter(address _propToSetter) external onlyOwner {
        propToSetter = _propToSetter;
    }

    function setBlackList(address user, bool isBlack) external  {
        require(msg.sender == propToSetter, "DCCSwapFactory::setBlackList::FORBIDDEN");
...
interface IModifyMasterChef {

    function addMaxFarmFreeAmount(uint256 amount) external  ;
