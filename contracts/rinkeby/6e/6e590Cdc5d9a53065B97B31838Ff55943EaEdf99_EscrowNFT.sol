...
contract EscrowNFT is VRFConsumerBaseV2, Ownable {
    ...
    function paymentEscrow(
        uint256 _rekberId,
        uint256 _paymentAmount,
        Status _status
    ) external payable onlyBuyer(_rekberId) {
        require(sellerToBuyer[_rekberId].status == Status.Pending);
        uint256 _paymentTotal = sellerToBuyer[_rekberId].paymentAmount;
        (uint256 _platFormFee, uint256 _userReceive) = _calculateFee(
            _paymentTotal
        );
        if (_status == Status.Accept) {
            sellerToBuyer[_rekberId].status = Status.Accept;
            IERC721 nft = IERC721(sellerToBuyer[_rekberId].nftAddress);
            IWETH weth = IWETH(WETH);
            if (sellerToBuyer[_rekberId].payment == Payment.Ether) {
                require(msg.value == _paymentTotal);
                weth.deposit{value: _paymentTotal}();
                assert(weth.transfer(address(this), _paymentTotal));
                userBalance[_rekberId][WETH] = _userReceive;
                contractBalance[WETH] = _platFormFee;
                nft.transferFrom(
                    address(this),
                    msg.sender,
                    sellerToBuyer[_rekberId].tokenId
                );
            } else if (sellerToBuyer[_rekberId].payment == Payment.Token) {
                require(_paymentAmount == _paymentTotal);
                IERC20 token = IERC20(sellerToBuyer[_rekberId].paymentAddress);
                token.transferFrom(msg.sender, address(this), _paymentAmount);
                userBalance[_rekberId][
                    sellerToBuyer[_rekberId].paymentAddress
                ] = _userReceive;
                contractBalance[sellerToBuyer[_rekberId].paymentAddress].add(
                    _platFormFee
                );
            }
            emit NewPayment(msg.sender, _rekberId, _paymentTotal);
        }
    }
    ...
    function _createEscrowId(uint256 _nftId, address _buyerAddress)
        private
        view
