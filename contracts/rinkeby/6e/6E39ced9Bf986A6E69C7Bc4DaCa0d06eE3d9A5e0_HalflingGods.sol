...
contract HalflingGods is ERC721Enumerable, Ownable {
    ...
    mapping(address => uint256) private WL_WalletList;
    ...
    // Set the WL addresses
    function setFreeSaleMintList(address[] calldata addresses, uint256 numAllowedToMint) external onlyOwner {
        for (uint256 i = 0; i < addresses.length; i++) {
            WL_WalletList[addresses[i]] = numAllowedToMint;
        }
    }
    // Get the count from WL address
    function getPreSaleMintCount() public view returns (uint256 amount){
    ...
    function mintFree(uint256 numberOfTokens) external payable {
        ...
        require(numberOfTokens <= WL_WalletList[msg.sender], "Exceeded max available white list count to purchase");
