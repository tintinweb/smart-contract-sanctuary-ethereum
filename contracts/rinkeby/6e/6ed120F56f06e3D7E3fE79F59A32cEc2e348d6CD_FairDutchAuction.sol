...
import "contracts/extensions/BaseTemplate.sol";
import "contracts/interfaces/IMiNFTContract.sol";

contract FairDutchAuction is BaseTemplate {
    ...
    function initialize(
        ...
        reservedTokens = saleConfig.reservedTokens;
    ...
    function mint(uint8 quantity)
        ...
        nonReentrant
    {
        // NFTs available to minters
        uint256 mintQuantity = totalQuantity - reservedTokens;

        require(
            block.timestamp >= startingTimestamp,
            "Minting has not started!"
        );
        require(block.timestamp <= endingTimestamp, "Minting has ended!");
        require(quantity > 0, "Quantity can not be 0");
        require(mintTotalSupply < mintQuantity, "Assets have sold out!");
        require(
            mintTotalSupply + quantity <= mintQuantity,
        ...
        require(
            tokensMinted(_msgSender()) + quantity <= maxMintPerWallet,
            "Mints allowed exceeded!"
        );
        ...
        // set final price.
        if (mintTotalSupply + quantity == mintQuantity) {
            finalPrice = mintPrice;
        }

        bidsPerWallet[_msgSender()] += (mintPrice * quantity);
        mintTotalSupply += quantity;
    ...
    function refund(address recipient)
        external
        onlyOwner
        whenMintHasEnded
        nonReentrant
    {
        require(
            ...
            "Address not eligible for refund"
        );

        uint256 balance = bidsPerWallet[recipient] -
            (finalPrice * tokensMinted(recipient));
        require(balance > 0, "Address not eligible for refund");
...
import "../extensions/TokenMetadata.sol";

abstract contract BaseTemplate is
    ERC721AUpgradeable,
    ...
    TokenMetadata
{
    ...
    // tracks the number of available NFTs minted
    ...
    // reserve tokens for promotional or reward purposes
    uint256 public reservedTokens;

    // checks if reserved tokens have been minted
    bool public isReservedTokensMinted;

    /***********************************************************************
                                    MODIFIERS
     *************************************************************************/

    modifier whenMintHasEnded() {
    ...
    function hasMintStarted() public view returns (bool) {
        return
            block.timestamp >= startingTimestamp &&
            block.timestamp <= endingTimestamp;
    }

    function tokensMinted(address wallet) public view returns (uint256) {
    ...
    function withdrawUnmintedTokens()
        external
        onlyOwner
        whenMintHasEnded
        nonReentrant
    {
        uint256 unmintedTokens = totalQuantity - totalSupply();
        require(unmintedTokens > 0, "No NFTs were left");

        while (unmintedTokens > 10) {
            unmintedTokens -= 10;
            _safeMint(owner(), 10);
        }

        _safeMint(owner(), unmintedTokens);
    }

    function mintReservedTokens() external onlyOwner {
        require(reservedTokens > 0, "No tokens were reserved");
        require(!isReservedTokensMinted, "Tokens have already been minted!");
        isReservedTokensMinted = true;

        while (reservedTokens > 10) {
            reservedTokens -= 10;
            _safeMint(owner(), 10);
        }

        _safeMint(_msgSender(), reservedTokens);
    }

    function withdrawFunds() external onlyOwner whenMintHasEnded nonReentrant {
    ...
    function setReservedTokens(uint256 tokens) external onlyOwner {
...
abstract contract TokenMetadata is OwnableUpgradeable {
    ...
    modifier whenRevealIsDelayed() {
        require(delayReveal, "Delay metatdata reveal is not enabled");
        _;
    }
    ...
    function setPreviewURI(string memory uri)
        public
        onlyOwner
        whenRevealIsDelayed
    {
    ...
    function reveal() external onlyOwner whenRevealIsDelayed {
