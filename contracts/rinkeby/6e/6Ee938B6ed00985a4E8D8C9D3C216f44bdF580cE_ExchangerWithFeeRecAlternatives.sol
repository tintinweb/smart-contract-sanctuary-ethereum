...
contract Exchanger is Owned, MixinSystemSettings, IExchanger {
    ...
    bytes32 private constant CONTRACT_CIRCUIT_BREAKER =
    ...
    function exchangeCircuitBreaker()
        internal
        view
        returns (IExchangeCircuitBreaker)
    {
        return
            IExchangeCircuitBreaker(
                requireAndGetAddress(CONTRACT_CIRCUIT_BREAKER)
            );
    }
    ...
    function debtCache() internal view returns (IExchangerInternalDebtCache) {
        return
            IExchangerInternalDebtCache(
                requireAndGetAddress(CONTRACT_DEBTCACHE)
            );
    }
    ...
    {
        (reclaimAmount, rebateAmount, numEntries, ) = _settlementOwing(
            account,
            currencyKey
        );
    }
        ...
        )
    {
        ...
        // For each unsettled exchange
         ...
         = new IExchanger.ExchangeEntrySettlement[](numEntries);
        for (uint256 i = 0; i < numEntries; i++) {
            ...
            // fetch the entry from storage
             ...
             = _getExchangeEntry(account, currencyKey, i);
            ...
            (uint256 destinationAmount, , ) = exchangeRates()
            ...
            uint256 amountShouldHaveReceived = _deductFeesFromAmount(
                destinationAmount,
            ...
            bool sip65condition = exchangeCircuitBreaker()
            ...
            if (!sip65condition) {
                if (exchangeEntry.amountReceived > amountShouldHaveReceived) {
                    ...
                    reclaim = exchangeEntry.amountReceived.sub(
                        amountShouldHaveReceived
                    );
                    reclaimAmount = reclaimAmount.add(reclaim);
                } else if (
                    amountShouldHaveReceived > exchangeEntry.amountReceived
                ) {
                    // if less, add to the rebate tally
                    rebate = amountShouldHaveReceived.sub(
                        exchangeEntry.amountReceived
                    );
    ...
    function calculateAmountAfterSettlement(
        ...
        uint256 balanceOfSourceAfterSettlement = IERC20(
            address(issuer().synths(currencyKey))
        ).balanceOf(from);
    ...
    function isSynthRateInvalid(bytes32 currencyKey)
        ...
        returns (bool)
    {
        (, bool invalid) = exchangeCircuitBreaker().rateWithInvalid(
            currencyKey
        );
        return invalid;
    }
        ...
        returns (uint256 amountReceived, IVirtualSynth vSynth)
    {
        ...
        if (from != exchangeForAddress) {
            require(
                delegateApprovals().canExchangeFor(exchangeForAddress, from),
    ...
    function _processTradingRewards(uint256 fee, address rewardAddress)
        internal
    {
        if (
            fee > 0 && rewardAddress != address(0) && getTradingRewardsEnabled()
        ) {
    ...
    function _updateSNXIssuedDebtOnExchange(
        ...
        uint256[2] memory currencyRates
    ) internal {
    ...
    function _settleAndCalcSourceAmountRemaining(
        ...
        (, uint256 refunded, uint256 numEntriesSettled) = _internalSettle(
            from,
            sourceCurrencyKey,
            false
        );
        ...
        if (numEntriesSettled > 0) {
            ...
            sourceAmountAfterSettlement = calculateAmountAfterSettlement(
                from,
                sourceCurrencyKey,
                sourceAmount,
                refunded
        ...
        )
    {
        ...
        entry.roundIdForSrc = exchangeRates().getCurrentRoundId(
            sourceCurrencyKey
        );
        entry.roundIdForDest = exchangeRates().getCurrentRoundId(
            destinationCurrencyKey
        );
        ...
        (
            entry.destinationAmount,
            entry.sourceRate,
            entry.destinationRate
        ...
        amountReceived = _deductFeesFromAmount(
            entry.destinationAmount,
    ...
    function _exchangeRatesCircuitBroken(
        ...
        if (sourceCurrencyKey != sUSD) {
            (, circuitBroken) = exchangeCircuitBreaker().rateWithBreakCircuit(
                sourceCurrencyKey
            );
        }
        ...
        if (destinationCurrencyKey != sUSD) {
            ...
            (, destCircuitBroken) = exchangeCircuitBreaker()
            .rateWithBreakCircuit(destinationCurrencyKey);
    ...
    function _convert(
        ...
        issuer().synths(sourceCurrencyKey).burn(
            from,
            sourceAmountAfterSettlement
        );
        ...
        if (virtualSynth) {
            ...
            vSynth = _createVirtualSynth(
                IERC20(address(synth.proxy())),
                recipient,
                amountReceived,
                destinationCurrencyKey
            );
    ...
    function suspendSynthWithInvalidRate(bytes32 currencyKey) external {
        ...
        // SIP-65: Decentralized Circuit Breaker
        (, bool circuitBroken) = exchangeCircuitBreaker().rateWithBreakCircuit(
            currencyKey
        );
    ...
    function _ensureCanExchange(
        ...
        require(
            !exchangeRates().anyRateIsInvalid(synthKeys),
    ...
    function _ensureCanExchangeAtRound(
        ...
        require(
            !exchangeRates().anyRateIsInvalidAtRound(synthKeys, roundIds),
            "src/dest rate stale or flagged"
        );
    }
    ...
    {
        require(
            maxSecsLeftInWaitingPeriod(from, currencyKey) == 0,
    ...
    function reclaim(
        ...
        ISynthetixInternal(address(synthetix())).emitExchangeReclaim(
            from,
            currencyKey,
            amount
        );
    ...
    function refund(
        ...
        ISynthetixInternal(address(synthetix())).emitExchangeRebate(
            from,
            currencyKey,
            amount
        );
    ...
    function secsLeftInWaitingPeriodForExchange(uint256 timestamp)
        internal
        view
        returns (uint256)
    {
    ...
    function feeRateForExchange(
        ...
        (uint256 feeRate, bool tooVolatile) = _feeRateForExchange(
            sourceCurrencyKey,
            destinationCurrencyKey
        );
    ...
    function _feeRateForExchange(
        ...
        uint256 baseRate = getExchangeFeeRate(sourceCurrencyKey).add(
            getExchangeFeeRate(destinationCurrencyKey)
        );
        uint256 dynamicFee;
        (dynamicFee, tooVolatile) = _dynamicFeeRateForExchange(
            sourceCurrencyKey,
            destinationCurrencyKey
        );
    ...
    function _dynamicFeeRateForExchange(
        ...
        (uint256 dynamicFeeDst, bool dstVolatile) = _dynamicFeeRateForCurrency(
            destinationCurrencyKey,
            config
        );
        (uint256 dynamicFeeSrc, bool srcVolatile) = _dynamicFeeRateForCurrency(
            sourceCurrencyKey,
            config
        );
    ...
    function _dynamicFeeCalculation(
        ...
        for (uint256 i = prices.length - 1; i > 0; i--) {
            ...
            uint256 deviation = _thresholdedAbsDeviationRatio(
                prices[i - 1],
                prices[i],
                threshold
            );
        ...
        )
    {
        ...
        (, bool srcInvalid) = exchangeCircuitBreaker().rateWithInvalid(
            sourceCurrencyKey
        );
        (, bool dstInvalid) = exchangeCircuitBreaker().rateWithInvalid(
            destinationCurrencyKey
        );
        ...
        bool tooVolatile;
        (exchangeFeeRate, tooVolatile) = _feeRateForExchange(
            sourceCurrencyKey,
            destinationCurrencyKey
        );
...
contract ExchangerWithFeeRecAlternatives is MinimalProxyFactory, Exchanger {
    ...
    function resolverAddressesRequired()
        ...
        returns (bytes32[] memory addresses)
    {
        bytes32[] memory existingAddresses = Exchanger
        .resolverAddressesRequired();
    ...
    function _checkAndUpdateAtomicVolume(uint256 sourceSusdValue) internal {
        uint256 currentVolume = uint256(lastAtomicVolume.time) ==
            block.timestamp
            ? uint256(lastAtomicVolume.volume).add(sourceSusdValue)
            : sourceSusdValue;
        require(
            currentVolume <= getAtomicMaxVolumePerBlock(),
    ...
    function _feeRateForAtomicExchange(
        ...
        uint256 baseRate = getAtomicExchangeFeeRate(sourceCurrencyKey).add(
            getAtomicExchangeFeeRate(destinationCurrencyKey)
        );
        if (baseRate == 0) {
            // If no atomic rate was set, fallback to the regular exchange rate
            baseRate = getExchangeFeeRate(sourceCurrencyKey).add(
                getExchangeFeeRate(destinationCurrencyKey)
            );
        }
