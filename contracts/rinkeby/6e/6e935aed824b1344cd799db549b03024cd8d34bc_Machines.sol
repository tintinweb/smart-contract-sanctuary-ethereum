...
contract ERC721DEYE is Context, ERC165, IERC721, IERC721Metadata {
    ...
    struct AddressData {
        ...
        uint64 numberBurned;
        // Encoded value of all allowances in all presale stages with per address restriction.
        uint64 encodedMintAllowance;
    }
    ...
    // Assumption: No address is allowed to mint more than 99 at any stage.
    uint64 private _mint_allowance_maximum = 100;

    /**
     * Returns the the number of tokens allowed to mint until the selected stage.
     * Note: Stage starts with 0.
     */
    function _getMintAllowance(address owner, uint64 stage) internal view returns (uint64) {
        return _addressData[owner].encodedMintAllowance / (_mint_allowance_maximum ** stage) % _mint_allowance_maximum;
    }

    /**
     * Sets the number of tokens allowed to mint until each stage.
     * Assumption: No address is allowed to mint more than 9 at any stage.
     */
    function _setMintAllowance(address owner, uint64 stageOneAllowances, uint64 stageTwoAllowances) internal {
        _addressData[owner].encodedMintAllowance = _mint_allowance_maximum * stageTwoAllowances + stageOneAllowances;
    }
...
contract Machines is Ownable, ERC721DEYE, ReentrancyGuard {
    ...
    function mintPresaleOne(uint64 quantity) public payable callerIsUser {
        ...
        require(msg.value >= price * quantity, "Not enough ETH sent");
        require(_numberMinted(msg.sender) + quantity <= _getMintAllowance(msg.sender, 0), "Exceeds allowed wallet quantity");
    ...
    function mintPresaleTwo(uint64 quantity) public payable callerIsUser {
        ...
        require(msg.value >= price * quantity, "Not enough ETH sent");
        require(_numberMinted(msg.sender) + quantity <= _getMintAllowance(msg.sender, 1), "Exceeds allowed wallet quantity");
    ...
    function mintAllowedQuantityForAddress(address account, ContractStatus stage) public view returns (uint256) {
        if (stage == ContractStatus.PresaleOne) {
            return _getMintAllowance(account, 0) - _numberMinted(account);
        }
    ...
    function setAllowlist(address[] memory addresses, uint64[] memory presaleOneQuantities, uint64[] memory presaleTwoQuantities) external onlyOwner {
        require(addresses.length == presaleOneQuantities.length && addresses.length == presaleTwoQuantities.length, "addresses does not match quatities length");
        for (uint256 i = 0; i < addresses.length; i++) {
            _setMintAllowance(addresses[i], presaleOneQuantities[i], presaleTwoQuantities[i]);
        }
    }
