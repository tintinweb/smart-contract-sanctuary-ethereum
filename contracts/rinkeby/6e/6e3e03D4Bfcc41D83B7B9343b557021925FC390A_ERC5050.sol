...
contract ERC5050 is ERC721A, Ownable, ReentrancyGuard {
    ...
    function mintPublic(uint256 _amount) external payable nonReentrant {
        ...
        if (chanceFreeMintsAvailable > 0) {
            uint256 freeCount = 0;
            for (uint256 i = 0; i < pricedAmount; i++) {
                uint256 tokenId = totalSupply() + i;
                if (chanceFreeMintsAvailable == 0 || !isFreeMint(tokenId)) continue;
                chanceFreeMintsAvailable = chanceFreeMintsAvailable - 1;
