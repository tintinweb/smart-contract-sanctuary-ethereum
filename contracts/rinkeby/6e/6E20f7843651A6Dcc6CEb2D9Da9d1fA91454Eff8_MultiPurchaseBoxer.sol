...
import "../tokens/interfaces/IERC721Mintable.sol";
...
contract MultiPurchaseBoxer is Context, Ownable, ETHAndERC20CheckoutCounter {
    ...
    function setBasicInfo(address _nftAddress, uint256 _capacity, PriceInfo memory _priceInfo) public onlyOwner {
        nftAddress = _nftAddress;
        capacity = _capacity;
        priceInfo = _priceInfo;
    }

    function ethPurchaseBatch(uint256 _amount) 
        public 
        payable
        onlyCapacityEnoughBatch(_amount)
    {
        uint256 remainingValue = msg.value;
        remainingValue = remainingValue.sub(_ethPurchaseLand(_amount));
        _refundRemainEth(remainingValue);
    }

    function tokenPurchaseBatch(address _token, uint256 _amount) 
        public 
        onlyCapacityEnoughBatch(_amount)
    {
        _tokenPurchaseLand(_token, _amount);
    }

    function _ethPurchaseLand(uint256 _amount) private returns (uint256 purchaseCost) {
        require(priceInfo.token == ZERO_ADDRESS, "unsupport eth");
        require(msg.value >= _amount.mul(priceInfo.price), "no enough value");
        IETHCheckouter(address(this)).ethPurchase{
            value: _amount.mul(priceInfo.price)
        } (
            _amount.mul(priceInfo.price), 
            priceInfo.billingType
        );
        _metaTrade(_amount);
        purchaseCost = _amount.mul(priceInfo.price);
    }

    function _tokenPurchaseLand(address token, uint256 _amount) private {
        require(token == priceInfo.token, "unsupport token");
        tokenPurchase(
            priceInfo.token,
            _msgSender(),
            _amount.mul(priceInfo.price), 
    ...
    function _metaTrade(uint256 _amount) private {
        ...
        seeds = _metaTradeBasic(_msgSender(),  _amount);
    ...
    function _metaTradeBasic(address to, uint256 _amount) private returns (uint256[] memory) {
        uint256[] memory seeds = new uint256[](_amount);
        for (uint256 i=0; i<_amount; i++) {
            IERC721Mintable(nftAddress).mint(to);
