...
contract StakeERC20 is ReentrancyGuard, Ownable, Pausable {
   ...
   function calculateReward(address _owner, uint256 tokenIndex) public returns (uint256){
        StakedToken memory staked = stakedTokens[_owner][tokenIndex];
        uint256 rewardPriceUsd = (staked.amount * getLatestEthPrice(tokenIndex) / 1e8 * rewardFactor * (block.timestamp - staked.startTimestamp)) / 100 / rewardInterval;
        return rewardPriceUsd / duxPrice / 100;                   
   }
   

    function incrementAccruedReward(address _owner, uint256 tokenIndex) internal {
        uint256 reward = calculateReward(_owner,tokenIndex );
        StakedToken memory staked = stakedTokens[_owner][tokenIndex];
    ...
    function getLatestEthPrice(uint256 tokenIndex ) public returns (uint256 latestTokenPrice) {


         if (keccak256(abi.encodePacked(tokenIndex)) == 0)  {
            //matic
