...
import {BFFMultiCharmRenderer2} from "./BFFMultiCharmRenderer2.sol";
import {ERC721} from "@openzeppelin/contracts/token/ERC721/ERC721.sol";


// Used for testnet stubbing of the Bracelet NFT contract
contract SimpleERC721 is ERC721, Ownable {

    address private rendererAddr;

    constructor() ERC721("Test bb nft", "Test BB") {
        for (uint256 i; i < 100; i++) {
            _safeMint(msg.sender, i);
        }
    }

    function setRendererAddr(address addr) external onlyOwner {
        rendererAddr = addr;
    }

    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        //_requireMinted(tokenId);
        BFFMultiCharmRenderer2 renderer = BFFMultiCharmRenderer2(rendererAddr);
