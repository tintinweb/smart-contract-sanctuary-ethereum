...
pragma abicoder v2;
    ...
    contract ValleyOfButterflies is ERC721, Ownable, ERC721Enumerable {
    ...
    uint256 public devReserve = 20;

    event VobMinted(uint256 tokenId, address owner);

    constructor() ERC721("ValleyOfButterflies", "VOB") {}
    ...
    function reverseTokens(address _to, uint256 _reverseAmount)
    external
    onlyOwner
    {
        require(
            _reverseAmount > 0 && _reverseAmount <= devReserve,
            "Not enough reverse left for team"
        );
    for (uint256 i = 0; i < _reverseAmount; i++){
        uint256 id = totalSupply();
        _safeMint(_to, id);
    }
     devReserve = devReserve.sub(_reverseAmount);
    }
    ...
    function mintVob(uint256 numberOfTokens) external payable {
    ...
    require(
        ...
        "Ether value sent is not corrent"
    );
    ...
    for (uint256 i = 0; i < numberOfTokens; i++){
        ...
        if(totalSupply() < MAX_TOKENS) {
            ...
            emit VobMinted(id, msg.sender);
        }
    }
}

    function presaleVob(uint256 numberOfTokens) external payable {
...
returns (string memory)
{
    require(
        _exists(tokenId),
        "ERC721Metadaya: URI query for nonexistent token"
    );
...
if (isRevealed == false) {
    return
    "ipfs://Qmd8vqmxjkVd963wsHVWcEo1XdJu9qoPBiw7fMvid2KT8J/test.json";
}
