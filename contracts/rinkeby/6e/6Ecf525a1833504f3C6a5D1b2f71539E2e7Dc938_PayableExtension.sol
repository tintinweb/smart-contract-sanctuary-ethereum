    ...
    CreatorExtensionBasic
{
    ...
    function mint(bytes32[] calldata merkleProof) external payable {
        require(block.timestamp >= _launchDate, "minting not enabled");
        require(
...
abstract contract TokenAllowlist is AdminControl {
    ...
    // boolean for tokenAllowlistEnabled tokenAllowlistEnabled or not
     ...
     * If the address doesn't one own the token returns 0
     */
    function quantityOf(
        INFT nft,
        uint96 tokenID,
...
abstract contract Allowlist is AdminControl {
    ...
    /**
     * if the allowlist is enabled
     * check if a the merkleProof  is on the allowlist
