...
contract MigrateAgency is Initializable, OwnableUpgradeable {
    using SafeMathUpgradeable for uint256;
    using SafeBEP20 for IBEP20;

    AgencyToken public agency;
    IBEP20 public bShiba;

    uint256 public migrationRate;
    mapping (address => uint256) public userInfo;

    function initialize(AgencyToken _agency, IBEP20 _bShiba) public initializer {
        __Ownable_init();
        
        agency = _agency;
        bShiba = _bShiba;
        migrationRate = 2*(10**9); //  0.000000002 agency per bShiba
    }

    function setAgencyTokenAddress(AgencyToken _agency) public onlyOwner{
        agency = _agency;
    }

    function setShibaTokenAddress(IBEP20 _bShiba) public onlyOwner{
        bShiba = _bShiba;
    }

    function setMigrationRate(uint256 _rate) public onlyOwner{
        migrationRate = _rate;
    }

    function bshibaBalance(address _user) public view returns (uint256) {
        return bShiba.balanceOf(_user);
    }

    function agencyaBalance(address _user) public view returns (uint256) {
        return agency.balanceOf(_user);
    }

    function migrationAmount(address _user) public view returns (uint256) {
        uint256 bShibaBlance = bShiba.balanceOf(_user);
        return migrationRate.mul(bShibaBlance).div(10**bShiba.decimals());
    }

    // Migrate bShiba to Agency.
    function Migrate() public {
        
        uint256 bShibaBlance = bShiba.balanceOf(address(msg.sender));
        require(bShibaBlance > 0, "Migrate: insufficient bShiba balance");

        bShiba.transferFrom(address(msg.sender), address(this), bShibaBlance);
        
        uint256 migrateAmount = migrationRate.mul(bShibaBlance).div(10**bShiba.decimals());

        uint256 agencyBalance = agency.balanceOf(address(this));

        if(migrateAmount > agencyBalance)
            migrateAmount = agencyBalance;

        agency.transfer(address(msg.sender), migrateAmount);
        userInfo[msg.sender] = migrateAmount;
    }

    function withdrawShiba() public onlyOwner{
        uint256 bShibaBlance = bShiba.balanceOf(address(this));
        require(bShibaBlance > 0, "withdrawShiba: insufficient bShiba balance");
        bShiba.transfer(address(msg.sender), bShibaBlance);
    }

    function withdrawAgency() public onlyOwner{
        uint256 agencyBlance = agency.balanceOf(address(this));
        require(agencyBlance > 0, "withdrawAgency: insufficient agency balance");
        agency.transfer(address(msg.sender), agencyBlance);
    }
}
