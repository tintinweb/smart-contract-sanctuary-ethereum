...
contract DefiMon is IERC721, ERC721URIStorage, Ownable {
   ...
   constructor() ERC721("Defimon", "DMON") { }
    ...
    function hatchEgg(uint256 _tokenId) public {
        ...
        require(msg.value == HATCHING_PRICE, "Price value sent is not correct");
        require(!isHatched[_tokenId], "Egg already hatched");
        DefimonStats storage baseStats = tokenIdToDefimonStats[_tokenId];
        Defigenetics.DefimonStats memory tempBaseStats;      
        tempBaseStats = defigenetics.generateStats(_tokenId);
    ...
    function mintEgg(uint numberOfTokens) public payable {
        ...
        for(uint i = 0; i < numberOfTokens; i++) {
            if (tokenCounter < MAX_MINT_EGGS) {
                 ...
                 _setTokenURI(tokenCounter,string(abi.encodePacked(EGG_URI, tokenCounter.toString())));
    ...
    function unStakeNFT(uint256 _tokenId) public {
        ...
        require(msg.sender == staker[_tokenId],"NFT not staked by this address");
