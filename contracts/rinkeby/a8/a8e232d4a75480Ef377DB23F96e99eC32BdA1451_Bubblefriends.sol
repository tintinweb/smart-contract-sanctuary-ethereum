...
contract Bubblefriends is TwoStage {
  uint256 public freelistReserved;
  ...
  function initialize(
    ...
    freelistReserved = 100;
  }

  function freelistMint(uint256 amount_, bytes32[] calldata proof_)
    external
    payable
    onlyWhitelisted(uint8(Stage.Whitelist), msg.sender, proof_)
  {
    require(stage == Stage.Whitelist, "Freemint phase sale not enabled");
    require(
      freelistReserved - amount_ >= 0,
      "Not enough reserved for whitelist"
    );
    uint8 _stage = uint8(Stage.Whitelist);
    _increaseBalance(_stage, msg.sender, amount_);
    freelistReserved -= amount_;
