...
contract TraitSniper is Ownable, ERC721AQueryable, ReentrancyGuard, ERC2981 {
    ...
    uint256 public constant MAX_PUBLIC_GENESIS_MINT = 10;
    uint256 public constant MAX_PUBLIC_ALPHA_MINT = 20;
    ...
    function mintGenesisPublic(uint256 _quantity) external payable callerIsUser {
        ...
        require(
            (totalGenesisPublicSaleMinted[msg.sender] + _quantity) <= MAX_PUBLIC_GENESIS_MINT,
    ...
    function mintAlphaPublic(uint256 _quantity) external payable callerIsUser {
        ...
        require(
            (totalAlphaPublicSaleMinted[msg.sender] + _quantity) <= MAX_PUBLIC_ALPHA_MINT,
            'Trait Sniper :: Already minted max allowed amount!'
        );
    ...
    function mintGenesisPresale(bytes32[] memory _merkleProof, uint256 _quantity) external payable callerIsUser {
        ...
        //create leaf node
        bytes32 sender = keccak256(abi.encodePacked(TierEnum.Genesis, msg.sender));
    ...
    function mintAlphaPresale(bytes32[] memory _merkleProof, uint256 _quantity) external payable callerIsUser {
        ...
        //create leaf node
        bytes32 sender = keccak256(abi.encodePacked(TierEnum.Alpha, msg.sender));
        require(MerkleProof.verify(_merkleProof, merkleRoot, sender), 'Trait Sniper :: You are not in whitelist');
