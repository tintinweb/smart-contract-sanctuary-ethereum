...
error Staker__NeedsMoreThanZero();
error Staker__TransferFailed();
error Staker__NotEligibleForWithdraw();
error Staker__AlreadyExecuted();
error Staker__StakingNotAllowedNow();
error Staker__Completed();

contract Staker {
    mapping(address => uint256) public balances;
    uint256 public constant threshold = 1 ether;
    uint256 public deadline = block.timestamp + 84 hours;
    ...
    modifier moreThanZero() {
        if (msg.value == 0) {
            revert Staker__NeedsMoreThanZero();
        }
        _;
    }
    ...
    function stake() public payable moreThanZero notCompleted {
        if (block.timestamp >= deadline || openForWithdraw) {
            revert Staker__StakingNotAllowedNow();
        }
    ...
    function execute() public notCompleted {
        ...
        if (address(this).balance >= threshold) {
            ...
            // Maybe here need to check the block.timestamp
    ...
    function withdraw() public {
        if (!openForWithdraw) {
            revert Staker__NotEligibleForWithdraw();
        }
