 ...
 */
contract Choochoos is ERC721Royalty, Ownable {


  /* ============ Variables ============ */
  /// @notice Contract address of the premint NFT token
  address private constant _TARGET_CONTRACT = 0x91A252E0bC6082e6cA0Fc554968177BC5dbd53f1; // railroader testnet address for now
  ...
  /// @notice Max number of Presale NFTs that can be minted in a single transaction
  ...
  /// @notice Map to track number of premints per address
  mapping(address => uint256) internal mintTracker;
   ...
   */
  function preMint(uint256 _numberOfTokens) external payable {
    bool _premintIsActive = premintIsActive;

    require(_premintIsActive,"CCNFT/premint-inactive");

    require(IERC721(_TARGET_CONTRACT).balanceOf(msg.sender) > 0, "CCNFT/premint-pass-non-holder");
