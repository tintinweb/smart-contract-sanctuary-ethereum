...
contract TokenFarm is Ownable {
    ...
    struct User {
        bool isUsed;
        //key is Token address,key is Token mounts
        mapping(address => uint256) userStakeTokensMount;
    }
    ...
    function issueTokens() public onlyOwner {
        ...
        for (
            ...
            address _staker = stakers[stakersIndex];
            uint256 userTotalValue = getUserTotalValue(_staker);
            dappToken.transfer(_staker, userTotalValue);
        }
    }
    ...
    function getUserTotalValue(address _user) internal view returns (uint256) {
        ...
        for (
            ...
            address allowedToken = allowedTokens[allowedTokensIndex];
            totalValue += getUserSingleTokenValue(_user, allowedToken);
        }
        return totalValue;
    }
        ...
        returns (uint256)
    {
        ...
        uint256 _tokenAmount = stakingBalance[_token][_user];
        (uint256 _value, uint256 _decimals) = getTokenValue(_token);
        return (_tokenAmount * _value) / 10**_decimals;
    }
    ...
    {
        // priceFeedAddress
        address _priceFeedAddress = tokenPriceFeed[_token];
        AggregatorV3Interface _priceFeed = AggregatorV3Interface(
            _priceFeedAddress
        );
        int256 _price = getLatestPrice(_priceFeed);
        uint256 decimals = _priceFeed.decimals();
        return (uint256(_price), decimals);
    }
        ...
        returns (int256)
    {
        (
            ,
            /*uint80 roundID*/
            int256 _price, /*uint startedAt*/ /*uint timeStamp*/ /*uint80 answeredInRound*/
            ,
            ,
    ...
    function stakeTokens(uint256 _amount, address _token) public {
        ...
        require(tokenIsAllowed(_token), "The Token not allowed.");
    ...
    function deleteStaker(address _user) internal {
