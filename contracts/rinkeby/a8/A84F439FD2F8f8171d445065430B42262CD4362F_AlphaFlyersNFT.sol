...
contract AlphaFlyersNFT is ERC721URIStorage, Ownable, ReentrancyGuard {
    ...
    function devMint(uint256 quantity) external onlyOwner {
        for (uint256 i = 0; i < quantity; i++) {
            ...
            string memory combinedName =
                string(abi.encodePacked("Alpha Flyers #", currentTokenIds));
            
            string memory json = Base64.encode(
                bytes(
                    string(
                        abi.encodePacked(
                            '{"name": "', combinedName, '", "description": "Alpha Flyers is a group for NFT traders. The collection contains 333 flyer passes as the membership pass into the club. As a holder, you\xE2\x80\x99ll have access to our trading live stream where we demonstrate and discuss all the nuances of NFT trading, answer every question you might have about NFT trading directly in the voice channel. If you are feeling lazy, no problem, copy trade us in high-probability setups, and you\xE2\x80\x99ll see profit.", "animation_url": "https://ipfs.infura.io/ipfs/QmRZMUrd8boMNHvo5xRDHaahms3HYhWLaBrA9QS6oTvz4Q"}'
                        )
                    )
                )
            );
    ...
    function refundIfOver(uint256 currentTokenIds) private {
        require(currentTokenIds > maxIds, "whitelist sale has been ended");
            ...
            bytes(
                string(
                    abi.encodePacked(
                        '{"name": "Alpha Flyers #',
                        currentTokenIds,
                        '", "description": "Make money together, as long as you hold the pass, you can ask any question regarding trading.", "image": "https://bafybeigpih4g3s5a3eu47cizfxtenzdffcbqbjxs2iw2hymsfgklek4lwm.ipfs.infura-ipfs.io"}'
                    )
                )
            )
        );
