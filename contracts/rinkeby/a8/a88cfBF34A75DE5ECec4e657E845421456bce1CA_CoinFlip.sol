...
contract CoinFlip is GamesCore {
    ...
    */
    function play(address _token,uint256 _choice, uint256 _betAmount, bytes32 _seed) public payable uniqueSeed(_seed) {
		...
		if (msg.value > 0) {
			possiblePrize = msg.value * (200 - edge) / 100;
		...
		possiblePrize = _betAmount * (200 - edge) / 100;
        ...
        IERC20(_token).safeTransferFrom(msg.sender, bank, _betAmount);
