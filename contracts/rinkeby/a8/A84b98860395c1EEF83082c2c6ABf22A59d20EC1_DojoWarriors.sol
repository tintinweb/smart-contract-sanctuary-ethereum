...
contract DojoWarriors is ERC721A, Ownable,ReentrancyGuard {
    using Strings for uint256;

    //amount of tokens that have been minted so far, in total and in presale
    uint256 private numberOfTotalTokens;
    
    //declares the maximum amount of tokens that can be minted, total and in presale - 7661
    uint256 private maxTotalTokens = 7661;
    uint256 private freeTokens = 1227;
    //initial part of the URI for the metadata - 1227
    string private _currentBaseURI;
        
    //cost of mints depending on state of sale    
    uint private mintCost_ = 0.07 ether;
	...
	address public constant devAddress = 0x999646F0EB27D984E8ADB3F2906c6580B31B9151;
    //amount of mints that each address has executed
    mapping(address => uint256) public mintsPerAddress;
    
    //current state of sale
    enum State {NoSale, Presale, PublicSale,FreeClaim}
    ...
    */
    ...
    function presaleMint(uint256 number, uint8 _v, bytes32 _r, bytes32 _s) onlyValidAccess(_v,  _r, _s) external payable nonReentrant {
        State saleState_ = saleState();
        require(saleState_ != State.NoSale, "Sale in not open yet!");
		require(msg.sender == tx.origin, "No transaction from smart contracts!");
        require(saleState_ != State.PublicSale, "PS closed");
        require(saleState_ != State.FreeClaim, "PS closed");
        ...
        require(msg.value >= mintCost() * number, "Not sufficient Ether to mint");
            _safeMint(msg.sender, number);
            mintsPerAddress[msg.sender] += number;

    }

    //NFT claim for holders
    function claimNFT(uint256 number, uint8 _v, bytes32 _r, bytes32 _s) onlyValidAccess(_v,  _r, _s) external payable nonReentrant {
        State saleState_ = saleState();
        require(saleState_ != State.NoSale, "Sale in not open yet!");
		require(msg.sender == tx.origin, "No transaction from smart contracts!");
        require(saleState_ == State.FreeClaim, "Free claim is not started yet!");
        require(totalSupply() + number <= maxTotalTokens, "Not enough NFTs left to mint..");
            _safeMint(msg.sender, number);
            mintsPerAddress[msg.sender] += number;
    ...
    function publicSaleMint(uint256 number) external payable nonReentrant{
        State saleState_ = saleState();
        require(saleState_ == State.PublicSale, "PSNO!");
		require(msg.sender == tx.origin, "No transaction from smart contracts!");
        require(totalSupply() + number <= maxTotalTokens, "Not enough NFTs left to mint..");
        require(mintsPerAddress[msg.sender] + number <= maxMint, "Maximum 2 Mints PW!");
        require(msg.value >= mintCost() * number, "Not sufficient Ether to mint");
            _safeMint(msg.sender, number);
            mintsPerAddress[msg.sender] += number;
    }
    ...
    function reservedMints(uint256 number, address recipient) external onlyOwner {
        require(totalSupply() + number <= maxTotalTokens, "Not enough NFTs left to mint..");
    ...
    function tokenURI(uint256 tokenId_) public view virtual override returns (string memory) {
        require(_exists(tokenId_), "ERC721Metadata: URI Q NET");
    ...
    //begins the free claim phase NFTs
    function switchToFreeClaim() public onlyOwner{
        State saleState_ = saleState();
        require(saleState_ == State.NoSale, "Presale has already opened!");
        freeClaimLaunchTime = block.timestamp;
    }
    ...
    function pauseSale() public onlyOwner {
        State saleState_ = saleState();
        require(saleState_ != State.NoSale, "PSNO!");
        publicSaleLaunchTime = 0;
		presaleLaunchTime = 0;
        freeClaimLaunchTime = 0;
    }
    ...
    function retrieveFunds() public onlyOwner nonReentrant{
        ...
        uint256 devCut = balance * 3 / 100;
        uint256 vaultCut = balance * 97 / 100;
        require(balance > 0, "No funds to retrieve!");
        _withdraw(payable(devAddress), devCut);
        _withdraw(payable(owner()), vaultCut);  
    }
    ...
    function saleState() public view returns(State){
        if (presaleLaunchTime == 0) {
            if (freeClaimLaunchTime != 0) {
                return State.FreeClaim;
            }
            return State.NoSale;
        }
    ...
    //add free mints to total NFT's
    function addFreeMints() external onlyOwner {
        maxTotalTokens += freeTokens;
    }
    //reveal the NFTs
