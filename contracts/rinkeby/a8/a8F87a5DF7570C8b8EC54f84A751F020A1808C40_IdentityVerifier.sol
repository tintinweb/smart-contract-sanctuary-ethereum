...
contract IdentityVerifier is AdminControl, IIdentityVerifier {
    ...
    mapping(address => bool) public adHocAddresses;
    ...
    function setAllowList(bytes32 merkleRoot) public adminRequired {
        _merkleRoot = merkleRoot;
    }

    function addAdHocAddresses(address[] calldata addresses) public adminRequired {
        for (uint i; i < addresses.length; i++) {
            adHocAddresses[addresses[i]] = true;
        }
    }

    function removeAdHocAddresses(address[] calldata addresses) public adminRequired {
        for (uint i; i < addresses.length; i++) {
            adHocAddresses[addresses[i]] = false;
        }
    }
    ...
    function verify(uint40, address identity, address, uint256, uint24, uint256, address, bytes calldata data) external override view returns (bool) {
        bytes32[] memory proof = abi.decode(data, (bytes32[]));
        if (onAllowList(identity, proof)) return true;
        if (adHocAddresses[identity]) return true;
        return false;
    }
}
