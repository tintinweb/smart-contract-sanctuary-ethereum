...
contract SimpleVault {
  ...
  function borrow(uint256 _amount) external {
    ...
    require(calculateCollateralValueUSD(msg.sender) - debt[msg.sender] >= _amount, "Whoah there bud!");
  ...
  function getCollateralPriceUSD() public view returns (uint256) {
    uint oraclePrice = getTwap();
    uint ethPrice = getEthUsd();
    return oraclePrice.mul(ethPrice);
  }
