...
contract GuestlistedArt {
    ...
    // --------------------------------------------------------
    // 
    //  Draw the token image (SVG) from token drawData
    // 
    // --------------------------------------------------------
    function draw (GuestlistedLibrary.DrawData memory drawData) public view returns (string memory) {
        uint rotate = drawData.randomNumber % 360;
        uint patternSize = 150;
        uint dasharray = drawData.randomNumber % 3;
        uint scale;
        ...
        // 
        // --------------------------------------------------------
        if (drawData.shapeIndex == 0) {
            // circle
            rotate = 0;
            scale = 8;
        } else if (drawData.shapeIndex == 1) {
            // prism
            scale = drawData.randomNumber % 8 + 4;
        } else if (drawData.shapeIndex == 2) {
            // square
            scale = drawData.randomNumber % 4 + 4;
        } else if (drawData.shapeIndex == 3) {
            // cube
            scale = drawData.randomNumber % 4 + 3;
            patternSize = 210;
        } else if (drawData.shapeIndex == 4) {
            // line
            scale = drawData.randomNumber % 5 + 1;
            patternSize = 50;
        }
        ...
        string memory pattern = string(
            abi.encodePacked(
                '<pattern id="p-',
                GuestlistedLibrary.toString(drawData.tokenId),
        ...
        if (drawData.shapeIndex == 0) {
            // circle
            uint position = drawData.randomNumber % 70 + 30;
            uint radius = drawData.randomNumber % 50 + 20;
            string memory random = string(abi.encodePacked(GuestlistedLibrary.toString(drawData.randomNumber % 10), '.2'));
            ...
            // prism
            uint position = drawData.randomNumber % 100;
            ...
            // square
            uint size = drawData.randomNumber % 100 + 40;
            uint random = drawData.randomNumber % 50;
            ...
            // cube
            uint size = drawData.randomNumber % 100 + 50;
            uint position = drawData.randomNumber % 50 + 10;
        ...
        drawing = string(
            abi.encodePacked(
                ...
                GuestlistedLibrary.toString(drawData.dj.fontSize),
                '">',
