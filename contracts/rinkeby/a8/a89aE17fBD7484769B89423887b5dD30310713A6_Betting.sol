...
contract Betting is OwnableUpgradeable, UUPSUpgradeable {
    ...
    struct Bet {
        ...
        // Participant address
        address player;
        // Snapshot of total amount to calculate commision
    ...
    ERC20Upgradeable public usdcContract;
    ...
    event BetPlaced(uint256 indexed poolId, address indexed player, uint256 amount);
    ...
    function initialize(address usdcContract_) public initializer {
        ...
        usdcContract = ERC20Upgradeable(usdcContract_);
    }
    ...
    function createPool(uint256 numberOfTeams_, uint256 startTime_, uint256 duration_) external onlyOwner {
        uint256 poolId = pools.length;
        pools.push(Pool(poolId, numberOfTeams_, 0, 0, PoolStatus.Created, 0, startTime_, startTime_ + duration_));

        emit PoolCreated(poolId, numberOfTeams_, startTime_);
    }

    function createTeam(uint256 poolId_) external onlyOwner validPool(poolId_) {
    ...
    function cancelPool(uint256 poolId_) external onlyOwner validPool(poolId_) {
        Pool storage pool = pools[poolId_];
    ...
    function startPool(uint256 poolId_) external onlyOwner validPool(poolId_) {
        Pool storage pool = pools[poolId_];
        require(pool.status == PoolStatus.Created, "Betting: Pool status should be Created");
    ...
    function gradePool(uint256 poolId_, uint256 winnerId_) external onlyOwner validPool(poolId_) {
        Pool storage pool = pools[poolId_];
    ...
    function refundBet(uint256 poolId_, uint256 teamId_) external onlyOwner {
        // Remove player from pool and issue refund
    }

    function placeBet(uint256 poolId_, uint256 teamId_, uint256 amount_) external validPool(poolId_) payable {
        Pool storage pool = pools[poolId_];
        ...
        // usdcContract.transferFrom(msg.sender, address(this), amount_);

        uint256 betId = bets.length;
        address player = msg.sender;
        bets.push(Bet(betId, poolId_, teamId_, amount_, player, pool.totalAmount, block.timestamp));
        userBets[poolId_][player].push(betId);

        // Store bet amount against team id
        poolBalance[poolId_][teamId_] += amount_;

        // Update commission stats
        uint256 _commission = _calculateCommission(amount_);
        ...
        emit BetPlaced(poolId_, player, amount_);
    }
    ...
    function claimPayment(uint256 poolId_) external validPool(poolId_) {
        ...
        require(claimedPayouts[msg.sender][poolId_] == 0, "Betting: Payout already claimed"); 

        uint256 _winningAmount = _totalAmountWon(msg.sender, poolId_);
        
        claimedPayouts[msg.sender][poolId_] = _winningAmount;
        payable(msg.sender).transfer(_winningAmount);
        // usdcContract.transferFrom(address(this), msg.sender, _winningAmount);

        emit WinningsClaimed(poolId_, msg.sender, _winningAmount);
    }
    ...
    function claimRefund(uint256 poolId_) external validPool(poolId_) {
        ...
        require(claimedRefunds[msg.sender][poolId_] == 0, "Betting: Refund already claimed"); 

        uint256 _refundAmount = _totalAmountRefunded(msg.sender, poolId_);
        
        claimedRefunds[msg.sender][poolId_] = _refundAmount;
        payable(msg.sender).transfer(_refundAmount);
        // usdcContract.transferFrom(address(this), msg.sender, _refundAmount);

        emit RefundClaimed(poolId_, msg.sender, _refundAmount);
    }

    function claimCommission(uint256 poolId_, uint256 total) external validPool(poolId_) {
        Pool storage pool = pools[poolId_];
        require(pool.status == PoolStatus.Decided, "Betting: Pool status should be Deciced");
        require(claimedCommissions[msg.sender][poolId_] == 0, "Betting: Commission already claimed");

        uint256 _commissionAmount = _totalCommissionGenerated(msg.sender, poolId_, total);
        claimedCommissions[msg.sender][poolId_] = _commissionAmount;
        payable(msg.sender).transfer(_commissionAmount);

        emit CommissionClaimed(poolId_, msg.sender, _commissionAmount);
    }
    ...
    function viewPoolDistribution(uint256 poolId_) external view returns (uint256[] memory _betAmounts){
        ...
        for (uint256 i = 0; i < pool.numberOfTeams; i++) {
            _betAmounts[i] = poolBalance[poolId_][i];
        }
    ...
    function totalPayouts(uint256 poolId_) external view returns(uint256) {
        return _totalAmountWon(msg.sender, poolId_);
    }

    function totalPayoutsClaimable(uint256 poolId_) external view returns(uint256) {
        return _totalAmountWon(msg.sender, poolId_) - claimedPayouts[msg.sender][poolId_];
    }

    function totalPayoutsClaimed(uint256 poolId_) external view returns(uint256) {
        return claimedPayouts[msg.sender][poolId_];
    }

    function totalRefunds(uint256 poolId_) external view returns(uint256) {
        return _totalAmountRefunded(msg.sender, poolId_);
    }

    function totalRefundClaimable(uint256 poolId_) external view returns(uint256) {
        return _totalAmountRefunded(msg.sender, poolId_) - claimedPayouts[msg.sender][poolId_];
    }

    function totalRefundClaimed(uint256 poolId_) external view returns(uint256) {
        return claimedRefunds[msg.sender][poolId_];
    }
    ...
    function _totalAmountWon(address who_, uint256 poolId_) internal view returns(uint256) {
        ...
        uint256 odds = _calculateOdds(poolId_, pool.winnerId);

        console.log("Odds: %s", odds);
        for (uint256 i = 0; i < _userBets.length; i++) {
            // Get bet reference
            Bet memory bet = bets[_userBets[i]];
            
            // Check if bet against winning player
            if (bet.teamId == pool.winnerId) {
                // Calculate winning amount
                _winningAmount += (odds * bet.amount) / SCALING_FACTOR + bet.amount;
            }
        } 
    ...
    function _totalAmountRefunded(address who_, uint256 poolId_) internal view returns(uint256) {
        ...
        for (uint256 i = 0; i < _userBets.length; i++) {
            // Get bet reference
            Bet memory bet = bets[_userBets[i]];
            _refundedAmount += bet.amount;
        } 

        return _refundedAmount;
    }

    function _totalCommissionGenerated(address who_, uint256 poolId_, uint256 total) internal view returns(uint256) {
        Pool memory pool = pools[poolId_];
        // Mark all payments by user in this pool as available for refund
        uint256[] memory _userBets = userBets[poolId_][who_];
        ...
        for (uint256 i = _userBets[0]; i < pool.totalBets; i++) {
            // Get commission reference
            Commission memory commission = poolCommission[poolId_][i];
            if (commission.player == who_) {
                numerator = numerator + commission.amount;
            }

            denominator = denominator + commission.totalAmount;
        } 
    ...
    function _calculateOdds(uint256 poolId_, uint256 teamId_) internal view returns (uint256) {
        Pool memory pool = pools[poolId_];
        uint256 _teamBalance = poolBalance[poolId_][teamId_];
        if (_teamBalance == 0) {
            return 0;
        }
    ...
    function _calculateCommission(uint256 amount_) internal pure returns (uint256) {
        return amount_ * COMMISION * SCALING_FACTOR / BPS_UNIT;
    }
}
