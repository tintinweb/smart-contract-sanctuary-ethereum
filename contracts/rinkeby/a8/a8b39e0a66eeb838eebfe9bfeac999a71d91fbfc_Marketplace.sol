...
contract OdysseyXp is ERC20 {
    ...
    mapping(address => uint256) public isExchange;
    address public owner;
    ...
    function notRouter() internal view returns (bool) {
        if (isMinter[msg.sender] == 1) return false;
        return msg.sender != router;
    }

    function notExchange() internal view returns (bool) {
        if (isExchange[msg.sender] == 1) return false;
...
contract Marketplace {
    ...
    constructor(
        ...
        treasury = treasury_;
        admin = msg.sender;
        treasryCommission = 200; //2% default
        if (xp_ == address(0)) {
    ...
    function setTreasuryCommission(uint256 points) external {
        if (msg.sender != admin) {
            revert OdysseyMarket_NoAccess();
        }
        treasryCommission = points;
    }
    ...
    function _takePayment(
        ...
        uint256 commission = (amount * treasryCommission) / 10000;
        ...
        if (currency == address(0)) {
            ...
            for (uint256 n; n < sRoyalties.length; ++n) {
                ...
                amount -= sAmount;
                (success, ) = sRoyalties[n].receiver.call{value: sAmount}("");
                if (!success) {
                    revert OdysseyMarket_FailedToPayEther();
                }
            }
