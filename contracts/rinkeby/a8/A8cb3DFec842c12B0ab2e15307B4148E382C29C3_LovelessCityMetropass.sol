 ...
 */
contract LovelessCityMetropass is
    Context,
    VRFConsumerBaseV2,
    ERC721,
    ERC721Enumerable,
    ...
    ReentrancyGuard
{
    ...
    bool provenanceHashLocked = false;
    ...
    address public constant TEAM_WALLET =
        0x811abB18A2BA64376AcfDa085ce732B871045515;

    // /// Royalty info
    // address public royaltyAddress;
    address public artist;
    // uint256 private ROYALTY_SIZE = 500;
    ...
    {
        //  _baseURI = "ipfs://QmQ5rX2TCtdwPkTm6Lt65mfeimrCddNKYpH2prDLykNirU/";
        COORDINATOR = VRFCoordinatorV2Interface(vrfCoordinator);
        LINKTOKEN = LinkTokenInterface(link);
        s_owner = msg.sender;
        s_subscriptionId = subscriptionId;
        // royaltyAddress = _artistAddress;
        artist = _artist;
        setRoyaltyInfo(_artist, 250);
    ...
    function publicMint(uint256 tokensNumber) public payable {
    ...
    function setRoyaltyInfo(address _artist, uint96 _royaltyFeeInBips)
        public
        onlyOwner
    {
        _setDefaultRoyalty(_artist, _royaltyFeeInBips);
    }

    function updateSaleStatus(bool status) public onlyOwner {
        require(startingIndexSet, "Random startingIndex not set");
        saleIsActive = status;
    }
    ...
    function fulfillRandomWords(
        ...
        startingIndex = (randomWords[0] % MAX_SUPPLY);
        ...
        )
    {
        ...
        return
            string(
                abi.encodePacked(
                    _metropassBaseURI,
                    ((tokenId + startingIndex) % MAX_SUPPLY).toString(),
                    ".json"
                )
            );
    }
    ...
    function teamMint() public onlyOwner {
        require(startingIndexSet, "wait for random starting index");
        require(
            totalSupply() + TEAM_TOKENS <= MAX_SUPPLY,
            "You tried to mint more than the max allowed"
        );
    ...
    // function provenanceHashLocked() public returns (bool) {
    //     if (bytes(PROVENANCE_HASH).length == 0) return false;
    //     return true;
    // }
    ...
    {
        require(provenanceHashLocked, "Lock provenanceHash first");
    ...
    //     token.transferFrom(from, artist, royaltyInfo());
    // }
}
