    ...
    MerkleMintable
{
    ...
    uint256 public maxReserve = 5; // 1 per team member
    ...
    constructor() ERC721A("Almost Fancy Founders Pass", "AFFC") {}

    modifier requireState(SaleState state) {
        require(saleState == state, "AFFC: Sale is not active");
        _;
    }

    modifier callerIsReal() {
        // solhint-disable-next-line avoid-tx-origin
        require(tx.origin == msg.sender, "AFFC: We only like real users!");
        _;
    }
    ...
    modifier callerIsAF() {
        ...
        require(
            ...
            "AFFC: Caller can only be AlmostFancy"
        );
        // solhint-enable
        _;
    }
    ...
    function _doMint(address minter) internal {
        ...
        require(
            ...
            "AFFC: Almost Fancy Founders Passes have sold out!"
        );
        // solhint-disable-next-line reason-string
        require(
            balanceOf(minter) == 0,
            "AFFC: You already own an Almost Fancy founders pass!"
        );

        _merkleMint(minter, 1);
        _safeMint(minter, 1);
    ...
    function consumePass(uint256 pass) external callerIsAF {
        // solhint-disable-next-line reason-string
        require(!usedPasses[pass], "AFFC: This pass has already been used!");
    ...
    function reserveTokens(uint256 quantity) external onlyOwner {
        // solhint-disable-next-line reason-string
        require(maxReserve - quantity > 0, "AFFC: Exceeds max reserve amount!");
    ...
    function refundIfOver(uint256 cost) internal {
        // solhint-disable-next-line reason-string
        require(msg.value >= cost, "AFFC: ETH value sent was too low");
        if (msg.value > cost) {
...
contract MerkleMintable {
    ...
    modifier canMint(address who, uint256 toMint, uint256 maxMint) {
        // solhint-disable-next-line reason-string
        require(amountMinted[who] + toMint <= maxMint, "MerkleMintable: You can't mint any more tokens!");
        _;
    }
    ...
    modifier isAbleToMint(address who, bytes32[] memory proof) {
        // solhint-disable-next-line reason-string
        require(isMerkleVerified(who, proof), "MerkleMintable: You are not on the merkle list");
        _;
    }
    ...
    function isMerkleVerified(address claimer, bytes32[] memory proof) public view returns (bool) {
