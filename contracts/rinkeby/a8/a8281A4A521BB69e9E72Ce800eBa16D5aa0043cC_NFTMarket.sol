...
contract NFTMarket is Ownable, ReentrancyGuard {
    ...
    function createBatchMarketItem(address nftContract,
        uint256[] memory tokenId,
        uint256[] memory price,
        uint256[] memory amount) public {

    require(tokenId.length ==  price.length,"Array length are not same");
    require(price.length ==  amount.length,"Array length is not same ");

    for(uint i = 0 ; i < tokenId.length; i++){
        createMarketItem(nftContract,tokenId[i],price[i],amount[i]);
    }
    }
