...
contract WhatIsYourDate is ERC721AQueryable, Ownable, ReentrancyGuard {
  ...
  event DatesMinted(address owner, bytes8[] dates, uint256 currentIndex, uint256 mintAmount);
  event RandomDatesMinted(address owner, uint256 currentIndex, uint256 mintAmount);

  string constant public TokenName = "SaveTheDate";
  string constant public TokenSymbol = "STD";
  ...
  function whitelistMint(uint256 _mintAmount, bytes8[] memory _dates, bytes32[] calldata _merkleProof) public payable mintCompliance(_mintAmount) mintPriceCompliance(_mintAmount) {
  ...
  function mint(uint256 _mintAmount, bytes8[] memory _dates) public payable mintCompliance(_mintAmount) mintPriceCompliance(_mintAmount) {
  ...
  function mintDates(address to, uint256 _mintAmount, bytes8[] memory _dates) internal {
 ...
 function internalMint(uint256 _teamAmount, bytes8[] memory _dates) external onlyOwner  {
    require(totalSupply() + _teamAmount <= maxSupply, 'Max supply exceeded!');
    mintDates(_msgSender(), _teamAmount, _dates);
  }
   
  function mintForAddress(uint256 _mintAmount, bytes8[] memory _dates, address _receiver) public mintCompliance(_mintAmount) onlyOwner {
