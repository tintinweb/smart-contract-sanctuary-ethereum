...
contract RoyaltyFeeManager is IRoyaltyFeeManager, Ownable {
    ...
    constructor(address _royaltyFeeRegistry) {
        royaltyFeeRegistry = IRoyaltyRegistry(_royaltyFeeRegistry);
    }
    ...
    function calculateRoyaltyFeeAndGetRecipient(
        ...
        (address receiver, uint256 royaltyAmount) = (address(0), 0);

        // 1. Check collection itself
        if (IERC165(collection).supportsInterface(INTERFACE_ID_ERC2981)) {
            ...
            address lookup = royaltyFeeRegistry.getRoyaltyLookupAddress(collection);
            if (IERC165(lookup).supportsInterface(INTERFACE_ID_ERC2981)) {
                (receiver, royaltyAmount) = IERC2981(lookup).royaltyInfo(tokenId, amount);
            }
        }
