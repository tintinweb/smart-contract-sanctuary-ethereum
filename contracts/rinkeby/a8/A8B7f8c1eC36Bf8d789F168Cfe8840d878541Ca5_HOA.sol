...
contract HOA is ERC721Enumerable, Ownable {
    ...
    constructor(string memory baseURI) ERC721("House of Assets", "HOA"){
        setBaseURI(baseURI);
    }
    ...
    function isStatus() public view returns (uint256) {
      if(block.timestamp < 1654441060) {
          return 0; // Before Mint
      } else if(block.timestamp >= 1654441060 && block.timestamp < 1654441060 + 5 minutes) {
    ...
    function whitelistMint(uint256 _tokenAmount, bytes32[] memory _merkleProof) public payable {
