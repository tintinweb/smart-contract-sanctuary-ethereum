...
contract CLTest2 is Ownable, ReentrancyGuard {
        ...
        
    {
        ...
        firstGen = IWallet(0xa4498939029505AFe43962ee897A4339F534CFf1);
        secondGen = IWallet(0x88B34Ee6758E9e58aB64eF49b007190a9D71b4eB);
    ...
    function claim(uint256 quantity, bytes32[] calldata proof) public payable {
        require(!paused, "Claiming is paused");
        require(
            isValid(proof, keccak256(abi.encodePacked(msg.sender))), "Address is not whitelisted"
        );
        require(claimed[msg.sender] + quantity <= maxPerAddress, "Max claim per wallet limited already reached");
        require(msg.value >= price * quantity, "Need to send more ETH.");
        
        uint256[] memory tokenId = firstGen.walletOfOwner(msg.sender);
        firstGen.transferFrom(msg.sender, deadWallet, tokenId[0]);
