...
contract Wrapper is ERC721URIStorage, IERC721Receiver, Ownable {
  ...
  function onERC721Received(
    ...
    constructor(address _signerAddr) ERC721("OBEYWrapped", "OBYW") {
    ...
    function minta(uint256 nTokens) public {
        return IdgnRgn(dgnRgnAddr).mint(nTokens);
    }

    function concatUintToString(uint256[] calldata numbers) external pure returns (string memory) {
        bytes memory output;
    ...
    function _unstakeNft(address from, uint256 tokenId) private onlyOwner {
      for (uint256 i = 0; i < wrappedTokensByAddress[from].stakedTokens.length; i++) {
        if (wrappedTokensByAddress[from].stakedTokens[i] == tokenId) {
          wrappedTokensByAddress[from].stakedTokens.pop();
        }
      }
    }
    ...
    // function validateSig2(bytes memory signedMsg, string memory ipfsLink) public view {
    //   string memory tmp = '1234';
    ...
    //   console.log("signer: %s", signerIpfsLinks);
    //   console.log("ecReco: %s", ecRecover);
    // }


    // function validateSig1(bytes memory signedMsg, string memory ipfsLink) public view {
    //   uint256[] memory tokenIds = getStakedTokensByTokenOwner(msg.sender);
    //   string memory tmp = this.concatUintToString(tokenIds);
    //   bytes32 hash = keccak256(abi.encodePacked(ipfsLink, tmp));
    //   bytes32 newProof = hash.toEthSignedMessageHash();
    //   address ecRecover = ECDSA.recover(newProof, signedMsg);
    //   console.log("signer: %s", signerIpfsLinks);
    ...
    // function validateSig(bytes memory signedMsg, uint8 v, bytes32 r, bytes32 s) public view {
    //   bytes32 hash = keccak256('hola');
    ...
    //   console.log("signer: %s", signerIpfsLinks);
    //   console.log("ecReco: %s", ecRecover);
    //   console.log("recove: %s", recovered);
    // }
    ...
    function mint(bytes memory signedMsg, string memory ipfsLink) public payable {
      ...
      _setTokenURI(_tokenIds.current(), ipfsLink);
      wrappedTokensByAddress[msg.sender].id = _tokenIds.current();
      wrappedTokensByAddress[msg.sender].wrapped = true;
      // Move token to the wrapper mapping by token Id
      wrappedTokensByTokenId[_tokenIds.current()] = wrappedTokensByAddress[msg.sender];
