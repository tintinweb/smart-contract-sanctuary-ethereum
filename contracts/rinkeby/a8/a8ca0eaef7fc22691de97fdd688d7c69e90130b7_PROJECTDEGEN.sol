...
contract PROJECTDEGEN is ERC721A, EIP712, Ownable{
    ...
    constructor() 
    ERC721A("PROJECTDEGEN", "DEGEN1")
    EIP712("PROJECT DEGEN", "1.0.0"){}

    function mint(bytes calldata _signature, uint256 _nftIndex, uint256 _quantity) external payable {
      require(_totalSupply + _quantity < MAX_SUPPLY, "NFT: Cannot mint more than maximum supply!");
      if(msg.value == 0){
  ...
  require(
    ...
    function withdraw() external payable onlyOwner {
      ...
      modifier mintCompliance() {
        require(publicSale, "Sale is not active yet.");
