...
import "contracts/erc721a/ERC721A.sol";
import "@openzeppelin/contracts/access/Ownable.sol";
import "@rari-capital/solmate/src/utils/ReentrancyGuard.sol";

// --------------------------------------------------------------
// INTERFACES TO CHECK OWNERSHIP
// --------------------------------------------------------------
interface IBalanceOf {
    function balanceOf(address owner) external view returns (uint256);
}

contract SkankinDAO is ERC721A, Ownable, ReentrancyGuard {
    // --------------------------------------------------------------
    // STORAGE
    // --------------------------------------------------------------
    ...
    // Mapping from token ID to dark/light bit
    // An empty struct value does not necessarily mean the token is unowned. See _ownershipOf implementation for details.
    mapping(uint256 => bool) internal _darkModes;

    // in order to mint, want to validate that they own one of the tokens from a required contract
    address public validationAddr;
    ...
    event DarkModeSet(uint256 tokenID, bool value);
    event ValidationAddressChanged(address validationAddress);
    ...
    function getBalanceOfSender() public view returns (uint) {
        return IBalanceOf(validationAddr).balanceOf(_msgSender());
    }
    ...
    function mint(uint256 quantity) public payable {
        ...
        require (getBalanceOfSender() > 0, string("The minting wallet does not include any required tokens to mint."));
    ...
    function setDarkModeOf(uint256 _tokenID, bool _darkModeNewVal) public {
        if (_tokenID > totalSupply()) revert NftIDOutOfRange();
        if (!_exists(_tokenID)) revert URIQueryForNonexistentToken();
        if (ERC721A.ownerOf(_tokenID) != _msgSender()) revert OnlyOwnerOfTokenCanSetDarkMode();
        bool darkModeOldVal = false;
        darkModeOldVal = _darkModes[_tokenID];
        require (darkModeOldVal != _darkModeNewVal, string("Dark Mode is already set to the same value sent."));
        require (balanceOf(_msgSender()) > 2, string("Must hold 3 or more tokens to switch to dark mode"));
        _darkModes[_tokenID] = _darkModeNewVal;
        emit DarkModeSet(_tokenID, _darkModeNewVal);
    }
    ...
    function setValidationAddress(address _validationAddress) public onlyOwner {
       validationAddr = _validationAddress;
       emit ValidationAddressChanged(_validationAddress);
    }
    ...
    function darkModeOf(uint256 _tokenID) public view returns (bool) {
        if (_tokenID > totalSupply()) revert NftIDOutOfRange();
        if (!_exists(_tokenID)) revert URIQueryForNonexistentToken();
        return bool(_darkModes[_tokenID]);
    }
    ...
    function tokenURI(uint256 _tokenID) public view override returns (string memory) {
        if (_tokenID > totalSupply()) revert NftIDOutOfRange();
        if (!_exists(_tokenID)) revert URIQueryForNonexistentToken();
        string memory darkOrLight = darkModeOf(_tokenID) ? "dark/" : "light/";
        if (bytes(baseURI).length == 0) {
            return presaleURI;
        } else {
            return string(abi.encodePacked(baseURI, darkOrLight, toString(_tokenID), uriExtension));
        }
    }
