...
contract xJoystickStaking is Authorizable, ReentrancyGuard {
    ...
    mapping(address => bool) public presaleExistence;
    ...
    constructor(
        ...
        uint256 _userDepFee,
        uint256[] memory _poolStart,
        uint256 _poolEpochCount,
        uint256[] memory _percentLockBonusReward
    ) {
        govToken = _govToken;
        lpToken = _lpToken;
        REWARD_PER_EPOCH = _rewardPerEpoch;

        epochLengthUpdate(7 days);
        POOL_START = _poolStart;
        POOL_EPOCH_COUNT = _poolEpochCount;
        userDepFee = _userDepFee;
        PERCENT_LOCK_BONUS_REWARD = _percentLockBonusReward;

        REWARD_MULTIPLIER = [256, 128, 96, 64, 56, 48, 40, 32, 28, 24, 20, 16, 15, 13, 12, 11, 10, 9];
        for (uint256 j = 19; j < 50; j++) {
            REWARD_MULTIPLIER.push(8);
        }

        for (uint256 j = 50; j < 180; j++) {
            REWARD_MULTIPLIER.push(4);
        }

        initStartTimestamp(_startTimestamp);

        FINISH_BONUS_AT_TIMESTAMP = EPOCH_LENGTH
                .mul(REWARD_MULTIPLIER.length - 1)
                .add(START_TIMESTAMP[0]);
    ...
    // Fetch the user information and stake from the presale contract
    function addPresale(address _presale) public onlyAuthorized {
        require(
            presaleExistence[_presale] == false,
            "xJoyStaking::addPresale: duplicated"
        );

        presaleList.push(JoystickPresale(_presale));
        presaleExistence[_presale] = true;
    }
    ...
    function getMultiplier(uint256 _from, uint256 _to) public view returns (uint256) {
        ...
        for (uint256 j = 0; j < 6; j++) {
            for (uint256 i = 0; i < EPOCH_LIST.length; i++) {
                ...
                if (j == 0) endEpoch = type(uint128).max;
                if (i > REWARD_MULTIPLIER.length-1) return 0;
                

                if (_to <= endEpoch) {
                    uint256 m = _to.sub(_from).div(EPOCH_LENGTH).mul(REWARD_MULTIPLIER[index]);
                    return result.add(m);
                }

                if (_from < endEpoch) {
                    uint256 m = endEpoch.sub(_from).div(EPOCH_LENGTH).mul(REWARD_MULTIPLIER[index]);  // convert by epoch unit, and multiply
                    _from = endEpoch;
                    result = result.add(m);
                }
                index++;
    ...
    function getLockPercentage(uint256 _from, uint256 _to) public view returns (uint256) {
        ...
        for (uint256 j = 0; j < 6; j++) {
            for (uint256 i = 0; i < EPOCH_LIST.length; i++) {
                ...
                if (j == 0) endTimestamp = type(uint128).max;
    ...
    function getReward(uint256 _from, uint256 _to) public view returns (uint256) {
        ...
        uint256 amount = multiplier.mul(REWARD_PER_EPOCH);
    ...
    function pendingReward(address _user) external view returns (uint256) {
        ...
        return user.amount.mul(accGovTokenPerShare_).div(1e12).sub(user.rewardDebt);
    }

    function claimReward() public {
        UserInfo storage user = userInfo[msg.sender];
        if (user.amount == 0) {
            _deposit(msg.sender, lpToken.balanceOf(msg.sender), address(0));
            return;
        }
    ...
    function getGlobalAmount(address _user) public view returns (uint256) {
        UserInfo storage current = userInfo[_user];
    ...
    function _deposit(address holder, uint256 _amount, address _ref) public nonReentrant {
        require(
            _amount > 0,
        ...
        UserInfo storage user = userInfo[holder];
        UserInfo storage refer = userInfo[_ref];

        if (refer.referrals[holder] > 0) {
            refer.referrals[holder] = refer.referrals[holder] + _amount;
    ...
    function deposit(uint256 _amount, address _ref) public nonReentrant {
        _deposit(msg.sender, _amount, _ref);
    }

    // Withdraw LP tokens from JDaoStaking.
    function withdraw(uint256 _amount, address _ref) public nonReentrant {
        UserInfo storage user = userInfo[msg.sender];
        UserInfo storage refer = userInfo[_ref];
        require(user.amount >= _amount, "xJoyStaking::withdraw: not good");
        if (_ref != address(0)) {           // in case of agent
            refer.referrals[msg.sender] = refer.referrals[msg.sender] - _amount;
            refer.globalRefAmount = refer.globalRefAmount - _amount;
        }
        user.globalAmount = user.globalAmount - _amount;
    ...
    function emergencyWithdraw() public nonReentrant {
        ...
        user.amount = 0;
        user.rewardDebt = 0;
        lpToken.safeTransfer(address(msg.sender), amountToSend);
    ...
    function getNewRewardPerEpoch() public view returns (uint256) {
        ...
        return multiplier.mul(REWARD_PER_EPOCH);
    }
    ...
    function reviseWithdraw(address _user, uint256 _timestamp) public onlyAuthorized() {
        UserInfo storage user = userInfo[_user];
        user.lastWithdrawTimestamp = _timestamp;
    }
    ...
    function lock(address _holder, uint256 _amount) public onlyAuthorized {
        ...
        govToken.transferFrom(_holder, address(this), _amount); // to be reviewed
