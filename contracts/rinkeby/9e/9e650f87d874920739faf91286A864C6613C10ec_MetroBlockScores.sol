...
contract MetroBlockScores is Ownable {
    ...
    function setBlocked(uint256[] calldata tokenIds) external onlyOwner {
       for (uint256 i; i < tokenIds.length; i++) {
        blocked[tokenIds[i]] = true;
       }
    }

    function setUnBlocked(uint256[] calldata tokenIds) external onlyOwner {
       for (uint256 i; i < tokenIds.length; i++) {
    ...
    function _getSpreadedBoost(uint256 hoodSize, uint256 stackedBoost) internal view returns (uint256) {
      ...
      if (spreadedBoost >= 3000) {
        spreadedBoost = 1750;
      } else if (spreadedBoost >= 2500) {
        spreadedBoost = 1625;
      } else if (spreadedBoost >= 2000) {
        spreadedBoost = 1500;
      } else if (spreadedBoost >= 1500) {
        spreadedBoost = 1250;
