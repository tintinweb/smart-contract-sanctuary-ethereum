...
contract DegePepe is ERC721A, Ownable {
    ...
    uint256 public batchLimit = 1;
    uint256 public price = 0.008 ether;
    mapping(address => uint256) public numPepes;

    constructor() ERC721A("Dege Pepe", "DGP") {}

    function mintPepeFree() external {
        uint256 currentPepe = totalSupply();
        require(publicSale, "Public sale not active");
        require(currentPepe < 10, "Free pepe supply has exceeded, please consider minting non-free pepe");
        require(numPepes[msg.sender] < batchLimit, "You can free mint max 1 DGP per wallet");
    ...
    function mintPepe() external payable {
        uint256 currentPepe = totalSupply();
        require(publicSale, "Public sale not active");
        require(currentPepe > 10, "Free pepe supply has not exceeded yet");
