...
abstract contract ERC721Marketplace is ERC721A, Ownable {
    ...
    event Loan(address indexed _from, address indexed to, uint256 _value);
    event LoanRetrieved(
    ...
    function buyToken(
        ...
        emit SaleCompleted(tokenId, currentPrice, seller, _msgSender(), hash);
...
contract ERC721ARandomizedCollection is ERC721Marketplace {
    ...
    // This should be enabled if owners want to provide more randomization on reveal,
    // but it will shift tokenId to tokenURI values.
    bool private shouldOffset;
    /*
    ...
    event TokensRevealed(string tokenDirectory, bool locked);
    ...
    function _init(
        ...
        if (_notSequentialMint) {
            emit TokensRevealed(tokenDirectory, false);
        }
    }

    /**
     * @dev returns if this collection already has the final URI of it's tokens
     * locked and revealed or not
     */
    ...
    function lockTokenURI(
        ...
        emit TokensRevealed(revealTokenDirectory, lockOnReveal);
    }
    ...
    function mintAllow(
        ...
        require(buyAmount <= amount, "Invalid buyAmount");
