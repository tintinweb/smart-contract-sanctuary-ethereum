...
contract PublicUnlockedVestingMock is KeeperCompatibleInterface, ReentrancyGuard {
  /*==================================================== State Variables ====================================================*/
  ...
  function checkUpkeep(
    ...
    address[] memory _users = vesting.getUsers(5);
    bool outBreak = false;
    for (uint256 i = 0; i < _users.length; i++) {
      uint256[] memory _indexes = vesting.getInvestorIndexes(_users[i], 5);
      for (uint256 k = 0; k < _indexes.length; k++) {
        uint256 withdrawableAmount = vesting.withdrawableTokens(_users[i],vesting.intToEnum(5),_indexes[k]);
  ...
  function performUpkeep(bytes calldata performData) external override nonReentrant {
    ...
    vesting.withdrawTokensAutomatically(_user, vesting.intToEnum(5), _index);
  }
}
