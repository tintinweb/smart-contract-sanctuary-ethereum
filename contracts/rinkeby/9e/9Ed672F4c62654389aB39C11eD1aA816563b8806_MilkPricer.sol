...
contract MilkPricer {
    ...
    constructor(
        ...
        require(
            _oracle != address(0),
            "MilkPricer: Cannot set 0 address as oracle"
        );
        require(
            _aggregator != address(0),
            "MilkPricer: Cannot set 0 address as aggregator"
        );
     ...
     */
    function setExpiryPriceInOracle(uint256 _expiryTimestamp, uint256 price)
        external
        onlyBot
    {
    ...
    function _scaleToBase(uint256 _price) internal view returns (uint256) {
        if (aggregatorDecimals > BASE) {
            uint256 exp = aggregatorDecimals - BASE;
            _price = _price / (10**exp);
        } else if (aggregatorDecimals < BASE) {
            uint256 exp = BASE - aggregatorDecimals;
            _price = _price * (10**exp);
        }
        return _price;
    }
