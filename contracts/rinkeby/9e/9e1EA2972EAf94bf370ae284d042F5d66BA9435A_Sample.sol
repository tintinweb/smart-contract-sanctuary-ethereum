...
contract Sample is Ownable, ERC721A, ReentrancyGuard {
    ...
    // allowList mint
    function getAllowListPrice(uint256 offset) public view returns(uint256){
        if (1001 - amountForAllowList - offset < 222){
            return 0.000000 ether;
        }
        return 0.200000 ether;
    }
    function getAllowListPriceTotal(uint256 quantity) public view returns(uint256){
        uint256 totalPrice = 0;
        for(uint256 i = 0;i < quantity;i++){
            totalPrice += getAllowListPrice(i);
        }
        return totalPrice;
    }
    // default false
    bool public allowListStatus = false;
    uint256 public amountForAllowList = 1001;
    ...
    function allowListMint(uint256 quantity, bytes32[] memory proof) external payable {
        ...
        refundIfOver(getAllowListPriceTotal(quantity));
    }
    ...
    function getPuiblicSalePrice(uint256 offset) public view returns(uint256){
        if (1042 - amountForAllowList - offset < 233){
            return 0.000000 ether;
        }
        return 0.200000 ether;
    }
    ...
    uint256 public amountForPublicSale = 1042;
    ...
    // auction
    uint256 public amountForAuction = 0;
    uint256 public constant AUCTION_START_PRICE = 0.000000 ether;
    uint256 public constant AUCTION_END_PRICE = 0.000000 ether;
    uint256 public constant AUCTION_PRICE_CURVE_LENGTH = 12 minutes;
    uint256 public constant AUCTION_DROP_INTERVAL = 12 minutes;
    uint256 public constant AUCTION_DROP_PER_STEP =
    (AUCTION_START_PRICE - AUCTION_END_PRICE) /
    (AUCTION_PRICE_CURVE_LENGTH / AUCTION_DROP_INTERVAL);
    uint256 public auctionSaleStartTime = 0;
    // per mint auction limitation
    uint256 public immutable auctionPerMint = 0;


    function getAuctionPrice() public view returns (uint256) {
        if (block.timestamp < auctionSaleStartTime || auctionSaleStartTime == 0) {
