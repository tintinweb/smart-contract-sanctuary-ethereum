...
contract YakuzaKumiaiSale is AccessControl {
    ...
    uint256 constant public preSalesCount = 500;
    
    uint256 public maxPreSaleMintPerWallet = 1;
    uint256 constant public maxPublicSaleMintPerWallet = 25;

    // uint256 constant public preSaleStart = 1648911600; // 4.3
    // uint256 constant public updatePreSaleMintCountPerWallet = 1649084400; // 4.5
    // uint256 constant public preSaleEnd = 1649257200;   // 4.7
    // uint256 constant public publicSaleStart = 1649516400; // 4.10
    // uint256 constant public updateAvaiableMerkleWL = 164887200; // 10 pm, 4.2

    uint256 constant public preSaleStart = 1648742400; // 4.3
    uint256 constant public updatePreSaleMintCountPerWallet = 1649778400; // 4.5
    uint256 constant public preSaleEnd = 1649257200;   // 4.7
    uint256 constant public publicSaleStart = 1649516400; // 4.10
    uint256 constant public updateAvaiableMerkleWL = 164887200; // 10 pm, 4.2

    uint256 constant public preSaleMintFee = 0.02 ether;
    ...
    bytes32 public merkleRootWL = 0xccc9a55b88a76ed0697663715202ff28dedd214735153cca78262518c82589e8;

    address payable public wallet1 = payable(0x7b62AC97F6a9Fb98Ec7F48EAA109014C1B685A95);   // wallet address for marketing
    address payable public wallet2 = payable(0x7b62AC97F6a9Fb98Ec7F48EAA109014C1B685A95);   // wallet address for giveaway
    address payable public wallet3 = payable(0x7b62AC97F6a9Fb98Ec7F48EAA109014C1B685A95);   // wallet address for tool development
    address payable public wallet4 = payable(0x7b62AC97F6a9Fb98Ec7F48EAA109014C1B685A95);   // wallet address for anime studio
    address payable public wallet5 = payable(0x7b62AC97F6a9Fb98Ec7F48EAA109014C1B685A95);   // wallet address for dev team
    address payable public wallet6 = payable(0x2C12c9b29681b6F29630A7201fa35958C63a3cC8);   // wallet address for addy (lead developer)
    ...
    function updateMerkleRootWL(bytes32 _merkleRootWl) external onlyAdmin {
        require(block.timestamp <= updateAvaiableMerkleWL, "MerkleRootWL: Not allowed to udpate the markleRoolWL");
    ...
    /// @notice Update the max count per wallet for presale
    function updatePreSaleMintAmount() external onlyAdmin {
        require(block.timestamp >= updatePreSaleMintCountPerWallet, "PS: Updating is available after only 2 days since startig presale is started!");
        require(!isPresaleFinished(), "PS: Updating is availbe for only presale sale!");
        maxPreSaleMintPerWallet = 20;
    }

    /// @notice Get max count per wallet for presale
    function getPreSaleMintAmount() public view onlyAdmin returns (uint256) {
        return maxPreSaleMintPerWallet;
    }
    ...
    function isPresaleFinished() public view returns (bool) {
        return block.timestamp > preSaleEnd || totalMints >= preSalesCount;
    }
    ...
    function whiteListMint(bytes32[] calldata _merkleProof, address _to, uint256 _count) external payable{
        require(block.timestamp >= preSaleStart, "PS: Presale is not started yet" );
        require(!isPresaleFinished(), "PS: Presale Finished");
        require(totalMints <= preSalesCount, "PS: Presale Ended");
        require(mintedTokens[_to] + _count <= maxPreSaleMintPerWallet, "PS: Max limited per wallet");
        require(msg.value >= preSaleMintFee * _count, "PS: Not enough funds sent");
    ...
    function withdrawToMarketingFunder() public payable onlyAdmin {
        ...
        require( marketWithdrawlAmount < 60 ether, "PS: Marketing funds has already been finished");
        uint256 total = address(this).balance;
        // wallet1.transfer(total);
        uint256 restAmount = 60 ether - marketWithdrawlAmount;
        if (total > restAmount) {
    ...
    function withdrawToFounders() public payable onlyAdmin {
        ...
        require( marketWithdrawlAmount >= 60 ether, "PS: Marketing funds is not finished yet!");
        uint256 total = address(this).balance;

        wallet2.transfer(total * 189 / 1000);
        wallet3.transfer(total * 278 / 1000);
        wallet4.transfer(total * 278 / 1000);
        wallet5.transfer(total * 23 / 1000);
