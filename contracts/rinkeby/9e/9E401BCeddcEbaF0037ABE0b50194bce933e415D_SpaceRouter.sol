...
contract SpaceRouter {
    ...
    /**
     * @notice Deposits an amount of Eth and SPCE into the pool
     * @param spcAmount an amount of space being deposited
     * msg.value is the amount of eth deposited
     */
    function addLiquidity(uint256 spcAmount) external payable {
        if (spcAmount == 0 || msg.value == 0)
            revert InvalidAmounts(msg.value, spcAmount);
            
        /// @dev This gets the miniumum expected Eth amount given a SPC amount. 
        /// As a feature of constant-product forumla, the minimum increases towards actual amounts as reserves increase.
        uint256 expectedEthAmount = spacePool.getOutputAmount(spcAmount, spacePool.reserveSPC(), spacePool.reserveEth(), 0);
        if (msg.value < expectedEthAmount) revert KHasBeenViolated(msg.value, spcAmount);

        if (spaceCoin.transferFrom(msg.sender, address(spacePool), spcAmount))
            spacePool.mint{value: msg.value}(msg.sender);
    }

    /**
     * @notice Removes an amount of liquidity and get back Eth and SPC
     * @param amount LP token amount to remove
     */
    function removeLiquidity(uint256 amount) external {
        if (amount == 0)
            revert InvalidAmount(amount);
        if(spacePool.transferFrom(msg.sender, address(spacePool), amount))
            spacePool.burn(msg.sender);
    }

     /**
     * @notice Swaps for Eth for SPC
     * @param minSpcOut the minimum amount of SPC that is required to be returned 
     */
    function swapEthForSpc(uint256 minSpcOut) external payable {
        spacePool.swapEthForSpc{ value: msg.value }(msg.sender, minSpcOut);
    }

    /**
     * @notice transfers SPC into the pool and swaps for Eth
     * @param amount LP token amount
     * @param minEthOut the minimum amount of Eth that is required to be returned 
     */
    function swapSpcForEth(uint256 amount, uint256 minEthOut) external {
        if (spaceCoin.transferFrom(msg.sender, address(spacePool), amount))
            spacePool.swapSpcForEth(msg.sender, minEthOut);
    }
}
