...
contract Minisquatch is ERC721Enumerable, Ownable, ControlledAccess{
    using Strings for uint256;
    ...
    function mint(uint _mintAmount) public payable {
        ...
        require(_mintAmount > 0, "You have to mint at least one minisquatch.");
        require(_mintAmount <= maxMintAmount, "Trying to mint too many nfts");

        require(hasParents(msg.sender), "Need more Metasquatchs/Land");

        require(supply + _mintAmount <= maxSupply, "Tokens are sold out");
        if (msg.sender != owner()) {
        require(msg.value >= cost * _mintAmount, "Send more ether.");
        }
    ...
    function swapTokensForEth(uint256 tokenAmount) private {
        ...
        IERC20(MSToken).approve(address(uniswapV2Router), tokenAmount);
