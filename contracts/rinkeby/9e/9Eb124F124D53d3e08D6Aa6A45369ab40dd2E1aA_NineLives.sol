...
contract NineLives is ERC721A, Ownable, ReentrancyGuard {    
    ...
    /** @dev allow whitelisted addresses to mint when true */
    ...
    function mintNineLives(uint256 quantity) public payable callerIsUser {
        ...
        if (whiteList[msg.sender] == true) {
            _mintWhiteList(quantity);
        } else {
            _mintPublicSale(quantity);
        }
    ...
    function _mintWhiteList(uint256 quantity) internal {
        require( whiteListSale == true, "Whitelist minting disabled.");
        require(minters[msg.sender] + quantity <= 5, "Mint exceeding the allowed limit");
            _safeMint(msg.sender, quantity);
    }

    function _mintPublicSale(uint256 quantity) internal {
    ...
    function setRevealNft(bool _isRevealed) public onlyOwner {
    ...
    function setWhiteListedSale(bool _WhiteListedSale) public onlyOwner {
        whiteListSale = _WhiteListedSale;
    }
    ...
    function setBaseURI(string memory _baseURI) public onlyOwner notFinalized {
        require(bytes(baseURI).length == 0, "baseURI cannot be empty.");
        baseURI = _baseURI;
        
    }

    function setPlaceholderURI(string calldata _placeholderURI) public onlyOwner {
        require(bytes(baseURI).length == 0, "Placeholder URI cannot be empty.");
