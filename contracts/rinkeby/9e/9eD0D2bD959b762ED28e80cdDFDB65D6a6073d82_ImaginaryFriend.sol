    ...
    ERC2981SinglePercentual
{
    ...
    using Serializer for TokenData;
    ...
    /// @notice Contains quiz results, etc. for the tokens in the collection.
    /// @dev Null corresponds to the quiz not being answered.
    /// @dev Storing a serialized version of the data was needed to enable
    /// more efficient handling in memory.
    mapping(uint256 => TokenDataSerialized) private tokenData;

    /// @notice The base URIs used to generate the image field in the metadata.
    BaseURIs internal _baseURIs;
    ...
    constructor(
        ...
        ERC721ACommon("asfgiouzhxcbkl", "ASDF")
        FixedPriceSeller(
            0.001 ether,
            Seller.SellerConfig({
                totalInventory: 3000,
    ...
    function setQuizResults(
        ...
        if (tokenData[tokenId].isSet()) revert QuizResultsAlreadySet();
    ...
    function hasQuizResults(uint256 tokenId)
        ...
        returns (bool)
    {
        return tokenData[tokenId].isSet();
    }

    /// @notice Sets the features for a given token and records the block.
    ...
    function _setFeatures(uint256 tokenId, Features memory features) internal {
        ...
        tokenData[tokenId] = TokenData({
            setOnBlock: uint32(block.number),
            features: features
        }).serialize();
    }

    /// @notice Checks if token features are valid.
    /// @dev Reverts on invalid featuers.
    ...
    /// @notice Sets the base image URIs used by `tokenURI`.
    /// @dev See the definition of `BaseURIs` for more information.
    function setBaseURIs(BaseURIs calldata baseURIs) external onlyOwner {
        if (bytes(baseURIs.revealed).length > 0) {
            _baseURIs.revealed = baseURIs.revealed;
        }
        if (bytes(baseURIs.unrevealed).length > 0) {
            _baseURIs.unrevealed = baseURIs.unrevealed;
        }
    }

    /// @notice Retrieves the features of a given, existing token.
    /// @param tokenId The token of interest.
    /// @dev Forwards the call to the metadata render with entropy computed from
    /// all quiz answers.
    ...
    {
        TokenDataSerialized data = tokenData[tokenId];
        TokenDataSerialized[] memory all = _loadAllTokens();
    ...
    /// @notice Retrieves the data-uri encoded metadata json for a given,
    /// existing token.
    /// @param tokenId The token of interest.
    ...
    /// @notice Loads all token data to memory.
    function _loadAllTokens()
        private
        view
        returns (TokenDataSerialized[] memory)
    {
        uint256 num = totalSupply();
        TokenDataSerialized[] memory all = new TokenDataSerialized[](num);
        for (uint256 idx = 0; idx < num; idx++) {
            all[idx] = tokenData[idx];
        }
        return all;
    }

    /// @notice Computes the entropy of given quiz results.
    /// @dev The entropy is only computed for a finished `quizState`. Otherwise
    /// we return zero.
    /// @param data The tokenData to be hashed -usually the output of
    /// `_loadAllTokens`. See e.g. also `tokenURI`.
    function _computeEntropy(TokenDataSerialized[] memory data)
        private
        view
        returns (bytes32)
    {
        ...
        for (uint256 idx = 0; idx < num; idx++) {
            // To accumulate entropy from the individual tokens, we have to
            // use an operation that corresponds to the identity function in the
            // case of the zero element. This ensures that the entropy can
            // only be affected by actual quiz answers - allowing tokens to be
            // minted even after the quiz was already finished.
            entropy = entropy ^ TokenDataSerialized.unwrap(data[idx]);
        }

        return keccak256(abi.encode(entropy));
    }
    ...
    /// @notice Sets the royalty percentage (in units of 0.01%).
    function setRoyaltyPercentage(uint96 percentage) external onlyOwner {
        _setRoyaltyPercentage(percentage);
    }

    /// @notice Sets the address to receive the royalties.
