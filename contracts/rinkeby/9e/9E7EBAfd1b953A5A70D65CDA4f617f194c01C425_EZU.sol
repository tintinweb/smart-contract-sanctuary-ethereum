...
contract EZU is ERC721BatchEnumerable, ERC2981Base, IOwnable, IPausable, ITradable, IWhitelistable {
	...
	// **************************************
		...
		* - Caller must be the contract owner.
		*/
		function setBaseURI( string calldata unrevealedBaseURI_, string calldata revealedBaseURI_ ) external onlyOwner {
    	    ...
    	    if (isRevealed) {
    	        return
    	            bytes(revealedURI).length > 0
    	                ? string(
    	                    abi.encodePacked(revealedURI, _toString(tokenId))
    	                )
    	                : "";
    	    }
    	    return
