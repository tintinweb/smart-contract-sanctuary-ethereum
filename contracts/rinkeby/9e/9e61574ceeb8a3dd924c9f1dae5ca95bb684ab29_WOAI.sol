...
contract ERC721 is Context, ERC165, IERC721, IERC721Metadata, IERC721Enumerable {
    ...
    // WOAI Generator mapping
    mapping (uint256 => string) private _generatorValues;

    // WOAI Referrer mapping (addresses to count of referred mints)
    ...
    /** 
     * @dev Mints a token and increments the referrer counter of the referrer
     */
    function _referAndMint(address to, uint256 tokenId, address referrer) internal virtual {
        _safeMint(to, tokenId, "");
        _addReferrer(referrer);
    }
    
    /**
     * @dev Increments the referrer counter
     */
    function _addReferrer(address referrer) internal virtual {
        if (_referrerCounts[referrer] == 0) {
            _referrerCounts[referrer] = 1;
            _nonZeroReferrers.push(referrer);
        } else {
            _referrerCounts[referrer]++;
        }
    }
     ...
     * @dev Top referrer
     */
    function listTopReferrer() public view returns(address,uint) {
        uint maxVal = 0;
        address topRef;
        uint len = _nonZeroReferrers.length;
        uint i = 0;
        for (i;i != len; i++) {
            if (_referrerCounts[_nonZeroReferrers[i]] > maxVal) {
                maxVal =_referrerCounts[_nonZeroReferrers[i]];
                topRef = _nonZeroReferrers[i];
            }
        }
    ...
    /**
     * @dev Sets the token to the failed URI.
     *
     * Requirements:
     *
     * - `tokenId` must exist.
    ...
    /**
     * @dev Returns the generator value for a given token if it exists
     * WOAI
     */
    function generatorValue(uint256 tokenId) public view virtual returns (string memory) {
        require(_exists(tokenId), "Token does not exist");
        return _generatorValues[tokenId];
    }
 ...
 */
contract WOAI is ERC721, Ownable {
    using SafeMath for uint256;

    // Global constant variables
    uint256 public constant woaiPrice = 1; //0.05 ETH
    ...
    /**
     * @notice Use this function to mint as many WOAI as defined as a parameter
    ...
    function mintWoai(uint numberOfTokens) external payable {
        ...
        require(woaiPrice.mul(numberOfTokens) <= msg.value, "Transaction needs to be accompanied with Ether to mint a WOAI");
    ...
    function mintAndRefer(uint numberOfTokens, address referrer) external payable {
        ...
        require(totalSupply().add(numberOfTokens) <= MAX_WOAI, "Purchase would exceed max supply of WOAI");
        require(woaiPrice.mul(numberOfTokens) <= msg.value, "Transaction needs to be accompanied with Ether to mint a WOAI");
        require(referrer != address(0), "Zero address cannot be a referrer");
