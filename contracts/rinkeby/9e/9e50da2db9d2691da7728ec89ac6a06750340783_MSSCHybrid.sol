...
contract MSSCHybrid is MSSCBase, MembraneAuth, HTLC {
  ...
  //////////////////////////////////////////////////////////////*/
    ...
    /**
     * @notice Relocate lock for a hybrid settlement cycle.
     * @dev    Caller must be an allowed membrane operator.
     *
     * @param  cycleId Instruction to claim deposited funds.
     */
    function relocateLock(bytes32 cycleId) external requiresAuth {
        _relocateLock(cycleId);
    }
...
abstract contract HTLC {
    event InitLock(bytes32 cycleId, uint256 startTime);
    event RelocateLock(bytes32 cycleId, uint256 newStartTime);

    uint256 public immutable claimablePeriod;
    uint256 public immutable refundablePeriod;
    ...
    function _relocateLock(bytes32 lockId_) internal {
        if (_locks[lockId_].relocated) {
            revert AlreadyRelocated();
        }

        _locks[lockId_].relocated = true;
        _locks[lockId_].startTime = block.timestamp;

        emit RelocateLock(lockId_, block.timestamp);
    }
