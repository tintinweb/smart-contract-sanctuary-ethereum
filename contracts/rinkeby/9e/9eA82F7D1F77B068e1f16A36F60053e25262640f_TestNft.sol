...
contract TestNft is ERC721("Test NFT", "TFT") {
  ...
  uint256 public totalPreSaleReserved = 5_000;
  uint256 public totalRaffleReserved = 5_000;

  // Limits
  uint256 internal walletLimit = 3;

  mapping(address => bool) preSaleList;
  mapping(address => bool) raffleSaleList;
    ...
    tokensAvailable(_amount)
  {
    ...
    require(isSaleActive, "Public sale not started");
    require(addressIndices[minter] + _amount <= walletLimit, "Mint min 1, max 3");
    ...
    raffleSaleStarted()
  {
    ...
    require(addressIndices[minter] + _amount <= walletLimit, "Mint min 1, max 3");
    require(totalRaffleReserved - _amount > 0, "Raffle sale sold out");
    require(msg.value >= _amount * raffleSalePrice, "Try sending more ETH");
  ...
  function tokenURI(uint256 tokenId) public view override returns (string memory) {
    return isRevealed ? string(abi.encodePacked(baseURI, '/', Strings.toString(tokenId), ".json")) : baseURI; //"--NOT-REVEALED-GIF-URL--";
  }
  ...
  function updatePreSaleReserveds(uint256 _amount) external onlyOwner {
    totalPreSaleReserved = _amount;
  }
  function updateRaffleSaleReserveds(uint256 _amount) external onlyOwner {
    totalRaffleReserved = _amount;
  }
