...
contract NFTPMysteryBox is ERC1155, ReentrancyGuard, Ownable {
    ...
    event RevealRequest(address owner, uint256 collectionId, uint256 amount);
    ...
    event Reveal(uint256 collectionId, uint256 nftId, uint16 rarity, uint16 photo, address owner);
    ...
    * @param amount amount of boxes to be minted */
    function mint(uint256 id, uint256 amount) external payable {
        ...
        //uint256 price = prices.getPrice([prices.WETH(), stableCoin])

        uint256 price = priceBNB == 0 ? 0 : c.price * 10**18 / priceBNB;

        require(msg.value >= price, "Not enouth BNB sent");
        _mint(c, id, amount);
        if (msg.value > price) {
            uint256 ret = msg.value - price;
    ...
    */
    function mintWithToken(uint256 id, uint256 amount) external {
        ...
        if (price > 0) {
            paidWith.transferFrom(msg.sender, address(this), amount*price);
        }
    ...
    */
    ...
    function reveal(uint256 id, uint256 amount) external {
        ...
        emit RevealRequest(msg.sender, id, amount);
    }

    function revealFullfill(address account, uint256 id, uint256 amount, uint256 seed) external onlyAdmin {
        require(balanceOf(account, id) >= amount, "Not enough MysteryBoxes");

        for(uint256 i=0; i<amount; i++) {
            _reveal(account, id, _random(account, seed));
        }
    }


    /**
    * @notice Allows the box owner to reveal the NFT. Burns the MysteryBox and mints the final NFT
    * @param id collection id
    */
    function _reveal(address account, uint256 id, uint256 rand) internal nonReentrant {
        MysteryBox.Collection storage c = collections[id];
        ...
        uint256 tokenId = nftp.totalSupply();

        RarityPhoto memory rp = RarityPhoto(
                uint16(rand % c.getCopies(uint16(rand % photoCount))),
                uint16(rand % photoCount),
                0,
                0
            );
        ...
        while(gasUsed == 0 || gasUsed < gasleft() - GAS_BUFFER) {
            ...
            if(collectionPhotoRarityMinted[id][rp.photo][rp.rarityGroup] + 1 <= c.rarities[rp.rarityGroup][rp.photo]) {
                ...
                _burn(account, id, 1);
                nftp.mint(account, tokenId, id, rp.rarityGroup, rp.photo);
                minted = true;
                break;
            } 
        ...
        emit Reveal(id, tokenId, rp.rarityGroup, rp.photo, account);
    }

    /**
    * @notice Produces a pseudo random uin16 number
    * @param from sender address
    * @param seed random seed from oracle
    */
    function _random(address from, uint256 seed) internal view returns (uint16){
        return uint16(uint256(keccak256(abi.encodePacked(block.difficulty, block.timestamp, blockhash(block.number + 2), from, seed))));
    }

    /**
    * @notice Allows setting prices of Pussies and BNB tokens in wei
    * @param paidWithPrice_ price of the Pussies token
    * @param priceBNB_ price of the BNB token
    */
    function setPrices(uint256 paidWithPrice_, uint256 priceBNB_) external {
        require(msg.sender == priceSetter, "Cannot set prices");
        require(paidWithPrice_ > 0, "Price cannot be 0");
        require(priceBNB_ > 0, "BNB price cannot be 0");

        paidWithPrice = paidWithPrice_;
        priceBNB = priceBNB_;
        _seed = paidWithPrice_ * priceBNB_ * block.timestamp;
    }

    function getPrices() external view returns(uint256, uint256) {
        return (paidWithPrice, priceBNB);
    }

    /**
    * @notice Sets the address allowed to configure prices
    * @param account address of the price setter
    */
    function setPriceSetter(address payable account)
        external
        onlyOwner
    {
        require(
...
interface IPussiesNFT {
    function mint(address to, uint256 id, uint256 collection, uint16 rarity, uint16 photo) external;
    function totalSupply() external view returns(uint256);
    function getInfo(uint256 id) external view returns(uint256 collection, uint16 rarity, uint16 photo);
}

contract PussiesNFT is IPussiesNFT, IERC721, ERC721Enumerable, ReentrancyGuard, Ownable {
    ...
    mapping(uint256 => uint16) public photos;
    mapping(uint256 => uint256) public collections;

    constructor(string memory contractMetadataURI) ERC721("NFTPussies", "NFTP") { //! FIXME
        _contractMetadataURI = contractMetadataURI;
    }

    /**
    * @notice Mints the NFT, only callable by owner and mysteryBox contract
    * @param to recipient of the NFT
    * @param id id of token to be minted
    * @param collection collection id the token belongs to
    * @param rarity token rarity
    * @param photo photo id for the token
    */
    function mint(address to, uint256 id, uint256 collection, uint16 rarity, uint16 photo) external { 
    ...
    */
    ...
    function _baseURI() internal pure override returns (string memory) {
        return "https://api.nftpussies.com/";
    }
    ...
    function totalSupply() public view override(ERC721Enumerable, IPussiesNFT) returns(uint256) {
