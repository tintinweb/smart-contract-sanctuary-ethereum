...
UUPSUpgradeable
{
    ...
    struct DutchAuction {
        uint256 startDate; // Initial startDate
    ...
    function initialize(
        ...
        // set the initial DutchAuction config
        auction = _auction;
        // set the initial max Mint Per Address
    ...
    function getPrice(uint256 n) public view returns (uint) {
        require(block.timestamp > auction.startDate, "Auction not started yet.");
        uint256 decreases = MathUpgradeable.min(
            (block.timestamp - auction.startDate) / auction.decreaseInterval,
                auction.numDecreases
        );
        return n * (mintPrice - decreases * auction.decreaseSize);
    }
    ...
    function safeMint(address to) public payable limited {
        ...
        require(getPrice(1) == msg.value, "Mint price is not correct.");
    ...
    function safeBatchMint(address to, uint256 amount) public payable {
        ...
        require(getPrice(amount) == msg.value, "Mint price is not correct.");
