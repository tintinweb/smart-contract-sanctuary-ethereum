    ...
    IERC721Receiver
{
    ...
    function buyTickets(uint256 count) external payable {
        ...
        require(
            lottery.soldCount + count < ticketToSold() + 1,
            "Not enough tickets"
        );
        ...
        if (lottery.soldCount == ticketToSold()) {
            setLotteryStatus(Status.Openable);
        }
    }

    function ticketToSold() internal view returns (uint256) {
        return (lottery.ticketCount * (100 + fee)) / 100;
    }
