...
contract BaseFixedPriceAuctionERC1155 is ERC1155, ReentrancyGuard, Ownable {
    ...
    mapping(uint256 => TokenIdInfo) private tokenIdInfos;
    
    // mapping(uint256 => uint256) public nonReservedMax;
    // mapping(uint256 => uint256) public reservedMax;
    // mapping(uint256 => uint256) public whitelistMaxMint;
    // mapping(uint256 => uint256) public publicListMaxMint;
    ...
    constructor(
        ...
        string memory _uri,
        TokenIdInfo[] memory _tokenIdInfos
    )
        ERC1155(_uri)
    {
        setTokenIdInfo(_tokenIdInfos);
        _splitter = new PaymentSplitter(payees, shares);
    }

    function setTokenIdInfo(TokenIdInfo[] memory _tokenIdInfos) public onlyOwner {
        uint256 len = _tokenIdInfos.length;
        TokenIdInfo memory tmpInfo;
        uint256 _tokenId;
        for (uint256 i = 0; i < len; i ++) {
            tmpInfo = _tokenIdInfos[i];
            _tokenId = tmpInfo.tokenId;
            tokenIdInfos[_tokenId] = tmpInfo;
            max[_tokenId] = tmpInfo.nonReservedMax + tmpInfo.reservedMax;
            // nonReservedMinted[_tokenId] = 0;
            // reservedMinted[_tokenId] = 0;
        }
    }

    function getTokenIdInfo(uint256 _tokenId) external returns (TokenIdInfo memory) {
        return tokenIdInfos[_tokenId];
    }
    ...
    function setPrice(uint256 _tokenId, uint256 _price) public onlyOwner {
        tokenIdInfos[_tokenId].price = _price;
    }
    ...
    function setWhitelistMaxMint(uint256 _tokenId, uint256 _whitelistMaxMint) external onlyOwner {
        tokenIdInfos[_tokenId].whitelistMaxMint = _whitelistMaxMint;
    }

    function setPublicListMaxMint(uint256 _tokenId, uint256 _publicListMaxMint) external onlyOwner {
        tokenIdInfos[_tokenId].publicListMaxMint = _publicListMaxMint;
    }

    function mintCapsule(uint256 tokenId, uint256 numberOfTokens) external payable {
        require(_publicListClaimed[tokenId][msg.sender] + numberOfTokens <= tokenIdInfos[tokenId].publicListMaxMint, 'You cannot mint this many.');
    ...
    function mintCapsuleWhitelist(bytes32 hash, bytes memory signature, uint256 tokenId, uint256 numberOfTokens) external payable {
        ...
        require(_whitelistClaimed[tokenId][msg.sender] + numberOfTokens <= tokenIdInfos[tokenId].whitelistMaxMint, 'You cannot mint this many.');
    ...
    function _nonReservedMintHelper(uint256 tokenId, uint256 numberOfTokens) internal {
        require(numberOfTokens * tokenIdInfos[tokenId].price == msg.value, "Invalid amount.");
    ...
    function mintReservedCapsule(uint256 tokenId) external onlyOwner {
        ...
        require(tokenIdInfos[tokenId].reservedMax != 0, 'reserved not set');
        _mint(msg.sender, tokenId, tokenIdInfos[tokenId].reservedMax, "");
        mintedAmounts[tokenId] = tokenIdInfos[tokenId].reservedMax;
    }
