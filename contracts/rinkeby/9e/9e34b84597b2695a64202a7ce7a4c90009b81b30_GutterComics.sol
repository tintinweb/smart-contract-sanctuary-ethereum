...
contract GutterComics is ERC721A, Ownable, ReentrancyGuard {
    ...
    uint256 public maxNormalSupplyID = 11999; //tokens start from 0
    uint256 public presaleStartID = 60;
    uint256 public presaleStopID = 8999;
    uint256 public publicSaleStartID = 0; // to be set once the presale is over
    uint256 public publicSaleStopID = 0; // to be set once the presale is over
    uint256 public freeMintStartID = 0; // to be set once the public sale is over
    uint256 public freeMintStopID = 0; // to be set once the public sale is over
    ...
    function presale(
        ...
        uint256 lastMintedId = totalSupply() - 1;
        require(lastMintedId + qty <= maxNormalSupplyID, "out of stock");
        require(lastMintedId + qty >= presaleStartID, "wrong presale id");
        require(lastMintedId + qty <= presaleStopID, "exceeds presale");
        ...
        nonReentrant
    {
        ...
        uint256 lastMintedId = totalSupply() - 1;
        require(lastMintedId + qty <= maxNormalSupplyID, "out of stock");
        require(lastMintedId + qty >= publicSaleStartID, "wrong public sale id");
        require(lastMintedId + qty <= publicSaleStopID, "exceeds public sale");
        require(pricePerComic * qty == msg.value, "exact amount needed");
    ...
    function freeSale(uint256 key, uint256 catID) external {
        ...
        uint256 lastMintedId = totalSupply() - 1;
        require(lastMintedId + 1 <= maxNormalSupplyID, "out of stock");
        require(lastMintedId + 1 >= freeMintStartID, "wrong free mint id");
        require(lastMintedId + 1 <= freeMintStopID, "exceeds free mint");
    ...
    function upgradeComic(
        ...
        uint256 lastMintedId = totalSupply() - 1;
        require(lastMintedId >= maxNormalSupplyID, "wrong max supply settings");
        ...
        emit TokenUpgraded(lastMintedId + 1, coverType);
    }

    //admin mint
    function adminMint(address to, uint256 qty) external onlyOwner {
        uint256 lastID = totalSupply() + qty - 1;
        require(lastID <= maxNormalSupplyID, "out of stock");
        _safeMint(to, qty);
    }
    ...
    function setPublicSaleIntervals(uint256 start, uint256 end)
        ...
        onlyOwner
    {
        publicSaleStartID = start;
        publicSaleStopID = end;
    }
    ...
    function decreaseMaxNormalSupplyID(uint256 newSupplyID) external onlyOwner {
        require(
            maxNormalSupplyID > newSupplyID && newSupplyID >= totalSupply() - 1,
            "invalid new supply"
        );
