...
contract Staker {
    ...
    //shows balance of user's staked funds
    ...
    //checking for boolean set for if threshold isnt reached by deadline
    bool public openForWithdraw;

    // EVENTS
    /// event to emit when stake function is called
    event Stake(address sender, uint256 value);

    /// Modifier to check if the required deadline has passed
    ...
    function stake() public payable deadlinePassed(false) notCompleted {
        ...
        // emiting Stake event to notify the UI
