...
contract BURNER is Context, ERC721Burnable {
	...
	string private _description = "BURNER is a dynamic cryptoart series based on the information content in Ethereum gas prices. A total of 256 interconnected artworks that dynamically react to live gwei fees. As Ethereum network usage increases, due to price changes, NFT sales or other factors, a more complex artwork appears. Gas price data is made up of the competing desires, intentions and beliefs of millions of entities. Transactions on Ethereum create overwhelming amounts of information in real-time, cyclically revealing previously unseen states. BURNER pictures an abstract flow in this massive decentralised process. The image files in BURNER begin as pictures of true physical gas. They are extruded into an extra dimension, rotated and glitched until they are unrecognisable. These physical-based forms interact differently with each other in each digital artwork. Every BURNER has a unique base image, unique seed and 60 million potential states. Rarer dark and bright types can have over 1 billion states. Each BURNER is dynamically created from 7-9 image layers. Besides a unique base layer image, each layer has 10 possible forms, all stored as separate PNG files on IPFS. As gas increases, more layers are revealed. PNGs also change dynamically every block according to micro-fluctuations in gas. Gas data is handled by a server-side oracle while image mixing is done by a client stored on-chain on Ethereum. BURNER is not controlled by the artist, but by the Ethereum Virtual Machine and its users.";
