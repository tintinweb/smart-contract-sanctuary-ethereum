...
contract nill is Ownable, ERC721A, ReentrancyGuard {
    uint256 public MAX_SUPPLY = 4222;
    ...
    // getters
    function getNFTsRemaining(uint8 playerID) public view returns (uint256) {
        uint256 remaining = NFTsPerPlayer - _numPlayerMinted[playerID];
        return remaining;
    }

    function getNFTsQuantities() public view returns (uint256[] memory) {
        uint256[] memory mintedArr = new uint256[](numPlayers);
        for (uint8 i = 0; i < numPlayers; i++) {
            mintedArr[i] = _numPlayerMinted[i];
        }
        return mintedArr;
    }
    ...
    // Presale
    function mintPreSaleTokens(uint8 quantity, bytes32[] calldata _merkleProof, uint8 playerID)
        external
        payable
        callerIsUser
        nonReentrant
    {
        ...
        _safeMint(msg.sender, quantity, playerID);
    ...
    // public mint
    function publicSaleMint(uint256 quantity, uint8 playerID, address to)
        external
        payable
        nonReentrant
        callerIsUser
    {
        ...
        uint256 startingID = _numPlayerMinted[playerID] + (playerID * NFTsPerPlayer) + numInternal;
        _numPlayerMinted[playerID] = _numPlayerMinted[playerID] + quantity;

        _safeMint(to, quantity, startingID);
    }
