...
contract AncientDevice is ERC721Enumerable, OwnableWithAdmin, Pausable {
  ...
  uint256 public   YIELD_END_TIME     = 1742839200; // March 24, 2025, 19:00 UTC: Mercury's first Inferior Conjunction of 2025

  uint256 private lineOfClue;

  string public baseURI_level1;
  string public baseURI_level2;
  string public baseURI_level3;
  string public baseURI_level4;
  string public baseURI_level5;
  string public baseURI_secret6;
  ...
  /**
    ...
    */
  function setBaseURI_level1(string memory _baseURI) external onlyOwnerOrAdmin {
      baseURI_level1 = _baseURI;
  }
  /**
    * @dev include trailing /
    */
  function setBaseURI_level2(string memory _baseURI) external onlyOwnerOrAdmin {
      baseURI_level2 = _baseURI;
  }
  /**
    * @dev include trailing /
    */
  function setBaseURI_level3(string memory _baseURI) external onlyOwnerOrAdmin {
      baseURI_level3 = _baseURI;
  }
  /**
    * @dev include trailing /
    */
  function setBaseURI_level4(string memory _baseURI) external onlyOwnerOrAdmin {
      baseURI_level4 = _baseURI;
  }
  /**
    * @dev include trailing /
    */
  function setBaseURI_level5(string memory _baseURI) external onlyOwnerOrAdmin {
      baseURI_level5 = _baseURI;
  }
  /**
    * @dev include trailing /
    */
  function setBaseURI_secret6(string memory _baseURI) external onlyOwnerOrAdmin {
      baseURI_secret6 = _baseURI;
  }
  ...
  function claimDevices(uint256 _amount, bytes32[] calldata _merkleProof, uint256 _allowance) external whenNotPaused {
    ...
    require(addressHasClaimed[_msgSender()] == false, "You have already claimed devices"); // User can only claim once
  ...
  function magnesiumAvailable(uint256 tokenId) public view returns (uint256) {
    ...
    if (currentTimestamp > YIELD_END_TIME) // if its past the yield end time
      currentTimestamp = YIELD_END_TIME; // stop the yield at yield end time
   ...
   */
  function magnesiumAvailableInMany(uint256[] calldata tokenIds) public view returns (uint256) {
    uint256 available;
  ...
  /**
   * claim MAGNESIUM from a device
   * @param _tokenId the token to claim MAG from
   */
  function claimMag(uint256 _tokenId) external whenNotPaused {
    require(ownerOf(_tokenId) == _msgSender(), "NOT YOUR DEVICE");
    uint256 available = magnesiumAvailable(_tokenId);
    require(available > 0, "NO MAG AVAILABLE");
    ...
    emit MagnesiumClaimed(_msgSender(), _tokenId, available);
  }
  ...
  function upgradeDeviceAndClaim(uint256 _tokenId, uint256 _targetLevel) external whenNotPaused {
    ...
    require(_msgSender() == ownerOf(_tokenId), "NOT YOUR TOKEN");
    Device storage device = devices[_tokenId];
    ...
    magnesium.burn(_msgSender(), magCost); // if user does not have enough MAG, burn requirement will fail
    device.level = uint16(_targetLevel);
    emit DeviceUpgradedMagClaimed(_msgSender(), _tokenId, _targetLevel, available);
  } // 377981

  /*
   * User can upgrade 1 level at a time or multiple
   * 1 to 2 : 7 MAG
   * 2 to 3 : 35 MAG
   * 3 to 4 : 350 MAG
   * 4 to 5 : 3000 MAG
  ...
  function secretUnlockWithClaimIfNeeded(uint256 _tokenId) external whenNotPaused whenSecretUnlockEnabled {
    ...
    require(block.timestamp > YIELD_END_TIME, "Device not ready");
    Device storage device = devices[_tokenId];
  ...
  function tokenURI(uint256 _tokenId) public view override returns (string memory) {
    ...
    uint16 level = devices[_tokenId].level;
    if(level == 1) return string(abi.encodePacked(baseURI_level1, Strings.toString(_tokenId), baseExtension));
    if(level == 2) return string(abi.encodePacked(baseURI_level2, Strings.toString(_tokenId), baseExtension));
    if(level == 3) return string(abi.encodePacked(baseURI_level3, Strings.toString(_tokenId), baseExtension));
    if(level == 4) return string(abi.encodePacked(baseURI_level4, Strings.toString(_tokenId), baseExtension));
    if(level == 5) return string(abi.encodePacked(baseURI_level5, Strings.toString(_tokenId), baseExtension));
    if(level == 6) return string(abi.encodePacked(baseURI_secret6, Strings.toString(_tokenId), baseExtension));

    return string(abi.encodePacked(baseURI_level1, Strings.toString(_tokenId), baseExtension));
  }
...
contract MAGNESIUM is ERC20, OwnableWithAdmin {
  ...
  uint256 constant HARD_CAP = 22000000 ether; // the actual max supply will likely be much lower when yielding ends at epoch
  ...
  function mint(address to, uint256 amount) external {
    require(totalSupply() + amount <= HARD_CAP, "Supply cap reached"); // safety valve
