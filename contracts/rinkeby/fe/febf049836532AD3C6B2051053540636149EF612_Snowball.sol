...
contract Snowball is Ownable, ERC721Enumerable {
    ...
    function redeem(uint256 tokenId) public virtual {
        ...
        payable(msg.sender).transfer(address(this).balance / totalSupply());
        _burn(tokenId);
    }

    function buy() external payable {
        uint256 supply = totalSupply();
        require(supply + 1 <= MAXSNOW, "Not enough Tokens remaining.");
        if (totalSupply() >= startSupply) {
            price = ((address(this).balance / totalSupply()) * 120) / 100;
        }
    ...
    function getpricecurrent() external view returns (uint256) {
        if (totalSupply() >= startSupply) {
        return((address(this).balance / totalSupply() * 120) / 100);
        }else{
        return (price);
    }}
    ...
    function _safeMint(address to, uint256 tokenId) internal virtual override {
        ...
        require(tokenId < MAXSNOW, "Max supply");
