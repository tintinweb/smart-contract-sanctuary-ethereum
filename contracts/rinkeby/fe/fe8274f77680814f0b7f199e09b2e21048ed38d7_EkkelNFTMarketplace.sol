...
interface IERC721 is IERC165 {
    ...
    function royaltyInfoOwner(uint tokenId) external view returns (address payable);
    function royaltyInfoPercentage(uint tokenId) external view returns (uint);

    function royaltyInfoIntention(uint tokenId) external view returns (bool);

    function isContentLocked(uint _tokenID) external view returns (bool);

    function flipContentLockedStatus(uint _tokenID) external;
...
contract EkkelNFTMarketplace is Pausable, ERC721Holder {
...
//                Mappings
    ...
    function _createBid(address _tokenAddress, uint256 _tokenId,  uint256 value)  internal   {
        ...
        } else // First bid
        {
            ...
            orderByTokenId[_tokenAddress][_tokenId].expiryTime =  block.timestamp.add(15 seconds); // 1 day auction time
        }
