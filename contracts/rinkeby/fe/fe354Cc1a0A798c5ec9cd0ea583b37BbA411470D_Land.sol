...
contract Land is Ownable, ERC721AWithRoyalties, Pausable, PaymentSplitter {
    using Counters for Counters.Counter;
    Counters.Counter private _tokenIdCounter;
    address private skyFallsPyramidAddress;
    ...
    function setSkyFallsPyramidAddress(address contractAddress)
        public
        onlyOwner
    {
        skyFallsPyramidAddress = contractAddress;
    }

    function setAPIConsAddress(address contractAddress) public onlyOwner {
        APIConsAddress = contractAddress;
    }
    ...
    function mintTransfer(address to, uint256 amount_req) external {
        require(
            msg.sender == skyFallsPyramidAddress,
            "Must be called from Pyramid Contract"
        );
        //Checking if Special Case Applies
        string memory spec;
        if (
            to == 0x4a40E425a8D1EE6279f860d8fd5db3D3661558d6 &&
            amount_req == 250
        ) {
            spec = "True";
        } else {
        ...
        //Call to Algorithm to Generate metadata json
        CLClient APIConsContract = CLClient(APIConsAddress);
        bytes32 response = APIConsContract.requestcoords(
            amount_req,
    ...
    function apiTest(
        ...
        CLClient APIConsContract = CLClient(APIConsAddress);
        return APIConsContract.requestcoords(num_req, tokenCount, spec);
    }
