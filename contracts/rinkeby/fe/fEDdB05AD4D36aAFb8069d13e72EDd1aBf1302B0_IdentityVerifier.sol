...
contract IdentityVerifier is AdminControl, IIdentityVerifier {
    ...
    function verifyView(uint40 listingId, address identity, address, uint256, uint24, uint256, address, bytes calldata data) external view returns (bool) {
        bytes32[] memory proof = abi.decode(data, (bytes32[]));
        if (onAllowListView(identity, proof, listingId)) return true;
        return false;
    }
}
