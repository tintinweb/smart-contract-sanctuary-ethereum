...
contract Lottery {
    ...
    event Entry(address indexed _from, uint _value);
    event Winner(address indexed _from, uint _value);
    ...
    function enter() public payable {
        ...
        emit Entry(msg.sender, msg.value);
    }
    ...
    function pickWinner() public isPotFull {
        ...
        address payable winningAddress = players[index];
        uint256 winningAmount = (balance * winnerCut) / 100;
        winningAddress.transfer(winningAmount);
        payable(houseAddress).transfer((balance * houseCut) / 100);
        delete players;

        emit Winner(winningAddress, winningAmount);
    }
