...
contract Celephais is ERC721A, Ownable{
    ...
    uint public presalePrice  = .088 ether;
    uint public publicPrice =   .12 ether;
    uint public OIL_PRICE = 390000 ether; //decimals are 18

    string public baseURI = "ipfs:/CID/activeURI.json";
    ...
    function drowsyEthMint(uint max,bytes memory signature) external payable 
    {
        ...
        if(wlCounter + 1 > maxWlSupply) revert SoldOut();
        if(numMinted[msg.sender][0] + 1 > maxMintsPerWallet) revert MaxMints();
        numMinted[msg.sender][0] += 1;
        wlCounter += 1;
        _mint(msg.sender,1);
    }

    function drowsyOilMint() external 
    {
        if(saleStatus == SaleStatus.INACTIVE) revert SaleNotActive();
        if(oilCounter + 1 > maxOilMints) revert SoldOut();
        if(numMinted[msg.sender][1] + 1 > maxMintsPerWallet) revert MaxMints();
        OIL.transferFrom(msg.sender, address(this), OIL_PRICE );
        oilCounter += 1;
        numMinted[msg.sender][1] += 1;
        _mint(msg.sender,1);
    }
    ...
    function publicMint() external payable
    {
        ...
        if(numMinted[msg.sender][2] + 1 > maxMintsPerWallet) revert MaxMints();
        numMinted[msg.sender][2] += 1;
        _mint(msg.sender,1);
    }
