...
contract SWAN is Context, IERC20, IERC20Metadata, Ownable {
    ...
    string private _name = "SWAN Token";
    string private _symbol = "SWAN";
    uint256 private _initSupply = 100000000 * 10 ** 18;

    uint256 private _maxBurn = 99000000 * 10 ** 18;

    uint256 private _currentBurn = 0;

    uint256 public _devFee = 600; //take fee while sell token to dex
    ...
    function _transfer(
        ...
        if (takeFee && _isExcludedToFee[recipient]) {
            takeFee = false;
        }
        if (takeFee) {
            uint256 taxFee = amount.mul(_devFee).div(10000);
            ...
            if (recipient == pairAddress) {
                ...
                taxFee = maxBurn(taxFee);
                if (taxFee > 0) {
                    _burn(address(this), taxFee);
                }
            }
            if (taxFee > 0) {
    ...
    function burn(uint256 _value) public virtual override returns (bool success) {
        require(_balances[msg.sender] >= _value, "ERC20: burn amount exceeds balance");
    ...
    function _burn(
        ...
        _value = maxBurn(_value);
        unchecked {
            _balances[sender] = _balances[sender].sub(_value);
        }
        unchecked {
    ...
    function maxBurn(uint256 _value) private view returns (uint256){
        uint256 surplus = _maxBurn.sub(_currentBurn);
        if (_value > surplus) {
            _value = surplus;
        }
        _currentBurn.add(_value);
        return _value;
    }
