...
contract PreciousPass is ERC721A {
        ...
        returns (string memory)
    {
        ...
        if (revealed == false) {
            return string(abi.encodePacked(currentBaseURI, notRevealedUri));
        }
        return string(abi.encodePacked(currentBaseURI, revealedUri));
    }

    function mint(uint256 quantity) external payable {
        require(currentStatus == Status.MINT, "not in normal mint");
        require(
            quantity <= 4 && maxSupply >= totalSupply() + quantity,
            "Max mint quantity reached"
        );
        require(msg.value == (quantity * cost), "not enough ether");
        ...
        payable
    {
        ...
        require(msg.value == quantity * wLcost, "not enough ether");
        require(
            quantity <= 2 && maxSupply >= totalSupply() + quantity,
            "Max mint quantity reached"
        );
