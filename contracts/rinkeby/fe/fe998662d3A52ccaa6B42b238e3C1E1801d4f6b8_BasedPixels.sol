...
//        }
        ...
        bool[16] memory reds = [false, false, false, false, false, false, false, false, false, false, false, false, false, false, false, false];
        bool[16] memory greens = [false, false, false, false, false, false, false, false, false, false, false, false, false, false, false, false];
        bool[16] memory blues = [false, false, false, false, false, false, false, false, false, false, false, false, false, false, false, false];
        ...
        for (uint256 i = 0; i < 4; i++) {
            for (uint256 j = 0; j < 64; j++) {
                uint256 xBase = (i*64 + j)%16;
                uint256 yBase = (i*64 + j)>>4;
                uint256 hcR = hexChars[(slices[i*3 + 1]>>(j*4))%16];
                uint256 hcG = hexChars[(slices[i*3 + 1]>>(j*4))%16];
                uint256 hcB = hexChars[(slices[i*3 + 1]>>(j*4))%16];
                uint256 strNum1 = rectStart+((48 + ((xBase - (xBase%10))/10))<<176)+((48 + (xBase%10))<<168)+inner1;
                strNum1 = strNum1+((48 + ((yBase - (yBase%10))/10))<<120)+((48 + (yBase%10))<<112)+inner2;
                uint256 strNum2 = heightNum+(hcR<<80);
                strNum2 = strNum2+(hcG<<72);
                strNum2 = strNum2+(hcB<<64)+(39<<56)+(47<<48)+(62<<40);
                rectComponents[(i*64 + j)*2] = bytes32(strNum1);
                rectComponents[(i*64 + j)*2 + 1] = bytes32(strNum2);
                if (!(reds[hcR] && greens[hcG] && blues[hcB])) {
                    uniqueColors += 1;
                }
                reds[hcR] = true;
                greens[hcG] = true;
        ...
        return string(
            abi.encodePacked(
                "data:application/json;utf8,","{\"name\":\"Token: ",
                    Strings.toString(tokenId),"\",\"description\":\"Token",
