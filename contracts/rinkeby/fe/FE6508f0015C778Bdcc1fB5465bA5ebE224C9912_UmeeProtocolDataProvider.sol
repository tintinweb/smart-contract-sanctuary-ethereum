...
contract UniswapRepayAdapter is BaseUniswapAdapter {
   ...
   * The user should give this contract allowance to pull the UTokens in order to withdraw the underlying asset, swap it
   ...
   * The user should give this contract allowance to pull the UTokens in order to withdraw the underlying asset
  ...
  function swapAndRepay(
    ...
    if (collateralAsset != debtAsset) {
      ...
      // Pull uTokens from user
      _pullUToken(
        collateralAsset,
        collateralReserveData.uTokenAddress,
        msg.sender,
        amounts[0],
        permitSignature
      );
...
contract UiPoolDataProvider is IUiPoolDataProvider {
  ...
  constructor(IUmeeIncentivesController _incentivesController, IPriceOracleGetter _oracle) public {
    ...
    )
  {
    ...
    for (uint256 i = 0; i < reserves.length; i++) {
      ...
      // we're getting this info from the uToken, because some of assets can be not compliant with ETC20Detailed
      reserveData.symbol = IERC20Detailed(reserveData.uTokenAddress).symbol();
      reserveData.name = '';
      ...
      if (address(0) != address(incentivesController)) {
        (
          ...
          reserveData.uTokenIncentivesIndex
        ) = incentivesController.getAssetData(reserveData.uTokenAddress);
      ...
      if (user != address(0)) {
        // incentives
        if (address(0) != address(incentivesController)) {
          userReservesData[i].uTokenincentivesUserIndex = incentivesController.getUserAssetData(
            user,
            reserveData.uTokenAddress
          );
        ...
        userReservesData[i].scaledUTokenBalance = IUToken(reserveData.uTokenAddress)
...
contract WethMockAggregator is IChainlinkAggregator {
  ...
  function latestAnswer() external view override returns (int256) {
    ...
    return baseContract.getWethPrice();
  }
...
contract UsdtMockAggregator is IChainlinkAggregator {
  address public baseMockAddress;
  ...
  function latestAnswer() external view override returns (int256) {
    BaseMockAggregator baseContract = BaseMockAggregator(baseMockAddress);
    return baseContract.getUsdtPrice();
  }
...
contract DaiMockAggregator is IChainlinkAggregator {
  address public baseMockAddress;
  ...
  function latestAnswer() external view override returns (int256) {
    BaseMockAggregator baseContract = BaseMockAggregator(baseMockAddress);
    return baseContract.getDaiPrice();
  }
...
import "../../../protocol/libraries/umee-upgradeability/VersionedInitializable.sol";
...
contract AtomMockAggregator is IChainlinkAggregator {
  address public baseMockAddress;
  ...
  function latestAnswer() external view override returns (int256) {
    BaseMockAggregator baseContract = BaseMockAggregator(baseMockAddress);
    return baseContract.getAtomPrice();
  }
