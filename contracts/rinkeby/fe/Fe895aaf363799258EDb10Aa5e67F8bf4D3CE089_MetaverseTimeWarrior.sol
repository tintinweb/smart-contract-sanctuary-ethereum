...
contract MetaverseTimeWarrior is ERC721, Ownable {
    ...
    bytes32 public merkleRootAirdrop =
        0xe5840aec0dd09e333d7a91755692f64db00e74eb5359b0e4aa75a9dc80a8cda5;

    constructor() ERC721("Metaverse Time Warrior", "MTW") {
        setNotRevealedURI(
            "ipfs://QmYcsFFCaZbCozvj4dRHC2dDg78WFUVmTgUSvpsvkcARKC"
        );
    }
    ...
    function mintAirdrop(bytes32[] calldata _merkleProof) public payable {
        uint256 supply = totalSupply();
        require(supply + 1 <= maxSupply, "max NFT limit exceeded");
        require(currentState > 0, "the contract is paused");
        require(airdropAllowed(msg.sender, _merkleProof), "user is not airdrop allowed");
    ...
    function airdropAllowed(address _user, bytes32[] calldata _merkleProof) public view returns (bool) {
        bytes32 leaf = keccak256(abi.encodePacked(_user));
        return airdropClaimed[_user] == false && MerkleProof.verify(_merkleProof, merkleRootAirdrop, leaf);
    }
    ...
    function setAirdropRoot(bytes32 _merkleRoot) public onlyOwner {
        merkleRootAirdrop = _merkleRoot;
    }
