...
contract NFTCollectible is ERC721Enumerable, Ownable {
    ...
    // Declaring state variable, instead of writing here, can write it when being deployed

    // Maximum limit of tokens that can ever exist
    uint256 public constant MAX_SUPPLY = 10000;
    uint256 public constant MAX_WL_SUPPLY = 4000;
    ...
    // Price of each token
    uint256 public initial_price = 0.085 ether;
    ...
    // mapping(uint256 => uint256) private _info;

    //Event is an inheritable member of a contract. An event is emitted, it stores the arguments passed in transaction logs
    event NFTCreated(address indexed to, uint256 tokenId);
    event EthersWithdrawn(address indexed to);

    //A constructor code is executed once when a contract is created and it is used to initialize contract state.
    // Creating a constructor to set value for variables declared
    // defining parameters and using parameters to define declared variables
    constructor() ERC721("NFT Collectible", "NFTC") {
        price = initial_price;
        setHiddenMetadataUri(
            "ipfs://Qmej1mq4a374vhvBMPpmBTfrszA7vTG9ySt99AJ7YKYVKd/0.json"
        );
    }

    // currentTokenCount is a view function
    function currentTokenCount() public view returns (uint256) {
        return uint256(_tokenIds.current());
    }

    //However, we need to tell our contract that the baseTokenURI variable that we defined is the base URI that the contract must use.
    //To do this, we override an empty function called _baseURI() and make it return baseTokenURI.

    function _baseURI() internal view virtual override returns (string memory) {
        return baseTokenURI;
    }

    // There, calling the parameter baseTokenURI would shadow the existing state variable baseTokenURI,
    ...
    function mintWhitelist(
        ...
        require(whitelistActive = true, "Whitelist isn't active");
        require(_amount <= MAX_PER_MINT, "Can't mint more than max");
        require(totalMinted.add(_amount) <= MAX_WL_SUPPLY, "Not enough NFTs!");
    ...
    function _mintSingleNFT() private {
        ...
        emit NFTCreated(msg.sender, newTokenID);
    }
    ...
    //Let’s write a simple function that returns all IDs owned by a particular holder. This is made super simple by ERC721Enumerable‘s balanceOf and tokenOfOwnerByIndex functions.
    //The former tells us how many tokens a particular owner holds, and the latter can be used to get all the IDs that an owner owns.
    ...
    function withdraw() public payable onlyOwner {
        ...
        emit EthersWithdrawn(msg.sender);
    }
}