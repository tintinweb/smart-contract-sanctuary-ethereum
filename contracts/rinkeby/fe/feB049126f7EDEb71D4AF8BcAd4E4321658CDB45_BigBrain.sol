...
import "./IUSDCVault.sol";
import "hardhat/console.sol";


contract BigBrain is Context, Ownable, IERC20 {
    ...
    /// When the balance reaches this, the contract sends the various amounts to the different tx endpoints, 
    /// This saves on gas deployments
    ...
    /// @param _lock Lock to be timelocked
    ...
    constructor () {
        ...
        // IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0xaEb4334BeB46079a72CC52AcAEB54EA733264361);
    ...
    function setTaxFeePercent(uint256 taxFee) external onlyOwner unlocked(Lock.TAX_FEE) {
        require(taxFee <= 150, "Amount must be less than or equal to 15");
        _taxFee = taxFee;
    ...
    function setLiquidityFeePercent(uint256 liquidityFee) external onlyOwner unlocked(Lock.LIQUIDITY_FEE) {
        require(liquidityFee <= 150, "Amount must be less than or equal to 15");
    ...
    function calculateLiquidityFee(uint256 _amount) private view returns (uint256) {
        return _amount.mul(_liquidityFee + _vaultFee).div(
            10**3
        );
    }
    ...
    function removeAllFee() private {
        if(_taxFee == 0 && _liquidityFee == 0 && _vaultFee == 0) return;
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 _fee = _vaultFee.add(_liquidityFee);

        uint256 _forLiquidity = contractTokenBalance.mul(_liquidityFee) / _fee;
        uint256 half = _forLiquidity.div(2);

        uint256 tokenAmountToBeSwapped = contractTokenBalance.sub(half);
        ...
        // swap tokens for ETH
        swapTokensForUSDC(tokenAmountToBeSwapped); // <- this breaks the ETH -> BB swap when swap+liquify is triggered

        // how much ETH did we just swap into?
        uint256 newBalance = IERC20(USDC).balanceOf(address(this)).sub(initialBalance);

        uint256 halfLpFee = _liquidityFee.div(2);
        uint256 swappedFee = halfLpFee.add(_vaultFee);

        uint256 usdcForLiquidity = newBalance.mul(halfLpFee).div(swappedFee);
        uint256 usdcForVault = newBalance.sub(usdcForLiquidity);

        IERC20(USDC).approve(vault, usdcForVault);
        IUSDCVault(vault).deposit(usdcForVault);

        // add liquidity to uniswap
        addLiquidity(half, usdcForLiquidity);

        emit SwapAndLiquify(half, usdcForLiquidity, half);
    }
    ...
    /// @dev    Token holders burn their BB tokens from the vault
    /// @param  from Address of the token holder
    /// @param  amount Amount of BB tokens to burn
    /// @notice Called from the vault
    function burnForShare(address from, uint256 amount) external {
        require(_msgSender() == vault);
        require(amount <= balanceOf(from));

        // _transferBothExcluded(from, ZERO, amount);
        _tokenTransfer(from, ZERO, amount, false);
        
        _tTotal -= amount;
    }

    /// @dev   Set the vault address after deployment
    /// @param _vault Address of the vault    
    function setVaultAddress(address _vault) public onlyOwner {
        console.log("---Setting vault address to: ", _vault);
        vault = _vault;
    }
