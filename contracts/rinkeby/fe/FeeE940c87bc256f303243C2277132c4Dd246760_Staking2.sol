...
contract Staking2 is Ownable, ReentrancyGuard {
    ...
    event UpdatedVerifier(address _oldVerifier, address _newVerifier, uint256 _when);
    event UpdatedPoolEndTime(uint256 _oldTime, uint256 _newTime, uint256 _when);

    constructor(address _stakingToken, uint256 _startTime, uint256 _endTime, Staking _oldStaking, address _verifier, uint256 _apr) {
    ...
    function unStake(uint256 _amount) external nonReentrant {
        migratePool(msg.sender);
        require(userStake[msg.sender] <= _amount, "Unstake:: Invalid Amount");
    ...
    function calimRewards(uint256 _amount, uint8 v, bytes32 r, bytes32 s) external nonReentrant {
        require(oldStake(msg.sender) > 0, "ClaimRewards:: Invalid Stake");
        require(!isClaimed[msg.sender], "ClaimRewards:: Already Claimed Rewards for Pool 1");
        address signedBy = ecrecover(generateSignerMessage(msg.sender, _amount), v, r, s);
    ...
    function generateSignerMessage(address _user, uint256 _amount) public view returns (bytes32) {
    ...
    function updateVerifier(address _newVerifier) external onlyOwner {
        require(_newVerifier != address(0), "UpdateVerifier:: Verifier can not be zero address");
    ...
    function updatePoolEndTime(uint256 _newTime) external onlyOwner {
        require(_newTime > block.timestamp, "UpdatePoolEndTime:: Newtime can not be past time");
