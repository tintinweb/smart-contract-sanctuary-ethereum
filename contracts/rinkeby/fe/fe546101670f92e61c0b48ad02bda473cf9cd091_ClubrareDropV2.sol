...
contract ClubrareDropV2 is Ownable, ERC721, ERC721Enumerable, ERC721URIStorage, Pausable, AccessControl, ERC721Burnable,Royalties {
   using Counters for Counters.Counter;
    ...
    constructor(address _admin, address _marketplace,string memory _contractURI,
        string memory tokenURIPrefix) ERC721("Clubrare Drop", "CBRD"){
        
        TREASURY_WALLET = _marketplace;
  ...
  function safeMint(string memory uri, uint256 value) public virtual onlyRole(MINTER_ROLE) returns(uint){
        uint256 tokenId = _tokenIdCounter.current();
        _tokenIdCounter.increment();
        _safeMint(TREASURY_WALLET, tokenId);
        _setTokenURI(tokenId, uri);
     
         if(value>0){
            _setTokenRoyalty(tokenId,TREASURY_WALLET,value);
        }
        return tokenId;
    }
    ...
    function renounceAdmin(address account)
        public virtual onlyAdmin
    {
