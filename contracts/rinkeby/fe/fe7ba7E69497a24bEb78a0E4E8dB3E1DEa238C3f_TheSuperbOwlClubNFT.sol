...
contract TheSuperbOwlClubNFT is ERC721, Ownable, Pausable {
    ...
    constructor() payable ERC721('TheSuperbOwlClub', 'TSOC')
    {
        //revealed = false;
        mintPrice = 100 ether;
        rookieMintPrice = 0.001 ether;
        veteranMintPrice = 0.002 ether;
        allProMintPrice = 0.003 ether;
        totalSupply = 1;
        //maxSupply = 1000;
        //maxPerWallet = 3;
        rookieCap = 10000;
...
/*
    function mint(uint256 quantity_,  uint256 setMintPrice_) public payable {
        ...
        {
            require(rookieCounter <= rookieCap, 'Rookie is currently out of stock. Check back again later!');
        }
        else if (mintPrice == 0.002 ether)
        {
            require(veteranCounter <= veteranCap, 'Veteran is currently out of stock. Check back again later!');
        }
        else if (mintPrice == 0.003 ether)
        {
            require(allProCounter <= allProCap, 'All-Pro is currently out of stock. Check back again later!');
        }
...
function rMint(uint256 quantity_) external payable whenNotPaused() {
        ...
        require(msg.value == quantity_ * rookieMintPrice, string(abi.encodePacked("wrong mint value. The quantity is: ", int(quantity_), "The mint price is: ", int(mintPrice), "the value sent in is: ", int(msg.value))));
        //require(totalSupply + quantity_ <= maxSupply, 'sold out');
        //require(walletMints[msg.sender] + quantity_ <= maxPerWallet, 'exceed per wallet');
