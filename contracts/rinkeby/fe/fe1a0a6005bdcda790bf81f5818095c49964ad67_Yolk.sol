...
contract Yolk is ERC20, Ownable {
    ...
    function setClaim(address _address, uint256 _amount) external {
        require(!rewardPaused,                "Claiming reward has been paused"); 
    ...
    function getPendingReward(address user) internal view returns(uint256) {
        return (Hatchlingz._walletBalanceOfCommon(user) * COMMON_RATE * (block.timestamp - lastUpdate[user]) / 86400) +
        (Hatchlingz._walletBalanceOfRare(user) * RARE_RATE * (block.timestamp - lastUpdate[user]) / 86400) +
        (Hatchlingz._walletBalanceOfLegendary(user) * LEGENDARY_RATE * (block.timestamp - lastUpdate[user]) / 86400);
