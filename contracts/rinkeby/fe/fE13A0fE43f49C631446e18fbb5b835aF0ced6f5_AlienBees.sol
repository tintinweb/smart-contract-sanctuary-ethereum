...
contract AlienBees is ERC721, Ownable, ReentrancyGuard, RandomlyAssigned{    
    ...
    constructor(uint256 _whiteListedLimit, uint256 _HPLimit, uint256 _publicLimit, uint256 _reservedNFTCount, address _reservedNFTaddress, uint256 _MAX_SUPPLY, string memory _baseURI, string memory _placeholderURI, string memory _provinanceHash, address[] memory whiteListedAddress, address[] memory _HpSaleWallets) ERC721("testing", "test")RandomlyAssigned(_MAX_SUPPLY, 1){
        require(_MAX_SUPPLY > 0, "The maximum supply value cannot be less than 1 ");
        require(_reservedNFTCount < _MAX_SUPPLY, "you cannot reserved all nft");
