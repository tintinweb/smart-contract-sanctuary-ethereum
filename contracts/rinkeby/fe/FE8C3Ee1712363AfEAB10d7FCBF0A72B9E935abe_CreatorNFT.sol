    ...
    AcceptedTokenList
{
    ...
    function initialize(uint256 maxBatchSize_, uint256 collectionSize_)
        external
        initializer
    {
        __ERC721_init("Creator NFT", "CNFT", maxBatchSize_, collectionSize_);
        __Ownable_init();
        maxPerAddressDuringMint = maxBatchSize_;
        currentIndex = 0;
        revealed = false;
        defaultURI = "ipfs://default";
    ...
    modifier callerIsUser() {
        require(
            ...
            "CreatorNFT: The caller is another contract"
        );
        _;
    }

    function preSaleMint(address token, uint256 quantity)
        external
        payable
        callerIsUser
    {
        uint256 price = uint256(salePrice[token].preSalePrice);
        require(price != 0, "CreatorNFT: price preSale not set");
        require(isTokenAccepted(token), "CreatorNFT: token payment not accept");
        require(
            isInWhiteList(msg.sender),
            "CreatorNFT: caller is not whiteList"
        );
        require(
            totalSupply() + quantity <= collectionSize,
            "CreatorNFT: reached max supply"
        );
        require(
            numberMinted(msg.sender) + quantity <= maxPerAddressDuringMint,
            "CreatorNFT: can not mint this many"
        );
        require(isPreSaleOn(token), "CretorNFT: pre sale has not begun yet");
        if (token == address(0)) {
    ...
    function publicSaleMint(
        ...
        uint256 publicPrice = uint256(salePrice[token].publicPrice);
        require(
            publicSaleKey == callerPublicSaleKey,
            "CretorNFT: called with incorrect public sale key"
        );
        require(
            isPublicSaleOn(token),
            "CretorNFT: public sale has not begun yet"
        );
        require(
            totalSupply() + quantity <= collectionSize,
            "CretorNFT: reached max supply"
        );
        require(
            numberMinted(msg.sender) + quantity <= maxPerAddressDuringMint,
            "CretorNFT: can not mint this many"
        );
        if (token == address(0)) {
            transferETH(publicPrice * quantity);
        } else {
            transferToken(token, publicPrice * quantity);
        }
        _safeMint(msg.sender, quantity);
    }
    ...
    function privateMint(uint256 quantity) public onlyOwner {
        require(
            ...
            "CretorNFT: can only mint a multiple of the maxBatchSize"
        );
    ...
    function isInWhiteList(address user) public view returns (bool) {
        for (uint256 index = 0; index < whiteList.length; index++) {
            if (whiteList[index] == user) return true;
        }
        return false;
    }
    ...
    function setPreSaleConfig(
        ...
        for (uint256 i = 0; i < _tokens.length; i++) {
            require(
                ...
                "CreatorNFT: tokens is not accept"
            );
    ...
    function transferETH(uint256 totalPrice) private {
        require(msg.value >= totalPrice, "CreatorNFT: insufficient balance");
    ...
    function transferToken(address token, uint256 amount) private {
        require(
            ...
            "CreatorNFt: Insufficient balance"
        );
...
contract AcceptedTokenList is OwnableUpgradeable {
  ...
  function addAcceptedTokens(
    address[] memory _tokens,
    uint128[] memory _preSalePrice,
    uint128[] memory _publicPrice
  )
    public onlyOwner
  {
    require(
      _tokens.length == _preSalePrice.length && _tokens.length == _publicPrice.length,
      'invalid lengths'
    );
    for (uint256 i = 0; i < _tokens.length; i++) {
      acceptedTokens.add(_tokens[i]);
      salePrice[_tokens[i]] = SalePrice({
        preSalePrice: _preSalePrice[i],
        publicPrice: _publicPrice[i]
      });
    }
  ...
  function getSalePrice(address _token) public view returns (SalePrice memory) {
    return salePrice[_token];
  }
}
