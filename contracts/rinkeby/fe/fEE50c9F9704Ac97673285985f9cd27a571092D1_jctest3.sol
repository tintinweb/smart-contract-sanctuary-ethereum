...
contract jctest3 is ERC721, Ownable {
    using Strings for uint256;
    ...
    address public constant MAIN_WALLET =
        0x49753d8cd0CD3Df5a0Ce606298C453096a236dFA;
    address public constant COMMUNITY_WALLET =
        0xab0863FC15748730DD05e958f31316d5F1FE031e;
    ...
    uint256 public constant PRE_SALE_SUPPLY = 4888;
    ...
    uint256 public constant PUBLIC_SALE_DROP_INTERVAL = 900; // 900 secs = 15mins
    ...
    function getDAPrice() public view returns (uint256) {
        ...
        return
            publicSaleStartPrice > dropPrice &&
    ...
    function preSaleBuy(
        ...
        require(tx.origin == msg.sender, "DIRECT_MINT_FROM_CONTRACT_NOT_ALLOWED");
        require(isPreSaleLive && !isPublicSaleLive(), "PRE_SALE_INACTIVE");
        require(
            preSaleAmountMinted + mintQuantity <= PRE_SALE_SUPPLY,
            "EXCEEDS_PRE_SALE_SUPPLY"
        );
        require(msg.value >= floorPrice * mintQuantity, "INSUFFICIENT_ETH_SENT");
        require(
            whitelistClaimed[msg.sender] + mintQuantity <= allowedMintQuantity,
            "EXCEEDS_PRE_SALE_ALLOCATED"
        );
    ...
    function publicBuy(bytes calldata signature, uint256 mintQuantity)
        external
        payable
    {
        require(tx.origin == msg.sender, "DIRECT_MINT_FROM_CONTRACT_NOT_ALLOWED");
        require(isPublicSaleLive(), "PUBLIC_SALE_INACTIVE");
        require(totalSupply.current() < MAX_SUPPLY, "OUT_OF_STOCK");
        require(
            publicSaleAmountMinted + mintQuantity <= publicSupply,
            "EXCEEDS_PUBLIC_SALE_SUPPLY"
        );
        require(msg.value >= getDAPrice() * mintQuantity, "INSUFFICIENT_ETH_SENT");
        require(
            publicSaleClaimed[msg.sender] + mintQuantity <= PUBLIC_SALE_MINT_LIMIT,
            "EXCEEDS_PUBLIC_SALE_ALLOCATED"
        );
        require(
            matchAddresSigner(
                hashTransaction(msg.sender, mintQuantity),
                signature
            ),
            "DIRECT_MINT_FROM_BOT_NOT_ALLOWED"
        );

        publicSaleAmountMinted += mintQuantity;
        publicSaleClaimed[msg.sender] += mintQuantity;
    ...
    function hashTransaction(address sender, uint256 mintQuantity)
        private
        pure
        returns (bytes32)
    {
        bytes32 hash = keccak256(
            abi.encodePacked(
                ...
                keccak256(abi.encodePacked(sender, mintQuantity))
            )
        );
        return hash;
    }

    function communityMint(uint256 mintQuantity) external onlyOwner {
        require(
            communityAmountMinted + mintQuantity <= COMMUNITY_SUPPLY,
            "EXCEEDS_COMMUNITY_ALLOCATED"
        );
    ...
    function setPublicStartPrice(uint256 _price) external onlyOwner {
