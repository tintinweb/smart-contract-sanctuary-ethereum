...
contract Mutant_Sloths is ERC721A, Ownable, ReentrancyGuard {
    ...
    */
    ...
    constructor()
        ERC721A("Sleepy Sloth Society Mutants ", "SSM")
    ...
    event MUTATION(uint256 indexed tokenId, uint256 indexed serumId);
    //GETTERS


   function mutateMint(uint256 serum_id) external {
       serums(serum_contract).burnForOwner(_msgSender(),serum_id,1);
       _mint(msg.sender,1);
       emit MUTATION(currTokenId,serum_id);
       currTokenId++;
   }


   function setSerumContract(address _address) public onlyOwner {
       serum_contract = _address;
...
interface serums {
    function burnForOwner(address owner, uint _id, uint _amount) external;
}
