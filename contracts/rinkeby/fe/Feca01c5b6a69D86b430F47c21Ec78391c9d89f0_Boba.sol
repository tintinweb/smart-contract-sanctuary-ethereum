...
contract Boba is ERC20, AccessControl, Pausable, ReentrancyGuard {
     ...
     * @dev This should be called BEFORE any operation that changes values used in `getPending(address)`
     * @param account Address to update the rewards of
     */
    function _updateStash(address account) internal returns (uint256) {
        stash[account] += getPending(account);
    ...
    function withdraw() external nonReentrant {
        _mint(_msgSender(), stash[_msgSender()] + getPending(_msgSender()));
