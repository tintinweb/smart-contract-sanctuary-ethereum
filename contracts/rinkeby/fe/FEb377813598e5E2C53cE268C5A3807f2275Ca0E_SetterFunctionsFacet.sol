...
contract SetterFunctionsFacet is Modifiers {
    ...
    // Set approved projects
    function setApprovedProjects(address[] memory projects) external onlyEditor {
        for (uint256 index = 0; index < projects.length; index++) {
            s._approvedProjects[projects[index]] = true;
        }
    }
    ...
    function setRoyaltyTarget(address targetAddress) external onlyEditor {
        if(address(targetAddress) == address(0))
