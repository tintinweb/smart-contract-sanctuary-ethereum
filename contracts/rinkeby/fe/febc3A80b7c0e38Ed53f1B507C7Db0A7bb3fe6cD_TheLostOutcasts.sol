...
// File: contracts/TheLostOutcasts.sol
...
contract TheLostOutcasts is ERC721A, Ownable, ReentrancyGuard {
  ...
  mapping(uint => uint) private availableTokens;
  ...
  function totalSupply() public view virtual override returns (uint256) {
        return currentMaxSupply - numAvailableTokens;
    }
...
function mint(uint256 _mintAmount) internal virtual mintCompliance(_mintAmount) mintPriceCompliance(_mintAmount) {
        ...
        require(numAvailableTokens >= _mintAmount, "ERC721r: minting more tokens than available");
        
        uint updatedNumAvailableTokens = numAvailableTokens;
        for (uint256 i; i < _mintAmount; ++i) { // Do this ++ unchecked?
            uint256 tokenId = getRandomAvailableTokenId(updatedNumAvailableTokens);
    ...
    function getRandomAvailableTokenId(uint updatedNumAvailableTokens)
        internal
        returns (uint256)
    {
    ...
    // function mintAtIndex(address to, uint index) public mintCompliance(_mintAmount) onlyOwner {
    //     require(_msgSender() == tx.origin, "Contracts cannot mint");
