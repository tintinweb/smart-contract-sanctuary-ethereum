...
contract ContractNFTFactory is OwnableUpgradeable {
  ...
  function createClone(address _newOwner, uint256 _tokenId, string memory _uri) public onlyOwner returns (address clone){
    ...
    ContractNFT(_clone).initialize(_newOwner, _tokenId, _uri); // owned by this contract
...
contract ContractNFT is OwnableUpgradeable {
    ...
    function initialize(address newOwner, uint256 _tokenId, string memory _uri) public initializer {
        __Ownable_init();
        tokenId = _tokenId;
        uri = _uri;
