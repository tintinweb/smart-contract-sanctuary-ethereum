...
contract StakeForMCSlot is Ownable {
    ...
    StakeConfig[] internal stakeconfigs;
    ...
    */
    function _stake(uint256 _amount) internal{
        ...
        stakeholders[index].address_stakes.push(Stake(msg.sender, currentConfig.tokenAddress,_amount, timestamp,currentConfig.timePeriod, _amount*currentConfig.rewardPerMillion/1000000000));
        // Emit an event that the stake has occured
        totalStaked+=_amount;
        totalRewards+=_amount*currentConfig.rewardPerMillion/1000000000;
    ...
    */
     function _withdrawStake(uint256 index) internal returns(uint256){
        ...
        uint256 total=current_stake.reward+current_stake.amount;
        require(current_stake.since+current_stake.stakePeriod >= block.timestamp, "Staking: Cannot withdraw before staking ends");
        totalStaked-=current_stake.amount;
        totalRewards-=current_stake.reward;
    ...
    function hasStake(address _staker) public view returns(StakingSummary memory){
        ...
        for (uint256 s = 0; s < summary.stakes.length; s += 1){
           totalStakeAmount = totalStakeAmount+summary.stakes[s].amount+summary.stakes[s].reward;
       }
    ...
    *
    ...
    function stakeForSlot(uint256 quantity) public {
        require(stakeconfigs.length>0,"No valid stake config");
        StakeConfig memory currentConfig=stakeconfigs[stakeconfigs.length-1];
        require(currentConfig.remainSlot>=quantity,"Insufficent remaining slot");
        ...
        MartianClub mcContract;
        require(mcContract.numberMinted(msg.sender)<= mcContract.maxPerAddressDuringMint()-quantity,"Cannot get too many slots");

        _stake(_amount);
        address[] memory addresses;
        uint256[] memory numslots;
        addresses[0]=msg.sender;
        numslots[0]=mcContract.allowlist(msg.sender)+quantity;
    ...
    function withdrawStake(uint256 stake_index)  public {
        ...
        uint256 amount_to_mint = _withdrawStake(stake_index);
        require(stakeconfigs.length>0,"No valid stake config");
        StakeConfig memory currentConfig=stakeconfigs[stakeconfigs.length-1];
        IERC20 tokenContract = IERC20(currentConfig.tokenAddress);
