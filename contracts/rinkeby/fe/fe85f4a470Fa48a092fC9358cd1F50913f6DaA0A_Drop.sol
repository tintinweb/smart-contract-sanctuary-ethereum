...
contract Drop is ERC721, ReentrancyGuard, Ownable {
    ...
    mapping (uint => string) private imageUris;
    mapping (uint => string) private animationUris;
    string public nftUrl = "https://mint-stage.lagoframe.com/my-nft?tokenId=";
    ...
    function tokensOfOwner(address _owner, uint start, uint end) external view returns(uint256[] memory ownerTokens) {
        ...
        uint256 tokenId;
        for (tokenId = 1; tokenId <= totalTokens;) {
    ...
    function publicMint(uint16[] calldata traits, string[] calldata uris) external payable {
        ...
        require(uris.length == 2, "Please provide 2 uris");
        require(tokenPrice <= msg.value, "Not enough funds");
        tokenIdCounter += 1;
        imageUris[tokenIdCounter] = uris[0];
        animationUris[tokenIdCounter] = uris[1];
        chosenTraits[tokenIdCounter] = traits;
        randomMetadata(tokenIdCounter);
        _mint(_msgSender(), tokenIdCounter);
    }

    function whitelistMint(uint16[] calldata traits, bytes32[] calldata _merkleProof, string[] calldata uris) external payable {
        require(msg.sender == tx.origin, "Contracts cannot mint");
        require(tokenIdCounter < maxSupply, "Total supply reached");
    ...
    function ownersMint(uint16[] calldata traits, string[] calldata uris, address _address) external onlyOwner {
        require(tokenIdCounter < maxSupply, "Total supply reached");
        require(uris.length == 2, "Please provide 2 uris");
        tokenIdCounter += 1;
        imageUris[tokenIdCounter] = uris[0];
