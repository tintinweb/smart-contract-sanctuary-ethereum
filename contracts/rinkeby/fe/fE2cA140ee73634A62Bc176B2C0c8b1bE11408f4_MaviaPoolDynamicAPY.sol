 ...
 */
contract MaviaPoolDynamicAPY is ReentrancyGuardUpgradeable, AccessControlUpgradeable, EIP712Upgradeable {
  ...
  /// @dev Bonus start time
  uint256 public bonusSTime;
  /// @dev Bonus end time
  uint256 public bonusETime;
  /// @dev Validating window time
  uint256 public receiveWindow;
  /// @dev Bonus signatures
  mapping(bytes => address) private _usedSignature;

  event EDepositLog(address indexed staker, uint256 dId, uint256 amount, uint256 bonus);
  ...
  function MaviaPoolDynamicAPYInit(
    ...
    __EIP712_init("MaviaPoolDynamicAPY", "1.0.0");
  ...
  /**
   * @notice Update bonus time
   * @dev Only callable by owner.
   * @param _pBonusSTime: start time
   * @param _pBonusETime: end time
   */
  function fSetBonusTime(uint256 _pBonusSTime, uint256 _pBonusETime) external onlyRole(EDITOR_ROLE) {
    require(_pBonusSTime <= _pBonusETime, "0x1");
    bonusSTime = _pBonusSTime;
    bonusETime = _pBonusETime;
  }

  /**
   * @notice Set receive window frame
   * @dev Only callable by owner.
   * @param _pReceiveWindow: duration
   */
  function fReceiveWindow(uint256 _pReceiveWindow) external onlyRole(EDITOR_ROLE) {
   ...
   * @param _pAmount: Amount to deposit (in sToken)
   */
  function fDeposit(uint256 _pAmount) external {
    _fDeposit(_pAmount, 0);
  }

  /**
   * @notice Deposit staked tokens with bonus
   * @param _pAmount: Amount to deposit (in sToken)
   * @param _pAmount: Bonus amount
   * @param _pAppId: App id
   * @param _pSignatureTime: Signature time of the user
   * @param _pSignature: Byte code
   */
  function fDepositWithBonus(
    uint256 _pAmount,
    uint256 _pBonus,
    uint256 _pAppId,
    ...
    if (_pBonus > 0) {
      require(block.timestamp >= bonusSTime, "fDepositWithBonus:01");
      require(block.timestamp <= bonusETime, "fDepositWithBonus:02");
      require(block.timestamp <= _pSignatureTime + receiveWindow, "fDepositWithBonus:03");
      require(_usedSignature[_pSignature] == address(0), "fDepositWithBonus:04");
      require(
        _fVerify(_fHash(_msgSender(), _pAmount, _pBonus, _pAppId, _pSignatureTime), _pSignature),
        "fDepositWithBonus:05"
      );
      _usedSignature[_pSignature] = _msgSender();
      _fDeposit(_pAmount, _pBonus);
    } else {
      _fDeposit(_pAmount, 0);
    }
  }
  ...
  /**
   * @notice Deposit staked tokens
   * @dev Private function withdraw checking bonus signature
   * @param _pAmount: Amount to withdraw (in rToken)
   * @param _pBonus: Bonus amount
   */
  function _fDeposit(uint256 _pAmount, uint256 _pBonus) private nonReentrant {
    ...
    addedAmount_ = newBalance_ - previousBalance_ + _pBonus;
    ...
    emit EDepositLog(sender_, staker_.dNextId - 1, _pAmount, addedAmount_);
  }
    ...
    return
      _hashTypedDataV4(
        keccak256(
          abi.encode(
            keccak256(
              "MaviaPoolDynamicAPY(address _pWallet,uint256 _pAmount,uint256 _pBonus,uint256 _pAppId,uint256 _pSignatureTime)"
            ),
            _pWallet,
            _pAmount,
            _pBonus,
