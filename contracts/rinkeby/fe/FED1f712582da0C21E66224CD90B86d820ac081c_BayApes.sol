...
contract BayApes is ERC721A, Ownable, Pausable {
  ...
  constructor() ERC721A("Bay Apes", "BAYAPES") {}

  function price() public view returns (uint256) {
    // return presaleActive ? _presalePrice : _price;
    return allowlist[msg.sender] ? _presalePrice : _price;
  }

  function mint(uint256 quantity) external payable isAllowed(msg.sender) {
  ...
  modifier isAllowed(address _address) {
    if (presaleActive) {
      require(allowlist[_address], "You are not on allow list");
    }
    _;
  }
  ...
  function ownerMint(uint256 quantity) public onlyOwner {
    require(_ownerMinted + quantity <= 50, "Owner already minted");
    _safeMint(msg.sender, quantity);
