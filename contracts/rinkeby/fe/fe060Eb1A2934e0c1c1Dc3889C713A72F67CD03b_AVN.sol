...
interface IAVN {
  ...
  function registerValidator(address t1Address, bytes calldata t1PublicKey, bytes calldata t2PublicKey) external;

  // Validator only
  function addValidator(bytes memory t1PublicKey, bytes32 t2PublicKey, uint256 t2TransactionId, bytes calldata confirmations)
      external;
  function activateValidator(bytes32 t2PublicKey, uint256 t2TransactionId, bytes calldata confirmations) external;
  function deregisterValidator(bytes32 t2PublicKey, uint256 t2TransactionId, bytes calldata confirmations) external;
...
contract AVN is IAVN, Owned {
  ...
  address immutable public AVT_ADDRESS;
  ...
  constructor(address avtAddress, LegacyValidatorsManager avnValidatorsManagerAddress, IAvnFTTreasury avnFTTreasuryAddress)
  {
    ERC1820_REGISTRY.setInterfaceImplementer(address(this), ERC777_TOKENS_RECIPIENT_HASH, address(this));
    AVT_ADDRESS = avtAddress;
    LEGACY_AVN_VALIDATORS_MANAGER = avnValidatorsManagerAddress;
    LEGACY_AVN_TREASURY = avnFTTreasuryAddress;
    ...
    quorum[0] = 2;
    quorum[1] = 3;
    unliftedLegacyStakes = 2500000 * 10**uint256(18);
  }
  ...
  // TODO: Do we want to add an isContract check here to show standard addresses can't access authorised functions?
    ...
    external
  {
    ...
    emit LogLifted(AVT_ADDRESS, address(this), checkedT2PublicKey, amount);
  }

  function registerValidator(address t1Address, bytes memory t1PublicKey, bytes calldata t2PublicKey)
    onlyOwner
    external
  {
    require(t1PublicKey.length == 64, "T1 public key must be 64 bytes");
    require(address(bytes20(uint160(uint256(keccak256(abi.encodePacked(t1PublicKey)))))) == t1Address, "T1 account mismatch");
    bytes32 checkedT2PublicKey = checkT2PublicKey(t2PublicKey);
    uint256 id = doRegisterValidator(t1Address, checkedT2PublicKey);
    ...
    emit LogValidatorRegistered(t1PublicKeyLHS, t1PublicKeyRHS, checkedT2PublicKey, id);
  }
  ...
  {
    require(t1PublicKey.length == 64, "T1 public key must be 64 bytes");
    address t1Address = address(bytes20(uint160(uint256(keccak256(abi.encodePacked(t1PublicKey))))));
    bytes32 validatorHash = keccak256(abi.encodePacked(t1PublicKey, t2PublicKey));
    verifyConfirmations(toConfirmationHash(validatorHash, t2TransactionId), confirmations);
    doStoreT2TransactionId(t2TransactionId);
    uint256 id = t1AddressToId[t1Address];
    require(isActiveValidator[id] == false, "Validator is already active");
    if (id == 0) {
      id = doRegisterValidator(t1Address, t2PublicKey);
    } else {
      require(idToT2PublicKey[id] == t2PublicKey, "Cannot change T2 public key");
    }
    isActiveValidator[id] = true;
    numActiveValidators++;
    emit LogValidatorActivated(t1Address, t2PublicKey, t2TransactionId, id);
  }

  function activateValidator(bytes32 t2PublicKey, uint256 t2TransactionId, bytes calldata confirmations)
    external
  {
    uint256 id = t2PublicKeyToId[t2PublicKey];
    require(id != 0, "Validator must be registered");
    require(isActiveValidator[id] == false, "Validator is already active");
    verifyConfirmations(toConfirmationHash(t2PublicKey, t2TransactionId), confirmations);
    doStoreT2TransactionId(t2TransactionId);
    isActiveValidator[id] = true;
    numActiveValidators++;
    emit LogValidatorActivated(idToT1Address[id], t2PublicKey, t2TransactionId, id);
  }

  function deregisterValidator(bytes32 t2PublicKey, uint256 t2TransactionId, bytes calldata confirmations)
    external
  {
    uint256 id = t2PublicKeyToId[t2PublicKey];
    require(id != 0, "Validator must be registered");
    require(isActiveValidator[id], "Validator is already inactive");
    verifyConfirmations(toConfirmationHash(t2PublicKey, t2TransactionId), confirmations);
    doStoreT2TransactionId(t2TransactionId);
    isActiveValidator[id] = false;
    numActiveValidators--;
    emit LogValidatorDeregistered(idToT1Address[id], t2PublicKey, t2TransactionId, id);
  }
  ...
  function unlockETH(address recipient, uint256 amount)
    onlyAuthorisedContract
    external
  {
    ...
    external
  {
    ...
    require(address(this).balance <= LIFT_LIMIT, "Exceeds ETH lift limit");
    ...
    external
  {
    ...
    require(uint8(leaf[ptr]) & 128 != 0, "Not a lower leaf"); // bitwise version check to ensure leaf is a signed transaction
    // TODO: confirm mystery ???? byte
    ...
    if (token == PSEUDO_ETH_ADDRESS) {
      (bool success, ) = t1Address.call{value: amount}("");
      assert(success);
  ...
  function doRegisterValidator(address t1Address, bytes32 t2PublicKey)
    private
    returns (uint256 id)
  {
    require(t1AddressToId[t1Address] == 0, "Validator is already registered");
  ...
  // reference: https://docs.substrate.io/v3/advanced/scale-codec/
  ...
  function verifyConfirmations(bytes32 msgHash, bytes memory confirmations)
    ...
    view
  {
    require(isActiveValidator[t1AddressToId[msg.sender]], "Must be an active validator");
    ...
    for (uint256 i; i < numConfirmations; i++) {
      ...
      if (v != 27 && v != 28 || uint256(s) > 0x7FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF5D576E7357A4501DDFE92F46681B20A0) {
        ...
        if (isActiveValidator[id] && confirmed[id] == false) {
