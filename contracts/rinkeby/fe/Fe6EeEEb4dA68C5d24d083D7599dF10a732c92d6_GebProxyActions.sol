...
// hevm: flattened sources of /nix/store/xnb72bvqa188nb41c35nsz3pw04mhhvc-geb-proxy-actions/dapp/geb-proxy-actions/src/GebProxyActions.sol
pragma solidity =0.6.7 >=0.6.7;

////// /nix/store/b298h1bp5ad0j7qld0lim2nb00lr3ypi-ds-auth/dapp/ds-auth/src/auth.sol
...
////// /nix/store/xnb72bvqa188nb41c35nsz3pw04mhhvc-geb-proxy-actions/dapp/geb-proxy-actions/src/GebProxyActions.sol
/// GebProxyActions.sol
...
abstract contract CollateralJoinLike_2 {
    ...
    function collateral() virtual public returns (CollateralLike_2);
...
abstract contract CoinJoinLike_2 {
    function safeEngine() virtual public returns (SAFEEngineLike_4);
    function systemCoin() virtual public returns (DSTokenLike_2);
    function join(address, uint) virtual public payable;
    function exit(address, uint) virtual public;
}
...
abstract contract TaxCollectorLike_2 {
    function taxSingle(bytes32) virtual public returns (uint);
}

abstract contract CoinSavingsAccountLike_1 {
...
contract Common {
    ...
    function _coinJoin_join(address apt, address safeHandler, uint wad) internal {
        ...
        CoinJoinLike_2(apt).systemCoin().approve(apt, wad);
        // Joins COIN into the safeEngine
        CoinJoinLike_2(apt).join(safeHandler, wad);
    }

    // Public functions
    function coinJoin_join(address apt, address safeHandler, uint wad) public {
        // Gets COIN from the user's wallet
        CoinJoinLike_2(apt).systemCoin().transferFrom(msg.sender, address(this), wad);
...
contract BasicActions is Common {
    ...
    function _getGeneratedDeltaDebt(
        ...
        uint rate = TaxCollectorLike_2(taxCollector).taxSingle(collateralType);
        require(rate > 0, "invalid-collateral-type");

        // Gets COIN balance of the handler in the safeEngine
        uint coin = SAFEEngineLike_4(safeEngine).coinBalance(safeHandler);
    ...
    function _getRepaidDeltaDebt(
        ...
        (, uint rate,,,) = SAFEEngineLike_4(safeEngine).collateralTypes(collateralType);
        require(rate > 0, "invalid-collateral-type");

        // Gets actual generatedDebt value of the safe
        (, uint generatedDebt) = SAFEEngineLike_4(safeEngine).safes(collateralType, safe);
    ...
    function _getRepaidAlDebt(
        ...
        uint coin = SAFEEngineLike_4(safeEngine).coinBalance(usr);
    ...
    function _generateDebt(address manager, address taxCollector, address coinJoin, uint safe, uint wad, address to) internal {
        ...
        if (SAFEEngineLike_4(safeEngine).canModifySAFE(address(this), address(coinJoin)) == 0) {
            SAFEEngineLike_4(safeEngine).approveSAFEModification(coinJoin);
        }
        // Exits COIN to this contract
        CoinJoinLike_2(coinJoin).exit(to, wad);
    }
    ...
    function _lockETH(
        ...
        SAFEEngineLike_4(ManagerLike(manager).safeEngine()).modifySAFECollateralization(
    ...
    function _repayDebt(
        ...
        if (own == address(this) || ManagerLike(manager).safeCan(own, safe, address(this)) == 1) {
            ...
            modifySAFECollateralization(manager, safe, 0, _getRepaidDeltaDebt(safeEngine, SAFEEngineLike_4(safeEngine).coinBalance(safeHandler), safeHandler, collateralType));
        } else {
             // Joins COIN amount into the safeEngine
            if (transferFromCaller) coinJoin_join(coinJoin, address(this), wad);
            else _coinJoin_join(coinJoin, address(this), wad);
            // Paybacks debt to the SAFE
            SAFEEngineLike_4(safeEngine).modifySAFECollateralization(
                collateralType,
                safeHandler,
                address(this),
                address(this),
    ...
    function _repayDebtAndFreeETH(
        ...
        modifySAFECollateralization(
            ...
            safe,
            -toInt(collateralWad),
            _getRepaidDeltaDebt(ManagerLike(manager).safeEngine(), SAFEEngineLike_4(ManagerLike(manager).safeEngine()).coinBalance(safeHandler), safeHandler, ManagerLike(manager).collateralTypes(safe))
        );
    ...
    function transfer(address collateral, address dst, uint amt) external {
        CollateralLike_2(collateral).transfer(dst, amt);
    }
...
contract GebProxyActions is BasicActions {
    ...
    function repayAllDebt(
        ...
        address safeHandler = ManagerLike(manager).safes(safe);
        bytes32 collateralType = ManagerLike(manager).collateralTypes(safe);
        (, uint generatedDebt) = SAFEEngineLike_4(safeEngine).safes(collateralType, safeHandler);
    ...
    function lockTokenCollateralAndGenerateDebt(
        ...
        CoinJoinLike_2(coinJoin).exit(msg.sender, deltaWad);
    }
    ...
    function openLockGNTAndGenerateDebt(
        ...
        CollateralLike_2(CollateralJoinLike_2(gntJoin).collateral()).transfer(bag, collateralAmount);
...
contract GebProxyActionsGlobalSettlement is Common {
    ...
    function _freeCollateral(
        ...
        bytes32 collateralType = ManagerLike(manager).collateralTypes(safe);
        address safeHandler = ManagerLike(manager).safes(safe);
        SAFEEngineLike_4 safeEngine = SAFEEngineLike_4(ManagerLike(manager).safeEngine());
        uint generatedDebt;
    ...
    function prepareCoinsForRedeeming(
        ...
        SAFEEngineLike_4 safeEngine = CoinJoinLike_2(coinJoin).safeEngine();
...
contract GebProxyActionsCoinSavingsAccount is Common {
    ...
    function deposit(
        ...
        uint accumulatedRates = CoinSavingsAccountLike_1(coinSavingsAccount).updateAccumulatedRate();
        ...
        CoinSavingsAccountLike_1(coinSavingsAccount).deposit(multiply(wad, RAY) / accumulatedRates);
    }
    ...
    function withdraw(
        ...
        CoinSavingsAccountLike_1(coinSavingsAccount).withdraw(savings);
        // Checks the actual balance of COIN in the safeEngine after the coinSavingsAccount exit
        uint bal = CoinJoinLike_2(coinJoin).safeEngine().coinBalance(address(this));
        // Allows adapter to access to proxy's COIN balance in the safeEngine
    ...
    function withdrawAll(
        ...
        uint savings = CoinSavingsAccountLike_1(coinSavingsAccount).savings(address(this));
        // Exits COIN from the coinSavingsAccount
        CoinSavingsAccountLike_1(coinSavingsAccount).withdraw(savings);
        // Allows adapter to access to proxy's COIN balance in the safeEngine
        if (safeEngine.canModifySAFE(address(this), address(coinJoin)) == 0) {
        ...
        CoinJoinLike_2(coinJoin).exit(msg.sender, multiply(accumulatedRates, savings) / RAY);
    }
}