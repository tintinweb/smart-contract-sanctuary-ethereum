...
contract AI is ERC721A, Ownable, ReentrancyGuard {

    mapping(address => bool) public freePublicMinted;

    bool public MintStartEnabled  = false;
    uint public MintPrice = 0.02 ether; //0.01 ETH
    uint public freeSupply = 222;
    string public baseURI;  
    uint public maxMintPerTx = 2;  
    uint public maxMint = 444;
    

    constructor() ERC721A("AI", "AI", 44, 444){}

    function mint(uint256 qty) external payable
    {
        require(MintStartEnabled , "Notice AI :  Minting Public Pause");
        require(qty <= maxMintPerTx, "Notice AI :  Limit Per Transaction");
        require(totalSupply() + qty <= maxMint,"Notice AI:  Soldout");
        if (freePublicMinted[msg.sender]) {
            require(msg.value >= MintPrice * qty, "Insufficient Funds");
        } else {
            require(totalSupply() < freeSupply, "Free Supply Sold Out");
            require(msg.value >= (MintPrice * qty) - MintPrice, "Insufficient funds!");
