...
contract CryptoChampion is ERC721Enumerable, Ownable {
    ...
    function setBaseInfo(string[] memory baseURI, uint256[] memory supply, bool isEnable) external onlyOwner {
        for (uint i = 0 ; i < supply.length ; i++){
            Items[i].supply = supply[0];
            Items[i].base_uri = baseURI[i];
            if (i == 0) Items[i].order_cnt = supply[0];
            else Items[i].order_cnt = Items[i - 1].order_cnt + supply[0];
            Items[i].order_id = 0;
        }
        currentID = 0;
    ...
    function _mintTo(address toAddress, uint256 numberOfTokens) internal {
        for (uint256 i = 0; i < numberOfTokens; i++) {
            uint256 tokenId = Items[currentID].order_id;
            if (tokenId >= Items[currentID].order_cnt)currentID++;
            Items[currentID].order_id++;
