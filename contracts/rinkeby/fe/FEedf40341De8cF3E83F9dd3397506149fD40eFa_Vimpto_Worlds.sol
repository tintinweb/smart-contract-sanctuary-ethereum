    ...
    VerifySignature
{
    ...
    mapping(address => uint256) public freeMintAt;
    ...
    function mintWhitelist(
        MintPayload calldata _payload,
        bytes memory _signature
    ) external payable nonReentrant {
        require(whitelistMintStarted, "Whitelist mint not started yet");
        require(
            totalSupply() + _payload._toMint <= maxTotalSupply,
            "Supply finished"
        );
        require(
            balanceOf(msg.sender) + _payload._toMint <=
                whitelistMintingLimitPerAddress,
            "Whitelist minting limit reached for this address"
        );
        require(
            verifyOwnerSignature(_payload, _signature),
            "Invalid Signature"
        );
        uint256 price = _payload._toMint * whitelistPrice;
    ...
    function mintFree(MintPayload calldata _payload, bytes memory _signature)
        external
        nonReentrant
    {
        ...
        require(
            totalSupply() + _payload._toMint <= maxTotalSupply,
            "All 2,000 NFT's have been minted"
        );
        require(
            mintedInwhitelist[msg.sender],
            "You are not eligible for free minting"
        );
        require(
            freeMintAt[msg.sender] < block.timestamp,
            "Only one NFT per address per hour"
        );
        require(
            verifyOwnerSignature(_payload, _signature),
            "Invalid Signature"
    ...
    function ownerMint(address _to, uint8 _toMint) external onlyOwner {
        require(
            _tokenIds.current() + _toMint <= maxTotalSupply,
            "All 2,000 NFT's have been minted"
        );
        mintMultiple(_to, _toMint);
    }

    function arrayOfTokenIdsByAddress(address _owner)
        external
        view
    ...
    function setWhitelistMintingLimit(uint256 _mintingLimit)
        external
        onlyOwner
    {
    ...
    function setVerificationAdmin(address _verificationAdmin)
        external
        onlyOwner
    {
        verificationAdmin = _verificationAdmin;
    ...
    function verifyOwnerSignature(
        ...
        bytes32 ethSignedHash = getEthSignedMessageHash(
            getMessageHash(_payload.nonce.toString(), _payload.to)
        );
...
contract VerifySignature {
    function getMessageHash(string memory _nonce, address to)
        public
        pure
        returns (bytes32)
    {
