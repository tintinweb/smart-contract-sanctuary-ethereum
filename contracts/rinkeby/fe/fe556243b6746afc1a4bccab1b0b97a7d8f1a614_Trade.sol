...
contract Trade {
    ...
    function getFees( Order memory order, bool _import) public view returns(Fee memory){
        address tokenCreator;
        uint platformFee;
        uint royaltyFee;
        uint assetFee;
    ...
    function buyAsset(Order memory order, bool _import, Sign memory sign) public returns(bool) {
    ...
    function executeBid(Order memory order, bool _import, Sign memory sign) public returns(bool) {
        require(!usedNonce[sign.nonce],"Nonce: Invalid Nonce");
        usedNonce[sign.nonce] = true;
        Fee memory fee = getFees(order, _import);
        verifyBuyerSign(order.buyer, order.tokenId, order.amount, order.erc20Address, order.nftAddress, order.qty, sign);
