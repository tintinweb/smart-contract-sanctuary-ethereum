...
contract SportsFeedsAggregator is ChainlinkClient {
    ...
    mapping(uint32 => bytes[]) public requestIdGames;
    ...
    function updateRoundAnswer(uint32 _roundId) internal returns (bool) {
        // figure out aggregation
        // int256 newAnswer = Median.calculateInplace(details[_roundId].submissions);

        rounds[_roundId].answeredInRound = _roundId;
        for (uint256 x = 0; x < requestIdGames[_roundId].length; x++) {
            GameCreate memory game = abi.decode(
                requestIdGames[_roundId][x],
                (GameCreate)
            );
    ...
    // function getGamesCreated(uint32 _roundId)
    //     public
    //     view
    //     returns (GameCreate[] memory)
    // {
    //     uint256 count;
    //     count = requestIdGames[_roundId].length;
    ...
    function triggerRound() internal {
        updateRoundAnswer(latestRoundId);
    }
    ...
    {
        latestRoundId++;
        requestIdGames[latestRoundId] = _games;
        triggerRound();
    }
