...
function buyLevel(uint buyLvl,address _refferal)public nonReentrant {
    ...
    require(user.referral != _refferal);
    user.referral = _refferal;
    require(user.currentlvl+1 >= buyLvl);
    IERC20(Token).transfer(address(this), lvl.priceToStart);
    ...
    if (lvl.refillCount == current.length){
        _sendPayment(lvl.currentLvlLine, msg.sender, buyLvl);
...
function _sendPayment(address[]storage array,address newUser,uint _lvl) internal {
    Level storage lvl = levels[_lvl];
    address paymentToUser = array[0];
    uint amountForlvl = lvl.priceToStart / lvl.refillPercent / 100;
    IERC20(Token).transfer(paymentToUser, amountForlvl);
...
function _sendToRefferalPercent(address _user) internal {
    ...
    uint amountBonus = (lvl.priceToStart / ReferallBonusPercent);
    User storage refUser = users[user.referral];
    refUser.earned = refUser.earned + amountBonus;
    IERC20(Token).transfer(reseiver, amountBonus);
