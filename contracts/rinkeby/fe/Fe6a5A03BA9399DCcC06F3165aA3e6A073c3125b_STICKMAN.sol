...
// File @openzeppelin/contracts/token/ERC721/ERC721[email protected]
...
contract STICKMAN is ERC721A, Ownable {

    using Strings for uint256;

    string public PROVENANCE;
    uint256 public constant tokenPrice = 0.04 ether; // 0.04 ETH
    uint256 public constant presaleTokenPrice = 0.031 ether; // 0.031 ETH
    uint public maxTokenPurchase = 20;
    uint public maxTokenPerWallet = 50;
    ...
    uint public presaleEndTokenNumber = 0;
    ...
    function PauseORPlay() public onlyOwner {
        isPause = !isPause;
    }

    function setPresaleEndTokenNumber(uint _endTokenNumber) public onlyOwner {
        presaleEndTokenNumber = _endTokenNumber;
    }

    function setPresaleFlag(bool _presaleFlag) public onlyOwner {
        presaleFlag = _presaleFlag;
    }

    function isPreSaleEnabled() public onlyOwner returns (bool) {
        if(presaleEndTokenNumber != 0) {
            return totalSupply() < presaleEndTokenNumber;
        } else {
            return presaleFlag;
        }
    }
    ...
    function preSaleToken(uint numberOfTokens) public payable {
        ...
        require(isPreSaleEnabled(), "Presale is not enabled now");
        require(_isWhiteListed[msg.sender] == true, "Could not pre-mint because you are not in whitelist");
    ...
    function mintToken(uint numberOfTokens) public payable {
        require(!isPause, "Paused");
        require(!isPreSaleEnabled(), "Public Sale is not enabled now");
        require(numberOfTokens <= maxTokenPurchase, "Exceeded max token purchase");
        require(_numberOfWallets[msg.sender] + numberOfTokens <= maxTokenPerWallet, "Exceeded max token purchase per wallet");
        require(totalSupply() + numberOfTokens <= MAX_TOKENS, "Purchase would exceed max supply of tokens");
