...
contract AikoVirtual is Ownable, ERC721A {
	...
	uint256 public constant HOLDER_PRICE = 0.1 ether;
	...
	function totalMinted(address who) external view returns (uint256) {
		...
		uint256 purchased = accountInfo_.freeMinted + accountInfo_.purchasedFirst + accountInfo_.purchasedSecond + accountInfo_.purchasedPublic;
		return purchased;
	}
	...
	function withdrawETH() external onlyOwner {
		...
		address MATARELLI = 0xB278d156d318B1E06c248b8Fb4c11d59011d5FB0;
		...
		address SHVRK = 0x78CEa29aCA1d3de197Ef761fF0169Beac6877124;
		address AIKO_MULTISIG = 0x354A70969F0b4a4C994403051A81C2ca45db3615;

		uint256 fullAmount = address(this).balance;
		_sendValue(payable(VINNE), _percentOfTotal(fullAmount, 0.225 gwei)); // (22.5%)
		_sendValue(payable(KYO), _percentOfTotal(fullAmount, 0.225 gwei)); // (22.5%)
		_sendValue(payable(GARRID), _percentOfTotal(fullAmount, 0.125 gwei)); // (12.5%)
		_sendValue(payable(MATARELLI), _percentOfTotal(fullAmount, 0.125 gwei)); // (12.5%)
		_sendValue(payable(CHASE), _percentOfTotal(fullAmount, 0.115 gwei)); // (11.5%)
		_sendValue(payable(KINOKO), _percentOfTotal(fullAmount, 0.075 gwei)); // (7.5%)
		_sendValue(payable(UWULABS), _percentOfTotal(fullAmount, 0.075 gwei)); // (7.5%)
		_sendValue(payable(SHVRK), _percentOfTotal(fullAmount, 0.035 gwei)); // (3.5%)
		_sendValue(payable(AIKO_MULTISIG), address(this).balance); // 30%
	}
	...
	function _startTokenId() internal virtual override pure returns (uint256) {
		return 1;
	}
