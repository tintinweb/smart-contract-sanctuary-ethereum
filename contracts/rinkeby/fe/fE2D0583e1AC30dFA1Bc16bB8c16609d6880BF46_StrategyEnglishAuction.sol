...
contract StrategyEnglishAuction is IAsynchronousStrategy {
    ...
    function canReplaceCurrentBestTakerAsk(
        Orders.TakerOrder calldata,
        Orders.MakerOrder calldata,
        Orders.TakerOrder calldata
    ...
    function canReplaceCurrentBestTakerBid(
        ...
        uint256 startPrice = abi.decode(makerAsk.strategyParams, (uint256));
        require(startPrice > 0, "StrategyEnglishAuction: INVALID_START_PRICE");

        uint256 bidPrice = abi.decode(takerBid.strategyParams, (uint256));
        uint256 bestBidPrice = abi.decode(bestTakerBid.strategyParams, (uint256));

        return (makerAsk.tokenId == takerBid.tokenId &&
    ...
    function canSettleTakerAsk(Orders.TakerOrder calldata, Orders.MakerOrder calldata)
        external
        pure
        returns (
            bool,
    ...
    function canSettleTakerBid(Orders.TakerOrder calldata takerBid, Orders.MakerOrder calldata makerAsk)
        external
        view
        override
        returns (
    ...
    {
        uint256 bidPrice = abi.decode(takerBid.strategyParams, (uint256));
        return ((makerAsk.validUntil < block.timestamp), makerAsk.tokenId, makerAsk.amount, bidPrice);
    }
