...
contract KEKW is ERC721A, Ownable {
    ...
    function mint(uint256 _amount, bytes32[] calldata proof) external payable {
        ...
        require(msg.value >= price(msg.sender, _amount, proof), "Not enough");
        require(_amount <= MAX_FREE_PER_WALLET, "Max Tx");
    ...
    function price(address minter, uint256 _amount, bytes32[] calldata proof) public view returns (uint256) {
        ...
        if(
            ...
            uint256 _current = mintCount[msg.sender];
            if(_current < MAX_FREE_PER_WALLET && totalSupply() <= FREE_ELEMENTS) _price -= min(MAX_FREE_PER_WALLET - _current, _amount) * premium;
        } 
