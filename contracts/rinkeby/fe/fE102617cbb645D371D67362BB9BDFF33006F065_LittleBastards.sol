...
contract LittleBastards is ReentrancyGuard , ERC721("The Little Bastards", "BAST") {
  ...
  uint256 public raffleSalePrice = 0.05 ether;
  uint256 public constant _totalSupply = 10000;
  uint256 public devReserved = 300;
  uint256 public raffleMax = 100;
  uint256 public raffleMinted;
  uint256 public presaleMax = 3000;
  uint256 public presaleMinted;
  string public provenance;

  // Limits
  uint256 internal walletLimit = 10;
    ...
    callerIsUser()
  {
    ...
    require(addressIndices[minter] + _amount <= walletLimit, "Max allowed minted tokens");
    require(msg.value >= _amount * itemPrice, "Incorrect amount of Eth");
    ...
    callerIsUser()
  {
    ...
    require(addressIndices[minter] + _amount <= walletLimit, "Max allowed minted tokens");
    require(msg.value >= _amount * whitelistPrice, "Incorrect amount of Eth");
    require(presaleMinted + _amount <= presaleMax, "Chosen amount would exceed whitelist alotted tokens");
    ...
    callerIsUser()
  {
    ...
    require(addressIndices[minter] + _amount <= walletLimit, "Max allowed minted tokens");
    require(msg.value >= _amount * raffleSalePrice, "Incorrect amount of Eth");
    require(presaleMinted + _amount <= raffleMax, "Chosen amount would exceed raffle alotted tokens");
  ...
  modifier tokensAvailable(uint256 _amount) {
      require(_amount <= tokensRemaining(), "Chosen amount would exceed total supply");
      _;
  }
  ...
  modifier callerIsUser() {
    require(tx.origin == msg.sender, "External contract minting not allowed");
    _;
  }
}
