...
contract TheForge is Ownable, ReentrancyGuard, Pausable {
    event SwordEnhanceSuccess(address owner, uint256 starsUsed, uint8 originalSwordId, uint8 enhancedSwordId);
    event SwordEnhanceFail(address owner, uint256 starsUsed, uint8 originalSwordId);

    IRandMath private __randContract;
    SwordOfBravery private __swordContract;
    Faith private __faithContract;

    uint32[] __breakPoints = [1, 451, 751, 901, 1000];
    uint8 private __legendaryCountLimit = 10;
    uint8 private __legendaryCount = 0;

    mapping(address => uint256) private __starBalances;
    uint256 private __priceInFaith = 2500 * (1e18);
    uint256 private __priceInEther = 0.0039 ether;
    ...
    constructor(
        ...
        // console.log('Rates0: %s/1000', (1000 * (__breakPoints[1] - __breakPoints[0])) / (__breakPoints[4]));
        // console.log('Rates1: %s/1000', (1000 * (__breakPoints[2] - __breakPoints[1])) / __breakPoints[4]);
        // console.log('Rates2: %s/1000', (1000 * (__breakPoints[3] - __breakPoints[2])) / __breakPoints[4]);
        // console.log('Rates3: %s/1000', (1000 * (__breakPoints[4] - __breakPoints[3])) / __breakPoints[4]);
        // console.log('Rates4: %s/1000', (1000 * __breakPoints[0]) / __breakPoints[4]);
    }
    ...
    function updateSeed() internal {
        unchecked {
            __randContract.seed(__starBalances[_msgSender()] + _msgSender().balance);
        }
    }
    ...
    function faithToStar(uint256 starQty) external nonReentrant nonContractCalls whenNotPaused {
        ...
        require(starQty > 0, 'Zero Qty');
        uint256 costAmount = starQty * __priceInFaith;
        require(__faithContract.balanceOf(_msgSender()) >= costAmount, 'Not enough Faith to purchase StarOfBravery');
        __faithContract.transferFrom(_msgSender(), address(this), costAmount);
    ...
    function enhanceSword(uint8 swordId, uint256 starsApplied) external nonReentrant nonContractCalls whenNotPaused {
        // console.log('!!! Enter enhanceSword(%s, %s)', swordId, starsApplied);
        require(getStarBalance(_msgSender()) >= starsApplied, 'Not enough stars');
        require(__swordContract.balanceOf(_msgSender(), swordId) > 0, 'No sword found');
        require(swordId < 4, 'Already maximum level reached');
        if (swordId == 3) {
            require(__legendaryCount < __legendaryCountLimit, 'Max number of legendaries reached');
        }

        uint256 rand = __randContract.getRand();
        uint8 currSwordId = swordId;
        uint256 starsUsed = 0;

        // console.log('> Before: From: %s To: %s, applied=%s', swordId, currSwordId, starsApplied);

        while (starsUsed < starsApplied) {
            uint256 nextRand = __randContract.nextRand(rand + starsUsed) % __breakPoints[4];

            bool hasReachMaxLevel = false;

            uint8 newSwordId = 0;
            if (nextRand >= __breakPoints[0] && nextRand < __breakPoints[1]) {
                newSwordId = 0;
            } else if (nextRand >= __breakPoints[1] && nextRand < __breakPoints[2]) {
                newSwordId = 1;
            } else if (nextRand >= __breakPoints[2] && nextRand < __breakPoints[3]) {
                newSwordId = 2;
            } else if (nextRand >= __breakPoints[3] && nextRand < __breakPoints[4]) {
                if (__legendaryCount >= __legendaryCountLimit) {
                    hasReachMaxLevel = true;
                }
                newSwordId = 3;
            } else if (nextRand >= 0 && nextRand < __breakPoints[0]) {
                if (__legendaryCount >= __legendaryCountLimit) {
                    newSwordId = 3;
                } else {
                    newSwordId = 4;
            ...
            // console.log('=== Round %s ===', starsUsed);

            // console.log('Swords: NEW:%s CURR:%s ... rand#=%s', newSwordId, currSwordId, nextRand);

            if (newSwordId > currSwordId) {
                // console.log('SwordEnhanceSuccess! Address=%s', _msgSender());
                emit SwordEnhanceSuccess(_msgSender(), starsUsed, currSwordId, newSwordId);
                currSwordId = newSwordId;
            }

            if (hasReachMaxLevel) {
                // console.log('MaxLevel Reached: ID#%s of %s/%s', newSwordId, __legendaryCount, __legendaryCountLimit);
                break; //exit loop
            }
        }

        // console.log('> After: From: %s, To: %s, rounds=%s', swordId, currSwordId, starsUsed);

        if (starsUsed > 0) {
            __starBalances[_msgSender()] -= starsUsed;
            // console.log('Reduce Start Balance! Used=%s, Balance=%s', starsUsed, __starBalances[_msgSender()]);
            // updateSeed();
        }

        if (currSwordId > swordId) {
            // console.log('Swap Swords! From: %s, To: %s', swordId, currSwordId);
            __swordContract.burn(_msgSender(), swordId, 1);
            __swordContract.mint(_msgSender(), currSwordId, 1, '0x0');
        } else {
            // console.log('SwordEnhanceFail! Address=%s', _msgSender());
            emit SwordEnhanceFail(_msgSender(), starsUsed, currSwordId);
        }
    }
