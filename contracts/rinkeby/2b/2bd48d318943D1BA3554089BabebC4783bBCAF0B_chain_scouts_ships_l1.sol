...
contract chain_scouts_ships_l1 is ERC721EnumerableUpgradeable, OwnableUpgradeable, PausableUpgradeable, NonblockingLzApp {
     ...
     function mint(uint _amount) external {
          require (maxMint >= userMinted[msg.sender]+_amount,'Error: Not Genesis Holder');
          require (finalIndex >= currentIndex + _amount, 'Max Amount Minted');
     ...
     function traverse(uint[] memory tokenIds, bytes memory _userAddress,  uint16 destinationChainId) external payable{
          burnTokens(tokenIds);
     ...
     function changeMaxMint(uint amount) external onlyOwner {
          maxMint = amount;
     }

     function changeFinalIndex(uint _finalIndex) external onlyOwner {
