...
contract PlantNursery is ERC721 {
    ...
    function plantPlant(uint _id) public {
        ...
        require(plants[_id].planted == false);
        plants[_id].planted = true;
        usedOwnerPlots[msg.sender]++;
        plants[_id].health = calculateHealthOrDead(plants[_id].health, ((uint(block.timestamp) - plants[_id].lastHealthBlock) * healthDecayMultiplier));
    ...
    function removePlant(uint _id) public {
        ...
        usedOwnerPlots[msg.sender]--;
        plants[_id].health = calculateHealthOrDead(plants[_id].health, ((block.timestamp - plants[_id].lastHealthBlock)));
        plants[_id].lastHealthBlock = block.timestamp;
    ...
    function getHealth(uint _id) public view returns (uint256) {
        if (plants[_id].size == 0 && plants[_id].planted == false) {
            return 100;
        } else if (plants[_id].planted == true) {
            // return calculateHealthOrDead(10, 5);
            return calculateHealthOrDead(plants[_id].health, ((block.timestamp - plants[_id].lastHealthBlock) * healthDecayMultiplier));
            // return plants[_id].health - ((block.timestamp - plants[_id].lastHealthBlock) * healthDecayMultiplier);
        } else if (plants[_id].size > 0) {
            return calculateHealthOrDead(plants[_id].health, ((block.timestamp - plants[_id].lastHealthBlock) * healthDecayMultiplier));
    ...
    function getSize(uint _id) public view returns (uint256) {
        if (plants[_id].planted == true) {
            return plants[_id].size + (block.timestamp - plants[_id].lastHealthBlock);
        } else {
    ...
    /** 
      ...
      **/ 
    // Helper Functions
    function calculateHealthOrDead(uint _currentHealth, uint _healthDelta) public pure returns (uint256) {
        if (_healthDelta > _currentHealth) {
            return uint256(0);
        } else {
            return _currentHealth - _healthDelta;
        }
    }

    function getPlantsByOwner(address _owner) public view returns (uint[] memory) {
        uint[] memory result = new uint[](ownerPlantCount[_owner]);
        uint counter = 0;
