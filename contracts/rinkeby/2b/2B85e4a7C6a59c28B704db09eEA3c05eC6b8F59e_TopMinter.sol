...
;
    ...
    {
        merkleRoot=_mr;
        nftPriceinETH=_nftWLPriceinETH*0.01 ether;
    ...
    function tokenURI(uint256 tokenId) external view returns (string memory) {
        require(_exists(tokenId));
        return append("https://sandbox.minter.top/token/",_name,"/",tokenId);
    }
    function append(string memory a, string memory b,string memory b1, uint c) internal pure returns (string memory) {

        return string(abi.encodePacked(a, b,b1, uintToString(c)));
