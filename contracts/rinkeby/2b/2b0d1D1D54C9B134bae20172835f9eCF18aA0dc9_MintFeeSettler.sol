...
contract MintFeeSettler is IMintFeeSettler, Ownable {
    ...
    uint256 public lastUnclaimTotal;
    ...
    function settleUnclaim(uint256 num) external {
        _settleUnclaim(num);
        lastUnclaimTotal = _balanceOfTotal();
    }

    function _settleUnclaim(uint256 num) internal {
        for (uint256 i = 0; i < num; i++) {
            if (!_unclaim.empty()) {
                Settle memory settle = _unclaim.front();
    ...
    function onDerivativeNftTransfer(uint256[] memory derivativeIds, address to) external {
        // 1.this Trade need allocation value
        uint256 needDistr = _balanceOfTotal() - lastUnclaimTotal;
        ...
        // 3. Maintain records of pending settlements
        _processPendingSettlement(derivativeIds, to, needDistr);

        // 4. update total last Unclaim amount
        lastUnclaimTotal = _balanceOfTotal();
    }

    function _processPendingSettlement(
        ...
        for (uint256 i = 0; i < metas.length; i++) {
            ...
            for (uint256 j = 0; j < licenses.length; j++) {
                ...
                settleTemp[i].recipient = IERC721(ILicenseToken(licenses[j].token).originToken()).ownerOf(
                    licenses[j].id
                );
                settleTemp[i].value =
                    (needDistr * tempEarnPoint) /
            ...
            //Less than 5% of the earnedPoint will be sent to the recipient

            if (totalEarnPoint < _config.getUint256(CONFIG_DEFAULT_MAX_LICESE_EARN_POINT_KEY)) {
                uint256 seed2ToValue = (needDistr *
                    (_config.getUint256(CONFIG_DEFAULT_MAX_LICESE_EARN_POINT_KEY) - totalEarnPoint)) /
                    _config.getUint256(CONFIG_DEFAULT_MAX_LICESE_EARN_POINT_KEY);
                if (seed2ToValue >= address(this).balance)
                    IWeth(_weth).withdraw(IERC20(_weth).balanceOf(address(this)));

                //solhint-disable avoid-low-level-calls
                (bool success, ) = to.call{value: seed2ToValue}("");
                if (!success) {
                    _unclaim.pushBack(Settle({recipient: to, value: seed2ToValue, index: ++index}));
                }
            }
        }
    }
        ...
        returns (uint256 totalFee)
    {
        ...
        for (uint256 i = 0; i < amounts.length; i++) {
            ...
            totalFee += (baseF * amounts[i] * (expiredAts[i] - block.timestamp));
        }
    }
