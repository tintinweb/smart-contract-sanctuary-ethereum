...
import {TickMath} from "../dependencies/uniswap-v3/TickMath.sol";
import {LiquidityAmounts} from "../dependencies/uniswap-v3/LiquidityAmounts.sol";
import {INonfungiblePositionManager} from "../dependencies/uniswap-v3/INonfungiblePositionManager.sol";
...
contract OmniOracle is IOmniOracle {
    ...
    address public immutable UNISWAP_V3_LP;
    ...
    constructor(
        ...
        UNISWAP_V3_LP = 0xC36442b4a4522E871399CD717aBDD847Ab11FE88;
        positionManager = INonfungiblePositionManager(UNISWAP_V3_LP);
    }
        ...
        returns (uint256)
    {
        ...
        if (asset == BASE_CURRENCY) {
            ...
            return calcUniswapV3Price(tokenId);
        } else {
            int256 price = source.latestAnswer();
            if (price > 0) {
                return uint256(price);
    ...
    function calcUniswapV3Price(uint256 tokenId) view internal returns (uint256) {
                (,,
        address token0, 
        address token1,,
        int24 tickLower, 
       ...
       function getPrices(address tokenA, address tokenB) internal view returns (uint priceA, uint priceB) {
        uint _priceA = getAssetPrice(tokenA);
        uint _priceB = getAssetPrice(tokenB);
        
        priceA = 10**IERC20Detailed(tokenA).decimals();
        priceB = _priceB * (IERC20Detailed(tokenB).decimals()) / (_priceA);
    }
    ...
    function getSqrtPriceX96(uint priceA, uint priceB) internal pure returns (uint) {
    ...
    function calculatePrice(uint amountA, uint amountB, address tokenA, address tokenB) internal view returns (uint price) {
        price = (amountA * (getAssetPrice(tokenA))) + (amountB *(getAssetPrice(tokenB)));
    }
...
library LiquidityAmounts {
    ...
    function getLiquidityForAmount0(
        ...
        uint256 intermediate = (sqrtRatioAX96 * sqrtRatioBX96) / FixedPoint96.Q96;
        return toUint128((amount0 * intermediate) / (sqrtRatioBX96 - sqrtRatioAX96));
    }
    ...
    function getLiquidityForAmount1(
        ...
        if (sqrtRatioAX96 > sqrtRatioBX96) (sqrtRatioAX96, sqrtRatioBX96) = (sqrtRatioBX96, sqrtRatioAX96);
        return toUint128((amount1 * FixedPoint96.Q96) / (sqrtRatioBX96 - sqrtRatioAX96));
    }
