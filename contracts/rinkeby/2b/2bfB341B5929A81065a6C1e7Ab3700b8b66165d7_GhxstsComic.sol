...
contract GhxstsComic is ERC721Enumerable, Ownable, ReentrancyGuard, ERC721Burnable {
  ...
  function getChapter(uint256 chapterId) public view returns (Chapter memory _chapter) {
    ...
    _chapter.name = chapterName[chapterId];
    _chapter.image = chapterImage[chapterId];
    _chapter.description = chapterDescription[chapterId];
    _chapter.merkleRoot = chapterMerkle[chapterId];
    return _chapter;
  }
  ...
  function createChapter(
    ...
    chapterName[chapterId] = name;
    chapterDescription[chapterId] = description;
    chapterImage[chapterId] = image;
   ...
   */
  function updateChapterImage(uint256 chapterId, string calldata url) external chapterExists(chapterId) onlyOwner {
    Chapter memory chapter = getChapter(chapterId);
    require(!chapter.frozen, "Chapter is frozen");
    chapterImage[chapterId] = url;
  }
  ...
  function auctionMint(uint256 chapterId, uint256 amount) external payable chapterExists(chapterId) callerIsUser {
    ...
    Chapter memory chapter = getChapter(chapterId);
    require((chapter.supply + amount) <= chapter.maxSupply, "Max purchase supply exceeded");
    require(auctionMinted[msg.sender] + amount <= MAX_MINT, "Max mint qty exceeded");
    uint256 totalCost = getAuctionPrice(chapterId, _saleStartTime) * amount;
    auctionMinted[msg.sender] = auctionMinted[msg.sender] + amount;
