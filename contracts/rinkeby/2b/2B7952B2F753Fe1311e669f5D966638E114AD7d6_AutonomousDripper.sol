...
import "EmissionsDripper.sol";

/**
 * @title The AutonomousDripper contract
 * @notice The EmissionsDripper contract made Chainlink Keeper-compatible;
 * removing the need to monitor the interval and/or call release manually
 */
contract AutonomousDripper is EmissionsDripper, KeeperCompatibleInterface {
     ...
     * @return An array of asset addresses of which the balance is at least 1
     ...
     */
    function getAssetsHeld() public view returns (address[] memory) {
        ...
        if ((block.timestamp - lastTimestamp) > interval) {
            ...
            for (uint idx = 0; idx < assetsHeld.length; idx++) {
                if (IERC20(assetsHeld[idx]).balanceOf(address(this)) > 0) {
                    EmissionsDripper.release(assetsHeld[idx]);
                }
            }
        }
    }
