...
contract MutantCats is ERC721Enumerable, ReentrancyGuard {
    using Strings for uint256;
    //token Index tracker 
    ...
    //constructor args 
    constructor(string memory baseURI_) ERC721("name_", "symbol_") {
        baseURI = baseURI_;
    }
    ...
    function mint(uint256 _quantity) payable public nonReentrant {
        ...
        for(uint256 i=0; i< _quantity; i++){
            _safeMint(_msgSender(), 1 + totalSupply());
        }
        (bool sent,) = payable(owner).call{value: msg.value}("");
        require(sent,"Ether transfer to multisig wallet failed");
