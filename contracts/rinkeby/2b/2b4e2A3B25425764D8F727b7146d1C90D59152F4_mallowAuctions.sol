...
contract mallowAuctions is Ownable{
    ...
    function getXAuctions(uint256 _start, uint256 _x, address _wallet) public view returns 
    (AllInfoRequest[] memory, AllInfoRequest[] memory, AllInfoRequest[] memory){
        ...
        for(uint256 i = _start; i < _x + _start; i++){
            ...
            Xopened[t].wlArray = getAuctionWhitelists(i);
            Xopened[t].price = getCurrentPrice(i);
            Xopened[t].isWL = isWhitelisted(_wallet,i);
            t++;
        }
        t = 0;
        for(uint256 i = _start; i < _x + _start; i++){
            ...
            Xclosed[t].wlArray = getAuctionWhitelists(i);
            Xclosed[t].price = getCurrentPrice(i);
            Xclosed[t].isWL = isWhitelisted(_wallet,i);
            t++;
        }
        for(uint256 i = 0; i < upcoming.length; i++){
            Xupcoming[i].auction = upcoming[i];
            Xupcoming[i].wlArray = getAuctionWhitelists(i);
            Xupcoming[i].price = getCurrentPrice(i);
            Xupcoming[i].isWL = isWhitelisted(_wallet,i);
        }
