...
contract NeonsColorChangingNoun is ERC721Delegated {
    ...
    function tokenURI(uint256 tokenId) external view returns (string memory) {
        uint256 number = unsafeGetRandBetweenZeroInclusiveAndThisExclusive(4);
        return string(abi.encodePacked(baseURI, number));
    }

    function testConcat(uint256 tokenId) external view returns (string memory) {
        return string(abi.encodePacked("test", tokenId.toString()));
    }

    function testConcatRandNumber(uint256 tokenId) external view returns (string memory) {
        uint256 number = unsafeGetRandBetweenZeroInclusiveAndThisExclusive(4);
        return string(abi.encodePacked(number.toString(), tokenId.toString()));
    }

    function unsafeGetRandomNumber() public view returns (uint256) {
    ...
    function unsafeGetRandBetweenZeroInclusiveAndThisExclusive(uint256 number) public view returns (uint256) {
