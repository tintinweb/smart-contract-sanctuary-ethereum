...
contract TestContract is ERC721A, Ownable {
  ...
  function mint(address minterAddress, uint256 _count) public payable saleIsOpen {
    uint256 mintIndex = totalSupply();

     if (msg.sender != owner()) {
      require(isActive, "Sale is not active currently.");
      require(balanceOf(minterAddress) + _count <= maximumAllowedTokensPerWallet, "Max holding cap reached.");
    }
    ...
    _safeMint(minterAddress, _count);
