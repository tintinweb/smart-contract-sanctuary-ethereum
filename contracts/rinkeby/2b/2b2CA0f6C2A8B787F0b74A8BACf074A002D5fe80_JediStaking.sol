...
contract JediStaking is Ownable, ReentrancyGuard {  
   ...
   /// @return true if transaction is successful
    ...
    function claimEmergency() external nonReentrant returns(bool) {
        Staker storage c = _stakers[msg.sender];
    ...
    function setSaferDepositFee(uint256 fee) external onlyOwner returns(bool) {
        require(fee > 0 && fee <= 20, "Fee must be between 1-99 range");
        _depositFee = fee;
