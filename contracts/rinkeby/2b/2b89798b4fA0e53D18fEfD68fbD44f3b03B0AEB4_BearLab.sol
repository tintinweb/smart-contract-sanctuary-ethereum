...
contract BearLab is ERC721, Ownable {
    ...
    uint256 private m_SerumSupply = 4444;
    ...
    constructor() ERC721("Bear Lab", "BEARLAB") {
    ...
    function mint(uint256 _numberOfTokens) public {
        ...
        require(
            balanceOf(_msgSender()).add(_numberOfTokens) <= m_MaxMintPerAddress,
            "Over Max Mint per Address"
        );
        IERC721 badbear = IERC721(m_Badbear);

        require(badbear.balanceOf(_msgSender()) > 0);
    ...
    function setSerumSupply(uint256 _maxSupply) external onlyOwner {
        m_SerumSupply = _maxSupply;
    }

    function getSerumSupply() external view returns (uint256) {
        return m_SerumSupply;
    }
