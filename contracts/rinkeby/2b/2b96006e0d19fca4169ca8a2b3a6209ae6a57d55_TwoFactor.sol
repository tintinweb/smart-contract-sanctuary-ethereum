...
/** 
 * @dev This contract module is inspired from https://github.com/OpenZeppelin/openzeppelin-contracts/blob/master/contracts/access/Ownable.sol
...
contract TwoFactor is Ownable, ERC721Holder, ERC1155Holder {
    ...
    address DAO_MULTI_SIG = 0xacF8922519cA7D5ce5FD85118Bb4BaFA678f9C80;
    bool private isInitialized = false;
    
    function init(address _sender, bytes32 _encryptedPassword) external encryptedPasswordNotNull(_encryptedPassword){
    ...
    function transferERC20AssetsToWallet(
        string memory _oldSignedPassword,
        bytes32 _newEncryptedPassword,
        address[] memory erc20tokenAddressList
    )
        external
        onlyOwner
        passwordMatchAndNewUpdated(_oldSignedPassword, _newEncryptedPassword)
    ...
    modifier passwordMatchAndNewUpdated(
        ...
        //TODO: string _oldPassword = "Fetch from signed of only owner";
        bytes32 _passwordSent = keccak256(abi.encodePacked(_oldSignedPassword));
        require(
    ...
    // If forgot password, you have to send funds to DAO and get them back after appropriate KYC done
    function sendERC20FundsForRecoveryAfterKYC(
        address[] memory erc20tokenAddressList
    ) external onlyOwner {
        _transferERC20FundsToAddress(DAO_MULTI_SIG, erc20tokenAddressList);
    }

    // If forgot password, you have to send funds to DAO and get them back after appropriate KYC done
    function sendERC721FundsForRecoveryAfterKYC(
        address tokenAddress,
        uint256 tokenId
    ) external onlyOwner {
        _transferERC721FundsToAddress(DAO_MULTI_SIG, tokenAddress, tokenId);
    }

    // If forgot password, you have to send funds to DAO and get them back after appropriate KYC done
    function sendERC1155FundsForRecoveryAfterKYC(
        address tokenAddress,
        uint256 tokenId
    ) external onlyOwner {
        _transferERC1155FundsToAddress(DAO_MULTI_SIG, tokenAddress, tokenId);
    }


    // If forgot password, you have to send funds to DAO and get them back after appropriate KYC done
    function sendNativeFundsForRecoveryAfterKYC() external onlyOwner {
        _transferNativeFundsToAddress(payable(DAO_MULTI_SIG));
    }
    ...
    function _transferERC721FundsToAddress(
        ...
        IERC721(tokenAddress).transferFrom(address(this), toAddress, tokenId);
    }
    ...
    function _transferERC20FundsToAddress(
        ...
        for (uint256 i = 0; i < tokenAddressList.length; i++) {
            ...
            if (balance > 0) {
                IERC20(tokenAddressList[i]).transfer(toAddress, balance);
            }
        }
    }
