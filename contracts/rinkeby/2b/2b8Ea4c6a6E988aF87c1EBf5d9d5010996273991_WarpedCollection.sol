 ...
 * PROVENANCE hash is published here. This will contain the SHA256 hash of all
 * SHA256 hashes of the animation files put together in order and in bytes form.
 ...
 * raffles, prizes, rewards, etc. The treasury account will receive the newly
 * minted tokens.
 *
 * The owner account is able to set the treasury account and to withdraw the eth
 * funds from minting in this smart contract into the treasury account. The
 * owner account is set as the default treasurt account on contract deployment.
 *
...
contract WarpedCollection is ERC721, Ownable, Pausable {
    ...
    // The account that will receive the funds and the owner minted tokens.
        ...
        ERC721("Warped", "WARP")
    {
        ...
        // Init the treasury account with the owner account.
    ...
    /**
     * @dev Makes sure this doesn't run when paused.
     */
    ...
    */
    ...
    /**
     * @notice Sets the treasury account, which is the account that can withdraw
     * the eth stored in the contract from minting.
     *
     * @param treasury - the address of the treasury account.
     */
    ...
    /**
     * @notice Transfers all the minting funds to the treasury account. 
     */
     ...
     *      resulting tokens will be held by the treasury account. This should
     *      fail during phases 0 and 4, when minting is deactivated for everyone.
     *         
     * @param mintNum the number of tokens to be minted.
     */
    function ownerMint(uint8 mintNum) public onlyOwner whenNotPaused {
        ...
        for (uint32 i = 0; i < mintNum; i++) {
            ...
            // Mint the token into the treasury account.
            _safeMint(_treasury, newTokenId);
        }
