...
contract BurnKing is ERC721Enumerable, Ownable {
   ...
   constructor(address _publicKey)ERC721("Burn Spot", "BSP"){
    ...
    function transferNftFromPool(address _burner, uint256 _burningUSD, Pools _poolType) internal {
     ...
     function burnEther(bytes memory _signature, string memory _concatinatedParams, uint256 _timestamp,uint256 _usdCost, uint256 _poolType) external payable{
      ...
      bool isVerified = isCorrectParams(_signature, _concatinatedParams);
     ...
     function burnERC20(bytes memory _signature, string memory _concatinatedParams, uint256 _tokenCount, uint256 _timestamp, uint256 _usdCost, uint256 _poolType, address _tokenContractAddress) external{
       require(block.timestamp - _timestamp > deadlineTime, "Transaction expired");
   ...
   function isCorrectParams(bytes memory _signature, string memory _concatenatedParams) public view returns(bool) {
        bytes32 message = prefixed(keccak256(abi.encodePacked(_concatenatedParams)));

        return recoverSigner(message, _signature) == publicKey;
    }
