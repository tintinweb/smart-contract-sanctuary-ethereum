...
contract ERC721A is Context, ERC165, IERC721, IERC721Metadata {
    ...
    function approve(address to, uint256 tokenId) public override {
        ...
        require(to != owner, "Approving current Owner!");

        require(_msgSender() == owner || !isApprovedForAll(owner, _msgSender()), "Not approved or owner!");
    ...
    function setApprovalForAll(address operator, bool approved) public virtual override {
        require(operator != _msgSender(), "Caller cannot be operator!");
    ...
    function safeTransferFrom(
        ...
        _transfer(from, to, tokenId);
        require(to.isContract() || _checkContractOnERC721Received(from, to, tokenId, _data), "Not an ERC721 Reciever");
    }
    ...
    function _transfer(
        ...
        if (to == address(0)) {
            revert("Transferring to zero address!");
        }
...
contract nft is ERC721A, Ownable {
    ...
    uint private constant mintCostGenesisSale = 0.05 ether;
    uint private constant mintCostPresale = 0.075 ether;
    uint private constant minMintCostPublicSale = 0.8 ether;
    ...
    uint256 public constant maxMintPresale = 10;

    //maximum amount of mints allowed per person
    uint256 public constant maxMintGenesis = 2;
    ...
    address payable public teamWallet = payable(0x074bbE19ffB04639B5C38e3AB862309A5ACD725C);
    address payable public LaunchpadWallet = payable(0x02d274Ad889bf84E5431293B9DCE985A6C12703c);
    address payable public DevWallet = payable(0xe075aF8537890847539b1F1a74E80FF11C4B9a3C);
    ...
    constructor() ERC721A('Who is Emily?', 'EMILY') {

        maxTotalTokens = 10000;
        maxTokensPresale = 1000;
        maxTokensGenesisSale = 100;

        unrevealedURI = "https://arweave.net/0wt6ZWanbUN4ajXy-fbQFeb4Tk2ak4KS6Js7eJk4j_I";
    ...
    */
    ...
    function genesisMint(uint256 number, uint8 _v, bytes32 _r, bytes32 _s) onlyValidAccess(_v,  _r, _s) public payable {
        ...
        require(mintsPerAddress[msg.sender] + number <= maxMintGenesis, "Maximum 2 Mints per Address allowed!");
    ...
    function presaleMint(uint256 number, uint8 _v, bytes32 _r, bytes32 _s) onlyValidAccess(_v,  _r, _s) public payable {
        ...
        require(mintsPerAddress[msg.sender] + number <= maxMintPresale, "Maximum 10 Mints per Address allowed!");
    ...
    function withdraw() public onlyOwner {
        ...
        _withdraw(payable(communityWallet), (balance * 15) / 100);
        _withdraw(payable(teamWallet), (balance * 15) / 100);
        _withdraw(payable(variable), (balance * 10) / 100);
        _withdraw(payable(LaunchpadWallet), (balance * 15) / 100);
        _withdraw(payable(DevWallet), (balance * 30) / 100);
        _withdraw(payable(owner()), accountBalance()); //to avoid dust eth, gets a little more than 15%
    }
  ...
  function bulkAirdrop(address[] memory _to, uint256 _startFrom) public returns(uint) {
        uint256 counter = _startFrom;
