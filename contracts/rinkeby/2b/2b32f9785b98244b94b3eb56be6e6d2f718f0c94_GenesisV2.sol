    ...
    ReentrancyGuardUpgradeable
{
    ...
    function mintTo(
        ...
        bytes32 validString = keccak256(abi.encodePacked(msg.value));
        ...
        require(status, "$GenesisV1: cannot purchase the token");
    ...
    function airdrop(address[] calldata _addresses) external virtual onlyOwner {
        require(saleRange[1] >= genesisMemberCounter, "GenesisV1: overflow supply");
    ...
    function checkMessageHash(address _address, bytes32 _hash, string memory message, bytes memory signature) external view returns(bool, bool, uint256){
        bool status = SignatureCheckerUpgradeable.isValidSignatureNow(
            _address,
            _hash,
            signature
        );
        bytes memory a = bytes(message);
        bytes32 validString = keccak256(abi.encodePacked('\x19Ethereum Signed Message:\n', a.length, message));
        return (_hash == validString, status, a.length);
    }
