...
contract FLUIDnftTest is ERC721Enumerable, ERC721Royalty, ERC721URIStorage, Ownable, ReentrancyGuard {
    ...
    ) ERC721("FLUID DAO NFT Test", "FLUIDidTest") {
        _tokenURI = __tokenURI;
        royaltyReceiver = _royaltyReceiver;

        _setDefaultRoyalty(_royaltyReceiver, 1000); // 10%
        uint256 currentSupply = 0;
        for(uint i=0; i< initialMintAmount; i++) {
            _mint(initialMintRecipient, ++currentSupply);
        }
    }
        ...
        returns (string memory)
    {
        ...
        return "https://gateway.pinata.cloud/ipfs/QmRcjhxsPTYhhnNZh8ENYwxkqp6T428Ccrm8XhcyLrdaXj";
    }
}
