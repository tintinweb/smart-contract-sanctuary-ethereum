    ...
    ERC2981Upgradeable
{
    ...
    function preSaleMint(bytes32[] memory typeMints, uint256[] memory quantities)
        external
        nonReentrant
        callerIsUser
        whenNotPaused
    ...
    function publicSaleMint(
        ...
        require(
            numberPublicMint[msg.sender] + totalMint <= maxPerAddressDuringMint,
            "Shiniki: can not mint this many"
        );
        numberPublicMint[msg.sender] += totalMint;
    ...
    function lock(uint256[] memory tokenIds) public {
        for (uint256 i = 0; i < tokenIds.length; i++) {
            ...
            _ownerships[tokenIds[i]].startTimestampLock = uint64(
                block.timestamp
            );
    ...
    function claim(
        ...
        require(
            SIGNATURE_VERIFIER.verifyClaim(token, receiver, amount, nonce, signature),
