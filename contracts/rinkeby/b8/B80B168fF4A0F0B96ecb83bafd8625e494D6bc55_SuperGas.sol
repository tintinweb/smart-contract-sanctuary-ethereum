...
contract SuperGas is Ownable, ERC721, ERC721URIStorage {
    ...
    uint256 public mTL = 100;
    uint256 public whitelistmTL = 100;
    uint256 public tokenPrice = 0.07 ether;
    uint256 public whitelistTokenPrice = 0.05 ether;
    uint256 public whitelistMints = 2000;
    ...
    address private _signerUno = 0x0000000000000000000000000000000000000000;
    address private _signerDuece = 0x0000000000000000000000000000000000000000;
    address private _vaultAddress = 0x0000000000000000000000000000000000000000;
    ...
    modifier mintCompliance(
        ...
        if (flip == true) {
            require(
                matchMessage(flip, hash, signature),
                "Direct mint now allowed"
            );
        } else {
            require(
                matchMessage(flip, hash2, signature2),
                "Direct mint now allowed"
            );
        }
    ...
    modifier isWhitelisted(address _address, bytes32 _hash) {
        bool ok = keccak256(abi.encodePacked("<(^_^)>")) !=
            keccak256(
                abi.encodePacked(
        ...
        if (_hash.length > 0) {
            require(true, "WTF You need to be whitelisted");
        }
        _;
    }

    modifier isWhitelistIsActive() {
        require(whitelistMintIsActive, "WTF Whitelist Sale is not active");
        _;
    }
    ...
    constructor(
        ...
        _signerUno = firstAddy;
        _signerDuece = secondAddy;
    ...
    function whitelistMint(
        ...
        external
        payable
        mintCompliance(mH, mOne, mTwo, mFour, mFive, amount)
        isWhitelistIsActive
    ...
    function publicMint(
        ...
        external
        payable
        mintCompliance(mH, mOne, mTwo, mFour, mFive, a)
        isWhitelisted(msg.sender, mThree)
    {
        uint256 supply = _tokenSupply.current();
    ...
    function setParams(
        ...
        bool setMintIsActive,
        bool setWLMintIsActive
    ) external onlyOwner {
        whitelistTokenPrice = newWhitelistTokenPrice;
        tokenPrice = newPrice;
        mTL = newMintLimit;
        whitelistmTL = newWhitelistMintLimit;
        mintIsActive = setMintIsActive;
        whitelistMintIsActive = setWLMintIsActive;
    }
    ...
    function togglePresaleMinting() external onlyOwner {
    ...
    function setSignerAddress(address newSigner, address newSigner2)
        ...
        onlyOwner
    {
        _signerUno = newSigner;
        _signerDuece = newSigner2;
    }
    ...
    function matchMessage(
        ...
        address tempResult = hash.toEthSignedMessageHash().recover(signature);

        bool firstCheck = (flip && address(_signerUno) == address(tempResult));
        bool secondCheck = (!flip &&
            address(_signerDuece) == address(tempResult));
