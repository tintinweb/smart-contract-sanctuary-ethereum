...
// Part: OpenZeppelin/openzeppelin-[email protected]/IERC165Upgradeable
...
interface ITaygenNFT is IERC721Upgradeable {
   ...
   function mint(address _to) external returns (uint256);
    ...
    function tokenURI(uint tokenId, uint _USDCBalance, uint _tUSDCAmount, uint _conversionRate, uint _usdcFee, uint _withdrawTime) external view returns (string memory);
}
    ...
    ERC721PausableUpgradeable
{
    ...
    */
    function tokenURI(uint _tokenId, uint _USDCBalance, uint _tUSDCBalance, uint _conversionRate, uint _usdcFee, uint _withdrawTime) external view override returns (string memory) {
        return
        _tokenURI(
            _tokenId,
            _USDCBalance,
    ...
    function _tokenURI(uint _tokenId, uint _USDCBalance, uint _tUSDCAmount, uint _conversionRate, uint _usdcFee, uint _withdrawTime) internal pure returns (string memory output) {
        ...
        output = string(abi.encodePacked(output, "conversion rate ", toString(_conversionRate), '</text></svg>'));
        output = string(abi.encodePacked(output, "USDC Withdrawal Fee ", toString(_usdcFee), '</text></svg>'));
