...
contract ADStaking is ERC721Holder, Ownable, ReentrancyGuard {
    ...
    function _calculateReward() private view returns (uint256) {
        return ((min(block.timestamp, lastWrldClaim + (distributionPeriod * 1 days)) - max(lastWrldClaim, lastClaimTime[msg.sender])) / 1 days) * dailyReward * staked[msg.sender].length;
    }

    function _calculateWhaleReward() private view returns (uint256) {
        uint256 multiplier = staked[msg.sender].length / 25;

        return ((min(block.timestamp, lastWrldClaim + (distributionPeriod * 1 days)) - max(lastWrldClaim, lastClaimTime[msg.sender])) / 1 days) * whaleDailyReward * multiplier;
    }
    
    function claim() external nonReentrant {
        uint256 reward = _calculateReward() + accumulated[lastWrldClaim][msg.sender];
        uint256 whaleReward = _calculateWhaleReward() + whaleAccumulated[lastWrldClaim][msg.sender];
        ...
        _wrldToken.transferFrom(address(this), msg.sender, reward + whaleReward);
    }
    ...
    function _stake(uint256[] calldata ids_, address to_) private {
        accumulated[lastWrldClaim][to_] += _calculateReward();
        whaleAccumulated[lastWrldClaim][to_] += _calculateWhaleReward();
    ...
    function _unstake(uint256[] calldata ids_, address to_) private {
        accumulated[lastWrldClaim][msg.sender] += _calculateReward();
        whaleAccumulated[lastWrldClaim][msg.sender] += _calculateWhaleReward();
    ...
    function getCurrentBalance() external view returns (uint256) {
        return accumulated[lastWrldClaim][msg.sender] + whaleAccumulated[lastWrldClaim][msg.sender] + _calculateReward() + _calculateWhaleReward();
    }
    ...
    function claimNFTWorldsReward(uint256 distributionPeriod_) external onlyOwner {
        ...
        _wrldToken.transferFrom(address(this), owner(), currentBalance - claimableRewards - whaleRewards);
