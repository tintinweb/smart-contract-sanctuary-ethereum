...
contract DAObuy is ReentrancyGuardUpgradeable, ERC721HolderUpgradeable{

    address public nftContract;
    uint256 public tokenId;
    address public admin;

    constructor(address _nft, uint256 _id, address _admin) {
        nftContract = _nft;
        tokenId = _id;
        admin = _admin;
    }
    ...
    ) external nonReentrant onlyAdmin(msg.sender) payable returns(bool, bytes memory ) {
        require(
    ...
    function transferToAdmin() public onlyAdmin(msg.sender) returns(address) {
        IERC721(nftContract).approve(admin, tokenId);
        IERC721(nftContract).transferFrom(address(this), admin, tokenId);
        return(_getOwner());
    }
    ...
    function receiveEther() public payable returns(uint256) {
        emit Received(msg.sender, msg.value);
