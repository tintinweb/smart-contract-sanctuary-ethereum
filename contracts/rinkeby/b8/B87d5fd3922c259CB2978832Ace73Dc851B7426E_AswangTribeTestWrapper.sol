...
error AswangTribe_InsufficientReserveSupply();
...
contract AswangTribe is ERC721ABase {
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        ...
        if (tokenId < MAX_MINTABLE) {
            metadataBase = revealed ? metadata.placeholderURI : rankURIs[rank(tokenId)];
        } else {
     ...
     * @param index TODO
     */
    function setRankURI(string calldata uri, uint256 index) external lockable onlyOwner {
        rankURIs[index] = uri;
    }
    ...
    function setPraying(uint256[] calldata tokenIds, bool pray) public {
        ...
        for (uint256 i; i < tokensLength;) {
            if (ownerOf(tokenIds[i]) != _msgSender()) revert AswangTribe_SenderNotTokenOwner();
    ...
    function reserve(uint256 amount, address receiver) external onlyOwner {
        ...
        if (reserved + amount > MAX_RESERVED) revert AswangTribe_InsufficientReserveSupply();
    ...
    function mint(uint256 amount, bytes memory signature) public override payable {
        if (msg.value > 0 && msg.value != mintConfig.price * amount) revert ERC721ABase_InvalidValue();
        if (mintConfig.signer != ECDSA.recover(
            ECDSA.toEthSignedMessageHash(keccak256(abi.encodePacked(_msgSender(), msg.value == 0))),
            signature
     ...
     * @param amount TODO
     */
    function summon(uint256 tokenId, uint256 amount) public {
        if (_totalMinted() + amount > MAX_SUPPLY) revert ERC721ABase_InsufficientSupply();
        ...
        for (uint256 i; i < amount;) {
            unchecked { emit Summon(nextTokenId + i++); }
        }
    }
