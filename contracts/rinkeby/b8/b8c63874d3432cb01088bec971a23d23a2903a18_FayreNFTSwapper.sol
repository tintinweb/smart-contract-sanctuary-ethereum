...
contract FayreNFTSwapper is FayreMultichainBaseUpgradable, IERC721ReceiverUpgradeable, IERC1155ReceiverUpgradeable {
    ...
    */
    ...
    function createSwap(SwapRequest calldata swapRequest, FayreMultichainMessage memory fayreMultichainMessage, uint8[3] calldata v, bytes32[3] calldata r, bytes32[3] calldata s) external payable {
        ...
        _processFee(_msgSender(), swapRequest.sellerAssetData.length > 1 || swapRequest.bidderAssetData.length > 1, _processSignedData(fayreMultichainMessage, v, r, s));
        ...
        for (uint256 i = 0; i < swapRequest.sellerAssetData.length; i++)
            if (swapRequest.sellerAssetData[i].assetType == SwapAssetType.LIQUIDITY)
                require(providedLiquidity == swapRequest.sellerAssetData[i].amounts[0], "E#4");
    ...
    function finalizeSwap(uint256 swapId, bool rejectSwap, FayreMultichainMessage memory fayreMultichainMessage, uint8[3] calldata v, bytes32[3] calldata r, bytes32[3] calldata s) external payable {
        ...
        _processFee(_msgSender(), swapData.swapRequest.sellerAssetData.length > 1 || swapData.swapRequest.bidderAssetData.length > 1, _processSignedData(fayreMultichainMessage, v, r, s));
        ...
        for (uint256 i = 0; i < swapData.swapRequest.bidderAssetData.length; i++)
            if (swapData.swapRequest.bidderAssetData[i].assetType == SwapAssetType.LIQUIDITY)
                require(providedLiquidity == swapData.swapRequest.bidderAssetData[i].amounts[0], "E#7");
    ...
    function _processMembershipCards(address owner, bool isMultiAssetSwap, uint256 fee, MultichainMembershipCardData memory multichainMembershipCardData) private returns(uint256) {
        ...
        if (fee > 0) {
            for (uint256 i = 0; i < membershipCardsAddresses.length; i++) {
                ...
                uint256 cardMultiAssetSwapFee = cardsMultiAssetSwapFee[membershipCardSymbol];
                ...
                for (uint256 j = 0; j < membershipCardsAmount; j++) {
                    ...
                    if (fee > cardMultiAssetSwapFee)
                        fee = cardMultiAssetSwapFee;
                }
            }
        }
