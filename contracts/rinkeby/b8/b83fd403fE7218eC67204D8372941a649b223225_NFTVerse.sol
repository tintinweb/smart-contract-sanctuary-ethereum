...
contract NFTVerse is ERC721, Ownable {
  ...
  // JS timestamp divided on 1000 (in seconds)
  ...
  function buyTokens(uint256 tokensNumber) public payable {
      require(block.timestamp >= publicSaleTimestamp, "Sale is closed at this moment");
      require(_mintedTokens[msg.sender].add(tokensNumber) <= 3, 'You cannot purchase more then 3 tokens');
      require((tokensNumber.mul(getCurrentPrice())) == msg.value, "Received value doesnt match the requested tokens");
      require((currentSupply.add(tokensNumber)) <= _totalSupply, "You try to mint more tokens than left");
  ...
  function setPublicSaleTimestamp(uint256 timestamp) public onlyOwner {
