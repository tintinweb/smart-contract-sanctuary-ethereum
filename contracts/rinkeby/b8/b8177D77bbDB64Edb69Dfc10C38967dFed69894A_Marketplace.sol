...
contract Marketplace is MarketplaceCore, TransferManager {
    function initialize(
        INftTransferProxy _transferProxy,
        IERC20TransferProxy _erc20TransferProxy,
        uint96 newOriginFee,
        address newOriginFeeReceiver
    ) external initializer {
        __Context_init_unchained();
        __Ownable_init_unchained();
        __TransferExecutor_init_unchained(_transferProxy, _erc20TransferProxy);
        __TransferManager_init_unchained(newOriginFee, newOriginFeeReceiver);
    ...
    ITransferManager
{
    ...
    //events
    event Cancel(uint salt, bytes32 hash);
    event Match(uint256 salt, uint256 newLeftFill, uint256 newRightFill);
    ...
    function cancel(LibOrder.Order memory order) external {
        ...
        emit Cancel(order.salt,orderKeyHash);
    }
    ...
    */
    function matchAndTransfer(
        ...
        emit Match(orderLeft.salt, newFill.rightValue, newFill.leftValue);
    }
...
abstract contract TransferManager is OwnableUpgradeable, ITransferManager {
    ...
    function __TransferManager_init_unchained(
        ...
        feeOrigins.push();
        feeOrigins[0].value = newOriginFee;
        feeOrigins[0].account = payable(newOriginFeeReceiver);
    }

    function setnewOriginFee(uint96 newOriginFee, address newOriginFeeReceiver) external onlyOwner {
        feeOrigins[0].value = newOriginFee;
        feeOrigins[0].account = payable(newOriginFeeReceiver);
    }
    ...
    function doTransfersWithFees(
        ...
        totalAmount = calculateTotalAmount(amount, feeOrigins);
        uint256 rest = amount;
        ...
        returns (LibPart.Part[] memory)
    {
        if (
            ...
            matchNft.assetClass == LibAsset.ERC1155_LAZY_ASSET_CLASS
        ) {
            ...
            matchNft.assetClass == LibAsset.ERC721_LAZY_ASSET_CLASS
        ) {
...
abstract contract AssetMatcher is Initializable, OwnableUpgradeable {
    ...
    function matchMakeAssets(
        LibOrder.Order memory orderLeft,
        LibOrder.Order memory orderRight
    ) internal pure returns (LibAsset.AssetType memory) {
...
import "../utils/transfer-proxy/interfaces/INftTransferProxy.sol";
import "../utils/transfer-proxy/interfaces/IERC20TransferProxy.sol";
import "./interfaces/ITransferExecutor.sol";
import "../utils/lazy-mint/erc-721/IERC721LazyMint.sol";
import "../utils/lazy-mint/erc-1155/IERC1155LazyMint.sol";
import "@openzeppelin/contracts-upgradeable/proxy/utils/Initializable.sol";
import "@openzeppelin/contracts-upgradeable/access/OwnableUpgradeable.sol";
  ...
  ITransferExecutor
{
  ...
  function __TransferExecutor_init_unchained(
    ...
    proxies[LibAsset.ERC721_LAZY_ASSET_CLASS] = address(transferProxy);
    proxies[LibAsset.ERC1155_LAZY_ASSET_CLASS] = address(transferProxy);
  }
  ...
  function transfer(
    ...
    if (asset.assetType.assetClass == LibAsset.ETH_ASSET_CLASS) {
      ...
      INftTransferProxy(proxies[LibAsset.ERC721_ASSET_CLASS])
        .erc721safeTransferFrom(IERC721Upgradeable(token), from, to, data.tokenId);
...
interface INftTransferProxy {
    ...
    function mintAndTransfer(LibAsset.Asset calldata asset, address to)
        external;
}
