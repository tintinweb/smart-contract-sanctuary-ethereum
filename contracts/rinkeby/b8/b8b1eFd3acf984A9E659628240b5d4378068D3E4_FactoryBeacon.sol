...
contract FactoryBeacon {
    ...
    address immutable public tokenBeacon;

    event NewClone(address _newClone, address _owner);

    constructor(address upgrader) {
        UpgradeableBeacon _tokenBeacon = new UpgradeableBeacon(address(new FairXYZDeployer()));

        // Ensure only upgrader can perform upgrades by transfering ownership to upgrader address
        _tokenBeacon.transferOwnership(upgrader);
        
        address implementation_address = _tokenBeacon.implementation();
        
        // Initialize implementation to ensure to protect from delegateCall risk
        FairXYZDeployer(implementation_address).initialize();

        tokenBeacon = address(_tokenBeacon);
    }

    function createContract(bytes memory _data) external returns (address) {
        BeaconProxy proxy = new BeaconProxy(tokenBeacon, _data);
        emit NewClone(address(proxy), msg.sender);
...
contract FairXYZDeployer is ERC721xyzUpgradeable, PausableUpgradeable, OwnableUpgradeable, UUPSUpgradeable {
    ...
    function initialize() public initializer {
        __ERC721_init("Fair.xyz","Fair.xyz");
        __Ownable_init();
        __Pausable_init();
     ...
     */
    function _initialize(uint256 maxTokens_, uint256 nftPrice_, string memory name_, string memory symbol_,
 ...
 */
contract ERC721xyzUpgradeable is ContextUpgradeable, ERC165Upgradeable, IERC721Upgradeable, ERC2981Upgradeable, IERC721MetadataUpgradeable {
     ...
     */
    function supportsInterface(bytes4 interfaceId) public view virtual override(ERC2981Upgradeable, ERC165Upgradeable, IERC165Upgradeable) returns (bool) {
        return
