...
contract Escrow {
    ...
    function placement () external payable returns (uint) {
    ...
    function confirmPurchase(uint _dealNumber) external condition(msg.value == (2 * deals[_dealNumber].valueSeller)) payable {
    ...
    function confirmReceived (uint _dealNumber) external  payable  {
        ...
        uint amountOwner = amountS*FEE/1000;
    ...
    function selector () external pure  returns (bytes memory,bytes memory,bytes memory,bytes memory ) {
    return (
        abi.encodeWithSignature("placement()"),
        abi.encodeWithSignature ("abort()"),
        abi.encodeWithSignature( "confirmPurchase()"),
        abi.encodeWithSignature("confirmReceived()"));
    }   
}