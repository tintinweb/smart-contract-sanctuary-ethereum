...
contract KindlyNFT is Ownable, ERC721 {
  ...
  function addStakedkindly(address holder, uint256 newBalance) public {
    // on staking in the marketplace the owner of the nft will receive extra stakedKindly
    require(holder != address(0), "Address should not be 0x");
    mappingAddressDeeds[holder].stakedKindly = mappingAddressDeeds[holder].stakedKindly.add(newBalance);
  }

  function removeStakedkindly(address holder, uint256 newBalance) public {
    // on removal of staked kindly in the marketplace the owner of the nft will lose the extra stakedKindly
    require(holder != address(0), "Address should not be 0x");
    mappingAddressDeeds[holder].stakedKindly = mappingAddressDeeds[holder].stakedKindly.sub(newBalance);
  }

  function updateRewardsFromStakedKindly(address holder, uint256 newBalance) public {
    // the owner of the nft will receive point from rewards collected from the staking process
    require(holder != address(0), "Address should not be 0x");
    mappingAddressDeeds[holder].rewardsFromStaking = mappingAddressDeeds[holder].rewardsFromStaking.add(newBalance);
  }
  ...
  function createNFT() public {
    ...
    mappingAddressDeeds[msg.sender] = Deeds(
      kindlyOnWallet, 
      uint256(0), // default value from staked tokens is 0
