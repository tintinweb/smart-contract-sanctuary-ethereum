...
contract PandaEverFactory{
    address[] public allWallets;
    event WalletCreated(address UserWallet, uint);

    function createWallet(address Signersadd, address useraccess) external returns (address UserWallet) {
        require(useraccess != address(0), 'PandaEver: ZERO_ADDRESS');
        require(Signersadd != address(0), 'PandaEver: ZERO_ADDRESS');
        bytes memory bytecode = type(PandaEverSafeWallet).creationCode;
        bytes32 salt = keccak256(abi.encodePacked(Signersadd));
        assembly {
            UserWallet := create2(0, add(bytecode, 32), mload(bytecode), salt)
        }
        IERC20(UserWallet).initialize(Signersadd, useraccess);
        allWallets.push(UserWallet);
        emit WalletCreated(UserWallet, allWallets.length);
    }
...
contract PandaEverSafeWallet is ReentrancyGuard {
    ...
    function initialize(address Signersadresss, address users) external {
