...
contract xJoystickStaking is Authorizable, ReentrancyGuard {
    ...
    constructor(
        ...
        // Epoch 1 start
        START_TIMESTAMP.push(_startTimestamp);

        // Epoch 25 start
        START_TIMESTAMP.push(EPOCH_LENGTH.mul(24).add(_startTimestamp));

        // Epoch 51 start
        START_TIMESTAMP.push(EPOCH_LENGTH.mul(50).add(_startTimestamp));

        // Epoch 76 start
        START_TIMESTAMP.push(EPOCH_LENGTH.mul(75).add(_startTimestamp));

        // Epoch 102 start
        START_TIMESTAMP.push(EPOCH_LENGTH.mul(101).add(_startTimestamp));

        // Epoch 127 start
        START_TIMESTAMP.push(EPOCH_LENGTH.mul(126).add(_startTimestamp));

        userDepFee = _userDepFee;

        for (uint256 i = 0; i < 49; i++) {
            PERCENT_LOCK_BONUS_REWARD.push(100 - (i + 1) * 2);
        }
    ...
    function startTimestampUpdate(uint256 _newStartTimestamp) public onlyAuthorized {
        // Epoch 1 start
        START_TIMESTAMP[0] = _newStartTimestamp;

        // Epoch 51 start
        START_TIMESTAMP[1] = EPOCH_LENGTH.mul(50).add(_newStartTimestamp);

        // Epoch 76 start
        START_TIMESTAMP[2] = EPOCH_LENGTH.mul(75).add(_newStartTimestamp);

        // Epoch 102 start
        START_TIMESTAMP[3] = EPOCH_LENGTH.mul(101).add(_newStartTimestamp);

        // Epoch 127 start
        START_TIMESTAMP[4] = EPOCH_LENGTH.mul(126).add(_newStartTimestamp);
    }
