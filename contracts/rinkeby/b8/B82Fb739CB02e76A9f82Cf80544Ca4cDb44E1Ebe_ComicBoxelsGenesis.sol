...
contract ComicBoxelsGenesis is ERC721A, ERC721ABurnable, Ownable, ReentrancyGuard {
  ...
  event Minted(address minter, uint256 amount, uint256 start, uint256 end);
  ...
  function mint(uint16 quantity) external payable {
    ...
    emit Minted(msg.sender, quantity, start, end);
  }
  ...
  /**
    @dev you can only change the URIs if the NFT has not been revealed yet
    */
