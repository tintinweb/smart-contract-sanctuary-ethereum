...
contract Lottery is Ownable, ReentrancyGuard {
    ...
    function activateLottery(uint256[] calldata _lotteryIds) public onlyOwner {
        for (uint256 i = 0; i < _lotteryIds.length; i++) {
            require(
                lotteries[_lotteryIds[i]].status == Status.inactive,
                "Please select only inactive drops."
            );
        }
        for (uint256 i = 0; i < _lotteryIds.length; i++) {
            lotteries[_lotteryIds[i]].status = Status.active;
        }
    }
