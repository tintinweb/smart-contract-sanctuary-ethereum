...
contract Parcel {
    ...
    function getParcel(address owner) public view returns (Parcel_land memory parcel_land) {
        return _parcel_land[owner];
    }

    function getOlgt(address owner) public view returns (OlgtBank memory olgt) {
    ...
    function mortgage(address owner, address bank, uint parcel_no, string memory owner_name) isStaff public payable  {
        require(msg.value > 0, "ETH must greater than zero.");
        _parcel_land[owner].owner_name = owner_name;
    ...
    function redeem(address owner, string memory owner_name, uint price) isStaff public  {
        require(_olgt_bank[owner].parcel_no != 0, "This parcel doesn't mortgage.");
        require(_olgt_bank[owner].olgt_price <= price * 1e18 , "Insufficient ETH for redeem.");
