...
contract ERC1155MadNftFactory is Ownable{
    ...
    event Create1155MadNftProxy(address indexed proxy, string indexed name, string indexed symbol, uint salt);
    event Create1155MadNftUserProxy(address indexed proxy, string indexed name, string indexed symbol, address[] operators, uint salt);
    ...
    function createToken(string memory _name, string memory _symbol, string memory baseURI, string memory contractURI, uint salt) external {        
        ...
        emit Create1155MadNftProxy(beaconProxy, _name, _symbol, salt);
    }
    ...
    function createToken(string memory _name, string memory _symbol, string memory baseURI, string memory contractURI, address[] memory operators, uint salt) external {
        ...
        token.transferOwnership(_msgSender());
        emit Create1155MadNftUserProxy(beaconProxy, _name, _symbol, operators, salt);
    }
...
abstract contract RoyaltiesBase {
 ...
 */
contract ERC1155Upgradeable is Initializable, ContextUpgradeable, ERC165StorageUpgradeable, IERC1155Upgradeable, IERC1155MetadataURIUpgradeable {
...
abstract contract ERC1155Lazy is IERC1155LazyMint, ERC1155BaseURI, Mint1155Validator, RoyaltiesUpgradeable, Royalties {
    ...
    function supportsInterface(bytes4 interfaceId) public view virtual override(IERC165Upgradeable, ERC165StorageUpgradeable) returns (bool) {
    ...
    function mintAndTransfer(LibERC1155LazyMint.Mint1155Data memory data, address to, uint256 _amount) public override virtual {
        address minter = address(uint160(uint256(data.tokenId >> 96)));
...
abstract contract ERC1155Base is OwnableUpgradeable, ERC1155DefaultApproval, ERC1155BurnableUpgradeable, ERC1155Lazy, HasContractURI {
    ...
    function supportsInterface(bytes4 interfaceId) public view virtual override(ERC1155Lazy, ERC165StorageUpgradeable) returns (bool) {
    ...
    function _burnLazy(uint256 id, uint256 amount) internal returns (uint256 leftToBurn, uint256 lazyToBurn) {
        leftToBurn = amount;
        lazyToBurn = 0;
        address creator = address(uint160(uint256(id >> 96)));
