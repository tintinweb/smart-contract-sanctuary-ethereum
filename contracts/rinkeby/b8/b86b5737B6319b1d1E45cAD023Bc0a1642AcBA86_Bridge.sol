...
contract Bridge {
    ...
    function mint_request(
        ...
        require(!blacklist[reciever], "in_blacklist");
        require(
        ...
        mintsInfo[otherChainNonce].reciever = reciever;
    ...
    function mint_sign(
        ...
        if (_sign) {
            ...
            IERC20(mintsInfo[_nonce]._token).transferFrom(address(this),
                mintsInfo[_nonce].reciever, mintsInfo[_nonce].amount); // minting some tokens for the reciever

            emit MintAccepted(
                mintsInfo[_nonce].otherChainNonce,
                msg.sender,
                mintsInfo[_nonce].reciever,
    ...
    function viewPendingLength() external view returns (uint256) {
        return pendingMintNonces.length;
    }

    function viewAcceptedLength() external view returns (uint256) {
        return acceptedMintNonces.length;
    }

    function viewDeniedLength() external view returns (uint256) {
