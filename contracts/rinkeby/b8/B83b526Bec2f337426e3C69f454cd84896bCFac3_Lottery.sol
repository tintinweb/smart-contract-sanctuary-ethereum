...
contract Lottery is VRFConsumerBase, ColorMap  {
    ...
    event LotteryTriggered(address recentWinner, uint lottPot, uint round);

    function addPixel(uint32 _color, uint256 _x, uint256 _y) public whenNotPaused {  // whenNotPaused
    ...
    function fulfillRandomness(bytes32 _requestId, uint256 _randomness) internal override {
        ...
        uint[] memory randomValues = expand(_randomness);
        if(randomValues[0] % 3 == 1) {
            calculateWinner(randomValues[1]);
            if(recentWinner == address(0)) {
                _unpause();
                return;  
            }
            uint lottPot = IERC20(coin).balanceOf(address(this));
            // if(coin == linkToken) {
            //     lottPot = IERC20(coin).balanceOf(address(this)) - 1 ether;
            // } else {
            //     lottPot = IERC20(coin).balanceOf(address(this));   
            // }
    ...
    function expand(uint256 randomValue) internal pure returns (uint256[] memory expandedValues) {
    ...
    function calculateWinner(uint _randValue) internal {
...
contract ColorMap is Ownable, ReentrancyGuard, Pausable {
    ...
    uint public gridSizeX = 48;
    uint public gridSizeY = 64;

    uint public pixelsAdded = 0;
    uint public lottCycle = 50;
    ...
    function changeGridSize(uint256 _gridSizeX, uint256 _gridSizeY) public onlyOwner {
