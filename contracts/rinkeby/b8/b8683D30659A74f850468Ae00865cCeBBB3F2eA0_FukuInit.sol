...
import { FukuStorage, EIP712Domain, EIP712Hashes, FukuTokenizedOptionsStorage } from "./libraries/FukuStorage.sol";
import { TokenizedOptions } from "./tokenized/TokenizedOptions.sol";
...
contract FukuInit {
     ...
     *
     * @param name The name of the contract (used for EIP712)
     * @param version The version of the contract (used for EIP712)
     */
    function init(string calldata name, string calldata version) external {
        // adding ERC165 data
        ...
        ds.supportedInterfaces[type(IFukuOptionExchange).interfaceId] = true;
        ...
        // set the EIP712Domain struct and create the domain hash
        ...
        EIP712Hashes storage hashes = FukuStorage.eip712Hashes();
        hashes.domain = keccak256(
            abi.encode(
        ...
        // create option listing type hash
        hashes.optionListing = keccak256(
            "OptionListing(address seller,address collection,uint256 tokenId,uint256 strikePrice,uint256 premium,uint256 duration,uint256 nonce)"
        );

        // deploy the tokenized options ERC721 contract
        FukuTokenizedOptionsStorage storage tokenizedOptionsStorage = FukuStorage.fukuTokenizedOptionsStorage();
        tokenizedOptionsStorage.tokenizedOptions = address(new TokenizedOptions("Fuku Tokenized Options", "FUKU"));
    }
}
...
contract TokenizedOptions is ERC721, Ownable, ITokenizedOptions {
    // mapping of the token ID to the option data
    mapping(uint256 => TokenizedOption) private options;

    // counter used when minting options to determine the token ID
    uint256 private nextOptionId;

    constructor(string memory name_, string memory symbol_) ERC721(name_, symbol_) {}

    /**
     * @notice Gets the option parameters for a token id
     *
     * @return The option parameters if token id has at some point been minted
     */
    function getOption(uint256 tokenId) external view override returns (TokenizedOption memory) {
        return options[tokenId];
    }

    /**
     * @notice Function called by Fuku exchange contract to tokenized an option contract
     *
     * @param option The option data
     * @param recipient The recipient of the tokenized options
     */
    function mintOption(Option calldata option, address recipient) external override onlyOwner {
        // mint and send to recipient
        _safeMint(recipient, nextOptionId);
        ...
        // add to mapping
        options[nextOptionId++] = TokenizedOption(option, block.timestamp);
    }

    /**
     * @notice Function called by Fuku exchange contract to burn a tokenized option contract
     *
     * @param tokenId The token id to burn
     */
    function burnOption(uint256 tokenId) external override onlyOwner {
        _burn(tokenId);

        emit OptionBurned(tokenId);
    }
}
...
/**
 * Vendored on June 10, 2022 from:
 * https://github.com/mudgen/diamond-3-hardhat/blob/main/contracts/interfaces/IERC173.sol
 */
pragma solidity ^0.8.0;
...
/**
 * Vendored on June 10, 2022 from:
 * https://github.com/mudgen/diamond-3-hardhat/blob/main/contracts/interfaces/IERC165.sol
 */
pragma solidity ^0.8.0;
