...
contract CyberGenie is Ownable, ERC721A {
    event GetDiscount(uint256 discount);
    ...
    // Max token supply
    uint256 public constant MAX_SUPPLY = 100; // Todo: change to 5000
    // Public mint price
    uint256 public mintPrice = 0.01 ether; // Todo: change price to 0.1
    ...
    modifier validatePrice(uint256 _count) {
        uint256 _price = getDiscountedPrice(_msgSender());
     ...
     *
     */
    function getDiscountedPrice(address _claimer)
        public
        view
        returns (uint256)
    {
        ...
        if (partnersWalletList[_claimer].discount > 0) {
            _price =
                _price -
                (_price * (partnersWalletList[_claimer].discount / 100));
        } else {
            ...
            for (uint256 i = 0; i < partnersSCListKeys.length; i++) {
                ...
                if (IERC721A(partnersSCListKeys[i]).balanceOf(_claimer) >= 1) {
                    uint256 _tmpPrice;
                    _tmpPrice =
                        _price -
                        ((_price * partnersSCList[partnersSCListKeys[i]]) /
                            100);
                    if (_tmpPrice < _price) {
                        _price = _tmpPrice;
                    }
    ...
    /**
     * @dev sets the hidden reserved base uri for {reservedBaseURI}
     */
    function setReservedBaseURI(string calldata _reservedBaseURI)
        external
        onlyOwner
    {
        reservedBaseURI = _reservedBaseURI;
    }

    /**
     * @dev sets the suffer uri for {suffixURI}
     */
        ...
        returns (string memory)
    {
        ...
        return
            bytes(baseURI).length > 0
                ? string(
                    abi.encodePacked(baseURI, _toString(_tokenId), suffixURI)
                )
                : "";
    }
