...
contract BACImplementationV3 is ERC721A, AccessControl {
    enum State {
        InitMint,
        BulkMint,
        ComboMint,
    ...
    uint256 public TOKENS_PER_WALLET = 1000;
    uint256 public TOKENS_PER_TX = 25;
    ...
    event PublicSaleMint(uint256 count, address owner);

    event RarityListHash(string value);
    event ReorderedListHash(string value);
    event StateChanged(State value);

    constructor(address contractAddress) ERC721A("Bored Ape Comic #2", "BAC2") {
    ...
    function publicMint(uint256 count) public payable {
        ...
        if ((_mintedTokens[msg.sender] + count) > TOKENS_PER_WALLET) {
        ...
        _mintedTokens[msg.sender] += count;

        emit PublicSaleMint(count, msg.sender);
    }

    function setMaxPerTransaction(uint256 maxTransactions)
        external
        onlyRole(OPERATOR_ROLE)
    {
        TOKENS_PER_TX = maxTransactions;
    }

    function getMaxPerTransaction() external view returns (uint256) {
        return TOKENS_PER_TX;
    }

    function setMaxPerWallet(uint256 tokensPerWallet)
        external
        onlyRole(OPERATOR_ROLE)
    {
        TOKENS_PER_WALLET = tokensPerWallet;
    }

    function getMaxPerWallet() external view returns (uint256) {
        return TOKENS_PER_WALLET;
    }
