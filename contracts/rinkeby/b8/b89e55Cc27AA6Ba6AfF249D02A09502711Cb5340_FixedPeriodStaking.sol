...
contract FixedPeriodStaking is AccessControl {
    ...
    event Withdrawal(address user, uint256 amount, uint256 reward, uint256 depositId);
    ...
    //poolId => cycleNumber => sharesToRemove
    mapping(uint256 => mapping(uint256 => uint256)) public sharesToRemove;

    uint256 public cycleDuration;
    uint256 public currentCycle;
    uint256 public lastCycleTimestamp;
    uint256 public mintPerCycle;
     ...
     * uint256 _mintPerCycle - Amount of tokens to mint per cycle.
    ...
    constructor(
        ...
        uint256 _mintPerCycle,
        uint256 _snapshotRewardPerSecond,
        address governanceAddress
    ) {
        stakeToken = _stakeToken;
        cycleDuration = _cycleDuration;
        mintPerCycle = _mintPerCycle;
        ...
        returns (uint256 depositId)
    {
        ...
        require(
            stakeToken.allowance(msg.sender, address(this)) >= amount,
        ...
        bool toNextCycle = block.timestamp >
            (lastCycleTimestamp + cycleDuration / 2);
        uint256 depositStartingCycle;
        uint256 depositDuration = pools[poolId].duration;
        if (toNextCycle) {
        ...
        sharesToRemove[poolId][
            depositStartingCycle + depositDuration
        ] += shares;
        ...
        depositId = deposits[msg.sender].length - 1;
    ...
    function withdraw(uint256 depositId) external {
        ...
        uint256 allowedCycleToWithdraw = userDeposit.cycleNumber +
            pools[poolId].duration;
        require(currentCycle >= allowedCycleToWithdraw, "Can't withdraw yet");

        PoolSnapshot[] memory snapshots = poolSnapshots[poolId];

        uint256 rewardAmount = 0;
        uint256 startCycle = userDeposit.cycleNumber -
            snapshots[1].cycleNumber +
            1;
        ...
        uint256 _depositId = deposits[msg.sender].length - 1;
        emit Withdrawal(msg.sender, amountToTransfer, rewardAmount, _depositId);
    }
    ...
    function snapshotPools() external {
        require(
            block.timestamp >= lastCycleTimestamp + cycleDuration,
            "Can't snapshot yet"
        );
        uint256 snapshotReward = getSnapshotRewardAmount();
        stakeToken.transfer(msg.sender, snapshotReward);

        newWithdrawalsAmount += snapshotReward;
        uint256 toSplit = getCycleIncome();
        uint256 _mintPerCycle = mintPerCycle;
        if (_mintPerCycle > 0) {
            IMint(address(stakeToken)).mint(address(this), _mintPerCycle);
            toSplit += _mintPerCycle;
        }
        lastUpdate.cycleNumber = currentCycle;
        lastUpdate.contractBalance = stakeToken.balanceOf(address(this));

        for (uint256 i = 0; i < pools.length; i++) {
            //removing shares
            uint256 expiredShares = sharesToRemove[i][currentCycle];
            pools[i].totalShares -= expiredShares;
            ...
            poolSnapshots[i].push(
                PoolSnapshot({PPS: PPS, cycleNumber: currentCycle})
            );
    ...
    function getSnapshotRewardAmount() public view returns (uint256) {
        if (block.timestamp >= lastCycleTimestamp + cycleDuration) {
            ...
            uint256 _mintPerCycle = mintPerCycle;
            if (_mintPerCycle > 0) {
                income += _mintPerCycle;
            }
            uint256 calculatedReward = (block.timestamp -
                (lastCycleTimestamp + cycleDuration)) * snapshotRewardPerSecond;
    ...
    function setPools(
        ...
        for (uint256 i = 0; i < _duration.length; i++) {
            ...
            if (poolSnapshots[i].length == 0) {
                poolSnapshots[i].push(
                    PoolSnapshot({PPS: 0, cycleNumber: currentCycle})
                );
            }
        }
     ...
     */
    function changePoolDuration(uint256 poolId, uint256 _duration)
        external
     ...
     * Sets cycle duration
     */
    function setCycleDuration(uint256 _cycleDuration)
        external
        onlyRole(GOVERNANCE_ROLE)
    {
     ...
     */
    function setSnapshotRewardPerSecond(uint256 _snapshotRewardPerSecond)
        external
        onlyRole(GOVERNANCE_ROLE)
    {
        require(
            _snapshotRewardPerSecond != snapshotRewardPerSecond,
            "Already set"
        );
        snapshotRewardPerSecond = _snapshotRewardPerSecond;
    }
    ...
    /*
     * Params
     * uint256 _mintPerCycle - Amount of tokens to be minted during cycle
     *
     * Sets amount of tokens to be minted during cycle
     */
    function setMintPerCycle(uint256 _mintPerCycle)
        external
        onlyRole(GOVERNANCE_ROLE)
    {
        require(
            _mintPerCycle != mintPerCycle,
            "Already set"
        );
        mintPerCycle = _mintPerCycle;
    }
     ...
     */
    function getPendingReward(address user, uint256 depositId)
        external
        view
        returns (uint256 rewardAmount)
    {
        ...
        if (userDeposit.cycleNumber >= currentCycle || !userDeposit.active) return 0;

        rewardAmount = 0;
        uint256 startCycle = userDeposit.cycleNumber;
        uint256 endCycle = startCycle + pools[userDeposit.poolId].duration;
        uint256 subtractor = snapshots[1].cycleNumber;
        if(endCycle > currentCycle - 1) endCycle = currentCycle - 1;

        startCycle = startCycle - subtractor + 1;
        endCycle = endCycle - subtractor + 1;
