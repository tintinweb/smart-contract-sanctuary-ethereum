...
contract ERC1155Contract is ERC1155, Ownable, PaymentSplitter, ReentrancyGuard {
    ...
    function getClaimIneligibilityReason(uint256 _id, address _address, uint256 _quantity) public view returns (string memory) {
        return "";
    }

    function unclaimedSupply(uint256 _id) public view returns (uint256) {
        return tokenTypes[_id].maxSupply - tokenTypes[_id].totalSupply;
    }
    ...
    function mint(
        ...
        if(tokenTypes[_id].preSaleIsActive) {
            require(price(_id) * _quantity <= msg.value, "ETH sent is incorrect.");
    ...
    function claimTo(address userWallet, uint256[] memory quantity, uint256[] memory tokenIds) public payable nonReentrant onlyFiatMinter {
        _mintBatch(userWallet, tokenIds, quantity, "");
    }

    function reserve(uint256 _id, bytes memory _data, address _address, uint256 _quantity) public nonReentrant onlyOwner {
        uint256 _currentSupply = tokenTypes[_id].totalSupply;
        require(_currentSupply + _quantity <= tokenTypes[_id].maxSupply, "Requested quantity would exceed total supply.");
        tokenTypes[_id].totalSupply = _currentSupply + _quantity;
