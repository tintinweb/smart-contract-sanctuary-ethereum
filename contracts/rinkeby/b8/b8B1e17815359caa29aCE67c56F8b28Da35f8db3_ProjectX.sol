...
contract ProjectX is ERC721A, Ownable {
    ...
    uint8 public publicMintAmount = 10;
    ...
    bytes32 public whitelistMerkleRoot =
        0xc1bb7ed1fed03e1077696a2d938e6380d4f067d907453c52189df69950218dcb;

    mapping(address => uint8) private _mintAllowList;
    mapping(address => uint8) private _mintPublicList;

    constructor() ERC721A("Project X", "PX") {
    ...
    function setMintList(uint8 _mintAmount) internal {
        if (!isAllowListActive) {
            require(
                _mintPublicList[msg.sender] + _mintAmount <= publicMintAmount
            );
            _mintPublicList[msg.sender] =
                _mintPublicList[msg.sender] +
                _mintAmount;
        }
        ...
        if (isAllowListActive) {
            require(
                _mintAllowList[msg.sender] + _mintAmount <= allowListMintAmount
            );
            _mintAllowList[msg.sender] =
                _mintAllowList[msg.sender] +
                _mintAmount;
        }
    }
    ...
    function remainingAllowListMints(address userAddress)
        public
        view
        returns (uint256)
    {
    ...
    function remainingPublicMints(address userAddress)
        public
        view
        returns (uint256)
    {
    ...
    // Long lost community waller
    // function withdraw() public onlyOwner {
    //     //50% goes to community wallet
    //     (bool hs, ) = payable(0x086C47A4E99cd41aF68166Fbad163546AAA50e).call{
