...
* @author ProfWobble 
* @dev
*  - NFTs representing user staked position..
...
contract JellyPool is IJellyPool, IJellyContract, JellyPoolNFT, Documents {
    ...
    /// @notice Mapping from  tokenId => token reward info.
    ...
    function _beforeTokenTransfer(
        ...
        require(tokenInfo[tokenId].lastUpdateTime <= block.timestamp - poolSettings.transferTimeout, "Token recently updated");
        ...
        public
    {
        ...
        uint48 current_time = BoringMath.to48(block.timestamp);
        tokenInfo[_tokenId].lastUpdateTime = current_time;

        address[] memory rewardTokens = rewarder.rewardTokens(address(this));
        for(uint i = 0; i < rewardTokens.length; i++) {

            address rewardToken = rewardTokens[i];
            RewardInfo memory rInfo = poolRewards[rewardToken];
            uint256 currentRewardPoints = uint256(rInfo.rewardsPerTokenPoints);
            if (_exists(_tokenId) && stakedTokenTotal > 0) {
                ...
                currentRewardPoints = currentRewardPoints + (currentRewards * 1e18 * pointMultiplier / stakedTokenTotal);
            }
            poolRewards[rewardToken] = RewardInfo(current_time, BoringMath.to208(currentRewardPoints)) ;

            TokenRewards memory tRewards = tokenRewards[_tokenId][rewardToken];
            uint256 rewards = tokenInfo[_tokenId].staked * (currentRewardPoints - uint256(tRewards.lastRewardPoints))
                                                                / 1e18
                                                                / pointMultiplier;
            ...
            tokenRewards[_tokenId][rewardToken] = TokenRewards(tRewards.rewardsEarned + BoringMath.to128(rewards)
                                                                ...
                                                                , current_time
                                                                , BoringMath.to208(currentRewardPoints));
        }
    }
     ...
     * @notice Claiming rewards for user.
     */
    // PW: Can we get it so that the claim counts and only does one reward transfer?
    // PW: maybe this can accept user address for fancy claim. 
        ...
        if (tokenIds.length > 0) {
            for(uint i = 0; i < tokenIds.length; i++) {
                ...
                for(uint j = 0; j < rewardTokens.length; j++) {
                    _claimRewards(tokenIds[i], rewardTokens[j]);
                }
            }
        }
    }
    ...
    /**
     * @notice Claiming rewards for token ID.
    ...
    function _claimRewards(uint256 _tokenId, address _rewardToken) internal {
        ...
        OZIERC20(_rewardToken).safeTransfer(tokenOwner, payableAmount);
        emit RewardPaid(tokenOwner, payableAmount);
    }
