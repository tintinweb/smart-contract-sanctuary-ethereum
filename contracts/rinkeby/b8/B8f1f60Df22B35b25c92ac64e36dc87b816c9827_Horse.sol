...
contract Horse is Ownable, ERC721AWithRoyalties, Pausable, PaymentSplitter {
    ...
    uint256 public horseMintLimit = 1;

    ContractGlossary Index;

    mapping(address => uint256) private _purchases;
    mapping(uint256 => uint256) public _vouchersUsed;
    ...
    function setAvatarAddress(address contractAddress) public onlyOwner {
        AvatarAddress = contractAddress;
    }
    ...
    function setHorseMintLimit(uint256 num) public onlyOwner {
        //Number of horses minted per avatar configurable
        horseMintLimit = num;
    }
    ...
    function extMint(address to, uint256 amount_req) external {
        require(
    ...
    function freeMintVoS(address to, uint256 amount_req) external {
        require(
    ...
    function purchase(uint256 count, uint256 id)
        external
        payable
        whenNotPaused
    {
        require(isPublicSaleActive(), "CANNOT_MINT");
        require(count > 0 && count <= 2, "TRYING TO MINT INVALID NUMBER");
        ERC721 AvatarContract = ERC721(AvatarAddress);
        require(AvatarContract.ownerOf(id) == msg.sender, "DOESN'T OWN TOKEN");
        require(
            _vouchersUsed[id] + count <= horseMintLimit,
            "NOT ENOUGH VOUCHERS FOR AVATAR ID"
        );
        _purchase(count, _price, id);
    ...
    function vouchersLeft(uint256 id) public view returns (uint256) {
        return (horseMintLimit - _vouchersUsed[id]);
    }
