...
/**
 * @title MSHKToken token contract.
 * @author The MSHKToken team.
 */
contract MSHKToken is
    ContextUpgradeable,
    IERC20Upgradeable,
    ...
    OwnableUpgradeable
{
    ...
    // UniswapV2Factory部署在0x5C69bEe701ef814a2B6a3EDD4B1652CB9cc5aA6f以太坊主网上，以及Ropsten、Rinkeby、Görli和Kovan测试网。
    address private constant _factoryAddress =
        0x5C69bEe701ef814a2B6a3EDD4B1652CB9cc5aA6f;
    // UniswapV2Router02部署在0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D以太坊主网上，以及Ropsten、Rinkeby、Görli和Kovan测试网。
    address private constant _routerAddress =
    ...
    function initialize(
        ...
        // 初始化营销地址
        _marketingTaxAddress = 0x2eEE9f382f693DeDaf9f5E43a51B7E2d068Be9aB;

        // 初始化团队地址
        _teamTaxAddress = 0xe4586b46d0E08Ab292B49304396ed911d455F0Cb;
        ...
        // addPair(_factory.createPair(address(this), '0xe9e7CEA3DedcA5984780Bafc599bD69ADd087D56')); // BSC MainNet - BUSD
        // addPair(_factory.createPair(address(this), '0x55d398326f99059fF775485246999027B3197955')); // BSC MainNet - USDT
        // addPair(_factory.createPair(address(this), '0x2170Ed0880ac9A755fd29B2688956BD959F933F8')); // BSC MainNet - ETH
        // addPair(_factory.createPair(address(this), '0x1AF3F329e8BE154074D8769D1FFa4eE058B1DBc3')); // BSC MainNet - DAI
        // addPair(_factory.createPair(address(this), '0x7130d2A12B9BCbFAe4f2634d864A1Ee1Ce3Ead9c')); // BSC MainNet - BTCB

        // addPair(_factory.createPair(address(this), '0xed24fc36d5ee211ea25a80239fb8c4cfd80f12ee')); // BSC TestNet - BUSD
        // addPair(_factory.createPair(address(this), '0x7ef95a0fee0dd31b22626fa2e10ee6a223f8a684')); // BSC TestNet - USDT
        // addPair(_factory.createPair(address(this), '0xec5dcb5dbf4b114c9d0f65bccab49ec54f6a0867')); // BSC TestNet - DAI
        // addPair(_factory.createPair(address(this), '0x6ce8da28e2f864420840cf74474eff5fd80e65b8')); // BSC TestNet - BTCB

        // addPair(_factory.createPair(address(this), '0xdAC17F958D2ee523a2206206994597C13D831ec7')); // ETH MainNet - USDT
        // addPair(_factory.createPair(address(this), '0xa0b86991c6218b36c1d19d4a2e9eb0ce3606eb48')); // ETH MainNet - USDC
        // addPair(_factory.createPair(address(this), '0x6b175474e89094c44da98b954eedeac495271d0f')); // ETH MainNet - DAI
        // addPair(_factory.createPair(address(this), '0x2260fac5e5542a773aa44fbcfedf7c193bc2c599')); // ETH MainNet - WBTC
    ...
    /**
     * 更新税收。 确保总税额不超过 10%，并且奖励税始终为 2% 或更多。
    ...
    /**
     * 在奖励中包含地址。 计算新的奖励积分当前余额，以避免从现有持有者那里获得奖励。
    ...
    function excludeFromRewards(address account) public onlyOwner {
        ...
        // 根据余额计算奖励积分，accountSupplyBalance * rate
        uint256 accountRewardPoints = _getRewardPointsFromBalance(
            accountSupplyBalance,
            rate
        );
    ...
    function _transfer(
        ...
        // 如果其中之一是流动资金池地址。
        // 这不排除 _rewardPoints 和 _totalSupply 的税金。
        if (
        ...
        if (_excludedFromRewards[sender]) {
            ...
            // 我们应该已经有了发送者的 _ownedRewardPoints，但是
            // 由于汇率变化，我们应该重新计算
        ...
        // 从 _rewardPoints 和 _totalSupply 中排除税款（如果有）。
        if (taxesToExclude != 0) {
            // 排除总量加
        ...
        if (rewardsTaxAmount != 0) {
            // 在完成转账之前更新奖励率，以便发件人得到他们支付的部分奖励税。 （发送给收件人）
    ...
    /**
     * _transfer 的轻量级版本。 仅在税收交换期间使用以保持
     * gas fees low.
     */
    function _swapTransfer(
        ...
        // 如果 sender 和 recipient 同时在排除排除奖励列表
        require(
    ...
    /**
     * 将累积的营销和团队税交换为 ETH 并发送营销和团队税钱包的相应金额。
     */
