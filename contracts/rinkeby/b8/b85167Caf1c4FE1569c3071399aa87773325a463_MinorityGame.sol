...
contract MinorityGame is Ticket, IERC721Receiver {
    enum Vote {YES, NO}
    ...
    mapping(address => uint) public registeredTicketId;

    function register(uint tokenId) public nonReentrant {
        require(ownerOf(tokenId) == msg.sender, "Game: You don't own this ticket.");
        playerStatus[msg.sender].registered = true;
        registeredTicketId[msg.sender] = tokenId;
    ...
    function claim(uint tokenId) public nonReentrant {
        ...
        if (voteYesCount == voteNoCount) {
            ...
            // lose, new ticket is required to continue playing
            require(ownerOf(tokenId) == msg.sender, "Game: You don't own this ticket.");
            registeredTicketId[msg.sender] = tokenId;
            safeTransferFrom(msg.sender, address(this), tokenId);
        }
...
contract Ticket is Ownable, ERC721A, ReentrancyGuard {
    constructor(
    ...
    function publicSaleMint(uint256 quantity, string memory ownerName) external payable {
        ...
        "reached public sale wallet limit"
        );
  ...
  IERC721Enumerable
{
    ...
    returns (string memory)
  {
    ...
    parts[8] = '<text x="10" y="180" class="base">acknowledged your desireto play the Liar Game.</text>';
