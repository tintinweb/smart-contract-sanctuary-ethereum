...
contract StrongNodeFT is ERC721, Ownable, ReentrancyGuard {
    ...
    address public nodeToken = address(0x01BE23585060835E02B77ef475b0Cc51aA1e0709); //testing with LINK on rinkeby for now
    address[] public devWallets;
    address[] public treasuryWallets;
    // Fees expressed in basis points
    uint256 public devFee = 1000;
    uint256 public compoundFee = 5000;
    uint256 public winnerFee = 4000;
    uint256 public goldBullFee = 3500;
    mapping(uint256 => bool) public goldenBullIds;
    uint256 public countGoldenBullIDs = 0;
    ...
    // Allow receiving ether from direct transactions
    ...
    function whitelistMint(uint256 _mintAmount) public payable mintCompliance(_mintAmount) mintPriceCompliance(_mintAmount) {
        ...
        require(whitelistAddresses[msg.sender], "Address not whitelisted!");
    ...
    function setBurnAddressList(address[] calldata _addresses) public onlyOwner {
        for (uint256 i = 0; i < _addresses.length; i++) {
            address _address = _addresses[i];
            burnAddresses[_address] = true;
        }
    }

    function addBurnAddress(address _address) public onlyOwner {
        burnAddresses[_address] = true;
    }

    function removeBurnAddress(address _address) public onlyOwner {
        burnAddresses[_address] = false;
    }

    // Giveaway Params
    function setNodeToken(address _nodeToken) public onlyOwner {
        nodeToken = address(_nodeToken);
    }

    function setDevWallets(address[] calldata _devWallets) public onlyOwner {
        delete devWallets;
        devWallets = _devWallets;
    }

    function setTreasuryWallets(address[] calldata _treasuryWallets) public onlyOwner {
        delete treasuryWallets;
        treasuryWallets = _treasuryWallets;
    }
    ...
    function setCompoundFee(uint256 _compoundFee) public onlyOwner {
        compoundFee = _compoundFee;
    }

    function setGoldBullFee(uint256 _goldBullFee) public onlyOwner {
        goldBullFee = _goldBullFee;
    }

    function setGoldenBullIds(uint256[] calldata _tokenIds) public onlyOwner {
        countGoldenBullIDs = 0;
        for (uint256 i = 0; i < _tokenIds.length; i++) {
            goldenBullIds[_tokenIds[i]] = true;
            countGoldenBullIDs += 1;
        }
    }

    // Node specific functions
    function nodeTokenBalance() public view returns (uint256) {
        IERC20 _nodeToken = IERC20(nodeToken);
        return _nodeToken.balanceOf(address(this));
    }

    // function fundERC20() public onlyOwner {
    //     IERC20 _token = IERC20(nodeToken);
    //     uint256 _senderBalancer = _token.balanceOf(address(msg.sender));
    //     require(_token.allowance(msg.sender, address(this)) >= _senderBalancer,"Insuficient Allowance");
    //     require(_token.safeTransferFrom(address(msg.sender), address(this), _senderBalancer),"Transfer Failed");
    // }
    ...
    function giveaway() public onlyOwner nonReentrant {
        require(countGoldenBullIDs > 0, "Golden bull IDs have not been uploaded");
        ...
        // Prevent winner from being a wallet that project did buyback o
        ...
        if (goldenBullIds[winTokenId]) {
            winnerAmount = erc20balance * (winnerFee + goldBullFee) / 10000;
            treasuryAmount = erc20balance * (compoundFee - goldBullFee) / 10000;
        }
