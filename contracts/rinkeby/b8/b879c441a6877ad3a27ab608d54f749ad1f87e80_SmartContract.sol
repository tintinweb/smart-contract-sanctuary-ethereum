...
contract SmartContract is ERC721 , Ownable {
    ...
    uint256 private _mintPrice;                   // 1 eth = 1,000,000,000,000,000,000
    ...
    struct AssembleToken{
        uint256 id ; 
        string uri ;

    }

    constructor()  ERC721  ("Assemble", "AB") {}

    // function _setTokenURI(uint256 tokenId,string memory _tokenURI) internal {
    //     require(_exists(tokenId), "ETH721 Metadata: URI set of nonexistent token");
    ...
    function getAllTokens() public view returns(AssembleToken[] memory){
        uint256 lastestId = _tokenIdCounter.current();
        uint256 counter = 0 ;
        AssembleToken[] memory res = new AssembleToken[](lastestId);
        for(uint256 i=0 ; i< lastestId ; i++){
            if(_exists(counter)){
                string memory uri = tokenURI(counter);
                res[counter] = AssembleToken(counter,uri);
            }
            counter++ ;
        }
        return res;
    ...
    //     //require(msg.value >= minRate,"Not enough eth sent");
    ...
    function setupSale(uint256 newAntibotInterval, 
                       ...
                       uint256 newMintPrice) external onlyOwner{
