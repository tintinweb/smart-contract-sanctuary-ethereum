...
contract EveraiUpgradeArchive is Ownable {
    ...
    IEveraiArchive public everaiArchive;
    ...
    constructor(
        ...
        everaiArchive = IEveraiArchive(everaiArchiveAddress);
    }
    ...
    function link(
        uint256 everaiOriginArchiveId,
        uint256 everaiArchiveId,
        uint256[] memory memoryCoreIds
    ) external callerIsUser {
        address originArchiveOwner = everaiOriginArchive.ownerOf(
            everaiOriginArchiveId
        );
        require(
        ...
        if (everaiOriginArchiveId != 0) {
            everaiOriginArchive.burn(everaiOriginArchiveId);
            everaiArchive.mint(msg.sender, 1);
        }

        emit Link(everaiArchiveId);
    }
