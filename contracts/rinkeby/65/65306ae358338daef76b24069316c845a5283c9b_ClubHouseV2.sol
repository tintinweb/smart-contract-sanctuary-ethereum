    ...
    ReentrancyGuardUpgradeable
{
    ...
    function updateCrossMintaddress(address _address) external virtual onlyOwner {
    ...
    function mintTo(
        ...
        require(status, "$GenesisV1: cannot purchase the token OR incorrect price");
        tokenCounter += 1;

        _safeMint(msg.sender, tokenCounter); // minting nft
        merchantWalletAddress.transfer(msg.value); // transfering ETH to merachant wallet

        emit createdNft(to, id, tokenCounter, msg.value, dollarPrice, size_id);
