...
contract glfxMintertest1 is CreatorExtension, ICreatorExtensionTokenURI, IManifoldERC721Edition, ReentrancyGuard {
    ...
    function mint(address creator, uint256 series, address recipient, uint16 count) external payable override nonReentrant creatorAdminRequired (creator) {
        ...
        require(_totalSupply[creator][series]+count <= _maxSupply[creator][series], "Sorry, this class has been fully minted! Try again for a chance at a different class or in the next generation. You may also check out the secondary markets.");
        require(msg.value >= mintRate * count, "Sorry, not enough ether.");
