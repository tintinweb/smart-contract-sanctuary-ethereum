...
contract Renkon is ERC721A, Ownable, ReentrancyGuard {
  ...
  uint256 private _salePrice = 0.00000025 ether;
  ...
  constructor(bytes32 merkleRoot) ERC721A("Renkon", "RENKON") {
  ...
  function presaleMint(bytes32[] calldata merkleProof) external payable nonReentrant onlyEOA {
    if (!isPresaleActive())              revert SaleNotStarted();
    if (!isWhitelisted(msg.sender, merkleProof)) revert AccountNotWhitelisted();
  ...
  function isWhitelisted(address sender, bytes32[] calldata proof) public view returns (bool) {
