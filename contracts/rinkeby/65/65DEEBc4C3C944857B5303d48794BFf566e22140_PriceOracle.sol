...
contract PriceOracle is Ownable {
    ...
    function consult(
        ...
        if (_stableCoins.contains(tokenB)) {
            ...
            // amountA * ethPrices[tokenA] / 1e18 / (10**IERC20Metadata(tokenB).decimals()) * _fetchEthUsdPrice() * (10**IERC20Metadata(tokenB).decimals())

            return
                (amountA * ethPrices[tokenA] * _fetchEthUsdPrice() * (10**IERC20Metadata(tokenB).decimals())) /
                (1e18 * (10**IERC20Metadata(tokenB).decimals()));
            // return (amountA * ethPrices[tokenA] * (10**IERC20Metadata(tokenB).decimals())) /
            // (10**IERC20Metadata(tokenA).decimals() * _fetchEthUsdPrice());
        }
