...
contract KosekaiERC721A is ERC721A, Ownable {
    enum SaleStatus {
        PAUSED,
        SPIRITLIST,
        PUBLIC,
    ...
    // TODO: UPDATE TEAM RESERVED TOKENS
    uint256 public constant TEAM_RESERVED_TOKENS = 10; // 100
    uint256 public constant MAX_ETHEREAL_MINT = 3;
    ...
    // NOTE: UPDATE COST AFTER TESTING
    uint256 public spiritListCost = 0.01 ether;
    ...
    // Quantity checks
    // TODO: Check gas with uint256 vs uint8
    ...
    ) ERC721A("test34", "TESTING13128") {
        preRevealURI = _initNotRevealedUri;
        _mint(msg.sender, TEAM_RESERVED_TOKENS);
    }
        ...
        returns (string memory)
    {
        ...
        if (!revealed) return string(abi.encodePacked(preRevealURI, _tokenId.toString()));
        return
            string(
                abi.encodePacked(postRevealBaseURI, _tokenId.toString())
            );
    }
        ...
        callerIsUser
    {
        ...
        // Buy two get one free for Ethereals
        if (etherealMintedAmount[msg.sender] + _quantity == MAX_ETHEREAL_MINT) {
            require(
                msg.value == spiritListCost * (_quantity - 1),
                "INCORRECT ETH SENT"
            );
        } else {
            require(
                msg.value == spiritListCost * _quantity,
                "INCORRECT ETH SENT"
            );
        }
        ...
        callerIsUser
    {
        ...
        require(
            spiritListMintedAmount[msg.sender] + _quantity <= MAX_SPIRIT_LIST_MINT
        );
        require(
            msg.value == spiritListCost * _quantity,
            "INCORRECT ETH SENT"
    ...
    function publicListSale(uint8 _quantity)
        external
        payable
        callerIsUser
    {
        ...
        require(
            msg.value == publicCost * _quantity,
            "INCORRECT ETH SENT"
        );
        ...
        callerIsUser
    {
        ...
        require(
            modMintedAmount[msg.sender] + _quantity <= MAX_MOD_MINT
        );
    ...
    function withdrawFunds() external onlyOwner {
        ...
        // 19% of Current balance
        uint256 bal19 = currentBal * 19/100;

        // 5% for Mod team wallet
        uint256 bal5 = currentBal * 5/100;

        // ----- Developer, Art Team Budget Funds
        payable(0xEcCDEE30dDd5cB2B1A412366a5680FB33fDd1c1C).transfer(bal19);
        payable(0xe150A66AB734036e1EBEF707f061411617EB8C57).transfer(bal19);

        // ----- Founding Team Budget Funds
        payable(0xcA796e44b531830D02c293c78Fc33c22f7E3444c).transfer(bal19);
        payable(0x8422063D215DE59a2C6e06DF13792BeF74072D6f).transfer(bal19);

        // ----- Mod Funds
        payable(0x471EF53534E973B1c14Bc3cFa842204505686CEC).transfer(bal5);

        // ----- Vault
        payable(0xC4ecB40620A95c52b8aB47d6F707616ca6440af5).transfer(address(this).balance);
    }
}
