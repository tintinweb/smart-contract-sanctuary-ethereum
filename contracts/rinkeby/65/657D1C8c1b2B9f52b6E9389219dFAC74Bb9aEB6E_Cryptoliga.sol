...
contract Cryptoliga is ERC2981, ERC721, Pausable, AccessControl, CryptoligaFX {
    ...
    function whitelistMint(
        ...
        bytes32[] calldata _merkleProof
         ...
         if (saleStage < Stages.PRIVATE) {
            revert WrongStage(Stages.PRIVATE, saleStage);
        }
       
        if(!okToMint(to, quantity)) { revert CantMint(); }

        if (!checkValidity(_merkleProof)) {
            revert NotWhitelisted();
        }

        uint256 _totalPrice = quantity * presalePrice;
        ...
        whenNotPaused
    {
        if (saleStage != Stages.PUBLIC) {
            revert WrongStage(Stages.PUBLIC, saleStage);
        }

        if(!okToMint(to, quantity)) { revert CantMint(); }

        uint256 _totalPrice = quantity * mintPrice;
    ...
    function okToMint(address _to, uint256 _numberToMint) internal view returns(bool) {
