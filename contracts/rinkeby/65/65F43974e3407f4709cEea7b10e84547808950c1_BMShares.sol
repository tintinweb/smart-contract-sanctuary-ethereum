...
contract BMShares is IBMShares, ERC20, AbstractDividends, Killswitch {
    ...
    function mintForEth(address for_, uint256 amount) external override payable killswitch {
        ...
        require(msg.value >= amount * priceWei, "value doesn't match");
...
abstract contract AbstractDividends is IDividends {
    ...
    function sharesOf(address holder) external override view returns (int256) {
        return _sharesOf(holder);
    }
    ...
    function distribute() external override {
        ...
        if (contractBalance > _expectedBalance)
            distribute(contractBalance - _expectedBalance);
    }
    ...
    function stake(address holder, uint256 shares) internal correctPts(holder, -int256(shares)) {
        _totalShares = _totalShares.add(shares);
    }

    function unstake(address holder, uint256 shares) internal correctPts(holder, int256(shares)) {
        _totalShares = _totalShares.sub(shares);
    }

    function withdrawFor(address payable holder) public override {
...
interface IDividends {
    function sharesOf(address investor) external view returns (int256);
    ...
    function withdrawFor(address payable investor) external;
