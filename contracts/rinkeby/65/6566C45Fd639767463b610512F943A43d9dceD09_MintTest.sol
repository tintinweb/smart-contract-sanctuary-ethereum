...
contract MintTest is Ownable, ERC721A, ReentrancyGuard {
    ...
    // set price 1 ether: 1000000000000000000
    uint256 public allowListPrice = 0 ether; //elder and baptized
    uint256 public believerPrice = 0.008 ether; //believer
    ...
    uint256 private constant _TotalCollectionSize = 88; // 원래 8888개인데 테스트를 위해 100배 축소
    uint256 public MAX_PER_Transaction = 2; //이 값은 실제 서비스할 값과 같다 다만 public 에서는 수정이 필요할 수 있다
    uint256 public MAX_PER_Address_FOR_ALLOW_LIST = 2;// 확정민팅 전용 수량제한. public 에도 제한을 두려면 따로 설정 필요
    ...
    address public royaltyAddress; // 로얄티 관련하여 : opensea 나 다른 레거시 마켓플레이스는 이 로얄티를 지원하지 않는다고 한다 그러므로 지원하지 않는 마켓플레이스에 한하여 직접 가서 설정필요 
    uint96 public royaltyFeesInBips;
    // set each spots    
    uint256 private teamNFTsTaked;
    uint16 private teamLimit = 5; // 팀 물량 
    
    uint256 private soldElder;
    uint256 private elderSpots = 16; // 100배 줄임 

    uint256 private soldBaptized;
    uint256 private BaptizedSpots = 28; // 100배 축소 
    // 이 정보들이 너무 길어서 나중에 수정이 필요할 듯 
     
    constructor( string memory _hiddenURI, uint96 _royaltyFeesInBips, bytes32 _root, address _teamWallet) ERC721A("OpenseaMintForTest","OMFT")
    {
        //setBaseURI(_uri); //혹시 모르니까 
    ...
    modifier checkMintCount(uint256 _quantity,uint256 _price){
        require(totalSupply() + _quantity <= _TotalCollectionSize, "reached max supply");
        require(_quantity <= MAX_PER_Transaction, "Max per transaction exceeded");
        require(msg.value >= _price * _quantity, "Need to send more ETH."); // 여기로 넘어 오는지 확인이 필요함 
         _;
    }

    // sale function
     function reserveNFT(uint256 quantity) public onlyOwner {
        require(totalSupply() + quantity <= _TotalCollectionSize, "reached max supply");
        require(teamNFTsTaked + quantity <= teamLimit, "Reserve limit exceeded.");
        teamNFTsTaked = teamNFTsTaked + quantity;
    ...
    function whoMetCocoMint(uint256 quantity, bytes32[] calldata merkleproof) public payable checkMintCount(quantity,allowListPrice){     
         ...
         require((_numberMinted(msg.sender) + quantity <= MAX_PER_Address_FOR_ALLOW_LIST),"Quantity exceeds allowed Mints"); // 수정하는 포인트 

        if(saleStatus == SaleStatus.BAPTIZED) {
            require(soldBaptized + quantity <= BaptizedSpots,"Baptized sold out");
            soldBaptized = soldBaptized + quantity;
        } else {
            require(soldElder + quantity <= elderSpots,"ELDER sold out");
            soldElder = soldElder + quantity;
        }
        ...
        require( _exists(tokenId), "ERC721Metadata: URI query for nonexistent token");
           if (revealed) {
            ...
            return bytes(baseURI).length > 0 ? string( abi.encodePacked(baseURI, tokenId.toString()) ) : "";
                    // abi.encodePacked(baseURI, tokenId.toString(), ".json")이 아니라 이렇게 설정 문버드 링크 특장
        } else {
    ...
    function withdraw() public onlyOwner nonReentrant { 
        // If you delete this, no matter how much money you earn, you will not be able to transfer money.
    ...
    function airdrop(address beneficiary, uint256 amount) public onlyOwner {
        ...
        require(totalSupply() + amount <= _TotalCollectionSize, "reached max supply");
