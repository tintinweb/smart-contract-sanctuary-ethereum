...
contract NFTToken is ERC721, ERC721Enumerable, Ownable {
    ...
    /* Logic
        - consider an array of size equal to totalMintSupply
    ...
    */
    function safeMint() public payable {
        ...
        require(msg.value >= mintPrice, 'Insufficient purchase amount');
        payable(owner()).transfer(address(this).balance);

        uint256 _remainingAmt = totalMintSupply - _mintedAmt.current();
        require(_remainingAmt > 0, 'All tokens already minted');
        uint256 _maxIdx = _remainingAmt - 1;

        // randomIdx >= 0 && randomIdx <= _maxIdx
        uint256 _randomIdx = uint256(
            keccak256(
                abi.encodePacked(
                    msg.sender,
                    block.coinbase,
        ...
        _safeMint(msg.sender, _randomVal);
    }

    function modifyWhitelist(address[] calldata _accounts, bool _isAdd)
        external
        onlyOwner
    {
        if (_isAdd) {
    ...
    function manageWhitelisting(bool _isWhitelistOnly) external onlyOwner {
