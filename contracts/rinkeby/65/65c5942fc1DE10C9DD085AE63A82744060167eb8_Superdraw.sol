...
// CONSTRUCTOR
        ...
        onlyWhitelisted
    {
        ...
        emit TicketsAdded(amountAddedToPrizePool, numTickets);                // Write an event to the chain
    }
}
...
struct Draw { uint256 winnerPlayerIndex; bool prizeClaimed; address[] players; uint256 prizePool; State drawStatus; uint256 target; uint256 winnerPrize; uint256 numberOfTickets; }
...
// EVENTS
    ...
    event ContractAddressChanged(string addressType, address newAddress);
    event TicketsAdded(uint256 pricePaid, uint256 totalTickets);
    event DrawClosed(uint256 burnedAmount, uint256 closerFee, uint256 nextDrawAmount);
    event PrizeClaimed(uint256 amountClaimed);
    event PayoutStructureChanged(uint256 winnerPercent, uint256 burnPercent, uint256 closerFeePercent, uint256 nextDrawPercent, uint256 superDrawPercent);
    event DrawTargetChanged(uint256 newTarget);
...
// MODIFIERS
    ...
    modifier afterTarget() virtual {
        ...
        require(draw.prizePool > draw.target,                         "The draw target was not reached yet");
        _;
    }
    ...
    modifier withinTarget() virtual {
        Draw memory draw = draws[getCurrentDraw()];                   // Shortcut accessor for the Draw
        require(draw.prizePool <= draw.target,                        "This draw has reached its target");
        _;
    }
    ...
    modifier drawComplete(uint256 drawIndex) virtual {
        ...
        require(draw.prizePool > draw.target,                         "This draw is still open");
    ...
    modifier isWinner(uint256 drawIndex) virtual {
        require(drawIndex <= draws.length,                            "Draw too high");
        require(drawIndex >= 0,                                       "Draw too low");
        Draw memory draw = draws[drawIndex];                          // Shortcut accessor for the Draw
...
// PUBLIC FUNCTIONS
        ...
        isWinner(drawIndex)
    {
        ...
        emit PrizeClaimed(amount);                                      // Write an event to the chain
        ...
        afterTarget
    {
        ...
        emit DrawClosed(burnAmount, closersFee, nextDrawAmount);                      // Write an event to the chain
...
// ADMIN FUNCTIONS
        ...
        onlyOwner
    {
        if ( newDrawtarget != 0 && 
            ...
            emit DrawTargetChanged(newDrawtarget);
        }
    }
        ...
        onlyOwner
    {
        ...
        emit PayoutStructureChanged(winner, burn, closerFee, nextDraw, 0);
    }
        ...
        onlyOwner
    {
        ...
        emit ContractAddressChanged("Payment Contract", paymentContractAddress);
    }
        ...
        onlyOwner
    {
        ...
        emit ContractAddressChanged("Random Contract", randomContractAddress);
    }
...
// GETTERS
    ...
    {
        if ( drawIndex > draws.length || drawIndex < 0 ) {return players;} // Return empty array if draw out of range
        Draw memory draw = draws[drawIndex];                               // Shortcut accessor for the Draw
    ...
    {
        if ( drawIndex > draws.length || drawIndex < 0 ) {return 0;}                   // Return 0 if draw out of range
        Draw memory draw = draws[drawIndex];                                           // Shortcut accessor for the Draw
    ...
    {
        if ( drawIndex > draws.length || drawIndex < 0 ) {return address(0);} // Return empty array if draw out of range
        Draw memory draw = draws[drawIndex];                                  // Shortcut accessor for the Draw
...
// EVENTS
    ...
    event AddressWhitelisted(address whitelistAddress);
...
// ADMIN FUNCTIONS
        ...
        onlyOwner
    {
        ...
        emit AddressWhitelisted(whitelistAddress);         // Write an event to the chain
    }
