...
contract Orbits is Ownable, ERC721Enumerable {

    using Strings for uint256;

    string public sourceCode;
    bytes32 private _merkleRoot = 0xa4e1d8172f44e139813262e592b13aa318afb190a9b3952f7554e57273514563;
    string private _baseTokenURI;
    uint256 public constant MAX_ORBITS = 32; // max number of ORB to be minted publicly
    uint256 public constant ABS_MAX = 305; // hard cap to prevent funky business
    uint256 public constant orbitPrice = 200000000000000000; // 0.2 ETH
    ...
    function publicMint() public payable { 
        ...
        require(publicSaleIsActive, "wait for pub"); // open minting
        require(totalMinted+_mintNr <= MAX_ORBITS, "too many"); // does adding 1 exceed max?
        require(totalSupply()+_mintNr <= ABS_MAX, "max reached"); 
        require(msg.value >= orbitPrice, "more eth"); // value needs to be correct
        if (totalMinted < MAX_ORBITS) {_safeMint(msg.sender, totalSupply());totalMinted++;}
    }
