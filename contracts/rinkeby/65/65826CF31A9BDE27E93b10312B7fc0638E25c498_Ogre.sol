...
// https://rinkeby.etherscan.io/address/0x170C1f0Df03298f5a3Cc8BFfAa327EC5140d8E99
contract Ogre is ERC165, IERC721, IERC721Metadata, Ownable, ReentrancyGuard {
    ...
    uint256 public constant WL_MINT_PRICE = 0.01 ether;
	...
	*/
    function mint(uint256 _amount) external payable {
        if (_amount > MAX_MINT_PER_TRANSACTION) {
            revert MaxLimitPerTransactionExceeded();
        }
		if (!publicStatus) {
			revert SaleNotActive();
		}
		if (msg.value < PUBLIC_MINT_PRICE * _amount) {
			revert MintPriceIncorrect();
		}
        intMint(_amount);
    }
	...
	*/
    function whitelistMint(uint256 _amount, bytes32[] calldata _proof, bytes32 _merkleRoot) external payable {
        if (!_verify(_leaf(msg.sender), _proof, _merkleRoot)) {
            revert NotAWhitelistMember();
        }
        if (_amount > MAX_MINT_PER_TRANSACTION) {
            revert MaxLimitPerTransactionExceeded();
        }
	...
	*/
    ...
    /**
		Enable or disable an address to have administrator rights
