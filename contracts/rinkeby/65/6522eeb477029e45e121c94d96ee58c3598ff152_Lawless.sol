...
contract Lawless is ERC721, Ownable, WithLimitedSupply {
	using Strings for uint256;

	/*
	 * Private Variables
	 */
	// NOTE: remember to update this numbers before deploying to production
	uint256 private constant NUMBER_OF_IMAGES = 8;//68; 
	uint256 private constant NUMBER_OF_COPIES = 25; 
	uint256 private constant MAX_MINTS_PER_ADDRESS = 2; 
    ...
    string[] private _pinataCIDs;

    // Number of copies available per each image
    mapping(string => uint256) private _availability;
    ...
    // Used in the random algorithm
    mapping(uint256 => uint256) private tokenMatrix;

	/*
	 * Constructor
	 */
	constructor(string[] memory pinataCIDs)
		ERC721('Lawless NFT', 'LAWLESS')
        //RandomlyAssigned(NUMBER_OF_IMAGES*NUMBER_OF_COPIES,0)
        WithLimitedSupply(NUMBER_OF_IMAGES*NUMBER_OF_COPIES)
	{
		require(NUMBER_OF_IMAGES == pinataCIDs.length, 'Invalid number of CIDs');
        for (uint256 i = 0; i < pinataCIDs.length; i++) {
            
            _pinataCIDs.push(pinataCIDs[i]);
            _availability[pinataCIDs[i]] = NUMBER_OF_COPIES;
        }
	}
    ...
    function getOwnedTokenId(address address_) private view returns (uint256) {

        uint256 result = 0;
        
        for (uint256 i = 0; i < super.tokenCount(); i++) {
			// If tokenId has been assigned and it is owned by address_
            if (bytes(_tokenURIs[i]).length !=0 && ownerOf(i)==address_){
                result = i;
            }
        }
    ...
    function getRandomIndex(uint256 maxIndex) private view returns (uint256,uint256){
    ...
    function notRepeated(address address_, uint256 index) private view returns (bool){

        if (_availability[_pinataCIDs[index]] > 0){
                // If it is the first mint of the sender exit the loop
                if (addressToMints[msg.sender] == 0){
                    return true;
                }else{
                    
                    // Find the already owned token ID
                    // NOTE: this is predicated on the address owning a max of 2 nfts
                    uint256 ownedToken = getOwnedTokenId(address_);

                    // NOTE: perhaps there is a more efficient way of doing this
                    if (keccak256(bytes(_tokenURIs[ownedToken])) == keccak256(bytes(_pinataCIDs[index]))){
                        return false;
                    }else{
                        return true;
                    }
    ...
    function nextToken(address address_) internal ensureAvailability returns (uint256) {
        ...
        while(flag){
            ...
            (value,random) = getRandomIndex(maxIndex);

            if (notRepeated(address_,value % NUMBER_OF_IMAGES)){
                flag = false;
            }
        }
	...
	function mint()
		...
		returns (uint256)
	{
        // TODO: if phase is presale - signer have to be in the whitelist
		require(phase == SalePhase.PublicSale, 'Public sale is not active');
		require(1 + addressToMints[msg.sender] <= MAX_MINTS_PER_ADDRESS, 'Exceeds maximum allowable mints');

        uint256 newItemId = nextToken(msg.sender);

        _mint(msg.sender, newItemId);

        uint256 tokenURIIndex = newItemId % NUMBER_OF_IMAGES;

        _tokenURIs[newItemId] = _pinataCIDs[tokenURIIndex];

		addressToMints[msg.sender] += 1;
        _availability[_pinataCIDs[tokenURIIndex]] -= 1;
	...
	function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        return string(abi.encodePacked(_tokenBaseURI, '/', _tokenURIs[tokenId]));
