 ...
 * @title Struct Raffle Manager
 * @author Augminted Labs, LLC
 * @notice Raffles are created with an IPFS-hosted list of entrants, deterministic draw script, and PRNG seed.
 *         Using the draw script, winners are calculated off-chain by anyone who wishes to verify the results.
 */
contract StructRaffleManager is AccessControl {
    event PublishRaffle(
    ...
    mapping(address => uint256) public addressRaffleCount;
    string[] public scriptURIs;
    ...
    /**
     * @notice Push a new IPFS-hosted draw script to the contract
     * @param uri IPFS-hosted script to be used to draw raffle results
     */
    function addScript(string calldata uri) public onlyRole(DEFAULT_ADMIN_ROLE) {
        scriptURIs.push(uri);
    }

    /**
     * @notice Publish a new raffle event
     * @param winnerCount Number of raffle winners
     * @param scriptIndex Index of the script used to draw raffle winners
     * @param entrantsURI IPFS-hosted list of raffle entrants
     */
    function publishRaffle(
        uint256 winnerCount,
        uint256 scriptIndex,
        string calldata entrantsURI
    )
        public
    {
        if (scriptIndex >= scriptURIs.length) revert IndexOutOfRange();
        if (winnerCount == 0) revert InvalidWinnerCount();

        emit PublishRaffle(
            msg.sender,
            addressRaffleCount[msg.sender]++,
            entrantsURI,
            winnerCount,
            scriptIndex,
            uint256(keccak256(
