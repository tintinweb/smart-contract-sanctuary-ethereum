...
contract FourLotto is VRFConsumerBaseV2, KeeperCompatibleInterface {
    ...
    uint256[][] private s_winningNumberOrderArray = [
        [1, 2, 3, 4],
        [1, 2, 4, 3],
        [1, 3, 2, 4],
        [1, 3, 4, 2],
    ...
    function enterFourLotto(string memory _playerBet) public payable {
        ...
        if (getBetDetails(_playerBet).isValid == true) {
            revert FourLotto__NumberAlreadyTaken(_playerBet);
        }
        // revert if player has already placed a bet for current draw
        if (getPlayerDetails(msg.sender).isValid == true) {
            revert FourLotto__PlayerHasAlreadyEntered(getPlayerDetails(msg.sender).bet);
        }
        ...
        )
    {
        ...
        // bool hasBalance = address(this).balance > 0; // commented out because no need for this at this point in time
    ...
    function fulfillRandomWords(
        ...
        uint256[] memory orderOfWinningNumber = s_winningNumberOrderArray[(randomWords[0] % 24)];
    ...
    function pauseFourLotto() public payable onlyOwner {
        // block of code to prevent re-entrancy
        if (s_fourLottoState != FourLottoState.OPEN) {
            revert FourLotto__FourLottoNotOpen();
        }
    ...
    // gas cost of removeHistoryOfAPastDraw too high, but left here while figuring out how to lower gas cost
    ...
    // function getCurrentBetMappingKey(string memory _bet) public view returns (bytes32) {
    //     bytes32 betMappingKey = keccak256(abi.encode(s_drawNumber, _bet));
    //     return betMappingKey;
    // }

    function getBetDetails(string memory _bet) public view returns (Details memory) {
        bytes32 key = keccak256(abi.encode(s_drawNumber, _bet));
        return s_betDetails[key];
    }

    // // KIV, hidden for the time being
    // function getCurrentPlayerMappingKey(address _playerAddress) public view returns (bytes32) {
    //     bytes32 playerMappingKey = keccak256(abi.encode(s_drawNumber, _playerAddress));
    //     return playerMappingKey;
    // }

    function getPlayerDetails(address _playerAddress) public view returns (Details memory) {
    ...
    function getWinningNumberOrderArray() public view returns (uint256[][] memory) {
        return s_winningNumberOrderArray;
    }

    // others
    function compareStrings(string memory winningString, string memory playerString)
        public
        pure
        returns (bool)
    {
        bytes memory bytesOne = bytes(winningString);
        bytes memory bytesTwo = bytes(playerString);

        if (bytesOne.length != bytesTwo.length) {
            return false;
        }
        for (uint256 i = 0; i < bytesOne.length; i++) {
    ...
    // KIV, hidden for the time being as there seems to be some issue with this block of code
    // function getRemainingAvailableBets() public view returns (string[] memory) {
    //     uint256 index = 0;
    //     string[] memory remainingAvailableBets;
    //     for (uint256 i = 0; i < 10000; i++) {
    //         string memory currentBet;
    //         uint256 zero = 0;
    //         if (i >= 0 && i < 10) {
    //             currentBet = string(
    //                 abi.encodePacked(
    //                     Strings.toString(zero),
    //                     Strings.toString(zero),
    //                     Strings.toString(zero),
    //                     Strings.toString(i)
    //                 )
    ...
    //             currentBet = string(abi.encodePacked(Strings.toString(zero), Strings.toString(i)));
    //         } else {
    //             currentBet = string(Strings.toString(i));
    //         }
    //         if (s_betDetails[keccak256(abi.encode(s_drawNumber, currentBet))].isValid == false) {
    //             remainingAvailableBets[index] = currentBet;
    //             index++;
    //         }
    //     }
    //     return remainingAvailableBets;
    // }
}
