...
contract FishItemNFT is FishItemNFTCategory {
        ...
        public
    {
        ...
        bool result = erc20.transferFrom(msg.sender, address(this), totalPrice);
    ...
    function setFishChipsAddress(address newAddress) public onlyOwner {
        _fishChipsAddress = newAddress;
        emit FishChipsAddressSet(newAddress);
    }

    function getFishChipsAddress() public view returns (address) {
...
contract FishItemNFTCategory is ERC721Enumerable, Ownable {
    ...
    constructor() ERC721("Fish-Item NFT", "FISH-ITEM") {}
    ...
    function _mint(address to, uint256 category) internal override {
        // uint256 category = extractCategory(tokenId);
        ...
        require(categoryCap > categorySupply, "Cap reached");

        _categoryMap[category].supply = categorySupply + 1;

        uint256 tokenId = (category * CATEGORY_SEPARATOR) +
            categorySupply;
    ...
    function _burn(uint256 tokenId) internal override {
        uint256 category = extractCategory(tokenId);
        _categoryMap[category].supply = _categoryMap[category].supply - 1;

        super._burn(tokenId);
    }

    function extractCategory(uint256 tokenId) public pure returns (uint256) {
