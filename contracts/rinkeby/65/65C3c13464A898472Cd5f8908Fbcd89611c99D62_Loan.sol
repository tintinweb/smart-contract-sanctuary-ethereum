...
//        priceFeed = AggregatorV3Interface(0xdCA36F27cbC4E38aE16C4E9f99D39b42337F6dcf);
    ...
    function depositTokenA(uint256 _amount) public payable returns(bool){
        ...
        depositId++;
        int256 prices = getLatestPrice();
        uint256 tokenADeopsit = _amount;  
        uint256 weiAmt = _amount*(10**12)/uint256(prices);
    ...
    mapping (uint256 => uint256) requests;  // depositId to requestId
    mapping (uint256 => Request) requestData;   // requestId to Request
    // requestForWthdrawl
    ...
    // }
    int public price;
    function setPriceManual(int _price) public {
        price = _price*(10**12);
    }
    function getLatestPrice() public view returns (int) {
        return price;
