...
contract Noji is ERC721A, IERC2981, Ownable {
    ...
    uint256 public WHITELIST_SALE_PRICE = 0.5 ether;
    uint256 public PUBLIC_SALE_PRICE = 0.75 ether;
    ...
    modifier callerIsUser() {
        require(
            ...
            "Noji :: The caller is another contract"
        );
        _;
    }

    modifier mintIsActive(bool category) {
        require(!isPaused, "Noji :: Minting is paused");
        require(category, "Noji :: Minting is not active for this list");
        _;
    }

    modifier canAfford(uint256 categoryPrice, uint256 qty) {
        require(
            msg.value >= categoryPrice * qty,
            "Noji :: Funds not sufficient"
        );
        _;
    }
    ...
    modifier canMint(uint256 qty) {
        require(
            ...
            "Noji :: Cannot mint beyound Max Supply"
        );
        require(
            balanceOf(msg.sender) + qty <= MAX_NOJI_PER_WALLET,
            "Noji :: Cannot mint beyound Max Noji per Wallet"
        );
        require(qty > 0, "Noji :: You must mint at least 1 Noji");
        _;
    }
    ...
    modifier isValidMerkleProof(
        ...
        require(
            ...
            "Noji :: Address does not exist in list"
        );
        _;
    }
        ...
        canMint(qty)
    {
        ...
        refundIfOver(qty * PUBLIC_SALE_PRICE);
    }
    ...
    {
        _safeMint(msg.sender, qty);
        refundIfOver(qty * WHITELIST_SALE_PRICE);
    }
    ...
    {
        require(
            !teamListClaimed[msg.sender],
            "Noji :: You've already claimed your Noji"
        );
    ...
    function setRoyaltyInfo(address receiver, uint8 _royaltyFeesInBips)
        ...
        onlyOwner
    {
        require(_royaltyFeesInBips <= 10000, "Noji: Royalty fee is too high");
    ...
    function withdraw() external onlyOwner {
        ...
        require(balance > 0, "Noji :: There are no payout to withdraw");
        (bool success, ) = payable(msg.sender).call{value: balance}("");
        require(success, "Noji :: Withdrawal failed");
    }
