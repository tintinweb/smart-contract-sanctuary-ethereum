...
contract CloneMachineProxy is IJuice, ReentrancyGuard, Ownable {
    ...
    function upgradeD2Clone(
        ...
        require(cloneToLevel[upgradedCloneId] == 0, "upgraded clone is not d1");
        require(cloneToLevel[copiedCloneId] == 0, "copied clone is not d1");
        require(upgradedCloneId != copiedCloneId, "can't use the same id");
        
        require(cloneContract.ownerOf(upgradedCloneId) == msg.sender, "not clone owner");
        require(cloneContract.ownerOf(copiedCloneId) == msg.sender, "not clone owner");
        require(realJuice.ownerOf(juiceId) == msg.sender, "not juice owner");

        require(!copiedD1Clones[copiedCloneId], "clone already copied for this level");
    ...
    function upgradeD3Clone(
        ...
        require(cloneToLevel[cloneId] == 0, "not d1");

        require(cloneContract.ownerOf(cloneId) == msg.sender, "not clone owner");
        require(realJuice.ownerOf(juiceId) == msg.sender, "not juice owner");
