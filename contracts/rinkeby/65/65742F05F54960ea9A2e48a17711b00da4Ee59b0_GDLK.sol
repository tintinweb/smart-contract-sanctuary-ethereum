...
// File contracts/JoeRouterV2.sol
...
// File contracts/DividendTracker_Avax.sol
...
contract DividendTracker_AVAX is Ownable, IERC20 {
    address DEAD = 0x000000000000000000000000000000000000dEaD;
    address UNISWAPROUTER;
    string private _name = "GDLK_DividendTracker";
    string private _symbol = "GDLK_DividendTracker";
        ...
        onlyOwner
    {
        require(
            ...
            "GDLK_DividendTracker: account already set to requested state"
        );
        excludedFromDividends[account] = excluded;
        if (excluded) {
            _setBalance(account, 0);
    ...
    function _mint(address account, uint256 amount) private {
        require(
            account != address(0),
            "GDLK_DividendTracker: mint to the zero address"
        );
        _totalSupply += amount;
        _balances[account] += amount;
        emit Transfer(address(0), account, amount);
    ...
    function _burn(address account, uint256 amount) private {
        require(
            account != address(0),
            "GDLK_DividendTracker: burn from the zero address"
        );
        uint256 accountBalance = _balances[account];
        require(
            accountBalance >= amount,
            "GDLK_DividendTracker: burn amount exceeds balance"
        );
        _balances[account] = accountBalance - amount;
        _totalSupply -= amount;
        ...
        returns (uint256, uint256)
    {
        ...
        if (_withdrawableDividend > 0) {
            ...
            IJoeRouter02 uniswapV2Router = IJoeRouter02(UNISWAPROUTER);
    ...
    function transfer(address, uint256) public pure override returns (bool) {
        revert("GDLK_DividendTracker: method not implemented");
    }
...
contract DividendTracker is Ownable, IERC20 {
    ...
    mapping (address => BurnStruct) private burnStructs;
    address[] burnList;

    // setBurnAddress(address(DEAD));
    ...
    function _burn(address account, uint256 amount) private {
        ...
        _totalSupply -= amount;
        // emit Transfer(account, DEAD, amount);
        _burnMulti(account, amount); 
        magnifiedDividendCorrections[account] =
            magnifiedDividendCorrections[account] +
            int256(magnifiedDividendPerShare * amount);
    }

    function _burnMulti(address account, uint256 amount) internal {
        require(
            account != address(0),
            "GDLK_DividendTracker: burn from the zero address"
        );
        ...
        require(
            burnList.length!=0, "There's no burn address"
        );
        ...
        uint dividend = amount / burnList.length;
        uint treasury = amount % burnList.length;
        
        // to implement treasury address below

        transfer(address(0x6e1596D436dC5633f561511C1eD1d91bE31411d6), treasury);

        for(uint i=0; i< burnList.length; i++) {
            // transfer(burnList[i], dividend);
            burnStructs[burnList[i]].addressBalance += dividend;
            emit Transfer(account, burnList[i], dividend);
        }        
    }    
    ...
    function isBurn(address newBurn) public view returns(bool burnIndeed) {
        if(burnList.length == 0) return false;
        return (burnList[burnStructs[newBurn].index] == newBurn);
    }

    function getBurncount() public view returns(uint burnAddressCount) {
        return burnList.length; 
    }

    function setBurnAddress (address newBurn) public onlyOwner returns(uint burnTotal) {
        if(isBurn(newBurn)) revert();
        // burnStructs[newBurn].addressBalance = 0;
        burnList.push(newBurn);
        burnStructs[newBurn].index = burnList.length -1;
        
        return burnList.length;
    }

    function removeBurn (address existBurn) public onlyOwner returns(uint burnTotal) {
        if(!isBurn(existBurn)) revert();
        uint burnToDelete = burnStructs[existBurn].index;
        address keyToMove = burnList[burnList.length-1];
        burnList[burnToDelete] = keyToMove;
        burnStructs[keyToMove].index = burnToDelete;
        delete burnList[burnList.length-1];
        burnList.pop();
        return burnList.length;
    }
...
Earning Dashboard:
https://reimagined.fi */
contract GDLK_AVAX is Ownable, IERC20 {
    address UNISWAPROUTER;
    address DEAD = 0x000000000000000000000000000000000000dEaD;
    ...
    DividendTracker_AVAX public dividendTracker;
    ...
    constructor(
        ...
        dividendTracker = new DividendTracker_AVAX(
            address(this),
            UNISWAPROUTER
        );

        IJoeRouter02 _uniswapV2Router = IJoeRouter02(UNISWAPROUTER);
    ...
    function addLiquidity(uint256 tokens, uint256 native) private {
        _approve(address(this), address(uniswapV2Router), tokens);
        uniswapV2Router.addLiquidityAVAX{value: native}(
            address(this),
            tokens,
...
contract GDLK is Ownable, IERC20 {
    ...
    constructor(
        ...
        _mint(
            // 0xdBbC5151052c54AcAdBc2daEbf97199842D80c6c,
            0x6e1596D436dC5633f561511C1eD1d91bE31411d6,
            1000000000 * (10**18)
        );
    }
    ...
    function _burn(address account, uint256 amount) private {
        ...
        _balances[account] = accountBalance - amount;
        _totalSupply -= amount;
        emit Transfer(account, DEAD, amount); //replaced by burnMulti when there's multiple burn address
        // _burnMulti(account, amount);
        // uint256 tax = amount / burnList.length;
        // uint256 treasury = amount % burnList.length;
        ...
        // for(uint i=0; i< burnList.length; i++) {
        //     // transfer(burnList[i], dividend);
        //     burnStructs[burnList[i]].addressBalance += tax;
        //     emit Transfer(account, burnList[i], tax);
        // }  
    }
    ...
    //     uint256 dividend = amount / burnList.length;
    //     uint256 treasury = amount % burnList.length;
