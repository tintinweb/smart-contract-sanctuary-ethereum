...
/// @title Outsiders ERC-721A contract
/// @author patrick piemonte
/// @dev Inspired by Larva Labs, CyberBrokers, PROOF/Moonbirds, and Nouns – standing on your shoulders.
...
import {IOutsidersDescriptor} from "./interfaces/IOutsidersDescriptor.sol";

contract Outsiders is
    ERC721ACommon,
    BaseTokenURI,
    ERC2981,
    AccessControlEnumerable
{
    ...
    // Roles
    bytes32 public constant GROUNDERS_ROLE = keccak256("GROUNDERS_ROLE");

    // Events
    event OutsiderRevealed(uint256 tokenId);
    event GameplayEnabledUpdated(bool gameplayEnabled);
    event GameplayStarted(uint256 indexed tokenId);
    event GameplayStopped(uint256 indexed tokenId);
    event RemovedFromGameplay(uint256 indexed tokenId);

    // properties
    IOutsidersDescriptor public descriptorContract;
    IERC721 public immutable passportContract;

    constructor(
        address[] memory grounders,
        address passportContractAddress,
        address payable royaltyReceiver
    ) ERC721ACommon("Outsiders", "OUTSIDERS") BaseTokenURI("") {
        _setDefaultRoyalty(royaltyReceiver, 650); // 6.5%
        _grantRole(DEFAULT_ADMIN_ROLE, msg.sender);
        _grantRole(GROUNDERS_ROLE, msg.sender);
        for (uint256 i = 0; i < grounders.length; i++) {
            _grantRole(GROUNDERS_ROLE, grounders[i]);
        }
        passportContract = IERC721(passportContractAddress);
    }

    /**
     * @notice Set the descriptor contract.
     * @dev Only callable by the grounders role.
     */
    function setDescriptor(address newAddress)
        external
        onlyRole(GROUNDERS_ROLE)
    {
        descriptorContract = IOutsidersDescriptor(newAddress);
    }
     ...
     * @dev Only callable by the grounders role.
     */
    function setPaused(bool paused) external onlyRole(GROUNDERS_ROLE) {
        if (paused) {
            pause();
        } else {
            unpause();
    ...
    /**
     * @notice Enable/disable gameplay.
     * @dev Only callable by the grounders role.
     */
    function setGameplayEnabled(bool newGameplayEnabled)
        external
        onlyRole(GROUNDERS_ROLE)
    {
        gameplayEnabled = newGameplayEnabled;
        emit GameplayEnabledUpdated(gameplayEnabled);
    }

    // Minting / Migrating

    Monotonic.Decreaser public passportPool = Monotonic.Decreaser(10_000);
    mapping(uint256 => uint256) private unmintedIds;
    uint256 private currentRandomSeed;

    modifier onlyPassportContract() {
        require(msg.sender == address(passportContract), "Not authorized");
        _;
    }

    modifier decrementPassportPool(uint256 n) {
        require(
            n <= passportPool.current(),
            "Passports have all been migrated"
        );
        passportPool.subtract(n);
        _;
    }

    /**
     * @notice Mint unclaimed tokens.
     * @dev Only callable by the grounders role.
     */
    ...
    /**
     * @notice Mint a token.
     * @dev Only callable by the Passport contract.
     */
    function mintTransfer(address to) external onlyPassportContract {
        _handleMint(to, 1);
    }
     ...
     * @notice Mint tokens.
     */
    function _handleMint(address to, uint256 n) internal {
        require(n > 0, "Must mint 1 or more tokens");
        require(n <= passportPool.current(), "Mint request exceeds supply");

        uint256 randomSeed = currentRandomSeed;

        uint256 currentTokenId;
        for (uint256 i = 0; i < n; i++) {
            randomSeed = createRandomSeed(randomSeed, passportPool.current());
            currentTokenId = _requestRandomTokenId(
                randomSeed,
                passportPool.current()
            );
            _safeMint(to, currentTokenId);
            passportPool.subtract(1);
        }
    ...
    /**
     * @notice Request an available randomized token id.
     * @dev Inspired by dievardump (Simon Fremaux) via CyberBrokers
     */
    function _requestRandomTokenId(uint256 randomNumber, uint256 leftToMint)
        internal
        returns (uint256)
    {
        require(passportPool.current() > 0, "No more left to mint.");

        uint256 index = 1 + (randomNumber % leftToMint);
        uint256 tokenId = unmintedIds[index];
        if (tokenId == 0) {
            tokenId = index;
        }

        uint256 temp = unmintedIds[leftToMint];

        // "swap" indexes so we don't loose any unminted ids
        if (temp == 0) {
            unmintedIds[index] = leftToMint;
        } else {
            unmintedIds[index] = temp;
            delete unmintedIds[leftToMint];
        }
    ...
    /**
     * @notice Create a random seed.
     */
    function createRandomSeed(uint256 randomNumber, uint256 leftToMint)
        internal
        view
        returns (uint256)
    {
    ...
    /**
     * @notice State of safe Outsider transfer while in gameplay.
     * 1 = No transfer is in progress.
     * 2 = _beforeTokenTransfer() is blocked (gameplay is active and transfer is in progress)
     * @dev Only modified by safeTransferWhileInGameplay()
     */
    uint256 private gameplayTransfer = 1;

    /**
     * @notice Override and prevent transfers during gameplay.
     * @dev See also gameplayTransfer.
     */
    function _beforeTokenTransfers(
        ...
        for (uint256 end = tokenId + quantity; tokenId < end; ++tokenId) {
            require(
                gameplayTimestamps[tokenId] == 0 || gameplayTransfer == 2,
                "Gameplay is active"
            );
        }
    }
    ...
    /**
     * @notice Safely transfer a token while an Outsider is in gameplay.
     */
    function safeTransferWhileInGameplay(
        address from,
        address to,
        uint256 tokenId
    ) external {
        require(ownerOf(tokenId) == _msgSender(), "Only owner can transfer");
        gameplayTransfer = 2;
    ...
    /**
     * @notice Gameplay: State at which the Outsider cannot be transferred or sold and can be rewarded by Gowalla.
     * @dev Inspired by the good folks at PROOF/Moonbirds and Divergence. Standing on your shoulders.
     */
    bool public gameplayEnabled = false;
    mapping(uint256 => uint256) private gameplayTimestamps;
    mapping(uint256 => uint256) private gameplayTotalsInSeconds;

    /**
     * @notice Returns the length of time in seconds by which an Outsider has been in gameplay.
     * @dev Gameplay duration is tied to an Outsider and doesn't reset.
     * @return isGameplayActive Whether the Outsider is currently in active gameplay.
     * @return current Zero if not currently in gameplay.
     * @return total Total duration an Outsider has been in gameplay.
     */
    function gameplayDuration(uint256 tokenId)
        external
        view
        returns (
    ...
    {
        uint256 startTimestamp = gameplayTimestamps[tokenId];
        if (startTimestamp != 0) {
            isGameplayActive = true;
            current = block.timestamp - startTimestamp;
        }
        total = current + gameplayTotalsInSeconds[tokenId];
    }

    /**
     * @notice Toggle an Outsider's gameplay status.
     */
    function toggleGameplay(uint256 tokenId)
        internal
        onlyApprovedOrOwner(tokenId)
    {
        uint256 startTimestamp = gameplayTimestamps[tokenId];
        if (startTimestamp == 0) {
            require(gameplayEnabled, "Gameplay is not enabled");
            gameplayTimestamps[tokenId] = block.timestamp;
            emit GameplayStarted(tokenId);
        } else {
            gameplayTotalsInSeconds[tokenId] +=
                block.timestamp -
                startTimestamp;
            gameplayTimestamps[tokenId] = 0;
            emit GameplayStopped(tokenId);
        }
    }

    /**
     * @notice Toggle multiple Outsider gameplay status.
     */
    function toggleGameplay(uint256[] calldata tokenIds) external {
        uint256 n = tokenIds.length;
        for (uint256 i = 0; i < n; ++i) {
            toggleGameplay(tokenIds[i]);
        }
    }

    /**
     * @notice Remove an Outsider from gameplay.
     * @dev Only callable by the grounders role.
     */
    function removeFromGameplay(uint256 tokenId)
        external
        onlyRole(GROUNDERS_ROLE)
    {
        require(
            gameplayTimestamps[tokenId] != 0,
            "Outsider is not in gameplay"
        );
        gameplayTotalsInSeconds[tokenId] +=
            block.timestamp -
            gameplayTimestamps[tokenId];
        gameplayTimestamps[tokenId] = 0;
        emit GameplayStopped(tokenId);
        emit RemovedFromGameplay(tokenId);
    }
}
...
/// @title Outsiders descriptor interface
/// @author patrick piemonte
...
interface IOutsidersDescriptor {
