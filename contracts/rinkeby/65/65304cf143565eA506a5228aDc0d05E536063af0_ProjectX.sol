...
contract ProjectX is ERC721, IERC2981, Ownable, ReentrancyGuard {
  ...
  function mintPreSale(
    ...
    require(_verify(merkleProof, sender, maxAmount, mintType), "Invalid proof");
    
    if(mintType == MintType.BlooList) {
      require(msg.value == priceBlooList * amount, "Incorrect payable amount");
      _alreadyMinted[sender] += amount;
      _internalMint(sender, amount);
    } else if (mintType == MintType.Raffle){
  ...
  function _verify(
    ...
    bytes32 leaf = keccak256(abi.encodePacked(sender, maxAmount.toString(), uint(mintType).toString()));
  ...
  function royaltyInfo(uint256 _tokenId, uint256 _salePrice) external view returns (address, uint256 royaltyAmount) {
    ...
    royaltyAmount = (_salePrice / 1000) * 65;
