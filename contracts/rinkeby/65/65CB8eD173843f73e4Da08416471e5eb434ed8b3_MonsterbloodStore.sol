...
// File: contracts/ShitPlungerStore.sol
...
contract MonsterbloodStore is Ownable {
    ...
    IMonsterblood public immutable _monsterblood;
    IERC20 public immutable _rubbishCoin;
    uint32 public immutable _walletLimit;

    AuctionConfig public _auctionConfig;
    uint32 public _maxSupply = 3333 - 1862;
    uint32 public _minted;
    mapping(address => uint32) public _userMinted;

    constructor(address rubbishCoin, address monsterblood, uint32 walletLimit) {
        _rubbishCoin = IERC20(rubbishCoin);
        _monsterblood = IMonsterblood(monsterblood);
        _walletLimit = walletLimit;
    }

    function mint(uint32 amount) external payable {
        require(tx.origin == msg.sender, "MonsterbloodStore: ?");
        AuctionConfig memory auctionConfig = _auctionConfig;

        require(auctionConfig.startTime != 0 &&  block.timestamp > auctionConfig.startTime, "MonsterbloodStore: Not Started");
        require(_minted + amount <= _maxSupply, "MonsterbloodStore: Exceed max supply");
        require(_userMinted[msg.sender] + amount <= _walletLimit, "MonsterbloodStore: Exceed wallet limit");

        uint32 price = internalAuctionPrice(auctionConfig);
        uint256 requiredValue = price * PRICE_MULTIPLIER * amount;
        _rubbishCoin.transferFrom(msg.sender, address(this), requiredValue);

        _minted += amount;
        _userMinted[msg.sender] += amount;
        _monsterblood.mint(msg.sender, amount);
    }
    ...
    function setAuctionState(
        ...
        require(startingPrice <= endingPrice, "MonsterbloodStore: Starting Price too low");
        require(duration % interval == 0, "MonsterbloodStore: Duration % Interval != 0");

        uint32 priceDifference = endingPrice - startingPrice;
        uint32 step = duration / interval;
        require(priceDifference % step == 0, "MonsterbloodStore: PriceDiff % Step != 0");
