 ...
 */
contract Target {
  event CalledByExecutor(address indexed executor, address indexed sender, address indexed executor2);
  event UpdateErrorReason(string reason);
  ...
  function updateValueUnpermissioned(uint256 newValue) external {
    emit CalledByExecutor(executor, msg.sender, address(1));
    value = newValue;
  }

  // Permissioned function
  function updateValuePermissioned(uint256 newValue) external {
    emit CalledByExecutor(executor, msg.sender, address(IExecutor(msg.sender))); 
    uint32 origin;
    try IExecutor(msg.sender).origin() returns(uint32 v) {
      origin = v;
    } catch Error(string memory reason) {
      emit UpdateErrorReason(reason);
    }
    value = newValue;
  }
