...
contract S7SWAP {
  ...
  function exchange (address _openTrader, address _openContractAddress, address _closeTrader, address _closeContractAddress) public {
      ...
      require(openERC20Contract.approve(msg.sender, openTotal));
      require(openERC20Contract.transfer(msg.sender, openTotal));
  ...
  function check (address _openTrader, address _openContractAddress, address _closeTrader, address _closeContractAddress) public view returns (address msgSender, address addressThis, uint256 openTotalValue) {
      SLCToken openERC20Contract = SLCToken(_openContractAddress);
      SLCToken closeERC20Contract = SLCToken(_closeContractAddress);
      uint256 openTotal = openERC20Contract.totalSupply();
      uint256 closeTotal = closeERC20Contract.totalSupply();
      ...
      // require(openERC20Contract.transfer(address(this), openTotal));

      return (msg.sender, address(this), openTotal);
  }
