    ...
    Ownable
{
    ...
    constructor() ERC721 ("Puuunks - NotAudited.xyz", "Puuunks") {}
    ...
    mapping(uint256 => uint256) public tokenList;
    mapping(uint256 => uint256) public tokenWidth;
    uint256 nextMint = 1;
    uint256 maxWidth = 16384000;
    ...
    function _mint() internal {
        ...
        tokenList[nextMint] = 1;
        tokenWidth[nextMint] = 1000;
        ...
        returns (string memory)
    {
        ...
        // replace the head of the punk svg with a stretched style and filled background etc
        ...
        string memory output = string.concat(parts[0], parts[1], parts[2], parts[3], parts[4], parts[5]);
    ...
    // set market
    function authorRegisterCryptoPunksMarket(address addr) public onlyOwner{
        cryptoPunks = CryptoPunksMarket(addr);
    }
    // set data
    function authorRegisterCryptoPunksData(address addr) public onlyOwner{
        cryptoPunksData = CryptoPunksData(addr);
    }
    // set desc for metadata because its looong for puuunks
    function authorSetDesc(bytes calldata newDesc) public onlyOwner{
        desc = newDesc;
    }
    // set storage location for images
    ...
    function _beforeTokenTransfer(
        ...
        super._beforeTokenTransfer(from, to, tokenId);
        // anything too far above 16M seems to upset most svg interpreters 
        // I've seen up to 25-30M work okay which is around where this maxes out
        if(tokenWidth[tokenId] < maxWidth){
            // 1.2x to 2x growth rate depending on tokenId 0th to 20th
            // (tokenId %20)*50)+1150
            // it's scaled up and divided by 1000 to behave like a decimal place
