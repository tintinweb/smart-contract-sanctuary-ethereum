...
contract BrokerDefiPriceConsumer is Ownable{
    ...
    function mint() public payable {
        require(msg.value >= getTokenPriceInEth(), "Insufficient balance");
        test.transfer(msg.value);
    }

    function getTokenPriceInEth(uint ethPrice, uint price, uint _divider, uint _raiseToPower) public payable returns(uint) {
        require(msg.value >= ((price / (ethPrice / _divider)) * 10**_raiseToPower), "insufficient value");
        test.transfer(((price / (ethPrice / _divider)) * 10**_raiseToPower));
