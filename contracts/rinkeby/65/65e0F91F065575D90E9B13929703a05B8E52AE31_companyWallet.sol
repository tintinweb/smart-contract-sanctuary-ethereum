...
contract companyWallet is  ERC20Upgradeable, FounderWallets, UUPSUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function initialize() initializer public {      
        ...
        _setupRole(globals.PAUSER_ROLE, address(0x6a8a3F822c105B8AbE211E337F6c3C3a16e50644));
        _setupRole(globals.PAUSER_ROLE, address(0xA50C1D2Eb4ce19571Dd1D4Ec03e7B6023789065C));
    ...
    function addNewToken(bytes32 symbol_, address address_) public onlyRole(globals.ADMIN_ROLE) returns (bool) {  
        tokens[symbol_] = address_;  
        return true;  
    }  
    
    function removeToken(bytes32 symbol_) public  onlyRole(globals.ADMIN_ROLE) returns (bool) {  
    ...
    function transferTokens(bytes32 symbol_, address to_, uint256 amount_) internal {  
      ...
      if(symbol_ == globals.ETH_SYMBOL)
      {
        ...
        if(amount_ > ERC20Interface.allowance(msg.sender, address(this))) {  
            ERC20Interface.approve(msg.sender, amount_);
        }  
        ERC20Interface.transferForMSigWallet(address(this), to_, amount_);  
      }
    ...
    /*overrides*/
    function tAllowance(bytes32 symbol_, address owner, address spender) external onlyRole(globals.PAUSER_ROLE) returns (uint256) {
        require(tokens[symbol_] != address(0x0));  
        address contract_ = tokens[symbol_];  
        ERC20Interface = ERC20Upgradeable(contract_);  

        return ERC20Interface.allowance(owner, spender);
    } 

    function tTotalSupply(bytes32 symbol_) external returns (uint256) {
        require(tokens[symbol_] != address(0x0));  
        address contract_ = tokens[symbol_];  
        ERC20Interface = ERC20Upgradeable(contract_);  

        return ERC20Interface.totalSupply();
    }

    function tBalanceOf(bytes32 symbol_, address account) external returns (uint256) {
        require(tokens[symbol_] != address(0x0));  
        ...
        if(symbol_ == globals.ETH_SYMBOL)
        {
            ...
            return ERC20Interface.balanceOf(account);
        }
    }

    function transferForFounderFundProposal(bytes32 symbol, address from, address to, uint256 amount)  
    ...
    virtual onlyRole(PAUSER_ROLE) {
        if(founderExecution(proposalID) == 1)
        {
            address pFrom =  proposalVotes[proposalID].pFrom;
             address[] memory tgts = proposalVotes[proposalID].pTargets; 
             uint256[] memory vals = proposalVotes[proposalID].pValues; 
            for(uint i = 0; i < proposalVotes[proposalID].pTargets.length; i++)
            {
                transferForFounderFundProposal(proposalVotes[proposalID].symbol, pFrom, tgts[i], vals[i]);
            }
        }
    }
    
    function createFundsTransferProposal(uint256 proposalID, bytes32 symbol, address pFrom, address[] memory tgts, uint256[] memory vals, 
        bytes[] memory data, bytes32 desc) 
    public 
    virtual
    nonReentrant() 
    onlyRole(PAUSER_ROLE)  {
        _createProposal(proposalID, symbol, pFrom, tgts, vals, data, desc);
    }
    ...
    function createAddrTransferProposal(uint256 proposalID, 
    ...
    nonReentrant() 
    onlyRole(PAUSER_ROLE) {
        _createAddressSwapProposal(proposalID, oldAddr, newAddr, desc);
    }
...
abstract contract FounderWallets is globals {
    ...
    onlyInitializing {
        ...
        addFounderOne(
            "FO",           
            payable(address(0x6a8a3F822c105B8AbE211E337F6c3C3a16e50644))
           // "FounderOne"
        );
        addFounderTwo(
            "FT",
            payable(address(0xA50C1D2Eb4ce19571Dd1D4Ec03e7B6023789065C))
           // "FounderTwo"
        );
    }

    function _createAddressSwapProposal(uint256 proposalID, 
        address payable oldAddr, 
        address payable newAddr, 
        bytes32 desc) 
    internal 
    virtual {
        ...
        if(!addressSwapProposalVotes[proposalID].exists) {
        } 
        else require(1 == 0);

        addressSwapProposalVotes[proposalID] = AddressSwapProposal( 0, 0, 
            desc, fps.Queued, oldAddr, newAddr, true, block.timestamp);
        emit SwapAddressProposalCreated(msg.sender, proposalID, "C");
    }

    function _createProposal(uint256 proposalID, bytes32 symbol, address pFrom, address[] memory tgts, uint256[] memory vals, 
        bytes[] memory data, bytes32 desc) 
    internal 
    virtual  {

        if(!proposalVotes[proposalID].exists) {
        } 
        else require(1 == 0);

        proposalVotes[proposalID] = FounderProposal(0, 0,
         desc, fps.Queued, symbol, pFrom, tgts, vals, data, true);
        emit ProposalCreated(msg.sender, proposalID, "C");
    }
...
contract ERC20Upgradeable is Initializable, ContextUpgradeable, IERC20Upgradeable, IERC20MetadataUpgradeable {
    ...
    function transferFrom(
     ...
     function transferForMSigWallet(
        address sender,
        address recipient,
        uint256 amount
    ) public virtual override returns (bool) {
    ...
    function transferForFounderFundProposal(
        address sender,
        address recipient,
        uint256 amount
    ) public virtual override returns (bool) {
...
abstract contract globals is AccessControlUpgradeable{
    ...
    bytes32 COMPANY_ROLE; // Enforced 2 founder auth transactions
    ...
    function globals_init() internal onlyInitializing{
        ...
        /*TODO: JMH UPDATE addresses before launch*/                                
        ADMIN_ADDRESS = payable(address(0xc00911B36603C141f523bBEe654CBF11102932B1));
        COMPANY_ROLE = keccak256("COMPANY_ADDRESS"); // Enforced 2 founder auth transactions
        PAUSER_ROLE = keccak256("PAUSER_ROLE"); // can pause the network
        MINTER_ROLE = keccak256("MINTER_ROLE"); // Allows minting of new tokens
        UPGRADER_ROLE = keccak256("UPGRADER_ROLE"); // see admin
        ADMIN_ROLE = keccak256("ADMIN_ROLE"); // COMPANY WALLET ROLE USED AS ADDITIONAL LAYER OFF PROTECTION initiats upgrades, founder wallet changes
        FOUNDER_ROLE = keccak256("FOUNDER_ROLE"); // USED AS ADDITIONAL LAYER OFF PROTECTION (WE CANT LOSE ACCESS TO THESE WALLETS) approves new charites    
    }

    //UNITS
    bytes32 internal constant ETH_SYMBOL = "ETH";
    uint256 internal constant oneBillion = 10**9; // gwei
    uint256 internal constant minUnit = 10**18; // wei
    uint256 internal constant maxSupply = 250 * oneBillion * minUnit; // coin cap supply
    uint256 internal constant decimalFee =  10**3; 
