    ...
    IERC721Receiver
{
    ...
    mapping(uint256 => address) public ownerOfNFTMobile;
    ...
    mapping(uint256 => StateOfNFT) public TokenIdToState;
    mapping(uint256 => bytes) private TokenIdToListing;

    modifier onlyAuctionAndOwner(uint256 _tokenId) {
        if (msg.sender != auctionAddress && ownerOf(_tokenId) != msg.sender)
            revert InvalidOwner();
        _;
    }

    constructor() ERC721("Stan", "STAN") {}

    function Decimals() public pure returns (uint256) {
        return uint256(10**18);
    }

    function setCollectionNFT(address _collectionNFTAdrress)
        external
        onlyOwner
    {
        collectionNFT = CollectionNFT(_collectionNFTAdrress);
    }

    function setAuction(address _auctionId) external onlyOwner {
        auctionAddress = _auctionId;
    }
    ...
    function createNFTByStan(
        ...
        if (!_isWeb) {
            ownerOfNFTMobile[newTokenId] = _to;
        }
        _setTokenURI(newTokenId, _tokenURI);
        TokenIdToState[newTokenId] = StateOfNFT.ACTIVE;
        collectionNFT.addNFTtoCollection(
            _requestId,
            _idCollection,
            newTokenId,
            address(this),
        ...
        collectionNFT.updateOwnerNFT(_idCollection, address(0), address(this));
        _tokenIds.increment();

        emit STAN_EVENT(_requestId, "CREATE_NFT_BY_STAN", _isWeb, newTokenId);
    ...
    function createNFT(
        ...
        collectionNFT.updateOwnerNFT(_idCollection, address(0), msg.sender);
        _tokenIds.increment();

        emit STAN_EVENT(_requestId, "CREATE_NFT", _isWeb, newTokenId);
    ...
    function getOwnerOfNFTMobile(uint256 _tokenId)
        ...
        returns (address)
    {
        return ownerOfNFTMobile[_tokenId];
    }

    function updateTokenToListing(bytes memory _listing, uint256 _tokenId)
        external
        onlyAuctionAndOwner(_tokenId)
    {
        TokenIdToListing[_tokenId] = _listing;
    }

    function setPriceNFT(uint256 _tokenId, uint256 _amount)
        external
        onlyAuctionAndOwner(_tokenId)
    {
    ...
    function processNFTClaiming(
        ...
        if (_isClaim) {
            delete ownerOfNFTMobile[_tokenId];
        } else {
            ownerOfNFTMobile[_tokenId] = _from;
        }

        collectionNFT.updateOwnerNFT(
    ...
    function updateOwnerNFTAndTransferNFT(
        ...
        address _to,
        uint256 _tokenId
    ) external onlyAuctionAndOwner(_tokenId) {
        if (ownerOfNFTMobile[_tokenId] != address(0)) return;
...
contract CollectionNFT is Ownable {
    using Counters for Counters.Counter;
    StanNFT public stanNFT;
    ...
    modifier OnlyStanOrOwner() {
        if (owner() != _msgSender() && _msgSender() != address(stanNFT))
            revert InvalidOwner();
        _;
    }
    ...
    function processCreatingCollection(
        ...
        _collectionNFT[_collectionId].maxNumber = inforCollection
            .maxColletionNumber;
        _collectionNFT[_collectionId].currentNumber.current();
        ...
        emit STAN_EVENT(_requestId, "CREATE_COLLECTION", _isWeb, 0);
    }
    ...
    function addNFTtoCollection(
        ...
        if (
            _collectionNFT[_idCollection].currentNumber.current() >
        ...
        _collectionNFT[_idCollection].currentNumber.increment();
        tokenIdToCollectionId[_tokenId] = _idCollection;

        emit STAN_EVENT(_requestId, "ADD_NFT_TO_COLLECTION", _isWeb, 0);
    }
