...
contract DearMetaverse is ERC1155, Ownable, Pausable {
    ...
    mapping(uint256 => uint256) public unmintedToIndex;
    ...
    mapping(address => uint256) private _addressToAmountWLMintedSoFar; 
    ...
    function remove(uint _index)  internal { 
        require(_index < unmintedIndexArray.length);
        unmintedIndexArray[_index] = unmintedIndexArray[unmintedIndexArray.length-1];
    ...
    function metaCitznMint(uint256 _metaCitznTokenId) external {
        require(metaCITZNTokenIdsMinted[_metaCitznTokenId] == false, "metaCITZN token provided has already been utilized to mint");
        require(totalTokenSupply + 1 < TOTAL_MAX_SUPPLY + 1, "Max supply reached");
    ...
    function whitelistMint(bytes memory _signature, uint256 _nonce) external {
        require(_addressToAmountWLMintedSoFar[msg.sender] + 1 < 2, "1 WL mint per wallet allocation exceeded");
        ...
        _addressToAmountWLMintedSoFar[msg.sender] += 1;
    ...
    function ownerMint(uint256 _numberToMint) external {
    ...
    function getUnmintedIndex(uint256 _index) external view returns (uint256) {
        return unmintedIndexArray[_index];
    }

    function getUnmintedIndexCountSoFar(uint256 _id) external view returns (uint256) {
        return unmintedIndexCountArray[_id];
    }

    function getAddressToAmountWLMintedSoFar(address _address) external view returns (uint256) {
        return _addressToAmountWLMintedSoFar[_address]; 
    }
