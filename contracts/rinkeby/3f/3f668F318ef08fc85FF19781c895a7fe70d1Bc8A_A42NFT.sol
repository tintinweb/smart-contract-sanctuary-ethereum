...
contract A42NFT is HoleTokenMetadata, Ownable {

  using Strings for uint256;
  uint256 public MAX_HOLE;
  ...
  bool private _isSaleActive;
    ...
    constructor(string memory name, string memory symbol, uint256 maxNftSupply,string memory baseurl) 
    {
      nftName = name;
      nftSymbol = symbol;
      MAX_HOLE =  maxNftSupply;
      BaseURL = baseurl;
      _isSaleActive = true;
    }
...
//, string calldata _uri
  function mint( uint256 _count) external  payable {
    ...
    uint256 afterMintSupply  =  supply + _count;
    require(afterMintSupply <= MAX_HOLE, "Purchase would exceed max supply of Hole");
    ...
    for (uint256 i = 1; i <= _count; i++) {
          ...
          if (totalSupply() < MAX_HOLE)
          {
            ...
            //"https://h0-1302369398.cos.ap-hongkong.myqcloud.com/nft1/"
            string memory uri = strConcat(BaseURL,tokenid.toString(),".json");
