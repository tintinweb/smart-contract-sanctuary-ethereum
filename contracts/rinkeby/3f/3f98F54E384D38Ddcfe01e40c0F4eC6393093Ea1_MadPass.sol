...
contract MadPass is ERC1155, Ownable {
    ...
    uint256 private constant MAX_SUPPLY_TEAM = 27;
    uint256 private constant MAX_SUPPLY_OG = 2222 - MAX_SUPPLY_TEAM;
    uint256 private constant MAX_SUPPLY_NORMAL = 5555;
    uint256 private constant MAX_MINT_USER = 2;
    uint256 private constant MAX_MINT_OG = 200;
    address private immutable ogVestingAddress;
    ...
    uint256 public mintPrice = 0.002 ether; // non zero mintPrice at init : 0.002 on testnet, change to 0.2 on mainnet
    ...
    constructor(
        ...
        address _ogVestingAddress,
        address _signingAdrress
    ) ERC1155(uri_) {
        name = _name;
        symbol = _symbol;
        ogVestingAddress = _ogVestingAddress;
    ...
    function _beforeTokenTransfer(
        ...
        require(!_frozen[from], "MADPASS: frozen");
        ...
        payable
    {
        ...
        emit NormalMint(msg.sender, amounts);
    }
    ...
    function mintBatchNormalOG(
        ...
        _mint(ogVestingAddress, 1, amounts, "0x0");
        emit OGMint(msg.sender, amounts);
    }

    function mintBatchTeam(
        uint256 amounts,
        uint256 mintLimit,
        bytes calldata signature
    ) external {
        ...
        mintedUser_ += amounts;
        mintedbyAddress[msg.sender] = mintedUser_;
        uint256 mintedNumberTeam_ = mintedNumberTeam;
        mintedNumberTeam_ += amounts;
        mintedNumberTeam = mintedNumberTeam_;
        require(amounts > 0, "MADPASS: Nil mint");
        require(
            mintedNumberTeam_ <= MAX_SUPPLY_TEAM,
            "MADPASS: All the team passes have already been minted"
        );
        require(
            mintedUser_ <= mintLimit,
            "MADPASS: User Mint Allowance Exceeded"
        ...
        _checksignature(
            ...
            msg.sender,
            mintLimit,
            uint8(MintMode.TeamUser)
        );
        _mint(msg.sender, 2, amounts, "0x0");
        emit TeamMint(msg.sender, amounts);
    }
