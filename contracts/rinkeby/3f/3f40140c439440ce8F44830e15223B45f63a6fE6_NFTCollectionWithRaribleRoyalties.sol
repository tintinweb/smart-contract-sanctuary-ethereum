...
contract NFTCollectionWithRaribleRoyalties is NFTCollection, RoyaltiesV2Impl {
     ...
     */
    modifier onlyOwner(address caller) override {
    ...
    function transferOwnership(address newOwner) public virtual onlyOwner(_msgSender()) {
    ...
    function setRoyalties(uint _tokenId, address payable _royaltiesReceipientAddress, uint96 _percentageBasisPoints) public onlyOwner(_msgSender()) {
...
contract NFTCollection is ERC721PresetMinterPauserAutoId {
    uint256 internal _maxItems;
    ...
    constructor(
        ...
        string memory baseURI,
        uint256 maxItems
    ) ERC721PresetMinterPauserAutoId(platformSettings, contractURIValue, name, symbol, baseURI) {
        _maxItems = maxItems;
    }

    function _totalItems() internal view override returns (uint256) {
        return _maxItems;
    }

    function _minterRoleHash() internal view override returns (bytes32) {
        return _rolesManagerConsts().NFT_COLLECTION_MINTER_ROLE();
    }
    ...
    function _beforeTokenTransfer(
        ...
        require(
            _getPlatformSettingsValue(_settingsConsts().NFT_COLLECTION_PAUSED()) == 0,
            "NFT_COLLECTION_TOKEN_PAUSED"
        );
    }

    function setBaseURI(string memory baseURI) external onlyOwner(_msgSender()) {
        _setBaseURI(baseURI);
    }
}
...
abstract contract ERC721PresetMinterPauserAutoId is Base, Context, ERC721Burnable, ERC721Pausable, IERC721Mintable {
     ...
     * See {ERC721-tokenURI}.
     */
    constructor(address platformSettings, string memory contractURIValue, string memory name, string memory symbol, string memory baseURIValue) Base(platformSettings) ERC721(name, symbol) {
        _setBaseURI(baseURIValue);
     ...
     * Requirements:
     *
     * - the caller must have a custom minter role. See more info in `_minterRoleHash()` function.
     */
    function mint(address to) public override onlyCustomMinter(_msgSender(), _minterRoleHash()) {
        // We cannot just use balanceOf to create the new tokenId because tokens
        // can be burned (destroyed), so we need a separate counter.
        // We increment the ID tracker, so it starts from 1.
...
contract RolesManagerConsts {
    ...
    bytes32 public constant NFT_COLLECTION_MINTER_ROLE = keccak256("NFT_COLLECTION_MINTER_ROLE");
}
...
contract PlatformSettingsConsts {
    ...
    bytes32 public constant SKULLOIDS_PAUSED = "SkulloidsPaused";

    bytes32 public constant NFT_COLLECTION_PAUSED = "NFTCollectionPaused";
