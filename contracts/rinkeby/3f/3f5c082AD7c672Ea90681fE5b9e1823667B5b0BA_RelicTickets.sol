...
/**
 * @title RelicTickets
 * RelicTickets - ERC1155 contract with IPFS support
 */
contract RelicTickets is ERC1155, Ownable {
    ...
    mapping(uint256 => uint256) public _indexed;
    ...
    /**
     * Admin functions to set the minter state
     */
    function setMinterState(address minter, bool state) public onlyOwner {
    ...
    function prepare(
        ...
        require(
            _minters[msg.sender] && _metadataToId[metadata] == 0,
            "RelicTickets: Can't prepare"
        );
        uint256 id = uint256(
            keccak256(
        ...
        _prices[id] = price;
        _indexed[index] = id;
        emit Prepared(msg.sender, id, metadata, price, max_supply);
    }

    function fixMaxSupply(uint256 id, uint256 supply) public {
        require(
            _minters[msg.sender] || msg.sender == owner(),
            "RelicTickets: Can't prepare"
        );
        _max_supplies[id] = supply;
    }

    function fixNftPrice(uint256 id, uint256 price) public {
        require(
            _minters[msg.sender] || msg.sender == owner(),
            "RelicTickets: Can't prepare"
        );
    ...
    function buy(string memory ticket) public payable returns (uint256) {
        require(
            _metadataToId[ticket] > 0,
            "RelicTickets: Minting a non-existent nft"
        );
        uint256 id = _metadataToId[ticket];
        uint256 minting_price = _prices[id];
        bool canMint = true;
        ...
        require(
            canMint && amount >= 1 && msg.value % minting_price == 0,
            "Sorry you can't mint"
        );
    ...
    /*
        This method will allow owner to change the vault address
    */
    ...
    */
    function withdrawEther() external onlyOwner {
        ...
        require(
            vault != address(0) && balance > 0,
            "Can't withdraw on black hole and balance must be greater than zero."
        );
        bool success;
        (success, ) = vault.call{value: balance}("");
