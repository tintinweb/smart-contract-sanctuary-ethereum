...
contract Twitter {
    ...
    function commentPost(uint _id_post, string memory _comment) public {
        Feed[_id_post].listmsg[Feed[_id_post].nb_comments] = Commentaires(msg.sender, _comment, block.timestamp);
        Feed[_id_post].nb_comments++;
    }

    function getComments(uint _id_post) view public returns (string[] memory) {
        string[] memory list_comments = new string[](Feed[_id_post].nb_comments);

        for(uint i = 0; i < Feed[_id_post].nb_comments; i++) {
