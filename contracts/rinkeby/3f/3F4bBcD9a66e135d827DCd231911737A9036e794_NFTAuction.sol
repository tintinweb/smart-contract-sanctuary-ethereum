...
contract NFTAuction is IERC721Receiver {
    ...
    event create_auction_event(address seller,address nft_contract,uint256 price_of_nft,uint256 duration);
    event bidding_event(address nft_contract,uint256 token_id,uint256 bidding_price);
    event execute_sale_event(address nft_contract,uint256 token_id,address bidder);
    event cancel_auction_event(address nft_contract,uint256 token_id);
    event withdraw_event(address pay_out_address,uint256 amount_withdrawed);
    event transfer_ownership(address old_owner,address new_owner);
    ...
    */
    function createTokenAuction(
        ...
        emit create_auction_event(msg.sender,_nft,_price,_duration);
    }
    /**
    ...
    */
    function bid(address _nft, uint256 _tokenId) external payable {
        ...
        emit bidding_event(_nft, _tokenId, msg.value);
    }
    /**
    ...
    */
    function executeSale(address _nft, uint256 _tokenId) external {
        ...
        emit execute_sale_event(_nft, _tokenId, msg.sender);
    }
    ...
    */
    function cancelAuction(address _nft, uint256 _tokenId) external {
        ...
        emit cancel_auction_event(_nft, _tokenId);
    }
    ...
    function getUserAuctions(address user_address,address nft_contract)public view  returns(tokenDetails[]memory) {
     ...
     require(msg.sender==owner,"only Owner can Call this Function");
      ...
      emit withdraw_event(msg.sender,msg.value);
    }
    ...
    function TransferOwnership(address _new_owner)public{
        require(msg.sender==owner,"only owner can call this function");
        owner=_new_owner;
        emit transfer_ownership(msg.sender,_new_owner);
    }
