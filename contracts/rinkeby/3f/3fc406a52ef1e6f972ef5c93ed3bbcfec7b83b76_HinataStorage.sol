    ...
    UUPSUpgradeable
{
    ...
    function initialize(address[] memory owners, address _hinata) public initializer {
    ...
    function addArtist(address _user, uint256[] calldata _tokenIds) external {
        grantRole(MINTER_ROLE, _user);

        uint256 len = _tokenIds.length;
        require(len > 0, "HinataStorage: EMPTY_IDS");
        for (uint256 i; i < len; i += 1) {
            allowedIds[_user][_tokenIds[i]] = true;
        }
    }

    function removeArtist(address _user) external {
    ...
    function mintArtistNFT(
        ...
        for (uint256 i; i < ids.length; i += 1) {
            if (allowedIds[msg.sender][ids[i]]) count++;
        }
        if (count == 0) return;
        uint256[] memory _allowedIds = new uint256[](count);
        uint256[] memory _allowedAmounts = new uint256[](count);
        uint256 k;
        for (uint256 i; i < ids.length; i += 1) {
            if (allowedIds[msg.sender][ids[i]]) {
                _allowedIds[k] = ids[i];
                _allowedAmounts[k] = amounts[i];
                k += 1;
            }
        }
        mintBatch(msg.sender, _allowedIds, _allowedAmounts, data);
    }
