...
contract DopaminePartyNFT is ERC1155NT, IDopaminePartyNFT {
    ...
    string constant name = "Dopamine Party NFTs";

    string constant symbol = "PARTY";
    ...
    /// @inheritdoc IDopaminePartyNFT
    function setTokenURI(uint256 id, string calldata newTokenURI) external onlyOwner {
        if (id >= _id) {
    ...
    function uri(uint256 id) external view returns (string memory) {
        ...
        string memory resolvedURI = tokenURI[id];
        if (bytes(resolvedURI).length == 0) {
            resolvedURI = baseURI;
        }
        return string(abi.encodePacked(resolvedURI, _toString(id)));
    }
