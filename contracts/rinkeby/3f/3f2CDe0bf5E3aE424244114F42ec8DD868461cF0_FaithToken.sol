...
contract FaithToken is ERC20, IFaithToken, Ownable {
    ...
    function mint(address to, uint256 amount) public virtual {
        require(
            ...
            "ERC20Minting: must have minter role to mint"
        );
        _mint(to, amount);
    }
    ...
    // -- Minter roles --
    function addMinter(address _minterAddress) external onlyOwner {
        minterAddress[_minterAddress] = true;
    }

    function removeMinter(address _minterAddress) external onlyOwner {
        delete(minterAddress[_minterAddress]);
    }
}
