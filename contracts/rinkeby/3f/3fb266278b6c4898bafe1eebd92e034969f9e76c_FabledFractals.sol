...
contract FabledFractals is ERC721("Flames by Fabled Fractals | Official Collection RINKEBY2", "FLAME"), ERC721Enumerable, Ownable {
    ...
    uint256 public constant BASE_PRICE = 100000000000000000; // 0.1
    ...
    address public constant DEV3_ADDRESS = 0x5daCDeB05784AAa974a7228efDf7313002B7C376;
        ...
        onlyOwner 
    {
        ...
        payable(DEV1_ADDRESS).transfer(balance * 9 / 20);
        payable(DEV2_ADDRESS).transfer(balance * 9 / 20);
        payable(DEV3_ADDRESS).transfer(balance * 2 / 20);
    }
        ...
        returns (string memory) 
    {
        ...
        return string(abi.encodePacked(BASE_URI, _tokenId, ".json"));
    }
    ...
    function setWhitelist(address[] calldata addresses)        
        ...
        onlyOwner 
    {
        require(cur_phase == 0 || cur_phase == 1, "Can not batch set whitelist outside of phase 0/1");

        for (uint i = 0; i < addresses.length; i++) {
            require(addrInArray(addresses[i], whitelist_addresses) == false, "Address already exists in whitelist");

            whitelist_addresses.push(addresses[i]);
            whitelist_limits[addresses[i]] = WHITELIST_PURCHASE_LIMIT;
        }
    }
    ...
    function setGiveaway(address[] calldata addresses, uint256[] calldata counts)        
        ...
        onlyOwner 
    {
        require(cur_phase == 0, "Can not set giveaways outside of phase 0");
    ...
    // Unit tests
    constructor() {
        require(SALE_SUPPLY + AIRDROP_MAX_SUPPLY == MAX_SUPPLY, "Sale + Airdrop supply max needs to be MAX_SUPPLY");
        require(cur_phase == 0, "Current phase needs to be 0 on init");
    }
    ...
    */
    function presaleMintNFT(uint256 _numOfTokens)
        public
        payable
    {
        require(cur_phase == 1, 'Can only mint in presale');
        require(addrInArray(msg.sender, whitelist_addresses) == true, "You are not a member of the presale");
    ...
    function giveawayMintNFT(uint256 _numOfTokens)
        ...
        payable
    {
        require(cur_phase == 1 || cur_phase == 2 || cur_phase == 3, 'Can only mint giveaways in presale and sale');
    ...
    function mintNFT(uint256 _numOfTokens)
        ...
        payable
    {
        require(cur_phase == 2 || cur_phase == 3, 'Can only mint during sale');
