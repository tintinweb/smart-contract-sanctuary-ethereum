...
contract ennefftee is Ownable, ERC721A {
    uint public public_price = 0.003 ether;
    uint public public_price_update = 0.001 ether;

    bool public public_sale_status = true;

    uint public total_supply = 0;

    string public baseURI = "ipfs://QmZYEDv2Fmfzuz39K44Satbj6CFEpM5ZWxGjEHpFFdiFNN";
    uint public maxPerTransaction = 20;  // Max limit for purchase

    mapping(address => uint[]) private token_owner;
    mapping(uint => string) private json_uri;

    constructor() ERC721A("ENNEFFTEE", "NFTME"){}
    ...
    function buy(uint _count) public payable {
      ...
      for(uint i = 0; i < _count; i++) {
          ...
          token_owner[msg.sender].push(i);
      }
      total_supply += _count;
    }

    function update_json(uint _tokenId, string calldata _jsonuri) public payable  {
      require(msg.value >= public_price_update, "Incorrect ETH amount.");
      uint[] memory tokenIds = token_owner[msg.sender];
      ...
      require(token_found == true, "Token not owned by you."); 
      json_uri[_tokenId] = _jsonuri;
    }
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        if (bytes(json_uri[tokenId]).length == 0) {
            return bytes(baseURI).length != 0 ? string(abi.encodePacked(baseURI)) : '';
        } else {
            string memory jsonURL = json_uri[tokenId];
            return bytes(jsonURL).length != 0 ? string(abi.encodePacked(jsonURL)) : '';
        }
    }
