...
contract SpaceBuns is ERC721A, Ownable {
    using Counters for Counters.Counter;
    Counters.Counter private _tokenIds;

    bytes32 public alMerkleRoot;
    bytes32 public marketingMerkleRoot;
    ...
    function alMint(bytes32[] calldata _merkleProof, uint256 _quantity) external payable {
      ...
      require(_alWallets[msg.sender] <= maxAlMint, "You reached max per wallet.");
      require(_quantity > 0, "You need to mint at least 1 NFT.");
      require(msg.value >= mintPrice * _quantity, "Insufficient ETH");
      require(maxSupply >= _tokenIds.current() + _quantity, "Sold out or Exceeds max tokens");
    ...
    function marketingMint(bytes32[] calldata _merkleProof, uint256 _quantity) external payable {
      require(isMarketingMint, "Marketing minting is not live.");
      require(MerkleProof.verify(_merkleProof, alMerkleRoot, keccak256(abi.encodePacked(msg.sender))), "Address is not on the marketing list.");
      require(_marketingWallets[msg.sender] <= maxMarketingMint, "You reached max per wallet.");
      require(_quantity > 0, "You need to mint at least 1 NFT.");
      require(msg.value >= mintPrice * _quantity, "Insufficient ETH");
      require(maxSupply >= _tokenIds.current() + _quantity, "Sold out or Exceeds max tokens");

      for (uint256 i = 0; i < _quantity; i++) {
        _marketingWallets[msg.sender]++;
        _tokenIds.increment();
      }
      _safeMint(msg.sender, _quantity);
    }

    function reserveMint(bytes32[] calldata _merkleProof, uint256 _quantity) external payable {
    ...
    function setMarketingMerkleRoot(bytes32 _merkleRoot) external onlyOwner {
      marketingMerkleRoot = _merkleRoot;
    }
    ...
    function toggleMarketingMint() external onlyOwner {
      isMarketingMint = !isMarketingMint;
      isAlMint = false;
      isPublicMint = false;
      isReserveMint = false;
    }
    ...
    function setMaxMarketingMint(uint256 _maxMarketingMint) external onlyOwner {
      maxAlMint = _maxMarketingMint;
    }
