...
contract StreethTest is ERC20, ERC20Capped, Ownable, ReentrancyGuard {
    using ECDSA for bytes32;

    bool public claimEnabled = false;
    uint256 private maxClaims = 1;
    mapping(address => uint8) private claimCount;
    mapping(bytes => bool) private usedClaimSignatures;

    constructor()
        ERC20("StreethTest", "STEETHTEST")
    ...
    function claim(
        ...
        require(
            !usedClaimSignatures[_signature],
            "You have already claimed your WRLD tokens."
        );

        require(
            claimCount[msg.sender] < maxClaims,
            "You have already claimed the maximum amount."
        );
