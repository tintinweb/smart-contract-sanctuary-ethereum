...
contract ShadowDrop is Ownable {
    ...
    // mapping from tokenAddres to signer to params (see above)
    mapping(address => mapping(address => RewardInfo)) public trustedSigner;

    // mapping from nonce to block.timestamp when it was used
    ...
    event NonceUsed(uint256 indexed nonce, uint256 indexed timestamp);
    ...
    ) external 
    {
        ...
        require(nonceUsed[_nonce] == 0, "No Double Spend");
        nonceUsed[_nonce] = block.timestamp;
        // withdraw
        ...
        emit NonceUsed(_nonce, block.timestamp);
    }
    ...
    {
        return keccak256(abi.encode(_user, _amount, _rewardToken));
    }
