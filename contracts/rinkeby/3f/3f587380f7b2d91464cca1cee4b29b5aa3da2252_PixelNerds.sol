...
contract PixelNerds is Ownable, ERC721A, ReentrancyGuard {
    ...
    mapping(address => uint256) public nerdList;
    ...
    ) ERC721A("Pixel Nerds", "PIXEL_NERDS") {
    ...
    // minting functions
    function nerdListMint(uint256 quantity) external callerIsUser {
        require(
            nerdList[msg.sender] >= quantity,
            "Not eligible for nerdList mint"
        );
        require(
            totalSupply() + quantity + amountForDevs <= maxSupply,
            "Mint over max supply"
        );
        nerdList[msg.sender] = nerdList[msg.sender] - quantity;
        _safeMint(msg.sender, quantity);
    }

    // alternative allowList mint using erc721a aux data
    function nerdListMintAux(uint64 quantity) external callerIsUser {
        require(
            _getAux(msg.sender) >= quantity,
            "Not eligible for nerdList aux mint"
        );
        require(
            totalSupply() + quantity + amountForDevs <= maxSupply,
            "Mint over max supply"
    ...
    function nerdListMintsRemaining() public view returns (uint256) {
        return nerdList[msg.sender];
    } 
}