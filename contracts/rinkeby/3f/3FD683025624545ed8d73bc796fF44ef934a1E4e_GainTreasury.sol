...
// File: interfaces/IsGAIN.sol


pragma solidity >=0.7.5;


interface IsGAIN is IERC20 {
    function rebase( uint256 GAINProfit_, uint epoch_) external returns (uint256);
...
// File: interfaces/IGAIN.sol


pragma solidity >=0.7.5;


interface IGAIN is IERC20 {
...
contract GainTreasury is GainAccessControlled, ITreasury {
    /* ========== DEPENDENCIES ========== */
    ...
    constructor(
        ...
        uint256 _timelock,
        address _authority
    ) GainAccessControlled(IGainAuthority(_authority)) {
        require(_GAIN != address(0), "Zero address: GAIN");
        GAIN = IGAIN(_GAIN);
    ...
    /**
     * @notice allow approved address to deposit an asset for GAIN
    ...
    function deposit(
        ...
        // mint GAIN needed and store amount of rewards for distribution
        send_ = value.sub(_profit);
        GAIN.mint(msg.sender, send_);
    ...
    /**
     * @notice allow approved address to burn GAIN for reserves
     * @param _amount uint256
     * @param _token address
     */
    function withdraw(uint256 _amount, address _token) external override {
        ...
        uint256 value = tokenValue(_token, _amount);
        GAIN.burnFrom(msg.sender, value);
    ...
    /**
     * @notice mint new GAIN using excess reserves
    ...
    function mint(address _recipient, uint256 _amount) external override {
        ...
        GAIN.mint(_recipient, _amount);
        emit Minted(msg.sender, _recipient, _amount);
    }

    /**
     * DEBT: The debt functions allow approved addresses to borrow treasury assets
     * or GAIN from the treasury, using sGAIN as collateral. This might allow an
     * sGAIN holder to provide GAIN liquidity without taking on the opportunity cost
    ...
    function incurDebt(uint256 _amount, address _token) external override {
        uint256 value;
        if (_token == address(GAIN)) {
            require(permissions[STATUS.GAINDEBTOR][msg.sender], notApproved);
            value = _amount;
        } else {
        ...
        sGAIN.changeDebt(value, msg.sender, true);
        require(sGAIN.debtBalances(msg.sender) <= debtLimit[msg.sender], "Treasury: exceeds limit");
        totalDebt = totalDebt.add(value);

        if (_token == address(GAIN)) {
            GAIN.mint(msg.sender, value);
            GAINDebt = GAINDebt.add(value);
        } else {
            totalReserves = totalReserves.sub(value);
            IERC20(_token).safeTransfer(msg.sender, _amount);
        }
    ...
    function repayDebtWithReserve(uint256 _amount, address _token) external override {
        ...
        IERC20(_token).safeTransferFrom(msg.sender, address(this), _amount);
        uint256 value = tokenValue(_token, _amount);
        sGAIN.changeDebt(value, msg.sender, false);
    ...
    /**
     * @notice allow approved address to repay borrowed reserves with GAIN
     * @param _amount uint256
     */
    function repayDebtWithGAIN(uint256 _amount) external {
        require(permissions[STATUS.RESERVEDEBTOR][msg.sender] || permissions[STATUS.GAINDEBTOR][msg.sender], notApproved);
        GAIN.burnFrom(msg.sender, _amount);
        sGAIN.changeDebt(_amount, msg.sender, false);
        totalDebt = totalDebt.sub(_amount);
        GAINDebt = GAINDebt.sub(_amount);
        emit RepayDebt(msg.sender, address(GAIN), _amount, _amount);
    }
    ...
    function enable(
        ...
        if (_status == STATUS.SGAIN) {
            sGAIN = IsGAIN(_address);
        } else {
    ...
    function execute(uint256 _index) external {
        ...
        if (info.managing == STATUS.SGAIN) {
            // 9
            sGAIN = IsGAIN(info.toPermit);
        } else {
    ...
    function excessReserves() public view override returns (uint256) {
        return totalReserves.sub(GAIN.totalSupply().sub(totalDebt));
    }

    /**
     * @notice returns GAIN valuation of asset
     * @param _token address
     * @param _amount uint256
     * @return value_ uint256
     */
    function tokenValue(address _token, uint256 _amount) public view override returns (uint256 value_) {
        value_ = _amount.mul(10**IERC20Metadata(address(GAIN)).decimals()).div(10**IERC20Metadata(_token).decimals());
    ...
    function baseSupply() external view override returns (uint256) {
        return GAIN.totalSupply() - GAINDebt;
    }
}