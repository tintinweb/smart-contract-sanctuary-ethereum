...
library NFTSVG {
  ...
  // updated
  function generateFixedFilterGrey() internal pure returns (bytes memory) {
    return
      bytes.concat(
        bytes.concat('<filter id="FFG">'), //
        bytes.concat('<feColorMatrix values="0.132 0.318 0.603 0 0 0.132 0.318 0.603 0 0 0.132 0.318 0.603 0 0 0 0 0 1 0"/>'),
        "</filter>"
      );
  }
  ...
  function generateTitle(uint256 _tokenId, Params.Character[][8] memory _characters, uint8[8] memory _charactersIds) internal pure returns (bytes memory) {
    ...
    return
      bytes.concat(
        ...
        "</g>",
        "<g>",
        abi.encodePacked('<text class="colB" style="font-size:24px;font-weight:Bold;font-family=Courier,Monospace;" text-anchor="middle" transform="translate(760,840)">', Strings.toString(_tokenId),'</text>'),
        "</g>",
        "</g>",
        "</g>"
      );
