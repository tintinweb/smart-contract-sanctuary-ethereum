...
contract BetterticketMusicEvent is ERC721Enumerable, Ownable, KeeperCompatibleInterface {

    uint256 public SaleStartTime;
    uint public SaleDuration; 
    bool public paused = false;

    constructor(uint256 _saleDuration) ERC721("","") {
        SaleDuration = _saleDuration;
    }


    event NewEvent(uint _eventId);
    event SaleStarted(uint256 _saleDuration, address owner);
    ...
    function setPaused(bool _state) private{
    paused = _state;
    }

    function isEventStateOpenForSale() public view returns (bool) {
        return event_state == EVENT_STATE.OPENFORSALE;
    }
    ...
    function createEvent(address _artist, bytes32 _name, bytes32 _location, uint _price, uint _time, uint _salesCap) public onlyOwner {
        ...
        events.push(_event);
        uint _eventId = events.length - 1;
        event_state = EVENT_STATE.OPENFORSALE;
        SaleStartTime = block.timestamp;
        emit SaleStarted(block.timestamp, address(this));
    ...
    function purchaseTicket(uint _eventId) public payable {
        ...
        uint _ticketId = tickets.length - 1;
    ...
    function checkUpkeep(bytes calldata) external view  returns (bool upkeepNeeded, bytes memory performData) {
        upkeepNeeded = (event_state == EVENT_STATE.OPENFORSALE) && (block.timestamp - SaleStartTime) > SaleDuration;
        return (upkeepNeeded, performData);
    }    

    function performUpkeep(bytes calldata /* performData */) external view {
        require(!paused, "The contract is paused!");
        require(event_state==EVENT_STATE.OPENFORSALE);
    /// @dev Used to purchase a ticket for a given event
        if (event_state == EVENT_STATE.OPENFORSALE && (block.timestamp - SaleStartTime) > SaleDuration ) {
            purchaseTicket;
        }
    }
    ...
    /// @param only Address of someone
    function ticketsOf(address only) public view returns(uint[] memory) {
        uint tokenCount = totalOwnedTickets[only];
        if (tokenCount == 0) {
            return new uint[](0);
        } else {
            ...
            for (uint i = 0; i < tokenCount; i++) {
                result[resultIndex] = tokenOfOwnerByIndex(only, i);
                resultIndex++;
            }
            return result;
        }
