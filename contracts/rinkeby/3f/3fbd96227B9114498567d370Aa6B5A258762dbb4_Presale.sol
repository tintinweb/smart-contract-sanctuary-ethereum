...
contract Presale{
    ...
    constructor(address _creator, address _Admin, address stakingPool, address _token, uint256 _hardcap, uint256 _softCap, uint256 _liquidity, uint256 buyunit, uint exchangeUnit, uint _AdminFee, uint _inpresaleFee){
        ...
        ADMIN = _Admin;
         hardCap = _hardcap;
        softCap = _softCap;
        liquidityPercent = _liquidity  * 100;
...
//this is only valid if the creator sets whitelist to true 
    ...
    function startPresale(uint _startTime, uint _endTime) external onlyAdmin{
        uint liqp = liquidityPercent * hardCap/10000;
  ...
  function refund() external {
    ...
    //  tokens
        uint liqp = liquidityPercent * raisedBNB/10000;
        uint tokenAmount =  ExchangeUnit * liqp;
        uint FEE = inPresaleFee * token.balanceOf(address(this))/10000;
        uint tokenLiquidityCut = tokenAmount- FEE;
        uint tokenAdminCut = 5000 * FEE / 10000;
        uint tokenStakingCut = 5000 * FEE / 10000;

        //bnb
        uint coinAdminCut = AdminFee * raisedBNB / 10000;
        raisedBNB -= coinAdminCut;
        _coinliquidityCut = liqp;
        _coinliquidityCut -= coinAdminCut;
        raisedBNB -= _coinliquidityCut;

        // _addLiquidity(tokenLiquidityCut);
