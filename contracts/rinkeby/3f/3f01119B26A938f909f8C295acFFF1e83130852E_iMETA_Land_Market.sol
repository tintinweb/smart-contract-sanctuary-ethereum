...
contract iMETA_Land_Market is iMETA_UserInfo, Initializable, UUPSUpgradeable, ReentrancyGuardUpgradeable, OwnableUpgradeable {
    ...
    event MarketItemCreated_eth ( uint indexed itemId, address indexed nftContract, uint256 indexed tokenId, address  seller, address  owner, uint256 eth_price, uint256 price, bool sold);
    event MarketItemCreated_token ( uint indexed itemId, address indexed nftContract, uint256 indexed tokenId, address  seller, address  owner, uint256 eth_price, uint256 price, bool sold);
    ...
    event event_buy_eth( uint _itemId, address owner,uint coin_price, uint token_price, bool sold);
    event event_buy_token( uint _itemId, address owner,uint coin_price, uint token_price, bool sold);
    ...
    //유저 이더리움 출금
    function receive_user_eth() public {
        require(UserCoin[msg.sender] > 0, "Price must be above zero");
		payable(msg.sender).transfer(UserCoin[msg.sender]);
        emit event_receive_user_eth(UserCoin[msg.sender], msg.sender);
		UserCoin[msg.sender] = 0;
	}
    
    //유저 토큰 출금
	function receive_user_token(address _tokenContract) public {
        require(UserToken[msg.sender] > 0, "Price must be above zero");
		IERC20Upgradeable tokenContract = IERC20Upgradeable(_tokenContract);
		tokenContract.transfer(msg.sender, UserToken[msg.sender]);
		emit event_receive_user_token(UserToken[msg.sender], msg.sender);
    ...
    function createMarketItem_eth(address _nftContract, uint256 _tokenId, uint256 _eth_price) public payable nonReentrant{
        ...
        idMarketItem[_tokenId] = MarketItem(itemId, _nftContract, _tokenId, payable(msg.sender), payable(address(this)), _eth_price,0, false);
        IERC721Upgradeable(_nftContract).transferFrom(msg.sender, address(this), _tokenId);
        marketList.push(_tokenId);
        
        emit MarketItemCreated_eth(itemId, _nftContract, _tokenId, msg.sender, msg.sender, _eth_price, 0, false);
    ...
    function createMarketItem_token(address _nftContract, uint256 _tokenId, uint256 _price) public nonReentrant{
        ...
        uint256 itemId = _itemIds.current();
        //MarketItems.push(MarketItem(itemId, _nftContract, _tokenId, payable(msg.sender), payable(address(0)), _price, true));
        idMarketItem[_tokenId] = MarketItem(itemId, _nftContract, _tokenId, payable(msg.sender), payable(address(this)), 0, _price, false);
        IERC721Upgradeable(_nftContract).transferFrom(msg.sender, address(this), _tokenId);
        marketList.push(_tokenId);

        emit MarketItemCreated_token(itemId, _nftContract, _tokenId, msg.sender, msg.sender, 0, _price, false);
    }
    ...
    function buy_with_eth(address _nftContract, uint256 _tokenId, uint256 _eth_price) public payable nonReentrant{
        ...
        set_User_Coin(idMarketItem[_tokenId].seller, test_token_price);
        
        uint keyListIndex = idMarketItem[_tokenId].itemId - 1;
        uint keyListLastIndex = marketList.length - 1;
        idMarketItem[marketList[keyListLastIndex]].itemId = keyListIndex + 1;
    ...
    function buy_with_token(address _nftContract, uint256 _tokenId, uint256 _price) public nonReentrant{
        ...
        set_User_Token(idMarketItem[_tokenId].seller, test_token_price);
    ...
    function fetchMarketItemspage(uint page) public view returns (MarketItem[] memory){
        ...
        if(_itemIds.current() < marketlist){
            itemCount = _itemIds.current() + 1;
        }
    ...
    function InfoMarketItem(uint256 _tokenId) public view returns (MarketItem memory){
        //MarketItem memory items = idMarketItem[_tokenId];
        //return items; //return array of all unsold items
...
contract iMETA_UserInfo{
	...
	function get_User_Token(address _address) public view is_User() returns (uint256){
		return UserToken[_address];
	}

    function set_User_Token(address _address, uint256 _value) public is_User() returns (uint256){
        return UserToken[_address] = UserToken[_address] + _value;
    }

    function get_User_Coin(address _address) public view is_User() returns (uint256){
		return UserCoin[_address];
	}

    function set_User_Coin(address _address, uint256 _value) public is_User() returns (uint256){
        return UserCoin[_address] = UserCoin[_address] + _value;
    }
