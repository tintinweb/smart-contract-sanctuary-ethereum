...
contract TokenSale is Ownable {
    ...
    /**
     * Emitted when an `amount` of token is sold to the `buyer`
     */
    event TokenSold(address buyer, uint256 amount);

    /**
     * Emitted when `rate` be updating
     */
    event RateChanged(uint256 rate);

    /**
     * Sets the values for {token} and {rate}.
     */
    constructor(address _tokenAddress, uint256 rate_) {
        token = IERC20(_tokenAddress);
        _rate = rate_;
    }

    /**
     * Moves `msg.value` * `rate` tokens from the this account
     ...
     * Requirements:
     *
     * - this account must have a balance of at least `msg.value` * `rate`.
     */
    ...
    function updateRate(uint256 rate_) public onlyOwner {
        require(rate_ > 0, "TokenSale: rate need to be greater than zero");
        _rate = rate_;
    }
