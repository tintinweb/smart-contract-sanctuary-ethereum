...
import {IRaid} from "../interfaces/IRaidV4.sol";
...
contract RaidV4 is IRaid, Initializable, AccessControlUpgradeable, Seedable {
    bool public started;
    bool public halted;
    ...
    function _syncRounds() internal {
        // FIXME: check if we will overflow
        unchecked {
            uint32 finalBlock = rounds[roundId].finalBlock;

            while (block.number > finalBlock) {
                roundId += 1;
                seed = _rollSeed(seed);
                Round memory nextRound = _rollRound(seed, finalBlock + 1);
                finalBlock = nextRound.finalBlock;
        ...
        returns (Snapshot memory snapshot)
    {
        ...
        while (_roundId < roundId) {
            ...
            snapshot.attackDealt =
                (1e18 * (boss.blockHealth * boss.multiplier)) /
                PRECISION;
        ...
        returns (uint32, uint256)
    {
        ...
        return (_lazyFetchRoundId(), raider.pendingRewards);
    }
        ...
        returns (uint32 _roundId, uint256 rewards)
    {
        ...
        unchecked {
            ...
            if (roundId < _roundId) {
                ...
                uint32 syncDiff = _roundId - roundId;
                for (uint32 i = roundId; i <= syncDiff; i++) {
                    _roundId += 1;
                    _seed = _rollSeed(seed);
                    round = _rollRound(_seed, round.finalBlock + 1);
                }
    ...
    function _roundReward(
        ...
        unchecked {
            ...
            } else if (round.finalBlock > block.number) {
                blocksDefeated = block.number - round.startBlock;
            }

            return
                (1e18 * blocksDefeated * boss.multiplier * raider.dpb) /
                PRECISION;
        }
    }
}
