...
contract CryptWear is ERC721, Ownable, ReentrancyGuard {
    ...
    enum MintType{PRESALE, WHITELIST_N_PUBLIC}

    struct MintSettings {
        uint256 maxPresaleMintId;
        uint256 presaleMintId;
        ...
        uint256 maxPrivateMintPerWallet;
    ...
    function setMintSettings(
        ...
        uint256 _maxPresaleMintPerWallet,
        uint256 _maxPrivateMintPerWallet,
        ...
        mintSettings.whitelistSalePrice = _whitelistSalePrice;
        mintSettings.publicSalePrice = _publicSalePrice;

        mintSettings.maxPresaleMintPerWallet = _maxPresaleMintPerWallet;
        mintSettings.maxPrivateMintPerWallet = _maxPrivateMintPerWallet;
    ...
    bool public proxyRegistryAddressWhitelistedApproved;


    constructor(string memory _baseURI, string memory collectionURI) ERC721("CryptWear.io", "CryptWear") {

        // OpenSea proxy registry addresses for rinkeby and mainnet.
        if (block.chainid == 1) {
            proxyRegistryAddress = 0xa5409ec958C83C3f309868babACA7c86DCB077c1;
            proxyRegistryAddressWhitelistedApproved = true;
        ...
        mintSettings.maxPresaleMintId = 400;
        mintSettings.presaleMintId = 1;

        mintSettings.maxId = 2000;
        mintSettings.id = 400;

        mintSettings.whitelistSalePrice = 0.0003 ether;
        mintSettings.publicSalePrice = 0.0004 ether;

        mintSettings.maxPresaleMintPerWallet = 10;
        mintSettings.maxPrivateMintPerWallet = 10;

        withdrawSettings.withdrawAddress1 = owner();
        withdrawSettings.withdrawAddress2 = owner();
    ...
    modifier canMint(uint256 numberOfTokens, MintType mintType) {
        if (mintType == MintType.PRESALE)
            require(
                mintSettings.presaleMintId + numberOfTokens <= mintSettings.maxPresaleMintId,
                "Not enough tokens remaining to mint"
            );
        if (mintType == MintType.WHITELIST_N_PUBLIC)
            require(
                mintSettings.id + numberOfTokens <= mintSettings.maxId,
                "Not enough tokens remaining to mint"
            );
        _;
    }

    function mintPresale(address to, uint256 amount, bytes32[] calldata proof) external canMint(amount, MintType.PRESALE) nonReentrant {
        // Throw if address has already claimed tokens
        require(!claimedPresale[msg.sender], "NFT is already claimed by this wallet");

        // Verify merkle proof, or revert if not in tree
        bytes32 leaf = keccak256(abi.encodePacked(to, amount));
        bool isValidLeaf = MerkleProof.verify(proof, presaleMerkleRoot, leaf);
        require(isValidLeaf, "not part of Merkle tree");

        claimedPresale[to] = true;
        for (uint256 i = 0; i < amount; i++) {
            _mint(to, mintSettings.id++);
        }
    }
    ...
    */
    ...
    canMint(amount, MintType.WHITELIST_N_PUBLIC)
    nonReentrant
    {
    ...
    */
    ...
    function isApprovedForAll(
        ...
        if (proxyRegistryAddressWhitelistedApproved && _operator == proxyRegistryAddress) {
            return true;
        }
    ...
    */
    ...
    function setProxyRegistryAddressWhitelistedApproved(bool _proxyRegistryAddressWhitelistedApproved) public onlyOwner {
        proxyRegistryAddressWhitelistedApproved = _proxyRegistryAddressWhitelistedApproved;
    }
