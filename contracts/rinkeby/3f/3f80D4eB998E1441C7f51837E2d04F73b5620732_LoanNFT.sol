...
import {ILoanNFT, LoanMeta} from "./ILoanNFT.sol";
import {AddressResolver} from "../AddressResolver.sol";
import {ILendingPoolRegistry} from "../lendingPool/ILendingPoolRegistry.sol";

contract LoanNFT is ERC721URIStorage, ILoanNFT, AddressResolver {
  using Counters for Counters.Counter;

  Counters.Counter private nextLoanID;
  LoanMeta[] public loans;

  event LoanCreated(address indexed _lendingPoolAddress, uint256 indexed _loanID, string _purpose, string _description);

  constructor(address _registry) ERC721("Loan NFT", "COPNFT") AddressResolver(_registry) {}

  modifier onlyWhitelistedPool() {
    address lpRegistryAddr = _resolve("LendingPoolRegistry");
    ILendingPoolRegistry lpRegistry = ILendingPoolRegistry(lpRegistryAddr);
    require(lpRegistry.isWhitelisted(msg.sender), "not whitelisted");
    _;
  }
  ...
  function mint(
    ...
    string memory _purpose,
    string memory _description
  ) external override onlyWhitelistedPool returns (uint256) {
    uint256 newloanID = nextLoanID.current();
    _mint(msg.sender, newloanID);
    _setTokenURI(newloanID, _purpose);
    loans.push(LoanMeta({pool: msg.sender, loanID: _loanID}));
    emit LoanCreated(msg.sender, _loanID, _purpose, _description);
    nextLoanID.increment();
    return newloanID;
  }

  function getLoan(uint256 _loanID) external view override returns (LoanMeta memory) {
