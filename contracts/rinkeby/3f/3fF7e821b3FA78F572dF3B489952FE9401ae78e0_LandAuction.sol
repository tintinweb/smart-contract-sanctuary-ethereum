...
contract LandAuction is ILandAuction, AccessControl, ReentrancyGuard {
    ...
    uint16 public constant M = 199; // yHigh + 99 + 1

    /*
        xLow, yLow gets mapped to 1,1
        transform: +97, +99
    */

    int16 public constant xLow = -96;
    int16 public constant yLow = -98;
    ...
    event BidCreated(address user, int16 x, int16 y, uint256 price);
    event LandBought(
        address user,
        int16 x,
        int16 y,
    ...
    function getReservePrice(int16 x, int16 y) public view returns (uint256) {
        int8 category = getCategory(x, y);
        require(category != 0, "ERR_NOT_UP_FOR_SALE");
        return priceOfCategory[category];
    }
    ...
    function isShiboshiZone(int16 x, int16 y) public pure returns (bool) {
        ...
        */
        ...
        if (x >= 12 && x <= 48 && y >= -98 && y <= -64) {
            return true;
        }
        if (x >= 49 && x <= 77 && y >= -98 && y <= -77) {
            return true;
        }
        if (x >= 76 && x <= 77 && y >= -76 && y <= -49) {
            return true;
        }
        if (x >= 65 && x <= 75 && y == -49) {
            return true;
    ...
    function bidShiboshiZone(
        int16 x,
        int16 y,
        bytes calldata signature
    ) external payable onlyStage(Stage.Bidding) {
        require(
        ...
        onlyStage(Stage.PrivateSale)
    {
        ...
        emit LandBought(msg.sender, x, y, msg.value, Stage.PrivateSale);
    }
    ...
    function mintPrivateShiboshiZone(
        ...
        int16 y,
        bytes calldata signature
    ) external payable onlyStage(Stage.PrivateSale) {
        require(
            _verifySigner(_hashMessage(msg.sender), signature),
            "ERR_SIGNATURE_INVALID"
        );
    ...
    {
        _mintPublicOrPrivate(msg.sender, x, y);
        emit LandBought(msg.sender, x, y, msg.value, Stage.PublicSale);
    }
    ...
    {
        return (uint16(x + 97), uint16(y + 99));
    }
    ...
    function _bid(
        ...
        int16 x,
        int16 y
    ) internal onlyValid(x, y) nonReentrant {
        ...
        if (currentBid.amount == 0) {
            ...
            require(
                msg.value >= getOutbidPrice(currentBid.amount),
                "ERR_INSUFFICIENT_AMOUNT_SENT"
            );
        ...
        emit BidCreated(user, x, y, msg.value);
    }
