...
contract KillerKoalas is ERC721A, Ownable {
   ...
   uint256 publicIssued;
   uint256 MAX_MINT = 100; // Total Number of NFTs
   uint256 _price = 33000000000000000; //Price of NFTs 0.033 ETH
   // // metadata URI
  string private _baseTokenURI;

   constructor(string memory baseURI) ERC721A("KillerKoalas", "KK") {
   }
  ...
  function reserveNFTs(uint256 numTokens) external {
   //   require(SafeMath.add(_tokenIds.current(), 30) <= MAX_MINT , "Exceeds maximum token supply.");
      require(totalSupply() + numTokens <= MAX_MINT, "reached max supply");
      _safeMint(msg.sender, numTokens);
   }

   function updateSaleState(bool newState) external  {
      activeSale = newState;
    }
   ...
   function mintNFT(uint256 numTokens) external payable{
    ...
    //   require(SafeMath.add(_tokenIds.current(), numTokens) <= MAX_MINT , "Exceeds maximum token supply.");
  ...
  //    function burnNFTS(uint256 burnNumTokens) public {
//       for (uint i = 0; i < burnNumTokens; i++) {
//          _tokenIds.increment();
...
// //NUMBER MINTED??? UNSURE ON WHAT FUNCTION DOES-----------------------------------
// function numberMinted(address owner) public view returns (uint256) {
//     return _numberMinted(owner);
//   }

//OWNERSHIP DATA??? IS THIS SO WE CAN QUERY WHO OWNS WHAT??? ---------------------------
//  function getOwnershipData(uint256 tokenId)
//     external
//     view
//     returns (TokenOwnership memory)
