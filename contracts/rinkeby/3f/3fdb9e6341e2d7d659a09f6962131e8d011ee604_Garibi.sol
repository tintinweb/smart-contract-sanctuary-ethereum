...
contract Garibi is ERC721A, Ownable {
    enum Status {
        Waiting,
        Started,
        Finished,
    ...
    uint256 public constant MAX_RESERVE_AMOUNT = 2000;
    uint256 public constant MAX_PUBLIC_AMOUNT = 100;
    uint256 public constant MAX_ALLOWLIST_AMOUNT = 20;
    ...
    constructor(string memory initBaseURI) ERC721A("Garibi", "GAB") {
        baseURI = initBaseURI;
    }
    ...
    function mint(uint256 quantity) external {
        require(status == Status.Started, "Garibi: Public mint is not active.");
        require(tx.origin == msg.sender, "Garibi: ?? Not you!");
        require(
            numberMinted(msg.sender) + quantity <= MAX_PUBLIC_AMOUNT,
            "Garibi: Max public mint amount per wallet exceeded"
        );
        require(
            totalSupply() + quantity <= MAX_SUPPLY,
            "Garibi: Exceed max supply."
        );
        ...
        external
    {
        require(
            ...
            "Garibi: Allowlist mint is not active or passed."
        );
        require(tx.origin == msg.sender, "Garibi: ?? Not you!");
        require(
            allowlist[msg.sender] + quantity <= MAX_ALLOWLIST_AMOUNT,
            "Garibi: Max allowlist mint amount per wallet exceeded."
        );
        require(
            totalSupply() + quantity <= MAX_SUPPLY,
            "Garibi: Exceed max supply."
