...
contract FourHundredDrums is ERC721A, Ownable {
    ...
    mapping(address => uint256) private _devWallets;
    ...
    function devMint(bytes32[] calldata _merkleProof, uint256 _quantity) external payable {
      ...
      require(_devWallets[msg.sender] + _quantity <= reserveSupply, "You reached max per wallet.");
      require(maxSupply >= _tokenIds.current() + _quantity, "Sold out or Exceeds max tokens");

      for (uint256 i = 0; i < _quantity; i++) {
        _devWallets[msg.sender]++;
        _tokenIds.increment();
      }
    ...
    function withdrawSplit() public payable onlyOwner {
      ...
      (bool wallet2, ) = payable(msg.sender).call{value: address(this).balance}(""); // replace the msg.sender with 400drums wallet
      require(wallet1, "Withdraw 1 failed");
      require(wallet2, "Withdraw 2 failed");
    }

    function withdraw() public payable onlyOwner {
      // use this withdraw as an emergency
