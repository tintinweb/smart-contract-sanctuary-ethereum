...
// File miscsolidity/contracts/access/[email protected]
...
// File miscsolidity/contracts/token/ERC721/extensions/[email protected]
...
// File contracts/Apartments/IApartments.sol
...
interface IApartments {
    ...
    function APARTMENT_MINTER_ROLE() external view returns(bytes32);
    function APARTMENT_BURNER_ROLE() external view returns(bytes32);

    function mint(address to, ApartmentType apartmentType) external returns(uint tokenId);
...
// File contracts/Omnichain/token/onft/IONFT721Core.sol
...
// File contracts/Omnichain/token/onft/IONFT721.sol
...
// File contracts/Omnichain/interfaces/ILayerZeroReceiver.sol
...
// File contracts/Omnichain/interfaces/ILayerZeroUserApplicationConfig.sol
...
// File contracts/Omnichain/interfaces/ILayerZeroEndpoint.sol
...
// File contracts/Omnichain/lzApp/LzApp.sol
...
// File contracts/Omnichain/lzApp/NonblockingLzApp.sol
...
// File contracts/Omnichain/token/onft/ONFT721Core.sol
...
// File contracts/Omnichain/token/onft/ONFT721.sol
...
// File contracts/Apartments/Apartments.sol
...
contract Apartments is OwnableAccessControl, IApartments, ONFT721 {

    // keeps track of the ID based on apartment type.
    mapping (ApartmentType => uint256) public countersMapping;

    // tracks how many of each type have been minted.
    mapping (ApartmentType => uint256) public mintedPerType;

    // enforces the max amount that can be minted for each type of apartment.
    mapping (ApartmentType => uint256) public maxPerType;

    // roles
    bytes32 constant public override APARTMENT_MINTER_ROLE = keccak256("APARTMENT_MINTER_ROLE");
    bytes32 constant public override APARTMENT_BURNER_ROLE = keccak256("APARTMENT_BURNER_ROLE");
    string private _URI;

    uint256 public maxSupply = 9500;

    constructor(string memory name, string memory symbol, string memory uri, address endpoint) 
    ONFT721(name, symbol, endpoint) {
        setURI(uri);
        _initializeTokenIds();
    }

    function _initializeTokenIds() internal {
        countersMapping[ApartmentType.small] = 0;
        countersMapping[ApartmentType.medium] = 4500;
        countersMapping[ApartmentType.large] = 7500;
        countersMapping[ApartmentType.penthouse] = 9000;

        maxPerType[ApartmentType.small] = 4500;
        maxPerType[ApartmentType.medium] = 3000;
        maxPerType[ApartmentType.large] = 1500;
        maxPerType[ApartmentType.penthouse] = 500;
    }

    function mint(address to, ApartmentType apartmentType) public onlyRole(APARTMENT_MINTER_ROLE) returns(uint tokenId) {
        require(totalSupply() < maxSupply, "Max supply reached");
        require(mintedPerType[apartmentType] < maxPerType[apartmentType], "Max per type");
        //todo: uncomment line below so minting only happens on mainnnet (or ETH rinkeby for)
        // require(block.chainid == 1 || block.chainid == 4, "Not ETH chain");
        mintedPerType[apartmentType] += 1;
        countersMapping[apartmentType] += 1;
        tokenId = countersMapping[apartmentType];
        _mint(to, tokenId);
    }

    function burn(uint tokenId) public override onlyRole(APARTMENT_BURNER_ROLE) {
        _burn(tokenId);
    }

    function setURI(string memory uri) public override onlyOwner {
        _URI = uri;
    }
    ...
    function supportsInterface(bytes4 interfaceId) public override(AccessControl, ONFT721) view returns(bool) {
        return super.supportsInterface(interfaceId);
    }
}


// File contracts/NFTMint/NFTMinter.sol
...
/// @title NFTMinter
/// @notice NFT Minting contract with configurable sale periods.
contract NFTMinter is Ownable {
    ...
    event SaleCreated(address indexed by, uint256 start, uint256 end);

    using MerkleProof for bytes32[];

    /// @notice Defines the parameters for a sale period.
    /// @dev A sale period has a start time, end time, mint fee and max mint.
    /// This is activated by the contrat owner.
    /// A sale is active when start < block.timestamp < end.
    struct Sale {
        uint256 start;
        uint256 end;
        uint256[] mintFees;
    ...
    /// @notice the NFT being minted.
    IApartments public nft;

    /// @notice Mapping of the quantity of NFTs minted to each address.
    /// Used to track and cap how many tokens an address is allowed to mint per round.
    /// round start => address => number minted
    mapping(uint256 => mapping(address => mapping(ApartmentType => uint256)))
        public minted;

    /// @notice The mint fee recipient.
    address payable public feeRecipient;

    /// @notice The list of sale periods in this launch.
    Sale[] private _sales;

    /// The merkle tree root hash. Used to verify if an address is whitelisted.
    bytes32 public merkleRoot =
        0xd0fa3be45b05cd6f3107abbcd8140c68eb7e392b8c4af7b39dae718495bd700f;

    constructor(address _nft, address _feeRecipient) {
        nft = IApartments(_nft);
        feeRecipient = payable(_feeRecipient);
    }

    /// @notice Sets the merkle tree root hash used to verify belonging to the merkle tree.
    /// @dev Can only be called by contract owner.
    /// @param _root The merklel tree root hash.
    ...
    /// @param start The start of the sale.
    /// @param end The start of the sale.
    /// @param fees The mint fees during this round.
    /// fees[0] = fee for small apartment
    /// fees[1] = fee for medium apartment
    /// fees[2] = fee for large apartment
    /// fees[3] = fee for medium apartment
    /// @param limits The maximum of NFTs an account can mint during this period.
    /// @param whitelistRequired True if the minter would have be be on the whitelist to mint during this period.
    function addSale(
        uint256 start,
        uint256 end,
        ...
        _sales.push(sale);
        emit SaleCreated(msg.sender, start, end);
    }

    /// @notice Whitelisted sale mint function
    /// @param user The address to mint to
    /// @param quantity The quantity of NFTs to mint
    /// @param proof The merkle proof that the `user` parameter is found in the merkle tree.
    function whitelistMint(
        ...
        Sale memory current = getCurrentSale();
        require(current.start != 0, "Minter !In progress");
        bytes32 leaf = keccak256(abi.encodePacked(user));
        require(_verify(proof, leaf), "Minter: !Whitelisted");
        _mintTo(user, apartmentType, quantity, current);
    }

    /// @notice Public sale mint function
    /// @dev Can only be called during a sale period that does not requrire being whitelisted.
    /// @param user The address to mint to
    /// @param quantity The quantity of NFTs to mint
    function publicMint(
        address user, 
        ...
        Sale memory current = getCurrentSale();
        require(current.start != 0, "Minter !In progress");
        require(!current.whitelistRequired, "Minter: Whitelist required");

        _mintTo(user, apartmentType, quantity, current);
    }


    /// @notice internal mint funciton.
    function _mintTo(
        address user,
        ApartmentType apartmentType,
        uint256 quantity,
        ...
        uint8 typeAsInt = uint8(apartmentType);
        require(
            apartmentType < ApartmentType.penthouse,
            "Minter: Invalid Type");
        require(
            msg.value == current.mintFees[typeAsInt] * quantity,
            "Minter: Wrong fee"
        );
        require(
            minted[current.start][user][apartmentType] + quantity <= current.limits[typeAsInt],
            "Minter: Too many"
        );
        minted[current.start][user][apartmentType] += quantity;
        feeRecipient.transfer(msg.value);
        for (uint256 i = 0; i < quantity; i++) {
            nft.mint(user, apartmentType);
        }

        emit Minted(user, apartmentType, quantity);
    }

    /// @notice get the details of all sales.
    function allSales() external view returns (Sale[] memory sales) {
        sales = _sales;
    }

    /// @notice Returns the current sale period if any. Returns a zero filled struct if not within any sale period.
    function getCurrentSale() public view returns (Sale memory current) {
        for (uint256 i = 0; i < _sales.length; i++) {
            if (
                _sales[i].start < block.timestamp &&
                _sales[i].end > block.timestamp
            ) {
                current = _sales[i];
            }
        }
    ...
    function _verify(bytes32[] calldata proof, bytes32 leaf)
        internal
        view
        returns (bool verified)
    {
        verified = proof.verify(merkleRoot, leaf);
    }
}