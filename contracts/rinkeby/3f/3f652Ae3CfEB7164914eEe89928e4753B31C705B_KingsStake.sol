...
contract KingsStake is Ownable, ReentrancyGuard {
    ...
    mapping (address => uint256[]) public userUnstakeIdLists;
    ...
    function getUserAllUnstakeIds() public view returns(uint256[] memory){
        return (userUnstakeIdLists[msg.sender]);
    }
    ...
    function updateStakeArray(uint256 _stakeId) internal {
        ...
        for(uint i = 0; i < userStakesArray.length; i++){
            userStake memory userStakeFromArrayDetails = userStakesArray[0];
