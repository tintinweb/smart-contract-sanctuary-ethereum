    ...
    ITokenCreator
{
    ...
    function tokenCreator(uint256 tokenId) external view returns (address payable) {
        return payable(_tokenCreators[tokenId]);
    }
    ...
    function setCreatorFromRoyaltyReceiver(uint256 tokenID) external {
        address receiver = getRoyaltyReceiverByTokenID(tokenID);
        require(receiver != address(0), "receiver must not be 0.");
        _setCreatorForTokenID(tokenID, receiver);
    }

    function _setCreatorForTokenID(uint256 tokenID, address creator) internal {
        emit SetCreator(
            creator,
            tokenID
        );
    ...
    function mintAndTransferVoucher(
        ...
        _setCreatorForTokenID(voucher.tokenID, voucher.creator);
    ...
    mapping(uint256 => address) internal _tokenCreators;
}
    ...
    ContextUpgradeable
{
    ...
    function getRoyaltyReceiverByTokenID(uint256 tokenID)
        public
        view
        returns (address receiver)
    {
        return _royalties[tokenID].receiver;
    }
