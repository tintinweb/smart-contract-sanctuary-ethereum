        ...
        uint256[] calldata _tokens) public onlyRole(DEFAULT_ADMIN_ROLE)
    {
     ...
     function getQualifiedAllocation(address sender, 
        ...
        if(!returnAllocationOnly){
            require(token.mintingConfig.saleIsOpen, "5");
            require(!paused(), "6");
            require(token.mintingConfig.windowOpens > 0, "7");
            require (block.timestamp > token.mintingConfig.windowOpens && block.timestamp < token.mintingConfig.windowCloses, "v4");
            require(token.claimedTokens[sender].add(quantity) <= amount, "8");
            require(token.claimedTokens[sender].add(quantity) <= token.mintingConfig.maxPerWallet, "9");
            require(quantity <= token.mintingConfig.maxMintPerTxn, "10");
            require(getTokenSupply(tokenIndex) + quantity <= token.mintingConfig.maxSupply, "11");
        }
