...
contract lok666 is ERC721A, Ownable, ReentrancyGuard {
    ...
    modifier mintMaxPerWallet(uint256 amount){
        require(numberMinted(msg.sender) + amount <= maxPerWallet,"Exceeded max per wallet");
        _;
    }

    modifier mintMaxPerTxn(uint256 amount) {
    ...
    function mint(uint256 amount) external payable saleActive mintMaxPerTxn(amount) mintMaxPerWallet(amount) correctPrice(amount)
    {
        uint cost = price;
