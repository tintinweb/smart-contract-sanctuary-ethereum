...
contract Filipinoadz is ERC721A, Ownable, ReentrancyGuard {
    ...
    // mapping: minting stage => walletAddress => amountMinted
    ...
    /// Events:
    event Mint(address indexed to, uint256 amount, uint256 stage);
    event AirdropMint(address[] addresses, uint256 amount);
    event SetSaleState(bool isPublicSale, bool isWhitelistSale);
    event SetMerkleRoot(bytes32 root, uint256 stage);
        ...
        nonReentrant
    {
        ...
        require(minted[mintingStage][msg.sender] + _mintAmount <= allocation, "Mint limit exceeded for user");
    ...
    function setMerkleRoot(bytes32 _root) external onlyOwner {
        ...
        emit SetMerkleRoot(merkleRoot, mintingStage);
    }
