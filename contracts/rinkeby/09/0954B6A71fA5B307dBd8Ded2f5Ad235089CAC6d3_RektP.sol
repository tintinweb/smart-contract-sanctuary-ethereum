...
contract RektP is ERC721A, Ownable {
    ...
    constructor(
        ...
        require(rektContract != address(0));
        _rektContract = ERC721A(rektContract);
        _maxSupply = maxSupply;
    }

    function holderClaim(uint256[] memory tokenIds) external {
        require(stage == true, "Claiming not started yet");
        require(tokenIds.length > 0 && tokenIds.length % 2 == 0, "You must provide token pairs");
        uint32 pairs = uint32(tokenIds.length / 2);
        uint256 supply = totalSupply();
        require(uint256(pairs) + supply <= _maxSupply, "Exceed holder supply");

        for (uint256 i = 0; i < tokenIds.length; ) {
            _rektContract.transferFrom(msg.sender, BLACKHOLE, tokenIds[i]);
            unchecked {
                i++;
            }
        }
