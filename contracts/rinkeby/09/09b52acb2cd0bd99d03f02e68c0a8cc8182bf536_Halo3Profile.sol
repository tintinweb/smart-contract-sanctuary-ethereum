...
contract Halo3Profile is ERC721Enumerable, ReentrancyGuard, Ownable {
    ...
    function getGamertag(uint256 tokenId) public view returns (string memory) {
        return string(abi.encodePacked(gamertags[tokenId]));
    }    
    ...
    function getMap(uint256 tokenId) public view returns (string memory) {
        return pluck(tokenId, "Map", map);
    }

    function getGametype(uint256 tokenId) public view returns (string memory) {
        return pluck(tokenId, "Gametype", gametype);
    }
    ...
    function getMedal(uint256 tokenId) public view returns (string memory) {
        return pluck(tokenId, "Medal", medal);
    }
    ...
    function mint(string memory _gamertag) public nonReentrant {
        require(_tokenId <= 100, "Collection is Sold Out");
        require(_paused == false || msg.sender == owner(), "Minting is Paused");
        gamertags[_tokenId] = _gamertag;
    ...
    constructor() ERC721("Halo 3 Profile", "H3Profile") Ownable() {}
}
