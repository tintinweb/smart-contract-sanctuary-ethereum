...
contract Posts is Accounts, IERC721{
    ...
    function createPost(
        ...
        postDetails[postId.current()] = PostDetails(_title,_description,_contentUrl,_contentType,_tradeStatus,_baseBid,_lastBid,address(0),getAccountId(msg.sender));
    }
...
contract Accounts is Verify , Ownable{
    ...
    function createAccount(string memory _username , string memory _displayName , string memory _bio , string memory _profilePic , bytes memory _signature , bytes32 _signedMessageHash) external isNotAccount(msg.sender) infoIsVerified(owner(),_signature,_signedMessageHash) {
...
contract Verify{
    ...
    modifier infoIsVerified(address _signer, bytes memory _signature, bytes32 _signedMessageHash) {
        require(verify(_signer,_signature,_signedMessageHash),"Information is not verified.");
        _;
    }

    function verify (address _signer, bytes memory _signature, bytes32 _signedMessageHash) internal pure returns (bool){
