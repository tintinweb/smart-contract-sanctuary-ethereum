...
contract ArtisantVerseOrigin is OwnableUpgradeable, ERC721AUpgradeable, ReentrancyGuardUpgradeable {
	...
	function weakRandom() public view returns (uint8) {
		uint b = block.number;
		uint timestamp = block.timestamp;
        uint left = available().length;
    ...
    function available() internal view returns (uint256[] memory)
    {
        return tokensOf(address(this));
    }
    ...
    function tokensOf(address owner) public view returns (uint256[] memory)
    {
        ...
        uint256 id = 0;
        for(uint256 tokenId = _startTokenId(); tokenId <= collectionSize; tokenId++) {
    ...
    callerIsUser
    {
        uint256 publicPrice = uint256(priceWei);

        require(available().length >= quantity, "reached max supply");
        for(uint256 tbMinted=quantity; tbMinted > 0; tbMinted--) {
            uint256 tokenId = available()[weakRandom()];
