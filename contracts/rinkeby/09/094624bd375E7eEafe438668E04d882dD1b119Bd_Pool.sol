...
contract Pool is IPool {
    ...
    modifier isValidAmount(uint256 amount) {
        if(amount == 0) revert Pool__Null_Amount();
        _;
    }
    ...
    function sell(uint256 amount) external override isValidAmount(amount) returns (uint256 sent) {
    ...
    function buy(uint256 amount) external override isValidAmount(amount) returns (uint256 sent) {
