...
contract Blabla is ERC1155Burnable, ERC1155Supply, Ownable {
  struct TokenInfo {
    bool tradingPaused;
    bool isPublic;
    uint32 maxPerTxn;
  ...
  string public hashKey = "oshi-pellar";
  ...
  function setupToken(uint64 _tokenId, TokenInfo calldata _info) external onlyOwner {
    ...
    tokens[_tokenId].isPublic = _info.isPublic;
  ...
  // verified
  function setupMaxPerTxn(uint64[] calldata _tokenIds, uint32[] memory _maxPerTxn) external onlyOwner {
