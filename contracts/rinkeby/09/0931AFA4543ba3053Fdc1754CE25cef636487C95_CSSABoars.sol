...
contract CSSABoars is ERC721Enumerable, ERC2981, PaymentSplitter, Ownable {
  ...
  constructor(
    ...
    PaymentSplitter(payees, shares)
    {
      ...
      // Defender
      families.push(Family({
        presalePrice: 50_000_000 gwei,
        publicPrice: 30_000_000 gwei,
        airdropSupply: 22,
        freemintSupply: 22,
        maxSupply: 2500,
        currentIndex: 0,
  ...
  function mint(uint16[] memory _families, uint16[] memory _count) external payable {
    ...
    // TODO : vérifier la taille des tableaux
    // TODO : Vérifier le prix
    // TODO : intégrer le discount
  ...
  function _batchMint(address to,uint16[] memory family, uint16[] memory count) private {
    for (uint i = 0; i < count.length; i++) {
      //require(totalSupply() + count[i] <= MAXCOLLECTIONSIZE, "Collection is sold out");
      // TODO : check family max size
      // TODO : check family[i] is valid (0 < f < 6)
      Family storage currentFamily = families[family[i]-1];
      for(uint j = 0; j < count[i]; j++) {
        currentFamily.currentIndex++; // Ecrire une seule fois par step de boucle au lieu de à chaque fois
        _safeMint(to, currentFamily.offset + currentFamily.currentIndex);
      }
    }
  }
