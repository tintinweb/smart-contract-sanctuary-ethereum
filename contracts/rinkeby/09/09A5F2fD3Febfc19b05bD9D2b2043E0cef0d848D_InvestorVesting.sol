...
contract InvestorVesting is Ownable {
        ...
        returns (uint256 availablePercentage)
    {
        ...
        uint256 initialCliff = BokkyPooBahsDateTimeLibrary.addYears(
            _initialTimestamp,
            1
        );
        uint256 vestingDuration = BokkyPooBahsDateTimeLibrary.addYears(
            initialCliff,
            4
        );
        uint256 noOfRemainingMonths = 48;

        uint256 everyMonthReleasePercentage = remainingDistroPercentage
            .mul(1e18)
            .div(noOfRemainingMonths);
        ...
        if (currentTimeStamp > _initialTimestamp) {
            if (currentTimeStamp <= initialCliff) {
                ...
                uint256 noOfSeconds = BokkyPooBahsDateTimeLibrary.diffMonths(
                    initialCliff,
                    currentTimeStamp
                );

                return noOfSeconds.mul(everyMonthReleasePercentage);
            } else {
