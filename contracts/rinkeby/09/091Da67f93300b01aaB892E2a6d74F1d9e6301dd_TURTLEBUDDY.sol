...
contract TURTLEBUDDY is ERC721A, Ownable, ReentrancyGuard {
  ...
  function whitelistMint(uint256 _mintAmount, bytes32[] calldata _merkleProof, uint256 _value) external payable {
    ...
    require(totalSupply() + _mintAmount <= currentMaxSupply, 'Max supply exceeded!');
    uint free = _numberMinted(_msgSender()) > 0 ? 0 : 1;
    require(_value >= cost * (_mintAmount - free), "PAYMENT: invalid value");
