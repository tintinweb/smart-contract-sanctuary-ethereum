...
contract MacroVesting is Ownable {
    ...
    mapping(bytes32 => bool) public kHashTerms;
    mapping(address => UserData) public userDatas;
    mapping(address => uint32) public stopDates;
    ...
    /// @notice Contributers need to accept their allocation before they can claim
    /// @param userData the caller's data in the leaf node of the merkle tree
    function acceptAllocation(UserData calldata userData, bytes32[] memory proof) 
        external 
        notPermanentlyPaused 
    {

        require(_verify(_leaf(userData.allocation, userData.vestingSchedule, userData.cliff,
            userData.vestingCadence, userData.startDate, 
            msg.sender, userData.exempt), proof), "DATA_INVALID");

        bytes32 terms = keccak256(abi.encodePacked(userData.allocation, userData.vestingSchedule, userData.cliff,
            userData.vestingCadence, userData.startDate, msg.sender, userData.exempt));

        require(!kHashTerms[terms], "ALREADY_ACCEPTED");
        kHashTerms[terms] = true;

        userDatas[msg.sender].allocation = userData.allocation;
        userDatas[msg.sender].vestingSchedule = userData.vestingSchedule;
        userDatas[msg.sender].cliff = userData.cliff;
        userDatas[msg.sender].vestingCadence = userData.vestingCadence;
        userDatas[msg.sender].startDate = userData.startDate;
        userDatas[msg.sender].exempt = userData.exempt;

        macroCoin.transferFrom(treasury, address(this), userData.allocation);
    }

    /// @notice Owner can set a stop date for a user
    /// to the time of calling the function
    /// @param user the contributor the owner is setting the stop date for
    function setStopDate(address user) 
        external 
        onlyOwner 
        notPermanentlyPaused 
    {
        require(userDatas[user].allocation > 0, "NEVER_ACCEPTED");
        require(!userDatas[user].exempt, "EXEMPT");
        require(stopDates[user] == 0, "STOP_DATE_ALREADY_SET");
        require((block.timestamp - userDatas[user].startDate) <= userDatas[user].vestingSchedule, 
            "VESTING_COMPLETE");
        stopDates[user] = uint32(block.timestamp);

        uint256 totalTime = _timePassed(stopDates[user], userDatas[user].startDate);
        uint256 remaining = _claimable(totalTime, userDatas[user].allocation, coinsClaimed[user], user);

        macroCoin.transfer(treasury, userDatas[user].allocation - (coinsClaimed[user] + remaining));
    }
    ...
    /// @notice Contributors claim their macro coins that are 
    ...
    {
        uint256 amountToClaim = _claimableTokens();
        require(amountToClaim > 0, "NO_TOKENS_TO_CLAIM");
        coinsClaimed[msg.sender] += amountToClaim;
    ...
    function remainingVestedTokens() external view returns (uint256) 
    {
        ...
        */
        if (stopDates[msg.sender] == 0) {
            return uint256(userDatas[msg.sender].allocation) - coinsClaimed[msg.sender];
        } else if (stopDates[msg.sender] < block.timestamp) {
            return _claimableTokens();
        } else {
            uint256 totalTime = _timePassed(stopDates[msg.sender], userDatas[msg.sender].startDate);
            return _claimable(totalTime, userDatas[msg.sender].allocation, coinsClaimed[msg.sender], msg.sender);
        }  
    }
    ...
    function nextClaimDate() public view returns (uint256) 
    {
        ...
        // next claim date
        if (coinsClaimed[msg.sender] == userDatas[msg.sender].allocation) {
            return 0;
        }

        uint256 claimDate;
        if (lastClaimed[msg.sender] == 0) {
            claimDate = userDatas[msg.sender].startDate + userDatas[msg.sender].cliff;
        } else {
            // Find how many cadences they have claimed so far 
            uint256 cadencesClaimed = (lastClaimed[msg.sender] - userDatas[msg.sender].startDate) 
                / userDatas[msg.sender].vestingCadence;

            // Find their next claim date
            claimDate = userDatas[msg.sender].startDate + (cadencesClaimed * userDatas[msg.sender].vestingCadence) 
                + userDatas[msg.sender].vestingCadence;
        }

        // If their next claim date is past their stop date, they cannot claim anymore coins
        if ((stopDates[msg.sender] != 0) && (claimDate > stopDates[msg.sender])) {
            return 0;
        }
    ...
    /// @notice Returns the amount of macro coins that a whitelisted address can 
    /// claim at the current time
    function _claimableTokens() private view returns (uint256) {

        // Can only claim coins up to their stop date or end of vesting schedule
        uint256 timePassed;
        if (stopDates[msg.sender] == 0 || stopDates[msg.sender] > block.timestamp) {  
            timePassed = _timePassed(block.timestamp, userDatas[msg.sender].startDate);
        } else {
            timePassed = _timePassed(stopDates[msg.sender], userDatas[msg.sender].startDate);
        }

        /* If the time hasnt passed their cliff since their start date,
           they have 0 claimable coins at the current moment */
      
        if (timePassed < userDatas[msg.sender].cliff) {                                
            return 0;
        }

        if (timePassed < userDatas[msg.sender].vestingSchedule) {
            return _claimable(timePassed, userDatas[msg.sender].allocation, coinsClaimed[msg.sender], msg.sender);
        } else {
            return userDatas[msg.sender].allocation - coinsClaimed[msg.sender];
        }
        ...
        private view returns (uint256) 
    {
        ...
        uint256 cadencesPassed = time / userDatas[account].vestingCadence;
        // Every cadence, a contributor can claim cadence / vesting schedule of their total allocation 
        uint256 totalClaimable = allocation * cadencesPassed * 
            userDatas[account].vestingCadence / userDatas[account].vestingSchedule; 
    ...
    /// @notice Verifies that the leaf node created from the contributor's data is 
...
contract MacroCoin is ERC20, Pausable, AccessControl {
    ...
    function setNewMacroCoin(address newCoin) external onlyRole(DEFAULT_ADMIN_ROLE) {
        require(permanent && paused(), "MUST_BE_PERMANENTLY_PAUSED");
