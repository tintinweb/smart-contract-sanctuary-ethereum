...
import "./PaymentSplitterCloneFactory.sol";
import "@1001-digital/erc721-extensions/contracts/RandomlyAssigned.sol";

contract NFThing is ERC721Enumerable, Ownable, RandomlyAssigned{
    // =============================================================================
    ...
    PaymentSplitterCloneFactory pscf;
    ...
    constructor(
        ...
        address _paymentSplitterAddr
    )
    RandomlyAssigned(48, 1)
    ERC721(_name, _symbol)
    {
        setUriPrefix(_initBaseURI);
        payments = _payments; // address of the payment splitter created
        pscf = PaymentSplitterCloneFactory(_paymentSplitterAddr); 
    }
        ...
        mintCompliance(_mintAmount)
    {
        ...
        address[] memory payees = getPayees();
        pscf.releaseForSplitter(payments, payees);
    }

    function releaseRoyalties() public payable{
        address[] memory royalties = getPayees();
        for (uint256 i = 0; i < royalties.length; i++) {
            pscf.releaseAll(payable(royalties[i]));   
        }
    }
...
contract PaymentSplitterCloneFactory is Ownable {
    ...
    //release for the address
    function releaseForSplitter(address _splitter, address[] memory _payees)
        external{
            for(uint256 i = 0; i < _payees.length; i++){
                PaymentSplitter(payable(_splitter)).release(payable(_payees[i]));
        }
    }
...
abstract contract RandomlyAssigned is WithLimitedSupply {
    ...
    function nextToken() internal override ensureAvailability returns (uint256) {
        uint256 maxIndex = totalSupplyofToken() - tokenCount();
...
contract PaymentSplitter is Context {
    ...
    /**
     * @dev Creates an uninitialized instance of `PaymentSplitter`.
     */
    constructor() {}

    /**
     * @dev Initialized an instance of `PaymentSplitter` where each account in `payees` is assigned the number of shares at
     * the matching position in the `shares` array.
     *
     * All addresses in `payees` must be non-zero. Both arrays must have the same non-zero length, and there must be no
     * duplicates in `payees`. This function may be called only once and should be called in the same transaction that created
     * the splitter.
     */
        ...
        public
    {
        require(
            _payees.length == 0,
            "PaymentSplitterCloneable: already initialized"
        );
        require(
    ...
    /**
     * @dev Getter for the number of payees in `_payees`.
     */
...
abstract contract WithLimitedSupply {
    ...
    function availableTokenCount() public view returns (uint256) {
        return totalSupplyofToken() - tokenCount();
    }
    ...
    function _setSupply(uint256 _supply) internal virtual {
        ...
        emit SupplyChanged(totalSupplyofToken());
    }
}