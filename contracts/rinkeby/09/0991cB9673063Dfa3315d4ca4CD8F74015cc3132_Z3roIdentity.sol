...
contract Z3roIdentity is Ownable, ERC721A, ReentrancyGuard {
    ...
    uint256 public STAGE_MAX_SUPPLY_2; // todo: setters
    uint256 public STAGE_MAX_SUPPLY_3; // todo: setters
    ...
    constructor(
        ...
        z3roOwnership = new Z3roOwnership(address(this), address(multiSig));
        z3roUtility = new Z3roUtility(address(this), address(multiSig));
    ...
    // only multiSig can withdraw funds
    modifier isMultiSig() {
        require(_msgSenderERC721A() == address(multiSig));
        _;
    }
    ...
    modifier isEligibleMint(uint256 batchQty, bool isFl) {
        ...
        if (configs.onlyWhitelist) {
            ...
            require(
                MerkleProof.verify(
                    proof,
                    whitelist,
                    // todo: send bytes32 directly
                    /* keccak256(abi.encodePacked(_msgSenderERC721A())) */
                    leaf
                ),
                "You are not on the whitelist, please wait until public mint"
            );
        }
        _;
    }
    ...
    modifier isUnderWalletLimit(uint256 batchQty) {
        if (configs.stage == 1) {
            ...
            console.log("PREVIOUS MINTS => ", previousMints);
            require(
                (batchQty + _numberMinted(_msgSenderERC721A())) -
                    previousMints <=
    ...
    function withdrawFunds() external nonReentrant isMultiSig {
...
contract Z3roOwnership is Ownable, ERC721A, ReentrancyGuard {
    ...
    // only multiSig can withdraw funds
    modifier isMultiSig() {
        require(_msgSenderERC721A() == multiSig);
        _;
    }
