...
contract Moonkeys is ERC721A, ReentrancyGuard, Ownable {
    ...
    uint256 public constant FREE_SUPPLY = 999;
    uint256 public constant FREE_MAX_MINT = 3;
    uint256 public constant PUBLIC_MAX_MINT = 10;
    uint256 private FREE_PRICE = 0;
    uint256 private PUBLIC_PRICE = 0.01 * 10**18; // 0.01 ETH
    ...
    function freeMint(uint256 amount, bytes32[] memory proof) external payable {
        require(state == State.Public, "Public sale is not active.");
        ...
        require(
            msg.value >= FREE_PRICE * amount,
            "Ether value sent is incorrect."
        );

        bool isUserWhitelisted = isWhitelisted(msg.sender, proof);
        require(isUserWhitelisted, "User is not eligible to the free mint.");

        uint256 ownerMintedCount = addressMintedBalance[msg.sender];

        require(
            ownerMintedCount + amount <= FREE_MAX_MINT,
    ...
    function mint(uint256 amount) external payable {
        ...
        uint256 ownerMintedCount = addressMintedBalance[msg.sender];
        require(
            ownerMintedCount + amount <= PUBLIC_MAX_MINT,
            "Max NFT per address exceeded"
        );
    ...
    function withdraw() external nonReentrant onlyOwner {
        ...
        uint256 wallet1Value = balance / 4;
        uint256 wallet2Value = balance / 3;
        uint256 wallet3Value = balance - wallet1Value - wallet2Value;

        (bool success1, ) = payable(0x4069afe7352fD6697759A8FEe011b8aD5E8E3806)
            .call{value: wallet1Value}("");
        (bool success2, ) = payable(0x4069afe7352fD6697759A8FEe011b8aD5E8E3806)
