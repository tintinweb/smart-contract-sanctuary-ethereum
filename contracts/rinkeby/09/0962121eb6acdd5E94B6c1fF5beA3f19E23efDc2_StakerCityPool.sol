...
contract StakerCityPool is  Ownable{

	IERC20 staka = IERC20(0x87238DcE5FA8d0C1A971BaE3FAAa08e0a8E0861f);
    ...
    ////////// Rewards////////////////////
    address private refererRewardAccount ;
    uint256 public firstRefererReward ;
    uint256 public secondRefererReward ;
    ///////////////////////////////////////

    ////////// Packages////////////////////
    uint256 public STARTERS_minimumInvestment ;
    uint256 public RIDE_minimumInvestment ;
    uint256 public FLIGHT_minimumInvestment ;
    ...
    ////////// Affiliates ////////////////////
    uint256 public firstRefMilestone;
    uint256 public secondRefMilestone;
    uint256 public thirdRefMilestone;

    uint256 public firstMilestoneReward;
    uint256 public secondMilestoneReward;
    uint256 public thirdMilestoneReward;
    /////////////////////////////////////////
    ...
    struct user {
        ...
        bool firstMilestoneHit;
        bool secondMilestoneHit;
        bool thirdMilestoneHit;
    ...
    constructor( )  { 
        ...
        STARTERS_minimumInvestment = 5000;
        RIDE_minimumInvestment = 10000;
        FLIGHT_minimumInvestment = 20000;

        firstRefMilestone = 2;
        secondRefMilestone = 3;
        thirdRefMilestone = 4;

        firstMilestoneReward = 5000;
        secondMilestoneReward = 20000;
        thirdMilestoneReward = 40000;
        
    }


// The Admin becomes the owner of the contract when this is triggered
    function set_admin(address _admin) public onlyOwner {
        admin = _admin ;
    }

// The Admin can then set any account to be the Reward Account, and so Approval and Allowance should be set
    ...
    function set_minimumInvestment(uint256 _starter, uint256 _ride, uint256 _flight) public onlyAdmin {
        STARTERS_minimumInvestment = _starter;
        RIDE_minimumInvestment = _ride;
        FLIGHT_minimumInvestment = _flight;
    }

    function get_minimumInvestment(uint256 _package) public view returns(uint256 temp){
        if (_package==0)
            return STARTERS_minimumInvestment;
        if (_package==1)
            return RIDE_minimumInvestment;
        if (_package==2)
            return FLIGHT_minimumInvestment;
    }

    function set_RefMilestone(uint256 _firstRefMilestone, uint256 _secondRefMilestone, uint256 _thirdRefMilestone) public onlyAdmin {
        firstRefMilestone = _firstRefMilestone;
        secondRefMilestone = _secondRefMilestone;
        thirdRefMilestone = _thirdRefMilestone;
    }

    function set_MilestoneReward(uint256 _firstMilestoneReward, uint256 _secondMilestoneReward, uint256 _thirdMilestoneReward) public onlyAdmin {
        firstMilestoneReward = _firstMilestoneReward;
        secondMilestoneReward = _secondMilestoneReward;
        thirdMilestoneReward = _thirdMilestoneReward;
    }
    ...
    function getTotalStaked() public view returns(uint256) {
        return staka.balanceOf(address(this) ) ;
    }
    ...
    function withdrawRewards() public {
        ...
        staka.transferFrom (admin, msg.sender, w_reward);

    }

    function stake(uint256 _stake, uint256 _package, address _referer) public {

        require ( _stake >= get_minimumInvestment(_package), "Sent Less than Minimum investment");
        require ( staka.allowance(msg.sender, address(this)) >= _stake , "allowance not given");
        ...
        staka.transferFrom (msg.sender, address(this), _stake); // transferring stake to contract
        staka.transfer ( admin, _stake * 2/100); // 2% fee to Admin
    ...
    function unStake(uint256 _stake) public {
        ...
        if ( isUnstakingBeforeLockup(msg.sender) == true){
            ...
            staka.transfer (msg.sender, _stake*50/100); //50% to User
            staka.transfer (admin, _stake*50/100); //50% Fine given to Admin
            user_list[msg.sender].stakedAmount -= _stake;
            user_list[msg.sender].accumulatedReward = 0;
        ...
        staka.transferFrom (admin, msg.sender, w_reward);
        staka.transfer (msg.sender, _stake);
    ...
    // gives rewards to referer and pays Milestone rewards to Affiliates
    function distributeReward (uint256 _stake) private {
        address t_ref = user_list[msg.sender].referer ; 
        if ( t_ref != address(0)) {
            staka.transferFrom ( refererRewardAccount , t_ref , _stake * firstRefererReward /100 ); // referer of msg.sender
            payMilestonetoAffiliate(t_ref);
            ...
            if  ( t_ref != address(0) ){
                staka.transferFrom ( refererRewardAccount , t_ref , _stake * secondRefererReward /100 ); // referer of referer
                payMilestonetoAffiliate(t_ref);
    ...
    // Milestone need to be set
    // Get the total number of referral for an address
    // If the times reffered == MilestoneSet
    // Pay the Milestone price to the address
    // Only trigger any milestone once
        // If a milestone is reached
        // Turn on a flag
        // When next the function is called flag is checked
            // If on then skip else pay

    // function isMilstoneHit(address _affiliate) private{
    //     user_list[_affiliate].firstMilestoneHit = false; // Instan
    //     user_list[_affiliate].secondMilestoneHit = false; // 
    //     user_list[_affiliate].thirdMilestoneHit = false; //


    // }

    // Getter function for checking if any Milestone level is hit
    function getMilestoneFlag(address _affiliate, uint8 _level) public view returns(bool temp){
        if (user_list[_affiliate].firstMilestoneHit == true && _level == 1){ //False means the milestone is hit
            return true;
        } 
        if (user_list[_affiliate].secondMilestoneHit == true && _level == 2){
            return true;
        } 
        if (user_list[_affiliate].thirdMilestoneHit == true && _level == 3){
            return true;
        }
        else{
            return false;
    ...
    function payMilestonetoAffiliate (address _affiliate) private{
        if (getTimesReferred(_affiliate) == firstRefMilestone){ // Check if times reffered has hit firstMilestone
            if (user_list[_affiliate].firstMilestoneHit == false){ //Check again if previously paid (false means that user has not been paid)
            staka.transferFrom ( refererRewardAccount , _affiliate , firstMilestoneReward); // Pay the affiliate
            user_list[_affiliate].firstMilestoneHit = true; // Raise flag showing the user has been paid
            }
        }
        if (getTimesReferred(_affiliate) == secondRefMilestone){
            if (user_list[_affiliate].secondMilestoneHit == false){
            staka.transferFrom ( refererRewardAccount , _affiliate , secondMilestoneReward);
            user_list[_affiliate].secondMilestoneHit = true;    
            }        
        }
        if (getTimesReferred(_affiliate) == thirdRefMilestone){
            if (user_list[_affiliate].thirdMilestoneHit == false){            
            staka.transferFrom ( refererRewardAccount , _affiliate , thirdMilestoneReward);
            user_list[_affiliate].thirdMilestoneHit = true;
            }
        }
    }      
...
// Gettter function to know if any milestone is reached
    // function is_MilestoneReached
