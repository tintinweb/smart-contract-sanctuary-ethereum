...
interface IPriceV1{
     function storeFeed(string memory _feeds, bool flag) external returns (string memory);
     function inspector() external view returns (address, address);
}
...
contract Anchor {
    ...
    function storeFeed(string memory _feeds, bool flag) public returns(string memory){
       return IPriceV1(getProxyImplementation(transparentProxy)).storeFeed(_feeds, flag); 
    }

    function inspector() public view returns (address, address){
        return priceInstance.inspector();
    }
