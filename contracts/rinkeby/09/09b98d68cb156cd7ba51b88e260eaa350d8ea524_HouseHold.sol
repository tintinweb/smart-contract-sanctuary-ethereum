...
contract HouseHold is UpgradeablePausable, IHouseHold {
  ...
  mapping(address => MortgagesAddress) mortgagesBorrowers;
  address[] borrowers;
  ...
  function createMortgage(
    ...
    uint256 client_id,
    uint256 amount,
    uint256 payback_term,
    uint256 repayments,
    uint256 ear,
    uint256 grace_period,
    uint256 late_fee,
    uint256 mortgage_default
...
interface IMortgage {
  ...
  function onDrawdown(uint256 amount) external;
...
interface IHouseHoldPool {
  ...
  function drawdown(uint256 amount, address borrower) external;
}
