...
contract NFTFactory is Ownable {
    ...
    function allowAdmin(address _account, bool _flag) external onlyOwner {
        admins[_account] = _flag;
    }
    ...
    function createMultipleCollection(
        ...
        if (bPublic) {
            require(
                owner() == msg.sender,
                "Only owner can create public collection"
            );
        } else {
            require(admins[msg.sender], "Only admin can create collection");
        }

        bytes memory bytecode = type(MultipleNFT).creationCode;
        bytes32 salt = keccak256(
            abi.encodePacked(_uri, _name, block.timestamp)
        );
        assembly {
...
contract SingleNFT is ERC721, Ownable {
    ...
    event Burned(address owner, uint256 nftID);
	 ...
	 */
    function addItem(string memory _tokenURI)
        external
        payable
        returns (uint256)
    {
