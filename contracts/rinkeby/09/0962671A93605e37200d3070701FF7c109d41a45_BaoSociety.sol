...
import {VRFBaseRinkeby as VRFBase} from './VRFBase.sol';
import './ERC721A.sol';
...
contract BaoSociety is ERC721A, Ownable, VRFBase {
    ...
    string public unrevealedURI = 'ipfs://QmcTQWjwxD1Egq3DaKxe5VZ9DNyADc6nsPs5cdifkzc9Zm';
    ...
    address public signerAddress = 0x75f415923E71C54247c8B68666228AeC44C0C16D;

    constructor() ERC721A('BaoSociety', 'BAOSOC', MAX_SUPPLY, 1, MAX_PER_WALLET) {
        _mint(msg.sender, 188); // team reserve
    }
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        ...
        if (bytes(baseURI).length == 0 || !randomSeedSet()) return unrevealedURI;
...
contract VRFBase is VRFConsumerBase, Ownable {
    ...
    // function _getShuffledRangeAt(
    //     uint256 index,
    //     uint256 shift,
    //     uint256 n
    // ) internal view returns (uint256) {
    //     return shift + _getShuffledRangeAt(index - shift, n);
    // }

    // function _getShuffledRangeAt(uint256 index, uint256 n) internal view returns (uint256) {
    //     return _fisherYates(n, index)[index];
    // }

    // function _fisherYates(uint256 n, uint256 stopAtIndex) private view returns (uint256[] memory shuffled) {
    //     shuffled = new uint256[](n);
    //     if (n < 2) return shuffled;

    //     for (uint256 i; i < n; i++) shuffled[i] = i;

    //     if (stopAtIndex > n - 2) stopAtIndex = n - 2;

    //     uint256 r;
    //     for (uint256 i; i <= stopAtIndex; i++) {
    //         uint256 s = (i & 0xF) << 4; // 4-bit slot of random number r
    //         if (s == 0) r = uint256(keccak256(abi.encode(randomSeed, i)));
    //         uint256 j = i + (((r << s) >> (240 - s)) % (n - i));
    ...
    //     for (uint256 i; i < indices.length; i++) indices[i] = indices[i] - shift;
    //     uint256[] memory result = _getShuffledRangeAt(indices, n);
    //     for (uint256 i; i < result.length; i++) result[i] = shift + result[i];
    //     return result;
    // }

    // function _getShuffledRangeAt(uint256[] memory indices, uint256 n) internal view returns (uint256[] memory) {
    //     uint256 maxValue;
    //     for (uint256 i; i < indices.length; i++) if (indices[i] > maxValue) maxValue = indices[i];

    //     uint256[] memory shuffled = _fisherYates(n, maxValue);

    //     uint256[] memory result = new uint256[](indices.length);
    //     for (uint256 i; i < indices.length; i++) result[i] = shuffled[indices[i]];
...
contract ERC721A is Context, ERC165, IERC721, IERC721Metadata {
    ...
    function _mint(address to, uint256 quantity) internal {
        ...
        if (totalSupply_ + quantity > _collectionSize) revert MintExceedsLimit();

        unchecked {
            UserData memory userData = _userData[to];
            if (userData.numMinted + quantity > _maxPerWallet && address(this).code.length != 0)
