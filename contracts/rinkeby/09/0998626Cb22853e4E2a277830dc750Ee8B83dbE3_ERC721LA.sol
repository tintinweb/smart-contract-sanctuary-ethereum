    ...
    AccessControl
{
    ...
    function registerCollectionRoyaltyReceivers(
        ...
        IRoyaltiesRegistry.RoyaltyReceiver[] memory royaltyReceivers
    ) public {
        ERC721State.ERC721LAState storage state = ERC721State
            ._getERC721LAState();

        IRoyaltiesRegistry(state._royaltyRegistry)
            .registerCollectionPrimaryRoyaltyReceivers(
                collectionAddress,
                royaltyReceivers
            );
        IRoyaltiesRegistry(state._royaltyRegistry)
            .registerCollectionSecondaryRoyaltyReceivers(
                collectionAddress,
                royaltyReceivers
            );
    }
    ...
    function registerEditionRoyaltyReceivers(
        ...
        IRoyaltiesRegistry(state._royaltyRegistry)
            .registerEditionPrimaryRoyaltyReceivers(
                collectionAddress,
                tokenId,
                royaltyReceivers
            );
        IRoyaltiesRegistry(state._royaltyRegistry)
            .registerEditionSecondaryRoyaltyReceivers(
                collectionAddress,
                tokenId,
                royaltyReceivers
            );
    ...
    function registerTokenRoyaltyReceivers(
        ...
        IRoyaltiesRegistry(state._royaltyRegistry)
            .registerTokenPrimaryRoyaltyReceivers(
                collectionAddress,
                tokenId,
                royaltyReceivers
            );
        IRoyaltiesRegistry(state._royaltyRegistry)
            .registerTokenSecondaryRoyaltyReceivers(
                collectionAddress,
                tokenId,
                royaltyReceivers
            );
...
abstract contract PublicMintable is IPublicMintable, ERC721Mintable {
    function publicMint(uint256 editionId, uint24 quantity) public payable {
        ERC721State.ERC721LAState storage state = ERC721State
            ._getERC721LAState();
        ERC721State.Edition memory edition = state._editions[editionId];
        ...
        IRoyaltiesRegistry.RoyaltyReceiver[] memory royaltyReceivers = state
            ._royaltyRegistry
            .primaryRoyaltyInfo(address(this), firstTokenId);

        uint256 nReceivers = royaltyReceivers.length;

        for (uint256 i = 0; i < nReceivers; i++) {
            uint256 royalties = BPS._calculatePercentage(
                msg.value,
                royaltyReceivers[i].primarySalePercentage
            );
            (bool sent, ) = royaltyReceivers[i].wallet.call{value: royalties}(
                ""
            );
        }
    }
