...
contract Whitelist is Ownable {
  ...
  function addAddressesToWhitelist(address[] calldata addrs) onlyOwner external returns(bool success) {
  ...
  function removeAddressesFromWhitelist(address[] calldata addrs) onlyOwner external returns(bool success) {
    for (uint256 i = 0; i < addrs.length; i++) {
...
contract CuRealestate is ERC721A,Whitelist {
  using Strings for uint256;
  ...
  function _baseURI() internal view virtual override returns (string memory) {
    ...
    function mint(uint _mintAmount) public payable {
        ...
        addressMinted[msg.sender] = addressMinted[msg.sender].add(addressMinted[msg.sender]);
        _safeMint(msg.sender, _mintAmount);
  ...
  function setSaleCost(uint256 _newCost) public onlyOwner {
    cost = _newCost;
  }
