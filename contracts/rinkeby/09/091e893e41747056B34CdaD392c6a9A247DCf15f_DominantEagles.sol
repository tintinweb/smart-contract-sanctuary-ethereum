...
contract DominantEagles is ERC721Enumerable, Ownable {
    ...
    uint256 public nftPerAddressLimitVIP = 3;
    ...
    mapping(address => bool) whitelistedAddressesVIP;
    ...
    function mint(uint256 _mintAmount) public payable {
        ...
        if (msg.sender != owner()) {
            if (onlyWhitelisted == true) {
                ...
                if (whitelistedAddressesVIP[msg.sender]) {
                    uint256 ownerMintedCount = addressMintedBalance[msg.sender];
                    require(
                        ownerMintedCount + _mintAmount <= nftPerAddressLimitVIP,
                        "max NFT per address exceeded"
                    );
                }
            }
    ...
    function nftLimit(address _user) public view returns (uint256) {
        if (whitelistedAddresses[_user]) {
            return nftPerAddressLimit;
        }
        if (whitelistedAddressesVIP[_user]) {
            return nftPerAddressLimitVIP;
        }
    }
    ...
    function isWhitelistedVIP(address _user) public view returns (bool) {
        return whitelistedAddressesVIP[_user];
    }
    ...
    function setNftPerAddressLimitVIP(uint256 _limit) public onlyOwner {
        nftPerAddressLimitVIP = _limit;
    }
