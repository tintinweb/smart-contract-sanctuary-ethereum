...
/// @title The Moonwalkers V2 NFT Smart Contract.
/// @author The Moonwalkers V2 NFT project.
/// @notice This contract allows users to mint and transfer the Moonwalkers V2 NFTs.
/// @dev The contract inherits from the ERC721A contract.
contract MoonwalkersV2 is ERC721A, Pausable, Ownable, ReentrancyGuard {
  ...
  /// @dev Watchdog to protect metadata
  ...
  /// @dev Merke root that contains all the wallet that are allowed to claim an airdrop.
  ...
  /// @dev Mapping of the amount of NFT claimed by an address.
  ...
  /// @dev Constructor.
  constructor(uint256 _maxSupply) ERC721A("MoonwalkersV2", "MW2") {
    baseTokenURI = "";
  ...
  /// @dev Modifier used as a watchdog for the setBaseTokenURI function.
  modifier metadataEditable() {
    require(metadataFrozen == true, "The metadata are frozen.");
    _;
  }
  ...
  function alphaMint(uint256 _mintAmount, bytes32[] calldata _merkleProof) external payable whenNotLocked callerIsUser {
    ...
    require(_totalMinted().add(_mintAmount) < maxSupply.add(1), "Max supply reached.");
    require(msg.value == config.presaleMintPrice.mul(_mintAmount), "Tr. value did not equal the mint price.");
    require(amountMintedWhitelist[msg.sender].add(_mintAmount) < config.maxAlphaMintPerWallet.add(1), "You cannot mint that much.");
    bytes32 leaf = keccak256(abi.encodePacked(msg.sender));
  ...
  /// @dev Free claim function for gen' 1 holders.
  function airdropClaim(bytes32[] calldata _merkleProof) external payable whenNotLocked callerIsUser {
    SaleConfig memory config = saleConfig;
    require(block.timestamp > config.airdropStartTime, "Free claim has not started yet.");
    require(_totalMinted() < maxSupply, "Max supply reached.");
    bytes32 leaf = keccak256(abi.encodePacked(msg.sender));
    require(MerkleProof.verify(_merkleProof, merkleRootAirdrop, leaf), "Invalid Merkle Proof.");

    amountClaimed[msg.sender] += 1;
  ...
  /// @dev Sets the base token URI prefix. In case we need to load a patch (glitch on an NFT...).
  function setBaseTokenURI(string memory _baseTokenURI) external onlyOwner metadataEditable {
    baseTokenURI = _baseTokenURI;
  }

  /// @dev Sets the base token URI suffix.
  function setExtensionTokenURI(string memory _extensionTokenURI) external onlyOwner metadataEditable {
  ...
  function setRevealed() external onlyOwner {
    require(isRevealed == false, "The collection is already revealed.");
    isRevealed = true;
  }
  ...
  /// @dev Set the max supply (we are onmy able to cut the supply)
  function setMaxSupply(uint256 _maxSupply) external onlyOwner {
    require(_maxSupply < maxSupply, "You cannot up the supply");
    maxSupply = _maxSupply;
  }

  /// @dev Freeze the medata for ever.
  function freezeMetadata() external onlyOwner {
    require(metadataFrozen == false, "Metadata are already frozen.");
    metadataFrozen = true;
  }
