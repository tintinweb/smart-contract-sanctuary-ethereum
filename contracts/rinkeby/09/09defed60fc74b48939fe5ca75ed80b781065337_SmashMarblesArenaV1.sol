...
contract SmashMarblesArenaV1 is SmashMarblesStorage {
    ...
    function purchaseNFT(Order calldata order, bytes memory signature)
        external
        virtual
        isblacklistedAddresses
    {
        ...
        require(status == true, "$GenesisV2: cannot purchase the token");
    ...
    function claimNft() external virtual isblacklistedAddresses {
        require(
            rewardBalance[msg.sender] > 0,
            "SmashMarblesArenaV1: No reward for user"
        );

        rewardBalance[msg.sender] -= 1;
        _mint(msg.sender, tokenCounter);

        emit ClaimedNft(msg.sender, tokenCounter);
     ...
     *
     * @param _marblePresaleStatus for white list and public sale , @param _arenaSaleStatus for arena sale
...
contract SmashMarblesStorage is
    ...
    ReentrancyGuardUpgradeable
{
    ...
    event ClaimedNft(address user, uint256 TokenId);
}
