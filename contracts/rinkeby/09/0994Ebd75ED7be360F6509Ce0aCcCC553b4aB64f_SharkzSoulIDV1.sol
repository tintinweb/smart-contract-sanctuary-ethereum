...
contract SharkzSoulIDV1 is IVoteScore, Initializable, UUPSUpgradeable, AdminableUpgradeable, EIP712WhitelistUpgradeable, ERC4973SoulContainerUpgradeable, ReentrancyGuardUpgradeable {
        ...
        callerIsUser 
    {
        if (_publicMintMode == 1) {
            ...
            require(soulData.isExternalTokenOwner(_tokenContract, _msgSenderERC4973()), "Caller is not a target token owner");
        }
    ...
    function tokenBadgeTraits(uint256 _tokenId) public virtual view returns (string memory) {
        ...
        for (uint256 i = 0; i < badgeSettings.length; i++) {
            badgeContract = badgeSettings[i].contractAddress;
            output = string(abi.encodePacked(output, soulData.getBadgeTrait(badgeContract, i, _tokenId, ownerAddress)));
        }
        return output;
    }
}
...
interface ISoulData {
     ...
     */
    function getBadgeTrait(address _badgeContract, uint256 _traitIndex, uint256 _soulTokenId, address _soulTokenOwner) external view returns (string memory);

    /**
     * @dev Returns whether an address is token owner
     */
    function isExternalTokenOwner(address _contract, address _ownerAddress) external view returns (bool);
