...
contract ShipFuser is Managable {
    ...
    uint32 public shipClassPrice = 100;
    ...
    function fuse(uint256[] memory _botIds, address _token) external returns(uint256 totalShipsFused) {
        ...
        (uint256 _tokenPrice, uint256 _oilPrice) = getFusePrices(_botIds.length, _token);

        (uint[3] memory class, bool fullGenes) = _calculateClass(_botIds);
    ...
    function getFusePrices(
        ...
        require(_botNumber <= tokenPrices[_token].length && _tokenDecimals > 0, "not allowed");
        ...
        require(_botNumber <= tokenPrices[_oilAddress].length && _oilDecimals > 0, "not allowed");
    ...
    function _calculateClass(uint256[] memory _botIds) private view returns(uint[3] memory _totalClass, bool fullGenes){
        fullGenes = true;
