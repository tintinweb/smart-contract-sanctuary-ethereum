...
contract Voting {
    ...
    payable
    {
        ...
        for (uint i; i<votings[_vID].candidates.length; ++i){
            ...
            else if (votings[_vID].candidates[i].votes == maxVotes) winners+=1;

        }

        if (winners == 1){
            payable(votings[_vID].candidates[winnerID].addr).transfer(votings[_vID].prizePull*9/10);
            comission = votings[_vID].prizePull * 1/10;
            votings[_vID].finished = true;
            emit votingFinished (winners, votings[_vID].prizePull);
        }

        else if (maxVotes == 0){
            votings[_vID].finished = true;
            emit votingFinished (0, 0);
        }

        else{
            uint _prize = votings[_vID].prizePull/winners*9/10;
            comission = votings[_vID].prizePull * 1/10;

            for (uint i; i<votings[_vID].candidates.length; ++i){
                if (votings[_vID].candidates[i].votes == maxVotes){
                    payable(votings[_vID].candidates[i].addr).transfer(_prize);
                }
            }

            emit votingFinished(winners, _prize);
        }
