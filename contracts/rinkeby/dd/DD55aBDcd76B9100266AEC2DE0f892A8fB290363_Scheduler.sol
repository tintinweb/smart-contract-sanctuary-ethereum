    ...
    SchedulerStorage
{
    ...
    event SchmintCreated(address indexed target, bytes32 indexed schmintId);
    ...
    function _createSchmint(
        ...
        bytes32 schmintId;
        // scoping creation of schmint id
        {
            // generate resolver hash
            bytes memory emptyBytes;
            bytes32 resolverHash = getResolverHash(address(0), emptyBytes);
        ...
        // scoping creation of task
        {
            ...
            IOps(ops).createTaskNoPrepayment(
                ...
                this.executeSchmint.selector,
                address(0),
                abi.encodeWithSelector(this.executeSchmint.selector, schmintId),
                ETH
            );
        }

        // emit event when schmint created
        emit SchmintCreated(_target, schmintId);
    }

    function executeSchmint(bytes32 taskId) external nonReentrant {
        // -> check if balance < value to be sent, does the execution passes or how does it fail
        // -> check if user is eligible for free schmint
        // -> check if user wants to receive tokens in their wallet or not
        // -> if user wants to receive in their wallet, transfer tokens to their wallet
        // -> transfer schminting fees

        // read schmint
        Schmint memory schmint = schmints[taskId];
        ...
        // scoping checks into one block
        {
            ...
            if (schmint.gasPriceLimit > 0 && tx.gasprice <= schmint.gasPriceLimit) {
        ...
        schmints[taskId] = schmint;

        // scoping external call to one block
        {
            // execute schmint
            (bool success, ) = schmint.target.call{value: schmint.value}(
                schmint.data
            );
...
contract SchedulerStorage {
    ...
    mapping(bytes32 => Schmint) public schmints;
...
/// @title Executor
/// @author Simplr Team
/// @notice inspiration taken from Gnosis Safe Executor contract
contract Executor {
    function execute(
        address _target,
        uint256 _value,
        ...
        uint256 _gas
            ...
            assembly {
                success := call(_gas, _target, _value, add(_data, 0x20), mload(_data), 0, 0)
            }
    }
}