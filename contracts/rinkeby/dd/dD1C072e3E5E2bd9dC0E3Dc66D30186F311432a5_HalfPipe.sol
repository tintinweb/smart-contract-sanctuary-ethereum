...
contract ShareWrapper {
    ...
    function stake(uint256 amount) public virtual {
        uint256 beforeBalance = share.balanceOf(address(this));
        share.safeTransferFrom(msg.sender, address(this), amount);
        uint256 depositAmount = share.balanceOf(address(this)).sub(beforeBalance);
        _totalSupply = _totalSupply.add(depositAmount);
        _balances[msg.sender] = _balances[msg.sender].add(depositAmount);
    }

    function withdraw(uint256 amount) public virtual {
        uint256 masonShare = _balances[msg.sender];
        require(masonShare >= amount, "HalfPipe: withdraw request greater than staked amount");
...
contract HalfPipe is ShareWrapper, ContractGuard {
    using SafeERC20 for IERC20;
    ...
    function getThrivePrice() external view returns (uint256) {
        return treasury.getThrivePrice();
    }
    ...
    function claimReward() public updateReward(msg.sender) {
        ...
        if (reward > 0) {
            ...
            thrive.safeTransfer(msg.sender, reward);
    ...
    function allocateSeigniorage(uint256 amount) external onlyOneBlock onlyOperator {
        ...
        uint256 beforeBalance = thrive.balanceOf(address(this));
        thrive.safeTransferFrom(msg.sender, address(this), amount);
        uint256 depositAmount = thrive.balanceOf(address(this)).sub(beforeBalance);

        // Create & add new snapshot
        uint256 prevRPS = getLatestSnapshot().rewardPerShare;
        uint256 nextRPS = prevRPS.add(depositAmount.mul(1e18).div(totalSupply()));

        MasonrySnapshot memory newSnapshot = MasonrySnapshot({
            time: block.number,
            rewardReceived: depositAmount,
    ...
    function governanceRecoverUnsupported(IERC20 _token, uint256 _amount, address _to) external onlyOperator {
        ...
        require(address(_token) != address(thrive), "thrive");
