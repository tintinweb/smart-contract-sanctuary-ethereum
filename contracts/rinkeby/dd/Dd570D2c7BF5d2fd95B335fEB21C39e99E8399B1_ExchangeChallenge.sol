...
contract ExchangeChallenge is IExchangeChallenge, Ownable{
    ...
    function addChallenge(
        ...
        uint256 airdropEndAt)external override returns (bytes32){
            ...
            bytes32 encodedChallengeId = keccak256(
            abi.encodePacked(block.timestamp, msg.sender, newChallengeId)
        );

            challenges[encodedChallengeId] = Challenge(
                seller, 
                collection, 
                assetId, 
                amount, 
            ...
            emit AddChallenge(encodedChallengeId, seller, collection, assetId, amount, allowResell, saleEnd, airdropStartAt, airdropEndAt);

            return encodedChallengeId;
        }
    /**
...
interface IExchangeChallenge {
     ...
     */
    event AddChallenge(bytes32 id, address indexed seller, address indexed collection, uint256  assetId, uint256  amount,bool allowResell, uint256  saleEnd, uint256  airdropStartAt, uint256  airdropEndAt);
    /**
     * @dev save an offer challenge.
     */
    function addChallenge(address seller, address collection, uint256 assetId, uint256 amount, bool allowResell, uint256 saleEnd, uint256 airdropStartAt, uint256 airdropEndAt) external  returns (bytes32);
