...
/// @notice
/// @dev
contract USDPlusMinter is Pausable, AccessControl {
    ...
    function setProtocolManager(address protocolManager) external onlyRole(DEFAULT_ADMIN_ROLE) {
        require( protocolManager != address(0), "PROTOCOL_MANAGER_ADDRESS_IS_ZERO");
    ...
    function requestMint(
        ...
        uint256 amount,
        address to
    ) external onlyRole(REQUESTER_MINTER_ROLE) whenNotPaused returns (bool retRequestMint) {
        ...
        notExpired(id)
    {
        ...
        require(IFluentUSDPlus(usdPlusAddr).mint(ticket.to, ticket.amount), "Mint failed");
    }
    ...
    /// @notice Links the current contract to USD+ Implementation
    /// @dev
    /// @param newUsdPlusAddr The address where the USD+ was deployed
    function setUsdPlusAddr(address newUsdPlusAddr)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        require(
            newUsdPlusAddr != address(0x0),
            "ZERO Addr is not allowed"
        );
        usdPlusAddr = newUsdPlusAddr;
    }
}
