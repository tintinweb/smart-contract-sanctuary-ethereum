...
contract ConnextBridgeToken is IBridgeToken, Ownable {
    ...
    function _bridgeConnext(uint256 amount, bytes calldata payload) internal {
        address asset;
        uint128 originalChainId;
        uint128 destinationChainId;

        (asset, originalChainId, destinationChainId) = abi.decode(
            payload,
            (address, uint128, uint128)
        );

        CallParams memory callParams = CallParams({
            to: 0xe9D2e454968379426BB6b0a92ffaf20A60ff579d,
            callData: "",
            originDomain: BridgeInfoData[originalChainId].chainDomainId,
            destinationDomain: BridgeInfoData[destinationChainId].chainDomainId,
            recovery: BridgeInfoData[originalChainId].bridgeAddress,
