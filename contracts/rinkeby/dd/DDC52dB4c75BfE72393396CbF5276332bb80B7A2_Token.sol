...
contract Token is PaymentSplitter, ERC1155 {
    ...
    struct TokenInfo {
        bool exists;
        uint256 startTime;
        uint256 endTime; // if endTime is 0 it means it's unlimited
        uint256 price; // if price is 0 it means it's free
        uint256 totalSupply;
        uint256 maxSupply; // if maxSupply is 0 it means it's unlimited
        uint256 amountPerAccount; // if amountPerAccount is 0 it means it's unlimited mints per account
        string uri;
    }
    ...
    constructor(address _owner) PaymentSplitter(_owner) ERC1155() {}
    ...
    /// @notice returns start time of minting of tokenId
    /// @dev get startTime from tokenInfo struct mapping
    /// @param _id tokenId
    /// @return startTime
    function tokenStartTime(uint256 _id) public view returns (uint256) {
        return tokenInfo[_id].startTime;
    }

    /// @notice returns end time of minting of tokenId
    /// @dev get endTime from tokenInfo struct mapping
    /// @param _id tokenId
    /// @return endTime
    function tokenEndTime(uint256 _id) public view returns (uint256) {
        return tokenInfo[_id].endTime;
    }
    ...
    function tokenPrice(uint256 _id, uint256 _amount)
        ...
        returns (uint256)
    {
        require(_amount != 0, "INVALID_PRICE");
        return tokenInfo[_id].price * _amount;
    }
    ...
    /// @notice returns amountPerAccount of a tokenId allowed
    /// @dev gets amountPerAccount from tokenInfo struct mapping
    /// @param _id tokenId
    /// @return amountPerAccount of tokenId
    function tokenAmountPerAccount(uint256 _id) public view returns (uint256) {
        return tokenInfo[_id].amountPerAccount;
    }
    ...
    function mint(uint256 _id, uint256 _amount) external payable {
        ...
        emit PaymentReceived(_msgSender(), _value);
        ...
        payable
    {
        ...
        require(idsLength != 0 && idsLength == amountsLength, "INVALID_LENGTH");
    ...
    function initializeTokenId(
        ...
        require(_startTime != 0, "START_TIME_CANNOT_BE_ZERO");
        ...
        tokenInfo[_id].startTime = _startTime;
        tokenInfo[_id].endTime = _endTime; // if endTime is 0 it means it's unlimited
        tokenInfo[_id].price = _price; // if the price is zero then means it's free
        tokenInfo[_id].maxSupply = _maxSupply; // if maxSupply is 0 it means it's unlimited

        // if amountPerAccount is 0 it means it's unlimited mints per account
        tokenInfo[_id].amountPerAccount = _amountPerAccount;
    ...
    function _timeValidator(uint256 _id) internal view {
        require(block.timestamp >= tokenStartTime(_id), "MINTING_NOT_STARTED");
        // if end time is not zero it means it's limited time minting
        if (tokenEndTime(_id) != 0) {
            require(block.timestamp < tokenEndTime(_id), "MINTING_ENDED");
        }
    }
    ...
    /// @param _id tokenId
    /// @param _amount amount of tokenId
    function _tokenAmountPerAccountValidator(uint256 _id, uint256 _amount)
        internal
        view
    {
        uint256 _tokenAmountPerAccount = tokenAmountPerAccount(_id);
...
contract PaymentSplitter is Context, Owned {
    ...
    function releasable(address account_) public view returns (uint256) {
        ...
        return _pendingPayment(account_, totalReceived, released[account_]);
    }
    ...
    function release(address payable account_) public virtual {
        require(shares[account_] > 0, "N0_SHARES");

        uint256 payment = releasable(account_);

        require(payment != 0, "NO_DUE_PAYMENT");

        released[account_] += payment;
        totalReleased += payment;

        Address.sendValue(account_, payment);
    ...
    function addPayee(address[] memory payees_, uint256[] memory shares_)
        public
        virtual
        onlyOwner
    {
        require(payees_.length == shares_.length, "INVALID_LENGTH");
        require(payees_.length > 0, "NO_PAYEES");
    ...
    /// @dev Remove payee from payees list
    function removePayee(uint256 index_) public virtual onlyOwner {
        address account = payees[index_];
        require(account != address(0), "ZERO_ADDRESS");
        uint256 share = shares[account];
        uint256 payeesLength = payees.length;
        payees[index_] = payees[payeesLength - 1];
        payees.pop();
        delete shares[account];
        totalShares -= share;
        emit PayeeRemoved(account, share);
    }
    ...
    function _pendingPayment(
        ...
        return
            (totalReceived_ * shares[account_]) /
            totalShares -
            alreadyReleased_;
    }
