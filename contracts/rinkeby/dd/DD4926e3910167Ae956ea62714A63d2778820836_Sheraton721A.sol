...
// File: contracts/KiddieFinal_V11_ImproveArray.sol
...
contract Sheraton721A is ERC721A, Ownable, ReentrancyGuard {
    ...
    bool public privateWhitelisted = false;
    ...
    mapping (address => uint256) addressPWToNumberOfTokensMinted;
    bool PWMintPeriod = true;

    mapping (address => uint256) addressPublicToNumberOfTokensMinted;
    bool PublicMintPeriod = true;

    address constant public privateWLAddress = 0x5B38Da6a701c568545dCfcB03FcB875f56beddC4;

    constructor() ERC721A("Sheraton", "SHER") {
    ...
    function publicWhitelistMint(uint256 quantity) external payable mintCompliance(quantity) {
        ...
        addressPWToNumberOfTokensMinted[msg.sender] += quantity;
        if(PWMintPeriod) {
            require(addressPWToNumberOfTokensMinted[msg.sender] <= mintedPublicWhitelistAddressLimit, "No free mints left on Public Whitelist!");
        }
    ...
    function publicMint(uint256 quantity) external payable mintCompliance(quantity) {
        ...
        addressPublicToNumberOfTokensMinted[msg.sender] += quantity;
        if(PublicMintPeriod) {
            require(addressPublicToNumberOfTokensMinted[msg.sender] <= mintedPublicAddressLimit, "No free mints left on Public Sale!");
        }
