...
/*                                                                                          
 ...
 Artist: @elcinarpacay
 Devs: @takez0_o / @0xReddkidd
 Founders: @,@,@,@,@,@

 /////////////////////////

 /// DeadlyPerks Contract ///

 Founded by: @deadlydotgames
 Developed by: @takez0_o
 Sponsored by: @degen_fyi
 Contract Inspirations: Loot Project, Nuclear Nerds
...
contract DeadlyPerks is ReentrancyGuard, ERC721Enumerable, Ownable {
    ...
    mapping (address => address) public references; // how much does this cost for user to add 20 address? also test with nested mapping for amount.



    constructor(bytes32 _merkleRoot, address _proxyRegistryAddress, address _takezoFromAccounting, address _deadlyRegistry) ERC721("Deadly Perks", "dPERKS") {
        holyBook.merkleRoot = _merkleRoot;
        holyBook.proxyRegistryAddress = _proxyRegistryAddress;
        holyBook.takezoFromAccounting = _takezoFromAccounting;
        holyBook.deadlyRegistry = _deadlyRegistry;
        holyBook.MAX_PER_WL = 20;
        holyBook.MAX_PER_TX = 10;
        holyBook.RESERVES = 1000;
        holyBook.PRICE = 0.02 ether;
        holyBook.MAX_SUPPLY = 0;
        // transferOwnership(_takezoFromAccounting);
    }

    function enterReferences (address[] calldata _address, uint256[] calldata _tokenIds) external {
        require(isOwnerOf(_msgSender(), _tokenIds), "Must own the tokens.");
        for(uint256 i; i < _tokenIds.length; i++)
            references[_msgSender()] = _address[i];
    }
    
    function tokenURI(uint256 tokenId) public view override returns (string memory){  
        return IDeadlyRegistry(holyBook.deadlyRegistry).tokenURI(tokenId);
    }
    ...
    function togglePublicSale(uint256 _MAX_SUPPLY) external onlyOwner {
        delete holyBook.merkleRoot;
        holyBook.MAX_SUPPLY = _MAX_SUPPLY;
    }
    ...
    function _verify(bytes32 leaf, bytes32[] memory proof) internal view returns (bool) {
        return MerkleProof.verify(proof, holyBook.merkleRoot, leaf);
    }
    ...
    function collectReserves() external onlyOwner {
        ...
        for(uint256 i; i < holyBook.RESERVES; i++)
            _mint(_msgSender(), i);
    }
    ...
    function whitelistMint(uint256 amount, bytes32[] calldata proof) public payable {
        ...
        require(whitelistClaimed[_msgSender()] + amount <= holyBook.MAX_PER_WL, "Exceeds whitelist supply"); 
    ...
    function publicMint(uint256 amount) public payable {
        uint256 totalSupply = _owners.length;
        require(totalSupply + amount < holyBook.MAX_SUPPLY, "Exceeds max supply.");
