...
contract NobleWolfPack is Ownable, ERC721A {
    uint256 public presalePrice = 0.2 ether;
    uint256 public publicSalePrice = 0.14 ether;
    
    uint256 public constant devReservedTokens = 25;
    uint256 public constant giftReservedTokens = 20;
    uint256 public constant vipReservedTokens = 777;

    uint256 public giftedSupply = 0;
    uint256 public mintedSupply = 0;
    uint256 public constant totalTokenSupply = 7777 - devReservedTokens - giftReservedTokens;

    uint256 public constant maxTokensPerMint = 5;
    uint256 public constant maxTokensPerWhitelistedAddress = 5;

    bool public hasPresaleStarted = false;
    bool public hasPublicSaleStarted = false;

    string public tokenBaseURI = "ipfs://QmWAbumYQFAskr23uBcDUrFs5DHePTxtxEJ5YnKv12xpNz/";

    mapping(address => bool) public presaleWhitelist;
    mapping(address => uint256) public presaleWhitelistPurchased;

    constructor() ERC721A("NWP Test", "NWP", maxTokensPerWhitelistedAddress, 7777) {
    ...
    function setPublicSalePrice(uint256 val) external onlyOwner {
        publicSalePrice = val;
    }
    ...
    function devMint() external onlyOwner {
        for(uint256 i = 0; i < devReservedTokens / maxTokensPerWhitelistedAddress; i++) {
            _safeMint(msg.sender, maxTokensPerWhitelistedAddress);
        }
    }

    function gift(address[] calldata receivers) external onlyOwner {
        require(giftedSupply + receivers.length <= giftReservedTokens, "Not enough tokens reserved for gifting");

        for (uint256 i = 0; i < receivers.length; i++) {
            _safeMint(receivers[i], 1);
        }

        giftedSupply += receivers.length;
    }
    ...
    function mint(uint256 amount) external payable {
        ...
        if (!hasPublicSaleStarted) {
            ...
            require(mintedSupply + amount <= vipReservedTokens, "Cannot mint more than the max vip tokens reserved");
        }
        require(mintedSupply + amount <= totalTokenSupply, "No tokens left for minting");
