...
contract AuctionMarketPlace is IAuctionMarketPlace, Initializable,UUPSUpgradeable, OwnableUpgradeable, PausableUpgradeable,ReentrancyGuardUpgradeable,AuctionMarketPlaceStorage {
    ...
    function initialize() initializer public {
        ...
        //default auction Duration is 1 day
    ...
    function setAuctionDefaultDuration(uint256 _auctionDuration) external onlyOwner {
        require (_auctionDuration >= 15 minutes,"AuctionMarkerPlace: Auction Duration must be more than 15 min");
    ...
    function placeBid(uint256 _tokenId) payable external nonReentrant whenNotPaused {
        ...
        require(auction.startedAt == 0 || block.timestamp <= auction.startedAt + auctionDuration || block.timestamp < auction.lastBidAt + 15 minutes, "AuctionMarketPlace: Auction times up.");
        ...
        if (!auction.hasBid) {
            //TODO make safe
            //if is first bid in auction start auction
            auction.startedAt = block.timestamp;
            auction.hasBid = true;
            isFirstBid = true;
        } else {
            (bool sent, ) = payable(auction.lastBidBidder).call{value: auction.lastBidAmount}("") ;
            if(!sent){
                pendingWidthrawMapping[_tokenId][auction.lastBidBidder] = auction.lastBidAmount;
            }
        }
        ...
        uint256 endAt = (auction.startedAt + auctionDuration) > (auction.lastBidAt + 15 minutes) ? auction.startedAt + auctionDuration : auction.lastBidAt + 15 minutes;
    ...
    function settle(uint256 _tokenId) external whenNotPaused nonReentrant { 
        ...
        require(block.timestamp > auction.startedAt + auctionDuration && block.timestamp > auction.lastBidAt + 15 minutes, "AuctionMarketPlace: auction not finished yet.");
        ...
        //transfer royalty if not first sale
        if(royaltyAmount > 0)
            payable (tokenCreator).transfer(royaltyAmount);
    ...
    function withdraw(uint256 _tokenId) external nonReentrant  {
        uint bal = pendingWidthrawMapping[_tokenId][msg.sender];
