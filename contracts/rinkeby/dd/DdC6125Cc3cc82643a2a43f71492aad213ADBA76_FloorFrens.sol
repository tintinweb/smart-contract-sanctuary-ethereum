...
contract FloorFrens is ERC721A, ERC721ABurnable, Lockable, Toggleable, Administrable, EIP712Common{
  ...
  string private tokenDescription = "FILL ME IN";
  string private tokenName = "Floor Frens";
  ...
  function _imageURI(uint256 tokenId) internal view returns(string memory) {
    ...
    if(tokenId <= currentMaximum) {
      uri = string.concat(baseImageURI, "0.png");
    } else {
      uri = string.concat(baseImageURI,  Utils.uintToString(_placeInLine(tokenId)), ".png");
    }
