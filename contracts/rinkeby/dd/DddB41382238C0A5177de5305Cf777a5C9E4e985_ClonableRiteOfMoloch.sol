...
import "./ClonableRiteOfMoloch.sol";

contract CohortFactory is AccessControl {

    bytes32 public constant ADMIN = keccak256("ADMIN");

    event newCohort(address cohortAddress, address deployer);
    ...
    constructor() {
        ...
        implementations[iid] = address(new ClonableRiteOfMoloch());
     ...
     * @param membershipCriteria the contract address read from in order to ascertain cohort completion
     * @param stakingAsset the contract address for the asset which is staked into the cohort contract
     * @param treasury the address which receives tokens when initiates are slashed
     * @param threshold the minimum amount of criteria which constitutes membership
     * @param assetAmount the minimum amount of staking asset required to join the cohort
     * @param name the name for the cohort's soul bound tokens
     * @param symbol the ticker symbol for cohort's soul bound token
     * @param baseUri the uniform resource identifier for accessing soul bound token metadata
     */
    function createCohort(
        ...
        address clone = Clones.clone(implementations[iid]);

        // initialize the cohort clone
        ClonableRiteOfMoloch(clone).initialize(
            membershipCriteria,
            stakingAsset,
            treasury,
            threshold,
        ...
        emit newCohort(clone, msg.sender);
...
// SPDX-License-Identifier: MIT
// @author st4rgard3n, bitbeckers, MrDeadce11, plor / Raid Guild
pragma solidity ^0.8.4;
...
contract ClonableRiteOfMoloch is ERC721Upgradeable, AccessControlUpgradeable {
    ...
    /**
     * @dev Deploys a new clone proxy instance for cohort staking
     * @param daoAddress_ the contract address read from in order to ascertain cohort completion
     * @param tokenAddress_ the contract address for the asset which is staked into the cohort contract
     * @param treasury_ the address which receives tokens when initiates are slashed
     * @param shareThreshold_ the minimum amount of criteria which constitutes membership
     * @param minStake_ the minimum amount of staking asset required to join the cohort
     * @param name_ the name for the cohort's soul bound tokens
     * @param symbol_ the ticker symbol for cohort's soul bound token
     * @param baseURI_ the uniform resource identifier for accessing soul bound token metadata
     */
    function initialize(
        address daoAddress_,
        address tokenAddress_,
        address treasury_,
        uint256 shareThreshold_,
        uint256 minStake_,
