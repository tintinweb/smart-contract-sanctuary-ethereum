...
import "tf-nft-contracts/contracts/nft/upgradable/TokenForge1155v3Upgradeable.sol";

import "hardhat/console.sol";
//import "tf-nft-contracts/contracts/nft/TokenForge721.sol";

contract TokenForgeFactoryTF01 is Context, AccessControlEnumerable {
    event ContractDeployed(uint8 contractType, address contractAddress);

    uint8 public constant CONTRACT_TYPE_TokenForge721 = 1;
    uint8 public constant CONTRACT_TYPE_TokenForge721p = 2;

    uint8 public constant CONTRACT_TYPE_TokenForge1155v3 = 10;

    mapping(address => address[]) private _deployedContracts;
    mapping(address => uint8) private _deployedContractTypes;
    ...
    constructor() {
        ...
        tokenImplementation = address(new TokenForge1155v3Upgradeable());
    }

    function createTokenForge1155v3(address signer, string memory baseUri)
        public
        returns (address)
    {
        address clone = Clones.clone(tokenImplementation);
        TokenForge1155v3Upgradeable(clone).initialize(signer, baseUri);

        console.log("Owner 2: %s", TokenForge1155v3Upgradeable(clone).owner());

        // enable caller as minter role
        TokenForge1155v3Upgradeable(clone).grantRole(TokenForge1155v3Upgradeable(clone).MINTER_ROLE(), _msgSender());
        // renounce minting role from factory
        TokenForge1155v3Upgradeable(clone).renounceRole(TokenForge1155v3Upgradeable(clone).MINTER_ROLE(), address(this));

        TokenForge1155v3Upgradeable(clone).grantRole(TokenForge1155v3Upgradeable(clone).DEFAULT_ADMIN_ROLE(), _msgSender());

        TokenForge1155v3Upgradeable(clone).renounceRole(TokenForge1155v3Upgradeable(clone).DEFAULT_ADMIN_ROLE(), address(this));

        _deployedContracts[_msgSender()].push(address(clone));
        _deployedContractTypes[
            address(clone)
        ] = CONTRACT_TYPE_TokenForge1155v3;
    ...
    /*function createTokenForge721(address signer, string memory baseUri) public returns (TokenForge721) {
        TokenForge721 instance = new TokenForge721(signer, baseUri);

        _deployedContracts[_msgSender()].push(address(instance));
        _deployedContractTypes[address(instance)] = CONTRACT_TYPE_TokenForge721;

        emit ContractDeployed(CONTRACT_TYPE_TokenForge721, address(instance));

        return instance;
    }*/

    /*function createTokenForge721p(address signer, string memory baseUri) public returns (TokenForge721p) {
        TokenForge721p instance = new TokenForge721p(signer, baseUri);

        _deployedContracts[_msgSender()].push(address(instance));
        _deployedContractTypes[address(instance)] = CONTRACT_TYPE_TokenForge721p;

        emit ContractDeployed(CONTRACT_TYPE_TokenForge721p, address(instance));
