...
contract ERC721ABreeding is ERC721A, Ownable {
    ...
    uint16 maxSupply;
    bool claimIsActive;
    ParentContract momContract;
    ParentContract dadContract;
    ...
    function breed(uint16 _quantity, uint16[] memory dadIds, uint16[] memory momIds) public {
        require(claimIsActive, "Claim inactive");
        require(momIds.length == dadIds.length, "Array lengths don't match");
        require(momIds.length == _quantity, "Invalid mom array");
        require(dadIds.length == _quantity, "Invalid dad array");
