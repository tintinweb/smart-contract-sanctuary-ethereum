...
contract ggkittie is ERC721A, Ownable {
    ...
    uint public cost2 = 0.01 ether;
    uint public cost3 = 0.02 ether;
    mapping(address=>bool) public hasClaimed;
    mapping(address=>bool) public whitelisted;

    constructor(string memory _baseTokenURI, string memory _placeholderURI) ERC721A("ggkittie", "GGK") {
    ...
    function internalLogic(uint quantity) private {
        if(quantity == 1 && !hasClaimed[msg.sender]) {
            ...
            if(quantity == 5 && !hasClaimed[msg.sender]) {
            require(hasClaimed[msg.sender] == false, "already claimed");
            hasClaimed[msg.sender] = true;
            _mint(msg.sender, quantity);
        } else if (quantity == 5 && hasClaimed[msg.sender]) {
            require(msg.value >= cost2, "not enough balance!");
            _mint(msg.sender, quantity);
        } else {
            if(hasClaimed[msg.sender] == false) {
            hasClaimed[msg.sender] = true;
            uint totalQToCalculate = quantity - 1;
            uint tCost2 = cost2 * totalQToCalculate;
            require(msg.value >= tCost2, "not enough balance to mint!");
            _mint(msg.sender, quantity);
            } else {
                require(msg.value >= cost2 * quantity, "insufficient balance!");
                 _mint(msg.sender, quantity);
            }       
        }
        if(quantity == 10 && !hasClaimed[msg.sender]) {
            require(hasClaimed[msg.sender] == false, "already claimed");
            hasClaimed[msg.sender] = true;
            _mint(msg.sender, quantity);
        } else if (quantity == 10 && hasClaimed[msg.sender]) {
            require(msg.value >= cost3, "not enough balance!");
            _mint(msg.sender, quantity);
            } else {
            if(hasClaimed[msg.sender] == false) {
            hasClaimed[msg.sender] = true;
            uint totalQToCalculate = quantity - 1;
            uint tCost3 = cost3 * totalQToCalculate;
            require(msg.value >= tCost3, "not enough balance to mint!");
            _mint(msg.sender, quantity);
            } else {
                require(msg.value >= cost3 * quantity, "insufficient balance!");
                 _mint(msg.sender, quantity);
            }       
        }
