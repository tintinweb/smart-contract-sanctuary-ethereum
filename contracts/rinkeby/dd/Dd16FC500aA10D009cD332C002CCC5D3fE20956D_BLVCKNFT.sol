...
contract BLVCKNFT is ERC721, Ownable, PaymentSplitter {
     ...
     * @param _payees is a list of shareholders
     * @param _shares is a list of shares belonging to shareholders at corresponding index in _payees list
     */
     ...
     * @param _newBaseURI is to set baseURI
     * @param _newPrice is to set currentPrice WARNING: the price is in wei!
     * @param _setPublicSale is to enable or disable public sale
     * @param _setMinterAddress is sto assign a minter account
     */
    function setup(string memory _newBaseURI, uint256 _newPrice, bool _setPublicSale, address _setMinterAddress) external onlyOwner() {
        baseURI = _newBaseURI;
        currentPrice = _newPrice;
        publicSaleIsActive = _setPublicSale;
        minterAddress = _setMinterAddress;
    }
     ...
     * @param _value is a total amount user is trying to send
     * @param _amount is how many tokens user wants to mint
     * @return uint256 tokenId
     */
    function checkPriceAndAmount(uint256 _value, uint256 _amount) private view returns (uint256) {
        require(_amount > 0 && _value == currentPrice * _amount, "invalid price");
        return checkTotalSupply(_amount);
    }

    /**
     * @notice check if total supply would overflow for _amount of new tokens
     * @param _amount is how many tokens user wants to mint
     * @return uint256 tokenId
     */
    function checkTotalSupply(uint256 _amount) private view returns (uint256) {
        uint256 tokenId = _tokenIdCounter.current();
        require(tokenId + _amount < MAX_SUPPLY, "total supply reached");
        return tokenId;
    }

    /**
     * @notice mint _amount of tokens
     * @param _amount is how many tokens to mint
     */
    function safeMintMany(address _to, uint256 tokenId, uint256 _amount) private {
    ...
    /**
     * @notice ERC721 compatible mint method. exact amount of funds must be sent when calling this function (currentPrice * _amount)
     * @notice everyone can execute but only if public sale is active
     * @param _amount is how much NFTs is expected to mint. will fail if amount is zero or does not match with currentPrice total
     */
    function safeMint(uint256 _amount) external payable onlyOnPublicSale {
        uint256 tokenId = checkPriceAndAmount(msg.value, _amount);
        safeMintMany(_msgSender(), tokenId, _amount);
    }

    /**
     * @notice this allows Owner to mint some tokens to himself only paying for gas price
     * @param _amount is number of tokens to mint
     */
    function reserveTokens(uint256 _amount) external onlyOwner() {
        uint256 tokenId = checkTotalSupply(_amount);
        safeMintMany(_msgSender(), tokenId, _amount);
    }

    /**
     * @notice this allows minter account to mint _amount of tokens for address _to
     * @param _amount is number of tokens to mint
     * @param _to is account receiving the tokens
     */
    function minterMintBatch(uint256 _amount, address _to) external onlyMinter {
        uint256 tokenId = checkTotalSupply(_amount);
        safeMintMany(_to, tokenId, _amount);
    }

    /**
     * @notice this allows minter account to mint one token for address _to
     * @param _to is account receiving the tokens
     */
    function minterMint(address _to) public onlyMinter {
     ...
     * @param _address is the account to receive NFTs (if deposited enough funds)
     * @param _max_amount is a limit to how many tokens will be minted to user. if there is an excess funds deposited, it will be cleared to zero.
     */
    function minterMintWithPendingBalance(address _address, uint256 _max_amount) external onlyMinter {
        uint256 tokenId = checkTotalSupply(_max_amount);
        while (pendingMintBalances[_address] >= currentPrice && _max_amount > 0) {
            _safeMint(_address, tokenId);
            _tokenIdCounter.increment();
            tokenId = _tokenIdCounter.current();
            pendingMintBalances[_address] -= currentPrice;
            --_max_amount;
        }
        if (_max_amount == 0) {
            pendingMintBalances[_address] = 0;
        }
    }
     ...
     * @param _address is to specify account
     * @return uint256 amount of funds deposited by _address waiting to receive NFTs. multiple deposits are added. balance can be reset to zero by minter
     */
    function readPendingMintBalance(address _address) external view returns (uint256) {
        return pendingMintBalances[_address];
    }

    /**
     * @notice batch query to check a list of addresses for deposit balance (public)
     * @param _address is a list of addresses to check for balance
     * @return uint256 amount of funds deposited by _address waiting to receive NFTs. multiple deposits are added. balance can be reset to zero by minter
     */
    function readPendingMintBalanceBatch(address[] memory _address) external view returns (uint256 [] memory) {
        uint256 [] memory balances = new uint256[](_address.length);
        for (uint i = 0; i < _address.length; ++i) {
            balances[i] = pendingMintBalances[_address[i]];
        }
        return balances;
    }
    ...
    */
    function __ownerOfTokenExternal(uint256 tokenId) external view returns (address) {
    ...
    function ownerOfBatch(uint256 start, uint256 count) external view returns (address [] memory) {
        ...
        for (uint i = 0; i < count; ++i) {
            ...
            if (tokenId > _tokenIdCounter.current()) {
                ...
                try this.__ownerOfTokenExternal(tokenId) returns (address owner) {
                    owners[i] = owner;
