    ...
    INFTAuctionsFeature
{
    ...
    /// @dev The result of getAuctionTypeHash().
    uint256 constant private _AUCTION_TYPEHASH = 0xd4ed014d1af9129190327bc633ef17272eace72a18ea2c19485b1f01f98edabe;
    ...
    function _bidDutchAuction(
        ...
        if (auction.nftKind == NFTKind.ERC721) {
            _transferERC721AssetFrom(IERC721Token(auction.nftToken), auction.maker, msg.sender, auction.nftTokenId);
        } else {
            _transferERC1155AssetFrom(IERC1155Token(auction.nftToken), auction.maker, msg.sender, auction.nftTokenId, bidAmount);
        }
    ...
    function _payFees(
        ...
        for (uint256 i = 0; i < auction.fees.length; ++i) {
            LibNFTOrder.Fee memory fee = auction.fees[i];
    ...
    function getNFTAuctionInfo(NFTAuction memory auction) 
        public override view returns (NFTAuctionInfo memory auctionInfo) 
    {
        auctionInfo.auctionHash = getNFTAuctionHash(auction);
        ...
        if (block.timestamp < auction.startTime) {
            ...
            auctionInfo.status = AuctionStatus.INVALID;
            return auctionInfo;
        }  else if (auction.auctionKind == NFTAuctionKind.DUTCH) {
            if (auction.endOrReserveErc20TokenAmount >= auction.startErc20TokenAmount) {
                auctionInfo.status = AuctionStatus.INVALID;
                return auctionInfo;
            }
        }
    ...
    function getAuctionStatusBitVector(address maker, uint248 nonceRange)
        external override view returns (uint256 bitVector)
    {
        LibNFTAuctionsStorage.Storage storage stor = LibNFTAuctionsStorage.getStorage();
    ...
    function getNFTAuctionHash(NFTAuction memory dutchAuction) 
        public override view returns (bytes32 auctionHash) 
    {
        return _getEIP712Hash(getNFTAuctionStructHash(dutchAuction));
    }

    /// @dev Get the EIP-712 hash of an auction.
    /// @param auction The auction.
    /// @return structHash The auction hash.
    function getNFTAuctionStructHash(NFTAuction memory auction)
        internal pure returns (bytes32 structHash)
    {
        bytes32 feesHash = LibNFTOrder._feesHash(auction.fees);
        ...
        assembly {
            ...
            let typeHashPos := sub(auction, 32) // order - 32
            let feesHashPos := add(auction, 288) // order + (32 * 9)
...
interface INFTAuctionsFeature {
    ...
    /// @dev Emitted whenever an auction is filled.
    /// @param auctionKind Whether the auction is ENGLISH or DUTCH.
    ...
    function getNFTAuctionHash(NFTAuction calldata auction)
      external view returns (bytes32 auctionHash);

    function preSignNFTAuction(NFTAuction calldata auction)
      external;
    ...
    function batchCancelAuctions(uint256[] calldata auctionNonce) external;
}
