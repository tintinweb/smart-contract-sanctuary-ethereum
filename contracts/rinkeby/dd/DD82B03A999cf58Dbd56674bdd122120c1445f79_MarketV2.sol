...
contract MarketV2 is OwnableContract, ReentrancyGuardUpgradeable, IMarketV2 {
    ...
    function _setLending(
        Lending memory slending,
        address originalNftAddress,
        uint256 originalNftId
    ) internal {
        Lending storage lending = lendingMap[slending.doNftAddress][
            slending.doNftId
        ];
        lending.lender = slending.lender;
        lending.doNftAddress = slending.doNftAddress;
        lending.doNftId = slending.doNftId;
        lending.maxEndTime = slending.maxEndTime;
        lending.minDuration = slending.minDuration;
        lending.nonce = uint16(slending.nonce);
    ...
    function createLendOrder(
        ...
        _setLending(lending, originalNftAddress, originalNftId);
    }
        ...
        )
    {
        ...
        totalPrice = (uint256(pricePerDay) * (endTime - startTime + 1)) / 86400;
    ...
    function setBeneficiary(address erc4907NftAddress, address beneficiary)
        public
    {
    ...
    function getBeneficiary(address erc4907NftAddress)
        public
        view
        returns (address)
    {
