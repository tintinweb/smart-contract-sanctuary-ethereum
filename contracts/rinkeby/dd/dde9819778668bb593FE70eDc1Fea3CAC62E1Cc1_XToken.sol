...
interface ILANDXNFT {
    ...
    function culture(uint256 id) external view returns (string memory);
...
contract XToken is
    ...
    Ownable
{
    string public constant culture = "WHEAT";
    ...
    function getShards(uint256 _id) external {
        ...
        require(keccak256(abi.encodePacked(landXNFT.culture(_id))) == keccak256(abi.encodePacked(culture)), "wrong culture");
        require(xTokenRouter.getXToken(culture) == address(this), "tokens are not set for this culture");
        require(
        ...
        uint256 xTokensToInitialDeposit = initialDepositAmount * grainPrices.prices(culture) * grainPrices.getXTokenPrice(xTokenRouter.getXToken(culture));
    ...
    function claim() public {
        address cToken = xTokenRouter.getCToken(culture);
        uint256 yield = calculateYield(Staked[msg.sender]);
        ICTOKEN(cToken).mint(msg.sender,  (Yield[msg.sender] + yield) * (10**uint256(6)));
        Yield[msg.sender] = 0;
    }
    ...
    function preview(uint256 id) public view returns(uint256, uint256, uint256, uint256) {
        ...
        require(xTokenRouter.getXToken(landXNFT.culture(id)) == address(this), "Unable to shard this NFT");
        uint256 shards = (landXNFT.landArea(id) * landXNFT.rent(id) * 1e6) / 10000;
        uint256 fee = _calcPercentage(shards);
        uint256 xTokensToInitialDeposit = getAnnualRentAmount(id) * grainPrices.prices(culture) * grainPrices.getXTokenPrice(xTokenRouter.getXToken(culture));
