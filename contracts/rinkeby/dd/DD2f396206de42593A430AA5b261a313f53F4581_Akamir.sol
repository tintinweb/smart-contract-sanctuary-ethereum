...
contract Akamir is Ownable, ERC721A, ReentrancyGuard {
  ...
  function myTokens() public view returns (TokenOwnership[] memory) {}
...
contract ERC721A is Context, ERC165, IERC721, IERC721Metadata, IERC721Enumerable {
  ...
  function lock(uint256 tokenId, uint256 lockUntil) public {
    ...
    if (_lockUntil[tokenId] != 0 && lockUntil - _lockUntil[tokenId] > ONE_YEAR) revert LockNotUpperThanOneYear();
  ...
  function locks(uint256[] memory tokenIds, uint256 lockUntil) public {
    unchecked {
      for (uint256 i; i < tokenIds.length; i++) {
        ...
        if (_lockUntil[tokenIds[i]] != 0 && lockUntil - _lockUntil[tokenIds[i]] > ONE_YEAR)
