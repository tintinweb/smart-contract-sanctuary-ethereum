...
contract FOO is ERC20, Ownable {
    ...
    /// @notice Contract FOO balance threshold before `_swap` is invoked
    ...
    /// @notice Change the minimum contract FOO balance before `_swap` gets invoked
    ...
    /// @notice Rescue FOO from the treasury amount
    /// @dev Should only be used in an emergency
    /// @param _amount The amount of FOO to rescue
    /// @param _recipient The recipient of the rescued FOO
    ...
    /// @notice Rescue FOO from the reflection amount
    /// @dev Should only be used in an emergency
    /// @param _amount The amount of FOO to rescue
    /// @param _recipient The recipient of the rescued FOO
    ...
    function transferOwnership(address newOwner) public override onlyOwner {
        require(newOwner != address(0) && !isTaxExcluded(newOwner), "Ownable: new owner is the zero address");
        _transferOwnership(newOwner);
    }
    ...
    /// @notice Perform a Uniswap v2 swap from FOO to ETH and handle tax distribution
    /// @param amount The amount of FOO to swap in wei
    ...
    /// @notice Transfers FOO from an account to this contract for taxes
    /// @param _account The account to transfer FOO from
