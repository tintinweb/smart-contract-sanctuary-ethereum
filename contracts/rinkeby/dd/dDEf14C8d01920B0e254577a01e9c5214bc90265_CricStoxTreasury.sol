...
// 
contract CricStoxTreasury is Ownable {

    address public cricStoxMasterAddress;

    constructor(address adminWallet_) {
        transferOwnership(adminWallet_);
    }

    modifier onlyMaster() {
        require(cricStoxMasterAddress == _msgSender(), "Callable only by Master");
        _;
    }
    ...
    function withdrawTokens(address token_, uint256 amount_, address recipient_) external onlyOwner {
        // some logic for checking withdrawable amount
        IERC20 token = IERC20(token_);
