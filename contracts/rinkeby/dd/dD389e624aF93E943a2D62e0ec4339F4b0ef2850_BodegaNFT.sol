    ...
    ReentrancyGuard
{
    ...
    function startPresaleForAirdrop(address[] memory recipients) public onlyOwner {
        ...
        for (uint256 i = 0; i < recipients.length; i++) {
          airdropBodega(1, recipients[i]);
        }
        presaleM = true;
        privateM = false;
    }

     function airdropBodega(uint256 numNFTs, address recipient) public onlyOwner {
        require(totalSupply() + numNFTs <= maxSupply, 'Sale would exceed max supply');
