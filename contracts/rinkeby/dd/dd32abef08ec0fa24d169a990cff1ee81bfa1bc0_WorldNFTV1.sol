...
contract WorldNFTV1 is BaseNFT {
    constructor(ICRS _crs, bytes32 _baseNode) ERC721("VeVerse World v1", "WORLD") {
        crs = _crs;
        baseNode = _baseNode;
    }
    ...
    function tokenURI(uint256 _id) public override(BaseNFT) view returns (string memory) {
        ...
        if (_resolver != address(0)) _customAvatar = ITextResolver(_resolver).text(_nodehash, "VEVERSE_WORLD_IMAGE");
