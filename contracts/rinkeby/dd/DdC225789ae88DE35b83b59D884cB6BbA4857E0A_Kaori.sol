...
contract Kaori is ERC721A, Ownable {
    ...
    uint256 private maxMintAmountPerTx = 2;
    uint256 private maxMintAmountPerWallet = 2;
    ...
    // if can, set HiddenURL and hashRoot in contract before deploy, keep constructor cleaner.
    ...
    function whitelistMint(uint256 _mintAmount, bytes32[] calldata _merkleProof) external payable {
        ...
        require(MerkleProof.verify(_merkleProof, hashRoot, leaf),  "You are not whitelisted!");

        uint256 mintedByWallet = _getAux(msg.sender) + _mintAmount;
        require(mintedByWallet <= maxMintAmountPerWallet, "Max mint per wallet exceeded!");
    ...
    function setMaxSupply(uint256 _newSupply) external onlyOwner { //can remove if doesn't need
        maxSupply = _newSupply;
    }

    function setbaseURL(string memory uri) external onlyOwner { //change to public if constructor need to call
        baseURL = uri;
    }

    function setHashRoot(bytes32 hp) external onlyOwner { //change to public if constructor need to call
        hashRoot = hp;
    }
    ...
    function withdraw() external onlyOwner {
        ...
        (bool os, ) = payable(msg.sender).call{value: CurrentContractBalance}("");
        require(os);
    }
...
contract ERC721A is Context, ERC165, IERC721, IERC721Metadata {
     ...
     */
    function _getAux(address owner) internal view returns (uint256) {
