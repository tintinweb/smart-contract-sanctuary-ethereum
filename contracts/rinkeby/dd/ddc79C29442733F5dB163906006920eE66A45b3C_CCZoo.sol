...
// This version does text using a custom pixel font and submits spe
...
contract CCZoo is ERC721A, Ownable {
    using Strings for uint256;
    using DynamicBuffer for bytes;

    uint256 public constant MAX_SUPPLY = 3000;
    uint256 public constant MINT_PRICE = 50000000; //0.005 ETH SETTTT; 
    uint256 public constant MAX_WALLET = 100;
    uint256 public constant MAX_TX = 100;
    bool public mintStarted = false;
    uint8 constant NUM_LAYERS = 4;
    uint256 constant TRAIT_TOTAL = 99;
    ...
    uint16[][NUM_LAYERS] private TIERS;//2 is numb layers
    string[] private LAYER_NAMES = ["Palette", "Background", "Hat", "Head"];//, "accessory"];

    //some sort of mapping for the traits
    mapping(uint256 => Trait) traits;
    mapping(uint256 => string[]) traitNames; //could also do uint256
    Animal[] animals;

    //mapping for the metadata
    mapping(uint256 => uint256) metadata;
    mapping(uint256 => uint8) tokenAnimal;

    error animalDoesntExist();
    error MaxSupplyReached();
    error mintNotStarted();
    error maxMintsPerWallet();
    error sendRightAmount();
    error tokenDoesntExists();
    error notOwnerOfAnimal();
    error onlyWalletTx();

    constructor() ERC721A("CCZoo", "CCZ") {
        //set all rarity in constructor
        TIERS[0] = [100];
        TIERS[1] = [100];
        TIERS[2] = [100]; 
        TIERS[3] = [100];
    ...
    function enableMint(bool start) external onlyOwner {
      mintStarted = start;
    }
    ...
    function mint(uint256 quantity) external payable {
        ...
        if((balanceOf(msg.sender) + quantity) > MAX_WALLET) revert maxMintsPerWallet();
        if(quantity * MINT_PRICE != msg.value) revert sendRightAmount();
        if(msg.sender != tx.origin) revert onlyWalletTx();
        //add check so cntracts dont mint

        //Set metadata for tokens
        uint256 startTokenId = _nextTokenId();
        for(uint256 i = startTokenId ; i < startTokenId+quantity; i++) {
            metadata[i] = uint256(keccak256(abi.encodePacked(i,msg.sender,block.difficulty,block.timestamp)));
        }
        //mint
        _safeMint(msg.sender, quantity);
    }

    function getMetadataHash(uint256 tokenId) public view returns(uint256 ) {
    ...
    function addAnimal(string memory name, bytes memory _data, uint256 destLen) external onlyOwner {
        animals.push(Animal(name, SSTORE2.write(_data), destLen));
    }
    ...
    function setAnimal(uint256 tokenId, uint8 animal) public {
        if(ownerOf(tokenId) != msg.sender) revert notOwnerOfAnimal();
    ...
    function tokenURI(uint256 tokenId) override public view returns (string memory) {
        if(tokenId > totalSupply()) revert tokenDoesntExists();

        //We want to process the metadata hash and get the right components (steal from XX)
        //and then we render every component seperately 
        //The render function for each trait is the same 
        //we save all the data in the encoded SSTORE2 stored data this included
        // Location to render, length of the decoded, of the trait and then we pass the right trait to
        //render trait
    ...
    function getTokenTraits(uint256 tokenId) public view returns(uint256[NUM_LAYERS] memory tokenTraits) {
        uint16[NUM_LAYERS] memory dna = splitNumber(metadata[tokenId]);
    ...
    function _getTokenSVG(uint256[NUM_LAYERS] memory tokenTraits, uint256 tokenId, bytes memory buffer) public view {
        //This only returns the picture
        
        buffer.appendSafe(bytes('<svg xmlns="http://www.w3.org/2000/svg" shape-rendering="crispEdges" version="1.1" viewBox="0 0 24 24" width="240" height="240"> '));
        
        //render background

        _renderTrait(traits[1], tokenTraits[1], buffer);


        //render animal
        _renderAnimal(animals[tokenAnimal[tokenId]], buffer);

        for(uint256 i = 2; i < tokenTraits.length; i++) {
            _renderTrait(traits[i], tokenTraits[i], buffer);
        }
        buffer.appendSafe('</svg>');
    }

    function _renderTrait(Trait memory _currentTrait, uint256 _currentTokenTrait, bytes memory buffer) private view {
        (, bytes memory _toDecode) = inflateLib.puff(SSTORE2.read(_currentTrait.data), _currentTrait.destLen);
        (bytes[] memory _traitData, uint8[2][] memory _loc) = abi.decode(_toDecode, (bytes[], uint8[2][]));
        buffer.appendSafe(gfx.drawPixelsUnsafeWithOffset(_traitData[_currentTokenTrait], _loc[_currentTokenTrait][0], _loc[_currentTokenTrait][1]));
    }

    function _renderAnimal(Animal memory _currentAnimal, bytes memory buffer) private view {
        (, bytes memory _toDecode) = inflateLib.puff(SSTORE2.read(_currentAnimal.data), _currentAnimal.destLen);
        (bytes memory _animalData, uint8[2] memory _loc) = abi.decode(_toDecode, (bytes, uint8[2]));
        buffer.appendSafe(gfx.drawPixelsUnsafeWithOffset(_animalData, _loc[0], _loc[1]));
    }
    ...
    function getTokenSVGWithAnimal(uint256 tokenId, string memory name)  public view returns (string memory) {
        ...
        //render animal
        _renderAnimal(_getAnimal(name), buffer);
...
interface IExquisiteGraphics {
  ...
  /// @param data Binary data in the .xqst format.
  /// @return string the <rect> elements
  function drawPixelsUnsafeWithOffset(bytes memory data, uint8 xOffset, uint8 yOffset)
    external
    pure
