...
contract NftMintState is ERC721, ERC721Enumerable, Ownable, ReentrancyGuard {
    ...
    constructor(
        ...
        //mintPriceWhitelist = mintPriceWhitelist_;
        
    }


    modifier atState(MintState _state) {
        require(!isPaused, "The contract is paused");
        require(state == _state, "This action is not allowed in this status");
        _;
    }
    ...
    function addAddressToWhitelistAddress(address[] calldata _addresses) 
        onlyOwner
        atState(MintState.INITIATED)        
        external   
    {
    ...
    function whiteMint(uint256 amount) 
        atState(MintState.WHITEMINT)
        validateMint(amount)
        external 
        payable { 
            // require(block.timestamp - lastActivity > 1 minutes, "WhiteList not active yet");        
            ...
            // require(whiteListedAddress[msg.sender].allowedMints != 0, "cant mint more");            
            require(mintPriceWhitelist * amount <= msg.value, "insuficient fonds");            
    ...
    event StateChanged(MintState indexed, uint256 time );
