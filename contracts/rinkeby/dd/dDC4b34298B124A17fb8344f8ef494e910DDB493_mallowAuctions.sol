...
contract mallowAuctions is Ownable{
    ...
    function getXAuctions(uint256 _start, uint256 _x) public view returns 
    (Auction[] memory, Auction[] memory, Auction[] memory){
        ...
        Auction[] memory Xopened = new Auction[](_x);
        Auction[] memory Xclosed = new Auction[](_x);
        uint t = 0;
        for(uint256 i = _start; i < _x + _start; i++){
            if(i >= opened.length){
                break;
            }
            Xopened[t] = opened[i];
        ...
        return (Xopened, Xclosed, upcoming);
