...
contract NFT is ERC721, ReentrancyGuard, Signable {
    ...
    modifier phaseRequired(Phase phase_) {
        if (phase_ != _phase) revert MintNotAvailable();
        _;
    }
    ...
    function setMintPrice(uint256 mintPrice_) external onlyOwner {
        ...
        if (mintPrice != 0.3 ether) revert MintPriceAlreadyUpdated();
    ...
    function _mintLogic() private {
        if (msg.sender.code.length > 0) revert ContractCantMint();
        if (_nextTokenCount > maxSupply) revert SupplyLimitReached();
        if (minted[msg.sender]) revert AccountAlreadyMintedMax();
...
abstract contract ReentrancyGuard {
    ...
    modifier lock() {
        if (unlocked == 0) revert ContractLocked();
