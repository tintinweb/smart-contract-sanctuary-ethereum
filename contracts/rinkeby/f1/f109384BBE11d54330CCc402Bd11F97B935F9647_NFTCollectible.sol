...
contract NFTCollectible is ERC721, VRFConsumerBase {
    uint256 public tokenCounter;
    bytes32 public keyhash;
    uint256 public fee;
    // created type breed
    // removed enum breed because dont care about what type NFT is. It is based off number rather than classification. TokenId will name NFT.
    //    enum Breed {
    //        PUG,
    //        SHIBA_INU,
    //        ST_BERNARD
    ...
    // VRF coordinator ensures number is random, link token is token used to make payment for using oracle
    constructor(
    ...
    // create random keyhash for collectible
    function createCollectible() public returns (bytes32) {
        // requesting random number using keyhash and fee specific to network
        bytes32 requestId = requestRandomness(keyhash, fee);
        requestIdToSender[requestId] = msg.sender;
        emit requestedCollectible(requestId, msg.sender);
    }

    //return part of request and return, returnid is the number of request we want returned and its going to return number randomNumber
    function fulfillRandomness(bytes32 requestId, uint256 randomNumber)
        internal
        override
    {
        // type breed, define breed by a random number out of 3 breeds
        // no need for type, numerical convention
        //        Breed breed = Breed(randomNumber % 3);
        uint256 newTokenId = randomNumber % 100;
