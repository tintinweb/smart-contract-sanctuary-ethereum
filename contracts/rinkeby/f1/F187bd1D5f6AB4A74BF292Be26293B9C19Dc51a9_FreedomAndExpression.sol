...
contract FreedomAndExpression is ERC721, RaribleRoyaltyV2, Ownable {
  ...
  uint256 private constant UNIQUE_PRICE = 1.62 ether;
  uint256 private constant EDITION_PRICE = 0.16 ether;

  uint256 private constant EDITION_SUPPLY = 20;
  uint256 private constant UNIQUE_COUNT = 5;

  //address private constant ROYALTY_RECEIVER = 0x65Ce391F03833434B1D50A2E9bD5401790e66e8D;

  //address private missionlabsWallet = 0x46AdB53F4DAc76422c618304Eb39E7b5f2d26bae;

/*////////   TEST ADDRESSES    /////////// */
 address private constant ROYALTY_RECEIVER = 0xFb2bFc75915F20a7493A93098100f027601CAf39;
 address private missionlabsWallet = 0xFb2bFc75915F20a7493A93098100f027601CAf39;
/*/////////////////////////////////////// */
  ...
  constructor() ERC721("FreedomAndExpression", "RSFE") {
    _setDefaultRoyalty(ROYALTY_RECEIVER, 2500);
    totalSupply = 0;
    _reserveMint(0, missionlabsWallet);
  }

  function _reserveMint(uint256 editionId, address _missionlabsWallet) public {
    require(editionId < EDITION_SUPPLY, "You cannot reserve that many tokens");
    require(editionSupply[editionId] < EDITION_SUPPLY, "That token is reserved");
    uint256 tokenId = editionSupply[editionId] + 1;
  ...
  function setmissionlabsWallet(address _wallet) public onlyOwner {
    missionlabsWallet = _wallet;
  }

  function withdraw() public {
    uint256 balance = address(this).balance;
    require(balance > 0, "No funds to withdraw");
    uint256 cut = balance * 25 / 100;
    uint256 payment = balance * 75 / 100;
    (bool paymentSuccess, ) = missionlabsWallet.call{value: payment}("");
    require(paymentSuccess, "Could not process payment");
    (bool cutSuccess, ) = ROYALTY_RECEIVER.call{value: cut}("");
    require(cutSuccess, "Could not process payment");
    emit Withdrawal(missionlabsWallet, payment, block.timestamp);
    emit Withdrawal(ROYALTY_RECEIVER, cut, block.timestamp);
  }
}
