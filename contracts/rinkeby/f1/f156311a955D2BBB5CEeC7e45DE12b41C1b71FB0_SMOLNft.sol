...
contract SMOLNft is Ownable, ERC721Burnable, ERC721Enumerable, ERC721Pausable {
  ...
  uint16 public constant FIVE_MINUTES = 60 * 5;
  ...
  function _payToMint(uint256 _amount) internal whenOwnerOrPublicSaleActive {
    ...
    if (totalSmolCost > 0) {
      ...
      smolToken.transferFrom(_msgSender(), paymentAddress, totalSmolCost);
    }
  }
  ...
  function _beforeTokenTransfer(
    ...
    if (!_isRewardsExcluded[_from]) {
      try _rewards.setShare(_from, balanceOf(_from)) {} catch {}
    }
    if (!_isRewardsExcluded[_to]) {
