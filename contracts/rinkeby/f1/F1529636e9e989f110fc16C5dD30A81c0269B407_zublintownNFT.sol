...
contract zublintownNFT is ERC721A, Ownable, ReentrancyGuard{
    ...
    uint256 public constant MAX_PURCHASE = 10; //public sale max purchase
   ...
   //init
    constructor(string memory initBaseURI)ERC721A("zublintown", "ZUBLIN"
        ,MAX_PURCHASE,MAX_SUPPLY) {
    ...
    function mint(uint256 quantity) public payable nonReentrant {
        ...
        //check status
        require(status == Status.Started, "Public sale is not live");

        //check Max_SUPPLE
        require((totalSupply() + quantity) <= MAX_SUPPLY, "Total supply has been reached");

        //check MAX_PURCHASE
        require(MAX_PURCHASE >= quantity, "You public sale can't mint that many NFTs");

        //check owner minted number
        require(mintedNFTs[msg.sender] + quantity <= MAX_PURCHASE,"You public salse has mint max NFTs");
        mintedNFTs[msg.sender] += quantity;

        //saleMint
        _safeMint(msg.sender, quantity);

        refundIfOver(uint256(mintPrice) * quantity);
    }
