...
// ToDo. Check https://www.erc721a.org/ if we want mint multiple tokens at the same time.
contract NFT721 is ERC721, Ownable {
    ...
    // ToDo. Do we want to be able to pause the sale?

    mapping(address => bool) private whitelistMembers;

    constructor(address[] memory _whitelistMembers) ERC721("NFT721", "N721") {
    ...
    function _baseURI() internal pure virtual override returns (string memory) {
        return "ipfs://bafybeidl2pvyf34rqmsdtopejnjorbwa4vx2mg3cz4so5eubbt7wu3bm5m/";
    }
    ...
    function mint(uint _quantity) external payable {
        ...
        require(balanceOf(msg.sender) + _quantity <= MAX_MINTS, "Max mint per address reached");
        require(_tokenIdCounter.current() + _quantity <= TOTAL_SUPPLY, "Reached total supply");
    ...
    function whitelistMint() external payable {
        ...
        require(_tokenIdCounter.current() < TOTAL_SUPPLY, "Reached total supply");
