...
contract ClearingHouse is OwnerPausableUpgradeSafe, ReentrancyGuardUpgradeable, BlockContext {
    ...
    function setTollPool(address _tollPool) external onlyOwner {
        feePool = IMultiTokenRewardRecipient(_tollPool);
    }
    ...
    function _transferFee(
        ...
        if (_tollFee > 0) {
            require(address(feePool) != address(0), "Invalid"); //Invalid tollPool
            quoteAsset.safeTransferFrom(_from, address(feePool), _tollFee);
        }
    }
