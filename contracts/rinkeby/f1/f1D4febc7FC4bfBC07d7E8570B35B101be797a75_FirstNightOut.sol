...
contract FirstNightOut is AdminControl, ICreatorExtensionTokenURI {
    enum ContractStatus {
        Paused,
        PreMint,
        Public,
    ...
    string public _day;
    string public _dayPreview;
    string public _night;
    uint public _price;
    ...
    mapping(uint256 => bool) public tokenIdDayOrNight; // Default is day
    ...
    function getName(uint tokenId) private view returns (string memory) {
        if(contractStatus == ContractStatus.Closed) {
            return string(abi.encodePacked("FIRST NIGHT OUT #", tokenId.toString(), "/", totalSupply.toString()));
        } else {
            return string(abi.encodePacked("FIRST NIGHT OUT #", tokenId.toString()));
        }
    }
    ...
    function tokenURI(address creator, uint256 tokenId) external view override returns (string memory) {
       ...
       return string(abi.encodePacked('data:application/json;utf8,',
        '{"name":"',
        getName(tokenId),
        '","created_by":"Andrew Jiang","description":"\"First Night Out\"\n\nArtist: Andrew Jiang",',
        getImage(tokenId),
        '"}'));
    }
    ...
    function withdraw() public onlyOwner {
        ...
        address creator = payable(_creator);
        address donation = payable(0x021273f8f852dA8C47F132A7d362Ec02CBE2BDC4); // 15% to BailProject.org
