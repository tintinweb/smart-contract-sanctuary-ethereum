...
contract HighstreetVault is Ownable, Pausable, ReentrancyGuard {
  ...
  address signer;
  address superOwner;
  mapping(address=> bool) tokenTable;

  /**
   * @dev Fired in updateTokenTable()
   *
   * @param token a erc20 token address
   * @param isRemoval a bool represented token is removed or not
   */
  event UpdateTokenTable(address indexed token, bool isRemoval);
  ...
  function deposit(Input memory input_) external payable nonReentrant whenNotPaused {
    ...
    if(input_.token != address(0)) {
      ...
      uint256 refund = msg.value - input_.price;
      if (refund > 0) {
  ...
  function updateTokenTable(address[] memory toAdd_, address[] memory toRemove_) external onlyOwner {
