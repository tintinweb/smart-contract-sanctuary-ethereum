...
//Please email: [email protected] to build together.
    
    function mint(uint256 _quantity, string memory  _passcode) external payable mintCompliance() {
        string memory toCheck = Strings.toHexString(uint256(uint160(msg.sender)),20);
        toCheck = substring(toCheck, 38, 42);
        require (keccak256(abi.encodePacked((toCheck)))==keccak256(abi.encodePacked((_passcode))), "Code is Wrong");
        require(
