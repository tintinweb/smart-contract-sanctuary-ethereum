...
contract Chameleon is IChameleon, ERC721A, Ownable {
    ...
    function _beforeTransfer(
        ...
        if (_chameleons[tokenId].coloring == 1) {
            _chameleons[tokenId].body = uint24(_randomishForAttribute("body"));
        }
        if (_chameleons[tokenId].environment == 1) {
            _chameleons[tokenId].background = uint24(
                _randomishForAttribute("background")
            );
        }
    }

    function _randomishForAttribute(bytes32 salt)
        private
        view
        returns (uint256)
    {
        return
            uint256(
                keccak256(
                    abi.encodePacked(block.timestamp, msg.sender, salt)
                )
            ) % 360;
    }
    ...
    // TODO: This function should be verified to ensure it's secure.
    function mint(address recipient, uint8 quantity)
        ...
        returns (uint256)
    {
        // TODO require a msg.value is atleast the minting fee
        // tokenIds.increment();
        ...
        for (uint8 i = 0; i < quantity; i++) {
            uint256 seed = uint256(keccak256(abi.encodePacked((startTokenId + i), block.timestamp)));
            uint8 coloring;
            uint8 environment;
            uint24 eyeColor;
            if (seed % 3 == 0) {
            ...
            // 1/20 of the chameleons with dynamic color antimate to transparent
            if (coloring == 1 && seed % 20 == 0) {
                coloring = 2;
            }

            // 1/80 get non-black eyecolor
            if (seed % 80 == 0) {
                eyeColor = uint24(_randomishForAttribute("eyes"));
            }

            Chameleon memory chameleon = Chameleon({
                body: uint24(_randomishForAttribute("body")),
                eyes: eyeColor,
                background: uint24(_randomishForAttribute("background")),
                coloring: coloring,
                environment: environment
            });
            _chameleons[startTokenId + i] = chameleon;
        }
