...
contract Marketplace is ReentrancyGuard, Ownable, EIP712 {
    ...
    IProtocolFeeRegistry public protocolFeeRegistry;
    ...
    event NewProtocolFeeRegistry(address indexed protocolFeeRegistry);
    ...
    function matchAskWithTakerBidUsingETHAndWETH(
        ...
        uint256 price = takerBid.price;
        
        // If not enough ETH to cover the price, use WETH
        if (price > msg.value) {
            IERC20(WETH).safeTransferFrom(msg.sender, address(this), (price - msg.value));
        } else {
            require(price == msg.value, "Order: Msg.value too high");
        }
        ...
        _validateMakerOrder(makerAsk, askHash);

        // Check the taker bid order
        bytes32 bidHash = takerBid.hashTaker();
        _validateTakerOrder(takerBid, bidHash);
        ...
        // Execution part 2/2
        _transferNonFungibleToken(makerAsk.collection, makerAsk.maker, takerBid.taker, tokenId, amount);
        ...
        nonReentrant
    {
        ...
        // Execution part 1/2
        uint256 price = takerBid.price;
        uint256 membership = takerBid.membership;
        ...
        nonReentrant
    {
        ...
        _validateMakerOrder(makerBid, bidHash);

        // Check the taker ask order
        bytes32 askHash = takerAsk.hashTaker();
        _validateTakerOrder(takerAsk, askHash);
        ...
        // Execution part 1/2
        _transferNonFungibleToken(makerBid.collection, msg.sender, makerBid.maker, tokenId, amount);

        // Execution part 2/2
        uint256 price = takerAsk.price;
        uint256 membership = takerAsk.membership;
        uint256 minPercentageToAsk = takerAsk.minPercentageToAsk;
    ...
    /**
     * @notice Update protocol fee registry
     * @param _protocolFeeRegistry new registry
     */
    function updateProtocolFeeRegistry(address _protocolFeeRegistry) external onlyOwner {
        require(_protocolFeeRegistry != address(0), "Owner: Cannot be null address");
        protocolFeeRegistry = IProtocolFeeRegistry(_protocolFeeRegistry);
        emit NewProtocolFeeRegistry(_protocolFeeRegistry);
    }
    ...
    function _transferFeesAndFunds(
        ...
        // 1. Protocol fee
        {
            uint256 protocolFeeAmount = _calculateProtocolFee(strategy, currency, membership, amount);
    ...
    function _transferFeesAndFundsWithWETH(
        ...
        // 1. Protocol fee
        {
            uint256 protocolFeeAmount = _calculateProtocolFee(strategy, WETH, membership, amount);
    ...
    function _calculateProtocolFee(
        ...
        uint256 protocolFee = protocolFeeRegistry.getFee(executionStrategy, currency, membership);
     ...
     */
    function _validateMakerOrder(OrderTypes.MakerOrder calldata makerOrder, bytes32 orderHash) internal view {
    ...
    /**
     * @notice Verify the validity of the taker order
     * @param takerOrder taker order
     * @param orderHash computed hash for the order
     */
    function _validateTakerOrder(OrderTypes.TakerOrder calldata takerOrder, bytes32 orderHash) internal view {
        // Verify the validity of the signature
        require(
            SignatureChecker.verify(
                orderHash,
 ...
 */
library OrderTypes {
    bytes32 internal constant MAKER_ORDER_HASH = keccak256("MakerOrder(bool isOrderAsk,address maker,address collection,uint256 price,uint256 tokenId,uint256 amount,address strategy,address currency,uint256 nonce,uint256 startTime,uint256 endTime,uint256 minPercentageToAsk)");
    bytes32 internal constant TAKER_ORDER_HASH = keccak256("TakerOrder(bool isOrderAsk,address taker,uint256 price,uint256 tokenId,uint256 endTime,uint256 minPercentageToAsk,uint256 membership)");
    ...
    struct MakerOrder {
        ...
        address strategy; // allowed strategies for the item
...
library SignatureChecker {
    ...
    function verify(
        ...
        bytes32 signedMessageHash = ECDSA.toEthSignedMessageHash(digest);
        return recover(signedMessageHash, v, r, s) == signer;
    }
}
