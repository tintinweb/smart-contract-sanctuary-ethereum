...
/**
  @notice 
  This contract manages the Dali ecosystem, serves as a payment terminal, and custodies all funds.
...
contract TerminalV1_1 is Operatable, ITerminalV1_1, ITerminal, ReentrancyGuard, Ownable {
  ...
  address public daliToken = address(0x452bD47267444237d7C2B27aA1961643D59Bebb7);
  ...
  /// @notice The percent fee the Dali project takes from tapped amounts. Out of 200.
  uint256 public override fee = 10;

  // dali fee project
  uint256 public feeProjectId = 20;
  ...
  */
  ...
  function pay(
    ...
    require(IERC20(daliToken).balanceOf(msg.sender) >= _amount, "Not enough dali tokens in the wallet");
    // Positive payments only.
    require(_amount > 0, 'TV1_1::pay: BAD_AMOUNT');

    // Cant send tickets to the zero address.
    require(_beneficiary != address(0), 'TV1_1::pay: ZERO_ADDRESS');

    return _pay(_projectId, _amount, _beneficiary, _memo, _preferUnstakedTickets);
  }
  ...
  function tap(
    ...
    if (_leftoverTransferAmount > 0) IERC20(daliToken).transfer(_projectOwner, _leftoverTransferAmount);
    ...
    returns (uint256 amount)
  {
    ...
    if (amount > 0)
      ...
      IERC20(daliToken).transfer(_beneficiary, amount);
    ...
    for (uint256 _i = 0; _i < _mods.length; _i++) {
      ...
      if (_modCut > 0) {
        ...
        if (_mod.allocator != IModAllocator(address(0))) {
          ...
          // Get a reference to the Dali terminal being used.
          ...
          IERC20(daliToken).transfer(_mod.beneficiary, _modCut);
        }
      }
  ...
  function _pay(
    ...
    IERC20(daliToken).transfer(address(this), _amount);
  ...
  function _takeFee(
    ...
    if (terminalDirectory.terminalOf(1) == this) {
      ...
      _pay(feeProjectId, feeAmount, _beneficiary, _memo, false);
    } else {
      ...
      _terminal.pay(feeProjectId, _beneficiary, _memo, false, feeAmount);
    }
  }

  function setFeeProject(uint256 _feeProjectId) external onlyOwner {
...
struct FundingCycle {
    ...
    uint256 currency;
    // The percentage of each payment to send as a fee to the Dali admin.
    uint256 fee;
