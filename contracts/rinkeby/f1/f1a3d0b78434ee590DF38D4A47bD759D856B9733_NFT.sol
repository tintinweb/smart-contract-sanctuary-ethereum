...
contract NFT is ERC721Enumerable, Ownable, ReentrancyGuard {
        ...
        internal
    {
        ...
        for (uint256 i = 1; i <= _teamReserve; i++) {
            _safeMint(_teamWallet, supply + i);
        }
    }
    ...
    function mint() external payable nonReentrant {
...

        {
            ...
            mintedNums[msg.sender] = mintedNum + 1;
        }
    ...
    function isAllowListed(address _user) public view returns (bool) {
        address[] memory tempAllowlistUsers = allowlistUsers;
        for (uint256 i = 0; i < tempAllowlistUsers.length; i++) {
            if (tempAllowlistUsers[i] == _user) {
                return true;
            }
        }
        return false;
    ...
    function setAllowlistUsers(address[] calldata _users) external onlyOwner {
        delete allowlistUsers;
        allowlistUsers = _users;
    }
