...
contract Hashmasks is ERC721 {
  ...
  uint256 private _totalSupply = 16370;
  bytes4 private _ERC721_RECEIVED = IERC721Receiver.onERC721Received.selector;

  constructor() ERC721('Qiwihui NFT Token', 'QNT') {
    // Qiwihui NFT Token
  }
  ...
  function _safeMint(address to, uint256 tokenId) internal virtual override {
    ...
    require(!_exists(tokenId), 'ERC721: token already minted');
    _mint(to, tokenId); // mint NFT and assign it to address to
    bytes memory _data = bytes('0x');
    _totalSupply++; // increment totalSupply() by one
    // console.log("totalSuppply: ", _totalSupply);
    if (to.isContract()) {
      ...
      bytes4 retval = IERC721Receiver(to).onERC721Received(to, address(0), tokenId, _data);
