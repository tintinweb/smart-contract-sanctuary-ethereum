...
contract LoanServicePayment is Ownable {
    ...
    function payments(uint256 _paymentId) public view returns(LoanCommon.Payment memory) {
        address loanStoragePaymentAddress = LoanOracle_I(getLoanOracleAddress()).getLoanStoragePaymentAddress();
        return LoanStoragePayment_I(loanStoragePaymentAddress).payments(_paymentId);
    }

    /// @notice get a list of payment IDs by loan ID
    /// @param _loanId The Loan ID
    function paymentsByLoanId(uint256 _loanId) public view returns(uint256[] memory) {
        address loanStoragePaymentAddress = LoanOracle_I(getLoanOracleAddress()).getLoanStoragePaymentAddress();
        return LoanStoragePayment_I(loanStoragePaymentAddress).paymentsByLoanId(_loanId);
    }

    /// @notice list of offers by loan id
    /// @notice _loanId The loan id
    function listByLoanId(uint256 _loanId) public view returns(LoanCommon.Payment[] memory) {
        uint256[] memory paymentIds = paymentsByLoanId(_loanId);
        LoanCommon.Payment[] memory paymentList = new LoanCommon.Payment[](paymentIds.length);
    ...
    function makePayment(uint256 _loanId) public payable {
        ...
        require(loan.loanId != 0, "loan must exists");
        require (loan.status == LoanCommon.LoanStatus.ACTIVE, "Loan must be in an active status");
        ...
        uint256 payoff = payoffAmount(_loanId);
        require(msg.value <= payoff, "eth exceeds payoff amount");

        /// Create payment record
        address loanStoragePaymentAddress = LoanOracle_I(getLoanOracleAddress()).getLoanStoragePaymentAddress();
        LoanStoragePayment_I(loanStoragePaymentAddress).create(msg.sender, _loanId, msg.value);

        // Is paying off in full
        if (msg.value == payoff) {
            LoanService_I(loanServiceAddress).updateStatus(_loanId, LoanCommon.LoanStatus.PAID);
        }
    ...
    function isLoanInDefault(uint256 _loanId) public view returns (bool) {
        ...
        address loanServiceAddress = LoanOracle_I(getLoanOracleAddress()).getLoanServiceAddress();
        LoanCommon.Loan memory loan = LoanService_I(loanServiceAddress).loans(_loanId);
        uint256 currentDurationOfLoan = calculateDurationInDays(loan.activationDate, block.timestamp);
        address loanParametersAddress = LoanOracle_I(getLoanOracleAddress()).getLoanParametersAddress();
        return currentDurationOfLoan > periodInDays(loan.duration).add(
                periodInMinutes(LoanParameters_I(loanParametersAddress).loanDefaultGracePeriodInMinutes()
            )
        );
    }
    ...
    function payoffAmount(uint256 _loanId) public view returns (uint256) {
        ...
        // Calculate the yearly yield
        uint256 yearlyCalc = loan.calculatedAmount.mul(loan.calculatedInterest);

        // Calculate the daily yield
        uint256 dailyCalc = yearlyCalc.div(365);

        // Calculate the yield for the duration of the loan
        uint256 durationCalc = dailyCalc.mul(loan.duration);

        // Calculate payoff before any payment
        uint256 payoff = loan.calculatedAmount.add(
            // added a multiplier of 10 because we store calculated interest with five digits
            // the precision is setup for 4
            durationCalc.div(LoanParameters_I(loanParametersAddress).precision().mul(10))
        );

        // Subtract payments from amounts
        LoanCommon.Payment[] memory loanPayments = listByLoanId(_loanId);
        for (uint256 i = 0; i < loanPayments.length; i++) {
            payoff = payoff.sub(loanPayments[i].amount);
        }

        return payoff;
    }

    /// @notice convenient method to get a list of payments
    /// @param _start The starting index
    /// @param _end The ending index
    function list(uint256 _start, uint256 _end) public view returns(LoanCommon.Payment[] memory) {
        require(_end >= _start, "The end index must be greater than or equal to the start index");
        LoanCommon.Payment[] memory paymentList = new LoanCommon.Payment[](_end.sub(_start).add(1));

        uint256 index = 0;
        for (uint256 i = _start; i <= _end; i++) {
            paymentList[index] = payments(i);
            index++;
        }
