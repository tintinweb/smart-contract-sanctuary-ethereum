...
import "../BiletsClub.sol";
import "../BiletsClubCharStatus.sol";
import "../BiletsClubToken.sol";

/**
 * @title BiletsClubHealthPotion Contract
 */

contract BiletsClubHealthPotion is Ownable, ReentrancyGuard {
    string public name = "Bilets Club Health Potion";
    string public symbol = "BLT-HP";

    uint256 public HuntPrice = 0.000 ether;
    uint256 public HuntDamage = 10;
    uint256 public HuntExpEarn = 10;
    uint256 public HuntTokenEarn = 10;

    bool public pause = false;
    mapping(uint256 => uint256) public atackTime;
    mapping(address => bool) public admins;
    BiletsClub public contractBiletsClub;
    BiletsClubCharStatus public contractBiletsClubCharStatus;
    BiletsClubToken public contractBiletsClubToken;
    ...
    constructor(
        BiletsClub _contractBiletsClub,
        BiletsClubCharStatus _contractBiletsClubCharStatus,
        BiletsClubToken _contractBiletsClubToken
    ) {
        contractBiletsClub = _contractBiletsClub;
        contractBiletsClubCharStatus = _contractBiletsClubCharStatus;
        contractBiletsClubToken = _contractBiletsClubToken;
        admins[msg.sender] = true;
    }

    /******************** ADMINS ********************/
    function huntBLT(uint256 tokenID) external payable nonReentrant isPaused {
        require(msg.value >= HuntPrice, "Not enough ether.");
        require(
            msg.sender == contractBiletsClub.ownerOf(tokenID),
            "You do not own this NFT."
        );
        require(
            atackTime[tokenID] <= (block.timestamp + (2 * 60)),
            "You need to wait for the next hunt."
        );
        require(
            contractBiletsClubCharStatus.getTokenHealth(tokenID) >= HuntDamage,
            "You do not have enough health."
        );
        // Last Atack
        atackTime[tokenID] = block.timestamp;
        // earn?, tokenID, amountExp, amountHp, amountMana
        contractBiletsClubCharStatus.changeStatus(
            true,
            tokenID,
            HuntExpEarn,
            0,
        ...
        // Token Earn
        contractBiletsClubToken.mint(msg.sender, 50);
    }
...
/**
 * @title BiletsClub Contract
 * @dev Extends ERC721 Non-Fungible Token Standard basic implementation
 */

contract BiletsClub is ERC721A, Ownable {
    ...
    string public contractURI =
        "https://bilets.club/api/opensea_contracts_meta/contractClubMeta";
    ...
    /******************** OWNER ********************/
    function mint500ForBilet() external onlyOwner {
...
/**
 * @title BiletsClubToken Contract
 * @dev Extends ERC20 Standard basic implementation
 */

contract BiletsClubToken is ERC20, Ownable, ReentrancyGuard {
    bool public pause = false;
    mapping(address => bool) public admins;
    ...
    constructor() ERC20("Bilets Club Token", "BLT-Token") {
        admins[msg.sender] = true;
    }
