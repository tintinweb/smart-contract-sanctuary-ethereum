...
import "./LockableOwnableUpgradeable.sol";
import "@openzeppelin/contracts-upgradeable/token/ERC20/IERC20Upgradeable.sol";
import "@uniswap/v2-core/contracts/interfaces/IUniswapV2Factory.sol";
import "@uniswap/v2-periphery/contracts/interfaces/IUniswapV2Router02.sol";


contract LITHToken is 
    IERC20Upgradeable,
    LockableOwnableUpgradeable {
     ...
     */
    function initialize(
        address forLiquidity,
        address forILOBuyback,
        address forDevelopers,
        address forMarketing,
        address forPlatform
    ) external initializer {
        __LITH_init(forLiquidity, forILOBuyback, forDevelopers, forMarketing, forPlatform);
    }

    /**
     * @dev See {__LITH_init_unchained}.
     */
    function __LITH_init(
        address forLiquidity,
        address forILOBuyback,
        address forDevelopers,
        address forMarketing,
        ...
        __LockableOwnableUpgradeable_init_unchained();
        __LITH_init_unchained(forLiquidity, forILOBuyback, forDevelopers, forMarketing, forPlatform);
    }

    /**
     * @dev Initially minting _tTotal to caller.
     */
    function __LITH_init_unchained(
        address forLiquidity,
        address forILOBuyback,
        address forDevelopers,
        address forMarketing,
        ...
        _tokenTransfer(owner(), forLiquidity, _tTotal / 10, false);
        _tokenTransfer(owner(), forILOBuyback, _tTotal / 10, false);
        _tokenTransfer(owner(), forDevelopers, _tTotal / 40, false);
        _tokenTransfer(owner(), forMarketing, _tTotal / 40, false);
        _tokenTransfer(owner(), forPlatform, _tTotal / 20, false);

        _tsGenesis = block.timestamp;
        setFees();

        swapAndLiquifyEnabled = true;
    
        maxTxAmount = 10000 * 10**6 * 10**18;
    ...
    function setFees() public onlyOwner {
        if (block.timestamp > _tsGenesis + 90 * 86400) {
            taxFee = 5;
            liquidityFee = 0;
            burnFee = 0;
        } else if (block.timestamp > _tsGenesis + 14 * 86400) {
            taxFee = 3;
            liquidityFee = 0;
            burnFee = 2;
        } else {
    ...
    function _removeAllFee() private {
        if(taxFee == 0 && liquidityFee == 0 && burnFee == 0) return;
    ...
    function addLiquidity(uint256 tokenAmount) external payable lockTheSwap {
        require(tokenAmount > 0
            && balanceOf(_msgSender()) >= tokenAmount
            && msg.value > 0, "LITHToken: bad input");
 ...
 * the owner.
 */
abstract contract LockableOwnableUpgradeable is Initializable, ContextUpgradeable {
    ...
    /**
     * @dev See {__LockableOwnableUpgradeable_init_unchained}.
     */
    function __LockableOwnableUpgradeable_init() internal onlyInitializing {
        __Context_init_unchained();
        __LockableOwnableUpgradeable_init_unchained();
    }
     ...
     */
    function __LockableOwnableUpgradeable_init_unchained() internal onlyInitializing {
