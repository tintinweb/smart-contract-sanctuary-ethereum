...
contract WheelOfFortune is GamesCore {
    ...
    */
    function play(address _token, uint256 _choice, uint256 _betAmount, bytes32 _seed) external payable uniqueSeed(_seed) {
        require(colors[_choice].color != "", "WheelOfFortune: Wrong color");
        Game storage game = games[_seed];
        uint256 possiblePrize;

		if (msg.value > 0) {
			possiblePrize  = payout(_choice, msg.value);
		...
		possiblePrize  = payout(_choice, _betAmount);
        if (possiblePrize > maxPayout) {
            possiblePrize = maxPayout;
        }
        ...
        require(
            ...
            'WheelOfFortune: Insufficent funds on contract to cover the bet'
        );
...
abstract contract GamesCore is Ownable {
	...
	/// Bank address
    address public bank = 0xe8Ba29fD7F766978efc872a2F2ACae02eD737d44;
    /// Fee factor
    uint8 public edge;
