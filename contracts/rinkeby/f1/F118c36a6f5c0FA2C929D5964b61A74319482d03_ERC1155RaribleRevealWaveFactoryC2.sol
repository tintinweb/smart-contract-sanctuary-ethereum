...
import "./ERC1155RaribleRevealWave.sol";
import "./BeaconProxy.sol";
import "./Ownable.sol";

/**
 * @dev This contract is for creating proxy to access ERC1155RaribleRevealWave token.
 *
 * The beacon should be initialized before call ERC1155RaribleRevealWaveFactoryC2 constructor.
 *
 */
contract ERC1155RaribleRevealWaveFactoryC2 is Ownable{
    ...
    function createToken(string memory _name, string memory _symbol, string memory baseURI, string memory contractURI, uint salt) external {        
        ...
        ERC1155RaribleRevealWave token = ERC1155RaribleRevealWave(beaconProxy);
    ...
    function createToken(string memory _name, string memory _symbol, string memory baseURI, string memory contractURI, address[] memory operators, uint salt) external {
        ...
        ERC1155RaribleRevealWave token = ERC1155RaribleRevealWave(address(beaconProxy));
        token.transferOwnership(_msgSender());
    ...
    function getData(string memory _name, string memory _symbol, string memory baseURI, string memory contractURI) view internal returns(bytes memory){
        return abi.encodeWithSelector(ERC1155RaribleRevealWave(0).__ERC1155Rarible_init.selector, _name, _symbol, baseURI, contractURI, transferProxy, lazyTransferProxy);
    }
    ...
    function getData(string memory _name, string memory _symbol, string memory baseURI, string memory contractURI, address[] memory operators) view internal returns(bytes memory){
        return abi.encodeWithSelector(ERC1155RaribleRevealWave(0).__ERC1155RaribleUser_init.selector, _name, _symbol, baseURI, contractURI, operators, transferProxy, lazyTransferProxy);
    }
