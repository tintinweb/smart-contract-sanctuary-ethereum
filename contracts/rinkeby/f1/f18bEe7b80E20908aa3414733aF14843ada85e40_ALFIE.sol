 ...
 */
contract ALFIE is ERC721Tradable {
    ...
    uint256 private alfieReserve = 50;

    /**
     * @notice Presale start time
     */
    uint256 public presaleStartTime = 1652517000;

    /**
     * @notice Sale start time
     */
    uint256 public startTime = 1652603400;
     ...
     */
    function presaleMintAlfie(uint256 numberOfTokens, bytes32[] memory proof) public payable {
        ...
        require(balanceOf(msg.sender) == 0, 'WL access already used');
        require(msg.value >= AlfiePrice * numberOfTokens, 'Ether value sent is not correct');
        require(numberOfTokens > 0 && numberOfTokens <= maxAlfiePurchasePresale, 'Can only mint 2 tokens during presale');
     ...
     */
    function mintAlfie(uint256 numberOfTokens) public payable {
        uint256 supply = _alfiesId.current() - 1;
        ...
        require(balanceOf(msg.sender) + numberOfTokens <= maxAlfiePurchase, 'Cannot mint more than 5 Alfies');
        require(supply + numberOfTokens <= maxAlfies, 'Purchase would exceed max supply of Alfies');
        require(msg.value >= AlfiePrice * numberOfTokens, 'Ether value sent is not correct');
        for (uint256 i = 1; i <= numberOfTokens; i++) {
            _safeMint(msg.sender, supply + i);
