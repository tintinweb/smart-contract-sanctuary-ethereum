...
contract Movie is ERC721Optimized, Ownable, ReentrancyGuard {
    ...
    constructor(string memory initBaseURI) ERC721Optimized("Movie", "MOV") {
        baseURI = initBaseURI;
    }
    ...
    //设置mint价格
    function setMintPrice(uint256 newMinitPrice) public onlyOwner {
        require(newMinitPrice >= 0, "minit price must be greater than zero");
        mintPrice = newMinitPrice;
    }
    ...
    function SetTokenIdPrice(uint256 tokenId,uint256 newTokenPrice) public nonReentrant {
     ...
     //赠送
    function giveAway(address to, uint256 numberOfToken) public onlyOwner {
        _safeMint(to, numberOfToken);
    }
   ...
   mapping(address => bool) minted;
    function mint(uint256 numberOfTokens) public payable nonReentrant {
        ...
        //check status
        require(status == Status.Started, "Minit is not live");

        //check numberOfTokens ==1
        require(numberOfTokens == 1, "Only minit one");

        //check
        require((totalSupply() + numberOfTokens) <= supply, "Supply has been reached");
 
        //check owner minted number
        require(!minted[msg.sender],"Only minit one");
