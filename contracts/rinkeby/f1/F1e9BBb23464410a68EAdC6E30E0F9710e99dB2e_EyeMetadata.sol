...
contract EyeMetadata is IEyeMetadata, Ownable {
        ...
        returns (bytes memory)
    {
        bytes memory attributes = abi.encodePacked(
            ...
            _toString(_eye.getGreatness(tokenId)),
        ...
        returns (string memory)
    {
        ...
        bytes32[] memory stories = _eye.getStoriesOnNFT(tokenId);
        for (uint256 i = 0; i < stories.length; i++) {
            collection = string(
                abi.encodePacked(
                    collection,
                    "&s=",
                    _toHexString(uint256(stories[i]))
                )
            );
        }
        return collection;
        ...
        return
            abi.encodePacked(
                _confirmERC721Balance(_eye.ownerOf(tokenId), powerupAddress)
                    ...
                    : "0",
                "_",
                _toString(_eye.getAttunementIndex(tokenId)),
                "_",
                _toString(_eye.getOrderIndex(tokenId)),
                "_",
                _toString(_eye.getConditionIndex(tokenId)),
                "_",
                _toString(_eye.getVisionIndex(tokenId))
            );
    }
...
interface IEye is IERC721AUpgradeable {
    ...
    function getStoriesOnNFT(uint256 tokenId)
        external
        view
