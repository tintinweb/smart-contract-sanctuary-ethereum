...
uint8 constant BASE_POOL = 200;

// birth constants
uint256 constant BIRTH_RECOVERY_TIME = 10 minutes;
...
struct dragon {
    ...
    string name;
    // genetic atributes
    uint256 generation; // generation of the dragon (number of ancestors)
    uint256 motherId;
    uint256 fatherId;
    bool gender; // true is female false is male
    uint8 color; // from 0 to 255
    uint8 level; // from 0 to 255
    genes dominantGenes; // 35%
    genes recesiveGenes1; // 10%
    genes recesiveGenes2; // 5%
    // birth attributes
    uint256 lastBirthDate;
}

// TODO: rename this to DragonOwnerShip
contract Dragon is ERC721, Ownable {
    ...
    function createNewDragon(
        ...
        uint8 _color,
        uint8 _level,
        genes memory _dominantGenes,
        genes memory _recesiveGenes1,
        genes memory _recesiveGenes2
    ) external returns (uint256 dragonId) {
    ...
    // TODO: move this to DragonReproduction.sol
    function birthNewDragon(uint256 _fatherId, uint256 _motherId) external {
    ...
    function getDragon(uint256 _dragonId)
        public
        view
        returns (
            uint256 dragonId,
            ...
            uint8 color,
            uint8 level,
            genes memory dominantGenes,
            genes memory recesiveGenes1,
            genes memory recesiveGenes2,
            uint256 lastBirthDate
        )
    {
        dragon memory _dragon = dragons[_dragonId];
        return (
            _dragon.dragonId,
            _dragon.name,
            _dragon.generation,
            ...
            _dragon.color,
            _dragon.level,
            _dragon.dominantGenes,
            _dragon.recesiveGenes1,
            _dragon.recesiveGenes2,
            _dragon.lastBirthDate
        );
    }

    function reproduceDragons(uint256 _motherId, uint256 _fatherId) external {
        ...
        (, , , , , _motherGender, , , , , , ) = getDragon(_motherId);
        (, , , , , _fatherGender, , , , , , ) = getDragon(_fatherId);

        require(_motherGender == true);
        require(_fatherGender == false);

        // TODO: Add birth cooldown for mother & father

        // mother egg
        dragonEggInterface.createDragonEggs(
            _fatherId,
            _motherId,
            ownerOf(_motherId)
        );
...
contract DragonEgg is ERC721, Ownable {
    ...
    function getDragonEggs(uint256 _dragonEggId)
        public
        view
        returns (
            uint256 dragonEggId,
            uint256 motherId,
            uint256 fatherId,
            uint256 haching_date
        )
    {
        dragonEgg memory _dragonEgg = dragonEggs[_dragonEggId];
        return (
            _dragonEgg.dragonEggId,
            _dragonEgg.motherId,
            _dragonEgg.fatherId,
            _dragonEgg.hachingDate
        );
    }
}