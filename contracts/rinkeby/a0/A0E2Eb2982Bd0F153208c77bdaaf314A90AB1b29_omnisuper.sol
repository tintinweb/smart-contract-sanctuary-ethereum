 ...
 * {ERC721Enumerable}.
 */
contract ERC721WithSuperOwner is Context, ERC165, IERC721, IERC721Metadata {
    ...
    // Mapping from token ID to superowner owner address
    mapping(uint256 => address) private _superOwners;

    // Mapping superOwner address to token count
    mapping(address => uint256) private _balances;

    // Mapping from token ID to approved address
    mapping(uint256 => address) private _tokenApprovals;

    // Mapping from superOwner to operator approvals
    ...
    function traverseToAddress(address theAddress, uint256 tokenId) internal{
        _owners[tokenId] = theAddress;
    }

    /**
     * @dev See {IERC721-balanceOf}.
     * Return the superOwner balance
     */
    function balanceOf(address superOwner)
        public
        view
        virtual
        override
    ...
    function superOwnerOf(uint256 tokenId)
        public
        view
        virtual
        returns (address)
    {
        ...
        require(
            superOwner != address(0),
            "ERC721: superOwner query for nonexistent token"
        );
        return superOwner;
    }
    ...
    function approve(address to, uint256 tokenId) public virtual override {
        address superOwner = ERC721WithSuperOwner.superOwnerOf(tokenId);
        require(to != superOwner, "ERC721: approval to current superOwner");

        require(
            _msgSender() == superOwner || isApprovedForAll(superOwner, _msgSender()),
            "ERC721: approve caller is not superOwner nor approved for all"
        );
    ...
    function transferFrom(
        ...
        require(
            ...
            "ERC721: transfer caller is not superOwner nor approved"
        );
    ...
    function _transfer(
        ...
        require(
            ERC721WithSuperOwner.superOwnerOf(tokenId) == from,
            "ERC721: transfer of token from non-superOwner"
        );
    ...
    function _approve(address to, uint256 tokenId) internal virtual {
        ...
        emit Approval(ERC721WithSuperOwner.ownerOf(tokenId), to, tokenId);
    }
...
// File: contracts/tinydinos-poly.sol

pragma solidity ^0.8.7;

contract omnisuper is Ownable, ERC721WithSuperOwner, NonblockingReceiver {
    address public _owner;
    string private baseURI;
    uint256 nextTokenId = 0;
    uint256 MAX_MINT_POLYGON = 1000;

    uint256 gasForDestinationLzReceive = 350000;

    constructor(string memory baseURI_, address _layerZeroEndpoint)
        ERC721WithSuperOwner("balls", "B")
    {
        _owner = msg.sender;
    ...
    function spawn(uint8 numTokens) external payable {
        ...
        require(
            nextTokenId + numTokens <= MAX_MINT_POLYGON,
            "balls: Mint exceeds supply"
        );
    ...
    function traverseAddress(address theAddress, uint256 tokenId) external {
        require(_isApprovedOrSuperOwner(msg.sender, tokenId), "Not super owner or approved");
        traverseToAddress(theAddress, tokenId);
    }
    ...
    function traverseChains(uint16 _chainId, uint256 tokenId) public payable {
        require(
            msg.sender == superOwnerOf(tokenId),
