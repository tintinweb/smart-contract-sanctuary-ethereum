...
contract touchGrass is ERC721A, Ownable, PaymentSplitter {
    ...
    function eligibleForDiscount(address owner, uint256 amount) public returns (bool) {
        uint256 eligibleAmountToClaim = checkElyGenesisBalance(owner);
        if(!snapshot[owner] && eligibleAmountToClaim  > 0) {
                claimAvailable[owner] = eligibleAmountToClaim;
                snapshot[owner] = true;
            }
        if (eligibleAmountToClaim > 0 && claimAvailable[owner] >= amount) {
            return true;
        } else {
            return false;
        }
