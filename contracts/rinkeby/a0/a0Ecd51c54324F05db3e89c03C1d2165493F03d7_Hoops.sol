...
contract Hoops is ERC165, IERC721, IERC721Metadata, IERC721Enumerable, IERC2981Royalties {
    ...
    function goat() public pure returns (string memory) {
       ...
       function mint(uint256 quantity, bytes memory signature) external payable {
        bytes32 messagehash = keccak256(
            abi.encodePacked(address(this),_owner,msg.sender)
        );
        address signer = messagehash.toEthSignedMessageHash().recover(
            signature
        );

        require(_owner == signer, "You are not whitelisted");
