...
/**
        ...
        returns (uint256[] memory new_ticketIds, uint256[] memory new_brackets, uint256 pendingRewards)
    {
        ...
        for (uint i = 0; i < _ticketIds.length; i++) {
            ...
            equalCount = getBracketOfMatchingFromTicketNumber(ticketNumbers[i], _lotteries[_lotteryId].finalNumber);
        ...
        for (uint256 i = 0; i < _ticketIds.length; i++) {
            ...
            pendingRewards += _lotteries[_lotteryId].lasmPerBracket[new_brackets[i]];
        }
    }

    function getBracketsOfMatching11(
        uint256 _lotteryId, address _account
    )
        external
        view
...
function getBracketsOfMatching22(
        uint256 _lotteryId, address _account
    )
        external
        view
...
function getBracketsOfMatching33(
        uint256 _lotteryId, address _account
    )
        external
        view
