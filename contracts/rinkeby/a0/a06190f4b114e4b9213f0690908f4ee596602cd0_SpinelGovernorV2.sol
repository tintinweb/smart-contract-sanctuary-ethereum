...
contract SpinelGovernorV2 is  SpinelGovernor {
...
contract SpinelGovernor is
    ...
    OwnableUpgradeable, ISpinelGovernor
{
        ...
        returns (uint256)
    {
        ...
        timelockSpinel.reservedTreasury(proposalId, msg.sender, reservedAmount);
        return proposalId;
    }
    ...
    address public treasury;
     ...
     function getHolderBalance(address owner) internal view override returns (uint256){
        ISpinelTimelock timelockSpinel = ISpinelTimelock(timelock());
        return timelockSpinel.getHolderBalance(owner);
    }
     function provideFund(address owner, uint256 amount) external override{
         require(msg.sender == address(timelock()), "1");
        _mintPoint(owner, amount);
    }
     function releaseFund(address owner, uint256 amount) external override{
         require(msg.sender == address(timelock()), "1");
        _burnPoint(owner, amount);
    }
}
 ...
 * _Available since v4.3._
 */
abstract contract GovernorVotesUpgradeable is Initializable, GovernorUpgradeable, ERC20VotesUpgradeable {
...
abstract contract ERC20VotesUpgradeable {
    ...
    function getHolderBalance(address owner) internal virtual view returns (uint256);
    /**
     ...
     */
    function _mintPoint(address account, uint256 amount) internal virtual {
     ...
     */
    function _burnPoint(address account, uint256 amount) internal virtual {
    ...
    function _delegate(address delegator, address delegatee) internal virtual {
        ...
        uint256 delegatorBalance = getHolderBalance(delegator);
