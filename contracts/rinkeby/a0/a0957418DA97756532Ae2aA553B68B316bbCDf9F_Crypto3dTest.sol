...
library Counters {
    ...
    //  CHANGE THE VALUE OF 7
    function current(Counter storage counter) internal view returns (uint256) {
        return (counter._value +16);
    }
...
contract Crypto3dTest is ERC721, Pausable, Ownable, ERC721Burnable {
    using Counters for Counters.Counter;
    address public burnAddress = 0xd0691889BAE9dc81436C0baE01239aa5Ec266029;  // Amulay sir's wallet
    address ExistingERC1155;

    Counters.Counter private _tokenIdCounter;
    Counters.Counter private _gloablId;
    uint256[][] public amount = [
        [0, 0, 0, 0, 0, 350000000000000000], // amount[0][x] should not be used anywhere
        [
            50000000000000,
            50000000000000,
            50000000000000,
    ...
    // uint256[11] public roundCap = [0, 12, 24, 36, 48, 60, 72, 84, 96, 108, 120];
    uint256[11] public roundCap = [0, 20, 40, 60, 80, 100, 120, 140, 160, 180, 200];
    ...
    constructor(address _ExistingERC1155) ERC721("Crypto3d", "C3d") {
    ...
    // code erc1155 receiver function
    // ids start from 0
    // round 1 from 0 to 499
    // add unreviel/reveal stage
    // giveaway function
    // add stop transfer functionality for each stage
    // add withdraw ether functions
    // reviel/ unreveal add

    // pause/ unpase default behaviour + at any time
    // even the owner can not skip to next round it current round is not sold completely
    ...
    //  Function to update the price of crypto3ds for any round
    //  Only owner can call this function
    //  round can take values from 1 to 10
    //  batch can take values from 0 to 5
    //  0 means batch of 1
    //  1 means batch of 2
    //  2 means batch of 3
    //  3 means batch of 4
    ...
    function updatePrice(
        ...
        require(batch >= 0 && batch <= 5);
        amount[round][batch] = updatedValue;
    }
        ...
        whenNotPaused
    {
        ...
        if (msg.sender != owner()) {
            if (_quantity == 1) {
                require(msg.value >= amount[Round][0], "Not enough ethers");
            } else if (_quantity == 2) {
                require(msg.value >= amount[Round][1], "Not enough ethers");
            } else if (_quantity == 3) {
                require(msg.value >= amount[Round][2], "Not enough ethers");
            } else if (_quantity == 4) {
                require(msg.value >= amount[Round][3], "Not enough ethers");
            } else if (_quantity == 5) {
                require(msg.value >= amount[Round][4], "Not enough ethers");
            } else if (_quantity == 10) {
                require(msg.value >= amount[Round][5], "Not enough ethers");
            }
        }
