...
// VxlToken with Governance.
contract VxlToken is ERC20("VxlToken", "VXL"), Ownable {
    ...
    */
        ...
        external
    {
        ...
        require(signatory != address(0), "VXL::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "VXL::delegateBySig: invalid nonce");
        require(now <= expiry, "VXL::delegateBySig: signature expired");
    ...
    function getPriorVotes(address account, uint blockNumber)
        ...
        returns (uint256)
    {
        require(blockNumber < block.number, "VXL::getPriorVotes: not yet determined");
    ...
    function _writeCheckpoint(
        ...
        internal
    {
        uint32 blockNumber = safe32(block.number, "VXL::_writeCheckpoint: block number exceeds 32 bits");
...
interface IMigratorToVxlSwap {
    // Perform LP token migration from legacy UniswapV2 to VxlSwap.
    // Take the current LP token address and return the new LP token address.
    // Migrator should have full access to the caller's LP token.
    // Return the new LP token address.
    //
    // XXX Migrator must have allowance access to UniswapV2 LP tokens.
    // VxlSwap must mint EXACTLY the same amount of VXLSwap LP tokens or
    // else something bad will happen. Traditional UniswapV2 does not
    // do that so be careful!
    function migrate(IERC20 token) external returns (IERC20);
}

// VxlMasterFarmer is the master of VXL. He can make VXL and he is a fair guy.
//
// Note that it's ownable and the owner wields tremendous power. The ownership
// will be transferred to a governance smart contract once VXL is sufficiently
...
contract VxlMasterFarmer is Ownable {
    ...
    uint256 public constant PERCENT_FOR_DEV = 0; // 10% reward for dev

    // The migrator contract. It has a lot of power. Can only be set through governance (owner).
    IMigratorToVxlSwap public migrator;
    ...
    function setMigrator(IMigratorToVxlSwap _migrator) public onlyOwner {
        migrator = _migrator;
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        (vxlForDev, vxlForFarmer) = getPoolReward(pool.lastRewardBlock, block.number, pool.allocPoint);
        
        if (vxlForDev > 0) {
            vxl.mint(devaddr, vxlForDev);
            //For more simple, I lock reward for dev if mint reward in bonus time
            if (block.number <= FINISH_BONUS_AT_BLOCK) {
                vxl.lock(devaddr, vxlForDev.mul(PERCENT_LOCK_BONUS_REWARD).div(100));
            }
        }
        vxl.mint(address(this), vxlForFarmer);
        pool.accVxlPerShare = pool.accVxlPerShare.add(vxlForFarmer.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    function getPoolReward(uint256 _from, uint256 _to, uint256 _allocPoint) public view returns (uint256 forDev, uint256 forFarmer) {
        ...
        uint256 vxlCanMint = 0;

        if (vxlCanMint < amount) {
            forDev = 0;
            forFarmer = vxlCanMint;
        }
        else {
    ...
    function pendingReward(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply > 0) {
            ...
            accVxlPerShare = accVxlPerShare.add(vxlForFarmer.mul(1e12).div(lpSupply));

        }
        return user.amount.mul(accVxlPerShare).div(1e12).sub(user.rewardDebt);
    }
    ...
    function _harvest(uint256 _pid) internal {
        ...
        if (user.amount > 0) {
            uint256 pending = user.amount.mul(pool.accVxlPerShare).div(1e12).sub(user.rewardDebt);
            uint256 masterBal = vxl.balanceOf(address(this));
            ...
            if(pending > 0) {
                vxl.transfer(msg.sender, pending);
                uint256 lockAmount = 0;
                if (user.rewardDebtAtBlock <= FINISH_BONUS_AT_BLOCK) {
                    lockAmount = pending.mul(PERCENT_LOCK_BONUS_REWARD).div(100);
                    vxl.lock(msg.sender, lockAmount);
                }

                user.rewardDebtAtBlock = block.number;

                emit SendVxlReward(msg.sender, _pid, pending, lockAmount);
            }

            user.rewardDebt = user.amount.mul(pool.accVxlPerShare).div(1e12);
        }
    }

    // Deposit LP tokens to VxlMasterFarmer for SUSHI allocation.
    ...
    // Safe vxl transfer function, just in case if rounding error causes pool to not have enough VXLs.
    function safeVxlTransfer(address _to, uint256 _amount) internal {
        uint256 vxlBal = vxl.balanceOf(address(this));
        if (_amount > vxlBal) {
            vxl.transfer(_to, vxlBal);
        } else {
            vxl.transfer(_to, _amount);
        }
    }
