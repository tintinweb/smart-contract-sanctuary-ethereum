 ...
 */
contract SnapshotVaultMod is AccessControl, ISnapshotVault {
    ...
    constructor(address _daoAddress, address _sharesholderTokenAddress) {
        _setupRole(DEFAULT_ADMIN_ROLE, msg.sender);
        _setupRole(SNAPSHOT_ROLE, msg.sender);

        sharesholderTokenAddress = _sharesholderTokenAddress;
    ...
    modifier onlyGT() {
        require(IERC20(daoAddress).balanceOf(address(msg.sender)) > 1);
        _;
    }
    
    modifier onlyDAO() {
        require(daoAddress == msg.sender);
        _;
    }
    ...
    function nextSnapshotCashflowAmount(address cashflowTokenAddress) public view returns (uint256) {
        uint256 balance = IERC20(cashflowTokenAddress).balanceOf(address(daoAddress));
