...
contract LockFactory is AccessControl, DeployFee {
        ...
        onlyRole(FACTORY_MANAGER)
    {
        ...
        setupDeployFeeInternal(
            ...
            _deployFeeFirstToken,
            _deployFeeSecondToken,
            _priceFeed,
    ...
    function addToWhiteList(address[] memory whiteList) external onlyRole(FACTORY_MANAGER) {
        for(uint256 i; i < whiteList.length; i++) {
            if(!isWhiteListed[whiteList[i]]) {
    ...
    function verifyFeePayment(address paymentToken, bytes memory data) private {
        require(
        ...
        if (deployFeePaymentOption == PERCENTAGE_UPFRONT_PAYMENT_OPTION) {
            chargeDeployFee(paymentToken, true);
        } else {
            if (!feePaid[token] && !hasRole(FACTORY_MANAGER, _msgSender())) {
                chargeDeployFee(paymentToken, feePaid[token]);
                feePaid[token] = true;
            } else {
                if (
                    deployFeePaymentOption == COMBINED_PAYMENT_OPTION ||
                    hasRole(FACTORY_MANAGER, _msgSender())
                ) {
                    chargeDeployFee(paymentToken, true);
                }
            }
...
abstract contract DeployFee is Context {
        ...
        internal
    {
        ...
        if (!isPaid) {
            paymentToken.safeTransferFrom(
                ...
                ? deployFeeFixedAmount * ( tokenDecimals / EXP_VALUE )
            ...
            paymentToken.safeTransferFrom(
                ...
                deployFeeBeneficiary,
                tokenDecimals >= EXP_VALUE 
                ? ((deployFeeFixedAmount * deployFeePercentageAmount) / EXP_VALUE) * ( tokenDecimals / EXP_VALUE )
                : ((deployFeeFixedAmount * deployFeePercentageAmount) / EXP_VALUE) / ( EXP_VALUE / tokenDecimals )
            );
        }
    }
    ...
    */
    function cryptoChargeDeployFee(bool isPaid) internal {
        ...
        if(!isPaid) {
            uint256 requiredEth = priceFeedDecimals >= EXP_VALUE 
            ? deployFeeFixedAmount / (uint(ethUsdPrice) / EXP_VALUE)
            : deployFeeFixedAmount * priceFeedDecimals / uint256(ethUsdPrice);
            ...
            uint256 requiredEth = priceFeedDecimals >= EXP_VALUE 
            ? deployFeeFixedAmount / (uint(ethUsdPrice) / EXP_VALUE) * deployFeePercentageAmount / EXP_VALUE
            : deployFeeFixedAmount * priceFeedDecimals / uint256(ethUsdPrice) * deployFeePercentageAmount / EXP_VALUE;
            require(msg.value >= requiredEth);
            uint256 ethExceeded = msg.value - requiredEth;
            if(ethExceeded > 1 gwei) {
                bool sent = payable(_msgSender()).send(ethExceeded);
