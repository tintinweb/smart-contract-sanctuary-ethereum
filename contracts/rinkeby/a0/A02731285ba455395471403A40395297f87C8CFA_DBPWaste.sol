...
contract DBPWaste is ERC20, Ownable {

    iDBinvest public dbInvestz;
    ...
    constructor(address dbInvestAddress) ERC20("DBPWaste", "$DBP") {
        dbInvestz = iDBinvest(dbInvestAddress);
        START = block.timestamp;
    }

    function updateReward(address from, address to) external {
        require(msg.sender == address(dbInvestz));
        if(from != address(0)){
    ...
    function burn(address user, uint256 amount) external {
        require(allowedAddresses[msg.sender] || msg.sender == address(dbInvestz), "Address does not have permission to burn");
        _burn(user, amount);
    }
    ...
    function getPendingReward(address user) internal view returns(uint256) {
        return dbInvestz.balanceGenesis(user) * BASE_RATE * (block.timestamp - (lastUpdate[user] >= START ? lastUpdate[user] : START)) / 3600;
    }
