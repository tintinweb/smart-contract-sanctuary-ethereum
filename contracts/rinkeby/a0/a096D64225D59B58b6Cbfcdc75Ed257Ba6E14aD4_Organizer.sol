...
contract Organizer is TeamManager, OperatorManager, Pausable {
  //  Events
  //  DAO Onboarded
  ...
  function onboard(
    ...
    address safeAddress = dao;
    // TODO: verify that safeAddress is Gnosis Multisig

    require(_operators.length > 0, "Operators are mandatory");
    require(_teamCount > 0, "Atleast One team is mandatory");
    ...
    for (uint256 i = 0; i < _operators.length; i++) {
      ...
      require(
        daos[safeAddress].operators[operator] == address(0),
        "Duplicate operator Address provided"
      );
...
abstract contract OperatorManager is Getters {
  ...
  function modifyOperators(
    ...
    for (uint256 i = 0; i < _addressesToAdd.length; i++) {
      ...
      require(
        ...
        "Invalid Operator Address Provided"
      );
      require(
...
abstract contract MemberManager is Getters {
  function addContributors(uint256 _teamId, address[] memory _contributorsToAdd)
    internal
  {
    for (uint256 i = 0; i < _contributorsToAdd.length; i++) {
      require(
        !isContributor(_teamId, _contributorsToAdd[i]),
        "Contributor already exists in this team"
      );
    ...
    internal
  {
    for (uint256 i = 0; i < _reviewersToAdd.length; i++) {
      require(
        !isReviewer(_teamId, _reviewersToAdd[i]),
        "Reviewer already exists in this team"
      );
    ...
    teamExists(_safeAddress, _teamId)
  {
    if (_contributorsToAdd.length > 0 && _contributorsToRemove.length > 0) {
      ...
      while (cursor < transferCount) {
        require(
          isContributor(_teamId, _contributorsToRemove[cursor]),
          "Contributor not found in team"
        );
        require(
          !isContributor(_teamId, _contributorsToAdd[cursor]),
          "Contributor already in team"
        );
        safeTransferFrom(
      ...
      for (uint256 j = 0; j < mintCount; j++) {
        require(
          !isContributor(_teamId, _contributorsToAdd[cursor]),
          "Contributor is already in team"
        );
...
/// @title Getters for Organizer Contract
abstract contract Getters is Modifiers {
  // Get DAO operators
...
abstract contract Modifiers is Validators {
  ...
  modifier onlyOnboarded(address _safeAddress) {
    require(isDAOOnboarded(_safeAddress), "DAO has not onboarded");
    _;
  }
  ...
  modifier onlyMultisig(address _safeAddress) {
    require(
      msg.sender == _safeAddress,
      "Only DAO's Multisig can take this action"
    );
    _;
  }
  ...
  modifier onlyOperator(address _safeAddress) {
    require(
      isOperator(_safeAddress, msg.sender),
