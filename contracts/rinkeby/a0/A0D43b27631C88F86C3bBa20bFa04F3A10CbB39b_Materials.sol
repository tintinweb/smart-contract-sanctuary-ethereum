...
contract Materials is Ownable, ERC721A, ReentrancyGuard {
    ...
    function eligibleForFreeMint(address buyer) public view returns (bool) {
        return (activeFreeMintPeriod() && !mintedFree(buyer) && freeMintsAvailable() > 0);
    }
    ...
    function safeMint(uint256 quantity) external payable {
        ...
        require(quantity >= 1, "Invalid quantity. Quantity parameter must be greater than or equal to 1.");
        ...
        bool validFreeMintPrice = msg.value == (PRICE_PER_TOKEN_SET * (quantity - 1));
    ...
    function safeMint(uint256 _mintQuantity, bool _validMintRequest) private {
        safeMint(_mintQuantity, _validMintRequest, freeMintAttempt(_mintQuantity));
    }
    ...
    function _maxQuantityCheck(uint256 _mintQuantity, uint256 _freeMintQuantity) private {
        ...
        if (totalSupply() + _mintQuantity > MAX_SUPPLY) {
            ...
            uint256 captureAmount = ((paidForMints / TOKEN_SET_QUANTITY) * PRICE_PER_TOKEN_SET);
    ...
    function freeMintAttempt(uint256 _mintQuantity) private returns (bool) {
        return (msg.value == PRICE_PER_TOKEN_SET * ((_mintQuantity / TOKEN_SET_QUANTITY) - 1));
    }
