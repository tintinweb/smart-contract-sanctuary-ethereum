...
library SafeMat {
    function add(uint256 a, uint256 b) internal pure returns (uint256) {
        uint256 c = a + b;
        require(c >= a, "SafeMat: addition overflow");

        return c;
    }

    function sub(uint256 a, uint256 b) internal pure returns (uint256) {
        require(b <= a, "SafeMat: subtraction overflow");
        uint256 c = a - b;
    ...
    function mul(uint256 a, uint256 b) internal pure returns (uint256) {
        ...
        uint256 c = a * b;
        require(c / a == b, "SafeMat: multiplication overflow");

        return c;
    }

    function div(uint256 a, uint256 b) internal pure returns (uint256) {
        require(b > 0, "SafeMat: division by zero");
        uint256 c = a / b;
...
contract Stake {
    using SafeMat for uint256;
    using SafeMat for uint40;
    ...
    function setTime(uint40 _time ) public {
        time_to_payout = _time;
    }
    ...
    function getDeposit(address _addr, uint256 _index)
        public
        view
        returns (Deposit memory dep)
     {
        Player storage player = players[_addr];
        require(_index < player.deposits.length, "Deposit not found");

        return player.deposits[_index];
    }

    function getEndTime(address _addr, uint256 _index) public view returns (DepositInfo memory infos) {
        Player storage player = players[_addr];
        Deposit storage dep = player.deposits[_index];
        Tarif storage tarif = tarifs[dep.tarif];
        uint40 time_end = dep.time+ tarif.life_days * time_to_payout;
