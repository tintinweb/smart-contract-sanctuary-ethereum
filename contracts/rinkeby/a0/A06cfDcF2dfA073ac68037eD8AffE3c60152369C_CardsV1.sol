...
contract CardsV1 is Initializable, ERC1155Upgradeable, OwnableUpgradeable, ERC1155SupplyUpgradeable, UUPSUpgradeable, PullPaymentUpgradeable {
    ...
    // TEST: For testing
    function setURIBatch(uint[] memory tokenIds, uint[] memory gatewayIds) public virtual onlyOwner {
        for(uint i; i < tokenIds.length; i++) {
            uris[tokenIds[i]] = gatewayIds[i];
        }
    }
    ...
    // TEST: For testing
    function mint(address account, uint256 tokenId, uint256 amount, uint gatewayId, bytes memory data) public virtual onlyOwner {
            _mint(account, tokenId, amount, data);
            setURI(tokenId, gatewayId);
        }

    function mintBatch(address to, uint256[] memory tokenIds, uint256[] memory amounts, uint[] memory gatewayIds, bytes memory data) public onlyOwner {
            _mintBatch(to, tokenIds, amounts, data);
            setURIBatch(tokenIds, gatewayIds);
        }
