...
contract Hyaliko is ERC721A, Ownable {
    ...
    mapping(uint256 => HyalikoTypes.Hyaliko) hyaliko;

    uint256 public constant TOKEN_LIMIT = 10e3;
    ...
    function mintHyalikoZero() public onlyOwner {
        ...
        HyalikoTypes.Hyaliko memory hyalikoData;
        hyalikoData.dna = 1;
        ...
        public
    {
        ...
        // make sure to allow minting from airdrop up until the end. note: quantity is guaranteed to be a valid snapshotted value at this point
        if (startingIndex + quantity > TOKEN_LIMIT) {
            require(startingIndex < TOKEN_LIMIT, "b:07");
            _mint(msg.sender, TOKEN_LIMIT - startingIndex, "", false);
        } else {
    ...
    function mintFromWhitelist(uint16 quantity, bytes32[] calldata _merkleProof)
        public
        payable
    {
        ...
        require(quantity <= MAX_WHITELIST_MINT, "b:06");
    ...
    function mintFromSale(uint256 quantity) public payable {
        ...
        // Consider checking if max batch size creates a gas problem.
        // Probably should be something reasonable like 100
    ...
    function getDna(uint256 _tokenId) public view returns (uint256) {
        ...
        unchecked {
            if (_startTokenId() <= curr && curr < _currentIndex) {
                HyalikoTypes.Hyaliko memory hyalikoData = hyaliko[curr];
