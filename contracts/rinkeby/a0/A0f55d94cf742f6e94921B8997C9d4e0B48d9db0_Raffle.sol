...
contract Raffle is Ownable, TicketStorage, ERC721Holder, PullPayment, VRFConsumerBaseV2 {
    ...
    uint public immutable nftTokenId;
    ...
    function purchaseTicket(uint16 count) payable external {
    ...
    function giveawayTicket(AddressAndCount[] memory receivers) external onlyOwner {
        require(_state == State.WaitingForStart || _state == State.SellingTickets,
    ...
    function cancelBeforeStart() external onlyOwner {
        require(_state == State.WaitingForNFT || _state == State.WaitingForStart,
    ...
    function transferTicketRefundIfCancelled() external {
        ...
        uint refundAmount = _addressToPurchasedCountMap[msg.sender] * ticketPrice;
        if (refundAmount > 0) {
    ...
    function transferERC721ToOwnerIfCancelled(address contractAddress, uint tokenId) external {
        require(_state == State.Cancelled, "Must be in Cancelled");

        IERC721(contractAddress).transferFrom(address(this), owner(), tokenId);
    }

    function transferCryptoPunkToOwnerIfCancelled(address contractAddress, uint tokenId) external {
        require(_state == State.Cancelled, "Must be in Cancelled");
...
abstract contract TicketStorage {
    ...
    uint16 immutable internal _tickets;
    ...
    function _assignTickets(address owner, uint16 count) internal {
        ...
        _addressToAssignedCountMap[msg.sender] += count;
        _addressToAssignedTicketNumberRangesMap[msg.sender].push(uint16(_ticketNumberRanges.length - 1));
    ...
    function findOwnerOfTicketNumber(uint16 ticketNumber) public view returns (address) {
        ...
        uint16 ticketNumberRangeOwnersLength = uint16(_ticketNumberRanges.length);
        assert(_ticketNumberRanges[0].from == 0);
        assert(_ticketNumberRanges[ticketNumberRangeOwnersLength - 1].to == _tickets - 1);

        uint16 left = 0;
        uint16 right =  ticketNumberRangeOwnersLength - 1;
