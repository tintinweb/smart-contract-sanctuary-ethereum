    ...
    OnChainManagerVerifier
{
    ...
    // Require msg.sender == request.account?
    function mint(MintRequest calldata request, bytes calldata signature)
        external
        onlyIfPending(request.id)
        onlyIfMintAuthorized(request, signature)
        whenNotPaused
    {
        _markRequestAsFulfilled(request.id);

        if (request.raks > 0) {
            _raksToken.mint(request.account, request.raks);
        }

        if (request.plots > 0) {
            _plotToken.mint(request.account, request.plots);
        }

        if (request.bredStrains > 0) {
            _bredStrainToken.mint(request.account, request.bredStrains);
        }
    }
        ...
        whenNotPaused
    {
        ...
        if (request.raks > 0) {
            _raksToken.burn(request.account, request.raks);
        }

        if (request.plots > 0) {
            _plotToken.burn(request.account, request.plots);
        }

        if (request.bredStrainIds.length > 0) {
            for (uint256 i = 0; i < request.bredStrainIds.length; i++) {
                _bredStrainToken.burn(request.bredStrainIds[i]);
            }
        }
    }

    function cancelRequest(uint256 requestId) external {
        _markRequestAsCancelled(requestId);
    }
    ...
    function setPlotToken(address plotTokenAddress) external onlyAdmin {
        _plotToken = IPlot(plotTokenAddress);
    }
...
contract OnChainManagerVerifier is EIP712Upgradeable {
    using ECDSAUpgradeable for bytes32;

    address private _authorizedSigner;
    mapping(uint256 => RequestStatus) internal _requestStatuses;
    ...
    bytes32 private constant MINT_REQUEST_TYPE_HASH =
        keccak256(
            "MintRequest(uint256 id,address account,uint256 raks,uint256 plots,uint256 bredStrains)"
        );
    ...
    bytes32 private constant BURN_REQUEST_TYPE_HASH =
        keccak256(
            "BurnRequest(uint256 id,address account,uint256 raks,uint256 plots,uint256[] bredStrainIds)"
        );
    ...
    modifier onlyIfMintAuthorized(
        ...
        require(signer == _authorizedSigner, "Mint not authorized");
        _;
    }
    ...
    modifier onlyIfBurnAuthorized(
        ...
        address signer = digest.recover(signature);
        require(signer == _authorizedSigner, "Burn not authorized");
        _;
    }

    modifier onlyIfPending(uint256 requestId) {
        require(
            _requestStatuses[requestId] == RequestStatus.Pending,
    ...
    {
        return _requestStatuses[requestId];
    }

    function _markRequestAsFulfilled(uint256 requestId)
        internal
        onlyIfPending(requestId)
    {
        _requestStatuses[requestId] = RequestStatus.Fulfilled;
        emit RequestUpdated(requestId, RequestStatus.Fulfilled);
    }

    function _markRequestAsCancelled(uint256 requestId)
        internal
        onlyIfPending(requestId)
    {
        _requestStatuses[requestId] = RequestStatus.Cancelled;
        emit RequestUpdated(requestId, RequestStatus.Cancelled);
    }
