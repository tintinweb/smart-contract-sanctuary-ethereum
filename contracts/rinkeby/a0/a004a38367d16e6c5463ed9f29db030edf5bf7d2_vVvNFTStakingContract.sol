...
contract vVvNFTStakingContract is Ownable {
    ...
    function setSharkRaceByTokenIds(uint256[] memory _sharkTokenIds) external onlyOwner {
        for(uint i = 0; i < _sharkTokenIds.length; i++) {
            nftRace[i] = 1;
        }
    }

    function setWhaleRaceByTokenIds(uint256[] memory _whaleTokenIds) external onlyOwner {
        for(uint i = 0; i < _whaleTokenIds.length; i++) {
            nftRace[i] = 2;
        }
    }

    function setDolphinRace(uint256[] memory _dolphinTokenIds) external onlyOwner {
        for(uint i = 0; i < _dolphinTokenIds.length; i++) {
            nftRace[i] = 0;
        }
    }
    ...
    function stake(uint256[] memory tokenIds, uint32[] memory _lockTime) external {
        ...
        for(uint256 i = 0; i < tokenIds.length; i++) {
            ...
            IERC721(vVvS1RAddress).transferFrom(address(this), msg.sender, tokenIds[i]);
    ...
    function unstake(uint256[] memory tokenIds) external {
        ...
        for(uint256 i = 0; i < tokenIds.length; i++) {
            ...
            IERC721(vVvS1RAddress).transferFrom(msg.sender, address(this), tokenIds[i]);
    ...
    function getAmountOfValidStaked(address _user) public view returns(uint256 dolphinStakingAmount, uint256 sharkStakingAmount, uint256 whaleStakingAmount){
        uint32 race;
        for(uint256 i = 0; i < userInfo[_user].length; i++) {
            UserInfo memory ui = userInfo[_user][i];
