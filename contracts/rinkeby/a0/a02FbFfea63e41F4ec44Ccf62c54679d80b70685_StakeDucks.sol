...
contract StakeDucks is Ownable, IERC721Receiver {
    ...
    // Level
    uint256 public levelUpCostPerLevel = 5000 * 1e18;
    ...
    function calculateStakeBonus(uint256 secondsElapsed) public pure returns (uint256) {
        ...
        returns (uint256[] memory rewards)
    {
        ...
        for (uint256 i; i < tokenIds.length; i++) {
            ...
            uint256 blocksSinceStake = block.timestamp - _depositTimes[account][tokenId];
            rewards[i] =
                (_deposits[account].contains(tokenId) ? 1 : 0) *
                (rate * blocksSinceStake) *
                (calculateStakeBonus(blocksSinceStake) + 100) / 8640000;
        }
        return rewards;
    }
    ...
    function claimRewards(uint256[] calldata tokenIds) public {
        ...
        if (reward > 0) {
            if (!IBread(erc20Address).overTotalSupply()) {
