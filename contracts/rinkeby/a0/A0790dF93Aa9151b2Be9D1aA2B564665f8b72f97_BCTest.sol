...
contract BeerCoinHolder {
    ...
    address origAddr = 0x488648116627C3526B6ddde261FB8E1112d0Ad80;
    
    constructor(address wrapAddr, uint256 numBeers) {
        BCO(origAddr).setMaximumCredit(numBeers);
        BCO(origAddr).approve(wrapAddr, numBeers);
    }
}
...
contract BCTest is ERC721, ERC721Enumerable, Ownable {
    ...
    function Wrap(address debtor) public {
        ...
        require(BCO(origAddr).allowance(msg.sender, address(this)) >= pair.numBeers, "You did not give wrapper transfer permission.");
        require(BCO(origAddr).balanceOf(msg.sender, debtor) >= pair.numBeers, "Original IOU no longer exists.");
        ...
        require(BCO(origAddr).allowance(pair.holderAddr, address(this)) >= pair.numBeers, "Holder did not give wrapper transfer permission.");
        require(BCO(origAddr).maximumCredit(pair.holderAddr) >= pair.numBeers, "Holder does not have enough credit.");
        
        // transfer IOU to the holder
        if (BCO(origAddr).transferOtherFrom(msg.sender, pair.holderAddr, debtor, pair.numBeers)) {
    ...
    function Unwrap(uint256 pairId) public {
        ...
        require(BCO(origAddr).maximumCredit(msg.sender) >= pair.numBeers, "You do not have enough credit.");
        
        // transfer IOU from the holder
        if (BCO(origAddr).transferOtherFrom(pair.holderAddr, msg.sender, pair.debtor, pair.numBeers)) {
            _burn(pairId);
