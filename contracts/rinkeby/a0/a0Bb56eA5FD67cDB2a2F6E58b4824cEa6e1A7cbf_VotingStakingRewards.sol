    ...
    ReentrancyGuard
{
    ...
    modifier updateReward(address account) {
        ...
        if (account != address(0)) {
            (uint256 userEarned, uint256 toTreasury) = potentialXbfReturns(
                0,
                account
            );
    ...
    function setToken(address _newVeXBF)
        external
        onlyOwner
    {
        token = IVeXBF(_newVeXBF);
    }
        ...
        updateReward(_for)
    {
        ...
        if (block.timestamp >= rewardLock.unlockTime) {
            ...
            bondedRewardLocks[_for].amount = rewardLock.amount + amount;
        }
        bondedRewardLocks[_for].unlockTime = block.timestamp + bondedLockDuration;
    }
        ...
        updateReward(msg.sender)
    {
        ...
        amount = Math.min(amount, _balances[msg.sender] - escrowed);
        ...
        if (block.timestamp >= bondedRewardLocks[msg.sender].unlockTime) {
            ...
            uint256 penalty = amount * penaltyPct / PCT_BASE;
            uint256 toTransfer = amount - penalty;
            stakingToken.safeTransfer(msg.sender, toTransfer);
        ...
        updateReward(msg.sender)
    {
        ...
        uint256 escrowed = token.lockedAmount(msg.sender);
        require(
            _balances[msg.sender] - escrowed - bondedRewardLocks[msg.sender].amount >= amount,
    ...
    function _baseBoostLevel() internal view returns (uint256) {
        return PCT_BASE * inverseMaxBoostCoefficient / 100;
    }

    function lockedBoostLevel(address account) public view returns (uint256) {
        IVeXBF veXBF = token;
        uint256 votingBalance = veXBF.balanceOf(account);
        uint256 votingTotal = veXBF.totalSupply();
        uint256 lockedAmount = veXBF.lockedAmount(account);
        ...
        uint256 res = PCT_BASE * (
            inverseMaxBoostCoefficient + (
                (100 - inverseMaxBoostCoefficient)
                * lockedAmount
                * votingBalance
                / votingTotal
                / veXBF.lockedSupply()
        ...
        returns (uint256)
    {
        ...
        return
            (lockedBoost * lockedAmount + _baseBoostLevel() * (stakedAmount - lockedAmount)) / stakedAmount;
    }
        ...
        )
    {
        ...
        (uint256 userEarned, ) = potentialXbfReturns(duration, account);
        return userEarned;
    }

    function potentialXbfReturns(uint256 duration, address account)
        public
        view
        returns (
            uint256,
        ...
        )
    {
        ...
        uint256 maxBoostedReward = _balances[account] * (_rewardPerToken(duration) - userRewardPerTokenPaid[account]) / PCT_BASE;

        uint256 toUser = maxBoostedReward * boostLevel / PCT_BASE;
        uint256 toTreasury = maxBoostedReward - toUser;

        return (toUser + rewards[account], toTreasury);
    }
