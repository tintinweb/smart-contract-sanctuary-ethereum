...
contract ERSRegistry is IERSRegistry {
    ...
    bytes32 constant CLAIM_TYPEHASH = keccak256("Claim(bytes32 chipId, address claimant)");
    ...
    function hashClaim(bytes32 chipId, address claimant) private pure returns (bytes32) {
        return keccak256(abi.encode(CLAIM_TYPEHASH, chipId, claimant));
    }

    function verify(bytes32 chipId, address claimant, uint8 v, bytes32 r, bytes32 s) external {
