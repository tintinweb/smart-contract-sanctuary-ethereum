...
contract ReceiptBridge is Ownable, BridgeEIP712, IReceipt {
    ...
    function nonce() internal view override returns (uint256) {
        return nft.tokenId();
    }
...
abstract contract BridgeEIP712 is EIP712 {
    ...
    function _checkUseSignature(
        ...
        return checkRecoverOwner(hash, _signature);
    }
    ...
    function checkRequestLifetime(uint256 _creationTimestamp) internal view {
        if (_creationTimestamp + requestLifetime < block.timestamp)
    ...
    function checkRecoverOwner(bytes32 _hash, bytes memory _signature)
        internal
        pure
        returns (address)
    {
    ...
    function nonce() internal view virtual returns (uint256);
}
