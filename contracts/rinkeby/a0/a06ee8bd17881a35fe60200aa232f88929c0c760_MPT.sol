...
contract MPT is ERC721A, Ownable {
  ...
  uint256 public constant collectionSize = 7777;
  uint256 public constant presaleSupply = 1000;
  uint256 public constant presalePrice = 0.1 ether;
  uint256 public constant SALE_PRICE = 0.2 ether;
  string public uriSuffix = ".json";

  uint256 public presaleEpoch;
  ...
  constructor( 
        uint256 _presaleEpoch,
        uint256 _publicSaleEpoch, 
        string memory _initBaseURI
        )
    ERC721A("TEST", "TEST")
    {
    presaleEpoch = _presaleEpoch;
  ...
  function presaleMint(bytes32[] calldata merkleProof, uint256 count)
    external
    payable
    {
    require(block.timestamp >= presaleEpoch, "Too soon");
    require(block.timestamp < publicSaleEpoch, "presale over");
    require(
      MerkleProof.verify(
        merkleProof,
        whitelistMerkleRoot,
    ...
    require(msg.value >= presalePrice * count, "Ether value sent is below the price");
    require(usedAddresses[msg.sender] + count <= 1, "max per wallet reached");
    require(count > 0 && count <= 1, "You can drop minimum 1, maximum 1 NFTs");
    require(totalSupply() + count <= presaleSupply, "Cannot exceeds Presale supply");
  ...
  function publicMint(uint256 count) external payable {
    ...
    require(totalSupply() + count <= collectionSize, "Cannot Eexceeds max supply");
    require(count > 0 && count <= 10, "You can drop minimum 1, maximum 10 NFTs");

    _mint(msg.sender, count);
  }

  function reservedMint(address _addrs, uint256 count) public onlyOwner {
    require(totalSupply() + count <= 1, "Cannot Eexceeds reserved supply");
    require(count <= collectionSize, "Not enough NFTs left");
      _mint(_addrs, count);
  }
  ...
  */
  ...
  function setPresaleEpoch(uint256 _epoch) external onlyOwner {
    presaleEpoch = _epoch;
  }
