...
/** @dev Contract functionality overview
    ...
    read-functions
        ...
        totalFistbumpsEligableToMintFor(address) - public function returns (integer) the total (eligableFistbumps) to be minted for (inputAddress)
    ...
    helper-functions
        ...
        isThisFistbumpResponded(address, address) - private function checks if a fistbump is (respondedFistbump)
        isThisFistbumpMinted(address, address) - private function checks if a fistbump is (mintedFistbump)

*/
/** @dev Contract functionality example
    1. Event: Address A (A) fistbumps Address B (B) - A->fistbumpThis(B)
        totalFistbumps() = 1
        totalMintedFistbumps() = 0
        totalFistbumpsFor(A) = 1
        totalFistbumpsFor(B) = 0
        totalRespondedFistbumpsFor(A) = 0
        totalRespondedFistbumpsFor(B) = 0
        totalUnrespondedFistbumpsFor(A) = 1
        totalUnrespondedFistbumpsFor(B) = 0
        totalMintedFistbumpsFor(A) = 0
        totalMintedFistbumpsFor(B) = 0
        totalFistbumpsEligableToMintFor(A) = 0
        totalFistbumpsEligableToMintFor(B) = 0
        totalFistbumpsNotRespondedBy(A) = 0
        totalFistbumpsNotRespondedBy(B) = 1
        fistbumpedAddressesFor(A) = [B]
        fistbumpedAddressesFor(B) = []
        respondedFistbumpedAddressesFor(A) = []
        respondedFistbumpedAddressesFor(B) = []
        unrespondedFistbumpedAddressesFor(A) = [B]
        unrespondedFistbumpedAddressesFor(B) = []
        mintedFistbumpsFor(A) = []
        mintedFistbumpsFor(B) = []
        mintedFistbumpedAddressesFor(A) = []
        mintedFistbumpedAddressesFor(B) = []
        mintedFistbumpTokenIdsFor(A) = []
        mintedFistbumpTokenIdsFor(B) = []
        fistbumpedAddressesEligableToMintFor(A) = []
        fistbumpedAddressesEligableToMintFor(B) = []
        fistbumpsNotRespondedBy(A) = []
        fistbumpsNotRespondedBy(B) = [A]
    2. Event: Address B (B) fistbumps Address A (A) - B->fistbumpThis(A)
        totalFistbumps() = 2
        totalMintedFistbumps() = 0
        totalFistbumpsFor(A) = 1
        totalFistbumpsFor(B) = 1
        totalRespondedFistbumpsFor(A) = 1
        totalRespondedFistbumpsFor(B) = 1
        totalUnrespondedFistbumpsFor(A) = 0
        totalUnrespondedFistbumpsFor(B) = 0
        totalMintedFistbumpsFor(A) = 0
        totalMintedFistbumpsFor(B) = 0
        totalFistbumpsEligableToMintFor(A) = 1
        totalFistbumpsEligableToMintFor(B) = 1
        totalFistbumpsNotRespondedBy(A) = 0
        totalFistbumpsNotRespondedBy(B) = 0
        fistbumpedAddressesFor(A) = [B]
        fistbumpedAddressesFor(B) = [A]
        respondedFistbumpedAddressesFor(A) = [B]
        respondedFistbumpedAddressesFor(B) = [A]
        unrespondedFistbumpedAddressesFor(A) = []
        unrespondedFistbumpedAddressesFor(B) = []
        mintedFistbumpsFor(A) = []
        mintedFistbumpsFor(B) = []
        mintedFistbumpedAddressesFor(A) = []
        ...
        mintedFistbumpTokenIdsFor(A) = []
        mintedFistbumpTokenIdsFor(B) = []
        fistbumpedAddressesEligableToMintFor(A) = [B]
        fistbumpedAddressesEligableToMintFor(B) = [A]
        fistbumpsNotRespondedBy(A) = []
        fistbumpsNotRespondedBy(B) = []
    3. Event: Address A (A) mints fistbump for fistbump with Address B (B) - A->mintFistbumpForFistbumpWith(B)
        totalFistbumps() = 2
        totalMintedFistbumps() = 1
        totalFistbumpsFor(A) = 1
        totalFistbumpsFor(B) = 1
        totalRespondedFistbumpsFor(A) = 1
        totalRespondedFistbumpsFor(B) = 1
        ...
        unrespondedFistbumpedAddressesFor(A) = []
        unrespondedFistbumpedAddressesFor(B) = []
        mintedFistbumpsFor(A) = [1, B]
        mintedFistbumpsFor(B) = []
        mintedFistbumpedAddressesFor(A) = [B]
        mintedFistbumpedAddressesFor(B) = []
        mintedFistbumpTokenIdsFor(A) = [1]
        mintedFistbumpTokenIdsFor(B) = []
        fistbumpedAddressesEligableToMintFor(A) = []
        fistbumpedAddressesEligableToMintFor(B) = [A]
        fistbumpsNotRespondedBy(A) = []
        fistbumpsNotRespondedBy(B) = []
    4. Event: Address B (B) mints fistbump for fistbump with Address A (A) - B->mintFistbumpForFistbumpWith(A)
        totalFistbumps() = 2
        totalMintedFistbumps() = 2
        totalFistbumpsFor(A) = 1
        totalFistbumpsFor(B) = 1
        totalRespondedFistbumpsFor(A) = 1
        totalRespondedFistbumpsFor(B) = 1
        ...
        unrespondedFistbumpedAddressesFor(B) = []
        mintedFistbumpsFor(A) = [1, B]
        mintedFistbumpsFor(B) = [2, A]
        mintedFistbumpedAddressesFor(A) = [B]
        mintedFistbumpedAddressesFor(B) = [A]
        mintedFistbumpTokenIdsFor(A) = [1]
        mintedFistbumpTokenIdsFor(B) = [2]
        fistbumpedAddressesEligableToMintFor(A) = []
        fistbumpedAddressesEligableToMintFor(B) = []
        fistbumpsNotRespondedBy(A) = []
        fistbumpsNotRespondedBy(B) = []
...
contract fistbump is ERC721 {
    ...
    */
    ...
    function safeTransferFrom ( address from, address to, uint256 tokenId ) public virtual override {
        ...
        require ( 1 == 0, "Fistbumps are soulbound.");
