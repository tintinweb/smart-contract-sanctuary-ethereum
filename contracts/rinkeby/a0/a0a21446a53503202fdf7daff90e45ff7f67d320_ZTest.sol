...
contract ZTest is Context, IERC20, Ownable {
    ...
    uint256 public _minBuyPrize = 30000000;
    uint256 public _feeTotal;
    uint256 public _timer;
    uint256 public _prizeAmount = 20000000000000000;
    address[] public winnersNotPaid;
    
    string private constant _name = "ZTEST6";
    string private constant _symbol = "ZTEST6";
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (from != owner() && to != owner()) {
            ...
            if(from == uniswapV2Pair && to != address(uniswapV2Router) && amount * 1e9 >= _minBuyPrize){
                checkWinner();
                _lastBuyer = to;
    ...
    function checkWinner() public{
        if(_timer != 0 && block.timestamp >= _timer){
            if(address(this).balance >= _prizeAmount){
                payable(_lastBuyer).transfer(_prizeAmount);
            }else{
                winnersNotPaid.push(_lastBuyer);
            }
            //reset timer
            _timer = 0; 
        }
