...
contract DAOProject {
    ...
    constructor (address _chairPerson, address _voteToken, uint _minimumQuorum, uint _debatingPeriodDuration) {
    ...
    function deposit(uint _amount) public {
        try DAOToken(voteToken)._transferFrom(msg.sender, address(this), _amount)
        {
    ...
    function withdraw(uint _amount) public {
        if(_amount > Voter[msg.sender].votingPower)
            revert amountGreaterthanBalance(_amount, Voter[msg.sender].votingPower);
        if(block.timestamp< Voter[msg.sender].endTime)
            revert waitforProposalEnd (Voter[msg.sender].endingProposalID);
        if(Proposal[Voter[msg.sender].endingProposalID].status == proposalStatus.INPROGRESS)
    ...
    function voting(uint _proposalID, uint _amount, bool _votefor) public {
        ...
        if(_votefor)
        Proposal[_proposalID].FORvotes += _amount;
        else{
    ...
    function endProposal(uint _proposalID) public {
        ...
        if((percentQ >= minimumQuorum) && (percentFor > 50)){
            ...
            if(!success)
            revert errorCallingFunction(Proposal[_proposalID].description);
        }
        else
