...
contract Ticket is ERC721,Ownable{
    ...
    constructor(string memory uri) ERC721("RandomNft", "NFT"){
        setBaseURI(uri);
    }
    ...
    function startSale() external onlyOwner() {//initiates a 1 day long  sale
    ...
    function mintNft(uint256 quantity) external checkSale(quantity) payable {
        require(quantity<11,"Maximum 10 NFTs per mint so that function doesnt run out of gas");
        require(counter<10001,"Ticket limit reached");
    ...
    function closeSale() external onlyOwner{
        require(undergoingSale() == false,"Timeframe for sale still active");

        uint randomHash = uint(keccak256(abi.encodePacked(block.difficulty, block.timestamp)));
        address payable winner = payable(address(IERC721(address(this)).ownerOf(randomHash % counter)));
