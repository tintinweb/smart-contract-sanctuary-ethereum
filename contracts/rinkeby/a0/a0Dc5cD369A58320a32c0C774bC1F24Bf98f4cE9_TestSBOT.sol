...
contract TestSBOT is ERC721A, Ownable, ReentrancyGuard {
  ...
  // ERC721 params
  string private _name = "Test Bots";
  string private _symbol = "TBOTS";
  string private _baseTokenURI = "";

  uint256 public _tokenId = 0;

  // Withdraw address
  address public withdraw_address = address(0);

  // Collection params
  uint256 private TOTAL_SUPPLY = 100;
  uint256 private RESERVED_SUPPLY = 10;
  uint256 private DUTCH_SUPPLY = 20;


  uint256 public reserved_supply = RESERVED_SUPPLY;
  uint256 public dutch_supply = DUTCH_SUPPLY;
  uint256 public public_supply = TOTAL_SUPPLY - reserved_supply;

  uint256 private DUTCH_START_PRICE = 1 ether;
  uint256 private DUTCH_END_PRICE = 0.2 ether;
  uint256 public saleDuration = 14400; // seconds -> 0.05 ETH every 15 minutes
	uint256 public saleStartTime;

  uint256 private PRESALE_PRICE = 0.15 ether;
  uint256 public public_price = DUTCH_START_PRICE;

  uint256 private MAX_MINT = 10;
  uint256[4] private MAX_PER_STATUS = [0, 3, 1, 3];
  ...
  event ChangedSignerAddress(address newAddress);
  ...
  modifier qtyValidation(uint256 _qty) {
    ...
    require(_qty + _amountMinted[msg.sender][uint256(status)]<= MAX_PER_STATUS[uint256(status)], "Exceeded the max amount of mintable NFT");
    uint256 price = getPrice() * _qty;
    require(msg.value >= price, "Ether sent is not correct");
    _;
    // Refund if they send more than necessary
    if (msg.value > price) {
  ...
  // Constructor
  constructor() ERC721A(_name, _symbol, MAX_MINT , TOTAL_SUPPLY) {}
  ...
  // Dutch auction: Mint price
    ...
    if (status == MintStatus.DUTCH_AUCTION){
      ...
      if (elapsed >= saleDuration) {
        return DUTCH_END_PRICE;
      } else {
        uint256 price = DUTCH_START_PRICE.sub(elapsed.mul(DUTCH_START_PRICE.sub(DUTCH_END_PRICE)).div(saleDuration));
        return price;
      }
    } else if (status == MintStatus.PRESALE){
      return PRESALE_PRICE;
    } else if (status == MintStatus.PUBLIC){
      return public_price;
    } 

    // Default
    return DUTCH_START_PRICE;
	}

  // Owner mint function
  function ownerMint(address[] calldata _address_list, uint256[] calldata _qty_list) external nonReentrant onlyOwner {
    require(reserved_supply > 0, "Collection is sold out");
    require(_address_list.length == _qty_list.length, "The two list must contains the same amount of elements");
    uint256 qty_tot = 0;
    for(uint i; i < _qty_list.length; i ++){
    ...
    for(uint i; i < _address_list.length; i ++){
      _tokenId += _qty_list[i];
  ...
  // Public mint
  function presaleMint(uint256 _qty, string memory _nonce, bytes memory _signature) external payable nonReentrant qtyValidation(_qty) {
      require(status == MintStatus.PRESALE, "Status is not presale");
  
      checkSignature(_qty, _nonce, _signature);
      _privateMint(_qty);
  }

  function mint(uint256 _qty) external payable nonReentrant qtyValidation(_qty){
    require(status == MintStatus.DUTCH_AUCTION || status == MintStatus.PUBLIC, "Status is not dutch auction or public");
    if(status == MintStatus.DUTCH_AUCTION){
      require(dutch_supply > 0, "Dutch auction is sold out");
      require(_qty <= dutch_supply, "Not enough NFTs available");
      require(block.timestamp >= saleStartTime, "Dutch action not started yet"); 

      // Public price will be the one of the last sale during the dutch auction
      public_price = getPrice();
  ...
  function checkSignature(
    ...
    require(
      keccak256(abi.encodePacked(msg.sender, _qty, _nonce))
  ...
  function getStatus() external view returns(string memory status_, uint qty_, uint price_, uint256 available_){
    ...
    if(status == MintStatus.DUTCH_AUCTION){
      return("DUTCH_AUCTION", _remainingToMint, _price, dutch_supply);
    } else if (status == MintStatus.PRESALE){
      return("PRESALE", _remainingToMint, _price, public_supply);
    } else if (status == MintStatus.PUBLIC) {
      return("PUBLIC", _remainingToMint, _price, public_supply);
    } else {
  ...
  function setStatus(uint8 _status) external onlyOwner {
    // _status -> 0: CLOSED, 1: DUTCH_AUCTION, 2: PRESALE, 3: PUBLIC
    require(_status >= 0 && _status <= 3, "Mint status must be between 0 and 3");
  ...
  function setSignerAddress(address _signer_address) external onlyOwner {
    signer_address = _signer_address;
    emit ChangedSignerAddress(_signer_address);
  }

  function setSaleDuration(uint256 _saleDuration) external onlyOwner {
  ...
  IERC721Enumerable
{
  ...
  constructor(
    ...
    collectionSize = collectionSize_ + 1;
  }
