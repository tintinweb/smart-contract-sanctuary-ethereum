 ...
 */
  ...
  mapping(uint256 => TransferredToken) public transfarredLockToken;
  ...
  */
  function registerMetaverse(address _wallet, string memory _name, string memory _metaverseTemplateMetadata) external OnlyAdmin{
  ...
  */
  ...
  function mintNft(uint256 _baseId) external {
    ...
    require(itemNFTIdRangeAndUri[_baseId].mintedCount < itemNFTIdRangeAndUri[_baseId].maxId, "Can't mint anymore please talk to admins!");
    uint256 mintId = itemNFTIdRangeAndUri[_baseId].mintedCount;
    _mint(msg.sender, mintId, 1, "0x00");
    tokenURI[mintId] = itemNFTIdRangeAndUri[_baseId].uri;
    itemNFTIdRangeAndUri[_baseId].mintedCount++;
  }

  function mintAfterTransfer(uint _tokenId, uint256 _baseId, address _toAddress) internal {
    uint _mintId = (_baseId * 1000000) + transferredBaseIdCount[_baseId];
  ...
  */
  ...
  function checkItemBaseId(uint256 _tokenId) external view returns (uint256 _baseID) {
    for (uint256 i = 0; i < itemBaseCountId; i++) {
  ...
  function send(uint16 _dstChainId, bytes calldata _toAddress, uint _tokenId, uint _baseId, uint _amount, address payable _refundAddress, address _zroPaymentAddress, bytes calldata _adapterParam) external payable virtual override {
    _send(_msgSender(), _dstChainId, _toAddress, _tokenId, _baseId, _amount, _refundAddress, _zroPaymentAddress, _adapterParam);
  }
  ...
  function _send(address _from, uint16 _dstChainId, bytes memory _toAddress, uint _tokenId, uint _baseId, uint _amount, address payable _refundAddress, address _zroPaymentAddress, bytes calldata _adapterParam) internal virtual {
    ...
    tokenIds[0] = _tokenId;
    amounts[0] = _amount;
    bytes memory payload = abi.encode(_toAddress, tokenIds,_baseId, amounts);
  ...
  function _nonblockingLzReceive(uint16 _srcChainId, bytes memory _srcAddress, uint64 _nonce, bytes memory _payload) internal virtual override {
    ...
    (bytes memory toAddress, uint[] memory tokenIds,uint baseId, uint[] memory amounts) = abi.decode(_payload, (bytes, uint[],uint, uint[]));
  ...
  function _beforeSend(
    ...
    transfarredLockToken[_tokenId].srcBaseId = this.checkItemBaseId(_tokenId);
    transfarredLockToken[_tokenId].srcTokenId = _tokenId;
    transfarredLockToken[_tokenId].isLocked = true;
  ...
  function _afterReceive(
    ...
    if(transfarredLockToken[_tokenId].isLocked) {
       mintAfterTransfer(_tokenId, _baseId, _toAddress);
    }
    // safeTransferFrom(address(this),_toAddress,_tokenId,_amount,"0x00");
