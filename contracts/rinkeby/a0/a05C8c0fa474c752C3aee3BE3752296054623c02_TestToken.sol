  ...
  ERC721Burnable
{
  ...
  IStatsHandler private _handler;

  constructor(address _handlerAddress) ERC721('TestToken', 'TT') {
    _grantRole(DEFAULT_ADMIN_ROLE, msg.sender);
    _grantRole(PAUSER_ROLE, msg.sender);
    _grantRole(MINTER_ROLE, msg.sender);
    setHandler(_handlerAddress);
  }
  ...
  function setHandler(address handler)
    public
    override
    onlyRole(DEFAULT_ADMIN_ROLE)
  {
    require(handler != address(0), 'TestToken: Handler set to zero address');
    _handler = IStatsHandler(handler);
