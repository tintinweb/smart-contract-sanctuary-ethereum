...
// Claiming -- MOVED TO FINANCEDEPT
    ...
    //     uint _spread = getSpread(_jobId);
    //     uint _level = getLevel(_jobId);
    //     uint _timestamps = timestamps[_jobId];
    //     timestamps[_jobId] = block.timestamp;
    //     financeDept.claimSalary(
    //         _jobId, 
    //         _companyId, 
    //         _baseSalary, 
    //         _spread, 
...
// View Functions
    ...
    function getSalary(uint _jobId) public view returns (uint) {
        ...
        return financeDept.salary(
            _jobId, 
            _companyId, 
            getBaseSalary(_jobId),
            companies.getSpread(_companyId), 
            levels.level(_jobId)
        );
    }
...
// Private
    ...
    function resetJob(uint _jobId) private {    // when jobs are transferred, unassign their regular
        ...
        hasJob[_oldRegId] = false;
        timestamps[_jobId] = block.timestamp;   // Reset timestamp.. worked funds are lost.
    }
...
contract FinanceDept is AccessControl {
    ...
    event ClaimedSalary (uint256 jobId, uint amount);
        ...
        public view returns (uint) 
    {
        ...
        _spread = _spread * 10 ** 18;
        _result = _baseSalary + (uint(_r) % _spread) - (_spread / 2);
        _result = _result * (10 + _level) / 10; // Level Bonus -- TO-DO
        return _result;
    }

    // add 18 zeros
    function unclaimedSalary(
        uint _jobID, 
        uint _companyId, 
        uint _baseSalary, 
        uint _spread, 
        ...
        public view returns (uint) 
    {
        uint _salary = salary(_jobID, _companyId, _baseSalary, _spread, _level);
...
// ADMIN
    ...
    function claimSalary2(uint _jobId) public {
        ...
        uint _timestamps = jobs.getTimestamp(_jobId);
    ...
    {
        uint _amount = unclaimedSalary(_jobID, _companyId, _baseSalary, _spread, _level, _lastClaimed);
