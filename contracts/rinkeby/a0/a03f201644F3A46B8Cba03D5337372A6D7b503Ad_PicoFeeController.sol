...
contract PicoFeeController is Ownable, ReentrancyGuard, Pausable {
    ...
    function _initialProcessing(
        ...
        if (_isETH(_srcToken)) {
            ...
            require(
                msg.value == _calculateAmountWithFees(_amount),
                "less input tokens"
            );
        } else {
            ...
            _processTransferAndApproval(address(_srcToken), _amount);
        }
    }
    ...
    function _processTransferAndApproval(address _token, uint256 _amount)
        internal
    {
        IERC20(_token).safeTransferFrom(
            msg.sender,
            address(this),
            _calculateAmountWithFees(_amount)
        );
