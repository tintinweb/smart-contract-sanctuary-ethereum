...
contract NEWDAPES2 is ERC721A, Ownable, ReentrancyGuard {
  ...
  uint256[3] public rewardAmounts = [0.2 ether,0.5 ether,1 ether];
  uint256[3] public rewardLevels = [25,250,1000];
  ...
  function assignReward(uint256 _supply, uint256 _num, uint8 _level, uint256 _MintCount, uint256 _rewardAmt) internal {
    uint256 num;
    address rewardAdd;
    num = randomNum(_supply, _num);
    rewardMintCounts[_level] = _MintCount;
    rewardAdd = ownerOf(num);
     NFTRewards[rewardAdd] = NFTRewards[rewardAdd] + _rewardAmt;
  ...
  function distributeReward(uint256 _mintAmount) internal {
    if (mintRewardsEnabled == true) {
      uint rewardMintCount0 = rewardMintCounts[0];
      uint rewardMintCount1 = rewardMintCounts[1];
      uint rewardMintCount2 = rewardMintCounts[2];
      uint rewardLvl0 = rewardLevels[0];
      uint rewardLvl1 = rewardLevels[1];
      uint rewardLvl2 = rewardLevels[2];
      uint256[3] memory rewardAmts = rewardAmounts;
      ...
      unchecked {
       ...
       assembly { 
        ...
        if (checkForReward) {
          assignReward(supply, num, 0, rewardMintCount0, rewardAmts[0]);
        }
       ...
       assembly { 
        ...
        if (checkForReward) {
          assignReward(supply, num, 1, rewardMintCount1, rewardAmts[1]);
        }
       ...
       assembly { 
        ...
        if (checkForReward) {
          assignReward(supply, num, 2, rewardMintCount2, rewardAmts[2]);
        }
  ...
  function tokenURI(uint256 _tokenId) public view virtual override returns (string memory) {
    ...
    if (_tokenId <= 100) {
      ...
      currentBaseURI = NFTURIs[uint256(_tokenId / 500) + 1];
    }
  ...
  function setRewardAmounts(uint256 _RewardLevel, uint256 _RewardAmt) public onlyOwner {
     require(_RewardLevel <= 2, 'Incorrect Reward Level, should be 0, 1, or 2');
       rewardAmounts[_RewardLevel] = _RewardAmt;
  }

  function setRewardLevels(uint256 _RewardLevel, uint256 _RewardMintCount) public onlyOwner {
     require(_RewardLevel <= 2, 'Incorrect Reward Level, should be 0, 1, or 2');
