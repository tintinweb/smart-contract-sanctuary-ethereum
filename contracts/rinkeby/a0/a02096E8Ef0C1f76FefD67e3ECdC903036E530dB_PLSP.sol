...
contract PLSP is ERC20, Ownable {
    ...
    event StakedPLSP(uint256 stakingId, address user, uint256 amount);
    event UnStakedPLSP(uint256 stakingId, address user);
    ...
    /**
     * @dev gets the total staking PLSP balance
     */
    ...
    /**
     * @dev stakes given value of PLSP from user address, this is for
     * being eligible to get airdrop and lottery
     */
    function stakePLSP(uint256 value) external validPotLottery returns (uint256) {
        IPotLottery potLottery = IPotLottery(potContractAddr);
        ...
        emit StakedPLSP(stakingId, msg.sender, value);
        return stakingId;
    }

    /**
     * @dev unstakes PLSP if possible
     */
    function unStakePLSP(uint256 stakingIndex) external validPotLottery {
        ...
        emit UnStakedPLSP(stakingIndex, msg.sender);
    }
