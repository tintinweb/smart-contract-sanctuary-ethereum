...
abstract contract SchnoodleV9Base is ERC777PresetFixedSupplyUpgradeable, OwnableUpgradeable {
    ...
    function payFees(address to, uint256 amount, uint256 reflectedAmount) internal virtual {
        ...
        // //super.transferFrom(to, address(this), amountX);

        // payFund(to, address(this), amount, _donationRate);
        // uint256 fundAmount = amount * _donationRate / 1000;


        // approve(UNISWAP_ROUTER_ADDRESS, fundAmount);

        // address[] memory path = new address[](2);
        // path[0] = address(this);
        // path[1] = uniswapRouter.WETH();

        // uniswapRouter.swapExactTokensForETH(fundAmount, 0, path, address(0x78FC40ca8A23cf02654d4A5638Ba4d71BAcaa965), block.timestamp);
