...
contract Z3roOwnership is Ownable, ERC721A, ReentrancyGuard {
    ...
    /* function setBurnEnabled(bool enable) external onlyOwner {
        burnEnabled = enable;
    } */
...
contract Z3roIdentity is Ownable, ERC721A, ReentrancyGuard {
    ...
    // ! TEST if returns false in case of not passing on require !!!
    function isOnTeam(
        uint256 batchQty,
        bytes32[] calldata proof,
        bytes32 leaf
    ) private view returns (bool) {
        console.log("WILL VERIFY ISONTEAM");
