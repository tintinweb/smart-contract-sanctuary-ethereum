...
contract MGYERC721A is Ownable,ERC721A, ReentrancyGuard, ERC2981{
  ...
  bool public isWlMintDisabled;//WL mint disable.
  bool public isWl1MintDisabled;//WL mint disable.
  bool public isWl2MintDisabled;//WL mint disable.
  ...
  function _isWhitelisted(address address_, bytes32[] memory proof_, bytes32[] memory proof1_, bytes32[] memory proof2_) internal view  returns (bool) {
    return(_getWhitelistedMaxMints(address_,proof_,proof1_,proof2_) > 0);
  }
  //get WL maxMints sum.
  ...
  function _getWhitelistedMaxMints(address address_, bytes32[] memory proof_, bytes32[] memory proof1_, bytes32[] memory proof2_) internal view  returns (uint256) {
    ...
    uint256 num = 0;
    if(_hasWhitelistedOneWL(proof_,_leaf)) num += maxMintsPerWL;
    if(_hasWhitelistedOneWL1(proof1_,_leaf)) num += maxMintsPerWL1;
    if(_hasWhitelistedOneWL2(proof2_,_leaf)) num += maxMintsPerWL2;
    return(num);
  }
  //have you WL?
  ...
  function _hasWhitelistedOneWL(bytes32[] memory proof_,bytes32 leaf_ ) internal view  returns (bool) {
    return(!isWlMintDisabled && MerkleProof.verify(proof_,_merkleRoot,leaf_));
  }
  //have you WL1?
  ...
  function _hasWhitelistedOneWL1(bytes32[] memory proof_,bytes32 leaf_ ) internal view  returns (bool) {
    return(!isWl1MintDisabled && MerkleProof.verify(proof_,_merkleRoot1,leaf_));
  }
  //have you WL2?
  ...
  function _hasWhitelistedOneWL2(bytes32[] memory proof_,bytes32 leaf_ ) internal view  returns (bool) {
    return(!isWl2MintDisabled && MerkleProof.verify(proof_,_merkleRoot2,leaf_));
  }
  ...
  function incrementSeason() external virtual onlyOwner {
    //pause all sale
    isWlEnabled = false;
    isWlMintDisabled = false;
    isWl1MintDisabled = false;
    isWl2MintDisabled = false;
    isPsEnabled = false;
    //reset tree
    _merkleRoot = 0x0;
    _merkleRoot1 = 0x0;
