...
// Authorized minting contract with set price
...
contract DVinMint is EIP712Whitelisting {
    ...
    mapping(uint256 => uint256) public opensalePrices;

    constructor(address _dvinMembership, address payable _sink)
        EIP712Whitelisting("DVin")
    {
        _setupRole(MINTER_ROLE, msg.sender);
        _setupRole(OWNER_ROLE, msg.sender);
        _setupRole(DEFAULT_ADMIN_ROLE, msg.sender);
        dvin = ITieredNFT(_dvinMembership);
        ethSink = _sink;
    }
    ...
    function setContractState(ContractState _state, bool _enabled) external {
    ...
    function setPrice(
        ...
        require(hasRole(OWNER_ROLE, msg.sender), "!owner");
        presalePrices[_tier] = _presalePrice;
        opensalePrices[_tier] = _opensalePrice;
    }
    ...
    ) external payable requiresWhitelist(_signature, _tier, _nonce) {
        require(contractState[ContractState.Presale], "!round");
        require(presalePrices[_tier] > 0, "Invalid Tier"); /*Do not allow 0 value purchase. If desired use separate function for free claim*/
        require(msg.value >= presalePrices[_tier]);

        (bool _success, ) = ethSink.call{value: msg.value}(""); /*Send ETH to sink first*/
        require(_success, "could not send");

        dvin.mintTier(_tier, msg.sender);
    }

    function purchaseTokenOpensale(uint256 _tier) external payable {
        require(contractState[ContractState.Public], "!round");
        require(opensalePrices[_tier] > 0, "Invalid Tier"); /*Do not allow 0 value purchase. If desired use separate function for free claim*/
        require(msg.value >= opensalePrices[_tier]);
...
contract EIP712Whitelisting is AccessControl, EIP712Base {
    ...
    bytes32 public constant WHITLISTING_ROLE = keccak256("WHITELISTING_ROLE");
    ...
    bytes32 public constant MINTER_TYPEHASH =
        keccak256("Minter(address wallet,uint256 tier,uint256 nonce)");

    // todo add quantity?

    constructor(string memory name) EIP712Base(name) {
        _setupRole(WHITLISTING_ROLE, msg.sender);
    }
    ...
    modifier requiresWhitelist(
        ...
        require(
            hasRole(WHITLISTING_ROLE, recoveredAddress),
            "Invalid Signature"
        );
        _;
    }
