...
contract Staking is Initializable, ReentrancyGuardUpgradeable, OwnableUpgradeable, PausableUpgradeable {
    ...
    /// @dev Token rewards per block for staking
    uint256 public rewardsPerBlockForStaking;

    /// @dev Token rewards acculmation as the pool share holding
    uint256 public rewardsAccumalatedPerShare;
     ...
     * @param _rewardsPerBlockStaking array of rewards per block for staking
    ...
    function initialize(
        address _stakeToken,
        uint256 _rewardsPerBlockStaking,
        uint256 _startBlock,
        uint256 _period,
        uint256 _circulatingSupply
    ) external initializer {
        require(_stakeToken != address(0), "Invalid stake token address");
        require((_rewardsPerBlockStaking > 0 && _period > 0 && _circulatingSupply > 0), "Invalid params");
        ...
        rewardsPerBlockForStaking = _rewardsPerBlockStaking;
        endBlock = _startBlock + (_period * 365 days);
        lastRewardBlock = _startBlock;
    }

    function setRewardPerBlock(uint256 _rewardPerBlock) external onlyOwner {
        rewardsPerBlockForStaking = _rewardPerBlock;
    }
    ...
    function deposit(uint256 amount) external nonReentrant {
        ...
        uint256 pendingRewards = stakers[_msgSender()].reward;
     ...
     */
    function compound() internal nonReentrant {
    ...
    function calculatePendingRewards(address user) external view returns (uint256) {
        if ((block.number > lastRewardBlock) && (totalAmountStaked != 0)) {
            ...
            uint256 tokenRewardForStaking = multiplier * rewardsPerBlockForStaking;
