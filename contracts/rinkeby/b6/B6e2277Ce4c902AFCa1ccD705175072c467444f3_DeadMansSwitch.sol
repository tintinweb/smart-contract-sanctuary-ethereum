...
contract DeadMansSwitch {
    ...
    struct Switch {
        ...
        uint256 downCadence;
        // next block where the last heartbeat was signed & or switch was created
        uint256 blockHeartbeat;
        // symmetric key from the lit protocol encryption
        string symmetricKey;
        // simple name of the switch
        string name;
        // url pointing to the uploaded file that is secured by the DMS
        string fileUrl;
    ...
    event SwitchMinted(uint256 switchId, uint256 blockHeartbeat);
    ...
    event ChangedCadence(uint256 switchId, uint256 newCadence, uint256 newDownCadence);
    ...
    constructor() {
        console.log("Deploying the DeadMansSwitch Contract; Success:", true);
    }
    ...
    function mint(
        ...
        emit SwitchMinted(switchId, switches[switchId].blockHeartbeat);
        return switchId;
    }

    function update(
        uint256 _switchId,
        string calldata _symmetricKey,
        ...
        switches[_switchId].symmetricKey = _symmetricKey;
    ...
    function heartbeat(uint256 _switchId) external returns (bool) {
        ...
        uint256 startWindowBlock = switches[_switchId].blockHeartbeat + switches[_switchId].downCadence;
        if (blockNo > endWindowBlock) {
            revert ManIsUnfortunatelyAlreadyDead();
        } else if (blockNo < startWindowBlock) {
            revert SwitchNotReadyYet();
        } else {
            switches[_switchId].blockHeartbeat = blockNo;
    ...
    function changeCadence(
        ...
        if (blockNo > endWindowBlock) {
            ...
            switches[_switchId].downCadence = _newDownCadence;
            console.log(
                "Successful change of cadences to",
                switches[_switchId].cadence,
                switches[_switchId].downCadence
            );
            emit ChangedCadence(_switchId, switches[_switchId].cadence, switches[_switchId].downCadence);
            return true;
        }
    }
