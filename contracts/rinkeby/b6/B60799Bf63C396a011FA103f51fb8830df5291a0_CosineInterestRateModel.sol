...
contract CosineInterestRateModel is InterestRateModelBase, Ownable {
    ...
    // Event emitted when new zero rate is set
    event ZeroRateSet(uint256 zeroRate_);

    // Event emitted when new full rate is set
    event FullRateSet(uint256 fullRate_);

    // Event emitted when new kink is set
    event KinkSet(uint256 kink_);

    // Event emitted when new kink rate is set
    event KinkRateSet(uint256 kinkRate_);
    ...
    /// @notice This function is used by governor to set new zero rate
    /// @param zeroRate_ New zero rate value
    function setZeroRate(uint256 zeroRate_) external onlyOwner {
        zeroRate = zeroRate_;
        emit ZeroRateSet(zeroRate_);
    }

    /// @notice This function is used by governor to set new full rate
    /// @param fullRate_ New full rate value
    function setFullRate(uint256 fullRate_) external onlyOwner {
        fullRate = fullRate_;
        emit FullRateSet(fullRate_);
    }

    /// @notice This function is used by governor to set new kink
    /// @param kink_ New kink value
    function setKink(uint256 kink_) external onlyOwner {
        require(kink_ <= ONE.toUint256(), "GTO");
    ...
    /// @notice This function is used by governor to set new kink rate
    /// @param kinkRate_ New kink rate value
    function setKinkRate(uint256 kinkRate_) external onlyOwner {
        kinkRate = kinkRate_;
        emit KinkRateSet(kinkRate_);
    }
}
