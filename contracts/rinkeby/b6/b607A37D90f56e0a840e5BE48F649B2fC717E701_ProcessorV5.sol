...
/**
 * @title Processor V5
 * @dev Transfer to ETH rewards, team and control daily emission
 */
contract ProcessorV5 is Ownable {
    using SafeERC20 for IERC20;
    using SafeMath for uint256;

    address public constant CYCLE =
        address(0x4117C0C65411c90fA399a223F8B4197d34C36bac);
    address public constant WETH =
        address(0xc778417E063141139Fce010982780140Aa0cD5Ab);
    uint256 public constant BP_DIV = 10000;

    address public ETHRewards;
    ...
    event ETHRewardsUpdated(address ETHRewards);
    ...
    function setETHRewards(address _ETHRewards) external onlyOwner {
        ETHRewards = _ETHRewards;
        emit ETHRewardsUpdated(ETHRewards);
    }
    ...
    function process() external onlyProxy {
        ...
        if (balanceETH > 0) {
            IWETH(WETH).deposit{value: balanceETH}();
        }

        uint256 balWETH = balanceWETH();
        uint256 teamFee = balWETH.mul(teamBP).div(BP_DIV);
        IERC20(WETH).safeTransfer(Team, teamFee);
        uint256 rewardAmount = balanceWETH();
        IERC20(WETH).safeTransfer(ETHRewards, rewardAmount);
        IStakingRewards(ETHRewards).notifyRewardAmount(rewardAmount);
