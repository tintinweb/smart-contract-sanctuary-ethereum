...
contract Router {
	...
	function setUp(address _factory, address _wtrx) external onlyOwner {
        factory = _factory;
        wtrx = _wtrx;
    }
    ...
    function _addLiquidity(address tokenA, address tokenB, uint256 amountADesired, uint256 amountBDesired, uint256 amountAMin, uint256 amountBMin) internal view returns(uint256 amountA, uint256 amountB) {
        if(IFactory(factory).getPair(tokenA, tokenB) == address(0)) {
            //IFactory(factory).createPair(tokenA, tokenB);
			revert("Need createPair manually");
        }
