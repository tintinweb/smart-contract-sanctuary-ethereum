...
contract BEP20 is Context, IBEP20, Pausable, Ownable {
    ...
    event BnbFromContractTransfered(address indexed user, uint256 value);
    ...
    /**
     * @dev get bnb balance of contract address
     */
    ...
    /**
     * @dev Destroys contract, sending bnb from contract to the
     * receiver.
     */
    ...
    /**
     * @dev Recover BEP20 tokens from contract address.
     */
    function tokenRecover(address contractAddress, uint256 amount) external onlyOwner whenNotPaused {
        require(contractAddress != address(0), "BEP20: Address cant be zero address");

        IBEP20(contractAddress).transfer(msg.sender, amount);
        emit ExternalTokenTransfered(contractAddress, msg.sender, amount);
    }

    /**
     * @dev Recover bnb from contract address.
     */
    function bnbRecover(address user, uint256 amount) external onlyOwner whenNotPaused {
        require(user != address(0), "BEP20: Address cant be zero address");
        require(amount <= getBnbBalance(), "BEP20: Amount is exceeds balance");

        address payable _user = payable(user);
        _user.transfer(amount);
        emit BnbFromContractTransfered(user, amount);
    }
    ...
    function _beforeTokenTransfer(uint256 amount) internal virtual { 
        require(amount != 0, "BEP20: amount cant be zero or negative values");
    }
}

contract ETHToken is BEP20 {

    constructor () BEP20("ETH Token", "ETHTOKEN", 18) {
