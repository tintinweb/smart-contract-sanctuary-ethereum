...
contract VoteBoxV1 is IVoteBox, Initializable, UUPSUpgradeable, AdminableUpgradeable, EIP712VoteAllowlistUpgradeable {
        ...
        onlyProxy 
    {
        ...
        addrVote.score = uint128(_getVoterScore(polls[_pid].tokenContract, voterAddr));
    ...
    /**
     * @dev Retuns the score for a voter address
     */
    ...
    /**
     * @dev Returns the score from external token contract for an owner address
     */
    function _getVoterScore(address _tokenContract, address _ownerAddress) internal view returns (uint256) {
        try IScore(_tokenContract).scoreByAddress(_ownerAddress) returns (uint256 score) {
            return score;
        } catch (bytes memory) {
            // when reverted, just returns...
            return 1;
