...
contract FixedAssertsFactory is Ownable {
    ...
    mapping(string => CofferIface.Receipt) private receipts;
    ...
    function getCreationBytecode(
        NewAssertStruct.Assert memory newAssert
    )
    internal
    pure
    ...
    returns (address) {
        ...
        NewAssertStruct.Charge memory chargeFee;
        ...
        require(manager == ecrecovery(hash, managerSign), "Manager signature error");
    ...
    returns (bool) {
        ...
        CofferIface.Receipt storage receipt;
        ...
        if (chargeFee.feeToken == address(0)) {
            ...
            require(CofferIface(coffer).sendReceipt(receipt), "Receipt sending failed");
    ...
    returns (
        ...
        CofferIface.Receipt memory receipt = receipts[receiptNum_];
    ...
    returns (bytes32 hash){
        bytes memory message = abi.encode(newAssert.name, newAssert.symbol, newAssert.baseURI, newAssert.totalSupply, newAssert.repurchaseRatio, newAssert.repurchasePeriod, ChargeFee.feeToken, ChargeFee.feeAmount, msg.sender);
...
interface CofferIface {
    ...
    function sendReceipt(Receipt calldata receipt) external returns (bool);
}

interface NewAssertStruct {
    struct Assert {
        string name;
        string symbol;
        string baseURI;
...
contract FixedAsserts is ERC721 {
    ...
    event Cast(uint256 tokenId, address initialOwner, address coffer, string receiptNum, address operator, address NFTAddr, uint256 NFTId, address tokenAddr, uint256 tokenAmount, uint256 serviceFee, uint256 timestamp);
    ...
    public
    payable {
        NewAssertStruct.Token memory token;
        token.to = tx.origin;
    ...
    */
    ...
    returns (bool) {
        ...
        emit Cast(token.tokenId, tx.origin, address(coffer), receipt.receiptNum, receipt.operator, receipt.NFTAddr, receipt.NFTId, receipt.tokenAddr, receipt.tokenAmount, receipt.serviceFee, receipt.timestamp);
        return true;
    }
