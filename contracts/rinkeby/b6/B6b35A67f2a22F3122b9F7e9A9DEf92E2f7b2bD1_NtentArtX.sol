...
contract NtentArtX is ERC721A {
    ...
    function projectIdToTokenIds(uint256 _projectId) public view returns (uint256[] memory){

        uint256 current = 0;
        uint256[] memory tokenIds;
        uint256 tokenId;
        for(uint256 i = 0; i < projects[_projectId].invocations; i++){  
            tokenId = (_projectId * ONE_MILLION) + i;

            if(!isBurned(tokenId)){
                tokenIds[current] = tokenId;
                current++;
            }
        }
        return tokenIds;
