...
contract ImpactTwo is Ownable, AdminControl {
    ...
    uint256 price = 0.042 ether;

    bool isEnabled = false;

    uint256 startBlockTimestamp = type(uint256).max;
    uint256 endBlockTimestamp = type(uint256).max;

    constructor(address _creator, uint8 _tokenId) {
        require(IERC1155CreatorCore(_creator).totalSupply(_tokenId) > 0, "Token ID not initialised yet");
