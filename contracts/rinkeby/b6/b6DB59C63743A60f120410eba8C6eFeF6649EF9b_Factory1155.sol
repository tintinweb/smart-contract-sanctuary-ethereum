...
contract Factory1155 {
	...
	function deploy(bytes32 _salt, string memory name, string memory symbol, string memory tokenURIPrefix) external returns(address addr) {
		addr = address(new GDEXUserToken1155{salt: _salt}(name, symbol, tokenURIPrefix));
		GDEXUserToken1155 token = GDEXUserToken1155(address(addr));
...
contract GDEXUserToken1155 is ERC1155 {
