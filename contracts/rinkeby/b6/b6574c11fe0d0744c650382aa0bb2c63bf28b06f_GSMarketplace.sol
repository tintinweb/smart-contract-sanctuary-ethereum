    ...
    OwnableUpgradeable
{
    ...
    /**
     * @dev Address of WETH
     *///pass it thriugh constructor later
    ...
    function getListingPriceItemDetails(address _nft, uint128 _tokenId) external view returns (ListInfo[] memory){
        uint256 resultCount;

        for(uint256 i=0; i<listInfo.length; i++){
            if(listInfo[i]._nft == _nft && listInfo[i]._tokenId == _tokenId ){
                resultCount++;
            }
        }
