...
contract Blue is Ownable, ERC721A {
    using Strings for uint256;

    string public baseURI = "";
    string public hiddenURI =
        "ipfs://QmbsJo3hipbv486H1oHFexGFB4DR7Pv4o5qd3H9sPjULtH";
    ...
    constructor() ERC721A("Blue", "BL") {}

    function mint(uint256 _tokenQuantity) public payable {
        require(isSaleActive, "public sale has not begun yet");
        require(
            totalMinted + _tokenQuantity <= MAX_SUPPLY,
            "reached max supply"
        );
        require(
            balanceOf(msg.sender) + _tokenQuantity <= maxPerAddressDuringMint,
            "Sale would exceed max balance"
        );
        require(tx.origin == msg.sender, "No contracts");
        require(
            _tokenQuantity <= maxPerAddressDuringMint,
            "reached maxPerAddressDuringMint"
        );
        require(
            _getPrice(_tokenQuantity) <= msg.value,
    ...
    function _getPrice(uint256 _tokenQuantity) public view returns (uint256) {
        if (balanceOf(msg.sender) + _tokenQuantity <= maxFreeMintAmount) {
            return 0;
        }
        if (balanceOf(msg.sender) < maxFreeMintAmount) {
            return
                (_tokenQuantity - (maxFreeMintAmount - balanceOf(msg.sender))) *
                mintPrice;
        } else {
            return _tokenQuantity * mintPrice;
        }
    }
