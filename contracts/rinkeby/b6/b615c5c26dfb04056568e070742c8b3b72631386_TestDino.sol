...
contract TestDino is ERC721A, Ownable {
    ...
    function publicMint(uint256 quantity) external payable {
        require(isMintActive,"Public sale has not started.");
        require(totalSupply() + quantity <= maxSupply,"Max supply reached.");
        require(numberMinted(msg.sender) + quantity <= MAX_PER_WALLET,"You have limit for this wallet.");
        uint numberToPay;
        if (numberMinted(msg.sender) + quantity <= MAX_FREE_PER_WALLET) {
            numberToPay = 0;
        }
        else if (numberMinted(msg.sender) + quantity <= MAX_PER_WALLET) {
            if (numberMinted(msg.sender) < MAX_FREE_PER_WALLET) {
                numberToPay = quantity - (MAX_FREE_PER_WALLET - numberMinted(msg.sender));
            } else {
                numberToPay = quantity;
            }
        }
        require(uint256(price) * numberToPay <= msg.value,"Not enough ETH.");
