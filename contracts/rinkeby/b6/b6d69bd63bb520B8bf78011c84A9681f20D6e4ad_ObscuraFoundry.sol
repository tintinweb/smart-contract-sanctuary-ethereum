...
contract ObscuraFoundry is ERC721X, AccessControlEnumerable, IERC2981, IRNGrequestor, randomiser {
    ...
    constructor(address admin, address payable obscuraAddress, IRNG _rng)
        ERC721X("Obscura Magnum", "OM") randomiser(1)
    {
...
// SPDX-License-Identifier: MIT
// OpenZeppelin Contracts v4.4.1 (token/ERC721/ERC721X.sol)
 ...
 * {ERC721Enumerable}.
 */
contract ERC721X is ERC165, IERC721, IERC721Metadata {
   ...
   // using Strings for uint256;
    ...
    function _reassign(uint256 placeHolder, uint256 finalTokenId) internal {
        require(!_exists(finalTokenId),"final slot occupied");
        //require(_exists(placeHolder),"placeholder does not exist");
        address _owner =  _owners[placeHolder];
        (_owners[finalTokenId],_owners[placeHolder]) = (_owner,address(0));
        //(_tokenApprovals[finalTokenId],_tokenApprovals[placeHolder]) = (address(0),_tokenApprovals[placeHolder]);
        emit Transfer(_owner, address(0), placeHolder);
        emit Transfer(address(0),_owner,finalTokenId);
    }
...
contract randomiser {
    ...
    function randomTokenURI(uint256 projectID, uint256 rand) internal returns (uint256) {
        ...
        uint128 endval = getTID(projectID,ntl-1);
