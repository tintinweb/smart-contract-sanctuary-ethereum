...
abstract contract EthboxMetadata {
    function buildMetadata(address owner, EthboxStructs.Message[] memory messages, uint256 ethboxSize) public view virtual returns (string memory data);
}
...
contract Ethbox is ERC165, IERC721, IERC721Metadata, Ownable {
    ...
    uint256 public constant defaultDurationTime = 4 weeks;
    ...
    EthboxMetadata metadata;

    mapping(address => EthboxStructs.Message[]) public inboxMessages;
    mapping(address => uint256) public inboxSizes;
    mapping(address => uint256) public inboxDurationTimes;
    mapping(address => address) public inboxPayoutRecipients;
    mapping(address => bool) public inboxIsLocked;
    ...
    function tokenURI(uint256 _tokenId) public view override returns (string memory) {
        ...
        address owner = address(uint160(_tokenId));
        EthboxStructs.Message[] memory messages = getOrderedMessages(owner);
        uint256 ethboxSize = inboxSizes[owner];
    ...
    function mintMyInbox() public {
        ...
        inboxDurationTimes[msg.sender] = defaultDurationTime;
    ...
    function getClaimableValue(EthboxStructs.Message memory _message, uint256 _duration) public view returns(uint256 _claimableValue) {
        uint256 elapsedSeconds = block.timestamp - _message.timestamp;
        if (elapsedSeconds < 1) return 0;
        if (elapsedSeconds > _duration) return (_message.originalValue - _message.claimedValue);
        uint256 bps = (elapsedSeconds * 100) / _duration;
        uint256 subValue = (getOriginalValueMinusFees(_message) * bps) / 100;
        return (subValue - _message.claimedValue);
    }

    function getOriginalValueMinusFees(EthboxStructs.Message memory _message) private pure returns (uint256) {
        return ((_message.originalValue * (10000 - _message.feeBPS)) / 10000);
    }

    function getRemainingOriginalValue(EthboxStructs.Message memory _message) private pure returns (uint256) {
    ...
    function sendMessage(address _to, string calldata _message) public payable{
        ...
        require(!inboxIsLocked[_to], "inbox is locked");
        require(msg.value > 10000 || msg.value == 0, "message value incorrect");
        EthboxStructs.Message[] memory toMessages = inboxMessages[_to];
        ...
        if (minted[_to]){
            if (dynamicBoxSize < inboxSizes[_to]) {
                ...
                (bool qualifies, uint256 indexToRemove) = _getIndexToReplace(toMessages, dynamicBoxSize, msg.value);
                if (qualifies){
    ...
    function _getIndexToReplace(EthboxStructs.Message[] memory _toMessages, uint256 _boxSize, uint256 _value) private pure returns (bool qualifies, uint256 indexToRemove) {
        uint256 lowIndex;
        uint256 lowValue = _toMessages[0].originalValue;
        uint256 durationedValue;
        for (uint256 i=0; i< _boxSize; i++) {
            durationedValue = _toMessages[i].originalValue;
            if (durationedValue < lowValue) {
                lowIndex = i;
                lowValue = durationedValue;
            }
            if ( qualifies == false && _value > durationedValue) {
                qualifies = true;
            }
        }
    ...
    function _pushMessage(address _to, address _from, string calldata _message, uint256 _value, uint256 _index) private {
        inboxMessages[_to].push(EthboxStructs.Message(
            _from,
            _message,
            block.timestamp,
            _value,
    ...
    function _insertMessage(address _to, address _from, string calldata _message, uint256 _value, uint256 _index) private {
        inboxMessages[_to][_index] = EthboxStructs.Message(
            _from,
            _message,
            block.timestamp,
            _value,
    ...
    function _payRecipient(address _to, uint256 _value) private {
        (bool successRecipient, )= inboxPayoutRecipients[_to].call{value: _value}("");
    ...
    function _refundSender(EthboxStructs.Message memory _message) private {
        ...
        (bool successRefund, )= _message.from.call{value: refundValue }("");
        require(successRefund);
    }

    function getOrderedMessages(address _to) public view returns (EthboxStructs.Message[] memory) {
        EthboxStructs.Message[] memory messages = inboxMessages[_to];
    ...
    function setInboxIsLocked(bool _isLocked) external {
        require(minted[msg.sender], "inbox needs to be minted");
        inboxIsLocked[msg.sender] = _isLocked;
    }
    ...
    function changeInboxDurationTime(uint256 _durationTime) external {
        require(minted[msg.sender], "inbox needs to be minted");
        require(inboxIsLocked[msg.sender], "inbox needs to be locked");
        require(inboxMessages[msg.sender].length == 0, "inbox needs to be empty");
        inboxDurationTimes[msg.sender] = _durationTime;
    }
    ...
    function removeAll() external {
        ...
        for (uint256 i = 0; i< inboxMessages[msg.sender].length ; i++){
            claimValue = getClaimableValue(inboxMessages[msg.sender][i], inboxDurationTimes[msg.sender]);
            totalValue += claimValue;
            inboxMessages[msg.sender][i].claimedValue += claimValue;
            _refundSender(inboxMessages[msg.sender][i]);
            _removeMessage(msg.sender, i);
        }
    ...
    function removeOne(uint256 _index) external {
        require(minted[msg.sender], "inbox needs to be minted");
        uint256 claimValue = getClaimableValue(inboxMessages[msg.sender][_index], inboxDurationTimes[msg.sender]);
        inboxMessages[msg.sender][_index].claimedValue += claimValue;
        _refundSender(inboxMessages[msg.sender][_index]);
    ...
    function getClaimableValueOfEthbox(address _ethboxAddress) public view returns (uint256){
        uint256 claimValue = 0;
        uint256 totalValue = 0;
        for (uint256 i = 0; i< inboxMessages[_ethboxAddress].length ; i++){
            claimValue = getClaimableValue(inboxMessages[_ethboxAddress][i], inboxDurationTimes[_ethboxAddress]);
            totalValue += claimValue;
        }
        return totalValue;
    }
    ...
    //TODO add message removal on full claim of message value
}
