...
contract Maze is BinaryTree{
    ...
    function iterateTree(Directions direction) public{
        ...
        if(direction == Directions.Right){
            require(node.right != bytes32(0),"Right is blocked");
            ...
            {
                require(node.left != bytes32(0),"Left is blocked");
                node = tree[node.left];
                path.push(node);
            }else{
    ...
    function checkWinner() internal{
        if(node.value == Treasure){
            ...
            payable(msg.sender).transfer(7 * 10 ** 17);
            emit Winner(msg.sender);
        }
    }
    function setTreasure(uint256 treasure) public{
        Treasure = treasure;
    }
    function startGame() public payable {
        require(msg.value >= 5 * 10 ** 17,"You should pay 0.5 eth to play");
