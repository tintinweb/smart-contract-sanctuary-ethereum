    ...
    PaymentSplitter
{
    ...
    function presaleMint(address _to, uint256 quantity, bytes32[] calldata _proof)
    external
    payable
    isValidMerkleProof(_proof)
    onlyAccounts
    {
        require(msg.sender == _to,          "Grump Lumps: Not allowed");
        require(presaleM,                       "Grump Lumps: Presale is OFF");
        require(!paused,                        "Grump Lumps: Contract is paused");
        require(
            quantity <= presaleAmountLimit,      "Grump Lumps: You can not mint that many tokens");
        require(
            _presaleClaimed[msg.sender] + quantity <= presaleAmountLimit,  "Grump Lumps: You can not mint that many tokens");


        uint current = _tokenIds.current();

        require(
            current + quantity <= maxSupply,
    ...
    onlyAccounts
    {
        ...
        require(!paused, "Grump Lumps: Contract is paused");
        require(quantity > 0, "Grump Lumps: zero amount");
