...
contract StakingContract {
    ...
    function CreateBattle(uint256 _amount, uint256 _riskPercentage, address referalPerson) external {
       ...
       // require((joinWith == 1 || joinWith == 2),"You can only create with either 1 or 2.");
        ...
        if(referalPerson != 0x0000000000000000000000000000000000000000){
            player.refered = true;
        }
        ...
        player.battleCount++;
        player.playerstartingTime = block.timestamp;
    ...
    function JoinBattle(uint256 _amount, uint256 _battleId, address referalPerson) public {
        ...
        if(referalPerson != 0x0000000000000000000000000000000000000000){
            ...
            uint256 referalAmount = calculateReferalPercentage(joinerDeductedAmount);
            genToken.transfer(referalPerson,referalAmount);

            uint256 sendJoinerDeductionAmount = joinerDeductedAmount - referalAmount;
            genToken.transfer(treasuryWallet, sendJoinerDeductionAmount);

            player.referalAmount += referalAmount;
        }
        else{
            genToken.transfer(treasuryWallet, joinerDeductedAmount);
        }

        if(players[battle.creator].refered){
            
            uint256 referalAmount = calculateReferalPercentage(creatorDeductedAmount);
            genToken.transfer(referalPerson,referalAmount);

            uint256 sendCreatorDeductionAmount = creatorDeductedAmount - referalAmount;
            genToken.transfer(treasuryWallet, sendCreatorDeductionAmount);

            players[battle.creator].referalAmount += referalAmount;
        }
        else{
    ...
    function LeaveBattle(uint256 count) public {
       ...
       Battle storage battle = battles[count];
        ...
        if(!battle.joined){
            ...
            require(block.timestamp > (players[msg.sender].playerstartingTime + 3 minutes),"You haveto wait atleast 3 minutes to leave battle if no one join the battle.");
    ...
    function Withdraw() external {
        ...
        require(lastTransaction[msg.sender] < block.timestamp,"You canot withdraw amount before 24 hours");
      ...
      //  player.totalArenaTokens -= sendAreenaReward;
        ...
        lastTransaction[msg.sender] = block.timestamp + 24 hours;
