...
contract Bear is ERC721AQueryable, BaseTokenURI {
  ...
  function publicMint(uint256 _mintAmount) external payable {
    ...
    require(totalMints[msg.sender] + _mintAmount <= maxAllowance, "Bear: Purchase would exceed max allowance");
    require(mintNonce[msg.sender] == 0, "Bear: You have already minted in the whitelist phase");
  ...
  function mint(
    ...
    require(!isPhaseRunning[PUBLIC_MINT_PHASE], "Whitelist Mint is not allowed while Public Mint is running");
    ...
    if (totalMints[msg.sender] + _mintAmount == maxAllowance) {
      mintNonce[msg.sender] += 1;
      totalMints[msg.sender] = 0;
    } else {
      totalMints[msg.sender] += _mintAmount;
    }
  }
