...
error exceedMaxSupply(uint256 currentSupply);
error exceedMintLimit(uint256 currentMintCout);
error mintNotStart();
...
contract Onitama is Ownable, ERC721A {
    ...
    modifier checkValidMint(uint256 quantity) {
        ...
        if (totalSupply() + quantity > maxSupply)
            revert exceedMaxSupply(totalSupply());
        if (addressMintedCount[msg.sender] + quantity > maxBatchSize)
            revert exceedMintLimit(addressMintedCount[msg.sender]);
    ...
    function whitelistMint(
        ...
        if (!isAuthorized(msg.sender, v, r, s)) revert invalidSignature();
    ...
    function isAuthorized(
        ...
        bytes32 hash = keccak256(abi.encode(sender));
