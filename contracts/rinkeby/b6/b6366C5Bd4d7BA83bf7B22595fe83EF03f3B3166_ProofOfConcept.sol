...
contract ProofOfConcept is ERC1155Supply, Ownable, Pausable, ReentrancyGuard {
    ...
    uint256 public maxAllowlistMint = 10;
    uint256 public maxPublicMint = 5;

    bytes32 public allowlistMerkleRoot;

    uint256 public allowlistPrice = 0.12 ether;
    ...
    Phase public currentPhase = Phase.ENDED;
    ...
    // mint functions
    function _isMintable(uint256 id, uint256 quantity, uint256 price, uint256 limit) internal {
    ...
    function devMint(uint256 id, uint256 quantity) external onlyOwner {
        _isMintable(id, quantity, 0, maxSupply[id]);
    ...
    function allowlistMint(uint256 id, uint256 quantity, bytes32[] calldata _merkleProof) external payable whenNotPaused {
        require(currentPhase == Phase.ALLOWLIST, "Minting is not yet started");
        _isMintable(id, quantity, allowlistPrice, maxAllowlistMint);
    ...
    function publicMint(uint256 id, uint256 quantity) external payable whenNotPaused {
        require(currentPhase == Phase.PUBLIC, "Minting is not yet started");
        _isMintable(id, quantity, publicPrice, maxPublicMint);
