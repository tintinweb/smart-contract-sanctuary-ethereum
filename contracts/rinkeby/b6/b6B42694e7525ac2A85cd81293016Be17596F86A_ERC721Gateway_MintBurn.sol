...
abstract contract ERC721Gateway is IERC721Gateway, AnyCallApp {
    BlocoIMintBurn721 internal _token;
    uint256 public swapoutSeq;
    string public name = "bloco.exchange - NFT bridge";
    ...
    constructor (address anyCallProxy, uint256 flag, BlocoIMintBurn721 token_) AnyCallApp(anyCallProxy, flag) {
        setAdmin(msg.sender);
        _token = token_;
    }
    ...
    function _swapin(uint256 num_tokens, address receiver, bytes memory extraMsg) internal virtual returns (bool);
    ...
    function Swapout(uint256 tokenId, address receiver, uint256 destChainID) external payable returns (uint256) {
        uint256 num_tokens = _token.numTokens(tokenId);
    ...
    function _anyExecute(uint256 fromChainID, bytes calldata data) internal override returns (bool success, bytes memory result) {
        (uint256 num_tokens, , address receiver,,bytes memory extraMsg) = abi.decode(
            data,
            (uint256, address, address, uint256, bytes)
        );
        require(_swapin(num_tokens, receiver, extraMsg));
    }
...
interface BlocoIMintBurn721 {
    function ownerOf(uint256 tokenId) external view returns (address owner);
    function mint(address account, uint256 num_tokens) external;
    function burn(uint256 tokenId) external;
    function numTokens(uint256 tokenId) external view returns (uint256 num_tokens);
...
contract ERC721Gateway_MintBurn is ERC721Gateway {
    ...
    constructor (address anyCallProxy, uint256 flag, BlocoIMintBurn721 _token) ERC721Gateway(anyCallProxy, flag, _token) {}

    function _swapout(uint256 tokenId) internal override virtual returns (bool, bytes memory) {
        require(_token.ownerOf(tokenId) == msg.sender, "not allowed");
        try _token.burn(tokenId) {
            return (true, "");
        } catch {
            return (false, "");
        }
    ...
    function _swapin(uint256 num_tokens, address receiver, bytes memory extraMsg) internal override returns (bool) {
        try _token.mint(receiver, num_tokens) {
            return true;
        } catch {
            return false;
        }
    ...
    function _swapoutFallback(uint256 tokenId, address sender, uint256 swapoutSeq, bytes memory extraMsg) internal override returns (bool result) {
        try _token.mint(sender, tokenId) {
            result = true;
        } catch {
            result = false;
        }
