...
contract LaifuCat is ERC721Optimized, Ownable, ReentrancyGuard {
    using Strings for uint256;

    uint256 public constant MAX_LFC = 10000;
    uint256 public maxLFCPurchase = 10;
    uint256 public LFCPrice = 100 ether;
    string public _baseLFCURI;
    
    event LFCMinted(address indexed mintAddress, uint256 indexed tokenId);
    event PermanentURI(string _value, uint256 indexed _id);

    constructor(string memory baseURI) ERC721Optimized("LaifuCat", "LFC") {
        _baseLFCURI = baseURI;
    }
    ...
    function setMintPrice(uint256 newPrice) public onlyOwner {
        require(newPrice >= 0, "LFC price must be greater than zero");
        LFCPrice = newPrice;
    }

    function mintLFC(uint256 numberOfTokens) public payable nonReentrant {
        require((LFCPrice * numberOfTokens) <= msg.value, "Ether value sent is not correct");
        require(numberOfTokens <= maxLFCPurchase, "You can mint max 10 LFC per transaction");
        require((totalSupply() + numberOfTokens) <= MAX_LFC, "Purchase would exceed max supply of LFC");
    ...
    function createCollectible(address mintAddress) private {
        ...
        if (mintIndex < MAX_LFC) {
            _safeMint(mintAddress, mintIndex);
            emit LFCMinted(mintAddress, mintIndex);
        }
    }
