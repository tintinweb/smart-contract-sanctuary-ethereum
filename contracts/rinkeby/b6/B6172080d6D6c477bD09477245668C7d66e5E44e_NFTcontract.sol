...
//merkleProof for allowlist
import "@openzeppelin/contracts/utils/cryptography/MerkleProof.sol";

contract NFTcontract is Ownable, ERC721A, PaymentSplitter {
    ...
    // add seed by community and random offset variables
    uint public seed;
    ...
    constructor(address[] memory distro, uint[] memory distro_shares, address[] memory teamclaim, bytes32 _merkleRoot)
        ERC721A("STANDARD-TEMPLATE", "STANDARD")
    ...
    // add set seed value chosen by community
    ...
    function getRandomValue(address account, uint lowValue, uint highValue) public returns (uint) {
        ...
        uint random_id = lowValue + uint(keccak256(abi.encodePacked(block.difficulty, block.timestamp, block.gaslimit, account, seed)))% mod_operator;
        randomOffset = random_id;
        return random_id;
    }
    
    function showRandomValue() public view returns (uint){
        return randomOffset;
    }
    ...
    function airDropNFT(address[] memory _addr) external onlyOwner {
        ...
        _safeMint(msg.sender, _addr.length);
    emit Mint(msg.sender, totalSupply());
    ...
    function mint(uint _amount) external payable noReentrant {
        ...
        //     emit Mint(msg.sender, totalSupply());
        // }
	accounts[msg.sender].mintedNFTs = accounts[msg.sender].mintedNFTs + _amount;
        _safeMint(msg.sender, _amount);
    emit Mint(msg.sender, totalSupply());
    ...
    function allowlistMint(bytes32[] calldata _merkleProof, uint _amount) external payable noReentrant{
        require(_isPreSaleLive, "Error: Allowlist Sale is not active.");
        require(totalSupply() + _amount <= (MAXSUPPLY - _reserved), "Error: Purchase would exceed max supply.");
        require((_amount + accounts[msg.sender].mintedNFTs) <= WALLETLIMIT, "Error: You would exceed the wallet limit.");
        require(!isContract(msg.sender), "Error: Contracts cannot mint.");
        require(msg.value >= (PRESALEPRICE * _amount), "Error: Not enough ether sent.");
        require(!allowlistClaimed[msg.sender], "You have already claimed.");

        bytes32 leaf = keccak256(abi.encodePacked(msg.sender));
        require(MerkleProof.verify(_merkleProof, merkleRoot, leaf), "You are not allowlisted.");
