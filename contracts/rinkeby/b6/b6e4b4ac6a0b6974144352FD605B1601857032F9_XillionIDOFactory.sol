...
contract XillionIDO is XillionIDOCommon {
    ...
    function initialise(
        ...
        setAllocationDates(dates_.allocationStartDate, dates_.allocationEndDate);
        // setAllocationEndDate(dates_.allocationEndDate);
        setSwapDates(dates_.swapStartDate, dates_.swapEndDate);
     ...
     */
    function setAllocationDates(uint256 allocationStartDate_, uint256 allocationEndDate_) public {
        (bool success, bytes memory result) = getMasterIDOAddress().delegatecall(abi.encodeWithSignature("setAllocationDates(uint256, uint256)", allocationStartDate_, allocationEndDate_));
        revertOnDelegateCallFailure(success, result);
    }
     ...
     */
    function setSwapDates(uint256 swapStartDate_, uint256 swapEndDate_) public {
        (bool success, bytes memory result) = getMasterIDOAddress().delegatecall(abi.encodeWithSignature("setSwapStartDate(uint256, uint256)", swapStartDate_, swapEndDate_));
        revertOnDelegateCallFailure(success, result);
    }
