...
abstract contract ERC721Marketplace is ERC721A, Ownable {
    ...
    event RevenueShareUpdated(address revShare, uint256 revShareBPS);
    event WithdrawSplitsSet(
        uint256 payoutBPS,
        uint256 referralBPS,
        uint256 patnershipBPS
    );
    ...
    function _initWithdrawSplits(
        ...
        if (signatures[1].length == 0) {
            ...
            emit WithdrawSplitsSet(DEFAULT_PAYOUT_BPS, 0, 0);
        } else {
            require(block.timestamp < signatureTimestamp_, "Signature expired");
            require(
                payoutBPS_ +
                    referralBPS_ +
                    partnershipBPS_ +
            ...
            emit WithdrawSplitsSet(payoutBPS_, referralBPS_, partnershipBPS_);
        }
     ...
     */
    function setRevenueShare(address newAddresses, uint256 newBPS)
        external
        onlyOwner
    {
        require(
        ...
        revenueShareAddress = newAddresses;
        splits.revenueShareBPS = uint64(newBPS);
        emit RevenueShareUpdated(newAddresses, newBPS);
    }
...
contract ERC721ARandomizedCollection is ERC721Marketplace {
    ...
    function _init(
        ...
        _owner = msg.sender;
        revenueShareAddress = addresses[0]; // revenue share address
        _initWithdrawSplits(
            _owner,
            ...
            uints[5], // timestamp for signatures expiration
            strings[1],
            signatures
        );
        emit RevenueShareUpdated(revenueShareAddress, uints[2]);
