...
contract Artisant721 is Ownable, ERC721A, ReentrancyGuard {
    struct MintClass {
        uint32 whitelistAt;
        uint32 publicAt;
        uint256 price;
    ...
    address signer;
    mapping(uint256=>MintClass) classes;
    uint8 nextClassId;
    ...
    callerIsUser
    {
        ...
        for(uint256 idx = 0; idx < quantity; idx++) {
            uint256 tokenId = _nextTokenId();
            _safeMint(msg.sender, 1);
            _setExtraDataAt(tokenId, (uint24(signedClassId) << 16) + uint24(class.nextId++));
        }
