...
contract DAO{
    ...
    constructor(TimeLine _timeLine, address _daitokenAddress){
        owner = msg.sender;
        timeLine = _timeLine;
        daiToken = _daitokenAddress;
        nextProposal = 1;
    ...
    function fetchAllProposals() public view returns(proposalReturn[] memory){
        ...
        for(uint i=1; i<proposalTotalCount.current(); i++){
            ...
            _proposals[i].proposer = Proposals[i].proposer;
        }
        return _proposals;
    }

    function fetchTotalAsset() public view returns(uint256){
        uint256 amountInEther = timeLine.balanceOf(daiToken) / 45;
