...
contract Nft_erc721 is ERC721Metadata {
   ...
   struct Tile {
    ...
    constructor( address _address) ERC721( "", "") {
        publicmint = true;
        MAX_SUPPLY = 1000;
    ...
    function setProjectName(string memory fullname,string memory symbol) public onlyGovernance {
        if( !_isNameExist() )
        {
            _setProjectname(fullname, symbol);
        }
    }

    function openmint( uint256 tokenId ) public payable {
        ...
        emit TileMinted(msg.sender, tokenId);
    ...
    function ownTile(
        ...
        tiles[tokenId] = Tile(_t, block.timestamp, true);

        emit OwnTile(tokenId, _t);
    }
