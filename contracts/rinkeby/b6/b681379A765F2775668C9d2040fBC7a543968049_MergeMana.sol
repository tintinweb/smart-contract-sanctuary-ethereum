...
contract MergeMana is Ownable {
   ...
   * Mana based logic
   */
  function _getInherentManaWithNoPenalties(uint256 id)
    internal
    view
    returns (uint256)
  {
    ...
    return
      mana[id] +
      uint256(uint8(baseManaStorage.indexToData(id)[0])) *
      baseManaMultiplier;
  }

  function getConsumableMana(uint256 id) public view returns (uint256) {
    return getInherentMana(id) + getChanneledMana(id);
  }
  ...
  function getChanneledMana(uint256 id) public view returns (uint256) {
    ...
    if ((decay + consumedAmount) > channeledMana[id]) {
      return 0;
    }
    return channeledMana[id] - decay - consumedAmount;
  }
  ...
  function getInherentMana(uint256 id) public view returns (uint256) {
    ...
    uint256 consumedAmount = consumedMana[id];
    if ((decay + consumedAmount) > channeledMana[id]) {
      uint256 leftOverManaPenalty = (decay +
        consumedAmount -
        channeledMana[id]);
      if (leftOverManaPenalty > _getInherentManaWithNoPenalties(id)) {
        return 0;
      }
      return _getInherentManaWithNoPenalties(id) - leftOverManaPenalty;
    }
    return _getInherentManaWithNoPenalties(id);
  }
  ...
  function _channelMana(uint256 from, uint256 to) internal {
    uint256 inherentMana = getInherentMana(from);
    channeledMana[to] += inherentMana;
    numChanneledToTokenId[to]++;
    channelingRitualDestinationTokenId[from] = to;
    mana[from] = 0;
    emit ChanneledStatue(from, to, inherentMana);
  }
    ...
    onlyIsNotShutdown
  {
    ...
    for (uint256 i = 0; i < tokenIds.length; ++i) {
      ...
      _boostMana(tokenIds[i], boostedMana);
    }
  }
    ...
    onlyIsNotShutdown
  {
    uint256 boostedMana = getBoostManaAmount(msg.sender);
    for (uint256 i = 0; i < tokenIds.length; ++i) {
      if (merge.ownerOf(tokenIds[i]) == msg.sender && mana[tokenIds[i]] == 0) {
        _boostMana(tokenIds[i], boostedMana);
      }
    }
  }
