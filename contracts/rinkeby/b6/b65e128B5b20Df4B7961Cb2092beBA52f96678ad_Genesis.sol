...
contract Genesis is Ownable, ERC721A, ReentrancyGuard, Pausable {
    ...
    uint8 private constant STEP_COUNT = PUBLIC_STEP + 1;
    ...
    function mint(uint8 amount, uint8 step) external payable nonReentrant whenNotPaused isNotContract {
        ...
        (bool success, uint8 _currStep) = _getCurrentStep();
        if (!success || _currStep != step) {
    ...
    function _getCurrentStep() private view returns (bool, uint8) {
        uint8 _step;
    ...
    function _checkMintStepValid(uint8 step) internal virtual returns (bool) {
        return step == FREE_MINT_STEP || step == MINT_LIST_STEP || step == WHITE_LIST_STEP || step == PUBLIC_STEP;
    }
