...
contract MinarioMarketplace is Ownable, ReentrancyGuard{
    ...
    function buyPack(address callerAddress, uint256 packID, uint256 amount, uint256 pricePack, string memory transactionID, Sig memory buyPackRSV) external payable initializer {
        ...
        require(!handleStorage(storageAddress).getPreventReplayAttack(message), "Message is already used!");
    ...
    function mint(string memory pointerMoment, Sig memory mintRSV) public initializer{
        ...
        handleStorage(storageAddress).setPreventReplayAttack(message);
        Minario(NFTAddress).mint(msg.sender, pointerMoment);
    ...
    function sell(address nftAddress, uint256 tokenID, uint256 price, string memory transactionID, Sig memory sellRSV) public initializer onlyNFTOwner(nftAddress, tokenID){
        require(!handleStorage(storageAddress).getStatusNFT(nftAddress, tokenID), "NFT Already Listed!");
    ...
    function buy(Buy memory buyStruct, Sig memory buyRSV) public payable initializer {
        require(handleStorage(storageAddress).getStatusNFT(buyStruct.nftAddress, buyStruct.tokenID), "NFT is not listed!");
        ...
        require(handleStorage(storageAddress).getPriceNFT(buyStruct.nftAddress, buyStruct.tokenID) == msg.value, "MSG_VALUE is not match with listing price!");
        require(ERC721(buyStruct.nftAddress).ownerOf(buyStruct.tokenID) != buyStruct.buyer, "You can't buy your own NFT!");
        handleStorage(storageAddress).setPreventReplayAttack(message);
    ...
    function cancelSell(address nftAddress, uint256 tokenID, string memory transactionID, Sig memory cancelRSV) public initializer onlyNFTOwner(nftAddress, tokenID) {
        ...
        require(handleStorage(storageAddress).getStatusNFT(nftAddress, tokenID), "NFT is not listed!");
        handleStorage(storageAddress).setPreventReplayAttack(message);
    ...
    function changePrice(address nftAddress, uint256 tokenID, uint256 newPrice, string memory transactionID, Sig memory changePriceRSV) public initializer onlyNFTOwner(nftAddress, tokenID) {
        ...
        require(handleStorage(storageAddress).getStatusNFT(nftAddress, tokenID), "You can't change price NFT that not selled!");
        handleStorage(storageAddress).setPreventReplayAttack(message);
    ...
    function mintedSwapNFT(uint256[] memory fromTokenID, uint256[] memory toTokenID, address fromNFTAddress, address toNFTAddress, string memory transactionID, uint256 deadline, Sig memory swapRSV) external{
        ...
        require(!handleStorage(storageAddress).getPreventReplayAttack(message), "Message is already used!");
        require(verifySigner(signerAddress, message, swapRSV), "RSV FAIL");
    ...
    //@dev this is for updating minario nft address
