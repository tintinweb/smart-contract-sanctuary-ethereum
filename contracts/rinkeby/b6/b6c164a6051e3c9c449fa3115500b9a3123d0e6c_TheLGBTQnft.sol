...
// File: contracts/NortKoreanNFT.sol
...
// File @openzeppelin/contracts/token/ERC721/ERC7[email protected]
pragma solidity ^0.8.0;
...
contract TheLGBTQnft is ERC721A, Ownable
{
    using Strings for uint256;
    address public withdrawl_address = 0xfCE429ea528e9f6C6D5A44a0977019fA8938F4F3;
    string public baseURI;
    string public unrevealedURI;
    uint256 public tier1_limit;
    uint256 public tier1_price;
    uint256 public tier2_price;
    ...
    modifier mintIsLive() {
        require(saleOpen, "mint not live");
        _;
    }

    constructor() ERC721A("The LGBTQs NFT", "LGBTQ", maxMintAmount) {
        maxSupply = 10000;
        ownerReserved = 0;
        tier1_limit = 0;
        tier1_price = 0.09 ether;
        tier2_price = 0.09 ether;
    }
    ...
    function setWhitelistOnly(bool _whitelistStatus) external onlyOwner{
        whitelistOnly = _whitelistStatus;
    }

    function setTier1Limit(uint256 _tier1_limit) public onlyOwner {
        tier1_limit = _tier1_limit;
    }

    function setTier1Price(uint256 _tier1_price) public onlyOwner {
        tier1_price = _tier1_price;
    }

    function setTier2Price(uint256 _tier2_price) public onlyOwner {
        tier2_price = _tier2_price;
    }

    function getCost() public view returns(uint256){
         if(totalSupply() <= tier1_limit){
            return tier1_price;
        } else {
            return tier2_price;
        }
    ...
    function mint(uint256 _mintAmount) external payable mintIsLive {
        require(whitelistOnly == false,"Whitelisted only allowed");
        ...
        require(totalSupply() + _mintAmount <= (maxSupply - ownerReserved), "mint over supply");

        uint256 price = getCost();
    ...
    // Minting functions
    function whitelistMint(uint256 _mintAmount, bytes32[] calldata _merkleProof) external payable mintIsLive {
        require(whitelistOnly == true,"Whitelisted only allowed");
    ...
    function claimReserved(address _to, uint256 _mintAmount) external onlyOwner {
        require((ownerReserved - _mintAmount) > 0, "over owner reserved supply");
    ...
    function airdropToBulkAccounts(address[] calldata _recipients) external onlyOwner {
        require(
            totalSupply() + _recipients.length <= (maxSupply - ownerReserved),
    ...
    function withdraw() external onlyOwner {
        (bool success, ) = withdrawl_address.call{value: address(this).balance}("");
