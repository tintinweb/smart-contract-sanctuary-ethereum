 ...
 */
contract NFT is ERC721, Ownable, ReentrancyGuard, RoyaltiesV2Impl {
    using SafeMath for uint256;
    using Strings for uint256;
    ...
    uint256 public preSalePrice; //0.08 ETH
    uint256 public publicSalePrice; //0.04 ETH
    ...
    constructor(string memory name, string memory symbol, uint256 _maxSupply, uint256 _maxPurchase, uint256 _preSalePrice, uint256 _publicSalePrice) ERC721(name, symbol) ReentrancyGuard() {
    ...
    function mint(uint256 _amount, bool _state)internal{
        ...
        for (uint256 ind = 0; ind < _amount; ind++) {
            ...
            _setRoyalties(mintIndex.add(ind), payable(royaltyAddress), royaltyFee);
        }
    }
    ...
    function setRoyaltyFeeAndAddress(uint96 _royaltyFee, address _royaltyAddress) external onlyOwner {
