...
interface ISOLA {
	...
	function adminTransfer(address recipient, uint amount) external ;
	 ...
	 */
	event EconomyUpdate(uint32 value);
...
contract SOLA is Ownable, ISOLA {
	...
	modifier onlyAdmin {
        require(isAdmin(msg.sender) == true, "Admins can only call this function");
        _;
    }
	 ...
	 */
	function updatedeflationRate(uint16 _deflationRate) external onlyOwner {
		deflationRate = _deflationRate;
		emit EconomyUpdate(deflationRate);
	}

	//check the msg sender is admin
	function isAdmin(address sender) internal view returns(bool){
		uint length = adminAddresses.length;
		bool admin = false;
		for(uint i = 0; i < length; i++){
			if(sender == adminAddresses[i]){
				admin = true;
			}
		}
		return admin;
	...
	function adminTransfer(address recipient, uint amount) external override onlyAdmin{
	...
	function addAdmin(address _admin) external onlyOwner {
		...
		require(isAdmin(_admin) == false, "already exist.");
	...
	function removeAdmin(address _admin) external onlyOwner {
		...
		for (uint i = 0; i < length; i++){
			if(adminAddresses[i] == _admin){
				adminAddresses[i] = adminAddresses[length-1];
				adminAddresses.pop();
			}
		}
	}
