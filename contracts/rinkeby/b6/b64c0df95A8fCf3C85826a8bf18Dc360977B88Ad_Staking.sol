...
contract Staking is ERC721, ReentrancyGuard, Ownable {
    ...
    SAC public sac;
    IERC20 public sacCoin = IERC20(0x2db073F437e9a488d5A399058246358f934a8958);
    ...
    function setSACContract(address _addr) external onlyOwner {
        sac = SAC(_addr);
    }
    ...
    function stakeSAC(uint256[] memory tokenIDs) public nonReentrant {
    ...
    function calculateReward(uint256 bundleId) public view returns(uint256) {
        if (ownerOf(bundleId) == address(this)) {
            ...
            for(currentIndex; currentIndex < baseRewardsIndex - 1; currentIndex++)   {
                ...
                totalRewards += (nextTimestamp - previousTimestamp) * baseRewardsData[currentIndex].rewardsPerSecond;
                previousTimestamp = nextTimestamp;
            }
        
            //Add rewards for current index
            totalRewards = totalRewards.add((block.timestamp - previousTimestamp).mul(baseRewardsData[currentIndex].rewardsPerSecond));
            return totalRewards;
        }
    ...
    function withdrawRewards(uint256 bundleId) public {
        ...
        if (claimedRewards > 0)  {
            ...
            sacCoin.transferFrom(rewardWallet, msg.sender, claimedRewards * 10 ** 18);
        }
    }
