...
contract ComKings is Context, ERC721A, Ownable, ReentrancyGuard  {
    ...
    constructor(address signer) ERC721A("ComKings", "CK") {
    ...
    function redeemUpgrade(uint256 salt, bytes calldata signature) public nonReentrant {
        address sender = msg.sender;
        require(!LOCKED, "Supply is already capped");
        require(_validateSignatureForUpgrade(
          signature,
          sender,
          salt
        ), "Invalid data provided");
    ...
    function setLock(bool isLocked) public onlyOwner {
        LOCKED = isLocked;
    }
    ...
    function _validateSignatureForUpgrade(
        bytes calldata signature,
        address senderAddress,
        uint256 salt
        ) internal view returns (bool) {
        bytes32 dataHash = keccak256(abi.encodePacked(senderAddress, salt));
        bytes32 message = ECDSA.toEthSignedMessageHash(dataHash);
