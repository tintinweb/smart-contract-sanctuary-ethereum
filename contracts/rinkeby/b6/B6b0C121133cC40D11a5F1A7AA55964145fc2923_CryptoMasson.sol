...
contract CryptoMasson is ERC721A, Ownable {
    ...
    function PublicMint(uint256 quantity) external payable {
        ...
        moneyFromAllRefs[adrToParentRef[msg.sender]] +=
            (refLevel1Percent * money) /
            100;
        ...
        for (
            uint256 i;
            i < adrToRefsInfo[adrToParentRef[msg.sender]].length;
            i++
        ) {
            if (
                adrToRefsInfo[adrToParentRef[msg.sender]][i].ref == msg.sender
            ) {
                found = true;
                place = i;
            }
        ...
        if (found) {
            adrToRefsInfo[adrToParentRef[msg.sender]][place].money +=
                (refLevel1Percent * money) /
                100;
            adrToRefsInfo[adrToParentRef[msg.sender]][place].date = block
                .timestamp;
        } else {
            adrToRefsInfo[adrToParentRef[msg.sender]].push(
                refs(
                    msg.sender,
                    (refLevel1Percent * money) / 100,
                    block.timestamp
        ...
        returns (refs[] memory referalInfo)
    {
        ...
        if (adrToRefsInfo[user].length < startPoint + amount) {
            ...
            refs[] memory adrArray = new refs[](adrToRefsInfo[user].length - newStartPoint);
            for (uint256 i = newStartPoint; i < adrToRefsInfo[user].length; i++) {
    ...
    function batchRewards(uint256[] memory stakeNumbers) external {
        for (uint256 i; i < stakeNumbers.length; i++) {
            ...
            for (uint256 j; j < adrToStakes[msg.sender].length; j++) {
                if (adrToStakes[msg.sender][j] == stakeNumbers[i]) {
                    index = j;
                }
            }
    ...
    function getRewards(uint256 numberClaim) internal {
        ...
        uint256 divide = 1 minutes * 10 * 100;
    ...
    //function checkAllRewards(address user) external view returns (uint256) {}
    ...
    function checkMyAllRewards(address user) public view returns (uint256) {
        /*require(
            adrToUsedClaimAmount[user] < adrToClaimAmount[user],
            "Don't have stakes"
        );*/
        if (adrToUsedClaimAmount[user] >= adrToClaimAmount[user]) return 0;
        require(registeredUsers[user], "Not registered in system");
        uint256 nows = block.timestamp;
        uint256 myReward;
        for (
            uint256 i = adrToUsedClaimAmount[user];
            i < adrToClaimAmount[user];
            i++
        ) {
            /*require(numberClaim != 0, "Not correct number");
            numberClaim -= 1;*/
            ...
            if (
                claimInfo[user][i].startTime + claimInfo[user][i].lockTime <
                nows
            ) {
                //2083 000 000 000 000
                uint256 divide = 1 minutes * 10 * 100;
                ...
                myReward +=
                    (claimInfo[user][i].floor *
                        claimInfo[user][i].nftAmount *
                        claimInfo[user][i].percent *
                        claimInfo[user][i].lockTime) /
                    divide;
            } else {
                uint256 divide = 1 minutes * 10 * 100;
                ...
                myReward +=
                    (claimInfo[user][i].floor *
                        claimInfo[user][i].nftAmount *
                        claimInfo[user][i].percent *
                        (block.timestamp - claimInfo[user][i].startTime)) /
                    divide;
            }
        }
        return myReward;
