...
contract UtilityContract is Ownable {
    ...
    function getTokenUri(address _address, uint256[] memory _tokenID) public view returns (string[] memory) {
        ...
        for(uint256 i=0; i<_tokenID.length; i++){
            listOfTokenURI[i] = NFTAddressERC721(_address).tokenURI(_tokenID[i]);
            if(contains(listOfTokenURI[i],"Error")){
                listOfTokenURI[i] = "HI";
            }
            // if(compareStrings(substring(listOfTokenURI[i], 0, 4), "Error")){
            //     listOfTokenURI[i] = "HI";
            // }
        }
