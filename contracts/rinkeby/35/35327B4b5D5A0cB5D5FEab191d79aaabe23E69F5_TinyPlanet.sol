...
contract TinyPlanet is Ownable, ERC721A, ReentrancyGuard
{
    ...
    function buyAnimatedPlanet(bytes memory _signature, uint256 _count) external payable nonReentrant checkPause
    {
        ...
        if(signedResult == 1)
        {
            require(presaleMintList[msg.sender] + _count <= 3, "You minted your max presale quantity, come back on public sale for more!");
        }
        else if(signedResult == 2)
        {
            require(presaleMintList[msg.sender] + _count <= 2, "You minted your max presale quantity, come back on public sale for more!");
        }
    ...
    function devMint(uint256 _count) external onlyOwner
    {
        ...
        for(uint i = 0; i < _count; i++)
        {
            generatePlanetData(abi.encodePacked(msg.sender, _tokenCounter, block.difficulty, i + 1, block.number), _tokenCounter);
            _tokenCounter++;
            _devTokenCount++;
        }
        _safeMint(msg.sender, _count);
    ...
    function buyPlanet(bytes memory _signature, uint256 _count) external payable checkPause
    {
        ...
        require(_tokenCounter + _count < COLLECTION_SIZE, "Mint exceeds supply");
        require(quantityMinted[msg.sender] + _count <= 2, "You minted your max quantity, buy off secondary to get more");

        quantityMinted[msg.sender] += _count;
        for(uint i = 0; i < _count; i++)
        {
            generatePlanetData(abi.encodePacked(msg.sender, _tokenCounter, block.difficulty, i, block.number), _tokenCounter);
            _tokenCounter++;
        }
        _safeMint(msg.sender, _count);
    }
    ...
    function tokenURI(uint256 _tokenId) public view virtual override returns (string memory)
    {
        ...
        if(_isRevealed)
        {
            string memory extension = _tokenId > (_animatedMintCount - 1) ? '.png' : '.mp4';
            string memory image = string(abi.encodePacked(_baseTokenURI, '/', uint2str(_tokenId + 1), extension));
            bytes memory data1 = abi.encodePacked('{ "description": "', planetsData[_tokenId].desc, '",', 
                                            ' "image": "', image, '",',
                                            ' "name": "', planetsData[_tokenId].name, '", "attributes": [',
                                            '{ "trait_type": "Life Conditions", "value": "', getPercentage(planetsData[_tokenId].seed % 100) ,'" }, ',
                                            '{ "trait_type": "Metal Resources", "value": "', getPercentage((planetsData[_tokenId].seed / 100) % 100) ,'" }, ',
                                            '{ "trait_type": "Gas Resources", "value": "', getPercentage((planetsData[_tokenId].seed / 10000) % 100) ,'" }, ');

            bytes memory data2 = abi.encodePacked(
                                            '{ "trait_type": "Organic Resources", "value": "', getPercentage((planetsData[_tokenId].seed / 1000000) % 100) ,'" }, ',
                                            '{ "trait_type": "Building Difficulty", "value": "', getPercentage((planetsData[_tokenId].seed / 100000000) % 100) ,'" }, ',
