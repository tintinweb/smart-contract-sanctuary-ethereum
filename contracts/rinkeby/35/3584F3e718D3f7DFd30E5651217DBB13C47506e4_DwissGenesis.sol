...
contract DwissGenesis is Owned, ERC721 {
  ...
  function mint(
    ...
    // SLOAD everything in one go
    uint _price = price;
    uint _totalSupply = totalSupply;
    uint _saleStart = saleStart;
    bool _hasEnded = hasEnded;

    require(!_hasEnded, "Sale has ended");
    ...
    for (uint i = 0; i < amount;) {
      unchecked {++i;}
      uint id = _totalSupply + i;
      _mint(msg.sender, id);
    }

    // sstore
    totalSupply = uint32(_totalSupply + amount);
    ...
    if (msg.value > cost) {
      ...
      (bool ok, /* data */) = msg.sender.call{value : refund}("");
