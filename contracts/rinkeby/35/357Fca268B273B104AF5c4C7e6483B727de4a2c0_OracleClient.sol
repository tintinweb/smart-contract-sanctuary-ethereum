...
/// @title OracleClient Smart Contract
/// @author Faolain
/// @notice Allow the interaction between dClimate marketplace and oracle
contract OracleClient is IOracleClient, ChainlinkClient, Ownable {
    using Chainlink for Chainlink.Request;

    /// @dev Interface of dclimate Marketplace Smart Contract
    IMarketplace public dclimateMarketplace;

    /// @notice Constructor for initializing the Smart Contract variables
    constructor(address _dclimateMarketplace) {
        setPublicChainlinkToken();
        dclimateMarketplace = IMarketplace(_dclimateMarketplace);
    }
    ...
    /// @dev this function can be only called by dclimate Marketplace Smart Contract, in order to
    /// transfer this amount fee to the oracle using sendChainlinkRequestTo function
    ...
    function createOrder(
        ...
        require(msg.sender == address(dclimateMarketplace), "Unauthorized");
        Chainlink.Request memory req = buildChainlinkRequest(
            _jobId,
            address(this),
            this.completeOrder.selector
        );

        req.addUint("orderId", _orderId);
        req.addBytes("_publicKey", abi.encodePacked(_publicKey));
        req.add("encryptedCipher", _cipher);
    ...
    /// @param _orderId uint256 order id
    /// @param _cipher data encrypted with consumer public key
    function completeOrder(
        bytes32 _requestId,
        uint256 _orderId,
        string memory _cipher
    ) external override recordChainlinkFulfillment(_requestId) {
        dclimateMarketplace.completeOrder(_orderId, _cipher);
    }
}
