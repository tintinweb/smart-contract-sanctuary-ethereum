...
contract Nash21Factory is
    ...
    EIP712Upgradeable
{
    bytes32 internal constant _N21_FACTORY_TYPEHASH =
        keccak256(
    ...
    function initialize() public initializer {
        ...
        // _setBaseURI('https://nash21.io/nfts/');
        // _setContractURI('https://nash21.io/contractUri/');
    ...
    function getCreationFee(uint256 id) public view returns (uint256) {
        uint256 _creationFee = tokenCreationFee[data[id].hashId] == 0
            ? creationFee
            : tokenCreationFee[data[id].hashId];
        if (_creationFee > 100 ether) _creationFee = 0;
        INash21Guarantee guaranteeInterface = INash21Guarantee(
            INash21Manager(manager).get(keccak256("GUARANTEE"))
        );
        return
            guaranteeInterface.transformCurrency(
                data[id].currency,
                (data[id].value * _creationFee) / 100 ether
            );
    }

    /// @inheritdoc INash21Factory
    function getTransferFee(uint256 id) public view returns (uint256) {
        uint256 _transferFee = tokenTransferFee[id] == 0
            ? transferFee
            : tokenTransferFee[id];
        if (_transferFee > 100 ether) _transferFee = 0;
        INash21Guarantee guaranteeInterface = INash21Guarantee(
            INash21Manager(manager).get(keccak256("GUARANTEE"))
        );
        return
            guaranteeInterface.transformCurrency(
                data[id].currency,
                (data[id].value * _transferFee) / 100 ether
            );
    }
    ...
    function authAndCreate(
        ...
        uint256 endDate,
        string memory uri
    ) external payable onlyRole(_AUTH_ROLE) returns (uint256) {
        uint256 id = _authorize(
            hashId,
            account,
            value,
            currency,
    ...
    function authorizeAndSetFees(
        ...
        _setTokenCreationFee(_creationFee, hashId);
        return id;
    }

    /// @inheritdoc INash21Factory
    function unauthorizeOffchainSign(bytes32 hash)
        external
        payable
        onlyRole(_AUTH_ROLE)
    {
        unauthorized[hash] = true;
        emit UnauthorizeHash(hash);
    }
    ...
    function create(uint256 id) external payable {
        ...
        tokenInterface.transferFrom(account, vault, feeRequired);
        _create(id);
    }
    ...
    function createWithSignature(
        ...
        require(hasRole(_AUTH_ROLE, signer), "N21F003");
        require(block.timestamp <= deadline, "N21F004");
        ...
        require(validation, "N21F005");

        require(!unauthorized[hash], "N21F006");
        _create(id);
    }
    ...
    /// @inheritdoc INash21Factory
    function setTokenCreationFee(uint256 fee, bytes32 hashId)
        external
        payable
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
    ...
    /// @inheritdoc INash21Factory
    function setTokenTransferFee(uint256 fee, uint256 id)
        external
        payable
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
    ...
    function split(
        ...
        require(
            INash21Manager(manager).get(keccak256("GUARANTEE")) == msg.sender,
            "N21F007"
        );
        require(
            ownerOf(id) == account,
        ...
        uint256 value1 = (data[id].value * (timestamp - data[id].startDate)) /
            (data[id].endDate - data[id].startDate);
    ...
    function _checkTimestamp(uint256 id, uint256 timestamp) internal view {
        require(
            timestamp > data[id].startDate,
            "N21F015"
        );
        require(
            timestamp < data[id].endDate,
            "N21F016"
        );
    }
        ...
        whenNotPaused
    {
        ...
        require(
            (guaranteeInterface.distributed(id) == 0 || to == address(0)),
            "N21F017"
        );

        if ((startDate != address(0)) && (to != address(0))) {
            ...
            if (feeRequired > 0) {
                ...
                tokenInterface.transferFrom(startDate, vault, feeRequired);
            }
        }

        super._beforeTokenTransfer(startDate, to, id);
    }

    // Leaving some storage for upgrades
...
interface INash21Guarantee {
    ...
    event Claim(uint256 id, address account, uint256 amount, bytes32 currency);
    ...
    /// @param currency Hashed currency symbol
    event Pay(uint256 id, address account, uint256 amount, bytes32 currency);
    ...
    event NewFeed(bytes32 currency, address feed);
...
interface INash21Factory is IERC721EnumerableUpgradeable {
    ...
    event SetTokenCreationFee(bytes32 hashId, uint256 fee);

    /// @notice Emitted when the tokenTransferFee is set
    /// @param id Token identificator
    /// @param fee Transfer fee
    event SetTokenTransferFee(uint256 id, uint256 fee);
    ...
    /// @param id Token identificator
    event Unauthorize(
        uint256 id /*, uint256 id, address account, uint256 value, bytes32 currency, uint256 startDate, uint256 endDate, string uri, uint256 deadline*/
    );

    /// @notice Emitted when the creation of a token is unauthorized
    /// @param hash Authorized hash
    event UnauthorizeHash(
        bytes32 hash /*, uint256 id, address account, uint256 value, bytes32 currency, uint256 startDate, uint256 endDate, string uri, uint256 deadline*/
    );
