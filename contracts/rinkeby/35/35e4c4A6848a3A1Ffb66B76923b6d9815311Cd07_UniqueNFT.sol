...
// Part: smartcontractkit/chai[email protected]/LinkTokenInterface
...
//this contract uses chainlink VRF for getting a random number
contract UniqueNFT is VRFConsumerBase,ERC721{
    bytes32 internal keyHash;
    uint256 internal fee;
    uint256 public tokenCounter;
    enum Breed{PUGSHIBA_INU,ST_BERNARD}
    mapping(bytes32 => address) requestIdToAddress;
    mapping(uint256 => Breed) tokenIdToBreed;
    // generate events in solidity 
    // indexed keyword allows to search the events using the indexed parameter as filter
    event requestedCollectible(bytes32 indexed requestId, address requester);
    event breedAssigned(uint256 indexed tokenId, Breed breed);

    constructor(address _VRFCoorinator, address _LinkToken , bytes32 _keyHash, uint256 _fee) public
    VRFConsumerBase(_VRFCoorinator,_LinkToken)
    ERC721("DogToken","DTN") {
        tokenCounter = 0;
        keyHash = _keyHash;
        fee = _fee;
    ...
    //this is the callback function used by VRF
    function fulfillRandomness(bytes32 _requestId, uint256 _randomNumber) internal override {
        Breed breed = Breed(_randomNumber % 3);
        uint256 tokenId = tokenCounter;
        tokenIdToBreed[tokenId] = breed;
        emit breedAssigned(tokenId,breed);
        address owner = requestIdToAddress[_requestId];
        _safeMint(msg.sender,tokenId);
        tokenCounter = tokenCounter +1;
    }
    //since the token uri will be random based on the dog breed, we create a second fuunction that adds the tokenURI
