...
// ERC-721 Smart Contract for the CherryCat Test NFT Collection
...
contract CherryCat_Test is ERC721, ERC721Enumerable, Pausable, Ownable {
    using Strings for uint256;

    string public baseTokenURI;
    string public contractURI;
    uint256 public constant MAX_TOKENS = 100; // Total Supply
    uint256 public maxMint = 10; // Max 10 CherryCat Test per transaction
    uint256 public price = 0.05 ether; // Each CherryCat Test cost 0.05 ETH to mint
    ...
    constructor(string memory _baseTokenURI, string memory _contractURI) ERC721("CherryCat Test", "CCT") {
        setBaseURI(_baseTokenURI);
        setContractURI(_contractURI);

        // The first 100 CherryCat Test are minted for the team, giveaways and our community
        // mint(msg.sender, 100);
    ...
    function preMint(address _to, uint256 _quantity) public payable {
        ...
        require(hasPresaleAccess(_to), "You are not whitelisted for the CherryCat Test pre-sale");
        require(!mainSale, "You can't pre-mint CherryCat Test at the moment");
        require(_quantity > 0 && _quantity <= maxMint, "You can only mint 1 to 10 CherryCat Test");
    ...
    function mint(address _to, uint256 _quantity) public payable {
        ...
        if (msg.sender != owner()) {
            require(mainSale, "The CherryCat Test main sale is not open");
            require(_quantity > 0 && _quantity <= maxMint, "You can only mint 1 to 10 CherryCat Test");
