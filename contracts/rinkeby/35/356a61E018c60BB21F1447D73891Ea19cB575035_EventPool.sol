...
contract EventPool {
    ...
    payable {
        ...
        ETHUSDPrice =  getLatestPrice() * 1 ** 10; //8 decimal places of the return value
        uint256 ethToUsdAmount = (msg.value * ETHUSDPrice) / 10 ** 18;
        uint256 ticketPrice = allEvents[_eventId].price * 10 ** 18; //ticket set price for the event
        require(ethToUsdAmount >= ticketPrice); //checks that enough eth
