...
contract CyberGenie is Ownable, ERC721A {
    ...
    // Public mint price
    uint256 public mintPrice = 0.1 ether; // 200 Matic
    ...
    // List of partners sc with % discounts i.e nft community partners, a small list
    address[] public partnersSCListKeys;
    mapping(address => uint256) public partnersSCList;
    // List of partners wallets with % discounts and max_per_address i.e collaborator wallets, a small list
    ...
    modifier validateDiscount(uint256 _discount) {
        require(_discount >= 0 && _discount <= 100, "CG: Discount is between 0 & 100");
        _;
    }

    modifier validateCount(uint256 _count) {
        uint256 _maxMintPerAddress = getMaxMintPerAddress(_msgSender());
        require(
            _count > 0 &&
                _count + _numberMinted(_msgSender()) - _getAux(_msgSender()) <= _maxMintPerAddress,
            "CG: Exceeded maxMintPerAddress"
        );
        require(totalSupply() + _count <= MAX_SUPPLY, "CG: Exceeded max supply");
        _;
    }

    modifier validatePrice(uint256 _count) {
        uint256 _price = getDiscountedPrice();
        require(msg.value >= _count * _price, "CG: Insufficient funds");
        _;
    }

    modifier validateProvenance() {
        require(bytes(provenanceHash).length == 0, "CG: Provenance already set!");
        _;
    }
     ...
     */
    function getMaxMintPerAddress(address _claimer) public view returns (uint256) {
    ...
    /**
     * @dev Get discounted price if applicable
     *
     */
    function getDiscountedPrice() public view returns (uint256) {
        uint256 _price = mintPrice;
        // some wallet partners might have a discount i.e collaborators
        if (partnersWalletList[_msgSender()].discount > 0) {
            _price = _price - (_price * (partnersWalletList[_msgSender()].discount / 100));
        } else {
            // check if claimer belongs to a partner sc i.e nft community partner
            for (uint256 i = 0; i < partnersSCListKeys.length; i++) {
                // check if claimer has tokens with partner sc
                if (IERC721(partnersSCListKeys[i]).balanceOf(_msgSender()) > 0) {
                    _price = _price - (_price * (partnersSCList[partnersSCListKeys[i]] / 100));
                }
            }
        }
        return _price;
    ...
    /**
     * @dev sets the merkleRoot for {merkleRoot}
     */
     ...
     *
     */
    function setPartnersSCList(address _address, uint256 _discount)
        external
        onlyOwner
        validateDiscount(_discount)
    {
        partnersSCListKeys.push(_address);
        partnersSCList[_address] = _discount;
    }
    ...
    function setPartnersWalletList(
        ...
        uint256 _discount
    ) external onlyOwner validateDiscount(_discount) {
        partnersWalletList[_address] = PartnerConditions({
     ...
     *
     */
    function whitelistMint(uint32 _count, bytes32[] calldata _merkleProof)
        public
        payable
        validateCount(_count)
        validatePrice(_count)
    {
        require(isWhitelistMintEnabled, "CG: Whitelist sale is not active");
        require(!whitelistClaimed[_msgSender()], "CG: Address already claimed");
