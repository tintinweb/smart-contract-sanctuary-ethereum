...
contract SomaERC777Issuable is SomaERC777, ISecurityIssuable {
    ...
    function initialize(
        ...
        super.initialize(name, symbol, _defaultOperators);
    }
    ...
    function mint(uint256 value, bytes memory userData, bytes memory operatorData)
    public
    override
    onlyTokenManager {
        address mintTo = SOMA.mintTo();
        _mint(mintTo, value, userData, operatorData);
    }

    function seize(address tokenHolder, uint256 value) external override {
        seize(tokenHolder, value, "", "");
    }

    function seize(address tokenHolder, uint256 value, bytes memory userData, bytes memory operatorData)
    public
    override
    onlyTokenManager {
        address to = SOMA.seizeTo();
        _send(tokenHolder, to, value, userData, operatorData, true);
        emit Seized(_msgSender(), tokenHolder, value, userData, operatorData);
    }
}
...
interface ISecurityIssuable {
    event Seized(address indexed operator, address indexed from, uint256 value, bytes userData, bytes operatorData);

    function mint(uint256 value) external;
    function mint(uint256 value, bytes memory userData, bytes memory operatorData) external;

    function seize(address tokenHolder, uint256 value) external;
    function seize(address tokenHolder, uint256 value, bytes memory userData, bytes memory operatorData) external;
}
...
abstract contract GuardableUpgradeable is IGuardable, AccessibleUpgradeable {
    ...
    modifier onlyAccessApproved {
        require(canAccess(_msgSender()), 'required access not met');
        _;
    }
