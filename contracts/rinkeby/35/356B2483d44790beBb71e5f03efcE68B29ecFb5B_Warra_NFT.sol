...
contract Warra_NFT is ERC721URIStorage, Ownable  {
    ...
    event activityOnTokenCalled(uint256 tokenId, string tokenUri, string activity, address caller, address associated, uint256 timestamp);
    ...
    modifier onlyOwnerOrNominated() {
        require(msg.sender == owner() || isNominated[msg.sender], "Caller is neither owner nor nominated");
        _;
    }

    modifier onlyOwnerOfToken(uint256 tokenId) {
        require(msg.sender == getOwner(tokenId), "Caller is not owner of e-WNFT");
        _;
    }

    modifier onlyManufacturerOrOwner(uint256 tokenId) {
        require(msg.sender == provider[tokenId] || msg.sender == getOwner(tokenId), "Caller is not provider or owner of e-WNFT");
        _;
    }

    // function to mint warranty NFT
    function createWarranty(string memory tokenURI, uint256 numberOfDays) public onlyOwnerOrNominated returns (uint256) {
        ...
        // emit event
        emit activityOnTokenCalled(newItemId, tokenURI, "CREATE" , msg.sender, address(0), block.timestamp);
    ...
    function transfer(address to, uint256 tokenId) public {
        ...
        // emit event
        emit activityOnTokenCalled(tokenId, tokenUri, "TRANSFER", msg.sender, to, block.timestamp);
    }
    ...
    function deleteWarranty(uint256 tokenId) internal onlyOwnerOfToken(tokenId) {
        _burn(tokenId);

        string memory tokenUri = getURI(tokenId);

        // emit event
        emit activityOnTokenCalled(tokenId, tokenUri, "DELETE", msg.sender, address(0), block.timestamp);
    ...
    function availService(uint256 tokenId, string memory activity) public onlyManufacturerOrOwner(tokenId) {
        string memory tokenUri = getURI(tokenId);
        // emit event
        emit activityOnTokenCalled(tokenId, tokenUri, activity, msg.sender, address(0), block.timestamp);
    }
