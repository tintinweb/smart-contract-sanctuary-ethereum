...
contract PackWoodERC1155V2 is ERC1155Upgradeable, OwnableUpgradeable {
    ...
    event buyTokenDetails(address from, address to, uint256 tokenId, uint256 price);
    ...
    function mintToken(uint256 _amount) external returns(uint256){
    ...
    function buyToken(uint256 tokenId) payable external returns(bool){
        require(totalNum == msg.value, "PackWoodERC1155: Price is incorrect");
        payable(owner()).transfer(msg.value);

        _safeTransferFrom(owner(), msg.sender, tokenId, 1, "");

        emit buyTokenDetails(owner(), msg.sender, tokenId, msg.value);
