...
contract HoneyVesting is AccessControlEnumerable, VestingCalculator {
    ...
    function withdrawSpentHoney(uint256 _amount, address payable _destination) public onlyRole(WITHDRAW_ROLE) {
        ...
        require(
            _amount <= honeyContract.balanceOf(address(this)), 
            "withdrawSpentHoney: not enough honey remaining in vesting contract"
        );
        
        honeyContract.safeTransfer(_destination, spentHoneyBalance);
        spentHoneyBalance = 0;
        ...
        returns (uint256 releasable, uint256 released, uint256 spent, uint256 remaining) 
    {
        ...
        spent = honeyReleasedByBearId[_tokenId];
        ...
        returns (uint256 releasable, uint256 released, uint256 spent, uint256 remaining) 
    {
        ...
        spent = honeyReleasedByJarId[_tokenId];
