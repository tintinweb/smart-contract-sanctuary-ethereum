...
contract ToshimetaPasses is ERC1155Supply, Ownable, ReentrancyGuard {
    ...
    uint256 public genesisSupply = 700;
    ...
    constructor() ERC1155("https://gateway.pinata.cloud/ipfs/QmR68Ln6Uut45ok7aa2QyFa2xCD5FxXL2PZtQLGrThqo4z/{id}.json") {}
    ...
    function mintGenesisPass(uint256 _mintAmount) public payable callerIsUser{
        ...
        if (!publicSaleActive) {
            ...
            require(isOnGenesisWL(msg.sender) > 0, "user is not on genesis pass whitelist");
    ...
    function mintToshimetaPass(uint256 _mintAmount) public payable callerIsUser{
        require(!paused, "the contract is paused");
        require(_mintAmount > 0, "need to mint at least 1 NFT");
        ...
        if (!publicSaleActive) {
            ...
            require(isOnToshiWL(msg.sender) > 0 || isOnGenesisWL(msg.sender) > 0, "user is not on toshimeta pass whitelist");
    ...
    function addGenesisPresalers(address[] calldata _addresses) public onlyOwner {
        for (uint x = 0; x < _addresses.length; x++) {
            genesisPresaleAddresses[_addresses[x]] = 1;
        }
    }

    function addToshiPresalers(address[] calldata _addresses) public onlyOwner {
        for (uint x = 0; x < _addresses.length; x++) {
            toshimetaPresaleAddresses[_addresses[x]] = 1;
        }
    }

    function isOnGenesisWL(address _address) public view returns (uint8) {
        return genesisPresaleAddresses[_address];
    }


    function isOnToshiWL(address _address) public view returns (uint8) {
        return toshimetaPresaleAddresses[_address];
    }
