...
contract XANALand is ProxyBaseStorage, ERC721AQueryable, Ownable, ReentrancyGuard, Storage {
 ...
 using Address for address;
    ...
    //Discounted Mint Function
     function mintDiscountCommon(uint256 _mintAmount,  bytes32[] calldata proof) checkSupply(_mintAmount,1, "Common") external payable nonReentrant{
        string memory rarity = "Common";
        uint256 _size=1;
         //Check user buy limit merkle tree
        uint256 userBuyCount = _userBought[1][msg.sender] + _mintAmount;
        require(isWhitelisted(msg.sender, proof,  userBuyCount, 7, "Common"), "XanaLand: Not whitelisted");
        
        _userBought[1][msg.sender] += _mintAmount;
    ...
    function mintLand(string memory rarity,uint256 _mintAmount, uint256 _size, bytes32[] calldata proof, bool isLimit, uint256 whitelistType) checkSupply(_mintAmount,_size, rarity) external payable {
        ...
        checkWhiteList(  _mintAmount, proof,  whitelistType,  rarity,  isLimit);
    ...
    function freeMint(uint256 _mintAmount,  bytes32[] calldata proof) checkSupply(_mintAmount, 1, "Common") external{
        ...
        require(isWhitelisted(msg.sender, proof,  userBuyCount, 8, "Common"), "XanaLand: Not whitelisted");
        _userBought[8][msg.sender] += _mintAmount;
    ...
    function isWhitelisted(address account, bytes32[] calldata proof, uint256 quantity, uint256 typeWhitelist, string memory rarity) public view returns (bool) {
        return _verify(_leaf(account, quantity, rarity), proof, whitelistRoot[typeWhitelist]);
    }
