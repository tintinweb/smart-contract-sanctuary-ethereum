...
contract ManageVoting {
        ...
        returns (bool)
    {
        ...
        if (keccak256(bytes(va)) == keccak256(bytes("staff"))) {
            uint256 balance = token.balanceOf(msg.sender);
            require(balance > 29, "You are not a member of the staffs");
        }

        if (keccak256(bytes(va)) == keccak256(bytes("student"))) {
            uint256 balance = token.balanceOf(msg.sender);
            require(balance == 10, "You are not a member of the board of");
        }
