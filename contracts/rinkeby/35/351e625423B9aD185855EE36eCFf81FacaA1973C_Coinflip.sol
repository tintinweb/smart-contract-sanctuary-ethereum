...
contract Coinflip is VRFConsumerBaseV2, Ownable {
  ...
  function requestCoinflip(uint256 bet) external payable {
    ...
    if ((msg.value * 100 / _feeFactor) > maxBetAmount) {
      revert ExceededMaxBetAmount();
    }
    uint256 potentialReward = msg.value * 2;
    pendingPayouts += potentialReward;
    if (potentialReward > address(this).balance || pendingPayouts > address(this).balance) {
    ...
    override
  {
    ...
    uint256 betAmountWithoutFees = playerBet.amount * 100 / _feeFactor;
