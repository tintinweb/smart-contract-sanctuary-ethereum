...
contract Trade {
    ...
    function getFees(Order memory order) internal view returns(Fee memory){
        ...
        uint assetFee;
        uint price = order.amount * 1000 / (1000 + tradingFeePermille + buyerFeePermille );
        uint sellerFee = price * sellerFeePermille / 1000;
        uint buyerFee =  price * buyerFeePermille / 1000;
        platformFee = sellerFee + buyerFee; 
        _tradingFee =  price * tradingFeePermille / 1000;
        ...
        assetFee = price - (fee + sellerFee + _tradingFee);
        return Fee( platformFee, _tradingFee , assetFee, royaltyAddress, royaltyFee, price);
    }
