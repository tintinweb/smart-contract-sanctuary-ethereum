...
contract RNGChainlink is RNGInterface, VRFConsumerBase, Ownable {
    ...
    /// @return internalRequestId The last request id used in the last request
    function getLastRequestId()
        external
        view
        override
        returns (uint32 internalRequestId)
    {
        return requestCount;
    }
    ...
    /// @return internalRequestId The ID of the request used to get the results of the RNG service
    ...
    /// until the result is available via the `internalRequestId`
    function requestRandomNumber()
        external
        override
        returns (uint32 internalRequestId, uint32 lockBlock)
    {
        ...
        internalRequestId = _requestRandomness(seed);

        requestLockBlock[internalRequestId] = lockBlock;

        emit RandomNumberRequested(internalRequestId, msg.sender);
    }

    /// @notice Checks if the request for randomness from the 3rd-party service has completed
    /// @dev For time-delayed requests, this function is used to check/confirm completion
    /// @param internalRequestId The ID of the request used to get the results of the RNG service
    /// @return isCompleted True if the request has completed and a random number is available, false otherwise
    function isRequestComplete(uint32 internalRequestId)
        external
        view
        override
        returns (bool isCompleted)
    {
        return randomNumbers[internalRequestId] != 0;
    }

    /// @notice Gets the random number produced by the 3rd-party service
    /// @param internalRequestId The ID of the request used to get the results of the RNG service
    /// @return randomNum The random number
    function randomNumber(uint32 internalRequestId)
        external
        override
        returns (uint256 randomNum)
    {
        return randomNumbers[internalRequestId];
    }
    ...
    {
        // Get next request ID
        internalRequestId = _getNextRequestId();

        // Complete request
        bytes32 vrfRequestId = requestRandomness(keyHash, fee, seed);
        chainlinkRequestIds[vrfRequestId] = internalRequestId;

        emit VRFRequested(internalRequestId, vrfRequestId);
    }
    ...
    function fulfillRandomness(bytes32 vrfRequestId, uint256 randomness)
        ...
        override
    {
        uint32 internalRequestId = chainlinkRequestIds[vrfRequestId];
    ...
    /// @return internalRequestId The ID to be used for the next request
    function _getNextRequestId() internal returns (uint32 internalRequestId) {
        requestCount = uint256(requestCount).add(1).toUint32();
        internalRequestId = requestCount;
    }
