...
contract PlatziPunksDNA {
    ...
    // This pseudo random is DETERMINISTIC, not ideal for production
    // check an oracle in chainlink for some off chain random functions
    ...
    function getZodiacSign(uint256 _dna) public view returns(string memory) {
        uint8 dnaSection = _getDNASection(_dna, 26);
        return _zodiacSign[dnaSection % _zodiacSign.length];
    }
...
contract PlatziPunks is ERC721, ERC721Enumerable, PlatziPunksDNA {
    ...
    function mint() public {
        ...
        require(newId < maxSupply, "Ya estamos full");
    ...
    function addTokenDNA(uint256 newId) private {
        tokenDNA[newId] = deterministicPseudoRandomDNA(newId, msg.sender);
    }
    ...
    function _paramsURI(uint256 _dna) internal view returns (string memory) {
        ...
        { 
            params = string(abi.encodePacked(params,
              "&topType=",
                getTopType(_dna),
                "&zodiacSign=",
                getZodiacSign(_dna)));
    ...
    function tokenURI(uint256 tokenId)
        ...
        returns (string memory) 
        {
            require(_exists(tokenId), "Epa, este Id no lo conozco");
            ...
            string memory jsonUri = Base64.encode(abi.encodePacked(
                '{ "name": "PlatziPunks #',
                tokenId.toString(),
                '", "description": "Platzi Punks are randomized Avatars stored on chain to teach DApp development on Platzi", "image": "',
                image,
                '"}'
            ));
