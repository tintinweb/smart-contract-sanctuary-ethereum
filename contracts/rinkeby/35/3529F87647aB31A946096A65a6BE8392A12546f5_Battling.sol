...
contract Battling is Ownable, SignerVerifiable {
    ...
    modifier callerVerified(uint256 _amount, string memory _message, string memory _battle_id, uint256 _deadline, bytes memory _signature) {
    ...
    function initiateBattle(string memory _message, string memory _battle_id, uint256 _deadline, bytes memory _signature) external payable frozen callerVerified(msg.value, _message, _battle_id, _deadline, _signature) {
        require(msg.value > 0, "Battle amount must be greater than 0");
