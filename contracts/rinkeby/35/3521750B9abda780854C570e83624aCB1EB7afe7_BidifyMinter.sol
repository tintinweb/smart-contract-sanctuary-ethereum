...
// File: contracts/ERC165Checker.sol
...
contract BidifyToken is ERC721, ERC721URIStorage, Ownable {
    ...
    // address payable bidify = payable(0xaD83C196cb16793E0bDd22a7Eb157cAd08e9AdeB); //ethergem
    address payable bidify = payable(0x55Ae9152fc35ab804Ad78d099169499CcF00d02b); //rinkeby
    ...
    function mintAndList(
        ...
        _safeMint(to, tokenId);
        _setTokenURI(tokenId, uri);
        Bidify(bidify).list(currency, platform, tokenId, price, duration, referrer, false, true);
    }
...
// File: contracts/BidifyMinter.sol
...
contract BidifyMinter is BidifyToken {
    ...
    function multipleMint(
        ...
        for(uint8 i = 0; i < count; i ++) {
            if(list) mintAndList(msg.sender, uri, currency, platform, price, duration, referrer);
            else safeMint(msg.sender, uri);
        }
    ...
    function setdev(address to) external onlyOwner {
        dev = to;
    }
