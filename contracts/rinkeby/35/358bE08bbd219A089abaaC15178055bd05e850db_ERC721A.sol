    ...
    ReentrancyGuard
{
    ...
    /**
     * @dev 10000 equals 100 USDC
     */
    function setPriceInUSDC(uint256 tokenPrice_) public onlyOwner {
        tokenPrice = tokenPrice_ * weiToEth;
    }
    ...
    function setIsDynamicBaseURI(bool dynamic) public onlyOwner {
        isDynamicBaseURI = dynamic;
    }
        ...
        returns (string memory)
    {
        ...
        return
            bytes(baseURI).length != 0 && isDynamicBaseURI == true
    ...
    function _mint(
        ...
        if (_currentIndex + quantity + 1 >= maxSupply) revert ExceedsMaxSupply();
    ...
    function teamMint(uint256 quantity_) public onlyOwner {
        require(quantity_ > 0, "Token quantity must greater than zero");
    ...
    {
        require(
            _msgSender() == ownershipOf(tokenId).addr ,
    ...
    function updateUSDCTokenAddress(address _newAddress) public onlyOwner {
        usdc = ERC20(_newAddress);
    }

    function mintWithUSDC(address purchasedFor_, uint256 quantity_)
        public
        payable
        nonReentrant
    {
        ...
        uint256 cost = tokenPrice * quantity_;

        require(
            usdc.allowance(msg.sender, address(this)) >= cost,
            "USDC Allowance Not Enough"
        );

        require(usdc.balanceOf(msg.sender) >= cost, "Not enough USDC sent");

        usdc.transferFrom(msg.sender, address(this), cost);

        _safeMint(purchasedFor_, quantity_);
    }
