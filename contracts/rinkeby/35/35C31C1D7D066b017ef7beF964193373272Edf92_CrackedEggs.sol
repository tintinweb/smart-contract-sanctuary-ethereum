...
contract CrackedEggs is ERC721, Ownable, ReentrancyGuard {
    ...
    uint256 public maxTotalSupply = 20; //5000

    uint256 public freeMintSupply = 10; //4000
    ...
    //string public baseURI; //muss definiert werden
    string public uriPrefix = ""; //
    string public uriSuffix = ".json"; //
    string public hiddenMetadataUri; //

    address private vaultAddress = address(0); //verbindung zu env
    bytes32 public merkleRoot; //rausfinden was das ist => Weg um Whitelist zu kontrollieren
    ...
    function mint(uint256 _amount, bytes32[] memory _whitelistProof)
        external
        payable
        nonReentrant
    {
        ...
        require(
            _amount.add(walletCount) <= maxPerWallet,
            "Exceeds max per wallet"
        );

        uint256 availableFreeMintCount = freeMintCountForAddress(
            _msgSender(),
            _whitelistProof
        );
        require(
            mintPrice.mul(_amount.sub(availableFreeMintCount)) <= msg.value,
    ...
    function freeMint(bytes32[] memory _whitelistProof) external nonReentrant {
        require(isSaleOpen, "Sale not open");
        require(
            totalSupply().add(1) <= maxTotalSupply,
            "Exceeds max total supply"
        );
    ...
    function freeMintCountForAddress(
        address _address,
    ...
    function privateMint(uint256 _amount, address _receiver)
        external
        onlyOwner
    {
        require(_amount > 0, "Must mint at least one");
    ...
    function setMaxTotalSupply(uint256 _maxValue) public onlyOwner {
        maxTotalSupply = _maxValue;
    }

    function setFreeMintSupply(uint256 _maxValue) public onlyOwner {
        freeMintSupply = _maxValue;
    }

    function setMaxPerTransaction(uint256 _maxValue) public onlyOwner {
        maxPerTransaction = _maxValue;
    }

    function setMaxPerWallet(uint256 _maxValue) public onlyOwner {
        maxPerWallet = _maxValue;
    }

    function setIsSaleOpen(bool _open) public onlyOwner {
        isSaleOpen = _open;
    }

    function setIsFreeForWhitelistedOnly(bool _state) public onlyOwner {
