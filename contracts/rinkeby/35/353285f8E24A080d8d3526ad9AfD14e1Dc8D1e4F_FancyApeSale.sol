...
contract FancyApeSale is FancyV2Sale {
    constructor(
...
interface IFancy721 is IERC721Enumerable {

    function safeMint(address _to, uint256[] calldata _apeTokenIds) external;
    function tokensInWallet(address _owner) external view returns(uint256[] memory);
...
contract FancyV2Sale is AccessControlEnumerable {
    ...
    function mint(uint256[] calldata _referenceTokenIds) public payable {
        ...
        // Make sure to approve the Hive to pull Honey from the Presale contract
        ...
        hiveContract.depositHoneyToTokenIdsOfCollections(collections, _referenceTokenIds, amounts);
    }

    function withdrawETHBalance(address _address) public onlyRole(MANAGER_ROLE) {
        uint256 balance = address(this).balance;
        require(payable(_address).send(balance));
    }

    function withdrawHoneyBalance(address _address) public onlyRole(MANAGER_ROLE) {
        require(saleState == SaleState.Off, "withdrawHoneyBalance: claim must be off");
    ...
    function setClaimState(SaleState _claimState) public onlyRole(MANAGER_ROLE) {
    ...
    function updateHoneyReward(uint256 _honeyReward) public onlyRole(MANAGER_ROLE) {
        require(saleState == SaleState.Off, "updatePrice: claim must be off");
