...
contract GhoraGhoriClub is ERC1155, Ownable, Pausable, ERC1155Supply {
  ...
  mapping(uint256 => uint256) private currentUriIdx;
  ...
  function setURI(uint256 _tokenId, string memory newURI) public onlyController {
    uint256 currentIdx = currentUriIdx[_tokenId];
    uint256 newIdx = currentIdx + 1;
    uris[_tokenId][currentIdx + 1] = newURI;
    currentUriIdx[_tokenId] = newIdx;
  }

  function setReveal(bool _reveal) public onlyController {
    reveal = _reveal;
  }

  function setSaleContract(address _saleContract) public onlyController {
    saleContract = _saleContract;
  }

  function setLastTokenId(uint256 _tokenId) public onlyController {
  ...
  function uri (uint256 _tokenId) override public view returns (string memory) {
    ...
    uint256 currentIdx = currentUriIdx[_tokenId];
    string memory dataURI = uris[_tokenId][currentIdx];
