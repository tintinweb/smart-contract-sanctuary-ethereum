...
contract Vault is BaseContract
{
    ...
    function initialize() initializer public
    {
        ...
        maxReturn = 36000;
        period = 300; // DEV period is 5 minutes.
        //period = 86400 // PRODUCTION period is 24 hours.
        lookback = period * 28; // 28 periods.
        penaltyLookback = period * 7; // 7 periods.
        neutralClaims = 13;
        negativeClaims = 15;
        penaltyClaims = 7;
        penaltyRate = 50;
    ...
    function effectiveClaims(address player_) public view returns (uint256)
    {
        ...
        uint256 _penaltyClaims_ = currentPenaltyClaims(player_);
        if(_penaltyClaims_ >= penaltyClaims) {
            return 28;
        }
        uint256 _periodClaims_ = currentPeriodClaims(player_);
        ...
        if(startTime[player_] >= block.timestamp - lookback && _periodClaims_ < neutralClaims) {
    ...
    {
        return block.timestamp - lookback;
    }

    function penaltyStart() public view returns (uint256)
    {
        return block.timestamp - penaltyLookback;
    }

    function currentPeriodClaims(address player_) public view returns (uint256)
    {
        return _claimsSinceTimestamp(player_, periodStart());
    }

    function currentPenaltyClaims(address player_) public view returns (uint256)
    {
        return _claimsSinceTimestamp(player_, penaltyStart());
    }

    function claimLength(address player_) public view returns (uint256)
    {
        return claims[player_].length;
    }

    function _claimsSinceTimestamp(address player_, uint256 timestamp_) internal view returns (uint256)
    {
        uint256 _claims_ = 0;
        for(uint i = 0; i < claims[player_].length; i++) {
            if(claims[player_][i] >= timestamp_) {
                _claims_ ++;
            }
        }
        return _claims_;
    ...
    {
        uint256 _available_ = (rewardPeriod(player_) * rewardPercent(player_) * totalDeposit[player_]) / 100000000;
     ...
     * @param player_ Address of player.
     * @return uint256 Time since last player action.
     */
    function timeSinceLastAction(address player_) public view returns (uint256)
    {
        return block.timestamp - lastAction[player_];
    }

    /**
     * Reward period.
     * @param player_ Address of player.
     * @return uint256 Time since last action devided by period.
     * @dev Returns period * 10000
     */
    function rewardPeriod(address player_) public view returns (uint256)
    {
        return (timeSinceLastAction(player_) * 10000) / period;
    }
    ...
    function _claim(address player_) internal returns (bool)
    {
        ...
        claims[player_].push(block.timestamp);
        if(!maxed[player_]) {
    ...
    {
        return maxPayout / (maxReturn / 1000);
    }

    /**
     * Player status.
     * @param player_ Address of player.
     * @return uint256 Returns a player's status (1 = negative, 2 = neutral, 3 = positive).
     */
    function playerStatus(address player_) external view returns (uint256)
    {
        uint256 _status_ = 3;
        uint256 _effectiveClaims_ = effectiveClaims(player_);
        if(_effectiveClaims_ >= neutralClaims) _status_ = 2;
        if(_effectiveClaims_ >= negativeClaims) _status_ = 1;
        if(startTime[player_] == 0) {
            _status_ = 2;
        }
        return _status_;
     ...
     */
    function claimPrecheck(address player_) external view returns (uint256)
    {
        if(penalized[player_]) {
            return rates[28];
        }
        uint256 _penaltyClaims_ = currentPenaltyClaims(player_) + 1;
        if(_penaltyClaims_ >= penaltyClaims) {
            return rates[28];
        }
        uint256 _periodClaims_ = currentPeriodClaims(player_) + 1;
        if(negative[player_] && _periodClaims_ < negativeClaims) {
            _periodClaims_ = negativeClaims;
        }
        if(_periodClaims_ > 28) {
    ...
    /**
     * Update lookback.
     * @param lookback_ New lookback.
     */
    function updateLookback(uint256 lookback_) external onlyOwner
    {
        lookback = lookback_;
    }

    /**
     * Update penaltyLookback.
     * @param penaltyLookback_ New penaltyLookback.
     */
    function updatePenaltyLookback(uint256 penaltyLookback_) external onlyOwner
    {
        penaltyLookback = penaltyLookback_;
    }
    ...
    {
        negativeClaims = negativeClaims_;
    }
    ...
    {
        penaltyClaims = penaltyClaims_;
    }

    /**
     * Update penalty rate.
     * @param penaltyRate_ New penaltyRate.
     */
    function updatePenaltyRate(uint256 penaltyRate_) external onlyOwner
    {
