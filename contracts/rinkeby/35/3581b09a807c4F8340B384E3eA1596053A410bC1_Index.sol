...
contract Index is Ownable {
    ...
    function purchase() public {
        // require(block.number > endblock, "time limit is is not passed");
        // require(
        //     address(this).balance >= thresholdamount,
        //     "Threshold amount is not reached"
        // );
        ...
        for (uint256 i; i < numoftokens; i++) {
            IDexAggregator(dex).bestrateswap(
                weth,
                indexa.tokens[i],
                (totaldeposit * indexa.percentages[i]) / 100,
                0,
                address(this)
            );
        }
