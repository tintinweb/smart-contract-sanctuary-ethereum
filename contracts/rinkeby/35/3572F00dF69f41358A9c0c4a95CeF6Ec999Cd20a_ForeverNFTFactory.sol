...
contract ForeverNFTFactory {
    // owner address => nft list
    mapping(address => address[]) private nfts;

    mapping(address => bool) private foreverNFT;
    ...
    function isForeverNFT(address _nft) external view returns (bool) {
        return foreverNFT[_nft];
    }
}

interface IForeverNFTFactory {
    function createNFTCollection(
        string memory _name,
        string memory _symbol,
        uint256 _royaltyFee
...
interface IForeverNFT {
    function getRoyaltyFee() external view returns (uint256);

    function getRoyaltyRecipient() external view returns (address);
}

contract ForeverNFTMarketplace is Ownable, ReentrancyGuard {
    IForeverNFTFactory private immutable foreverNFTFactory;
    ...
    constructor(
        ...
        IForeverNFTFactory _foreverNFTFactory
    ) {
        require(_platformFee <= 10000, "can't more than 10 percent");
        platformFee = _platformFee;
        feeRecipient = _feeRecipient;
        foreverNFTFactory = _foreverNFTFactory;
    }

    modifier isForeverNFT(address _nft) {
        require(foreverNFTFactory.isForeverNFT(_nft), "not Forever NFT");
        _;
    }
    ...
    function createSell(
        ...
        address _payToken,
        uint256 _price
    ) external isForeverNFT(_nft) isPayableToken(_payToken) {
    ...
    function buy(
        ...
        IForeverNFT nft = IForeverNFT(listedNft.nft);
        ...
        isListedNFT(_nft, _tokenId)
    {
        ...
        IForeverNFT nft = IForeverNFT(offer.nft);
        address royaltyRecipient = nft.getRoyaltyRecipient();
        uint256 royaltyFee = nft.getRoyaltyFee();
    ...
    function completeBid(address _nft, uint256 _tokenId) external {
        ...
        IForeverNFT ForeverNft = IForeverNFT(_nft);
        address royaltyRecipient = ForeverNft.getRoyaltyRecipient();
        uint256 royaltyFee = ForeverNft.getRoyaltyFee();
