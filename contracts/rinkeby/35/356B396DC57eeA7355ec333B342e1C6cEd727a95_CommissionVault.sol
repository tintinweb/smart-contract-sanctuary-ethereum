...
contract CommissionVault is Ownable {
    ...
    function exchangeNEND() external {
        for (uint256 i = 0; i < tokens.length; i++) {
            ...
            (, bytes memory wethRes) = router.staticcall(abi.encodeWithSignature("WETH()"));

            assembly {
                weth := mload(add(0x20, wethRes))
            }
            ...
            // Call using selector since Uniswap Router contract is of version 0.6.6 which can not compile
            ...
            // UniswapV2Router02 rt = UniswapV2Router02(router);

            // rt.swapExactTokensForTokens(
            //     balance,
            //     0,
            //     [addr, rt.WETH, nend],
            //     pool,
