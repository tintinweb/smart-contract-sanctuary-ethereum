...
contract RapStars is Ownable, ERC721A {
    ...
    uint256 public WL_Price = 0.0555 ether;
    uint256 public maxSupply = 555;
    uint256 public perTxLimit = 1;
    uint256 public perTxLimitPresale = 1;
    
    mapping(address => bool) private presaleList;
    mapping(address => uint256) public publicsaleMinted;
    constructor() ERC721A("RapStars", "RS"){}

    function mintNFT(uint256 _quantity) public payable {
        require(_quantity > 0 && _quantity <= perTxLimit, "Wrong Quantity.");
        require(totalSupply() + _quantity < maxSupply, "Reaching max supply");
        require(msg.value == price * _quantity, "Incorrect Eth Amount");
        require(publicsaleMinted[msg.sender] + _quantity <= adoptionLimit, "Exceed max adoption amount");
        require(publicSaleOpen, "Public Sale Not Started Yet!");

        _safeMint(msg.sender, _quantity);
        publicsaleMinted[msg.sender] = publicsaleMinted[msg.sender] + _quantity;

    }
    function mintNFTPresale(uint256 _quantity) public payable {
        require(_quantity > 0 && _quantity <= perTxLimitPresale, "Wrong Quantity.");
        require(totalSupply() + _quantity < maxSupply, "Reaching max supply");
        require(msg.value == WL_Price * _quantity, "Incorrect Eth Amount");
        require(getMintedCount(msg.sender) + _quantity <= adoptionLimitPresale, "Exceed max adoption amount");
        require(privateSaleOpen, "Presale Not Started Yet!");
   ...
   function sendGiftsToWallet(address _wallet, uint256 _num) external onlyOwner{
    ...
    function setPerTxLimit(uint256 _limit) public onlyOwner() {
        perTxLimit = _limit;
    }
    function setPerTxLimitPresale(uint256 _limit) public onlyOwner() {
        perTxLimitPresale = _limit;
    }
