...
contract moonFly is ERC721, Ownable {
  ...
  function earlyMint(bytes32[] calldata _merkleProof, uint256 _mintAmount) public payable {
    ...
    require(_tokenSupply.current() + _mintAmount <= maxSupply, "all flies are minted!");

    bytes32 leaf = keccak256(abi.encodePacked(msg.sender));
    require(MerkleProof.verify(_merkleProof, merkleRoot, leaf), "invalid proof, you're not on the pregame list.");

    require(addressMintedBalance[msg.sender] + _mintAmount <= MintPerAddressLimit, "max flies per address exceeded");
