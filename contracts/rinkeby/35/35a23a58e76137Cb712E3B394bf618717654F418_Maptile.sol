...
// File: utility/interfaces/CitizenInterface.sol
...
interface CitizenInterface {
       ...
       // EVENTS
    event StakeNFT(uint32[] tokenIDs, address indexed owner);

    event UnstakeNFT(uint32[] tokenIDs, address indexed owner);

    event UpdateTokenURI(uint32 tokenId, string indexed uri);

    event Reveal();

    event ToggleStaking(bool value);
    ...
    function staking(uint32[] calldata tokenIds) external;

    function unstaking(uint32[] calldata tokenIds) external;

    function getStakingTime(uint32 tokenId) external view returns (bool, uint256, uint256);

    function updateTokenURI(uint32 tokenId, string memory uri, bytes memory sig) external;
    ...
    function preSaleSafeMint(uint256 quantity, bytes32[] calldata merkleProof) external payable;
...
// File: contracts/Copy_ERC721A.sol
...
contract Maptile is ERC721A, BaseTokenURI {
    using Strings for uint256;
    ...
    mapping(uint32 => uint256) private _totalSakingTime;

    // Mapping for token URIs
    mapping(uint256 => string) private _tokenURIs;

    // Mapping for maptile allotment
    mapping(uint32 => bool) public _maptileAllotted;

    // Whether staking is currently allowed or not
    bool public _stakingOpen = false;

    // Boolean to control claim maptile
    ...
    // String for not revealed URI
    string public _notRevealedURI;

    // Citizen contract interface
    CitizenInterface public _citizen;

    // TODO check for final name and symbol
    constructor(address platformAddress, string memory notRevealURI, uint32 maxSupply_, address citizen)
        ERC721A("Maptile", "MAPTILE") 
        BaseTokenURI("") 
        {
        _platformAddress = address(platformAddress);
        _notRevealedURI = notRevealURI;
        _maxSupply = maxSupply_ + 1;
        _citizen = CitizenInterface(citizen);
    }

    // EVENTS
    event StakeMaptile(uint32[] tokenIDs, address indexed owner, uint256 time);

    event UnstakeMaptile(uint32[] tokenIDs, address indexed owner, uint256 time);

    event UpdateMaptileURI(uint32 tokenID, string indexed uri, uint256 time);

    event Reveal(uint256 time);

    event ToggleStaking(bool value, uint256 time);

    event ToggleClaimMaptile(bool value, uint256 time);

    event TokenMinted(address to, uint32[] citizentokenIDs, uint256 userMinted, uint256 totalMinted);

    // MODIFIERS
    modifier claimIsOpen {
        require(_isClaimActive, "claim maptile is closed");
    ...
    function staking(uint32[] calldata tokenIDs)
        public
        {
        ...
        emit StakeMaptile(tokenIDs, msg.sender, block.timestamp);
    }

    function unstaking(uint32[] calldata tokenIDs)
        public
        {
        for (uint32 i = 0; i < tokenIDs.length; i++) {
            require(_exists(tokenIDs[i]), "requesting for non existing tokenID");
            ...
            _totalSakingTime[tokenIDs[i]] += start;
            _stakingStartTime[tokenIDs[i]] = 0;
        }
        emit UnstakeMaptile(tokenIDs, msg.sender, block.timestamp);
    }

    function getStakingTime(uint32 tokenID)
        public
        view
    ...
    function updateTokenURI(uint32 tokenID, string memory uri, bytes memory sig)
        public
        {
        ...
        emit UpdateMaptileURI(tokenID, uri, block.timestamp);
    }
    ...
    function toggleClaim()
        ...
        emit ToggleClaimMaptile(_isClaimActive, block.timestamp);
    }
    ...
    // minting function
    function claimMaptile(uint32[] calldata citizenTokenIds)
        public
        claimIsOpen
        {
        require(totalSupply() + citizenTokenIds.length < _maxSupply, "platform reached limit of minting");
        for(uint32 i = 0; i < citizenTokenIds.length; i++) {
            require(_citizen.ownerOf(citizenTokenIds[i]) == msg.sender, "caller is not the owner of this tokenID");
            require(!_maptileAllotted[citizenTokenIds[i]], "this NFTs maptile has already claimed");
            _maptileAllotted[citizenTokenIds[i]] = true;
        }
