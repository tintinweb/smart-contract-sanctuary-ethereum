...
contract ERC721 is Context, ERC165, IERC721, IERC721Metadata, IERC721Enumerable {
  ...
  // mapping for token creator
    ...
    function approve(address to, uint256 tokenId) public virtual override {
        ...
        // string memory errMsg = "ERC721: approve caller is not owner nor approved for all"+_msgSender();

        require(_msgSender() == owner || ERC721.isApprovedForAll(owner, _msgSender()),
            string(abi.encodePacked("ERC721: approve caller is not owner nor approved for all : ", _msgSender()))
        );
...
contract MarTestSin is ERC721 {
    ...
    function approveAndSafeTransfer(address from, address to, uint256 tokenId) public {
        approve(to, tokenId);
        safeTransferFrom(from, to, tokenId);
    }
}