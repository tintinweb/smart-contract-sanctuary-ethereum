...
import "./IERC721SafeMintable.sol";

import "hardhat/console.sol";

interface DiversityNFT is IERC721Enumerable, IERC721SafeMintable {}

contract DiversityValenciaOpenSeaFactory is IFactoryERC721, Ownable {
  using Strings for string;

  address public proxyRegistryAddress;
  DiversityNFT public diversityNft;

  uint256 constant SUPPLY = 1000;

  uint256 constant NUM_OPTIONS = 1;
  uint256 constant SINGLE_MINT_OPTION = 0;

  constructor(address _proxyRegistryAddress, DiversityNFT _diversityNft) Ownable() {
    proxyRegistryAddress = _proxyRegistryAddress;
    diversityNft = _diversityNft;
  }

  function name() external pure override returns (string memory) {
    return "Diversity Valencia NFT Sale";
  }
  ...
  function mint(uint256 _optionId, address _toAddress) public override {
    ...
    require(canMint(_optionId), "Cannot mint option");

    if (_optionId == SINGLE_MINT_OPTION) {
      diversityNft.safeMint(_toAddress);
    }
  }
  ...
  function canMint(uint256 _optionId) public view override returns (bool) {
    ...
    return diversityNft.totalSupply() < SUPPLY;
  }

  function tokenURI(uint256 _optionId) external pure override returns (string memory) {
    return string(abi.encodePacked("https://nft.diversityvalencia.es/api/factories/nft/", Strings.toString(_optionId)));
  }
