...
contract BuddiNFT is Ownable,RoyaltiesAddon,ERC2981
{
    using SafeMath for uint256;
    using Strings for uint256;

    uint public constant _TOTALSUPPLY =10000;
    uint public _BATCHSUPPLY = 2500;

    uint public maxQuantity = 10;
    uint256 public price = 0.005 ether;

    uint private tokenId=45;
  ...
  uint public status = 0; //0-pause 1-Whitelist 2-Public
    ...
    constructor() ERC721("My Buddi NFT", "MBD")  {
    }
  ...
  function setBaseURI(string memory _baseURI) external onlyOwner {
    ...
    function mint(uint chosenAmount) public payable isSaleOpen{
        ...
        require(totalSupply()+chosenAmount<=_BATCHSUPPLY ,"Quantity must be lesser then Batch Supply");
   ...
   function setStatus(uint256 s)external onlyOwner{
    ...
    uint256 public itemPriceWhiteList = 0.004 ether;
    ...
    function purchaseWhiteListTokens(uint256 _howMany, bytes32[] calldata _proof) external payable {
        ...
        require(totalSupply()+_howMany<=_BATCHSUPPLY,"Quantity must be lesser then Whitelist MaxSupply");
