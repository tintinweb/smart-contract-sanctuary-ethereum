...
*/
    ...
    function mint(uint256 numberOfCards) external payable {
        bool saleCheck = checkSaleIsActive();
        bool finalCheck = (saleCheck || checkPublicSaleIsActive());
    ...
    function setPublicDates(uint256 _start, uint256 _end) external onlyAllowed {
        _publicStart = _start;
        _publicEnd = _end;
    }
    ...
    function checkPublicSaleIsActive() public view returns (bool) {
        if (
            (_publicStart <= block.timestamp) && (_publicEnd >= block.timestamp)
        ) return true;
        return false;
    }
