...
contract BunnyTestv1 is ERC721A, Pausable, PaymentSplitter {
 ...
 struct PresaleConfig {
    ...
    address[] private _team = [
        0xEBFa7B1E9A45B9Cf4f47300c5d7A4328Bd8Bf735,
        0x4329Dc8C177034C57922935A6420eBeAF77c49B0,
        0xDCeE86B734C59C53987f6CF0402D6627e56d9eF5, 
 ...
 constructor(
  ...
  ) ERC721A("Bunny Test", "Bunny Test", _maxBatchSize, _maxTotalSupply)  
    PaymentSplitter(_team, _teamShares){

    require(
     ((_maxGiftSupply <= _maxTotalSupply) &&  (_maxBatchSize <= _maxTotalSupply)),
      "Bunny Test: larger collection size needed"
    );
...
function setBaseURI(string calldata _tokenBaseURI) external onlyOwner {
        ...
        onlyOwner
    {
        for (uint256 ind = 0; ind < _addresses.length; ind++) {
            require(
                ...
                "Bunny Test: Can't add a zero address"
            );
        ...
        onlyOwner
    {
        for (uint256 ind = 0; ind < _addresses.length; ind++) {
            require(
                _addresses[ind] != address(0),
                "Bunny Test: Can't remove a zero address"
            );
    ...
    function setUpPresale(uint256 _duration, uint256 _maxCount ) external onlyOwner {
      require( _duration > 0,
            "Bunny Test: presale duration is zero"
        );
        require(_maxCount <= maxBatchSize ,
            "Bunny Test: maxCount is higher than maxBatchSize "
        );
        ...
        whenNotPaused
    {
        require(
            ...
            "Bunny Test:  max total sypply is is exceeded"
        );
        require(
            giftCount + _amount <= maxGiftSupply,
            "Bunny Test: max gift supply exceeded"
        );
        ...
        for (uint256 ind = 0; ind < _users.length; ind++) {
            ...
            require(
                ...
                "Bunny Test: user is the zeo address"
            );
    ...
    function presaleMint(uint256 _amount) internal {
        ...
        require(
            ...
            "Bunny Test: Presale must be active"
        );
        require(
            block.timestamp >= _presaleConfig.startTime,
            "Bunny Test: Presale not started"
        );
        require(
            block.timestamp <=
                _presaleConfig.startTime + _presaleConfig.duration,
            "Bunny Test: Presale is ended"
        );
        if (isPresale){
        require(
            _presaleList[msg.sender] == true,
            "Bunny Test: Caller is not on the presale list"
        );
        }
         require(
            (totalSupply() +  _amount) <= collectionSize,
        ...
        require(
            ...
            "Bunny Test: max count per transaction is exceeded"
        );
       
        require(
            (price * _amount) <= msg.value,
            "Bunny Test: Ether value sent is not correct"
        );
    ...
    function saleMint(uint256 _amount) internal {
        ...
        require(_amount > 0, "Bunny Test: zero amount");
        require(_saleConfig.startTime > 0, "Bunny Test: sale is not active");
        require(
        ...
        require(
         ...
         require(
            (totalSupply() + _amount) <= collectionSize,
            "Bunny Test: max total sypply is exceeded"
        );
        require(
            (price * _amount) <= msg.value,
            "Bunny Test: Ether value sent is not correct"
    ...
    function burn(uint256 tokenId) external {
        require(isBurnEnabled, "Bunny Test: burning disabled");
        require(
            _isApprovedOrOwner(msg.sender, tokenId),
            "Bunny Test: burn caller is not owner nor approved"
        );
        _burn(tokenId);
    }
