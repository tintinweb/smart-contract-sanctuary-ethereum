...
contract NFT is ERC721, Ownable, ERC2981 {
  ...
  uint256 public constant T0SUPPLY = 17;
  ...
  uint256 public constant MAXLEGENDARYSUPPLY = T0SUPPLY + T1SUPPLY;
  uint256 public constant MAXGUARDIANSUPPLY = T0SUPPLY + T1SUPPLY + T2SUPPLY;
  uint256 public constant MAXKEEPERSUPPLY = T0SUPPLY + T1SUPPLY + T2SUPPLY + T3SUPPLY;
  uint256 public constant GUARDIANWLCOST = 0 ether; //0.15
  uint256 public constant GUARDIANCOST = 0 ether; //0.15
  uint256 public constant KEEPERWLCOST = 0 ether; //0.1
  uint256 public constant KEEPERCOST = 0 ether; //0.1
  ...
  bytes32 public gMerkleRoot = 0xf09f32dde82949941e7f100ab6e18016a2281851cfd1acf1726d5dbd998f7ae6;
  mapping(address => bool) public gWhitelistClaimed;

  bytes32 public kMerkleRoot = 0xf09f32dde82949941e7f100ab6e18016a2281851cfd1acf1726d5dbd998f7ae6;
  mapping(address => bool) public kWhitelistClaimed;

  constructor() ERC721("10 Alphabetss", "10AS") {
    //Contract interprets 10,000 as 100%.
    setDefaultRoyalty(w1, 1000); //10%
   }
...
//*** PUBLIC FUNCTION ***//
  function guardianSaleMint(uint256 _mintAmount, bytes32[] calldata _merkleProof) public payable {
    ...
    if (msg.sender != owner()) {
      ...
      require(msg.value >= GUARDIANWLCOST * _mintAmount, "Insufficient Eth.");
    }
  ...
  function keeperSaleMint(uint256 _mintAmount, bytes32[] calldata _merkleProof) public payable {
    ...
    if (msg.sender != owner()) {
      ...
      require(msg.value >= KEEPERWLCOST * _mintAmount, "Insufficient Eth.");
    }
...
//*** ONLY OWNER FUNCTION **** //
  ...
  function mintAllBeginner() public onlyOwner {
    require(!paused);
    require(_periodMintStatus == PeriodMintStatus.CLOSED, "CP: Minting status invalid.");
    uint256 mintAmount = T0SUPPLY - supply.current();
    require(mintAmount > 0 && supply.current() + mintAmount <= T0SUPPLY, "Out of beginner supply.");

    _mintLoop(w1, mintAmount);
  }

  function mintAllLegendary() public onlyOwner {
    require(!paused);
    require(_periodMintStatus == PeriodMintStatus.LEGENDARY, "CP: Minting status invalid.");
    uint256 mintAmount = MAXLEGENDARYSUPPLY - supply.current();
    require(mintAmount > 0 && supply.current() + mintAmount <= MAXLEGENDARYSUPPLY, "Out of legendary supply.");
  ...
  function mintAllKeeper() public onlyOwner {
    ...
    _mintLoop(w1, _mintAmount);
  }

  function bulkAirDropLegendary(address[] calldata _airDropAddresses) public onlyOwner {
    require(!paused);
    require(_periodMintStatus == PeriodMintStatus.LEGENDARY, "CP: Minting status invalid.");
    require(supply.current() + _airDropAddresses.length <= MAXLEGENDARYSUPPLY, "Out of legendary supply.");
