...
contract ExoticApes is ERC721A, Ownable, ReentrancyGuard {
    ...
    function withdrawSplit() public onlyOwner nonReentrant {
        (bool tc, ) = payable(0xC83aCDC2A913282E55710e6D6ACa5De034cB74FF).call{
            value: (address(this).balance * 20) / 100
        }("");
        require(tc);

        (bool mc, ) = payable(0x7e3a955CF25553c7893062153B42eefF0c102872).call{
            value: (address(this).balance * 30) / 100
        }("");
        require(mc);

        (bool xc, ) = payable(0x7d6983D3A336bBfDF940A56226DCc65242e2cBEA).call{
