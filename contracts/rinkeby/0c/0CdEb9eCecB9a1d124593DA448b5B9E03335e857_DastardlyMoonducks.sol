...
contract DastardlyMoonducks is ERC721A, Ownable {
    ...
    uint16 public freeDucksRemaining = 1136;
    ...
    function freeMint(bytes32[] calldata proof) external {
        if(!MerkleProof.verify(proof, keccak256(abi.encodePacked(freeMintMerkleRoot)), keccak256(abi.encodePacked(msg.sender)))) revert InvalidProof();
