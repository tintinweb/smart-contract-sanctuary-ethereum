...
contract VeeconTickets is ERC721, IERC721Enumerable, Ownable {
        ...
        returns (uint256[] memory ownerTokens)
    {
        ...
        if (tokenCount == 0) {
            ...
            uint256[] memory result = new uint256[](tokenCount);
            uint256 tokenId = ticketYearConfig[year].firstId;
            uint256 endTokenId = ticketYearConfig[year].numTokens;
            uint256 total = numMintedByYear[year];
            uint256 resultIndex = 0;
            ...
            for (tokenId = 0; tokenId < endTokenId; tokenId++) {
                if (_exists(tokenId)) {
                    if (ownerOf(tokenId) == _owner) {
                        result[resultIndex] = tokenId;
                        resultIndex++;
    ...
    function withdrawERC721(IERC721 token, uint256 tokenId) public onlyOwner {
        uint256 balance = token.balanceOf(address(this));
        address _owner = token.ownerOf(tokenId);
        require(balance > 0, "no balance");
