...
contract Habitats is ERC721AQueryable, ERC2981, Ownable, ReentrancyGuard {
    ...
    mapping(address => PreApprovalState) private preApprovalStates;
    ...
    function presaleMint(uint256 numTokens) external payable nonReentrant {
        ...
        require(totalSupply() + numTokens <= COLLECTION_SIZE, "Max supply reached");
    ...
    function reserve(uint256 numTokens) external onlyOwner {
        require(totalSupply() + numTokens <= RESERVE_LIMIT, "Cannot reserve that many");
        require(totalSupply() + numTokens <= COLLECTION_SIZE, "Max supply reached");
    ...
    function isApprovedForAll(address owner, address operator) public view override returns (bool) {
        ...
        return preApprovalStates[owner] == PreApprovalState.Active && _proxyRegistry.proxies(owner) == operator;
    }

    /**
     * @notice Allows users to disable OpenSea pre approval
     */
    function setPreApprovalState(bool isApproved) public {
        preApprovalStates[msg.sender] = isApproved ? PreApprovalState.Active : PreApprovalState.Inactive;
    }

    /**
     * @notice Returns true if the wallet has enabled OpenSea pre approval and false otherwise
     */
    function isPreApproved(address wallet) public view returns (bool) {
        return preApprovalStates[wallet] == PreApprovalState.Active;
    }
    ...
    function withdraw() external onlyOwner nonReentrant {
        ...
        (bool s1, ) = _a1.call{value: seventyPercentOfBalance}("");
        (bool s2, ) = _a2.call{value: sixteenPercentOfBalance}("");
        (bool s3, ) = _a3.call{value: fourteenPercentOfBalance}("");
        require(s1 && s2 && s3, "A transfer failed");
    }
