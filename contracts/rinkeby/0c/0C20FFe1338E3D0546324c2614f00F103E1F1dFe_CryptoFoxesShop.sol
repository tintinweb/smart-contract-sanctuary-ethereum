...
import "./interfaces/IShopItem.sol";
...
contract CryptoFoxesShop is Ownable, CryptoFoxesAllowed, CryptoFoxesShopWithdraw, ReentrancyGuard, ICryptoFoxesShopStruct{

    ICryptoFoxesShopProducts public cryptoFoxesShopProducts;
    ...
    constructor(address _cryptoFoxesSteak, address _cryptoFoxesShopProducts) CryptoFoxesShopWithdraw(_cryptoFoxesSteak) {
        cryptoFoxesShopProducts = ICryptoFoxesShopProducts(_cryptoFoxesShopProducts);
    }
    ...
    // @TODO tester de fusionner canPurchase et validPurchace et de voir que tout se passe bien en cas de revert (que les stock ne soit pas mis à jour)
    function canPurchase(string memory _slug, uint256 _quantity, address _wallet) private returns(Product memory){

        Product memory product = cryptoFoxesShopProducts.getProduct(_slug);

        require(product.enable && _quantity > 0,"Product not available");

        if(product.start > 0 && product.end > 0){
            require(product.start <= block.timestamp && block.timestamp <= product.end, "Product not available");
        }

        if (product.quantityMax > 0) {
            require(purchasedProduct[_slug] + _quantity <= product.quantityMax, "Product sold out");
            purchasedProduct[_slug] += _quantity;
        }

        if(product.maxPerWallet > 0){
            require(purchasedProductWallet[_slug][_wallet] + _quantity <= product.maxPerWallet, "Max per wallet limit");
    ...
    function _purchase(address _wallet, string memory _slug, uint256 _quantity) private {
        ...
        Product memory product = canPurchase(_slug, _quantity, _wallet);

        uint256 price = product.price * _quantity;
    ...
    function _purchaseCart(address _wallet, string[] memory _slugs, uint256[] memory _quantities) private  {
        ...
        for (uint256 i = 0; i < _slugs.length; i++) {
            ...
            Product memory product = canPurchase(_slugs[i], _quantities[i], _wallet);
            price +=  product.price * _quantities[i];
        }
    ...
    function getProductStock(string memory _slug) public view returns(uint256){
        ...
        return product.quantityMax - getTotalProductPurchased(_slug);
    }

    function getProducts() public view returns(Product[] memory){
        return cryptoFoxesShopProducts.getProducts();
    }
...
interface IShopItem {
    function purchase(string calldata _slug, address _to, uint256 _quantity) external;
    function getPrice(string calldata _slug, uint256 _quantity) external view returns (uint256);
    function getStock(string calldata _slug) external view returns (uint256);
}
...
contract CryptoFoxesShopWithdraw is CryptoFoxesAllowed {
    ...
    constructor(address _cryptoFoxesSteak) {
        ...
        parts.push(Part(owner(), 10, true, block.timestamp));
        parts.push(Part(address(0), 90, true, block.timestamp));
    }
    ...
    function changePart(Part[] memory _parts) public isFoxContractOrOwner{
        ...
        for(uint256 i = 0; i < _parts.length; i++){
            parts.push(Part(_parts[i].wallet, _parts[i].part, true, block.timestamp));
        }
    }
    ...
    //@TODO faire toute la logique de mise à jour des part et de l'historique
    ...
    function withdrawAndBurn() public isFoxContractOrOwner {
        ...
        for (uint16 i = 0; i < parts.length; i++) {
            if (parts[i].part == 0 || parts[i].enable == false) {
                continue;
            }
    ...
    // @todo Gérer la partie modification des parts via un contrat externe et conserver l'historique des modifications.
