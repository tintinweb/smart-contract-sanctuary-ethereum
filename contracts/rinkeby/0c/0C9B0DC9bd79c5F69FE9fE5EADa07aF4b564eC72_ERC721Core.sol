...
/// @author ArchTaqi
/// @notice Mints NFTs
contract ERC721Core is ERC721Enumerable, AccessControl, Ownable {
    ...
    /// @notice The maximum number of nft minting
    uint256 internal constant _cap = 65;
    /// @notice Counter to track the number minted so far
    uint256 internal numMinted = 0;

    /// ============ Modifiers ============

    /// @notice Requires sender to be contract super operator
    modifier isAdminOperator() {
    ...
    /// @notice Requires sender to be the minter (distributor contract)
    modifier isMinterOperator() {
        // Ensure sender is minter.
    ...
    function upgrade(address _upgradedToAddress) public isAdminOperator {
    ...
    function mint(address _mintTo, uint256 _tokenId)
        public
        isMinterOperator
        returns (bool)
    {
        ...
        require(_tokenId <= _cap, "Cap reached, maximum 5000 mints possible");
