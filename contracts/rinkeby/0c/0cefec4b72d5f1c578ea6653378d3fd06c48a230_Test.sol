...
contract Test is ERC721, Ownable {
  ...
  constructor() ERC721("TEST", "TT") {
    setHiddenMetadataUri("ipfs://QmTAKEWTZLXr2W2L28JokhpZ5XtS7J1mYeipZyDGeZHazG/notRevealed.json");
  }
  ...
  function mint(address _owner, uint256 _mintAmount) public payable mintCompliance(_mintAmount) {
  ...
  function isEpic(address _user) public view returns (bool) {
    for(uint256 i = 0; i < epicsAddresses.length; i++) {
      if (epicsAddresses[i] == _user) {
        return true;
      }
    }
    return false;
