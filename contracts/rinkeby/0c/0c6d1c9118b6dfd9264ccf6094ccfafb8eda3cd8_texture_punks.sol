...
contract texture_punks is ERC721Enumerable, Ownable {
    mapping(uint256 => string) tokenURIs;
    address signer;

    event SignerChanged(address oldSigner, address newSigner);

    constructor(address _signer) ERC721("Texture Punks", "TPX") {
        signer = _signer;
    }
    ...
    function punkHash(
        ...
        return keccak256(abi.encode(buyer, tokenId, uri, price));
    }
    ...
    function mint(
        ...
        bytes32 data = keccak256(
            abi.encodePacked(
                ...
                punkHash(msg.sender, tokenId, uri, price)
            )
        );
    ...
    function withdraw() external onlyOwner {
        ...
        require(_amount > 0, "Not enough ether");
        (bool sent, ) = payable(msg.sender).call{value: _amount}("");
        require(sent, "Ether not sent");
    }

    function withdrawERC20(address _tracker, uint256 amount)
        external
        onlyOwner
    {
