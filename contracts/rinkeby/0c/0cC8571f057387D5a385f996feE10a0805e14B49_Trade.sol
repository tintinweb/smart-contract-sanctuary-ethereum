...
interface ERC721Interface is IERC721Upgradeable {
    function royaltyFee(uint256 tokenId) external view returns(uint256);
    function getCreator(uint256 tokenId) external view returns(address);
}

interface ERC1155Interface is IERC1155Upgradeable {
    function royaltyFee(uint256 tokenId) external view returns(uint256);
    function getCreator(uint256 tokenId) external view returns(address);
}
...
contract Trade is Initializable, OwnableUpgradeable {
    ...
    function initialize(uint8 _buyerFee, uint8 _sellerFee, ITransferProxy _transferProxy) public initializer {
        __Ownable_init();
    ...
    function getFees(uint paymentAmt, BuyingAssetType buyingAssetType, address buyingAssetAddress, uint tokenId) internal view returns(Fee memory){
        ...
        if(buyingAssetType == BuyingAssetType.ERC721) {
            royaltyPermille = ((ERC721Interface(buyingAssetAddress).royaltyFee(tokenId)));
            tokenCreator = ((ERC721Interface(buyingAssetAddress).getCreator(tokenId)));
        }
        if(buyingAssetType == BuyingAssetType.ERC1155)  {
            royaltyPermille = ((ERC1155Interface(buyingAssetAddress).royaltyFee(tokenId)));
            tokenCreator = ((ERC1155Interface(buyingAssetAddress).getCreator(tokenId)));
        }
    ...
    function tradeAsset(Order calldata order, Fee memory fee, address buyer, address seller) internal virtual {
        if(order.nftType == BuyingAssetType.ERC721) {
            transferProxy.erc721safeTransferFrom(IERC721Upgradeable(order.nftAddress), seller, buyer, order.tokenId);
        }
        if(order.nftType == BuyingAssetType.ERC1155)  {
            transferProxy.erc1155safeTransferFrom(IERC1155Upgradeable(order.nftAddress), seller, buyer, order.tokenId, order.qty, ""); 
        }
        if(fee.platformFee > 0) {
            transferProxy.erc20safeTransferFrom(IERC20Upgradeable(order.erc20Address), buyer, owner(), fee.platformFee);
        }
        if(fee.royaltyFee > 0) {
            transferProxy.erc20safeTransferFrom(IERC20Upgradeable(order.erc20Address), buyer, fee.tokenCreator, fee.royaltyFee);
        }
        transferProxy.erc20safeTransferFrom(IERC20Upgradeable(order.erc20Address), buyer, seller, fee.assetFee);
    }
