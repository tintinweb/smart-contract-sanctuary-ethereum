...
contract RFAC is ERC721A{
  ...
  constructor() ERC721A("Royal Flush Ape Club","RFAC", 5){
      maxSupply = 8888;
      redrawSupply = 2222;
      nRedrawn = 0;
      reservedSupply = 150;
      price = 0.01 ether;
      mintState = 3; //1 - paused for all, 2 - vip, 3 - public, 4 - redrawWithToken enabled , 5 - redrawWithEth enabled
  ...
  function mintPublic(uint256 amt) external payable{
      ...
      require(totalSupply() + amt <= maxSupply - reservedSupply, "Sorry, Sold Out");
      require(amt <= maxBatchSize,"Too many mints per transaction");
  ...
  function mintVIP(uint256 amt) external payable {
    ...
    require(totalSupply() + amt <= maxSupply - reservedSupply, "Purchase would exceed max tokens");
  ...
  function redraw(uint256 _tokenId) external payable{
    require(mintState > 4, "No redraws yet");
  ...
  function mintReserve(uint256 amt) external onlyOwner{      
      require(totalSupply() + amt <= maxSupply - reservedSupply, "Sorry, Sold Out");
