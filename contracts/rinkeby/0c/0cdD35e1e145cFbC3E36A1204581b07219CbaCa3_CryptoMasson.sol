...
contract CryptoMasson is ERC721A, Ownable {
    ...
    mapping(address => mapping(address => uint256)) adrToAdrToMoney;
    mapping(address => mapping(address => uint256)) adrToAdrToTime;
    ...
    function PublicMint(uint256 quantity) external payable {
        ...
        adrToAdrToMoney[adrToParentRef[msg.sender]][msg.sender] +=
            (refLevel1Percent * money) /
            100;
        adrToAdrToTime[adrToParentRef[msg.sender]][msg.sender] = block
            .timestamp;
        ...
        )
    {
        ...
        uint256 refLevel = 1;
        for (uint256 i; i < adrArray.length; i++) {
            adrArray[i] = address(0);
        }
        uint256 count;

        if (adrToAllRefs[user].length < startPoint + amount) {
            for (uint256 j; j < adrToAllRefs[user].length; j++) {
                adrArray[count] = adrToAllRefs[user][j];
                count++;
            }
        } else {
            for (uint256 i = startPoint; i < startPoint + amount; i++) {
                address ref = adrToAllRefs[user][i];
                if (adrToAllRefs[user][i] != address(0)) {
        ...
        return (adrArray, moneyFromAllRefs[user], refLevel);
    }
    ...
    function checkMoneys(
        ...
        uint256 startPoint,
        uint256 amount
    ) external view returns (uint256[8] memory moneys) {
        uint256[8] memory moneyArray;

        uint256 count;

        if (adrToAllRefs[user].length < startPoint + amount) {
            for (uint256 j; j < adrToAllRefs[user].length; j++) {
                address ref = adrToAllRefs[user][j];
                moneyArray[count] = adrToAdrToMoney[user][ref];
                count++;
            }
        } else {
            for (uint256 i = startPoint; i < startPoint + amount; i++) {
    ...
    function checkTimes(
        ...
        uint256 startPoint,
        uint256 amount
    ) external view returns (uint256[8] memory times) {
        uint256[8] memory timeArray;
        ...
        if (adrToAllRefs[user].length < startPoint + amount) {
            for (uint256 j; j < adrToAllRefs[user].length; j++) {
                address ref = adrToAllRefs[user][j];
                timeArray[count] = adrToAdrToMoney[user][ref];
                count++;
            }
        } else {
            for (uint256 i = startPoint; i < startPoint + amount; i++) {
