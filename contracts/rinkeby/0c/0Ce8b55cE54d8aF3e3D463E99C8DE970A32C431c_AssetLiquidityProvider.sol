...
contract AssetLiquidityProvider is ERC1543, Votes {
    ...
    function _getVotingUnits(address account) internal view virtual override returns (uint256) {
		...
		for (uint256 i = 0; i < nftAmount; i++) {
			...
			//if (assetPairChecked(ownedIndex)) {
...
contract ERC1543 is SpaceAccessControlled, ERC721Enumerable, IERC1543 {
	...
	function checkAssetPair(uint256 tokenId) external override onlyGovernor {
		AssetPair storage pair = _assetPairs[tokenId];
...
interface ITreasury {
	...
	//function queueTimelock(STATUS status_, address address_, address calculator_) external;
	//function execute(uint256 index) external;
	//function nullify(uint256 index) external;
	...
	function futurePayments(uint256 tokenId, bool actual) external view returns (uint256);
	function fullDelta(uint256 tokenId) external view returns (int256);
	function treasuryUserBacking(uint256 tokenId) external view returns (uint256);
	function treasuryOwnerBacking(uint256 tokenId) external view returns (uint256);
	function treasuryInitialDeposit(uint256 tokenId) external view returns (uint256);
