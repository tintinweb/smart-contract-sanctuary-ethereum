...
contract CryptoSluts is Context, ERC721A, Ownable, ReentrancyGuard, PaymentSplitter {
    using Strings for uint256;

    struct SaleConfig {
        uint256 state;
        uint256 maxSupply; // 10460
    ...
    uint256 public maxGiftSupply = 412; // 412 gift NFTs
    ...
    function mint(uint256 qty) public payable {
        ...
        require(balanceOf(msg.sender) + qty <= saleConfig.maxTokensPerAddress, "Max tokens per address exceeded for this wave");
        require(totalSupply() + qty <= saleConfig.maxSupply, "Max supply exceeded");
        require(msg.value >= saleConfig.price * qty, "Please send the correct amount of ETH");

        _safeMint(msg.sender, qty);
        emit TokenMinted(totalSupply());
    }

    function gift(address _to, uint256 qty) public onlyOwner {
        require(totalSupply() + qty <= maxGiftSupply, "Cannot gift from outside reseve");
        _safeMint(_to, qty);
        emit TokenMinted(totalSupply());
    }
