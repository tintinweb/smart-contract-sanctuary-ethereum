...
contract BinkiesKids is ERC721A ,Ownable {
    
    string  public              BinkiesKidsURI;        
  ...
  constructor(string memory _tokenURI) 
    ERC721A("Binkies Kids", "BINKIESKIDS") {
        BinkiesKidsURI = _tokenURI;
    }
    ...
    function freeMint (uint256 count) external payable {
        ...
        require(addressToMinted[_msgSender()]+count<=FREE_MINTS,"5 free mints per wallet!");    
  ...
  function tokenURI(uint256 _tokenId) public view override returns (string memory) {
        ...
        bytes memory strBytes = bytes(BinkiesKidsURI);
        if( strBytes.length>0)
        return string(abi.encodePacked(BinkiesKidsURI, Strings.toString(_tokenId)));
        else
        return "";
    }
