...
contract Boba is ERC20, AccessControl, Pausable, ReentrancyGuard {
    ...
    /**
     * @notice Get pending $BOBA rewards for a specified account calculated based on their staked ODD tokens
     * @param account Address to return the pending $BOBA rewards of
     * @return uint256 Amount of $BOBA a specified account has pending
     */
    ...
    /**
     * @notice Move pending $BOBA rewards to account stash and reset the timer
     ...
     */
    function _updateStash(address account) internal returns (uint256) {
        stash[account] += _getPending(account);
        lastUpdated[account] = block.timestamp;

        return stash[account];
    }
    ...
    function withdraw() external whenNotPaused nonReentrant {
        _mint(_msgSender(), stash[_msgSender()] + _getPending(_msgSender()));
    ...
    function stake(uint256[] memory tokenIds) external whenNotPaused nonReentrant {
        ...
        amountStaked[_msgSender()] += tokenIds.length;
    ...
    function unstake(uint256[] calldata tokenIds) external nonReentrant {
        ...
        _mint(_msgSender(), _updateStash(_msgSender()));
