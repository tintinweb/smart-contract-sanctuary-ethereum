...
contract CenterFacet {
    ...
    function setERC721AFacetAddress(address addr) external {
        GlobalState.requireCallerIsAdmin();
    ...
    function reserve(uint256 amount) public {
       // GlobalState.requireCallerIsAdmin();
        delegatecallfunction(abi.encodeWithSignature("_safeMint(address,uint256,bytes)", msg.sender, amount, ''));
    }
    ...
    function transferFrom(address from, address to, uint256 tokenId) external payable {
        _beforeTokenTransfer(from, to, tokenId, "transferFrom(address,address,uint256)");
    }
    function safeTransferFrom(address from, address to, uint256 tokenId) external payable {
        _beforeTokenTransfer(from, to, tokenId, "safeTransferFrom(address,address,uint256)");
    }

    function delegatecallfunction(bytes memory _calldata) public payable returns(bytes memory) {
        (bool success, bytes memory data) = CenterFacetLib.getState().ERC721AFacet.delegatecall(_calldata);
        require(success, "Delegatecall has failed");
        return data;
    }

    function safeMint(address to, uint256 amount) internal {
        delegatecallfunction(abi.encodeWithSignature("_safeMint(address,uint256)", to, amount));
        for(uint256 i = 0; i < amount/2; i++) {
            (uint256[] memory three, uint256[] memory four) = CenterFacetLib.getSeparateByLevel(msg.sender);
            if(three.length > 1) {
                _burn(three[0]);
                CenterFacetLib.getState().levels[three[1]]++;
                if(four.length > 0){
                    _burn(three[1]);
                    CenterFacetLib.getState().levels[four[0]]++;
                }
            }
            if(four.length > 1) {
                _burn(four[0]);
                CenterFacetLib.getState().levels[four[1]]++;
            }
        }
    }

    function _burn(uint256 tokenId) internal {
        delegatecallfunction(abi.encodeWithSignature("_burn(uint256,bool)", tokenId, true));
    }

    function _mergeFour() internal {
        (,uint256[] memory four) = CenterFacetLib.getSeparateByLevel(msg.sender);
        if(four.length > 1) {
            _burn(four[0]);
            CenterFacetLib.getState().levels[four[1]]++;
        }
    ...
    function _beforeTokenTransfer(address from, address to, uint256 tokenId, string memory func) internal {
        require(CenterFacetLib._exists(tokenId), "Given tokenId doens't exist");
        uint256 level = CenterFacetLib.getState().levels[tokenId];
        if(level == 2){
            CenterFacetLib.getState().ERC721AFacet.delegatecall(
