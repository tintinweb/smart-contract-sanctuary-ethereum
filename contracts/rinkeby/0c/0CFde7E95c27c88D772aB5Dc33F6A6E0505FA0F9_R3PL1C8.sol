...
/*
____/\\\\\\\\\_________/\\\\\\\\\\___/\\\\\\\\\\\\\____/\\\__________________/\\\________/\\\\\\\\\_____/\\\\\\\\\____        
 __/\\\///////\\\_____/\\\///////\\\_\/\\\/////////\\\_\/\\\______________/\\\\\\\_____/\\\////////____/\\\///////\\\__       
  _\/\\\_____\/\\\____\///______/\\\__\/\\\_______\/\\\_\/\\\_____________\/////\\\___/\\\/____________\/\\\_____\/\\\__      
   _\/\\\\\\\\\\\/____________/\\\//___\/\\\\\\\\\\\\\/__\/\\\_________________\/\\\__/\\\______________\///\\\\\\\\\/___     
    _\/\\\//////\\\___________\////\\\__\/\\\/////////____\/\\\_________________\/\\\_\/\\\_______________/\\\///////\\\__    
     _\/\\\____\//\\\_____________\//\\\_\/\\\_____________\/\\\_________________\/\\\_\//\\\_____________/\\\______\//\\\_   
      _\/\\\_____\//\\\___/\\\______/\\\__\/\\\_____________\/\\\_________________\/\\\__\///\\\__________\//\\\______/\\\__  
       _\/\\\______\//\\\_\///\\\\\\\\\/___\/\\\_____________\/\\\\\\\\\\\\\\\_____\/\\\____\////\\\\\\\\\__\///\\\\\\\\\/___ 
        _\///________\///____\/////////_____\///______________\///////////////______\///________\/////////_____\/////////_____
...
contract R3PL1C8 is ERC721A, EIP2981AllToken, Ownable {
    using Strings for uint256;

    bool public isReplicated;
    uint256 public immutable minReplicates;
    uint256 public immutable maxReplicates;
    uint256 public numOfReplicates;
    uint256 public immutable replicateTime;
    ...
    /// @param _minReplicates is the minimum number of replicates
    /// @param _maxReplicates is the maximum number of replicates
    /// @param _replicateTime is time after which replication can occur
    constructor (string memory _name, string memory _symbol,
        address _royaltyRecipient, uint256 _royaltyPercentage, address _admin,
        uint256 _minReplicates, uint256 _maxReplicates, uint256 _replicateTime)
        ERC721A(_name, _symbol) EIP2981AllToken(_royaltyRecipient, _royaltyPercentage) Ownable() {

            admin = _admin;
            minReplicates = _minReplicates;
            maxReplicates = _maxReplicates;
            replicateTime = _replicateTime;
    }
    ...
    /// @notice function to set the image for the metadata
    /// @dev requires owner or admin
    /// @param _image is a string with the new uri
    function setImage(string calldata _image) external adminOrOwner {
        image = _image;
    }

    /// @notice function for minting the 1/1 to the owner's address
    /// @dev requires owner or admin
    /// @dev requires that the next token to be minted is token 1
    /// @dev using _mint function as owner() should always be an EOA
    function mint() external adminOrOwner {
        require(ERC721A._nextTokenId() == 1, "Already minted the first piece");
        numOfReplicates = 1;
        _mint(Ownable.owner(), 1);
    }

    /// @notice function for owner of token 1 to unlock the piece and turn it into an edition
    /// @dev requires msg.sender to be the owner of token 1
    /// @dev requires a number of editions less than or equal to maxReplicates or greater than or equal to minReplicates
    /// @dev requires isReplicated to be false
    /// @dev requires block timestamp to be greater than or equal to replicateTime
    /// @param _numOfReplicates is the total number of editions to make. Can be set between minReplicates and maxReplicates. This number is the total number of editions that will live on this contract
    function replicate(uint256 _numOfReplicates) external {
        require(msg.sender == ERC721A.ownerOf(1), "Caller is not owner of the 1/1 (token 1)");
        require(_numOfReplicates >= minReplicates && _numOfReplicates <= maxReplicates, "Cannot set number of editions above max or below the min");
        require(!isReplicated, "Already is replicated");
        require(block.timestamp >= replicateTime, "Cannot replicate prior to replicateTime");

        isReplicated = true;
        numOfReplicates = _numOfReplicates;
        if (_numOfReplicates > 1) {
            _mint(msg.sender, _numOfReplicates - 1);
        }
    }
    ...
    function tokenURI(uint256 tokenId) override public view returns(string memory) {
        ...
        string memory attr = string(abi.encodePacked('[', '{"trait_type": "Is Replicated", "value": "No"}', ']'));
        if (isReplicated) {
            if (numOfReplicates > 1) {
                name = string(abi.encodePacked(name, ' #', tokenId.toString(), '/', numOfReplicates.toString()));
                attr = string(abi.encodePacked('[', '{"trait_type": "Edition Number", "value": "', tokenId.toString(),'"}, ' 
                    '{"trait_type": "Is Replicated", "value": "Yes"}]'));
            } else {
                attr = string(abi.encodePacked('[', '{"trait_type": "Is Replicated", "value": "Yes"}', ']'));
            }
        }
        return string(
            abi.encodePacked(
