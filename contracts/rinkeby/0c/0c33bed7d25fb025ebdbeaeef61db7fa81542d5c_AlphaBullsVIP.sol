...
contract AlphaBullsVIP is ERC721AXU {
    using Strings for uint256;

    uint256 public version;
    uint256 public MAX_ARTS;
    string public baseTokenURI;

    event BaseURIChanged(string baseURI);

    function initialize(string memory baseURI) public initializer {
        ERC721AXU.__ERC721AXU_init("AlphaBullsVIP", "MLVIP");
    ...
    function airdrop(address receiver, uint256 amountOfArts)
        ...
        onlyOwner
    {
        require(totalSupply() + amountOfArts <= MAX_ARTS, "exceed");
        _safeMint(receiver, amountOfArts);
    }

    function setNewSupply(uint256 newMax) external onlyOwner {
        require(newMax > 0, "at least 1");
        require(newMax < MAX_ARTS, "cannot increase");
        require(newMax >= totalSupply(), "burning too many");
        MAX_ARTS = newMax;
    }
    ...
    function airdropMultiple(address[] memory receivers) external onlyOwner {
        require(receivers.length >= 1, "!len >= 1");
        require(totalSupply() + receivers.length <= MAX_ARTS, "exceed");
    ...
    function airdropMultipleWithAmounts(
        ...
        require(totalSupply() + total <= MAX_ARTS, "exceed");
        for (uint256 i; i < receivers.length; i++) {
            address receiver = receivers[i];
...
contract ERC721AXU is ERC721AUpgradeable, LockRegistry {
    /*
    ...
    function __ERC721AXU_init(string memory _name, string memory _symbol)
        internal
        onlyInitializing
    {
