...
contract NFTBid is NFTFactoryContract {
  ...
  function Bid(uint256 _saleId, uint256 _amount) external payable {
    if(_tokenMeta[_saleId].currency == 0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE || _tokenMeta[_saleId].currency == address(0)) {
      require(msg.value == _amount);
      LibMeta.checkBid(_tokenMeta[_saleId], msg.value);
    }
    else {
      LibMeta.checkBid(_tokenMeta[_saleId], _amount);
    }
    require(block.timestamp <= _tokenMeta[_saleId].bidEndTime);
    //  require(_timeOfAuction[_saleId] >= block.timestamp,"Auction Over");

    if(_tokenMeta[_saleId].currency != 0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE && _tokenMeta[_saleId].currency != address(0)) {
      IERC20Upgradeable(_tokenMeta[_saleId].currency).transferFrom(msg.sender, address(this), _amount);
    }
    ...
    _tokenMeta[_saleId].price = _amount;
    ...
    nonReentrant
  {
    ...
    if(_tokenMeta[_saleId].currency != address(0) && _tokenMeta[_saleId].currency != 0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE) {
      uint256 sum = Bids[_saleId][_bidOrderID].price;
            ...
            for (uint256 i = 0; i < royalties.length; i++) {
                ...
                sum = sum - amount;
                // address payable receiver = royalties[i].account;
                (bool royalSuccess) = IERC20Upgradeable(_tokenMeta[_saleId].currency).transfer(royalties[i].account, amount);
                require(royalSuccess, "Transfer failed");
            }

            (bool isSuccess) = IERC20Upgradeable(_tokenMeta[_saleId].currency).transfer(msg.sender, sum);
            require(isSuccess, "Transfer failed");
    }
    ...
    nonReentrant
  {
    ...
    if(_tokenMeta[_saleId].currency != address(0) && _tokenMeta[_saleId].currency != 0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE) {
      (bool success) = IERC20Upgradeable(_tokenMeta[_saleId].currency).transfer(msg.sender, Bids[_saleId][_bidId].price);
            if(success) {
