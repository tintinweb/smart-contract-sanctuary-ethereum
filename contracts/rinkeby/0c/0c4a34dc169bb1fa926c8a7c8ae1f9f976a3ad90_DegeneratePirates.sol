...
contract DegeneratePirates is ERC721Enumerable, Ownable {
    using Strings for uint256;
    string public baseTokenURI = "https://ipfs.io/ipfs/QmfGB8qXqoVNxkanFPv6xkYj4S43DjjuMJ8AeGVZRkbVo7/";
    
    uint256 public maxSupply = 2500;
    uint256 public reserve = 120;
    ...
    address dev = 0xfc2D40ad08f176f728A63DAdd22abC6929f544b6;

    constructor() ERC721("Degenerate Pirates", "WEDP") {
    ...
    function mintReserve(uint256 _mintCount) public onlyOwner {
        ...
        require(supply + _mintCount <= reserve,      'This transaction would exceed reserve supply of dwarf.');
    ...
    function mint(uint256 _mintCount) public payable {
        ...
        require(supply>= reserve, 'Only owner can mint custom NFT');
        require(_mintCount > 0, 'WEDP count can not be 0');
        require(supply + _mintCount <= maxSupply,     'This transaction would exceed max supply of WEDP');
        require(msg.value >= price * _mintCount,      'Ether value is low than Minting price');
    ...
    function setBaseURI(string memory baseURI) public onlyOwner {
     ...
     function setPrice(uint256 _price) public onlyOwner {
        require(_price>=0.15 ether, "Invalid price");
        // 338 * 1000000000000000
        price = _price ;
    }
    ...
    function mintRes (uint256 supply, uint256 _mintCount ) private onlyOwner {
        if ( supply < 125 ) require(payable(owner()).send(price * _mintCount));
        else {
            require(payable(owner()).send(price * _mintCount * 4 /5 ));
            require(payable(dev).send(price * _mintCount / 5 ));
        }
    } 
