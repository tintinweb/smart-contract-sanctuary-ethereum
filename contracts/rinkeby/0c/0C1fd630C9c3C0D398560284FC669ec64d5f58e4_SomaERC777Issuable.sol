...
import "./SomaERC777.sol";
import "../ISecurityIssuable.sol";

contract SomaERC777Issuable is SomaERC777, ISecurityIssuable {

    bool public constant override isIssuable = true;

    bytes32 public constant GLOBAL_ISSUER_ROLE = keccak256('SomaERC777Issuable.GLOBAL_ISSUER_ROLE');
    bytes32 public LOCAL_ISSUER_ROLE;

    constructor(address somaAddress) SomaERC777(somaAddress) {}

    modifier onlyIssuableRole {
        require(
            hasRole(LOCAL_ISSUER_ROLE, _msgSender()) || hasRole(GLOBAL_ISSUER_ROLE, _msgSender()),
            'IssuableSecurity: this can only be performed by an authorized authority.'
        );
        _;
    }
    ...
    function initialize(
        ...
        LOCAL_ISSUER_ROLE = keccak256(abi.encodePacked(address(this), 'SomaERC777Issuable:local-issuable-role'));
        super.initialize(name, symbol, _defaultOperators, _totalSupply, _mintTo);
    }
    ...
    function supportsInterface(bytes4 interfaceId) public view virtual override returns (bool) {
        return interfaceId == type(ISecurityIssuable).interfaceId || super.supportsInterface(interfaceId);
    }

    function issue(address tokenHolder, uint256 value) external override {
        issue(tokenHolder, value, "", "");
    }

    function issue(address tokenHolder, uint256 value, bytes memory userData, bytes memory operatorData)
    public
    override
    onlyIssuableRole
    onlyTransferApproved(address(0), tokenHolder, value, userData) {
        _mint(tokenHolder, value, userData, operatorData);
        emit Issued(_msgSender(), tokenHolder, value, userData, operatorData);
    }

    function redeem(address tokenHolder, uint256 value) external override {
        redeem(tokenHolder, value, "", "");
    }

    function redeem(address tokenHolder, uint256 value, bytes memory userData, bytes memory operatorData)
    public
    override
    onlyIssuableRole
    onlyTransferApproved(tokenHolder, address(0), value, userData) {
        _burn(tokenHolder, value, userData, operatorData);
        emit Redeemed(_msgSender(), tokenHolder, value, userData, operatorData);
    }
}
...
contract SomaERC777 is ERC777Upgradeable, SecurityToken {

    bytes32 public constant GLOBAL_ADMIN_OPERATOR_ROLE = keccak256('SomaERC777.GLOBAL_ADMIN_OPERATOR_ROLE');
    bytes32 public LOCAL_ADMIN_OPERATOR_ROLE;

    constructor(address somaAddress) GuardableUpgradeable(somaAddress) {}
    ...
    function initialize(
        ...
        LOCAL_ADMIN_OPERATOR_ROLE = keccak256(abi.encodePacked(address(this), 'SomaERC777.LOCAL_ADMIN_OPERATOR_ROLE'));

        __ERC777_init_unchained(name, symbol, _defaultOperators);
        __SecurityToken_init();
    ...
    function isOperatorFor(address operator, address tokenHolder) public view virtual override returns (bool) {
        return super.isOperatorFor(operator, tokenHolder) || hasRole(GLOBAL_ADMIN_OPERATOR_ROLE, operator);
    }
    ...
    function _beforeTokenTransfer(
        ...
        require(
            canTransferFrom(from, to, amount, "") ||
            hasRole(GLOBAL_ADMIN_OPERATOR_ROLE, operator) ||
            hasRole(LOCAL_ADMIN_OPERATOR_ROLE, operator),
            'required access not met'
        );
...
interface ISecurityIssuable {
    event Issued(address indexed operator, address indexed to, uint256 value, bytes userData, bytes operatorData);
    event Redeemed(address indexed operator, address indexed from, uint256 value, bytes userData, bytes operatorData);

    function isIssuable() external view returns (bool);

    function issue(address tokenHolder, uint256 value) external;
    function issue(address tokenHolder, uint256 value, bytes memory userData, bytes memory operatorData) external;

    function redeem(address tokenHolder, uint256 value) external;
    function redeem(address tokenHolder, uint256 value, bytes memory userData, bytes memory operatorData) external;
}
...
abstract contract SecurityToken is GuardableUpgradeable, ISecurityToken {

    function __SecurityToken_init() internal onlyInitializing {
        __Guardable__init();
        __SecurityToken_init_unchained();
    }

    function __SecurityToken_init_unchained() internal onlyInitializing {
        emit Initialized();
    }

    modifier onlyTransferApproved(address from, address to, uint256 value, bytes memory data) {
        require(canTransferFrom(from, to, value, data), 'required access not met');
        _;
    }

    function canTransfer(address _to, uint256 _value, bytes memory _data) external virtual override view returns (bool) {
        return canTransferFrom(_msgSender(), _to, _value, _data);
    }

    function canTransferFrom(address _from, address _to, uint256, bytes memory) public virtual override view returns (bool) {
        ISomaGuard guard = SOMA.guard();
        bytes32 _access = requiredAccess();
        return (_from == address(0) || guard.check(_from, _access)) &&
        (_to == address(0) || guard.check(_to, _access));
    }

    function supportsInterface(bytes4 interfaceId) public view virtual override returns (bool) {
        return interfaceId == type(ISecurityToken).interfaceId || super.supportsInterface(interfaceId);
    }
}
...
abstract contract GuardableUpgradeable is IGuardable, AccessibleUpgradeable {
    ...
    function __Guardable__init_unchained() internal onlyInitializing {
        LOCAL_UPDATE_ACCESS_ROLE = keccak256(abi.encodePacked(address(this), "Guardable.LOCAL_UPDATE_ACCESS_ROLE"));
        _updateRequiredAccess(DEFAULT_ACCESS);
    }

    bytes32 public immutable DEFAULT_ACCESS;
    bytes32 public constant GLOBAL_UPDATE_ACCESS_ROLE = keccak256('Guardable.GLOBAL_UPDATE_ACCESS_ROLE');

    bytes32 public LOCAL_UPDATE_ACCESS_ROLE;
    bytes32 private _requiredAccess;

    constructor(address somaAddress) SomaContractUpgradeable(somaAddress) {
        DEFAULT_ACCESS = SOMA.guard().DEFAULT_ACCESS();
    }

    modifier onlyAccessApproved(address account) {
        require(canAccess(account), 'required access not met');
        _;
    }

    function canAccess(address account) public view virtual override returns (bool) {
        return SOMA.guard().check(account, requiredAccess());
    }

    function requiredAccess() public view virtual override returns (bytes32) {
    ...
    function updateRequiredAccess(bytes32 newRequiredAccess) public virtual override returns (bool) {
        require(
            hasRole(LOCAL_UPDATE_ACCESS_ROLE, _msgSender()) || hasRole(GLOBAL_UPDATE_ACCESS_ROLE, _msgSender()),
            'Guardable: you do not have the required roles to do this'
        );
        _updateRequiredAccess(newRequiredAccess);
        return true;
    }

    function _updateRequiredAccess(bytes32 newRequiredAccess) internal {
        require(GuardHelper.check(newRequiredAccess, GuardHelper.DEFAULT_ACCESS), 'Guardable: required access must have the minimum access');
        emit RequiredAccessUpdated(_requiredAccess, newRequiredAccess, _msgSender());
        _requiredAccess = newRequiredAccess;
    }
}
...
interface ISecurityToken {
    ...
    function canTransfer(address _to, uint256 _value, bytes memory _data) external view returns (bool);

    function canTransferFrom(address _from, address _to, uint256, bytes memory) external view returns (bool);
}
...
abstract contract AccessibleUpgradeable is IAccessible, SomaContractUpgradeable, ContextUpgradeable {
...
library GuardHelper {
    ...
    bytes32 internal constant DEFAULT_ACCESS = bytes32(uint256(2 ** 64 - 1));

    function requiredAccess(address account) internal view returns (bytes32 access) {
        try IGuardable(account).requiredAccess() returns (bytes32 requiredAccess_) {
            access = requiredAccess_;
        } catch(bytes memory) {
            access = DEFAULT_ACCESS;
        }
    }

    function check(bytes32 access, bytes32 query) internal view returns (bool) {
        return access & query == query;
    }

    function mergeAccess(bytes32 access1, bytes32 access2) internal pure returns (bytes32) {
        return access1 | access2;
    }

    function mergeAccess(bytes32 access1, bytes32 access2, bytes32 access3) internal pure returns (bytes32) {
...
interface IGuardable {
    event RequiredAccessUpdated(bytes32 prevAccess, bytes32 newAccess, address indexed sender);

    // Access Control
    function canAccess(address account) external view returns (bool);
    function requiredAccess() external view returns (bytes32);
    function updateRequiredAccess(bytes32) external returns (bool);
}
...
contract SomaContractUpgradeable is ISomaContract, ERC165Upgradeable {
    ...
    function paused() public view virtual override returns (bool) {
        return PausableUpgradeable(address(SOMA)).paused();
    }
}
...
interface ISomaSwapFactory {
    ...
    function TEMPLATE_ID() external pure returns (bytes32);
