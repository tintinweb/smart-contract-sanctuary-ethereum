...
contract LGSCactusInternal is ERC721A, Ownable, ReentrancyGuard, RoyaltiesV2Impl {
  ...
  function mintWithId(address _receiver, uint256 tokenId) public virtual {
  ...
  function sendAirdrop(address[] memory _recipients, uint256 tokenPosition) external {
    require(msg.sender == admin, 'only admin');
    require(tokenPosition <= maxSupply, 'token not found');

    transferFrom(address(this), _recipients[0], tokenPosition);
  }
