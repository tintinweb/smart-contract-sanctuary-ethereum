...
contract LoanComic is ERC721, SlicerPurchasable {
    ...
    constructor(
        ...
        // _safeMint(0xf39Fd6e51aad88F6F4ce6aB8827279cffFb92266, 1); // testing address
    }
    ...
    function _beforeTokenTransfer(
        ...
        if (msg.sender != _productsModuleAddress) {
            ...
            // If sender is lender, check the loan has expired and the receiver is the lender
            else if (msg.sender == _loans[tokenId].lender) {
                if (block.timestamp < _loans[tokenId].expirationTimestamp) revert ActiveLoan();
                if (to != _loans[tokenId].lender) revert Invalid();
            }
            // If sender is not lender, revert if token is loaned
        ...
        returns (bool)
    {
        ...
        return (spender == owner ||
            ...
            // Allow loans to be claimed by lender
            msg.sender == _loans[tokenId].lender ||
