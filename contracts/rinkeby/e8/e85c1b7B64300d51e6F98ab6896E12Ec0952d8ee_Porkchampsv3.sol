 ...
 */
contract Porkchampsv3 is ERC721A, Ownable {
    string private _baseTokenURI;
    address private adminSigner = address(1);

    uint256 public collectionSize = 10000; // 10000
    ...
    constructor() payable ERC721A("Porkchampsv3", "PKC") {}
    ...
    // For marketing etc.
    // 200 less team avatars and mascot, +1 since using less than operator
    function teamMint(uint256 quantity) external onlyOwner {
        require(
            totalSupply() + quantity < 210,
    ...
    function allowlistMint(
        ...
        require( saleConfig.allowlistMintEnabled == true, "Allowlist event is not active" );
        require( numberMinted(msg.sender) + quantity <= allowance, "Reached max allowance" );
        require( totalSupply() + quantity <= collectionSize, "Reached max supply" );
        require( msg.value == prices.allowlistPriceWei * quantity, "Need to send the correct amount of eth" );
    ...
    function publicMint(uint256 quantity) external payable callerIsUser {
        ...
        require( quantity < 9, "Can only mint a max of 8 at a time" );
        _safeMint(msg.sender, quantity);
    ...
    function setCollectionSize(uint256 collectionSize_) external onlyOwner {
        require(
            collectionSize_ < collectionSize + 1,
            "Can only decrease collectionSize"
        );
        collectionSize = collectionSize_;
    }

    // To show on-chain the order of the images prior to minting
    ...
    // To show the starting index is out of our control
    // Even if set manually, the startIndexBlock is hashed so it's not sequential
    // The immediate token after the 8 team avatars will become the startingIndex
    ...
    function withdrawMoney() external onlyOwner {
        address vault = payable(msg.sender);
