...
contract PXRT is Singleton, ERC20Upgradeable, ProxyUtility {
    ...
    modifier onlyWhiteListedAddress(address receiver) {
        require(
            receiver ==
                ...
                receiver ==
                modusRegistry.getProjectDetails(projectId)._pxrtStableCoinSwap,
            "PXRT: Unauthorized 'To'address "
        );
        _;
    }
    ...
    function initialize(
        ...
        string memory _name = string(
            abi.encodePacked("P(", Strings.toString(projectId), ")RT")
        );
        ...
        if (operationalWalletFee > 0) {
            _mint(operationalWallet, operationalWalletFee);
        }
        _mint(
            modusRegistry.getProjectDetails(projectId)._pxtStaking,
            totalMint * 10**uint256(decimals())
        );
    }
