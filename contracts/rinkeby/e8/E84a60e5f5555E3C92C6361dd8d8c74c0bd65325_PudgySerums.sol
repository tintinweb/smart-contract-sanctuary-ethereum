...
contract PudgySerums is ERC1155, Pausable, Ownable, ReentrancyGuard {
    using ECDSA for bytes32;

    //Start timestamp
    uint public startTime = 1642633200;
    ...
    //Metadata
    string public name = "Pudgy Serums";
    ...
    //CLAIM
    function claimForSnack(uint[] calldata _ids, bytes[] memory signature, uint[] calldata serumType) external whenNotPaused nonReentrant {
        ...
        IERC20(SNAC).transferFrom(msg.sender,address(this), snackPrice*_ids.length);
        for(uint i = 0; i < _ids.length; i++) {
            require(isCorrectId(_ids[i], signature[i], serumType[i]), "PudgySerums: Invalid signature!");
            ...
            _mint(msg.sender, serumType[i], 1, "");
        }
    }

    function claimForETH(uint[] calldata _ids, bytes[] memory signature, uint[] calldata serumType) external payable whenNotPaused nonReentrant {
        require(isClaimingOpen(), "PudgySerums: Claiming is not open yet!");
    ...
    function isCorrectId(uint id, bytes memory signature, uint serumType) public view returns (bool) {
        bytes32 hash = keccak256(abi.encodePacked(id, serumType));
