...
contract Badge is ERC1155, BaseContract {
 ...
 mapping(uint256 => address) public ownerOfID;
    ...
    mapping(address => mapping(uint256 => uint256)) public tokenClaimedTime;
    ...
    function createBadge(
        ...
        if (flagFee == true) {
            uint256 priceCharge = numberOfCopies * _priceAmount + badgeFee;
            require(
 ...
 require(
                ...
                "This badge already applied one time"
            );
             require(
            badgeExpiry[badgeIds] > block.timestamp,
            "Error: badge expired"
        );
        mapBadgeUser[badgeIds].push(msg.sender);
        mapUserBadge[msg.sender].enumAdd(badgeIds);
         require(
           copiesLeft[badgeIds]>0,
            "no quantity available for apply all claimed"
        );
        ...
        onlySponsor
    {
        ...
        for (uint256 i = 0; i < badgeIds.length; i++) {
            ...
            require(
                balanceOf(msg.sender, badgeIds[i]) > 0,
                "not badge to transfer"
            );
            require(copiesLeft[badgeIds[i]] > 0, "No copy is left to assign");
            copiesLeft[badgeIds[i]] -= 1;
            amounts[i] = 1;
            tokenClaimedTime[to][badgeIds[i]] = coolingTime[badgeIds[i]] + block.timestamp;
            isBadgeAssigned[badgeIds[i]] = 1;
        ...
        onlySponsor
    {
        ...
        gets.transferPrice(msg.sender, owner(), tokenPrice);
    }
function _tokenClaimedTime(address owner , uint badgeId) external view returns(uint){
    return  tokenClaimedTime[owner][badgeId];
}
