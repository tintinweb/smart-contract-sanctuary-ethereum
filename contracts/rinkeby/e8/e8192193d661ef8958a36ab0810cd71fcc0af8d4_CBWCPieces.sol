...
contract CBWCPieces is CBWCBase {
    ...
    function claim(uint256[] calldata cbwcTokenIds_) external {
        ...
        _mint(msg.sender, unclaimedPieces);
    }
...
import "erc721a-upgradeable/contracts/extensions/ERC721AQueryableUUPSUpgradeable.sol";
    ...
    ERC2981Upgradeable
{
    ...
    function supportsInterface(bytes4 interfaceId)
        ...
        returns (bool)
    {
        bytes4 interfaceERC721 = 0x80ac58cd;
        bytes4 interfaceERC721Metadata = 0x5b5e139f;
        if(interfaceId == interfaceERC721 || interfaceId == interfaceERC721Metadata) {
            return true;
        }
    ...
    function tokenURI(uint256 tokenId_)
        ...
        returns (string memory)
    {
        require(tokenId_ < _totalMinted(), "CBWC: token does not exist");
        return
            string(
...
abstract contract ERC721AUUPSUpgradeable is IERC721AUUPSUpgradeable, ERC721AUpgradeable {
    ...
    /**
     * @dev Performs a Solidity delegate function call using a low level `call`. A
    ...
    function functionDelegateCall(
        ...
        require(address(target).code.length != 0, 'Address: delegate call to non-contract');
    ...
    function _setImplementation(address newImplementation) private {
        require(address(newImplementation).code.length != 0, 'ERC1967: new implementation is not a contract');
...
contract ERC721AUpgradeable is ERC721A__Initializable, IERC721AUpgradeable {
    ...
    /**
     * @dev Equivalent to `_burn(address, tokenId, false)`.
     */
    function _burn(uint256 tokenId, bool approvalCheck) internal virtual {
        _burn(_msgSenderERC721A(), tokenId, approvalCheck);
    }
     ...
     * Emits a {Transfer} event.
     */
    function _burn(address from, uint256 tokenId, bool approvalCheck) internal virtual {
        uint256 prevOwnershipPacked = _packedOwnershipOf(tokenId);
     ...
     * Emits a {Transfer} event.
     */
    function _burnBatch(address from, uint256[] memory tokenIds) internal virtual {
        uint256 i;
        for (i = 0; i < tokenIds.length; i++) {
            uint256 prevOwnershipPacked = _packedOwnershipOf(tokenIds[i]);

            if(address(uint160(prevOwnershipPacked)) != from) revert TransferFromIncorrectOwner();

            (uint256 approvedAddressSlot, address approvedAddress) = _getApprovedAddress(tokenIds[i]);
            ...
            unchecked {
                ...
                // - `burned` to `true`.
                // - `nextInitialized` to `true`.
                ERC721AStorage.layout()._packedOwnerships[tokenIds[i]] = _packOwnershipData(
                    from,
                    (BITMASK_BURNED | BITMASK_NEXT_INITIALIZED) | _nextExtraData(from, address(0), prevOwnershipPacked)
                );
                ...
                // If the next slot may not have been initialized (i.e. `nextInitialized == false`) .
                if (prevOwnershipPacked & BITMASK_NEXT_INITIALIZED == 0) {
                    uint256 nextTokenId = tokenIds[i] + 1;
                    // If the next slot's address is zero and not burned (i.e. packed value is zero).
                    if (ERC721AStorage.layout()._packedOwnerships[nextTokenId] == 0) {
                        // If the next slot is within bounds.
                        if (nextTokenId != ERC721AStorage.layout()._currentIndex) {
        ...
        unchecked {
            ...
            // We can directly decrement the balance, and increment the number burned.
            // This is equivalent to `packed -= 1; packed += 1 << BITPOS_NUMBER_BURNED;`.
            ERC721AStorage.layout()._packedAddressData[from] += (i << BITPOS_NUMBER_BURNED) - i;
            ERC721AStorage.layout()._burnCounter += i;
        }
    }
