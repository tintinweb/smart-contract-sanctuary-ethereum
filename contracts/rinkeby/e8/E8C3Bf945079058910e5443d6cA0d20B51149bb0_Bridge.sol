...
contract Bridge is Ownable {
    ...
    event Swap(address from, address to, uint256 amount, uint256 id);
    event Redeem(address to, uint256 amount, uint256 id);
    ...
    function swap(address receivingAddress, uint256 amount) external {
        token.burn(msg.sender, amount);
        if (receivingAddress == address(0)) revert CannotBeZero();

        emit Swap(msg.sender, receivingAddress, amount, id);
        id++;
    }
    ...
    function redeem(
        ...
        if (redeemed[id_] == true) revert AlreadyRedeem();

        bytes32 messageHash = keccak256(
            abi.encodePacked(receivingAddress, amount, id)
        ...
        emit Redeem(receivingAddress, amount, id);
    }
}
