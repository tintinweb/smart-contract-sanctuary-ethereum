...
contract Egg is ERC1155Supply, Ownable, ReentrancyGuard {
    ...
    function mint(uint256 _mintAmount,uint256 _id) external payable nonReentrant {
        uint256 s = totalSupply(_id);
        require(status, "Off");
    ...
    function mintPresale(uint256 _mintAmount, uint256 _id) external payable {
    ...
    // admin minting
    function gift(uint256[] calldata quantity, uint256[] calldata id, address[] calldata recipient)
        external
        onlyOwner
    {
        require(
        ...
        for (uint256 i = 0; i < recipient.length; ++i) {
            require(totalSupply(id[i]) + totalQuantity <= maxSupply, "Too many");

                _mint(recipient[i], id[i], quantity[i], "");
    ...
    {
        require(exists(tokenId), "ERC721Metadata: Nonexistent token");
