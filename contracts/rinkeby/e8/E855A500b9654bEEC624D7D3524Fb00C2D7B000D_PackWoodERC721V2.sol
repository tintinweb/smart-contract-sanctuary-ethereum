...
interface IERC721 is IERC165Upgradeable {
   ...
   function breedingInfo(uint256 tokenId) external view returns(breedInfomation1 memory);
...
contract PackWoodERC721V2 is ERC721URIStorageUpgradeable, OwnableUpgradeable, PackWoodERC721StorageV2{
    ...
    function initialize(address _parentAddress, address _monsterParent) public initializer {
        __ERC721_init("PackWood", "PACKWOOD");
        __Ownable_init(msg.sender);
        tokenCounter = 1; 
        _before = "https://assets.monsterbuds.io/Packwood-monster-uri";
       _after = "-token-uri.json";
       feeMargin = 10;
       processPrice = 10000000000000000;
       ERC1155Parent = IPackWoodERC1155(_parentAddress);
       MonsterParent = IERC721(_monsterParent);

       feeSKTWallet = payable(owner()); // SKT fee wallet
       SmartContractCommunity = payable(owner()); // community wallet
    }

    modifier onlyGenOne(uint256 tokenId) {
        require((11 <= tokenId && tokenId <= 100) || checkTokenId[tokenId], "$MONSTERBUDS: Only generation 1 monsterbuds can process");
        _;
    }
    ...
    function chechTokenGenOne(uint256 tokenId) public view returns(bool){
        if((11 <= tokenId && tokenId <= 100) || checkTokenId[tokenId]){
            return true;
        }
        return false;
    }
    ...
    */
    ...
    function process(uint256 _monsterTokenId, uint256 _packwoodERC1155TokenId) external virtual onlyAllowed onlyGenOne(_monsterTokenId) returns(uint256){
        ...
        require(!isProcessed[_monsterTokenId], "PackwoodERC721: Token can not be processed");
        ...
        tokenCounter += 1;
        isProcessed[_monsterTokenId] = true;
