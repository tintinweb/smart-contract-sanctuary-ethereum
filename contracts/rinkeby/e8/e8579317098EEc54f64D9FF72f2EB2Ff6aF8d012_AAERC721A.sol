...
contract AAERC721A is Ownable, ERC721A, ReentrancyGuard {
    ...
    function allowlistMint(uint256 quantity, bytes32[] calldata _proof) external payable callerIsUser {
        require(isWLMintActive, "allowlist sale has not begun yet");
    ...
    function canWLMintStart()
        ...
        returns (bool)
    {
        return !isPublicMintActive && publicMintStartTime != 0 && _wlRoot != ""  && price != 0 && (totalSupply() >= amountForDevs + amountForFree + amountForSale || block.timestamp - publicMintStartTime >= 5*60); // ensure that the whitelist mint starts
    }                                                                             
