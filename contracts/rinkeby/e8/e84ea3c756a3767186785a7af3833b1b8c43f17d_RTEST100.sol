...
contract RTEST100 is ERC721, Ownable {using Strings for uint256; using Counters for Counters.Counter; Counters.Counter private supply;
  ...
  constructor() ERC721("RTEST100", "RT100") {setHiddenMetadataUri("ipfs://QmWrX4cCtnQZPbDDVH1WMibnLYQNyW68WrEVHeAPF7u8VP/hidden.json");}
  ...
  modifier freeMintsAvailable() {require(totalSupply() <= maxFree, "Not enough free mints remain");_;}
  modifier mintFreeCompliance(uint256 _mintAmount) {require(msg.value >= freecost * _mintAmount, 'Insufficient funds!'); _;}
  function totalSupply() public view returns (uint256) {return supply.current();}

  
  function mint(uint256 _mintAmount) external payable mintCompliance(_mintAmount) mintPriceCompliance(_mintAmount) freeMintsAvailable() mintFreeCompliance (_mintAmount){
