...
contract ERC721AFacet is ERC721A {
        ...
        whenNotPaused
    {
        ...
        s.airdrop ? airdropCheck() : priceCheck();
    ...
    //                    Check functions
    // =============================================================
    function quantityCheck(address to, uint256 quantity) internal view {
        unchecked {
            require(
                (s.currentIndex + quantity) <= s.maxSupply,
                "Purchase would exceed max supply of tokens"
            );
            require(
                _numberMinted(to) + quantity <= s.maxMintPerAddress,
                "Exceeds max amount of mints per address"
            );
        }
        require(
            quantity <= s.maxMintPerTx,
            "Exceeds max amount of mints per transaction"
        );
    }

    function airdropCheck() internal view {
        LibDiamond.DiamondStorage storage ds = LibDiamond.diamondStorage();
        bool isContractOwner = _msgSender() == ds.contractOwner;

        require(
            isContractOwner,
            "Contract is an aidrop and can only be minted from approved accounts"
        );
    }
    ...
    function priceCheck() internal {
        ...
        console.log("msg.value >= price(): ", msg.value >= price());
    ...
    function burn(uint256 tokenId) public onlyOwner {
        ...
        console.log("Results Length (For Testing, Expect 0)");
...
import {Modifiers} from "../libraries/Modifiers.sol";
import {LibDiamond} from "../libraries/LibDiamond.sol";
import {AppStorage, TokenApprovalRef} from "../libraries/LibAppStorage.sol";
 ...
 * - The maximum token ID cannot exceed 2**256 - 1 (max value of uint256).
 */
abstract contract ERC721A is IERC721A, Pausable {
    ...
    /*
     * @dev This has been moved to the diamond init file
     */
    // constructor(string memory name_, string memory symbol_) {
    //     _name = name_;
    //     _symbol = symbol_;
    //     s.currentIndex = _startTokenId();
    // }
    ...
    function totalSupply() public view virtual override returns (uint256) {
        ...
        unchecked {
            return s.currentIndex - s.burnCounter - _startTokenId();
        }
    }
    ...
    function _totalMinted() internal view virtual returns (uint256) {
        ...
        unchecked {
            return s.currentIndex - _startTokenId();
        }
    }
        ...
        returns (uint256)
    {
        ...
        return s.packedAddressData[owner] & _BITMASK_ADDRESS_DATA_ENTRY;
    }
    ...
    function _numberMinted(address owner) internal view returns (uint256) {
        return
            (s.packedAddressData[owner] >> _BITPOS_NUMBER_MINTED) &
     ...
     *
     * Removed for diamond compliance
     */
    ...
    function _initializeOwnershipAt(uint256 index) internal virtual {
        if (s.packedOwnerships[index] == 0) {
            s.packedOwnerships[index] = _packedOwnershipOf(index);
        }
    }
    ...
    function _exists(uint256 tokenId) internal view virtual returns (bool) {
        return
            ...
            s.packedOwnerships[tokenId] & _BITMASK_BURNED == 0; // and not burned.
    }
    ...
    function transferFrom(
        ...
        unchecked {
            ...
            s.packedOwnerships[tokenId] = _packOwnershipData(
                to,
...
library PriceConsumer {
    ...
    function getLatestPrice() internal view returns (uint256) {
        ...
        // Matic: 87500000
        // Eth: 157487124093
        // price = 150000000000; // $1500 for testing
        uint256 newPrice = uint256(price);
        console.log("Latest Price: %s", newPrice);
...
struct AppStorage {
     ...
     * @dev Access Control
     */
    mapping(bytes32 => AccessControl.RoleData) roles;
