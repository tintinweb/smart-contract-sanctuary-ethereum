...
// File contracts/GolkyPunksDNA.sol



pragma solidity ^0.8.0;

contract GolkyPunksDNA {
...
contract GolkyPunks is ERC721, ERC721Enumerable, GolkyPunksDNA {
    ...
    constructor(uint256 _maxSupply) ERC721("Golky Punks", "GOLKY_PKS") {
        maxSupply = _maxSupply;
    }

    function mint() public {
        uint256 current = _idCounter.current();
        require(current < maxSupply, "No GolkyPunks left :(");
        ...
        returns (string memory)
    {
        ...
        string memory jsonURI = Base64.encode(
            abi.encodePacked(
                '{ "name": "GolkyPunks #',
                tokenId.toString(),
                '", "description": "Golky Punks are chain-stored random Avataaars for all Golky amateur football players", "image": "',
                image,
                '"}'
            )
        );
