...
contract RootzNFT is ERC721A, Ownable{
    using Strings for uint256;

    uint256 public constant MINT_PER_WALLET = 10;
    ...
    constructor() ERC721A("Rootz NFT", "ROOTZ"){
    ...
    function checkCurrentMintPrice(uint256 _quantity) public view returns (uint256) {
        uint8[5] memory tiers = [1, 2, 3, 4, 5];
        uint256 _price = 0;
        for (uint256 i = 1; i <= _quantity; i++) {
            ...
            if (currentToken > tiers[2] && currentToken <= tiers[3]){
                _price += PRICE * 3;
            }
            if (currentToken > tiers[3] && currentToken <= tiers[4]){
                _price += PRICE * 4;
            }
        }
        return _price;
    ...
    function mint(uint256 _quantity) external payable callerIsUser{
        ...
        require((totalMint[msg.sender] +_quantity) <= MAX_MINT, "Purchase exceeds max allowed per address");
