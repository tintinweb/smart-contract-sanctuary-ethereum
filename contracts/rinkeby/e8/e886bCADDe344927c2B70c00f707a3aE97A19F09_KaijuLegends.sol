...
contract KaijuLegends is Ownable, ERC721A, ReentrancyGuard, Pausable {
    ...
    event UpdateCryptoDotComMaxSupply(uint256 supply);
        ...
        onlyTreasuryOrOwner
    {
        ...
        emit UpdatePartnerMaxSupply(partnerMaxSupply_);
    }
    ...
    /*
     * @notice: Set the new max supply for CDC
     * @param: cryptoDotComMaxSupply_: The max supply for CDC
     */
    function setCryptoDotComMaxSupply(uint256 cryptoDotComMaxSupply_)
        external
        onlyTreasuryOrOwner
    {
        cryptoDotComMaxSupply = cryptoDotComMaxSupply_;
        emit UpdateCryptoDotComMaxSupply(cryptoDotComMaxSupply_);
    }
        ...
        if (!isPartner) {
            ...
            require(
                (totalSupply() + quantity_) <=
                    (COLLECTION_SUPPLY -
                        (partnerTokensLeft + cryptoDotComMaxSupply)),
    ...
    function batchMint(
        ...
        uint256 partnerTokensLeft = partnerMaxSupply - partnerSupply;
        require(
            (totalSupply() + quantity_) <= (COLLECTION_SUPPLY - (partnerTokensLeft + cryptoDotComMaxSupply)),
            "KaijuLegends: Reached max supply"
        );
