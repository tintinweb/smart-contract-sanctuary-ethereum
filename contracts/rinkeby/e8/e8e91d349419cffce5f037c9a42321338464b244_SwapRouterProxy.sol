...
contract SwapRouterProxy {
    ...
    function toBytesFromUint(uint24 a) public pure returns (bytes memory) {
        return abi.encodePacked(a);
    }
    ...
    function _getSwapPath(
        ...
        uint24[] memory poolFees,
        bool useReverse
    ) public pure returns(bytes memory swapPath) {
    ...
    function swapExactInputMultihop(address[] memory paths, uint24[] memory poolFees, uint256 amountIn, uint amountOutMinimum) external returns (uint amountOut) {
    ...
    function swapExactOutputMultihop(address[] memory paths, uint24[] memory poolFees, uint256 amountOut, uint256 amountInMaximum) external returns (uint256 amountIn) {
