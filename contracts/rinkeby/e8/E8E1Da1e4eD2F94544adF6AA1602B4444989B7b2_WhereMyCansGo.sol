...
contract WhereMyCansGo is ERC721Creator {

    constructor() ERC721Creator("Where My Cans Go", "WMCG") {}

    // * CONSTANTS * //
    address public constant openseaSharedContract = 0x88B48F654c30e99bc2e4A1559b4Dcf1aD93FA656; // Change to mainnet before deployment
    uint8 public constant collectionSize = 125;

    // * STORAGE * //
    address public purgatory = address(0); // Address that temporarily holds tokens before sent to dead address
    bytes32 public merkleRoot; // Merkle tree to validate old token IDs on OpenSea shared contract are part of WMCG collection
    mapping(uint256 => address) public oldTokenIdToAddress; // Tracking which wallets burn the old tokens
    mapping(uint8 => bool) public newTokenMinted; // Tracking which tokens are already minted

    function setPurgatory(address _purgatory) adminRequired public { // Sets purgatory address for temporary holding of tokens before sending to dead address
        purgatory = _purgatory;
    }

    function setMerkleRoot(bytes32 _merkleRoot) adminRequired public { // Sets Merkle Root for verifying old tokens are part of WMVG collection
        merkleRoot = _merkleRoot;
    }

    // * MAIN FUNCTIONS * // 
    function sendToPurgatory(
        ...
        require(tokenContract == openseaSharedContract, "Contract must be the OpenSea Shared Storefront Contract");
        
        ERC1155Partial baseContract = ERC1155Partial(tokenContract);
        require(baseContract.isApprovedForAll(msg.sender, address(this)), "Contract not yet approved for all transfers");
        
        bytes32 leaf = keccak256(abi.encodePacked(oldTokenId, newTokenId));
        require(verifyMerkleProof(merkleProof, merkleRoot, leaf),"Invalid proof"); // Ensures old and new tokenIds are valid

        baseContract.safeTransferFrom(msg.sender, purgatory, oldTokenId, 1, ""); // Transfer token to Purgatory
        oldTokenIdToAddress[oldTokenId] = msg.sender; // Record burner address of the old token
        resurrect(msg.sender, oldTokenId, newTokenId, uri); // Mint new token
    }
    ...
    function resurrect(
        ...
        require(newTokenId > 0 && newTokenId < 118, "Resurrected token ID must be between 1 and 117");
        require(oldTokenIdToAddress[oldTokenId]== msg.sender, "Must be burner of old token");
    ...
    function mint(address to, uint8 tokenId, string calldata uri) public virtual nonReentrant adminRequired returns(uint256) {
        ...
        require(tokenId < 1 || tokenId > 117, "New token ID must not be between 1 and 117"); // 
