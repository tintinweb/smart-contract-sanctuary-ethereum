...
contract DillonNFTCollection3 is ERC721A, Ownable, ReentrancyGuard {
    ...
    address public _NFT1CollectionAddress = 0xF5060b5d81b62ecf9a590D6F1061D724728851B0;
    address public _NFT2CollectionAddress = 0xCae723BCB8510E85ABFEfECc2e99376306591DaA;
    ...
    function checkIfValidUser(address _user) public view returns (bool) {
        if (
            NFTHelper(_NFT1CollectionAddress).balanceOf(_user) > 0 &&
            NFTHelper(_NFT2CollectionAddress).balanceOf(_user) > 0
        ) {
            return true;
        }
        return false;
    ...
    function publicMint() external payable onlySender nonReentrant {
        ...
        require(
            checkIfValidUser(msg.sender),
            'Minting not allowed, As you have to hold 1 nft each on NFTCOllection 1 & NFT collection 2'
        );
        ...
        payable(devWalletAddress).transfer((msg.value.mul(20)).div(100));
    ...
    function updateNFT2CollectionAddress(address nft2CollectionAddress_) external onlyOwner {
        _NFT2CollectionAddress = nft2CollectionAddress_;
    }
