...
contract LunchMoneyDistributor is Pausable, ReentrancyGuard, Ownable {
    ...
    uint[6] public multipliers = [32, 16, 8, 4, 2, 1];
    uint[6] public timestamps = [1645750800, 1646960400, 1648166400, 1649376000, 1650585600, 1708822800];

    // Daily emission rate base is 77 tokens per bully
    ...
    function _calculateRewards(address _user, uint256 start_time, uint256 end_time) internal view returns (uint256) {
        ...
        for (uint i = 0; i < multipliers.length; i++){
            if (start_time > timestamps[i])
                continue;
            uint256 current_end_time = min(timestamps[i], end_time);
            reward += bitbulliesContract.bulliesBalance(_user).mul(BASE_RATE.mul(multipliers[i].mul((current_end_time.sub(start_time))))).div(86400);
