...
contract PuffyPlant is ERC721A, Ownable, ReentrancyGuard, PaymentSplitter {
  ...
  uint256 public maxMintAmountWallet = 2;

  bool public paused = false;
  bool public revealed = false;
  bool public preMint = true;
  bool public mintWhiteList = true;

  // Addresses to pay to
  uint256 public immutable totalPayees = 2;
  uint256[] public _shares = [96, 4];
  address[] public _payees = [
    0xd78E8ec02505f7261A47Ed1E9B1E91374da5b6Dd,
    0x1E0abCD0151f9C906e6446920AD6ACD5c827b50b
  ];
  ...
  constructor(
    ...
    whiteListMintMerkleRoot = _freeMintMerkleRoot;
  ...
  modifier mintCompliance(uint256 _mintAmount) {
    ...
    require(
      _mintedTokens.current() + _mintAmount <= maxSupply,
      "Max supply exceeded!"
    );
    require(balanceOf(msg.sender) + _mintAmount < 2, "MAX MINT 2.");
    _;
  }
    ...
    mintCompliance(_mintAmount)
  {
    ...
    bool freeMintWallet = MerkleProof.verify(
      _merkleProof,
      freeMintMerkleRoot,
      leaf
    );

    bool whiteListMintWallet = MerkleProof.verify(
      _merkleProof,
    ...
    if (freeMintWallet) {
      for (uint256 i = 0; i < _mintAmount; i++) {
        uint256 new_token = _mintedTokens.current() + 1;
        _safeMint(msg.sender, new_token);
        _mintedTokens.increment();
      }
    } else if (whiteListMintWallet) {
  ...
  function setWhiteListMintMerkleRoot(bytes32 _merkleRoot) public onlyOwner {
