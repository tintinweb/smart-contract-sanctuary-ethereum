...
contract XeniaLand is ERC721P, Ownable {
    ...
    function allowMint(
        ...
        require(
            landMints[msg.sender] + _amount <= _landAllowance,
            "Exceeds allowance"
        );
        require(msg.sender == tx.origin, "bm8gcm9ib3Rz");

        bytes32 hash = hashTransaction(_msgSender(), _landAllowance);
        require(
    ...
    function hashTransaction(address _sender, uint256 _landAllowance)
        public
        pure
        returns (bytes32 _hash)
    {
        _hash = keccak256(abi.encode(_sender, _landAllowance));
    }
