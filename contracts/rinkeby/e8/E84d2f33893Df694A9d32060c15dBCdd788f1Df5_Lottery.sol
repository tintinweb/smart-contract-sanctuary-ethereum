...
contract Lottery is VRFConsumerBase, Ownable{
    ...
    function getTicketsAtAddress(address _address) public view returns (uint256) {
        return addressToTickets[_address];
    }

    function enterLottery() public payable {
        //$50 per ticket (i.e. $50 min)
        require(lottery_state == LOTTERY_STATE.OPEN);
        require(msg.value >= getEntranceFee(), "not enough ETH!");
        uint256 tickets = msg.value / getEntranceFee();
        participants.push(msg.sender);
        addressToTickets[msg.sender] = tickets;
    }

    function getEntranceFee() public view returns (uint256) {
        (,int256 ethPrice, , ,) = ethUsdPriceFeed.latestRoundData();
        // Set price @ $50
        // Price feed has 8 decimals, convert to 18
    ...
    function endLottery() public onlyOwner {
        // Ends lottery and requests a random number through VRF
