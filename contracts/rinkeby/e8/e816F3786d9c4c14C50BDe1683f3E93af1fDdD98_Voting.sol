...
//FOR A SIMPLE USE WITH ERC20 TOKEN
    ...
    IERC20 private Catcoin;
    IERC721 private CatCoinNfts; // 0x21540c9647535262b344ddc8b9bd676e18810804 rinkeby
    IERC721 private CatCoinYatchClub; // 0x21540c9647535262b344ddc8b9bd676e18810804 rinkeby
...
// Stating variables.
    ...
    uint public minimunToVote;
    uint public stage = 1; // Have to start in 1
...
// CONSTRUCTOR HERE! 
    ...
    constructor(address _Catcoin, address _CatCoinNfts, address _CatCoinYatchClub) {
        Catcoin = IERC20(_Catcoin);
        CatCoinNfts = IERC721(_CatCoinNfts);
        CatCoinYatchClub = IERC721(_CatCoinYatchClub);
...
Only voters that have equal or more than 1000 Catcoin will be able
to register to vote and vote in this smart contract.
*/

    function registerVoters() public {
        require(!voters[msg.sender].isRegistered, "Already registered to vote");
        require(canVote(msg.sender), "Not enough token to register");

        voters[msg.sender].isRegistered = true;
        voters[msg.sender].lastStagedVoted = 0;
        voters[msg.sender].voteProposalId = 0;

        emit VoterRegistered(msg.sender);
    }

    function canVote(address _addy) public view returns (bool) {
        return Catcoin.balanceOf(_addy) >= minimunToVote;
    }
...
*/
    ...
    //REGISTER A PROPOSAL.
    function registerProposal(string memory proposalDescription) public Admin Status1 {
    ...
    function getProposalStage(uint index) public view returns (uint) {
        return proposals[index].stage;
    }
...
*/
    ...
    //(ADMIN) PUBLIC VOTING FUNCTION. ONLY REGISTERED VOTERS CAN VOTE.
    function vote(uint proposalId, bool _positiveVote) public Status3 {
        if (!voters[msg.sender].isRegistered) {
            registerVoters();
        } else {
            require(canVote(msg.sender), "Wallet need conditions for vote");
        }

        require(voters[msg.sender].lastStagedVoted != stage, "You already voted");
        int cantVote = 0;

        if (Catcoin.balanceOf(msg.sender) > 0) {
            cantVote += 1;
        }
        
        if (CatCoinNfts.balanceOf(msg.sender) > 0) {
            cantVote += 2;
        }

        if (CatCoinYatchClub.balanceOf(msg.sender) > 0) {
            cantVote += 2;
        }

        voters[msg.sender].lastStagedVoted = stage;
        voters[msg.sender].voteProposalId = proposalId;

        if (_positiveVote) {
            proposals[proposalId].voteCount += cantVote;
        } else {
            proposals[proposalId].voteCount -= cantVote;
        }
    ...
    function setMinimunToVote(uint _minimunToVote) public Admin {
        minimunToVote = _minimunToVote;
    }
...
*/ 
    ...
    function tallyVotes(uint _stage) public Admin Status4 {
        int winningVoteCount = 0;
        uint winningProposalIndex = 0;

        for (uint i = 0; i < proposals.length; i++) {
            if (proposals[i].voteCount > winningVoteCount && proposals[i].stage ==_stage) {
    ...
    function winningCount() public view Status5 returns(int) {
        return proposals[winningProposalId].voteCount;
    }

    function resetSystem() public Status5 Admin returns(bool) {
        state = Status.RegisteringVoters;

        stage += 1;

        emit StatusChange(Status.AfterVoting, state);
        return true;
    }
