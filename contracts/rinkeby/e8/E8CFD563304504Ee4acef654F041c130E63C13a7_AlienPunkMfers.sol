...
contract AlienPunkMfers is Ownable, ERC721ABurnable {
    ...
    uint public tokenPrice = 0.0069 ether;
    uint public max = 6969;
    bool public saleIsActive = false;
    uint public pluckPrice = 0.0069 ether;
    bool public pluckingEnabled = true;
    ...
    constructor() 
        ...
        mfers = ERC721(0x5B98Ab35514C1C91F33ba12E0778D53E1eBDb106);
    }
    ...
    function mint(uint total) external payable {
        ...
        require(tokenPrice * total <= msg.value, "Not enough eth");  
    ...
    function aptClaim(uint[] calldata aptTokenIds) external {
        ...
        for(uint i=0; i < total; i++) {
            ...
            require(alienPunkThings.ownerOf(tokenId) == msg.sender, "Not your apt");
    ...
    function mferClaim(uint[] calldata mferTokenIds) external {
        ...
        for(uint i=0; i < total; i++) {
            ...
            require(mfers.ownerOf(tokenId) == msg.sender, "Not your mfer");
    ...
    function pluckHead(uint tokenId, uint head) external {
        ...
        require(pluckingEnabled, "Plucking paused");
        require(hasHead(tokenId, 1) && head == 1 || hasHead(tokenId,2) && head == 2 , "No head");
        require(ownerOf(tokenId) == msg.sender, "Not your alien punk mfer");
        
        // Make sure cooldown has passed
        uint time = block.timestamp;
        uint birthTime = tokenData[tokenId].birthBlockTime;
        require(birthTime == 0 || time - birthTime >= pluckCooldown, "Head is not ripe");

        // Create ancestry data for new token
        uint newId = _currentIndex;
    ...
    function hasHead(uint tokenId, uint head) public view virtual returns (bool) {
        ...
        // TODO: Figure out chance to get head 1 and head 2
        return (random(string(abi.encodePacked((head == 1 ? "HEAD1" : "HEAD2"), toString(geneOf(tokenId))))) % 500) < 250;
    }

    function birthBlockTimeOf(uint tokenId) external view virtual returns(uint birthBlockTime) {
        require(_existsOrBurned(tokenId), "Token does not exist.");
        return tokenData[tokenId].birthBlockTime;
    }
    ...
    function setPluckData(address droolAddr, uint price, uint cooldown, bool enabled) external onlyOwner {
        drool = DROOL(droolAddr);
        pluckPrice = price;
        pluckCooldown = cooldown;
        pluckingEnabled = enabled;
    }
    ...
    function tokenIdGene(uint tokenId) internal pure returns (uint256) {
    ...
    function _existsOrBurned(uint256 tokenId) internal view returns (bool) {
        return 0 <= tokenId && tokenId < _currentIndex;
    }
