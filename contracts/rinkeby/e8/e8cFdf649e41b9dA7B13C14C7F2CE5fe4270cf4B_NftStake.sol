...
contract NftStake is IERC721Receiver, ReentrancyGuard {
    ...
    // add reentrecency guard modifiers
    // comments

    IERC721 public _nftToken = IERC721(0xe3649bb23B3f658b5Ee86a7FfD9A4d05462483a2);    // PLACE OWL TOKEN ADDRESS HERE
    IERC20 public _erc20Token = IERC20(0xe3649bb23B3f658b5Ee86a7FfD9A4d05462483a2);   // PLACE HOOT TOKEN ADDRESS HERE
    ...
    event StakePayout(address indexed staker, uint256 tokenId, uint256 stakeAmount, uint256 startTime, uint256 endTime);
    event StakeRewardUpdated(uint256 rewardPerBlock, uint256 rewardPerSecond);
    ...
    modifier requireTimeElapsed(uint256 tokenId) {
        ...
        require(
            receipt[tokenId].startTime < block.timestamp + 60,
    ...
    constructor(
        ...
        require(bPortion_ < 50, "less than 5% only");
        _admin = admin_;
        _burnPortion = bPortion_;
    }
    ...
    function unStakeNFT(uint256[] memory tokenId) public nonReentrant {
        for(uint indx = 0; indx < tokenId.length; indx++) {
            _unStakeNFT(tokenId[indx]);
        }
    }

    function _unStakeNFT(uint256 tokenId) internal onlyStaker(tokenId) requireTimeElapsed(tokenId) {
    ...
    function reclaimTokens() external onlyAdmin {
        _erc20Token.transfer(_admin, _erc20Token.balanceOf(address(this)));
    }

    function updateStakingReward(uint256 traitType, uint rewardPerSecond) external onlyAdmin {
    ...
    function _payoutStake(uint256 tokenId) internal {
        ...
        if (_erc20Token.balanceOf(address(this)) < payout) {
            emit StakePayout(msg.sender, tokenId, 0, receipt[tokenId].startTime, block.timestamp);
            return;
        }
        ...
        emit StakePayout(msg.sender, tokenId, payout, receipt[tokenId].startTime, block.timestamp);
    }
    ...
    function setRewardPerTokenId(uint[] memory tokenId, uint[] memory rewardPerSecond) external onlyAdmin {
    ...
    function getRewardPerTokenId(uint tokenId) public view returns(uint256) {
