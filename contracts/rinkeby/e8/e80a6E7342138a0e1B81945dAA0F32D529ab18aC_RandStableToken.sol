...
/**
 * @title Rand Stable Token implementation
 * @dev Contract holding all the tokens associated to the USDC balance of the user.
    ...
    PausableUpgradeable
{
    ...
    function initialize() external initializer {
        ERC20Upgradeable.__ERC20_init("Rand", "RND");
    ...
    function mint(address _account, uint256 _amount) public override {
        require(
            ...
            "Rand Stable Token: You don't have permission to mint tokens!"
        );
    ...
    function burn(address _account, uint256 _amount) public override {
        require(
            hasRole(MINTER_AND_BURNER_ROLE, _msgSender()),
            "Rand Stable Token: You don't have permission to burn tokens!"
        );
     ...
     * @param _from address from which account the tokens will be transfered
     * @param _to address which can use the tokens
    ...
    function approveExternal(
        ...
        require(
            hasRole(MINTER_AND_BURNER_ROLE, _msgSender()),
            "Rand Stable Token: You don't have permission to call this function!"
        );
        super._approve(_from, _to, _amount);
    }
        ...
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        require(
            _address != address(0),
            "Rand Stable Token: Address cannot be address 0!"
        );
