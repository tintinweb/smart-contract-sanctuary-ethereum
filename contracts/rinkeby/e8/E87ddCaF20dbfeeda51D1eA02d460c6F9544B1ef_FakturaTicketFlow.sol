...
UUPSUpgradeable
{
	struct DutchAuction {
		...
		uint256 slope;				// Decrease percent with 6 decimals.	ex: 99000001 = 99.000001%
		uint256 floorPrice; 		// The minimum price.
		// uint8 fRestart;				// Flag for restart automatically
	}
	...
	function getPrice(uint8 n, uint256 sales) public view returns (uint256) {
		...
		if (_salesPhase.salesType == 1) {
			...
			for (uint i = 0; i < numDecreases; i ++) {
				_price = _price * _salesPhase.auction.slope / (10 ** 8);
			}

			return n * MathUpgradeable.max(_price, _salesPhase.auction.floorPrice);
		}
	...
	function safeMint(address to, uint8 amount, uint256 sales, uint256 category, bytes32 r, bytes32 s, uint8 v) public payable whenNotPaused {
		...
		require(
			...
			/*salesPhase[sales].auction.fRestart == 1*/
			"SalesPhase finished."
		);
		require(
