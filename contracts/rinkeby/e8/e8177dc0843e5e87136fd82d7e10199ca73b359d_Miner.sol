...
contract LITTLEBOYToken is ERC20, ReentrancyGuard, Ownable {
    ...
    // max supply 
    uint256 public constant MAX_COMMUNITY_FUND_SUPPLY = 1000000000000000000000000000 ether;
    ...
    constructor(address _miner) ERC20("LITTLEBOY TOKEN", "LBT") {
        ...
        totalCommunityFundSupply += 1000000000000000000000000000;
        totalYieldSupply += 1000000000000000000000000000;
    }
...
contract Miner is ERC721Enumerable, ReentrancyGuard, Ownable {
    ...
    string public Miner_PROVENANCE = "";
    string public baseTokenURI;

    uint256 public constant MAX_MINER = 7400;
    ...
    constructor(string memory baseURI, address _littleboy) ERC721("Miner", "Miner") {
        setBaseURI(baseURI);
        lbContract = ILittleBoy(_littleboy);
    }
  ...
  function withdrawAllERC20(address _tokenContract, uint256 _amount) public onlyOwner {
    ...
    function mintAdmin(uint256[] calldata tokenIds, address _to) public payable onlyOwner {
        ...
        for(uint256 i = 0; i < tokenIds.length; i++) {
            ...
            require(lbContract.ownerOf(tokenIds[i]) == _to, "Not the owner of this littleboy");
    ...
    function mint(uint256[] calldata tokenIds) public payable nonReentrant {
        ...
        for(uint256 i = 0; i < tokenIds.length; i++) {
            require(tokenIds[i] < MAX_MINER, "Invalid token ID");
            require(lbContract.ownerOf(tokenIds[i]) == msg.sender, "Not the owner of this littleboy");
            require(!_exists(tokenIds[i]), "Tokens has already been minted");
        ...
        //update reward on mint
        lbToken.updateRewardOnMint(msg.sender, tokenIds.length);
    }
