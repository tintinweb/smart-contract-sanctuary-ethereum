...
import "./base/DevCitiNFT.sol";

contract DevCitiBuilding is DevCitiNFT {
    /// @custom:oz-upgrades-unsafe-allow constructor
    constructor() initializer {}

    function initialize() initializer public {
        __DevCitiNFT_init("CitiBuilding", "CTB");
    }
}
...
/**
 * @dev Extension of BaseCitiNFT with dev features only available on testnets.
 */
contract DevCitiNFT is BaseCitiNFT {
    /**
     * @dev Initializes the `name` and `symbol` of the contract.
     */
    function __DevCitiNFT_init(string memory _name, string memory _symbol) internal onlyInitializing {
        require(!Utils.isMainnet(), "mainnet not allowed");
        __BaseCitiNFT_init(_name, _symbol);
    }
    
    /**
     * @dev Mints a token for `to` with `uri`.
     *
     * Returns the minted tokenId.
     */
    function devMint(address to, string calldata uri) public returns (uint256) {
        return _doSafeMint(to, uri);
    }

    /**
     * @dev Mints a token for `to` with `uri` and `numTokensToBurn` tokens to burn.
     *
     * Returns the minted tokenId.
     */
    function devMint2(address to, string calldata uri, uint256 numTokensToBurn) public returns (uint256) {
        if (numTokensToBurn > 0) {
            require(_token != BaseToken(address(0)), "bad token contract");
    ...
    /**
     * @dev Mints a token for `to` with `uri` and `tokenId`.
     *
     * Returns the minted tokenId.
     */
    function devMintWithTokenId(address to, string calldata uri, uint256 tokenId) public returns (uint256) {
    ...
    /**
     * @dev Updates the URI for token `tokenId` to `newURI`.
     *
     * Emits {TokenURIUpdated}
     */
    function devUpdateTokenURI(uint256 tokenId, string calldata newURI) public {
...
contract BaseCitiNFT is Initializable, ERC721Upgradeable, ERC721EnumerableUpgradeable, ERC721URIStorageUpgradeable, PausableUpgradeable, AccessControlUpgradeable, ERC721BurnableUpgradeable, Versioned {
    ...
    /**
     * @dev Emitted when the next value of the tokenId counter is changed from `oldCounter` to `newCounter`.
     */
    event TokenIdCounterOverriden(uint256 oldCounter, uint256 newCounter);
    
    /**
     * @dev Emitted when the dummy owner is changed from `oldOwner` to `newOwner`.
     */
     ...
     * @dev Initializes the `name` and `symbol` of the contract.
     */
    function __BaseCitiNFT_init(string memory _name, string memory _symbol) internal onlyInitializing {
    ...
    /**
     * @dev Mints a token to `to`, allows specifying the token URI to `uri`, requires `MINTER_ROLE`
     *
     * Returns the minted tokenId.
     */
    ...
    /**
     * @dev See {ERC721Upgradeable}, {ERC721EnumerableUpgradeable}
     */
    ...
    /**
     * @dev See {ERC721Upgradeable}, {ERC721URIStorageUpgradeable}
     */
     ...
     * Use the computed URI if `baseTokenURI` is set, otherwise use the stored one.
     */
    ...
    /**
     * @dev See {ERC721Upgradeable}, {ERC721EnumerableUpgradeable}, {AccessControlUpgradeable}
     */
     ...
     */
    function _doSafeMint(address to, string calldata uri) internal returns (uint256) {
    ...
    /**
     * @dev Returns the URI for the token owned by `account` at `index`.
     */
    ...
    function adminSetTokenContract(BaseToken token) public onlyRole(DEFAULT_ADMIN_ROLE) whenNotPaused {
        require(token != BaseToken(address(0)), "bad token contract");
    ...
    function adminSetValidatorContract(Validator validator) public onlyRole(DEFAULT_ADMIN_ROLE) whenNotPaused {
        require(validator != Validator(address(0)), "bad validator contract");
    ...
    function upgradeWithDetails(uint256 tokenId, string calldata details, 
        ...
        require(tokenNonce == _tokenNonces[tokenId], "bad nonce");
    ...
    /**
     * @dev Admin function to assign ownership of `tokenIds` to `owners`, requires `DEFAULT_ADMIN_ROLE`.
     * 
     * Emits {Transfer}
     */
    function adminMigrateOwnership(uint256[] calldata tokenIds, address[] calldata owners) public onlyRole(DEFAULT_ADMIN_ROLE) whenNotPaused {
        require(tokenIds.length == owners.length, "tokenId/owner mismatch");
    ...
    /**
     * @dev Admin function to assign ownership of `tokenIds` to `owners`, requires `DEFAULT_ADMIN_ROLE`.
     * The URIs of the tokens are set to `uris`
     *
     * Emits {Transfer}
     */
    function adminMigrateOwnershipWithUri(uint256[] calldata tokenIds, address[] calldata owners, string[] calldata uris) public onlyRole(DEFAULT_ADMIN_ROLE) whenNotPaused {
        require(tokenIds.length == owners.length, "tokenId/owner mismatch");
        require(tokenIds.length == uris.length, "tokenId/uri mismatch");
        for (uint index = 0; index < tokenIds.length; ++index) {
            _mint(owners[index], tokenIds[index]);
    ...
    /**
     * @dev Admin function to override the tokenId counter, requires `DEFAULT_ADMIN_ROLE`.
     *
     * Emits {TokenIdCounterOverriden}
     */
    function adminOverrideTokenIdCounter(uint256 newCounter) public onlyRole(DEFAULT_ADMIN_ROLE) whenNotPaused {
        require(newCounter >= _tokenIdCounter.current(), "too small");
    ...
    modifier whenHasValidator() {
        require(_validator != Validator(address(0)), "bad validator contract");
        _;
    }
}
...
contract BaseToken is Initializable, ERC20Upgradeable, ERC20BurnableUpgradeable, PausableUpgradeable, AccessControlUpgradeable, Versioned {
     ...
     * @dev Initializes the `name` and `symbol` of the contract.
     */
    function __BaseToken_init(string memory _name, string memory _symbol) internal onlyInitializing {
        require(Utils.isKnownNetwork(), "unknown network");
...
contract Versioned {
    string public constant version = "451cfd3f2d00d1d8265bff5f2a406c4a51854f1c";
}
...
contract Validator is Initializable, PausableUpgradeable, AccessControlUpgradeable, Versioned {
     ...
     * Grant `DEFAULT_ADMIN_ROLE`, `PAUSER_ROLE` to the caller
     */
    function initialize() initializer public {
        require(Utils.isKnownNetwork(), "unknown network");
        __Pausable_init();
    ...
    /**
     * @dev Verify if `message` is signed by enough validators whose signatures is in `signature`
     * The length of the signature is expected to be 32 + number of validators * 65
     */
...
library Utils {
    ...
    * @dev Returns if the current network is known.
     */
    function isKnownNetwork() internal view returns (bool) {
