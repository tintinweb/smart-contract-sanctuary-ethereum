...
contract NFTLauncher is VRFConsumerBaseV2, ChainlinkClient {
    ...
    // address CHAINLINK_ADDRESS = 0x01BE23585060835E02B77ef475b0Cc51aA1e0709;
    ...
    uint256 PACK_SALE_BASE_FEE_PERCENTAGE = 1000; //10%
    ...
    constructor(string memory chainLinkJobId, uint64 subscriptionId)
        VRFConsumerBaseV2(vrfCoordinator)
    {
        s_subscriptionId = subscriptionId;
        CHAINLINK_RANDOMNESS_BASE_FEE = 0.5 ether; // 0.25 LINK (Varies by network)
    ...
    function getContractDetails() public view returns(uint256 RANDOMNESS_BASE_FEE, uint256 PACK_GEN_CHAINLINK_FEE, uint256 PACK_GEN_ETH_FEE, uint256 PACK_SALE_FEE_PERCENTAGE) {
        return (CHAINLINK_RANDOMNESS_BASE_FEE, CHAINLINK_FEE_PACK_GENERATION, PACK_LAUNCH_COST, PACK_SALE_BASE_FEE_PERCENTAGE);
    }
        ...
        )
    {
        ...
        uint256 REMAINDER_FEE_PERCENTAGE = PACK_SALE_BASE_FEE_PERCENTAGE -
            FEE_PERCENTAGE_SAVED_USING_THIS_METHOD;
    ...
    function purchasePack(
        ...
        uint256 tmpProjectID = projectid;
        // uint32 callBackGasLimit = (20000 * project.itemsPerPack) + 200000;
        // uint32 chainLinkToPurchase = CHAINLINK_RANDOMNESS_BASE_FEE;

        pb.ownedBy = msg.sender;
        if (project.whitelisted) {
            // project.whitelist[msg.sender];
            require(project.whitelist[msg.sender] == true, "Not whitelisted");
        }
        (
            ,
            address paymentMethodAddress,
        ...
        LinkTokenInterface(chainlinkTokenAddress()).approve(chainlinkTokenAddress(), CHAINLINK_RANDOMNESS_BASE_FEE);
        LinkTokenInterface(chainlinkTokenAddress()).transferAndCall(vrfCoordinator, CHAINLINK_RANDOMNESS_BASE_FEE , abi.encodePacked(s_subscriptionId));
        //we need to calcualate the fee from the original balance. not from afterwards we did the chainlink token transfer.
        // transferLinkToSubscription(CHAINLINK_RANDOMNESS_BASE_FEE);
    ...
    // function transferLinkToSubscription(uint256 finalAmt) public {
    //     // s_subscriptionId
    //     // CHAINLINK_RANDOMNESS_BASE_FEE
    //     // LinkTokenInterface(CHAINLINK_ADDRESS).approve(vrfCoordinator, finalAmt);
    //     LinkTokenInterface(CHAINLINK_ADDRESS).approve(CHAINLINK_ADDRESS, finalAmt);
    //     LinkTokenInterface(CHAINLINK_ADDRESS).transferAndCall(vrfCoordinator, finalAmt , abi.encodePacked(s_subscriptionId));
    ...
    function splitRemainingFunds(
        ...
        address projectOwner = getProjectOwner(projectid);
    ...
    function updatePackOwner(uint256 projectid, uint packId, address newOwner) external {
        Projects storage project = projects[projectid];
        ProjectsP2 storage p2 = projectsP2[projectid];
        require(msg.sender == project.packAddress, "Caller needs to be the pack");
        PacksBought storage pb = project.packsBought[packId];
        pb.ownedBy = newOwner;
    }
    function swapAndTransferTo(
        ...
        if (swapAddressTo == uniswapV2Router.WETH()) {
            ...
            //if this is a token transferFrom, we need to swap the amount from the token to endToken
        ...
        //how much did we recieve from swap
        uint256 recieved = balAfterTransfer - balBeforeTransfer;
        token.transfer(owner, recieved);
    }
        ...
        returns (uint256)
    {
        ...
        // function.
        uint32 callBackGasLimit = (20000 * project.amountToPayOutPerPack) +
            200000;
    ...
    function openPack(uint256 projectid, uint256 packSoldID_) public {
        ...
        require(tx.origin == pb.ownedBy, "Not owner of pack.");
    ...
    function updateProject(
        ...
        for (uint256 index = 0; index < whitelistedAddresses.length; index++) {
            project.whitelist[whitelistedAddresses[index]] = whitelistedAddressesEnabled;
        }
    }
...
contract Pack is ERC721, Ownable {
    ...
    function burnPack(uint256 tokenId) public {
        require(ownerOf(tokenId) == msg.sender || msg.sender == parent, "you cant burn this pack");
        _burn(tokenId);
    }
