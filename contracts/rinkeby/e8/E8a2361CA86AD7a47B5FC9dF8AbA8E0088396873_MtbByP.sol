...
contract MtbByP is ERC721A, Ownable {
    ...
    uint256 public constant PUBLICSALE_PRICE = 0.1 ether;
    ...
    modifier onlyStatus(Status s) {
        require(status == s, "Sale is not active.");
        _;
    }
    ...
    function presaleMint(uint256 amount) external payable onlyStatus(Status.PreSale) onlyEOA() {
    ...
    function mint(uint256 amount) external payable onlyStatus(Status.PublicSale) onlyEOA() {
