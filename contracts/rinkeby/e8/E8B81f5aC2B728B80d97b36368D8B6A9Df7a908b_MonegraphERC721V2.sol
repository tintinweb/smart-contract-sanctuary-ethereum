...
contract MonegraphERC721V2 is MonegraphERC721, OwnableUpgradeable {
    function initialize(
        string memory name,
        string memory symbol,
        address admin,
        ...
        super.initialize(name, symbol, admin, info);

        _transferOwnership(admin);
    }

    function setOwnerFromInitializedBy() public onlyRole(DEFAULT_ADMIN_ROLE) {
        require(
            this.owner() == address(0),
            "MonegraphERC721V2: Owner already set"
        );

        require(
            initializedBy != address(0),
            "MonegraphERC721V2: Initialized by not set"
        );

        this.transferOwnership(initializedBy);
    }
}
