...
error TWG_ExceedsTrancheSupply();
...
contract TWG is ERC1155, Ownable, ReentrancyGuard {
    ...
    string private constant NAME = "The Wolf Guild";
     ...
     */
    modifier lockable() {
        if (functionLocked[msg.sig]) revert TWG_FunctionLocked();
        _;
    }
    ...
    function setTrancheMax(uint256 max) external onlyOwner {
        if (max > publicSupply) revert TWG_ExceedsPublicSupply();
    ...
    function publicMint(uint256 amount) external payable nonReentrant {
        ...
        if (publicMinted > trancheMax) revert TWG_ExceedsTrancheSupply();
