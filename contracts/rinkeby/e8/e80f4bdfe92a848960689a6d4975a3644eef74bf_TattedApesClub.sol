...
contract TattedApesClub is Delegated, ERC721EnumerableLite, PaymentSplitterMod {
    ...
    uint public maxWhitelistMint = 2; // number of tokens that can be minted per whitelisted address
    uint public salePrice        = 0.1 ether; // price during public mint
    uint public whitelistPrice   = 0.08 ether; // price for whitelisted minters
    ...
    address[] private addressList = [
        ...
        0x7B850fEf1064680d2F9e86CD1a0B83C052a2620c,
        0x9e25Ce06a0b5Df2C8D7A8893E83AfB89966BfC92,
        0x015a5DBA55D37E2779e0FA244840061D00F507F7,
    ...
    constructor()
    Delegated()
    ERC721B("Tatted Apes Club", "TAC", 0)
    ...
    modifier whiteListMint {
        require(isPresaleActive, "Whitelist mint is not active!");
        _;
    }
    ...
    function mint(uint amount_) external payable onlySender {
        ...
        require(msg.value >= salePrice * amount_, "Ether sent is not correct!");
    ...
    /**
     * @dev Activate/deactivate whitelisted presale and main mints. Can only be called by delegates.
     */
     ...
     */
    function setPrice(uint whitelistPrice_, uint salePrice_) external onlyDelegates {
        require(whitelistPrice != whitelistPrice_ || salePrice != salePrice_, "New value matches old");
    ...
    /**
     * @dev Set the max number of tokens that can be minted at whitelist price.
     */
    function setMaxWhitelistMint(uint amount_) public onlyDelegates {
        maxWhitelistMint = amount_;
    }


    // General Whitelist Mint Functions
    ...
    /**
     * @dev Checks merkle proof to determine if address is whitelisted.
     */
    ...
    /**
     * @dev Whitelisted users can mint tokens with valid merkle proof.
     */
    function mintWhitelist(bytes32[] memory proof_, uint amount_) public payable onlySender whiteListMint {
        ...
        require(addressToWhitelistMinted[msg.sender] + amount_ <= maxWhitelistMint, "You do not have enough whitelist mints available!");
        require(msg.value == whitelistPrice * amount_, "Invalid value sent!");

        uint _supply = totalSupply();
        require(_supply + amount_ <= maxTokens, "Mint request exceeds supply!");

        addressToWhitelistMinted[msg.sender]++; // increments the tracker so that they cannot mint more than allowed
