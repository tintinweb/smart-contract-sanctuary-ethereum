...
contract QueenZ is ERC721A, Ownable {
    bytes32 public root;
    ...
    uint public MAX_FREE_MINT = 96;
	uint public maxSupply = 222;

	bool public isPublicMint = false;
    bool public isMetadataFinal;
    string private _baseURL = "ipfs://bafybeiatzczdgeloljm4rmbunjtgytczuahplzffrchsxvz7d7k3keefyq/";
	string public prerevealURL = '';
	mapping(address => uint) private _walletMintedCount;

    // Name
	constructor()
	ERC721A('QueenZ', 'QUEENZ') {
    }
    ...
    function mintQueenz(uint count) external payable {
		require(isPublicMint, "The mint has not started");
        ...
        uint payForCount = count * MINT_PRICE;

        if (_totalMinted() <= MAX_FREE_MINT) {
            payForCount = 0;
        }

		require(
			msg.value >= payForCount,
