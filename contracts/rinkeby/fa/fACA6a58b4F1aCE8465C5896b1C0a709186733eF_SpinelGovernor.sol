    ...
    GovernorTimelockControlUpgradeable, OwnableUpgradeable
{
        ...
        returns (uint256)
    {
        ...
        bytes memory data = calldatas[0];
        bytes4 methodSignature;
        uint256 callAmount;
        assembly {
            methodSignature := mload(add(data, 32))
            callAmount := mload(add(data, 68))
        }
        require(methodSignature == 0xa9059cbb, "Invalid transfer method");
    ...
    function markProposalFill(
        ...
        require(
            ...
            "Governor: proposal not executed"
        );
        ISpinelTimelock timelockSpinel = ISpinelTimelock(timelock());
    ...
    ) internal override(GovernorUpgradeable, GovernorTimelockControlUpgradeable) onlyOwner{
    ...
    ISpinelTimelock
{
    ...
    function provideFund(uint256 _fundAmount) public payable {
        require(msg.value > 0, "Provied fund amount too small");
        require(msg.value == _fundAmount, "Provied fund not match");
        ...
        emit FundProvided(msg.sender, _fundAmount);
    }
