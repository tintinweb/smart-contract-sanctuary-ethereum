...
contract PaperHandSocietyDAO is ERC721AEnumerable, Payable {
    ...
    /**
     * Return the number of tokens available for this address.
     * @notice Presale allowance may show tokens are available when user is not listed.
     * @param addr The address to check.
     */
    function allowance(address addr) public returns (uint256) {
        if (contractState == ContractState.PRESALE) {
            return PRESALE_ALLOWANCE_P1 - _numberMinted(addr);
        } else if (contractState == ContractState.PUBLIC) {
            return PUBLIC_ALLOWANCE_P1 - _numberMinted(addr);
        }
        return 0;
    }
}
