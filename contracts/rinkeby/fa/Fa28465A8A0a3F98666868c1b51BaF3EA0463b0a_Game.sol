...
import "./staking/SimpleERC721StakingPool.sol"; 
...
contract Game is Ownable, IERC1155Receiver { 
    ...
    SimpleERC721StakingPool public staking;
    ...
    mapping(address => mapping(uint256 => bool)) public walletHasPlayed;

    constructor(address groupies_, address tokenERC1155_, address staking_, address owner_) Ownable(owner_) {
        groupies = IERC721(groupies_);
        tokenERC1155 = IERC1155(tokenERC1155_);
        staking = SimpleERC721StakingPool(staking_);
    }
    ...
    function claimTokenWithSolution(string memory incomingSolution_) public {
        ...
        require(groupiesBalanceOf(sender) > 0 || staking.balanceOf(sender) > 0, "No groupies balance");
        require(!walletHasPlayed[sender][tokenId], "Wallet has already played");

        require(_solution == sha256(abi.encode(incomingSolution_)), "Wrong solution");
        walletHasPlayed[sender][tokenId] = true;

        tokenERC1155.safeTransferFrom(address(this), sender, tokenId, 1, "");
        emit NewWinner(sender, tokenId);
    }
...
contract SimpleERC721StakingPool {
