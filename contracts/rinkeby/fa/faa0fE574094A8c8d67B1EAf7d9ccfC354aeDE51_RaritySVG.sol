...
abstract contract InfinityMintAsset is AssetInterface, Authentication {
		...
		returns (uint64)
	{
		...
		if (result >= tokenPaths.length) return uint64(safeDefaultReturnPath);
	...
	function isValidPath(uint64 pathId) public view override returns (bool) {
		return (pathId > 0 &&
	...
	function getColours(uint64 pathId, RandomNumber randomNumberController)
		...
		returns (uint64[] memory result)
	{
		uint64 pathSize = getPathSize(pathId);
		...
		if (pathSize <= div) {
			result = new uint64[](4);
			result[0] = uint64(randomNumberController.getMaxNumber(0xFFFFFF));
			result[1] = pathSize;
			result[2] = uint64(randomNumberController.getMaxNumber(0xFFFFFFFF));
			result[3] = uint64(valuesController.tryGetValue("extraColours"));
			return result;
		}

		uint64 groups = 0;
		for (uint256 i = 0; i < pathSize; i++)
			if (i == 0 || i % div == 0) groups++;

		uint64 size = (groups * 2);
		uint64 tempPathSize = (pathSize);
		uint256 count = 0;
		result = new uint64[](size + 2);
		for (uint256 i = 0; i < size; i++) {
			if (i == 0 || i % 2 == 0)
				...
				result[i] = uint64(tempResult > div ? div : tempResult);
			}
		}

		result[result.length - 2] = uint64(
			randomNumberController.getMaxNumber(0xFFFFFFFF)
		);
		result[result.length - 1] = uint64(
