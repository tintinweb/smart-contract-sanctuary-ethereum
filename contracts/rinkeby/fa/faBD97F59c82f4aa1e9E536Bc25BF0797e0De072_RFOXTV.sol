...
contract RFOXTV is ERC721ABurnable, Ownable {
  ...
  event UpdatePriceSettings(IERC20 oldSaleToken, uint256 oldTokenPrice, IERC20 newSaleToken, uint256 newTokenPrice);
  ...
  constructor(string memory name_, string memory symbol_, address authorizedSigner_, IERC20 saleToken_, uint256 tokenPrice_) ERC721A(name_, symbol_) {
  ...
  */
  ...
  /**
    ...
    function safeMint(address to, uint256 totalMint, bytes32[] memory externalId, uint256 salt, bytes calldata signature) external payable checkUsedSignature(signature) {
      ...
      if (address(saleToken) == address(0)) {
        require(
            msg.value == tokenPrice*totalMint,
      ...
      for(uint256 i = previousIndex; i < _currentIndex; i++) {
        ...
        require(tempExternalId != 0 && externalID[tempExternalId] == 0, "INVALID_EXTERNAL_ID");
