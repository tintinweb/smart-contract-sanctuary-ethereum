...
contract LPStaking is Initializable, ReentrancyGuardUpgradeable, OwnableUpgradeable {
    ...
    mapping(uint256 => uint256) public periodValueMap;
     ...
     */
    function initialize(uint256 _startBlock) external initializer {
        __Context_init();
        __Ownable_init();
    ...
    function stake(
        ...
        uint256 amount,
        uint256 _period
    ) external nonReentrant poolExists(_poolId) returns (uint256) {
    ...
    function withdraw(
        ...
        require(pools[_poolId].totalAllotedReward > reward, "Staking: Insufficient reward allowance from the Admin");
        ...
        IERC20Upgradeable staketoken = IERC20Upgradeable(pools[_poolId].token);
        staketoken.transfer(msg.sender, amount);

        // Update pools stats
        pools[_poolId].totalAllotedReward -= reward;
        pools[_poolId].totalClaimedReward += reward;
        pools[_poolId].tvl -= details.stake;
        uint256 period = (details.lastRewardCalculated - details.period) / 30 days;
