...
contract wSACX is ERC20 {
  ...
  IsacxToken public immutable SACX;
  
  constructor (address _SACX) ERC20( 'wrapped sacx', 'wsacx') {
    require(_SACX != address(0));
    SACX = IsacxToken(_SACX);
  }
  ...
  function unwrap(uint _amount) external returns (uint) {
    ...
    SACX.transfer( msg.sender, value);
    return value;
  }

  function wSACXToSACX(uint _amount) public view returns (uint) {
    return _amount.mul(SACX.index()).div(10 ** decimals());
  }

  function SACXTowSACX( uint _amount ) public view returns (uint) {
    return _amount.mul(10 ** decimals()).div(SACX.index());
  }
