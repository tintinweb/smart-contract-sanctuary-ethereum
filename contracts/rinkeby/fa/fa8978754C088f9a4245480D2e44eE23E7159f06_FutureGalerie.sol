...
contract FutureGalerie is Satoshigoat {

	uint8 constant public NUM_LEVELS = 4;

	mapping(uint256 => string) internal _tidToHash;//token id -> ipfs hash
	mapping(address => uint256) internal _addressToTid;//wallet address -> token id

	//HUGE PROBLEM: IF YOU OWN MORE THAN ONE OF THESE, THE TOKEN ID ASSOCIATED WITH YOUR ADDRESS WILL BE ERASED
	...
	function _beforeTokenTransfers(
    	...
    	// If this isn't a transfer associated with minting,
    	// change the known owner of the token id
    	...
    	unchecked {
    		...
    		for (i = startTokenId; i < endingId; i++) {
		    	// This check may seem redudant since `_addressToTid[to]`is assigned
		    	//  after the call to `_safeMint` in airdrop functions, but this will save
		    	//  on pointless writes to storage for minting
				_addressToTid[from] = MAX_NUM_TOKENS + 1;//non-existent token id
				_addressToTid[to] = i;
	...
	//@dev Allows owners to airdrop `numToken` (orbs) to address `to` (member level)
	...
	(
		...
		nonReentrant
		notContract(to)
		enoughSupply(numTokens) 
	{
		uint currentIdx = _totalMinted();

		_safeMint(to, 1);

		_tidToHash[currentIdx] = _levelHashes[0];//assign this token id to 'member' level
		_addressToTid[to] = currentIdx;//associate the token id with the owner address
	}

	//@dev Allows owners to mint/airdrop to a list of addresses (all 'member' level)
	//NOTE: function is not checking to see if the address is a contract
	function bulkAddressAirdrop
	(
		address[] calldata airdropAddressesArray, 
		...
		enoughSupply(addressArrayLength)
	{
		...
		unchecked {
			...
			for (i = currentIdx; i < lastIdx; i++) {
				...
				_tidToHash[i] = _levelHashes[0];//assign this token id to 'member' level
				_addressToTid[airdropAddressesArray[i]] = i;//associate the token id with the owner address
			}
		}
	}
	...
	//@dev [GRNAULAR UPDATE] "Level up" the token id associated with `wallet` (must be `tid`) 
	...
	(
		...
		levelInBounds(level)
	{
		_tidToHash[_addressToTid[wallet]] = _levelHashes[level-1];
	}

	//@dev [BULK UPDATE] "Level up" multiple addresses at once to the same new level
	...
	(
		...
		levelInBounds(level)
	{
		if (addressArrayLength != addressesArray.length)
			revert DataError("length(`addressesArray)` != `addressArrayLength`");
		if (level < 1 || level > NUM_LEVELS)
			revert DataError("level must be between 1 and 4");
		...
		unchecked {
			...
			for (i = 0; i < addressArrayLength; i++) {
				//NOTE: this check has the potential to waste a lot of money on a txn
				// that fails due to one wallet that showed up to an event then 
				// transferred the NFT to someone else
				addressEye = addressesArray[i];
				if (!_exists(_addressToTid[addressEye]))
		...
		purchaseArgsOK(to, 1, msg.value)
	{
		unchecked {
			...
			_tidToHash[currentIdx] = _levelHashes[0];//assign this token id to 'member' level
			_addressToTid[to] = _currentIndex;//associate the token id with the owner address
		}
	}
