...
interface iSVGManip {
    ...
    function get_cell_entropy(uint256 tokenId, uint64 entropy) external view returns (uint256);
...
contract QTC2 is ERC721A, Ownable, ReentrancyGuard {
    ...
    function _make_json(uint256 tokenId, uint64 eukaryote, uint64 _type, uint64 generation, uint64 entropy) internal view returns (bytes memory) {
        ...
        output = SVGManip.add_trait(output, "Entropy", entropies[entropy]);
    ...
    function tokenURI(uint256 tokenId) override public view returns (string memory) {
        ...
        output.json = _make_json(tokenId, cell.eukaryote, cell._type, cell.generation, cell.entropy);
    ...
    function _breed_eukaryotes(uint256 tokenId1, uint256 tokenId2) external nonReentrant payable {
        ...
        if (dead_cell > 0) {
            ...
            uint256 rand_type = SVGManip.random(abi.encodePacked("CELL TYPE", _seed_phrase, tokenId1, tokenId2));
            uint256 rand_entropy = SVGManip.random(abi.encodePacked("CELL TYPE", _seed_phrase, tokenId1, tokenId2));
            if (new_type == 1) {
            ...
            if (new_entropy == 1) {
                ...
                new_cell.entropy = uint64(rand_entropy % 2 + 1);
            }
...
contract svgManip {
    ...
    function _get_cell_entropy(uint256 tokenId, uint64 entropy) internal view returns (uint256){
        if (entropy > 0) {
            return entropy;
        }
        // Give ~1/10 cells a dark core + nucleus
    ...
    function get_cell_entropy(uint256 tokenId, uint64 entropy) external view returns (uint256) {return _get_cell_entropy(tokenId, entropy);}
    ...
    function _add_first_gradient_stop(uint256 tokenId, uint64 entropy) view internal returns (bytes memory) {
        ...
        if (_get_cell_entropy(tokenId, entropy) == 2) {
            output = abi.encodePacked(output, '\n<animate repeatCount="indefinite" attributeName="stop-color" dur="',
    ...
    function _add_second_gradient_stop(uint256 tokenId, uint64 entropy) view internal returns (bytes memory) {
        ...
        } else if (_get_cell_entropy(tokenId, entropy) == 1) {
            output = abi.encodePacked(output, 
