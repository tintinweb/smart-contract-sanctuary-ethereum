...
contract Large is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;
    address payable public marketingAddress =
        payable(0x99C4ee0755CC4419ca0f53Fb1AB31436F6ea54Da); // Marketing Address
    address payable public fundAddress =
        payable(0xb984E33C48Ca98B73dD2BF51F9c4E65d1507F538); // Marketing Address
    ...
    string private _name = "Large Format";
    string private _symbol = "$LARGE";
    ...
    uint256 public _taxFee = 8; //fund+marketing+liquidity
    uint256 private _previousTaxFee = _taxFee;

    uint256 public _liquidityFee = 2;
    uint256 private _previousLiquidityFee = _liquidityFee;

    uint256 public _buyTaxFee = 8; //fund+marketing+liquidity
    uint256 public _buyLiquidityFee = 2;

    uint256 public _sellTaxFee = 8; //fund+marketing+liquidity
    ...
    uint256 public fundDivisor = 3;

    uint256 public _maxTxAmount = 40 * 10**6 * 10**9;
    uint256 public _maxBuyAmountForWallet = 10**6 * 10**9;
    uint256 private minimumTokensBeforeSwap = 0.02 * 10**6 * 10**9;
    ...
    function _transfer(
        ...
        uint256 totalFee = _liquidityFee.add(_taxFee);
        ...
        if (
            from != owner() &&
            from == uniswapV2Pair &&
            block.timestamp < (startSaleTime + 72 hours)
        ) {
            require(
                checkAmount < _maxBuyAmountForWallet,
                "Nobody can take more than maxWalletAmount in 72 hours."
            );
        }
    ...
    function swapTokens(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 amtForLiquidity = contractTokenBalance.div(100).mul(
            _liquidityFee
        );
        uint256 tokenForLiquidity = amtForLiquidity.div(2);

        swapTokensForEth(contractTokenBalance);
        uint256 transferredBalance = address(this).balance.sub(initialBalance);
        uint256 ethAmount = transferredBalance.div(1000).mul(_liquidityFee);
        addLiquidity(tokenForLiquidity, ethAmount);
        ...
        transferToAddressETH(
            fundAddress,
            transferredBalance.mul(fundDivisor).div(100)
        );
    }
    ...
    function setMaxWalletBuyAmount(uint256 maxBuyAmount) external onlyOwner {
        _maxBuyAmountForWallet = maxBuyAmount;
    }
    ...
    function setNumTokensSellForSwap(uint256 _minimumTokensBeforeSwap)
        external
        onlyOwner
    {
