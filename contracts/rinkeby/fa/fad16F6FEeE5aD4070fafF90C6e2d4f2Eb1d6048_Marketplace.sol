...
contract Marketplace is AccessControl, MarketplaceSignature {
    ...
    uint16 public constant maxPacksCount = 10;

    address payable public feeCollector;

    uint32 public constant hundredPercents = 100000;
    ...
    function setFeeCollector(address payable _feeCollector) public onlyRole(ADMIN) {
    ...
    function createMission(
        ...
        _createPacks(_missionId, _packIds, _packsNames, _packsSymbols, _packsUris);
    ...
    function _createPacks(
        ...
        for (uint8 i = 0; i < _packIds.length; ++i) {
            PackProxy packProxy = new PackProxy(address(packImplementation));
            PackNft(address(packProxy)).initialize(_names[i], _symbols[i], _uris[i], feeCollector);
    ...
    function _distributeErc20(
        ...
        if (_celebrity != address(0)) {
            ...
            feeCollectorFee = _value - _fundTreasuryShare;
        }
     ...
     * @param purchaseData struct with args for signing
...
contract MarketplaceSignature {
    ...
    bytes32 constant SIGNDATA_TYPEHASH =
        keccak256(
            'SignData(address buyer,uint32 missionId,address packAddress,uint128 seed,uint256[] availableIds,uint256 price,address celebrity,address missionTreasury,uint256 treasuryShare,uint256 feeCollectorShare,uint256 celebrityShare,uint32 quantity,address tokenAddress)'
        );
