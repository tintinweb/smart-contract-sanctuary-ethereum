...
import "./interfaces/IReusableFactory.sol";
import "./VZOO.sol";

/// @title VZOO Item Sale
/// @author VZOO
/// This factory contract
///     - mints 5 different options (1, 3, 9, 18, 36)
///     - implements reusable factory functionality
///     - allows to transfer collection ownership back to the owner
///     - allows to set a new nft contract address for future collections
///     - allows to set a new proxy address
contract VZOOFactory is FactoryERC721, ReusableFactory, Ownable {
    ...
    /// @dev Constants for this NFT factory.
    string private FACTORY_NAME = "VZOO Public Sale";
    string private FACTORY_SYMBOL = "VZOO";

    /// Can be changed after deployment.
    /// @dev Proxy registry address for a marketplace.
    address public proxyRegistryAddress =
        0x1E525EEAF261cA41b809884CBDE9DD9E1619573A;

    /// Can be changed after deployment.
    /// @dev Contract address of the currently active collection sale.
    address public nftAddress = 0x315e1dD28fe93b8543f56A05999D38CB1bA83733;

    /// Can be changed after deployment.
    /// @dev Base URI for token metadata.
    string public baseURI =
        "https://bafybeielduga6juelb7zrfk6o3eguakgtxwen3r2dksfjnuaqwexyke4iy.ipfs.nftstorage.link/contract-metadata/";

    /// @dev Five different options for minting (common, uncommon, rare, epic and legendary).
    ...
    /// @dev Set mint amount per option id.
    ...
    /// @dev Constructor for the ERC721 factory contract.
    constructor() {
    ...
    /// @dev Overrides check for factory interface support.
    /// @return True because this contract implements the factory interface.
    function supportsFactoryInterface() public pure override returns (bool) {
        return true;
    }

    /// @dev Returns the available number of minting options for this factory contract.
    /// @return Amount of available minting options.
    function numOptions() public view override returns (uint256) {
        return NUM_OPTIONS;
    }

    /// @dev Transfers ownership and fires events accordingly.
    ...
    /// @dev Transfers collection contract ownership back to this factory contract owner.
    function reclaimCollection() public onlyOwner {
        VZOO vzoo = VZOO(nftAddress);
        require(
            vzoo.supportsReclaimableCollectionInterface(),
            "collection must support reclaimable collection interface"
        );
        vzoo.transferOwnershipToFactoryOwner(address(this), owner());
    }

    /// @dev Transfer events for this factory contract.
    ...
    /// @dev External function to set a new nft address (for future use).
    /// @param _newNFTAddress Address of the new collection contract.
    function setNFTAddress(address _newNFTAddress) external onlyOwner {
        nftAddress = _newNFTAddress;
    }

    /// @dev Mints a new item (must be sent from the owner proxy or owner).
    /// @param _optionId id of option to mint
    ...
    function canMint(uint256 _optionId) public view override returns (bool) {
        ...
        VZOO vzoo = VZOO(nftAddress);
        uint256 vzooSupply = vzoo.totalSupply();
        ...
        return vzooSupply < (vzoo.maxSupply() - numItemsAllocated);
    }
    ...
    function canMintAndOwn(address _toAddress) public view returns (bool) {
        VZOO vzoo = VZOO(nftAddress);
        uint256 mintedBalance = vzoo.addressMintedBalance(_toAddress);
        /// @dev Minter is owner - check mint limit for team
        if (_toAddress == owner()) {
            require(
                mintedBalance + 1 <= vzoo.maxMintPerAddressTeam(),
                "mint limit for team exceeded"
            );
            return true;
        }
        /// @dev Minter is not owner - check allowed amount of mints and NFTs for address
        uint256 addressBalance = vzoo.balanceOf(_toAddress);
        require(
            mintedBalance + 1 <= vzoo.maxMintPerAddress(),
            "mint limit for address exceeded"
        );
        require(
            addressBalance + 1 <= vzoo.maxNFTPerAddress(),
    ...
    /// @param _newBaseURI to build the tokenURI.
    function setBaseURI(string memory _newBaseURI) public onlyOwner {
        baseURI = _newBaseURI;
    }

    /// Public function to receive the maximum amount of NFTs that can exist.
    /// @return Amount of NFTs that can exist.
    function nftMaxSupply() public view returns (uint256) {
        VZOO vzoo = VZOO(nftAddress);
        return vzoo.maxSupply();
    }

    /// @dev Returns encoded baseURI and _optionId (tokenURI) for an item.
    /// @param _optionId item id to get the URI for
    ...
    /// @param _operator Operator of the contract.
    /// @return Boolean true if owner or proxy address is operator and false if not.
    ...
    /// Use isApprovedForAll so the frontend doesn't have to worry about different method names.
    /// @dev Hack to get things to work automatically on OpenSea.
    /// @return _owner Wallet address of the NFT owner.
...
/**
    This is a generic contract for a reusable factory that can be used to transfer
    ownership of the associated collection contract back to the current owner of its contract.
    Collection contract must implement IReclaimableCollection interface.
 */
interface ReusableFactory {
    /**
        @dev Transfers collection contract ownership back to the factory contract owner.
     */
    function reclaimCollection() external;

    /**
        @dev Sets a new address for a reclaimable collection contract.
        @param _newNFTAddress New collection contract address for minting.
     */
...
contract VZOO is ERC721Tradable {
    /// @dev Constants for this NFT collection.
    string private COLLECTION_NAME = "VZOO Collection #1";
    string private COLLECTION_SYMBOL = "VZOO";
    ...
    string private _initBaseContractURI =
...
///     - has functionality to set new URIs for tokenURI and contractURI
    ...
    NativeMetaTransaction
{
    ...
    constructor(
        ...
        string memory _initBaseContractURI
    ) ERC721(_name, _symbol) {
        proxyRegistryAddress = _proxyRegistryAddress;
        setBaseURI(_initBaseURI);
        setContractURI(_initBaseContractURI);
