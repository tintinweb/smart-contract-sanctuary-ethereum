...
// File: contracts/UpcomingNFT.sol
...
contract UpcomingNFTPass is ERC721A, Ownable {
    using Strings for uint256;

    uint256 public MAX_SUPPLY = 2000;
    uint256 public FREE_SUPPLY = 200;
    uint256 public MINT_PER_WALLET = 5;
    uint256 public PUBLIC_MINT_PRICE = 0.09 ether;
    uint256 public isPaused = 0;
    
    string public tokenBaseUrl = "https://ipfs.io/ipfs/bafybeidz3gax4gto2z4jg5ocqtnwjvy5xlbhyppomdhbfzozw6rfqfpgma/assets/";
    string public tokenUrlSuffix = ".json";

    constructor () ERC721A("UpcomingNFTPass", "UNFT") {
    }
    ...
    function setisPaused(uint256 _ispaused) external onlyOwner {
        isPaused = _ispaused;
    }
    ...
    function celebMint(address _to, uint256 numTokens) external payable onlyOrigin celebmintCompliance(numTokens, _to) {
        if(FREE_SUPPLY > totalSupply()){
            require(msg.value == 0,"Value supplied is incorrect");
        }else{
            require(msg.value == PUBLIC_MINT_PRICE, "Value supplied is incorrect");
    ...
    modifier mintCompliance(uint256 _numTokens) {
        require(isPaused == 1, "Minting is paused.");
    ...
    modifier celebmintCompliance(uint256 _numTokens, address to) {
        require(isPaused == 1, "Minting is paused.");
        require(_numTokens > 0, "You must mint at least one token.");
        require(totalSupply() + _numTokens < MAX_SUPPLY, "Max supply exceeded!");
