...
contract ToxicSkullsClub is ERC721Enumerable, Ownable, ReentrancyGuard {
    ...
    //date variables
    uint256 public publicDate = 1649324629;
    uint256 public preDate = 1649324629;
    ...
    modifier checkPreDate() {
        ...
        require((preDate <= block.timestamp),"Pre is not yet!");
        _;
    }
    ...
    function mintPre(uint256 count, bytes32[] calldata _merkleProof) external payable checkLimit(count) checPrePrice(count) checkPreDate {
        ...
        require((claimed[_msgSender()] + count) > (maxMintPre + 1), "Address has no allowance!");
        ...
        claimed[_msgSender()] = claimed[_msgSender()] + count;
        delete totalSupply;
        delete leaf;
    }
    //only owner
