...
interface iSVGManip {
    ...
    function add_filters(string memory tokenId, bytes memory output, uint64 eukaryote, uint64 _type, uint64 generation, bool blur) view external returns (bytes memory);
    ...
    function get_scale_value(string memory tokenId, uint64 _type) external view returns (bytes memory);
    function breedProkaryote() external view returns (uint256);
    function breedEukaryotes() external view returns (uint256, uint256);
}
...
contract cellNFT is ERC721A, Ownable, ReentrancyGuard {
    ...
    string _seed_phrase;
    uint256 PRICE_PER_AA_SACRIFICE = 10000000 gwei; // 10000000 gwei == 0.01 ether
    uint256 PRICE_PER_PRO_REPRO = 1000000 gwei; // 1000000 gwei == 0.001 ether
    uint256 PRICE_PER_EU_REPRO = 10000000 gwei; // 10000000 == 0.01 ether
    ...
    constructor(address _AA_addy, address _iSVGManip_addy) ERC721A("cellNFT", "CL") {
        SVGManip_addy = _iSVGManip_addy;
        AA_CONTRACT_ADDY = _AA_addy;
        SVGManip = iSVGManip(SVGManip_addy);
        AANFT = iAANFT(AA_CONTRACT_ADDY);
        _seed_phrase = SVGManip.get_seed_phrase();
    }


    function _sacrifice_AA(uint256 AA_tokenId) internal {
        require(AANFT._ownerOf(AA_tokenId) == msg.sender, "don't own");
        require(AANFT._isAlive(AA_tokenId) == 1, "not alive");
        AANFT.sacrifice(AA_tokenId);
        _safeMint(msg.sender, 1);
        cells[TOKENS_MINTED] = CellData({eukaryote: uint64(AANFT._isMutant(AA_tokenId)), generation: 1, _type: 0});
        ++TOKENS_MINTED;
    }

    function _make_svg(string memory tokenId, uint64 eukaryote, uint64 _type, uint64 generation) internal view returns (bytes memory) {
        bytes memory output;
        ...
        // Nucleus
        if (eukaryote == 1) {output = SVGManip.add_nucleus(tokenId, output, _type);}
    ...
    function tokenURI(uint256 tokenId) override public view returns (string memory) {
        ...
        output.svg_data_bytes = _make_svg(tokenId_str, cell.eukaryote, cell._type, cell.generation);
    ...
    function _sacrifice_protein_for_cell(uint256 tokenId) external nonReentrant payable {
        require(MINT_ACTIVE, "not open");
        require(msg.sender == tx.origin);
        require(msg.value == PRICE_PER_AA_SACRIFICE);
        _sacrifice_AA(tokenId);
    }

    function _sacrifice_proteins_for_cells(uint256[] memory tokenIds) external nonReentrant payable {
        require(MINT_ACTIVE, "not open");
        require(msg.sender == tx.origin);
        require(msg.value == PRICE_PER_AA_SACRIFICE * tokenIds.length);
        for (uint256 i = 0; i < tokenIds.length; ++i) {
            _sacrifice_AA(tokenIds[i]);
        }
    }

    function _breed_prokaryotes(uint256[] memory tokenIds) external payable {
        require(msg.sender == tx.origin);
        ...
        while (repro.cycles < tokenIds.length) {
            ...
            uint256 repro_outcome = SVGManip.breedProkaryote();
            if (repro_outcome == 1) {
    ...
    function _breed_eukaryotes(uint256 tokenId1, uint256 tokenId2) external payable {
        require(msg.sender == tx.origin);
        ...
        if (dead_cell > 0) {
            ...
            uint256 rand = SVGManip.random(abi.encodePacked("CELL TYPE", _seed_phrase, Strings.toString(tokenId1), Strings.toString(tokenId2)));
            if (new_type == 1) {
    ...
    function z_updateSacrificePrice(uint256 _PRICE_PER_AA_SACRIFICE) external onlyOwner {PRICE_PER_AA_SACRIFICE = _PRICE_PER_AA_SACRIFICE;}
    function z_updateProReproPrice(uint256 _PRICE_PER_PRO_REPRO) external onlyOwner {PRICE_PER_PRO_REPRO = _PRICE_PER_PRO_REPRO;}
    function z_updateEuReproPrice(uint256 _PRICE_PER_EU_REPRO) external onlyOwner {PRICE_PER_EU_REPRO = _PRICE_PER_EU_REPRO;}
...
contract svgManip {
    ...
    function get_rand_rgb(uint256 rgb_idx, string memory prefix, string memory tokenId, uint256 min, uint256 range) view internal returns (string memory) {
        if (rgb_idx == 0) {
    ...
    function get_values_and_dur_for_animate(string memory tokenId, string memory prefix, AnimateParams memory a) internal view returns (bytes memory) {
        // Get a random start point, end point and duration for the animation given constraints for each in the AnimateParams object
        ...
        return abi.encodePacked('\n<animate repeatCount="indefinite" attributeName="', a.attribute, '" values="', 
            ...
            a.value_prefix1, _get_rand_in_range_toStr(_random(abi.encodePacked(prefix, a.attribute, _seed_phrase, "1 One 1", tokenId)), a.start_min, a.start_range), 
            '" dur="', _get_rand_in_range_toStr(_random(abi.encodePacked(prefix, a.attribute, _seed_phrase, "DURATION", tokenId)), a.dur_min, a.dur_range), 's"/>'
        );
    }
    ...
    function _add_filters(string memory tokenId, bytes memory output, uint64 eukaryote, uint64 _type, uint64 generation, bool blur) view internal returns (bytes memory) {
    ...
    function get_cell_core_color(string memory tokenId) external view returns (uint256){return _get_cell_core_color(tokenId);}
    ...
    function get_scale_value(string memory tokenId, uint64 _type) external view returns (bytes memory) {return _get_scale_value(tokenId, _type);}
    ...
    function breedProkaryote() external view returns (uint256) {
        // Breed prokaryotes has a 1% chance of mutation to a eukaryote and a 10% chance of death
        uint256 rand = _random(abi.encodePacked(Strings.toString(block.timestamp), "BREED CELLS", _seed_phrase)) % 100;
    ...
    function breedEukaryotes() external view returns (uint256, uint256) {
        // Breeding eukaryotes has a 10% chance of a random cell dying
        ...
        uint256 rand = _random(
            abi.encodePacked("BREED EUKARYOTE CELLS", _seed_phrase, Strings.toString(block.timestamp))
        ) % 100;
        if (rand < 10) { // death - rand == 0, 10% chance
            rand = _random(abi.encodePacked("CELL DAEATH", _seed_phrase, Strings.toString(block.timestamp))) % 2;
            if (rand < 5) {
                // cell 1 dies
                return (1, 0);
            } else {
        ...
        // birth - `rand` IN (8, 9), 20% chance
        } else if (rand > 70) { 
            // Inherit a parent's type - rand == 8, 10% chance
            if (rand < 80) {
                return (0, 1);
            // mutatation to new type - rand == 9, 10% chance
            } else { 
                return (0, 2);
            }
        // safe failure - no baby but no death, 70% chance
        } else { 
            return (0, 0);
        }
    }
