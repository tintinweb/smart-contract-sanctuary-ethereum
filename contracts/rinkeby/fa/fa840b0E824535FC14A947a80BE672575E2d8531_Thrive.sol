...
contract Thrive is ERC20Burnable, Operator {
    using SafeMath8 for uint8;
    using SafeMath for uint256;

    // Initial distribution for the genesis pools
    ...
    uint256[] public taxTiersRates = [100, 100, 100, 100, 100, 100, 100, 100, 100, 100, 100, 100, 100, 100];
    ...
    /**
     * @notice Constructs the THRIVE ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("THRIVE", "THRIVE") {
        // Mints 1 THRIVE to contract creator for initial pool setup
    ...
    function _getThrivePrice() internal view returns (uint256 _thrivePrice) {
        try IOracle(thriveOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Thrive: failed to fetch THRIVE price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _thrivePrice) internal returns (uint256){
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_thrivePrice >= taxTiersTwaps[tierId]) {
                    require(taxTiersRates[tierId] < 500, "tax equal or bigger to 5%");
    ...
    function setThriveOracle(address _thriveOracle) public onlyOperatorOrTaxOffice {
        require(_thriveOracle != address(0), "oracle address cannot be 0 address");
        thriveOracle = _thriveOracle;
    }
    ...
    /**
     * @notice Operator mints THRIVE to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of THRIVE to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentThrivePrice = _getThrivePrice();
            currentTaxRate = _updateTaxRate(currentThrivePrice);
            if (currentThrivePrice < burnThreshold) {
                _burnTax = true;
            }
        }
