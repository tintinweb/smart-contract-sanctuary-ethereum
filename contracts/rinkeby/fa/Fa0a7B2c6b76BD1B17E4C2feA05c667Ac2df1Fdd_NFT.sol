...
contract NFT is ERC721A, Ownable, ReentrancyGuard {
    ...
    uint256 private _allowListSaleStart = FAR_FUTURE;
    uint256 private _publicSaleStart = FAR_FUTURE;
    uint256 private _auctionMaxSupply = 5000;
    uint256 private _ownerReserve = 200;
    uint256 private _salePrice = 0.02 ether;

    address private _verifier;
    string private _baseTokenURI;
    mapping(address => bool) private _mintedAllowList;
    ...
    modifier walletIsNotContract() {
        if (tx.origin != msg.sender)
            revert OnlyExternallyOwnedAccountsAllowed();
        _;
    }

    constructor(address verifier) ERC721A("Peaceful Pizzas", "PPZZA") {
        _verifier = verifier;
    }
    ...
    function isAllowListActive() public view returns (bool) {
        return block.timestamp > _allowListSaleStart;
    }
    ...
    function allowListMint(bytes calldata sig)
        ...
        walletIsNotContract
    {
        if (!isAllowListActive()) revert SaleNotStarted();
        if (!isAllowlisted(msg.sender, sig)) revert AccountNotAllowlisted();
        ...
        _mintedAllowList[msg.sender] = true;
        _safeMint(msg.sender, 1);
    }

    function hasMintedAllowList(address account) public view returns (bool) {
        return _mintedAllowList[account];
    }

    function isAllowlisted(address account, bytes calldata sig)
        internal
        view
        returns (bool)
    {
    ...
    function setAuctionMaxSupply(uint256 auctionMaxSupply) external onlyOwner {
        if (_ownerReserve + auctionMaxSupply > MAX_SUPPLY)
            revert AmountExceedsSupply();
    ...
    function startAuction() external onlyOwner {
        if (isAllowListActive() || isPublicSaleActive())
    ...
    function startAllowListSale(uint256 price) external onlyOwner {
        ...
        _allowListSaleStart = block.timestamp;
        _salePrice = price;

        emit AllowListSaleStart(price, MAX_SUPPLY - totalSupply());
    }

    function startPublicSale() external onlyOwner {
        if (isAuctionActive() || isAllowListActive()) revert SaleInProgress();
    ...
    function pauseSale() external onlyOwner {
        ...
        _allowListSaleStart = FAR_FUTURE;
    ...
    function reserve(uint256 quantity) external onlyOwner {
        if (quantity > _ownerReserve || totalSupply() + quantity > MAX_SUPPLY) {
            revert AmountExceedsSupply();
        }
