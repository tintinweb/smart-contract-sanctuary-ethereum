...
contract C3 is ERC721("C3", "Component 3"), IERC2981, Ownable, nonReentrant {
    ...
    function claimC3(bytes calldata _signature, uint256 _amount) external {
        if (!saleIsActive) revert SaleNotActive();
        if (_amount == 0) revert InvalidAmount();
        if (hasClaimed[msg.sender]) revert ExceedsAllowance();

        bytes32 hash = keccak256(abi.encodePacked(msg.sender, _amount));
        if (signer != hash.toEthSignedMessageHash().recover(_signature))
            revert Unauthorized();
