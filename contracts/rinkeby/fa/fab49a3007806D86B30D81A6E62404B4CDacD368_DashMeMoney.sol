...
contract DashMeMoney {
    ...
    function sendMoney() public payable returns (bool) {
        require(fundMeetThreshold(msg.value), "Omo shey you dey craze,");
        addToMap();
        return true;
    }

    function fundMeetThreshold(uint256 ethToDeposit) public returns (bool) {
        return convertToGwei(ethToDeposit) >= convertToETH(miniumumDepositUSD);
    }

    function convertToETH(uint256 usd) public returns (uint256) {
        return usd * getLatestPrice();
    }

    function convertToGwei(uint256 ethers) public pure returns (uint256) {
        return ethers * 1**8;
    }
