...
contract PNFTTest3 is ERC721A , Ownable{  
    constructor() ERC721A("PNFTTest3", "PNFT3") {}  
	...
	function getTotalFeedFactor(uint256 tokenId) public view returns(uint256) {
		require(_exists(tokenId), "Token does not exist");
	...
	function timeUntilNextAge(uint256 tokenId) public view returns(uint256){
		...
		uint256 timeElapsed = block.timestamp - _feeds[tokenId].timeIn;
	...
	function getEligibleForBreeding() public view returns (uint256[] memory) {
		uint256[] memory nftIds;
		uint256 nftCount = 0;
		
		for (uint256 i = 0; i < _publicPNFT.current(); i++) {
			if (_burnToAdults[i] == true || ((calcAge(i) == MAX_AGE) && (timeSinceAdult(i) > BREEDING_DELAY))) {
				nftIds[nftCount] = i;
				nftCount++;
			}
		}
