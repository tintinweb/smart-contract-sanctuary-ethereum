...
contract Token is ERC721A, Pausable, AccessControl {
    ...
    event ChangedBaseURI(string _baseURI);
    ...
    function mint(address _owner, uint256 _amount) external onlyRole(MINTER_ROLE) {
        require(_amount <= maxTokens, "max amount exceeded");
        require(totalSupply() + _amount <= totalTokens - tokenReserve, "no supply left");
        _safeMint(_owner, _amount);

    }

    function adminMint(address _owner, uint256 _amount) external onlyRole(DEFAULT_ADMIN_ROLE) {
        require(totalSupply() + _amount <= totalTokens, "no supply left");
