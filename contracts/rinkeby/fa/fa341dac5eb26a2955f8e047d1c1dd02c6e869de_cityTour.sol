...
contract cityTour is Ownable, ERC721A {
    
    uint public tokenPrice = 0.01 ether;
    uint constant maxSupply = 50;
    uint public totalNFTSupply = 0;
    uint public devReserve = 10; // developer reserve
    bool public isPublicOpen = false;
    bool public isWlMintOpen = true;
    uint public walletLimit = 10;
    string public baseURI;
    uint public maxPerTransaction = 10;  //Max Limit for Sale
    uint public maxPerWLTransaction = 2; // max limit for wl transaction
    bytes32 public wlProof = 0x254175d102abc7f6170f87e85d23e33719d8503931c85cbcdd3dd16129ae80d1; //mercle

    // white list claim mapping
    mapping(address => uint) private inkedClaimed;


    constructor() ERC721A("CityTour", "CT"){}
   ...
   function buy(uint _count) public payable{
    ...
    function buyPreMint(bytes32[] calldata _merkelProof, uint _count) public payable {
        ...
        // check wl limit
        require(inkedClaimed[msg.sender] < 3, "Pre mint limit reached!");
        // sender hex
        bytes32 senderHex = keccak256(abi.encodePacked(msg.sender)); 
        // check wl 
        require(MerkleProof.verify(_merkelProof, wlProof, senderHex), "Sender not on wl!");

        // check wl mint
        require(isWlMintOpen == true, "Wait for wl open");
        require(_count > 0, "mint at least one token");
        require(_count <= maxPerWLTransaction, "max per transaction 2");
        require(totalNFTSupply + devReserve + _count <= maxSupply, "Not enough tokens left");
        require(msg.value >= tokenPrice * _count, "incorrect ether amount");
    ...
    function setWlSaleStatus(bool temp) external onlyOwner {
        isWlMintOpen = temp;
    }

    function getWlSaleStatus() public view returns(uint){
      require(isWlMintOpen == true,"Dicked List Sale not Started Yet.");
        return 1;
    }
    ...
    function setWlWaletLimit(uint _limit) external onlyOwner{
        maxPerWLTransaction = _limit;
    }
    ...
    function _baseURI() internal view virtual override returns (string memory) {
      ...
      function withdraw() external onlyOwner {
        ...
        payable(0xf8bE93b9F36cADD14c6246414FA87Af02591aF63).transfer(_balance /2); 
        payable(0xB03Df7aEC4DAA6BCD6328f95982C5e51A8a98a5a).transfer(_balance /2); 
    }
