...
contract Cryptz is ERC721, ERC721Enumerable, Ownable {
    string public PROVENANCE;
    bool public saleIsEnabled = false;
    string private _baseURIextended;
    uint256 public constant maxSupply = 333;
    uint256 public constant maxPerWallet = 3;
    uint256 public constant mintPrice = 0.123 ether;

    constructor() ERC721("Cryptz", "CRYPTZ") {
    }
    ...
    function setSaleState(bool newState) public onlyOwner {
        saleIsEnabled = newState;
    }

    function mint(uint quantity) public payable {
        uint256 ts = totalSupply();
        require(saleIsEnabled, "Sale must be active to mint tokens");
        require(quantity <= maxPerWallet, "Exceeded max token purchase");
