...
contract Test is ERC721A, Ownable, ReentrancyGuard {
    ...
    modifier maxMintsPerTX(uint256 _mintAmount) {
        require(
            _mintAmount <= maxMintsPerTx,
            "Max mints per transaction exceeded"
        );
        _;
    }
    modifier canMintNFTs(uint256 _mintAmount) {
        require(
    ...
    function mint(uint256 _mintAmount) public payable 
        ...
        maxMintsPerTX(_mintAmount)
    {
		uint256 s = totalSupply();
        uint256 addressFreeMintedCount = addressFreeMintedBalance[msg.sender];
        require(addressFreeMintedCount + _mintAmount <= maxMintsPerTx, "max NFT per address exceeded");
