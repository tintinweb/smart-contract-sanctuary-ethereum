...
contract MATICALPHA {
	...
	uint256 constant public DEVELOPER_FEE = 5;
    	uint256 constant public commission_FEE = 5;
	...
	function _invest(address payable referrer, uint8 _plan, address payable sender, uint256 value) private {
		...
		if (user.referrer != address(0)) {
			...
			if (upline != address(0)) {	
                ...
                if(user.referrals<11){
                    REFERRAL_PERCENTS = 200;
                }else if(user.referrals<31){
                    REFERRAL_PERCENTS = 300;
                }else if(user.referrals<51){
                    REFERRAL_PERCENTS = 500;
                }else if(user.referrals<71){
                    REFERRAL_PERCENTS = 700;
                }else if(user.referrals<81){
                    REFERRAL_PERCENTS = 1000;
                }else if(user.referrals>80){
		...
		user.deposits.push(Deposit(_plan, percent, value, profit, block.timestamp, finish,s,h,true));
	...
	function _reinvest( uint8 _plan, address payable sender, uint256 value) private {
        ...
        uint256 reinvestmentbonus = value.add(plans[_plan].reinvestmentbonus.div(10000).mul(value));
	...
	function withdrawall(uint _type) public {
		...
		for(uint8 i=0;i<users[msg.sender].deposits.length;i++){
			if(_type==1){
				_sbalance += users[msg.sender].deposits[i].stackedbalance;
				users[msg.sender].deposits[i].stackedbalance = 0;
			}
			if(users[msg.sender].deposits[i].elegible){
				if(_type==2){
					_hbalance += users[msg.sender].deposits[i].holdbalance;
	...
	function getResult(uint8 plan, uint256 deposit) public view returns (uint256 percent, uint256 profit, uint256 finish) {
	    ...
	    profit = deposit.mul(percent).mul(plans[plan].time);
