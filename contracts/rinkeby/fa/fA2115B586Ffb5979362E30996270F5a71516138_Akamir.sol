...
contract Akamir is Ownable, ERC721A, ReentrancyGuard {
  ...
  function auctionMint(uint256 quantity, uint256 lockUntil) external payable callerIsUser {
    ...
    console.log("auction sale price: ",price);
    console.log("auction sale discount : ", getDiscountPrice(price, lockUntil) );
    console.log("auction sale price - discount price : ",price - getDiscountPrice(price, lockUntil) );
    console.log("auction sale totalCost : ",totalCost);
  ...
  function allowlistMint(uint256 quantity, uint256 lockUntil) external payable callerIsUser {
    ...
    console.log("allow sale price: ",price);
    console.log("allow sale discount : ", getDiscountPrice(price, lockUntil));
    console.log("allow sale price - discount price : ",price - getDiscountPrice(price, lockUntil));
    console.log("allow sale totalCost : ",totalCost);
  ...
  function publicSaleMint(
    ...
    require(
      isPublicSaleOn(publicPrice, publicSaleKey, publicSaleStartTime, saleLimit),
    ...
    require(totalSupply() + quantity <= saleLimit, "can not mint over sale limit");
     ...
     console.log("public sale price: ",publicPrice);
      console.log("public sale discount : ", getDiscountPrice(publicPrice, lockUntil));
      console.log("public sale price - discount price : ",publicPrice - getDiscountPrice(publicPrice, lockUntil));
      console.log("public sale totalCost : ",totalCost);
  ...
  function isPublicSaleOn(
    ...
    return publicPriceWei != 0 && publicSaleKey != 0 && saleLimit != 0 && block.timestamp >= publicSaleStartTime;
  }
  ...
  function getDiscountPrice(uint256 price,uint256 lockUntil) public view returns (uint256) {
    ...
    do{
      ...
      uint32 monthInterval =  ONE_MONTH *(saleConfig.discountInterval * i);
        if(lockUntil == block.timestamp +  monthInterval ||
        (lockUntil < block.timestamp +  monthInterval + 1 minutes &&
        lockUntil > block.timestamp +  monthInterval- 1 minutes)) break;
    } while(true);
}
    // require(lockUntil == block.timestamp + DISCOUNT_INTERVAL , "lockuntil must bigger than seted min month" );
    ...
    // require(_discountList[discountIndex] > 0, "discount error2");
  ...
  function allowSlot() public view returns (uint256) {
      // require((allowlist[msg.sender] == 0), "does not exist allowlist");    
      for(uint256 i; i< allowers.length;  i++ ) {
        if(allowers[i] == msg.sender)return allowlist[msg.sender];
      }
      revert ("caller does not have allow");
  }
...
contract ERC721A is Context, ERC165, IERC721, IERC721Metadata, IERC721Enumerable {
   ...
   *TODO lockup 기능 추가
   */
  uint256 TWO_YEAR = 60 * 60 * 24 * 365 * 2;
  ...
  function lock(uint256 tokenId, uint256 lockUntil) public {
    ...
    if (_lockUntil[tokenId] != 0 && lockUntil - _lockUntil[tokenId] > TWO_YEAR) revert LockNotUpperThanOneYear();
    ...
    unchecked {
      for (uint256 i; i < tokenIds.length; i++) {
        ...
        if (_lockUntil[tokenIds[i]] != 0 && lockUntil - _lockUntil[tokenIds[i]] > TWO_YEAR)
          revert LockNotUpperThanOneYear();
      }
