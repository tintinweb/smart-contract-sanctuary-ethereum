...
contract Demo is ERC721A, ReentrancyGuard {
    ...
    address payable private immutable developerWallet;
    ...
    constructor(string memory _baseURL, string memory _unrevealedURL, address _developerWallet) ERC721A("Demo", "YDLSDEMO"){
       owner = payable(msg.sender); //payable(0x5153B2aB0D6E31EB2bA25c4676758922fA68C753);
       baseURL = _baseURL;
    ...
    /* Public Functions */
    function mint(uint256 _mintAmount) external payable nonReentrant mintRequirements(_mintAmount) {

        if(saleState == SaleState.OG_SALE){
            require(_mintAmount <= ogMaxPerWallet, "You can't mint that many");
            require(amountMinted[msg.sender] < ogMaxPerWallet, "Mint amount exceeded");
        }
            ...
            require(_mintAmount <= publicMaxPerWallet, "You can't mint that many!");
            require(amountMinted[msg.sender] < publicMaxPerWallet, "Mint amount exceeded");
        }
        else if(saleState == SaleState.PUBLIC_SALE && totalSupply() > 4444){
            require(msg.value >= mintPrice * _mintAmount, "You did not send enough ETH");
            require(_mintAmount <= maxPerWalletAfter4444, "You can't mint that many!");
            require(amountMinted[msg.sender] < maxPerWalletAfter4444, "Mint amount exceeded");
        }
    ...
    function withdrawFunds() public payable onlyOwner nonReentrant {
        ...
        (bool withdrawOwner,) = owner.call{value: contractBalance * 95 / 100}("");
        /* Pay developers 5% */
        (bool withdrawDeveloper,) = developerWallet.call{value: contractBalance * 5 / 100}("");
