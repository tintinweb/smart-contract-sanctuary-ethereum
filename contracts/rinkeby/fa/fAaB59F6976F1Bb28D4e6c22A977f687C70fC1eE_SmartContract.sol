...
https://rinkeby.etherscan.io/address/0xa93282ff10DF8339c844a472cf5e6cB77Be02493#code
1. autoliquidty is working no autocore
1 wallet for deployer and  autocore
1 wallet for sendEth
if burnoff is working
else if half send to deployer is working half is used to buy, burn core token 
*/
...
contract SmartContract is Context, IERC20, Ownable {
    ...
    string private constant _symbol = "MSTaug28v5";
    ...
    uint256 public totalFee = autoLiquidityRatio + autoBurnFeeRatio + autoRewardFeeRatio + autoBurnFeeRatio;
    ...
    constructor () {
        ...
        feeAddr2Wallet = payable(0x944C9B1d3A459728172A880C4Ca0ceC4257Ba5b5);  //autocorebuybackandburnwallet
        ...
        totalFee = autoLiquidityRatio + autoBurnFeeRatio + autoRewardFeeRatio + autoBurnFeeRatio;
      ...
      if (from == uniswapV2Pair && to != address(uniswapV2Router) && ! _isExcludedFromMaxWalletSize[to]) {             
        ...
        uint256 contractTokenBalance = balanceOf(address(this));
            if (!inSwap && from != uniswapV2Pair && swapEnabled && contractTokenBalance>0) {
                ...
                uint256 tokensToAddLiquidityWith = (contractTokenBalance * autoLiquidityRatio) / (totalFee);
                uint256 toSwapForEth = contractTokenBalance - tokensToAddLiquidityWith; 
                uint256 tokensForAutoCore = (contractTokenBalance * autoBurnCoreRatio) / (totalFee);
                uint256 ethValuePerToken = contractETHBalance / toSwapForEth;
                uint256 ethForAutoCore = ethValuePerToken * tokensForAutoCore; 
                uint256 ethForBuyBurn = contractETHBalance - ethForAutoCore;
                uint256 ethFroDeployer = contractETHBalance - ethForBuyBurn;
                ...
                } else if (burnMode && (ethForBuyBurn > buyCoreUpperLimit)) {
                    sendETHToFee(ethFroDeployer);
                    uint256 buyAmount = ethForBuyBurn;
                    buyCore(buyAmount);                 
                } else {
                    sendETHToFee(address(this).balance);
                } 
    ...
    function swapAndLiquify(uint256 tokenAmount) private lockTheSwap {
        ...
        /** 
        uint256 tokensForAutoCore = (tokenAmount * autoBurnCoreRatio) / (totalFee);
        uint256 ethForAutoCore = ethValuePerToken * tokensForAutoCore; 
        if  (ethForAutoCore > 0) {
            payable(feeAddr2Wallet).transfer(ethForAutoCore);
        } 
        */
