...
contract Token is ERC721, Ownable, Pausable {
    ...
    string private _metadataBaseURI = ""; //todo
    bool private _baseURILocked = false;

    string public PROVENANCE = "";

    // Default to 01/01/2029 - the year when the T-800 Terminator is sent back
    // in time.
    uint256 private  _mintStartTime = 1861948800;
    ...
    function setBaseURI(string memory baseURI) public onlyOwner {
        require(!_baseURILocked, "BASE_URI_LOCKED");
    ...
    function isMintingActive() public view returns (bool){
        return block.timestamp >= _mintStartTime;
    }

    // Todo add Merkel Proof for whitelisting
    function mint() public whenNotPaused returns (uint256) {
        require (isMintingActive(), "MINTING_NOT_ACTIVE");

        // Start with ID 1, so increment here.
