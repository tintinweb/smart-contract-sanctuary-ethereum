...
contract NFTContract is ERC721, Ownable, PullPayment {
  ...
  uint256 public constant payPrice = 0.1 ether;
  ...
  uint256 private freeTokenId = 2000;

  constructor() ERC721("NFTContract", "NFT") {}

  /// @dev One address can mint 5 tokens free, pay according to mood
  function freeMint(uint256 amount, address recipient) public payable {
    require(freeMintCount[recipient] + amount <= maxFreeMint, "One address only can mint 5 tokens");

    require(freeTokenId + amount <= total, "Max supply free reached");
