...
contract PolicyBookFacade is IPolicyBookFacade, AbstractDependant, Initializable {
    ...
    /// @notice get utilization rate of the pool on chain
    function getUtilizationRatioPercentage(bool withLeverage)
        external
        view
        override
        returns (uint256 utilizationRatioPercentage)
    {
        (uint256 newTotalCoverTokens, uint256 newTotalLiquidity) =
            policyBook.getNewCoverAndLiquidity();

        if (withLeverage) {
            utilizationRatioPercentage = newTotalCoverTokens.mul(PERCENTAGE_100).div(
                newTotalLiquidity.add(totalLeveragedLiquidity)
            );
        } else {
            utilizationRatioPercentage = newTotalCoverTokens.mul(PERCENTAGE_100).div(
                newTotalLiquidity
