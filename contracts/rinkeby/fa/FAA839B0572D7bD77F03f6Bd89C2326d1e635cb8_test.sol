...
contract test is Ownable, ERC721A, ReentrancyGuard {
    ...
    uint256 public MAX_PER_Transaction_WL = 5; // maximum amount that user can mint per transaction for whitelisted members
    ...
    uint256 public freezeTill= 20; //tokenId till which team will mint
    
    uint256 public season = 1; //tokenId till which team will mint


    
    uint256 private constant TotalCollectionSize_ = 10; // total number of nfts
    ...
    bool public roundThree = false;
    bool public roundFour = false;
    bool public roundFive = false;
    ...
    address[] teamAddy=[0x8D25C539557196049C6097d21c05aBd9b1cA6d6B]; // store as an array

    event burned(address indexed burner);
    event roundState(bool state);
    event roundsState(uint256 roundNumber,bool state);
...
///////////////////////////
        ...
        isCorrectPayment(whiteListCost, quantity)
    {
        ...
        require(supply + quantity <= TotalCollectionSize_, "max NFT limit exceeded");
        require(numberMinted(msg.sender) + quantity <= MAX_PER_Wallet_WL, "limit per wallet exceeded");
        require(quantity <= MAX_PER_Transaction_WL, "can not mint this many");
        _safeMint(msg.sender, quantity);
    }
    ...
    function mintTeam(bytes32[] calldata merkleProof)
        ...
        isCorrectPayment(whiteListCost, 5)
    {
        require(!paused, "mint is paused");
        uint256 supply = totalSupply();
        require(supply + 5 <= TotalCollectionSize_, "max NFT limit exceeded");
    ...
    function _shouldPay(uint256 _quantity) 
        ...
        returns(uint256)
    {
        uint256  shouldPay=cost*_quantity;
        return shouldPay;
    }
    ...
    function setRoundState(bool _state,uint256 roundNum) public onlyOwner {
        ...
        emit roundsState(roundNum,_state);
    }
