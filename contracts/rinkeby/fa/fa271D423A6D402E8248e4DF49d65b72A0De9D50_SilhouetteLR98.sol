    ...
    ReentrancyGuard
{
    ...
    uint256[2] public maxSupply = [750, 750];
    address public adminMinter;
    address public royaltyAddress;
    uint256 public royaltyShare10000;

    IERC721AQueryable private runLolaRun;
    ...
    constructor(
        ...
        runLolaRun = IERC721AQueryable(
            0x36759B8EA98Cdd42093F0304fc81a4b49A193322
        );
        transferOwnership(_owner);
        adminMinter = _adminMinter;
        royaltyAddress = 0xa6160180dF5D140C2622cdAD07b1FD29DE307958;
    ...
    function setClaimActive(bool value) external onlyOwner {
        isClaimActive = value;
    }

    function claim() external claimActive nonReentrant {
        uint256[] memory tokenIds = runLolaRun.tokensOfOwnerIn(
            msg.sender,
            0,
            751
        );
        ...
        for (uint256 i = 0; i < tokenIds.length;) {
            if (!isClaimed(tokenIds[i])) {
                unchecked{ claimAmount++; }
                _markClaimed(tokenIds[i]);
            }
            unchecked{ i++; }
        }
        require(claimAmount > 0, "No valid MovieShots tokens!");
        require(
            (totalSupply(0) + claimAmount) <= maxSupply[0],
            "Cannot mint over supply cap of token!"
        );
        _mint(msg.sender, 0, claimAmount, new bytes(0));
        require(
            (totalSupply(1) + claimAmount) <= maxSupply[1],
            "Cannot mint over supply cap of token!"
        );
        _mint(msg.sender, 1, claimAmount, new bytes(0));
    }

    function getClaimCount(address account) external view returns (uint256) {
        uint256[] memory tokenIds = runLolaRun.tokensOfOwnerIn(account, 0, 751);
        uint256 claimAmount = 0;
    ...
    function _markClaimed(uint256 tokenId) private {
    ...
    function adminMint(
        ...
        require(id < 2, "Nonexistent token");
        require(amount > 0, "Minting quantity must be over 0");
        require(
    ...
    function decreaseMaxSupply(
        uint256 decreaseByAmount1,
        uint256 decreaseByAmount2
    ) public onlyOwner {
        require(
            (maxSupply[0] - decreaseByAmount1) >= totalSupply(0),
            "Amount cannot be decreased as it has already been minted"
        );
        require(
            (maxSupply[1] - decreaseByAmount2) >= totalSupply(1),
            "Amount cannot be decreased as it has already been minted"
        );

        maxSupply[0] = maxSupply[0] - decreaseByAmount1;
        maxSupply[1] = maxSupply[1] - decreaseByAmount2;
    }

    function name() public pure returns (string memory) {
        return "MovieShots - LR98 Silhouette";
    }

    function symbol() public pure returns (string memory) {
        return "LR98-SILHOUETTE";
    }
