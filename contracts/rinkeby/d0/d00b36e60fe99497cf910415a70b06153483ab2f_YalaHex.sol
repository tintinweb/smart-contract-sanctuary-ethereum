...
/*
    ...
    ||===;        \  \       ╚═╝   ╚═╝  ╚═╝╚══════╝╚═╝  ╚═╝╚═╝  ╚═╝╚══════╝╚═╝  ╚═╝       ╚═╝╚══════╝       ╚═╝  ╚═╝╚══════╝╚═╝  ╚═╝   ╚═╝   
    ||===;        \  \  
    ||   |         | |    1- HEXY is a contract for trustlessly pooling 14 hex stakes laddered yearly with 5.555% of total HEX pledged and one final stake with 22.23% for the 15th year.
    || .-\ '     _/_/     2- Anyone may choose to mint 1 HEXY per HEX deposited into the HEXY Contract Address during the minting phase.
    |:'  _;.    (_  \     3- Anyone may choose to pay for the gas to start and end the stake on behalf of the HEXY Contract.
    /  .'  `;\   \\_/     4- Anyone may choose to pay for the gas to mint Hedron the stake earns on behalf of the HEXY Contract.
   |_ /     |||  |\\      5- HEXY is a standard ERC20 token, minted upon HEX deposit and burnt upon HEX redemption, with 1 HEXY airdrop to each HEX holder on launch date,
  /  _)=====|||  | ||          and an extra 5.555% of total HEX pledged to pay for new HEX holders airdrop, development, transaction fees, audit, and marketing.
 /  /|      ||/  / //     6- HEXY holders may choose to burn HEXY to redeem HEX principal and yield (Including HEDRON) pro-rata from the HEXY Contract Address after the minting phase.
 \_/||      ( `-/ ||
    ||======/  /  \\ .-.
    ||      \_/    \'-'/
    ||      ||      `"`
...
contract YalaHex is ERC20, ERC20Burnable, ReentrancyGuard, Ownable {
    ...
    constructor(uint256 mint_duration, uint256 stake_duration, uint256 mintAmount)ERC20("YalaHex", "HEXY")ReentrancyGuard() {
    ...
    address constant HEX_ADDRESS = 0x78a52239383f4B772781276b3De36086A5a2169e; // "2b, 5 9 1e? that is the question..."
    address constant HEDRON_ADDRESS = 0x523aDE599F37A4799b31B7FB5Fe41251B92b04c4;
    ...
    function redeemHEX(uint256 amount_HEXY)nonReentrant external {
        require(HAS_STAKE_STARTED == false || HAS_STAKE_ENDED == true || hex_token.currentDay() > STAKE_START_DAY + 365, "Redemption can only happen before stake starts or after stake ends.");
        uint256 yourHEXY = balanceOf(msg.sender);
        require(yourHEXY >= amount_HEXY, "You do not have that much HEXY.");
