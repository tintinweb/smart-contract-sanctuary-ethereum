...
contract SpaceCoinIco {
    ...
    uint256 private constant SEED_PHASE_CONTRIBUTION_LIMIT = 1500 ether;
    uint256 private constant GENERAL_PHASE_CONTRIBUTION_LIMIT = 1000 ether;
    ...
    error MustBeAllowed(address, string);
    error MustBeSeedPhase();
    error MustBeOpenPhase();
    error MustBeNotOpenPhase();
    error MustBeUnderGoal(uint256, uint256);
    error MustBeUnderLimit(uint256, uint256);
    error MustBeGreaterThanZero();
    error MustHaveClaimableTokens(address);
    error MustHaveDifferentPauseState();
    ...
    modifier onlyOwner() {
        if (_owner != msg.sender) {
            revert MustBeOwner("Not owner");
        }
        _;
    }

    function isOpenPhase() internal view {
        if (currentPhase != IcoPhase.OPEN) {
            revert MustBeOpenPhase();
        }
    }

    function canContribute(uint256 contribution) internal view {
        /* check with Individual limits */
        uint256 amount = addressToContribution[msg.sender] + contribution;

        if (
            currentPhase == IcoPhase.SEED &&
            !allowlistPrivateInvestors[msg.sender]
        ) {
            revert MustBeAllowed(msg.sender, "Not allowed");
        }

        if (
            (currentPhase == IcoPhase.SEED &&
                amount > SEED_PHASE_CONTRIBUTION_LIMIT) ||
            (currentPhase == IcoPhase.GENERAL &&
                amount > GENERAL_PHASE_CONTRIBUTION_LIMIT)
        ) {
            revert MustBeUnderLimit(amount, SEED_PHASE_CONTRIBUTION_LIMIT);
        }

        /* check with Phase limits */
        amount = amountRaised + contribution;
        if (currentPhase == IcoPhase.SEED && amount > SEED_PHASE_GOAL) {
            revert MustBeUnderGoal(amount, SEED_PHASE_GOAL);
        }

        if (currentPhase != IcoPhase.SEED && amount > GOAL) {
            revert MustBeUnderGoal(amount, GOAL);
        }
    }

    function hasClaimableTokens() internal view returns (bool) {
        if (addressToTokenDistribute[msg.sender] <= 0) {
            revert MustHaveClaimableTokens(msg.sender);
        }
        return true;
    }

    function checkAmountGreaterThanZero(uint256 amount) internal pure {
        if (amount <= 0) {
    ...
    function contribute() external payable {
        if (paused) revert MustBeActive();
        checkAmountGreaterThanZero(msg.value);
        ...
        if (currentPhase == IcoPhase.SEED || currentPhase == IcoPhase.GENERAL) {
            // calcEligibleTokens could be passed as parameter to addressToTokenDistribute, but prefer this readability
        ...
        // calcEligibleTokens could be passed as parameter to _token.transfer, but prefer this readability
    ...
    function isNotOpenPhase() internal view {
        if (currentPhase == IcoPhase.OPEN) {
            revert MustBeNotOpenPhase();
        }
    }
    ...
    function setPaused(bool newValue) external onlyOwner {
        if (paused == newValue) revert MustHaveDifferentPauseState();
        paused = newValue;
    }
}
