        ...
        if(!isExcluded(from) && !isExcluded(to)) {
            ...
            if(from == address(uniswapV2Pair)) {
                ...
                if(tax > 0) {
                    ...
                    // i think by doing this we changed the ratio givng the in pancakepair thus costing user more on pancake swap(tax)
                    // given to us(contract that we then assign to the respective tax wallets just below
                }
                ...
                if(tax > 0) {
                    _transfer(from, address(this), tax);
                    // this and the lines to follow basically say. take this make it mines and gimmie eth for it from swap before u
                    // u finish processing the new balance for this fool. 
                    // take that eth and distribue it to the respective tax wallets
                    // including to the swap LP(liquidity tax), adding eth to that pool and 1/2 less of our token than we addes last time. 
                }
