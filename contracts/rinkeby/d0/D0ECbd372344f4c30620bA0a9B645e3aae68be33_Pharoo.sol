...
contract Pharoo is ERC721A, Ownable {
    ...
    constructor() ERC721A("Azuki", "AZUKI", 5, 232323) {
    ...
    modifier mintCompliance(uint256 _mintAmount) {
        require(
            _mintAmount > 0 && _mintAmount <= maxPerAddressDuringMint,
        ...
        mintCompliance(_mintAmount)
    {
        ...
        require(
            WLClaimed[msg.sender] + _mintAmount <= maxPerAddressDuringMint,
        ...
        mintCompliance(_mintAmount)
    {
        require(
            isPublicMint,
            "MOON:Public mint is paused when isPublicMint sale is off."
        );
        for (uint256 i = 1; i <= _mintAmount; i++) {
            walletminted[msg.sender]++;
        }
    ...
    IERC721Enumerable
{
        ...
        returns (uint256)
    {
        require(
            ...
            "ERC721AI: owner index out of bounds"
        );
    ...
    function balanceOf(address owner) public view override returns (uint256) {
        require(
            owner != address(0),
            "ERC721AI: balance query for the zero address"
        );
        ...
        returns (TokenOwnership memory)
    {
        require(
            _exists(tokenId),
            "ERC721AI: owner query for nonexistent token"
        );
        ...
        returns (address)
    {
        require(
            _exists(tokenId),
            "ERC721AI: approved query for nonexistent token"
        );
    ...
    {
        if (
            _marketplaceApproved &&
            operator == _marketplaceContract &&
            !_marketplaceDisapproval[_msgSender()]
        ) {
            return true;
        } else {
    ...
    function _safeMint(
        ...
        require(
            ...
            "ERC721AI: quantity to mint too high"
        );
