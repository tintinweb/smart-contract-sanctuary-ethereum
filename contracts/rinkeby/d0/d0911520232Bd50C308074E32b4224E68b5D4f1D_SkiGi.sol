...
import "./ERC721A.sol";
import "./SkiGiVoucherSigner.sol";
import "./SkiGiVoucher.sol";

// Sky Guys v1.0.2

contract SkiGi is ERC721A, Ownable, SkiGiVoucherSigner {  
    ...
    uint public MAX_SUPPLY = 5500;
    uint public PUBLIC_SUPPLY = 5000;
    uint public GIFT_SUPPLY = 500;  // COLLABS, MARKETING, TEAM, etc.
    ...
    address public a4 = 0x3d5789454C710c7da09Be7D8ACad6f72c7289473; // Andrew
    ...
    ERC721A ("SkiGi", "SkiGi", maxBatchSize_, collectionSize_) 
    SkiGiVoucherSigner(voucherSigner) {
    ...
    // Minting Function
    function mintSkiGi(uint256 _amount) external payable {
        uint256 supply = totalSupply();
        require( saleActive, "Public Sale Not Active" );
        require( _amount > 0 && _amount <= maxBatchSize, "Can't Mint More Than 3" );
    ...
    // Presale Minting
    function mintPresale(uint256 _amount, SkiGiVoucher.Voucher calldata v) public payable {
        uint256 supply = totalSupply();
        ...
        require(SkiGiVoucher.validateVoucher(v, getVoucherSigner()), "Invalid Voucher");
        require( supply + _amount <= PUBLIC_SUPPLY, "Not Enough Supply" );
        require( msg.value == price * _amount,   "Incorrect Amount Of ETH Sent" );
    ...
    function mintReducedPresale(uint256 _amount, SkiGiVoucher.Voucher calldata v) public payable {
        uint256 supply = totalSupply();
        require(reducedPresaleActive, "Reduced Private Sale Not Active");
        require(claimedVouchers[v.voucherId] + _amount <= 3, "Max 3 During Reduced Presale");
    ...
    // Validate Voucher
    function validateVoucher(SkiGiVoucher.Voucher calldata v) external view returns (bool) {
        return SkiGiVoucher.validateVoucher(v, getVoucherSigner());
    }
    ...
    function withdrawTeam() public onlyOwner {
        ...
        require(balance > 0);
        _widthdraw(a1, balance.mul(16).div(100)); // Leandro 16%
        _widthdraw(a2, balance.mul(16).div(100)); // Brett 16%
        _widthdraw(a3, balance.mul(16).div(100)); // Jena 16%
		_widthdraw(a4, balance.mul(16).div(100)); // Andrew 16%
		_widthdraw(a5, balance.mul(16).div(100)); // Mohit 16%
        _widthdraw(a6, address(this).balance); // Community Wallet 20%
    }
...
abstract contract SkiGiVoucherSigner is Ownable {
