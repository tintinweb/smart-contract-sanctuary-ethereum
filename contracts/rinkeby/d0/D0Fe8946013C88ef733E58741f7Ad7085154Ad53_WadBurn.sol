...
contract WadBurn is AdminControl, ICreatorExtensionTokenURI {
  ...
  uint public burnableTokenId;
  uint public redemptionTokenId;
  bool public initialized;
  string public assetURI;

  address public wadContract;
   ...
   */
  function initialize(string calldata uri) public adminRequired {
    ...
    // forge 1 completely separate 1155 token that this extension can mint
    uint256[] memory mintedTokenIds = IERC1155CreatorCore(_creator).mintExtensionNew(addressToSend, amount, uris);
    redemptionTokenId = mintedTokenIds[0]; // record the first token ID we mint
  }
  ...
  function mint(uint256 amount) public {
    require(IERC1155(wadContract).balanceOf(msg.sender, burnableTokenId) >= 10 * amount, "Must own 10x Kisses");
    ...
    uint256 burnAmounts = 10 * amount;

    try IERC1155(wadContract).safeTransferFrom(msg.sender,address(0xdead), burnableTokenId, burnAmounts, "0x0") {
    } catch (bytes memory) {
    ...
    tokenToSend[0] = redemptionTokenId;
