...
contract Mooncatz is Ownable, ERC721A, PaymentSplitter {
    ...
    uint private constant MAX_WHITELIST = 4437;
    uint private constant MAX_VIP = 1111;
    ...
    constructor(address[] memory _team, uint[] memory _teamShares, address _signerAddressVIP, address _signerAddressWL, string memory _baseURI) ERC721A("Mooncatz", "MOON")
    ...
    function VIPMint(address _account, uint _quantity, bytes calldata signature) external {
        ...
        // Automatically set step to whitelist sale if vip nft sold out
        if(totalSupply() + _quantity == MAX_VIP){
    ...
    function publicSaleMint(address _account, uint _quantity) external payable {
        ...
        if(mintedAmountNFTsperWalletPublicSale[msg.sender] + _quantity > maxMintAmountPerPublic) revert("You can only get 1 NFT on the Public Sale");
        // Add check if already minted in wl or VIP

        if(_numberMinted(msg.sender) != 0) revert("User has minted in VIP or WL stage already");
    ...
    function WLMint(address _account, uint _quantity, bytes calldata signature) external payable {
        ...
        if(totalSupply() + _quantity > MAX_WHITELIST) revert("Max supply for WL exceeded");
        if(signerAddressVIP == keccak256(
            abi.encodePacked(
                "\x19Ethereum Signed Message:\n32",
                bytes32(uint256(uint160(msg.sender)))
            ...
            if(_numberMinted(msg.sender) != 0) revert("User has mint in VIP stage already");
        }
        if(msg.value < price * _quantity) revert("Not enough funds");          
