...
contract Avatar is IAvatar, ERC721, Ownable {
    ...
    mapping(uint256 => uint256) tokenIdToConfiguration;

    // Mapping from configuration bit mapping to token id
    mapping(uint256 => uint256) configurationToTokenId;

    uint256 constant fiveMinutes = 5 * 60;

    uint256 private _maxSupply = 10000;

    ILockerRoomAttendant public attendant;
    ...
    function mintAvatar(
        ...
        require(
            configurationToTokenId[desiredConfiguration] == uint256(0),
        ...
        tokenIdToConfiguration[newItemId] = desiredConfiguration;
        configurationToTokenId[desiredConfiguration] = newItemId;
    ...
    function _afterTokenTransfer(
        ...
        if (attendant != ILockerRoomAttendant(address(0)))
            attendant.afterAvatarTokenTransfer(from, to, tokenId);
    }
    ...
    function tokenByConfiguration(uint256 configuration) public view returns (uint256) {
        return configurationToTokenId[configuration];
    }
}
...
interface IAvatar {
    ...
    function attendant() external returns (ILockerRoomAttendant);
}
