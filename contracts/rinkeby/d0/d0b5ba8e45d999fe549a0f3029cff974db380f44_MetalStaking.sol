...
contract MetalStaking is Ownable, IERC721Receiver, ReentrancyGuard, Pausable {
    ...
    function setRates(uint256 _legendsRate,uint256 _cubsRate, uint256 _padRate, uint256 _tigerRate, uint256 _fundaeRate, uint256 _minBlockToClaim ) public onlyOwner() {
    ...
    //forge function. 
    function legendsForge(uint256[] calldata tokenIds) external whenNotPaused {
        for (uint256 i; i < tokenIds.length; i++) {
            require(IERC721(legendsAddress).ownerOf(tokenIds[i]) == msg.sender,"you do not own that Legend");
            require(_legendsTokenForges[tokenIds[i]] != msg.sender,"at least one token already forged for this address");
    ...
    //forge function. 
    function padForge(uint256[] calldata tokenIds) external whenNotPaused {
        for (uint256 i; i < tokenIds.length; i++) {
            require(IERC721(padAddress).ownerOf(tokenIds[i]) == msg.sender,"you do not own that PAD");
            require(_padTokenForges[tokenIds[i]] != msg.sender,"at least one token already forged for this address");
    ...
    //forge function. 
    function tigerForge(uint256[] calldata tokenIds) external whenNotPaused {
        for (uint256 i; i < tokenIds.length; i++) {
            require(IERC721(tigerAddress).ownerOf(tokenIds[i]) == msg.sender,"you do not own that Tiger");
            require(_tigerTokenForges[tokenIds[i]] != msg.sender,"at least one token already forged for this address");
    ...
    //forge function. 
    function cubsForge(uint256[] calldata tokenIds) external whenNotPaused {
        for (uint256 i; i < tokenIds.length; i++) {
            require(IERC721(cubsAddress).ownerOf(tokenIds[i]) == msg.sender,"you do not own that Cub");
            require(_cubsTokenForges[tokenIds[i]] != msg.sender,"at least one token already forged for this address");
    ...
    function fundaeCalculateReward(address account, uint256 tokenId) 
      ...
      returns (uint256) 
    {
      require(Math.min(block.number, expiration) > _fundaeForgeBlocks[tokenId], "Invalid blocks");
      return fundaeRate * 
          (_fundaeTokenForges[tokenId] == account ? 1 : 0) * 
          (Math.min(block.number, expiration) - 
            _fundaeForgeBlocks[tokenId]);
    }
    ...
    //forge function. 
    function fundaeForge(uint256[] calldata tokenIds) external whenNotPaused {
          for (uint256 i; i < tokenIds.length; i++) {
            require(IERC721(fundaeAddress).ownerOf(tokenIds[i]) == msg.sender,"you do not own that Fundae");
            require(_fundaeTokenForges[tokenIds[i]] != msg.sender,"at least one token already forged for this address");
