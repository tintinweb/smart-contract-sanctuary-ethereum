...
contract Beta is ERC721A, Ownable, ReentrancyGuard {
    using Address for address;
    using SafeMath for uint256;

    uint256 public constant presaleMintPrice = 0.1 ether;
    uint256 public constant publicMintPrice = 0.02 ether;
    uint256 public constant MAX_PER_TX = 11; // max per tx + 1 for gas optimization
    uint256 public constant MAX_SUPPLY = 31; // max supply + 1 for gas optimization
    uint256 public constant MAX_PRESALE_SUPPLY = 11; // max tokens available to presale members. Amount +1 for gas optimization
    uint256 public constant TEAM_SUPPLY = 11; // For team, marketing, and giveaways. Amount +1 for gas optimization
    ...
    // Storing as uint256 to reduct gas on boolean conversions during every mint
    uint256 public presaleActive = _IS_NOT_ACTIVE;
    uint256 public publicSaleActive = _IS_NOT_ACTIVE;

    // Used to check presale whitelist membership
    bytes32 public presaleMerkleRoot; // Settable by setPresaleMerkleRoot by owner
    ...
    /**
     * @dev validates that the correct payment has been sent to match the number of tokens being minted
     */
    ...
    /**
     * @dev validates that public minting is enabled, tokens per tx is not exceeded, and tokens are avail
     */
    ...
    /**
     * @dev validates that private minting is enabled, tokens per tx is not exceeded,  presale tokens are available, and overall token supply exists
     */
    modifier canMintPresale(uint256 numberOfTokens) {
        require(presaleActive == _IS_ACTIVE, "Presale is not live");
        require(
            numberOfTokens < MAX_PER_TX,
            "Exceeds max mints per transaction"
        );
        require(
            totalSupply() + numberOfTokens <= MAX_PRESALE_SUPPLY,
            "Not enough tokens remaining to mint"
        );
        // This should not ever trigger during normal presale -> public sale flow, but leaving for safety net
        require(
            totalSupply() + numberOfTokens <= MAX_SUPPLY,
            "Not enough tokens remaining to mint"
        );
    ...
    /**
     * @dev set Merkle root for presale list
     */
    ...
    // ================ Sale Status Controls ================ //

    // Sets the  preSale status. true = Active, false = Not Active. Stores as 1(Active) or 2(Not Active)
    ...
    function setPresaleStatus(bool value) public onlyOwner {
        if (value == true) {
            presaleActive = _IS_ACTIVE;
        } else {
            presaleActive = _IS_NOT_ACTIVE;
        }
    }

    // Sets the public sale status. true = Active, false = Not Active
    ...
    function setPublicSaleStatus(bool value) public onlyOwner {
        if (value == true) {
            ...
            presaleActive = _IS_NOT_ACTIVE; //Presale is over when public sale starts. Execute in one transaction.
        } else {
    ...
    {
        require(msg.sender == tx.origin, "EOA only");
        require(teamMintCount + amount < TEAM_SUPPLY);
