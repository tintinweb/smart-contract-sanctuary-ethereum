...
contract Swap is AccessControlEnumerable {
    ...
    function updateCollateralAddress(address _collateralAddress) external onlyAdmin {
        ...
        collateral = ERC721(collateralAddress);
    }

    function updateSwappableAddress(address _swappableAddress) external onlyAdmin {
        swappableAddress = _swappableAddress;

        swappable = Collectibles(swappableAddress);
    }

    /// @dev set pair in pairs
    /// @param fromTokenId tokenId which is swapped
    /// @param toTokenId tokenId which is consumed
    function setPair(uint256 fromTokenId, uint256 toTokenId) external onlyAdmin {
    ...
    function swap(
        ...
        uint256 toTokenId = pairs[fromTokenId];

        if (toTokenId == 0) {
            revert PairNotFoundError();
        }

        if (toTokenId != _toTokenId) {
        ...
        swappable.mint(sender, toTokenId, amount);

        emit SwapEvent(sender, fromTokenId, toTokenId, amount);
    }
}
