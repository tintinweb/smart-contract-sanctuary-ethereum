...
/**
 * @title HungryHungryHamstersClub contract
...
contract HungryHungryHamsters is ERC721Enumerable, Ownable
{
  ...
  uint256 private constant HAMSTER_RESERVE_AMOUNT =           25;
  ...
  string private baseURI = "https://ipfs.io/ipfs/bafybeigyedd6vxth54eb3ahdwjowuak7hivn2to7alahlirlcwe2gygnlq/";                         //Unrevealed URI
  ...
  */
   ...
   */
  function mintHamster() private onlyOwner
  {
    if(startingIndex == 0)
    {
      uint256 startingIndexBlock = block.number;

      startingIndex = uint(blockhash(startingIndexBlock)) % MAX_HAMSTERS;

      if(block.number.sub(startingIndexBlock) > 255)
      {
        startingIndex = uint(blockhash(block.number - 1)) % MAX_HAMSTERS;
      }
