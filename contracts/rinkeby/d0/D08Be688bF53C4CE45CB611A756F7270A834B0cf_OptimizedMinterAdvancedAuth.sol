...
//import IERC721 interface so that opensea recognises it as ERC721 compliant
...
contract OptimizedMinterAdvancedAuth is ERC721AQueryable, ReentrancyGuard {
    ...
    //max amount of tokens that can be minted
    ...
    //to store whether the whitelist mint is active or not
    bool whitelistActive;

    //store the amount a given user has minted
    mapping(address => uint8) private numMintedByUser;

    //string for storing the base URI
    ...
    constructor(
        ...
        whitelistLimit = 10; //3333 is the amount for mainnet, 5 is for testing
        totalLimit = 20; //9999 is the amount for mainnet, 10 is for testing
    ...
    modifier onlyApproved(
        ...
        require(VerifyMessage(_hashedMessage, _v, _r, _s), "ERR:NA");
        _;
    }

    //--------------OnlyAdmin can call these ------------//

    function changeAdmin(address _new) external onlyAdmin {
        admin = _new;
    }

    //for setting a new merkle root
    ...
    //--------------Only addresses calling with a signed message from the Admin can call these ------------//
    ...
    function VerifyMessage(
        ...
        require(_hashedMessage == keccak256(abi.encode(msg.sender)), "ERR:NA"); // NA -> not approved
    ...
    //for checking if msg.sender is whitelisted
    ...
    function whitelistMint(uint8 amount, bytes32[] calldata _merkleProof)
        ...
        nonReentrant
    {
        //amount must not be larger than 3
        require(amount <= 3, "ERR:SA"); //SA -> sent amount

        //amount + amountAlreadyMintedByUser must not be larger than 3
        require(numMintedByUser[msg.sender] + amount <= 9, "ERR:MA"); //MA -> mint amount

        if (whitelistActive) {
            //checking the validity of the whitelist
            require(checkValidity(_merkleProof), "ERR:WP"); //WL -> whitelist proof

            //checking that the amount + totalSupply is not larger than 3333
            require(amount + totalSupply() <= whitelistLimit, "ERR:WL"); //WL -> whitelist limit

            if (msg.sender != admin) {
                //caluclate due amount
                uint256 amountDue = amount * whitelistPrice;

                //checking that the value sent is the exact amount * whitelist price
            ...
            //revert as wrong function has been called
            revert(
                "Whitelist is not active, this function is not available right now"
            );
        }
    }
    ...
    function mint(uint8 amount) external payable nonReentrant {
        ...
        //amount + amountAlreadyMintedByUser must not be larger than 3
        require(numMintedByUser[msg.sender] + amount <= 18, "ERR:MA"); //MA -> mint amount

        if (whitelistActive) {
            //revert as wrong function has been called
            revert("Whitelist is active, this mint is not available");
        } else {
            if (msg.sender != admin) {
                //calculate amount due
                uint256 amountDue = amount * normalPrice;

                //checking that the value sent is the exact amount * normal price
                require(msg.value == amountDue, "ERR:NF"); //NF -> not enough funds
            ...
            //checking that the totalSupply + amount do not exceed the total limit
