...
contract LordsAndCo is Ownable, ReentrancyGuard {
    ...
    */
    ...
    /**
    * @dev Returns claimable $loomi amount for user based on baseRate and pot change
    */
    function getClaimableAmount(address staker) external view returns (uint256) {
      uint256 change = getPriceChange(staker);
      uint256 finalAmount = _stakers[staker].accumulatedAmount * change / DIVIDER;
      return finalAmount;
    }
