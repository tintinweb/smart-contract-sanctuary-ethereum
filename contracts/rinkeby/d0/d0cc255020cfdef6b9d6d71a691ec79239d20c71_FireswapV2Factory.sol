...
contract FireswapV2Factory is IFireswapV2Factory {
    ...
    modifier onlyFeeToSetter() {
        require(msg.sender == feeToSetter, 'FireswapV2: FORBIDDEN');
        _;
    }
    ...
    function pairCodeHash() external pure returns (bytes32) {
        return keccak256(type(FireswapV2Pair).creationCode);
    }

    function createPair(address tokenA, address tokenB) external override returns (address pair) {
        require(tokenA != tokenB, 'FireswapV2: IDENTICAL_ADDRESSES');
        (address token0, address token1) = tokenA < tokenB ? (tokenA, tokenB) : (tokenB, tokenA);
        require(token0 != address(0), 'FireswapV2: ZERO_ADDRESS');
        require(getPair[token0][token1] == address(0), 'FireswapV2: PAIR_EXISTS'); // single check is sufficient
        bytes memory bytecode = type(FireswapV2Pair).creationCode;
        bytes32 salt = keccak256(abi.encodePacked(token0, token1));
        assembly {
            pair := create2(0, add(bytecode, 32), mload(bytecode), salt)
        }
        (uint totalFeePerThousand, uint alpha, uint beta) = getFeeInfo(token0, token1);
        FireswapV2Pair(pair).initialize(token0, token1, totalFeePerThousand, alpha, beta);
    ...
    function setTopCoinFee(uint _totalFee, uint _alpha, uint _beta) external onlyFeeToSetter {
        require(_alpha < _beta, 'FireswapV2: IMPROPER FRACTION');
    ...
    function updatePairFee(address token0, address token1, uint _totalFee, uint _alpha, uint _beta) external onlyFeeToSetter {
        ...
        require(pair != address(0), 'FireswapV2: PAIR_DOES_NOT_EXIST');
        FireswapV2Pair(pair).updateFee(_totalFee, _alpha, _beta);
