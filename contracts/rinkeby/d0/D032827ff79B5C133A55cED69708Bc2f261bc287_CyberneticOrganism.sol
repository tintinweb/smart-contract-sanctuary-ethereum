...
//        int8 strength = DiceUtilities.rollToScore(ThreeDice);
        int8 strength = DiceUtilities.rollToScore(manyRolls[0]);
...
//        int8 agility = DiceUtilities.rollToScore(uint(threeDice));
        int8 agility = DiceUtilities.rollToScore(manyRolls[1]);
        emit scoreCalculated(manyRolls[1], agility);

        int8 presence = DiceUtilities.rollToScore(manyRolls[2]);
        emit scoreCalculated(manyRolls[2], presence);
        int8 toughness = DiceUtilities.rollToScore(manyRolls[3]);
        emit scoreCalculated(manyRolls[3], toughness);
        int8 knowledge = DiceUtilities.rollToScore(manyRolls[4]);
...
//            addedUp += uint8(uint256(keccak256(abi.encodePacked(randomValue, i, tx.gasprice))) % die + 1);
        ...
        function dieRollsMultiple(uint die, uint dice, uint256 n) internal returns (uint8[] memory expandedRolls) {
            uint256 randomValue = uint256(keccak256(abi.encodePacked(block.difficulty, block.timestamp)));
