...
/** 
 *  SourceUnit: h:\work\TheDawgChain\contracts\contracts\TheDawgChainMarket.sol
*/
...
contract TheDawgChainNFT is ERC721 {
    ...
    event ItemCreated(uint256 tokenId, string uri, uint256 motherId, uint256 fatherId);

    constructor (string memory _name,
        string memory _uri) ERC721(_name,_name) {
        owner = msg.sender;
        collection_uri = _uri;
	 ...
	 */
    function addItem(string memory _tokenURI, uint256 _motherId, uint256 _fatherId) public returns (uint256){
        currentID = currentID.add(1);        
        _safeMint(msg.sender, currentID);
        Items[currentID] = Item(currentID, _tokenURI, _motherId, _fatherId);
        emit ItemCreated(currentID, _tokenURI, _motherId, _fatherId);
        return currentID;
    }
...
contract TheDawgChainMarket is Ownable, ERC721Holder {
    using SafeMath for uint256;
	using EnumerableSet for EnumerableSet.AddressSet;
	...
	uint256 public totalEarning; /* Total TheDawgChain Token */
	uint256 public totalSwapped; /* Total swap count */

	/** Events */
    event ItemListed(uint256 pairID, uint256 tokenId, uint256 price, address owner);
    ...
    function list(uint256 _tokenId, uint256 _price) OnlyItemOwner(_tokenId) public {
    ...
    function delist(uint256 _pairID) external {        
        ...
        require(msg.sender == pairs[_pairID].owner || msg.sender == owner(), "Error, you are not the owner");        
        nftCollection.safeTransferFrom(address(this), pairs[_pairID].owner, pairs[_pairID].tokenId);        
        pairs[_pairID].bValid = false;
        emit ItemDelisted(_pairID);        
    }


    function buy(uint256 _pairID) external ItemExists(_pairID) {
        require(pairs[_pairID].bValid, "invalid Pair id");
		require(pairs[_pairID].owner != msg.sender, "owner can not buy");
		...
		nftCollection.safeTransferFrom(address(this), msg.sender, pair.tokenId);
