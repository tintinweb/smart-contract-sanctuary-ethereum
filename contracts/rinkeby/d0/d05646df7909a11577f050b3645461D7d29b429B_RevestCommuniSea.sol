 ...
 * @dev 
 */
contract RevestCommuniSea is IOutputReceiverV3, Ownable, ERC165, IFeeReporter, ReentrancyGuard {
    ...
    /// Address which will control the escrow lock
    address public immutable ADMIN;

    // Template address for VE wallets
    address public immutable TEMPLATE;

    /// Lock contract which will be handling lockups
    address public ESCROW_LOCK;

    // The file which tells our frontend how to visually represent such an FNFT
    string public METADATA = "https://revest.mypinata.cloud/ipfs/QmQNeCySwjobkmYSYtcQ4pf7FH2LgFB1mtW28kRe5sfKvs";
    ...
    uint public constant AMOUNT_TO_LOCK = 400000 ether; 

    uint80 public constant LOCKUP = 180 days;

    // Initialize the contract with the needed valeus
    constructor(address _provider, address _token, address _escrowLock, address _admin) {
        addressRegistry = _provider;
        TOKEN = _token;
        ADMIN  = _admin;
        ESCROW_LOCK = _escrowLock;
        RevestSmartWallet wallet = new RevestSmartWallet(_provider);
    ...
    // No up-front fee
    function depositToEscrow() external nonReentrant returns (uint fnftId) {    

        uint80 endTime = uint80(block.timestamp) + LOCKUP; 
        bytes memory arguments = abi.encode(endTime, ADMIN);

        /// Mint FNFT
        /// Presume that we are whitelisted for fees
        {
            ...
            fnftId = IRevest(revest).mintAddressLock(ESCROW_LOCK, arguments, recipients, quantities, fnftConfig);
        }

        address smartWallAdd = getAddressForFNFT(fnftId);
        IERC20(TOKEN).safeTransferFrom(msg.sender, smartWallAdd, AMOUNT_TO_LOCK);
        emit DepositERC20OutputReceiver(msg.sender, TOKEN, AMOUNT_TO_LOCK, fnftId, abi.encode(smartWallAdd));
    }
    ...
    function receiveRevestOutput(
        ...
        require(quantity == 1, 'E061');

        address smartWallAdd = Clones.cloneDeterministic(TEMPLATE, keccak256(abi.encode(TOKEN, fnftId)));
        uint amountToWithdraw = IERC20(TOKEN).balanceOf(smartWallAdd);
        RevestSmartWallet wallet = RevestSmartWallet(smartWallAdd);

        wallet.withdraw(amountToWithdraw, TOKEN, caller);

        emit WithdrawERC20OutputReceiver(caller, TOKEN, amountToWithdraw, fnftId, abi.encode(smartWallAdd));
    }
    ...
    /// Not applicable
    function handleTimelockExtensions(uint fnftId, uint expiration, address) external override {}

    /// Not applicable
    function handleAdditionalDeposit(uint fnftId, uint amountToDeposit, uint, address caller) external override nonReentrant {}

    /// Not applicable
    function handleSplitOperation(uint fnftId, uint[] memory proportions, uint quantity, address caller) external override {}

    /// Not applicable
    function triggerOutputReceiverUpdate(uint fnftId, bytes memory) external override {}       
    ...
    function getValue(uint fnftId) public view override returns (uint) {
        return IERC20(TOKEN).balanceOf(Clones.predictDeterministicAddress(TEMPLATE, keccak256(abi.encode(TOKEN, fnftId))));
    }
    ...
    function getOutputDisplayValues(uint fnftId) external view override returns (bytes memory displayData) {
        ...
        displayData = abi.encode(smartWallet);
    }
    ...
    function getERC20Fee(address) external view override returns (uint) {
        return RevestSmartWallet(TEMPLATE).FEE_NUM();
    }
...
contract RevestSmartWallet {
    ...
    uint public constant FEE_NUM = 5;
    uint public constant FEE_DENOM = 1000;
    ...
    function withdraw(uint value, address token, address recipient) external {
        require(msg.sender == MASTER, 'E016');
        uint fee = value * FEE_NUM / FEE_DENOM;
        address admin = IAddressRegistry(PROVIDER).getAdmin();
        value -= fee;
