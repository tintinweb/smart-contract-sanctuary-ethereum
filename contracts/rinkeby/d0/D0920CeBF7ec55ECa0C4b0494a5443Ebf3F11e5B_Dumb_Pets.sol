...
contract Dumb_Pets is 
	...
	function airdropPets(address[] calldata whitelist, uint256[] calldata whitelist_quanityPerUser) whenNotPaused public onlyOwner {
		...
		for(uint256 i = 0; i < whitelist.length; i++)
		{
			...
			// Finally mint the desired amount
	...
	function mintPurchase(uint256 _mintAmount) public payable whenNotPaused {
		...
		// Ensure that minting this many doesnt go over the total supply and ensure 
		// 	that the cost passed in matches the cost of minting these NFTs
...
contract Pets_Settings is AccessControl, Roles {
    ...
    string public hiddenURI =
        "https://nft-test-alex.s3.ap-southeast-2.amazonaws.com/NFTTest/hidden/hidden.json";
    ...
    function setSignerAddress(address newSignerAddress)
        public
        onlyRole(Roles.ROLE_SAFE)
    {
        signerAddress = newSignerAddress;
    }

    // @dev Sets the collection to be revealed or hidden ( will show a different URI )
    function setRevealed(bool _revealed) public onlyRole(Roles.ROLE_SAFE) {
        revealed = _revealed;
    }
