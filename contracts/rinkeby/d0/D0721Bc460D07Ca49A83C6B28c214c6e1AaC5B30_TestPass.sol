...
contract TestPass is ERC721Enumerable, ReentrancyGuard, Ownable {
    uint256 public constant maxSupply = 1000;
    uint256 public vaultLimit = 100;
    ...
    mapping(address => bool) public alreadyMintedPass;
    ...
    uint256 public passMultiplier = 1;

constructor() ERC721("testpass", "TESTPS") {}
    ...
    function withdraw() external onlyOwner {
        require(payable(teamAddress).send(address(this).balance));
    }
    ...
    function mint(uint8 numberOfTokens) external payable nonReentrant {
        ...
        require(!alreadyMintedPublicSale[msg.sender], "Address has already minted");
        require(balanceOf(msg.sender) < perWallet, "Per wallet limit exceeded");

        alreadyMintedPublicSale[msg.sender] = true;
        for (uint256 i = 0; i < numberOfTokens; i++) {
            _safeMint(msg.sender, nextTokenId());
        }
    }

    function mintPresale(uint8 numberOfTokens) external payable nonReentrant {
    ...
    function mintPass(uint8 numberOfTokens) external payable nonReentrant {
        require(passSaleIsActive, "Passsale not active");
        require(msg.value >= mintPrice, "More eth required");
        require(totalSupply() < maxSupply, "Sold out");
        require(!alreadyMintedPass[msg.sender], "Address has already minted");

        require((walletHoldsToken(msg.sender, passContract) * passMultiplier) >= numberOfTokens, "Need a token from a qualified collection for each mint");
    
        alreadyMintedPass[msg.sender] = true;
        for (uint256 i = 0; i < numberOfTokens; i++) {
            _safeMint(msg.sender, nextTokenId());
        }
    }
    ...
    function flipPassSaleState() public onlyOwner {
    ...
    function setPassContract(address _newPassContract) public onlyOwner() {
        passContract = _newPassContract;
    }

    function setPassMultiplier(uint256 _newPassMultiplier) public onlyOwner() {
        passMultiplier = _newPassMultiplier;
    }
