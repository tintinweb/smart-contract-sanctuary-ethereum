...
contract Pixelghostmintbutton is Ownable, ERC721A, PaymentSplitter {
    ...
    uint private constant MAX_TEAM = 600;
    uint private constant MAX_COMMUNITY = MAX_SUPPLY - MAX_TEAM;

    uint public wlSalePrice = 0 ether;
    uint public publicSalePrice = 0 ether;

    uint public saleStartTime = 1664649054;

    bytes32 public merkleRoot;

    string public baseURI;

    mapping(address => uint) amountPerWallet;

    uint private constant maxPerAddress = 100;
    ...
    ERC721A("PixelGhost MintButton", "PGMB") 
    ...
    **/
    function whitelistMint(address _account, uint _quantity, bytes32[] calldata _proof) external payable callerIsUser{
        baseURI = "ipfs://QmSEszxJrPVPy9NYn4XPz9JPnxqPqUQindGsPNQLYzFXmg/"; //Red Ghost pour la whitelist
        require(!isPaused, "Contract is paused");
        require(currentTime() >= saleStartTime + 10 hours, "Sale has not started yet");
        ...
        require(amountPerWallet[msg.sender] + _quantity <= maxPerAddress, "You can only get 1 NFT on the Whitelist Sale");
        require(totalSupply() + _quantity <= MAX_COMMUNITY, "Max supply exceeded");
        require(msg.value >= price * _quantity, "Not enought funds");
        amountPerWallet[msg.sender] += _quantity;
    ...
    **/
    function publicMintGreen(address _account, uint _quantity) external payable callerIsUser {
        baseURI = "ipfs://QmdQ24hp6hXaw8bRn7HyynfG57gNCbfua6Q9twmNthGZ37/"; 
        require(!isPaused, "Contract is Paused");
        ...
        require(amountPerWallet[msg.sender] + _quantity <= maxPerAddress, "You can only get 3 NFTs on the Whitelist Sale");
        require(totalSupply() + _quantity <= MAX_COMMUNITY, "Max supply exceeded");
        require(msg.value >= price * _quantity, "Not enought funds");
        amountPerWallet[msg.sender] += _quantity;
        _safeMint(_account, _quantity);
    ...
    * @param _quantity Amount of NFTs the user wants to mint
    **/
    function publicMintBlue(address _account, uint _quantity) external payable callerIsUser {
        baseURI = "ipfs://QmbPpLJZBsQepz9vbrFja7SRqaqPmmAtsJgvRLGU4tkw6F/";
        require(!isPaused, "Contract is Paused");
        require(currentTime() >= saleStartTime + 24 hours, "Public sale has not started yet");
        require(currentTime() < saleStartTime + 48 hours, "Public Sale is finished");
        uint price = publicSalePrice;
    ...
    * @param _quantity Amount of NFTs the user wants to mint
    **/
    function publicMintRed(address _account, uint _quantity) external payable callerIsUser {
        baseURI = "ipfs://QmSEszxJrPVPy9NYn4XPz9JPnxqPqUQindGsPNQLYzFXmg/"; 
        require(!isPaused, "Contract is Paused");
        require(currentTime() >= saleStartTime + 24 hours, "Public sale has not started yet");
        require(currentTime() < saleStartTime + 48 hours, "Public Sale is finished");
        uint price = publicSalePrice;
    ...
    * @param _baseURI Declare baseURI parameter to choose which character to gift
    **/
    function teamGift(address _to, uint _quantity, string memory _baseURI) external onlyOwner {
        baseURI = _baseURI;
        require(sellingStep > Step.PublicSale, "Team gift is after the public sale");
