...
contract LamearBrand is ERC721Enumerable, Ownable
{
    using SafeMath for uint256;
    using Strings for uint256;

    uint public constant _TOTALSUPPLY = 4000;
    // uint public maxQuantity =10;
    // uint public maxPerUser=2;
    uint public reserve=88;
    ...
    constructor(string memory baseURI) ERC721("LamearBrand", "LB") {
        setBaseURI(baseURI);
    ...
    function reserveNFT() public onlyOwner {
        require(totalSupply()+reserve<=_TOTALSUPPLY,"Quantity must be lesser then MaxSupply");
        for (uint i = 0; i < reserve; i++) {
            _safeMint(msg.sender, totalsupply());
        }
    }

    function freeMint(uint chosenAmount, address addressReceiver) public onlyOwner {
        ...
        for (uint i = 0; i < chosenAmount; i++) {
            _safeMint(addressReceiver, totalsupply());
        }
    }
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        ...
        if(bytes(base).length > 0){
            return reveal ? string(abi.encodePacked(base, tokenId.toString(), ".json")) : string(abi.encodePacked("https://gateway.pinata.cloud/ipfs/QmbkKaMAVBGwB7XmKwwzuWQbrzBMcqYcAoVoAtNEEYM9Fr"));
        }
        else 
        return "";
    }
    function contractURI() public view returns (string memory) {
        string memory json = Base64.encode(bytes(string(abi.encodePacked('{"name": "Lamear Brand", "description": "Lamear Brand NFTs", "seller_fee_basis_points": 700, "fee_recipient": "0xf4a07925cFCe8F3abF79539f584dC86F4964E512"}'))));
