...
contract Giza is Ownable, ERC721iEnumerable {
    ...
    function _beforeTokenTransfer(
        ...
        if (to == address(sudoPool) || from == address(sudoPool)) {
            ...
            sudoPool.withdrawETH((uint256(price) * 5) / 100);
        }

        if (to == address(sudoPool)) {
            if (totalSupply() > 100) {
                // if supply over 100 we need to change price to do avg
    ...
    function _afterTokenTransfer(
        ...
        if (to == address(sudoPool)) {
            ...
            // if supply over 100 we need to change price to do avg
            if (totalSupply() > 100) {
                price += price + 0.05 ether;

                sudoPool.changeSpotPrice(price);
            }
        }
    ...
    // think about this later if we do from the pool on setup or manually and we change owner
