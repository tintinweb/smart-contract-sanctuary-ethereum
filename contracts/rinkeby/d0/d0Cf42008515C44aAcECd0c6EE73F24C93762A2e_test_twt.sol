...
contract test_twt is ERC721, Ownable {
    using Strings for uint256;

    uint start = 3001;
    address testAddress = 0xe515A326fD0BAE4465Ce685050584CE5736fa996;

    string  public baseURI;

    constructor() ERC721("NA10K","NA10K") {

        _mint(testAddress,3001);
        _mint(testAddress,3002);
        _mint(testAddress,3003);
        _mint(testAddress,3004);
        _mint(testAddress,3005);
        _mint(testAddress,3006);
        baseURI = "https://twt-meta.obscura.io/meta/";
        
    }

    function _mint(uint256 tokenId) external onlyOwner {
        _mint(testAddress,tokenId);
    }
    ...
    function tokenURI(uint256 tokenId) public view override(ERC721) returns (string memory) {
       require(_exists(tokenId), 'Token does not exist');
        ...
        uint256 folderNo = (((tokenId / 100) % 100) + 1);
        string memory folder     = folderNo.toString();
        string memory file     = (tokenId % 100) .toString();
        //
       

        return string(abi.encodePacked(baseURI,folder,"/",file)) ;
        //       //
    }
