...
contract ParalellONFT1155Proxy is ProxyONFT1155 {

    IERC1155MetadataURI public immutable parallelAlpha;

    constructor(address _lzEndpoint, address _token) ProxyONFT1155(_lzEndpoint, _token) {
        parallelAlpha = IERC1155MetadataURI(_token);
    }

    function _sendBatch(address _from, uint16 _dstChainId, bytes memory _toAddress, uint[] memory _tokenIds, uint[] memory _amounts, address payable _refundAddress, address _zroPaymentAddress, bytes memory _adapterParams) internal virtual override {
        _debitFrom(_from, _dstChainId, _toAddress, _tokenIds, _amounts);

        string[] memory _uris = new string[](_tokenIds.length);

        for (uint i = 0; i < _tokenIds.length; i++) {
            _uris[i] = parallelAlpha.uri(_tokenIds[i]);
        }

        bytes memory payload = abi.encode(_toAddress, _tokenIds, _amounts, _uris);
    ...
    function _nonblockingLzReceive(uint16 _srcChainId, bytes memory _srcAddress, uint64 _nonce, bytes memory _payload) internal virtual override {
        ...
        (bytes memory toAddressBytes, uint[] memory tokenIds, uint[] memory amounts,) = abi.decode(_payload, (bytes, uint[], uint[], string[]));
        address toAddress;
        assembly {
