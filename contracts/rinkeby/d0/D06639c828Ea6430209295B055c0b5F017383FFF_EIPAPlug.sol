...
contract EIPAPlug is ERC1155Holder, Ownable, AccessControl {
    ...
    function _claim(uint tokenId, uint amount) internal {
        ...
        require(tokenClaimInfo[tokenId].totalClaimed[msg.sender] + amount <= tokenClaimInfo[tokenId].claimLimit, "You have already claimed the maximum allowed");
        eipa.safeTransferFrom(address(this), msg.sender, tokenId, amount, "");
        tokenClaimInfo[tokenId].totalClaimed[msg.sender] += amount;
    }

    function setClaimLimit(uint tokenId, uint claimLimit) external onlyRole(SUPPORT_ROLE) {
    ...
    function setTokenClaimInfo(uint tokenId, bool claimActive, uint claimLimit, uint transactionLimit, uint unreservedTotal, bytes32 merkleRoot) external onlyRole(SUPPORT_ROLE) {
