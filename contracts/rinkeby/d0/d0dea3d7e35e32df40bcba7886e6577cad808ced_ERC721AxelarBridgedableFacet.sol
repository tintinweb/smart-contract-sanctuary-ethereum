...
contract ERC721AxelarBridgedableFacet is IERC721AxelarBridgedableFacet {
    ...
    function moveToViaAxelar(
        ...
        axelarSeneder.moveToTargetChain{value: msg.value}(
            _chain,
            _destinationBridge,
            sender,
            sender,
    ...
    function mintOnViaAxelar(
        ...
        axelarSeneder.mintOnTargetChain{value: msg.value}(
            ...
            mintTo,
            _tokenId,
            LibERC721TokenURI.predictableTokenURI(s.diamondAddress, _tokenId)
        );
    }
}
...
library LibERC721TokenURI {
    ...
    function predictableTokenURI(address tokenAddress, uint256 tokenId)
        internal
        view
        ensureIsContract(tokenAddress)
        returns (string memory)
    {
        ...
        (bool success, bytes memory result) = tokenAddress.staticcall(
            abi.encodeWithSelector(
                IERC721TokenURIFacet.predictableTokenURI.selector,
                tokenId
            )
        );
...
interface IERC721TokenURIFacet {
    ...
    function predictableTokenURI(uint256 tokenId)
        external
        view
        returns (string memory);
}
