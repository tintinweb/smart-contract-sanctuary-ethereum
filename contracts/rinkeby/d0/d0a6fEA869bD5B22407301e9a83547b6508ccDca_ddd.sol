...
contract ddd is Ownable, ERC721A, ReentrancyGuard {
    ...
    function alreadyMintedPresale(address addr) public view returns (uint256) {
        return _preSaleCounter[addr];
    }

    function alreadyMintedPublic(address addr) public view returns (uint256) {
        return _publicCounter[addr];
    }
    ...
    function internalMint(uint256 quantity, address to) external onlyOwner {
        require(
            _teamCounter + quantity <= amountForTeam,
        ...
        _teamCounter = _teamCounter + quantity;
    }
    ...
    function mintPresale(
        ...
        require(_presaleActive, "Presale is closed");
        require(
            amount <= maxAmount - _preSaleCounter[sender],
        ...
        _preSaleCounter[sender] += amount;
        _safeMint(sender, amount);
    }

    // public mint
    function publicSaleMint(uint256 amount) external nonReentrant callerIsUser {
        require(amount > 0, "Must mint more than 0 tokens");
        require(_isActive, "public sale has not begun yet");
        require(
            _publicCounter[msg.sender] + amount <= maxPublic,
            "exceeds max per address"
        );
        require(totalSupply() + amount <= MAX_SUPPLY, "reached max supply");

        _safeMint(msg.sender, amount);
        _publicCounter[msg.sender] = _publicCounter[msg.sender] + amount;
    }
