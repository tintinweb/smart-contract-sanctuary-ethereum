...
/////////////////////////////
    ...
    function getAllRegisteredForSlot(uint256 slotIndex) public view returns (uint256[] memory, string[] memory){
        address slotAddr = addyArray[slotIndex];
        string[] memory tokenUriArray = new string[](slotStock[slotAddr].length);
        ERC721 nftContract;
    ...
    function fulfillRandomness(bytes32 requestId, uint256 randomness) internal override  {
        ...
        removeFirstElementAndShiftArrayDown(slotWinner); //remove tokenId from array and shift array contents down 1 slot. 
        ...
        _burn(sender); //burn the treasure chest
                                                                      ...
                                                                      //internal
    function removeFirstElementAndShiftArrayDown(uint addyArrayIndex) public  {
        require (slotStock[addyArray[addyArrayIndex]].length > 0, "array is empty / no registered tokenIds for slot..");

        for (uint i = 0; i < slotStock[addyArray[addyArrayIndex]].length; i++){
            slotStock[addyArray[addyArrayIndex]][i] = slotStock[addyArray[addyArrayIndex]][i+1];
        }
        delete slotStock[addyArray[addyArrayIndex]][slotStock[addyArray[addyArrayIndex]].length]; //very last element
    }
