...
contract Blackhole is IERC721Receiver, ReentrancyGuard, Ownable {
    using SafeMath for uint256;

    // Set the external contract references

    IERC721 public spaceBatsUtilityToken;
    IERC20 public spaceToken;

    function setSpaceBatsUtilityToken(IERC721 _spaceBatsUtilityToken) public onlyOwner {
        spaceBatsUtilityToken = _spaceBatsUtilityToken;
    }

    function setSpaceToken(IERC20 _spaceToken) public onlyOwner {
        spaceToken = _spaceToken;
    }

    uint256 public maximumSpace;

    function setMaximumSpace(uint256 _maximumSpace) public onlyOwner {
        maximumSpace = _maximumSpace;
        emit MaximumSpaceUpdated(maximumSpace);
    }

    // Set external contract references so that they can be assigned
    address public spaceBatsUtilityContractAddress;
    address public spaceContractAddress;
    ...
    function setSpaceContractAddress(address _spaceContractAddress) public onlyOwner {
        spaceContractAddress = _spaceContractAddress;
    }
    ...
    * @notice Staked event is triggered whenever a user stakes tokens, address is indexed to make it filterable
    ...
    event MaximumSpaceUpdated(uint256 maximumSpace);
    ...
    modifier onlyStaker(uint256 tokenId) {
        ...
        require(spaceBatsUtilityToken.ownerOf(tokenId) == address(this), "onlyStaker: Contract is not owner of this NFT");
    ...
    // constructor(
    //     IERC721 _spaceBatsUtilityToken,
    //     IERC20 _spaceToken,
    ...
    function onERC721Received(
        ...
        require(from == address(0x0), "No sending tokens directly to Blackhole");
        return IERC721Receiver.onERC721Received.selector;
    }

    //User must give this contract permission to take ownership of it.
    function stakeSpaceBat(uint256 tokenId) nonReentrant external {
        // SpaceBatsUtility spaceBatsUtility = SpaceBatsUtility(spaceBatsUtilityContractAddress);
        // spaceBatsUtility.app

        // require(account == _msgSender() || _msgSender() == address(spaceBatsUtilityToken), "Invalid staking address");
        // require(spaceBatsUtilityToken.ownerOf(tokenId) == _msgSender(), "You have to owhn the token to stake it");
        // allow for staking multiple NFTS at one time.
        // for (uint256 i = 0; i < tokenId.length; i++) {
        //     _stakeSpaceBat(tokenId[i]);
        // }

        _stakeSpaceBat(tokenId);

    }

    function _stakeSpaceBat(uint256 tokenId) internal returns (bool) {
      ...
      // Mappings in solidity creates all values, but empty, so we can just check the address
        ...
        require(spaceBatsUtilityToken.ownerOf(tokenId) != address(this), "Stake: Token is already staked in this contract");

        // Approve address for transfer
        // Allows non-owner wallets to stake their NFT
        // spaceBatsUtilityToken.approve(address(this), tokenId);

        // take possession of the NFT
        spaceBatsUtilityToken.transferFrom(_msgSender(), address(this), tokenId);

        // check that this contract is the owner
        require(spaceBatsUtilityToken.ownerOf(tokenId) == address(this), "Stake: Failed to take possession of NFT");
    ...
    function unstakeSpaceBat(uint256 tokenId) nonReentrant external {
        _unstakeSpaceBat(tokenId);
    }

    function _unstakeSpaceBat(uint256 tokenId) internal onlyStaker(tokenId) requireTimeElapsed(tokenId) returns (bool) {
        ...
        // return token
        spaceBatsUtilityToken.safeTransferFrom(address(this), _msgSender(), tokenId);
        // spaceBatsUtility.safeTransferFrom(address(this), _msgSender(), tokenId);
    ...
    function getSpaceBalance() public view returns (uint256) {
        // Space space = Space(spaceContractAddress);
        return spaceToken.balanceOf(address(this));
    }
    ...
    function reclaimTokens() external onlyOwner {
        // Space space = Space(spaceContractAddress);
        spaceToken.transfer(msg.sender, spaceToken.balanceOf(address(this)));
    }
    ...
    function _payoutStake(uint256 tokenId) internal {
        ...
        if (spaceToken.balanceOf(address(this)) < payout) {
            emit StakePayout(msg.sender, tokenId, 0, receipt[tokenId].stakedFromBlock, block.number);
            return;
        }

        // payout stake
        // spaceToken.transferController(receipt[tokenId].owner, payout);
        spaceToken.transfer(receipt[tokenId].user, payout);
...
contract SpaceBatsUtility is ERC721, ERC721Enumerable, Pausable, Ownable, ReentrancyGuard {
    ...
    * @param _blackhole the address of the Barn
    */
    function setBlackholeAddress(address _blackhole) external onlyOwner {
        blackhole = IBlackhole(_blackhole);
    }
    ...
    //     // if (_msgSender() != address(blackhole))
...
interface IBlackhole {
    function stakeNFT(address account, uint256 tokenId) external;
