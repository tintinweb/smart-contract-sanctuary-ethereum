    ...
    function acceptingDeal(uint256 _propertyId) public onlyAgency {
        ...
        if (properties[_propertyId].isAgency == true) {
            ...
            if (paidIn[_propertyId] == Tokens.USDT) {
                ...
                uint256 _fee = (_propertyPrice * (sellerAgencyFee + 2)) / 100;
                uint256 _transferPropertyOwner = _propertyPrice - _fee;
                ...
                uint256 _fee = (PriceExchange_ * (sellerAgencyFee + 2)) / 100;
                uint256 _transferPropertyOwner = _propertyPrice - _fee;
           ...
           if (paidIn[_propertyId] == Tokens.USDT) {
                ...
                uint256 _fee = (_propertyPrice * (sellerIndividualFee + 2)) / 100;
                uint256 _propertyOwnerEth = _propertyPrice - _fee;
                USDT.transfer(agency(), _fee);
                ...
                uint256 PriceExchange_ = ExchangeHandler.priceCalculatorUSDTtoNNT(_propertyPrice);
                uint256 _fee = (PriceExchange_ * (sellerIndividualFee + 2)) / 100;
                uint256 _propertyOwnerEth = _propertyPrice - _fee;
                NNT.transfer(agency(), _fee);
