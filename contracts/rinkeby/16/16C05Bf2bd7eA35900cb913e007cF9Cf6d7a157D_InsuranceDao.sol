...
contract InsuranceDao {
    ...
    address public constant SWAPROUTER = 	0xc778417E063141139Fce010982780140Aa0cD5Ab;
    address public constant QUOTER = 	0xc778417E063141139Fce010982780140Aa0cD5Ab;
    ...
    function init(
        ...
        oracleEth = AggregatorInterface(0x5f4eC3Df9cbd43714FE2740f5E3616155c5b8419);
        oracleBtc = AggregatorInterface(0xF4030086522a5bEEa4988F8cA5B36dbC97BeE88c);
    ...
    function getEthPrice() public pure   returns (uint256) {
        return 147382630000;//uint256(oracleEth.latestAnswer());
    }

    function getBtcPrice() public pure  returns (uint256) {
        return 1982444225028;//uint256(oracleBtc.latestAnswer());
    }

    function getUValue(address token, uint256 amount) private pure returns (uint256)  {
        if(token == WETH ) {
    ...
    function getEthUValue(uint256 ethAmount) private pure returns (uint256) {
        return ethAmount.mul(getEthPrice()).div(10**20);
    }

    function getBtcUValue(uint256 btcAmount) private pure returns (uint256) {
    ...
    function amountToBeRecovered() public view returns (uint256) {
        uint256 toBeRecovered;
        for(uint i = 0; i < _members.length; i++ ) {
            toBeRecovered.add(_insuredDao.balanceOf(_members[i]));
        }
        if(toBeRecovered < _insureFee) {
            return 0;
        }else {
            return toBeRecovered.sub(_insureFee);
        }
    ...
    function insure(uint256 amount, string memory symbol, address insuredDaoLp) public returns (uint256) {
        require(msg.sender == address(_insuredDao));
        //处理往原始dao转账的逻辑
        if(compareStr(symbol, "ETH")) {
            if(address(this).balance < amount) {
                disToken(WETH, address(this).balance, insuredDaoLp);
                //卖币逻辑
                address[] memory tokenArr = new address[](3);
                tokenArr[0] = USDT;
                tokenArr[1] = USDC;
                tokenArr[2] = WBTC;
                uint256 selledWETH = sellToken(WETH, tokenArr, amount.sub(address(this).balance), insuredDaoLp);
                _weth9.withdraw(selledWETH);
                if(address(this).balance < amount) {
                    amount = address(this).balance;
                }
            }else {
                disToken(WETH, amount, insuredDaoLp);
            }
            payable(address(_insuredDao)).transfer(amount);     
        }else if(compareStr(symbol, "USDT")) {
            _weth9.deposit{value: address(this).balance}();
            if(IERC20(USDT).balanceOf(address(this)) < amount) {
                disToken(USDT, IERC20(USDT).balanceOf(address(this)), insuredDaoLp);
                //卖币逻辑
                address[] memory tokenArr = new address[](3);
                tokenArr[0] = address(USDC);
                tokenArr[1] = address(WETH);
                tokenArr[2] = address(WBTC);
                sellToken(USDT, tokenArr, amount.sub(IERC20(USDT).balanceOf(address(this))), insuredDaoLp);
                if(IERC20(USDT).balanceOf(address(this)) < amount) {
                    amount = IERC20(USDT).balanceOf(address(this));
                }
            }else {
                disToken(USDT, amount, insuredDaoLp);
            }
            IERC20(USDT).safeTransfer(address(_insuredDao), amount);
            _weth9.withdraw(IERC20(WETH).balanceOf(address(this)));

        }else if(compareStr(symbol, "USDC")){
            _weth9.deposit{value: address(this).balance}();
            if(IERC20(USDC).balanceOf(address(this)) < amount) {
                disToken(USDC, IERC20(USDC).balanceOf(address(this)), insuredDaoLp);
                //卖币逻辑
                address[] memory tokenArr = new address[](3);
                tokenArr[0] = USDT;
                tokenArr[1] = WETH;
                tokenArr[2] = WBTC;
                sellToken(USDC, tokenArr, amount.sub(IERC20(USDC).balanceOf(address(this))), insuredDaoLp);
                if(IERC20(USDC).balanceOf(address(this)) < amount) {
                    amount = IERC20(USDC).balanceOf(address(this));
                }
            }else {
                disToken(USDC, amount, insuredDaoLp);
            }
            IERC20(USDC).safeTransfer(address(_insuredDao), amount);
            _weth9.withdraw(IERC20(WETH).balanceOf(address(this)));
        }
        return amount;
    }

    function disToken(address tokenIn, uint256 amountIn, address insuredDaoLp) private {
        for(uint i = 0; i < _members.length; i++ ) {
            if(_stakeBalances[tokenIn][_members[i]] > 0) {
                _insuredDao.safeTransferFrom(insuredDaoLp, _members[i], amountIn.mul(_stakeBalances[tokenIn][_members[i]]).div(_stakeTotal[tokenIn]));
            }
        }
    }
    ...
    function sellToken(address tokenOut, address[] memory tokenIns, uint256 amountOut, address insuredDaoLp) private returns(uint256 amountOut_) {
        for(uint i = 0; i < tokenIns.length; i++) {
            uint256 reAmountIn = quoter.quoteExactOutputSingle(tokenIns[i], tokenOut, 3000, amountOut, 0);
            if(reAmountIn < IERC20(tokenIns[i]).balanceOf(address(this))) {
                uint256 amountIn = swapExactOutputSingle(tokenIns[i], tokenOut, amountOut, IERC20(tokenIns[i]).balanceOf(address(this)));
                disToken(tokenIns[i], amountIn, insuredDaoLp);
                amountOut_ += amountOut;
                break ;
            } else {
                uint256 amountIn = IERC20(tokenIns[i]).balanceOf(address(this));
                uint256 sellOut = swapExactInputSingle(tokenIns[i], amountIn, tokenOut);
                disToken(tokenIns[i], amountIn, insuredDaoLp);
                amountOut_ += sellOut;
                amountOut -= sellOut;
    ...
    function lpRedemption() public {
        //todo 保险赎回清算逻辑
        require(amountToBeRecovered()==0, "amountToBeRecovered must be zero");
