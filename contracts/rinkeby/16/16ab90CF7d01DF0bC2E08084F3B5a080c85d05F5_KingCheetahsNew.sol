...
contract KingCheetahsNew is Ownable, ERC721A, ReentrancyGuard {    
    // constants
    uint256 constant MAX_FREE_ELEMENT = 1000;    // 1000
    uint256 constant MAX_ELEMENT_PER_USER = 10;

    uint256 constant MAX_ELEMENTS = 4000;       // 4000
    uint256 constant PUBLIC_PRICE = 0.004 ether;

    // state variable
    bool public MINTING_PAUSED = true;
    string public baseTokenURI;
    string public _contractURI = "";
    mapping(address => uint256) public freeMemberList;

    constructor() ERC721A("KingCheetahsNew", "KINGCHEETAHS_NEW", 10) {}

    function setPauseMinting(bool _pause) public onlyOwner {
        MINTING_PAUSED = _pause;
    }

    function freeMint(uint256 numberOfTokens) external nonReentrant {
        require(!MINTING_PAUSED, "Minting is not active");
        require(totalSupply() < MAX_FREE_ELEMENT, 'All free tokens have been minted');
        require(totalSupply() + numberOfTokens <= MAX_FREE_ELEMENT, 'Free purchase would exceed max supply');
        require(freeMemberList[msg.sender] + numberOfTokens <= MAX_ELEMENT_PER_USER, 'Your free purchase would exceed max(10) supply');
