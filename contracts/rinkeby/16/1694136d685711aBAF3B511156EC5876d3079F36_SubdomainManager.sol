...
contract SubdomainManager is IManager, IERC721Receiver, Ownable, ERC165, ERC721, IERC2981 {
    ...
    mapping(uint256 => uint256) public OwnerFunds;
    ...
    constructor() ERC721("ENS sub-domains", "ESD"){
         MetaData = MetadataProviderV1(address(this));
    }
    ...
    function withdrawEns(string calldata _label) external {
        uint256 id = getIdFromLabel(_label);
        require(msg.sender == TokenOwnerMap[id] || msg.sender == EnsToken.ownerOf(id), "not owner");
        require(!TokenLocked[id], "token is locked");
        EnsToken.safeTransferFrom(address(this), msg.sender, id);
    }
    ...
    function getIdFromLabel(string calldata _label) private pure returns(uint256 id) {
        bytes32 labelHash = keccak256(abi.encodePacked(_label));
        id = uint256(labelHash);
    }

    function ownerRegisterSubdomain(uint256 _id, string memory _label) public tokenOwner(_id) {
    ...
    function purchaseSubdomain(uint256 _tokenId, string memory _label) payable public {
        ...
        OwnerFunds[_tokenId] = OwnerFunds[_tokenId] + msg.value;
    }
    ...
    function text(bytes32 node, string calldata key) external view returns (string memory) {
        ...
        if(keccak256(abi.encodePacked(key)) == keccak256("avatar") && keccak256("") == keccak256(abi.encodePacked(_val))){
    ...
    function withdraw(uint256 _id) external tokenOwner(_id) {        
        require(OwnerFunds[_id] > 0, "no funds to claim");
        
        uint256 balance = OwnerFunds[_id];
        OwnerFunds[_id] = 0;
        payable(TokenOwnerMap[_id]).transfer(balance);
    }
