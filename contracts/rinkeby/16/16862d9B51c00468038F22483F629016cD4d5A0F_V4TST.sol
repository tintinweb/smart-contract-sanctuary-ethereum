...
contract V4TST is ERC721A, Ownable {
    ...
    uint private constant MAX_SUPPLY = 7887;
    uint private constant MAX_TEAM = 789;
    uint private constant MAX_TX = 1;
    uint private constant MAX_PRESALE_WALLET = 3;
    uint private constant MAX_WALLET = 5;
    ...
    constructor(string memory _baseURI, bytes32 _merkleRoot) ERC721A("V4", "V4TST")
    {
        baseURI = _baseURI;
    ...
    function whitelistMint(address _account, uint _quantity, bytes32[] calldata _proof) external payable isNotContract {
        ...
        require(balanceOf(msg.sender)+ _quantity <= MAX_PRESALE_WALLET, "Exceeded Max per Wallet");
    ...
    function publicMint(address _account, uint _quantity) external payable isNotContract {
        ...
        require(totalSupply() + _quantity <= MAX_SUPPLY, "NFT can't be minted anymore");
        require(_quantity <= MAX_TX, "Exceeded Max per TX");
        require(balanceOf(msg.sender)+ _quantity <= MAX_WALLET, "Exceeded Max per Wallet");
        if(FreeMintCount[msg.sender] < 1){
		    if (_quantity >1) { payCount = _quantity - 1; }
            else{ payCount = 0; }
		FreeMintCount[msg.sender] = 1;
