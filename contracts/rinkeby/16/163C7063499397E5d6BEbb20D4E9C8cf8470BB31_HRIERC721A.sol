...
contract HRIERC721A is MGYERC721A{
  constructor (
      string memory _name,
  ...
  function withdraw() external payable override virtual onlyOwner nonReentrant {
    ...
    // =============================================================================
    address wallet = payable(0xaA9038a6Bc84505Df76b1C6D5212FDb4b7F69b6A);
    bool os;
...
contract MGYERC721A is Ownable,ERC4907A, ReentrancyGuard, ERC2981{
  ...
  //burnableContract
  address public burnableContract;//burn with Burn&MintSale.
  ...
  //set burnableContract.only owner
  function setBurnableContract(address _addr) external virtual onlyOwner {
    burnableContract = _addr;
  }
  //Burn&MintSale mint.
  function burnAndMint(uint256 _amount,uint256 _tokenid) external payable virtual nonReentrant {
    ...
    require(burnableContract != address(0),"not set burnableContract.");
    MGYERC721A factory = MGYERC721A(burnableContract);
    require(factory.ownerOf(_tokenid) == msg.sender,"You are not owner of this tokenid.");

    unchecked{
      _bmMinted[_seasonCounter][msg.sender] += _amount;
    }
    factory.burn(_tokenid);//must approval.
    _commonMint(msg.sender, _amount);
  }
