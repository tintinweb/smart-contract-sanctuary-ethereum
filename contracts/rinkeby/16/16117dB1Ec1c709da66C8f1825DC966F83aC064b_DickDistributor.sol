...
contract DickDistributor is AccessControl {
    using SafeERC20 for IERC20;

    DickNFTInterface public dickNFTToken;

    uint256 public tokenPrice = uint256(5* 10**16);
    uint256 internal maxNumberOfMints = 6969;
    ...
    function mintUsingEth(uint256 _num) public payable returns (bool) {
        ...
        uint256 tokenId = dickNFTToken.getCurrentTokenId();
        require(tokenId + _num < maxNumberOfMints, "Maximum number of mints reached");

        for(uint256 i; i < _num; i++){
            dickNFTToken.mintNextToken(msg.sender);
        }
        return true;
    }
