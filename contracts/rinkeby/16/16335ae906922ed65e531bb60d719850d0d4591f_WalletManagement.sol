...
contract WalletManagement is Owner {
  ...
  // ========= EVENT ========= //
  event WalletAdded(WalletConfig _walletConfigs);
  event WalletRemoved(WalletConfig _walletConfigs);
  event WalletUpdated(WalletConfig _walletConfigs);
    ...
    isOwner
  {
    for (uint256 i = 0; i < _walletConfigs.length; i++) {
      ...
      emit WalletAdded(walletConfig.config);
    }
  }
  ...
  function removeWallets(string[] calldata _keys) external isOwner {
    for (uint256 i = 0; i < _keys.length; i++) {
      ...
      emit WalletRemoved(walletConfig);
    }
  }
