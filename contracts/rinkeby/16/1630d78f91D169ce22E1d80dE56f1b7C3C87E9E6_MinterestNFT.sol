 ...
 *      Each token connected with image. The image saves on IPFS and this contract
 *      store the multihash. Also each token belongs one of the Minterest tiers, and give
 *      some emission boost for Minterest distribution system.
 */
contract MinterestNFT is ERC1155, AccessControl {
    ...
    /// @notice Emitted when new Multihash was created
    event TokenMultihash(uint256 indexed id, bytes32 digest, uint8 hashFunction, uint8 size);
     ...
     * @param _name The name of ERC1155 standard contract
    ...
    constructor(
        ...
        require(_proxyRegistryAddress != address(0), "Address Zero");
    ...
    function mintBatch(
        ...
        require(tiers_.length == amounts_.length, "Error");
