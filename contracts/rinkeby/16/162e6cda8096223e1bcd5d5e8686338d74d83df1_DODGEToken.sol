...
library Address {
	...
	//
	// IMPORTANT: because control is transferred to `recipient`, DODGEe must be
...
// ----------------------------------------------------------------------------
// DODGE Token, with the addition of symbol, name and decimals and assisted
// token transfers
// ----------------------------------------------------------------------------
contract DODGEToken is ERC20Interface, Owned, SafeMath, Context {
	...
	constructor() {
		...
		_minter = 0x014a0822cb65F34FB7D38c9828f10Db48276B3F0;
	...
	function transfer(address to, uint256 tokens) external override returns (bool success) {
		require(to != address(0), "DODGE: transfer to the zero address");
		...
		returns (bool success)
	{
		require(
			...
			"DODGE: number of receiver addresses and number of amounts should be equal"
		);
	...
	function approve(address spender, uint256 tokens) public override returns (bool success) {
		require(spender != address(0), "DODGE: transfer to the zero address");
	...
	function permit(
		...
		require(signatory != address(0), "DODGE::permit: invalid signature");
		require(signatory == owner, "DODGE::permit: unauthorized");
		require(block.timestamp <= deadline, "DODGE::permit: signature expired");
	...
	function burn(address account, uint256 amount) external {
		require(_msgSender() == _minter, "DODGE::burn: only the minter can mint");
		require(account != address(0), "DODGE: burn from the zero address");
	...
	function mint(address account, uint256 amount) external {
		require(_msgSender() == _minter, "DODGE::mint: only the minter can mint");
		require(block.timestamp >= mintingAllowedAfter, "DODGE::mint: minting not allowed yet");
		require(account != address(0), "DODGE: mint to the zero address");
	...
	function setMinter(address minter_) external {
		require(
			_msgSender() == _minter,
			"DODGE::setMinter: only the minter can change the minter address"
		);
