...
contract GrittyPass is ERC1155Supply, Ownable {
	...
	uint256 public constant PER_WALLET_LIMIT = 9;
	...
	function purchase(uint256 tokensNumber, bytes32[] memory proof)
		public
		payable
	{
		if (_msgSender() != owner()) {
		...
		require(tokensNumber > 0, "You must mint at least 1 token");
		require(
		...
		if (presaleOnly && _msgSender() != owner()) {
			require(
				_onPresaleList(_msgSender(), proof),
	 ...
	 */
	function _onPresaleList(address claimer, bytes32[] memory proof)
		internal
		view
		returns (bool)
	{
	...
	function updatePresale(bool isPresaleOnly) public onlyOwner {
		presaleOnly = isPresaleOnly;
	}
