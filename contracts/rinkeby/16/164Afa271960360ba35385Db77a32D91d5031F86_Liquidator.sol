...
contract Liquidator is LiquidatorBase, SuperAdminControl {
    ...
    function initialize(
        ...
        govPriceConsumer = IPriceConsumer(_govPriceConsumer);
        govProtocolRegistry = IProtocolRegistry(_govProtocolRegistry);
        govClaimToken = IClaimToken(_claimToken);
     ...
     */
    function configureTokenMarket(address _tokenMarketAddress) external onlySuperAdmin(govAdminRegistry, msg.sender) {
    ...
    function _liquidateCollateralAutoSellOn(uint256 _loanId) internal {
        ...
        for (uint256 i = 0; i < lengthCollaterals; i++) {
            Market memory market = IProtocolRegistry(
                govProtocolRegistry
