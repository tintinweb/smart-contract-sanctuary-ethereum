...
contract PedroCoin is ERC721, ERC721Enumerable, Pausable, Ownable, ERC721Burnable {
    ...
    uint128 startTime = 30 * 1 seconds; 
    uint128 public mintingPrice = 2 gwei;
    uint256 startSale = 1663264828;
    uint256 internal nonce = 0;
    uint256[MINTING_LIMIT] internal indices;
    bool internal isReentrancyLock = false;
    
    constructor(string memory _URI) ERC721("PedroCoin", "PC") {
        baseURI = _URI;
    }

    function Mint(uint256 amount) public payable mintingEnabled reentrancyGuard {

        require(msg.value >= mintingPrice, "Insufficient funds to purchase");        
        require(amount >= 1, "You cannot mint lower than 1 NFT");
        require(amount <= 20, "You cannot mint more than 20 NFTs per once");
        require((totalSupply() + amount) < MINTING_LIMIT, "Exceeds maximum supply. Please try to mint less NFTs.");
  ...
  function _tryGenerateRandomId() private returns (uint256) {
    ...
    function withdrawal (address _to) public onlyOwner reentrancyGuard {
        require(address(this).balance > 0 , "Not balance");
        payable (_to).transfer(address(this).balance);
        
    }

    // function updateBaseURI(string memory _URI) public onlyOwner reentrancyGuard{
    //     baseURI = _URI;
    // }

    modifier mintingEnabled() {
        require(block.timestamp > startSale , "Can't start. wait please.");
