    ...
    ResourceImplementationPointer
{
    ...
    function stakeTokens(uint256 _nftId, uint256 _amount) external nonReentrant whenNotPaused resourceSet {
        ...
        require(block.timestamp >= user.lastProcessingTimestamp + compoundDelay, "NFTs: You must wait by next time");
    ...
    function _getNFTCashoutRewardsFromGift(uint256 _nftId) private returns (uint256) {
        ...
        uint256 reward = _calculateRewardsFromValue(nft.giftValue, nft.lastProcessingTimestamp - block.timestamp, rewardPerDay);
    ...
    function changeGiftFee(uint256 _giftFee) public onlyOwner {
        require(_giftFee <= 30, "NFTs:  Fee must be less than 30");
        giftFee = _giftFee;
    }
