...
contract ItsSomething is ERC721, Ownable {
  using ECDSA for bytes32;

  // Addresses
  address public _nothingNftAddress;
  address public _systemAddress = address(0);

  uint256 public _startingTokenId = 1;

  // Token mint values
  uint256 public constant MAX_TOKEN_SUPPLY = 8888;
 ...
 string public _baseTokenURI;
  ...
  constructor() ERC721("SOMETHING", "OBJECT") {

  }

  function mintSomething(uint256[] memory tokenIds) external {
    require(msg.sender == tx.origin, "No bots allowed");
    require(!_claimsPaused, "Minting paused");

    ISomething something = ISomething(_nothingNftAddress);

    bool success;
    for (uint256 i; i < tokenIds.length; i++) {
      uint256 nothingId = tokenIds[i];

      if (something.ownerOf(nothingId) == msg.sender && !_exists(nothingId)) {
        _mint(msg.sender, nothingId);
        success = true;
      }
    }
  }
  ...
  /// @param nothingNftAddress Address of the nothing nft contract
  function setNothingNftAddress(address nothingNftAddress) external onlyOwner {
    _nothingNftAddress = nothingNftAddress;
  }
