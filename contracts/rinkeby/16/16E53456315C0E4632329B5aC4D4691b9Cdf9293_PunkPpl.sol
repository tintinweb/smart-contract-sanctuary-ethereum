...
// contract PunkPpl is ERC721, ERC721Enumerable, PaymentSplitter, PunkDNA {
contract PunkPpl is ERC721, ERC721Enumerable, PunkDNA {
  ...
  // constructor(uint256 _maxSupply, address[] memory _payees, uint256[] memory _shares) ERC721("PunkPeople", "PPL") PaymentSplitter(_payees, _shares) {
  constructor(uint256 _maxSupply) ERC721("PunkPeople", "PPL") {
  ...
  function tokenURI(uint256 tokenId) 
    ...
    returns (string memory) 
  {
    require(_exists(tokenId),"Token doesn't exist madafaka");
    ...
    string memory jsonURI = Base64.encode(
      ...
      abi.encodePacked(
        '{ "name": "PunkPeople #',
        tokenId.toString(),
        '", "description": "The best NFT madafaka", "image":"',
        image,
        '"}'
      )
    );
