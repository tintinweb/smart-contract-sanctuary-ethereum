...
contract LunacatStake is Ownable {
    ...
    constructor() {   
        ...
        addStakePool(
            ...
            0, // Withdrawal Fee 2%
            0 // Unstaking Penalty 0% (For flexy pools 0% unstaking penalty)
        );
        addStakePool(
            baseTokenAddress,
            7, // 30 Days lock pool
    ...
    function getAllPoolDetails() public view returns(stakePool[] memory, uint256[] memory){
        ...
        for(uint256 i=0; i < stakePoolArray.length; i++){
            APYarray[i] = getAPY(rarity,stakePoolArray[i].duration);
        }
        
        return (stakePoolArray,APYarray);
    }

    function getAllPoolsAPYOld() public view returns(uint256[] memory){
        uint256[] memory APYarray;
        uint256 rarity = getRarityToApply();
    ...
    function getAllPoolsAPY() public view returns(uint256,uint256,uint256,uint256,uint256){
        uint256 rarity = getRarityToApply();
        
        uint256 pool1APY = getAPY(rarity,stakePoolArray[0].duration);
        uint256 pool2APY = getAPY(rarity,stakePoolArray[1].duration);
        uint256 pool3APY = getAPY(rarity,stakePoolArray[2].duration);
        uint256 pool4APY = getAPY(rarity,stakePoolArray[3].duration);
        uint256 pool5APY = getAPY(rarity,stakePoolArray[4].duration);
        
        return (pool1APY,pool2APY,pool3APY,pool4APY,pool5APY);
    }
