...
contract TestPEP is ERC721, Ownable, ReentrancyGuard, TokenManager {
  ...
  // ERC721 params
  string private _name = "testpep";
  string private _symbol = "testpep";
  string private _baseTokenURI = "www.test.com/";

  // Smilesssvrs
  ERC721Enumerable public smilesssvrs = ERC721Enumerable(0x79FCDEF22feeD20eDDacbB2587640e45491b757f);
  ...
  function claim(uint256[] calldata tokenIds) external nonReentrant{
    require(tx.origin == msg.sender, "Smart contract interactions are disabled");

    uint256 tot = tokenIds.length;
    uint256 smilesssvrsSupply = smilesssvrs.totalSupply();

    for(uint256 i = 0; i<tot; ++i) {
      uint256 _tokenId = tokenIds[i];
      require(smilesssvrsSupply >= _tokenId, "Smilesssvrs id does not exists");
      require(smilesssvrs.ownerOf(_tokenId) == msg.sender, "Ownership requirements not satisfied");
  ...
  function canClaim(uint256 _tokenId, address _address) public view returns(bool) {
    if(_exists(_tokenId)) {
      return (ownerOf(_tokenId) != _address && smilesssvrs.ownerOf(_tokenId) == _address);
    } else {
      return true;  
    }
  }
  ...
  function claimableBy(address _owner) public view returns (uint256[] memory) {
    uint256 tokenCount = smilesssvrs.balanceOf(_owner);
    uint256 counter = 0;

    for (uint256 i=0; i < tokenCount; i++) {
      uint256 _tokenId = smilesssvrs.tokenOfOwnerByIndex(_owner, i);
  ...
  function setStatus(uint256 _status) external onlyOwner {
    ...
    require(_status >= 0 && _status <= 1, "Mint status must be between 0 and 2");
  ...
  ////////////////////////// REMOVE //////////////////////////
  function setSmilesssvrs(address _address) public onlyOwner {
    smilesssvrs = ERC721Enumerable(_address);
  }
}
