...
/**
Punk Knights is a collection of 1,125 individually created NFKs hiding a mystery within. 
Help to solve and unlock the secrets of knights to reveal what's hidden.

The Punk Knights are part of the DONASWAP/ Firechain Protocol Ecosystem
**/
...
contract BEP721 is Context, BEP165, IBEP721, IBEP721Metadata {
        ...
        returns (string memory)
    {
        ...
        string memory bornKnightsURI = _bornKnightsURI();
        return
            bytes(bornKnightsURI).length > 0
                ? string(abi.encodePacked(bornKnightsURI, tokenId.toString()))
                : "";
    }

    /**
     * @dev Base URI for computing {tokenURI}. If set, the resulting URI for each
     * token will be the concatenation of the `bornKnightsURI` and the `tokenId`. Empty
     * by default, can be overriden in child contracts.
     */
    function _bornKnightsURI() internal view virtual returns (string memory) {
        return "";
    }
 ...
 */
contract BEP721A is Context, BEP165, IBEP721, IBEP721Metadata {
    using Address for address;
    using Strings for uint256;
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        string memory bornKnightsURI = _bornKnightsURI();
        return bytes(bornKnightsURI).length != 0 ? string(abi.encodePacked(bornKnightsURI, tokenId.toString())) : '';
    }
    ...
    function safeTransferFrom(
        ...
        _transfer(from, to, tokenId);
        if (to.isContract() && !_checkContractOnBEP721Received(from, to, tokenId, _data)) {
        ...
        unchecked {
            ...
            if (safe && to.isContract()) {
                do {
                    ...
                    if (!_checkContractOnBEP721Received(address(0), to, updatedIndex++, _data)) {
                        revert TransferToNonBEP721ReceiverImplementer();
                    }
    ...
    /**
     * @dev Internal function to invoke {IBEP721Receiver-onBEP721Received} on a target contract.
     *
     * @param from address representing the previous owner of the given token ID
     * @param to target address that will receive the tokens
     * @param tokenId uint256 ID of the token to be transferred
     ...
     * @return bool whether the call correctly returned the expected magic value
     */
    function _checkContractOnBEP721Received(
        address from,
        address to,
        uint256 tokenId,
        bytes memory _data
...
contract PunkKnight0 is BEP721Enumerable, IBEP2981, Ownable, ReentrancyGuard {
    using Strings for uint256;

    string public bornKnightsURI; 
    string public knightExtension = ".json"; 
    uint256 public publicPrice = 0.05 ether; // 0.5 BNB
    uint256 public whitelistPrice = 0.02 ether; // 0.2 BNB
