...
contract ByeBanx is Ownable, ERC721A, ReentrancyGuard {
    ...
    string public baseURI =
        "ipfs://QmQg8XwNyrEuLSNfsaCXUAGBekwS5vRRfo25165ZseaKHB/";
    ...
    function privateSaleMint(uint256 _mintAmount, bytes memory signature)
        external
        payable
    {
        ...
        require(
            totalSupply() + _mintAmount <= privateMaxSupply,
            "Purchase would exceed max supply of private sale"
        );
        require(
            balanceOf(msg.sender) + _mintAmount <= group,
            "Minting would exceed max per wallet for presale"
        );
        mintX(_mintAmount);
    }

    function mintX(uint256 _mintAmount) private {
