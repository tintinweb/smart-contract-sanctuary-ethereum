...
contract Fartz is Ownable, ERC721A, IERC2981, ReentrancyGuard {
    /* ========== DEPENDENCIES ========== */
    ...
    uint64 public PUBLIC_SALE_PRICE = 0.003 ether;
    ...
    string private constant svgStart = '<svg id="fartz.wtf" xmlns="http://www.w3.org/2000/svg" width="700" height="700"><rect x="0" y="0" width="700" height="700" fill="black"/> <text x="50%" y="388" text-anchor="middle" font-weight="bold" font-size="77px" fill="white">';
    string private constant svgEnd = '</text></svg>';
    ...
    constructor(address owner1_, address owner2_, address dev_) ERC721A("Fartz", "FRTZ") {
        _owner1 = payable(owner1_);
        _owner2 = payable(owner2_);
        _dev = payable(dev_);


        // bytes32 fart_ = _generateFart(0);
        // _registeredDisturbances[0] = fart_; 
        ...
        bytes32 fart_ = 0x0102020201010102020000000000000000000000000000000000000000000000;
        _registeredDisturbances[0] = fart_; 
        _knownDisturbances[fart_] = true; 
    ...
    nonReentrant
    {
        ...
        for (uint256 i = 0; i < quantity_; i++) {
            bytes32 fart_ = _generateFart(supply_ + i);
    ...
    // Add a mint promotional NFT

    /* ========== INTERNAL FUNCTION ========== */

    function _generateFart(uint256 tokenId_) internal returns (bytes32) {
        // Create the seed
        uint256 seed_ = uint256(
            keccak256(abi.encode(block.timestamp, msg.sender, tokenId_))
        );
    ...
    function _buildFart(uint256 tokenId_) public view returns (string memory properties, string memory svg) {
    ...
    function withdraw() public {
        uint256 split_ = address(this).balance / 3;
