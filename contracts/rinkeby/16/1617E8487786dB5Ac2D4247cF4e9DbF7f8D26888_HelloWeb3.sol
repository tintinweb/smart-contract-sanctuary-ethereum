	...
	LockedPaymentSplitter
{
	...
	constructor(
		...
		_uris[
			PASS_ALL_ACCESS_ID
		] = "ipfs://QmeHCkytK7vesf8rC4UxvFiRHYFxr44apHSbQ1Ak3ZnJTt/0.json";
		_maxSupplies[PASS_ALL_ACCESS_ID] = 200;
		_maxBatchSizes[PASS_ALL_ACCESS_ID] = 200;

		// Events Only Pass
		_costs[PASS_EVENTS_ONLY_ID] = 0.05 ether;
		_uris[
			PASS_EVENTS_ONLY_ID
		] = "ipfs://QmeHCkytK7vesf8rC4UxvFiRHYFxr44apHSbQ1Ak3ZnJTt/1.json";
		_maxSupplies[PASS_EVENTS_ONLY_ID] = 200;
		_maxBatchSizes[PASS_EVENTS_ONLY_ID] = 200;
	}
	...
	function _isMaxTokensPerWallet(uint256 id, uint256 quantity) internal view {
		if (_msgSender() != owner()) {
			uint256 mintedBalance = _mintedPerAddress[_msgSender()][id];
	...
	function _isLessThanMaxSupply(uint256 id, uint256 quantity) internal view {
		uint256 maxSupply = _maxSupplies[id];
		if (totalSupply(id) + quantity > maxSupply) {
			revert MaxSupplyExceeded(totalSupply(id) + quantity, maxSupply);
		}
	}
	...
	function reserveSingle(
		...
		uint256 maxBatchSize = _maxBatchSizes[id];
		uint256 multiple = quantity / maxBatchSize;
		for (uint256 i = 0; i < multiple; i++) {
			_internalMintTokens(fren, id, maxBatchSize);
		}
		...
		onlyUsers
	{
		...
		if (_msgSender() != owner()) {
			_mintedPerAddress[_msgSender()][id] += quantity;
		}
