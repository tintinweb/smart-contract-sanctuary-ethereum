...
contract ProjectXWL is ERC721X, Ownable, ReentrancyGuard {
    bool publicSale = true;
    uint256 nbFree = 444;
    uint256 price = 0.001 ether;
    uint256 collectionSize = 8888;
    string _strBaseURI;

    mapping(uint256 => uint256) public spawnDate;

    constructor() ERC721X("Project XWL", "META") {}
    ...
    function updateSpawnDate(uint256 tokenId) internal {
        spawnDate[tokenId] = block.number;
    }

    function freeMint(uint256 quantity) external callerIsUser {
        require(publicSale, "Public sale has not begun yet");
        require(nextId + quantity <= nbFree, "Reached max free supply");
        require(quantity <= 1, "can not mint this many free at a time");
        for (uint256 i = nextId; i < nextId + quantity; i++) {
            updateSpawnDate(i);
        }
    ...
    function mint(uint256 quantity) external payable callerIsUser {
        require(publicSale, "Public sale has not begun yet");
        require(nextId + quantity <= collectionSize, "Reached max supply");
        require(quantity <= 1, "can not mint this many at a time");
        require(
    ...
    function getAge(uint256 tokenId) public view returns (uint256) {
        ...
        return block.number - spawnDate[tokenId];
    }
    ...
    //                     uint256(getAge(tokenId)).toString()
    //                 )
    //             )
    //             : "";
    // }
