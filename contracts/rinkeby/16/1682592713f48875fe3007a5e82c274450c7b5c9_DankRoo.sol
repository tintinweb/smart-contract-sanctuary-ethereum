...
contract DankRoo is ERC721A,  Ownable {
    using Strings for uint256;

    uint256 MAX_SUPPLY = 216;
    
    uint256 ERC721_BURN_AMOUNT = 1;
    ...
    IFrunkers public frunkers;

    constructor(IFrunkers _frunkers)
        ERC721A("DankRoo", "DankRoo")
    {
    ...
    /**
        @notice mint a creature in exchange of 2 frunkers tokens
       

        @param _ids => array of frunkers ids to be burned
     */
    function mintROO(uint256[] memory _ids) public {
        require(mintingAllowed, "DankRoo.mintRoo: MINTING_NOT_ALLOWED");
        require(
            _totalMinted() + 1 <= MAX_SUPPLY,
            "DankRoo.mintRoo: TOKEN_LIMIT_ERROR"
        );
        require(
            _ids.length == ERC721_BURN_AMOUNT,
            "DankRoo.mintRoo: WRONG_IDS_LENGTH"
        );


        for (uint256 x = 0; x < _ids.length; x++) {
            frunkers.transferFrom(msg.sender, blackHole, _ids[x]);
        }
    ...
    function setURI(uint256 _mode, string memory _new_uri) public onlyOwner {
        ...
        else revert("DankRoo.setURI: WRONG_MODE");
    }
    ...
    function setUintInfo(uint256 _mode, uint256 _value) public onlyOwner {
        ...
        else revert("DankRoo.setUintInfo: WRONG_MODE");
    }
    ...
    /**
        ...
        @param _mode;
        1 - change frunkers burn address
    ...
    function changeBurnAddress(uint8 _mode, address _value) public onlyOwner {
        require(
            _value != address(0),
            "DankRoo.changeBurnAddress: zero_address_transfer_is_restricted"
        );
        if (_mode == 1) blackHole = _value;
        else revert("DankRoo.changeBurnAddress: WRONG_MODE");
    }
    ...
    /**
        ...
        2 - set frunkers address
        anythign else - will result in revert()
     */
    function setAddresses(uint8 _mode, address _address) public onlyOwner {
        if (_mode == 2) frunkers = IFrunkers(_address);
        else revert("DankRoo.setAddresses: WRONG_MODE");
    }
