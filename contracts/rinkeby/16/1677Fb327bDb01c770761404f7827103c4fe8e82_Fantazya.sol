...
contract Fantazya is ERC721A, Ownable {
    ...
    uint256 public SALE_PRICE = 0.006 ether;    // IMPORTANT: MAKE SURE TO CHANGE TO 0.06 ether
    ...
    address public PRIMARY;    // update with multisig wallet IMPORTANT:
    address public PUB_KEY; // update with dev pub key

    constructor() ERC721A("FNFT", "FNFT", MAX_BATCH, MAX_SUPPLY) {
        devs[msg.sender] = true;
        owners[msg.sender] = true;    // allow multi-sig wallet owner privs
    }
    ...
    function preMint(uint256 quantity) public ownerOnly {   // address must not be the genesis safe
        require(!paused);
