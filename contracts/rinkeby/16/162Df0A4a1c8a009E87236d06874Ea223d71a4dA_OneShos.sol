...
// ...............?SS+..;*%SSSSSSSSSSSSSSSSSSSSSSS%?+,.+SS%......,SSS:..:*%S####S%*;....;*%SSSSSSSSSSSSSSSSSSSSSSS%?+,.............
// ..........................................................................,,....................................................
...
contract OneShos is ERC721A, SalesActivation {
    ...
    address private immutable i_oneShos1155ContractAddress;
    uint64 private constant TOKEN_ID = 1;
    uint256 private s_price = 0.05 ether;
    uint256 private s_max_one_shos = 10000;
    ...
    mapping(address => uint256) private s_PresaleListBought;
    // team addresses
    ...
    {
        i_oneShos1155ContractAddress = oneShos1155ContractAddress;
    ...
    function claimFrom1155(uint256 numberToClaim) external isClaimActive {
        OneShos1155Interface oneShos1155contract = OneShos1155Interface(i_oneShos1155ContractAddress);
        // Check if is approved
        if (!oneShos1155contract.isApprovedForAll(msg.sender, address(this))) {
            revert OneShos__1155NotApproved();
        }
        uint256 oneShos1155Balance = oneShos1155contract.balanceOf(msg.sender, TOKEN_ID);
        ...
        oneShos1155contract.burn(msg.sender, TOKEN_ID, oneShos1155Balance);
        _safeMint(msg.sender, oneShos1155Balance);
    }
    ...
    function presales(uint256 oneshosNumber) external payable isPreSalesActive {
        ...
        require(supply + oneshosNumber <= s_max_one_shos - s_team_allocated, "Exceeds maximum nuts supply");
        require(s_PresaleListBought[msg.sender] + oneshosNumber <= s_max_per_wallet, "Purchase exceeds max allowed");
        require(tx.origin == msg.sender, "Contracts not allowed to mint");
        _safeMint(msg.sender, oneshosNumber);
        s_PresaleListBought[msg.sender] += oneshosNumber;
    ...
    function mint(uint256 oneShosNumber) external payable isPublicSalesActive {
        uint256 supply = totalSupply();
        require(msg.value >= s_price * oneShosNumber, "Ether sent is not correct");
        require(supply + oneShosNumber <= s_max_one_shos - s_team_allocated, "Exceeds maximum one shos supply");
    ...
    function ownerMint(address _to, uint256 oneShosNumber) external onlyOwner {
        uint256 supply = totalSupply();
        require(supply + oneShosNumber <= s_max_one_shos, "Exceeds maximum one shos supply");

        _safeMint(_to, oneShosNumber);
        s_team_allocated -= oneShosNumber;
    }
    ...
    function addToPresaleList(address[] calldata _presale_list) public onlyOwner {
        for (uint256 i = 0; i < _presale_list.length; i++) {
            ...
            s_PresaleListBought[_presale_list[i]] > 0 ? s_PresaleListBought[_presale_list[i]] : 0;
        }
    }
    ...
    function withdraw() public onlyOwner {
        ...
        require(payable(s_one_shos_wallet).send(_balance));
    }

    /* View / Pure Functions */

    function getOneshos1155ContractAddress() external view returns (address) {
        return i_oneShos1155ContractAddress;
    }

    function getTokenIdOf1155Contract() external pure returns (uint64) {
        return TOKEN_ID;
    }
    ...
    function presaleBought(address wallet) external view returns (uint256) {
        return s_PresaleListBought[wallet];
    }
}
