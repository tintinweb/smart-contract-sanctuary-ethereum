    ...
    AccessControl
{
    ...
    uint256 public constant rollScorePrice = 100 ether;
    ...
    modifier onlyOperator() {
        if(!(hasRole(OPERATOR_ROLE, msg.sender)||owner()==_msgSender())) revert CallerIsNotOperator();
        _;
    }
    ...
    function mappedTokenFor(uint _randomSeed) internal returns (uint64) {
        ...
        uint maxIndex = MAX_SUPPLY - lockedSupply-1-_mintCounter;
        uint random = _randomSeed % (maxIndex+1);
...
/** Score Functions */
    ...
    function recordHighScore(uint8 v, bytes32 r, bytes32 s, UserData memory record) external {
    ...
    function saveScore(UserData memory record) internal {
        scores[record.rollerId] = record;
    }

    function getScore(uint _tokenId) public view returns (UserData memory) {
...
/** OWNER FUNCTIONS **/
    ...
    function ownerMint(address to, uint256 quantity) public callerIsUser onlyOperator {
        ...
        for(uint i = 0; i < quantity; i++){
            _mintToken(to, mappedTokenFor((random+i)%(MAX_SUPPLY-lockedSupply)));
        }

    }

    function toggleMint() public onlyOperator {
        mintActive = !mintActive;
    }

    function setReservedSupply(uint _reservedSupply) public onlyOperator {
        reservedSupply = _reservedSupply;
        setPublicSupply();
    }

    function setLockedSupply(uint _lockedSupply) public onlyOperator {
    ...
    function setStartTime(uint64 _startTime) public onlyOperator {
        startTime = _startTime;
    }

    function setRollAddress(address _rollToken) public onlyOperator {
        rollToken = IROLL(_rollToken);
    }

    function setScoreSignerAddress(address _scoreSigner) public onlyOperator {
    ...
    if(!_exists(_id)) revert TokenNonExistent();
		_lockId(_id);
	}
    ...
    /** Standard withdraw function for the owner to pull the contract */
     function withdraw() external onlyOperator {
        ...
        address rollers = payable(0xb2584a9414c54D0DCe290CE85aD8Ae8C1CD8827F); // SET UP MULTI-SIG WALLET
        bool success;
    ...
    ,Ownable
{
        ...
        ) 
    {
        ...
        uint256 generationTokenId = getGenerationToken(_tokenId);
        return string(
            abi.encodePacked(
