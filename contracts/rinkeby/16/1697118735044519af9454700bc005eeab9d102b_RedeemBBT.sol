...
contract RedeemBBT is Owned, Pausable {
    ...
    constructor(address _bbtCNV, address _CNV)
        Owned(msg.sender)
    {
        bbtCNV = _bbtCNV;
        CNV = _CNV;
    ...
    function setPause(bool _toPause)
        external
        onlyOwner
    {
        if (_toPause) _pause();
    ...
    function _percentOf(uint256 _start, uint256 _point, uint256 _length)
        internal
        pure
        returns(uint256 elapsedPct)
    {
    ...
    /// @param  _length     lenght
    /// @return point       point
    function _linearMapping(uint256 _start, uint256 _pct, uint256 _length)
        internal
        pure
