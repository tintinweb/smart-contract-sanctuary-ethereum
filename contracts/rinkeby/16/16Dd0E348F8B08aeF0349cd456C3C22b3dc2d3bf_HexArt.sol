...
contract HexArt is ERC721URIStorage, ERC2981, Ownable {
    ...
    event assetRemoved(uint256 tokenId, uint256[] assetId, address assetContract);
        ...
        isListedOnSale(_tokenId)
    {
        for (uint256 i = 0; i < _tokenIds.length; i++) {
            ...
            require(success, "Asset not added");
    ...
    function removeErc20(address _assetContract, uint256 _nftTokenId) internal {
        for (uint256 i = 0; i < erc20AssetDetails[_nftTokenId].length; i++) {
            if (erc20AssetDetails[_nftTokenId][i].assettype == _assetContract) {
                ...
                require(success, "Asset not removed");
            }
        }
    }
    ...
    {
        // TO-DO
        // Need to add check, if new owner is buyer from HEXArt Marketplace or first owner
    ...
    function removeNFT(
        ...
        for (uint256 i = 0; i < _assetId.length; i++) {
            if (
                isAssetAttached(_nftTokenId,_assetId[i],_assetContract)
            ) {
                require(
    ...
    function isAssetAttached(uint256 _nftTokenId,
        uint256 _assetId,
        address _assetContract
