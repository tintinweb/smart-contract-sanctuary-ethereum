...
// File: contracts/WomenHackersClub_v3.sol
...
contract WomenHackersClub is Ownable, ONFT721 {
    ...
    bool public saleIsActive = true; // need to change that for Mainnet

    bool public PresaleIsActive = true; // need to change that for Mainnet

    mapping(address => uint256) public addressMintedBalance;

    address private a1 = 0x7d52923Ca0135F59B15986FCADeC7107758BbeFd; // need to change that for Mainnet
    ...
    function allowlistMint(uint256 mintCount,uint8 v, bytes32 r,bytes32 s,uint256 mint_allowed,uint256 free) external payable {
        ...
        require(ownerMintedCount + mintCount < mint_allowed + 1, "Individual minting limit exceeded!");
...
function mintpassMint(uint256 mintCount,uint8 v, bytes32 r,bytes32 s,uint256 mint_allowed,uint256 free) external payable {
        ...
        require(free == 2, "you do not have a mintpass");
    ...
    function claim(uint256 mintCount,uint8 v, bytes32 r,bytes32 s,uint256 mint_allowed,uint256 free) external payable {
        ...
        require(free == 1, "Not allowed to claim a free WHC");

        uint256 ownerMintedCount = addressMintedBalance[msg.sender];

        require(ownerMintedCount + mintCount < mint_allowed +1, "Exceeds your allowed free claims");
    ...
    function publicMint(uint256 mintCount) external payable {
        ...
        require(mintCount < perTxLimit + 1, "Quantity per transaction too large");

        uint256 ownerMintedCount = addressMintedBalance[msg.sender];

        require(ownerMintedCount + mintCount + 1 < walletLimit, "Maximum amount of WHC mints for this wallet exceeded!");
