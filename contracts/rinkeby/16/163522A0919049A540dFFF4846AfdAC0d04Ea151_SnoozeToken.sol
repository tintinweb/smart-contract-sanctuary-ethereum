...
contract SnoozeToken is ERC20, Ownable, Pausable, ReentrancyGuard {
    ...
    function setReward(uint16 _reward) external onlyOwner {
        reward = _reward;
    }

    function setBooster(uint16 _booster) external onlyOwner {
        booster = _booster;
    }
    ...
    function available() external view whenNotPaused returns (uint256) {
        ...
        unchecked {
            ...
            amount = count_ * reward * booster * factor + stash[msg.sender];
        }
        return amount;
    }
