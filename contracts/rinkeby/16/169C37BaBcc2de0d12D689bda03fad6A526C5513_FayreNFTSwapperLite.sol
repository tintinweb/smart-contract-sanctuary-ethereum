...
contract FayreNFTSwapperLite is Ownable, IERC721Receiver, IERC1155Receiver {
    ...
    */
    ...
    function createSwap(SwapRequest calldata swapRequest) external payable {
        ...
        _processFee(_msgSender(), swapsData[_currentSwapId].isMultiAssetSwap);

        _transferAsset(swapRequest.seller, address(this), swapsData[_currentSwapId].swapRequest.sellerAssetData);

        usersSwapsIds[swapRequest.seller].push(_currentSwapId);
        usersSwapsCount[swapRequest.seller]++;

        usersSwapsIds[swapRequest.bidder].push(_currentSwapId);
