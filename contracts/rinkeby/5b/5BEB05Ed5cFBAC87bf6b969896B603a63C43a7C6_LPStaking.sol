...
contract LPStaking is Initializable, ReentrancyGuardUpgradeable,OwnableUpgradeable {
   ...
   //poolid=>period=>APR
    ...
    mapping (uint=>stakeDetails) public depositDetails;
    ...
    mapping(uint => periodPool)public periodPoolMap;
     ...
     */
    function initialize(uint _startBlock,address _feeToken, uint[] memory _periods) external initializer {
    ...
    function receiveWETHFee(uint256 amount) external onlyAddress nonReentrant {
        ...
        for(uint i=0;i<periods.length;i++){
            sum +=periods[i]==0?1:periods[i];
            periodPoolMap[periods[i]].totalAllotedFeeReward+=periods[i]==0?(1/sum)*amount:(periods[i]/sum)*amount;
        }
    ...
    function AdminWETHWithdraw() external onlyOwner nonReentrant {
        uint accMarketFee = WETH.balanceOf(address(this));
        ...
        returns (uint256 reward)
    {
        ...
        stakeDetails memory stakeDetail = _stakeHolder!=address(0)?deposits[_stakeHolder][_dId]:depositDetails[_dId];
    ...
    function withdraw(uint256 _poolId,uint _dId,bool isForceWithdraw) external nonReentrant poolExists(_poolId) {
        ...
        uint locktime = (details.lastRewardCalculated - details.period)/30 days;
        periodPoolMap[locktime].tvl-=amount;
    ...
    function harvestFee(address _user, uint _dId) internal {
            stakeDetails memory deposit = deposits[_user][_dId];
            require(deposit.stake > 0, "Harvest: Not a staker");
            uint locktime = (deposit.lastRewardCalculated - deposit.period)/30 days;
            ...
            periodPoolMap[locktime].totalAllotedFeeReward -= rewardFee;
            WETH.transfer(_user, rewardFee);
    ...
    function getHavestAmount(address _user, uint _dId) public view returns(uint) {
            ...
            uint pendingReward = (block.timestamp-deposit.lastRewardCalculated)*feeRewardPerSecond;
            
            uint reward= ((deposit.stake*1e18)/periodPoolMap[locktime].tvl)*pendingReward;
     ...
     */
    function haveSuffecientFundsForReward(uint256 _poolId)
        public
        view
        returns (bool)
    {
    ...
    function getAccMarketFee() public view returns(uint){
