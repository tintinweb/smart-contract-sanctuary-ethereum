...
contract FractonTokenFactory is IFractonTokenFactory {
  ...
  function updateSwap(address swapAddress) external onlyFactoryOwner {
    require(swapAddress != address(0), 'swap address can not be 0');
    FractonSwap = swapAddress;
  }
  ...
  function updateVault(address vaultAddress) external onlyDao returns (bool) {
    _FractonVault = vaultAddress;
    return true;
  }

  function updatePFVault(address PFvaultAddress)
    external
    onlyDao
    returns (bool)
  {
...
contract FractonFFT is ERC20, IFractonFFT {
    ...
    returns (bool)
  {
    ...
    if (isExcludedFromFee(from) || isExcludedFromFee(to)) {
      ...
      uint256 fee = cut(value);
      uint256 half_fee = fee / 2;
      uint256 tokensToTransfer = value.sub(fee);
      _transfer(
        from,
        IFractonTokenFactory(_factory).getPoolFundingVaultAddress(),
        half_fee
  ...
  function updateBasePercent(uint256 newPercent)
    external
    onlyDAO
    returns (bool)
  {
    basePercent = newPercent;

    emit SetBasePercent(newPercent);
    return true;
  }
