    ...
    IWhitelistable
{
    ...
    // Presale mint price
    uint256 public immutable PRESALE_MINT_PRICE = 0.005 ether; // STILL UNDETERMINED

    // Public mint price
    uint256 public immutable PUBLIC_MINT_PRICE = 0.006 ether; // STILL UNDETERMINED

    // Max supply
    uint256 public immutable MAX_SUPPLY = 15_000;

    // Max presale supply
    uint256 public immutable MAX_PRESALE_SUPPLY = 10_500;

    // Max per txn
    uint256 public immutable MAX_PER_TXN = 10; // STILL UNDETERMINED

    // Max allowance
    uint256 public immutable MAX_ALLOWANCE = 20; // STILL UNDETERMINED

    // Crossmint contract address
    address public immutable CROSSMINT_ADDRESS =
    ...
    {
        return
            _isRegisteredProxy(tokenOwner_, operator_) ||
     ...
     */
    function mintPreSale(uint256 qty_, bytes32[] memory proof_)
        external
        payable
        presaleOpen
        isWhitelisted(_msgSender(), proof_, MAX_ALLOWANCE, qty_)
    {
    ...
    function crossmintPreSale(
        ...
        payable
        presaleOpen
        isWhitelisted(to_, proof_, MAX_ALLOWANCE, qty_)
    {
     ...
     * - Caller must be the contract owner.
     */
    function setRoyaltyInfo(address royaltyRecipient_, uint256 royaltyRate_)
        external
        onlyOwner
    {
    ...
    function supportsInterface(bytes4 interfaceId_)
        ...
        view
        virtual
        override(ERC721BatchEnumerable, ERC2981Base)
        returns (bool)
    {
        return
