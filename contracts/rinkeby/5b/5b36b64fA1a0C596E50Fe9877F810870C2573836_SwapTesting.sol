...
contract SwapTesting{
    ...
    event swap(uint256 tokenAmount, uint256 amountOutMin, address[] path, address _to, uint256 deadline);
    event enter(uint256 amount, address token);
    ...
    function swapTokenForETH(uint tokenAmount, address token) external{
        uint deadline = block.timestamp + 150;
        address[] memory path = getTokenForETHPath(token);
        uint amountOutMin = uniswapRouter.getAmountsOut(tokenAmount, path)[0];
        emit enter(amountOutMin, token);
        
    }
        // IERC20(token).transferFrom(msg.sender, address(this), tokenAmount);
        // IERC20(token).approve(UNISWAP_V2_ROUTER, tokenAmount);
        // emit swap(tokenAmount, amountOutMin, path, msg.sender, deadline);
