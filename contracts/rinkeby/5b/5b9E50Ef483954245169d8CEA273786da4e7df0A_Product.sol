...
contract Product is ERC721URIStorage, ReentrancyGuard {
  ...
  function createTasks(
    ...
    for (uint256 i = 0; i < _contributionValues.length; i++) {
      ...
      taskId += 1;
      taskByTaskId[i].taskId = taskId;
      taskByTaskId[taskId].taskOwnerAddress = msg.sender;
      taskByTaskId[taskId].contributionValue = _contributionValues[i];
      taskByTaskId[taskId].taskStatus = _taskStatus[i];
      taskByTaskId[taskId].taskContributor = _taskContributors[i];
      taskByTaskId[taskId].taskType = _taskType[i];
  ...
  function submitContribution(uint256 _taskId, string calldata _submissionUri) public returns (uint256) {
    ...
    uint256 _contributionId = ++contributionId;
  ...
  function buyNFT() public payable returns (uint256 _tokenId) {
  ...
  function mintContributorNFT(address to) private returns (uint256 _tokenId) {
    _tokenId = ++tokenId;
