...
contract ERC1155Collection is ERC1155, Ownable, Pausable {
    struct MintInfo {
        bytes32 merkleRoot;
        uint256 maxPerUser;
        uint256 mintAmount;
    ...
    // **********************  Events  **********************
    event AddCollection(uint256 tokenId, string uri, MintInfo mintInfo);
    ...
    function addCollection(
        ...
        emit AddCollection(tokenId, uri(tokenId), mintInfo(tokenId));
    }
    ...
    function publicMint(
        ...
        uint256 quantity_,
        bytes memory data_
    ) external existTokenId(tokenId_) {
        if (_msgSender() != owner()) {
            MintInfo memory mi = mintInfo(tokenId_);

            if (mi.merkleRoot != 0) revert WhitelistMint();
            if (_totalSupply[tokenId_] + quantity_ > mi.mintAmount) revert MaxMintAmount();
            if (_mintedAmount[tokenId_][_msgSender()] + quantity_ > mi.maxPerUser) revert MaxMintPerUser();

            _mintedAmount[tokenId_][_msgSender()] += quantity_;
        }
        _totalSupply[tokenId_] += quantity_;
    ...
    function whitelistMint(
        ...
        if (_totalSupply[tokenId_] + quantity_ > mi.mintAmount) revert MaxMintAmount();
        if (_mintedAmount[tokenId_][_msgSender()] + quantity_ > mi.maxPerUser) revert MaxMintPerUser();
        if (_mintedAmount[tokenId_][_msgSender()] + quantity_ > maxAmount) revert MaxMintWhitelist();
    ...
    function mintBatch(
        ...
        for (uint256 i = 0; i < tokenIds_.length; i++) {
            ...
            if (_msgSender() != owner()) {
                MintInfo memory mi = mintInfo(i);

                if (mi.merkleRoot != 0) revert WhitelistMint();
                if (_totalSupply[tokenId] + quantity > mi.mintAmount) revert MaxMintAmount();
                if (_mintedAmount[tokenId][_msgSender()] + quantity > mi.maxPerUser) revert MaxMintPerUser();

                _mintedAmount[tokenId][_msgSender()] += quantity;
            }
    ...
    function setMintInfo(
        ...
        uint256 maxPerUser,
        uint256 mintAmount
    ) external onlyOwner existTokenId(tokenId) {
        _mintInfos[tokenId] = MintInfo({merkleRoot: merkleRoot, maxPerUser: maxPerUser, mintAmount: mintAmount});

        emit SetMintInfo(mintInfo(tokenId), tokenId);
    }

    /**
     * @dev Get the minted amount of user in sub collection.
     */
    function mintedAmountOf(uint256 tokenId, address account) public view returns (uint256) {
