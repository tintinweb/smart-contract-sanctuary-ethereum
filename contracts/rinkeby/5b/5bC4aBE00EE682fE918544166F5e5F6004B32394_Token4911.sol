...
// Interface
import { ITokenERC721 } from "../int/ITokenERC721.sol";

//int
import "../int/Contract.sol";
import "../int/aIDFee.sol";
import "../int/rIDFee.sol";
import "../int/PrimarySale.sol";
import "../int/Royalty.sol";
import "../int/Ownable.sol";
import "../int/CurrencyTransferLib.sol";
import "../int/ERC2771ContextUpgradeable.sol";
    ...
    ITokenERC721
{
    ...
    bytes32 private constant TYPEHASH =
        keccak256(
            "MintRequest(address to,address royaltyRecipient,uint256 royaltyBps,address primarySaleRecipient,address aIDFeeRecipient,address rIDFeeRecipient,string uri,uint256 price,address currency,uint128 validityStartTimestamp,uint128 validityEndTimestamp,bytes32 uid)"
        );
   ...
   // address private constant NATIVE_TOKEN = 0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE;
    address private constant NATIVE_TOKEN = 0xDf032Bc4B9dC2782Bb09352007D4C57B75160B15;
    ...
    /// @dev The adress that receives all aID sales value.
    address public aIDFeeRecipient;

    /// @dev The adress that receives all rID sales value.
    address public rIDFeeRecipient;
    ...
    uint128 public aIDFeeBps;

    /// @dev The % of primary sales collected by the contract as fees.
    uint128 public rIDFeeBps;
    ...
    function initialize(
        ...
        uint128 _royaltyBps,
        uint128 _aIDFeeBps,
        address _aIDFeeRecipient,
        uint128 _rIDFeeBps,
        address _rIDFeeRecipient
        ...
        aIDFeeRecipient = _aIDFeeRecipient;
        rIDFeeRecipient = _rIDFeeRecipient;
        primarySaleRecipient = _saleRecipient;
        contractURI = _contractURI;
        aIDFeeBps = _aIDFeeBps;
        rIDFeeBps = _rIDFeeBps;
...
//###################### OLD ###################
 ...
 /// @dev Mints an NFT according to the provided mint request.
    ...
    function setaIDFeeInfo(address _aIDFeeRecipient, uint256 _aIDFeeBps)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        require(_aIDFeeBps <= MAX_BPS, "bps <= 10000.");

        aIDFeeBps = uint64(_aIDFeeBps);
        aIDFeeRecipient = _aIDFeeRecipient;

        emit aIDFeeInfoUpdated(_aIDFeeRecipient, _aIDFeeBps);
    }

    /// @dev Lets a module admin update the fees on primary sales.
    function setrIDFeeInfo(address _rIDFeeRecipient, uint256 _rIDFeeBps)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        require(_rIDFeeBps <= MAX_BPS, "bps <= 10000.");

        rIDFeeBps = uint64(_rIDFeeBps);
        rIDFeeRecipient = _rIDFeeRecipient;

        emit rIDFeeInfoUpdated(_rIDFeeRecipient, _rIDFeeBps);
    }
    ...
    /// @dev Returns the royalty fee bps and recipient.
    ...
    function _encodeRequest(MintRequest calldata _req) private pure returns (bytes memory) {
        return
            abi.encode(
                ...
                _req.aIDFeeRecipient,
                _req.rIDFeeRecipient,
    ...
    function collectPrice(MintRequest memory _req) internal {
        ...
        uint256 aIDFees = (totalPrice * aIDFeeBps) / MAX_BPS;
        uint256 rIDFees = (totalPrice * rIDFeeBps) / MAX_BPS;
        ...
        CurrencyTransferLib.transferCurrency(_req.currency, _msgSender(), aIDFeeRecipient, aIDFees);
        CurrencyTransferLib.transferCurrency(_req.currency, _msgSender(), rIDFeeRecipient, rIDFees);
        CurrencyTransferLib.transferCurrency(
            _req.currency,
            _msgSender(),
            saleRecipient,
            totalPrice - aIDFees - rIDFees
        );
    }
...
interface ITokenERC721 is IERC721Upgradeable {
     ...
     *  @param aIDFeeRecipient aID recipient for token.
     *  @param rIDFeeRecipient rID recipient for token.
     */
    struct MintRequest {
        address to;
        ...
        address aIDFeeRecipient;
        address rIDFeeRecipient;
        string uri;
        uint256 price;
        address currency;
...
library CurrencyTransferLib {
    ...
    //address public constant NATIVE_TOKEN =  0xBb7299A4312cbfF377767184986432fbe5b866c7; //weth
    //address public constant NATIVE_TOKEN =  0xc778417E063141139Fce010982780140Aa0cD5Ab; //WETH last test
   // 0xdf032bc4b9dc2782bb09352007d4c57b75160b15 wet in metamask erc20 low level call failed
   //address public constant NATIVE_TOKEN = 0x8967BCF84170c91B0d24D4302C2376283b0B3a07; //ocean
   address public constant NATIVE_TOKEN = 0xDf032Bc4B9dC2782Bb09352007D4C57B75160B15;
    ...
    function transferCurrencyWithWrapper(
        ...
        if (_currency == NATIVE_TOKEN) {
            if (_from == address(this)) {
                ...
                WETH(_nativeTokenWrapper).withdraw(_amount);
                safeTransferNativeTokenWithWrapper(_to, _amount, _nativeTokenWrapper);
            } else if (_to == address(this)) {
                // store native currency in weth
                require(_amount == msg.value, "msg.value != amount");
                WETH(_nativeTokenWrapper).deposit{ value: _amount }();
            } else {
                safeTransferNativeTokenWithWrapper(_to, _amount, _nativeTokenWrapper);
            }
        } else {
    ...
    function safeTransferNativeTokenWithWrapper(
        ...
        (bool success, ) = to.call{ value: value }("");
        if (!success) {
            WETH(_nativeTokenWrapper).deposit{ value: value }();
