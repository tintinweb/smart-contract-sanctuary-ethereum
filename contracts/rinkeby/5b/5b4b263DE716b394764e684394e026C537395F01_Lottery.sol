...
contract Lottery{
    ...
    bool public lottery_is_started ;

    constructor(address _priceFeed) {
        usd_entry_fee = 50 * 10**18 ;
        owner = msg.sender ;
        priceFeed = AggregatorV3Interface(_priceFeed);
        lottery_is_started = false ;
    }

    modifier only_if_lottery_started(){
        require(lottery_is_started == true);
        _;
    }

    function enter() public only_if_lottery_started payable{
        // 50$ minimum
    ...
    function getEntranceFee() public view returns(uint256){ // entrance fee is in wei
        ...
        return ((usd_entry_fee * 10**18) / price_ETH) ;
    }
