...
contract GoldenDaoNFT is ERC721, Ownable {
    ...
    /**********  staging value for 
        ...
        ended (value = 3)
    *******************************/
    uint256 _stagingValue = 0;

    Counters.Counter private _tokenIds;

    uint256 MAX_SUPPLY = 888;

    uint256 PRIVATESALE_MAX_PER_USER = 1;
    uint256 MINT_MAX_PER_TRANSACTION = 1;

    uint256 _privateSalePrice = 3000000000000000000; // 3 ETH
    uint256 _publicSalePrice = 3000000000000000000; // 3 ETH
    uint256 _publicStartedAt = 0; // time
    uint256 _presaleStartedAt = 0; // time

    mapping(address => bool) private _whitelist;
    mapping(address => uint256) private _countlist;

    constructor() public payable ERC721("GoldenDao", "GDT") {}
    ...
    function requestPrivateSale() external payable {
        require(super.totalSupply() < MAX_SUPPLY, "Maximum supply reached.");
        require(_stagingValue == 1, "Presale is not allowed.");
        uint256 secondsPassed = 0;
        secondsPassed = now.sub(_presaleStartedAt);
        require(
            _whitelist[msg.sender],
            "This address is not included in whitelist."
        );
        require(_countlist[msg.sender] < 1, "Overflow 1 tokens");
        require(msg.value == _privateSalePrice, "Invalid funds");
        _tokenMint();
        _countlist[msg.sender] = _countlist[msg.sender] + 1;
    }

    function requestPublicSale() external payable {
        require(super.totalSupply() <= MAX_SUPPLY, "Maximum supply reached.");
        require(_stagingValue == 2, "PublicSale is not allowed.");
        require(_countlist[msg.sender] < 1, "Overflow 1 tokens");
        require(msg.value == _publicSalePrice, "Invalid funds");
        _tokenMint();
        _countlist[msg.sender] = _countlist[msg.sender] + 1;
    }
