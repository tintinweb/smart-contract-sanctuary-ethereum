...
// File: @zoralabs/nft-editions-contracts/contracts/IPublicSharedMetadata.sol
...
// File: @zoralabs/nft-editions-contracts/contracts/SharedNFTLogic.sol
...
// for remix builds only


contract AuraeOnChain is ERC721Delegated {
  
   IPublicSharedMetadata private metadataEngine;

  uint256 numMinted;

  mapping (uint256 => uint256) public auraeMemory;
   
   // rinkeby: baseFactory = 0x86c67a16C16BF784BdFE7D4b7575dB664D191F88, sharedMetadata = 0x2a3245d54E5407E276c47f0C181a22bf90c797Ce
   // mainnet: baseFactory = 0x43955024b1985E2b933A59021500aE5f55b04091, sharedMetadata = 0x7eB947242dbF042e6388C329A614165d73548670
    constructor(
        address baseFactory,
        IPublicSharedMetadata sharedMetadata
    )
        ERC721Delegated(
            baseFactory,
            "Aurae New Test",
    ...
    {
        metadataEngine = sharedMetadata;
    }

  // public
  function mint() public payable {
    
    require(numMinted + 1 <= 999, "Sold out");

    if (msg.sender != _owner()) {
      require(msg.value >= 0.001 ether, "Wrong cost");
    }
    ...
    auraeMemory[numMinted] = uint256(keccak256(abi.encodePacked(block.difficulty, block.timestamp, block.gaslimit, msg.sender)));
    _mint(msg.sender, numMinted);
  }
   
  function tokenURI(uint256 _tokenId) public view returns (string memory) {
      require(_exists(_tokenId),"ERC721 Metadata: Nonexistent token");
      
      string memory generatedAura;
        {
            string memory aura1 = metadataEngine.numberToString(auraeMemory[_tokenId] % 361);
            string memory aura2 = metadataEngine.numberToString(auraeMemory[_tokenId] << 20 % 361);
            string memory aura3 = metadataEngine.numberToString(auraeMemory[_tokenId] << 40 % 361);
             
      generatedAura = metadataEngine.base64Encode(bytes(
          abi.encodePacked(
              '<svg width="1000" height="1000" xmlns="http://www.w3.org/2000/svg">'
                '<defs>'
                    '<radialGradient id="l1" cx="0.5" cy="0.5" r="1" fx="0.69" fy="0.45" >'
                    '<stop offset="0%" stop-color="hsl(',aura1,', 69%, 75%)" />'
                    '<stop offset="90%" stop-color="hsl(',aura2,', 69%, 35%)" />'
                    '</radialGradient>'
                    '<radialGradient id="l2" cx="0.5" cy="0.5" r="0.8" fx="0.5" fy="0.5" >'
                    '<stop offset="20%" stop-color="hsl(',aura3,', 80%, 55%)" />'
                    '<stop offset="99%" stop-color="hsl(',aura1,', 69%, 35%)" />'
                    '</radialGradient>'
                    '<linearGradient id="l3" gradientTransform="rotate(90)">'
                    '<stop offset="20%" stop-color="#FFFFFF" />'
                    '<stop offset="100%" stop-color="hsl(',aura1,', 80%, 15%)" />'
                    '</linearGradient>'
                    '<radialGradient id="l4" cx="0.5" cy="0.5" r="0.75" fx="0.5" fy="0.5" >'
                    '<stop offset="0%" stop-color="hsl(',aura2,', 100%, 55%)" />'
                    '<stop offset="100%" stop-color="hsl(',aura3,', 100%, 55%)" />'
                    '</radialGradient>'
                '</defs>'
                '<style>'
                'rect.a1 {fill: url(#l1); mix-blend-mode: normal; }'
                'rect.a2 {fill: url(#l2); mix-blend-mode: hard-light; }'
                'rect.a3 {fill: url(#l3); mix-blend-mode: difference; }'
      ...
      return string(abi.encodePacked(
              'data:application/json;base64,', metadataEngine.base64Encode(abi.encodePacked(
                          '{"name":"Aura #', 
                           metadataEngine.numberToString(_tokenId + 1),
                          '" "description":"999 aurae generated 100% on-chain. Colors dynamically set based on minting wallet and time.", "image": "'
