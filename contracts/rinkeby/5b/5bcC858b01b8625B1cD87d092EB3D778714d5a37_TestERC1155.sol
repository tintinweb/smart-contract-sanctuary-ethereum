...
// File: contracts\openzeppelin-contracts\contracts\token\ERC1155\extensions\ERC1155Supply.sol
...
pragma solidity ^0.8.0;
contract TestERC1155 is ERC1155Supply{
    ...
    function mint(uint256 id, uint256 amount) external{
        require(validUri(id), "uri not set");
        _mint(msg.sender, id, amount, "0x");
    }
    
    function validUri(uint256 id) internal view returns(bool){
        return bytes(_uri[id]).length > 0;
    }
    
    function setURI(uint256 id, string memory uri_) external{
        require(!validUri(id), "uri is set");
        _uri[id] = uri_;
    }
