...
/**
* TODO:
* - collection name + adjust unit test
* - collection symbol + adjust unit tests
*/
 ...
 */
contract NFTA is Ownable, ERC721A, VRFConsumerBase, ReentrancyGuard {
  ...
  function presaleMint(bytes32[] calldata _merkleProof, uint256 _mintAmount) public payable correctValue(_mintAmount){
    require(presale, "Presale is not active");
    require(isWhitelisted(msg.sender, _merkleProof), "Sorry, no access unless you're whitelisted");

    _mint(_mintAmount);
  }

  function publicMint(uint256 _mintAmount) public payable correctValue(_mintAmount){
  ...
  function metadataOf(uint256 _tokenId) internal view returns (string memory) {
    ...
    uint256[] memory randomIds = new uint256[](MAX_SUPPLY);
    for (uint256 i = 1; i <= MAX_SUPPLY; i++) {
      randomIds[i-1] = i;
    }

    for (uint256 i = 0; i < MAX_SUPPLY - 1; i++) {
      uint256 j = i + (uint256(keccak256(abi.encode(_seed, i))) % (MAX_SUPPLY - i));
 ...
 function isWhitelisted(address _user, bytes32[] calldata _merkleProof) public view returns (bool) {
  ...
  function _mint(uint256 _mintAmount) internal nonReentrant{
    ...
    require(_totalMinted() + _mintAmount <= MAX_SUPPLY, "Not enough tokens left to mint that many");
  ...
  function airdropList(address[] memory _users) external onlyOwner {
    require(_totalMinted() + _users.length <= MAX_SUPPLY, "Not this many tokens left");
    for(uint256 i = 1; i <= _users.length; i++){
      _safeMint(_users[i-1], 1);
    }
  }

  ///@notice This function mint N tokens for one address.
  function airdropUser(address to, uint256 amount) external onlyOwner {
    require(_totalMinted() + amount <= MAX_SUPPLY, "Not this many tokens left");
    _safeMint(to, amount);
  }
