...
contract MPWRStaking is Initializable, ReentrancyGuardUpgradeable, OwnableUpgradeable, PausableUpgradeable {
    ...
    function AdminWithdraw() external onlyOwner nonReentrant {
        ...
        mpwrToken.safeTransferFrom(address(this), _msgSender(), amount);
        totalSupply = 0;
        emit MPRWWithdraw(_msgSender(), amount);
    }

    function AdminWETHWithdraw() external onlyOwner nonReentrant {
        WETH.safeTransferFrom(address(this), _msgSender(), accMarketFee);
        accMarketFee = 0;
