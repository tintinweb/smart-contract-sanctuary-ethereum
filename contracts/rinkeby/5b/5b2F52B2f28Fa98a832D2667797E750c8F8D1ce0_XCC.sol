...
contract XCC is ERC721A, AuthorityControlled, ReentrancyGuard {
        ...
        onlyManager
    {
        ...
        for (uint256 i = 0; i < _addrList.length; i++) {
            ...
            if (have) {
                // exist already
                bidRecord[recordIndex].amount += newPrice;
            } else {
                // not exist yet
                bidRecord.push(
                    BidRecord({
            ...
            bidInfo.push(BidInfo({addr: _addrList[i], amount: newPrice}));
            emit Bided(_addrList[i], newPrice);
        }
    }

    function mint(address[] memory _addrList, uint256 _type)
        external
        onlyManager
    {
        require(
    ...
    function refundIfNoWin(address[] memory _addrList, uint256 _type)
        external
        onlyManager
    {
        require(
        ...
        for (uint256 i = 0; i < _addrList.length; i++) {
            ...
            if (have) {
                ...
                if (bidRecord[recordIndex].status == BidStatus.Bidding) {
                    ...
                    payable(bidRecord[recordIndex].addr).transfer(
    ...
    function drop(address addr_, uint8 _type) external onlyManager {
        drop_(addr_, _type);
    }

    function drop_(address addr_, uint8 _type) internal {
        require(
            status == Status.Refunded,
            "XCC: Only Refunded status can execute this operation."
        );
        require(address(0) != addr_, "XCC: zero address");
        require(bidRecord.length > 0, "XCC: No data found.");
        bool have;
        uint256 recordIndex;
        (have, recordIndex) = getRecordIndex(addr_);
        if (have) {
            // exist already
            if (bidRecord[recordIndex].status == BidStatus.Minted) {
                // drop token
                market.add(
                    bidRecord[recordIndex].addr,
                    tokens.mul(bidRecord[recordIndex].amount).div(totalBid)
                );
            }
        }
        if (1 == _type) {
