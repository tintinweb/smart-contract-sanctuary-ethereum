...
contract nft_royalties2981 is ERC721, ERC721URIStorage, Ownable, ERC721Royalty {
    ...
    // la siguiente función ya te está diciendo que los URI los tiene que poner el comprador, así que ya vamos mal, pero bueno. Vamos a ver qué sale en opensea
    ...
    function setTokenURI(uint256 itemId, string calldata _tokenURI)
        public
        onlyOwner
    {
    ...
    function setTokenURIs(
        ...
        for (uint256 i = 0; i < itemIds.length; i++) {
            _setTokenURI(itemIds[i], tokenURIs[i]);
        }
    }

    /*
    function createCollectibles(string[] calldata tokenURIS) public {
    ...
    */
    function createCollectiblesV2(string[] calldata tokenURIS) public {
        uint256 newItemId;
        uint256 length = tokenURIS.length;
        for (uint256 i = 0; i < length; i++) {
            ...
            //setRoyalties(newItemId, payable(owner()), 500); //5%
            _tokenIds.increment();
        }
    }
    /*
