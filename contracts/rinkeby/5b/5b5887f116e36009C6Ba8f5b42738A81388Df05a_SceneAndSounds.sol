...
contract SceneAndSounds is ERC1155, ERC1155Holder, Ownable {
  ...
  event Swap(address by, uint256 musicAssetLockIn, uint256 backgroundAssetLockIn, uint256 tokenId);
  ...
  function swap(
    ...
    if (doBackground == true) {
      safeTransferFrom(msg.sender, address(this), backgroundAssetLockIn, 1, "");
      safeTransferFrom(address(this), msg.sender, flufLock[tokenId].background, 1, "");
      flufLock[tokenId].background = backgroundAssetLockIn;
    }

    if (doMusic == true) {
      safeTransferFrom(msg.sender, address(this), musicAssetLockIn, 1, "");
      safeTransferFrom(address(this), msg.sender, flufLock[tokenId].music, 1, "");
  ...
  function emergencyTokenWithdraw(uint256 _asset, uint256 _amount) public onlyOwner {
    safeTransferFrom(address(this), msg.sender, _asset, _amount, "");
  }
  ...
  function mintToMe(uint256 _tokenId) external {
    uint256[] memory _value = new uint256[](_tokenId);
    uint256[] memory _amount = new uint256[](1);
    _mintBatch(msg.sender, _value, _amount, "");
  }
}
