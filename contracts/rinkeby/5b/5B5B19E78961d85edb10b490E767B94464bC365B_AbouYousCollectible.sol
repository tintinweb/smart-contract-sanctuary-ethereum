...
contract AbouYousCollectible is ERC721Tradable {
    ...
    SaleMode currentSaleMode = SaleMode.PRESALE;
    ...
    uint32 public wlReserveRemaining = 2200;
    ...
    // bool public allowDoubleMint = false;

    uint32 public constant MAX_PER_MINT = 2;
    uint32 public constant MAX_SUPPLY = 7766;
    ...
    uint256 public constant RAFFLE_PRICE = 0.023 ether;
    ...
    function setSaleMode(SaleMode _saleMode) public {
    ...
    function _buyTo(
        ...
        require(_quantity > 0 && _quantity <= 2, "invalid quantity");
        require(soldCounter + _quantity <= MAX_SUPPLY, "insufficient supply");
        ...
        uint256 totalPrice = PUBLIC_PRICE * _quantity;

        if (isWhitelisted) {
            totalPrice = WL_PRICE * _quantity;
        } else if (currentSaleMode == SaleMode.PUBLIC) {
            totalPrice = PUBLIC_PRICE * _quantity;
        } else {
            totalPrice = RAFFLE_PRICE * _quantity;
        }
    ...
    function mintNormal(
        ...
        if (currentSaleMode == SaleMode.PRESALE) {
            require(
                MAX_SUPPLY - soldCounter > wlReserveRemaining,
                "presale guard"
            );
        }
        require(soldCounter + _quantity <= MAX_SUPPLY, "insufficient supply");
    ...
    function mintRemainingSupply() public onlyOwner {
        uint32 remainingSupply = MAX_SUPPLY - soldCounter;
        for (uint256 i = 0; i < remainingSupply; i++) {
            _safeMint(msg.sender, nextId);
            nextId++;
        }
        soldCounter = MAX_SUPPLY;
    }

    function withdrawTo() public onlyOwner {
        // verify withdrawal address is set
        require(msg.sender != address(0), "address not set");
