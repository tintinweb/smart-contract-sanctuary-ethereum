...
import "../../base/ERC2891.sol";
import "@openzeppelin/contracts/access/Ownable.sol";
import "@openzeppelin/contracts/utils/Strings.sol"; 
import "@openzeppelin/contracts/utils/cryptography/draft-EIP712.sol";

contract Jukiverse is ERC721A, ERC2981, EIP712, Ownable {
    ...
    uint256 public constant WL_SUPPLY = 2072;
    ...
    /// @notice Error if the caller was a smart contract
    error CallerIsContract();
    /// @notice Error is raised if Limit mint has been exceeded for msg.sender
    error LimitExcedeed();
    /// @notice Error is raised if the given signature was Invalid
    error InvalidSigner();
    /// @notice Error is raised if user try to mint nft that will be exceeded max supply
    error MaxSupplyExceeded();
    /// @notice Error is raised if user fund is insufficient to mint
    error InsufficientFunds();
    /// @notice Error is raised if user try to mint on disabled function (by stage)
    /// @param stage return the current stage
    error StageDisabled(Stage stage);
    ...
    constructor(
        ...
        ERC721A("JUKIVERSE", "JUKI")
        EIP712("Jukiverse", "1.0.0")
    {
        stage = Stage.Presale;
        signer = _signer;
        previewBaseURI = _previewURI;
        _setRoyalties(msg.sender, 5000);
    }
        ...
        payable 
    {
        if(stage != Stage.Presale)
            revert StageDisabled(stage);        
        if(WhitelistMint[msg.sender] + _amount > _limit)
            revert LimitExcedeed();
        if(signer != _verify(_msgSender(), _limit, _signature))
            revert InvalidSigner();
        if(totalSupply() + _amount > WL_SUPPLY)
            revert MaxSupplyExceeded();
        if(msg.value < (WL_PRICE * _amount))
            revert InsufficientFunds();

        WhitelistMint[msg.sender] += _amount;
        ...
        onlyOwner
    {
        for (uint256 i = 0; i < _to.length; i++) {
            ...
            _mint(_to[i], _amount[i], "", false);
            emit Mint(msg.sender, _amount[i]);
        }
    }
    ...
    /// @param _stage stage of nft contract (0 = disabled, 1 = presale, 2 = public sale)
    ...
    function setRoyalties(address _recipient, uint256 _amount) 
        external 
        onlyOwner 
    {
        _setRoyalties(_recipient, _amount);
    }
