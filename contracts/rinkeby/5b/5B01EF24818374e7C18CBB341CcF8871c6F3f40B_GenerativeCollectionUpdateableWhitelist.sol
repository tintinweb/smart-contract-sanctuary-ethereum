  ...
  ReentrancyGuard
{
  ...
  modifier whenNotExceededMaxPresaleMintLimit(
    ...
    if (isPresale_ && presaleWhitelistMints == 0) {
      require(_presaleMintedAddresses[sender] + numberOfTokens <= MAX_PRESALE_MINTING, 'Presale mint limit reached');
    } else if (isPresale_ && (presaleWhitelistMints > 0)) {
      require(numberOfTokens <= presaleWhitelistMints, 'Requested more mints than allowed for presale');
    }
  ...
  function determineIsEligibleForPresale (address sender) internal view returns (bool) {
  ...
  function updatePresaleWhitelist(address[] calldata _addresses, uint256[] calldata _mints) external onlyRole(DEFAULT_ADMIN_ROLE) {
    require(_addresses.length == _mints.length, "Addresses and mints should have same length");
