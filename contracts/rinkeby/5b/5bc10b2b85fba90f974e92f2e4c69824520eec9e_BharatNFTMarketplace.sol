...
contract BharatNFTMarketplace is Ownable, ReentrancyGuard, IERC721Receiver {
    ...
    Listing1155[] public listingsArray1155;


    mapping(uint256 => Listing1155) public idToListing1155;

    mapping(uint256 => tokenDetails721) public tokenToAuction721;

    mapping(uint256 => mapping(address => uint256)) public bids721;

    mapping(uint256 => MarketItem721) public idToMarketItem721;
...
function createTokenAuction721(
    ...
    */
    function bid721(uint256 _tokenId) external payable nonReentrant {
        ...
        if (bids721[_tokenId][msg.sender] > 0) {
            (bool success, ) = msg.sender.call{value: bids721[_tokenId][msg.sender]}("");
            require(success);
        }
        bids721[_tokenId][msg.sender] = msg.value;
            ...
            for (uint256 i = 0; i < auction.users.length; i++) {
                if (auction.users[i] != auction.maxBidUser) {
                    (success, ) = auction.users[i].call{
                        value: bids721[_tokenId][auction.users[i]]
                    }("");
                    require(success);
                }
            }
    ...
    */
    function cancelAuction721(uint256 _tokenId) external nonReentrant {
        ...
        bool success;
        for (uint256 i = 0; i < auction.users.length; i++) {
        (success, ) = auction.users[i].call{value: bids721[_tokenId][auction.users[i]]}("");        
        require(success);
        }
...
//-----------------------------------------------------------------------------ERC1155--------------------------------------------------------------------------------//
     function listToken1155(uint256 tokenId, uint256 amount, uint256 price, address[] memory privateBuyer) public nonReentrant returns(uint256) {
