// this allows for contracts that can continually add multi-IPFS behavior
...
/**
 * @dev This implements a lazy-minted, multiple-IPFS hash folder version of ERC721Marketplace.
 * It gives the creator flexibility to mint singular hashes at a time or entire batches of tokens
 * under a single IPFS folder. Batching tokens together saves an immense amount of gas, but
 * the owner of this collection gives up some control over the tokenIds exact numbering.
 *
 ...
 * existing batch or set a price for a single new IPFS hash that will get added to this token.
 */
contract ERC721BatchableCollection is ERC721Marketplace {
    ...
    mapping(uint256 => string) public batchHashes;
    ...
    function _init(
        ...
        _name = strings[0];
        _symbol = strings[1];
        if (initialRelease > 0 && bytes(strings[2]).length > 0) {
            _addIPFSRelease(strings[2], initialRelease);
        }
    }
     ...
     * IPFS hash saved for that token both for batch and individual tokens.
     */
    function tokenURI(uint256 tokenId)
        public
        view
    ...
    /**
     * @dev adds another batch of NFTs that are all defined under the same IPFS folder
     */
    function addIPFSRelease(
        string memory releaseIPFSHash,
        uint256 releaseAmount
    ) external onlyOwner {
        _addIPFSRelease(releaseIPFSHash, releaseAmount);
    }

    /**
     * @dev adds another batch of NFTs that are all defined under the same IPFS folder
     */
    function _addIPFSRelease(
        string memory releaseIPFSHash,
        uint256 releaseAmount
    ) internal {
        ...
        batchHashes[nextBatchId] = releaseIPFSHash;
    ...
    function giftBatchedNFT(address giftAddress, uint256 tokenId)
        ...
        onlyOwner
    {
        _createBatchNFT(giftAddress, tokenId);
    }
     ...
     */
    function _createBatchNFT(address collectorAddress, uint256 tokenId)
        internal
    {
     ...
     * @dev Only for NFTs that are already part of a defined batch
     */
    function giftIndividualNFT(address giftAddress, string memory ipfsHash)
        external
        onlyOwner
    {
        _createIndividualNFT(giftAddress, ipfsHash);
    }

    /**
     * @dev mints an NFT from the individual block section with a given IPFS
     * hash. If the individual batch has reached full size, it will allocate
    ...
    function _createIndividualNFT(
        ...
        _individualTokenMetadata[tokenId] = ipfsHash;
        _safeMint(collectorAddress, tokenId);
        return tokenId;
    }
    ...
    function buyIndividualToken(
        ...
        require(!_cancelledOrFinalizedSales[hash], "Sale no longer active");
        require(hash.recover(signature) == owner(), "Not signed by owner");
        require(
            dualSignerAddress == address(0) ||
                hash.recover(dualSignature) == dualSignerAddress,
            "Not signed by dual signer"
        ...
        uint256 tokenId = _createIndividualNFT(_msgSender(), ipfsHash);
        emit TokenBought(_msgSender(), tokenId, ipfsHash);
    ...
    function hashToSignToSellFromBatch(
        ...
        _checkValidSigner(_msgSender());
        // require(tokenId < batchTokensCount, "Invalid tokenId");
        require(
    ...
    function buyTokenFromBatch(
        ...
        _createBatchNFT(_msgSender(), tokenId);
        emit TokenBought(_msgSender(), tokenId, "");
        payable(_msgSender()).transfer(msg.value - currentPrice);
    }
