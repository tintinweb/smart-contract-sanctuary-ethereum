    ...
    IERC2981
{
    ...
    function initialize(string memory _uri, uint256 _royaltyBps)
        external
        initializer
    {
        __ERC1155_init(_uri);
    ...
    function setRoyaltyBps(uint256 _royaltyBps) public onlyModuleAdmin {
        require(_royaltyBps <= MAX_BPS, "NFT: Bps must be less than 10,000");
    ...
    function addQuantity(uint256 id, address _to, uint256 amount) public {
        require(hasRole(MINTER_ROLE, msg.sender), "only minter");
        string memory tokenURI = _tokenURIs[id];
        require(bytes(tokenURI).length > 0, "token uri not found");
    ...
    function setURI(uint256 tokenId, string memory tokenURI) external onlyAdmin virtual {
