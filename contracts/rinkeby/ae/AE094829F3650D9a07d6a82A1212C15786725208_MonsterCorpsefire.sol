...
contract MonsterCorpsefire is MonsterBase {
  ...
  string internal constant _CHAMBER_NAME = "Chamber(1) with Corpsefire";
  ...
  function init(
    ...
    level : _CHAMBER_LEVEL * StatLib.DUNGEON_LEVEL_STEP - 1,
    experience : 5_000,
...
abstract contract MonsterBase is ChamberBase {
  ...
  IFightCalculator.AttackInfo[] public attackInfos;
  ...
  function _action(
    ...
    IFightCalculator.FighterInfo memory _monsterInfo = monsterInfo;
    {
      IFightCalculator.AttackInfo memory attackInfo = _decodeAndCheckAttackInfo(data_);
      ...
      IFightCalculator.FighterInfo memory heroFightInfo = IFightCalculator.FighterInfo({
      ...
      _generateMonster(_monsterInfo.fighterAttributes);
      monsterRarity = amplifier;

      IFightCalculator.FighterInfo memory monsterFightInfo = IFightCalculator.FighterInfo({
      fighterAttributes : monsterAttributes,
      fighterStats : IStatController.ChangeableStats({
      level : _monsterInfo.fighterStats.level,
      experience : _monsterInfo.fighterStats.experience,
    ...
    }
    {
      ...
      address[] memory _mintItems = _mintRandomItems(monsterRarity);
  ...
  function _generateMonster(IStatController.Attributes memory base) internal returns (IStatController.Attributes memory, uint) {
    ...
    energy : 0,
    // adjustable attributes
    damageMin : _amplify(base.damageMin, amplifier),
    damageMax : _amplify(base.damageMax, amplifier),
    attackRating : _amplify(base.attackRating, amplifier),
    defense : _amplify(base.defense, amplifier),
    blockRating : _amplify(base.blockRating, amplifier),
    life : _amplify(base.life, amplifier),
    mana : _amplify(base.mana, amplifier),
    // static
    fireResistance : _amplify(base.fireResistance, amplifier),
    coldResistance : _amplify(base.coldResistance, amplifier),
    lightningResistance : _amplify(base.lightningResistance, amplifier)
    }), amplifier);
  }

  function _amplify(uint value, uint amplifier) internal pure returns (uint){
    if (value == 0) {
      return 0;
    }
    return value + (value * amplifier / _MAX_AMPLIFIER);
  }
  ...
  function _decodeAndCheckAttackInfo(bytes memory data) internal returns (IFightCalculator.AttackInfo memory) {
