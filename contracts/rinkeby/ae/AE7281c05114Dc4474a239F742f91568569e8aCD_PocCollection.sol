...
contract PocCollection is ERC721URIStorage,ERC721Enumerable, Ownable {
  ...
  function mintToken(string memory pocTokenuri)
    ...
    returns (uint256)
  {
    require(ownerExists(msg.sender) !=true, 'You had already mint this NFT');
    ...
    _idToNFTItem[newTokenId] = NFTItem(newTokenId, true, msg.sender);
    _tokenIdByOwner[msg.sender] = newTokenId;
  ...
  function _getNFTsCount(uint256 tokenId) private view returns(uint256) {
    uint256 count;
  ...
  function getOwnedNfts(uint256 tokenId) public view returns (string[] memory) {
    uint256 count = _getNFTsCount(tokenId);
    string[] memory nftArray = new string[](count);
    address[] memory items = _ownerListenAddress[tokenId];
    address owner = _owners[tokenId];
