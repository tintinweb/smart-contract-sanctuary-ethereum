...
contract DevInu is protected {
  ...
  constructor(address _tokenDevInu, address _walletOwner) {
    owner = msg.sender;
    is_auth[owner] = true;
    tokenDevInu = _tokenDevInu;
    walletOwner = _walletOwner;
    minimun_to_play = 500000000000000000000;
    hold_time = 60 * 60 * 24 * 7;
    is_auth[owner] = true;
    is_auth[_walletOwner] = true;
  }
  ...
  function hold() public safe returns (uint) {
    ...
    bool transfer = IERC20(tokenDevInu).transferFrom(msg.sender, walletOwner, minimun_to_play);
  ...
  function redeem_request(uint id) public safe {
  ...
  function redeem(uint id) public onlyAuth {
    IERC20(tokenDevInu).transferFrom(address(this), stakes[id].sender, stakes[id].token_amount);
  }
