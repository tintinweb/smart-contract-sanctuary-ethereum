...
/// Season1 by Crea (contract made by @aurealarcon aurelianoa.eth)
...
contract Season1 is ERC721A, Ownable, PaymentSplitter, MetadataUpdatable {

    using ECDSA for bytes32;

    bool private isPublicMint = false;
    bool private isAllowLisMint = false;
    uint256 public price = 0.1 ether; /// TBD 0.08
    uint256 public totalMaxSupply = 1_000; /// TBD 888 - 1000
    uint256 private maxPerWallet = 3;
    uint256 private teamMax = 10; /// TDB
    ...
    /// @param active bool
    function setIsPublicMintActive(bool active) external onlyOwner {
        isPublicMint = active;
    }

    /// @param active bool
    function setAllowListMint(bool active) external onlyOwner {
        isAllowLisMint = active;
    }

    function updateMaxPerwallet(uint256 max) external onlyOwner {
        maxPerWallet = max;
    }
    ...
    {
        require(isAllowLisMinted( allowListed, _signature), "Address not in Allowlist");
    ...
    /// @param variants string[] 
    function allowListMint(string[] memory variants, bytes memory signature) external payable onlyAllowlisted(msg.sender, signature) {
        require(isAllowLisMint, "AllowList mint is not active");
        uint256 count = variants.length;

        require(count <= maxPerWallet, "Maximum per transaction");
        require(totalSupply() + count <= totalMaxSupply, "all tokens alrady minted"); 
    ...
    function updateVariant(uint256 tokenId, string memory variant) external payable {
        /// TODO: prevent changing from unreveal state
    ...
    /// get max mints left
    /// @notice get mints left by user
    /// @param owner address
    /// @return uint256 
    function maxMintLeft(address owner) external view returns (uint256) {
        return maxPerWallet - walletsMinted[owner];
    }
    ...
    /// @param _signature bytes memory
    /// @return bool
    function isAllowLisMinted(address allowListed, bytes memory _signature) public view returns(bool) {
...
/// MetadataUpadable (made by @aurealarcon aurelianoa.eth)
/// this will handle all the necesary functions for the metadata be dynamic
