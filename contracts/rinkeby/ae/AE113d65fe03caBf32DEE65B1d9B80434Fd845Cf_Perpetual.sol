...
contract Perpetual is Users, Deposits, Withdrawals, UpdateState, GlobalConfig, Dac, Custom {
    ...
    struct perpetualParams {
        ...
        //Verifier verifierEscapeAddress;
        bytes32 accountRoot;
    ...
    function initialize(
        ...
        // verifier/escapeVerifier
        verifier = param.verifierAddress;
        //escapeVerifier = param.verifierEscapeAddress;
        ...
        // DAC
        MIN_DAC_MEMBER = 6;
        TIMELOCK_DAC_REG = 0 days;    // TODO : 2 days
        ...
        TIMELOCK_GLOBAL_CONFIG_CHANGE = 0 days;    // TODO : 2 days
        ...
        // security parameters
        DEPOSIT_CANCEL_TIMELOCK = 7 days; // TODO
        FORCED_ACTION_EXPIRE_TIME = 14 days; // TODO
    }

    function upgrade(bytes calldata args) external {

        // upgrade test1 : new added state variable need being initilized
        MAX_ASSETS_COUNT = 31;
        MAX_NUMBER_ORACLES = 6;
        systemTokenDecimal = 6;
        TIMELOCK_GLOBAL_CONFIG_CHANGE = 0 days;
        if (newGlobalConfigValidBlockNum == 0) {
            resetGlobalConfigValidBlockNum();
        }

        DEPOSIT_CANCEL_TIMELOCK = 7 days; // TODO
        FORCED_ACTION_EXPIRE_TIME = 7 days; // TODO
    ...
    function recoverWrongToken(address token, uint256 amount) external onlyGovernor nonReentrant {
        require(token != address(collateralToken), "cbrst");  // "Cannot be recovery system token"
        if (token == address(0)) {
...
abstract contract UpdateState is ForcedWithdrawals, ConditionalTransfer, Config {
    ...
    function postProcess(
        ...
        uint256 offset = 0;
        uint256 factor = 10 ** (systemTokenDecimal - innerDecimal); // TODO : const?
...
contract Storage is Governance, ReentrancyGuard {
    ...
    uint32 totalBlocksVerifySigNature;
    ...
    // TODO : remove, now keep for test1/5 upgrade
    bool OracleFeederRegisterActive;
    uint256 OracleFeederRegisterTime;
    address pendingOracleFeeder;
    mapping(address => bool) oracleFeeders;

    bytes32 public globalConfigHash;
    uint16 public n_synthetic_assets_info;      // TODO, remove, no used, and make globalConfig upgrade confusion
    ...
    function freeze(bytes32 req) public {
		require(forcedActionRequests[req] + FORCED_ACTION_EXPIRE_TIME > block.timestamp, "ftne0");  // "freeze timestamp not expired!"
		stateFrozen = true;
    }
