...
contract LonelyLegendsAudit is ERC721A, Ownable, ReentrancyGuard {
...
// Configuration 
  ...
  constructor() ERC721A("Lonely Legends AUDIT", "LLA") {}
  address payable public payments = payable(0x7d0Bc6f563BFEEfBAB0bDa1203181838B3513D45); 
...
// Dutch auction configuration
  ...
  uint256 public startTimeDA = 1651356000;
...
// Will upload Metadata after deployment so it stays hidden. 
  string public MetadataURI = 'ipfs://__TBD__/';
  string public MetadataURIext = '.json';
  string public preRevealMetadataURI = 'ipfs://__TBD__/';
...
// Metadata functions
  function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
    ...
    return bytes(currentBaseURI).length > 0
        ? string(abi.encodePacked(currentBaseURI, tokenId.toString(), MetadataURIext))
        : '';
  }
...
// Function to withdraw funds
  function transferFunds() public onlyOwner nonReentrant {
    (bool os, ) = payable(owner()).call{value: address(payments).balance}('');
    require(os);
  }
}
