...
contract MfersPetTest is ERC721A, Ownable, ReentrancyGuard {
  ...
  string public hiddenMetadataPrefix; // ipfs://Qme5NGW1Vhf4ZxVxoWE6dFhULnrrobwJVThU4AsM2jqqiU/
  ...
  uint256 public maxMferHolderMintAmount = 5;
  ...
  // mfer id -> bool
  mapping(uint256 => bool) public personalized_minted;
  // pet token id -> if cat (true)
  mapping(uint256 => bool) public isCat;
  // giveaways
  bool public onlyGiveAways = false;
    ...
    payable
  {
    ...
    for (uint256 i = 0; i < _mferIds.length; i++) {
      require(
        !personalized_minted[_mferIds[i]],
    ...
    for (uint256 i = 0; i < _mferIds.length; i++) {
      ...
      personalized_minted[_mferIds[i]] = true;
    }
  ...
  // personalization mint check: (1) minter hold the mfer nft (2) all of the mfer nft ids belong to this minter
