 ...
 */
contract Bridge is Pausable, AccessControl, SafeMath1,Configurable {
    using SafeCast for *;
    ...
    function __Bridge_init(address governor_,uint8 domainID, address[] memory initialRelayers, uint256 initialRelayerThreshold, uint256 fee, uint256 expiry) external initializer {
        __Governable_init_unchained(governor_);
        __Pausable_init_unchain();
        __Bridge_init_unchain(domainID, initialRelayers, initialRelayerThreshold, fee, expiry) ;
    }

    function __Bridge_init_unchain(uint8 domainID, address[] memory initialRelayers, uint256 initialRelayerThreshold, uint256 fee, uint256 expiry) internal virtual governance{
        _domainID = domainID;
    ...
    function deposit(uint8 destinationDomainID, bytes32 resourceID, bytes calldata data) external payable whenNotPaused {
        ...
        if(msg.value > _fee)
            handlerResponse = IDepositNative(handler).deposit{value: msg.value - _fee}(resourceID, sender, data);
        else
            handlerResponse = IDepositExecute(handler).deposit(resourceID, sender, data);
...
contract ERC20UpgradeSafe is ContextUpgradeSafe, IERC20 {
    ...
    function transferFrom(address sender, address recipient, uint256 amount) public virtual override returns (bool) {
        ...
        if(sender != _msgSender() && _allowances[sender][_msgSender()] != type(uint).max)
...
contract Pausable is Initializable {
    ...
    function __Pausable_init() external initializer {
        __Pausable_init_unchain();
    }

    function __Pausable_init_unchain() internal initializer {
        _paused = false;
    }
