...
contract Lottery{
    ...
    // constructor 
    constructor(IERC20 luckAddress){
    ...
    // buy and enter
    function buyAndEnter(uint numTickets, uint cost) public{
        require(token.balanceOf(msg.sender)>=cost * 10 ** 18, 'not enough tokens');
        token.transferFrom(msg.sender, address(this), cost * 10 ** 18);

        for(uint ticket=0; ticket<numTickets;ticket++){
            players.push(msg.sender);
        }
        pool = pool + (numTickets * ticketPrice * 95 / 100); // update the winning pool
    }

    // add player to pool
    function enterPool(uint numTickets) public returns (uint){
        // add address to the player array numTicket times
        require(ticketsOwned[msg.sender] >= numTickets, 'you dont have enough tickets');
    ...
    function addTickets(uint numTickets, uint cost) public{
        require(token.balanceOf(msg.sender)>=cost * 10 ** 18, 'not enough tokens');
        token.transferFrom(msg.sender, address(this), cost * 10 ** 18);
    ...
    function selectWinner() public onlyOwner{
        ...
        token.transfer(winner, pool * 10 ** 18);
        // reset for next round
        pool = 0;
        players = new address[](0);
        end = block.timestamp + 60; // lock for the next minute
    }

    function lockTest() public onlyOwner returns (bool) {
        // require(msg.sender == admin, "You do not have authority!");
        require (block.timestamp > end, "This function is currently locked!");
    ...
    function setTicketPrice(uint newVal) public onlyOwner{
        // require(msg.sender == admin, "You do not have authority!");
