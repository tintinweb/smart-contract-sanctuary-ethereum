...
contract mynft is ERC721, Ownable, Pausable {
    ...
    function mint(uint256 amount) public payable whenNotPaused {
        ...
        require(amountMinted[msg.sender] + amount < 5, "You can mint only 5");
        require(currentSupply + amount < 5555);

        if(msg.sender != owner()) {
            require(msg.value >= price * amount, "Not enough funds in wallet:");
            if (presale) {
    ...
    function addToWhiteList(address[] calldata accounts) public onlyOwner {
        for (uint8 i = 0; i < accounts.length; ++i) {
            require(!whitelist[accounts[i]], "Already added to list");
    ...
    function getMaxSupply() pure public returns (uint) {
        return 5555;
    }
