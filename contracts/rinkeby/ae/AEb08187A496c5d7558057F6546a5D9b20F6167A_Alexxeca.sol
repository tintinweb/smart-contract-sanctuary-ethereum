...
contract Alexxeca {
    ...
    event GetEachBuyLevelInfo(address user, uint256 blockTimestamp, uint256 level, uint256 amount, uint256[4] uplineIds);
    ...
    function emitEvent(uint256 _level, uint256 _amount, uint256[4] memory _uplineIds) private {
         
        emit GetEachBuyLevelInfo(msg.sender, block.timestamp, _level,  _amount,
        _uplineIds);
    ...
    function viewUpline(address _userAddress)
        external
        view
        returns (address directRefer, address upline1, address upline2, address upline3, address upline4)
