...
contract StakingContract {
    ...
    function setGenRewardPercentage(uint256 _percentage, uint value) external  onlyOwner {
        genRewardMultiplicationValue = value;
        genRewardPercentage = _percentage.mul(value);
    }

    function setAreenaBosterPrice(uint256 _price) external  onlyOwner {
        areenaBosterPrice = _price.mul(1e18);
    }

    function getGenRewardPercentage() external view returns(uint256) {
        uint256 genReward = genRewardPercentage.div(1e9);
        return genReward;
    }
