...
contract ArtistBody is ERC721DropBase {
...
contract ERC721DropBase is ERC721A, ContractMetadata, Multicall, Ownable, Royalty, PrimarySale, PermissionsEnumerable, Whitelistable {
    ...
    uint32 public claimerLimit; // 0 means no limit
    ...
    modifier checkMaxSupply(uint256 n) {
        if (maxSupply > 0) {
            uint256 supply = totalSupply();
            require(supply + n <= maxSupply, "Mint would exceed max supply");
        }
        _;
    }

    modifier checkLimits(uint256 n) {
        require(
            ERC721A._numberMinted(msg.sender) + n <= claimerLimit,
    ...
    function _isWhitelistActive()
        ...
        returns (bool)
    {
        return saleState == SaleState.WHITELIST;
    }

    function reserve(uint256 n) public onlyRole(MINTER_ROLE) checkMaxSupply(n) {
    ...
    function mintTo(
        ...
        checkLimits(_quantity)
        checkPaused
        onlyWhitelisted(_proof)
    {
        uint256 totalPrice = pricePerToken * _quantity;
        require(
    ...
    function setSaleState(SaleState _saleState) public onlyRole(DROP_ROLE) {
        saleState = _saleState;
    }

    function setClaimerLimit(uint32 _claimerLimit) public onlyRole(DROP_ROLE) {
        require(_claimerLimit <= maxSupply, "Claimer limit too high");
        claimerLimit = _claimerLimit;
    }

    function setPricePerToken(uint256 price) public onlyRole(DROP_ROLE) {
...
abstract contract Whitelistable {
    ...
    function _setupMerkleRoot(bytes32 _merkleRoot) internal {
    ...
    function _isWhitelistActive() internal virtual returns (bool);
}
