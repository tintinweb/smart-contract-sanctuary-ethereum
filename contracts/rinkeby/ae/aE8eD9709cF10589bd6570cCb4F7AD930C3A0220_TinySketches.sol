...
contract TinySketches is iTinySketches, ERC721, ReentrancyGuard, Ownable {
  ...
  function mintableLastTokenId() public view returns (uint256) {
    ...
    console.log("_startDate: %d", _startDate);
    require(t > _startDate, "TinySketches: no tokens are on sale yet.");
    uint256 passedWeeks = (t - _startDate) / A_WEEK + 1;
    if (passedWeeks > 3) {
      return 239;
    } else {
