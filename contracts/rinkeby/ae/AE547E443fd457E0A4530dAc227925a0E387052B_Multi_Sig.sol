...
contract Multi_Sig is Ownable, ReentrancyGuard {
    ...
    */
    ...
    mapping(address => uint256) public cosmeticsMinted;
   ...
   address[] public payroll = [
    ...
    {
            //init all admins
     adminMapping[0x6884efd53b2650679996D3Ea206D116356dA08a9] = keccak256("admin"); //coso
      adminMapping[0xfc494FDbD08913D81a2e2437ab408E624611F2D1] = keccak256("admin"); //manic
       adminMapping[0x055defF0B8921Be843E62750dfAdE964d2B7C3c2] = keccak256("admin"); //syed
        adminMapping[0x8bCE8074a9CFbC8c6D1c53AC66e906614120b0af] = keccak256("admin"); //meri
  ...
  //MODIFIER check if sender == admin
    ...
    function setLoomiAddress(address deployment_address) public onlyLetExecuteOnce onlyAdmin nonReentrant{
        hasRan = true;
        loomiHeads = deployment_address;
    }

    function setCosmeticAddress(address _address) public onlyAdmin {
    ...
    */
    ...
    function setOracleAddress(address _oracleAddress) external onlyAdmin {
     ...
     modifier checkOwner(uint256 avatarId,uint256[] calldata cosmeticIds,address _holder){
        require(_holder == IOracle(oracleAddress).getAvatarOwner(avatarId),"You Don't Own This Avatar");
        for(uint256 i; i<cosmeticIds.length;i++){
            require(_holder == IOracle(oracleAddress).getCosmeticOwner(cosmeticIds[i]),"You Don't Own This Cosmetic");
        }
        _;
    }

    function getAvatarOwner(uint256 avatarId) public view returns(address){
        return IOracle(oracleAddress).getAvatarOwner(avatarId);
    }

    function getCosmeticOwner(uint256 cosmeticId) public view returns(address) {
        return IOracle(oracleAddress).getCosmeticOwner(cosmeticId);
    }

    function regerateAvatar(uint256 avatarId, 
    uint256[] calldata cosmeticIds,
    bytes memory holder_sig, 
    address holder) 
    external onlyAdmin checkOwner(avatarId,cosmeticIds,holder) {


        uint256 currTimesRegenerated = IOracle(oracleAddress).getTimesRegenerated(holder);
        bytes32 hash = keccak256(abi.encodePacked(currTimesRegenerated,holder));
        require(hash.toEthSignedMessageHash().recover(holder_sig) == holder,"Holder Hasn't Authorized This Transaction");
        IOracle(oracleAddress).regenerateAvatar(avatarId,cosmeticIds);
        
    }


        /* @dev cosmeticMint calls the mint function from the cosmetics contract. 
                for safety, it requires two signatures from admins in case of a hack of an admin wallet
                it also verifies that these signatures are not duplicate.
        */
  function cosmeticMint(address to, uint256 amount,bytes memory sig1,bytes memory sig2) external onlyAdmin{
        
      
        require(keccak256(sig1) != keccak256(sig2), "Duplicate Signatures");
        //Remember to add a keccak hash to the last thing just in case
        bytes32 hash = keccak256(abi.encodePacked(to,cosmeticsMinted[to]));
        require(isAdmin(hash.toEthSignedMessageHash().recover(sig1)), "Not Admin");
        require(isAdmin(hash.toEthSignedMessageHash().recover(sig2)), "Not Admin");

        ICosmeics(cosmeticAddress).cosmeticMint(to,amount);
    }
...
interface IOracle {
    ...
    function getAvatarOwner(uint256 avatarId) external view returns(address);
    function getCosmeticOwner(uint256 avatarId) external view returns(address);
    function regenerateAvatar(uint256 avatarId, uint256[] calldata cosmeticIds) external returns(bool);
}

interface ICosmeics {
    function cosmeticMint(address to,uint256 amount) external;
}
