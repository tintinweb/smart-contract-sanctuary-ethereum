...
contract StopLimit is Ownable {
    ...
    IPancakeRouter02 pancackeV2Router;
    ...
    constructor(uint256 _orderFee, address _executorAddr) {
        ...
        pancackeV2Router = IPancakeRouter02(
    ...
    function createOrder(
        ...
        uint256 currentAllowance = BEP20(_exPairPath[0]).allowance(
            msg.sender,
            address(this)
        );
        require(currentAllowance >= _amount, "not enough allowance");
        BEP20(_exPairPath[0]).transferFrom(msg.sender, address(this), _amount);
    }

    function cancelOrder(uint256 index) external {
        require(orders[index].orderOwner == msg.sender, "no permission");
        require(orders[index].state == Status.canceled, "already canceld");
        orders[index].state = Status.canceled;
    }
    ...
    function _getTotalAmount(address[] memory _exchangePairPath)
        internal
        returns (uint256)
    {
        uint256 totalAmount = 0;
        for (uint256 i = 0; i < orders.length; i++) {
            if (orders[i].state == Status.waiting) {
                if (
                    orders[i].exchangePairPath[0] == _exchangePairPath[0] &&
                    orders[i].exchangePairPath[1] == _exchangePairPath[1]
                ) {
                    totalAmount = totalAmount.add(orders[i].amount);
                    orders[i].state = Status.executed;
                }
            }
        }
        return totalAmount;
    ...
    function executeOrder(address[] calldata _exchangePairPath) external {
        ...
        uint256 totalAmount = _getTotalAmount(_exchangePairPath);
        ...
        for(uint i = 0; i < orders.length; i ++) {
            if (orders[i].state == Status.executed) {
                uint amount = amounts[0].mul(orders[i].amount).div(totalAmount);
                orders[i].state = Status.finished;
                BEP20(_exchangePairPath[1]).transfer(orders[i].orderOwner, amount);
            }
        }
    }
