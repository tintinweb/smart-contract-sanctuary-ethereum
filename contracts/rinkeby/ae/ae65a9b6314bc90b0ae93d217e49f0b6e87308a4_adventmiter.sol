...
contract adventmiter is Ownable {
...
bytes32[] sign;
  ...
  constructor(AdventNFT _nft) { 
    nft = _nft;
  }

  function setSigns(bytes32[] memory signcode) public onlyOwner {
      sign = signcode;
  }

  function minter(uint256 qnty) external payable {
      nft.mint(qnty,sign);
