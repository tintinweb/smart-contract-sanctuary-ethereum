...
contract FlypeTokenSale is Ownable{
    ...
    struct PoolInfo
    {
        ...
        uint256 lockup;
        mapping(uint => bool) usedNFTid;
        mapping(address => bool) usedADDRESS;
    }
    ...
    ERC1155 public immutable FlypeNFT;
    ...
    mapping(uint256 => bool) public banlistNFTid;
    ...
    /// @param pid pool id
    /// @param usedNFTid nft which was marked for this sale
    ...
    constructor(ERC1155 _FlypeNFT, IERC20 _USDC){
        FlypeNFT = _FlypeNFT;
        USDC = _USDC;
    }
    ...
    /// @notice Function that allows contract owner to ban NFT id from sale
    /// @param NFTid NFT id which whould be banned or unbanned
    /// @param isBanned state of ban
    function banId(uint256 NFTid, bool isBanned) external onlyOwner{
        banlistNFTid[NFTid] = isBanned;
    }
    ...
    /// @param pid Pool id 
    /// @param nftID id of NFT which whould be marked on this pool
    function buyTokens(
        uint256 pid,
        uint256 nftID
    )
    ...
    poolExist(pid)
    {
        ...
        require(!banlistNFTid[nftID], "This NFT id is banned");
        require(FlypeNFT.balanceOf(_msgSender(), nftID) == 1, "NFT isn't on the balance");
        PoolInfo storage pool = poolInfo[pid];  
        require(pool.takenSeats <= pool.maxSeats, "No seats left");  
        require(!pool.usedADDRESS[_msgSender()], "User already participated");
        require(!pool.usedNFTid[nftID], "NFT already used");

        pool.takenSeats++;
        pool.usedADDRESS[_msgSender()] = true;
        pool.usedNFTid[nftID] = true;
    ...
    /// @notice return true if given address already participate in given pool
    /// @param pid pool id
    /// @param user address to check
    /// @return state of participation
    function checkUsedAddress(uint256 pid, address user) 
    external 
    view 
    poolExist(pid)
    returns (bool)
    {
        return !poolInfo[pid].usedADDRESS[user];
    }

    /// @notice return true if given NFT id already participate in given pool
    /// @param pid pool id
    /// @param nftID NFT id to check
    /// @return state of participation
    function checkUsedNFT(uint256 pid, uint256 nftID) 
    external 
    view 
    poolExist(pid)
    returns (bool)
    {
        return !poolInfo[pid].usedNFTid[nftID];
    }
