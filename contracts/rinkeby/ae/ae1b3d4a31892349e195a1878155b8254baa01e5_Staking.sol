...
contract Staking is Ownable{
    ...
    IERC20 public shoefy = IERC20 (0x78435169d8dB4826d30ac13b08f010357693ff20);
    ...
    function stake(uint256 amount,uint256 stakestep) external {
        ...
        uint256 lockBalance = amount.mul(dates[stakestep]).mul(rates[stakestep]).div(year); 
        lockBalance = lockBalance.add(amount);
        lockedBalance = lockedBalance.add(lockBalance);
        
        require(lockedBalance < shoefy.balanceOf(address(this)),"Stake : staking is full");
    ...
    function withdraw() external {
        ...
        shoefy.transfer(msg.sender, _stakers[msg.sender].amount);
    ...
    function withdrawOwner(uint256 amount) external onlyOwner{
        require(amount < shoefy.balanceOf(address(this)).sub(lockedBalance));
