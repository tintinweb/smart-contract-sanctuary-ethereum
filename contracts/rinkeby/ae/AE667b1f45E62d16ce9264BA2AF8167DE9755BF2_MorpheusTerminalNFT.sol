...
contract MorpheusTerminalNFT is ERC1155 {
    bytes32 _phrase;
    string public name;
    string public symbol;
    bool public paused;
    uint256 currentSuccessfulTokenId = 2;
    uint256 priceToMint = 0.02 ether;
    mapping(address => mapping(uint256 => bool)) mintedOnCurrentIteration;
    ITreasury treasury;
    ...
    modifier onlyAdmin() {
        require(treasury.isAdmin(msg.sender), "Admin only method");
        _;
    }
    ...
    constructor(
        ...
        _phrase = secretPhrase;
        name = _name;
        symbol = _symbol;
        treasury = _treasury;
    }

    function mint(string memory phrase) public payable onlyUnpaused {
        if (mintedOnCurrentIteration[msg.sender][currentSuccessfulTokenId] == true) {
            require(msg.value >= priceToMint, "Not enough message value");
            (bool success, ) = address(treasury).call{value: msg.value}("");
            require(success);
        } else {
            mintedOnCurrentIteration[msg.sender][currentSuccessfulTokenId] = true;
        }
        if (_phrase == keccak256(bytes(phrase))) {
    ...
    function nextStage(bytes32 newPhrase) public onlyAdmin {
        _phrase = newPhrase;
        paused = false;
