...
contract NFTSaleWL is Ownable, ReentrancyGuard, EIP712Whitelisting {
    bool public hasSaleStarted = false;
    uint256 public price = 0;
    uint256 public maximumSale = 0;
    ...
    function mintNFT(bytes calldata signature)
        public
        requiresWhitelist(signature) 
        payable whenSaleActive nonReentrant {
        require(price <= msg.value, "Amount received is not correct");
        require(totalSale + 1 <= maximumSale, "Purchase would exceed maximum sale");
        require(MCN(nft).totalSupply() + 1 <= MCN(nft).MAX_NFT(), "Purchase would exceed max supply");
        require(MCN(nft).balanceOf(address(msg.sender)) + 1 <= maxMintPerAddress, "NFT exceed maximum per wallet");
        require(
    ...
    function setMaxMintPerAddress(uint _maxMintPerAddress) public onlyOwner {
