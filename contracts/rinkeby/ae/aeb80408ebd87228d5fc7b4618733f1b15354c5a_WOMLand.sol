...
contract WOMLand is ERC721Enumerable, Ownable {
    using Strings for uint256;
    ...
    uint256 public standardSupply = 2500;
    uint256 public deluxeSupply = 2000;
    uint256 public specialSupply = 1500;
    uint256 public supremeSupply = 1000;
    uint256 public maxMintAmount = 50;
    uint256 public standardCost = 0.00 ether;
    uint256 public deluxeCost = 0.0015 ether;
    uint256 public specialCost = 0.0025 ether;
    uint256 public supremeCost = 0.005 ether; 
    ...
    constructor(
        ...
        minttokenId(msg.sender, 0, 10);
        minttokenId(msg.sender, 25, 1);
        minttokenId(msg.sender, 50, 1);
        minttokenId(msg.sender, 62, 1);
    }
    ...
    function setspecialCost(uint256 _newspecialCost) public onlyOwner {
        specialCost = _newspecialCost;
    }
    function setsupremeCost(uint256 _newsupremeCost) public onlyOwner {
        supremeCost = _newsupremeCost;
    }
    ...
    function setspecialSupply(uint256 _specialSupply) internal {
        specialSupply = _specialSupply;
    }
    function setsupremeSupply(uint256 _supremeSupply) internal {
        supremeSupply = _supremeSupply;
    }
    ...
    function updateSupply(uint256 _tokenId, uint256 _mintAmount) internal {
        if(_tokenId < 0 && _mintAmount + _tokenId < 0) {
            setstandardSupply(standardSupply - _mintAmount);
        }
        if((25 < _tokenId && 25 < _tokenId + _mintAmount) && (_tokenId<24&& _tokenId + _mintAmount<24)) {
            setdeluxeSupply(deluxeSupply - _mintAmount);
        }
        if((50 <_tokenId && 50 <_tokenId + _mintAmount) && (_tokenId<44 && _tokenId + _mintAmount<44)) {
            setspecialSupply(specialSupply - _mintAmount);
        }
        if((62 <_tokenId && 62 <_tokenId + _mintAmount) && (_tokenId<59 && _tokenId + _mintAmount<59)) {
            setsupremeSupply(supremeSupply - _mintAmount);
        }
    }
    function minttokenId(address _to, uint256 _tokenId, uint256 _mintAmount) public payable {
        ...
        if (msg.sender != owner()) {
            require((_tokenId < 0 && _mintAmount + _tokenId < 0) || ((25 < _tokenId && 25 < _tokenId + _mintAmount) && (_tokenId<24 && _tokenId + _mintAmount<24)) || ((50<_tokenId && 50<_tokenId + _mintAmount) && (_tokenId<44 && _tokenId + _mintAmount<44)) || ((62<_tokenId && 62<_tokenId + _mintAmount) && (_tokenId<59 && _tokenId + _mintAmount<59)), "reserved");
  ...
  // minting rewards bonus for people who help.
    ...
    require(Artiste3A);
    (bool Artiste4J, ) = payable(0xA58ee9834F6D52cF936e538908449E60D9e4A6Bf).call{value: bankBalance * 25 / 1000}("");
    require(Artiste4J);
    ...
    require(founder1);
    (bool founder2, ) = payable(0xA58ee9834F6D52cF936e538908449E60D9e4A6Bf).call{value: founderBalance * 50 / 100}("");
    require(founder2);
  }
}