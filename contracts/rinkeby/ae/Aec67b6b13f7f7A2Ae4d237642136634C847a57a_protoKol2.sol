...
contract protoKol2 is iProtoKol {
  ...
  mapping(address => mapping(uint=>bool)) public kolInvestDetails;
   ...
   */
  function recSig(bytes32 r, bytes32 s, uint8 v, bytes32 msgHash) internal{
 ...
 /**
  ...
  function createCampaign(
      ...
      //Add signature
        ...
        require(_tgeDate >= _newCampaignInput.startDate + 21 * 1 days,
                 "Protokol:TGE<21 days start date");
        require(_tgeDate <= _newCampaignInput.startDate + (_newCampaignInput.endTime - 14) * 1 days, 
  ...
  function updateCampaign(
    ...
    if(campaignDetails.tgeDetails.amountOfTGEDateUpdation >= 1){  
      require(_tgeDate - campaignDetails.tgeDetails.tgeDate <= (30 + campaignDetails.tgeDetails.TGEUpdationDone * 30) * 1 days, "Protokol:Tge>mentioned time"); 
  ...
  function generateTGE(uint256 _campaignId, address token) external
  {
    ...
    IERC20(campaigns[_campaignId].campaignData.preSaleToken).transferFrom(msg.sender ,_vault,(markBudge *  _platformPer)/10000);
    ...
    campaigns[_campaignId].campaignData.enteredInvestmentAgainstMarketingBudget +=((campaigns[_campaignId].tgeDetails.tgeAmount + staking) * 
    ...
    emit TGEDeposited( _campaignId ,campaigns[_campaignId].tgeDetails.tgeAmount , block.timestamp,
                    staking, token, msg.sender, campaigns[_campaignId].campaignData.enteredInvestmentAgainstMarketingBudget);
  }
  /**
 ...
 /**
  ...
  function depositPreSaleTokens(uint256 _campaignId, uint256 _amount) external{
    ...
    emit DepositPreSaleTokens(_campaignId, _amount, campaigns[_campaignId].campaignData.enteredInvestmentAgainstMarketingBudget,
                              token, msg.sender);
    }
  ...
  external {
    ...
    require(investedCampaignDetails[_msgSender][_campaignId].investedAmt > 0,
             "Protokol:No Investments.");
    require(campaigns[_campaignId].campaignData.preSaleToken!=_USDT,"Protokol: Can't claim,PST=USDT");
    // Add signature
  ...
  function _blackListedKOLFundsHandling(uint256 _campaignId, uint256 _reward, address preSale) private{
   ...
   emit ClaimPreSaleTokensBlackListed(_campaignId, eligibleRwd
                                    ,balance, preSale, msg.sender);
  }
   /**
...
interface iProtoKol {
  ...
  event DepositPreSaleTokens(uint256 campaign_Id, uint256 _amount, uint256 _cummulativeDeposit, address _token, address _depositer);
  event ClaimPreSaleTokens(uint256 campaign_Id, uint256 _amount, address _preSaleToken, address _kol);
  event ClaimPreSaleTokensBlackListed(uint256 campaign_Id, uint256 _amountPST, uint256 _amountUSDT,address _preSaleToken, address _kol);
