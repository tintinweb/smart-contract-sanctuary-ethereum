...
contract TestNFT is ERC721URIStorage, Ownable {
    ...
    uint private _mintEnd = 1649608302;

    event Minted(uint256 id, address minter);
    event DateStartChanged(uint newDate);
    event DateEndChanged(uint newDate);
    ...
    function setMintStart(uint date) public onlyOwner {
        _mintStart = date;
        emit DateStartChanged(_mintStart);
    }

    function setMintEnd(uint date) public onlyOwner {
        _mintEnd = date;
        emit DateEndChanged(_mintEnd);
    }
