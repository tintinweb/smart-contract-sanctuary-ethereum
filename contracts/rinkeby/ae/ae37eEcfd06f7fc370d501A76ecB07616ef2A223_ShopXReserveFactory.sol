...
interface IShopXReserveNFT {
    ...
    function grantRole(bytes32 role, address account) external;
    function hasRole(bytes32 role, address account) external view returns (bool);
    function initialize(string memory _baseURI, uint256[5] memory _uintArgs, address[3] memory _addressArgs, address[] memory _shopxAdmins, address[] memory _brandAdmins, address[] memory _saleAdmins) external;
