...
contract KYC is Context {
    IModusRegistry modus_registry;
    mapping(address => bool) kycStatus;

    constructor(address _modusRegistry) {
        modus_registry = IModusRegistry(_modusRegistry);
    }
    ...
    {
        require(
            _msgSender() == (modus_registry.modusAdmin()) ||
                _msgSender() == (modus_registry.kycAdmin()),
            "KYC: Access Denied"
        );
