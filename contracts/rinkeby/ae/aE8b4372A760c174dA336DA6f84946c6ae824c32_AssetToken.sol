...
contract AssetToken is ERC20, CompoundRateKeeper {
    ...
    function requestRedemption(uint256 _assetTokenAmount, string memory _receipt)
        external
    ...
    /// @param _receipt the off chain hash of the redemption transaction
    function safeguardStake(uint256 _amount, string memory _receipt) external onlyActiveState {
        ...
        if (_requestID == 0) {
            ...
            redemptionRequests[redemptionRequestID].receipt = _receipt;
...
abstract contract AccessManager is AccessControl {
    ...
    function mustBeAuthorizedHolder(address _account) public view returns (bool) {
        ...
        for (uint256 i = 0; i < authorizationContracts.length; i++) {
            authorizationList = IAuthorizationContract(authorizationContracts[i]);
