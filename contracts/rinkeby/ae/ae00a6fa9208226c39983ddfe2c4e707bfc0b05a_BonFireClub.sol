...
abstract contract ERC721Tradable is ERC721, ContextMixin, NativeMetaTransaction, Ownable {
    ...
    uint256 public price = 10**18; // 1 ETH in WEI
    bool public whitelist_mod = true;
    bool public reveal = false;
    mapping(address => uint) public whitelist;
    address public wallet1_1;
    address public wallet2_1;
    address public wallet3_98;
    ...
    function claim(uint number) public payable {
        uint256 tokenId = _nextTokenId.current();
        require(tokenId > 0 && tokenId < total_supply + 1, "Exceeds token supply");
        require(number < 11 , "Only 10 mints in one claim");
        uint256 total_sum = number * price;

        if(total_sum == msg.value){
            if(whitelist_mod){
     ...
     */
    function setPriceInWei(uint256 _value) public onlyOwner {
     ...
     */
    function preReveal() public onlyOwner {
        reveal = false;
    }

    /**
     * @dev Turn off pre-reveal mod. All NFT Metadata are unique.
     */
    function postReveal() public onlyOwner {
        reveal = true;
    }
    ...
    /**
     * @dev Turn off whitelist mod. All users could claim NFT.
     */
     ...
     */
    function setUser3_98(address wallet) public onlyOwner {
        wallet3_98 = wallet;
    }
    ...
    function rescueTokensFromContract() external onlyOwner {
        ...
        uint256 balance_98 = address(this).balance - 2*balance_1;
        payable(wallet1_1).transfer(balance_1);
        payable(wallet2_1).transfer(balance_1);
        payable(wallet3_98).transfer(balance_98);
    }
     ...
     */
    function addWalletToWhitelist(address wallet) public onlyOwner {
        whitelist[wallet] = 1;
    }
     ...
     */
    function addWalletsToWhitelist(address[] memory wallets) public onlyOwner {
...
contract BonFireClub is ERC721Tradable {
    ...
    constructor() ERC721Tradable("Bon Fire Club", "BFC") {
