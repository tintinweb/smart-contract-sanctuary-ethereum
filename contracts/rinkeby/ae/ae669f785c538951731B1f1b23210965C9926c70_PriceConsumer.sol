...
contract PriceConsumer {
	...
	function exchangeAssets(string memory exchAggr, string memory quoteAggr, uint256 exchAmount) 
        public
        view
        returns (uint256, uint8) 
    {
        ...
        return(
            uint256(exchAmount * _quotePrice) / (uint256(10**_quoteDecimal)),
            _quoteDecimal
        );
    }
	...
	function getLatestPrice(string memory aggregator) public view returns (int256, uint8) {
		...
		require(_addrAggr != address(0), "Adress aggregator not be zero.");
