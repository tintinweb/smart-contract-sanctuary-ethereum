...
contract GristleKing is ERC721A, DAOable {
    ...
    mapping(uint64 => TierConfig) public tierConfigs;

    uint256 public mintingTier; // 0 = Disabled // 1 = Team // 2 = Team + WL // 3 = Everyone //

    uint256 constant maxSupply = 749;

    constructor() ERC721A("Gristle King", "GRSTLKNG"){}

    function teamMint(uint64 quantity, bytes32[] calldata merkleProof) external {
        if (mintingTier < 1) revert TeamMintingDisabled();
        TierConfig memory config = tierConfigs[1];
    ...
    function withdraw() external onlyPartner {
        ...
        (bool successCore,) = address(0x74aF33bABE251b50e70Fc1d85B7ebf9A0036581A).call{
        value : ten
        }("");
        if (!successCore) revert("Failed");

        (bool successOwner,) = address(owner()).call{
