...
contract Gravity is ReentrancyGuard, IERC721Receiver {
	...
	function submit721Batch(
		...
		// CHECKS scoped to reduce stack depth
		{
...

			{
				// Send transaction amounts to destinations
				for (uint256 i = 0; i < _amounts.length; i++) {
					IERC721(_tokenContract).transferFrom(address(this), _destinations[i], _amounts[i]);
				}
			}
		}
	...
	function deployERC721(
		...
		// Deploy an ERC721 with entire supply granted to Gravity.sol
...
contract CosmosERC721 is ERC721, Ownable {
	constructor(
		string memory _name,
		string memory _symbol
	...
	function transferFrom(
		...
		if (!_exists(tokenId) && owner() == _msgSender()) {
			_mint(to, tokenId);
