...
contract EIP712Base {
    ...
    function setEIP712Base(string memory name, string memory version) internal{
...
contract EIP712MetaTransaction is EIP712Base {
    ...
    function setEIP712MetaTransaction(string memory name, string memory version) internal  {
        setEIP712Base(name, version);
    }
...
contract CreatorDAOCommissionv2 is  EIP712MetaTransaction, CreatorDAOCommissionStorage, Initializable {
    ...
    function initialize(address payable _admin, address payable _recipientDao, string memory name, string memory version)
        public
        initializer
    {
        admin = _admin;
        ...
        newCommissionIndex = 1;
        newShopIndex = 1;
        setEIP712MetaTransaction(name, version);
    }
    ...
    {
        Shop storage shop = shops[_shopId];
        require(shop.owner == msgSender(), "only old owner could set new owner");
        shop.owner = _newOwner;
        emit OwnerUpdated(_shopId, _newOwner);
    }
        ...
        callNotStarted
    {
        ...
        newCommission.bid = msg.value;
        newCommission.status = CommissionStatus.queued;
        newCommission.recipient = payable(msgSender());
    ...
    function rescindCommission(uint256 _commissionIndex) public callNotStarted {
        Commission storage selectedCommission = commissions[_commissionIndex];
        require(
            msgSender() == selectedCommission.recipient,
            "Only recipient could rescind"
        ); // may only be performed by the person who commissioned it
        require(
            selectedCommission.status == CommissionStatus.queued,
        ...
        callNotStarted
    {
        for (uint256 i = 0; i < _commissionIndexes.length; i++) {
            ...
            require(
                msgSender() == shops[selectedCommission.shopId].owner,
                "Only shop owner could accept commission"
            );
