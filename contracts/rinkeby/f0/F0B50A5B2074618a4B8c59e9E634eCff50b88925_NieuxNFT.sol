...
contract NieuxNFT is ERC721, IERC2981, Ownable, EIP712Whitelisting, ReentrancyGuard {
    ...
    constructor(
        ...
        _splitter = new PaymentSplitter(payees_, shares);
        phase = Phase.Private;
    ...
    */
        ...
        nonReentrant 
    {
        ...
        for (uint i = 0; i < to.length; i++) {
            require(hasMinted[to[i]] == false, "already minted");
            _safeMint(to[i]);
        }
    }
    ...
    function _safeMint(address to) internal virtual {
        ...
        (bool sent,) = payable(_splitter).call{value: msg.value}("");
    ...
    */
        ...
        virtual 
    {
        ...
        for (uint256 i = 0; i < payees.length; i++) {
            _splitter.release(payable(payees[i]));
        }
    }
    ...
    function totalReleased() public view returns (uint256) {
        return _splitter.totalReleased();
    }
    ...
    function released(address account) public view returns (uint256) {
        return _splitter.released(account);
    }

    function payee(uint256 index) public view returns (address) {
        return _splitter.payee(index);
    }
