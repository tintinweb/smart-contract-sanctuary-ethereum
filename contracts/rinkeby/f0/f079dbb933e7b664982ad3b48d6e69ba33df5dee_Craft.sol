...
contract Craft is ERC721, Ownable {
    ...
    uint256 public remainingGeneralToken;
    ...
    function airDrop(address[] memory _accounts, uint256 _amount) external onlyOwner {
        require(getAirDropSupply().add((_accounts.length * _amount)) <= reserve + remainingPreToken + remainingGeneralToken, "Can no longer airdrop.");
    ...
    function allSaleClose() external onlyOwner {
        preSaleActive = false;
    ...
    function setRemainingGeneralToken() external onlyOwner {
        require(!generalSaleActive, "General-Sale still in progress");
        remainingGeneralToken = GENERAL_TOKEN_MAX - generalSupply;
    }
