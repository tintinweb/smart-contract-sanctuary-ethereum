 ...
 * different roles - head to its documentation for details - https://docs.openzeppelin.com/contracts/4.x/access-control#querying-privileged-accounts.
 *
    ...
    AccessControlUpgradeable // {AccessControl} doesn’t allow enumerating role members, whereas {AccessControlEnumerable} allows it. The additional "enumerable" functions are getRoleMember(role, index) and getRoleMemberCount(role). The role member can be enumerated by frontend using events, therefore the enumerable extension is not needed to be read on-chain.
{
    ...
    CountersUpgradeable.Counter private _orderCount; // orders count could be useful for frontends
    CountersUpgradeable.Counter private _offerCount; // offers count only used internally

    bytes32 private constant ARTIST_ROLE = 0x877a78dc988c0ec5f58453b44888a55eb39755c3d5ed8d8ea990912aa3ef29c6; // keccak256("ARTIST_ROLE"); // The ARTIST_ROLE allows an address to mint tokens
    uint24 private ninfaPrimaryFee;      // commissions on all primary sales paid to Ninfa (artist or gallery receives the remainder)
    uint24 private constant NINFA_SECONDARY_FEE = 500;     // 5% fee on all secondary sales paid to Ninfa (seller receives remainder after paying 10% secondary fees to artist/gallery and 5% Ninfa, 85%)
    bytes4 private constant INTERFACE_ID_ERC2981 = 0x2a55205a; // https://eips.ethereum.org/EIPS/eip-2981
    address payable private feeAccount; // EOA or contract where to send trading fees
    mapping(address => mapping(uint256 => _Commission)) private _royaltiesCommission; // collection => tokenId => _Commission
    mapping(uint256 => _Commission) private _primarySalesCommission;
    mapping(address => mapping(uint256 => uint256)) private _tokenToOrderId; // mapping to find orderId using given an ERC721 address and a tokenId
    ...
    mapping(uint256 => _Invoice) private orders;
    mapping(uint256 => _Invoice) private offers;
    ...
    struct _Commission {
        address receiver; // receiver of royalty fees
        uint24 feeBps; // royalties's receiver fees, expressed in basis points 0 - 10000
    }
    ...
    event Commission(       address indexed collection, uint256 indexed tokenId, address indexed to, uint24 primaryBps, uint24 royaltyBps); // using Commission event as cheap storage for commission info. In order to tell whether this event was emitted by `setRoyaltyCommission()` or by `setPrimarySaleCommission()` is given away by whichever "bps" parameter is not 0. E.g. if `primaryBps` has a value (1-10000) that means the split was set for primary sales, therefore `royaltyBps` must be 0, and viceversa.
    event FeeAccount(       address feeAccount);
    ...
    modifier offerValid(uint256 _id) {
        require(_id > 0 && _id <= _offerCount.current(), "Offer Id does not exist"); // not strictly necessary, but prevents weird reverts, like accepting an inexistent offer would return "ERC721: transfer to the zero address" because the "from" is 0x0.
    ...
    function mintAndTransfer(
        ...
        // this if check may seem redundant since we are checking both conditions again, but it is just so that the commision event is only emitted once in order to save a little gas (assuming this function is mostly called with both parameters set), in case both primary and royalty sale commissions are set
     ...
     * TODO currently this allows to submit more than one offer per order, there needs to be a way to forbid the same account to bid twice. It is not a bug, only to protect the user from making two offers by mistake.
    ...
    function submitOffer(address _collection, uint256 _tokenId) external payable {
        ...
        // note there is no require to check that an order doesn't already exist and if so, that the offer amount is not greater than the order itself.
        //      this was omitted in order to save gas; the frontend should check this in order to prevent mistakes from the user
    ...
    /**
     * @dev cancels offer and transfers ETH back to bidder.
     * @param _offerId the Id of the offer.
     * Note that when an order gets filled, the offer isn't marked as cancelled, in order to allow users to claim back their money.
     */
     ...
     *      Regarding the modifier, When an order gets filled, offers don't get automatically cancelled. Therefore we must check that the order was not filled or cancelled in order to protect users from mistakenly believing the order is still valid (this should be the frontend's job but you never know)
     * @param _offerId the id of the offer
     */
    ...
    /**
     * @dev the collector calls this function to buy an NFT 
     */
    ...
    function acceptOffer(uint256 _offerId) offerValid(_offerId) external {
        // creating an additional variable in this particular case is cheaper than calling `offers[_offerId].something` each time.
        // See https://ethereum.org/en/developers/tutorials/downsizing-contracts-to-fight-the-contract-size-limit/#avoid-additional-variables
        _Invoice memory _offer = offers[_offerId];
        require(_offer.price > 0); // checks that the offer exists, since offer price cannot be 0
        // _orderId: If there is no order, the Id will be set to 0, else the order will be marked as filled by the _trade() function, orderFilled[_orderId] = true;
        uint256 _orderId = _tokenToOrderId[_offer.collection][_offer.tokenId];
        ...
        if ( IERC165(_collection).supportsInterface(INTERFACE_ID_ERC2981) ) {
            ...
            if ( _collection == address(NinfaERC721) ) {
                ...
                if ( !secondarySale[_tokenId] ) {
                    // overrides the `marketplaceFeeAmount` that is by default calculated using NINFA_SECONDARY_FEE
                    ...
                    // Set secondarySale bool to true after first sale on NinfaMarketplace.
                    // The token may change hands or be sold elsewhere insead of being sold immediately on NinfaMarketplace, as this cannot be enforced; this will still count as a primary sale.
                    // There is no security risk but the downside is that users may end up selling a token from the Ninfa collection that they bought elsewhere and get charged a fee as if it were a primary sale.
                    // The artist or whoever sells a Ninfa collection token on this marketplace for the first time gets charged 10%, if the token is not Ninfa they get charged 5%; this incentivates people to mint elsewhere and sell here.
                    secondarySale[_tokenId] = true;
                    
                    // store for later gallery address and amount, if any
            ...
            } else { // if the external collection supports royalties
                // if there are royalties for the specified token && a malicious erc721 could implement a royaltyInfo function that simply returns an arbitrary amount (the ETH balance of this contract for example) and transfers it all to the attacker!
                if( royaltyReceiver != address(0) && royaltyAmount > 0 && royaltyAmount <= (_price - marketplaceFeeAmount) ) { // can we just skip this check and just send royalties last, i.e. whatever is left, rather than having an if check which basically won't pay royalties at all if they exceed 95% of the price? I mean even if this check passed, all profits would go to the royalty creator should they set it to 95% so the seller gets nothing!
                    sellerAmount = ( _price - royaltyAmount - marketplaceFeeAmount );
                    payRoyalties(royaltyReceiver, royaltyAmount, _collection, _tokenId);
                }
            }
    ...
    function payRoyalties(address _royaltyReceiver, uint256 _royaltyAmount, address _collection, uint256 _tokenId) private {
        ...
        // even if an artist is associated to a gallery, they may chose to pay 0 commission on royalties.
        ...
        if (commissionAmount > 0) { // if there is a gallery commission on royalties, pay it
     ...
     * @dev if feeInfo is stored in a mapping where the key is the collection and tokenId, then we may only allow to change the split to the token creator; all external tokens are considered as secondary sales, therefore the split can only be applied on royalties and the only one who may decide to share royalties is the artist or creator. It follows that if the token doesn't support royalties then this function cannot be used.
     *      if we need to store a fees split for primary sales, then this function may also be used since NinfaERC721 supports erc2981, however this is already done when calling the `mintAndTranfer` function.
     *      if the artist wants to modify the primary sale split, they may use this or another dedicated function, since most often as this is an external function it will be called by an artist trying to set fees for a token of theirs. Now this is quite wasteful because truly the cost of calling this function is more expensive that manually splitting the royalty profits, it would only make sense if a token is sold on Ninfa marketplace more then once, but there is no guarantee for that. Moreover, they may have to set primary fee also even if they don't use it!
     *      alternatively instead of mapping tokens to split, we could map addresses to split. However, we couldn't allow just anyone to modify royalty fees, it needs to be the royalty receiver, but how do we differentiate if the artist wants to split royalties for certain tokenIds but not for others, or if they want to split for different galleries.
     *      The best solution perhaps would be to create a splitter contract for every artist/gallery pair, then split whatever money it receives, then have it receive the money. But wouldn't work if the artis wanted to set different fees for each token.
     *      in conclusion, there are 2 solutions: 1) a splitter contract. 2) Setting split for every token. By going for n2 we need to create two setter functions, one for setting royalty split and one for primary sales.
     *      External tokens will only ever need the setRoyaltyFee function, whereas artists may want to use both in order to change, say, primary sales fee before it is sold or secondary sale for any secondary sales.
     * @dev "For precision purposes, it's better to express the royalty or fee percentage as "basis points" (points per 10_000, e.g., 10% = 1000 bps) and compute the amount is `(royaltyBps[_tokenId] * _salePrice) / 10000`" - https://forum.openzeppelin.com/t/setting-erc2981/16065/2
     ...
     * @param _bps percentage (using 2 decimals - 10000 = 100, 0 = 0).
     *        No need to require that _bps be different from 0, because imagine somebody set it to 1, 1 in 10000 still returns 0 if the percentage is calculated on a number smaller than 10000 ( (9999 * 1) / 10000 = 0 ), therefore we cannot rely on this check to prevent 0 fee transfers.
     */
    function setRoyaltyCommission (
        ...
        require(_bps <= 9000); // max 90% of royalties to gallery. Enforce here rather than inside private _transfer function as this would cost more gas for every trade.
        require( _collection == address(NinfaERC721) || ERC721Whitelist[_collection] == true);
    ...
    function setPrimarySaleCommission (
        ...
        // Enforce here rather than inside private _transfer function as this would cost more gas for every trade.
        require(_bps <= 8500); 
        require(_receiver != address(0)); // placing this check here avoids checking it each time a sale occurs
     ...
     *      No need to add access control modifier as there is no security issue should somebody call this by mistake; removing the modifier also saves gas.
     */
    ...
    */
    ...
    /**
     * @dev Grants `DEFAULT_ADMIN_ROLE`, `ARTIST_ROLE` to the
    ...
    function initialize(address _NFT, address _feeAccount) public initializer {
        ...
        feeAccount = payable(_feeAccount); // account or contract that can redeem funds from fees. The fee account could become a governance contract in the future!
    ...
    */
     ...
     * @param _amount amount of the sale or royalty
    ...
    function test(address _collection) public view returns (bool) {
        return IERC165(_collection).supportsInterface(0x80ac58cd); // type(IERC721).interfaceId
    }
