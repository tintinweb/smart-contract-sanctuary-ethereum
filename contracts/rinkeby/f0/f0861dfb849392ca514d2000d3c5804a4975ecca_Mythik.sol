...
contract Mythik is ERC721, Ownable, StringUpper, DenyList {
    ...
    mapping(address => uint256) _mintedAmount;

    // mapping(address => uint256[2]) _monitorlimit;

    bool public saleIsActive = false;

    uint256 public defaultmonitorlimit = 1;
    ...
    // event whitelistRemoved(address oldmember);
    event maxsupplyChanged(uint256 newamount);
    event walletlimitChanged(uint256 walletLimit);
    event baseURIChanged(string baseURI);
    event mintPriceChanged(uint256 mintPrice);
    event activeChanged(bool saleIsActive);
    // event whitelistMultiAdded(address[] members);
    // event whitelistMultiRemoved(address[] members);

    constructor() ERC721("Mythik Characters", "MTC") {
    ...
    // {
    //     require(!whitelist.contains(member), "Sender is already added");
    //     whitelist.add(member);
    //     emit whitelistAdded(member);
    // }

    // function removeWhiteList(address member) public onlyOwner
    // {
    //     require(whitelist.contains(member), "Sender is not a white list member");
    //     whitelist.remove(member);
    //     emit whitelistRemoved(member);
    // }

    // function multiAddToWhiteList(address[] memory members) public onlyOwner
    // {
    //     uint256 count = members.length;
    //     for(uint256 i = 0; i < count; i++) {
    //         if(!whitelist.contains(members[i])) {
    //             whitelist.add(members[i]);
    //         }
    //     }

    //     emit whitelistMultiAdded(members);
    // }

    // function multiRemoveWhiteList(address[] memory members) public onlyOwner
    // {
    //     uint256 count = members.length;
    //     for(uint256 i = 0; i < count; i++) {
    //         if(whitelist.contains(members[i])) {
    //             whitelist.remove(members[i]);
    //         }
    //     }
    //     emit whitelistMultiRemoved(members);
    // }

    // function getWhiteCount () public view returns (uint256) {
    //     return whitelist.length();
    // }

    // function getWhiteMember (uint256 index) public view returns (address) {
    ...
    function Mint(string memory _name, string memory cid, uint256 characterId) public payable {
        ...
        require(_mintedAmount[msg.sender] < walletLimit, "Exceeds personal wallet limits");
    ...
    function ownerMint(string memory _name, string memory cid, uint256 characterId) public onlyOwner{
        ...
        require(saleIsActive, "Sale must be active to mint");
        require(totalSupply() < MAX_SUPPLY, "Exceeds max supply of contract");
        require(character_minted[characterId] < character_supply[characterId], "The character amount for this archetype has reached its limit");
    ...
    function flipSaleState() public onlyOwner {
        ...
        emit activeChanged(saleIsActive);
    }
    ...
    function setMaxSupply(uint256 amount) public onlyOwner {
        require(amount < ~uint256(0), "Exceeds numeric limits");
        require(amount >= totalSupply(), "The amount is less than total minted");
        MAX_SUPPLY = amount;
        emit maxsupplyChanged(amount);
    }

    function setWalletLimit(uint256 limit) public onlyOwner {
        require(limit < MAX_SUPPLY, "Exceeds max supply of contract");
        walletLimit = limit;
        emit walletlimitChanged(walletLimit);
    }
