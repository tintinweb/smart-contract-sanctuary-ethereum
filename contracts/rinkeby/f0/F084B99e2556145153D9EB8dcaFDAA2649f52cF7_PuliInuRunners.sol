...
// Dependency file: src/PuliInuNFT.sol
...
abstract contract PuliInuNFT is
    ERC721,
    ContextMixin,
    ...
    Ownable
{
    ...
    mapping(bytes32 => WhiteListInfo) public whiteListMap;

    mapping(address => uint256) pricePublic;
    ...
    event SetWhiteList(
        bytes32 indexed batchDescription,
    ...
    function setWhiteList(
        ...
        require(
            whiteListMap[batchDescription].max == 0,
            "Batch already existed, cannot add user to existing batch"
        );

        whiteListMap[batchDescription].start = tokenIdCounter;
        whiteListMap[batchDescription].counter = 0;
        whiteListMap[batchDescription].max = whitelistAddress.length * quantity;
        tokenIdCounter += (whitelistAddress.length * quantity);
        ...
        for (
            ...
            whiteListMap[batchDescription].users[
                whitelistAddress[address_index]
            ] = quantity;
        }
        emit SetWhiteList(batchDescription, whitelistAddress, quantity);
    }
    ...
    function setWhiteListPrice(
        ...
        for (uint256 index = 0; index < tokenPrice.length; index++) {
            require(
                tokenPrice[index] > 0,
                "Invalid quantity, should be greater than 0"
            );
            whiteListMap[batchDescription].price[
                tokenAddress[index]
            ] = tokenPrice[index];
        }
    ...
    function setPrice(
        ...
        for (uint256 index = 0; index < tokenAddress.length; index++) {
            ...
            pricePublic[address(tokenAddress[index])] = tokenPrice[index];
        }
        emit SetPrice(tokenAddress, tokenPrice);
    }

    function mintWhitelist(bytes32 batchDescription, uint256 quantity)
        public
        payable
    {
        require(
            whiteListMap[batchDescription].start != 0,
            "batch does not exist"
        );
        require(
            whiteListMap[batchDescription].price[NATIVE] > 0,
            "Price is not set for minting"
        );
        require(
            msg.value >=
                whiteListMap[batchDescription].price[NATIVE] * quantity,
            "Insufficient amount, cannot mint"
        );
        _mintWhitelist(batchDescription, quantity);
    }
    ...
    function mintWhitelistToken(
        ...
        require(
            whiteListMap[batchDescription].price[address(token)] > 0,
            "Price is not set for minting"
        );
        // Transafer IERC20 to contract
        uint256 price = whiteListMap[batchDescription].price[address(token)];
        token.transferFrom(msgSender(), address(this), quantity * price);
        _mintWhitelist(batchDescription, quantity);
    }

    function _mintWhitelist(bytes32 batchDescription, uint256 quantity)
        internal
    {
        require(
            whiteListMap[batchDescription].users[_msgSender()] > 0,
            "Minting by current user is invalid"
        );
        require(
            whiteListMap[batchDescription].counter <
                whiteListMap[batchDescription].max,
            "batch cannot mint, maximum reserved reached"
        );
        require(
            maxSupply == 0 || maxSupply >= (totalSupply + quantity),
            "Max limit exceeded"
        );
        ...
        for (uint256 index = 0; index < quantity; index++) {
            ...
            whiteListMap[batchDescription].users[_msgSender()]--; // Decrement counter for specific user
    ...
    function mintByToken(IERC20 token, uint256 quantity) public {
        require(
            pricePublic[address(token)] > 0,
            "Price is not set for minting"
        );
        // Transfer IERC20 to contract
        uint256 price = pricePublic[address(token)];
        token.transferFrom(msgSender(), address(this), quantity * price);
    ...
    function mint(uint256 quantity) public payable {
        require(pricePublic[NATIVE] > 0, "Price is not set for minting");
        require(
            msg.value >= pricePublic[NATIVE] * quantity,
            "Insufficient amount, cannot mint"
        );
    ...
    function getPrice(address tokenAddress) public view returns (uint256) {
        ...
        require(pricePublic[address(tokenAddress)] > 0, "Price is not yet set");
        return pricePublic[tokenAddress];
    }

    function getWhitelistPrice(bytes32 batchDescription, address tokenAddress)
        public
        view
        returns (uint256)
    {
        ...
        require(
            whiteListMap[batchDescription].price[address(tokenAddress)] > 0,
            "Price is not yet set"
        );
        return whiteListMap[batchDescription].price[address(tokenAddress)];
    }
    ...
    function removeWhitelist(
        ...
        require(
            whiteListMap[batchDescription].users[whitelistAddress] > 0,
            "Whitelist address minting capacity already reached"
        );
        require(
            whiteListMap[batchDescription].counter <
                whiteListMap[batchDescription].max,
        ...
        for (uint256 index = 0; index < quantity; index++) {
            ...
            whiteListMap[batchDescription].users[whitelistAddress]--; // Decrement counter for specific user
            whiteListMap[batchDescription].counter++; // Increment batch counter
            totalSupply++;
            emit WhiteListMint(msg.sender, tokenId);
        }
...
// Root file: src/PuliInuRunners.sol


pragma solidity ^0.8.0;

// import "src/PuliInuNFT.sol";
// import "@openzeppelin/contracts/utils/Address.sol";
// import "@openzeppelin/contracts/interfaces/IERC20.sol";
// import "src/IPayroll.sol";

contract PuliInuRunners is PuliInuNFT {
    ...
    constructor(address proxyAddress)
        PuliInuNFT("Puli Inu Runners", "PRNR", proxyAddress, 1060)
    {
        withdrawalAddress = msg.sender;
    }

    function baseTokenURI() public pure override returns (string memory) {
        return "https://runners.pulitoken.net/";
    }

    function contractURI() public pure override returns (string memory) {
        return "https://runners.pulitoken.net/contract.json";
    }
