...
contract SweetNFTreatAffiliate is Ownable, ERC721Enumerable, PaymentSplitter {

    uint public MAXSUPPLY = 10000; // * Hard-coded max for setMaxSupply()
    uint public THEMINTPRICE = 0.02 ether;
    ...
    // Q FROM JIM:  What do you think of changing the vars below to like:
    // affiliateFee, affiliateMintedNFTs, affiliateAmountEarned, affiliateReceiver, and affiliateIsActive ?
    // or maybe affiliate_fee, affilaite_mintedNFTs, etc. ??  -- just to avoid any confusion with the 
    // other vars above.
    struct AffiliateAccount {
    	uint fee;
    	uint mintedNFTs;
    	uint totalmintedNFTs;
    	uint amountEarned;
        address receiver;
        bool isActive;
    }
        ...
        PaymentSplitter(distro, distro_shares)
    {
        ...
        accounts[teamclaim[0]] = Account( 5, 0, 1); // .
        accounts[teamclaim[1]] = Account( 5, 0, 2); // .
    ...
    function addAffiliate(address _addr, string memory affiliateRef, uint fee) external onlyOwner { 

        // add require that affiliateRef does not exist already (must be unique)
        // I think we do NOT really need a require that the wallet _addr is unique
        // just for sanity, let's also add some validations. (See below)
        // Our params: no initial or trailing spaces, between 3 and 10 characters, no spaces
        // probably use a function, as we can double check on affiliate mints

        require(validateName(affiliateRef), "Affiliate Reference code used doesn't follow validations");
    ...
    function disableAffiliate(string memory affiliateRef) external onlyOwner { 
        affiliateAccounts[affiliateRef].isActive = false;
    }
    ...
    function getAffiliate(string memory affiliateRef) external onlyOwner view returns (AffiliateAccount memory){ 
        return affiliateAccounts[affiliateRef];
    }
    ...
    function mint(uint _amount, bool isaffiliate, string memory affiliateRef) external payable noReentrant {
        ...
        // Also, if this fails (if the affiliate acct is not active), will this kill the mint()? 
        // We would still want the mint() to go through, only it would not record the affiliate sale.
        // Not sure if we need to revalidate the afiliateRef, as your first bool would be 0 if not found.
        if(isaffiliate)
        {
        	//require(affiliateAccounts[affiliateRef], "Affiliate reference not found.");
        	bool isActive = affiliateAccounts[affiliateRef].isActive;
        	require(isActive, "Affiliate Account is not enabled.");
       		affiliateAccounts[affiliateRef].mintedNFTs += _amount;
       		affiliateAccounts[affiliateRef].totalmintedNFTs += _amount;
        }
    ...
    function sendNFT(uint _amount, address _addr, bool isaffiliate, string memory affiliateRef) external payable noReentrant {
        ...
        require(SALEISLIVE, "Error: Sale is not active.");
        require(totalSupply() + _amount <= (MAXSUPPLY - RESERVEDNFTS), "Error: Purchase would exceed max supply.");
        require((_amount + accounts[_addr].mintedNFTs) <= WALLETLIMIT, "Error: You would exceed the wallet limit.");
        require(!isContract(msg.sender), "Error: Contracts cannot mint.");
    ...
    function distributeAffiliateShares() external minAdmin2 noReentrant{

        // iterate through all affiliates:
        for (uint i = 0; i < affiliateDistro.length; i++) {

            // the ref name -- eg. jim, sneha, etc.
		    string memory affiliateRef = affiliateDistro[i];

            // the wallet addr to be paid for this affiliate
		    address receiver = affiliateAccounts[affiliateRef].receiver;

            // the fee due per sale for this affiliate...
		    uint fee = affiliateAccounts[affiliateRef].fee;

            // the # of mints they are credited with...
		    uint mintedNFTs = affiliateAccounts[affiliateRef].mintedNFTs;

            // require that the contract balance is enough to send out ETH
            // I wonder if we should assign a var to fee * mintedNFTs, since we use it 3x below?
            // not sure what's better gas-wise. Any thoughts?
		    require(address(this).balance >= (fee * mintedNFTs), "Insufficient balance");

            // calc the amt earned for this person (can use += instead)
		    affiliateAccounts[affiliateRef].amountEarned = affiliateAccounts[affiliateRef].amountEarned + (fee * mintedNFTs);

            //  set their mintedNFTs back to 0 (maybe should do this last after send?)
            //  (just because, if the send fails, the person would lose credit for their minted NFTs)
		    affiliateAccounts[affiliateRef].mintedNFTs = 0;

	       	(bool sent, bytes memory data) = payable(receiver).call{value: (fee * mintedNFTs)}("");
