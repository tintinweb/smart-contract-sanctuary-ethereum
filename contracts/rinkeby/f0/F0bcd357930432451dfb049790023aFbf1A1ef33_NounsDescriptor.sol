...
contract NounsDescriptor is INounsDescriptor, Ownable {
    ...
    /**
     * @notice Batch add Noun arts.
     * @dev This function can only be called by the owner when not locked.
     */
    function addManyArts(bytes[] calldata _arts) external override onlyOwner whenPartsNotLocked {
        for (uint256 i = 0; i < _arts.length; i++) {
            _addArt(_arts[i]);
        }
    }
     ...
     * @dev This function can only be called by the owner when not locked.
     */
    function addArt(bytes calldata _art) external override onlyOwner whenPartsNotLocked {
        _addArt(_art);
    }
