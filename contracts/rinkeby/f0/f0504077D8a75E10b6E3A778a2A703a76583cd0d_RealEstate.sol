...
contract RealEstate is Pausable, Ownable {
        ...
        )
    {
        ...
        _endAt = TICKET_TOKEN.TokenTransferTimestamp(properties[_propertyId].folioNumber).add(burnTimer[properties[_propertyId].folioNumber]);
    ...
    function addProperty(
        ...
        properties[propertyId] = _property(
            propertyId, (_price + percent2), _price, _folioNumber, _isAgency,
    ...
    function updatePropertyAsPropertyOwner(
        uint256 _propertyId, uint256 _price, uint256 _folioNumber, bool _isAgency, string memory _fullName,
        string memory _phoneNumber, string memory _details, string memory _houseAddress, string memory _pictures
        ...
        properties[_propertyId] = _property(
            _propertyId, (_price + percent2), _price, _folioNumber , _isAgency,
    ...
    function updatePropertyAsAgency(
        uint256 _propertyId, uint256 _price, uint256 _folioNumber, bool _isAgency, string memory _fullName,
        string memory _phoneNumber, string memory _details, string memory _houseAddress, string memory _pictures
    ) public onlyAgency {
    ...
    mapping(uint256 => uint256) public paidInAmount;
    ...
    function buyPropertyToken( uint256 _propertyId, string memory fullName, string memory phoneNumber, uint256 amount_, Tokens token_) public {
        ...
        paidInAmount[_propertyId] = amount_;
    ...
    function acceptingDeal(uint256 _propertyId) public onlyAgency {
        ...
        if (properties[_propertyId].isAgency == true) {
            ...
            if (paidIn[_propertyId] == Tokens.USDT) {
                ...
                uint256 _fee = (paidInAmount[_propertyId] * (sellerAgencyFee + 2)) / 100;
                uint256 _transferPropertyOwner = paidInAmount[_propertyId] - _fee;
           ...
           if (paidIn[_propertyId] == Tokens.USDT) {
                ...
                uint256 _fee = (paidInAmount[_propertyId] * (sellerIndividualFee + 2)) / 100;
                uint256 _propertyOwnerEth = paidInAmount[_propertyId] - _fee;
