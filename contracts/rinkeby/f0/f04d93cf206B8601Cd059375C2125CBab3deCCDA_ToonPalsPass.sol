...
contract ToonPalsPass is Ownable, ERC721A {
    ...
    function mint(uint256 quantity) external payable {
        if (false == saleActive) revert SaleNotActive();
        if (_msgSender() != tx.origin) revert RecipientIsContract();
        if (totalSupply() + quantity > maxSupply) revert MaxSupplyExceeded();
        if (balanceOf(_msgSender()) + quantity > maxMintQuantity)
            revert MaxMintQuantityExceeded();
