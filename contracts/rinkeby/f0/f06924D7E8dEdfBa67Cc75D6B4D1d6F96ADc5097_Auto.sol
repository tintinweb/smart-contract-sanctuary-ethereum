...
contract Auto {
    constructor(address tomi, address stableCoinAddress, uint amountTomiPool, uint amountStablePool, address delegateAddress, 
        ...
        IERC20(stableCoinAddressLocal).transferFrom(msg.sender, address(this), amountStablePoolLocal + amountUSwapLocal);
        ...
        //     IERC20(tomiAddress).approve(delegateAddress, amountApprove);
        //     IERC20(stableCoinAddressLocal).approve(delegateAddress, amountApprove);

        //     // TO DO: check number of LP return
        //     ITomiDelegate(delegateAddress).addLiquidity(tomiAddress, stableCoinAddressLocal, amountTomiPoolLocal, amountStablePoolLocal, 0, 0, block.timestamp + 3 hours);
        // }
        ...
        //     ITomiPlatform(tomiPlatform).swapExactTokensForTokens(amountUSwap, 0, pathStableCointoWeth, address(this), block.timestamp + 3 hours);
        ...
        //     ITomiDelegate(sushiRouter).swapExactTokensForTokens(amountUSwap, 0, pathStableCointoWeth, address(this), block.timestamp + 3 hours);
        ...
        // // Remove Liquidity
        // uint256 balanceLp = IERC20(lpAddress).balanceOf(address(this));
        // if (balanceLp > 0) {
        //      ITomiDelegate(delegateAddress).removeLiquidity(tomiAddress, stableCoinAddressLocal, balanceLp, 0, 0, block.timestamp + 3 hours);
        // }
