...
contract ERC721Mintable is ERC721EnumerableUpgradeable, ITokenBridgeable, OwnableUpgradeable, IERC721MintableInitializable {
    ...
    (string memory name_, string memory symbol_, address owner_, address system_) = decodeInitializer_ERC721Mintable(initializerEncoded_);
        _initializeUnchainedInternal(name_, symbol_, owner_, system_);
    }

    function decodeInitializer_ERC721Mintable(bytes memory initializerEncoded_) override public pure returns (string memory, string memory, address, address){
        return abi.decode(initializerEncoded_, (string, string, address, address));
    }

    function _initializeUnchainedInternal(string memory name_, string memory symbol_, address owner_, address system_) onlyInitializing internal {
    ...
    function encodeInitializerParams(string memory name_, string memory symbol_, address owner_, address system_) public pure returns (bytes memory) {
        return abi.encode(
            name_,
            symbol_,
            owner_,
    ...
    function encodeInitializer(string memory name_, string memory symbol_, address owner_, address system_) override public pure returns (bytes memory){
        bytes memory params = encodeInitializerParams(name_, symbol_, owner_, system_);
    ...
    * @dev Returns collection identifier`
    ...
    /**
    * @dev Returns contract URI for particular collection`
     */
    function contractURI() public view returns(string memory) {
        return string(abi.encodePacked(
                system.contractUriBase(),
...
contract SystemContext is SystemContextSubscribable {
    ...
    constructor (AccessControlList accessControlList_, address multisigWallet_) {
    ...
    function setMultisigWallet(address multisigWallet_) external onlyRole(accessControlList.SYSTEM_CONTEXT_DEFAULT_ADMIN_ROLE()) {
        multisigWallet = multisigWallet_;
    }

    function setContractUriBase(string memory contractUriBase_) external onlyRole(accessControlList.SYSTEM_CONTEXT_DEFAULT_ADMIN_ROLE()) {
        contractUriBase = contractUriBase_;
    }
}
...
interface IERC721MintableInitializable {
    function encodeInitializer(string memory name_, string memory symbol_, address owner_, address system_) external pure returns (bytes memory);
    function decodeInitializer_ERC721Mintable(bytes calldata initializerEncoded_) external pure returns (string memory, string memory, address, address);
}
...
contract CollectionsRegistry is ICollectionRegistry {
    ...
    */
    ...
    function updateRecord(Record calldata record_, bytes32 collectionId_) public onlyRole(systemContext.getAccessControlList().COLLECTION_REGISTRY_DEFAULT_ADMIN_ROLE()){
        records[collectionId_] = record_;
    }

    function updateCollection(address addr_, bytes32 collectionId_) public onlyRole(systemContext.getAccessControlList().COLLECTION_REGISTRY_DEFAULT_ADMIN_ROLE()){
        collections[addr_] = collectionId_;
    }

    function updateExternalToCollection(address addr_, bytes32 collectionId_) public onlyRole(systemContext.getAccessControlList().COLLECTION_REGISTRY_DEFAULT_ADMIN_ROLE()){
        externalToCollection[addr_] = collectionId_;
    }
}
...
contract AccessControlList is OmniteAccessControl {
    ...
    constructor(address admin) {
        ...
        _setupRole(BRIDGE_DEFAULT_ADMIN_ROLE, toBytes(admin));
...
//    nativePayment(params_.refundAddress, params_.value)
    ...
    struct DeployExternalParams {
        ...
        uint256 gasAmount;
        address owner;
        bytes ctorParams; // TODO not used at all - remove after Tom recording
    }
    ...
    function _deployExternalCollection(
        ...
        bytes memory ctorParams = abi.encode( // TODO what to do with contractURI_???
            orig.name(), orig.symbol(), systemContext.getMultisigWallet(), address(systemContext)
        );
        ...
        for (uint256 i = 0; i < deploymentParams_.length; i++) {
            ...
            _sendMessageWithValue(
                SendMessageWithValueParams({
                    ...
                    buffer : deployContractEncode(ERC721_BLUEPRINT_NAME, collectionId_, ctorParams, params.collectionName, systemContext.getMultisigWallet()),
    ...
    function deployExternalCollection(
        ...
        require(IERC721(params.originalCollection).supportsInterface(type(IERC721Metadata).interfaceId), "Original collection is not IERC721Metadata");
...
abstract contract OmniteAccessControl is AclContext, IAccessControl, ERC165 {
    ...
    function renounceRole(bytes32 role, bytes memory account) public virtual override {
        require(keccak256(account) == keccak256(_msgSender()), "OmniteAccessControl: can only renounce roles for self");
