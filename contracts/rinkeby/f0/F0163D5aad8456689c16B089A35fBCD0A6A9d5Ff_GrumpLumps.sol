    ...
    RandomlyAssigned
{
    ...
    constructor(string memory uri, bytes32 merkleroot, address _proxyRegistryAddress)
        ERC721("Grump Lumps", "Grump")
        PaymentSplitter(_team, _teamShares) // Split the payment based on the teamshares percentages
        ReentrancyGuard() // A modifier that can prevent reentrancy during certain functions
        RandomlyAssigned(maxSupply,1) // Max of 5010 NFTs available; Start countiing from 1, instead of 0
    {
    ...
    onlyAccounts
    {
        ...
        require(
            _amount <= presaleAmountLimit,      "Grump Lumps: You can not mint that many tokens");
        require(
            _presaleClaimed[msg.sender] + _amount <= presaleAmountLimit,  "Grump Lumps: You can not mint that many tokens");

        uint256 id = nextToken();

        require(id + _amount <= maxSupply, "Grump Lumps: max supply exceeded");
