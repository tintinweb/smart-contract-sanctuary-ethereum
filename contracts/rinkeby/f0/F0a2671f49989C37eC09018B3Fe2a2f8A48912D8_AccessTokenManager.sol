...
contract AccessTokenManager is Initializable, UUPSUpgradeable, ReentrancyGuardUpgradeable, AccessControlUpgradeable {
    ...
    function purchase(string memory channelId) 
        ...
        nonReentrant 
    {   
        require(bytes(channelId).length != 0, "ERR-1-ATM");
        AccessTokenItem memory accessToken = idToAccessTokenItem[channelId];
        require(msg.sender != accessToken.creator, "ERR-2-ATM");
        require(msg.value == accessToken.finalPrice, _concatenateStringUint("ERR-3-ATM", accessToken.finalPrice));
    ...
    {
        require(bytes(channelId).length != 0, "ERR-1-ATM");
        require(creator != address(0), "ERR-8-ATM");
        require(royaltyPercentage <= 10000, "ERR-9-ATM");
        require(bytes(uriMetadata).length != 0, "ERR-10-ATM");
        require(tickets > 0, "ERR-11-ATM");
        require(bytes(communityId).length != 0, "ERR-12-ATM");
