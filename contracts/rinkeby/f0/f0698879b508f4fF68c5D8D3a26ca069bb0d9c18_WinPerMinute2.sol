...
contract WinPerMinute2 is ContractData {
    event Deposit(
  ...
  // --------------------- PUBLIC METHODS ---------------------------
    function testEmit(address _address, uint _ammount) external {
        emit Deposit(_address, _ammount);
    }
    ...
    function withdrawalTotal() external {
        address sender = msg.sender;
        require(waitingToReceive(sender) == 0, "insufficient funds");
        require((accountsInfo[sender].lastWithdraw + 1 days) < block.timestamp , "Time limit");
        (,uint freeToWithdrawl,,,) = availableForWithdrawal(sender);
    ...
    function upgradeDeposit() external payable {
        require(msg.value >= minAllowedDeposit, "Min amount not reached");
        _registerDeposit(msg.sender, msg.value, 1);
    }

    function registerAccount(address ref) external payable {
        address sender = msg.sender;
        require(accountsInfo[ref].registered == true, "Invalid sponsor");
...
contract ContractData is Authorized {
  string public name = "Win Per Minute Now copy";
  string public url = "";
  ...
  function projectInfo() public view returns(uint, uint) {
    return(address(this).balance, networkSize);
  }
  ...
  function passiveEarning(address _address) public view returns(uint) {
    ...
    uint pasiveAvailable = (((dailyRentability  * accountsEarnings[_address].depositValue) / 1000) /  1.6 minutes) * secondsRunning;
