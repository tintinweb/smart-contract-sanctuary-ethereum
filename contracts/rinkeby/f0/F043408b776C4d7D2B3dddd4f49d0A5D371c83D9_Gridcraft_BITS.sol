...
contract Gridcraft_BITS is ERC20, Ownable {
  ...
  address signer;
  address stakingCoordinator;

  mapping(uint256 => bool) withdrawn;

  modifier onlyCoordinator {
    require (_msgSender() == stakingCoordinator, "Not allowed");
    _;
  }
  ...
  constructor(
    ...
    _mint(_teamWallet, 60000000 ether);     // 60M BITS tokens
    _mint(_rewardsWallet, 60000000 ether);
  }
  ...
  function setCoordinator(address _newCoordinator) external onlyOwner {
    stakingCoordinator = _newCoordinator;
  }
