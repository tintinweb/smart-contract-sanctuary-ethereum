...
contract PackWoodERC1155V2 is ERC1155Upgradeable, OwnableUpgradeable {
    ...
    event buyTokenDetails(address from, address to, uint256 tokenId1, uint256 tokenId2, uint256 tokaenId3, uint256 tokenId1Amt, uint256 tokenId2Amt, uint256 tokenId3Amt, uint256 price);
    ...
    function buyToken(uint256[3] memory tokenIds, uint256[3] memory random) payable external returns(bool){
        uint256 amount = random[0] + random[1] + random[2];
        require(totalNum * amount == msg.value, "PackWoodERC1155: Price is incorrect");
        payable(owner()).transfer(msg.value);
    
        _safeTransferFrom(owner(), msg.sender, tokenIds[0], random[0], "");
        _safeTransferFrom(owner(), msg.sender, tokenIds[1], random[1], "");
        _safeTransferFrom(owner(), msg.sender, tokenIds[2], random[2], "");   

        emit buyTokenDetails(owner(), msg.sender, tokenIds[0], tokenIds[1], tokenIds[2], random[0], random[1], random[2], msg.value);
