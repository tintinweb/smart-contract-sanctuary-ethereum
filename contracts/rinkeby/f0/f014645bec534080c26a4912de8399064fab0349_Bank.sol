...
// File: https://github.com/paulrberg/prb-math/blob/v1.0.3/contracts/PRBMathCommon.sol
...
// File: contracts/bank2042797.sol
...
contract Bank {
    ...
    string internal _bankAddress = "bank address";
    bool private isOpen = true;
    int private branchNumber;

    enum Status { Waiting, Open, Closed }
    Status private status;

    enum Position {Cashier, Cleaner, CEO}
    Position private position;

    enum HomeOffice {Amsterdam, Berlin, Warsaw}
    HomeOffice private office;

    mapping(HomeOffice => Employee) public officeEmployeeMapping;
    ...
    function registerEmployee(Employee memory _emp, 
        HomeOffice _homeOffice) internal {
            employees.push(_emp);
            officeEmployeeMapping[_homeOffice] = _emp;
        }

    function addEmployee(string memory fn, string memory ln, 
        Position pos, HomeOffice homeOffice) public onlyOwner{
        Employee memory emp = Employee(fn, ln, pos);

        registerEmployee(emp, homeOffice);
    }
    ...
    function setOpen(bool _value) public {
        isOpen = _value;
    }

    function getBranchNumber() public view returns(int) {
        return branchNumber;
    }

    function setBranchNumber(int _value) public {
        branchNumber=_value;
    }
}

contract Bank2042797 is Bank {
    using PRBMathCommon for uint;

    uint private interestRate = 3;

    constructor(string memory bankAddress, address payable _wallet) Bank(_wallet) {
        _bankAddress=bankAddress;
    }

    function getIncrease() public view returns(uint) {
        //return mulDivFixedPoint(balances[wallet], 12);
        uint currentBalance = balances[wallet]; 
        return currentBalance.mulDivFixedPoint(12);


        //exp2(uint256 x) internal pure returns (uint256 result) 
        //mostSignificantBit(uint256 x) internal pure returns (uint256 msb) 
         
        //mulDiv(uint256 x, uint256 y, uint256 denominator ) internal pure returns (uint256 result)

      ///  @param x The multiplicand as an unsigned 60.18-decimal fixed-point number.
    /// @param y The multiplier as an unsigned 60.18-decimal fixed-point number.
    /// @return result The result as an unsigned 60.18-decimal fixed-point number
        //mulDivFixedPoint(uint256 x, uint256 y) internal pure returns (uint256 result)
    }
}