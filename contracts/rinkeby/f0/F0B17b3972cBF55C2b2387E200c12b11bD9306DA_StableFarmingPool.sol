    ...
    ReentrancyGuardUpgradeable
{
     ...
     */
    function farmingAssignStakedData(
        uint256 _poolId,
        address _from,
        address _to
    ) external onlyAdmin stableFarmingValidatePoolById(_poolId) {
        require(_from != address(0), "stableFarmingStakingPool: invalid address");
        require(_to != address(0), "stableFarmingStakingPool: invalid address");
        require(_from != _to, "stableFarmingStakingPool: invalid address");

        StableFarmingStakingData storage stakingData = stableFarmingStakingData[_poolId][_to];
        StableFarmingPendingWithdrawal
            storage pendingWithdrawData = stableFarmingPendingWithdrawals[
                _poolId
            ][_to];
        require(
            ...
            "stableFarmingStakingPool: user already staked"
        );

        // LinearStakingData
        stableFarmingStakingData[_poolId][_to] = stableFarmingStakingData[_poolId][_from];
        delete stableFarmingStakingData[_poolId][_from];

        // LinearPendingEarlyWithdrawal
        stableFarmingPendingWithdrawals[_poolId][
            _to
        ] = stableFarmingPendingWithdrawals[_poolId][_from];
        delete stableFarmingPendingWithdrawals[_poolId][_from];

        farmingPendingEarlyWithdrawals[_poolId][
            _to
        ] = farmingPendingEarlyWithdrawals[_poolId][_from];

        delete farmingPendingEarlyWithdrawals[_poolId][_from];
