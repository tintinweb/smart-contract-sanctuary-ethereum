...
// File: contracts/ForestNaughtyshrooms.sol
...
contract ForestNaughtyShrooms is ERC721, ERC721URIStorage, Ownable {
	...
	//uint256 public MAX_TOKENS_PRESALE=111;
	uint256 public FEE;
	//uint256 public FEEpresales;
	uint256 public MAXNFTperADDR;
	uint256 public revealuntil;
    ...
    constructor(string memory mybaseuri, string memory myrevealuri) ERC721("Forest of naughty shrooms", "FNS") {
		_tokenIdCounter.increment();
		setBaseURI(mybaseuri);
		setprerevealURI(myrevealuri);
		FEE=4000000000000000;
		//FEEpresales=30000000000000000;
		MAXNFTperADDR = 5;
		revealuntil=0;
	...
	function setRevealuntil(uint256 _newrevealuntil) public onlyOwner {
		revealuntil = _newrevealuntil;
	}
	...
	function safeMint(uint minttimes) public payable {
		...
		require(!paused);
			...
			/*else if (presale==true){
				...
				require(minttimes<=MAXNFTperADDR,"Max 5 NFTs per transaction!");
	...
	function safeMintAirdrop(uint minttimes, address toAddress) public payable onlyOwner {
		...
		require(minttimes <= MAXNFTperADDR,"Max 10 NFTs per tx!");
