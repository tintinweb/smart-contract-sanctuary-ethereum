    ...
    Ownable
{
     ...
     */
    modifier checkBuyPeriod(bytes32[] calldata proof) {
        if (isWhitelistActivated) {
            bool isWhitelisted = checkWhitelisted(msg.sender, proof);
            require(
                (isWhitelisted && block.timestamp >= saleStartTime) ||
                    (!isWhitelisted && block.timestamp >= publicSaleStartTime),
    ...
    function buyNFTs(uint256 tokensNumber, bytes32[] calldata proof)
        external
        payable
        whenNotPaused
        checkBuyPeriod(proof)
    {
        require(
