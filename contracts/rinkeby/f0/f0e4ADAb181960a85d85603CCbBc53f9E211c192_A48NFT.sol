...
contract A48NFT is HoleTokenMetadata, Ownable {
  ...
  bool private _isSaleActive;
    ...
    constructor(string memory name, string memory symbol, uint256 maxNftSupply,string memory baseuri)
    {
      nftName = name;
      nftSymbol = symbol;
...
//, string calldata _uri
  function mint( uint256 _count) external  payable {
    ...
    require(afterMintSupply <= maxSupply, "Purchase would exceed max supply of Hole");
    ...
    for (uint256 i = 1; i <= _count; i++) {
          ...
          if (totalSupply() < maxSupply)
          {
            ...
            //"ipfs://QmeEk4gC4FUdBtE8VvHsvT6kWnaQRiUSBGKHAdvMa8X8Zd/"
            string memory uri = strConcat(BaseURI,tokenid.toString(),".json");
