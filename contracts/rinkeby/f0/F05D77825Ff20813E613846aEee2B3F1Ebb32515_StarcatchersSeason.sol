...
contract StarcatchersSeason is ERC1155, Ownable, ERC1155Burnable, VRFConsumerBaseV2 {
    ...
    mintCompliance(_addresses.length, _episodeId, EpisodeStatus.STOPPED)
  {
    ...
    for (uint64 i = 0; i < _addresses.length; i++) {
      require(!e.airdrop[_addresses[i]], 'Call includes address which has already been airdropped');
    }
    _requestMint(_addresses, uint32(_addresses.length), _episodeId);
    e.supply += uint64(_addresses.length);
    for (uint64 i = 0; i < _addresses.length; i++) {
      e.airdrop[_addresses[i]] = true;
    }
  }
    ...
    mintCompliance(_stars.length, _episodeId, EpisodeStatus.STARS_MINT)
  {
    Episode storage e = Episodes[_episodeId];
    require(
      msg.value == _stars.length * e.starPriceWei,
      "Incorrect payment"
    );
