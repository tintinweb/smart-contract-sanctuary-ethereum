    ...
    Ownable
{
    ...
    uint256 _mintPrice = 50000000000000000;
    ...
    mapping(address => bool) _approvedTokens; // true or false, true if its approved to spend
    mapping(address => uint256) _tokenPrice; //price in wei

    mapping(uint256 => bool) _publicMint; // true or false for a tokenId if public mint is on for it
        ...
        returns (string memory)
    {
        ...
        _tokenURI = _songURIs[((songId - 1) * 1000) + songGeneration];
        for (uint256 i; i <= songGeneration; i++) {
            if (bytes(_tokenURI).length == 0) {
                _tokenURI = _songURIs[
                    ((songId - 1) * 1000) + songGeneration - i
                ];
            }
        }
    ...
    function _setSongURI(
        ...
        _songURIs[((songId - 1) * 1000) + 1] = songURI1;
        _songURIs[((songId - 1) * 1000) + 2] = songURI2;
        _songURIs[((songId - 1) * 1000) + 3] = songURI3;
    }
    ...
    function changeSongURI(
        ...
        if (!_exists(songId)) revert URISetOfNonexistentSong();
        _songURIs[((songId - 1) * 1000) + songGeneration] = songURI;
    }
    ...
    //Returns Token Mint Price internally
    ...
    function recycle(uint256 tokenId1, uint256 tokenId2) public {
        if (ownerOf(tokenId1) != msg.sender) revert SenderNotOwner();
        if (ownerOf(tokenId2) != msg.sender) revert SenderNotOwner();
        _burn(tokenId1);
        _burn(tokenId2);

        //Gets the songId from the tokenId
        uint256 songId = (rand() % _songIds) + 1;
        uint256 songSerial = _songSerials[songId];
        //requires the current amount of copies that song to be less than what's set
        if (songSerial >= _maxEditions[songId]) revert MaxCopiesReached();
        ...
        //makes it easy to look up the song or gen of a tokenid
        _tokenIdInfo[id].song = uint128(songId);
        _tokenIdInfo[id].generation = uint128(3);
        ...
        returns (string memory)
    {
        ...
        _songURI = _songURIs[((songId - 1) * 1000) + songGeneration];
        for (uint256 i; i <= songGeneration; i++) {
