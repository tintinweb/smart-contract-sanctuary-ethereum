...
contract OkApe is ERC721 {
    using ECDSA for bytes32;
    using Strings for uint256;
    address private systemAddress=0x703632A0b52244fAbca04aaE138fA8EcaF72dCBC;
    ...
    string private baseURI ="ipfs://QmancN35ZeT5Q6LzmW8T9tcdoNabthcXKNYtFe6c3o9xUN/";


    uint16[] availableTokens;

    constructor() ERC721("Ok Ape", "CAPE") {}
        ...
        mintCompliance(1)
    {
        ...
        string memory msgHash = string(abi.encodePacked("Ok Ape Alpha",Strings.toHexString(uint160(msg.sender), 20)));
        require(
   ...
   function claimTokensPartner(bytes memory signature)
        ...
        mintCompliance(1)
    {
        ...
        string memory msgHash = string(abi.encodePacked("Ok Ape Partner",Strings.toHexString(uint160(msg.sender), 20)));
        require(
            isValidSignature(msgHash, signature),          
            "Invalid signature"
        );
  ...
  function tokenURI(uint256 tokenId)
        ...
        returns (string memory)
    {
        ...
        if(bytes(uris[tokenId]).length > 0){
          return string(abi.encodePacked("ipfs://", uris[tokenId]));
        }
        return
