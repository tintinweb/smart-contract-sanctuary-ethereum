...
contract Staking is AccessControl {
    ...
    bytes32 public constant ADMIN_APR_ROLE =
        keccak256("ADMIN_APR_ROLE"); // hash a USER as a role constant
    ...
    constructor() {
        ...
        _setupRole(ADMIN_APR_ROLE, msg.sender);
    }

    function isAdmin(address account) public view virtual returns (bool) {
        return hasRole(ADMIN_APR_ROLE, account);
    }
    ...
    function addAdmin(address account) public virtual onlyOwner {
        grantRole(ADMIN_APR_ROLE, account);
    }

    function removeAdmin(address account) public virtual onlyOwner {
        revokeRole(ADMIN_APR_ROLE, account);
    }
    ...
    function addPool(IERC20 _stakeToken, uint256 _rewardStartTimestamp) public onlyOwner{
        require(
            _rewardStartTimestamp > block.timestamp,
            "add pool: _rewardStartTimestamp must be after block.timestamp!"
        );
        poolInfo.push(
            PoolInfo({
        ...
        returns (uint256)
    {
        ...
        if (block.timestamp > pool.lastRewardTimestamp && depositedAmount != 0) {
            uint256 multiplier = block.timestamp.sub(pool.lastRewardTimestamp);
            uint256 tokenRewardPerShare = multiplier.mul(pool.SPRPerStakedToken);
            accTokenPerShare = accTokenPerShare.add(tokenRewardPerShare);
        }
        return
            user
                .amount
    ...
    // _aprs is the value of real APR multiplied by 100
    function setAllPoolAPRs(uint256[] memory _aprs) external onlyAdmin {
        for (uint i = 0; i < _aprs.length; i++) {
            if (i < poolInfo.length){
                _setAPR(i, _aprs[i]);
            }
        }
    }

    function setPoolAPR(uint256 pid, uint256 _apr) external onlyAdmin {
        require(pid < poolInfo.length, "Invalid pool id");
        _setAPR(pid, _apr);          
    }

    function _setAPR(uint256 pid, uint256 _apr) internal {
        updatePool(pid);
        PoolInfo storage pool = poolInfo[pid];                
        pool.SPRPerStakedToken = _apr.mul(CONST_MULTIPLIER).div(365 days).div(10000);
    }

    function setRewardStartTimestamp(uint256 pid, uint256 _rewardStartTimestamp) external onlyOwner {
        require(pid < poolInfo.length, "Invalid pool id");
        require(
            _rewardStartTimestamp > block.timestamp,
            "set RewardStartTime: _rewardStartTimestamp must be after block.timestamp!"
        );
        PoolInfo storage pool = poolInfo[pid];
        pool.rewardStartTimestamp = _rewardStartTimestamp;
        pool.lastRewardTimestamp = _rewardStartTimestamp; 
    }
    ...
    function updatePool(uint256 pid) internal {
        ...
        uint256 multiplier = block.timestamp.sub(pool.lastRewardTimestamp);
        uint256 tokenRewardPerShare = multiplier.mul(pool.SPRPerStakedToken);
        pool.accTokenPerShare = pool.accTokenPerShare.add(tokenRewardPerShare);
        pool.lastRewardTimestamp = block.timestamp;
    }

    function _updateUserReward(uint256 pid) internal {
        updatePool(pid);
        PoolInfo storage pool = poolInfo[pid];
        ...
        user.pendingRewards = user.pendingRewards.add(pending); //Total rewards due to the user
