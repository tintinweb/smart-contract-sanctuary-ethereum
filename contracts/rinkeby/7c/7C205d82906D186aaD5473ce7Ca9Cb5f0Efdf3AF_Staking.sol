...
contract Staking is AccessControl, ReentrancyGuard {
    ...
    constructor(address _lpToken, address _xxx, address _dao) {
    ...
    function modifyMinUnstakeFreezeTime(uint256 _freezeDays) external onlyRole(DAO_ROLE) returns (bool success) {
        minUnstakeFreezeTime = _freezeDays * 1 days;
    ...
    function claim() external nonReentrant {
        ...
        require(_stakes[msg.sender].balance > 0 && rewardInXXX > 0, "No XXX to claim");

        xxx.mint(msg.sender, rewardInXXX - _stakes[msg.sender].claimed);
        _stakes[msg.sender].timestamp = block.timestamp;
        _stakes[msg.sender].claimed += rewardInXXX;
    ...
    function unstake() external nonReentrant {
        ...
        require(block.timestamp - s.timestamp >= minUnstakeFreezeTime, "Less than minUnstakeFreezeTime");
        require(dao.unstakeAllowance(msg.sender), "Voting is not finished");

        uint256 rewardInXXX = _calculateRewards(msg.sender);

        if (rewardInXXX > s.claimed) {
            xxx.mint(msg.sender, rewardInXXX - s.claimed);
        }
    ...
    function stakeOf(address _staker) external view onlyRole(DAO_ROLE) returns (uint256) {
        return _stakes[_staker].balance;
    }

    function getStakeInfo(address _from) external view returns (uint256 _balance, uint256 _timestamp, uint256 _rewards) {
        return(_stakes[_from].balance, _stakes[_from].timestamp, _calculateRewards(_from));
    }

    function getStakeSettings() external view returns (uint256 _interest, uint256 _minRewardsTimestamp, uint256 _minUnstakeFreezeTime) {
