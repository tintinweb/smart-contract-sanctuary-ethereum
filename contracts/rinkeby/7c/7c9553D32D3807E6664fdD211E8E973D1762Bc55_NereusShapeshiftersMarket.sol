    ...
    INereusShapeshiftersMarket
{
    ...
    * amount - amount of nfts to purchase
     */
    function getWavePriceForAddress(address wallet, uint256 amount) external view returns(uint256) {
        uint256 claimedAmountPerUser = _saleWavesClaimed[_currentSaleWave][wallet];

        if(_saleWaves[_currentSaleWave].claimAllowance < claimedAmountPerUser + amount) {
            return 0;
        }
        ...
        for (uint256 i = claimedAmountPerUser + 1; i <= claimedAmountPerUser + amount; i++) {
        ...
        isSalesAllowed(amount)
    {
        ...
        for (uint256 i = 0; i < amount; i++) {
            ...
            emit NFTPurchased(_msgSender(), tokenId, tokenURI, _currentSaleWave);
        }
    ...
    * Lets minter ability to DeNFTBridge
     */
    function giveawayToDeNFTBridge() external onlyOwner {
         IDeNFT(_nftContractAddress).giveawayToDeNFTBridge();
         emit CollectionMinterGiveaway();
    }

    /**
    * Generate and buy single item
     */
    function generateSingleItem() private returns(uint256 tokenId, string memory tokenURI) {
    ...
    function withdrawBalance() external onlyOwner nonReentrant {
        ...
        payable(owner()).transfer(withdrawBalance);

        emit BalanceWithdrawn(withdrawBalance);
    }
...
interface INereusShapeshiftersMarket {
    ...
    event CollectionMinterGiveaway();
}
...
interface IDeNFT is IERC721Upgradeable, IERC4494 {
    ...
    function giveawayToDeNFTBridge() external;
