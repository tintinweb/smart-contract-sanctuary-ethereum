...
import "Ownable.sol";
 ...
 contract Lottery is VRFConsumerBaseV2, Ownable
 {
    ...
    function approveLink(uint256 amount) public payable returns(bool success)
    {
        //return LINKTOKEN.approve(address(this), amount);
        return LINKTOKEN.approve(msg.sender, amount);
    }

    function getLinkAllowance(address owner, address spender) public view returns(uint256 remaining)
    {
    ...
    function startLottery() public payable onlyOwner
    {
        ...
        if (getBalance() < 3000000000000000000)
        {
            LINKTOKEN.transferFrom(msg.sender, address(this), 3000000000000000000);
        }

        if( 3000000000000000000 > LINKTOKEN.balanceOf(address(this))) //if less than 2 link left in contract
        {
            topUpSubscription(3000000000000000000); //topup 3 link
        }
