...
contract Machines is Ownable, ERC721DEYE, ReentrancyGuard {
    ...
    function mintPublic(uint64 quantity) public payable callerIsUser {
        ...
        // require(_numberMinted(msg.sender) + quantity <= publicMintWalletLimit, "Exceeds allowed wallet quantity");

        _safeMint(msg.sender, quantity);
    }

    function mintPrivate(uint64 quantity) public payable callerIsUser {
        require(contractStatus == ContractStatus.Presale, "Private minting not available");
        require(msg.value >= price * quantity, "Not enough ETH sent");
        require(_numberMinted(msg.sender) + quantity <= _getAux(msg.sender), "Exceeds allowed wallet quantity");
    ...
    function mintAllowedQuantityForAddress(address account) public view returns (uint256) {
        if (contractStatus == ContractStatus.Presale) {
            return _getAux(account) - _numberMinted(account);
        }
        if (contractStatus == ContractStatus.Public) {
            return publicMintTransactionLimit;
            // return publicMintWalletLimit - _numberMinted(account);
        }
        return 0;
    }
    ...
    function setAllowlist(address[] memory addresses, uint64[] memory quatities) external onlyOwner {
        require(addresses.length == quatities.length, "addresses does not match quatities length");
        for (uint256 i = 0; i < addresses.length; i++) {
            _setAux(addresses[i], quatities[i]);
        }
    }
