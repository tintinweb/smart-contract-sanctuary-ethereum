...
contract PoolFactory is Ownable {
    ...
    // vesting contract caddress
    address public vesting;

    // offered currency used to buy ticket
    address public offeredCurrency;
    ...
    // Mapping from User token. From tokens to array of created Pools for token
    mapping(address => address) public getTickets;
    ...
    constructor(address _vesting, address _offerCurrency, string memory _name, string memory _symbol) {
        vesting = _vesting;
        offeredCurrency = _offerCurrency;
        name = _name;
        symbol = _symbol;
    ...
    // Update ticket name, ticket symbol, vesting address and offered currency
    function updateInformation(
        ...
        require(admin[msg.sender],"ICOFactory::ONLY_ADMIN_CAN_CREATE_POOL");
        name =  _name; 
        symbol = _symbol;
        vesting = _vesting;
        offeredCurrency = _offeredCurrency;

        emit UpdateInformation(_offeredCurrency, _vesting, _name, _symbol);
    }
    ...
    // */
    function registerPoolAndTicket(
        address _token,
        uint256 _amountIDO,
        uint256 _openTime,
        uint256 _duration,
        ...
        require(_amountIDO > 0, "ICOFactory::ZERO_AMOUNT_IDO_TOKEN");
        require(_ticketCloseTime != 0, "ICOFactory::ZERO_CLOSE_TIME");
        require(_openTime != 0, "ICOFactory::ZERO_OEPN_TIME");        
        
        (address createIDO, address createTicket) = initialize(
            _token,
            _amountIDO,
            _openTime,
            _duration,
        ...
        getPools[msg.sender][_token].push(createIDO);
        allPools.push(createIDO);

        getTickets[createIDO] = createTicket;
        allTickets.push(createTicket);

        emit PoolCreated(msg.sender, _token, createTicket, vesting, createIDO, allPools.length - 1);
    }
    ...
    function initialize(
        ...
        uint256 _ticketPrice,
        string memory _uri
    ) internal returns (address createIDO, address createTicket)  {
        bytes memory bytecode = type(IDOPool).creationCode;
        bytes memory bytecodeTicket = type(Ticket1155).creationCode;
        ...
        assembly {
            createIDO := create2(0, add(bytecode, 32), mload(bytecode), salt)
            createTicket := create2(0, add(bytecodeTicket, 32), mload(bytecodeTicket), salt)
        }

        IDOPool(payable(createIDO)).addressInitialize(
            _token,
            offeredCurrency,
            createTicket,
            vesting
         ...
         IDOPool(payable(createIDO)).uintInitialize(
            _amountIDO,
            _openTime,
            _duration,
            _ticketCloseTime,
        ...
        IVesting(vesting).setPoolVesting(createIDO);


        Ticket1155(createTicket).initialize(
            name,
            symbol,
            _uri,
            createIDO
    ...
    */
     ...
     * @return Return number of created pools
    */
    function allTicketsLength() public view returns (uint256) {
    ...
    //  * @dev User can retrieve their created pool by address of tokens
    //  * @param _creator Address of created pool user
    //  * @param _pool Address of token want to query
    //  * @return Created Pool Address
    // */
    function getCreatedTicketsByPool(address _pool)
        public
        view
        returns (address)
    {
        return getTickets[_pool];
    }
}
...
contract IDOPool is Ownable, Pausable, KLAYDAOWhitelist, IERC1155Receiver, VRFConsumerBase {
    ...
    function setUpVestingInformation(uint256 _start, uint256 _cliff, uint256 _duration, uint128 _TGE) external onlyOwner {
        ...
        require(_cliff != 0, "POOL::INVALID_VESTING_CLIFF_TIME");
    ...
    function createVestingSchedule(
        ...
        require(_amountIDOtoken > 0, "POOL::INVALID_VESTING_DURATION");
...
contract Ticket1155 is ERC1155, Ownable {
    ...
    event Mint(address _to, uint256 _id, uint256 _quantity, bytes _data);
    event Burn(address _from, uint256 _id, uint256 _quantity);
    event Initialize(string _name, string _symbol, address _minter);

    constructor() ERC1155("") {
       
    }

    function initialize(string memory _name, string memory _symbol, string memory _uri, address _idoPool) external {
        require(init == false, "POOL::INITIALIZED");
        ...
        emit Initialize(_name, _symbol, _idoPool);
    }
    ...
    function mint(
        ...
        require(msg.sender == idoPool, "TICKET::ONLY_IDO_POOL_CAN_MIN");

        tokenSupply[_id] = tokenSupply[_id] + _quantity;

        _mint(_to, _id, _quantity, _data);

        emit Mint(_to, _id, _quantity, _data);
    ...
    function burn(
        ...
        require(msg.sender == idoPool, "TICKET::ONLY_IDO_POOL_CAN_BURN");
         
        _burn(_from, _id, _quantity);

        tokenSupply[_id] = tokenSupply[_id] - _quantity;

        emit Burn(_from, _id, _quantity);
    }
