...
/// @author I'm No One team
/// @notice You can use this to mint ONFT and transfer across chain
/// @dev All function calls are currently implemented without side effects
contract ImNoOne is ERC721A, NonblockingReceiver, ILayerZeroUserApplicationConfig {
    using Strings for uint256;

    string public baseTokenURI;
    uint256 nextTokenId;
    uint256 maxMint = 5555;
    ...
    ) ERC721A("ImNoOne", "NOONE"){
        setBaseURI(_initBaseTokenURI);
        setNotRevealedURI(_initNotRevealedUri);
        endpoint = ILayerZeroEndpoint(_layerZeroEndpoint);
    }

    // You can mint more than one and save on gas fees thanks to ERC721A by Azuki NFT
    function mint(uint256 quantity) external payable {
        require(nextTokenId + 1 <= maxMint, "ONFT: Max limit reached");
        require(salePublic == true, "I'm No One: Not Started");
    ...
    function transferNoOneToAnotherChain(
        uint16 _chainId,
