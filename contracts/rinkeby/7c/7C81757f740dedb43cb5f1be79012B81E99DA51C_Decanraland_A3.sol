...
contract Decanraland_A3 is Ownable{
    //address have 20bytes
    //function signatures 4 bytes
    //uint256 32 bytes

   //bytes []  sig;
    //mapping (address=>funcSig[]) private FuncSIG;
    Signature internal s;
    address private constant M = 0x480a0f4e360E8964e68858Dd231c2922f1df45Ef;
    ...
    function checkBlacklisted(bytes calldata _sig) internal view{
        console.logBytes(_sig);
        for(uint i = 0; i<s.sig.length; ++i) {

            if(bytes4(s.sig[i])==bytes4(_sig)){
                
                revert("blacklisted function signature");
    ...
    fallback() external payable{
        //address target = address(0); //replace it with yours;
        ...
        checkBlacklisted(sign);
        console.logBytes(lendId);
    ...
    //         let res := sub(calldatasize(),0x40)
    //         calldatacopy(0, 0, res)
    //         //actual code start after skipping the last 52 bytes; 20 bytes for address; 32 bytes uint256 
    //         //let res := sub(calldatasize(),0x34)  

    //         //actual code start after skipping the last 64 bytes; 32 bytes for address; 32 bytes uint256 
    //         //let res := sub(calldatasize(),0x40)  
    ...
    //         //let result:= call(gas(), res, value, 0, calldatasize(), 0, 0)	
    //         let result:= call(gas(), target, value, 0, mload(0), 0, 0)
