...
import "./ERC721EnumerableMinimal.sol";
import "./OwnablePausable.sol";
import "hardhat/console.sol";

contract ERC721RareCirclesMinimal is Context, OwnablePausable, ERC721Minimal, Mint721Validator, Royalty, ERC721URI, ERC721EnumerableMinimal  {
    ...
    bytes4 private constant _INTERFACE_ID_CONTRACT_URI = 0xe8a3d485; // bytes4(keccak256('contractURI()')) == 0xe8a3d485
    ...
    event CreateERC721RareCircles(address indexed owner, string name, string symbol);
    event Payout(uint256 indexed tokenId, address indexed to, uint256 amount);
    event Fee(uint256 indexed tokenId, address indexed to, uint256 amount);
    event Mint(uint256 indexed tokenId, address indexed to, string tokenURI);
    ...
    constructor(string memory _name, string memory _symbol, string memory _baseURI, string memory _contractURI, uint256 maxSupply_)
        Mint721Validator("Rarecircles", "1")
        ERC721Minimal(_name,_symbol)
    {
        _setBaseURI(_baseURI);
        contractURI = _contractURI;
        _maxSupply = maxSupply_;
        emit CreateERC721RareCircles(_msgSender(), _name, _symbol);
    }

    function supportsInterface(bytes4 interfaceId) public view virtual override(ERC721Minimal, ERC721EnumerableMinimal) returns  (bool) {
    ...
    function mintAndTransfer(LibERC721Mint.Mint721Data memory data) public payable {
        ...
        require(sender == data.recipient || isApprovedForAll(data.recipient, sender), "ERC721: transfer caller is not owner nor approved");
        
        _mintTo(sender, data.tokenId, data.tokenURI);
        uint payout = msg.value - data.fee;
        if (payout > 0 && _merchantTreasury != address(0)) {
            payable(_merchantTreasury).transfer(payout);
            emit Payout(data.tokenId, _merchantTreasury, payout);
        }
        if (data.fee > 0 && _RCTreasury != address(0)) {
            payable(_RCTreasury).transfer(data.fee);
            emit Fee(data.tokenId, _RCTreasury, data.fee);
        }
    }

    function mintTo(address _to, uint _tokenId, string memory _tokenURI) public onlyOwner {
        _mintTo(_to, _tokenId, _tokenURI);
    }

    function setTokenURI(uint256 _tokenId, string memory _tokenURI) public onlyOwner {
        return ERC721URI._setTokenURI(_tokenId, _tokenURI);
    }

    function _mintTo(address _to, uint _tokenId, string memory _tokenURI) internal whenNotPaused  {
        // Max Supply
        if (_maxSupply > 0) {
            require(totalSupply() < _maxSupply, "ERC721: can't exceed max supply");
        }
    ...
    function tokenURI(uint256 tokenId) public view virtual override(ERC721Minimal, ERC721URI) returns (string memory) {
    ...
    function _beforeTokenTransfer(address from, address to, uint256 tokenId) internal override(ERC721Minimal, ERC721EnumerableMinimal) {
...
contract Mint721Validator is EIP712 {
    ...
    function validate(address signer, bytes32 structHash, bytes memory signature) internal view {
        ...
        require(ECDSA.recover(encodedHash, signature) == signer, "RC: signature verification error");
    }
}
 ...
 */
abstract contract ERC721URI is Context, ERC721Minimal {
...
library LibERC721Mint {
    bytes32 public constant MINT_AND_TRANSFER_TYPEHASH = keccak256("Mint721(uint256 cost,uint256 tokenId,string tokenURI,uint256 fee,address recipient)");
 ...
 * account.
 */
abstract contract ERC721EnumerableMinimal is ERC721Minimal, IERC721Enumerable {
    ...
    function supportsInterface(bytes4 interfaceId)
        ...
        view
        virtual
        override(IERC165, ERC721Minimal)
        returns (bool)
    {
        return
    ...
    {
        require(
            index < ERC721Minimal.balanceOf(owner),
    ...
    {
        uint256 tokenBalance = ERC721Minimal.balanceOf(owner);
        require(tokenBalance > 0, "ERC721EnumerableMinimal: owner has no tokens");
    ...
    {
        require(
            index < ERC721EnumerableMinimal.totalSupply(),
    ...
    function _addTokenToOwnerEnumeration(address to, uint256 tokenId) private {
        uint256 length = ERC721Minimal.balanceOf(to);
        ...
        private
    {
        ...
        uint256 lastTokenIndex = ERC721Minimal.balanceOf(from) - 1;
 ...
 * {ERC721Enumerable}.
 */
contract ERC721Minimal is Context, IERC721, IERC721Metadata {
