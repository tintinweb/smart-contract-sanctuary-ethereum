...
contract TheCollection is Owned, ERC721A, Banker {
    ...
    uint256 public constant TotalSupply = 6699;
    ...
    string public constant ContractUri = "https://lipsweater.com/contract.json";

    /// @dev Not required to be hosted on IPFS and doesn't change between mainnet and testnets
    /// TODO: Update with production url once available
    string public constant UnrevealedUri = "https://lipsweater.com/unrevealed.json";

    uint256 public constant DraftPriceGM = 1 ether;
    uint256 public constant DraftPriceAGM = 0.5 ether;
    uint256 public constant DraftPriceFO = 0.19 ether;
    uint256 public constant Phase2AllowlistPrice = 0.1 ether;
    uint256 public constant Phase2PublicPrice = 0.15 ether;

    uint32 public constant MaxMintPhase1Allowlist = 1;
    uint32 public constant MaxMintPhase1Total = 5;

    uint32 public constant MaxMintPhase2Allowlist = 2;
    uint32 public maxMintPhase2Total = 10; // Owner has to be able to change this value

    /// @dev Remaing supplies of ranks (after external presale has ended) to be minted during draft day
    uint256 public immutable mintableSupplyGM;
    uint256 public immutable mintableSupplyAGM;
    uint256 public immutable mintableSupplyFO;

    /// @dev Totals of mints for each rank in phase 1.
    /// Doesn't take in account claims from external presale
    uint256 public totalMintedGM;
    uint256 public totalMintedAGM;
    uint256 public totalMintedFO;
    ...
    uint256 public phase1EndTokenId;

    /// @dev Admin settings to manage tokens' URIs
    ...
    constructor(
        ...
        mintableSupplyGM = supplyGM;
        mintableSupplyAGM = supplyAGM;
        mintableSupplyFO = supplyFO;

        openSeaProxyAddress = proxies.openSea;
        raribleProxyAddress = proxies.rarible;
    ...
    ///   - URI must ends with a ".json" extension
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        if (!_exists(tokenId)) revert InvalidToken();

        if (phase2Revealed) {
            return string(abi.encodePacked(phase2RevealedBaseUri, tokenId.toString(), ".json"));
        } else if (phase1Revealed) {
            if (tokenId <= phase1EndTokenId) {
                return string(abi.encodePacked(phase1RevealedBaseUri, tokenId.toString(), ".json"));
            } else {
    ...
    function crossmintDraft(address to, uint32 quantity)
        external
        payable
        isPhase(Phase.One)
        isSale(Sale.Public)
    ...
    function _mintDraft(address to, uint256 quantity) internal {
        if (msg.value == DraftPriceGM * quantity) {
            if (quantity + totalMintedGM > mintableSupplyGM) {
            ...
            if (quantity + totalMintedAGM > mintableSupplyAGM) {
                revert NotEnoughSupply();
            }
            ...
            if (quantity + totalMintedFO > mintableSupplyFO) {
                revert NotEnoughSupply();
            }
    ...
    function claimFromDraft(
        ...
        bytes32[] calldata proof
    ) external payable isPhase(Phase.Two) totalSupplyNotReached(quantity) {
        ...
        withProperFunds(quantity, Phase2AllowlistPrice)
    {
        ...
        _counters[msg.sender].phase2Public += quantity;
    ...
    /// Each provided account gets 1 GM
    ...
    /// Each provided account gets 1 Gold
    function airdropGold(address[] memory accounts) external onlyOwner {
        for (uint256 i = 0; i < accounts.length; ++i) {
    ...
    function airdropGoldsToVault(address vault, uint256 quantity) external onlyOwner {
    ...
    function setMaxMintPhase2Total(uint32 total) external onlyOwner {
    ...
    modifier totalSupplyNotReached(uint256 quantity) {
        if (_totalMinted() + quantity > TotalSupply) {
            revert NotEnoughSupply();
        }
...
abstract contract Banker is IERC2981, Owned {
    ...
    function withdraw(address payable account_) external {
        ...
        unchecked {
            ...
            _totalWithdrawn += uint64(availableFunds);
        }
