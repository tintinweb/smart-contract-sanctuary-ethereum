...
contract NounCats is ERC721, ERC2981, Ownable {
  ...
  // uint256 public price = 0.025 ether; // todo
  ...
  function burnAndReveal(uint256[] calldata _ids) external mintIsOpen() {
  ...
  function mintWithIds(uint256[] calldata _ids) external payable mintIsOpen() {
    IInvisibles invis = IInvisibles(INVISIBLES);
  ...
  function mintCats(uint[] calldata _ids, bool mintNounterparts) external payable mintIsOpen() {
    if (totalSupply() == 5000) revert MintedOut();
    ...
    for (uint i; i < idsLength;) {
      ...
      if (id < 3782 || id > 5000) revert BadID();
      if (_exists(id)) revert CatAlreadyMinted();

      if (mintNounterparts) {
        // if (invis.ownerOf(id) == TREASURY) {
          // Transfer the Invisible to msg.sender
          // invis.transferFrom(TREASURY, msg.sender, id);

          emit InvisibleMinted(msg.sender, id);
        // } else {
        //   revert InvisibleAlreadyTransferred(id);
        // }
      }
