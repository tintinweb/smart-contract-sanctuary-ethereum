...
contract SuperBockRoyalties is Ownable, ReentrancyGuard {
  ...
  function addEarnings(uint[] calldata amounts, uint[] calldata tokenIds) external onlyOwner {
    ...
    require(newEarnings <= unaccounted, "Reported earnings exceed factual balance");
  ...
  function withdraw(uint[] calldata tokenIds) external nonReentrant {
    ...
    require(trackedBalance.withdrawn + withdrawnAmount >= trackedBalance.earned, "Withdrawals exceed earnings");
