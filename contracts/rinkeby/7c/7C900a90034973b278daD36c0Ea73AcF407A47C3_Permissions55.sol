...
import "./permissions/PermissionSet.sol";

contract Permissions55 is Context, ERC1155Burnable, IPermissions55, PermissionRoles, PermissionSet {
    ...
    constructor() ERC1155("https://permissions55.token-forge.io/assets") {
        ...
        _mint(_msgSender(), TOKEN_ROLE_ADMIN, 1, data);
    }

    function addCustomTokenSet(uint256 roleTokenId, uint256 customTokenId) external {
            ...
            revert(
                string(
                    abi.encodePacked(
                        ...
                        " is missing role ",
                        Strings.toHexString(requiredPermission)
                    )
                )
            );
        }
    ...
    function isWhiteListAdmin(address account) public view returns (bool) {
        return balanceOf(account, TOKEN_ROLE_WHITELIST_ADMIN) > 0;
    }

    function isBlackListAdmin(address account) public view returns (bool) {
        return balanceOf(account, TOKEN_ROLE_BLACKLIST_ADMIN) > 0;
    }
...
abstract contract PermissionRoles {
    // ***** Roles ********
...
contract PermissionSet {
    ...
    function addPermissionSet(uint256 id, string calldata name) public {
    ...
    function permissionSets() external view returns (uint256[] memory, string[] memory) {
        ...
        for (uint256 i = 0; i < keys.length; i++) {
            values[i] = _permissionSets.get(i);
        }
        return (keys, values);
    }

    function removePermissionSet(uint256 id) public {
