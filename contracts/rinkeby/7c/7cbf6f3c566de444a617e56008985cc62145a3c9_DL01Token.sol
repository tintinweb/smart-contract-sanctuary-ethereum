...
struct TGamingDetails
{
    address[]   race1;          // doolnies/hunters
    address[]   race2;          // doolz
    address[]   race3;
}
...
struct TRentInfo
{
    ...
    uint256     renterPercent;
}

//--------------------------------------------------------------------------------
interface   IGAME
{
    function    canSpawnLife(string memory collectionName, uint256 tokenId, address userWallet, string memory lifeType) external returns(bool);
}
//--------------------------------------------------------------------------------
contract    ExternalClassesAndObjects
{
    IGAME   public      P2PGAME;
}
//--------------------------------------------------------------------------------
...
abstract contract Ownable is Context
{
    ...
    {
        require(msg.sender==owner() || msg.sender==admin(), "Ownable: Only Admin or Owner are allowed");
        _;
    }
...
//--------------------------------------------------------------------------------
contract ERC721 is Context, ERC165, IERC721, IERC721Metadata, Ownable, ExternalClassesAndObjects
{
    ...
    mapping(uint256 => bool)    public teamTokensStates;    // Is a token was received from the Team reserve? important to establish firstOwner ownership to the new owner
   ...
   constructor(string memory name_, string memory symbol_)
    ...
    function _transfer(address from, address to, uint256 tokenId) internal virtual
    {
        ...
        if (teamTokensStates[tokenId])         // C'est un transfert qui vient de la reserve de la TEAM. Il faut etablir le firstOwner(ship) au new proprio
        {
            teamTokensStates[tokenId] = false;  // ce n'est plus la propriété du owner
            firstOwners[tokenId]      = to;     // Voila . Le nouveau proprietaire devient un VIP
        }
    }
...
//--------------------------------------------------------------------------------
//--------------------------------------------------------------------------------
contract DL01Token     is  ERC721Enumerable, ReentrancyGuard
{
    using Address for address;
    using Strings for uint256;
    ...
    uint256 public      presalesTimestamp  = 1640995200;     // Jan 1st 2022
    uint256 public      salesTimestamp     = 1649980800;     // Apr 15th 2022

    uint256 public      mintOnlyPrice      = 0.0005 ether;
    uint256 public      whitelistMintPrice = 0.0004 ether;
    ...
    mapping(uint256 => uint256)     public originTokenIds;
    mapping(uint256 => uint256[])   public nftSpawnedTokenIds;      // liste des token ids des clones
    mapping(uint256 => bool)        public rentingStates;

                //-----

    uint256 RND=0;

    string[] public consons = [ "B","C","D","F","G","H","J","K","L","M","N","P","Q","R","S","T","V","W","X","Z" ];
    string[] public voyels  = [ "A","E","I","O","U","Y" ];
    string[] public precons = [ "L","R","H" ];
    string[] public endcons = [ "K","L","R","S","T","X","Z"];
    ...
    constructor()   ERC721("The DLN7 Token", "DLN06")   // temporary Symbol and title
    {
        ownerWallet = msg.sender;

        RND = block.timestamp;
        
        setGamingContract(0x89435A7EBD9Ae3452903d28Fd0843DCE560B2874);
    }
    //=============================================================================
    ...
    //-----------------------------------------------------------------------------
    function nftTransfer(address wallet, uint256 tokenId) external
    {
        require(_isApprovedOrOwner(msg.sender, tokenId), "ERC721: transfer caller is not owner nor approved");
       
        _transfer(msg.sender, wallet, tokenId);

        require(isERC721ReceivedCheck(msg.sender, wallet, tokenId, ""), "ERC721: transfer to non ERC721Receiver implementer");
    }
    //-----------------------------------------------------------------------------
    function nftBulkTransfer(address[] memory wallets, uint256[] memory tokenIds) external
    {
        require(wallets.length==tokenIds.length, "Invalid quantities for wallets or token ids");

        for (uint256 i; i<wallets.length; i++)
        {
            uint256 id       = tokenIds[i];
            address toWallet = wallets[i];

            require(_isApprovedOrOwner(msg.sender, id), "ERC721: transfer caller is not owner nor approved");
       
            _transfer(msg.sender, toWallet, id);

            require(isERC721ReceivedCheck(msg.sender, toWallet, id, ""), "ERC721: transfer to non ERC721Receiver implementer");
        }
    }
    //=============================================================================
    //=============================================================================
    ...
    function    getCurrentPhase() public view returns(string memory)
    {
             if (block.timestamp>=salesTimestamp)       return "sales";
                                                        ...
                                                        return "launch";            // pas de vente a ce moment là
    }
    //=============================================================================
    function    getPriceForMinting()            view public returns(uint256)
    {
        uint256 price = mintOnlyPrice;
    ...
    //=============================================================================
    function    setPrices(uint256 newPriceInWei, uint256 newWhitelistPrice)             external onlyOwner
    {
        mintOnlyPrice      = newPriceInWei;
    ...
    function    reserve(uint256 quantity)                   external onlyOwner
    {
        ...
        for (uint256 i=0; i<quantity; i++)
        {
            ...
            generateDoolnyName(mintedTokenCount);

            tokenCreationEpochs[mintedTokenCount] = block.timestamp;
            teamTokensStates[mintedTokenCount]    = true;               // This is to dispatch firstOwners feature correctly. Since it's not for the team advantage
        }
    }
    //=============================================================================
    function    mint(uint256 quantity, uint256 affHash)                      external payable callerIsUser nonReentrant
    {
        ...
        uint256 orderTotalPrice = price * quantity;
    ...
    function    withdraw() external
    {
        ...
        //----- Let's withdraw to ALL collaborators since one collaborator asked for a withdraw
        //----- It's the way to always send the right share to each one on demand
    ...
    //=============================================================================
    //=============================================================================
    function    setAffiliate(string memory username) external returns(uint256)
    {
        return setAffiliateEx(username, msg.sender, 800);       // 8% for standard affiliates. they login by themselves
    }
    //=============================================================================
    function    setAffiliateByOwner(string memory username, address wallet, uint256 percentInM100) external onlyOwner returns(uint256)
    {
        return setAffiliateEx(username, wallet, percentInM100);
    }
    //=============================================================================
    function    setAffiliateEx(string memory username, address wallet, uint256 percentInM100) internal returns(uint256)
    {
        uint256 affHash = forgeAffHash(username, wallet);
        
        TAffiliate memory lastAff = affiliates[affHash];

             if (percentInM100<10)          return 0;
        else if (percentInM100>50*100)      return 0;

        if (lastAff.walletAddress==address(0x0))        // NEW CREATION
        {
            affiliates[affHash] = TAffiliate( msg.sender, percentInM100, username, true, 0 );       // 8%
        }
        else                                            // Already set, change values
        {
            affiliates[affHash].shareValue = percentInM100;
        }

        return affHash;
    }
    //=============================================================================
    function    isAffiliateExists(string memory username) external view returns(bool)
    {
        uint256 affHash = forgeAffHash(username, msg.sender);
        
        TAffiliate memory lastAff = affiliates[affHash];

        return (lastAff.walletAddress==address(0x0));       
    }
    //=============================================================================
    function    getAffiliateStatsFromHash(uint256[] memory affHashes) view external returns(uint256[] memory)
    {
    ...
    //=============================================================================
    function    getAffiliateStats(string[] memory usernames, address[] memory wallets) view external returns(uint256[] memory)
    {
        uint256          nUser   = usernames.length;
        uint256          nWallet = wallets.length;

        require(nUser==nWallet && nUser!=0, "Invalid datas in request");

        uint256[] memory affSales = new uint256[](nUser);

        for (uint256 i=0; i<nUser; i++)
        {
            uint256 hash = forgeAffHash(usernames[i], wallets[i]);
    ...
    //=============================================================================
    function    payAffiliate(uint256 affHash) internal
    {
    ...
    //=============================================================================
    //=============================================================================
    function    setGamingContract(address gameContractAddress) public onlyOwner
    {
        P2PGAME = IGAME(gameContractAddress);
    }
    //=============================================================================
    //=============================================================================
    //=============================================================================
    //=============================================================================
    function    generateDoolnyName(uint256 tokenId) internal returns(string memory)
    {
        string memory word1;
        string memory word2;
        string memory word3;

        uint256 n1 = consons.length;

        uint256 steps = rand(4,6);

        uint256 lastKind = 111;
        ...
        for (uint j; j<100; j++)
        {
            ...
            {
                uint256 kind = rand(0,3);

                uint256 v = (lastKind*256) + kind;

                     if (v==0)              kind++;   
                else if (v==256)            kind+=2;
                else if (v==512)            kind++;
                else if (v>=769 && v<=771)  kind=0;

                if (kind==0)                            // VOYELLE 
                {
                    name = string(abi.encode(name, voyels[ rand(0,voyels.length-1) ]));    
                    nChar++;
                }
                else if (kind==1)                       // CONSONNE + VOYELLE     
                {
                    word1 = consons[ rand(0,n1-1) ];     
                    word2 =  voyels[ rand(0,voyels.length-1) ];
                    name  = string(abi.encode(name, word1, word2 ));    
                    nChar+=2;
                }
                else if (kind==2)                       // CONSONNE + PRE + VOYELLE     
                {
                    word1 = consons[ rand(0,n1-1) ];     
                    word2 = precons[ rand(0,precons.length-1) ];
                    word3 =  voyels[ rand(0,voyels.length-1) ];
                    name  = string(abi.encode(name, word1, word2, word3 ));    
                    nChar+=3;
                }
                else                                    // CONSONNE + VOYELLE + END
                {
                    word1 = consons[ rand(0,n1-1) ];     
                    word2 =  voyels[ rand(0,voyels.length-1) ];
                    word3 = endcons[ rand(0,endcons.length-1) ];
                    name  = string(abi.encode(name, word1, word2, word3 ));   
                    nChar+=3;
                }
            ...
            if (registeredNames[name]==true)    j--;        // loop
            else                                break;      // quit J loo
        }
    ...
    //--------------------------------------------------------------------------------------
    function    rand(uint256 from, uint256 to) internal returns(uint256) 
    {
        RND = RND+1;
        
        uint256 seed  = uint256(keccak256(abi.encodePacked(block.difficulty, block.timestamp, RND)));
        uint256 range = (to-from)+1;

        return from + (seed % range);
    }
    //=============================================================================
}
/*
