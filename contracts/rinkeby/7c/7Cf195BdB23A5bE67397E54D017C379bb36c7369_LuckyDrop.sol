...
contract LuckyDrop is Ownable, ReentrancyGuard, ERC1155Holder, ERC721Holder {
    ...
    address internal signerAddress = owner();
    ...
    mapping(string => bool) internal nonceInfoMap;
    ...
    function creatLuckyDrop(uint256 tokenId, uint256 luckyDropCategory, address tokenAddress, uint256 totalAmount, uint256 luckyDropId, bytes32 hash, bytes memory signature, string memory nonce) payable public {
        require(nonceInfoMap[nonce] == false, "LuckyDrop:Invalid nonce!");
        //生成parametersBytes
        bytes memory parametersBytes = abi.encode(tokenId, luckyDropCategory, tokenAddress, totalAmount, luckyDropId);
        //验证hash
        require(hashTransaction(msg.sender, nonce, parametersBytes, "creatLuckyDrop") == hash, "LuckyDrop:Invalid hash!");
        //验证签名
        require(matchAddresSigner(hash, signature), "LuckyDrop:Invalid signature!");
    ...
    function releaseLuckyDrop(uint256 releaseLuckyDropId, address payable account, uint256 shareAmount, bytes32 hash, bytes memory signature, string memory nonce) public {
        require(nonceInfoMap[nonce] == false, "LuckyDrop:Invalid nonce!");
        //生成parametersBytes
        bytes memory parametersBytes = abi.encode(releaseLuckyDropId, account, shareAmount);
        //验证hash
        require(hashTransaction(msg.sender, nonce, parametersBytes, "releaseLuckyDrop") == hash, "LuckyDrop:Invalid hash!");
        //验证签名
        require(matchAddresSigner(hash, signature), "LuckyDrop:Invalid signature!");
    ...
    function reBackLuckyDrop(uint256 reBackLuckyDropId, address account, bytes32 hash, bytes memory signature, string memory nonce) public {
        require(nonceInfoMap[nonce] == false, "LuckyDrop:Invalid nonce!");
        //生成parametersBytes
        bytes memory parametersBytes = abi.encode(reBackLuckyDropId, account);
        //验证hash
        require(hashTransaction(msg.sender, nonce, parametersBytes, "reBackLuckyDrop") == hash, "LuckyDrop:Invalid hash!");
        //验证签名
        require(matchAddresSigner(hash, signature), "LuckyDrop:Invalid signature!");
    ...
    function resetSignerAddress(address newSignerAddress) onlyOwner public {
        signerAddress = newSignerAddress;
    }

    function hashTransaction(address sender, string memory nonce, bytes memory parametersBytes, string memory functionName) public pure returns (bytes32) {
        bytes32 transactionHash = keccak256(abi.encodePacked("\x19Ethereum Signed Message:\n32", keccak256(abi.encode(sender, nonce, parametersBytes, functionName))));
