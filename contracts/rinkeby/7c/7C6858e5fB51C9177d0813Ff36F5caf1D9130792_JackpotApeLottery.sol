...
contract JackpotApeLottery is ReentrancyGuard, IJackpotApeLottery, Ownable {
    ...
    // check free tickets of user in lottery
        ...
        nonReentrant
    {
        ...
        uint256 _freeTickets = calculateFreeTickets();
        ...
        nonReentrant
    {
        ...
        uint256 _extraFreeTickets = calculateExtraFreeTickets();
    ...
    function calculateFreeTickets() public view returns (uint256) {
        uint256 tickets = 0;
        if (address(freeTicketToken) != address(0) && freeTicketTokenPrice > 0) {
            tickets = freeTicketToken.balanceOf(msg.sender) / freeTicketTokenPrice;
        }
    ...
    function calculateExtraFreeTickets() public view returns (uint256) {
        uint256 extraTickets = 0;
        if (extraFreeTicketTokens.length > 0) {
            for(uint256 i = 0; i < extraFreeTicketTokens.length; i ++) {
                if (extraFreeTicketTokenPrice[extraFreeTicketTokens[i]] > 0 && IERC20(extraFreeTicketTokens[i]).balanceOf(msg.sender) >= extraFreeTicketTokenPrice[extraFreeTicketTokens[i]]) {
