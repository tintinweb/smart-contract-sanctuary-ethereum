...
contract AltavaNFTStakingUSDT is Ownable, Pausable, IUSDTStaking {
    ...
    function SetPlan(uint8 _duration, uint8 _earn, uint256 _startTime) external override onlyOwner {
        infoPlan.earn = _earn;
    ...
    function GetPlan() view override external returns (uint8, uint8, uint256, uint256) {
        return (infoPlan.earn, infoPlan.duration, infoPlan.planStartTime, infoPlan.planEndTime);
    }

    function SetStaking(uint256[] calldata _tokenIdArr) external override whenNotPaused VerifyToken(_tokenIdArr) VerifyTokenApproved(_tokenIdArr) {
        for (uint256 i = 0; i < _tokenIdArr.length; i++) {
    ...
    function GetStaking(address _account, uint256 _tokenId) external override view returns (uint8 _earn, uint8 _duration, uint256 _startTime, uint256 _endTime){
    ...
    function Claim(uint256[] calldata _tokenIdArr) external override whenNotPaused {
        ...
        for (uint256 i = 0; i < _tokenIdArr.length; i++) {
            ...
            rewardContract.transferFrom(address(this), _msgSender(), earn * duration * 1e5);
