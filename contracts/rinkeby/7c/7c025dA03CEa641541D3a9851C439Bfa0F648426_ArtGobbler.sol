...
contract ArtGobbler is ERC721, ERC1155TokenReceiver {
    ...
    constructor(string memory _BASE_URI) ERC721("ArtGobblerDemo", "ArtGobblerDemo") {
        BASE_URI = _BASE_URI;
    }

    function tokenURI(uint256 gobblerId) public view virtual override returns (string memory) {
         return string(abi.encodePacked(BASE_URI, Strings.toString(gobblerId), ".json"));
     }

    function mint() external {
        ++currentNonLegendaryId;
        getGobblerData[currentNonLegendaryId].owner = msg.sender;
