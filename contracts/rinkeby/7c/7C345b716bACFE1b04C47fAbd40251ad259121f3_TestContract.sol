...
contract TestContract is ERC721, ReentrancyGuard, VRFConsumerBase, Ownable {
    ...
    uint256 public constant TEAM_RESERVE = 1000;

    // ======== Price =========
    uint256 public constant PRICE_PUBLIC = 0.001 ether;
    uint256 public constant PRICE_WHITELIST = 0.0005 ether;

    // ======== Max Mints Per Address =========
    uint256 public constant MAX_PER_MINT_WHITELIST = 1;
    uint256 public constant MAX_PER_MINT_PUBLIC = 3;
    ...
    function whitelistMint(uint256 amount_, Coupon memory coupon_)
        ...
        payable
        onlyWhitelistPhase
        validateAmount(amount_, MAX_PER_MINT_WHITELIST)
        validateSupply(amount_)
        validateEthPayment(amount_, PRICE_WHITELIST)
        nonReentrant
    {
        require(amount_ + addressToMints[msg.sender]._numberOfMintsByAddress <= MAX_PER_MINT_WHITELIST, "Exceeds number of whitelist mints allowed.");
    ...
    function mint(uint256 amount_) 
        ...
        payable
        onlyPublicPhase
        validateAmount(amount_, MAX_PER_MINT_PUBLIC)
        validateSupply(amount_)
        validateEthPayment(amount_, PRICE_PUBLIC)
        nonReentrant
    {
        require(amount_ + addressToMints[msg.sender]._numberOfMintsByAddress <= MAX_PER_MINT_PUBLIC, "Exceeds number of mints allowed.");
