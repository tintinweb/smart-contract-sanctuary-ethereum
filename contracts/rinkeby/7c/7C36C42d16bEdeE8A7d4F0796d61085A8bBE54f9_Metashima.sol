 ...
 */
contract Metashima is Initializable, ERC721Upgradeable {
    ...
    uint256 public saleOpens;
    uint256 public saleCloses;
    ...
    */
    function mint(uint256 _numberOfTokens) external payable {
        require(block.timestamp >= saleOpens && block.timestamp <= saleCloses, "Purchase: window closed");
        require(totalTokens + _numberOfTokens <= MAX_SUPPLY, "Not enough left");
        require(mintsForWallet[msg.sender] + _numberOfTokens <= MAX_TOKENS_PER_WALLET, "Not enough left");
        require(TOKEN_PRICE * _numberOfTokens <= msg.value, 'missing eth');

        for (uint256 i = 1; i <= _numberOfTokens; i += 1) {
            _safeMint(msg.sender, totalTokens + i);
        }

        totalTokens += _numberOfTokens;
        mintsForWallet[msg.sender] += _numberOfTokens;
    }
    ...
    */
    function mintPresaleMerkle(uint256 _numberOfTokens, bytes32[] calldata _merkleProof) external payable {
    ...
    */
    function airdrop(uint256 _numberOfTokens, address _userAddress) external onlyOwner {
        for (uint256 i = 1; i <= _numberOfTokens; i += 1) {
            _safeMint(_userAddress, totalTokens + i);
        }
        totalTokens += _numberOfTokens;
    }
    ...
    *
    * @param _openTime the unix timestamp
    */
    function setPresaleTimes(uint256 _openTime, uint256 _closeTime) external onlyDevOrOwner {
    ...
    * @param _openTime the unix timestamp
    */
    function setSaleTimes(uint256 _openTime, uint256 _closeTime) external onlyDevOrOwner {
        saleOpens = _openTime;
    ...
    */
    function setTokenPrice(uint256 _price) external onlyDevOrOwner {
        TOKEN_PRICE = _price;
    }

    /**
    * @notice set the maximum supply of NFTs that can be minted
    *
    * @param _quantity the total quantity to be allowed
    */
    function setMaxSupply(uint256 _quantity) external onlyDevOrOwner {
    ...
    */
    function setMaxPerWallet(uint256 _quantity) external onlyDevOrOwner {
    ...
    */
    ...
    /**
    * @notice return an array of token IDs owned by the given wallet address.
    *
    ...
    */
    function initialize(string memory name_, string memory symbol_) public initializer {
        ...
        //if doing this, update the token counter
        _owner = msg.sender;
        MAX_SUPPLY = 14159;
        MAX_TOKENS_PER_WALLET = 25;
        TOKEN_PRICE = 0.08 ether;
        BASE_URI = "https://enefte.info/metashima/?token_id=";
        saleOpens = 0;
