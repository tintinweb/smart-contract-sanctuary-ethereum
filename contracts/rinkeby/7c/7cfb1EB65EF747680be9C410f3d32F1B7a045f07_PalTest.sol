...
contract PalTest is ERC721A, Ownable {
  
  uint256 maxSupply = 5;
  string secretSeed;

    constructor() ERC721A("PalTest", "pt") {}
    ...
    function getHexCode(uint256 tokenId, string memory secret) internal pure returns (string memory) {
        return Strings.toHexString(uint256(keccak256(abi.encodePacked(tokenId, secret))));
    }
    ...
    function getBalance() view external {
