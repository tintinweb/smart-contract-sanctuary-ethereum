...
contract FullDigiHLA is Ownable, ERC721A, MerkleWhitelist, ReentrancyGuard {
    ...
    uint256 public WL_START_TIME = 1651201200;
    uint256 public WL_END_TIME = 1650772800;

    uint256 public DA_START_TIME = 1650344400;
    uint256 public DA_END_TIME = 1650780000;

    uint256 public MAIN_START_TIME = 1650351600;
    uint256 public MAIN_END_TIME = 1650787200;

    uint256 public EXTRA_START_TIME = 1650358800;
    uint256 public EXTRA_END_TIME = 1650794400;
       ...
       // onlyExtraWhitelist(proof)
    {
        ...
        require(block.timestamp < EXTRA_END_TIME, "Extra is over");
        require(EXTRA_WALLET_CAP[msg.sender] + quantity <= EXTRA_LIMIT, "Purchase would exceed max number of metacards per wallet."); 
        require(msg.value >= ((DA_FINAL_PRICE / 100) * 80)* quantity,"Must send enough eth for MAIN Mint");
        require(EXTRA_MINTED + quantity <= EXTRA_AMOUNT,"Max supply for EXTRA reached!");
        require(totalSupply() + quantity <= ALL_AMOUNT,"reached max supply");
   ...
   function withdraw() public onlyOwner nonReentrant {
    ...
    function setAllStartTime(uint256 wlTime,uint256 wlEndTime,uint256 daTime,uint256 daEndTime,uint256 mainTime,uint256 mainEndTime,uint256 extraTime,uint256 extraEndTime) external onlyOwner {
    ...
    function setMainStartTime(uint256 mainTime,uint256 mainEndTime) external onlyOwner {
         MAIN_START_TIME = mainTime;
        MAIN_END_TIME = mainEndTime;
    }

    function setExtraStartTime(uint256 extraTime,uint256 extraEndTime) external onlyOwner {
        EXTRA_START_TIME = extraTime;
        EXTRA_END_TIME = extraEndTime;
    }
