...
contract DlpPool is Initializable, CorePool {
    using ErrorHandler for bytes4;
    using Stake for uint256;
    using SafeERC20Upgradeable for IERC20Upgradeable;
    using SafeCast for uint256;
    using VeEarn for uint256;

    mapping(address => uint256) public veBalance;


    event AddAMount(address indexed user, uint256 value);
    event ReduceAMount(address indexed user, uint256 value);
     ...
     * @dev Executed when _useRun is false and pool is not an DLP pool -
    ...
    function stakeAsPool(
        ...
        uint256 veAmount = _value.veEarnd(Stake.MAX_STAKE_PERIOD);

        _addAmount(_staker, veAmount);
     ...
     * in 1 transaction.
     *
     * @notice DLP pool works as a router for claiming multiple pools registered
     *         in the factory.
     *
     * @param _pools array of pool addresses
     * @param _useRun array of bool values telling if the pool should claim reward
     *                 as DLP or RUN
     */
    function claimYieldRewardsMultiple(
        address[] calldata _pools,
        bool[] calldata _useRun
    ) external virtual {
        // checks if contract is paused
        _requireNotPaused();
        ...
        fnSelector.verifyInput(_pools.length == _useRun.length, 0);
        ...
        for (uint256 i = 0; i < _pools.length; i++) {
            ...
            // if the pool passed is the DLP pool (i.e this contract),
            // just calls an internal function
            if (ICorePool(pool).poolToken() == _dlp) {
                // call internal _claimYieldRewards
                _claimYieldRewards(msg.sender, _useRun[i]);
            } else {
                // if not, executes a calls to the other core pool which will handle
                // the other pool reward claim
                UniLPPool(pool).claimYieldRewardsFromRouter(
                    msg.sender,
                    _useRun[i]
                );
            }
     ...
     * @param _pools array of pool addresses
     */
    function claimVaultRewardsMultiple(address[] calldata _pools)
        external
        virtual
    {
        // checks if contract is paused
        ...
        for (uint256 i = 0; i < _pools.length; i++) {
            ...
            // factory contract
            this.claimVaultRewardsMultiple.selector.verifyAccess(
            ...
            if (ICorePool(pool).poolToken() == _dlp) {
                ...
                // if not, executes a calls to the other core pool which will handle
                // the other pool reward claim
                UniLPPool(pool).claimVaultRewardsFromRouter(msg.sender);
            }
        }
    }

    function addVEAmount(address _user, uint256 _amount) external {
        // checks if contract is paused
        _requireNotPaused();
        // expects caller to be a valid contract registered by the pool factory
        this.addVEAmount.selector.verifyAccess(_factory.poolExists(msg.sender));
        _addAmount(_user, _amount);
    }

    function _addAmount(address _user, uint256 _amount) internal {
        require(_user != address(0));
        uint256 balance = veBalance[_user];
        veBalance[_user] = balance + _amount;
        emit AddAMount(_user, _amount);
    }

    function reduceVEAmount(address _user, uint256 _amount) external {
        // checks if contract is paused
        _requireNotPaused();
        require(_user != address(0));
        // expects caller to be a valid contract registered by the pool factory
        this.reduceVEAmount.selector.verifyAccess(
            _factory.poolExists(msg.sender)
        );
        uint256 balance = veBalance[_user];
        require(balance >= _amount);
        veBalance[_user] = balance - _amount;
        emit ReduceAMount(_user, _amount);
    }
}
