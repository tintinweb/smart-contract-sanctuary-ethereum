...
contract Token is IToken, ERC1404, AgentRoleUpgradeable, TokenStorage {
    ...
    function init(
        ...
        tokenCompliance = Compliance(_compliance);
        maxSupply = _cap;
    ...
    /**
     *  @notice The specifications have changed and transfer eligibility checks have 
     *          since have been offloaded to the token compliance contract in favor of modularity 
     *          and compliance with both erc3643 and erc1404 standards. See Compliance.sol::detectTransferRestriction()
     *          and Compliance.sol::messageForTransferRestriction().
    ...
    function transfer(address _to, uint256 _amount) public override whenNotPaused returns (bool) {
        uint8 restrictionCode = tokenCompliance.detectTransferRestriction(msg.sender, _to, _amount);
        if (restrictionCode == tokenCompliance.SUCCESS_CODE()) {
            tokenCompliance.transferred(msg.sender, _to, _amount);
            _transfer(msg.sender, _to, _amount);
            return true;
        } else {
            revert(tokenCompliance.messageForTransferRestriction(restrictionCode));
        }
    }
     ...
     */
    function compliance() external view override returns (Compliance) {
    ...
    function transferFrom(
        ...
        uint8 restrictionCode = tokenCompliance.detectTransferRestriction(_from, _to, _amount);
        if (restrictionCode == tokenCompliance.SUCCESS_CODE()) {
    ...
    function detectTransferRestriction (address _from, address _to, uint256 _value) public override(ERC1404, IToken) view returns (uint8 restrictionCode) {
        restrictionCode = tokenCompliance.detectTransferRestriction(_from, _to, _value);
    }
    ...
    function messageForTransferRestriction (uint8 _restrictionCode) public override(ERC1404, IToken) view returns (string memory message) {
        message = tokenCompliance.messageForTransferRestriction(_restrictionCode);
    }
...
contract TokenStorage {
    ...
    Compliance internal tokenCompliance;
}
...
interface IToken is IERC20 {
     ...
     */
    function compliance() external view returns (Compliance);
...
abstract contract Compliance {
     ...
     */
    function isTokenAgent(address _agentAddress) external virtual view returns (bool);

    /**
     *  @dev Returns true if the address given corresponds to a token that is bound with the Compliance contract
     *  @param _token address of the token
     */
    function isTokenBound(address _token) external virtual view returns (bool);
     ...
     */
    function addTokenAgent(address _agentAddress) external virtual;
     ...
     */
    function removeTokenAgent(address _agentAddress) external virtual;
     ...
     *  emits an `OwnershipTransferred` event
     */
    function transferOwnershipOnComplianceContract(address newOwner) external virtual;

    function detectTransferRestriction(address _from, address _to, uint256 _value) external virtual view returns (uint8 restrictionCode);

    function messageForTransferRestriction(uint8 _restrictionCode) external virtual view returns (string memory message);
}
