...
contract HITODAMA is Ownable, ERC721A {
    ...
    function mint(uint256 _quantity) external payable {
        require(saleStatus, "NotStarted");
        /* require(msg.sender == tx.origin, "NotAllowed"); */
        require(totalSupply() + _quantity <= maxSupply, "MaxSupply");
        require(_numberMinted(msg.sender) + _quantity <= maxMintPerWallet,  "ExceedLimit");
    ...
    function getElapsedTimeFromLastPray(uint256 tokenId) public view returns (uint256) {
        require(_exists(tokenId), "no token");
        return block.timestamp - _lastPray[tokenId];
    }
