...
/**
 * @dev Each attribute is now its own NFT
 */
contract ERC721OwlAttribute is ERC721Owl {
    ...
    function initialize(
        ...
        revert('ERC721OwlAttribute: function disabled');
    }
    ...
    function __ERC721OwlAttribute_init(
        address _admin,
        string memory _name,
        string memory _symbol,
        string memory baseURI_,
        ...
        __ERC721OwlAttribute_init_unchained(_geneStart, _geneEnd, _transferable);
    }

    function __ERC721OwlAttribute_init_unchained(
        uint16 _geneStart,
        uint16 _geneEnd,
        bool _transferable
    ) internal onlyInitializing {
        ...
        geneStart = _geneStart;
        geneEnd = _geneEnd;
        transferable = _transferable;
    }

    /**
     * @dev this function will be called by {ERC721OwlAttribute}
    ...
    function safeMint(
        ...
        uint256 gene = _isolateGene(dna);
        genes[nextId.current()] = gene;

        nextId.increment();
    }

    function _isolateGene(uint256 dna) private view returns (uint256) {
        uint256 bitmask = get256Bitmask(geneStart, geneEnd);
    ...
    function getGene(uint256 tokenId) external view returns (uint256) {
        // tokenId = 0 will return null/default value
    ...
    function _beforeTokenTransfer(
        ...
        // If this is non-transferable, all transfers expect the mint will fail
        require(transferable || from == address(0), 'ERC721OwlAttribute: This attribute is non-transferable');
    }
}
