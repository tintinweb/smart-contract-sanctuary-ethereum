...
contract HomelessFriends is Ownable, ERC721A, ReentrancyGuard {
  ...
  uint256 public  PRICE = 0.0010 ether; //0.10 during public sale & 0.085 during pre sale
  ...
  mapping(address => bool) private whitelistedAddressesForPreSale;
...
function mint(uint256 quantity) external payable callerIsUser {
    ...
    //require(whitelistedAddressesForFreeMint[msg.sender], "You aren't White Listed For Free Mint");
    ...
    //require(whitelistedAddressesForPreSale[msg.sender], "You aren't White Listed For Pre Sale");
  ...
  function isWhitelistedForPreSale(address _user) public view returns (bool) {
    return whitelistedAddressesForPreSale[_user];
  }

  
  function addNewWhitelistUserForPreSale(address[] calldata _users) public onlyOwner {
    // ["","",""]
    for(uint i=0;i<_users.length;i++)
        whitelistedAddressesForPreSale[_users[i]] = true;
  }
