...
contract FlashLoanLiquidator is LiquidatorBase, IFlashLoanSimpleReceiver {
  ...
  function liquidateERC20(LiquidateERC20Params memory params)
    public
    onlyExecutor
  {
    address receiverAddress = address(this);

    address asset = params.debtAsset;

    uint256 amount = params.debtToCover;
    ...
    // 2. call callback
    // 3. repay money
    CONFIG._flashLoanPool().flashLoanSimple(
      receiverAddress,
      asset,
      amount,
      paramsBytes,
  ...
  function executeOperation(
    ...
    initiator;
    LiquidateERC20Params memory params = abi.decode(
      params_,
      (LiquidateERC20Params)
    );
    //        IERC20 assetFromToken;
    //
    //        assetFromToken = IERC20(_assetFrom);
    //
    //        uint256[] memory amountsIn = _uniswapV2Router.getAmountsIn(
    //            debtToCover, swapPath);
    //        assetFromToken.safeApprove(address(_uniswapV2Router), amountsIn[0]);
    //
    ...
    IERC20(params.debtAsset).safeApprove(
      address(CONFIG._pool()),
      params.debtToCover
    );

    CONFIG._pool().liquidationCall(
    ...
    IERC20(asset).approve(address(CONFIG._flashLoanPool()), amountOwing);
