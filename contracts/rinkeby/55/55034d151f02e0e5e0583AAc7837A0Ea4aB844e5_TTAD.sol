...
contract TTAD is ERC721, ERC721Enumerable, Ownable {
  ...
  uint256 public tierSupply = 2013;
  ...
  function getttByOwner(address _owner) public view returns (uint256[] memory) {
  ...
  function minttt(uint256 numtts) external payable {
    uint256 ts = totalSupply();
    require(_isSaleActive, 'Sale must be active to mint tts');
    require(ts + numtts <= tierSupply, 'Sale would exceed tier supply');
    require(ts + numtts <= MAX_SUPPLY, 'Sale would exceed max supply');
    require(balanceOf(msg.sender) + numtts <= maxBalance, 'Sale would exceed max balance');
    require(numtts <= maxMint, 'Sale would exceed max mint');
    require(numtts * mintPrice <= msg.value, 'Not enough ether sent');
    _minttt(numtts, msg.sender);
    emit TokenMinted(totalSupply());
  }

  function mintWhiteList(uint256 numtts) external payable {
    uint256 ts = totalSupply();
    require(isWhiteListActive, "White list is not active to mint tts");
    require(numtts <= _allowList[msg.sender], "Exceeded max available to purchase");
    require(ts + numtts <= (tierWhitelist+tierSupply), 'Sale would exceed Whitelist supply');
    require(ts + numtts <= MAX_SUPPLY, 'Sale would exceed max supply');
    require(balanceOf(msg.sender) + numtts <= maxBalance, 'Sale would exceed max balance');
    require(numtts <= maxMint, 'Sale would exceed max mint');
    _allowList[msg.sender] -= numtts;
    _minttt(numtts, msg.sender);
    emit TokenMinted(totalSupply());
  }

  function auctionMinttt(uint256 numtts) external payable {
    uint256 ts = totalSupply();
    require(_isAuctionActive, 'Auction must be active to mint tts');
    require(block.timestamp >= auctionStartTime, 'Auction not start');
    require(ts + numtts <= tierSupply, 'Auction would exceed tier supply');
    require(ts + numtts <= MAX_SUPPLY, 'Auction would exceed max supply');
    require(balanceOf(msg.sender) + numtts <= maxBalance, 'Auction would exceed max balance');
    require(numtts <= maxMint, 'Auction would exceed max mint');
    require(numtts * getAuctionPrice() <= msg.value, 'Not enough ether sent');
    _minttt(numtts, msg.sender);
    emit TokenMinted(totalSupply());
  }

  function _minttt(uint256 numtts, address recipient) internal {
    uint256 supply = totalSupply();
    for (uint256 i = 0; i < numtts; i++) {
