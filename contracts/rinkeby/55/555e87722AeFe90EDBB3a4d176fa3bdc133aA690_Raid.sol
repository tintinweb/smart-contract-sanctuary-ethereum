...
contract Raid is IRaid, Initializable, AccessControlUpgradeable, Seedable {
    ...
    function getPendingRewards(address user) external view returns (uint256) {
        UserSnapshot memory snapshot = _users[user];
        return _getRewards(snapshot.dpb, snapshot.lastRound, snapshot.lastBlock, _roundId);
    }
    ...
    function claimRewards(address user, uint256 maxRound) public hasStarted {
        ...
        uint256 rewards = _getRewards(snapshot.dpb, snapshot.lastRound, snapshot.lastBlock, maxRound);
    ...
    function _getRewards(uint256 dpb, uint256 lastRound, uint256 lastBlock, uint256 maxRound) internal view returns (uint256 rewards) {
        if (dpb > 0) {
            for (uint256 i = lastRound; i < maxRound; i++) {
                Round memory round = _rounds[i];
                Boss memory boss = _bosses[round.boss];

                if (i == lastRound) {
                    uint256 damage = dpb *
                        (round.lastBlock - lastBlock);
