 ...
 * ChlngrBrandNFT - ERC721 contract that whitelists an operator address, has create and mint functionality, and supports useful standards from OpenZeppelin,
  like _exists(), name(), symbol(), and totalSupply()
 */
contract ChlngrBrandNFT is ERC721A, Ownable {

  // max sizes for every generation
  uint16 constant gen0TokenMaxSize = 3333;
  uint16 constant gen1TokenMaxSize = 3334;
  ...
  mapping(uint256 => Attribute) private attribute; // attribute of chlngr brand nft
  ...
  uint256 private tokinPrice = TOKIN_PRICE;

  constructor(address _proxyRegistryAddress) ERC721A("Chlngr Brand", "CHLNGRBRAND") {
  ...
  */
  function getGenMap() public view returns (uint8[] memory) {
    uint256 len = totalSupply();
    uint8[] memory genList = new uint8[](len);
  ...
  */
  function updateFloor(uint256 _id, uint8 _floor, uint16 _passTime) public {
    require(msg.sender == ownerOf(_id), "You can unlock the floor for only your NFT.");
    ...
    require(attribute[_id].tokinDayStamp >= reqTokins, "Your tokins are not enough to unlock next level");
    attribute[_id].tokinDayStamp += reqTokins;
  }
  ...
  */
  function getTokinPrice() public view returns (uint256) {
    return tokinPrice;
  }
  ...
  */
  function updateTokinPrice(uint256 _price) public onlyOwner {
    tokinPrice = _price;
  }
  ...
  */
  function depositTokins(uint256 _id, uint16 _tokins) public {
    require(msg.sender == ownerOf(_id), "You can deposit tokins for only your NFT.");

    attribute[_id].tokinDayStamp -= _tokins;
  ...
  * @param _tokins the number of tokin
  */
  function claimTokins(uint256 _id, uint16 _tokins) public {
    require(msg.sender == ownerOf(_id), "You can claim tokins for only your NFT.");

    require(attribute[_id].tokinDayStamp >= _tokins, "Not enough tokins to claim");
  ...
  */
  function getAttribute(uint256 _id) public view returns (Attribute memory) {
    Attribute memory attr = Attribute(attribute[_id].tokinDayStamp, attribute[_id].currentFloor, attribute[_id].totalTime);
    return attr;
  }
  ...
  */
  ...
  function transferFrom(
    ...
    // reset tokin value
    uint16 tokinDayStamp = uint16(block.timestamp / (24 * 60 * 60));
    attribute[_id].tokinDayStamp = tokinDayStamp;
  }
