...
contract TheForge is Ownable, ReentrancyGuard, Pausable {
    ...
    function faithToStar(uint256 starQty) external nonReentrant nonContractCalls whenNotPaused {
        // console.log('---------------------- [%s]Calling faithToStar(%s) ----------------------', _msgSender(), starQty);
    ...
    function purchaseStar(uint256 starQty) external payable nonReentrant nonContractCalls whenNotPaused {
        require(__enablePurchaseStar, 'Sales not start');
    ...
    function enhanceSword(uint8 swordId, uint32 starsApplied) external nonReentrant nonContractCalls whenNotPaused {
        // console.log('!!! Enter enhanceSword(%s, %s)', swordId, starsApplied);
        ...
        // console.log('> Before: SwordId=%s, applied=%s', swordId, starsApplied);

        (uint32 maxRandNum, uint32 starsUsed) = __randContract.getMaxRandNumInRange(
            starsApplied, // loops
            __breakPoints[5], // MaxLimit == Range
            __legendaryCount < __legendaryCountLimit ? __breakPoints[4] : __breakPoints[3] // stop loop if larger than peak
        );

        // console.log('! maxRandNum=%s, starsUsed=%s', maxRandNum, starsUsed);
        ...
        // console.log('Reduce Star Balance! Used=%s, Balance=%s', starsUsed, __starBalances[_msgSender()]);
        // updateSeed();

        if (newSwordId <= swordId) {
            // console.log('SwordEnhanceFail! Address=%s', _msgSender());
            emit SwordEnhanceFail(_msgSender(), starsUsed, swordId);
            // console.log('> After: From: %s, To: %s, rounds=%s', swordId, newSwordId, starsUsed);
            return;
        }

        // console.log('SwordEnhanceSuccess! Address=%s', _msgSender());
        emit SwordEnhanceSuccess(_msgSender(), starsUsed, swordId, newSwordId);

        // console.log('Swap Swords! From: %s, To: %s', swordId, newSwordId);
    ...
    function tooglePurchaseStar() public onlyOwner {
        __enablePurchaseStar = !__enablePurchaseStar;
    }
...
interface IRandMath {
    ...
    function nextRand(uint256 prevRand) external view returns (uint256);
