...
contract CBCMigration is Ownable {
    ...
    constructor(
        ...
        onlyValidAddress(address(_newCBC))
        onlyValidAddress(address(_oldCBC))
        onlyValidAddress(_adminWallet)
        onlyValidAddress(_serverKey)
        onlyValidAddress(_burnWallet)
    {
        newCBC = _newCBC;
        oldCBC = _oldCBC;
    ...
    function migrate(
        ...
        require(
            ...
            "migrate: Caller cannot be zero address!"
        );
        ...
        require(
            ...
            "migrate: This transaction signature is expired"
        );
        ...
        oldCBC.safeTransferFrom(msg.sender, burnWallet, _amount);
        newCBC.safeTransferFrom(adminWallet, msg.sender, _amount);
     ...
     */
    function updateAdminWallet(address _newAdminWallet)
        external
        onlyOwner
        onlyValidAddress(_newAdminWallet)
    {
        adminWallet = _newAdminWallet;
    }

    /**
     * @dev update burn wallet which will be used for tokens burning
     * @param _newBurnWallet new burn wallet address to update
     */
    function updateBurnWallet(address _newBurnWallet)
        external
        onlyOwner
    ...
    /**
     * @dev update server key
     * @param _newServerKey new server key
     */
    function updateServerKey(address _newServerKey)
        external
        onlyOwner
        onlyValidAddress(_newServerKey)
    {
