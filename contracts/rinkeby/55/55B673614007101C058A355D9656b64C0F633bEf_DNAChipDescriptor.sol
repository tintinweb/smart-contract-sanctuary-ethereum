...
contract DNAChipDescriptor is Ownable {
    ...
    string[] public baseNames = ["Freak", "Robot", "Underworld", "Alien"];
    string[] public traitTypeNames = ["Character", "Earrings", "Eyes", "Hat", "Mouth", "Neck", "Nose", "Whisker"];
    // character => traitType => traits
    mapping(uint8 => mapping(uint8 => TraitData[10])) private _traitDataByCharacter;
                        ...
                        bytes(
                            string(
                                abi.encodePacked(
                                    name,
                                    '", "image": "data:image/svg+xml;base64,',
                                    AnonymiceLibrary.encode(bytes(getEvolutionPodSVG(traits))),
                                    '","attributes":',
                                    getEvolutionPodMetadata(traits),
                                    ', "description": "DNA Chips is a collection of 3,550 DNA Chips. All the metadata and images are generated and stored 100% on-chain. No IPFS, no API. Just the Ethereum blockchain."',
                                    "}"
                                )
                            )
    ...
    function getEvolutionPodImageTag(uint256 base) internal view returns (string memory) {
        if (base == 0) {
        ...
        if (base == 2) {
            return underworldChip.getEvolutionPodImageTag();
        }
        if (base == 3) {
    ...
    function getTraitsImageTags(uint8[8] memory traits) public view returns (string memory) {
        uint8 base = traits[0];
    ...
    function getEvolutionPodSVG(uint8[8] memory traits) public view returns (string memory) {
        ...
        preview = string(abi.encodePacked(preview, getTraitsImageTags(traits), "</g>"));
        ...
        result = string(abi.encodePacked(result, getEvolutionPodImageTag(base), preview, "</svg>"));
        return result;
    }
    ...
    function getBreedingSVG(uint8[8] memory traits) public view returns (string memory) {
        ...
        result = string(abi.encodePacked(result, getTraitsImageTags(traits), "</svg>"));
        return result;
    }
    ...
    function getMetadata(uint256 base, uint256 level) internal view returns (string memory) {
        ...
        metadataString = string(
            abi.encodePacked(metadataString, '{"trait_type": "Type","value":"', baseNames[base], '"}', ",")
        );
        metadataString = string(
    ...
    function getEvolutionPodMetadata(uint8[8] memory traits) public view returns (string memory) {
        string memory metadataString;
        uint8 base = traits[0];
        for (uint8 index = 0; index < traits.length - 1; index++) {
            ...
            metadataString = string(
                abi.encodePacked(
                    ...
                    '","value":"',
                    _traitDataByCharacter[base][index][traitValue].name,
                    '"}',
                    ","
                )
            );
        }
        console.log("metadataString", traitTypeNames[traits.length - 1]);
        metadataString = string(
            abi.encodePacked(
                metadataString,
                '{"trait_type": "',
                ...
                '","value":"',
                _traitDataByCharacter[base][uint8(traits.length - 1)][traits[traits.length - 1]].name,
                '"}'
            )
        );
