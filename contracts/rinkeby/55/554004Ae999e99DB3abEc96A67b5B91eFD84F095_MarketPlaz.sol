...
contract PlatDNA {
    ...
    // TODO: Calculate DNA
    // This pseudo randowm function is deteministic and should not be used in production.
    function deterministicPseudoRandomDNA(uint256 _tokenI, address _minter) public pure returns (uint256) {
        uint256 combinedParams = _tokenI + uint160(_minter);
    ...
    // This is a function private only avalible for this contract
...
contract MarketPlaz is ERC721, ERC721Enumerable, PlatDNA {
    ...
    constructor(uint256 _maxSupply) ERC721("PlatMark", "PLTMK") {
    ...
    function mint() public {
        ...
        require(current < maxSupply, "No Plat let");
    ...
    function _paramsURI(uint256 _dna) internal view returns (string memory) {
        ...
        // Este bloque lo separa porque al compilar con solidity aroja un error que el stack de solidity no puede manejar por ser demasiados datos.
        {
             params = string(
            
            abi.encodePacked(
            "accessoriesType=", getAccesoriesType(_dna),
                "&clotheColor=",
                getClotheColor(_dna),
                "&clotheType=",
                getClotheType(_dna),
    ...
    function tokenURI(uint256 tokenId) public view override returns(string memory) {
        ...
        require(_exists(tokenId),"ERC721 Metadata: URI query or nonexistent token");
        ...
        string memory jsonURI = Base64.encode(
            abi.encodePacked(
                '{ "name": "PlatMarket #', tokenId.toString(),
                '", "description": "Plat es una descripcion algo larga sobre este nft" , "image": "',
                image,
                '"}'
            )
        );
