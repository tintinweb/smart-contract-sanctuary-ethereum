...
contract JellyRewarder is IJellyRewarder, IJellyContract {
        ...
        external
    {
        ...
        rewardData.lastRewardTime = BoringMath.to48(_startTime);
        ...
        returns(bool)
    {
        ...
        if(block.timestamp <= startTime ) {
            rewardData.lastRewardTime = BoringMath.to48(block.timestamp);
            return false;
        }

        _updateWeights();
        // Not sure if rewards should be set
        if (_rewardData.totalPoolsWeight == 0 ) {
            rewardData.lastRewardTime = BoringMath.to48(block.timestamp);
            return false;
        }

        for (uint256 i = 0; i < poolAddresses.length; i++) {
            _updatePoolRewards(poolAddresses[i]);
        }
        ...
        returns(bool)
    {
        ...
        // uint256 weight = _pool.poolWeight;

        // /// @dev check that the rewards have started
        // if(_pool.poolWeight == 0  ) {
        //     return false;
        // }
    ...
    /// PW: initialisation with total pools weight = 0 initially
    ...
    function poolRewards(address _poolAddress, address _rewardToken, uint256 _from, uint256 _to) public override view returns (uint256 rewards) {
        ...
        // PW: this might not be right, Might need to store it for every period.
        // uint256 poolWeight = poolData[_poolAddress].poolWeight;
        uint256 totalWeights = rewardData.totalPoolsWeight;

        //  poolWeight == 0 || 
        if (_to <= startTime ||totalWeights == 0 ) { 
            return 0;
        }
        if (_from < startTime) {
            _from = startTime;
        ...
        if (_poolAddress == poolAddresses[0]) {
            ...
            uint256 initialRemander = startTime.add((fromPeriod+1).mul(periodSeconds)).sub(_from);
            ...
            for (uint256 i = fromPeriod+1; i < toPeriod; i++) {
                rewards = rewards.add(_rewardsFromWeight(periodRewardsPerSecond[i],
                                                            periodSeconds,periodWtPoints[fromPeriod].sweetWtPoints));
            }
            uint256 finalRemander = _to.sub(toPeriod.mul(periodSeconds).add(startTime));
            rewards = rewards.add(_rewardsFromWeight(periodRewardsPerSecond[toPeriod],
                                                        finalRemander,periodWtPoints[fromPeriod].sweetWtPoints));
            return rewards;
            ...
            for (uint256 i = fromPeriod+1; i < toPeriod; i++) {
                rewards = rewards.add(_rewardsFromWeight(periodRewardsPerSecond[i],
                                                            periodSeconds,periodWtPoints[fromPeriod].royalWtPoints));
            }
            uint256 finalRemander = _to.sub(toPeriod.mul(periodSeconds).add(startTime));
            rewards = rewards.add(_rewardsFromWeight(periodRewardsPerSecond[toPeriod],
                                                        finalRemander,periodWtPoints[fromPeriod].royalWtPoints));
            return rewards;
        } else {
            return 0;
        }
    ...
    // PW: The weights on the other pool should factor the changing weights of the either pool
    // PW: Look for weighted average done per period
    function _updateWeightsOld() internal {
        uint256 totalWeights = 0;
        for (uint256 i = 0; i < poolAddresses.length; i++) {
            uint256 staked = IJellyPool(poolAddresses[i]).stakedTokenTotal();
            uint256 weight = staked.mul(poolData[poolAddresses[i]].poolPoints);
            // poolData[poolAddresses[i]].poolWeight = BoringMath.to128(weight);
            // poolData[poolAddresses[i]].lastRewardTime = BoringMath.to48(block.timestamp);
            totalWeights += weight;
        }
        rewardData.totalPoolsWeight = BoringMath.to128(totalWeights);
    ...
    function _updateWeights() internal {
        ...
        uint256 sP = s.mul(poolData[poolAddresses[0]].poolPoints);

        uint256 r = 0;
        uint256 rP = 0;
        if (poolAddresses.length > 1) {
            uint256 r = IJellyPool(poolAddresses[1]).stakedTokenTotal();
            uint256 rP = s.mul(poolData[poolAddresses[1]].poolPoints);
        }

        uint256 totalWeights = rP + sP;

        if (totalWeights == 0 ) {
            _updateWeightingAcc(0, 0);
    ...
    function _updateWeightingAcc(uint256 sW, uint256 rW) internal {
        ...
        uint256 startCurrentPeriod = startTime.add(currentPeriod.mul(periodInSeconds())); 

        /// @dev Initialisation of new weightings and fill gaps
        if (periodWtPoints[0].sweetWtPoints == 0 
                && periodWtPoints[0].royalWtPoints == 0  ) {
            periodWtPoints[0] = Weights(BoringMath.to128(sW), BoringMath.to128(rW));
        ...
        periodWtPoints[currentPeriod] = Weights(
                    _calcWeightPoints(uint256(periodWtPoints[currentPeriod].sweetWtPoints),sW,startCurrentPeriod),
                    _calcWeightPoints(uint256(periodWtPoints[currentPeriod].royalWtPoints),rW,startCurrentPeriod));
    }
    ...
    function _calcWeightPoints(
        ...
        returns(uint128) 
    {
        uint256 previousWeighting = prevWeight.mul(uint256(rewardData.lastRewardTime).sub(startCurrentPeriod));
        uint256 currentWeighting = newWeight.mul(block.timestamp.sub(uint256(rewardData.lastRewardTime)));
        return BoringMath.to128(previousWeighting.add(currentWeighting)
                                .div(block.timestamp.sub(startCurrentPeriod)));
    }
