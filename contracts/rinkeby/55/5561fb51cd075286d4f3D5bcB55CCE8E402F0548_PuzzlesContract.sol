...
contract PuzzlesContract is ERC721, Ownable {
    string private _baseURIextended;
    string private _baseURInotRevealed;
    uint256 public constant MAX_SUPPLY = 100;
    uint256 public constant PRICE_PER_TOKEN = 0.0001 ether;
    uint256 public tokenId;

    bool revealed = false; // Not in use
    bool whitelistIsActive = false; // Not in use
    bool saleIsActive = false; // Not in use

    mapping(address => uint8) public _whiteList;

    uint256[] public _tokensMinted;

    // address => puzzle.id&puzzle.size => score
    mapping(address => mapping(string => uint256)) public scoresRequire;
    mapping(address => uint256) public scores;
    address[] public scoreWallets;

    // event scoreWallets(
    //     address indexed from,
    //     uint256 indexed puzzleId,
    //     uint8 indexed puzzleSize,
    //     uint256 score
    // );

    constructor() ERC721("Parts", "PRT") {
        tokenId = 1;
    }
    ...
    function getScoreWallets() public view returns (address[] memory) {
        return scoreWallets;
    }

    function mint(uint256 _tokenId) public payable {
        require(saleIsActive, "Minting is not active");
        _safeMint(msg.sender, _tokenId);
        _tokensMinted.push(_tokenId);
    }

    function setScore(
        address _user,
        string memory _puzzleIdSize,
        uint256 _score
    ) public {
        require(
            scoresRequire[_user][_puzzleIdSize] <= 1,
            "Puzzle already done"
        );
        if (scores[_user] == 0) {
            scoreWallets.push(_user);
        }
        scores[_user] += _score;
        scoresRequire[_user][_puzzleIdSize] = _score;
        //emit scoreWallets(_user, _puzzleId, _puzzleSize, _score);
    }
