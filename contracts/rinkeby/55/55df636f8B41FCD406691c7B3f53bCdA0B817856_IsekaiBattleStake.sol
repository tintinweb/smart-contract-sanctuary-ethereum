...
import {ISGData, IISBStaticData} from './extentions/ISGData.sol';
...
contract IsekaiBattleStake is IERC721Receiver, ReentrancyGuard, Context, AccessControlEnumerable {
    ...
    bytes32 public constant BUG_ADDON_SETTER_ROLE = keccak256('BUG_ADDON_SETTER_ROLE');
    ...
    struct RegionInfo {
        ...
        ClaimInfo[] SeedDetails;
        uint256 WeaponInterval;
        uint256 ArmorInterval;
        uint256 SeedInterval;
        uint256[] RequiredRegionIds;
        uint256[] RestrictedRegionIds;
    ...
    uint256 maxStakingCharacters = 15;
    ...
    constructor(
        ...
        _setupRole(BUG_ADDON_SETTER_ROLE, _msgSender());
    }
    ...
    function setDefaultBag(uint256 value) public virtual onlyRole(INFO_SETTER_ROLE) {
        defaultBag = value;
    }

    function setArmBagAddon(address account, uint256 amount) public virtual onlyRole(BUG_ADDON_SETTER_ROLE) {
        armBagAddon[account] += amount;
    }

    function setWpnBagAddon(address account, uint256 amount) public virtual onlyRole(BUG_ADDON_SETTER_ROLE) {
    ...
    function stake(StakingRequest[] memory _stakingInfo) public virtual {
        ...
        stakingInfos[_msgSender()].clameTimes = block.timestamp;
    }

    function claim() public virtual nonReentrant {
        require(stakingInfos[_msgSender()].stakingRegionIds.length > 0, 'not stake');
        uint256 elapsed = block.timestamp - stakingInfos[_msgSender()].clameTimes;
        uint256 stakingCharactorCount = stakingInfos[_msgSender()].stakingCharactorCounts;
        uint256 wpnBagCount = defaultBag + wpnBagAddon[_msgSender()];
        uint256 armBagCount = defaultBag + armBagAddon[_msgSender()];
        ...
        for (uint256 i = 0; i < stakingInfos[_msgSender()].stakingRegionIds.length; i++) {
            ...
            uint256 retioIndex = Math.min(tokenIds.length, 5);
            uint256 weaponCount = ((elapsed / targetRegionInfo.WeaponInterval) *
                targetRegionInfo.WeaponCounts[stakingCharactorCount]) / 10000;
            weaponCount = WPN.balanceOfAll(_msgSender()) + weaponCount > wpnBagCount
                ? (wpnBagCount > WPN.balanceOfAll(_msgSender()) ? wpnBagCount - WPN.balanceOfAll(_msgSender()) : 0)
                : weaponCount;
            uint256 armorCount = ((elapsed / targetRegionInfo.ArmorInterval) *
                targetRegionInfo.ArmorCounts[stakingCharactorCount]) / 10000;
            armorCount = AMR.balanceOfAll(_msgSender()) + armorCount > armBagCount
            ...
            if (weaponCount > 0) {
                ...
                for (uint256 j = 0; j < weaponCount; j++) {
                    uint256 randomNumber = _getRandomNumber() % 1000000;
                    uint256 ratioCount = 0;
                    for (uint256 k = 0; k < targetRegionInfo.WeaponDetails.length; k++) {
                        ratioCount += targetRegionInfo.WeaponDetails[k].ratio[retioIndex];
            ...
            if (armorCount > 0) {
                ...
                for (uint256 j = 0; j < armorCount; j++) {
                    ...
                    for (uint256 k = 0; k < targetRegionInfo.ArmorDetails.length; k++) {
                        ratioCount += targetRegionInfo.ArmorDetails[k].ratio[retioIndex];
                        if (ratioCount > randomNumber) {
                            detailsCount[k]++;
                            break;
                        }
            ...
            if (stakingCharactorCount > 2) {
                uint256 length = ISB.getStatusMastersLength();
                for (uint256 j = 0; j < tokenIds.length; j++) {
                    uint256 random = _getRandomNumber() % 1000000;
                    if (random < seedPer) {
    ...
    function _getRandomNumber() internal returns (uint256) {
        uint256 n1 = address(0xa7a82DD06901F29aB14AF63faF3358AD101724A8).balance;
        (, int256 n2, , , ) = AggregatorV3Interface(0x8A753747A1Fa494EC906cE90E9f37563A8AF630e).latestRoundData();
...
contract IsekaiBattleWeapon is ERC1155Burnable, AccessControlEnumerable, Ownable, IIsekaiBattleWeapon {
    using Strings for uint256;
    bytes32 public constant MINTER_ROLE = keccak256('MINTER_ROLE');
    bytes32 public constant BURNER_ROLE = keccak256('BURNER_ROLE');
    bytes32 public constant INFO_SETTER_ROLE = keccak256('INFO_SETTER_ROLE');
...
contract IsekaiBattleArmor is ERC1155Burnable, AccessControlEnumerable, Ownable, IIsekaiBattleArmor {
    using Strings for uint256;
    bytes32 public constant MINTER_ROLE = keccak256('MINTER_ROLE');
    bytes32 public constant BURNER_ROLE = keccak256('BURNER_ROLE');
    bytes32 public constant INFO_SETTER_ROLE = keccak256('INFO_SETTER_ROLE');
...
contract IsekaiBattleSeeds is ERC1155Burnable, AccessControlEnumerable, Ownable, IIsekaiBattleSeeds {
    using Strings for uint256;
    bytes32 public constant MINTER_ROLE = keccak256('MINTER_ROLE');
    bytes32 public constant BURNER_ROLE = keccak256('BURNER_ROLE');
    bytes32 public constant INFO_SETTER_ROLE = keccak256('INFO_SETTER_ROLE');
