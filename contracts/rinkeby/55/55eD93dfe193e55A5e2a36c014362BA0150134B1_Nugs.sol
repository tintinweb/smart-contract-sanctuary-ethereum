...
contract Nugs is ERC721A, ERC2981, Ownable {
  ...
  // If the mint is open
  bool public _squatting = true; // todo: make false
  ...
  modifier squatting() {
    if (_squatting == false) revert MintClosed();
    _;
  }
  ...
  */
  function claimPooPoos(address _partner) external squatting() noContracts() {
    uint256[] memory _rayRayIDs = walletOfRayRayOwner(msg.sender, _partner);
  ...
  */
  function mintYourPooPoo(uint256 _amount) external payable squatting() noContracts() ensureSupply(_amount) {
  ...
  */
  ...
  @notice Sets whether the contract is open for use
  */
  function setSquatting(bool _val) external onlyOwner {
    _squatting = _val;
  }
