...
contract SwapContract is Ownable {
    ...
    address public rtoken = 0x779595F212B53e4f1F51b0875405Bf21B5e4D805;
    address public genbetaaddress = 0x1e7A02f1595fB6D57E980562E3241E2a92017452;
    ...
    event UnSwap(address indexed user, uint256 amount);
    ...
    function changeGENBETATokenAddress(address _genbetaTokenAddress) public onlyOwner {
        genbetaaddress = _genbetaTokenAddress;
    }
    ...
    function changeRewardToken(uint256 _RewardTokenAmount) public onlyOwner {
    ...
    function swap(uint256[] memory tokenIds) public {
        for(uint256 i = 0; i < tokenIds.length; i++) {
            if(IERC721(genbetaaddress).ownerOf(tokenIds[i]) != msg.sender) continue;

            IERC721(genbetaaddress).transferFrom(address(msg.sender), address(this), tokenIds[i]);

            UserInfo memory info;
            info.tokenId = tokenIds[i];
            info.startTime= block.timestamp;
            info.lockedTime = block.timestamp + lockTime;

            userInfo[msg.sender].push(info);
            swapAmount[msg.sender] = swapAmount[msg.sender] + 1;
            emit Swap(msg.sender, 1);
        }
    }

    function getSwapItemInfo(address _user, uint256 _tokenId) public view returns(bool _isSwaped, uint _lockedTime) {
        for(uint256 i = 0; i < userInfo[_user].length; i++) {
            if(userInfo[_user][i].tokenId == _tokenId) {
                _isSwaped = true;
                _lockedTime = userInfo[_user][i].lockedTime;
                break;
            }
        }
    }
    ...
    function withdraw(uint256[] memory tokenIds) public {
        for(uint256 i = 0; i < tokenIds.length; i++) {
            (bool _isSwaped, uint _lockedTime) = getSwapItemInfo(msg.sender, tokenIds[i]);
            if(!_isSwaped) continue;
            if(_lockedTime > block.timestamp) continue;
            if(IERC721(genbetaaddress).ownerOf(tokenIds[i]) != address(this)) continue;
            IERC20(rtoken).transfer(msg.sender, RewardTokenAmount);
            removeFromUserInfo(tokenIds[i]);
            emit UnSwap(msg.sender, 1);
        }
    }
