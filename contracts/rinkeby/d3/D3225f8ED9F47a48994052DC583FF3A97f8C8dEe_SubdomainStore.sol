...
contract SubdomainStore is IERC721Receiver, Ownable, ApproveAndCallFallBack {
  ...
  function receiveApproval(address, uint256, address token, bytes memory data) public override {
    ...
    (label, subdomain, _subdomainOwner, resolver) = abi.decode(data, (bytes32, string, address, address));

    register(label, subdomain, _subdomainOwner, resolver);
  }

  function encodeHelper(bytes32 label, string calldata subdomain, address _subdomainOwner, address resolver) external pure returns (bytes memory data) {
    return abi.encode(label, subdomain, _subdomainOwner, resolver);
  }
