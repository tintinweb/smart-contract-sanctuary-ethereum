...
contract TESTTESTTEST is AdminControl, ICreatorExtensionTokenURI, ERC721 {
    ...
    mapping(address => uint256) public splittingValues;
    ...
    function mint(
        ...
        uint256 newItemId = IERC721CreatorCore(_creator).mintExtension(to);
        ...
        adminRequired
    {
        ...
        for (uint8 i = 0; i < splittingWallets.length; i++) {
            if (splittingWallets[i] == wallet) {
                splittingValues[splittingWallets[i]] = percent;
                istSet = true;
            }
        }
        if (!istSet) {
            splittingWallets.push(wallet);
            splittingValues[wallet] = percent;
        }
    }

    function payout() private adminRequired {
        require(splittingWallets.length > 0, "No splittingWallets are set");

        uint256 valueTotal = msg.value;
        for (uint8 i = 0; i < splittingWallets.length; i++) {
            address payable reviever = splittingWallets[i];
            uint256 valueTransfer = (msg.value / 100) *
                splittingValues[splittingWallets[i]];

            require(valueTotal >= valueTransfer, " valueTransfer false");
            valueTotal = valueTotal - valueTransfer;

            bool sent = reviever.send(valueTransfer);
    ...
    {
        require(_creator == from, 'Only creater can transfer');
