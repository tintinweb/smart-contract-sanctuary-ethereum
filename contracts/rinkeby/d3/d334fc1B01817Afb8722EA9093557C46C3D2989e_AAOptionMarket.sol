...
contract AAOptionMarket is AccessControl, IAAOptionMarket{
    ...
    function checkSign(AAOptionMsg memory optionMsg, uint256 optionId, TradeType tradeType) public view override {
        ...
        require(SignatureChecker.isValidSignatureNow(
                ...
                ECDSA.toEthSignedMessageHash(
                    keccak256(abi.encodePacked(
                        address(this),optionId,optionMsg.price,optionMsg.signAmount,tradeType,optionMsg.signTime,block.chainid
                    )
                    )
                ),
   ...
   */
        ...
        if(optionDetail.optionType){
            ...
            if(optionDetail.strikePrice < optionLockPool[optionId].spotPrice){
                ...
                if(lockBalance > 0){
                    ...
                    require(baseAsset.transferFrom(address(this), msg.sender, amount), "BaseTransferFailed");
                }
                // out/at the time
            }else{
                if(optionBalance > 0){
                ...
                if(lockBalance > 0){
                    optionProof.burn(msg.sender, optionId, lockBalance);
                    amount = lockBalance;
                    require(baseAsset.transferFrom(address(this), msg.sender, lockBalance), "BaseTransferFailed");
                }
            }
            ...
            if(optionDetail.strikePrice > optionLockPool[optionId].spotPrice){
                if(optionBalance > 0){
                    ...
                    optionToken.burn(msg.sender, optionId, optionBalance);
                    require(quoteAsset.transferFrom(address(this), msg.sender, profit), "QuoteTransferFailed");
                }
                if(lockBalance > 0){
                    amount = lockBalance - lockBalance * (optionDetail.strikePrice - optionLockPool[optionId].spotPrice) / optionDetail.strikePrice;
                    optionProof.burn(msg.sender, optionId, lockBalance);
                    require(quoteAsset.transferFrom(address(this), msg.sender, amount), "QuoteTransferFailed");
                }
                // out/at the time
            }else{
                if(optionBalance > 0){
                ...
                if(lockBalance > 0){
                    optionProof.burn(msg.sender, optionId, lockBalance);
                    amount = lockBalance;
                    require(quoteAsset.transferFrom(address(this), msg.sender, lockBalance), "QuoteTransferFailed");
                }
            }
        }
    ...
    function getPriceForBase() internal view returns(uint80 _roundId,uint256 price){
        ...
        _roundId = roundId;
        price = uint256(answer) / (10 ** priceFeed.decimals() * (10 ** ITokenDecimals(address(quoteAsset)).decimals()));
    }
...
interface IAAOption is IERC1155{
    ...
    event OptionCreated(uint256 expireTime, uint256 strikePrice, bool optionType, uint256 optionId);
