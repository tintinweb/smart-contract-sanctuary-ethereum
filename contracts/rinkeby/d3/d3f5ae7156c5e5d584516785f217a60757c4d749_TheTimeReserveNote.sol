...
abstract contract ERC721Enumerable is ERC721, IERC721Enumerable {
    ...
    contract TheTimeReserveNote is ERC721Enumerable, Ownable {

    using SafeMath for uint256;
    using Strings for uint256;


    uint256 public  PRICE_PER_NFT = 70000000000000000 wei;  //0.07 ETH

    uint256 public  PRICE_PER_Burn = 220000000000000000 wei;  //0.22 ETH

    uint256 public constant Max_Supply = 10000;

    uint256 public constant Max_Reserved = 10;

    uint256 public nftsMintedReserve;
    ...
    string public _metaBaseUri = "";



    constructor(string memory MetaUri) ERC721("TheTimeReserveNote", "TTRN") {

        _saleIsActive = true;
         _metaBaseUri = MetaUri;
    }
      
     //Enable/Disable Whitelist
       function setWhiteListEnable(bool active) external onlyOwner {
        whitelistEnable = active;
    }

    // Mint NFTs for White List
    function mintNft(uint16 numberOfTokens) public payable {
        if(whitelistEnable){
             require(_whiteListedMembers[msg.sender] == MemberClaimStatus.Listed, "You are not a whitelisted member !");
        }
        require(saleIsActive(), "Sale is not active");
        require(PRICE_PER_NFT.mul(numberOfTokens) <= msg.value, "Ether value sent is incorrect.Need 0.07 ETH for each NFT");
        require(totalSupply() < Max_Supply, "collection sold out");
        require(totalSupply().add(numberOfTokens) < Max_Supply, "Insufficient supply");
        _mintTokens(numberOfTokens,msg.sender);
        
    }

      // Mint NFTs Without White List
    function mintNftPublic(uint16 numberOfTokens) public payable {
        require(saleIsActive(), "Sale is not active");
        require(PRICE_PER_NFT.mul(numberOfTokens) <= msg.value, "Ether value sent is incorrect.Need 0.07 ETH for each NFT");
        require(totalSupply() < Max_Supply, "collection sold out");
        require(totalSupply().add(numberOfTokens) < Max_Supply, "Insufficient supply");
     ...
     function reserveMint(uint16 numberOfTokens) external onlyOwner {
        require(nftsMintedReserve.add(numberOfTokens) <= Max_Reserved, "Cannot mint more nfts");
        require(totalSupply().add(numberOfTokens) < Max_Supply, "Insufficient supply");
        nftsMintedReserve += numberOfTokens;
        _mintTokens(numberOfTokens,msg.sender);
    }


    function burnNFT(uint256 tokenId) public payable{
        require(PRICE_PER_Burn <= msg.value, "Ether value sent is incorrect.Need 0.22 ETH");
         require(ownerOf(tokenId) == msg.sender, "NOT OWNER");
        _burn(tokenId);   
    }
    
    function burnMultipleNFT(uint256[] memory tokenIds) public{
