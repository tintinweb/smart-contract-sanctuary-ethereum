...
contract CyberWorldTest is ERC721, ChainlinkClient, ConfirmedOwner {
  ...
  constructor() ERC721("Harlans Test Rinkeby Token", "HRL") ConfirmedOwner(msg.sender) {
  ...
  function requestMetadata(string memory _polygonString, string memory _imageCID) public payable returns (bytes32 requestId) {
    Chainlink.Request memory metadataRequest = buildChainlinkRequest('7da2702f37fd48e5b1b9a5715e3509b6', address(this), this.fulfillMint.selector);
    metadataRequest.add('get', string.concat("https://cyberworld.earth/php/addLandMetadata.php?token-id=", Strings.toString(currentTokenId.current()), "&owner-address=", Strings.toHexString(uint160(msg.sender), 20), "&polygon-string=", _polygonString, "&polygon-price=", Strings.toString(msg.value / (10 ** 18)), "&image-cid=", _imageCID));
  ...
  function updateLandOwner(uint256 _tokenId) public returns (bytes32 requestId) {
    Chainlink.Request memory transferOwnershipRequest = buildChainlinkRequest('7d80a6386ef543a3abb52817f6707e3b', address(this), this.fulfillOwnerUpdate.selector);
    transferOwnershipRequest.add('get', string.concat("https://cyberworld.earth/php/updateLandOwner.php?token-id=", Strings.toString(_tokenId)));
