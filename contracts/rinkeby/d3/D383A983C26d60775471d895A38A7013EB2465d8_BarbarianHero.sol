...
abstract contract HeroBase is NftBase, IHero {
  ...
  function __HeroBase_init(
    ...
    require(heroClass_ != 0, "HB: Zero hero class");
  ...
  function onlyDungeon() internal view {
    require(IController(_controller()).validDungeons(msg.sender), "H: Not dungeon");
  }

  function onlyApprovedOrOwnerOrDungeon(uint tokenId) internal view {
    require(
      _isApprovedOrOwner(msg.sender, tokenId)
      || IController(_controller()).validDungeons(msg.sender),
      "H: Not dungeon or owner");
  }

  function onlyApprovedOrOwner(uint tokenId) internal view {
    require(_isApprovedOrOwner(msg.sender, tokenId), "H: Not owner");
  }
  ...
  function kill(uint tokenId) external override {
    onlyApprovedOrOwnerOrDungeon(tokenId);
    ...
    level : 0,
    experience : 0,
    life : _statController().heroStats(address(this), tokenId).life,
    mana : 0
    }),
      false
    );
    ...
    TokenTreasury memory tokenTreasury = _tokenTreasures[tokenId];
