...
contract CBWCPieces is CBWCBase {
    ...
    event CBWCWatchChanged(address indexed sender, address previousCBWCWatch, address cbwcWatch);
    ...
    event CBWCStakingChanged(address indexed sender, address previousCBWCStaking, address cbwcStaking);
    ...
    function initialize(address cbwcWatch_, address cbwcStaking_)
        external
        initializer //initializerERC721A
    {
        __CBWCBase_init(
            'CBWCPieces',
            'CBWCP',
    ...
    function airdrop(address[] calldata receivers_, uint256[] calldata quantities_) external onlyRole(MINTER_ROLE) {
        require(receivers_.length == quantities_.length, 'CBWCP: receivers and quantities length does not match');
        for (uint256 i = 0; i < receivers_.length; i++) {
            mint(receivers_[i], quantities_[i]);
        }
    }
    ...
    function claim(uint256[] calldata cbwcTokenIds_) external {
        ...
        mint(msg.sender, unclaimedPieces);
    }
    ...
    function setWave(uint256 wave_, bool active_) external onlyRole(DEFAULT_ADMIN_ROLE) {
    ...
    function startNextWave(bool endCurrentWave_) external onlyRole(WAVE_MANAGER) {
    ...
    function getPendingClaims(uint256[] calldata cbwcTokenIds_) external view returns (uint256) {
        uint256 unclaimedPieces = 0;
        for (uint256 i = 0; i < activeWaves.length; i++) {
    ...
    /// @return bool success
    function burnPieces(uint256[] calldata tokenIds_, address owner_) external onlyRole(BURNER_ROLE) returns (bool) {
...
abstract contract ERC721KFNCUUPSUpgradeable is IERC721KFNC, UUPSUpgradeable {
    ...
    function totalSupply() external view returns (uint256) {
        return _currentTokenId > _startingTokenId ? _currentTokenId - _startingTokenId : 0;
    }
    ...
    function safeTransferFrom(
        ...
        if (_to.code.length > 0) {
            _checkContractOnERC721Received(_from, _to, _tokenId, data);
        }
    }
