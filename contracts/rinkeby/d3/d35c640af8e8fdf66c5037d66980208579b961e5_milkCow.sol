...
contract milkCow is Context, Ownable {
    ...
    function setWhiteList(address[] memory users, bool val) public onlyOwner {
        ...
        uint iniCow = 0;
        uint StartCountingTime = 0;
        if (val) {
            iniCow = 1;
            StartCountingTime = 1651352400; // Assume this is the launch time (Not decided yet).
        }
        for ( uint i=0; i < users.length; i++) {
            whiteList[users[i]] = val;
            hatcheryCows[users[i]] = iniCow;
            lastHatch[users[i]] = StartCountingTime;  // Assume this is the launch time (Not decided yet).
        }
    }
...
//////////////////////////////////// view functions /////////////////////////////////
    ...
    function getMilkSinceLastHatch(address adr) public view returns(uint256) {
        uint256 lastHatchTime = min(block.timestamp, lastHatch[adr]);  // Prevent whitelist mining before launch
        uint256 secondsPassed=min(MILK_TO_HATCH_1COW,SafeMath.sub(block.timestamp,lastHatchTime));
        return SafeMath.mul(secondsPassed,hatcheryCows[adr]);
    }

    function checkLastHatch(address adr) public view returns(uint256) {
