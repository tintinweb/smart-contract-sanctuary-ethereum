...
import "./DutchAuctionWithRefund.sol";

/**
 * ERC721 Collection Drop Contract - Dutch Auction Pricing
 */
contract ERC721DutchAuction is ERC721CollectionBase, ERC721, DutchAuctionWithRefund, AdminControl {

    constructor(address signingAddress_) ERC721("DA Test", "DATEST") {
        _initialize(
            5000, // purchaseMax,
            10000000000000000, // purchasePrice, 
            0, 
            0, 
            5000000000000000, 
            0, 
        ...
        priceDropMagnitude = 50000000000000;
    }
     ...
     */
    function purchase(uint16 amount, bytes32 message, bytes calldata signature, string calldata nonce) public override payable nonReentrant {
        ERC721CollectionBase.purchase(amount, message, signature, nonce);
        uint currentPrice = getCurrentPrice(presalePurchasePrice, purchasePrice);
        uint256 totalPrice = amount * currentPrice;
        if (msg.value > totalPrice) {
            Address.sendValue(payable(msg.sender), msg.value - totalPrice);
        }
        recordPurchase(currentPrice, amount);
    }
...
/**
 * Dutch auction library - purchase price descends to 0 over the duration of the sale at a set rate.
 * After purchase period, buyers can claim a refund 
 */
abstract contract DutchAuctionWithRefund is DutchAuction, ReentrancyGuard {
    ...
    mapping(address => Purchase[]) private _purchaseHistory;

    uint256 public lastPurchasePrice;

    /**
     * @dev Record purchase for refund eligiblity after sale.
     * @param currentPrice The current price of a single token.
     * @param amount The number of tokens purchased.
     */
    function recordPurchase(uint256 currentPrice, uint16 amount) internal {
        _purchaseHistory[msg.sender].push(
            Purchase(currentPrice, amount)
        );
        lastPurchasePrice = currentPrice;
    }

    function claimRefund() public nonReentrant {
        require(block.timestamp > endTime, "Purchase period must be over to claim refund");

        Purchase[] storage purchases = _purchaseHistory[msg.sender];
        require(purchases.length > 0, "Address is not eligible for refund");

        uint256 totalRefund;

        for (
            uint256 i = purchases.length;
            i > 0;
            i--
        ) {
            Purchase storage purchase = purchases[i-1];
            uint256 actualPrice = purchase.amount * lastPurchasePrice;
            uint256 refund = purchase.price * purchase.amount - actualPrice;
            totalRefund += refund;
            purchases.pop();
        }

        require(totalRefund > 0, "Address is not eligible for refund");
        Address.sendValue(payable(msg.sender), totalRefund);
    }

    function purchaseHistory(address user) external view returns (Purchase[] memory) {
        return _purchaseHistory[user];
    }
}
...
abstract contract ERC721CollectionBase is CollectionBase, IERC721Collection {
    ...
    function purchase(uint16 amount, bytes32 message, bytes calldata signature, string calldata nonce) public payable virtual override {
        ...
        if (isPresale) {
            require((presalePurchaseLimit == 0 || useDynamicPresalePurchaseLimit || amount <= (presalePurchaseLimit-_addressMintCount[msg.sender])) && (purchaseLimit == 0 || amount <= (purchaseLimit-_addressMintCount[msg.sender])), "Too many requested");
...
abstract contract DutchAuction is CollectionBase {
    ...
    /**
     * @dev returns pricing data used to computer current price
     */
    ...
    function getCurrentPrice(uint256 presalePurchasePrice, uint256 startPrice) internal view returns (uint256) {
        ...
        uint256 currentPrice = startPrice - (timeElapsed / priceDropInterval) * priceDropMagnitude;
