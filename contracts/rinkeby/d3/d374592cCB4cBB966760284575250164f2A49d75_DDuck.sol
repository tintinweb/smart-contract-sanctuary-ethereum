...
contract DDuck is ERC721AQueryable, Ownable {
    ...
    function mint(uint256 quantity) external payable nonContractCaller {
        ...
        uint256 previewTotalMintCount = mintCount() + quantity;
        require(previewTotalMintCount <= MAX_SUPPLY, "Mint amount exceed max supply");

        uint256 refunds;
        if (previewTotalMintCount > FREE_MINTS) {
            uint256 payedCount = previewTotalMintCount - FREE_MINTS;
            if (payedCount > quantity) payedCount = quantity;
            else require(quantity - payedCount + balanceOf(msg.sender) <= MAX_FREE_MINT_PER_ACCOUNT, "Exceed free mint amount per account");
            uint256 payedPrice = payedCount * mintPrice;
