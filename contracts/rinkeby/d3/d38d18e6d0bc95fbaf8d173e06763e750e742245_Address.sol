...
contract FloorArmy is ERC721A, Ownable {
    ...
    uint private mintCost_ = 0.01 ether;
    ...
    */
    ...
    function presaleMint(uint256 number, uint8 _v, bytes32 _r, bytes32 _s) onlyValidAccess(_v,  _r, _s) public payable {
        ...
        require(mintsPerAddress[msg.sender] + number <= maxMint, "This is more than allowed mints per address!");
    ...
    function setMaxTotalTokens(uint number) public onlyOwner {
        require(numberOfTotalTokens <= number, " must be larger or equal than numberOfTotalTokens");
        maxTotalTokens = number;
    }

    //set new mint cost
    function setMintCost(uint number) public onlyOwner {
        require(0 <= number, "mintCost must be larger than 0");
        mintCost_ = number;
    }

    //set new mint maxMint
    function setMaxMint(uint number) public onlyOwner {
        require(0 <= number, "maxMint must be larger than 0");
        maxMint = number;
    }

    //set new mint maxReservedMints
    function setMaxReservedMints(uint number) public onlyOwner {
        require(_reservedMints <= number, "must be larger than already reserved");
