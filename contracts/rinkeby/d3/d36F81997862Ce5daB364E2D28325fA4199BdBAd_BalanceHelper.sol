...
contract BalanceHelper is Ownable, AccessControl {
    ...
    function rebalance_removeUsdb() external {
        ...
        require(totalBalances[0].div(totalBalances[1]) > uint(1000).div(triggerRatio), "NOT_ENOUGH_RATIO");
    ...
    function rebalance_removeDai() external {
        require(hasRole(MANAGER_ROLE, msg.sender), "MISSING_MANAGER_ROLE");
        ...
        require(totalBalances[1].div(totalBalances[0]) > uint(1000).div(triggerRatio), "NOT_ENOUGH_RATIO");
    ...
    function rebalance_addUsdb() external {
        require(hasRole(MANAGER_ROLE, msg.sender), "MISSING_MANAGER_ROLE");
    ...
    function rebalance_addDai() external {
        require(hasRole(MANAGER_ROLE, msg.sender), "MISSING_MANAGER_ROLE");
    ...
    function rebalance_sellDai(uint _daiAmountWithoutDecimals) external {
        require(hasRole(MANAGER_ROLE, msg.sender), "MISSING_MANAGER_ROLE");

        (IERC20[] memory tokens, uint[] memory totalBalances) = getPoolTokens();
        require(address(tokens[0]) == USDB && address(tokens[1]) == DAI, "WRONG_TOKEN_ORDER");

        uint toAdd = _daiAmountWithoutDecimals.mul(1e18);
        uint futureDaiAmount = totalBalances[1].add(toAdd);

        // dai / usdb should be greater than 1 there should be more usdb
        require(totalBalances[1].div(totalBalances[0]) > 1, "NOT_ENOUGH_RATIO");

        // manage LP token
        treasury.manage(DAI, toAdd);

        uint usdbOut = swapAmountOut(DAI, USDB, toAdd, IStablePool(lpToken).getPoolId());
        // deposit into treasury without generating any profit
    ...
    function rebalance_sellUsdb(uint _usdbAmountWithoutDecimals) external {
        require(hasRole(MANAGER_ROLE, msg.sender), "MISSING_MANAGER_ROLE");

        (IERC20[] memory tokens, uint[] memory totalBalances) = getPoolTokens();
        require(address(tokens[0]) == USDB && address(tokens[1]) == DAI, "WRONG_TOKEN_ORDER");

        uint toAdd = _usdbAmountWithoutDecimals.mul(1e18);
        uint futureUsdbAmount = totalBalances[0].add(toAdd);

        // usdb / dai should be greater than 1 there should be more dai
        require(totalBalances[0].div(totalBalances[1]) > 1, "NOT_ENOUGH_RATIO");

        // manage LP token
        treasury.manage(USDB, toAdd);

        uint daiOut = swapAmountOut(USDB, DAI, toAdd, IStablePool(lpToken).getPoolId());
        // deposit into treasury without generating any profit
    ...
    * @dev This helper function is a fast and cheap way to convert between IERC20[] and IAsset[] types
    ...
    function swapAmountOut(
        ...
        return balancerVault.swap(singleSwap, funds, type(uint).max, block.timestamp);
    }
