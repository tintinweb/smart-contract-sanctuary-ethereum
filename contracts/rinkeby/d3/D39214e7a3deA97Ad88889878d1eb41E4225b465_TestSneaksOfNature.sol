...
contract SneaksOfNature is ERC721AQueryable, ERC2981, Pausable, Ownable, ReentrancyGuard, MintAllowance {
  ...
  function closedMint(address receiver, uint16 tokenAmount) external payable whenNotPaused nonReentrant {
    ...
    // Get the mint allowance
    MintAllowanceData[] memory mintAllowance = getMintAllowance(receiver, false, false);

    // Check if the allowance allows the required amount of tokens to be minted
    require(tokenAmount <= mintAllowance.length, "Cannot mint the required number of tokens");

    // Check if all tokens can already be minted and accumulate the total required
    uint256 requiredEther = 0;
    for (uint256 i = 0; i < tokenAmount; i++) {
      MintAllowanceData memory mint = mintAllowance[i];

      require(
        !mint.isPublicMintOnly || block.timestamp >= closedMintEndTimestamp,
        "Cannot mint the required number of tokens before the public mint starts"
      );

      requiredEther += mint.price;
    }
  ...
  function getMintAllowance(
  ...
  ) public view returns (MintAllowanceData[] memory mints) {
    // Get the Relic Pass IDs the wallet owns
    uint16[] memory relicPassIds = IMetarelics(RELIC_PASS_CONTRACT_ADDRESS).walletOfOwner(receiver);

    // Retrieve how many tokens were minted from each Relic Pass
    uint8[] memory relicPassUsageOfReceiver = new uint8[](relicPassIds.length);
    for (uint16 i = 0; i < relicPassIds.length; ++i) {
      relicPassUsageOfReceiver[i] = relicPassTokenUsage[relicPassIds[i]];
    }
    ...
    return
      _mintAllowance(
        receiver,
        relicPassIds,
        relicPassUsageOfReceiver,
        isSneaksList,
        isGuaranteedAllowlist,
        discountedSneaksListTokens[receiver],
        isSneaksListUsed[receiver],
        isGuaranteedAllowlistUsed[receiver]
      );
  }

  // TODO: Remove this workaround for having too many local variable (Stack too deep error).
  function _mintAllowance(
    address receiver,
    uint16[] memory relicPassIds,
    uint8[] memory relicPassUsageOfReceiver,
    bool isSneaksList,
    bool isGuaranteedAllowlist,
    uint8 discountedSneaksListTokensLeft,
    bool isSneaksListUsedByReceiver,
    bool isGuaranteedAllowlistUsedByReceiver
  ) internal view returns (MintAllowanceData[] memory mints) {
    return
      mintAllowance(
        uint16(balanceOf(receiver)),
        maxTokensPerUser,
        relicPassIds,
        isSneaksList,
        isGuaranteedAllowlist,
        relicPassUsageOfReceiver,
        discountedSneaksListTokensLeft,
        isSneaksListUsedByReceiver,
        isGuaranteedAllowlistUsedByReceiver
      );
  }
...
abstract contract MintAllowance {
   ...
   * 2. The user is on the Discounted Sneaks List - they may mint one or more tokens at 0.33 ether in addition to the tokens from the Relic Pass tokens.
   * 3. The user is on the Sneaks List or the Guaranteed Allowlist - they may mint one token at 0.4 ether, but only if they don't already have any Relic Pass tokens and are not on the Discounted Sneaks List.
   * 4. The user is on the public waitlist - they may mint one token at 0.45 ether (this restriction may be increased by the contract admin)
   * @dev Get information about the tokens the user is allowed to mint and their prices
   * @param currentBalance Number of Sneaks tokens the user already owns
   * @param maxTokensPerUser The maximum number of tokens the user is allowed to mint during the public mint (does not apply for the closed mint)
   ...
   * @param discountedSneaksListTokensLeft Number of tokens the user can still mint from the Discounded Sneaks List
  ...
  function mintAllowance(
    uint16 currentBalance,
    uint16 maxTokensPerUser,
    uint16[] memory relicPassIds,
    bool isSneaksList,
    bool isGuaranteedAllowlist,
    uint8[] memory relicPassesUsage,
    uint8 discountedSneaksListTokensLeft,
    bool isSneaksListUsed,
    bool isGuaranteedAllowlistUsed
  ) internal pure returns (MintAllowanceData[] memory mints) {
    uint256 relicPassCount = relicPassIds.length;

    // Allocate enough memory to fit all tokens
    MintAllowanceData[] memory tempMints = new MintAllowanceData[](
      relicPassCount * 3 + discountedSneaksListTokensLeft + 1 + maxTokensPerUser
    );
    uint256 mintIndex = 0;

    // Add the unused free tokens from the Relic Pass tokens
    for (uint256 i = 0; i < relicPassCount; ++i) {
      uint16 relicPassId = relicPassIds[i];

      if (relicPassesUsage[i] == 0) {
        tempMints[mintIndex++] = MintAllowanceData(uint8(MintAllowanceType.RELIC_PASS), 0, relicPassId, false);
      }
    }

    // Add the unused tokens from the Discounted Sneaks List
    for (uint256 i = 0; i < discountedSneaksListTokensLeft; ++i) {
      tempMints[mintIndex++] = MintAllowanceData(
        uint8(MintAllowanceType.DISCOUNTED_SNEAKS_LIST),
        DISCOUNTED_SNEAKS_LIST_PRICE(),
        0,
        false
      );
    }

    // Add the unused reduced price mints from the Relic Pass tokens
    for (uint256 i = 0; i < relicPassCount; ++i) {
      uint16 relicPassId = relicPassIds[i];

      if (relicPassesUsage[i] <= 1) {
        tempMints[mintIndex++] = MintAllowanceData(
          uint8(MintAllowanceType.RELIC_PASS),
          RELIC_PASS_REDUCED_PRICE(),
          relicPassId,
          false
        );
      }
    ...
    // Add the unused full price mints from the Relic Pass tokens
    for (uint256 i = 0; i < relicPassCount; ++i) {
      uint16 relicPassId = relicPassIds[i];

      if (relicPassesUsage[i] <= 2) {
        tempMints[mintIndex++] = MintAllowanceData(
          uint8(MintAllowanceType.RELIC_PASS),
          FULL_PRICE(),
          relicPassId,
    ...
    // Add unused tokens from the Sneaks List or Guaranteed Allowlist
    if (relicPassCount == 0 && discountedSneaksListTokensLeft == 0 && currentBalance == 0) {
      if (isSneaksList) {
        if (!isSneaksListUsed) {
          tempMints[mintIndex++] = MintAllowanceData(
            uint8(MintAllowanceType.SNEAKS_LIST),
            SNEAKS_LIST_PRICE(),
            0,
            false
          );
        ...
        if (!isGuaranteedAllowlistUsed) {
          tempMints[mintIndex++] = MintAllowanceData(
            uint8(MintAllowanceType.GUARANTEED_ALLOWLIST),
            FULL_PRICE(),
            0,
            false
          );
    ...
    // Add unused tokens from the public mint
    if (relicPassCount == 0 && discountedSneaksListTokensLeft == 0 && !isGuaranteedAllowlist && !isSneaksList) {
      for (uint16 i = 0; i < maxTokensPerUser - currentBalance; ++i) {
        tempMints[mintIndex++] = MintAllowanceData(uint8(MintAllowanceType.PUBLIC_MINT), FULL_PRICE(), 0, true);
      }
    }

    // Copy only the filled elements to the result
    mints = new MintAllowanceData[](mintIndex);
    for (uint16 i = 0; i < mintIndex; ++i) {
      mints[i] = tempMints[i];
    }
  }
}
