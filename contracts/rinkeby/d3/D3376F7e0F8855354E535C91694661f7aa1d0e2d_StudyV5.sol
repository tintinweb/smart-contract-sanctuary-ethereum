...
contract StudyV5 is ERC721Enumerable,ERC2981,Ownable,Pausable,ReentrancyGuard{
  ...
  constructor() ERC721("StudyV5", "SV5") {
  ...
  function checkStakePoint(address _address) public view returns (uint256) {
      ...
      for (uint256 i; i < at_Contract.walletOfOwner(_address).length; i++) {
        if (stampOwnerMap[i] == _address){
          point=point+(timeStampMap[i]-block.timestamp);
        }
      }
    return point;
  }
