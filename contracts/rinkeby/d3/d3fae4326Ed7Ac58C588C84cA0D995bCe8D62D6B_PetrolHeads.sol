...
contract PetrolHeads is Ownable, Pausable, RandomlyAssigned, ERC721Enumerable {
	...
	bytes32 public claimCodesMerkleRoot = "";
	mapping(string => bool) public whitelistClaimed;
    ...
    /// @param claimCode - the 6 digits claim code won
	/// @param _merkleProof - the merkleProof set to verify claim code
    function giveawayByClaimCode(string memory claimCode, bytes32[] calldata _merkleProof) external payable returns (bool){
        require(
            PRICE <= msg.value,
            "ETH amount is not sufficient"
        );		
		bytes32 leaf = keccak256(abi.encodePacked(claimCode));
		bool verified = MerkleProof.verify(_merkleProof, claimCodesMerkleRoot, leaf);
		if (!verified) {	
		...
		require(
			!whitelistClaimed[claimCode],
			"Code has been already claimed."
		);		
        require(
            _purchaseCounter.current() + 1 <=
                MAX_TOKENS - GIVEAWAY_TOKENS - TEAM_RESERVE_TOKENS,
            "Maximum purchase amount has been reached"
        );
		whitelistClaimed[claimCode] = true;
        _giveawayCounter.increment();
        _mintToken(msg.sender);		
		return true;
    }	
    ...
    /// @notice only owner can set merkle root for claim codes
    /// @param merkleRoot - hash of merkle root for claim codes
    function setClaimCodesMerkleRoot(bytes32 merkleRoot) external onlyOwner {
        claimCodesMerkleRoot = merkleRoot;
    }	
...
abstract contract WithLimitedSupply {
    ...
    /// @dev Changed from totalSupply to maximumSupply because it clashes with IERC721Enumerable
