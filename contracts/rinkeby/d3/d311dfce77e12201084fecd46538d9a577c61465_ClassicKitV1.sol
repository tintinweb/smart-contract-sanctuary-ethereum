...
import "./interfaces/IClassicKit.sol";
import "./interfaces/IBaseToken.sol";

contract ClassicKitV1 is Initializable, OwnableUpgradeable, IClassicKit {
    ...
    uint96 private constant _rate = 0; // parts per 10,000
    EnumerableSetUpgradeable.AddressSet _collections;
    EnumerableSetUpgradeable.AddressSet _tokens;
    ...
    mapping(address => IClassicKit.Entry) private _rateOverride;
    ...
    function initialize(address token, address collection, address dropCollection, address dropCollectionA) public initializer {
        __Ownable_init();
        _treasury = _msgSender();
        _implementations[11] = token;
        _implementations[1] = collection;
        _implementations[2] = dropCollection;
        _implementations[3] = dropCollectionA;
    }
    ...
    function createToken(
        ...
        require(implementation != address(0), "Invalid implementation");
        require(
            implementation.supportsInterface(type(IBaseToken).interfaceId),
            "Not supported"
        );

        address deployed = _createToken(
            implementation,
            name,
            symbol,
            amount
        );
        _tokens.add(deployed);
        emit TokenCreated(typeId, deployed, amount);
    }
    ...
    function _createToken(
        ...
        address deployed = ClonesUpgradeable.clone(implementation);
        IBaseToken collection = IBaseToken(deployed);
        collection.initialize(name, symbol, amount);
        return deployed;
    }
}
...
interface IClassicKit {
    ...
    */
    event TokenCreated(uint96 indexed typeId, address indexed tokenAddress, uint256 amount);
...
interface IBaseToken {
    ...
    function initialize(
        string memory name_,
        string memory symbol_, uint256 amount_
    ) external;
}
