...
contract DigidorablesC1 is ERC721A, Ownable, ReentrancyGuard {
  ...
  constructor(
    ...
    boxelContract = IERC721(boxelContract_); //0xfF58403B9b011659f45d12744a0bE5F01c9FB607
  ...
  //redeem a Digidorable token, providing Merkle Proof to the token number
  ...
  function boxelsOwned() public view returns(uint256) {
    return boxelContract.balanceOf(msg.sender);
  }
  ...
  function calculatePrice(uint16 quantity) public view returns (uint256) {
    if(msg.sender == owner()) {
      ...
      return ((price - 0.01 ether) * uint256(quantity));
    }
    else {
      return (price * uint256(quantity));
    }
  }
