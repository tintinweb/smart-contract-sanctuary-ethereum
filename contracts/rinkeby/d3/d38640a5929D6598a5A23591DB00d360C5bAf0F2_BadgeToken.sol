...
contract BadgeToken is ERC721 {
    uint private _currentTokenId; // tokenId will start from 1

    constructor(
        string memory _name,
        string memory _symbol
    ) ERC721(_name, _symbol) { // Отправляем в наследуемый конструктор
    }
    ...
    function _getNextTokenId() public view returns(uint) {
    ...
    function tokenURI(uint tokenId) override public pure returns(string memory) {
