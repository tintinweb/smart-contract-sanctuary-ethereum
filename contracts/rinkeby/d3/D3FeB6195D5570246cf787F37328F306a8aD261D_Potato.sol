...
contract Potato is ERC721Delegated {
    uint public constant price = 0.0069 ether;
    string constant name = "Potato";
    string constant symbol = "Potato";

    uint public collectionSize = 101;
    ...
    function mint() external payable {
        require(atId.current() < collectionSize, "All have already been minted");

        if(freeMinted[msg.sender] == true){
            require(msg.value == price, "User already minted free mint");
        }
    ...
    function mintBatch (uint256 amount) external payable {
        require(atId.current() + amount < collectionSize, "All have already been minted");

        uint payableAmount = amount * price;
        if(freeMinted[msg.sender] == false){
            payableAmount -= price;
        }

        require(msg.value == payableAmount, "User already minted free mint");
