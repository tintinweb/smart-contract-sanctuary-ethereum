    ...
    OwnableUpgradeable
{
    ...
    modifier onlyApprovedUser() {
        require(!blacklistedUsers[msg.sender], "CollectionFactory: BLACKLISTED_USER");
        _;
    }
    ...
    function initialize(address storage_) public initializer {
        require(storage_ != address(0), "CollectionFactory: ZERO_HINATA");
        ...
        collections[0] = Collection(msg.sender, storage_, 0, false);
        collectionIds[storage_] = 0;
    }

    function _authorizeUpgrade(address) internal override onlyOwner {}

    function addUserToBlacklist(address user, bool blacklisted) external onlyOwner {
    ...
    function spawn(
        ...
        uint256 royalty,
        bool is721
    ) external override onlyApprovedUser returns (address) {
        if (is721) {
            Hinata721 nft = new Hinata721(msg.sender, name, symbol, uri);
            _store(msg.sender, address(nft), royalty, is721);
            return address(nft);
        } else {
            Hinata1155 nft = new Hinata1155(msg.sender, name, symbol, uri);
            _store(msg.sender, address(nft), royalty, is721);
            return address(nft);
        }
    }

    function register(address collection, uint256 royalty)
        external
        override
        onlyApprovedUser
        notDuplicated(collection)
    {
        uint256 csize;
        assembly {
            csize := extcodesize(collection)
        }
        require(csize > 0, "CollectionFactory: NON_CONTRACT");

        bool is721;
        try IERC165Upgradeable(collection).supportsInterface(type(IERC721).interfaceId) returns (
            bool result1
        ) {
            is721 = result1;
            if (!result1)
                try
                    IERC165Upgradeable(collection).supportsInterface(type(IERC1155).interfaceId)
                returns (bool result2) {
                    require(result2, "CollectionFactory: NOT_NFT_COLLECTION");
                } catch {
                    revert("CollectionFactory: NOT_NFT_COLLECTION");
                }
        } catch {
            revert("CollectionFactory: NOT_NFT_COLLECTION");
        }
        ...
        _store(msg.sender, collection, royalty, is721);
    }
    ...
    function _store(
        ...
        lastCollectionId += 1;
        collections[lastCollectionId] = Collection(owner, collection, royalty, is721);
        collectionIds[collection] = lastCollectionId;
        emit CollectionWhitelisted(lastCollectionId, owner, collection, royalty, is721);
    }

    function setRoyaltyFee(address collection, uint256 fee) external {
        ...
        require(fee <= 10000, "CollectionFactory: INVALID_FEE");
        collection_.royalty = fee;
    }
...
contract Hinata721 is ERC721Enumerable {
    ...
    constructor(
        ...
        require(owner_ != address(0), "Hinata721: ZERO_OWNER");
        owner = owner_;
        baseURI = uri_;
    }
...
contract Hinata1155 is ERC1155Supply {
    ...
    constructor(
        ...
        require(owner_ != address(0), "Hinata1155: ZERO_OWNER");
        owner = owner_;
        name = name_;
        symbol = symbol_;
        uri_ = uri__;
...
interface ICollectionFactory {
    ...
    function register(address collection, uint256 royalty) external;
