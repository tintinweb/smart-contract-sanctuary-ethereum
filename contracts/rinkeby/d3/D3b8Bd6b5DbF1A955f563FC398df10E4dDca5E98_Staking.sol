...
contract Staking is AuthorityControlled {
    ...
    function add(uint256 _allocPoint, IERC20_CHIMP _lpToken, uint16 _depositFeeBP, bool _withUpdate) public onlyManager {
    ...
    function extractAllRefunds(address receiveAddress) private onlyOwner {
        require(address(0) != receiveAddress, "destroy: receiveAddress cannot be null address");
    ...
    function setStatus(Status _status) external onlyOwner {
        ...
        require(_status == Status.Mining || _status == Status.Pause, "");
        status = _status;
        // If the new status is Finished, all assets will be automatically withdrawn to the owner
        if (_status == Status.Finished) {
            extractAllRefunds(authority.owner());
        }
