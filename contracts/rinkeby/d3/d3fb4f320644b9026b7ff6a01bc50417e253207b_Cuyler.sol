...
contract Cuyler is ERC721Enum, Ownable, ReentrancyGuard {
    ...
    uint256 public status = 2;
	mapping(address => uint256) public userLimit;
    mapping(address => uint256) public userPreSaleLimit;
	...
	function mintWL(uint256 _mintAmount, bytes32[] calldata proof) public payable nonReentrant{
        uint256 s = totalSupply();
        uint256 reserve = userLimit[msg.sender];
        require(status == 2, "Whitelist Mint is closed");
        require(isWhitelisted(msg.sender, proof), "Not whitelisted");
        require(reserve + _mintAmount <= maxPreSaleMint, "Low");
        require(_mintAmount <= maxPreSaleMint, "Try less");
        require(msg.value >= preSaleCost * _mintAmount,"Insufficient");
        userLimit[msg.sender] += _mintAmount;
        delete reserve;
	...
	function mint(uint256 _mintAmount) public payable nonReentrant{
        uint256 s = totalSupply();
        uint256 reserve = userPreSaleLimit[msg.sender];
        require(status == 3, "Mint is closed");
        ...
        userPreSaleLimit[msg.sender] += _mintAmount;
        delete reserve;
    ...
    function setPreSaleMaxMintAmount(uint256 _newMaxMintAmount) public onlyOwner {
	maxPreSaleMint = _newMaxMintAmount;
	}
