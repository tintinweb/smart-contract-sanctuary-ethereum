...
// File contracts/interfaces/IDelegate.sol
...
// File contracts/lib/MarketConsts.sol
...
struct OpenseaLiquidateERC721Params {
...
struct X2Y2LiquidateERC721Params {
  Market.RunInput input;
  address nftApprover; // executionDelegate
  LiquidateCallParams liquidateParams;
  FlashLoanParams flashLoanParams;
}
...
// File contracts/nftSellAdaptors/OpenseaAdaptor.sol
...
contract OpenseaAdaptor is Ownable {
  ConsiderationInterface public seaport;

  function fulfillBasicOrder(
    BasicOrderParameters memory parameters,
    address conduit
  ) internal virtual returns (bool fulfilled) {
    console.log("start opensea fulfillBasicOrder");

    IERC721(parameters.considerationToken).setApprovalForAll(conduit, true);
    console.log("considerationToken approvalForAll");

    return seaport.fulfillBasicOrder(parameters);
  }
...
// File contracts/interfaces/IX2Y2_r1.sol
...
interface IX2Y2_r1 {
  ...
  function signers(address _signer) external view returns (bool);
}


// File contracts/nftSellAdaptors/X2Y2Adaptor.sol
...
contract X2Y2Adaptor is Ownable {
  IX2Y2_r1 public x2y2;

  function x2y2Run(Market.RunInput memory input, address collateralAsset)
    internal
    virtual
  {
    console.log("start x2y2 run");
    IERC721(collateralAsset).setApprovalForAll(
      address(input.details[0].executionDelegate),
      true
    );
    console.log("collateralAsset approvalForAll");
    x2y2.run(input);
  }

  function setX2Y2Market(address _x2y2Market) public onlyOwner {
    x2y2 = IX2Y2_r1(_x2y2Market);
  }
}
...
// File contracts/nftSellAdaptors/LooksRareAdaptor.sol
...
contract LooksRareAdaptor is Ownable {
  ...
  function matchBidWithTakerAsk(
    ...
    console.log("start looksRare matchBidWithTakerAsk");

    IERC721(makerBid.collection).setApprovalForAll(transferManager, true);
    console.log("makerBid.collection approvalForAll");

    looksRareExchange.matchBidWithTakerAsk(takerAsk, makerBid);
  }

  function setlooksRareExchange(address _looksRareExchange) public onlyOwner {
...
// File contracts/nftSellAdaptors/ParaspaceAdaptor.sol
...
abstract contract ParaspaceAdaptor is LiquidatorBase {
  function acceptBidWithCredit(
    AcceptBidWithCreditParams memory params,
    address nft,
    address approver
  ) internal virtual {
    console.log("start paraspace acceptBidWithCredit");

    IERC721(nft).setApprovalForAll(approver, true);
    console.log("nft approvalForAll");

    CONFIG._pool().acceptBidWithCredit(
  ...
  ParaspaceAdaptor
{
  ...
  constructor(
    ...
    address seaport,
    address x2y2Market,
    address looksrateExchange
  ) LiquidatorBase(config) {
    super.setSeaport(seaport);
    super.setX2Y2Market(x2y2Market);
    super.setlooksRareExchange(looksrateExchange);
  }

  /**
   * @dev use opensea market to flashloan liquidate ERC721
   */
  function openseaLiquidateERC721(OpenseaLiquidateERC721Params calldata params)
    public
    onlyExecutor
  {
    bytes memory _paramsBytes = abi.encode(MarketType.OPENSEA, params);
  ...
  /**
   * @dev use x2y2 market to flashloan liquidate ERC721
   */
  function x2y2LiquidateERC721(X2Y2LiquidateERC721Params calldata params)
    public
    onlyExecutor
  {
    bytes memory _paramsBytes = abi.encode(MarketType.X2Y2, params);
    _flashLoanAndExecute(
      params.liquidateParams,
      params.flashLoanParams,
      _paramsBytes
  ...
  /**
   * @dev use looksRare market to flashloan liquidate ERC721
   */
  function looksRareLiquidateERC721(
    LooksRareLiquidateERC721Params calldata params
  ) public onlyExecutor {
    bytes memory _paramsBytes = abi.encode(MarketType.LOOKSRARE, params);
    _flashLoanAndExecute(
      params.liquidateParams,
      params.flashLoanParams,
      _paramsBytes
  ...
  /**
   * @dev use paraspace market to flashloan liquidate ERC721
   */
  function paraspaceLiquidateERC721(
    ParaspaceLiquidateERC721Params calldata params
  ) public onlyExecutor {
    bytes memory _paramsBytes = abi.encode(MarketType.PARASPACE, params);
    _flashLoanAndExecute(
      params.liquidateParams,
      params.flashLoanParams,
      _paramsBytes
  ...
  function _flashLoanAndExecute(
    ...
    console.log("Got flash loan amount:", amounts[0]);
  ...
  function executeOperation(
    ...
    if (marketType == MarketType.OPENSEA) {
      _executeOperationForOpensea(params_, amounts, premiums);
    } else if (marketType == MarketType.X2Y2) {
      _executeOperationForX2Y2(params_, amounts, premiums);
  ...
  /**
   * @dev flashloan callback for opensea type
   */
  function _executeOperationForOpensea(
    bytes memory _paramsBytes,
    uint256[] memory _amounts,
    uint256[] memory _premiums
  ) internal {
    (, OpenseaLiquidateERC721Params memory params) = abi.decode(
      _paramsBytes,
      (uint256, OpenseaLiquidateERC721Params)
    );

    _beforeSellNFT(params.liquidateParams, params.flashLoanParams, _amounts);

    bool fulfilled = super.fulfillBasicOrder(params.order, params.nftApprover);
    console.log("opensea fulfilled", fulfilled);
  ...
  /**
   * @dev flashloan callback for x2y2 type
   */
  function _executeOperationForX2Y2(
    bytes memory _paramsBytes,
    uint256[] memory _amounts,
    uint256[] memory _premiums
  ) internal {
    (, X2Y2LiquidateERC721Params memory params) = abi.decode(
      _paramsBytes,
      (uint256, X2Y2LiquidateERC721Params)
    );
    _beforeSellNFT(params.liquidateParams, params.flashLoanParams, _amounts);

    super.x2y2Run(params.input, params.liquidateParams.collateralAsset);
    console.log("x2y2 fulfilled");
    ...
    _afterSellNFT(
      ...
      _amounts,
      _premiums,
      address(params.input.orders[0].currency)
    );
  }
  ...
  function _executeOperationForLooksRare(
    ...
    super.matchBidWithTakerAsk(
      params.takerAsk,
      params.makerBid,
      params.nftApprover
    );
    console.log("looksRare fulfilled");
  ...
  function _executeOperationForParaspace(
    ...
    super.acceptBidWithCredit(
      params.acceptBidInfo,
      params.liquidateParams.collateralAsset,
      params.nftApprover
    );
    console.log("paraspace acceptBidWithCredit");
  ...
  function _beforeSellNFT(
    ...
    console.log("Params decoded, flashLoanAsset to debtAsset swap done");
    ...
    console.log("debtAsset approved to pool");
    ...
    console.log("liquidationCall to pool done");
  }
  ...
  function _afterSellNFT(
    ...
    if (_offerToken != _flashLoanParams.flashLoanAsset) {
      ...
      console.log("offerToken approved to uniswap");
      ...
      console.log("offerToken to flashLoanAsset swap done");
    ...
    console.log("Profit transferred to owner done");
    ...
    if (_flashLoanParams.mode != 0) {
      ...
      console.log("Left debtAsset transferred to owner done");
    }
    ...
    console.log("flashLoanAsset approved to flashLoanPool done");
  }
