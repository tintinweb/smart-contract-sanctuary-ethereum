...
contract Kayoko is ERC721A, ERC721AOwnersExplicit, Pausable, Ownable, ReentrancyGuard {
    uint internal immutable collectionSize;
    ...
    function whilteListMint(uint256 quantity, bytes32[] calldata _merkleProof)
        external
        payable
        whenNotPaused
        callerIsUser
    {
        uint256 _saleStartTime = uint256(saleConfig.whitelistMintStartTime);
    ...
    function publicMint(uint quantity) external payable whenNotPaused callerIsUser {
        ...
        require(numberMinted(msg.sender) + quantity <= 2, "can not mint this many");
        uint totalCost = saleConfig.price * quantity;
        super._safeMint(msg.sender, quantity);
        refundIfOver(totalCost);
    }
    ...
    function safeMint(address to, uint quantity) public onlyOwner {
    ...
    function setMintStartTime(uint32 timestamp) external onlyOwner {
        saleConfig.whitelistMintStartTime = timestamp;
    }
