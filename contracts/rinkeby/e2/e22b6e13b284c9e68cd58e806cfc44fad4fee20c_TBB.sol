...
contract TBB is ERC721Enum, Ownable, ReentrancyGuard {
    using Strings for uint256;
	
    uint256 public SALE_NFT = 9500;
	uint256 public GIVEAWAY_NFT = 500;
	
	uint256 public MAX_MINT_PRESALE_PARTNERS = 4;
	uint256 public MAX_MINT_PRESALE = 4;
	uint256 public MAX_MINT_SALE = SALE_NFT;
	
	uint256 public MAX_BY_MINT_IN_TRANSACTION_PRESALE_PARTNERS = 4;
	uint256 public MAX_BY_MINT_IN_TRANSACTION_PRESALE = 4;
	uint256 public MAX_BY_MINT_IN_TRANSACTION_SALE = 2;
	
	uint256 public PARTNERS_PRESALE_PRICE = 7 * 10**16;
	...
	bool public partnersPreSaleEnable = false;
	...
	address[] public addressList = [
		0xC9E5Ae70C91F8444E2FF18080036d8896C4317A8,
		0x05efEc7c78A70Fc5df213344ea46DA64a7FFD71E
	];
	uint[] public shareList = [8750,1250];
	
	struct User {
	    uint256 presaleMintPartners;
		uint256 presaleMint;
		uint256 saleMint;
    }
	mapping (address => User) public users;

    constructor() ERC721P('The Beauty Bunch', 'TBB') {}
	...
	function mintPartnersPreSaleNFT(uint256 _count, bytes32[] calldata merkleProof) public payable nonReentrant{
		bytes32 node = keccak256(abi.encodePacked(msg.sender));
		uint256 totalSupply = totalSupply();
		require(
			partnersPreSaleEnable, 
		...
		require(
			MerkleProof.verify(merkleProof, merkleRootPartners, node), 
			"MerkleDistributor: Invalid proof."
		);
		require(
			users[msg.sender].presaleMintPartners + _count <= MAX_MINT_PRESALE_PARTNERS,
			"Exceeds max mint limit per wallet"
		);
		require(
			_count <= MAX_BY_MINT_IN_TRANSACTION_PRESALE_PARTNERS,
			"Exceeds max mint limit per tnx"
		);
		require(
			msg.value >= PARTNERS_PRESALE_PRICE * _count,
			"Value below price"
		);
		for (uint256 i = 0; i < _count; i++) {
		...
		users[msg.sender].presaleMintPartners = users[msg.sender].presaleMintPartners + _count;
    }
	...
	function withdrawAll() public onlyOwner {
        ...
        for (uint256 i = 0; i < addressList.length; i++) {
		   payable(addressList[i]).transfer((balance * shareList[i]) / 10000);
        }
    }
	...
	function updatePartnersPreSalePrice(uint256 newPrice) external onlyOwner {
        PARTNERS_PRESALE_PRICE = newPrice;
    }
	...
	function setPartnersPreSaleStatus(bool status) public onlyOwner {
	   require(partnersPreSaleEnable != status, "Incorrect value");
       partnersPreSaleEnable = status;
    }
	...
	function updatePartnersPreSaleMintLimit(uint256 newLimit) external onlyOwner {
	    require(SALE_NFT >= newLimit, "Incorrect value");
        MAX_MINT_PRESALE_PARTNERS = newLimit;
    }
	...
	function updateMintLimitPerTransectionPartnersPreSale(uint256 newLimit) external onlyOwner {
	    require(SALE_NFT >= newLimit, "Incorrect value");
        MAX_BY_MINT_IN_TRANSACTION_PRESALE_PARTNERS = newLimit;
    }
	...
	function updateMerkleRootPartners(bytes32 newRoot) external onlyOwner {
	   merkleRootPartners = newRoot;
	}
	
	function updateAddressList(address ownerAddressOne, address ownerAddressTwo) external onlyOwner {
		require(ownerAddressOne != address(0), "owner one address is the zero address");
		require(ownerAddressTwo != address(0), "owner two address is the zero address");
		
		addressList[0] = ownerAddressOne;
		addressList[1] = ownerAddressTwo;
	}
	
	function updateShareList(uint256 ownerShareOne, uint256 ownerShareTwo) external onlyOwner {
	    require(ownerShareOne + ownerShareTwo == 10000, "share must be 100%");
		
		shareList[0] = ownerShareOne;
		shareList[1] = ownerShareTwo;
	}
}