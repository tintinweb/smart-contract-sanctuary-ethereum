...
//  (`' )/`--' / ,-.|  \ _ \   | ,_/  \__)/ `-' \ / ( ` )   '`--.  ,---'  \  _. /  '             
// (_ o _).   ;  \  '_ /  | :,-./  )       `-'`"`. (_ o _)  |   |   \      _( )_ .'                  
//  (_,_). '. |  _`,/ \ _/  |\  '_ '`)     .---. |  (_,_)___|   :_ _:  ___(_ o _)'         (`' )    __/\__    (`' )
// .---.  \  :: (  '\_/ \   ; > (_)  )  __ |   | '  \   .---.   (_I_) |   |(_,_)'         (_ o _)   \    /   (_ o _)
// \    `-'  | \ `"/  \  ) / (  .  .-'_/  )|   |  \  `-'    /  (_(=)_)|   `-'  /           (_,_)    / /\ \    (_,_) 
//  \       /   '. \_/``".'   `-'`-'     / |   |   \       /    (_I_)  \      /                  
//   `-...-'      '-----'       `._____.'  '---'    `'-..-'     '---'   `-..-'                   
...
contract StardustGeneration is ERC721AQueryable, Ownable {
    ...
    bytes32 public merkleRoot; // @notice for starlist and reservelist

    // @notice minting phase limits and wallet limit
    ...
    // @notice supply and cost (same for each phase)
    uint256 public constant MAX_SUPPLY = 7777; 
    uint256 public cost = 0.04 ether;
    
    // @notice toggle minting phases
    bool public starlistOpen = false;
    bool public reservelistOpen = false;
    bool public publicOpen = false;
    bool public isRevealed = false; 
  
    // @notice check if spots have already been minted
    mapping(address => uint256) private starlistRedeemed; 
    mapping(address => uint256) private reservelistRedeemed;

    // @notice emit events
    event StarlistMint(address indexed to, uint256 amount);
    event ReservelistMint(address indexed to, uint256 amount);
    event PublicMint(address indexed to, uint256 amount);
    event Withdraw(address indexed to, uint256 amount);
    event SpreadingStardust(uint256 totalSent);

    // @notice initialized values in constructor 
    constructor( 
            string memory _name, 
    ...
    // @notice set metasuffix function
    ...
    // @notice set merkle root for starlist and reservelist
    function setMerkleRoot(bytes32 _root) external onlyOwner {
        merkleRoot = _root;
    }

    // @notice set cost function
    function setCost(uint256 _newCost) external onlyOwner {
        cost = _newCost;
    }

    // @notice set max mints allowed per minting phase for public and total per wallet
    ...
    // @notice safely mint each nft to the caller/receiver 
    function _mintLoop(address _receiver, uint256 _mintAmount) internal {
        _safeMint(_receiver, _mintAmount);
    }

    // @notice Generate the leaf node (the hash of tokenID concatenated with the account address)
    function _leaf(address account) internal pure returns (bytes32) {
        return keccak256(abi.encodePacked(account));
    }

    // @notice Merkle Proof Verification
    ...
    function mintStarlist(
        ...
        unchecked {
            ...
            require(
                ...
                "Your mint amount exceeds the max mint per starlist (3)"
            );
        ...
        emit StarlistMint(msg.sender, _mintAmount);
    }
...
//                                                                                                                           \_/``"             (_I_)
    ...
    function mintReservelist(                                                                                       
        ...
        require(starlistOpen == false, "The starlist is open");
        ...
        unchecked {
            ...
            require(
                ...
                "Your mint amount exceeds the max mint per reservelist (3)"
            );
            require(totalSupply() + _mintAmount <= MAX_SUPPLY,
                "You would exceed the max supply of tokens"
            );
        ...
        emit ReservelistMint(msg.sender, _mintAmount); //                                                   ᖭི༏ᖫྀ
    }
...
//                                                                                    (_I_)  ᖭི༏ᖫྀ
    ...
    function mintPublic(uint256 _mintAmount) 
        external payable  
    {
        /// @dev using _numberMinted versus balanceOf, balanceOf is # of nfts you have at time of t(x)
        // @notice users can transfer them out to mint more
    ...
    function airdrop(address[] calldata _airdropAddresses, uint256 _mintAmount) external onlyOwner {
        ...
        emit SpreadingStardust(totalSent);
    }

    // `✦ ˑ ִֶָ 𓂃⊹ withdraw stardust `✦ ˑ ִֶָ 𓂃⊹ \\
    function setWithdrawAddress(address[] memory wAddress) public onlyOwner {
        w1 = wAddress[0]; 
    }
    function withdrawStardust() external onlyOwner {
        uint256 amount = address(this).balance;
        require(amount > 0);
        (bool s1, ) = payable(w1).call{value: amount}("");
        require(s1, "Not successful, the withdrawal failed");

        emit Withdraw(w1, amount);
    }
