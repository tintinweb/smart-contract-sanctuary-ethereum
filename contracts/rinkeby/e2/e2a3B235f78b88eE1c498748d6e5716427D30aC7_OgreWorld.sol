...
contract OgreWorld is ERC165, IERC721, IERC721Metadata, Ownable, ReentrancyGuard {
	...
	*/
	...
	/**
		...
		@param _amount The amount of tokens to mint.
	*/
    function whitelistMint(uint256 _amount, bytes32[] calldata _proof, bytes32 _merkleRoot) external payable {
        if (!_verify(_leaf(msg.sender), _proof, _merkleRoot)) { revert NotAWhitelistMember(); }
        if (_amount > MAX_MINT_PER_TRANSACTION) { revert MaxLimitPerTransactionExceeded(); }
