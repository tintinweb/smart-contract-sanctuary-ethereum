...
contract EIGHTBITSHIELDS is ERC721Enumerable, ReentrancyGuard, Ownable {
    ...
    uint256 private constant FOUNDERS_RESERVE_AMOUNT = 10;
    uint256 private constant MAX_PUBLIC_SHIELDS = MAX_SHIELDS - FOUNDERS_RESERVE_AMOUNT;
...
library factory {
  ...
  function draw(uint256 background, uint256 checkerPattern, uint256 color, uint256 eight) public pure returns (string memory) {
    ...
    string[2] memory eights = [
     ...
     string memory top = '<svg version="1.1" width="200" height="200" xmlns="http://www.w3.org/2000/svg" shape-rendering="crispEdges">';
     string memory style = '<style><![CDATA[';
     string memory closeStyle = ']]></style>';
     
     if (checkerPattern == 0) {
       string memory s = _getBigShield();
       return string(abi.encodePacked(top,style,bg,c,e,closeStyle,s,eightDraw,outline,'</svg>'));
     } else {       
       string memory s = _getSmallShield();
      return string(abi.encodePacked(top,style,bg,c,e,closeStyle,s,eightDraw,outline,'</svg>'));
     }
