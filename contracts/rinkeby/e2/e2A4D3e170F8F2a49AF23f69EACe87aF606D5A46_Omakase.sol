...
// File: contracts/ERC721Extra.sol
 ...
 */
abstract contract ERC721Extra is ERC721A {
    /// @notice Requires that msg.sender owns or is approved for the token.
    modifier onlyApprovedOrOwner(uint256 tokenId) {
        require(
            _ownershipOf(tokenId).addr == _msgSenderERC721A() ||
...
contract Omakase is ERC721A, ERC721Extra, Ownable {
    ...
    /// @notice User is not whitelisted
    error NotWhitelist();
    /// @notice User is not whitelisted
    ...
    /**
    @dev tokenId to staking start time (0 = not nesting).
     */
    ...
    /**
    @dev Emitted when a Omakase is staked.
     */
    event Staked(uint256 indexed tokenId);

    /**
    @dev Emitted when a Omakase stops being staked
     */
    ...
    modifier mintCompliance(uint256 _mintAmount) {
        ...
        if (supply + _mintAmount >= currentmaxSupply) revert MaxSupplyReached();
        _;
    }
    ...
    {
        if (onlyWhitelisted) revert NotWhitelist();
    ...
    function mint(uint256 _mintAmount, bytes32[] calldata _merkleProof)
        ...
        mintCompliance(_mintAmount)
    {
        if (_mintAmount + addressMintedBalance[msg.sender] >= maxMintAmountWL)
            revert();

        require(msg.value <= costWL * _mintAmount, "Not enough funds");
    ...
    */
    function toggleStake(uint256 tokenId)
        internal
    ...
    function stakedPeriod(uint256 tokenId)
        external
        view
        returns (
            bool staked,
