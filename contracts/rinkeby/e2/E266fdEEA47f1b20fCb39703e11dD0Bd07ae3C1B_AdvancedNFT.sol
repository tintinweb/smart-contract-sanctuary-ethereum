...
contract AdvancedNFT is ERC721EnumerableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function whitelistMintNft(uint256 _ammount, bytes calldata _signature) public payable nonReentrant {
        uint256 numberNftSold = totalSupply();

        require(totalSupply() < MAX_SUPPLY_NFT, "Sorry, no nfts left.");
        require(sellingStep == Steps.WhitelistedSale, "Sale has not started yet.");
        require(nftsPerWallet[msg.sender] < maxNftPurchase, "You can't mint more than X nfts");
        require(numberNftSold + _ammount <= MAX_SUPPLY_NFT, "Sale is almost done and we don't have enought NFTs left.");
        require(msg.value >= publicSaleNftPrice * _ammount, "You don't have enough funds.");

        // Our original pre-signed message on chain, converting that message to an Ethereum signed message
        require(_signerAddress == keccak256(
            abi.encodePacked(
                "\x19Ethereum Signed Message:\n32",
                bytes32(uint256(uint160(msg.sender)))
            )
        ).recover(_signature), "Signer address mismatch.");
