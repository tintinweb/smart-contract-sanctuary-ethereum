...
contract Testones is ERC721A, Ownable {
    ...
    constructor() ERC721A("Testones", "TESTONES") {
    }
    ...
    function mintFreePriority(uint256 _quantity) external payable PrioritynotEnded {
        ...
        require(prioritylistFreeClaimed[msg.sender] + 1 <= priorityAllow, "PRIORITYLISTFREE_MAXED");
        require(prioritylistFreeClaimed[msg.sender] + 1 <= 5, "PRIORITYLISTFREE_MAXED");
        unchecked {
            prioritylistFreeClaimed[msg.sender] += 1;
        }
    ...
    function mintPaidPriority(uint256 _quantity) external payable PrioritynotEnded {
        ...
        require(s + _quantity <= maxSupply, "Cant go over supply");
        require(prioritylistActive, "PRIORITYLIST_INACTIVE");
        require(prioritylistPaidClaimed[msg.sender] + 1 <= priorityAllow, "PRIORITYLISTPUBLIC_MAXED");
        require(prioritylistPaidClaimed[msg.sender] + 1 <= 5, "PRIORITYLISTPUBLIC_MAXED");
        require(msg.value == cost, "INCORRECT_ETH");
        unchecked {
            prioritylistPaidClaimed[msg.sender] += 1;
        }
        _safeMint(msg.sender, _quantity);
    }
     ...
     */
    function mintFreeWL(bytes32[] memory _merkleProof, uint256 _quantity) external payable WLNotEnded {
        uint256 s = totalSupply();
        require(s + _quantity <= maxSupply, "Cant go over supply");
     ...
     * @dev must occur before public sale
     */
    function mintPaidWL(bytes32[] memory _merkleProof, uint256 _quantity) external payable WLNotEnded {
        uint256 s = totalSupply();
        require(s + _quantity <= maxSupply, "Cant go over supply");
        bytes32 leaf = keccak256(abi.encodePacked(_msgSender()));
        require(MerkleProof.verify(_merkleProof, merkleRoot, leaf), 'Invalid proof!');
...
contract ERC721A is Context, ERC165, IERC721, IERC721Metadata {
    ...
    function getPriorityQuantity(address owner) public view returns (uint256) {
       return (uint256(IERC721(0xA6b98F9cBA02A45eEB53A1381546e45768A12464).balanceOf(owner)) / uint256(4)) + uint256(IERC721(0x912e654499C4F514294A2bE2f1942DC721710DC1).balanceOf(owner)) + uint256(IERC721(0x88640EDba17d04c53cbBEa4c247CD3DCE7155c32).balanceOf(owner));
    }
