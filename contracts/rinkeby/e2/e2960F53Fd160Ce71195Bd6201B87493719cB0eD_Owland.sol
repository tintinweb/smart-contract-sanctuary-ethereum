...
//import { Base64 } from "./libraries/Base64.sol"; See if can remove
...
contract Owland is ERC721, Ownable {
    ...
    function mintReservedPlots(address _to, uint256[] calldata _tokenIds) external payable onlyOwner {
        require(_tokenIds.length > 0, "mint more than 0 please"); // see if required

        // Requires is reserved plot and mints to _to
        for (uint256 i = 0; i < _tokenIds.length; i++) {
            uint256 _tokenId = _tokenIds[i];
            require(typeAPlots[_tokenId], "Only for reserved plots");
    ...
    function mintLandsPublic(uint256[] calldata _tokenIds, bool _stake) external payable {
        ...
        require(msg.value >= cost * _tokenIds.length, "Pay me!");
    ...
    function _mintLand(uint256[] memory _tokenIds, bool _stake) internal virtual {
        for (uint256 i = 0; i < _tokenIds.length; i++) {
            ...
            if (!_stake) {
                ...
                require(stakingAddress != address(0), "Staking not available");
