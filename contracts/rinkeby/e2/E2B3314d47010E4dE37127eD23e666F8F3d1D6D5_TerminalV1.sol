...
contract TerminalV1 is Operatable, ITerminalV1, ITerminal, ReentrancyGuard {
  ...
  function _takeFee(
    ...
    if (terminalDirectory.terminalOf(DaliProject(governance).projectId()) == this) {
      // Use the local pay call.
      _pay(DaliProject(governance).projectId(), feeAmount, _beneficiary, _memo, false);
    } else {
      // Use the external pay call of the governance contract.
      DaliProject(governance).pay{value: feeAmount}(_beneficiary, _memo, false);
    }
  }
}
