...
contract Coordinate is ERC721A, Ownable {
  using Strings for uint256;

  uint256 public constant maxSupply = 9393;
  uint256 public constant freeSupply = 393;
  uint256 public publicMocaSupply = 939;
  uint256 public publicSupply = maxSupply - freeSupply - publicMocaSupply;
  uint256 public constant ethPrice = 0.06 ether;
  uint256 public constant mocaPrice = 100 * 10 ** 18;
  ...
  constructor(string memory _name, string memory _symbol, IERC20 _mocaAddress)
  ...
  function publicMint(uint256 _qty)
    ...
    whenNotPaused
  {
    require(_totalMinted() <= maxSupply, "Out of stock");
  ...
  {
    require(_totalMinted() <= maxSupply, "Out of stock");
    require(mocaMinted < publicMocaSupply, "Public Moca supply is out of stock");
  ...
  function freeMint(uint256 _qty) external whenNotPaused {
    require(freeMinted < freeSupply, "Free mint supply is out of stock");
    require(freeMinterQuota[msg.sender] >= _qty, "Insufficient quota left");
    ...
    onlyOwner
  {
    ...
    for (uint256 i = 0; i < _addresses.length; i++) {
       ...
       */
      freeClaimed[_addresses[i]] = freeClaimed[_addresses[i]] > 0 ?
        freeClaimed[_addresses[i]] :
        0;
    }
  }
  ...
  function delegateFreeMintTo(
    ...
    external
  {
    require(freeMinterQuota[msg.sender] > 0, "Address have 0 quota left");
    require(_addresses.length == _qtys.length, "Mismatch data length");
    ...
    require(
      freeMinterQuota[msg.sender] >= total,
      "Insufficient quota left to delegate"
    );
    ...
    for (uint256 i = 0; i < _addresses.length; i++) {
      /**
        * @dev We don't want to reset freeMinterQuota count
        * if we try to add someone more than once.
        */
  ...
  function isFreeMinter(address _address) external view returns(bool) {
    require(_address != address(0), "Zero address not found");
    return freeMinterQuota[_address] > 0;
  }
  ...
  function setMocaSupply(uint256 _qty) external onlyOwner {
    publicMocaSupply = _qty;
  }
  ...
  function withdrawMoca() external onlyOwner {
