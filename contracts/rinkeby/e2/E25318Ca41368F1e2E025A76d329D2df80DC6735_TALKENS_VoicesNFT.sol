...
contract TALKENS_VoicesNFT is ERC721P, Ownable {
  ...
  string public baseAIURI;
  string public voiceProvenance = "";
  ...
  constructor() ERC721P("HumanVoice", "VOICE") {}

  function authorizedMint(uint256 _amount, uint256 _nonce, bytes memory _signature) external {
    require(saleIsActive, "Not active");
    require(totalSupply() + _amount <= maxSupply - reserved, "Exceeds max supply");
    bytes32 hash = hashTransaction(_msgSender(), _amount, _nonce);
    require(!hashUsed[hash], "Nonce already used");
    require(matchSignerAdmin(signTransaction(hash), _signature), "Signature mismatch");

    hashUsed[hash] = true;
    mint(_amount, _msgSender());
  }

  function ownerMint(uint256 _amount, address _recepient) external onlyOwner {
    require(totalSupply() + _amount <= maxSupply, "Exceeds max supply");
    //require(reserveMinted + _amount <= reserved, "Amount exceeds reserve");
    ...
    mint(_amount, _recepient);
  }

  function mint(uint256 _amount, address _recepient) internal {
  ...
  function setAIURI(string calldata _newBaseAIURI) external onlyOwner {
    baseAIURI = _newBaseAIURI;
  }
  ...
  function setProvenanceHash(string memory provenanceHash) external onlyOwner {
    voiceProvenance = provenanceHash;
  }
  ...
  function emergencyReSetStartingIndex() external onlyOwner { // Ideally this never gets called
    startingIndexBlock = block.number - 1;
  ...
  function hashTransaction(address _sender, uint256 amount, uint256 nonce) public pure returns (bytes32) {
    bytes32 _hash = keccak256(abi.encode(_sender, amount, nonce));
    return _hash;
  }
