...
contract SimpleVault {
  ...
  function calculateCollateralValueUSD(address _addr) public view returns (uint256) {
    return bal[_addr] * oraclePrice * ethPrice;
  }

  function showEthPriceUsdWei() public view returns (uint256) {
    return ethPrice;
  }

  function getCollateralPriceEth() public view returns (uint256) {
    return oraclePrice;
  }
  function getCollatalPriceUSD() public view returns (uint256) {
    return oraclePrice * ethPrice;
  }
  ...
  function setEthPrice() external {
    ethPrice = uint256(getEthUsd()) * 10**10;
  }
}
