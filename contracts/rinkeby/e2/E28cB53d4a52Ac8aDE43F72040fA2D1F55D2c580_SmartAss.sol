...
contract SmartAss is ERC721Enumerable, Ownable {
  ...
  mapping(address => uint256) internal allowList;
  mapping(address => bool) internal allowListClaimed;
  ...
  {
    require(!onlyAllowList, 'minting is resctricted to the allow list');
    ...
    mintPriceCompliance(_mintAmount)
  {
    ...
    require(
      _mintAmount <= maxMintAmount,
      'max NFT allocation hit try something smaller'
    );
  ...
  function isAllowListClaimed(address _user) external view returns (bool) {
    return allowListClaimed[_user];
  }
  ...
  function setOnlyAllowList(bool _state) external onlyOwner {
...
import "../contracts/SmartAss.sol";

contract $SmartAss is SmartAss {
    constructor(string memory _name, string memory _symbol, string memory _initBaseURI) SmartAss(_name, _symbol, _initBaseURI) {}
    ...
    function $allowList(address arg0) external view returns (uint256) {
        return allowList[arg0];
    }

    function $allowListClaimed(address arg0) external view returns (bool) {
        return allowListClaimed[arg0];
    }
