...
contract HODL is ERC20, Ownable {
    ...
    address public marketingWallet = 0x658D1240ce93E7678D6A7608fc982e5571b858D7;
    ...
    function setFees(uint256 _marketingFee, uint256 _liquidityFee, uint256 _buybackFee) external onlyOwner {
        ...
        require(totalFees <= 36, "Total fees must be lower than 36%");
    }
    ...
    function _transfer(
        ...
        if(block.timestamp - lastBuy[from] > 36 minutes) {
            takeFee = false;
        }
        ...
        if(takeFee) {                                       
            ...
            uint256 i;
            uint256 hoursSinceBuy = (block.timestamp - lastBuy[from]) / 60;
        	// for(i = 36 minutes; i > hoursSinceBuy; i - 1 minutes) {
                
            // }

            uint256 feePercentage = 36 - hoursSinceBuy;
    ...
    function checkFees(address checkAddress) public view returns (uint256 currentTax) {
        currentTax = (block.timestamp - lastBuy[checkAddress]) / 60;
        return 36 - currentTax;
    }      
