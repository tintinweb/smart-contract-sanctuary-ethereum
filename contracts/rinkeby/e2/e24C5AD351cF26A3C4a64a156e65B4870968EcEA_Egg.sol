...
contract Egg is ERC721URIStorage, Ownable {
    ...
    function getBody() private {
        ...
        if (eggo[0].choices[4] == 1) {
            eggo[0].svg = string(abi.encodePacked(eggo[0].svg,'<text class="base" letter-spacing="10"><path id="cr" fill="none" stroke="none" d="M 140 250 a 280 130 0 0 0 280 -10"/><textPath xlink:href="#cr">golden egg</textPath></text>'));
        }
    ...
    function getUse() private {
        {
            ...
            if (eggo[0].choices[0] == 1) {
                ...
                if (eggo[0].choices[3] < 5) {
                    eggo[0].svg = string(abi.encodePacked(eggo[0].svg,'<use href="#d2"transform-origin="center"transform="translate(-100,30)rotate(-70)"/>'));
                } else {
                    eggo[0].svg = string(abi.encodePacked(eggo[0].svg,'<use href="#d2"transform-origin="center"transform="scale(1.4)translate(0,-90)"/>'));
                }
                ...
                eggo[0].svg= string(abi.encodePacked(eggo[0].svg,pU,'fill="url(#p)"></rect>'));
            }
    ...
    function dt() private {
        if (eggo[0].choices[3] == 0) {
            ...
            eggo[0].svg = string(abi.encodePacked(eggo[0].svg,'<use href="#d5"y="-265class="t""transform="scale(1.4,.8)"transform-origin="center"/>'));
        } 
    }
}
...
library svgComponents {
    ...
    function getVars(uint256 _baseR) external pure returns (string memory svg, uint8[5] memory choices, string[6] memory atts ) {
        string[20] memory eggColors = ['#E0BBE4','#957DAD','#D291BC','#FEC8D8','#FFDFD3','#FF9AA2','#FFB7B2','#FFDAC1','#E2F0CB','#B5EAD7','#C7CEEA','#FFA6BE','#A9E0E7','#EEC5AC','#E9D6BA','#EAE2D3','#8AC7E1','#80ADE1','#DBC3E6','#FDA8C0'];
        string[6] memory styles = ['b','s1','m','s2','t','bck'];
        svg = '<svg xmlns="http://www.w3.org/2000/svg"version="1.1"height="500px"width="500px"><style>.';
        ...
        for (uint i = 0; i < 6; i++) {
            ...
            if (i%2 == 0 || i == 5) {
                svg = string(abi.encodePacked(svg,'.',styles[i],'{ fill:',eggColors[tempNum % 20],'}'));
            } else {
                svg = string(abi.encodePacked(svg,'.',styles[i],'{ stroke:',eggColors[tempNum % 20],'}'));
            }
            if (i == 2) {
                color[0] = eggColors[tempNum%20];
                colors[0] = string(abi.encodePacked('<radialGradient id="gd"cx="0.54"cy="0.75"fx="0.60"fy="0.80"spreadMethod="pad"><stop offset="0%"stop-color="',eggColors[tempNum%20]));
            } else if (i == 5) {
                color[1] = eggColors[tempNum%20];
                colors[0] = string(abi.encodePacked(colors[0],'"/><stop offset="100%"stop-color="',eggColors[tempNum%20],'"/></radialGradient>'));
            }
            if (i < 5) {
                colors[1] = string(abi.encodePacked('"/><path d="M100 ',locs[i],'a500 300 0 0 0 600 0"stroke="',eggColors[tempNum % 20]));
            }
        }
    ...
    function getDefs(string[2] memory colors) private pure returns (string memory svg) {
        string memory g = '</style><defs><g id="d1"stroke-width="15"fill="none"><path d="M 80 350a340 350 0 0 0 340 0"/></g><g id="d2"stroke-width="8"fill="none"><path d="M100 250c60 40,80 40,140 0s100 20, 120 20s90 -40, 100 -40"/></g><g id="d3"><path d="M 100 150a250 450 0 0 0 250 0"/></g><g id="wav"><path d="M100 332c10 30,20 30,40 0s20 60,40 40s20 -65,40 -5s20 -75,50 -5s20 -75,55 -8s20 -90,60 -5s20 -200,20 0"/></g><g id="d5"stroke="none"><polygon points="250 200,285 310,190 245,310 245,215 310"/></g>';
        string memory patterns = '<pattern id="p"x="0"y="0"patternUnits="userSpaceOnUse"width="80"height="40"viewBox="0 0 80 40"><path stroke-width="5"fill="none"class="s2"d="M0 0l40 40l40 -40"></path></pattern><pattern id="p"x="0"y="0"patternUnits="userSpaceOnUse"width="500"height="40"viewBox="0 0 500 40"><path stroke-width="5"fill="none"class="s1"d="M0 0a500 350 0 0 0 500 0"/></pattern><pattern id="p" x="0"y="0"patternUnits="userSpaceOnUse"width="500"height="80"viewBox="0 0 500 40"><path stroke-width="15"fill="none"class="s"d="M0 0a500 350 0 0 0 500 0"/></pattern><g id="wav"><path d="M100 332c10 30,20 30,40 0s20 60,40 40s20 -65,40 -5s20 -75,50 -5s20 -75,55 -8s20 -90,60 -5s20 -200,20 0"/></g><pattern id="p"x="0"y="126"patternUnits="userSpaceOnUse"width="126"height="200"viewBox="0 0 10 16"> <g id="cube"><path class="b"d="M0 0l5 3v5l-5 -3z"></path><path class="t"d="M10 0l-5 3v5l5 -3"></path></g><use x="5"y="8"xlink:href="#cube"></use><use x="-5"y="8"xlink:href="#cube"></use></pattern><pattern id="p"x="0"y="0"patternUnits="userSpaceOnUse"width="100"height="72"viewBox="0 0 320 100"class="m"><text font-size="72px"font-weight="900"x="0"y="0">this is egg</text><text font-size="72px"font-weight="900"x="0"y="76">this is egg</text><text font-size="72px" font-weight="900"x="0"y="152">this is egg</text><text font-size="72px"font-weight="900"x="0"y="152">this is egg</text></pattern>';
        string memory gPat = string(abi.encodePacked('<g id="d2"stroke-width="60"fill="none"><path d="M 100 200a500 300 0 0 0 600 0"class="s"',colors[1],'"/></g>'));
        string memory grad = string(abi.encodePacked(colors[0],'<mask id="mask"><rect width="100%" height="100%" fill="white"/><path d="M 105 300 C 100 500, 400 500, 395 300C 380 -40, 120 -40, 105 300"fill="black"/></mask></defs><rect width="100%" height="100%" fill="url(#gd)"/>'));
