...
contract NOMORE is ERC721A, Ownable, ReentrancyGuard {
  ...
  uint256 public constant MAX_RESERVE = 200;
  uint256 public constant MAX_PER_WHITELIST = 2;
  uint256 public constant WHITELIST_PRICE = 0.09 ether;
  uint256 public constant MAX_PER_RAFFLE = 5;
  uint256 public constant RAFFLE_PRICE = 0.18 ether;
  ...
  constructor(string memory initbaseURI, address initSigner) ERC721A("NOMORECLUB", "NOMORE") {
    baseURI = initbaseURI;
    _signer = initSigner;
  }
  ...
  function raffleMint(
    ...
    require(tx.origin == msg.sender, "NOMORE: contract is not allowed");
    require(quantity > 0 && quantity <= MAX_PER_RAFFLE, "NOMORE: invalid quantity");
    require(totalSupply() + quantity <= MAX_SUPPLY, "NOMORE: reached max supply");
    require(numberMinted(msg.sender) + quantity <= MAX_PER_RAFFLE, "NOMORE: max mint exceeded");
    require(_verify(_hash(msg.sender, 2, salt), signature), "NOMORE: invalid signature");
    _safeMint(msg.sender, quantity);
    checkAndRefundIfOver(RAFFLE_PRICE * quantity);
    emit Minted(msg.sender, quantity);
  }
  ...
  function reserve(
    ...
    require(totalSupply() + quantity <= MAX_SUPPLY, "NOMORE: reached max supply");
    require(reservedCount + quantity <= MAX_RESERVE, "NOMORE: reached max reserve");
  ...
  function reserveBatch(
    ...
    require(reservedCount + total <= MAX_RESERVE, "NOMORE: reached max reserve");
