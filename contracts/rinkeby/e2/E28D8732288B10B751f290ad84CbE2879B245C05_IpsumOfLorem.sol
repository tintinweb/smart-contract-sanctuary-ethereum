...
contract IpsumOfLorem is Ownable {
    ...
    function isownerOf(address tokenContract, uint256 tokenId) public view onlyAllowedTokenContract(tokenContract) returns(bool){
        address tokenOwner = IERC721(tokenContract).ownerOf(tokenId);
        return tokenOwner == _msgSender();
    }

    function addIpsum(address tokenContract, uint256 tokenId, bool userflag, string memory ipsumMetadataURI) 
    public onlyAllowedTokenContract(tokenContract) {
        ...
        tokenIpsum[tokenContract][tokenId].push(
            Ipsum(_msgSender(), userflag, true, ipsumMetadataURI)
            );
        emit IpsumAdded(
            tokenContract,
            tokenId,
    ...
    function addIpsumWithSignature(bytes memory signature, address tokenContract, uint256 tokenId, uint256 ipsumIdx,  bool userflag, string memory ipsumMetadataURI
    ) public onlyAllowedTokenContract(tokenContract) {
