...
contract MutantZukis is ERC721A, ERC721AQueryable, ERC721APausable, ERC721ABurnable, Ownable, ReentrancyGuard {
    ...
    function mint(uint32 _mintAmount, bytes memory signature, uint32 freeMints) public payable mintCompliance(_mintAmount) {
        require(_mintAmount <= (SALE_STATE == 1 ? MAX_MINT_AMOUNT_PER_TX_PRE_SALE : MAX_MINT_AMOUNT_PER_TX_PUBLIC_SALE), "Mint limit exceeded!");
        require(_currentIndex >= 10, "First 10 mints are reserved for the owner!");
        require(SALE_STATE > 0, "Sale not started");

        bool inAllowList = recoverSigner(keccak256(abi.encodePacked(msg.sender, freeMints)), signature) == SIGNER;
        ...
        require(SALE_STATE == 2 || _mintAmount <= remainingAllowListMints, "Trying to mint too much");
        ...
        if (inAllowList && SALE_STATE == 1) {
            remainingAllowListMints -= _mintAmount;
            remainingAllowListMints += freeMinted;
        }
