 ...
 * @notice It is used to allow creators to set royalty parameters in the RoyaltyFeeGetter.
 */
contract RoyaltyFeeSetter is Ownable {
    ...
    function updateRoyaltyInfoForCollectionIfSetter(
        ...
        (address currentSetter, , ) = IRoyaltyFeeGetter(royaltyFeeRegistry)
            .royaltyFeeInfoCollection(collection);
        require(msg.sender == currentSetter, "Setter: Not the setter");

        IRoyaltyFeeGetter(royaltyFeeRegistry).updateRoyaltyInfoForCollection(
            collection,
            setter,
            receiver,
            fee
    ...
    function updateRoyaltyFeeLimit(uint256 _royaltyFeeLimit)
        ...
        onlyOwner
    {
        IRoyaltyFeeGetter(royaltyFeeRegistry).updateRoyaltyFeeLimit(
            _royaltyFeeLimit
        );
    }
