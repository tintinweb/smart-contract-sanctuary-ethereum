...
contract LambDuhPX is ERC721Enumerable, Ownable, nonReentrant {

    // Set production values here to avoid mistakes in migration

    // uint256 public pxPS_Price = 50000000000000000;		// 0.050 ETH
    // uint256 public pxLambPrice = 55000000000000000;		// 0.055 ETH
    // uint public constant maxPxLambPurchase = 10;
    ...
    // uint public immutable MAX_PXLAMBRESERVE = 100;		// total team reserves allowed
    // uint public immutable MAX_PXPORTALS = 1500;			// total portal lambs allowed
	...
	uint256 public pxPS_Price = 1000000000000000;		// 0.0010 ETH TEST
    uint256 public pxLambPrice = 1100000000000000;		// 0.0011 ETH TEST
    uint256 public constant maxPxLambPurchase = 20; 	// TEST

    uint256 public immutable MAX_PXLAMB = 50;			// TEST
	uint256 public immutable MAX_PRESALE = 40;		// total presale lambs allowed
    // Reserve PX Lamb for team - Giveaways/Prizes etc
    uint256 public immutable MAX_PXLAMBRESERVE = 5;		// total team reserves allowed
    uint256 public immutable MAX_PXPORTALS = 10;		// total portal lambs allowed
	...
	address public tokenDeposit;						// address to receive custom ERC20
	...
	ILambDuh LambContract = ILambDuh(0x1F0f72e6Dc2EA6FDe3A32A1B3fD47A26a3293Dc9);
	...
	// uint256 public MAX_TOKEN = 100;					// 100 mints
	// uint256 public roolahCost = 100000000000000000000;	// 100 ROOLAH
	// uint256 public MAX_STAR = 100;						// 100 mints
	// uint256 public starCost = 2000000000000000000000;	// 2000 STAR
	// IERC20 roolah = IERC20(0xC6cA1e69bf227eCB34c44CA4C1BA63EDD7DEefd4);
	// IERC20 star = IERC20(0x24Eb24647786135352cbB14f888B23277F0014e7);
	...
	constructor() ERC721("Lamb Duh PX", "LDPX") {}
	...
	function mintPresale(bytes32[] memory _proof, bytes2 _maxAmountKey, uint256 _mintAmount, TokenName _tokenName) public payable reentryLock {
		require(MerkleRoot > 0x00 && saleState > SaleState.Paused, "presale period not open!");
		
		uint256 supply = totalSupply();
		require(supply + _mintAmount < MAX_PXLAMB + 1, "max supply exceeded");
		
		require(MerkleProof.verify(_proof, MerkleRoot, keccak256(abi.encodePacked(msg.sender, _maxAmountKey))), "invalid  proof-key combo");
        uint8 allowed = uint8(uint16(_maxAmountKey));
		
		if(_tokenName == TokenName.ETH) {
			require(saleState < SaleState.Open, "presale closed!");
			require(msg.value >= _mintAmount * pxPS_Price, "not enough ETH sent");
			require(mintableTokens[_tokenName].tokenSupply + _mintAmount < MAX_PRESALE + 1, "presale sold out");
			
			if(saleState == SaleState.Presale) {
				require(addressInfo[msg.sender].ownerPresaleMints + _mintAmount < allowed + 1, "max presale claims exceeded");
			} else if(saleState == SaleState.Phase2) {
				require(addressInfo[msg.sender].ownerPresaleMints + _mintAmount < 2 * allowed + 1, "max phase 2 claims exceeded");
			}
			...
			mintableTokens[_tokenName].tokenSupply += uint64(_mintAmount);
			addressInfo[msg.sender].tokenMinted[uint(_tokenName)] = true;
		}
	...
	// TokenName: BAMBOO, PIXL, ROOLAH, SEED, STAR | LAMEX, SPIT
	function mintWithToken(TokenName _tokenName) public reentryLock {
		require(MerkleRoot > 0x00 && saleState > SaleState.Paused, "token mint not open!");
		
		uint supply = totalSupply();
		require(supply < MAX_PXLAMB, "max supply exceeded");

		require(mintableTokens[_tokenName].tokenSupply < mintableTokens[_tokenName].MAX_TOKEN, "max tokenMint exceeded");
		
		require(addressInfo[msg.sender].tokenMinted[uint(_tokenName)] == false, "tokenMint already claimed");

		require(_tokenName < TokenName.LAMEX, "invalid ERC20");
		mintableTokens[_tokenName].Contract.transferFrom(msg.sender, tokenDeposit, mintableTokens[_tokenName].Cost);
	...
	function mintFreeClaim(bytes32[] memory _proof, bytes2 _maxAmountKey, uint256 _mintAmount) public reentryLock {
		require(MerkleRoot > 0x00 && saleState > SaleState.Paused, "free claim period not open!");
		
		uint supply = totalSupply();
		require(supply + _mintAmount < MAX_PXLAMB + 1, "max supply exceeded");
		uint portalSupply = supply - portalsIDs.length;
		require(portalSupply + _mintAmount < MAX_PXPORTALS + 1, "max portals exceeded");
		
		require(MerkleProof.verify(_proof, MerkleRoot, keccak256(abi.encodePacked(msg.sender, _maxAmountKey))), "invalid proof-key combo");
        uint8 allowed = uint8(bytes1(_maxAmountKey));
		require(addressInfo[msg.sender].ownerFreeClaims + _mintAmount < allowed + 1, "max allowed claims exceeded");
		require(addressInfo[msg.sender].ownerFreeClaims + _mintAmount < MAX_FREECLAIM + 1, "max free claims exceeded");
		require(LambContract.balanceOf(msg.sender) >= 3 * (_mintAmount + addressInfo[msg.sender].ownerFreeClaims), "insuffient OG Lamb balance");
    ...
    function getTokensMinted(address _sender) public view returns (bool[7] memory) {
	...
	function setTokenConfig(TokenName _token, uint64 _MAX_TOKEN, uint128 _Cost, address _tokenAddress) public onlyOwner {
		if(mintableTokens[_token].tokenSupply > 0) {
			require(mintableTokens[_token].tokenSupply >= _MAX_TOKEN, "supply already greater than max");
		}
    ...
    function teamMint(address _to, uint256 _reserveAmount) public onlyOwner {
        require(_reserveAmount + teamMints < MAX_PXLAMBRESERVE + 1, "Not enough reserve left for team");
		uint256 supply = totalSupply();
