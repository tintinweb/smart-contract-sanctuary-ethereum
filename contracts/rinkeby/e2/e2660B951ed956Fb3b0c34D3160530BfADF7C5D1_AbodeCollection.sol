...
contract AbodeCollection is ERC721, ERC721URIStorage, Pausable,
    ERC721Burnable, AccessControl, Ownable  {
    ...
    function rollback(address payable tokenOwner) external onlyRole(VALIDATE_ADDRESSES_ROLE) {
      ...
      for(uint i = 0; i < ownerTokens[tokenOwner].length; i++){
        ...
        _burn(ownerTokens[tokenOwner][i]);
      }
      tokenOwner.transfer(price.mul(ownerTokens[tokenOwner].length));
