...
contract JekyllAndHydeNFT is ERC2981, ERC721, Ownable {
    // fixed values
    uint256 private constant collectionSize = 1100; 
    uint256 private constant earnableNFTs = 100; 
    uint256 private mintableNFTsAvailable = collectionSize - earnableNFTs;
    uint256 private nextEarnedNFTId;

    // mutable properties
    string private _baseTokenURI =
        "https://bookcoin.mypinata.cloud/ipfs/QmcjAK4ua3cYkzt7rJnLqFX7U9VDJdHrpjS9nDDHLjxD4U/";
    string private _contractUri =
        "https://bookcoin.mypinata.cloud/ipfs/QmdazdUZVgVPCkjoEmutrnd2DWSoCiRcdTVpE3cM99TndY";
    ...
    // TODO: dates below are in the future - must be set post deploy
    uint256 private _bookMintStartTime = 1700000000;
    uint256 private _bookMintEndTime = 1800000000;

    // used for psuedo-Random mint mechanic
    uint256[collectionSize] private _tokenIds;
    ...
    uint96 royaltyPercent = 500; // denominator is 10000, so this is 5%
    ...
    function earn(string calldata code, bytes32[] calldata proof) external {
        address account = _msgSender();
        require(_verifyEarn(_leafString(code), proof), BAD_PROOF);
     ...
     */
    function canEarn(string calldata code, bytes32[] calldata proof) public view returns(bool)
    {
    ...
    function _mintEarned(address account) internal {
        ...
        require(nextEarnedNFTId < earnableNFTs, "No more earnable NFTs");
        uint256 nextToken = nextEarnedNFTId;
    ...
    function _mintOne(address account) internal {
        require(mintableNFTsAvailable > 0, "No more mintable NFTs");
        _safeMint(account, _nextToken());
        _mintedSupply += 1;
    }
    ...
    // gets a psuedo-random id from remaining mintable (not earnable) NFTs
    function _nextToken() internal returns (uint256) {
        // get highest number between the last earnable NFT id and the highest id
        // less the already minted supply
        uint256 tempMax = maxSupply() - mintedSupply() + nextEarnedNFTId - earnableNFTs;
        // Psuedo-random number generator:
        // hash a few changing values from the block and tx, then get the modulus
        uint256 psuedoRandom = uint256(
            keccak256(
                abi.encodePacked(
                    msg.sender,
                    block.timestamp,
        ...
        // set index to either the generated number, or the value already set in the array
        if (_tokenIds[psuedoRandom] == 0) {
            index = psuedoRandom;
        } else {
            index = _tokenIds[psuedoRandom];
        }

        // consume the highest possible index by placing it in the array
        if (_tokenIds[tempMax - 1] == 0) {
            _tokenIds[psuedoRandom] = tempMax - 1;
        } else {
            _tokenIds[psuedoRandom] = _tokenIds[tempMax - 1];
        }

        // decrement number of available NFTs (reducing tempMax on next run)
        mintableNFTsAvailable -= 1;

        // make sure the earnable NFT IDs are not returned
        return index + earnableNFTs;
    }
}
