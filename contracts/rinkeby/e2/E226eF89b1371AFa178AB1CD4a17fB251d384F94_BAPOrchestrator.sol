...
contract BAPOrchestrator is ReentrancyGuard, Ownable {
    ...
    constructor(address _bapGenesis, address _bapMethane, address _bapUtilities, address _bapTeenBulls, address _orchestratorV1, address _orchestratorV2) {
    ...
    function claimMeth(bytes memory signature, uint256[] memory bulls, uint256[] memory gods) external nonReentrant {
    ...
    function generateGodBull(bytes memory signature, uint256 bull1, uint256 bull2, uint256 bull3, uint256 bull4) external nonReentrant {
        require(
    ...
    function buyIncubator(bytes memory signature, uint256 bull1, uint256 bull2) external nonReentrant {
        require(
    ...
    function claimExtraBreeding(uint256 tokenId, uint256 godBull) external checkUpdate(godBull) {
        require(
    ...
    function buyMergeOrb(bytes memory signature, uint256 teen) external nonReentrant { 
        require(
            _verifyHashSignature(keccak256(abi.encode(msg.sender, teen)), signature),
            "Signature is invalid"
        );
    ...
    function refund(uint256 tokenId) external nonReentrant noZeroAddress(treasuryWallet) checkUpdate(tokenId) {
        require(
    ...
    function _claimMeth(uint256 tokenId) internal checkUpdate(tokenId) returns (uint256 amount) {
        require(
    ...
    function getClaimableMeth(uint256 tokenId, bool isGodBull) public view returns (uint256 methAmount) {
    ...
    function setGenesisContract(address _newAddress) external onlyOwner noZeroAddress(_newAddress) {
        bapGenesisAddr = _newAddress;
        bapGenesis = BAPGenesisInterface(bapGenesisAddr);
    }

    function setMethaneContract(address _newAddress) external onlyOwner noZeroAddress(_newAddress) {
        bapMethAddr = _newAddress;
        bapMeth = BAPMethaneInterface(bapMethAddr);
    }

    function setUtilitiesContract(address _newAddress) external onlyOwner noZeroAddress(_newAddress) {
        bapUtilitiesAddr = _newAddress;
        bapUtilities = BAPUtilitiesInterface(bapUtilitiesAddr);
    }

    function setTeenBullsContract(address _newAddress) external onlyOwner noZeroAddress(_newAddress) {
        bapTeenBullsAddr = _newAddress;
        bapTeenBulls = BAPTeenBullsInterface(bapTeenBullsAddr);
    }

    function setTreasuryWallet(address _newTreasuryWallet) external onlyOwner noZeroAddress(_newTreasuryWallet) {
        treasuryWallet = _newTreasuryWallet;
    }

    function setPrevOrchestrators(address _orchestratorV1, address _orchestratorV2) external onlyOwner noZeroAddress(_orchestratorV1) noZeroAddress(_orchestratorV2) {
        orchestratorV1 = _orchestratorV1;
        orchestratorV2 = _orchestratorV2;
    }
