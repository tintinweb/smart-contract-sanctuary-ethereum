...
contract UnlimitedNFT is ERC721, ERC721URIStorage, Ownable, ReentrancyGuard {
    ...
    constructor(address _fee) ERC721("Unlimited NFT", "UFT") {
        info.royalty = 10;
        info.adminFee = 25;
    ...
    function createItem(
        ...
        require(!_auctionable || _count == 1, "can mint only 1");
...

        {
            ...
            item.phrase = "";
            item.auction.startTime = _auctionable ? _startTime : 0;
            item.auction.endTime = _auctionable ? _endTime : 0;
            item.flags.mintable = true;
            item.flags.isSold = false;
            item.flags.auctionable = _auctionable;
        }
    ...
    //     require(isSupportedToken(_currency), "invalid payment currency");
    //     require(
    //         ERC721(_collection).ownerOf(_tokenId) == _msgSender(),
    //         "not owner"
    //     );
    ...
    //     {
    //         AuctionTime memory _auction;
    //         _auction.startTime = _auctionable ? _startTime : 0;
    //         _auction.endTime = _auctionable ? _endTime : 0;

    //         ItemFlags memory _flags;
    //         _flags.mintable = false;
    //         _flags.isSold = false;
    //         _flags.auctionable = _auctionable;
    ...
    function buyItem(uint256 _id, uint256 _amount)
        external
        payable
        nonReentrant
    {
        ...
        require(!item.flags.auctionable, "no auctionable");
        require(_amount == items[_id].price, "Below price");
        require(_amountToPay == msg.value, "invalid eth");

        // Process the payment
        _processPayment(_id, item.price);

        uint256 tokenId;
        if (item.flags.mintable) {
            _tokenIds.increment();
        ...
        nonReentrant
    {
        ...
        uint256 _amountToPay = _amount; //calcBidAmount(_id, _amount);

        require(item.flags.auctionable, "no auctionable");
        require(_amount == _amountToPay, "invalid eth");
        require(_amount > amount && _amount > item.price, "insufficient");
    ...
    // {
    //     Item storage item = items[_id];
    //     require(item.flags.auctionable, "Not an auctionable sale");
    //     require(!item.flags.isSold, "Item already sold");
    ...
    //     Item storage item = items[_id];
    //     require(!item.flags.auctionable, "Auctionable sale");
    //     require(!item.flags.isSold, "Item already sold");
    //     require(!item.flags.mintable, "Item is mintable");
     ...
     */
    function acceptWinningBid(uint256 _id)
        external
        onlyItemCreator(_id)
        nonReentrant
    {
        require(items[_id].flags.auctionable, "no auction");
        require(!items[_id].flags.isSold, "already sold");
        require(block.timestamp > items[_id].auction.endTime, "running now");
        ...
        if (bidsLength == 0) {
            ...
            // Process the payment
            uint256 amountToPay = _lastBid.amount; //calcBidAmount(_id, _lastBid.amount);
            _processPayment(_id, amountToPay);
    ...
    function _checkCanBuy(uint256 _id) internal view {
        ...
        require(!item.flags.isSold, "sold out");
        require(item.count > 0, "sold out");

        if (item.flags.auctionable) {
            require(block.timestamp >= item.auction.startTime, "not started");
            require(block.timestamp <= item.auction.endTime, "auction over");
        }

        if (!item.flags.mintable) {
            require(
                IERC721(item.collection).ownerOf(item.tokenId) == _msgSender(),
                "insufficient"
            );
        }

        require(item.creator == _msgSender(), "insufficient");
    }

    function _processPayment(uint256 _id, uint256 _amount) internal {
        Item memory item = items[_id];

        uint256 _total = _amount; //calcBidAmount(_id, _amount);
        uint256 _commissionValue = _total.mul(info.adminFee).div(1000);
        uint256 _royalties = _total
            .mul(
                item.collection == address(this) &&
                    _creators[item.tokenId] != address(0x0)
                    ? info.royalty
                    : 0
            )
            .div(1000);
        ...
        if (_commissionValue > 0)
            _safeTransferEth(info.feeAddress, _commissionValue);
        if (_royalties > 0)
            _safeTransferEth(_creators[item.tokenId], _royalties);
    }
    ...
    function _refundLastBid(uint256 _id) internal {
        ...
        if (bidsLength > 0) {
            ...
            if (
                _lastBid.isActive &&
                _lastBid.amount > 0 &&
                _lastBid.bidder != address(0)
            ) {
                uint256 amountToRefund = _lastBid.amount; //calcBidAmount(_id, _lastBid.amount);
                _safeTransferEth(_lastBid.bidder, amountToRefund);
                _lastBid.isActive = false;
            }
        }
    }

    function lastBid(uint256 _id)
        public
        view
        returns (
            address,
    ...
    // function supportedTokensLength() public view returns (uint256) {
    ...
    // function setFeePercent(uint256 _adminPercent, uint256 _royalty)
    //     external
    //     onlyOwner
    // {
    ...
    modifier onlyItemCreator(uint256 _id) {
        require(
            items[_id].creator == _msgSender() || owner() == _msgSender(),
            "only for item creator"
        );
        _;
    }
}
