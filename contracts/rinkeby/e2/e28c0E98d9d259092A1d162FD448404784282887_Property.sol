...
*/
    ...
    uint8[] traitVals = [10,8,2,4,2,3,3,4,4,
                         10,4,2,4,4,4,6,6,3,
                         10,5,2,3,4,4,4,1,1];
    ...
    function mintProperty(uint8 propType) public {
        ...
        // require this owner already has a banker
        require(IERC721(debtCityAddress).balanceOf(msg.sender) > 0, "You must own at least one banker to mint new property!");
    ...
    function applyPropRules(uint8 i, uint8 propType, uint8 randDigit) pure internal returns (uint8) {
        ...
        if (i == 6 && (randDigit > 5 || propType == 5 || propType == 4)) randDigit = 0;
        if (i == 7 && (propType == 3 || (propType == 4 && randDigit > 5))) randDigit = 0;
        if (i == 8 && (randDigit > 5 && propType > 4)) randDigit = 0;
    ...
    function getPropTitle(uint8 job) internal pure returns (string memory) {
    ...
    function outputFullSVG(string memory _hash) public view returns (string memory) {
        ...
        for (uint8 i = 0; i < numTraits; i++) {
            ...
            //if (thisTraitIndex != 0) 
            svgString = string(abi.encodePacked(svgString, drawTrait(traitData[i + adder][thisTraitIndex])));
        }
