...
contract rinkebyTest is ERC721A, Pausable, ReentrancyGuard, Ownable {
    ...
    bytes32 public ogMerkleRoot = 0xb925e25c93e0f3b4f1788dda1de423f0594a91627fb1dc6f1293c0f747977eeb;
        ...
        whenNotPaused
    {
        ...
        if (addressTokenMintData[msg.sender].maxMintsUpdated == false) {
            ...
            if (token1.balanceOf(msg.sender) > 0) {
                for (uint256 i = 0; i < token1.balanceOf(msg.sender); i++) {
            ...
            if (
                MerkleProof.verify(_merkleProof, ogMerkleRoot, leaf) == true &&
                token1.balanceOf(msg.sender) == 0
            ) {
                addressTokenMintData[msg.sender].maxOGMint += 3;
            }
            if (
                MerkleProof.verify(_merkleProof, wlMerkleRoot, leaf) == true &&
                token1.balanceOf(msg.sender) == 0
            ) {
                addressTokenMintData[msg.sender].maxWhitelistMint += 2;
            }
        ...
        returns (bool result)
    {
        ...
        if (
            MerkleProof.verify(_merkleProof, teamMerkleRoot, leaf) == true ||
            token1.balanceOf(msg.sender) > 0 ||
            MerkleProof.verify(_merkleProof, ogMerkleRoot, leaf) == true ||
            MerkleProof.verify(_merkleProof, wlMerkleRoot, leaf) == true ||
            token2.balanceOf(msg.sender) > 0
