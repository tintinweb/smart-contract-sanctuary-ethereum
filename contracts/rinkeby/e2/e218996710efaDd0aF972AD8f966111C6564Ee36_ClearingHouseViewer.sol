...
contract ClearingHouseViewer is IClearingHouseViewer {
    ...
    function marginRatio(
        ...
        (int256 unrealizedPositionPnl, int256 openNotional) = isTrader
            ? clearingHouse.perpetuals(idx).getTraderPositionHealth(account)
            : clearingHouse.perpetuals(idx).getLpPositionHealth(account);
    ...
    function getTraderFundingPayments(uint256 idx, address account) public view override returns (int256) {
        if (getTraderPosition(idx, account).openNotional == 0) {
            return 0;
        }
    ...
    function getLpFundingPayments(uint256 idx, address account) public view override returns (int256) {
        if (getLpPosition(idx, account).openNotional == 0) {
            return 0;
        }
