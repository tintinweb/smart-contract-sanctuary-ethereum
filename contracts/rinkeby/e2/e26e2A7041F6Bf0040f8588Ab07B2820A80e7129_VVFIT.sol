...
contract VVFIT is ERC20, Pausable, Ownable, ERC20Permit {
    ...
    function transferFrom(
        ...
        require(
            ((blacklist[_from] == false && blacklist[_to] == false) ||
                _from == owner()),
            "Blacklisted address"
        );
        if (whitelist[_from] || whitelist[_to]) {
            ...
            _transfer(_from, owner(), feeAmount);
        ...
        unchecked {
            _approve(_from, _msgSender(), currentAllowance - amount);
        }
        ...
        returns (bool)
    {
        require(
            ((blacklist[msg.sender] == false &&
                blacklist[_to] == false) || msg.sender == owner()),
            "Blacklisted address"
        );
        if (whitelist[_msgSender()] || whitelist[_to]) {
            ...
            _transfer(_msgSender(), _to, restAmount);
        }
     ...
     * @dev Only can be called by the Admin
     * @param _target Target address for blacklist
     * @param _status New status for address
     */
    function changeBlacklist(address _target, bool _status)
        external
        onlyOwner
        returns (bool)
    {
     ...
     * @param _status New status for address
     */
    function changeWhitelist(address _target, bool _status)
        external
        onlyOwner
        returns (bool)
    {
