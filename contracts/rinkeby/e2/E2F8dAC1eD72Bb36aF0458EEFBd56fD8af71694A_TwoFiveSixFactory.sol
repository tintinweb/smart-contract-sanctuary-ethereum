...
contract TwoFiveSixFactory is Ownable {
    ...
    address public masterRoyaltySplitter;

    address[] public projects;
    address[] public royaltySplitters;
    ...
    function createRoyaltySplitter(
        ...
        address a = clone(masterRoyaltySplitter);
    ...
    function setMasterRoyaltySplitter(address _masterRoyaltySplitter)
        public
        onlyOwner
    {
...
contract TwoFiveSixProject is ERC721EnumerableUpgradeable, OwnableUpgradeable {
    ...
    function showCaseMint(uint256[] calldata the256ArtIds) public onlyOwner {
        ...
        require(
            count == project.showCaseAmount,
            "Must mint the entire showcase"
        );
        require(
            project.showCaseAmount % 2 == 0,
            "Showcase must be even quantity."
        );
        ...
        payable
    {
        ...
        require(gmDaoSaleIsActive, "Member mint is not active.");
        require(count > 0, "Must mint at least one");
        require(totalSupply + count < project.maxSupply, "Exceeds max supply.");
        require(count < maxPerTx, "Exceeds max per transaction.");
        require(
        ...
        ERC721Upgradeable gmToken = ERC721Upgradeable(project.gmTokenAddress);
        ...
        for (uint256 i; i < count; i++) {
            ...
            bool unset = getGmTokenUsed(gmTokenIds[i]);

            require(unset, "Gm token already used");
    ...
    function withdraw() public onlyOwner {
        ...
        project.twoFiveSixFundsAddress.transfer((balance / 10000) * 1000);
        project.artistAddress.transfer((balance / 10000) * 9000);
    }
...
contract RoyaltySplitter is Initializable {
    ...
    function withdraw() public {
        require(
            ...
            "Not artist or 256ART"
        );
        require(
            address(this).balance > 100000000000000000,
            "Balance is < 0.1 ETH"
        );
        uint256 balance = address(this).balance;
