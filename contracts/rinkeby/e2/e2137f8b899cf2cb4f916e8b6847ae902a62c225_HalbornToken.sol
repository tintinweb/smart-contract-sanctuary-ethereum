...
contract HalbornToken is ERC20{
    ...
    function newTimeLock(uint256 timelockedTokens_, uint256 vestTime_, uint256 cliffTime_, uint256 disbursementPeriod_)
        public
    {
        // * in general, the following checks does not comply with the flow rules.

        // * should check the tokens to ==  100.
        // * require(timelockedTokens_ == 100_000000000000000000000, "token should be 100.");
        ...
        // * this should be equal to 1 year in millseconds.
        // * require(disbursementPeriod_ == 365 days, "Cannot have disbursement period of 0");
        require(disbursementPeriod_ > 0, "Cannot have disbursement period of 0");

        // * there is no need for the user to supply this paramter at all
        // * and so, this require statment can be removed and replace it
        // * vestTime_ = block.timestamp
        require(vestTime_ > block.timestamp, "vesting start must be in the future");

        // * the cliff time should be 6 months from, meaning that it should be
        // * cliffTime = block.timestamp + 180 days (assmuing a month has 30 days.)
        // * no need to have the user supply this paramter as well.
        ...
        vestTime[msg.sender] = vestTime_; // * replace with vestTime[msg.sender] = block.timestamp
        cliffTime[msg.sender] = cliffTime_; // * replace with cliffTime[msg.sender] = block.timestamp + 180 days
    ...
    function _beforeTokenTransfer(
        ...
        // ! ------------ FINDING --------------- ! //
        // ! the function should check if the amount to
        // ! be sent is smaller than or equal the MaxTransferrable.

        // * possible fix: 
        // * if (from != address(0))
        // *    require(amount <= maxTokens, "cannot transfer more than the maxTransferrable");
        ...
        returns (uint256)
    {
        if(timelockedTokens[who] == 0){
            // if no tokens lock, then return balance.
            return balanceOf(who);
        }

        uint256 maxTokens;

        // * if the cliff time is still in the future we set maxTokens to 0 
        // * if the vestTime is in the future, we set the maxTokens to 0
        if( vestTime[who] > block.timestamp || cliffTime[who] > block.timestamp){
            maxTokens = 0;
            // ? According to the flow, wouldn't the vest time ALWAYS be less than the current
            // ? block.timestamp ?
            // ? since we only reach this branch if the user locked some tokens.
            // ? meaning that the vestTime[who] is already in the past.
        } else {
            maxTokens = timelockedTokens[who] * (block.timestamp - vestTime[who]) / disbursementPeriod[who];
        }
    
        if (timelockedTokens[who] < maxTokens){
            // * in case we transfer AFTER the disbursementPeriod
          return balanceOf(who);
        }
        // * get me the total tokens i own - the ones locked + the ones i can spend from the locked.
    ...
    function setSigner(address _newSigner) public {
        // ! --------------------- FINIDING  ----------------------- ! //
        // ! this function allows ANYONE except the CURRENT to change
  ...
  function mintTokensWithSignature(uint256 amount, bytes32 _r, bytes32 _s, uint8 _v) public {
    ...
    // function getSignature(uint256 amount) public view returns (bytes32){
    //     bytes memory prefix = "\x19Ethereum Signed Message:\n32";
    //     bytes32 messageHash = keccak256(
