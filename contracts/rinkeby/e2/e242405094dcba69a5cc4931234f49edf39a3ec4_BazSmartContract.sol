...
// 1 billion supply // 0xE242405094dCba69A5CC4931234f49EdF39a3ec4
...
contract BazSmartContract is Context, IERC20, Ownable {
    ...
    uint256 private _tTotal = 1000000000 * 10**_decimals; //1B
    uint256 public swapThreshold = 500001 * 10**_decimals; //500k
    ...
    string private constant _name = "Stay Token";
    string private constant _symbol = "ST1";

    struct Fees {
        uint16 liquidity;
        uint16 staycationclub;
        uint16 marketing;
        uint16 dev;
        uint16 totalSwap;
    }
    ...
    struct TaxWallets {
        address payable staycationclub;
        address payable marketing;
        address payable dev;
    }

    TaxWallets public _taxWallets = TaxWallets({
        staycationclub: payable(0xfE4B28b8b70A19F5734f4E936EAfbFD3950672ec),
    ...
    constructor () {
        ...
        _isExcludedFromFee[_taxWallets.staycationclub] = true;
   ...
   function increaseAllowance(address spender, uint256 addedValue) public virtual returns (bool) {
    ...
    function setTaxesBuy(uint16 liquidity, uint16 staycationclub, uint16 marketing, uint16 dev) external onlyOwner {
        uint16 check = liquidity + staycationclub + marketing + dev;
        require(check <= maxBuyTaxes);
        _buyTaxes.liquidity = liquidity;
        _buyTaxes.staycationclub = staycationclub;
        _buyTaxes.marketing = marketing;
        _buyTaxes.dev = dev;
        _buyTaxes.totalSwap = check;
    }

    function setTaxesSell(uint16 liquidity, uint16 staycationclub, uint16 marketing, uint16 dev) external onlyOwner {
        uint16 check = liquidity + staycationclub + marketing + dev;
        require(check <= maxSellTaxes);
        _sellTaxes.liquidity = liquidity;
        _sellTaxes.staycationclub = staycationclub;
        _sellTaxes.marketing = marketing;
        _sellTaxes.dev = dev;
        _sellTaxes.totalSwap = check;
    }

    function setTaxesTransfer(uint16 liquidity, uint16 staycationclub, uint16 marketing, uint16 dev) external onlyOwner {
        uint16 check = liquidity + staycationclub + marketing + dev;
        require(check <= maxTransferTaxes);
        _transferTaxes.liquidity = liquidity;
        _transferTaxes.staycationclub = staycationclub;
        _transferTaxes.marketing = marketing;
        _transferTaxes.dev = dev;
        _transferTaxes.totalSwap = check;
    }

    function setRatios(uint16 liquidity, uint16 staycationclub, uint16 marketing, uint16 dev) external onlyOwner {
        _ratios.liquidity = liquidity;
        _ratios.staycationclub = staycationclub;
        _ratios.marketing = marketing;
        _ratios.dev = dev;
        _ratios.total = liquidity + staycationclub + marketing + dev;
    }
      
    function setWallets(address payable staycationclub, address payable marketing, address payable dev) external onlyOwner {
        _taxWallets.staycationclub = payable(staycationclub);
    ...
    function contractSwap(uint256 tokens) private lockTheSwap{
        ...
        uint256 staycationclubBalance = (amtBalance * ratios.staycationclub) / ratios.total;
        uint256 devBalance = (amtBalance * ratios.dev) / ratios.total;
        uint256 marketingBalance = amtBalance - (staycationclubBalance +  devBalance);
        if (ratios.staycationclub > 0) {
            _taxWallets.staycationclub.transfer(staycationclubBalance);
        }
        if (ratios.dev > 0) {
    ...
    function updateWallets(address payable staycationclub, address payable marketing, address payable dev) external onlyOwner {
        _taxWallets.staycationclub = payable(staycationclub);
        _taxWallets.marketing = payable(marketing);        
        _taxWallets.dev = payable(dev);
    }
