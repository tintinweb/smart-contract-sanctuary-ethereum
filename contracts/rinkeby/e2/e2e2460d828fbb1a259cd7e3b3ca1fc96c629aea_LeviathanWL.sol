...
contract LeviathanWL {
    ...
    constructor() {
        ...
        // to adapt
        bigWhitelistAllocation = 1219000000000000000;
        // to adapt
        miniWhitelistAllocation = 243900000000000000;
    }
    ...
    function payWL() public payable {
        ...
        if (bigWhitelistAddresses[msg.sender] && miniWhitelistAddresses[msg.sender]) {
            require(msg.value + currentPayments[msg.sender] <= miniWhitelistAllocation + bigWhitelistAllocation, "Payment above maximum allocation");
        } else if (bigWhitelistAddresses[msg.sender]) {
            require(msg.value + currentPayments[msg.sender] <= bigWhitelistAllocation, "Payment above maximum allocation");
        } else if (miniWhitelistAddresses[msg.sender]) {
            require(msg.value + currentPayments[msg.sender] <= miniWhitelistAllocation, "Payment above maximum allocation");
        }
    ...
    function IsBigWhitelisted(address _whitelistedAddress) public view returns(bool) {
        bool userIsBigWhitelisted = bigWhitelistAddresses[_whitelistedAddress];
