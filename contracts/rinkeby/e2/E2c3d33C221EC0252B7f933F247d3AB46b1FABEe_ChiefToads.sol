...
contract ChiefToads is ERC721A, VRFConsumerBaseV2, Ownable {
    ...
    function communityMint(
        ...
        require(
            totalSupply() + _mintAmount <= communityMintMaxSupply,
    ...
    function whitelistMint(
        ...
        require(
            ...
            "ChiefToads: Wallet has already minted Max Amount for Private Mint 2!"
        );
    ...
    function rizardMint(
        ...
        require(stage == 3, "ChiefToads: Rizard Mint is not open");
    ...
    function publicMint(uint256 _mintAmount) external payable onlyEOA {
        ...
        require(stage == 4, "ChiefToads: Public Sale Closed!");
    ...
    function whitelistSigned(
        ...
        require(stage > 0, "Minting has not started");
        // Community Mint
        if (stage == 1) {
        ...
        // Rizard Mint (which includes whitelist mint)
        else if (stage == 3) {
            bytes32 hash = keccak256(abi.encodePacked(sender, nonce));
            return
                rizardSignerAddress == hash.recover(signature) ||
                whitelistSignerAddress == hash.recover(signature);
        }
        return false;
    }
        ...
        return
            bytes(baseURI).length != 0
                ? string(
                    abi.encodePacked(
                        baseURI,
                        ((tokenId + shift) % totalMaxSupply).toString()
                    )
                )
                : "";
    }
