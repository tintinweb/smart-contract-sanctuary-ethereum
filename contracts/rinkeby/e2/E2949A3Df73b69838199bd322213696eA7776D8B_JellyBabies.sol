...
//    // Unpaired eggz go brrrrrr
//    function findDaEggzzz(address whomst) public view returns (uint256[] memory) {
//        uint256 whatsDaNumburrr = cbeContract.balanceOf(whomst);
//        uint256[] memory wichWunzz = new uint256[](whatsDaNumburrr);
//
//        uint256 writePos = 0;
//        for (uint256 i = 0; i < whatsDaNumburrr; i++) {
//            uint256 eggByIndex = cbeContract.tokenOfOwnerByIndex(whomst, i);
//            if (!hazCompanion(eggByIndex)) {
//                wichWunzz[writePos] = eggByIndex;
//                writePos++;
//            }
//        }
//
//        if (writePos == whatsDaNumburrr) {
//            return wichWunzz;
//        }
//        else {
//            uint256[] memory aloneEggz = new uint256[](writePos);
//
//            for (uint256 i = 0; i < writePos; i++) {
//                aloneEggz[i] = wichWunzz[i];
//            }
//
//            return aloneEggz;
//        }
...
//            require((companionFound[curBlocc] >> eggSlot)%2 == 0, 'c');
//            require(cbeContract.ownerOf(eggz[i]) == msg.sender);
