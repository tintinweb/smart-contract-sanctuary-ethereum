...
contract NftMinter is ERC721Enumerable, Ownable {
    ...
    mapping(uint256 => bool) internal revealedId;
    mapping(address => uint8) internal whitelistedNftMinted;
    mapping(address => uint8) internal reserveNftMinted;
        ...
        returns (string memory)
    {
        ...
        if (revealedId[tokenId] == true || revealed == true) {
    ...
    function whitelistMint(bytes32[] memory proof,uint _amount) public payable{
        require(block.timestamp < whitelistMintPeriod,"Minting period over");
        require(isValid(proof, keccak256(abi.encodePacked(msg.sender)),whitelistRoot), "Not a part of Allowlist");
        require(!emergencyStop, "Contract is paused");
        require(_amount > 0, "Value less than 0");
        require(_amount <= maxAmount, "Exceeded max amount");
        uint256 supply = totalSupply();
        require(_amount + supply <= 4500, "Exceeded max supply");
        require(whitelistedNftMinted[msg.sender] <= 2,"Exceeded max allowed mint");
        if (msg.sender != owner()) {
            require(
                msg.value >= cost * _amount,
                "Amount is less then nft price"
    ...
    //reserveRoot
    function reserveMint(bytes32[] memory proof,uint _amount) public {
        require(isValid(proof, keccak256(abi.encodePacked(msg.sender)),reserveRoot), "Not a part of Allowlist");
        require(!emergencyStop, "Contract is paused");
        require(_amount > 0, "Value less than 0");
        require(_amount <= maxAmount, "Exceeded max amount");
        uint256 supply = totalSupply();
    ...
    function isValid(bytes32[] memory proof, bytes32 leaf, bytes32 root) public pure returns (bool) {
    ...
    function revealId(uint256 _tokenId) public onlyOwner {
        revealedId[_tokenId] = true;
    }
    ...
    // function setPublicMintPeriod(uint _timePeriod) public onlyOwner {
    //     publicMintPeriod = block.timestamp.add(_timePeriod);
    // }
