...
contract Xenia_Land is ERC721P, Ownable {
  using ECDSA for bytes32;

  address public signer = 0x3F91aC3cAe19a1966d0d30cd4C1eaD80714b0673;
  ...
  function allowMint(uint256 _amount, uint256 _landAllowance, bool _startStaking, bytes memory _signature) external payable {
    ...
    require (landMints[msg.sender] + _amount <= _landAllowance, "Exceeds allowance" );
    ...
    unchecked { landMints[msg.sender] += _amount; }
  ...
  function saleMint(uint256 _amount, bool _startStaking) external payable {
    ...
    require (_amount <= remaining(), "Max land supply reached");
    require (msg.value == _amount * landPrice, "Wrong amount sent");
    require (landMints[msg.sender] + _amount <= landMax, "Exceeds allowance" );
    require (msg.sender == tx.origin, "bm8gcm9ib3Rz");
  ...
  function hashTransaction(address _sender, uint256 _landAllowance) public pure returns (bytes32 _hash) {
