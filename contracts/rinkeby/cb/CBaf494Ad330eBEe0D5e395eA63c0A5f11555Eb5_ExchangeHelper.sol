...
import { LibBp } from '../../libraries/LibBp.sol';
...
contract ExchangeHelper is Ownable, ExchangeAutoProxy, IExchangeHelper {
    ...
    constructor(address _proxy) ExchangeAutoProxy(_proxy, address(this)) {}
    ...
    // LibBp
    function bp(uint256 value, uint256 bpValue) external pure override returns (uint256) {
        return LibBp.bp(value, bpValue);
    }
    ...
    function subFeeInBp(
        ...
        return LibExchange.subFeeInBp(rest, total, feeInBp);
    }
...
library LibExchange {
    ...
    function subFeeInBp(
        ...
        uint256 _fee = LibBp.bp(_total, _feeInBp); // Calculate fee
        ...
        returns (uint256 _total)
    {
        ...
        for (uint256 i = 0; i < _orderOriginFees.length; i++) {
            _total = _total.add(LibBp.bp(_amount, _orderOriginFees[i].value));
        }
    }
