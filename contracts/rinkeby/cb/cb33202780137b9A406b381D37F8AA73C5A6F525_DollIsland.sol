...
contract DollIsland is Ownable, ERC721A {
  ...
  function presaleMint(
    ...
    require(msg.value >= saleConfig.presalePrice * quantity);
  ...
  function publicMint(uint256 quantity) external payable noContract {
    ...
    require(totalSupply() + quantity <= collectionSize, "max supply reached");
    require(msg.value >= saleConfig.publicPrice * quantity);

    super._safeMint(msg.sender, quantity);
  }

  function verifySignature(uint256 nonce, bytes calldata signature) internal view returns (bool) {
    require(nonce > userToUsedNonce[_msgSender()], "nonce has already been used");
    address recoveredAddress = keccak256(abi.encodePacked(_msgSender(), nonce)).toEthSignedMessageHash().recover(signature);
