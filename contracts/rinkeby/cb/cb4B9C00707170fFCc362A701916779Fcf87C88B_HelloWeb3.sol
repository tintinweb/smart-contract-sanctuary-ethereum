	...
	LockedPaymentSplitter
{
	...
	mapping(uint256 => uint256) public _maxSupplies;
	mapping(uint256 => uint256) public _maxBatchSizes;
	...
	function _isCorrectAmountProvided(uint256 id, uint256 quantity)
		...
		view
	{
		uint256 totalCost = quantity * _costs[id];
	...
	function _isLessThanMaxSupply(uint256 id, uint256 quantity) internal view {
		if (totalSupply(id) + quantity > _maxSupplies[id]) {
	 ...
	 * @param id is the token id to mint
	...
	function _internalMintTokens(
		...
		_mintBatchTokens(minter, id, quantity);
	}
	...
	function reserveSingle(
		...
		uint256 multiple = quantity / _maxBatchSizes[id];
		for (uint256 i = 0; i < multiple; i++) {
			_internalMintTokens(fren, id, _maxBatchSizes[id]);
		}
		uint256 remainder = quantity % _maxBatchSizes[id];
		if (remainder != 0) {
		...
		onlyUsers
	{
		...
		if (quantity > 1) {
			_mintBatchTokens(_msgSender(), id, quantity);
		} else {
	 ...
	 * @param cost new cost to associate with minting tokens (in wei)
	 */
	function setMintCost(uint256 id, uint256 cost) public onlyOwner {
		_isNotFree(cost);
		uint256 currentCost = _costs[id];
		_costs[id] = cost; // in wei
		emit CostUpdated(currentCost, cost);
	}
