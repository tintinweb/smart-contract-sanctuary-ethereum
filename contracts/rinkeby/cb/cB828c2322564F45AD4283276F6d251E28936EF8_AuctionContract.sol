...
contract AuctionContract {
  ...
  event BidSuccess(uint256 tokenId, address bidder, uint256 price);
  event withdrawSuccess(address signer, uint256 price);
  event AuctionEndSuccess(uint256 tokenId, address winner, uint256 amount);
  ...
  function openAuction(
    ...
    emit OpenAuctionSuccess(tokenId, msg.sender, biddingDuration, minimumPrice);
  }
  ...
  function bid(uint256 tokenId) external payable {
    ...
    emit BidSuccess(tokenId, msg.sender, msg.value);
  }
