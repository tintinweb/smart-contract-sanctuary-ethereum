...
contract MasterContract is AccessControl, IMaster, ERC721 {
  ...
  constructor(
    ...
    address[] memory rounds
      ...
      for(uint8 round = uint8(Round.Legendary); round <= uint8(Round.Public); round++ ) {
        ...
        _setupRole(ROUND_ROLE, rounds[round]);
      }
      _setupRole(ROLE_ADMIN, OWNER_ADDRESS);
  }

  function setMaxSupply(uint16 maxSupply_) external onlyRole(ROLE_ADMIN) {
    _maxSupply = maxSupply_;
  ...
  function mint(uint[] memory tokenIdxs) override(IMaster) external onlyRole(ROUND_ROLE) {
    ...
    for (uint256 i = 0; i < tokenIdxs.length; i++) {
      ...
      _safeMint(msg.sender, tokenIdxs[i]);
      emit MintRand(msg.sender, tokenIdxs[i]);
    }
  }
}
...
contract RoundContract is AccessControl {
  ...
  function mintTokens(uint nTokens) public payable inWhitelist returns(uint[] memory) {
...
interface IMaster {
  ...
  function mint(uint[] memory tokenIdxs) external;
