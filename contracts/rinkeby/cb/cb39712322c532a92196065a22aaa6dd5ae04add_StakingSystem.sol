...
contract StakingSystem is Ownable, ERC721Holder {
    ...
    event SystemUnstakedByProductId(address user, uint256 productId);
    /// @notice event emitted when a user token unstaked by system
    event SystemUnStaked(address user, uint256 tokenId);
    ...
    function stakeBatch(uint256 _productId, uint256[] memory _tokenIds) public {
        require(!_hasStakeProduct(_productId, msg.sender), "Stake product has been staked");
        ...
        for (uint256 i = 0; i < _tokenIds.length; i++) {
            _stake(msg.sender, _productId, _tokenIds[i]);
        }
    }
    ...
    function unstakeBatch(uint256[] memory _tokenIds) public {
        for (uint256 i = 0; i < _tokenIds.length; i++) {
    ...
    function _unstake(address _user, uint256 _tokenId) internal {
        ...
        uint256 _productId = _extractTokenProductId(_tokenId);
    ...
    function _extractTokenProductId(uint256 _tokenId) internal view returns (uint256) {
        if (tokenOwner[_tokenId].user != address(0x0)) {
            return tokenOwner[_tokenId].productId;
        }
        return 0;
    }
    ...
    function systemUnstakeByProductId(address _user, uint256 _productId) public onlyOwner {
        ...
        for (uint256 i = 0; i < staker.tokenIds.length; i++) {
            _systemUnstake(staker.tokenIds[i]);
            staker.balance--;
        }

        emit SystemUnstakedByProductId(_user, _productId);
        delete stakers[_productId][_user];
    }

    function _systemUnstake(uint256 _tokenId) internal {
        OwnerInfo storage _ownerInfo = tokenOwner[_tokenId];
        require(_ownerInfo.user != address(0x0), "_systemUnstake Invalid user address error");

        delete tokenOwner[_tokenId];
        nft.safeTransferFrom(address(this), _ownerInfo.user, _tokenId);
        emit SystemUnStaked(_ownerInfo.user, _tokenId);
        stakedTotal--;
    }
}
