...
contract NFTMinter is ERC721URIStorage, RoyaltiesV2Impl {
    using Address for address;
    ...
    uint256 public maxMintAmount = 20; // see  this
    ...
    uint256 private commonCost = 0.1 ether; // Black
    uint256 private uncommonCost = 0.4 ether; // Purple
    uint256 private rareCost = 0.7 ether; // White
    uint256 private epicCost = 1 ether; // Red
    uint256 private legendaryCost = 1.65 ether; // Yellow

    // event ChangeRoyaltyInfo(uint256 _id, address _from, address _to);
    constructor(string memory _initBaseURI) ERC721("Nakashi Manga", "NM") {
        OWNER = payable(_msgSender());
        setBaseURI(_initBaseURI);
        setUnityDetails("common", commonCost, 225);
        setUnityDetails("uncommon", uncommonCost, 150);
        setUnityDetails("rare", rareCost, 75);
        setUnityDetails("epic", epicCost, 35);
        setUnityDetails("legendary", legendaryCost, 15);
    }
    ...
    event SetBaseExtension(string extension);
    ...
    modifier check(string memory unityName) {
        ...
        require(
            _tokenIds.current() <= maxMintSupply,
            "Check: Maximum NFT's minted!"
        );
        require(
            packDetail.itemsMinted <= packDetail.maxSupplyOfNFT,
            "Check: Max supply minted of the particular NFT!"
        );
        require(
            msg.value == packDetail.cost,
            "Check: Need to add more funds to your account!"
        );
        _;
    }
    ...
    function mint(string memory unityName, string memory metadataURL)
        public
        payable
        check(unityName)
        returns (uint256)
    {
        ...
        unityDetails[unityName].tokenId.push(newItemId);

        emit NFTMinted(_msgSender(), newItemId, unityName, msg.value);
    ...
    function unlockNFT(uint256 tokenId) public payable onlyOwner {
        locked[tokenId] = false;
        emit UnlockNFT(tokenId);
    }

    function lockNFT(uint256 tokenId) public payable onlyOwner {
        locked[tokenId] = true;
    ...
    function setUnityDetails(
        ...
        unityDetails[unityName].cost = _cost;
        unityDetails[unityName].maxSupplyOfNFT = _maxSupplyOfNFT;

        emit SetUnityDetails(unityName, _cost, _maxSupplyOfNFT);
    }
    ...
    function updateRoyaltyInfo(
        ...
        require(
            ERC721.ownerOf(tokenId_) == from_,
            "ERC721: transfer from incorrect owner"
        );
        _updateAccount(tokenId_, from_, to_);
        emit ChangeRoyaltyInfo(tokenId_, from_, to_);
    }

    function fetchMintedNFTs() public view returns (uint256[] memory) {
        uint256 totalItemCount = _tokenIds.current();
        uint256 itemCount = 0;
        uint256 currentIndex = 0;

        for (uint256 i = 0; i < totalItemCount; i++) {
            if (ERC721.ownerOf(i + 1) == _msgSender()) {
                itemCount += 1;
            }
        }

        uint256[] memory nftsMinted = new uint256[](itemCount);
        for (uint256 i = 0; i < totalItemCount; i++) {
            if (ERC721.ownerOf(i + 1) == _msgSender()) {
                nftsMinted[currentIndex] = i + 1;
                currentIndex++;
            }
        }
        return nftsMinted;
