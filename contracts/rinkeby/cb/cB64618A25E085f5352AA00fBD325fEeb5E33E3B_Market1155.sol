...
contract Market1155 is ERC20, Ownable, IERC1155Receiver, Pausable {
    /**
     * @notice Whether or not new loans are being issued, rollovers allowed
     */
    bool public newActivityAllowed;
    ...
    /**
     * @notice Address of contract administrator to call emergency functions.
     */
     ...
     */
    address public immutable tnftAddress;
     ...
     */
    uint256 public constant INITIAL_EXCHANGE_RATE_MANTISSA = 0.2 * 1e28;
    ...
    /**
     * @notice Event emitted when owner externally updates a state variable
     */
    event StateUpdated(string name, uint256 newVal);

    /**
     * @dev Throws error if called by any account other than the admin.
     */
    modifier onlyAdmin() {
        require(contractAdmin == msg.sender, "Unauthorized");
        _;
    }

    /**
     * @dev throws error when new newActivityAllowed is set to false
     */
    modifier whenNewActivityAllowed() {
        require(newActivityAllowed, "New issuing paused");
        _;
    }
    ...
    function _setNewActivityAllowed(bool _newVal) external onlyOwner {
        ...
        emit StateUpdated("newActivityAllowed", _newVal ? 1 : 0);
    }

    /**
     * @notice Setter function for termLengthDays
     */
    function _setTermLengthDays(uint256 _newVal) external onlyOwner {
        termLengthDays = _newVal;
        emit StateUpdated("termLengthDays", _newVal);
    }

    /**
     * @notice Setter function for reserveFactor in Percent (0-100%)
     */
    function _setReserveFactorMantissaPct(uint256 _newVal) external onlyOwner {
        reserveFactorMantissa = _newVal * 1e16;
        emit StateUpdated("reserveFactorMantissa", reserveFactorMantissa);
    }

    /**
     * @notice Setter functions for lend value function parameters
     * @param _baseLendValue new baseLendValue
     * @param _lendUtilMultiplier new lendUtilMultiplier
     * @param _maxLendValue new maxLendValue
     */
    function _setLendValueParams(
        uint256 _baseLendValue,
        uint256 _lendUtilMultiplier,
        uint256 _maxLendValue
    ) external onlyOwner {
        baseLendValue = _baseLendValue;
        lendUtilMultiplier = _lendUtilMultiplier;
        maxLendValue = _maxLendValue;
        emit StateUpdated("baseLendValue", _baseLendValue);
        emit StateUpdated("lendUtilMultiplier", _lendUtilMultiplier);
        emit StateUpdated("maxLendValue", _maxLendValue);
    }

    /**
     * @notice Setter functions for _maxLendValue param
     * @dev Emergency fn. needs to remain outside of timelock for emergency market scenarios
     * @param _maxLendValue new maxLendValue
     */
    function _setMaxLendValue(uint256 _maxLendValue) external onlyAdmin {
        maxLendValue = _maxLendValue;
        emit StateUpdated("maxLendValue", _maxLendValue);
    }
    ...
    function _setInterestRateParams(
        ...
        jumpMultiplierPerBlock =
            ...
            uint256(2102400);
        kink = kinkPct_ * 1e16;
        emit StateUpdated("multiplierPerBlock", multiplierPerBlock);
        emit StateUpdated("baseRatePerBlock", baseRatePerBlock);
        emit StateUpdated("jumpMultiplierPerBlock", jumpMultiplierPerBlock);
        emit StateUpdated("kink", kink);
    }

    /**
     * @dev Transfers adminstratorship of the contract to a new account (`newAdmin`).
     */
    function _changeAdmin(address newAdmin) external onlyOwner {
        contractAdmin = newAdmin;
    }

    ////// CORE INTERFACE FUNCTIONS
    ////// Borrower functions: takeLoan, repayLoan, rollover, foreclose
    ////// LP functions: addLiqudity, removeLiquidity
    /**
    ...
    {
        //check if requested loan amount is within cap
        require(
        ...
        //transfer over the collateral NFT
        IERC1155 collateralNft = IERC1155(collectionAddress);
     ...
     */
    function repayLoan(uint256 tnftId) external payable whenNotPaused {
        // fetch the debt NFT
        TNFT debtNft = TNFT(tnftAddress);
        // check if debtNFT belongs to sender
        require(debtNft.ownerOf(tnftId) == msg.sender, "Invalid owner");
        // check if this is the correct market for debt NFT
        require(
            collectionAddress == debtNft.getCollectionAddress(tnftId),
            "Incorrect Market"
        );
        // check if user adequately paid debt balance
        require(
        ...
        // update state variables to represent collected interest
        _onInterestCollected((msg.value - principal));
        // reduce total borrows to show closed position
        totalBorrows -= principal;
        // burn the debt NFT
        debtNft.burn(tnftId);
        // return the collateral NFT
        IERC1155 collateralNft = IERC1155(collectionAddress);
        collateralNft.safeTransferFrom(
            address(this),
            msg.sender,
        ...
        whenNewActivityAllowed
    {
        ...
        uint256 principal = debtNft.getPrincipal(tnftId);
        //check if post rollover amount is less than principal and current lend value
        require(
        ...
        //interest due on new loan
        uint256 newInterest = getInterestDue(
            requestedLoanAmount,
            termLengthDays
        );
        ...
        //burn old NFT
        debtNft.burn(tnftId);
        //reduce the amount in totalBorrows
        totalBorrows -= (principal - requestedLoanAmount);
        //contract collected interest for previous debt position
        _onInterestCollected(interest);

        // issue a new NFT signifying, new debt position
        debtNft.mint(
            msg.sender,
            collectionAddress,
            debtNft.getCollectionTokenId(tnftId),
    ...
    function foreclose(uint256 tnftId) external payable whenNotPaused {
        ...
        // check if paid value is greater than current auction price
        require(
        ...
        // transfer over the NFT to sender
        IERC1155 collateralNft = IERC1155(collectionAddress);
        collateralNft.safeTransferFrom(
            address(this),
            msg.sender,
    ...
    function addLiquidity() external payable whenNotPaused {
        // calculate LP tokens based on current exchange rate
     ...
     */
    function removeLiquidity(uint256 tTokenAmount) external whenNotPaused {
    ...
    ////// ALL OTHER HELPER AND ADMIN FUNCTIONS
    /**
     ...
     */
    function _withdrawContractRevenue(uint256 amount, address receiver)
        external
        onlyOwner
    {
     ...
     */
    function getUsableCash() private view returns (uint256) {
     ...
     * @dev Emergency fn. needs to remain outside of timelock
     */
    function _releaseWithheldInterest() external onlyAdmin {
        interestWithheld = 0;
    ...
    function getCurrentLendValue() public view returns (uint256) {
        ...
        uint256 lendVal = baseLendValue +
            ((oneMinusUtilization * lendUtilMultiplier) / 1e18);
        ...
        returns (uint256)
    {
        ...
        // 5760 = 2102400/365; blocks per day
     ...
     */
    function getCashPrior() private view returns (uint256) {
    ...
    /**
     * @notice Internal functions to update relevant state vars whenever interest is collected
     * @param amt Interest amount collected
     */
    function _onInterestCollected(uint256 amt) private {
        uint256 reserveContribution = ((amt * reserveFactorMantissa) / 1e18);
        totalReserves += reserveContribution;
        interestWithheld += (amt - reserveContribution);
    }
...
contract TNFT is IERC721Metadata, ERC721Enumerable, Ownable {
    ...
    string internal constant BASE_URI = "https://tangelo.finance/tnft/";
     ...
     */
    uint256 public auctionStartingMultiplierPercent = 100;

    /**
     * @notice daily price drop percent
     */
    uint256 public auctionDiscountFactorPercent;

    /**
     * @notice number of days foreclosure is disallowed after termLength ends
     */
    uint256 public graceDays;
     ...
     */
    function _setAuctionDiscountFactorPercent(uint256 _newVal)
        external
        onlyOwner
    {
     ...
     */
    function _setAuctionStartingMultiplierPercent(uint256 _newVal)
        external
        onlyOwner
    {
     ...
     */
    function _setGraceDays(uint256 _newVal) external onlyOwner {
        graceDays = _newVal;
    }
