...
contract MATICALPHA {
	...
	uint256 constant public TIME_STEP = 30; //30 seconds for testing
	...
	function getUserPlanBalance(address userAddress, uint256 _plan) public view returns (uint256) {
		...
		for (uint256 i = 0; i < user.deposits.length; i++) {
			if (user.deposits[i].plan == _plan) {
				uint256 planStart = user.deposits[i].start; //plan start time
				uint256 currentTime = 0;
				if(user.deposits[i].finish > block.timestamp) {
					currentTime = block.timestamp;
				}
				else {
					currentTime = user.deposits[i].finish;
				}
	...
	function getTotalPlanBalance(address userAddress) public view returns (uint256) {
		...
		for(uint256 i=0; i<5; i++) {
			planBalance = planBalance.add(getUserPlanBalance(userAddress, i));
		}
	...
	function getTotalHoldBalance(address userAddress) public view returns (uint256) {
		...
		for(uint256 i=0; i<5; i++) {
			holdBalance = holdBalance.add(getUserHoldBalance(userAddress, i));
		}
	...
	function getUserTotalWithdrawn(address userAddress) public view returns(uint256) {
		return users[msg.sender].withdrawn;
	}
