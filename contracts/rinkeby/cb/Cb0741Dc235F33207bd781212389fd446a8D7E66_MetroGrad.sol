...
contract MetroGrad is ERC721A, Ownable, ReentrancyGuard{
    ...
    constructor()
        ERC721A("MetroGrad Gen.1", "Survivor")
    {
        baseURI;
    ...
    function presaleMint(uint8 _amount) external payable callerIsUser {
        ...
        if(_numberMinted(msg.sender) + _amount > maxMintAmount) revert MaxPerWalletExceeded();
        if(msg.value != presaleCost * _amount
    ...
    function mint(uint8 _amount) external payable callerIsUser {
        ...
        if(msg.value != publicCost * _amount
            ) revert InsufficientValue();
    ...
    function mintUnlimited(uint8 _amount) external callerIsUser {
        if(
            totalSupply() + _amount > maxSupply
        ) revert MaxSupplyExceeded();
    ...
    function airDrop(address[] memory targets) external onlyOwner {
        if (targets.length + reserveLimit > 25)
            revert MaxSupplyExceeded();
    ...
    function setAllowlistAddresses(address[] calldata _users) external onlyOwner {
    ...
    function withdraw() external onlyOwner nonReentrant {
        ...
        payable(0xd346F04605cf0c65CDcc6368ABf82Bc1B646d381).transfer(payoutSelHelp);
