...
contract Fiat24Account is ERC721EnumerableUpgradeable, ERC721PausableUpgradeable, AccessControlUpgradeable {
    ...
    function mint(address _to, uint256 _tokenId, bool _isMerchant, uint256 _merchantRate) public {
        require(hasRole(OPERATOR_ROLE, msg.sender) || hasRole(MINTER_ROLE, msg.sender), "Fiat24Account: Not an operator/minter");
    ...
    function addRole(string memory role, address owner) external {
        require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "Fiat24Account: Not admin");
        _setupRole(keccak256(abi.encode(role)), owner);
    }
