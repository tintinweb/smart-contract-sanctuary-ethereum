...
contract Finch is ERC721A, Ownable {
  ...
  bool public reservedTokensMinted = false;
	string public baseURI = "ipfs://QmXLiVHmj3uGgGvUki66dcZBFucRnSapRqZ7RLcwPy3o51/";
  bytes32 public merkleRoot;
	uint256 public salePrice = 0.025 ether;
  uint256 public constant MAX_SUPPLY = 2222;
  uint256 public constant MAX_MINTS_PER_TRX = 10;
  uint256 public constant MAX_MINTS_ON_WL = 3;
  uint256 public constant RESERVED_TOKENS = 100;
	address public constant PROXY_ADDRESS = 0xa5409ec958C83C3f309868babACA7c86DCB077c1;

  constructor() ERC721A("Finch Setaverse", "FINCHSETA") {
  }

  // Flip the minting for whitelist from active or pause
  ...
  // Specify a new URI for metadata
  ...
  function mintTokens(uint256 numberOfTokens) external payable {
    ...
    require(totalSupply().add(numberOfTokens) <= MAX_SUPPLY, "Minting would exceed max supply");
    require(numberOfTokens <= MAX_MINTS_PER_TRX, "Max. 10 per tx during public sale");
    _mintTokens(numberOfTokens);
  }

  function mintTokenWL(bytes32[] calldata merkleProof, uint256 numberOfTokens) external payable {
  ...
  function withdraw() external onlyOwner {
    ...
    payable(0xfd37aCcf4C6F5825d9C5253A31669E92b50EF723).transfer(balance * 10 / 100); //vincenzor.eth
    payable(0x255CcE87dD20315209D3eeef3406dC2eD3c00f07).transfer(balance * 90 / 100); //0xodissey.eth
  }

  // Whitelist proxy contracts for easy trading on platforms (Opensea is default)
  function isApprovedForAll(address _owner, address _operator) public view override(ERC721A) returns (bool isOperator) {
