...
abstract contract IMintableToken {
    ...
    function creators(uint256 _tokenId) public view virtual returns (address payable);
...
contract Storage is OwnableUpgradeable {
    ...
    mapping(address => TokenDets) tokensForSalePerUser;
    TokenDets fixedPriceTokens;
    TokenDets auctionTokens;

    TokenDets tokensForSale1;
    ERC20Addresses.erc20Addresses erc20TokensArray1;
    ...
    function setUpdatedClosingTime(uint256 _updateTime) public onlyOwner {
       _setUpdatedClosingTime(_updateTime);
    }
     function _setUpdatedClosingTime(uint256 _updateTime) internal{
...
contract BrokerV2 is ERC721HolderUpgradeable, BrokerModifiers {
    ...
    mapping(address => uint256) public brokerageBalance;
     using TokenDetArrayLib for TokenDets;
     using ERC20Addresses for ERC20Addresses.erc20Addresses;
    // constructor(uint16 _brokerage, uint256 _updatedTime)  {
    //     brokerage = _brokerage;
    //     transferOwnership(msg.sender);
    //     _setUpdatedClosingTime(_updatedTime);
        
    // }

       function initialize(uint16 _brokerage, uint256 _updatedTime)
        public
        initializer
    {
        ...
        activeAuction(tokenID, _mintableToken)
    {
        ...
        if (_auction.erc20Token == address(0)) {
            ...
            if (
              ...
              payable(offerprice[_mintableToken][tokenID].offerer)
        ...
        activeAuction(tokenID, _mintableToken)
    {
        ...
        TokenDet memory _tokenDet = TokenDet(
            _mintableToken,
            tokenID
        );
    ...
    function _revertOffer(address _mintableToken, uint256 tokenID) private {
       payable(offerprice[_mintableToken][tokenID].offerer).transfer(
            offerprice[_mintableToken][tokenID].amount
        );
    }
