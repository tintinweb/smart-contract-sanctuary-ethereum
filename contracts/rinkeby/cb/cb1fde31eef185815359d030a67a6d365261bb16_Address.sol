...
contract NFeeT is ERC721AQueryable, Ownable, MessageSigning, PaymentSplitter {

    using Strings for uint256;

    uint32 public maxSupply = 1000; 
    uint32 public remainingReserves  = 130;
	...
	address[] private addressList = [
		0xe72E76cB4f05a90d92849234A7B3e649539E0811,  
		0x00d0BC9a3DE7577b0B9f9663E7bAFB1E72bc9972
	];
	uint256[] private shareList = [85, 15];

    constructor() ERC721A("NFeeT", "NFeeT") MessageSigning(0x73467c97C95551F64642086586Bd91B4aa88b2a9)  PaymentSplitter(addressList, shareList) {
        
        royaltyAddress = address(this);
        setBaseURI("ipfs://bafybeifd4qblchjbcedfdfuf3ngsfjs64fxpwy2ibl5oumghclprpd5cl4/"); 
    }
    ...
    function setRoyaltyBasisPoints(uint96 newRoyaltyBasisPoints) external onlyOwner {
    ...
    */
    function whitelistMinting(uint32 amountToMint, bytes memory mintPass) external payable {
        require (presale, "Presale over!");
        require (isValidSignature(abi.encodePacked(_msgSender()), mintPass), "Wallet not whitelisted");
        require (_getAux(msg.sender) + amountToMint <= maxWLmint, "max NFTs per address exceeded");
    ...
    */
    function publicMint(uint amountToMint) external payable{
        ...
        require(_numberMinted(msg.sender) - _getAux(msg.sender) + amountToMint <= 5 , "max NFTs per address exceeded");
