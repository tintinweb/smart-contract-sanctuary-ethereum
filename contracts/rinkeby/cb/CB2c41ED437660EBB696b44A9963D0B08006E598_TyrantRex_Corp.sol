...
import "./TRXCToken.sol";


contract TyrantRex_Corp is Ownable, ERC721A, ReentrancyGuard {
    ...
    uint256 public immutable MAX_PER_ADDRESS_DURING_MINT;

    uint256 public constant FREE_TOKEN_PRICE = 0.0 ether;

    uint256 public earningsPerDay = 1;
    ...
    //for the soft staking 
    //Shows for a TokenID its Timestamp 
    mapping (uint256 => uint256) public claimTime;


    constructor(uint256 maxBatchSize_, uint256 collectionSize_)
    ERC721A("Tyrant-Rex Corp", "TRXC", maxBatchSize_, collectionSize_) {
    MAX_PER_ADDRESS_DURING_MINT = maxBatchSize_;
    MAX_SUPPLY = collectionSize_;
    }
    
    // Sets the emitted Token

    function setToken(TRXCToken _newToken) public onlyOwner {
        token = _newToken;
    }
    ...
    // Adjust EarningsPerDay

    function adjustEarningsPerDay(uint256 _earningsPerDay) external onlyOwner {
        earningsPerDay = _earningsPerDay;
    }
    ...
    function safeMint(address _to, uint256 numberOfTokens) internal {
    ...
    function mintAllowList(uint8 numberOfTokens) external payable callerIsUser{
    ...
    function mintFreeList(uint8 numberOfTokens) external payable callerIsUser {
    ...
    function mint(uint numberOfTokens) public payable callerIsUser {
        ...
        require(numberMinted(msg.sender) + numberOfTokens <= MAX_PER_ADDRESS_DURING_MINT, "Exceeded max token purchase");
        require(totalSupply() + numberOfTokens <= MAX_SUPPLY, "Purchase would exceed max tokens");
        require(PRICE_PER_TOKEN * numberOfTokens <= msg.value, "Ether value sent is not correct");
    ...
    // Claim function for Softstaking
    // Notes Check if ownerOf works properly
    function claim(uint256[] calldata tokenIds) public callerIsUser {
        uint256 total = 0;
        for(uint256 i; i < tokenIds.length; i++){
            require(ownerOf(tokenIds[i]) == msg.sender, "");
            total += 1 ether * earningsPerDay * (block.timestamp - claimTime[tokenIds[i]]) / 1 days;
...
contract TRXCToken is ERC20, ERC20Burnable, Ownable {

 uint256 public MAX_SUPPLY_OF_TOKENS = 69420000 * 10 **18;

  mapping(address => bool) controllers;
  
  constructor() ERC20("TRCCoin", "TRCC") { }
  ...
  function adjustSupply(uint256 _maxSupply) external onlyOwner {
    MAX_SUPPLY_OF_TOKENS = _maxSupply;
  }
}
