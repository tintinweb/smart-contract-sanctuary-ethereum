    ...
    EIP712Upgradeable
{
    ...
    bytes32 private constant _ORDER_TYPEHASH = 0x23278edfcdd6433a8f8f2bce8eb630c4242b0056ca62950c706b1babe3e13999;
    ...
    function initialize(
        ...
        require(
            keccak256(
                "Order(address seller,address contractAddress,uint256 royaltyFee,address royaltyReceiver, address paymentToken,uint256 basePrice,uint256 listingTime,uint256 expirationTime,uint256 nonce,uint256 tokenId,Type orderType,string uri,string objId)"
            ) == _ORDER_TYPEHASH
        );
        ...
        
    {
        ...
        uint256 earning = _chargeAndSplit((paid - royaltyAmt), msg.sender, _order.paymentToken, royaltyAmt, creator);

        uint256 id;
        if (_order.orderType == Type.Digital) {
            _executeExchange(_order, _order.seller, msg.sender, earning, isToken);
        } else {
    ...
    {
        (bytes32 digest, address signer) = _verifySig(order);
        require(validateOrder(digest, order, order.signature, _amount), "Invalid Order");
        Order calldata _order = order;
        uint256 amt = _amount;
        address taker = buyer;
        INFT nftContract = INFT(order.contractAddress);
        uint256 _tokenId = _order.tokenId;
        if (_tokenId == 0) {
            // mint if not Minted
            bool isAdminContract = adminContracts[_order.contractAddress];
    ...
    function _executeExchange(
        ...
        IERC721Upgradeable(order.contractAddress).safeTransferFrom(seller, buyer, order.tokenId);
        address _seller = adminContracts[order.contractAddress] && admins[seller] ? TREASURY_WALLET : seller;
        isToken
    ...
    function _verifySi(Order calldata order) public view returns (bytes32, address) {
        bytes32 digest = hashToSign(order);
        address signer = ECDSAUpgradeable.recover(digest, order.signature);
        return (digest, signer);
    }

    function _verifyR(Order calldata order) public view returns (bytes32, address) {
        bytes32 digest = hashOrder(order);
    ...
    function _verifyV(Order calldata order) public view returns (bytes32, address) {
        bytes32 digest = hashToSign(order);
        (bytes32 r, bytes32 s,uint8 v) = splitSignature(order.signature);
        address signer = ecrecover(digest, v, r,s);
        return (digest, signer);
     ...
     */
    function hashOrder(Order memory _order) public view returns (bytes32 hash) {
        bytes memory array = abi.encodePacked(
             keccak256(
                "Order(address seller,address contractAddress,uint256 royaltyFee,address royaltyReceiver,address paymentToken,uint256 basePrice,uint256 listingTime,uint256 expirationTime,uint256 nonce,uint256 tokenId,Type orderType,string uri,string objId)"
            ),
            _order.seller,
