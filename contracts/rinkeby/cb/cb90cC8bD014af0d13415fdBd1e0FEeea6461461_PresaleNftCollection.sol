...
contract PresaleNftCollection is BaseCollection {
    ...
    // Sales Parameters
    uint256 private _maxAmount = 5000;
    uint256 private _maxPerMint = 10;
    uint256 private _maxPerWallet = 20;
    uint256 private _price = 0.049 ether;
    uint256 public _preSaleCost = 0.049 ether;
    ...
    function initialize(
        ...
        baseExtension = ".json";
        _collectionType = "presale";
    }

    modifier onlyMintable(uint256 numberOfTokens) {
        
        if (presale_list[_msgSender()] == true && _preSaleActive == true) {
            //presale list only
            require(msg.value >= _preSaleCost * numberOfTokens);
        } 
        else {
            //general public list only
            //require sale to be active and enough value sent
            require(_publicSaleActive, "Public Minting is not active right now. Please, check back later.");
    ...
    function ownerSafeMint(uint256 numberOfTokens, address sender) public onlyOwner {
    ...
    function startPreSale( uint256 preSalePrice_ ) public onlyOwner {
        _preSaleActive = true;
        _preSaleCost = preSalePrice_;
    }
    ...
    function updatePrice( uint256 newPrice_ ) public onlyOwner {
        _price = newPrice_;
    }

    function updatePreSalePrice( uint256 newPreSalePrice_ ) public onlyOwner {
        _preSaleCost = newPreSalePrice_;
    }
    
    //uses onlyMintable to check validity
    ...
    function addAddressToPresaleList(address _user) public onlyOwner {
        presale_list[_user] = true;
    }

    function addAddressesToPresaleList(address[] memory _users) public onlyOwner {
        for (uint256 i = 0; i < 2; i++) {
            presale_list[_users[i]] = true;
        }
    }

    function removeAddressFromPresaleList(address _user) public onlyOwner {
        delete presale_list[_user];
    }

    function removeAddressesFromPresaleList(address[] memory _users) public onlyOwner {
        for (uint256 i = 0; i < 2; i++) {
            delete presale_list[_users[i]];
        }
    }
