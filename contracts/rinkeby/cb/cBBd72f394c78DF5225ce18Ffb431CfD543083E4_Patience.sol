...
contract Patience is ERC721A, Ownable, ReentrancyGuard {
    ...
    constructor(
        ...
        ERC721A(
            ...
            presaleLimits[PreSaleType.N] = 100;
            presaleLimits[PreSaleType.GM] = 100;
            presaleLimits[PreSaleType.AL] = 120;
        }
    ...
    uint16 private constant MAX_PRESALE_MINTS = 200;
    uint16 private constant BASIS_POINTS = 1000;

    bytes32 public merkleRoot = 0xf6aeecde4c5d2177b925a4d3019fa9fdf9c26423588bf7a4ac7144db152be140;

    uint256 public constant MINT_PRICE = 0.15 ether;

    uint256 private maxMintPerTx = 200;
    uint256 private maxMintPerWallet = 3000;
    uint256 private gmDaoTokenId = 706480;

    uint256 private preSaleLaunchTime = 1652578320;
    uint256 private publicSaleLaunchTime = 1652578800;
    ...
    function _canMintPresale(address wallet, uint256 amount, bytes memory data) internal view returns (bool, PreSaleType) {
        ...
        if (gmDao.balanceOf(wallet, gmDaoTokenId) > 0 && presaleLimits[PreSaleType.GM] - amount >= 0) {
            return (true, PreSaleType.GM);
        }

        if (nOwnerResolver.balanceOf(wallet) > 0 && presaleLimits[PreSaleType.N] - amount >= 0) {
    ...
    function mint(uint256 numberOfMints, bytes memory data) public payable nonReentrant {
        ...
        if (preSaleEligible && (isPreSaleActive() && !isPublicSaleActive())) {
            presaleLimits[presaleType] -= uint16(numberOfMints);
        }
    }
...
interface IGmDao is IERC721Enumerable, IERC721Metadata {
