...
contract BToken is Initializable, IBToken, IncentivizedERC20 {
    ...
    function transferUnderlyingToTreasury(uint256 amount) external override onlyLendPool returns (uint256) {
        require(_treasury != address(0), "_treasury is zero address");
        IERC20Upgradeable(_underlyingAsset).safeTransfer(_treasury, amount);
        return amount;
    }
    ...
    function _transfer(
        ...
        uint256 fromFTokenBal = super.balanceOf(from);
        uint256 fromBalanceBefore = fromFTokenBal.rayMul(index);
        uint256 toBalanceBefore = super.balanceOf(to).rayMul(index);

        uint256 transferAmt = amount.rayDiv(index);
        if (fromFTokenBal < transferAmt) {
            emit TransferAdjust(fromFTokenBal, fromBalanceBefore, transferAmt, index);
            transferAmt = fromFTokenBal;
        }
