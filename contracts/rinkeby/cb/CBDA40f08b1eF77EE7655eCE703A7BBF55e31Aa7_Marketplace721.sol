...
contract Marketplace721 is Ownable {
    ...
    event TradeStatusChangeEvent(uint256 indexed nftId, bytes16 status);
    event PriceChangeEvent(uint256 Counter, uint256 newPrice);

    mapping(uint8 => address) paymentToken;
    ...
    struct Trade {
        ...
        uint256 price;
        uint8 paymentIndx;
        bytes16 status; // e.g Open, Executed, Cancelled
    }
    ...
    */
    ...
    function openTrade(address _nft, uint8 _tokenIndx, uint256 _nftId, uint256 _price) external {
        require(_nft != address(0),"address zero");

        _transfer(_nft, payable(msg.sender), address(this), _nftId);
        trades[tradeCounter] = Trade({
            poster: payable(msg.sender),
            nft: _nft,
            nftId: _nftId,
            price: _price,
            paymentIndx: _tokenIndx,
            status: "Open"
        });
    ...
    */
    ...
    function executeTrade(uint256 _trade) external {
        Trade memory trade = trades[_trade];

        require(trade.status == "Open", "Error: Trade is not Open");
        require(
            msg.sender != address(0) && msg.sender != trade.poster,
            "Error: zero address or the owner is trying to buy his own nft"
        );

        IERC20(getPaymentToken(trade.paymentIndx)).transferFrom(msg.sender, trade.poster, trade.price);
        _transfer(trade.nft,
            address(this),
            payable(msg.sender),
            trade.nftId
    ...
    */
    ...
    function cancelTrade(uint256 _trade) public {
        ...
        require(
            ...
            "Error: only by poster"
        );
        require(trade.status == "Open", "Error: not Open");

        _transfer(trade.nft, address(this), trade.poster, trade.nftId);
        trades[_trade].status = "Cancelled";

        emit TradeStatusChangeEvent(_trade, "Cancelled");
    }

    function _transfer(address _contractAddress, address _from, address _to, uint256 _id) private {
        IERC721(_contractAddress).transferFrom(_from, _to, _id);
    }

    // Get all items which are on sale in the market place
    function getAllOnSale() external view virtual returns (Trade[] memory) {
        uint256 counter = 0;
    ...
    function setPaymentToken(uint8 _indx, address _token) external onlyOwner {
        require(_token != address(0), "zero address");
        paymentToken[_indx] = _token;
    }

    function getPaymentToken(uint8 _indx) public view returns(address) {
        return paymentToken[_indx];
    }

    // get all items owned by a perticular address
    function getAllByOwner(address owner) external view returns (Trade[] memory) {
        uint256 counter = 0;
        uint256 itemCounter = 0;
        for (uint256 i = 0; i < tradeCounter; i++) {
    ...
    /*
    Seller can change the price of item by specifing trade number and new price
    */
    function modifyPrice(uint256 _trade, uint256 newPrice) external {
        require(
            msg.sender == trades[_trade].poster,
            "Error: set by poster only"
        );
        require(trades[_trade].status == "Open", "Error: Trade is not Open");

        trades[_trade].price = newPrice;
        emit PriceChangeEvent(_trade, newPrice);
    }
