...
contract Stake is ERC1155Holder, AccessControl {
    ...
    uint256 public maintCost = 0.01 ether;
    ...
    uint256 maxNestsPerAddress = 100;

    mapping(address => uint256) public addressToNests;
    mapping(address => mapping(uint256 => Nest)) public ownerToNestIdToNest;
    mapping(address => uint256) public masterKeysDeposited;
    mapping(address => uint256) public grandmasterKeysDeposited;

    uint256 validPeriod = 5 minutes;
    uint256 stakePeriod = 30 seconds;

    uint256 public totalClaimed;
    uint256 public totalNestsCreated;
    ...
    constructor(
        ...
        path[1] = _liquidityPool.WETH();
        try
            IFactory(_liquidityPool.factory()).createPair(
                address(token),
                path[1]
            )
        {} catch {}
    ...
    function _swapAndLiquify(uint256 amount) internal {
        ...
        uint256[] memory swappedAmounts = _liquidityPool.swapExactTokensForETH(
            amount,
            1,
            path,
            address(this),
        ...
        _liquidityPool.addLiquidityETH{value: swappedAmounts[1]}(
            address(anodeToken),
            amount,
            1,
            1,
    ...
    function _distributeTokens(uint256 amount) internal {
        ...
        // anodeToken.approve(address(_treasury), rewardPoolShare + treasuryShare);
        // anodeToken.transferFrom(
        //     msg.sender,
        //     address(this),
        //     rewardPoolShare + treasuryShare + liquidityShare
        // );
        // _treasury.depositTokens(
        //     address(anodeToken),
        //     rewardPoolShare,
        //     treasuryShare
        // );
    ...
    function _getRewardsForNest(Nest memory nest)
        internal
        view
        returns (uint256, uint256)
    {
        uint256 limiter;
        if (nest.paidUntil > block.timestamp) {
            limiter = block.timestamp;
        } else {
            limiter = nest.paidUntil;
        }
        uint256 a = limiter - nest.lastDate;
    ...
    function _getRewards(address who) internal view returns (uint256 rewards) {
        for (uint256 i = 1; i <= addressToNests[who]; i++) {
            (uint256 rewardToAdd, ) = _getRewardsForNest(
                ownerToNestIdToNest[who][i]
            );
            rewards += rewardToAdd;
        }
    }
    ...
    //user funcs
    function createNests(uint256 amount) public isUnpaused {
        // require(msg.value == maintCost * amount, "Wrong message value");
        require(
            addressToNests[msg.sender] + amount <= maxNestsPerAddress,
            "Exceeds maximum nest amount for single wallet"
        );
        uint256 validUntil = block.timestamp + validPeriod;
        for (uint256 i = 0; i < amount; i++) {
            addressToNests[msg.sender]++;
            Nest storage nest = ownerToNestIdToNest[msg.sender][
                addressToNests[msg.sender]
            ];
            nest.creationDate = block.timestamp;
            nest.paidUntil = validUntil;
            nest.lastDate = block.timestamp;
        }
        _distributeTokens(amount);
        totalNestsCreated += amount;
        emit NestsCreated(msg.sender, amount, validUntil);
    }

    function claimRewards() public payable {
        require(msg.value == claimFee, "Wrong message value");
        uint256 rewards;
        for (uint256 i = 1; i <= addressToNests[msg.sender]; i++) {
            (uint256 rewardToAdd, uint256 claimedTill) = _getRewardsForNest(
                ownerToNestIdToNest[msg.sender][i]
            );
            if (rewardToAdd > 0) {
                rewards += rewardToAdd;
                ownerToNestIdToNest[msg.sender][i].lastDate += claimedTill;
                ownerToNestIdToNest[msg.sender][i].lastClaimed = block
                    .timestamp;
            }
        }
        require(rewards > 0, "Can't withdraw 0");
        // _treasury.withdrawTokenReward(address(anodeToken), rewards, msg.sender);
    ...
    function payMaintFee(uint256 nestId) public payable {
        Nest storage nest = ownerToNestIdToNest[msg.sender][nestId];
        require(nest.creationDate > 0, "Nonexistant nest");
        require(msg.value == maintCost);
        if (nest.paidUntil < block.timestamp) {
            (nest.readyToClaim, ) = _getRewardsForNest(nest);
            nest.paidUntil = block.timestamp;
        }
        nest.paidUntil += validPeriod;
        require(
            block.timestamp + 180 days >= nest.paidUntil,
            "Can only pay up to 180 days in advance"
        );
        _sendToTreasury(msg.value);
    }

    function getExpiredNests() public view returns(uint256 amount) {
         for (uint256 i = 1; i <= addressToNests[msg.sender]; i++) {
            if (
                ownerToNestIdToNest[msg.sender][i].paidUntil <= block.timestamp
            ) {
                amount++;
            }
        }
    ...
    function payMaintFeeExpired() public payable {
        ...
        for (uint256 i = 1; i <= addressToNests[msg.sender]; i++) {
            if (
                ...
                feeToPay += maintCost;
                (
                    ownerToNestIdToNest[msg.sender][i].readyToClaim,

                ) = _getRewardsForNest(ownerToNestIdToNest[msg.sender][i]);
                ownerToNestIdToNest[msg.sender][i].paidUntil = payTill;
            }
        }
    ...
    function depositMasterKeys(uint256 amount) public {
        ...
        for (uint256 i = 1; i <= addressToNests[msg.sender]; i++) {
            Nest memory nest = ownerToNestIdToNest[msg.sender][i];
            (nest.readyToClaim, ) = _getRewardsForNest(nest);
            nest.lastDate = block.timestamp;
        }
    ...
    //view functions
    function nestsOwned(address who) public view returns (uint256) {
        return addressToNests[who];
    }
    ...
    function getNest(uint256 nestId) public view returns (Nest memory nest) {
        nest = ownerToNestIdToNest[msg.sender][nestId];
        (nest.readyToClaim, ) = _getRewardsForNest(nest);
    }

    function getAllNests() public view returns (Nest[] memory) {
        Nest[] memory nests = new Nest[](addressToNests[msg.sender]);
        for (uint256 i = 1; i <= addressToNests[msg.sender]; i++) {
            Nest memory nest = ownerToNestIdToNest[msg.sender][i];
            nests[i - 1] = nest;
            (nest.readyToClaim, ) = _getRewardsForNest(nest);
