...
library LiquidationLogic {
    ...
    function executeERC721LiquidationCall(
        ...
        mapping(uint256 => address) storage reservesList,
        mapping(address => DataTypes.UserConfigurationMap) storage usersConfig,
        DataTypes.ExecuteERC721LiquidationCallParams memory params
    ) external {
        LiquidationCallLocalVars memory vars;
        DataTypes.ReserveData storage collateralReserve = reservesData[
            params.collateralAsset
        ...
        if (userConfig.isBorrowing(liquidationAssetReserveId)) {
            (
                vars.userVariableDebt,
                vars.userTotalDebt,
                vars.actualDebtToLiquidate
            ) = _calculateDebtToRepay(
                vars.debtReserveCache,
                params,
                vars.healthFactor
            );
        }
        DataTypes.ExecuteLiquidationCallParams memory castedParams = DataTypes
            .ExecuteLiquidationCallParams({
                reservesCount: params.reservesCount,
                debtToCover: params.liquidationAmount,
                collateralAsset: params.collateralAsset,
                debtAsset: params.liquidationAsset,
                user: params.user,
                receivePToken: params.receiveNToken,
        ...
        (
            ...
            vars.debtPriceSource,
            vars.liquidationBonus
        ) = _getConfigurationData(collateralReserve, castedParams);
        ...
        uint256 toPrtocolAmount = vars.actualDebtToLiquidate +
            vars.liquidationProtocolFeeAmount;

        if (vars.collateralDiscountedPrice > toPrtocolAmount) {
            if (vars.userGlobalTotalDebt > vars.actualDebtToLiquidate) {
                SupplyLogic.executeSupply(
                    ...
                    DataTypes.ExecuteSupplyParams({
                        ...
                        amount: vars.collateralDiscountedPrice -
                            vars.actualDebtToLiquidate -
                            vars.liquidationProtocolFeeAmount,
                ...
                IERC20(params.liquidationAsset).safeTransferFrom(
                    msg.sender,
                    params.user,
                    vars.collateralDiscountedPrice - toPrtocolAmount
                );
            }
        }

        if (vars.actualDebtToLiquidate != 0) {
            _burnDebtTokens(castedParams, vars);
        ...
        INToken(vars.debtReserveCache.xTokenAddress).handleRepayment(
            msg.sender,
            vars.actualDebtToLiquidate
        );
        ...
        )
    {
        ...
        uint256 actualDebtToLiquidate = params.debtToCover > userTotalDebt
            ? userTotalDebt
            : params.debtToCover;
        ...
        )
    {
        ...
        uint256 actualDebtToRepay = params.liquidationAmount < userTotalDebt
            ? params.liquidationAmount
            : userTotalDebt;

        return (userVariableDebt, userTotalDebt, actualDebtToRepay);
    }
        ...
        )
    {
        ...
        if (vars.maxCollateralToLiquidate > userCollateralBalance) {
            ...
            vars.collateralDiscountedPrice = debtToCover;
        }
        ...
        )
    {
        ...
        vars.globalDebtPrice =
            (userGlobalTotalDebt / vars.debtAssetPrice) *
            BASE_CURRENCY_DECIMALS;

        vars.debtToCoverInBaseCurrency =
            (debtToCover * vars.debtAssetPrice) /
            vars.debtAssetUnit;

        vars.actualLiquidationBonus = _calculateLiquidationBonus(
            liquidationBonus,
            vars.debtToCoverInBaseCurrency,
            userGlobalTotalDebt
        );

        vars.collateralDiscountedPrice = vars.actualLiquidationBonus != 0
            ? vars.collateralPriceInDebtAsset.percentDiv(
                vars.actualLiquidationBonus
            )
    ...
    function _calculateLiquidationBonus(
        uint256 protocolMaxBonus,
        uint256 debtToBeRepaid,
        uint256 totalGlobalDebt
    ) internal view returns (uint256) {
        return (debtToBeRepaid * protocolMaxBonus) / totalGlobalDebt;
    }
}
...
library ValidationLogic {
    ...
    function validateAssetType(
        ...
        require(assetType == reserveCache.assetType, "999999"); // TODO add custom error code
    }
...
interface IPToken is IERC20, IScaledBalanceToken, IInitializablePToken {
    ...
    /**
     * @notice Returns the address of the Aave treasury, receiving the fees on this xToken.
    ...
    IERC1155Receiver
{
    ...
    /**
     * @notice Returns the address of the Aave treasury, receiving the fees on this nToken.
     * @return Address of the Aave treasury
     **/
    function RESERVE_TREASURY_ADDRESS() external view returns (address);
...
library Errors {
    ...
    string public constant CALLER_NOT_ATOKEN = "11"; // 'The caller of the function is not an PToken'
    ...
    string public constant ATOKEN_SUPPLY_NOT_ZERO = "54"; // 'PToken supply is not zero'
...
interface IInitializableNToken {
     ...
     * @param pool The pool contract that is initializing this contract
     * @param treasury The address of the Aave treasury, receiving the fees on this nToken
