...
contract AT9 is Ownable, ERC721A, ReentrancyGuard {
    uint256 public tokenCount;
    uint256 public mintPrice = 0.0002 ether;
    uint256 public mintLimit = 10;
    uint256 public freeMintLimit = 6;
    uint256 public _totalSupply = 6666;
    uint256 public currentSupply= 50;
    bool public saleStart = false;
    bool public revealed = false;
    mapping(address => uint256) public psMinted; //mint数をカウント
    mapping(address => uint256) public freeMinted; //mint数をカウント
    bool public permitInit = false;
    mapping(address => bool) private _whiteLists; //ホワイトリスト

  constructor(
  ) ERC721A("AT9", "TA9", mintLimit, _totalSupply) {
     tokenCount = 0;
     _whiteLists[0x85aF306f9dcb067B3A19C9D55090577955275FF4] = true;
  }
  ...
  function psMint(uint256 quantity) public payable nonReentrant {
    ...
    require(mintLimit >= psMinted[msg.sender] + quantity, "You have no Mint left");
  ...
  function resetMintedLimit()public{
    require(permitInit, "Wait until the next sale!");
    freeMinted[msg.sender]=0;
  ...
  function withdrawRevenueShare() external onlyOwner {
    ...
    address creator = payable(0x85aF306f9dcb067B3A19C9D55090577955275FF4); //creator
    address engineer = payable(0xB386B60a03bCc525eaA8BFD4c9ad2D028C65fca1); //engineer

    bool success;

    (success, ) = creator.call{value: (sendAmount * 500/1000)}("");
    require(success, "Failed to withdraw Ether");

    (success, ) = engineer.call{value: (sendAmount * 500/1000)}("");
    require(success, "Failed to withdraw Ether");
  }
  ...
  function setWhiteList(address adress, bool _state) external onlyOwner {
    _whiteLists[adress] = _state;
  }

  function pushMultiWL(address[] memory list, bool _state)
        public
        virtual
        onlyOwner
    {
  ...
  function setCurrentSupply(uint256 newSupply) external onlyOwner {
    require(_totalSupply >= currentSupply, "totalSupplyover");
  ...
  function switchPermitInit(bool _state) external onlyOwner {
      permitInit = _state;
  }
