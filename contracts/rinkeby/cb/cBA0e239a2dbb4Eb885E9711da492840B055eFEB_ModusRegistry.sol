...
contract ModusRegistry {
    using PRBMathUD60x18 for uint256;
    address public modusAdmin;
    address public potentialModusAdmin;
    address public modusFactory;
    address public modusStaking;
    address public kycAdmin;
    address public proxyAdmin;
    address public modus;
    address public lostWalletAdmin;
    address public priceModule;
    address public kycModule;
    address public modusTreasuryWallet;
    address public pxt;
    address public stableCoinPXTSwap;
    address public pxtStaking;
    address public pxrt;
    address public pxtModusSwap;
    address public pxtStableCoinSwap;
    address public pxrtModusSwap;
    address public pxrtStableCoinSwap;
    uint256 private schemeIDCounter = 1;

    mapping(address => bool) private isBlacklisted;
    mapping(address => bool) private stablecoinSupport;
    mapping(uint256 => address) public projectStableCoin;

    ///@notice structure to store the scheme details
    struct Scheme {
        uint256 schemeId;
        uint256 maxDays;
        uint256 exponentBase;
        uint256 percentageRewardTokens; //with 2 decimal places
    }
    Scheme rewardScheme;
    mapping(uint256 => Scheme) public schemeDetails;
    Scheme[] schemes;

    event NominateModusAdmin(address indexed _potentialModusAdmin);
    event ModusAdminChanged(address indexed modusAdmin);
    event ProxyAdminChanged(address indexed proxyAdmin);
    event KYCAdminChanged(address indexed kycAdmin);

    ///@notice structure to store the project Details(includes proxy contract addreses)
    struct ModusProject {
        uint256 projectId;
    ...
    ModusProject project;
    mapping(uint256 => ModusProject) projectDetails;
    ModusProject[] projects;

    /// @notice Modifier to check whether the message.sender is Modus Admin
    modifier onlyModusAdmin() {
        require(msg.sender == modusAdmin, "ModusRegistry: Not ModusAdmin");
        _;
    }

    /// @notice Modifier to check whether the message.sender is Modus Factory
    modifier onlyModusFactory() {
        require(msg.sender == modusFactory, "ModusRegistry: Not ModusFactory");
        _;
    }

    /// @notice Modifier to check whether the message.sender is Modus Factory
    modifier onlyLostWalletAdmin() {
        require(
            msg.sender == lostWalletAdmin,
            "ModusRegistry: Not lostWalletAdmin"
        );
        _;
    }

    modifier zeroAddressCheck(address _theAddress) {
        require(_theAddress != address(0), "ModusRegistry: Zero Address");
        _;
    }

    ///@notice store the address of the modus admin and modus treasury wallet
    ///@param  _modusTreasuryWallet,Modus treasury wallet
    constructor(
        address _modusTreasuryWallet,
        address _admin,
        address _lostWalletAdmin
    ) {
        modusTreasuryWallet = _modusTreasuryWallet;
        modusAdmin = _admin;
        lostWalletAdmin = _lostWalletAdmin;
    }

    /// @notice nominate a new Modus Admin
    /// @param _potentialModusAdmin, address sent by the admin
    function addPotentialModusAdmin(address _potentialModusAdmin)
        external
        onlyModusAdmin
        zeroAddressCheck(_potentialModusAdmin)
    {
        require(
            _potentialModusAdmin != modusAdmin,
            "ModusRegistry: Potential Admin should not be current Admin"
        );
        potentialModusAdmin = _potentialModusAdmin;
        emit NominateModusAdmin(_potentialModusAdmin);
    }

    ///@notice accept nomination for Modus Admin
    ///only accesible by potential modus admin
    function acceptModusAdminNomination() external {
        require(
            msg.sender == potentialModusAdmin,
            "ModusRegistry: Only the potential admin can accept nomination"
        );
        modusAdmin = potentialModusAdmin;
        potentialModusAdmin = address(0);
        emit ModusAdminChanged(modusAdmin);
    }

    ///@notice sets the address of the KYC Admin
    ///@param kyc_admin, address of the kyc admin
    ///only accesible by modus admin
    function setKYCAdmin(address kyc_admin)
        external
        onlyModusAdmin
        zeroAddressCheck(kyc_admin)
    {
        kycAdmin = kyc_admin;
        emit KYCAdminChanged(kyc_admin);
    }

    ///@notice sets the address of the proxyContracts upgrade Admin
    ///@param _proxyAdmin, address of the proxyContracts upgrade Admin
    ///only accesible by modus admin
    function setProxyAdmin(address _proxyAdmin)
        external
        onlyModusAdmin
        zeroAddressCheck(_proxyAdmin)
    {
        proxyAdmin = _proxyAdmin;
        emit ProxyAdminChanged(_proxyAdmin);
    }

    ///@notice stores the contract addresses of the base contracts
    ///@param  _modusFactory,_priceModule, _kycModule, _stableCoin
    ///only accessible by Modus Admin
    function setBaseContracts(
        address _modusFactory,
        address _modusStaking,
        address _priceModule,
        address _kycModule,
        address _modus
    ) external onlyModusAdmin {
        modusFactory = _modusFactory;
        modusStaking = _modusStaking;
        priceModule = _priceModule;
        kycModule = _kycModule;
        modus = _modus;
    }

    function setModus(address _modus)
        external
        onlyModusAdmin
        zeroAddressCheck(_modus)
    {
        modus = _modus;
    }

    function setModusFactory(address _modusFactory)
        external
        onlyModusAdmin
        zeroAddressCheck(_modusFactory)
    {
        modusFactory = _modusFactory;
    }

    function setModusStaking(address _modusStaking)
        external
        onlyModusAdmin
        zeroAddressCheck(_modusStaking)
    {
        modusStaking = _modusStaking;
    }
    ...
    function setPriceModule(address _priceModule)
        external
        onlyModusAdmin
        zeroAddressCheck(_priceModule)
    {
        priceModule = _priceModule;
    }

    function setKycModule(address _kycModule)
        external
        onlyModusAdmin
        zeroAddressCheck(_kycModule)
    {
        kycModule = _kycModule;
    }

    ///@notice Function to allow ModusAdmin to add Stable coin Address to the Modus
    ///@param _stableCoin, address of the stable coin
    function addStableCoin(address _stableCoin)
        external
        onlyModusAdmin
    {
        stablecoinSupport[_stableCoin] = true;
    }

    ///@notice Function to allow ModusAdmin to remove Stable coin Address from the Modus
    ///@param _stableCoin, address of the stable coin
    function removeStableCoin(address _stableCoin)
        external
        onlyModusAdmin
    {
        stablecoinSupport[_stableCoin] = false;
    }

    function isStableCoinSupported(address _stableCoin)
        external
        view
        returns (bool)
    {
        return stablecoinSupport[_stableCoin];
    }

    ///@notice stores the contract addresses of the Implemetation contracts
    ///@param  _pxt_, _stableCoinPXTSwap_, _pxtStaking_, _pxrt_,_pxtModusSwap_,
    ///_pxtStableCoinSwap_, _pxrtModusSwap_, _pxrtStableCoinSwap_
    ///only accessible by Modus Admin
    function setImplementationContracts(
        address _pxt_,
        address _stableCoinPXTSwap_,
        address _pxtStaking_,
        address _pxrt_,
        address _pxtModusSwap_,
        address _pxtStableCoinSwap_,
        address _pxrtModusSwap_,
        address _pxrtStableCoinSwap_
    ) external onlyModusAdmin {
        pxt = _pxt_;
        stableCoinPXTSwap = _stableCoinPXTSwap_;
        pxtStaking = _pxtStaking_;
        pxrt = _pxrt_;
        pxtModusSwap = _pxtModusSwap_;
        pxtStableCoinSwap = _pxtStableCoinSwap_;
        pxrtModusSwap = _pxrtModusSwap_;
        pxrtStableCoinSwap = _pxrtStableCoinSwap_;
    }

    ///@notice stores scheme details
    ///@param _maxDays, _exBase, percentageRewardTokens
    ///Return schemeId
    function setScheme(
        uint256 _maxDays,
        uint256 _exBase,
        uint256 _percentageRewardTokens
    ) external onlyModusAdmin returns (uint256) {
        require(
            _percentageRewardTokens <= 10000,
            "ModusRegistry: Reward Percentage cannot be more than 100%"
        );
        rewardScheme = Scheme(
            schemeIDCounter,
            _maxDays,
            _exBase,
            _percentageRewardTokens
        );
        schemeDetails[rewardScheme.schemeId] = rewardScheme;
         schemes.push(schemeDetails[schemeIDCounter]);
        schemeIDCounter++;
        return (rewardScheme.schemeId);
    }

    ///@notice allows modusFactory to map the stablecoin to be used for a Particular project ID
    ///@param _projectId, index of the Stable coin
    function setStableCoin(uint256 _projectId, address _stableCoin)
        external
        onlyModusFactory
    {
        projectStableCoin[_projectId] = _stableCoin;
    }

    ///@notice stores the details about a project at different stages of deployment
    ///@param project_id,projectStage, bytesdata(contains address of the contracts in bytes format)
    ///Return Project ID
    ///only Accessible by ModusFactory
    function setProjectDetails(
        uint256 project_id,
        uint256 project_stage,
        bytes memory data
    ) external onlyModusFactory returns (uint256) {
        require(project_stage > 1, "ModusRegistry: Function Mismatch");
        projectDetails[project_id].projectStage = project_stage;
        if (project_stage == 3) {
            (
                projectDetails[project_id]._pxrt,
                projectDetails[project_id]._pxtModusSwap,
                projectDetails[project_id]._pxrtModusSwap
            ) = abi.decode(data, (address, address, address));
        }
        if (project_stage == 4) {
            (
                projectDetails[project_id]._pxtStableCoinSwap,
                projectDetails[project_id]._pxrtStableCoinSwap
            ) = abi.decode(data, (address, address));
            projects.push(projectDetails[project_id]);
        }

        return project.projectId;
    }

    ///@notice stores the details about a project at different stages of deployment- overloaded function only to be called in stage 1
    ///@param project_id,projectStage, bytesdata(contains details of custodian wallet, feepercentage and scheme id in bytes format),
    /// tokenData which contains the address of the token contracts
    ///Return Project ID
    ///only Accessible by ModusFactory
    function setProjectDetails(
        uint256 project_id,
        ...
        require(project_Stage == 1, "ModusRegistry: Function Mismatch");
        projectDetails[project_id].projectId = project_id;
        projectDetails[project_id].projectStage = project_Stage;
        (
            projectDetails[project_id].custodianWallet,
            projectDetails[project_id].feePercentage,
            projectDetails[project_id].schemeId
        ) = abi.decode(data, (address, uint256, uint256));
        (
            projectDetails[project_id]._pxt,
            projectDetails[project_id]._stableCoinPXTSwap,
            projectDetails[project_id]._pxtStaking
        ) = abi.decode(tokenData, (address, address, address));
    ...
    ///@notice Blacklists the address of an account
    ///@param account, address of the account
    ///only accesible by PXTStaking contract of the respective project
    function blackListAccount(uint256 projectId, address account) external {
        require(
            msg.sender == projectDetails[projectId]._pxtStaking,
            "ModusRegistry: Caller is not valid PXTStaking"
        );
        isBlacklisted[account] = true;
    }

    ///@notice whitelist the address of an account
    ///@param account, address of the account
    ///only accesible by ModusAdmin
    function whiteListAccount(address account) external onlyModusAdmin {
        isBlacklisted[account] = false;
    }

    ///@notice blacklist the address of an account
    ///@param account, address of the account
    ///only accesible by ModusAdmin
    function blackListAccountByAdmin(address account) external onlyModusAdmin {
        isBlacklisted[account] = true;
    }

    ///@notice retieves the scheme details by providing the scheme ID
    ///@param scheme_id, the scheme ID
    ///Return max days and the exponential base for that scheme ID
    function getSchemeDetails(uint256 scheme_id)
        external
        view
        returns (
            uint256,
    ...
    {
        return (
            schemeDetails[scheme_id].schemeId,
            schemeDetails[scheme_id].maxDays,
            schemeDetails[scheme_id].exponentBase,
            schemeDetails[scheme_id].percentageRewardTokens
        );
    }

    //@notice calculates the reward Percentage using schem id and the number of days token has been staked
    //@param scheme_id, _days
    //Returns the Reward Percentage
    function getRewardPercentage(uint256 scheme_id, uint256 _days)
        external
        view
        returns (uint256 rewardPercentage)
    {
        if (_days > schemeDetails[scheme_id].maxDays) {
            rewardPercentage =
                (
                    (schemeDetails[scheme_id].exponentBase).pow(
                        schemeDetails[scheme_id].maxDays
                    )
                ) -
                1e18;
            ...
            rewardPercentage =
                ((schemeDetails[scheme_id].exponentBase).pow(_days)) -
                1e18;
        }
    }

    ///@notice retrieves the list of all the schemes
    ///Return array of Scheme
    function getSchemesList() external view returns (Scheme[] memory) {
        return schemes;
    }

    ///@notice retrieves the project details by providingthe project ID
    ///@param project_id , Id of the project
    ///Return ModusProjectStruct
    function getProjectDetails(uint256 project_id)
        external
        view
        returns (ModusProject memory)
    {
        return projectDetails[project_id];
    }

    ///@notice retrieves the list of all the project in the modus application
    ///Return array of ModusProject
    function getProjectList() public view returns (ModusProject[] memory) {
        return projects;
    }
}
