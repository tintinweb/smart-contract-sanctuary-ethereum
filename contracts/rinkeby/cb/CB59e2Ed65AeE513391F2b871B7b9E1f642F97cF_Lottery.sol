...
contract Lottery is Ownable, VRFConsumerBaseV2 {
  ...
  function _settleLottery(uint256 requestId, uint256[] memory randomWords)
    internal
  {
    uint256 _currentLottery = _lotterySelectReqIdInit[requestId];
    require(_currentLottery != 0, 'lottery selection does not exist');

    uint256 _winnerIdx = randomWords[0] %
      lottoParticipants[_currentLottery].length;
    address _winner = lottoParticipants[_currentLottery][_winnerIdx];
    ...
    isLotterySettled[_currentLottery] = true;
    emit SelectedWinner(_currentLottery, _winner, _amountWon);
  }
  ...
  function getCurrentLotteryRewardAmount() public view returns (uint256) {
    ...
    if (_entryFeesTotal < currentMinWinAmount) {
      return currentMinWinAmount;
    }
    return _entryFeesTotal;
  }
