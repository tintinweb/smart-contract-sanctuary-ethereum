...
contract PunksDedAuction is Pausable, Ownable {
    ...
    // The address of the DAO multi-sig
    address public daoAddress = 0xC3b94e21337E9c07DaBADbF299C8590Cf7D2Efbf;

    // The minimum amount of time left in an auction after a new bid is created
    uint256 public timeBuffer = 300;

    // The minimum price accepted in an auction
    uint256 public reservePrice = 1;
     ...
     * @dev Only callable by the owner.
     */
    function setWeth(address _address) external onlyOwner {
        weth = _address;
    }
    ...
    function _createAuction() internal {
        token.mintReserved(address(this), 1);
     ...
     * @dev If there are no bids, the token is transferred to DAO address.
     */
    function _settleAuction() internal {
        ...
        if (_auction.bidder == address(0)) {
            token.transferFrom(daoAddress, _auction.bidder, _auction.tokenId);
        } else {
    ...
    /**
     * @notice Transfer token ownership.
     */
    function transferTokenOwnership(address _address) external onlyOwner {
        token.transferOwnership(_address);
    }

    /**
     * @notice Mint from token contract.
     */
    function mintReserved(address _address, uint _numberOfTokens) external onlyOwner {
        token.mintReserved(_address, _numberOfTokens);
    }

    /**
     * @notice Returns total supply from token contract.
     */
...
interface IToken is IERC721 {
    function mintReserved(address _to, uint numberOfTokens) external;
    function totalSupply() external view returns (uint256);
