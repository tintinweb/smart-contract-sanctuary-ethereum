...
contract NounsToken is INounsToken, Ownable, ERC721Checkpointable {
    ...
    NFTDescriptor private descriptor;
    ...
    // FUNCTIONS STOLEN FROM NOUNS DESCRIPTOR FOR URI STUFF
    ...
    //  * token ID if {tokenURI} is empty. See: https://forum.openzeppelin.com/t/what-is-setbaseuri-used-for/16294
    //  * @dev This can only be called by the owner.
    //  */
    ...
    function dataURI(uint256 tokenId)
        ...
        returns (string memory)
    {
        string memory name = string(abi.encodePacked("Noun Name"));
        string memory description = string(
            abi.encodePacked("Noun  is a member of the Nouns DAO")
        );
        string memory image = string(
            abi.encodePacked(
                "https://gateway.pinata.cloud/ipfs/QmNq2tM4gD8H2uCvtCTiP7U16Lf5kBuxMGmB8s2KZyTjde"
            )
        );
