...
contract WhitelistLowGas is ERC721A, Ownable, ReentrancyGuard {
  using Strings for uint256;
  ...
  uint256 public wlPrice = 0;
  uint256 public maxPerTx = 20;
  uint256 public maxPerWL = 2;
  uint256 public maxWLSupply = 6;
  ...
  constructor() ERC721A('WhitelistLowGas', 'WLG') {
   ...
   */
  function startTokenId() internal view virtual returns (uint256) {
    return 1;
  }
  ...
  function teamMint(uint256 _quantity) external onlyOwner {
    ...
    require(_quantity <= maxPerTx, 'Max per TX reached.');
    require(totalSupply() + _quantity <= maxSupply, 'Sold out');
    _safeMint(msg.sender, _quantity);
  }
