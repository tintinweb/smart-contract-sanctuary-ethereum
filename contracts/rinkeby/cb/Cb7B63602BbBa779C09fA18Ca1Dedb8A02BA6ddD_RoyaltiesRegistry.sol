...
contract RoyaltiesRegistry is Ownable, RoyaltiesRegistryAutoProxy, IRoyaltiesRegistry {
    ...
    modifier requireTokenIdCreator(address token, uint256 tokenId) {
        require(
            _callerAddress() == ICreator(token).creator(tokenId),
            'RoyaltiesRegistry: FORBIDDEN, not token ID creator'
        );
        _;
    }
    ...
    function setCreatorRoyaltiesByTokenAndTokenId(
        ...
        uint256 tokenId,
        LibPartTypes.Part[] memory royalties
    ) external override requireTokenIdCreator(token, tokenId) {
        // Delete previous royalties data
        ...
        returns (LibPartTypes.Part[] memory)
    {
        ...
        if (ownerRoyaltiesSet.initialized && creatorRoyaltiesSet.initialized) {
            ...
            (bool result, LibPartTypes.Part[] memory providerRoyalties) = providerExtractor(
                token,
                tokenId
            );
        ...
        returns (bool result, LibPartTypes.Part[] memory royalties)
    {
        ...
        if (providerAddress != address(0x0)) {
            ...
            try IRoyaltiesRegistry(providerAddress).getRoyalties(token, tokenId) returns (
                ...
                if (royaltiesByProvider.length > 0) {
                    return (true, royaltiesByProvider);
                }
            } catch {}
            ...
            try IRoyaltiesProviders(providerAddress).getRoyalties(token, tokenId) returns (
                ...
                if (royaltiesByProvider.length > 0) {
                    for (uint256 i = 0; i < royaltiesByProvider.length; i++) {
                        require(
                            royaltiesByProvider[i].value <= 10000,
                            'RoyaltiesRegistry: providerExtractor invalid uint96 royalty value'
                        );
            ...
            try IRoyaltiesProviders(providerAddress).royaltyFeeInfoCollection(token) returns (
                ...
                if (royaltiesByProvider.setter != address(0x0)) {
                    require(
                        royaltiesByProvider.fee <= 10000,
                        'RoyaltiesRegistry: providerExtractor invalid uint256 royalty value'
                    );
        ...
        // if we find no data from any provider we return empty royalties
