...
contract NftPuzzle is ERC721A, Ownable, ReentrancyGuard {
    ...
    string  public baseTokenURI = "https://bafybeiebdu322mqzk666kifyv3ykoflhzzc2frx5iafevc3njetvx526pu.ipfs.nftstorage.link/metadata-puzzle";
    ...
    function makePuzzle(uint256 puzzles) external payable
    {
        ...
        require(puzzles <= MAX_KUMAS_PER_TX, "Max monsters per transaction exceeded");

        require(totalSupply() + puzzles <= MAX_PUZZLES, "The mint end");

        uint256 tokensOfSender = balanceOf(msg.sender);

        require(tokensOfSender + puzzles <= MAX_KUMAS_PER_WALLET, "More than limit");
        ...
        if (canMintAFree && !isTryingToMintAOneFreeMint) {
            require(msg.value >= (PRICE * (puzzles - MAX_FREE_PER_WALLET)), "Error");
        }

        bool itsAPaidMint = !isTryingToMintAOneFreeMint && alreadyMintedAFree;
        if (itsAPaidMint) {
            require(msg.value >= (PRICE * puzzles) , "Incorrect ETH value sent");
        }
        ...
        emit NewAdoptedKumas(msg.sender, puzzles);
    }
