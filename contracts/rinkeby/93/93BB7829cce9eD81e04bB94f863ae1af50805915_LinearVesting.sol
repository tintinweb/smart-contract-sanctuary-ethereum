...
contract LinearVesting is Ownable {
    ...
    uint256 public immutable vestingPeriod = 365 days;
    ...
    function getLocked(address beneficiary) external view returns (uint256) {
        (uint256 vestedd, ) = getVested(beneficiary);
        return recipients[beneficiary].totalAmount.sub(vestedd);
    }
    ...
    {
        (uint256 vestedd, ) = getVested(beneficiary);
        return vestedd.sub(recipients[beneficiary].amountWithdrawn);
    }
    ...
    function getVested(address beneficiary)
        ...
        view
        virtual
        returns (uint256 _amountVested, uint256 _vestedPercent)
    {
        ...
        uint256 firstVestingPoint = _vestingSchedule.startTime;
        ...
        if (
            ...
            vestedPercent = (block.timestamp - firstVestingPoint)
                .mul(10000)
                .div(vestingPeriod);
        ...
        if (vestedAmount > _vestingSchedule.totalAmount) {
            return (_vestingSchedule.totalAmount, vestedPercent);
        }

        return (vestedAmount, vestedPercent);
    }
}
