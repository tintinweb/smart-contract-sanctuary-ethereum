...
contract PaymentModuleV1 is IDepositHandler, IPaymentModule, AccessControlEnumerable {
        ...
        for (uint256 i = 0; i < fungibleTokenDeposits.length; i++) {
            ...
            if (fungibleTokenDeposits[i].tokenAddress == lpTokens[i]) {
                ...
                if (referrer != address(0)) {
                    ...
                    _removeLiquidity(fungibleTokenDeposits[i].tokenAddress, referrerLPShare, referrer);
                }

                // Not all networks are supported by Chainlink Keepers. For
                // these cases, no LP token processor module will be deployed.
                // The LP tokens will instead be liquidated immediately.
                if (address(lpTokenProcessor) == address(0)) {
                    _removeLiquidity(fungibleTokenDeposits[i].tokenAddress, lpAmounts[i] - referrerLPShare, treasury);
                } else {
                    // Any remaining LP tokens are sent to the Keepers-powered
                    // LP token processor.
                ...
                // Contract assumes that all deposits are unique tokens,
                // i.e. the array does not contain duplicate addresses.
                // Hence it can safely break the inner loop here.
                break;
            }
        }
    ...
    function _removeLiquidity(
        ...
        IUniswapV2Router01 router = IUniswapV2Router01(lpTokenProcessor.getRouter(lpTokenAddress));
        address WETHAddress = router.WETH();
        IUniswapV2Pair lpToken = IUniswapV2Pair(lpTokenAddress);
        lpToken.approve(address(router), lpTokenAmount);
