...
/** 
 * @title  Disciplina_Plataforma_Ethereum_PUCMG202202
...
contract DisciplinaPlataformaEthereumPUCMG202202 {
    ...
    PucCoin private token;

    // modificador para verificar se é professor
    modifier apenasProfessor() {
        require(msg.sender == professor, "Apenas o professor pode realizar essa operacaoo.");
        _;
    }
    ...
    constructor(PucCoin coin) public {
    ...
    function registra(string memory nome, address contrato) public {
        ...
        require(!aluno.registrado, "Aluno ja esta registrado na disciplina.");
        
        ContratoAlunoPUCMG202202 contratoAluno = ContratoAlunoPUCMG202202(contrato);
        address owner = contratoAluno.getOwner();
        require(owner == msg.sender, "O contrato informado nao pertence ao aluno.");
    ...
    /* Operação para desregistrar o aluno da disciplina */
    function desregistra(address addAluno) public apenasProfessor {
        Aluno storage aluno = alunos[addAluno];
        require(aluno.registrado, "Aluno nao esta registrado na disciplina.");
        require(!aluno.resgateLiberado, "Nao e possivel desregistrar. O resgate ja foi liberado.");
 
        aluno.registrado = false;
        ContratoAlunoPUCMG202202 contratoAluno = ContratoAlunoPUCMG202202(aluno.contrato);
        contratoAluno.desregistra();
    ...
    function liberaResgate(address addAluno) public apenasProfessor {
        ...
        require(aluno.registrado, "Aluno nao esta registrado na disciplina.");
        require(!aluno.resgateLiberado, "O resgate ja foi liberado.");
    ...
    function abrirSessaoDeAula(uint aula) public apenasProfessor {
        require(aula < 5, "Numero invalido para a aula.");
    ...
    function checkin(uint aula) private {
        ...
        require(aluno.registrado, "Aluno nao esta registrado na disciplina.");
        require(sessaoAulaAberta[aula], "Sessao de aula nao esta aberta para checkin.");

        if (aula == 0) {
            require(!aluno.checkinAula01, "Aluno ja fez checkin.");
            aluno.checkinAula01 = true;
        }
        else if (aula == 1) {
            require(!aluno.checkinAula02, "Aluno ja fez checkin.");
            aluno.checkinAula02 = true;
        }
        else if (aula == 2) {
            require(!aluno.checkinAula03, "Aluno ja fez checkin."); 
            aluno.checkinAula03 = true;
        }
        else if (aula == 3) {
            require(!aluno.checkinAula04, "Aluno ja fez checkin.");
            aluno.checkinAula04 = true;
        }
        else if (aula == 4) {
            require(!aluno.checkinAula05, "Aluno ja fez checkin.");
...
contract ContratoAlunoPUCMG202202 {
    ...
    PucCoin private token;
    address private aluno;
    address private professor;              // conta do professor na Ethereum
    address private contratoDisciplina;

    bool resgateLiberado = false;

    // modificador para verificar se é professor
    modifier apenasContratoDisciplina() {
        require(msg.sender == contratoDisciplina, "Apenas o contrato da disciplina pode realizar essa operacao.");
        _;
    }

    // modificador para verificar se é professor
    modifier apenasAluno() {
        require(msg.sender == aluno, "Apenas o aluno dono do contrato pode realizar essa operacao.");
        _;
    }

    constructor(address _contratoDisciplina) public {
        aluno = msg.sender;
    ...
    function desregistra() public apenasContratoDisciplina {
    ...
    function resgate() public apenasAluno {
        require(resgateLiberado, "O resgate nao foi liberado pelo professor.");
