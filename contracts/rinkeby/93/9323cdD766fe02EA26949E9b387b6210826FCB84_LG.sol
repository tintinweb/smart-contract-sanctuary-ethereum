 ...
 * via governance vote.
 */
contract LG is ERC20_1155Votes, AccessControl, AllocationControl, EmissionCurves, IAllocationMinter {
    ...
    constructor(address daoMultisig, string memory baseURI)
        ERC20_1155Votes("Loci Global", "LG", baseURI, 2)
    ...
    function votingWeight(uint256 id)
        public
        pure
        returns (uint256)
    {
    ...
    function votingPower(uint256 id, uint256 amount)
        ...
        returns (uint256)
    {
        return amount * votingWeight(id) / allocationSupplyCapPerTokenId(id);
    }
