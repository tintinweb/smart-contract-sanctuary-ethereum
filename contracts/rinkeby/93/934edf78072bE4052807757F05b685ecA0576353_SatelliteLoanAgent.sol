    ...
    CommonModifiers
{
     ...
     */
    function borrow(uint256 borrowAmount) public virtual override nonReentrant {
...
abstract contract LoanAgentAdmin is ILoanAgent, LoanAgentModifiers {
    function setPUSD(address newPUSD) public virtual onlyOwner {
        PUSD = newPUSD;
    }
    ...
    function setMasterCID(uint16 _cid) public virtual onlyOwner {
        masterCID = _cid;
    }

    function setMasterMiddleLayerAddress(address _master)
        public
        virtual
        onlyOwner
    {
...
abstract contract LoanAgentMessageHandler is ILoanAgent, LoanAgentModifiers, LoanAgentEvents {
    function _sendBorrow(address user, uint256 amount)
        internal
        virtual
        override
    {
    ...
    function borrowApproved(
        ...
        public virtual override onlyMid()
    {
        uint256 accountBorrowsNew;
        uint256 totalBorrowsNew;

        emit IsBorrowAllowed(borrower, borrowAmount, true);
        ...
        IPUSD(PUSD).mint(borrower, borrowAmount);

        /* We write the previously calculated values into storage */
        accountBorrows[borrower] = borrowBalanceStored(borrower) + borrowAmount;
...
abstract contract LoanAgentInternals is ILoanAgent {
    function _borrow(uint256 borrowAmount)
        internal
        virtual
        override
    /* nonReentrant */
    ...
    function _repayBorrowFresh(
        ...
        uint256 _accountBorrows = accountBorrows[borrower];

        /* If repayAmount == -1, repayAmount = accountBorrows */
        repayAmount = repayAmount == type(uint256).max
            ? _accountBorrows
            : repayAmount;
        ...
        */
        ERC20Burnable(PUSD).burnFrom(payer, repayAmount); // burn the pusd
         ...
         */
        require(
            _accountBorrows >= repayAmount,
            "Repay greater than borrows"
        );
        accountBorrows[borrower] = _accountBorrows - repayAmount;
        ...
        // ? Is this updated in master?
        // accountBorrows[borrower].interestIndex = borrowIndex;

        bytes memory payload = abi.encode(IHelper.MRepay({
            selector: IHelper.Selector.MASTER_REPAY,
            borrower: borrower,
            prevAmount: _accountBorrows,
