...
contract KittyCards is ERC721('ZenNFTPunks', 'ZNFTP'), ERC721Enumerable, Ownable, ReentrancyGuard {
    ...
    function getUnredeemedTokens(address user) public view returns(uint256[] memory) {    
        uint256 balance = balanceOf(user);
        uint256[] memory tokens = new uint256[](balance);
        for (uint256 i; i < balance; i++) {
            uint256 token  = tokenOfOwnerByIndex(user, i);
            if(redeemed[token]==address(0)){
                tokens[i] = token;
            }
        }
        return(tokens);
