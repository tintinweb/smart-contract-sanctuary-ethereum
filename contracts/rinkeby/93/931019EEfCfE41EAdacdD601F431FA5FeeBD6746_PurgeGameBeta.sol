...
contract PurgeGameBeta is ERC721, ERC721Enumerable, Pausable, Ownable
{
    ...
    mapping(uint16 => address) public MAPtokenAddress;

    mapping(uint16 => uint8[8]) public tokenTraits;
    mapping(uint16 => uint16) public traitRemaining;
    mapping(uint16 => address[]) public traitPurgeAddress;

    //mapping(uint16 => address) public purgeAddress;
    //mapping(address => uint16[]) public tokensPurgedByAddress;


    uint8[] private colorRarityMath =[20,20,15,10,10,10,10,5];
    uint8[] private shapeRarityMath =[20,20,15,10,10,10,10,5];

    uint16 winningTrait;
    address exterminator;

    uint16 public MAPtokens = 0;
...
*/
    function setTraits(uint16 _tokenId) private
    {
        ...
        tokenTraits[_tokenId][0] = rarity(uint8(randomHash % 100), true);
        tokenTraits[_tokenId][1] = rarity(uint8((randomHash / 100) % 100), false);
        tokenTraits[_tokenId][2] = rarity(uint8((randomHash / 10000) % 100), true);
        tokenTraits[_tokenId][3] = rarity(uint8((randomHash / 1000000) % 100), false);
        tokenTraits[_tokenId][4] = rarity(uint8((randomHash / 100000000) % 100), true);
        tokenTraits[_tokenId][5] = rarity(uint8((randomHash / 10000000000) % 100), false);
        tokenTraits[_tokenId][6] = rarity(uint8((randomHash / 100000000000) % 100), true);
        tokenTraits[_tokenId][7] = rarity(uint8((randomHash / 10000000000000) % 100), false);

        if (_tokenId < 40000)
        {
            addTraitRemaining(tokenTraits[_tokenId][0] * 10 + tokenTraits[_tokenId][1] + 100);
            addTraitRemaining(tokenTraits[_tokenId][2] * 10 + tokenTraits[_tokenId][3] + 200);
            addTraitRemaining(tokenTraits[_tokenId][4] * 10 + tokenTraits[_tokenId][5] + 300);
            addTraitRemaining(tokenTraits[_tokenId][6] * 10 + tokenTraits[_tokenId][7] + 400);
        }
    }

    function addTraitRemaining(uint16 trait) private 
    {
        traitRemaining[trait] += 1;
    }

    function removeTraitRemaining(uint16 trait) private 
    {
...
*/
    function rarity(uint8 entropy, bool shapeColor) private view returns(uint8)
    {
        uint8 i = 0;
        uint8 count = 0;
        while (entropy > i)
        {
            ...
            if (shapeColor == false) 
            {
                i += colorRarityMath[count];
            }
            else
            {
                i += shapeRarityMath[count];
            }
            count++;
        }
        return(count);
    ...
    function purge(uint16[] calldata _tokenIds) public  
    {
        ...
        for(uint16 i = 0; i < _tokenIds.length; i++) 
        {
            ...
            _burn(_tokenId);
            removeTraitRemaining(tokenTraits[_tokenId][0] * 10 + tokenTraits[_tokenId][1] + 100);
            removeTraitRemaining(tokenTraits[_tokenId][2] * 10 + tokenTraits[_tokenId][3] + 200);
            removeTraitRemaining(tokenTraits[_tokenId][4] * 10 + tokenTraits[_tokenId][5] + 300);
            removeTraitRemaining(tokenTraits[_tokenId][6] * 10 + tokenTraits[_tokenId][7] + 400);
           // purgeAddress[_tokenId] = msg.sender;
    ...
    {
        require(gameOver == false, "Game Over");
        uint16 _randomToken = randomtoken();
        address tokenowner = ownerOf(_randomToken);
        PurgedCoinInterface(purgedCoinContract).mintFromPurge(tokenowner, cost * 1000);

        // FIX TRAIT ADDRESS SAVING

        removeTraitRemaining(tokenTraits[_randomToken][0] * 10 + tokenTraits[_randomToken][1] + 100);
        removeTraitRemaining(tokenTraits[_randomToken][2] * 10 + tokenTraits[_randomToken][3] + 200);
        removeTraitRemaining(tokenTraits[_randomToken][4] * 10 + tokenTraits[_randomToken][5] + 300);
        removeTraitRemaining(tokenTraits[_randomToken][6] * 10 + tokenTraits[_randomToken][7] + 400);
        if (gameOver == true) {exterminator = tokenowner;}
        _burn(_randomToken);
        //purgeAddress[_randomToken] = tokenowner;
        //tokensPurgedByAddress[tokenowner].push(_randomToken);
        emit Purge("Purged",_randomToken,tokenowner);
    }
    
    function randomtoken() private view returns (uint16) 
    {
        uint randomHash = uint(keccak256(abi.encodePacked(block.difficulty, block.timestamp)));
        return uint16(tokenByIndex(randomHash % totalSupply()));
    } 
    ...
    function newmintAndPurge(uint16 _number) external payable 
    {
       ...
       mapTokenNumber += MAPtokens;
        for(uint16 i= 0; i < _number; i++)
        {
            ...
            traitPurgeAddress[tokenTraits[mapTokenNumber][0] * 10 + tokenTraits[mapTokenNumber][1] + 100].push(msg.sender);
            traitPurgeAddress[tokenTraits[mapTokenNumber][2] * 10 + tokenTraits[mapTokenNumber][3] + 200].push(msg.sender);
            traitPurgeAddress[tokenTraits[mapTokenNumber][4] * 10 + tokenTraits[mapTokenNumber][5] + 300].push(msg.sender);
            traitPurgeAddress[tokenTraits[mapTokenNumber][6] * 10 + tokenTraits[mapTokenNumber][7] + 400].push(msg.sender);
            MAPtokens ++;
            mapTokenNumber++;
        }
        emit MintAndPurge("mintandpurge",_number,msg.sender);
