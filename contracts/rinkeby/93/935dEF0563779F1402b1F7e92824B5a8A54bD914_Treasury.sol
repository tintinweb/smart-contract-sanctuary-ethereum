...
contract Treasury is Ownable, ITreasury, CommonModifiers {
     ...
     */
    function buyPUSD(address stablecoinAddress, uint256 _PUSDamount)
        external
        override
        nonReentrant
        returns (bool)
    {
        ...
        uint256 stablecoinAmount = (buyPrice * _PUSDamount) / multiplier;
        ...
        stablecoinReserves[stablecoinAddress] =
            stablecoinReserves[stablecoinAddress] +
            stablecoinAmount; // increment the reserves

        stablecoin.transferFrom(msg.sender, address(this), stablecoinAmount); // transfer the stablecoin in
        pusd.mint(msg.sender, _PUSDamount); // mint the user PUSD
        ...
        returns (bool)
    {
        ...
        stablecoinReserves[stablecoinAddress] =
            stablecoinReserves[stablecoinAddress] -
            stablecoinAmount; // decrement that reserves

        pusd.burnFrom(msg.sender, _PUSDamount); // burn the pusd
        stablecoin.transfer(msg.sender, stablecoinAmount); // transfer out the stablecoin
    ...
    function deposit(address tokenAddress, uint256 amount) external override nonReentrant {
        ...
        stablecoinReserves[tokenAddress] =
            stablecoinReserves[tokenAddress] +
            amount;
    ...
    function withdraw(
        ...
        stablecoinReserves[tokenAddress] =
            stablecoinReserves[tokenAddress] -
            amount; // decrement the reserves
        token.transfer(recipient, amount); // transfer out the token in reserves
    }
     ...
     */
    function addReserveStablecoin(address stablecoinAddress)
        public
        override
        onlyOwner
    {
     ...
     */
    function setPUSDAddress(address _pusdAddress) external override onlyOwner {
        pusdAddress = _pusdAddress;
    }
     ...
     */
    function setLoanAgent(address _loanAgentAddress)
        external
        override
        onlyOwner
    {
...
abstract contract ITreasury is Ownable {
     ...
     */
    function setPUSDAddress(address pusd) external virtual;

    /**
     * @notice Sets the address of the loan agent contract
     * @param loanAgentAddress Address of the new loan agent contract
     */
    function setLoanAgent(address loanAgentAddress) external virtual;
