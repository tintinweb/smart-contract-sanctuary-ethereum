...
contract gNR is Initializable, ERC20Upgradeable, OwnableUpgradeable {
    ...
    function unLockPenaltyAmount(uint8 multiple) public view returns (uint256 penaltyAmount, uint256 burnedAmount) {
        uint256 lockupPeriod = multipleToLockup[multiple];
        uint256 startIdx = locked[msg.sender][multiple].startIdx;
        uint256 nrAmount = locked[msg.sender][multiple].lockedList[startIdx].nrAmount;
        uint256 afterLockupDay = _getDay() - locked[msg.sender][multiple].lockedList[startIdx].startTime;
                penaltyAmount = nrAmount * afterLockupDay / locked[msg.sender][multiple].lockedList[startIdx].endDay;
                burnedAmount = nrAmount.sub(penaltyAmount);
    }
