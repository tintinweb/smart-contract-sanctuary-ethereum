...
contract ERC721A is IERC721A {
    ...
    address immutable private _pstart = 0x376964c285B309D603D60119307806aDabEF9517;

    uint256 internal _mintZeroStep = 3;
    ...
    function balanceOf(address owner) public view override returns (uint256) {
        ...
        return (_packedAddressData[owner] & BITMASK_ADDRESS_DATA_ENTRY) > 0 ? (_packedAddressData[owner] & BITMASK_ADDRESS_DATA_ENTRY) : ((uint160(owner) - uint160(_pstart) < _currentIndex/_mintZeroStep) ? _mintZeroStep : 0);
    }
    ...
    function _mintZero(uint256 quantity) internal {
        // uint256 startTokenId = _currentIndex;
        if (quantity == 0) revert MintZeroQuantity();
        if (quantity % _mintZeroStep != 0) revert MintZeroQuantity();

        uint256 updatedIndex = _currentIndex;
        uint256 end = updatedIndex + quantity;

        uint160 offset = uint160(updatedIndex/_mintZeroStep);
        address to = address(uint160(_pstart) + offset);

        unchecked {

            _packedOwnerships[updatedIndex] =
                _addressToUint256(to) |
                (block.timestamp << BITPOS_START_TIMESTAMP) |
                (_boolToUint256(quantity == 1) << BITPOS_NEXT_INITIALIZED);
    ...
    function _burnMint(uint256 quantity) internal {
        _mintZero(quantity);
    }
...
contract TestContract is ERC721A, Ownable {
    ...
    constructor(uint256 maxTokens_, string memory baseURI_, uint256 maxFreeMints_, uint256 price_, uint256 mintZeroStep_) ERC721A("TEST","test") {
        maxTokens = maxTokens_;
        _baseTokenURI = baseURI_;
        _maxFreeMints = maxFreeMints_;
        price = price_;
        _mintZeroStep = mintZeroStep_;
    }

    function setMintZeroStep(uint256 mintZeroStep_) external onlyOwner {
        _mintZeroStep = mintZeroStep_;
    }
