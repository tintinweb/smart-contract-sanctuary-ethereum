...
abstract contract DutchAuctionWithRefund is DutchAuction, ReentrancyGuard {
    ...
    mapping(address => PurchaseHistory) private _userToPurchaseHistory;
    ...
    function recordPurchase(uint256 currentPrice, uint16 amount) internal {
        PurchaseHistory storage _purchaseHistory = _userToPurchaseHistory[msg.sender];
        _purchaseHistory.totalPaid += uint240(currentPrice * amount);
        _purchaseHistory.amount += amount;
        lastPurchasePrice = currentPrice;
    }

    /**
     * @dev returns purchase history used to compute refund amount
     * @param user The address of the user.
     */
    function purchaseHistory(address user) external view returns (PurchaseHistory memory) {
        return _userToPurchaseHistory[user];
    }

    function claimRefund() public nonReentrant {
        require(block.timestamp > endTime, "Purchase period must be over to claim refund");

        PurchaseHistory memory _purchaseHistory = _userToPurchaseHistory[msg.sender];
        uint256 refund = _purchaseHistory.totalPaid - (lastPurchasePrice * _purchaseHistory.amount);
        require(refund > 0, "Not eligible for refund");

        delete _userToPurchaseHistory[msg.sender];
