...
contract ETHGorillas is Ownable, ERC721 {
    ...
    uint public constant MAX_PURCHASE = 20; // set 1 to high to avoid some gas
    ...
    address public MutantApe = 0x60E4d786628Fea6478F785A6d7e704777c86a7c6; 
    address public BoredApe = 0xBC4CA0EdA7647A8aB7C2061c2E118A18a936f13D; 
	address public LemonFriends = 0x0B22fE0a2995C5389AC093400e52471DCa8BB48a; 
	address public DeadFellaz = 0x2acAb3DEa77832C09420663b0E1cB386031bA17B;	
    address public FangGang = 0x9d418C2Cae665D877F909a725402EbD3A0742844;
    address public Noundles = 0x4cFF01DBed00A5E95D705F96ACF369F210C203c8;	
    address public proxyRegistryAddress = 0xa5409ec958C83C3f309868babACA7c86DCB077c1;
    address private constant Developer = 0xb96766BE58Df259F071c7d704142D3403c7545C1;
  
    
    event priceChange(address _by, uint256 price);
    event PaymentReleased(address to, uint256 amount);
    
    constructor() ERC721("ETH Gorillas", "ETHG") {
        _baseTokenURI = ""; 
        _tokenSupply.increment();
		_safeMint( Developer, 0);
    ...
    */
    ...
    function reserveTokens(address to,uint numberOfTokens) public onlyOwner {    
        ...
        require(numberOfTokens < MAX_RESERVE, "Can only mint 20 tokens at a time");
    ...
    * Set mintPass contract address
    */
    function setAzukiPass(address newAddress) external onlyOwner {
         Azuki = newAddress;
    }
    /**    
    * Set mintPass contract address
    */
    function setCoolCatsPass(address newAddress) external onlyOwner {
         CoolCats = newAddress;
    }
    /**    
    * Set mintPass contract address
    ...
    * Set mintPass contract address
    */
    function setMutantApePass(address newAddress) external onlyOwner {
         MutantApe = newAddress;
    }
	/**    
    * Set mintPass contract address
    */
    function setBoredApePass(address newAddress) external onlyOwner {
         BoredApe = newAddress;
    }
    /**
    * Set mintPass contract address
    */
    function setLemonFriendsPass(address newAddress) external onlyOwner {
         LemonFriends = newAddress;
    }
	
    /**    
    * Set mintPass contract address
    */
    function setDeadFellazPass(address newAddress) external onlyOwner {
         DeadFellaz = newAddress;
    }
	
	 /**    
    * Set mintPass contract address
    */
    function setFangGangPass(address newAddress) external onlyOwner {
         FangGang = newAddress;
    }
    ...
    */
    ...
    function hasMintPass(address sender) public view returns (bool){
        if(sender==address(0)){
            return false;
        } else if (IERC721(Azuki).balanceOf(sender)>0){
            return true;
        } else if (IERC721(CoolCats).balanceOf(sender)>0){
            return true;
        } else if (IERC721(Doodles).balanceOf(sender)>0){
            return true;
        } else if (IERC721(MutantApe).balanceOf(sender)>0){
            return true;
        } else if (IERC721(BoredApe).balanceOf(sender)>0){
            return true;
		} else if (IERC721(LemonFriends).balanceOf(sender)>0){
            return true;
		} else if (IERC721(DeadFellaz).balanceOf(sender)>0){
            return true;
        } else if (IERC721(FangGang).balanceOf(sender)>0){
            return true;
