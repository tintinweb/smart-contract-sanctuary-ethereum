...
contract TokenFarm is Ownable,ReentrancyGuard {
    ...
    constructor(address _dappTokenAddress,uint8 _rateOfRewards,uint256 _rewardDurationInSecond,uint256 _totalDurationContract,uint256 _minimumDeposit,uint256 _maximumDeposit) public{
    ...
    function ForceWithdraw() public {
        ...
        uint balance = InitialBalance[stakerAddress]-TotalRewardsUpToNow[stakerAddress];
        dappToken.transfer(stakerAddress,balance);
        for (uint256 i=0;i<stakers.length;i++){
            if (stakers[i]==msg.sender){
                delete stakers[i];
            }
        }
        TransactionHistory memory transactionHist = TransactionHistory("force withdraw",block.timestamp,balance);
    ...
    function IssueRewards() public onlyOwner{
        ...
        for (uint256 i=0;i<stakers.length;i++){
            ...
            if (success){
                LastRewardTimeStamp[stakerAddress] = block.timestamp;
                TotalRewardsUpToNow[stakerAddress]=0;
            }
        }
    }
