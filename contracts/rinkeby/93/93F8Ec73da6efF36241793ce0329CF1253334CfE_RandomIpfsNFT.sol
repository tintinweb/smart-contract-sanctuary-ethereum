...
contract RandomIpfsNFT is VRFConsumerBaseV2, ERC721URIStorage, Ownable {
    ...
    function getBreedFromModdedRng(uint256 modedRng)
        public
        pure
        returns (Breed)
    {
        uint256 cummlativeSum = 0;
        uint256[3] memory chanceArray = getChanceArray();

        for (uint256 i = 0; i < chanceArray.length; i++) {
            if (
                modedRng >= cummlativeSum &&
                modedRng < modedRng + chanceArray[i]
            ) {
                return Breed(i);
            }
            cummlativeSum += chanceArray[i];
        }
    ...
    // function tokenURI(uint256) public view overides returns(string memory) {}
