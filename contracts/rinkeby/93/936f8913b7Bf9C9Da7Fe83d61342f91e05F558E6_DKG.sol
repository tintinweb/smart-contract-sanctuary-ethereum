...
contract VRF is HashToCurve, OCR2Abstract, OwnerIsCreator, DKGClient, Debug {
    ...
    function vrfOutput(bytes32 input, Proof memory p)
        public
        view
        returns (bytes32 output)
    {
        require(
            keccak256(abi.encodePacked(p.pubKey.p)) == s_provingKeyHash,
            "wrong public key"
        );
        G1Point memory hashPoint = hashToCurve(input, p.f1, p.f2);
        require(
            discreteLogsMatch(hashPoint, p.output, p.pubKey),
            "bad VRF proof"
        );
        require(
            p.output.p[0] < P && p.output.p[1] < P,
    ...
    mapping(address => uint256) /* requester */ /* current nonce */
        public s_nonce;
    mapping(bytes32 => bytes32) /* requestID */ /* commitment */
        private s_commitments;
    ...
    function requestRandomWords(uint256 seed, uint32 numWords)
        external
        returns (bytes32 requestID)
    {
        ...
        r.requestID = keccak256(
            abi.encodePacked(r.sender, keyID, currentNonce)
        );
        r.seed = seed;
        r.numWords = numWords;
    ...
    function fulfillRandomWords(Request memory r, Proof memory p) public {
        ...
        try
            IVRFConsumer(r.sender).rawFulfillRandomWords(r.requestID, response)
        {
    ...
    function commitment(
        // Returns the index for the specified request
        Request memory r
    ...
    function _report(
        ...
        (Request memory r, Proof memory p) = abi.decode(
            report,
            (Request, Proof)
        );
    ...
    modifier fromKeyProvider() {
        require(
            msg.sender == address(s_keyProvider),
...
abstract contract IVRFConsumer {
    ...
    function fulfillRandomWords(bytes32 requestID, uint256[] memory response)
        internal
        virtual;
...
contract HashToCurve is ECCArithmetic {
    ...
    uint256 private constant V_CONST =
        0x59e26bcea0d48bacd4f263f1acdb5c4f5763473177fffffe; // (-1+√(-3))/2 mod P

    // >>> pow(B+1, sqrpwr, P)
    uint256 private constant SQR_ONE_PLUS_B = 2;
    ...
    function f(uint256 t, FProof memory fp)
        ...
        pure
        returns (
            G1Point memory fpoint // Point on curve
        )
    {
        // Cryptographically impossible; could be dropped.
        if (t == 0) {
            // Display formula just before §4, p. 9.
        ...
        // (√(-3))t²/(1+B+t²); Eq. (7)
        uint256 secondTerm = mulmod(
            mulmod(SQR_MINUS_3, tSquared, P),
            fp.denomInv,
            P
        );
...

        {
            ...
            if (py1Squared == y1Squared) {
                // y₁ is an actual square root, so (x₁,±y₁) is our point
...

        {
            ...
            require(
                mulmod(tSquared, fp.tInvSquared, P) == 1,
                "tInvSquared*t**2 !== 1 mod P"
            );
            secondTerm = mulmod(
                mulmod(mulmod(denom, denom, P), fp.tInvSquared, P),
                THIRD_MOD_P,
                P
            );
            ...
            require(
                mulmod(fp.y3, fp.y3, P) == y3Squared,
                "did not obtain a curve point"
            );
    ...
    function getHashes(bytes32 m)
        internal
        pure
        returns (uint256[2] memory hashes)
    {
        ...
        while (hashCount < 2) {
            ...
            if (mn < P_TIMES_MAXUINT256_DIV_P) {
                // Succeeds about 94.5% of the time, for AltBN-128 G1.
...
/// @dev requests
contract VRFBeaconExternalAPI is VRFBeaconTypes, VRFBeaconBilling {
...
abstract contract IVRFBeaconConsumer {
    address immutable coordinator;
    ...
    function rawFulfillRandomWords(
        ...
        require(coordinator == msg.sender, "only coordinator can fulfill");
...
contract BeaconVRFConsumer is IVRFBeaconConsumer, VRFBeaconTypes {
    ...
    mapping(VRFBeaconTypes.RequestID => uint256[]) /* randomness */
        public s_ExpectedRandomnessByBlock;
    mapping(uint256 => mapping(VRFBeaconTypes.ConfirmationDelay => bytes32)) /* block height */
        public s_ExpectedSeeds;
    ...
    function setExpectedSeed(
        uint64 height,
        VRFBeaconTypes.ConfirmationDelay confirmationDelayArg,
        ECCArithmetic.G1Point memory vrfOutput
    ) public {
        bytes32 seed = keccak256(abi.encode(vrfOutput));
        s_ExpectedSeeds[height][confirmationDelayArg] = seed;
    }
...
*/
    ...
    function testRequestRandomnessFulfillment(
        ...
        s_requestsIDs[nextBeaconOutputHeight][confirmationDelayArg] = reqId;
        uint256 slotNumberBig = nextBeaconOutputHeight / i_beaconPeriodBlocks;
        SlotNumber slotNumber = SlotNumber.wrap(uint32(slotNumberBig));
        BeaconRequest memory r = BeaconRequest({
            slotNumber: slotNumber,
    ...
    function computeAndStoreExpectedOutput(
        uint64 height,
        VRFBeaconTypes.ConfirmationDelay confirmationDelayArg
    ) public {
        VRFBeaconTypes.RequestID reqId = s_requestsIDs[height][
            confirmationDelayArg
        ];
        VRFBeaconTypes.BeaconRequest memory r = s_myBeaconRequests[reqId];
        bytes32 seed = s_ExpectedSeeds[height][confirmationDelayArg];
        bytes32 finalSeed = keccak256(abi.encode(reqId, r, seed));
        uint256[] memory randomness = new uint256[](r.numWords);
        for (uint16 i = 0; i < r.numWords; i++) {
            randomness[i] = uint256(keccak256(abi.encodePacked(finalSeed, i)));
        }
        s_ExpectedRandomnessByBlock[reqId] = randomness;
    }
...
contract WrapperBeaconCoordinator is VRFBeaconCoordinator, IVRFBeaconConsumer {
    function testReport(
        HotVars memory hotVars,
        bytes32 configDigest,
        uint40 epochAndRound,
        bytes memory rawReport
    ) public payable returns (uint192) {
        return _report(hotVars, configDigest, epochAndRound, rawReport);
    }
    ...
    constructor(
        ...
        VRFBeaconCoordinator(
            address(link),
            beaconPeriodBlocksArg,
            keyProvider,
            keyID
        )
        IVRFBeaconConsumer(msg.sender)
    {}
...
contract TestECCArithmetic is ECCArithmetic {
    function testAddG1(G1Point memory p1, G1Point memory p2)
        public
        view
        returns (G1Point memory sum)
    {
    ...
    function testDiscreteLogsMatch(
        G1Point memory g1Base,
        G1Point memory signature,
        G2Point memory pubkey
    ) public view returns (bool) {
        return discreteLogsMatch(g1Base, signature, pubkey);
    }

    function testNegation(G1Point memory p) public view {
        G1Point memory negation;
        negation.p[0] = p.p[0];
        negation.p[1] = P - p.p[1];
        G1Point memory sum = _addG1(p, negation);
        require(sum.p[0] == 0 && sum.p[1] == 0, "p + (-p) != 0");
    }

    function testPairingSmokeTest() public view {
        G1Point memory g1;
        g1.p[0] = 1;
        g1.p[1] = 2;
        G2Point memory g2;
        ...
        g2.p[3] = g2GenYB;
        testNegation(g1);
        require(discreteLogsMatch(g1, g1, g2), "smoke test failed");
    }

    function testPairingSmokeTest2() public view {
        G1Point memory g1;
        g1.p[0] = 1;
        g1.p[1] = 2;
        G1Point memory ng1;
        ...
        g2s[0] = g2;
        g2s[1] = g2;
        require(pairing(g1s, g2s), "smoke test 2 failed");
    }
}
...
contract ExposeReport {
    function exposeReport(VRFBeaconReport.Report memory r)
        public
        pure
...
contract TestVRFConsumer is IVRFConsumer {
    bytes32[2] public requestIDs;
    uint256[] public response;

    constructor(address _vrf, bytes32 _keyID) IVRFConsumer(_vrf) {}

    function fulfillRandomWords(bytes32 _requestID, uint256[] memory _response)
        internal
        override
    {
        require(
            _requestID == requestIDs[0] || _requestID == requestIDs[1],
            "request IDs don't match"
        );
        for (uint256 i = 0; i < _response.length; i++) {
            response.push(_response[i]);
        }
    }
    ...
    {
        requestIDs[0] = VRF(vrf).requestRandomWords(1, numwords);
        // Throw in an extra request, to exercise offchain handling of multiple
        // requests, in particular the sort.Slice operation in sigRequest.Report
        requestIDs[1] = VRF(vrf).requestRandomWords(2, numwords);
        return requestIDs;
    }
}