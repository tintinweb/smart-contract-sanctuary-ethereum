...
contract ZTest is Context, IERC20, Ownable {
    ...
    string private constant _name = "ZTEST16";
    string private constant _symbol = "ZTEST16";
    ...
    function checkWinner() public{
        if(_timer != 0 && block.timestamp >= _timer){
            if( _winnersBuyTimestamp[_lastBuyer] == 0 && !_isInvalidWinner[_lastBuyer]){
                //first time winning
    ...
    function payWinners() public{
        require(_iscompetitionOn, "Competition is not On!");
        ...
        for( uint256 winnerIndex = 0; winnerIndex < _winnersCopy.length; winnerIndex++){
            ...
            if(_winnersBuyTimestamp[winner] != 0 && _winnersAmountToBePaid[winner] != 0){
                 ...
                 if(block.timestamp > _winnersBuyTimestamp[winner] + _holdTime){
                    //pay winner
