...
contract TestContract is Ownable {
    ...
    function verifyMessage(address addr, string memory message, bytes memory signature) public view  returns(address) {
        //hash the plain text message
        bytes32 messagehash =  keccak256(abi.encodePacked(addr, message));
        address signeraddress = messagehash.toEthSignedMessageHash().recover(signature);
        return signeraddress;
    }

    // function verifyMessage(string memory message, bytes memory signature) public view  returns(address) {
    //     //hash the plain text message
