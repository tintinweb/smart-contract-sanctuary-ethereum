...
contract Vulcan is ERC1155, Ownable {
    ...
    mapping(uint256 => bool) _tier_exists;
     ...
     */
    function _setApprovalForAll(address, address, bool) internal virtual override(ERC1155) {
    ...
    function buy(uint256 _tier) external payable {
        ...
        require(_tier_exists[_tier], "Vulcan/tier-not-found");
    ...
    function upgradeToTier(uint256 _prev, uint256 _next) external payable {
        ...
        require(_tier_exists[_prev] && _tier_exists[_next], "Vulcan/tiers-not-found");
    ...
    /**
     * @notice Burns token of tier `_tier` owned by account `dest`
     * @dev Prior ownership of token does not have to be checked because it is already done inside the ERC1155 `_burn` function
     */
    function purgeToken(address dest, uint256 amt, uint256 _tier) external onlyOwner {
        Tier memory tier = _tiers[_tier];

        _burn(dest, tier.id, amt);

        emit Purge(dest, tier.id);
    }

    /**
     * @notice Mints 1 token to each address in array `dests` of tier in the array `ids` at the same position
     */
    function mint(address[] calldata dests, uint256[] calldata ids) external onlyOwner {
        for (uint256 i; i < dests.length; i++) {
            uint256 tier = ids[i];
            address dest = dests[i];

            require(_tier_exists[tier], "Vulcan/tier-not-found");

            _mint(dest, tier, 1, "");

            emit OwnerMint(dest, tier);
        }
    }

    function addTier(Tier memory tier) external onlyOwner {
        require(!_tier_exists[tier.id], "Vulcan/tier-already-exists");

        _tiers[tier.id] = tier;
        _tier_exists[tier.id] = true;
    ...
    function withdraw(address payable dest, uint256 amt) external onlyOwner {
        dest.transfer(amt);
    }
}
