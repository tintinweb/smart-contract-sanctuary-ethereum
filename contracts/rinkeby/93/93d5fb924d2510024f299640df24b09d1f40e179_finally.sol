 ...
 */
 ...
 contract finally is Context, IERC20, IERC20Metadata, Ownable {
    ...
    event UpdateTaxPercentage(uint8 taxPercentage);
    ...
    function calculateTaxFee(uint256 amount) internal view returns (uint256){
        return (amount * _taxPercentage) / 10**2;
    } 
    ...
    function updateTaxEnable(bool taxStatus) external onlyOwner {
        ...
        emit UpdateTaxEnable(_taxStatus);
    }

    function updateTaxPercentage(uint8 taxPercent) external onlyOwner {
        _taxPercentage = taxPercent;
        emit UpdateTaxPercentage(_taxPercentage);
    }
