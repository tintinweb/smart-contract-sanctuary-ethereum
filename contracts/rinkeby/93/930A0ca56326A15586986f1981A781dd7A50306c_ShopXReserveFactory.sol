...
contract ShopXReserveNFT is ERC721A, Ownable, ERC2981ContractWideRoyalties, AccessControl, Pausable {
  ...
  */
  ...
  function updateCrossmintAddress (address _crossmintAddress) onlyRole(SHOPX_ADMIN) external {
    require(_crossmintAddress != address(0), "ReserveX: address zero is not a valid crossmintAddress");
 ...
 * @dev Sets the merkle root.
  ...
  * @dev function to set the base URI for all token IDs. It is
  ...
  function destroySmartContract(address _caller, address payable _to) external {
    require(msg.sender == factory, "ReserveNFT: FORBIDDEN");
    require(hasRole(SHOPX_ADMIN, _caller) == true, "ReserveNFT: Caller is not a shopx admin");
    selfdestruct(_to);
  }
...
* @dev Mints a token to the msg.sender. It consumes whitelisted supplies.
  ...
  /**
  * @dev Mints a token to an address with a tokenURI, but also checks if the destination address is in the allowList.
    * @param to address of the future owner of the token.
    */
  function crossMint(bytes32[] calldata merkleProof, address to, uint256 quantity) payable external whenNotPaused {
    require(msg.sender == crossmintAddress, "CrossMint: Caller is not CrossMint");
    require(totalSupply() + quantity <= maxSupply, "NotEnoughNFT: NFT supply not available");
    require(claimed[to] + quantity <= mintLimitPerWallet, "ReserveNFT: NFTs already claimed.");
    require(msg.value == mintPrice * quantity, "WrongETHAmount: More or less than the required amount of ETH sent.");
