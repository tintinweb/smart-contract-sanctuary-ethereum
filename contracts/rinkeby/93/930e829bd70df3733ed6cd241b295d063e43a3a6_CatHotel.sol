    ...
    UUPSUpgradeable
{
    ...
    function unstake(uint256 _catId) external nonReentrant {
        ...
        require(room.unlockTime < block.timestamp, "catHotel: cat in a cooling off period");

        room.unlockTime += coolingOffPeriod;

        emit Unstaked(msg.sender, _catId, room.unlockTime);
    }

    function claim(uint256 _catId) external nonReentrant {
        CatRoom storage room = checkinBooks[_catId];
        ...
        emit Claimed(msg.sender, _catId, block.timestamp);
    }
