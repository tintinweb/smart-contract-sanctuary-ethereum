...
contract StickyDescriptor is  IJellyContract, NFTDescriptor {
    ...
    function formatAmountSeperators(uint256 amount) internal view returns (string memory) {
        ...
        uint256 first = (amount / 10 ** 21) % 1000;
        uint256 second = (amount / 10 ** 24) % 1000;
        uint256 third = (amount / 10 ** 27) % 1000;
        uint256 fourth = (amount / 10 ** 30) % 1000;

        if (modAmount > 999999999) {
            return string(abi.encodePacked((fourth).toString(), ',', (third).toString(), ',', (second).toString(),',', (first).toString()));
        } else if (modAmount > 999999) {
            return string(abi.encodePacked((third).toString(),',',  (second).toString(),',',  (first).toString()));
        } else if (modAmount > 999) {
            return string(abi.encodePacked((second).toString(), ',', (first).toString()));
        } else  {
