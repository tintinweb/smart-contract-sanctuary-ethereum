...
*/
contract Comic2Apes is PVERC721, PVAllowlist, PVPaymentSplitter {
    uint256 constant MAX_PURCHASE = 750;

    uint256 mintPrice = 1e15;
    uint256 stageLength = 60;

    uint256 purchaseWindowOpens;
    uint256 purchaseWindowOpensPublic;

    bytes32[] merkleRoots;

    IEliteApeHelper public eliteApeHelper;
    ...
    constructor (
        ...
        eliteApeHelper = IEliteApeHelper(_eliteApeHelper);    

        merkleRoots = _merkleRoots;
    }    

    function setStageLength(uint256 _stageLength) external onlyOwner {
        stageLength = _stageLength;
    }

    function setMerkleRoots(bytes32[] calldata _merkleRoots) external onlyOwner {
        merkleRoots = _merkleRoots;
    }

    function setHelper(address _eliteApeHelper) external onlyOwner {
        eliteApeHelper = IEliteApeHelper(_eliteApeHelper);    
    }

    function ownerMint (uint256 amount, address to) external onlyOwner {
        require(totalSupply() + amount <= MAX_PURCHASE, "max supply exceeded");
        ...
        whenInAllowlist(index, maxAmount, merkleProof, merkleRoots[stage-1]) 
    {
        ...
        require(eliteApeHelper.purchased(msg.sender) + amount <= maxAmount , "max amount exceeded");

        require(block.timestamp >= purchaseWindowOpens && (block.timestamp - purchaseWindowOpens) / stageLength >= stage-1, "stage not open yet"); 

        eliteApeHelper.burnAndIncrease(msg.sender, amount);
    ...
    function purchasePublic(uint256 amount) external payable whenNotPaused {
        require(msg.value == mintPrice * amount, "Purchase: payment incorrect");
        require(totalSupply() + amount <= MAX_PURCHASE, "Purchase: max purchase supply exceeded");
 ...
 * function.
 */
contract PVPaymentSplitter is Context {
