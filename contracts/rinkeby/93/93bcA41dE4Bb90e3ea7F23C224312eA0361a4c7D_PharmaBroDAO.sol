...
contract PharmaBroDAO is ERC721Enumerable, ReentrancyGuard, Ownable {
    ...
    function mint(uint256 quantity) public payable nonReentrant {
        uint256 currId = totalSupply();
        require(
            currId + quantity <= maxTokens,
            "Minting this many would exceed max supply of 7777!"
        );
        require(
        ...
        returns (string memory)
    {
        ...
        return string(
          abi.encodePacked(
            'https://gateway.pinata.cloud/ipfs/QmdY46x128HwsvEa19wdy8VDFGjN339x5CMN5QWLQfDZUC/',
            Utils.toString((tokenId%2 + 1))
          )
        );
    }
