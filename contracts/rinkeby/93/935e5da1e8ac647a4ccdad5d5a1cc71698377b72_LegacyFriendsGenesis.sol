...
contract LegacyFriendsGenesis is ERC721A, Ownable, ReentrancyGuard {
    ...
    uint256 public minTokenIdPerTimeframe = 1;
    ...
    modifier mintPriceCompliance(uint256 _mintAmount) {
        if(_mintAmount >= 10) {
            require(msg.value >= discountedCost * _mintAmount, "Insufficient funds!");
        } else {
    ...
    modifier tokenMintPriceCompliance(uint256 _mintAmount, uint256 _tokenamount) {
        require(_tokenamount * _mintAmount >= GetAllowance(), "Please approve tokens before transferring");
        require(_tokenamount * _mintAmount >= tokenCost, "Insufficient funds!");
    ...
    function setDiscountedCost(uint256 _discountedCost) public onlyOwner {
        discountedCost = _discountedCost;
    }
