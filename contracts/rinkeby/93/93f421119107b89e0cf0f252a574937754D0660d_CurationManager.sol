...
contract CurationManager is Ownable {
    ...
    event SeasonPaused(address sender);

    event SeasonResumed(address sender);
    ...
    constructor(
        ...
        isActive = _isActive;
        if (isActive == true) {
            emit SeasonResumed(_msgSender());
        } else {
            emit SeasonPaused(_msgSender());
        }
    }
