...
contract BeamStarterLaunchPad is  Ownable{
	...
	function buy(address _token, uint _quantity) public payable {
		...
		uint amount = _quantity;
		if (remainToken <= pool.minPurchase.mul(10 ** tokenDecimals)) {
			pool.closeTime = _now;
			amount = remainToken; 
			isClean = true;
		}
		if (isClean || (amount >=  pool.minPurchase.mul(10 ** tokenDecimals) && amount <=  pool.maxPurchase.mul(10 ** tokenDecimals))) {
			pool.totalSold = pool.totalSold.add(amount.div(10 ** tokenDecimals));
			uint total = amount.mul(pool.price).div(10 ** (tokenDecimals));
			if(pool.receiveToken == address(0)){
				require(msg.value == total, 'Pools: not enough value');
			} else {
				ERC20(pool.receiveToken).safeTransferFrom(msg.sender, address(this), total);
			}
			ERC20(_token).safeTransfer(msg.sender, amount);
			emit Buy(_token, msg.sender, amount);
		}
	}
