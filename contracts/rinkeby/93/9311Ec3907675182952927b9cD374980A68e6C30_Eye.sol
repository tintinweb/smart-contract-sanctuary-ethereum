    ...
    ReentrancyGuard
{
    ...
    mapping(address => bool) public friendMintClaimed;
    ...
    uint16 private _namePrefixCount;
    uint16 private _nameSuffixCount;
    uint16 private _orderCount;
    /* -------------------------- COLLECTION CURATION --------------------------- */
    mapping(uint16 => bytes32) public stories;
    uint16[] public storyList;
    ...
    modifier onlyIfNotAlreadyClaimedFriendMint() {
        if (friendMintClaimed[msg.sender]) revert MintAlreadyClaimed();
        _;
    }
    ...
    constructor(
        ...
        _namePrefixCount = IEyeConstants(_eyeConstantsAddress)
            .getNamePrefixCount();
        _nameSuffixCount = IEyeConstants(_eyeConstantsAddress)
            .getNameSuffixCount();
        _orderCount = IEyeConstants(_eyeConstantsAddress).getOrderCount();
    }
    ...
    function mintWindranger(uint256 amount)
        external
        onlyIfMintingPhaseIsSetToOrAfter(Phase.PREMINT)
    ...
    function mintFriend(bytes32[] calldata merkleProof)
        external
        ...
        nonReentrant
    {
        ...
        friendMintClaimed[msg.sender] = true;
    }
    ...
    function addStory(uint16 storyId, bytes32 librariumId)
        external
        onlyCurator
    {
    ...
    function removeStory(uint16 storyId) external onlyCurator {
        stories[storyId] = "";
    ...
    function getStories() external view returns (Story[] memory storiesList) {
        storiesList = new Story[](storyList.length);
        for (uint16 i = 0; i < storyList.length; i++) {
            storiesList[i] = Story(storyList[i], stories[storyList[i]]);
        }
    }
    ...
    function getNamePrefix(uint256 tokenId)
        ...
        returns (string memory)
    {
        // return _randomEYE(_toString(tokenId)) % _namePrefixCount;
        return
            IEyeConstants(_eyeConstantsAddress).getNamePrefix(
                _randomEYE(_toString(tokenId)) % _namePrefixCount
            );
    }
    ...
    function getNameSuffix(uint256 tokenId)
        ...
        returns (string memory)
    {
        //return _randomEYE(_toString(tokenId)) % _nameSuffixCount;
        return
            IEyeConstants(_eyeConstantsAddress).getNameSuffix(
                _randomEYE(_toString(tokenId)) % _nameSuffixCount
            );
    }

    /// @notice Get the NFTs name based on the Loot contract pluck function.
    function getName(uint256 tokenId) public view returns (string memory) {
        string memory name = "Genesis Eye";
    ...
    function getOrderIndex(uint256 tokenId) public view returns (uint256) {
        return _randomEYE(_toString(tokenId)) % _orderCount;
    }
