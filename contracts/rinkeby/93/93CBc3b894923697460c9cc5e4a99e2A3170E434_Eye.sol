...
contract Eye is ERC721URIStorage, Ownable, ReentrancyGuard {
	...
	string public baseTokenURI = "ipfs://QmTAaLfx6dNXbjV41EKbpW4ey7A7C5f3RXzgTbq1fDv8Eu/";
		...
		nonReentrant
	{
		if (mintStatus == MintStatus.PRESALE) {
			...
			require(_genesisAmount > 0, "Must mint at least 11 genesis NFT");
	...
	function rand() public view returns (uint256) {
		...
		return (seed - ((seed / 5) * 5));
	}

	function _createTokenUri() private returns (string memory) {
		uint256 randomNo = rand();
		string memory tokenUri = string(abi.encodePacked(baseTokenURI, randomNo.toString()));
		return tokenUri;		
	}
