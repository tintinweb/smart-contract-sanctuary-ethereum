...
contract ERC721ABreeding is ERC721A, Ownable {
    ...
    function getAvailable(address _address, uint256 _type) public view returns (uint256) {
    ...
    // function getAvailable(address _owner, uint[] memory _males, uint[] memory _females) public view returns (uint[] memory) {
    //     ParentContract parentContract = ParentContract(_contract);
    //     uint balance = parentContract.balanceOf(_owner);
    //     uint[] memory tokens = new uint[](balance);
    //     for (uint i=0; i < balance; i++) {
    //         tokens[i] = (IERC721Enumerable(_contract).tokenOfOwnerByIndex(_owner, i));
    //     }
    //     return tokens;
    // }
