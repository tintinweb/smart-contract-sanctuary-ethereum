...
contract FitnessFriends is Ownable, ERC721A {
    string private _baseTokenURI;
    uint16 public supply = 10000;

    struct SaleConfig {
        bytes32 merkleRoot;
        uint256 price;
        uint16 maxMint;
        uint16 supplyCeiling; // Need to pass in the supply ceiling after the tier
        uint256 time;
    }
    ...
    constructor() ERC721A("Fitness Friends", "FIT") {}
    ...
    function setConfig(
        ...
        uint256 price,
        uint16 maxMint,
        uint16 supplyCeiling,
        uint256 time
    ) public onlyOwner {
        tierToConfig[tier] = SaleConfig(
            merkleRoot,
            price,
            maxMint,
            supplyCeiling,
    ...
    function tierMint(
    ...
    ) public payable isMintValid(tier, numToMint) {
        ...
        require(
            MerkleProof.verify(
                merkleProof,
                merkleRoot,
                keccak256(abi.encodePacked(msg.sender, maxMint))
            ),
    ...
    function devMint(
        ...
        address to,
        uint16 numToMint
    ) public payable onlyOwner isMintValid(tier, numToMint) {
    ...
    modifier isMintValid(string calldata tier, uint16 numToMint) {
        SaleConfig storage config = tierToConfig[tier];
        uint16 maxMint = config.maxMint;
        uint16 supplyCeiling = config.supplyCeiling;
        ...
        require(
            numberMinted(msg.sender) + numToMint < supplyCeiling + 1,
            "Too many minted"
        );
