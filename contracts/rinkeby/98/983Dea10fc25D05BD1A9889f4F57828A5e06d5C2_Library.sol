...
contract Library is Initializable, UUPSUpgradeable {
  ...
  event Record(string title, string author, address authorWallet, string content, string[] tags);
  event Revoke(bytes id);

  error NoPublishAccess();
  error NotOwner();

  function initialize(address owner_, address writePassContract_) public initializer {
  ...
  function revoke(bytes memory id) 
    public 
    onlyValidPublishAccess()
    {
  ...
  function addPublisher(address newPublisher) 
    public 
    onlyOwner()
    {
  ...
  function removePublisher(uint i) 
    public 
    onlyOwner()
    {
