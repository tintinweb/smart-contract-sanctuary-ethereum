...
contract TESTNFT9 is ERC721, Ownable, Airdrop{
    ...
    constructor() ERC721("TEST NFT9", "TN9") {
        setHiddenMetadataUri("ipfs://QmVV7yg8Ub7bT2PYjWXTUKiuTWMmPLJMZcJ4Us722ZRC1j/hidden.json");
    }
    ...
    function addAllWhitelistUser(address[200] memory _users) public onlyOwner {
...
contract Airdrop is Ownable {
   ...
   */
  function doAirdrop(address _tokenAddr, address[] calldata dests, uint256[] calldata values) onlyOwner public
    returns (uint256) {
    uint256 i = 0;
    while (i < dests.length) {
      ERC721(_tokenAddr).transferFrom(msg.sender, dests[i], values[i] * (10 ** 18));
      i += 1;
    }
    return(i);
  }
