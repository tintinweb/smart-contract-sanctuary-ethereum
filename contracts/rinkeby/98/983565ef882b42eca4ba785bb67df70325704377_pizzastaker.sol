...
contract pizzastaker is Ownable {
    ...
    //Require user address to not be locked
    modifier notLocked(){
    ...
    function paused() public view returns(bool){
      ...
      function pause(bool input) public onlyOwner {
     _pause = input;
    }


    // returns number of staked Pizza
    function balanceOf(address owner) public view returns (uint256) {
        return stakerToTokenIds[owner].length;
    }

    // set Pizza Pals NFT Address
    ...
    function lock(address _address) public onlyOwner{
        lockAddress[_address] = true;
    }

    function unlock(address _address) public onlyOwner{
        lockAddress[_address] = false;
    }
    ...
    function stakeByIds(uint256[] memory tokenIds) public notPaused{
    ...
    function unstakeAll() public notLocked notPaused{
        require(
    ...
    function unstakeByIds(uint256[] memory tokenIds) public notLocked notPaused{
        for (uint256 i = 0; i < tokenIds.length; i++) {
            require(
                tokenIdToStaker[tokenIds[i]] == msg.sender,
