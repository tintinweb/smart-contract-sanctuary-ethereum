...
contract NFT is ERC721URIStorage, ERC721Enumerable, ERC2981PerTokenRoyalties {
    ...
    mapping(uint256 => Revealed) private revealeds;
    ...
    constructor(string memory _name, string memory _symbol, address _owner, string memory _contractURI, address[] memory whitelistAddresses)
    ...
    function createToken(string memory _tokenURI, address _recipient, uint256 _amount)
        public
        returns (uint256)
    {
        if (msg.sender != owner) {
            require(
                ...
                "Only whitelisted addresses can create tokens"
            );
        }
        ...
        revealeds[newItemId] = Revealed(newItemId, "", true);
        unlockables[newItemId] = UnLockable(newItemId, "", false);
        return newItemId;
    }
    ...
    function createToken(
        ...
        if (_isRevealed) {
            revealeds[newItemId] = Revealed(newItemId, _notRevealedUri, true);
        } else {
            revealeds[newItemId] = Revealed(newItemId, _notRevealedUri, false);
        }
        if (_locked) {
            unlockables[newItemId] = UnLockable(newItemId, _unlockedUri, true);
        } else {
            unlockables[newItemId] = UnLockable(newItemId, _unlockedUri, false);
        }
        return newItemId;
    }
    ...
    function revealTokenURI(uint256 tokenId) public returns (bool) {
        require(revealeds[tokenId].revealed == false, "Already revealed");
        require(
            _isApprovedOrOwner(_msgSender(), tokenId),
            "ERC721: caller is not owner nor approved"
        );
        revealeds[tokenId].revealed = true;
        ...
        returns (string memory)
    {
        ...
        if (revealeds[tokenId].revealed == false) {
            return string(abi.encodePacked(_baseURI(), revealeds[tokenId].notRevealedUri));
        } else {
            return super.tokenURI(tokenId);
        }
    }
    ...
    function addWhitelistedAddress(address _address)
        public
        onlyOwner
        returns (bool)
    {
    ...
    function removeWhitelistedAddress(address _addressToRemove)
        external
        onlyOwner
    {
...
contract NFTAuction is ERC2981Base {
    using Counters for Counters.Counter;
    Counters.Counter public totalReturns;
    ...
    function bid(uint256 _bidAmount) public onlyNotOwner {
        require(cancelled == false, "Cancelled");
        require(_bidAmount >= minBidPrice, "bid is less than min bid price");
        ...
        emit highestBidIncrease(msg.sender, _bidAmount);
    }

    function withdraw(uint256 _bidId) public returns (bool) {
    ...
    function isAuctionTimePassed() public view returns (bool) {
        if (block.timestamp < auctionEndTime) {
            return false;
        }
        return true;
    ...
    function auctionEnd() external {
        ...
        if (ended) {
            revert("Auction ended called already.");
        }
        ended = true;
        if (highestBid != 0) {
            uint256 commission = (highestBid * commissionRate) / 10000;
            uint256 salePrice = highestBid - commission;
            IERC20(tokenContract).transfer(marketOwner, commission); // transfer commission to platform owner

            RoyaltyInfo memory royalty = NFT(nftContract).royaltyInfo(tokenId);
            if (
                address(royalty.recipient) != payable(address(0)) &&
                royalty.amount > 0
            ) {
                uint256 royaltyFee = (royalty.amount * salePrice) / 10000;
                IERC20(tokenContract).transfer(royalty.recipient, royaltyFee);
                salePrice = salePrice - royaltyFee;
            }
            IERC20(tokenContract).transfer(beneficiary, salePrice);
            emit AuctionEnded(highestBidder, highestBid);
        }
        for (uint256 i = 0; i < totalReturns.current(); i++) {
            if (
                !bids[i + 1].withdrawn &&
                pendingReturns[bids[i + 1].bidder] != 0
            ) {
                IERC20(tokenContract).transfer(
                    bids[i + 1].bidder,
                    pendingReturns[bids[i + 1].bidder]
                );
                pendingReturns[bids[i + 1].bidder] = 0;
                bids[i + 1].withdrawn = true;
            }
        }
        emit AuctionEnded(highestBidder, highestBid);
    }
    ...
    function cancel() external {
        ...
        if (highestBid != 0) {
            IERC20(tokenContract).transfer(highestBidder, highestBid);
            for (uint256 i = 0; i < totalReturns.current(); i++) {
                if (
                    !bids[i + 1].withdrawn &&
                    pendingReturns[bids[i + 1].bidder] != 0
