...
contract Presale is Ownable {
    ...
    event tokenPurchased(address buyer, uint256 price, uint256 tokenValue);
    ...
    receive() external payable {
        // If anyone transfer ETH then sender will receive token
        buy();
    }
    ...
    function buy() public payable returns (bool) {
        ...
        uint256 _tokenValue = msg.value.mul(price);
        require(
            _tokenValue <= tokenForSale(),
            "Remaining tokens less than buying token"
        );

        // transfer ETH seller address
    ...
    function withdrawToken(IERC20 _tokenAddress)
        public
        onlyOwner
        returns (bool)
    {
