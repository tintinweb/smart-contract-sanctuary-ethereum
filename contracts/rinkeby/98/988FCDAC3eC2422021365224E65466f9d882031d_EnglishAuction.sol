...
contract EnglishAuction is ERC721 {
    ...
    mapping ( uint => uint[] ) public topBidsYet;
    mapping ( uint => address[] ) public topBidsAddress;
    ...
    /**
    * for unrevealed after deploying the smart contracts put base uri in it
    * EX: ipfs://<HASH>
    */
    ...
    function createNewAuction(uint _endAt,uint _lowestbid,uint _NoOfWinners) public onlyOwner returns(bool) {
        forceEnd();
        AuctionNo+=1;
    ...
    function bid() external payable {
        ...
        // Top Bids Yet
        topBidsYet[AuctionNo].push( msg.value );
        topBidsAddress[AuctionNo].push( msg.sender );
    ...
    function ClaimsAndRefunds( uint _auctionNo ) public {
        ...
        require (
            block.timestamp > a.endAt && a.ended == true,
    ...
    function checkTopBids(uint _AuctionNo ) public view returns ( uint[] memory ) {
        return topBidsYet[_AuctionNo]; 
    }
    function checkTopBidsAddress(uint _AuctionNo ) public view returns ( address[] memory ) {
        return topBidsAddress[_AuctionNo]; 
    }

    function getAllBids(uint _AuctionNo) public view returns ( uint[] memory _bids, address[] memory _address ) {
        _bids = topBidsYet[_AuctionNo]; 
        _address = topBidsAddress[_AuctionNo]; 
    }
