...
abstract contract NonblockingReceiver is ILayerZeroReceiver {
    ...
    function failedMessagesTest(uint16 _srcChainId, bytes memory _srcAddress, uint64 _nonce, bytes calldata _payload) external view 
    returns(uint, uint, bytes32, bytes32){
        // assert there is message to retry
        FailedMessages storage failedMsg = failedMessages[_srcChainId][_srcAddress][_nonce];
        require(failedMsg.payloadHash != bytes32(0), "NonblockingReceiver: no stored message");
        return(_payload.length, failedMsg.payloadLength, keccak256(_payload), failedMsg.payloadHash);
