...
contract YecheLangeCakes is ERC721A, Ownable, ReentrancyGuard {  
    ...
    address public pieContractAddress = 0xd48c3bc7fb60aA437Cb99F70AB2AC4Ae9A956690;

    bool public pieHolderMint = true;
    bool public mintPaused = true;
    bool public baseURILocked = false;

    uint256 private mintPrice = 0.1 ether;

    uint256 private pieDiscountPct = 0; 
    ...
    uint256 private reservedGifts = 50; 
    ...
    constructor() ERC721A("YecheLangeCakes", "CAKES") {
    ...
    function updatePieHolderMint(bool _pieHolderMint) public onlyOwner {
        pieHolderMint = _pieHolderMint;
    }
    ...
    function updatePieDiscountPercent(uint256 _pieDiscountPct) public onlyOwner {
        pieDiscountPct = _pieDiscountPct;
    }
    ...
    function checkPrice(uint256 quantity, address user) public view returns (uint256) {
        ...
        if(hasPie) {
            price = price * (100 - pieDiscountPct) / 100;
        }
    ...
    function mint(uint256 quantity) public payable nonReentrant callerIsUser {
        ...
        bool hasPie = checkPieHolder(msg.sender);

        if(pieHolderMint) {
            require(hasPie, "must have a pie to mint");
        }
