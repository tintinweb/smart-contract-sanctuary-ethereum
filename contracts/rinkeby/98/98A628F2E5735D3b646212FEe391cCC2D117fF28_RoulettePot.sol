...
contract RoulettePot is Ownable, ReentrancyGuard {
    ...
    event RouletteWon(uint256 tokenId, address winner, uint256 nonce, Bet[] bets, uint256 amount);
    event RouletteLost(uint256 tokenId, address loser, uint256 nonce, Bet[] bets);
    ...
    function placeBetsWithTokens(uint256 tokenId, Bet[] calldata bets) external nonReentrant {
        ...
        if (totalReward > 0) {
            ...
            emit RouletteWon(tokenId, msg.sender, nonce, bets, totalReward);
        } else {
            emit RouletteLost(tokenId, msg.sender, nonce, bets);
        }
