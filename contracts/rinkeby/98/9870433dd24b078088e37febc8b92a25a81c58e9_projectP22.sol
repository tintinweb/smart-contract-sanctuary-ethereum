...
contract projectP22 is ERC1155, Ownable {
    ...
    function addNewRelease(uint32 newPackId, string memory newPackName, string memory newPackType, string memory description, uint32 numOfTokensInPack, uint32 maxPackSupplyInPack, uint128 price) external onlyOwner {
    ...
    function checkIndexIsAvailbaleTest(uint randomNumber, uint32 packId) public returns (uint) {
        uint newImageIndex = checkIndexIsAvailbale(randomNumber, packId);
