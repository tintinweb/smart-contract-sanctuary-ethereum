...
contract merkleWhitelist is Delegated, ERC721EnumerableLite, PaymentSplitterMod {
    ...
    uint public maxPremiumMint   = 4; // number of tokens that can be minted per premium address
    uint public salePrice        = 0.125 ether; // price during public mint
    uint public whitelistPrice   = 0.08 ether; // price for whitelisted minters
    uint public premiumPrice     = 0.02 ether; // price for premium minters
    bool public isPresaleActive;
    bool public isPremiumActive;
    ...
    event MintPremium(address indexed to, uint tokenId);
    ...
    modifier premiumMint {
        require(isPremiumActive, "Premium mint is not active!");
        _;
    }
    ...
    function setActive(bool isPresaleActive_, bool isMintActive_, bool isPremiumActive_) external onlyDelegates {
        require(isPresaleActive != isPresaleActive_ || isMintActive != isMintActive_ || isPremiumActive != isPremiumActive_, "New value matches old");
        isPresaleActive = isPresaleActive_;
        isMintActive = isMintActive_;
        isPremiumActive = isPremiumActive_;
    }
    ...
    function setPrice(uint whitelistPrice_, uint salePrice_, uint premiumPrice_) external onlyDelegates {
        require(whitelistPrice != whitelistPrice_ || salePrice != salePrice_ || premiumPrice != premiumPrice_, "New value matches old");
    ...
    mapping(address => uint) public addressToPremiumMinted; // mapping for premium to minted
    bytes32 public merkleRoot = 0xfbaa96a1f7806c1ab06f957c8fc6e60875b6880254f77b71439c7854a6b47755;
    bytes32 public premiumMerkleRoot = 0xfbaa96a1f7806c1ab06f957c8fc6e60875b6880254f77b71439c7854a6b47755;

    function setMaxWhitelistMint(uint amount_) public onlyDelegates {
        maxWhitelistMint = amount_;
    }

    function setMaxPremiumMint(uint amount_) public onlyDelegates {
        maxPremiumMint = amount_;
    }
    ...
    // Premium Whitelist Mint Functions
    function setPremiumMerkleRoot(bytes32 merkleRoot_) public onlyDelegates {
        premiumMerkleRoot = merkleRoot_;
    }

    function isPremium(bytes32[] memory proof_) public view returns (bool) {
        bytes32 _leaf = keccak256(abi.encodePacked(msg.sender));
        
        for (uint i = 0; i < proof_.length; i++) {
            _leaf = _leaf < proof_[i] ? keccak256(abi.encodePacked(_leaf, proof_[i])) : keccak256(abi.encodePacked(proof_[i], _leaf));
        }
        return _leaf == premiumMerkleRoot;
    }

    function mintPremium(bytes32[] memory proof_, uint amount_) public payable onlySender premiumMint {
        require(isPremium(proof_), "You are not a premium member!");
        require(addressToPremiumMinted[msg.sender] + amount_ <= maxPremiumMint, "You do not have enough premium mints available!");
        require(msg.value == premiumPrice * amount_, "Invalid value sent!");

        uint _supply = totalSupply();
        require(_supply + amount_ <= maxTokens, "Mint request exceeds supply!");

        addressToPremiumMinted[msg.sender]++; // increments the tracker so that they cannot mint more than allowed

        for(uint i; i < amount_; ++i){
            uint _mintId = _supply++;
            _mint(msg.sender, _mintId);
            emit MintPremium(msg.sender, _mintId);
        }
