...
contract Library {
    ...
    function check721Compliancy(address _contract) public view returns (bool) {
        require(IERC721(_contract).supportsInterface(0x80ac58cd), "No IERC721 implemented");

        try IERC721(_contract).balanceOf(0xdb8FFd3c97C1263ccf6AD75e43d46ecc65ef702a) returns (uint256 _count)  {
    ...
    function check721EnumerableCompliancy(address _contract) public view returns (bool) {
        require(check721Compliancy(_contract), "No IERC721 implemented");
        require(IERC721Enumerable(_contract).supportsInterface(0x780e9d63), "No IERC721Enumerable implemented");
        ...
        try IERC721Enumerable(_contract).tokenOfOwnerByIndex(owner_example, 0) returns (uint256 _first_nft_owned)  {
        }  catch Error(string memory reason) {
            return false;
        }

        try IERC721Enumerable(_contract).balanceOf(owner_example) returns (uint256 _first_nft_owned)  {
        }  catch Error(string memory reason) {
            return false;
        }
