...
import "./token/ONFT721.sol";
import "@openzeppelin/contracts/token/ERC20/extensions/ERC20Pausable.sol";
import "@openzeppelin/contracts/security/ReentrancyGuard.sol";
import "@openzeppelin/contracts/utils/cryptography/MerkleProof.sol";

// File: contracts/FireGoose.sol

/*

 _______ _________ _______  _______    _______  _______  _______  _______  _______ 
(  ____ \\__   __/(  ____ )(  ____ \  (  ____ \(  ___  )(  ___  )(  ____ \(  ____ \
| (    \/   ) (   | (    )|| (    \/  | (    \/| (   ) || (   ) || (    \/| (    \/
| (__       | |   | (____)|| (__      | |      | |   | || |   | || (_____ | (__    
|  __)      | |   |     __)|  __)     | | ____ | |   | || |   | |(_____  )|  __)   
| (         | |   | (\ (   | (        | | \_  )| |   | || |   | |      ) || (      
| )      ___) (___| ) \ \__| (____/\  | (___) || (___) || (___) |/\____) || (____/\
...
/**
 * @title FireGoose contract
 * @dev OmniChain ERC721 Non-Fungible Token Standard basic implementation
 */
contract FireGoose is ONFT721, Pausable, ReentrancyGuard {
    // Constant variables
    // ------------------------------------------------------------------------
    ...
    function collectReserves(uint256 numberOfTokens) external onlyOwner {
        require(
            nextMintId + numberOfTokens <= RESERVES,
        ...
        onlyWhitelistSale
    {
        ...
        require(nextMintId + numberOfTokens <= maxMintId, "Exceed max supply");
        require(
            numberOfTokens <= MAX_PER_TRANSACTION,
            "Exceeds per transaction"
        );
        require(
            purchased[_msgSender()] + numberOfTokens <= MAX_PER_ADDRESS,
            "Exceeds per address max supply"
        );

        purchased[_msgSender()] = purchased[_msgSender()] + numberOfTokens;
