...
contract ALIEN is ERC721A, Ownable, ReentrancyGuard {
    ...
    mapping(address => bool) private founderslistMinted;
...
// Mint tokens to the given address if that address has founder tokens
    function foundersMint(address _address, bytes32[] memory proof) public payable onlySender nonReentrant {
        ...
        //get the amount of founder token
        uint256 tokensCount = IERC721(0x6A7cCcf637a65A239101D37D1a91F38EE5f93bE6).balanceOf(_address);
        require(tokensCount > 0, "Insufficient Tokens");
        require(founderslistMinted[_address] == false, "You have already minted");
        uint256 mintAmount = 6 * tokensCount;
        require((totalSupply() + mintAmount) <= MAX_SUPPLY, "Sold out!");
        require(isAddressOnFounderslist(proof, _address), "You are not eligible for a Founders mint");
        founderslistMinted[_address] = true; 
        _mintWithoutValidation(_address, mintAmount);
    }
