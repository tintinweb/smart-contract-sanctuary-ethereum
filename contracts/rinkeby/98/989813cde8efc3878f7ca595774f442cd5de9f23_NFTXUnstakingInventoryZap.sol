...
contract NFTXUnstakingInventoryZap is Ownable, ReentrancyGuard {
    ...
    function unstakeInventory(
        ...
        if (remainingPortionToUnstake == 10e17) {
            ...
            if ((reqXTokens * shareValue) / 10e17 < numNfts * 10e17) {
                ...
                if (xToken.balanceOf(msg.sender) > reqXTokens) {
                    console.log("reqXTokens += 1");
                    reqXTokens += 1;
                } else if (xToken.balanceOf(msg.sender) == reqXTokens) {
                    console.log("shortByRoundingError");
                    shortByRoundingError = true;
                } else {
                    revert("not enough xTokens");
                }
            }
            if (remainingPortionToUnstake == 0) {
                xTokenAmount = reqXTokens;
            } else {
        ...
        if (shortByRoundingError) {
            // possible that vToken rounding error > 1, if xTokenShareValue > 2
            uint256 missingVToken = (numNfts * 10e17) -
                (vToken.balanceOf(address(this)) - initialVTokenBal);
            require(missingVToken < 1000, "possible underflow");
            if (
    ...
    function maxNftsUsingXToken(uint256 vaultId, address staker)
        public
        view
        returns (uint256 numNfts, bool oneWeiShort)
    {
        ...
        uint256 vTokensB = ((xTokenBal + 1) * shareValue) / 10e17;

        console.log("xTokenBal", xTokenBal);
        console.log("shareValue", shareValue);
        console.log("xTokenBal * shareValue", xTokenBal * shareValue);

        console.log("vTokensA", vTokensA);
        console.log("vTokensB", vTokensB);
        ...
        if (vTokensIntB > vTokensIntA) {
            ...
            // TODO: check if liquidity or vtoken balance avail
