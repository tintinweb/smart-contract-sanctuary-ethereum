...
contract TokenOperator is ITokenOperator, IERC1155Receiver, OriginMulticall {
    ...
    function _createLicense(
        ...
        require(earnPoint <= MAX_EARN_POINT, "over 5%");
    ...
    function _batchCreateLicense(
        ...
        for (uint256 i = 0; i < metas.length; i++) {
            ...
            require(totalPoint <= MAX_EARN_POINT, "over 5%");
        }
        _batchFeeProcess(amounts, expiredAts);
        token.batchCreate(_msgsender(), metas, amounts);
    }
    ...
    function _batchCreateDerivative(
        ...
        token.batchCreate(_msgsender(), derivativeMetas, amounts);
    }
    ...
    function _mint(
        ...
        if (token.kind() == TokenKind.OriLicense) {
            ...
            require(lMeta.earnPoint <= MAX_EARN_POINT, "over 5%");
    ...
    function batchMint(
        ...
        token.batchMint(_msgsender(), ids, amounts);
    }
    ...
    function batchBurn(
        ...
        require(_isEnableToken(address(token)), "not whitelist");
        require(ids.length == amounts.length, "invalid length");
        token.batchBurn(_msgsender(), ids, amounts);
    }
