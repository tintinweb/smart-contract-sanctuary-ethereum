    ...
    KeeperCompatibleInterface
{
        ...
        nonReentrant
    {
        ...
        paymentToken.transfer(address(this), tokenPricePerMint * amount);
        _queueMinter(amount);
    ...
    function requestRandomWords() public {
        uint32 mintersInQueue = uint32(pendingMinters[batchMintNonce].length);
        ...
        vrfIdToNonce[requestId] = batchMintNonce;
        nonceToVrfId[batchMintNonce] = requestId; // for manual failsafe
        batchMintNonce++;
    }
    ...
    function _queueMinter(uint256 amount) internal {
        require(
            pendingMinters[batchMintNonce].length + amount <=
                MAX_MINT_QUEUE_LENGTH,
            "Mint queue full. Please try again in a few minutes."
        );
    ...
    function _shouldSendBatch() internal view returns (bool) {
        ...
        if (mintersInQueue > MAX_MINT_QUEUE_LENGTH - 50) {
