...
import "./IGMGN.sol";
import "./IGMGNDetonationA.sol";

contract AirD is ERC721A, Ownable {
    using Strings for uint256;

    uint8 constant DETONATION_TYPES = 2;
    uint256 constant AIRDROP_LIMIT = 752;
    // TODO: REMOVE DEV ADDRESSES
    //IERC721 constant GENESIS_TOKEN_CONTRACT = IERC721(address(0x19b86299c21505cdf59cE63740B240A9C822b5E4));
    IERC721 constant GENESIS_TOKEN_CONTRACT = IERC721(address(0x862843A41e714813Cc7cf525f2C91c29a41B591f));
    // TODO: REMOVE DEV ADDRESSES
    //IGMGN constant GMGN_DYNAMITE_CONTRACT = IGMGN(address(0x390416aE4324494338293974EE6388E777faC34b));
    IGMGN constant GMGN_DYNAMITE_CONTRACT = IGMGN(address(0xA326A7cA6ec7caed31cEE01dE24a9da4DA2c8691));
    // TODO: REMOVE DEV ADDRESSES
    //GMGNDetonation constant GMGN_DETONATION_A_CONTRACT = GMGNDetonation(address());
    IGMGNDetonationA constant GMGN_DETONATION_A_CONTRACT = IGMGNDetonationA(address(0x25B4b180e0BfFaC1f15f0bc73f6E6D9d2bd369A5));
    ...
    function airdrop(uint256 endIndex) public onlyOwner {
        uint256 startIndex = _currentIndex;

        if (endIndex > AIRDROP_LIMIT) {
            revert AirdropLimitExceeded();
        }

        for (uint256 currentTokenId = startIndex; currentTokenId < endIndex; currentTokenId++) {
            (uint256 genesisTokenId, uint256 detonationType) = GMGN_DETONATION_A_CONTRACT.tokenData(currentTokenId);
            address owner = GMGN_DETONATION_A_CONTRACT.ownerOf(currentTokenId);
    ...
    function tmp() public onlyOwner {
        uriPrefix = '9qudb92b392b393b293b922bu3buwbdiub';
    }
}
...
interface IGMGNDetonationA is IERC721 {
