...
contract PackWoodERC1155V2 is ERC1155Upgradeable, OwnableUpgradeable {
    ...
    */
    ...
    function updateSKTCommunityWallet(address _account1, address _account2, address _account3) external onlyOwner {
    ...
    */
     ...
     *
     * @param to send to.
     * @param tokenIds array of token id
     * @param random supply of token id
     *
     * returns:
     * - bool.
     *
    ...
    */
    ...
    function buyCrossMint(address to, uint256[] calldata tokenIds, uint256[] calldata random) payable external returns(bool){
        ...
        for(uint256 i = 0; i < tokenIds.length; i++){
            if(random[i] > 0){
                _safeTransferFrom(owner(), to, tokenIds[i], random[i], "");
            } 
        }

        emit crossmintTokenDetails(owner(), to, tokenIds[0], tokenIds[1], tokenIds[2], random[0], random[1], random[2], msg.value);
    ...
    */
    ...
    function mintTo(address to, uint256 _count, uint256 _tokenId) payable external returns(bool){
        require(msg.sender == CrossMintAddress, "PackWoodERC1155: Method can be only called by Cross mint address");
        
        require(tokenPrice * _count == msg.value, "PackWoodERC1155: Price is incorrect");
        ...
        emit TransferSingle(owner(), owner(), to, _tokenId, _count);
