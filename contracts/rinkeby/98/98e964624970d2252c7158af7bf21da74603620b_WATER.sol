...
contract WATER is Context, IERC20, Ownable {
  ...
  address public buybackTokenAddress = address(0x07a1Bb827476fE6BC8B4D9544F5fc4F4A7fc46AD);
  ...
  bool private  _isBuyBackActivated = true;
  ...
  function _transfer(
    ...
    if (
      ...
      if (to != address(this) && from != address(this) && to != owner() && from != owner()) {
        require(amount <= _maxTxAmount, "TOKEN: Max Transaction Limit");
      }
    }

    if (to != uniswapV2Pair &&  !_isUniswapPair[to] && to != owner() && to != address(this)) {
  ...
  function setRewardsClaimTimeSeconds(uint256 _seconds) external onlyOwner {
    require(_seconds >= 0 &&_seconds <= 60 * 60 * 24, 'claim time delay must be greater or equal to 0 seconds and less than or equal to 24 hours');
    rewardsClaimTimeSeconds = _seconds;
  }

  function setReflectionFeePercent(uint256 _newFee) external onlyOwner {
    require(_newFee > 0 && _newFee <= 1, 'fee must be greater than 0% and less than or equal to 1%');
    reflectionFee = _newFee;
  }

  function setTreasuryFeePercent(uint256 _newFee) external onlyOwner {
    require(_newFee > 0 && _newFee <= 10, 'fee must be greater than 0% and less than or equal to 10%');
    treasuryFee = _newFee;
  }

  function setETHRewardsFeeFeePercent(uint256 _newFee) external onlyOwner {
    require(_newFee > 0 && _newFee <= 5, 'fee must be greater than 0% and less than or equal to 5%');
  ...
  function setIsBuyBackActivated(bool _value) public onlyOwner {
    _isBuyBackActivated = _value;
  }
