...
contract HyewonNft is IHyewonNft, ERC721A, Ownable, ReentrancyGuard, Pausable {
     ...
     */
    string public defaultUnrevealedURI;
        ...
        nonReentrant
    {
        ...
        if (!_isTokenAvailable(quantity, maxMintingId)) {
            revert ExceedMaximumForTheRound(maxMintingId - _nextTokenId());
        }
        ...
        returns (string memory)
    {
        ...
        if (round.revealed == false || round.randomSelection == 0) {
            if (
                keccak256(abi.encodePacked(round.unrevealedURI)) !=
    ...
    function newRound(
        ...
        string memory tokenURIPrefix, // tokenURIs folder cid
        ...
        string memory unrevealedURI, // image before reveal
     ...
     *  OnlyOwner functions
     */
    function setMintingFee(uint16 roundNumber, uint256 fee) public onlyOwner {
        Round storage round = rounds[roundNumber];
    ...
    {
        Round storage round = rounds[roundNumber];
        round.entropyPoint = block.number + 50;

        emit RevealProgress(roundNumber);
    }
    ...
    function setRandomSelection(uint16 roundNumber) public {
        ...
        uint256 entropyPoint = round.entropyPoint;

        if (entropyPoint > block.number) {
            revert BadRequest("Random selection is not ready");
        }

        bytes32 entropy = keccak256(
            abi.encodePacked(blockhash(entropyPoint), block.timestamp)
        );
    ...
    /**
     *  Set before-revealed uri for the current round
     *  If it is not set, use defaultUnrevealdUri
     *  For owner and admin
     */
    function setUnrevealedURI(string memory uri) external {
        setUnrevealedURI(currentRoundNumber, uri);
    }

    /**
     *  Set before-revealed uri for the specified round
     *  If it is not set, use defaultUnrevealdUri
     *  For owner and admin
     */
    function setUnrevealedURI(uint16 roundNumber, string memory uri)
        public
        onlyOwnerOrAdmin(roundNumber)
    {
        Round storage round = rounds[roundNumber];
        round.unrevealedURI = uri;

        emit UnrevealedURIUpdated(roundNumber, uri);
    }
     ...
     * Only for the owner
     */
    function setDefaultUnrealedURI(string memory _defaultUnrevealedURI)
        external
        onlyOwner
    {
        defaultUnrevealedURI = _defaultUnrevealedURI;
        ...
        private
    {
        ...
        // check if minting started
        if (
            currentRound.startTime != 0 &&
            block.timestamp < currentRound.startTime
        ) {
            revert MintingNotStarted(currentRound.startTime);
        }

        // check if minting ended
        if (
            currentRound.endTime != 0 && block.timestamp > currentRound.endTime
        ) {
            revert MintingEnded(currentRound.endTime);
        }
        ...
        if (!_isTokenAvailable(quantity, currentRound.maxMintingId)) {
            revert ExceedMaximumForTheRound(
                currentRound.maxMintingId - _nextTokenId()
            );
        }
    }
    ...
    function _updateWhitelistUsed(
        ...
        bytes32 hash = _getHash(_msgSender(), "whitelistClaim");
        if (!_verifySig(hash, v, r, s, admin)) {
    ...
    function _getHash(address sender, string memory message)
        private
        view
        returns (bytes32)
    {
        return keccak256(abi.encodePacked(address(this), sender, message));
    }
    ...
    function _verifySig(
        ...
        return signer == owner() || signer == admin;
    }
...
interface IHyewonNft {
    ...
    error ExceedMaximumForTheRound(uint256 remainingCount);
    ...
    struct Round {
        ...
        uint256 entropyPoint; // blocknubmer which entropy will calculated
        string unrevealedURI; // token uri before reveal
    ...
    event RevealProgress(uint16 roundNumber);
