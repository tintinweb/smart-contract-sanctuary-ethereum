...
contract PlatziPunks is ERC721, ERC721Enumerable, PlatziPunksDNA {
  ...
  function _paramsURI(uint256 _dna) internal view returns (string memory) {
    ...
    return string(abi.encodePacked(params, "&getTopType", getTopType(_dna)));
  }
  ...
  function tokenURI(uint256 tokenId)
    ...
    returns (string memory)
  {
    require(_exists(tokenId), "ERC721 Metadata: URI query for non existing token");
    ...
    string memory jsonURI = Base64.encode(
      abi.encodePacked(
        "{ 'name': 'PlatziPunks #', ",
        tokenId,
        "'descripcion': 'Platzi Punks are randomized Avataaars stored on chain to teach DApp development on Platzi', 'imagen': '",
        image,
        "'}"
      )
    );
