...
abstract contract IWhitelistAuction {
    function isBidderWhitelisted(address _bidderAddress) external view virtual returns(bool);
}
...
contract NFTAuction is Ownable, Pausable{
    ...
    struct Auction {
        ...
        address nftRecipient; //The bidder can specify a recipient for the N
    ...
    event AuctionSettled(
        address indexed nftContractAddress,
        uint256 indexed tokenId,
        address indexed auctionSettler
    );
    ...
    function addWhitelist(
        ...
        whenNotPaused()
    {
        nftContractAuctions[_nftContractAddress][_tokenId].whitelistAddress = _whitelistAddress;
    }
    ...
    {
        if(nftContractAuctions[_nftContractAddress][_tokenId].whitelistAddress != address(0)){
            require(IWhitelistAuction(nftContractAuctions[_nftContractAddress][_tokenId].whitelistAddress).isBidderWhitelisted(msg.sender), "Caller is not whitelisted");
        }
