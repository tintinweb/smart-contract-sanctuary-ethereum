...
contract ApeHaterClub is ERC721AQueryable, Ownable, ReentrancyGuard {    
    ...
    function MintFreeMutation(uint256 amount) external  {
        ...
        require(bnno.balanceOf(msg.sender, 0) > 0 && bnno.balanceOf(msg.sender, 0) >= amount,"Must own at least 1 Banano, or not exceed Bananos amount to mutate."); //testing this
/*
        uint256 totalSmash = 0;

        for (uint256 j = 0; j < amount;i++ ) {
            totalSmash++;
            require(totalSmash <= bnno.balanceOf(msg.sender, 0), "exceed bananos");
        }
