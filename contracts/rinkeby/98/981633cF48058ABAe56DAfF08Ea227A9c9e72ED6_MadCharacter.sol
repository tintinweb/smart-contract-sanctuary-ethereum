...
contract MadCharacter is ERC721A, Ownable {
    ...
    address immutable MADPASS;
    uint256 constant IDMADPASSNORMAL = 1;
    uint256 constant IDMADPASSTEAM = 2;
    ...
    function _beforeTokenTransfers(
        ...
        require(!_frozen[from], "MADCHARACTER: frozen");
        super._beforeTokenTransfers(from, to, startTokenId, quantity);
    }

    function _mintTeam(address _MADPASS, address teamAddress) internal {
        uint256 id_ = IDMADPASSTEAM;
        uint256 amount = IMadPass(_MADPASS).balanceOf(teamAddress, id_);
        IMadPass(_MADPASS).safeTransferFrom(
            msg.sender,
            address(this),
            id_,
            amount,
        ...
        IMadPass(_MADPASS).burn(id_, amount);
        _safeMint(teamAddress, amount);
    }

    /* ========== MUTATIVE FUNCTIONS ========== */

    // the mint function must be modified to burn an ERC1155 after approving it to the ERC721
    function mint(uint256 amount) external {
        require(canMint, "Minting did not start");
        uint256 id_ = IDMADPASSNORMAL;
        address _MADPASS = MADPASS;
        IMadPass(_MADPASS).safeTransferFrom(
            msg.sender,
            address(this),
            id_,
