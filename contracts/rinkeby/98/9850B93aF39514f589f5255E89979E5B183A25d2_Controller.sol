...
contract Controller is Controllable, IController {
  ...
  mapping(uint => uint) public override dungeonMultiplierByLevel;
  ...
  event DungeonMultiplierByLevelChanged(uint level, uint value);
  ...
  /// @dev Multiplier will be used for adding additional monster attributes
  function setDungeonMultiplierByLevel(uint level, uint value) external {
    onlyGovernance();
    // !!! no time-lock in alpha version !!!
    dungeonMultiplierByLevel[level] = value;
    emit DungeonMultiplierByLevelChanged(level, value);
  }
  ...
  function updateProxies(address[] memory proxies, address[] memory newLogics) external {
    ...
    for (uint i; i < proxies.length; i++) {
      ...
      emit ProxyUpdated(proxies[i], newLogics[i]);
    }
  }
  ...
  function registerDungeon(address dungeon_) external override {
    ...
    emit DungeonRegistered(dungeon_);
  }
