...
contract Barkhouse is ERC721A, Ownable, ReentrancyGuard {
    ...
    ERC721A public ApeDogContract;
    constructor(
        address _signer,
        address _apeContract) ERC721A("Barkhouse", "BARKHOUSE") {
        signer = _signer;
        ApeDogContract = ERC721A(_apeContract);
    }
    ...
    function publicMint(uint64 expireTime, bytes memory sig, uint256 tokenId) external isSecured(1) payable {
        ...
        require(ApeDogContract.ownerOf(tokenId) == msg.sender, "NOT_THE_OWNER");
        require(1 + totalSupply() <= MAX_SUPPLY,"NOT_ENOUGH_SUPPLY");
        require(!isClaimed[tokenId],"ALREADY_CLAIMED");
