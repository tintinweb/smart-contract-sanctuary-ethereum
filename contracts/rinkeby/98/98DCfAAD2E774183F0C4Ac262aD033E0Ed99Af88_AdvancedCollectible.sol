...
contract AdvancedCollectible is ERC721, VRFConsumerBase, Ownable {
    ...
    event modelAssigned(uint256 indexed tokenId, Model model);
    ...
    constructor(
        ...
        bytes32 _keyhash,
        uint256 _fee
    ) public VRFConsumerBase(_VRFCoordinator, _link) ERC721("BMW", "BMW") {
        keyhash = _keyhash;
        fee = _fee;
        tokenCounter = 0;
    }
    ...
    function createCollectible(uint256 _cid) public payable returns (bytes32) {
        require(
    ...
    function fulfillRandomness(bytes32 requestId, uint256 randomNumber)
        ...
        override
    {
        Model model = Model(randomNumber % 3);
        uint256 newTokenId = tokenCounter;
        tokenIdToModel[newTokenId] = model;
        emit modelAssigned(newTokenId, model);
