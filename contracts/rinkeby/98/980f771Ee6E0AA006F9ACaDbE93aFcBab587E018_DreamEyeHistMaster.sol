...
/* Features: 
 *   - Public sale (Max allowed public sale mints, public sale price, public sale enablement flag)
 *   - Stage NFT: 
 *        > Stage starts with 0
 *        > Owner can change the stage
 *        > User can change the stage
 *        > Only one stage will be active at a time  
 *   - Added historical contract feature
 */
...
contract DreamEyeHistMaster is IMasterContract, ERC721AQueryable, ERC721ABurnable, Ownable, ReentrancyGuard, ERC2981 {
  ...
  ISideCar[] public sideCars;
  mapping(address => uint256) public sideCarIndex;
  mapping (address => bool) public disabledSideCars;
  ...
  function addSideCar(address _sideCarContract) public onlyOwner {
    sideCars.push(ISideCar(_sideCarContract));
  ...
  function totalSupply() public view virtual override(ERC721A, IMasterContract) returns (uint256) {
    return ERC721A.totalSupply();
  }

  function name() public view override(ERC721A, IMasterContract) returns (string memory) {
    return ERC721A.name();
  }

  function symbol() public view override(ERC721A, IMasterContract) returns (string memory) {
  ...
  //TODO - Remove during prod deployment
...
interface ISideCar {
  function transferData( address from, address to, uint256 tokenId ) external;
}
...
interface IMasterContract {
  ...
  function disabledSideCars(address _address) external view returns (bool); 
}
