...
contract TestContract is ERC721, Ownable {
    ...
    uint256 public currentGameTimeStamp; //!!!! This might be exploitable?
    ...
    function guessWord(string memory inputString) public {
        ...
        require(
            ...
            "Maxed out guesses for this Nordle or already solved it!"
        );
    ...
    function checkWord(bytes memory inputChars)
        private
        returns (uint8[6] memory)
    {
        ...
        for (uint16 i = 0; i < maxLetters; i++) {
            // console.logBytes1(secretWordCharArray[i]);
            // console.log("-----");
            charCount[secretWordCharArray[i]] += 1; //Have to increment because if a key is not defined in a mapping it has a default value of 0
            // console.logBytes1(secretWordCharArray[i]);
        ...
        for (uint16 i = 0; i < maxLetters; i++) {
            ...
            // console.logBytes1(secretWordCharArray[i]);
            // console.logBytes1(inputChars[i]);
            // console.log(secretWordCharArray[i] == inputChars[i]);
            // console.log(i);
            // console.logBytes1(inputChars[i]);
            // console.log(charCount[inputChars[i]]);
            if (charCount[inputChars[i]] == 0) {
                // Char is not in array. prevent underflow
            } else if (secretWordCharArray[i] == inputChars[i]) {
                // Char is in the correct position
                indexStates[i] = 1;
                charCount[inputChars[i]] -= 1;
                // console.logBytes1(inputChars[i]);
                correctLetters += 1;
            } else if (charCount[inputChars[i]] > 1) {
                // Char is somewhere in the word
                indexStates[i] = 2;
                charCount[inputChars[i]] -= 1;
            }
    ...
    function generateRowEmojis(uint8[maxLetters] memory indexStates)
        private
        view
        returns (string memory)
    {
    ...
    function generateMetadata(string[] memory rowEmojis)
        private
        view
        returns (string memory)
    {
        ...
        svgData[
            0
        ] = '<svg xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="xMinYMin meet" viewBox="0 0 350 350"><style>.base { fill: white; font-family: serif; font-size: 18px; } .header{ font-size: 34px; font-family: "Clear Sans", "Helvetica Neue", Arial, sans-serif;  }</style>';
        svgData[1] = header;
