...
contract LostBody is ERC721A, Ownable, Pausable {
    ...
    function findlostbody_allowlist(bytes32[] calldata merkleProof, uint256 _quantity) external callerIsUser payable whenNotPaused {
        ...
        uint256 userallowlistmint = allowlistuserinfo[msg.sender];
        require (userallowlistmint<5,"Exceed Allowlist Sale");

        uint256  _remainFreeQuantity = 0;
        if (userallowlistmint == 0) {
            _remainFreeQuantity=1;
        }
        ...
        uint256 _needPayPrice =  (_quantity - _remainFreeQuantity) * 0.0038 ether;
    ...
    function findlostbody_public(uint256 _quantity) external callerIsUser payable whenNotPaused {
        ...
        uint256 _hasMinted = totalSupply();
        require(_hasMinted + _quantity <= maxSupply, "Exceed supply");
        require (publicsaleuserinfo[msg.sender]<5,"Exceed Allowlist Sale");
        ...
        uint256 _needPayPrice =  (_quantity - _remainFreeQuantity) * 0.0048 ether;
