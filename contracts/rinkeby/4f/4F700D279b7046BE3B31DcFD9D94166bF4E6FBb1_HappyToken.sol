...
contract HappyToken is INounsToken, Ownable, ERC721Enumerable {
  ...
  /**
    ...
    */
  function customMint(uint256 tokenId) public  returns (uint256) {
  ...
  function _generateSVG(uint256 tokenId) internal view returns (bytes memory) {
    bytes memory pack = abi.encodePacked(
      '<svg width="1024" height="1024" viewBox="0 0 1024 1024" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" shape-rendering="crispEdges">\n'
      );
  ...
  function _generateImage(uint256 _tokenId, uint256 _length, string[6] memory _colors) internal view returns(bytes memory) {
    bytes memory pack = abi.encodePacked('<rect width="1024" height="1024" fill="#71BCE1" />\n');
    bytes memory lifepath = _lifePath(_tokenId);
  ...
  function _lifePath(uint256 tokenId) internal view returns (bytes memory) {
    /*
    planned ages point, but it over fllowed.
    uint len = 10;
    uint8[10] memory ages_point; //example 0x64 50 38 38 80 00 00 00 00 00 00:
    //means   0-10yeas 100 point, 10-20 years old 80 points,,, until 90-100 years
    for (uint i=0; i<10; i++) {
      uint256 mask = 0xff;
      ages_point[i] = uint8((tokenId >> (8*i+3)) & mask);
      console.log(ages_point[i]);
    }
    */
...
*/
    pack = abi.encodePacked(
        '<path id="my_path" d="M 100,300 C 240,40 760,40 900,300" fill="none"  /><text font-size="30" font-weight="bold" fill="Yellow"> <textPath xlink:href="#my_path">');
    pack = abi.encodePacked(
        pack,
