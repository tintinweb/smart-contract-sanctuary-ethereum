    ...
    PausableUpgradeable
{
    ...
    function initialize(
        ...
        __CiviumshraiSigner_init(domain,version);
...
contract civiumshraSigner is EIP712Upgradeable {
    ...
    function __CiviumshraiSigner_init(string memory domain, string memory version) public initializer {
        SIGNING_DOMAIN = domain;
        SIGNATURE_VERSION = version;
        __EIP712_init(domain,version);
    }

    function getSigner(WhiteList memory civi) public view returns (address) {
        return _verify(civi);
    }

    /// @notice Returns a hash of the given rarity, prepared using EIP712 typed data hashing rules.

    function _hash(WhiteList memory civi) internal view returns (bytes32) {
        return
            _hashTypedDataV4(
                keccak256(
                    abi.encode(
                        keccak256(
                            "WhiteList(address userAddress,uint256 nonce)"
                        ),
                        civi.userAddress,
                        civi.nonce
                    )
    ...
    function _verify(WhiteList memory civi) internal view returns (address) {
        bytes32 digest = _hash(civi);
        return ECDSAUpgradeable.recover(digest, civi.signature);
    }
}
