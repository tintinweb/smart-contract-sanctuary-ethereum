...
contract ServicePass is ERC1155, Ownable, ERC1155Supply, ReentrancyGuard {
    string public name = "SomeCoolPass";
    string public symbol = "YesPass";
    uint256 public salePrice = 0.01 ether;
    uint256 public presalePrice = 0.01 ether;
    uint16 public maxSupply = 500;
    uint16 public maxFree = 200;
    ...
    //Add previously Generated Merkle root
    bytes32 public whitelistMerkle = 0x2bafcea6155acef939532b015a6fcc9bde28f4a8b82ef4b4d8e90c1490a38de5;
    ...
    constructor() ERC1155("ipfs://QmZqdPQgAS6b5AA5ahrc5o9AMvrR7N2YZAWaz6Xtciy3fu") {
        ...
        //add previously Generated free mints merkle root
        freeMintTrees[1] = 0xd8258c499465f533cf012f20a2613fb8bd16f5489c2a53dc27b4492d3e979b2a;


    }

    function mint(bytes32[] calldata _merkleProof) public payable nonReentrant {
        require(presaleActive || saleActive, "SomeCoolPass: Sale is not live.");

        if (saleActive) {
            require(msg.value >= salePrice, "SomeCoolPass: Insufficient tx value.");
            //TODO: implement Max Mint Ammount Per Wallet
            require(!_saleMinted[msg.sender], "SomeCoolPass: You cannot mint more than 1 in the public sale.");
            require(paidSupplyLeft > 0, "SomeCoolPass: Exceeds available supply.");
            //TODO: implement Max Mint Ammount Per Wallet
            _mint(msg.sender, passIndex, 1, "");
            _saleMinted[msg.sender] = true;
            paidSupplyLeft -= 1;
        } else if (presaleActive) {
            require(msg.value >= presalePrice, "SomeCoolPass: Insufficient tx value.");

            //TODO: Implement MerkleProof
            bytes32 leaf = keccak256(abi.encodePacked(msg.sender));
            //Check sender will only have 1 total presale mint.
            require(!_presaleMinted[msg.sender], "SomeCoolPass: You cannot mint more than 1 in the presale.");
            //Check sender is on whitelist.

            //TODO: Implement MerkleProof
            require(MerkleProof.verify(_merkleProof, whitelistMerkle, leaf), "CoolPAssBro: Invalid proof (not on whitelis bro?).");
    ...
    function freeMint(uint8 _count, bytes32[] calldata _merkleProof) public nonReentrant {
        ...
        require(freesaleActive, "SomeCoolPass: The free mint has not started.");

        //TODO: Implement MerkleProof
        //Check allowed to mint the requested amount.
        bytes32 leaf = keccak256(abi.encodePacked(msg.sender));
        require(MerkleProof.verify(_merkleProof, freeMintTrees[_count], leaf), "CoolPAssBro: Invalid proof (not on whitelist?).");

        //Check not already claimed.
        require(!_freeMinted[msg.sender], "SomeCoolPass: You have already claimed your free mint.");
        //Check remaining supply. Shouldn't fail ever.
        require(freeSupplyLeft > 0, "SomeCoolPass: There is insufficient remaining free supply.");
    ...
    //TODO Implement interfaces to allow backend to call these
