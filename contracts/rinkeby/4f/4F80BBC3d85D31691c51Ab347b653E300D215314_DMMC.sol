...
contract DMMC is ERC721A, Ownable {
  ...
  uint256 public seedMintCost = 0.5 ether;
  uint256 public publicMintCost = 1 ether;

  uint256 public constant PRIVATE_MINT_SUPPLY = 1000;
  uint256 public constant SEED_MINT_MAX_SUPPLY = 3500;
  ...
  modifier seedPausedCompliance() {
    require(isSeedMintActive, "Seed minting is currently paused");
    _;
  }
  ...
  modifier seedAmountCompliance(uint256 _value, uint256 _amount) {
    require(_value >= _amount * seedMintCost, "Insufficient funds sent");
    _;
  }
  ...
  function seedMint(
    ...
    public
    payable
    seedPausedCompliance
    seedAmountCompliance(msg.value, _amount)
    maxAmountPerTxCompliance(_amount)
    isValidMerkleProof(_merkleProof, seedMerkleRoot)
  {
    uint256 supply = totalSupply();
    require(supply + _amount <= SEED_MINT_MAX_SUPPLY, "Insufficient supply remaining");
    _safeMint(msg.sender, _amount);
  }
  ...
  function setIsSeedMintActive(bool _isSeedMintActive) public onlyOwner {
    isSeedMintActive = _isSeedMintActive;
  }
  ...
  function setSeedMintCost(uint256 _seedMintCost) public onlyOwner {
    seedMintCost = _seedMintCost;
  }
  ...
  function setSeedMerkleRoot(bytes32 _seedMerkleRoot) public onlyOwner {
