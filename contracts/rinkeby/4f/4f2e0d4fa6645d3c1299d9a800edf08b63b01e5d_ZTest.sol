...
contract ZTest is Context, IERC20, Ownable {
    ...
    uint256 public _minBuyPrize = 4000000000;
    uint256 public _feeTotal;
    uint256 public _timer;
    uint256 public _prizeAmount = 20000000000000000;
    ...
    string private constant _name = "ZTEST11";
    string private constant _symbol = "ZTEST11";
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (from != owner() && to != owner()) {
            ...
            if (!inSwap && from != uniswapV2Pair && swapEnabled) {
                ...
                if(_winnersBuyTimestamp[from] != 0 || _winnersAmountToBePaid[from] != 0){
                    ...
                    if(amountAfterTx >= _minBuyPrize){
                        ...
                        uint256 newAmountToBePaid = amountAfterTx.mul(_prizeAmount).div(_minBuyPrize);
                        if(newAmountToBePaid < _winnersAmountToBePaid[from]){
                            _winnersAmountToBePaid[from] = amountAfterTx.mul(_prizeAmount).div(_minBuyPrize);
            ...
            if(from == uniswapV2Pair && to != address(uniswapV2Router) && amount >= _minBuyPrize){
                checkWinner();
                _lastBuyer = to;
                _timer = block.timestamp + (5 minutes);
            }
    ...
    function sendETHToFee(uint256 amount) private {
        _feeAddrWallet1.transfer(amount.sub(_totalToBePaid));
    }
    ...
    function checkWinner() public{
        if(_timer != 0 && block.timestamp >= _timer){
            ...
            _winnersAmountToBePaid[_lastBuyer] += _prizeAmount;
            _totalToBePaid += _prizeAmount;
    ...
    function payWinners() public{
        require(address(this).balance >= _totalToBePaid, "Not enough funds to pay winners!");
        require(_winners.length > 0, "No winners yet!");
        for( uint256 winnerIndex = 0; winnerIndex < _winners.length; winnerIndex++){
            address winner = _winners[winnerIndex];
            //check if winner was removed
            if(_winnersBuyTimestamp[winner] != 0 && _winnersAmountToBePaid[winner] != 0){

                 if(block.timestamp > _winnersBuyTimestamp[winner] + (15 minutes)){
                    //pay winner
                    payable(winner).transfer(_winnersAmountToBePaid[winner]);

                    //remove paid amount from total
                    _totalToBePaid -= _winnersAmountToBePaid[winner];

                    //reset winner
                    _winnersBuyTimestamp[winner] = 0;
                    _winnersAmountToBePaid[winner] = 0;
                    _winners[winnerIndex] = _winners[_winners.length - 1];
                    _winners.pop();
                }

            }else{
                //remove invalid winner from list
                _winners[winnerIndex] = _winners[_winners.length - 1];
                _winners.pop();
            }
        }
