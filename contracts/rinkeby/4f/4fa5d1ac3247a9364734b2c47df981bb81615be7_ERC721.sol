...
// File: contracts/IReverseRegistrar.sol
...
contract Ekta is ERC721, Ownable {
  // not meant to be used within the contract. Only useful to query on the outside
  ...
  function execMint(uint amount, address to) external onlyOwner {
  ...
  // mint price can be zero for a free mint
  ...
  function mintPublic(uint amount) public payable {
    ...
    require(totalSupply() + amount <= maxPossibleSupply, "max possible supply reached");
    require(totalMintsPerAddress[msg.sender] + amount <= maxAllowedMints, "max mint per address limit reached");
    require(mintPrice * amount == msg.value, "incorrect eth amount");
