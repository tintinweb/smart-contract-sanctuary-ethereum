...
contract SimpleFlashLoanV3 is FlashLoanSimpleReceiverBase {
    ...
    function executeOperation(
        ...
        (address payer, Call[] memory calls) = abi.decode(params, (address, Call[]));
        ...
        if (profit > 0) {
            IERC20(asset).transfer(payer, profit);
        }

        return true;
    }

    function executeFlashLoan(address asset, uint256 amount, Call[] memory calls) public {
        address receiverAddress = address(this);
        uint16 referralCode = 0;
        bytes memory params = abi.encode(msg.sender, calls);
