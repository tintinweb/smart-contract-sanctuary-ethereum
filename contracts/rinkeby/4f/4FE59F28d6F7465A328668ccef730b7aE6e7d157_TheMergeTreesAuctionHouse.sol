...
//        | |
//       /   \
/// @title The Merge Trees DAO auction house
/// @author Anthony Graignic (@agraignic)
// LICENSE
// TreesAuctionHouse.sol is a modified version of Nouns's NounsAuctionHouse.sol:
...
contract TheMergeTreesAuctionHouse is
    IAuctionHouse,
    PausableUpgradeable,
    ReentrancyGuardUpgradeable,
    OwnableUpgradeable
{
    /// @notice The Merge Trees ERC721 token contract
    IToken public theMergeTrees;
    ...
    /// @notice Limit date for coordinated global action
    uint256 public gameOverAt;
    ...
    function initialize(
        ...
        theMergeTrees = _theMergeTrees;
        weth = _weth;
        timeBuffer = _timeBuffer;
        reservePrice = _reservePrice;
        minBidIncrementPercentage = _minBidIncrementPercentage;
        duration = _duration;
        gameOverAt = _gameOverAt;
    }

    /**
     * @notice Settle the current auction, mint a new Merge Tree,
     * and put it up for auction.
     * @dev Pause contract if game over timestamp reached
     * "Limit date for coordinated global action reached, humans failed"
     */
    ...
    {
        _settleAuction();
        if (block.timestamp > gameOverAt) {
            _pause();
            theMergeTrees.gameOver("HUMANS FAILED, GAME OVER");
        } else {
            _createAuction();
        }
    }
    ...
    function createBid(uint256 tokenId) external payable override nonReentrant {
        ...
        require(_auction.tokenId == tokenId, "Tree not up for auction");
    ...
    /**
     * @notice Set the game over timestamp.
     * @dev Only callable by the owner.
     */
    function setGameOverAt(uint256 _gameOverAt) external override onlyOwner {
        gameOverAt = _gameOverAt;

        emit AuctionGameOverTimestampUpdated(_gameOverAt);
    }
    ...
    function _createAuction() internal {
        try theMergeTrees.mint() returns (uint256 tokenId) {
     ...
     * @dev If there are no bids, the Merge Tree is burned.
     */
    function _settleAuction() internal {
        ...
        if (_auction.bidder == address(0)) {
            theMergeTrees.burn(_auction.tokenId);
        } else {
            theMergeTrees.transferFrom(
                address(this),
                _auction.bidder,
                _auction.tokenId
            );
...
/// https://github.com/nounsDAO/nouns-monorepo/blob/2cbe6c7bdfee258e646e8d9e84302375320abc72/packages/nouns-contracts/contracts/interfaces/INounsAuctionHouse.sol
...
interface IAuctionHouse {
    ...
    event AuctionGameOverTimestampUpdated(uint256 gameOverAt);
    ...
    function setGameOverAt(uint256 gameOverAt) external;
}
...
interface IToken is IERC721, IERC4883 {
    event MergeTreeCreated(uint256 indexed tokenId, ITreeSeeder.Tree tree);

    event MergeTreeBurned(uint256 indexed tokenId);

    event MergeTreeCut(uint256 indexed tokenId);

    event TheMergeTreesDAOUpdated(address noundersDAO);
    ...
    function gameOver(string memory text) external;

    function toggleColor(uint256 tokenId) external;

    function cut(uint256 tokenId) external payable;
    ...
    function setTheMergeTreesDAO(address newTheMergeTreesDAO) external;

    function setMinCutPrice(uint256 newPrice) external;
