...
contract PITPrizes is ERC1155Extended, AllowList, TwoStepOwnable {
    using Counters for Counters.Counter;

    Counters.Counter private _tokenIdCounter; // Token ID counter
    address private constant _openseaProxyAddress =
    ...
    constructor(
        ...
        ERC1155Extended(
            ...
            uri_,
            maxSupply_,
            _openseaProxyAddress
        )
        AllowList(maxRedemption_, merkleRoot_)
    {
    ...
    function mintAllowList(bytes32[] calldata proof_)
        external
        payable
        virtual
        nonReentrant
        whenNotPaused
        onlyAllowListed(proof_)
    {
    ...
    function _mintAndIncrement() internal {
        _ensureSupplyAvailableForIdAndIncrement(_tokenIdCounter.current(), 1);
...
interface IAllowList {
    ...
    function isAllowListed(bytes32[] calldata proof_, address address_)
        external
        view
        returns (bool);
...
contract AllowList is Ownable {
    uint256 public immutable maxRedemptions;
    bytes32 public merkleRoot;
    mapping(address => uint256) private _addressRedemptions;

    error NotAllowListed();
    error MaxAllowListRedemptions();

    modifier onlyAllowListed(bytes32[] calldata proof_) {
        if (!isAllowListed(proof_, msg.sender)) {
            revert NotAllowListed();
        }
        _;
    }

    constructor(uint256 maxRedemptions_, bytes32 merkleRoot_) {
        maxRedemptions = maxRedemptions_;
        merkleRoot = merkleRoot_;
    }

    function _ensureAllowListRedemptionsAvailableAndIncrement(uint256 quantity_)
        internal
    {
        if ((_addressRedemptions[msg.sender] + quantity_) > maxRedemptions) {
            revert MaxAllowListRedemptions();
        }
        unchecked {
            _addressRedemptions[msg.sender] += quantity_;
        }
    }
    ...
    {
        return
            MerkleProof.verify(proof_, merkleRoot, keccak256(abi.encodePacked(address_)));
    }
    ...
    ReentrancyGuard
{
    ...
    uint256 public immutable maxSupplyPerTokenId;
    ...
    constructor(
        ...
        name = name_;
        symbol = symbol_;
        maxSupplyPerTokenId = maxSupply_;
    }
    ...
    function _ensureSupplyAvailableForIdAndIncrement(
        ...
        if ((numMinted[tokenId_] + amount_) > maxSupplyPerTokenId) {
            revert MaxSupplyForID();
        }
        unchecked {
            numMinted[tokenId_] += amount_;
        }
    }
