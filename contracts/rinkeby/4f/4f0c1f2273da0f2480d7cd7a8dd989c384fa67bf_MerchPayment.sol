...
contract MerchPayment is Owned {
    ...
    constructor(address _POWToken, address _PUNKSToken, address _dest) Owned(msg.sender) {
    ...
    function addItem(
        ...
        sizesOf[id] = sizeQuantities.length;

        for(uint256 i = 0; i < sizeQuantities.length; i++) {
    ...
    function purchase(uint256[] memory sizes, bool isPOW) public {
        ...
        if(isPOW) {
            POWToken.transferFrom(msg.sender, dest, totalCost);
        } else {
            PUNKSToken.transferFrom(msg.sender, dest, totalCost);
        }
