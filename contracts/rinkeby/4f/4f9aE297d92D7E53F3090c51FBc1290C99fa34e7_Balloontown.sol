...
contract Balloontown is ERC721A, Ownable, PaymentSplitter {
    ...
    function addFiatMinter(address[] memory _addresses) public onlyOwner {
        for (uint16 i = 0; i < _addresses.length; i++) {
            fiatAllowlist[_addresses[i]] = true;
        }
    }

    function removeFiatMinter(address[] memory _addresses) public onlyOwner {   
        for (uint16 i = 0; i < _addresses.length; i++) {
            delete fiatAllowlist[_addresses[i]];
        }
    }
    ...
    function updateSaleState(
        ...
        require(_salePhase == 0 || _salePhase == 1 || _salePhase == 2 || _salePhase == 3, "Invalid phase.");
