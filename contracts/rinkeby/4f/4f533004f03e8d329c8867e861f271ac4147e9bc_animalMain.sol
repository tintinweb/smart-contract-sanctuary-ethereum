...
contract animalMain is  ERC721,Ownable {
  ...
  using SafeMath for uint256;
    ...
    uint public constant mintFee = 1 finney;
  ...
  function createAnimal() public payable{
      ...
      dna = getRandom();
      star = getRandom(5);
      roletype = getRandom(4);
      //動手玩創意，可以限制每次建立動物需要花費多少ETH
      require(mintFee <= msg.value);

      uint id = animals.push(animal(dna, uint8(star), uint8(roletype))) - 1;
      animalToOwner[id] = msg.sender;
      ownerAnimalCount[msg.sender]++;

      emit Create(id, dna, uint8(star), uint16(roletype));
  }

  function getRandom() private view returns(bytes32) {
      return keccak256(abi.encodePacked(block.timestamp, block.difficulty, now));
  }

  function getRandom(uint maxValue) private view returns(uint) {
      return uint(getRandom()) % (maxValue + 1);
  }
