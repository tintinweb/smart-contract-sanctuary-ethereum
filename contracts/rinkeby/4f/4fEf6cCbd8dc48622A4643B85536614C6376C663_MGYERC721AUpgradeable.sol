...
contract MGYERC721AUpgradeable is Initializable,OwnableUpgradeable,ERC721AUpgradeable, ReentrancyGuardUpgradeable, MerkleProofUpgradeable {
  ...
  address payable private _withdrawWallet;//withdraw wallet
  ...
  bool private _isRevealed;//reveal enable.
  //mint record
  mapping(address => uint256) private _wlMinted;//wl.mint num by wallet.
  mapping(address => uint256) private _psMinted;//PublicSale.mint num by wallet.

  //Upgradeable need this line.
  ...
  function initialize (
    ...
    //transferOwnership(msg.sender);//Not execute for multisig.
    //initialize
    wlMintPrice = 0.001 ether;
    psMintPrice = 0.003 ether;
    maxMintsPerWL = 2;
    maxMintsPerPS = 5;
    maxSupply = 10;
    isWlEnabled = false;
  ...
  function setWithdrawWallet(address _owner) external onlyOwner {
  ...
  function setPsPrice(uint256 newPrice) external onlyOwner {
  ...
  //retuen _wlMinted
  function wlMinted(address _address) public view returns (uint256){
    return _wlMinted[_address];
  }
  //retuen _psMinted
  function psMinted(address _address) public view returns (uint256){
    return _psMinted[_address];
  }

  //set wl's max mint num.only owner.
  function setWlMaxMints(uint256 _max) external onlyOwner {
    maxMintsPerWL = _max;
  }
  //set PublicSale's max mint num.only owner.
  function setPsMaxMints(uint256 _max) external onlyOwner {
    maxMintsPerPS = _max;
  }
  ...
  //retuen BaseURI.private.
  function _currentBaseURI() private view returns (string memory){
  ...
  function tokenURI(uint256 _tokenId) public view virtual override returns (string memory) {
    ...
    return string(abi.encodePacked(_currentBaseURI(), StringsUpgradeable.toString(_tokenId), ".json"));
  }
  ...
  function tryOwnerOf(uint256 tokenId) external view  returns (address) {
...
abstract contract MerkleProofUpgradeable is Initializable {
