...
contract WakumbaBless is WakumbaOwnedUpgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function recover(
        ...
        bytes32 messageHash = keccak256(abi.encodePacked(addr, qty, limit, nonce));
    ...
    function airdropFaith(
        ...
        require(isValidSignature(to, qty, maxLimit, nonce, signature), 'Invalid signature');
        __claimedFaithQty[_msgSender()] += qty;
        __faithContract.mint(
            to,
            qty * 1e18 /* Contract Digits */
        );
