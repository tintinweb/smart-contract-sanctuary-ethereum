...
contract SimpleNftGasless is  ERC721, Ownable, ERC2771Context{
  ...
  event Registered(address indexed who, string nickname);
  event NFTCounter(address buyer, uint tokens);

  mapping(address => string) public nicknames;
  mapping(string => address) public senders;
  ...
  constructor(MinimalForwarder forwarder) ERC721("Metagency by KRDS", "meta") ERC2771Context(address(forwarder)){
    ...
    setUriPrefix('ipfs://QmacNfSECfhKJfApcmmYmDfv4TXgd9UP3scG3q6u3GuvtX/');
  ...
  function register(string memory nickname, uint256 _mintAmount) external {
    
    address sender = _msgSender(); // Changed from msg.sender
    uint256 ownerTokenCount = balanceOf(sender);
    emit NFTCounter(sender, ownerTokenCount);
    require(ownerTokenCount < maxMintAmountPerTx, "Only 1 NFT allowed per user.");
    senders[nickname] = sender;
    nicknames[sender] = nickname;
    mint(_mintAmount);
    emit Registered(sender, nickname);
  }
