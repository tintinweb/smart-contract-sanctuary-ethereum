...
contract KingsStake is Ownable, ReentrancyGuard {
    ...
    //address rewardTokensAddress = 0xd9145CCE52D386f254917e481eB44e9943F39138;
    ...
    /*
    function isStakedAlready(uint256 _stakePoolId) internal view returns(bool, uint256) {
        uint256[] memory stakeIds = getUserStakeIds();
        uint256 stakePoolId;

        for(uint256 i = 0; i < stakeIds.length; i++) {
            stakePoolId = getStakePoolIdByStakeId(stakeIds[i]);
            if(_stakePoolId == stakePoolId){
                return (true,stakeIds[i]);
            }
        }
        return (false,0);
    }
    */
    ...
    /*
    function getUserStakeIndexFromUserStakeLists(uint256 _stakePoolId) public view returns (uint256){
        userStake[] memory userAllStakesArray = userStakeLists[msg.sender];
        for(uint256 i = 0; i < userAllStakesArray.length; i++){
            if(userAllStakesArray[i].stakePoolId == _stakePoolId) {
                return i;
            }
        }
        return 0;
    ...
    */
    ...
    function stake(uint256 _stakePoolId, uint256 _amount) unpaused external returns (bool) {
        ...
        //(bool isStaked, uint256 stakeId) = isStakedAlready(_stakePoolId);
    ...
    function claimRewards(uint256 _stakeId) nonReentrant unpaused public returns (bool){
        ...
        require(((block.timestamp - userStakeDetails.lastClaimedTime)/3600) > stakePoolDetails.claimFrequency,"You already claimed rewards today");
