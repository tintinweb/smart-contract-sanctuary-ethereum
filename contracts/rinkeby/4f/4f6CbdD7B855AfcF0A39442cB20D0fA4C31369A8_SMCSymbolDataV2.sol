...
import "./ISMCSymbolV2.sol";

contract SMCSymbolDataV2 is ISMCSymbolDataV2 {
    mapping(uint256 => string[]) public CodesOfArts;

    function CodesOfArt(uint256 respect)
        external
        view
        override
        returns (string[] memory)
    {
        return CodesOfArts[respect];
    }

    string[] public respects = [
        "",
        "MakotoKobayashi",
        "MichinoriChiba",
        "HiroyaIijima",
    ...
    function Respects() external view override returns (string[] memory) {
        return respects;
    }

    uint256[] public respectColorCodes = [
        0x0,
        0xfb4d0f,
        0x00ff00,
        0x8cfafa,
    ...
    function RespectColorCodes()
        external
        view
        override
        returns (uint256[] memory)
    {
        return respectColorCodes;
    }
...
interface ISMCSymbolDescriptorV2 {
    function tokenURI(ISMCManagerV2 manager, uint256 tokenId)
        external
        view
        returns (string memory);
}

interface ISMCSymbolDataV2 {
    function CodesOfArt(uint256 respect)
        external
        view
        returns (string[] memory);

    function Respects() external view returns (string[] memory);

    function RespectColorCodes() external view returns (uint256[] memory);
...
interface ISMCManagerV2 is IERC721 {
    function getRespect(uint256 tokenId) external view returns (string memory);

    function getRespectColorCode(uint256 tokenId)
        external
        view
        returns (uint256);
