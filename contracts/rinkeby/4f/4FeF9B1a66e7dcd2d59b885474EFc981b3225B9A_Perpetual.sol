...
import "./dac.sol";
import "./OracleFeeder.sol";

/// @title main contract
/// @author Stars Labs
contract Perpetual is Users, Deposits, Withdrawals, UpdateState, Dac, OracleFeeder, ReentrancyGuard {
    ...
    function initialize(
        ...
        synthetic_assets_info[] calldata synthetic_assets
	) external {
        console.log(" +++ perpetual initlized +++");
        ...
        ) = abi.decode(parameters, (IERC20, uint8, address, address, address));
    ...
    function initGlobalConfig(
        ...
        (
            uint16 _n_synthetic_assets_info,
            ...
            // synthetic_assets_info[] memory synthetic_assets
        //) = abi.decode(globalConfigData, (uint16, uint32, uint32, uint64, synthetic_assets_info[]));

        n_synthetic_assets_info = _n_synthetic_assets_info;
        ...
        bytes memory globalConfig = abi.encodePacked(
    ...
    function encodeOracleSigners (
        ...
        for (uint32 i=0; i< signers.length; ++i) {
            config = abi.encodePacked(config, signers[i]);
        }
    }
    ...
    function encodeSyntheticAssets (
        ...
        for (uint32 i=0; i< synthetic_assets.length; ++i) {
            ...
            config =
                abi.encodePacked(config, 
                    abi.encodePacked(
                        synthetic_assets[i].resolution,
                        synthetic_assets[i].risk_factor,
                        synthetic_assets[i].asset_name,
                        //encodeOracleSigners(synthetic_assets[i].oracle_price_signers_pubkey_hash)
        ...
        // pad
        bytes memory padZero = new bytes((MAX_ASSETS_COUNT - synthetic_assets.length) * (24 + MAX_NUMBER_ORACLES * 20));
        config = abi.encodePacked(config, padZero);
    }
    ...
    function addSyntheticAssets (
        ...
        globalConfigHash == sha256(abi.encodePacked(
            oldGlobalConfig,
            encodeSyntheticAssets(synthetic_assets)
        ));
    }
    ...
    function recoverWrongToken(address token, uint256 amount) external onlyGovernor {
    ...
    function escape(
        ...
        require(stateFrozen, "escape invalid in non-frozen state");

        // TODO : compute withdraw amount
...
pragma solidity >= 0.8.0;
pragma abicoder v2; // Not Expiremental Since 0.7.4
...
uint16 constant MAX_ASSETS_COUNT = 15;
uint16 constant MAX_NUMBER_ORACLES = 6;

struct synthetic_assets_info {
    uint64 resolution;
...
/// @author Stars Labs
contract Storage is Governance {
    /* block chain root hash */
    bytes32 public accountRoot;
    bytes32 public orderStateHash;

    /// @notice Total blocks verified signature in validum-mode.
    ...
    // mapping(uint8 => synthetic_assets_info) public syntheticAssets;
    bytes32 public globalConfigHash;  // hash of (synthetic_assets_info, globalConfig)
    ...
    // map l2 key => quantized amount.
    IERC20 public collateralToken;       // TODO : immutable
    ...
    bool stateFrozen;
    Verifier public escapeVerifier;
    mapping(uint256 => bool) escapesUsed;   // l2key
...
abstract contract Users is Storage {
    ...
    function registerUser(
        ...
        bytes32 concatKeyHash = keccak256("");
        for (uint256 i = 0; i < l2Keys.length; ++i) {
            concatKeyHash = keccak256(abi.encodePacked(concatKeyHash, l2Keys[i]));
        }

        //bytes memory orig_msg = abi.encode(
        bytes memory orig_msg = abi.encodePacked(
        ...
        // bytes32 concatKeysHash;
        address signer = Utils.recoverAddressFromEthSignature(signature, msgHash);
...
abstract contract Deposits is Storage {
    ...
    function depositERC20(
        ...
        require(pendingDeposits[l2Key] >= amount, "DEPOSIT_OVERFLOW");  // TODO : remove as 0.8 overflow evm assert works

        // Disable the cancellationRequest timeout when users deposit into their own account.
        if (cancellationRequests[l2Key] != 0 //TODO: && isMsgSenderKeyOwner(l2Key)
        ) {
        ...
        require(Utils.transferFromERC20(token, msg.sender, address(this),
                SafeCast.toUint128(amount)), "depoist fail");
    ...
    function depositReclaim(
        ...
        require(ethKeys[l2Key] != address(0), "depositReclaim : Invalid l2Key");
        ...
        assert(freeTime >= DEPOSIT_CANCEL_TIMELOCK);
        ...
        require(Utils.sendERC20(collateralToken, ethKeys[l2Key], amount), "depositReclaim fail"); 
...
abstract contract Withdrawals is Storage {
    ...
    function withdrawERC20(IERC20 token, uint256 ownerKey, address payable recipient) internal {
        ...
        require(Utils.sendERC20(token, recipient, amount), "withdraw fail"); 
    ...
    function withdraw(uint256 ownerKey) external {
    ...
    function withdrawTo(
        ...
        external onlyKeyOwner(ownerKey)
    {
        withdrawERC20(collateralToken, ownerKey, recipient);
    }
...
abstract contract UpdateState is ForcedWithdrawals, ConditionalTransfer, Config {
    ...
    uint256 constant MIN_DAC_SIGNATURE = 1;

    event BlockUpdate(uint32 firstBlock, uint32 lastBlock);
    event DEBUG_PROOF(bytes32 newOrderStateHash, uint256 proofCommitment, uint256 localCommitment);
    event DEBUG_COMMITMENT(
        bytes32 h1,
        bytes32 h2,
        bytes32 h3,
        bytes32 h4,
    ...
    function createBlockCommitment(
        ...
        // TODO : remove abi.encodePacked
        bytes32[7] memory h;
        h[0] = sha256(abi.encodePacked(uint256(newBlock.blockNumber), oldAccountRoot));
        h[1] = sha256(abi.encodePacked(h[0], newBlock.accountRoot));
        h[2] = sha256(abi.encodePacked(h[1], oldOrderStateHash));
        h[3] = sha256(abi.encodePacked(h[2], newOrderStateHash));
        h[4] = sha256(abi.encodePacked(h[3], globalConfigHash));
        h[5] = sha256(abi.encodePacked(h[4], newBlock.validiumAccountRoot));
        bytes32 rollup_col_commitment;
        uint256 padLen = getPadLen(
            newBlock.collateralBalancePubData.length,
            newBlock.blockChunkSize * Operations.ACCOUNT_COLLATERAL_BALANCE_PUBDATA_BYTES
        );
        if (padLen != 0) {
            bytes memory padZero = new bytes(padLen);
            rollup_col_commitment = sha256(abi.encodePacked(newBlock.collateralBalancePubData, padZero));
        } else {
            rollup_col_commitment = sha256(newBlock.collateralBalancePubData);
        }

        bytes32 rollup_assets_commitment;
        padLen = getPadLen(
            newBlock.positonPubData.length,
            newBlock.blockChunkSize * Operations.ACCOUNT_POSITION_PUBDATA_BYTES
        );
        if (padLen != 0) {
            bytes memory padZero = new bytes(padLen);
            rollup_assets_commitment = sha256(abi.encodePacked(newBlock.positonPubData, padZero));
        } else {
            rollup_assets_commitment = sha256(newBlock.positonPubData);
        }

        bytes32 rollup_data_commitment = sha256(abi.encodePacked(rollup_col_commitment, rollup_assets_commitment));
        bytes32 account_data_commitment = sha256(abi.encodePacked(rollup_data_commitment, newBlock.all_data_commitment));
        bytes32 h6_2 = sha256(abi.encodePacked(h[5], account_data_commitment));

        bytes32 onchain_commitment;
        padLen = getPadLen(
            newBlock.onchainPubData.length,
            newBlock.blockChunkSize * Operations.DEPOSIT_WITHDRAW_PUBDATA_BYTES
        );
        if (padLen != 0) {
            bytes memory padZero = new bytes(padLen);
            onchain_commitment = sha256(abi.encodePacked(newBlock.onchainPubData, padZero));
        } else {
            onchain_commitment = sha256(newBlock.onchainPubData);
        }
        commitment = sha256(abi.encodePacked(h6_2, onchain_commitment));
        emit DEBUG_COMMITMENT(h[0], h[1], h[2], h[3], h[4], h[5], commitment);
    ...
    function postProcess(
        ...
        while (offset < pubData.length) {
            ...
            if (op.amount > (1 << 63)) {
                uint256 innerAmount = (uint256(op.amount) - uint256(1 << 63)) * 1000000;
                pendingDeposits[op.l2Key] -= innerAmount * (10 ** (collateralToken.decimals() - innerDecimal));
            } else {
                uint256 innerAmount = (uint256(1 << 63) - uint256(op.amount)) * 1000000;
                pendingWithdrawals[op.l2Key] += innerAmount * (10 ** (collateralToken.decimals() - innerDecimal));
            }
    ...
    event DEBUG_ORDER_STATE_HASH(
        bytes32 oraclePriceHash,
        bytes32 globalFundingIndexHash,
        bytes32 global_state_hash,
    ...
    function getOrderStateHash(
    ...
    ) internal returns (bytes32 newOrderStateHash) {
        ...
        if (oracle_price.length != 0) {
            if (oracle_price.length != MAX_ASSETS_COUNT) {
                uint64[] memory padZero = new uint64[](MAX_ASSETS_COUNT - oracle_price.length );
                oraclePriceHash = sha256(abi.encodePacked(oracle_price, padZero));
            } else {
                oraclePriceHash = sha256(abi.encodePacked(oracle_price));
            }
        }

        bytes32 globalFundingIndexHash = b.globalFundingIndexHash;
        if (b.globalFundingIndex.length != 0) {
            if (b.globalFundingIndex.length != MAX_ASSETS_COUNT) {
                uint64[] memory padZero = new uint64[](MAX_ASSETS_COUNT - b.globalFundingIndex.length );
                globalFundingIndexHash = sha256(abi.encodePacked(b.globalFundingIndex, padZero));
            } else {
                globalFundingIndexHash = sha256(abi.encodePacked(b.globalFundingIndex));
            }
        }

        bytes32 global_state_hash = sha256(abi.encodePacked(b.timestamp, globalFundingIndexHash, oraclePriceHash));
        newOrderStateHash = sha256(abi.encodePacked(b.orderRoot, global_state_hash));

        emit DEBUG_ORDER_STATE_HASH(
            oraclePriceHash,
    ...
    function verifyProofCommitment(
        ...
        for (uint256 i = 0; i < _newBlocksData.length; ++i) {
            ...
            // require(_proof.globalConfigHash == globalConfigHash, "proof globalConfigHash invalid");
            uint64[] memory oraclePrice;
            if (i == _newBlocksData.length - 1) {
                oraclePrice = lastestOraclePrice;
            }
            bytes32 newOrderStateHash = getOrderStateHash(_newBlocksData[i], oraclePrice);
            bytes32 commitment = createBlockCommitment(curAccountRoot, curOrderStateHash, newOrderStateHash, _newBlocksData[i]);
            emit DEBUG_PROOF(newOrderStateHash, _proof.commitments[i] & INPUT_MASK, uint256(commitment) & INPUT_MASK);
            require(_proof.commitments[i] & INPUT_MASK == uint256(commitment) & INPUT_MASK, "proof commitment invalid");

            curAccountRoot = _newBlocksData[i].accountRoot;
            curOrderStateHash = newOrderStateHash;
        }
    }

    event DEBUG_DAC_SIGNATURE(
        bytes32 concatValdiumHash,
        bytes32 msgHash,
        bytes[] validium_signature
    );
    ...
    function verifyValidiumSignature(
        ...
        require (validium_signature.length >= MIN_DAC_SIGNATURE);

        bytes32 concatValdiumHash = keccak256("");
        for (uint256 i = 0; i < _newBlocksData.length; ++i) {
            concatValdiumHash = keccak256(abi.encodePacked(concatValdiumHash, _newBlocksData[i].all_data_commitment));
        }
        ...
        for (uint256 i = 0; i < validium_signature.length; ++i) {
            ...
            address signer = Utils.recoverAddressFromEthSignature(validium_signature[i], msgHash);
            require(dacs[signer] != false, "ValidiumHash Sinature Invalid");

            sigHashs[sig_dac_num++] = sigHash;
            if (sig_dac_num == MIN_SIGNATURE_MEMBER) { // ignore additional signature.
                break;
            }
        }
    ...
    function updateBlocks(
        ...
        bytes[] calldata validium_signature,
        ProofInput calldata _proof,
        uint64[] calldata lastestOraclePrice  // concat(BTC-USD,ETH-USD,...)
        ) external onlyValidator {
        //) external nonReentrant {

    //     requireActive();

        require (_newBlocksData.length >= 1);
        verifyValidiumSignature(_newBlocksData, validium_signature);
        bytes32 newOrderStateHash = verifyProofCommitment(_newBlocksData, _proof, lastestOraclePrice);

    // TODO: verify lastestOraclePrice signature
        ...
        for (uint256 i = 0; i < _newBlocksData.length; ++i) {
            postProcess(_newBlocksData[i].onchainPubData);
        }

        // update block status
        accountRoot = _newBlocksData[_newBlocksData.length - 1].accountRoot;
        orderStateHash = newOrderStateHash;
        emit BlockUpdate(_newBlocksData[0].blockNumber,
                         _newBlocksData[_newBlocksData.length - 1].blockNumber);
    }
...
abstract contract Dac is Storage {
    ...
    function regDac(address member) external onlyGovernor {
        ...
        require(!DacRegisterActive, "oracleFeeder in register");
        require(!dacs[member], "oracleFeeder already exist");
...
/// @author Stars Labs
abstract contract OracleFeeder is Storage {

    function regsiterOracleFeeder(address feeder) public {
        // requireGovernor
        // TimeLock : User not trust the feeder, will be able to withdraw
        require(!OracleFeederRegisterActive, "oracleFeeder in register");
        require(!oracleFeeders[feeder], "oracleFeeder already exist");
        OracleFeederRegisterActive = true;
        OracleFeederRegisterTime = block.timestamp;
        pendingOracleFeeder = feeder;
    }

    function cancelOracleFeederRegister() public {
        // requireGovernor
        OracleFeederRegisterActive = false;
    }

    function updateOracleFeeder() public {
        // requireGovernor
        require(OracleFeederRegisterActive, "oracleFeeder not register");
        require(block.timestamp > OracleFeederRegisterTime + TIMELOCK_ORACLE_FEEDER_REG, "oracle feeder still in timelock");
        OracleFeederRegisterActive = false;

        oracleFeeders[pendingOracleFeeder] = true;

        // [TODO] update globalConfig Hash, used as future proof's input
    }
}
...
contract KeysWithPlonkVerifier is VerifierWithDeserialize {
    ...
    uint256 constant VK_TREE_ROOT = 0x04208511510747d468ba60aab791075099b69e8ea9c587ad319577320ac95444;
...
abstract contract ConditionalTransfer is Storage {
    ...
    function conditionalTransferRegProof(
		...
		require(!proofRegister[cond], "proof already exit");
    ...
    function condTransfer(
        ...
        require(Utils.sendERC20(collateralToken, reciever, amount), "conditon transfer fail"); 
