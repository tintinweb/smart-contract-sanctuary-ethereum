...
contract PledgeMint is Ownable, ReentrancyGuard {
	...
	ERC20 public USDC = ERC20(0xD92E713d051C37EbB2561803a3b5FBAbc4962431); // USDC
	...
	function updateMerkleRoot(uint16 phaseId, bytes32 newMerkleRoot) external onlyAdminOrOwner(phaseId) {
	   phases[phaseId].merkleRoot = newMerkleRoot;
	}
	
	function saleStatus(uint16 phaseId, bool status) external onlyAdminOrOwner(phaseId) {
	   phases[phaseId].saleEnable = status;
	}
	
	function updatePrice(uint16 phaseId, uint256 mintPrice, uint256 mintPriceWhiteList) external onlyAdminOrOwner(phaseId) {
	   phases[phaseId].mintPrice = mintPrice;
	   phases[phaseId].mintPriceWhiteList = mintPriceWhiteList;
	}
	
	function updateMintLimit(uint16 phaseId, uint8 maxPerWallet) external onlyAdminOrOwner(phaseId) {
