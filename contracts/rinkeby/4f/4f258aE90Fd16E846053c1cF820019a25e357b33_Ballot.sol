...
/// @dev Interface for ERC20Votes in order to use only the functions needed and save on deployment costs
interface IERC20Votes {
  function getPastVotes(address, uint256) external view returns (uint256);
}

/// @dev Custom errors that can be thrown
error Ballot__Ipfs_CIDs_CollectionSize_Mismatch(); /// @dev If collections size array length != ipfs CIDs array length
error OnlyKeeperRegistry(); /// @dev If msg.sender != keeper's registry address

/// @title Ballot with automatic NFT minting for the winning collection after voting period has elapsed
/// @author The Buidlers
/// @notice Vote on different NFT collections and get NFT airdrops if the winning collection was the one you voted for
/// @dev This contract uses Chainlink Keepers under the hood to trigger automatic minting logic
contract Ballot is KeeperCompatibleInterface, NFTContract {
  using Strings for uint256; ///@dev Used for toString() function

  /// @dev Representation of a voter
  struct Voter {
    bool voted; /// @dev Indicates whether that person already voted
    uint256 vote; /// @dev Index of the voted proposal
  }

  /// @dev Representation of a proposal
  struct Proposal {
    uint256 index; /// @dev Index for the NFT collection
    uint256 voteCount; /// @dev Number of accumulated votes
    uint256 collectionSize; /// @dev Number of items in collection starting from 0
    bool active; /// @dev Indicates if the proposal can still be voted on
    string ipfsFolderCID; /// @dev The ipfs CID folder for the NFT collection
  }

  uint256 private totalVotes = 0; /// @dev Total votes count
  mapping(address => Voter) public voters; /// @dev Maps an address to a Voter struct
  mapping(uint256 => address[]) public proposalVoters; /// @dev Mapping to link the index of a proposal to voters addresses

  Proposal[] public proposals; /// @dev Array of Proposal structs
  string[] public collectionTokenUris; /// @dev Contains the different tokens URIs for the winning NFT collection
  uint256 private quorum; /// @dev Total votes count should be greater or equal to this quorum

  uint256 public immutable interval; /// @dev Will be used to check if blocks timestamps difference is greater than the interval
  uint256 public immutable ethereumBase = 10**18; /// @dev Wei unit
  uint256 public lastTimeStamp; /// @dev Serves as a reference to compare a current block timestamp with
  GovernanceToken public governanceTokenContract; /// @dev GovernanceToken contract
  IERC20Votes public voteToken; /// @dev Contract implementing getPastVotes function
  uint256 public referenceBlock; /// @dev Block number at initialization

  /// @dev Event emitted when upkeep has been done by the keeper
  ...
  address private keeperRegistryAddress; /// @dev The keeper registry's address that will perform the upkeep

  /// @dev Modifier to prevent functions to be called if the sender is not the keeper registry's address
   ...
   *  @param _voteToken Address of the GovernanceToken deployed contract
   *  @param _ipfsFolderCIDs Array of IPFS Cids containing NFT collections data
   *  @param _collectionsSize Number of items in each NFT collection
   *  @param _quorum Minimum votes needed before voting can be finished
   */
  constructor(
    address _voteToken,
    ...
    interval = 1 minutes; /// @dev This short interval is for testing purposes only
  ...
  /// @notice Gives your vote to proposal `proposals[proposal].index`.
  ...
  function vote(uint256 proposal, uint256 amount) external {
    ...
    governanceTokenContract.burn(msg.sender, votingPowerUsed); /// @dev Burn the governance tokens once the participant has voted
  ...
  function getWinningProposal() public view returns (uint256 winningProposal_) {
    ...
    Proposal[] memory localProposals = proposals; /// @dev Gas optimization
  ...
  /// @dev Gets the index of the winning proposal to match with ipfsFolderCIDs indexes
  /// @return winnerIndex_ The index of the winning proposal
  ...
  /// @return votingPower_ The voting power for the address `msg.sender`
  function votingPower() public view returns (uint256 votingPower_) {
    votingPower_ = governanceTokenContract.balanceOf(msg.sender);
  }

  /// @notice Indicates if keepers should trigger custom logic or not
  /// @dev This method is called by the keepers to check if performUpkeep should be called
  /// @return upkeepNeeded Whether performUpkeep function should be called or not
  ...
  /// @notice Get the voters addresses for the proposal with index `index`
  /// @return The addresses of the voters for the given proposal
  function getVotersForProposal(uint256 index) public view returns (address[] memory) {
    return proposalVoters[index];
  }

  /// @dev Should be called by the keepers only. It will mint the NFT collection to those who voted for the winning proposal
  ...
  /// @dev Sets the keeper registry's address. Should only be called by admin
  ...
  /// @notice Get the total number of votes since the Ballot was created
  /// @return The total votes count
  function getTotalVotes() public view returns (uint256) {
    return totalVotes;
  }

  /// @notice Get the quorum used for the Ballot
  /// @return The quorum used for the Ballot
  function getQuorum() public view returns (uint256) {
    return quorum;
  }

  /// @notice Get the proposals
  /// @return The proposals to vote on
  function getAllProposals() public view returns (Proposal[] memory) {
    return proposals;
  }

  /// @notice Get the NFT collection's metadata if you have voted for the winning proposal and voting has ended
  /// @dev There is currently only one collection, logic must be changed if multiple collections are minted
  /// @return The collection token URIs
