...
contract ChallengeManager is Ownable {
  ...
  function submitData(Results[] memory results) public payable onlyApi {
    ...
    for (uint256 i = 0; i < results.length; i++) {
      require(
        challenges[results[i].challengeId].start < block.timestamp,
        'ChallengeManager: Challenge did not start yet'
      );
      require(
        challenges[results[i].challengeId].end > block.timestamp,
        'ChallengeManager: Challenge already over'
      );
      //todo add condition check and type check

      if (
        !hasUnlockedChallenge(results[i].challengeId, results[i].athleteAddress)
      ) {
        require(
          challenges[results[i].challengeId].submissionFee <= fund,
          'ChallengeManager: Not enough funds'
        );
        //todo add substract from ChallengeAccount here
        fund -= challenges[results[i].challengeId].submissionFee;

        (bool sent, ) = _gymnasiaAddress.call{
          value: challenges[results[i].challengeId].submissionFee -
            getKeyPrice(results[i].challengeId)
        }('');
        require(sent, 'ChallengeManager: Failed to send ether');

        challenges[results[i].challengeId].prizePool += getKeyPrice(
          results[i].challengeId
        );

        challengeKeys[results[i].challengeId][results[i].athleteAddress] = true;
      }

      leaderboards[results[i].challengeId][
        results[i].athleteAddress
      ] += results[i].value;
      athleteAddresses[results[i].challengeId].push(results[i].athleteAddress);
    }
  }
  ...
  function withdraw(uint256 challengeId) public {
    ...
    bool sent = payable(winner).send(challenges[challengeId].prizePool);
    require(sent, 'Challengemanager: Failed to send ether');
  }
