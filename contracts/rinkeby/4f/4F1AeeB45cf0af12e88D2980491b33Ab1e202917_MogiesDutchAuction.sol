...
contract MogiesDutchAuction is
    VRFConsumerBase,
    Ownable,
    ERC721A,
    ReentrancyGuard
{
    ...
    uint256 tier1UsersNotMinted = 0;
    uint256 tier1UsersMinted = 0;
    // provenance
    uint256 public startingIndex = 0;

    uint16[] public rebateFactor = [15000, 13000, 10000];
    ...
    constructor(
        ...
        address _link,
        address _owner,
        uint256 _ethPrice, // Price to lock these at beginning (used for rebate)
        uint256 _starsPrice, // Price to lock these at beginning (used for rebate)
    ...
    ) ERC721A("Mogies", "MOGIES") VRFConsumerBase(_vrfCoordinator, _link) {
    ...
    modifier saleMaxSupply(uint256 quantity) {
        require(
        ...
        callerIsUser
    {
        ...
        // caveate must mint all team reserved nfts
        require(
        ...
        if (isUsingStars) {
            ...
            saleConfig.ethPrice = (auctionPrice * ethPrice) / starsPrice;
            totalPaid = totalCost;
            stars.safeTransferFrom(msg.sender, address(this), totalPaid);
        } else {
            saleConfig.ethPrice = auctionPrice;
            saleConfig.starsPrice = (auctionPrice * starsPrice) / ethPrice;
            totalPaid -= refundIfOver(totalCost);
        }
        ...
        if (tier == 0) {
            tier1UsersNotMinted++;
            remainingMintAmount[msg.sender] = tier1UsersNotMinted;
        }
    ...
    function allowlistMint(
    ...
    ) external payable callerIsUser saleMaxSupply(quantity) {
        require(
        ...
        require(
            block.timestamp > saleConfig.whitelistSaleStartTime &&
                block.timestamp < saleConfig.whitelistSaleEndTime + 4 days,
            "outside of allowlist sale times"
        );
        uint256 price;
        if (isStars) {
            price = saleConfig.starsPrice;
            stars.safeTransferFrom(msg.sender, address(this), price * quantity);
        } else {
            price = saleConfig.ethPrice;
            refundIfOver(price);
        }
    ...
    function togglePublicSale() external onlyOwner {
        saleConfig.hasPublicSale = !saleConfig.hasPublicSale;
    }

    // ETH prices for auction
    uint256 public constant AUCTION_START_ETH_PRICE = 1 ether;
    uint256 public constant AUCTION_END_ETH_PRICE = 200000000 gwei; //0.2 eth
    uint256 public constant AUCTION_DROP_PER_STEP_ETH = 200000000 gwei; //0.2 eth

    // TODO: find out what stars prices will be
    uint256 public constant AUCTION_START_STARS_PRICE = 1 ether;
    uint256 public constant AUCTION_END_STARS_PRICE = 200000000 gwei; //0.2 eth
    uint256 public constant AUCTION_DROP_PER_STEP_STARS = 200000000 gwei; //0.2 eth
        ...
        returns (uint256)
    {
        if (_isUsingStars) {
            ...
            if (
                block.timestamp - _saleStartTime >= AUCTION_PRICE_CURVE_LENGTH
            ) {
                ...
                uint256 steps = (block.timestamp - _saleStartTime) /
                    AUCTION_DROP_INTERVAL;
                return
                    AUCTION_START_STARS_PRICE -
                    (steps * AUCTION_DROP_PER_STEP_STARS);
            }
        } else {
            if (block.timestamp < _saleStartTime) {
            ...
            if (
                ...
                uint256 steps = (block.timestamp - _saleStartTime) /
                    AUCTION_DROP_INTERVAL;
                return
                    AUCTION_START_ETH_PRICE -
                    (steps * AUCTION_DROP_PER_STEP_ETH);
            }
        }
    }
    ...
    function rebate() external {
        ...
        for (uint256 i = 0; i < sales[msg.sender].length; i++) {
            ...
            if (sales[msg.sender][i].isStars) {
                ...
                // rateBIP = x
                // differenceInEth = (pricePaidTotal - (finalPrice1Unit * quantity))
                // differenceValEth = differenceInEth * ethPrice
                // rebateAmount = (differenceValEth * rateBIP / starsPrice)/ 10000
                    ...
                    rebateAmount +=
                        (15000 *
                            ((sales[msg.sender][i].pricePaid -
                                (saleConfig.ethPrice * quantity)) * ethPrice)) /
                        (10000 * starsPrice);
    ...
    // For marketing etc.
    // assuming that these will be minted in the beginning
    function devMint(uint256 quantity, address recipient) external onlyOwner {
        require(
    ...
    function mintRemaining() external {
        ...
        uint256 quantity;
        if (tier1UsersNotMinted >= (totalAmount - totalSupply())) {
            quantity = 1;
        } else {
            quantity = (totalAmount - totalSupply()) / tier1UsersNotMinted;
            if (
                remainingMintAmount[msg.sender] <=
                (totalAmount - totalSupply() + (tier1UsersMinted * quantity)) %
                    (tier1UsersNotMinted)
            ) {
                quantity++;
            }
        ...
        override
    {
        if (startingIndexRequestId == _requestId) {
            startingIndex = _randomness % totalAmount;
        }
    }
