...
contract DappTest is ERC721A, Ownable, ReentrancyGuard {
    ...
    constructor(string memory _initBaseURI, string memory _notRevealedUri) ERC721A("DappTest", "DT") {
    ...
    // Returns metadata link of tokenid
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
    ...
    // Returns the number of NFTs minted for a wallet address
    function numberMinted(address owner) public view returns (uint256) {
        return _numberMinted(owner);
        }

    // Returns the tokens owned in a wallet address
    ...
    // Changes the public max NFTs per wallet
    function setMaxPerWallet(uint256 _limit) public onlyOwner {
      MaxperWallet = _limit;
    }

    // Changes the max free NFTs per wallet
    ...
    // Use this to shorten the supply of the collection
    function setMaxsupply(uint256 _newsupply) public onlyOwner {
      maxSupply = _newsupply;
    }

    // Use this to shorten the free supply for the collection
    function setFreesupply(uint256 _newsupply) public onlyOwner {
      FreeSupply = _newsupply;
    }

    // Sets the baseuri of the collection
    ...
    // Sets hidden uri for the collection
    ...
    // Withdraw funds currently held on the contract balance
