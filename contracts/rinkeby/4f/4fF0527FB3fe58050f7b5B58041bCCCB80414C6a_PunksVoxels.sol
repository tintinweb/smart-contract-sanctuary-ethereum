...
contract PunksVoxels is ERC721, ReentrancyGuard, IERC721Receiver, IERC1155Receiver {
    ...
    function wrapMany0(uint[] calldata tokenIds, bool mint) nonReentrant external {
        require(
        ...
        unchecked {
            ...
            if(mint) {
                for(uint i = 0; i < count; i++) {
                    require(_exists(toPunkId(tokenIds[i])) == false, "Already minted");
                }
                ...
                punkApes.update(count, msg.sender);
            } else {
                for(uint i = 0; i < count; i++) {
