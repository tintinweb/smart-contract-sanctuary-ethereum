...
contract P2PExchange is ReentrancyGuard {
    ...
    function transferToken(address _dst, address _token, uint256 _amount) onlyOwner external nonReentrant {
        require(users[address(this)][_token].amount >= _amount && _amount > 0, "Amount is invalid");
        uint256 tokenBalanceBefore = IERC20(_token).balanceOf(address(this));
    ...
    function transferETH(address _dst, address _user,uint256 _amount) external onlyOwner nonReentrant {
        require(users[address(this)][WETH].amount >= _amount && _amount > 0, "eth amount exceeds or zero");
