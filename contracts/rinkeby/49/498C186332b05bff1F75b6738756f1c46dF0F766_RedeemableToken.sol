...
contract RedeemableToken is ERC1155, Ownable, ERC1155Burnable {    
    ...
    address redemptionMiddlewareContract = 0x495524Cfe2d9A9Ba0666cDE34139d2966626149C;

    mapping (address => mapping (uint256 => uint256)) public currentSupply;
    mapping (address => mapping (uint256 => uint256)) public supplyLimit;
    ...
    function redeem(address owner, address initialCollection, uint256 tokenId) public payable returns(uint256) {
        ...
        require(currentSupply[initialCollection][authorizedCollections[initialCollection]] + 1 <= supplyLimit[initialCollection][authorizedCollections[initialCollection]], "Limit reached");
        ERC721 collectionRedeem = ERC721(initialCollection);
        require(collectionRedeem.ownerOf(tokenId) == owner, "Don't own that token");
        require(redeemedToken[initialCollection][tokenId] == 0x0000000000000000000000000000000000000000, "Token redeemd already");
        
        currentSupply[initialCollection][authorizedCollections[initialCollection]] = currentSupply[initialCollection][authorizedCollections[initialCollection]] + 1;
    ...
    function redeemBatch(address owner, address initialCollection, uint256[] calldata tokenIds) public payable {
        ...
        require(authorizedCollections[initialCollection] != 0, "Collection not authorized");
        require(currentSupply[initialCollection][authorizedCollections[initialCollection]] + tokenIds.length <= supplyLimit[initialCollection][authorizedCollections[initialCollection]], "Limit reached");
        ERC721 collectionRedeem = ERC721(initialCollection);

        for(uint256 i = 0; i < tokenIds.length; ++i) {
            require(redeemedToken[initialCollection][tokenIds[i]] == 0x0000000000000000000000000000000000000000, "Token redeemd already");
            require(collectionRedeem.ownerOf(tokenIds[i]) == owner, "Don't own that token");
            redeemedToken[initialCollection][tokenIds[i]] = owner;
        }

        currentSupply[initialCollection][authorizedCollections[initialCollection]] = currentSupply[initialCollection][authorizedCollections[initialCollection]] + tokenIds.length;
    ...
    function setSupply(address collectionAddress, uint256 tokenId, uint256 newSupply) public onlyOwner {
        supplyLimit[collectionAddress][tokenId] = newSupply;
    }
