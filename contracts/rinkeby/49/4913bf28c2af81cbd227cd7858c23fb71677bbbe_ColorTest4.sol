...
// File: contracts/ColorTest4.sol


// Amended by HashLips
// Modified by Nova.ETH
...
contract ColorTest4 is ERC721, Ownable {
    using Strings for uint256;
    ...
    function mint(uint256 _mintAmount) public payable mintCompliance(_mintAmount) {
        ...
        require(totalSupply() + _mintAmount <= maxSupply, "Purchase would exceed max NFT supply.");

        if (msg.sender != owner()) {
            if (onlyWhitelisted) {
                require(isWhitelisted(msg.sender), "Wallet address is not whitelisted.");
            }
            uint256 ownerTokenCount = balanceOf(msg.sender);
            require(ownerTokenCount < nftPerAddressLimit, "Tokens per wallet address limit reached.");
        }
    ...
    function _mintLoop(address _receiver, uint256 _mintAmount) internal {
        for (uint256 i = 0; i < _mintAmount; i++) {
            ...
            _safeMint(_receiver, _supplyCounter.current());
        }
    }
