...
contract Shrimp is ERC721ABurnable, Ownable, ReentrancyGuard {
 ...
 function whitelistFreeFishing(bytes32[] memory _merkleProof, uint fishCount) validateFishing(fishCount) nonReentrant external payable {
     require(currentMode == ReleaseMode.OPEN, "It's not go time yet.");
     require(freeClaimed < 10, "too many shrimpies claimed");
 ...
 @param fishCount Amount to mint.
 @notice This is the mint function called by folks before the general sale, assuming they're allow-listed.
.
 */
 function preSaleFishing(bytes32[] memory merkleProof, bytes32[] memory freeMerkleProof, uint fishCount) validateFishing(fishCount) validatePreSaleAction nonReentrant external payable {
   bytes32 leaf = keccak256(abi.encodePacked(msg.sender));
   require(MerkleProof.verify(merkleProof, _presaleMerkleRoot, leaf), "Not on allowlist");
   require(freeClaimed < 500, "too many shrimpies claimed");
   // You can get your free mint during the presale, which is in another Merkle Tree.
   // The amount we're charging (e.g, the amount we're validating) will depend on
   // if we detected a free mint match. If so we check for a lesser amount of eth.
   uint fishingFeesToCharge = fishCount;
   if(MerkleProof.verify(freeMerkleProof, _freeFishMerkleRoot, leaf)){
     if(_freeFishRedeemed[msg.sender] != 1){ // if the free is NOT already redeemed, mark it as used and let us use the free one.
       fishingFeesToCharge = fishingFeesToCharge - 3;
       _freeFishRedeemed[msg.sender] = 1;
       freeClaimed = freeClaimed + 3;
     }
