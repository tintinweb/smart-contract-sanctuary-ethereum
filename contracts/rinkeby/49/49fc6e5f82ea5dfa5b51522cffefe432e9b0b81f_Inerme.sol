...
contract Inerme is ERC721Enumerable, Ownable {
    using SafeMath for uint256;

    uint256 public constant _TOTALSUPPLY        = 10;
    uint256 public maxQuantity                  = 3;
    
    uint256 public price                        = 0.07 ether;
    uint256 public preSaleprice                 = 0.09 ether;
    // uint256 public maxPublicSaleMintPerWallet   = 5;
    uint public maxPerWallet =4;
    
    bool public isPaused                        = true;
    bool public isPreSalePaused                 = true;
    
    uint256 public preSaleSupply                = 5;
    uint256 public reserve                      = 2;
    
    uint256 private tokenId                     = 1;
    // uint256 public preSaleMaxQuantity           = 60;

    bytes32 private rootHash;

    address public partnerWallet1 = 0xAb8483F64d9C6d1EcF9b849Ae677dD3315835cb2;   
     address public partnerWallet2 = 0x4B20993Bc481177ec7E8f571ceCaE8A9e22C02db; 
    address public communityWallet = 0x78731D3Ca6b7E34aC0F824c42a7cC18A495cabaB;
    ...
    // constructor(string memory baseURI,bytes32 finalRootHash) ERC721("INERME", "INM") {
    //     setBaseURI(baseURI);
    //     rootHash = finalRootHash;
    // }
 constructor(string memory baseURI) ERC721("INERME", "INM") {
        setBaseURI(baseURI);
...
function setRootHash(bytes32 updatedRootHash) public onlyOwner {
    ...
    function preSalemint( uint256 quantity) public payable {
        ...
        // require(totalSupply() + quantity <= _TOTALSUPPLY, "Quantity is greater than remaining Supply");
        require(totalSupply() + quantity <= _TOTALSUPPLY- reserve, "Quantity is greater than remaining Supply");
        // require(_preSaleAddresses[msg.sender].counter + quantity <= preSaleMaxQuantity, "Quantity Must Be Lesser Than Max Supply");
        require(_preSaleAddresses[msg.sender].counter + quantity <= maxPerWallet, "Quantity Must Be Lesser Than Max Supply");
    ...
    //     }
    //     require(isPaused == true, "turn on minting");
    //     require(
    //         chosenAmount > 0,
    ...
    function mint(uint256 chosenAmount) public payable isSaleOpen {
        ...
        // require(_publicSaleAddresses[msg.sender].counter + chosenAmount <= maxPublicSaleMintPerWallet, "Max mints per wallet is 5");
         require(_preSaleAddresses[msg.sender].counter + chosenAmount <= maxPerWallet, "Quantity Must Be Lesser Than Max Supply");
        ...
        for (uint256 i = 0; i < chosenAmount; i++) {
            _safeMint(msg.sender, totalsupply());
            tokenId++;
        }
    ...
    function withdraw() public onlyOwner {
        ...
        uint256 ownerBalance = totalbalance * 25 / 100;
        uint256 partnerBalance1 = totalbalance * 25 / 100;
        uint256 partnerBalance2 = totalbalance * 25/ 100;
        uint256 communityBalance = totalbalance * 25 / 100;
        payable(msg.sender).transfer(ownerBalance);
        payable(partnerWallet2).transfer(partnerBalance2);
        payable(communityWallet).transfer(communityBalance);
        payable(partnerWallet1).transfer(partnerBalance1);
    }
