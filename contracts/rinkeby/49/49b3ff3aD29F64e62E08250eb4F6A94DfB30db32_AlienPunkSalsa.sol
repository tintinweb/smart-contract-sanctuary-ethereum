...
contract AlienPunkSalsa is ERC721A, Ownable {
    ...
    DroolRewards public immutable droolRewards;
    ...
    event Salsafied(uint256 startId, uint[] _aptIds);
    event Randomized(uint256 startId, uint quantity);

    constructor(address _apt, address _drool, address _droolRewards) ERC721A("AlienPunkSalsa", "SALSA") {
    ...
    function mintSalsafied(uint256[] memory aptIds) external {
        
        uint256 quantity = aptIds.length;
        uint256 totalMinted = _totalMinted();
        uint256 price = quantity * _salsafiedPrice;
        ...
        emit Salsafied(totalMinted, aptIds);
    }
