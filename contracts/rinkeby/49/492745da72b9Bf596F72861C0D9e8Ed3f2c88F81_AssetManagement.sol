...
contract AssetManagement is AccessControl {
    ...
    event Widthdraw(address reciver, address token, uint256 value,string  action);
    event WidthdrawETH(address reciver, uint256 value,string  action);
    ...
    function withdraw(address token,address to, uint256 amount,string memory action) public {
        require(hasRole(OPERATORROLE, msg.sender), "Caller is not a operator");
        IERC20(token).transfer(to,amount);
        emit Widthdraw(to,token,amount,action);

    }

    function withdrawETH(address to, uint256 amount,string memory action) public {
        require(hasRole(OPERATORROLE, msg.sender), "Caller is not a operator");
        IWETH(WETH).withdraw(amount);
        _safeTransferETH(to, amount);
        emit WidthdrawETH(to,amount,action);
    ...
    function userWalletBalance(address user) public view returns (address[] memory,uint256[] memory,uint256 ) {
