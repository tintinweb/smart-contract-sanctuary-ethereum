 ...
 * sale.
 */
contract BowlineMonsters is MintpassValidator, ERC721A, ERC2981, Ownable {
    ...
    uint256 public allowlistMintLimitPerWallet = 100;

    // Sale Stages Enabled / Disabled
    bool public USDMintEnabled = false;
    bool public allowlistMintEnabled = false;
    bool public publicMintEnabled = false;

    // Mint Prices
    int256 public mintUSDPrice = 1000000000; // = 10 USD
    ...
    function mint(address minter, uint256 quantity) public payable {
        require(
            ...
            "BowlineMonsters: Public Minting is not Enabled"
        );

        require(
            msg.value >= (publicMintPrice * quantity),
            "BowlineMonsters: Insufficient Amount"
        );

        require(
            TOKEN_LIMIT >= _nextTokenId(),
            "BowlineMonsters: Token Limit reached"
        );
     ...
     * - `quantity` user that should receive the token
     */
    function mintWithFiat(address minter, uint256 quantity) public payable {
        require(
            USDMintEnabled == true,
            "BowlineMonsters: Public Minting is not Enabled"
        );

        uint256 mintPrice = USDMintPrice();
        require(
    ...
    function allowlistMint(
        ...
        require(
            ...
            "BowlineMonsters: Allowlist Minting is not Enabled"
        );
        require(
        ...
        require(
            ...
            "BowlineMonsters: Maximum Allowlist per Wallet reached"
        );
    ...
    function USDMintPrice() public view returns (uint256) {
     ...
     * @param _USDMintEnabled true/false
     */
    function setMintingEnabled(
        bool _allowlistMintEnabled,
        bool _publicMintEnabled,
        bool _USDMintEnabled
    ) public virtual onlyOwner {
        allowlistMintEnabled = _allowlistMintEnabled;
        publicMintEnabled = _publicMintEnabled;
        USDMintEnabled = _USDMintEnabled;
    }

    /**
     * @dev Function to be called by contract owner to change the prices
     * for different mint tiers
     *
     * @param _allowlistMintPrice ether
     * @param _publicMintPrice ether
     * @param _USDMintPrice USD 1000000000; // = 10 USD
     */
    function setMintPrices(
        uint256 _allowlistMintPrice,
        uint256 _publicMintPrice,
        int256 _USDMintPrice
    ) public virtual onlyOwner {
        allowlistMintPrice = _allowlistMintPrice;
        publicMintPrice = _publicMintPrice;
        mintUSDPrice = _USDMintPrice;
    }
...
abstract contract MintpassValidator is EIP712 {
    constructor() EIP712("BowlineMonsters", "1") {}
