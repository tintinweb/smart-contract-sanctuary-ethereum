...
/**
 * @title EVAEDITIONS
 * @dev EVAEDITIONS ERC1155 NFT contract.
 * @author zero (kanon.art) for EVADAO
 */
...
contract AnymaFactory is Ownable {
  Anyma[] public addresses;
  event AnymaContractCreated(Anyma anymaContract);

  function createAnyma() external onlyOwner {
    Anyma anymaContract = new Anyma();

    anymaContract.transferOwnership(owner());

    addresses.push(anymaContract);
    emit AnymaContractCreated(anymaContract);
  }

  function getAnymaContracts() external view returns (Anyma[] memory) {
    return addresses;
  }
}
...
contract Anyma is IERC1155MetadataURI, ERC1155Supply, Ownable {
  struct Item {
    string uri;
    uint256 maxSupply;
  ...
  address[] freeMintWhitelist;
  address[] mintWhitelist;
  uint256 mintStartTimestamp;
  uint256 mintEndTimestamp;
  uint256 whitelistEndTimestamp;
  address proceedsRecipient;
  string private _contractURI;

  mapping(uint256 => string) tokensGatedURI;

  mapping(uint256 => Item) public items;
  uint256 public itemsCount = 0;

  modifier tokenExists(uint256 _tokenId) {
    require(_tokenId < itemsCount, "Token does not exist");
    _;
  }

  modifier onlyTokenArtist(uint256 _tokenId) {
    require(items[_tokenId].artist == msg.sender, "caller is not the token artist");
    _;
  }

  modifier onlyHolder(uint256 _tokenId) {
    require(balanceOf(msg.sender, _tokenId) > 0, "caller is not a token holder");
    _;
  }

  /**  @notice Constructor function
  */
  constructor() ERC1155("https://evadao.com/api/item/{id}.json") Ownable() {
  }
  ...
  */
  ...
  function addNewToken(
    ...
    Item memory item = Item(_tokenUri, _maxSupply, _maxMintPerWallet, _royaltyPercent, _royaltyRecipient, _artist);

    items[itemsCount] = item;
    tokensGatedURI[itemsCount] = _gatedURI;
    itemsCount++;
  }
  ...
  function setGatedURI(uint256 _tokenId, string memory _gatedURI)
    public
    tokenExists(_tokenId)
    onlyTokenArtist(_tokenId)
  {
    tokensGatedURI[_tokenId] = _gatedURI;
  }

  function gatedURI(uint256 _tokenId)
    public
    view
    tokenExists(_tokenId)
    onlyHolder(_tokenId)
    returns(string memory gatedUri)
  {
    return tokensGatedURI[_tokenId];
  }
  ...
  function setMintParameters(
    ...
    uint256 _whitelistEndTimestamp,
    address _proceedsRecipient
  ) public onlyOwner {
    mintPrice = _mintPrice;
    mintCurrency = _mintCurrency;
    mintStartTimestamp = _mintStartTimestamp;
    mintEndTimestamp = _mintEndTimestamp;
    whitelistEndTimestamp = _whitelistEndTimestamp;
    proceedsRecipient = _proceedsRecipient;
  }

  function getMintParameters() public view returns(
    uint256,
    address,
    uint256,
    uint256,
  ...
  function addToMintWhitelist(address[] memory _addresses) public onlyOwner {
    for (uint256 _i = 0; _i < _addresses.length; _i++) {
      mintWhitelist.push(_addresses[_i]);
    }
  }

  function addToFreeMintWhitelist(address[] memory _addresses) public onlyOwner {
    for (uint256 _i = 0; _i < _addresses.length; _i++) {
      freeMintWhitelist.push(_addresses[_i]);
    }
  }

  function getMintWhitelist() public view onlyOwner returns(address[] memory _addresses) {
    return mintWhitelist;
  }

  function getFreeMintWhitelist() public view onlyOwner returns(address[] memory _addresses) {
    return freeMintWhitelist;
  }

  function removeFromMintWhitelist(uint256[] memory _ids) public onlyOwner {
    for (uint256 _i = 0; _i < _ids.length; _i++) {
      _removeFromMintWhitelist(_ids[_i]);
    }
  }

  function removeFromFreeMintWhitelist(uint256[] memory _ids) public onlyOwner {
    for (uint256 _i = 0; _i < _ids.length; _i++) {
      _removeFromMintWhitelist(_ids[_i]);
    }
  }

  function resetMintWhitelist() public onlyOwner {
    delete mintWhitelist;
  }

  function resetFreeMintWhitelist() public onlyOwner {
    delete freeMintWhitelist;
  }

  function _removeFromMintWhitelist(uint256 _i) internal {
    mintWhitelist[_i] = mintWhitelist[mintWhitelist.length - 1];
    mintWhitelist.pop();
  }

  function _removeFromFreeMintWhitelist(uint256 _i) internal {
    freeMintWhitelist[_i] = freeMintWhitelist[freeMintWhitelist.length - 1];
    freeMintWhitelist.pop();
  }

  function mint(uint256 _tokenId, uint256 _amount, bytes memory _data)
    public
    payable
    tokenExists(_tokenId)
  {
    require(mintStartTimestamp < block.timestamp, "Mint phase has not yet started");
    require(mintEndTimestamp > block.timestamp, "Mint phase has ended");

    if (whitelistEndTimestamp > block.timestamp) {
      require(_isInFreeWhitelist(msg.sender) || _isInWhitelist(msg.sender), "You are not in whitelist");
    }

    Item memory _token = items[_tokenId];

    require(balanceOf(msg.sender, _tokenId) + _amount <= _token.maxMintPerWallet, "Cannot mint this amount to a single wallet");
    require(totalSupply(_tokenId) + _amount <= _token.maxSupply, "Amount exceeds max token supply");

    _mint(msg.sender, _tokenId, _amount, _data);

    if (block.timestamp > whitelistEndTimestamp || !_isInFreeWhitelist(msg.sender)) {
      if (mintCurrency == address(0)) {
        require(msg.value >= (mintPrice * _amount), "sent amount is not enough");

        (bool sent,) = payable(proceedsRecipient).call{value: msg.value}("");
        require(sent, "value not sent to recipient");
      } else {
        IERC20(mintCurrency).transferFrom(msg.sender, proceedsRecipient, mintPrice * _amount);
      }
    }
  }

  function checkMintedMax(uint256 _tokenId)
    public
    view
    tokenExists(_tokenId)
    returns(bool)
  {
    return (balanceOf(msg.sender, _tokenId) >= items[_tokenId].maxMintPerWallet);
  }
    ...
    returns (address, uint256)
  {
    ...
    uint256 royaltyAmount = (_salePrice * item.royaltyPercent) / _feeDenominator();

    return (item.royaltyRecipient, royaltyAmount);
  }

  function _feeDenominator() internal pure returns(uint8) {
    return 100;
  }
  ...
  function checkFreeMintWhitelist() public view returns(bool) {
    return _isInFreeWhitelist(msg.sender);
  }

  function _isInFreeWhitelist(address _minterAddress) internal view returns(bool) {
    bool _isWhitelisted = false;

    for (uint256 _i = 0; _i < freeMintWhitelist.length; _i++) {
      if (freeMintWhitelist[_i] == _minterAddress) {
        _isWhitelisted = true;
        break;
      }
    }
  ...
  function _isInWhitelist(address _minterAddress) internal view returns(bool) {
    bool _isWhitelisted = false;

    for (uint256 _i = 0; _i < mintWhitelist.length; _i++) {
      if (mintWhitelist[_i] == _minterAddress) {
        _isWhitelisted = true;
        break;
      }
    }
