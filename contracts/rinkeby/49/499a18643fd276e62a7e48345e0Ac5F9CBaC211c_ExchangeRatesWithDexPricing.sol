...
contract ExchangeRates is Owned, MixinSystemSettings, IExchangeRates {
    ...
    function synthTooVolatileForAtomicExchange(bytes32)
        external
        view
        returns (bool)
    {
    ...
    function _rateIsFlagged(bytes32 currencyKey, FlagsInterface flags)
        internal
        view
        returns (bool)
    {
...
contract ExchangeRatesWithDexPricing is ExchangeRates {
    ...
    constructor(address _owner, address _resolver)
        public
        ExchangeRates(_owner, _resolver)
    {}

    /* ========== SETTERS ========== */

    function setDexPriceAggregator(IDexPriceAggregator _dexPriceAggregator)
        external
        onlyOwner
    {
    ...
    function atomicEquivalentForDexPricing(bytes32 currencyKey)
        external
        view
        returns (address)
    {
    ...
    function atomicVolatilityConsiderationWindow(bytes32 currencyKey)
        external
        view
        returns (uint256)
    {
    ...
    function atomicVolatilityUpdateThreshold(bytes32 currencyKey)
        external
        view
        returns (uint256)
    {
        ...
        )
    {
        (
            ...
            bool usePureChainlinkPriceForSource
         = getPureChainlinkPriceForAtomicSwapsEnabled(sourceCurrencyKey);

            bool usePureChainlinkPriceForDest
         = getPureChainlinkPriceForAtomicSwapsEnabled(destinationCurrencyKey);
        uint256 sourceRate;
        uint256 destRate;
    ...
    function _getMinValue(uint256 x, uint256 y)
        internal
        pure
        returns (uint256)
    {
    ...
    function _getMaxValue(uint256 x, uint256 y)
        internal
        pure
        returns (uint256)
    {
    ...
    function _getPriceFromDexAggregator(
        ...
        IERC20 sourceEquivalent = IERC20(
            getAtomicEquivalentForDexPricing(sourceCurrencyKey)
        );
        require(
            address(sourceEquivalent) != address(0),
            "No atomic equivalent for source"
        );

        IERC20 destEquivalent = IERC20(
            getAtomicEquivalentForDexPricing(destCurrencyKey)
        );
        require(
            address(destEquivalent) != address(0),
            "No atomic equivalent for dest"
        );

        uint256 result = _dexPriceDestinationValue(
            sourceEquivalent,
            destEquivalent,
            amount
        ...
        return
            destCurrencyKey == "sUSD"
                ? result
                : SafeDecimalMath.unit().divideDecimalRound(result);
    }
    ...
    function _dexPriceDestinationValue(
        ...
        uint256 sourceAmountInEquivalent = (
            sourceAmount.mul(10**uint256(sourceEquivalent.decimals()))
        )
        ...
        return
            (twapValueInEquivalent.mul(SafeDecimalMath.unit())).div(
                10**uint256(destEquivalent.decimals())
            );
    }

    function synthTooVolatileForAtomicExchange(bytes32 currencyKey)
        external
        view
        returns (bool)
    {
        ...
        uint256 considerationWindow = getAtomicVolatilityConsiderationWindow(
            currencyKey
        );
        uint256 updateThreshold = getAtomicVolatilityUpdateThreshold(
            currencyKey
        );
        ...
        uint256 considerationWindowStart = block.timestamp.sub(
            considerationWindow
        );
        uint256 roundId = _getCurrentRoundId(currencyKey);
        for (updateThreshold; updateThreshold > 0; updateThreshold--) {
            (uint256 rate, uint256 time) = _getRateAndTimestampAtRound(
                currencyKey,
                roundId
            );
