...
contract SDLC_NFT is ERC721A, Ownable {
    ...
    mapping(address => uint256) public MAP_WHITELIST_MINT;
    ...
    //whitelist mint
    function mintWhitelist(uint256 _mintAmount, uint256 _index, uint256 _amount, bytes32[] calldata _proof) public {
        require(isWhiteListed(toLeaf(msg.sender, _index, _amount), _proof), "Invalid proof");
        
        require((MAP_WHITELIST_MINT[msg.sender] + _mintAmount) <= _amount, "Exceeds Max Mint Amount");

        //Mint
        _mintLoop(msg.sender, _mintAmount);
        MAP_WHITELIST_MINT[msg.sender] = MAP_WHITELIST_MINT[msg.sender] + _mintAmount;
    }
    ...
    function stake( uint256[] calldata _tokenIDList) external {
        ...
        for (uint256 i = 0; i < _tokenIDList.length; i++) {
            if (MAP_STAKED_TOKEN[_tokenIDList[i]] == false) {
                newStakedCount++;
