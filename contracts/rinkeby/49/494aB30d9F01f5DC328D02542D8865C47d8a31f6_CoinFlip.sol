...
contract CoinFlip is VRFConsumerBaseV2 {
    ...
    function verdict(uint256 random) public payable onlyVFRC {
        ...
        lastGameId = gameId;
        // payWinner(lastWinAmount,lastGameId);
        
    }

    // function payWinner(uint256 Amount, uint256 Id) public payable onlyPlayer {
    //     // for(uint256 i=lastGameId; i<gameId; i++){
    //     //     games[i].player.transfer(lastWinAmount);
    //     // }
    //     // gameId.player.transfer(winAmount);
    //      games[Id].player.transfer(Amount);
