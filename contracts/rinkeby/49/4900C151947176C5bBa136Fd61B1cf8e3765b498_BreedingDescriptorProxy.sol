...
contract BreedingDescriptorProxy is Ownable {
    ...
    function tokenURI(uint256 _tokenId) public view returns (string memory) {
        ...
        if (evolutionPodId > 0) {
            return IDescriptor(dnaChipDescriptorAddress).breedingTokenURI(evolutionPodId);
        }
...
interface IDescriptor {
    ...
    function breedingTokenURI(uint256 _tokenId) external view returns (string memory);
}
