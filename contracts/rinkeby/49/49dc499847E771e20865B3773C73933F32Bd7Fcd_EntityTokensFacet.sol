...
interface IEntityTokensFacet {
   ...
   */
  function getTokenInfo(address _unit) external view returns (
    address tokenContract_,
    uint256 currentTokenSaleOfferId_
  );
  ...
  function tknAllowance(address _unit, address _spender, address _owner) external view returns (uint256);
  // ERC-20 mutations
  function tknApprove(address _unit, address _spender, address _from, uint256 _value) external;
  function tknTransfer(address _unit, address _operator, address _from, address _to, uint256 _value) external;
}
