    ...
    Ownable
{
        ...
        returns (string memory)
    {
        ...
        for(uint256 i = 1; i < attrParts.length; i++){
            trait = string.concat(trait,',{"trait_type": "Stuuuf","value": "', attrParts[i], '"}');
        }
        // count the attributes like OpenSea displays
        trait = string.concat(trait,',{"trait_type": "Stuuuf","value": "',  Strings.toString(attrParts.length-1)  , ' Attributes"}');
        // show how stretched they are
        // reusing attrs here because the stack is getting deep
        attrs = "i";
        uint256 count = 1;
        while(count < (newWidth/500)*100){
