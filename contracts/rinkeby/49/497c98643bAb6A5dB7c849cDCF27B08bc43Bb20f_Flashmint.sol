...
import {IERC165, ERC721, Strings} from "@openzeppelin/contracts/token/ERC721/ERC721.sol";
...
contract Flashmint is 
    ...
    /// @notice the settings, which manages royalties on the wrapped NFT
    address public immutable settings; 

    /// @notice the price a lender must pay to loan the NFT. in wei, ie 18 decimals (10**18 = 1)
    uint256 public fee;
    ...
    event FlashmintedUnsafe(address indexed minter, address indexed minted, uint256 amount, uint256 fee);
    ...
    // @dev for use with calls to external contracts, esp `token`
    modifier noStealing {
        _;
    ...
    function _withdrawEth() internal {
        ...
        if (protocolFee > 0 && protocol != address(0)) {
            (bool sent, ) = payable(protocol).call{value: protocolFee}("");
            require(sent, "_withdrawEth: protocol payment failed");
        }

        (bool sent, ) = payable(recipient).call{value: amount - protocolFee}("");
        ...
        noStealing
    {
        ...
        require(req.flightStatus == FlightStatus.LANDED, "Didn't get an NFT, or NFT doesn't use safeMint");
    ...
    /// @notice this function is unsafe if the NFT has a nonstandard `approve` fn
    /// @notice this function is unsafe if the NFT has a 
    /// @dev see `lendToMint`
    function lendToMintUnsafe(
        address _derivativeNFT,
        bytes calldata _mintData
    ) external payable
        nonReentrant
        ...
        noStealing
    {
        ...
        emit FlashmintedUnsafe(msg.sender, _derivativeNFT, gained, fee);
    }
    ...
    /// === TOKENURI FUNCTIONS  ===
    /// ---------------------------
    // TODO: move baseURI from Settings
...
contract Settings is Ownable {
    ...
    constructor() {
        baseURI = "http://flashmint.vercel.app/api/tokenURI/";
    }
    ...
    function setProtocolFee(uint256 _protocolFeeBips) external onlyOwner {
        require(_protocolFeeBips < 1000, "fee: cmon");
