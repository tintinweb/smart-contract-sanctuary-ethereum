...
function createNewLvl(uint _numberLvl,uint _priceStart,uint _refillC,uint _refillPer)public onlyOwner returns(uint){
    Level storage lvl = levels[_numberLvl];
    lvl.numberLvl = _numberLvl;
    lvl.priceToStart = _priceStart;
    lvl.refillCount = _refillC;
    lvl.refillPercent = _refillPer;
    emit CreatedNewLevel(_numberLvl, _priceStart, _refillC, _refillPer, block.timestamp);
    return _numberLvl;
}
function buyLevel(uint buyLvl,address _refferal)public nonReentrant {
    User storage user = users[msg.sender];
    user.Dibil = _refferal;
...
// function UserInfoView(address _user) public view returns(address ,uint ,uint ,uint){
// User storage user = users[_user];
// return(user.Dibil,user.currentlvl,user.earned);
//}
...
function _sendToRefferalPercent(address _user) internal {
    ...
    User storage refUser = users[user.Dibil];
