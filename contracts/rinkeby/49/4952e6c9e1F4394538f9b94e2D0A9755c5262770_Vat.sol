...
contract Vat {
    ...
    mapping (address => uint256)                   public mrt;  // [rad]
    ...
    function move(address src, address dst, uint256 rad) external {
        require(wish(src, msg.sender), "Vat/not-allowed");
        mrt[src] = _sub(mrt[src], rad);
        mrt[dst] = _add(mrt[dst], rad);
    }
    ...
    function heal(uint rad) external {
        ...
        mrt[u] = _sub(mrt[u], rad);
        vice   = _sub(vice,   rad);
        debt   = _sub(debt,   rad);
    }
    function suck(address u, address v, uint rad) external auth {
        sin[u] = _add(sin[u], rad);
        mrt[v] = _add(mrt[v], rad);
        vice   = _add(vice,   rad);
        debt   = _add(debt,   rad);
    }
