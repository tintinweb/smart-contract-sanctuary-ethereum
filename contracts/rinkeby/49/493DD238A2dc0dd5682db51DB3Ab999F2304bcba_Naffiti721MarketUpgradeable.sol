...
contract Naffiti721MarketUpgradeable is Initializable, ERC721Holder, OwnableUpgradeable {
    ...
    /*╔═════════════════════════════╗
      ║          MODIFIERS          ║
      ╚═════════════════════════════╝*/
    modifier isOrderExists(address _nftContractAddress, uint256 _tokenId) {
        require(nftContractAuctions[_nftContractAddress][_tokenId].nftSeller != address(0), "The order not exists");
        _;
    }
    ...
    function _transferNftToSeller(address _nftContractAddress, uint256 _tokenId) internal {
        address nftSeller = nftContractAuctions[_nftContractAddress][_tokenId].nftSeller;
        IERC721(_nftContractAddress).safeTransferFrom(address(this), nftSeller, _tokenId);
        require(IERC721(_nftContractAddress).ownerOf(_tokenId) == nftSeller, "nft transfer failed");
    }

    /**
     * Setup parameters applicable to all auctions and whitelised sales:
     * -> collection address
     * -> array of the token need to reset the order
     */

    function resetAllOrders(address _nftContractAddress, uint256[] memory _tokenArray) public onlyOwner {
        for (uint256 i = 0; i < _tokenArray.length; i++) {
            _withdrawAuction(_nftContractAddress, _tokenArray[i]);
        }
    }
    ...
    function _setupAuction(
        ...
        address _erc20Token,
        uint128 _minPrice
    ) internal minPriceDoesNotExceedLimit(2**128 - 1, _minPrice) {
        if (_erc20Token != address(0)) {
            nftContractAuctions[_nftContractAddress][_tokenId].ERC20Token = _erc20Token;
        }
        nftContractAuctions[_nftContractAddress][_tokenId].buyNowPrice = 2**128 - 1;
        nftContractAuctions[_nftContractAddress][_tokenId].minPrice = _minPrice;
        nftContractAuctions[_nftContractAddress][_tokenId].nftSeller = msg.sender;
        nftContractAuctions[_nftContractAddress][_tokenId].nftHighestBid = _minPrice;
    }
    ...
    {
        require(
            _auctionEnd - block.timestamp > 10800 && _auctionEnd - block.timestamp < 604800,
            "Auction End Timestamp is Invalid"
        );
        ...
        _createAuction(_nftContractAddress, _tokenId, _erc20Token, _minPrice, _auctionEnd, _identifier);
    }
    ...
    function createSale(
        ...
        _setupSale(_nftContractAddress, _tokenId, _erc20Token, _buyNowPrice);
        emit SaleCreated(_nftContractAddress, _tokenId, msg.sender, _erc20Token, _buyNowPrice, _identifier);
    }
    ...
    function makeBid(
        ...
        address _erc20Token,
        uint128 _tokenAmount
    ) external payable isOrderExists(_nftContractAddress, _tokenId) auctionOngoing(_nftContractAddress, _tokenId) {
        require(
            nftContractAuctions[_nftContractAddress][_tokenId].bidIncreasePercentage != 0,
            "This is a sale order, not an auction order"
        );
        ...
        bidAmountMeetsBidRequirements(_nftContractAddress, _tokenId, _tokenAmount)
    {
        require(
            nftContractAuctions[_nftContractAddress][_tokenId].bidIncreasePercentage == 0,
            "This is an auction order, not a sell order"
        );
        _reversePreviousBidAndUpdateHighestBid(_nftContractAddress, _tokenId, _tokenAmount);
    ...
    function _payFeesAndSeller(
        ...
        // Pay commission
        // 1.0% = 1 / 100, 1.5% = 15 / 1000, 1.75% = 175 / 10000
        uint256 commission = (_highestBid * commissionPercentage) / 10000;
        ...
        isAuctionOver(_nftContractAddress, _tokenId)
    {
        require(
            ...
            "Only highest bidder can settle auction"
        );
        _transferNftAndPaySeller(_nftContractAddress, _tokenId);
        emit AuctionSettled(_nftContractAddress, _tokenId, msg.sender);
    }

    function _withdrawAuction(address _nftContractAddress, uint256 _tokenId) internal {
        _transferNftToSeller(_nftContractAddress, _tokenId);
        address highestBidder = nftContractAuctions[_nftContractAddress][_tokenId].nftHighestBidder;
        if (highestBidder != address(0)) {
            uint256 highestBid = nftContractAuctions[_nftContractAddress][_tokenId].nftHighestBid;
            _payout(_nftContractAddress, _tokenId, highestBidder, highestBid);
        }
        _resetAuction(_nftContractAddress, _tokenId);
        _resetBids(_nftContractAddress, _tokenId);
    }
    ...
    function withdrawAuction(address _nftContractAddress, uint256 _tokenId) external {
        require(nftContractAuctions[_nftContractAddress][_tokenId].nftSeller == msg.sender, "Not NFT seller");
        _withdrawAuction(_nftContractAddress, _tokenId);
    ...
    function setCommissionReceiver(address _newReceiver) public onlyOwner {
        commissionReceiver = _newReceiver;
    }

    function setCommissionPercentage(uint32 _newPercentage) public onlyOwner {
