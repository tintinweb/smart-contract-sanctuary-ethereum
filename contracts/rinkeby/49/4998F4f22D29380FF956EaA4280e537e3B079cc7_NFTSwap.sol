...
contract NFTSwap is ReentrancyGuard, IERC721Receiver {
    ...
    mapping (IERC20 => bool) public rewardsTokens;

    function addReward(INFTFull _nftContract, IERC20 _rewardToken, uint _amount) public nonReentrant {
        require(address(_nftContract) != address(0) && address(_rewardToken) != address(0), "NFTSwap: ZERO_ADDRESS");
        require(_amount != 0, "NFTSwap: ZERO_AMOUNT");
        require(_nftContract.owner() == msg.sender, "NFTSwap: AUTH_FAILED");
        require(!rewardsTokens[_rewardToken] || rewardsTokensInternal[_nftContract].contains(address(_rewardToken)), "NFTSwap: TOKEN_ALREADY_USED");

        rewardsTokensInternal[_nftContract].add(address(_rewardToken)); // don't check if existed before, just add more rewards
        require(rewardsTokensInternal[_nftContract].length() <= MAX_TOKENS_FOR_REWARDS, 'NFTSwap: TO_MUCH_TOKENS');

        rewardsTokens[_rewardToken] = true;
