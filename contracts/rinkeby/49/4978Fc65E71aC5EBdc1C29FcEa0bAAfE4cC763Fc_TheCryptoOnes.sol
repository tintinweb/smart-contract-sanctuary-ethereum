...
contract TheCryptoOnes is ERC721A, Ownable, Pausable, ReentrancyGuard {
    ...
    uint256 public constant MINT_PRICE_TEAM = 0.02 ether;
    ...
    bool public teamSaleStarted = false;
    bool public preSaleStarted = false;
    bool public lock = false;
    string private _identifierTeam;
    string private _identifierWl;
    address private _signerAddress;

    string public baseTokenURI = "";

    address payable walletDev;
    address payable walletTeam;

    uint256 devCut = 5;

    mapping(uint256 => string) public tokenURIs;

    constructor() ERC721A("TheCryptoOnes", "THECRYPTOONES") { }
    ...
    function mintTeam(
        ...
        returns (uint256)
    {
        require(teamSaleStarted, "MINT_NOT_STARTED");
        require(_totalMinted() <= MAX_SUPPLY, "MAX_SUPPLY_REACHED");
        require(msg.value == MINT_PRICE_TEAM, "INVALID_ETH_SENT");

        bytes32 hash = keccak256(abi.encode(_msgSender(), _identifierWl));
        ...
        returns (uint256)
    {
        ...
        require(_totalMinted() <= MAX_SUPPLY, "MAX_SUPPLY_REACHED");
        require(_totalMinted() >= TEAM_SUPPLY, "TEAM_NOT_MINTED");
        require(msg.value == MINT_PRICE_WL, "INVALID_ETH_SENT");
    ...
    function isWhiteList(
        ...
        bytes32 _hash = keccak256(abi.encode(_sender, _identifierWl));
    ...
    function mint() public payable nonReentrant returns (uint256) {
        ...
        require(_totalMinted() >= TEAM_SUPPLY, "TEAM_NOT_MINTED");
        require(_totalMinted() <= MAX_SUPPLY, "MAX_SUPPLY_REACHED");
        require(msg.value == MINT_PRICE_PUBLIC, "INVALID_ETH_SENT");
    ...
    function setTeamSaleStarted(bool _hasStarted) external onlyOwner {
    ...
    function setDevWallet(address _address) external onlyOwner {
        ...
        walletDev = payable(_address);
    }

    function setTeamWallet(address _address) external onlyOwner {
        require(_address != address(0), "ADDRESS_ZERO");
        walletTeam = payable(_address);
    }

    function withdraw() external {
        uint256 balance = address(this).balance;
        uint256 walletBalance = balance.mul(devCut).div(100);
        payable(walletDev).transfer(walletBalance);
        payable(walletTeam).transfer(balance.sub(walletBalance));
    }
    ...
    function withdrawERC20(address _token) external {
        ...
        targetToken.transferFrom(address(this), walletDev, walletBalance);
        targetToken.transferFrom(address(this), walletTeam, balance.sub(walletBalance));
    }
