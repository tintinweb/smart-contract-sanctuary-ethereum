...
contract DreamCarClub is ERC721Enumerable, Ownable {
    ...
    uint256 public PRICE = 300 * 1000000;
    ...
    //address public constant creatorAddress = 0x2b73bB18F16CAC1cC277D706cf075c063624eDDB;
    ...
    event welcomeToDCC(uint256 indexed id);
    event NewPriceEvent(uint256 price);
    event NewMaxElement(uint256 max);

    constructor(address _singenr, address _tokenAddress) ERC721("Dream Car Club NFTs", "DCC") {
        ...
        saleIsOpen
    {
        ...
        require(IERC20(tokenAddress).balanceOf(wallet) >= price(_tokenAmount), "DCC: Value below price");
        IERC20(tokenAddress).transferFrom(wallet, address(this), price(_tokenAmount));
    ...
    function crossmint(address _to, uint256 _tokenAmount) public payable saleIsOpen {
        ...
        address wallet = _msgSender();
        require(wallet == CROSSMINT_WALLET, "DCC: This function is for Crossmint only.");
    ...
    function withdrawAll() public onlyAdmin {
        ...
        require(balance > 0);
        IERC20(tokenAddress).transfer(creatorAddress, balance);
    }
    ...
    function mintUnsoldTokens(uint256[] memory _tokensId) public onlyOwner {
        require(PAUSE, "DCC: Pause is disable");
...
interface IERC20 {
    ...
    function transferFrom(address sender, address account, uint256 amount) external;
}
