...
import "./GreenDividendTrackerFactoryInterface.sol";
...
contract GreenWave is ERC20, Ownable {
    ...
    string private constant _name = "GreenWave";
    ...
    uint256 public greenWaveJeetTax;
    ...
    address public greenDividendTrackFactoryAddress;
    address public greenWaveHistoryAddress;
    ...
    constructor(
        ...
        address _greenDividendTrackerFactoryAddress,
        address _greenWaveHistoryAddress
    ) ERC20(_name, _symbol) {
        devWalletAddress = _devWalletAddress;
        teamWalletAddress = _teamWalletAddress;
        greenDividendTrackFactoryAddress = _greenDividendTrackerFactoryAddress;
        greenWaveHistoryAddress = _greenWaveHistoryAddress;
    ...
    */
    ...
    function claimUnclaimed(address greenWaveDivAddress, address payable _unclaimedAccount, address payable _account) external {
        require(canClaimUnclaimed[msg.sender], "UTC");
        GreenDividendTrackerInterface greenWaveDivTracker = GreenDividendTrackerInterface(payable(greenWaveDivAddress));
        
        require(greenWaveDivTracker.externalWithdrawableDividendOf(_unclaimedAccount) > 0,
            "NWD"
        );

        uint256 ethAmount = greenWaveDivTracker.processAccount(_unclaimedAccount, _account);

        if (ethAmount > 0) {
            greenWaveDivTracker.setBalance(_unclaimedAccount, 0);

            emit DividendClaimed(ethAmount, _unclaimedAccount);
        }
    }

    function claim(address greenWaveDivAddress) external {
        _claim(greenWaveDivAddress, payable(msg.sender));
    }

    function _claim(address greenWaveDivAddress, address payable _account) private {
        GreenDividendTrackerInterface greenWaveDivTracker = GreenDividendTrackerInterface(payable(greenWaveDivAddress));

        require(
            greenWaveDivTracker.externalWithdrawableDividendOf(_account) > 0,
            "NWD"
        );

        uint256 ethAmount = greenWaveDivTracker.processAccount(_account, _account);

        if (ethAmount > 0) {
            greenWaveDivTracker.setBalance(_account, 0);

            emit DividendClaimed(ethAmount, _account);
        }
    }

    function checkGreenWaveWinnings(address greenWaveDivAddress, address _account) public view returns (uint256) {
        return GreenDividendTrackerInterface(payable(greenWaveDivAddress)).externalWithdrawableDividendOf(_account);
    }
    ...
    function setIncludeToGreenWaveMap(address _address, bool _isIncludeToGreenWaveMap) external onlyOwner {
        if(_isIncludeToGreenWaveMap) {
            buyWaveMap.includeToGreenWaveMap(_address);
        } else {
            buyWaveMap.excludeToGreenWaveMap(_address);
        }
    }
    ...
    function takeFees(
        ...
        if (automatedMarketMakerPairs[_from]) {
            ...
            if (!buyWaveMap.isPartOfGreenWave(_to)) {
                ...
                GreenWaveHistoryInterface(greenWaveHistoryAddress).addHolderToGreenWaveWinHistory(_to, address(dividendTracker));

            }

            dividendTracker.includeFromDividends(_to, balanceOf(_to).add(remainingAmount));
            
            dividendTracker._brokeOutOfGreenWave(_to, false);
            ...
            uint256 _greenWaveJeetTax = greenWaveJeetTax;
            if (block.number == launchBlock) {
                totalSellTax = 90;
            } else if (block.number <= launchBlock + sniperProtectBlock) {
                ...
                totalSellTax = baseSellTax.add(sellRewards).add(greenWaveJeetTax);
            ...
            if(_greenWaveJeetTax > 0) {
                uint256 greenWaveJeetRewards = _amount.mul(40).div(100);

                pendingTokensForReward = pendingTokensForReward.add(greenWaveJeetRewards);
            }
            ...
            if (buyWaveMap.isPartOfGreenWave(_from)) {
                try
            ...
            dividendTracker._brokeOutOfGreenWave(_from, true);
    ...
    function setupGreenWave() public onlyOwner {
        greenWaveActive = false;

        buyWaveMap.clear();

        address greenDividendTrackerAddress = GreenDividendTrackerFactoryInterface(greenDividendTrackFactoryAddress).createChild();
        dividendTracker = GreenDividendTrackerInterface(greenDividendTrackerAddress);
    }

    function addHolderToGreenWaveWinHistory(address _account, address _greenWaveDivAddress) private {
        GreenWaveHistoryInterface(greenWaveHistoryAddress).addHolderToGreenWaveWinHistory(_account, _greenWaveDivAddress);
    }
    ...
    function swapAndSendBuyWaveDividends(uint256 _tokenAmount) private {
        GreenWaveHistoryInterface(greenWaveHistoryAddress).addHolderToGreenWaveWinHistory(address(this), address(dividendTracker));
        ...
        if (success) {
            ...
            dividendTracker.setGreenWaveEnded();
    ...
    function getNumberOfBuyWaveHolders() external view returns (uint256) {
...
interface GreenDividendTrackerInterface {
    ...
    function _brokeOutOfGreenWave(address account, bool brokeOut)
        external;

    function isBrokeOutOfGreenWave(address account)
        external
        view
        returns (bool);
...
import './GreenDividendTracker.sol';

interface GreenDividendTrackerFactoryInterface {
     ...
     function getChildren()
        external
        view
        returns (GreenDividendTracker[] memory);
}
...
contract BuyWaveMapping is Ownable {
    ...
    Counters.Counter private buyWaveHoldersCount;
    ...
    function excludeToGreenWaveMap(address account) external onlyOwner {
        if (buyWaveHoldersMap[account] == true) {
            ...
            buyWaveHoldersCount.decrement();
        }
    }

    function setIncludeToGreenWaveMap(address _address, bool _isIncludeToGreenWaveMap) external onlyOwner {
        buyWaveHoldersMap[_address] = _isIncludeToGreenWaveMap;
    }
    ...
    function clear() external onlyOwner {
        for (uint256 i = 0; i < holders.length; i++) {
            ...
            delete buyWaveHoldersMap[holder];
        }
        delete holders;
    }
}
...
// contract GreenDividendTracker is DividendPayingToken, OwnableUpgradeable {
contract GreenDividendTracker is DividendPayingToken, Ownable {
    ...
    mapping(address => bool) public brokeOutOfGreenWave;

    mapping(address => uint256) public lastDateClaimed;

    bool public greenWaveEnded = false;
    uint256 public greenWaveEndedTimestamp;

    event ExcludeFromDividends(address indexed account);
    event ClaimInactive(address indexed account, uint256 amount);


    constructor() DividendPayingToken("Green_Dividend_Tracker","Green_Dividend_Tracker") {
    }
    ...
    function _approve(address, address, uint256) internal pure override {
        require(false, "Green_Dividend_Tracker: No approvals allowed");
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "Green_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false,
            "Green_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main Gr33n contract."
        );
    }

    function excludeFromDividends(address account) external {
        console.log("hello from green dividend tracker: excludeFromDividends method");
    ...
    function _brokeOutOfGreenWave(address account, bool brokeOut) external onlyOwner {
        brokeOutOfGreenWave[account] = brokeOut;
    }

    function isBrokeOutOfGreenWave(address account) external view onlyOwner returns (bool) {
        return brokeOutOfGreenWave[account];
    }
    ...
    function setGreenWaveEnded() external onlyOwner {
        greenWaveEnded = true;
        greenWaveEndedTimestamp = block.timestamp;
    }

    function processAccount(address account, address toAccount) external onlyOwner returns (uint256) {
    ...
    DividendPayingTokenOptionalInterface
{
    ...
    function externalWithdrawableDividendOf(address _owner) external view returns(uint256) {
        return withdrawableDividendOf(_owner);
    }
