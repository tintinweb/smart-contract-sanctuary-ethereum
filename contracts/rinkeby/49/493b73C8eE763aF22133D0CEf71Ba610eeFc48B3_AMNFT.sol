...
contract AMNFT is ERC721, ERC721Enumerable, ERC721URIStorage, AccessControl {
    ...
    uint256 public passDuration = 5 minutes;
    ...
    /** 
    ***************** onlyRole(ADMIN_ROLE) methods ************************************
    */

    function changePassPrice(uint256 _changedPassPrice) external onlyRole(ADMIN_ROLE) {
        require(passPrice != _changedPassPrice, "Pass price did not change.");
        passPrice = _changedPassPrice;
    }

    function changeRenewalPrice(uint256 _changedRenewalPrice) external onlyRole(ADMIN_ROLE) {
     ...
     */
    function setShowTokenURI(bool _showtokenURI) public onlyRole(ADMIN_ROLE) {
     ...
     */
    function setWhitelistMintSwitch(bool _whitelistMintSwitch) public onlyRole(ADMIN_ROLE) {
     ...
     */
    function setPassDuration(uint256 _passDuration) external onlyRole(ADMIN_ROLE) {
     ...
     */
    function setMintSwitch(bool _mintSwitch) public onlyRole(ADMIN_ROLE) {
     ...
     */
    function setDateSpecificDropSwitch(bool _dateSpecificDropSwitch) public onlyRole(ADMIN_ROLE) {
     ...
     */
    function setRenewTokenSwitch(bool _renewTokenSwitch) public onlyRole(ADMIN_ROLE) {
     ...
     * @param _tokenId - tokenId 
     */
    function renewPassOwner(uint _tokenId, uint256 timestampUntil) public payable onlyRole(ADMIN_ROLE){
        require(_exists(_tokenId), "Pass does not exist.");
     ...
     */  
    function withdraw(address _address) external payable onlyRole(ADMIN_ROLE) {
     ...
     */  
    function initNewDrop(uint256 numberOfNewSlots, address[] calldata addresses) public onlyRole(ADMIN_ROLE) {
     ...
     */  
    function initDateSpecificDrop(uint256 numberOfNewSlots, address[] calldata addresses, uint256 expiryDate) external onlyRole(ADMIN_ROLE) {
     ...
     */  
    function stopDrop() external onlyRole(ADMIN_ROLE) {
        setDateSpecificDropSwitch(false);
        setMintSwitch(false);
        setWhitelistMintSwitch(false);
    }

    function addAddressesToWhitelist(address[] calldata addresses) public onlyRole(ADMIN_ROLE) {
    ...
    function increaseMaxSupply(uint256 numberOfNewSlots) public onlyRole(ADMIN_ROLE) {
        maxSupply += numberOfNewSlots;

    }

    function decreaseMaxSupply(uint256 _numTokens) external onlyRole(ADMIN_ROLE) {
        require(maxSupply - _numTokens >= lastTokenId(), "Supply cannot fall below minted tokens.");
        maxSupply -= _numTokens;
    }

    function deleteWhitelistedAddresses() public onlyRole(ADMIN_ROLE) {
        delete whitelistedAddresses;
    }
   
    function deleteMintedAddressesForDrop() public onlyRole(ADMIN_ROLE) {
        delete mintedAddressesForDrop;
    }

    function burnToken(uint256 tokenId) public virtual onlyRole(ADMIN_ROLE) {
        _burn(tokenId);
    }
    
    function burnExpiredTokens() public virtual onlyRole(ADMIN_ROLE) {
