...
contract MetaBrot is ERC721TradableMetaBrot {
    ...
    function add_to_WL(address[] memory _addresses, uint256[] memory _quantities) external 
    {
        for (uint256 i = 0; i < _addresses.length; i++) {
            WL_spots[_addresses[i]] = _quantities[i];
        }
    }    
    function add_to_FM(address[] memory _addresses, uint256[] memory _quantities) external 
    {
        for (uint256 i = 0; i < _addresses.length; i++) {
