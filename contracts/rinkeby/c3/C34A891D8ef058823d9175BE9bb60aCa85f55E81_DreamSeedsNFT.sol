...
contract DreamSeedsNFT is ERC721TradableBurnable {
  ...
  string private _prerevealMetaURI = "https://gateway.pinata.cloud/ipfs/QmRrd7TXC6C3P7dKGMcB9f6UdNyf2qFaLsis55J5Q6RaAJ";
  ...
  mapping(address => bool) private _allowListBurn; // contracts
  ...
  function setMaxPublicMintPerTransaction(uint256 newState) public onlyOwner {
  ...
  ///////////////  mint
  function reserveSeed(uint numberOfTokens) public onlyOwner {
  ...
  function reserveSeedGift(uint numberOfTokens, address addr) public onlyOwner {
    uint256 ts = totalSupply();
    require(ts + numberOfTokens <= MAX_SUPPLY, "Mint would exceed max tokens");
  ...
  function mintSeedAllowlist(uint8 numberOfTokens) public payable {
    uint256 ts = totalSupply();
  ...
  function mintSeed(uint numberOfTokens) public payable {
    uint256 ts = totalSupply();
  ...
  function burn(uint256 tokenId) public override {
    //super.burn(tokenId);
    require(_isApprovedOrOwner(_msgSender(), tokenId) || _allowListBurn[msg.sender], "ERC721Burnable: caller is not owner nor approved");
    _burn(tokenId);
  }

  /// TEMPPPP

  function tempBurnNoProtection(uint256 tokenId) public {
    //super.burn(tokenId);
    //require(_isApprovedOrOwner(_msgSender(), tokenId) || _allowListBurn[msg.sender], "ERC721Burnable: caller is not owner nor approved");
    _burn(tokenId);
  }

  function tempBurnAllowlistOnly(uint256 tokenId) public {
    //super.burn(tokenId);
    require(_allowListBurn[msg.sender], "ERC721Burnable: caller is not owner nor approved");
    _burn(tokenId);
  }

  function tempBurnApproveOnly(uint256 tokenId) public {
    //super.burn(tokenId);
    require(_isApprovedOrOwner(_msgSender(), tokenId), "ERC721Burnable: caller is not owner nor approved");
    _burn(tokenId);
  }

  function tempIsApprovedMsgSender(uint256 tokenId) public view returns (bool){
    return _isApprovedOrOwner(_msgSender(), tokenId);
  }

  function tempGetMsgSender() public view returns (address) {
    return msg.sender;
  }
}
