...
contract NWAllianceMembership is ERC721A, Ownable {
    ...
    bytes32 public presale = 0x46a485f755a9ddf097b241b8d920aa11183adf292c17d4d05b8c52daabf7dc3e;
    bool public presaleOnly = true;
    bool public mintPaused = false;
    uint8 public maxMintPerWalletPlus1 = 6;
    uint8 public mintCost = 0; // in 0.1 ETH increments
    ...
    function presaleMint(bytes32[] calldata presale_, uint8 amt_) external payable priceMet(amt_) notPaused {
        ...
        require ((nextTokenId-1 + amt_) < maxPreSaleSupplyPlus1, "This order exceeds maximum Pre-Sale Total Supply");
    ...
    modifier priceMet (uint amt_) {
        if (mintCost > 0) {
            require ((amt_ * mintCost * 0.1 ether) < msg.value + 1, "Mint price not met" );
        }
        _;
    }
    ...
    function get_config() external view returns (uint16, bool, bool, uint8, uint8, uint16, uint16) {
      return (
        nextTokenId,
        mintPaused,
        presaleOnly,
    ...
    function set_mintCost(uint8 costInTenths_) external onlyOwner { mintCost = costInTenths_; }
