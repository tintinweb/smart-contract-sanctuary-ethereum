...
contract Minting is Initializable,  ERC1155Upgradeable , OwnableUpgradeable  {
    ...
    function mint(uint256 amount, uint256 price)
        public
        onlyOwner
    {
        counter=counter.add(1);
        NFTInfoForTokenId[counter]._id = counter;
        NFTInfoForTokenId[counter]._artist = payable(msg.sender);
        NFTInfoForTokenId[counter]._price = price;
        NFTInfoForTokenId[counter]._index = NFTInfoForTokenId[counter]._index.add(amount);
    ...
    function purchaseRecord( uint256 id) external payable{
        ...
        require(amt >= NFTInfoForTokenId[id]. _price, "Amount mismatched");
        address payable admin = payable(0x821fC28a7f932fCeDACd44627810379bA49D1356);
        NFTInfoForTokenId[id]._index=NFTInfoForTokenId[id]._index.sub(1);
        NFTInfoForTokenId[id]._purchaseValue=NFTInfoForTokenId[id]._purchaseValue.add(1);
        admin.transfer(amt); 
        _tokenURI[id] = string(abi.encodePacked(_baseURI, Strings.toString(id), ".json")); 
        buyers[id][msg.sender] = NFTInfoForTokenId[id]._purchaseValue;
    }
    ...
    function uri(uint256 _id) public view virtual override(ERC1155Upgradeable) returns (string memory) {
        return bytes(_baseURI).length > 0 ? string(abi.encodePacked(_baseURI, Strings.toString(_id), ".json")) : _baseURI;
   }
}
