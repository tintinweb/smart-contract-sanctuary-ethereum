...
// RebelsPass Clone with Merkle Proof for WL sale.
...
error CantSubDeployerFromOwners();

contract RebelsClone is ERC721A, Ownable {  
    using Strings for uint256;
    ...
    mapping(address => uint256) private owners; // more selective than tribulators

    uint256 public sale_state = 0;

    address ownerA = 0x97C86B00A1aE8203f64E00D92004463C900D10D5;
    address ownerB = 0xAf499D5Ea28583d4C3F227FFdB41a09D4eDd4B9F;

    // uint256 public pre_sale_cost = 0.079 ether;
    // uint256 public public_sale_cost = 0.089 ether;
    // for testing purposes
    uint256 public pre_sale_cost = 0.00079 ether;
    uint256 public public_sale_cost = 0.00089 ether;
	...
	constructor() ERC721A("RC11N", "RC11S") {
        owners[_msgSender()] == 1;
    }

    modifier onlyOwners() {
        if (owners[_msgSender()] != 1) revert NotAnOwner();
        _;
    }

    function setMyBaseURI(string memory _myNewBaseURI) external onlyOwners {
    ...
    function setMerkleRoot(bytes32 _merkleRoot) external onlyOwners {
 	...
 	function setMaxSupply(uint256 _maxSupply) external onlyOwners {
         max_supply = _maxSupply;
    }
    function setSaleState(uint256 _saleState) external onlyOwners {
        sale_state = _saleState;
    }
    function setPreSaleCost(uint256 _preSaleCost) external onlyOwners {
        pre_sale_cost = _preSaleCost;
    }
    function setPublicSaleCost(uint256 _publicSaleCost) external onlyOwners {
        public_sale_cost = _publicSaleCost;
    }
    function setPreSaleMintLimit(uint256 _preSaleMintLimit) external onlyOwners {
        pre_sale_mint_limit = _preSaleMintLimit;
    }
    function setPublicSaleMintLimit(uint256 _publicSaleMintLimit) external onlyOwners {
    ...
    function addTribulator(address trib) external onlyOwners {
        tribulator[trib] = 1;
    }
    function subTribulator(address trib) external onlyOwners {
        tribulator[trib] = 0;
    }
    function addOwner(address ownerToAdd) external onlyOwners {
        owners[ownerToAdd] = 1;
    }
    function subOwner(address ownerToSub) external onlyOwners {
        if (ownerToSub == owner()) revert CantSubDeployerFromOwners();
        owners[ownerToSub] = 0;
    }
    function setTribulationWallet(address _tribulationWallet) public override onlyOwners {
        tribulationWallet = _tribulationWallet;
    }
    ...
    function withdraw() external payable onlyOwners nonReentrant {
        uint256 balance0 = address(this).balance;
        (bool Os, ) = payable(owner()).call{value: balance0 * 5 / 100}("");
        if (!Os) revert WithdrawOFailed();

        (bool As, ) = payable(ownerA).call{value: balance0 * 47 / 100}("");
        if (!As) revert WithdrawAFailed();

        (bool Bs, ) = payable(ownerB).call{value: balance0 * 48 / 100}("");
        if (!Bs) revert WithdrawBFailed();

        (bool Ds, ) = payable(owner()).call{value: address(this).balance}("");
