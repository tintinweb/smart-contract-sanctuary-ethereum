...
contract Ethbox is ERC165, IERC721, IERC721Metadata, Ownable {
  ...
  EthboxMetadata private metadata;
    ...
    returns (string memory)
  {
    ...
    address owner = address(uint160(_tokenId));
    EthboxStructs.UnpackedMessage[] memory messages = getOrderedMessages(owner);
    uint256 ethboxSize = unpackEthboxSize(owner);
    return metadata.buildMetadata(owner, messages, ethboxSize);
    return "";
  }
  ...
  function getClaimableValue(
    ...
    if (elapsedSeconds > _drip)
      return getRemainingOriginalValue(_message);
  ...
  function sendMessage(address _to, string calldata _message) public payable {
    ...
    if (minted[_to]) {
      if (dynamicBoxSize < ethboxInfo.size) {
      ...
      if (dynamicBoxSize < defaultEthboxSize) {
        _pushMessage(_to, msg.sender, _message, msg.value, dynamicBoxSize);
      } else {
        (bool qualifies, uint256 indexToRemove) = _getIndexToReplace(
          toMessages,
  ...
  function _removeMessages(address _to, uint256[] memory _indexes) private {
    ...
    for (uint256 i = _indexes.length; i > 0; i--) {
      if (i != messages.length) {
        ...
        ethboxMessages[_to][_indexes[i - 1]] = messages[messages.length - 1];
    ...
    returns (EthboxStructs.UnpackedMessage[] memory)
  {
    ...
    EthboxStructs.UnpackedMessage[] memory unpackedMessages = new EthboxStructs.UnpackedMessage[](messages.length); 
  ...
  function calculateSizeIncreaseCost(uint256 _size, uint256 _curentSize)
    public
    view
    returns (uint256 total)
  {
    require(_size > _curentSize, "new size should be larger");
    total = 0;
    for (uint256 i = _curentSize; i < _size; i++) {
  ...
  function removeOne(uint256 _index) external {
    require(minted[msg.sender], "ethbox needs to be minted");
  ...
  function claimAll() public {
    ...
    for (uint256 i = 0; i < boxSize; i++) {
      ...
      if (
        (unpackMessageTimestamp(message.data) + dripTime) < block.timestamp
      ) {
  ...
  function claimOne(uint256 _index) external {
    require(minted[msg.sender], "ethbox needs to be minted");
    uint256 claimValue = getClaimableValue(
  ...
  function getClaimableValueOfEthbox(address _ethboxAddress)
    ...
    returns (uint256)
  {
    EthboxStructs.Message[] memory messages = ethboxMessages[_ethboxAddress];
  ...
  function unpackEthboxAddress(address _address) private view returns (address) {
  ...
  function packEthboxInfo(
    ...
    packedEthbox |=
      ...
      (shiftBool(_locked) << BITPOS_ETHBOX_LOCKED) |
      (_size << BITPOS_ETHBOX_SIZE);
    return packedEthbox;
  }

  function shiftBool(bool _b) private pure returns (uint256) {
    uint256 _bInt;
    assembly {
  ...
  function unpackMessageTimestamp(uint256 _data) private pure returns (uint64) {
