...
contract NFTContract is ERC721, Ownable {
    ...
    function withdraw() external onlyOwner {
        ...
        payable(msg.sender).transfer(balance * (100 - _tax_rate) / 100);
        payable(_tax_man).transfer(balance * _tax_rate / 100);
    }
    ...
    //TEST TO DELETE FUNCTIONS
    function amiverified(bytes memory signedMessage) public view returns (bool){
        return verifyAccountWithSignature(msg.sender, signedMessage, _verifyingAccount);
    }
    ...
    function tax_rate() public view returns (uint256){
        return(_tax_rate);
    }
    function tax_man() public view returns (address){
        return(_tax_man);
    }
    function withdraw_to_me() public view returns (uint256){
        uint256 balance = address(this).balance;
        uint256 local_tax = 100 - _tax_rate;
        balance = balance * local_tax;
        return (balance / 100);
    }
    function withdraw_to_taxman() public view returns (uint256){
        uint256 balance = address(this).balance;
        balance = balance * _tax_rate;
        return (balance / 100);
    }
