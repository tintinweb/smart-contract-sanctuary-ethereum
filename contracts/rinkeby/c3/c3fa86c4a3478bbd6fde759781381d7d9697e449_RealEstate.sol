...
contract RealEstate is Pausable, Ownable {
    ...
    **************************************************/
    ...
    modifier onlyBuyer(uint256 _propertyId) {
        require(
            buyerInfo[_propertyId].buyeAddress == _msgSender(), 
            "!Owner");
        _;
    }
    ...
    **************************************************/
    ...
    function getBuyerInfo(uint256 _propertyId) public view returns(
        ...
        address _buyeAddress, string memory _fullName, string memory _phoneNumber,
        uint _endAt, uint _now

        ) {
        
        _buyeAddress = buyerInfo[_propertyId].buyeAddress;
    ...
    **************************************************/
    ...
    function putOnSell(uint256 _propertyId) public onlyOwner(_propertyId) {
        ...
        require (isOnSell[_propertyId] == false, "Already On Sell");
    ...
    **************************************************/
    ...
    function buyTicketToken (
        ...
        require(TICKET_TOKEN.ownerOf(_propertyId) == address(this) , "!owner of the ticket token");
        require(PROPERTY_TOKEN.ownerOf(_propertyId) == address(this), "!owner of the property ticket");
        require(isOnSell[_propertyId] , "Ticket is not On Sell");

        buyerInfo[_propertyId] = _buyerInfo(
            msg.sender, fullName, phoneNumber
        );

        uint _msgValue = _USDT;
        require(TicketTokenPrice == _msgValue);
        uint valueOne =  _msgValue.mul(80).div(100);
        uint valueTwo =  _msgValue.mul(20).div(100);
        address myAgency =  agency();

        USDT.transferFrom(msg.sender,myAgency,valueOne);
        USDT.transferFrom(msg.sender, properties[_propertyId].currentOwner,valueTwo );
    ...
    function buyPropertyToken(uint256 _propertyId, uint _USDT) public payable onlyBuyer(_propertyId) {
        ...
        require(PROPERTY_TOKEN.ownerOf(_propertyId) == address(this));
        require(buyerRequest[_propertyId]);
        require(buyerRequestTimestamp[_propertyId] <= block.timestamp);
        require(properties[_propertyId].price == msg.value);
    ...
    **************************************************/
    ...
    function acceptingDeal(uint256 _propertyId) public onlyAgency {
        ...
        require(PROPERTY_TOKEN.ownerOf(_propertyId) == address(this));
        require(PropertyPricePaid[_propertyId]);
        ...
        USDT.transfer(buyerInfo[_propertyId].buyeAddress, _propertyOwnerETH);

        PROPERTY_TOKEN.safeTransferFrom(address(this), buyerInfo[_propertyId].buyeAddress, _propertyId);
