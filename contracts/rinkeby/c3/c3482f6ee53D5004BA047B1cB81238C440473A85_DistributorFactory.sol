...
contract DistributorFactory is Operators, IDistributorFactory { 
    ...
    function withdraw(
        ...
        override
    {
        distributors[_poolID].withdraw(_campaignID,_amount,_maxAmount, _merkleProof);
    }
    ...
    function setCampaignMerkleTree(
        string calldata _poolID,
        string calldata _campaignID,  
        bytes32 merkleTreeRoot
    ) public override onlyOperator
    {
        distributors[_poolID].setCampaignMerkleTree(  
            _campaignID, 
            merkleTreeRoot
        );
    }
...
contract MixDistributor is ReentrancyGuard, BaseDistributor {
    ...
    mapping(string => bytes32) campaignMerkleTrees;
    ...
    function setCampaignMerkleTree(string calldata _campaignID, bytes32 merkleTreeRoot) 
        external 
        onlyOwner 
    {
        campaignMerkleTrees[_campaignID] = merkleTreeRoot;
    }
    ...
    function withdraw(
        ...
        bytes32 leaf = keccak256(
            abi.encodePacked(_campaignID, _user, _maxAmount)
        );
        require(
            MerkleProof.verify(_merkleProof, campaignMerkleTrees[_campaignID], leaf),
        ...
        require(_amount <= _maxAmount - claimedAmount[_campaignID][_user], "Exceed max amount of withdraw.");
        claimedAmount[_campaignID][_user] += _amount;
