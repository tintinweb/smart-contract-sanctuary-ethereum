...
contract AlienBees is ERC721, Ownable, ReentrancyGuard, RandomlyAssigned{    
    ...
    mapping (address => bool) public HpSaleWallets;
    ...
    mapping (address => uint256) public countDetails;
    constructor(uint256 _whiteListedLimit, uint256 _HPLimit, uint256 _publicLimit, uint256 _reservedNFTCount, address _reservedNFTaddress, uint256 _maxNftSupply, string memory _baseURI, string memory _placeholderURI, string memory _provinanceHash, address[] memory whiteListedAddress, address[] memory _HpSaleWallets) ERC721("testing", "test")RandomlyAssigned(_maxNftSupply, 1){
        require(_maxNftSupply > 0, "The maximum supply value cannot be less than 1 ");
        require(_reservedNFTCount < _maxNftSupply, "you cannot reserved all nft");
        ...
        for(uint32 HpwalletLoop =0; HpwalletLoop < _HpSaleWallets.length; HpwalletLoop++){
            if(HpSaleWallets[_HpSaleWallets[HpwalletLoop]] != true ){
                HpSaleWallets[_HpSaleWallets[HpwalletLoop]] = true;
            }
        }
    }

    function setHPSale(bool flag) external onlyOwner{
        HPSale = flag;
    }
    ...
    function addNewWalletForHPSale(address[] memory _hpsaleWallets)external onlyOwner{  
        for(uint32 hpwalletLoop =0; hpwalletLoop < _hpsaleWallets.length; hpwalletLoop++){
            if(HpSaleWallets[_hpsaleWallets[hpwalletLoop]] != true ){
                HpSaleWallets[_hpsaleWallets[hpwalletLoop]] = true;
            }
    ...
    function mintBee(uint256 quantity) public payable {
        ...
        require(HPSale == true ,"Minting is not yet open");
        require( (quantity + tokenCount()) <= totalSupply(), "Mint Completed"); 
        require(msg.value >= (mintPrice * quantity), "Mint Price can not be empty!");
        require(HpSaleWallets[msg.sender] == true && HPSale == true || whiteListedWallets[msg.sender] == true && whiteListSale == true || publicSale == true,"Minting is not yet open");
        uint256 remainingLimit = countDetails[msg.sender];
        console.log("Limit remaining" , (remainingLimit+quantity));

        uint256 totalLimit = (HpSaleWallets[msg.sender] == true && HPSale == true) ?  HPLimit: 0;
        totalLimit += (whiteListedWallets[msg.sender] == true && whiteListSale == true) ? WhiteListedLimit : 0;
        totalLimit += (publicSale == true) ? publicLimit : 0;
        console.log("Total limit are " , totalLimit);

        require( (remainingLimit + quantity) <= totalLimit, "Sorry, you are not allowed to mint any more. Mint Limit has been met");
        for (uint256 i = 1; i <= quantity; i++) {
            uint256 id = nextToken();
            _safeMint(msg.sender, id);
        }
        countDetails[msg.sender] +=quantity;
    ...
    function updateWalletLimit(uint256 saleType, uint256 newLimit) public onlyOwner{
        require(newLimit > 0 , "Limit should be greater than 0!");
        require(saleType == 1 || saleType == 2 || saleType == 3, "Type is not valid!");
        if(saleType == 1){
    ...
    function updateReservedWalletLimit(uint256 newLimit) public onlyOwner{
        require(newLimit > 0, "New limit should be greater than 0");
        reservedNFTCount = newLimit;
    }
