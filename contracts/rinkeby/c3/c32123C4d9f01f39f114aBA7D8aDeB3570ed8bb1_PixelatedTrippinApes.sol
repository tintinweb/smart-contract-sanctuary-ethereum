...
contract PixelatedTrippinApes is ERC721A, Ownable {
   using Strings
    ...
    uint256 public isMaxPublicSaleMints = 3234234;
    uint256 public isFree = 5; // RIP, 696.9 but rounded!
    string public BaseURI = "https://remix.ide/";
    ...
    modifier transactionCheck(uint256 _quantity, uint256 hasMinted) {   
         if(totalSupply() + _quantity <= isFree) {
            isPublicCost = 0;
         } else if(totalSupply()  != isFree){
            uint256 pricing = ((_quantity)-(totalSupply() - isFree)) *isPublicCost;
            isPublicCost = pricing;
         } else {
            uint256 pricing = _quantity*isPublicCost;
            isPublicCost = pricing;
         }
        // This above, is some real big brain stuff you code stealers don't understand 😂😂😂 get on my level
        require(msg.value == isPublicCost, "Pixelated Trippin Apes :: Looks like the incorrect eth amount!");
    ...
    function getPrice(uint256 _quantity) public view  returns(string memory) {
        require(_quantity >= 1, "Pixelated Trippin Apes :: Must be more than 1.");
         if(totalSupply() + _quantity <= isFree) {
            return string(abi.encodePacked("0"));
         } else if(totalSupply()  != isFree){
            uint256 pricing = ((_quantity)-(totalSupply() - isFree)) *isPublicCost;
            return string(abi.encodePacked(pricing.toString()));
         } else {
            uint256 pricing = _quantity*isPublicCost;
            return string(abi.encodePacked(pricing.toString()));
         }
    } // Easy to read to send value for the minting DApp.
