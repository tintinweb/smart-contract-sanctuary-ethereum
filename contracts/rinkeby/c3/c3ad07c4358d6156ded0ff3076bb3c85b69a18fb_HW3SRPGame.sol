//請使用 Ropsten或 Rinkeby 測試網測試，不能用單機 VM 測試
...
contract HW3SRPGame is usingOraclize {
    ...
    event resultEvent(bytes32 indexed queryId, uint indexed contractGuess, bytes32 indexed result);
    ...
    function __callback(bytes32 _queryId, string _result, bytes _proof) public { 
        ...
        if (oraclize_randomDS_proofVerify__returnCode(_queryId, _result, _proof) != 0) {
            ...
            emit resultEvent(_queryId, contractGuess, stringToBytes32(ans));
        }
    }
    ...
    function bytes32ToString(bytes32 x) constant public returns(string){
