...
contract TPOR is ERC721Enumerable, Ownable {
  ...
  function setPresaleUsers(address[] memory presaleAddresses) public onlyOwner {
    whitelistedAddresses = presaleAddresses;
  }
  ...
  function transferRoyalty(uint _amount) public payable onlyOwner{
      require(_amount < msg.sender.balance,"Not enough Balance");
      require(msg.value >= _amount);
      require(whitelistedAddresses.length > 0);
        uint256 userRoyalty = _amount / whitelistedAddresses.length;
        for (uint256 i = 1; i <= whitelistedAddresses.length; i++) {
            // for (uint256 j = 1; j <= totalSupply(); j++) {
            //     if(whitelistedAddresses[i-1] == ownerOf(j)){
                    payable(whitelistedAddresses[i-1]).transfer(userRoyalty);
                // }
            // }
        }
  }
