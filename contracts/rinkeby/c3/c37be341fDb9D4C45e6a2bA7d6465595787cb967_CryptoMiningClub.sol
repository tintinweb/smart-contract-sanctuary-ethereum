...
contract CryptoMiningClub is ERC721DividentPaying, Ownable {
	...
	uint256 public maxMintAmountPerAddressOG;
	uint256 public maxMintAmountPerAddressNonOG;

	mapping (address => uint256) public mintsOfAddress;

	bytes32 private _whitelistMerkleRootOG;
	bytes32 private _whitelistMerkleRootNonOG;

	constructor(string memory initUnrevealedUriPrefix) ERC721DividentPaying("Crypto Mining Club", "CMC") {
		paused = true;
		revealed = false;
		whitelistOnly = true;

		_whitelistMerkleRootOG = 0x8ecc88a984d1fdbdbf7c86ea512ef01d23279492cda675404d50db47ded37c11;
		_whitelistMerkleRootNonOG = 0xca45c7dbd392e79b7fed39e67c44a46ccbdb259cadf662fd7c3c3ec79028ea77;

		cost = 0.14 ether;
		presaleCost = 0.10 ether;
		maxSupply = 10_005;
		presaleSupply = 400;
		maxMintAmountPerAddressNonOG = 5;
		maxMintAmountPerAddressOG = 8;
		maxMintAmountPerAddress = 8;
	...
	function mint(uint256 amount) external payable {
		require(amount > 0 && getMintsOfAddress(_msgSender()) + amount <= maxMintAmountPerAddress, "CryptoMiningClub: invalid mint amount");

		uint256 supply = totalSupply();

		require(supply + amount <= maxSupply, "CryptoMiningClub: max token supply exceeded");
		require(!paused, "CryptoMiningClub: minting is paused");
		// require(!whitelistOnly, "CryptoMiningClub: minting currently in presale, use presale mint");
		require(msg.value >= cost * amount, "CryptoMiningClub: insufficient ether");


		_mintLoop(_msgSender(), amount, supply);

		mintsOfAddress[_msgSender()] += amount;
	}

	function presaleMint(uint256 amount, bytes32[] calldata merkleProof) external payable {
		
		require(!paused, "CryptoMiningClub: minting is paused");
		require(whitelistOnly, "CryptoMiningClub: minting no longer restricted to presale, use public mint");

		uint256 supply = totalSupply();
		uint256 newSupply = supply + amount;

		require(newSupply <= presaleSupply, "CryptoMiningClub: presale token supply exceeded");
		
		if (isAddressWhitelistedOG(_msgSender(), merkleProof)) {
			require(amount > 0 && getMintsOfAddress(_msgSender()) + amount <= maxMintAmountPerAddressOG, "CryptoMiningClub: invalid mint amount");
		} else if (isAddressWhitelistedNonOG(_msgSender(), merkleProof)) {
			require(amount > 0 && getMintsOfAddress(_msgSender()) + amount <= maxMintAmountPerAddressNonOG, "CryptoMiningClub: invalid mint amount");
		} else {
			require( false, "CryptoMiningClub: invalid merkle proof");
		}
		
		require(msg.value >= presaleCost * amount, "CryptoMiningClub: insufficient ether");
	...
	function airDrop(address[] calldata addresses, uint8[] calldata amounts) external onlyOwner {
		require(addresses.length == amounts.length, "CryptoMiningClub: invalid function arguments");
		require(addresses.length > 0, "CryptoMiningClub: invalid function arguments");


		uint256 supplyIndex = totalSupply();
		uint256 newSupply = supplyIndex;

		for (uint256 i = 0; i < amounts.length; i++) newSupply += amounts[i];

		require(newSupply <= maxSupply, "CryptoMiningClub: max token supply exceeded");


		for (uint256 i = 0; i < addresses.length; i++) {
			_mintLoop(addresses[i], amounts[i], supplyIndex);

			supplyIndex += amounts[i];
		}
	...
	function flipWhitelistOnlyState() external onlyOwner {
	...
	function isAddressWhitelistedOG(address account, bytes32[] calldata merkleProof) public view returns(bool) {
		bytes32 leaf = keccak256( abi.encodePacked(account) );
		bool whitelisted = MerkleProof.verify(merkleProof, _whitelistMerkleRootOG, leaf);

		return whitelisted;
	}

	function isAddressWhitelistedNonOG(address account, bytes32[] calldata merkleProof) public view returns(bool) {
		bytes32 leaf = keccak256( abi.encodePacked(account) );
		bool whitelisted = MerkleProof.verify(merkleProof, _whitelistMerkleRootNonOG, leaf);

		return whitelisted;
	}

	function getMintsOfAddress(address account) public view returns(uint256) {
		return mintsOfAddress[account];
	}
	...
	function setPresaleSupply(uint256 newAmount) external onlyOwner {
		require(newAmount < maxSupply, "CryptoMiningClub: presale supply must be smaller than the total supply");

		presaleSupply = newAmount;
	}
	...
	function setWhitelistMerkleRoot(bytes32 newWhitelistMerkleRootNonOG, bytes32 newWhitelistMerkleRootOG) public onlyOwner {
		_whitelistMerkleRootNonOG = newWhitelistMerkleRootNonOG;
		_whitelistMerkleRootOG = newWhitelistMerkleRootOG;
	}
	...
	function _mintLoop(address to, uint256 amount, uint256 supply) internal {
