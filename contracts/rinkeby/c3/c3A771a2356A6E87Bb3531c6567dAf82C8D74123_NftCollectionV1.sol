...
contract NftCollectionV1 is LazyMinting {
  ...
  function mintTo(string memory nftId, address to) public payable nonReentrant {
    mintNftToAnAddress(nftId, to);
  }

  function mintNftToAnAddress(string memory nftId, address to) private {
  ...
  function mint(string memory nftId) public payable override nonReentrant {
    mintNftToAnAddress(nftId, msg.sender);
  }
  ...
  IERC2981
{
  ...
  constructor(
    ...
    for (uint256 i = 0; i < nftParamsArr.length; i++) {
      NftParams memory nftParams = nftParamsArr[i];
      require(!isEmptyString(nftParams._id), 'nftId cannot be empty');
