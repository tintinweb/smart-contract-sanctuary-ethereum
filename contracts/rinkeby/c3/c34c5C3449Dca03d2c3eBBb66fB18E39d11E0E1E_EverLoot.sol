...
contract EverLoot is ERC721A, AccessControl, ReentrancyGuard {
    ...
    mapping(uint => mapping(address => bool)) private _claimed;

    // tracks a full bulk claim
    event Claim(address indexed to, bytes32[] indexed uniqueItemIds, uint256 indexed startingTokenId);

    // tracks a purchased loot chunk
    event Forge(address indexed to, uint256 amount, uint256 indexed startingTokenId);
    ...
    * @param claimedIndex the _claimed index to check
    * @param user the user address to check
    * wraps _claimed mapping
    */
    function getClaimStatus(uint claimedIndex, address user)
        public
        view
        returns (bool status) {
        return _claimed[claimedIndex][user];
    }
    ...
    */
    function claim(
        address to,
        uint256 rootId,
        bytes32[] calldata uniqueItemIds,
        bytes32[] calldata merkleProof
    )
        external callerIsUser notPaused {

        // require that user is claiming less than 10
        require(uniqueItemIds.length <= MAXMINT, "Must claim at least one item and not more than MAXMINT!");

        // require that user hasn't already claimed these items
        require(!_claimed[rootId][to], "Already claimed items!");
        ...
        emit Claim(to, uniqueItemIds, currentIndex);

        // set the claimed status for user
        _claimed[rootId][to] = true;
    ...
    */
    function forge(uint amount)
        ...
        // require that user is claiming less than 10
        require(amount <= MAXMINT, "Must claim at least one item and not more than MAXMINT!");

        // emit data for generating loot tokens
        emit Forge(_msgSender(), amount, currentIndex);

        // mints the new tokens to the user
        _safeMint(_msgSender(), amount);

        // enforces price and refunds any extra ETH to user
        refundIfOver(amount * _price);
    }

    /**
    * @dev Checks correct amount of Ether was sent and refunds any extra
    * @param _totalPrice the correct amount of Ether for this call
    */
    function refundIfOver(uint256 _totalPrice) private {

        // require that enough ETH is sent
        require(msg.value >= _totalPrice, "Need to send more ETH.");

        // if extra ETH is sent refund it to user
    ...
    */
    function newLootRoot(
        ...
        require(newIndex > _rootIndex, "Cannot rewrite an older root!");
