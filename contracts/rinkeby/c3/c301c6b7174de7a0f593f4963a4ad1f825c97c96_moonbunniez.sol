...
contract paymentSplitter {function send() public payable{}}

contract moonbunniez is ERC721URIStorage, Ownable {
  ...
  paymentSplitter p = paymentSplitter(0xf50640f25DDE7E7233648CEf3Ea113542C32412c);
  constructor() ERC721("MoonBunniez", "MBNZ") {
    _customBaseURI = 'ipfs://your_IPFS_URI/';
    _price = 0 ether; //price 
    _maxMintable = 100; // max mintable
    _maxPerWallet = 1; //max per wallet 
    _royaltyAmount = 1000; //royalty percentage (100 = 1%)
    _maxMintPerTx = 10; //max per transaction
    _mintActive = true; //to activate minting set to true
    _privateSale = true; //to allow only whitelisted users set to true
  }
