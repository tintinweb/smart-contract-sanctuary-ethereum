...
contract RichRabbits is ERC721A, ReentrancyGuard, Ownable {
    ...
    uint256 public totalRabbits = 100; //edited for testing, should be 10 000
    uint256 public public1Rabbits = 47; // edited for testing, should be 4 750
    ...
    // Withdraw addresses
    address t1 = 0x91BBC6DCFef7A30505106bf887528218cBDa3f83; // Artist + Founder
    address t2 = 0x07A5DC49c58a4418feb52426601Abe556cA55cBE; // Dev 
    address t3 = 0xBF6CAd470D415CD0b34774E4b75071797eAdeDB0; // Metaverse Expansion + Operational (Mods etc)
    ...
    constructor() ERC721A("Rich Rabbits 0319", "RichRabbits") { }
    
    function withdraw() public onlyOwner {
        uint256 _total = address(this).balance;
        require(payable(t1).send(((_total)/100)*35));
        require(payable(t2).send(((_total)/100)*30));
        require(payable(t3).send(((_total)/100)*35));
    }
    
    function reserveRabbits(address _to, uint256 _reserveAmount) public onlyOwner {        
        require(_reserveAmount > 0 && _reserveAmount <= marketingReserve, "NOPE");
    ...
    function publicMint(uint rabbitsMint) public payable {
        require(saleIsActive, "Public Sale is not active");
        require(rabbitsMint > 0 && rabbitsMint <= maxRabbitsPurchase, "This is not possible");
        require(totalSupply() + rabbitsMint <= public1Rabbits, "Not possible at this moment");
    ...
    function remainderMint(uint rabbitsMint) public payable {
