...
// File contracts/manolopunksdna.sol


pragma solidity ^0.8.0;

contract manolopunksdna {
    ...
    function deterministicPseudoRandon(uint256 _tokenId, address _minter)
    public
    pure
    returns(uint256)
    {
...
// File contracts/manolopunks.sol
...
//se agregaron
contract manolopunks is ERC721, ERC721Enumerable, manolopunksdna {
    ...
    constructor(uint256 _maxSupply) ERC721("manolopunks", "mp"){
    ...
    function mint() public{
        ...
        require(current < maxSupply, "no manolo punks left :( ");
        
        tokenDNA[current] = deterministicPseudoRandon(current, msg.sender);
    ...
    function tokenURI(uint256 tokenId) 
        ...
        returns (string memory)
        {
            require( 
            _exists(tokenId),
            "ERC721 metadata, uri query for noneexistent token");
            ...
            string memory jsonURI = Base64.encode(
                abi.encodePacked(
                    '{ "name": "manolopunks #',
                    tokenId.toString(),
                    '", "description": "manolo punks are preatty thinks", "imagen": "',
                    image, 
                    '"}'
