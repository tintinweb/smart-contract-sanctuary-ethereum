...
// This contract handles "serving up" rewards for xEdan, xOmega, tAlpha holders by trading tokens collected from fees into the corresponding form.
...
contract TreasureFinder is BoringOwnable {
    ...
    function buryTokens(uint amountALPHA) internal {
        ...
        uint ethForEdan = ethToSwap / 2;
        uint ethForOmega = ethToSwap.sub(ethForEdan);

        uint amountEDANtoBury = _swap(weth, edan, ethForEdan, buryEdan);
