...
contract DreamCarClub is ERC721AQueryable, Ownable, ReentrancyGuard {
    ...
    constructor(address _singenr, address _tokenAddress)
    ...
    function mint(
        ...
        require(
            IERC20(tokenAddress).balanceOf(wallet) >= price(_tokenAmount),
            "DCC: Value below price"
        );
    ...
    function crossmint(address _to, uint256 _tokenAmount) public saleIsOpen {
        uint256 total = totalSupply();
        require(_tokenAmount <= LIMIT_PER_MINT, "DCC: Max limit per mint");
        require(total + _tokenAmount <= MAX_ELEMENTS, "DCC: Max limit");
