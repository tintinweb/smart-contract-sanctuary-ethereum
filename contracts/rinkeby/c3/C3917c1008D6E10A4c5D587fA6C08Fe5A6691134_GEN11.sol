...
contract GEN11 is ERC721NOIDS, Ownable {
    string public baseURI;

    mapping(address => bool) private _allowedMinters;
    address private _signerAddress;

    constructor() ERC721NOIDS("Test Gen 1.1", "GEN11") {
    }
    ...
    function claim(uint256 amount, uint256 amountMax, uint256 deadlineTimestamp, bytes32 signatureR, bytes32 signatureVS) external {
        ...
        _mint(msg.sender, uint48(amount), uint48(amountMax), uint64(deadlineTimestamp));
    }
    ...
    function mint(address to, uint256 amount, uint256 amountMax, uint256 deadlineTimestamp) external {
        ...
        _mint(to, uint48(amount), uint48(amountMax), uint64(deadlineTimestamp));
    }

    function mintStatusOf(address account) public view returns (OwnerData memory) {
...
contract ERC721NOIDS is Context, ERC165, IERC721, IERC721Metadata {
    ...
    uint256 internal constant TOKEN_ID_LAST = TOKEN_ID_FIRST + MAX_SUPPLY - 1;
    ...
    function _exists(uint256 tokenId) internal view virtual returns (bool) {
        return tokenId >= TOKEN_ID_FIRST && tokenId <= TOKEN_ID_LAST;
    }
     ...
     */
    function _mint(address to, uint48 quantity, uint48 stageMax, uint64 stageTimestamp) internal virtual {
        unchecked {
            uint48 minted = quantity;
            ...
            _ownerData[to].balance += quantity;
            _ownerData[to].totalMinted += quantity;
            _ownerData[to].stageMinted = minted;
            _ownerData[to].stageMax = stageMax;
            _ownerData[to].stageTimestamp = stageTimestamp;
            ...
            if (quantity > 1) {
                ...
                uint s = quantity << 1;
                batchCount <<= 1;
                for (uint i=2; i<batchCount; i+=2) {
                    n += s;
                    _owners[(n / batchCount) + tokenIdStart] = to;
                }
            }
