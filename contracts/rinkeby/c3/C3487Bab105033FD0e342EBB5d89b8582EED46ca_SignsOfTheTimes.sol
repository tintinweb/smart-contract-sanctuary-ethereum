...
contract SignsOfTheTimes is AdminControl, ICreatorExtensionTokenURI {
    ...
    address _signsOfTimesCore;
    bytes32 public _merkleRoot;

    mapping(uint256 => TokenType) public tokenToTokenType; // 0 = Beginning; 1 = End; 2 = Composite
    mapping(address => uint16) public addressToTokenCount;
    ...
    function setSignsOfTimesCore(address signsOfTimesCore)
        public
        adminRequired
    {
        _signsOfTimesCore = signsOfTimesCore;
    }
    ...
    function incrementCounts(bool beginningOrEnd) private {
        ...
        isOpen
    {
        ...
        incrementCounts(beginningOrEnd);
        totalSupply += 1;
    }
    ...
    function mintComposite(uint256[] calldata tokenIds) public isPaused {
        ...
        require(
            IERC721(_signsOfTimesCore).ownerOf(tokenIds[0]) == msg.sender &&
                IERC721(_signsOfTimesCore).ownerOf(tokenIds[1]) == msg.sender,
            "Tokens must be owned by message sender"
        );

        require(
            tokenToTokenType[tokenIds[0]] != TokenType.Composite &&
                tokenToTokenType[tokenIds[1]] != TokenType.Composite,
            "Tokens must be a Beginning or End token"
        );

        try IERC721(_signsOfTimesCore).getApproved(tokenIds[0]) returns (
        ...
        try
            IERC721(_signsOfTimesCore).transferFrom(
                msg.sender,
                address(0xdEaD),
                tokenIds[0]
            )
        {} catch (bytes memory) {
            revert("Could not burn 1st token");
        }
        ...
        {} catch (bytes memory) {
            revert("Could not burn 2nd token");
        }
