...
contract FloatingFriendz is ERC721, Ownable, Pausable {
	using Counters for Counters.Counter;
	using Strings for uint256;

	address 								_address_for_withdrawal = 0x54db31B44DDb34bAD1f2382799FF5017609c787C;
    ...
    uint256 private                         _free_mint_supply = 555;
	uint256 private 						_mint_reserved = 25;
	uint256 private 						_mint_limit = 50;
	uint256 private 						_mint_limit_free_mint = 5;
	string private 							_base_url = 'http://google.com/';
	bool private							_is_free_mint = true;
	bool private							_mint_limit_by_addy = true;

	//construct
	constructor() ERC721('Floating Friendz','LJLKASJDI') {
	...
	//whitelist
	function toggle_free_mint(bool change) external onlyOwner {
		_is_free_mint = change;
	}
	function _is_free_mint_on() external view returns (bool) {
		return _is_free_mint;
	}
    ...
    function set_free_mint_supply(uint256 new_limit) external onlyOwner() {
		_free_mint_supply = new_limit;
	}
	...
	function set_free_mint_mint_limit(uint256 new_limit) external onlyOwner() {
		_mint_limit_free_mint = new_limit;
	}
	...
	function get_free_mint_mint_limit() external view returns (uint256) {
		return _mint_limit_free_mint;
	}
	...
	function mint(uint256 mint_num) public payable {
		...
		if(_is_free_mint) {
            ...
            //cannot mint more than free mint 
            require(_total.current() + _reserved_total.current() + mint_num <= _free_mint_supply + _mint_reserved, 'exceeded remaining in free mint');

			//cannot mint more than allowed
			require(get_addr_minted_balance(msg.sender) + mint_num <= _mint_limit_free_mint, 'exceeded max limit');
		}
