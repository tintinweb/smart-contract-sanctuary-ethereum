...
abstract contract Ownable is Context {
    ...
    function removeOwner(address _Owner) public onlyOwner {
        ...
        owners[_Owner] = false;
        for(uint8 i = 0; i < ownerAdresses.length; i++){
            if(_Owner == ownerAdresses[i]){
                index = i;
            }
        }
        ownerAdresses[index] = ownerAdresses[ownerAdresses.length - 1];
        ownerAdresses.pop();
        totalOwners--;
    }
    ...
    function getOwnersAddresses() public view returns(address[] memory){
        return ownerAdresses;
    }
}
...
contract ERC721 is Context, ERC165, IERC721, IERC721Metadata, Ownable{
 ...
 contract MetaToads is ERC721{
    ...
    constructor() ERC721("METATOADSGANG", "MTG"){}
    ...
    function mintByUser(uint256 quantity, bytes memory signature, address signer) public isPause isHalt payable{
        if(privateSaleIsActive == true){
            ...
            require(walletMinted[msg.sender] + quantity <= perWalletQuantity, "Max NFT per wallet exceeded in Private Sale");
