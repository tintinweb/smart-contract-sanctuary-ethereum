...
contract RewardSharks is Ownable, ReentrancyGuard {
    ...
    mapping(address => uint256[]) ownerTokenList;
    ...
    function returnData(uint256 _tokenID) external view returns (Sharks memory, string memory) {
        Sharks memory _shark = getSharks[_tokenID];
        string memory _boost = '';
        
        for(uint256 i=0;i<_shark.activeBoost.length;i++){
            _boost = concat(_boost, Strings.toString(_shark.activeBoost[i]));
            _boost = concat(_boost, ' ');
        }
        return (_shark, _boost);
    }

    function returnAllData(address _address) external view returns (Sharks[] memory) {
