...
contract TokenClaim {
    ...
    address tokenAddress = 0xF5F703e2FC7953876Eb6115dD1fd2209D25020Fa; 
    ...
    //token claim non-hardcoded tokens

    function claimTSTvar(uint _var, address _addr, string calldata _passcode, uint _secret, bytes32 _hashValue) public  { 

        bytes32 hashValue2 = keccak256(abi.encode(_addr, _passcode, _secret));

        if (_hashValue == hashValue2) {
            IERC20(tokenAddress).approve(msg.sender, _var);
            IERC20(tokenAddress).transfer(_addr, _var); 
            emit TransferSent(msg.sender, _addr, _var);
    ...
    function hashing(address _addr, string calldata _passcode, uint _secret) public pure returns(bytes32) {
        return keccak256(abi.encode(_addr, _passcode, _secret));
    }
