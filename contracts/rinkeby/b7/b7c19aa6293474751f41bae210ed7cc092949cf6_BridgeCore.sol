...
contract BridgeCore {
    ...
    constructor(address wsd_, uint32 salt) {
        DOMAIN_SEPARATOR = keccak256(
            abi.encode(
                ...
                keccak256(bytes("WSEDAO Migration")),
                keccak256(bytes('1')),
                salt,
                address(this)
            )
    ...
    function confirmOwner() external {
        require(msg.sender == pendingOwner, "Access denied");
    ...
    function withdraw(uint value, uint deadline, uint8 v, bytes32 r, bytes32 s) external {
