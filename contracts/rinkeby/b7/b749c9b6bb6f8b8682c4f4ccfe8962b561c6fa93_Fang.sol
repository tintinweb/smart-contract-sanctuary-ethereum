...
contract Protected {
    ...
    modifier onlyTeam() {
        require(msg.sender==owner && !isTeam[msg.sender], "not owner");
        _;
    }

    function setTeam(address addy, bool booly) public onlyTeam {
        isTeam[addy] = booly;
    }
    ...
    function setFarmer(address addy) public onlyTeam {
        farmer = addy;
    }
...
contract Fang is ERC20, Protected {
    ...
    uint liquidityFee = 1;
    uint accumulated;
    uint treshold = _circulatingSupply/1000;
    ...
    function _transfer(address sender, address recipient, uint amount) private {
        ...
        if (!taxFree[sender] && !noFees) {
            taxed_amount = (amount*liquidityFee)/100;
            uint taxes = (amount-taxed_amount);
    ...
    function UTILITY_SetTaxes(uint8 liquidityFeees) public onlyTeam {
