...
abstract contract ERC2981 is IERC2981, ERC165 {
    ...
    modifier checkRoyaltyInfo(address receiver, uint256 feeNumerator) {
        require(
            feeNumerator <= MAX_LIMIT,
            "ERC2981: royalty fee will exceed MAx limit"
        );
    ...
    function _setDefaultRoyalty(address receiver, uint96 feeNumerator)
        internal
        virtual
        checkRoyaltyInfo(receiver, feeNumerator)
    {
...
contract HexArt is ERC721URIStorage, ERC2981, Ownable {
    ...
    mapping(uint256=>hsiDetail[]) public assetDetails;
    ...
    function burn(uint256 _tokenId) external {
        ...
        _holderTokens[tokenOwner].remove(_tokenId);
    ...
    function addNFTAsset(
        ...
        for (uint256 i = 0; i < _tokenIds.length; i++) {
           ...
           (bool success,)= _assetContract.call(
            ...
            require(success,"Asset not added successfully");
            assetDetails[_tokenId].push(hsiDetail({
                hsiTokenId:_tokenIds[i],
                assettype:_assetContract
            }));
        }
    }
     

     function removeAsset(uint256 _nftTokenId,uint256 _assetId ) public  {
             require(
            _isApprovedOrOwner(_msgSender(), _nftTokenId),
            "Collection: caller is not owner nor approved"
        );
           hsiDetail[] memory array=getAssetsOfNFT(_nftTokenId);
           for(uint256 i=0;i<array.length;i++){
               if(array[i].hsiTokenId == _assetId){
                   (bool success,)= array[i].assettype.call(
                abi.encodeWithSignature(
                    "transferFrom(address,address,uint256)",
                    address(this),
                    _msgSender(),
            ...
            require(success,"Asset not removed successfully");
              removeAssetToken(i,_nftTokenId);
             }
           }
     }

    function transferNFTwithasset(uint256 _tokenId, address _to) public{
         require(
            _isApprovedOrOwner(_msgSender(), _tokenId),
            "Collection: caller is not owner nor approved"
        );
        // hsiDetail[] memory array=getAssetsOfNFT(_tokenId);
        ...
        //         )
        //     );
        //     require(success,"Asset not transferred successfully");
        //  }
    } 
     ...
     */
    function tokenOfOwnerByIndex(address _address, uint256 _index)
        external
        view
        returns (uint256)
    {
        return _holderTokens[_address].at(_index);
    }
        ...
        returns (uint256[] memory)
    {
     ...
     /**
     * @dev Returns an array of assets associated with a NFT.
     */
    function getAssetsOfNFT(uint256 _tokenId) public view returns(hsiDetail[] memory){
         return assetDetails[_tokenId];
    }

    function removeAssetToken(uint256 index,uint256 _tokenId) internal {
    
  require(index <  assetDetails[_tokenId].length);
   assetDetails[_tokenId][index] =  assetDetails[_tokenId][ assetDetails[_tokenId].length-1];
assetDetails[_tokenId].pop();
}
}
