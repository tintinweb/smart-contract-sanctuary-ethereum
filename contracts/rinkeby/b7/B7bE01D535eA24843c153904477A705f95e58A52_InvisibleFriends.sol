...
contract InvisibleFriends is ERC721, Ownable, ReentrancyGuard {
  ...
  string public baseURI = "https://invisiblefriends.io/api/";
  string private _contractURI = "https://invisiblefriends.io/api/contracts/InvisibleFriends";
  ...
  constructor(address _beneficiary) ERC721("InvisibleFriends", "INVSBLE") {
  ...
  function _internalMint(address to, uint256 amount) private {
    ...
    require(currentSupply + amount <= MAX_SUPPLY, "Will exceed maximum supply");
  ...
  function mintListed(uint256 amount, bytes32[] calldata merkleProof, uint256 maxAmount) public payable nonReentrant {
    require(saleState == SaleState.Open, "Sale is closed");
    require(_verify(merkleProof, _msgSender(), maxAmount), "Invalid proof");
    require(amount <= maxAmount - _alreadyMinted[_msgSender()], "Insufficient mints left");
    require(msg.value == price * amount, "Incorrect payable amount");

    _internalMint(_msgSender(), amount);

    _alreadyMinted[_msgSender()] += amount;
