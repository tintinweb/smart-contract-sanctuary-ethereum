...
import "./gmiHoodieSale.sol";

contract GMIHoodieReferral is AccessControl {
  ...
  uint256 public referralCut; // Assumes 2 decimals. e.g. 10000 = 100%
  ...
  constructor(
    address payable migratedShopAddress,
    address erc721Address,
    uint256 _referralCut
  ) {
    isSaleActive = false;
    referralCut = _referralCut;
    signVerifier = 0xF504941EF7FF8f24DC0063779EEb3fB12bAc8ab7;

    GMIHoodieSale shopToMigrate = GMIHoodieSale(migratedShopAddress);

    price = shopToMigrate.price();

    maxAmount[Size.XS] = shopToMigrate.maxAmount(GMIHoodieSale.Size.XS);
    maxAmount[Size.S] = shopToMigrate.maxAmount(GMIHoodieSale.Size.S);
    maxAmount[Size.M] = shopToMigrate.maxAmount(GMIHoodieSale.Size.M);
    maxAmount[Size.L] = shopToMigrate.maxAmount(GMIHoodieSale.Size.L);
    maxAmount[Size.XL] = shopToMigrate.maxAmount(GMIHoodieSale.Size.XL);
    maxAmount[Size.XXL] = shopToMigrate.maxAmount(GMIHoodieSale.Size.XXL);

    soldAmount[Size.XS] = shopToMigrate.soldAmount(GMIHoodieSale.Size.XS);
    soldAmount[Size.S] = shopToMigrate.soldAmount(GMIHoodieSale.Size.S);
    soldAmount[Size.M] = shopToMigrate.soldAmount(GMIHoodieSale.Size.M);
    soldAmount[Size.L] = shopToMigrate.soldAmount(GMIHoodieSale.Size.L);
    soldAmount[Size.XL] = shopToMigrate.soldAmount(GMIHoodieSale.Size.XL);
    soldAmount[Size.XXL] = shopToMigrate.soldAmount(GMIHoodieSale.Size.XXL);

    remainingFreeClaims = shopToMigrate.remainingFreeClaims();
  ...
  // Public functions
  function getReferralSigningHash(
    address referrer,
    uint256 blockExpiry,
    address recipient,
    Size size,
    ...
    return
      keccak256(
        abi.encodePacked(referrer, blockExpiry, recipient, size, quantity, address(this), recipientNonces[recipient])
      );
  }
  ...
  function setSignVerifier(address verifier) external virtual onlyRole(IYK_MANAGER_ROLE) {
  ...
  function buyWithReferral(
    ...
    require(erc721.balanceOf(referrer) > 0, "Referrer must own nfts to offer a referral");
    bytes32 message = getReferralSigningHash(referrer, deadline, msg.sender, size, quantity).toEthSignedMessageHash();
    require(ECDSA.recover(message, signature) == signVerifier, "Permission to call this function failed");
    require(block.number < deadline, "Referral signature expired");

    // Transfer funds
    payable(referrer).transfer((price * quantity * referralCut) / 10000);

    soldAmount[size] += quantity;
    recipientNonces[msg.sender]++;

    emit Purchase(msg.sender, size, quantity);
  }

  // Admin functions
  function mintBatch(uint256[] memory quantities) public onlyRole(MINTER_ROLE) {
  ...
  function setReferralCut(uint256 _referralCut) external onlyRole(IYK_MANAGER_ROLE) {
    require(_referralCut <= 10000, "Assumes 2 decimal precision for referral cut, maxing at 10000 for 100%");
    referralCut = _referralCut;
  }
  ...
  function getRecipientNonce(address refererr) public view returns (uint256) {
    return recipientNonces[refererr];
  }
