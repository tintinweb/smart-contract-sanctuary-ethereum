...
*/                                                                   
  ...
  modifier mintCompliance(address _sender) {
    require(_currentIndex - pages[currentPage].startIndex + 1 <= 491, "Comic pages are sold out!");
  ...
  function mintForAddressesAdmin( address[] calldata _receivers, uint256[] calldata _mintAmounts) public onlyOwner {
    for (uint i=0; i<_receivers.length; i++) {
       require(_currentIndex - pages[currentPage].startIndex + _mintAmounts[i] <= 491, "Amount minted would exceed max supply!");
  ...
  function pageShowAllTokens(uint _pageId) public view returns (uint256[] memory){
    ...
    if(_pageId == currentPage){
      ...
      numberOfTokensOnPage = pages[_pageId].endIndex - pages[_pageId].startIndex;
