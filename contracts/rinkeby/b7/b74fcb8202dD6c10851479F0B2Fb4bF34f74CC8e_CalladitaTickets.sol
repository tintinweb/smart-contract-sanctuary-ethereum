...
contract CalladitaTickets is ERC1155Supply, Mintable {
    ...
    constructor() ERC1155("ipfs://QmWNDwP7YaJVR7biNcRARAiHCtisk5MyhFWoJwUDumKLYa/") {
    ...
    */
     ...
     */
    function airdrop(address[] calldata addresses, uint256 token, uint numberOfTokens) external onlyOwner {
        if (token==0){
    ...
    function mintWithCaladita(uint[] calldata tokenIds) external mintConditions(tokenIds.length){
        ...
        _mint(msg.sender, PORNCORN_TOKEN_ID, 10, "");
    }
    ...
    function _mintEligiblePOPCORN(address mintTo, uint multiplier) internal {
        ...
        if (totalSupply(MOVIE_TOKEN_ID) < 1000) {
            _mint(mintTo, PORNCORN_TOKEN_ID, 5*multiplier, "");
        }
        else if (totalSupply(MOVIE_TOKEN_ID)  < 3000) {
            _mint(mintTo, PORNCORN_TOKEN_ID, 3*multiplier, "");
        }
        else if (totalSupply(MOVIE_TOKEN_ID)  < 6000) {
