...
contract TestEthToken is ERC20, Ownable, SafeToken, LockToken {
    ...
    uint256 public maxSellTransactionAmount = 1_000_000_000 * (10**18); 
    ...
    event FeeUpdated(uint256 _totalFee, uint256 _timestamp);
    function updateAllFee(uint256 _bnbRewardFee, uint256 _liquidityFee, uint256 _marketingFee) 
    public onlyOwner 
    {
    ...
    function setExtraFeeOnSell(uint256 _extraFeeOnSell) 
    public onlyOwner 
    {
        extraFeeOnSell = _extraFeeOnSell; // extra fee on sell
        require(extraFeeOnSell<=10, "Too High Fee");
    }

    function setMaxtx(uint256 _maxSellTxAmount) 
    public onlyOwner 
    {
        maxSellTransactionAmount = _maxSellTxAmount;
        require(maxSellTransactionAmount>_totalSupply.div(1000), "Too less limit");
    }

    constructor() ERC20("Crafty Metaverse", "CMETA") 
    {
        BNBRewardsFee = 1;
        liquidityFee = 3;
        marketingFee = 6;
        extraFeeOnSell = 10; // extra fee on sell

        marketingWallet = payable(0x6F8A67F746e740a64E7d29e53BF51EBC394e11EB);
        totalFees = BNBRewardsFee.add(liquidityFee).add(marketingFee); // total fee transfer and buy
    ...
    event BridgeSet(address owner, address _bridge);
    ...
    function mintTokens(uint256 totalSupply) external onlyOwner 
    {
