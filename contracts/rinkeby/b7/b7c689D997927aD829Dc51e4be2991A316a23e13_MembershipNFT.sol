...
contract MembershipNFT is ERC721A, Ownable {

  event Mint(address indexed operator, address indexed to, uint256 quantity, uint256 tierIndex);
  ...
  constructor(
    ...
    function mint(address to, uint256 quantity, uint256 tierIndex) external onlyOwner {
        // _safeMint's second argument now takes in a quantity, not a tokenId.
        require(defaultTiers.length > tierIndex,"MembershipNFT: The tier index is overflow");
        require(defaultTiers[tierIndex].count == quantity,"MembershipNFT: The tier qunatity doesn't match");
        uint256 startId = _currentIndex;
        _safeMint(to, quantity);
        defaultTiers[tierIndex].startId = startId;
        defaultTiers[tierIndex].endId = _currentIndex - 1;
    ...
    function setImage(string[] memory urls) external onlyOwner {
        ...
        require(urls.length == defaultTiers.length, "MembershipNFT: The length is not equal");
    ...
    function setGameProp(uint256 tokenId, uint256 gameplay, uint256 tokentransaction) external onlyOwner {
