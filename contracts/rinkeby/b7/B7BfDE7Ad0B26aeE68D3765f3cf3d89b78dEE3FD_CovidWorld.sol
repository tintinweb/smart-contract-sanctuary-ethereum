...
//     _____ _        _
//    / ____| |      | |           /\
//   | (___ | |_ __ _| |_ _   _   /  \   __ _  ___ _ __   ___ _   _
//    \___ \| __/ _` | __| | | | / /\ \ / _` |/ _ \ '_ \ / __| | | |
//    ____) | || (_| | |_| |_| |/ ____ \ (_| |  __/ | | | (__| |_| |
//   |_____/ \__\__,_|\__|\__,_/_/    \_\__, |\___|_| |_|\___|\__, |
//   ** https://statu.co **              __/ |                 __/ |
//                                      |___/                 |___/

// @title World of Covid 19
// @author twitter.com/cinaxriver
// @author twitter.com/altiokberk
...
contract CovidWorld is ERC721r, Ownable, ReentrancyGuard {
    ...
    mapping(address => uint256) public _preClaims;

    uint256 _price = 0.06 ether;

    Counters.Counter private _tokenIds;

    constructor(string memory uri, bytes32 merkleroot, address _proxyRegistryAddress)
    ERC721r("World Of Covid19", "WOC19", 10000)
    ReentrancyGuard() {
        root = merkleroot;
    ...
    function mintByOwner(uint256 _amount) external onlyOwner {
        require(_amount > 0, "WOC19: zero amount");
        uint current = _tokenIds.current();
        require(
            current + _amount <= ERC721r.maxSupply(),
            "WOC19: max supply exceeded"
        );
    ...
    function presaleMint(address account, uint256 _amount, bytes32[] calldata _proof) external payable isValidMerkleProof(_proof) onlyAccounts {
        require(msg.sender == account, "WOC19: Not allowed");
        require(preMint, "WOC19: Presale is OFF");
        require(!paused, "WOC19: Contract is paused");
        //negative?
        require(_amount > 0, "WOC19: zero amount");
        require(
            _amount <= presaleMintLimit, "WOC19: You can't mint so much tokens");
        require(
            _preClaims[msg.sender] + _amount <= presaleMintLimit, "WOC19: You can't mint so much tokens");
        ...
        require(
            ...
            "WOC19: Not enough ethers sent"
        );

        _preClaims[msg.sender] += _amount;

        mintInternal(_amount);
    }

    function communitySaleMint(uint256 _amount) external payable onlyAccounts {
        require(communityMint, "WOC19: CommunitySale is OFF");
        require(!paused, "WOC19: Contract is paused");
        require(_amount > 0, "WOC19: zero amount");
    ...
    function mintInternal(uint amount) internal nonReentrant {
        _tokenIds.increment();
    ...
    function tokensOfOwner(address _owner) external view returns (uint256[] memory) {
        uint256 tokenCount = ERC721r.balanceOf(_owner);
        if (tokenCount == 0) {
            return new uint256[](0);
        } else {
            uint256[] memory result = new uint256[](tokenCount);
            uint256 totalSupplied = ERC721r.maxSupply();
            uint256 inx = 0;
            uint256 id;
            for (id = 1; id <= totalSupplied; id++) {
                if(!ERC721r._exists(id)) {
                   continue;
                }
                if (ERC721r.ownerOf(id) == _owner) {
                    result[inx] = id;
                    inx++;
                }
            }
