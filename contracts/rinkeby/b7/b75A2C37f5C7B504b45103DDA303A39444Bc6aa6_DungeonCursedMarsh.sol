...
contract DungeonCursedMarsh is DungeonBase {
  ...
  string internal constant _DUNGEON_NAME = "Cursed Marsh";
  string internal constant _URI = "https://raw.githubusercontent.com/tetu-io/tetu-game-assets/master/dungeon/1/cursed_marsh/logo.png";
  ...
  function DUNGEON_NAME() external pure override returns (string memory) {
...
abstract contract DungeonBase is Controllable, IDungeon, ERC721Holder {
  ...
  /// @dev Make a bit harder to understand what will be in the next chamber
  function previousChamber(uint index) external view returns (IChamber) {
    require(index <= _CURRENT_CHAMBER_SLOT.getUint(), "Too high index");
    return _chambers[index];
  }
  ...
  function openChamber(bytes calldata data) external override returns (IChamber.ChamberResult memory){
    ...
    emit ChamberCompleted(result, _currentChamber, heroToken, heroTokenId);

    // KILL
    if (result.kill || stats.life <= result.damage) {
      IHero(heroToken).changeCurrentDungeon(heroTokenId, address(0));

      IStatController.NftItem[] memory drop = IHero(heroToken).kill(heroTokenId);
