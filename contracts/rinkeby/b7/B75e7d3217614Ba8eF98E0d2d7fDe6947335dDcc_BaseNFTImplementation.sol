...
contract BaseNFTImplementation is ERC721, BaseNFT  {
    ...
    constructor(ICRS _crs, bytes32 _baseNode) ERC721("Comunity public record","CRS") {
        crs = _crs;
        baseNode = _baseNode;
    }

    modifier live {
        require(crs.owner(baseNode) == address(this));
        _;
    }
    ...
    function setResolver(address resolver) external override onlyOwner {
        crs.setResolver(baseNode, resolver);
    }
    ...
    function _register(uint256 id, address owner, uint duration, bool updateRegistry) internal live onlyController returns(uint) {
        ...
        _mint(owner, id);
        if(updateRegistry) {
            crs.setSubnodeOwner(baseNode, bytes32(id), owner);
        }
...
abstract contract BaseNFT is Ownable, IERC721 {
