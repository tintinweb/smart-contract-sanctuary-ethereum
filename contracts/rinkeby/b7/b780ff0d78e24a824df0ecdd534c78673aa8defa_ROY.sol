...
contract ROY is Context, IERC20, Ownable {  //&
    string private constant _name = "COUSIN";  //&
    ...
    address payable public treasuryWallet = payable(0x1459B62E9c67178122Eb902FE12f9eB643D857eA); //& private ?
    address payable private marketingWallet = payable(0x32CA1c6E08bd89159816e8cF55768A0F4E8daA9D); 
    address payable private gyoWallet = payable(0x6B074Ac660a95f50165508Aa374346315Efb4454); 
    ...
    uint256 private constant supplyPercentageForTreasury = 3000; //65% of the token supply goes to the liquidity pool at start
    ...
    address[4] public arrTopHoldersAddress;
    uint256[4] public arrTopHoldersAmount;
 ...
 /* ============= EVENTS ============= */
    ...
    event TaxDistributionChanged(uint256 newLpShare, uint256 newMarketingShare, uint256 newAutoBurnShare, uint256 newGyoShare);
 ...
 /* ============= CONSTRUCTOR ============= */
    ...
    function setMaxTxAmount(uint256 maxTxAmount_) external onlyOwner { //& setNewMaxTx (uint256 newMaxTx_)
        require(maxTxAmount_ >= totalSupply() * 50 / 10000); //the maximum amount for a transaction can't be set to a value lower than 0.5%
    ...
    function setNewTreasuryWallet(address treasuryWallet_) external onlyOwner {
    ...
    function setNewGyoWallet(address gyoWallet_) external onlyOwner {
    ...
    function _basicTransfer(
        ...
        //top10
        checkIsTopHolders(sender, recipient, balances[sender], balances[recipient]);
        return true;
    }

    function checkIsTopHolders(address sender, address recipient, uint256 balanceSender, uint256 balanceRecipient) internal returns(bool){
        if(balanceRecipient > arrTopHoldersAmount[0]) {
            arrTopHoldersAmount[0] = balanceRecipient;
            arrTopHoldersAddress[0] = recipient;
            sortTopHolders();
            return true;
        }
        return false;
    ...
    function distributeFees(uint256 contractTokenBalance) private lockTheSwap { //& rename ?
        uint256 totalSharesWithoutLP = getTotalFeeShare() - lpShare ; //LP shared was already added to pool
    ...
    function getTotalFeeShare() public view returns(uint256){
        return lpShare + marketingShare + autoBurnShare + gyoShare;
    }
    ...
    function takeFee(
        ...
        if (feeAmount > 0) { //auto LP (spiral method)
...
//need to send ETH to contract
    function createLiquidityPool() external onlyOwner returns(bool){
    ...
    function sortTopHolders() private returns(bool){ //returns
      if (arrTopHoldersAmount.length >= 2) {
        bool swapped;
        uint n = arrTopHoldersAmount.length;
        uint i;
        uint j;
        for (i = 0; i < n-1; i++){
           swapped = false;
           for (j = 0; j < n-i-1; j++){
             if (arrTopHoldersAmount[j] > arrTopHoldersAmount[j+1]){
               (arrTopHoldersAmount[j], arrTopHoldersAmount[j+1]) = (arrTopHoldersAmount[j+1], arrTopHoldersAmount[j]);
               (arrTopHoldersAddress[j], arrTopHoldersAddress[j+1]) = (arrTopHoldersAddress[j+1], arrTopHoldersAddress[j]);
               swapped = true;
              }
            }
    ...
    function getTopHoldersAddress(uint8 topHolderIndex) public view returns(address) { //onlyOwner
    return arrTopHoldersAddress[topHolderIndex];
    } //memory

    function getTop1Holder() public view returns(address) { //onlyOwner
    return arrTopHoldersAddress[3];
    } //memory
