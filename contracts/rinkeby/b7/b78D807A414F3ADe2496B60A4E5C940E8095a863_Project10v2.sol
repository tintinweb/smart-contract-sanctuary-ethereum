...
contract Project10v2 is ERC721AQueryable, Ownable, ReentrancyGuard {
  ...
  uint256 constant private ABSOLUTE_MAX_SUPPLY = 9966;
  uint256 constant private PRESALE_MAX_SUPPLY = 996;
  uint256 public maxTokenMint = 14;
  ...
  constructor(
    ...
    uint256 mintTime,
    bytes32 root
  ) ERC721A('Project10v2', 'P10V2') Ownable() {
    payee = payeeAddress;
  ...
  function maxSupply() public view returns (uint256) {
    if (isPublicMintActive()) {
      return ABSOLUTE_MAX_SUPPLY;
    } else {
      return PRESALE_MAX_SUPPLY;
    }
  }

  function _verifiedMint(bytes memory signature,  uint256 quantity) internal {
    require(
      _verifySignature(
    ...
    require(quantity < maxTokenMint + 1, 'You can only mint a maximum of 14 tokens at once');
    require(_tokenIds.current() + quantity <= maxSupply(), 'Supply unavailable to mint');
    require(quantity * mintPrice <= msg.value, 'You have not supplied enough eth');
    _mint(msg.sender, quantity);
  }

  function publicMint(bytes memory signature, uint256 quantity) external payable nonReentrant {
    require(isPublicMintActive(), 'Public mint is not active right now');

    _verifiedMint(signature, quantity);
  }

  function accessListMint(
