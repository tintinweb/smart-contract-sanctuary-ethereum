...
contract Staker {
    ...
    modifier deadlineReached(){
        ...
        require(timeRemaining == 0, "DeadLine is not reached already");
        _;
    }
  ...
  //  ( make sure to add a `Stake(address,uint256)` event and emit it for the frontend <List/> display )
    function stake() public payable deadlineRemaining notCompleted{
        require(msg.value > 0, "amount should't be 0");
  ...
  //  It should either call `exampleExternalContract.complete{value: address(this).balance}()` to send all the value
    ...
    require(threshold <= address(this).balance);

    (bool sent,) = address(exampleExternalContract).call{value: address(this).balance} (abi.encodeWithSignature("complete()")); 

    require(sent, "exampleExternalContract.compile failed");
  }

  // if the `threshold` was not met, allow everyone to call a `withdraw()` function
  // Add a `withdraw()` function to let users withdraw their balance

function withdraw(address payable withdrawer) public deadlineReached notCompleted {
