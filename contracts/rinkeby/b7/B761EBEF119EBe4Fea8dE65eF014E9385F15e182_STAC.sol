...
contract STAC is ISTAC, ERC721Enumerable, Pausable, Ownable, ReentrancyGuard, PaymentSplitter {
	...
	function whitelistMint(
		...
		for (uint256 i = 0; i < qty; i++) {
			seed = randomizer.random(1000, 1125899906842624, _seed + _tokenIds.current());
			...
			uint256 isStonedApeInt = seed % 100; //90% bacon
			uint256 alpha = 1;
			bool isFed = false;
	...
	function selectRecipient(uint256 seed) internal view returns (address) {
		seed = (seed * 1087) / _tokenIds.current();
	 ...
	 * @param _growOperationAddress the address of the Frying Pan
	 */
