...
contract AvatarTreasury is AccessControl, ReentrancyGuard {
    ...
    function purchaseMintingRight() external payable saleConditions {
        ...
        incrementSalesCount(msg.value);
    }

    /**
     * @dev purchase avatar minting by stable coins right by public users.
     */
    function purchaseMintingRightByUSDT(uint256 amount)
        external
        saleConditions
    {
        require(
        ...
        incrementSalesCount(amount);
    }

    /**
     * @dev purchase avatar minting by stable coins right by public users.
     */
    function purchaseMintingRightByUSDC(uint256 amount)
        external
        saleConditions
    {
        require(
        ...
        emit PaymentReceived(_soldItemIdCounter.current(), msg.sender, amount);
    }
    ...
    function adminWithdrawAll() external onlyRole(ADMIN_ROLE) nonReentrant {
        require(
            ethBalance() > 0 || usdcBalance() > 0 || usdtBalance() > 0,
            "No funds left"
        );
    ...
    function incrementSalesCount(uint256 amount) internal {
        _soldItemIdCounter.increment();
        emit PaymentReceived(_soldItemIdCounter.current(), msg.sender, amount);
    }
