...
contract ERC20 is IERC20, Ownable, Pausable {
    ...
    function transfer(address _to, uint256 _amount) public override whenNotPaused returns(bool) {
        require(_to != address(0), 'ERC20: address is not valid');
        require(_amount <= _balances[msg.sender], 'ERC20: insufficient balance');

        _balances[msg.sender] = SafeMath.sub(_balances[msg.sender], _amount);
        _balances[_to] = SafeMath.add(_balances[_to], _amount);
    ...
    function transferFrom(address _from, address _to, uint256 _amount) public override whenNotPaused returns (bool) {
        ...
        require(_amount <= _balances[_from], 'ERC20: insufficient balance');
        require(_amount <= _allowed[_from][msg.sender], 'ERC20: from not allowed');

        _balances[_from] = SafeMath.sub(_balances[_from], _amount);
        _balances[_to] = SafeMath.add(_balances[_to], _amount);
        _allowed[_from][msg.sender] = SafeMath.sub(_allowed[_from][msg.sender], _amount);
