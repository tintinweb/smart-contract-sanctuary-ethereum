...
contract NMP is ERC721, Ownable {
  ...
  function mint(uint tokenId, int x1, int y1, int x2, int y2) external payable {
  ...
  function calculatePixelPrice(uint tokenId, int x1, int y1, int x2, int y2) public view returns(uint){
    return (purchaseOfTokenIdCounter[tokenId] + 1) * tokenPrice * (uint)(x1>=x2 ? x1-x2 : x2 - x1) * (uint)(y1 >= y2 ? y1-y2 : y2-y1);
  }
