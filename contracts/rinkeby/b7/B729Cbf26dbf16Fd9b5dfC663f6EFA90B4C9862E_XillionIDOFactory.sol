    ...
    function getIDOState() external view returns (IDOState) {
        ...
        if(_state == IDOState.Unconfirmed){
            ...
            if(VotingState(votingState) == VotingState.Preparing){
                ...
                if(allocationStartDate > 0 && block.timestamp >= allocationStartDate && block.timestamp <= allocationEndDate) {
                    return IDOState.Allocation;
                }
                if (swapStartDate > 0 && block.timestamp >= swapStartDate && block.timestamp <= swapEndDate) {
                    return IDOState.Swap;
                }
                if(_state == IDOState.Refund){
                    return IDOState.Refund;
                }
                if(_state == IDOState.Claim){
                    return IDOState.Claim;
                }
