...
contract DropAuctionShop721 is Ownable, ReentrancyGuard {
  ...
  */
  constructor (
    ...
    // For starting purposes, lock in the last sale time as the starting time.
    lastSaleTime = startTime;
  }
  ...
  */
  function currentPrice () public view returns (uint256) {
    ...
    uint256 timeElapsed = (block.timestamp - lastSaleTime);
  ...
  /**
    A private helper function to sell an item to a public sale participant.
  ...
  */
  function sellPublic (
    ...
    if (msg.value < totalCharge) { revert CannotTransferIncorrectAmount(); }

    // refund any overpayment
    uint256 diff = msg.value - totalCharge;
    if(diff > 0 ){
        (bool returned, ) = payable(_msgSender()).call{
            value: diff
        }("");
        if (!returned) revert CannotRefundDifference();
    }
  }
  ...
  */
  function mint (
    ...
    */
    ...
    // Update the time of the last sale.
