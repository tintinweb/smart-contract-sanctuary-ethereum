...
contract host is ERC721Enumerable, Ownable {
  ...
  mapping(uint256 => RemoteNFT[]) public features;
  ...
  function register(
    ...
    // if this RemoteNFT feature already exists
    for (uint256 i = 0; i < features[_tokenId].length; i++) {
      if (features[_tokenId][i].remoteContractAddr == _remoteContractAddr) {
        // update the remoteTokenId
        features[_tokenId][i].remoteTokenId = _remoteTokenId;
        updated = true;
        break;
      }
    }

    // if this RemoteNFT feature did not already exist
    if (!updated) {
      // add it
      features[_tokenId].push(RemoteNFT(_remoteContractAddr, _remoteTokenId));
    }
  }

  function deregister(uint256 _tokenId, address _remoteContractAddr) public {
    if (msg.sender != ownerOf(_tokenId)) revert NotNFTOwner();

    for (uint256 i = 0; i < features[_tokenId].length; i++) {
      if (features[_tokenId][i].remoteContractAddr == _remoteContractAddr) {
        // remove via im-place swap
        features[_tokenId][i] = features[_tokenId][
          features[_tokenId].length - 1
        ];
        features[_tokenId].pop();
        break;
      }
    }
  }

  function getFeatureList(uint256 _tokenId) public view returns (RemoteNFT[] memory) {
    if (_exists(_tokenId) == false) {
      revert BadTokenID();
    }
    return features[_tokenId];
  }
