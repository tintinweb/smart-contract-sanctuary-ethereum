...
contract TowerPool is LPTokenWrapper, IRewardDistributionRecipient {
    ...
    modifier updateReward(address account) {
        ...
        if (account != address(0)) {
            ...
            userPoints[account]=points(account);
    ...
    function lastTimeRewardApplicable() public view returns (uint256) {
        uint256 r = block.timestamp < periodFinish ? block.timestamp:periodFinish;
        return r;
    }
    ...
    function points(address account) public view returns(uint256){
        return balanceOf(account).mul(lastTimeRewardApplicable().sub(userLastUpdateTime[account])).add(userPoints[account]);
    }
