    ...
    ReentrancyGuardUpgradeable
{
    ...
    mapping(address => uint) public ogListMintTracker1;
    mapping(address => uint) public ogListMintTracker2;
    mapping(address => uint) public wlListMintTracker;
    mapping(address => uint) public publicMintTracker;
    ...
    modifier checkSupply(uint256 _amount) {
        ...
        require(
            totalSupply() + _amount <= TotalSupply - ownerReserve,
    ...
    */
        ...
        initializer 
    {
        ...
        __ERC721A_init(_name, _symbol); // name: Wayfarers; symbol: PRL
        ...
        ogList1.startTime = 1660043183; // Tuesday, 9 Aug 2022 11:05:25 GMT
        ogList1.endTime = ogList1.startTime + 2 days;
        ogList1.purchaseLimit = 2;
        ogList1.listCap = 600;
        ogList1.mintPrice = 0.05 ether;

        ogList2.startTime = ogList1.startTime + 2 days;
        ogList2.endTime = ogList1.startTime + 4 days;
        ogList2.purchaseLimit = 2;
        ogList2.mintPrice = 0.05 ether;

        wlList.startTime = 1660825654; // Thursday, 18 Aug 2022 5:57:30 GMT
        wlList.endTime = wlList.startTime + 3 days;
        wlList.purchaseLimit = 1;
        wlList.mintPrice = 0.05 ether;
        wlList.listCap = 4400;

        publicMintStartTime = wlList.startTime + 3 days;
        publicMintEndTime = wlList.startTime + 6 days;
        publicMintLimitPerWallet = 2;
    ...
    function ownerMint(uint _amount) 
        ...
        onlyOwner
    {
        require (_amount + totalSupply() <= TotalSupply, "Supply Reached");
        require(_amount <= ownerReserve, "Owner Mint Limit Exceeded");
    ...
    */
    ...
    function OGListMint1(whitelist memory _whitelist, uint _amount) 
        external
        payable
        nonReentrant 
        checkSupply(_amount) 
    {
            ...
            require(_whitelist.listType == 1, "OGList1: Wrong list type");
            require(
                    block.timestamp > ogList1.startTime && block.timestamp <= ogList1.endTime,
                    "OG1 Sale Closed"
                );
            require(
                    _amount + ogListMintTracker1[_whitelist.userAddress] <= ogList1.purchaseLimit,
                    "OG1 Quota Depleted"
                );
            require(_amount <= ogList1.listCap, "OG-1 Sold Out");
            require(msg.value == _amount * ogList1.mintPrice, "OG List-1: Pay exact fees");
            ogListMintTracker1[_whitelist.userAddress] += _amount;
            ogList1.spotsSold += _amount;
    ...
    */
    ...
    function OGListMint2(whitelist memory _whitelist, uint _amount) 
        external
        payable
        nonReentrant 
        checkSupply(_amount) 
    {
        require(getSigner(_whitelist) == designatedSigner, "!Signer");
        require(_whitelist.userAddress == msg.sender, "!Sender");
        require(_whitelist.listType == 1, "OGList2: Wrong list type");
        require(
                block.timestamp > ogList2.startTime && block.timestamp <= ogList2.endTime, 
                "OG2 Sale Closed"
            );
        require(
                _amount + ogListMintTracker2[_whitelist.userAddress] <= ogList2.purchaseLimit,
                "OG2 Quota Depleted"
            );
        // probability of getting total sold out in this particular mint
        // require(_amount <= ogList2.listCap, "OG-2 Sold Out");
        require(msg.value == _amount * ogList2.mintPrice, "OG List-2 : Pay exact fees");

            ogList2.listCap -= _amount;
            _mint(_whitelist.userAddress, _amount);
    } 

    /**
    @notice This is function is used to mint tokens for OG List sale  
    @param _whitelist whitelisting object which contains user address and backend signature for verification  
    @param _amount amount of tokens to mint in one transaction  
    */
        ...
        checkSupply(_amount)
    {
        require(getSigner(_whitelist) == designatedSigner, "!Signer");
        require(_whitelist.userAddress == msg.sender, "!Sender");
        require(_whitelist.listType == 2, "WL: Wrong list type");
        require(
        ...
        require(_amount <= wlList.listCap, "WL Sold Out");
        require(msg.value == _amount * wlList.mintPrice, "WhiteList: Pay exact fees");
            
        wlListMintTracker[_whitelist.userAddress] += _amount;
        wlList.listCap -= _amount;
        _mint(_whitelist.userAddress, _amount);
    }
    ...
    @param _amount amount of tokens to mint in one transaction   
    */
    function publicMint(uint256 _amount) external payable nonReentrant checkSupply(_amount) {
        require(msg.sender == tx.origin, "PublicMint: Only wallets allowed");
        require(block.timestamp > wlList.endTime, "PublicMint: Not Live yet");
        require(
            _amount + publicMintTracker[msg.sender] <= publicMintLimitPerWallet,
            "PublicMint: Individual Cap exceeding"
        );
        require(msg.value == _amount * publicMintPrice, "PublicMint: Pay exact fees");
    ...
    */
    ...
    function setOG1IndividualCap(uint _cap) external onlyOwner {
        ogList1.purchaseLimit = _cap;
    }
    function setOG2IndividualCap(uint _cap) external onlyOwner {
        ogList2.purchaseLimit = _cap;
    }

    function setOGList1Cap(uint _cap) external onlyOwner {
        require(ogList1.spotsSold < _cap, "Sold Spots OG-1 Exceeding Cap");
        ogList1.listCap = _cap;
    }
    
    function setOGList2Cap(uint _cap) external onlyOwner {
        require(ogList2.spotsSold < _cap, "Sold Spots OG-2 Exceeding Cap");
        ogList2.listCap = _cap;
    }
    ...
    function setWLListCap(uint _cap) external onlyOwner {
        require(wlList.spotsSold < _cap, "Sold Spots WL Exceeding Cap");
        wlList.listCap = _cap;
    }

    function setPublicMintLimitPerWallet(uint256 _cap) external onlyOwner {
        publicMintLimitPerWallet = _cap;
    }

    function setPublicMintSupplyCap(uint256 _cap) external onlyOwner {
        require(publicMinted < _cap, "Sold Spots Exceeding Cap");
        publicMintSupplyCap = _cap;
    }
    ...
    function setRevealAfter(uint256 _revealAfter) external onlyOwner {
        revealAfter = _revealAfter;
    }
