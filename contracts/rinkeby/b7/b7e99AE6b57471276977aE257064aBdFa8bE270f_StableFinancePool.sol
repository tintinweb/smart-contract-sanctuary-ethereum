...
import "./LinerPool/LinerPoolGetter.sol";
    ...
    Getter
{
    ...
    function linerAddEarlyWhitelist(uint256 _poolId, address _accounts)
        external
        onlyAdmin
        stableFinanceValidatePoolById(_poolId)
    {
        ...
        if (!pool.isWhitelisted[_accounts])
            pool.isWhitelisted[_accounts] = true;
        emit StableFinanceAddEarlyWhitelist(_poolId, _accounts);
    }
    ...
    function stableFinanceRemoveEarlyWhitelist(
        ...
        if (pool.isWhitelisted[_accounts])
            pool.isWhitelisted[_accounts] = false;
        emit StableFinanceRemoveEarlyWhitelist(_poolId, _accounts);
    }
        ...
        stableFinanceValidatePoolById(_poolId)
    {
        ...
        require(
            ...
            "StableFinanceStakingPool: invalid percentage"
        );
        require(
            poolInfo.isWhitelisted[account],
            "StableFinanceStakingPool: not whitelisted"
        );
        ...
        uint256 reward;
        {
            ...
            pending.applicableAt = block.timestamp + poolInfo.delayDuration;

            pool.totalStaked -= amount;

            pool.stableFinanceAcceptedToken.safeTransfer(treasury, penaltyFee);
        }
    ...
    function stableFinanceUpdatePool(
        ...
        require(
            ...
            "StableFinanceStakingPool: update pool after pool started"
        );
        ...
        notInBlackList(msg.sender)
    {
        ...
        require(amount > 0, "StableFinanceStakingPool: nothing");
        require(
        ...
        notInBlackList(msg.sender)
    {
        ...
        require(
            amount > 0,
            "StableFinanceStakingPool: nothing is currently pending"
        );
...
contract Getter is State {
  ...
  modifier onlyWhiteList(uint256 _poolId) {
        ...
        require(
            ...
            "StableFinanceStakingPool: not in white list"
        );
        _;
    }
...
contract State is Struct, Event {
