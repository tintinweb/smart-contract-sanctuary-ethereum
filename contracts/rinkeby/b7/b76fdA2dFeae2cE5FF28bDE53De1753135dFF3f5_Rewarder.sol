...
contract Rewarder is ReentrancyGuard, Pausable, IRewarder, AccessControl {
    ...
    function updateStakingPool(
        ...
        stakePool.rewardPerTokenStored = calculateRewardRatePerTokenForStakingPool(
            duration,
            allocationPoints,
    ...
    function stakingRewardPerToken(uint256 duration) public view override returns (uint256 rewardPerToken) {
        ...
        return
            calculateRewardRatePerTokenForStakingPool(
                duration,
                _stakingPool.allocationPoints,
                _totalAllocationPoints,
                _stakingPool.balance
            );
    }

    function calculateRewardRatePerTokenForStakingPool(
        uint256 duration,
        uint256 allocationPoints,
        uint256 totalAllocationPoints,
        uint256 balance
    ) internal view returns (uint256 rewardRatePerToken) {
        uint256 rateForStakePool = duration == 0
            ? _stoa().getStakingRewardRateForPeriod(_stoa().getPeriodIndex())
            : _stoa().getCommitmentRewardsForPeriod(_stoa().getPeriodIndex()).div(_stoa().getPeriodDuration()).mul(allocationPoints).div(
                totalAllocationPoints
            );

        return balance == 0 ? rateForStakePool : rateForStakePool.mul(1e18).div(balance);
    }
        ...
        returns (uint256 rewardAmount)
    {
        ...
        if (!_stake.exists && duration > 0 && !isSagePathUser(user)) {
            return rewardAmount;
        }

        rewardAmount = _stake.userStakeRewardPerToken.mul(block.timestamp - _stake.lastClaimed).add(_stake.rewards);

        uint256 stakeDuration = block.timestamp - _stake.timestamp;
        if ((duration == 0 && stakeDuration < 30 days) || (stakeDuration < duration * 1 days)) {
    ...
    function getAccomplishmentReward(address user) public view returns (uint256 rewardAmount) {
        ...
        if (periodsSinceLastClaimed > 0) {
            for (uint256 i = periodsSinceLastClaimed; i >= 1; --i) {
                rewardAmount += _calculateAccomplishmentReward(user, _periodIndex - i);
            }
        }
    ...
    // Mutative functions
    // Claims
    function claimRewards(uint256[] calldata stakingPools) public nonReentrant onlyActiveUser(msg.sender) onlySagePath {
    ...
    modifier onlyActiveUser(address user) {
        require(_isActiveUser(user), "No meditation in last seven days");
        _;
    }
    ...
    modifier onlyValidStakeDuration(uint256 duration) {
        require(
            ...
            "Invalid staking period duration"
        );
        _;
    }
