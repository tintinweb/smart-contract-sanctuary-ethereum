 ...
 * @title MintingModule
 */
contract MintingModule is  MintingModuleStorage{

    bytes32 public constant MINTER_ROLE = keccak256("MINTER_ROLE");

	modifier onlyMinter() {
		require(roleModule.hasRole(MINTER_ROLE, msg.sender), "MintingModule: Invalid minter");
		_;
	}
    /**
     * @notice Creates a new collection
     * @dev name and symbol are ignored for erc1155
     * @dev can only be called by minters for this module
     * @param channel       Address of the channel
     * @param assetClass    Collection asset class, ERC721 | ERC1155 asset class
    ...
    function createCollection(
        ...
        if (
            ...
            data = abi.encodeWithSignature(
                "createCollection(bytes4,string,string)",
                assetClass,
                name,
                symbol
            );
        } else {
            revert("Module Error: Invalid Asset Class");
        }

        executeTransaction(channel, collectionFactory, data);
    }

    /**
     * @notice Create new Forging to a collection
     * @param channel       Channel address of the collection
     * @param collection    Asset collection for the forging
     * @param assetClass    NFT asset classes ERC721 | ERC1155
     * @param forging   Forging details
     */
    function createForging(
        address channel,
        address collection,
        bytes4 assetClass,
        Forging memory forging
    ) public onlyMinter {
        bytes memory data;
        ...
        if (
            ...
            data = abi.encodeWithSignature(
                "mintForging((string,uint256,address,uint96))",
                forging
            );
        } else {
            revert("Module Error: Invalid Asset Class");
        ...
        executeTransaction(channel, collection, data);
    }

    /**
     * @notice Execute Transaction as the channel
     * @param channel   Channel Address
     * @param dest      Execute call destination
     * @param data      Function data to execute
     */
    function executeTransaction(
        ...
        require(
            ...
            "Minting Module: Invalid Transaction"
        );
    }
}
