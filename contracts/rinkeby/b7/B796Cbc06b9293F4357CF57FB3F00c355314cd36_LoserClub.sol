...
/**
 * @dev alternate version of the contract implementing the
 * experimental ERC721A Contract of which was modified
 */
contract LoserClub is ERC721A {
  // sale control
  ...
  constructor(uint256 maxPerTxn_, uint256 maxSupply_)
    payable
    ERC721A('Loser Club', 'LOSERCLUB', maxPerTxn_, maxSupply_)
  {
  ...
  */
  ...
  /**
   ...
   ============================================
  */
  function checkWhitelist(bytes32[] calldata proof_, bytes32 leaf_)
    external
    view
    returns (bool)
  {
 ...
 *
 * Assumes the number of issuable tokens (max per transaction) is capped and fits in a uint128.
 *
 * Does not support burning tokens to address(0).
 *
 * Assumes that an owner cannot have more than the 2**128 (max value of uint128) of supply
 *
 * @dev customization to ERC721A:
 *   1. modified collectionSize name to maxSupply and maxBatchSize to maxPerTxn
 *   2. also added setMaxPerTxn and setMaxSupply functions for adjustment purposes.
 *   3. removed quantity <= maxPerTxn require statement for safeMint as it's already being
 *      taken care of in main contract
 *   4. manually cherrypicked and added desired changes from v.2.0 updates:
 *      https://github.com/chiru-labs/ERC721A/releases/tag/v2.0.0
 */
contract ERC721A is
  Context,
  Ownable,
  ...
  IERC721Enumerable
{
  ...
  /**
   * @dev
   * `maxPerTxn` refers to how much a minter can mint at a time.
   * `maxSupply_` refers to how many tokens are in the collection.
   */
  constructor(
    string memory name_,
    ...
    require(
      maxSupply_ > 0,
      'ERC721A: collection must have a nonzero max supply'
    );
    require(maxPerTxn_ > 0, 'ERC721A: max per transaction must be nonzero');
    _name = name_;
    _symbol = symbol_;
  ...
  /**
   * @dev allows owner to adjust the max per transaction
   */
  ...
  /**
   * @dev allows owner to adjust the max supply amount
   */
   ...
   * This read function is O(maxSupply). If calling from a separate contract, be sure to test gas first.
   * It may also degrade with extremely large max per transactions (e.g >> 10000), test for your use case.
   */
    ...
    returns (TokenOwnership memory)
  {
    ...
    if (tokenId >= maxPerTxn) {
      lowestTokenToCheck = tokenId - maxPerTxn + 1;
    }
   ...
   * - `quantity` cannot be larger than the max per transaction.
   *
  ...
  function _safeMint(
    ...
    // We know if the first token in the transaction doesn't exist, the other ones don't as well, because of serial ordering.
