...
/** 
 * * Error Handler
 * @dev Error handler to identify errors through a single error function
 * @param _code The error code triggered by the error
 * ! 100: Invalid Phase: Whitelist Not Active
 * ! 101: Invalid Phase: Public Sale Not Active
 * ! 102: Invalid Phase: All Phases Inactive
 * ! 103: Invalid Phase: Reserve Phase Not Active
 * ! 200: Invalid Quantity: Exceeds Max Supply
 * ! 201: Invalid Quantity: Exceeds Max Alloted
 * ! 202: Invalid Quantity: Exceeds Max Reserve Supply
 * ! 203: Invalid Quantity: Exceeds Max Whitelist Supply
 * ! 204: Invalid Quantity: Exceeds Max Per Transaction
 * ! 300: Invalid Coupon: 
 * ! 400: Invalid Payment: Insufficient Funds
 * ! 500: Invalid Address: Zero Address
 * ! 501: Invalid Address: Restricted
 */
error ErrorHandler(uint256 _code);

contract LacunaStrategiesAS is ERC721A, Ownable {
    using Counters for Counters.Counter;

    string  public              customBaseURI; // Variable to override Base URI

    address private             couponSigner; // Address of the wallet that generated the coupons
    address public              bankManager; // Address that controls payment splitter release
    
    uint256 public              maxSupply = 10000; // Maximum tokens in the full collection
    ...
    struct MintTypes {
		uint256 _mintsByAddress; // Mint type used to track number of mints by address
    ...
    ) ERC721A("LacunaStrategiesAS", "LSAS") {
     ...
     * @param _maxSupply The new maximum supply available to be minted
     */
    ...
    function release(address payable _account) public virtual {
        if (msg.sender == bankManager || msg.sender == owner()) {
            splitter.release(_account);
        } else {
            revert ErrorHandler(501); // ! 501: Invalid Address: Restricted
        }
    }
	...
	function _isVerifiedCoupon(bytes32 _digest, Coupon memory _coupon) internal view returns (bool) {
		...
		if (signer == address(0)) revert ErrorHandler(500); // ! 500: Invalid Address: Zero Address
    ...
    function mintReserves(address _to, uint256 _qty) external onlyOwner {
        ...
        if (phase != SalePhase.Reserve) revert ErrorHandler(103); // ! 103: Invalid Phase: Reserve Phase Not Active
        // Verify available supply has not been exceeded
        if (reserveMintCounter.current() + _qty > maxReserveSupply) revert ErrorHandler(200); // ! 202: Invalid Quantity: Exceeds Max Reserve Supply
    ...
    function mint(uint256 _qty) external payable {
        ...
        if (phase != SalePhase.PublicSale) revert ErrorHandler(101); // ! 101: Invalid Phase: Public Sale Not Active
        // Verify available supply has not been exceeded
        if (supplyMintCounter.current() + mintQty > maxPublicSupply) revert ErrorHandler(200); // ! 200: Invalid Quantity: Exceeds Max Supply
        // Verify quantity minted has not exceed max allowed per transaction
        if (_qty > maxPerTx) revert ErrorHandler(204); // ! 204: Invalid Quantity: Exceeds Max Per Transaction
        // Verify Payment
        if (msg.value < mintPrice * _qty) revert ErrorHandler(400); // ! 400: Invalid Payment: Insufficient Funds
     ...
     */
    function mintWhitelist(uint256 _qty, uint256 _allotted, Coupon memory _coupon) external payable {
        // Verify phase is not locked
        if (phase != SalePhase.Whitelist) revert ErrorHandler(100); // ! 100: Invalid Phase: Whitelist Not Active
        ...
        if (!_isVerifiedCoupon(digest,_coupon)) revert ErrorHandler(300); // ! 300: Invalid Coupon
        // Verify quantity (including already minted reserves) does not exceed allotted reserve amount
        if (_qty + addressToMinted[msg.sender]._whitelistMintsByAddress > _allotted) revert ErrorHandler(201); // ! 201: Exceeds Max Allotted 
