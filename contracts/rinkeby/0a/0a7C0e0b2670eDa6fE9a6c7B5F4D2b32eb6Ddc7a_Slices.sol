...
contract Slices is iSlices, ERC721, ReentrancyGuard, Ownable {
  ...
  uint256 private constant EACH_SUPPLY_SPECIAL = 5;
  uint256 private constant EACH_SUPPLY_PHYSICAL = 5;
  uint256 private constant PRICE = 0.1 ether;
  uint256 private constant PRICE_SPECIAL = 0.5 ether;
  uint256 private constant PRICE_PHYSICAL = 2.5 ether;
  ...
  // TODO: Delete
  function toDay(uint256 tokenId) public view returns (uint256) {
    return tokenId / ONE_MILLION;
  }

  // TODO: Delete
  function toLevel(uint256 tokenId) public view returns (uint256) {
    return (toDay(tokenId) - 1) % LEVEL;
  }
    ...
    returns (uint256[] memory)
  {
    ...
    for (uint256 i = 0; i < MAX_SUPPLY; i++) {
      // TODO: toDayFromIdを使わず、100万, 200万...をループする処理を入れれば良いかも？
  ...
  // TODO: 一度セットしたら変更できないようにした方が美しいかも？
