...
//https://etherscan.io/address/0x6970a2d8df91afbd9a6560829689f57ae2773a68#code

// @todo add contract comments ie Netsec
// @todo
contract Airdrop is ERC721Enumerable, Ownable, ReentrancyGuard {
    ...
    constructor(string memory _uri,uint256 _weiAmount) ERC721("NFT", "NFT") {
        baseURI = _uri;
    ...
    // @todo Must add function of tokenURI

    // Multiple user's must get different NFT with non-sequencial tokens.
    // @to-thik Add a check to ensure that the token Balance must be zero.
    // Check should not exceed max supply.
    // Optmize for loop via use of unchecked
    // Security check the smart contract
    // @to-thik Single mint  and Multiple mint based contract functions (Cant implemented)
    // check quantity > 0

    // uusing this looping concept we will be facing out the issues like ..
    // user A want to receive 5 toekens then token id's alloted will be sequencial
    // Either to overcome this we will be need to use single loop with address as 5 times in CSV
    /** 
    function mint(address[] calldata array, uint256[] calldata quantity) external onlyOwner nonReentrant {
...
*/
    ...
    function multiMint(address[] calldata _addressList) external onlyOwner nonReentrant {
        ...
        for (uint256 i = 0; i < _addressList.length; i = uncheck_inc(i)) {
    ...
    function uncheck_inc(uint256 x) private pure returns (uint256) {
        unchecked {
            return x + 1;
        }
    }

    //@TODO add non-Renetrant
    //@TODO Add check for tokenId less then MAX_SUPPLY
    function redeemToken(uint256 _tokenId) external payable nonReentrant {
        ...
        // owner can be updated by the admin
    ...
    //@todo add funcion to update deposit adress by admin only.
    // validate deposit address == 0 or already same address.
    ...
    function setSaleState(bool _status) external onlyOwner {
        require(_status != isSalePaused, "Invalid input");
...
// Can some-one sends an NFT to contract and can update the totalSupply for contract
// Do ERC-721 contracts have default token Receive method/interface implemented.

// Bug test
// We must check if the receiver is a contract with mallicious code is able to mint /renetracnt or attack

// Do we need check if receiver is non-cotract in script validation
