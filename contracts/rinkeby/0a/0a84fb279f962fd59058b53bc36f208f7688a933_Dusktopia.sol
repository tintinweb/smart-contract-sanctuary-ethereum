...
contract Dusktopia is IDusktopia, Ownable, ERC721A, PaymentSplitter {
    ...
    uint256 public constant DL_SUPPLY = 4055;

    uint256 public tokenCost = 0.12 ether;
    uint256 public dusklistLimit = 2;
    uint256 public reserveLimit = 1;
    uint256 public publicLimit = 1;

    event Minted(address indexed addr, uint256 quantity);
    ...
    function dusklistMint(uint256 quantity, bytes calldata signature) external payable {
        ...
        if (_totalMinted() + quantity > DL_SUPPLY) revert DuskSupplyExceeded();
        if (msg.value != quantity * tokenCost) revert InvalidEtherAmount();
        if (_numberMinted(msg.sender) + quantity > dusklistLimit) revert OverTokenTxnLimit();
    ...
    function reserveMint(uint256 quantity, bytes calldata signature) external payable {
        ...
        if (_totalMinted() + quantity > DL_SUPPLY) revert DuskSupplyExceeded();
        if (msg.value != quantity * tokenCost) revert InvalidEtherAmount();
        if (_numberMinted(msg.sender) != 0) revert TokensAlreadyClaimed();
    ...
    /// @notice Function used to mint tokens during the public sale.
    ...
    function _teamMint(address receiver) internal {
        _mint(receiver, RESERVED_TOKENS);
    }
    ...
    /// @notice Function used to set a new signer address.
    ...
    /// @return The current _signerAddress value.
