...
contract PolkaPetBooster is VRFConsumerBase, Ownable, ERC1155Holder, ReentrancyGuard, Pausable {
	...
	mapping(address => bytes32[]) openPackUserRequests;
	...
	function addPolkapetNFT(address _polkapetNFT) external onlyOwner {
	...
	function openPack(uint256 id) external whenNotPaused nonReentrant {
		...
		openPackRequests[requestId] = OpenPackPending(msg.sender,1);

		bytes32[] storage userRequests = openPackUserRequests[msg.sender];
		userRequests.push() = requestId;

		IChildToken(lootBoxNFT).burn(msg.sender, id, 1);
		availablePacks -= 1;

		emit OpenPackRequest(msg.sender, 1, requestId);
	}
	 ...
	 * Callback function used by VRF Coordinator
	 */
	function getUserRequests()  public view returns (bytes32[] memory)  {
		return openPackUserRequests[msg.sender];
	}
	...
	function fulfillOpenPack(bytes32 requestId, uint256 randomness) internal {
		...
		// randoms = expand(randomness, slots.length);
		uint generated = 0;
		SlotId[] memory slotIds = new SlotId[](packSize);
		uint256[] memory globalIds = new uint256[](packSize);
		while(generated < packSize) {
			uint256 randomSlot = expandOnce(randomness, generated) % slots.length;
			bool found = false;
			uint256 iterations = generated;
			while(! found){
				uint256 n = slots[randomSlot].index.length; //total tokens of slot
				uint x = expandOnce(randomness, iterations) % ONE_FIXED_POINT;
				uint256 nx = n * x;
				uint256 selectedSlotIndex = nx / ONE_FIXED_POINT;      //random slot card index
				uint256 y = nx % ONE_FIXED_POINT;      //random drop chance
				SlotIndex storage slotIndex = slots[randomSlot].index[selectedSlotIndex];
				NFT storage nft;

				if(y < slotIndex.prob){
					nft = slotIndex.nft;
				} else {
					nft = slotIndex.aliasNFT;
				}
				
				uint256 nftQty = nft.currentQty;
				if(nftQty > 0) {
					globalIds[generated] = nft.ID;
					slotIds[generated].id = nft.ID;
					slotIds[generated].slot = randomSlot;
					nft.currentQty = nftQty - 1;
					found = true;
					generated++;
				}

				if(nftQty <= 1 && (slotIndex.prob == ONE_FIXED_POINT || nft.currentQty == 0)){
					slots[randomSlot].index[selectedSlotIndex] = slots[randomSlot].index[n - 1];
					slots[randomSlot].index.pop();
				}
		...
		//delete
		bytes32[] storage userRequests = openPackUserRequests[reciepient];
		...
		delete  openPackRequests[requestId];

		emit OpenedPack(reciepient, slotIds);

		//safe batch transfer
		for(uint i = 0; i<slots.length;i++) {
			uint count = 0;
			for(uint j = 0; j<slotIds.length;j++) {
				if(slotIds[j].slot == i) {
					count++;
				}
			}
