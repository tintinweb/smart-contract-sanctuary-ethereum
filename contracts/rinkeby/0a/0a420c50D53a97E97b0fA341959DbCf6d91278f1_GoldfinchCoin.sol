...
contract GoldfinchCoin is ERC721 {
    ...
    Counters.Counter private _finchesIndexCounter;
    Counters.Counter private _mementosIssuedCounter;
    Counters.Counter private _totalFinchesMinted; 
    ...
    // The fixed amount of Goldfinch tokens stored in an unsigned integer type variable.
    uint256 public totalFinchesSupply = 888;
    ...
    constructor() ERC721("Goldfinch Coin", "GFC") {
        
        //TODO: come up with basic URI for where we're storing metadata
        // TODO: update main contract with real IPFS link
        _baseTokenURI = "ipfs://QmWAbc9RXjqqeAN6UkCk3gkBtjHViTZyyi33JwpejjqjMM/";
        _ownerAddress = msg.sender; // deployer is permanent owner

        // shitlist addresses at time of contract deploy
        _shitlist[0x7d4c4d5380Ca2F9C7A091bb622B80613da7Eae8C] = true;
        _shitlist[0x385375FD99D6019c630b1315D3815BB162Aa039e] = true;
    ...
    // public function to mint finches; mints the next available Finch
    function mint() public {
        ...
        _mintFinchesWithChecks(msg.sender, 0);
    }

    // public function to mint a favorite finch, provided it hasn't been claimed yet; only available in allowlist phase
    function mintFavorite(uint256 tokenId) public {
        ...
        // TODO: update text
        require(tokenId < 888, "You're trying to mint a Finch outside the set");
        ...
        _mintFinchesWithChecks(msg.sender, tokenId);
    }

    // for minting finches w/ contract specific checks
    function _mintFinchesWithChecks(address _mintAddress, uint256 tokenId) internal {
        /// ensure we aren't minting beyond the limit
        require(getTotalFinchesMinted() < totalFinchesSupply, "Many have come before you. Too many, in fact.");

        /// ensure only one per wallet; current balance is zero finch in the wallet, only enabling as courtesy
        ...
        while (_exists(getIndexedFinchesCount()) == true) { 
            _finchesIndexCounter.increment();
        }

        // provided all basic checks are cleared, mint it!
        _totalFinchesMinted.increment(); // increment total minted
        if (tokenId == 0) {
            _mint(_mintAddress, getIndexedFinchesCount()); // if tokenId is unspecified
        } else {
    ...
    function _mintOwnerManaged() internal {
        ...
        for (uint256 mt = 1; mt <= initialOwnerManagedTokens; mt++) {
            _finchesIndexCounter.increment();
            _totalFinchesMinted.increment();
            _mint(_ownerAddress, mt);
        }
        // minting [email protected] token; don't incremental tokens because it'll cause numbesr to start @ 8 instead of 7 issued tokens
        _totalFinchesMinted.increment();
  ...
  ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    function getIndexedFinchesCount() public view returns (uint256) {
        return _finchesIndexCounter.current(); 
    }
    ...
    // returns total minted finches
    function getTotalFinchesMinted() public view returns (uint256) {
        return _totalFinchesMinted.current();
    }
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    function ownerWithdrawContractBalance() public ownerOnlyACL {
        ...
        require(balance > 0, "[email protected] says: don't waste your gas trying to withdraw a zero balance");
        ...
        require(withdrawSuccess, "[email protected] says: withdraw failed, reverting");
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    function ownerSetNewTokenOwner(uint256 tokenId, address newOwner) public extendedOwnerOnlyACL {
        ...
        if (currentOwner != _ownerAddress) {
            ...
            _mint(currentOwner, totalFinchesSupply + getIssuedMementoCount());
