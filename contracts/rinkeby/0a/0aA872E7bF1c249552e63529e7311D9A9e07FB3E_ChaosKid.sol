...
interface IChaosKidScores {
    function getKidScore(uint256 tokenId) external view returns (uint256 score);
    function getKidBoost(uint256[] calldata tokenIds) external view returns (uint256 score);
}


contract ChaosKid is ERC721A, Ownable, ReentrancyGuard, VRFConsumerBaseV2  {
    using Strings for uint256;

    enum Stage { NotOpen, PreSale, PublicSale, SoldOut }
    ...
    uint256 public constant WL_SALE_PRICE = 0.001 ether;
    uint256 public constant BANAGER_SALE_PRICE = 0 ether;
    ...
    string private preRevealURI = "ipfs://QmT59RB8yk4n67vPGT3Psj3VndaMNyhbAKbuJawhA96PpD/";
    ...
    modifier preSaleActive(){
        require(!isPublicSaleActive && (isWlActive || isBanagerActive),"Presale is not open");
        _;
    }
    ...
    modifier canMintKidsGlobal(uint256 numberOfTokens) {
        require(numberOfTokens > 0,"Mint count must be greater than 0");
        require(
            totalSupply() + numberOfTokens <=
                maxSupply,
            "Not enough kids remaining to mint"
        );
        _;
    }
    ...
    function fulfillRandomWords(
        ...
        revealed = true;
        tokenOffset = randomWords[0] % maxSupply;
    }

    function setChainlinkConfig(uint32 _callbackGasLimit, bytes32 _keyHash, uint64 _subscriptionId) external onlyOwner {
    ...
    function setAllMerkleRoot(bytes32[2] calldata merkleRoots) external onlyOwner {
    ...
    function tokenURI(uint256 _tokenId) public view override returns (string memory) {
        ...
        if (!revealed) return bytes(preRevealURI).length != 0 ? string(abi.encodePacked(preRevealURI, _tokenId.toString())) : '';
        uint256 shiftedTokenId = (_tokenId + tokenOffset) % maxSupply;
        return string(abi.encodePacked(postRevealBaseURI, shiftedTokenId.toString()));
    }


    function getKidScore(uint256 tokenId) public view returns (uint256 score) {
      ...
      return IChaosKidScores(scoresContractAddress).getKidScore(tokenId);
    }

    function getKidBoost(uint256[] calldata tokenIds) public view returns (uint256 score) {
      if (scoresContractAddress == address(0x0)) {
        return 10000;
      }
      return IChaosKidScores(scoresContractAddress).getKidBoost(tokenIds);
    }

    // ============ PUBLIC FUNCTIONS FOR MINTING ============
    function getCurrentStage() public view returns (Stage){
        Stage curStage;
        if(getLeftKidCount() == 0){
            curStage = Stage.SoldOut;
        }
        else if(isPublicSaleActive){
            curStage = Stage.PublicSale;
        }else if(isWlActive || isBanagerActive){
    ...
    function getLeftKidCount() public view returns(uint256){
        uint256 numMintedSoFar = totalSupply();
        return maxSupply - numMintedSoFar;
    }

    function getCanFreeMintCount(bytes32[][2] calldata merkleProofs) public view returns (uint256 )
    {
        require(merkleProofs.length == 2, "Not right length");
    ...
    function mintNFTPresale(uint256 amount,bytes32[][2] calldata merkleProofs) 
    external
    payable
    nonReentrant
    isNotContract
    preSaleActive
    canMintKidsGlobal(amount)
    {
        require(merkleProofs.length == 2, "Not right merkleProofs length");
        uint256 count = getCanFreeMintCount(merkleProofs);
        require(amount == count,"Invalid amount");

        if(count == WL_MINT_MAX_PER_WALLET){
            require(WL_SALE_PRICE * amount == msg.value, "Incorrect ETH value sent");
            preSaleMinted[msg.sender] = true;
            whitelistMintCounts += uint64(count);
        }else if(count == BANAGER_MINT_MAX_PER_WALLET){
            require(BANAGER_SALE_PRICE * amount == msg.value, "Incorrect ETH value sent");
            preSaleMinted[msg.sender] = true;
            banagerMintCounts += uint64(count);
        }else{
            revert('Interal error when mintNFTPresale.');
        }
    ...
    function tokenByIndex(uint256 _index) public view returns (uint256 tokenId){
        if(_index < totalSupply()){
            ...
            revert('Not so many nft token has been minted so far.');
        }
    }
}
