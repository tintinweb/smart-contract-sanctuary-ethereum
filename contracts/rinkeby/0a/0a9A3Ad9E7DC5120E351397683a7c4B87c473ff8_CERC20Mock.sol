...
contract CERC20Mock is ERC20 {
    ...
    function mint(uint256 amount) public returns (uint256) {
        ...
        _mint(msg.sender, amount.mul(COMPOUND_EXCHANGE_RATE_PRECISION).tryDiv(exchangeRate));
        exchangeRate = exchangeRate.add(1521 * 10**10);
        return 0;
    }

    function redeemUnderlying(uint256 amount) external returns (uint256) {
        _burn(msg.sender, amount.mul(COMPOUND_EXCHANGE_RATE_PRECISION).tryDiv(exchangeRate));
