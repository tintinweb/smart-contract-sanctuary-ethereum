...
contract ForzaCars is ERC1155, Ownable, ReentrancyGuard {
    ...
    // // logic de pourrit a retravailler
    // function getMysteryCar(uint256 forzaCoin) payable public nonReentrant returns(uint carId){
    //     require(forzaCoin > 0 || msg.value > 0, "Payable function received no payment");

    //     uint256 amountToMint = 0;
    //     uint256 refund = 0;
    //     if (forzaCoin > 0){
    //         require(forzaCoinBank[msg.sender] >= forzaCoin, "Insuficent fund");
    //         require(forzaCoin >= MYSTERY_CAR_PRICE_FORZACOIN, "Insuficent fund to mint at least one, Forza.");
    //         uint256 _amountToMint = forzaCoin / MYSTERY_CAR_PRICE_FORZACOIN;
    //         forzaCoinBank[msg.sender] = forzaCoinBank[msg.sender] - (_amountToMint * MYSTERY_CAR_PRICE_FORZACOIN);
    //         amountToMint += _amountToMint;
    //     }
    //     if (msg.value > 0){
    //         require(msg.value >= MYSTERY_CAR_PRICE_ETH, "Insuficent fund to mint at least one, Eth.");
    //         uint256 _amountToMint = msg.value / MYSTERY_CAR_PRICE_ETH;
    //         refund = msg.value - (_amountToMint * MYSTERY_CAR_PRICE_ETH);
    //         amountToMint += _amountToMint;
    //     }
    //     (uint256[] memory carIds,uint256[] memory carAmounts) = _mintMysteryCar(msg.sender,amountToMint);
    //     if (refund > 0) {
    ...
    // function _mintMysteryCar(address to, uint256 qty) internal returns(uint256[] memory _carIds, uint256[] memory _carAmounts){
    //     uint256[] memory carIds;
    //     uint256[] memory carAmounts;
    //     for(uint256 i = 0; i < qty; i++){
    //         carIds[i] = (i % currentBaseCar) + BASE_CAR_OFFSET; // random bientot
    //         carAmounts[i] += 1;
    //     }
    //     _mintBatch(to, carIds, carAmounts, "");
    //     safeBatchTransferFrom(address(this), to, carIds, carAmounts, "");

    //     return (carIds,carAmounts);
    // } 

    // //Temporaire random value
    // function random() private view returns(uint){
    //     uint diff = block.difficulty;
    //     bytes32 hash = blockhash(block.number-1);
    //     return uint(keccak256(abi.encodePacked(block.timestamp, diff, hash)));
    // }

    // function setMysteryPrice(uint256 ethPrice, uint256 forzaCoinPrice) public {
    //     require(ethPrice > 0 && forzaCoinPrice > 0, "Price must be greater than zero");
    //     MYSTERY_CAR_PRICE_ETH = ethPrice;
    //     MYSTERY_CAR_PRICE_FORZACOIN = forzaCoinPrice;
    //     emit UpdateMysteryCarPrice(MYSTERY_CAR_PRICE_ETH,MYSTERY_CAR_PRICE_FORZACOIN);
    // }
    ...
    function mintBatchBaseCar(uint256[] memory carIds) external onlyOwner{
        ...
        uint256[] memory ids = new uint[](index);
        uint256[] memory amounts = new uint[](index);
    ...
    function airdropBaseCar(uint256 carId, address to) external onlyOwner{
        ...
        require(balanceOf(address(this), carId) > 0, "No more token avalaible");

        baseCarAirdropped[to]++;

        safeTransferFrom(address(this), to, carId, 1, "");
    }


    function updateValidCars(uint256[] memory carIds, bool[] memory status) external onlyOwner {
        require(carIds.length == status.length,"Both arrays must be same length");
