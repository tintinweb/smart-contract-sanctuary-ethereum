...
contract SynthetixBridgeToBase is
    BaseSynthetixBridge,
    ...
    iOVM_L2DepositedToken
{
    ...
    function _initiateWithdraw(address to, uint256 amount) private {
        require(
            synthetix().transferableSynthetix(msg.sender) >= amount,
        ...
        bytes memory messageData = abi.encodeWithSelector(
            bridgeToOptimism.finalizeWithdrawal.selector,
            to,
            amount
        );
    ...
    function finalizeEscrowMigration(
        ...
        rewardEscrow.importVestingEntries(
            account,
            escrowedAmount,
            vestingEntries
        );
    ...
    // invoked by Messenger on L2
    function finalizeFeePeriodClose(
        uint256 snxBackedAmount,
