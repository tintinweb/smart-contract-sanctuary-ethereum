...
def __init__():
    ...
    self.base_uri = "ipfs://bafybeicrbcbmxaeluzapoczl3tt3a23hj6crx3lzgvcilbccjzs2srhksy/"
...
def tokenURI(tokenId: uint256) -> String[206]:
    if self.idToOwner[tokenId] == empty(address):
        raise 


    return concat(self.base_uri, uint2str(tokenId))
...
def mint(receiver: address, seat_id : uint256):
    ...
    #assert len(self.token_log) < MAX_SUPPLY # dev: Minted must be less than MAX_SUPPLY

    # Add NFT. Throws if `_token_id` is owned by someone
    self._addTokenTo(receiver, seat_id)
    self.total_supply += 1
...
@external
def contractURI(addr: address, val: uint256) -> String[1]:
    return "X"

@external
def symbol(addr: address, val: uint256) -> String[1]:
    return "X"

@external
def name(addr: address, val: uint256) -> String[1]:
    return "X"