...
contract Wallet  {
    ...
    event swapeth1toBm(address indexed account, uint amount, uint value);
    event swapBmTToEt(address indexed account, uint amount, uint value);
...
IERC20 public ethAddress;
   ...
   uint amounts=amount ;   // convert the eth token to the bmt token
    ...
    uint256  value1=1;
     ...
     emit swapeth1toBm(msg.sender,amount,value1);
    }
    ...
    function swapBmTToEth(uint amount) public { //get the user input in the line
         ...
         uint _amount=(amount/(tokenPrice  )) ;//convert the bmt toten to eth
         ...
         emit swapBmTToEt(msg.sender,amount,value);
    }
