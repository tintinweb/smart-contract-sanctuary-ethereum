...
contract ModernistMinter is AccessControl {
    ...
    mapping (address => uint256) _whitelistedAddresses;
    ...
    function setGenesisContract(address newAddress) public {
        Genesis = IGenesis(newAddress);
    }

    function setSplitContract(address newAddress) public {
        Split = ISplitMintable(newAddress);
    }

    function setMintableContract(address newAddress) public {
    ...
    function isWhitelisted(address to) public view returns (bool) {
        return hasRole(MINTER_ROLE, msg.sender) || _whitelistedAddresses[to] > 0;
    }
    ...
    function addWhitelistedUsers(address[] memory users) public onlyRole(MINTER_ROLE) {
        for (uint256 i = 0; i < users.length; i++) {
            _whitelistedAddresses[users[i]] = 1;
        }
    }

    function removeWhitelistedUsers(address[] memory users) public onlyRole(MINTER_ROLE) {
        // Recover the storage used up by whitelist
        for (uint256 i = 0; i < users.length; i++) {
            delete _whitelistedAddresses[users[i]];
        }
    }
    ...
    function stitch(uint256 modernistId, uint256[] memory quadrants) public {
        ...
        for (uint256 i = 0; i < quadrants.length; i++) {
            ...
            // Validate if quadrants belong to correct NFT
            // +2 to match offset ranges
    ...
    function isValidQuadrant(uint256 modernistId, uint256 quadrantToken, uint256 quadrant) private pure returns (bool) {
        ...
        // Start of valid range, e.g. 20001 for modernistId=1 and quadrant=1

        uint256 start = offset + (EDITIONS * (modernistId - 1)) + 1;
        // End of valid range, e.g. 20022 for modernistId=1 and quadrant=1
    ...
    function isValidSplitQuadrant(uint256 quadrantToken, uint256 quadrant) private pure returns (bool) {
        uint256 offset = TOKEN_OFFSET * quadrant;
        // Start of valid range for quadrant, e.g. 20001 quadrant=1
        uint256 start = offset;
        // End of valid range, e.g. 29724 for quadrant=1
        uint256 end = offset + TOTAL_SUPPLY;

        return quadrantToken >= start && quadrantToken <= end;
    }

    /// @dev Returns if the ID has been split     
