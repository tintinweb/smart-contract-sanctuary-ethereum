...
contract MintlistSimpleFolderIndexSale is Initializable, OwnableUpgradeable {
  // zNS Registrar
  ...
  string public baseFolderHashWithoutQm;

  // The folder prefix with `ipfs://Qm.../`
  string public prefix;

  // Merkle root data to verify on mintlist
  bytes32 public mintlistMerkleRoot;

  // Time in blocks that the mintlist will occur
  ...
  function __MintlistSimpleFolderIndexSale_init(
    ...
    string calldata baseFolderHashWithoutQm_,
    uint256 numForSale
  ) public initializer {
    __Ownable_init();
    ...
    baseFolderHashWithoutQm = baseFolderHashWithoutQm_;
    prefix = string(
      abi.encodePacked("ipfs://Qm", baseFolderHashWithoutQm, "/")
    );
    totalForSale = numForSale;
  }
  ...
  function setBaseFolderHash(string calldata folderHashWithoutQm)
    external
    onlyOwner
  {
    require(
      keccak256(bytes(folderHashWithoutQm)) !=
        keccak256(bytes(baseFolderHashWithoutQm)),
      "Cannot set to the same folder uri"
    );
    baseFolderHashWithoutQm = folderHashWithoutQm;
  }
  ...
  function _canAccountPurchase(
    ...
    require(
      block.number <= saleStartBlock + mintlistSaleDuration,
      "Sale has ended"
    );
  }
  ...
  function _mintDomains(uint256 numPurchased) internal {
    ...
    for (uint256 i = 0; i < numPurchased; ++i) {
      ...
      zNSRegistrar.registerDomainAndSend(
        parentDomainId,
        Strings.toString(startingMetadataIndex + i),
        address(this),
        getNftByIndex(startingMetadataIndex + i),
        0,
        true,
        msg.sender
      );
