...
contract WebacyBusinessU is Initializable, AccessControlUpgradeable, PausableUpgradeable {
    ...
    function _hasPaidMembership(address _address) private view {
    ...
    function storeERC20Data(
        ...
        _checkERC20EquallySizeLength(destinationAddresses, destinationPercents);

        _hasPaidMembership(msg.sender);
    ...
    function storeERC721Data(
        ...
        _checkERC721EquallySizeLength(destinationAddresses, destinationTokenIds);
    ...
    function _checkERC721EquallySizeLength(address[] memory destinationAddresses, uint8[] memory destinationTokenIds)
        private
        pure
    {
    ...
    function _checkERC20EquallySizeLength(address[] memory destinationAddresses, uint8[] memory destinationPercents)
        private
        pure
    {
    ...
    function withdrawBalance(
        ...
        uint256 availableBalance = IERC20Upgradeable(_contract).balanceOf(address(this));
        require(availableBalance >= _amount, "Insufficient funds");
        IERC20Upgradeable(_contract).transfer(_recipient, _amount);
    }

    function withdrawAllBalances(address _recipient) external onlyRole(DEFAULT_ADMIN_ROLE) {
        for (uint256 i = 0; i < contractBalances.length; i++) {
            address iContract = contractBalances[i];
            uint256 availableBalance = IERC20Upgradeable(iContract).balanceOf(address(this));
            if (availableBalance > 0) {
                IERC20Upgradeable(iContract).transfer(_recipient, availableBalance);
            }
        }
    }
