...
contract MCCToken is BEP20('MCC', 'MCC') {
    ...
    */
        ...
        external
    {
        ...
        require(now <= expiry, "MCCSwap::delegateBySig: signature expired");
...
contract MCC_Masterchef is Ownable {
    using SafeMath for uint256;
    ...
    function add(uint256 _allocPoint, IBEP20 _lpToken, uint16 _depositFeeBP, bool _withUpdate,uint256 _vestInterval,uint256 _vestPercent) public onlyOwner {
    ...
    function set(uint256 _pid, uint256 _allocPoint, uint16 _depositFeeBP, bool _withUpdate , uint256 _vestPercent , uint256 _vestInterval) public onlyOwner {
    ...
    function VestedClaim(address _user,uint256 _pid) internal {
        ...
        safeMCCTransfer(_user, toTransfer);
    ...
    // Safe mcc transfer function, just in case if rounding error causes pool to not have enough mcc.
    function safeMCCTransfer(address _to, uint256 _amount) internal {
        uint256 mccBal = mcc.balanceOf(address(this));
        if (_amount > mccBal) {
            mcc.transfer(_to, mccBal);
        } else {
            mcc.transfer(_to, _amount);
        }
    }
