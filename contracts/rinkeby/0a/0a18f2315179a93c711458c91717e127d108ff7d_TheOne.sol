...
contract TheOne is Ownable, ERC721A {
    ...
    uint256 constant public FREE_MINT_LIMIT = 2; 
    uint256 constant public FREE_MINT_LIMIT_TXN = 1;
    ...
    string public hiddenURI = "ipfs://QmYuw7ABgSPdjWHFYAH9e9kzEYayRqpNfULBEZ4uB5EJkN";
    ...
    function freeMint(uint256 quantity) external payable mintCompliance(quantity) {
        ...
        require(quantity <= FREE_MINT_LIMIT_TXN);

        uint256 newSupply = totalSupply() + quantity;
        uint256 currMints = numUserFreeMints[msg.sender];
        
        require(newSupply <= freeMintSupply, "Not enough free supply");
        require(currMints + quantity <= FREE_MINT_LIMIT, "User max free mint limit");
        
        userMintedFree[msg.sender] = true;

        numUserFreeMints[msg.sender] = (currMints + quantity);

        if(newSupply >= freeMintSupply) {
            freeSale = false;
            publicSale = true;
        }
