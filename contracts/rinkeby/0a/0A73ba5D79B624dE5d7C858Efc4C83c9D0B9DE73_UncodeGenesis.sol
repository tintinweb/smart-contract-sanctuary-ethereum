  ...
  VRFConsumerBaseV2
{
  ...
  uint256 public finalInventory = 0;
  ...
  /// @dev Emitted when a Uncode Genesis stops staking; either through standard means or by expulsion.
  ...
  constructor(
    ...
    ERC721Psi("Uncode Genesis", "UNCODE")
  ...
  modifier onlyApprovedOrOwner(uint256 tokenId) {
    require(
      ...
      "The caller is not approved nor owner"
    );
    _;
  }
  ...
  function requestRandomSeed() external onlyOwner {
    require(!metadataMapIsLocked, "The random seed was already requested");
  ...
  function setFinalInventory(uint256 _finalInventory) external onlyOwner {
    require(finalInventory == 0, "The final inventory was already set");
  ...
  /**
    ...
    been staked.
    @dev Staking is tied to a specific Uncode Genesis, not to the owner, so it doesn't
    reset upon sale.
  ...
  function toggleStaking(uint256 tokenId) public onlyApprovedOrOwner(tokenId) {
    uint256 start = stakingStarted[tokenId];
    if (start == 0) {
      require(stakingIsOpen, "Avatars: staking closed");
