...
contract WhitelistChecker is EIP712{
    ...
    string private constant SIGNING_DOMAIN = "Square Bears";
...
contract Staking is Initializable, ReentrancyGuardUpgradeable, OwnableUpgradeable, WhitelistChecker, ERC721EnumerableUpgradeable, IERC721ReceiverUpgradeable {
	...
	function initialize(address _gen2, address _rewardsToken) initializer public {
	...
	function stake(
		...
		for (uint256 i = 0; i < _bags.length; i++) {
			if (_bagIds[i] == 0) {
				_addNewBag( _bags[i].gen2TokenIds );
			}
			else {
				_addToBag(_bagIds[i], _bags[i].gen2TokenIds);
			}
		}
	}
	...
	function unstake(
		...
		for (uint256 i = 0; i < totalBags; i++) {
			...
			require(msg.sender == ownerOf(_bagId), "not bag owner");
			require(_gen2TokenIds[i].ids.length != 0, "nothing to unstake");
			...
			if (bags[_bagId].gen2TokenIds.length == 0) {
	...
	function modifyGen2Earnings(uint256 _rate) external onlyOwner {
		gen2Earnings = _rate;
	}
