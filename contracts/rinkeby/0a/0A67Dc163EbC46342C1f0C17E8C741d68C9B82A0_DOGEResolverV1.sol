...
// File: https://github.com/DOGE-domains/DOGE-contracts/blob/main/contracts/lib/strings.sol
...
// File: https://github.com/DOGE-domains/DOGE-contracts/blob/main/contracts/interfaces/IBaseDOGETLD.sol


pragma solidity ^0.8.4;


interface IBaseDOGETLD is IERC721 {

  struct Domain {
    string name; // domain name that goes before the TLD name; example: "tempetechie" in "tempetechie.web3"
    uint256 tokenId;
    address holder;
    string data; // stringified JSON object, example: {"description": "Some text", "twitter": "@techie1239", "friends": ["0x123..."], "url": "https://DOGE.domains"}
  }
...
// File: https://github.com/DOGE-domains/DOGE-contracts/blob/main/contracts/interfaces/IBaseDOGETLDFactory.sol


pragma solidity ^0.8.4;

interface IBaseDOGETLDFactory {
...
// File: https://github.com/DOGE-domains/DOGE-contracts/blob/main/contracts/resolver/DOGEResolverV1.sol
...
/// @title DOGE Domains Resolver v1
/// @author Tempe Techie
/// @notice This contract resolves all DOGE domains and TLDs on the particular blockchain where it is deployed
contract DOGEResolverV1 is Initializable, OwnableUpgradeable {
  ...
  function getDefaultDomain(address _addr, string calldata _tld) public view returns(string memory) {
    ...
    for (uint256 i = 0; i < fLength;) {
      address tldAddr = IBaseDOGETLDFactory(factories[i]).tldNamesAddresses(_tld);

      if (tldAddr != address(0) && !isTldDeprecated[tldAddr]) {
        return string(IBaseDOGETLD(tldAddr).defaultNames(_addr));
      }
  ...
  function getDefaultDomains(address _addr) public view returns(string memory) {
    ...
    uint256 fLength = factories.length;
    for (uint256 i = 0; i < fLength;) {
      string[] memory tldNames = IBaseDOGETLDFactory(factories[i]).getTldsArray();

      for (uint256 j = 0; j < tldNames.length; ++j) {
        string memory tldName = tldNames[j];
        address tldAddr = IBaseDOGETLDFactory(factories[i]).tldNamesAddresses(tldName);
        string memory defaultName = IBaseDOGETLD(tldAddr).defaultNames(_addr);
  ...
  function getDomainHolder(string calldata _domainName, string calldata _tld) public view returns(address) {
    ...
    for (uint256 i = 0; i < fLength;) {
      ...
      if (tldAddr != address(0) && !isTldDeprecated[tldAddr]) {
        return address(IBaseDOGETLD(tldAddr).getDomainHolder(_domainName));
      }
  ...
  function getDomainData(string calldata _domainName, string calldata _tld) public view returns(string memory) {
    ...
    for (uint256 i = 0; i < fLength;) {
      ...
      if (tldAddr != address(0) && !isTldDeprecated[tldAddr]) {
        return string(IBaseDOGETLD(tldAddr).getDomainData(_domainName));
      }
  ...
  function getDomainTokenUri(string calldata _domainName, string calldata _tld) public view returns(string memory) {
    ...
    for (uint256 i = 0; i < fLength;) {
      ...
      if (tldAddr != address(0) && !isTldDeprecated[tldAddr]) {
        (, uint256 _tokenId, , ) = IBaseDOGETLD(tldAddr).domains(_domainName);
  ...
  function getTldAddress(string calldata _tldName) public view returns(address) {
    uint256 fLength = factories.length;
    for (uint256 i = 0; i < fLength;) {
      address tldAddr = IBaseDOGETLDFactory(factories[i]).tldNamesAddresses(_tldName);
