...
contract RewardSharks is Ownable, ReentrancyGuard {
    ...
    uint256 public shiverCounter = 1;
    ...
    function userBoostAvailabitiy(uint256 _boostType) internal returns (bool) {
        uint256[] memory listOfBoosts = availableBoosts[msg.sender];
        uint256[] memory listOfNewBoosts;
        bool flag = false;
        uint256 counter = 0;
        for(uint256 i=0; i<listOfBoosts.length; i++)
        {
            if(listOfBoosts[i] == _boostType){
                ...
                listOfNewBoosts[counter] = listOfBoosts[i];
                counter = counter + 1; 
            }
        }
        if(flag)
        {
            availableBoosts[msg.sender] = listOfNewBoosts;
            return true;
        }
        else{
            return false;
    ...
    function claimRewards(uint256 _tokenId) external onlyOwner {
    ...
    function checkShiverBreak(uint256 _tokenId) public {
        Sharks memory _shark = getSharks[_tokenId];
    ...
    function updateAllRewards(uint256[] memory _rewards) external onlyOwner {
        for(uint256 i=0;i<_rewards.length;i++)
        {
            tokenIdReward[i] = _rewards[i];
        }
    }
