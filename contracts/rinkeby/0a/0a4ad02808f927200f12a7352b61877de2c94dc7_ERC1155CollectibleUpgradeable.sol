    ...
    ERC1155BurnableUpgradeable
{
    ...
    */
    ...
    mapping(uint256 => uint40) private prefixToToken;
     ...
     * @return numberOfCollectibles number of collectibles of a prefix for NFT and number of shards in case of FT
     */
    function collectiblesPerPrefix(
        uint32 _season,
        uint8 _tier,
        uint48 _type
    ) public view returns (uint40 numberOfCollectibles) {
        uint256 prefix = calculatePrefix(_season, _tier, _type);
        numberOfCollectibles = prefixToToken[prefix];
    }
     ...
     * @param _tier tier of the prefix (bronze, silver, gold, platinum or legendary)
     * @param _type type of the prefix (champion or item)
     * @param _collectibleId collectible ID for a prefix e.g sukimi for champion types and swords, wands for item types
     * @return collectibleTokenId token Id calculated using the prefix + collectible ID
     */
    function getCollectibleTokenId(
        uint32 _season,
        uint8 _tier,
        uint48 _type,
        uint40 _collectibleId
    ) public view returns (uint256 collectibleTokenId) {
        uint256 prefix = calculatePrefix(_season, _tier, _type);
        require(
            _collectibleId > 0 && _collectibleId <= prefixToToken[prefix],
            "This collectible hasn't been created yet for this prefix"
        );
        collectibleTokenId = uint256(_collectibleId) << 128;
        collectibleTokenId += prefix;
    }
