...
interface IXCOPYBAYC {
  function ownerOf(uint256 tokenId) external returns (address);
}

contract XCOPYMAYC is ERC721EnumerableLite, Ownable, PaymentSplitter {
  using Strings for uint256;

  IXCOPYBAYC private XCOPYBAYCContract;
  ...
  string public _baseTokenURI =
    "ipfs://QmcR6rFHX3fY2vQPnMeGj7QKox7dj8rrxFcisLcrWHXN7j?";

  bool public paused = true;

  mapping(uint256 => bool) public claimedXCOPYBAYC;

  address s1 = 0xA1e37bFd170B210c83587F6E9DDF32F314068C52;
  address s2 = 0x0CF777D59b87245228a55B034a259aB5018a389D;

  address[] addressList = [s1, s2];
  uint256[] shareList = [10, 90];

  constructor(address _IXCOPYBAYCAddress)
    ERC721B("XCOPYMAYC", "XMAYC")
    PaymentSplitter(addressList, shareList)
  {
    XCOPYBAYCContract = IXCOPYBAYC(_IXCOPYBAYCAddress);
  }
  ...
  function mint(uint256 _count) external payable {
    ...
    require(mintCount + _count <= 3333, "Exceeds mint supply");
  ...
  function claim(uint256[] calldata tokenIds) external {
    ...
    uint256 supply = totalSupply();
    uint256 _count = tokenIds.length;

    require(supply + _count <= MAX_SUPPLY, "Exceeds max supply");
    require(claimCount + _count <= 3333, "Exceeds claim supply");

    claimCount += _count;

    for (uint256 i = 0; i < _count; ++i) {
      require(
        XCOPYBAYCContract.ownerOf(tokenIds[i]) == msg.sender,
        "Not token owner"
      );
      require(claimedXCOPYBAYC[tokenIds[i]] == false, "Token already claimed");

      claimedXCOPYBAYC[tokenIds[i]] = true;
