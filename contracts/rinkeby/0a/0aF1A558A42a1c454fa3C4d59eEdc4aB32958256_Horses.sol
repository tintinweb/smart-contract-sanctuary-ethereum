...
contract Horses is Ownable, ERC721AQueryable {
    ...
    function claimHorse(uint256 tokenId) external payable onlyWhenNotPaused callerIsUser {
        if(_nextTokenId() > maxTokenIds) revert SupplyExceeded();
        if(_numberMinted(msg.sender) + 1 > maxPerWallet) revert WalletLimitExceeded();
        // require(msg.value >= price * quantity, "More ETH required");
        if (usedGunslingers[tokenId]) revert UsedGunslingersToken();
        if(Gunslingers.ownerOf(tokenId) != msg.sender) revert NotOwnerOfGunslinger();
        usedGunslingers[tokenId] = true;
        _mint(msg.sender, 1);
    }
