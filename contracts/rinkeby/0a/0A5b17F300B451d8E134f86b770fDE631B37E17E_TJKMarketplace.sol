...
// File: contracts/TjkMarketplace.sol
...
contract TJKMarketplace is ReentrancyGuard {
    ...
    uint256 public listingFee = 2 * 10**18;
    // uint256 public listingFee = 2 ether;
    ...
    function createMarketItem(
        ...
        require(tokenAddress.balanceOf(msg.sender) >= listingFee, "Fee must be equal to listing fee");
    ...
    function deleteMarketItem(uint256 itemId) public nonReentrant {
        ...
        require(
            IERC721(nftAddress).ownerOf(item.tokenId) == msg.sender,
            "must be the owner"
        );
        require(
            IERC721(nftAddress).getApproved(item.tokenId) ==
                address(this),
            "NFT must be approved to market"
        );
        ...
        nonReentrant
    {
        ...
        require(tokenAddress.balanceOf(msg.sender) >= price, "Your TJK Balance is not enough");
        require(
            IERC721(nftAddress).getApproved(tokenId) == address(this),
            "NFT must be approved to market"
        );
        ...
        tokenAddress.transfer(marketowner, listingFee);
        tokenAddress.transfer(seller, price);
