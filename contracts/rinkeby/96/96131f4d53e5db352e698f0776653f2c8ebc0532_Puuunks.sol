    ...
    Ownable
{
    ...
    constructor() ERC721 ("Puuunks", "Puuunks") {}
    ...
    IdeasOfMountains ideasOfMountains;
    ...
    mapping(uint256 => bool) public tokenList;
    mapping(uint256 => uint256) public tokenWidth;

    mapping(uint256 => bool) public claimedMessage;
    mapping(uint256 => bool) public claimedLockout;
    mapping(uint256 => bool) public claimedMountain;
    ...
    // ahead of the linear sequence
    // we need to find the next available unclaimed
    // there's probably a lower gas way of doing this 
    // let me know if you think of one
    function _mint() internal {
        bool found = false;
        while(found==false){
            if(tokenList[nextMint]!=true){
                found=true;
            } else{
                unchecked{
                    nextMint = nextMint + 1;
        ...
        tokenList[nextMint] = true;
        tokenWidth[nextMint] = rand(((nextMint%3)+1)*416) + 750 ;
        _safeMint(msg.sender, nextMint);
        unchecked{
            nextMint = nextMint + 1;
        }
    }
    // just mint the next available token sequentially 
    ...
    function claimTokenByPunkId(uint256 id) external nonReentrant callerIsUser {
        ...
        tokenList[id] = true;
        tokenWidth[id] = rand(((nextMint%3)+1)*416) + 750 ;
        _safeMint(msg.sender, id);
    }
    // claim a few tokens if you are a Lockout holder
    function claimTokenByLockoutId(uint256 id) external nonReentrant callerIsUser {
        require(
            balanceOf(msg.sender) + 5 <= 75,
            "you own too many"
        );
        require(
            lockout.ownerOf(id) == msg.sender,
            "not your lockout"
        );
        require(
            claimedLockout[id] == false,
            "lockout already claimed"
        );
        require(
            totalSupply() + 5 <= maxSupply,
            "Request to mint tokens exceeds supply"
        );
        claimedLockout[id] = true;
        uint256 i;
        while (i < 5) {
            _mint();
            unchecked{
    ...
    // claim a few tokens if you are a Message holder
    function claimTokenByMessageId(uint256 id) external nonReentrant callerIsUser {
        require(
            balanceOf(msg.sender) + 5  <= 75,
            "you own too many"
        );
        require(
            usrMessage.ownerOf(id) == msg.sender,
            "not your Message"
        );
        require(
            claimedMessage[id] == false,
    ...
    // claim a few tokens if you are an Ideas of Mountains holder
    function claimTokenByMountainId(uint256 id) external nonReentrant callerIsUser {
        require(
            balanceOf(msg.sender) + 5  <= 75,
            "you own too many"
        );
        require(
            ideasOfMountains.ownerOf(id) == msg.sender,
            "not your Mountain"
        );
        require(
            claimedMountain[id] == false,
            "Mountain already claimed"
        );
        require(
            totalSupply() + 5 <= maxSupply,
            "Request to mint tokens exceeds supply"
        );
        claimedMountain[id] = true;
        uint256 i;
        while (i < 5) {
            _mint();
            unchecked{
    ...
    // reset the stretch if you're the puuunk holder
    ...
    //set price for five mints price
    function authorSetMintPriceFive(uint256 newMintPrice) external onlyOwner {
        require(
            newMintPrice > 0.01 ether,
            "wrong units"
        );
        MINT_PRICE_FIVE = newMintPrice;
    }
    //set price for 69 mints price
    function authorSetMintPriceSixNine(uint256 newMintPrice) external onlyOwner {
        require(
            newMintPrice > 0.01 ether,
            "wrong units"
        );
        MINT_PRICE_SIXNINE = newMintPrice;
    }
    // generate Puuunks
    ...
    {
        // get punk attributes which come in as a string like "Male 1, Smile, Mohawk"
        ...
        uint256 newWidth = tokenWidth[tokenId];
        ...
        // replace the svg head of the punk svg with a stretched style and filled background etc
        ...
        // the new x offset so the puuunk stays centered
        parts[1] = Strings.toString((newWidth-500)/2);
        parts[2] = '" width="';
        // the new stretched width
        parts[3] = Strings.toString(newWidth);
        ...
        // all the accessories for the punk
        for(uint256 i = 1; i < attrParts.length; i++){
            trait = string.concat(trait,',{"trait_type": "Accessory","value": "', attrParts[i], '"}');
        }
        // count the attributes like OpenSea displays
        trait = string.concat(trait,',{"trait_type": "Accessory","value": "',  Strings.toString(attrParts.length-1)  , ' Attributes"}');
        // show how stretched they are
        trait = string.concat(trait,',{"trait_type": "Accessory","value": "Width ',  Strings.toString((newWidth/500)*100)  , ' %"}');
        // build the metadata and encode the image as base64
        string memory json = Base64.encode(bytes(string.concat('{"name": "Puuunk #', Strings.toString(tokenId),trait, '], "description": "',string(desc),'", "image": "data:image/svg+xml;base64,', Base64.encode(bytes(output)), '"}')));
    ...
    // register contracts
    function authorRegisterContracts(address[] calldata addr) public onlyOwner{
        lockout = Lockout(addr[0]);
        usrMessage = UsrMessage(addr[1]);
        ideasOfMountains = IdeasOfMountains(addr[2]);
        cryptoPunks = CryptoPunksMarket(addr[3]);
        cryptoPunksData = CryptoPunksData(addr[4]);

    }
    // set desc for metadata because its looong for puuunks
    function authorSetDesc(bytes calldata newDesc) public onlyOwner{
        desc = newDesc;
    }
    ...
    // why did you send shitcoins to this contract
    function authorForwardERC20s(IERC20 _token, address _to, uint256 _amount) public onlyOwner {
        require(
    ...
    // force stretch token for testing
    function authorMultiStretch(uint256 _timesToStretch, uint256 startingId, uint256 tokensToStretch) public onlyOwner {

        uint256 t = startingId;
        while(t < tokensToStretch+startingId){
            uint256 i;
            while (i < _timesToStretch) {
                if(tokenWidth[t] < maxWidth){
                    // 1.2x to 2x growth rate depending on tokenId 0th to 20th
                    // (tokenId %20)*50)+1150
                    // it's scaled up and divided by 1000 to behave like a decimal place
                    unchecked {
                        tokenWidth[t] =  (tokenWidth[t]* ((t % 20)*50+1150 ))/1000;
                    }
                }
                unchecked{
                    i++;
    ...
    // force stretch token for testing
    function authorStretch(uint256 tokenId) public onlyOwner {
