...
contract LegendaryBattleDogs is ERC721Enumerable, Ownable {
    using Strings for uint256;
    string private baseURI = "https://gateway.pinata.cloud/ipfs/QmZAL6nNHzDwfU5oFwLqHqLwHN3VmrvZyKzomkS6MaDUL9/";
    string public baseExtension = ".json";
    string public notRevealedUri;
    uint256 public preSaleCost = 0.09 ether;
    uint256 public cost = 0.12 ether;
    uint256 public cavlaryCost = 0.6 ether;
    uint256 public EliteCost = 0.7 ether;
    uint256 public LegendaryCost = 0.8 ether;
    uint256 public maxSupply = 11000;
    uint256 public commonSupply = 10000;
    uint256 public cavlarySupply = 10900;
    uint256 public EliteSupply = 10990;
    uint256 public preSaleMaxSupply = 3000;
    uint256 public maxMintAmountPresale = 10;
    uint256 public maxMintAmount = 10;
    uint256 public nftPerAddressLimitPresale = 20;
    uint256 public nftPerAddressLimit = 100;
    uint256 public preSaleDate = 1650442326;
    uint256 public preSaleEndDate = 1650443424;
    uint256 public publicSaleDate = 1650443424;
    bool public paused = false;
    bool public revealed = true;
    mapping(address => bool) whitelistedAddresses;
    mapping(address => uint256) public addressMintedBalance;

    constructor() ERC721("Legendary Battle Dogs", "LBD") {
        setNotRevealedURI("");
    }
    ...
    function publicsaleValidations(uint256 _ownerMintedCount, uint256 _mintAmount) internal {
        uint256 currentCost = getCurrentCost(_mintAmount);
        require(_ownerMintedCount + _mintAmount <= nftPerAddressLimit,"max NFT per address exceeded");
        require(msg.value >= currentCost * _mintAmount, "insufficient funds");
    ...
    function getCurrentCost(uint256 _mintamount) internal view returns (uint256) {
        uint256 supply = totalSupply();
        if(supply + _mintamount > commonSupply) {
            return cavlaryCost;
        }
        else if(supply + _mintamount > cavlarySupply) {
            return EliteCost;
        }
         else if(supply + _mintamount > EliteSupply) {
            return LegendaryCost;
        }
        else if (block.timestamp < preSaleEndDate) {
            return preSaleCost;
        } else {
            return cost;
        }
    ...
    function setCavlaryCost(uint256 _newCost) public onlyOwner {
        cavlaryCost = _newCost;
    }

     function setEliteCost(uint256 _newCost) public onlyOwner {
        EliteCost = _newCost;
    }
