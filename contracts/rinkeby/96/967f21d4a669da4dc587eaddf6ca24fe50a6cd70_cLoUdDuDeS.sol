...
contract cLoUdDuDeS is ERC721A, Ownable, Pausable, ReentrancyGuard {
    ...
    function CloudFreeMint(address Wallet, uint256 _FreeCloudsAmount) public onlyOwner {
        require(!IsFreeMintingONReverse, "Cloud FREE minting paused, more in Twitter! 0-0");
        
        uint256 AllCloudsFreeSupply = totalSupply();

        require(_FreeCloudsAmount > 0, "You can't mint 0 Clouds! LOL 0-0!");
        require(_FreeCloudsAmount < 2, "You can FREE mint only 1 Cloud! LOL 0-0!");
        require(AllCloudsFreeSupply + _FreeCloudsAmount <= FreeSupplyClouds, "You can't exceed FREE Clouds Supply! LOL 0-0!");
        require(AllCloudsFreeSupply + FreeCloudsPerAddress <= FreeSupplyClouds, "You can't exceed FREE Clouds Supply! LOL 0-0!");
        require(!freeMinted[msg.sender],"You have mint 1 FREE NFT, it's time to mint a new NFT through public mint at the low price! ;)");
        freeMinted[msg.sender] = true;
           
          _safeMint(Wallet, _FreeCloudsAmount);
        delete AllCloudsFreeSupply;
 }
