...
interface TimestampInterface {
    ...
    function setMany(uint[] memory _jobIds, uint _startIndex, uint32 _timestamp) external;
}
...
// CLAIM
    ...
    function claim(uint[][] memory _sortedIds, address _ownerAddr) public whenNotPaused {
        ...
        for (uint i = 0; i < _numCompanies; i++) {
            ...
            timestamps.setMany(_sortedIds[i], 1, uint32(block.timestamp)); // Offset index by 1 to disclude company ID
        }
