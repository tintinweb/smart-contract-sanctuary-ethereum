...
contract ProjectAndProposal{
    ...
    function setSubsequentId(uint _val) public {
        require(msg.sender == founder,"you are not founder");
        founderAndSubsequentId[founder] = _val;
        subsequentProjectId = _val;
    }

/*
    Read Function based on initial and subsequent id:
*/

    function gettotalValueForProject(uint _initialId) public view returns(uint){
        require(founderAndInitialId[founder] == _initialId, "Please check if you are passing the correct initial id");
        return totalValueForProject;
    }

    function getTenPercentBalanceOfStableCoin(uint _initialId) public view returns(uint){
        require(founderAndInitialId[founder] == _initialId, "Please check if you are passing the correct initial id");
        return TenPercentBalanceOfStableCoin;
    }

    function getTenPercentBalanceOfFounderToken(uint _initialId) public view returns(uint){
        require(founderAndInitialId[founder] == _initialId, "Please check if you are passing the correct initial id");
        return TenPercentBalanceOfFounderToken;
    }

    function gettotalDepositedStableCoinsInThePot(uint _subsId) public view returns(uint){
        require(founderAndSubsequentId[founder] == _subsId, "The validation is not matched with subsequent id");
        return totalDepositedStableCoinsInThePot;
    }

    function gettotalDepositedFounderTokenInPot(uint _subsId) public view returns(uint){
        require(founderAndSubsequentId[founder] == _subsId, "The validation is not matched with subsequent id");
        return totalDepositedFounderTokenInPot;
    }

// ends the read function

    function Validate(bool _choice, address _ad, address _contractad, uint _subsId) public returns (bool voted){
    ...
    function depositTokens(uint256 amount, bytes32 symbol,address tokenAddress, uint _initialId) external {
    ...
    function depositFoundersToken(uint256 amount, bytes32 symbol, address tokenAddress, uint _initialId) external founderAction{
    ...
    function withdrawAllStableCoinFromThePool(uint256 amount, bytes32 symbol, uint _subsId) external founderAction returns(bool condition){
        bool status = false;
        require(founderAndSubsequentId[founder] == _subsId, "The withdrawl is not matched with subsequent id");
    ...
    function withdrawAllFounderTokenFromThePool(uint256 amount, bytes32 symbol, uint _subsId) external investorAction returns(bool condition) {
        bool status = false;
        require(founderAndSubsequentId[founder] == _subsId, "The withdrawl is not matched with subsequent id");
        require(arrApprovedValidator[_subsId].length > 3,"maximum validators has not voted yet");
        if(arrApprovedValidator[_subsId].length > 3){
    ...
    function Withdraw10PercentOfStableCoin(bytes32 symbol, uint _initialId) public founderAction {
    ...
    function Withdraw10PercentOfFounderToken(bytes32 symbol, uint _initialId) public investorAction {
