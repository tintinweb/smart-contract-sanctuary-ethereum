...
import {ILoans, Loan} from "./Interfaces/ILoans.sol";
...
contract DwarfPawn is Ownable, ReentrancyGuard {
    ...
    /// @param _nftId ID of the NFT token
    ...
    /// @notice Returns a specific loans data
    /// @param _loanId ID of the loan
    function getLoan(uint256 _loanId) external view returns(Loan memory) {
        return ILoans(loansContract).loans(_loanId);
    }

    /// @notice Returns a specific listing data
    /// @param _listingId ID of the listing
    function getListing(uint256 _listingId) external view returns(Listing memory) {
        return IListings(loansContract).listings(_listingId);
    }

    function getOffer(uint256 _listingId, uint256 _offerId) external view returns(Offer memory) {
        return IListings(loansContract).offer(_listingId, _offerId);
    }
...
interface IListings {
    ...
    /// @notice Returns the data for a specific Listing
    function listings(uint256) external view returns (Listing memory);

    /// @notice Returns the data for a specific Offer
    function offer(uint256, uint256) external view returns (Offer memory);
