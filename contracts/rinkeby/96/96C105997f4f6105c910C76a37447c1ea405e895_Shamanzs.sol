...
contract Shamanzs is ERC721, Ownable, PaymentSplitter, ReentrancyGuard {
    ...
    uint256 public DUTCH_AUCTION_SUPPLY = 20;
    ...
    mapping(address => minted[]) public ADDRESS_PAID;
    ...
    function claim(address _to, uint256 _mintAmount, bytes32 _hash, bytes memory _signature) external payable {
        require(!PAUSED, "Contract paused");
        require(CLAIMED[_to] == 0, "Claimed all");
        require(_mintAmount > 0, "No mint amount set");
        require(block.timestamp >= START_AUCTION_AT, "Dutch Auction not started");
        require(totalSupply() + _mintAmount <= DUTCH_AUCTION_SUPPLY, "Auction Ended");
        require(_mintAmount <= MAX_MINT_AMOUNT, "Mint amount exceeded");
        require(ECDSA.recover(_hash, _signature) == PUBLIC_MINTING_ADDRESS, "Signature not match");
    ...
    function whitelistedClaim(address _to, bytes32[] calldata _merkleProof, uint256 _mintAmount) external payable {
        ...
        require(MerkleProof.verify(_merkleProof, MERKLE_ROOT, keccak256(abi.encodePacked(_to))), "Not Whitelisted nor Allowlisted");
        require(_mintAmount > 0, "No mint amount set");
        ...
        require(CLAIMED[_to] + _mintAmount <= mintAmount, "Claimed all");
