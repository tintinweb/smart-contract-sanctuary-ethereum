...
contract OVRRegistrar is ERC721Full, AccessControl {
     ...
     */
    modifier _requireBalance(address _user) {
        require(acceptedToken.balanceOf(_user) >= price, "Insufficient funds");
        require(
     ...
     */
    modifier requireNameValid(string memory _name) {
        ...
        for (uint256 i = 0; i < tempName.length; i++) {
            require(
                _isLetter(tempName[i]) || _isNumber(tempName[i]),
                "Invalid Character"
            );
        }
        _;
    ...
    constructor(
        ...
        IERC20 _token,
        uint256 _price
    ) ERC721Full("OVR Registrar", "OVERENS") {
     ...
     * @param _price - price in wei  (e.g. 1 ether = 10^18 wei)
     */
        ...
        requireNameValid(_subdomain)
    {
        ...
        require(
            acceptedToken.transferFrom(_msgSender(), treasury, price),
    ...
    function changeNotice(uint256 _tokenId, string memory _value) public {
        require(
            _isApprovedOrOwner(_msgSender(), _tokenId),
            "Only an authorized account can change the subdomain settings"
        );

        IResolver resolver = IResolver(registry.resolver(bytes32(_tokenId)));
        resolver.setText(
            bytes32(_tokenId),
            "notice",
            _value
    ...
    function reclaimAfterTransfer(uint256 _tokenId, address _owner) internal {
        // Check if the sender is authorized to manage the subdomain
        require(
            _isApprovedOrOwner(_owner, _tokenId),
            "Only an authorized account can change the subdomain settings"
        );
        registry.setSubnodeOwner(domainNameHash, bytes32(_tokenId), _owner);

        //ADD TIMESTAMP
        emit ReclaimedAfterTransfer(_owner, _tokenId);
    }
    ...
    function _afterTokenTransfer(
        ...
        // Re-claim to update the owner at the ENS Registry
        reclaimAfterTransfer(tokenId, to);
    }
    ...
    function supportsInterface(bytes4 interfaceId)
        public
        view
        override(ERC721Full, AccessControl)
        returns (bool)
    {
