...
contract IntreeCards is ERC721A, ERC2981, Ownable, ReentrancyGuard {
        ...
        mintCompliance(_mintAmount) 
    {
        ...
        require(_mintAmount + whitelistTotalSupply <= currentMaxSupply(true), "Exceeds allowed max supply for this phase!"); 
        ...
        mintCompliance(_mintAmount)
    {
        ...
        require(_mintAmount + totalSupply() <= currentMaxSupply(false), "Exceeds allowed max supply for this phase!"); 
    ...
    function currentMaxSupply(bool presale) public view returns (uint256) {
        // param presale: Whitelist sale or public sale.
        // * If presale is true, then we are in presale phase.
        // * If presale is false, then we are in public sale phase.
        uint256 _supply;
        uint256[] memory _mintTypeMaxSupply;
        if (presale) {
            // WL phase supply mapping
            _mintTypeMaxSupply = WL_SUPPLY_MAX;
        } else {
            // Public phase supply mapping
            _mintTypeMaxSupply = SUPPLY_MAX;
        }
        uint i;
    ...
    function walletStatus(address _address) public view returns (uint256[] memory) {
        ...
        while (ownedTokenIndex < ownerTokenCount && currentTokenId <= currentMaxSupply(false)) {
