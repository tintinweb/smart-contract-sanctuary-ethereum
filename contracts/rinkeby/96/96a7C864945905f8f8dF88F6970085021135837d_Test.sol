...
contract Test{
    ...
    function uniswapTestingTokenforTokens(uint256 price,uint256 deadline, address[] calldata path) external view returns(uint256[] memory){
        uint[] memory amount = UniswapV2Library.getAmountsOut(factory, price, path);
    //     IERC20(tokenAddress1).transferFrom(msg.sender ,address(this),price);
    //     IERC20(tokenAddress1).approve(UNISWAP_ROUTER_ADDRESS,price);
    ...
    function pair(address factory, address tokenA, address tokenB) public view returns (address){
        address pa = UniswapV2Library.pairFor(factory, tokenA, tokenB);
        return pa;
    }   
    function reserves(address tokenA, address tokenB) external view returns(uint reserveA, uint reserveB){
        (reserveA,reserveB) = UniswapV2Library.getReserves(factory, tokenA, tokenB);
    }
    function getRes(address factory,address tokenA,address tokenB) external view returns(uint reserve0, uint reserve1){
        (reserve0, reserve1,) = IUniswapV2Pair(pair(factory, tokenA, tokenB)).getReserves();
    }
    function uniswapTestingTokenforTokens2(uint256 price, uint256 price2, uint256 deadline, address[] calldata path) external returns(uint256){
        // uint[] memory amount = UniswapV2Library.getAmountsOut(factory, price, path);
        IERC20(path[0]).transferFrom(msg.sender ,address(this),price);
        IERC20(path[0]).approve(UNISWAP_ROUTER_ADDRESS,price);
    ...
    function uniswapTestingTokensforEth(uint256 price, uint256 deadline, address tokenAddress) external returns(uint256){
        address[] memory path = new address[](2);
      path[0] = tokenAddress;
      path[1] = _WETH;
      uint[] memory amount = UniswapV2Library.getAmountsOut(factory, price, path);
    //   require(amount[1] >= price2, "MarketPlace:Amount entered is lower than the price");
