...
contract Land is ERC721, ERC2981, ReentrancyGuard, Ownable, Pausable {
    ...
    // **NOTE: MAX_SUPPLY value should be change to 10000 before deploying to mainnet
    uint256 public constant MAX_SUPPLY = 20;
    uint256 public totalMinted = 0;
    // **NOTE: value should be replace before mainnet deployment
    ...
    * @dev Field is declared for opensea contract-level metadata see https://docs.opensea.io/docs/contract-level-metadata
    */
    ...
    modifier isCallerEOA() {
        require(tx.origin == msg.sender, "Not allowed");
        _;
    }

    modifier isEtherAmountValid(uint8 _nftAmount) {
        require(msg.value >= (mintPrice * _nftAmount));
        _;
    }
    ...
    function mint(
        ...
        whenNotPaused
        isCallerEOA
        isTotalMintedExceedsMaxSupply(1)
        isValidAmount(1)
        isNFTBalanceExceedsMaxMintPerAddress(msg.sender, 1)
        ...
        isWhitelisted(_merkleProof)
    {
        for(uint8 i;i < _tokenIds.length; i++) {
            _mintNFT(msg.sender,_tokenIds[i],_tokenURIs[i]);
        }
    }
