...
contract Juice is WizardsERC1155 {
    ...
    function _merge(
        ...
        unchecked {
            ...
            if (mintQty + totalMinted(tierId) > maxSupply(tierId + 1)) {
                revert MaxJuiceSupplyReached();
            }

            // for each higher level, determine if user could receive the next highest tier
            for (uint256 i = tierId + 1; i < _juiceBurnAmount.length - 1; i++) {
                uint256 tb = _juiceBurnAmount[i];

                if (mintQty >= tb) {
                    uint256 tmpToMint = mintQty / tb;
                    uint256 remaining = mintQty - (tmpToMint * tb);

                    // mint the remainign balance resulting from the left over
                    if (remaining > 0) {
                        _mint(account, i, remaining, "");
                    }
                    ...
                    // verify new amount to be minted is not greater than allowable amount
                    if (mintQty + totalMinted(tierId) > maxSupply(tierId)) {
                        revert MaxJuiceSupplyReached();
                    }
                } else {
    ...
    Administration
{
    ...
    mapping(uint256 => uint256) private _issuanceCounter;
    mapping(uint256 => uint256) private _burnCounter;
