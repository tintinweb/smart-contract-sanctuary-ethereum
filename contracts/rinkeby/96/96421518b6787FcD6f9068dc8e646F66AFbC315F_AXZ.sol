...
// File: contracts/nft-sale/AXZ.sol
...
error WaitingSlotIdOutOfRange();
error OnlyWaitingSlotOwnerAllowed();

contract AXZ is ERC721A, ERC721AQueryable, Ownable, ReentrancyGuard {
    ...
    uint256 private _auctionMaxSupply = 8000;
    uint256 private _marketingSupply = 500;
    uint256 private _presalePrice = 0.25 ether;
    uint256 private _publicPrice;

    address private _verifier;
    string private _baseTokenURI;
    IWaitingSlot waitingSlotManager;
    uint256 private _maxAllowedWaitingSlotIndex;
    ...
    constructor(address verifier, address waitingSlot) ERC721A("AlienxZodiac", "AXZ") {
        _verifier = verifier;
        waitingSlotManager = IWaitingSlot(waitingSlot);
    }
    ...
    function getAuctionPrice() public view returns (uint256) {
        ...
        if (block.timestamp - _auctionStart >= AUCTION_PRICE_CURVE_LENGTH) {
    ...
    function auctionMint(uint256 quantity, uint256 waitingSlotID)
        external
        payable
        nonReentrant
        onlyEOA
    {
        ...
        _safeMint(msg.sender, quantity, waitingSlotID);
    ...
    function presaleMint(uint256 quantity, bytes calldata sig)
        external
        payable
        nonReentrant
        onlyEOA
    {
        ...
        if (getPresalePrice() != msg.value) revert IncorrectPayment();
        if (quantity > MAX_MINTS_PER_TX) revert AmountExceedsTransactionLimit();
    ...
    function publicSaleMint(uint256 quantity, uint256 waitingSlotID)
        external
        payable
        nonReentrant
        onlyEOA
    {
        ...
        if (getPublicSalePrice() * quantity != msg.value)
    ...
    struct MintSummary {
        ...
        uint256 maxAllowedWaitingSlotIndex;
    }

    function getMintSummary()
        external
        view
        returns (MintSummary memory summary)
    {
        ...
        summary = MintSummary({
            ...
            presalePrice: getPresalePrice(),
            publicSalePrice: getPublicSalePrice(),
            maxSupply: MAX_SUPPLY,
            totalSupply: totalSupply(),
            auctionSupply: _auctionMaxSupply,
            blockTime: block.timestamp,
            maxAllowedWaitingSlotIndex: _maxAllowedWaitingSlotIndex
        });
    }
    ...
    // WAITING SLOT
    modifier onlyWaitingSlotInOpenRange(uint256 waitingSlotTokenID) {
        if (waitingSlotTokenID > _maxAllowedWaitingSlotIndex)
            revert WaitingSlotIdOutOfRange();
        _;
    }

    modifier onlyWaitingSlotOwner(uint256 waitingSlotTokenID) {
        //Should we need it?
        if (waitingSlotManager.ownerOf(waitingSlotTokenID) != tx.origin)
            revert OnlyWaitingSlotOwnerAllowed();
        _;
    }

    function setMaxAllowedWaitingSlotIndex(uint256 tokenIndex)
        external
        onlyOwner
    {
        _maxAllowedWaitingSlotIndex = tokenIndex;
    }

    function _safeMint(
        address to,
        uint256 quantity,
        uint256 waitingSlotTokenID
    )
        internal
        onlyWaitingSlotInOpenRange(waitingSlotTokenID)
        onlyWaitingSlotOwner(waitingSlotTokenID)
    {
        bool success = waitingSlotManager.setUsed(waitingSlotTokenID);
        if (success) {
            _safeMint(to, quantity);
        }
    }
