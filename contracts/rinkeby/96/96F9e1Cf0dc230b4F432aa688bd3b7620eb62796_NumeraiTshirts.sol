...
//  EnumerableMap.UintToAddressMap private _tokenOwners;
  ...
  /// @notice Redeems a voucher for a NFT and returns the tokenId
  function bulkRedeem(Voucher[] calldata vouchers) external returns (uint256) {
    ...
    for(uint i = 0; i < vouchers.length; i++) {
      ...
      // Mint the NFT to msg.sender. Will revert if tokenId is already owned!
      _mint(msg.sender, vouchers[i].tokenId);
    }

    // TODO: return tokenids
    return 1;
  }
  ...
  function bulkMintToAddress(uint256[] calldata  tokenIds, address[] calldata recipients) external onlyOwner returns (bool) {
