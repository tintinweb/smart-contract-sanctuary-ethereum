...
contract CosmoGangStaking is Ownable, ReentrancyGuard {
    ...
    uint256 private rewardsPerHour = 2083000000000000; // = 0.20830
    mapping(uint256 => uint256) public tokenIdRewardsPerHour;
    ...
    function _withdraw(uint256[] calldata _tokenIds, bool calledBySender, address receiver)
        private nonReentrant
    {
        ...
        uint256 len = _tokenIds.length;
        for (uint256 i; i < len; ++i) {
            require((calledBySender && stakerAddress[_tokenIds[i]] == receiver) ||
                !calledBySender, "Can't withdraw staking amount of token you don't own!");
            stakerAddress[_tokenIds[i]] = address(0);
            nftCollection.transferFrom(address(this), receiver, _tokenIds[i]);
        }
    ...
    // Set the rewardsPerHour variable
    function setTokenIdRewardsPerHour(uint256 tokenId, uint256 _newValue)
        public onlyApproved
    {
         tokenIdRewardsPerHour[tokenId] = _newValue;
    }
