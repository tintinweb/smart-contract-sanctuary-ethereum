...
contract Lv1Hunter is ERC721A, Payable {
    using Strings for uint256;

    //NOTE Token values incremented for gas efficiency
    uint256 private maxSalePlusOne = 3334;
    uint256 public tokenPriceOGSale = 0.01 ether;
    uint256 public tokenPricePresale = 0.02 ether;
    uint256 public tokenPricePublicSale = 0.03 ether;

    uint256 public ogSaleAllowancePlusOne = 4;
    ...
    mapping(uint => mapping(address => uint)) public mintsPerAddress;
    ...
    constructor() ERC721A("Lv 1. Hunter", "LVL") {
        placeholderURI = "ipfs://...";

        merkleRootOG = 0xa984514abfdccdac96248e8d10633ca5af864decaf82818369af17381f952123; //0x should be prepended
        merkleRootPresale = 0x935323dfdc2225bd42c05cdcbeb7d4502e6a24cf363845ee8538427fca3329db; //0x should be prepended
        

        // 5% royalties
        _setRoyalties(recipient, 350);
    }
    ...
    {
        require(mintsPerAddress[3][msg.sender] + quantity < publicSaleAllowancePlusOne, "Exceeds allowance!");
        _safeMint(msg.sender, quantity);
        mintsPerAddress[3][msg.sender] += quantity;
    }
    ...
    {
        require(mintsPerAddress[2][msg.sender] + quantity < presaleAllowancePlusOne, "Exceeds allowance!");
        bytes32 leaf = keccak256(abi.encodePacked(msg.sender));
        require(MerkleProof.verify(proof, merkleRootPresale, leaf), "Not a valid proof!");
        _safeMint(msg.sender, quantity);
        mintsPerAddress[2][msg.sender] += quantity;
    }
     ...
     * @param proof The Merkle proof used to validate the leaf is in the root.
     */
    function mintOGSale(uint256 quantity, bytes32[] calldata proof)
        external
        payable
        noBots
        isContractState(ContractState.PRESALE)
        withinMintLimit(quantity)
        correctValue(tokenPriceOGSale * quantity)
    {
        require(mintsPerAddress[1][msg.sender] + quantity < ogSaleAllowancePlusOne, "Exceeds allowance!");
        bytes32 leaf = keccak256(abi.encodePacked(msg.sender));
        require(MerkleProof.verify(proof, merkleRootOG, leaf), "Not a valid proof!");
        _safeMint(msg.sender, quantity);
        mintsPerAddress[1][msg.sender] += quantity;
    }

    /**
     * Mint tokens from OG and Presale simultaneously.
     * @notice This function is only available to those on og and whitelist
     * @param quantity The number of tokens to mint.
     */
    function mintPreAndOG(uint256 quantity, bytes32[] calldata proofPresale, bytes32[] calldata proofOG)
        external
        payable
        noBots
        isContractState(ContractState.PRESALE)
        withinMintLimit(quantity)
    {
        require(mintsPerAddress[1][msg.sender] + mintsPerAddress[2][msg.sender] + quantity < ogSaleAllowancePlusOne + presaleAllowancePlusOne - 1, "Exceeds allowance!");
        require(mintsPerAddress[1][msg.sender] < ogSaleAllowancePlusOne - 1, "Cannot Mint from OG Sale!");
        require(mintsPerAddress[2][msg.sender] < presaleAllowancePlusOne - 1, "Cannot Mint from  Presale!");
        bytes32 leaf = keccak256(abi.encodePacked(msg.sender));
        require(MerkleProof.verify(proofPresale, merkleRootPresale, leaf), "Not a valid proof for Presale!");
        require(MerkleProof.verify(proofOG, merkleRootOG, leaf), "Not a valid proof for OG!");

        uint amountToMintInOG = ogSaleAllowancePlusOne - mintsPerAddress[1][msg.sender] - 1; 
        require(msg.value == (tokenPriceOGSale * amountToMintInOG ) + (tokenPricePresale * (quantity - amountToMintInOG)), "Ether value sent is not correct");

        _safeMint(msg.sender, quantity);
        mintsPerAddress[1][msg.sender] += amountToMintInOG;
        mintsPerAddress[2][msg.sender] += quantity - amountToMintInOG;
    }
     ...
     */
    function getMintsPerAddress(uint sale, address minter) public view returns(uint) {
        return mintsPerAddress[sale][minter];
    }
}