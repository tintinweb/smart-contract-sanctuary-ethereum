 ...
 * sale.
 */
contract BowlineFiatToken is MintpassValidator, ERC721A, ERC2981, Ownable {
    ...
    uint256 public freeMintLimitPerWallet = 1;
    uint256 public publicLimitPerWallet = 10;
    mapping(address => uint256) public boughtFreemintAmounts;
    ...
    // Limits
    int256 public allowlistLimit = 250;
    int256 public freeMintLimit = 150;

    // Minted Amounts
    int256 public allowlistMinted = 0;
    int256 public freeMinted = 0;
    ...
    function mint(address minter, uint256 quantity) public payable {
        require(
            ...
            "BowlineFiatToken: Public Minting is not Enabled"
        );

        require(
            msg.value >= (mintPrice() * quantity * 199) / 200, // 0.5% treshold due to conversion rate swings
            "BowlineFiatToken: Insufficient Amount"
        );

        require(
            TOKEN_LIMIT >= _nextTokenId(),
            "BowlineFiatToken: Token Limit reached"
        );
    ...
    function freeMint(address minter) public payable {
        ...
        require(
            boughtFreemintAmounts[minter] + 1 <= freeMintLimitPerWallet,
            "BowlineFiatToken: Maximum Amount per Wallet reached"
        );

        require(
            freeMintLimit >= freeMinted,
            "BowlineFiatToken: No Free Mints left"
        );
        ...
        if (freeMinted >= freeMintLimit) {
            freeMintEnabled = false;
        }

        boughtFreemintAmounts[minter] =
            boughtFreemintAmounts[minter] +
            1;
    ...
    function allowlistMint(
        ...
        require(
            ...
            "BowlineFiatToken: Allowlist Minting is not Enabled"
        );
        ...
        require(
            TOKEN_LIMIT >= _nextTokenId(),
            "BowlineFiatToken: All Tokens minted"
        );

        require(
            boughtFreemintAmounts[mintpass.wallet] + quantity <=
                freeMintLimitPerWallet,
            "BowlineFiatToken: Maximum Amount per Wallet reached"
        );

        require(
            allowlistLimit >= allowlistMinted,
            "BowlineFiatToken: No Allowlist Mints left"
        );

        validateMintpass(mintpass, mintpassSignature);
        _mint(mintpass.wallet, quantity);
        allowlistMinted = allowlistMinted + 1;

        // autoclose allowlist minting on limit
        if (allowlistMinted >= allowlistLimit) {
            allowlistMintEnabled = false;
        }

        boughtFreemintAmounts[mintpass.wallet] =
            boughtFreemintAmounts[mintpass.wallet] +
            quantity;
    }
    ...
    function reduceTokenLimit(uint256 tokenLimit) public virtual onlyOwner {
        require(
            ...
            "BowlineFiatToken: New Limit must be below current Limit."
        );
        require(
            tokenLimit >= _nextTokenId(),
            "BowlineFiatToken: New Limit must be higher than current Token Supply"
        );
        TOKEN_LIMIT = tokenLimit;
    }
...
abstract contract MintpassValidator is EIP712 {
    constructor() EIP712("BowlineFiatToken", "1") {}
