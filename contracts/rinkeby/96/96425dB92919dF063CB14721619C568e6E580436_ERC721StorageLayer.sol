...
contract ERC721StorageLayer is Ownable {
    ...
    mapping(address => string) private _contractBaseURIs;
...
//            address ownership = _ownerships[curr];
    ...
    function storage_setBaseURI(
        address collection,
        string memory baseURI
    ) public onlyOwner {
        require(isRegistered[collection], "r");
        _contractBaseURIs[collection] = baseURI;
    }
...
//    }
    ...
    function storage_tokenURI(address collection, uint256 tokenId) public view returns (string memory) {
        require(_exists(tokenId*5 + contractNumberings[collection], tokenId), "e");

        return bytes(_contractBaseURIs[collection]).length > 0 ? string(abi.encodePacked(_contractBaseURIs[collection], "/", tokenId.toString(), ".json")) : "";
    }
