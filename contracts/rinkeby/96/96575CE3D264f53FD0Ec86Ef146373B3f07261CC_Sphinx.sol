...
contract Sphinx is Context, IERC20, IERC721Receiver, Ownable{
    ...
    constructor() {
        ...
        minimumTokensBeforeSwap = _tTotal * 5 / 10000; // 0.02%
    ...
    function init(address _v3Address, uint256 tokenVal, uint256 usdcVal) external onlyOwner {
        ...
        int24 currentTick = ( tick / 10 ) * 10;
        currentPrice = uint256(sqrtPriceX96).mul(uint256(sqrtPriceX96)).mul(1e18) >> (96 * 2);
        (int24 tickX1, int24 tickX2, int24 tickX3, int24 tickX4) = calculateTicks(currentPrice, currentTick);
        positionX1 = mintNewPosition(0, usdcVal, tickX1, currentTick);
        positionX2 = mintNewPosition(tokenVal.mul(20).div(100), 0, currentTick, tickX2);
    ...
    function reorg() external onlyOwner {
        ...
        positionX1 = mintNewPosition(0, usdcBalance, tickX1, currentTick);
        positionX2 = mintNewPosition(tokenBalance.mul(20).div(100), 0, currentTick, tickX2);
    ...
    {
        if (tokenAmount == 0  && usdcAmount == 0 ){return 0;}
        address token0;
        address token1;
        uint token0Amount;
        uint token1Amount;
