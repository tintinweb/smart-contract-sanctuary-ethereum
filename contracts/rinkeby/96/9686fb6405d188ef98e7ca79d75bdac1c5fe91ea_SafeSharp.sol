...
contract SafeSharp is Ownable {
    ...
    SurvivorBet[] public survivorSmallBets;
    ...
    function pickSurvivorWinnerSmall(uint256[] memory correctPicks) external onlyOwner returns (SurvivorBet[] memory){
        ...
        for (uint256 i; i<survivorBetSmall.length; i++) {
            ...
            if (thisPickersCount > highestRightCount) {
                delete survivorSmallBets;
                survivorSmallBets.push(survivorBetSmall[i]);
