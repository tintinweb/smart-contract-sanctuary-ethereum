...
contract SlateDAO is ERC721, ReentrancyGuard, Ownable, EIP712Whitelisting {
  ...
  constructor (
    ...
    MAX_EXCLUSIVE_SUPPLY = 2727;
    MAX_STANDARD_RESERVED_SUPPLY = 636;
    MAX_PREMIUM_RESERVED_SUPPLY = 272;
    MAX_EXCLUSIVE_RESERVED_SUPPLY = 272;
    MAX_SUPPLY = MAX_STANDARD_SUPPLY + MAX_PREMIUM_SUPPLY + MAX_EXCLUSIVE_SUPPLY;

    MAX_STANDARD_WHITELIST_SUPPLY = 3181;
    MAX_PREMIUM_WHITELIST_SUPPLY = 1363;
    MAX_EXCLUSIVE_WHITELIST_SUPPLY = 1363;
  }
  ...
  function setExclusivePrice(uint256 price) external onlyOwner {
    EXCLUSIVE_PRICE = price;
  }

  function setExclusiveLimitPerWallet(uint256 maxPerWallet) external onlyOwner {
    MAX_EXCLUSIVE_PER_WALLET = maxPerWallet;
  }
  ...
  function setMaxExclusiveWhitelistSupply(uint256 maxSupply) external onlyOwner {
    MAX_EXCLUSIVE_WHITELIST_SUPPLY = maxSupply;
  }
  ...
  mapping(address => uint256) private exclusiveMintCountMap;
  ...
  function updatePremiumMintCount(address minter, uint256 count) private {
   ...
   function allowedExclusiveMintCount(address minter) public view returns (uint256) {
    return MAX_EXCLUSIVE_PER_WALLET - exclusiveMintCountMap[minter];
  }

  function updateExclusiveMintCount(address minter, uint256 count) private {
    exclusiveMintCountMap[minter] += count;
  }
  ...
  Counters.Counter private exclusiveSupplyCounter;
  Counters.Counter private exclusiveReservedSupplyCounter;
  Counters.Counter private standardWhitelistMintCounter;
  Counters.Counter private premiumWhitelistMintCounter;
  Counters.Counter private exclusiveWhitelistMintCounter;
  ...
  function totalExclusiveSupply() public view returns (uint256) {
    return exclusiveSupplyCounter.current();
  }

  function totalExclusiveReservedSupply() public view returns (uint256) {
    return exclusiveReservedSupplyCounter.current();
  }
  ...
  function totalStandardWhitelistMints() public view returns (uint256) {
   ...
   function totalExclusiveWhitelistMints() public view returns (uint256) {
    return exclusiveWhitelistMintCounter.current();
  }
  ...
  function mintExclusive(uint256 count) public payable nonReentrant {
    require(saleIsActive, "Sale not active");
    require(totalExclusiveSupply() + count - 1 < MAX_EXCLUSIVE_SUPPLY - MAX_EXCLUSIVE_RESERVED_SUPPLY, "Exceeds max supply");
    require(count - 1 < MAX_MULTIMINT, "Trying to mint too many at a time");
    require(msg.value >= EXCLUSIVE_PRICE , "Insufficient payment");

    if (allowedExclusiveMintCount(_msgSender()) > 0) {
      updateExclusiveMintCount(_msgSender(), count);
    } else {
      revert("Minting limit exceeded");
    }

    for (uint256 i = 0; i < count; i++) {
      exclusiveSupplyCounter.increment();
      _safeMint(_msgSender(), MAX_STANDARD_SUPPLY + MAX_PREMIUM_SUPPLY + MAX_EXCLUSIVE_RESERVED_SUPPLY + totalExclusiveSupply());
    }
  ...
  function mintExclusiveReserved(uint256 count) external onlyOwner{
    require(totalExclusiveReservedSupply() + count - 1 < MAX_EXCLUSIVE_RESERVED_SUPPLY, "Exceeds max supply");

    for (uint256 i = 0; i < count; i++) {
      exclusiveReservedSupplyCounter.increment();
      _safeMint(_msgSender(), MAX_STANDARD_SUPPLY + MAX_PREMIUM_SUPPLY + totalExclusiveReservedSupply());
    }
  }

  function mintExclusiveReservedToAddress(uint256 count, address account) external onlyOwner{
    require(totalExclusiveReservedSupply() + count - 1 < MAX_EXCLUSIVE_RESERVED_SUPPLY, "Exceeds max supply");

    for (uint256 i = 0; i < count; i++) {
      exclusiveReservedSupplyCounter.increment();
      _safeMint(account, MAX_STANDARD_SUPPLY + MAX_PREMIUM_SUPPLY + totalExclusiveReservedSupply());
    }
  }
  ...
  function mintExclusiveWhitelist(uint256 count, bytes calldata signature) public payable requiresWhitelist(signature) nonReentrant {
    require(whitelistSaleIsActive, "Sale not active");
    require(totalExclusiveWhitelistMints() + count - 1 < MAX_EXCLUSIVE_WHITELIST_SUPPLY, "Exceeds whitelist supply");
    require(totalExclusiveSupply() < MAX_EXCLUSIVE_SUPPLY - MAX_EXCLUSIVE_RESERVED_SUPPLY + count - 1, "Exceeds max supply");
    require(count - 1 < MAX_MULTIMINT, "Trying to mint too many at a time");
    require(msg.value >= EXCLUSIVE_PRICE * count, "Insufficient payment");
    ...
    for (uint256 i = 0; i < count; i++) {
      exclusiveSupplyCounter.increment();
      exclusiveWhitelistMintCounter.increment();
      _safeMint(_msgSender(), MAX_STANDARD_SUPPLY + MAX_PREMIUM_SUPPLY + MAX_EXCLUSIVE_RESERVED_SUPPLY + totalExclusiveSupply());
    }
