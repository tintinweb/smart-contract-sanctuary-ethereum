    ...
    PausableUpgradeable
{
    ...
    IERC20Upgradeable public weth;    // Locking's reward amount produced per distribution time.
    ...
    function migrationLock(address[] memory _users, uint256[] memory _amounts) external onlyRole(DEFAULT_ADMIN_ROLE){
        ...
        for(uint i = 0; i < _users.length; i++){
            ...
            locker.amount = locker.amount + _amounts[i];
            locker.depositUnlockTime = block.timestamp + depositLockDuration;
    ...
    function unlock(uint256 _amount) public {
        ...
        require(
            ...
            "Locking: Not enough lp tokens to unlock"
        );
    ...
    function unlockAll() public {
        ...
        require(amount > 0, "Locking: Not enough lp tokens to unlock");
     ...
     * @return amount of vote/locked tokens
     */
    function balanceOfInAlluo(address _address)
        external
        view
        returns (uint256)
    {
        uint256 userBalancerLp = _lockers[_address].amount;
        uint256 alluoPerLp = FakeBalancer(alluoBalancerLp).alluoPerLp();
        return 
        userBalancerLp * alluoPerLp / 100;
    }

    /**
     * @dev Returns balance of the specified locker
     * @param _amount Locker's address
     * @return amount of vote/locked tokens
     */
