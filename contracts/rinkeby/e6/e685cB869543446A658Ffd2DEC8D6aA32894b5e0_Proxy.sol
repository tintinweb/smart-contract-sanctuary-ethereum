...
contract Proxy is AccessControl, Ownable, ReentrancyGuard, Structs {
    ...
    function getActiveSalePhase() public view returns (Phase memory) {
        uint256 _now = timeNow();
        uint8 _currentPhase = 0;

        // not at any current phase
    ...
    function getPrice() public view returns (uint256) {
        uint256 _now = timeNow();
        Phase memory _phase = getActiveSalePhase();
        uint256 _price = _phase.startPrice;

        if (_phase.intervalMinutes > 0 && _phase.priceDecrease > 0) {
            _price -= (
                ((_now - _phase.startTime) / 60) / _phase.intervalMinutes
            ) * _phase.priceDecrease;
        }

        // If price floor reached (endPrice) should be that value:
        if (_phase.endPrice > 0 && _price < _phase.endPrice) {
    ...
    function reveal(uint256 _cryochamberTokenId) public nonReentrant {
        ...
        require(timeNow() >= revealTime, "It's not reveal time just yet!");
