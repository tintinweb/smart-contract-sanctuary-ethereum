...
contract ElementExHelper {
    ...
    function isERC721OrderNonceFilled(address user, uint256 nonce) public view returns (bool filled) {
        uint256 bitVector = IERC721OrdersFeature(ELEMENT_EX).getERC721OrderStatusBitVector(user, uint248(nonce >> 8));
        uint256 flag = 1 << (nonce & 0xff);
        return (bitVector & flag) != 0;
    }

    function getHashNonce(address maker) public view returns (uint256) {
        try IERC721OrdersFeature(ELEMENT_EX).getHashNonce(maker) returns (uint256 hashNonce) {
            return hashNonce;
        } catch {
        }
        return 0;
