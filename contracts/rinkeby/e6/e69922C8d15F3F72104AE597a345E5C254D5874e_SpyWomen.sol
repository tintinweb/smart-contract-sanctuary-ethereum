...
contract SpyWomen is ERC721, Ownable, ReentrancyGuard {
  ...
  constructor() ERC721("Spy Women", "SW") {
    setHiddenMetadataURI("ipfs://QmTUima1RVTghTLvCooFKmRqgC79Rw1kJjcrbMZJqCSHv1/hiddenMetadata1.json");
  ...
  function mint() public payable {
    ...
    require((totalMinted + maxPerMint) <= maxSupplyPrivate, "Not enough women.");
    require(msg.value >= price, "Insufficient balance to mint.");
    require(
      mintedWallets[msg.sender] < maxPerWallet,
      "The number of minted women will exceed the maximum amount of allowed per wallet."
    );
  ...
  function reservewomen(uint256 _count) public onlyOwner {
    uint256 totalMinted = totalSupply();

    require((totalMinted + _count) <= maxSupplyTotal, "Not enough Women.");
