...
contract ERC721ABreeding is ERC721A, Ownable {
    ...
    function canBreed(address _address, uint[] memory _tokenIds, uint _type) public view returns (bool[] memory) {
        ParentContract parentContract = _type == 1 ? dadContract : momContract;
        bool[] memory tokens = new bool[](_tokenIds.length);
        for (uint i = 0; i < _tokenIds.length; i++) {
            require(parentContract.ownerOf(_tokenIds[i]) == _address, "Not owner");
            tokens[i] = _type == 1 ? isDad[_tokenIds[i]] : isMom[_tokenIds[i]];
        }
        return tokens;
    }
