...
/// @notice The Master ETH staking contract. It allows for a fixed ETH
/// rewards distributed evenly across all stakers per second.
contract EthAndPrimeRewards is PrimeRewards {
    ...
    uint256 private constant ethAmountPerSecondPrecision = 1e18; // ethAmountPerSecond is carried around with extra precision to reduce rounding errors
    ...
    /// @param _tokenIds TokenIds for a ParallelAlpha ERC1155 paraset.
        ...
        onlyOwner
    {
        ...
        ethPoolInfo.push(
            EthPoolInfo({
                ...
                lastRewardTimestamp: ethStartTimestamp
            })
        );
    ...
    function setEthPerSecond(
        ...
        require(
            address(this).balance + msg.value >= _ethAmount,
            "Insufficient ETH"
        );
        ...
        ethAmountPerSecond =
            (_ethAmount * ethAmountPerSecondPrecision) /
    ...
    function setEthEndTimestamp(uint256 _ethEndTimestamp) external onlyOwner {
        ...
        require(ethEndTimestamp < _ethEndTimestamp, "invalid end timestamp");
        for (uint256 i = 0; i < ethPoolInfo.length; ++i) {
            updateEthPool(i);
        }
        // Update ethPool stuffs
        ethEndTimestamp = _ethEndTimestamp;
        ethAmountPerSecond =
    ...
    function addEthAmount(uint256 _ethAmount) external payable onlyOwner {
        require(
            ethStartTimestamp < block.timestamp &&
        ...
        ethAmountPerSecond =
            (ethAmount * ethAmountPerSecondPrecision) /
            (ethEndTimestamp - block.timestamp);
        emit RewardIncrease(_ethAmount, ID_ETH);
    }
        ...
        returns (uint256 pending)
    {
        ...
        if (
            ...
            uint256 ethReward = (secondsStaked *
                ethAmountPerSecond *
                ethPool.allocPoint) /
                (ethTotalAllocPoint * ethAmountPerSecondPrecision);
            accEthPerShare += ethReward / totalSupply;
        }
        pending = uint256(
            int256(deposit_.amount * accEthPerShare) - ethDeposit.rewardDebt
        );
    }

    /// @notice Update reward variables for all ethPools. Be careful of gas spending!
    /// @param pids Pool IDs of all to be updated. Make sure to update all active ethPools.
    function massUpdateEthPools(uint256[] calldata pids) external {
        uint256 len = pids.length;
        for (uint256 i = 0; i < len; ++i) {
            updateEthPool(pids[i]);
        }
    }

    /// @notice Update reward variables of the given ethPool.
    /// @param pid The index of the ethPool. See `ethPoolInfo`.
    function updateEthPool(uint256 pid) public {
        PoolInfo memory pool = poolInfo[pid];
        EthPoolInfo storage ethPool = ethPoolInfo[pid];
        uint256 totalSupply = pool.totalSupply;
        if (
            ethStartTimestamp > block.timestamp ||
            ethPool.lastRewardTimestamp >= block.timestamp
    ...
    /// @notice Deposit parasets for ETH allocation.
    /// @param pid The index of the ethPool. See `ethPoolInfo`.
    /// @param amount Amount of parasets to deposit for pid.
    function deposit(uint256 pid, uint256 amount) public virtual override {
        require(amount > 0, "Specify valid paraset amount to deposit");
        updateEthPool(pid);
        EthDepositInfo storage ethDeposit = ethDepositInfo[pid][msg.sender];

        // Effects
        ethDeposit.rewardDebt += int256(
            amount * ethPoolInfo[pid].accEthPerShare
        );

        PrimeRewards.deposit(pid, amount);
    }

    /// @notice Withdraw parasets.
    /// @param pid The index of the ethPool. See `ethPoolInfo`.
    /// @param amount amounts to withdraw for the paraset
    function withdraw(uint256 pid, uint256 amount) public virtual override {
        updateEthPool(pid);
        EthDepositInfo storage ethDeposit = ethDepositInfo[pid][msg.sender];
        ...
        PrimeRewards.withdraw(pid, amount);
    }

    /// @notice Claim proceeds for transaction sender to msg.sender.
    /// @param pid The index of the ethPool. See `ethPoolInfo`.
    function claimEth(uint256 pid) public {
        updateEthPool(pid);
        DepositInfo storage deposit_ = depositInfo[pid][msg.sender];
        EthDepositInfo storage ethDeposit = ethDepositInfo[pid][msg.sender];
        int256 accumulatedEth = int256(
            (deposit_.amount * ethPoolInfo[pid].accEthPerShare) /
                ethAmountPerSecondPrecision
        );
        uint256 _pendingEth = uint256(accumulatedEth - ethDeposit.rewardDebt);
        ...
        emit Claim(msg.sender, pid, _pendingEth, ID_ETH);
    }

    /// @notice ClaimPrime and ClaimETH a pool
    /// @param pid Pool IDs of all to be claimed
    function claimEthAndPrime(uint256 pid) public virtual {
        PrimeRewards.claimPrime(pid);
        claimEth(pid);
    }

    /// @notice ClaimPrime multiple ethPools
    /// @param pids Pool IDs of all to be claimed
    function claimPools(uint256[] calldata pids) external virtual {
        for (uint256 i = 0; i < pids.length; ++i) {
            claimEthAndPrime(pids[i]);
        }
    }

    /// @notice Withdraw parasets and claim proceeds for transaction sender to msg.sender.
    /// @param pid The index of the ethPool. See `ethPoolInfo`.
    /// @param amount Paraset amount to withdraw.
    function withdrawAndClaimEthAndPrime(uint256 pid, uint256 amount)
        external
        virtual
    {
        // Claim ETH
        ...
        ethDeposit.rewardDebt =
            accumulatedEth -
            int256(amount * ethPoolInfo[pid].accEthPerShare);
        ...
        PrimeRewards.withdrawAndClaimPrime(pid, amount);
        emit Claim(msg.sender, pid, _pendingEth, ID_ETH);
    }
}
...
/// @notice The Prime Key caching contract. It allows for a fixed PRIME token
/// rewards distributed evenly across all stakers per second.
contract PrimeRewards is Ownable, ERC1155Holder {
    ...
    uint256 private constant primeAmountPerSecondPrecision = 1e18; // primeAmountPerSecond is carried around with extra precision to reduce rounding errors

    uint256 internal constant MAX_NUM_POOLS = 500;
    ...
    // @notice Deposits can be paused
    bool public depositsPaused;

    uint256 internal constant ID_PRIME = 0;
    uint256 internal constant ID_ETH = 1;

    /// @dev internal lock for receiving ERC1155 tokens. Only allow during deposit calls
    bool internal onReceiveLocked = true;
    ...
    function addPool(uint256 _allocPoint, uint256[] memory _tokenIds)
        ...
        onlyOwner
    {
        require(poolInfo.length <= MAX_NUM_POOLS, "Max 500 pools reached");
    ...
    // Set remaining prime to distribute between endTimestamp-startTimestamp
    ...
    function setEndTimestamp(uint256 _endTimestamp) external onlyOwner {
        ...
        require(endTimestamp < _endTimestamp, "invalid end timestamp");
        updateAllPools();
        // Update pool stuffs
        endTimestamp = _endTimestamp;
        primeAmountPerSecond =
    ...
    function addPrimeAmount(uint256 _primeAmount) external onlyOwner {
        require(
        ...
        emit RewardIncrease(_primeAmount, ID_PRIME);
    }
        ...
        returns (uint256 pending)
    {
        ...
        pending = uint256(
            int256(
                (_deposit.amount * accPrimePerShare) /
    ...
    function deposit(uint256 pid, uint256 amount) public virtual {
        ...
        uint256[] memory amounts = new uint256[](poolInfo[pid].tokenIds.length);
    ...
    /// @notice claimPrime proceeds for transaction sender to msg.sender.
    /// @param pid The index of the pool. See `poolInfo`.
    function claimPrime(uint256 pid) public {
        updatePool(pid);
        DepositInfo storage _deposit = depositInfo[pid][msg.sender];
        ...
        emit Claim(msg.sender, pid, _pendingPrime, ID_PRIME);
    }

    /// @notice claimPrime multiple pools
    /// @param pids Pool IDs of all to be claimed
    function claimPrimePools(uint256[] calldata pids) external virtual {
        for (uint256 i = 0; i < pids.length; ++i) {
            claimPrime(pids[i]);
        }
    }
    ...
    /// @param pid The index of the pool. See `poolInfo`.
    /// @param amount Paraset amount to withdraw.
    function withdrawAndClaimPrime(uint256 pid, uint256 amount) public virtual {
        updatePool(pid);
        DepositInfo storage _deposit = depositInfo[pid][msg.sender];
        int256 accumulatedPrime = int256(
            (_deposit.amount * poolInfo[pid].accPrimePerShare) /
