...
library UInt256Extensions {
    ...
    function secondsCutDays(uint256 secondTime) internal pure returns(uint256){
        return secondTime / day;
    }

    function secondsToDayTimeUints(uint256 secondTime) internal pure returns(uint256 d, uint256 h, uint256 m, uint256 s){
        d = secondTime / day;
    ...
    function secondsToDayTimeString(uint256 secondTime) internal pure returns(string memory){
        ...
        return string(abi.encodePacked(UInt256Extensions.toString(d),"D ",UInt256Extensions.toString(h),"H ",UInt256Extensions.toString(m),"M ",UInt256Extensions.toString(s),"S"));
    }

}

library Strings{
    function GetLength(string memory str) pure internal returns (uint length)
    {
        uint i=0;
...
interface ITheStolenGenerator{
    ...
    struct FactionImage{
        string fillFaction;
        string fillOwner;
        string fillHighscore;
        string factionName;
        string highscore;
        string member;
        string factionSteals;
        string steals;
        string current;
    }


    function factionTokenURI(FactionImage memory tokenData, bool isHolding) external view returns(string memory);

    function flagTokenURI(string memory factionName, string memory current) external view returns(string memory);
}


contract TheStolen is Ownable, ERC165, IERC721, IERC721Metadata, ReentrancyGuard {
    ...
    event Merge(address owner, uint oldTokenId);
    ...
    modifier unlocked(uint8 quantity){
        require(_locked != 0x01 && _currentIndex + quantity < collectionSize+1 && block.timestamp < _mintStarttime + _mintTime, "still unlocked");
        _;
    }

    modifier locked(){
        require(_locked == 0x01 || _currentIndex == collectionSize || block.timestamp > _mintStarttime + _mintTime, "still unlocked");
        _;
    }
    ...
    function ownerOf(uint256 tokenId) public view override returns (address){
        require(_exists(uint16(tokenId)), "nonexistent token");
        if(tokenId==0){
            if(flagReleased()){
                return _flag.owner;
            }
    ...
    function factionMintCost() public view returns(uint){
        return mintPrice;
    }

    function flagReleased() public view returns(bool){
        return  _locked == 0x01 || _currentIndex == collectionSize || block.timestamp > _mintStarttime + _mintTime;
    }

    function flagMintPrice() public view returns(uint){
        return flagPrice;
    }
    ...
    function getAddressData(address a) public view returns(AddressID memory,AddressData memory){
        return (_addressIDs[a],_addressData[a]);
    }

    function getHolderFactionId() public view returns(uint16){
        require(_flag.owner != address(0) && _flag.withoutFaction != 0x01,"no faction owns the flag right now.");
        return _flag.factionId;
    }

    function getFactionData(uint16 factionId) public view returns(FactionID memory, FactionData memory){
    ...
    function getLeaderHoldtime() public view returns(uint256){
        ...
        uint currentHolderHoldtime = _factionData[flag.factionId].currentHoldtime + (block.timestamp-_flagLastTransferTime);
        uint currentLongestHoldtime = _currentLongestHoldtime;

        if(flag.factionId != _currentLeader && currentLongestHoldtime > currentHolderHoldtime){
    ...
    function getLeaderName() public view returns(string memory){
        ...
        if(flag.factionId != currentLeader && _currentLongestHoldtime > currentHolderHoldtime){
    ...
    function getCurrentHoldtime() public view returns(uint256){
        ...
        return _factionData[holderId].currentHoldtime + (block.timestamp-_flagLastTransferTime);
    }
    ...
    function _ownershipOf(uint16 tokenId, uint16 lowestToCheck) internal view returns (FactionToken memory){
        FactionToken memory ft = _factionOwnerships[tokenId];
        require(ft.isBurned != 0x01, "nonexistent token");
        lowestToCheck--;
        ...
        revert("ERC721V: unable to determine the owner of token");
    }

    function _ownershipOf(uint16 tokenId) internal view returns (FactionToken memory){
        FactionToken memory ft = _factionOwnerships[tokenId];
        require(ft.isBurned != 0x01, "nonexistent token");
    ...
    function _burned(uint16 tokenId) internal view returns (bool) {
        ...
        return _ownershipOf(tokenId).isBurned == 0x01 ? true : false;
    }
    ...
    function _approve(address owner, address to, uint16 tokenId) private {
    ...
    function getApproved(uint256 tokenId) public view override returns (address) {
        ...
        require(_exists(tId), "approved query for nonexistent token");
    ...
    function _isApprovedOrOwner(address spender, uint16 tokenId) internal view virtual returns (address owner, bool isApprovedOrOwner) {
    ...
    function _transfer(address from, address to, uint16 tokenId) private {
        ...
        if(tokenId == 0){
            ...
            uint8 chanceForStealing = uint8(uint256(keccak256(abi.encodePacked((block.difficulty+ block.timestamp))))%100);
    ...
    function _transferFaction(address prevOwner, address newOwner, uint16 tokenId, FactionToken memory ft) private{
        ...
        if(no.hasRogue == 0x01){
            _burn(newOwner, no.rogueId, _ownershipOf(no.rogueId,noID.factionTokenId));
            no.hasRogue = 0x00;
        }
        ...
        if(noID.factionBalance>0){
            _burn(newOwner, noID.factionLastTokenId, _ownershipOf(noID.factionLastTokenId, noID.factionTokenId));
    ...
    function _removeFrom(address prevOwner, uint16 tokenId, FactionToken memory ft) private{
    ...
    function _checkRogue(AddressID memory ownerID, AddressData memory ownerData) private returns(AddressID memory oID, AddressData memory o){
    ...
    function _checkNewLast(AddressID memory ownerID, FactionToken memory ft, uint16 tokenId) private returns(AddressID memory oID){
        oID = ownerID;
        ...
        if(prevTokenId != 0){
            ...
            FactionToken memory ft2 = _ownershipOf(prevTokenId,oID.factionTokenId);
            if (ft2.isBurned != 0x01 && ft2.owner == address(0)) {
                _factionOwnerships[prevTokenId] = ft;
                oID.factionLastTokenId = prevTokenId;
                return oID;
            }else{
                for (uint16 curr = prevTokenId; curr >= oID.factionTokenId; curr--) {
                    ft2 = _ownershipOf(curr,oID.factionTokenId);
                    if (ft2.isBurned != 0x01 && (ft2.owner == address(0)||ft2.owner == ft.owner)) {
    ...
    function _checkNewFirst(AddressID memory ownerID, FactionToken memory ft, uint16 tokenId) private returns(AddressID memory oID){
        oID = ownerID;
        ...
        if (ft2.isBurned != 0x01 && ft2.owner == address(0)) {
            ...
            for (uint16 curr = nextTokenId; curr <= oID.factionLastTokenId; curr++) {
                ft2 = _ownershipOf(curr, oID.factionTokenId);
                if (ft2.isBurned != 0x01 && (ft2.owner == address(0)||ft2.owner == ft.owner)) {
                    _factionOwnerships[curr] = ft;
    ...
    function _addReservedFaction(string memory factionName, address contractAddress) private {
        require(contractAddress != address(0), "reserving 0 address not supported.");
    ...
    function _addToFaction(string memory factionName) private returns(uint16){
        require(Strings.GetLength(factionName) < 17 && Strings.GetLength(factionName) > 2, 'name can only be between 3 - 16 letters');
        ...
        if(factionNameID.factionId != 0){
            ...
            if(factionNameID.contractAddress != address(0)){
                ...
                require(c.balanceOf(msg.sender) > 0, "must own reserved faction nft");
            }
        }
    ...
    function factionMint(uint8 quantity, string memory factionName) external payable isOpen unlocked(quantity) {
        require(msg.sender != address(0), "Mint to the zero address not supported");
        require(msg.sender == tx.origin, "No Bot minting!");

        if(freeMintList[msg.sender] == 0x01){
            require(msg.value == mintPrice * (quantity-1), 'invalid mint price');
        }else{
        ...
        require(_addressIDs[msg.sender].hasMinted != 0x01, "address already used its chance");

        _addressIDs[msg.sender] = AddressID(factionId,startTokenId, endTokenId-1,quantity, 0x01);

        _factionOwnerships[startTokenId] = FactionToken(msg.sender, factionId, 0x00, 0x00);
    ...
    function flagVulnerable() private view returns(bool){
        uint lastTransfer = _flagLastTransferTime;
        return (_flag.vulnerable == 0x01 && lastTransfer+_flagFastStealTime > block.timestamp) || lastTransfer+_flagStealTime < block.timestamp;
    }
    ...
    function huntFlag() external payable locked{
        ...
        if(!flagVulnerable()){
            require(msg.value == mintPrice, "invalid get price");
            //change price
            ...
            if(currentFlagOwner != address(0)){
                ...
                _addressData[msg.sender] = _changeFlagOwner(_addressData[msg.sender],msg.sender,0x00);
    ...
    function _saveOwnerTime() private {
        if(_flag.withoutFaction == 0x00 && _flagLastTransferTime != 1){
            ...
            uint currentHoldtime = _factionData[fof].currentHoldtime + (block.timestamp - _flagLastTransferTime);
    ...
    function _changeFlagOwner(AddressData memory newOwnerData, address newOwner, bytes1 stolen) private returns(AddressData memory no){
        //Read owner Data
        no = newOwnerData;
        //Write owner Data
        (_addressData[newOwner], _addressData[_flag.owner]) = _changeFlagOwner(no,newOwner,_addressData[_flag.owner],_flag.owner,stolen);
    }

    function _changeFlagOwner(address newOwner,address prevOwner, bytes1 stolen) private{
        //Read owner Data
        AddressData memory no = _addressData[newOwner];
        AddressData memory po = _addressData[prevOwner];
        //Write owner Data
        (_addressData[newOwner],_addressData[prevOwner]) = _changeFlagOwner(no,newOwner,po,prevOwner,stolen);
    }

    function _changeFlagOwner(AddressData memory noData, address newOwner, AddressData memory poData, address prevOwner, bytes1 stolen) private returns(AddressData memory no,AddressData memory po){
        //Initialize return Variables
        no = noData;
        po = poData;
    ...
    function burn(uint16 tokenId) public {
        ...
        require(_exists(tokenId) && ft.isBurned != 0x01,"nonexistent or burned token");
    ...
    function _burn(address owner, uint16 tokenId, FactionToken memory ft) internal {
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory){
        ...
        if(tokenId == 0){
            ...
            string memory factionName = _flag.withoutFaction != 0x01 ? _factionIDs[factionId].name : "";
            string memory current = _flag.withoutFaction != 0x01 ? UInt256Extensions.secondsToDayTimeString(getCurrentHoldtime()) : "";
            return stolenImage.flagTokenURI(factionName,current);
        }else{
            uint16 tId = uint16(tokenId);
            require(_exists(tId) && !_burned(tId), "nonexistent token");
            ...
            factionImageData.fillFaction = factionId ==_currentFactionHolding && _flag.withoutFaction != 0x01 ? colorActive : colorNormal;
            factionImageData.fillOwner = _ownershipOf(tId).owner ==_flag.owner && _flag.withoutFaction != 0x01 ? colorActive : colorNormal;
            factionImageData.fillHighscore = _currentLeader == factionId ? colorActive : colorNormal;
            factionImageData.factionName = _factionIDs[factionId].name;
            factionImageData.highscore = UInt256Extensions.secondsToDayTimeString(fdata.highscore);
            factionImageData.member = UInt256Extensions.toString(_factionIDs[factionId].memberCount);
            factionImageData.factionSteals = UInt256Extensions.toString(fdata.stolen);
            factionImageData.steals = UInt256Extensions.toString(data.stolen);
            factionImageData.current  = factionId==_currentFactionHolding && _flag.withoutFaction != 0x01 ? UInt256Extensions.secondsToDayTimeString(getCurrentHoldtime()) : "";

            return stolenImage.factionTokenURI(factionImageData,factionId ==_currentFactionHolding && _flag.withoutFaction != 0x01);
        }
    }
    ...
    function _checkOnERC721Received(address from, address to, uint256 tokenId, bytes memory _data) private returns (bool) {
        if (to.isContract()) {
            ...
            returns (bytes4 retval) {
                ...
                if (reason.length == 0) {
                    revert("ERC721V: transfer to non ERC721Receiver implementer");
                } else {
                    assembly {
