...
contract Juice is WizardsERC1155 {
    ...
    uint256 private _ethPrice = 1300000000000000;
        ...
        whenNotPaused
    {
        ...
        if (p.price == 0) revert PaymentFailed();
    ...
    function purchaseOg(uint256 quantity, uint256 paymentToken)
        public
        payable
        whenNotPaused
    {
        _paymentog(quantity, paymentToken);
        _mint(_msgSender(), 0, quantity, "");
    }
    ...
    function randomPurchase(
        ...
        if (payWithToken) _payment(quantity, paymentToken);
        else {
            uint256 total = _ethPrice * quantity;
    ...
    function _paymentog(uint256 quantity, uint256 paymentToken) private {
        PaymentToken memory p = _paymentToken[paymentToken];
        uint256 total = quantity * p.price;
