...
contract MonolithYachtClub is ERC721, ERC721Enumerable, ERC721URIStorage, Ownable {
    ...
    uint256 private price = 0.16 ether;
    string private URL;
    ...
    function publicMint(uint256 tokenAmount) external payable{
        require(msg.value <= msg.sender.balance, "Not enough ETH in wallet");
        require(price*tokenAmount <= msg.value, "Not enough ETH sent; Price is 0.16 eth per NFT, Maximum 100 per request");
        uint256 localCounter = counter;
        ...
        if (localCounter + tokenAmount > 60000) {
    ...
    function privateMint(uint256 tokenAmount, address wallet) external onlyOwner{
        uint256 localCounter = counter;
        if (localCounter + tokenAmount > 60000) {
            revert('Supply Not Available, Try Less');
        }
