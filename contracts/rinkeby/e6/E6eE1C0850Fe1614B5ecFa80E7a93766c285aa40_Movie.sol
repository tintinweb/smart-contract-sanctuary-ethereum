...
contract Movie is ERC721Optimized, Ownable, ReentrancyGuard {
    ...
    event Minted(address indexed mintAddress, uint256 indexed quantity);
    ...
    function giveAway(address to, uint256 numberOfToken) public onlyOwner {
        ...
        emit Minted(to, numberOfToken);
    }

    //Mint
   mapping(address => uint256) minteds;
    function mint(uint256 numberOfTokens) public payable nonReentrant {
        ...
        require(minteds[msg.sender] + numberOfTokens <= 1,"Only minit one");
        minteds[msg.sender] += numberOfTokens;

        //mint
        _safeMint(msg.sender, 1);
        emit Minted(msg.sender, 1);

        refundIfOver(uint256(mintPrice) * numberOfTokens);
    }
    ...
    //购买
    function buy(address seller,uint256 tokenId) public nonReentrant {

        //检查tokenId
        require(tokenId >= 0, "tokenId must be greater than zero");

        //检查tokenId是否归属于卖方
        require(_isApprovedOrOwner(seller, tokenId), "ERC721: transfer caller is not owner nor approved");
       
        //检查当前余额要大于购买的金额
        uint256 amount=tokenIdsPrice[tokenId]/10**18;
        require(msg.sender.balance >= amount, "Insufficient balance");
 
        //计算交易费
        uint256 tradeFee = (amount * tradeFeePrecent) / 1000;//TODO
        address author = 0x3be3f904996a79d8E8334B6DB7593108e06fA280;
        Address.sendValue(payable(author), tradeFee);

        //扣除交易费转入卖出方
        uint256 tradeAmount = amount - tradeFee;
        Address.sendValue(payable(seller), tradeAmount);

        //tokenId转入给买入方,通过合约转出
         _beforeTokenTransfer(seller, msg.sender, tokenId);
