...
contract Jabali is VRFConsumerBaseV2, IERC721Receiver, Ownable, Pausable {
  ...
  mapping(uint256 => address payable) public s_requestIdToAddress;
  ...
  uint256[5] public groupCumValues;
  ...
  // @dev Variable used to calculate the Probabilities of the groups. Tweaking this variable, and adjusting the average
  //      group cost, it gives the flexibility to approximate a Margin for the lottery. This number has been adjusted to
  //      give a value of 1.65 for the ratio of (revenueForTheProject / CostOfNFTsGivenAway).
  //      Note: Is difficult to estimate this ratio since variables are dynamic (LINK and ETH price, gasFees,
  //            ChainLink Premium Fee, gasUsage, distribution of throws per player which affects the slugMeter multiplier,
  //            throws with referrals, etc...)
  uint8 public constantProbability = 25;

  // @dev There is a 6 decimals precision for the Probabilities. In solidity, currently there is no float available, so
  //      must represent the probabilities as integers. In this code, a probability of 1 is equivalent to ProbabilityEquivalentToOne.
  uint256 public ProbabilityEquivalentToOne = 1000000;
  ...
  // @dev Percentage of the value of the ticket which go to the player if the player has a referrer
  ...
  function setReferrer(address referrerAddress) public whenNotPaused {
    require(moneySpent[referrerAddress] >= .5 ether , "The referrer must have spent more than 0.5ETH in the lottery");
  ...
  function setconstantProbability(uint8 _constantProbability) public onlyOwner {
  ...
  // @dev Function to be able to modify the ProbabilityEquivalentToOne in case we want to make more difficult the JackPot
  // @param _constantProbability New constant Probability to adjust margin
  function setProbabilityEquivalentToOne(uint256 _ProbabilityEquivalentToOne) public onlyOwner {
    ProbabilityEquivalentToOne = _ProbabilityEquivalentToOne;
  ...
  function withdrawFunds(address payable _to) public onlyOwner {
    uint256 balanceToTransfer = address(this).balance - jackPotBalance;
    _to.transfer(balanceToTransfer);
  }

  // @dev Function to withdraw all the NFTs deposited from the smart contract
  function withdrawAllNFTsToOwner() public onlyOwner {
    for (uint i=0; i<TopNFTs.length; i++) {
      ERC721(TopNFTs[i].contractAddress).safeTransferFrom(address(this), msg.sender, TopNFTs[i].tokenID);
    }
    for (uint i=0; i<MediumNFTs.length; i++) {
      ERC721(MediumNFTs[i].contractAddress).safeTransferFrom(address(this), msg.sender, MediumNFTs[i].tokenID);
    }
    for (uint i=0; i<NormalNFTs.length; i++) {
      ERC721(NormalNFTs[i].contractAddress).safeTransferFrom(address(this), msg.sender, NormalNFTs[i].tokenID);
    }
    delete TopNFTs;
    delete MediumNFTs;
    delete NormalNFTs;
  ...
  function depositTopNFT(address contractAddress, uint256 tokenID, uint256 WeiCost) public onlyOwner {
  ...
  // @param tokenID The token ID of the NFT to be deposited
  // @param WeiCost Estimated cost of the NFT in Wei
  function depositMediumNFT(address contractAddress, uint256 tokenID, uint256 WeiCost) public onlyOwner {
    ERC721 NFTContract = ERC721(contractAddress);
    NFTContract.safeTransferFrom(msg.sender, address(this), tokenID);
  ...
  // @param tokenID The token ID of the NFT to be deposited
  // @param WeiCost Estimated cost of the NFT in Wei
  function depositNormalNFT(address contractAddress, uint256 tokenID, uint256 WeiCost) public onlyOwner {
    ERC721 NFTContract = ERC721(contractAddress);
    NFTContract.safeTransferFrom(msg.sender, address(this), tokenID);
  ...
  function recalulateCumGroupProb() internal {
    groupCumValues = [JackPotProbability,
                      JackPotProbability + TopGroupProbability,
                      JackPotProbability + TopGroupProbability + MediumGroupProbability,
                      JackPotProbability + TopGroupProbability + MediumGroupProbability + NormalGroupProbability,
                      JackPotProbability + TopGroupProbability + MediumGroupProbability + NormalGroupProbability + NoneGroupProbability];
  }
  ...
  function recalculateGroupProbabilities() internal {
    ...
    NoneGroupProbability = ProbabilityEquivalentToOne - (JackPotProbability + TopGroupProbability + MediumGroupProbability + NormalGroupProbability);
  }
  ...
  function enterThrow() public payable whenNotPaused returns (uint256){
  ...
  function requestRandomWords() internal returns (uint256){
    ...
    s_requestIdToAddress[requestId] = payable(msg.sender);
    return requestId;
  }
  ...
  function getSlugmeterMultiplier(address player) internal view returns (uint8) {
    ...
    else if (_unluckyThrows >= 5 && _unluckyThrows < 20) {
        slugMultiplier = 2;
    }
    else {
        slugMultiplier = 3;
  ...
  // @param requestId requestId generated by ChainLink VRF to identity different requests
  function checkIfWinner(uint256 requestId) internal {
    ...
    // Top NFT Prize
    else if (groupCumValues[0] * slugMultiplier < groupRandomRange && groupRandomRange <= groupCumValues[1] * slugMultiplier) {
      uint256 index = checkNFTPrize(nftRandomRange, 1);
      withdrawTopNFT(index, player);
      unluckyThrows[player] = 0;
    }
    // Medium NFT Prize
    else if (groupCumValues[1] * slugMultiplier < groupRandomRange && groupRandomRange <= groupCumValues[2] * slugMultiplier) {
      uint256 index = checkNFTPrize(nftRandomRange, 2);
      withdrawMediumNFT(index, player);
      unluckyThrows[player] = 0;
    }
    // Normal NFT Prize
    else if (groupCumValues[2] * slugMultiplier < groupRandomRange && groupRandomRange <= groupCumValues[3] * slugMultiplier) {
  ...
  // @dev TEST FOR DEBUGGING!!!
  function setFakeRandomWords(uint256 requestId, uint256[] memory randomWords) public onlyOwner {
    s_requestIdToRandomWords[requestId] = randomWords;
    s_requestIdToAddress[requestId] = payable(msg.sender);
  }
