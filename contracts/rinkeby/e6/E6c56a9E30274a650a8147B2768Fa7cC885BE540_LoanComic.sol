...
import "./structs/Loan.sol";

contract LoanComic is ERC721, SlicerPurchasable {
    /// ============ Errors ============

    // Thrown when an invalid query is made
    error Invalid();
    // Thrown when an token is transferred while loaned
    error ActiveLoan();

    /// ============ Storage ============

    uint24 private constant LOAN_TIME_UNIT = 5 minutes;
    // Mapping from token Ids to loans
    ...
    constructor(
        ...
        _safeMint(0x60535A6605958fFf6cEC5B1e92892601EFb3473b, 1); // 0xgogo.eth
    }
     ...
     * @dev Return true if account is allowed to buy product
     */
    function isPurchaseAllowed(
        ...
        // TODO: Hardcode to 1? Or to productId? Or to what?
        uint256 tokenId = 1;

        // Returns true if token is not being loaned
        isAllowed = _loans[tokenId].lender == ownerOf(tokenId);
    }
    ...
    function onProductPurchase(
        ...
        // TODO: Hardcode to 1? Or to productId? Or to what?
        uint256 tokenId = 1;
        // Set expiration timestamp based on quantity purchased
        _loans[tokenId].expirationTimestamp = uint40(block.timestamp + quantity * LOAN_TIME_UNIT);
        // Send token to account
        safeTransferFrom(ownerOf(tokenId), account, tokenId);
    }

    /**
     * @notice Sends back a loaned token to the lender.
     */
    function claimExpiredLoan(uint256 tokenId) external {
        // Add reference for owner and lender
        address owner = ownerOf(tokenId);
        address lender = _loans[tokenId].lender;

        if (owner == lender) revert Invalid();
        if (block.timestamp < _loans[tokenId].expirationTimestamp) revert ActiveLoan();

        // Transfer token from owner to lender
        safeTransferFrom(owner, lender, tokenId);
    }

    /**
     * @notice Transfer loan ownership while a loan is active.
     */
    function transferLenderWhileActive(
        address from,
        address to,
        uint256 tokenId
    ) external {
        // Sender needs to be the lender and not the current owner of token
        if (from != msg.sender || from != _loans[tokenId].lender || from == ownerOf(tokenId))
            revert Invalid();

        // Transfer loan ownership to receiver
        _loans[tokenId].lender = to;
    }
    ...
    function _beforeTokenTransfer(
        ...
        // If it is not a loan performed via Slice
        if (msg.sender != _productsModuleAddress) {
            // During mint or when lender transfers a token in his possession
            if (from == address(0) || from == _loans[tokenId].lender) {
                // Set receiver as the lender
                _loans[tokenId].lender = to;
            }
            // During transfer, revert if token is loaned
            else revert ActiveLoan();
        }
    }

    /**
     * @dev Returns whether `spender` is allowed to manage `tokenId`.
     * @dev Ovverridden to mark this contract as allowed for all transfers, to initiate loans
     */
        ...
        returns (bool)
    {
        ...
        return (spender == owner ||
            ...
            // Allow loans to be claimed
            msg.sender == address(this) ||
            // set {productsModule} as allowed spender to allow loans via Slice
            spender == _productsModuleAddress);
    }

    // TODO: Add function to cancel loan?
}
