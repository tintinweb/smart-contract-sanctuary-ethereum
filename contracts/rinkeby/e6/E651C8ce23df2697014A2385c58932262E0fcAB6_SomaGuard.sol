...
contract SomaGuard is ISomaGuard, Accessible {
    ...
    bytes32 public immutable override DEFAULT_PRIVILEGES;

    /// @custom:oz-upgrades-unsafe-allow constructor
    constructor(address somaAddress)
    SomaContract(somaAddress) {
        DEFAULT_PRIVILEGES = GuardHelper.DEFAULT_PRIVILEGES;
    }
    ...
    function privileges(address account) public override virtual view returns (bytes32) {
        return _accounts.contains(account) ? _privileges[account] : DEFAULT_PRIVILEGES;
    }
    ...
    returns (
        ...
        require(_currentIndex < totalRecords, 'SomaGuard: invalid index. Must be less than the total accounts');
        ...
        privileges_ = new bytes32[](maxLength);

        for (uint i; i < maxLength; ++i) {
            accounts_[i] = _accounts.at(i + _currentIndex);
            privileges_[i] = _privileges[accounts_[i]];
        }
    }
    ...
    function batchUpdate(
        ...
        for (uint i; i < accounts_.length; ++i) {
            bytes32 newPrivileges = privileges_[i];

            for (uint j; j < accounts_[i].length; j++) {
                address account = accounts_[i][j];

                emit PrivilegesUpdated(_msgSender(), _privileges[account], newPrivileges, account);

                _privileges[account] = newPrivileges;
                if (newPrivileges == DEFAULT_PRIVILEGES) {
        ...
        emit BatchUpdate(_msgSender(), accounts_, privileges_);
