...
contract NFT is ERC721, Ownable {
    ...
    uint256 public constant MAX_SUPPLY = 23665; // do there need amount -
    uint256 public constant MAX_PER_TX = 6; // MAX_PER_TX is 5

    uint256 private constant MAX_PRESALE_MINTS_PER_ADDRESS = 6; // max mints during presale per address is 5
    ...
    uint256 public mintPriceDefender = 0.005 ether;
    uint256 public mintPriceConqueror = 0.01 ether;
    ...
    uint256 public maxPerPresalePhase;
    ...
    constructor(string memory _baseURI, uint256 _maxPerPresalePhase, address _owner) ERC721("TokenName","TokenSymbol") {
        _defaultUri = _baseURI;
        maxPerPresalePhase = _maxPerPresalePhase; // set eg 1001
    ...
    /// Set max per current presale phase
    /// @dev modifies the state of the `maxPerPresalePhase` variable
    /// @param _maxPerPresalePhase The new max per current presale phase
    /// @notice maxPerPresalePhase set previous day limit + new limit for next presale day
    function setMaxPerPresalePhase(uint256 _maxPerPresalePhase) external onlyOwner {
        maxPerPresalePhase = _maxPerPresalePhase;
    }
    ...
    function presaleMintDefender(uint256 count) external payable validateEthPaymentDefender(count) {
        // TBC
        // uint256 totalSupply = _owners.length;
        // require(totalSupply + count < maxPerPresalePhase, 'Excedes max per current presale phase.'); // previous day limit + new limit for next presale day
        ...
        for (uint256 i; i < count; i++) {
            _safeMint(_msgSender(), _tokenIdDefender);
            _tokenIdDefender++;
        }
    ...
    /// @param count number of tokens to mint in transaction
    /// @notice mints tokens with counter token IDs to addresses eligible for presale
    function presaleMintConqueror(uint256 count) external payable validateEthPaymentConqueror(count) {
        // TBC
        // uint256 totalSupply = _owners.length;
        // require(totalSupply + count < maxPerPresalePhase, 'Excedes max per current presale phase.');

        require(conquerorMinted + count <= NUMBER_OF_CONQUER - NUMBER_OF_RESERVED_CONQUER, 'Excedes max supply of Defender.');
        require(count < MAX_PER_TX, 'Exceeds max per transaction.');
        require(phase == SalePhase.PreSale, 'Presale event is not active');
        require(addressToMinted[_msgSender()] + count < MAX_PRESALE_MINTS_PER_ADDRESS, "Exceeds number of presale mints allowed"); 
        ...
        addressToMinted[_msgSender()] += count;
        for (uint256 i; i < count; i++) {
            _safeMint(_msgSender(), _tokenIdConqueror);
            _tokenIdConqueror++;
        }
