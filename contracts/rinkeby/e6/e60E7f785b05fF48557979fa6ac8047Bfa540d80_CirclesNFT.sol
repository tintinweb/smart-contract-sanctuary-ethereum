...
contract CirclesNFT is ERC721URIStorage {

    event NewCirclesCreated(uint indexed tokenId, string tokenURI);

    using Counters for Counters.Counter;
    Counters.Counter private _tokenIds;

    constructor() ERC721("CirclesNFT", "CIRCLES") {
    }

    function create(string memory _svg) public {
        // Get the current token ID and set it to our newItemId for our next mint ID
        uint256 newItemId = _tokenIds.current();

        // Mint NFT to msg.sender w/ ID of the current _tokenIds counter
        ...
        // Increment counter for next minted item
        _tokenIds.increment();

        // emit NewCirclesCreated event
        emit NewCirclesCreated(newItemId, tokenURI);

    }

    // helper function to turn svg code into an image URI
    function svgToImageURI(string memory _svg) public pure returns (string memory) {
        ...
        // data:image/svg+xml;base64,PHN2ZyB4bWxucz0iaHR0cDovL3d3dy53My5vcmcvMjAwMC9zdmciIGhlaWdodD0iMTAwIiB3aWR0aD0iMTAwIj4KICAgICAgICA8Y2lyY2xlIGN4PSI1MCIgY3k9IjUwIiByPSI0MCIgc3Ryb2tlPSJibHVlIiBzdHJva2Utd2lkdGg9IjMiIGZpbGw9InJlZCIgLz4KICAgICAgICA8L3N2Zz4=

        string memory baseURL = "data:image/svg+xml;base64,";
        string memory svgBase64Encoded = Base64.encode(bytes(string(abi.encodePacked(_svg))));
        // concat base, base64 svg to get imageURI
    ...
    function formatTokenURI(string memory _imageURI) public pure returns (string memory) {
        ...
        return string(abi.encodePacked(baseURL, Base64.encode(bytes(abi.encodePacked(
            '{ "name": "Circles", ',
             '"description": "Straight up, the coolest circles around... haven\t you heard?", ',
             '"attributes": "", ',
             '"image": "', _imageURI, '" }'
             )))));
    }
