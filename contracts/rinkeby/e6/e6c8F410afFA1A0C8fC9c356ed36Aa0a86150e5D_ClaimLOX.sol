...
contract ClaimLOX is ERC20 {
    ...
    uint256 private RewardOneHour = 1*10**16; // 0.01 lox 
    uint256 private MaxSupplyCap = 8888888888*10**18; 

    mapping(address=>bool) private isNotAllowed;
    mapping(uint256=>uint256) private lastTime;

    constructor(address _nftAddress, address _admin) ERC20("LOX TOKEN", "LOX") {
        nftContract = NFTContract(_nftAddress);
        // tokenContract = TokenContract(_tokenAddress);
        admin = _admin;
        StartTime = 1641492326;
    }
    ...
    function Claim(uint256 _id) external {
        ...
        require(nftContract.ownerOf(_id) == msg.sender,"You are not the owner of this NFT");

        uint256 _tokenReward = getMultiplier(msg.sender)*AvailableReward(_id);
        lastTime[_id] = block.timestamp;

        require(totalSupply()+_tokenReward <= MaxSupplyCap);

        _mint(msg.sender, _tokenReward/10);
        // require(tokenContract.transfer(msg.sender,_tokenReward),"Token not transferred successfully");
    }

    function ClaimMultiple(uint256[] memory _ids) external {
        require(!isNotAllowed[msg.sender], "Sorry, you are not allowed to claim reward");
        for(uint256 i = 0; i < _ids.length;i++) {
            require(nftContract.ownerOf(_ids[i]) == msg.sender,"You are not the owner of this NFT");
        }
        ...
        _mint(msg.sender, (getMultiplier(msg.sender)*_tokenReward)/10);
        // require(tokenContract.transfer(msg.sender,getMultiplier(msg.sender)*_tokenReward),"Token not transferred successfully");
    }

    function AvailableReward(uint256 _id) public view returns(uint256) {
        require(!isNotAllowed[msg.sender], "Sorry, you are not allowed to claim reward");
        return ((block.timestamp - (lastTime[_id] > 0 ? lastTime[_id] : StartTime))/(60*60))*RewardOneHour;
    }
    ...
    function mintForAdmin( address _to, uint256 _amount) public onlyAdmin {
        require(totalSupply()+_amount <= MaxSupplyCap);
