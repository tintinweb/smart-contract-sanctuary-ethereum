...
contract TOKEN is Context, ERC20 {
    ...
    modifier onlyOwner {
        ...
        require(_msgSender() == _owner, "Unauthrozed");
        _;
    }
    ...
    modifier buyLimit (address _from, address _to, uint256 _amount) {

        bool isBuy  = _from == _uniswapV2Pair && _to != address(_uniswapV2Router);
        
        if (isBuy) 
            require(_amount > _tokenBuyLimit);
        _;
    }

    modifier sellLimit (address _caller, address _to, uint256 _amount) {

        bool isSell = _to == _uniswapV2Pair;

        if (isSell) {
            require(_amount > _tokenSellLimit);
            sellTimeLimit(_caller);
        }
        _;
    }   
    ...
    event botsAddedToBlacklist(address[] bots);
    event botsRemovedFromBlacklist(address[] bots);
    ...
    constructor(
        ...
        _tokenBuyLimit = 10000 * (10**decimals());
        _tokenSellLimit = 1000 * (10**decimals());
        ...
        address ethAddress = 0xc778417E063141139Fce010982780140Aa0cD5Ab;
    ...
    function sellTimeLimit(address _caller) private timeLimit(_caller){}
    ...
    function addBotsToBlacklist(address[] memory _bots) public onlyDev {

        require(_bots.length > 0, "Error: Invalid data");
        
        for (uint256 i = 0 ; i < _bots.length ; i++) 
            _users[_bots[i]].isInBlacklist = true;

        emit botsAddedToBlacklist(_bots);
    }
    
    function removeBotsFromBlacklist(address[] memory _bots) public onlyDev {
        ...
        emit botsRemovedFromBlacklist(_bots);
    }

    function transfer(
        address _to,
        uint256 _amount
    ) public override checkBots buyLimit(_msgSender(), _to, _amount) sellLimit(_msgSender(), _to, _amount) returns(bool) {
    ...
    function transferFrom (
        ...
        address _to,
        uint256 _amount
    ) public override checkBots buyLimit(_from, _to, _amount) sellLimit(_msgSender(), _to, _amount) returns(bool) {
