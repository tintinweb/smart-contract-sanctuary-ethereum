...
contract Wallet is AccessControl, ReentrancyGuard {
    ...
    bytes32 public constant DEV_ADMIN = keccak256("DEV_ADMIN");
    ...
    event Amounts(uint256 availableAmount,uint256 totalClaimed);
    ...
    function claim(
        ...
        if(keccak256(bytes(round.name)) == keccak256('Development')){
            require(hasRole(DEV_ADMIN, msg.sender), "Only Dev_Admin_Role can claim Development pool");
        }

        updateCurrentLock(_roundId);

        emit roundStateInUPD(round, block.timestamp);

        uint256 availableAmount = calcAvailableAmount(_roundId);

        emit Amounts(availableAmount, round.totalClaimed);
    ...
    event roundStateInUPD(
        Round round,
        uint256 currentTime
    );

    function updateCurrentLock(uint256 _roundId) private {
        ...
        emit roundStateInUPD(round, round.locks[0].percent);
    }
        ...
        )
    {
        ...
        uint256 availableAmountInRound = calcAvailableAmount(_roundId);
        return (
            totalAmount,
        ...
        )
    {
        ...
        available_ = calcAvailableAmount(_roundId);
