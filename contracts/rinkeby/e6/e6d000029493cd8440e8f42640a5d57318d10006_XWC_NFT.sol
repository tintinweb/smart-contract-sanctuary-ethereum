...
contract XWC_NFT is Ownable,ReentrancyGuard {
    ...
    event setETHPriceOnSaleEvent(address indexed _address,uint256 _tokenId,uint256 _price);
    event CancelSellEvent(address indexed _address,uint256 _tokenId,uint256 _price);
    event Mint(address indexed from, address indexed to, uint256 indexed tokenId);
    event PurchaseWithEthEvent(address _seller, address _buyer, uint256 _price);
    ...
    function setETHPriceOnSale(uint256 _tokenId,uint256 _price) public returns(bool) {
        ...
        emit setETHPriceOnSaleEvent(msg.sender,_tokenId,_price);
        return true;
    }
    ...
    function cancelSell(uint _tokenId) public notContract returns(bool) {
        ...
        emit CancelSellEvent(msg.sender,_tokenId,tokenDescriptionMaps[_tokenId].Price);
        return true;
    }

    function getTokenDescriptionByTokenId(uint256 _tokenId) external view returns(TokenDescription memory) {
        require(_tokenId <= getLastTokenId(),"NFT not exist!");
        return tokenDescriptionMaps[_tokenId];
    }
    ...
    function purchaseWithETH(uint256 _tokenId) external virtual payable nonReentrant {
        ...
        emit PurchaseWithEthEvent(_seller, msg.sender, msg.value);
    }
