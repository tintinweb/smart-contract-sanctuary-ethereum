...
contract SpecialSluts is Context, ERC721A, Ownable, ReentrancyGuard, PaymentSplitter {
    using Strings for uint256;

    IERC721 private cryptosluts;
    ...
    uint256 public maxTokensPerAddress = 2;
    bool public saleIsActive;
    mapping(address => bool) public isWhitelistedForAirdrop;

    event TokenMinted(uint256 supply);

    constructor(IERC721 _cryptosluts, address[] memory _team, uint256[] memory _shares) 
        ERC721A("SpecialSluts", "SSLUT") 
        PaymentSplitter(_team, _shares)
    {
        cryptosluts = _cryptosluts;
    }
    
    function mint(uint256 qty) public payable {
        require(saleIsActive, "Sale is not active");
        require(cryptosluts.balanceOf(msg.sender) > 0, "Slut owners only");
        require(qty > 0, "You must mint at least 1 NFT");
        require(totalSupply() + qty <= maxSupply, "Not enough supply");
        require(balanceOf(msg.sender) + qty <= maxTokensPerAddress, "Max tokens per address limit exceeded");
        require(msg.value >= price * qty, "Please send the correct amount of ETH");
    ...
    function setBaseURI(string memory _baseURI, string memory _baseExtension)
        public
        onlyOwner
    {
        baseURI = _baseURI;
