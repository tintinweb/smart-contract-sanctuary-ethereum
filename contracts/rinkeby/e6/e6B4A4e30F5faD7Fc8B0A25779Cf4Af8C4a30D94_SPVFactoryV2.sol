...
// Author: Hagen Hübel, [email protected]ge.io
...
contract SPVFactoryV2 is PermissionsAware {
    ...
    function create(
        ...
        ERC20SPV2 token = new ERC20SPV2(_permissions, permissionSetId, name, symbol, spvName, denomination);
...
// uint256: as long as we have only one (unpacked) variable in Ctor, we don't need to reduce the integer size becaude it won't save any gas
type PermissionSetIdType is uint256;

contract ERC20SPV2 is ERC20Burnable, PermissionsAware {
    ...
    function setProperty(bytes32 key, string memory val_) external onlyRole(TOKEN_ROLE_ADMIN) {
        ...
        if (keccak256(bytes(oldValue)) == keccak256(bytes(val_))) {
            revert("ERC20SPV1: new value is equal to old value");
        }
    ...
    function mint(address to, uint256 amount) public onlyRole(TOKEN_ROLE_MINTER) {
        // require(hasRole(MINTER_ROLE, _msgSender()), "ERC20SPV1: must have minter role to mint");
        _mint(to, amount);
    }
    ...
    function _beforeTokenTransfer(
        ...
        if (from != address(0) && _hasRole(TOKEN_ROLE_IS_BLACKLISTED, from)) {
            revert(
                string(
                    abi.encodePacked("ERC20SPV2: account ", Strings.toHexString(uint160(from), 20), " is blacklisted")
                )
            );
        }
...
abstract contract PermissionsAware is Context, PermissionRoles {
    ...
    function addCustomRoleToken(uint256 roleTokenId, uint256 tokenId) external {
        _customRoleTokens[roleTokenId].add(tokenId);
    }

    function removeCustomRoleToken(uint256 roleTokenId, uint256 tokenId) external {
    ...
    function _checkRole(uint256 roleTokenId, address account) internal view virtual {
        if (!_hasRole(roleTokenId, account)) {
            revert(
                string(
                    abi.encodePacked(
                        "AccessControl: account ",
