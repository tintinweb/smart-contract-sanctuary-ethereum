...
/**
 * @title For mint one LoFi Advertise
 */
...
contract AdClaim_TEST is  Ownable {
    ...
    address public ad_contract = address(0xBf3260729E71Fc330fE442e173522ca473c381ae);  
    ...
    function setADContract( address _contractaddr ) external onlyOwner {
        ad_contract = _contractaddr;
    }

    function claim_go( address reg_addr, uint256 tokeid, uint256 size, uint256 blocknumber) internal returns (uint16) 
    {
        IADContract _ad_contract = IADContract(ad_contract);
        _ad_contract.mint(reg_addr, tokeid, size, blocknumber );
    }

    function Cliam(uint256 price, uint256 tokenid, uint256 size, uint256 blocknumber, uint256 expiry, uint256 _nonce, bytes memory sig) public payable {
            ...
            if( recoverSigner( message, sig ) == checker )
            {
               ...
               claim_go(msg.sender,tokenid, size, blocknumber);
            }
    }  
