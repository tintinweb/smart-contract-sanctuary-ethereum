...
contract AssetManagerUniswap is MultiToken {
    ...
    function decodeUniswapData(address to, bytes memory _data, uint256 amount) external pure returns (address tokenIn, address tokenOut, uint256 amountIn, uint256 amountOut, bool estimatedIn, bool estimatedOut) {
        
        if(to == address(0x68b3465833fb72A70ecDF485E0e4C7bD8665Fc45)) {
            // Uniswap Swap Router
            (tokenIn, tokenOut, amountIn, amountOut, estimatedIn, estimatedOut) = _decodeUniswapRouter(_data);
    ...
    function _decodeWETH9(bytes memory _data, uint256 amount) private pure returns(address tokenIn, address tokenOut, uint256 amountIn, uint256 amountOut) {

        if(BytesLib.toBytes4(_data, 0) == 0xd0e30db0) {
            // deposit()
            return (address(0x0), address(0xc778417E063141139Fce010982780140Aa0cD5Ab), amount, amount);
        } else if (BytesLib.toBytes4(_data, 0) == 0x2e1a7d4d) {
            // withdraw(uint256)
            return (address(0xc778417E063141139Fce010982780140Aa0cD5Ab), address(0x0), amount, amount);
        }

        revert("Unexpected Function");
    }

    function _decodeUniswapRouter(bytes memory _data) private pure returns(address tokenIn, address tokenOut, uint256 amountIn, uint256 amountOut, bool estimatedIn, bool estimatedOut) {
    ...
    function execTransaction(address to, bytes memory _data) external payable {
