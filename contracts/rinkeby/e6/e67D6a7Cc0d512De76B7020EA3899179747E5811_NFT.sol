...
contract NFT is ERC721URIStorage, Ownable {
  ...
  function setTokenURI(uint256 tokenId_, string memory tokenURI_) public {
    ...
    require(marketplace != address(0), "Marketplace has not been set");

    Marketplace _marketplace = Marketplace(marketplace);
    require(
      _marketplace.isAlreadySold(tokenId_) == false,
      "NFT has already been sold"
    );
    require(
      _marketplace.isListedForSale(tokenId_) == false,
    ...
    returns (bool)
  {
    ...
    require(
      nftContract.getApproved(tokenId) == address(this) ||
        nftContract.isApprovedForAll(order.seller, address(this)),
