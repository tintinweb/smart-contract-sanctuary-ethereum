...
contract Prop is ERC20, Operators {

    event Payment(address account, uint ethValue, uint propValue, string product, string info);

    constructor()
        ERC20("Propertiez ERC20 token", "PROP")
    {
    }
    ...
    function pay(uint propValue, string calldata product, string calldata info) external payable {
        if (propValue > 0) {
            // take payment in PROP
            _burn(msg.sender, propValue);
        }
        emit Payment(msg.sender, msg.value, propValue, product, info);
    }
