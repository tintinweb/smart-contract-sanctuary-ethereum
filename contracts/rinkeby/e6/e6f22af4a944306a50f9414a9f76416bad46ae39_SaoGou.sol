...
contract SaoGou is ERC721A, Ownable {
    ...
    uint256 private _teamSupply = 10;
        ...
        callerIsUser
    {
        ...
        if (totalPrice > msg.value)
            revert("SaoGou: Insufficient fund");

        if (!isSaleActive()) revert("SaoGou: Sale not started");
        
        if (quantity > MAX_MINTS_PER_TX)
            revert("SaoGou: Amount exceeds transaction limit");
        if (quantity + userMintCount > MAX_PER_WALLET)
            revert("SaoGou: Amount exceeds wallet limit");
        if (totalSupply() + quantity > (MAX_SUPPLY))
            revert("SaoGou: Amount exceeds supply");
    ...
    function adminMint(uint256 quantity) external onlyOwner {
        require(_reservedSupply + quantity <= _teamSupply, "SaoGou: Reserved amount is exceeded");
        require(totalSupply() + quantity <= MAX_SUPPLY, "SaoGou: Amount exceeds supply");
