...
contract GambitChessClub is ERC721Enumerable, Ownable, IERC2981 {
    ...
    uint256 public constant maxSupply = 5; //10000
    uint8 public maxMintAmount = 2; // 5 - max number of items that can be minted
    ...
    function setAllowList(address[] calldata addresses) external onlyOwner {
        for (uint256 i = 0; i < addresses.length; i++) {
            _allowList[addresses[i]] = maxMintAmount;
        }
    }
    ...
    function mintAllowList(uint8 numberOfTokens) external payable {
        ...
        require(
            ts + numberOfTokens <= maxMintAmount,
    ...
    function whitelistUser(address _user) public view onlyOwner returns (bool) {
        if (_allowList[_user] > 0) return true;
