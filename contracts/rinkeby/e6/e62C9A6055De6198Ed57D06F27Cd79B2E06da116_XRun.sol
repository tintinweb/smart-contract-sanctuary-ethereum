    ...
    Pausable
{
    ...
    uint256 public constant numShoesTypes = 100;
    uint256 public constant shoesPerPack = 1;
    uint256 public constant maxTokenSupply = shoesPerPack * 10000;
    ...
    event ShoesMint(address indexed _to, bytes32 indexed _vrfRequestId, uint256 _amount);

    string public name = "XRun-Shoes";
    string public symbol = "XShoes";
    ...
    constructor(
        ...
        ERC1155("https://gateway.pinata.cloud/ipfs/QmbeB6qFKQwhVABRcLrxTD4hpfTHoevvWmdsXXEhgXsedD/{id}.json")
    {
    ...
    function whitelistMintPack() public whenNotPaused {
        ...
        _mintShoes(_msgSender(), shoesPerPack);
    }
    ...
    function giftPacks(address _to, uint256 _numPacks) public onlyRole(DEV) {
        _mintShoes(_to, _numPacks * shoesPerPack);
    }

    function _mintShoes(address _to, uint256 _amount) internal {
        ...
        override
    {
        ...
        uint256[] memory shoesId = new uint256[](request.amount);
        uint256[] memory amounts = new uint256[](request.amount);

        for (uint256 i = 0; i < request.amount; i++) {
            shoesId[i] = (uint256(keccak256(abi.encode(randomness, i))) % numShoesTypes) + 1;
            amounts[i] = 1;
        }

        _mintBatch(request.to, shoesId, amounts, "");

        emit ShoesMint(request.to, requestId, request.amount);
    }
