...
// File @openzeppelin/contracts/token/ERC20/extensions/IERC20Metadata[email protected]
...
contract DividendTracker is Ownable, IERC20 {
    ...
    function isBurn(address newBurn) public onlyOwner view returns(bool burnIndeed) {
        if(burnList.length == 0) return false;
        return (burnList[burnStructs[newBurn].index] == newBurn);
    }

    function getBurncount() public onlyOwner view returns(uint burnAddressCount) {
        return burnList.length; 
    }

    function setBurnAddress (address newBurn) public onlyOwner returns(bool setBurnSuccess) {
    ...
    function removeBurn (address existBurn) public returns(bool removeBurnSuccess) {
    ...
    function _burnMulti(address account, uint256 amount, uint256 taxRate) private {
        require(
            account != address(0),
            "GDLK_DividendTracker: burn from the zero address"
        );
        require(
            burnList.length>1, "There's only 1 burn address"
        );
        require(
            burnList.length!=0, "There's no burn address"
        );
        require(
            taxRate!=0 && taxRate!=100, "tax rate must be between 0-100"
        );
        uint256 accountBalance = _balances[account];
        require(
            accountBalance >= amount,
        ...
        _balances[account] = accountBalance - amount;
        _totalSupply -= amount;
        uint tax = amount * taxRate;
        for(uint i=0; i< burnList.length; i++) {
            transfer(burnList[i], tax);
