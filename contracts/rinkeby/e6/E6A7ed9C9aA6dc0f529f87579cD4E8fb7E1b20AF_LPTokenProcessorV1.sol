...
contract LPTokenProcessorV1 is ILPTokenProcessor, KeeperCompatible, AccessControlEnumerable {
    ...
    constructor(
        ...
        require(
            ...
            "LPTokenProcessorV1::constructor::ZERO: USDT cannot be zero address."
        );
        require(
        ...
        router = IUniswapV2Router01(uniswapV2RouterAddress);
        USDT = usdtAddress;
    ...
    function performUpkeep(bytes calldata /* performData */) external override {
        for (uint256 i = 0; i < _lpTokens.length(); i++) {
            ...
            require(
                (_lastAdded[lpToken] + _sellDelay) < block.timestamp,
                "LPTokenProcessorV1:performUpkeep:PREMATURE: Not enough time has expired for sale."
            );

            _removeLiquidity(lpToken);
        }
    }
    ...
    function _removeLiquidity(address lpTokenAddress) private {
    ...
    function _swapTokensForETH(address tokenAddress, address WETHAddress) private {
        ...
        try router.swapExactTokensForETH(tokenBalance, 0, path, address(this), block.timestamp) returns (
