...
contract SoulData is ISoulData, Adminable {
     ...
     */
     function getBadgeTrait(address _badgeContract, uint256 _traitIndex, uint256 _soulTokenId, address _soulTokenOwner) external view returns (string memory) {
        ...
        if (isImplementing(_badgeContract, 0x80ac58cd)) {
            if (isExternalTokenOwner(_badgeContract, _soulTokenOwner)) {
            ...
            // try to detect if it is soul badge
            if (getSoulBadgeBalanceForSoul(address(this), _soulTokenId, _badgeContract) > 0) {
                traitValue = getTokenCollectionName(_badgeContract);
                // target contract is sharkz Soul Badge contracts
                if (bytes(traitValue).length != 0) {
    ...
    /**
     * @dev See {ISharkzSoulIDData-isExternalTokenOwner}.
     */
    function isExternalTokenOwner(address _contract, address _ownerAddress) public view returns (bool) {
