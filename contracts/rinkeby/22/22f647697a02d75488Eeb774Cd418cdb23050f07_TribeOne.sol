...
contract TribeOne is ERC721Holder, ERC1155Holder, ITribeOne, Ownable, ReentrancyGuard {
    ...
    function createLoan(
        ...
        require(_loanRules[1] > 0, "T1: LTV should not be ZERO");
        ...
        _loans[loanID].fundAmount = _fundAmount;
    ...
    function payInstallment(uint256 _loanId, uint256 _amount) external payable nonReentrant {
        ...
        uint256 passedTenors = (paidAmount * _loan.loanRules.tenor) / _totalDebt;
    ...
    function _updatePenalty(uint256 _loanId) private {
        DataTypes.Loan storage _loan = _loans[_loanId];
        require(
            _loan.status == DataTypes.Status.LOANACTIVED || _loan.status == DataTypes.Status.DEFAULTED,
            "T1: Not actived loan"
        );
        uint256 expectedNr = expectedNrOfPayments(_loanId);
    ...
    function rentNFTCall(
        address _adapter,
        uint256 _loanId,
        bytes memory rentCallData
    ) external nonReentrant {
        DataTypes.Loan storage loan = _loans[_loanId];
        require(msg.sender == loan.borrower, "T1: Only borrower can call this function");
        require(rentAdaptersSet.contains(_adapter), "T1: Not available adapter");

        (bool success, ) = address(_adapter).call(rentCallData);
        require(success, "T1: Rent call was failed");
    }

    function listNFTToRent(uint256 _loanId) external override {
        require(_loanRents[_loanId] == address(0), "T1: Already rented");
        require(rentAdaptersSet.contains(msg.sender), "T1: Invalid call");
        _loanRents[_loanId] = msg.sender;

        emit LoanRented(_loanId, msg.sender);
    }

    function removeFromRent(uint256 _loanId) external override {
        require(_loanRents[_loanId] == msg.sender, "T1: Only relative adapter can call this function.");
        delete _loanRents[_loanId];

        emit LoanWithdrawFromRent(_loanId, msg.sender);
    }

    /**
     * @dev We will use this function to withdraw NFT when we should liquidate loaned NFT
     */
    function withdrawNFTFromRent(address _adapter, uint256 _loanId) external nonReentrant {
        require(_loans[_loanId].status == DataTypes.Status.DEFAULTED, "T1: Invalid status");
        require(expectedLastPaymentTime(_loanId) + GRACE_PERIOD < block.timestamp, "T1: Not overdue yet");
        require(rentAdaptersSet.contains(_adapter), "T1: Not available adapter");

        IBaseRentAdapter(_adapter).withdrawNFTFromRent(_loanId);
    }
    ...
    function grantNFTApproveAllForAdapter(address adapter, address nft) external {
        TribeOneHelper.safeNFTApproveForAll(nft, adapter, true);
    }

    function revokeNFTApproveAllForAdapter(address adapter, address nft) external onlySuperOwner {
        TribeOneHelper.safeNFTApproveForAll(nft, adapter, false);
    }
}
...
interface ITribeOne {
    ...
    function listNFTToRent(uint256 _loanId) external;

    function removeFromRent(uint256 _loanId) external;
}
