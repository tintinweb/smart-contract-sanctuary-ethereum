...
/** 
 *  SourceUnit: /home/abims/Desktop/multisender/src/Multisend.sol
*/
...
contract Multisender is Ownable {
    ...
    function multiSender(address payable[] memory addrs, 
        ...
        for (uint8 i=0; i < addrs.length; i++) {
           ...
           _tokenTransfer(token, addrs[i], msg.sender);
        }
   
    } 
    
    // Internal token Transfer
    function _tokenTransfer(IERC20 token, address payable addrs, address sender) internal {
        token.transferFrom(sender, addrs, 1*10**18);
   } 
}