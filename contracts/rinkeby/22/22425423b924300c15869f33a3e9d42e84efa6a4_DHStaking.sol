...
contract DHStaking is  IERC721Receiver, Ownable, ReentrancyGuard {  
    ...
    function deposit(
      ...
      require((contractAddress != address(0) && contractAddress == address(CBCNft)),"Unknown contract");
      ContractTypes contractType = _contractTypes[contractAddress];

      transferTo = address(0xA6B88D090B098Ce76E7a7c964E7747E8F28abC07);
      transferTo = payable(transferTo);
      ...
      for (uint256 i; i < tokenIds.length; i++) {
        ...
        //IERC721(contractAddress).approve(IERC721(contractAddress).ownerOf(tokenIds[i]), tokenIds[i]);
        //IERC721(contractAddress).setApprovalForAll(address(this),true);
        //IERC721(contractAddress).approve(address(this),tokenIds[i]); 
        
         address _seller = IERC721(contractAddress).ownerOf(tokenIds[i]);
        // aprove nft sell
        //IERC721(contractAddress).approve(_seller, tokenIds[i]);
        IERC721(contractAddress).setApprovalForAll(_msgSender(), true);
      
        IERC721(contractAddress).transferFrom(_seller, address(this), tokenIds[i]);
    ...
    function getStakerTokens(address staker) public view returns (uint256[] memory) {
      return (_stakers[staker].stakedCBC);
    }
    ...
    */
    ...
    function getOOOOOOwner(address contractAddress, uint256 tokenId) external view returns(address){
