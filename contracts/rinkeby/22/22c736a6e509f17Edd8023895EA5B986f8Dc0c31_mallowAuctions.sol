...
contract mallowAuctions is Ownable{
    ...
    IMetaMallow public metaMallowContract; //Reference to metaMallow contract

    modifier onlyApprovedAddresses{
        require(isApprovedAddress[msg.sender], "UNAUTHORIZED");
        _;
    }
    function setDependency(address _loveAddress, address _metaMallowAddress) external onlyOwner{
        loveContract = teste_3(_loveAddress);
        metaMallowContract = IMetaMallow(_metaMallowAddress);
    }
    ...
    function enterAuction(uint256 _auctionId, uint256[] calldata _tokenIds, bytes calldata _signature) external {
        ...
        require(metaMallowContract.isOwnerOfStakedTokens(_tokenIds, msg.sender),"Not owner");
        require(_tokenIds.length >= auctionSettings[_auctionId].req, "Insufficient reqs");
    ...
    (AllInfoRequest[] memory, AllInfoRequest[] memory, AllInfoRequest[] memory){
        ...
        for(uint256 i = _start; i < _x + _start; i++){
            ...
            Xclosed[t].auction = closed[i];
            Xclosed[t].wlArray = getAuctionWhitelists(closed[i].id);
            Xclosed[t].price = getCurrentPrice(closed[i].id);
            Xclosed[t].isWL = isWhitelisted(_wallet,closed[i].id);
            t++;
        }
        for(uint256 i = 0; i < upcoming.length; i++){
