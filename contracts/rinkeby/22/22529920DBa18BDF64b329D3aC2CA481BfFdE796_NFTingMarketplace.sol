...
contract NFTingMarketplace is NFTingAuction {
    ...
    function listOnSale(
        ...
        isTokenOwner(_nftAddress, _tokenId, _amount, _msgSender())
...
contract NFTingBase is Context {
    ...
    modifier isTokenOwner(
        ...
        if (_supportsInterface(_nftAddress, INTERFACE_ID_ERC1155)) {
            if (IERC1155(_nftAddress).balanceOf(_addr, _tokenId) < _amount) {
                revert NotTokenOwnerOrInsufficientAmount();
            }
            _;
        } else if (_supportsInterface(_nftAddress, INTERFACE_ID_ERC721)) {
            if (IERC721(_nftAddress).ownerOf(_tokenId) != _addr) {
                revert NotTokenOwnerOrInsufficientAmount();
            }
            _;
        } else {
