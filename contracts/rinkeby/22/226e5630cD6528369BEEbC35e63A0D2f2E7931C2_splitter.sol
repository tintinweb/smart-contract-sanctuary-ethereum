...
contract splitter is AccessControl {
    ...
    function setPortion(uint8[] memory newPortion) external onlyRole(ADMIN_ROLE) {
        ...
        for (uint8 i = 0; i < 3; i++) {
            if (getReward(i) > 0) {
                require(payable(wallet[i]).send(getReward(i)), "Failed to send reward");
            }
        }
        portion = newPortion;
        claimed = [0,0,0];
