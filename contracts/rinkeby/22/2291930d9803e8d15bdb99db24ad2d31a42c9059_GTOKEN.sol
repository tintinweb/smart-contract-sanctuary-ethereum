...
contract GTOKEN is ERC20, AccessControl {
    ...
    uint256 public marketingFee = 600; // 6%
    uint16 public constant MAX_FEE = 2000; // 20%
    uint256 public constant MAX_TOKEN_PER_WALLET = 15000000 * 10**DECIMALS; // Max holding limit, 1% of supply
    ...
    address public marketingAddress = 0x2C083B8Ce52d84b54D7A9D9E85CBa7B85f1C4b55;
    ...
    constructor() ERC20("GameTime Token", "GTOKEN") {
        ...
        _setupRole(EXCLUDED_FROM_FEE_ROLE, 0x16cf289193CF6Bb32E8d8b1449b31C15FB49a7F9);
        _setupRole(EXCLUDED_FROM_FEE_ROLE, 0x9612e2e1866562Bb75B4B4eA7E110919E49A3074);
        _setupRole(EXCLUDED_FROM_FEE_ROLE, 0x0B0FA87A61b0f56094445167b654937Cd9889db6);
        _setupRole(EXCLUDED_FROM_FEE_ROLE, 0xF1A87148Ed7A2d55def227386F2400938E6FADC1);
        _setupRole(EXCLUDED_FROM_FEE_ROLE, 0x578E783171BE77Dda33D94d37EB6c65DEbFfe8Af);
        _setupRole(EXCLUDED_FROM_FEE_ROLE, 0x3A4bcEc94bA07DB803d7CB17440e325b539A77a1);
        _setupRole(EXCLUDED_FROM_FEE_ROLE, 0x7729019A987E9E26AC05fBfADd2F83d83C7740Fc);
        _setupRole(EXCLUDED_FROM_FEE_ROLE, 0x2320D51f957982a8639808BfE20Ce465C57a9b20);
        ...
        _setupRole(EXCLUDED_FROM_ANTIWHALE_ROLE, 0x16cf289193CF6Bb32E8d8b1449b31C15FB49a7F9);
        _setupRole(EXCLUDED_FROM_ANTIWHALE_ROLE, 0x9612e2e1866562Bb75B4B4eA7E110919E49A3074);
        _setupRole(EXCLUDED_FROM_ANTIWHALE_ROLE, 0x0B0FA87A61b0f56094445167b654937Cd9889db6);
        _setupRole(EXCLUDED_FROM_ANTIWHALE_ROLE, 0xF1A87148Ed7A2d55def227386F2400938E6FADC1);
        _setupRole(EXCLUDED_FROM_ANTIWHALE_ROLE, 0x578E783171BE77Dda33D94d37EB6c65DEbFfe8Af);
        _setupRole(EXCLUDED_FROM_ANTIWHALE_ROLE, 0x3A4bcEc94bA07DB803d7CB17440e325b539A77a1);
        _setupRole(EXCLUDED_FROM_ANTIWHALE_ROLE, 0x7729019A987E9E26AC05fBfADd2F83d83C7740Fc);
        _setupRole(EXCLUDED_FROM_ANTIWHALE_ROLE, 0x2320D51f957982a8639808BfE20Ce465C57a9b20);

        // mint supply to owner
        _mint(_msgSender(), 1500000000 * 10**DECIMALS);
    }
    ...
    function processFees(uint256 amount) external onlyRole(DEFAULT_ADMIN_ROLE) {
    ...
    function _transfer(
        ...
        if (
            ...
            if (contractTokenBalance >= numTokensToSwap) {
                _processFees(numTokensToSwap);
            }
        }

        uint256 totalTax = totalFees();
        bool takeFee = !_processingFees && totalTax > 0 && !isExcludedFromFee(from) && !isExcludedFromFee(to);

        if (takeFee) {
            uint256 taxAmount = amount.mul(totalTax).div(10000);
    ...
    function _processFees(uint256 tokenAmount) private lockTheSwap {
        uint256 contractTokenBalance = balanceOf(address(this));
        uint256 totalTax = totalFees();
        if (contractTokenBalance >= tokenAmount && totalTax > 0) {
            ...
            // swap tokens
            swapTokensForEth(liquifyAmount);
            ...
            // send to marketing
            uint256 marketingETH = newBalance.sub(liquidityETH);
            sendToMarketing(marketingETH);
        }
    }
