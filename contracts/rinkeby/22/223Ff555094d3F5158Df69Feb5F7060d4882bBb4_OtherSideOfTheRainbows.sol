...
contract OtherSideOfTheRainbows is ERC20, Ownable, Pausable {
  ...
  modifier onlyFromNoundles() {
    require(
      (msg.sender == address(Noundles)),
      "Your address doesn't have permissing"
    );
    _;
  }
  ...
  modifier onlyFromRestricted() {
    require(
      extendedAccess[msg.sender],
      'Your address does not have permission to use.'
    );
    _;
  }

  constructor(
    address noundlesAddress,
    address evilNoundlesAddress,
    address ogRainbowsAddress
  ) ERC20('NoundlesRainbows', 'Rainbows') {
  ...
  // Set the start block.
  function setIntervalAndRate(uint256 _interval, uint256 _rate)
    public
    onlyOwner
  {
    interval = _interval;
  ...
  // Set the steal rate.
  function setStealPercentage(uint256 _arg) public onlyOwner {
    stealPercentage = _arg;
  }

  // Set useMultiple.
  function setUseMultiple(bool _arg) public onlyOwner {
    useMultiple = _arg;
  }

  // Set the land protection rate.
  function setLandProtectionRates(
    uint256 _low,
    uint256 _mid,
    uint256 _high
  ) public onlyOwner {
  ...
  // Set the start block.
  function setCompanionIntervalAndRate(uint256 _interval, uint256 _rate)
    public
    onlyOwner
  {
  ...
  function burnMultiple(address[] memory users, uint256[] memory amount)
    external
    onlyFromRestricted
  {
  ...
  function adminCreate(address[] memory users, uint256[] memory amount)
    public
    onlyOwner
  {
    for (uint256 i = 0; i < users.length; i += 1) {
  ...
  function getOGRainbowsClaimable(address user)
    external
    view
    returns (uint256)
  {
    ...
    if (
      UseOriginalRainbowsBalance == false ||
      address(OriginalRainbows) == address(0)
    ) {
      return 0;
    }
  ...
  // The rewards to the user.
  function getTotalCompanionClaimable(address user)
    external
    view
    returns (uint256)
  {
  ...
  // The rewards to the user.
  function getLastUpdateCompanion(address user)
    external
    view
    returns (uint256)
  {
  ...
  // Set the address for the contract.
  function setLastUpdate(address[] memory _noundles, uint256[] memory values)
    public
    onlyOwner
  {
  ...
  // Set the address for the contract.
  function setLastUpdateCompanion(
    address[] memory _noundles,
    ...
    whenNotPaused
  {
    ...
    if (_from != address(0)) {
      ...
      companionRewards[_from] +=
        (getPendingCompanionReward(_from) * (100 - tradeLoss)) /
        100;
  ...
  function claimReward() external whenNotPaused {
    ...
    // Add up the totals.
    uint256 totalRewardsWithoutEvil = _ogRewards +
      _compRewards +
      pendingOGRewards +
      pendingCompanionRewards;

    // Block if we hit our limit.
    require(
      totalSupply() + totalRewardsWithoutEvil < maximumSupply,
      'No longer able to mint tokens.'
    );
    ...
    if (stealingEnabled) {
      ...
      uint256 rewardPerEvilNoundle = (percent * calculatedStealPercentage) /
        evilNoundleLists.length;
    ...
    uint256 totalRewards = (percent * (100 - calculatedStealPercentage)) +
      _evilRewards +
      pendingOGRainbows;
  ...
  function getPendingOGReward(address user) public view returns (uint256) {
    ...
    return
      (Noundles.noundleBalance(user) *
        rate *
        (block.timestamp -
          (lastUpdate[user] >= startBlock ? lastUpdate[user] : startBlock))) /
      interval;
  }
    ...
    returns (uint256)
  {
    ...
    return
      (EvilNoundles.companionBalance(user) *
        companionRate *
        (block.timestamp -
          (
            lastUpdateCompanion[user] >= startBlockCompanion
              ? lastUpdateCompanion[user]
              : startBlockCompanion
          ))) / companionInterval;
  }
  ...
  function getOGRainbowsPendingUnclaimed(address user)
    public
    view
    returns (uint256)
  {
