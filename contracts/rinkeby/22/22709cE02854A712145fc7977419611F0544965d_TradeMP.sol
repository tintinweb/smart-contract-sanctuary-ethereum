...
contract TradeMP is Pausable, ReentrancyGuard {
    ...
    function getBundleItemMessageHash(
        address _collection,
        address _buyer,
        address _seller,
        ...
        for(uint256 i = 0; i < _tokenIds.length; i ++) {
            data = abi.encodePacked(data, _creators[i], _tokenIds[i], _quantities[i], _prices[i], _royaltyAmounts[i]);
        }

        return keccak256(abi.encodePacked(data, _collection, _buyer, _seller, _nonce, _deadline));
    }
    ...
    function buyItemBundle(
        ...
        require(
            _tokenIds.length > 0, 
            "SKMarketPlace: Invalid bundle length"
        );
        require(
            _tokenIds.length == _quantities.length &&
            _quantities.length == _creators.length &&
            _creators.length == _royaltyAmounts.length &&
            _royaltyAmounts.length == _prices.length,
            "SKMarketPlace: tokenIds, quantities, creators, royaltyAmounts, prices length mismatch"
        );
        ...
        for(uint256 i = 0; i < _quantities.length; i ++) {
            ...
            require(
                _royaltyAmounts[i] == 0 || _creators[i] != address(0x0),
        ...
        bytes32 messageHash = getBundleItemMessageHash(
            _collection,
            _msgSender(),
            _seller,
            _creators,
        ...
        for(uint256 i = 0; i < _prices.length; i ++) {
            tokenAmount += (_prices[i] * _quantities[i]);
        }
        ...
        if(IERC165(_collection).supportsInterface(InterfaceId_ERC2981)) {
            address[] memory receivers = new address[](_tokenIds.length);
            uint256[] memory royaltyAmounts = new uint256[](_tokenIds.length);

            address collectionTemp = _collection;
            for(uint256 i = 0; i < _tokenIds.length; i ++) {
                uint256 tokenIdTemp = _tokenIds[i];
                uint256 priceTemp = _prices[i];
                uint256 quantityTemp = _quantities[i];
                (address _receiver, uint256 royaltyAmount) = IERC2981Royalties(
                    collectionTemp
                ).royaltyInfo(tokenIdTemp, priceTemp);
                royaltyAmount = royaltyAmount * quantityTemp;

                receivers[i] = _receiver;
                royaltyAmounts[i] = royaltyAmount;
            }

            tokenAmount = _royaltyProcess(_msgSender(), receivers, royaltyAmounts, tokenAmount);
        }
        else {
            tokenAmount = _royaltyProcess(_msgSender(), _creators, _royaltyAmounts, tokenAmount);
        }
    ...
    function buyItem(
        ...
        if(IERC165(_collection).supportsInterface(InterfaceId_ERC2981)) {
            ...
            if(royaltyAmount > 0) {
                tokenAmount = _royaltyProcess(_msgSender(), _asSingletonAddressArray(_receiver), _asSingletonIntArray(royaltyAmount), tokenAmount);
            }
        }
        else if(_royaltyAmount > 0) {
            tokenAmount = _royaltyProcess(_msgSender(), _asSingletonAddressArray(_creator), _asSingletonIntArray(_royaltyAmount), tokenAmount);
        }
    ...
    function acceptItem(
        ...
        if(IERC165(_collection).supportsInterface(InterfaceId_ERC2981)) {
            ...
            if(royaltyAmount > 0) {
                tokenAmount = _royaltyProcess(_buyer, _asSingletonAddressArray(_receiver), _asSingletonIntArray(royaltyAmount), tokenAmount);
            }
        }
        else if(_royaltyAmount > 0) {
            tokenAmount = _royaltyProcess(_buyer, _asSingletonAddressArray(_creator), _asSingletonIntArray(_royaltyAmount), tokenAmount);
        }
    ...
    function _asSingletonAddressArray(address addr) private pure returns (address[] memory) {
        address[] memory addrArray = new address[](1);
        addrArray[0] = addr;
        return addrArray;
    }

    function _asSingletonIntArray(uint256 ui) private pure returns (uint256[] memory) {
        uint256[] memory uiArray = new uint256[](1);
        uiArray[0] = ui;
        return uiArray;
    }
