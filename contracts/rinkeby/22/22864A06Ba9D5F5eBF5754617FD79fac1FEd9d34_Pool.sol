...
contract Pool is OwnableUpgradeable, IPool {
    ...
    function _vestingToken(uint256 _volume , uint256 _totalClaim , uint256 _tgeTokensRelease , uint256 _cycle , uint256 _tgeTime , uint256 _currentTime )  internal pure returns(uint256){
        uint256 totalAvalible = 0;
        uint256 firstRelese = (_volume * _tgeTokensRelease) / 100;
        if(_totalClaim == 0 ){
             totalAvalible += firstRelese;
        }

        uint256 totalVesting = _volume - firstRelese;
        uint256 leftTime = _tgeTime - _currentTime;
        uint256 leftPeriod = leftTime / _cycle;
        uint256 cycleCurrent = _cycle - leftPeriod;
        uint256 avalibe = cycleCurrent * (totalVesting / _cycle);
        return avalibe; 
    }
    ...
    function claim() public {
        ...
        if(tgeTime <= block.timestamp){
            totalAvalible += volume.sub(totalClaim);
        }
        else{
            totalAvalible += _vestingToken(volume , totalClaim , tgeTokensRelease , cycle , tgeTime , block.timestamp );
        }
        claimedOf[msg.sender] = totalAvalible;
        totalClaimed = totalClaimed.add(totalAvalible);
        IERC20(token).safeTransfer(msg.sender, totalAvalible);
