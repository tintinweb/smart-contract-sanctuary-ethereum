...
contract Marketplace is IMarketplace, Ownable {
	mapping(address => bool) public _advisors;
	...
	modifier onlyAdvisor(address _addr) {
		require(_advisors[_addr], 'The caller must be the Advisor.');
		_;
	}

	modifier onlyContractor(bytes32 _offerId, address _addr) {
		require(
			_offers[_offerId].seller == _addr || _offers[_offerId].buyer == _addr,
	...
	function sendOffer(
		...
		bytes32 _generatedId = keccak256(abi.encodePacked(_seller, _buyer));
		_offers[_generatedId] = Offer(
			_seller,
			_buyer,
			_collection,
			_assetId,
			_token,
			_price,
			false, // buyer_accepted
			false, // seller_accepted
			false, // buyer_declined
			false, // seller_declined
			false, // offer_achieved
			false // offer_canceled
		);
	...
	function acceptOffer(bytes32 _offerId) public onlyContractor(_offerId, msg.sender) {
		Offer memory _offer = _offers[_offerId];
		...
		if (msg.sender == _offer.seller) {
			_offers[_offerId].sellerAccepted = true;
		} else {
			_offers[_offerId].buyerAccepted = true;
		}

		if (
			!_offers[_offerId].achieved &&
			_offers[_offerId].buyerAccepted &&
			_offers[_offerId].sellerAccepted
		) {
			...
			_offers[_offerId].achieved = true;
		}
	}

	function cancelOffer(bytes32 _offerId) public onlyAdvisor(msg.sender) {
		require(!_offers[_offerId].canceled, 'The offer already canceled.');

		emit OfferCanceled(_offerId);

		_offers[_offerId].canceled = true;
	}

	function declineOffer(bytes32 _offerId) public onlyContractor(_offerId, msg.sender) {
		require(!_offers[_offerId].canceled, 'The offer already canceled.');

		if (_offers[_offerId].seller == msg.sender) {
			_offers[_offerId].sellerDeclined = true;
		} else {
			_offers[_offerId].buyerDeclined = true;
		}
	}
}
