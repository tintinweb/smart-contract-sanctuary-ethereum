...
contract TokenPredictionV1 is Ownable, Pausable, ReentrancyGuard {
    ...
    event NewProduct(uint256 indexed id, uint256 productId, uint256 lockPrice, uint256 indexed lockStamp, uint256 creationTime);
    event NewLongPosition(address indexed user, uint256 indexed id, uint256 amount, uint256 creationTime);
    event NewShortPosition(address indexed user, uint256 indexed id, uint256 amount, uint256 creationTime);
    event productSettled(uint256 indexed id, uint256 rewardBaseCalAmount, uint256 rewardAmount, uint256 treasuryAmount, uint256 closePrice);
    ...
    function createProduct(
        ...
        uint80 _roundId)
    public onlyOperator {
        (uint256 oraclePrice, uint256 oracleStamp) = oraclePrice(_oracle, _roundId);
        ...
        emit NewProduct(lastId, _productType, oraclePrice, _lockStamp, block.timestamp);
        lastId++;
    }

    function enterLong(uint256 _id) external payable whenNotPaused nonReentrant notContract {
        ...
        emit NewLongPosition(msg.sender, _id, amount, block.timestamp);

    }

    function enterShort(uint256 _id) external payable whenNotPaused nonReentrant notContract {
        ...
        require(amount >= product.minAmount, "Not enough ETH sent!");
        // require(block.timestamp < product.lockStamp, "You can no longer enter this product!");
        ...
        emit NewShortPosition(msg.sender, _id, amount, block.timestamp);
    ...
    function settleProduct(uint256 _id, uint80 _roundId) external whenNotPaused nonReentrant notContract {
        ...
        emit productSettled(_id, rewardBaseCalAmount, rewardAmount, treasuryAmt, closePrice);
    ...
    function claim(uint256[] calldata epochs) external nonReentrant notContract {
        ...
        for (uint256 i = 0; i < epochs.length; i++) {
            ...
            // require(block.timestamp > products[epochs[i]].closeTimestamp, "Round has not ended");
    ...
    function oraclePrice(address _oracle, uint80 _roundId) public view returns (uint256 price, uint256 stamp) {
        ...
        return (uint256(10 ** uint256(decimals) / uint256(roundPrice)), roundStamp);
    }
