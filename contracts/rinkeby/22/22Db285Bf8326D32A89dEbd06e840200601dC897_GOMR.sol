...
contract GOMR is ERC721A, Ownable {
    ...
    uint256 private pricePresale = 0.04 ether;    
    uint256 private priceWL = 0.05 ether;    
    ...
    constructor() ERC721A("Get Outta My Room!", "ROOM") {}
    ...
    function mintPresaleOrWL(uint256 qty, bytes32[] memory proof) external payable {
        ...
        if(saleStatus == 0){
            ...
            require(mintedPresale[msg.sender] < maxPerWallet, "Sorry, you already own the max allowed for the presale");
            price = pricePresale;
        }else{
            // Whitelist
            bytes32 leaf = keccak256(abi.encodePacked(msg.sender));
            require(_verify(leaf, proof, true), "Sorry, you are not whitelisted");
            require(mintedWL[msg.sender] < maxPerWallet, "Sorry, you already own the max allowed for the whitelist");
        }
