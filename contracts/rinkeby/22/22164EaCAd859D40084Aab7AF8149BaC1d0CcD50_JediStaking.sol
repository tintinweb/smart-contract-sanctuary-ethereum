...
contract JediStaking is Ownable, ReentrancyGuard {  
   ...
   /// @return true if transaction is successful
    ...
    function pauseToggle() external onlyOwner returns(bool) {
        isPaused = !isPaused;
   ...
   /// @return true if transaction is successful
    ...
    function setFeeReciever(address _reciever) external onlyOwner returns(bool) {
        require(_reciever != address(0), "Reciever can't be zero address");
        _feeReciever = _reciever;
    ...
    function stake(uint256 _amount) external nonReentrant returns(bool) {
        uint256 fee = (_amount * _depositFee) / 100;
        _lpToken.transferFrom(msg.sender, address(this), _amount - fee);
        _lpToken.transferFrom(msg.sender, _feeReciever, fee);

        Staker storage s = _stakers[msg.sender];
        s.amount += _amount - fee;
    ...
    //     require(!isPaused, "Paused customly!");
    ...
    function claimPausable() external nonReentrant notPaused returns(bool) {
        Staker storage c = _stakers[msg.sender];
    ...
    // function setSafeWithdrawalFee(uint256 fee) external onlyOwner returns(bool) {
    //     require(fee > 0 && fee <= 5, "Fee must be between 1-99 range");
    //     _withdrawFee = fee;

    //     return true;
    // }

    function setWithdrawalFee(uint256 fee) external onlyOwner returns(bool) {
        require(fee > 0 && fee <= 100, "Fee must be between 1-99 range");
        _withdrawFee = fee;

        return true;
    }

    // function setDepositFee(uint256 fee) external onlyOwner returns(bool) {
    //     require(fee > 0 && fee <= 100, "Fee must be between 1-99 range");
    //     _depositFee = fee;
    ...
    function setDepositFee(uint256 fee) external onlyOwner returns(bool) {
        require(fee > 0 && fee <= 20, "Fee must be between 1-20 range");
        _depositFee = fee;
    ...
    function unstakePausable() external nonReentrant notPaused returns(bool) {
        require(!isPaused, "Paused customly!");
        ...
        uint256 amountToSend = u.amount - (u.amount * _withdrawFee) / 100;
        _lpToken.transfer(msg.sender, amountToSend);
        u.amount = 0;
        u.claimed = 0;
    ...
    // function unstakeEmergency() external nonReentrant returns(bool) {
    //     require(!isPaused, "Paused customly!");
    //     Staker storage u = _stakers[msg.sender];
