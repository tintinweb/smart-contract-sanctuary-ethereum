...
contract ReentalCrowdsale is UUPSUpgradeableByRole, RefundableUpgradeable {
  ...
  event ListedToken(address token, uint256 price, uint256 poolPercent);
  ...
  function _getRate(
    ...
    uint div = uint256(10**uint256(18 - decimals));
  ...
  function _contribution(
    ...
    // 1 wei tolerance applied
    require(tokens >= minimum - 1, "ReentalCrowdsale: contribution failed, not enough tokens");
    require(tokens <= getTokensLeft(token), "ReentalCrowdsale: not enough tokens left");
  ...
  function contributionExactETHForTokens(
    ...
    uint256 tokens = _contribution(account, token, amounts[amounts.length - 1]);
  ...
  function contributionDividendsByTokensOut(
    ...
    uint256 tokens = _contribution(account, token, amountIn);
  ...
  function contributionTokensForExactTokens(
    ...
    IERC20Upgradeable(entryToken).transferFrom(account, address(this), amountInMax);

    if (entryToken == IReentalManager(config).get(keccak256("USDT"))) {
      uint256 tokens = _contribution(account, token, amountInMax);
      emit Contribution(token, entryToken, account, tokens, 0, false);
    } else {
      IERC20Upgradeable(entryToken).approve(router, amountInMax);
      uint256 amountOut = getAmountFromTokens(token, tokensOut);
      ...
      if (payback > 0) {
        IERC20Upgradeable(entryToken).transfer(account, payback);
      }
  ...
  function getTokensLeft(address token) public view returns(uint256){
    ...
    return tokenInterface.cap() - (tokenInterface.totalSupply() + ((tokenInterface.cap() * poolPercent[token]) / 100 ether));
  }
  ...
  function getTokensFromAmount(address token, uint256 amount) public view returns(uint256){
    ...
    return amount * 1 ether * usdtRate / price[token] / eurRate;
  }
  ...
  function getTokensOut(address token, uint256 amountIn, address[] calldata path) public view checkPath(path) returns (uint256) {
    ...
    if (entryToken == usdt) {
      ...
      return tokens > left ? left : tokens;
    } else {
  ...
  function listToken(address token, uint256 price_, uint256 poolPercent_) public onlyRole(_CROWDSALE_ADMIN_ROLE) {
    ...
    emit ListedToken(token, price_, poolPercent_);
  }
}
