...
contract VoteExecutorMaster is
    ...
    function getSubmitedData(uint256 _dataId) external view returns(bytes memory data, uint256 time, bytes[] memory signs){
        data = submittedData[_dataId].data;
        time = submittedData[_dataId].time;
        signs = submittedData[_dataId].signs;
    }
    
    function decodeData(bytes memory _data) public pure returns(bytes32 dataHash, Message[] memory messages){
        (dataHash, messages) = abi.decode(_data, (bytes32, Message[]));
    } 

    function decodeFinalData(bytes memory _finalData) public pure returns(bytes memory data, bytes[] memory signs){
        (data, signs) = abi.decode(_finalData, (bytes, bytes[]));
    } 
