    ...
    ReentrancyGuardUpgradeable
{
    ...
    function claimNft(
        ...
        require(
            _tokenIdSource.length == (_tokenIdDest.length + _tokenURI.length),
            "Not mapping array NFT"
        );
        require(
            _checkClaimNftSign(
                _signature,
        ...
        uint256[] memory tokenIdSourceUnlock = new uint256[](
            _tokenIdDest.length
        );
        uint256[] memory tokenIdSourceMint = new uint256[](
            _tokenIdSource.length - _tokenIdDest.length
        );
        for (uint256 i; i < _tokenIdSource.length; i++) {
            if (i < _tokenIdDest.length) {
                tokenIdSourceUnlock[i] = _tokenIdSource[i];
            } else {
                tokenIdSourceMint[i - _tokenIdDest.length] = _tokenIdSource[i];
            }
        }
        if (_tokenIdDest.length > 0) {
            require(
            ...
            for (uint256 i; i < _tokenIdDest.length; i++) {
                require(
                    IMintNFTToon(tooNft).ownerOf(_tokenIdDest[i]) ==
                        address(this),
...
contract Validator is EIP712Upgradeable {
    ...
    // keccak256("ClaimNft(address _recipient,uint256[] _arrTokenIdDest,uint256[] _arrTokenIdSource,bytes32 _interchainTX,uint256 _chainIdDest,uint256 _chainIdSource,string _tokenURI,uint256 _timeEnd)");
    bytes32 public constant CLAIM_NFT_TYPEHASH =
        0xd1f460de8aef19ede8a0dc29c128eb1a50544fc93dfead6ddcff169f449f2b81;
    ...
    function _checkClaimNftSign(
        ...
        string memory uriConcat;
        if (_tokenURI.length > 0) {
            uriConcat = _tokenURI[0];
