...
contract DevAuctionV2 is DutchAuctionV2
{
    ...
    {        
        ...
        for (uint256 index = 0; index < stageInfos.length; ++index) {
            ...
            _linearPrices[_stages.length - 1] = priceInfos[index];
            uint256 startPrice = priceInfos[index].startPrice;
            uint256 endPrice = priceInfos[index].endPrice;
    ...
    {        
        ...
        for (uint256 index = 0; index < stageInfos.length; ++index) {
            ...
            _stages.push(stageInfo);
            _manualPrices[_stages.length - 1] = priceInfos[index];
            uint256 startPrice = priceInfos[index].prices[0];
            uint256 endPrice = priceInfos[index].prices[priceInfos[index].prices.length - 1];
            emit StageAdded(_stages.length - 1, stageInfo, startPrice, endPrice);
        }
    }
}
