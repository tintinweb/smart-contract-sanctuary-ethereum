...
contract BufferPool is IBufferPool, Ownable, CheckContract {
    uint256 public ETH;
    uint256 public USDC;
    ...
    onlyOwner
    {
        collatralPoolAddress = _collateralPoolAddress;
        ...
        emit CollateralPoolAddress(_collateralPoolAddress);
        emit ActivePoolAddress(_activePoolAddress);
        emit StakingPoolAddress(_stakingPoolAddress);
        emit UsdcTokenAddress(_usdcTokenAddress); 
    ...
    function sendETH(address _receiver, uint _amount) external payable override returns (bool) {
        _callerIsCollatralPoolOrAP();
        ETH -= _amount;
        (bool success, ) = payable(_receiver).call{ value: _amount }("");    
        require(success, "BufferPool: ETH transfer failed");
        return success;
    }

    function sendUSDC(address _receiver, uint _amount) external payable override returns (bool) {
        _callerIsStakingPool();
        USDC -= _amount;
        bool success = usdcToken.transfer(_receiver, _amount);    
        require(success, "BufferPool: USDC transfer failed");
        return success;
    }

    function sendETHToExteranlWallet(uint _amount) external payable override returns (bool) {
        _requireCallerExternalWallet();
        ETH -= _amount;
        (bool success, ) = payable(externalWalletAddress).call{ value: _amount }("");    
        require(success, "BufferPool: ETH transfer failed");
        return success;
    }

    function sendUSDCToExteranlWallet(uint _amount) external payable override returns (bool) {
        _requireCallerExternalWallet();
        USDC -= _amount;
        bool success = usdcToken.transfer(externalWalletAddress, _amount);    
        require(success, "BufferPool: USDC transfer failed");
        return success;
    }

    function sendUSDCFromExteranlWallet(uint _amount) external payable override returns (bool) {
        _requireCallerExternalWallet();
        USDC += _amount;
        bool success = usdcToken.transfer(address(this), _amount);    
        require(success, "BufferPool: USDC transfer failed");
        return success;
    }
    ...
    function _callerIsStakingPool() internal {
        require(msg.sender == stakingPoolAddress, "BufferPool, sender is not Staking Pool");
    }

     function _requireCallerExternalWallet() internal {
        require(msg.sender == externalWalletAddress, "BufferPool : Caller is not externalWalletAddress");
    }

    receive() external payable {
        ETH += msg.value;
        emit BufferPoolETHUpdated(msg.value);
    }

    function receiveUSDC(uint _amount) external override{
        _callerIsStakingPool();
        USDC += _amount;
        emit BufferPoolUSDCUpdated(_amount);   
    }
}
