...
contract CheetosDrop is ERC721Enumerable, Ownable {
    using MerkleProof for bytes32[];

    mapping(address => bool) public alreadyMinted;

    uint16 private reserveCheetosId;
    uint16 private cheetosId;
    ...
    constructor() ERC721("Pepsi Mic Drop", "PEPSIMICDROP") {
        reserveCheetosId = 1; // item 1-50
        cheetosId = 51; // item 51-1893
    }
    ...
    function mint(bytes32[] memory proof, bytes32 leaf) public returns (uint256) {
        ...
        require(cheetosId <= maxMint, "Mint limit reached");

        _safeMint(msg.sender, cheetosId++);
    ...
    function reserveMicDrops(address to, uint8 amount) public onlyOwner {
        require(reserveCheetosId + amount <= 51, "Out of stock");

        for (uint8 i = 0; i < amount; i++) _safeMint(to, reserveCheetosId++);
    }
