...
contract LendableNft is ERC721C
{
    constructor() ERC721C("Lendable NFT", "LNFTRQ") {
        _mint(msg.sender, 1);
    }
...
//ERC721C (Cosmic Labs) (Token Lending implementation built on top of OpenZeppelins ERC721) 
//Created by: RQ#9126
...
contract ERC721C is Context, ERC165, IERC721, IERC721Metadata {
    ...
    //Total ETH Saved for lenders (contract implementing ERC721C should account for eth saved for lenders inside withdrawal function)
    uint256 private _totalEthForLenders;
    ...
    modifier onlyERC721CLendee(uint256 _tokenId) {
    ...
    function leaseDetailedERC721C(address _Lendee, uint256 _tokenId, uint256 _ethValue, uint256 _lengthOfDays) public virtual {
        require(msg.sender == _owners[_tokenId], "ERC721C: msg.sender is not the owner");
        require(msg.sender != _Lendee, "ERC721C: lending to yourself?");
    ...
    function leaseERC721C(uint256 _tokenId, uint256 _ethValue, uint256 _lengthOfDays) public virtual {
        require(msg.sender == _owners[_tokenId], "ERC721C: msg.sender is not the owner");
        require(_stateOfID[_tokenId] != State.PENDING_END_OF_LEASE, "You are currently leasing");
    ...
    function payToLease(uint256 _tokenId) onlyERC721CLendee(_tokenId) public payable virtual
    {
    ...
    function endLease(uint256 _tokenId) endOfLease(_tokenId) public virtual {
        ...
        require(msg.sender == owner, "ERC721C: msg.sender is not the owner");

        if(_stateOfID[_tokenId] == State.PENDING_END_OF_LEASE)
        {
            payable(msg.sender).transfer(_ethAmount[_tokenId]);
        }
        
        _totalEthForLenders -= _ethAmount[_tokenId];
    ...
    function balanceOf(address owner) public view virtual override returns (uint256) {
        require(owner != address(0), "ERC721C: address zero is not a valid owner");
    ...
    function ownerOf(uint256 tokenId) public view virtual override returns (address) {
        ...
        require(owner != address(0), "ERC721C: owner query for nonexistent token");
    ...
    function transferFrom(
        ...
        require(_isApprovedOrOwner(_msgSender(), tokenId), "ERC721C: transfer caller is not owner nor approved");
        require(_isLent[tokenId] == false, "ERC721C: currently in a lending contract");
    ...
    function _safeTransfer(
        ...
        _transfer(from, to, tokenId);
        require(_checkOnERC721Received(from, to, tokenId, _data), "ERC721C: transfer to non ERC721Receiver implementer");
    }
    ...
    function _isApprovedOrOwner(address spender, uint256 tokenId) internal view virtual returns (bool) {
        require(_exists(tokenId), "ERC721C: operator query for nonexistent token");
        address owner = _owners[tokenId];
    ...
    function _transfer(
        ...
        require(_owners[tokenId] == from, "ERC721C: transfer from incorrect owner");
