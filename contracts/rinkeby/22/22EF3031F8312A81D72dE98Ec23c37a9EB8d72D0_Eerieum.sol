...
contract Eerieum is ERC721A, Ownable, ReentrancyGuard {
    ...
    constructor(
        ...
        string memory baseURI_) ERC721A("Eerieum", "VALUE") {
    ...
    function allowlistMint() external payable callerIsUser {
        require(hasWhiteListStarted == true, "allowlist sale has not begun yet");
        require(allowlist[msg.sender] > 0, "not eligible for allowlist mint");
        require(totalSupply() + 1 <= collectionSize, "reached max supply");
        require(_currentIndex <= 500, "reached max supply");

        allowlist[msg.sender]--;

        // Check if tokenID is less then or equal to 500
