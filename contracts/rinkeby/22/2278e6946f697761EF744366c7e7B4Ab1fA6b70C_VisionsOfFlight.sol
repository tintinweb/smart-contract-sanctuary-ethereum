...
contract VisionsOfFlight is ERC721, Ownable, Pausable {
    ...
    // REAL PRICE: uint MINT_PRICE = 8e15;
    uint MINT_PRICE = 1e14;
    ...
    // VERIFY DIVIS ST SHARE
    uint8 divisionStShare = 50;

    // add manual NFT mints for athlethes (with explicit Token ID)
    constructor(
    ...
    function freeMint(address recipient, bytes32[] calldata proof) public whenNotPaused {
        require(block.timestamp >= claimOpensAt, "Visions of Flight: claim window has not opened");
        require(numVouchers[recipient] > 0 || hasPOAPVoucher(recipient, proof), "Visions of Flight: No freemints available");


        for (uint8 i = 0; i < numVouchers[recipient]; i++) {
            mintToken(recipient);
        }
        numVouchers[recipient] = 0;

        if (hasPOAPVoucher(recipient, proof)) {
            mintToken(recipient);
            claimedPOAPVoucher[recipient] = true;
        }
    }
    ...
    function hasPOAPVoucher(address recipient, bytes32[] calldata proof) internal view returns (bool) {
    ...
    function mint(address recipient, uint8 count) public payable whenNotPaused {
        ...
        require(txsPerWallet[msg.sender] <= MAX_TXS_PER_WALLET, "Visions of Flight: User has exceeded 10 mint transactions per wallet");
