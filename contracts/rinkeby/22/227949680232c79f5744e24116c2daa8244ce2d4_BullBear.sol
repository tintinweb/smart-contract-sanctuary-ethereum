...
// @dev NFT contract that changes based on external conditions, by use of chainlink
// @dev Rinkeby rng contract info https://docs.chain.link/docs/vrf-contracts/#rinkeby-testnet
contract BullBear is ERC721, ERC721Enumerable, ERC721URIStorage, KeeperCompatibleInterface, Ownable, VRFConsumerBaseV2  {
    ...
    uint64 subId;
    bytes32 keyhash =  0xd89b2bf150e3b9e13446986e571fb9cab24b13cea0a43ea20a6049a85cc807cc; // keyhash ofr 30 Gwei, see https://vrf.chain.link/ 
    ...
    */
    ...
    enum eNftType{BULL, BEAR} // Create Enum
    eNftType public nftType = eNftType.BULL; 
    eNftType public newNftType = eNftType.BULL; 
    uint256 idx = 0;
    ...
    event TokensUpdated(string newtype, uint256 newIdx);
    error SubscriptionIsNotSet();
    ...
    function safeMint(address to) public  {
        ...
        string memory defaultUri = bullUrisIpfs[idx];
    ...
    function performUpkeep(bytes calldata /* performData */ ) external override {
        ...
        if ((block.timestamp - lastTimeStamp) > interval ) {
            ...
            if (latestPrice < currentPrice) {
                if (nftType == eNftType.BEAR) {
                    console.log("It's time for another bear");
                } else {
                    newNftType = eNftType.BEAR;
                    console.log("It's time for bears");
                }
            } else {
                if (nftType == eNftType.BULL) {
                    console.log("It's time for another bull");
                } else {
                    newNftType = eNftType.BULL;
                    console.log("It's time for bulls");
                }
            }
            ...
            // Will revert if subscription is not set.
            if (subId == 0)
                revert SubscriptionIsNotSet();

            // Will revert if subscription is not funded.
            reqId = coordinator.requestRandomWords(
                keyhash,
                subId, // See https://vrf.chain.link/
    ...
    // @dev Overrides virtual interface from VRFConsumerBaseV2
    function fulfillRandomWords(
        ...
        string[] memory nftUris = (newNftType == eNftType.BULL ? bullUrisIpfs : bearUrisIpfs);
        string memory typeStr = (newNftType == eNftType.BULL ? "Bull" : "Bear");

        if (newNftType == nftType)
        {        
            uint256 step = randomWords[0] % (nftUris.length - 1);
            idx = (idx + step) % nftUris.length;
        } else {
            idx = randomWords[0] % nftUris.length;
            nftType = newNftType;
        }

        for (uint i = 0; i < _tokenIdCounter.current() ; i++) {
            _setTokenURI(i, nftUris[idx]);
        } 
        
        emit TokensUpdated(typeStr, idx);
