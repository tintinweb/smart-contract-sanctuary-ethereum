...
contract Theirsverse is ERC721A, Ownable, AccessControl, ReentrancyGuard {
    uint256 public immutable maxSupply;
    IStarNFT public immutable starNFT;
    address payable public immutable payment;
    ...
    constructor(
        ...
        string memory _baseURIPart1,
        string memory _baseURIPart2
        ...
        baseURI = string(abi.encodePacked(_baseURIPart1, addressToString(address(this)), _baseURIPart2));
    }
    ...
    function glodMemberMint(uint256 _quantity) external payable _notContract _saleBetweenPeriod(0, 1) nonReentrant {
        ...
        require(nfts.length == neededNFTs, "Goldmember does not own enought verified StarNFTs");
    ...
    function whitelistMint(
        ...
        uint16 phase
    ) external payable _notContract _saleBetweenPeriod(1, 2) nonReentrant {
        if (phase == 0 || phase == 1) {
            require(
    ...
    function publicSaleMint(uint256 _quantity) external payable _notContract _saleBetweenPeriod(2, 3) nonReentrant {
        ...
        nonReentrant
    {
        ...
        for (uint256 i = 0; i < _accounts.length; i++) {
            ...
            require(nfts.length == neededNFT, "Goldmember does not own enought verified StarNFTs");
            for (uint256 j = 0; j < neededNFT; j++) {
                amountNFTsGoldMemberSale[nfts[j]] += 10;
            }
            _safeMint(_accounts[i], quantity);
        }
    }
    ...
    function claim(address _account, uint256 _quantity) external onlyRole(DEFAULT_ADMIN_ROLE) nonReentrant {
        require(totalSupply() + _quantity <= maxSupply, "Max supply exceed");
