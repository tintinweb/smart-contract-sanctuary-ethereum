...
contract HATVaultsNFT is ERC1155, Ownable {
    ...
    uint256 public tokenIdsCounter;
    mapping(address => mapping(uint256 => bool)) public pausedVaults;
    mapping(address => mapping(uint256 => mapping(uint8 => uint256))) public tokenIds;
    mapping(address => mapping(uint256 => mapping(uint8 => mapping(address => bool)))) public tokensRedeemed;
    ...
    modifier notPaused(address hatVaults, uint256 pid) {
        require(!pausedVaults[hatVaults][pid], "Vault paused");
        _;
    }
    ...
    function _mintTokens(
        ...
        for(uint8 i = 1; i <= tier; i++) {
            require(tokenIds[hatVaults][pid][i] != 0, "Token does not exist");
            if (!tokensRedeemed[hatVaults][pid][i][account]) {
                tokensRedeemed[hatVaults][pid][i][account] = true;
                _mint(account, tokenIds[hatVaults][pid][i]);
            }
        }
    }
    ...
    function isEligible(
        ...
        uint8 tier = getTierFromShares(hatVaults, pid, account);
        return tier != 0 && (tokenIds[hatVaults][pid][tier] != 0 && !tokensRedeemed[hatVaults][pid][tier][account]);
    }
