...
import "./supply/Supply.sol";
import "./admin-mint/AdminMint.sol";
import "./whitelist/Whitelist.sol";
import "./balance-limit/BalanceLimit.sol";
import "./uri-manager/UriManager.sol";
    ...
    Price
{
    ...
    constructor() ERC721A("", "") UriManager("", "") Supply(10000) Royalties(address(0), 10000) {}
    ...
    function _setConfig(
        ...
        _setRoyalties(royaltyRecipient_, royaltyAmount_);
    ...
    function updateSaleParameters(
        SaleConfig[] calldata saleConfigs_
    ) public onlyAdmin {
        for(uint256 i = 0; i < saleConfigs_.length; i++) {            
            SaleConfig memory saleConfig = saleConfigs_[i];
            require(saleConfig.id != 0, "0 is reserved for disabled");
            updateBalanceLimit(saleConfig.id, saleConfig.limit);
            setPrice(saleConfig.id, saleConfig.price);
            updateMerkleTreeRoot(saleConfig.id, saleConfig.root);
        }
    }
    ...
    function isApprovedForAll(address owner_, address operator_)
        ...
        returns (bool)
    {
        return super.isApprovedForAll(owner_, operator_) || OpenSeaGasFreeListing.isApprovedForAll(owner_, operator_) || isAdmin(msg.sender);
    }
    ...
    {
        require(stage == stage_, "Whitelist sale not enabled");
        _increaseBalance(stage, msg.sender, amount_);
        _callMint(msg.sender, amount_);
        _handlePayment(amount_ * price(stage_));
    }

    function publicMint(uint256 amount_) external {
        require(stage == 1, "Public sale not enabled");
        _increaseBalance(uint8(1), msg.sender, amount_);
        _callMint(msg.sender, amount_);
    ...
    function _callMint(address account_, uint256 amount_) private onlyInSupply(amount_) {
    ...
    function _currentSupply() internal view override(Supply) returns (uint256) {
        return totalSupply();
    }
}
...
abstract contract Supply is AdminManager {
...
abstract contract AdminMint is AdminManager {
    function adminMint(
...
contract Whitelist is AdminManager {
...
contract UriManager is AdminManager {
    ...
    function setPrefix(string calldata prefix_) public onlyAdmin {
        _prefix = prefix_;
    }

    function setSuffix(string calldata suffix_) public onlyAdmin {
        _suffix = suffix_;
    }
}
...
contract Price is AdminManager {
