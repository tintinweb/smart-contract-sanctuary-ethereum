...
contract Factory1155 {
	...
	function deploy(bytes32 _salt, string memory name, string memory symbol, string memory tokenURIPrefix) external returns(address addr) {
		addr = address(new RUSTNFTUserToken1155{salt: _salt}(name, symbol, tokenURIPrefix));
		RUSTNFTUserToken1155 token = RUSTNFTUserToken1155(address(addr));
...
contract RUSTNFTUserToken1155 is ERC1155 {
    ...
    function mint(string memory uri, uint256 supply, address[] memory royaltyAddress, uint256[] memory royaltyFee)  public {
