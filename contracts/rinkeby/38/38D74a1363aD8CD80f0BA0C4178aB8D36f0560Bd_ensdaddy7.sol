...
constructor(){}
    ...
    function register_subdomain(string memory domain, string memory subdomain) external payable {
        require(
                 (_saleIsActive),
                "Minting is not Live"
        );
        ...
        ensusers[enssubdomains_count] = ensdata(enssubdomains_count, subdomain, domain, msg.sender, "Under process",msg.value);
    ...
    function is_available(string memory _subdomain) public view returns (string memory) {
        for (uint256 i=0; i< enssubdomains_count; i++) {
            if(keccak256(bytes(ensusers[i].subdomain)) == keccak256(bytes(_subdomain))){
