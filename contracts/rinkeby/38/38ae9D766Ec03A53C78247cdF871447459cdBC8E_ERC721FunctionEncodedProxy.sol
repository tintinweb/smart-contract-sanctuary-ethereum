...
contract ERC721FunctionEncodedProxy is IERC721Receiver {
    event ProxyMinted(
    ...
    event GetTokenIdFromProxy(uint256);

    fallback() external payable {}

    receive() external payable {}

    function copyBytes(bytes memory _bytes)
        private
        pure
    ...
    function proxyMint(
        ...
        bytes4 functionSignature = bytes4(
            keccak256(copyBytes(bytes(_functionSignature)))
        );

        if (mintType == 1) {
            (bool success, bytes memory mintData) = address(nftAssetContract)
                .call{value: msg.value}(
                abi.encodePacked(functionSignature, uintParamA)
            );

            uint256 tokenId = abi.decode(mintData, (uint256));
            emit GetTokenIdFromProxy(tokenId);
            ERC721(nftAssetContract).safeTransferFrom(
                address(this),
                addressParamA,
                tokenId
            );
            ...
            (bool success,) = address(nftAssetContract)
                .call(
            ...
            revert("Invalid mint type");
        }

        // ERC721(nftAssetContract).safeTransferFrom(address(this), addressParamA, tokenId);
        // require(success, "Execution failed");
    }
