...
contract testGameLogic is Pausable, Ownable, IERC721Receiver, rarityCheck, Initializable {
    ...
    constructor(address _yeti, address _frxst, address _treasury, address _rs,address _vrf) Pausable() Ownable() {
    ...
    function init(address _yeti, address _frxst, address _treasury, address _rs,address _vrf)
        virtual
        public
        onlyOwner 
    {
...
//        require(frxst.balanceOf(msg.sender) > _nextLevelExp(levels[tokenId]) * LEVEL_UP_COST_MULTIPLIER, "Insufficient FRXST");
    ...
    function _claimYetiFromPalace(uint tokenId, bool unstake) internal returns (uint owedFrxst) {
        ...
        uint hourly;
        //todo: fix the hourly for Palace.
        if (stake.activityId == 1) {
            hourly = rates[c][stake.activityId] * 2;
            hourly = hourly + hourly*(levels[tokenId]-1)/10;
        } else{
            hourly = rates[c][stake.activityId];
            hourly = hourly + hourly*(levels[tokenId]-1)/10;
        }
        if (unstake){
            //todo change this to 1 days in main-net
            uint mod = (block.timestamp - stake.value) / 10;
            owedFrxst =  (hourly * mod* 1 ether);
    ...
    function _claimYetiFromFighting(uint tokenId, bool unstake) internal returns (uint owedFrxst) {
        ...
        //todo: fix hourly
        // uint hourly = rates[tokenRarity[tokenId]][stake.activityId] * levels[tokenId] * (1 + 0.1*(levels[tokenId]-1));
        uint hourly = rates[tokenRarity[tokenId]][stake.activityId] * levels[tokenId];
        hourly = hourly + hourly* (levels[tokenId]-1)/10; //
