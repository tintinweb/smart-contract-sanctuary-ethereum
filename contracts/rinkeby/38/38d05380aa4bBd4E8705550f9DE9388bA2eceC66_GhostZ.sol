...
contract GhostZ is Ownable, ERC721A, VRFConsumerBase {
        ...
        returns (string memory)
    {
        ...
        for (uint256 i = 0; i < MAX_SUPPLY - 1; i++) {
            uint256 j = i +
                (uint256(keccak256(abi.encode(_seed, i))) % (MAX_SUPPLY - i));
    ...
    function _mint(uint256 _mintAmount) internal {
        ...
        require(
            _totalMinted() + _mintAmount <= MAX_SUPPLY,
    ...
    function revealURI() external onlyOwner returns (bytes32 requestId) {
        revealed = true;
    ...
    function airdrop(address[] calldata _users, uint256[] calldata _amounts)
        external
        onlyOwner
    {
        ...
        require(
            usersLength == amountsLength,
            "Users and amounts doesn't match"
        );
        uint256 total = 0;
