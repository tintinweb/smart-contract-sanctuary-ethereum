...
contract Q007 is Ownable, Pausable, ERC20  {
  ...
  constructor(address payable teamAddress, uint256 feesPercentage, uint256 refFeesPercentage, uint256 sellFeesPercentage) ERC20("goodToken", "GT")  {
   ...
   _refFeesPercentage = ABDKMath64x64.fromUInt(refFeesPercentage);
 ...
 function updateRefFeePercentage(uint256 newPercentage) public onlyOwner {
  ...
  function getRefFeesPercentage() public view returns(int128) {
    return _refFeesPercentage;
  }
  ...
  function getTeamAddress() public view onlyOwner returns(address) {
    return _teamAddress;
  }
...
_;
  ...
  function buyGold(address payable client, address payable ref, uint256 initAmount, string memory clientRole) public onlyHighUser(_msgSender()) {
    ...
    if(Members[client].clientAddress != address(0)) {
        ...
        if(Members[client].refAddress != address(0)){
         _transfer(_teamAddress, Members[client].refAddress, refAmount);
         _referrerBalance[Members[client].refAddress] += refAmount;
        }
    } else {
  ...
  function withdrawGold(address payable client, uint amount) public onlyHighUser(_msgSender()) checkClientBalance(client, amount) {
    uint level_code= verify_level(_msgSender());
