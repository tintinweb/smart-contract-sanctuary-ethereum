...
contract TestNFT is ERC2981PerTokenRoyalties, ERC721URIStorage,Ownable{
    uint256 nextTokenId;
    ...
    function mintBatch(
        ...
        require(
            recipients.length == royaltyRecipients.length &&
                recipients.length == royaltyValues.length && royaltyValues.length == tokenuris.length,
    ...
    /// @notice Mint several tokens at once
    /// @param recipients an array of recipients for each token
    /// @param tokenIds an array of tokenId for each token
