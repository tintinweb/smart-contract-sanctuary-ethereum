...
contract NFTStaking is Ownable, Pausable {
    ...
    function addManyToPoll(address account, uint256[] calldata tokenIds, uint256 _periodType)
        public
    {
        ...
        require(_periodType >= 0 && _periodType <=3, "NFTStaking: Staking Period Type Failed.");
    ...
    function claimManyFromPoll(uint256[] calldata tokenIds)
        external
    {
        require(tx.origin == _msgSender(), "NFTStaking: Only EOA");
        uint256 owed = 0;
    ...
    function _addBotPoll(address account, uint256 tokenId, uint256 _periodType) internal {
        uint256 stakingPeriod = 0;
        uint256 dailyAmount = 0;
        if(_periodType == 0){
            stakingPeriod = 7 minutes; // 7 days
            dailyAmount = CLAIM_AMOUNT_1;
        }
        else if(_periodType == 1) {
            stakingPeriod = 14 minutes; // 14 days
            dailyAmount = CLAIM_AMOUNT_2;
        }
        else if(_periodType == 2) {
            stakingPeriod = 30 minutes; // 30 days
            dailyAmount = CLAIM_AMOUNT_3;
        }
        else if(_periodType == 3) {
            stakingPeriod = 60 minutes; // 60 days
        ...
        stakedBot++;
        emit TokenStaked(account, tokenId, block.timestamp, stakingPeriod, dailyAmount);
    }
        ...
        returns (uint256 owed)
    {
        ...
        require(lastClaimTime >= stake.periodTime, "NFTStaking: You can unstake NFT after lock time.");

        // need to fix 1 minutes to 1 days on mainnet
        owed = (stake.periodTime.div(1 minutes)).mul(stake.dailyAmount);
