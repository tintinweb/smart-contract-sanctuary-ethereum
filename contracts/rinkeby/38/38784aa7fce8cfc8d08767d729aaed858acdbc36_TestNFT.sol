    ...
    Ownable
{
    ...
    function publicMint(uint256 _amount) external payable {
        require(publicState, "TestNFT: Public is OFF");
        require(_amount > 0, "TestNFT: zero amount");
        require(
            totalNFT + _amount <= maxSupply,
            "TestNFT: max supply exceeded"
        );
        require(
            _price * _amount <= msg.value,
            "TestNFT: Ether value sent is not correct"
        );
        require(!paused, "TestNFT: contract is paused");
        uint256 _newItemId;
    ...
    function gromMint(uint256 _amount) external payable {
        require(publicState, "TestNFT: Public is OFF");
        require(_amount > 0, "TestNFT: zero amount");
        require(
            totalNFT + _amount <= maxSupply,
        ...
        require(
            _grPrice * _amount <= msg.value,
            "TestNFT: GROM value sent is not correct"
        );
        require(!paused, "TestNFT: contract is paused");
        uint256 _newItemId;
        for (uint256 ind = 0; ind < _amount; ind++) {
    ...
    function burn(uint256 tokenId) external {
        require(isBurnEnabled, "TestNFT : burning disabled");
        require(
            _isApprovedOrOwner(msg.sender, tokenId),
            "TestNFT : burn caller is not owner nor approved"
        );
        _burn(tokenId);
        totalNFT = totalNFT - 1;
    }
