...
contract AquaCultureNFT is ERC721, ERC721URIStorage, Ownable {
    ...
    function createAssetPublic(uint8 maxAllowed, string memory metadata, uint256 price) public {
        _assetIds.increment();
        uint256 newAssetId = _assetIds.current();
        assetInfo memory newAsset = assetInfo(
            msg.sender,
    ...
    function buyAndMintAsset(uint256 assetId, bytes32 hash, bytes memory signature, uint payThrough) external payable {
        address signer = hash.recover(signature);
        // require(signer == msg.sender && _nftData.creator == msg.sender, "Invalid creator signature");
    ...
    function buyAndMintAssetByAccept(uint256 assetId, address newOwner, bytes32 hash, bytes memory signature, uint256 amount) external payable {
        address signer = hash.recover(signature);
        // require(signer == msg.sender && _nftData.creator == msg.sender, "Invalid creator signature");
        assetInfo memory assetInfoById = allAssetsInfo[assetId];
        require(assetInfoById.maxMints > assetInfoById.currentMints, "NFT Not available");
        ...
        _mint(msg.sender, newTokenId);
        _setTokenURI(newTokenId, assetInfoById.metadata);
        _transfer(msg.sender, newOwner, newTokenId);
        assetInfoById.currentMints = assetInfoById.currentMints+1;
        allAssetsInfo[assetId] = assetInfoById;
        tokenInfo memory newTokenInfo = tokenInfo(
            newTokenId,
    ...
    function transferByAccept(uint256 tokenId, address newOwner, bytes32 hash, bytes memory signature, uint payThrough, uint256 amount) external payable onlyNftOwner(tokenId) {
        address signer = hash.recover(signature);
