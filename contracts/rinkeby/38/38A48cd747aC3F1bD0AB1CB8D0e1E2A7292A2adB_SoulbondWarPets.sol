...
contract SoulbondWarPets is ERC1155, AccessControl, Pausable {
    ...
    function addNation(address _activeContract, uint _warPetId, bool _switchNation, bool _replace) public onlyRole(OPERATOR_ROLE) {
        if (!_replace) {
            require(warPetToNation[_warPetId] == address(0), "This war pet belongs to other nation");
        }
        warPetToNation[_warPetId] = _activeContract;
        
        if(_switchNation) switchNation(_warPetId);
    }

    function switchNation(uint _warPetId) public onlyRole(OPERATOR_ROLE) {
        require(warPetToNation[_warPetId] != address(0), "Selected war pet has no nation");
        warPetId = _warPetId;
    }
