...
contract Amm is IAmm, AmmStorageV1, BlockContext, OwnerPausable, ReentrancyGuardUpgradeable {
    ...
    // Events
    event CalcEvent(uint256 tag, uint256 value);
    ...
    function initialize(InitializeParams memory params) external initializer {
        ...
        // Pack A and gamma:
        // shifted A + gamma
        uint256 A_gamma = SigmaMath.shift(params.A, 128);
        A_gamma = SigmaMath.bitwise_or(A_gamma, params.gamma);
        _globalInitializeParams.initialAGamma = A_gamma;
        _globalFutureParams.futureAGamma = A_gamma;
        ...
        )
    {
        ...
        al.xp_old = SigmaMath.copy(al.xp);
        ...
        al.xp_old = [al.xp_old[0] * _coinPrecisions[0], (al.xp_old[1] * priceScaleTemp) / PRECISION];
        ...
        if (t > 0) {
            al.old_D = AmmLib.newton_D(al.A_gamma[0], al.A_gamma[1], al.xp_old);
        ...
        uint256 D = AmmLib.newton_D(al.A_gamma[0], al.A_gamma[1], al.xp);

        uint256 liquidity;
        if (al.old_D > 0) {
            liquidity = (_totalLiquidity * D) / al.old_D - _totalLiquidity;
        } else {
            liquidity = _getXcp(D); // making initial virtual price equal to 1
        }
        ...
        if (al.old_D > 0) {
            _tweakPrice(al.A_gamma, al.xp, 0, D);
        } else {
            _globalD = D;
        }
    ...
    function removeLiquidity(
        ...
        uint256 total_supply = _totalLiquidity;
        _totalLiquidity -= liquidity;
        ...
        for (uint8 i = 0; i < N_COINS; i++) {
            uint256 tokenAmount = (tokenAmounts[i] * calcLiquidity) / total_supply;
        ...
        if (_totalLiquidity != 0) {
            _globalD = _globalD - (_globalD * calcLiquidity) / total_supply;
        } else {
            _globalD = 0;
        }
    ...
    //     uint256[2] memory xp_old = SigmaMath.copy(xp);
    ...
    //     xp_old = [xp_old[0] * _coinPrecisions[0], (xp_old[1] * _priceScale * _coinPrecisions[1]) / PRECISION];

    //     uint256 old_D;
    //     uint256 t = _globalFutureParams.futureAGammaTime;
    //     if (t > 0) {
    //         old_D = AmmLib.newton_D(A_gamma[0], A_gamma[1], xp_old);
    //     } else {
    //         old_D = _globalD;
    //     }

    //     uint256 new_D = AmmLib.newton_D(A_gamma[0], A_gamma[1], xp);

    //     uint256 liquidity;
    //     if (old_D > 0) {
    //         liquidity = (_totalLiquidity * new_D) / old_D - _totalLiquidity;
    //     } else {
    //         liquidity = _getXcp(new_D);
    //     }
        ...
        returns (uint256)
    {
        ...
        emit CalcEvent(10, amounts[0]);
        emit CalcEvent(11, amounts[1]);

        uint256[2] memory A_gamma = _A_gamma();
        emit CalcEvent(20, A_gamma[0]);
        emit CalcEvent(21, A_gamma[1]);

        uint256[2] memory xp = SigmaMath.copy(_coinBalances);
        emit CalcEvent(30, xp[0]);
        emit CalcEvent(31, xp[1]);

        uint256[2] memory xp_old = SigmaMath.copy(xp);
        emit CalcEvent(40, xp_old[0]);
        emit CalcEvent(41, xp_old[1]);

        for (uint8 i = 0; i < N_COINS; i++) {
            uint256 bal = xp[i] + amounts[i];
            xp[i] = bal;
        }
        emit CalcEvent(50, xp[0]);
        emit CalcEvent(51, xp[1]);

        xp = [xp[0] * _coinPrecisions[0], (xp[1] * _priceScale * _coinPrecisions[1]) / PRECISION];
        emit CalcEvent(60, xp[0]);
        emit CalcEvent(61, xp[1]);

        xp_old = [xp_old[0] * _coinPrecisions[0], (xp_old[1] * _priceScale * _coinPrecisions[1]) / PRECISION];
        emit CalcEvent(70, xp_old[0]);
        emit CalcEvent(71, xp_old[1]);
        ...
        emit CalcEvent(80, old_D);

        uint256 new_D = AmmLib.newton_D(A_gamma[0], A_gamma[1], xp);
        emit CalcEvent(81, new_D);
        ...
        )
    {
        ...
        try this.quoteSwapInternal(i, j, dx, dyLimit, isExactInput) {} catch (bytes memory reason) {
            if (reason.length != 96) {
    ...
    function _swap(
        ...
        sp.prec_i = _coinPrecisions[0];
        sp.prec_j = _coinPrecisions[1];

        if (i == 1) {
            sp.prec_i = _coinPrecisions[1];
            sp.prec_j = _coinPrecisions[0];
        }

        {
            ...
            if (t > 0) {
                ...
                sp.xp[i] = sp.x0;
                _globalD = AmmLib.newton_D(sp.A_gamma[0], sp.A_gamma[1], sp.xp);
        ...
        if (isExactInput) {
            ...
            dy = sp.xp[j] - AmmLib.newton_y(sp.A_gamma[0], sp.A_gamma[1], sp.xp, _globalD, j);
            ...
            dy = AmmLib.newton_y(sp.A_gamma[0], sp.A_gamma[1], sp.xp, _globalD, j) - sp.xp[j];
        ...
        // Calculate price
        if (dx > 10**5 && dy > 10**5) {
            uint256 _dx = dx * sp.prec_i;
            uint256 _dy = dy * sp.prec_j;
            if (i == 0) {
                sp.p = (_dx * 10**18) / _dy;
            } else {
                // j == 0
        ...
        _tweakPrice(sp.A_gamma, sp.xp, sp.p, 0);
    ...
    function _tweakPrice(
        ...
        uint256 D_unadjusted = new_D; // Withdrawal methods know new D already
        if (new_D == 0) {
            // We will need this a few times (35k gas)
            D_unadjusted = AmmLib.newton_D(A_gamma[0], A_gamma[1], _xp);
        }
        ...
        if (p_i > 0) {
            ...
            uint256 dx_price = __xp[0] / 10**6;
            __xp[0] += dx_price;
            priceLastTemp =
                (priceScaleTemp * dx_price) /
                (_xp[1] - AmmLib.newton_y(A_gamma[0], A_gamma[1], __xp, D_unadjusted, 1));
        }

        _priceLast = priceLastTemp;

        _repegging(A_gamma, _xp, D_unadjusted);
    }
    ...
    function _repegging(
        ...
        if (_globalInitializeParams.adjustmentStep == 1e18) {
            uint256 p_new = _priceOracle;

            // Calculate balances*prices
            uint256[N_COINS] memory xp = [_xp[0], (_xp[1] * p_new) / _priceScale];

            // Calculate "extended constant product" invariant xCP and virtual price
            uint256 D = AmmLib.newton_D(A_gamma[0], A_gamma[1], xp);
        ...
        if (norm > adjustmentStepTemp) {
            uint256 p_new = (_priceScale * (norm - adjustmentStepTemp) + adjustmentStepTemp * _priceOracle) / norm;
    ...
    //     uint256 D_unadjusted
    // ) internal {
    //     uint256 old_xcp_profit = _xcpProfit;
    //     uint256 old_virtual_price = _virtualPrice;

    //     uint256[2] memory __xp = [D_unadjusted / N_COINS, (D_unadjusted * PRECISION) / (N_COINS * _priceScale)];
    //     uint256 virtual_price = (10**18 * AmmLib.geometric_mean(__xp, true)) / _totalLiquidity;
    //     uint256 xcp_profit = (old_xcp_profit * virtual_price) / old_virtual_price;

    //     _xcpProfit = xcp_profit;
    ...
    //         norm = 10**18 - norm;
    //     }
    //     uint256 adjustmentStepTemp = SigmaMath.max(_globalInitializeParams.adjustmentStep, norm / 5);
    ...
    //         uint256 new_virtual_price = (10**18 * AmmLib.geometric_mean(xp, true)) / _totalLiquidity;

    //         console.log("p_new", p_new);
    //         console.log("new_virtual_price", new_virtual_price);
    //         // console.log("xcp_profit", xcp_profit);

    //         // if ((new_virtual_price > 10**18) && (2 * new_virtual_price - 10**18 > xcp_profit)) {
    //         if (new_virtual_price > 10**18) {
    //             _priceScale = p_new;
    //             _globalD = D;
    //             _virtualPrice = new_virtual_price;

    //             console.log("!!!!!!!!adjust = true");

    //             return;
    //         } else {
    //             _globalD = D_unadjusted;
    //             _virtualPrice = old_virtual_price;
    ...
    function _A_gamma() internal view returns (uint256[2] memory) {
        ...
        uint256 A_gamma_1 = _globalFutureParams.futureAGamma;
        uint256 gamma1 = SigmaMath.bitwise_and(A_gamma_1, 2**128 - 1);
        uint256 A1 = SigmaMath.shift(A_gamma_1, -128);

        if (_blockTimestamp() < t1) {
            // handle ramping up and down of A
            uint256 A_gamma_0 = _globalInitializeParams.initialAGamma;
            ...
            // uint256 gamma0 = SigmaMath.bitwise_and(A_gamma_0, 2**128-1)
            // uint256 A0 = SigmaMath.shift(A_gamma_0, -128)
            // A1 = A0 + (A1 - A0) * (_blockTimestamp() - t0) / (t1 - t0)
            // gamma1 = gamma0 + (gamma1 - gamma0) * (_blockTimestamp() - t0) / (t1 - t0)
            t1 -= t0;
            t0 = _blockTimestamp() - t0;
            uint256 t2 = t1 - t0;
            A1 = (SigmaMath.shift(A_gamma_0, -128) * t2 + A1 * t0) / t1;
            gamma1 = (SigmaMath.bitwise_and(A_gamma_0, 2**128 - 1) * t2 + gamma1 * t0) / t1;
        }
    ...
    function _getXcp(uint256 D) internal view returns (uint256) {
        ...
        return AmmLib.geometric_mean(x, true);
    }
    ...
    function rampAGamma(
        ...
        uint256[2] memory A_gamma = _A_gamma();
        uint256 initialAGammaTemp = SigmaMath.shift(A_gamma[0], 128);
        initialAGammaTemp = SigmaMath.bitwise_or(initialAGammaTemp, A_gamma[1]);

        uint256 ratio = (10**18 * futureA) / A_gamma[0];
        ...
        ratio = (10**18 * futureGamma) / A_gamma[1];
        ...
        futureAGammaTemp = SigmaMath.bitwise_or(futureAGammaTemp, futureGamma);
        _globalFutureParams.futureAGammaTime = futureTime;
        _globalFutureParams.futureAGamma = futureAGammaTemp;

        emit RampAgamma(A_gamma[0], futureA, A_gamma[1], futureGamma, _blockTimestamp(), futureTime);
    }

    function stopRampAGamma() external onlyOwner {
        uint256[2] memory A_gamma = _A_gamma();
        uint256 currentAGamma = SigmaMath.shift(A_gamma[0], 128);
        currentAGamma = SigmaMath.bitwise_or(currentAGamma, A_gamma[1]);
        ...
        emit StopRampA(A_gamma[0], A_gamma[1], _blockTimestamp());
    }
...
interface IAmm {
    ...
    //     view
    //     returns (uint256);
    function calcLiquidityByTokenAmounts(uint256 amount0Desired, uint256 amount1Desired) external returns (uint256);
...
library AmmLib {
     ...
     * Math functions
     */
    function geometric_mean(uint256[N_COINS] memory unsorted_x, bool sort) public pure returns (uint256) {
        // (x[0] * x[1] * ...) ** (1/N)
