...
interface IFeeSharingSystem {
    ...
    function ryltRareToken() external view returns (IERC20);
    function tokenDistributor() external view returns (ITokenDistributor);
 ...
 * @notice It receives protocol fees and owns the FeeSharingSystem contract.
...
contract FeeSharingSetter is ReentrancyGuard, AccessControl {
    ...
    constructor(
        ...
        ryltRareToken = feeSharingSystem.ryltRareToken();
     ...
     * based on the balance of RYLT in fee-staking addresses that exist in the set.
     */
    function updateRewards() external onlyRole(OPERATOR_ROLE) {
        ...
        if (numberAddressesForFeeStaking > 0) {
            uint256[] memory ryltBalances = new uint256[](numberAddressesForFeeStaking);
            ...
            for (uint256 i = 0; i < numberAddressesForFeeStaking; i++) {
                uint256 ryltBalance = ryltRareToken.balanceOf(_feeStakingAddresses.at(i));
                totalAmountStaked += ryltBalance;
                ryltBalances[i] = ryltBalance;
            }
            ...
            if (totalAmountStaked > 0) {
                ...
                for (uint256 i = 0; i < numberAddressesForFeeStaking; i++) {
                    uint256 amountToTransfer = (ryltBalances[i] * reward) / totalAmountStaked;
