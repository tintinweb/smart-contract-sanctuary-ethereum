...
contract NFTFactory is AccessControl, ReentrancyGuard {
    ...
    event NewPool(uint256 indexed pid, uint256 maxNum, uint256 startBlock, uint256 endBlock, uint256 price, bytes32 root, uint256 limitPerAccount);
    ...
    function addPool(uint256 _maxNum, uint256 _startBlock, uint256 _endBlock, uint256 _price, uint256 _limitPerAccount, bytes32 _root) external onlyOperator {
        ...
        emit NewPool(pools.length.sub(1), _maxNum, _startBlock, _endBlock, _price, _root, _limitPerAccount);
    }
    ...
    function mintNFT(uint256 _pid, uint256 _num, bytes32[] calldata _proof) external payable nonReentrant {
        ...
        require(block.number >= pool.startBlock, "NFTFactory::mintNFT: Too early");
        require(block.number < pool.endBlock, "NFTFactory::mintNFT: Too late");
    ...
    function mintAirdrop(uint256 _pid, address[] calldata _accounts) external onlyOperator {
        PoolInfo storage pool = pools[_pid];
        require(block.number >= pool.startBlock, "NFTFactory::mintAirdrop: Too early");
        require(block.number < pool.endBlock, "NFTFactory::mintAirdrop: Too late");
    ...
    function setPoolMaxNumAndLimit(uint256 _pid, uint256 _maxNum, uint256 _price, uint256 _limitPerAccount) external onlyOperator {
        ...
        require(block.number < pool.startBlock, "NFTFactory::setPoolMaxNumAndLimit: Has started");
    ...
    function setPoolRoot(uint256 _pid, bytes32 _root) external onlyOperator {
        require(block.number < pools[_pid].startBlock, "NFTFactory::setPoolRoot: Has started");

        pools[_pid].root = _root;
        emit SetPoolRoot(_pid, _root);
    }

    function setPoolStartAndEnd(uint256 _pid, uint256 _startBlock, uint256 _endBlock) external onlyOperator {
        require(block.number < pools[_pid].startBlock, "NFTFactory::setPoolStartAndEnd: Has started");
        require(block.number < _startBlock, "NFTFactory::setPoolStartAndEnd: New startBlock must be higher than current block");
        require(_startBlock < _endBlock, "NFTFactory::setPoolStartAndEnd: StartBlock must be lower than new endBlock");
