...
contract randomMint is ERC721r, Ownable, ReentrancyGuard  {
    ...
    uint256 public publicSupply = 15;
    uint256 public whiteListSupply = 45;// 6000
    ...
    //mapping keep track of address mint
  ...
  /**
    ...
    modifier mintLimitCompliance(uint256 _mintAmount) {
      ...
      if(whiteListingSale){
        require(addressDetails[msg.sender].mintCount + _mintAmount <= maxMintAmountPerWhitelist,
             "Whitelist sale allowance Exceeded"); 
        require( whitelistMinted + _mintAmount <= whiteListSupply , "Whitelist supply Exceeded"); 
        } 
         require(addressDetails[msg.sender].mintCount + _mintAmount <= maxMintAmountPerWallet,
             "Public sale allowance Exceeds"); 
        require( minted + _mintAmount <= publicSupply , "Max supply Exceeded"); 
  ...
  /**
   ...
   /**
        ...
        mintLimitCompliance(_mintAmount)
    {
         ...
         // 1 < 1
         if(freeMintlimit < addressDetails[msg.sender].mintCount){
            // 2 = 3 - 1
            uint256 reamaningMint = maxMintAmountPerWhitelist - freeMintlimit;
            if (reamaningMint < _mintAmount) {
              require(msg.value >= whitelistCost * reamaningMint, "Insuffient funds ");         
            }    

         }
         _mintLoop(msg.sender, _mintAmount);
         whitelistMinted += _mintAmount;
         addressDetails[msg.sender].mintCount += _mintAmount; 
    }
  ...
  /**
    ...
    */
    ...
    function _mintLoop(address _receiver, uint256 _mintAmount) internal {
            _mintRandom(_receiver, _mintAmount);
    }
    ...
    function withdraw() public onlyOwner nonReentrant {
        ...
        //7% of initial sale funds ill go the A wallet
        (bool hs, ) = payable(WALLET_A).call{value: balance * 3 / 100}("");
