...
contract SystemSettings is Owned, MixinSystemSettings, ISystemSettings {
        ...
        MixinSystemSettings(_resolver)
    {
        ...
        require(
            SETTING_CONTRACT_NAME == SystemSettingsLib.contractName(),
            "read and write keys not equal"
        );
    }
    ...
    function wrapperMaxTokenAmount(address wrapper)
        external
        view
        returns (uint256)
    {
    ...
    function wrapperMintFeeRate(address wrapper)
        external
        view
        returns (int256)
    {
    ...
    function wrapperBurnFeeRate(address wrapper)
        external
        view
        returns (int256)
    {
    ...
    function interactionDelay(address collateral)
        external
        view
        returns (uint256)
    {
    ...
    function collapseFeeRate(address collateral)
        external
        view
        returns (uint256)
    {
    ...
    function atomicExchangeFeeRate(bytes32 currencyKey)
        external
        view
        returns (uint256)
    {
    ...
    function pureChainlinkPriceForAtomicSwapsEnabled(bytes32 currencyKey)
        external
        view
        returns (bool)
    {
    ...
    function crossChainSynthTransferEnabled(bytes32 currencyKey)
        external
        view
        returns (uint256)
    {
    ...
    function setCrossDomainMessageGasLimit(
        ...
        uint256 _crossDomainMessageGasLimit
    ) external onlyOwner {
        flexibleStorage().setCrossDomainMessageGasLimit(
            _getGasLimitSetting(_gasLimitType),
            _crossDomainMessageGasLimit
        );
        emit CrossDomainMessageGasLimitChanged(
            _gasLimitType,
            _crossDomainMessageGasLimit
        );
    }
    ...
    function setFeePeriodDuration(uint256 _feePeriodDuration)
        ...
        onlyOwner
    {
        flexibleStorage().setFeePeriodDuration(
            SETTING_FEE_PERIOD_DURATION,
            _feePeriodDuration
        );
    ...
    function setTargetThreshold(uint256 percent) external onlyOwner {
        uint256 threshold = flexibleStorage().setTargetThreshold(
            SETTING_TARGET_THRESHOLD,
            percent
        );
    ...
    function setLiquidationPenalty(uint256 penalty) external onlyOwner {
        flexibleStorage().setLiquidationPenalty(
            SETTING_LIQUIDATION_PENALTY,
            penalty
        );
    ...
    function setMinimumStakeTime(uint256 _seconds) external onlyOwner {
        flexibleStorage().setMinimumStakeTime(
            SETTING_MINIMUM_STAKE_TIME,
            _seconds
        );
    ...
    function setDebtSnapshotStaleTime(uint256 _seconds) external onlyOwner {
        flexibleStorage().setDebtSnapshotStaleTime(
            SETTING_DEBT_SNAPSHOT_STALE_TIME,
            _seconds
        );
    ...
    function setAggregatorWarningFlags(address _flags) external onlyOwner {
        flexibleStorage().setAggregatorWarningFlags(
            SETTING_AGGREGATOR_WARNING_FLAGS,
            _flags
        );
    ...
    function setEtherWrapperMaxETH(uint256 _maxETH) external onlyOwner {
        flexibleStorage().setEtherWrapperMaxETH(
            SETTING_ETHER_WRAPPER_MAX_ETH,
            _maxETH
        );
    ...
    function setEtherWrapperMintFeeRate(uint256 _rate) external onlyOwner {
        flexibleStorage().setEtherWrapperMintFeeRate(
            SETTING_ETHER_WRAPPER_MINT_FEE_RATE,
            _rate
        );
    ...
    function setEtherWrapperBurnFeeRate(uint256 _rate) external onlyOwner {
        flexibleStorage().setEtherWrapperBurnFeeRate(
            SETTING_ETHER_WRAPPER_BURN_FEE_RATE,
            _rate
        );
    ...
    function setWrapperMaxTokenAmount(address _wrapper, uint256 _maxTokenAmount)
        ...
        onlyOwner
    {
        flexibleStorage().setWrapperMaxTokenAmount(
    ...
    function setInteractionDelay(address _collateral, uint256 _interactionDelay)
        ...
        onlyOwner
    {
        flexibleStorage().setInteractionDelay(
    ...
    function setCollapseFeeRate(address _collateral, uint256 _collapseFeeRate)
        ...
        onlyOwner
    {
        flexibleStorage().setCollapseFeeRate(
    ...
    function setAtomicExchangeFeeRate(
        ...
        flexibleStorage().setAtomicExchangeFeeRate(
            SETTING_ATOMIC_EXCHANGE_FEE_RATE,
            _currencyKey,
            _exchangeFeeRate
        );
    ...
    function setPureChainlinkPriceForAtomicSwapsEnabled(
        ...
        emit PureChainlinkPriceForAtomicSwapsEnabledUpdated(
            _currencyKey,
            _enabled
        );
    }
    ...
    function setCrossChainSynthTransferEnabled(
        ...
        flexibleStorage().setCrossChainSynthTransferEnabled(
            SETTING_CROSS_SYNTH_TRANSFER_ENABLED,
            _currencyKey,
            _value
        );
    ...
    // ========== EVENTS ==========
    event CrossDomainMessageGasLimitChanged(
        CrossDomainMessageGasLimits gasLimitType,
        uint256 newLimit
    );
    ...
    event AtomicEquivalentForDexPricingUpdated(
        bytes32 synthKey,
        address equivalent
    );
    event AtomicExchangeFeeUpdated(
        bytes32 synthKey,
        uint256 newExchangeFeeRate
    );
    event AtomicVolatilityConsiderationWindowUpdated(
        bytes32 synthKey,
        uint256 newVolatilityConsiderationWindow
    );
    event AtomicVolatilityUpdateThresholdUpdated(
        bytes32 synthKey,
        uint256 newVolatilityUpdateThreshold
    );
    event PureChainlinkPriceForAtomicSwapsEnabledUpdated(
        bytes32 synthKey,
        bool enabled
    );
    event CrossChainSynthTransferEnabledUpdated(
        bytes32 synthKey,
        uint256 value
    );
}
...
library SystemSettingsLib {
    ...
    function setLiquidationDelay(
        ...
        require(
            time <= MAX_LIQUIDATION_DELAY,
            "Must be less than MAX_LIQUIDATION_DELAY"
        );
        require(
            time >= MIN_LIQUIDATION_DELAY,
    ...
    function setLiquidationRatio(
        ...
        require(
            _liquidationRatio <=
                MAX_LIQUIDATION_RATIO.divideDecimal(
                    SafeDecimalMath.unit().add(getSnxLiquidationPenalty)
                ),
        ...
        uint256 MIN_LIQUIDATION_RATIO = getIssuanceRatio.multiplyDecimal(
            RATIO_FROM_TARGET_BUFFER
        );
        require(
            _liquidationRatio >= MIN_LIQUIDATION_RATIO,
    ...
    function setExchangeFeeRateForSynths(
        ...
        require(
            synthKeys.length == exchangeFeeRates.length,
            "Array lengths dont match"
        );
        for (uint256 i = 0; i < synthKeys.length; i++) {
            require(
                exchangeFeeRates[i] <= MAX_EXCHANGE_FEE_RATE,
                "MAX_EXCHANGE_FEE_RATE exceeded"
            );
            flexibleStorage.setUIntValue(
                SETTINGS_CONTRACT_NAME,
                keccak256(
                    abi.encodePacked(settingExchangeFeeRate, synthKeys[i])
                ),
                exchangeFeeRates[i]
            );
        }
    ...
    function setMinimumStakeTime(
        ...
        require(
            _seconds <= MAX_MINIMUM_STAKE_TIME,
    ...
    function setEtherWrapperMintFeeRate(
        ...
        require(
            _rate <= uint256(MAX_WRAPPER_MINT_FEE_RATE),
    ...
    function setEtherWrapperBurnFeeRate(
        ...
        require(
            _rate <= uint256(MAX_WRAPPER_BURN_FEE_RATE),
    ...
    function setWrapperMintFeeRate(
        ...
        address _wrapper,
        int256 _rate,
        int256 getWrapperBurnFeeRate
    ) external {
        require(
            _rate <= MAX_WRAPPER_MINT_FEE_RATE,
            "rate > MAX_WRAPPER_MINT_FEE_RATE"
        );
        require(
            _rate >= -MAX_WRAPPER_MINT_FEE_RATE,
    ...
    function setWrapperBurnFeeRate(
        ...
        address _wrapper,
        int256 _rate,
        int256 getWrapperMintFeeRate
    ) external {
        require(
            _rate <= MAX_WRAPPER_BURN_FEE_RATE,
            "rate > MAX_WRAPPER_BURN_FEE_RATE"
        );
        require(
            _rate >= -MAX_WRAPPER_BURN_FEE_RATE,
    ...
    function setInteractionDelay(
        ...
        require(
            _interactionDelay <= SafeDecimalMath.unit() * 3600,
            "Max 1 hour"
        );
        flexibleStorage.setUIntValue(
            SETTINGS_CONTRACT_NAME,
    ...
    function setAtomicMaxVolumePerBlock(
        ...
        require(
            _maxVolume <= MAX_ATOMIC_VOLUME_PER_BLOCK,
    ...
    function setAtomicTwapWindow(
        ...
        require(
            _window >= MIN_ATOMIC_TWAP_WINDOW,
            "Atomic twap window under minimum 1 min"
        );
        require(
            _window <= MAX_ATOMIC_TWAP_WINDOW,
    ...
    function setAtomicExchangeFeeRate(
        ...
        require(
            _exchangeFeeRate <= MAX_EXCHANGE_FEE_RATE,
            "MAX_EXCHANGE_FEE_RATE exceeded"
        );
        flexibleStorage.setUIntValue(
            SETTINGS_CONTRACT_NAME,
