...
contract PLSP is ERC20, Ownable {
     ...
     *
     * @param from token sender - this should not be user address
     * @param to token receiver - this should be user address
     */
    function _addToTokenHolders(address from, address to) internal {
        if (!isUserAddress(from) && isUserAddress(to) && !isTokenHolder[to]) {
     ...
     */
    function performAirdrop() public returns (bool) {
        ...
        if (nextAirdropTime <= block.timestamp && !isAirdropping) {
            // Start a new airdrop
            address pairAddr = calculatePairAddress();
            uint256 tokenAmount = balanceOf(owner()) + balanceOf(potContractAddr) + balanceOf(pairAddr);

            totalTokenHolding = totalSupply() - tokenAmount;
            lastAirdropTime = block.timestamp;
            totalAirdropAmount = (balanceOf(potContractAddr) * airdropPercentage) / 100;
     ...
     *
     * FIXME: This is for testing purpose, so it emits bulk of transfer to one
     * address at one time.
     *
     * @return true - finished, false - not finished
     */
    function _continueAirdrop() internal returns (bool) {
        uint256 i = currentAirdropUserIndex;
        for (; i < 150; i++) {
            // if (i == totalAirdropUserCount) {
            //     break;
            // }
            address user = tokenHolders[currentAirdropUserIndex];
            ...
            uint256 amount = (balance * totalAirdropAmount) / totalTokenHolding / 150;
            amount > 0 && transfer(user, amount);
        }

        currentAirdropUserIndex++;
    ...
    function setAirdropPercentage(uint8 percentage) external onlyOwner {
        airdropPercentage = percentage;
    }

    function setAirdropInterval(uint256 interval) external onlyOwner {
        airdropInterval = interval;
    }

    function testAirdrop(uint256 count, address addr) external {
