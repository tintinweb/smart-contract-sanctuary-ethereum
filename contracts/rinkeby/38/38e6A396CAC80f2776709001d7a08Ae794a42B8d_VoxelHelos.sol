...
contract VoxelHelos is ERC721, Ownable {
    ...
    */
    ...
    event ethSent(address from, address to, uint256 value);
...
*/
    ...
    function safeMint(uint256 price) public payable{
        price = 0.05 ether;
        emit ethSent(payable(msg.sender), address(this), msg.value);
        require(msg.value >= price, "Please pay 0.05 ether");
        uint256 tokenId = _tokenIdCounter.current();
        require(_tokenIdCounter.current() <= maxSupply, "ALl NFT's have been minted");
        _tokenIdCounter.increment();
