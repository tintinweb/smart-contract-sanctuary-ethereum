...
contract CrazyGoatClub is ERC721, ERC721Enumerable, Ownable {

	
	uint public constant MAX_TOKENS = 9990;
	uint public constant MAX_TOKENS_VIP = 10;
	...
	constructor() ERC721("Crazy Goat Club", "CGC") {
		setBaseURI("http://api.crazygoatclub.io/cgc/");
	}
	...
	function mintNFT(uint _mintAmount, bytes32[] memory proof) external payable {
		...
		if(raffleRound == true)
		{
			require((currentVIPs + _mintAmount) <= MAX_TOKENS_VIP, "Exceeded VIP supply");
		}
		else
		{
			require(_mintAmount + currentNormal <= CURR_ROUND_SUPPLY, "We're at max supply!");
		}
		...
		for (uint256 i = 1; i <= _mintAmount; i++) {
			...
			if(raffleRound == true)
			{
				...
				theToken = currentVIPs + MAX_TOKENS;
			}
			else
			{
                currentNormal++;
				theToken = currentNormal;
				CURR_ROUND_SUPPLY--;
			}
   ...
   function getInformations() external view returns (string memory, uint, uint, uint, uint,uint,uint, bool,bool)
   {
	...
	function setWhitelistAndRaffle(bool _whitelisted, bool _raffle) external onlyOwner {
	...
	function Giveaways(uint numTokens, address recipient) external onlyOwner {
		require((currentNormal + numTokens) <= MAX_TOKENS, "Exceeded supply");
		uint index;
		for(index = 1; index <= numTokens; index++) {
			_currentToken++;
            currentNormal++;
