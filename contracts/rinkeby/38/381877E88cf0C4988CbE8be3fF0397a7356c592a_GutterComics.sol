...
contract GutterComics is ERC721, Ownable, ReentrancyGuard {
	...
	uint256 public maxNormalSupplyID = 12000; //tokens start from 0
	...
	function presale(uint256 qty, uint256 gutterCatorRatID) external payable nonReentrant {
	...
	function upgradeComic(
		...
		require(_currentIndex >= maxNormalSupplyID, "wrong max supply settings");
		require(upgradeLive, "upgrade not live");
		require(matchAddresSigner(hash, sig), "no direct mint");
		require(hashTransaction(firstId, secondId, coverType) == hash, "hash check failed");
		require(ownerOf(firstId) == msg.sender && ownerOf(secondId) == msg.sender, "not the owner");
