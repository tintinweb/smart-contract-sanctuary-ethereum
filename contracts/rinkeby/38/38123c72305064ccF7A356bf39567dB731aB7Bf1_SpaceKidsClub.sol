...
contract SpaceKidsClub is ERC721A, Pausable, Ownable, PaymentSplitter {
    using Strings for uint;

    //Max NFT supply
    uint256 public maxSupply = 3600;
    uint256 public whitelistSupply = 300;
    ...
    uint256 public saleMintLimit = 30;

    //URI of the NFTs when revealed
    string public baseURI = "UNREVEALED";
    ...
    //Events
    event Mint(address indexed _minter, uint256 _amount, uint256 _price);
    event StatusChange(Status _previousStatus, Status _newStatus);
    event Reveal(bool _revealed);

    //Address of the team
    address[] private _team = [0xd199c98318A81E629EE522Aa85E1d94E4d2EfC6b];

    //Shares
    uint256[] private _teamShares = [100];

    constructor()
        ERC721A("SpaceKidsClub", "SKC")
    ...
    /**
     * @notice Change the number of NFTs that can be minted
     *
     * @param _maxSupply The number of NFT that can me minted
     **/
    ...
    /**
     * @notice Change the number of NFTs that can be minted during the whitelist sale
     *
     * @param _whitelistMintLimit The number of NFT that can me minted
     **/
    ...
    /**
     * @notice Change the number of NFTs that can be minted during the sale
     *
     * @param _saleMintLimit The number of NFT that can me minted
     **/
    function setMintLimit(uint256 _saleMintLimit) external onlyOwner {
        saleMintLimit = _saleMintLimit;
    }

    /**
     * @notice Change the price of one NFT for the whitelist sale
     *
     * @param _whitelistPrice The new price of one NFT for the whitelist sale
     **/
    function setPriceWhitelist(uint256 _whitelistPrice) external onlyOwner {
     ...
     **/
    function setPriceSale(uint256 _priceSale) external onlyOwner {
    ...
    /**
     * @notice Allows to set the revealed variable to false
     **/
    ...
    function setupWhitelistSale() external onlyOwner {
        require(
            ...
            "The collection is not in the right status to set up whitelist sale"
        );
    ...
    function setupSale() external onlyOwner {
        require(
            status == Status.WhitelistSale,
            "The collection is not in the right status to set up sale"
        );
        status = Status.Sale;
     ...
     **/
    function whitelistMint(uint256 _amount) external payable whenNotPaused {
        //Number of NFT sold
        uint256 supply = totalSupply();
        //If Sale didn't start yet
        require(
            status == Status.WhitelistSale,
            "Sorry, sale has not started yet."
        );
        //Has user minted too many tokens
        require(_amount <= whitelistMintLimit, "You cannot mint more tokens.");
        require(
            _numberMinted(msg.sender) + _amount <= whitelistMintLimit,
            "You cannot mint more tokens."
        );
        //Has user paid enough Ethers
        require(msg.value >= whitelistPrice * _amount, "Insufficent funds.");
        //If the user try to mint any non-existent token
        require(
            supply + _amount <= whitelistSupply,
            "Sale is almost done and we don't have enough NFTs left."
        );
        //Minting all the account NFTs
        _safeMint(msg.sender, _amount);
        emit Mint(msg.sender, _amount, whitelistPrice * _amount);
    }
     ...
     **/
    function saleMint(uint256 _amount) external payable whenNotPaused {
        //Number of NFT sold
        uint256 supply = totalSupply();
        ...
        //Has user minted too many tokens
        require(_amount <= saleMintLimit, "You cannot mint more tokens.");
        require(
            _numberMinted(msg.sender) + _amount <= saleMintLimit,
            "You cannot mint more tokens."
        );
        //Has user paid enough Ethers
        ...
        //Minting all the account NFTs
        _safeMint(msg.sender, _amount);
        emit Mint(msg.sender, _amount, salePrice * _amount);
    }
