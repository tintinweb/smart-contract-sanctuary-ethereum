...
// File: contracts/SuperCreatorsJr.sol
...
contract SuperCreatorsJr is ERC721Enumerable, Ownable,Pausable {
    using Strings for uint256;
  ...
  constructor() ERC721("Supercreator Jr", "SCJR") {
  ...
  function mint(uint256 _noOfTokens,bytes32[] calldata _merkleProof) public whenNotPaused payable {
   ...
   function superBabiesMint(uint256[] memory babiesTokens,bytes32[] calldata _merkleProof)public {
    ...
    for (uint256 i = 0; i < babiesTokens.length; i++) {
      require(ownerOf(babiesTokens[i]) == msg.sender, "Cannot interact with a SuperCreators Jr you do not own.");
    }
