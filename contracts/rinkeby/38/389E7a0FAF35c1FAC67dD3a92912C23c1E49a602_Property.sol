...
*/
    ...
    function hash(
        ...
        // The lat 10 digits are random, the first is 1-6 depending on the random
        ...
        currentHash = string(abi.encodePacked(currentHash, pstyle.toString()));
    ...
    function getPropertyType(uint16 rando, uint8 propType) internal returns (uint8) {     
        ...
        if (propType == 1) {
            if (REMAINING_GOOD_HOUSES > 0 && randVal % 2 == 0) { // 50% chance
        ...
        if (propType == 2) {
            if (REMAINING_GOOD_APARTMENTS > 0 && randVal % 3 == 0) { // 33.3% chance
        ...
        if (propType == 3) {
            if (REMAINING_GOOD_HOTELS > 0 && randVal % 4 == 0) { // 25% chance for good hotel
        ...
        returns (string memory)
    {
        ...
        for (uint8 i = 0; i < numTraits; i++) {
            ...
            if (thisTraitIndex != 0) {
                ...
                if (i != (numTraits - 1)) metadataString = string(abi.encodePacked(metadataString, ","));
            }
        }
    ...
    function outputFullSVG(string memory _hash) public view returns (string memory) {
        ...
        for (uint8 i = 0; i < numTraits; i++) {
            ...
            if (thisTraitIndex != 0) svgString = string(abi.encodePacked(svgString, drawTrait(traitData[i + adder][thisTraitIndex])));
        }
