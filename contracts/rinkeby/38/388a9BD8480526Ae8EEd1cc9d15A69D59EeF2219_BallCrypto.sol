...
contract BallCrypto is ERC20Burnable, Operator {
    ...
    /**
     * @notice Constructs the BALL ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("BallCrypto", "BALL") {
        // Mints 200000 BALL to genesis farm
    ...
    function _getBallPrice() internal view returns (uint256 _ballPrice) {
        try IOracle(oracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Ball: failed to fetch BALL price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _ballPrice) internal returns (uint256){
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_ballPrice >= taxTiersTwaps[tierId]) {
    ...
    function setBallOracle(address _oracle) public onlyOperatorOrTaxOffice {
    ...
    /**
     * @notice Operator mints BALL to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of BALL to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentBallPrice = _getBallPrice();
            currentTaxRate = _updateTaxRate(currentBallPrice);
            if (currentBallPrice < burnThreshold) {
                burnTax = true;
            }
        }
