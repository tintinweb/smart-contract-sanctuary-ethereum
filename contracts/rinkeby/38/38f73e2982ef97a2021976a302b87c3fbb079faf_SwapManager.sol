...
contract SwapManager {
    ...
    address public DWE = 0x15C5c5D7989A22567E8869D3ceE3311E64A5c071;
    ...
    function swapDWEToWETH() external returns (uint256 amountOut) {
        amountOut = swapExactInputSingle(DWE, WETH, IERC20(DWE).balanceOf(msg.sender));
    }

    function swapWETHToDWE() external returns (uint256 amountOut) {
        amountOut = swapExactInputSingle(WETH, DWE, IERC20(WETH).balanceOf(msg.sender));
    }

    function swapExactInputSingle(address from, address to, uint256 amountIn) internal returns (uint256 amountOut) {
