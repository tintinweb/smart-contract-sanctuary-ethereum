...
/**
 * @title GutterPunks contract.
 * @author The GutterPunks team.
 *
 ...
 */
contract GutterPunks is ERC721, Ownable {
    using Strings for uint256;
    ...
    function mintReservedTokens(address recipient, uint256 numToMint) external onlyOwner {
        ...
        for (uint256 i = 0; i < numToMint; i++) {
            ...
            _mint(recipient, startingSupply + i + 1);
        }
    }
    ...
    function _mintInner(uint256 numToMint, uint256 voucherAmount) internal {
        ...
        for (uint256 i = 0; i < numToMint; i++) {
            // Use _mint() instead of _safeMint() since we won't mint to contracts.
            _mint(msg.sender, startingSupply + i + 1);
        }
    }
}
