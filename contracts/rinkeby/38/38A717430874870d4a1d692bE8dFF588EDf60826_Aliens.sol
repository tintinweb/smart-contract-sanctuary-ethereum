...
contract Aliens is ERC721Enumerable, Ownable {
    ...
    * - sender can't hold more tokens than maxPurchasePrivate value
    *
    ...
    */
    function airdrop(uint8 numberOfTokens) external onlyOwner {
        ...
        require(!isPublicSaleActive() && !isPrivateSaleActive(), "public and private sales shoud be not active");

        for (uint8 i = 0; i < numberOfTokens; i++) {
            _mintTokens(IERC721Enumerable(MCVAddress).ownerOf(airdropNonce+i+1), 1);
        }

        airdropNonce = airdropNonce+numberOfTokens;
    }
