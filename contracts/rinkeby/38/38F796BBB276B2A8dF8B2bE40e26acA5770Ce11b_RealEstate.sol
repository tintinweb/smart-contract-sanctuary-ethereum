...
contract RealEstate is Pausable, Ownable {
    ...
    function updateProperty(
        ...
        properties[_propertyId] = _property(
            _propertyId, (_price + percent2), properties[_propertyId].folioNumber, _isAgency,
    ...
    function acceptingDeal(uint256 _propertyId) public onlyAgency {
        ...
        if (properties[_propertyId].isAgency == true) {
            ...
            if (paidIn[_propertyId] == Tokens.USDT) {
                ...
                uint256 _fee = _propertyPrice.mul(sellerAgencyFee + 2).div(100);
                uint256 _transferPropertyOwner = _propertyPrice.sub(_fee);
                USDT.transfer(agency(), _fee);
                USDT.transfer( properties[_propertyId].currentOwner, _transferPropertyOwner );
            }

            if (paidIn[_propertyId] == Tokens.NNT) {
                
                uint256 PriceExchange_ = ExchangeHandler.priceCalculatorUSDTtoNNT(_propertyPrice);
                uint256 _fee = PriceExchange_.mul(sellerAgencyFee + 2).div(100);
                uint256 _transferPropertyOwner = _propertyPrice.sub(_fee);
        ...
        if (properties[_propertyId].isAgency == false) {

            uint256 _propertyPrice = properties[_propertyId].price;

           if (paidIn[_propertyId] == Tokens.USDT) {

                uint256 _fee = _propertyPrice.mul(sellerIndividualFee + 2).div(100);
                uint256 _propertyOwnerEth = _propertyPrice.sub(_fee);
                USDT.transfer(agency(), _fee);
            ...
            if (paidIn[_propertyId] == Tokens.NNT) {
                ...
                uint256 PriceExchange_ = ExchangeHandler.priceCalculatorUSDTtoNNT(_propertyPrice);
                uint256 _fee = PriceExchange_.mul(sellerIndividualFee + 2).div(100);
                uint256 _propertyOwnerEth = _propertyPrice.sub(_fee);
                NNT.transfer(agency(), _fee);
