...
contract MintVialMerkle is ERC1155, ERC1155Burnable, IERC2981, AccessControl, Ownable {
    ...
    uint256 private currentStageId = 1;
    uint256 private tokenId = 0;
    uint256 private amountMinted = 0;
    bool private migrationStarted = true;

    address private creator;
    address private collectionContractAddress;
    address private royaltiesAddress = 0x0000000000000000000000000000000000000000;
    uint256 private royaltiesPercentage = 0;

    mapping(address => uint256) public claimedExplorerTokens;
    mapping(address => uint256) public claimedHelmetTokens;
    ...
    constructor(address payable owner) ERC1155("ipfs://QmYoeae1rQgtR25aKEuNBzbmqzpNh4TryrbdmviGHABuo5") {
        listsData[1]._root = 0x0357a7cc631a700827724ce79355ecca939d0951d58ab2886ad560bc2e49b785;
        listsData[1]._price = 0.05 ether;

        listsData[2]._root = 0x0;
        listsData[2]._price = 0.05 ether;

        listsData[3]._root = 0x0;
        listsData[3]._price = 0 ether;

        listsData[4]._root = 0x0;
        listsData[4]._price = 0.15 ether;
    }
    ...
    modifier verifyProof(bytes32[] calldata _merkleProof, uint _maxAmount, uint256 _listId) {
        ...
        require(MerkleProof.verify(_merkleProof, listsData[_listId]._root, leaf), "Invalid proof of inclusion.");
        _;
    }

    function mintVial(bytes32[] calldata _merkleProof, uint _amount, uint _maxAmount, uint256 _listId) public payable verifyProof(_merkleProof, _maxAmount, _listId) returns(uint256) {
        require(amountMinted <= MAX_SUPPLY, "Max supply reached");

        if (currentStageId == 1) {
            if (_listId == 1) {
                require(claimedExplorerTokens[msg.sender] <= _maxAmount, "Address already claimed all his NFTs");
                require(msg.value >= (_amount * listsData[_listId]._price), "Insufficient funds to mint");
                ...
                require(claimedHelmetTokens[msg.sender] <= _maxAmount, "Address already claimed all his NFTs");
                require(msg.value >= (_amount * listsData[_listId]._price), "Insufficient funds to mint");
                ...
                for (uint i = 0; i < _amount; i++) {
                    ...
                    _mint(msg.sender, tokenId, 1, "");
                    amountMinted++;
                    claimedHelmetTokens[msg.sender] += 1;
                }
                return tokenId;
            }
            else {
    ...
    function royaltyInfo(uint256 _tokenId, uint256 _salePrice) external view override returns (address receiver, uint256 royaltyAmount) {
        return (royaltiesAddress, (_salePrice * 1000) / 10000);
    }
