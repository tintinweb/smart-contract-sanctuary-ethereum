...
contract STOA is ReentrancyGuard, ISTOA, AccessControl {
    ...
    constructor(
        ...
        currentPeriod = Period({
            ...
            commitmentRewards: rewardsForNextPeriod.mul(rewardRates[2]).div(1e18),
            accomplishmentRewards: rewardsForNextPeriod.mul(rewardRates[3]).div(1e18),
            bronze: bronze,
            silver: silver,
            gold: gold,
    ...
    function getMeditation(
        ...
        Meditation memory meditation = userSeasons[user][periodIndex].meditations[index];
        return (meditation.duration, meditation.timestamp);
    }

    /// @notice Returns the seconds meditation since a given timestamp
    /// @dev Itterares over all periods passed since timestamp and sums total meditation
    /// @dev sumDuration in seconds
    function getSecondsMeditatedSince(address user, uint256 timestamp)
        public
        view
        isUser(user)
        returns (uint256 sumDuration)
    {
        sumDuration = 0;
        uint256 periodsSince = getPeriodsSince(timestamp);

        if (periodsSince == 0) {
            UserSeason memory season = userSeasons[user][currentPeriod.index];
            uint256 duringSeason = 0;

            for (uint256 i = season.meditations.length; i > 0; i--) {
                Meditation memory med = season.meditations[i - 1];
                if (med.timestamp >= timestamp) {
                    duringSeason += med.duration;
                }
            }

            sumDuration += duringSeason;
        } else {
            for (uint256 i = periodsSince; i > 0; i--) {
                UserSeason memory season = userSeasons[user][currentPeriod.index - i];
                uint256 duringSeason = 0;

                if (i == periodsSince) {
                    for (uint256 j = season.meditations.length; j > 0; j--) {
                        Meditation memory med = season.meditations[j - 1];
                        if (med.timestamp >= timestamp) {
                            duringSeason += med.duration;
                        }
                    }
                } else {
                    duringSeason = season.secondsMeditated;
                }

                sumDuration += duringSeason;
            }
            sumDuration += userSeasons[user][currentPeriod.index].secondsMeditated;
        }
    ...
    function storeMeditation(uint256 _duration) external updatePeriod {
        ...
        if (!_isUser(msg.sender)) {
            ...
            users[msg.sender].totalMeditation += _duration;
            users[msg.sender].lastMeditated = block.timestamp;
        }
    ...
    {
        UserSeason memory season = userSeasons[user][periodIndex];
        uint256 count = season.meditations.length;
        if (count == 0) {
            return (0, 0);
        }
        Meditation memory meditation = season.meditations[season.meditations.length - 1];
        return (meditation.duration, meditation.timestamp);
    }
    ...
    function getAccomplishmentLevel(uint256 secondsMeditated) public pure returns (uint256 accomplishmentLevel) {
        if (secondsMeditated >= 5000) {
            return accomplishmentLevel = 3;
        }
        if (secondsMeditated >= 1800) {
            return accomplishmentLevel = 2;
        }
        if (secondsMeditated >= 600) {
    ...
    function _updatePeriodExpires() internal {
        ...
        console.log("Current period total supply: ", currentPeriod.totalSupply);
        console.log("Current period rewards paid: ", currentPeriod.rewardsPaid);

        uint256 leftoverAccomplishmentRewards = _calculateLeftoverAccomplishmentRewards();
        uint256 rewardsForNextPeriod = _calculateRewardForPeriod();
        console.log("Calculated rewards for season: ", currentPeriod.index + 1);
        Counters.Counter memory bronze;
        Counters.Counter memory silver;
        Counters.Counter memory gold;
        ...
        console.log("Updated period to: ", currentPeriod.index);
    ...
    function getPeriodsSince(uint256 timestamp) public view returns (uint256 periodCount) {
        ...
        periodCount = (diff / periodDuration) % 4;
    }

    // Config
    function setRewardRates(uint256[4] memory _rewardRates) external onlyRole(CONFIG_ROLE) {
        require(
