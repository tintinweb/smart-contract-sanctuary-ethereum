...
contract MercuryStaking is ReentrancyGuard, Ownable {
    ...
    uint256 public constant YIELD_PERIOD = 180 days;
    ...
    {
        /**
        * NOTE: this is a misnomer if rewardNumerator/rewardDenominator != 1,
        * thus it's deprecated and only for backwards compatibility
        */
    ...
    function _unstakeUser(address user, uint256 amount) private {
        ...
        for (; i < userDataStakesLength; i++) {
            ...
            if (userData.stakes[i].amount > amountLeft) {
                ...
                /**
                 * NOTE
                 * this is need to be deleted in prodaction
                 * this should be safe and saves a little bit of gas, but also leaves a gap in the array
                 */
                //delete userData.stakes[i];
            }
            //calculate penalty amount
            uint256 daysStaked = (block.timestamp -
                userData.stakes[0].timestamp) / 86400;
            if (daysStaked < 90) {
