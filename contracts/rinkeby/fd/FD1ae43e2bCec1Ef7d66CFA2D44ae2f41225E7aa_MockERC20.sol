...
////// /nix/store/8xb41r4qd0cjb63wcrxf1qmfg88p0961-dss-6fd7de0/src/MockERC20.sol
...
contract MockERC20 is LibNote {
  ...
  event Transfer(address indexed src, address indexed dst, uint wad);
    ...
    modifier auth {
        require(wards[msg.sender] == 1, "MockERC20/not-authorized");
        _;
    }
    ...
    function transferFrom(address src, address dst, uint wad) public returns (bool) {
      ...
      require(balanceOf[src] >= wad, "MockERC20/insufficient-balance");
        if (src != msg.sender && _allowance( src, msg.sender ) != uint(-1)) {
            require(_allowance( src, msg.sender ) >= wad, "MockERC20/insufficient-allowance");
    ...
    function burn(address usr, uint wad) external {
        require(balanceOf[usr] >= wad, "MockERC20/insufficient-balance");
        if (usr != msg.sender && _allowance( usr, msg.sender ) != uint(-1)) {
            require(_allowance( usr, msg.sender ) >= wad, "MockERC20/insufficient-allowance");
                    ...
                    bool allowed, uint8 v, bytes32 r, bytes32 s) external
    {
        ...
        require(holder != address(0), "MockERC20/invalid-address-0");
        require(holder == ecrecover(digest, v, r, s), "MockERC20/invalid-permit");
        require(expiry == 0 || block.timestamp <= expiry, "MockERC20/permit-expired");
        require(nonce == nonces[holder]++, "MockERC20/invalid-nonce");
