...
contract WildEXOplanetGame is ERC721, EIP712, Ownable{
    ...
    string[] public tierBaseUris = [
      'ipfs://QmPuFpAPvYDVK9FwMQ6FnPmpp19q5dqdgnNLj4pNtgNhMA/',
    ...
    uint256[] public tierStartMintTimes = [1643645759, 1643647559];
    uint256[] public tierMintEndTimes = [1643646659, 1643648459];
    uint256[] public tierMintFees = [0.025 ether, 0.07 ether, 0.11 ether, 0.15 ether, 0.25 ether];
    ...
    //maps token of the different tier to their real Mapping     
    ...
    uint256 public whitelistMintFee = 0.02 ether;
    uint256 public whitelistSaleDisableTime = 1643645759;
    uint256[] public revealTime = [1643646659, 1643648459]; //tier [(1),(2,3,4,5)]
    ...
    constructor() 
    ERC721("Wild EXOplanet Game", "WEXO")
    EIP712("Wild EXOplanet Game", "1.0.0"){} 
    ...
    function tokenURI(uint256 _tokenId) public view virtual override returns (string memory) {
      ...
      if(_tokenId < 99){
        //giveaway
        uri = bytes(giveawayBaseUris[tier]).length > 0 ? string(abi.encodePacked(giveawayBaseUris[tier], _tokenId.toString(), ".json")) : "";
      }
      else{
        //non give away
        if(tier > 0){
          ...
          if(block.timestamp >= revealTime[1] )
            ...
            uri = 'ipfs://<cid-unrevealed-1>/1.json';
        }
        else{
          //phase 1
          if(block.timestamp >= revealTime[0])
            uri = bytes(tierBaseUris[0]).length > 0 ? string(abi.encodePacked(tierBaseUris[0], _tokenId.toString(), ".json")) : "";
          else
            uri = 'ipfs://<cid-unrevealed-2>/1.json';
        }
      }
      return uri;
    }
