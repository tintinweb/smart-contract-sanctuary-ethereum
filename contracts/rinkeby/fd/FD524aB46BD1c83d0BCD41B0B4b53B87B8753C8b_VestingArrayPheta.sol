...
contract VestingArrayPheta is Ownable {
    ...
    */
    ...
    function addRecipients(
        ...
        for (uint256 i = 0; i < _amounts.length; i++) {
            ...
            require(
                _cliffDurations[i] < _endTimes[i]-(_startTimes[i]),
            ...
            recipients[_recipients[i]] = Recipient(
                ...
                _amounts[i]/(
                    _endTimes[i]-(_startTimes[i]+(_cliffDurations[i]))
                ),
                _amounts[i],
                0,
                Status.UnPaused
        ...
        totalAllocatedSupply = totalAllocatedSupply+(
            _before-(_totalAmount)
        );
        ...
        isNonZeroAddress(recipient)
    {
        ...
        uint256 pausedFor = block.timestamp-(
        ...
        recipients[recipient].endTime = recipients[recipient].endTime+(
            pausedFor
        );
    }
        ...
        isNonZeroAddress(recipient)
    {
        ...
        uint256 _bal = recipients[recipient].totalVestingAmount-(
    ...
    {
        return
            recipients[recipient].startTime+(
        ...
        returns (uint256)
    {
        ...
        return
            _recipient.totalVestingAmount-(
                _recipient.totalClaimed+(totalLockedOf(recipient))
            );
    }
