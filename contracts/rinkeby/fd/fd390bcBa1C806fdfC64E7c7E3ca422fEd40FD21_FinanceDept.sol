...
// TO-DO: Require reg is owned by same owner as JOB
...
contract FinanceDept is AccessControl, Pausable {
    ...
    uint payDuration = 10 minutes;
    uint maxClaimTime = 8 weeks;
    ...
    function salary(uint _jobId) public view returns (uint) {
        ...
        uint _baseSalary = jobs.getBaseSalary(_companyId);
        uint _spread = jobs.getSpread(_companyId);
        bytes32 _r = keccak256(abi.encodePacked(_jobId, randomSeed));
        uint _salary;
        _baseSalary *= 10 ** 18; 
        _spread *= 10 ** 18;
        _salary = _baseSalary + (uint(_r) % _spread) - (_spread / 2);
        return _salary;
    }

    function levelBonus(uint _level, uint _salary) public pure returns (uint) {
        if (_level > 0) {
            return (2 ** (_level - 1) * 10 * _salary / 100); // TO-DO -- TEST THIS
        } else
            return 0;
    }
    ...
    function unclaimedTotal(uint _jobId) public view returns (uint) {
        uint _level = jobs.getLevel(_jobId);
        uint _salaryWithBonus = salary(_jobId) + levelBonus(_level, salary(_jobId));
        uint _timestamp = jobs.getTimestamp(_jobId);
        uint _duration = block.timestamp - _timestamp;
        if (_duration > maxClaimTime)
            _duration = maxClaimTime;
        uint _result = (_duration * 10) / payDuration * _salaryWithBonus / 10;
        return _result;
    } 
    ...
    function claimSalary(uint _jobId) public whenNotPaused {
        ...
        require(jobs.ownerOfReg(jobs.getRegId(_jobId)) == msg.sender,"Must assign a reg you own.");
        jobs.setTimestamp(_jobId, block.timestamp);
        uint _amount = unclaimedTotal(_jobId);
        regularsToken.drawTokens(msg.sender,_amount); // SEND THE TOKENS!
...
//  It was compiler without optimization. Make sure you turn that off before goign mainnet

//  There is a ERC20Capped in OZ, I would use that over a manual implementation
//  the this.transfer is confusing, I would advice using _transfer(address(this), ...) instead
//  I don't understand why there are 3 functions drawTokens, sendFromTreasury, fundTreasury ... that seams overkill and simplifiable
//  treasuryBalance() is useless, balanceOf already covers that

//  I would recommand you add extra stuff though.
//  - ERC1363 
//  - ERC20Permit (ERC2612)
//  - Voting ?
...
contract RegularToken is ERC20, ERC20Burnable, Pausable, AccessControl {
    bytes32 public constant MINTER_ROLE = keccak256("MINTER_ROLE");
    uint _cap;

    constructor() ERC20("Regular Token", "REG 0708") {
        _grantRole(DEFAULT_ADMIN_ROLE, msg.sender);
        _grantRole(MINTER_ROLE, msg.sender);
        _cap = 100 * 1000000 * 10 ** decimals(); // 100MM
        fundTreasury(10000 * 10 ** decimals());   // Initial Funding
    }

// ADMIN

    function drawTokens(address to, uint256 amount) public onlyRole(MINTER_ROLE) {
        if (!atCap(amount))
            _mint(to, amount);
        else 
...
// PRIVATE
    ...
    function atCap(uint _amount) private view returns (bool) { 
        return (totalSupply() + _amount >= _cap);
    }
...
// RINKEBY ONLY!!!! 
    ...
    function gimmeRinkeby() public {
        uint _faucetAmount = 140 * 10 ** decimals();
        this.transfer(msg.sender,_faucetAmount);
    }
}
...
// To-DO: Make ERC20 claim for many JobIds
// TO-DO: Store companies? Get members of companies?
// TO-DO: How to change companies?
...
contract Jobs is ERC721, ERC721Enumerable, AccessControl {
    ...
    constructor() ERC721("Regular Jobs", "Jobs") {
        console.log("MESSAGE SENDER 1" ,msg.sender);
        _grantRole(DEFAULT_ADMIN_ROLE, msg.sender);
        _grantRole(MINTER_ROLE, msg.sender);
        ...
        levels = new Levels();
        titles = new Titles(address(levels));
    }
...
// View Functions
    ...
    function getJobFullDetails(uint _jobId) public view returns (uint, uint, uint, string memory, uint, string memory){
        ...
        uint _level = levels.level(_jobId);
        string memory _title = titles.title(_jobId);
        return (_salary, _regId, _companyId, _companyName, _level, _title);
    }
    ...
    function getLevel(uint _jobId) public view returns (uint) {
        require(_exists(_jobId), "Query for nonexistent token");
        return levels.level(_jobId);
    }
...
// Setting and getting contract addresses
    ...
    function setContractAddr(bytes memory _contractName, address _addr) public onlyRole(MINTER_ROLE){
        ...
        } else if (keccak256(_contract) == keccak256(bytes("Levels"))) {
            levels = Levels(_addr);
...
// Private
    ...
    function resetJob(uint _jobId) private {    // when jobs are transferred, unassign their regular
...
// Overrides
    ...
    function _beforeTokenTransfer(address from, address to, uint256 tokenId) internal override(ERC721, ERC721Enumerable) {
        if (from != 0x0000000000000000000000000000000000000000){ // if not minting, then reset on transfer
            resetJob(tokenId);  
...
contract Companies is AccessControl {
    ...
    Company[] companies;                // Companies stored by company ID

    mapping(uint => uint[]) private specials;
	...
	constructor() {
        ...
        // _baseSalary / _spread / _limit /  _startIndex / _name
        uint startIndex = 0;
        addCompany( 99, 30, 10, startIndex, "Dollar Store");
        startIndex += companies[numCompanies() - 1].limit;
        addCompany(1, 30, 10, startIndex, "McDanny's");
        startIndex += companies[numCompanies() - 1].limit;
        addCompany(2, 30, 10, startIndex, "Whoppy Jr.");
        startIndex += companies[numCompanies() - 1].limit;
        addCompany(3, 30, 10, startIndex, "WGMI Inc.");
        startIndex += companies[numCompanies() - 1].limit;
        addCompany(4, 30, 10, startIndex, "Global International");
        startIndex += companies[numCompanies() - 1].limit;
        addCompany(5, 30, 10, startIndex, "N.E.X.T. Rugs");
        startIndex += companies[numCompanies() - 1].limit;
        addCompany(6, 30, 10, startIndex, "Alpha Limited");
        startIndex += companies[numCompanies() - 1].limit;
        addCompany(7, 30, 10, startIndex, "G.M.");
        startIndex += companies[numCompanies() - 1].limit;
        addCompany(8, 30, 10, startIndex, "Partners & Partners");
        startIndex += companies[numCompanies() - 1].limit;
        addCompany(9, 30, 10, startIndex, "Boss e-Systems");
        startIndex += companies[numCompanies() - 1].limit;
        addCompany(10, 30, 10, startIndex, "Block Busters");
        startIndex += companies[numCompanies() - 1].limit;
        addCompany(11, 30, 10, startIndex, "Hexagon Research Group");
        startIndex += companies[numCompanies() - 1].limit;
        // Note! Companies of 'special' regulars MUST come after the regular regulars
        addCompany(12, 30, 32, startIndex, "Crabby Shack"); // special
        startIndex += companies[numCompanies() - 1].limit;
        addCompany(13, 30, 55, startIndex, "Wenders"); // special
        startIndex += companies[numCompanies() - 1].limit;

        // SPECIAL SETS OF REGULAR IDs
        ...
        specials[13] = [
            ...
            3544, 3607, 3617, 4103, 4117, 4149, 4195, 4230, 4425, 5065, 5101, 5188, 5494, 5845, 6016, 6042, 6073, 
            6109, 6436, 6649, 7092, 7574, 7863, 8077, 8110, 8326, 8359, 8480, 8629, 8825, 9303, 9319, 9339, 9770, 
            9800, 9858, 9870
        ];

        // McD's Workers + Managers
        specials[14] = [
        ...
        specials[16] = [
            ...
            610, 638, 679, 681, 703, 743, 784, 867, 917, 959, 1013, 1032, 1042, 1084, 1127, 1142, 1196, 1234, 1279, 
            1295, 1296, 1297, 1310, 1323, 1356, 1390, 1405, 1440, 1482, 1566, 1596, 1598, 1660, 1663, 1695, 1700, 
            1708, 1905, 1929, 1986, 2018, 2026, 2037, 2067, 2097, 2125, 2148, 2176, 2207, 2247, 2262, 2347, 2494, 
            2516, 2543, 2549, 2603, 2612, 2622, 2662, 2731, 2741, 2776, 2788, 2809, 2965, 3004, 3013, 3014, 3031, 
            3035, 3044, 3112, 3113, 3144, 3236, 3287, 3338, 3385, 3519, 3545, 3548, 3647, 3800, 3896, 3985, 4156, 
            4165, 4203, 4215, 4243, 4273, 4294, 4328, 4365, 4402, 4463, 4587, 4691, 4799, 4814, 4920, 4923, 4926, 
            4934, 4947, 4981, 5018, 5049, 5104, 5110, 5131, 5135, 5152, 5313, 5317, 5333, 5376, 5379, 5386, 5423, 
            5426, 5510, 5608, 5638, 5663, 5667, 5758, 5780, 5787, 5792, 5838, 5883, 5934, 5977, 6068, 6081, 6099, 
            6120, 6150, 6191, 6213, 6243, 6251, 6257, 6373, 6385, 6409, 6418, 6445, 6454, 6481, 6519, 6549, 6563, 
            6581, 6606, 6619, 6622, 6718, 6742, 6808, 6868, 6906, 6920, 7028, 7111, 7131, 7156, 7171, 7212, 7223, 
            7253, 7269, 7305, 7326, 7346, 7410, 7414, 7434, 7465, 7470, 7537, 7593, 7683, 7689, 7697, 7725, 7824, 
            7838, 7847, 7885, 7952, 8014, 8056, 8064, 8102, 8124, 8151, 8153, 8161, 8243, 8372, 8390, 8391, 8424, 
            8435, 8455, 8467, 8468, 8472, 8481, 8493, 8555, 8609, 8642, 8659, 8692, 8768, 8781, 8818, 8822, 8899, 
            8930, 8940, 8950, 8975, 9011, 9055, 9060, 9106, 9129, 9192, 9231, 9244, 9275, 9324, 9413, 9455, 9638, 
            9689, 9970, 9977
        ];

        // create new decks for each set of special regulars
        uint notSpecialCount = 0;
        for (uint i = 0; i < numCompanies(); i++) {
            if (specials[i].length > 0){
                specialDecks[i].setup(specials[i].length);
                // console.log("Special deck created for CompanyId = ", i, ", length = ", specials[i].length);
            } else {
                notSpecialCount += companies[i].limit;
            }
        }
        
        // setup the main deck for the remaining non-special regulars
        mainDeck.setup(notSpecialCount);
        console.log("Maximum Job Ids : ", maxJobIds());
        console.log("Maximum NOT-SPECIAL Job Ids : ", notSpecialCount);
	}

// ADMIN FUNCTIONS

    // Companies are added in constructor
    function addCompany(
        uint _baseSalary, 
        uint _spread, 
        uint _limit, 
    ...
    {
        require(_baseSalary > 0, "baseSalary must be > 0");
    ...
    // Pull a new id for the JOB NFT based on ID from their REG NFT
    function makeNewJob(uint _regularId) public onlyRole(MINTER_ROLE) returns (uint, uint) {
        uint _pull;
        uint _offset;
        uint _newJobId;
        // Pull random card from appropriate deck
        if (!isSpecial(_regularId)) { // If Regular id is NOT special
            _pull = mainDeck.draw();
            _offset == 0; 
        } else { // If Regular id IS special
            _pull = specialDecks[getSpecialCompanyId(_regularId)].draw();
            _offset = companies[getSpecialCompanyId(_regularId)].startIndex;
        }
        _newJobId = _pull + _offset;

        // Determine company ID based on _newJobID
        uint i;
        for (i = 0; i < numCompanies() -1; i++) {
            if (_newJobId >= companies[i].startIndex && _newJobId < companies[i+1].startIndex){
                break;
            }
        }
        console.log("Adding jobId",_newJobId, "in company", i);
        emit jobIDCreated(_regularId, _newJobId, i, msg.sender);
        return (_newJobId, i); // return jobID + companyId
    }

    function updateCompany(uint _companyId, string memory _name, uint _baseSalary) public onlyRole(MINTER_ROLE)  {
        companies[_companyId].name = _name;
...
// View Functions
    ...
    function getSpread(uint _companyID) public view returns (uint) {
        return companies[_companyID].spread;
    }

    function getName(uint _companyID) public view returns (string memory) {
        return companies[_companyID].name;
    }

    function getLimit(uint _companyID) public view returns (uint) {
        return companies[_companyID].limit;
    }
    ...
    function maxJobIds() public view returns (uint) {
        uint _total = 0;
        for (uint i = 0; i < numCompanies(); i++) {
            _total += companies[i].limit;
        }
        return _total;
    }

    function specialCountByCompany(uint _companyId) public view returns (uint) {
        return specials[_companyId].length;
    }

// Internal

    // check if a regular ID is special
    function isSpecial(uint _regTokenId) internal view returns (bool) {
        for (uint i = 0; i < numCompanies(); i++) {
            for (uint s = 0; s < specials[i].length;s++){
                if (specials[i][s] == _regTokenId)
                    return true;
            }
        }
        return false;
    ...
    function getSpecialCompanyId(uint _tokenId) internal view returns (uint) {
        for (uint i = 0; i < numCompanies(); i++) {
            for (uint s = 0; s < specials[i].length;s++){
                if (specials[i][s] == _tokenId)
                    return i;
            }
        }
        revert("TokenId not special");
    }

    function getStartIndex(uint _companyId) internal view returns (uint) {
        return companies[_companyId].startIndex;
    }
...
contract Levels is AccessControl {
...
contract Titles is AccessControl {
    ...
    event LevelUpdate (uint _tokenId, uint _newLevel, string _newPrefix);
