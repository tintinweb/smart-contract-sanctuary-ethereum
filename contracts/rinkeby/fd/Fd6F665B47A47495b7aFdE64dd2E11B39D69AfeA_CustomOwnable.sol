...
contract Escrow is CustomOwnable {
    ...
    function appendVestingEntry (
        ...
        uint256 isEnoughBalance = totalVestedBalance + quantity;
        require(
            isEnoughBalance <= IERC20(token).balanceOf(address(this)),
            "Must be enough balance in the contract to provide for the vesting entry"
        );

        totalVestedBalance = totalVestedBalance + quantity;
        totalVestedAccountBalance[account] = quantity + totalVestedAccountBalance[account];
    ...
    function vest() external onlyOwner {
        for(uint256 i = 0; i < countSchedules; i++){
            ...
            for (uint256 j = 1; j <= MAX_ACCOUNTS_COUNT; j++) {
