...
interface IERC721G is IERC721 {
     ...
     */
    event SetAuth(address auth, bool isOpen);

    /**
     * @dev Returns all auth info.
     */
    function getAllAuth() external view returns (Auth[] memory);
...
contract ERC721G is IERC721G, ERC721 {
    ...
    function _setAuth(address erc721, bool isOpen) internal {
        uint v = isOpen ? 1 : 0;
        allAuth.set(erc721, v);
        emit SetAuth(erc721, isOpen);
    }
    ...
    // ------------------- view function -------------------
    function getAllAuth() public view returns(Auth[] memory) {
        uint v;
        address erc721;
...
// File contracts/extensions/ERC721GWhitelist.sol
...
// ERC721GWhitelist
contract ERC721GWhitelist is Ownable, ERC721G, ReentrancyGuard {

    enum Mode { NONE, GIFT, WHITELIST, GIFT_WHITELIST }

    uint immutable public maxTotalSupply;
    ...
    Mode public mintMode = Mode.NONE;

    constructor(string memory name_, string memory symbol_, uint maxTotalSupply_, uint startTime_)
    ...
    function setAuth(address erc721, bool isOpen) external onlyOwner {
        ERC721G._setAuth(erc721, isOpen);
    }

    function setGiftMintLimit(uint limit_) external onlyOwner {
        ERC721G._setGiftMintLimit(limit_);
    }

    function setMaxTotalGiftMint(uint maxTotalGiftMint_) external onlyOwner {
        ERC721G._setMaxTotalGiftMint(maxTotalGiftMint_);
    }

    function setMintLimit(uint limit_) external onlyOwner {
        mintLimit = limit_;
    }

    function openingMint(uint quantity) external nonReentrant {
        require(block.timestamp >= startTime, "not yet started");
        require(mintMode == Mode.NONE, "invalid minting mode");
        require(quantity <= maxTotalSupply, "mint too many");
        require(totalSupply + quantity <= maxTotalSupply, "over total supply");
        require(mintQuantity[msg.sender] + quantity <= mintLimit, "over minting limit");
        for (uint i = 0; i < quantity; i++) {
            ERC721._mint(msg.sender, totalSupply+i);
        }

        totalSupply += quantity;
        mintQuantity[msg.sender] += quantity;
    }

    function giftMint(address authERC721, uint authTokenId, uint quantity) external nonReentrant {
        require(block.timestamp >= startTime, "not yet started");
        require(mintMode == Mode.GIFT || mintMode == Mode.GIFT_WHITELIST, "invalid minting mode");
        require(quantity <= maxTotalSupply, "mint too many");
        require(totalSupply + quantity <= maxTotalSupply, "over total supply");

        address recipient = ERC721G._getRecipient(authERC721, authTokenId);
        require(recipient != address(0), "invalid auth");
        require(mintQuantity[recipient] + quantity <= mintLimit, "over minting limit");

        for (uint i = 0; i < quantity; i++) {
            ERC721G._mint(totalSupply+i, authERC721, authTokenId);
        }

        totalSupply += quantity;
        mintQuantity[recipient] += quantity;
    }

    function whitelistMint(uint quantity, bytes32[] calldata proof) external nonReentrant {
        require(block.timestamp >= startTime, "not yet started");
        require(mintMode == Mode.WHITELIST || mintMode == Mode.GIFT_WHITELIST, "invalid minting mode");
        require(quantity <= maxTotalSupply, "mint too many");
        require(totalSupply + quantity <= maxTotalSupply, "over total supply");
        require(mintQuantity[msg.sender] + quantity <= mintLimit, "over minting limit");
