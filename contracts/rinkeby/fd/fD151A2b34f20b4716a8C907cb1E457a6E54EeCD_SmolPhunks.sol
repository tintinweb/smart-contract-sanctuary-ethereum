...
contract SmolPhunks is Ownable, ERC721A {
    ...
    bytes constant tokenDescription = "One of 10,000 tokens in the OC SmolPhunks collection, an on-chain transformation of the original CryptoPunks.";
    ...
    function adminMint(address toAddress, uint numTokens) private onlyOwner {
        // require(msg.value == totalMintCost(numTokens), "Need exact payment");
        // require(msg.sender == tx.origin, "Contracts cannot mint");
        require(numTokens + totalSupply() <= maxSupply, "Supply limit reached.");
        // require(isMintActive, "Mint is not active");
    ...
    function tokenImage(uint16 tokenId) public view returns (string memory) {
        ...
        svgBytes.appendSafe('<svg width="48" height="48" shape-rendering="crispEdges" xmlns="http://www.w3.org/2000/svg" version="1.2" viewBox="0 0 24 24"><style>rect{width:1px;height:1px} g {width: 24px; height: 24px;}</style><rect x="0" y="0" style="width:100%;height:100%" fill="#638596" /><g style="transform: translate(calc(50% - 12px), calc(50% - 12px))">');
