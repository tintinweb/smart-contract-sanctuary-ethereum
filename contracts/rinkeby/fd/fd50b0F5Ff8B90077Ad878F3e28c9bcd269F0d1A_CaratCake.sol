	...
	ERC721Burnable
{
	...
	uint168 public constant PRICE = 0.005 ether;
	...
	bytes32 public merkleRoot =
		0xd8257ee5281bc533765559f672a12fc29fd7c0c25e1e65b464123003d63e8fe1;
	...
	address payable private Founder6 =
		payable(0xcCBd5730Dea52276b5d3c2dc3Ce565DE543Cc2d8);
	...
	modifier onlyAdmin() {
		if (isAdmin[msg.sender] || msg.sender == address(this)) {
			_;
		} else {
	...
	constructor(string memory _initNotRevealedUri) ERC721("CaratCake", "CC") {
		setUnrevealedUri(_initNotRevealedUri);
		isAdmin[msg.sender] = true;
		isAdmin[Founder1] = true;
		isAdmin[Founder2] = true;
		isAdmin[Founder3] = true;
		isAdmin[Founder4] = true;
		isAdmin[Founder5] = true;
		isAdmin[Founder6] = true;
	...
	function getRemainingSupply() public view returns (uint) {
		return MAX_SUPPLY - (_tokenIdCounter.current() - 1);
	}
	...
	function setKingsCourt(address addr, bool should) public onlyOwner {
		require(
			inKingsCourt[addr] != should,
			"Cannot change user to the same state. "
		);
		inKingsCourt[addr] = should;
	}
		...
		onlyWhen(MINT_IS_LIVE || MINT_IS_PUBLIC)
	{
		...
		require(
			...
			"Cannot mint more than remaining tokens in Pre-mint supply"
		);
		require(
	...
	function handleCounters(uint currentToken) internal {
