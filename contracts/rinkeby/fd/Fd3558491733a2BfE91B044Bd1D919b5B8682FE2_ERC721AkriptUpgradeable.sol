...
import "./ERC721AkriptUpgradeableLazy.sol";

contract ERC721AkriptUpgradeable is
    Initializable,
    ERC721Upgradeable,
    ...
    ERC721AkriptUpgradeableLazy
{
    ...
    function initialize(
        ...
        __ERC721AkriptUpgradeableLazy_init(name, version);
    }
    ...
    function mintAndTransfer(
        address _to,
        LibERC721LazyMint.NFT memory _nft,
...
abstract contract ERC721AkriptUpgradeableLazy is
    Initializable,
    EIP712Upgradeable,
    ERC721URIStorageUpgradeable
{
    ...
    event LazyMintedTransfer(
        address indexed from,
        address indexed to,
        uint256 tokenId
    );

    function __ERC721AkriptUpgradeableLazy_init(
        string memory name,
        string memory version
    ...
    function mintAndTransferLazy(
        ...
        require(
            signer == nft.creator,
            "creator not matched. invalid signature"
        );
        ...
        returns (bytes32)
    {
        return
            _hashTypedDataV4(
                keccak256(
                    abi.encode(
                        keccak256("NFT(address creator,uint256 tokenId)"),
                        nft.creator,
                        nft.tokenId
                    )
                )
