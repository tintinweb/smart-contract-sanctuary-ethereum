...
contract UnistoryMarketplace is SwapMarketplaceInterface, Ownable {
	 ...
	 * Orders functions
	 */
	function createOrder(OfferItem calldata _offerItem, address _considCollection)
		external
	...
	function execute(uint256 orderId, uint256 proposalId) external {
		...
		require(!order.isActive, 'Order was discontinued');
...
interface SwapMarketplaceInterface {
	 ...
	 * Orders functions
	 */
	function createOrder(OfferItem calldata offer, address considCollection) external returns (bytes32 orderHash);
