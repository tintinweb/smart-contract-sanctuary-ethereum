...
contract Executor is LiquityMath{
	...
	function adjustAcrForRequestor(uint16 _acr, address _requestor) internal view returns (uint16) {
		...
		uint16 rccar = ca.getAcknowledgementRate(keccak256(abi.encode(_requestor)));
	...
	function adjustCreditLineLiquity(
		...
		if (vars.isDebtIncrease) {
			...
			if (vars.neededLUSDChange > 0) {
				...
				pullLUSDFrom(_LUSDFrom, vars.neededLUSDChange);
			}
		}
		...
		if (vars.isDebtIncrease) {
			...
			// Send LUSD to the requested address
			sendLUSD(_LUSDTo, _LUSDRequestedChange);
			...
			logger.log(
				...
				abi.encode(
					...
					vars.mintedLUSD, 
//					_LUSDRequestedChange, // Stack too deep error
					_LUSDTo
					)
			);
			...
			logger.log(
				address(this), _caller, "adjustCreditLineLiquity",
				abi.encode(AdjustCreditLineLiquityChoices.DebtDecrease, _LUSDRequestedChange, _LUSDFrom)
			);
		}
