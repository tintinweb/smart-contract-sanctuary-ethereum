...
function getShareHolderRevenue() public view returns(uint256[] memory) {
    uint256[] memory _revenue = new uint256[](1);
    for (uint256 i = 0; i < ShareHolderCount; i++) {
       shareHolderStruct storage shareHolder = shareHolders[i];
      if(shareHolder.walletAddress == _msgSender()) _revenue[i] = shareHolder.revenue;
    }
    return (_revenue);
  }
