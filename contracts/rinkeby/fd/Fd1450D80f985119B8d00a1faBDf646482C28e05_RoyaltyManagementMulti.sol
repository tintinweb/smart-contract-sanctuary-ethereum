...
contract RoyaltyManagementMulti {
    ...
    mapping(uint256 => address[]) public workToArtists;
    ...
    ***************/
    ...
    function createWork(string memory cid) public {
        ...
        Work memory work;
        work.CID = cid;
        workToArtists[workId].push(msg.sender);
    ...
    function fund(uint256 work_id) public payable {
        ...
        for (
            uint8 artistIntex = 0;
            artistIntex < workToArtists[work_id].length;
            artistIntex++
        ) {
            address artist = workToArtists[work_id][artistIntex];
    ...
    function signTransaction(uint256 transactionId) public payable {
        ...
        //emit an event
        emit TransactionSigned(msg.sender, transactionId); // mettere alla fine della funzione
        ...
        if (
            ...
            workToArtists[work.id].push(transaction.to);
    ...
    function deleteArtist(uint256 work_id) private {
        ...
        for (uint8 i = 0; i < workToArtists[work_id].length - 1; i++) {
            if (artistsBalance[workToArtists[work_id][i]] == 0) {
                artistIndex = i;
            }

            if (i >= artistIndex) {
                workToArtists[work_id][i] = workToArtists[work_id][i + 1];
            }
        }
        delete workToArtists[work_id][workToArtists[work_id].length - 1];
        workToArtists[work_id].pop();
    }
    ...
    ***************/
    ...
    function getWorks() public view returns (uint256[] memory) {
        return works;
    }

    function getWork(uint256 work_id) public view returns (Work memory) {
        return idToWork[work_id];
    }
    ...
    function getAddressToSharesOnSale(address artist, uint256 work_id)
        public
        view
        returns (uint8)
    {
        return addressToSharesOnSale[work_id][artist];
    }
