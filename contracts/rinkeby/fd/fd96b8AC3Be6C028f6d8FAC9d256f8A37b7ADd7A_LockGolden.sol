...
contract LockGolden {
    ...
    IERC20 public golden = IERC20 (0x0Ff3AEFb87b0cD7163d13e45db1a916632ef41Eb);
    //0x26635e7Ecd121B4d546Fa7cD4f6c86F3CFf85254

    mapping (address => staker)  _stakers;

    function stake(address account, uint256 amount, uint8 unlockTime) public returns(uint256 currentBalance , staker memory currentStaker) {
        require(amount != 0 , "amount must not 0");
        require(unlockTime == 10 || unlockTime == 20 || unlockTime == 30, "unlockTime must be 10, 20 or 30.");
        ...
        require(_stakers[account].amount == 0, "You have already bet. Wait until current bet expires");
        ...
        if(golden.myTransferFrom(account, golden.bank(), amount)) {
            emit Stake(account, amount, block.timestamp + unlockTime);
            return (golden.balanceOf(account), _stakers[account]);
        }
    }

    //0xfd96b8AC3Be6C028f6d8FAC9d256f8A37b7ADd7A
    function withdraw(address account) public returns (uint256 currentBalance) {
        ...
        require(block.timestamp > currentStaker.expireTime, "Please wait until bet expires.");

        if(golden.myTransferFrom(golden.bank(), account, currentStaker.amount)) {
