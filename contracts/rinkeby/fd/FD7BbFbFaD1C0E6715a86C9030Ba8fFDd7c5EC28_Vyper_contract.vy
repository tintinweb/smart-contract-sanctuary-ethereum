...
def auction_index() -> uint256[max_seat_count]:
    """
    @notice Retrieve leader for all seats
...
def auction_resolve(
    ...
    """
    assert (
        0 < self.auction_deadline[seat_id]
    ), "Auction never started"  # dev: Auction never started
    assert (
        block.timestamp > self.auction_deadline[seat_id]
    ), "Auction not over"  # dev: Auction not over

    assert keccak256(hash) == leaf, "Failed to hash"
    assert (
        self._calc_merkle_root(leaf, seat_id, proof) == self.merkle_root
...
def mint_finalize(
    ...
    # Verify Merkle
    assert (self._calc_merkle_root(leaf, index, proof) == self.merkle_root)  # dev: Merkle Verify Fail
