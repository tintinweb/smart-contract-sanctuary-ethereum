...
contract gt10 is ERC721, RrpRequesterV0, Ownable {
    ...
    constructor(address _airnodeRrp) RrpRequesterV0(_airnodeRrp) ERC721("GT10", "GT10") {}
    ...
    function isWinner(uint256 tokenId) public view returns (bool) {
      ...
      (win,a,b) = getGTInfo(tokenId);
      return win;
    }

    function setPU(uint256 _fpu, uint256 _ppu) external onlyOwner() {
      fpu = _fpu;
      ppu = _ppu;
    }
    ...
    function getPU() public view returns (uint256, uint256) {
      return (fpu,ppu);
    }

    function mintIt() payable public returns (bytes32) {
	require(mintingEnabled, "Minting is not currently enabled.");

	if (GTMintedF[msg.sender]) {
	  require(msg.value >= paidMintPrice, "Maximum free mints is 1 per address");
	}

	if (msg.value >= paidMintPrice) {
	  ispaid = true;
	} else {
	  isfree = true;
	}
    ...
    function getRStats(uint256 tokenId) public view returns (uint256, uint256, uint256, uint256, bool) {
        return (
            gti[tokenId].GWDF,
            gti[tokenId].FPU,
            gti[tokenId].PPU,
            gti[tokenId].UIN,
            gti[tokenId].paidmint
        );
    }

    function getGTInfo(uint256 tokenId) public view returns (bool, uint256, uint256) {
        return (
            gti[tokenId].GW,
            gti[tokenId].gtdrawn,
