...
contract mockEC is ERC721, Ownable {
    using Strings for uint256;
    using Counters for Counters.Counter;
    Counters.Counter private _tokenIds;

    constructor() ERC721("Mock EC", "mEC") {}

    string public __baseURI = "https://heroku.ether.cards/card/";
    string public baseExtension = ".json";

    function superMintTo(address _receiver, uint16 _count) external {
...
*/
        ...
        returns (string memory)
    {
        ...
        string memory slash = "/";
        return
            string(abi.encodePacked(__baseURI, folder, slash, file, ".json"));
    }
    ...
    function checkIfHolder(address _caller) external view returns (bool) {
        if (this.balanceOf(_caller) > 0) {
            return true;
        } else {
            return false;
        }
...
library EnumerableSet {
    ...
    */
    ...
    {
        address[] memory output = new address[](length(set));
        for (uint256 i; i < (length(set)); i++){
            output[i] = address(uint160(uint256(_at(set._inner, i) )));
        }
        return output;
    }
