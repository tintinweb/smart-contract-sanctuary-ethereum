...
contract ERC1155Token is Context, Ownable, AccessControl, ERC1155Pausable {
    ...
    mapping(uint256 => string) internal _nftId;
    mapping(uint256 => string) internal _model;
   ...
   //string memory uri ERC1155(uri)
    ...
    function batchMint(
        ...
        for (uint256 i = 0; i < ids.length; ++i) {
            ...
            _nftId[ids[i]] = nftId_[i];
            _model[ids[i]] = model_[i];
            _uri[ids[i]] = uri_[i];
        }
    ...
    function nftId(uint256 id) public view returns (string memory) {
        return _nftId[id];
    }
    function model(uint256 id) public view returns (string memory) {
        return _model[id];
    }
