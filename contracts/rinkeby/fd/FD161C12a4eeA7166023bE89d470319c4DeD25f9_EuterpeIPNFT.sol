...
contract EuterpeIPNFT is ERC1155URIStorage, Ownable, ReentrancyGuard {
    ...
    event BatchMintEvent(uint256[] amounts, uint256[] tokenIds);
    ...
    function batchMint(uint256[] memory amounts_, string[] calldata tokenURIs_) external nonReentrant {
        require(minters[msg.sender]);
        uint256[] memory tokenIds = new uint256[](amounts_.length);
        for (uint256 i = 0; i < amounts_.length; i++) {
            _mint(msg.sender, _lastMintedId, amounts_[i], "");
            _setURI(_lastMintedId, tokenURIs_[i]);
            tokenIds[i] = _lastMintedId;
            _lastMintedId++;
        }
        emit BatchMintEvent(amounts_, tokenIds);
    }
