...
contract Copyrights721 is IERC721Metadata, Ownable, ERC721URIStorage, IERC721Receiver {
  ...
  mapping(uint => RightsSale) public RoleToken; //this is where buyers cand go check their rights holdings and this is also how each rights sale is tracked
  ...
  function _onlyRightOwner(uint256 _rightid) private view {
    require(RoleToken[_rightid]._RoleOwner==msg.sender,"6"); //"Ownable: caller is not this right's owner!");
  }
  ...
  function onERC721Received(address _operator, address _from, uint256 _tokenId, bytes calldata _data) external override returns(bytes4)  {
    ...
    RoleToken[_uniqueRightID]=RightsSale(_from,_uniqueRightID, _ContentID, 0, RightsType[0]);
    _uniqueRightID++;
  ...
  function AddCoCreators(address _cocreator, uint _ContentTokenID, uint _percentage) external onlyContentCreator {
    ...
    RoyaltyTokenCount++;
    RoleToken[_uniqueRightID]=RightsSale(_cocreator,_uniqueRightID, _ContentTokenID, 0,RightsType[1]);
  ...
  function BuyRightsFromCreator( uint _contentTokenID, uint _rightsID) external  onlyApprovedBuyer {
    ...
    RoleToken[_uniqueRightID]=RightsSale(msg.sender,_uniqueRightID, _contentTokenID, _rightsID, RightsType[_rightsID]);
  ...
  function BuyRightsFromPrivateSeller(uint256 _rightsid) external onlyApprovedBuyer {
    ...
    _safeTransfer(RoleToken[_rightsid]._RoleOwner, msg.sender, _rightsid, "");
    UsersCreditBalance[msg.sender]-=RightForSale[_rightsid];
    UsersCreditBalance[RoleToken[_rightsid]._RoleOwner]+=RightForSale[_rightsid];
    RoleToken[_rightsid]._RoleOwner=msg.sender;
    emit RightsSold(msg.sender, _rightsid, RoleToken[_rightsid]._MainSkill, RoleToken[_rightsid]._Abilities);
    RightsTokensSupply[RoleToken[_rightsid]._MainSkill][RoleToken[_rightsid]._SecondarySkill]--;
    _ComputeRoyalties( RoleToken[_rightsid]._MainSkill,RightForSale[_rightsid]);
  ...
  function PostRightForSale(uint _rightsid, uint256 _price) external onlyRightOwner(_rightsid) {
    ...
    RightsTokensSupply[RoleToken[_rightsid]._MainSkill][RoleToken[_rightsid]._SecondarySkill]+=1;
  }
  ...
  function _ComputeRoyalties(uint256 _contentid, uint256 _price) internal {
    ...
    for (uint i=0; i < RoyaltyTokenCount; i++) {
      if (RoleToken[RoyaltyTokenIDs[i]]._MainSkill==_contentid){
  ...
  function exerciseRights(uint _uniqueRightsID, uint _AmountOfTimes) external {
    ...
    require(RoleToken[_uniqueRightsID]._RoleOwner==msg.sender,"3");//"The account used is not the owner of this right.");
    require(UsersCreditBalance[msg.sender]-(PriceOfRightUsage[RoleToken[_uniqueRightsID]._MainSkill][RoleToken[_uniqueRightsID]._SecondarySkill]*_AmountOfTimes)>=0, "4");//"Add more credit in order to be able to exercise this right." );
    UsersCreditBalance[msg.sender]-=(PriceOfRightUsage[RoleToken[_uniqueRightsID]._MainSkill][RoleToken[_uniqueRightsID]._SecondarySkill]*_AmountOfTimes);
    ExercisedRights[RoleToken[_uniqueRightsID]._MainSkill][RoleToken[_uniqueRightsID]._SecondarySkill]+=_AmountOfTimes;
    emit ExercisedRight(RoleToken[_uniqueRightsID]._MainSkill, RoleToken[_uniqueRightsID]._SecondarySkill, _AmountOfTimes);
    _ComputeRoyalties(RoleToken[_uniqueRightsID]._MainSkill, PriceOfRightUsage[RoleToken[_uniqueRightsID]._MainSkill][RoleToken[_uniqueRightsID]._SecondarySkill]*_AmountOfTimes);
  }
...
/*
  ...
  function transferRight(address _from, address _to, uint256 _UniqueRightID) external onlyApprovedBuyer {
    ...
    emit RightsTransferred(_from, _to, _UniqueRightID, RoleToken[_UniqueRightID]._ContentID, RoleToken[_UniqueRightID]._RightID);
  } 
*/
