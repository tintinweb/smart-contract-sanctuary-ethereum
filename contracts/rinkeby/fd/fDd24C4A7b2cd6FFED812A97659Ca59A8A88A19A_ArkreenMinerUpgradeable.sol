    ...
    UUPSUpgradeable 
{
    ...
    address public minerManager;                    // Manager approving miners registering  
    uint256 public nonceManager;                    // Nonce of the manager approving miners registering
    ...
    bool public gamingPhase;

    //
    mapping(uint256 => Miner) public AllMinerInfo;

    // keccak256("MinerRegister(address owner,address[] miners,uint256 nonce,uint256 feeRegister,uint256 deadline)");
    bytes32 public constant PERMIT_TYPEHASH = 0x8301B1A705596B24755477641B1212F9E4325F12EE729D2979EB7FFDC028A480;  
...
/*     = keccak256(
            abi.encode(
                ...
                keccak256(bytes("ArkreenMinerUpgradeable")),
                keccak256(bytes('1')),
                block.chainid,
                address(this)
            )
...
*/
    ...
    modifier isGamingPhase() {
        require(gamingPhase, 'Arkreen Miner: Gaming Phase Ended');
        _;
    }

/*
    constructor(address _tokenAKRE, address _minerManager) ERC721(NAME, SYMBOL) {
        tokenAKRE = _tokenAKRE;
        minerManager = _minerManager;
        gamingPhase = true;
...
*/
    ...
    /**
     * @dev Push game miners to the user having bought some mining DTUs. If there is one game miner running, 
     * activate this miner first. 
     * @param receiver address receiving the game miner tokens
     * @param miners address of the game miners, the first address is zero if there is one game miner is running        
    ...
    function PushMiners(
        ...
        tmpMiner.mOrderID = orderID;

        uint256 index = 0;

        // If any, get current free game miner 
        uint256 freeGameID = getRunningMiner(receiver, address(0));
        if(miners[0] == address(0)) {
            // Check there is one running game miner
            require(freeGameID != type(uint256).max, 'Game Miner: Holding No Game Miner');

            // Remove from the pending airdropped set if it pending
            if(AllMinerInfo[freeGameID].mStatus == MinerStatus.Pending) {
                allPendingGameMiners.remove(freeGameID);
            }

            // Update the running game miner status
            miners[0] = tmpMiner.mAddress = AllMinerInfo[freeGameID].mAddress;
            AllMinerInfo[freeGameID] = tmpMiner;

            // Flag that the first miner handled
            index = 1;
        } else {
            // Must consuming the running game miner first 
            require(freeGameID == type(uint256).max, 'Game Miner: Holding Game Miner');
        }

        for(; index < miners.length; index++) {
     ...
     * @param signature signature of onboarding manager to approve the onboarding
     */
    function GameMinerOnboard(
    ...
    ) external ensure(signature.deadline) isGamingPhase {
        // Miner onboarding must be EOA address 
        require(!miner.isContract(), 'Game Miner: Not EOA Address');

        // Check signature
        bytes32 hashRegister = keccak256(abi.encode(PERMIT_TYPEHASH, owner, miner, bAirDrop, 
                                        nonceManager++, signature.feeRegister, signature.deadline));
        ...
        require(recoveredAddress != address(0) && recoveredAddress == minerManager, 'Game Miner: INVALID_SIGNATURE');
        ...
        if(bAirDrop) {
            ...
            require(balanceOf(owner) == 0, 'Game Miner: Holding Game Miner');
            uint256 gMinerID = totalSupply();
            _safeMint(owner, gMinerID);
            AllMinerInfo[gMinerID] = tmpMiner;
    ...
    /**
     * @dev Onboarding a new DTU miner, and terminate a running game miner if any.
     * @param owner address receiving the DTU miner
     * @param gameMiner address of the game miner to terminate
     * @param miner address of the DTU miner onboarding
     * @param minerType type of the DTU miner onboarding
     * @param signature signature of onboarding manager to approve the DTU miner onboarding
     */
    function MinerOnboard(
    ...
    ) external ensure(signature.deadline) {
        // Check if the manufacturer is registered
        require(AllManufactures[msg.sender], 'Arkreen Miner: Not Registered');

        // Miner onboarding must be an EOA address 
        require(!miner.isContract(), 'Arkreen Miner: Must Be EOA Address');

        // New miner cannot be game miner
        require(minerType != MinerType.GameMiner, 'Arkreen Miner: Cannot Be Game Miner');

        // Check signature
        bytes32 hashRegister = keccak256(abi.encode(PERMIT_TYPEHASH, owner, gameMiner, miner, minerType,
                                        nonceManager++, signature.feeRegister, signature.deadline));
        bytes32 digest = keccak256(abi.encodePacked('\x19\x01', DOMAIN_SEPARATOR, hashRegister));

        address recoveredAddress = ecrecover(digest, signature.v, signature.r, signature.s);
        require(recoveredAddress != address(0) && recoveredAddress == minerManager, 'Arkreen Miner: INVALID_SIGNATURE');

        // If any, get current running game miner 
        uint256 runningGameID = getRunningMiner(owner, address(0));

        if(gameMiner == address(0)) {
            // Must stopping the running game miner first 
            require(runningGameID == type(uint256).max, 'Arkreen Miner: Game Miner Still Running');
        } else {
            // Bording a DTU miner to replace the game miner
            // if there are running game miners, must specify the correct game miner address to stop
            require(runningGameID != type(uint256).max, 'Arkreen Miner: No Game Miner running');
            runningGameID = getRunningMiner(owner, gameMiner);
            require(runningGameID != type(uint256).max, 'Arkreen Miner: Wrong Game Miner Specified');
            if(AllMinerInfo[runningGameID].mOrderID == bytes6(0)) {
                // Flag the time point calcualting gAKRE conserviosn
                AllMinerInfo[runningGameID].mOrderID = bytes6(0x999999999999);
                AllMinerInfo[runningGameID].timestamp = uint32(block.timestamp);    
            }
            AllMinerInfo[runningGameID].mStatus = MinerStatus.Terminated;
        }

        // Prepare to mint new DTU miner
        ...
        // mint new DTU miner
        uint256 idDTUMiner = totalSupply();               
        _safeMint(owner, idDTUMiner);    
        AllMinerInfo[idDTUMiner] = newMiner;

        // Transfer onboarding fee
        TransferHelper.safeTransferFrom(tokenAKRE, msg.sender, address(this), signature.feeRegister);
    ...
    /**
     * @dev Close the phase mining with game miner
     */
    function CloseGamePhase() external onlyOwner {
        require(gamingPhase, 'Game Miner: Game Miner Phase Closed');
        gamingPhase = false;
    }    
     ...
     * @param receiver address the onboarding fee
     */
    function withdraw(address receiver) public onlyOwner {
        uint256 balance = IERC20(tokenAKRE).balanceOf(address(this));
...
//    }
    ...
    function _baseURI() internal pure virtual override returns (string memory) {
        return "https://www.aitos.io/miners/";
    }
