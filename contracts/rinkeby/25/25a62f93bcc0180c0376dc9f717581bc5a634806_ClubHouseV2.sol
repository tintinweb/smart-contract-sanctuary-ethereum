...
contract ClubHouseV2 is ERC721Upgradeable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
     ...
     */
    event createdNft(address user, uint256 referenceId, uint256 tokenId, uint256 price);

     /**
     * @dev Emitted when token is purchased by user.
     */
    event purchaseNft(address previousOwner, address newOwner, uint256 tokenId, uint256 price);
    ...
    function mint(Order calldata _order, bytes memory _signature)
        external
        payable
        virtual
        nonReentrant
        returns (bool)
    {
        ...
        emit createdNft(msg.sender, _order.id, tokenCounter, msg.value);
