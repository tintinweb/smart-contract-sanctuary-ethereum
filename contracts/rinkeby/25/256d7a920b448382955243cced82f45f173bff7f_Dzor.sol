...
contract Dzor is ERC721,Whitelist {
    ...
    function checkExitscurrency(address currency) private view returns (bool){
       bool found=false;
    ...
    function addCurrenicies(address currency) public onlyOwner{
        require(checkExitscurrency(currency)==false,"Currency Alreday Added");
        currencies.push(currency);
    }
     function setTokenPrice(uint256 price, uint256 tokenId,bool sell,address paymentCurrency,bool isToken,uint256 _discount) public {
          ...
          nftINFO[tokenId].discount=_discount;
    }
	...
	function mintNFT(string memory _tokenURI,uint256 amount, uint256 tokenId, bool isToken,uint256 price,address paymentCurrency,uint256 royalties,uint256 discount) public payable {
        ...
        nftINFO[tokenId].discount=discount;
	}
    ...
    function buyWithTokens(uint256 tokenId,address payable _reference,uint256 amount) internal {
        uint256 _discount=nftINFO[tokenId].price.sub((nftINFO[tokenId].price.mul(nftINFO[tokenId].discount)).div(100));
        require(!isBlacklisted[msg.sender], "caller is backlisted");  
        require(hasSaleStarted == true, "Sale hasn't started");
        ...
        require(amount >=_discount, "The Token submitted with this transaction is too low.");
	...
	function buyWithBNB(uint256 tokenId,address payable _reference) public payable {
        ...
        require(msg.value >=_discount, "The value submitted with this transaction is too low.");
		uint256 owner_fees = 0;
		if(_reference == address(0)){
		    owner_fees = uint256(100) - sellingFess - nftINFO[tokenId].royalty;
		}else{
