...
contract PlayerStoxToken is Context, IERC20 {
     ...
     */
    constructor (string memory name_, string memory symbol_, uint8 decimals_, uint256 totalSupply_, address cricStoxMasterAddress_) {
        _name = name_;
        _symbol = symbol_;
        _decimals = decimals_;
...
contract CricStoxFactory is Ownable {
    ...
    function createPlayerStox(string memory name_, string memory symbol_, uint8 decimals_, uint256 totalSupply_) external onlyOwner {
        PlayerStoxToken playerStox = new PlayerStoxToken(name_, symbol_, decimals_, totalSupply_, cricStoxMasterAddress);
