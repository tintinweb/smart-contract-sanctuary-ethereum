...
contract StickyPool is IJellyContract,  IStickyPool,  StickyPoolNFT {
  ...
  /// @notice Deposit `_value` tokens for `_tokenId` and add to the lock
    ...
    function _tokenURI(uint _tokenId, uint _balanceOf, uint _locked_end, uint _value) internal pure returns (string memory output) {
        ...
        output = string(abi.encodePacked(output, "token ", _tokenId.toString(), '</text><text x="10" y="40" class="base">'));
        output = string(abi.encodePacked(output, "balanceOf ", _balanceOf.toString(), '</text><text x="10" y="60" class="base">'));
        output = string(abi.encodePacked(output, "locked_end ", _locked_end.toString(), '</text><text x="10" y="80" class="base">'));
        output = string(abi.encodePacked(output, "value ", _value.toString(), '</text></svg>'));

        string memory json = Base64.encode(bytes(string(abi.encodePacked('{"name": "lock #', _tokenId.toString(), '", "description": "StickyPool NFT. Represents staked tokens, used for voting and escrowed until locked_end.", "image": "data:image/svg+xml;base64,', Base64.encode(bytes(output)), '"}'))));
...
contract StickyPoolNFT is IERC721Metadata {
    ...
    string constant public override name = "Sticky Pool NFT";
