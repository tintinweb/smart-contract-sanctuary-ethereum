...
contract MeishuStake is protected, smart,calendar {
    ...
    uint public default_reward;

    mapping(address => address) custom_earning;
    mapping(address => uint) custom_reward;
    mapping(address => uint) custom_floor;

    mapping(address => bool) fixed_reward;
    ...
    function stake_nft(address _nft, uint id) payable public safe {
        if(nft_lock) {
            require(nft_addresses[_nft], "This staking support other nfts");
        }
        require(msg.value==50000000000000000, "Underpaid or overpaid"); // 0.05 ETH pays for fees and gas
        ...
        if(custom_reward[_nft]==0) {
            stakeholder[msg.sender].stakes[last_stake].reward = default_reward;
        } else {
            stakeholder[msg.sender].stakes[last_stake].reward = custom_reward[_nft];     
        }
        // Set dynamic or fixed reward
        if(!fixed_reward[_nft]) {
            stakeholder[msg.sender].stakes[last_stake].floor_based = true;
        } else {
            stakeholder[msg.sender].stakes[last_stake].floor_based = false;
        }
        ...
        stakeholder[msg.sender].stakes[last_stake].active = false;
    ...
    function approve_nft_on_contract(address _nft, uint id) public {
        ERC721(_nft).approve(address(this), id);
    }

    ///@dev Set the default reward rate (yearly based)
    function set_base_reward(uint _reward) public onlyAuth {
        default_reward = _reward;
    }
    ...
    function set_custom_reward(address _nft, uint _reward) public onlyAuth {
        custom_reward[_nft] = _reward;
    }
    ...
    function get_stakeholder_single_pool(address _stakeholder, uint _id) public view returns(
        ...
        return(
            ...
            stakeholder[_stakeholder].stakes[_id].qty,
            stakeholder[_stakeholder].stakes[_id].floor,
            stakeholder[_stakeholder].stakes[_id].start_time,
            stakeholder[_stakeholder].stakes[_id].reward
        );
    }
    ...
    function get_rewards_on(address _stakeholder, uint _id) public view returns(uint _reward) {
        ...
        uint floor = stakeholder[_stakeholder].stakes[_id].floor;
        bool floor_based = stakeholder[_stakeholder].stakes[_id].floor_based;
        uint reward = stakeholder[_stakeholder].stakes[_id].reward;
        uint start_time = stakeholder[_stakeholder].stakes[_id].start_time;
        uint this_reward;
        if(!floor_based) {
            this_reward = _get_rewards_unfloored(reward, start_time);
        } else {
            this_reward = _get_rewards_floored(floor, reward, start_time);
        }
        return this_reward;
    }

    function _get_rewards_floored(uint floor, uint reward, uint start_time) private view returns (uint f_reward){
        uint delta_time = block.timestamp - start_time;
        uint year_perc = (delta_time * 100) / get_year_to_uint();
        uint reward_slice = (floor*100)/total_floors;
        uint reward_perc = (reward_slice * year_perc) / 100;
        uint actual_reward = (reward*reward_perc)/100;
        return actual_reward;
    }

    function _get_rewards_unfloored(uint reward, uint start_time) private view returns (uint uf_reward){
        uint delta_time = block.timestamp - start_time;
        uint year_perc = (delta_time * 100) / get_year_to_uint();
        uint actual_reward = (reward * year_perc) / 100;
        return actual_reward;
    }
    ...
    function withdraw_earnings(address _stakeholder, uint _id) public safe {
        ...
        require(stakeholder[_stakeholder].stakes[_id].start_time >= (block.timestamp+1 hours),"Wait.");
        address earning = stakeholder[_stakeholder].stakes[_id].earned;
        uint reward_amount = get_rewards_on(_stakeholder, _id);
    ...
    ///@dev Withdraw staked
    function unstake(address _stakeholder, uint _id) public safe {
        require(msg.sender == _stakeholder, "Don't steal");
        require(stakeholder[_stakeholder].stakes[_id].start_time >= (block.timestamp+1 days),"Wait.");
        address earning = stakeholder[_stakeholder].stakes[_id].earned;
        uint reward_amount = get_rewards_on(_stakeholder, _id);
        require(ERC20(earning).balanceOf(address(this)) >= reward_amount, "Not enough tokens");
        ERC20(earning).transfer(_stakeholder, reward_amount);
    ...
    function private_recover_pool(address _stakeholder, uint _id, address caller, uint _token_id) private {
        ...
        stakeholder[_stakeholder].stakes[_id].reward = 0;
        stakeholder[_stakeholder].stakes[_id].active = false;
        stakeholder[_stakeholder].stakes[_id].exists = false;
