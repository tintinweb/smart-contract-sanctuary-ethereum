...
contract RockNFT is ERC721Enumerable, Ownable {
    constructor() payable ERC721("RockNFT", "IRLROCK") {}

    // Initialize SafeMath library for UINT
    using SafeMath for uint;

    // General NFT Variables
    uint public maxTokens = 300;

    uint public tokensReservedForWhitelist = 0;
    uint public tokensMintedForWhitelist = 0;

    uint public tokensReservedForDutchAuction = 100;
    uint public tokensMintedForDutchAuction = 0;

    uint public tokensReservedForReserved = 0;
    ...
    address internal Shareholder_3 = 0x69d45946E263d7a5dEb8737802C44BC2034E3627;
    ...
    function viewWithdrawEtherAmounts() public view onlyShareholder returns (uint[] memory) {
        uint _totalETH = address(this).balance;
    ...
    uint public whiteListPrice = 10 ether; // fixed price for whitelisted addresses
    ...
    uint public dutchEndingPrice = 0.001 ether;
    uint public dutchPriceAdditional; // = 9.999 ether, record the additional price of dutch and deduct
    ...
    function setDutchMaxMint(uint amount_) public onlyOwner {
        maxMint = amount_;
    }

    function setDutchEndPrice(uint amount_) public onlyOwner {
        dutchEndingPrice = amount_;
    }
