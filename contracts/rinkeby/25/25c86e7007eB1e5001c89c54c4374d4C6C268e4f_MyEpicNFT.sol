...
contract MyEpicNFT is ERC721URIStorage {
    ...
    // 4つの配列 string[] に、それぞれランダムな単語を設定しましょう。
    string[] firstWords = ["Y", "U", "K", "I"];
    string[] secondWords = ["Y", "U", "K", "I"];
    string[] thirdWords = ["Y", "U", "K", "I"];
    string[] fourthWords = ["Y", "U", "K", "I"];

    event NewEpicNFTMinted(address sender, uint256 tokenId, uint256 _tokenIds);
    ...
    // pickRandomThirdWord関数は、4番目に表示される単語を選びます。
    ...
    function makeAnEpicNFT() public {
        ...
        string memory combinedWord = string(
            abi.encodePacked(first, second, third, fourth)
        );

        // 4つの単語を連結して、<text>タグと<svg>タグで閉じます。
