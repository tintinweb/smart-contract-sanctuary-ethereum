...
contract Spell is Ownable, ERC721AS, ReentrancyGuard {
    ...
    function publicSaleMint(uint256 quantity) external payable {
        ...
        bool potionHolder = isPotionHolder();
        if (potionHolder && numberMinted(msg.sender) + quantity > 5) {
            uint numberToPay;
            ...
            require(uint256(publicPrice) * numberToPay <= msg.value,"Not enough ETH, potion holders can mint 5 potion for free");
        } else if (!potionHolder && numberMinted(msg.sender) + quantity > 1) {
            uint numberToPay;
            ...
            require(uint256(publicPrice) * numberToPay <= msg.value,"Not enough ETH, you don't own any potion, 1 free mint is allowed");
        }
  ...
  IERC721Enumerable
{
  ...
  function isPotionHolder() public view returns (bool) {
    Potion potionContract = Potion(potionAddress);
     return potionContract.balanceOf(msg.sender) > 0;
  }
  ...
  function craft(uint potionId, uint spellId) public {
    ...
    if (bookTypeOf(spellId) == 0) {
      ...
      require(potionContract.typeOf(potionId) == spellTypeOf(spellId), "Need potion with the same type.");
      craftDataMap[msg.sender].newBookType = bookTypeOf(spellId);
  ...
  function claim() public payable {
    ...
    require(ownerOf(spellId) == msg.sender, "This is not your spell book.");
    require(potionContract.ownerOf(potionId) == msg.sender, "This is not your potion.");
    require(block.number - startBlock > 1, "Still crafting..."); //TEST, PROD 3600
