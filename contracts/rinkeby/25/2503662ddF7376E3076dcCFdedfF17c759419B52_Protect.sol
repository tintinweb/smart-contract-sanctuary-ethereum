...
contract Protect is IERC721Receiver {
    ...
    function protectId() public view returns (uint256) {
        return _protectId;
    }

    function mergeId() public view returns (uint256) {
        return merge.tokenOf(address(this));
    }
    ...
    function removeProtection(address to) public virtual holdsProtectedToken {
        uint256 protectId_ = _protectId;
        uint256 mergeId_ = mergeId();
        _protectId = 0;
        require(pMerge.burn(protectId_), "failed to burn wrapped merge");
        merge.transferFrom(address(this), to, mergeId_);
    }

    function owner() public view virtual returns (address) {
        return pMerge.ownerOf(_protectId);
    }
    ...
    function onERC721Received(
        ...
        if (_protectId == 0) {
            ...
            uint256 protectId_ = pMerge.mint(from);
            _protectId = protectId_;
        } else {
            // verify only the owner can send tokens to the contracts
            require(owner() == from, "only the owner can merge");
        }

        uint256 _mass = merge.massOf(tokenId);

        emit Received(from, tokenId, _mass);
