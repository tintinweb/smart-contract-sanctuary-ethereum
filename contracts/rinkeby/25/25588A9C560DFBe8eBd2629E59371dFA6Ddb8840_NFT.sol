...
// Imports
import "../extensions/Reveal.sol";
import "../extensions/ERC721A/ERC721A.sol";
import "../extensions/ERC721A/ERC721AQueryable.sol";
...
contract NFT is ERC721A, ERC721AQueryable, Ownable, Reveal {
    ...
    string public tokenName = "Lost Alien Society ";
    /// @notice The symbol of the NFT collection.
    string public tokenSymbol = "LAS";
     ...
     * @param unrevealedUri_ The URL of a media that is shown for unrevealed NFTs.
    ...
    ) ERC721A(tokenName, tokenSymbol) Reveal(unrevealedUri_) {
        // Set the variables.
    ...
    /**
      ...
      * @param flag The flag to approve or disapprove a minter.
      */
    ...
    function mint(address recipient, uint256 quantity) external {
        ...
        require(areReservesMinted, "RESERVED_TOKENS_NOT_MINTED");
        // Check that the number of tokens to mint does not exceed the total amount.
        require(totalSupply() + quantity <= maxSupply, "MAX_SUPPLY_EXCEEDED");
        // Mint the tokens.
        _safeMint(recipient, quantity);
    }

    /**
      * @notice Mints multiple NFT tokens for multiple addresses.
      * @param recipients The NFT tokens recipients.
      * @param quantities The number of NFT tokens to mint for each recipient.
      */
    function batchMint(address[] memory recipients, uint256[] memory quantities) external {
        require(isMinter[msg.sender], "NOT_AUTHORIZED");
        // Reserves should be minted before minting standard tokens.
        require(areReservesMinted, "RESERVED_TOKENS_NOT_MINTED");
        // Recipients length matches quantities length
        require(recipients.length == quantities.length, "RECIPIENTS_DO_NOT_MATCH_QUANTITIES");
        uint256 totalQuantity = 0;
        for (uint i = 0; i < quantities.length; i++) {
            totalQuantity = totalQuantity + quantities[i];
        }
        // Check that the number of tokens to mint does not exceed the total amount.
        require(totalSupply() + totalQuantity <= maxSupply, "MAX_SUPPLY_EXCEEDED");
    ...
    function mintReserves(address recipient, uint256 quantity) external onlyOwner{
        ...
        require(areReservesMinted == false, "RESERVED_TOKENS_ALREADY_MINTED");
        // Check if the desired quantity of the reserved tokens to mint doesn't exceed the reserve.
        require(totalSupply() + quantity <= numReservedTokens, "RESERVED_SUPPLY_EXCEEDED");
     ...
     /**
      * @notice Batch transfers NFTs.
      * @param ids[] The ids of the NFT to transfer.
      * @param recipients[] The NFT tokens recipients.
      */
    function transferBatch(uint256[] memory ids, address[] memory recipients) external {
        // This function assumes the sender is approved or owns the NFT.
        require(ids.length == recipients.length, "LENGTH_MISMATCH");
        for (uint i = 0; i < ids.length; i++) {
            safeTransferFrom(_msgSender(), recipients[i], ids[i]);
        }
    }
     ...
     * @return The base URLs for tokens.
     */
