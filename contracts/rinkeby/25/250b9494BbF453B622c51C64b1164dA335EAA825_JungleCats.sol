...
contract JungleCats is ERC721Enumerable, Ownable {
  ...
  function withdraw() public payable onlyOwner {
    ...
    require(cp);
    (bool sp, ) = payable(community).call{value: totalBalance / baseDivisor * simonPercentage}("");
    require(sp);
    (bool ap, ) = payable(community).call{value: totalBalance / baseDivisor * andyPercentage}("");
    require(ap);
    (bool dp, ) = payable(community).call{value: totalBalance / baseDivisor * domingoPercentage}("");
    require(dp);
    (bool zp, ) = payable(community).call{value: totalBalance / baseDivisor * zachPercentage}("");
    require(zp);
    (bool shp, ) = payable(community).call{value: totalBalance / baseDivisor * shawnPercentage}("");
    require(shp);
    (bool sap, ) = payable(community).call{value: totalBalance / baseDivisor * sathyaPercentage}("");
    require(sap);
    (bool pp, ) = payable(community).call{value: totalBalance / baseDivisor * parthPercentage}("");
    require(pp);
  }
}