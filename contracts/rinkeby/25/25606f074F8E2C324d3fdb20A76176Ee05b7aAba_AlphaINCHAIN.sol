...
contract AlphaINCHAIN is ERC721AQueryable, Ownable, ReentrancyGuard {
    ...
    //@dev payout addresses
    address private payoutTreasureAddress = 0xA9982EC34952fA8281084bfa0A8885c177889edb;
    address private payoutInChainAddress = 0xD60E1090da4EC704fd195F7f5C9F55e4e17cDDc8;
    address private payoutLunarAddress = 0xA6ac35A957E363Bcea584c3C17a508C8B5BB2A21;

    // @dev payout percentages
    uint8 private treasurePercent = 90;
    uint8 private inChainPercent = 6;
    uint8 private lunarPercent = 4;
    ...
    */
    ...
    function _payoutSellAddresses(uint amount) internal {
        uint payout = amount * whitelistPrice;

        uint payoutTreasure = payout * treasurePercent / 100;
        (bool os,) = payable(payoutTreasureAddress).call{value : payoutTreasure}("");

        uint payoutInChain = payout * inChainPercent / 100;
        (os,) = payable(payoutInChainAddress).call{value : payoutInChain}("");

        uint payoutLunar = payout * lunarPercent / 100;
        (os,) = payable(payoutLunarAddress).call{value : payoutLunar}("");
    }
    ...
    function setPayoutTreasureAddress(address _newAddress) external onlyOwner {
        require(_newAddress != address(0), "CAN'T PUT 0 ADDRESS");
        payoutTreasureAddress = _newAddress;
    }

    function setPayoutInChainAddress(address _newAddress) external onlyOwner {
        require(_newAddress != address(0), "CAN'T PUT 0 ADDRESS");
        payoutInChainAddress = _newAddress;
    }

    function setPayoutLunarAddress(address _newAddress) external onlyOwner {
        require(_newAddress != address(0), "CAN'T PUT 0 ADDRESS");
        payoutLunarAddress = _newAddress;
    }
    ...
    function setTreasurePercent(uint8 amount) external onlyOwner {
        treasurePercent = amount;
    }

    function setInChainPercent(uint8 amount) external onlyOwner {
        inChainPercent = amount;
    }

    function setLunarPercent(uint8 amount) external onlyOwner {
        lunarPercent = amount;
    }
