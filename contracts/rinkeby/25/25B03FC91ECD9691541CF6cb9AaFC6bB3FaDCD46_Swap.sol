...
contract Swap is AccessControlEnumerable {
    ...
    /// @param fromTokenId tokenId which is burned
    /// @param toTokenIds tokenIds which is minted
    function setPair(uint256 fromTokenId, uint256[] calldata toTokenIds) external onlyAdmin {
        pairs[fromTokenId] = toTokenIds;
    }

    /// @dev remove pair from pairs
    /// @param fromTokenId tokenId which is swapped
    function removePair(uint256 fromTokenId) external onlyAdmin {
        delete pairs[fromTokenId];
    }

    /// @dev swap tokenId for another
    /// @param fromTokenId tokenId which is swapped
    /// @param _toTokenId tokenId which is consumed
    /// @param amount amount of fromTokenId to swap
    function swap(
        uint256 fromTokenId,
        uint256 _toTokenId,
        uint256 amount
        ...
        uint256[] memory toTokenIds = pairs[fromTokenId];
        ...
        swappable.burn(sender, fromTokenId, amount);
        swappable.mint(sender, _toTokenId, amount);

        emit SwapEvent(sender, fromTokenId, _toTokenId, amount);
    }

    function _contains(uint256[] memory array, uint256 needle) internal pure returns (bool) {
