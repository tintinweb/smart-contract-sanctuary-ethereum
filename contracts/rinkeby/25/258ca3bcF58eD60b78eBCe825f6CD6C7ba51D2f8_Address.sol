...
contract NFTMarket is ReentrancyGuard ,Ownable   {
     ...
     */
    mapping(address => mapping(uint256 => NftData)) public nftMarketItems;
    ...
    modifier isCheckOwner(address _nftContractAddress,uint256 _tokenId) {
      require(
          nftMarketItems[_nftContractAddress][_tokenId].owner == msg.sender,
          "ony nft owner can do this operation"
      );
      _;
    }
    ...
    function sellNFTOnMarket(
        ...
        nftMarketItems[_nftContractAddress][_tokenId] =  NftData(
            _nftContractAddress,
            _tokenId,
            payable(msg.sender),
            _price,
    ...
    function buyNFTOnMarket(
        ...
        nonReentrant {
            ...
            require(nftMarketItems[_nftContractAddress][_tokenId].nftContractAddress == _nftContractAddress, "Incorrect contract address!");
            require(msg.value >= nftMarketItems[_nftContractAddress][_tokenId].price, "Invalid price!");
            require(_tokenId >= 0, "Incorrect token id!"); 

            uint256 commision = SafeMath.div(
                SafeMath.mul(msg.value, commisionFeesPercentage),
                1000
            );

            uint256 royalties = SafeMath.div(
                SafeMath.mul(
                  msg.value, nftMarketItems[_nftContractAddress][_tokenId].royalties
                ),
                1000
            );

            Address.sendValue(
              nftMarketItems[_nftContractAddress][_tokenId].owner,
              SafeMath.sub(
                msg.value,
                SafeMath.add(
                  commision ,
            ...
            Address.sendValue(
                payable(nftMarketItems[_nftContractAddress][_tokenId].nftMintOwner),
                royalties
            );
            ...
            _resetValue(_nftContractAddress,_tokenId);
    }
     ...
     */
    function _resetValue(address _nftContractAddress, uint256 _tokenId)
        internal
    {
        nftMarketItems[_nftContractAddress][_tokenId].nftContractAddress =address(0);
        nftMarketItems[_nftContractAddress][_tokenId].tokenId = 0;
        nftMarketItems[_nftContractAddress][_tokenId].owner = payable(address(0));
        nftMarketItems[_nftContractAddress][_tokenId]
            .price = 0;
        nftMarketItems[_nftContractAddress][_tokenId].nftMintOwner = address(
            0
        );
        nftMarketItems[_nftContractAddress][_tokenId]
            .royalties = 0;
        nftMarketItems[_nftContractAddress][_tokenId].onSelling = false;
    }
