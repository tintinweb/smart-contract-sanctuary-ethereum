    ...
    ERC1820Implementer
{
    ...
    uint256 public streamingDuration = 1800; //here the streaming duration can be defined in seconds
    uint256 public streamingInterval = 18; //here the streaming payout Intervall can be defined in seconds
        ...
        for (uint256 i = buyList.length - 1; i >= 0; ) {
            if (
                ...
                if (buyList[i].amount >= _amount) {
                    ...
                    units = units + _amount;
                    newStream.agreedAmount = _amount;
                    ...
                    //regrister active stream from the seller to buyer and make entry for the seller address
                    newStream.agreedAmount = buyList[i].amount;
                    addPartnerToSeller(newStream);
                    //transfer tokens
        ...
        for (uint256 i = sellList.length - 1; i >= 0; ) {
            if (
                ...
                if (sellList[i].amount >= _amount) {
                    ...
                    //regrister active stream from the seller to buyer and make entry for the seller address
                    newStream.agreedAmount = sellList[i].amount;
                    addPartnerToSellerNewBuy(newStream);
                    //transfer tokens
