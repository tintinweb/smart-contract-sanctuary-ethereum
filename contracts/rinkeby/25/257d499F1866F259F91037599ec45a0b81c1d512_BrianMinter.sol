...
import './BrianToken.sol';

contract BrianMinter is Ownable {
  BrianToken private token;
  ...
  uint256 public MAX_PER_WALLET = 9999999;
  uint256 public FREE_MINT = 9999999;
  ...
  constructor(address payable _contractAddress) {
    ...
    token = BrianToken(_contractAddress);
  }
  ...
  ***************************
  ...
  function mint(address to) public payable {
    ...
    if (token.totalSupply() < FREE_MINT) {
      // free minting
      token.safeMintNoURI(to);
    } else {
  ...
  ***************************
  ...
  function setFreeMind(uint256 _freeMint) public onlyOwner {
    FREE_MINT = _freeMint;
  }
}
  ...
  UUPSUpgradeable
{
  ...
  function safeMintNoURI(address to) public onlyRole(MINTER_ROLE) {
    uint256 tokenId = _tokenIdCounter.current();
    _tokenIdCounter.increment();
    _safeMint(to, tokenId);
  }
