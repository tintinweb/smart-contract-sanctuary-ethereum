...
contract ClubKawaiiPeaceProject is ERC1155, Ownable, Pausable, ERC1155Supply {
    ...
    function batchCreateNewTokens( uint256[] memory tokenIds_, uint256[] memory supplies_ )
    public
    onlyOwner
    {
        ...
        for ( uint256 i = 0; i < tokenIds_.length; i++ ) {
            ...
            uint256 supply = supplies_[i];
            if ( exists(tokenId) ) {
                _maxSupply[tokenId] = supply;
                tokensToMint[n] = tokenId;
                amountsToMint[n] = 1;
                n++;
            }
        }

        // mint first tokens to the owner
        _mintBatch( msg.sender, tokensToMint, amountsToMint, "" );
        emit BatchMint( msg.sender, tokensToMint, amountsToMint );

    }

    function reduceTokenSupply( uint256 tokenId_, uint256 newSupply_ )
    public
    onlyOwner
    {
        ...
        require(
            newSupply_ < maxSupply(tokenId_),
            "New supply must be lower than current max"
        );
    ...
    function batchReduceTokenSupply( uint256[] memory tokenIds_, uint256[] memory newSupplies_ )
    public
    onlyOwner
    {
        ...
        for ( uint256 i = 0; i < tokenIds_.length; i++ ) {
            uint256 tokenId = tokenIds_[i];
            uint256 newSupply = newSupplies_[i];
            if ( exists(tokenId) && newSupply < maxSupply(tokenId) && newSupply >= totalSupply(tokenId) ) {
                _maxSupply[tokenId] = newSupply;
            }
        }
    ...
    function batchMint( uint256[] memory tokenIds_, uint256[] memory amounts_ )
    public
    payable
    whenNotPaused
    {
        ...
        for ( uint256 i = 0;  i < tokenIds_.length; i++ ) {
            ...
            if ( exists(tokenId) && totalSupply(tokenId) + amount <= maxSupply(tokenId) ) {
                tokensToMint[n] = tokenId;
                amountsToMint[n] = amount;
                totalAmount += amount;
                n++;
            }
        }
        ...
        require(
            tokensToMint.length == tokenIds_.length,
            "Not all tokens or amounts available"
        );
    ...
    onlyOwner
    {
        ...
        _mintBatch( to_, tokensToMint, amountsToMint, "" );
        emit BatchMint( to_, tokensToMint, amountsToMint );
