...
contract HitchHikerOfStar is ERC721A, Ownable {
    ...
    function setDutchAuctionMintMaxAmount(uint256 amount) external onlyOwner {
    ...
    function addFreeMintCriteria(address[] memory criteria) external onlyOwner {
    ...
    function setCryptoPunksAddress(address _cryptoPunksAddress) external onlyOwner {
    ...
    function setFreeMintBaseTokenURI(string memory _baseUri) public onlyOwner {
        freeMintBaseTokenURI = _baseUri;
    }

    function setDutchAuctionBaseTokenURI(string memory _baseUri) public onlyOwner {
        dutchAuctionBaseTokenURI = _baseUri;
    }

    function setWhiteListMintBaseTokenURI(string memory _baseUri) public onlyOwner {
    ...
    function getFreeMintRemaining(address who) external view returns (uint256) {
        uint256 total;
        uint256 len = freeMintCriteria.length;
        for (uint256 i; i < len; ++i) {
    ...
    function getBaseURI(uint256 tokenId) internal view returns (string memory baseUri) {
