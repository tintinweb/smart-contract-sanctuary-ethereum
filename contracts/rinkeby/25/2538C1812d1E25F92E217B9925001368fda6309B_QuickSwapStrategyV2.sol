...
import "polygon/contracts/interfaces/quickswap/IStakingDualRewards.sol";
 ...
 */
contract QuickSwapStrategyV2 is BasePairLPStrategy, Multiex {
  using SafeERC20 for IERC20;
  using OndoSaferERC20 for IERC20;

  string public constant name = "Ondo QuickSwap V2 Strategy";
   ...
   */
  function _getPair(address senior, address junior)
    internal
    view
    returns (address)
  {
    return QuickSwapLibrary.pairFor(quickswapFactory, senior, junior);
  }
    ...
    returns (uint256 lpAmount)
  {
    ...
    if (rewardTokenAAmount > 10000 && rewardTokenBAmount > 10000) {
      ...
      if (
        ...
        (uint256 reserveA, ) =
          QuickSwapLibrary.getReserves(quickswapFactory, tokenA, tokenB);
  ...
  function _depositIntoStaking(uint256 _vaultId, uint256 _lpTokens) internal {
    ...
    if (poolData.totalLp == 0 || poolData.totalShares == 0) {
      poolData.totalShares = _lpTokens;
      poolData.totalLp = _lpTokens;
      vault.shares = _lpTokens;
  ...
  function addVault(
    ...
    require(_poolExists(_senior, _junior), "Pool doesn't exist");
    Vault storage vault_ = vaults[_vaultId];
    vault_.origin = IPairVault(msg.sender);
    vault_.pool = IERC20(_getPair(address(_senior), address(_junior)));
   ...
   * @param _junior Asset used for junior tranche
   */
  function _poolExists(IERC20 _senior, IERC20 _junior)
    internal
    view
    returns (bool)
  {
    return _getPair(address(_senior), address(_junior)) != address(0);
  }
  ...
  {
    return
      QuickSwapLibrary.getAmountsOut(quickswapFactory, juniorReceived, jr2Sr);
  }
