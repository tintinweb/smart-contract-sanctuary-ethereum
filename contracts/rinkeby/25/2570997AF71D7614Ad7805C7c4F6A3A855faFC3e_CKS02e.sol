...
contract CKS02e is Auth {
    uint256 private _settingsWindowDuration = 86400; // settings get unlocked only for 24 hours (86400), then will be locked again
    uint256 private _settingsUnlockTimer = 180; //settings unlock wait time - 30 sec local, 3 minutes (180) in testnet, 14 days (14*86400) in mainnet

    uint256 private _migrationMinUnlockWait = 180; //TODO: 30s in local test, 3 mins (180) in testnet, 30 days (30 * 86400) in Mainnet
    ...
    function settingsUnlockStartTimer() external onlyOwner {
        //settings can be changed after being unlocked, unlock is on a timer - 5 minutes in testnet, 14 days in mainnet
    ...
    function stakeTokensOnBehalf(address[] calldata addresses, uint256 amountNonDecimal, uint256 lockedUntil) external onlyOwner {
        ...
        //send tokens to staking contract in one bundle to save a fuckton of gas
        IERC20 tokenContract = IERC20(stakingSettings.caToken);
        require( tokenContract.allowance(msg.sender, address(this)) >= totalAmount, "Token allowance too low" );
        tokenContract.transferFrom(msg.sender, address(this), totalAmount);

        for(uint i=0; i < addresses.length; i++) {
            // _stakeTokens(msg.sender, addresses[i], amount); //replaced by direct call to increaseStaking because we already transferred the bundle of tokens for all recipients...
            _increaseStaking(addresses[i], amount);
            emit TokensStaked(addresses[i], amount);
