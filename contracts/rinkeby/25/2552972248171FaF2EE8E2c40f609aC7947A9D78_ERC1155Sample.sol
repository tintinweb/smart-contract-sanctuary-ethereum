    ...
    UUPSUpgradeable
{
    ...
    address private RARIBLE_ROYALTY_REGISTRY;
    ...
    function __ERC1155Sample_init(
        ...
        string memory uri_,
        address royaltyRegistry_
    ) external initializer {
        _name = name_;
        _symbol = symbol_;
        RARIBLE_ROYALTY_REGISTRY = royaltyRegistry_;
        __ERC1155_init(uri_);
    ...
    function mint(
        ...
        if (_royaltyValue > 0) {
            ...
            if (RARIBLE_ROYALTY_REGISTRY != address(0)) {
                LibPart.Part[] memory royalties = new LibPart.Part[](1);
                royalties[0].account = payable(_account);
                royalties[0].value = _royaltyValue;
                (bool success, ) = address(RARIBLE_ROYALTY_REGISTRY).call(
                    abi.encodeWithSignature(
                        "setRoyaltiesByToken(address token,LibPart.Part[] memory royalties)",
                        address(this),
                        royalties
                    )
                );
