...
contract Will {
    ...
    event Decased(address indexed owner,address indexed heir,uint amount);
    ...
    function decased(address owner) public {
        require(msg.sender == _admin,"unauthorized");
        require(_balances[owner] >0,"no testament");

        emit Decased(owner,_heirs[owner],_balances[owner]);
