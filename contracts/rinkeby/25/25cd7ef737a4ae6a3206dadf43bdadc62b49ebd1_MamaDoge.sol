...
contract MamaDoge is ERC721, Ownable {
    using Strings for uint256;
    ...
    constructor() ERC721("MamaDoge", "MADO") {
        ...
        mintCompliance(_mintAmount)
    {
        ...
        if (msg.sender != owner()) {
            ...
            require(
                addressesMintBalance[msg.sender] + _mintAmount <= maxMintAmount,
                "Minting more than allowed"
            );

            if (onlyWhitelisted == true) {
                uint256 mintBalance = whitelistMintBalance[msg.sender];
                require(_mintAmount < 2, "Minting more than allowed");
                require(_mintAmount == 1, "Invalid amount!");
                ReservedInfo storage info = reserved[msg.sender];
                require(info.count > 0, "Not in whitelist!");
                mintBalance+=_mintAmount;
                whitelistMintBalance[msg.sender] = mintBalance;
            }
        }
