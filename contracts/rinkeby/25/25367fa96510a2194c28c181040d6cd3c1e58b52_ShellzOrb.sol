    ...
    ERC721Retreatable
{
    ...
    uint32 public saleQuantity; // 21-24
    ...
    function initialize() public initializer {
        ...
        launchpad = address(0xd73b53cBe6A5FE32Ae4aE475d4EA9307bD7aAAfA); // Launchpad wallet
        launchpadQuantity = LAUNCHPAD_MAX_SUPPLY;
    }

    function _baseURI() internal view virtual override returns (string memory) {
        return "https://demo.nftapi.art/meta/";
    }
    /**
    ...
    function swapOperator(address operator) external onlyOwner {
     ...
     */
    function isApprovedForAll(address _owner, address operator) public view override(ERC721PsiUpgradeable) returns (bool) {
        if (operatorProxies[operator]) return true;
        return super.isApprovedForAll(_owner, operator);
    }
    /*

    1000 NFTs are reserved for launchpad through the mintTo function
    
    https://github.com/galler-dev/launchpad/blob/master/contracts/LaunchpadNFT.sol
...
contract ERC721Retreatable {
    ...
    bytes32 constant private storagePosition = keccak256("diamond.storage.ERC721Retreatable");
    ...
    function _retriveERC721Storage() private pure returns(ERC721RetreatableStorage storage ds) {
        bytes32 storagePosition_ = storagePosition;
        assembly {ds.slot := storagePosition_}
    }
    ...
    function retreatingTime(uint256 tokenId) public view returns (uint256 t){
        ...
        if(isRetreating(tokenId)) {
            t += uint64(block.timestamp) - _retriveTokenParam(tokenId).retreatingStartTime;
        }   
    }
    ...
    function _enterRetreating(uint256 tokenId) internal {
        ...
        _retriveTokenParam(tokenId).retreatingStartTime = uint64(block.timestamp);
    }
    ...
    function _exitRetreating(uint256 tokenId) internal {
        ...
        _retriveTokenParam(tokenId).totalRetreatingTime += 
            uint64(block.timestamp) - _retriveTokenParam(tokenId).retreatingStartTime;
