...
contract DaoPFP is ERC721Enumerable, ERC721URIStorage, Initializable {
    // for inherit
    ...
    constructor() ERC721("Dao PFP Card", "DapPFPCARD") {
        Admin = _msgSender();
    }
    ...
    function newCard(uint cardId_,string memory name_, string memory cardURI_) external onlyAdmin {
    ...
    function newCards(uint startCardId_, uint endCardId_) external onlyAdmin {
        require(startCardId_ != 0 && cardInfoes[startCardId_].cardId == 0
            && endCardId_ !=0 && cardInfoes[endCardId_].cardId == 0 && startCardId_<=endCardId_, "ERC721: wrong cardId");
        for(uint i = startCardId_; i <= endCardId_; i++){
            cardIds.add(i);
            cardInfoes[i] = CardInfo({
            cardId : i,
            name : Strings.toString(i),
            currentAmount : 0,
            cardURI : Strings.toString(i)
            });
        }
    }

    function updateCard(string memory name_, uint cardId_,string memory cardURI_) external onlyAdmin {
        require(cardId_ != 0 && cardInfoes[cardId_].cardId != 0, "ERC721: wrong cardId");
    ...
    function mint(uint cardId_,address payUser) external onlyAdmin{
        require(cardId_ != 0 && cardInfoes[cardId_].cardId != 0, "ERC721: wrong cardId");
