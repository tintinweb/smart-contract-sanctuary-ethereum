...
contract RandomSVG is ERC721URIStorage, VRFConsumerBase {
  ...
  function finishMint(uint256 _tokenId) public {
    ...
    //tokenCounter should be greater than tokenID if randomness has been requested
    require(tokenCounter > _tokenId, "tokenID has not been minted yet");
    //check if tokenID has a random number assigned  
    require(tokenIdToRandomNumber[_tokenId] > 0, "tokenID does not have random number set. Wait for chainlink VRF");
  ...
  function generateSVG(uint _randomNumber) public view returns (string memory finalSvg) {
    ...
    finalSvg = string(abi.encodePacked("<svg xmlns='http://www.w3.org/2000/svg' height='", uint2str(heightWidth), "' width='", uint2str(heightWidth), "'>"));
  ...
  function generatePath(uint256 _randomNumber) public view returns(string memory pathSvg) {
    ...
    for(uint i = 0; i < numberOfPathCommands; i++){
      uint256 newRNG = uint256(keccak256(abi.encode(_randomNumber, heightWidth +i)));
  ...
  function generatePathCommand(uint256 _randomNumber) public view returns(string memory pathCommand) {
    ...
    uint256 parameterOne = uint256(keccak256(abi.encode(_randomNumber, heightWidth *2))) % heightWidth;
    uint256 parameterTwo = uint256(keccak256(abi.encode(_randomNumber, heightWidth *3))) % heightWidth;
  ...
  function formatTokenURI(string memory _imageURI) public pure returns (string memory) {
    ...
    return string(abi.encodePacked(baseURL, Base64.encode(
      bytes(
        abi.encodePacked('{"name": "SVG NFT", ',
        '"description": "NFT with on-chain SVG", ',
        '"attributes":"", ',
        '"image": "', _imageURI, '"}'
        )
      ))
