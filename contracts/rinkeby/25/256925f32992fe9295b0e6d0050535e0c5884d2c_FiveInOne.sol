...
// File: node_modules\erc721a\contracts\extensions\IERC721AQueryable.sol
...
contract FiveInOne is ERC721AQueryable, Ownable {

  uint256 public MAX_Supply = 5000; // maximum supply is 10K.
  uint256 public publicMintFee = 0.002 ether; // public minting fees in ether.

  bool public revealed = false; // All Nfts revealed or not?

  string private baseUri; 
  string private unrevealedUri = "ipfs://QmXwyTMQGTTuLxTV5S2WQPGzHsmcJ5BMaWnzCRtf6hLW6S/hidden.json"; // uri for unrevealed nfts.
  ...
  modifier checkMintSupply(){
    require(totalSupply() + 5 <= MAX_Supply, "Amount Exceeds Supply");
    _;
  }

  constructor(string memory baseUri_,  address feeReceiver_) ERC721A("SHiBGOTCHi", "SHiBGOTCHi") {
    baseUri = baseUri_;
  ...
  function mint() public payable checkMintSupply() {
    if(msg.sender != owner()){
      require((publicMintFee) <= msg.value, "Insufficient Fee value");
      payable(feeReceiver).transfer(msg.value);
    }
    _safeMint(msg.sender, 5);
  }

  function tokenURIsOfOwner(address _address) public view onlyOwner returns(string[] memory array) {
