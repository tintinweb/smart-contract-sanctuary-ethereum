...
contract WineTrustToken is ERC1155PresetMinterPauserUpgradeable, OwnableUpgradeable, IWineTrustToken {
    /* GLOBAL VARIABLES */
    /**
    ...
    mapping(uint256 => bytes32) public productCodes;

    mapping(uint256 => bytes32) public productIds;
     ...
     */
    function mintNFT(address account, string calldata tokenMetadataHash, bytes32 _productId, bytes32 _productCode)
        external
        override
    {
    ...
    function productId(uint256 id) public view override returns (bytes32) {
        return productIds[id];
    }

    function productCode(uint256 id) public view override returns (bytes32) {
...
interface IWineTrustToken {
    ...
    function mintNFT(address account, string calldata tokenMetadataHash, bytes32 _productId, bytes32 _productCode)
        external;
    function productId(uint256 id) external view returns (bytes32);
    function productCode(uint256 id) external view returns (bytes32);
}
