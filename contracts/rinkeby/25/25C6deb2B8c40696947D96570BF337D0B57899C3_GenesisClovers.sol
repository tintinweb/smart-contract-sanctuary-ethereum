...
contract GenesisClovers is Initializable, ERC721AUpgradable, ERC721AUpgradableOwnersExplicit, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    ) external payable isMintValid(invocations, maxInvocation) isPresaleLive() {
        require(_mintOf(msg.sender) == 0, "GC/invalid-double-mint");
        require(
            MerkleProofUpgradeable.verify(proof, presaleMerkleRoot, keccak256(abi.encodePacked(msg.sender, maxInvocation))),
    ...
    function publicMint(uint256 invocations)
        external
        payable
        isMintValid(invocations, PER_WALLET_MAX)
        isPublicSaleLive()
    {
        _safeMint(msg.sender, invocations);
        emit Created(msg.sender, invocations);
    ...
    modifier isMintValid(uint256 invocations, uint256 maxInvocation) {
        ...
        require(totalSupply().add(invocations) <= SUPPLY_MAX, "GC/invalid-total-supply");
        require(invocations > 0, "GC/invalid-invocation-value");
        
        if (MINT_PRICE > 0) {
            require(msg.value == MINT_PRICE.mul(invocations), "GC/invalid-mint-value");
        ...
        require(
            ...
            "GC/invalid-invocation-upper-boundary"
        );
        _;
    }
    ...
    function setBaseURI(string calldata baseTokenURI, bool reveal) external onlyOwner {
    ...
    function setOffset(uint256 nonce) external onlyOwner returns (uint256) {
        ...
        require(!revealed, "GC/already-revealed");
    ...
    function reserveMint(uint256 invocations) external onlyOwner {
        require(totalSupply().add(invocations) <= RESERVE_MAX, "GC/invalid-reserve-supply");
        require(_reservedTeamTokens.add(invocations) <= RESERVE_MAX, "GC/not-enough-team-tokens");

        uint256 maxBatchSize = PER_WALLET_MAX;
        require(invocations.mod(maxBatchSize) == 0, "GC/invalid-batch-multiple");
        ...
        _reservedTeamTokens = _reservedTeamTokens.add(invocations);
        emit Created(msg.sender, invocations);
    }
