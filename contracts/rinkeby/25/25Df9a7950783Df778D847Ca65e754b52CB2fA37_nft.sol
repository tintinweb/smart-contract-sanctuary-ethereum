...
contract nft is ERC721A, Ownable {
  ...
  uint256 public maxAllowedTokensPerWallet = 1;
  uint256 public FREE_AllowedTokensPerWallet = 1;

  constructor(string memory baseURI) ERC721A("NFT", "DAN") {
    setBaseURI(baseURI);
  }
  ...
  function mint(uint256 _count) public payable saleIsOpen {
    ...
    if (msg.sender != owner()) {
      ...
      if(balanceOf(msg.sender) < FREE_AllowedTokensPerWallet && mintIndex < FREE_MAX_SUPPLY) {
          require(msg.value >= mintPrice * (_count > 1? (_count - FREE_AllowedTokensPerWallet) : 0), "insufficient funds");
      } else {
