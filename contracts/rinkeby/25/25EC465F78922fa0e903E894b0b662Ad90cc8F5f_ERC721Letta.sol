...
contract ERC721 is Context, ERC165, IERC721, IERC721Metadata {
     ...
     */
    function getApprovedOperatorsByOwner(address _owner)
        public
        view
    ...
    ERC721Pausable
{
    ...
    modifier onlyMinter() {
        ...
        require(canIMint(), "MinterRole: caller does not have the Minter role");
        _;
    }
}
...
contract ERC721Letta is ERC721Mochi, ReentrancyGuard {
    ...
    constructor(
        ...
        require(
            _commissionRate <= 100,
            "ERC721Letta: Commission rate has to be between 0 and 100"
        );
    ...
    function offerFinalize(
        ...
        // onlyOwner
        require(
            (ownerOf(tokenId) == msg.sender || admin == msg.sender),
            "ERC721Letta: Only owner or admin can finalize an offer"
        );
        ...
        // to owner
        require(
            payment_token.transferFrom(to, _wallets[tokenId], amount4owner),
            "Transfer failed."
        );
        ...
        if (amount4admin > 0) {
            require(
                payment_token.transferFrom(to, admin, amount4admin),
                "Transfer failed."
            );
        }
    ...
    function sell(
        ...
        require(
            ...
            "ERC721Letta: Only owner can sell this item"
        );

        // cannot set a price if auction is activated
        require(
            !auctions[tokenId].open,
            "ERC721Letta: Cannot sell an item which has an active auction"
        );
    ...
    function buy(uint256 tokenId) public nonReentrant {
        ...
        require(
            ...
            "ERC721Letta: The collectible is not for sale"
        );
        ...
        if (amount4admin > 0) {
            require(
                payment_token.transferFrom(msg.sender, admin, amount4admin),
                "Transfer failed."
            );
        }
    ...
    function createAuction(
        ...
        require(
            sellBidPrice[tokenId] == 0,
            "ERC721Letta: The selected NFT is open for sale, cannot be auctioned"
        );
        require(
            !auctions[tokenId].open,
            "ERC721Letta: The selected NFT already has an auction"
        );
        require(
            ownerOf(tokenId) == msg.sender,
            "ERC721Letta: Only owner can auction this item"
        );
    ...
    function bid(
        ...
        require(
            from != owner,
            "ERC721Letta: The owner cannot bid his own collectible"
        );

        // owner or sender
        require(
            (from == msg.sender || admin == msg.sender),
            "ERC721Letta: The bidder must be sender or admin"
        );
    ...
    function auctionFinalize(uint256 tokenId) public nonReentrant {
        ...
        if (auctions[tokenId].highestBid > 0) {
            ...
            if (amount4creator > 0) {
                require(
                    payment_token.transfer(creators[tokenId], amount4creator),
                    "Transfer failed."
                );
            }
            ...
            if (amount4admin > 0) {
                require(
                    payment_token.transfer(admin, amount4admin),
                    "Transfer failed."
                );
            }
    ...
    function callOptionalReturn(IERC721 token, bytes memory data) private {
        ...
        require(
            ...
            "SafeERC721: call to non-contract"
        );
        ...
        if (returndata.length > 0) {
            ...
            require(
                ...
                "SafeERC721: ERC20 operation did not succeed"
            );
        }
    }
    ...
    function withdrawBalanceOfToken(uint256 _amount) public nonReentrant {
        ...
        require(
            payment_token.transfer(contract_owner, _amount),
            "Transfer failed."
        );
    }
