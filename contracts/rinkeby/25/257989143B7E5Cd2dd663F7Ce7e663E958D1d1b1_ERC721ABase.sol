...
import {ERC721ADelegated} from "./ERC721ADelegated.sol";
...
contract TimeWarp is ERC721ADelegated {
...
{}
    //giftSouvenir is a batch mint operation that mints a batch of souvenirs
    function giftSouvenier(address[] calldata _addresses, uint256[] calldata _quantity) external onlyOwner {
        require(_addresses.length == _quantity.length);
        for (uint256 i = 0; i < _addresses.length; i++) {
            require(_addresses[i] != address(0));
            require(_quantity[i] > 0);
                _mint(_addresses[i], _quantity[i]);
        }
    }
...
import {IBaseERC721AInterface, ConfigSettings} from "./ERC721ABase.sol";

contract ERC721ADelegated {
    uint256[100000] gap;
    ...
    function base() private view returns (IBaseERC721AInterface) {
        return IBaseERC721AInterface(address(this));
    }
...
import {ERC721AUpgradeable} from "./github/chiru-labs/ERC721A-Upgradeable/contracts/ERC721AUpgradeable.sol";
import {IERC2981Upgradeable, IERC165Upgradeable} from "@openzeppelin/contracts-upgradeable/interfaces/IERC2981Upgradeable.sol";
import {OwnableUpgradeable} from "@openzeppelin/contracts-upgradeable/access/OwnableUpgradeable.sol";
import {IBaseERC721AInterface} from "./IBaseERC721AInterface.sol";
...
/**
    ...
    while also implementing all normal ERC721A functions as expected
*/
contract ERC721ABase is
    ERC721AUpgradeable,
    IBaseERC721AInterface,
    IERC2981Upgradeable,
    OwnableUpgradeable
{
    ...
    /// added initializerERC721A to initialize ERC721A as per https://chiru-labs.github.io/ERC721A/#/upgradeable?id=usage
    function initialize(
        address newOwner,
        string memory _name,
    ...
    {
        return
            ERC721AUpgradeable.isApprovedForAll(_owner, operator) ||
...
/// Hook that when enabled manually calls _beforeTokenTransfers on
/// Mind the difference between _beforeTokenTransfers and _beforeTokenTransfer(OZ Implementation) 
///                                                 ^
    ...
    /**
      ...
      @dev This mints [quantity] number of NFTs to a given address by an allowed minter on the edition instance.
      @dev Mind the difference between regular _mint via OZ implementation, which takes to, tokenId as arguments, while this one takes to,quantity and tokenId is auto-incremented.
     */
    function __mint(address to, uint256 quantity)
        external
        override
        onlyInternal
    {
    ...
    /// Internal only
    function __nextTokenId() public view onlyInternal {
        _nextTokenId();
    }
    ...
    function __owner()
        public
        view
        override(IBaseERC721AInterface)
        returns (address)
    {
        return owner();
    }
        ...
        returns (bool)
    {
        return
            ...
            type(IBaseERC721AInterface).interfaceId == interfaceId ||
...
interface IBaseERC721AInterface {
    /*
