...
/// version must match on hardhat.config.js
pragma solidity ^0.8.4;

import "@divergencetech/ethier/contracts/erc721/ERC721ACommon.sol";
import '@openzeppelin/contracts/access/Ownable.sol'; /// provides owner (the one who deploys) exclusive functionality
import '@openzeppelin/contracts/security/ReentrancyGuard.sol'; /// protects against reentrancy during certain functions (acts as modifier: nonReentrant) - doesn't allow function to call itself
import '@openzeppelin/contracts/finance/PaymentSplitter.sol'; /// enables payment splitting
import '@openzeppelin/contracts/utils/cryptography/MerkleProof.sol'; /// merkletree created from whitelist array - person needs to provide PROOF theyre on whitelist
import '@openzeppelin/contracts/utils/Counters.sol'; /// for counting # of NFTS
import '@openzeppelin/contracts/utils/Strings.sol'; /// for String methods
...
contract NoNameCharli is Ownable, ReentrancyGuard, ERC721ACommon, ERC2981, PaymentSplitter {
    ...
    /// Root - Merkleroot with whitelist addresses
    bytes32 public root;

    /// Depends on the network - helps users do gasless listings on OpenSea after minting process
    /// this is an address opensea provides you
    address proxyRegistryAddress;

    /// max supply of NFTs
    uint256 public maxSupply = 5000;

    /// URIs to NFTs on pinata
    string public baseURI;
    /// "?" when first minting on website - after minting, will show baseURI
    string public notRevealedUri = 'ipfs://asdfasdf/hidden.json';
    string public baseExtension = '.json';

    /// Allows admin the pause the contract, to reveal (notRevealedURI -> baseURI), start presale, start public mint
    bool public isPaused = false;
    bool public revealed = false;
    bool public presaleM = false;
    bool public publicM = false;

    /// limit presale limit per user
    uint256 presaleAmountLimit = 3;
    /// tracking presale claimed per user
    ...
    address[] private _team = [
        ...
        // 0xFAffC62907A435c4601E5cA6A390E7d72c07dBC4 // Test Account gets 35% of the total revenue
    ...
    */
    ...
    /**
        3. Modifiers
    */
    /// Modifier: prevents phising attacks
    modifier onlyAccounts() {
        require(msg.sender == tx.origin, 'Not allowed origin');
        _;
    }

    /// Modifier: used in pre-sale methods to verify proof
    ...
    */
    ...
    /// Toggle: public sale on/off
    function togglePublicSale() public onlyOwner {
        publicM = !publicM;
    }

    /// Presale Mint (account #, how many NFTs, proof)
    function presaleMint(address account, uint256 _amount, bytes32[] calldata _proof) 
        ...
        require(!isPaused, 'CryptoPunks: Contract is isPaused');
        ...
        require(_price * _amount <= msg.value, 'Sleptmfers: Not enough ethers sent');
