...
contract Drip2 is ERC721Enumerable, Ownable, ReentrancyGuard {
    ...
    constructor() ERC721("Drip2", "DRIP") {
        baseURI = "https://drip.com/";
        redeemTime[0] = 1664539200;
        uint256 _supply = supply;
        for (uint256 i; i < _supply; ++i) {
    ...
    function updateSupply(uint256 _redeemTime, uint256 _price) external onlyOwner {
        require(supply+3<= 9, "Total Supply Reached");
        drop += 1;
    ...
    function updateParams(uint256 _drop, uint256 _redeemTime, uint256 _price) external onlyOwner {
        redeemTime[_drop] = _redeemTime;
        redeemTime[_drop] = _price;
    }
    ...
    function redeem(uint256 tokenId) external {
        ...
        if(tokenId<=2){
            ...
            require(block.timestamp<=redeemTime[uint256((tokenId-3)/5) + 1], "Redeem time finished");
            redeemed[tokenId] = msg.sender;
        }
    }
