...
contract LoanStorageBid is Ownable {
    ...
    /// @dev bids[bidId] => Bid
    mapping(uint256 => LoanCommon.Bid) public bidMapping;

    /// @notice Bid tracking ID
    uint256 public bidIdTracker = 0;

    /// @dev loanBidMapping[loanId] => unit256[] list of bid ids for a loan
    mapping(uint256 => uint256[]) public loanBidMapping;

    /// @notice loan bid activities
    enum LoanStorageBidEvent {
        CREATED     // 0
    }

    /// @notice loan bid event
    event LoanStorageBidActivity(uint256 indexed bidId, uint256 indexed loanId, LoanStorageBidEvent activity, uint256 timestamp);

    modifier onlyServiceContractOrOwner() {
        address loanServiceBidAddress = LoanOracle_I(getLoanOracleAddress()).getLoanServiceBidAddress();
        require(
            msg.sender == loanServiceBidAddress || msg.sender == owner(),
    ...
    /// @notice get a bid object by bid ID
    /// @param _bidId The Bid ID
    function bids(uint256 _bidId) public view returns(LoanCommon.Bid memory) {
        return bidMapping[_bidId];
    }

    /// @notice get a list of offer IDs by loan ID
    /// @param _loanId The Loan ID
    function bidsByLoanId(uint256 _loanId) public view returns(uint256[] memory) {
        return loanBidMapping[_loanId];
    }

    /// @notice Create storage bid for a loan
    /// @param _bidder The wallet address of the bidder
    /// @param _loanId The ID of the loan
    /// @param _amount The amount of the bid
    function create(address _bidder, uint256 _loanId, uint256 _amount) public onlyServiceContractOrOwner {
        bidIdTracker++;
        bidMapping[bidIdTracker] = LoanCommon.Bid({
            bidId: bidIdTracker,
            loanId: _loanId,
            bidder: _bidder,
            amount: _amount,
            status: LoanCommon.BidStatus.ACTIVE,
            createdDate : block.timestamp
        });
        loanBidMapping[_loanId].push(bidIdTracker);

        // Emit payment event
        emit LoanStorageBidActivity(bidIdTracker, _loanId, LoanStorageBidEvent.CREATED, block.timestamp);
    }
}
