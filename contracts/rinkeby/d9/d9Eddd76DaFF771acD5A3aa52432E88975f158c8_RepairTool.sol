...
abstract contract RepairToolRandomVariable is Ownable {
    ...
    uint256 toolProperty = 21330100602301000051530;
...
abstract contract RepairToolContractsInterfaces is RepairToolERC721Base {
        ...
        onlyBanksysWayContract
    {
        if (_toolCharge[_toolId] <= 1) {
            ...
            _toolLastUse[_toolId] = block.timestamp;
        }
    }
        ...
        )
    {
        ...
        if (value == 1) {
            uint256 lastUse = _toolLastUse[_toolId];
            if (
                block.timestamp - lastUse >
                _getGlobalToolProperty(10000, 100) * _seconds
            ) {
                canBeUsed = true;
            } else {
                canBeUsed = false;
                timeLeft =
                    _getGlobalToolProperty(10000, 100) *
                    _seconds -
                    (block.timestamp - lastUse);
            }
        } else if (value == 2) {
            ...
            if (
                block.timestamp - lastUse >
                _getGlobalToolProperty(100, 100) * _seconds
            ) {
                canBeUsed = true;
            } else {
                canBeUsed = false;
            ...
            if (
                block.timestamp - lastUse >
                _getGlobalToolProperty(1, 100) * _seconds
            ) {
                canBeUsed = true;
            } else {
                canBeUsed = false;
...
abstract contract RepairToolRandom is RepairToolContractsInterfaces {
    ...
    function _setRandomValueToTool(uint256 _toolId, uint256 _randomNumber)
        internal
    {
        if (_randomNumber <= _getGlobalToolProperty(10000000000000000, 1000)) {
            ...
            _randomNumber <= _getGlobalToolProperty(10000000000000, 1000)
        ) {
            ...
            emit ToolValue(2);
        } else if (_randomNumber <= _getGlobalToolProperty(10000000000, 1000)) {
    ...
    function _getRandomValue(uint256 _toolId) internal {
        uint256 total = _getGlobalToolProperty(1000000, 10000);
        uint256 seed = rn.getRandomNumber(msg.sender);
