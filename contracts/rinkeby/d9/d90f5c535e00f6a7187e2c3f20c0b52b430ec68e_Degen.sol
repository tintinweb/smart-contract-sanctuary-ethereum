...
error SaleNotActiveYet();
error CallerIsContract();
error InsufficientFunds();
error ExceedsMaxSupply();
error ExceedsMaxMintsPerAddress();
error ExceedsMaxFreeSupply();
error ExceedsMaxFreeMintsPerAddress();
error NewSupplyLowerThanMintedTokens();

contract Degen is Ownable, ERC721A {
    ...
    constructor(string memory unrevealed) ERC721A("Degen", "DGN") {
        _baseTokenURI = unrevealed;
    }

    modifier mintCompliance() {
        if (!saleIsActive) {
            revert SaleNotActiveYet();
        }
    ...
    function mint(uint256 _quantity) external payable mintCompliance {
        ...
        if (_mintedAmount + _quantity > maxPerAddressDuringMint) {
            revert ExceedsMaxMintsPerAddress();
        }
    ...
    function freeMint(uint256 _quantity) external mintCompliance {
        if (maxFreeSupply < totalSupply() + _quantity) {
            revert ExceedsMaxFreeSupply();
        }
        uint256 _freeMintedAmount = freeMintedAmount[msg.sender];
        if (_freeMintedAmount + _quantity > maxPerAddressDuringFreeMint) {
            revert ExceedsMaxFreeMintsPerAddress();
        }
    ...
    function burnSupply(uint256 _newSupply) public onlyOwner {
        if (_newSupply < totalSupply()) {
            revert NewSupplyLowerThanMintedTokens();
        }
        maxSupply = _newSupply;
    }

    function revealCards(string calldata baseURI) external onlyOwner {
