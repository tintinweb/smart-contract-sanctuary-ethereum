...
// HaylandiaGenesisCollection.sol
...
contract HaylandiaGenesisCollection is Ownable, ReentrancyGuard, ERC721A {
    uint8 public MAX_NFT_QUANTITY = 100;
    uint8 public MAX_NFT_RESERVE_QUANTITY = 10;
    uint8 public MAX_NFT_PRESALE_QUANTITY = 20;
    uint8 public MAX_NFT_FOR_PER_TX = 2;
    uint8 public MAX_NFT_FOR_WHITELIST = 2;
    uint256 public NFT_PRICE = 5000000000000000; // 0.005;
    bool public IS_REVEALED = false;
    bool public IS_PAUSED = false;
    bool public IS_PRESALE_OPENED = true;
    ...
    constructor() ERC721A("Haylandia Genesis Collection", "HAYLANDIA") {}
    ...
    function togglePreSale() public onlyOwner {
        IS_PRESALE_OPENED = !IS_PRESALE_OPENED;
    }
    ...
    function addAddressToWhitelist(address _address) public onlyOwner {
        require(!isWhitelisted(_address), "Already whitelisted");
        ...
        returns (string memory)
    {
        ...
        if (IS_REVEALED && bytes(BASE_URI).length > 0) {
            return
                string(
                    abi.encodePacked(
                        BASE_URI,
        ...
        return "ipfs://QmNtMu2V56Kfb11vEgaeWHDYJ7sV2CsFpuKtni1BTbsQF2";
    }
    ...
    function reserveNFT(uint8 numberOfTokens) public onlyOwner {
        uint256 supply = totalSupply();

        require(
            supply + numberOfTokens <= MAX_NFT_RESERVE_QUANTITY,
            "Will exceed the total reserve limit"
        );
    ...
    function mintNFT(uint8 numberOfTokens) public payable callerIsUser {
        ...
        if (IS_PRESALE_OPENED) {
            require(
                supply + numberOfTokens < MAX_NFT_PRESALE_QUANTITY,
                "Will exceed the total presale nft limit"
            );
            require(isWhitelisted(msg.sender), "Address is not on whitelist");
            require(whitelist[msg.sender].hasMinted + numberOfTokens <= MAX_NFT_FOR_WHITELIST, "Above presale mint limit");
        ...
        require(
            numberOfTokens > 0 && numberOfTokens <= MAX_NFT_FOR_PER_TX,
