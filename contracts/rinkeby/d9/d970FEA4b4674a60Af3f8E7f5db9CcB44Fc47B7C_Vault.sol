...
contract Vault {
    ...
    uint256 public c_A;
    uint256 public c_i = 8; //deciminal 2 (1 - 10)
    uint256 public c_t = 10; // 10 days
    uint256 public c_y0 = 10; //deciminal 3 (0 - 100)

    event Withdraw(address withdrawer, uint256 amount);

    /**
    @notice set inital parameters for this contract
    @param _c_i Concave (0 ~ 10)
    @param _c_t Days (1 ~ )
    @param _c_y0 Starting Point (0 ~ 100%)
    */
    ...
    */
    function getUnlocked_TotalBalance() public view returns (uint256, uint256){
        uint256 period = (block.timestamp - startTime) / 1 minutes;
        uint256 temp_unlock = IERC20(tknContract).balanceOf(address(this));
        if (initialTotal != 0)
            temp_unlock = initialTotal;
        if (period > c_t) period = c_t;
        uint256 a1 = (100 - c_y0);
        uint256 a2 = 10000 * (10 ** period - c_i ** period) / (c_i ** period);
        uint256 a3 = 10000 * (c_i ** c_t) / (10 ** c_t - c_i ** c_t);
        
        temp_unlock = temp_unlock * (a1 * a2 * a3 + c_y0 * 10000 * 10000) / 100 / 10000 / 10000;
