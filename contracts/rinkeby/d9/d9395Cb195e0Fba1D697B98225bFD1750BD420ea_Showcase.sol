...
contract Showcase is Ownable, ERC721A, ReentrancyGuard {
    ...
    uint256 private maxMintPerTxn = 1;
    uint256 private maxMintPerWallet = 1;
    address payable public beneficiary;
    bool private ownerMintAvailable = true;
    ...
    function setWhitelistMintPrice(uint256 _newWhitelistMintPrice) public onlyOwner {
        ...
        nonReentrant
    {
        ...
        //require(_mintedAmount[msg.sender] + _amount <= maxMintPerWallet, "Can't mint that many!");
        require(_amount > 0, "Can't mint zero!"); 

        uint256 supply = totalSupply();
             
        require(supply + _amount <= TOTAL_SUPPLY, "Can't mint that many!");   
    ...
    function _mintWhitelist(uint256 _amount, bytes32[] calldata proof) private {
        require(mintStatus == MintStatus.WHITELIST, "Whitelist Sale inactive!");
        require(MerkleProof.verify(proof, merkleRoot, keccak256(abi.encodePacked(msg.sender))), "Not on whitelist!");
        require(msg.value == whitelistMintPrice * _amount, "The ether value sent is not correct!");
    ...
    function ownerMint(address to) public onlyOwner {
        require(ownerMintAvailable, "The owner mint was already done!");
        _internalMint(to, 10);
        ownerMintAvailable = false;
    }
