...
import "../../dependencies/utils/DecimalsConverter.sol";
    ...
    ReentrancyGuardUpgradeable
{
    ...
    function buyCover(
        ...
        // @dev get payable contract address
        IPolicyBookV2 policyBook = IPolicyBookV2(_bridgeProductAddress);
        IPolicyBookFacadeV2 policyBookFacade = policyBook.policyBookFacade();
        IERC20Upgradeable stblToken = IERC20Upgradeable(
            contractsRegistry.getUSDTContract()
        );

        // @dev check previous allowance
        require(
            stblToken.allowance(_buyerAddress, address(this)) >= _premium,
            "BridgeDistributor: Need funds approval"
        );

        // @dev transfer erc20 funds to this contract
        stblToken.safeTransferFrom(
            _buyerAddress,
            address(this),
            DecimalsConverter.convertFrom18(_premium, policyBook.stblDecimals())
        );

        if (
            stblToken.allowance(address(this), address(policyBook)) ==
            uint256(0)
        ) {
            // @dev safe as this contract has no funds stored & will be called once only
            stblToken.approve(address(policyBook), _premium);
        }

        // @dev buy policyBookFachade bridgeV2
        ...
        // @dev product id is product address
        emit BuyCoverEvent(
            _bridgeProductAddress,
            1,
            _epochsNumber,
...
library DecimalsConverter {
    ...
    function convert(
        ...
        if (baseDecimals > destinationDecimals) {
            conversion = amount.div(10**(baseDecimals - destinationDecimals));
        } else if (baseDecimals < destinationDecimals) {
            conversion = amount.mul(10**(destinationDecimals - baseDecimals));
        }
