...
contract BloodShedBearsStaking is Ownable, Pausable {
    ...
    function moveIntoDifferentStake(StakeSelection[] calldata selections_) external whenNotPaused {
        uint256 totalAccumulatedYield;
        for (uint256 i = 0; i < selections_.length; ++i) {
            totalAccumulatedYield += _exitStakeStanceMany(selections_[i].collectionAddress, selections_[i].tokens, false, false);
    ...
    function _getYield(
        ...
        if (
            ...
            || stakedTokensData[collection_][tokenId_].stakeType == stakeTypesVariables["home"]
        ) {
            if (
                !IGameStats(contractsAddressesMap["gameStats"])
    ...
    function calculateYieldSafe(address collection_, uint256[] calldata tokenIds_) external view returns(uint256) {
        uint256 totalYield;
        for (uint256 i = 0; i < tokenIds_.length; ++i) {
