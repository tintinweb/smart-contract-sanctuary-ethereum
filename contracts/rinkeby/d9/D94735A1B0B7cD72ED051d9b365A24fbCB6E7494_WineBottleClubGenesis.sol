...
contract WineBottleClubGenesis is ERC721, EIP712, Ownable {
    ...
    uint16 private constant EXCLUSIBLE_LIST_ID = 1;
    ...
    function publicMint(address to, uint16 count) external payable {
        ...
        _mintMany(to, _tokenPublicPrice, count);
    }
    ...
    function setCurrentListId(uint256 currentListId) public onlyOwner {
    ...
    function accessMint(
        ...
        require(
            MerkleProof.verify(
                proof,
                _root,
                keccak256(abi.encode(msg.sender, tokenPrice, listId))
            ),
            "!proof"
        );
        unchecked {
        ...
        require(listId <= _currentListId, "!list");
        if (EXCLUSIBLE_LIST_ID == listId) {
    ...
    bytes32 private constant WBC_ASSET_STORED_TYPEHASH =
        keccak256("WbcIsStored(bool isStored,uint64 timestamp)");
    ...
    function verifyIsStored(
        ...
        bytes32 digest = _hashTypedDataV4(
            keccak256(
                abi.encode(WBC_ASSET_STORED_TYPEHASH, isStored, timestamp)
            )
        );
        return ECDSA.recover(digest, signature) == signer;
    }
    ...
    function _beforeTokenTransfer(
        ...
        require(
            (0 == _redeemedTimestamp[tokenId]) ||
                // solhint-disable-next-line not-rely-on-time
                (block.timestamp - _redeemedTimestamp[tokenId] >= 30 days),
            "!locked"
        );
