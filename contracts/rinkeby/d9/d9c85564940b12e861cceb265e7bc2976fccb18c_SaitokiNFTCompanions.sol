...
contract SaitokiNFTCompanions is ERC721, ERC721Enumerable, Ownable, ReentrancyGuard {
    ...
    uint256 public HOLD_AMOUNT_MONONOKE = 1e9 * (10**9);
    uint256 public HOLD_AMOUNT_FORTUNE = 1e9 * (10**9);
    uint256 public HOLD_AMOUNT_SAITOKI = 1e9 * (10**9);
    uint256 public COMMON_CHANCE = 45;
    uint256 public UNCOMMON_CHANCE = 30;
    uint256 public RARE_CHANCE = 15;
    uint256 public EPIC_CHANCE = 7;
    ...
    function setHoldAmounts(uint256 mono, uint256 fort, uint256 saito) public onlyOwner {
        HOLD_AMOUNT_MONONOKE = mono;
        HOLD_AMOUNT_FORTUNE = fort;
        HOLD_AMOUNT_SAITOKI = saito;
    }

    function setRarities(uint256 common, uint256 uncommon, uint256 rare, uint256 epic) public onlyOwner {
        COMMON_CHANCE = common;
        UNCOMMON_CHANCE = uncommon;
        RARE_CHANCE = rare;
        EPIC_CHANCE = epic;
    }
    ...
    function mint(uint numberOfTokens) public payable nonReentrant {
        ...
        for (uint256 i = 0; i < numberOfTokens; i++) {
            ...
            _genes[tokenId] = getGen();
            emit TokenMinted(tokenId, _genes[tokenId]);
        }
    }

    function hasDiscount(address account) public view returns (bool) {
        return ((IERC20(MononokeInu).balanceOf(account) >= HOLD_AMOUNT_MONONOKE) || (IERC20(Fortune).balanceOf(account) >= HOLD_AMOUNT_FORTUNE) || (IERC20(SaitokiInu).balanceOf(account) >= HOLD_AMOUNT_SAITOKI));
    }
    ...
    function getGen() private view returns (uint256) {
        uint p = (random() % 100) + 1;
        if (p <= COMMON_CHANCE) {
            return 0;
        } else if (p <= (COMMON_CHANCE + UNCOMMON_CHANCE)) {
            return 1;
        } else if (p <= (COMMON_CHANCE + UNCOMMON_CHANCE + RARE_CHANCE)) {
            return 2;
        } else if (p <= (COMMON_CHANCE + UNCOMMON_CHANCE + RARE_CHANCE + EPIC_CHANCE)) {
            return 3;
        } else {
            return 4;
        }
    ...
    function random() private view returns (uint8) {
        return uint8(uint256(keccak256(abi.encodePacked(block.timestamp, block.difficulty, blockhash(block.number%257))))%251);
    }
