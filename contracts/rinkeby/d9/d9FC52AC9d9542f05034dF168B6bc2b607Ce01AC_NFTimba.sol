...
pragma solidity ^0.8.0;
  ...
  // This pseudorandom func is deterministic and should not be used on prod
...
// File contracts/NFTimba.sol


pragma solidity ^0.8.4;
contract NFTimba is ERC721, ERC721Enumerable, TimbaDNA {
  ...
  constructor(uint256 _maxSupply) ERC721("NFTimba", "TIMBA") {
    maxSupply = _maxSupply;
  }

  function mint() public{
    uint256 current = _idCounter.current();
    require(current <= maxSupply, "No NFTimbas left :/");
    ...
    returns(string memory)
    {
      ...
      string memory jsonURI = Base64.encode(
        abi.encodePacked(
          '{ "name": "NFTimba #',
                tokenId.toString(),
                '", "description": "Timba are randomized Avataaars stored on chain to teach DApp development on home", "image": "',
                image,
                '"}'
          )
        );
