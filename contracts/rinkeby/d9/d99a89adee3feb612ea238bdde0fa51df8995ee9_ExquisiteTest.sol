...
contract ExquisiteTest is ERC1155, Ownable {
    uint256 public constant MINT_PRICE = 0.064 ether;
    uint256 public constant MAX_MINT = 853; // pi * e
    ...
    mapping(uint256 => Art) public tokens;
    uint256 public artCount;

    IExquisiteGraphics public gfx =
        IExquisiteGraphics(address(0xF9041442F7Bf48f6B1134d332633Aa853C6a0C69));

    constructor() ERC1155("") {
        _transferOwnership(0xD286064cc27514B914BAB0F2FaD2E1a89A91F314);
    }

    // maybe a few genesis artworks that are hard coded?
    // doing ON-CHAIN in RGB
    // on chain rendered incorrectly, like my first renderer attempt

    function addArtwork(bytes memory data) public onlyOwner {
        // write data to chain
        address artData = SSTORE2.write(data);

        // store it
        Art storage art = tokens[artCount];
        art.num = 0;
        art.data = artData;

        // mint to owner
        _mint(owner(), artCount, 1, bytes(""));

        // increment art count
        artCount++;
    }

    function mintGenesis(uint256 num) public payable {
        if (msg.value * num != MINT_PRICE * num) revert InvalidPaymentAmount();
        _mint(msg.sender, 0, num, bytes(""));
    }
