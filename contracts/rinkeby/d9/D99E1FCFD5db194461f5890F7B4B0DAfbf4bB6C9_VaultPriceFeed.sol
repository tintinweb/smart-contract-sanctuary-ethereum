...
contract VaultPriceFeed is IVaultPriceFeed{
    ...
    uint256 private constant ONE_USD = PRICE_PRECISION;
    uint256 private constant BASIS_POINTS_DIVISOR = 10000;
    uint256 private constant MAX_SPREAD_BASIS_POINTS = 50;
    uint256 private constant MAX_ADJUSTMENT_INTERVAL = 2 hours;
    uint256 private constant MAX_ADJUSTMENT_BASIS_POINTS = 20;

    // Identifier of the Sequencer offline flag on the Flags contract
    address constant private FLAG_ARBITRUM_SEQ_OFFLINE = address(bytes20(bytes32(uint256(keccak256("chainlink.flags.arbitrum-seq-offline")) - 1)));

    address private gov;
    address private chainlinkFlags;

    bool private isAmmEnabled = true;
    bool private isSecondaryPriceEnabled = true;
    bool private useV2Pricing = false;
    bool private favorPrimaryPrice = false;
    uint256 private priceSampleSpace = 3;
    uint256 private maxStrictPriceDeviation = 0;
    address private secondaryPriceFeed;
    uint256 private spreadThresholdBasisPoints = 30;
    ...
    mapping (address => uint256) private priceDecimals;
    mapping (address => uint256) private spreadBasisPoints;
    // Chainlink can return prices for stablecoins
    // that differs from 1 USD by a larger percentage than stableSwapFeeBasisPoints
    // we use strictStableTokens to cap the price to 1 USD
    // this allows us to configure stablecoins like DAI as being a stableToken
    // while not being a strictStableToken
    mapping (address => bool) private strictStableTokens;

    mapping (address => uint256) public override adjustmentBasisPoints;
    mapping (address => bool) public override isAdjustmentAdditive;
    mapping (address => uint256) private lastAdjustmentTimings;
