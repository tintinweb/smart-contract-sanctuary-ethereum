...
contract animalMain is  ERC721,Ownable {
    ...
    function createAnimal() public payable{
        require(msg.value >= 0.001 ether, "Create an animal needs 0.001  ether.");    
        bytes32 _dna;
        uint8 _star;
        ...
        if(rand<40) roletype = "Ant";
        else if (rand<50) roletype = "Cat";
        else if (rand<80) roletype = "Dog";
        else if (rand<90) roletype = "Dragon";
        else roletype = "Ant";
           
        uint id = animals.push(animal(_dna, uint8(_star), roletype)) - 1;  //將此animal的數量
        animalToOwner[id] = msg.sender; // 每隻動物都有一個獨特編號，呼叫mapping，得到相對應的主人 轉移擁有權 
        ownerAnimalCount[msg.sender]++; //回傳某帳號底下的動物數量  擁有者的個數＋1
        emit Create(id, _dna, _star, roletype);
    }
