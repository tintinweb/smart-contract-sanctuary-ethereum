...
contract ETHWSwap is Ownable{
    ...
    // UniswapV3 fee level: 0.05%, 0.3%, 1%
    uint24[] internal tradingFeesV3 = [500, 3000, 10000];

    ISwapRouter internal v3Router = ISwapRouter(0xE592427A0AEce92De3Edee1F18E0157C05861564);

    // White list tokens(USDC, DAI, USDT, WBTC), ordered by TVL
    ...
    function swapExactTokenIn(
        ...
        console.log("v3 fee level: ", v3FeeLevel);
        if (v3FeeLevel != 0) {
            console.log("------enter path one-----------");
            // Set the order parameters
            console.log("transfer token to address this");
            IERC20(tokenIn).safeTransferFrom(msg.sender, address(this), amountIn);
            console.log("erc20 balance: ", IERC20(tokenIn).balanceOf(address(this)));
            ...
            console.log("out put amount: ", outPutAmount);
        ...
        if (outPutAmount > 0) {
            ...
            if (tokenOut == WETH) {
                ...
                require(
                    IERC20(tokenOut).balanceOf(address(this)) >= outPutAmount,
                    "Insufficient ERC20 balance"
                );
                IERC20(tokenOut).safeTransfer(msg.sender, adjustAmount);
    ...
    function getDeepestPoolOfTokenPair(address tokenA, address tokenB) public view returns(uint24 deepestPoolFee) {
        ...
        for (uint256 i = 0; i < tradingFeesV3.length; ++i) {
            console.log("----in trading fee------:", tradingFeesV3[i]);
            ...
            if (address(pool) != address(0)) {
                ...
                console.log("deepest pool fee:", deepestPoolFee);
            }
        }
    }
    /**
    ...
    function _checkandAdjustERC20Allowance(address token, uint256 amount) internal {
        if (IERC20(token).allowance(address(this), address(v3Router)) < amount) {
