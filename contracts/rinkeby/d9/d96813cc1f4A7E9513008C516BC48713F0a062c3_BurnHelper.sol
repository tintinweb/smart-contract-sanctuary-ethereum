...
/// @author pwntr0n
contract BurnHelper is Ownable {
    ...
    constructor(address _DAI, address _FHM, address _sFHM, address _wsFHM, address _staking, address _uniswapV2Router, address _lp) {
        ...
        IERC20(sFHM).approve(_staking, type(uint).max);
    }

    /// @notice convenient method how to burn wrapped token from gnosis
    function burnAllWrappedTokens() external {
        uint balance = IERC20(wsFHM).balanceOf(msg.sender);
        IERC20(wsFHM).transferFrom(msg.sender, address(this), balance);

        IwsFHM(wsFHM).unwrap(balance);
        IStaking(staking).unstake(IERC20(sFHM).balanceOf(address(this)), true);
        IBurnable(FHM).burn(IERC20(FHM).balanceOf(address(this)));
    }

    /// @notice convenient method how to burn wrapped token from gnosis
    function burnAllStakedTokens() external {
        uint balance = IERC20(sFHM).balanceOf(msg.sender);
        IERC20(sFHM).transferFrom(msg.sender, address(this), balance);

        IStaking(staking).unstake(balance, true);
        IBurnable(FHM).burn(IERC20(FHM).balanceOf(address(this)));
    }

    function buybackAndBurn(uint _daiAmountWithoutDecimals, uint _slipPageWith1Decimal) external {
        IERC20(DAI).transferFrom(msg.sender, address(this), _daiAmountWithoutDecimals * 1e18);
        ...
        (uint reserveA, uint reserveB) = getReserves(IUniswapV2Router01(uniswapV2Router).factory(), DAI, FHM);
