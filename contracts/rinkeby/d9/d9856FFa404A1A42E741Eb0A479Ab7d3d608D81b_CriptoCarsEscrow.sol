...
contract CriptoCarsEscrow is Initializable, ReentrancyGuardUpgradeable  {
   ...
   function getState(uint _orderId) public view returns (EscrowStatus){
    ...
    function createEscrow(uint _orderId, address payable _buyer, address payable _seller, uint _value, uint _sellerfee, uint _buyerfee, IERC20 _currency) external nonReentrant {
        ...
        require( _currency.allowance( _buyer, address(this)) >= (_value),"Seller approve to Escrow first");              

        _currency.safeTransferFrom(_buyer, address(this) , (_value) );
        
        escrows[_orderId] = Escrow(_buyer, _seller, _value, _sellerfee, _buyerfee,  _currency, EscrowStatus.Funded);
    ...
    function releaseEscrow(uint _orderId) external onlyBuyer(_orderId) nonReentrant{            
        ...
        escrows[_orderId].currency.safeTransfer( escrows[_orderId].seller, escrows[_orderId].value -(_tmpBuyerFee+_tmpSellerFee));        
    ...
    function refundBuyer(uint _orderId) external onlyOwner nonReentrant {       
    ...
    //TODO: sacar los fees en la moneda nativa
    ...
    */
    ...
    function cancelEscrow(uint _orderId) external nonReentrant onlySeller(_orderId){
