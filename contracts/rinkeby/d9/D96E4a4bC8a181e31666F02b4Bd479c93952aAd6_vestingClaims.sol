    ...
    */
        ...
        if(_startTime < getCurrentTime()){
            ...
            if(!nftData[_nftId][_matchId].initialized){
                ...
                nftData[_nftId][_matchId] = claimSchedule({
                    ...
                    initialized:true,
                    totalEligible:_tokenAmount*getDecimal(),
                    totalClaimed:0,
                ...
                if(_amount>0){
                    ...
                    nftData[_nftId][_matchId].totalClaimed=nftData[_nftId][_matchId].totalClaimed.add(_amount);
    ...
    function getClaimableAmount(uint _nftId,uint8 _matchId) public view returns(uint _claimAmount) {
        ...
        if(!nftData[_nftId][_matchId].initialized){
            ...
            _amount = _tokenAmount*getDecimal();
            claimAmount =0;
    ...
    function getVestingInfo(uint _nftId,uint8 _matchId) public view returns(uint _totalEligible,uint _claimAmount,uint _totalClaimed,uint _remainingBalTokens,uint _lastClaimedAt){
        (,uint _tokenAmount)= getMatchData(_matchId);

        if(!nftData[_nftId][_matchId].initialized){
            return (_tokenAmount*getDecimal(),getClaimableAmount(_nftId,_matchId).div(getDecimal()),0,_tokenAmount,0);
        }else{
