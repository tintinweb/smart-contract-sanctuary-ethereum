...
contract NotGonnaLie is ERC721A, Ownable, ReentrancyGuard {
  ...
  constructor() ERC721A('NotGonnaLie', 'NGL') {
  }

  function ownerMint(address _address, uint _mintAmount) external onlyOwner {
    require(maxSupply >= totalSupply() + _mintAmount, "NotGonnaLie: Sold out");
    _safeMint(_address, _mintAmount);
  }

  function mint(uint256 _mintAmount) external payable {
    require(mintStart, "NotGonnaLie: Mint is not live yet");
    require(maxSupply >= totalSupply() + _mintAmount, "NotGonnaLie: Sold out");
    require(_mintAmount > 0, "NotGonnaLie: Invalid mint amount");
    require(_mintAmount  <= maxMint, "NotGonnaLie: Exceed max mint");
  ...
  function _safemint(uint256 _mintAmount) internal {
    if(addressMinted[msg.sender] < freeMint) {
      ...
      require(msg.value >= (_mintAmount - freeMint) * mintPrice,"NotGonnaLie: Claim Free NFT");
      addressMinted[msg.sender] += _mintAmount;
      _safeMint(msg.sender, _mintAmount);
    }
    else {
      require(msg.value >= _mintAmount * mintPrice,"NotGonnaLie: Invalid mint price");
      addressMinted[msg.sender] += _mintAmount;
      _safeMint(msg.sender, _mintAmount);
    }
  }
  ...
  function numberMinted() public view returns (uint) {
    return totalSupply();
  }
