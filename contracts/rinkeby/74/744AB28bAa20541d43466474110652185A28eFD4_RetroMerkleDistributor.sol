 ...
 * we transfer it to the vestedStaking contract
 */
contract RetroMerkleDistributor is Ownable {
    ...
    event Claimed(uint256 _index, address indexed _account, uint256 _amount);
    ...
    /**
     * @dev setter for the VestedStaking contract, needed, because they cross-reference each other
     * has to be set up in deployment sequence after VestedStaking is deployed
     * (todo could look into the hack where we get the 2 contract address pre deplyoment
     * and then they could be provided in the constructor)
     * @param _vestedStaking (new) address of VestedStaking contract
     */
    ...
    /**
     * @dev modification from original: instead of transferring the tokens to the user,
     * we transfer it to the vestedStaking contract
     */
    function claim(
        uint256 _index,
