...
contract btbt is Ownable, ERC721A, ReentrancyGuard {
    ...
    function allowlistMint() external payable callerIsUser {
        ...
        require(totalSupply() + 1 <= maxCollectionSize, "reached max supply");
    ...
    function publicSaleMint(uint256 quantity) external payable callerIsUser {
        uint256 publicPrice = 0.0001 ether;
        require(publicSaleEnabled, "not open");

        require(
            totalSupply() + quantity <= maxCollectionSize,
            "reached max supply"
        );
        require(
            numberMinted(msg.sender) + quantity <= maxPerAddressDuringMint,
    ...
    function getMaxCollectionSize() public view returns (uint256) {
