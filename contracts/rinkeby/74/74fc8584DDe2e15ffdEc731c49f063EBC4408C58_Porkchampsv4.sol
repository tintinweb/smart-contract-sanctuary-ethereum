...
/**
 * @title Porkchampsv3 contract
 * @dev Extends ERC721A Non-Fungible Token Standard basic implementation
 */
contract Porkchampsv4 is ERC721A, Ownable, PaymentSplitter {
    ...
    // The size of this can be further reorganized and optimized potentially
    struct Prices {
    ...
    Prices public prices = Prices(0.03 ether, 0.04 ether);
    SaleConfig public saleConfig = SaleConfig(false, false);

    constructor(address[] memory _payees, uint256[] memory _shares)
        payable
        ERC721A("Porkchampsv4", "PKC")
    ...
    function avatarMint(address[] calldata addresses) external payable onlyOwner {
        require(totalSupply() < 18, "Too many minted before teamMint");
        require(addresses.length == 17, "Only 17 mints allowed for the team");
        for (uint256 i = 0; i < addresses.length; i++) {
            _safeMint(addresses[i], 1);
        }
    }

    // The team reserves the next 160 mints after the team avatars (#17-176) for team members
    function teamMint(address[] calldata addresses) external payable onlyOwner {
        require(
            totalSupply() < 177,
            "Too many already minted before allowlistMint"
        );
        require(addresses.length < 9, "Only 160 mints allowed for the team");
    ...
    // The last batch of nfts (max of 1111) are reserved for team marketing etc.
    function teamReserveMint(uint256 quantity) external payable onlyOwner {
        require(
            totalSupply() + quantity < collectionSize,
            "Cannot exceed collectionSize"
        );
        uint256 numChunks = quantity / 5;
        // Double check if mod works in solidity
