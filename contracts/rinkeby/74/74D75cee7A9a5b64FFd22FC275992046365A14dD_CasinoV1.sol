...
contract CasinoV1 is VRFConsumerBaseV2, KeeperCompatibleInterface, Ownable {
    /* Type declarations */
    enum CasinoStates {
        OPEN,
        PROCESSING
    ...
    constructor(
        ...
        i_subId = i_vrfCoordinator.createSubscription();
        i_vrfCoordinator.addConsumer(i_subId, address(this));
    ...
    function fulfillRandomWords(
        ...
        (bool successWinnerTX, ) = winner.call{value: address(this).balance - ownerCommission}("");
        (bool successOwnerTX, ) = owner().call{value: ownerCommission}("");
        if (!successWinnerTX || !successOwnerTX) {
    ...
    function cancelSubscription() external onlyOwner {
        i_vrfCoordinator.cancelSubscription(i_subId, msg.sender);
    }

    // FundVRFSubscription function
    function fundVRFSubscription(uint256 amount) public onlyOwner returns (bool success) {
        success = i_linkToken.transferAndCall(
