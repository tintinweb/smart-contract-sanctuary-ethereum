...
contract Vendor is Ownable {
  ...
  function sellTokens(uint256 tokensToSell) public {
    require(tokensToSell > 0, "Amounts of tokens to sell must be greater than 0");

    uint256 userBalance = yourToken.balanceOf(msg.sender);
    require(userBalance >= tokensToSell, "User has insufficient tokens to sell");

    uint256 ethAmtToTransfer = tokensToSell/tokensPerEth;
    uint256 ownerBalance =address(this).balance;

    require(ownerBalance >= ethAmtToTransfer, "Owner has insuffient balance to transfer");
