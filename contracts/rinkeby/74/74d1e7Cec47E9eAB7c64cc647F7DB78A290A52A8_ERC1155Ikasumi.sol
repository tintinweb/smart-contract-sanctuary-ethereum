...
contract ERC1155Marketplace is ERC1155YuzuInternal, ReentrancyGuard {
    ...
    function distributeFunds(uint256 totalPrice, address from, address to, uint256 tokenId, bool useTransferFrom) private {
    ...
    function updateAdmin(address _admin, uint256 _commissionRate, uint256 _royaltiesCommissionRate, bool _anyoneCanMint, IERC20 _paymentToken) external {
        ...
        if (paymentToken != _paymentToken) {
            // @dev we do not want to let the user update the token unless the balance of the contract is empty
            // strict-equialities here is not affecting the logic of this line
            // https://github.com/crytic/slither/wiki/Detector-Documentation#dangerous-strict-equalities
            require(paymentToken.balanceOf(address(this))==0, "Ikasumi: the contract has funds in the old token");
        }
        admin = _admin;
        commissionRate = _commissionRate;
        royaltiesCommissionRate = _royaltiesCommissionRate;
...
contract ERC1155Ikasumi is ERC1155Marketplace, EIP712 {
    ...
    function redeem(
        ...
        require(paymentToken.transferFrom(_msgSender(), contractOwner, qtyToMint * price), "Transfer failed.");
