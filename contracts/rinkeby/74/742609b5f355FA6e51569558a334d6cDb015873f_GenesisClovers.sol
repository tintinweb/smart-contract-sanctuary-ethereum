...
contract GenesisClovers is ERC721A, Ownable, VRFConsumerBase {
    ...
    constructor() ERC721A("GenesisClovers", "GENESISCLOVERS") VRFConsumerBase(_vrfAddress, _linkToken) {
        _safeMint(address(this), 1);
        _burn(0);
    }

    function reserveTeamTokens(address to, uint256 quantity) external onlyOwner {
        require(_reservedTeamTokens + quantity <= RESERVE_MAX, "GenesisClovers: Team Tokens Already Minted");
        _reservedTeamTokens += quantity;
        _safeMint(to, quantity);
    }

    function mint(uint256 quantity, bytes calldata signature, bytes32 nonce) external payable {
        require((state == Status.SALE_LIVE || state == Status.PRESALE_LIVE), "GenesisClovers: Sale Not Live");
        require(msg.sender == tx.origin, "GenesisClovers: Contract Interaction Not Allowed");
        require(ECDSA.recover(keccak256(abi.encodePacked(msg.sender, quantity, state, nonce)), signature) == _signer, "GenesisClovers: Invalid Signer");

        __mint(msg.sender, quantity, signature, nonce);
    }

    function __mint(address to, uint256 quantity, bytes calldata signature, bytes32 nonce) internal {
        require(totalSupply() + quantity <= SUPPLY_MAX, "GenesisClovers: Exceed Max Supply");
        require(quantity <= 2, "GenesisClovers: Exceeds Max Per TX");
        require(msg.value >= PRICE * quantity, "GenesisClovers: Insufficient ETH");

        if(state == Status.PRESALE_LIVE) {
            require(_numberMinted(msg.sender) + quantity <= 2, "GenesisClovers: Exceeds Max Per Wallet");
        } else {
            require(!_nonceUsed[nonce], "GenesisClovers: Nonce Used");
    ...
    function setOffset() external onlyOwner returns (bytes32 requestId) {
        require(offset == 0, "GenesisClovers: Offset Already Declared");
