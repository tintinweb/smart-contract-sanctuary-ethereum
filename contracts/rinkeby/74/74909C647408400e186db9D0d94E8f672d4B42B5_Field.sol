...
contract Field is Ownable, IERC721Receiver, Pauseable {
    ...
    mapping(address => uint16) playerCount;
    mapping(address => uint16) goalCount;
    mapping(address => uint16[]) ownerPlayer;
    mapping(address => uint16[]) ownerGoal; 
    mapping(uint => address) tokenByUser;

    mapping(uint => uint) unstackDate; 
    ...
    function addManyToFieldAndPack(address account, uint16[] calldata tokenIds) external nonReentrant {
        ...
        for (uint i = 0; i < tokenIds.length; i++) {
            ...
            if (isPlayer(tokenIds[i])) {
                ...
                //ownerPlayer[msg.sender][playerCount[msg.sender]] = tokenIds[i];
                ownerPlayer[msg.sender].push(tokenIds[i]);

                playerCount[msg.sender] += 1;
                unstackDate[tokenIds[i]]=block.timestamp + MINIMUM_TO_EXIT;
            }
            else
             {
                _addGoalkeeperToPack(account, tokenIds[i]);
               
               ownerGoal[msg.sender].push(tokenIds[i]);
                goalCount[msg.sender] += 1;
             }
        }
    }
   ...
   */
    function _claimPlayerFromField(uint256 tokenId, bool unstake) internal returns (uint256 owed) {
        ...
        if (unstake) {
            ...
            totalPlayerStaked -= 1;
            uint16 playerIdx = getIndexPlayer(msg.sender,tokenId);
            deletePlayerEntry(msg.sender, playerIdx);
   ...
   */
    function _claimGoalkeeperFromPack(uint256 tokenId, bool unstake) internal returns (uint256 owed) {
        ...
        if (unstake) {
            ...
            // Delete old mapping
            uint16 goalIdx = getIndexGoal(msg.sender,tokenId);
            deleteGoalEntry(msg.sender, goalIdx);
            goalCount[msg.sender] -= 1;
        } else {
    ...
    function rescue(uint256[] calldata tokenIds) external nonReentrant {
        ...
        for (uint i = 0; i < tokenIds.length; i++) {
            ...
            if (isPlayer(tokenId)) {
                ...
                delete field[tokenId];
                totalPlayerStaked -= 1;
                uint16 playerIdx = getIndexPlayer(msg.sender,tokenIds[i]);
                deletePlayerEntry(msg.sender, playerIdx);
                ...
                uint goalIdx = getIndexGoal(msg.sender,tokenIds[i]);
                deleteGoalEntry(msg.sender, goalIdx);
                goalCount[msg.sender] -= 1;
   ...
   */
    ...
    function getPlayer(address _address) public view returns  (uint16[] memory){
    // uint16[][] memory tab = ownerPlayer[msg.sender];
    uint16[] memory tab = ownerPlayer[_address];
    return tab;
    }
    
    function getGoalKeeper(address _address) public view returns  (uint16[] memory){
    uint16[] memory tab = ownerGoal[_address];
    return tab;
    }


     function getIndexPlayer(address _address, uint _tokenId) public view returns (uint16) {
        for(uint16 i = 0; i<ownerPlayer[_address].length; i++){
            if(_tokenId == ownerPlayer[_address][i]){
            return i;
            }
        }
    }
       function getIndexGoal(address _address, uint _tokenId) public view returns (uint16) {
        for(uint16 i = 0; i<ownerGoal[_address].length; i++){
            if(_tokenId == ownerGoal[_address][i]){
            return i;
            }
    ...
    function deletePlayerEntry(address _address, uint16 _tokenIdx) internal {
            ownerPlayer[_address][_tokenIdx] = ownerPlayer[_address][ownerPlayer[_address].length-1];
            ownerPlayer[_address].pop();


    }
    function deleteGoalEntry(address _address, uint _tokenIdx) internal {
            ownerGoal[_address][_tokenIdx] = ownerGoal[_address][ownerGoal[_address].length-1];
            ownerGoal[_address].pop();


    }

    function whenUnstack(uint _tokenId) external view returns (uint ) {
        return unstackDate[_tokenId];
    }

    function playerRewards(uint tokenId ) external view returns (uint) {
        Stake memory stake = field[tokenId];
        uint owed = (block.timestamp - stake.value) * DAILY_GOAL_RATE / 1 days;
        return owed;
    }

    function goalRewards(uint tokenId) external view returns (uint) {
        uint256 alpha = _alphaForGoalkeeper(tokenId);
        Stake memory stake = pack[alpha][packIndices[tokenId]];
        uint owed = (alpha) * (goalPerAlpha - stake.value);
        return owed;
    }
}
