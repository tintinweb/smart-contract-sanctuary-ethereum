...
contract LIFECRYPTO {
    ...
    function transferUserName(string memory userName, address toAddress) public returns (bool) {
        require(!checkUserNameAndAddress(userName,toAddress), "UserName Already exist!.");
        require(checkUserNameAndAddress(userName,msg.sender), "UserName not found!.");
        userInfobyAddress[toAddress].user_name.push(userName);
        removeItem(userName,msg.sender);
        return true;
    }
