...
contract Ethcc is ERC2981, ERC721A, Ownable, PaymentSplitter {
    ...
    address[] private ogAdresses;
    address[] private wlAdresses;
    ...
    uint public saleStartTime = 1658070000;

    uint private teamLength;
    address[] private team;

    constructor(string memory _baseURI, address[] memory _team, uint[] memory _teamShares, string memory _name, string memory _symbol) 
    ...
    function mint(uint16 _quantity) public payable {
        ...
        if (sellingStep == Step.WhitelistSale) {
            ...
            require(currentTime() < saleStartTime + 240 minutes, "Whitelist Sale is finished");
            require(totalSupply() + _quantity <= MAX_SUPPLY - maxGift, "Max supply exceeded");
            bool isOg = false;
            for (uint j = 0; j < ogAdresses.length; j++) {
                if (ogAdresses[j] == msg.sender) {
            ...
            if (!isOg){
                for (uint j = 0; j < wlAdresses.length; j++) {
                    if (wlAdresses[j] == msg.sender) {
    ...
    function publicMint(uint16 _quantity) public payable {
        require(sellingStep == Step.PublicSale, "Public sale not opened");
        require(msg.value >= price * _quantity, "Not enought funds");
        require(amountNFTsperWalletPublic[msg.sender] + _quantity <= 4, "You can only get 4 NFTs per wallet");
        require(totalSupply() + _quantity <= MAX_SUPPLY + maxGift, "Max supply exceeded");
        amountNFTsperWalletPublic[msg.sender] += _quantity;
    ...
    function setOGAddresses(address[] memory _ogAddresses) external onlyOwner {
        ogAdresses = _ogAddresses;
    }


    function setWLAddresses(address[] memory _wlAddresses) external onlyOwner {
        wlAdresses = _wlAddresses;
    }
