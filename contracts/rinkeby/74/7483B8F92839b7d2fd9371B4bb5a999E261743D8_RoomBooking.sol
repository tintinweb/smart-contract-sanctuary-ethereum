...
contract RoomBooking {
    ...
    function bookMeeting(address _user, uint256 _startingTime, uint256 _endingTime, Brand _brand, string memory _roomName) external {
        require(!isAlreadyBooked(_startingTime, _endingTime, _roomName), "Room already Booked");
        reservation memory thisReservation = reservation(idReservation.current(), _user, _startingTime, _endingTime, _brand, _roomName);
        reservations.push(thisReservation);
        idReservation.increment();
    }

    function cancelMeeting(uint256 _id) external {
    ...
    function isAlreadyBooked(uint256 _startingTime, uint256 _endingTime, string memory _roomName) internal view returns(bool) {
        bool result = false;
