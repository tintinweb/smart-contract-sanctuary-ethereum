...
// REVIEW Is ABIEncoderV2 even useful?
...
contract protected {
    ...
    modifier onlyAuth() {
        require( derived.is_auth[msg.sender] || msg.sender==derived.owner, "not owner");
        _;
    }
...
contract ERC721E is IERC721A, IERC721E, protected{
    ...
    struct USER {
        mapping(uint => uint) owned;
        uint array_head;
    }
    mapping(address => USER) public user;

    function set_max_mintable(uint MAX) public {
    ...
    modifier onlyContract() {
      require(msg.sender==address(this), "Only contract is allowed to call this");
      _;
    }
    ...
    function _mint(address to, uint256 quantity) internal {
        require( (_currentIndex + quantity) <= MAX_MINTABLE, "Mint limit reached (if you are an admin, you can increase it)");
        uint256 startTokenId = _currentIndex;
        if (_addressToUint256(to) == 0) revert MintToZeroAddress();
        if (quantity == 0) revert MintZeroQuantity();
        ...
        unchecked {
            ...
            // REVIEW TEST using a local copy to save gas
            uint _array_head = user[to].array_head;

            // Loop to emit transfer event and update owned indexes
            // REVIEW Now updated index is supposed to be right
            uint n; // ANCHOR Manual Debug: starts 0
            do {
                emit NftTransfer(address(0), to, updatedIndex++);
    ...
    function _NFT_controlled_transfer(
        ...
        bool force_allow
            ...
            uint local_from_head = user[from].array_head;
            uint local_to_head = user[to].array_head;
            ...
            for(uint i; i < amount; i++) {
                ...
                uint local_id = user[from].owned[local_from_head];
    ...
    function _checkOwnedArrayOf(address actor) public view returns(uint _head_) {
    ...
    */
    ...
    function setMetadata(string memory SVG_Image, string memory Image_Uri, string[] memory properties, 
                        string[] memory attributes) public {
        require(minimal_authorized[msg.sender], "Not auth");
...
interface ERC20 {
    ...
    /// @param _value The amount of token to be transferred
    /// @return success Whether the transfer was successful or not
    function ERCtransferFrom(address _from, address _to, uint256 _value, address _origin) external returns (bool success);
...
abstract contract iNFToken is ERC20, ERC721E {
    ...
    function balanceOf(address _owner) external virtual view returns(uint) {}
    function transferFrom(
        address from,
        address to,
