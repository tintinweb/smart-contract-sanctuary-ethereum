...
contract ShufflerDescriptor is IShufflerDescriptor, Ownable {
    ...
    function getAttributes(IShufflerToken.Seed memory seed) public view returns (string memory) {
        return NFTDescriptor.generateTraitsDesc(_getPartNamesForSeed(seed), backgrounds[seed.background]);
    }
    ...
    function _getPartNamesForSeed(IShufflerToken.Seed memory seed) internal view returns (string[] memory) {
        string[] memory _partNames = new string[](5);
        _partNames[0] = cardNames[seed.card];
        _partNames[1] = sideNames[seed.side];
        _partNames[2] = _getSuit(seed.corner);
        _partNames[3] = _getRank(seed.corner);
        _partNames[4] = centerNames[seed.center];
        return _partNames;
    }
...
library NFTDescriptor {
                ...
                Base64.encode(
                    bytes(
                        abi.encodePacked(
                            ...
                            '", "attributes":', generateTraitsDesc(params.partNames, params.background),
                            '}')
                    )
                )
            )
     ...
     */
    function generateTraitsDesc(string[] memory traitNames, string memory background)
        public
        pure
        returns (string memory traitDesc)
    {
        return string(
            abi.encodePacked(
                ...
                '"},{"trait_type": "Card", "value": "', traitNames[0],
                '"},{"trait_type": "Frame", "value": "', traitNames[1],
                '"},{"trait_type": "Suit", "value": "', traitNames[2],
                '"},{"trait_type": "Rank", "value": "', traitNames[3],
                '"},{"trait_type": "Theme", "value": "', traitNames[4],
                '"}]'
            )
        );
    }
