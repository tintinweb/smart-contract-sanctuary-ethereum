...
contract BlindBox is Ownable, BlindDomain {
  ...
  function open(
    ...
    validateTime(blindbox,whiteList[buyer].whitelisted);
    validateBlindBoxSig(blindbox, sig);
    validateBuyerFeeSig(blindbox, buyerFee, buyerFeeSig);
    validateLimitPerAddress(buyer, amount);
  ...
  function validateTime(
    ...
    {
      require((blindbox.startTime + 30 minutes < block.timestamp && block.timestamp < blindbox.endTime ), "incorrect time out WhiteList");
    }
  }
  ...
  function validateLimitPerAddress(address buyer,uint amount) internal view {
    require(totalOpen + amount < openLimit, "Token limit reached");
    if(buyer != owner())
    {
      require(openPerAddress[buyer] < limitPerAddress, "already reached the limit per address client");
      require(amount==limitPerAddress,"already reached the limit per address client 2");
    }
    else
    {
      require(openPerAddress[buyer] + amount <= ownerLimit,"already reached the limit per address owner");
    }
  }

  function addToWhiteList(address[] memory newAddress) public onlyOwner {
    for (uint256 i=0; i< newAddress.length; i++)
    {
        whiteList[newAddress[i]] = Whitelist(true);
    }
  }
}
