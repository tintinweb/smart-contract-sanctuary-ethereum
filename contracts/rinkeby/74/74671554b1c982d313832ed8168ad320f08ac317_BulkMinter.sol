...
contract BulkMinter {
    ...
    function mintMany(
        ...
        uint256[] memory signatureExpirations,
        uint256[] memory prices,
        uint256[] memory desiredConfigurations,
        address transferTo
    ) external returns (uint256[] memory) {
        uint256[] memory tokenIds = new uint256[](signatures.length);
        for (uint256 i = 0; i < signatures.length; i++) {
            tokenIds[i] = avatar.mintAvatar(
                signatures[i],
                signatureExpirations[i],
                prices[i],
                desiredConfigurations[i]
            );

            avatar.safeTransferFrom(address(this), transferTo, tokenIds[i]);
        }
        return tokenIds;
    }
}
