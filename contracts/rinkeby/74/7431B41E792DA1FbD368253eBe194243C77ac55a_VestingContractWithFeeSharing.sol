 ...
 * @notice It vests the RYLT tokens to an owner over a linear schedule.
...
contract VestingContractWithFeeSharing is Ownable, ReentrancyGuard {
     ...
     * @param _maxAmountToWithdraw maximum amount in RYLT to withdraw per period
    ...
    /**
     * @notice Unlock RYLT tokens
     * @dev It includes protection for overstaking
     */
    function unlockRyltToken() external nonReentrant onlyOwner {
        require(
            (numberPastUnlocks == NUMBER_UNLOCK_PERIODS) || (block.number >= nextBlockForUnlock),
            "Unlock: Too early"
        );

        uint256 balanceToWithdraw = ryltToken.balanceOf(address(this));
        ...
        // Transfer RYLT to owner
        ryltToken.safeTransfer(msg.sender, balanceToWithdraw);
    ...
    function withdrawOtherCurrency(address _currency) external nonReentrant onlyOwner {
        require(_currency != address(ryltToken), "Owner: Cannot withdraw RYLT");
