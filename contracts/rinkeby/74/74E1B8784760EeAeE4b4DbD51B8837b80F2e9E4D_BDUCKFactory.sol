...
contract BDUCKFactory {
    constructor() {}
    uint16 public index = 0;
    event PresalePoolCreated(address[10] collections);

    function deployCollection() external returns (address[10] memory collections) {
        
        for (uint16 i; i < 10; i++) {
            BDUCK pool = new BDUCK(string(abi.encodePacked("BDUCK", Strings.toString(index++))), "BDUCK");
            collections[i] = address(pool);
        }

        emit PresalePoolCreated(collections);
    }
}