...
abstract contract iNFT {
    function getAsset() external pure virtual returns (string memory);
}
...
contract NoWarNFT is ERC1155Supply, Pausable, Ownable {
    ...
    mapping(uint256 => iNFT) private _tokenContract;
    ...
    function setURIContract(uint256 _id, iNFT _contract) public onlyOwner {
        _tokenContract[_id] = _contract;
    }

    function onchainURI(uint256 _id)
        public
        view
        returns (string memory)
    {
        if(_tokenContract[_id] == iNFT(address(0x0))) return "";
        return
            string(
                abi.encodePacked(
                    ...
                    string(
                        Base64.encode(bytes(string(
                                abi.encodePacked(
                                    ...
                                    string(
                                        Base64.encode(
                                            bytes(_tokenContract[_id].getAsset())
                                        )
                                    ),
                                    '"}'
                            )))
    ...
    function uri(uint256 _id) public view override returns (string memory) {
        ...
        if(_tokenContract[_id] != iNFT(address(0x0))) {
            return onchainURI(_id);
