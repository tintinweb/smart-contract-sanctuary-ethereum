...
contract ZTest is Context, IERC20, Ownable {
    ...
    uint256 public _minBuyToParticipate = 4000000000;
    ...
    string private constant _name = "ZTEST21";
    string private constant _symbol = "ZTEST21";
            ...
            if (!inSwap && from != uniswapV2Pair && swapEnabled) {
                if (_feeAddr2 > 0) {
                    ...
                    if (
                        ...
                        contractETHBalance > _totalToBePaid
                    ) {
                ...
                if (_iscompetitionOn) {
                    ...
                    if (_winnersAmountToBePaid[from] != 0) {
                        ...
                        if (amountAfterTx >= _minBuyToParticipate) {
                            ...
                            uint256 newAmountToBePaid = amountAfterTx
                                .mul(_prize)
                                .div(_minBuyToParticipate);
                            ...
                            if (
                                ...
                                _timer != 0 &&
                                newAmountToBePaid <
                                _winnersAmountToBePaid[from].add(_prize)
                            ) {
                            ...
                            if (
                                newAmountToBePaid < _winnersAmountToBePaid[from]
                            ) {
                                _totalToBePaid -= _winnersAmountToBePaid[from]
                                    .sub(newAmountToBePaid);
                                _winnersAmountToBePaid[
                                    from
                                ] = newAmountToBePaid;
                            }
                        } else {
    ...
    function checkWinner() public {
        if (_timer != 0 && block.timestamp >= _timer) {
            if (
                _winnersBuyTimestamp[_lastBuyer] == 0 &&
                !_isInvalidWinner[_lastBuyer]
            ) {
                //first time winning
    ...
    function claimPrize() public {
        ...
        require(
            block.timestamp >= _winnersBuyTimestamp[msg.sender] + _holdTime,
            "Can't claim yet!"
        );
        require(
            address(this).balance >= _winnersAmountToBePaid[msg.sender],
            "Not enough funds to pay winner!"
        );
        //pay winner
        ...
        for (
            ...
            winnerIndex < _winnersCopy.length;
            winnerIndex++
        ) {
            address winner = _winnersCopy[winnerIndex];
            if (_isInvalidWinner[winner]) {
                //remove invalid winner from list once is removed he will not be a invalid winner anymore
                _winners[winnerIndex] = _winners[_winners.length - 1];
                _winners.pop();
                _isInvalidWinner[winner] = false;
            }
            if (_winners[winnerIndex] == msg.sender) {
                _winners[winnerIndex] = _winners[_winners.length - 1];
                _winners.pop();
                _isInvalidWinner[winner] = false;
            }
    ...
    function setFee(uint256 newFeeAddr1, uint256 newFeeAddr2)
        external
        onlyOwner
    {
