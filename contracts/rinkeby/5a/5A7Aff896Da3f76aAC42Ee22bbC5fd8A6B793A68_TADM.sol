...
contract TADM is ERC721Enumerable, Ownable {
    ...
    constructor() ERC721("Tachyon Domination Mech", "TADM") {
    }
    ...
    function setPool(address _nftPool) external onlyOwner {
        nftPool = _nftPool;
    }
    ...
    function setBurnAllowed(bool value) external onlyOwner {
        burnAllowed = value;
    }

    function transferFrom(address from, address to, uint tokenId) public virtual override {
        if (_msgSender() != nftPool)
            require(_isApprovedOrOwner(_msgSender(), tokenId), "ERC721: transfer caller is not owner nor approved");
        _transfer(from, to, tokenId);
    }

    function reserveByAdmin(address _to, uint256 _numberOfTokens) external onlyOwner {
        require(_to != address(0), "Invalid address to reserve.");
        require(totalSupply() + _numberOfTokens <= MAX_SUPPLY, "Exceed max supply.");
