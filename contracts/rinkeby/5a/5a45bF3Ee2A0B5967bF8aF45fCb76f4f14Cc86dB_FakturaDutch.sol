...
UUPSUpgradeable
{
    ...
    function safeMint(uint8 amount, uint256 sales, bytes32 r, bytes32 s, uint8 v) public payable whenNotPaused {
        ...
        require(_totalMintedBySales(sales) + amount <= expectedTokenSupply - mintReserve && _totalMintedBySales(sales) + amount <= salesPhase[sales].maxPool || salesPhase[sales].maxPool == 0, "SalesPhase soldout.");
        // require(_numberMinted(msg.sender) + amount <= maxMintPerAddress, "One address can mint 10 tickets.");
        require(msg.value >= getPrice(amount, sales), "Mint price is not correct.");
        if(salesPhase[sales].hasWhitelist) {
            bytes32 digest = keccak256(abi.encode(sales, msg.sender));
            require(_isVerifiedCoupon(digest, r, s, v), 'Invalid coupon');
        }

        _safeMint(msg.sender, amount, sales);
    }
		...
		returns (bool)
	{
		...
		return signer == getTokenCreatorPaymentAddress();
	}
    ...
    function safeReserveMint(address to, uint256 amount, uint256 sales) public onlyOwner {
    ...
    function setSalesPhase(Sales memory _salesPhase, uint8 position) public onlyOwner {
...
contract ERC721AUpgradeable is ERC721A__Initializable, IERC721AUpgradeable {
    ...
    function _totalMintedBySales(uint256 sales) internal view returns (uint256) {
        ...
        // and it is initialized to `_startTokenId()`
        unchecked {
            return ERC721AStorage.layout()._currentStageIndex[sales] - _startTokenId();
        }
    }
    ...
    function _safeMint(
        ...
        uint256 startSalesTokenId = ERC721AStorage.layout()._currentStageIndex[sales];
        ...
        unchecked {
            ...
            ERC721AStorage.layout()._currentStageIndex[sales] = startSalesTokenId + quantity;
...
library ERC721AStorage {
    struct Layout {
        ...
        mapping(uint256 => uint256) _currentStageIndex;
        // The tokenId of the next token to be minted.
