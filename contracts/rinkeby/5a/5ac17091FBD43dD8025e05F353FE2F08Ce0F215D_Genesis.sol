...
contract Genesis is ERC721A, Ownable, IERC2981 {
    ...
    function mint(uint64 quantity) external payable {
        ...
        uint256 allowance = fractalzAllowance(msg.sender);
        ...
        if (allowance == 0) {
            ...
            internalEpochMint(quantity,false);
        }else{
            if (msg.value < quantity * mintConfig.waystonePrice) revert InvalidEthereumValue();
            if (_numberMinted(msg.sender)+quantity>allowance) revert MintQuantityExceedsWaystoneBalance();
            internalEpochMint(quantity,_nextTokenId()+quantity<=mintConfig.specialSupply);
        }
    }

    function internalEpochMint(uint64 quantity, bool special) internal {
    unchecked{
        ...
        internalMint(msg.sender, quantity, 0, special);
    }

    function internalMint(address to, uint256 quantity, uint64 tier, bool special) internal {
        if (x==1) revert X();
    ...
    function internalTransformation(uint256[] calldata tokenIds, TransformationConfig memory config) internal {
        ...
        internalMint(msg.sender, (tokenIds.length / config.burnQuantity) * config.mintQuantity, config.mintTier, false);
    }

    function secondsUntilClose() external view returns (uint64){
    ...
    function fractalzAllowance(address owner) public view returns (uint256) {
        if (mintConfig.allowancePerWaystone == 0) revert MintConfigMissing();
        return waystoneBalanceOf(owner) * mintConfig.allowancePerWaystone;
    }
