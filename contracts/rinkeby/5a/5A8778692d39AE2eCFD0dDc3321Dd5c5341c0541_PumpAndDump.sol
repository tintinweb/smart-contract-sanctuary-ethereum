...
import "./PrivateSalePnD.sol";
import "./PublicSale.sol";

contract PumpAndDump is ERC721Enumerable, Ownable, PrivateSalePnd, PublicSale {
  using SafeMath for uint256;
  using Counters for Counters.Counter;

  Counters.Counter private _tokenIds;
  Counters.Counter private _reserveTokenIds;

  string public baseTokenURI;
  uint256 public maxSupply = 2020;
  uint256 public price;
  uint256 public reservedSupply = 50;
  address public addressCanMintReservedToken;
  ...
  constructor(
    ...
    ERC721("Pump and Dump", "PnD")
    PrivateSalePnd(_privSaleStart, _privSaleExpiry)
  ...
  /**
    Function for update max supply (admin/owner only can update) 
  */

  function updateSupply(uint256 _supply) public onlyOwner {
    uint256 current = _tokenIds.current() + _reserveTokenIds.current();
    require(_supply >= current, "supply must greater than current");
    maxSupply = _supply;
  }

  /**
    Function for update price (admin/owner only can update) 
  */
  ...
  /**
    Function for update base URI (admin/owner only can update) 
  */
  ...
  /**
    Function for set address can mint reserved token
  */

  function addAddressForReservedToken(address _address) public onlyOwner {
    addressCanMintReservedToken = _address;
  }

  /**
    Function for mint reserved token, can return id token from 0 - 49
  */

  function mintForReservedToken(uint256 qty) public payable {
    require(qty > 0, "Cannot mint nothing");
    require(msg.value == price * qty, "Ether is too high or low");
    require(
      msg.sender == addressCanMintReservedToken,
      "Address can't mint reserved token"
    );

    uint256 tokenToMint = _reserveTokenIds.current();

    require(
      tokenToMint + qty - 1 < reservedSupply,
      "NFT reserve run out of supply!"
    );

    for (uint256 i = 0; i < qty; i++) {
      _safeMint(msg.sender, tokenToMint);
      _reserveTokenIds.increment();
      tokenToMint = _reserveTokenIds.current();
    }
  ...
  /**
    Function for mint in private sale time, only address in whitelisted can mint NFT.
  */
  ...
  /**
    Function for mint in public sale, function can be invoke after private sale time has been end
  */
  ...
  /**
    Function which can find out the number of tokens from an address.
  */
...
contract PrivateSalePnd is Ownable {
  using Counters for Counters.Counter;
  ...
  address[] private unmintAddressFromWhitelist;

  Counters.Counter public lockedQtyForPrivateSale;
  ...
  /**
    Function can update private sale start (only admin can update)
  */
  ...
  /**
    Function can update private sale expiry (only admin can update)
  */
    ...
    onlyOwner
  {
    ...
    uint256 currentQty = privateSaleMinterToAllowedMintQty[_address];
    bool isDifferencePositive = qtyToMint >
      privateSaleMinterToAllowedMintQty[_address];
    ...
    if (isDifferencePositive) {
      for (uint256 i = 0; i < qtyToMint - currentQty; i++) {
        lockedQtyForPrivateSale.increment();
      }
    } else {
      for (uint256 i = 0; i < currentQty - qtyToMint; i++) {
        lockedQtyForPrivateSale.decrement();
      }
    }
  }
  ...
  function addAddressesToWhitelist(
    ...
    for (uint256 i = 0; i < _addresses.length; i++) {
      uint256 currentQty = privateSaleMinterToAllowedMintQty[_addresses[i]];
      uint256 diff = _qtys[i] - currentQty;
      privateSaleMinterToAllowedMintQty[_addresses[i]] = _qtys[i];
      if (currentQty == 0) {
        whitelistedAddresses.push(_addresses[i]);
      }
      if (diff < 0) {
        uint256 positiveDiff = diff * 1;
        for (uint256 j = 0; j < positiveDiff; j++) {
          lockedQtyForPrivateSale.decrement();
        }
      } else {
  ...
  function unmintwhitelistedAddressesCount() public view returns (uint256) {
    return unmintAddressFromWhitelist.length;
  }

  function listOfUnmintwhitelistedAddresses()
    public
    view
    returns (address[] memory)
  {
    return unmintAddressFromWhitelist;
  }

  function unwhitelistAddressForPrivateSale() public onlyOwner {
    for (uint256 i = 0; i < whitelistedAddresses.length; i++) {
      uint256 previousQty = privateSaleMinterToAllowedMintQty[
        whitelistedAddresses[i]
      ];
      privateSaleMinterToAllowedMintQty[whitelistedAddresses[i]] = 0;
      for (uint256 j = 0; j < previousQty; j++) {
        lockedQtyForPrivateSale.decrement();
      }
    }
  }

  function unwhitelistSpecificAddressForPrivateSale(address _address)
    public
    onlyOwner
  {
  ...
  function getUnmintAddressFromWhitelist() public onlyOwner {
    delete unmintAddressFromWhitelist;
    for (uint256 i = 0; i < whitelistedAddresses.length; i++) {
      if (addressToHasMintedOnPrivateSale[whitelistedAddresses[i]] == false) {
        unmintAddressFromWhitelist.push(whitelistedAddresses[i]);
      }
    }
  }
