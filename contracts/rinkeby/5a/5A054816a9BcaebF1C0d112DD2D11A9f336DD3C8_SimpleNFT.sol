...
contract SimpleNFT is Ownable, ERC721Min {
    ...
    function mint(address receiver) external onlyApproved {
        _mintMin2(receiver);
    }

    function mintMultiple(address receiver, uint16 amount) external onlyApproved {
        for (uint256 i; i < amount; i++) {
            _mintMin2(receiver);
        }
    }    

    function burn(uint256 tokenId) external onlyApproved {
        _burn(tokenId);
    }

    function burnMultiple(uint256[] calldata tokenIds) external onlyApproved {
    ...
    modifier onlyApproved() {
        require(proxyToApproved[_msgSender()] || owner() == _msgSender(), "onlyProxy");
        _;
    }    
}
