...
contract Reserve is IShares, Ownable {
  ...
  uint16 public constant RESERVE_SHARES = 1500;
  uint16 public constant TOTAL_SHARES = 10000;

  event WithdrawReserve(address admin, uint256 amount);

  modifier onlyAdmin() {
    require(nftspacexAccessControls.hasAdminRole(_msgSender()), "Reserve: only admin");
    _;
  }
  ...
  function maxBalanceNST() public view override returns (uint256) {
    return (nftspacexToken.maxTotalSupply() * RESERVE_SHARES) / TOTAL_SHARES;
  }
  ...
  function withdrawReserve(uint256 amount) public onlyAdmin {
    nftspacexToken.safeTransfer(_msgSender(), amount);
    emit WithdrawReserve(_msgSender(), amount);
  }
}
