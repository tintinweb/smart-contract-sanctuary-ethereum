...
/// @notice Smart contract to store, generate and manage access token for BELLA chat using ERC1155 standard
contract AccessTokenManager is Initializable, UUPSUpgradeable, ReentrancyGuardUpgradeable, AccessControlUpgradeable {

    // ATM-ERR-1		ChannelId cannot be empty
    // ATM-ERR-2		Buyer cannot be the channel creator
    // ATM-ERR-3		Please submit required price of:PRICE
    // ATM-ERR-4		tickets sold out
    // ATM-ERR-5		nothing to withdraw
    // ATM-ERR-6		Access Token address cannot be zero-address
    // ATM-ERR-7		Backend address cannot be zero-address
    // ATM-ERR-8		Creator address cannot be zero-address
    // ATM-ERR-9		Royalty cannot be greater than 100%
    // ATM-ERR-10		UriMetadata cannot be empty
    // ATM-ERR-11		Ticket supply must be greater than zero
    // ATM-ERR-12		CommunityId cannot be empty		
    // ATM-ERR-13		ChannelID already exists
    // ATM-ERR-14		newImplementation cannot be zero-address
    ...
    function purchase(string memory channelId) 
        ...
        nonReentrant 
    {   
        require(bytes(channelId).length != 0, "ATM-ERR-1");
        AccessTokenItem memory accessToken = idToAccessTokenItem[channelId];
        require(msg.sender != accessToken.creator, "ATM-ERR-2");
        require(msg.value == accessToken.finalPrice, _concatenateStringUint("ATM-ERR-3", accessToken.finalPrice));
        if (accessToken.tokenId != 0 
            && IERC1155(accessTokenContract).balanceOf(accessToken.creator, accessToken.tokenId) == 0) {            
            revert("ATM-ERR-4");
        }
        ...
        pendingWithdrawals[backendAddress] += accessToken.feeCreator;
        pendingWithdrawals[backendAddress] += accessToken.feeBuyer;
    ...
    {
        require(bytes(channelId).length != 0, "ATM-ERR-1");
        require(creator != address(0), "ATM-ERR-8");
        require(royaltyPercentage <= 10000, "ATM-ERR-9");
        require(bytes(uriMetadata).length != 0, "ATM-ERR-10");
        require(tickets > 0, "ATM-ERR-11");
        require(bytes(communityId).length != 0, "ATM-ERR-12");
        if (idToAccessTokenItem[channelId].tokenId > 0){
            revert("ATM-ERR-13");
        }
        (uint finalPrice,uint feeCreator,uint feeBuyer) = _calculateFeesAndPrice(price, buyerFeePercentage, creatorFeePercentage, minBuyerFee);
        ...
        initializer 
    {
        ...
        creatorFeePercentage = 500; //5%
        buyerFeePercentage = 500; //5%
        minBuyerFee = 0;
    }
    ...
    {
        require(atc != address(0), "ATM-ERR-6");
    ...
    function initBackendAddress(address ab) 
        ...
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        require(ab != address(0), "ATM-ERR-7");
        backendAddress=ab;
    ...
    function _authorizeUpgrade(address newImplementation) 
        ...
        view 
    {
        require(newImplementation != address(0), "ATM-ERR-14");
    }

    // Private Pure Functions

    //Calculate price and fees.
    /// @param price the price on which calculate fee and final price
    /// @param feeBuyerPercentage fees percentage for buyer if price is greater than zero
    /// @param feeCreatorPercentage fees percentage for creator if price is greater than zero
    /// @param minBuyerFeeValue fees percentage for buyer if price is equal to zero
    /// @notice calculate the fees and the final price for a given price
    function _calculateFeesAndPrice(
        uint price, 
        ...
        returns(uint, uint, uint) 
    {
        ...
        uint feeBuyer = 0;
        if(price > 0) {
            if(feeCreatorPercentage != 0) {
                feeCreator = _calculateFee( price, feeCreatorPercentage );
            }
            if(feeBuyerPercentage != 0) {
                feeBuyer = _calculateFee( price, feeBuyerPercentage );
            }
        } else {
            if(minBuyerFeeValue != 0) {
                feeBuyer = minBuyerFeeValue;
            }
        }
        uint finalPrice = price + feeBuyer;
    ...
    /// @notice function that concatenate a string with a uint by converting the uint to string using the openzeppelin library
...
contract AccessToken is ERC1155, AccessControl, ERC1155Burnable, ERC1155Supply, ReentrancyGuard, ERC2981 {
    ...
    // AT-ERR-1		Access Token Manager address cannot be zero-address
    // AT-ERR-2		tokenId not valid
    // AT-ERR-3		Cannot remove approval to manager
    // AT-ERR-4		Cannot set uri twice
    ...
    string private _contractUri = "https://ipfs.moralis.io:2053/ipfs/QmSjmgH3nWLyMVLS7aXgkYmgGKhB1urKvYhZBDpaaxQjKN//BELLA/BAT.json";
    ...
    constructor(address contractAddress) ERC1155("") {    
        require(contractAddress != address(0), "AT-ERR-1");
    ...
    function setApprovalForAll(address operator, bool approved) public override {
        require(operator != managerContract, "AT-ERR-3");    
        _setApprovalForAll(_msgSender(), operator, approved);
    }
