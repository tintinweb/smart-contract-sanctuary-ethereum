...
contract DutchAuctionSeller is Ownable {
   using Address for address payable;
    ...
    function setBeneficiary(address payable targetAddress) external onlyOwner {
        beneficiary = targetAddress;
    }

    function retrieveFunds() public {
        require( msg.sender == owner() || msg.sender == beneficiary, "Not allowed" );
        require(address(this).balance > 0,"Balance is empty");
        payable(beneficiary).sendValue(address(this).balance);
    }

    function balance() external view returns (uint256){
        require(msg.sender == owner() || msg.sender == beneficiary, "Only owner and beneficiary are allowed");
