...
contract ExchangeCore is ReentrancyGuarded, Ownable {
        ...
        reentrancyGuard
    {
        ...
        if (buy.maker == msg.sender) {
            require(validateOrderParameters(buy),"buy.maker == msg.sender");
        } else {
        ...
        if (sell.maker == msg.sender) {
            require(validateOrderParameters(sell),"sell.maker == msg.sender");
        } else {
        ...
        require(ArrayUtils.arrayEq(buy.calldata, sell.calldata),"ArrayUtils.arrayEq");

        /* Retrieve delegateProxy contract. */
        OwnableDelegateProxy delegateProxy = registry.proxies(sell.maker);

        /* Proxy must exist. */
        require(delegateProxy != address(0),"delegateProxy != address(0)");

        /* Assert implementation. */
        require(delegateProxy.implementation() == registry.delegateProxyImplementation(),"delegateProxy.implementation()");
        ...
        require(proxy.proxy(sell.target, sell.howToCall, sell.calldata),"proxy.proxy()");

        /* Static calls are intentionally done after the effectful call so they can check resulting state. */

        /* Handle buy-side static call if specified. */
        if (buy.staticTarget != address(0)) {
            require(staticCall(buy.staticTarget, sell.calldata, buy.staticExtradata),"buy.staticTarget != address(0)");
        }

        /* Handle sell-side static call if specified. */
        if (sell.staticTarget != address(0)) {
            require(staticCall(sell.staticTarget, sell.calldata, sell.staticExtradata),"staticCall()");
        }
