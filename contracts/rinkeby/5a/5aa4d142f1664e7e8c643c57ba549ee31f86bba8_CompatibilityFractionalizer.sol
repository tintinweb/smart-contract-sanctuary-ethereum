...
contract CompatibilityFractionalizer is ReentrancyGuard
{
	using Address for address;
	...
	function fractionalize(address _target, uint256 _tokenId, string memory _name, string memory _symbol, uint8 _decimals, uint256 _fractionsCount, uint256 _fractionPrice, address _paymentToken) external nonReentrant returns (address _fractions)
	{
		...
		while (true) {
			_fractions = _computeAddress(fractionalizer, ++nonce);
            emit SetFractions(_fractions);
			if (!_fractions.isContract()) break;
		}
		Fractionalizer(fractionalizer).fractionalize(_target, _tokenId, _name, _symbol, _decimals, _fractionsCount, _fractionPrice, _paymentToken);
		//IERC20(_fractions).safeTransfer(_from, _fractionsCount);
		return _fractions;
	}

    event SetFractions(address _fractions);

	function _computeAddress(address _account, uint256 _nonce) internal pure returns (address _address)
	{
		bytes memory _data;
		if (_nonce == 0x00) {
