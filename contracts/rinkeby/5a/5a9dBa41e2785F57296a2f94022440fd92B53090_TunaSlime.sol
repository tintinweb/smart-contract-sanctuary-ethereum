...
contract TunaSlime is ERC20, Ownable {

    iTunaFish public TunaFish;
    ...
    constructor(address tunaAddress) ERC20("Tuna Slime", "SLYME") {
        TunaFish = iTunaFish(tunaAddress);
        DeployTime = block.timestamp;
    }

    function isRewardPaused() external view returns(bool){
        return rewardPaused;
    } 

    function updateReward(address from, address to) external {
        require(msg.sender == address(TunaFish));
        if(from != address(0)){
    ...
    function burn(address user, uint256 amount) external {
        require(allowedAddresses[msg.sender] || msg.sender == address(TunaFish), "Address does not have permission to burn");
        _burn(user, amount);
    }
    ...
    function getPendingReward(address user) internal view returns(uint256) {
        return TunaFish.balanceOf(user) * Rate * (block.timestamp - lastUpdate[user]) / 86400;
    }
