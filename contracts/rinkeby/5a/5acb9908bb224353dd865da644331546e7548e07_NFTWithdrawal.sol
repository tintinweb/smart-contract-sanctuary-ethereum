...
contract NFTWithdrawal is Initializable, OwnableUpgradeable, EIP712, Pausable {
	...
	function _hashNFTBillCallData(NFTBillCallData calldata data) internal view returns (bytes32) {
		return _hashTypedDataV4(keccak256(abi.encode(
			keccak256("NFTBillCallData(address owner,address contractAdd,uint256 tokenId, uint256 nonce)"),
			data.owner,
			data.contractAdd,
			data.tokenId,
            data.nonce
	...
	function extractNFT(NFTBillCallData calldata data, address target) external {
		require(!ifNonceUsed[data.nonce]);
		require(isNFTBillCallDataValid(data));
		ERC721Interface erc721 = ERC721Interface(data.contractAdd);
		require(erc721.ownerOf(data.tokenId) == target);
		require(erc721.isApprovedForAll(target, address(this)));
