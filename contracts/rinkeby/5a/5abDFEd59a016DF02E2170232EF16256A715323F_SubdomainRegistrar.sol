...
contract SubdomainRegistrar is Ownable, ERC721A {
    ...
    // 0x57f1887a8BF19b14fC0dF6Fd9B2acc9Af147eA85 - Rinkeby and Mainnet
    IBaseRegistrar public ensRegistrarContract = IBaseRegistrar(0x57f1887a8BF19b14fC0dF6Fd9B2acc9Af147eA85);
    ...
    function stakeDomain (string memory domain, bool enabled, uint256 price, uint256 unlockUnixTimestamp) external {
        ...
        uint256 expiration = getExpiration(domain);
        require(expiration > unlockUnixTimestamp, "Provided unlock time is greater than your domain expiration.");
        ...
        if (domainLabelMap[domain] == 0) {
            ...
            // For this to work, owner has to call:
            // - approve(contract_address, domain_hash);
            // Could work it into the web3 application.
            ensRegistrarContract.transferFrom(msg.sender, address(this), domainToTokenId(domain));
    ...
    function unstakeDomain (uint256 domainIndex) external {
        ...
        //ensRegistrarContract.transferFrom(address(this), domainOwnerMap[domainIndex], domainToTokenId(domain));
    ...
    function domainToTokenId(string memory domain) public pure returns(uint256 id) {
        return uint256(keccak256(bytes(domain)));
    }

    function getExpiration(string memory domain) public view returns(uint256 expiration) {
      return ensRegistrarContract.nameExpires(domainToTokenId(domain));
    }
}
