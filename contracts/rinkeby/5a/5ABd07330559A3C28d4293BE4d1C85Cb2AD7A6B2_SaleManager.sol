...
contract SaleManager is ReentrancyGuard {
  ...
  struct Sale {
    ...
    address manager; // the address managing the sale
  ...
  modifier validSale (bytes32 saleId) {
    // if the manager is address(0) there is no sale struct at this saleId
    require(
      sales[saleId].manager != address(0),
      "invalid sale id"
    );
    _;
  }

  modifier isManager(bytes32 saleId) {
    // msg.sender is never address(0) so this handles uninitialized sales
    require(
      sales[saleId].manager == msg.sender,
      "must be manager"
    );
    _;
  }
  ...
  // Accessor functions
  function getManager(bytes32 saleId) public validSale(saleId) view returns(address) {
    return(sales[saleId].manager);
  }
  ...
  function generateRandomishValue(bytes32 merkleRoot) public view returns(uint160) {
    /**
      ...
      - managers can repeatedly call setMerkleRoot()
    */
  ...
  function getFairQueueTime(bytes32 saleId, address buyer) public validSale(saleId) view returns(uint) {
    /**
      ...
      Although miners and managers can minimize the delay for an arbitrary address, these are not significant threats
      ...
      - managers can repeatedly set merkle roots (but managers already control the tokens being sold!)
  ...
  - the address calling this method is the manager: only the manager can change sale configuration
  ...
  */
  function newSale(
    ...
    require(sales[saleId].manager == address(0), "a sale with these parameters already exists");
  ...
  function setStart(bytes32 saleId, uint startTime) public validSale(saleId) isManager(saleId) {
    // manager can update start time until the sale starts
  ...
  function setEnd(bytes32 saleId, uint endTime) public validSale(saleId) isManager(saleId){
    // manager can update end time until the sale ends
  ...
  function setMerkleRoot(bytes32 saleId, bytes32 merkleRoot) public validSale(saleId) isManager(saleId){
  ...
  function setMaxQueueTime(bytes32 saleId, uint160 maxQueueTime) public validSale(saleId) isManager(saleId) {
  ...
  // function setUriAndMerkleRoot(bytes32 saleId, bytes32 merkleRoot, string calldata uri) public validSale(saleId) isManager(saleId) {
  ...
  function buy(
    ...
    // forward the eth to the manager
  ...
  function registerClaimManager(bytes32 saleId, address claimManager) public validSale(saleId) isManager(saleId) {
  ...
  function recoverERC20(bytes32 saleId, address tokenAddress, uint256 tokenAmount) public isManager(saleId) {
