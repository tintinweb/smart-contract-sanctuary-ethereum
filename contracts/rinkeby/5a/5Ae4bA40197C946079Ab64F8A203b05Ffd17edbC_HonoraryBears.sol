...
contract HonoraryBears is ERC721, Ownable {
    ...
    // PUBLIC FUNCTIONS
    function allowlistMint(uint256 _numTokens) external payable {
        require(isAllowListActive, "Allowlist is not active.");
        require(_allowList[msg.sender] == true, "Not allowlisted.");
        require(_numTokens <= MAX_MINT_PER_TX, "You can only mint a maximum of 5 per transaction.");
        require(mintedPerWallet[msg.sender] + _numTokens <= 1000, "You can only mint 1000 per wallet.");
    ...
    function withdrawAll() external payable onlyOwner {
        ...
        (bool transferOne, ) = payable(0xA7f5cF766926291e9422E652164F7241Bdf09eb3).call{value: balanceOne}("");
