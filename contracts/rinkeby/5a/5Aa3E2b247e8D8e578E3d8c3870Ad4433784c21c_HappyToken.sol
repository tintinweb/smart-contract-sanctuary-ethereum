...
contract HappyToken is INounsToken, Ownable, ERC721Enumerable {
  ...
  // description
  string public description = "HappyToken 2022";
  ...
  // AssetStoreTokens
  //IAssetStoreToken public immutable assetStoreToken;
  ...
  constructor(
    ...
    ) ERC721('Happy Token 2022', 'HappyToken22') {
      limit = _limit;
  ...
  /**
    ...
    */
  function customMint(uint256 tokenId, uint256 assetTokenId, string calldata message) public  returns (uint256) {
    require(balanceOf(msg.sender) == 0, "You already have one.");
    require(_currentNounId < limit, "Sold out.");
    _currentNounId++;
    messages[tokenId]=message;
    console.log(tokenId);
    console.log("new Token with asset:");
    console.log(assetTokenId);
    assets[tokenId]=assetTokenId;
    console.log(assets[tokenId]);    
    _mint(owner(), msg.sender, tokenId);
    emit NounBought(tokenId, msg.sender);
    return tokenId;
  }

  function setMessage(uint256 tokenId, string calldata message) public{
    require(_exists(tokenId), 'URI query for nonexistent token');    
    messages[tokenId]=message;
  }

  function setAssetId(uint256 tokenId, uint256 assetId) public {
    require(_exists(tokenId), 'URI query for nonexistent token');    
  ...
  /**
    * @notice Anybody can mint, one per wallet.
    */
  function mint() public pure override returns (uint256) {
    require(false, "Sorry thi is not supported.");
    return 0;
  }
  /**
    * @notice Burn a noun.
  ...
  function _generateSVG(uint256 tokenId) internal view returns (bytes memory) {
    bytes memory pack = abi.encodePacked(
      '<?xml version="1.0" encoding="utf-8"?><svg width="1024" height="1024" viewBox="0 0 1024 1024" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" shape-rendering="crispEdges">\n'
      );
    uint256 assetId = assets[tokenId];
    if (assetId == 0){
      assetId = 1; //3: Share Icon assert
    }
    (IAssetStore.AssetAttributes memory attr,) = getAttributes(assetId);
    console.log(attr.name);
    console.log(attr.tag);
    //string memory imaget = assetStore.generateSVGPart(1, attr.tag);
    bytes memory image = bytes(assetStore.generateSVG(assetId));
    pack = abi.encodePacked(pack,'<rect width="1024" height="1024" fill="#71BCE1" />\n');
    bytes memory lifepath = _happyMessage(tokenId);
    pack = abi.encodePacked(pack,lifepath);
    return abi.encodePacked(pack,'<g transform="translate(300,400) scale(0.4)">', image,'</g>' ,'</svg>');   
  }
  function getAttributes(uint256 assetId) private view returns (IAssetStore.AssetAttributes memory attr, bool success){
    console.log(block.number);
    console.log("call GetAttributes");
    try  assetStore.getAttributes(assetId) returns (IAssetStore.AssetAttributes memory a) {
        console.log("success!");
        return (a, true);
        ...
        console.log("error");
        console.log(reason);
        IAssetStore.AssetAttributes memory a;
        return (a, false);
...
function _happyMessage(uint256 tokenId) internal view returns (bytes memory) {
    uint[4] memory birthdate; //example 0x160613 means (19th June 2022)
    for (uint i=0; i<4; i++) {
      uint256 mask = 0xff;
      birthdate[i] = uint((tokenId >> (8*i)) & mask);
      //console.log(birthdate[i]);
    }
    uint birthStamp = timeLib.timestampFromDate(birthdate[3]*100+birthdate[2],birthdate[1],birthdate[0]);
    //console.log(birthStamp);
    //console.log(timeLib.diffYears(birthStamp, block.timestamp));
    uint lastbirthStamp = timeLib.timestampFromDate(timeLib.getYear(block.timestamp)-1,birthdate[1],birthdate[0]);
    uint day = timeLib.diffDays(lastbirthStamp, block.timestamp);
    if(day > 365){
      day -= 365;
    }
    console.log(day);
    ...
    bytes memory pack;
    /*
    (uint year,uint month,uint day,uint hour,uint minute,uint second) = timeLib.timestampToDateTime(block.timestamp);
    console.log(minute);
    */
    pack = abi.encodePacked(
        '<path id="p1" d="M 100,300 C 240,40 760,40 900,300" fill="none"  /><text font-size="32" font-weight="bold" fill="Yellow"> <textPath xlink:href="#p1">');
    pack = abi.encodePacked(
        pack,
        timeLib.getYear(block.timestamp ).toString(),'/',
        timeLib.getMonth(block.timestamp).toString(),'/',
        timeLib.getDay(block.timestamp ).toString(),' ',
        timeLib.getHour(block.timestamp ).toString(),':',
        timeLib.getMinute(block.timestamp ).toString(),'(UTC)');
    pack = abi.encodePacked(
        pack,'  Happy',
        timeLib.diffYears(birthStamp,block.timestamp).toString(),'Years and',
        day.toString(),'Days!!',
        '</textPath></text>');
    pack = abi.encodePacked(
        pack,'<text x="100" y="400" font-size="20" font-weight="bold" fill="Yellow"> ');
    pack = abi.encodePacked(pack,'Message:',messages[tokenId],'</text>');
    return pack;
  ...
  function dataURI(uint256 tokenId) public view override returns (string memory) {
    ...
    string memory name = string(abi.encodePacked('HappyToken#', nounId));
