...
contract Billionaire is Ownable,PaymentSplitter, ERC721A("Test YAllah", "TYA"){
 //To concatenate the URL of an NFT
    ...
    uint public max_mint_allowed_public =  1;
...
/**we c est vrai, apres k finis la minting dapp, j vais optimizer ce contract, enlever ERC721Enumerable parsque counter peut faire la mm chose et enlever d autre module si j ai le mm problem  */
    ...
    constructor(string memory _theBaseURI, string memory _notRevealedURI, bytes32 _merkleRoot) PaymentSplitter(_team, _teamShares) {
    ...
    **/
    function changeMaxMintAllowedPresale(uint _maxMintAllowed) external onlyOwner {
        max_mint_allowed_presale = _maxMintAllowed;
    }
    function changeMaxMintAllowedPublicSale(uint _maxMintAllowed) external onlyOwner {
    ...
    **/
    function setUpSale() external onlyOwner {
        require(sellingStep == Steps.Presale, "Set up the presale first, then the sale.");
    ...
    **/
    ...
    function presaleMint(bytes32[] calldata _proof, uint256 _ammount) external payable  {
        //Is paused?
        ...
        require(_ammount > 0 && nftsPerWallet[msg.sender] + _ammount <= max_mint_allowed_presale, "You can only get 2 NFT on the Presale");
        ...
        require(msg.value >= price * _ammount, "Not enought funds."); //fix 1 .test it please
    ...
    **/
    function saleMint(uint256 _ammount) external payable  {
        ...
        require(nftsPerWalletSale[msg.sender] + _ammount <= max_mint_allowed_public, "You can only get 1 NFT on the Sale");
    ...
    **/
    ...
    if (bytes(notRevealedURI).length > 0) return notRevealedURI;
