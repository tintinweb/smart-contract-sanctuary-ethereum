...
contract NeoRunners is ERC721A, Ownable {
    ...
    string public baseURI = "ipfs://QmfSiRUfZJGvcfepJ2bkwMESSLzMjA5Na26KQDyU9EbMAe/";
    
	constructor() ERC721A("Neo Runners", "NRS") {}

    function mint(uint256 quantity) external payable {
        
        require(!isPaused, "Sales are off");
		require(numberMinted(msg.sender) + quantity <= maxPerWallet,"Mint limit exceeded!");
        require(quantity > 0, "Wrong quantity");
        require(msg.value >= quantity * mintPrice, "Please send the exact amount");
    ...
    function cutSupply(uint256 _maxSupply) external onlyOwner {
		require(_maxSupply < maxSupply , "we are not that kind of people...");
