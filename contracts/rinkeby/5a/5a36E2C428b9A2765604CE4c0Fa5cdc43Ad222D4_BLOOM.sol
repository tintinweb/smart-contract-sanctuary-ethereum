...
contract BLOOM is ERC721, Ownable {
    ...
    function airdropTokens(
        ...
        for (uint256 i = 0; i < listOfAddresses.length; i++) {
            ...
            timeOfLastSeeding[Ids[i]] = block.timestamp;
        }
    }
    ...
    function getSeedingTime(uint256 tokenId) private view returns (uint256) {
        uint256 res;
        if (_exists(tokenId)) {
            res = block.timestamp - timeOfLastSeeding[tokenId];
        }

        return res;
    }

    function getSeedingTimes(uint256[] calldata tokenIds)
        external
        view
        returns (uint256[] memory)
    {
        ...
        for (uint256 i = 0; i < resLength; i++) {
            tokenIDsTimes[i] = getSeedingTime(tokenIds[i]);
        }
