...
contract ERC721A is Context, ERC165, IERC721 {
    ...
    function _safeMintRandonNFT(address to, uint256 tokenId) internal {
        _safeMintRandonNFT(to, tokenId, '');
    }
...
contract Senra is ERC721A, Ownable {
  ...
  mapping(address => uint256[]) private _addressLastMintedTokenIds;
  mapping(uint256 => bool) public _mintedNFTs;

  //Para validação da whitelist
  //BufferRoot criado a partir do array: ["0x2Bb339799866B0eBAD53D8b493a923Ffb67D9032", "0x93062D0Cc58505dDb7a8D993CdA32Bc25090c72C"]
  bytes32 public root = 0xec49f8cd322a5455ca76476e4acaf3e8873381cb46fd636acc1f1de0184a67ff;
  //bytes32 public root = 0x030e929822f3d90977f609cc762f1b924182dee60ba902126970109040abd7f9;
  bytes32[] public merkleProof; //Proof que vem do JS
  mapping(address => bool) public whitelistClaimed; //Informa se já usou os direitos da whitelist.
  ...
  // public
  function mint(address _to, uint256 _mintAmount, bool whitelist) public payable {
    uint256 supply = totalSupply();
    require(!paused, "O contrato esta pausado.");
    require(_mintAmount > 0, "Precisa mintar pelo menos 1 NFT.");
    require(_mintAmount <= maxMintAmount, "Excedida a quantidade limite de mint por vez.");
    require(supply + _mintAmount <= maxSupply, "Quantidade limite de mint excedida.");
    require(maxMintAmount > _freeMintAmountPerWallet[_to] + _mintAmountPerWallet[_to], "Esta carteira mintou a quantidade limite de tokens permitidos por carteira.");
    ...
    //bool whitelist = true; //TODO: Colocar a função para checar a whitelist offchain.
    ...
    if(!_freemint){
        ...
        //whitelist = isWhitelisted(msg.sender, proof); //TODO: Descomentar

        //Public mint
        if(!whitelist){
          require(_mintAmountPerWallet[msg.sender] < _maxMintAmountAllowedPerWallet, "Quantidade limite de mint publico por carteira excedida");
          require(msg.value >= (_nftEtherValue * _mintAmount), "Valor da mintagem diferente do valor definido no contrato");
          _mintAmountPerWallet[msg.sender]++;
        ...
        if(whitelist){
          ...
          if(_freeMintAmountPerWallet[msg.sender] < _maxFreeMintAmountAllowedPerWallet){
            _freeMintAmountPerWallet[msg.sender]++;
            ...
            require(_mintAmountPerWallet[msg.sender] < _maxMintAmountAllowedPerWallet + _maxFreeMintAmountAllowedPerWallet, "Quantidade limite de mint por carteira excedida");
            require(msg.value >= (_nftEtherValue * _mintAmount), "Valor da mintagem diferente do valor definido no contrato");
    ...
    */
    function _mint(
        ...
        while (_mintedNFTs[tokenId]) {
          tokenId = getRandomAvailableTokenId(payable(msg.sender), updatedNumAvailableTokens);
        }

        _mintedNFTs[tokenId] = true;
        ...
        _addressLastMintedTokenIds[msg.sender].push(tokenId);
