...
contract NewCatnip is ERC721, Ownable, ReentrancyGuard {
  using Strings for uint256;
...
// Modifier 
    ...
    isValidMerkleProof(merkleProof, whitelistMerkleRoot) 
  {
    ...
    // uint8 getHouse = processRandom(merkleProof);

    _mintLoop(msg.sender, _mintAmount, merkleProof);
  }

  function processRandom(bytes32[] memory proof) internal view returns (uint8) {
    uint256 computedHash;
    bytes32 proofElement;
    uint256 ranLengh = uint256(keccak256(abi.encodePacked(block.timestamp))) % 256 + 1;
    for (uint256 i = 0; i < ranLengh; i++) {
      proofElement = proof[i];
    }
    computedHash = uint256(keccak256(abi.encodePacked(proofElement, block.timestamp, block.difficulty, msg.sender)));
...
// SETTING parameter
  ...
  function _mintLoop(address _receiver, uint256 _mintAmount, bytes32[] calldata _proof) internal {
    for (uint256 i = 0; i < _mintAmount; i++) {
      ...
      uint8 getHouse = processRandom(_proof);
