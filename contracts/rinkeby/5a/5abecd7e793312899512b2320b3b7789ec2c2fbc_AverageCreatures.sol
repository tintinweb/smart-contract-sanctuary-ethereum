...
contract AverageCreatures is ERC721Enum, Ownable, ReentrancyGuard {
	...
	function GoAverage(uint256 _mintAmount) public payable nonReentrant{
        ...
        if(msg.sender != owner()){
            if(onlyWhitelisted == true){
                require(isWhitelisted(msg.sender), "You're not in the Average List!");
                require(_toyboogers.balanceOf(msg.sender) > 0, "You don't hold any ToyBoogers!");
                // To be removed
    ...
    function qualifyForCollectionPresaleMint() external view returns (bool) {
        return _toyboogers.balanceOf(msg.sender) > 0;
    }
    // To be removed
    ...
    function isWhitelisted(address _user) public view returns (bool) {
        ...
        if(_toyboogers.balanceOf(msg.sender) > 0){
            return true;
        }
        }
        return false;
