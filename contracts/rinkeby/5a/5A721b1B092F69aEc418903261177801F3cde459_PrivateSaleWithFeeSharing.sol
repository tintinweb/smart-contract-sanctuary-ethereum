 ...
 * @notice It handles the private sale for RYLT tokens (against ETH) and the fee-sharing
 * mechanism for sale participants. It uses a 3-tier system with different
 * costs (in ETH) to participate. The exchange rate is expressed as the price of 1 ETH in RYLT token.
...
contract PrivateSaleWithFeeSharing is Ownable, ReentrancyGuard {
    ...
    IERC20 public immutable ryltToken;
    ...
    // Total RYLT expected to be distributed
    uint256 public immutable TOTAL_RYLT_DISTRIBUTED;

    // Current sale phase (uint8)
    SalePhase public currentPhase;

    // Block where participants can withdraw the RYLT tokens
    uint256 public blockForWithdrawal;

    // Price of WETH in RYLT for the sale
    uint256 public priceOfETHInRYLT;
    ...
    event NewPriceOfETHInRYLT(uint256 price);
    ...
    /**
     * @notice Withdraw RYLT + pending WETH
     */
    function withdraw() external nonReentrant {
        ...
        // Calculate amount of RYLT to transfer based on the tier
        uint256 ryltAmountToTransfer = allocationCostPerTier[userInfo[msg.sender].tier] * priceOfETHInRYLT;

        // Transfer RYLT token to sender
        ryltToken.safeTransfer(msg.sender, ryltAmountToTransfer);

        emit Withdraw(msg.sender, userInfo[msg.sender].tier, ryltAmountToTransfer);
    }
    ...
    /**
     * @notice Update block deadline for withdrawal of RYLT
     * @param _blockForWithdrawal block for withdrawing RYLT for sale participants
     */
    ...
    /**
     * @notice Set price of 1 ETH in RYLT
     * @param _priceOfETHinRYLT price of 1 ETH in RYLT
     */
    function setPriceOfETHInRYLT(uint256 _priceOfETHinRYLT) external onlyOwner {
        require(currentPhase == SalePhase.Pending, "Owner: Phase must be Pending");
        priceOfETHInRYLT = _priceOfETHinRYLT;

        emit NewPriceOfETHInRYLT(_priceOfETHinRYLT);
    }
    ...
    function updateSalePhase(SalePhase _newSalePhase) external onlyOwner {
        if (_newSalePhase == SalePhase.Deposit) {
            ...
            // Risk checks
            require(priceOfETHInRYLT > 0, "Owner: Exchange rate must be > 0");
            require(getMaxAmountRYLTToDistribute() == TOTAL_RYLT_DISTRIBUTED, "Owner: Wrong amount of RYLT");
            require(
                ryltToken.balanceOf(address(this)) >= TOTAL_RYLT_DISTRIBUTED,
                "Owner: Not enough RYLT in the contract"
            );
    ...
    /**
     * @notice Withdraw the total commited amount (in ETH) and any RYLT surplus.
    ...
    function withdrawCommittedAmount() external onlyOwner nonReentrant {
        ...
        // If some tiered users did not participate, transfer the RYLT surplus to contract owner
        if (totalAmountCommitted * priceOfETHInRYLT < (TOTAL_RYLT_DISTRIBUTED)) {
            uint256 tokenAmountToReturnInRYLT = TOTAL_RYLT_DISTRIBUTED - (totalAmountCommitted * priceOfETHInRYLT);
            ryltToken.safeTransfer(msg.sender, tokenAmountToReturnInRYLT);
        }
    ...
    /**
     * @notice Retrieve max amount to distribute (in RYLT) for sale
     */
    function getMaxAmountRYLTToDistribute() public view returns (uint256 maxAmountCollected) {
        for (uint8 i = 1; i <= NUMBER_TIERS; i++) {
            maxAmountCollected += (allocationCostPerTier[i] * numberOfParticipantsForATier[i]);
        }

        return maxAmountCollected * priceOfETHInRYLT;
    }
}