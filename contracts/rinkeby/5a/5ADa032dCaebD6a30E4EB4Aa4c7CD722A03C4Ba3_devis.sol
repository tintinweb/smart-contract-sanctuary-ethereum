...
//Contratto prova di David. Contratto con funzioni base più merkleproof per eventuale whitelist senza reveal

//Contratti da importare
...
//Tipologia contratto
contract devis is ERC721Enumerable, Ownable {
    ...
    //Massimo mintabili per categorie ipotizzate OGWhitelist, Whitelist, Community, Public
    uint256 public maxOgSaleMint = 2;
    uint256 public maxPreSaleMint = 1;
    uint256 public maxCommunitySaleMint = 1;
    uint256 public maxPublicSaleMint = 1;

    //Prezzi per categorie
    uint256 public ogSalePrice; 
    ...
    //Mappature
    mapping(address => uint256) public ogSaleMinted;
    mapping(address => uint256) public preSaleMinted;
    mapping(address => uint256) public communitySaleMinted;
...
constructor(string memory baseURI) ERC721("DEVISNFT", "DEVIS") {
     setBaseURI(baseURI);
}
...
function setBaseURI(string memory _baseTokenURI) public onlyOwner {
    ...
    //Funzioni Mint (Da riguardare)
function ogSaleMint(uint256 _quantity, uint256 _maxAmount,  bytes32[] calldata _proof) external payable callerIsUser {
        require(isOgSaleActive, "PRESALE NOT ACTIVE");
        require(msg.value == ogSalePrice * _quantity, "NEED TO SEND CORRECT ETH AMOUNT");
        require(totalSupply() + maxOgSaleMint <= MAX_SUPPLY, "MAX SUPPLY REACHED" );
        require(ogSaleMinted[msg.sender] + _quantity <= _maxAmount, "EXCEEDS MAX CLAIM"); 
        bytes32 sender = keccak256(abi.encodePacked(msg.sender, _maxAmount));
        bool isValidProof = MerkleProof.verify(_proof, ogSaleMerkleRoot, sender);
        require(isValidProof, "INVALID PROOF");

        ogSaleMinted[msg.sender] += _quantity;
        _safeMint(msg.sender, maxOgSaleMint);
    }
...
function preSaleMint(uint256 _quantity, uint256 _maxAmount,  bytes32[] calldata _proof) external payable callerIsUser {
        ...
        require(totalSupply() + maxPreSaleMint <= MAX_SUPPLY, "MAX SUPPLY REACHED" );
...
function publicSaleMint(uint256 _quantity) external payable callerIsUser {
    ...
    //Attivare o disattivare mint
function setOgSaleStatus(bool _status) external onlyOwner {
        isOgSaleActive = _status;
    } //OGSale
...
function setPublicSaleStatus(bool _status) external onlyOwner {
    ...
    //Settare MerkleRoot per categorie allowlist
...
function setCommunitySaleMerkleRoot(bytes32 _merkleRoot) external onlyOwner {
    ...
    //Settaggio prezzi
function setOgSalePrice(uint256 _price) external onlyOwner {
        ogSalePrice = _price;
    } //prezzo OGWL
...
function setPublicSalePrice(uint256 _price) external onlyOwner {
    ...
    //Riservare nft al team
function reserveNFTs() public onlyOwner {
     uint totalMinted = _tokenIds.current();
     require(
        totalMinted.add(20) < MAX_SUPPLY, "Sveglia mammoccio, sono finiti"
     );
     for (uint i = 0; i < 20; i++) {
          _mintSingleNFT();
     }
...
function tokensOfOwner(address _owner) external view returns (uint[] memory) {
    ...
    //Funzione prelievo fondi contratto effettuabile solo da proprietario
function withdraw() public payable onlyOwner {
        uint balance = address(this).balance;
        require(balance > 0, "Non ci sta manco uno spiccio bucato");

        (bool success, ) = (msg.sender).call{value: balance}("");
        require(success, "Dio bastardo, prelievo fallito");
    }
}
