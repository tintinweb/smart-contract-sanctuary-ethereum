...
interface Kimono {
    function onMintingWithKimono(address _samurai, uint256 _id) external;

    function canClaimWithKimono(uint256 _id, address _samurai)
        external
        view
        returns (bool);
}
...
contract Katana is ERC165, IERC1155, IERC1155MetadataURI, Ownable {
    ...
    uint256 public constant FIRST_GIFTS = 50;
    ...
    function canClaimWithKimono(uint256 _id, address _samurai)
        ...
        returns (bool)
    {
        return Kimono(kimono).canClaimWithKimono(_id, _samurai);
    }

    function verifyClaim(address _account, bytes32[] calldata _merkleProof)
        public
        view
        returns (bool)
    {
    ...
    function claim(bytes32[] calldata _merkleProof) public returns (uint256) {
        require(tx.origin == msg.sender, "VC: We love People.");
        require(_paused == false, "VC: Minting is paused");
        require(
            verifyClaim(_msgSender(), _merkleProof),
        ...
        require(nextTokenId <= FIRST_GIFTS, "VC: No more rewards");
    ...
    function mintPublic(uint256 amount) public payable {
        require(tx.origin == msg.sender, "Vc: We love People.");
        if (
            Shiburai(shiburai).balanceOf(msg.sender) >= shiburaiDiscountAtAmount
        ) {
            require(msg.value == (price / 2) * amount, "Vc: Wrong mint price");
        } else {
            require(msg.value == price * amount, "Vc: Wrong mint price");
        }
        require(
