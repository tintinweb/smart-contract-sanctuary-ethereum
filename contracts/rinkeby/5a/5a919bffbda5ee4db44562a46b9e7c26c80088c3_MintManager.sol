    ...
    ERC1155Holder
{
    ...
    uint256 public constant PODTOWN_PERCENTAGE = 500;
    uint256 public constant base = 1e16;
    ...
    // TODO: whitelist
    // TODO: transfer bundle
    // TODO: multiple token

    function initialize(IMintNFT _rabby, IMintNFT _owl) public initializer {
        __Ownable_init();
        rabbyAddress = _rabby;
        owlAddress = _owl;
        bundles[0].index = 100;
    ...
    function mintByCustomToken(
        ...
        require(supportedToken[tokenIndex].initialized, "unsupported token");
    ...
    function withdraw() public payable onlyOwner {
        ...
        uint256 podtownBalance = (balance * PODTOWN_PERCENTAGE) / 1000;
        uint256 basedBalance = balance - podtownBalance;
        require(payable(msg.sender).send(basedBalance));
    }

    function calculatePriceBundle(uint256 id, uint8 tokenIndex)
        public
        view
        returns (uint256)
    {
    ...
    function calculatePrice(uint8 num, uint8 tokenIndex)
        public
        view
        returns (uint256)
    {
    ...
    function calculatePrices(uint256 quantity, uint8 tokenIndex)
        public
        view
        returns (uint256)
    {
        if (tokenIndex == 0) {
            return price * 1e18 * quantity;
            // return price * base;
        } else {
            return
                ((price * base) *
                    supportedToken[tokenIndex].conversionRate *
                    quantity) / 10**3;
        }
    }
