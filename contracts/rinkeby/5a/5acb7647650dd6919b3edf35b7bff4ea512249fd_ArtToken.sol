...
contract ArtToken is AccessControlUpgradeable, ERC721Upgradeable, ReentrancyGuardUpgradeable, IERC2981Upgradeable {
  ...
  bytes4 constant internal ERC1271_MAGICVALUE = bytes4(keccak256("isValidSignature(bytes32,bytes)"));
  ...
  bytes32 constant internal MINT_APPROVAL_TYPEHASH = keccak256(
      "MintApproval(uint tokenId,uint price,uint platformFee,uint galleryFee,uint royaltyFee,address artistAddress,address galleryAddress,address royaltyAddress)");

  function mintApprovalHash(MintApproval memory approval) public pure returns (bytes32) {
    return keccak256(abi.encode(
  ...
  function initialize() public initializer {
    ...
    _setRoleAdmin(ARTIST, AUTHORIZER);
  ...
  function mint(MintApproval calldata approval) external payable nonReentrant {
    ...
    require(hasRole(GALLERY, approval.galleryAddress), "mint: Gallery is not authorized.");
    require(hasRole(ARTIST, approval.artistAddress), "mint: Artist is not authorized.");
    require(ECDSAUpgradeable.recover(
            ECDSAUpgradeable.toTypedDataHash(eip712DomainSeparator(), hash),
            approval.gallerySignature) == approval.galleryAddress,
        "mint: Invalid gallery signature.");
    require(ECDSAUpgradeable.recover(
            ECDSAUpgradeable.toTypedDataHash(eip712DomainSeparator(), hash),
            approval.artistSignature) == approval.artistAddress,
        "mint: Invalid artist signature.");
    ...
    require(approval.platformFee + approval.galleryFee < approval.price,
    ...
    payable(approval.artistAddress).transfer(approval.price - approval.galleryFee - approval.platformFee);
    royaltyData[approval.tokenId] = RoyaltyInfo(approval.royaltyAddress, approval.royaltyFee);
  }

  function royaltyInfo(uint tokenId, uint salePrice) external view override returns (address receiver, uint royaltyAmount) {
  ...
  function tokenURI(uint tokenId) public view override returns (string memory) {
	...
	function supportsInterface(bytes4 interfaceId) public view virtual override(AccessControlUpgradeable, ERC721Upgradeable, IERC165Upgradeable) returns (bool) {
