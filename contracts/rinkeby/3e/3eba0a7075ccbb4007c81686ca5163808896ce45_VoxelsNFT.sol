  ...
  ReentrancyGuardUpgradeable
{
  ...
  // Merkle root 1 for any amount between 1-3
  bytes32 public amountMerkleRoot;
  mapping(address => uint) public whitelistRemaining;
  mapping(address => bool) public whitelistAmountUsed;

  // Merkle root 2 for selection 1
  bytes32 public selectionMerkleRoot;
  mapping(address => bool) public whitelistSelectionUsed;
  ...
  uint public whitelistAllocation;
  uint public mintAllocation;
  bool public whitelistUsable;
  bool public voxelsBurnable;
  ...
  uint public burnStartTimestamp;
  uint public burnDuration;
  ...
  modifier mintingStarted() {
    require(mintStartTimestamp != 0, "Mint timestamp not set");
    _;
  }

  modifier burningWorked() {
    require(burnStartTimestamp != 0, "Burn timestamp not set");
    require(voxelsBurnable, "Burn is not allowed");
    require(block.timestamp >= burnStartTimestamp, "Not burn open yet");
    require(
      block.timestamp <= burnStartTimestamp + burnDuration,
      "Burn closed"
    );
    _;
  }
  ...
  function initialize(
    ...
    mintAllocation = 1;
    whitelistUsable = false;
    voxelsBurnable = false;
    isPublic = false;
    singleBurnAllowed = false;
    stageAdvanceAllowed = false;
    burnDuration = 48 hours;
  ...
  function publicMint(uint amount)
    ...
    nonReentrant
  {
    require(isPublic, "Public mint is not allowed");
    // Check proper amount sent
    require(msg.value == amount * mintPrice, "Send proper ETH amount");
    _randomMint(msg.sender, amount, mintAllocation);
  }

  function whitelistAmountMint(
    uint amount,
    bytes32 leaf,
    bytes32[] memory proof
  ) external payable whenNotPaused mintingStarted nonReentrant {
    ...
    whitelistRemaining[msg.sender] -= amount;
    _randomMint(msg.sender, amount, whitelistAllocation);
  }

  function whitelistSelectionMint(bytes32 leaf, bytes32[] memory proof)
    external
    payable
    whenNotPaused
    mintingStarted
    nonReentrant
  {
    ...
    require(!whitelistSelectionUsed[msg.sender], "Already allocated");
    ...
    // Verify that (leaf, proof) matches the Merkle root
    require(
      verify(selectionMerkleRoot, leaf, proof),
      "Not a valid leaf in the Merkle tree"
    );

    // Check proper amount sent
    require(msg.value == mintPrice, "Send proper ETH amount");
    whitelistSelectionUsed[msg.sender] = true;
    _randomMint(msg.sender, 1, 1);
  }
  ...
  function genesisMint(uint256[] calldata tokenIds)
    ...
    whenNotPaused
  {
    require(_genesisTokenId <= MAX_NFT_SUPPLY, "genesisMint: Sold out");
    require(
  ...
  function singleBurn(uint tokenId) external burningWorked whenNotPaused {
  ...
  function startBurning() external onlyAdmin {
    burnStartTimestamp = block.timestamp;
    voxelsBurnable = true;
  }

  function endBurning() external onlyAdmin {
    burnStartTimestamp = 0;
    voxelsBurnable = false;
  }

  function setWhitelistUsable(bool _whitelistUsable) external onlyAdmin {
    whitelistUsable = _whitelistUsable;
  }

  function setIsPublic(bool _isPublic) external onlyAdmin {
    isPublic = _isPublic;
  }
  ...
  function setMintAllocation(uint _mintAllocation) external onlyAdmin {
    mintAllocation = _mintAllocation;
  }

  function setVoxelsBurnable(bool _voxelsBurnable) external onlyAdmin {
    voxelsBurnable = _voxelsBurnable;
  }
  ...
  function setMintStartTimestamp(uint _mintStartTimestamp) external onlyAdmin {
    mintStartTimestamp = _mintStartTimestamp;
  }

  function setBurnStartTimestamp(uint _burnStartTimestamp) external onlyAdmin {
    burnStartTimestamp = _burnStartTimestamp;
  }

  function setBurnDuration(uint _burnDuration) external onlyAdmin {
  ...
  function setAmountMerkleRoot(bytes32 _merkleRoot) public onlyAdmin {
    amountMerkleRoot = _merkleRoot;
  }

  function setSelectionMerkleRoot(bytes32 _merkleRoot) public onlyAdmin {
    selectionMerkleRoot = _merkleRoot;
  }
  ...
  function setPaymentSplitter(address[] memory payees, uint256[] memory shares)
    external
    onlyOwner
  {
