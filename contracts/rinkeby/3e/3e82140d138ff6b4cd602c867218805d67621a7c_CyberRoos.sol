...
contract CyberRoos is ERC721A, Ownable, ReentrancyGuard {
    ...
    uint256 public gamifiedCap = 600; //todo
    uint256 public saleCap = 500; //todo
    ...
    uint256 constant    EMMISSION_HALT_TIME             = 1655524773 + (1 days); //  midnight tn todo    mainnet         // 1742050800 1,000 days from launch. (03/15/2025 : 15:00 GMT)
    ...
    uint256 public hopsCostToBox = 5000 ether; // $HOPS
    uint256 public costToLock = 100000;

    
    address teamAllocationWallet =  0x5CF1C721c6C909B5990B0503251C1792Cd286E6A; // todo c1 //0xdD870fA1b7C4700F2BD7f44238821C26f7392148; // todo JVM last
    address discordDev = 0x07c47A72c65ce8A37622Ea8B15765dAD60163120; //todo
    address dev = 0x4E9f7618F72F3d497f4e252eBB6a731d715e7af5; // B7 todo ?
    address frontEndDev = 0x583031D1113aD414F02576BD6afaBfb302140225; // todo
    address public honoraryTeamMember = teamAllocationWallet; // default

    HOPS public hops;
    event HopsClaimed(address recipient, uint256 tokenId, uint256 amount);
    event RooBoxed(address fighter, address winner, uint256 id);

    constructor(address _hopsAddr) ERC721A("rCRoos", "rCROOS") { // todo
        hops = HOPS(_hopsAddr); 
        uint96 _currentTime = uint96(block.timestamp);
        baseURI = "https://cyberroos-next.vercel.app/api/"; // todo constructor mainnet

        // TEAMMEMBER ROOS (NON-TRADABLE & REVOCABLE)
        _mint(address(this), 10); // decrease batch size to save on gas down the road
        ...
        _mint(dev, 8); // decrease batch size to save on gas down the road
    ...
    function testMint(uint256 _amount) external { // todo remove
    ...
    function unlockFromGame(uint256[] calldata _tokenIds) external {
        ...
        require(saleState == 0);
        for (uint256 i = 0; i < _tokenIds.length; i++) {
            require(ownerOf(_tokenIds[i]) == tx.origin, "NOT YOUR ROO(S)");
    ...
    */
    ...
    function chooseHonoraryTeamMember() external onlyOwner returns(address) {
        address _winner = ownerOf(chooseTokenWeightedSequentially());
        honoraryTeamMember = _winner;

        return _winner; // CONGRATS!
    }

    function chooseTokenWeightedSequentially() public view returns(uint256) { // todo internal
        uint256 _totalMinted = _totalMinted();
        ...
        do {
            ...
            _attemptCount++;
        } while(_winningToken < 50);
        return _winningToken;
    }

    // HELPER

    // _upperBound is exclusive
    ...
    function setSaleCap(uint256 _newSaleCap) external onlyOwner {
        require(_newSaleCap < 501, "Sale Cap can't be increased");
        saleCap = _newSaleCap;
    }

    /**
     * @notice set gamified cap
     */
  ...
  function removeController(address controller) external onlyOwner {
    ...
    */
    ...
    function withdrawToTeam() external onlyOwner {
        ...
        sendEth(teamAllocationWallet, balance * 68 / 100);
        sendEth(dev, balance * 18 / 100);
        sendEth(discordDev, balance * 5 / 100);
