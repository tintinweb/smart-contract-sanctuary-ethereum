...
contract testErc is ERC20("TestErc", "ERC"), Ownable {
    ...
    function claimReward() public returns (uint256) {
        ...
        uint256 currentReward = unclaimedReward - stake[msg.sender].deposit_amount; 
        uint256 rewardToClaim =currentReward  - stake[msg.sender].claimedReward;
        ...
        if(stake[msg.sender].otherUserReferral != 0){
            uint256 referralUserPercentage = ((refeeRewardPercentage * currentReward) / 100) - stake[msg.sender].refreeClaimedReward;
    ...
    function stakeTokenForThreeMonths(uint256 amount, uint256 referralCode) public {
        ...
        if(stake[msg.sender].deposit_amount != 0 ){
           require(stake[msg.sender].returned == true,"tokens already staked");
        }
    ...
    function getUnclaimedRewardAmount(address user) public view returns (uint256) {
        ...
        return  computeUnclaimedReward(user) - stake[user].deposit_amount - stake[user].claimedReward;
        }else{
            return 0;
        }
    }
