...
contract NFTTPreSale is ERC721Enumerable, Ownable {
    ...
    uint256 public costPreSale = 200000000000000000;
    uint256 public maxSupply =2222;
    uint256 public maxWhitelist =999;
    uint256 countWhitelisted = 0;
    uint256 public maxSupplyPresale = 3333;
    mapping(address=>bool) public whitelistedAdresses;
    bool onlyWhitelisted = false;
    ...
    function mint(uint256 _mintAmount) public payable {
        ...
        if (msg.sender != owner()) {
            if(onlyWhitelisted) {
                ...
                require(msg.value >= costPreSale * _mintAmount,"not enough value");
            }else{
    ...
    function addWhitelistUsersOrUpdate(address[] memory _users,bool whitelisted) public onlyOwner() {
        require(_users.length + countWhitelisted<=maxWhitelist,"Whitelist is greater than the maximum");
        for(uint256 i = 0; i< _users.length;i++){
