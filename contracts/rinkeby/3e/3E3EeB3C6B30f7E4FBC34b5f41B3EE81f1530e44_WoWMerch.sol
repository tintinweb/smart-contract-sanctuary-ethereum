...
abstract contract MigrateTokenContract {
    function mintTransfer(address to, uint256 tokenId, uint256 amount) external virtual;
}

/// @title WoW Merch contract
/// @author World of Women
contract WoWMerch is
    ERC1155,
    ERC1155Burnable,
    Ownable
{
    mapping (uint => address) private authorizedContracts;
    mapping (uint => bool) private migrationActivePerToken;
    mapping (uint => bool) private lockedTokens;
    ...
    function airdropToken(uint256 tokenId, uint256[] calldata amount, address[] calldata dropAddresses) external onlyOwner {
        require (!lockedTokens[tokenId], "Token locked");
    ...
    function toggleAuthorizedContractForToken(uint256 tokenId, address contractAddress) external onlyOwner {
        authorizedContracts[tokenId] = contractAddress;
    }
    ...
    function toggleMigrationForToken(uint256 tokenId) external onlyOwner {
        require(authorizedContracts[tokenId] != address(0), "Authorized Contract not set");
        migrationActivePerToken[tokenId] = !migrationActivePerToken[tokenId];
    }
    ...
    function migrateTokens(uint256 tokenId, uint256 amount) external {
        require(migrationActivePerToken[tokenId], "Migration is not active");
        require(balanceOf(msg.sender, tokenId) >= amount, "Doesn't own that amount of token");

        burn(msg.sender, tokenId, amount); // Burn

        MigrateTokenContract migrationContract = MigrateTokenContract(authorizedContracts[tokenId]);
        migrationContract.mintTransfer(msg.sender, tokenId, amount); // Mint ... What?
    }

    /// @param newuri Set the global uri for all the tokens, following the standard `newuri/${id}`
