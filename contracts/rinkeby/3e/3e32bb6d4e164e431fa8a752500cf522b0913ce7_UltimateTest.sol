...
// File: contracts/UltimateTest.sol
...
contract UltimateTest is ERC721A, Ownable, ReentrancyGuard {
    using Strings for uint256;

    uint256 public constant MAX_SUPPLY = 10;
    uint256 public constant MAX_FREE_MINTS = 2;
    uint256 public constant MAX_MINTS_WALLET = 10;
    uint256 public constant MAX_FREE_MINTS_WALLET = 1;
    ...
    constructor(string memory tokenData, string memory contractData) ERC721A("Ultimate Test", "ULTE") {
        _baseTokenURI = tokenData;
        _contractURI = contractData;
    }    
    ...
    function togglePresaleOpen() external onlyOwner {
    ...
    function publicMint(uint256 quantity) external payable callerIsUser nonReentrant {
        ...
        require(totalSupply() + quantity <= MAX_SUPPLY, "Max supply reached, try minting less");
        require((totalMints[msg.sender] + quantity) <= MAX_MINTS_WALLET, "Exceeds max mints per Wallet");
        require(msg.value >= (MINT_PRICE * quantity), "Insufficient funds");

        _safeMint(msg.sender, quantity);

        totalMints[msg.sender] += quantity;
    }

    function freeMint() external callerIsUser nonReentrant {
        require(isPresaleOpen, "Presale not open");
        require(MAX_SUPPLY > totalSupply(), "Sold out");
        require(totalSupply() + 1 <= MAX_FREE_MINTS, "Presale not open");
        require((totalFreeMints[msg.sender]) < MAX_FREE_MINTS_WALLET, "Only one free mint per Wallet");
    ...
    // @dev adjusts tokenURI for .json extension
