...
contract Proxy is AccessControl, Ownable, ReentrancyGuard, Structs {
    ...
    ///     mint, send [0, 0]
    function mint(uint256[] memory _cm21tokenIds) public payable nonReentrant {
        ...
        if (_phase.maxPerWallet > 0) {
            ...
            require(_quantity <= 4, "Max Cryochambers to mint per transaction is 4");
        }

        // Calculate price
        uint256 _price = getPrice(_timeNow) * _quantity;
        require(msg.value == _price, "ETH sent does not match Cryochamber(s) value");
