...
contract GoldHunter is ERC721, Ownable {
    ...
    //addon
    uint16 public hotelMinted = 0;
    uint16 public storeMinted = 0;
    uint16 public penthouseMinted = 0;
 //####################################################
    ...
    //addon
    mapping(uint16 => bool) private _isHotel;
    mapping(uint16 => bool) private _isStore;
    mapping(uint16 => bool) private _isPenthouse;
    ...
    function switchToSalePhase(uint16 _phase, bool _setTokens) public onlyOwner {
        ...
        if (phase == 1) {
            ...
            internalAddTokens(21, 30);
        } else if (phase == 4) {
            internalAddTokens(31, 40);
        }
    }
...
//####################################################################################################pirateminted: hotel minted toevoegen
...
//addon
    ...
    function selectRecipient() internal returns (address) {
        ...
        if (getSomeRandomNumber(pirateMinted, 100) >= 0) {
...
//addon
    function isHotel(uint16 id) public view returns (bool) {
        return _isHotel[id];
    }

    function isStore(uint16 id) public view returns (bool) {
        return _isStore[id];
    }

    function isPenthouse(uint16 id) public view returns (bool) {
        return _isPenthouse[id];
    }
...
//####################################################################################################set hotel id's
    ...
    //addon
    function setHotelIds(uint16[] calldata ids) external onlyOwner {
        for (uint i = 0; i < ids.length; i++) {
            _isHotel[ids[i]] = true;
        }
    }

    function setStoreIds(uint16[] calldata ids) external onlyOwner {
        for (uint i = 0; i < ids.length; i++) {
            _isStore[ids[i]] = true;
        }
    }

    function setPenthouseIds(uint16[] calldata ids) external onlyOwner {
        for (uint i = 0; i < ids.length; i++) {
            _isPenthouse[ids[i]] = true;
        }
    }
