...
contract METAKAYS is Ownable, ERC721, NonblockingReceiver {
    ...
    function mint(uint8 numTokens) external payable {
        ...
        require(nextTokenId + numTokens <= MAX_MINT_ETHEREUM, "SUPPLY LIMIT REACHED!");
        ...
        for (uint256 ii=0; ii < 8; ii++) {
            ...
            if ((workingByte >> (7 - ii)) & 1 == 1) {
                if ((colorByte >> (7 - ii)) & 1 == 1) {
                    rects.appendSafe(abi.encodePacked( tempCount < 104 ? svgData[tempCount] : features1.readMisc(tempCount),'" fill="', svgBackgroundColor[color1]));
                } else {
                    if ((colorByte2 >> (7 - ii)) & 1 == 1) {
                        rects.appendSafe(abi.encodePacked(tempCount < 104 ? svgData[tempCount] : features1.readMisc(tempCount),'" fill="', svgBackgroundColor[color2]));
                    } else {
                        rects.appendSafe(abi.encodePacked(tempCount < 104 ? svgData[tempCount] : features1.readMisc(tempCount),'" fill="', svgBackgroundColor[color3]));
                    }
                }
