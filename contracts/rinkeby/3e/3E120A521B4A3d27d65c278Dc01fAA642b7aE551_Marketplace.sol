...
contract Marketplace is IMarketplace, Ownable {
	...
	bytes32[] public _offerlist;
	mapping(bytes32 => uint) private _offerIndex;
	...
	function sendOffer(
		...
		_offerlist.push(_generatedId);
		_offerIndex[_generatedId] = _offerlist.length - 1;
	...
	function cancelOffer(bytes32 _offerId) public onlyAdvisor(msg.sender) {
		...
		_offerlist[_offerIndex[_offerId]] = _offerlist[_offerlist.length - 1];
		_offerIndex[_offerlist[_offerlist.length - 1]] = _offerIndex[_offerId];
		delete _offerIndex[_offerId];
		delete _offerlist[_offerlist.length - 1];
	}
	...
	function offerList() public view returns(bytes32[] memory) {
		return _offerlist;
	}
}
