...
contract NarratorsHut is INarratorsHut, ERC721Hut, IERC2981, Ownable {
    ...
    bytes32 private immutable MINT_DOMAIN_SEPARATOR;
    ...
    bytes private constant TOKEN_DATA_TYPE_DEF =
    ...
    bytes32 private constant TOKEN_DATA_TYPEHASH =
        keccak256(TOKEN_DATA_TYPE_DEF);
        ...
        ERC721Hut("The Narrator's Hut", "HUT")
    {
        ...
        MINT_DOMAIN_SEPARATOR = keccak256(
            abi.encode(
                DOMAIN_TYPEHASH,
    ...
    modifier canMintArtifact(TokenData calldata tokenData) {
        if (getHasMinted(tokenData.artifactId, tokenData.witchId)) {
    ...
    {
        mintArtifacts(mintInput.tokenDataArray);
    }
    ...
    function hasMinted(uint32 artifactId, uint32 witchId)
        external
        view
        returns (bool)
    {
        return getHasMinted(artifactId, witchId);
    }
    ...
    function getHasMintedKey(uint32 artifactId, uint32 witchId)
        private
        view
        returns (bytes32)
    {
        ...
        if (witchId != 0) {
            ...
            // minted the specified artifact type
            return bytes32(abi.encodePacked(msg.sender, artifactId));
        }
    }

    function getHasMinted(uint32 artifactId, uint32 witchId)
        private
        view
        returns (bool)
    {
        bytes32 hasMintedKey = getHasMintedKey(artifactId, witchId);
        return _hasMinted[hasMintedKey];
    }

    function mintArtifacts(TokenData[] calldata tokenDataArray) private {
        for (uint256 i; i < tokenDataArray.length; ) {
            mintArtifact(tokenDataArray[i]);
            unchecked {
                ++i;
            }
        }
        ...
        canMintArtifact(tokenData)
    {
        ...
        // given artifact type
        bytes32 hasMintedKey = getHasMintedKey(
            tokenData.artifactId,
            tokenData.witchId
        );
        _hasMinted[hasMintedKey] = true;
    ...
    function isApprovedForAll(address owner, address operator)
        ...
        returns (bool)
    {
        // NOTE: Now that OpenSea has migrated to Seaport, orders are now routed through
        // a PausableZone that they control - in the event of a serious vulnerability,
        // they can pause the zone to prevent orders from being fulfilled while Seaport
        // is patched and liquidity is migrated:
        // https://etherscan.io/address/0x44dc42076eca1b8e35b222a54b04daa333ce1f63
        // For this reason we may not need the extra SLOAD and having to manually
        // deactivate the OpenSea conduit, if the goal is to prevent holders against
        // a marketplace vulnerability.
        if (
...
library Signatures {
    ...
    function recreateMintHash(
        bytes32 MINT_DOMAIN_SEPARATOR,
        bytes32 MINT_TYPEHASH,
        bytes32 TOKEN_DATA_TYPEHASH,
        ...
        return _eip712Message(MINT_DOMAIN_SEPARATOR, mintHash);
    }
    ...
    function _hashMint(
        ...
        for (uint256 i; i < tokenDataArray.length; ) {
            tokenDataHashes[i] = _hashTokenData(
                TOKEN_DATA_TYPEHASH,
                tokenDataArray[i]
            );
            unchecked {
...
struct TokenData {
    ...
    uint32 witchId;
    uint32 artifactId;
    uint32 preassignedAttributeIndex; // 1-based index, 0 is treated as null
}
