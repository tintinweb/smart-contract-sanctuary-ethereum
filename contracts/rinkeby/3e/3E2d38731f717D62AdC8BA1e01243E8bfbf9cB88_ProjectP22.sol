...
interface IVestedRandomness { function receiveRandomness(uint256 requestId, uint256 randomNumber) external; }
interface VestedRandomnessContract { function getRandomNumber(address senderAddress, uint256 _modulus) external returns (uint256); }
...
contract ProjectP22 is ERC1155Upgradeable, OwnableUpgradeable, IVestedRandomness {
    ...
    mapping(uint256 => AsyncRandomnessProviderRequest) private asyncRandomnessProviderRequestMapping;
    ...
    function uri(uint256 id) public view override returns (string memory) {
        require(PackDetails[uint32(id)].Active, "URI requested for invalid pack");
        return
