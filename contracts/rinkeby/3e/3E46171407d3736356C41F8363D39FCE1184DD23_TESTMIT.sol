...
contract TESTMIT is ERC721, Ownable{
    ...
    uint256 limit_A_whitelist = 1730000000000000000;
    uint256 limit_A_special = 380000000000000000000;
    uint256 limit_A_normal = 480000000000000000000;

    uint256 limit_B_whitelist = 20000000000000000; 
    uint256 limit_B_special = 150000000000000000;
    uint256 limit_B_normal = 200000000000000000;
    ...
    )ERC721("Test Mit collection","MIT") payable {
    ...
    function multiTokensTransfer(address _to, uint _type, uint _amount, uint _counter, uint256 _cost) public payable{
        if(_type == 1){
            if(onlyWhitelisted == true){
                require(msg.value >= limit_A_whitelist * _amount, "You dont have enough balance");
            }else if(_counter < specialSaleSuply){
                require(msg.value >= limit_A_special * _amount, "You dont have enough balance");
            }else if(_counter >= specialSaleSuply){
                require(msg.value >= limit_A_normal * _amount, "You dont have enough balance");
            }

        }else if (_type == 2){
            if(onlyWhitelisted == true){
                require(_cost >= limit_B_whitelist);
                uint256 precio = _cost * _amount;
                IERC20(tokenContract).transferFrom(msg.sender, _to, precio);
            }else if(_counter < specialSaleSuply){
                require(_cost >= limit_B_special);
                uint256 precio = _cost * _amount;
                IERC20(tokenContract).transferFrom(msg.sender, _to, precio);
            }else if(_counter >= specialSaleSuply){
                require(_cost >= limit_B_normal);
                uint256 precio = _cost * _amount;
                IERC20(tokenContract).transferFrom(msg.sender, _to, precio);
            }
        }
   ...
   function tokenURI(uint256 _tokenId) public view virtual override returns (string memory)
  {
    ...
    function Activar_Mint()public onlyOwner returns(bool){
        activar = !activar;
        return true;
    }
    ...
    function Apagar_Whitelist() public onlyOwner{
    ...
    // SET SUPPLY in TOKEN A

    function set_A_Supply (uint256 _special, uint256 _normal, uint256 _whitelist) public onlyOwner returns(bool){
        
        limit_A_special = _special;
        limit_A_normal =  _normal;
        limit_A_whitelist = _whitelist;
        return true;
    }

    function set_B_supply(uint256 _special, uint256 _normal, uint256 _whitelist) public onlyOwner returns(bool){
        
        limit_B_special = _special;
        limit_B_normal = _normal;
        limit_B_whitelist = _whitelist;
        return true;
    }
