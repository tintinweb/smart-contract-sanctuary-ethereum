...
contract SpaceICO {
    ...
    // The amount ether in total claimable for SEED and GENERAL phases
    uint256 public preOpenContributionsClaimable;
    ...
    constructor(address _owner, address _treasury) {
        owner = _owner;
        spaceCoin = new SpaceCoin(_owner, _treasury, address(this), 500_000);
    ...
    function claimTokens() external isPhase(Phase.OPEN) {
        ...
        preOpenContributionsClaimable -= amountToClaim;
        spaceCoin.mint(msg.sender, amountToClaim);
    }
    
    /**
     * @notice Mints any remainder up to 500,000 SPCE to treasury
     */
    function mintRemainderToTreasury() external onlyOwner {
        uint256 amountToMint = spaceCoin.maxSupply() - (spaceCoin.totalSupply() + preOpenContributionsClaimable);
        spaceCoin.mint(spaceCoin.treasury(), amountToMint);
    }
     ...
     * Before the OPEN phase, ether is just tracked as a claimable. Otherwise, mint instantly
     */
    function purchase() external payable isWhitelisted {
        ...
        if (spaceCoin.totalSupply() + preOpenContributionsClaimable + msg.value > getPhaseLimitTotal()) revert PhaseLimitHit();
        
        if (phase < uint256(Phase.OPEN)) {
            preOpenContributions[msg.sender] += msg.value;
            preOpenContributionsClaimable += msg.value;
        } else {
            spaceCoin.mint(msg.sender, msg.value);
        }
    }
...
contract SpaceCoin is ERC20 {
    // The owner of this contract. Allows certain rights during ICO process
    address private immutable owner;

    // The address of the SPCE minter. Only this address can run mint(). 
    // Expected to be SpaceCoinICO contract
    address private immutable minter;

    // The treasury address. Used to deposit tax (if enabled)
    address public immutable treasury;

    // Max supply of SpaceCoin mintable
    uint256 public immutable maxSupply; // 500,000

    // Percentage amount to tax
    ...
    event Contributed();
    event Minted();
    event TransferTaxChanged(uint8);
    event TransferTaxApplied(uint256 amount);

    constructor(address _owner, address _treasury, address _minter, uint256 _maxSupply) ERC20("SpaceCoin", "SPCE") {
        owner = _owner;
        treasury = _treasury;
        minter = _minter;
    ...
    function toggleTransferTax() external onlyOwner {
        transferTaxPercentage = transferTaxPercentage == 0 ? 2 : 0;
    ...
    function _afterTokenTransfer(address from, address to, uint256 amount) internal virtual override {
        // Done as best practice according to OZ. Does not do anything.
