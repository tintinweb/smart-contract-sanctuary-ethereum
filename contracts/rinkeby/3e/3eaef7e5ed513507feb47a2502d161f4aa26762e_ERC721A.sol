...
contract ERC721A is Context, ERC165, IERC721, IERC721Metadata, IERC721Enumerable, Ownable {
    ...
    //  Init values for price, max supply, max mint amount, and allowlist mapping
    ...
    uint256 public immutable maxMintAmountWhitelist = 2;
    ...
    // The tokenId of the next token to be mintet and number of tokens burned.
    ...
    */
    ...
    constructor(
        ...
        // set time for public launch or use setPublicSaleStartTime() to set later
        setPublicSaleStartTime(_timestamp);
    ...
    function publicMint(uint256 _mintAmount) public payable mintCompliance(_mintAmount) {
        require(_mintAmount > 0 && _mintAmount + balanceOf(msg.sender) <= maxMintAmountPublic, "Invalid mint amount!");
    ...
    function allowlistMint(uint256 _mintAmount) public payable mintCompliance(_mintAmount){
        require(_mintAmount > 0 && _mintAmount + balanceOf(msg.sender) <= maxMintAmountWhitelist, "Invalid mint amount!");
        require(allowlist[msg.sender] > 0, "not eligible for allowlist mint or you maxed out your mint");
    ...
    //  add mapping of whitelisted addresses
    function seedAllowlist(address[] memory addresses) public onlyOwner{
        for (uint256 i = 0; i < addresses.length; i++) {
        allowlist[addresses[i]] = maxMintAmountWhitelist;
        }
    }
    ...
    //get number minted of each user to make sure can't mint over max
