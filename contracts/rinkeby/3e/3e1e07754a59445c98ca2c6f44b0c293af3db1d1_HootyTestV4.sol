...
contract HootyTestV4 is ERC721A, Ownable, ReentrancyGuard {
  ...
  modifier mintQuantityCheck(uint256 _mintQuantity) {
    require(_mintQuantity > 0 && _mintQuantity <= maxMintQuantityPerTx, 'Invalid mint amount!');
  ...
  modifier promoMintPriceCheck(uint256 _mintQuantity) {
    require(msg.value >= promoCost * _mintQuantity, 'Insufficient funds!');
    _;
  }

  function promoMint(uint256 _mintQuantity) public payable promoMintPaused mintQuantityCheck(_mintQuantity) nonReentrant {
    bool promoCondition = false;
    if (true) { //check merkleroot for freemint
        promoCondition = true;
        require(msg.value >= promoCost * _mintQuantity - 1, 'Insufficient funds!'); // 1 freemint allowed
    }

    if (false) { //check merkleroot for WL
        promoCondition = true;
        require(msg.value >= promoCost * _mintQuantity, 'Insufficient funds!');
    }

    require(promoCondition, 'No Promotion Condition Eligible');

    numberOfMintsOnAddress[msg.sender] += _mintQuantity;
    _safeMint(_msgSender(), _mintQuantity);
  }

//   function whitelistMint(uint256 _mintQuantity, bytes32[] calldata _merkleProof) public payable mintQuantityCheck(_mintQuantity) mintPriceCheck(_mintQuantity) nonReentrant {
//     require(whitelistMintEnabled, 'The whitelist sale is not enabled!');
//     require(numberOfMintsOnAddress[msg.sender] + _mintQuantity <= maxMintQuantityPerAddress, "Sender is trying to mint more than their whitelist amount");
...
//   }
  ...
  function mintForAddress(uint256 _mintQuantity, address _receiver) public mintQuantityCheck(_mintQuantity) onlyOwner {
  ...
  function setPromoCost(uint256 _promoCost) public onlyOwner {
    promoCost = _promoCost;
  }

  function setmaxMintQuantityPerTx(uint256 _maxMintQuantityPerTx) public onlyOwner {
    maxMintQuantityPerTx = _maxMintQuantityPerTx;
  }

  function setmaxMintQuantityPerAddress(uint256 _maxMintQuantityPerAddress) public onlyOwner {
    maxMintQuantityPerAddress = _maxMintQuantityPerAddress;
  }
  ...
  function setPromoPaused(bool _state) public onlyOwner {
    promoPaused = _state;
  }
