  ...
  UUPSUpgradeable
{
  ...
  function createIndexFund(
    ...
    require(_dex != address(0),"dex address is 0");
    ...
    address indexAddress = Clones.cloneDeterministic(implementation, salt);
    IndexInterface(indexAddress).initialize(_percentages,_tokens,_blocks,_ptoken,_dex);
    
    indexAddressArray.push(indexAddress);
    emit CreateIndexFund(msg.sender,id,_percentages,_tokens,_blocks,_dex);
  ...
  function updateIndexFund(
   ...
   IndexInterface(indexAddressArray[id]).udpateindex(_percentages,_tokens);
emit UpdateIndexFund(msg.sender,id,_percentages,_tokens);
  ...
  function getIndexInfo(uint256 id)
    ...
    returns (
      ...
      uint16[] memory percentage,
      address indexaddress
    )
  {
    (tokens, percentage,indexaddress) = IndexInterface(indexAddressArray[id]).getCurrentIndexInfo();
  ...
  function updateIndexOwner(address newOwner,uint id)external onlyOwner{
