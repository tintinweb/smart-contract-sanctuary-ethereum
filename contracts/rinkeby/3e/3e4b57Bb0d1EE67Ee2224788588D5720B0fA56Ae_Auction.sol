...
contract Auction is Ownable {
    ...
    event OnSetAuction (address _token);
    ...
    function buy (
        ...
        require ( tokenPrice(_token) <= (_busdAmount * 10 ** 18 / _tokenAmount) * (_slippage + 10000) / 10000, "Slippage too high");
        ...
        uint256 boughtAmount = _busdAmount * 10**18 / newPrice;
    ...
    */
    ...
    function setAuction (
        ...
        if (_startTime == 0 && _endTime == 0) {
            ...
            _endTime = uint32(block.timestamp) + 36000;
        } else {
        ...
        emit OnSetAuction(_token);
    }

    function setSoldUSD (address _token, uint256 _soldUSD) public onlyOwner {
        token[_token].soldUSD = _soldUSD;
    }

    function setSoldAmount (address _token, uint256 _soldAmount) public onlyOwner {
        token[_token].soldAmount = _soldAmount;
    }
    ...
    function claimFund (
        ...
        IERC20(BUSD).safeTransfer(_msgSender(), token[_token].soldUSD);
    ...
    function tokenPrice (
        ...
        uint256 _lastPrice = (token[_token].endTime - currentTime) * 10 ** 18 / (token[_token].endTime - token[_token].startTime) * ( token[_token].lastPrice - token[_token].bottomPrice ) / 10 ** 18 + token[_token].bottomPrice;
        return _lastPrice;
    }
