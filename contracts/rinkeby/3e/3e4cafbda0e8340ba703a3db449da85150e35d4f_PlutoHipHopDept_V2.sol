...
contract PlutoHipHopDept_V2 is ERC721Enumerable, Ownable {
  ...
  PlutoHipHopDept private PHHD = PlutoHipHopDept(0xC548a42CCe5dAF3866Bd11CA2Ea4895091141a80);
  string public baseURI;
  string public baseExtension = ".json";
  uint256 public maxSupply = 3333;
  uint256 public maxSaleNum = 3300;
  uint256 public v2Index = PHHD.totalSupply() + 1;
  ...
  function presaleMint(uint256 _mintAmount) public payable {
    ...
    require(supply + _mintAmount <= maxSaleNum, "max NFT limit exceeded");
    require(_mintAmount <= presaleLimit, "only one NFT per wallet in presale");
    require(msg.value >= presalePrice * _mintAmount, "insufficient funds");
    require(isWhitelisted(msg.sender), "user is not whitelisted");

    uint256 ownerMintedCount = addressMintedBalance[msg.sender];
    require(ownerMintedCount + _mintAmount <= presaleLimit, "max NFT per address exceeded");
    
    for (uint256 i = 1; i <= _mintAmount; i++) {
        addressMintedBalance[msg.sender]++;
        uint mintIndex = v2Index;
  ...
  function publicMint(uint256 _mintAmount) public payable {
    ...
    require(_mintAmount <= txLimit, "max mint amount per session exceeded");
    require(supply + _mintAmount <= maxSaleNum, "max NFT limit exceeded");
