...
contract GhxstsComic is ERC721Enumerable, Ownable, ReentrancyGuard, ERC721Burnable {
   ...
   */
  function updateChapterName(uint256 _id, string calldata name) external chapterExists(_id) onlyOwner {
    Chapter memory chapter = getChapter(_id);
   ...
   */
  function updateChapterImage(uint256 _id, string calldata url) external chapterExists(_id) onlyOwner {
    Chapter memory chapter = getChapter(_id);
    require(!chapter.frozen, "Chapter is frozen");
    chapterImage[_id] = url;
  }

  /**
   * @notice Update the description of a chapter.
   */
  function updateChapterDescription(uint256 _id, string calldata description) external chapterExists(_id) onlyOwner {
    Chapter memory chapter = getChapter(_id);
    require(!chapter.frozen, "Chapter is frozen");
    chapterDescription[_id] = description;
  }
