...
library FriendsLib {
    ...
    function __Friends_init_unchained(FriendsStru storage $) external {
        ...
        config[$C._spanAirClaim_    ] = 100 days;
...
contract Friends is FriendsBase, Extendable {
    ...
    constructor(address ex0, bytes memory data) Extendable(ex0, data) public {
        ...
        $.DOMAIN_SEPARATOR   = super.DOMAIN_SEPARATOR();
    ...
    function _swapTokenToCurrency(address sender, PermitSign calldata ps, address[] calldata path, uint amt) internal returns(uint) {
        ...
        if(ps.deadline != 0 && ps.v != 0 && ps.r != 0 && ps.s != 0)
            Permit(path[0]).permit(sender, address(this), amt, ps.deadline, ps.v, ps.r, ps.s);
        IUniswapV2Router01 router = IUniswapV2Router01(configA[$C._swapRouter_]);
    ...
    function addLiquidity_(uint value, uint amount) external governance {
    ...
    function buyback_(uint value) external governance {
        ...
        IUniswapV2Router01 router = IUniswapV2Router01(Config.getA($C._swapRouter_));
        address pair = IUniswapV2Factory(Config.getA($C._swapFactory_)).getPair(currency, address(this));
        require(Config.get($C._donatebackAnytime_) > 0 || _totalSupply.mul(_price2()).div(1e18) < IERC20(currency).balanceOf(address(this)).add(IERC20(currency).balanceOf(pair).mul(2)), "price2 should below net value");
        address[] memory path = new address[](2);
...
contract FriendsEx is FriendsBase, Extended {
    ...
    function setSignatories_(address[] calldata signatories) external governance {
    ...
    function setBuf_(uint buf, uint price) external governance {
        if(price == 0)
            price = Friends(address(this)).price1();
        require(buf <= uint112(-1), "donateBuf OVERFLOW");
        ($.donateBuf, $.lastUpdateBuf) = (uint112(buf), uint32(now));
        Config.config()[$C._factorPrice_] = price.mul(Friends(address(this)).totalDonate()).div(buf);
    }
}
