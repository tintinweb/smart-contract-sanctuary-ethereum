...
contract BaseZooFunctions is Ownable, VRFConsumerBase
{
	...
	function setNftBattleArena(address _nftBattleArena, address listingList, address owner) external onlyOwner {
	...
	function computeVotesByDai(uint256 amount, address collection) external view returns (uint256 votes)
	{
		uint256 epochIndex = list.getEpochNumber(block.timestamp);
		uint256 teamBonus = amount * list.poolWeight(collection, epochIndex) / list.poolWeight(address(0), epochIndex);

		if (block.timestamp < battles.epochStartDate() + battles.firstStageDuration() + battles.secondStageDuration() / 3)
		{
			votes = (amount + teamBonus) * 13 / 10;                                          // 1.3 multiplier for votes.
		}
		else if (block.timestamp < battles.epochStartDate() + battles.firstStageDuration() + ((battles.secondStageDuration() * 2) / 3))
		{
			votes = amount + teamBonus;                                                          // 1.0 multiplier for votes.
		}
		else
		{
			votes = (amount + teamBonus) * 7 / 10;                                           // 0.7 multiplier for votes.
		}
	}
