...
/*
.___  ___.  _______ .______       __   _______   __       ___      .__   __. 
|   \/   | |   ____||   _  \     |  | |       \ |  |     /   \     |  \ |  | 
|  \  /  | |  |__   |  |_)  |    |  | |  .--.  ||  |    /  ^  \    |   \|  | 
|  |\/|  | |   __|  |      /     |  | |  |  |  ||  |   /  /_\  \   |  . `  | 
|  |  |  | |  |____ |  |\  \----.|  | |  '--'  ||  |  /  _____  \  |  |\   | 
|__|  |__| |_______|| _| `._____||__| |_______/ |__| /__/     \__\ |__| \__|
...
error MintExceedsMaxTechSupply();
error MintExceedsMaxMedicSupply();
error MintExceedsMaxWarriorSupply();
    ...
    ,INonDilutive
{
    ...
    address signer = 0x008250f59C91AeDf499002F50C2508b4251f005F;
     ...
     */
    function toggleWhitelistMint() public virtual onlyOwner { 
     ...
     *      start your sale through Flashbots so that bots cannot backrun you.
     */
    function togglePublicMint() public virtual onlyOwner { 
        publicMintOpen = !publicMintOpen;
    }

    function isMessageValid(bytes memory _signature, uint256 quantity) public virtual
    {

        // Validate signature 
        if (signer != keccak256(abi.encodePacked(address(this), msg.sender)
            ).toEthSignedMessageHash().recover(_signature)) revert SignatureNotValid();
        
        require(_numberMinted(msg.sender) + quantity < 4, "Max mint 3 per wallet");
    }

    //Figure out identifiers
    function updateBaseURI(string memory newBaseUri, uint256 _layerId) public virtual onlyOwner {
        generations[_layerId].baseURI = newBaseUri;
    }

    /**
     * @notice The whitelist minting function of this contract while making sure that
     *         supply is not exceeded and the proper $$ has been supplied.
     */
    function whitelistMint(uint256 quantity) public virtual payable {
        if(tx.origin != msg.sender) revert NoContracts();
        if(!whitelistMintOpen) revert WhitelistMintNotEnabled();
        if(msg.value != price * quantity) revert MintCostMismatch();
        if(totalSupply() + quantity > MAX_SUPPLY) revert MintExceedsMaxSupply();
        require(_numberMinted(msg.sender) + quantity < 4, "Max mint 3 per wallet"); //use 4
    ...
    {
        if(tx.origin != msg.sender) revert NoContracts();
        if(!publicMintOpen) revert PublicMintNotEnabled();
        if(msg.value != price * quantity) revert MintCostMismatch();
        if(totalSupply() + quantity > MAX_SUPPLY) revert MintExceedsMaxSupply();
        require(_numberMinted(msg.sender) + quantity < 4, "Max mint 3 per wallet"); //use 4
        _mint(msg.sender, quantity, '', true);
    ...
    // Team mint
    function batchReserve(address[] memory targets, uint256[] memory quantities) 
        external 
        onlyOwner 
    {
        for (uint256 i = 0; i < targets.length; i++) {
            _mint(targets[i], quantities[i], '', true);
        }
    }
    ...
    /**
     * @notice Withdraws the money from this contract to the owner.
     */
    function withdraw() 
        public 
        payable 
    ...
    ,Ownable
{
    ...
    { 
        ...
        if (generationTokenId + 1 > MAX_SUPPLY) {
            generationTokenId = generationTokenId - MAX_SUPPLY;
        }
    }
