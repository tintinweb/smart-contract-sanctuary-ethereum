...
contract CyboGenerator is ERC721, ERC721URIStorage, Pausable, Ownable {

    using Counters for Counters.Counter;
    using SafeMath for uint256;
    string private _baseURIPrefix;
    uint256 private constant nftsPublicNumber = 8788;
    uint256 private tokenPrice = 1 * 10 ** 16; //Cost set to 0.1 ETH
    uint private constant maxTokensPerTransaction = 25;
    address private constant withdrawAddress = 0x0EF2Cd4072329C6f27DE1E0D90A5539473c11822;
    ...
    constructor() ERC721("Cyboverse", "CYBO") {
    ...
    function buyPreSaleCybo(uint tokensNumber) public payable {
        ...
        require(tokenPrice.mul(tokensNumber) <= msg.value, "Not enough sent ETH, price is 0.1 per token");
    ...
    function buyWhitelistCybo(uint tokensNumber) public payable {
    ...
    function mintGiveawayCybos(address to, uint256 tokenId) public onlyOwner {
        require(tokenId > nftsPublicNumber, "Sale has not ended yet!");
        _safeMint(to, tokenId);
        _tokenIdCounter.increment();
    }

    function buyCybos(uint tokensNumber) public payable {
