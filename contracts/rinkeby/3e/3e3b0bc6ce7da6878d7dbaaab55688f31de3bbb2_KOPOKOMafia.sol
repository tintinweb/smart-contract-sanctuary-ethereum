...
contract KOPOKOMafia is ERC721Enumerable, PaymentSplitter, Ownable, ReentrancyGuard {
    ...
    constructor(string memory _theBaseURI, string memory _notRevealedURI, bytes32 _merkleRoot) ERC721("KPKPROJECT I DROPX00X", "KPD") PaymentSplitter(_team, _teamShares) {
    ...
    **/
    function presaleMint(address account, bytes32[] calldata _proof) external payable nonReentrant {
        //Are we in Presale ?
        require(sellingStep == Steps.Presale, "Presale has not started yet.");
        //Did this account already mint an NFT ?
        require(nftsPerWallet[account] < max_mint_allowed_presale, string(abi.encodePacked("You can only get ",
        Strings.toString(max_mint_allowed_presale)," NFT on the Presale")));
        //Is this user on the whitelist ?
        require(isWhiteListed(account, _proof), "You are not on the whitelist");
    ...
    **/
    function saleMint(address account, uint256 _ammount) external payable nonReentrant {
        ...
        nftsPerWallet[account] += _ammount;
        //Minting all the account NFTs
        for(uint i = 1 ; i <= _ammount ; i++) {
            _safeMint(account, numberNftSold + i);
        }
