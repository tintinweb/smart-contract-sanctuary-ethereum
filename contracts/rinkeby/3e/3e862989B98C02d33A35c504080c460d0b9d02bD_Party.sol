    ...
    ERC721HolderUpgradeable
{
    ...
    uint256 private constant FIGHTER_SLOTS = 6;
    uint256 private constant HERO_SLOTS = 8;
    ...
    function getEquipment() external view returns (IEquipment) {
        return _equipment;
    }
    ...
    function getUserEquipment(address user)
        ...
        returns (uint256[] memory)
    {
        uint256[] memory equipment = new uint256[](HERO_SLOTS);
        for (uint8 i = 0; i < HERO_SLOTS; i++) {
    ...
    function act(
        ...
        uint256[] memory equipmentEquipped;
        uint256[] memory equipmentUnequipped;
        ...
        if (equipmentActions.length > 0) {
            (equipmentEquipped, equipmentUnequipped) = _act(
                Property.EQUIPMENT,
                equipmentActions
            );
        }
    ...
    function equip(
        ...
        if (item == Property.HERO) {
            ...
            if (unequipped != 0) {
                prev[0] = _damageCalculator.getEquipmentDamageComponent(
                    unequipped
                );
            }

            curr[0] = _damageCalculator.getEquipmentDamageComponent(id);
        }
    ...
    function unequip(Property item, uint8 slot) public override {
        ...
        if (item == Property.HERO) {
            ...
            prev[0] = _damageCalculator.getEquipmentDamageComponent(unequipped);
        }
    ...
    function onEnhancement(uint256 id, uint8 prev)
        public
        override
        returns (bytes4)
    {
        ...
        if (msg.sender == address(_hero)) {
            _updateDamageEnhancement(
                _ownership[address(_hero)][id],
                Property.HERO,
                id,
                prev
            );
            ...
            _updateDamageEnhancement(
                _ownership[address(_fighter)][id],
                Property.FIGHTER,
                id,
                prev
            );
        ...
        return super.onEnhancement(id, prev);
    }
    ...
    // TODO: Update raid damage on both updateDamage / updateDamageEnhancement
    ...
    function _updateDamage(
        ...
        uint256 updatedDPB = _damage[user].computeDamage();
        _raid.updateDamage(user, updatedDPB);

        emit DamageUpdated(user, uint32(updatedDPB));
    }
    ...
    function _updateDamageEnhancement(
        ...
        Damage.DamageComponent memory prevComponent;
        Damage.DamageComponent memory currComponent;

        if (item == Property.HERO) {
            prevComponent = _damageCalculator.getHeroDamageComponent(id, prev);
            currComponent = _damageCalculator.getHeroDamageComponent(id);
        } else {
            prevComponent = _damageCalculator.getFighterDamageComponent(
                id,
                prev
            );
            currComponent = _damageCalculator.getFighterDamageComponent(id);
        }

        _damage[user] = _damage[user].getDamageUpdate(
            prevComponent,
            currComponent
        );
...
contract Enhancer is IEnhancer {
    function onEnhancement(uint256, uint8)
        public
        virtual
        override
        returns (bytes4)
