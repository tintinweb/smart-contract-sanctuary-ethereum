    ...
    VRFConsumerBaseV2
{
    ...
    bytes32 constant USERDATA_TYPEHASH = keccak256(
        "UserData(uint256 rollerId,uint256 score,uint256 timestamp,address wallet)"
    );
    ...
    uint256 public constant SCORE_PRICE_ROLL = 100 ether;
    ...
    mapping(address => uint) private requestIds;
    mapping(uint => MintRequest) private mintRequests;
    ...
    function recordHighScore(uint8 v, bytes32 r, bytes32 s, UserData memory record) external {
        ...
        require(signer == record.wallet, "InvalidSignature");
        require(signer != address(0), "ECDSAInvalidSignature");
        require(record.rollerId < 8888, "InvalidTokenId");
        ...
        mintCompliance
    {
        ...
        if(quantity + addressData.numberMinted - addressData.reserveMinted > maxPublicMintPerAddress) revert MaxMintAmountReached();

        delete totalSupply;
        requestRandomWords(uint32(quantity));
        refundIfOver(quantity * mintPrice);
    }

      // Assumes the subscription is funded sufficiently.
    function requestRandomWords(uint32 quantity) internal {
        // Will revert if subscription is not set and funded.
        uint request = COORDINATOR.requestRandomWords(
        keyHash,
        s_subscriptionId,
        ...
        requestIds[msg.sender] = request;
        mintRequests[request].minter = msg.sender;
        mintRequests[request].quantity = quantity;
    }
    ...
    function fulfillRandomWords(
        ...
        uint64 toMint = mintRequests[requestId].quantity;

        if(addressData.mintTime < startTime) {
            addressData.reserveMinted = 0;
        } else if (addressData.numberMinted >= maxPublicMintPerAddress) revert MaxMintAmountReached();


        if(mintRequests[requestId].whitelisted  && addressData.mintTime < startTime) {
            addressData.numberMinted = 0;
        } else { 
            if (addressData.reserveMinted == 1) revert MaxMintAmountReached();
        }

        for(uint i = 0; i < toMint; i++){
            console.log(mintRequests[requestId].minter);
            _mintToken(mintRequests[requestId].minter, mappedTokenFor(randomWords[i]));
        }

        addressData.numberMinted += toMint;
        addressData.mintTime = uint64(block.timestamp);
    ...
    function mappedTokenFor(uint _randomSeed) internal returns (uint64) {
        ...
        uint maxIndex = MAX_SUPPLY - lockedSupply;
...
/** OWNER FUNCTIONS **/
    ...
    function setReservedSupply(uint _reservedSupply) public onlyOwner {
        reservedSupply = _reservedSupply;
        setPublicSupply();
    }

    function setLockedSupply(uint _lockedSupply) public onlyOwner {
    ...
    function setCoordinatorAddress(address _coordinator) public onlyOwner {
        vrfCoordinator = _coordinator;
        COORDINATOR = VRFCoordinatorV2Interface(vrfCoordinator);
    }


    function setRollAddress(address _rollToken) public onlyOwner {
