...
contract OKOKU is ERC721Enum, Ownable, ReentrancyGuard {
    ...
    uint256 public SALE_NFT = 3330;
	uint256 public GIVEAWAY_NFT = 3;
	
	uint256 public MAX_MINT_PRESALE = 3;
	uint256 public MAX_MINT_PRESALE_HONORARY_MEMBERS = 6;
	uint256 public MAX_MINT_SALE = SALE_NFT;
	
	uint256 public FREE_NFT_PER_MINT_PRESALE = 2;
	uint256 public FREE_NFT_PER_MINT_PRESALE_HONORARY_MEMBERS = 2;
	
	uint256 public MAX_BY_MINT_IN_TRANSACTION_PRESALE = 1;
	uint256 public MAX_BY_MINT_IN_TRANSACTION_PRESALE_HONORARY_MEMBERS = 2;
	uint256 public MAX_BY_MINT_IN_TRANSACTION_SALE = SALE_NFT;
	
	uint256 public PRESALE_PRICE = 33 * 10**15;
	uint256 public SALE_PRICE = 33 * 10**15;
	...
	function mintPreSaleNFT(uint256 _count, uint256 roottype, bytes32[] calldata merkleProof) public payable{
		...
		if(roottype==1)
		{
		    uint256 count = _count * FREE_NFT_PER_MINT_PRESALE;
			require(
			...
			require(
				users[msg.sender].presalemint + count <= MAX_MINT_PRESALE,
		...
		{
		    uint256 count = _count * FREE_NFT_PER_MINT_PRESALE_HONORARY_MEMBERS;
			require(
				SALE_MINTED + count <= SALE_NFT, 
				"Exceeds max limit"
			);
			...
			require(
				users[msg.sender].presalemint + count <= MAX_MINT_PRESALE_HONORARY_MEMBERS,
				"Exceeds max mint limit per wallet"
			);
			require(
				_count <= MAX_BY_MINT_IN_TRANSACTION_PRESALE_HONORARY_MEMBERS,
				"Exceeds max mint limit per txn"
			);
			require(
				msg.value >= PRESALE_PRICE * _count,
	...
	function updatePreSaleMintLimit(uint256 newLimit, uint256 newHonorary) external onlyOwner {
	    require(SALE_NFT >= newLimit, "Incorrect value");
		require(SALE_NFT >= newHonorary, "Incorrect value");
		
        MAX_MINT_PRESALE = newLimit;
		MAX_MINT_PRESALE_HONORARY_MEMBERS = newHonorary;
    }
	...
	function updateMintLimitPerTransectionPreSale(uint256 newLimit, uint256 newHonorary) external onlyOwner {
	    require(SALE_NFT >= newLimit, "Incorrect value");
		require(SALE_NFT >= newHonorary, "Incorrect value");
		
        MAX_BY_MINT_IN_TRANSACTION_PRESALE = newLimit;
		MAX_BY_MINT_IN_TRANSACTION_PRESALE_HONORARY_MEMBERS = newHonorary;
    }
	...
	function updateFreeLimitPerMint(uint256 newLimit, uint256 newHonorary) external onlyOwner {
        FREE_NFT_PER_MINT_PRESALE = newLimit;
		FREE_NFT_PER_MINT_PRESALE_HONORARY_MEMBERS = newHonorary;
    }
