...
contract BAPOrchestrator is ReentrancyGuard, Ownable {
    ...
    mapping(uint256 => uint256) public claimedMethTeen;
    ...
    function initializeTeenBull(uint256[] memory teens) external {
        for (uint256 index; index < teens.length; index++) {
            require(bapTeenBulls.ownerOf(teens[index]) == msg.sender, "Only owner can initialize");
            require(bapUtilities.balanceOf(msg.sender, 2) > 0, "needs to have at least 1 Merge Orb");            
            teenLastClaim[teens[index]] = block.timestamp;
        }
    }
    ...
    function claimMeth(
        ...
        for (uint256 index; index < teensCount; index++) {
            amount += _claimRewardsFromTeen(teens[index]);
        }
    ...
    function _teenDailyRewards() internal pure returns (uint256){
        return 5;
    }
    ...
    function _claimRewardsFromTeen(uint256 tokenId) internal returns (uint256){
        require(bapTeenBulls.ownerOf(tokenId) == msg.sender, "Only owner can claim");
        require (teenLastClaim[tokenId] != 0, "Need to initialize teen bull before claim");
        
        uint256 timeFromCreation = (block.timestamp - teenLastClaim[tokenId]).div(1 hours);
        
        uint256 methAmount = _teenDailyRewards() * timeFromCreation;

        teenLastClaim[tokenId] = block.timestamp;

        claimedMethTeen[tokenId] += methAmount;
