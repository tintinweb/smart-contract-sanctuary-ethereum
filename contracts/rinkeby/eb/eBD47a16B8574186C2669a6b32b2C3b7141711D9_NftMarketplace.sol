...
contract NftMarketplace is Ownable {
    ...
    // modifier isOwnerERC20(address tokenAddress, address spender) {
    //     bool tmp;
    //     for (uint256 i = 0; i < sellerList.length; i++) {
    //         address owner = seller.seller;
    //         if (spender != owner) {
    //             revert NotOwner();
    //         }
    //     }
    ...
    modifier isListedERC20(address tokenAddress, address seller) {
        for (uint256 i = 0; i < sellerList.length; i++) {
            if (seller == sellerList[i].seller) {
                ListingERC20 memory listing = sellerList[i];
    ...
    function listItemERC20(
        ...
        bool tmp = true;
        for (uint256 i = 0; i < sellerList.length; i++) {
            if (msg.sender == sellerList[i].seller) {
                tmp = false;
            }
        }
        if (tmp == false) {
            ...
            ERC20List[tokenAddress] = ListingERC20(price, msg.sender, amount);
            sellerList.push(ERC20List[tokenAddress]);

            emit ERC20Listed(tokenAddress, msg.sender, price, amount);
        }
    }
    ...
    function buyItemERC20(
        ...
        for (uint256 i = 0; i < sellerList.length; i++) {
            if (sellerList[i].seller == ERC20List[tokenAddress].seller) {
                sellerList[i].amount = ERC20List[tokenAddress].amount;
                sellerList[i].price = ERC20List[tokenAddress].price;
            }
        }
    ...
    function updateListingERC20(
        ...
        for (uint256 i = 0; i < sellerList.length; i++) {
            if (sellerList[i].seller == seller) {
                sellerList[i].amount = newAmount;
                sellerList[i].price = newPrice;
            }
        }
    }
    ...
    function remove(uint256 index) private {
        require(index < sellerList.length, "index out of bound");
        for (uint256 i = index; i < sellerList.length; i++) {
            sellerList[i] = sellerList[i + 1];
        }
        sellerList.pop();
    }

    function getListingERC20(address tokenAddress, address seller)
        external
        view
        returns (ListingERC20 memory)
    {
        ListingERC20 memory result;
        for (uint256 i = 0; i < sellerList.length; i++) {
            if (sellerList[i].seller == seller) {
                result = sellerList[i];
            }
    ...
    function cancelListingERC20(address tokenAddress, address seller) external {
        for (uint256 i = 0; i < sellerList.length; i++) {
            if (sellerList[i].seller == seller) {
                remove(i);
            }
