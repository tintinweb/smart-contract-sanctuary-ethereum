...
contract svgManip {
    ...
    function _add_path_point(string memory, string memory, string memory points) pure internal returns (string memory) {
        // return (random(abi.encodePacked(prefix, tokenId, "ADD PATH POINT", _seed_phrase)) % 4 < 2) ? points : '';
        return points;
    }

    function _get_nerve_path(string memory tokenId) pure internal returns (bytes memory) {
