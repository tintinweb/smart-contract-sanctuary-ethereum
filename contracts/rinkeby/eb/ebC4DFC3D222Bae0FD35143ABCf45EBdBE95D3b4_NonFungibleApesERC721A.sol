...
contract NonFungibleApesERC721A is Ownable, ERC721A {
    ...
    uint256 private constant MAX_SUPPLY = 10; // il faudra modifier apres : 110000
    uint256 private constant MAX_GIFT = 2; // il faudra modifier apres : 10000
    uint256 private constant MAX_PUBLIC = 8; // il faudra modifier apres : 100000
    uint256 private constant MAX_SUPPLY_MINUS_GIFT = MAX_SUPPLY - MAX_GIFT;

    uint256 public publicSalePrice = 0.0001 ether;

    uint256 public saleStartTime = 1653521400;

    string public baseURI;

    mapping(address => uint256) amountNFTperWalletPublicSale;

    uint256 private constant maxPerAddressDuringPublicMint = 3; // il faudra modifier apres 500;

    bool public isPaused;

    //Constructor
    constructor(string memory _baseURI) ERC721A("Non Fungible Apes", "NFA") {
        baseURI = _baseURI;
    }
    ...
    /**
     * @notice Change the starting time (timestamp) of the Public sale
     *
     * @param _saleStartTime The new starting timestamp of the Public sale
     */
    ...
    function publicMint(address _account, uint256 _quantity)
        ...
        callerIsUser
    {
        require(!isPaused, "Contract is Paused"); // il faudra raviver les comments en dessous
        require(
            currentTime() >= saleStartTime,
            "Public sale has not started yet"
        );
        require(
            currentTime() < saleStartTime + 10 minutes,
