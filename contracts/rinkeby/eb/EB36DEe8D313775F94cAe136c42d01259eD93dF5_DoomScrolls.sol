...
contract DoomScrolls is ERC721A, Ownable {
    /*-----------------------------VARIABLES------------------------------*/
    ...
    string[] private _whoWillMissYou = [
    ...
    /*--------------------------CONSTRUCTOR-------------------------------*/
    constructor() ERC721A("LittlePepe", "LP") {}
    ...
    function getRandomTrait(
        ...
        string memory output = traits[randomInt % traits.length];
        return output;
    }

    /// @notice Get "Season" trait for input token ID
    function getSeason(uint256 tokenId) public view returns (string memory) {
        return getRandomTrait(tokenId, _season, "TIME OF DAY");
    }

    /// @notice Get "Time of Day" trait for input token ID
    function getTimeOfDay(uint256 tokenId) public view returns (string memory) {
        return getRandomTrait(tokenId, _timeOfDay, "SEASON");
    }

    /// @notice Get "What You'll Wear" trait for input token ID
    function getWhatYoullWear(uint256 tokenId) public view returns (string memory) {
        return getRandomTrait(tokenId, _whatYoullWear, "WHAT YOU'LL WEAR");
    }

    /// @notice Get "Chase Scene" trait for input token ID
    function getChaseScene(uint256 tokenId) public view returns (string memory) {
        return getRandomTrait(tokenId, _chaseScene, "CHASE SCENE");
    }

    /// @notice Get "Cause of Death" trait for input token ID
    function getCauseOfDeath(uint256 tokenId) public view returns (string memory) {
        return getRandomTrait(tokenId, _causeOfDeath, "CAUSE OF DEATH");
    }

    /// @notice Get "Burial" trait for input token ID
    function getBurial(uint256 tokenId) public view returns (string memory) {
        return getRandomTrait(tokenId, _burial, "BURIAL");
    }

    /// @notice Get "Who Will Miss You" trait for input token ID
    function getWhoWillMissYou(uint256 tokenId) public view returns (string memory) {
        return getRandomTrait(tokenId, _whoWillMissYou, "WHO WILL MISS YOU");
    }

    /// @dev Get HTML strings with x and y spacing, and font
    ...
    /// @notice Override tokenURI view function with on-chain metadata generator
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        if (!_exists(tokenId)) revert URIQueryForNonexistentToken();

        string memory season = getSeason(tokenId);
        string memory timeOfDay = getTimeOfDay(tokenId);
        string memory whatYoullWear = getWhatYoullWear(tokenId);
        string memory chaseScene = getChaseScene(tokenId);
        string memory causeOfDeath = getCauseOfDeath(tokenId);
        string memory burial = getBurial(tokenId);
        string memory whoWillMissYou = getWhoWillMissYou(tokenId);

        // Concatenation needs to be split up because there's a limit to the number of inputs
        string memory output = string(
            abi.encodePacked(
                '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 550 550"><rect width="100%" height="100%" fill="black" /><text x="20" y="20" class="base">',
                season,
                getSpacer(20, 60),
                timeOfDay,
                getSpacer(20, 100),
        ...
        output = string(
            abi.encodePacked(
                ...
                getSpacer(20, 300),
                "</text>",
                "<g><image style='overflow:visible;enable-background:new    ;' width='525' height='1462' xlink:href='https://acegif.com/wp-content/uploads/2021/4fh5wi/pepefrg-4.gif'  transform='matrix(0.24 0 0 0.24 188.7147 167.7253)'></image></g>",
                "<style>",
                "@font-face{"
                ...
                string(SSTORE2.read(font)),
                ") format('truetype')"
                "}",
                ".base{font-family:covered_by_your_graceregular;font-size:20px;fill:#fff}",
                "</style>"
                "</svg>"
            )
        );
    ...
    function mintPublic(uint256 nMints) external payable callerIsNotContract validAmount(nMints) {
        ...
        validAmount(nMints)
    {
        ...
        if (_numberMinted(msg.sender) + nMints > MAX_ALLOWLIST_MINTS) revert ExceedsAllowlistAmount();

        _safeMint(msg.sender, nMints);
    }

    /// @notice Airdrop mint for anyone to send a Doom Scroll to someone else
    function mintAirdrop(address recipient, uint256 nMints) external payable validAmount(nMints) {
        if (!isPublicMintActive) revert PublicMintInactive();
        if (nMints > MAX_PUBLIC_MINTS) revert ExceedsTxnLimit();
        if (msg.value < publicMintPrice * nMints) revert InsufficientAmountSent();
    ...
    /// @notice Reserved mints for team
    function mintReserve(uint256 nMints) external onlyOwner validAmount(nMints) {
        _safeMint(msg.sender, nMints);
    }

    /*-----------------------------MODIFIERS------------------------------*/

    /// @notice Requires that a contract is not calling the function
    modifier callerIsNotContract() {
        if (msg.sender != tx.origin) revert CallerIsContract();
        _;
    }

    /// @notice Requires that the number of mints is greater than 0 and does not exceed the maximum supply
    modifier validAmount(uint256 nMints) {
        if (totalSupply() + nMints > MAX_SUPPLY) revert ExceedsMaxSupply();
        _;
    }
    ...
    // @notice Allows the owner to store the font-family for the SVG
    function saveFile(string calldata fileContent) public onlyOwner {
        font = SSTORE2.write(bytes(fileContent));
    }
}
