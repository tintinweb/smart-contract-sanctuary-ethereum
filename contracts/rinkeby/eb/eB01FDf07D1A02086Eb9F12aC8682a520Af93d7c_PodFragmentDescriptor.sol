...
contract PodFragmentDescriptor is Ownable {
    function tokenURI() public pure returns (string memory) {
        string memory name = string(abi.encodePacked('{"name": "Pod Fragment'));
                        ...
                        bytes(
                            string(
                                abi.encodePacked(
                                    ...
                                    '","attributes": [],',
                                    '"description": "Pod Fragments is a collection of 10,650 Pod Fragments. All the metadata and images are generated and stored 100% on-chain. No IPFS, no API. Just the Ethereum blockchain."',
                                    "}"
                                )
                            )
                        )
