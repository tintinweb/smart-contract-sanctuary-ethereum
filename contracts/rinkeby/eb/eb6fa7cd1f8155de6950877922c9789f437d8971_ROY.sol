...
contract ROY is Context, IERC20, Ownable {  //&
    ...
    uint256 public gyoShare = 0; // Will be used later
 ...
 /* ============= CONSTRUCTOR ============= */
    ...
    function distributeFees(uint256 contractTokenBalance) private lockTheSwap { //& rename ?
        ...
        uint256 daiForMarketing = IERC20(DAI).balanceOf(address(this)); //DAI
        if (daiForMarketing > 0) {
            //transferToAddressETH(marketingWallet, ethForMarketing); //ETH
            IERC20(DAI).transfer(marketingWallet, daiForMarketing); //DAI
        }
...
//need to send ETH to contract
    function createLiquidityPool() external onlyOwner returns(bool){
        ...
        require(IERC20(DAI).balanceOf(address(this)) > 0, "Must have DAI on contract to launch"); //DAI

        // create pair
        //lpPair = IDexFactory(dexRouter.factory()).createPair(address(this),dexRouter.WETH()); ETH
