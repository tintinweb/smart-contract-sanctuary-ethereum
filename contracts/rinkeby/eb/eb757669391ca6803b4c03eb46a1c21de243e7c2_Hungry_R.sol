...
contract Hungry_R is ERC721Enumerable, Ownable, ReentrancyGuard {
  using Strings for uint256;
  ...
  AppState _state = AppState(0.001 ether, 0, uint256(blockhash(block.number)) ^ uint256(block.timestamp));
  ...
  function nextNonce(uint256 previousNonce, uint256 counter) internal returns (uint256 nonce) {
  ...
  function setMinFreeAmount(uint32 supply) external onlyOwner {
    _free.startSupply = supply;
  }

  function setFreeSinglesIndex(uint8 idx) external onlyOwner {
    _free.offset = idx;
  }

  function setFreeHundredsIndex(uint8 idx) external onlyOwner {
    _free.hundreds = idx;
  }
  ...
  function setMinPaidAmount(uint32 supply) external onlyOwner {
    _paid.startSupply = supply;
  }

  function setPaidSinglesIndex(uint8 idx) external onlyOwner {
    _paid.offset = idx;
  }

  function setPaidHundredsIndex(uint8 idx) external onlyOwner {
    _paid.hundreds = idx;
  }
