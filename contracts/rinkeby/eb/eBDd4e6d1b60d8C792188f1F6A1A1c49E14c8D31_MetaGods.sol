...
contract MetaGods is ERC721Enum, Ownable, ReentrancyGuard, PaymentSplitter {
    ...
    uint256 reserveSupply = 200;

    mapping(address => uint256) public presaleWhitelistMints;

    mapping(address => uint256) public passesUsedByAddress;
    ...
    function mint(uint256 _mintAmount) external payable {
        ...
        require(totalSupply + _mintAmount <= _publicSupply(), "Exceeds supply" );
    ...
    function mintWithPass(uint256 _mintAmount) external {
        ...
        require(_mintAmount + passesUsedByAddress[msg.sender] <= passesOwned, "Exceeds owned");

        passesUsedByAddress[msg.sender] += _mintAmount;
    ...
    function reserve(address _to, uint256 _reserveAmount) public onlyOwner {
        ...
        require(_reserveAmount <= reserveSupply);
    ...
    function setMintPassStakingContract(address _mintPassStakingContract) public onlyOwner {
        MintPassStaking = IMintPassStaking(_mintPassStakingContract);
    }
    ...
    function _publicSupply() internal view virtual returns (uint256) {
        return MAX_SUPPLY - reserveSupply;
    }
