...
contract RandomIpfsNft is VRFConsumerBaseV2, ERC721URIStorage, Ownable {
    ...
    // vrf helpers
    mapping(uint256 => address) public s_reqIdToSender;
    ...
    function requestNft() public payable returns (uint256 requestId) {
        ...
        s_reqIdToSender[requestId] = msg.sender;
        emit NftRequested(requestId, msg.sender);
    }

    function fulfillRandomWords(uint256 requestId, uint256[] memory randomWords) internal override {
        address dogOwner = s_reqIdToSender[requestId];
        uint256 newTokenId = s_tokenCounter;
        uint256 moddedRng = randomWords[0] % MAX_CHANCE_VALUE;
        Breed dogBreed = getBreedFromModRng(moddedRng);
        s_tokenCounter += 1;
    ...
    function getBreedFromModRng(uint256 moddedRng) public pure returns (Breed) {
