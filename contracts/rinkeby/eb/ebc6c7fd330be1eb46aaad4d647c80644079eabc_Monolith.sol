    ...
    ReentrancyGuardUpgradeable
{
    ...
    uint256 public constant DEV_MINT = 250;
    uint256 public constant MAX_PER_TX = 10;
    uint256 public constant DA_START_PRICE = 1 ether;
    ...
    function initialize(
        ...
        dutchAuctionStatus = DAStatus({
            ...
            lastPrice: uint80(DA_START_PRICE)
        });
    ...
    function _calculatePrice(DAStatus memory status) internal view returns (uint80) {
        ...
        uint256 currentPrice = DA_START_PRICE - discount;
    ...
    function getRefundInfo() external view returns (uint256) {
        DAStatus memory status = dutchAuctionStatus;
        Bidder memory bidder = bids[msg.sender];
    ...
    function whitelistMint(uint256 quantity) external payable nonReentrant {
        ...
        if (status.whitelistMints + quantity > WHITELIST_SUPPLY) {
        ...
        if (status.whitelistMints + 1 == WHITELIST_SUPPLY) {
