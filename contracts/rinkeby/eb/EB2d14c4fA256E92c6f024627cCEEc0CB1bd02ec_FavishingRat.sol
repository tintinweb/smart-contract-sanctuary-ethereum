...
contract FavishingRat is ERC721, Ownable, ReentrancyGuard, PaymentSplitter {
  ...
  function presaleMint(
    ...
    require(msg.sender == account, 'FavishingRat: Not allowed');
    require(presaleM, 'FavishingRat: Presale is OFF');
    require(!paused, 'FavishingRat: Contract is paused');
    require(
      _amount <= presaleAmountLimit,
      "FavishingRat: You can't mint so much tokens"
    );
    require(
      _presaleClaimed[msg.sender] + _amount <= presaleAmountLimit,
      'FavishingRat: You can not mint so many tokens'
    );
    ...
    require(
      ...
      'FavishingRat: max supply exceeded'
    );
    require(
      _price * _amount <= msg.value,
      'FavishingRat: Not enough ethers sent'
    );
  ...
  function publicSaleMint(uint256 _amount) external payable onlyAccounts {
    require(publicM, 'FavishingRat: PublicSale is OFF');
    require(!paused, 'FavishingRat: Contract is paused');
    require(_amount > 0, 'FavishingRat: zero amount');
