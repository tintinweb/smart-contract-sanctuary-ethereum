...
import {TimerLib, TimerData} from "../../libraries/Timer/TimerLib.sol";
...
/// @dev a helper struct to reduce contract calls and get all the state variables for the caller in a single struct
struct ContractState {
    bool isClosed;
    bool isOpen;
    bool paused;
    uint256 entrantsSize;
    uint256[] tokenIds;
    bool didWin;
    bool didClaim;
...
contract LeaderBoardAuction is AdminControl {
    ...
    /// @notice an event to call when a refund is sent
    /// @param walletAddress the wallet who made the refund request
    /// @param refund the amount of funds refunded
    /// @param productId the product id
    /// @param accountId the account who made the refund request
    event OnRefund(
        address walletAddress,
        uint256 refund,
        bytes32 productId,
    ...
    /// @notice an event to call when the auction is closed
    /// @param recipient the wallet who would get the funds from the auction
    /// @param revenue the amount of funds being sent, the revenue from the auction
    /// @param productId the product related to the auction
    ...
    /// @notice an event to call when the auction is started
    event OnAuctionStarted();
    ...
    event OnBid(address bidder, bytes32 productId, uint256 bid, bool isNew);
    ...
    /// @dev the address of the nft contract we use to give tokens
    address public nftContractAddress;
    /// @dev the address of the wallet keeping custody of the tokens
    address public nftCustodial;
    ...
    function _increaseBid(address addr, uint256 bid) private {
        ...
        require(entries[addr].bid + bid >= floor, AUCTION_ERR_FLOOR);
    ...
    /// @dev make sure the nftCustodial is giving this contract approval to operate on the nftContractAddress
    /// @param _nftContractAddress the address of the contract that manages the given tokens
    /// @param _nftCustodial the wallet which holds custody of the tokens
    /// @param _tokenIds the new list of tokens the contract can manage (ordered from 1st to last place)
    function setTokens(
        address _nftContractAddress,
        address _nftCustodial,
        uint256[] memory _tokenIds
    ) external adminRequired {
        tokenIds = _tokenIds;
        nftContractAddress = _nftContractAddress;
        nftCustodial = _nftCustodial;
    }

    /// @notice winners can claim tokens based on their position
    function claimToken() external {
        require(isWinner[msg.sender], AUCTION_ERR_NOT_WINNER);
        require(!didClaim[msg.sender], AUCTION_ERR_ALREADY_CLAIMED);
        ...
        for (uint256 i = 0; i < winnersSize; i++) {
            if (winners[i] == msg.sender) {
                ...
                nft.transferFrom(nftCustodial, msg.sender, tokenIds[i]);
                emit OnTokenClaim(
                    msg.sender,
                    tokenIds[i],
                    productId,
    ...
    /// @notice get all the contract state in a single request
    /// @param limit limits the amount of entries to get back
    /// @return state the current state of the contract relative to the caller
    /// @return auctionEntries the entries of the auction, based off the limit parameter
    /// @return settings the current settings of the contract
    function getContractState(uint256 limit)
        external
        view
        returns (
            ContractState memory state,
            AuctionEntry[] memory auctionEntries,
            Props memory settings
        )
    {
        state = ContractState({
            isClosed: isClosed,
            isOpen: timer._isRunning() && !isClosed,
            paused: timer.paused,
            endTime: timer._deadline(),
            startTime: timer.startTime,
            entrantsSize: entrantsSize,
            nftContractAddress: nftContractAddress,
            tokenIds: tokenIds,
            didWin: isWinner[msg.sender],
            didClaim: didClaim[msg.sender],
            entry: entries[msg.sender]
        });
        settings = Props({
            floor: floor,
            step: step,
            minutesIncrease: minutesIncrease,
            minutesPadding: minutesPadding,
            productId: productId,
            maxWinners: maxWinners
        });
        auctionEntries = getEntries(0, limit);
    }
    ...
    function withdrawBid() external {
        ...
        require(!isWinner[msg.sender], AUCTION_ERR_IS_WINNER);
    ...
    function makeBid(bytes32 id) external payable {
        ...
        if (entries[msg.sender].addr == address(0)) {
            ...
            emit OnBid(msg.sender, productId, msg.value, true);
        } else {
            _increaseBid(msg.sender, msg.value);
            emit OnBid(msg.sender, productId, msg.value, false);
        }
...
/// @dev happens when you try to claim a token, but you are not a winner - non winners (losers) should claim refund
string constant AUCTION_ERR_NOT_WINNER = "Auction: not winner";
/// @dev happens when you try to claim a refund, but you are a winner - winners should claim tokens
string constant AUCTION_ERR_IS_WINNER = "Auction: is winner";
/// @dev happens when someone tries to make a bid, but the auction is closed (by an admin)
string constant AUCTION_ERR_CLOSED = "Auction: closed";
/// @dev happens when someone tries to make a bid, but the auction is paused
string constant AUCTION_ERR_PAUSED = "Auction: paused";
/// @dev happens when someone tries to make a bid, but the auction is over (based on time)
string constant AUCTION_ERR_IS_OVER = "Auction: is over";
/// @dev happens when a bid is lower than the floor price
string constant AUCTION_ERR_FLOOR = "Auction: lower than floor";
/// @dev happens when a bid is made outside the step rules
string constant AUCTION_ERR_STEP = "Auction: not within step";
/// @dev happens after you already asked for a refund
string constant AUCTION_ERR_ALREADY_REFUNDED = "Auction: already refunded";
/// @dev happens after you already claimed a token
string constant AUCTION_ERR_ALREADY_CLAIMED = "Auction: already claimed";
/// @dev happens if you ask for a refund, but you never made a bid
string constant AUCTION_ERR_NOTHING_TO_REFUND = "Auction: nothing to refund";
/// @dev happens when you try to withdraw funds or claim token while the auction is not over
