 ...
 */
contract WhiteList is ERC712, EIP712 {
 ...
 */
contract ERC712 is IERC721Enumerable, IERC721Metadata, Ownable {
    string public constant NAME = "Adventurers Of Ether";
    string public constant SYMBOL = "KOE";
    uint public constant TOKENS_PER_REVEAL_GROUP = 1000;
    uint public constant MAX_SUPPLY = 9001; // +1 extra 1 for <

    /* state */
    uint256 public maxSupply = 3000;
    uint public minted; // count of minted tokens
    mapping(address /* minter */ => /* minted tokes count */ uint) public minters;
    uint[(MAX_SUPPLY / TOKENS_PER_REVEAL_GROUP) + 
        (((MAX_SUPPLY % TOKENS_PER_REVEAL_GROUP) + TOKENS_PER_REVEAL_GROUP - 1) / TOKENS_PER_REVEAL_GROUP)]
        public revealGroups;

    address[MAX_SUPPLY] internal owners;

    mapping(address /* owner */ => /* tokens count */ uint) internal balances;
    ...
    function _mint(address _to, uint256 _amount) internal returns (uint oldIndex, uint newIndex) {
    ...
    function _mintBatch(address[] memory _to, uint[] memory _amounts)
        internal returns (uint oldIndex, uint newIndex)
    {
    ...
    function tokenURI(uint _tokenId) external view returns (string memory) {
        ...
        uint _groupIndex = _tokenId / TOKENS_PER_REVEAL_GROUP;
        uint _groupId = revealGroups[_groupIndex];
        if (_groupId > 0) {
            return string(abi.encodePacked(
                _groupURI(_groupId),
                Strings.toString((_tokenId%TOKENS_PER_REVEAL_GROUP) + 1)
            ));
        }
        return "";
    }
    ...
    function reveal(uint _index, uint _revealGroup) external onlyOwner {
        revealGroups[_index] = _revealGroup;
    }
