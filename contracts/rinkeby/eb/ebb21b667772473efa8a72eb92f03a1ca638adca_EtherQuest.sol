...
contract EtherQuest is ERC721, ERC721Enumerable, ERC721URIStorage, Pausable, Ownable, ERC721Burnable {
    ...
    uint public gen2Cost;
    string public baseURI;

    address devWallet = 0xcBF18174054a53C023dCc69c538BC81AA724F823;
    ...
    constructor() ERC721('EtherQuest', 'ETHERQUEST') {}
    ...
    function mint(uint256 amount) public payable {
        ...
        require(amount <= 6, 'only max of 6 per transaction allowed');
        for (uint256 i = 0; i < amount; i++) internalMint(msg.sender);
    }

    function mintGen2(address token, uint256 amount) public {
        require(allowed[token], "Token must be allowed to be deposited"); 
        require(amount == gen2Cost, "Must deposit token minimums");
        IERC20(token).transferFrom(msg.sender, address(burnAddress), amount);
    ...
    function setAllowed(address token, bool allow) public onlyOwner {
        allowed[token] = allow; 
    }
