...
interface IFeeCollection{
    function feeCollector(uint256 amount,uint256 share, address  wallet) external returns (uint256) ;
}
...
contract HSIMarketplace {
     ...
     */
    function setFeeShare(address payable wallet, uint256 percentage)
        external
        isOwner
    {
      

        if (feeShare[wallet] >0) {
    ...
    function feeCollector(uint256 amount) internal returns (uint256) {
        ...
        for (uint256 i=0; i < feeCollectors.length; i++) {
          ...
          uint256 shareInAmount=  fee.feeCollector(amount,share,feeCollectors[i]);
    ...
    function remove(uint index) public isOwner returns(address[] memory)  {
        require (index < feeCollectors.length,"enter valid index") ;

            feeCollectors[index] = feeCollectors[feeCollectors.length - 1];
