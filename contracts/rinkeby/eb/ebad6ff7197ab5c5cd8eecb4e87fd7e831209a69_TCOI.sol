...
contract TCOI is ERC721, Ownable {
    ...
    bool public preSaleMintable = true;
    ...
    ) ERC721(_projectName, _tokenSymbol) {
    ...
    modifier isMintable() {
        if (msg.sender != owner())
        require(mintable, "TCOI: NFT cannot be minted yet.");
        _;
    }

    modifier isPreSaleMintable() {
        require(preSaleMintable, "TCOI: NFT cannot be minted yet.");
        _;
    }
    ...
    modifier isNotExceedMaxMintPerTx(uint256 amount) {
        require(
            ...
            "TCOI: Mint amount exceeds max limit per tx for PublicSale."
        );
        _;
    }
    ...
    modifier isNotExceedAvailableSupply(uint256 amount) {
        require(
            ...
            "TCOI: There are no more remaining NFT's to mint."
        );
        _;
    }
    ...
    modifier isPaymentSufficient(uint256 amount) {
        require(
            ...
            "TCOI: There was not enough/extra ETH transferred to mint an NFT."
        );
        _;
    }
    ...
    modifier isAllowList() {
        require(
            ...
            "TCOI: You're not on the list for the presale."
        );
        _;
    }
  ...
  function reveal(bool _state) public onlyOwner {
        ...
        isPaymentSufficient(amount)
    {
        ...
        if(preSaleMintable == true) {
            ...
            require(ownerMintedCount + amount <= totalMintablePresale, "max NFT per address exceeded");
        }
    ...
    require(preSaleMintable, "Presale is still active, please try later");
        for (uint256 index = 1; index <= amount; index++) {
            uint256 id = supply.current();
            _safeMint(msg.sender, id);
            supply.increment();
