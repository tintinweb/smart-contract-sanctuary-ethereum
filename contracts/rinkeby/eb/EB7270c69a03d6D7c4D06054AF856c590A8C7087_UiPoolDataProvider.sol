...
contract InterestRate is IInterestRate {
    ...
    uint256 internal immutable multiplierPerBlock;

    // Slope of the variable interest curve when utilization rate > OPTIMAL_UTILIZATION_RATE. Expressed in ray
    uint256 internal immutable jumpMultiplierPerBlock;
    ...
    function getMaxVariableBorrowRate() external view override returns (uint256) {
        return _baseVariableBorrowRate + (multiplierPerBlock) + (jumpMultiplierPerBlock);
    }
    ...
    function calculateInterestRates(
        ...
        if (vars.utilizationRate > OPTIMAL_UTILIZATION_RATE) {
            uint normalRate = OPTIMAL_UTILIZATION_RATE.rayMul(multiplierPerBlock) + _baseVariableBorrowRate;
            uint256 excessUtilizationRateRatio = vars.utilizationRate - OPTIMAL_UTILIZATION_RATE;

            vars.currentVariableBorrowRate = jumpMultiplierPerBlock.rayMul(excessUtilizationRateRatio) + normalRate;
        } else {
            vars.currentVariableBorrowRate =
            _baseVariableBorrowRate + vars.utilizationRate.rayMul(multiplierPerBlock);
        }
