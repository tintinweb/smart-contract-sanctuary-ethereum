...
contract KOC2 is ERC721, EIP712, Ownable{
    ...
    constructor( string memory _baseUri, address _proxyRegistryAddress, uint256 _MAX_SUPPLY) 
    ERC721("Knight Of Chain 2", "KOC2")
    EIP712("KOC2", "1.0.0")
    {
      baseUri = _baseUri;
    ...
    function mint(bytes calldata _signature, uint256 _nftIndex, uint256 _quantity) external payable {
      require(runningSupply + _quantity < MAX_SUPPLY, "NFT: Cannot mint more!");
      if(msg.value == 0){
    ...
    function claimNFT(bytes calldata _signature, uint256 _nftIndex, uint256 _quantity) internal {
      ...
      for(uint i=0; i< _quantity; i++){
        ...
        emit FreeMint(newTokenId, msg.sender);
      }
      runningSupply += _quantity;
    }
    ...
    function claimWhitelist(bytes calldata _signature, uint256 _nftIndex, uint256 _quantity) internal{
      ...
      for (uint i =0 ;i <_quantity ; i++){
        ...
        uint256 newTokenId = _tokenIds.current();
        _mint(msg.sender, newTokenId);
        emit WhiteListMint(newTokenId, msg.sender);
      }
      runningSupply += _quantity;
    }
    ...
    function updatesWhitelistMintFee(uint256 updateFee) external onlyOwner {
        whitelistMintFee = updateFee;
    }

    function updatePubilcSale(bool publicSale) external onlyOwner {
