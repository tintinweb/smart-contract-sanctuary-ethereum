...
contract animalMain is  ERC721,Ownable {
  ...
  struct animal {
    uint8 Variety; //DNA
    uint8 color; //幾星級(抽卡的概念)
    uint8 pupil; //(哪種動物 1老虎  2狗  3獅子  4鳥)
  }
  ...
  function seeAnimalDna(uint256 _tokenId) public view returns (uint8 Variety) {
    return animals[_tokenId].Variety;
  }

  function seeAnimalStar(uint256 _tokenId) public view returns (uint8 color) {
    return animals[_tokenId].color;
  }
  
  function seeAnimalRole(uint256 _tokenId) public view returns (uint8 pupil) {
    return animals[_tokenId].pupil;
  }
  ...
  function random(uint8 number, uint8 number2) public view returns(uint8){
    return uint8(keccak256(abi.encodePacked(block.timestamp,block.difficulty,  
    msg.sender,number2))) % number;
  }

  function makeNFT() public payable returns(uint8, uint8, uint8) {
    uint8 rand0 = random(255, 255);
    uint8 rand1 = random(255, rand0);
  ...
  modifier maxPrice() { 
    require(msg.value >= 0 && msg.value <= 0.01 ether); //限制每次建立動物需要花費多少ETH
    _; 
  }

  function mint() public {

       uint8 Variety; //品種:緬因 布偶 挪威森林
       uint8 color; //毛色:黃白 黑白 三花
       uint8 pupil; //瞳孔:藍色 黃色 異瞳
       (Variety, color, pupil)=makeNFT();
      //TO DO 
      //使用亂數來產生DNA, 星級, 動物種類
      //限制每次建立動物需要花費多少ETH
       
      uint id = animals.push(animal(uint8(Variety), uint8(color), uint8(pupil))) - 1;
