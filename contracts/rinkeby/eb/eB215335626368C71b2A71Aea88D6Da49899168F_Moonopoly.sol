...
contract Moonopoly is ERC1155, Ownable, Pausable, ReentrancyGuard
{
    ...
    uint256 public _MINIMUM_CARD_INDEX = 0;
    uint256 public _SUM_OF_PROBABILITIES = 5555;
    ...
    function _drawRandomCard() internal virtual returns (uint256 cardID) 
    {
        uint256 value = _random() % _SUM_OF_PROBABILITIES;
        for (uint256 i = 1; i < _UNIQUE_CARDS; i++) 
        {
            uint256 probability = _CARD_ID_MAPPING[i];
    ...
    function drawCard() private returns (uint256 ID) 
    {
    ...
    function _canMint(uint256 cardID) private view returns (bool) { return _CARD_ID_MINTED[cardID] < _CARD_ID_MAPPING[cardID]; }
