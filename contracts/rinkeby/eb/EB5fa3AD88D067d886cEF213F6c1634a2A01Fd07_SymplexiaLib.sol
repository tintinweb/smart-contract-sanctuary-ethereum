...
//-------------------------------------------------------------------------
    ...
    function setInternalStatus (InventoryStorage storage self, address account, bool newStatus, bool isLocked) public {
        self.Basis[account].isInternal     = newStatus;
        self.Basis[account].isTaxFree      = newStatus;
        self.Basis[account].isLocked       = isLocked;
        if (newStatus) {
            self.Basis[account].isNonBonus     = newStatus;
            self.Basis[account].isUnrewardable = newStatus;
...
//------------------------------------------------------------------
   ...
   function investorBurn (InventoryStorage storage self, address Sender, uint256 burnAmount) public { 
        ...
        require(!self.Basis[Sender].isInternal, "Internal Address");
...
//-----------------------------------------------------------------------------
   ...
   function sendAndFreeze (InventoryStorage storage self, address _sender, address _recipient, uint256 _amountToFreeze, uint64 _freezeDuration) public {
        ...
        require(!self.Basis[_recipient].isLocked && !self.Basis[_recipient].isInternal, "Recipient locked");
...
//-----------------------------------------------------------------------------
   ...
   function freezeUnfitEarnings (InventoryStorage storage self, address Sender, address _unfitTrader) public {  
        ...
        freezeAssets (self, _unfitTrader, _shortFreeze, 180);                   // Freeze half earning for 180 days
        freezeAssets (self, _unfitTrader, _longFreeze, 1095);                   // Freeze the other half for 3 years
 
        emit UnfitEarningsFrozen(Sender, _unfitTrader, _bonusUnfit);
    }
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
    ...
    function excludeFromBonus (InventoryStorage storage self, address sender, address account) public {
        ...
        self.noBonusList.push(account);
        emit SetExcludedFromBonus (sender, account, true);
    }
//-----------------------------------------------------------------------------
    ...
    function includeInBonus (InventoryStorage storage self, address Sender, address account) public {
        ...
        emit SetExcludedFromBonus (Sender, account, false);
    }
//-----------------------------------------------------------------------------
