  ...
  OwnableUpgradeable
{
  ...
  function toggleMinting() public onlyOwner {
    require(msg.sender == tx.origin, "smart contract sender is not allowed");
    mintingEnabled = !mintingEnabled;
  }

  function setValidTokens(uint256[] memory tokensToEnable) public onlyAdmin {
    require(msg.sender == tx.origin, "smart contract sender is not allowed");
    require(tokensToEnable.length < 51, "Cannot add more than 50 tokens in one transaction");
    //++i instead of i++
  ...
  function mint(uint256 token, uint32 variant_id) public nonReentrant {
    // WarpSoundAPI api = WarpSoundAPI(warpSoundContract);
    // address base_token_owner = api.ownerOf(token);
    require(msg.sender == tx.origin, "Minting from a smart contract is not allowed");
    ...
    require(variant_id > 0, "you have provided an invalid variant id");
  ...
  function mintTest(uint256 token, uint32 variant_id) public nonReentrant onlyAdmin {
    require(
      tokensConsumed[token] == false,
      "This token's ability to mint has already been consumed."
    );
