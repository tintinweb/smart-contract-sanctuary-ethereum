...
contract DBAGAAP is ERC721Enumerable, Ownable {
    ...
    WhiteList wl1 = new WhiteList(100);
    WhiteList wl2 = new WhiteList(100);
    WhiteList wl3 = new WhiteList(100);
    WhiteList wl4 = new WhiteList(100);
    ...
    function redeem(uint256 _tokenId) public {
        require(!pR, "Paused");
        require(
            DBAGNFTAddress != 0x0000000000000000000000000000000000000000,
            "Redeem contract address missing"
        );
        ...
        bytes memory payloadNFT = abi.encodeWithSignature(
        ...
        bytes memory payloadMP = abi.encodeWithSignature(
            "redeemMP(address[],uint256)",
            addressArray,
            _tokenId
        );
    ...
    function isWL1() public view returns (bool) {
        return wl1.isWL(msg.sender);
    }

    function isWL2() public view returns (bool) {
        return wl2.isWL(msg.sender);
    }

    function isWL3() public view returns (bool) {
        return wl3.isWL(msg.sender);
    }

    function isWL4() public view returns (bool) {
        return wl4.isWL(msg.sender);
    }

    function isWLM1() public view returns (bool) {
        return wl1.isWLM(msg.sender);
    }

    function isWLM2() public view returns (bool) {
        return wl2.isWLM(msg.sender);
    }

    function isWLM3() public view returns (bool) {
        return wl3.isWLM(msg.sender);
    }

    function isWLM4() public view returns (bool) {
        return wl4.isWLM(msg.sender);
    }
    ...
    function setDBAGMPAddress(address _DBAGMPAddress) public onlyOwner {
        DBAGMPAddress = _DBAGMPAddress;
    }

    function WLA1(address[] calldata _user) public onlyOwner {
        for (uint256 i; i < _user.length; i++) {
            wl1.WLA(_user[i]);
        }
    }

    function WLA2(address[] calldata _user) public onlyOwner {
        for (uint256 i; i < _user.length; i++) {
            wl2.WLA(_user[i]);
        }
    }

    function WLA3(address[] calldata _user) public onlyOwner {
        for (uint256 i; i < _user.length; i++) {
            wl3.WLA(_user[i]);
        }
    }

    function WLA4(address[] calldata _user) public onlyOwner {
        for (uint256 i; i < _user.length; i++) {
            wl4.WLA(_user[i]);
        }
    }
...
contract WhiteList is Ownable {
    ...
    constructor(uint256 _nbWLSC) {
        nbWLSC = _nbWLSC;
    }

    function nbWLSCValue() external view returns (uint256) {
        return nbWLSC;
    }

    function isWL(address _user) external view returns (bool) {
        return wlC[_user];
    }

    function isWLM(address _user) external view returns (bool) {
        return wlMintedC[_user];
    }
