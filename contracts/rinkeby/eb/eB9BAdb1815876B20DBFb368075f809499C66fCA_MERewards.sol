...
contract MERewards is ReentrancyGuard, Ownable {
  ...
  function claim(Voucher calldata voucher) external nonReentrant {
    address _signer = _verifyMintData(voucher);
  ...
  function _verifyMintData(Voucher calldata voucher)
    internal
    view
    returns (address)
  {
  ...
  function _hashVoucher(Voucher calldata voucher)
    ...
    returns (bytes32)
  {
    bytes memory changeInfo = abi.encodePacked(voucher.beneficiary, voucher.amount);
