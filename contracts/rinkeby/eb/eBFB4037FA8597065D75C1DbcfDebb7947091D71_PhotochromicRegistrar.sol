...
contract PhotochromicRegistrar is ERC721Enumerable, Ownable {
    ...
    function isUserIdAvailable(uint256 id) public view returns (bool) {
        uint256 subnode = uint256(keccak256(abi.encodePacked(baseNode, bytes32(id))));
        return available(subnode);
    }

    function getNode(string memory userid) external view returns (bytes32) {
        return keccak256(abi.encodePacked(baseNode, keccak256(abi.encodePacked(userid))));
    }
    ...
    function _register(
        ...
        require(isUserIdAvailable(id));

        bytes32 label = bytes32(id);
        // replace id with full node, this way the avatar can be composed using the node
        bytes32 subnode = keccak256(abi.encodePacked(baseNode, label));

        uint256 tokenId = uint256(subnode);

        _safeMint(owner, tokenId);
        // Although the NFT owner should be the photochromic end user,
        // the owner of the subdomain should still be the PhotochromicCore contract
        //TODO write a test to validate that only the PhotochromicCore contract can change the ENSRegistry node record namehash.hash('userId.pctest.eth')
        ens.setSubnodeOwner(baseNode, label, msg.sender);
        hashes[tokenId] = ipfsHash;
        return tokenId;
    }

    function supportsInterface(bytes4 interfaceID) public view override(ERC721Enumerable) returns (bool) {
        return interfaceID == INTERFACE_META_ID || interfaceID == ERC721_ID || ERC721Enumerable.supportsInterface(interfaceID);
    }

    /**
     * user: photochromic user
     * userId: label (label.pctest.eth)
    ...
    function register(
        ...
        uint256 userIdInUint256 = uint256(keccak256(abi.encodePacked(userId)));
        uint256 result = _register(userIdInUint256, user, ipfsHash);
        return result;
    }
    ...
    /*
     * disable transferFrom ERC721 functions
     */

    string constant notAllowedMessage = "transfer is not allowed for this token";
