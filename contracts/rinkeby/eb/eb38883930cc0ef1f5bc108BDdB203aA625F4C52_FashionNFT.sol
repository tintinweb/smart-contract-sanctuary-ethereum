...
contract FashionNFT is ERC721A, Ownable {
    using Strings for uint256;

    uint256 public constant WHITELIST_MAX = 4400; // placeholder
    uint256 public constant RESERVE_MAX = 88; // placeholder
    uint256 public constant SALE_MAX = 4400; // placeholder
    uint256 public constant TOTAL_MAX = WHITELIST_MAX + RESERVE_MAX + SALE_MAX;
    uint256 public constant MAX_SALE_QUANTITY = 6; // placeholder
    uint256 public whitelistPrice = 0.19 ether; // placeholder
    uint256 public saleStartPrice = 5 ether; // placeholder
    ...
    string private baseURI = "https://xyz.com/api/metadata/"; // placeholder

    address private paymentAddress = 0xCe90a7949bb78892F159F428D0dC23a8E3584d75; // placeholder
    address private royaltyAddress = 0xCe90a7949bb78892F159F428D0dC23a8E3584d75; // placeholder
    uint96 private royaltyBasisPoints = 750; // placeholder
    ...
    /**
     * @notice mint whitelist
     * @dev whitelist should occur before sale, to ensure fairlaunch of numberMinted tracking
     */
    function mintWhitelist() external payable {
        require(whitelistActive, "WHITELIST_INACTIVE");
        uint256 quantity = whitelists[msg.sender];
        require(whitelistCount + quantity <= WHITELIST_MAX, "WHITELIST_MAXED");
        require(quantity > 0, "UNAUTHORIZED");
        require(msg.value >= whitelistPrice * quantity, "INSUFFICIENT_ETH");
    ...
    /**
     * @notice buy from sale
     */
    function buy(uint256 _quantity) external payable {
        require(saleActive, "SALE_INACTIVE");
        require(tx.origin == msg.sender, "NOT_EOA");
        require(
            numberMinted(msg.sender) + _quantity <= MAX_SALE_QUANTITY,
            "QUANTITY_MAXED"
        );
        require(
            (totalSupply() - (whitelistCount + reserveCount)) + _quantity <=
                SALE_MAX,
            "SALE_MAXED"
        );
        uint256 mintCost;
        // placeholders below
        if (block.timestamp - startTime >= 180 minutes) {
            mintCost = 0.5 ether * _quantity;
        } else {
            uint256 steps = (block.timestamp - startTime) / 10 minutes;
            mintCost = (saleStartPrice - (steps * 0.25 ether)) * _quantity;
        }
    ...
    function getCurrentPrice() external view returns (uint256) {
        ...
        if (block.timestamp - startTime >= 180 minutes) {
            ...
            uint256 steps = (block.timestamp - startTime) / 10 minutes;
            return saleStartPrice - (steps * 0.25 ether);
        }
    }

    /**
     * @notice return array of tokenIds of owner
     * @dev gas estimation may fail if owner holds many (+100) tokens
     */
     ...
     */
    function setSaleStartTime(uint32 _startTime) external onlyOwner {
        require(!saleActive);
     ...
     * @notice set base URI
     */
    function setBaseURI(string memory _baseURI) external notLocked onlyOwner {
        baseURI = _baseURI;
    }
     ...
     */
    function lockMetaData() external notLocked onlyOwner {
        metaDataLocked = true;
    }
