...
contract TerminalDirectory is ITerminalDirectory, Operatable {
  ...
  /// @notice For each project ID, the Dali terminal that the direct payment addresses are proxies for.
  ...
  /** 
      @notice 
      Update the Dali terminal that payments to direct payment addresses will be forwarded for the specified project ID.
...
contract DirectPaymentAddress is IDirectPaymentAddress {
    ...
    /** 
      @param _terminalDirectory A directory of a project's current Dali terminal to receive payments in.
