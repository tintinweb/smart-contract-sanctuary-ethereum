...
/**
A flock of 10,000 groovy animated parrots partying it up in the ether. Species
and Trait data for each parrot is dynamically generated and stored on-chain
within the smart contract and is accessible to anyone.

Flock Party art falls under the CC0 license and members of the flock are free
...
contract FlockParty is ERC721Enumerable, ReentrancyGuard, Ownable {
  ...
  string private constant NO_TRAIT_FLAG = "0";
  string private constant TRAIT_FLAG = "1";
  ...
  function mint(uint256 _count) public payable nonReentrant {
    uint256 totalMinted = currentId.current();
  ...
  function getTraits(uint256 _tokenId) public view returns (Traits memory) {
    ...
    traits.partiedTooHearty = _getPartiedTooHearty(_tokenId);
  ...
  function _getHead(uint256 _tokenId) internal view returns (string memory) {
    ...
    if (_isPartiedTooHearty(_tokenId)) return NO_TRAIT_FLAG;
  ...
  function _getEyes(uint256 _tokenId) internal view returns (string memory) {
    ...
    if (_stringToBytes(_getHead(_tokenId)) == _stringToBytes("HelmetAstronaut")) return "EyesOval";
  ...
  function _getBackground(uint256 _tokenId) internal view returns (string memory) {
    ...
    if (_isPartiedTooHearty(_tokenId)) return animatedBg[seed % animatedBg.length];
  ...
  function _getAccessory(uint256 _tokenId) internal view returns (string memory) {
    ...
    if (_isPartiedTooHearty(_tokenId)) return "NeckBrace";
    else if (_randomOdds(seed, 900)) return NO_TRAIT_FLAG;
    else return accessory[seed % accessory.length];
  }

  function _getPartiedTooHearty(uint256 _tokenId) internal view returns (string memory) {
    uint256 seed = _getTraitSeed(_tokenId, "PARTIED_TOO_HEARTY");
  ...
  function _isPartiedTooHearty(uint256 _tokenId) internal view returns (bool) {
    return _stringToBytes(_getPartiedTooHearty(_tokenId)) == _stringToBytes(TRAIT_FLAG);
  }
  ...
  function _getTraitSeed(uint256 _tokenId, string memory _traitName) internal view returns (uint256) {
    uint256 tokenIdSeed = _getTokenIdSeed(_tokenId);
  ...
  function _getTokenIdSeed(uint256 tokenId) internal view returns (uint256) {
