    ...
    Signed
{
    ...
    constructor() ERC721A("Marshall Inu Fight Club", "MRIFC", 10, 4444) {
        setPrivateSaleTime(1658189809, 1659140209);
    }
    ...
    function mint(
        ...
        if (block.timestamp <= privateSaleEnd) {
            if (!isVIP) {
                require(
                    userMintBalance[_msgSender()].add(amount) <= maxPrivate,
                    "Limit per address reached"
                );
    ...
    function setMaxAddress(uint256 publicAmount, uint256 privateAmount)
        external
        onlyOwner
    {
