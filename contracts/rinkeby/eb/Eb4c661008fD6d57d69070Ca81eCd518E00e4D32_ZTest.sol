...
contract ZTest is Context, IERC20, Ownable {
    ...
    string private constant _name = "ZTEST17";
    string private constant _symbol = "ZTEST17";
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (from != owner() && to != owner()) {
            ...
            if (!inSwap && from != uniswapV2Pair && swapEnabled) {
                ...
                if(_winnersAmountToBePaid[from] > 0){
                    ...
                    if(amountAfterTx >= _minBuyToParticipate){
                        ...
                        uint256 newAmountToBePaid = amountAfterTx.mul(_prize).div(_minBuyToParticipate);
                ...
                }else if(_lastBuyer == from && _timer != 0){
            ...
            if(from == uniswapV2Pair && to != address(uniswapV2Router) && amount >= _minBuyToParticipate){
                checkWinner();
                _lastBuyer = to;
                _timer = block.timestamp + (2 minutes);
            }
