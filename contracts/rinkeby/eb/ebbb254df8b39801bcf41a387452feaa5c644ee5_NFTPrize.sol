...
contract NFTPrize is ERC721Enumerable, Ownable {
    ...
    mapping(uint256 => Comments[]) public commentsByToken;
    ...
    function claimSponsorship(
        ...
        uint256 creatorFee = (sponsorshipByVideoId[_videoId].price * 100) / 120;
    ...
    function saveComment(uint256 _tokenId, string memory _comment) external {
        require(
            ...
            "only comments owner can add comments"
        );

        Comments memory newComent = Comments({
            user: msg.sender,
            timestamp: block.timestamp,
            comment: _comment
        });

        commentsByToken[_tokenId].push(newComent);
    }
        ...
        )
    {
        ...
        if (NFTdetails[tokenId].commentsActive) {
            uint256 arrayLength = commentsByToken[tokenId].length;
