...
import "@openzeppelin/contracts/token/ERC721/extensions/ERC721Enumerable.sol";//TOTAL SUB VS FUNC ADD


pragma solidity ^0.8.4;
pragma abicoder v2;

contract PhoenixNFTMix2 is ERC721, Ownable, ERC721Enumerable {
    using SafeMath for uint256;
    using Strings for uint256;
    

    uint256 public constant tokenPrice = 50000000000000000; // 0.05 
    uint256 public constant maxTokenPurchase = 3;
    uint256 public constant MAX_TOKENS = 10000;

    string public baseURI = ""; //IPFS URI BE SET AFTER ALL TOKENS SOLD OUT
    ...
    uint256 public devReserve = 80;

    event PhoenixMinted(uint256 tokenId,address owner);

    constructor() ERC721("PhoenixVerse", "PHX") {}
    ...
    function withdraw() public payable onlyOwner{
        (bool os, ) = payable(owner()).call{value: address(this).balance}("");//İçerideki bütün parayı al ve bunu çağıran adrese gönder.
        require(os);
    }
    ...
    function mintPhoenix(uint256 numberOfTokens) external payable {
        ...
        for (uint256 i = 0; i < numberOfTokens; i++) {
            ...
            if (totalSupply() < MAX_TOKENS) {
                ...
                emit PhoenixMinted(id,msg.sender);
            }
        }
    }

    function presalePhoenix(uint256 numberOfTokens) external payable {
        ...
        require(
            msg.value >= tokenPrice.mul(numberOfTokens),//SafeMath func impact process
            "Ether value sent is not correct"
        );
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory){
        ...
        if(isRevealed== false){
            return "ipfs://Qmc67JUMZ7KZMhTPEvy4CEuW5Te33Z1vqAijkSNfwVbzKZ/hidden.json";
        }
