...
contract RandomConsumer is VRFConsumerBaseV2, Ownable {
    VRFCoordinatorV2Interface immutable _coordinator;

    bytes32 public _gasLane;
    uint64 public _subscriptionId;
    uint16 public _requestConfirmations;
    uint32 public _callbackGasLimit;
    uint32 public _numWords;
    ...
    function updateGasLane(bytes32 newGasLane) public onlyOwner {
        _gasLane = newGasLane;
    }

    function updateRequestConfirmations(uint16 newRequestConfirmations) public onlyOwner {
        _requestConfirmations = newRequestConfirmations;
    }

    function updateCallbackGasLimit(uint32 newGasLimit) public onlyOwner {
        _callbackGasLimit = newGasLimit;
    }

    function updateNumWords(uint32 newNumWords) public onlyOwner {
