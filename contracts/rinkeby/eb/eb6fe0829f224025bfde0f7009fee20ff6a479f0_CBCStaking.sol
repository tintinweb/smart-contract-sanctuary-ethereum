...
contract CBCStaking is Ownable {
    ...
    mapping(uint256 => uint256[]) internal TokenIdTimeUnstaked;
    ...
    function stakeByIds(uint256[] memory tokenIds) public stakingEnabled {
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            ...
            TokenIdTimeStaked[id].push(block.timestamp);
            TokenIdToAddress[id] = msg.sender;
        }
    }

    function unstakeAll() public {
        require(getStakedCount(msg.sender) > 0, "You do not have any tokens staked!");

        for (uint256 i = AddressToTokenIds[msg.sender].length; i > 0; i--) {
            uint256 tokenId = AddressToTokenIds[msg.sender][i - 1];

            _IERC721.transferFrom(address(this), msg.sender, tokenId);
            AddressToTokenIds[msg.sender].pop();
            TokenIdToAddress[tokenId] = address(0);
            TokenIdTimeUnstaked[tokenId].push(block.timestamp);
        }
    ...
    function unstakeByIds(uint256[] memory tokenIds) public {
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            ...
            TokenIdTimeUnstaked[id].push(block.timestamp);
    ...
    function getBlockStaked(uint256 tokenId) public view returns (uint256[] memory) {
        return TokenIdTimeStaked[tokenId];
    }

    function getBlockUnstaked(uint256 tokenId) public view returns (uint256[] memory) {
        return TokenIdTimeUnstaked[tokenId];
    }
