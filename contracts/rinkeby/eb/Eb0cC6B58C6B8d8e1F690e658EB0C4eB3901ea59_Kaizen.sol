...
contract Kaizen is Ownable, ERC721A {
    ...
    uint256 public Rebels_PUBLIC = 1000;
    ...
    function gift(address[] calldata receivers) external onlyOwner {
        require(publicTokensMinted + receivers.length <= Rebels_PUBLIC, "EXCEED_TEAM_MAX");
        publicTokensMinted = publicTokensMinted + receivers.length;
        for (uint256 i = 0; i < receivers.length; i++) {
            _safeMint(receivers[i], 1);
        }
    }
    function founderMint(uint256 tokenQuantity) external onlyOwner {
        require(publicTokensMinted + tokenQuantity <= Rebels_PUBLIC, "EXCEED_TEAM_MAX");
        publicTokensMinted = publicTokensMinted + tokenQuantity;
    ...
    function claim(uint256 tokenQuantity) external {
        ...
        require(_totalMinted() + tokenQuantity <= Rebels_MAX, "EXCEED_MAX");
