...
contract QBURN is ERC721A, Ownable {

    uint256 public constant maxTokenSupply = 7777;
    ...
    event mintNFT(address owner, uint tokenUri);
    event WhitelistAdded(address indexed account);
    event WhitelistRemoved(address indexed account);
    
    /** end events*/

    constructor() ERC721A('QBURN', 'QBURN') {
        _saleStarted = false;
        isPublicSale = false;
    }

    function mint() public payable {
        require(_saleStarted, "Sale is not available now");
        ...
        emit mintNFT(_msgSender(), totalSupply());
    }

    function mintWhitelist() public payable {
        require(_saleStarted, "Sale is not available now");
        require(!isPublicSale, "Sale is not whitelist sale");
