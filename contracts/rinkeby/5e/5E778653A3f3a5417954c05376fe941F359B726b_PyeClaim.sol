...
contract PyeClaim is Ownable, ReentrancyGuard {
    ...
    uint256 public claimStartDate; //constant claimStartDate =  1655985600. This is exactly 30 days after startTime.
    ...
    function updateIndex() external {
        require(block.timestamp > claimDate && claimDateIndex < maxIndex && claimDateIndex >= minIndex); {
            claimDateIndex = block.timestamp.sub(claimStartDate).div(60);
            claimDate += 1 minutes;
        }
    }
    ...
    function claimTokens() external nonReentrant notContract() {
        require(getElapsedTime() > 8 minutes , "You have not waited the 30-day cliff period!");
        ...
        } else if (getElapsedTime() >= 8 minutes && getElapsedTime() < 10 minutes) {
            withdrawablePYE = (2 * tenPercent).sub(withdrawn);
            checkThenTransfer(withdrawablePYE, withdrawableApple, withdrawableCherry, available);

        } else if (getElapsedTime() >= 6 minutes && getElapsedTime() < 8 minutes) {
    ...
    function calculateWithdrawableAmounts(address _address) external view returns (uint256, uint256, uint256) {
        require(getElapsedTime() > 8 minutes , "You have not waited the 30-day cliff period! Your withdrawable amounts are 0.");
        ...
        if (getElapsedTime() >= 17 minutes) {withdrawablePYE = available;
        } else if (getElapsedTime() >= 16 minutes && getElapsedTime() < 17 minutes) {withdrawablePYE = (9 * tenPercent).sub(withdrawn);
        } else if (getElapsedTime() >= 15 minutes && getElapsedTime() < 16 minutes) {withdrawablePYE = (8 * tenPercent).sub(withdrawn);
        } else if (getElapsedTime() >= 14 minutes && getElapsedTime() < 15 minutes) {withdrawablePYE = (7 * tenPercent).sub(withdrawn);
        } else if (getElapsedTime() >= 13 minutes && getElapsedTime() < 14 minutes) {withdrawablePYE = (6 * tenPercent).sub(withdrawn);
        } else if (getElapsedTime() >= 12 minutes && getElapsedTime() < 13 minutes) {withdrawablePYE = (5 * tenPercent).sub(withdrawn);
        } else if (getElapsedTime() >= 11 minutes && getElapsedTime() < 12 minutes) {withdrawablePYE = (4 * tenPercent).sub(withdrawn);
        } else if (getElapsedTime() >= 10 minutes && getElapsedTime() < 11 minutes) {withdrawablePYE = (3 * tenPercent).sub(withdrawn);
        } else if (getElapsedTime() >= 8 minutes && getElapsedTime() < 10 minutes) {withdrawablePYE = (2 * tenPercent).sub(withdrawn);
        } else if (getElapsedTime() >= 6 minutes && getElapsedTime() < 8 minutes) {withdrawablePYE = tenPercent.sub(withdrawn);
