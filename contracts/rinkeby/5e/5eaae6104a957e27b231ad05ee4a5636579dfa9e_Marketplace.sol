...
abstract contract DirectSale is Initializable, Constants, Core, Payment {
    ...
    function listDirectSale(
        ...
        _setRoyalties(_directSaleId, royalties);
    ...
    function updateDirectSale(
        ...
        if (royalties.length > 0) {
            ...
            _setRoyalties(saleId, royalties);
        }
...
abstract contract Payment is Constants {
    ...
    function _splitPayment(
        ...
        uint256 royaltiesCut = _payToRoyaltiesIfExist(saleId, price);

        uint256 _sellerCut = (price) - (dissrupCut + royaltiesCut);
    ...
    function _checkRoyalties(Royalty[] calldata royalties) internal pure {
        uint256 totalShares;

        for (uint256 i = 0; i < royalties.length; i++) {
            totalShares = totalShares + royalties[i].share;
        }
        // dissrup cut is 15%
        if (totalShares > 85) {
    ...
    function _setRoyalties(uint256 saleId, Royalty[] calldata royalties)
        internal
    {
        for (uint256 i = 0; i < royalties.length; i++) {
            Royalty memory royalty = Royalty({
                payee: royalties[i].payee,
                share: royalties[i].share
            });
