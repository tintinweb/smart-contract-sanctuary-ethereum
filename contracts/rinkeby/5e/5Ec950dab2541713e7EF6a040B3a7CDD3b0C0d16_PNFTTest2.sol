...
contract PNFTTest2 is ERC721A , Ownable{  
    constructor() ERC721A("PNFTTest2", "PNFT2") {}  

    using Counters for Counters.Counter;
    using Strings for uint256;

    address payable private _PaymentAddress = payable(0x9bf8FaE6B3A5EB044A92d0d4Dd270c1C76b7DCEf);

    Counters.Counter private _publicPNFT;
    uint256 private _PNFT_MAX = 1337;
    uint256 private _activeDateTime = 1656524929; 
    string private _tokenBaseURI = "";
    string private _revealURI = "";
	address private _stakeContract;
    
    uint256 private PRESALE_PRICE = 0.02 ether;
    uint256 private PUBLIC_PRICE = 0.03 ether;
    ...
    uint256 private MAX_AGE = 3; 				// max age of a dog 
	uint256 private TOKEN_FACTOR = 10000;		//100000000	// 100m Pochi Tokens base rate per growth age
	uint256 private TIME_FACTOR = 600;			//604800		// 1 week phase
	uint256 private BURN_FACTOR_NEW = 4;		// burn factor for permanent adulthood on new Pochi (25%)
	...
	// Grow your Pochi puppy by Feeding it (Staking Pochi Inu Tokens)
	function giveFeed(uint256 tokenId, uint256 feedAmt, address tokenOwner) public {
        ...
        _feeds[tokenId] = Feed(block.timestamp, feedAmt, tokenOwner, 0, 0);
		emit AddFeed(tokenId, feedAmt, block.timestamp);
    }
	
	// Remove Feed from your Pochi puppy (Un-Staking Pochi Inu Tokens)
	...
	function setBurnFactorNew(uint256 burnFactor) external onlyOwner {
        BURN_FACTOR_NEW = burnFactor;
    }
	
	function setBurnFactorPost(uint256 burnFactor) external onlyOwner {
        BURN_FACTOR_POST = burnFactor;
    }
    ...
    function setPublicsaleMintMaxLimit(uint256 publicsaleMintLimit) external onlyOwner {
    ...
    function getBurnAmount(uint256 tokenId) public view returns(uint256){
        if (_feeds[tokenId].amt > 0){
			...
			if (curAge == MAX_AGE) {
				return (tokenFeed.amt / BURN_FACTOR_POST);
			}
		}
    ...
    function calcAge(uint256 tokenId) public view returns(uint256){
		...
		if (tokenFeed.amt == 0){
			curAge = tokenFeed.ageTimeOut - ((block.timestamp - tokenFeed.timeOut) / TIME_FACTOR);
			return curAge;
		}
		...
		curAge = (block.timestamp - tokenFeed.timeIn) / totalFeedAmt;
	...
	function timeUntilNextAge(uint256 tokenId) public view returns(uint256){
		...
		divFactor = (divFactor == 0) ? 1 : divFactor;
		uint256 totalFeedAmt = divFactor * TIME_FACTOR;
		uint256 curAge = (block.timestamp - tokenFeed.timeIn) / totalFeedAmt;
		
		if (curAge == MAX_AGE){
			return 0;
		}
		
		uint256 timeElapsed = block.timestamp - tokenFeed.timeIn;
		if ((totalFeedAmt * (curAge + 1)) > timeElapsed) {
			uint256 timeRemaining = (totalFeedAmt * (curAge + 1)) - timeElapsed;
	...
	function isPresaleMintActive() public view returns (bool) {
		if (block.timestamp > _activeDateTime - PRESALE_HOURS) {
			return true;
		} else {
			return false;
		}
	...
	function _beforeTokenTransfers(address from, address to, uint256 startTokenId, uint256 quantity) internal virtual override(ERC721A) {
		...
		for(uint256 i = startTokenId; i < startTokenId + quantity; i++) {
			if (_feeds[i].amt > 0){
				uint256 curAge = calcAge(i);
				Feed memory tokenFeed = _feeds[i];
				tokenFeed.amt = 0;
				tokenFeed.timeOut = block.timestamp;
				tokenFeed.ageTimeOut = curAge;
				_feeds[i] = tokenFeed;
