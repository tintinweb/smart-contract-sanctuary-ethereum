...
contract Reptilands is ERC721, Pausable, AccessControl {
    ...
    uint256 public limitSecondMint = 4097;
    ...
    modifier mintCompliance(uint256 _mintAmount) {
        require(state != State.NOT_STARTED);
    ...
    modifier mintAllowance(uint256 _mintAmount, address _objective) {
        ...
        if (state == State.FIRST_WHITELIST) {
            ...
            require(_mintAmount <= objectiveFirstWhitelist);
            require(actualSupply + _mintAmount <= limitFirstMint);
            ...
            require(_mintAmount <= objectiveSecondWhitelist);
            require(actualSupply + _mintAmount <= limitSecondMint);
        } else {
            require(actualSupply + _mintAmount <= limitSecondMint);
        }
