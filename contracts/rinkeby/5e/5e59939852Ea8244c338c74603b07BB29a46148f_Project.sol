...
contract Project is Initializable, OwnableUpgradeable {
    ...
    function initialize(address _owner, address _nftChecker) external initializer {
        ...
        require(_nftChecker != address(0), "Invalid nftChecker address");
    ...
    event SetAdmin(address account, bool allow);
    ...
    event SetIDO(IDO params, ProjectStatus indexed status);
    event RequestApproval(uint256 indexed projectId, uint256 indexed profitShare);
    event SetApproval(uint256 indexed projectId, bool isApproval);
    ...
    function setJoinTime(uint256 _projectId, uint256 _start, uint256 _end) external projectExists(_projectId) onlyManager(_projectId) {
        ...
        require(project.status == ProjectStatus.STARTED, "Invalid status");
    ...
    function setIDO(IDO memory _params) external projectExists(_params.projectId) onlyManager(_params.projectId) {
        ProjectInfo storage project = projects[_params.projectId];
        require(project.status == ProjectStatus.INACTIVE, "Invalid status");
        require(sale.getSalesProject(_params.projectId).length > 0, "The project has no products for sale");
        require(_params.joinStart >= block.timestamp && _params.joinStart < _params.joinEnd, "Invalid join start");
        require(_params.saleStart > _params.joinEnd && _params.saleStart < _params.saleEnd, "Invalid sale start");

        if (_params.minSales > 0) {
            require(!_params.isInstantPayment, "Invalid isInstantPayment");
            require(_params.minSales <= project.amount, "Invalid minSales");
            project.minSales = _params.minSales;
        }

        project.joinStart = _params.joinStart;
        project.joinEnd   = _params.joinEnd;
        project.saleStart = _params.saleStart;
        project.saleEnd   = _params.saleEnd;
        project.isInstantPayment  = _params.isInstantPayment;
        if (project.isCreatedByAdmin) project.status = ProjectStatus.STARTED;
        emit SetIDO(_params, project.status);
    }

    function setTokenAmount(uint256 _projectId, uint256 _amount) external projectExists(_projectId) onlySale {
        projects[_projectId].amount = _amount;
    }
    ...
    function setApproval(uint256 _projectId, bool _isApproval) external projectExists(_projectId) onlyAdmin {
        require(!projects[_projectId].isCreatedByAdmin, "Cannot set approve project created by admin");
        require(_isApproval != approvals[_projectId].isApproved, "Approval value already exists");
        ...
        approvals[_projectId].isApproved = _isApproval;
        projects[_projectId].status = _isApproval ? ProjectStatus.STARTED : ProjectStatus.INACTIVE;
        emit SetApproval(_projectId, _isApproval);
    }         
