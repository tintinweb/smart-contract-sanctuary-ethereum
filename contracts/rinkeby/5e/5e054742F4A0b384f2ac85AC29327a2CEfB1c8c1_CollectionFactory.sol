...
contract CollectionFactory is NonblockingLzApp {
    ...
    function mintCollection(
        ...
        require(chainSymbolToId[chainSymbol] > 0, "Invalid chain symbol");

        address tokenOwner = destinationAddress != address(0) ? destinationAddress : msg.sender;

        if (chainSymbolToId[chainSymbol] == contractChainId) {
            ...
            uint16 mappedChainId = uint16(chainSymbolToId[chainSymbol]);
...
contract CollectionsRepository {
    ...
    function addCollection(
        ...
        uint256 _dropFrom,
        uint256 _dropTo
    ) external returns (address) { // TODO: (Must) onlyFactory
...
contract CollectionFactoryLzReceiver is ILayerZeroReceiver {
    ...
    event LzOnCreateCollection();
    event CollectionCreated(address collectionAddress, address creator);

    address public factoryLzReceiverAddress;
    ...
    constructor(
        ...
        endpoint = ILayerZeroEndpoint(address(0x4D747149A57923Beb89f22E6B7B97f7D8c087A00)); // 0x4D747149A57923Beb89f22E6B7B97f7D8c087A00 Arbitrum Rinkeby
        _collectionsRepository = collectionsRepository;
        _factory = factory;
        factoryLzReceiverAddress = address(this);
        contractChainId = 10010;
        chainSymbolToId["eth"] = 10001;
        chainSymbolToId["bsc"] = 10002;
        chainSymbolToId["avalanche"] = 10006;
        chainSymbolToId["polygon"] = 10009;
    ...
    function setRemote(uint16 _chainId, bytes calldata _remoteAddress) external onlyFactory {
        require(remotes[_chainId].length == 0, "The remote address has already been set for the chainId");
        remotes[_chainId] = _remoteAddress;
    }
