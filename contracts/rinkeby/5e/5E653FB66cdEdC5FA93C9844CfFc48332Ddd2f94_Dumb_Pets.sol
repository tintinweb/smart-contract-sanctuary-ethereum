...
// PlaySide Contracts
import "./Pets_ErrorCodes.sol";
import "./Pets_Settings.sol";

contract Dumb_Pets is 
	ERC721A,
	Pausable, 
	Ownable, 
	Pets_Settings {
	...
	function airdropPets(address[] calldata whitelist, uint256[] calldata whitelist_quanityPerUser) whenNotPaused public onlyOwner {

		if(whitelist.length <= 0 || whitelist_quanityPerUser.length != whitelist.length) revert Pets_ErrorCodes.ArrayMissmatch();
		...
		for(uint256 i = 0; i < whitelist.length; i++)
		{
			...
			if(targetQuantity <= 0) revert(); 

			// Ensure that each address passed into this function is a valid address
			if(targetAddress != address(targetAddress))
				revert Pets_ErrorCodes.AirDrop_AddressInvalid(targetAddress);

			safeMint(targetAddress, targetQuantity);
		}
	}
	
	function mintPurchase(uint256 _mintAmount) public payable whenNotPaused {
		
		// Ensure that the public mint is available
		if(Pets_Settings.publicMintActive == false) revert Pets_ErrorCodes.PublicMintNotActive();

		CheckRequires(_mintAmount, true);

		// Cache the number minted for gas optimisation
		uint256 amountMinted = addressMinted[msg.sender];
		
		// Check that this person has minted less than the max per wallet
		if(amountMinted + _mintAmount > Pets_Settings.maxMintPerWallet) 
			revert Pets_ErrorCodes.MaxAmountMinted(Pets_Settings.maxMintPerWallet, amountMinted + _mintAmount);
			
		safeMint(msg.sender, _mintAmount);
	}

	function safeMint(address to, uint256 quantity) internal {
		// Keep Track Of Total Minted
    ...
    function CheckRequires(
		...
		// Check Not Sold Out
		if(supply + mintAmount > Pets_Settings.maxSupply) revert Pets_ErrorCodes.SoldOut();

		// Check price of this message
		if((shouldCheckCost) && (msg.value < (mintCost * mintAmount))) revert Pets_ErrorCodes.InsufficientFunds(mintCost, msg.value);
    }
...
library Pets_ErrorCodes {
    // ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    // *                    ERROR MESSAGES
    // ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    // Mint Errors
    error AirDrop_AddressInvalid(address InvalidAddress);
    error SoldOut();
...
contract Pets_Settings is AccessControl, Roles
{
	...
	function setPublicMintActive(bool active) public {
