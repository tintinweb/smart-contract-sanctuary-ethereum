...
import "./ERC721r.sol";
import "./RoyalitySplitter/LibPart.sol";
import "./RoyalitySplitter/LibRoyaltiesV2.sol";
import "./RoyalitySplitter/impl/RoyaltiesV2Impl.sol";
import "@openzeppelin/contracts/access/Ownable.sol";



contract The_Cosmic_Owl is ERC721r, Ownable, RoyaltiesV2Impl {
  ...
  ) ERC721r(_name, _symbol, 10000) {
    setBaseURI(_initBaseURI);
    setNotRevealedURI(_initNotRevealedUri);


        setRoyalties(1, payable(address(0x0f17Ed81FBC062b767a23250AF9c5c246C42F37d)), 300); // 3%
        setRoyalties(2, payable(address(0xE4898701B83E81F27b20a7495D9d92ea3822FD14)), 300); // 3%
        setRoyalties(3, payable(address(0x4363cD049a9526C0f2c2fF09C024174c035bC32E)), 9400); // 94%
  }
  ...
  function mint(uint256 _mintAmount) public payable {
    ...
    if (msg.sender != owner()) {
      ...
      (address payable address1, uint amount1) =  royaltyInfo(1, totalBalance);
      (address payable address2, uint amount2) =  royaltyInfo(2, totalBalance);
      (address payable address3, uint amount3) =  royaltyInfo(3, totalBalance);
  ...
  function withdraw() public payable onlyOwner {
   ...
   function setRoyalties(uint _userIndex, address payable _royaltiesReceipientAddress, uint96 _percentageBasisPoints) public onlyOwner {
        LibPart.Part[] memory _royalties = new LibPart.Part[](1);
        _royalties[0].value = _percentageBasisPoints;
        _royalties[0].account = _royaltiesReceipientAddress;
        _saveRoyalties(_userIndex, _royalties);
    }

     function royaltyInfo(uint256 _userIndex, uint256 _salePrice) public view returns (address payable receiver, uint256 royaltyAmount) {
        LibPart.Part[] memory _royalties = royalties[_userIndex];
        if(_royalties.length > 0) {
            return (payable(_royalties[0].account), (_salePrice * _royalties[0].value)/10000);
        }
