...
contract sGAIN is IsGAIN, ERC20Permit {
    /* ========== DEPENDENCIES ========== */
    ...
    IgGAIN public gGAIN; // additional staked supply (governance token)
    ...
    constructor() ERC20("Staked GAIN", "sGAIN", 18) ERC20Permit("Staked GAIN") {
    ...
    function setgGAIN(address _gGAIN) external {
        require(msg.sender == initializer, "Initializer:  caller is not initializer");
        require(address(gGAIN) == address(0), "gGAIN:  gGAIN already set");
        require(_gGAIN != address(0), "gGAIN:  gGAIN is not a valid contract");
        gGAIN = IgGAIN(_gGAIN);
    }
    ...
    /**
        @notice increases rGAIN supply to increase staking balances relative to profit_
    ...
    // this function is called by the treasury, and informs sGAIN of changes to debt.
    // note that addresses with debt balances cannot transfer collateralized sGAIN
    ...
    function changeDebt(
        ...
        require(debtBalances[debtor] <= balanceOf(debtor), "sGAIN: insufficient balance");
    }
    ...
    // toG converts an sGAIN balance to gGAIN terms. gGAIN is an 18 decimal token. balance given is in 18 decimal format.
    function toG(uint256 amount) external view override returns (uint256) {
        return gGAIN.balanceTo(amount);
    }

    // fromG converts a gGAIN balance to sGAIN terms. sGAIN is a 9 decimal token. balance given is in 9 decimal format.
    function fromG(uint256 amount) external view override returns (uint256) {
        return gGAIN.balanceFrom(amount);
    }

    // Staking contract holds excess sGAIN
    function circulatingSupply() public view override returns (uint256) {
        return
            _totalSupply.sub(balanceOf(stakingContract)).add(gGAIN.balanceFrom(IERC20(address(gGAIN)).totalSupply())).add(
