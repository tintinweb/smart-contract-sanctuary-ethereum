 ...
 * @title Base contract for standard ERC721 token drops using an "initializable" pattern
 * @author Augminted Labs, LLC
 * @notice Contract has been optimized for security and fairness
 */
contract ERC721Offsetable is ERC721AQueryableUpgradeable, PaymentSplitterUpgradeable, OwnableUpgradeable, Offsetable {
    ...
    address internal constant _OFFSET_MANAGER = 0xAB5Db3E4D2760Acc62fb0F701eF91e3e8Aba5f2c;
    address payable internal constant _STRUCT_ADDRESS = payable(0xff467a85932cF543Df50255f00A8A829c12a3A11);
     ...
     */
    function setProvenanceHash(string calldata _provenanceHash) public lockable onlyOwner {
        if (config.bypassOffset) revert ERC721Offsetable_TokenOffsetBypassed();
     ...
     * @param _uri Placeholder or base token URI depending on revealed state
     */
    function setRevealed(bool _revealed, string calldata _uri) public lockable onlyOwner {
     ...
     */
    function setOffset(uint256 randomness) public virtual override {
        if (config.bypassOffset) revert ERC721Offsetable_TokenOffsetBypassed();
