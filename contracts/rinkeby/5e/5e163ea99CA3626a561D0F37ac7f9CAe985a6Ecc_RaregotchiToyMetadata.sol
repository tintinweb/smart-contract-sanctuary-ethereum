...
contract RaregotchiToyMetadata is Ownable {
    ...
    function unveil() external {
        ...
        require(unveiled == false, "Already unveiled");
        unveiled = true;
    }
    ...
    /**
     * @dev Set the packages baseUri
     */
    function setPackageBaseUri(string calldata _baseUri)
        external
        onlyOwner
        contractIsNotFrozen
    {
        packageBaseUri = _baseUri;
    }
    ...
    function tokenURI(uint256 _tokenId) external view returns (string memory) {
        ...
        string memory baseUri = unveiled == false ? veiledBaseUri : isOpen
            ? openBaseUri
            : unveiledBaseUri;
    ...
    function packageURI(uint256 tokenId) external view returns (string memory) {
        return
            string(abi.encodePacked(packageBaseUri, Strings.toString(tokenId)));
    }
}