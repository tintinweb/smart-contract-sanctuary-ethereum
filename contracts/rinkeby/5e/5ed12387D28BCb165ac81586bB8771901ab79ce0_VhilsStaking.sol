...
contract VhilsStaking is Ownable, ERC721Holder, ERC1155Holder {
  ...
  // TO DO: add way to get all holders of one tier
  ...
  function _withdrawForMedals(uint16 _tokenId) internal {
    ...
    _removeTokenFromStakedList(msg.sender, _tokenId); // remove from list after withdrawal
  }

  function _withdrawForTears(uint16 _tokenId) internal {
    uint24[3] memory stakingDays = [
      120 * 60 * 60, //
      70 * 60 * 60,
      40 * 60 * 60
    ];
  ...
  function _removeTokenFromStakedList(address _owner, uint16 _tokenId) internal {
    ...
    // find target and replace with last value, then pop last value since its removed
    uint16 _finalIndex = uint16(stakedLayers[_owner].length - 1); // final index
    if (_index != _finalIndex) { // no swap needed if same
