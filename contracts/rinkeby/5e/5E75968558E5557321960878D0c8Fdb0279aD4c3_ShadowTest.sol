...
contract ShadowTest is Ownable, ERC721A {
    using Strings for uint256;

    string public constant shadowSvg =
        // solhint-disable-next-line
        '<svg viewBox="0 0 220 100" xmlns="http://www.w3.org/2000/svg"><path style="fill:gold;box-shadow:0 25px 50px -12px #646464;-webkit-filter:drop-shadow(3px 3px 2px rgba(0,0,0,.7));filter:drop-shadow(3px 3px 2px rgba(0,0,0,.7))" d="M0 0h10v10H0z"/></svg>';

    error AirdropQuantityZero();

    constructor() ERC721A("ShadowTest", "ST") {
    ...
    function constructTokenURI(uint256 tokenId) public pure returns (string memory) {
        ...
        string memory name = string(abi.encodePacked("ShadowTest", tokenId.toString()));
        string memory description = string(abi.encodePacked("ShadowTest", tokenId.toString()));
                ...
                'data:application/json;base64,',
                    Base64.encode((bytes.concat(
                        ...
                        '{',
                            ...
                            '", "image": "data:image/svg+xml;base64,', Base64.encode(bytes(shadowSvg)),
                        '}')
                    )))
            )
        );
