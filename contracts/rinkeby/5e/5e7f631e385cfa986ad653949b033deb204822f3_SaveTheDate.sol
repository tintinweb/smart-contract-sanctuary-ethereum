...
contract SaveTheDate is ERC721, Ownable, ReentrancyGuard {
    ...
    mapping(uint256 => DateToken) private mintedDates;
     ...
     */
    function checkDateAvailability(DateToMint calldata date) public returns (bool) {
        DateToken memory token = mintedDates[_tokenDateIndex(date)];
    ...
    function mintDate(DateToMint calldata tokenDate, bytes32[] calldata merkleProof) external payable ensureAvailability nonReentrant returns (uint256) {
        ...
        DateToken memory token = mintedDates[_tokenDateIndex(tokenDate)];
        ...
        _setTokenDate(tokenDate, tokenId);
    ...
    function mintDonatedDates(DateToMint[] calldata tokenDates, address[] calldata recipients) external ensureAvailability onlyOwner {
        ...
        for (uint i = 0; i < recipients.length; i++) {
            ...
            DateToken memory token = mintedDates[_tokenDateIndex(tokenDates[i])];
            require(token.tokenId == 0 , "Date already minted");
            ...
            _setTokenDateURI(tokenId, _tokenDate);
            _setTokenDate(tokenDates[i], tokenId);
    ...
    function reserveDates(DateToMint[] calldata bookedDates, address[] calldata bookers) external onlyOwner {
        ...
        for (uint i = 0; i < bookedDates.length; i++) {
            ...
            DateToken memory token = mintedDates[_tokenDateIndex(bookedDates[i])];
    ...
    function _setTokenDate(DateToMint memory tokenDate, uint256 tokenId) internal virtual {
        mintedDates[_tokenDateIndex(tokenDate)].tokenId = tokenId;
    }

    function _tokenDateIndex(DateToMint memory tokenDate) internal virtual returns (uint256) {
        return tokenDate.year + tokenDate.month + tokenDate.day;
    }
