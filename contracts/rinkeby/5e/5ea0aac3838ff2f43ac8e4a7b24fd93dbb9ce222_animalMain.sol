...
contract animalMain is ERC721, Ownable {
    ...
    mapping(uint256 => address) public animalToOwner; //每隻動物都有一個獨一無二的編號，呼叫此mapping，得到相對應的主人
    mapping(address => uint256) ownerAnimalCount; //回傳某帳號底下的動物數量
    mapping(uint256 => address) animalApprovals; //和 ERC721 一樣，是否同意被轉走
    ...
    event Take(address _to, address _from, uint256 _tokenId);
    event Create(uint256 _tokenId, bytes32 dna, uint8 star, uint16 roletype);
    ...
    function checkAllOwner(uint256[] _tokenId, address owner)
        public
        view
        returns (bool)
    {
    ...
    function seeAnimalRole(uint256 _tokenId)
        public
        view
        returns (uint16 roletype)
    {
        return animals[_tokenId].roletype;
    }

    function getAnimalByOwner(address _owner)
        external
        view
        returns (uint256[])
    {
        //此方法回傳所有帳戶內的"動物ID"
        uint256[] memory result = new uint256[](ownerAnimalCount[_owner]);
        uint256 counter = 0;
    ...
    function createAnimal() public payable{
        ...
        uint256 price = 1e18;
        require(msg.value >= price, "You need to pay over 1ETH");

        uint256 id = animals.push(animal(dna, uint8(star), uint8(roletype))) - 1;
