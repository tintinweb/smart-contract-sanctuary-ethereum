...
contract LaCosaNostra is ERC20("La Cosa Nostrak", "LCN"), Ownable {
    ...
    function getStakedTokenOfUser() public view returns(uint256[] memory ) {
        uint256[] memory stakedToken;
        // uint256 userNftBalance = iERC721Call.balanceOf(msg.sender);/
        // for(uint i; i < userNftBalance; i++){
        // for(uint i; i < 1; i++){
            // uint256 _tokenId = iERC721Call.tokenOfOwnerByIndex(msg.sender, i);
