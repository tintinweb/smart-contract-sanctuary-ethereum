...
contract Nova is ERC721Enumerable, Ownable {
   ...
   constructor() ERC721("Nova", "$NOVA") {
    ...
    function mint() public payable notPaused {
        ...
        require(addressMintedBalance[msg.sender] == 0, "You've already minted!");
    ...
    function getAddresses() public view onlyOwner returns(address[] memory){
     ...
     function useRandomAvailableToken() internal view returns (uint256){
        uint256  _numAvailableTokens = 2022;
        uint[] memory _availableTokens = new uint[](_numAvailableTokens);
