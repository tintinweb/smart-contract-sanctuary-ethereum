...
contract DefinitelyNothing is ERC721A, Ownable {
    ...
    function mintPublic(uint256 _quantity) public payable {
        require(totalSupply() + _quantity <= MAX_SUPPLY - MAX_TEAM_MINT, "Surpasses supply");
        ...
        require(_quantity > 0 && publicAddressMintCount[msg.sender] + _quantity <= MAX_PER_WALLET,"Minting above public limit");
         if (publicAddressMintCount[msg.sender] > 0) {
            require(msg.value >= _quantity * MINT_PRICE, "Insufficient funds");
        } 
         else {
            require(msg.value >= (_quantity - 1) * MINT_PRICE, "Insufficient funds");
        }
