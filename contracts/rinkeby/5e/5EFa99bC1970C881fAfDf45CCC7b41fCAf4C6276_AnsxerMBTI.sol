...
contract AnsxerMBTI is ERC721AQueryable, Ownable, Pausable {
  event Staked(address owner, uint256[] tokenIds);
  event Unstaked(address owner, uint256[] tokenIds);
  ...
  function teamMint() external onlyOwner {
    ...
    _mintMBTI(teamAddress, TEAM_TOTAL_RESERVED);
  }
  ...
  function allowlistMint(Order[] calldata orders, bool staked, uint256 v, bytes32 r, bytes32 s) external whenNotPaused callerIsUser {
    ...
    uint256 totalAmount = _getTotalAmount(orders, staked);
    ...
    _mintMBTI(msg.sender, totalAmount);
  }
  ...
  function _getTotalAmount(Order[] calldata orders, bool staked) internal returns (uint256) {
    uint256 totalAmount;
    ...
    for (uint i; i < orders.length; i++) {
      ...
      for (uint n; n < amount; n++) {
        ...
        if (staked) {
          stakeAt[startTokenId + totalAmount + n] = timestamp;
        }
      }
  ...
  function _mintMBTI(address to, uint256 amount) internal {
    _safeMint(to, amount);
