...
import { Offer } from "./libraries/bonds/Offer.sol";
import { Bond } from "./libraries/bonds/Bond.sol";
import { Constants } from "./libraries/Constants.sol";

/// @title ERC721 Bonds Registry
/// @notice Bonds holding underlying tokens are minted through this contract.
/// @dev Bonds registry supports 2 types of users:
/// 1. Users holding a vesting position in the vesting contract - Locked token holders.
/// 2. Users interested in purchasing locked underlying using purchase token (originally
/// designed to use USDC) - Bonders.
/// @dev When providing purchase tokens through the {buy} function, bonders mint an ERC721
/// bond NFT, which holds a payout amount of locked tokens and registers an expiration timestamp,
/// which is when the bonder is able to burn the NFT in exchange for the locked underlying (ILV tokens).
/// @dev Bonds registry uses an oracle contract, which gets data from Chainlink's Aggregator and applies
/// a valid discount to determine underlying tokens bond price.
    ...
    PausableUpgradeable
{
    ...
    /// @dev ERC20 token used for purchases (USDC)
    IERC20Upgradeable public purchaseToken;
    /// @dev Price oracle address
    IOracle public oracle;
    /// @dev ERC721 staking contract address
    IStakingRewards public staking;
    /// @dev Total amount of underlying tokens offered
    uint256 public underlyingSupplied;
    /// @dev Amount of purchase tokens available per underlying supplied
    /// @dev Offers have their payout updated through the {updateOffer} modifier
    uint256 public payoutPerToken;
    /// @dev Amount of underlying that needs to be removed from offers
    /// @dev This value is used to reduce an offer balance in its next interaction
    /// through the {updateOffer} modifier
    uint256 public debtPerToken;
    /// @dev Tracks token id to be used for next minted  bond
    CountersUpgradeable.Counter public bondIdTracker;
    /// @dev Value used to store baseURI returned in {ERC721Upgradeable._baseURI}
    string internal baseURI_;
    /// @inheritdoc IBondsRegistry
    mapping(uint256 => Offer.Data) public offers;
    /// @inheritdoc IBondsRegistry
    mapping(uint256 => Bond.Data) public bonds;
    /// @inheritdoc IBondsRegistry
    mapping(uint256 => uint256) public discountOptions;

    /* ======== MODIFIERS ======== */
    /// @dev Updates offer payout and balance values
    modifier updateOffer(uint256 _offerId) {
        Offer.Data storage _offer = offers[_offerId];
        _offer.payout = _offer.payoutEarned(payoutPerToken).toUint128();
        _offer.balance -= _offer.pendingDebt(debtPerToken).toUint128();
        _offer.payoutPerTokenPaid = payoutPerToken;
        _offer.debtPerTokenApplied = debtPerToken;
        _;
    }

    /* ======== EVENTS ======== */
    event LogSetOracle(address oldOracle, address newOracle);
    event LogSetStaking(address oldStaking, address newStaking);
    event LogSetDiscounts(DiscountOption[] discountOptions);
    event LogOffer(address indexed caller, uint256 indexed offerId, uint256 value);
    event LogResign(address indexed caller, uint256 indexed offerId, uint256 value);
    event LogBuy(
        address indexed bonder,
        uint256 indexed bondId,
        uint256 purchaseTokenIn,
        uint256 underlyingOut,
        uint256 duration,
        bool autostake
    );
    event LogClaimOfferPayout(address indexed caller, uint256 indexed offerId, uint256 payout);
    event LogClaimBondPayout(address indexed bonder, uint256 indexed bondId, uint256 payout);
    ...
    /// @inheritdoc IBondsRegistry
    function payoutEarnedFor(uint256 _offerId) external view virtual returns (uint256) {
        return offers[_offerId].payoutEarned(payoutPerToken);
    }

    /// @inheritdoc IBondsRegistry
    function availableBalanceFor(uint256 _offerId) external view virtual returns (uint128) {
        return offers[_offerId].balance - (offers[_offerId].pendingDebt(debtPerToken)).toUint128();
    }

    /// @inheritdoc IBondsRegistry
    function setOracle(IOracle _oracle) external virtual onlyOwner {
        require(address(_oracle) != address(0), "invalid _oracle");
        emit LogSetOracle(address(oracle), address(_oracle));
        oracle = _oracle;
    }

    /// @inheritdoc IBondsRegistry
    function setStaking(IStakingRewards _staking) external virtual onlyOwner {
        require(address(_staking) != address(0), "invalid _staking");
        emit LogSetStaking(address(staking), address(_staking));
        staking = _staking;
    }

    /// @inheritdoc IBondsRegistry
    function setDiscounts(DiscountOption[] calldata _discountOptions) external virtual onlyOwner {
        for (uint256 i = 0; i < _discountOptions.length; i++) {
            require(
                _discountOptions[i].lockDuration > Constants.MINIMUM_LOCK_DURATION &&
                    _discountOptions[i].discount < Constants.DISCOUNT_BASE &&
                    _discountOptions[i].discount > 0,
                "invalid discount provided"
            );
            discountOptions[uint256(_discountOptions[i].lockDuration)] = uint256(_discountOptions[i].discount);
        }
        emit LogSetDiscounts(_discountOptions);
    }
    ...
    /// @inheritdoc IBondsRegistry
    function offer(uint256 _offerId, uint256 _value) external virtual whenNotPaused updateOffer(_offerId) {
        underlyingSupplied += _value;
        offers[_offerId].balance += _value.toUint128();

        vesting.afterUnderlyingOffer(msg.sender, _offerId, _value);
        emit LogOffer(msg.sender, _offerId, _value);
    }

    /// @inheritdoc IBondsRegistry
    function resign(uint256 _offerId, uint256 _value) external virtual whenNotPaused updateOffer(_offerId) {
        offers[_offerId].balance -= _value.toUint128();
        uint256 currentUnderlyingSupplied = underlyingSupplied;
        // we assume _value is valid if offer.balance didn't underflow
        unchecked {
            underlyingSupplied = _value >= currentUnderlyingSupplied ? 0 : currentUnderlyingSupplied - _value;
        }
        vesting.afterOfferResignation(msg.sender, _offerId, _value);

        underlying.safeTransfer(address(vesting), _value);
        emit LogResign(msg.sender, _offerId, _value);
    }
    ...
    function buy(
        ...
        uint256 selectedDiscount = discountOptions[_duration];
        require(_value > 0 && selectedDiscount > 0 && underlyingSupplied > 0, "invalid state or inputs");
        uint256 underlyingOut = oracle.purchaseTokenToUnderlying(_value, selectedDiscount);
        require(underlyingOut >= _minUnderlyingOut && underlyingOut <= underlyingSupplied, "not enough underlyingOut");
        bondIdTracker.increment();
        uint256 currentTokenId = bondIdTracker.current();
        _mint(_autostake ? address(this) : msg.sender, currentTokenId);
        bonds[currentTokenId] = Bond.Data({
            payout: underlyingOut.toUint128(),
            expiresAt: (block.timestamp + _duration).toUint128()
        });
        payoutPerToken += (_value * Constants.BASE_MULTIPLIER) / underlyingSupplied;
        debtPerToken += (underlyingOut * Constants.BASE_MULTIPLIER) / underlyingSupplied;
        underlyingSupplied -= underlyingOut;
        if (_autostake) {
            _approve(address(staking), currentTokenId);
            staking.stake(msg.sender, currentTokenId);
        }
        purchaseToken.safeTransferFrom(msg.sender, address(this), _value);

        emit LogBuy(msg.sender, currentTokenId, _value, underlyingOut, _duration, _autostake);
    }

    /// @inheritdoc IBondsRegistry
    function claimOfferPayout(uint256 _offerId) external virtual whenNotPaused updateOffer(_offerId) {
        require(vesting.isApprovedOrOwner(msg.sender, _offerId), "unauthorized");
        uint256 pendingPayout = offers[_offerId].payout;
        offers[_offerId].payout = 0;
        address offerOwner = IERC721Upgradeable(address(vesting)).ownerOf(_offerId);

        purchaseToken.safeTransfer(offerOwner, pendingPayout);
        emit LogClaimOfferPayout(msg.sender, _offerId, pendingPayout);
    }

    /// @inheritdoc IBondsRegistry
    function claimBondPayout(uint256 _bondId) external virtual whenNotPaused {
        require(msg.sender == ownerOf(_bondId) && block.timestamp > bonds[_bondId].expiresAt, "unauthorized");
        uint256 bondPayout = bonds[_bondId].payout;
        delete bonds[_bondId];
        _burn(_bondId);

        underlying.safeTransfer(msg.sender, bondPayout);
        emit LogClaimBondPayout(msg.sender, _bondId, bondPayout);
    }
...
library Offer {
    /// @param balance amount of locked tokens offered
    /// @param payout amount of purchase tokens accumulated from sold balance
    /// @param payoutPerTokenPaid last payourPerToken applied to the offer
    /// @param debtPerTokenApplied last debt per token applied to the offer
    struct Data {
        uint128 balance;
        uint128 payout;
        uint256 payoutPerTokenPaid;
        uint256 debtPerTokenApplied;
    }

    /// @dev Calculates payout earned for a given offer using the latest payoutPerToken value.
    /// @param _offer storage pointer to offer
    /// @param _payoutPerToken latest payoutPerToken value
    function payoutEarned(Data storage _offer, uint256 _payoutPerToken) internal view returns (uint256) {
        return
            (_offer.balance * (_payoutPerToken - _offer.payoutPerTokenPaid)) /
            Constants.BASE_MULTIPLIER +
            _offer.payout;
    }

    /// @dev Calculates the amount of tokens that need to be removed from an offer's balance,
    /// due to underlying tokens being sold to bonders.
    /// @param _offer storage pointer to offer
    /// @param _debtPerToken latest debtPerToken value
    function pendingDebt(Data storage _offer, uint256 _debtPerToken) internal view returns (uint256) {
        uint256 balance = _offer.balance;
        uint256 expectedDebt = (balance * (_debtPerToken - _offer.debtPerTokenApplied)) / Constants.BASE_MULTIPLIER;

        return expectedDebt > balance ? balance : expectedDebt;
    }
}
