...
contract GAX_FNFT is ERC721Upgradeable {
    ...
    function mintToken(
        ...
        // nft mint
        _safeMint(gaxAdmin, tokenId);
        _setTokenURI(tokenId, _uri);

        // erc20 mint
        GAX_Artbits ERC20Instance = new GAX_Artbits();
        address ERC20InstanceAdr = address(ERC20Instance);
        ERC20Instance.initialize(
            name,
            symbol,
            _noOfArtbits,
            gaxAdmin,
        ...
        nftAssets[tokenId] = newNft;
        emit ERC20InstanceCreated(tokenId, ERC20InstanceAdr, _uri);
    }
    ...
    function transferToken(
        ...
        GAX_Artbits ERC20Instance = getERC20Instance(_assetId);
        ERC20Instance.transferPlatform(msg.sender, _to, _artBits);
        emit ArtbitTransferred(msg.sender, _to, _assetId, _artBits);
    }

    function getERC20Instance(uint256 _nftAssetId)
        public
        view
        returns (GAX_Artbits)
    {
        NFT_Asset memory nftAsset = nftAssets[_nftAssetId];
        GAX_Artbits ERC20Instance = GAX_Artbits(nftAsset.erc20_addr);
        ...
        returns (uint256)
    {
        NFT_Asset memory nftAsset = nftAssets[_nftAssetId];
        GAX_Artbits ERC20Instance = GAX_Artbits(nftAsset.erc20_addr);
        return ERC20Instance.balanceOf(userAddr);
    }
    ...
    function addCommissionAmt(uint256 amt) external {
        totalCommission += amt;
    }
    ...
    function addRoyaltyAmt(uint256 amt) external {
        totalRoyalty += amt;
    }
...
contract GAX_Artbits is ERC20Upgradeable {
    uint256 nftId;
    uint256 dex_rp;
    uint256 p2p_rp;
    ...
    event TransferFromCalled(address sender, address receiver, uint256 amount);
    ...
    modifier onlyAdmin() {
        require(msg.sender == gaxInstance.getAdmin(), "Sender is not Admin.");
        _;
    }
    ...
    function transferFrom(
        ...
        emit TransferFromCalled(sender, recipient, amount);
        return true;
    }
    ...
    function transferArtbits(
        ...
        address gaxAdmin = gaxInstance.getAdmin();
        ...
        // DEX transaction NFT_Asset owner receive royalty
            ...
            gaxInstance.isDexAddress(sender) ||
            gaxInstance.isDexAddress(recipient)
        ) {
            uint256 royalty_amt = (dex_rp * amount) / 100; // admin royalty amt
            _transfer(sender, nftOwner, royalty_amt);
            gaxInstance.addRoyaltyAmt(royalty_amt); // add royalty amount to NFT_Asset owner wallet

            transferAmount = amount - royalty_amt; // AB sent to client
            _transfer(sender, recipient, transferAmount);
        } else {
            uint256 commission = (p2p_rp * amount) / 100; // admin commission amt
            _transfer(sender, nftOwner, commission);

            transferAmount = amount - commission; // AB sent to client
            gaxInstance.addCommissionAmt(commission); // add commission amount to NFT_Asset owner wallet
            _transfer(sender, recipient, transferAmount);
        }
