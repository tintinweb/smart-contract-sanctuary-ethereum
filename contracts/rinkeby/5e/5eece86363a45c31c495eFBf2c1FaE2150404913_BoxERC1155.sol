...
import "./BoxRandomness.sol";

contract BoxERC1155 is ERC1155ForBox, ReentrancyGuard {
  using BoxRandomness for BoxRandomness.BoxRandomnessState;
  using SafeMath for uint256;

  BoxRandomness.BoxRandomnessState state;

  constructor (string memory _baseTokenURI)
    ERC1155ForBox("Box ERC1155", "BERC1155") {
  ...
  function setState(
    ...
    BoxRandomness.initState(state, _factoryAddress, _numOptions, _numClasses, _seed);
  }

  function setTokenIdsForClass(uint256 _classId, uint256[] memory _tokenIds) public onlyOwner {
    BoxRandomness.setTokenIdsForClass(state, _classId, _tokenIds);
  }
  ...
  function setOptionSettings(
    ...
    BoxRandomness.setOptionSettings(state, _option, _maxQuantityPerOpen, _classProbabilities, _guarantees);
  }

  function unpack(uint256 _optionId, address _toAddress, uint256 _amount) external {
    _burn(_msgSender(), _optionId, _amount);
    BoxRandomness._mint(state, _optionId, _toAddress, _amount, "", address(this));
  }
  ...
  function mint(
    ...
    require(_optionId < state.numOptions, "BoxERC1155: invalid option");
