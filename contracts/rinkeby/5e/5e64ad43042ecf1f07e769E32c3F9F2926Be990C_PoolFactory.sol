...
contract PoolFactory is OwnableUpgradeable {
    ...
    function transferStake(address receiver) external {
        ManagerInfo storage info = managerInfo[getPoolManager(msg.sender)];
    ...
    /**
     * @notice Function returns manager of the given pool by it's address and check if sender if manager
     * @param pool Address of the pool to get manager for
     * @return manager Pool's manager
     */
        ...
        returns (address manager)
    {
        ...
        require(msg.sender == managerInfo[manager].pool, "SNP");
    }
    ...
    function _proposePool(address currency) private {
        ...
        info.proposalId = uint32(flashGovernor.propose());
