...
contract LL420BudReveal is Ownable, Pausable, ReentrancyGuard {
    ...
    function setBudTHCs(
        ...
        require(_proofs.length == _proofFlags.length && _proofs.length > 0, "Merkle proofs not provided");

        bytes32[] memory nodes = new bytes32[](_ids.length);
        for (uint256 i = 0; i < _ids.length; i++) {
            nodes[i] = keccak256(abi.encodePacked(_ids[i], _thcs[i]));
        }

        nodes = ArraySort.sort(nodes);

        require(MerkleMultiProof.verifyMultiProof(merkleRoot, nodes, _proofs, _proofFlags), "Invalid proof");
