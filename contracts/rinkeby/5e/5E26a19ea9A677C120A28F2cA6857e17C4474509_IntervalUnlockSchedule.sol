...
contract IntervalUnlockSchedule is BaseGovernanceWithUserUpgradable {
    ...
    function __IntervalUnlockSchedule_init(ILock _lock, uint256[] memory _keyPoints, uint256[] memory _amounts, address governanceAddress) internal initializer {
        __BaseGovernanceWithUser_init(governanceAddress);
        __IntervalUnlockSchedule_init_unchained(_lock, _keyPoints, _amounts);
    }

    function __IntervalUnlockSchedule_init_unchained(ILock _lock, uint256[] memory _keyPoints, uint256[] memory _amounts) internal initializer {
     ...
     * @param _lockStartTimeStamp Timestamp when the Lock start (provide by the Lock Contract)
     * @param _initialAmount Initial amount of the lock
     * @return _unlockedAmount The unlocked tokens
     */
    function unlockedAmount(uint256 _lockStartTimeStamp, uint256 _initialAmount) external view returns (uint256) {
        uint256 time = block.timestamp;
        if(time < keyPoints[0]) {
            return 0;
        }
        if(time > keyPoints[keyPoints.length - 1]) {
            return _initialAmount;
        }
        uint256 _unlockedAmount;
        uint256 sumAmounts;
        uint256 index;
        for(uint i = 0; time > keyPoints[i]; i++) {
            sumAmounts += amounts[i];
            index = i;
        }
        uint256 nextAmount = amounts[index + 1];
        uint256 nextKey = keyPoints[index + 1];
        uint256 prevKey = index < 1 ?  _lockStartTimeStamp : keyPoints[index];
        _unlockedAmount = _initialAmount * (sumAmounts + ((nextAmount * (((time - prevKey) * EXP) / (nextKey - prevKey)) / EXP )));
        _unlockedAmount /= EXP;
