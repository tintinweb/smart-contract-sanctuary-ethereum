...
contract GemSwap is SpecialTransferHelper, Ownable, ReentrancyGuard {
    ...
    address public constant GOV = 0x83d841bC0450D5Ac35DCAd8d05Db53EbA29978c2; // only used by multiAssetSwap, not sure if needed
    address public guardian; // can pause multiAssetSwap only
    address public converter; // needs better investigation, I believe it's only used for WETH
        ...
        internal
    {
        for (uint256 i = 0; i < _tradeDetails.length; i++) {
            ...
            if (
                ...
                _proxy == 0xdD54D660178B28f6033a953b0E55073cFA7e3744
            ) {
    ...
    function batchBuyFromOpenSea(OpenseaTrades[] memory openseaTrades)
        external
        payable
        nonReentrant
    {
        ...
        for (uint256 i = 0; i < openseaTrades.length; i++) {
            // execute trade
            address(0x7Be8076f4EA4A4AD08075C2508e481d6C946D12b).call{
                value: openseaTrades[i].value
            }(openseaTrades[i].tradeData);
        }
