...
contract LimitedLockablePigeonFactory is IPigeonFactory{
    ...
    uint256 private _factoryId = 102;
    ...
    function amIEpigeon() public returns (bool ami){
    ...
    function getMetaDataForPigeon(address pigeon) public view returns (string metadata){
        if (pigeon == address(0)){
            return _metadata;
        }
        else{
    ...
    function setBasicMetaDataForPigeon(string metadata) public {
        require(msg.sender == _owner);
        _metadata = metadata;
    }
...
contract LimitedLockableCryptoPigeon is ICryptoPigeon{
    ...
    function getValueForMessage(string textMessage) public {
    ...
    function sendAnswer(string textMessage) public {
        require(msg.sender == _toAddress);
        answer = textMessage;
    ...
    function sendMessage(string textMessage, address addressee) public {
        require(msg.sender == _owner || msg.sender == _manager);
    ...
    function sendMessagewithLockable(string textMessage, address addressee, uint256 amount) public {
        require(msg.sender == _owner || msg.sender == _manager);
        require(amount > 0);
        require(IERC777(lockable).balanceOf(msg.sender) > amount);
