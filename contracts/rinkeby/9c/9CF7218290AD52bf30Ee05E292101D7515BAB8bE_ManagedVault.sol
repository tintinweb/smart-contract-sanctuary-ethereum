...
contract ManagedVault is Initializable, OwnableUpgradeable, ERC20Upgradeable, IManagedVault {
    ...
    /**
     * @notice Sets the vault's token price for a given blockNumber.
     *
     ...
     * setVaultPrice call.
     *
     * Emits a {SetVaultPrice} event.
     */
    function setVaultPrice(uint256 blockNumber, uint256 price) external onlyOwner {
        require(blockNumber < block.number);
        require(hasPendingMints() == false);
        require(price != vaultPrice || vaultPriceBlockNumber != blockNumber);

        vaultPrice = price;
        vaultPriceBlockNumber = blockNumber;

        emit SetVaultPrice(blockNumber, price);
    }

    /**
     * @notice Sets the native cryptocurrency's (Ether for the Ethereum network) price for minting.
     *
     * Emits a {SetNativePrice} event if a whitelist state is changed for a token.
     */
    function setNativePrice(uint256 price) external onlyOwner {
        require(nativePrice != price);
        
        nativePrice = price;

        emit SetNativePrice(price);
    }

    /**
     * @notice Sets a deposit token's price for minting.
     *
     * Emits a {SetTokenPrice} event if a whitelist state is changed for a token.
     */
    function setTokenPrice(IERC20 tokenAddress, uint256 price) external onlyOwner {
        require(tokenPrices[tokenAddress] != price);

        tokenPrices[tokenAddress] = price;

        emit SetTokenPrice(tokenAddress, price);
    }
    ...
    function depositToken(IERC20 tokenAddress, uint256 amount) external {
        ...
        bool success = tokenAddress.transferFrom(depositor, address(this), amount);
        require(success);
...
library PendingDeposit { 
    ...
    function peekLast(Queue storage queue) internal view isNotEmpty(queue) returns (Item memory pendingDeposit) {
        return queue._pendingDeposits[queue._last];
    }
