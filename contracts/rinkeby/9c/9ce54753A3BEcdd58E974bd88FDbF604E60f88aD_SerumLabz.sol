...
contract SerumLabz is ERC721A, Ownable, ReentrancyGuard {
    ...
    address public SLPOOLAddress = payable(0x499ea9F91118B5CaF0df0541Ca9884a7369eF0d5);
    address public FUTDEVAddress = payable(0x499ea9F91118B5CaF0df0541Ca9884a7369eF0d5);
    address public MARKETAddress = payable(0x499ea9F91118B5CaF0df0541Ca9884a7369eF0d5);
    address public SERUMAddress = payable(0x499ea9F91118B5CaF0df0541Ca9884a7369eF0d5);

    mapping(address => bool) internal presaleMinters;
    
    constructor(string memory _initBaseURI, bytes32 _root) ERC721A("SerumLabz", "SL") {
    ...
    function setPrivateKey(uint8 mintKeyIn) external onlyOwner {
        mintKey = mintKeyIn;
    }

    function getPrivateKey() external view onlyOwner returns (uint8) {
        return mintKey;
    }
    ...
    // ===== Private mint =====
    function privateMint(bytes32[] memory proof, uint8 quantity, uint8 suppliedKey) external payable nonReentrant whenPrivateMint {
        if(!presaleMinters[msg.sender]) {
            require(quantity == 1, "[Presale Error] Can only mint one serum at half price");
        ...
        // track mints
        mintCount += quantity;      
        presaleMinters[msg.sender] = true;
    }

    // ===== Public mint =====
    function mint(uint8 quantity, uint8 suppliedKey) external payable nonReentrant whenPublicMint {
        require(msg.value >= cost * quantity, "[Value Error] Not enough funds supplied for mint");
        require(totalSupply() + quantity < maxSupply, "[Supply Error] Not enough left for this mint amount");
        require(mintKey == suppliedKey, "[Mint Security] This NFT can only be minted from the dapp");
	...
	function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        require(tokenId != 0, "[URI Error] ID cannot = 0"); 
		return baseURI;
	}
