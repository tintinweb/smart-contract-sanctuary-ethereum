...
contract MetaDrag is ERC721A, Ownable, PaymentSplitter {
    ...
    function canMint(uint16 _maxMint, address _address) public view returns (uint256) {
        return _maxMint > hasMinted[_address] ? _maxMint - hasMinted[_address] : 0;
    }
    ...
    function mintPresale(uint16 _quantity, uint16 _maxMint, bytes32[] memory _proof) public payable {
        require(price() * _quantity <= msg.value, "ETH incorrect");
        require(uint16(totalSupply()) + _quantity <= token.maxSupply, "Insufficient supply");
        require(token.saleIsActive && token.preSaleIsActive, "Pre-sale inactive");
        uint16 mintedAmount = hasMinted[msg.sender] + _quantity;
        require(mintedAmount <= _maxMint, "Invalid quantity");
        ...
        uint16 _canMint = _maxMint - hasMinted[msg.sender];
        require(_quantity <= _canMint, "Invalid quantity");
        hasMinted[msg.sender] = mintedAmount;
        _safeMint(msg.sender, _quantity);
    }

    function mintPublic(uint16 _quantity) public payable {
        require(price() * _quantity <= msg.value, "ETH incorrect");
        require(uint16(totalSupply()) + _quantity <= token.maxSupply, "Insufficient supply");
        require(token.saleIsActive, "Sale inactive");
        require(_quantity <= token.maxPerTransaction, "Invalid quantity");
        _safeMint(msg.sender, _quantity);
    }
