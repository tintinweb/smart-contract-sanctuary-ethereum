    ...
    IMarketToken
{
    ...
    string private _noRevealMetadataURI;
    bool public revealed;
    mapping (uint256 => bool) public _concealedTokens;
    ...
    function initialize(
        ...
        bool revealed_,
        string memory noRevealMetadataURI_
    ) public initializer {
        ...
        if (revealed_ == false) {
            _noRevealMetadataURI = noRevealMetadataURI_;
        }
        revealed = revealed_;
    }
    ...
    function setBaseURIAndReveal(string memory _newBaseURI) public override {
        require(
            hasRole(MINTER_ROLE, msgSender()),
            "Caller doesnot have minter role"
        );
        require(bytes(_newBaseURI).length > 0, "Base URI is required");
        ...
        returns (string memory)
    {
        ...
        if (revealed == false) {
            return string(
                    abi.encodePacked(currentBaseURI, _noRevealMetadataURI)
                ) ;
        }

        if (revealed == true && _concealedTokens[tokenId] == true) {
            // if token is pre revealed then return path of directory plus token hash
            string memory tokenURI_ = super.tokenURI(tokenId);
            return string(
                    abi.encodePacked(currentBaseURI, _tokenBaseURI, tokenURI_)
                );
        }
    ...
    function contractURI() public view returns (string memory) {
        return string(abi.encodePacked("ipfs://", _contractMetaHash));
    }
}
...
interface IMarketToken {
    ...
    function setBaseURIAndReveal(string memory _newBaseURI) external;
}
