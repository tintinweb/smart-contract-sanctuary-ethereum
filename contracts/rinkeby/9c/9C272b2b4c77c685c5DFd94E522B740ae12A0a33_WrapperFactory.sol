    ...
    IWrapper
{
    ...
    function mint(uint256 amountIn) external notPaused issuanceActive {
        ...
        uint256 actualAmountIn = currentCapacity < amountIn
            ? currentCapacity
            : amountIn;

        (uint256 feeAmountTarget, bool negative) = calculateMintFee(
            actualAmountIn
        );
        uint256 mintAmount = negative
    ...
    function _mint(uint256 amount) internal {
        ...
        uint256 excessAmount = reserves > targetSynthIssued.add(amount)
            ? reserves.sub(targetSynthIssued.add(amount))
            : 0;
        uint256 excessAmountUsd = exchangeRates().effectiveValue(
            currencyKey,
            excessAmount,
            sUSD
        );
    ...
    function _burn(uint256 amount) internal {
        ...
        uint256 excessAmount = reserves.add(amount) > targetSynthIssued
...
contract WrapperFactory is Owned, MixinResolver, IWrapperFactory {
    ...
    function isWrapper(address possibleWrapper) external view returns (bool) {
        return
            flexibleStorage().getUIntValue(
                CONTRACT_NAME,
                bytes32(uint256(address(possibleWrapper)))
            ) > 0;
    }
    ...
    function createWrapper(
        ...
        flexibleStorage().setUIntValue(
            CONTRACT_NAME,
            bytes32(uint256(address(wrapper))),
            WRAPPER_VERSION
        );
