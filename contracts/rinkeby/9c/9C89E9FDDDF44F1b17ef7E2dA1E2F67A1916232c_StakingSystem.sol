...
contract StakingSystem is Ownable, ERC721Holder {
    ...
    // the timeframe for which $BOBL rewards compound
    uint256 public epochTime = 1 weeks;

    // $BOBL yield rate per epoch for each rarity
    ...
    //FUNCTIONS:
    // sets the genesis NFT contract address
    ...
    // sets the signer addresses which verify rarities
    ...
    function currentRewardsOf(uint16 _tokenId) public view returns (uint256) {
        ...
        // calculate the amount of time that the NFT has been staked in seconds
        uint256 stakedSeconds = block.timestamp - stakes[_tokenId].time;

        // calculate the amount of epochs that the NFT has been staked
    ...
    function _stake(address _owner, uint16 _tokenId, bytes32 _hash, bytes memory _signature) internal {
        ...
        stakes[_tokenId] = Stake(_rarity, _tokenId, (uint48(block.timestamp)), _owner);
    ...
    function _claimRewardsAndUnstake(uint16 _tokenId, bool _unstake, uint48 _time) internal returns (uint256 reward) {
        ...
        // calculate the reward for the token
        reward = currentRewardsOf(_tokenId);

        if (_unstake) {
            // delete stake struct from mapping
            delete stakes[_tokenId];
            // remove the tokenId from the stakers array
