...
contract Community is ICommunity {
        ...
        override
    {
        ...
        // update community
        uint256 _communityCount = _community.memberCount;
        _community.memberCount = _communityCount + 1;
        _community.members[_communityCount] = _newMemberAddr;
    ...
    function publishProject(
        ...
        require(!_communityProject.exists, "Community::Published");
        ...
        require(_projectDetails.length == 3, "Community::Details !OK");
        require(
            _projectInstance.projectCost() >= _projectDetails[2],
            "Community::invalid investment"
        );
        uint256 _projectCount = _community.projectCount;
        _community.projectCount = _projectCount + 1;
        _community.projects[_projectCount] = _project;
        _communityProject.exists = true;
        _communityProject.apr = _projectDetails[0];
        _communityProject.repaymentEndDate = _projectDetails[1];
        _communityProject.investmentNeeded = _projectDetails[2];
    ...
    function toggleInvestmentNeeded(
        uint256 _communityID,
        address _project,
        uint256 _investmentNeeded
    ) external {
        IProject _projectInstance = IProject(_project);
        ...
        require(
            _investmentNeeded >= _communityProject.totalInvested &&
                _investmentNeeded <= _projectInstance.projectCost(),
            "Community::invalid investment"
        );

        _communityProject.investmentNeeded = _investmentNeeded;
        eventsInstance.toggleInvestmentNeeded(
            _communityID,
            _project,
            _investmentNeeded
        );
    ...
    function investInProject(
        ...
        require(
            _msgSender() == communities[_communityID].owner,
            "Community::!owner"
        );
        require(
    ...
    function repayInvestor(
    ...
    ) external payable virtual override nonReentrant nonZero(_investor) {
        CommunityStruct storage _community = communities[_communityID];
        ProjectDetails storage _communityProject = _community.projectDetails[
            _project
        ];

        require(_communityProject.exists, "Community::Project !Exists");
        require(
            _msgSender() == IProject(_project).builder(),
            "Community::!Builder"
        );

        IToken20 _currency = _community.currency;
        IToken20 _wrappedToken = IToken20(
            homeFiInstance.wrappedToken(address(_currency))
        );
        ...
        uint256 _amountToReturn = _communityProject
            .investmentDetails[_investor]
            .investedAmount;

        require(_repayAmount <= _amountToReturn, "Community::!Liquid");
        _communityProject.investmentDetails[_investor].investedAmount =
            _amountToReturn -
            _repayAmount;
        ...
        override
    {
        ...
        // check signatures
        checkSignatureValidity(_investor, _hash, _signature, 0); // must be investor
        ...
        // update debt
        _reduceDebt(_communityID, _project, _repayAmount, _details, _investor);
    }
        ...
        returns (bool)
    {
        ...
        claimInterest(_communityID, _project, _msgSender(), _wrappedToken);
        uint256 _userInvestedAmount = _communityProject
            .investmentDetails[_msgSender()]
            .investedAmount;
        ...
        if (_communityProject.investmentDetails[_to].investedAmount > 0) {
            claimInterest(_communityID, _project, _to, _wrappedToken);
        }

        _communityProject.investmentDetails[_msgSender()].investedAmount =
            _userInvestedAmount -
            _amount;
        _communityProject.investmentDetails[_to].investedAmount += _amount;
        ...
        )
    {
        ...
        uint256[] memory _totalInvested = new uint256[](
            communities[_communityID].projectCount
        );
        ...
        for (uint256 i = 0; i < communities[_communityID].projectCount; i++) {
            ...
            _investmentNeeded[i] = communities[_communityID]
                .projectDetails[_projectArray[i]]
                .investmentNeeded;
            _totalInvested[i] = communities[_communityID]
                .projectDetails[_projectArray[i]]
                .totalInvested;
        }
        return (
    ...
    function checkSignatureValidity(
        ...
        approvedHashes[_address][_hash] = false;
    }
    ...
    ReentrancyGuardUpgradeable
{
    ...
    /**
     * Reduce debt using escrow. Here Investor can come in
     * terms with the Builder and an agent to reduce debt.
     *
     * @param _data bytes - - data encoded:
     * _communityID - community index associated with investment
     * _builder - builder address associated with investment
     * _investor - investor address associated with investment
     * _agent - agent address associated with an external agent
     * _project - project address associated with investment
     * _repayAmount - amount to repay
     * _details - IPFS hash
     * @param _signature bytes - _data signed by investor, builder and agent
     */
    function escrow(bytes calldata _data, bytes calldata _signature)
        external
        virtual;
     ...
     *  - [0] = APR; [1]: Max repayment timestamp; [2]: investment needed
     ...
     * @param _investment uint256 - the number of tokens of the community currency to invest
     * @param _hash bytes - IPFS hash of signed agreements document urls
     */
    function investInProject(
        uint256 _communityID,
     ...
     * @param _repayAmount uint256 - the amount of funds repaid to the investor, in the project currency
     * @param _details bytes - some details on why debt is reduced (off chain documents or images)
     * @param _investor address - investor address
     */
    function _reduceDebt(
        uint256 _communityID,
        address _project,
...
abstract contract IEvents is Initializable {
    ...
    /**
     * Call to emit when a home builder investment needed for his project
     * @dev modifier onlyCommunityContract
     *
     * @param _communityID uint256 - the uuid/ serial of the community being published to
     * @param _project address - the address of the deployed project contract where loans are escrowed
     * @param _investmentNeeded uint256 - the new investment need for the project
     */
    function toggleInvestmentNeeded(
        uint256 _communityID,
        address _project,
