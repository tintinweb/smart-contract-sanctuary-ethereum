...
import "./NoOwnerLaunchpadNFT.sol";

// for test
contract LaunchpadNoOwnerNFTFactory {

    event CreateNFT(address nftAddress, string name, string symbol);

    function createNoOwnerNFT(string memory name, string memory symbol, string memory baseURI, string memory suffix, address launchpad, uint256 maxSupply) external {
        NoOwnerLaunchpadNFT nft = new NoOwnerLaunchpadNFT(name, symbol, baseURI, suffix, launchpad, maxSupply);
...
// this a demon NFT
contract NoOwnerLaunchpadNFT is ERC721Enumerable{
