...
import "./HighStreetFurniture.sol";
import "./utils/PriceConverter.sol";

contract HighStreetFurnitureMarketplace is Context, Ownable, ReentrancyGuard, Pausable, PriceConverter {

  uint8 public constant CONST_EXCHANGE_RATE_DECIMAL = 18;

  HighStreetFurniture public immutable furniture;
  ...
  event Update(uint256 indexed id, ItemType itemType, uint256 data);
  event Add(uint256 indexed id, Item item);
  event Received(address indexed from, uint256 amount);
  event Buy(address indexed buyer, uint256[] ids, uint256[] amounts, bool isPaidByEth, uint256 price);
  ...
  constructor (
    ...
    require(furniture_ != address(0), "invalid furniture address");
    ...
    furniture = HighStreetFurniture(furniture_);
    high = IERC20(high_);
  ...
  function name() public view virtual returns (string memory) {
    return "HighStreet Furniture Marketplace";
  }
  ...
  function addItemToSale(uint256 id_, Item memory item_) public onlyOwner {
    require(furniture.minters(address(this)), "don't have minter role");
    require(salelist[id_].price == 0, "id have already in the list");
    require(item_.price > 0, "invalid price");
    salelist[id_] = item_;
    furniture.setMaxSupply(id_, uint256(item_.totalSupply));
    emit Add(id_, item_);
  }

  function updateItem(uint256 id_, ItemType type_, uint256 data_) public onlyOwner {
    require(salelist[id_].price != 0, "id isn't existing");
    // setup price
    if(type_ == ItemType.PRICE) {
      require(data_ != 0, "invalid price");
      salelist[id_].price = data_;
    }
    //reset count
    if (type_ == ItemType.RESET) {
      salelist[id_].count = 0;
    }
    //update max supply
    if (type_ == ItemType.MAX) {
      salelist[id_].maxSupply = uint32(data_);
    } else if (type_ == ItemType.TOTAL) {
      salelist[id_].totalSupply = uint32(data_);
      furniture.setMaxSupply(id_, uint256(data_));
    } else if (type_ == ItemType.RESTRICT) {
      salelist[id_].restrict = uint32(data_);
    }
    //update time
    if (type_ == ItemType.START) {
      salelist[id_].startTime = uint64(data_);
    } else if (type_ == ItemType.END) {
  ...
  function buy(uint256[] memory ids_, uint256[] memory amounts_) external payable virtual nonReentrant whenNotPaused {
    ...
    for (uint256 i = 0; i < ids_.length; i++) {
      ...
      require(item.count + amount < item.maxSupply, "exceed max amount");
      require(item.restrict == 0  ||  amount <= item.restrict , "exceed max per buy");
      totalPrice += item.price * amount;
    }

    if(isPaidByEth) {
      priceInEth = exchange(totalPrice);
      require(eth >= priceInEth, "not enough ether");
      uint256 reimburse = eth - priceInEth;
      Address.sendValue(payable(_receiver), priceInEth);
      Address.sendValue(payable(_msgSender()), reimburse);
    } else {
      SafeERC20.safeTransferFrom(high, _msgSender(), _receiver, totalPrice);
    }

    for (uint256 i = 0; i < ids_.length; i++) {
      uint256 id = ids_[i];
      uint256 amount = amounts_[i];
      furniture.mint(_msgSender(), id, amount, "");
      salelist[id].count += uint32(amount);
    }
    emit Buy(_msgSender(), ids_, amounts_, isPaidByEth, isPaidByEth ? priceInEth : totalPrice);
  }

  function exchange(uint256 value) public view virtual returns (uint256) {
...
contract HighStreetFurniture is Context, ERC1155Burnable, ERC1155Supply, Ownable, MinterAccessControl {
