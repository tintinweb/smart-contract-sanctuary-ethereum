...
contract DutchAuctionRefundable is Ownable, Pausable, ReentrancyGuard {
  ...
  /// @param qtyAllowed The amount of tokens user is able to mint, checks against the merkleroot
  function whitelistSale(
    ...
    require(
      isWhitelisted(_msgSender(), proof, qtyAllowed),
      "not whitelisted"
    );
    require(whitelistCount[_msgSender()] == 0, "all NFTs purchased");

    whitelistSold += requested;
    whitelistCount[_msgSender()] += requested;
    
    uint256 totalCost = (requested * finalRestingPrice * whitelistPercPrice) / 10_000;
    transferFunds(treasury, totalCost);

    token.mint(requested, _msgSender());

    emit WhitelistSale(_msgSender(), requested);
  }
