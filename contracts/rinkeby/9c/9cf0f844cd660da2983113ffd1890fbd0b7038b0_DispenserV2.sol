...
contract DispenserV2 {
    ...
    function dispenseERC20(IERC20 token, address[] calldata to, uint256[] calldata value)  external {
    ...
    function dispenseERC721(IERC721 token, address[] calldata to, uint256[] calldata value) external {
        require(to.length == value.length, "Receivers and amounts are different length!");
        for(uint256 i = 0; i <= to.length; i++) {
    ...
    function dispenseERC1155(IERC1155 token, address[] calldata recipients, uint256[] calldata ids, uint256[] calldata values, bytes calldata data) external {
        require(recipients.length == ids.length && ids.length == values.length, "Receivers, ids and amounts are different length!");
