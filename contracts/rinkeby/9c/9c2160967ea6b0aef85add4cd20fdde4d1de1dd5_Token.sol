...
contract Token is ERC721, Ownable, ERC721Enumerable, ReentrancyGuard {
    using Strings for uint256;
    bytes32 public saleMerkleRoot;

    constructor() ERC721("HUHUNFT", "HUHU") {}
    
    string private baseURI;
    string private blindURI;
    uint256 constant public MAX_NFT = 3999;
    uint256 public price = 0.07 * 10 ** 18;
    address private root = 0x7E9528813fE917d0D53b3118d13A0e9DDE748a57;
    ...
    function starttime(uint256 _time) public onlyOwner(){
        START_TIME = _time;
    }
    ...
    modifier mintCheck()
    {
        ...
        require(totalSupply() + 1 <= MAX_NFT, "Purchase would exceed max public supply of NFTs");
        uint256 time = block.timestamp;
        require(time >= START_TIME && time < END_TIME,"mint is not start");
        require(_nftMintCount[msg.sender] == 0,"Cannot mint above limit");
        require(msg.value == price, "Ether value sent is not correct");
        _;
    }

    function mint(bytes32[] calldata merkleProof) external payable mintCheck() nonReentrant
    {
        (bool success, ) = payable(root).call{value: msg.value}("");
        require(success, "Failed to withdraw payment");
        bytes32 leaf = keccak256(abi.encodePacked(msg.sender));
        require(MerkleProof.verify(merkleProof,saleMerkleRoot,leaf),
