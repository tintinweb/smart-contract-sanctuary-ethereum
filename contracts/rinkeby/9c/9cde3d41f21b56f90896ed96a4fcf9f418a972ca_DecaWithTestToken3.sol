...
contract DecaWithTestToken3 is VRFConsumerBaseV2, Pausable, Ownable, ReentrancyGuard {
    ...
    uint32 numWords = 1;
    uint256 housePoolDepositFeePercent = 5000000000000000000;
    uint256 maxBetPercentage = 100000000000000000000;
    ...
    uint256 public payout = 1960000000000000000;
    ...
    // Info of each pool.
    Balance[] public userHouseBalances;
    ...
    function depositHouse(uint256 _amount) external {
        ...
        if (newUserAddress) {
            userHouseBalances.push(Balance(userAddress, _amount));
        }
        // lock 처리해야함
    }
    ...
    function bet(uint256 _betAmount, uint256 _faceSide) external {
        ...
        require(_betAmount <= maxBetPercentage * housePoolBalance.depositAmount / 100 / 1e18, "maxBet");
        require(_faceSide >= 1 && _faceSide <= 100, "Face side must be 1 to 100");
        require(_betAmount <= userBankRollBalance[msg.sender].depositAmount, "exceed user bankroll amount");
        require(_betAmount <= housePoolBalance.depositAmount, "exceed house pool amount");
        betAmount = _betAmount / 1e18;
    ...
    function fulfillRandomWords(
        ...
        uint256 calculatedFee = betAmount * housePoolDepositFeePercent / 100;
        treasuryPoolBalance.depositAmount += calculatedFee;
        uint256 amountToPay = (payout * betAmount) - betAmount;
        if (userWon) {
            userBankRollBalance[msg.sender].depositAmount += amountToPay;
            for (uint256 i = 0; i < userHouseBalances.length; i++) {
                uint256 userContribution = userHouseBalances[i].depositAmount / housePoolBalance.depositAmount / 1e18;
                userHouseBalances[i].depositAmount -= (amountToPay + calculatedFee) / 1e18 * userContribution;
            }
            housePoolBalance.depositAmount -= amountToPay + calculatedFee;
        } else {
            userBankRollBalance[msg.sender].depositAmount -= amountToPay;
            for (uint256 i = 0; i < userHouseBalances.length; i++) {
                uint256 userContribution = userHouseBalances[i].depositAmount / housePoolBalance.depositAmount / 1e18;
                userHouseBalances[i].depositAmount += (betAmount - calculatedFee) / 1e18 * userContribution;
            }
