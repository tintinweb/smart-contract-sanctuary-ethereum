...
contract GoblinLootWTF is ERC721A, Ownable, Pausable, PullPayment {
    using SafeMath for uint256;

    uint256 public constant MAX_SUPPLY = 9999;

    /// @dev max tokens mintable in one transaction
    uint256 public maxPerMint;

    /// @dev mint price per token
    uint256 public mintPrice;

    /// @dev Base token URI used as a prefix by tokenURI().
    string public baseTokenURI;

    /// @dev Contract URI used as collection metadata
    ...
    function mint(uint256 quantity) external payable whenNotPaused {
        ...
        require(quantity > 0, "Quantity iz 1 or more init");
        require(quantity <= maxPerMint, "Too many you is greedy");
        require(totalMinted.add(quantity) < MAX_SUPPLY, "Not dat many iz left!");
