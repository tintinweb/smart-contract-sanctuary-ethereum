  ...
  IERC721Enumerable
{
  ...
  //NFT Settings
  string internal _baseTokenURI = "ipfs://bafybeibx57qy6sbnuacabx3xyd5aktjgx6f4fpdy464t2vw5elwmhstvpu/"; //Metadata Token URI
  uint256 internal NFTPrice = 0.005 ether; //NFT Normal price without incentive
  uint256 internal InitialIncentivePrice = 0 ether; //NFT price in the initial phase (with incentive)
  uint256 internal InitialIncentiveCap = 10; //Initial number of NFT with incentive
  uint256 internal NFTTotalS = 100; //Total NFT Supply (-1 if mint start from id 0)
  uint256 internal MaxMintPerW = 20; //Max mintable per wallet
  uint256 internal CreFee = 3; //Percentage of creator fee when traded on markets (not all market accept it in contract, modify it on the site if not)
  //End Settings

  uint256 internal FinCF = CreFee * 100;
...
abstract contract EarlyMintIncentive is Ownable, ERC721A {
  uint256 public PRICE = NFTPrice;
  uint256 public EARLY_MINT_PRICE = InitialIncentivePrice;
  uint256 public earlyMintTokenIdCap = InitialIncentiveCap;
...
abstract contract IGERC721A is 
    Ownable,
    ERC721A,
    ReentrancyGuard,
    EarlyMintIncentive 
    ...
    
{
  constructor(
  ...
  ) ERC721A(tokenName, tokenSymbol, MaxMintPerW, NFTTotalS) { }
    ...
    uint256 public MAX_WALLET_MINTS = MaxMintPerW;

  
     function mintToAdminV2(address _to, uint256 _qty) public onlyOwner{
         require(_qty > 0, "Must mint at least 1 token.");
         require(currentTokenId() + _qty <= collectionSize, string(abi.encodePacked("Cannot mint over supply cap of"," ",NFTTotalS.toString())));
         _safeMint(_to, _qty, true);
     }

  
    function mintTo(address _to) public payable {
        require(getNextTokenId() <= collectionSize, string(abi.encodePacked("Cannot mint over supply cap of"," ",NFTTotalS.toString())));
        ...
        require(msg.value == getPrice(1), string(abi.encodePacked("Insufficient payment, price is ", getPrice(1).toString()," WEI per item")));
    ...
    function mint(address _to, uint256 _amount) public payable {
        ...
        require(currentTokenId() + _amount <= collectionSize, string(abi.encodePacked("Cannot mint over supply cap of"," ",NFTTotalS.toString())));
        require(msg.value == getPrice(_amount), string(abi.encodePacked("Insufficient payment, price is ", getPrice(_amount).toString()," WEI for ", _amount.toString(), " tokens.")));
...
contract PixelSheepArt is IGERC721A, IERC2981 {
    constructor() IGERC721A("Pixel Sheep Art", "PXSA"){}

  function royaltyInfo(uint256, uint256 salePrice) external view override
    returns (address receiver, uint256 royaltyAmount)
  {
    return (address(this), (salePrice * FinCF) / NFTTotalS);
  }
}

// Developer: Giuliano AKA IronGiu (https://irongiu.com)
// Developed for my brother InstantSharing