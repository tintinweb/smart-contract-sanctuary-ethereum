...
library SafeERC721 {
  ...
  function safeTransfer(
    ...
    try _token.transferFrom(_from, _to, _tokenId) {
      return;
    } catch {}
...
interface IAuctionFractionalizer {
  function fractionalize(
    ...
    uint256 _duration,
    uint256 _fee
  ) external returns (address _fractions);
}
...
contract CollectivePurchase is ERC721Holder, Ownable, ReentrancyGuard {
  ...
  uint8 public constant FRACTIONS_DECIMALS = 6;
  uint256 public constant FRACTIONS_COUNT = 100000e6;
  ...
  function buyerFractionsCount(uint256 _listingId, address _buyer)
    external
    view
    inState(_listingId, State.Ended)
    returns (uint256 _fractionsCount)
  {
    ListingInfo storage _listing = listings[_listingId];
  ...
  function sellerPayout(uint256 _listingId)
    external
    view
    returns (
      uint256 _netAmount,
      uint256 _feeAmount,
      uint256 _netFractionsCount,
      uint256 _feeFractionsCount
    )
  {
    ListingInfo storage _listing = listings[_listingId];
  ...
  ) external nonReentrant returns (uint256 _listingId) {
  ...
  ) external onlySeller(_listingId) inState(_listingId, State.Created) {
  ...
  function _fractionalize(uint256 _listingId, uint256 _fractionPrice) internal returns (address _fractions) {
    ListingInfo storage _listing = listings[_listingId];
    (bytes32 _type, string memory _name, string memory _symbol, uint256 _duration, uint256 _fee) = abi.decode(
      _listing.extra,
      (bytes32, string, string, uint256, uint256)
    );
    IERC721(_listing.collection).approve(fractionalizers[_type], _listing.tokenId);
    return
      IAuctionFractionalizer(fractionalizers[_type]).fractionalize(
        _listing.collection,
        _listing.tokenId,
        _name,
        _symbol,
  ...
  event UpdatePrice(
    ...
    uint256 _oldReservePrice,
    uint256 _oldLimitPrice,
    uint256 _newReservePrice,
    uint256 _newLimitPrice
  );
...
contract Investment is ERC721Holder, Ownable, ReentrancyGuard {
  using SafeERC20 for IERC20;
  using SafeERC721 for IERC721;
  ...
  function list(
    ...
    creators.push(CreatorInfo({ creator: _creator, fee: _fee }));
  ...
  ) external payable nonReentrant inState(_listingId, State.Created) {
    address payable _buyer = msg.sender;
    uint256 _value = msg.value;
    ListingInfo storage _listing = listings[_listingId];
  ...
  function flashAcquire(
    uint256 _listingId,
    uint256 _minReservePrice,
    address payable _to,
    bytes calldata _data
  ) external inState(_listingId, State.Created) {
    ListingInfo storage _listing = listings[_listingId];
    require(_listing.reservePrice >= _minReservePrice, 'price slippage');
    _listing.state = State.Ended;
    _listing.seller = _to;
...
contract ExternalAcquirer is FlashAcquireCallee {
  ...
  constructor(address _collective) public {
    ...
    vault = Investment(_collective).vault();
  }
  ...
  function acquire(
    ...
    Investment(collective).flashAcquire(_listingId, 0, address(this), _data);
    if (_relist) {
      Investment(collective).relist(_listingId);
    }
  }
  ...
  function flashAcquireCall(
    ...
    (, , address _collection, uint256 _tokenId, , address _paymentToken, , , , , , , ) = Investment(collective).listings(_listingId);
    ...
    Investment(collective).acquire(_listingId, 0);
  }
