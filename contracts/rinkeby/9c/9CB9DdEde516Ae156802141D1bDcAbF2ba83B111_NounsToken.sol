...
//        descriptor.toggleDataURIEnabled();
    ...
    function mint(uint256 _count) external payable nonReentrant override {
        ...
        require(_nextTokenId() + _count <= MAX_SUPPLY, "Not enough supply");
        require(_count * MINT_PRICE == msg.value, "Incorrect amount of ether sent");

        uint256 startTokenId = _nextTokenId();
        _mint(_msgSenderERC721A(), _count);
        emit NounCreated(_msgSenderERC721A(), startTokenId, startTokenId + _count - 1);
    }
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        ...
        if (isRevealed) {
            return descriptor.tokenURI(tokenId, seeds[(tokenId + seedsOffset) % seeds.length]);
        } else {
