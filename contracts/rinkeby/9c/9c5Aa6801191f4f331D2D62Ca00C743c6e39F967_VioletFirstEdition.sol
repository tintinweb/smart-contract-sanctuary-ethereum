 ...
 *                          -------------                     line
 *
 *      VIOLET First Edition Contract ........................ 56
 *
 *          1.  Events ....................................... 58
 *
 *          2.  State variables .............................. 67
 *
 *          3.  Constructor .................................. 109
 *
 *          4.  Public minting ............................... 141
 *
 *          5.  Minting from Glass Token ..................... 234
 *
 *          6.  Token URI .................................... 267
 *
 *          7.  Withdrawal ................................... 290
...
/**
 *  @title Violet First Edition
 *  @author Nicolás Acosta (nicoacosta.eth) @0xnico_
 *          linktr.ee/nicoacosta.eth
 *  @notice Jadyn Violet Genesis Collection - First edition
 *          This contract interacts with the original NFT contract for minting first edition tokens
 *          and provides it with first edition tokens' metadata IPFS URI.
 *  @dev Implements IVioletEdition
 *       Inherits from openzeppelin/contracts `Ownable`
 *       Calls `violet.mintToken(editionId, to)` and `violet.mintTokens(editionId, to, amount)` for minting first edition tokens
 *       Violet NFT contract calls `tokenURI(tokenId)` for getting first edition tokens' metadata IPFS URI.
 */
contract VioletFirstEdition is Ownable, IVioletEdition {
    /// ------------
    /// 1. Events
    /// ------------
    ...
    /// @notice Edition id
    /// @return EDITION Edition id (1)
    uint256 public constant EDITION = 1;

    /// @notice Maximum amount of tokens that can be publicly minted
    /// @return MAX_PUBLIC_MINTING_TOKENS Maximum amount of tokens that can be publicly minted
    uint256 public constant MAX_PUBLIC_MINTING_TOKENS = 60;

    /// @notice Public minting start date
    /// @return mintingStartDate Public minting start date
    uint256 public immutable mintingStartDate; // Tue Apr 05 17:00:00 2022 UTC

    /// @notice Violet NFT contract
    /// @dev Used for minting first edition tokens
    /// @return violet Violet NFT contract
    IViolet public violet;
    ...
    uint256 public publicMintingTokensMinted;

    /// @notice First edition tokens' metadata IPFS URI
    string private _editionURI;
    ...
    // Glass token id -> already used to mint
    ...
    constructor(
        address _violet,
        address _owner,
        uint256 _mintingStartDate,
        string memory editionURI_,
        address withdrawalAddress1_,
        address withdrawalAddress2_
    ) {
        // Set Violet NFT interface
        violet = IViolet(_violet);

        // Transfer ownership from deployer (Violet NFT contract) to Violet NFT's deployer
        _transferOwnership(_owner);

        // Set minting start date
        mintingStartDate = _mintingStartDate;

        // Set metadata URI for this edition
        _editionURI = editionURI_;

        // Set withdrawal addresses
        _withdrawalAddress1 = withdrawalAddress1_;
        _withdrawalAddress2 = withdrawalAddress2_;

        // Set public minting count to 0
        publicMintingTokensMinted = 0;
    }
    ...
    function isFirstMintingPeriodActive() public view returns (bool) {
        return block.timestamp < mintingStartDate + 11 days;
    }
    ...
    function mintingPrice() external view returns (uint256) {
        if (isFirstMintingPeriodActive()) return 0.04 ether;
        return 0.08 ether;
    }
    ...
    function tokensLeftToMint() external view returns (uint256) {
        return MAX_PUBLIC_MINTING_TOKENS - publicMintingTokensMinted;
    }
    ...
    /// @dev Calls Violet NFT contract `mintToken(editionId, to)`
    ...
    function mintViolet(address to) external payable mintingStarted {
        ...
        if (isFirstMintingPeriodActive()) {
            ...
            _mintingPrice = 0.08 ether;

            require(
                publicMintingTokensMinted < MAX_PUBLIC_MINTING_TOKENS,
                "VioletFirstEdition: Maximum amount of tokens already minted"
            );
        }

        require(
            msg.value == _mintingPrice,
    ...
    /// @dev Calls Violet NFT contract `mintTokens(editionId, to, amount)`
    /// @param to Token recipient
        ...
        mintingStarted
    {
        ...
        if (isFirstMintingPeriodActive()) {
            ...
            require(
                publicMintingTokensMinted + amount <= MAX_PUBLIC_MINTING_TOKENS,
        ...
        // Mint Violet tokens
        violet.mintViolets(EDITION, to, amount);
    }

    /// ------------------------------
    /// 5. Minting from Glass Token
    /// ------------------------------
    ...
    function mintVioletFromGlassVideo(
        ...
        // Mint Violet token
        violet.mintViolet(EDITION, videoTokenOwner);
    }
    ...
    /// @notice Returns edition URI
    /// @dev This function is called by Violet NFT contract to get a token's metadata IPFS URI
    /// @param tokenId Token id (not used in this edition to determine URI)
    /// @return tokenURI Edition default URI
    ...
    /// @notice Returns edition URI
    /// @return tokenMetadata Edition default URI
...
interface IViolet {
    function mintViolet(uint256 editionId, address to)
        external
        returns (uint256);

    function mintViolets(
        uint256 editionId,
        address to,
        uint256 amount
