...
contract DeoniCrowsale1 is Crowdsale {
    ...
    constructor(
        ...
        Crowdsale(rate_, wallet, token, cap_)
    {
        _minCap = minCap;
        _rate = rate_;
        _cap = cap_;
...
abstract contract Crowdsale is Context, ReentrancyGuard, Pausable, Ownable {
    ...
    function buyTokens(address beneficiary) public virtual whenNotPaused nonReentrant payable {
        ...
        timelockcontract = address(new TokenTimelock(_token, beneficiary, release_time));

        emit TokensPurchased(_msgSender(), timelockcontract, weiAmount, tokens);

        _processPurchase(timelockcontract, tokens);

        _updatePurchasingState(timelockcontract, weiAmount);

        _forwardFunds();
        _postValidatePurchase(timelockcontract, weiAmount);
    }
