...
contract ViriumIdPrime is AccessControl, ERC721Burnable, ERC721Lockable {
    ...
    IViriumDestate public viriumDestate;
    uint256 public price = 0.01 ether; //todo prod
    ...
    constructor(address viriumIdAddress, address viriumDestateAddress) ERC721Lockable("ViriumIdPrime", "VIDP"){
        ...
        viriumId = IViriumId(viriumIdAddress);
        viriumDestate = IViriumDestate(viriumDestateAddress);
        _mint(msg.sender, 0);
    }

    function setPrice(uint256 _price) external onlyRole(MAINTAIN_ROLE) {
        price = _price;
    }
    ...
    function mint() external payable {
        require(Address.isContract(msg.sender) == false, "ViriumIdPrime: Prohibit contract calls");
        require(_mintedCounter.current() < MAX_TOTAL_SUPPLY, "ViriumIdPrime: Mint would exceed max supply");
        require(price <= msg.value, "ViriumIdPrime: Ether value sent is not correct");
        require(status == Status.Live, "ViriumIdPrime: Status mismatch");

        uint256 tokenId = viriumId.softMint();
        _mint(msg.sender, tokenId);
        viriumDestate.mint(msg.sender);
        _mintedCounter.increment();
    }
