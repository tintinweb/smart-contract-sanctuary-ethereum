...
contract DappRegistry is IAuthoriser {
    ...
    constructor(uint64 _timelockPeriod) {
        ...
        // set the owner of the Clutch Registry (registryId = 0)
    ...
    */
    function isEnabledRegistry(address _wallet, uint8 _registryId) external view returns (bool isEnabled) {
        ...
        return (((registries >> _registryId) & 1) > 0) /* "is bit set for regId?" */ == (_registryId > 0) /* "not Clutch registry?" */;
    }
    ...
    */
    function isAuthorised(address _wallet, address _spender, address _to, bytes calldata _data) public view override returns (bool) {
        uint registries = uint(enabledRegistryIds[_wallet]);
        // Check Clutch Default Registry first. It is enabled by default, implying that a zero 
        // at position 0 of the `registries` bit vector means that the Clutch Registry is enabled)
        for(uint registryId = 0; registryId == 0 || (registries >> registryId) > 0; registryId++) {
            bool isEnabled = (((registries >> registryId) & 1) > 0) /* "is bit set for regId?" */ == (registryId > 0) /* "not Clutch registry?" */;
    ...
    */
    function toggleRegistry(uint8 _registryId, bool _enabled) external {
        ...
        bool current = (((registries >> _registryId) & 1) > 0) /* "is bit set for regId?" */ == (_registryId > 0) /* "not Clutch registry?" */;
        if(current != _enabled) {
    ...
    /**
    * @notice Create a new registry. Only the owner of the Clutch registry (i.e. the registry with id 0 -- hence the use of `onlyOwner(0)`)
    ...
    */
    ...
    /**
    * @notice Request a change of the timelock value. Only the owner of the Clutch registry (i.e. the registry with id 0 -- 
