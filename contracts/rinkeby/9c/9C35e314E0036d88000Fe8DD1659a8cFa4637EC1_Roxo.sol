...
contract Roxo is ERC20,Ownable {
    ...
    address public usd;//A base fiat token for buying and selling.
    uint256 public basePrice;// A rate of buying  and selling.
    ...
    uint public totalEmissionBlocks; //Numbers of max blocks that will emit reward,Only for polygon mainnet...
 ...
 constructor() ERC20("Roxo", "Roxo") {
        ...
        usd = 0x2D48D479A03Acf4a7D1BB7aF5CEB30f6660Ed595;
        usdDecimal = 6;
    }
  ...
  function  buy()external returns (bool){
        ...
        require(preSaleAmount >= _amount * (1 * 10**usdDecimal) / basePrice);
...
function setUsdDecimals(uint decimal)external onlyOwner returns (bool)
{
    require(!lock,"Cannot Change after lock");
    usdDecimal = decimal;
    return true;
...
function controlLock()external onlyOwner{
    require(lock == false,"Already Locked");
    lock = true;
}
