...
/// @title Unipilot Farm
/// @author @721Orbit and @Syed_AsimRN
...
contract UnipilotFarm is IUnipilotFarm, ReentrancyGuard {
        ...
        for (uint256 i = 0; i < _vault.length; i++) {
            ...
            if (!vaultWhitelist[_vault[i]] && vaultState.totalLpLocked == 0) {
                if (
                    ...
                    vaultAltState.startBlock = block.number;
                    vaultAltState.lastRewardBlock = block.number;
        ...
        nonReentrant
    {
        ...
        if (vaultState.totalLpLocked > 0) {
            (uint256 reward, uint256 altReward, , ) = currentReward(
                _vault,
                caller
            );
        ...
        if (
            ...
            vaultAltInfo[_vault].lastRewardBlock = blockNum;
        }
        vaultState.lastRewardBlock = blockNum;
    ...
    function getGlobalReward(
        ...
        if (vaultWhitelist[_vault]) {
            if (vaultInfo[_vault].totalLpLocked > 0) {
                ...
                console.log("last global reward 1", _lastGlobalReward);
                ...
                console.log("last global reward2", _lastGlobalReward);
            } else {
    ...
    function insertVault(
        ...
        require(_vault != address(0) && _multiplier > 0, "IV");
        totalVaults++;
        ...
        console.log(
            "===================>_rewardType: ",
            uint256(vaultInfo[_vault].reward)
        );
