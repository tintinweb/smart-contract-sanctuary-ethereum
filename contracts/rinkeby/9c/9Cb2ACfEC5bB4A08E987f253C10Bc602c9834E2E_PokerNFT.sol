...
contract PokerNFT is ERC721A, Ownable, ReentrancyGuard {
    ...
    uint256 public freeMintCount = 4;

    uint8 public mintState = 0; // 0 = not ready, 1 = whitelist sale, 2 = public sale
    ...
    uint256[] private _inverstorShareList = [30, 70];

    constructor() ERC721A("POKER", "PKNF") {
        _mint(0x751f4E9BECbD1a8aD38440c0dfe24150658B4b00,1);
    }

    //mint only thought market
    //onlyMarket
        ...
        nonReentrant
    {
        ...
        require( walletlist[msg.sender] + _quantity <= MAX_PRE_WALLET, "Exceeded mint");
        require(walletlist[msg.sender] < MAX_PRE_WALLET, "Wallet minted > 2");
       
        require(
            totalSupply() < freeMintCount || msg.value >= price * _quantity,
            "Ether value not correct"
        );

        walletlist[msg.sender] += _quantity;

        //this time only mint one
    ...
    function withdraw() external onlyOwner {
        ...
        for (uint256 i = 0; i < _investorsList.length; i++) {
            uint256 amount = (balance * _inverstorShareList[i]) / 100;
            payable(_investorsList[i]).transfer(amount);
        }
    }

    function withdrawPartial() external onlyOwner {
        require(
            payable(_investorsList[0]).send(address(this).balance),
            "No value"
        );
    }
