...
contract StrainFactory is StrainContract, StrainAdmin {
    ...
    /// @dev rarity score
    uint8[4] public rarityScores = [0, 20, 50, 100];
    ...
    uint256 MAX_NFT_MINT = 10;
    uint256 MAX_NFT_COUNT = 100;
    ...
    function incF(uint8 x) private pure returns(uint8) {
    ...
    function randMod(uint8 _times) 
        internal 
        view
        returns (uint8) 
    {
        uint random = uint(keccak256(abi.encodePacked(randNonce+_times,block.timestamp,_times,msg.sender,randNonce)))%10;
        uint converted = (random == 0) ? 10 : random;
        return uint8(converted);
    }

    function createStrain(uint _times) public payable {
        ...
        require(ownerStrainCount[msg.sender]+_times <= 100, "Select less NFTs, You've reached your limit");
        payable(treasuryAddr).transfer(msg.value);
        _createStrain(_times);

    }

    function _createStrain(uint _times) 
        internal
    {
    ...
    function getStage(uint  _strainId) public view returns (uint8) {
    ...
    function updateStrainLimit(uint _newLimit) public onlyOwner {
        mintLimit = _newLimit;
    }

    function calculateRarityScore(uint128 _dna)
        private
        pure
        returns (uint128)
    {
    ...
    function getRarity(uint _strainId) public view returns (uint) {
        Strain storage strain = strains[_strainId];
    ...
    function updateRarity(uint _strainId)
        public
        ...
        onlyStakingContract(msg.sender)
    {
        ...
        strain.rarityScore += rarityScores[stage];
        strain.extraRarity += rarityScores[stage];
    }
        ...
        onlyStakingContract(msg.sender)
    {
        ...
        string memory newTokenUri = (strain.stage > 0) ? tokenURI(_strainId) : _tokenURI;
