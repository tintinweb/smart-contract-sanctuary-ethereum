...
contract EncryptoTRCDapp is Ownable, IEncryptoTRCAsset {
	...
	function standardTransfer(
		...
		if (address(previousOwner) == address(0)) {
			...
			payable(address(liquidityWallet)).transfer(sellerBalance); //correct
			sellerBalance = 0;
		}

		/// @dev Transfer WORLDTAX_FEE to liquidity wallet
		if (tax.WORLDTAX_FEE > 0) {
			payable(address(liquidityWallet)).transfer(tax.WORLDTAX_FEE); //correct
		}

		if (tax.MARKETING_FEE > 0) {
			payable(address(marketingWallet)).transfer(tax.MARKETING_FEE); //correct
		}
