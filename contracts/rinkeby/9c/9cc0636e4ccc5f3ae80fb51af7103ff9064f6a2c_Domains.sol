...
// File: https://github.com/ekazukii/dns-polygon/blob/main/contracts/contracts/libraries/Base64.sol
...
// File: https://github.com/ekazukii/dns-polygon/blob/main/contracts/contracts/libraries/StringUtils.sol
...
// File: https://github.com/ekazukii/dns-polygon/blob/main/contracts/contracts/DNS.sol
...
contract Domains is ERC721 {
    ...
    // Bind all domains name to the records
    mapping(string => Record) public records;

    // Bind all NFTs ID to the domains name
    mapping(uint => string) public names;
    // Now in the other direction, domain name to NFT ID
    mapping(string => uint) public ids;

    // Store the top level domain string
    string public tld;
    ...
    // Static parts of the NFT's default avatar. Need to add dynamically the name of the domain
    ...
    // Thrown when user don't own the domain
    error Unauthorized();
    // Thrown when domain is already owned
    error AlreadyRegistered();
    // Throw when domain has an invalid name (too short, too long, ...)
    error InvalidName(string name);

    // We init the NFT with the owner of the contract and set the first token ID to 1
    ...
    // Withdraw the money in the contract
    ...
    // Return all the domains name registered in the contract
    // Can be used to display all domains on a website or make a search
    ...
    // Calculate the domain name price based on the length of the name
    ...
    // Pay to register a new domain. Check if the name is available, valid and if the sender has enough money
    ...
    // Return the NFT uri for the given token ID
    // The metadata contains:
    //   - The name of the domain with the TLD
    //   - The description of the NFT
    //   - The image of the NFT, by default it's an SVG with the name of the domain
    //   - The length of the name of the domain
    ...
    // Return NFT id for the given domain name
    // Used to get metadata from an domain name 
