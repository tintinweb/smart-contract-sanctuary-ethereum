...
contract Simple {
    ...
    function buy() payable public {
        ...
        require(amountTobuy <= customerStructs[msg.sender].amount, "Not enough in the wallet");
        token.transfer(msg.sender, amountTobuy);
        customerStructs[msg.sender].amount = customerStructs[msg.sender].amount + amountTobuy;
            for (uint index = 0; index < userAddresses.length; index++) {
                if (userAddresses[index] == msg.sender) {
                userAmount[index] = customerStructs[msg.sender].amount;
            }
