...
contract NFTBoilMerkle is ERC721Enumerable, Ownable, Pausable {
    ...
    string constant private AUTHER = "t.chiba";
    ...
    function getBalanceEachWallet() public view returns(address[] memory walletList, uint256[] memory balances){
    uint256 totalSupply = totalSupply();
    uint256 walletCnt = 0;
    address[] memory walletTmpList = new address[](totalSupply);

    for (uint256 id = 1; id <= totalSupply; id++) {
        address tmp = ownerOf(id);
        
        for (uint256 i = 0; id < walletTmpList.length; i++) {
            if( walletTmpList[i] != 0x0000000000000000000000000000000000000000){
                continue;
            }
            walletTmpList[i] = tmp;
            walletCnt++;
    ...
    walletList = new address[](walletCnt);
    balances = new uint256[](walletCnt);

    console.log(walletCnt);
    ...
    for (uint256 id = 1; id <= totalSupply; id++) {
        ...
        for (uint256 i = 0; i < walletList.length; i++){
            ...
            if(walletList[i] == 0x0000000000000000000000000000000000000000){
                console.log("push");
                walletList[i] = tmp;
                balances[i] = 1;
                break;
    ...
    function execTransfer(address _address, uint256 _id) public {
        statuses[_id - 1].transferCnt++;
        safeTransferFrom(msg.sender, _address, _id);
    }
