    ...
    Ownable
{
  ...
  event PayeeChanged(address newAccount, uint256 newShares);
  ...
  function changePayeeAccount(uint256 index, address newAccount, uint256 newShares) public onlyOwner {
    require(newAccount != address(0), "Account is the zero address");
    require(newShares > 0, "Shares are 0");

    address oldAccount = _payees[index];
    uint256 oldShares = _shares[oldAccount];

    _payees[index] = newAccount;
    _shares[newAccount] = newShares;
    _totalShares = _totalShares - oldShares + newShares;
    emit PayeeChanged(newAccount, newShares);
  }
