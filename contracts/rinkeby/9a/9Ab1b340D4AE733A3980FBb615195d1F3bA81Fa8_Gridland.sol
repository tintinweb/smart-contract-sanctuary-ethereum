...
contract Gridland is ERC721A, Ownable {
    uint256 public immutable cap;
    uint256 public immutable percentOfDevs;
    uint256 public immutable percentOfWinners;
    ...
    mapping(uint256 => CellData) internal tokenIdData;

    constructor(
        uint256 _cap,
        uint256 _percentOfDevs,
        uint256 _percentOfWinners
    ) ERC721A("Gridland Test", "GRLD") {
        cap = _cap;
        percentOfDevs = _percentOfDevs;
        percentOfWinners = _percentOfWinners;

        require(
            _percentOfDevs + _percentOfWinners == 100,
            "Distribution percentage is not 100%"
        );
    }
    ...
    distributionTrigger
    {
        require(winnersIdx.length > 0, "Winners number is not set yet");
        require(block.timestamp >= saleStartTime, "Sale has not begun yet");
        require(totalSupply() + quantity <= cap, "Reached max supply");
        require(msg.value >= 0.05 ether * quantity, "Not enough balance");
    ...
    private
    {
        for (uint256 i = currentIndex; i < currentIndex + quantity; i++) {
    ...
    function setTokenData(uint256 tokenId, string memory imageUrl, string memory link)
    public
    {
        require(ownerOf(tokenId) == msg.sender, "msg.sender is not token owner");
        tokenIdData[tokenId].imageUrl = imageUrl;
        tokenIdData[tokenId].link = link;
    }

    function getTokenData(uint256 tokenUrl)
    public
    view
    returns (CellData memory)
    {
        return tokenIdData[tokenUrl];
    }

    function setSaleStartTime(uint32 startTime)
    external
    onlyOwner
    {
