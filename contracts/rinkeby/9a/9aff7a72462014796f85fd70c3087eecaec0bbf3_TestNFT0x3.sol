...
abstract contract LzApp is Ownable, ILayerZeroReceiver, ILayerZeroUserApplicationConfig {
    ...
    function _lzSend(uint16 _dstChainId, bytes memory _payload, address payable _refundAddress) internal virtual {
        ...
        lzEndpoint.send{value: msg.value}(_dstChainId, trustedRemote, _payload, _refundAddress, zroPaymentAddress, _adapterParams);
    }
    ...
    // set destination gas amount for traversal
    function setTraversalGasParams(address zroAddress, uint256 _destGasAmount) external onlyOwner {
        destGasAmount = _destGasAmount;
        zroAddress = zroPaymentAddress;
    }

    function retryPayload(uint16 _srcChainId, bytes calldata _srcAddress, bytes calldata _payload) external payable {
        return lzEndpoint.retryPayload(_srcChainId, _srcAddress, _payload);
    }    
...
// File: github/LayerZero-Labs/solidity-examples/contracts/TestNFT0x4.sol
...
contract TestNFT0x3 is UniversalONFT721 {
    constructor(
        ...
        uint _startMintId,
        uint _endMintId
    ) UniversalONFT721("TestNFT0x4", "TNFT0x4", _layerZeroEndpoint, _startMintId, _endMintId) {}
}