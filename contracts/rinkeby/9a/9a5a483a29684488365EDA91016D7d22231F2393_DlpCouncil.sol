...
contract DlpCouncil is DlpDaoCore {
    ...
    function submitTransaction(
        ...
        onlyCouncil
        nonReentrant
        proposalIsSubmit(_proposalId)
        ipfsIsConfirmed(_ipfsUrl)
    {
        Proposal storage _proposal = proposals[_proposalId];
        _proposal.councilFlags[1] = true;
        _proposal.ipfsUrl = _ipfsUrl;

        bytes32 _hash = keccak256(abi.encodePacked(_ipfsUrl));
        ipsfUrlIsConfirmed[_hash] = true;
        ...
        proposalIsConfirm(_txIndex)
    {
        ...
        Proposal storage _proposal = proposals[_proposalId];
        if (transaction.pass && transaction.executed) {
    ...
    function transferToken(
        ...
        // bytes4(keccak256(bytes('transfer(address,address,uint256,string,uint256)')));
...
contract DlpDaoCore is ReentrancyGuard, Timestamp {
    ...
    //keccak256(abi.encodePacked(_pro.ipfsUrl)=>bool
    mapping(bytes32 => bool) ipsfUrlIsConfirmed;
    ...
    modifier proposalSafeCouncilIsExecute(uint256 _proposalId) {
        Proposal memory _pro = proposals[_proposalId];
        require(_pro.councilFlags[1] && !_pro.executed, "DLP102");
        require(
    ...
    modifier ipfsIsConfirmed(string memory _ipfsUrl) {
        require(bytes(_ipfsUrl).length > 0, "DLP107");
        bytes32 _hash = keccak256(abi.encodePacked(_ipfsUrl));
        require(!ipsfUrlIsConfirmed[_hash], "DLP108");
        _;
    }

    constructor(address _securityCouncil) {
        require(_securityCouncil != address(0), "DLP109");
        securityCouncil = _securityCouncil;
    }

    //The ID of the newly-created proposal, which is created sequentially
    function createProposalId() public nonReentrant returns (uint256 _id) {
        require(
    ...
    function getCouncils() public view returns (address[] memory) {
        uint256 j = 0;
        address[] memory _councils = new address[](ownerCount);
        for (uint256 i = 0; i < councils.length; i++) {
