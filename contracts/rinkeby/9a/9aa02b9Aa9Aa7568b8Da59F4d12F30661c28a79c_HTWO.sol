...
contract HTWO is ERC721A, Ownable, ReentrancyGuard {
    ...
    function reserve(address recipient, uint256 amount) external onlyOwner {
        require(recipient != address(0), "BBD: zero address");
        require(amount > 0, "BBD: invalid amount");
        require(
            totalSupply() + amount <= collectionSize,
            "BBD: max supply exceeded"
        );

        require(
            tokensReserved + amount <= reserveAmount,
            "BBD: max reserve amount exceeded"
        );
        require(
            amount % maxPerMint == 0,
            "BBD: can only mint a multiple of the maxBatchSize"
        );
    ...
    function mint(uint256 amount) external payable {
        require(status == Status.PublicSale, "BBD: Public sale is not active.");
        require(
            tx.origin == msg.sender,
            "BBD: contract is not allowed to mint."
        );
        ...
        require(
            amount <= maxPerMint,
            "BBD: Exceeds the single maximum limit."
        );
    ...
    function refundIfOver(uint256 price) private {
        require(msg.value >= price, "BBD: Need to send more ETH.");
        if (msg.value > price) {
