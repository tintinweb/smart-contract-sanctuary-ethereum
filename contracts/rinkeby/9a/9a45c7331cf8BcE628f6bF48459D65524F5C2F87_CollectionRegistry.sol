...
contract CollectionRegistry is ICollectionRegistry {
    ...
    */
    function listCollections(address userAddress_) external view returns (Record[] memory) {
        bytes32[] memory _collectionIds = userCollections[userAddress_].values();
        Record[] memory _recordsResult = new Record[](_collectionIds.length);
        for (uint256 i = 0; i < _collectionIds.length; i++) {
            _recordsResult[i] = records[_collectionIds[i]];
        }
