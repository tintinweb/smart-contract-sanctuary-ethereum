...
contract BrewBuddies is ERC721Enumerable, Ownable
{
    ...
    uint256 public status = 1; // 0-pause, 1- Resume
    bool public reveal = false;
    

    constructor(string memory baseURI) ERC721("Brew Buddies", "BB") {
        setBaseURI(baseURI);
    ...
    function mint(uint chosenAmount) public payable isSaleOpen {
        ...
        require(status == 1, "Sorry the Sale is not open");
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        ...
        if(bytes(base).length > 0){
            return reveal ? string(abi.encodePacked(base, tokenId.toString(), ".json")) : string(abi.encodePacked("https://gateway.pinata.cloud/ipfs/QmbrpFvhaTpQC5TBJuAwFLy8ypHchwQrEozyPXNgMq1Z1x"));
        }
        else 
        return "";
    }
    ...
    function contractURI() public view returns (string memory) {
        string memory json = Base64.encode(bytes(string(abi.encodePacked('{"name": "Brew Buddies", "description": "Brew Buddies are a collection of 2,000 freshly brewed tasty cups of coffee that live on the Ethereum blockchain", "seller_fee_basis_points": 200, "fee_recipient": "0xe119FDcF665365371d3C227A0d86A47C471059E6"}'))));
