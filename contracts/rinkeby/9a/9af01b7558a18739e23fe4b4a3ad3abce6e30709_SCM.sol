...
contract SCM is Ownable, ERC721A, ReentrancyGuard {
    ...
    string private _modeBTokenURI;
    string private _modeCTokenURI;
    ...
    mapping(uint256 => uint256) public mapToekenABC; 


    address watcherddress = 0xF9951627EcD7F94890A0b325C91C63FD963C011C;//ここに読み込みたい独コンのアドレスを記載
    WatcherInterface watcherContract = WatcherInterface(watcherddress);


  constructor(
  ) ERC721A("SCM", "SCM", mintLimit, _totalSupply) {
     tokenCount = 0;
  }
  ...
  function _ModeA(uint256 tokenId) public{
    require(ownerOf(tokenId) == msg.sender, "You are not the holder of this token.");
    mapToekenABC[tokenId] = 1;
  }
  function _ModeB(uint256 tokenId) public{
    require(ownerOf(tokenId) == msg.sender, "You are not the holder of this token.");
    mapToekenABC[tokenId] = 2;
  }
  function _ModeWatcher(uint256 tokenId) public{
    require(ownerOf(tokenId) == msg.sender, "You are not the holder of this token.");
    require(0<watcherContract.balanceOf(msg.sender), "You don't have watcher.");
    mapToekenABC[tokenId] = 3;
  }

  function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
      require(_exists(tokenId), "ERC721Metadata: URI query for nonexistent token");
      if(mapToekenABC[tokenId] == 2) {
          return string(abi.encodePacked(_baseBURI(), Strings.toString(tokenId)));
      }
      if(mapToekenABC[tokenId] == 3) {
          return string(abi.encodePacked(_baseCURI(), Strings.toString(tokenId)));
      }
  ...
  function withdrawRevenueShare() external onlyOwner {
    ...
    address engineer = payable(0xc0095c3b4C5021C867Cf39758bfcC803392D471d); 
  ...
  function _baseBURI() internal view virtual  returns (string memory) {
    return _modeBTokenURI;
  }
  function _baseCURI() internal view virtual  returns (string memory) {
    return _modeCTokenURI;
  }
  

  function setModeAURI(string calldata baseURI) external onlyOwner {
    _baseTokenURI = baseURI;
  }
  function setModeBURI(string calldata baseURI) external onlyOwner {
    _modeBTokenURI = baseURI;
  }
  function setModeCURI(string calldata baseURI) external onlyOwner {
