...
contract Revan is Context, IERC20, Ownable, Pausable {
    ...
    uint256 private _tTotal = 99999999999999;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint8 private _decimals = 9;
    uint256 public setTaxFee = 3;
    uint256 public setLiquidityFee = 3;
    uint256 private _tFeeTotal = setTaxFee + setLiquidityFee;
    uint256 private _feeRate = setTaxFee + setLiquidityFee;
    uint256 private _previousTaxFee = setTaxFee;
    uint256 private _previousLiquidityFee = setLiquidityFee;
    uint256 launchTime;
    ...
    function transfer(address recipient, uint256 amount) public override returns (bool) {
        ...
        require(!blacklisted(tx.origin), "sender is blacklisted!!");
    ...
    function pause() public onlyOwner {
        require(!paused(), "Contract is currently paused. Please stand by while Dev solves the issue. Thank You!");
        _pause();
    }

    function unpause() public onlyOwner {
        require(paused(), "Contract is currently unpaused. Have a good day.");
        _unpause();
    }
    ...
    function calculateTaxFee(uint256 _amount) private view returns (uint256) {
        return _amount.mul(setTaxFee).div(
            10**2
        );
    }
    
    function calculateLiquidityFee(uint256 _amount) private view returns (uint256) {
        return _amount.mul(setLiquidityFee).div(
            10**2
        );
    }
    
    function removeAllFee() private {
        if(setTaxFee == 0 && setLiquidityFee == 0) return;
        
        _previousTaxFee = setTaxFee;
        _previousLiquidityFee = setLiquidityFee;
    ...
    function restoreAllFee() private {
        setTaxFee = _previousTaxFee;
        setLiquidityFee = _previousLiquidityFee;
    }
