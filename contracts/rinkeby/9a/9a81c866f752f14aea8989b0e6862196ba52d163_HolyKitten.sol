...
contract HolyKitten is ERC721A, Ownable {
    ...
    uint public priceWL = 0.015 ether;
    uint public maxSupply = 2000;
    uint public maxTx = 3;
    uint public maxTxWL = 3;
    ...
    function setPriceWL(uint newPrice) external onlyOwner {
        priceWL = newPrice;
    }
    ...
    function setMaxTx(uint newMax) external onlyOwner {
        ...
        function setMaxTxWL(uint newMax) external onlyOwner {
        maxTxWL = newMax;
    }
    ...
    function whitelistMint(uint qty, bytes32[] memory proof) external payable {
        ...
        require(
            _preSaleCounter[msg.sender] + qty <= maxTxWL,
    ...
    function _mint(uint qty) internal {
        ...
        function _mintWL(uint qty) internal {
        require(qty <= maxTxWL && qty > 0, "TRANSACTION: qty of mints not alowed");
        require(msg.value >= priceWL * (qty), "PAYMENT: invalid value");
        _mintTo(_msgSender(), qty);
    }
