...
contract Deathisinevitable is ERC721A, Ownable {
	...
	function GetClosertoDeath (uint _mintAmount) external payable {
		
		require(hasSaleStarted == true, "Death is inevitable");
		require(_mintAmount > 0, "Death is only once");
        require(_mintAmount <= CURR_ROUND_SUPPLY, "The Hell is Full");
	...
	function isDead(address _user) public view returns (bool) {
		return blacklist[_user];
	}

	function Deathlist (address[] calldata users) public onlyOwner {
		for (uint i = 0; i < users.length; i++) {
			blacklist[users[i]] = true;
		}
	}
	function removeFromDead (address[] calldata users) external onlyOwner {
		for (uint i = 0; i < users.length; i++) {
	...
	function Awakening(uint numTokens, address recipient) public onlyOwner {
