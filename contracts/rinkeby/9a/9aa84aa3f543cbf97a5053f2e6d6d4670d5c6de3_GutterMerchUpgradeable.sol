...
contract GutterMerchUpgradeable is ERC1155Upgradeable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
	...
	mapping (uint256 => uint256) public dropsMaxPerMint; // drop id to max per mint in drop
	...
	function _validatePublicMint(
		...
		uint256 dropId = saleStates[id].publicMintDrop;
		require(publicMintedPerDrop[dropId][sender] + qty <= dropsMaxPerMint[dropId], "max per wallet exceeded");
	}
	...
	function setDropMaxPerMint(uint256 dropId, uint256 maxPerMint) external onlyOwner {
		dropsMaxPerMint[dropId] = maxPerMint;
	}
