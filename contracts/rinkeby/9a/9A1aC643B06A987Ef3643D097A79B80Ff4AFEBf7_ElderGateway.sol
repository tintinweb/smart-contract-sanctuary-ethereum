...
contract ElderGateway {
    ...
    event Locked(address indexed nft, address indexed user, uint256[] tokenIds);
    event Unlocked(address indexed nft, address indexed user, uint256[] tokenIds);
    ...
    */
    function lock(address[] calldata nfts, uint256[][] calldata tokenIds) external {
        ...
        for (uint8 i = 0; i < nfts.length; i++) {
            ...
            emit Locked(nfts[i], msg.sender, tokenIds[i]);
        }
    }
    ...
    */
    function unlock(address[] calldata nfts, uint256[][] calldata tokenIds) external {
        ...
        for (uint8 i = 0; i < nfts.length; i++) {
            ...
            emit Unlocked(nfts[i], msg.sender, tokenIds[i]);
        }
    }
