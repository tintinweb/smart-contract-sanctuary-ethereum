...
{
    uint256     tokenId;
    uint256     playerIndex;        // 0: original NFT  1: clone #1  2: clone #2
    bool        isRentable;
    bool        isRented;
    uint256     since;
...
contract DL01Token     is  ERC721Enumerable, ReentrancyGuard
{
    ...
    uint256 public      nameChangePrice    = 0.0002 ether;
    ...
    mapping(uint256 => string)      public  doolnyNames;
    ...
    //=============================================================================
    event AFF_BEGIN();
    event AFF1(TAffiliate affiliate);
    event AFF2();
    event AFF3();
    event AFF4(uint256 commisionAmount, uint256 shareValueInM100, address affAddr);
    event AFF5();
    event AFF_END();
    //=============================================================================
    function    setAffiliate(string memory username) external returns(uint256)
    {
        return setAffiliateEx(username, msg.sender, 800, true);       // 8% for standard affiliates. they login by themselves
    }
    //=============================================================================
    function    setAffiliateByOwner(string memory username, address wallet, uint256 percentInM100, bool isEnabled) external onlyAdminOrOwner returns(uint256)
    {
        return setAffiliateEx(username, wallet, percentInM100, isEnabled);
    }
    //=============================================================================
    function    disableAffiliate(string memory username, address wallet) external view onlyAdminOrOwner
    {
    ...
    //=============================================================================
    function    setAffiliateEx(string memory username, address wallet, uint256 percentInM100, bool isEnabled) internal returns(uint256)
    {
        uint256 affHash = forgeAffHash(username, wallet);
        ...
        {
            affiliates[affHash] = TAffiliate( msg.sender, percentInM100, username, isEnabled, 0 );       // 8%
        }
    ...
    function    payAffiliate(uint256 affHash) internal
    {
        ...
        if (affiliate.enabled)
        {
            ...
            {                                                   // Attention pendant les debugs si on a deployer le contrat sur la meme addresse que l'affilié, ca marchera pas 
                uint256 commission = (affiliate.shareValue * msg.value) / (100*100);
    ...
    //=============================================================================
    //=============================================================================
    event onDoolnyName(string doolnyName);
    //=============================================================================
    function    generateDoolnyName(uint256 tokenId) public returns(string memory)
    {
        string memory name = string( bytes.concat( bytes( "Doolny #"), bytes( tokenId.toString() )));

        doolnyNames[tokenId] = name;

        return name;
    }
    //=============================================================================
    function    renameDoolny(uint256 tokenId, string memory newName) external payable
    {
        require(ownerOf(tokenId)==msg.sender, "You are not the owner of this Doolny");    

        doolnyNames[tokenId] = newName;

        emit onDoolnyName(newName);
    }
    //=============================================================================
    function    renameDoolnyByOwner(uint256 tokenId, string memory newName) external onlyAdminOrOwner
    {
        doolnyNames[tokenId] = newName;
    ...
    //=============================================================================
    function    getDoolnyName(uint256 tokenId) external view returns(string memory)
    {
        return doolnyNames[ tokenId ];
    }
    //=============================================================================
    function    getDoolnyNames(uint256[] memory tokenIds) external view returns(string[] memory)
    {
        string[] memory foundNames = new string[](tokenIds.length);

        for(uint256 i; i<tokenIds.length; i++)
        {
            foundNames[i] = doolnyNames[ tokenIds[i] ];
        }
