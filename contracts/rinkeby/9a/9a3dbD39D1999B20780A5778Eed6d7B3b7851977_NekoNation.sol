...
import './lib/MerkleDistributorLimit.sol';
import './lib/SteppedDutchAuction.sol';

contract NekoNation is ERC721A, MerkleDistributorLimit, SteppedDutchAuction, ReentrancyGuard, AccessControl {
    ...
    uint256 public pricePerToken = 0.19 ether;

    address public immutable shareholderAddress;
    uint256 public immutable maxAuctionSupply;
    uint256 public immutable maxReserveSupply;

    constructor(
        address shareholderAddress_,
        uint256 maxAuctionSupply_,
        uint256 maxReserveSupply_,
        uint256 startPrice,
        uint256 endPrice,
        uint256 priceStep,
        uint256 timeStepSeconds
    ) ERC721A('NekoNation', 'NEKO') SteppedDutchAuction(startPrice, endPrice, priceStep, timeStepSeconds) {
        // set immutable variables
        shareholderAddress = shareholderAddress_;
        maxAuctionSupply = maxAuctionSupply_;
        maxReserveSupply = maxReserveSupply_;
    ...
    event PricePerTokenChanged(uint256 price);
    ...
    function devMint(uint256 numberOfTokens) external onlyRole(SUPPORT_ROLE) ableToMint(numberOfTokens) nonReentrant {
        require(reserveSupply + numberOfTokens <= maxReserveSupply, 'Number would exceed max reserve supply');
    ...
    function setPricePerToken(uint256 price) external onlyRole(SUPPORT_ROLE) {
        pricePerToken = price;

        emit PricePerTokenChanged(price);
    }
     ...
     * public
     */
    function mintAuction(uint256 numberOfTokens)
        external
        payable
        isAuctionActive
        ...
        nonReentrant
    {
        ...
        require(auctionSupply + numberOfTokens <= maxAuctionSupply, 'Purchase would exceed max tokens for auction');

        uint256 totalMintPrice = getAuctionPrice() * numberOfTokens;
        require(totalMintPrice <= msg.value, 'Ether value sent is not correct');

        auctionSupply += numberOfTokens;
        _safeMint(msg.sender, numberOfTokens);
    }
    ...
    function mintAllowList(
        ...
        payable
        isAllowListActive
        ableToClaim(msg.sender, totalTokenAmount, merkleProof)
    ...
    function withdraw() external onlyRole(DEFAULT_ADMIN_ROLE) nonReentrant {
        (bool success, ) = payable(shareholderAddress).call{value: address(this).balance}('');
...
contract SteppedDutchAuction {
    ...
    uint256 public auctionTimeStepSeconds;
     ...
     */
    event AuctionStart(uint256 indexed auctionStartTime, uint256 indexed auctionDuration);
    ...
    /**
     * @dev throws when auction parameters are not set correctly
     */
    constructor(
        uint256 startPrice,
        uint256 endPrice,
        ...
        require(
            startPrice >= endPrice && (startPrice - endPrice) >= priceStep,
            'Invalid auction parameters'
        );

        auctionStartPrice = startPrice;
        auctionEndPrice = endPrice;
        auctionPriceStep = priceStep;
        auctionTimeStepSeconds = timeStepSeconds;

        // startPrice greater than endPrice, should not underflow
        auctionDuration = ((startPrice - endPrice) / priceStep) * timeStepSeconds;
    }

    /**
     * @dev starts the auction and emits an event
     */
    function _startAuction() internal {
        require(!auctionActive, 'Auction has already started');

        auctionStartTime = block.timestamp;
        auctionActive = true;

        emit AuctionStart(auctionStartTime, auctionDuration);
    }

    /**
     * @dev ends the auction only if auction has started
     */
    function _endAuction() internal isAuctionActive {
        auctionActive = false;
    }

    /**
     * @dev returns the elapsed time since the start of the auction, or 0
     */
    function _getElapsedAuctionTime() internal view returns (uint256) {
        return auctionStartTime > 0 ? block.timestamp - auctionStartTime : 0;
    }

    /**
     * @dev returns the remaining sale time if it has started, or the auction duration if it hasn't started yet
     */
    function getRemainingSaleTime() external view returns (uint256) {
        if (auctionStartTime == 0 || !auctionActive) {
            return auctionDuration;
        } else if (_getElapsedAuctionTime() >= auctionDuration) {
            return 0;
        }

        return (auctionStartTime + auctionDuration) - block.timestamp;
    }

    /**
     * @dev calculates the current auction price. If not active, returns the auction start price
     */
    function getAuctionPrice() public view returns (uint256) {
        ...
        // step function
        uint256 steps = elapsed / auctionTimeStepSeconds;
        uint256 auctionPriceDecrease = steps * auctionPriceStep;

        return auctionStartPrice - auctionPriceDecrease;
    }
}
...
contract MerkleDistributorLimit {
    ...
    modifier ableToClaim(
        ...
        require(onAllowList(claimer, totalTokenAmount, proof), 'Not on allow list');
        _;
    }
    ...
    function onAllowList(
        ...
        bytes32 leaf = keccak256(abi.encodePacked(claimer, totalTokenAmount));
