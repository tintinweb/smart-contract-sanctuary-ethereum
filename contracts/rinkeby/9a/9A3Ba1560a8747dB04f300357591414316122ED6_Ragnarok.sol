...
contract Ragnarok is ERC1155, ERC1155Pausable, ERC1155Supply, Ownable {
    ...
    address payable public DEFAULT_PLATFORM_ADDRESS;
    bytes32 public merkleRootOfPixelMintWhitelistAddresses;
    bytes32 public merkleRootOfPillMintWhitelistAddresses;

    uint256 public immutable DEFAULT_SALE_START_TIME;
    uint256 public constant DEFAULT_MAX_MINTING_SUPPLY = 7777;
    uint256 public constant DEFAULT_MAX_FIRST_PUBLIC_SUPPLY = 5000;
    uint256 public constant DEFAULT_NFT_PRICE = 0.0077 * 1 ether;
    uint256 public constant DEFAULT_DECREASE_NFT_PRICE_AFTER_TIME_INTERVAL =
        0.000035 * 1 ether;
    uint256 public constant DEFAULT_TIME_INTERVAL = 10 minutes;
    uint256 public constant MAX_DECREASE_ITERATIONS = 16;

    uint256 public DUTCH_AUCTION_LAST_PRICE = 0;

    uint256 public constant DEFAULT_INITIAL_PUBLIC_SALE = 2 hours; // Actual value -> 1 days;
    uint256 public constant DEFAULT_PIXELMINT_SALE = 6 hours; // Actual value -> 3 days;
    uint256 public constant DEFAULT_PILLMINT_SALE = 8 hours; // Actual value -> 5 days;
    uint256 public constant DEFAULT_TEAMMINT_SALE = 10 hours; // Actual value -> 6 days;
    ...
    error TransactionFailed(bool sent, bytes data);
    ...
    ) public ERC1155(newURI) {
        DEFAULT_SALE_START_TIME = startTimestamp;
        DEFAULT_PLATFORM_ADDRESS = payable(platformAddress);
    ...
    function firstPublicMintingSale() external payable returns (bool) {
        if (checkSaleType() == SaleType.FirstPublicMint) {
            ...
            if (totalSupply(_tokenIds) > 0)
                revert NFTAlreadyMintedByID(_tokenIds);

            if (_tokenIds > DEFAULT_MAX_FIRST_PUBLIC_SUPPLY)
                revert MaximumPublicMintSupplyReached();

            if (firstPublicSale[msg.sender] >= 3)
            ...
            nftPriceStorage[_tokenIds].ownerAddress = payable(msg.sender);
            nftPriceStorage[_tokenIds].buyPrice = getPriceOFNFT;
    ...
    function pillMintingSale(bytes32[] calldata _merkleProof)
        ...
        returns (bool)
    {
        if (checkSaleType() == SaleType.PillMint) {
            _tokenIds++;
            ...
            if ((DUTCH_AUCTION_LAST_PRICE / 2) != msg.value)
                revert InvalidBuyNFTPrice(
                    (DUTCH_AUCTION_LAST_PRICE / 2),
                    msg.value
                );
    ...
    function teamMintingSale() external returns (bool) {
        if (checkSaleType() == SaleType.TeamMint) {
            if (msg.sender != DEFAULT_PLATFORM_ADDRESS)
                revert UnAuthorizedRequest();
    ...
    function lastPublicMintingSale() external payable returns (bool) {
        if (checkSaleType() == SaleType.LastPublicMint) {
            _tokenIds++;
            ...
            if (lastPublicSale[msg.sender] >= 3)
    ...
    {
        if (checkSaleType() == SaleType.FirstPublicMint) {
            if (tokenCount == 0) revert InvalidTokenCount(tokenCount);

            if (
                (totalSupply(_tokenIds + 1) > 0) ||
                (totalSupply(_tokenIds + tokenCount) > 0)
            ...
            if (firstPublicSale[msg.sender] >= 3)
                revert MaximumMintLimitReachedByUser();
            if (firstPublicSale[msg.sender] + tokenCount > 3)
                revert MaximumMintLimitReachedByUser();
        ...
        returns (bool)
    {
        if (checkSaleType() == SaleType.LastPublicMint) {
            ...
            if (lastPublicSale[msg.sender] >= 3)
                revert MaximumMintLimitReachedByUser();
            if (lastPublicSale[msg.sender] + tokenCount > 3)
                revert MaximumMintLimitReachedByUser();

            if ((DUTCH_AUCTION_LAST_PRICE * tokenCount) != msg.value)
                revert InvalidBuyNFTPrice(
                    (DUTCH_AUCTION_LAST_PRICE * tokenCount),
                    msg.value
                );
    ...
    function withdrawPayment() external returns (bool) {
        ...
        (bool sent, bytes memory data) = DEFAULT_PLATFORM_ADDRESS.call{value: contractBalance}("");

        if (!sent) revert TransactionFailed(sent, data);
        ...
        returns (uint256)
    {
        ...
        if (nftPriceStorage[tokenID].ownerAddress != msg.sender)
            revert UnAuthorizedRequest();

        if (nftPriceStorage[tokenID].isClaimed)
            revert ReimbursementAlreadyClaimed();

        if (balanceOf(msg.sender, tokenID) != 1) revert UnAuthorizedRequest();

        uint256 calculateReimbursement = nftPriceStorage[tokenID].buyPrice -
