...
contract WASDMinting is Ownable, Pausable {
    ...
    function getPhaseInfo()
        external
        view
        returns (uint256 currentPhase, int256 nextPhaseIn)
    {
        currentPhase = _getCurrentPhase();
        if (currentPhase == _phaseInfos.length) nextPhaseIn = -1;
        else {
            uint256 elapsedTime = block.timestamp - _deployedTime;
            uint256 totalDurations = 0;
            for (uint256 i = 0; i <= currentPhase; i++)
                totalDurations += _phaseInfos[i].duration;
            nextPhaseIn = int256(totalDurations) - int256(elapsedTime);
        }
    }
        ...
        )
    {
        ...
        if (!_rolePermission[_getCurrentPhase()][roleId])
            availableMintCount = 0;
        else {
            require(
    ...
    function _getCurrentPhase() private view returns (uint256) {
        uint256 elapsedTime = block.timestamp - _deployedTime;
