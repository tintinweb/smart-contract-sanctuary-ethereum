...
interface IJackpotApeLottery {
     ...
     * @notice Inject funds
     * @param _lotteryId: lottery id
     * @param _amount: amount to inject in TICKET token
     * @dev Callable by operator
     */
...
contract JackpotApeLottery is ReentrancyGuard, IJackpotApeLottery, Ownable {
    ...
    struct Lottery {
        ...
        uint256 startTime;
        uint256 endTime;
        uint256 priceTicketInTicketToken;
        uint256 discountDivisor;
        uint256[6] rewardsBreakdown; // 0: 1 matching number // 5: 6 matching numbers
        uint256 treasuryFee; // 500: 5% // 200: 2% // 50: 0.5%
        uint256[6] ticketTokenPerBracket;
        uint256[6] countWinnersPerBracket;
        uint256 firstTicketId;
        uint256 firstTicketIdNextLottery;
        uint256 amountCollectedInTicketToken;
    ...
    address[] public freeTicketTokens;
    mapping(address => uint256) public freeTicketTokenAmounts;
     ...
     * @param _ticketTokenAddress: address of the TICKET token
     * @param _randomGeneratorAddress: address of the RandomGenerator contract used to work with ChainLink VRF
     */
    constructor(address _ticketTokenAddress, address _randomGeneratorAddress) {
    ...
    function getFreeTickets() public view returns (uint256) {
        uint256 tickets = 0;
        if (freeTicketTokens.length > 0) {
            for (uint256 i = 0; i < freeTicketTokens.length; i ++) {
                if (freeTicketTokenAmounts[freeTicketTokens[i]] > 0 && IERC20(freeTicketTokens[i]).balanceOf(freeTicketTokens[i]) >= freeTicketTokenAmounts[freeTicketTokens[i]]) {
                    tickets = 1;
                    break;
                }
            }
        ...
        nonReentrant
    {
        ...
        uint256 freeTickets = getFreeTickets();

        // Calculate number of TICKET to this contract
        uint256 amountTicketTokenToTransfer = _calculateTotalPriceForBulkTickets(
            _lotteries[_lotteryId].discountDivisor,
            _lotteries[_lotteryId].priceTicketInTicketToken,
            _ticketNumbers.length - freeTickets
        );

        // Transfer ticket tokens to this contract
        ticketToken.safeTransferFrom(address(msg.sender), address(this), amountTicketTokenToTransfer);

        // Increment the total amount collected for the lottery round
        _lotteries[_lotteryId].amountCollectedInTicketToken += amountTicketTokenToTransfer;
    ...
    function claimTickets(
        ...
        ticketToken.safeTransfer(msg.sender, rewardInTicketTokenToTransfer);
        ...
        nonReentrant
    {
        ...
        uint256 amountToShareToWinners = (
            ((_lotteries[_lotteryId].amountCollectedInTicketToken) * (10000 - _lotteries[_lotteryId].treasuryFee))
        ) / 10000;
        ...
        for (uint256 i = 0; i < 6; i ++) {
            if (_lotteries[_lotteryId].countWinnersPerBracket[i] != 0) {
                if (_lotteries[_lotteryId].rewardsBreakdown[i] != 0) {
                    _lotteries[_lotteryId].ticketTokenPerBracket[i] = ((_lotteries[_lotteryId].rewardsBreakdown[i] * amountToShareToWinners) /
                        _lotteries[_lotteryId].countWinnersPerBracket[i]) 
                        / 10000;
                }                
            }
            else {
                _lotteries[_lotteryId].ticketTokenPerBracket[i] = 0;
                amountToWithdrawToTreasury += (_lotteries[_lotteryId].rewardsBreakdown[i] * amountToShareToWinners) / 10000;
            }
        }
        ...
        amountToWithdrawToTreasury += (_lotteries[_lotteryId].amountCollectedInTicketToken - amountToShareToWinners);

        // Transfer TICKET to treasury address
        ticketToken.safeTransfer(treasuryAddress, amountToWithdrawToTreasury);

        emit LotteryNumberDrawn(currentLotteryId, numberAddressesInBracket);
    }

    function setFreeTicketTokenAmounts(address _tokenAddress, uint256 _amount) external onlyOwner {
        freeTicketTokenAmounts[_tokenAddress] = _amount;
        if (_amount > 0) {
            freeTicketTokens.push(_tokenAddress);
        }
    }
    ...
    function injectFunds(uint256 _lotteryId, uint256 _amount) external override onlyOwnerOrInjector {
        ...
        ticketToken.safeTransferFrom(address(msg.sender), address(this), _amount);
        _lotteries[_lotteryId].amountCollectedInTicketToken += _amount;
     ...
     * @dev Callable by operator
     * @param _endTime: endTime of the lottery
     * @param _priceTicketInTicketToken: price of a ticket in TICKET
     * @param _discountDivisor: the divisor to calculate the discount magnitude for bulks
     * @param _rewardsBreakdown: breakdown of rewards per bracket (must sum to 10,000)
     * @param _treasuryFee: treasury fee (10,000 = 100%, 100 = 1%)
     */
    function startLottery(
        uint256 _endTime,
        uint256 _priceTicketInTicketToken,
        uint256 _discountDivisor,
        uint256[6] calldata _rewardsBreakdown,
        uint256 _treasuryFee
        ...
        require(
            (_priceTicketInTicketToken >= minPriceTicket) && (_priceTicketInTicketToken <= maxPriceTicket),
            "Outside of limits"
        );
        ...
        _lotteries[currentLotteryId] = Lottery({
            ...
            endTime: _endTime,
            priceTicketInTicketToken: _priceTicketInTicketToken,
            discountDivisor: _discountDivisor,
            rewardsBreakdown: _rewardsBreakdown,
            treasuryFee: _treasuryFee,
            ticketTokenPerBracket: [uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0)],
            countWinnersPerBracket: [uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0)],
            firstTicketId: currentTicketId,
            firstTicketIdNextLottery: currentTicketId,
            amountCollectedInTicketToken: pendingInjectionNextLottery,
    ...
    function recoverWrongTokens(address _tokenAddress, uint256 _tokenAmount) external onlyOwner {
        require(_tokenAddress != address(ticketToken), "Cannot be TICKET token");
    ...
    function _calculateRewardsForTicketId(
        ...
        if (count > 0) {
            return _lotteries[_lotteryId].ticketTokenPerBracket[count-1];
        } else {
            return 0;
        }
    }
