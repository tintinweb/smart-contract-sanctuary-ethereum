...
contract Land is ERC721, ERC2981, ReentrancyGuard, Ownable, Pausable {
    ...
    *
    * @notice Elite tokens are minted only during private and open sale
    *         while non-elite tokens are minted only during public and
    *         open sale
    *
    * @dev Passed on function saleMode
    */
    uint256 public eliteTokenStart;
    ...
    *      Open means open sale - mints both elite and non-elite tokens
    ...
    */
    ...
    /**
    * @dev Returns uint where 0, 1 and 2 means open,
    *      public and private sale respectively
    */
    ...
    */
    ...
    /**
    * @dev Maps tokenIds => unlock status to track individual elite
    *      tokenIds and its correspondng unlock status
    * @dev Default unlock status of all elite tokens is `false`
    * @dev If unlock status is false, elite tokens are not transferrable
    *      if unlock status is true, elite tokens are transferrable
    * @dev Once unlock status is set to true, it will be its permanent state
    */
    mapping(uint256 => bool) public eliteTokenUnlockStatus;
    ...
    */
    ...
    /**
    * @dev Fired in `unlockEliteToken()` when new eliteTokenUnlockStatus is set
    *
    * @param _tokenId elite token unlock status is set to true
    */
    event EliteTokenUnlocked(uint256 _tokenId);
    ...
    */
    ...
    /**
    * @dev Throws if _tokenId is not an elite token
    *
    * @param _tokenId elite token id to check if valid passed by caller
    */
    modifier isEliteTokenIdValid(uint256 _tokenId) {
        require(_tokenId >= eliteTokenStart, "Invalid elite token id");
        _;
    }
    ...
    */
    modifier isSaleStatusValid(uint8 _saleStatus) {
        require (
            _saleStatus == uint8(Sale.Open) ||
            _saleStatus == uint8(Sale.Public) ||
            _saleStatus == uint8(Sale.Private), "Invalid sale status"
        );
        _;
    }
    ...
    */
    ...
    /**
    * @param _tokenId elite tokenId is catched and throws
    *        if the unlock status is false
    *
    * @dev Used in overriden ERC721 internal function _transfer()
    */
    modifier isEliteTokenUnlocked (uint256 _tokenId) {
        if (_tokenId >= eliteTokenStart) {
            require (eliteTokenUnlockStatus[_tokenId] == true,
                "Elite token is locked");
        }
        _;
    }
    ...
    */
    ...
    /**
    * @dev Set the eliteTokenUnlockStatus of elite tokenIds
    *      to true, permanently.
    *
    * @param _tokenId throws if not a valid elite token
    *
    * @notice _tokenId is an elite token id
    */
    function unlockEliteToken(uint256 _tokenId)
        external
        onlyOwner
        isEliteTokenIdValid(_tokenId)
        isTokenExist(_tokenId)
    {
        eliteTokenUnlockStatus[_tokenId] = true;

        emit EliteTokenUnlocked(_tokenId);
    }
    ...
    *
    * @param _saleStatus accepts only uint
    *        0, 1, 2 which represents open,
    *        publc and private respectively
    */
    ...
    */
    ...
    /**
    * @dev Overrides _transfer() internal function of the inherited
    *      ERC721 contract
    * @dev Used in transferFrom() public function
    *      Used in _safeTransfer() internal function, thus covering
    *      the safeTransferFrom() function also.
    */
    function _transfer(
        address from,
        address to,
