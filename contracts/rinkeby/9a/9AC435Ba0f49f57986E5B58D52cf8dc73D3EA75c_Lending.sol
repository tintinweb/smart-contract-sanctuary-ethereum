...
contract Lending is ReentrancyGuard {
    ...
    event RewardRatesUpdated(uint256 newRewardRate, uint256 rewardUpdateBlock);
    ...
    event TokensDeposited(address indexed account, uint256 amount, uint256 amountAux);
    ...
    /// @dev when movemetns are bigger than this number we harvest yield.
    uint256 public harvestActivator;
    ...
    constructor(IMintableERC20 _usdctoken, IMintableERC20 _gsauxToken, IMintableERC20 _auxtoken, address _governance, address _sentinel) public {
        require(
            ...
            "GsdLending: governance address cannot be 0x0."
        );
        require(
            _sentinel != ZERO_ADDRESS,
            "GsdLending: sentinel address cannot be 0x0."
        );
        ...
        flushActivator = 10000 * 1e6;
        harvestActivator = 1000 * 1e6;
        ...
        _ctx.lastUpdateBlock = block.timestamp;
        _ctx.lastRewardRate = 0;
    }
    ...
    function setTransmuter(address _transmuter) external onlyGov {
        ...
        require(
            ...
            "GsdLending: transmuter address cannot be 0x0."
        );
    ...
    /// @dev Sets the harvestActivator.
    ///
    /// @param _harvestActivator the new harvestActivator.
    function setHarvestActivator(uint256 _harvestActivator) external onlyGov {
        harvestActivator = _harvestActivator;
    }

    /// @dev Sets the price oracle for AUXUSDC.
    ...
    function setStaking(address _staking) external onlyGov {
        ...
        // transfers to the address because of `safeTransfer` checks.
        require(
            _staking != ZERO_ADDRESS,
            "GsdLending: staking address cannot be 0x0."
        );
    ...
    function setStakingFee(uint256 _stakingFee) external onlyGov {
        ...
        require(
            _stakingFee <= PERCENT_RESOLUTION,
            "GsdLending: staking fee above maximum."
        );
    ...
    function setRewards(address _rewards) external onlyGov {
        ...
        require(
            ...
            "GsdLending: rewards address cannot be 0x0."
        );
    ...
    function setHarvestFee(uint256 _harvestFee) external onlyGov {
        ...
        require(
            ...
            "GsdLending: harvest fee above maximum."
        );
    ...
    function setCollateralizationLimit(uint256 _limit) external onlyGov {
        require(
            ...
            "GsdLending: collateralization limit below minimum."
        );
        require(
            _limit <= MAXIMUM_COLLATERALIZATION_LIMIT,
            "GsdLending: collateralization limit above maximum."
        );
    ...
    function initialize(IVaultAdapter _adapter) external onlyGov {
        ...
        require(
            ...
            "GsdLending: cannot initialize transmuter address to 0x0"
        );
        require(
            rewards != ZERO_ADDRESS,
            "GsdLending: cannot initialize rewards address to 0x0"
        );
        ...
        returns (uint256, uint256, uint256)
    {
        ...
        uint256 _totalYield = _vault.totalYield();
        if (_totalYield == 0) return (0,0,0);

        (uint256 _harvestedAmount, uint256 _decreasedValue) = _vault.harvest(address(this));
        if (_harvestedAmount == 0) return (0,0,0);
        uint256 _realizedAux = SwapRouter.swapUsdcForAux(_harvestedAmount);
    ...
    function _distributeAuxYield(uint256 _distributeAmount) internal {
        ...
        uint256 stakingShares = _distributeAmount.sub(transmuterShares);
        if (totalDepositedAux > 0 && stakingShares > 0) {
            FixedPointMath.FixedDecimal memory _weight = FixedPointMath.fromU256(stakingShares).div(totalDepositedAux);
    ...
    ///
    /// @return _claimable the amount of funds that were harvested from active vault.
    function claim() external expectInitialized returns (uint256 _claimable) {
        CDP.Data storage _cdp = _cdps[msg.sender];
        _cdp.update(_ctx);
        _claimable = _cdp.totalCredit;

        if (_claimable == 0) return 0;

        uint256 _balance = auxToken.balanceOf(address(this));
        if (_balance < _claimable) {
            _claimable = _balance;
        }

        auxToken.safeTransfer(msg.sender, _claimable);
        _cdp.totalCredit = _cdp.totalCredit.sub(_claimable);
    }
        ...
        expectInitialized
    {
        ...
        uint256 _amountAux = _oracle.usdcToAuxAmount(totalDepositedUsdc);
        totalDeposited = _amountAux;

        _cdp.totalDepositedUsdc = _cdp.totalDepositedUsdc.add(_amount);
        uint256 _cdpAmountAux = _oracle.usdcToAuxAmount(_cdp.totalDepositedUsdc);
        _cdp.totalDeposited = _cdpAmountAux;
        _cdp.lastDeposit = block.number;

        updateRewardRates();
        emit TokensDeposited(msg.sender, _amount, _amountAux);
    }
    ...
    /// note: we should modify checkHealth() method to take non-stablecoin debt into account.
    /// NoteAugusto: Checked.
    ...
    /// NoteAugusto: Checked.
    function repay(uint256 _usdcAmount, uint256 _gsAuxAmount)
        external
        nonReentrant
        noContractAllowed
        onLinkCheck
        expectInitialized
    {
        ...
        uint256 _totalAmount = _gsAuxAmount;
        uint256 _remainingDebt = _cdp.totalDebt;

        if (_gsAuxAmount > 0) {
            gsAuxToken.burnFrom(msg.sender, _gsAuxAmount);
            //lower debt cause burn
            gsAuxToken.lowerHasMinted(_gsAuxAmount);
            _remainingDebt = _remainingDebt.sub(_gsAuxAmount);
        }

        if (_usdcAmount > 0) {
            usdcToken.safeTransferFrom(msg.sender, address(this), _usdcAmount);
            uint256 _auxAmount = SwapRouter.swapUsdcForAux(_usdcAmount);
            _totalAmount = _totalAmount.add(_auxAmount);

            if (_auxAmount > _remainingDebt) {
                uint256 _diff = _auxAmount.sub(_remainingDebt);
                auxToken.safeTransfer(msg.sender, _diff);
                _auxAmount = _auxAmount.sub(_diff);
                _remainingDebt = 0;
            } else {
                _remainingDebt = _remainingDebt.sub(_auxAmount);
            }

            _distributeAuxYield(_auxAmount);
        }

        totalDebt = totalDebt.sub(_cdp.totalDebt.sub(_remainingDebt), "");
        _cdp.totalDebt = _remainingDebt;
    ...
    ///
    /// @param _amount the amount of gsdAux tokens to borrow.
    ...
    /// @dev Gets estimated usdc per block based on current earnings from last harvest block.
    ///
    /// The function takes in the last harvest block, current earnings and derive an estimated usdc per block.
    ///
    function rewardRate()
        external
        view
        returns (uint256)
    {
        if (block.timestamp <= _ctx.lastUpdateBlock) return _ctx.lastRewardRate;

        Vault.Data storage _vault = _vaults.last();
        uint256 diff = block.timestamp.sub(_ctx.lastUpdateBlock);
        return _vault.totalYield().div(diff);
    }

    /// @dev Gets total usdc current yield.
    ///
    /// @return the total usdc.
    function totalYieldUsdc()
        external
        view
        returns (uint256)
    {
        Vault.Data storage _vault = _vaults.last();
        return _vault.totalYield();
    }

    /// @dev Updates reward blocks and reward rates.
    ///
    function updateRewardRates()
        public
        expectInitialized
    {
        if (block.timestamp <= _ctx.lastUpdateBlock) return;

        Vault.Data storage _vault = _vaults.last();
        if (_vault.totalYield() == 0) {
            _ctx.lastUpdateBlock = block.timestamp;
            return;
        }

        uint256 diff = block.timestamp.sub(_ctx.lastUpdateBlock);
        _ctx.lastRewardRate = _vault.totalYield().div(diff);
        _ctx.lastUpdateBlock = block.timestamp;

        emit RewardRatesUpdated(_ctx.lastRewardRate, _ctx.lastUpdateBlock);
    }
    ...
    {
        CDP.Data storage _cdp = _cdps[_account];
        return _cdp.getEarnedYield(_ctx);
    }
    ...
    function _updateActiveVault(IVaultAdapter _adapter) internal {
        require(
            ...
            "GsdLending: active vault address cannot be 0x0."
        );
