...
contract TheHappyChemicalClub is Bundle {
    ...
    function mint(uint256 _quantity) public payable gate {
        require(msg.value >= mintPrice(), "TheHappyChemicalClub: not enough funds provided");
        require(_quantity + totalSupply() <= 4200, "TheHappyChemicalClub: maximum tokens minted");
        require(_quantity <= mintLimit(), "TheHappyChemicalClub: 10 tokens allowed to mint");
        require(_quantity + _mintLimit[msg.sender] <= mintLimit(), "TheHappyChemicalClub: 10 tokens allowed to mint");
