...
contract VaginalCavity is ERC20, Ownable, VRFConsumerBaseV2 {
    // Basic units
    ...
    // Stats for analytics
    uint256 public RPSLSWonAgainstChainlink;
    uint256 public RPSLSLostAgainstChainlink;
    uint256 public RPSLSDrawsAgainstChainlink;
    uint256 public RPSLSAmountWonAgainstChainlink;
    uint256 public RPSLSAmountLostAgainstChainlink;
    ...
    constructor(
        ...
        address _linkToken,
        bytes32 _keyHash
    ) ERC20("Vaginal Cavity", "VGN") VRFConsumerBaseV2(_vrfCoordinator) {
    ...
    function _settleRPSLSvsChainlink(
        ...
        // There is a special chance for a Lizard Wizard, when Lizard is played
        if (_move == Move.LIZARD) {
            if (rektageOrNot < 2) {
                // Rektage Finger
                _burn(address(this), _amountWagered);
                RPSLSLostAgainstChainlink++;
                RPSLSAmountLostAgainstChainlink += _amountWagered;
                ...
                RPSLSWonAgainstChainlink++;
                RPSLSAmountWonAgainstChainlink += _amountToWin;
                ...
                // Normal Logic
                if (rng > 2) {
                    uint256 _amountToWin = (_amountWagered *
                        RPSLSWinPercentage) / PERCENT_DENOMENATOR;
                    _transfer(address(this), _user, _amountWagered);
                    _mint(_user, _amountToWin);
                    RPSLSWonAgainstChainlink++;
                    RPSLSAmountWonAgainstChainlink += _amountToWin;
                    ...
                    _transfer(
                        address(this),
                        _user,
                        (_amountWagered * RPSLSDrawPercentage) /
                            PERCENT_DENOMENATOR
                    );
            ...
            // Any other move has the same logic (without Lizzy Wizzy)
            if (rng > 2) {
                uint256 _amountToWin = (_amountWagered * RPSLSWinPercentage) /
                    PERCENT_DENOMENATOR;
                _transfer(address(this), _user, _amountWagered);
        ...
        if (
            ...
            if (tax > 0) {
                if (_isSell && _taxIsElevated) {
                    tax = tax * (sellTaxUnwageredMultiplier - 1);
