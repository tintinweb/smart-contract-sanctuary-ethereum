...
import "./NativeMetaTransactions.sol";
    ...
    NativeMetaTransaction
{
    ...
    ) ERC721("RinkWursta Whitelist Test", "WURSTWL") OwnableRoyalties() {
    ...
    function mint(address _to) external onlyOwner {
        uint256 _id = mintCount.current() + startIndex;
        require(canMint(_id), "All NFTs have been minted.");
        _safeMint(_to, _id);
        mintCount.increment();
    }

    function airdrop(address[] memory _addrs) external onlyOwner {
        uint256 supply = mintCount.current();
        require((supply + _addrs.length) <= maxNFTs, "Can't mint that many.");
    ...
    function canMint(uint256 _id) public view virtual returns (bool) {
        return !_exists(_id) && _id >= startIndex && _id < startIndex + maxNFTs;
    }
