...
contract KosherKit is ERC721Enumerable, Ownable, ReentrancyGuard {
    ...
    uint256 public immutable maxWLMint = 1;
    uint256 public price = .199 ether;
    uint256 public renewalPrice = .199 ether;

    address private mTeam = 0x1243cFF98A1066381D760578a9b4432B31e3c693;
    address private kTeam = 0x7C2956ECa6BF48A2956a03787284BcAB6054E221;
    address private cTeam = 0xfcB86dd15ef02E31F18e6BDaf0b36f0e572CddEE;
    address private gTeam = 0x8aC9A5AcE0B0AcA69834eC04B8FA44450126462f;
    address private devAddr = 0x17Be91E788ccFe82E22aA6e9076A2a4f4c13F9D0;
    uint256 private mPCT = 29;
    uint256 private cPCT = 19;
    uint256 private gPCT = 19;
    uint256 private devPCT = 4;
    ...
    modifier validAmount(uint _amount) {
        ...
        require(
            ...
            "Mint will exceed max supply!"
        );
        _;
    }

    modifier validValue(uint _amount) {
        require(msg.value >= price*_amount, "Insufficient funds to cover mint");
        _;
    }
    ...
    validValue(_amount) {
        ...
        require(!whiteListClaimed[msg.sender], "Already claimed WL mint!");
    ...
    validAmount(_amount)
    validValue(_amount) {
        require(!paused && publicSaleEnabled, "Sale is not enabled.");
    ...
    function renewToken(uint _tokenId) external payable {
        require(_exists(_tokenId), "Token ID not minted!");
    ...
    function purgeBurnedIDs() public onlyOwner {
        delete burnedIds;
    }
    ...
    function resetWL(address[] memory _whitelist) external onlyOwner {
        for (uint i; i < _whitelist.length; ++i) {
            whiteListClaimed[_whitelist[i]] = false;
        }
    }
    ...
    function withdrawAll() public onlyOwner {
        ...
        if (initMint) {
            addedPCT = 0;
            _withdraw(devAddr, ((balance * devPCT) / 100)); //4% to dev
        }

        _withdraw(mTeam, ((balance * (mPCT + addedPCT)) / 100)); //29% to M
        _withdraw(cTeam, ((balance * (cPCT + addedPCT)) / 100)); //19% to C
        _withdraw(gTeam, ((balance * (gPCT + addedPCT)) / 100)); //19% to G
        _withdraw(kTeam, address(this).balance); //29% to K (coded like this so there is no remainder left in contract)
    }
