    ...
    Ownable
{
    ...
    function mint(address to, uint16 _id) public payable {
        require(!_exists(_id), "Token already minted");
        uint16 drop = checkDrop(_id);
    ...
    function enableDrop(uint drop, uint256 start, uint256 _price) public{
        require(enabledDrops[1] == 0 || enabledDrops[1] > block.timestamp , "Drop already enabled");
        enabledDrops[drop] = start;
        dropsPrices[drop] = _price;
    }
    
    function checkDrop(uint16 _tokenId) public view returns (uint16) {
