...
contract ACDMToken {
    ...
    address private _platform; // address of the ACDM platform

    modifier requirePlatform {
        require(msg.sender == _platform, "Not a platform");
        _;
    }
    ...
    function burn(address account, uint256 amount) public requirePlatform {
    ...
    function mint(address account, uint256 amount) public requirePlatform {
        _totalSupply += amount;
        _balances[account] += amount;
    ...
    function connectPlatform(address platform_) external requireOwner {
        require(_platform == address(0), "Already connected");
        _platform = platform_;
    }
}