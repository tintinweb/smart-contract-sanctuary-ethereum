...
contract PutinBloodChain is ERC721, Ownable, RaribleRoyaltyV2 {
  ...
  event CharityTransfer(address indexed to, uint256 indexed amount, uint256 indexed timestamp);

  string private __baseURI = "https://putinbloodchain.com/metadata/";

  uint256 private _price = 0.00125 ether; //g1.25 ether;
  uint256 private _minId = 1;
  uint256 private _maxId = 24;

  address public charity = 0xA59B29d7dbC9794d1e7f45123C48b2b8d0a34636; // uniceffrance.eth
  address public dev = 0x9Eb3a30117810d5a36568714EB5350480942f644; // mouradif.eth

  constructor() ERC721("PutinBloodChain", "PBC") {
    _safeMint(dev, 0);
    _setDefaultRoyalty(charity, 1000);
  }
  ...
  function transferToCharity() public {
    ...
    emit CharityTransfer(charity, balance, block.timestamp);
  }

  function updateCharityWallet(address _new) public onlyOwner {
    charity = _new;
  }
