...
contract Nuke is ERC721, IERC2981 {

  IERC721 private baseToken;
  ...
  constructor(
    ...
    baseToken = IERC721(baseTokenAddress_);
  ...
  function mint(uint tokenId) external {
    require(!_exists(tokenId), "mint: Token has already been minted");
    require(baseToken.ownerOf(tokenId) == _msgSender(),
        "mint: Only the owner of the original Nuketown NFT can mint");
    _mint(_msgSender(), tokenId);
  }

  /*** Nuke ***/

  function nukeLevel(uint tokenId) external view returns (uint) {
    require(_exists(tokenId), "nukeLevel: Token does not exist");
    uint rawLevel = uint(keccak256(abi.encode(salt, tokenId))) % 6;
    if (rawLevel < 3) return 1; // 3 out of 6
    if (rawLevel < 5) return 2; // 2 out of 6
    return 3; // 1 out of 66
  }

  function burn(uint tokenId) public {
    require(_isApprovedOrOwner(_msgSender(), tokenId),
        "burn: Caller is not owner or approved");
    _burn(tokenId);
  }
}
