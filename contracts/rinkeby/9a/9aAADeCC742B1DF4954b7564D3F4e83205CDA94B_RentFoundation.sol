    ...
    AccessControlEnumerable
{
    ...
    bytes32 public constant CTOKEN_CONVERTER = keccak256("CTOKEN_CONVERTER");
    ...
    function payRent(uint256 tokenID, uint256 amount) public {
        ...
        uint256 grainAmount = amount / grainPrices.prices(landXNFT.culture(tokenID));
   ...
   function payInitialRent(uint256 tokenID, uint256 amount) external {
    ...
    function sellCToken(address account, uint256 amount) public {
        require(hasRole(CTOKEN_CONVERTER, msg.sender), "not initial payer");
        require(xTokenRouter.getCToken(ICTOKEN(msg.sender).culture()) == msg.sender, "no valid cToken");
        string memory culture = ICTOKEN(msg.sender).culture();
        usdc.transfer(account, amount * grainPrices.prices(culture) / (10 ** 6));
    }
