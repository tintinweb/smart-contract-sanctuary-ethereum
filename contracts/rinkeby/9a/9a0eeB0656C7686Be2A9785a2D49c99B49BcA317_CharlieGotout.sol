...
contract CharlieGotout is ERC721A, Ownable, ReentrancyGuard{
    using Strings for uint256;
    ...
    constructor(
    ) ERC721A("YouGotCharlie V1", "YGCV1", 5, 101){
    }
   ...
   function mint(uint256 _quantity) public payable callerIsUser mintCompliance(_quantity) {
    ...
    require((totalPublicMint[msg.sender] +_quantity) <= maxMintAmountPerTx, "Maximum mint! Already minted 3 times!");
    ...
    function whitelistMint(bytes32[] memory _merkleProof, uint256 _quantity) public payable callerIsUser mintCompliance(_quantity){
        require(whiteListSale, "WhiteList Mint is on Pause");
