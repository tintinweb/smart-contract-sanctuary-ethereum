...
contract ICE64Renderer is Owned {
    ...
    function drawSVGToBytes(bytes memory data) public view returns (bytes memory) {
        string memory rects = xqstgfx.drawRectsUnsafe(data);
    ...
    function tokenURI(uint256 id) public view returns (string memory) {
        ...
        bytes memory data = ice64.getRawEditionPhotoData(id);
        string memory idString = ice64.getOriginalTokenId(id).toString();
