...
contract LittleAndy is Context, ERC1155, Ownable {
  ...
  uint256 public comicSalePrice = 37 * 10**15;
  uint256 public constant maxNfts = 2222;
  ...
  event UpdateRoyalty(address indexed _address, uint256 _bps);
    ...
    function _totalSupply() internal view returns (uint) {
        return _tokenIdTracker.current() + totalComics;
    }


    modifier saleIsOpen {
        require(_totalSupply() <= maxNfts, "Sale end");
    ...
    modifier comicSaleIsOpen {
        require(totalComics <= 1111, "Sale end");
    ...
    function mint(uint8 _count, address _to, uint256 code) public payable saleIsOpen {
        ...
        require(total + _count <= maxNfts, "Max limit");
