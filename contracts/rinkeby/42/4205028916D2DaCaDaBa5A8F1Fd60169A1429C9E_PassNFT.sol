    ...
    Ownable
{
    ...
    /** 
     ...
     @param _liqdNFTAddress address
     @param _maxMint  uint256 the max number of mints on this chain
     @param _baseTokenURI string token metadata URI
     */

    constructor(
        address _liqdNFTAddress,
        uint256 _maxMint,
        string memory _baseTokenURI
    ) ERC721A("Gold NFT", "GOLD-NFT") {
        require(_liqdNFTAddress != address(0));
        liqdNFTAddress = _liqdNFTAddress;
        maxMint = _maxMint;
    ...
    */
    ...
    function mint(address _user)
        ...
        returns (uint256)
    {
        require(_totalSupply + 1 <= maxMint, "GoldNFT: Max limit reached");
        uint256 tokenId = _tokenIds.current();

        //burn liqdnft
        LiqdNFT(liqdNFTAddress).burn(tokenId);
