...
/** 
 *  SourceUnit: /Users/xuezhao/Documents/hardhat_space/double/contracts/DoNFTFactory.sol
*/
...
contract DoNFTFactory is OwnableContract{
    ...
    address public wrapDoNftLogic;

    constructor(){
        initOwnableContract();
    }

    function deployWrapDoNFT(string memory name, string memory symbol,address originalAddress,address market,address tempRoyaltyAdmin,string calldata gameKey) external returns(ProxyAdmin proxyAdmin,TransparentUpgradeableProxy proxy) {
        ...
        proxy = new TransparentUpgradeableProxy(wrapDoNftLogic,address(proxyAdmin),_data);
    ...
    function deployWrapDoNFTWithAdmin(string memory name, string memory symbol,address originalAddress,address market,address tempRoyaltyAdmin,string calldata gameKey,address proxyAdmin) external returns(TransparentUpgradeableProxy proxy) {
        require(IERC165(originalAddress).supportsInterface(type(IERC721).interfaceId),"not ERC721");
        require(wrapDoNftMap[originalAddress][gameKey] == address(0),"already create");
        bytes memory _data = abi.encodeWithSignature("initialize(string,string,address,address,address)",name,symbol,originalAddress,market,tempRoyaltyAdmin);
        proxy = new TransparentUpgradeableProxy(wrapDoNftLogic,address(proxyAdmin),_data);
    ...
    function setWrapDoNftLogic(address logic) public onlyAdmin {
        wrapDoNftLogic = logic;
    }
