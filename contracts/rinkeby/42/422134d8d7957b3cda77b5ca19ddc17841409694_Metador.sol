...
contract Metador is ERC721A, Ownable {
    using Strings for uint256;
    ...
    function presaleMint(uint256 _amount) external payable {
        ...
        require(presaleActive, "Metador - Presale is not active.");
        require(
            totalSupply() + _amount <= maxSupply,
            "Metador - max NFT limit exceeded"
        );

        if (msg.sender != owner()) {
            require(
                isWhitelisted(_msgSender()),
                "Metador - Presale Only for Whitelist users."
            );
            require(
                _amount <= maxMintAmount,
                "Metador - max mint amount limit exceeded"
            );
            uint256 ownerMintedCount = addressMintedBalance[msg.sender];
            require(
                ownerMintedCount + _amount <= maxMintAmount,
                "Metador - max NFT per address exceeded"
            );
            require(
                msg.value >= cost * _amount,
                "Metador - insufficient ethers"
            );
        }
    ...
    function mint(uint256 _amount) external payable {
        require(!paused, "Metador - The Contract is Paused");
        require(mainsaleActive, "Metador - Mint is not active.");
        require(
            totalSupply() + _amount <= maxSupply,
            "Metador - max NFT limit exceeded"
        );
