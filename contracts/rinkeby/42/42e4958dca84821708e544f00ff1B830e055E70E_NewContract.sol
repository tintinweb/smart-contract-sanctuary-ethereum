...
contract NewContract is ERC721A, Ownable {
  // general
  ...
  constructor(address _mainWallet) payable ERC721A('No Financial', 'NOFINTHRY') {
    maxSupply = 4444;
    maxPerWalletWhitelist = 2;
    maxPerWalletPublic = 5;
    mainWallet = payable(address(_mainWallet));
  }
  ...
  */
  ...
  function setIsWhitelistMintEnabled(bool _isWhitelistMintEnabled)
    external
    onlyOwner
  {
  ...
  function setBaseTokenUri(string calldata _newBaseTokenUri)
    external
    onlyOwner
  {
  ...
  function setBaseTokenUriExt(string calldata _newBaseTokenUriExt)
    external
    onlyOwner
  {
    baseTokenUriExt = _newBaseTokenUriExt;
  }
  ...
  function withdraw() external onlyOwner {
    require(mainWallet != address(0), "Withdrawal not set");
  ...
  */
  function mintPublic(uint256 _quantity) public payable {
    ...
    require(msg.value == getPrice(_quantity), 'wrong value');
    require(
      _numberMinted(msg.sender) + _quantity <= maxPerWalletPublic,
      'exceeds max wallet'
    );
  ...
  function mintWhitelist(uint256 _quantity, bytes32[] calldata _proof)
    public
    payable
  {
    ...
    require(tx.origin == msg.sender, 'contracts not allowed');
    require(msg.value == getPrice(_quantity), 'wrong value');
    require(checkWhitelist(_proof), 'address supplied is not on the whitelist');
    require(
      _numberMinted(msg.sender) + _quantity <= maxPerWalletWhitelist,
      'exceeds max wallet whitelist'
    );
    require(totalSupply() < maxSupply, 'sold out');
    require(totalSupply() + _quantity <= maxSupply, 'exceeds max supply');
  ...
  /**
   ...
   ============================================
  */
  function checkWhitelist(bytes32[] calldata _proof)
    public
    view
    returns (bool)
  {
