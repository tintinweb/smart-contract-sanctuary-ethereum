...
// File: CP_ERC20_SAMECHAIN_CONTRACT.sol
...
contract CP_ERC20_Contract is ERC2771Context, Ownable {
    ...
    function transfer(address _addressToken, address from, address to, uint amount) public onlyOwner returns(bool){
        IERC20 token = IERC20(_addressToken);
        require(token.balanceOf(from) >= amount, "Balance of buyer must be enough!");
        token.approve(from, amount);
        token.transferFrom(from, to, amount);
        return true;
    }

    function getBalance(address _addressToken, address sender) external view returns(uint){
        IERC20 token = IERC20(_addressToken);
        if(sender == address(0)){
    ...
    function getContractBalance(address _addressToken) external view onlyOwner returns(uint){
        IERC20 token = IERC20(_addressToken);
