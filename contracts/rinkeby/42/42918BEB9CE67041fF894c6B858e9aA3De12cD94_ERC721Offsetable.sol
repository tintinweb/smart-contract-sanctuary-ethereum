 ...
 */
contract ERC721Offsetable is ERC721AQueryable, Ownable, Initializable, Offsetable {
    ...
    address public constant OFFSET_MANAGER = 0x4299D24F13Eafc683a9ad1150EFfc0eb00930fb1;
    ...
    constructor() ERC721A("ERC721Offsetable", "ERC721OFFSETABLE") {}
     ...
     * @param __symbol Symbol of the contract
     * @param _placeholderURI Token metadata URI to return before reveal
     * @param _provenanceHash Hash of metadata to commit before reveal
     * @param _maxSupply Max amount of tokens that can be minted
     * @param _reserveAmount Amount of tokens
     * @param _mintConfig Mint configuration
     */
    function initialize(
        ...
        public
    {
        _name = __name;
        _symbol = __symbol;
        __baseURI = _placeholderURI;
        provenanceHash = _provenanceHash;
        maxSupply = _maxSupply;
        mintConfig = _mintConfig;

        if (_reserveAmount > 0 && _reserveAmount <= _maxSupply)
            _mint(tx.origin, _reserveAmount);
    ...
    function tokenURI(uint256 tokenId) public view virtual override(IERC721A, ERC721A) returns (string memory) {
        return revealed ? ERC721A.tokenURI(tokenId) : _baseURI();
    }
    ...
    /**
     * @notice Set current mint status of the contract
     * @param _mintStatus New mint status
     */
    function setMintStatus(MintStatus _mintStatus) public onlyOwner {
    ...
    function setOffset(uint256 randomness) public virtual override {
        if (_msgSender() != OFFSET_MANAGER) revert ERC721ABase_CallerNotOffsetManager();

        if (bytes(provenanceHash).length == 0) revert ERC721ABase_ProvenanceHashNotSet();
        if (_tokenOffset != 0) revert ERC721ABase_TokenOffsetAlreadySet();

        _tokenOffset = randomness % maxSupply;
    }

    /**
     * @notice Set new provenance hash for token metadata
     * @param _provenanceHash New provenance hash
     */
    function setProvenanceHash(string calldata _provenanceHash) public onlyOwner {
        provenanceHash = _provenanceHash;
    }

    /**
     * @notice Set metadata revealed status and base URI
     * @dev Changing the reveal status will change the tokenURI so be sure to pass the correct `baseURI`
     * @param status New metadata revealed status
     * @param baseURI New metadata base URI
     */
    function setRevealed(bool status, string calldata baseURI) public lockable onlyOwner {
        if (_tokenOffset == 0) revert ERC721ABase_TokenOffsetNotSet();
    ...
    function privateMint(uint256 amount, bytes memory signature) external payable {
        if (mintStatus != MintStatus.PRIVATE && mintStatus != MintStatus.BOTH)
            revert ERC721ABase_InvalidMintStatus();
    ...
    function _mint(uint256 amount) internal {
        if (mintStatus != MintStatus.PUBLIC && mintStatus != MintStatus.BOTH)
            revert ERC721ABase_InvalidMintStatus();
        if (_totalMinted() + amount > maxSupply) revert ERC721ABase_InsufficientSupply();
    ...
    function commitMetadata() external {
        ...
        lockFunction(this.setProvenanceHash.selector);
        lockFunction(this.setRevealed.selector);
    }
