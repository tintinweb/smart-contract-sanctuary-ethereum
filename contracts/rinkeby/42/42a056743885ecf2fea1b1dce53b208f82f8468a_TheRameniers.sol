...
contract TheRameniers is ERC721, ERC721Enumerable, Ownable, ReentrancyGuard {  
    using Address for address;
    ...
    function mintPresale(uint256 _amount) public payable {
        ...
        require( supply + _amount <= MAX_PRESALE_SUPPLY,            "Can't mint more than max supply of the Pre Sale." );
    ...
    function mintToken(uint256 _amount) public payable {
        ...
        require( supply + _amount <= (MAX_SUPPLY-MAX_PRESALE_SUPPLY-reserved),          "Can't mint more than max supply. Not enough tokens remaining to mint." );
        require( msg.value == sale_price * _amount,                                     "Wrong amount of ETH sent" );
        for(uint256 i; i < _amount; i++){
            _safeMint( msg.sender, supply + i );
        }
    }
  ...
  /** URI HANDLING **/
    //string private customBaseURI= "https://ipfs.io/ipfs/QmSuECBtENsfHredQDfKCEN2Tc8HjkvMjGDCWgUfHQL8eA/";
    string private customBaseURI= "https://ipfs.io/ipfs/QmNecbbWcL4wMMxKFkHBKHaFwh1NSQPVcuq1k2euYNYQFv/";
...
// ============ OWNER-ONLY ADMIN FUNCTIONS ============
    ...
    // Set a new amount of reserved tokens
    function setReserved(uint256 newReserved) public onlyOwner {
        reserved = reserved + newReserved;
    }

    /** PAYOUT **/
    /** Withdraws all the money to the addres owner. **/
    ...
    /** Withdraws an amoutn of money specified in the first parameter to the addres owner. **/
    ...
    /** Withdraws money to the addres specified in the first parameter. Amount in second. **/
    function withdrawMoneyTo(address to, uint256 _amount) public onlyOwner{
        address payable payableTo = payable(to);
        uint256 balance = address(this).balance;
        require(balance > 0, "No ether left to withdraw");
        Address.sendValue(payableTo, _amount);
    }
    ...
    // Provenance
    string private TheRameniers_HASH;
    function setProvenanceHash(string memory provenanceHash) external onlyOwner  {
        TheRameniers_HASH = provenanceHash;
    }
