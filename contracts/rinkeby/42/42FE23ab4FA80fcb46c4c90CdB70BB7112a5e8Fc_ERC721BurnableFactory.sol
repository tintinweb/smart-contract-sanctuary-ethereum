    ...
    UUPSUpgradeable
{
    ...
    function isApprovedForAll(address owner_, address operator_) public view virtual override returns (bool) {
        ...
        if (status != ApprovalStatus.Deny && _gaslessListingManager.checkAllMarketplaceAddresses(owner_, operator_)) {
            return true;
        }
...
contract GaslessListingManager is Initializable, OwnableUpgradeable {
    address private looksRareTransferManagerERC721;
    address private openSeaProxyRegistry;
    address private raribleTransferProxy;
    address private sloikaFixedPriceAuction;
    address private sloikaGachaAuction;
    ...
    function initialize(
        ...
        __Ownable_init();
        looksRareTransferManagerERC721 = looksRareTransferManagerERC721_;
        openSeaProxyRegistry = openSeaProxyRegistry_;
        raribleTransferProxy = raribleTransferProxy_;
        sloikaFixedPriceAuction = sloikaFixedPriceAuction_;
        sloikaGachaAuction = sloikaGachaAuction_;
    }

    function setLooksRareTransferManagerERC721(address looksRareTransferManagerERC721_) external onlyOwner {
        looksRareTransferManagerERC721 = looksRareTransferManagerERC721_;
    }
    ...
    function setRaribleTransferProxy(address raribleTransferProxy_) external onlyOwner {
        raribleTransferProxy = raribleTransferProxy_;
    }

    function setSloikaFixedPriceAuction(address sloikaFixedPriceAuction_) external onlyOwner {
        sloikaFixedPriceAuction = sloikaFixedPriceAuction_;
    }

    function setSloikaGachaAuction(address sloikaGachaAuction_) external onlyOwner {
        sloikaGachaAuction = sloikaGachaAuction_;
    }

    function checkLooksRareForGaslessListing(address operator_) internal view returns (bool) {
        return looksRareTransferManagerERC721 == operator_;
    }

    function checkOpenSeaForGaslessListing(address owner_, address operator_) internal view returns (bool) {
        IWyvernProxyRegistry wyvernProxyRegistry = IWyvernProxyRegistry(openSeaProxyRegistry);

        return address(wyvernProxyRegistry.proxies(owner_)) == operator_;
    }

    function checkRaribleForGaslessListing(address operator_) internal view returns (bool) {
        return raribleTransferProxy == operator_;
    }

    function checkSloikaFixedPriceForGaslessListing(address operator_) internal view returns (bool) {
        return sloikaFixedPriceAuction == operator_;
    }

    function checkSloikaGachaAuctionForGaslessListing(address operator_) internal view returns (bool) {
        return sloikaGachaAuction == operator_;
    }

    function checkAllMarketplaceAddresses(address owner_, address operator_) public view returns (bool) {
        return
            checkOpenSeaForGaslessListing(owner_, operator_) ||
            checkLooksRareForGaslessListing(operator_) ||
            checkSloikaFixedPriceForGaslessListing(operator_) ||
            checkSloikaGachaAuctionForGaslessListing(operator_) ||
            checkRaribleForGaslessListing(operator_);
    }
}
