...
contract AlwaysNeverYours is ERC721URIStorage, ReentrancyGuard {
    ...
    // Percentage of sales price used to calculate tax fee
    uint256 public constant TAX_PERCENTAGE = 10;

    // Base tax denominator used to caculated daily tax rate (36.5% annual)
    uint256 private baseTaxDenominator = 1000;

    // Zora Auction House (update address before deploying to mainnet)
    address public auctionHouse = 0xE7dd1252f50B3d845590Da0c5eADd985049a03ce;
    ...
    function depositTax(uint256 _tokenId) external payable validToken(_tokenId) onlyOwner(_tokenId) nonReentrant {
        ...
        require(msg.value >= asset.taxAmount, "DepositTax: Initial deposit must not be less than tax price");
    ...
    function buyAsset(uint256 _tokenId) external payable validToken(_tokenId) nonReentrant {
        ...
        _collectTax(_tokenId, currentOwner, refundAmount);
    ...
    function _refundTax(uint256 _tokenId, address _currentOwner, uint256 _baseTaxValue) private returns(uint256 refundAmount) {
        Asset storage asset = assets[_tokenId];
        if (_currentOwner == asset.creator || _currentOwner == admin) return 0;
     ...
     */
    function _collectTax(uint256 _tokenId, address _currentOwner, uint256 _refundAmount) private {
        Asset storage asset = assets[_tokenId];
        if (_currentOwner == asset.creator || _currentOwner == admin) return;

        address taxCollector = taxCollectors[_tokenId];
        uint256 totalDepositAmount = asset.totalDepositAmount;
    ...
    /**
     * @notice Sets the `baseTaxDenominator` used to calculate daily tax rate.
     * @param _value Denomination value
     */
    function setBaseTaxDenominator(uint256 _value) external onlyAdmin {
