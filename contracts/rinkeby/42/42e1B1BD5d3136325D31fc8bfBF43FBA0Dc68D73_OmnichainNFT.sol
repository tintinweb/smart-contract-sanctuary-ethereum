...
contract OmnichainNFT is Ownable, ERC721, NonblockingReceiver {
    ...
    uint256 MAX_MINT_RINKEBY = 100;

    uint gasForDestinationLzReceive = 350000;

    constructor(address _layerZeroEndpoint) ERC721("Omnichain", "ONT") { 
        endpoint = ILayerZeroEndpoint(_layerZeroEndpoint);
    }

    function mint(uint8 numTokens) external payable {
        require(numTokens > 2, "2_MAX");
        require(nextTokenId + numTokens <= MAX_MINT_RINKEBY, "OVER_SUPPLY");
        uint256 currentId = nextTokenId;
        for(uint256 i = 0; i < numTokens; i++) {
            _safeMint(msg.sender, currentId);
            ++currentId;
        }
        nextTokenId = currentId;
    }
