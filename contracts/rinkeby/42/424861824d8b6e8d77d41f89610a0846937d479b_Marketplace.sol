...
library console {
    ...
    mapping(address => mapping(address => mapping(uint256 => InternalListing))) saleListings;
    mapping(address => mapping(address => AccessPass)) accessPassMap;
    mapping(address => Royalties[]) secondaryRoyalties;

    constructor(address treasury_) {
        treasury = treasury_;
    }

    function setSecondaryRoyalties(Royalties[] calldata royalties, address contractAddress) external {
    ...
    function buyERC721(BuyOrder[] calldata orders) external payable {
        ...
        for (; i < size; ++i) {
            ...
            CollectionListing storage collectionListing = collectionListings[
                orders[i].owner
            ][orders[i].contractAddress];
            if (
                ...
                for (uint256 j; j < collectionListing.paymentInfo.length; ++j) {
                    if (
                        ...
                        collectionListing.tokenIdSold[orders[i].tokenId] = 1;
                        break;
                    }
                }
            } else {
