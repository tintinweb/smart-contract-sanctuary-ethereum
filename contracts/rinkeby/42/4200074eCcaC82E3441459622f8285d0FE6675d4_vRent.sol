...
contract vRent is IvRent, ERC721Holder { 
    ...
    */
    ...
    function manageLease(CallData memory _cd) private {
        ...
        // set token URI
        ERCX(ERCXImp).setTokenUri(_cd.tokenIds[i], ERC721(_cd.nfts[i]).tokenURI(_cd.tokenIds[i]));
        // set lien
        _ERCX.setLien(leasingId);
        IERC721(_cd.nfts[i]).transferFrom(msg.sender, address(this), _cd.tokenIds[i]);
        leasingId++;
        }
    }
    ...
    function manageStopLeasing(CallData memory _cd) private {
        ...
        for (uint256 i = 0; i < _cd.nfts.length; i++) { 
            ...
            IERC721(_cd.nfts[i]).transferFrom(address(this), msg.sender, _cd.tokenIds[i]);
        }
    }
