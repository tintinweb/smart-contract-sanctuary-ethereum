...
import "./libraries/ERC721BatchEnumerable.sol";
import "./libraries/ERC2981Base.sol";
import "./libraries/IOwnable.sol";
import "./libraries/IPausable.sol";
import "./libraries/ITradable.sol";
import "./libraries/IWhitelistable.sol";

contract EZU is ERC721BatchEnumerable, ERC2981Base, IOwnable, IPausable, ITradable, IWhitelistable {
	// Errors
	...
	// Presale mint price
	uint public immutable PRESALE_MINT_PRICE = 0.05 ether;				// STILL UNDETERMINED

	// Public mint price
	uint public immutable PUBLIC_MINT_PRICE = 0.06 ether;				// STILL UNDETERMINED

	// Max supply
	uint public immutable MAX_SUPPLY = 15_000;

	// Max presale supply
	uint public immutable MAX_PRESALE_SUPPLY = 10_500;

	// Max per txn
	uint public immutable MAX_PER_TXN = 10; 							// STILL UNDETERMINED

	// Max allowance
	uint public immutable MAX_ALLOWANCE = 10; 							// STILL UNDETERMINED
	...
	// **************************************
		...
		*/
		function mintPreSale( uint256 qty_, bytes32[] memory proof_ ) external payable presaleOpen isWhitelisted( _msgSender(), proof_, MAX_ALLOWANCE, qty_ ) {
			if ( _supplyMinted() + 1 > MAX_PRESALE_SUPPLY ) {
	...
	// **************************************
		/**
		* @dev Allows owner to mint tokens for future airdrops
		* 
		* Requirements:
		* 
		...
		* - There must be enough tokens left to mint outside of the reserve.
		*/
		function ownerMint( uint256 qty_ ) external onlyOwner saleOpen {
			uint256 _endSupply_  = _supplyMinted() + qty_;
			if ( _endSupply_ > MAX_SUPPLY ) {
				revert EZU_MAX_SUPPLY();
			}
		...
		*/
		...
		/**
		* @dev Withdraws all the money stored in the contract and sends it to contract owner.
		* 
		* Requirements:
		* 
		* - Caller must be the contract owner.
		*/
		function withdraw() external onlyOwner {
			...
			if ( _balance_ == 0 ) {
				revert EZU_NO_ETHER_BALANCE();
			}
			_sendValue( payable( _msgSender() ), _balance_ );
		}
	// **************************************
	...
	// **************************************
		...
		*/
		function supportsInterface( bytes4 interfaceId_ ) public view virtual override(ERC721BatchEnumerable, ERC2981Base) returns ( bool ) {
			return 
...
abstract contract IPausable {
	...
	*/
	...
	/**
	* @dev Sale state can have one of 4 values, ``CLOSED``, ``PRESALE``, ``FREESALE``, or ``SALE``.
	*/
