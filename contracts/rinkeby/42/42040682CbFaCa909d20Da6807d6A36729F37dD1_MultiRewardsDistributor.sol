 ...
 * @notice It distributes RYLT tokens with parallel rolling Merkle airdrops.
...
contract MultiRewardsDistributor is Pausable, ReentrancyGuard, Ownable {
    ...
    // Time buffer for the admin to withdraw RYLT tokens if the contract becomes paused
    uint256 public constant BUFFER_ADMIN_WITHDRAW = 3 days;

    // Standard safe guard amount (set at 1 RYLT)
    ...
    // Total amount claimed by user (in RYLT)
     ...
     */
    constructor(address _ryltToken) {
    ...
    /**
     * @notice Transfer RYLT tokens back to owner
