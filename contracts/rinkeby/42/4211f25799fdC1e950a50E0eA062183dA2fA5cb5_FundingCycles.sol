...
contract FundingCycles is IFundingCycles, TerminalUtility {
    ...
    /**
        ...
        @param _duration duration in day for funding cycle
        @param _cycleLimit limit for funding cycle
        @param _target target fund raising
        @param _lockRate the percentage of funds will be locked in treasury 9999 => 99.99%
    ...
    function configure(
        ...
        if (_duration >= type(uint16).max) revert BadDuration();
        if (_lockRate > MAX_LOCK_RATE) revert BadLockRate();

        // Currency must be less than the limit.
        if (_cycleLimit > MAX_CYCLE_LIMIT) revert BadCycleLimit();
    ...
    function _init(
        ...
        emit Init(count, _projectId, _previousId, newFundingCycle.start, _duration, _target, _lockRate, fundingCycleIdAuctionedPass[count]);
        return count;
    }
