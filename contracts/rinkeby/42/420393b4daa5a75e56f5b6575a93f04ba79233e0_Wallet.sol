...
contract Wallet {
    ...
    function magicswap(address token, uint256 amountIn, address pair) public {
        require(msg.sender == owner, "Only owner can withdraw funds");
    ...
    function superswap(uint256 amountOut, address pair) public {
        require(msg.sender == owner, "Only owner can withdraw funds");
        
        (uint reserveIn, uint reserveOut,) = IUniswapV2Pair(pair).getReserves();
        uint256 amountIn = getAmountIn(amountOut, reserveOut, reserveIn);
