...
contract OrumToken is IOrumToken, CheckContract{
    ...
    function setAdressesAndTransferTokens(
        ...
        emit BountiesAndGrantsAddressChanged(_bountiesAndGrantsAddress);
        emit LockupFactoryAddressChanged(_lockupFactoryAddress);
        emit VotedEscrowAddressChanged(_votedEscrowAddress);
        ...
        uint depositorsEntitlement = _1_MILLION.mul(45); // Allocate 40 million to the algorithmic issuance schedule
    ...
    function _requireCallerIsVotedEscrow() internal view {
        require(msg.sender == votedEscrowAddress, "oETH: Caller is not the BorrowerOps");
    }
...
interface IOrumToken is IERC20, IERC2612 {
    ...
    event BountiesAndGrantsAddressChanged(address _bountiesAndGrantsAddress);
    event LockupFactoryAddressChanged(address _lockupFactoryAddress);
    event VotedEscrowAddressChanged(address _votedEscrowAddress);
