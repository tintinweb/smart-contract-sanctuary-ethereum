  ...
  AttestationsRegistryConfigLogic
{
  ...
  function recordAttestation(Attestation calldata attestation) external override whenNotPaused {
    ...
    if (!_isAuthorized(issuer, attestation.collectionId))
      revert IssuerNotAuthorized(issuer, attestation.collectionId);

    uint256 previousAttestationValue = _attestationsData[attestation.collectionId][
      attestation.owner
    ].value;
  ...
  {
    AttestationData memory data = _attestationsData[collectionId][owner];
    return (data.issuer, data.value, data.timestamp, data.extraData);
  }
