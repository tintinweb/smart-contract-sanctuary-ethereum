...
contract PottedPotted is Ownable, ERC721A, ReentrancyGuard{
    ...
    */
    ...
    PPRenderer public renderer = PPRenderer(0x5429d27325C8765166C8a0faa203466eDa4F581B);
    ...
    *******/
    function setPottedReveal(uint tokenId, uint revealNum) external validOwner(tokenId) {
        ...
        potteds[tokenId] = PottedTypes.Gene(potteds[tokenId].dna, revealNum, false);
    }

    /*******
    * Reroll by owner
    *******/
    function rerollPotted(uint tokenId, uint petId) external validOwner(tokenId) {
        address petOwner = ERC721A(config.petContract).ownerOf(petId);
        require(!potteds[tokenId].isReroll, "rerolled");
        require(petOwner == msg.sender, "Not pet owner");
        require(petId >= 2000 && petId <= 3825, "Not valid Tamagogi Pets id");
    ...
    *******/
    function previewPottedNoBg(uint tokenId, uint scale) external onlyReveal(tokenId) view returns (string memory) {
    ...
    *******/
    ...
    function _getUnreveal(PottedTypes.Gene memory gene, PottedTypes.MyPotted memory myPotted) private view returns (string memory) {
        string memory json = 
                string(
                    abi.encodePacked(
                        '{"name": "#",',
                        ...
                        '"attributes": [',
                            ...
                            '{ "trait_type": "pot", "value": "',myPotted.potted.traitName,'"},',
    ...
    function _getReveal(PottedTypes.Gene memory gene, PottedTypes.MyPotted memory myPotted) private view returns (string memory) {
    ...
    function _tokenURI(uint256 tokenId) private view  returns (string memory) {
        PottedTypes.Gene memory gene = potteds[tokenId];
        string memory json = potteds[tokenId].revealNum == 0 ? _getUnreveal(gene, renderer.getPP(gene)) : _getReveal(gene, renderer.getPP(gene));
