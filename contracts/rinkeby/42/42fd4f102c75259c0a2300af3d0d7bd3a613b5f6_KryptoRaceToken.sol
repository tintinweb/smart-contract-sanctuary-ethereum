...
contract KryptoRaceToken is IBEP20, Context, Ownable {
    ...
    constructor(address addressOfRewardToken) {
        _balances[_msgSender()] = _totalSupply;
        emit Transfer(address(0), _msgSender(), _totalSupply);
        iReward = IReward(addressOfRewardToken);
    }
    ...
    function setNumberOfToken (uint256 _amount) public onlyOwner {
        tokenAmount = _amount;
    }

    function setRewardToken(address _address) public onlyOwner {
        iReward = IReward(_address);
    }

    function gamePlay(uint256 _sessionId) public {
        address  _player = msg.sender;
        // require(msg.value >= 0.0001 * 10**18, "you can't have enough balance");
        // _transfer(_player, address(this), 10000000 * 10**9);
        iReward.transferFrom(_player, address(this), tokenAmount * 10**9);
    ...
    function checkPaid(address player, uint256 id) public view returns(bool) {
     ...
     function claimReward(uint256 _sessionId) public {
        ...
        require(gameRecords[_sessionId].length >= 1, "No record found against this sessionId");
        for (uint i = 0; i<=gameRecords[_sessionId].length; i++){
            if(gameRecords[_sessionId][i] == winnerAddress){
               
                if(gameRecords[_sessionId].length > 1){
                iReward.transfer(winnerAddress, gameRecords[_sessionId].length * tokenAmount * 10**9);

                }else{
                iReward.transfer(winnerAddress, tokenAmount * 10**9);
