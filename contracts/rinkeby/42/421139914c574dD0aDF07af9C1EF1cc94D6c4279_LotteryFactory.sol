...
contract Lottery is Ownable, VRFConsumerBaseV2 {
    ...
    mapping(uint16 => address[]) public participants;
    ...
    function buy() payable external timedTransitions atStage(Stages.IN_PROGRESS) {
    ...
    function fulfillRandomWords(uint256 requestId, uint256[] memory randomNumbers) internal override {
        for (uint i = 0; i < randomNumbers.length; i++) {
            ...
            participants[ticket].push(requestToSender[requestId]);
        }
    }

    function withdraw() external onlyOwner atStage(Stages.FINISHED) {
        uint currentBalance = address(this).balance;
        uint ownerProfit = currentBalance / 10;
        uint participantsProfit = currentBalance - ownerProfit;
        uint participantProfit = participantsProfit / participants[maxCurrentTicket].length;

        payable(owner()).transfer(ownerProfit);
        for (uint i = 0; i < participants[maxCurrentTicket].length; i++) {
            payable(participants[maxCurrentTicket][i]).transfer(participantProfit);
        }
    }
}
