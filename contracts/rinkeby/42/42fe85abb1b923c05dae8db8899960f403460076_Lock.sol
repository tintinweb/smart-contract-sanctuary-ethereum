...
contract Lock is ILock, Ownable, Initializable, ReentrancyGuard {
    using SafeERC20 for IERC20;

    address public controllerAddress;
    mapping(address => mapping(address => uint256)) public userBalances;
    mapping(address => mapping(address => uint256)) public updatedLockTime;

    event ClaimToken(address indexed claimer, address indexed tokenAddress, uint256 amount, uint256 timestamp);

    constructor() {}

    function initialize(address _controllerAddress) external onlyOwner initializer {
        controllerAddress = _controllerAddress;
    }

    function lock(address userAddress, address tokenAddress, uint256 tokenAmount) external override onlyOwner {
        userBalances[userAddress][tokenAddress] += tokenAmount;
        updatedLockTime[userAddress][tokenAddress] = block.timestamp;
    } 

    function claim(address tokenAddress) external override nonReentrant {
        uint256 lockTime = IController(controllerAddress).lockTime();
        require((block.timestamp - updatedLockTime[msg.sender][tokenAddress]) > lockTime, "Still locking");
        require(userBalances[msg.sender][tokenAddress] > 0, "Nothing to be claimed");
        uint256 tokenAmount = userBalances[msg.sender][tokenAddress];
        IERC20(tokenAddress).safeTransfer(msg.sender, tokenAmount);
        delete userBalances[msg.sender][tokenAddress];
        delete updatedLockTime[msg.sender][tokenAddress];
        emit ClaimToken(msg.sender, tokenAddress, tokenAmount, block.timestamp);
    }
}