...
contract Grill is Ownable, ERC1155Holder {
    ...
    constructor(address _parentAddr) {
        ...
        emissions[emChanges.current()] = Emission(
            secondsIn45Days,
            block.timestamp
        );
    }
    ...
    function setEmissionRate(uint256 _seconds) external onlyOwner {
        require(
            !BAILED_OUT,
            "GRILL: cannot change emission rate after bailout"
        );
    ...
    function _isOwnerOfBatch(
        ...
        for (uint256 i = 0; i < _tokenIds.length; ++i) {
            if (
                parentBalance(_operator, _tokenIds[i]) == 0 || _amounts[i] != 1
            ) {
                _b = false;
                break;
            }
    ...
    function _addStakes(address _staker, uint256[] memory _tokenIds) private {
        for (uint256 i = 0; i < _tokenIds.length; ++i) {
            ...
            /// set mappings
            addrStakesIds[_staker][
                addrStakesCount[_staker].current()
            ] = _tokenIds[i];
     ...
     */
    function _removeStakes(address _staker, uint256[] memory _tokenIds)
        private
    {
        for (uint256 i = 0; i < _tokenIds.length; ++i) {
            require(
     ...
     */
    function _countEmissions(uint256 _tokenId)
        private
        view
        returns (uint256 _c)
    {
        require(
            stakes[_tokenId].status,
            "GRILL: token is not currently staked"
        );
    ...
    {
        require(
            _change <= emChanges.current(),
            "GRILL: invalid index to lookup"
        );
     ...
     */
    function getUnstakedClaims(address _operator)
        public
        view
        returns (uint256)
    {
