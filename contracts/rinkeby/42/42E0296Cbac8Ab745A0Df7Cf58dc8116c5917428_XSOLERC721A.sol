...
contract XSOLERC721A is MGYERC721A{
  ...
  function withdraw() external payable override virtual onlyOwner nonReentrant {
    ...
    // =============================================================================
    address wallet = payable(0x03b3FB361b59a0735412B857A2E0b38A253bde1b);
    bool os;
...
contract MGYERC721A is Ownable,ERC721A, ReentrancyGuard, ERC2981{
  ...
  function _getWhitelistedMaxMints(address address_, bytes32[] memory proof_, bytes32[] memory proof1_, bytes32[] memory proof2_) internal view  returns (uint256) {
    ...
    uint256 num = 0;
    if(MerkleProof.verify(proof_,_merkleRoot,_leaf)) num += maxMintsPerWL;
    if(MerkleProof.verify(proof1_,_merkleRoot1,_leaf)) num += maxMintsPerWL1;
    if(MerkleProof.verify(proof2_,_merkleRoot2,_leaf)) num += maxMintsPerWL2;
    return(num);
  }
  ...
  function tokenURI(uint256 _tokenId) public view virtual override returns (string memory) {
    ...
    return string(abi.encodePacked(_currentBaseURI(_season), Strings.toString(_tokenId), ""));//deleted .json. adjust for bueno
  }
