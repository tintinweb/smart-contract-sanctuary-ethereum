...
contract Rpcp is ERC721A, PaymentSplitter, Ownable, ReentrancyGuard {
    ...
    **/
    ...
    function giftRaton(address _account) external onlyOwner {
        uint supply = totalSupply();
        require(supply + 1 <= MAX_SUPPLY_RATON, "Sold out");
        _safeMint(_account, supply + 1);
    }

    function giftCarcajou(address _account) external onlyOwner {
        uint supply = totalSupply();
        require(supply + 1 <= MAX_SUPPLY_CARCAJOU, "Sold out");
        _safeMint(_account, supply + 1);
    }

    function giftRenard(address _account) external onlyOwner {
        uint supply = totalSupply();
        require(supply + 1 <= MAX_SUPPLY_RENARD, "Sold out");
        _safeMint(_account, supply + 1);
    }

    function giftLoup(address _account) external onlyOwner {
        uint supply = totalSupply();
        require(supply + 1 <= MAX_SUPPLY_LOUP, "Sold out");
        _safeMint(_account, supply + 1);
    }

    function giftOurs(address _account) external onlyOwner {
        uint supply = totalSupply();
        require(supply + 1 <= MAX_SUPPLY_OURS, "Sold out");
        _safeMint(_account, supply + 1);
    }
