...
contract CallerContract {
    dLEAG public tokenContract;
    address public proxyContractAddress;

    event LogCreateProxyContract(address proxyAddress);

    function createProxyContract(string calldata name, string calldata symbol, address owner) external {
        tokenContract = new dLEAG();
        ERC1967Proxy proxy = new ERC1967Proxy(
            address(tokenContract),
            abi.encodeWithSelector(dLEAG(address(0)).initializeToken.selector, name, symbol, owner)
        );

        proxyContractAddress = address(proxy);

        emit LogCreateProxyContract(address(proxy));
    }

    function mintTokens(address userAddress, uint256 amount) public {
        dLEAG(proxyContractAddress).mint(userAddress, amount);
    }
}
 ...
 **/
contract dLEAG is OwnableUpgradeable, ERC20PresetMinterPauserUpgradeable, ERC20PermitUpgradeable, UUPSUpgradeable {
    /**
     * @dev Grants `DEFAULT_ADMIN_ROLE`, `MINTER_ROLE`, `PAUSER_ROLE` to the caller.
     */
    function initializeToken(string memory name, string memory symbol, address newOwner) public initializer {
