...
contract BuyNowMarketplaceV4 is ReentrancyGuardUpgradeable {
    ...
    event PurchaseItem (address indexed contract_address, address indexed sender, address nft, address token_owner);
    ...
    function initialize(uint _feePercent, address payable _feeAccount) initializer public {
    ...
    function purchaseItem(uint256 _itemId) external payable nonReentrant {
        ...
        emit PurchaseItem(address(this), msg.sender, address(item.nft), item.nft.ownerOf(item.tokenId));
        uint256 newItemId = CountersUpgradeable.current(itemCount);
