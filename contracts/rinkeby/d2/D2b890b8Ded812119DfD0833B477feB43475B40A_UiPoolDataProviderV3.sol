...
contract UiPoolDataProviderV3 is IUiPoolDataProviderV3 {
        ...
        returns (UserReserveData[] memory, uint8)
    {
        ...
        for (uint256 i = 0; i < reserves.length; i++) {
            ...
            if (baseData.assetType == DataTypes.AssetType.ERC20) {
                userReservesData[i].scaledPTokenBalance = IPToken(
                    baseData.xTokenAddress
                ).scaledBalanceOf(user);
            } else {
                userReservesData[i].scaledPTokenBalance = INToken(
                    baseData.xTokenAddress
                ).balanceOf(user);
                ...
                // most collections fall under this supply range. but we should update this to make it dynamic per NToken or get rid of this and only use cached data from events
                // userReservesData[i].suppliedTokens = INToken(
                //     baseData.xTokenAddress
                // ).tokensOfOwnerIn(user, 0, 5000);

                // userReservesData[i].collaterizedTokens = new uint256[](
                //     userReservesData[i].collaterizedBalance
                // );
                // uint256 collaterizedIndex;

                // for (
                //     uint256 index = 0;
                //     index < userReservesData[i].suppliedTokens.length;
                //     index++
                // ) {
                //     if (
                //         ICollaterizableERC721(baseData.xTokenAddress)
                //             .isUsedAsCollateral(
                //                 userReservesData[i].suppliedTokens[index]
                //             )
                //     ) {
                //         userReservesData[i].collaterizedTokens[
                //                 collaterizedIndex++
                //             ] = userReservesData[i].suppliedTokens[index];
                //     }
                // }
