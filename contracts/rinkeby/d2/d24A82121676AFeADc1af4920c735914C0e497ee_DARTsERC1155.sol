...
contract DARTsERC1155 is EIP712,ERC1155, DartsAccessControl, ERC1155URIStorage {
    ...
    constructor() ERC1155("") EIP712("DartsMeta", "1") {
    ...
    // ) external onlyRole(MINTER_ROLE) returns (bool) {
        ...
        __create(_msgSender(), owners, creator, sponsor, thanks, _tokenUri);
        return true;
    }
    ...
    function createMeta(
        ...
        bytes32 digest = _hashTypedDataV4(keccak256(abi.encode(
            keccak256("create(address owner,address[] memory owners, address creator, address sponsor,address[] memory thanks,string memory _tokenUri, uint256 nonce,uint256 deadline)"),
            owner,
            owners,
            creator,
            sponsor,
        ...
        require(signer == owner && signer != address(0), "Create: invalid signature");

        require(block.timestamp < deadline, "Create: signed transaction expired");
        nonces[owner]++;

        require(owners[0] == owner);

        __create(owner, owners, creator, sponsor, thanks, _tokenUri);
    }
    ...
    function __create(
        ...
        sponsors[0] = sponsor;
        _create(signer, owners, creators, sponsors, thanks, _tokenUri, newItemId);
    }

    function clone(uint formerId, uint _initSupply, uint _editType) external onlyOwner {
        require(balanceOf(_msgSender(), formerId) == 1);
        require(_msgSender() == Rights[formerId].owners[0]);
        require(Rights[formerId].iscloneable == true);

        uint initSupply = 1;
        bool iscloneable = true;
    ...
    // function setGroupAdmin(uint _tokenId, address _newAdmin, uint8 _adminType) external  onlyOwner onlyValidToken(_tokenId) {
    ...
    function safeTransferFrom(
        ...
        // if (Rights[id].iscloneable == true) {_grantRole(MINTER_ROLE, to);}
    }
    ...
    function setTokenRoyalty(
        ...
        _setTokenRoyalty(_tokenId, recipient, value, is_del_add, is_thanks);
    }
...
contract DartsAccessControl is Ownable, ERC2981TokenRoyalties{
    ...
    function _setERC1155construct() internal{
        uint32[] memory zero;
        Rights[0] = DataTypes.Right(0, 0,
            new address[](0), zero, new address[](0), zero,
            new address[](0), zero, new address[](0), zero,
            '', false, false, false, false, 0, DataTypes.royaltyTable(3000, 0, 6000, 0, 0, 0, 1000),
    ...
    function _create(
        ...
        string memory _tokenUri,
        uint newItemId
       // ) internal onlyRole(MINTER_ROLE) returns (bool) {
        ...
        Rights[newItemId] = DataTypes.Right(
            ...
            creators, UintArrayUtils.defaultSetRoyaltyForMember(1),
            sponsors, UintArrayUtils.defaultSetRoyaltyForMember(1),
            thanks, UintArrayUtils.defaultSetRoyaltyForMember(thanks.length),
            _tokenUri, true, false, false, false, 1, DataTypes.royaltyTable(3000, 0, 6000, 0, 0, 0, 1000),
            // DataTypes.RoyaltyVoting(address(0x0), address(0x0), 0, false),
            "");

        defaultRoyaltyTable(newItemId);

        emit updatedRoyalty(newItemId, signer);

        return true;
    }

    //todo isInheritable function needed 
    //todo isCommercialzed function needed
    //todo isInheritableChangable function needed
    //todo default or case is needed. now false is set as default
    function _clone(uint formerId, uint newItemId, string memory tokenURI) 
        internal onlyOwner {
       // internal onlyRole(MINTER_ROLE) {
        ...
        Rights[newItemId] = DataTypes.Right(
            ...
            new address[](0), zero,
            new address[](0), zero,
            tokenURI, cloneable, false, false, false, _order, RT,
            // DataTypes.RoyaltyVoting(address(0x0), address(0x0), 0, false),
            '');
        defaultRoyaltyTable(newItemId);
    ...
    // function _setGroupAdmin(uint _tokenId, address _newAdmin, uint8 _adminType) internal{

    //     require(_adminType < 3);
    ...
    //         }
    //         tempright.creators = tempright.creators.add(_newAdmin);
    ...
    //         }
    //         tempright.sponsors = tempright.sponsors.add(_newAdmin);
    //         _grantRole(ADD_MEMBER_ROLE, _newAdmin);
    //     }
    //     //  todo if thanks need to group admin; just open this
    //     //  else if (_adminType == 2) {
    //     //     if (tempright.thanks.length != 0) {
    //     //         _revokeRole(ADD_MEMBER_ROLE, tempright.thanks[0]);
    //     //     }
    //     //     tempright.thanks = tempright.thanks.add(_newAdmin);
    //     //     _grantRole(ADD_MEMBER_ROLE, _newAdmin);
    //     // }
    // }

    // function _propose(uint _tokenId, uint32 _newRoyalty) internal{
    ...
    function supportsInterface(bytes4 interfaceId)
        public view virtual override(ERC2981Base) returns (bool) {
        return super.supportsInterface(interfaceId);
    }

    function getItem(uint _tokenId) external view returns (DataTypes.Right memory) {
...
import "./lib/ERC1155/extensions/ERC1155Supply.sol";
import "@openzeppelin/contracts/utils/Strings.sol";
...
library DataTypes {
    ...
    struct Right {
        ...
        bool iscloneable;
        bool isInheritable;
        bool isCommercialzed;
        bool isInheritableChangable;      
        uint order;
    ...
    //Todo : 1500, 1000, ...  is the hardcoding need to config 
    function defaultRoyaltyTable(Right storage self) internal returns (royaltyTable memory) {
        ...
        // default royalty is 60% = 6000; 
        // creator 15% out of default royalty
        if (self.creators.length != 0) {
            creatorpercent = 900;
            self.creatorpercent = UintArrayUtils.defaultSetRoyaltyForMember(self.creators.length);
        }
        // sponsor 10% out of default royalty
        if (self.sponsors.length != 0) {
            sponsorpercent = 600;
            self.sponsorpercent = UintArrayUtils.defaultSetRoyaltyForMember(self.sponsors.length);
        }
        //thanks 10% out of default royalty
        if (self.thanks.length != 0) {
            thankpercent = 600;
            self.thankpercent = UintArrayUtils.defaultSetRoyaltyForMember(self.thanks.length);
        }
        // inherit royalty is half of default royalty
        if (self.order != 1) {royalty = 3000;}

        // the other is publishers(holder) share 
        uint32 holderspercent = 6000 - creatorpercent - sponsorpercent - thankpercent - royalty;
        self.ownerspercent = UintArrayUtils.defaultSetRoyaltyForMember(self.owners.length);

        // the other part is the originality
        saleProfit = 10000 - creatorpercent - sponsorpercent - thankpercent - holderspercent - royalty - adsFee;

        return self.royalty_table = royaltyTable(saleProfit, royalty, holderspercent, creatorpercent, sponsorpercent, thankpercent, adsFee);
    }

    function setRoyalty(Right storage self, address[] memory recipient, uint32[] memory value, bool is_del_add, bool is_thanks) internal {
        uint32 total = 0;
        ...
        if (msg.sender == self.owners[0]) {
            if (is_del_add == true) {
                if(is_thanks == false){self.owners = self.owners.concat(recipient);}
                else{self.thanks = self.thanks.concat(recipient);}
            }
            else {
                if(is_thanks == false){self.owners = self.owners.removeAddr(recipient);}
    ...
    function royaltyInfo(Right storage self, uint _salePrice) internal view returns (RoyaltyInfo memory) {
        ...
        if (self.owners.length == 1) {
            ...
            for (j = 0; j < self.owners.length; j++) {
                ...
                _shareflag[index] = status.Owners;
                _royaltyAmount[index] = self.ownerspercent[j] * self.royalty_table.holdershare * _salePrice / 100000000;
                index++;
            }
        }
        if (self.creators.length > 0) {
            for (j = 0; j < self.creators.length; j++) {
                ...
                _royaltyAmount[index] = self.creatorpercent[j] * self.royalty_table.creatorshare * _salePrice / 100000000;
                index++;
            }
        }
        if (self.sponsors.length > 0) {
            for (j = 0; j < self.sponsors.length; j++) {
                ...
                _royaltyAmount[index] = self.sponsorpercent[j] * self.royalty_table.sponsorshare * _salePrice / 100000000;
                index++;
            }
        }
        if (self.thanks.length > 0) {
            for (j = 0; j < self.thanks.length; j++) {
                ...
                _royaltyAmount[index] = self.thankpercent[j] * self.royalty_table.thanksshare * _salePrice / 100000000;
                index++;
            }
        }
...
abstract contract ERC2981TokenRoyalties is ERC2981Base {
    ...
    function setRoyaltyTable(uint _tokenId, uint32[6] memory _tabledata) external returns (DataTypes.royaltyTable memory) {
        require(10000 - Rights[_tokenId].royalty_table.Royalty ==
            _tabledata[0] + _tabledata[1] + _tabledata[2] + _tabledata[3] + _tabledata[4] + _tabledata[5], "Table data Input error");

        Rights[_tokenId].royalty_table = DataTypes.royaltyTable(_tabledata[0], Rights[_tokenId].royalty_table.Royalty,
            _tabledata[1], _tabledata[2], _tabledata[3], _tabledata[4], _tabledata[5]);
    ...
    function _setTokenRoyalty(
        ...
        Rights[tokenId].setRoyalty(recipient, value, is_del_add, is_thanks);
    }
