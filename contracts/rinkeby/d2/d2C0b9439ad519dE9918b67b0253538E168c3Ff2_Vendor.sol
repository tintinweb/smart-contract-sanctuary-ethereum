...
contract Vendor is Ownable {
  ...
  // ToDo: create a payable buyTokens() function that transfers tokens to the caller based on msg.value and tokensPerEth
  ...
  function sellTokens(uint256 amount) public {
    ...
    // 2. calculate the amount of ETH to send to the caller
    uint256 ethAmount = amount / tokensPerEth;
    // 3. send ETH to the caller
    payable(msg.sender).transfer(ethAmount);
    // 4. emit a SellTokens event
