    ...
    ILendFlareVault
{
        ...
        returns (uint256 share)
    {
        ...
        _pool.totalShare = _totalShare.add(_shares);
        _pool.totalUnderlying = _totalUnderlying.add(_amount);

        UserInfo storage _userInfo = userInfo[_pid][msg.sender];
        _userInfo.shares = _shares.add(_userInfo.shares);
    ...
    function withdraw(
        ...
        require(
            _withdrawable <= _userInfo.shares.sub(_userInfo.lendingLocked),
            "!_withdrawable"
        );

        _pool.totalShare = _totalShare.sub(_shares);
        _pool.totalUnderlying = _totalUnderlying.sub(_withdrawable);
        _userInfo.shares = _userInfo.shares.sub(_shares);
