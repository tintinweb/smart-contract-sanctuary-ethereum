    ...
    Ownable
{
     ...
     */
    constructor(address payable shareholderAddress_, address contractAddress) ERC721("Cool", "COOL") {
    ...
    function claimByTokenIds(uint256[] memory tokenIds, uint256 numberOfTokens)
        ...
        nonReentrant
    {
        uint256 numberOfTokensInBaseContract = tokenIds.length;
        uint256 mintPassesClaimed = Math.ceilDiv(numberOfTokens, 2);
        if (mintPassesClaimed > numberOfTokensInBaseContract) revert PurchaseWouldExceedClaimSupply();
        if (numberOfTokens * price != msg.value) revert EtherValueSentIsNotCorrect();
        uint256 ts = totalSupply();

        // set passes claimed
        for (uint256 index; index < mintPassesClaimed; index++) {
    ...
    function claim(uint256 numberOfTokens) external payable {
        uint256 numberOfTokensInBaseContract = baseContractAddress.balanceOf(msg.sender);
        uint256[] memory tokenIds = new uint256[](numberOfTokensInBaseContract);

        for (uint256 index; index < numberOfTokensInBaseContract; index++) {
            tokenIds[index] = baseContractAddress.tokenOfOwnerByIndex(msg.sender, index);
        }
     ...
     */
    function availableToClaim(address from) public view returns (uint256) {
     ...
     * @param from the address to check
     */
    function availableIdsToClaim(address from) external view returns (uint256[] memory) {
        uint256 numberOfTokens = availableToClaim(from);
        uint256[] memory availableIds = new uint256[](numberOfTokens);
        ...
        for (uint256 index; index < baseBalance; index++) {
            ...
            if (!isClaimed(tokenId)) {
                availableIds[claimableIndex] = tokenId;
                claimableIndex++;
            }
        }
