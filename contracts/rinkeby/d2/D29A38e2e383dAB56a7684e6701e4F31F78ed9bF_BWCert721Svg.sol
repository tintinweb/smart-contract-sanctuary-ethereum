...
import "./SVG/NFTSVG.sol";

 /// @title A modified ERC721 token for Bitwave Certification.
 /// @author Inish Crisson
 /// @author Jesse Farese
 /// @author Kunz Mainali
 /// @dev This contract is a modified version of the ERC721 token. It is used to store Bitwave Certificates.
contract BWCert721Svg is ERC721, Ownable {
    /// @notice Constructor, sets name, symbol.
    constructor(
        string memory _name,
        string memory _symbol,
        string memory _description,
        string memory _titleLineOne, 
        string memory _titleLineTwo,
        string memory _titleLineThree,
        string memory _colourOne,
        string memory _colourTwo
        ) ERC721(_name, _symbol) {
            localNftInfo.titleLineOne = _titleLineOne;
            localNftInfo.titleLineTwo = _titleLineTwo;
            localNftInfo.titleLineThree = _titleLineThree;
            localNftInfo.colourOne = _colourOne;
            localNftInfo.colourTwo = _colourTwo;
            localNftInfo.description = _description;
        }
    ...
    NFTSVG.nftInfo internal localNftInfo;

    /// @notice A mapping of tokenIds to NFT holder names.
    mapping(uint256 => string) public tokenHolderNames;
    ...
    function bulkMint(address[] memory to, string[] memory inputNames) public onlyOwner {
        for (uint i = 0; i < to.length; i++) {
            uint256 tokenId = currentTokenId();
            _mint(to[i], tokenId);
            tokenHolderNames[tokenId] = inputNames[i];
        }
    }
    ...
    function mint(address to, string memory tokenHolderName) public onlyOwner {
        uint256 tokenId = currentTokenId();
        _safeMint(to, tokenId);
        tokenHolderNames[tokenId] = tokenHolderName;
    }
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        string memory svg = NFTSVG.generateSVG(localNftInfo, tokenId, tokenHolderNames[tokenId]);
        return NFTSVG.constructURI(name(), localNftInfo.description, svg);
    }
}
