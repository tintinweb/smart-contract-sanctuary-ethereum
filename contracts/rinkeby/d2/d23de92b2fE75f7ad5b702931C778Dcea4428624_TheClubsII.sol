...
contract TheClubsII is ERC721A, Ownable, ReentrancyGuard {
    ...
    bool public bulkMintEnabled = true;
    ...
    mapping(address => uint256) public claimedFree;
    mapping(address => uint256) public claimedLegendary;
    mapping(address => uint256) public claimedWhitelist;

    constructor() payable ERC721A("The Club(s) II", "TCTII") {}
        ...
        nonReentrant
    {
        _checkPaused();
        _checkCanMint(_qty);
        if (totalSupply() + _qty < freeAmount + 1) {
            require(
                _qty < maxFreePerAddress + 1 ,
                "Cannot exceed max amount!"
            );
            require(
                claimedFree[msg.sender] < maxFreePerAddress,
            ...
            require(
                claimedLegendary[msg.sender] < maxPerAddress,
                "Max NFTs per address is claimed!"
            );
            require(msg.value >= legendaryPrice, "Insufficient funds!");
            claimedLegendary[msg.sender] += 1;
    ...
    function setMaxFreePerAddress(uint256 _val) public onlyOwner {
        maxFreePerAddress = _val;
    }
    ...
    function setFreeAmount(uint256 _val) public onlyOwner {
        freeAmount = _val;
    }
