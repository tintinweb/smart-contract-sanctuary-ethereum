...
contract PyeClaim is Ownable, ReentrancyGuard {
    ...
    uint256 private claimDateIndex = 0;
    uint256 private constant minIndex = 0;
    uint256 private constant maxIndex = 64;
    uint256 private claimDate; //=  1656072000; // claimStartDate + 1 minutes
    uint256 public claimStartDate; //constant claimStartDate =  1655985600;
    ...
    constructor() {
        startTime = block.timestamp; //_startTime; //1653393600 for 24 May 2022 @ 12:00:00 PM UTC
        PYE = 0xb5726580Db3db36b92DDAd287A9489275E3565Df;
        Apple = 0x81A2d6e7c2E566776EC26384f668Fc94260027e2;
        Cherry = 0xC7Fb1e8ED0C3026428D12B4511CC4342555F7d67;
        totalAllotments = (4 * 10**9) * 10**9; // 4 billion PYE tokens
        claimDate = block.timestamp + 1 minutes;
    ...
    function setClaimDateIndex(uint256 _claimDateIndex) external {
        claimDateIndex = _claimDateIndex;
    }
    ...
    function removePYE(uint256 _amountPYE) external onlyOwner {
        require(getElapsedTime() < 30 minutes || getElapsedTime() > 180 minutes , "Cannot withdraw PYE during the vesting period!");
    ...
    function removeApple(uint256 _amountApple) external onlyOwner {
        require(getElapsedTime() > 180 minutes , "Can only remove apple after vesting period!");
    ...
    function removeCherry(uint256 _amountCherry) external onlyOwner {
        require(getElapsedTime() > 180 minutes , "Can only remove cherry after vesting period!");
    ...
    function claimTokens() external nonReentrant notContract() {
        ...
        } else if (getElapsedTime() >= 12 minutes && getElapsedTime() < 14 minutes) {
            withdrawablePYE = (2 * tenPercent).sub(withdrawn);
            checkThenTransfer(withdrawablePYE, withdrawableApple, withdrawableCherry, available);

        } else if (getElapsedTime() >= 10 minutes && getElapsedTime() < 12 minutes) {
    ...
    function calculateWithdrawableAmounts(address _address) external view returns (uint256, uint256, uint256) {
        ...
        uint256 weightedAllotment = (original.mul(accuracyFactor)).div(totalAllotments);
        uint256 withdrawableApple = ((weightedAllotment.mul(dailyApple).div(1 * 10**18)).mul(claimDateIndex).div(accuracyFactor)).sub(Allotments[_address].claimedApple);
        uint256 withdrawableCherry = ((weightedAllotment.mul(dailyCherry).div(1 * 10**18)).mul(claimDateIndex).div(accuracyFactor)).sub(Allotments[_address].claimedCherry);

        uint256 withdrawablePYE;

        if (getElapsedTime() >= 22 minutes) {withdrawablePYE = available;
        } else if (getElapsedTime() >= 21 minutes && getElapsedTime() < 22 minutes) {withdrawablePYE = (9 * tenPercent).sub(withdrawn);
        } else if (getElapsedTime() >= 20 minutes && getElapsedTime() < 21 minutes) {withdrawablePYE = (8 * tenPercent).sub(withdrawn);
        } else if (getElapsedTime() >= 19 minutes && getElapsedTime() < 20 minutes) {withdrawablePYE = (7 * tenPercent).sub(withdrawn);
        } else if (getElapsedTime() >= 18 minutes && getElapsedTime() < 19 minutes) {withdrawablePYE = (6 * tenPercent).sub(withdrawn);
        } else if (getElapsedTime() >= 17 minutes && getElapsedTime() < 18 minutes) {withdrawablePYE = (5 * tenPercent).sub(withdrawn);
        } else if (getElapsedTime() >= 16 minutes && getElapsedTime() < 17 minutes) {withdrawablePYE = (4 * tenPercent).sub(withdrawn);
        } else if (getElapsedTime() >= 14 minutes && getElapsedTime() < 16 minutes) {withdrawablePYE = (3 * tenPercent).sub(withdrawn);
        } else if (getElapsedTime() >= 12 minutes && getElapsedTime() < 14 minutes) {withdrawablePYE = (2 * tenPercent).sub(withdrawn);
        } else if (getElapsedTime() >= 10 minutes && getElapsedTime() < 12 minutes) {withdrawablePYE = tenPercent.sub(withdrawn);
