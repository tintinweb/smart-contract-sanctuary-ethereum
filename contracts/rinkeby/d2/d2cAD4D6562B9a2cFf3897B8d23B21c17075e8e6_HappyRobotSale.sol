...
contract HappyRobotSale is Ownable {
    ...
    uint8 constant SALETYPES = 2;
    uint8 constant public MAX_NFT_PER_WALLET = 3;
    uint8 constant public MAX_WL_TOKEN_PER_WALLET = 3;

    uint8 constant MIDDLE_FLOOR = 8;
    uint8 constant FIRST_LEVEL_TOKINS = 30;
    uint8 constant SECOND_LEVEL_TOKINS = 50;
    uint16 constant public MAX_WHITELIST_TOKENS = 300;
    ...
    address[] private whitelistTokenMinters;
    address[] private giftWhitelist;
    mapping(address => uint32) neuronsMap;

    event MintedNFT(address _owner, uint16 _quantity, uint256 _totalNFT, uint256 _totalWhitelistToken);
    event MintedWhitelistToken(address _owner, uint16 _quantity, uint256 _totalWhitelistToken);

    /**
    * Require msg.sender to be the master or dev team
    */
    modifier onlyMaster() {
        require(isMaster(msg.sender), "HappyRobot#onlyMaster: ONLY_CREATOR_ALLOWED");
        _;
    }
    ...
    */
    function setWhitelistTokenURI(string memory _uri) public onlyMaster { 
        HappyRobotWhitelistToken(whitelistTokenAddress).setURI(_uri);
    }
    ...
    * @return 
    */
    function existInWhitelistTokenMinters(address _account) public view returns (bool) {
        for (uint256 i = 0; i < whitelistTokenMinters.length; i++) {
            if (whitelistTokenMinters[i] == _account)
                return true;
        }
        return false;
    }
    ...
    * @param _account address
    */
    function addToWhitelistTokenMinters(address _account) internal {        
        // if already registered, skip
        for (uint16 i = 0; i < whitelistTokenMinters.length; i++) {
            if (whitelistTokenMinters[i] == _account)   return;
        }

        // add address to the list
        whitelistTokenMinters.push(_account);
    }
    ...
    */
    function setWhitelistTokenMintFee(uint256 _wlTokenMintFee) public onlyMaster {
        wlTokenMintFee = _wlTokenMintFee;
    }

    /**
    * check if mint is possible for sale type
    * @param _saleType sale type
    * @param _quantity quantity
    * @return true or false
    */
    function canMintNFTForSaleType(uint8 _saleType, uint16 _quantity) public view returns (bool) {
        return HappyRobotNFT(nftAddress).canCreateToken(_saleType, _quantity);
    }
    ...
    * @return true or false
    */
    function canMintNFTForAccount(address _account, uint16 _quantity) public view returns (bool) {
        if (isMaster(_account)) return true;
        
        unchecked {
            return HappyRobotNFT(nftAddress).balanceOf(_account) + _quantity <= MAX_NFT_PER_WALLET;
        }
    }
    ...
    * @return true or false
    */
    function canMintWLTokenForAccount(address _account, uint16 _quantity) public view returns (bool) {
        if (isMaster(_account)) return true;
        
        unchecked {
            return HappyRobotWhitelistToken(whitelistTokenAddress).balanceOf(_account) + _quantity <= MAX_WL_TOKEN_PER_WALLET;
        }
    }

   /**
   * mint a new NFT 
   * @param _saleType token sale type index
   * @param _quantity token quantity
   * @param _data token data
   */
    function mint(uint8 _saleType, uint16 _quantity, bytes calldata _data) public payable {

        require(saleStauts != SALE_STATUS_NON, "Happy Robot: It is not the sale period");

        require(_saleType < SALETYPES , "Happy Robot: Invalid Generation Index");

        require(canMintNFTForSaleType(_saleType, _quantity) == true, "Happy Robot: Maximum NFT mint already reached for the generation");

        require(canMintNFTForAccount(msg.sender, _quantity) == true, "Happy Robot: Maximum NFT mint already reached for the account");
        ...
        bool isFreeMint = true;
        if (saleStauts == SALE_STATUS_PRE) { // presale
            if (!hasGift(msg.sender) && !isMaster(msg.sender)) {
                require(hasWhitelistToken || existInWhitelistTokenMinters(msg.sender) || existInWhitelist(msg.sender), "Happy Robot: You should be in the whitelist or you should have a whitelist token");

                if (hasWhitelistToken) {
                    require(whitelistToken.balanceOf(msg.sender) >= _quantity, "Happy Robot: Not enough whitelist token for NFT mint quantity");
                }
            }

            if (!hasGift(msg.sender) && !isMaster(msg.sender) && !hasWhitelistToken) {
                isFreeMint = false;
            }
        ...
        // perform mint
        if (isFreeMint) {
            nft.mint(msg.sender, _saleType, _quantity, _data);
        ...
        if (saleStauts == SALE_STATUS_PRE && !hasGift(msg.sender) && !isMaster(msg.sender) && hasWhitelistToken) {
            whitelistToken.burn(msg.sender, _quantity);
        }

        // trigger nft token minted event
        emit MintedNFT(msg.sender, _quantity, nft.totalSupply(), whitelistToken.totalSupply());
    }
    ...
    */
    function mintWhitelistToken(uint16 _quantity) public payable {
        ...
        require(token.totalSupply() + _quantity <= MAX_WHITELIST_TOKENS, "Happy Robot: Maximum whitelist token already reached");
        ...
        addToWhitelistTokenMinters(msg.sender);

        // trigger whitelist token minted event
        emit MintedWhitelistToken(msg.sender, _quantity, token.totalSupply());
    }

    /**
    * withdraw balance to only master wallet
    */
    function withdrawAll() public onlyMaster {
