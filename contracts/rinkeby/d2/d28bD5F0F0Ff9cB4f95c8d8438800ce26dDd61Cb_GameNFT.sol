...
contract GameNFT is ERC721A, Whitelist{
    ...
    function _setTokenURI(uint _tokenId, string memory _uri) public {
       ...
       require(ownerOf(_tokenId) == msg.sender, "Only token owner can set");
      _tokenURI[_tokenId] = _uri;
    }

    // function bulkMint(uint256 _amount) public payable onlyWhitelisted{
    ...
    //             _setTokenURI(idx, baseURI);
    //         }
    //     }
    // }

    function bulkMint(uint256 _amount) external payable onlyWhitelisted {
        require(saleActivated, "bulkMint: Mint process is pause");
        require(_amount > 0, "bulkMint: amount cannot be 0");
        require(totalSupply() < TOTAL_SUPPLY, "bulkMint: sale has already ended");
        require(totalSupply().add(_amount) <= TOTAL_SUPPLY, "bulkMint: sale has already ended");
        require(_amount <= MAX_BULK_TOKEN, "Mint limit reached");
        require(balanceOf(msg.sender) <= MAX_BULK_TOKEN, "Mint limit reached");
        require(msg.value == _amount.mul(NFT_MINT_PRICE), "bulkMint: Ether value sent is not correct");
