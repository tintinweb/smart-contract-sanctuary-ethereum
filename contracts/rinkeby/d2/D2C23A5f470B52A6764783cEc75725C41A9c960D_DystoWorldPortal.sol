...
contract DystoWorldPortal is ERC721AUpgradeable, OwnableUpgradeable {
    ...
    address public dyStakingAddress;
    ...
    event ExpirationTimestampUpdated(uint256 oldExpirationTimestamp, uint256 newExpirationTimestamp);
    event PaymentMethodUpdated(address oldPaymentMethod, address newPaymentMethod);
    event PriceUpdated(uint256 oldPrice, uint256 newPrice);
    event Purchased(address owner, uint256 firstTokenId, uint256 numTokens, uint256 indexed transactionId);
    ...
    function mint(
        ...
        bytes32 _messageHash = keccak256(
            abi.encodePacked(msg.sender, _issued, _expiration, _numTokens, _transactionId)
        );
        require(signerAddress == _messageHash.toEthSignedMessageHash().recover(signature), "Signer address mismatch.");
    ...
    function setStakingContractAddress(address newStakingAddress) external virtual onlyOwner {
        dyStakingAddress = newStakingAddress;
    }
    ...
    function transferFrom(
        ...
        if (from == dyStakingAddress) {
            isStaked[tokenId] = false;
        } else if (to == dyStakingAddress) {
            isStaked[tokenId] = true;
        } else {
            require(isStaked[tokenId] == false, "Token is staked.");
