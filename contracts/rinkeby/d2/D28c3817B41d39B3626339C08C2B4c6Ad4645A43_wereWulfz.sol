...
contract wereWulfz is ERC721A, Ownable, ReentrancyGuard {
    ...
    mapping (address => uint256) internal walletNumberMintedTierSale;
    ...
    function tier1Mint(bytes32[] calldata _merkleProof, uint256 _count) public nonReentrant isValidMerkleProofTier1(_merkleProof) payable {
        ...
        uint256 mintCount = walletNumberMintedTierSale[msg.sender].add(_count);
        ...
        walletNumberMintedTierSale[msg.sender] += _count;
    }

    function tier2Mint(bytes32[] calldata _merkleProof, uint256 _count) public nonReentrant isValidMerkleProofTier1(_merkleProof) payable {
        require(presaleActive, "Presale: not active");
        uint256 mintCount = walletNumberMintedTierSale[msg.sender].add(_count);
        require(_count > 0, "Cannot mint 0");
    ...
    function tier2Mints(bytes32[] calldata _merkleProof, uint256 _count) public nonReentrant isValidMerkleProofTier2(_merkleProof) {
        require(presaleActive, "Presale: not active");
        uint256 newCount = walletNumberMintedTierSale[msg.sender].add(_count);
        require(newCount > 0, "Invalid purchase amount");
        require(newCount <= maxMintTier2, "Purchase would exceed max number of mints allocated");
        require(totalSupply().add(_count) <= totalMaxSupply - reservedAmount, "Mintable: supply exceeded"); 
        walletNumberMintedTierSale[msg.sender] += _count;
   ...
   function publicMint(uint256 _count) public nonReentrant payable {
    ...
    function adjustMaxPerTier1(uint256 newCap) external onlyOwner {
        maxMintTier1 = newCap;
    }

    function adjustMaxPerTier2(uint256 newCap) external onlyOwner {
        maxMintTier2 = newCap;
    }
