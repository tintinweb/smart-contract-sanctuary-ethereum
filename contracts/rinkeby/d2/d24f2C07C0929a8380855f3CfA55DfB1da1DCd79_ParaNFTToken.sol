...
contract ParaNFTToken is Context, IERC20, Ownable {
    ...
    string private _name = "ParaNFTToken";
    string private _symbol = "ParaNFT";
    uint8 private _decimals = 9;
    
    uint256 public _taxFee = 2; // 2% Reflection fee
    uint256 private _previousTaxFee = _taxFee;

    // uint256 public marketingFeePercent = 38;
    uint256 public burnFeePercent = 37; // 5% buyback burn
    uint256 public devFeePercent = 63; // 3% development fee
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256, uint256, uint256) {
        ...
        uint256 tLiquidity = calculateExtraFee(tAmount);
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 burnFee = newBalance.mul(burnFeePercent).div(100);
        uint256 devFee = newBalance.mul(devFeePercent).div(100);
        // payable(marketingWallet).transfer(marketingshare);
        payable(burnWallet).transfer(burnFee);
        ...
        emit SwapAndLiquify(contractTokenBalance, newBalance, 0);
    }
