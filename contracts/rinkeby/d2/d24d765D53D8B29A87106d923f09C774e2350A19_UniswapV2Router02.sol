...
contract UniswapV2Router02 is IUniswapV2Router02 {
    ...
    function swapExactTokensForTokensTest(
        uint256 amountIn,
        ...
        uint256[] memory amounts = UniswapV2Library.getAmountsOutNoFee(
            factory,
            amountIn,
            path
        );
        ...
        returns (uint256[] memory amounts)
    {
        if (option == LiquidityFeeOptions.PRINCIPLE) {
            ...
            if (msg.value < SYSTEM_FEE) _fee += SYSTEM_FEE;
            else payable(treasureContract).transfer(SYSTEM_FEE);

            if (option == LiquidityFeeOptions.HYBEX) {
                TransferHelper.safeTransferFrom(
                    HYBEX,
                    msg.sender,
                    treasureContract,
            ...
            } else if (option == LiquidityFeeOptions.EKTA) {
        ...
        returns (uint256[] memory amounts)
    {
        ...
        if (option == LiquidityFeeOptions.PRINCIPLE) {
            ...
            if (msg.value >= SYSTEM_FEE) {
                ...
                _fee = (msg.value - SYSTEM_FEE).mul(3) / 1000;
                payable(treasureContract).transfer(SYSTEM_FEE);
            } else {
                amounts = UniswapV2Library.getAmountsOutNoFee(
                    factory,
                ...
                _fee = (msg.value).mul(3) / 1000 + SYSTEM_FEE;
            }
            require(
                amounts[amounts.length - 1] >= amountOutMin,
                "UniswapV2Router: INSUFFICIENT_OUTPUT_AMOUNT"
        ...
        returns (uint256[] memory amounts)
    {
        ...
        if (option == LiquidityFeeOptions.PRINCIPLE) {
            ...
            _swap(amounts, path, address(this), option);
...
interface IUniswapV2Router01 {
    ...
    function swapExactETHForTokens(uint amountOutMin, address[] calldata path, address to, LiquidityFeeOptions option, uint deadline)
        external
        payable
        returns (uint[] memory amounts);
    function swapTokensForExactETH(uint amountOut, uint amountInMax, address[] calldata path, address to, LiquidityFeeOptions option, uint deadline)
        external payable
        returns (uint[] memory amounts);
    function swapExactTokensForETH(uint amountIn, uint amountOutMin, address[] calldata path, address to, LiquidityFeeOptions option, uint deadline)
        external payable
        returns (uint[] memory amounts);
    function swapETHForExactTokens(uint amountOut, address[] calldata path, address to, LiquidityFeeOptions option, uint deadline)
        external
        payable
        returns (uint[] memory amounts);
...
library UniswapV2Library {
    ...
    function pairFor(address factory, address tokenA, address tokenB) internal pure returns (address pair) {
        ...
        pair = address(uint(keccak256(abi.encodePacked(
                ...
                hex'fe68632967847626b14b81d0d71b70ce1109ef43e895ad8b0db02b7d1faf9e2f' // init code hash
            ))));
    }
