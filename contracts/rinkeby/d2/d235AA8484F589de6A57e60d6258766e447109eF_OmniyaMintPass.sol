...
contract OmniyaMintPass is AbstractERC1155Factory {
    ...
    function mint(uint256 _amount)  public payable {
        require(totalSupply(0) + _amount <= maxSupply, "Exceeds max token supply!");
        require(saleIsActive, "Sale is not active!");
        require(!mintingClosed, "Minting is closed!");

        transferFunds(_amount);
        _mint(msg.sender, MINT_PASS_ID, _amount, "");
    } 
    
    /// @notice Allows owner to mint team tokens
    /// @param _to The address to send the minted tokens to
    /// @param _amount The amount of tokens to mint
    function mintTeamTokens(address _to, uint256 _amount) public onlyOwner {        
        require(totalSupply(0) + _amount <= maxSupply, "Exceeds max token supply!");
        require(!mintingClosed, "Minting is closed!");
        
         _mint(_to, MINT_PASS_ID, _amount, "");
    }      
    ...
    function burnFromRedeem(address _account, uint256 _amount) external {
        ...
        _burn(_account, MINT_PASS_ID, _amount);
    }  

    function finalizeRedeemContract(address _redeemContract) external onlyOwner {
