...
contract HunnysMerch is ERC1155Supply, Ownable {  
    ...
    function addNewProducts(string[] memory _uriString, uint256[] memory _prices, bool[] memory _redeemable, uint256[] memory _layerIds, bool[] memory _isLimited, bool[] memory _isHolderOnly, uint256[] memory _limits) public onlyOwner {
        uint256 currentId = products.length;
        for(uint256 i = 0; i < _uriString.length; i++){
            ProductData memory newProduct = ProductData(currentId + i, _uriString[i], _prices[i], _redeemable[i], _layerIds[i], _isLimited[i], _isHolderOnly[i], 1, _limits[i]);
            products.push(newProduct);
        }
    }

    // Change existing product by id 
    function changeProductById(uint256 _id, string memory _uriString, uint256 _price, bool _redeemable, uint256 _layerId, bool _isLimited, bool _isHolderOnly, uint256 _limit) public onlyOwner {
