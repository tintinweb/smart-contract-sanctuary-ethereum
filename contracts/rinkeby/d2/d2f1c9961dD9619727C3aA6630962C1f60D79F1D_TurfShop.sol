...
// WIP!!! Not finalized. April 5, 2022.

/*  TurfShop: It's like an ERC1155 store front that backs multiple sales and give-aways.
    ...
    There's a simple role system:
    Admins can manage the overall contract and shift funds around.
    "Merch" role holders can configure TurfObject releases. This allows for a full CMS to manage these objects down the road.
*/

contract TurfShop is ERC1155, ERC1155Supply, AccessControl {
  ...
  struct TurfObject {
    ...
    uint initialSupply; // How many items are availale to begin with?
    ...
    address eligibilityCheckerAddress;
    address fundsRecipientAddress;
    bool emergencyStopper;
    ...
    uint price; // The price of this item
  ...
  // Everyone else overrides this so I will too, but the OZ 1155 doesn't suggest this is necessary.
  function uri(uint256 _tokenId) public view virtual override returns (string memory) {
    return string(abi.encodePacked(ERC1155.uri(_tokenId), Strings.toString(_tokenId), '.json'));
  }

  // Sets up a new Turf Object, also minting its initial supply, with that stock assigned to this contract.
  // Emits an event with the new object's ID in our `turfObjects` storage array.
  function setupNewObject(TurfObject calldata turfObject) public onlyStaff returns (uint) {
    // While we allow a lot of configuration options there are very few actual rules. Just a few:
    ...
    return turfObjectCounter.current();
  }

  // Replaces the object at the given `objectId` with a new TurfObject consisting of the given arguments.
  // Admin only, not staff, to prevent needless meddling.
  // This won't re-mint the balance for the object.
  ...
  // The mint function. Requires the ID of the object being minted and the desired mint quantity.
  // Note that the quantity might be ignored depending on the logic of the given TurfObject itself.
  // For example, in the case of give-aways, the mintee will be transferred a quantity of objects
  // according to that mint's logic regardless of what's specified here.
  //
  function mint(uint objectId, uint amount, bytes32[] memory merkleProof) public payable checkReleaseStatus(objectId) {
    ...
    // Some sales can allow the user to picj the quantity.
    if(obj.allowAdjustableQty){
      ...
      // only the eligiblity. The use case for this is really just allow lists. maxAllowed is ignored because we set the maxAmountPerWallet on the turfOjject itself.

      if(obj.eligibilityCheckerAddress != address(0)){
        (bool isEligible, uint maxAllowed) = checker.check(msg.sender, merkleProof);
    ...
    uint mintedPerWallet = mintsPerWalletPerObjectId[objectId][msg.sender];
    ...
    // Probably only worry about exceeding limits in the case of a "normal" style mint.
    // When we're minting according to some fixed logic, like "mint 1 for every Turf Plot", we can just be sure to set the max supply to the appropriate amount.
    // But, better safe than sorry.
    
    // Make sure you haven't previously minted too much.
    require(mintedPerWallet + _amount <= obj.maxAmountPerWallet, "exceeded limit per wallet");

    require(_amount <= obj.maxAmountPerWallet, "exceeded limit per wallet");
    ...
    if(obj.maxAmountPerWallet > 0){
      mintsPerWalletPerObjectId[objectId][msg.sender] = _amount;
    }
  }

  // Fires off the funds for the given Turf Object, off to wherever they go.
  function transferFunds(uint objectId) public onlyStaff {
  ...
  // Reduces the totalSupply of the given object, used to adjust the balance after a mint closes and we have remaining stock.
  function burn(uint objectId, uint amount) public onlyAdmin {
    _burn(address(this), objectId, amount);
  }

  // Kind of an emegency option, if funds are stuck or something weird is happening with tracking individual tokens' funds.
  function withdrawAll(address addr) public onlyAdmin {
  ...
  // Need this so we can mint our 1155s to ourself initially.
  ...
  // In case any wayward tokens make their way over.
  function withdrawTokens(IERC20 token) external onlyAdmin {
      uint256 balance = token.balanceOf(address(this));
      token.transfer(msg.sender, balance);
  }

  // Allow us to receive arbitrary ETH if sent directly.
...
/*
  Each TurfShop mint can be configured to refer to an external contract for eligiblity checks.
  ...
  which would entail Checker contract get a user's balance from the original Turf contract.
  Or we might just have a snapshot of some arbitrary data, stored in a Merkle Tree.
  Maybe we want to interface with another community's contract, who knows what.
  Either way, we can develop that later, on a per-Turf Object basis.

  This contract only needs to expose a single method, defined below.
  That should return a bool (is the person eligible or not), and
  if they are eligible a number of items they're eligible to mint.
...
interface TurfShopEligibilityChecker {
  function check(address addr, bytes32[] memory merkleProof) external view returns (bool, uint);

  // If you're going to record the mint in the checker, you better add some logic to prevent it from being publicly available.
  // Lock it to the TurfShop contract's address in your constructor or something.
