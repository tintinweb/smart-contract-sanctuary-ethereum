    ...
    PausableUpgradeable
{
    ...
    /**
     * @notice Migrates contract storage for lending volume filled, upon implementation upgrade & sets a new reputation manager contract.
     * @param _reputationManager The new reputation manager contract address.
     * @param _lendingTokenToMigrate The asset to migrate lending volume for.
     * @param _lendingTokenToAdd The asset to add as a lending token.
     * @param _lenderAddresses Array of lender addresses to migrate filled volume for.
     */
    function upgradeStorage(
        address _reputationManager,
        address _lendingTokenToMigrate,
        address _lendingTokenToAdd,
        address[] memory _lenderAddresses
    ) public {
        require(version == 0, "Storage already upgraded!");
        version++;
        for (uint256 i = 0; i < _lenderAddresses.length; i++) {
            address lender = _lenderAddresses[i];
            lenderVolumeFilled[_lendingTokenToMigrate][
                lender
            ] = _lenderVolumeFilled[lender];
        }
        totalVolumeFilled[_lendingTokenToMigrate] = _totalVolumeFilled;
        reputationManager = IReputationManager(_reputationManager);
        lendingTokensSet.add(_lendingTokenToAdd);
    }
    ...
    function submitBid(
        ...
        require(
            marketRegistry.isVerifiedBorrower(_marketplaceId, _msgSender()),
            "Not verified borrower"
        );
        require(
        ...
        acceptedLoan(_bidId, "liquidateLoan")
    {
        ...
        bids[_bidId].state = BidState.LIQUIDATED;

        emit LoanLiquidated(_bidId, _msgSender());
    }
