...
contract Dao {
    ...
    struct Category {
        ...
        Voter[] voters;
        mapping(address => address) validVoters;
    ...
    function createCategory(string memory _description) public {
        uint256 token = 0;
        require(
        ...
        categories.push();
        Category storage newCategory = categories[idx];
        ...
        newCategory.validVoters[msg.sender] = msg.sender;
    ...
    function addValidVoter(address newVoter, uint256 idCategory) public {
        ...
        if (categories[idCategory].validVoters[newVoter] != newVoter) {
            if (newVoter != owner && !checkProposalEligibility(newVoter)) {
                categories[idCategory].validVoters[newVoter] = newVoter;
    ...
    function removeValidVoter(address oldVoter, uint256 idCategory) public {
        ...
        if (categories[idCategory].validVoters[oldVoter] == oldVoter) {
            if (oldVoter != owner && !checkProposalEligibility(oldVoter)) {
                delete categories[idCategory].validVoters[oldVoter];
                categories[idCategory].idVoter--;
    ...
    function createProposal(uint256 idCategory, string memory description)
        public
    {
        ...
        categories[idCategory].proposals.push(newProposal);
    }
}