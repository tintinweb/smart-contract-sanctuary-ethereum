...
contract Field is Ownable, IERC721Receiver, Pauseable {
    ...
    uint8 public constant MAX_ALPHA = 6;
    ...
    uint256 public DAILY_GOAL_RATE = 1000 ether;
   ...
   */
    function _alphaForGoalkeeper(uint256 tokenId) internal view returns (uint256) {
       // (, , , , , , , , uint8 alphaIndex) = game.tokenTraits(tokenId);
        uint256  alphaIndex = 5;
   ...
   */
    function randomGoalkeeperOwner(uint256 seed) external view returns (address) {
        ...
        for (uint i = MAX_ALPHA - 5; i <= MAX_ALPHA - 4 ; i++) {
...
contract CronosFootballClub is ICronosFootballClub, ERC721Enumerable, Ownable, Pauseable, PaymentSplitter {
    ...
    // mint price
    uint256 public MINT_PRICE = 400 ether;
    ...
    function mint(uint256 amount, bool stake) external payable nonReentrant whenNotPaused {
        ...
        if(isWhitelisted(_msgSender())){
            require(balanceSale[_msgSender()] + amount <= MAX_MINT_VIP, "Max limit of 3 mint per WL");
            MINT_PRICE = 200 ether;
        } 
   ...
   */
    function selectTraits(uint256 seed) internal view returns (PlayerGoalkeeper memory t) {
       ...
       //t.isPlayer = seed%10 !=0;
        ...
        seed >>= 16;
        if (!t.isPlayer) {
            t.captain = selectTrait(uint16(seed & 0xFFFF), 7 + shift);
