...
contract CP is Ownable, ERC721A, ERC721ABurnable {
    enum ContractStatus {
        Paused,
        Public,
        AllowList
    ...
    uint256 public immutable maxPerAddressDuringMint = 6;
    uint256 public immutable amountForDevs = 9;
    uint256 public immutable collectionSize = 15;
    ...
    bool public intuitivePoemsSet = false;
    bool public puzzlePoemsSet = false;
    ...
    function claimPuzzlePoem(uint256 tokenId, bytes memory signature)
        external
        allPoemsMinted
        tokenInBounds(tokenId)
        callerIsTokenOwner(tokenId)
    {
        ...
        address puzzleKey = address(uint160(uint256(bytes32(signature))));
        require(puzzleKey == poem.addr, "Incorrect answer");
    ...
    function listIntuitivePoems() external view returns (uint256[] memory) {
        require(intuitivePoemsSet, "Intuitive Poems have not been set yet");
    ...
    function listPuzzlePoems() external view returns (uint256[] memory) {
        require(puzzlePoemsSet, "Puzzles have not been set yet");
