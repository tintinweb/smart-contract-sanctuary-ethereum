...
contract FixedValueLock is BaseGovernanceWithUserUpgradable, ERC721EnumerableUpgradeable {
    ...
    */
    ...
    function withdrawUnallocated() external onlyBeneficiaryManager {
        
        bool isOwnerAsBen = false;
        if(schedule.ifOwnerAsBen()){
            isOwnerAsBen = true;
        }
        require(isOwnerAsBen, "ERROR: The funds could only be claimed by the beneficieries");
        require(totalLockedAmount - schedule.unlockedAmount(totalLockedAmount) > 0, "ERROR: undistributed amount should be greater than 0");

        uint256 undistributedAmount = (totalLockedAmount - schedule.unlockedAmount(totalLockedAmount));

        uint256 lockEnd = schedule.lockEnd();
        require(block.timestamp > lockEnd, "ERROR: Lock is not ended");
        assignedAmount = undistributedAmount;
        
        tokenERC20.safeTransfer(_msgSender(), undistributedAmount);
    }
    ...
    function debugDetails() public view returns(bool, bool, uint256, uint256, uint256){
        ...
        //uint256 unassignedAmount = totalLockedAmount - assignedAmount;
        uint256 unassignedAmount = isOwnerAsBen ? (totalLockedAmount - schedule.unlockedAmount(totalLockedAmount)) : (totalLockedAmount - assignedAmount);

        return (schedule.ifOwnerAsBen(), isOwnerAsBen, assignedAmount, unassignedAmount, totalLockedAmount);
    }
