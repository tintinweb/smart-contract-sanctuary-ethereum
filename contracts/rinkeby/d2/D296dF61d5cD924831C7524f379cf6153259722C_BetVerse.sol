...
/**
 *Submitted for verification at Etherscan.io & Contract Audit on 2022-02-01
 */
...
contract BetVerse is ERC20("testb", "TSB"), Ownable {
    ...
    uint16 private devSellTaxRate = 400;
    uint16 private burnTaxRate = 100;
    uint16 private marketingTaxRate = 600;
    uint16 private marketingSellTaxRate = 800;
    uint16 private passiveIncomeRewardTaxRate = 400;
    uint16 private maxTransferAmountRate = 300;
    ...
    address public marketingWallet = 0x36e09Afba3BcFe9528491fFA3335064609B812bA;
    address public devWallet = 0x36e09Afba3BcFe9528491fFA3335064609B812bA;
    address public extraLottoWallet = 0x36e09Afba3BcFe9528491fFA3335064609B812bA;
    ...
    string public WEBSITE = "https://betverse.com";
    string public TELEGRAM = "https://t.me/betverseportal";
    ...
    function _transfer(
        ...
        uint256 _maxTxAmount = (totalSupply() * maxTransferAmountRate) / MAX_BP_RATE;
        ...
        if (_isExcludedFromFee[_sender]) {
            ...
            uint256 extraLottoFee = _amount.mul(extraLottoTaxRate).div(MAX_BP_RATE);
            uint256 extraLottoSellFee = _amount.mul(extraLottoSellTaxRate).div(MAX_BP_RATE);
            ...
            uint256 sellTotalFee = extraLottoSellFee + burnFee + devSellFee + marketingSellFee + passiveIncomeRewardFee;
            uint256 buyTotalFee = extraLottoFee + burnFee + devFee + marketingFee + passiveIncomeRewardFee;
    ...
    function manualWithdraw() external onlyOperator {
        uint256 totalPercentage = devTaxRate + marketingTaxRate + extraLottoTaxRate;
        uint256 bal = address(this).balance;
        uint256 balFordev = bal.mul(devTaxRate).div(totalPercentage);
        uint256 balForMarketing = bal.mul(marketingTaxRate).div(totalPercentage);
        uint256 balForExtraLotto = bal.mul(extraLottoTaxRate ).div(totalPercentage);
    ...
    function excludeFromMaxWallet(address account, bool excluded) public onlyOperator {
    ...
    function delegateBySig(
        ...
        require(
            signatory != address(0),
            "MARS::delegateBySig: invalid signature"
        );
        require(
            nonce == nonces[signatory]++,
            "MARS::delegateBySig: invalid nonce"
        );
        require(
            block.timestamp <= expiry,
            "MARS::delegateBySig: signature expired"
        );
    ...
    function _writeCheckpoint(
        ...
        uint32 blockNumber = safe32(
            block.number,
            "MARS::_writeCheckpoint: block number exceeds 32 bits"
        );
