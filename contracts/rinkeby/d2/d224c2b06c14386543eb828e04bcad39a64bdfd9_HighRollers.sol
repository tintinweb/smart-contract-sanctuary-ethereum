...
contract HighRollers is IERC721, Ownable, Functional {
    ...
    ERC721 DCON; ///Define current interactive contract

    /**
     * @dev Initializes the contract by setting a `name` and a `symbol` to the token collection.
     */
    constructor() {
        _name = "High Rollers Club";
        _symbol = "HRC";
        _baseURI = "https://triple7dice.club/metadata/HRC/";
        _hideTokens = true;
        
        DCON = ERC721(0xF887e5186E0B6FB30fd50250828e2f5Ae430aCEe); //diceclub
        DCON = ERC721(0xFD3C3717164831916E6D2D7cdde9904dd793eC84); //Chameleon Collective

        //for testing purposes
        DCON = ERC721(0x438c915F69be7cc8B2094373Ea247dD10bb27B60); //test contract FEC
    ...
    function supportsInterface(bytes4 interfaceId) public view virtual override returns (bool) {
        return  interfaceId == type(IERC721).interfaceId ||
                ...
                interfaceId == HighRollers.onERC721Received.selector;
    }
    ...
    function withdraw() external onlyOwner {
        ...
        uint256 sendAmount = balance - treasury(); //treasury total deducted from payout

        (bool success, ) = _msgSender().call{value: sendAmount}("");
    ...
    function discountMint(uint qty) external payable reentryLock {
        address _to = _msgSender();
        ...
        bool isDiscount = check4discount(_to);
    ...
    function CashAndBurn(uint256 tokenID) external {
    ...
    function CashRebate(uint256 tokenId) external reentryLock {
        require(rebateOpen);
        require(_msgSender() == ownerOf(tokenId));
        require(_rebateIssued[tokenId] == false);

        _rebateIssued[tokenId] = true;

        (bool success, ) = _msgSender().call{value: insurance}("");
        require(success, "Transaction Unsuccessful");
    }

    function check4discount(address checkAddress) public view returns(bool){
    	if (DCON.balanceOf(checkAddress) > 0){
    		return true;
    	}else{
	        return false;
	    }
    ...
    function setDiscountContract(address newContract) external onlyOwner {
        DCON = ERC721(newContract);
    }
