    ...
    Ownable
{
    ...
    constructor() ERC721 ("Lockout - notAudited.eth", "LOCKOUT") {}
        ...
        returns (string memory)
    {
        ...
        parts[0] = '<svg xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="xMinYMin meet" viewBox="0 0 512 512"><rect width="100%" height="100%" fill="black" /><defs><linearGradient id="Lg" x1="0" x2="0" y1="0" y2="1"><stop offset="0" stop-color="white" stop-opacity="';
        parts[1] = '0.99"/><stop offset="0.99" stop-color="white" stop-opacity="0.0" /></linearGradient></defs><circle stroke="url(#Lg)" cx="256" cy="256" r="219" fill="none" stroke-width="25" stroke-dasharray="251,0" stroke-linecap="round"> </circle><text fill="white" font-size="150px" x="256" y="306" text-anchor="middle">';
        parts[2] = Strings.toString(tokenLifespan[tokenId]);
        parts[3] = '</text><text fill="white" font-size="25px" x="256" y="356" text-anchor="middle">';
        parts[4] = Strings.toString(dateTime.getMonth(tokenEndDate[tokenId]));
        parts[5] = '/';
        parts[6] = Strings.toString(dateTime.getDay(tokenEndDate[tokenId]));
        parts[7] ='/';
        parts[8] = Strings.toString(dateTime.getYear(tokenEndDate[tokenId]));
        parts[9] = '</text><text fill="white" font-size="12px" x="256" y="386" text-anchor="middle">';
        parts[10] = Strings.toString(dateTime.getHour(tokenEndDate[tokenId]));
        parts[11] = ':';
        parts[12] = Strings.toString(dateTime.getMinute(tokenEndDate[tokenId]));
        parts[13] = '</text></svg>';

        string memory output = string(abi.encodePacked(parts[0], parts[1], parts[2], parts[3], parts[4], parts[5], parts[6], parts[7],parts[8]));
        output = string(abi.encodePacked(output, parts[9], parts[10], parts[11], parts[12],parts[13]));
        
        string memory json = Base64.encode(bytes(string(abi.encodePacked('{"name": "Lockout Burnout #', Strings.toString(tokenId), '", "description": "LOCKOUT", "image": "data:image/svg+xml;base64,', Base64.encode(bytes(output)), '"}'))));
        output = string(abi.encodePacked('data:application/json;base64,', json));

        return output;
    }

    function authorBurnDeadToken(uint256 tokenId) external onlyOwner{
        require(
            block.timestamp + tokenLifespan[tokenId] * 2 days> tokenEndDate[tokenId],
            "Token not dead yet"
        );
        super._burn(tokenId);
    }
    ...
    function _beforeTokenTransfer(
        ...
        if(to != address(0)){
            require(
                block.timestamp < tokenEndDate[lastMintedId],
