...
contract BuyAndSellDecentraLandRealEstateV1 is BuyAndSellDecentraLandRealEstate {
    ...
    function createListing(
        ...
        require(royaltyPercentage <= 100, "Invalid royalties");
        ...
        require(
            ...
            existingSale.endTime < block.timestamp, //Listing expired
    ...
    function buyListing(uint256 listingId) external payable override {
        ...
        require(_listing.buyItNowPrice != 0, "Cannot instant buy listing");
        require(msg.value == _listing.buyItNowPrice, "Invalid input amount");
        require(
            _listing.publicDate <= block.timestamp,
            "Listing has not started"
        );
        require(_listing.endTime >= block.timestamp, "Listing is over");

        LAND.safeTransferFrom(_listing.owner, msg.sender, _listing.idFromNft);

        //takeFee and send money to seller
        uint256 buyFee = msg.value.mul(_listing.royaltyPercentage).div(100);
    ...
    function fullfillAuction(
        ...
        uint256 buyFee = price.mul(_listing.royaltyPercentage).div(100);
        bool sentA = WETH.transferFrom(
            buyer,
            _listing.owner,
            price.sub(buyFee)
        );
        bool sentR = WETH.transferFrom(buyer, _royaltyReceiver, buyFee);
    ...
    function updateListing(
        ...
        listingsMap[listingId].endTime = endTime;
    ...
    returns (uint256)
    {
        listingsMap[listingId].endTime = newEndDate;
        return listingsMap[listingId].endTime;
    }
    ...
    returns (uint256[] memory listingIds)
    {
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            ...
            if (
                ...
                // might compare sale seller and current owner
    ...
    function setLAND(address LANDAddress)
    external
    onlyRole(CONFIG_MANAGER)
    {
...
interface IBuyAndSellDecentraLandRealEstate {
    ...
    struct Listing {
        ...
        uint256 endTime; // date when the listing will be closed
        uint256 buyItNowPrice; // price of the buy-it-now, if 0, no buy-it-now
        uint256 royaltyPercentage; // the amount to be paid to the royaltyReceiver. This is set on a per-listing basis
     ...
     * approve on ERC721
     */
    //Cannot be don within the contract, only msg.sender can approve for owned token, has to be done outside of the contract in web3
