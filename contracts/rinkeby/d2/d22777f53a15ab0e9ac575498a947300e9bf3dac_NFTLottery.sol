    ...
    OwnableUpgradeable
{
    ...
    function createNFTLottery(
        ...
        lottery.initialize(this, treasuryFee, info);
    ...
    IERC721Receiver
{
    ...
    function initialize(
        ...
        require(
            _lottery.maxBuy == 0 || _lottery.maxBuy >= _lottery.minBuy,
            "Invalid buy limit"
        );
        __Ownable_init();
    ...
    function buyTickets(uint256 count) external payable {
        ...
        require(count >= lottery.minBuy, "Below minBuy");
        require(
            lottery.maxBuy == 0 || count < lottery.maxBuy + 1,
            "Exceed maxBuy"
        );
        require(
        ...
        require(msg.value == lottery.ticketPrice * count, "Only match value");
        require(
    ...
    function claimNFT(address winner, uint256 idx) external {
        require(lottery.status == Status.Claimable, "Not claimable");
        require(userTicketIdxs[winner].length > idx, "Only valid idx");
        uint256 ticketStartIdx = userTicketIdxs[winner][idx][0];
        uint256 ticketEndIdx = userTicketIdxs[winner][idx][1];
        require(
    ...
    function claimPayment() external {
        ...
        uint256 feeCost = balance.mul(fee).div(10000);
    ...
    function getUserTicketIdxs(address a)
        public
        view
        returns (uint256[][] memory)
    {
        return userTicketIdxs[a];
    }
