...
contract Staking is Ownable, IERC721Receiver {
    ...
    *@dev lastClaim is in epoch time.
    ...
    function setERC721A(address _contractAddress) external onlyOwner {
        require(_contractAddress.isContract(), "is not a contract");
        whiteListedContract = _contractAddress;
        AA = ERC721A(_contractAddress);
    }

    function setERC20(address _contractAddress) external onlyOwner {
        require(_contractAddress.isContract(), "is not a contract");
        bounty = BOUNTY(_contractAddress);
    }
    ...
    function _stake(uint _tokenId, address _from) private {
        ...
        StakingInfo storage copy = stakingInfo[_from];
        //copy to store data

        if (copy.stakedNftNum > 0) _claim(_from);
        //make sure to claim only if NFT were already staked
        //Update user staking informations
        stakingInfo[_from].lastClaim = block.timestamp;
        stakingInfo[_from].multiplier = 10 + (copy.stakedNftNum * 10 + (copy.stakedNftNum));
        // add 1.1 every time
        stakingInfo[_from].tokenIds[stakingInfo[_from].stakedNftNum] = _tokenId;
        // store the tokenId
        stakingInfo[_from].stakedNftNum = copy.stakedNftNum + 1;
        _ownerOf[_tokenId] = _from;
        // user is owner of staked token
    ...
    function unstake(uint _index) external {
        uint tokenId = stakingInfo[msg.sender].tokenIds[_index];
        require(_ownerOf[tokenId] == msg.sender, "not owner or staked");
        StakingInfo storage copy = stakingInfo[msg.sender];

        _claim(msg.sender);
        // claim gains and creates ERC20 tokens into the user's address
        //Update user staking informations
        (copy.stakedNftNum > 1) ? stakingInfo[msg.sender].multiplier = copy.multiplier - 11 : stakingInfo[msg.sender].multiplier = 0;

        stakingInfo[msg.sender].stakedNftNum = copy.stakedNftNum - 1;
        //Keeps track ofhow many NFT are in staking
        stakingInfo[msg.sender].tokenIds[_index] = stakingInfo[msg.sender].tokenIds[stakingInfo[msg.sender].stakedNftNum];
        // tokenId is replaced by last in array
        stakingInfo[msg.sender].tokenIds[stakingInfo[msg.sender].stakedNftNum] = 0;
        //last in array is deleted
        _ownerOf[tokenId] = address(0);
        // This contract doesn't retain an owner.
        AA.safeTransferFrom(address(this), msg.sender, tokenId);
        // Token is unlocked and sent back to owner
     ...
     * @dev the base rate is 100 $BOUNTY per day.
     */
