...
contract Challenger is Ownable {
    ...
    function submitDataMulti(
        ...
        for (uint256 i = 0; i < challengeData.length; i++) {
            if (!manager.hasUnlockedChallenge(challengeData[i].challengeId, challengeData[i].atheletAddress)) {
        ...
        for (uint256 i = 0; i < challengeData.length; i++) {
            if (challengeData[i].time.length != challengeData[i].conditions.length ||
            ...
            if (!manager.hasUnlockedChallenge(challengeData[i].challengeId, challengeData[i].atheletAddress)) {
                ...
                if (manager.multiSubmitAllowed(challengeData[i].challengeId))
                    continue;
            }

            manager.addLeaderboardEntry{value: keyPrice}(
                challengeData[i].challengeId,
                challengeData[i].atheletAddress,
                challengeData[i].conditions,
                challengeData[i].time,
                withUnlock
            );
        }
