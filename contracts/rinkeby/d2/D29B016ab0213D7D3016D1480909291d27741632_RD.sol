...
contract RD is BEP721A, Ownable {
    ...
    uint256 public totalPubsaleAmount = 0;
  ...
  /**
    * Token Price : Pre = Pub
    ...
    function onMint(bytes32[] calldata merkleProof, uint numberOfTokens) public payable {
      if((block.timestamp > presaleEndTime || block.timestamp < presaleStartTime) || totalSupply() >= MAX_PRESALE) {
            mintStep = 2;
            // presaleEndTime = 0;
        }
        ...
        if(mintStep == 1) {
          if( presaleEndTime > block.timestamp && block.timestamp > presaleStartTime && totalSupply() <= MAX_PRESALE) {
              ...
              require(numberOfTokens + totalSupply() <= MAX_PRESALE, "Have to mint less than remaining tokens.");
              require(tokenPrice * numberOfTokens <= msg.value, "Ether value sent is not correct");
              _safeMint(msg.sender, numberOfTokens);
              totalPresaleAmount += numberOfTokens;
        ...
        //   require(mintStep == 2, "Not Public Mint Step, Correct you mintStep");
          ...
          require(tokenPrice * numberOfTokens <= msg.value, "Ether value sent is not correct");
          _safeMint(msg.sender, numberOfTokens);
          totalPubsaleAmount += numberOfTokens;
