...
error ExceedKingSupply();
error ExceedQueenSupply();
error NotEnoughPaid();
error NonExistentToken();

contract CourageAndCloutAdvanced is ERC721AQueryable, Pausable, RaribleRoyaltyV2, Ownable {
  uint256 public constant MAX_SUPPLY = 1212;
  uint256 public constant KING_SUPPLY = 606;
  uint256 public constant QUEEN_SUPPLY = 606;

  uint256 public constant MINT_PRICE = 0.195632 ether; // ~ $212
  ...
  uint256 public kingMintCount = 0;
  mapping(uint256 => bool) public kingVariant;

  string private _baseTokenURI;
  
  constructor(string memory _baseUri) ERC721A("CourageAndClout", "CLOUT") {
  ...
  function mint(uint256 _kingAmount, uint256 _queenAmount) public payable whenNotPaused {
    uint256 queenMintCount = _totalMinted() - kingMintCount;
    uint256 _amount = _kingAmount + _queenAmount;
    uint256 _price = _amount * MINT_PRICE;

    if (_kingAmount <=0 || kingMintCount + _kingAmount > KING_SUPPLY) revert ExceedKingSupply();
    if (_queenAmount <=0 || queenMintCount + _queenAmount > QUEEN_SUPPLY) revert ExceedQueenSupply();
    if (msg.value < _price) revert NotEnoughPaid();

    uint256 startTokenId = _nextTokenId();
    for (uint256 id = startTokenId; id < startTokenId + _kingAmount; id ++) {
      kingVariant[id] = true;
    }

    kingMintCount += _kingAmount;
  ...
  public
  view
  override(IERC721A, ERC721A, RaribleRoyaltyV2)
  returns (bool)
  {
    ...
    return ERC721A.supportsInterface(interfaceId) || RaribleRoyaltyV2.supportsInterface(interfaceId);
  }

  function isKing(uint256 tokenId) public view returns (bool) {
    if (!_exists(tokenId)) revert NonExistentToken();

    return kingVariant[tokenId];
  }

  function remainingSupply() external view returns (uint256 _king, uint256 _queen) {
    _king = KING_SUPPLY - kingMintCount;
    _queen = QUEEN_SUPPLY - _totalMinted() + kingMintCount;
  }
