...
contract GrayBoys_Locker_Exchange is AccessControl, VRFConsumerBaseV2 {
        ...
        override
    {
        ...
        _processRandomnessFulfillment(requestId, randomness, to);
    }
    ...
    /// @dev Handle randomness fulfillment and executes the process of minting the items to the associated address
    function _processRandomnessFulfillment(
        uint256 requestId,
        uint256 randomness,
        address to
    ) internal {
        uint256 chance = randomness % 50;
        // Mint 1/1 of shard if chance or when we are at the end
        if (
            (chance < 8 && raresToSend > 0) || (lockersOpened - raresToSend < 1)
        ) {
    ...
    /**
     * Chainlink values for subscription
     */
...
interface IGrayBoys_Science_Lab is IERC1155 {
    function burnMaterialForOwnerAddress(
        uint256 _typeId,
        uint256 _quantity,
        address _materialOwnerAddress
    ) external;

    function mintMaterialToAddress(
        uint256 _typeId,
        uint256 _quantity,
        address _toAddress
    ) external;
