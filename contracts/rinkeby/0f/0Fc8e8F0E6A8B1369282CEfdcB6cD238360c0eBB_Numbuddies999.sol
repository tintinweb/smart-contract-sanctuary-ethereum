 ...
 * @dev VicMac
 */
contract Numbuddies999 is ERC721, Ownable, ReentrancyGuard {
    using Strings for uint256;
    using Counters for Counters.Counter;

    mapping(address => bool) public isAdmin;
    string public baseURI;
    string public DELETEDELETEDELETEDELETE;
    ...
    error NotValidENSName();

    function fakeMint(uint256 mints) external {
        for (uint256 index = 0; index < mints; index++) {
    ...
    function length3Pad(uint256 tokenNum)
        internal
        pure
        returns (string memory)
    {
        string memory rep = Strings.toString(tokenNum);
        if (tokenNum < 10) rep = string(abi.encodePacked("00", rep));
        else if (tokenNum < 100) rep = string(abi.encodePacked("0", rep));
        return rep;
    }

    function getRemainingFree() external view returns (uint256) {
        if (totalSupply.current() < 100) return 100 - totalSupply.current();
        else return 0;
    }
    ...
    /*
     * For clarity we specify the fixed price for each count range
     */
    function getPrice() public view returns (uint256) {
        if (totalSupply.current() < 100) return 0;
        else if (totalSupply.current() < 200) return 0.0111 ether;
        else if (totalSupply.current() < 300) return 0.0222 ether;
        else if (totalSupply.current() < 400) return 0.0333 ether;
        else if (totalSupply.current() < 500) return 0.0444 ether;
        else if (totalSupply.current() < 600) return 0.0555 ether;
        else if (totalSupply.current() < 800) return 0.0666 ether;
        else if (totalSupply.current() <= MAX_SUPPLY) return 0.0777 ether;
    }

    function ownerOfENS(uint256 tokenId) public view returns (address) {
        string memory name = length3Pad(tokenId);
        bytes32 label = keccak256(bytes(name));
        // Official ENS: Base Registrar Implementation
        IRegistrarController registrarController = IRegistrarController(
            0x283Af0B28c62C092C9727F1Ee09c02CA627EB7F5
        );
        if (!registrarController.valid(name)) revert NotValidENSName();
    ...
    // phase 1, 3 - 1) token #NNN buyable by NNN.eth 3) #NNN buyable by any NNN.eth
        ...
        callerIsUser
    {
        ...
        if (msg.value < getPrice()) revert InsufficientETHSent();
    ...
    // phase 2 - token #NNN buyable by NNN.eth
