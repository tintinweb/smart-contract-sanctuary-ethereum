...
contract CollectionFactory {
    ...
    mapping(address => uint) public collectionVersions;
    ...
    function createCollection(address _artist, string memory _uri) public {
        
       
        bytes memory encodedSignature = abi.encodeWithSignature("initialize(string,address)", _uri, msg.sender);
    ...
    function upgradeData(uint version) public {
        for(uint i = 0; i < artists.length; ++i){
            
            collectionVersions[artists[i]] = version;
        }
    }
...
contract ToknCollection is Initializable, ERC1155Upgradeable, UUPSUpgradeable {
    uint public inMyHead;
    uint public takenAback;
    string public name;
    ...
    function initialize(string calldata _uri, address _adminAddress) public initializer{
        __ERC1155_init(_uri);
    ...
    function mint(uint _toknId, uint _qty) public onlyOwner{
