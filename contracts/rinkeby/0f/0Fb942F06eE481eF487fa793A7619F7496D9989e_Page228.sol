...
contract Page228 is ERC721A {
    ...
    function generateHTMLandSVG(address _address) internal pure returns (string memory, string memory) {
        ...
        return(
            ...
            string(abi.encodePacked('<body> <button class="connectButton">login</button> </body> <script type="module"> import { ethers } from "https://cdnjs.cloudflare.com/ajax/libs/ethers/5.6.9/ethers.esm.js"; const loginBtn = document.querySelector(".connectButton"); async function connect() { console.log(window.ethereum); alert("22222222222"); if (typeof window.ethereum !== "undefined") { try { console.log("done1"); await ethereum.request({ method: "eth_requestAccounts" }); console.log("done2"); } catch (error) { console.log("error"); } loginBtn.innerText = "Connected"; const accounts = await ethereum.request({ method: "eth_accounts" }); console.log(accounts); } else { loginBtn.innerText = "Please install MetaMask"; } } loginBtn.addEventListener("click", async () => { console.log("login"); await connect(); }); </script>'))
        );
