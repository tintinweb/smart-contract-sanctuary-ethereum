...
contract NewEscrow is ERC165 {
        ...
        returns (address)
    {
        ...
        if (holder[_tokenId][_editionNumber] == address(0)) {
            (address creator) = ERC1155Interface.ownerOfToken(_tokenId);
            return creator;
        }
    ...
    function buyNowPayment(
        ...
        uint256 finalCut;
        uint256 creatorCut;
        address _creator = ERC1155Interface.ownerOfToken(_order.tokenId);
        ...
        if (!secondHand[_order.tokenId][_editionNumber]) {
            ...
            creatorCut = payAmount.sub(platformCut);
            ...
            creatorCut = payAmount.mul(10).div(100);
            sendValue(payable(_creator), creatorCut);
            finalCut = payAmount.sub(
                platformCut.add(creatorCut)
            );
