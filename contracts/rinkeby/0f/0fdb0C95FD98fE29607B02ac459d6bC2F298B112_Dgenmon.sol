...
contract Dgenmon is ERC721AQueryable, Ownable, Pausable, Presalable, ReentrancyGuard {
  ...
  function presaleMint(uint256 _amount, bytes32[] memory _merkleProof) public payable whenPresaled whenNotPaused {        
  ...
  function isProofed(bytes32[] memory _merkleProof) public view returns (bool) {
    bytes32 leaf = keccak256(abi.encodePacked(_msgSender()));
