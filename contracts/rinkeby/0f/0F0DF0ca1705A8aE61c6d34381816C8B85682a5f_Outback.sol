...
contract Outback is Ownable, ERC721A {
  ...
  uint128 public saleStartTimeLock;
  ...
  constructor(uint128 _saleStartTime) ERC721A("BackOut", "OBSH") {
    ...
    saleStartTimeLock = saleStartTime + 2 hours;
  }

  function mint(uint _amount) external  {
    require(addressClaimed[_msgSender()] + _amount <= 2, "Exceeds wallet mint amt.");
    require(_amount <= 2, "You cannot mint this many!");
    ...
    require(block.timestamp < saleStartTimeLock, "Sale has ended");
  ...
  function teamClaim(uint128 amount) external  onlyOwner{
    require(teamClaimed <= 30, "Team has already claimed!");
