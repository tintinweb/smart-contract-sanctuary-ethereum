    ...
    OwnableUpgradeable 
{
    ...
    event NewAnomura(uint indexed crabId, string claws, string legs, string body, string shell, string background, string headpieces);
    ...
    modifier onlyBowlContract(address _bowlOwner) {
        require(msg.sender == bowlAddress, "Can only be called by Bowl");
        _;

        entropyMix = keccak256(abi.encodePacked(_bowlOwner, block.coinbase));
    }
    ...
    function initialize(address _bowlAddress, address _anomuraDataAddress) external initializer {
        require(_bowlAddress != address(0x0),"Bowl contract address is 0");
        require(_anomuraDataAddress != address(0x0),"Anomura Data address is 0");
        __ERC721_init("AnomuraV1", "ANOMURA");
        __ERC721Enumerable_init();
        __ReentrancyGuard_init();
        __Ownable_init();

        baseURI = "https://anomura-staging.vercel.app/api/crabs/";
    ...
    /**
    @notice return anomura part as string output
    a. We generate a random number, then modulo 47 to get a greatness value.
    b. 
        If greatness > 45, ~2% chance then the output would have an unique part.
        If greatness > 40, ~15% chance then the output would have prefix, and suffix.
        If greatness > 35, ~25.5% chance then the output would have suffix only.
        If greatness > 30, ~34% chance then the output would have prefix only.
    ...
    */
    ...
    (
        ...
        bytes32[] memory sourceArray,
        bytes32[] memory prefixes,
        bytes32[] memory suffixes,
        bytes32[] memory unique
    ) 
        internal view returns (string memory) 
    {
        uint256 rand = random(string(abi.encodePacked(keyPrefix, StringsUpgradeable.toString(tokenId))));
        string memory output = string(abi.encodePacked(sourceArray[rand % sourceArray.length]));
        uint256 greatness = rand % 47;
        
        if (greatness > 45) {
            output = string(abi.encodePacked(unique[rand % unique.length], output));
            return output;
        }

        if (greatness > 40) {            
            output = string(abi.encodePacked(prefixes[rand % prefixes.length], " ", output, " ", suffixes[rand % suffixes.length]));
            return output;
        }
    ...
    */
    ...
    (
        ...
        string memory keyPrefix, 
        bytes32[] memory sourceArray,
        bytes32[] memory backgroundPrefixes
    ) 
    internal view returns (string memory) 
    {
        uint256 rand = random(string(abi.encodePacked(keyPrefix, StringsUpgradeable.toString(tokenId))));
        ...
        if (greatness > 45) {
            output = string(abi.encodePacked(backgroundPrefixes[rand % backgroundPrefixes.length], " ", output));
        }
    ...
    We generate each part of the anomura, and store in the mapping to be view later
    */
    function ownerMintAnomura(address _address) external nonReentrant onlyOwner 
    {
        require(address(anomuraData) != address(0x0), "Anomura data address is 0");
        ...
        string memory generatedClaws = pluck(newTokenId, "CLAW", data.claws, data.prefixes, data.suffixes, data.unique);
        string memory generatedLegs  = pluck(newTokenId, "LEG", data.legs, data.prefixes, data.suffixes, data.unique);
        string memory generatedBody = pluck(newTokenId, "BODY", data.body, data.prefixes, data.suffixes, data.unique);
        string memory generatedShell = pluck(newTokenId, "SHELL", data.shell, data.prefixes, data.suffixes, data.unique);
        string memory generatedHeadPieces = pluck(newTokenId, "HEADPIECE", data.headPieces, data.prefixes, data.suffixes, data.unique);
        string memory generatedBackground = pluckBackground(newTokenId, "BACKGROUND", data.background, data.backgroundPrefixes);

        anomuraMap[newTokenId] = 
            AnomuraEntity({
                body: generatedBody,
        ...
        emit NewAnomura(newTokenId, generatedClaws, generatedLegs, generatedBody, generatedShell, generatedBackground, generatedHeadPieces);
    }
    ...
    We generate each part of the anomura, and store in the mapping to be view later
    */
    // function mintAnomura(address _address) external nonReentrant onlyBowlContract(_address) 
    function mintAnomura(address _address) external nonReentrant
    {
        require(address(anomuraData) != address(0x0), "Anomura data address is 0");
    ...
    We generate each part of the anomura, and store in the mapping to be view later
    */
    function mintMultiple(address _address, uint256 _quantity) external nonReentrant onlyBowlContract(_address) {
    ...
    function tokensOfOwner(address _ownerAddr) external view returns (uint[] memory)
    {
        uint tokenCount = balanceOf(_ownerAddr); // number of token owned by this address
        uint[] memory tokenIds = new uint[] (tokenCount); // array should be same as number of token owned
...
interface IAnomuraData
{
    struct Data {
        ...
        bytes32[] suffixes;
        bytes32[] unique;
        bytes32[] backgroundPrefixes;
    }

    function getClaws() external view returns (bytes32[] memory);
    function getBodies() external view returns (bytes32[] memory);
    function getLegs() external view returns (bytes32[] memory);
    function getShells() external view returns (bytes32[] memory);
    function getHeadPieces() external view returns (bytes32[] memory);
    function getBackgrounds() external view returns (bytes32[] memory);
    function getPrefixes() external view returns (bytes32[] memory);
    function getSuffixes() external view returns (bytes32[] memory);
    function getUnique() external view returns (bytes32[] memory);
    function getBackgroundPrefixes() external view returns (bytes32[] memory);
    function getAnomuraData() external view returns(Data memory);
}

contract AnomuraData is IAnomuraData, Ownable 
{
    bytes32[] private clawData;
    bytes32[] private legData;
    bytes32[] private bodyData;
    bytes32[] private shellData;
    bytes32[] private backgroundData;
    bytes32[] private headpiecesData;
    bytes32[] private prefixesData;
    bytes32[] private suffixesData;
    bytes32[] private uniqueData;
    bytes32[] private backgroundPrefixesData;
    ...
    constructor()
    {
        ...
        clawData.push("Asteroid Lasergun");
        ...
        backgroundData.push("Asteroid Space");
    ...
    function getClaws() external view returns(bytes32[] memory claws){
        claws = clawData;
    }

    function getBodies() external view returns(bytes32[] memory bodies){
        bodies = bodyData;
    }

    function getShells() external view returns(bytes32[] memory shells){
        shells = shellData;
    }

    function getLegs() external view returns(bytes32[] memory legs){
        legs = legData;
    }

    function getHeadPieces() external view returns(bytes32[] memory headpieces){
        headpieces = headpiecesData;
    }

    function getBackgrounds() external view returns(bytes32[] memory backgrounds){
        backgrounds = backgroundData;
    }

    function getPrefixes() external view returns(bytes32[] memory prefixes){
        prefixes = prefixesData;
    }

    function getBackgroundPrefixes() external view returns(bytes32[] memory backgroundPrefixes){
        backgroundPrefixes = backgroundPrefixesData;
    }

    function getSuffixes() external view returns(bytes32[] memory suffixes){
        suffixes = suffixesData;
    }

    function getUnique() external view returns(bytes32[] memory unique){
        unique = uniqueData;
    }
