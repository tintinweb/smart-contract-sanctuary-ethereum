...
contract Ether is ERC721A, Ownable, ReentrancyGuard {
    ...
    mapping(string => mapping(uint => string)) public dataAddress;
    ...
    function getDataAddress(string memory ether_name, uint varib) public view returns(string memory) {
        return dataAddress[ether_name][varib];
    }
    ...
    function nameBurnAndChange(string calldata old_ether_name , uint256 tokenId , string calldata new_ether_name) external onlyOwner {
...
function walletOfOwnerMap(address _owner)
    ...
    returns (string[] memory)
  {
    ...
    while (ownedTokenIndex < ownerTokenCount) {
      ...
      if (currentTokenOwner == _owner) {
        ownedTokenName[currentTokenId] = tokenIDandAddress[currentTokenId];
        ownedTokenIndex++;
      }
