...
contract SneaksOfNature is ERC721AQueryable, ERC2981, Pausable, Ownable, ReentrancyGuard {
  ...
  function publicMint(
    ...
    // Check that user can mint the specified amount of tokens
    require((_totalMinted() + tokenAmount) <= MAX_TOKEN_SUPPLY, "Not enought tokens left to mint");
    require(balanceOf(receiver) + tokenAmount < maxTokensPerUser, "User is not allowed to mint that many tokens");
   ...
   */
  function setMaxTokensPerUser(uint16 newMaxTokensPerUser) external onlyOwner {
    maxTokensPerUser = newMaxTokensPerUser;
  }
