...
contract Renkon is ERC721A, Ownable, ReentrancyGuard {
  ...
  function checkProof(bytes32[] calldata proof, bytes32 leaf) view external returns (bool) {
    return MerkleProof.verify(proof, _verifier, leaf);
  }
  ...
  function isWhitelisted(bytes32[] calldata proof, bytes32 leaf) internal view returns (bool) {
    return MerkleProof.verify(proof, _verifier, leaf);
  }
