...
contract EvolutionPool is MonsterCollectible {
    ...
    function evolveMonster(uint256[] memory _monsterIds) public {       //[11,12,13,14,15]
        ...
        if (rarityLevel == 1 && _monsterIds.length == 10) {     //TODO: change back to 20!
    ...
    function burnMonsters(uint256[] memory _burnIds) internal {
        for (uint i = 0; i < _burnIds.length - 1; i++) {
            _burn(_burnIds[i]);
        }
        emit BurnedMonsterIds(msg.sender, _burnIds);
    }

    function mint_commons() public onlyOwner {
        uint i = 0;
        while (i < 10) {
    ...
    function mint_rares() public onlyOwner {
        uint i = 0;
        while (i < 5) {
