...
contract CrowdFunding {
    ...
    */
    ...
    event eventFunder(address addr, uint256 amount);

    function getFunders(uint256 cID,uint256 fID) public returns (address,uint256)  {
        Campaign storage c = campaigns[cID];
        Funder storage f = c.funders[fID];

        emit eventFunder(f.addr,f.amount);
