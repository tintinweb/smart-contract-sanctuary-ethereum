...
//Added public mint switch as an option to mint copies traditionally. Added recycle to burn 2 songs to mint 1.
//
//
//
contract ERC721J is
...
function initialize(
     ...
     */
    function __ERC721J_init(string memory name_, string memory symbol_, address owner, uint256 mintPrice_) internal onlyInitializing {
        __ERC721J_init_unchained(name_, symbol_, owner, mintPrice_);
    }

    function __ERC721J_init_unchained(string memory name_, string memory symbol_, address owner, uint256 mintPrice_) internal onlyInitializing {
        _name = name_;
        _symbol = symbol_;
        ...
        returns (string memory)
    {
        ...
        _tokenURI = _songURIs[(songId * 1000) + songGeneration];
        for (uint256 i; i <= songGeneration; i++) {
            if (bytes(_tokenURI).length == 0) {
                _tokenURI = _songURIs[
                    (songId * 1000) + songGeneration - i
                ];
            }
        }
    ...
    function _setSongURI2(
        ...
        _songURIs[(songId * 1000) + 1] = songURI1;
        _songURIs[(songId * 1000) + 2] = songURI2;
    }
    //sets the songURIs when minting a new song
    function _setSongURI(
        ...
        _songURIs[(songId * 1000) + 1] = songURI1;
        _songURIs[(songId * 1000) + 2] = songURI2;
        _songURIs[(songId * 1000) + 3] = songURI3;
    }
    ...
    function changeSongURI(
        ...
        if (!_exists(songId)) revert URISetOfNonexistentSong();
        _songURIs[(songId * 1000) + songGeneration] = songURI;
    }
    ...
    function changeSongURIs(
        ...
        if (!_exists(songId)) revert URISetOfNonexistentSong();
        _songURIs[(songId * 1000) + songGeneration1] = songURI1;
        _songURIs[(songId * 1000) + songGeneration2] = songURI2;
    }
        ...
        returns (string memory)
    {
        ...
        _songURI = _songURIs[(songId * 1000) + songGeneration];
        for (uint256 i; i <= songGeneration; i++) {
...
contract YourCollectible is Ownable {
    ...
    // true or false, true if its approved to spend
