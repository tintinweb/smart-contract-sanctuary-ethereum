...
contract Seapad is VRFV2WrapperConsumerBase, ChainlinkClient {
    ...
    //total the callback gas fees depending on how many items. (The callback fees per item are manually configured by the owner)
    ...
    //total the callback gas fees depending on how many items.
    ...
    ) VRFV2WrapperConsumerBase(linkTokenAddress, _wrapperAddress) {
        owner = msg.sender;
    ...
    function purchasePack(
        ...
        if (project.whitelisted) {
            ...
            require(project.whitelist[msg.sender] == true, "6"); //not whitelisted
        }
        (
            ,
            address paymentMethodAddress,
        ...
        if (msg.value == 0) {
            require(
                ...
                "7" // "Please allow contract to spend your tokens!"
            );
            ...
            require(paymentMethod == 1, "5"); //"Payment method not 1, but paid with ETH"
            //convert to weth
            // ERC20SWAP(WETHADDRESS).deposit();
            ERC20SWAP(WETHADDRESS).deposit{value: msg.value}();
            require(msg.value >= pricePerPackInChoosenPaymentMethodAfterSavedFeeRemoved, "8"); //"Not enough ETH to pay for pack"
            // require(msg.value >= )
        }
    ...
    function openPack(uint256 projectid, uint256 packSoldID_) public payable {
        ...
        require(msg.value >= ethToPay, "4"); //notEnoughETHProvided
        uniswapV2Router.swapExactETHForTokens{value: msg.value}(
            amountToPay,
            SeapadHelper.getPathForToken(address(LINK), WETHADDRESS),
            address(this),
            block.timestamp + 150
        );
        ...
        emit OpenPackRequest(pb.projectID, packSoldID_, msg.sender, requestid);
    }
