...
contract HatchMusic2 is ERC1155, Ownable, ReentrancyGuard {
  ...
  uint256 public featurePackPrice = 0.01 ether;
  ...
  mapping (uint256 => Track) public track;
  mapping (uint => bytes32) private roots; 
  mapping (uint256 => bool) public existingTrackId;
  mapping (uint256 => bool) public existingPackId;
  mapping (uint256 => bool) private featuredPackIds;
  ...
  function mintBatch(
    ...
    require(msg.value >= totalCost, "Wrong amount of ETH sent");
    for (uint256 i = 0; i < _trackIds.length; ++i) {      
      uint256 trackCost = track[_trackIds[i]].trackCost;      
      sendEth(track[_trackIds[i]].artistWallet, trackCost - (trackCost / 100 * hatchMintingFee));
    }
  ...
  function createTrackPack(
    ...
    // uint256 _rootId
  ) external onlyOwner maxAmmount(_trackNames.length) {
  ...
  function featurePack(
    ...
    require(existingPackId[_collectionId], "This collection doesn't exist");
    require(!existingPackId[_collectionId], "This collection is already featured");
    require(msg.value >= featurePackPrice, "Invalid ammount sent");
    featuredPackIds[_collectionId] = true;
    sendEth(hatchWallet, msg.value);
    emit NewFeaturedPack(_collectionId, msg.value);
  }
  ...
  function withdrawAll() public onlyOwner nonReentrant {      
    sendEth(hatchWallet, address(this).balance);
  }
}
