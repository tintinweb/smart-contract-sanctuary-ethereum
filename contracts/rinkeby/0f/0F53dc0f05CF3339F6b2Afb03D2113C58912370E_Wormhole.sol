...
contract Wormhole is IWormhole, ERC1155Holder, Ownable, Pausable, ReentrancyGuard {
  ...
  function buyTicketCard(uint256 amount) public override whenNotPaused nonReentrant {
    require(canBuyTicketCard(_msgSender(), amount), "Wormhole: cannot buy");
  ...
  function canBuyTicketCard(address user, uint256 amount) public view override returns (bool) {
    if (nftspacexTicketCard.isIdExpired(nftspacexTicketCard.currentIdSupported())) {
      return false;
    }

    return (
      nftspacexToken.balanceOf(user) >= amount * TICKET_CARD_PRICE * BASE &&
  ...
  function buyBox(uint256 optionId, uint256 amount) public override whenNotPaused nonReentrant {
    require(canBuyBox(_msgSender(), optionId, amount), "Wormhole: inssuficient fund");
  ...
  function canBuyBox(address user, uint256 optionId, uint256 amount) public view override returns (bool) {
    return (
      nftspacexToken.balanceOf(user) >= amount * BOX_PRICE[optionId] * BASE &&
...
interface IWormhole {
  ...
  function canBuyTicketCard(address user, uint256 amount) external view returns (bool);
  function canBuyBox(address user, uint256 optionId, uint256 amount) external view returns (bool);
