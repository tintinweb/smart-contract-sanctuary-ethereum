...
contract LITxToken is 
    ...
    uint256 private constant TOTAL_SUPPLY = 5_417_770_823;

    /*
     * @dev Burn the token on target chain
     */
    event BridgeGotOn(address sender, uint256 amount, uint256 chainId);
    /*
     * @dev Mint the token on target chain
     */
    event BridgeGotOff(address beneficiary, uint256 amount);
    /**
     ...
     */
    modifier onlyBridge() {
        require(bridge == _msgSender(), "LITxToken: caller is not the bridge");
        _;
    }
     ...
     */
    modifier canMigrate() {
        require(block.timestamp < migrateBy, "LITxToken: migration finished");
        _;
    }
    ...
    function initialize(
        ...
        uint256 migrateBy_) public initializer {
        __LITxToken_init(NAME, SYMBOL, bridge_, ecosystem_, migrateToken_, migrateBy_);
    }
    ...
    function __LITxToken_init(
        ...
        address ecosystem_,
        address migrateToken_,
        uint256 migrateBy_) internal onlyInitializing {
        __Context_init_unchained();
        __Ownable_init_unchained();
        __ReentrancyGuard_init_unchained();
        __ERC20_init_unchained(name_, symbol_);
        __ERC20Permit_init_unchained(name_);
        __LITxToken_init_unchained(bridge_, ecosystem_, migrateToken_, migrateBy_);
    }
    ...
    function __LITxToken_init_unchained(
        ...
        require(bridge_ != address(0) 
            && migrateToken_ != address(0), "LITxToken: bad input");
        bridge = bridge_;
    ...
    function setFeeDistributor(address feeDistributor_) external onlyOwner  {
        require(feeDistributor_ != address(0), "LITxToken: bad input");
     ...
     */
    function bridgeGetOn(uint256 amount, uint256 chainId) external  {
        super._burn(msg.sender, amount);
        emit BridgeGotOn(msg.sender, amount, chainId);
    }

    /**
     * @dev Get off the bridge.
     */
    function bridgeGetOff(address beneficiary, uint256 amount) external onlyBridge {
        super._mint(msg.sender, amount);
        emit BridgeGotOff(beneficiary, amount);
    }
    ...
    function migrate(address _beneficiary, uint256 _amount) external canMigrate nonBanned nonReentrant {
        require(_amount > 0, "LITxToken: bad input");
        migrateToken.safeTransferFrom(_msgSender(), address(0), _amount);
    ...
    function finalize() external {
        require(block.timestamp >= migrateBy, "LITxToken: too early");
...
abstract contract BannedUpgradeable is ContextUpgradeable {
    ...
    function __BannedUpgradeable_init() internal onlyInitializing {
        __Context_init_unchained();
    }
