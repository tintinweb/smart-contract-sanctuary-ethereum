...
contract Kash is ERC20, Ownable, BaseToken {
    ...
    bool public feeOnWalletTransfer;
    ...
    constructor (address router_) ERC20("KASH", "KASH") 
    {
        buyFee  = 8;
        sellFee = 8;
        liquidityShare  = 25;
        marketingShare  = 75;
        marketingWallet = 0x999619733a2df505229967Db54b84D6F4709DC45;
        feeOnWalletTransfer = false;
        maxTransactionLimitEnabled = true;
        maxTransactionRateBuy      = 10 * 10**9 * (10 ** 18) / 100; // 1% of total supply
        maxTransactionRateSell     = 10 * 10**9 * (10 ** 18) / 100; // 1% of total supply
        maxWalletLimitEnabled      = true;
        maxWalletLimitRate         = 10 * 10**9 * (10 ** 18) / 100; // 1% of total supply
        ...
        swapTokensAtAmount = 10 * 10**9 * (10 ** 18) / 5000; // 0.02% of total supply
        _mint(owner(), 10 * 10**9 * (10 ** 18));
    }
    ...
    function excludeFromFees(address account) external onlyOwner {
        require(_isExcludedFromFees[account] != true, "Account is already excluded");
    ...
    function updateFeeShares(uint256 _marketingFeeShare, uint256 _liquidityFeeShare) external onlyOwner {
        require(_marketingFeeShare + _liquidityFeeShare == 100, "Total fee shares must be equal to 100");
        marketingShare = _marketingFeeShare;
        liquidityShare = _liquidityFeeShare;
        emit FeeSharesUpdated(marketingShare, liquidityShare);
    }

    function enableFeeOnWalletTransfer(bool enable) external onlyOwner {
        require(feeOnWalletTransfer != enable, "Wallet to wallet transfer without fee is already set to that value");
        feeOnWalletTransfer = enable;
    }
    ...
    function _transfer(
        ...
        if(feeOnWalletTransfer && from != uniswapV2Pair && to != uniswapV2Pair) {
            takeFee = false;
        }
