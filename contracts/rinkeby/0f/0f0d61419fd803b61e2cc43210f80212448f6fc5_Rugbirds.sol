...
contract Rugbirds is ERC721, ERC721Enumerable, Ownable {
    ...
    // one mint for all 
    // consider adding seperate tracking for free mints
    function mint(uint8 numberOfTokens) external payable {
        ...
        if(_mintedFromAddress[msg.sender] == 0){
            ...
            require(numberOfTokens + _mintedFromAddress[msg.sender] <= FREE_MINT_AMOUNT, "You are only allowed one mint during the free phase!");
            require(ts + numberOfTokens <= MAX_FREE_SUPPLY, "Purchase would exceed max tokens");
...
*/            
    ...
    function mintForFreeOnce() internal {
        uint256 ts = totalSupply();
/*       require(isFreeMintActive, "Free mint is not active");
        ...
        require(FREE_TOKEN_PRICE * numberOfTokens <= msg.value, "This is free");
*/
        _mintedFromAddress[msg.sender] += 1;
