...
contract XillionModerators is Ownable{
     ...
     */
    function isUserModerator(address userAddress_) public view returns (bool) {
        for(uint i; i<moderators.length; i++){
          if(userAddress_ == moderators[i]){
              return true;
          }
        }
        return false;
...
contract XillionIDO is XillionIDOCommon {
     ...
     */
    function setWalletAddresses(XillionIDOStructs.WalletAddressV2 memory walletAddresses_) public onlyOwner  {
    ...
    function confirmIdo(uint256 targetVotes_, uint256 minVotingValue_, uint256 votingStartTimestamp_, uint256 votingEndTimestamp_) public {
        require(_state == IDOState.Unconfirmed);
        ...
        if(_state == IDOState.Unconfirmed){
            ...
            if(VotingState(votingState) == VotingState.Preparing){
                ...
                if(_state == IDOState.Claim || _state == IDOState.Refund) {
                    return _state;
                }else if(block.timestamp < allocationStartDate && VotingState(votingState) == VotingState.Successed){
                    return IDOState.Voting;
...
contract XillionVotingFactory is OwnablePausable, ReentrancyGuard {
    ...
    modifier onlyIDOFactory() { // Modifier
        require(
             msg.sender == address(idoFactoryAddress),
            "Only Ido Facotory"
        );
        _;
    } 
