...
contract NOVASTAKE is Proxy, Functional {
    ...
    function claim() public reentryLock {
    	...
    	NOVACOIN.transferFrom( address(this), _msgSender(), ((claimTime * dailyReward) / DAY) );
    ...
    function withdrawDAOcoins() public proxyAccess {
        // To recover unspent coins at the end of a staking period
    	NOVACOIN.transferFrom(address(this), _msgSender(), treasury);
    	treasury = NOVACOIN.balanceOf(address(this));    
    }

    function withdrawEth() public proxyAccess {
        //Handle residual funds from burning or other deposits
        uint256 sendAmount = address(this).balance;
        (bool success, ) = msg.sender.call{value: sendAmount}("");
        require(success, "Transaction Unsuccessful");
    }
    
    function burnDAOcoins() public proxyAccess {
        // To burn unspent coins at the end of a staking period, returns ETH to this contract
    	NOVACOIN.sellNova(treasury);
    	treasury = NOVACOIN.balanceOf(address(this));
    }
    ...
    function depositRewardFunds() public payable proxyAccess {
        uint256 payAmount = msg.value;

        //Send eth to the coin contract which will return $NOVA coins to this contract
        NOVACOIN.buyNova{value: payAmount}();
