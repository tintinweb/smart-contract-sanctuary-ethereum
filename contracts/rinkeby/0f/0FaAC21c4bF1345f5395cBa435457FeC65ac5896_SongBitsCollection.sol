...
contract SongBitsCollection is
    OwnableUpgradeable,
    PausableUpgradeable,
    ICollection
{
    ...
    modifier onlyArtist(uint256 _metadataId) {
        require(metadata[_metadataId].artist == msg.sender);
        _;
    }
    ...
    function setCost(uint256 _cost, uint256 _metadataId)
        public
        onlyArtist(_metadataId)
    {
        metadata[_metadataId].cost = _cost;
    }
    ...
    function preMint(
        ...
        emit PreMint(artist, _id, _duration, _cost, uri);
    }
    ...
    function calculatePartCost(uint256 _metadataId, uint256 _bits)
        public
        view
        returns (uint256 partCost)
    {
        Metadata memory _metadata = getMetadata(_metadataId);

        partCost = _bits * _metadata.cost;
    }
