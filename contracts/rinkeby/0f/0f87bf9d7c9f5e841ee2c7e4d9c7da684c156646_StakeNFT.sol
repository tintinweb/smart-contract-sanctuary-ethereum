    ...
    IStaking
{
     ...
     */
    function setAccPerShare(uint256 amount) public onlyOwner {
        accPerShare = amount;
    }
    ...
    function deposit(uint256[] calldata tokenIds) external {
        for (uint256 i; i < tokenIds.length; i++) {
            ...
            address owner = IERC721Upgradeable(ERC721_CONTRACT).ownerOf(
                tokenIds[i]
            );
            require(
    ...
    function withdraw(uint256[] calldata tokenIds) external {
        ...
        uint256 userShare = tokenIds.length.mul(accPerShare).div(10000);
        rewardDebt[msg.sender] -= userShare;
        MyToken(ERC20_CONTRACT).burn(msg.sender, userShare);
