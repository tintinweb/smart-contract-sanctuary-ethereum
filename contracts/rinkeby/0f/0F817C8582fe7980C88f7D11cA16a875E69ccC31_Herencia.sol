...
/**
 * @title Herencia
 * WhoIsSamot - an 1155 contract for  Herencia Argentina
 */
contract Herencia is ERC1155Supply, Ownable , ReentrancyGuard{
    ...
    function publicSale(uint256 _quantity,uint256 _id,address _receiverAddress) external payable {
        ...
        require(
                balanceOf(_receiverAddress,_id) + _quantity <= idStats[_id].maxPerWallet, 
        ...
        _mint(_receiverAddress,_id,_quantity,"");
    }


    function whitelistSale(uint256 _quantity,uint256 _id, bytes32[] calldata _merkleProof,address _receiverAddress) external payable {
        bytes32 leaf = keccak256(abi.encodePacked(_receiverAddress));        
    ...
    function ownerSale(uint256 _quantity,uint256 _id,address _receiverAddress) external payable {
        require(ownerSaleIsActive, "Owner sale is not active.");
        require(idStats[_id].saleIdActive, "Sale not available for this item now.");
        require(balanceOgRole(_receiverAddress) > 0 , "You are not an Og user");
    ...
    function withdraw() external onlyOwner nonReentrant {
        ...
        (bool bgc, ) = payable(0x547b4BF7f39FAE562d2d0d5CFc329B05ec3694F2).call{value: (80 * contractBalance) / 100}("");
