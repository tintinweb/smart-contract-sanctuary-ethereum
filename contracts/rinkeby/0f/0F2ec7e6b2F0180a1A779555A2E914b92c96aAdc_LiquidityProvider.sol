...
import './NonfungiblePositionManagerInterface.sol';

contract LiquidityProvider is IERC721Receiver {
    address public constant nonfungiblePositionManager_ = 0xC36442b4a4522E871399CD717aBDD847Ab11FE88;
    uint24 public constant poolFee = 3000;

    // INonfungiblePositionManager public immutable nonfungiblePositionManager;
    INonfungiblePositionManagerInterface nonfungiblePositionManager = INonfungiblePositionManagerInterface(nonfungiblePositionManager_);
    ...
    function _createDeposit(address owner, uint256 tokenId) internal {
        (, , address AddressToken0, address AddressToken1, , , , uint128 liquidity, , , , ) =
            nonfungiblePositionManager.positions(tokenId);
        // set the owner and data for position
        deposits[tokenId] = Deposit({owner: owner, liquidity: liquidity, token0: AddressToken0, token1: AddressToken1});
    }
    ...
    function mintNewPosition(address AddressToken0, uint256 amountToken0ToMint, address AddressToken1, uint256 amountToken1ToMint)
        external
        returns (
            uint256 tokenId,
            uint128 liquidity,
        ...
        )
    {
        ...
        TransferHelper.safeTransferFrom(AddressToken0, msg.sender, address(this), amountToken0ToMint);
        TransferHelper.safeTransferFrom(AddressToken1, msg.sender, address(this), amountToken1ToMint);

        // Approve the position manager
        TransferHelper.safeApprove(AddressToken0, address(nonfungiblePositionManager), amountToken0ToMint);
        TransferHelper.safeApprove(AddressToken1, address(nonfungiblePositionManager), amountToken1ToMint);
        ...
        INonfungiblePositionManager.MintParams memory params =
            INonfungiblePositionManager.MintParams({
                ...
                amount0Desired: amountToken0ToMint,
                amount1Desired: amountToken1ToMint,
                amount0Min: 0,
                amount1Min: 0,
        ...
        if (amount0 < amountToken0ToMint) {
            TransferHelper.safeApprove(AddressToken0, address(nonfungiblePositionManager), 0);
            uint256 refund0 = amountToken0ToMint - amount0;
            TransferHelper.safeTransfer(AddressToken1, msg.sender, refund0);
        }

        if (amount1 < amountToken1ToMint) {
            TransferHelper.safeApprove(AddressToken0, address(nonfungiblePositionManager), 0);
            uint256 refund1 = amountToken1ToMint - amount1;
            TransferHelper.safeTransfer(AddressToken1, msg.sender, refund1);
        }
    }
