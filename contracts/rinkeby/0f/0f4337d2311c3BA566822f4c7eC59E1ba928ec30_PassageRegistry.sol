...
contract PassageRegistry is Initializable, AccessControlUpgradeable, UUPSUpgradeable, ERC2771Recipient {
    ...
    mapping(address => LoyaltyData) public loyaltyLedgers;
    ...
    modifier onlyFromDeployedPassport() {
        require(passports[_msgSender()].managed, "ONLY_PASSPORT");
        _;
    }
    ...
    function createPassport(bytes memory data) external returns (address passportAddress) {
        ...
        passports[address(proxy)] = PassportData({managed: true});
    ...
    function upgradePassport(uint256 version, address passportAddress) external returns (uint256 newVersion) {
        ...
        require(passports[passportAddress].managed, "no longer managed");
    ...
    function createLoyalty(bytes memory data) external returns (address loyaltyAddress) {
        ...
        loyaltyLedgers[address(proxy)] = LoyaltyData({managed: true});
    ...
    function upgradeLoyalty(uint256 version, address loyaltyAddress) external returns (uint256 newVersion) {
        ...
        require(loyaltyLedgers[loyaltyAddress].managed, "no longer managed");
