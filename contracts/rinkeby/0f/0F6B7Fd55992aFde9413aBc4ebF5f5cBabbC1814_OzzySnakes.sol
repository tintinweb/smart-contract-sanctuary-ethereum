...
contract OzzySnakes is Ownable, ERC721A {
    enum SaleConfig {
        WHITELIST,
        PUBLIC
    }
    ...
    //  <-  WHITELIST MINT => PRICE = 0.01, MAX MINT = 3    ->   //

    uint256 private whiteListPrice = 0.01 ether;
    uint256 private maxWhiteListMint = 3;
    ...
    function whiteListMint(uint256 quantity) external payable callerIsUser {
        require(mintFreeze == false, "Mint Freezed");
        require(
            saleConfig == SaleConfig.WHITELIST,
            "WhiteList Mint ended or not started"
        );
        require(msg.value >= whiteListPrice, "Mint Price is more");
        require(
            totalSupply() + quantity <= maxSupply,
            "not enough remaining reserved for auction to support desired mint amount"
        );
        require(quantity <= maxWhiteListMint, "Can't mint more than max limit");
        _safeMint(msg.sender, quantity);
        returnExtra(quantity * whiteListPrice);
    }

    //    <-  PUBLIC MINT => PRICE = 0.015, MAX MINT = 1     ->  //

    uint256 private publicPrice = 0.015 ether;
    uint256 private maxPublicMint = 1;
    ...
    function publicMint(uint256 quantity) external payable callerIsUser {
        ...
        require(msg.value >= publicPrice, "Mint Price is more");
        require(
            totalSupply() + quantity <= maxSupply,
            "not enough remaining reserved for auction to support desired mint amount"
        );
        require(quantity <= maxPublicMint, "Can't mint more than max limit");
        _safeMint(msg.sender, quantity);
        returnExtra(quantity * publicPrice);
    }

    function returnExtra(uint256 price) private {
        require(msg.value > price, "No refunds");
        if (msg.value > price) {
