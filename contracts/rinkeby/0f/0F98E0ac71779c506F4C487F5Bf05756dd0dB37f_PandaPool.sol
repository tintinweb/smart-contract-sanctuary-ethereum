...
import "../interfaces/IPandaReborn.sol";
import "../interfaces/IPandaBuy.sol";
import "../interfaces/IPandaPoolData.sol";
import "../libraries/PandaMap.sol";
/**
 *  todo:是否支持一次购买多个？
 *  todo:改进721A协议，支持多数量NFT transfer
 *  todo:税收改为7.5%，可调整；一般税收打入国库
 *  todo:slot优化，buygas与reborngas精准计算
 *  todo：pandamap压测
 *  todo:可用性联调，并行联调
 */

contract PandaPool is Ownable, IERC721Receiver, IPandaPool {
    using SafeERC20 for IERC20;
    using PandaMap for mapping(uint256 => uint256);
    struct RequestInfo {
        uint256 reqFunc;
        address sender;
    ...
    bool public closeLiquidity ;
    mapping(uint256 => RequestInfo) reqInfos;
    mapping(uint256 => uint256) public poolNftMap;
    uint256 public poolSize = 2000;
    IPandaReborn public pandaReborn;
    IPandaBuy public pandaBuy;
    IPandaPoolData public pandaPoolData;
    ...
    uint256 constant buyGas = 1_243_315;
    uint256 fixedGasPrice = 30 gwei;
    uint256 constant rebornGas = 1_160_269;


    event PreReborn(address indexed recipient,uint256 ownerTokenId, uint256 fee);
    event PreBuy(address indexed recipient, uint256 payAmount);
    ...
    event Buy(address indexed recipient, uint256 targetTokenId, uint256 payAmount);
    ...
    event Test(address indexed thisAddress, address sender);
    ...
    function reset(address _reborn, address _buy, address _data)  external onlyOwner  {
        pandaReborn = IPandaReborn(_reborn);
        pandaBuy = IPandaBuy(_buy);
        pandaPoolData = IPandaPoolData(_data);
    }
    ...
    /**
     * @dev redeem NFT, you will receive $PANDA, dont use ReentrancyGuard,it will revert Chainlink callback.
     * @param _ownerTokenId The NFT Id you want to redeem
     * @param _amountRedeemMin  Less than this value you will not redeem
     * @param _deadline deadline
     */
    function redeem(uint256 _ownerTokenId, uint256 _amountRedeemMin, uint256 _deadline) external override ensure(_deadline) poolOpen {
        ...
        if (_pandaNFT.balanceOf(address(this)) >= NFT_TOTAL_SUPPLY - 100) {
            revert PoolNFTTooMany(_pandaNFT.balanceOf(address(this)), NFT_TOTAL_SUPPLY - 100);
        }
        uint256 _price = _currentPrice();
        if (_amountRedeemMin > _price * 9250 / 10000) {
            revert MinReceiveNotMatch(_amountRedeemMin, _price * 9250 / 10000);
        }
        _pandaNFT.safeTransferFrom(msg.sender, address(this), _ownerTokenId, "");
        
        IERC20(pandaToken).safeTransfer(msg.sender,  _price * 9250 / 10000);
        pandaPoolData._poolPushNft(_ownerTokenId);
        emit Redeem(msg.sender, _ownerTokenId, _price * 9250 / 10000);
    }


    /**
     * @dev  reborn NFT, dont use ReentrancyGuard,it will revert Chainlink callback.
     ...
     */
    function reborn(uint256 _ownerTokenId, uint256 _amountFeeMax, uint256 _deadline, uint8 _gasLevel) payable external override ensure(_deadline)  poolOpen {
        ...
        uint256 _price = _currentPrice();
        if (_amountFeeMax < _price * 750 / 10000) {
            revert MaxFeeNotMatch(_amountFeeMax, _price * 750 / 10000);
        }
        if (_pandaToken.balanceOf(msg.sender) < _price * 750 / 10000) {
            revert PandaNotEnough(msg.sender, _pandaToken.balanceOf(msg.sender), _price * 750 / 10000);
        }
        _pandaToken.safeTransferFrom(msg.sender, address(this), _price * 375 / 10000);
        _pandaToken.safeTransferFrom(msg.sender, owner(), _price * 375 / 10000);  
        
        pandaReborn.reborn(msg.sender, _ownerTokenId, _amountFeeMax, keyHash);        

        emit PreReborn(msg.sender, _ownerTokenId, _price * 750 / 10000);
    }


    /**
     * @dev buy NFT, you should pay $PANDA, dont use ReentrancyGuard,it will revert Chainlink callback.
     * @param _amountBuyMax  Above this value you will not make a purchase
     * @param _deadline deadline
     * @param _gasLevel gas level you want pay for
     */
    function buy(uint256 _amountBuyMax, uint256 _deadline, uint8 _gasLevel) payable external override ensure(_deadline)  poolOpen {
        bytes32 keyHash = keyHash200;
        uint256 gasPrice = fixedGasPrice;
        if (_gasLevel == GAS_LEVEL_200) {
            gasPrice = 200 gwei;
        ...
        IERC20 _pandaToken = IERC20(pandaToken);
        _pandaToken.safeTransferFrom(msg.sender,  owner(), _price * 375 / 10000);
        pandaBuy.buy(msg.sender, _amountBuyMax, _deadline, keyHash);
        emit PreBuy(msg.sender, _deadline);
    }

    /**
     * @dev internal reborn NFT, dont use ReentrancyGuard,it will revert Chainlink callback.
     * @param _ownerTokenId The NFT Id you want to reborn
     * @param _targetTokenId  random target NFT Id
     */
    function _onReborn(address sender, uint256 _ownerTokenId, uint256 _targetTokenId) external override onlyAdmin poolOpen {
        IERC721 _pandaNFT = IERC721(pandaNFT);
        if (_pandaNFT.ownerOf(_targetTokenId) != address(this)) {
            revert NFTOwnerNotMatch(_targetTokenId, address(this), _pandaNFT.ownerOf(_targetTokenId));
        }
        _pandaNFT.safeTransferFrom(address(this), sender, _targetTokenId, "");
        emit Reborn(sender, _ownerTokenId, _targetTokenId);
    }


    /**
     * @dev internal buy NFT, you will receive $PANDA, dont use ReentrancyGuard,it will revert Chainlink callback.
     * @param _targetTokenId The NFT Id you want to buy
     * @param _amountBuyMax  Above this value you will not make a purchase
     */
    function _onBuy(address buyer, uint256 _targetTokenId, uint256 _amountBuyMax) external override onlyAdmin poolOpen {
        IERC721 _pandaNFT = IERC721(pandaNFT);
        if (_pandaNFT.ownerOf(_targetTokenId) != address(this)) {
            revert NFTOwnerNotMatch(_targetTokenId,  address(this), _pandaNFT.ownerOf(_targetTokenId));
        }
        uint256 _price = _currentPrice();
        if (_amountBuyMax < _price * 10750 / 10000) {
            revert MaxBuyNotMatch(_amountBuyMax, _price * 10750 / 10000);
        }
        IERC20 _pandaToken = IERC20(pandaToken);
        
        if (_pandaToken.balanceOf(buyer) < _price * 750 / 10000) {
            revert PandaNotEnough(buyer, _pandaToken.balanceOf(buyer), _price * 10750 / 10000);
        }
        ...
        _pandaToken.safeTransferFrom(buyer, address(this), _price * 10375 / 10000);
        _pandaNFT.safeTransferFrom(address(this), buyer, _targetTokenId, "");
        emit Buy(buyer, _targetTokenId, _price * 10750 / 10000);
    }

    /**
     * @dev owner addLiquidity, dont use ReentrancyGuard,it will revert Chainlink callback.
     ...
     * @param _deadline deadline
     */
    function addLiquidity(uint256[] calldata _supplyTokens, uint256 _amountTokenMax, uint256 _updateMinPoolNftCount, uint256 _deadline) external ensure(_deadline) onlyOwner {
        ...
        for (uint256 i = 0; i < _length; i++) {
            ...
            tranTokens += _price;
            pandaPoolData._poolPushNft(_iTokenId);
        }
    ...
    /**
     * @dev owner removeLiquidity, dont use ReentrancyGuard,it will revert Chainlink callback.
     ...
     * @param _updateMinPoolNftCount update pool min nfts 
     */
    function removeLiquidity(uint256[] calldata _targetTokens, uint256[] calldata _targetPoolIndexs, uint256 _amountTokenMin, uint256 _updateMinPoolNftCount) external onlyOwner  {
        ...
        for (uint256 i = 0; i < _length; i++) {
            ...
            {
                if(pandaPoolData._getIdByIndex(_iPoolIndex) != _iTargetTokenId) {
                    revert PoolIndexNotMatchNFTId(_iPoolIndex, pandaPoolData._getIdByIndex(_iPoolIndex), _iTargetTokenId);
                }
                pandaPoolData._poolRemoveNft(_iPoolIndex);
            }
     ...
     * @param _close true or false
     */
    function setLiquidityClose(bool _close) external onlyOwner {
        closeLiquidity = _close;
    }
     ...
     */
    function _currentPrice() private view returns(uint256 _price) {
        if (IERC721(pandaNFT).balanceOf(address(this)) < NFT_TOTAL_SUPPLY) {
            _price = IERC20(pandaToken).balanceOf(address(this)) / (NFT_TOTAL_SUPPLY - IERC721(pandaNFT).balanceOf(address(this)));
        } else {
...
interface IPandaPool {
    function reborn(uint256 _ownerTokenId, uint256 _amountFeeMax, uint256 _deadline, uint8 _gasLevel) payable  external;
    function buy(uint256 _amountBuyMax, uint256 _deadline, uint8 _gasLevel) payable external;
    function redeem(uint256 _ownerTokenId, uint256 _amountRedeemMin, uint256 _deadline) external;
    function _onBuy(address buyer, uint256 _targetTokenId, uint256 _amountBuyMax) external;
    function _onReborn(address sender, uint256 _ownerTokenId, uint256 _targetTokenId) external;
...
interface IPandaReborn {
    function reborn(address sender,uint256 _ownerTokenId, uint256 _amountFeeMax, bytes32 _keyHash) payable external;
}
...
interface IPandaBuy {
        function buy(address sender, uint256 _amountBuyMax, uint256 _deadline, bytes32 _keyHash) payable external;
}
...
// SPDX-License-Identifier: MIT
pragma solidity ^0.8.6;
// a library for mapping with size, it's useful when there are too many items during initialization
library PandaMap {
    function remove(mapping(uint256 => uint256) storage _map, uint256 _idx, uint256 _size) public {
        require(_idx < _size, "Array Index Out Of Bounds");
        if (_idx == _size - 1) {
            _map[_idx] = 0;
        } else {
            _map[_idx] = _map[_size - 1];
           _map[_size - 1] = 0;
        }
    }

    function getValueByIndex(mapping(uint256 => uint256) storage _map, uint256 _idx, uint256 _size, uint256 _existValue) public returns(uint256) {
        require(_idx < _size, "Array Index Out Of Bounds");
        if(_map[_idx] == 0) {
            _map[_idx] = _existValue;
            return _existValue;
        } else {
            return _map[_idx];
        }
    ...
    function push(mapping(uint256 => uint256) storage _map, uint256 _value, uint256 _size) public {
        _map[_size] = _value;
    }
}
