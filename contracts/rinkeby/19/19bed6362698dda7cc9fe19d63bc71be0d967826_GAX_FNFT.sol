...
contract GAX_FNFT is ERC721Upgradeable {
    ...
    function transferToken(
     ...
     function callBackTransferToken(
        ...
        emit ArtbitTransferred(_from, _to, _assetId, _artBits, _royaltyArtbits, _transferArtbits, _royaltyPercentage, _transferType, _caller);
    }
    ...
    function updateDexPercentage(uint256 _nftAssetId, uint256 _dex_rp)
        external
        onlyAdmin
    {
        GAX_Artbits artbitInstance = getArtbitInstance(_nftAssetId);
        artbitInstance.updateDexPercentage(_dex_rp);
    }

    function updateP2PPercentage(uint256 _nftAssetId, uint256 _p2p_rp)
        external
        onlyAdmin
    {
        GAX_Artbits artbitInstance = getArtbitInstance(_nftAssetId);
        artbitInstance.updateP2PPercentage(_p2p_rp);
    }
        ...
        onlyAdmin
    {
        p2p_rp = _p2p_rp;
        dex_rp = _dex_rp;
        emit RoyaltyUpdated(p2p_rp, dex_rp);
    }
...
contract GAX_Artbits is Singleton, ERC20Upgradeable {
    ...
    function updateDexPercentage(uint256 percentage)
        public
        onlyNFT
        returns (bool)
    {
    ...
    function updateP2PPercentage(uint256 percentage)
        public
        onlyNFT
        returns (bool)
    {
    ...
    function transferArtbits(
        ...
        if (
            ...
            gaxInstance.callBackTransferToken(sender, recipient, nftId, amount, 0, transferAmount, 0, "whitelist", msg.sender);
        }
        ...
        // DEX transaction NFT_Asset owner receive royalty
            ...
            gaxInstance.callBackTransferToken(sender, recipient, nftId, amount, royalty_amt, transferAmount, drp , "DEX", msg.sender);
        } else {
           uint256 p2p = p2p_rp;  
           if(p2p_rp == 0) {p2p = gaxInstance.p2p_rp();}
            ...
            gaxInstance.callBackTransferToken(sender, recipient, nftId, amount, commission, transferAmount, p2p, "P2P", msg.sender);
        }
