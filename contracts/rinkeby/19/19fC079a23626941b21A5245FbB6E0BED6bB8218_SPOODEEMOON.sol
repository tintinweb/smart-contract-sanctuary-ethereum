...
contract SPOODEEMOON is ERC721A, Ownable {
    ...
    function stake(
        ...
        for (uint256 i; i < tokenIds.length; i++) {
            ...
            claimInfo[user1][adrToClaimAmount[user1]].nftIds.push(tokenIds[i]);
            this.safeTransferFrom(user1, address(this), tokenIds[i]);
            count++;
        }
        claimInfo[user1][adrToClaimAmount[user1]].nftAmount = count;
    }
    ...
    {
        return (
            claimInfo[user][claimNumber].nftIds,
            claimInfo[user][claimNumber].floor,
            claimInfo[user][claimNumber].percent,
            claimInfo[user][claimNumber].startTime,
            claimInfo[user][claimNumber].startTime +
                claimInfo[user][claimNumber].lockTime
        );
    }
        ...
        returns (uint256)
    {
        ...
        if (
            ...
            uint256 day = block.timestamp -
                claimInfo[user][numberClaim].startTime /
                1 days;
            uint256 hour = (block.timestamp -
                claimInfo[user][numberClaim].startTime -
                day *
                1 days) / 1 hours;
            uint256 minute = (block.timestamp -
                claimInfo[user][numberClaim].startTime -
                hour *
                1 hours) / 1 minutes;
            uint256 fullPercent = claimInfo[user][numberClaim].percent *
