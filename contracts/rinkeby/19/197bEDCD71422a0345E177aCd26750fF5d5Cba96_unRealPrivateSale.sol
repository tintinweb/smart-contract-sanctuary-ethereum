...
contract unRealPrivateSale is Crowdsale, FinalizableCrowdsale, AccessControlUpgradeable {
    ...
    // fee(%) in token sold fee to unReal owner
    uint256 public tokenSoldFee;

    // unReal owner account
    ...
    function initialize(
      ...
      address _unRealOwner,    // unReal owner account
  ...
  function updateCapPercentage(uint256 _capPercentage) external onlyOwner {
    ...
    function finalization() virtual internal override{
      ...
      // (%) of bnb raised to unReal owner
      uint256 fee = (bnbBalanceInContract * bnbRaisedFee) / 10**2;
      address payable _owner = payable(unRealOwner);   
      _owner.transfer(fee);

      // (%) of token sold to unReal owner
      require(token.balanceOf(address(this)) > 0, "Finalize: Insufficient token balance");
      uint256 tokensForunRealOwner = (tokensSold * tokenSoldFee) / 10 ** 4;
      token.transfer(unRealOwner, tokensForunRealOwner);

       //burn or refund depends upon refund type
      uint256 refundBalance = tokenBalanceInContract - tokensSold - tokensForunRealOwner;
      if(refundType == 1){
