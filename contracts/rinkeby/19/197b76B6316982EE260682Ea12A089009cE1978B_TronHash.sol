...
contract TronHash is ERC20, Ownable, ERC20Permit, ERC20FlashMint {


    event checkWin(Voter v,bytes32 blockHash);
    uint32 openBlockVoter_befer =  2;
    ...
    constructor() ERC20("tron hash", "THX") ERC20Permit("tron-hash") {}
    ...
    function openBox()public onlyOwner {
        ...
        for( uint i=0;i< voters.length;i++){
           ...
           bytes32  blockHash= blockhash(blockNum);
            if(v.blockNum+10<blockNum&& v.weit){
                 v.weit=false;
                isWin(v,blockHash);
            }
        }
    ...
    function isWin(Voter memory v,bytes32  blockHash) private returns(bool){
        emit checkWin(v,blockHash);
        return true;
    }

    receive() external payable { 
        Voter memory vv= Voter({
