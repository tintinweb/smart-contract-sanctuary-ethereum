...
contract OPAD_ICO2 {
    ...
    uint256 public priceRate = 80; // OPAD : USDC = 1 : 0.0125 = 80 : 1
    uint256 public MIN_ALLOC_USDC = 200 * 1e6; // USDC min allocation for each presale buyer
    uint256 public MAX_ALLOC_USDC = 20000 * 1e6; // USDC max allocation for each presale buyer
    uint256 public MIN_ALLOC_OPAD = priceRate * MIN_ALLOC_USDC * 1e12; // min OPAD allocation for each presale buyer
    uint256 public MAX_ALLOC_OPAD = priceRate * MAX_ALLOC_USDC * 1e12; // max OPAD allocation for each presale buyer
    ...
    address public USDC_Addr = 0x64560E3968f9c8AF2300dA48976641b7d30592e7; // USDC on rinkeby
    ...
    /**
     * @dev function that sets price rate (OPAD : USDC)
     */
    ...
    /**
     * @dev function that sets MIN_ALLOC_USDC
     */
    function setMIN_ALLOC_USDC(uint newMIN_ALLOC_USDC) external onlyOwner {
        MIN_ALLOC_USDC = newMIN_ALLOC_USDC;
    }

    /**
     * @dev function that sets MAX_ALLOC_USDC
     */
    function setMAX_ALLOC_USDC(uint newMAX_ALLOC_USDC) external onlyOwner {
        MAX_ALLOC_USDC = newMAX_ALLOC_USDC;
    }
    ...
    function deposit(uint256 USDC_amount) external payable whileOnGoing returns (uint256) {
        ...
        uint256 newOPADAmount = USDC_amount.div(1e6).mul(priceRate * 1e18);
        ...
        emit PresaleRegistered(msg.sender, USDC_amount, recipients[msg.sender].amountOPAD);
     ...
     */
    function addPreviousParticipants(address[] memory _oldRecipients, uint256[] memory _USDCtokenAmounts, uint256[] memory _OPADtokenAmounts) external onlyOwner {
        ...
        emit PrevParticipantsRegistered(_oldRecipients, _USDCtokenAmounts, _OPADtokenAmounts);
    }
}
