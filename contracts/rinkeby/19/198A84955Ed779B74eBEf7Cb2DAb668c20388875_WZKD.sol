...
contract WZKD is ERC721Base, PaymentSplitter, Ownable, AccessControl, Pausable, ReentrancyGuard, MerkleTree {
    ...
    function privateMint(uint amount, bytes32[] calldata _proof) 
        ...
        require(isAllowed(_proof, msg.sender) || 
                apeLiquidContract.balanceOf(msg.sender) > 0, 
            "not allowed");
    ...
    address constant private APE_LIQUID_WALLET = 0x0B0237aD59e1BbCb611fdf0c9Fa07350C3f41e87;

    function freeMint(uint amount) external {
        require(apeLiquid + amount <= APE_LIQUID_MAX, "mint quota reached");
        require(msg.sender == APE_LIQUID_WALLET, "not allowed");
        apeLiquid += amount;
        mint(msg.sender, amount);
    }
...
contract ERC721Base is ERC721A {
    ...
    // Mint
    function mint(address wallet, uint256 amount) internal {
        uint256 supply = totalSupply();
        require(amount > 0, "amount too little");
        require(msg.sender != address(0), "empty address");
        require(supply + amount <= _maxSupply, "exceed max supply");

        _safeMint(wallet, amount);
    }

    function _setMaxSupply(uint amount) internal {
        _maxSupply = amount;
    }
