    ...
    UUPSUpgradeable
{
    ...
    {
        require(
            newSupply > 0 && newSupply > _totalMinted(),
            "ERC721A: set from low max supply"
        );
        maxSupply = newSupply;
    }
    ...
    {
        require(quantity <= mintMax, "ERC721A: mint limit reached");
        require(
            msg.value >= quantity * mintPrice,
            "ERC721A: insufficient ETH amount"
        );
        // if (whitelisted) {
        //     require(
        //         _numberMinted(to) + quantity <= holdMax,
    ...
    function _requireNonZero(uint256 value) internal view virtual {
        require(value > 0, "ERC721A: value cannot be zero");
    }

    function _requireURINonEmpty(string memory uri) internal view virtual {
        require(bytes(uri).length > 0, "ERC721A: set from invalid URI");
    }
}
