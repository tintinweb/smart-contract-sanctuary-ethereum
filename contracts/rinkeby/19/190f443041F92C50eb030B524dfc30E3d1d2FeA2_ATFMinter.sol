...
// sssAddress = 0xa7eE407497b2Aeb43580cabe2B04026b5419D1dc
// artAddress = tbd
// repAddress = tbd
...
contract ATFMinter is Ownable {
    ...
    constructor (address sssAddress, address artifactsAddress, address repusAddress) {
        sssContract = SSSInterface(sssAddress);
        artifactsContract = ArtifactsInterface(artifactsAddress);
        repusContract = RepusInterface(repusAddress);
    }
    ...
    function mintRepus(uint256 sssId) external {
        require(saleIsActive, 'sale not live');
        require(!sssClaimed[sssId], 'token is claimed');
        require(sssContract.ownerOf(sssId) == msg.sender, "not the owner of token");
        claimedSupply++;
        sssClaimed[sssId] = true;
        repusContract.directMint(msg.sender, 1);
    }
    ...
    function claimMulti(uint256[] calldata sssIds) external {
        ...
        for (uint i = 0; i < amount; i++) {
            require(!sssClaimed[i], 'token claimed already');
            require(sssContract.ownerOf(i) == msg.sender, "not the owner of this token");
            sssClaimed[i] = true;
        }
    ...
    function claimLeftovers(address to, uint256 amount) external onlyOwnerOrManager {
