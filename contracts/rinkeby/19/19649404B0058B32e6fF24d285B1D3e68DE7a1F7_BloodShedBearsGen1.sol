...
interface IGameStats {
    function pickWinnerFromElites(bool faction) external returns (address);
}
    ...
    function mint(uint256 mintAmount_) external payable whenNotPaused returns (uint256) {
        ...
        for (uint256 i = 0; i < mintAmount_; i++) {
            ...
            if(_isLost(totalSupply + i)) {
                address winner = IGameStats(gameStatsContract).pickWinnerFromElites(tokenFactions[totalSupply + i]);
                if(winner != address(0)) {
                    ++lostBears;
                    recipient = winner;
                }
