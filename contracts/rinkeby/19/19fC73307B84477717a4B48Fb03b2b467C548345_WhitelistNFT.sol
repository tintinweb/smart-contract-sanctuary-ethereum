...
contract WhitelistNFT is ERC721Enumerable, Ownable {
    ...
    constructor(string memory baseURI) ERC721("WhitelistNFT", "WLN") {
        setBaseURI(baseURI);
    }
    ...
    function setMaxPerMint(uint256 _perMint) public onlyOwner {
        MAX_PER_MINT = _perMint;
    }
    ...
    function setMerkelRoot(bytes32 _merkleRoot) public onlyOwner{
        merkleRoot = _merkleRoot;
    }
    
    function mintNFTs(bytes32[] memory _merkleProof, uint256 _count) public payable {
        uint256 totalMinted = _tokenIds.current();
