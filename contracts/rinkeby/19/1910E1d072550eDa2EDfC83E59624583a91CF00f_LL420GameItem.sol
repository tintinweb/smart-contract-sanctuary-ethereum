...
contract LL420GameItem is ERC1155, ReentrancyGuard, Ownable, Pausable {
    ...
    uint256 public constant ROLL_PAPER_ID = 0;
    uint256 public constant HOODIE_ID = 1;
    uint256 public constant SERUM_ID = 2;
    uint256 public constant FARMER_PASS_ID = 3;
    uint256 public constant DR_PASS_ID = 4;
    uint256 public constant ITEM_TYPES = 5;
    uint256 public constant MAX_PER_TX = 3;

    uint32[5] public supplyCaps = [420, 200, 350, 2700, 1000];
    uint32[5] public prices = [12600, 42000, 70000, 35000, 42500];
     ...
     * @param _wallet Address of wallet contract
     * @param _account Validator account
     */
    constructor(
     ...
     *
     * @param _id Game item id 0, 1, 4
     * @param _amount Amount to mint
     */
    function buyA(uint256 _id, uint256 _amount) external nonReentrant whenNotPaused {
        if (_id != ROLL_PAPER_ID && _id != HOODIE_ID && _id != DR_PASS_ID) revert InvalidId();
        ...
        if (_id == ROLL_PAPER_ID) {
            ...
            if (burnt == 0) revert NotEnoughBurnt();
        }

        ILL420Wallet wallet = ILL420Wallet(walletAddress);
        uint256 totalPrice = prices[_id] * _amount;
        if (wallet.balance(_msgSender()) < totalPrice) revert NotEnoughBalance();
        wallet.withdraw(_msgSender(), totalPrice);
    ...
    function buy(
        ...
        if (_id != SERUM_ID && _id != FARMER_PASS_ID) revert InvalidId();
        if (_amount == 0 || _amount > MAX_PER_TX) revert InvalidAmount();
        if (_msgSender() != _to) revert InvalidSender();
    ...
    function uri(uint256 _id) public view override returns (string memory) {
        if (_id >= ITEM_TYPES) revert InvalidId();
    ...
    function _totalPrice(uint256[] memory _ids, uint256[] memory _amounts) internal view returns (uint256) {
        if (_ids.length != _amounts.length) revert NotMatch();

        uint256 total = 0;
        for (uint256 i; i < _ids.length; i++) {
            uint256 id = _ids[i];
            uint256 amount = _amounts[i];
            if (id >= ITEM_TYPES) revert InvalidId();
            if (amount == 0) revert InvalidAmount();
            total += (prices[id] * amount);
        }
        return total;
    }
     ...
     */
    function setSupply(uint32[] memory _caps) external onlyOwner {
        if (_caps.length != ITEM_TYPES) revert NotMatch();
     ...
     *
     * @param _prices Array of 3 game item price
     */
    function setPrice(uint32[] memory _prices) external onlyOwner {
        if (_prices.length != ITEM_TYPES) revert NotMatch();
