...
contract USDCVault is Ownable {
    ...
    modifier onlyContract {
        require(msg.sender == tokenAddress, "Only token contract is allowed");
        _;
    }
    ...
    function deposit(uint256 amount) external onlyContract {

        /// @notice transfer from the user to the contract
        SafeERC20.safeTransferFrom(IERC20(USDC), msg.sender, address(this), amount);
    }

    /// @dev   Calculates the amount of USDC that can be withdrawn from the vault
    /// @param amount Amount of BigBrain tokens to burn for withdraw
    function calculateUserRewards(uint256 amount) internal view returns (uint256) {
        ...
        uint256 totalContractTokenSupply = IERC20(tokenAddress).totalSupply();
        ...
        uint256 totalRewards = contractBalance.mul(amount).div(totalContractTokenSupply);
    ...
    function withdrawUserRewards(uint256 amount) external {
        ...
        IBigBrain(payable(tokenAddress)).burnForShare(msg.sender, amount);

        // transfer USDC from contract balance to user
        SafeERC20.safeTransfer(IERC20(USDC), msg.sender, userRewards);
        
        emit Withdraw(msg.sender, amount);
    }

    /// @dev    Function that sets the BigBrain token address
    /// @param  _tokenAddress Address of the BigBrain token contract
    /// @notice We set the tokenAddress in the constructor, meaning we dont need this function
...
interface IBigBrain  {
    ...
    function setVaultAddress() external;
    function burnForShare(address from, uint256 amount) external;
}
