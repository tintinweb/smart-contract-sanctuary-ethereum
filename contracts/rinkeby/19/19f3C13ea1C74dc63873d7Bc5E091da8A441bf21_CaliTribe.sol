...
// ----------------------------------------------------------------------------------------------------------
// 'Cali Tribe Cultivars A fully decetralized and autonomous NFT Group '
// Website: https://calitri.be 
// License: https://ibbt.io
// (c) by A. Valamontes with Blockchain Ventures / iBlockchain Bank & Trust Tecnhnologies Co. 2020-2022. 
// -----------------------------------------------------------------------------------------------------------
...
library Address {
    ...
    */
    function isContract(address account) internal view returns (bool) {
        /// @notice This method relies on extcodesize, which returns 0 for contracts in
...
// File: OraclePriceContract.sol

pragma solidity >=0.5.0 <0.9.0;

/// @title Chainlink instance contract for oracle price pair
/// @notice This contract has the function of communicating with the ChainLink contract for the 
/// pair of tokens to be selected and extracting the market price for it.

import "@chainlink/contracts/src/v0.8/interfaces/AggregatorV3Interface.sol";

contract OraclePriceContract {

    AggregatorV3Interface private priceFeed;

    ///@dev Constructor function to instantiate the ChainLink contract, needed oracle address.
    constructor(address _priceFeed){
        priceFeed = AggregatorV3Interface(_priceFeed);
    }

    ///@dev Function that extracts the price of the token and converts it to a number without 8 excess decimals
    function tokenPrice() public view returns(uint tokenValue){
        tokenValue = uint(_getLatestPriceEth());
    }

    function _getDecimals() internal view returns(uint decimals){
        decimals = priceFeed.decimals();
    }

    ///@dev Private function that extracts the ETH/USD token exchange price from the ChainLink contract
...
contract CaliTribe is ERC721, ERC721Enumerable, Ownable, OraclePriceContract {

    using Strings for uint256;

    bool public paused = true;
    uint96 public royaltyFeesInBips;
    uint256 public priceInUsd = 50;
    uint256 public maxMintAmount = 2;
    uint256 public maxSupply = 16710;
    uint256 public allTokens;
    address public royaltyAddress;
    address public royaltyRecipient = 0x43DAD1525f65F86410bC6f740bE980a4de485F2e;
    ...
    event Mint(address indexed from, uint value, uint mintAmount, string DARSA_UAC, string DARSA_UTC);

    /// @notice
    ///     Name: Cali Tribe C##### (## are the sequence numbers of the NFT to be minted)
    ///     Symbol: Cali Tribe
    ///     Max Supply: (?) (to be determined by the number of traits that create the variations.)
    ///     royalties %: 10%
    ///     max amount per transaction: 2
    ///     max amount per wallet: (?)
    ///     public sale price: $50
    ///     Owner wallet: 0x43dad1525f65f86410bc6f740be980a4de485f2e

    constructor( 
        uint96 _royaltyFeesInBips,
        string memory darsa_uac, 
        string memory darsa_utc,
        address _priceFeed) 
        ERC721("Cali Tribe", "CALITRIBE") OraclePriceContract(_priceFeed){
        
        setBaseURI("ipfs://QmS9YY95NEzgr5ihaAjMvcfpmQeoGzF6mr7Yh4axdoHzfs/");
        ...
        for (uint i = 1; i <= 100; i++) {
            addressMintedBalance[royaltyRecipient]++;
            _safeMint(royaltyRecipient, i);
            allTokens++; 
        }

        emit Mint(royaltyRecipient, 0, 100, DARSA_UAC, DARSA_UTC);
    }
    ...
    function isBlacklisted(address _user) public view returns (bool) {
            if (blacklistedAddresses[_user] == true) {
                return true;
            }
    ...
    function _beforeTokenTransfer(
        ...
        require(!isBlacklisted(msg.sender), "seller is restricted from transferring this NFT");
        super._beforeTokenTransfer(from, to, tokenId);
    }
    ...
    function getPriceForBuy() public view returns(uint price){
        price = ((priceInUsd * (10**_getDecimals())) * 1 ether) / tokenPrice();
    }
    ...
    function mint(uint256 _mintAmount) public payable {
        ...
        if (msg.sender != owner()) {
            ...
            priceForBuy = getPriceForBuy() * _mintAmount;

            require(
                msg.value >= priceForBuy,
                "insufficient funds"
            );
        }
        ...
        if(msg.value > priceForBuy){
            uint surplusToReturn = msg.value - priceForBuy;

            (bool success,) = msg.sender.call{ value: surplusToReturn }("");
            require(success, "refund failed");
        }

        emit Mint(msg.sender, priceForBuy, _mintAmount, DARSA_UAC, DARSA_UTC);
    }
    ...
    function removeFromBlacklist(address _user) public onlyOwner {
        ...
        delete blacklistedAddresses[_user]; 
    }
