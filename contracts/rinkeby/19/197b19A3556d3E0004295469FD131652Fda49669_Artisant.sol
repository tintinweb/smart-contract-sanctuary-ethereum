...
contract ERC721 is ERC165, IERC721 {
  ...
  */
  function transferFrom(
    ...
    public
  {
    require(_isApprovedOrOwner(msg.sender, tokenId),"this sender cant transfer this token");
...
contract Artisant is ERC721Full, IERC721Creator, Ownable {

    using SafeMath for uint256;

    // Percentage to owner of Artisant for primary token sale. (* 10) to allow for < 1% 
    uint256 public primaryMaintainerPercentage = 150; 

    // Percentage to owner of Artisant for secondary token sale. (* 10) to allow for < 1% 
    uint256 public secondaryMaintainerPercentage = 50; 

    // Percentage to creator of artwork. (* 10) to allow for tens decimal. 
    uint256 public creatorPercentage = 0; 
    ...
    event SalePriceSet(address indexed _owner, uint256 indexed _tokenId, uint256 indexed _price);
    event TokenMinted(address indexed _creator, address indexed _owner, uint256 indexed _tokenId, string _uri);
    ...
    /**
     * @dev Checks that the token is NOT owned by the sender.
     * @param _tokenId uint256 ID of the token.
     */
    modifier notOwnerOf(uint256 _tokenId) {
      address owner = ownerOf(_tokenId);
      require(owner != msg.sender, "cant be the owner of the token");
      _;
    }
    ...
    function setExchangeFees(uint256 _primaryArtisantFee, uint256 _secondaryArtisantFee, uint256 _cratorRoyalty) public onlyOwner() {
      primaryMaintainerPercentage = _primaryArtisantFee;
      secondaryMaintainerPercentage = _secondaryArtisantFee;
      creatorPercentage = _cratorRoyalty;
    }
     ...
     */
    function _updateTokenMetadata(uint256 _tokenId, string _uri) internal {
    ...
    */
     ...
     */
    function addNewTokenWithEditions(string _uri, uint256 _editions, uint256 _salePrice) public {
      require(_editions < 101, 'creating more then 100 editions is not allowed');
      for (uint256 i=0; i<_editions; i++){
        uint256 newId = createToken(_uri, msg.sender, msg.sender);
        tokenSalePrice[newId] = _salePrice;
        emit SalePriceSet(msg.sender,newId, _salePrice);
      }
    }

    function addTokensAsAdmin(address _creator, address _owner, string _uri, uint256 _editions, uint256 _salePrice) public onlyOwner() {
      require(_editions < 101, 'creating more then 100 editions is not allowed');
      for (uint256 i=0; i<_editions; i++){
        uint256 newId = createToken(_uri, _creator, _owner);
        tokenSalePrice[newId] = _salePrice;
        emit SalePriceSet(_owner, newId, _salePrice);
      }
    }
     ...
     */
    function setSalePrice(uint256 _tokenId, uint256 _salePrice) public onlyTokenOwner(_tokenId) {
        address owner = ownerOf(_tokenId);
        tokenSalePrice[_tokenId] = _salePrice;
        emit SalePriceSet(owner, _tokenId, _salePrice);
    }
    ...
    */
    ...
    function buy(uint256 _tokenId) public payable notOwnerOf(_tokenId) {
        ...
        address creator = tokenCreator(_tokenId);
        address mantainer = owner();
        require(salePrice > 0, 'price of token should be > 0');
        require(sentPrice >= salePrice, 'input price lower than salePrice');
        clearApprovalAndTransfer(tokenOwner, buyer, _tokenId);
        payout(sentPrice, mantainer, creator, tokenOwner, _tokenId);
    ...
    */
    function payout(uint256 _val, address _maintainer, address _creator, address _tokenOwner, uint256 _tokenId) private {
        ...
        if (tokenSold[_tokenId]) {
            maintainerPayment = _val.mul(secondaryMaintainerPercentage).div(1000);
            creatorPayment = _val.mul(creatorPercentage).div(1000);
            ownerPayment = _val.sub(creatorPayment).sub(maintainerPayment); 
        } else {
            maintainerPayment = _val.mul(primaryMaintainerPercentage).div(1000);
            creatorPayment = _val.sub(maintainerPayment); 
            ownerPayment = 0;
        }
        tokenSold[_tokenId] = true;
        if (maintainerPayment > 0) {
     ...
     * @param _uri string metadata uri associated with the token
     */
    function createToken(string _uri, address _creator, address _firstOwner) private returns (uint256){
        uint256 newId = totalSupply() + 1;
        _mint(_firstOwner, newId);
        tokenCreators[newId] = _creator;
        _updateTokenMetadata(newId, _uri);
        emit TokenMinted(_creator,_firstOwner,newId,_uri);
        return newId;
    }
