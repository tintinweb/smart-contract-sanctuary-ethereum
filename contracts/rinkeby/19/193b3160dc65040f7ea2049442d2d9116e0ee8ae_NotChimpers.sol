...
contract NotChimpers is ERC721A, Ownable {
	...
	string public prerevealURL = "ipfs.//QmVmdjhfJ8qP3URfY9KfbYgqvre2F7219EFBDg7epPtzjM";
	mapping(address => uint) private _walletMintedCount;

	constructor(address splitterAddress)
	ERC721A('TrulyChimpers', 'NCHIMP') {
	...
	function contractURI() public pure returns (string memory) {
		return "ipfs://QmRoR51vNnDb5AKx2wMUemeF2AL77dL9odouR3jx7SXMnj";
	}
	...
	function reveal(string memory url) external onlyOwner {
        require(!isMetadataFinal, "Not Chimper: Metadata is finalized");
		_baseURL = url;
	}
	...
	function withdraw() external onlyOwner {
		...
		require(balance > 0, 'Not Chimper: No balance');
	...
	function mint(uint count) external payable {
		require(!isPaused, 'NotChimper: Sales are off');
		require(count <= MAX_NFT_PER_TRAN,'NotChimper: Exceeds NFT per transaction limit');
		require(_totalMinted() + count <= maxSupply,'NotChimper: Exceeds max supply');
		...
		require(
			...
			'NotChimper: Ether value sent is not sufficient'
		);
