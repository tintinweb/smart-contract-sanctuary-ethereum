...
function test1() external view returns (address tokenAddress) {
	address myTokenAddress = erc721token();
	return myTokenAddress;	
}

function test2(uint256 tokenId) external view returns (address val) {
	ERC721 token = erc721token();
	return token.getApproved(tokenId);
}

function test3() external view returns (address val) {
	address to = address(this);

	return to;
}

function test4() public view returns (address val) {
	ERC721 token = erc721token();
	return token.whoareyou();
}
