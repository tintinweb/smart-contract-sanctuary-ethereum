...
contract TheOtherSideV16 is ERC721EnumerableT, Delegated, ITokenTransfer, ReentrancyGuard {
  ...
  ITOSStake private stakingAddress;
  ...
  constructor()
    ERC721T("The Other Side v.16", "TOSV16"){
  }
  ...
  /**
    ...
    */
    function getStakedTokens(address _user) public view returns (uint256[] memory tokenIds) {
        return stakingAddress.getStakedTokens(_user);
    }
  ...
  */
    function stake( uint[] calldata tokenIds ) external {
        ...
        for( uint i; i < tokenIds.length; ++i ){
            ...
            stakingAddress.stake(msg.sender, tokenIds[i]);
        }
    }
...
/**
  ...
  */
    function unStake( uint[] calldata tokenIds ) external {
        ...
        for( uint i; i < tokenIds.length; ++i ){
            stakingAddress.unStake(msg.sender, tokenIds[i]);         
        }
    }
  ...
  /**
    ...
    */
    function setStakingAddress(address _stakingAddress) external onlyOwner {
        stakingAddress = ITOSStake(_stakingAddress);
        ITOSStake(_stakingAddress).regTokenTransfer(address(this));
    }

  /**
    * @dev Transfer the token bsed the stake states and emit event if Staked/Unstaked.
    */
    function transferStakeToken(StakeState _stakeState,address _from, uint tokenId) external override{
      if (_stakeState == StakeState.Stake) {
        _transfer(_from,address(this), tokenId);
         emit Staked(_from, tokenId);
      } else {
         _transfer(address(this),_from,tokenId);
         emit Unstaked(address(this), tokenId);
      }
    }
}
...
interface ITokenTransfer {
    ...
    function transferStakeToken(StakeState _stakeState, address _from, uint tokenId) external;
}
