...
contract MoBuDao is ERC721A, Ownable {
    ...
    constructor(string memory initBaseURI) ERC721A("MoBuDao", "MBD", 5) {
        __baseURI = initBaseURI;
    }

    function mint(uint256 amount) external payable {
        require(amount * MINT_PRICE >= msg.value, "not enough fund");
        require(totalSupply() + amount <= MAX_SUPPLY, "exceed max supply");
        _safeMint(_msgSender(), amount);
    }

    /// @dev Owner reserve
    function reserve(address payable to, uint256 amount) external onlyOwner {
        require(totalSupply() + amount <= MAX_SUPPLY, "exceed max supply");
        _safeMint(to, amount);
    }
