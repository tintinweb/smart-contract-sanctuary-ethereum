...
contract HashlipsTest is ERC721, Ownable {
    ...
    uint public constant TOKENS_PER_PERSON_PUB_LIMIT = 30;
    uint public constant TOKENS_PER_PERSON_WL_LIMIT = 30;
    ...
    function reveal(string memory uri) external onlyOwner {
        require(!isRevealed, "ZeroCodeNFT: Already revealed");
        isRevealed = true;
        _baseURL = uri;
    }
    ...
    function redeem(bytes32[] calldata merkleProof, uint count) external payable {
        ...
        if(saleStatus == SaleStatus.PRESALE) {
            ...
            require(_whitelistMintedCount[msg.sender] + count <= TOKENS_PER_PERSON_WL_LIMIT, "ZeroCodeNFT: Requested token count exceeds allowance (30)");
            ...
            require(_mintedCount[msg.sender] + count <= TOKENS_PER_PERSON_PUB_LIMIT, "ZeroCodeNFT: Requested token count exceeds allowance (30)");
