...
contract Nfinity is ERC721Upgradeable,OwnableUpgradeable,PausableUpgradeable,ReentrancyGuardUpgradeable {
    ...
    mapping(address => uint256) private publicMintClaimed;
    struct NftData {
    ...
    function withdraw(address payable _manager) external onlyOwner {
        uint256 balance = address(this).balance;
        require(balance > 0, "Withdraw: Balance should be more then zero !");
        _manager.transfer(balance);
    }
    ...
    function whiteListMint(
        ...
        require(
            quantity > 0 &&
                quantity <= MAX_PER_TX &&
                whiteListClaimed[to] <= MAX_PER_TX,
            "WhiteList: Max per tx amount exceeded"
        );
        require(
            totalSupply() + quantity <= MAX_NFT - supply(),
        ...
        require(MerkleProofUpgradeable.verify(_merkProof, merkle, leaf), "Invalid proof");
        whiteListClaimed[to] += quantity;
        ...
        isHuman
    {
        ...
        require(
            quantity > 0 &&
                quantity <= MAX_PER_TX &&
                publicMintClaimed[to] <= MAX_PER_TX,
            "PublicMint: quantity prohibited"
        );
        require(
            totalSupply() + quantity <= MAX_NFT - supply(),
        ...
        publicMintClaimed[to] += quantity;
        currentIndex += quantity;
        multiplex(to, quantity);
    }

    function airdrop(address[] calldata user,uint256[] calldata amounts) external onlyOwner {
        require(airDropSwitch, "AirDrop is not active");
        require(
            user.length == amounts.length && airDrop > 0 && amounts.length < airDrop  && count <= airDrop,
            "AirDrop:The number of people exceeds the limit"
        );
        require(
            totalSupply() + amounts.length <= MAX_NFT,
            "AirDrop: The number of people exceeds the limit"
        );
        for (uint256 j = 0; j < amounts.length; ) {
            ...
            currentIndex++;
            count++;
            _safeMint(user[j], tokenId);
            emit Cast(tokenId, user[j]);
            unchecked {
                j++;
            }
        }
    ...
    {   
        ...
        return blindBoxOpened ? blindTokenURI :  string(
        abi.encodePacked(
          "https://ipfs.io/ipfs/",
