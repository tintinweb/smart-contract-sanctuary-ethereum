...
contract Minter is Ownable, Pausable {
    ...
    mapping(address => uint32) public waitlist;
    Stage public stage;    
    ...
    function withdrawERC721(address _to, address _tokenContract, uint256 _tokenId) external onlyOwner {
    ...
    function privateMint(uint32 _amount, LibWaitlist.Waitlist calldata _waitlist, bytes calldata _signature, uint256[] calldata _options) external payable whenNotPaused passingStage(Stage.PRIVATE) {
        require(verifyTypedDataHash(domainSeparator, _waitlist, _signature, owner()), "bad sig");
        require(_waitlist.owner == msg.sender, "bad sender");
        require(waitlist[msg.sender] <= _waitlist.amount + _amount, "bad amount");
        require(msg.value >= _amount * price, "bad value");

        waitlist[msg.sender] += _amount;
    ...
    function verifyTypedDataHash(bytes32 _domainSeparator, LibWaitlist.Waitlist calldata _waitlist, bytes calldata _signature, address _owner) internal pure returns (bool) {
