    ...
    Ownable
{
    ...
    // 地址令牌总数 addr -> tokenCount
    mapping(address => uint256) private _balances;
    // id对应地址 mapping tokenID -> addr
    mapping(uint256 => address) private _owners;
    // id授权  tokenID_appr - addr
    mapping(uint256 => address) private _tokenApprovals;
    ...
    function mintTo(address _to, bytes memory _data) public whenNotPaused {
...
contract AssetContract is ERC721Tradable, ReentrancyGuard {
    ...
    uint256 constant PRODUCE_TOKEN_MAX = 100;
    ...
    // id对应地址  ( tokenId  => tokenType )
    // mapping(uint256 => string) private _addrTokenTyepOf;
    ...
    function mint(
        ...
        payable(super._getManagerAddress()).transfer(msg.value);
        for (uint256 i = 0; i < numberOfTokens; i++) {
            uint256 mintIndex = super.totalSupply();
            mintTo(_msgSender(), _data);
            if (bytes(_data).length > 0) {
                _setURI(mintIndex, string(_data));
            }
            //   _addrTokenTyepOf[mintIndex] = _tokenType;
        }
    ...
    function _ownsTokenAmount(address _from, uint256 _id)
        ...
        returns (bool)
    {
        return _isCreatorOrProxy(_from) ? true : ownerOf(_id) == _from;
    }

    function _isCreatorOrProxy(address _address) internal view returns (bool) {
        ...
        returns (string memory)
    {
        ...
        if (bytes(tokenUri).length > 0) {
            return string(abi.encodePacked(base, tokenUri, ".json"));
        }
...
contract Commodity is AssetContract {
    constructor()
        AssetContract(
            ...
            0.1 * 10**18,
            "hotsales",
            "https://nft-buc-1302315235.cos.ap-beijing.myqcloud.com/txt0/"
        )
    {}
}