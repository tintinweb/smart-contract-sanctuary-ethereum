...
contract PunkDNA {
    ...
    // This function is deterministic and should not be used on production, it is only for testing purposes use ChainLink
    ...
    function _getDNASection(uint256 _dna, uint8 _rightDiscard) public pure returns(uint8)  {
        return uint8(_dna%(10**(_rightDiscard+ADN_SECTION_SIZE))/
                     (10**(_rightDiscard)));
    }
    ...
    function getClotheColor(uint256 _dna) public view returns(string memory){
        return _accessoriesType[_getDNASection(_dna, 2)%_accessoriesType.length];
    }
...
// File contracts/SesshoPunks.sol
