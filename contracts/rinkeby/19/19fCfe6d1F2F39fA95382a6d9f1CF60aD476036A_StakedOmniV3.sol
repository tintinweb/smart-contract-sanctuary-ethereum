...
// File @omni/omni-stake/contracts/interfaces/[email protected]
...
// File @omni/omni-token/contracts/open-zeppelin/[email protected]
...
// // File @omni/omni-token/contracts/interfaces/[email protected]
...
// File @omni/omni-stake/contracts/lib/[email protected]
...
// File @omni/omni-stake/contracts/utils/[email protected]
...
interface IOmniDistributionManager {
    function configureAssets(
        DistributionTypes.AssetConfigInput[] calldata assetsConfigInput
    ) external;
}

// File @omni/omni-stake/contracts/stake/[email protected]

pragma solidity 0.8.10;

/**
 * @title OmniDistributionManager
 * @notice Accounting contract to manage multiple staking distributions
 * @author Omni
 **/
contract OmniDistributionManager is IOmniDistributionManager {
    using SafeMath for uint256;
     ...
     * @param underlyingAsset The address used as key in the distribution, for example sOMNI or the aTokens addresses on Omni
     ...
     * @notice This field remains virtual to extend at OmniDistributionManageV2, keeping V1 logic
...
// File @omni/omni-token/contracts/token/base/[email protected]
    ...
    IGovernancePowerDelegationToken
{
     ...
     * In this initial implementation with no OMNI minting, simply returns the current supply
     * A snapshots mapping will need to be added in case a mint function is added to the OMNI token in the future
     **/
    function totalSupplyAt(uint256 blockNumber)
        external
        view
     ...
     * the OMNI token and StakeToken already include a mapping for the snapshots, so we require contracts
    ...
    GovernancePowerDelegationERC20
{
     ...
     * _omniGovernance => _omniGovernance
     */
    mapping(address => mapping(uint256 => Snapshot)) public _votingSnapshots;
    mapping(address => uint256) public _votingSnapshotsCounts;

    /// @dev reference to the Omni governance contract to call (if initialized) on _beforeTokenTransfer
    /// !!! IMPORTANT The Omni governance is considered a trustable contract, being its responsibility
    /// to control all potential reentrancies by calling back the this contract
    ITransferHook public _omniGovernance;

    function _setOmniGovernance(ITransferHook omniGovernance) internal virtual {
        _omniGovernance = omniGovernance;
    }
}
...
/**
 * @title StakedToken
 * @notice Contract to stake Omni token, tokenize the position and get rewards, inheriting from a distribution manager contract
 * @author Omni
 **/
    ...
    OmniDistributionManager
{
    ...
    constructor(
        ...
        public
        ERC20(name, symbol)
        OmniDistributionManager(emissionManager, distributionDuration)
    {
        ...
        _omniGovernance = ITransferHook(governance);
    ...
    function _beforeTokenTransfer(
        ...
        // caching the omni governance address to avoid multiple state loads
        ITransferHook omniGovernance = _omniGovernance;
        if (omniGovernance != ITransferHook(address(0))) {
            omniGovernance.onTransfer(from, to, amount);
        }
    }
...
/**
 * @title RoleManager
 * @notice Generic role manager to manage slashing and cooldown admin in StakedOmniV3.
...
/**
 * @title StakedOmniV3
 * @notice StakedTokenV3 with OMNI token as staked token
 * @author Omni
 **/
contract StakedOmniV3 is StakedTokenV3 {
    string internal constant NAME = "Staked Omni";
    string internal constant SYMBOL = "stkOMNI";
...
// File contracts/stake/StakedOmniV3.sol