...
// File: contracts/Leatherface.sol
...
abstract contract Salable is ERC721Enumerable, Ownable {
    ...
    function setSaleActiveState(bool state) external onlyOwner {
        if (state == true) {
            require(
                isWhitelistSaleActive == false && isPreSaleActive == false,
                "Other sales not inactive"
            );
        }
    ...
    function setWhitelistSaleState(bool state) external onlyOwner {
        if (state == true) {
            require(
                isPublicSaleActive == false && isPreSaleActive == false,
                "Other sales not inactive"
            );
        }
    ...
    function setPreSaleState(bool state) external onlyOwner {
        if (state == true) {
            require(
                isPublicSaleActive == false && isWhitelistSaleActive == false,
                "Other sales not inactive"
            );
        }
    ...
    function mintForAirdrop(uint256 numberToMint) external onlyOwner {
        require(
            totalSupply() + numberToMint <= maxNFTCirculation,
            "TCM: Cap exceeded"
        );
...
contract URMintPass is ERC721, ERC721Enumerable, Ownable, Salable {
    ...
    function mintInPublic() external payable onlyWhenPublicSaleIsActive {
        ...
        require(
            ...
            "TCM: User may only buy one pass"
        );
...
contract TCM is ERC721, ERC721Enumerable, Ownable, Salable {
    ...
    constructor() ERC721("TexasChainsawMassacre", "TCM") {
        isPublicSaleActive = false;
        isWhitelistSaleActive = false;
    ...
    function mintInPublic(uint256 numberToMint)
        external
        payable
        onlyWhenPublicSaleIsActive
    {
        ...
        require(
            mintCost * numberToMint <= msg.value,
            "TCM: Incorrect Ether Value"
        );
    ...
    function mintInPresale(uint256 numberToBuy)
        external
        payable
        onlyWhenPreSaleIsActive
    {
        ...
        require(
            mintCost * numberToBuy <= msg.value,
            "TCM: Incorrect Ether Value"
        );
        ...
        for (uint256 bPos = 0; bPos < balanceOf; bPos++) {
            uint256 mintPassTokenIndex = mintPass.tokenOfOwnerByIndex(
                msg.sender,
                bPos
            );
