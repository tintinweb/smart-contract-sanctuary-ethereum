    ...
    MintPassRarityGenerator
{
    ...
    // Bid[] bidsArray;
    mapping(uint256 => Bid[]) public bidsArray;

    uint256 public constant MIDDLE_LEAGUES = 10000000000000000; // 0.01
    uint256 public constant TOP_LEAGUES = 100000000000000000; // 0.1
    ...
    {
        leagues1 = bidsArray[0];
        leagues2 = bidsArray[1];
        leagues3 = bidsArray[2];
    }

    function getUserBidIds(address _address)
        public
        view
    ...
    function claimPass() external onlyWhenFinished {
        ...
        require(
            userBidIds[_msgSender()].length > 0 &&
                (latestBidId <= totalBidsLimit ||
                    userBidIds[_msgSender()][
                        userBidIds[_msgSender()].length - 1
                    ] >
                    latestBidId - totalBidsLimit),
            "User didn't win auction"
        );

        // A user could have thousands of bids, may provoke a gas problem here
        for (uint256 i = 0; i < userBidIds[_msgSender()].length; i++) {
            if (
                bids[userBidIds[_msgSender()][i]].bidValue > 0 &&
                (latestBidId <= totalBidsLimit ||
                    userBidIds[_msgSender()][i] > latestBidId - totalBidsLimit)
            ) {
                bids[userBidIds[_msgSender()][i]].bidValue = 0;
                ...
                uint256 position = latestBidId <= totalBidsLimit
                    ? userBidIds[_msgSender()][i]
                    : userBidIds[_msgSender()][i] -
                        (latestBidId - totalBidsLimit);
    ...
    {
        require(
            bidValue > lastBidAmount,
            "There is not enough funds to make a bid"
        );
        require(
        ...
        for (uint256 i = 0; i < bidsAmount; i++) {
            ...
            if (bidValue < MIDDLE_LEAGUES) {
                bidsArray[2].push(bids[latestBidId]);
            } else if (bidValue >= MIDDLE_LEAGUES || bidValue < TOP_LEAGUES) {
                bidsArray[1].push(bids[latestBidId]);
            } else {
                bidsArray[0].push(bids[latestBidId]);
            }
            latestBidId++;
    ...
    function _calculateRarityForBids(uint256 position)
        private
        returns (Rarity)
    {
