...
contract PirateGames is IPirateGames, VRFConsumerBaseV2, Pausable {
    ...
    mapping(uint16 => uint256) private _VRFrandom;


    uint16 randomToggle = 0;
    uint16 toggleLimit;
    uint16 private _commitId = 0;
    ...
    function canMint(address addr) external view returns (bool) {
        return
            ...
            _VRFrandom[_pendingCommitId[addr]] > 0;
    }
  ...
  function requestRandomWords() internal onlyOwner {
        ...
        noCheaters
    {
        ...
        _commitId += 1;
        randomToggle += 1;
        if (randomToggle == toggleLimit) {requestRandomWords();
        randomToggle = 0;}
    ...
    function pirateAttempt(address addr) internal {
        ...
        require(commitIdCur > 0, "No pending commit");
        require(randomness.length > commitIdCur, "Commit random not set");
        MintCommit memory commit = _mintCommits[addr][commitIdCur];
        pendingMintAmt -= 1;
        uint16 colonistId = commit.tokenId;
        uint16 piratesMinted = pirateNFT.piratesMinted();
        uint256 seed = randomness[_commitId];
