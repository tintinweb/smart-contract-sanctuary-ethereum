...
contract AmvNftStaker is Ownable {
     ...
     * @param stakingTokenIds The tokenIDs of these NFTs.
     * @param timestamp The execution timestamp of the staking function.
     */
    event NFTStaked(
        address owner,
        uint256[] stakingTokenIds,
        uint256 timestamp
    );
     ...
     * @param owner The address of the owner of these NFTs.
     * @param unstakingTokenIds The tokenIDs of these NFTs.
     * @param timestamp The execution timestamp of the unstaking function.
     */
    event NFTUnstaked(
        address owner,
        uint256[] unstakingTokenIds,
        uint256 timestamp
    );
    ...
    function setMaxInputSize(uint256 _maxInputSize) public onlyOwner {
         ...
         */
        require(_maxInputSize <= 100, "Can not set MaxInputSize more than 100");
        /**
     ...
     */
    function stake(uint256[] calldata tokenIds, uint256 timeLockType) external {
        /**
        ...
        emit NFTStaked(msg.sender, tokenIds, block.timestamp); // emiting NFTStaked event.
    }
    ...
    function unstake(uint256[] calldata tokenIds) external {
        ...
        emit NFTUnstaked(msg.sender, tokenIds, block.timestamp); //emiting NFTUnstaked event.
    }

    /**
     * @dev Throws if the differnce between the current timestamp and staking timestamp is not greater than tokens owner's predefined time-lock.
     */
    function timeLockCheck(uint256 stakedAt, uint256 timeLock) public view {
        require(
            (block.timestamp - stakedAt) > timeLock || !isTimeLockActive,
