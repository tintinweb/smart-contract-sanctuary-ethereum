...
contract OwnedDistributor is Ownable, ReentrancyGuard, IClaimable {
    using SafeERC20 for IERC20;

    address public immutable MON = 0x7eCF284BbC833b8AEFFF79796E752538f48598b0; // MON (Ethereum) 0x0b3E2EaCA41fcC50fFFCCEc467c9f7d2479Af709
    address public claimable; // Vester Contract
    address public liqBootstrap; // LiqBootstrap Contract
    ...
    modifier onlyLiqBootstrap() {
        require(msg.sender == liqBootstrap, "Not authorized");
        _;
    }

    constructor(address _claimable) {
        claimable = _claimable;
    }

    function updateShareIndex() public nonReentrant returns (uint256 _shareIndex) {
    ...
    function editRecipient(address account, uint256 shares) public onlyLiqBootstrap {
        _editRecipientInternal(account, shares);
    }

    function _editRecipientInternal(address account, uint256 shares) internal {
    ...
    function getShareIndex() public view returns (uint256 _shareIndex) {
        (uint256 amount, ) = IVester(claimable).getUnlockedAmount();
        _shareIndex = ((amount * 2**160) / totalShares) + shareIndex;
    }

    function setLiqBootstrap(address _liqBootstrap) external onlyOwner {
        require(_liqBootstrap != address(0), "Not valid address");
        liqBootstrap = _liqBootstrap;
    }
}
