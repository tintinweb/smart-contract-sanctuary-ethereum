    ...
    ICollection
{
        ...
        onlyArtist(_metadataId)
    {
        ...
        emit SetCost(_cost, _metadataId);
    }
    ...
    function approve(uint256 _metadataId, bool isApproved) public onlyOwner {
        metadata[_metadataId].approved = isApproved;
        emit Approve(_metadataId, isApproved);
    }

    function approveBatch(uint256[] memory _metadataIds, bool isApproved)
        public
        onlyOwner
    {
        for (uint256 index = 0; index < _metadataIds.length; index++) {
            approve(_metadataIds[index], isApproved);
        }
    }
    ...
    function preMint(
        ...
        createMetadata(artist, _id, _duration, _duration, _cost, uri, false);
...
interface ICollection is IERC721, IMetadata {
    event Approve(uint256 metadataId, bool isApproved);
    event SetCost(uint256 cost, uint256 metadataId);
    event Mint(address to, uint256 duration, uint256 cost, string uri);
    event PreMint(
        address to,
        uint256 id,
        uint256 duration,
