...
contract GranChacoImpactNFT is ERC721Enumerable, Ownable {
    ...
    function mintOneOfOne(address to, uint256 tokenId, uint256 tierNumber) external payable {
        ...
        if (tierNumber == 1) {
            ...
            payable(COST_WALLET).transfer(msg.value * TIER1_COST_PERCENT / 10000);
        } else if (tierNumber == 2) {
            require(TIER2_PRICE <= msg.value,"ETH amount is not sufficient");
            payable(COST_WALLET).transfer(msg.value * TIER2_COST_PERCENT / 10000);
        } else if (tierNumber == 3) {
            require(TIER3_PRICE <= msg.value,"ETH amount is not sufficient");
            payable(COST_WALLET).transfer(msg.value * TIER3_COST_PERCENT / 10000);
        } 
    ...
    function mintCommon(address to, uint256 tokenAmount) external payable {
        ...
        payable(COST_WALLET).transfer(msg.value * TIERX_COST_PERCENT / 10000);
