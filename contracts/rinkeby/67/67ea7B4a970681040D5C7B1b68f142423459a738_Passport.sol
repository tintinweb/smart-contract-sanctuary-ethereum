    ...
    ERC2771Recipient
{
    ...
    /// @param _amount Number of tokens to mint, must be same as amount in merkle tree leaf
    function claimWhitelist(bytes32[] calldata _proof, uint256 _amount) external payable {
        require(whitelistClaimEnabled, "whitelist claim disabled");
        if (whitelistClaimFee > 0) require(msg.value == whitelistClaimFee * _amount, "fee amount incorrect");
        bool validProof = MerkleProof.verify(_proof, whitelistRoot, keccak256(abi.encodePacked(_msgSender(), _amount)));
    ...
    /// @notice Allows manager to set the minting claim fee
    /// @param _claimFee The claim fee in wei
    function setClaimLimit(uint256 _claimFee) external onlyRole(MANAGER_ROLE) {
        claimFee = _claimFee;
    }
