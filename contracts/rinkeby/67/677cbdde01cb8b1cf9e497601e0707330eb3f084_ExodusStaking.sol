...
// File: types/ExodusAccessControlled.sol
...
contract ExodusStaking is ExodusAccessControlled {
    ...
    /**
     * @notice stake EXO to enter warmup
     * @param _to address
     * @param _amount uint
     * @param _claim bool
    ...
    /**
     * @notice forfeit stake and retrieve EXO
     * @return uint
     */
    ...
    function unstake(
        ...
        if (_rebasing) {
            ...
            gEXO.burn(msg.sender, _amount); // amount was given in gEXO terms
            amount_ = gEXO.balanceFrom(amount_).add(bounty); // convert amount to EXO terms & add bounty
        }

        require(amount_ <= EXO.balanceOf(address(this)), "Insufficient EXO balance in contract");
        EXO.safeTransfer(_to, amount_);
    }

    /**
     * @notice convert _amount sEXO into gBalance_ gEXO
     * @param _to address
     * @param _amount uint
    ...
    /**
     * @notice convert _amount gEXO into sBalance_ sEXO
     * @param _to address
     * @param _amount uint
    ...
    function rebase() public returns (uint256) {
        ...
        if (epoch.end <= block.timestamp) {
            ...
            if (address(distributor) != address(0)) {
                ...
                bounty = distributor.retrieveBounty(); // Will mint exo for this contract if there exists a bounty
            }
    ...
    /**
     * @notice send staker their amount as sEXO or gEXO
     * @param _to address
     * @param _amount uint
     * @param _rebasing bool
     */
    function _send(
        ...
        if (_rebasing) {
            sEXO.safeTransfer(_to, _amount); // send as sEXO (equal unit as EXO)
            return _amount;
        } else {
            gEXO.mint(_to, gEXO.balanceTo(_amount)); // send as gEXO (convert units from EXO)
    ...
    /**
     * @notice returns the sEXO index, which tracks rebase growth
     * @return uint
     */
