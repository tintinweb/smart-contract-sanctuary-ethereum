...
abstract contract ERC721URIStorageUpgradeable is Initializable, ERC721Upgradeable {
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        if (bytes(_tokenURI).length > 0) {
            return string(abi.encodePacked(base, _tokenURI, _baseextention));
        }
...
// File: upgradablenft.sol


contract TestUpgradecaculite is Initializable, ERC721Upgradeable, ERC721EnumerableUpgradeable, ERC721URIStorageUpgradeable, PausableUpgradeable, OwnableUpgradeable {
    ...
    function initialize() initializer public {
        __ERC721_init("TestUpgradecaculite", "TUCT");
    ...
    function userMint() public payable {
        ...
        require(msg.value >= price , "The floor price for each token is 0.01 ether");
        require(tx.origin == msg.sender , "External contract cannot call mint");
       
        _tokenIdCounter.increment();
        uint256 tokenId = _tokenIdCounter.current();
        uint contractbal = contractbalance;
        contractbalance = contractbal + msg.value;
    ...
    //     string memory baseURI = _baseURI();
    //     return bytes(baseURI).length > 0 ? string(abi.encodePacked(baseURI, tokenId.toString(), baseextention)) : "";
    // }
    ...
    function _baseURI() internal view override returns (string memory) {
     ...
     function setbaseURI(string memory _baseurl) public onlyOwner returns (string memory) {
      baseurl = _baseurl;
      return baseurl;
    }
}