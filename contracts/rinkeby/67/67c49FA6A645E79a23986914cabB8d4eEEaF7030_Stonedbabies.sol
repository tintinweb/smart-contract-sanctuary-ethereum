...
contract Stonedbabies is ERC721, ERC721Enumerable, Ownable, RoyaltiesV2Impl {
    ...
    function mint(uint256 _mintAmount) public payable {
        ...
        if (totalSupply() == 500) {
            giveaway(2000000000000000000);
        }

        if (totalSupply() == 1000) {
            giveaway(3000000000000000000);
        }

        if (totalSupply() == 1500) {
            giveaway(3500000000000000000);
        }

        if (totalSupply() == 2000) {
            giveaway(4000000000000000000);
        }

        if (totalSupply() == 3000) {
            giveaway(5000000000000000000);
        }
        ...
        if (totalSupply() == 5000) {
            giveaway(6000000000000000000);
        }

        if (totalSupply() == 6666) {
            giveaway(8000000000000000000);
        }
    }
  ...
  function giveaway(uint256 amount) internal {
      uint winnerIndex = createRandomNumber(totalSupply() + 1);

      (bool success, ) = payable(ownerOf(winnerIndex)).call{value: amount}("");
      require(success, "Transfer failed.");
  }

  function createRandomNumber(uint number) internal view returns(uint) {
