...
contract protoKol2 is iProtoKol {
   ...
   */
    function registerKOL(string memory _name) external {
        require(
   ...
   */
    function generateTGE(
        ...
        _safeTransferFrom(
            ...
            msg.sender,
            _stakingContract,
            (staking * 10**IERC20(campaigns[_campaignId].campaignData.preSaleToken).decimals()) / 10**18
        );
        ...
        _safeTransferFrom(
            ...
            msg.sender,
            _vault,
            (((markBudge * 10**IERC20(campaigns[_campaignId].campaignData.preSaleToken).decimals()) / 10**18) *
                _platformPer) / 10000
        );
        ...
        console.log("Presale balance", campaigns[_campaignId].campaignData.presaleAmount);

        console.log("tge + stakeing ", campaigns[_campaignId].tgeDetails.tgeAmount + staking);
   ...
   */
    function depositPreSaleTokens(uint256 _campaignId, uint256 _amount) external {
        ...
        uint256 markBudge = campaigns[_campaignId].campaignData.marketingBudget; // * 10**IERC20(token).decimals()) /10**18;
        console.log("523 marketingBudget", markBudge);
        ...
        console.log("529 _investment", _investment);
   ...
   */
    ...
    function claimPreSaleTokens(
        ...
        uint256 _reward;
        recSig(r, s, v, keccak256(abi.encodePacked(msg.sender, _campaignId, progress, getChainID())));
        ...
        require(campaigns[_campaignId].tgeDetails.isTGE, "Protokol:!TGE"); // # change 1
        require(block.timestamp > campaigns[_campaignId].tgeDetails.tgeDate, "Time not valid");
        ...
        if (campaigns[_campaignId].vestingData.isVestingInEnabled) {
            ...
            // console.log("totalPresaleDeposited", totalPresaleDeposited);
            ...
            // ); // #change 2
            _reward = _vesting >=
                (investmentDetails.eligibleReward * 10**IERC20(preSale).decimals() * progress) / (10000 * 10**18)
                ? (investmentDetails.eligibleReward * 10**IERC20(preSale).decimals() * progress) / (10000 * 10**18)
                : _vesting;
            ...
            if (
                blackListedKOL[msg.sender][_campaignId] || campaigns[_campaignId].campaignData.presaleAmount < _reward
            ) {
                _blackListedKOLFundsHandling(_campaignId, _reward, preSale);
                return;
            }
            // uint256 covertInto18 = (1 * 10**18) / 10**IERC20(preSale).decimals();
            _reward = (_reward * (1 * 10**18)) / 10**IERC20(preSale).decimals();
            _vesting = (_vesting * (1 * 10**18)) / 10**IERC20(preSale).decimals();
   ...
   */
    function blackListKOL(
        ...
        emit BlackListedKol(_kol, _campaignId, investedCampaignDetails[_kol][_campaignId].leftOverInvestment);
    }
   ...
   */
    function _blackListedKOLFundsHandling(
        ...
        if (balance < _reward) {
            ...
            if (balance > 0) {
                console.log("802 balance", balance);
                _safeTransfer(preSale, msg.sender, balance);
                _reward = _reward - balance;
                // console.log("_reward", _reward);
                console.log("_reward", _reward);
                campaigns[_campaignId].campaignData.presaleAmount -= balance * convertInto18;
                eligibleRwd = balance;
            }
        } else {
            ...
            console.log("811 _reward", _reward);
        ...
        if (campaigns[_campaignId].campaignData.preSaleToken != _USDT) {
            ...
            //         }
            //     }
            uint256 leftOver = investedCampaignDetails[msg.sender][_campaignId].leftOverInvestment;
            _safeTransfer(_USDT, msg.sender, (leftOver * 10**IERC20(_USDT).decimals()) / 1e18);
        }

        investedCampaignDetails[msg.sender][_campaignId].investedAmt = 0;

        emit ClaimPreSaleTokensBlackListed(
            _campaignId,
            eligibleRwd,
            investedCampaignDetails[msg.sender][_campaignId].leftOverInvestment,
            preSale,
   ...
   */
    ...
    function setNewAdmin(address _newAdmin) external onlyAdmin {
        require(_newAdmin != address(0), "NA");
        admin = _newAdmin;
    }
    ...
    function withdrawPenaltyAndUpdateAddress(address _newAddress, bool wantToWithdraw) external onlyAdmin {
        if (!wantToWithdraw) {
            _stakingContract = _newAddress;
        } else {
            _safeTransfer(_USDT, msg.sender, (penaltyAmount * (10**IERC20(_USDT).decimals())) / 1e18);
        }
        emit WithdrawPenaltyAndUpdateAddress(_newAddress, penaltyAmount);
    }
    ...
    function withdawPresaleByCampaignOwner(uint256 _campaignId) external {
        ...
        uint256 totalPresaleDeposited = (campaignDetails.campaignData.enteredInvestmentAgainstMarketingBudget *
            (marketingBudget / campaignDetails.campaignData.requiredInvestment)) -
            campaigns[_campaignId].campaignData.stakingAmount;
        ...
        uint256 transferPresale = totalPresaleDeposited - shareOfKolsInTermsToken;
...
interface iProtoKol {
    ...
    event WithdrawPenaltyAndUpdateAddress(address _newAddress, uint256 penaltyAmount);
