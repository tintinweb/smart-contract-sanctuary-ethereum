...
/*
    //  IK TEST CONTRACT
    //  VERSION 20220808
    //  PRESALE:  8-15-22, 0.04 ETH
    //  PUBLIC:   8-19-22, 0.06 ETH
    //  ASCII art in readme doc
    //  NOTES FOR CLIENT:
    //  Added a wallet limit of 1000, as that's way more than s/b needed.
    //  Added a transaction limit of 200, as we don't want any weird issues.
    //  Mention that: Image copyright passes to NFT holder.
*/
...
contract IKTEST is Ownable, ERC721A, PaymentSplitter {

    // DETAILS TBD ON THIS DROP...
    uint public MAXSUPPLY = 10000;          // Hard-coded in setMaxSupply() also.
    uint public THEMINTPRICE = 0.01 ether; // Test price. Presale 0.04, Public 0.06
    uint public WALLETLIMIT = 1000; // No practical limit
    uint public TRANSACTIONLIMIT = 200; // per mint tx
        ...
        PaymentSplitter(distro, distro_shares)
    {
        ...
        accounts[teamclaim[0]] = Account( 100, 0, 1 ); // IK 100
        accounts[teamclaim[1]] = Account( 100, 0, 1 ); // IK 100
    ...
    function mint(uint _amount) external payable noReentrant {
        ...
        require( _amount <= TRANSACTIONLIMIT, "Error: You would exceed the per-transaction limit.");
