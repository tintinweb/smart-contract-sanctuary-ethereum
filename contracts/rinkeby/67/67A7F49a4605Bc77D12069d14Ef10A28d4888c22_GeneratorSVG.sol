...
contract GeneratorSVG is Ownable {
    using Strings for uint256;

    mapping(string => bytes[]) public traitImages;
    string[] public traitTypes; //order here is order of layers

    constructor() {
        traitTypes.push("bodies");
        traitTypes.push("accessories");
        traitTypes.push("heads");
        traitTypes.push("glasses");
        traitTypes.push("stamps");
    }
    ...
    function changeTraitTypes(string[] memory _traitTypes) external onlyOwner {
        traitTypes = _traitTypes;
    }

    function traitImagesCount(string memory name)
        public
        view
        returns (uint256)
    {
        return traitImages[name].length;
    }
    ...
    function addManyTraitImages(
        string memory name,
        bytes[] calldata _backgrounds
    ) external onlyOwner whenPartsNotLocked {
        uint256 bgLength = _backgrounds.length;
        for (uint256 i = 0; i < bgLength; i++) {
            _addTraitImages(name, _backgrounds[i]);
        }
    }
    ...
    function addTraitImages(string memory _name, bytes calldata _data)
        external
        onlyOwner
        whenPartsNotLocked
    {
        _addTraitImages(_name, _data);
    }
    ...
    function tokenURI(uint256 tokenId, uint48[] memory seed)
        external
        view
    ...
    function dataURI(uint256 tokenId, uint48[] memory seed)
        public
        view
        returns (string memory)
    {
    ...
    function genericDataURI(
        ...
        NFTDescriptor.TokenURIParams memory params = NFTDescriptor
            .TokenURIParams({
                ...
                background: backgrounds[seed[0]]
            });
        return NFTDescriptor.constructTokenURI(params, palettes);
    }

    function generateSVGImage(uint48[] memory seed)
        external
        view
        returns (string memory)
    {
    ...
    function _addTraitImages(string memory name, bytes calldata _data)
        internal
    {
        traitImages[name].push(_data);
    }

    function _getPartsForSeed(uint48[] memory seed)
        internal
        view
        returns (bytes[] memory)
    {
        uint256 traitsLength = traitTypes.length;
        bytes[] memory _parts = new bytes[](traitsLength);
        for (uint256 c = 0; c < traitsLength; c++) {
            _parts[c] = traitImages[traitTypes[c]][seed[c + 1]];
        }
        return _parts;
    }
        ...
        returns (uint48[] memory)
    {
        ...
        uint48[] memory seed = new uint48[](traitTypes.length + 1);
        seed[0] = uint48(uint48(pseudorandomness >> (0)) % backgroundCount());
        for (uint256 c = 0; c < traitTypes.length; c++) {
            seed[c + 1] = uint48(
                uint48(pseudorandomness >> ((c + 1) * 48)) %
                    traitImagesCount(traitTypes[c])
            );
        }
