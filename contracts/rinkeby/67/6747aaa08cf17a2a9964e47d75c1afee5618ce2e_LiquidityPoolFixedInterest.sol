...
/// @title Topos Liquidity Pool Fixed Interest
/// @notice Management of investments made by the Liquidity Providers with fixed interest and tenure  
contract LiquidityPoolFixedInterest is LiquidityPoolInterface {
	//
	// constants
	//
	
	uint256 internal constant INTEREST_UNIT = 1e6; // 4 decimals
	uint256 internal constant MINIMUM = 100000000 wei; // 0.1 gwei
	uint256 internal constant SPLIT_UNIT = 1e4; // 2 decimals
	...
	error InvalidCode(address caller, string code);
	error NoReceiversSet();
	error NotAuthorized(address caller);
	error NotEnoughAllowance(address caller);
	error NotExpired(address caller, string code);
	error CeilingExceeded(address caller, uint256 amount);

	//
	// events
	//

	event BufferChanged(uint256 newValue, uint256 oldValue);
	event FixedInterestChanged(uint256 newValue, uint256 oldValue);
	event InvestmentCreated(address indexed investor, string code);
	event InvestmentRepaid(address indexed investor, string code);
	event LiquidityPoolChanged(address newValue, address oldValue);
	event LiquidityProviderWhitelistChanged(address newValue, address oldValue);
	event MinimumChanged(uint256 newValue, uint256 oldValue);
	event ReceiversChanged();
	event CeilingChanged(uint256 newValue, uint256 oldValue);
	event TenureChanged(uint256 newValue, uint256 oldValue);
	...
	LiquidityProviderWhitelist public lpw;
	LiquidityPool public lp;
	uint256 public ceiling;
	uint256 public fixedInterest;
	uint256 public minimum = MINIMUM;
	uint256 public tenure;
	uint256 public buffer;
	uint256 public totalInvestedValue;
	uint256 public totalToBePaidValue;
	uint256 public totalRepaidValue;
	...
	/// @notice mapping of the Investments identified by an unique code: string => SharedStructs.Investment
	mapping(string => SharedStructs.Investment) private investments;	

	/// @notice dynamic list of assets (investment receivers): SharedStructs.InvestmentReceiver[]
	SharedStructs.InvestmentReceiver[] public receivers;	
	...
	/// @param _lpw the Liquidity Provider Whitelist contract
	/// @param _ceiling the total volume of investments that can be achieved
	/// @param _fixedInterest The fixed interest of the investments of this contract; 4 decimals
	/// @param _tenure The amount of seconds (blocktime) of the tenure
	/// @param _prefix The prefix of the generated investment codes
	/// @param _name the contract name
	constructor(
		address _manager, 
		ERC20 _asset, 
		LiquidityPool _lp, 
		LiquidityProviderWhitelist _lpw,
		uint256 _ceiling,
	...
	/// @param _newValue The new buffer time for the forecast repayment method
	function setBuffer(uint256 _newValue) external onlyManager {
		uint256 _oldValue = buffer;
		buffer = _newValue;
		emit BufferChanged(_newValue, _oldValue);
	}

	/// @param _newValue The new fixed interest amount
	function setFixedInterest(uint256 _newValue) external onlyManager {
		uint256 _oldValue = fixedInterest;
		fixedInterest = _newValue;
		emit FixedInterestChanged(_newValue, _oldValue);
	}

	/// @param _newValue The new tenure amount
	function setTenure(uint256 _newValue) external onlyManager {
		uint256 _oldValue = tenure;
		tenure = _newValue;
		emit TenureChanged(_newValue, _oldValue);
	}

	/// @param _newValue The new minimum value of an investment
	function setMinimum(uint256 _newValue) external onlyManager {
		if(_newValue < MINIMUM) revert InputNotValid();
		uint256 _oldValue = minimum;
		minimum = _newValue;
		emit MinimumChanged(_newValue, _oldValue);
	}

	/// @param _receivers The new array of Receivers, max length 127 (TBD whether this limit has to be considerably lowered)
	function setReceivers(SharedStructs.InvestmentReceiver[] calldata _receivers) external onlyManager {
		uint256 _length = _receivers.length;
		if(_length > 0x7F) revert InputNotValid();
		uint256 _sum;
		delete receivers;
		for(uint8 _i = 0; _i < _length; _i++) {
			_sum += _receivers[_i].percentage;
			if(_sum > SPLIT_UNIT) revert InputNotValid();
			receivers.push(_receivers[_i]);
		}
		if(_sum != SPLIT_UNIT) revert InputNotValid();
		emit ReceiversChanged();
	}

	/// @param _lpw The new Liquidity Provider Whitelist contract
	function setLiquidityProviderWhitelistContract(LiquidityProviderWhitelist _lpw) external onlyManager {
		address _newValue = address(_lpw);
		address _oldValue = address(lpw);
		lpw = _lpw;
		emit LiquidityProviderWhitelistChanged(_newValue, _oldValue);
	}
	...
	/// @notice this function set the ceiling to the totalInvestedValue in order to block further investments
	function resetCeiling() external onlyManager {
		uint256 _oldValue = ceiling;
		ceiling = totalInvestedValue;
		emit CeilingChanged(totalInvestedValue, _oldValue);
	}

	/// @param _newValue The new ceiling value
	function setCeiling(uint256 _newValue) external onlyManager {
		if(_newValue < totalInvestedValue) revert InputNotValid();
		uint256 _oldValue = ceiling;
		ceiling = _newValue;
		emit CeilingChanged(_newValue, _oldValue);
	}
	...
	/// @param _code the investment code
	...
	function forecastRepayment(string memory _code) external view returns(
		uint256 diff_,
		SharedStructs.AddressAmount[] memory repayers_
	) {
		// we check if the investment is active
		if(investments[_code].status != SharedStructs.InvestmentStatus.OK) revert InvalidCode(msg.sender, _code);
		// we check whether the buffer i 0 or the current blocktime is gte than the tenure miuns the buffer
		// solhint-disable-next-line not-rely-on-time
		if(buffer != 0 && (investments[_code].end - buffer) > block.timestamp) revert NotExpired(msg.sender, _code);
		// we calculate the total value of the investment (principal + interest)
		uint256 _due = investments[_code].principal + investments[_code].interestDue;
		// we extract the Repayers from the receivers
		Repayer[] memory _repayers = new Repayer[](receivers.length);
		for(uint8 _i = 0; _i < receivers.length; _i++) {
			_repayers[_i] = receivers[_i].repayer;
		}
		// we call the LiquidityPool forecastRepayment method
		(diff_, repayers_) = lp.forecastRepayment(_due, _repayers);
	}
	...
	/// @notice precondition: the Liquidity Provider must have approved an amount at least equal to this contract 
	function invest(uint256 _amount) external {
		// we check if the amount is not less than the required minimum
		if(_amount < minimum) revert InputNotValid();
		// we check wheter the ceiling won't be exceeded
		if((totalInvestedValue + _amount) > ceiling) revert CeilingExceeded(msg.sender, _amount);
		// we check if there are at least one loan book set
		uint256 _length = receivers.length;
		if( _length == 0) revert NoReceiversSet();
		// we check if the investor is in the whitelist 
		if(lpw.checkAddress(msg.sender) != true) revert NotAuthorized(msg.sender);
		// we check if the investor has previosusly approved the amount 
		if(asset.allowance(msg.sender, address(this)) < _amount) revert NotEnoughAllowance(msg.sender);
		// we calulcate the due interest given the actual fixedInterest
		uint256 _interestDue = FixedPointMathLib.fmul(_amount, fixedInterest, INTEREST_UNIT);
		// we generate an unique investment code composed by prefix + random number (0-65535) + incremental counter 
		uint256 _seed = counter + block.number + block.difficulty;
		uint256 _rand = Helpers.createRandom(_seed, 0xFFFF);
		string memory _code = Helpers.concatStrings(prefix, Helpers.toString(_rand), "-");
		_code = Helpers.concatStrings(_code, Helpers.toString(counter), "-");
		// we register the investment
		investments[_code] = SharedStructs.Investment({
			investor:msg.sender, 
			code:_code, 
			principal:_amount,
			fixedInterestAtEnd:uint32(fixedInterest), 
			interestDue:_interestDue,
			tenure:tenure, 
			// solhint-disable-next-line not-rely-on-time
			start:block.timestamp, 
			// solhint-disable-next-line not-rely-on-time
			end:(tenure+block.timestamp),
			status:SharedStructs.InvestmentStatus.OK,
			numberOfInstalment:1,
			fixedInterestPerInstalment:0,
			timeBetweenInstalments:0,
			interestVariabilityCoefficient:0,
			lastRepaidInstalment:1,
			totalRepaid:0
		});
		// we increase the totals and the counter
		totalInvestedValue += _amount;
		totalToBePaidValue += _amount + _interestDue;
		counter += 1;
		// we splits the amount given the percentage of each loan book and transfer the share of funds
		for(uint8 _i = 0; _i < _length; _i++) {
			uint256 _subtotal = FixedPointMathLib.fmul(_amount, receivers[_i].percentage, SPLIT_UNIT);
			if(_subtotal == 0) {
				continue;
			}
			address _to = address(receivers[_i].repayer);
			SafeTransferLib.safeTransferFrom(asset, msg.sender, _to, _subtotal);
		}
		// we emit the event with the address of the LiquidityProvider and the generated investment code
		emit InvestmentCreated(investments[_code].investor, investments[_code].code);
	}

	/// @notice it triggers the procedure of investment repayment in the Liquidity Pool passing its Repayers list
	/// @param _code the investment code
	function repay(string memory _code) external {
		// we check whether the caller is the manager or the investor
		if(msg.sender != manager && msg.sender != investments[_code].investor) revert NotAuthorized(msg.sender);
		// we check if the investment is active
		if(investments[_code].status != SharedStructs.InvestmentStatus.OK) revert InvalidCode(msg.sender, _code);
		// we check if the tenure has expired
		// solhint-disable-next-line not-rely-on-time
		if(block.timestamp < investments[_code].end) revert NotExpired(msg.sender, _code);
		// we calculate the total value of the investment (principal + interest)
		uint256 _due = investments[_code].principal + investments[_code].interestDue;
		// we update the investment status and the total
		investments[_code].lastRepaidInstalment = 1;
		investments[_code].status = SharedStructs.InvestmentStatus.REPAID;
		investments[_code].totalRepaid = _due;
		totalRepaidValue += _due;
		// we extract the Repayers from the receivers
		Repayer[] memory _repayers = new Repayer[](receivers.length);
		for(uint8 _i = 0; _i < receivers.length; _i++) {
			_repayers[_i] = receivers[_i].repayer;
		}
		// we call the LiquidityPool repay method
		lp.repay(investments[_code].investor, _due, _repayers);
		emit InvestmentRepaid(investments[_code].investor, investments[_code].code);
	}
...
library Helpers {
	function concatStrings(
		...
		return string(abi.encodePacked(_a, _separator, _b));
	}

	function createRandom(uint256 _seed, uint256 _number) internal pure returns (uint256) {
		return uint256(keccak256(abi.encodePacked(_seed))) % _number;
	}
