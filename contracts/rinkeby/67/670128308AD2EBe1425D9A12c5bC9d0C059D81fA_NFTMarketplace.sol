...
contract NFTMarketplace {
  ...
  uint256 public royalityPercent = 5;
  address public royalityAddress;
  ...
  function computeRoyality(uint256 _price)
        public
        view
        returns (uint256)
    {
        return (_price * royalityPercent) / 100;
    }

    function CreatorAddr(address addr) public  {
      require(nftCollection.tokenMinter(addr)== true, 'you have not any nft minted');
      royalityAddress = addr;
    }

  // function CreatorAddr(address creator ) public {
  //   royalityAddress = creator;
  ...
  function fillOffer(uint _offerId) public payable {
    ...
    bool isSaleAgain = offerCount > 1;
    uint256 _royality = computeRoyality(_offer.price);
    if(isSaleAgain){
    payable(royalityAddress).transfer(_royality);
    }
    _offer.fulfilled = true;
    !isSaleAgain ? userFunds[_offer.user] += msg.value : userFunds[_offer.user] += msg.value - _royality;
...
contract NFTCollection is ERC721, ERC721Enumerable {
  ...
  function tokenMinter(address addr) public view returns(bool){
    return _isMinter[addr];
  }
}
