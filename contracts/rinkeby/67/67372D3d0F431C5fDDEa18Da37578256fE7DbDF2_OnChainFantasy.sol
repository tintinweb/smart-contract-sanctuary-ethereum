...
library OnChainFantasyRenderer {
    ...
    function getProperties(
        ...
        trait.def2 = teams[
            shuffledTeams[(((defIndex + 1) % teams.length))] - 1
        ];
    ...
    VRFConsumerBaseV2
{
    ...
    function _reroll(uint256 tokenId) internal {
        ...
        _batchHeadtokenGen[_getMetaDataBatchHead(tokenId)] = currentGen;
        _setMetaDataBatchHead(tokenId + 1);
...
abstract contract ERC721PsiBatchMetaData is ERC721Psi {
     ...
     *  The returned tokenId will remain the same after the token transfer.
     */
    function _setMetaDataBatchHead(uint256 tokenId) internal {
        _metaDataBatchHead.set(tokenId);
    }
}
