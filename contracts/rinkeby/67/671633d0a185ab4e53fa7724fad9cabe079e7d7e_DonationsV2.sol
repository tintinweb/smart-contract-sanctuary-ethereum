...
contract DonationsV2 is Ownable {
    ...
    struct Account { uint256 account_num; uint256 balance; address release_address; bool claimed;}
    ...
    function submitDonation(address payable recipient, uint256 user_id) public payable{
        ...
        uint _amount;
        _amount = msg.value;
        require(balance > msg.value, "Not enough for this Transaction");
        ...
        if(recipient == address(0x0)) {
            ...
            if(unclaimed_user == false) {
                ...
                Account memory new_account = Account(user_id,fee_processed_donation,recipient,false);
    ...
    function approveNFTForSend( address nft_contract_address, uint256 nft_token_id, address payable recipient, uint256 user_id) public {
        ERC721 token = ERC721(nft_contract_address);
        require(token.balanceOf(msg.sender) > 0, "caller must own given token");
        require(token.isApprovedForAll(msg.sender, address(this)), "You must approve this NFT with OhNo.TV for donation");
        sendNFT(recipient,user_id,nft_contract_address,nft_token_id);
    }

    function sendNFT(address payable recipient, uint256 user_id, address nft_contract_address, uint256 nft_token_id) public payable {
        ERC721 token = ERC721(nft_contract_address);
        require(msg.value >= 0, "Not enough for transaction");
        token.safeTransferFrom(msg.sender, recipient, nft_token_id, "");
    }
