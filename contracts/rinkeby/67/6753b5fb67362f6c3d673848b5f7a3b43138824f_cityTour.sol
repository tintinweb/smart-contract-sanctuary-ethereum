...
contract cityTour is Ownable, ERC721A {
   ...
   function buy(uint _count) public payable{
    ...
    function buyPreMint(bytes32[] calldata _merkelProof, uint _count) public payable {
        ...
        // check wl limit
        require(inkedClaimed[msg.sender] < 2, "Pre mint limit reached!");
        // sender hex
        bytes32 senderHex = keccak256(abi.encodePacked(msg.sender)); 
        // check wl 
        require(MerkleProof.verify(_merkelProof, wlProof, senderHex), "Sender not dicked!");

        // check wl mint
        require(isWlMintOpen == true, "Wait for dicked open");
        require(_count > 0, "mint at least one token");
    ...
    function sendGifts(address[] memory _wallets) public onlyOwner{
      ...
      function isSaleActive() public view returns(uint){
      require(isPublicOpen == true,"Sale not Started Yet.");
        return 1;
     }

     function isPreSaleActive() public view returns(uint){
    ...
    function publicSaleStatus(bool temp) external onlyOwner {
        isPublicOpen = temp;
    }

    function whiteListSaleStatus(bool temp) external onlyOwner {
        isWlMintOpen = temp;
    }
