...
import "./IStarCards.sol";

contract StarCardsWrapper is ERC721, Ownable {
  IStarCards immutable public STAR_CARDS;
  ...
  constructor(address starCards) ERC721("Star Cards", "STAR") {
    STAR_CARDS = IStarCards(starCards);
  }

  modifier onlyApproved() {
    require(
      STAR_CARDS.isApprovedForAll(msg.sender, address(this)),
  ...
  function wrap(uint256 tokenId) public onlyApproved {
    _wrap(tokenId);

    totalSupply += 1;
  }

  function wrap(uint256[] memory tokenIds) public onlyApproved {
