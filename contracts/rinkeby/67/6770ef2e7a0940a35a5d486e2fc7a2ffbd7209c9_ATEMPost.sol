...
contract ATEMPost is ERC1155Pausable, Ownable, ReentrancyGuard {
    ...
    constructor(
        ...
        address initialPlatformFeeAccount) ERC1155(uri) {
            name = _name;
            symbol = _symbol;
    ...
    modifier onlyCreator(uint256 _id) 
    {
        require(
            ...
            "ATEMPost#onlyCreator: only the token creator can do this"
        );
        _;
    }

    modifier onlyCreatorBatch(uint256[] memory _ids) 
    {
        for (uint256 i = 0; i < _ids.length; i++) {
            require(
                _isCreator(_ids[i], _msgSender()),
                "ATEMPost#onlyCreatorBatch: only the token creator can do this"
            );
        }
        _;
    }
     ...
     */
    function setCreator(uint256 _id, address _to) public onlyCreator(_id) {
        require(
            _to != address(0),
            "ATEMPost#setCreator: invalid address"
        );
    ...
    /**
     * @dev Mint a single token id and set its price. Only creator can do this
     */
    function mint(
        address to,
        uint256 id,
        uint256 amount,
        uint256 price
    ) public nonReentrant() onlyCreator(id) 
    {  
        require(
            _supply[id] + amount <= id.tokenMaxSupply(),
            "ATEMPost#mint: minting would exceed total supply"
        );

        if (_supply[id] == 0) {
            require(
                id.tokenIndex() == _totalCreations[_msgSender()],
                'ATEMPost#mint: token index is not the total number of creations'
            );
            _totalCreations[_msgSender()]++;
            emit PostCreated(_msgSender(), id, to, price);
        }
    ...
    /**
     * @dev Mint a batch of token ids and set their prices. Only creator can do this
     */
    function mintBatch(
        address to,
        uint256[] memory ids,
    ...
    ) public nonReentrant() onlyCreatorBatch(ids)  
    {
        require(
            (ids.length == amounts.length) && (ids.length == prices.length),
            "ATEMPost#mintBatch: ids, amounts and prices must have the same length"
        );

        uint256 _userTotalCreations = _totalCreations[_msgSender()];
        for (uint256 i = 0; i < ids.length; i++) {
            require(
                _supply[ids[i]] + amounts[i] <= ids[i].tokenMaxSupply(),
                "ATEMPost#mintBatch: some id(s) in this batch would exceed their total supply"
            );

            if (_supply[ids[i]] == 0) {
                require(
                    ids[i].tokenIndex() == _userTotalCreations,
                    'ATEMPost#mintBatch: token index is not the total number of creations'
                );
                _userTotalCreations++;
                emit PostCreated(_msgSender(), ids[i], to, prices[i]);
            }

            if (prices[i] != _price[ids[i]]) {
                _setPrice(ids[i], prices[i]);
            }
        }
        _totalCreations[_msgSender()] = _userTotalCreations;
    ...
    /**
     * @dev Purchase a single token id
     */
    function purchase(
        address to,
        uint256 id,
    ...
    ) public payable nonReentrant()
    {
        require(
             _supply[id] + amount <= id.tokenMaxSupply(),
            "ATEMPost#purchase: purchasing would exceed total supply"
        );
        require(
            _supply[id] > 0,
            "ATEMPost#purchase: cannot purchase a token not created yet"
        );

        // Compute the actual value
        uint256 totalPriceBeforePlatformFee = _price[id] * amount;
        uint256 platformFee = computePlatformFee(totalPriceBeforePlatformFee);
        uint256 totalPrice = totalPriceBeforePlatformFee + platformFee;

        require(
            totalPrice == msg.value,
            "ATEMPost#purchase: incorrect payment value"
        );
        ...
        require(
            ...
            "ATEMPost#purchase: not enough balance"
        );

        address creator_ = creator(id);
        _withdraw(creator_, totalPriceBeforePlatformFee);
    ...
    /**
     * @dev Purchase a batch of token ids
     */
    ...
    //         "ATEMPost#purchaseBatch: ids and amounts must have the same length"
    //     );

    //     uint256 totalPriceBeforePlatformFee = 0;
    //     for (uint256 i = 0; i < ids.length; i++) {
    //         require(
    //             _supply[ids[i]] + amounts[i] <= ids[i].tokenMaxSupply(),
    //             "ATEMPost#purchaseBatch: some id(s) in this batch would exceed their total supply"
    //         );
    //         require(
    //             _supply[ids[i]] > 0,
    //             "ATEMPost#purchaseBatch: some id(s) in this batch have not been created yet"
    //         );

    //         // Compute the actual value
    //         totalPriceBeforePlatformFee += _price[ids[i]] * amounts[i];

    //         _supply[ids[i]] += amounts[i];
            
    //         emit PostPurchased(creator(ids[i]), ids[i], _msgSender(), to, _price[ids[i]], amounts[i]);
    //     }
    ...
    //     require(
    //         totalPrice == msg.value,
    //         "ATEMPost#purchaseBatch: incorrect payment value"
    //     );

    //     for (uint256 i = 0; i < ids.length; i++) {
    //         _withdraw(creator(ids[i]), totalPriceBeforePlatformFee);
    //     }
    //     _withdraw(_platformFeeAccount, platformFee);
