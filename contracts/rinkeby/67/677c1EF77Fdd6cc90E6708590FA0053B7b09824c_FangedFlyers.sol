...
contract FangedFlyers is ERC721Psi, Ownable, Pausable {
    uint256 public mint1Price = 0.005 ether;
    uint256 public mint5Price = 0.025 ether;
    uint256 public mint10Price = 0.05 ether;
    uint256 public mint50Price = 0.25 ether;
    uint256 public constant maxFlyers = 10;

    string public constant baseURI = "ipfs://QmSxPbk5Tw51obzrJERSobo72Y5UW3jo3Sfg8d1xqs3YXu/";

    constructor()
        ERC721Psi ("FangedFlyers", "FANGED") {}

    function mintFlyer() external payable whenNotPaused {
        require(mint1Price <= msg.value, 'LOW_ETHER');
        unchecked { require(totalSupply() + 1 <= maxFlyers, 'MAX_REACHED'); }
        _safeMint(msg.sender, 1);
    }

    function mint5Flyers() external payable whenNotPaused {
        require(mint5Price <= msg.value, 'LOW_ETHER');
        unchecked { require(totalSupply() + 5 <= maxFlyers, 'MAX_REACHED'); }
        _safeMint(msg.sender, 5);
    }

    function mint10Flyers() external payable whenNotPaused {
        require(mint10Price <= msg.value, 'LOW_ETHER');
        unchecked { require(totalSupply() + 10 <= maxFlyers, 'MAX_REACHED'); }
        _safeMint(msg.sender, 10);
    }

    function mint50Flyers() external payable whenNotPaused {
        require(mint50Price <= msg.value, 'LOW_ETHER');
        unchecked { require(totalSupply() + 50 <= maxFlyers, 'MAX_REACHED'); }
