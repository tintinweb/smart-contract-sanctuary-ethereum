...
contract ERC721A is Context, ERC165, IERC721, IERC721Metadata {
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        return bytes(hiddenuri).length != 0 ? string(abi.encodePacked(hiddenuri, tokenId.toString())) : '';
    }

    /**
     * @dev Base URI for computing {tokenURI}. If set, the resulting URI for each
     * token will be the concatenation of the `hiddenuri` and the `tokenId`. Empty
...
contract MYBODY is ERC721A, Ownable, ReentrancyGuard {
    ...
    string private hiddenuri = "ipfs://QmTcr48HjNSo2uP3gFAsReMLCYSzHD4se68sxMZHA92sHu/1.json";
    ...
    constructor(string memory hiddenurimeta) ERC721A("My Body", "YDOM") {
        hiddenuri = hiddenurimeta;
        _safeMint(msg.sender, MAX_FREE_MINT_PER_ADDR);
    }

    function _hiddenuri() internal view override returns (string memory) {
        return hiddenuri;
    }
        ...
        returns (string memory)
    {
        ...
        return
            string(
                abi.encodePacked(hiddenuri, "/", _tokenId.toString(), ".json")
            );
    }
