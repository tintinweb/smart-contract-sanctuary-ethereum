...
contract FutureQuest is ERC1155, ERC2981Base, Ownable {
  using Strings for uint256;
  using SafeMath for uint256;

  address public _couponSigner; // Address of the wallet that generated the coupons
  uint256 public _mintPrice = 0.05 ether; // Presale mint price is free
  ...
  mapping(address => bool) public _isAllowedlistAddress;

  mapping(address => MintTypes) public _addressToMinted;
  ...
  constructor(address couponSigner) ERC1155("") {
  ...
  function mintPresale(
    ...
    require(
      quantity + _addressToMinted[_msgSender()]._presaleMintsByAddress <
        allotted + 1,
      "Exceeds Max Allotted"
    );
    // Increment number of presale tokens minted by wallet
    _addressToMinted[_msgSender()]._presaleMintsByAddress += quantity;
    // Mint Reserve Tokens
   ...
   */
  function create(uint256 maxSupply) external onlyOwner returns (uint256) {
  ...
  function setPremintActive(bool active) external onlyOwner {
    premintActive = active;
  }
  ...
  function setAllowedlistAddresses(address[] calldata wAddresses)
    public
    onlyOwner
  {
    for (uint256 i = 0; i < wAddresses.length; i++) {
      _isAllowedlistAddress[wAddresses[i]] = true;
    }
  }
