...
contract MinerApePool is ERC721A, Ownable, ReentrancyGuard {
    ...
    function refund(address to, uint nftTokenId) external nonReentrant {
        ...
        require(
            address(this).balance >= nftRefundAmount[nftTokenId],
            "Not enough funds on the contract"
        );

        safeTransferFrom(to, owner(), nftTokenId);

        (bool success, ) = to.call{value: nftRefundAmount[nftTokenId]}("");
    ...
    function checkRefundability(uint nftTokenId) external view returns (uint256) {
        require(ownerOf(nftTokenId) == _msgSender(), "This NFT does not belong to you");
        require(nftMinter[nftTokenId] == _msgSender(), "You are not the minter");
        require(nftRefundAmount[nftTokenId] > 0, "Non-refundable NFT");
