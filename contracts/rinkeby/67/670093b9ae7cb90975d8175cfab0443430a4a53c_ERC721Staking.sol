...
contract ERC721Staking is Ownable, ReentrancyGuard {
    ...
    function stake(uint256[] calldata _tokenIds) external nonReentrant {
        ...
        if(len > 0){
            ...
            staker.timeOfLastStake = block.timestamp;
    ...
    {
        uint256[] memory tokenIds = new uint256[](_stakedTokenIds.length);
        uint256 index = 0;
        for(uint256 i = 0; i < stakedTokenIds.length; i++){
            if(stakerAddress[stakedTokenIds[i]] == _user){
                tokenIds[index] = stakedTokenIds[i];
                index++;
            }
        }
        return (tokenIds, availableRewards(_user));
    }
