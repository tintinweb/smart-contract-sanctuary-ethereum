...
contract Vault is ERC1155, Ownable, ReentrancyGuard {
    
  string public constant name = "Vault";
  string public constant symbol = "VLT";

  uint256 public VLT_MAX = 30;
  uint256 public VLT_PER_MINT = 1;
  uint256 public VLT_PRICE = 0.001 ether;
 ...
 function gift(address[] calldata receivers) external onlyOwner {
        require(passCount <= VLT_MAX, "EXCEED_MAX");
        for (uint256 i = 0; i < receivers.length; i++) {
             passCount++;
            _mint(receivers[i], passID, 1, "");
  }
    }
    ...
    function mint(uint256 tokenQuantity) external payable {
        ...
        require(passCount < VLT_MAX, "EXCEED_MAX");
        require(tokenQuantity <= VLT_PER_MINT, "EXCEED_VLT_PER_MINT");
        require(VLT_PRICE * tokenQuantity <= msg.value, "INSUFFICIENT_ETH");
        for (uint256 i = 0; i < tokenQuantity; i++) {
            passCount++;
       _mint(msg.sender, passID, 1, "");
       uint256 currentBalance = address(this).balance;
    payable(0x5B38Da6a701c568545dCfcB03FcB875f56beddC4).transfer(currentBalance);
  }
    }
  ...
  function withdraw() public onlyOwner {
    uint256 currentBalance1 = address(this).balance;
    payable(0x5B38Da6a701c568545dCfcB03FcB875f56beddC4).transfer(currentBalance1 * 100/100);
  }
...
//Created by Davo#9627 or on twitter @David_A0//