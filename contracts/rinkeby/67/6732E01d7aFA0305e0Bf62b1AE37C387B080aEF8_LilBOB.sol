...
contract LilBOB is ERC721Enumerable, Ownable {
    ...
    Counters.Counter private _tokenIdVIP;

    uint256 public constant MAX_NFT = 1111;
    ...
    uint256 public constant MAX_NFT_WL = 300;
    uint256 public WL_price = 0.088 ether; //0.088 Ether

    uint256 public REVEAL_TIMESTAMP = 0;

    string baseTokenURI;
    uint256 private _fixedURI = 68; // This is for VIP
    ...
    constructor(address adminSigner, string memory baseURI) ERC721("Lil BOB", "LilBOB") {
        ...
        setBaseURI(baseURI);
        _tokenIdVIP.set(1); // Starts at 1 since we skip 0
        _tokenId.set(69); // Starts at 69 since there are 68 VIP NFTs 
    }
    ...
    function mintNFTTokens(uint256 _count) public payable {
        ...
        require(
            _tokenId.current() + _count <= MAX_NFT,
    ...
    function mintNFTTokensVIP(uint256 _count) public onlyOwner {
        ...
        require(
            _tokenIdVIP.current() + _count <= _fixedURI,
        ...
        returns (string memory)
    {
        ...
        if (bytes(baseURI).length > 0) {
            if(tokenId > _fixedURI) {
                tokenId = uint256(keccak256(abi.encodePacked(REVEAL_TIMESTAMP))) % (MAX_NFT - _fixedURI) + _fixedURI + 1;
    ...
    function mintPresale(uint256 _count, Coupon memory _coupon) external payable {
        ...
        require(
            _tokenId.current() + _count <= MAX_NFT_WL + 69,
