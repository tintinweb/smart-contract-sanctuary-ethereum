...
interface IPartnerContract { // todo rename for deploy
  function ownerOf(uint256 tokenId) external returns (address);
}

// todo: rename contract (AxB), constructor, file name
contract Lock is ERC1155, ERC2981, Ownable {
  ...
  // This is redundant due to a 1:1 claim with each VSP token but nice to have for good measure
  ...
  // todo: add real partner contract hardcoded before deploy

  mapping(string => bool) private _usedNonces;

  // https://gateway.pinata.cloud/ipfs/QmcFizF4xELLTT8oTmTWteL58e6vjvxtx1WzS6PgEYJXjz/{id}
  constructor(address _partnerContract) ERC1155("") {
    // partnerContract = ERC721(_partnerContract);
    partnerContract = _partnerContract;

  // constructor(address _systemAddress) ERC1155('') {
  ...
  function mintMyTrophies(uint256[][] calldata _counts, string calldata _nonce, bytes calldata _signature) external {
    ...
    IPartnerContract partner = IPartnerContract(partnerContract);
    ...
    for(uint256 i; i < _counts.length;) {
      ...
      for(uint256 n; n < _counts[i].length;) {
        ...
        if (!hasIdBeenUsed(id) && partner.ownerOf(id) == msg.sender) {
  ...
  */
  ...
  /**
  @notice Boilerplate to support ERC1155 and ERC2981
  */
