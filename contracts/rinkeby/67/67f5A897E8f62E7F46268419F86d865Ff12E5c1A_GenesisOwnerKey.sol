...
contract GenesisOwnerKey is ERC721A, IGenesisOwnerKey, Ownable {
    ...
    function mintToPool() external onlyOwner {
        ...
        require(tierSupply > totalSupply(), "GenesisOwnerKey: ALREADY_MINT_TO_POOL");
        uint mintAmount = tierSupply - totalSupply();
    ...
    function _beforeTokenTransfers(
        ...
        // If transfer, then checking nftPool address
        if (nftPoolAddress != to) {
            require(
                balanceOf(to) + quantity <= maxOwnLimit,
                "GenesisOwnerKey: Member Maximum Limit"
            );
        }
        // If transferred from a user, check trading flag. 
        else if (nftPoolAddress != from) {
            require(tradingAllowed, "GenesisOwnerKey: user transfer coming soon");
        }
    }
...
interface IGenesisOwnerKey {
    ...
    event MintToPool(address indexed operator, uint quantity);
}
