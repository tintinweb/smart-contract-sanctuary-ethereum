...
contract FractonTokenFactory is IFractonTokenFactory {
  ...
  function createCollectionPair(
    ...
    address newFFTContract = Create2.deploy(
      0,
      salt,
      FractonFFTHelper.getBytecode(name, symbol, FractonSwap)
    );
...
library FractonFFTHelper {
  function getBytecode(
    ...
    return abi.encodePacked(bytecode, abi.encode(name, symbol, swap));
  }
}
...
contract FractonFFT is ERC20, IFractonFFT {
  ...
  function swapmint(uint256 amount, address to)
    ...
    returns (bool)
  {
    require(msg.sender == swapaddress, 'Fracton: caller is not swap');
    _mint(to, amount);
    return true;
  }
