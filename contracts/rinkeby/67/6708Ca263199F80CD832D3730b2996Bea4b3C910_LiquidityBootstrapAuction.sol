...
contract LiquidityBootstrapAuction is ReentrancyGuard, Ownable {
    ...
    mapping(address => bool) public usdcWithdrawnAfterDepositEnds;
    ...
    function depositASTO(uint256 amount) external nonReentrant {
    ...
    function withdrawableUSDCAmount(address recipient) public view returns (uint256) {
        ...
        // USDC can only be withdrawn once after usdcDepositEndTime()
        // Withdrawable USDC amount on Day 6: half of deposited USDC amount
        // Withdrawable USDC amount on Day 7: linear decrease from half of deposited USDC amount to 0
    ...
    function addLiquidityToExchange(address router, address factory) external nonReentrant onlyOwner {
        ...
        usdc.approve(router, totalDepositedUSDC);
        asto.approve(router, totalDepositedASTO);
        ...
        (uint256 amountA, uint256 amountB, uint256 moreLPAmount) = IUniswapV2Router02(router).addLiquidity(
        ...
        uint256 totalASTOSent = isReserved ? amountB + astoSent : amountA + astoSent;
        uint256 totalUSDCSent = isReserved ? amountA + usdcSent : amountB + usdcSent;
        emit LiquidityAdded(totalASTOSent, totalUSDCSent, lpTokenAmount);
    }

    function claimLPToken() external nonReentrant {
        require(currentTime() >= lpTokenReleaseTime(), "claim not enabled");

        uint256 claimable = claimableLPAmount(msg.sender);
        require(claimable > 0, "all claimed");
        lpClaimed[msg.sender] += claimable;

        require(IUniswapV2Pair(liquidityPair).transfer(msg.sender, claimable), "insufficient ASTO balance");
    }
    ...
    function claimableRewards(address recipient) public view returns (uint16, uint256) {
        ...
        uint16 elapsedWeeks = uint16(elapsedTime / (SECONDS_PER_DAY * 7));

        if (elapsedWeeks >= 12) {
            uint256 remaining = calculateRewards(recipient) - rewardClaimed[recipient];
            return (12, remaining);
        } else {
            uint256 amountVestedPerWeek = calculateRewards(recipient) / 12;
