...
contract Podium is Ownable, ERC721R, ReentrancyGuard, Pausable {
	...
	uint256 internal reservedQuantity_ = 77; // For dev mint
	...
	uint64 public mintListPrice = 0.0012 ether;
	uint64 public publicPrice 	= 0.0015 ether;
     ...
     */
    function fundsNeededForRefund() public view onlyTeamMember returns(uint256) {
  ...
  IERC721Enumerable
{
  ...
  */
  ...
  function _transfer(
    ...
    if(_checkPurchasePriceCurrent(tokenId) > 0){
      ...
      _tokenPurchasePriceCurrent[tokenId] = 0; // Transfer void refund
    }
  ...
  */
  ...
  /**
   * @dev Function that returns the mint 'type' of the token. These can be customizable to represent
   * multiple different types (i.e. allowList, public, dutch auction, free mint, etc.)
   *
   * Calling conditions:
   *
