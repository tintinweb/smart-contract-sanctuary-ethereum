...
contract Auction721 is AuctionStruct721,AuctionBase,TokenToAuction {
    ...
    function getMinimalNextBid(uint _auctionId) internal view returns (uint minBid){
        ...
        if(currentAuction.buyer == address(0x0)){
            ...
            // have old bid
            minBid = (currentAuction.lastBid.amount * percentNextAmount) / 10000;
        }
    }
    ...
    function calculateTotalAmount(uint _auctionId) internal returns(uint){
        Auction memory currentAuction = auctions[_auctionId];
        uint total;
        if(currentAuction.buyer == address(0x0)){
            //first bid
            total = currentAuction.minimalPrice; 
        }else{
            // have old bid
            total = (currentAuction.lastBid.amount * percentNextAmount) / 10000;
        }
        return total;
    }
    ...
    function check(bytes4 dataType,bytes memory data) public view returns(LibAucDataV1.DataV1 memory){
        LibAucDataV1.DataV1 memory aucData = LibAucDataV1.parse(data, dataType);
        return aucData;
    }

    function paytest() payable public returns(uint){
        address payable newBuyer = payable(msg.sender);
        uint amount = msg.value;
        balance[newBuyer] = amount;
        return amount;
    }
    ...
    function widthdrawn(uint _amount) internal {
...
abstract contract AuctionBase is OwnableUpgradeable{
    ...
    /// @dev latest auctionId
    uint32 public percentNextAmount;
    ...
    event PercentNextAmountChanged(uint32 oldValue, uint32 newValue);
    ...
    function __AuctionBase_init() internal initializer {
        ...
        percentNextAmount = 500; // default 5%
    ...
    function changePercentNextAmount(uint32 newValue) external onlyOwner{
        emit PercentNextAmountChanged(percentNextAmount,newValue);
        percentNextAmount = newValue;
    }
