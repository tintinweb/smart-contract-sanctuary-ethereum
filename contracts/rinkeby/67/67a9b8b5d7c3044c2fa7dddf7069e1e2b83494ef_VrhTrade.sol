...
contract VrhTrade is Ownable {
    ...
    function purchaseHero(uint256 _heroBoxId, uint256 _heroBoxCategory, address _paymentToken, uint256 _price,  uint256 _quantity, bytes32 _randomHash, bytes memory _rsv) external payable{
        ...
        if(_paymentToken == address(0)){
            ...
            require(IERC20(_paymentToken).transferFrom(msg.sender, fundAddress, amount), "token insufficient");
    ...
    function purchaseWeapon(uint256 _weaponBoxId, uint256 _weaponBoxCategory, address _paymentToken, uint256 _price,  uint256 _quantity, bytes32 _randomHash, bytes memory _rsv) external payable{
    ...
    function buy(address _nftToken, uint256 _tokenId, address _paymentToken, uint256 _price,uint256 _fee,
        ...
        address _signer = hash.toEthSignedMessageHash().recover(_rsv);
        require(_signer == _seller, "signer not match");
    ...
    function cancelSellOrder(address _nftToken, uint256 _tokenId, address _paymentToken, uint256 _price, uint256 _fee,
        ...
        address _signer = hash.toEthSignedMessageHash().recover(_rsv);
        require(_signer == msg.sender, "signer not match");
    ...
    function cancelSellHero(uint256 _tokenId, address _paymentToken, uint256 _price, uint256 _fee,
        uint256 _listingTime, uint256 _expirationTime, bytes memory _rsv) external{
    ...
    function upgradeHero(uint256 _tokenId, address _paymentToken, uint256 _price, uint256 _level, bytes memory _rsv) external payable{
