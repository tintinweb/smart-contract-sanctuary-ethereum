...
contract MyStorage is ERC721, EthUsPriceConversion, VRFConsumerBase, Ownable {
    ...
    EthUsPriceConversion(_priceFeedAddress, minimumEntreeFee)
    ...
    /**
     *  Remove all token URIs and reset the counters
     */
    function resetAllTokenURI() external onlyOwner {
        for (uint256 index = 0; index < tokenCounter; index++) {
            _tokenURIs[index] = ""; 
            
            bytes32 requestId = tokenIdTorequestId[index];
            requestIdToTokenId[requestId] = 0;
            tokenIdToBreed[index] = Breed.PUG;
            requestIdToSender[requestId] = address(0);
            requestIdToTokenURI[requestId] = "";

            tokenIdTorequestId[index] = 0;
