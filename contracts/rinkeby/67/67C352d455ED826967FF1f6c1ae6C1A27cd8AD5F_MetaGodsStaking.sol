...
contract MetaGodsStaking is Initializable, OwnableUpgradeable, PausableUpgradeable {
    ...
    function getStakedTokens(address wallet_, address collection_) external view returns (StakedTokenRes[] memory) {
        ...
        for (uint256 i = 0; i < stakedTokensByWallet[wallet_][collection_].length; ++i) {
            ...
            stakedTokens[i].maxClaimable = getYield(collection_, tokenId);
        }
    ...
    function getYield(
        ...
        if (collection_ == contractsAddressesMap["god"]) {
            if (stakedTokensData[collection_][tokenId_].stakeType == stakeTypesVariables["lowRisk"]) {
                if (_didWin(tokenId_)) {
                    yield += yield * 20 / 100;
                } else {
                    yield -= yield * 10 / 100;
            ...
            if (stakedTokensData[collection_][tokenId_].stakeType == stakeTypesVariables["highRisk"]) {
                if (_didWin(tokenId_)) {
                    yield += yield * 50 / 100;
                } else {
                    yield -= yield * 50 / 100;
    ...
    function _getYieldAndUpdateDate(
        ...
        return getYield(collection_, tokenId_);
    }
    ...
    function _exitStakeStanceMany(
        ...
        for (uint256 i = 0; i < tokenIds_.length; ++i) {
            ...
            yieldedAmount += getYield(collection_, tokenIds_[i]);
