...
contract DropV1 is ERC1155AB {
    /**
     * @notice
     *  DropV1 contract constructor
     **/
    constructor(string memory _uri, address _anotherblock)
        ERC1155AB(_uri, _anotherblock)
    {}
    ...
    function claimTo(
        ...
        /** Potential custom logic goes here
         * ...
         * ...
         */
        _mintAB(tokenId, quantity, _proof, userWallet);
    }

    function unclaimedSupply(uint256 tokenId) public view returns (uint256) {
        IAnotherblockV1.Drop memory drop = IAnotherblockV1(anotherblock).drops(
            tokenId
        );
...
contract ERC1155AB is ERC1155, ERC1155ABErrors, ERC165Storage, AccessControl {
    // AnotherblockV1 address
    address public anotherblock;

    // Role used for minting
    bytes32 public constant AB_ROLE = keccak256('AB_ROLE');
    ...
    mapping(uint256 => uint256) public sharesPerToken;

    // Stores the last Token SubID minted for a given Drop ID
    mapping(uint256 => uint256) public lastSubIdPerDrop;

    // Stores the owner address of a Token SubId for a given Drop ID
    mapping(uint256 => mapping(uint256 => address)) public subIdOwner;

    // Stores the Tokens SubIds of an user for a given Drop ID
    mapping(uint256 => mapping(address => uint256[])) public userOwnedSubId;

    // Stores the amount of tokens minted per address and per drop
    mapping(uint256 => mapping(address => uint256)) public mintedPerDrop;

    /**
     * @notice
     *  ERC1155AB contract constructor
     **/
    constructor(string memory _uri, address _anotherblock) ERC1155(_uri) {
        if (
            !ERC165Checker.supportsInterface(
                _anotherblock,
                type(IAnotherblockV1).interfaceId
            )
        ) revert IncorrectInterface();
        anotherblock = _anotherblock;
        _grantRole(DEFAULT_ADMIN_ROLE, msg.sender);
        _registerInterface(type(IERC1155AB).interfaceId);
    }
     ...
     * @param _proof : merkle tree proof used to verify whitelisted user
     * @param _userWallet : user to mint for
     */
    function _mintAB(
        uint256 _dropId,
        uint256 _quantity,
        ...
        _checkMintEligibility(drop, _quantity, _proof, _userWallet);
        ...
        if (drop.tokenInfo.tokenCount == 1) {
            _simpleMintAB(drop, drop.firstTokenIndex, _quantity, _userWallet);
        }
        // Else, if the quantity is greater than 1, calculate the amount to be minted for each tokenID
        else if (_quantity > 1) {
            _batchMintAB(drop, _quantity, _userWallet);
        }
        // Else, mint 1 token of next available tokenID to the user address
        else {
            uint256 tokenId = (drop.sold % drop.tokenInfo.tokenCount) +
                drop.firstTokenIndex;
            _simpleMintAB(drop, tokenId, _quantity, _userWallet);
        }

        lastSubIdPerDrop[_dropId] += _quantity;
        mintedPerDrop[_dropId][_userWallet] += _quantity;
        IAnotherblockV1(anotherblock).updateDropDetails(_dropId, _quantity);
    }
    ...
    function getUserSubIds(address _user, uint256 _dropId)
        ...
        returns (uint256[] memory)
    {
        return userOwnedSubId[_dropId][_user];
    }
    ...
    function withdrawAll() external onlyRole(DEFAULT_ADMIN_ROLE) {
        ...
        payable(IAnotherblockV1(anotherblock).treasury()).transfer(balance);
    }

    /**
     * @notice
     *  Increment the current token index
     *  Only AB_ROLE can perform this operation
     *
     * @param _amount : amount to increment
     */
    function incrementTokenIndex(uint256 _amount) external onlyRole(AB_ROLE) {
        currentTokenIndex += _amount;
    }

    function supportsInterface(bytes4 _interfaceId)
        public
        view
        override(ERC1155, ERC165Storage, AccessControl)
        returns (bool)
    {
        return super.supportsInterface(_interfaceId);
    }

    function _simpleMintAB(
        IAnotherblockV1.Drop memory drop,
        uint256 _tokenId,
        uint256 _quantity,
        address _userWallet
    ) internal {
        if (sharesPerToken[_tokenId] == 0) {
            dropIdPerToken[_tokenId] = drop.dropId;
            sharesPerToken[_tokenId] = drop.tokenInfo.royaltySharePerToken;
        }
        for (uint256 i = 1; i <= _quantity; ++i) {
            subIdOwner[drop.dropId][drop.sold + i] = _userWallet;
            userOwnedSubId[drop.dropId][_userWallet].push(drop.sold + i);
        }
        _mint(_userWallet, _tokenId, _quantity, '');
    }

    function _batchMintAB(
        IAnotherblockV1.Drop memory drop,
        uint256 _quantity,
        address _userWallet
    ) internal {
        uint256[] memory tokenIds = new uint256[](drop.tokenInfo.tokenCount);
        uint256[] memory amounts = new uint256[](drop.tokenInfo.tokenCount);

        for (uint256 i = 0; i < _quantity; ++i) {
            tokenIds[i % drop.tokenInfo.tokenCount] =
                ((drop.sold + i) % drop.tokenInfo.tokenCount) +
                drop.firstTokenIndex;

            amounts[i % drop.tokenInfo.tokenCount] += 1;

            subIdOwner[drop.dropId][drop.sold + i + 1] = _userWallet;

            userOwnedSubId[drop.dropId][_userWallet].push(drop.sold + i + 1);
        }
        uint256 tokenId;
        for (uint256 i = 0; i < tokenIds.length; ++i) {
            tokenId = tokenIds[i];
            if (sharesPerToken[tokenId] == 0) {
                dropIdPerToken[tokenId] = drop.dropId;
                sharesPerToken[tokenId] = drop.tokenInfo.royaltySharePerToken;
            }
        }
        _mintBatch(_userWallet, tokenIds, amounts, '');
    }
    ...
    function _checkMintEligibility(
        ...
        if (
            mintedPerDrop[drop.dropId][_userWallet] + _quantity >
            drop.maxAmountPerAddress
        ...
        if (
            ...
            bool isWhitelisted = MerkleProof.verify(
                _proof,
                drop.merkleRoot,
                keccak256(abi.encodePacked(_userWallet))
            );
    ...
    function _beforeTokenTransfer(
        ...
        if (to != address(0) && from != address(0)) {
            uint256 dropId;
            uint256[] memory previousOwnerSubIds;
            uint256[] memory newOwnerSubIds;

            for (uint256 i = 0; i < ids.length; ++i) {
                // Retrieve associated Drop ID
                dropId = dropIdPerToken[ids[i]];

                for (uint256 j = 0; j < amounts[i]; ++j) {
                    // Retrieve subId owned by the `from` address
                    previousOwnerSubIds = userOwnedSubId[dropId][from];

                    // Retrieve subId owned by the `to` address
                    newOwnerSubIds = userOwnedSubId[dropId][to];

                    // Update current owner's last subId to the new owner address
                    subIdOwner[dropId][
                        previousOwnerSubIds[previousOwnerSubIds.length - 1]
                    ] = to;

                    // Add the new subId to `to` address (now new owner)
                    userOwnedSubId[dropId][to].push(
                        previousOwnerSubIds[previousOwnerSubIds.length - 1]
                    );

                    // Remove the last subIds from `from` address (now previous owner)
                    userOwnedSubId[dropId][from].pop();
                }
            }
        }
    }
...
contract ERC1155ABErrors {
    // Error returned if the drop is sold-out
    error DropSoldOut();

    // Error returned if the whitelist sale has not started yet
    error SaleNotStarted();

    // Error returned if user attempt to mint more than allowed by the drop
    ...
    // Error returned if there are no ETH to withdraw from the contract
