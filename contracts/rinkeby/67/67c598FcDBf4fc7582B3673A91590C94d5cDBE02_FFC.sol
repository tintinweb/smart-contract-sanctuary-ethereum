...
contract FFC is Ownable, ERC721A {

    string private _baseTokenURI;
    bool private _revealed = false;

    uint256 private presaleTime;
    uint256 private affectedTime;
    uint256 private publicRaffleTime;

    bytes32 public presaleMerkleRoot;
    bytes32 public affectedMerkleRoot;
    bytes32 public publicRaffleMerkleRoot;

    uint16 public devAmount = 200; // Need to update
    uint16 public affectedAmount = 200; // Need to update
    uint16 public presaleAmount = 8488; // Need to update
    uint256 public price = 0.055 ether;
    uint16 public maxMintAmount = 8888;

    constructor(
        bytes32 _presaleMerkleRoot, 
        bytes32 _affectedMerkleRoot, 
        uint16 _devAmount, 
        uint16 _presaleAmount, 
        uint16 _affectedAmount
    ) ERC721A("Fortune Friends Club", "FFC", maxMintAmount) {
    ...
    function reveal(bool revealed, string calldata baseURI) public onlyOwner {
        _revealed = revealed;
        _baseTokenURI = baseURI;
    }
    ...
    function setMintTime(uint256 _presaleTime, uint256 _affectedTime, uint256 _publicRaffleTime) public onlyOwner {
        require(_presaleTime > block.timestamp && _affectedTime > _presaleTime && _publicRaffleTime > _affectedTime, "Invalid mint timings");
    ...
    function affectedMint(
        ...
        require(totalSupply() + numToMint < affectedAmount + 1, "Not enough remaining for mint amount requested");
    ...
    function publicRaffleMint(
        ...
        require(numberMinted(msg.sender) < 1); // cant have minted before
    ...
    // Note: Do we want to expose this?
    ...
    modifier isMintValid(uint16 numToMint, uint16 mintCap) {
        require(totalSupply() + numToMint < maxMintAmount + 1, "Not enough remaining for mint amount requested");
