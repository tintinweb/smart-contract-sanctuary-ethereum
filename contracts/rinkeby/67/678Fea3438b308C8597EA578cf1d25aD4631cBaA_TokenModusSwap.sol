...
contract TokenModusSwap is Singleton, ProxyUtility {
    using SafeERC20 for IERC20;
    using EquateDecimals for *;
    address Modus;
    uint256 tokenStableCoinConversionRate;
    ...
    function initialize(
        ...
        uint256 _projectId,
        // address _Modus,
        uint256 _tokenStableCoinConversionRate
    )
        external
        ...
        initializer
    {
        ...
        Modus = modusRegistry.modus();
        tokenStableCoinConversionRate = _tokenStableCoinConversionRate; ///conversion rate with 18 decimal places
        //tokenAddress = _tokenAddress;
    }

    function setTokenAddress(address token) external onlyModusFactory {
        tokenAddress = token;
    }

    /// @notice Function to allow investors to unstake tokenAddress for Modus Tokens
    function swap(address investor, uint256 amount) external {
        ...
        require(
            ...
            "PXTModusSwap: Caller is not PXTStaking"
        );
        ...
        uint256 modusPrice = uint256(
            IPriceModule(modusRegistry.priceModule()).getUSDPrice(Modus)
        );
        uint256 modusEquivalent = (tokenAddress.decimal18Equivalent(amount) *
            tokenStableCoinConversionRate) / modusPrice;

        IERC20(Modus).safeTransfer(investor, modusEquivalent);
    }

    /// @notice Function to allow ModusTreasury wallet account to retrieve the Modus from the contract
    function collectRemainingModus() public {
        address pxtStaking = modusRegistry
            .getProjectDetails(projectId)
            ._pxtStaking;
        require(
            msg.sender == pxtStaking,
            "PXRTModusSwap: Caller is not PXTStaking"
        );
        address modusTreasury = modusRegistry.modusTreasuryWallet();
        IERC20(Modus).safeTransfer(
            modusTreasury,
            IERC20(Modus).balanceOf(address(this))
        );
    }

    /// @notice Function to allow ModusTreasury wallet account to retrieve certain quantity of  Modus from the contract
    function collectBackModus(uint256 tokens) external {
        address modusTreasury = modusRegistry.modusTreasuryWallet();
        require(
            msg.sender == modusTreasury,
            "PXTModusSwap: Caller is not Modus Treasury"
        );
        IERC20(Modus).safeTransfer(modusTreasury, tokens);
    }
}
...
interface IPriceModule {
    ...
    function tokenFeed(address) external returns (Token memory);

    function setTokenFeedAddress(address, address) external;
