...
import "./utils/SafeTransactions.sol";
import "./NFT.sol";

contract FinalizeAuctionControllerMint is
    IFinalizeAuctionController,
    EnglishAuctionStorage
{
    ...
    function finalize(uint32 _auctionId) external override {
        AuctionStruct storage auction = auctionIdToAuction[_auctionId];

        NFT nft = NFT(auction.nftContractAddress);
        ...
        if (
            ...
            SafeTransactions.sendEth(receiver, royaltyAmount);

            SafeTransactions.sendEth(
                withdrawalAddress,
    ...
    function cancel(uint32 _auctionId) external override {
        revert();
    }

    function adminCancel(uint32 _auctionId, string memory _reason)
        external
        override
    {
        require(
...
library SafeTransactions {
    function sendEth(address _addressTo, uint256 _amount) internal {
        (bool success, ) = _addressTo.call{value: _amount, gas: 5000}("");

        require(success, "library SafeTransactions: sendEth failed");
    }
}
