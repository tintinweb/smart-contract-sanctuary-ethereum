...
/**
 * @title TestAirdrop
 * @notice Contract distributes Test tokens with a merkle tree airdrop.
 */
contract TestAirdrop is Pausable, ReentrancyGuard, Ownable {
    using SafeERC20 for IERC20;

    IERC20 public immutable testToken;
    uint256 public immutable MAXIMUM_AMOUNT_TO_CLAIM;
    uint256 public immutable MAXIMUM_MULTIPLIER;
    uint256 public immutable BASE_AMOUNT_TO_CLAIM;
     ...
     * @param _testToken address of the TEST token
     * @param _baseAmountToClaim base amount to claim per a user
     * @param _maximumAmountToClaim maximum amount to claim per a user
     * @param _maxMultiplier maximum multiplier for airdrop
     * @param _vault transfer address
     */
    constructor(
        uint256 _endTimestamp,
        uint256 _baseAmountToClaim,
        ...
        BASE_AMOUNT_TO_CLAIM = _baseAmountToClaim;
        MAXIMUM_AMOUNT_TO_CLAIM = _maximumAmountToClaim;
        MAXIMUM_MULTIPLIER = _maxMultiplier;
     ...
     * @param multiplier multiplier to claim for the airdrop
    ...
    function claimAndTransfer(
        ...
        require(vault != address(0), "Airdrop: Vault contract must be set");
        require(isMerkleRootSet, "Airdrop: Merkle root not set");
        require(
            multiplier >= 1 && multiplier <= MAXIMUM_MULTIPLIER,
            "Airdrop: Multiplier not within range"
        );
        ...
        bytes32 leaf = keccak256(abi.encodePacked(msg.sender, multiplier));
        require(
        ...
        testToken.safeTransfer(vault, BASE_AMOUNT_TO_CLAIM * multiplier);

        emit AirdropRewardsClaim(msg.sender, BASE_AMOUNT_TO_CLAIM * multiplier);
    }

    /**
     * @notice Check whether it is possible to claim (it doesn't check orders)
     * @param user address of the user
     * @param multiplier multiplier of reward amount
     * @param merkleProof array containing the merkle proof
     */
    function canClaim(
        address user,
        ...
        if (block.timestamp <= endTimestamp) {
            ...
            bytes32 leaf = keccak256(abi.encodePacked(user, multiplier));
    ...
    function updateEndTimestamp(uint256 newEndTimestamp) external onlyOwner {
        require(
            newEndTimestamp > (endTimestamp + 1 days),
            "Owner: New timestamp too early"
        );
    ...
    function withdrawTokenRewards() external onlyOwner {
        ...
        uint256 balanceToWithdraw = testToken.balanceOf(address(this));
        testToken.safeTransfer(msg.sender, balanceToWithdraw);
