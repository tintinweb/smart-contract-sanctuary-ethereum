...
contract GAT is ERC721A, Ownable, ReentrancyGuard {
  ...
  uint256 public robloxCost = .075 ether;
  ...
  constructor() ERC721A("Gods & Titans,  ", "GAT") {    
  ...
  function _setProjectWallet(address _PROJECT_WALLET) external onlyOwner {
  ...
  function verifyTokenForAddress(string calldata _salt, bytes calldata _token, address _address, uint256 _amount ) internal view returns (bool) {
      return _verify(keccak256(abi.encode(_salt, _address, _amount)), _token);
  }  
  ...
  function mint(uint256 _mintAmount, string calldata _salt, bytes calldata _token) external payable nonReentrant mintCompliance(_mintAmount) {
    ...
    if (!verifyTokenForAddress(_salt, _token, msg.sender, msg.value))
  ...
  */
  ...
  function withdrawToProjectWallet() external onlyOwner {
