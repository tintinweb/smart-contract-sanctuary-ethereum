...
contract HowresMarket is Ownable, ERC721Holder {
    using SafeMath for uint256;
    using Address for address;

    uint256 constant public PERCENTS_DIVIDER = 1000;
    uint256 public swapFee = 1;  // 0.1% for admin tx fee
    ...
    function createCollection(string memory _name, string memory _uri, bool bPublic) external returns(address collection) {
        bytes memory bytecode = type(HowresNFT).creationCode;
        ...
        IHowresNFT(collection).initialize(_name, _uri, msg.sender, bPublic);
    ...
    function list(address _collection, uint256 _token_id, uint256 _price) OnlyItemOwner(_collection, _token_id) external {
        ...
        IHowresNFT nft = IHowresNFT(_collection);
    ...
    function delist(uint256 _id) external {
        ...
        IHowresNFT(pairs[_id].collection).safeTransferFrom(address(this), msg.sender, pairs[_id].token_id);
        pairs[_id].bValid = false;
        pairs[_id].price = 0;
        emit ItemDelisted(_id);
    }

    function buy(uint256 _id) external payable ItemExists(_id) {
		...
		require(msg.value >= totalAmount, "insufficient token balance");
        ...
        (bool fs, ) = payable(swapFeeAddress).call{value: msg.value.mul(feeAmount)}("");
        require(fs);
        // transfer token to creator as royalty
        (bool hs, ) = payable(pair.creator).call{value: msg.value.mul(creatorAmount)}("");
        require(hs);
        // transfer token to owner
        (bool os, ) = payable(pair.owner).call{value: msg.value.mul(sellerAmount)}("");
        require(os);
        // transfer NFT token to buyer
        IHowresNFT(pairs[_id].collection).safeTransferFrom(address(this), msg.sender, pair.token_id);
        pairs[_id].bValid = false;
    ...
    modifier OnlyItemOwner(address _collection, uint256 _tokenId) {
        IHowresNFT collectionContract = IHowresNFT(_collection);
