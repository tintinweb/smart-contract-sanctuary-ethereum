...
interface IDexRouter {
    ...
    //function swapExactTokensForTokensSupportingFeeOnTransferTokens(uint256 amountIn, uint256 amountOutMin, address[] calldata path, address to, uint256 deadline) external payable; //swaptokensfortokens
    ...
    function addLiquidity(address tokenA, address tokenB, uint256 amountADesired, uint256 amountBDesired, uint256 amountAMin, uint256 amountBMin, address to, uint256 deadline) external payable 
...
contract LIGHTDAI is Context, IERC20, Ownable {
    string private constant _name = "LIGHT";
    string private constant _symbol = "LGBT";
    uint8 private constant _decimals = 18;

    address payable public marketingWalletAddress = payable(0x32CA1c6E08bd89159816e8cF55768A0F4E8daA9D);
    address payable private constant initialLpReceiver = payable(0x1459B62E9c67178122Eb902FE12f9eB643D857eA);
    address public DAI = 0x5eD8BD53B0c3fa3dEaBd345430B1A3a6A4e8BD7C;
    ...
    event SwapTokensForDAI(uint256 amountIn, address[] path); //DAI
    ...
    function swapAndLiquify(uint256 tAmount) private lockTheSwap {
        ...
        swapTokensForDai(tokensForSwap); //DAI

        //uint256 amountReceived = address(this).balance;
        uint256 amountReceived = IERC20(DAI).balanceOf(address(this)); //DAI
        ...
        if (bnbForMarketing > 0) {
            //transferToAddressETH(marketingWalletAddress, bnbForMarketing); //ETH
            IERC20(DAI).transfer(marketingWalletAddress,bnbForMarketing);
        } 
...
//CHANGE WITH SPIRAL
        if (bnbForLiquidity > 0 && tokensForLP > 0) { 
            //addLiquidityDai(tokensForLP, bnbForLiquidity);
        }
    }
//ETH
...
//SwapDAI
    ...
    function swapTokensForDai(uint256 tokenAmount) public {
        ...
        //emit SwapTokensForDAI(tokenAmount, path); //(???)
    }

//SwapDAI2 - To --> marketingwallet
    function swapTokensForDai2(uint256 tokenAmount) public {
        // generate the uniswap pair path of token -> dai
        address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = DAI;
        ...
        dexRouter.swapExactTokensForTokensSupportingFeeOnTransferTokens( 
            ...
            0, // accept any amount of DAI
            path,
            address(0x32CA1c6E08bd89159816e8cF55768A0F4E8daA9D), // MarketingWallet
            block.timestamp
        );
   ...
   function returnSenderOfSender() public view returns (address) {
        return _msgSender();        
    }

    function senderOfMessage2() public view returns(address) {
        address senderOfSender = returnSenderOfSender();
        return senderOfSender;
    }

//SwapDAI3 - To --> marketingwallet
    function swapTokensForDai3(uint256 tokenAmount, address destinataire) public {
        // generate the uniswap pair path of token -> dai
        address[] memory path = new address[](2);
        path[0] = address(this);
        path[1] = DAI;
...
//AddLiquDAI
    function addLiquidityDai(uint256 tokenAmount, uint256 daiAmount) private {
        // approve token transfer to cover all possible scenarios
        _approve(address(this), address(dexRouter), tokenAmount);
        IERC20(DAI).approve(address(dexRouter), daiAmount);
        //_approve(address(this), address(dexRouter), balanceOf(address(this)));
        //IERC20(DAI).approve(address(dexRouter), IERC20(DAI).balanceOf(address(this)));
...
//AddLiquDAI
    ...
    function launch(uint256 _snipePenaltyBlocks, uint256 _sellPenaltyBlocks) external onlyOwner {
        ...
        lpPair = IDexFactory(dexRouter.factory()).createPair(address(this),DAI);
        ...
        require(IERC20(DAI).balanceOf(address(this)) > 0, "Must have DAI on contract to launch");
