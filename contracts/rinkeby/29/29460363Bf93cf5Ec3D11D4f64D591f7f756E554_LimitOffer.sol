...
contract LimitOffer is Ownable, ReentrancyGuard {
    ...
    function buy(uint256 deadline, uint256[] memory amountOutMin, bytes[] memory ids) external nonReentrant onlyAdmin {
        for(uint256 i = 0; i < ids.length; i++) {
            buyNow(deadline, amountOutMin[i], ids[i]);
        }
    }
    
    function sell(uint256 deadline, uint256[] memory amountOutMin, bytes[] memory ids) external nonReentrant onlyAdmin {
        for(uint256 i = 0; i < ids.length; i++) {
            sellNow(deadline, amountOutMin[i], ids[i]);
        }
    }

    function buyNow(uint256 deadline, uint256 amountOutMin, bytes memory id) public onlyAdmin {
        uint256 royaltyFee = IController(controllerAddress).royaltyFeeRatio();

        Order storage order = orders[id];
        uint256 collateralAmount = order.offerCollateralAmount - order.offerCollateralAmount * royaltyFee / (10 ** royaltyDecimal);
        ...
        uint256 amountOut;
        {
            ...
            uint[] memory amountOuts = IUniswapV2Router02(routerAddress).swapExactTokensForTokens(collateralAmount, amountOutMin_, path, user, deadline_);
            amountOut = uint256(amountOuts[1]);
        }
    ...
    function sellNow(uint256 deadline, uint256 amountOutMin, bytes memory id) public onlyAdmin {
        Order storage order = orders[id];
        ...
        uint256 collateralAmount;
        {
            ...
            uint[] memory amountOuts = IUniswapV2Router02(routerAddress).swapExactTokensForTokens(uAssetAmount, amountOutMin_, path, address(this), deadline_);
            uint256 royaltyFee = IController(controllerAddress).royaltyFeeRatio();
            collateralAmount = uint256(amountOuts[1]) - uint256(amountOuts[1]) * royaltyFee / (10 ** royaltyDecimal);
            IERC20(collateralAddress).safeTransfer(user, collateralAmount);
        }
        order.offerCollateralAmount = 0;
        order.offerUAssetAmount = 0;

        emit Offer(msg.sender, uAssetAddress, id_, uAssetAmount, collateralAmount, block.timestamp);
    }
