...
/**
 * @title Azuki Frens Contract
 * @author ilikecalculus (https://twitter.com/i_like_calculus)
 */
contract AzukiFrens is ERC721A, Ownable, ReentrancyGuard {
    ...
    // Mapping of whether the tokenId is completed
    mapping(uint256 => bool) public tokenCompleted;

    modifier notCompleted() {
        require(!isCompleted, "Collection is complete, cannot make changes.");
        _;
    }

    /**
     * @notice Constructor for AzukiFrens
    ...
    function isCollectionComplete() public view virtual returns (bool) {
        return isCompleted;
    }
    ...
    function mint(address[] calldata receivers, uint256 mintNumber)
        external
        nonReentrant
        onlyOwner
    {
    ...
    function setMaxSupply(uint256 _maxSupply) external notCompleted onlyOwner {
        maxSupply = _maxSupply;
    }

    function setCompletedForTokenId(uint256 tokenId)
        external
        notCompleted
        onlyOwner
    {
        _setCompletedForTokenId(tokenId);
    }

    function setCompletedForTokenIds(uint256[] calldata tokenIds)
        external
        notCompleted
        onlyOwner
    {
        for (uint256 i = 0; i < tokenIds.length; i++) {
            _setCompletedForTokenId(tokenIds[i]);
        }
    }

    function _setCompletedForTokenId(uint256 tokenId) internal {
        tokenCompleted[tokenId] = true;
    }
}
