...
contract ProposalSystem is Modifiers, Utils{
    ...
    function addMoreGoods(uint256 _dealID)public openDeal(_dealID) isPartTaker(_dealID) { 
        // TODO> Pendiente por hacer Test para el require Allowance
        _token = IERC20 (DS.getVar().tokens[DS.getVar().deals[_dealID].coin]);
        DS.historyUpdates storage _historyUpdates = DS.getVar().updates[_dealID];

        uint256 _lastProposal = DS.getVar().updates[_dealID].lastUpdateId;
        (,uint256 _type , , , , ,bool _status) = seeProposals(_dealID, _lastProposal);

        require(_type == 3 && _status == false, "Make a proposal to add more goods and must be accepted");
        ...
        uint256 additionalTokens = _historyUpdates.proposalsInfo[_lastProposal].subOrAddToken;
        (bool _success) =_token.transferFrom(msg.sender, address(this), additionalTokens);
        if(!_success) revert("Error in transfer tokens");
        
        if (DS.getVar().deals[_dealID].goods == DS.getVar().deals[_dealID].amount){
            //update goods balance
            DS.getVar().deals[_dealID].goods += additionalTokens;
            return;
        }else{
            revert("Goods and deal Amount have a diff");
        }
    }
