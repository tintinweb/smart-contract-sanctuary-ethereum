...
contract AdventNFT is ERC721A, Ownable {
    ...
    constructor(
        ...
        bytes32 _allow, bytes32 _wl, uint256 _altime, uint256 _wltime) ERC721A("Advent NFT ", "ADV") {
            baseURI = _uri;
    ...
    function SetMintTime(uint256 _WLTime, uint256 _AllowTime) public onlyOwner{
        WLStartTime = _WLTime;
        AllowStartTime = _AllowTime;
    }
    ...
    function mint(uint256 quantity, bytes32[] memory sign) external payable {
        require(totalSupply()<=MAX_SUPPLY,"All NFTs were SoldOut");
