...
contract TimedCacheEthAndPrimeRewards is PrimeRewards, ReentrancyGuard {
    ...
    /// @param pid Token id to claim.
    function claimEth(uint256 pid) public nonReentrant {
        ...
        emit Claim(msg.sender,pid, pendingEthReward, ID_ETH);
    }

    /// @notice Claim eth and PRIME for transaction sender.
    /// @param pid Pool id to claim.
    function claimPrimeAndEth(uint256 pid) public {
        claimPrime(pid);
        claimEth(pid);
    }
    ...
    function claimPoolsPrimeAndEth(uint256[] calldata pids) public {
        for (uint256 i = 0; i < pids.length; ++i) {
            claimPrimeAndEth(pids[i]);
        }
    }

    /// @notice Withdraw Masterpiece and claim eth for transaction sender.
    /// @param pid Token id to withdraw.
    function withdrawAndClaimEth(uint256 pid, uint256 amount) public {
        claimEth(pid);
        withdraw(pid, amount);
    }
    ...
    /// @param pid Token id to withdraw.
    function withdrawAndClaimPrimeAndEth(uint256 pid, uint256 amount) public {
        claimEth(pid);
        withdrawAndClaimPrime(pid, amount);
    }
...
contract PrimeRewards is Ownable, ERC1155Holder {
    ...
    function setPrimePerBlock(
        ...
        require(
            ...
            "Only updates after initial endBlock"
        );
