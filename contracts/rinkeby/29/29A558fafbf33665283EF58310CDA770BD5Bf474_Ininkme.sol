...
contract Ininkme is ERC721A, EIP712Whitelisting {
    ...
    uint256 public constant customMaxSupply = 10;
    ...
    function customMint(address _auction) external onlyOwner {
        require(
            _currentIndex >= whitelistMaxSupply,
            "The whitelist has not been cast yet."
        );
        require(salePhase == SalePhase.CustomPhase, "Sale not available.");
        require(
            ++totalCustom <= customMaxSupply,
            "Purchase exceed customMaxSupply."
        );
        _safeMint(_auction, 1);
    }

    function setCustomTokenURI(uint256 _customId, string memory _tokenURI)
        external
        onlyOwner
    {
        require(
        ...
        returns (string memory)
    {
        ...
        if (bytes(_tokenBaseURI).length == 0) {
            return _defaultURI;
        } else if (tokenId < totalSupply() - totalCustom) {
            return
                string(
