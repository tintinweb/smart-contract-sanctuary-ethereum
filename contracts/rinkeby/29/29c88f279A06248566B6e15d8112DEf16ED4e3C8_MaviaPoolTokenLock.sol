...
/**
 * @notice Mavia Staking Token Pool
 * @dev Version 1.0
 * Each staker has unlimited deposit package with specific locked duration
 * We provide several pool with fixed locked duration and APY
 ...
 */
contract MaviaPoolTokenLock is ReentrancyGuardUpgradeable, AccessControlUpgradeable {
  ...
  struct MDeposit {
    ...
    uint256 lkTo;
    /// @dev Fixed apy of the deposit
    uint256 APY;
    /// @dev Last reward time
    uint256 lastRTime;
    /// @dev Deposit time
    uint256 dTime;
    /// @dev Withdraw time
  ...
  /// @dev Reward limited mode
  bool public rewardLimitMode;
  ...
  /// @dev Estimate reward needed based on current stake and apy
  uint256 public poolRewardNeededAmount;
  /// @dev Enabled reward needed based on current stake and apy
  bool public enabledPoolRewardNeededAmount;
  ...
  /// @dev Fixed APY
  uint256 public APY;
  /// @dev Fixed APY Bonus for first hour
  uint256 public earlyBonus;
  /// @dev Bonus launch time
   ...
   * @param _pAPY Fixed APY
   * @param _pEarlyBonus Fixed bonus will add in fixed apy if its in within bonus time
   * @param _pBonusDuration Bonus launch duration
   */
  function MaviaPoolTokenLockInit(
    ...
    uint256 _pAPY,
    uint256 _pEarlyBonus,
    uint256 _pBonusDuration
    ...
    APY = _pAPY;
    earlyBonus = _pEarlyBonus;
    bonusDuration = _pBonusDuration;
    enabledPoolRewardNeededAmount = true;
  }

  /**
   * @notice Get remaining reward
   */
  function fGetRemainingReward() public view returns (uint256) {
  ...
  /**
   * @notice Estimate max reward by duration.
   * @param _pAmount: staked amount
   * @param _pFixedAPY: APY
   * @param _pLockTime: lock duration time
   * @return Max reward
   */
  function fGetMaxReward(
    uint256 _pAmount,
    uint256 _pFixedAPY,
    uint256 _pLockTime
  ) public view virtual returns (uint256) {
    return (_pAmount * _pFixedAPY * _pLockTime) / _SECONDS_YEAR / _RATE_NOMINATOR;
  }
   ...
   */
  function fVestReward(uint256 _pDepositId) public nonReentrant {
    // Withdraw is frozen
   ...
   */
  function fGetPendingReward(address _pStaker, uint256 _pDepositId) public view virtual returns (uint256) {
    MDeposit storage deposit_ = depositLogs[_pStaker][_pDepositId];
    uint stakerReward_;
    if (block.timestamp > deposit_.lastRTime && poolCurrStakedAmount != 0) {
      uint256 multiplier = _fGetMultiplier(deposit_.lastRTime, block.timestamp, deposit_.lkFrom, deposit_.lkTo);
      if (multiplier == 0) return 0;
      // InterestAmount = P * APY/SecondsPerYear * n
      stakerReward_ = fGetMaxReward(deposit_.dAmount, deposit_.APY, multiplier);
    }
    return stakerReward_;
  }

  /**
   * @notice Get remaining reward needed
   */
  function fGetPoolRewardNeededAmount() external view returns (uint256) {
    return fGetRemainingReward() - poolRewardNeededAmount;
  }
  ...
  /**
   * @notice Update limit reward mode
   * @dev Only callable by owner.
   * @param _pRewardLimitMode: mode
   */
  function fSetRewardLimitMode(bool _pRewardLimitMode) external onlyRole(EDITOR_ROLE) {
    rewardLimitMode = _pRewardLimitMode;
  }
   ...
   */
  function fSetDepositTime(uint256 _pPoolStartTime, uint256 _pPoolEndTime) external onlyRole(EDITOR_ROLE) {
  ...
  /**
   * @notice Update enabled pool reward needed amount
   * @dev Only callable by owner.
   * @param _pEnabledPoolRewardNeededAmount: mode
   */
  function fSetEnabledPoolRewardNeededAmount(bool _pEnabledPoolRewardNeededAmount) external onlyRole(EDITOR_ROLE) {
    enabledPoolRewardNeededAmount = _pEnabledPoolRewardNeededAmount;
  }
  ...
  /**
   * @notice Update fixed apy config
   * @dev This function is only callable by editor.
   * @param _pAPY: Set the fixed apy
   * @param _pEarlyBonus: Set the fixed apy bonus
   * @param _pBonusDuration Bonus Launch time
   */
  function fSetAPYConfig(
    uint256 _pAPY,
    uint256 _pEarlyBonus,
    uint256 _pBonusDuration
  ...
  function fDeposit(uint256 _pAmount) external nonReentrant {
    ...
    uint256 APY_ = APY;
    if (block.timestamp <= poolStartTime + bonusDuration) {
      APY_ = APY + earlyBonus;
    }
    ...
    // Reward needed
    if (enabledPoolRewardNeededAmount) {
      poolRewardNeededAmount += fGetMaxReward(addedAmount_, APY_, lockedDuration);
      if (rewardLimitMode) {
        require(fGetRemainingReward() >= poolRewardNeededAmount, "0x5");
      }
    }
  ...
  function fClaimReward(uint256 _pRewardId) external nonReentrant {
    ...
    if (enabledPoolRewardNeededAmount) poolRewardNeededAmount -= amount_;
   ...
   * @param _pDepositId: Deposit id of staker investment
   */
  function _fVestReward(uint256 _pDepositId) internal {
    address sender_ = _msgSender();
    MStaker storage staker_ = stakers[sender_];
    MDeposit storage deposit_ = depositLogs[sender_][_pDepositId];

    // Vesting reward
    uint256 pending_ = fGetPendingReward(sender_, _pDepositId);
    if (pending_ > 0) {
      // Locked in 365 days
      ...
      staker_.rNextId++;
      deposit_.lastRTime = block.timestamp;
   ...
   * @param _pTo: address where tokens will transfer
   * @param _pAmount: amount of tokens
   */
  function _fSafeRewardTransfer(address _pTo, uint256 _pAmount) private {
    uint256 rewardBal = rToken.balanceOf(address(this));
    uint256 remaining = fGetRemainingReward();
    if (remaining > rewardBal) {
      remaining = rewardBal;
    }

    if (_pAmount > remaining) {
      poolClaimedReward += remaining;
      rToken.safeTransfer(_pTo, remaining);
    } else {
      poolClaimedReward += _pAmount;
      rToken.safeTransfer(_pTo, _pAmount);
    }
  }
