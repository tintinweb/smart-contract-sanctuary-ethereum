...
contract FlowerShoppe is ERC721A, Ownable, ReentrancyGuard {
    ...
    event gifted(address from, address to, uint256 amount, string flower);
    event typeAdded(string name, string uri, uint256 index, uint256 maxSupply, uint256 price, bool hasEP, address exclusiveProject);
    ...
    function gift(address recipient, uint256 amount, uint256 flowertype) external payable nonReentrant{
        ...
        emit gifted(_msgSender(), recipient, amount, flowertypes[flowertype].name);
    }
    ...
    function addFlower(string memory _name, string memory uri, uint256 maxSupply, uint256 _price, bool _PSOnly, bool _hasEP, address _exclusiveProject) external onlyOwner {
        ...
        emit typeAdded(_name, uri, flowertypes.length - 1, maxSupply, _price, _hasEP,  _exclusiveProject);
    }
