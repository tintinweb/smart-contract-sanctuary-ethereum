...
contract NFTTweetMarketplace is Ownable, Pausable, ReentrancyGuard {
        ...
        nonReentrant
    {
        ...
        require(
            block.timestamp <= sl.endTime,
            "NFTTweetMarketplace: sale period expired"
        );
        require(sl.sold == false, "NFTTweetMarketplace: can't buy a sold item");
        require(
            msg.value == sl.price,
            "NFTTweetMarketplace: msg.value == listing price"
        );
        ...
        // also check if escrow state is already active, if so delete so it would be replaced
        if (
        ...
        if (isERC721(sl.nftContract)) {
            ...
            revert(
                "NFTTweetMarketplace: only ERC721 and ERC1155 types of tokens are supported for sale"
            );
        }
    ...
    function putForSale(
        ...
        require(
            price >= minSalePrice,
            "NFTTweetMarketplace: price must be >= minSalePrice"
        );
        require(
            saleDurationInSeconds >= minSaleTime,
            "NFTTweetMarketplace: sale time < minSaleTime"
        );
        require(
            saleDurationInSeconds <= maxSaleTime,
            "NFTTweetMarketplace: sale time > maxSaleTime"
        );
        ...
        if (
            ...
            revert("NFTTweetMarketplace: tweet request on process");
        }
        ...
        nonReentrant
    {
        ...
        require(
            sl.sold == false,
            "NFTTweetMarketplace: can't claim a sold item"
        );
        require(
            msg.sender == sl.seller,
            "NFTTweetMarketplace: only the seller can remove it"
        );
    ...
    function reclaimERC20(address _tokenContract) external onlyOwner {
        require(
            ...
            "NFTTweetMarketplace: Invalid address"
        );
        IERC20 token = IERC20(_tokenContract);
        uint256 balance = token.balanceOf(address(this));
        require(
            token.transfer(msg.sender, balance),
            "NFTTweetMarketplace: Transfer failed"
        );
    }
        ...
        onlyOwner
    {
        require(
            ...
            "NFTTweetMarketplace: no escrow"
        );
        EscrowListing storage el = escrowListings[
            escrowContracts[_nftContract][_tokenID]
        ];
        require(!el.settled, "NFTTweetMarketplace: already settled");
    ...
    function withdraw(uint256 amount) external onlyOwner {
        ...
        require(success, "NFTTweetMarketplace: withdraw failed");
    }
