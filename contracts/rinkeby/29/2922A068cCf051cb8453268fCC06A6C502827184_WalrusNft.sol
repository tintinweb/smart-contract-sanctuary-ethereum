...
contract WalrusNft is ERC721A , Ownable, Pausable {

    using Strings for uint256;

    //Image and metadata,
    bool public uriLocked; //baseURI & contractURI lock
    string public _contractBaseURI = "https://ikzttp.mypinata.cloud/ipfs/QmQFkLSQysj94s5GvTHPyzTxrawwtjgiiYS2TBLgrvw8CW/";
    string public _contractURI = "https://mywebsite.com/contract_uri/contract_uri.json";
    uint256 public tokenPrice = 0.11 ether; //price per token
    address proxyRegistryAddress;

    uint256 public maxSupply = 9999; //tokenIDs start from 0
    constructor() ERC721A("WalrusNft", "WALRUSNFT") {}

    function mint(uint256 quantity) external payable whenNotPaused {

        require(tokenPrice * quantity <= msg.value, "exact amount needed"); //Todo == or <=
        require(quantity < 11, "max 10 at once");//Todo
...
//        return string(abi.encodePacked(_contractBaseURI, _tokenId.toString(), ".json")); @todo check for .json extensions
    ...
    //change the max supply (even for adminMint)
    function setmaxMintAmount(uint256 newMaxSupply) public onlyOwner {
        maxSupply = newMaxSupply;
    }

    //blocks staking but doesn't block adminMint
