...
contract SLEEPY_SLOTHS_META is Ownable, ERC721A, ReentrancyGuard {
    using Strings for uint256;
...
//   uint256 public MAX_PER_Address = 1; // maximam amount that user can mint
  ...
  uint256 private constant TotalCollectionSize_ = 10500; // total number of nfts
  ...
  uint private stopat = 1000;
  uint private reserve = 500;

  uint public status = 0; //0-pause 1-whitelist 2-public

  mapping(address => bool) private whitelistedAddresses;

  constructor() ERC721A("SLEEPY_SLOTHS_META","SLEEPY_SLOTHS_META", MaxMintPerBatch_, TotalCollectionSize_) {
  ...
  function mint(uint256 quantity) external payable callerIsUser {
    ...
    // require( status == 2 , "Sale not started!" );
  ...
  function isWhitelisted(address _user) public view returns (bool) {
   ...
   function conf(uint256 Status , uint256 MaxPerWallet , uint256 Price, uint256 Stop_At )external onlyOwner{
      status = Status;
      stopat = Stop_At;
      PRICE = Price;
