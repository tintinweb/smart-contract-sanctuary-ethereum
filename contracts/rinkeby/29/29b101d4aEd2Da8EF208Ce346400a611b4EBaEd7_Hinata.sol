...
contract Hinata is ERC20Capped, ERC20Burnable, Ownable {
    ...
    function claimNFT(
        ...
        bytes32 message = keccak256(
            abi.encodePacked(recipient, id, amount, nonce)
        );
...
interface IHinataStorage {
    ...
    function createDrop(uint256 dropId) external;

    function ownerOfDrop(uint256 dropId) external view returns (address);

    function transferDrop(address to, uint256 dropId) external;

    function transferDropFrom(
        address from,
        address to,
        uint256 dropId
    ) external;
    ...
    function setDrop(
        uint256 dropId,
        uint256[] memory ids,
        uint256[] memory idAmounts
    ) external;

    function getDrop(uint256) external view returns (Drop calldata);

    function getDropOwner(uint256) external view returns (address);

    function getMaxDropId() external view returns (uint256);

    function createDropOnBehalf(uint256 dropId, address creator) external;
