...
contract Ghostcolor is Ownable, ERC721A, PaymentSplitter {
    ...
    uint public saleStartTime = 1654638300;

    bytes32 public merkleRoot;

    string public baseURIgreen;
    string public baseURIblue;
    string public baseURIred;
    ...
    address[] private _team = [
        0xf5700B9B1b70c649511EDB341678B8d4Cc7c3689, //adresse Metamask principale
        0x343ed9d8ae76493d661296D57773Acf11EEf14Eb //adresse Metamask secondaire 
    ];
    ...
    //Constructor => Remplacer la parenthèse du contrat par le nom du contrat souhaité
    constructor(bytes32 _merkleRoot, string memory _baseURIgreen, string memory _baseURIblue, string memory _baseURIred) 
    ERC721A("Ghostcolor", "GC") 
    PaymentSplitter(_team, _teamShares) {
        merkleRoot = _merkleRoot;
        baseURIgreen= _baseURIgreen;
        baseURIblue= _baseURIblue;
        baseURIred= _baseURIred;
    ...
    * @param _baseURIgreen 
    **/
    function publicMintgreen(address _account, uint _quantity, string memory _baseURIgreen) external payable callerIsUser {
        require(!isPaused, "Contract is Paused");
        ...
        require(amountNFTperWalletPublicSale[msg.sender] + _quantity <= maxPerAddressDuringPublicMint, "You can only get 3 NFTs");
    ...
    * @param _baseURIblue 
    **/
    function publicMintblue(address _account, uint _quantity, string memory _baseURIblue) external payable callerIsUser {
        require(!isPaused, "Contract is Paused");
        require(currentTime() >= saleStartTime + 24 hours, "Public sale has not started yet");
        require(currentTime() < saleStartTime + 48 hours, "Public Sale is finished");
        uint price = publicSalePrice;
    ...
    * @param _baseURIred 
    **/
    function publicMintred(address _account, uint _quantity, string memory _baseURIred) external payable callerIsUser {
        require(!isPaused, "Contract is Paused");
        require(currentTime() >= saleStartTime + 24 hours, "Public sale has not started yet");
        require(currentTime() < saleStartTime + 48 hours, "Public Sale is finished");
        uint price = publicSalePrice;
    ...
    *
    * @param _tokenIdgreen The ID of the NFT you want to have the URI of the metadatas
    *
    * @return the token URI of an NFT by his ID
    */
    function tokenURIgreen(uint _tokenIdgreen) public view  returns(string memory) {
        require(_exists(_tokenIdgreen), "URI query for nonexistent token");

        return string(abi.encodePacked(baseURIgreen, _tokenIdgreen.toString(), ".json"));
    }
    ...
    * @notice Get the token URI of an NFT by his ID
    *
    * @param _tokenIdblue The ID of the NFT you want to have the URI of the metadatas
    *
    * @return the token URI of an NFT by his ID
    */
     function tokenURIblue(uint _tokenIdblue) public view returns(string memory) {
        require(_exists(_tokenIdblue), "URI query for nonexistent token");

        return string(abi.encodePacked(baseURIblue, _tokenIdblue.toString(), ".json"));
    }

    /**
    * @notice Get the token URI of an NFT by his ID
    *
    * @param _tokenIdred The ID of the NFT you want to have the URI of the metadatas
    *
    * @return the token URI of an NFT by his ID
    */
    function tokenURIred(uint _tokenIdred) public view returns(string memory) {
        require(_exists(_tokenIdred), "URI query for nonexistent token");

        return string(abi.encodePacked(baseURIred, _tokenIdred.toString(), ".json"));
    }
    ...
    *
    * @param _baseURIgreen the new base URI of the NFTs
    */
    function setBaseURIgreen(string memory _baseURIgreen) external onlyOwner {
        baseURIgreen = _baseURIgreen;
    }
    ...
    * @notice Change the base URI of the NFTs
    *
    * @param _baseURIblue the new base URI of the NFTs
    */
    function setBaseURIblue(string memory _baseURIblue) external onlyOwner {
        baseURIblue = _baseURIblue;
    }
    ...
    * @notice Change the base URI of the NFTs
    *
    * @param _baseURIred the new base URI of the NFTs
    */
    function setBaseURIred(string memory _baseURIred) external onlyOwner {
        baseURIred = _baseURIred;
    }
