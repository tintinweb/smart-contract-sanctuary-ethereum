    ...
    ERC721Delegated, IEditionSingleMintable
{
    ...
    uint256 public constant maxNumberCanMint = 1;

    // Name of the NFT edition
    string public name;

    // Symbol of the NFT edition
    ...
    /**
      @dev This allows a user to purchase a edition edition
           at the given price in the contract and for a given token ID.
           If token ID is 0, it will mint a new edition at the next 
           available increment.
     */
    function purchase(uint256 tokenId) external payable returns (uint256) {
        require(!_exists(tokenId), "Token exists");
        require(msg.value == salePrice, "Wrong price");
        require(dropSize == 0 || atEditionId.current() <= dropSize, "Sold out");
        require(!paused, "Contract is paused");
        ...
        // Save metadata
        metadataJson[tokenId] = sharedNFTLogic.base64Encode(
            abi.encodePacked('{"generation":"', sharedNFTLogic.numberToString(generation), '", "claims":[]}')
        );

        // Increment edition id
    ...
    /**
      @param _salePrice the amount of ETH needed to start the sale.
        ...
        returns (uint256)
    {
        ...
        while (atEditionId.current() <= endAt) {
            revealed[atEditionId.current()] = true;
            _mint(
    ...
    */
    ...
    {
        require(_exists(tokenId), "No token found");
        metadataJson[tokenId] = sharedNFTLogic.base64Encode(bytes(_metadata));
    }
