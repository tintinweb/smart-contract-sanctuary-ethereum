...
contract LockedCoin is ERC777 {
	...
	string public uri ="www.epigeon.org/lockable/";
    ...
    function burnByHash(address to, bytes32 hash) public returns (uint256 amount){
        ...
        require(_lockedAmmountByHash[to][hash] > 0, "There is no amount locked with the given hash");
        uint256 lockedAmount = _lockedAmmountByHash[to][hash];
        _burn(to, lockedAmount, "", "");
    ...
    function burnByHash(address from, address to, bytes32 hash) public returns (uint256 amount){
        require(msg.sender == address(coin), "Only available for master coin");
        require(_lockedAmmountByHash[to][hash] > 0, "No amount locked with this hash");
    ...
    function mintByHash(address to, uint256 amount, bytes32 hash, address from) public {
        require(msg.sender == address(coin), "Only available for master coin");
        require(_lockedAmmountByHash[to][hash] == 0, "There is already amount locked with the same hash");
        _mint(to, amount, "", "");
    ...
    function reclaim(address to, string unlockerPhrase) public {
