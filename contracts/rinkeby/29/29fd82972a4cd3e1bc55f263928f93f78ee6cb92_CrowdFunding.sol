...
// File: tests/StructCrowdFunding.sol
...
contract CrowdFunding {
    ...
    */
    ...
    function showFunder(uint256 campaignID) public returns(address funder, uint256 amount){ 
        Campaign storage c = campaigns[campaignID];
        uint number = c.numFunders;
        for( uint i = 0; i<number; i++)
        {
            funder = c.funders[i].addr;
            amount = c.funders[i].amount;
