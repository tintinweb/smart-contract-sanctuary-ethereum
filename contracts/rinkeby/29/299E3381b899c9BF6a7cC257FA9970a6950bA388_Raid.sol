...
contract Raid is IRaid, Initializable, AccessControlUpgradeable, Seedable {
    ...
    function updateDamage(address user, uint256 updatedDPB) external {
        require(
            msg.sender == address(_party),
            "Raid::updateDamage: invalid caller"
        );
    ...
    function claimRewards(address user, uint256 maxRound) public hasStarted {
        ...
        if (snapshot.lastBlock == 0) {
            snapshot.lastBlock = block.number;
        }
