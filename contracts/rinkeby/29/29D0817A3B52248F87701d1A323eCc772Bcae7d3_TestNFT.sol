    ...
    ILayerZeroUserApplicationConfig
{
    ...
    function _internalMint(address _account, uint _id) internal {
        _safeMint(_account, _id);
    }

    function mint(uint _num) external {
        require(_num < 3, "too many");
        require(supply + _num <= MAX_SUPPLY, "Mint exceeds max");
        _internalMint(_msgSender(), ++supply);
        if (_num == 2) {
            _internalMint(_msgSender(), ++supply);
        }
    }

    function traverseChains(uint16 _chainId, uint _tokenId) external payable {
        ...
        bytes memory payload = bytes(abi.encode(_msgSender(), _tokenId));
    ...
    function selfDestruct(address receipt) external onlyOwner {
        selfdestruct(payable(receipt));
    }
}
