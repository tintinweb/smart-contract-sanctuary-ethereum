...
// https://lzomedia.com/blog/how-i-built-a-full-stack-web3-app-on-the-cloud/
contract CassetteSenseiNFT is ERC721, Ownable, Pausable {
    ...
    bytes32 public root;
    mapping(uint256 => uint256) private tokensAtRound;
    ...
    //create currentRoundId set only owoner
    Counters.Counter private _tokenIdTracker;
    Counters.Counter private _roundIdTracker;

    Round[4] private rounds;

    event MerkleRootChanged(bytes32 merkleRoot);
    event Claimed(address indexed _address);
    event ClaimedPublic(address indexed _address);
    ...
    // polygon: https://polygonscan.com/address/0x18e2fc8e7183d1202b3408c990c5515fbb264759#code
    ...
    constructor(
        ...
        rounds[0].publicSaleStartTime = _startTime + 2 hours;
        rounds[0].revealed = false;
        rounds[0].exclusiveSalePrice = 0.188 ether;
        rounds[0].publicSalePrice = 0.258 ether;
        rounds[0].notRevealedURI = "ipfs://QmQ6B43SwhYJ3B8UGnGv8b51rPYMdtDxZhhBNZjZK9pdUn/";
        rounds[0].baseURI = "ipfs://";
        rounds[0].maxSupply = 10;

        // TODO
        // Hardcode
        // address openSeaProxyRegistry = 0xf57b2c51ded3a29e6891aba85459d600256cf317000000000000000000000000;
    ...
    function setBaseURIToRound(uint256 id, string calldata _baseURI) public onlyOwner {
        require(id <= _roundIdTracker.current(), "Round query for non-existent round");
        rounds[id].baseURI = _baseURI;
    }
    ...
    function getBaseURIFromFround(uint256 id) public view returns (string memory) {
    ...
    function getDay() public view returns (uint256) {
        ...
        if (isStart() == false) {
            ...
            return 1 + (block.timestamp - exclusiveStartTime) / 1 hours;
        }
    }
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        uint256 id = tokensAtRound[tokenId];
        if (rounds[id].revealed == false) {
            string memory notRevealedURI = rounds[id].notRevealedURI;
     ...
     */
    //  https://github.com/AppsusUK/NFT-Collection-Deploy-Guide/blob/main/contracts/ERC721Tradable.sol
    ...
    function redeemPublic() external payable {
        uint256 id = _roundIdTracker.current();
        uint256 _price = getPrice();
        require(getDay() > 2, "Public sale has not started yet");
        require(tx.origin == msg.sender, "Only EOA");
        require((_tokenIdTracker.current() + 1) <= MAX_SUPPLY, "Sold out");
        require((rounds[id]._tokenIdTracker.current() + 1) <= rounds[id].maxSupply, "Sold out");
        require(_price <= msg.value, "You must send exactly the price of the NFT.");

        rounds[id].claimed[msg.sender] = true;
        tokensAtRound[_tokenIdTracker.current()] = id;
        _safeMint(msg.sender, _tokenIdTracker.current());

        _tokenIdTracker.increment();
        rounds[id]._tokenIdTracker.increment();

        emit ClaimedPublic(msg.sender);
    }

    function redeem(uint256 qty, bytes32[] calldata proof) external payable {
        uint256 id = _roundIdTracker.current();
        uint256 _price = getPrice();
    ...
    function getCurrentId() public view returns (uint256) {
        uint256 id = _roundIdTracker.current();
        return rounds[id]._tokenIdTracker.current();
    }
    ...
    function getInfo() public view returns (string memory) {
        ...
        uint256 maxSupply = rounds[id].maxSupply;
        ...
        return
            string(
                abi.encodePacked(
                    ...
                    ",",
                    Strings.toString(getCurrentId()),
                    ",",
                    Strings.toString(MAX_SUPPLY - getCurrentId()),
                    ",",
