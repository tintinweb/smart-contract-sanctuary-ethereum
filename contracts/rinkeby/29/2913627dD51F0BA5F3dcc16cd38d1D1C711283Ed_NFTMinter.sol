...
contract NFTMinter is MedievalAccessControlled, ReentrancyGuard{
    IMedievalNFT immutable public nft;
    NFTVoucher immutable public voucher;
    ...
    function mintWithVoucher(uint256 tokenId) external nonReentrant {
        address owner = voucher.ownerOf(tokenId);
        require(msg.sender==owner, "msg.sender is not the voucher owner");
        nft.mint(owner, 1);
...
import "./access/MedievalAccessControlled.sol";
import "./erc721/ERC721BEnumerable.sol";
import "./opensea/IProxyRegistry.sol";


contract NFTVoucher is ERC721BEnumerable, MedievalAccessControlled{
    using Counters for Counters.Counter;

    constructor(
        address _controlCenter
    ) ERC721B("Medieval Voucher", "MV") { //TODO
        _setControlCenter(_controlCenter, msg.sender);
    }
    ...
    function multiMint(address[] calldata tos) external onlyAdmin {
        uint256 tokenId = _owners.length;
 ...
 */
abstract contract ERC721BEnumerable is ERC721B, IERC721Enumerable {
