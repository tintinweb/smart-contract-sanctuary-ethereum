// SPDX-License-Identifier: MIT
  ...
  function whitelistMint(uint256 _mintAmount, bytes32[] calldata _merkleProof) public payable mintCompliance(_mintAmount) mintPriceCompliance(_mintAmount) {
    ...
    modifier mintPriceCompliancefree(uint256 _mintAmount) {
      ...
      require(!paused, 'The contract is paused!');
        if (freeMintCount < 3) {
          if (_mintAmount > 3) {
            payForCount = _mintAmount - 3;
          } else {
            payForCount = 0;
          }
          _freeMintedcount[_msgSender()] = 3;
        }
      require(msg.value >= cost * _mintAmount , 'Insufficient funds!');
    _;  
  }
    function mint(uint256 _mintAmount) public payable mintCompliance(_mintAmount) mintPriceCompliancefree(_mintAmount){
      require(
