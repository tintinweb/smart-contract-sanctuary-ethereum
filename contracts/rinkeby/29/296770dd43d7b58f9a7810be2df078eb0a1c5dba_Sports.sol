...
contract Sports is ERC721, ERC721URIStorage, Ownable, EIP712, ERC721Votes {
    using Counters for Counters.Counter;

    Counters.Counter private _tokenIdCounter;
    string metadataURI;
    string collectionURI;
    mapping(uint => string) private tokenNames;
    uint supply;
    uint minted;

    constructor(uint _supplyTot,string memory _metadataURI,string memory _collectionURI) ERC721("Sports", "SPT") EIP712("Sports", "1") {
        supply = _supplyTot;
        minted = 0;
    ...
    function myMint(address to, string memory uri, string memory _tokenName) public onlyOwner {
        require(minted<=supply,"Collection Ended!");
        uint256 tokenId = _tokenIdCounter.current();
        _tokenIdCounter.increment();
        _safeMint(to, tokenId);
        _setTokenURI(tokenId, uri, _tokenName);
        minted++;
    }
    ...
    function _setTokenURI(uint256 tokenId, string memory _tokenURI, string memory _tokenName) internal {
    ...
    {
        string memory baseURI = _baseURI();
        return bytes(baseURI).length > 0 ? string(abi.encodePacked(baseURI,metadataURI,"/",tokenNames[tokenId],".json")) : super.tokenURI(tokenId);
    }

    function contractURI() public view returns (string memory){
        string memory baseURI = _baseURI();
        return string(abi.encodePacked(baseURI,collectionURI,"/","collection.json"));
    }
}