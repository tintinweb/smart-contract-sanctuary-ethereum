...
contract Market is AuthorityControlled {
    ...
    function draw() public returns (bool, uint256) {
        ...
        start = start.add(nftRule.lock);
        bool linearFlag = share >= start;               // 是否满足线性释放
        ...
        uint256 actual = 0;  // 计算实际可领取奖励的月份
        if(share >= start) {
            actual = share - start + 1 - nftRecord.past;
            actual = actual > nftRecord.future ? nftRecord.future : actual;
            require(actual > 0 && nftRecord.flag, "You have no new rewards to claim");
            uint256 avaible = actual.mul(nftRecord.stage);                            // 计算可领取的奖励数量
    ...
    function open(uint256 amount) public returns (bool) {
        require(0 < amount && amount <= 10000000000, "Incorrect purchase quantity");	// 检查传入的数量是否合法
