...
contract StakingClaim is IStakingClaim, Ownable, ReentrancyGuard {
        ...
        returns(uint256 _elapsedDays)
    {
        ...
        uint256 _paymentDt = rootHashes[_rootHashIdx].paymentDt;
        if(block.timestamp + _duration > _paymentDt) {
            return (block.timestamp - _paymentDt).div(_duration) + 1;
        } else {
            return 1;
        }
    }
        ...
        returns(uint256 _tokensCurrentlyReciveable)
    {
        ...
        if(_totalReciveableToken > _amountReceived) {
            return _totalReciveableToken - _amountReceived; // 현재 받을 수 있는 총 토큰량 - 현재까지 받은량 = 현재 받을 수 있는 량
        } else {
            return 0;
        }
    }

    function SetRootHash(bytes32 _root, uint256 _paymentDt, uint256 _vestedTokenAmount) 
        external override 
        onlyOwner 
    {
        TotalVestedTokenAmount += _vestedTokenAmount;
        rootHashes.push(merkleTreeInfo(_root, _paymentDt));
        emit setRootHash (_paymentDt, rootHashes.length);
    }
    ...
    function ClaimToStaking(uint256 _roothashIdx, bytes32[] memory _proof, bool[] memory _proofFlags, bytes32[] memory _leaves) 
        ...
        nonReentrant
    {
        bytes32 root = rootHashes[_roothashIdx].rootHash;
        require(MerkleProof.multiProofVerify(_proof, _proofFlags, root, _leaves), "ClaimToStaking_ERR01"); //머클트리에 포함되지 않은 데이터
        uint256 _amount = compensationAmount.mul(_proof.length); // 받을 수 있는 총량 [단위 ether]
        uint256 _amountReceived = receivedInfos[_msgSender()].amountReceived; // 받은 총량 [단위 ether]
        require(_amount > _amountReceived,"ClaimToStaking_ERR02"); // 받은 금액이 총량 보다 같거나 크면 더이상 받을 수 없음 [require]

        uint256 _amountReceive = GetTokensCurrentlyReciveable(_roothashIdx ,_proof.length); // 현재 받을 수 있는 량

        // 각각 구간에 맞는 수령액 설정하여 지급
