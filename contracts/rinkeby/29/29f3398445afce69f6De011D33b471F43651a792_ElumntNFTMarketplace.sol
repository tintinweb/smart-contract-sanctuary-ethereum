...
contract ElumntNFTMarketplace is Ownable, Verification {
    ...
    uint256 PLATFORM_SHARE_PERCENT_FOR_LESS = 0;
    uint256 PLATFORM_SHARE_PERCENT_FOR_MORE = 0;
    uint256 PIXEL_LIMIT = 0;
    ...
    function checkPlatformSharebyPixel(uint256 balance) public view returns (uint256) {
        uint256 pecentValue = 0;
        if(PIXEL_LIMIT == 0) {
            pecentValue = PLATFORM_SHARE_PERCENT_FOR_MORE;
        }
        else {
            if(PLATFORM_SHARE_PERCENT_FOR_LESS > 0 && PLATFORM_SHARE_PERCENT_FOR_MORE > 0) {
                if(balance > PIXEL_LIMIT) {
                    pecentValue = PLATFORM_SHARE_PERCENT_FOR_MORE;
                }
                else {
                    pecentValue = PLATFORM_SHARE_PERCENT_FOR_LESS;
                }
            }
        }
        return pecentValue;
    ...
    // }
    function checkPixelLimitWithPercent() public view returns (pixulLimitWithPercentData memory) {
        pixulLimitWithPercentData memory _pixulLimitWithPercentData = pixulLimitWithPercentData(
            PIXEL_LIMIT,
            PLATFORM_SHARE_PERCENT_FOR_LESS,
            PLATFORM_SHARE_PERCENT_FOR_MORE
        );
        return _pixulLimitWithPercentData;
    }
    ...
    function acceptOfferBid721(acceptOfferBid721Data memory _transferData) external payable {
        ...
        uint256 pecentValue = checkPlatformSharebyPixel( _transferData.pixel);
        if(pecentValue > 0) {
            uint256 platformSharePercent = calculatePercentValue(amountToTransfer, pecentValue);
        ...
        if(_transferData.percent > 0 && _transferData.creator != msg.sender) {
    ...
    function buy721(buy721Data memory _buyData) external payable {
        ...
        uint256 pecentValue = checkPlatformSharebyPixel(_buyData.pixel);
        if(pecentValue > 0) {
            uint256 platformSharePercent = calculatePercentValue(amountToTransfer, pecentValue);
            amountToTransfer = amountToTransfer-platformSharePercent;
        ...
        if(_buyData.percent > 0 && _buyData.creator != _buyData.owner) {
    ...
    // }
    function updatePixelLimitWithPercent(uint256 lessPercent, uint256 morePercent, uint256 pixelLimit) public onlyOwner {
        PLATFORM_SHARE_PERCENT_FOR_LESS = lessPercent;
        PLATFORM_SHARE_PERCENT_FOR_MORE = morePercent;
