...
contract BatStarWarrior is ERC721Enumerable, Ownable, ReentrancyGuard {
    ...
    // sale time
    uint256 public publicSaleStartTime = 1653215400;
    uint256 public preSaleStartTime = 1653215400;
        ...
        callerIsUser
    {
        ...
        if (tokenIds.current() + _amount > MAX_SUPPLY)
    ...
    {
        require(
            isValidPreAllowlist(msg.sender, _proof),
    ...
    function claimUnclaimedAndUnsold(address recipient) external onlyOwner {
        claimUnclaimedAndUnsoldWithAmount(recipient, MAX_MINT_PER_BLOCK);
    }
    ...
    function claimUnclaimedAndUnsoldWithAmount(
        ...
        uint256 totalMinted = tokenIds.current();
        uint256 toBeMint = MAX_SUPPLY - totalMinted;
        uint256 toMint = amount < toBeMint ? amount : toBeMint;
        mintCommon(toMint, recipient);
        emit Minted(recipient, amount, 0);
    }

    function mintCommon(uint256 amount, address recipient) private {
    ...
    function isValidPreAllowlist(address _address, bytes32[] memory _proof)
        public
        view
        returns (bool)
    {
    ...
    function getRemainingSupply() public view returns (uint256) {
        return MAX_SUPPLY - tokenIds.current();
    }
