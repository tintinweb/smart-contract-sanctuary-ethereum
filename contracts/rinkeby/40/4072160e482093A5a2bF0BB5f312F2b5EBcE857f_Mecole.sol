...
contract Mecole is ERC721A, Ownable, ReentrancyGuard {
    ...
    // used to get tokenURI
    mapping(uint256 => uint256) public _idToEdition;
    mapping(address => uint256) public _addressToLatestEdition;
    ...
    modifier canMintEdition(uint256 requestedEdition) {
        ...
        require(_addressToLatestEdition[_msgSender()] + 1 >= requestedEdition, "Need earlier editions");
        require(_nextTokenId() <= _editionToMaxSupply[requestedEdition], "Not enough supply");
        require(msg.value >= _editionToPrice[requestedEdition], "Incorrect price");
        _;
    }

    constructor() ERC721A("Mecole Hardman", "MC") {
        _editionToTokenUri[0] = "edition 1";
        _editionToTokenUri[1] = "edition 2";
        _editionToTokenUri[2] = "edition 3";

        _editionToMaxSupply[0] = 10;
        _editionToMaxSupply[1] = 10;
        _editionToMaxSupply[2] = 10;

        _editionToPrice[0] = 10000000000000000;
        _editionToPrice[1] = 10000000000000000;
        _editionToPrice[2] = 10000000000000000;
    }

    function mintEdition(uint256 edition, uint256 quantity) external payable canMintEdition(edition) nonReentrant {
        if (_addressToLatestEdition[_msgSender()] < edition) {
            _addressToLatestEdition[_msgSender()] = edition;
        }
    ...
    returns (string memory)
    {
        ...
        return string(abi.encodePacked(_editionToTokenUri[_idToEdition[tokenId]], ".json"));
    }
