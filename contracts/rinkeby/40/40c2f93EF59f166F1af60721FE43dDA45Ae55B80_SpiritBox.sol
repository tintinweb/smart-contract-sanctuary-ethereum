...
contract SpiritBox is ERC721Enumerable, Ownable {
    ...
    uint256 public prvSaleCost = 5000000000000000;
    ...
    function setRandom(bool _flag) onlyOwner public {
        randomFlag = _flag;
    }
    ...
    function mint() public payable{
        ...
        uint256 tokenIdToBe;
        if(randomFlag) {
            tokenIdToBe = getRandomness(msg.sender);
        }
        else {
            tokenIdToBe = SafeMath.add(totalSupply(), 1) ;
        }
        _mint(tokenIdToBe);
    }

    // function mintId(uint256 _tokenId) public payable{
    //     require(getCost() <= msg.value);
    //     require(!isTokenIdExisting[_tokenId]);

    //     _mint(_tokenId);
    // } 

    function _mint(uint256 tokenIdToBe) internal {
        ...
        require(publicSale || isWhitelisted[currentWhiteListVersion][msg.sender], "Exception: signer is not whitelisted");
    ...
    function upgradeWhitelistVersion() external onlyOwner {
    ...
    function setSaleType(bool isPublic) external onlyOwner {
        publicSale = isPublic;
    }
