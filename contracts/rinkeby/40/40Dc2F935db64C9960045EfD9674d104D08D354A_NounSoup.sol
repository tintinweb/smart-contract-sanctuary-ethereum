...
contract NounSoup is ERC721A, Ownable {
    ...
    string public tokenDescription =
        "Noun Soups are completely on-chain pop-art. They are a derivative of Nouns, a CC0 project.";
    ...
    /// override ERV721A to have token start at 1
    ...
    function toggleMintingPaused() public onlyOwner {
        ...
        // first time we enable minting, record a seed
        if (renderSeed == 0) {
            renderSeed =
                uint256(
                    keccak256(
    ...
    function _randomizeSeed(uint256 seed) public view returns (uint256) {
        return
            uint256(keccak256(abi.encodePacked(seed, renderSeed))) %
            (MAX_TOKENS * 16);
    }

    /// @notice svg snippet for can & noun
    function _svgForCanAndNoun(uint256 seed) private view returns (string memory) {
        ...
        )
    {
        ...
        bool isStandardColors = (seed % 5) == 0; // || (seed % 7) == 0;

        // return all black very very rarely.
        ...
        return (SoupPalette(p1, s1, p2, s2), false, isStandardColors);
    }
    ...
    function _backgroundForSeed(
        ...
        string memory background = _svgRect(
            ...
            100,
            100,
            forceWhiteBG ? _DEFAULT_BOTTOM_COLOR : palette.p1
        );
        ...
        if (seed % 29 == 0) {
            ...
            background = string(
                abi.encodePacked(
                    background,
                    _svgRect(0, 50, 100, 50, palette.s1)
                )
            );
        }
    ...
    function svgForToken(uint256 tokenId_) public view returns (string memory) {
        ...
        (
            SoupPalette memory palette,
            bool isBlackLabel,
            bool isStandardColors
        ) = _paletteForSeed(seed);
        seed = _randomizeSeed(seed);
        bool forceWhiteBG = (seed % 5) == 0 || isBlackLabel || isStandardColors;

        string memory bg = isStandardColors || forceWhiteBG || isBlackLabel
            ? ""
            : _backgroundForSeed(seed, palette, forceWhiteBG);

        string memory svg1 = string(
            abi.encodePacked(
                _CAN_1,
                isBlackLabel ? _NEAR_BLACK_COLOR : _DEFAULT_BOTTOM_COLOR,
                _CAN_2,
                bg,
                _svgForCanAndNoun(seed),
                _CAN_3,
                palette.s2
            )
        );
        ...
        string memory svg3 = string(
            abi.encodePacked(
                ...
                palette.p2,
                _CAN_9,
                isStandardColors ? _BLACK_COLOR : palette.s1,
                _CAN_10
            )
        );
        ...
        returns (string memory)
    {
        ...
        bytes memory _header = abi.encodePacked(
            ...
            Base64.encode(bytes(svgForToken(tokenId_))),
            '", "external_url": "',
            tokenExternalUrl,
            '", "description": "',
            tokenDescription,
        ...
        string memory _metadata = string(
            abi.encodePacked(_header, _attributes, _footer)
        );
        return _metadata;
    }
