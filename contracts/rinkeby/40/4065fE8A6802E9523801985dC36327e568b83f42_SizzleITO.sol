...
import {Options as SizzleTokenOptions, SizzleToken} from "./SizzleToken.sol";
...
contract SizzleITO is Verifiable {
    ...
    constructor(
        ...
        token = new SizzleToken(
            SizzleTokenOptions(
                tokenName,
                tokenSymbol,
                address(this),
                opts.buckets[0].amount +
                    opts.buckets[1].amount +
                    opts.buckets[2].amount
            )
        );
    }
    ...
    function buy(address source, uint256 value) external isVerified payable returns (uint256) {
    ...
    function claim() external isVerified returns (uint256) {
...
contract SizzleToken is ERC20 {
    constructor(Options memory opts) ERC20(opts.name, opts.symbol) {
        _mint(opts.seller, opts.balance);
    }
}
