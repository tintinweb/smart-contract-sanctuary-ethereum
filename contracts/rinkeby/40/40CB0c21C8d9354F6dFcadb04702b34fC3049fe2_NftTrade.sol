...
contract NftTrade is ReentrancyGuard, Ownable{
  ...
  bytes32 private constant SALE_TYPE_HASH = keccak256("Sale(address currency,address nftContract,uint256 tokenId,uint256 quantity,uint256 price,uint256 acceptMinPrice,uint8 method,address seller,address buyer,uint256 nonce,uint256 beginTime,uint256 expireTime,uint256 maxFee)");

  bytes32 private constant OFFER_TYPE_HASH = keccak256("Offer(address currency,address nftContract,uint256 tokenId,uint256 quantity,uint256 price,uint8 method,address seller,address buyer,uint256 nonce,uint256 beginTime,uint256 expireTime)");
  ...
  function hashSale(Sale memory sale) private pure returns (bytes32) {
    return keccak256(
      abi.encode(
        SALE_TYPE_HASH,
        sale
  ...
  function _validateSale(Sale memory sale, bytes memory saleSig) private view returns (bool) {
    return hashPacked(hashSale(sale)).recover(saleSig) == sale.seller;
  }

  function _validateOffer(Offer memory offer, bytes memory offerSig) private view returns (bool) {
    return hashPacked(hashOffer(offer)).recover(offerSig) == offer.buyer;
  }
  ...
  function _deal(
    ...
    if (offer.buyer != msg.sender) {
      require(_validateOffer(offer, offerSig), "Invalid offer signature");
    }
    if (sale.seller != msg.sender) {
      require(_validateSale(sale, saleSig), "Invalid seller signature");
    }
    ...
    emit DealedDetail(
      ...
      royaltyFeeRecipient,  // roalityAccount, 
      payments[2],      // serviceFee
      adminFeeReceiver,  // serviceAccount
      latestOrderId
    );
  ...
  function convertSale(
    address[4] memory addresses, // [0 currency, 1 nftContract, 2 seller, 3 buyer]
    uint256[9] memory values // [0 tokenId, 1 quantity, 2 price, 3 acceptMinPrice, 4 method, 5 sellNonce, 6 saleBeginTime, 7 saleExpireTime, 8 maxFee]
  ) public pure returns (Sale memory) {
    Sale memory sale;
    sale.currency = addresses[0];
    sale.nftContract = addresses[1];
    sale.tokenId = values[0];
    sale.quantity = values[1];
    sale.price = values[2];
    sale.acceptMinPrice = values[3];
    sale.method = SellMethod(values[4]);
    sale.seller = addresses[2];
    sale.buyer = addresses[3];
    sale.nonce = values[5];
    sale.beginTime = values[6];
    sale.expireTime = values[7];
    sale.maxFee = values[8];
    return sale;
  }
  ...
  function convertOffer(
    address[4] memory addresses, // [0 currency, 1 nftContract, 2 seller, 3 buyer]
    uint256[7] memory values // [0 tokenId, 1 quantity, 2 price, 3 method, 4 buyerNonce, ,5 offerBeginTime,6 offerExpireTime]
  ) public pure returns (Offer memory) {
    Offer memory offer;
    offer.currency = addresses[0];
    offer.nftContract = addresses[1];
    offer.tokenId = values[0];
    offer.quantity = values[1];
    offer.price = values[2];
    offer.method = SellMethod(values[3]);
    offer.seller = addresses[2];
    offer.buyer = addresses[3];
    offer.nonce = values[4];
    offer.beginTime = values[5];
    offer.expireTime = values[6];
    return offer;
  }
  ...
  function recoverSale(
    ...
    return hashPacked(hashSale(convertSale(addresses, values))).recover(sig);
  }

  function recoverOffer(
    address[4] memory addresses,
    uint256[7] memory values,
    bytes memory sig
  ) public view returns (address) {
    return hashPacked(hashOffer(convertOffer(addresses, values))).recover(sig);
  }
  ...
  function validateSale(
    ...
    return _validateSale(convertSale(addresses, values), sig);
  }
  ...
  function validateOffer(
    ...
    return _validateOffer(convertOffer(addresses, values), sig);
  }

  function deal(
    address[6] memory addresses,
    uint256[12] memory values,
    bytes memory saleSig,
    bytes memory offerSig,
    bytes memory data
  ) public nonReentrant payable returns (uint256) {
    // address[] addresses = [0 currency, 1 nftContract, 2 sale seller, 3 sale buyer, 4 offer seller, 5 offer buyer];
    // uint256[] values = [
    //   0 tokenId, 1 quantity, 2 price, 3 acceptMinPrice, 4 method, 
    //   5 sellerNonce, 6 buyerNonce, 
    //   7 saleBeginTime, 8 saleExpireTime, 9 offerBeginTime, 10 offerExpireTime, 11 maxFee
    // ];

    Sale memory sale = convertSale([
      addresses[0], // currency
      addresses[1], // nftContract
      addresses[2],  // seller
      addresses[3]  // buyer
    ], [
      values[0], // tokenId
      values[1], // quantity
      values[2], // price
      values[3], // acceptMinPrice
      values[4], // method
      values[5], // sellNonce
      values[7], // saleBeginTime
      values[8], // saleExpireTime
      values[11] // maxFee
    ]);
    
    Offer memory offer = convertOffer([
      addresses[0], // currency
      addresses[1], // nftContract
      addresses[4],  // seller
      addresses[5]  // buyer
      ...
      values[4], // method
      values[6], // buyerNonce
      values[9], // saleBeginTime
      values[10] // offerExpireTime
    ]);
