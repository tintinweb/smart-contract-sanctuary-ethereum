...
contract NFTMintNumber is ERC721, Ownable {
    ...
    constructor(string memory _baseTokenURI,uint256 _totalSupply) ERC721("NFTMintNumber", "NFTTWO") {
        baseTokenURI = _baseTokenURI;
        TOTAL_SUPPLY  = _totalSupply;
    }


    function mintWithId(uint256 itemId)
        public
        payable
    {
        require(tx.origin == msg.sender,"CONTRACTS NOT ALLOWED TO MINT");
        require(totalMinted <= TOTAL_SUPPLY);
        uint cost = MINT_PRICE;
        require(msg.value == cost, "wrong amount paid");
        totalMinted += 1;
