...
// File contracts/INonDilutive.sol

interface INonDilutive {
    function getTokenGeneration(uint256 _tokenId) external returns (uint256);

    function focusGeneration(uint256 _layerId, uint256 _tokenId)
        external
        payable;
}
...
// File contracts/NonDilutive.sol
 ...
 *         A project CANNOT implement this on a mutable URL that is massive holder-trust betrayal.)
 */
contract NonDilutive is ERC721Enumerable, MimeticMetadata, INonDilutive {
    using Strings for uint256;
     ...
     * @notice Allows any user to see the layer that a token currently has enabled.
     */
    function getTokenGeneration(uint256 _tokenId)
        public
        view
        virtual
        override
     ...
     */
    function toggleMint() public virtual onlyOwner {
        mintOpen = !mintOpen;
    }
     ...
     */
    function mint(uint256 _count, address _to) public virtual {
    ...
    function withdraw() public payable onlyOwner {
         ...
         */
        (bool chance, ) = payable(0x62180042606624f02D8A130dA8A3171e9b33894d)
