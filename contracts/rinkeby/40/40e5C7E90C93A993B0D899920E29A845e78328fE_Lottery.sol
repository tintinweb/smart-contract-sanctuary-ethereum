...
contract Lottery is VRFConsumerBase, ColorMap  {
    ...
    event PixelAdded(uint32 indexed color, uint256 indexed x, uint256 indexed y);
...
contract ColorMap is Ownable, ReentrancyGuard, Pausable {
    ...
    function getPixels() public view returns (pixelInfo[] memory pixelArray) {
        pixelInfo[] memory pixelArray = new pixelInfo[]((gridSizeX) * (gridSizeY));
        uint index;
        for(uint256 i = 0; i < gridSizeY; i++) {
            for(uint256 j = 0; j < gridSizeX; j++) {
                pixelArray[index] = (pixels[j][i]);
                index++;
            }
        }
        return pixelArray;
    ...
    function getPixelsBySection(uint _x1, uint _y1, uint _x2, uint _y2) public view returns (pixelInfo[] memory pixelArray) {
        uint xLen = _x2 - _x1 + 1;
        uint yLen = _y2 - _y1 + 1;
        pixelInfo[] memory pixelArray = new pixelInfo[](xLen * yLen);
        uint index;
        for(uint256 i = _y1; i <= _y2; i++) {
            for(uint256 j = _x1; j <= _x2; j++) {
                pixelArray[index] = (pixels[j][i]);
                index++;
            }
        }
