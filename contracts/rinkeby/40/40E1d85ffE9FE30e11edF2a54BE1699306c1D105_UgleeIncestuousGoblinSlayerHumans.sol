	...
	PausableUpgradeable
{
		...
		nonReentrant
	{
        ...
        if (totalSupply() < FREE_MINT_CAP){
            if (totalSupply() + _amount > FREE_MINT_CAP){
                price = PRICE_PER_TOKEN * (totalSupply() + _amount - FREE_MINT_CAP);
            } else {
                price = 0;
            }
        } else {
            price = PRICE_PER_TOKEN * _amount;
        }
    ...
    function setMaxMintPerAddress(uint256 _maxTokensPerAddress) external onlyOwner {
        MAX_TOKENS_PER_ADDRESS = _maxTokensPerAddress;
    }


    function setPricePerUnit(uint256 _pricePerToken) external onlyOwner {
