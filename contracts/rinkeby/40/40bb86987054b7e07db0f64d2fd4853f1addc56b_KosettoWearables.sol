...
contract KosettoWearables is ERC1155, Ownable {
    ...
    function mint(string memory listingId, uint256 tokenId, uint256 amount, uint256 price, uint256 listingIdSupply, bytes memory signature) public {
        bytes32 hash = keccak256(abi.encodePacked(listingId, tokenId, amount, price, listingIdSupply));
        bytes32 messageHash = hash.toEthSignedMessageHash();
        address signer = messageHash.recover(signature);
        require(signer == owner(), "Not signed by owner");
        require(listingIdSupply <= listingMintedAmount[listingId] + amount , "Out of stock");
        // Check price/payable
