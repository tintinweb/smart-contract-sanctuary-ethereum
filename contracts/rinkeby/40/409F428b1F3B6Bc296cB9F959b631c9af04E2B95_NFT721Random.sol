...
import "../lib/721/ERC721.sol";
import "../lib/712/EIP712Whitelist.sol";
import "./MintSetup.sol";

contract NFT721Random is IScore, Adminable, ERC721, EIP712Whitelist, MintSetup, VRFConsumerBaseV2 {
    ...
    // keep track of number of address transfer to allow soft-staking (ignore mint, burn)
    mapping(uint256 => uint256) public tokenTransfer;
    ...
    function scoreByAddress(address _addr) external view virtual override returns (uint256) {
        return balanceOf(_addr) * baseScore();
    }
    
    // soft-staking can be done using this token transfer counter to 
    //  make sure token is keep in the same address at the whole time
    function _afterTokenTransfer(address _from, address _to, uint256 _tokenId) internal virtual override {
        super._afterTokenTransfer(_from, _to, _tokenId);

        if (_from != address(0) && _to != address(0)) {
          tokenTransfer[_tokenId] += 1;
        }
    }
 ...
 * _addressData included balance, numberMinted, numberBurned...
 *
 * Creator: Sharkz
 * Author: Jason Hoi
 * Date: 2022-07-14
...
contract ERC721 is Context, IERC721, IERC721Metadata {
    ...
    struct AddressData {
        ...
        uint64 numberBurned;
        // Keep track of whitelist counter
...
contract MintSetup is Adminable {
    ...
    //////// Public minting
    function setPublicMint(uint32 _startTime, uint256 _price) external onlyAdmin {
    ...
    //////// Presale minting
    function setPresale(uint32 _startTime, uint32 _endTime, uint _price) external onlyAdmin {
    ...
    //////// Auction minting
    function setAuction(uint32 _startTime, uint32 _endTime, uint32 _dTimeInterval, uint256 _daPriceInterval, uint256 _startPrice, uint256 _finalPrice) external onlyAdmin {
    ...
    function changeAuctionSupply(uint32 _qty) external onlyAdmin {
