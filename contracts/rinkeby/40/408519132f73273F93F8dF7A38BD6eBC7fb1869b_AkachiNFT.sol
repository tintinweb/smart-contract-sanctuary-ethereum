...
contract AkachiNFT is ERC1155Supply, Ownable, ERC1155Pausable, ERC1155Burnable {
    ...
    function mintNewToken(uint256 amount, string memory hash, uint32 royalty)
        public
        returns (uint256)
    {
        ...
        tokenRoyal.push(royalty);
        _setTokenURI(hash, newTokenId);
        emit Mint(newTokenId, amount, msg.sender);
        return newTokenId;
    }

    function getTokenRoyal(uint256 tokenId_) public view returns(uint256){
        return tokenRoyal[tokenId_];
    }
