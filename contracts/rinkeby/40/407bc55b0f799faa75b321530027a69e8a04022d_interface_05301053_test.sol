...
//metadata
    ...
    constructor() ERC721("m_05302142" , "m_05302142" ) {
...
contract interface_05301053_test is ERC721{
    using strings for *;
...
//metadata
    ...
    m_05302142 mainContract = m_05302142(0x67bBABf854E6A8F468EaFDa186EDD2d9C0e7525E);
    
    constructor() ERC721("interface_05301053_test" , "interface_05301053_test" ) {
    ...
    function setMainContruct(address _address)public onlyOwner{
        mainContract = m_05302142(_address);
    }
    ...
    function addLayers(string memory _layerInfo)public onlyOwner{
        ...
        for (uint i = 0; i < s.count(":".toSlice()) +1 ; i++) {
    ...
    function sendMint(string memory _layerInfo,string memory _onlyLayerInfo,string memory _imageIPFS)public payable {
        ...
        //24519134421 区切り文字
        {
            ...
            for (uint i = 0; i < 8 ; i++) {
                parts[i] = uintTo_StringNumToStringLayerName[i+1][su.str2uint(parts[i])];
            ...
            require(!getExist(string(abi.encodePacked(parts[2],parts[5],parts[6],parts[7]) )) );
        }
        {
            ...
            mainContract._inMint(msg.sender,imageInfo.Layer1,imageInfo.Layer2,imageInfo.Layer3,imageInfo.Layer4,imageInfo.Layer5,imageInfo.Layer6,imageInfo.Layer7,imageInfo.Layer8,imageInfo.Image) ;
        }
    }
    ...
    function test(string memory _layerInfo)public view returns(string memory)
    {
        ...
        string memory out ="";
        {
            ...
            for (uint i = 0; i < 9 ; i++) {
                ...
                part = uintTo_StringNumToStringLayerName[i+1][su.str2uint(parts[i])];
                out = string(abi.encodePacked(out,"|",part));
                //mapping string の初期値は""
                if(strings.equals( part.toSlice() ,"".toSlice() ) && i < 8){
                    require(false,string(abi.encodePacked("miss",out )) );
                }
            }
        }
        return string(abi.encodePacked("mint Completed","|",out));
    }
