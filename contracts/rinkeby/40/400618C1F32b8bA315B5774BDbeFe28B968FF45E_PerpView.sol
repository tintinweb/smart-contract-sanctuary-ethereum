...
import { IPoolRegistry } from "./interface/IPoolRegistry.sol";
...
contract PerpView is Initializable {
    ...
    // Trader view functions
    function getAllPendingFundingPayment(address trader) external view returns (int256) {
        return ITrader(_trader).getAllPendingFundingPayment(trader);
    }

    function getPendingFundingPayment(address trader, address baseToken) external view returns (int256) {
        return ITrader(_trader).getPendingFundingPayment(trader, baseToken);
    }

    function getTwapMarkPrice(address baseToken, uint32 twapInterval) external view returns (uint256) {
        return ITrader(_trader).getTwapMarkPrice(baseToken, twapInterval);
    }

    function getPnlToBeRealized(ITrader.RealizePnlParams memory params) external view returns (int256) {
        return ITrader(_trader).getPnlToBeRealized(params);
    }
    ...
    function getTotalTokenAmountInPoolAndPendingFee(
        ...
        return
            ILiquidityProvider(_liquidityProvider).getTotalTokenAmountInPoolAndPendingFee(trader, baseToken, fetchBase);
    }
    ...
    /// @return liquidityCoefficientInFundingPayment the funding payment of all orders/liquidity of a lp
    function getLiquidityCoefficientInFundingPayment(
        ...
        return
            ILiquidityProvider(_liquidityProvider).getLiquidityCoefficientInFundingPayment(
                trader,
                baseToken,
                fundingGrowthGlobal
            );
    ...
    // PoolRegistry view functions
    function getPool(address baseToken) external view returns (address) {
        return IPoolRegistry(_poolRegistry).getPool(baseToken);
    }

    function getFeeRatio(address baseToken) external view returns (uint24) {
        return IPoolRegistry(_poolRegistry).getFeeRatio(baseToken);
    }

    function getInsuranceFundFeeRatio(address baseToken) external view returns (uint24) {
        return IPoolRegistry(_poolRegistry).getInsuranceFundFeeRatio(baseToken);
    }

    function getPoolInfo(address baseToken) external view returns (IPoolRegistry.PoolInfo memory) {
        return IPoolRegistry(_poolRegistry).getPoolInfo(baseToken);
    }

    function hasPool(address baseToken) external view returns (bool) {
        return IPoolRegistry(_poolRegistry).hasPool(baseToken);
    }
    ...
    function getAccountInfo(address trader, address baseToken) external view returns (Account.TraderInfo memory) {
        return IPositionMgmt(_positionMgmt).getAccountInfo(trader, baseToken);
    }

    function getTraderOpenNotional(address trader, address baseToken) external view returns (int256) {
        return IPositionMgmt(_positionMgmt).getTraderOpenNotional(trader, baseToken);
    }
    ...
    function getTraderPositionSize(address trader, address baseToken) external view returns (int256) {
        return IPositionMgmt(_positionMgmt).getTraderPositionSize(trader, baseToken);
    }
    ...
    function getMaxPoolsPerAccount() external view returns (uint8) {
        return IClearingHouseConfig(_clearingHouseConfig).getMaxPoolsPerAccount();
    }
    ...
    function getMaxFundingRatio() external view returns (uint24) {
        return IClearingHouseConfig(_clearingHouseConfig).getMaxFundingRatio();
    }
...
interface IClearingHouse {
    ...
    /// @param quoteFee the amount of quote token the lp received as fees
    ...
    function cancelOpenOrder(address lp, address baseToken) external;
...
interface IPositionMgmt {
    ...
    /// @param baseToken The address of the pool's base token
    ...
    /// @return pendingFee the pending fee of lp earned
    function getPnlAndPendingFee(address trader)
        external
        view
        returns (
    ...
    /// @return sum up positions value of every pool, it calls `getTotalPositionValue` internally
    function getTotalAbsPositionValue(address trader) external view returns (uint256);
}
...
interface ITrader {
    /// @param amount when closing position, amount(uint256) == traderPositionSize(int256),
    ///        as amount is assigned as traderPositionSize in ClearingHouse.closePosition()
    struct SwapParams {
        address trader;
        address baseToken;
        bool isBaseToQuote;
    ...
    /// @return fundingPayment the funding payment of a trader in one pool should be settled into owned realized Pnl
    ...
    /// @return the pending funding payment of a trader in one pool, including liquidity & balance coefficients
    function getPendingFundingPayment(address trader, address baseToken) external view returns (int256);
...
interface ILiquidityProvider {
    ...
    event TraderChanged(address indexed trader);
    ...
    ///      the return value includes lp fee.
    ...
    function updateExChangeFee(address baseToken, uint256 fee) external;
}
...
interface IPoolRegistry {
    ...
    event PoolAdded(address indexed baseToken, address indexed pool);
    ...
    function getPoolInfo(address baseToken) external view returns (PoolInfo memory);
...
library Account {
    struct TraderInfo {
        int256 traderPositionSize;
        int256 traderOpenNotional;
        int256 lastTwPremiumGrowthGlobal;
    }
...
library Funding {
    ...
    function calcPendingFundingPaymentWithLiquidityCoefficient(
        ...
        int256 pendingFundingPayment = (liquidityCoefficientInFundingPayment + balanceCoefficientInFundingPayment) /
            _DEFAULT_FUNDING_PERIOD;
