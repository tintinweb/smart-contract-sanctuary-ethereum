...
contract PoolManager is OwnableUpgradeable, IPoolFactory, IPoolManager {
  ...
  function payAmaPartner(
     ...
     for (uint256 i = 0; i < _partnerAddress.length; i++) {
        ...
        (bool success , ) = payable(_partnerAddress[i]).call{value: amapartner[_partnerAddress[i]].fees}("");    
   ...
   function getCumulativePoolInfo(uint256 start, uint256 end)
        ...
        returns (CumulativeLockInfo[] memory)
    {
        ...
        for (uint256 i = start; i <= end; i++) {
            (address token , uint8[] memory saleType , uint256[] memory info , string memory name , string memory symbol) = IPool(_pools.at(i)).getPoolInfo();
        ...
        returns (CumulativeLockInfo[] memory)
    {
        ...
        for (uint256 i = start; i <= end; i++) {
            (address token , uint8[] memory saleType , uint256[] memory info , string memory name , string memory symbol) = IPool(_contributedPoolsOf[user].at(i)).getPoolInfo();
            lockInfo[currentIndex] = CumulativeLockInfo(
                _pools.at(i),
                token,
                saleType[0],
