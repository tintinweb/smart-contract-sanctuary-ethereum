...
contract WOWD is ERC721A, Ownable, ReentrancyGuard {
    using Strings for uint256;
    ...
    modifier mintCompliance(uint256 _mintAmount) {
        require( //check if there is not too much mint at one time
            _mintAmount > 0 && _mintAmount <= MAX_MINT_AMOUNT_PER_TX,
            "Invalid mint amount!"
        ); //check if there is enought supply
        require(
    ...
    address private payoutAddress1 = 0xd67c0fa3fDEcc260bD663e49396420E53b120a1c;
    ...
    function setPayoutAddress(address addy, uint256 id) external onlyOwner {
        if (id == 0) {
            payoutAddress1 = addy;
        }
    }
