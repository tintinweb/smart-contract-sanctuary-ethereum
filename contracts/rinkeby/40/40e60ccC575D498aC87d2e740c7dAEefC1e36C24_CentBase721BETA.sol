...
import "../Whitelist/CentBaseWhitelistBETA.sol";

/// @title CentBase721BETA.
/// @author Dadogg80. @Viken Blockchain Solutions
/// @notice This is the base smart contract used to mint NFT's for Centaurify's BETA launch.
...
contract CentBase721BETA is ERC721, ERC721URIStorage, ERC721Royalty, Ownable, CentBaseWhitelistBETA {
    ...
    /// @dev The default royalty fee is set to 0% of sales price.
    uint96 public _defaultRoyaltyFee = 0;

    /// @dev Emitted by the { splitRoyalties() } method.
    event RoyaltySplitter(address RoyaltySplitter);
    event Minted(uint256 indexed TokenId, string tokenURI, address indexed to, address indexed RoyaltyReceiver, uint96 RoyaltyFee);
    event NewTokenAndSplitter(uint256 indexed TokenId, string tokenURI, address indexed to, address indexed RoyaltySplitter);

    /// @dev _contractURI Used by OpenSea's URI method.
    /// @dev ref. { https://docs.opensea.io/docs/contract-level-metadata } 
    constructor(string memory _contractUri) ERC721("CentBase721BETA", "") {
        contractUri = _contractUri;
    }
    
    /// @notice Mint a new token.
    /// @dev Can only be called by whitelisted accounts during the BETA release.
    /// @param to The receiver of the token to be minted. 
    ...
    function mint(address to, string memory _tokenURI, address payable royaltyAddress, uint96 royaltyFee) external onlyWhitelistedUsers {
        _tokenIdCounter.increment();
        uint256 tokenId = _tokenIdCounter.current();

        super._safeMint(to, tokenId);
        super._setTokenURI(tokenId, _tokenURI);
        super._setTokenRoyalty(tokenId, royaltyAddress, royaltyFee);
    ...
    /// @param to The account of the reciever of the minted token.
    /// @param _tokenURI The uri pointing to the metadata of this token.
    /// @param _recipients Array of accounts to split the royalty amount between.
    /// @param _shares Array of shares correlating to the recipient array. total shares should amount to 10000 shares. 
    /// @dev Example of shares inputs: 250 = 2,5%, 500 = 5%, 1000 = 10%, etc.  
    /// @param royaltyFee The royalty fee as percentage points. 1000 = 10% .
    function mintWithMultiRoyalty(address payable to, string memory _tokenURI, address[] memory _recipients, uint[] memory _shares, uint96 royaltyFee) external payable onlyWhitelistedUsers {
        _tokenIdCounter.increment();
        uint256 tokenId = _tokenIdCounter.current();
        super._safeMint(to, tokenId);
        super._setTokenURI(tokenId, _tokenURI);
        ...
        (address royaltySplitter) = splitRoyalties(_recipients, _shares, tokenId);
        super._setTokenRoyalty(tokenId, payable(royaltySplitter), royaltyFee);

        emit NewTokenAndSplitter(tokenId,  _tokenURI, to, royaltySplitter);
    }
    ...
    /// @notice Admin method used to set the defaultRoyaltyFee.
    /// @param defaultRoyaltyFee The default royalty fee to charge secondary market sale.
    /// @dev Example input: 10 = 0.1%, 250 = 2,5%, 500 = 5%, 1000 = 10%, 1500 = 15%.
    function setDefaultRoyalty(uint96 defaultRoyaltyFee) external onlyOwner {
        _defaultRoyaltyFee = defaultRoyaltyFee;
    }

    /// @notice Admin method to set the tokenURI of a specific token.
    /// @param _tokenId The id of the token.
    /// @param _tokenURI The URI to the metadata of this token id. 
    ...
    function setContractURI(string memory _contractUri) internal onlyOwner {
        contractUri = _contractUri;
    }
    ...
    /// @notice Private method for deploying a paymentsplitter smartcontract.
    ...
    function splitRoyalties(address[] memory recipients, uint256[] memory shares, uint salt) private returns (address payable royaltyContract) {
        PaymentSplitter royaltySplitter = new PaymentSplitter{salt: bytes32(salt)}(recipients,shares);

        emit RoyaltySplitter(address(royaltySplitter));
        return royaltyContract;
    }
...
contract CentBaseWhitelistBETA is Ownable {
    ...
    constructor() {
        isWhitelistedUser[payable(address(msg.sender))] = true;
        whitelisted_users.push(msg.sender);
    }
    ...
    function whitelistUsers(address[] calldata accounts) external onlyOwner {
    ...
    function whitelistCollections(address[] calldata collections) external onlyOwner {
