...
contract PartyUnicoinBarn is Ownable, IERC721Receiver {
    ...
    function setSimps(address _partyUnicorn) external onlyOwner {
        partyUnicorn = ISimps(_partyUnicorn);
    }
    ...
    function addTokensToStake(address account, uint16[] calldata tokenIds) external {
        require(account == msg.sender || msg.sender == address(partyUnicorn), "You do not have a permission to do that");

        for (uint i = 0; i < tokenIds.length; i++) {
            if (msg.sender != address(partyUnicorn)) {
                // dont do this step if its a mint + stake
                require(partyUnicorn.ownerOf(tokenIds[i]) == msg.sender, "This NTF does not belong to msg.sender address");
                partyUnicorn.transferFrom(msg.sender, address(this), tokenIds[i]);
            } else if (tokenIds[i] == 0) {
                continue; // there may be gaps in the array for stolen tokens
            }

            if (partyUnicorn.isQueen(tokenIds[i])) {
    ...
    function _claimFromSimp(uint16 tokenId, bool unstake) internal returns (uint owed) {
        ...
        if (unstake) {
            ...
            partyUnicorn.safeTransferFrom(address(this), msg.sender, tokenId, "");
        } else {
    ...
    function _claimFromQueen(uint16 tokenId, bool unstake) internal returns (uint owed) {
        require(partyUnicorn.ownerOf(tokenId) == address(this), "This NTF does not belong to contract address");
