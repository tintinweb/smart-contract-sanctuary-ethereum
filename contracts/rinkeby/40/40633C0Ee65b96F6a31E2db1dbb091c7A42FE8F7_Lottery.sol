...
contract Lottery is Ownable, VRFConsumerBaseV2 {
  ...
  uint256 public lottoEntryFee = 10**18 * (1 / 2);
  ...
  function enterLotto(uint256 _entries) external {
    _enterLotto(msg.sender, msg.sender, _entries);
  }

  function enterLottoForUser(address _user, uint256 _entries) external {
    ...
    internal
  {
    ...
    uint256 _amountWon = getCurrentLotteryRewardAmount();
  ...
  function getNumberLotteries() public view returns (uint256) {
    return lotteries.length;
  }

  function getCurrentLotteryRewardAmount() public view returns (uint256) {
    uint256 _participants = getCurrentLotterEntries();
  ...
  function getCurrentLotterEntries() public view returns (uint256) {
    return lottoParticipants[getCurrentLottery()].length;
  }
