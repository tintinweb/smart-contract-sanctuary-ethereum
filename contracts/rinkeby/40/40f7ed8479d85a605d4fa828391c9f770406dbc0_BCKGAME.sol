...
contract BCKGAME {
    ...
    event Upgrade(uint256 _tokenId, uint8 _level);
    ...
    function plant(uint256 _tokenId, uint8 _position) public {
        ...
        if (game.isTemp) {
            require(land.lastHarvestAt == 0, "game is temp");
        }
    ...
    function harvest(uint256 _tokenId, uint8 _position) public {
        ...
        require(
            block.timestamp > land.lastHarvestAt + epoch,
            "Error: lastHarvestAt epoch"
        );
    ...
    function harvestAll(uint256 _tokenId) public {
        GameInfo memory game = gameInfo[_tokenId];
        require(game.status, "game is not started");
    ...
    function _harvest(uint256 _tokenId, uint8 _position) private {
        ...
        if (reward > 0) {
            ...
            if (land.times > 0) {
                ...
                land.times -= 1;
                land.lastHarvestAt = block.timestamp;
    ...
    function rewardAmount(uint256 _tokenId, uint8 _position)
        ...
        returns (uint256)
    {
        LandInfo memory land = landInfo[_tokenId][_position];
        if (block.timestamp < land.lastHarvestAt + epoch) {
            return 0;
        }
