...
contract AVN_test is IAVN, IERC777Recipient, Owned {
  ...
  IAVN immutable internal originalAvn;
  bool internal avnInitialised;
    ...
    external
  {
    ...
    for (uint256 i; i < t1Address.length; i++) {
      ...
      require(address(bytes20(uint160(uint256(keccak256(abi.encodePacked(t1PublicKey)))))) == t1Address[i], "T1 account mismatch");
      ...
      numActiveValidators++;
      emit LogValidatorActivated(t1Address[i], t2PublicKey[i], 0, nextValidatorId);
      nextValidatorId++;
    }
  ...
  function recoverERC777TokensFromLegacyTreasury(address erc777Address)
    ...
    external
  {
    uint256 originalAvnBalance = IERC777(erc777Address).balanceOf(address(originalAvn));
    originalAvn.unlockERC777Tokens(erc777Address, address(this), originalAvnBalance);
  }
  ...
  function recoverERC20TokensFromLegacyTreasury(address erc20Address)
    ...
    external
  {
    uint256 originalAvnBalance = IERC20(erc20Address).balanceOf(address(originalAvn));
    originalAvn.unlockERC20Tokens(erc20Address, address(this), originalAvnBalance);
  }
  ...
  function recoverETH(uint256 amount)
    ...
    external
  {
    originalAvn.unlockETH(payable(address(this)), amount);
  }

  receive() payable external {
    require(msg.sender == address(originalAvn), "Only accept ETH without data from original AVN");
  }
    ...
    external
  {
    ...
    originalAvn.storeT2TransactionId(t2TransactionId);
    ...
    external
  {
    ...
    originalAvn.storeT2TransactionId(t2TransactionId);
    originalAvn.storeRootHash(rootHash);
  ...
  function storeLiftProofHash(bytes32 proofHash)
    ...
    external
  {
    originalAvn.storeLiftProofHash(proofHash);
  }
  ...
  function storeLoweredLeafHash(bytes32 leafHash)
    ...
    external
  {
    originalAvn.storeLoweredLeafHash(leafHash);
  }
  ...
  function getIsPublishedRootHash(bytes32 rootHash)
    ...
    returns (bool)
  {
    return originalAvn.getIsPublishedRootHash(rootHash);
  }
    ...
    external
  {
    if (msg.sender != approver) {
      originalAvn.storeLiftProofHash(keccak256(proof));
  ...
  function tokensReceived(address /* operator */, address from, address to, uint256 amount, bytes calldata data,
    ...
    external
  {
    if (from == address(originalAvn)) return; // recovering funds from the original AVN so we don't lift here
    ...
    private
  {
    ...
    for (uint256 i; i < numConfirmations; i++) {
      ...
      if (v != 27 && v != 28 || uint256(s) > 0x7FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF5D576E7357A4501DDFE92F46681B20A0) {
        ...
        if (isActiveValidator[id] == false && isRegisteredValidator[id]) {
