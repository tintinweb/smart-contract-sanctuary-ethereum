...
contract Truplay is ERC721, Ownable, Pausable {
    /**
        ...
        E#7: Message value not equal to whitelist nft price
    */
    event SaleState(bool isSaleActive);
    event MintSuccessful(uint256 tokenId, uint256 nftPrice);
    event ReserveMintSuccessful(uint256 tokenId);
    event WhitelistMintSuccessful(uint256 tokenId, uint256 nftPrice);
    ...
    function safeMint() public payable whenNotPaused checkIsSaleIsActive checkTokenCounter {
        require(msg.value == nftPrice, "E#2");

        _tokenIdCounter.increment();
        uint256 tokenId = _tokenIdCounter.current();
        _safeMint(msg.sender, tokenId);
        emit MintSuccessful(tokenId, nftPrice);
    }
        ...
        checkTokenCounter
    {
        ...
        require(msg.value == nftPrice / 2, "E#7");

        claimedTokensViaWhitelist[msg.sender]++;
        _tokenIdCounter.increment();
        uint256 tokenId = _tokenIdCounter.current();
        _safeMint(msg.sender, tokenId);
        emit WhitelistMintSuccessful(tokenId, nftPrice / 2);
    }
