...
contract test is ERC721Enumerable, Ownable, ERC721Burnable, ERC721Pausable {
    ...
    uint256 private constant MAX_SUPPLY = 3353; // 3353 test
    ...
    address private fundWallet = payable(0x3546C7148e96c1a51C5fA0cB294CAe665B4f0FB4); // Owner Fund Wallet Address

    event DoodleFriendSummoned(address to, uint256 indexed id);

    modifier saleIsOpen {
        if (_msgSender() != owner()) {
            require(SALE_OPEN == true, "SALES: Please wait a big longer before buying test ;)");
        }
    ...
    function mint(address payable _to, uint256[] memory _ids) public payable saleIsOpen {
        ...
        require(total <= maxNFTSupply, "MINT: Please go to the Opensea to buy test.");
        require(_ids.length <= maxMintCount, "MINT: Current count exceeds maximum mint count.");
        require(msg.value >= price * _ids.length, "MINT: Current value is below the sales price of test");
