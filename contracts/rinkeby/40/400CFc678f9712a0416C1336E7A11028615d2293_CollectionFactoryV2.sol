...
contract CollectionFactoryV2 is  Initializable,OwnableUpgradeable,PausableUpgradeable{
    ...
    mapping(address => uint[]) public _userCollections;
    // Mapping to store collection address
    // Collection Id => collection address
    mapping(uint => address) public _collections;
    ...
    function initialize(address _marketplace) initializer external{
        __Ownable_init();
        __Pausable_init();
    ...
    function createCollection( string memory name, string memory symbol) public returns (address){
        ...
        collection.initialize(name,symbol,msg.sender,marketplace);
        uint id = _proxyIdCounter.current();
        _proxyIdCounter.increment();
        _collections[id]=(address(collection));
        _userCollections[msg.sender].push(id);
        emit CollectionCreated(msg.sender, address(collection), name, symbol,block.timestamp);
     ...
     */
    function removeCollection(uint id) public{
        // Check conditions
...
contract CollectionV2 is Initializable,ReentrancyGuardUpgradeable,OwnableUpgradeable,PausableUpgradeable,CBRNFTDefault{
    using CountersUpgradeable for CountersUpgradeable.Counter;
    CountersUpgradeable.Counter private _tokenIdCounter;

   function initialize (string memory name, string memory symbol,address _admin, address _marketplace)  public override initializer{
        CBRNFTDefault.initialize(name, symbol, _admin,_marketplace);
    ...
    function __CollectionV2_init(address _admin) internal initializer {
        transferOwnership(_admin);
        __CollectionV2_init_unchained(_admin);
    }

    function __CollectionV2_init_unchained(address _admin) internal initializer {}


    function safeMint(address to ,string memory tokenURI,address creator, uint256 value) onlyOwner override public  returns(uint){
...
contract CBRNFTDefault is Initializable, ERC721Upgradeable, ERC721EnumerableUpgradeable, ERC721URIStorageUpgradeable, PausableUpgradeable, AccessControlUpgradeable, ERC721BurnableUpgradeable,Royalties {
   using CountersUpgradeable for CountersUpgradeable.Counter;
    CountersUpgradeable.Counter private _tokenIdCounter;
    ...
    function initialize(string memory name, string memory symbol,address _admin, address _marketplace) public virtual initializer{
    ...
    function __CBRNFTDefault_init(address _marketplace) internal initializer {
        
        __CBRNFTDefault_init_unchained(_marketplace);
    }

    function __CBRNFTDefault_init_unchained(address _marketplace) internal initializer {}
  ...
  function safeMint(address to ,string memory uri,address creator, uint256 value) public virtual onlyRole(MINTER_ROLE) returns(uint){
     ...
     function safeMint(address to,uint tokenId,string memory uri,address creator, uint256 value) internal onlyRole(MINTER_ROLE) returns(uint){
        _safeMint(to, tokenId);
        _setTokenURI(tokenId, uri);
         if(value>0){
            _setTokenRoyalty(tokenId,creator,value);
