...
contract RedeemableToken is ERC1155, Ownable, ERC1155Burnable {    
    constructor() ERC1155("https://rtfkt.mypinata.cloud/ipfs/QmRbUSDAFLf5iEuU9v49Vt7YppyCjGsJQMxqrt6PWF5RF6/1") {}

    uint256 public currentSupply = 0;
    uint256 public supplyLimit = 10000;
    mapping (address => mapping (uint256 => address)) redeemedToken;
    address forgingContractAddress = 0xaa74fe2e99aB350508bBB2531b349e028C385881;
    address redemptionMiddlewareContract = 0x495524Cfe2d9A9Ba0666cDE34139d2966626149C;
    address authorizedCollection = 0xddE6627574A43Dfe7D8cCc8A3112B71B7f6d37b7;
    ...
    function redeem(address owner, address initialCollection, uint256 tokenId) public payable returns(uint256) {
        ...
        require(currentSupply + 1 <= supplyLimit, "Limit reached");
    ...
    // Forge function
    function forgeToken(uint256 amount) public {
        require(balanceOf(msg.sender, 1) >= amount, "Doesn't own the token"); // Check if the user own one of the ERC-1155
        burn(msg.sender, 1, amount); // Burn one the ERC-1155 token
        ForgeTokenContract forgingContract = ForgeTokenContract(forgingContractAddress);
        forgingContract.forgeToken(amount, 1, msg.sender); // Mint the ERC-721 token
    }
