...
contract AnonymiceBreedingDescriptor is Ownable {
    /*
...
*/
        ...
        returns (string memory)
    {
        ...
        for (uint8 i = 0; i < 9; i++) {
            ...
            for (
                ...
                uint16 x = letterToNumber(substring(thisPixel, 0, 1));
                uint16 y = letterToNumber(substring(thisPixel, 1, 2));
    ...
    {
        if (
            IAnonymiceBreeding(ANONYMICE_BREEDING_ADDRESS)._tokenToRevealed(
                _tokenId
            )
        ) {
            ...
            if (
                IAnonymiceBreeding(ANONYMICE_BREEDING_ADDRESS)
                    ._tokenIdToLegendary(_tokenId)
            ) {
                uint8 legendaryId = IAnonymiceBreeding(
                    ANONYMICE_BREEDING_ADDRESS
        ...
        if (
            ...
            return
                string(
                    abi.encodePacked(
                        SVG_PIECE,
                        IAnonymice(ANONYMICE_ADDRESS).fpi(
                        ...
                        '</svg> <svg x="222" y="204" width="84" height="84">',
                        IAnonymice(ANONYMICE_ADDRESS).fpi(
                            IAnonymice(ANONYMICE_ADDRESS)._tokenIdToHash(
                                IAnonymiceBreeding(ANONYMICE_BREEDING_ADDRESS)
                                    ._tokenToIncubator(_tokenId)
                        ...
                        '</svg> <svg width="300" height="100" x="32" y="4.5">  <text x="50%" y="50%" dominant-baseline="middle" text-anchor="middle" style="font-size: 24px;fill: #d5eef5;font-family: PixelFont;">#',
                        toString(_tokenId),
                        '</text> </svg> <svg width="300" height="80" x="32" y="76">  <text x="50%" y="50%" dominant-baseline="middle" text-anchor="middle" style="font-size: 16px;fill: #d5eef5;font-family: PixelFont;">',
                        toString(
                            blocksTill(
                                IAnonymiceBreeding(ANONYMICE_BREEDING_ADDRESS)
                                    ._tokenToIncubator(_tokenId)
                        ...
                        '</text> </svg> <svg width="160" height="80" x="18" y="297.5">  <text x="50%" y="50%" dominant-baseline="middle" text-anchor="middle" style="font-size: 20px;fill: #d5eef5;font-family: PixelFont;">',
                        toString(
                            IAnonymiceBreeding(ANONYMICE_BREEDING_ADDRESS)
                                ._tokenToIncubator(_tokenId)
                                .parentId1
                        ),
                        '</text> </svg> <svg width="160" height="80" x="186" y="297.5">  <text x="50%" y="50%" dominant-baseline="middle" text-anchor="middle" style="font-size: 20px;fill: #d5eef5;font-family: PixelFont;">',
                        toString(
                            IAnonymiceBreeding(ANONYMICE_BREEDING_ADDRESS)
                                ._tokenToIncubator(_tokenId)
                                .parentId2
