...
contract RarePepeBirds is ERC721A, AccessControl {
    uint256 public constant MAXIMUM_SUPPLY = 10000;
    uint64 public constant PRESALE_FREE_CLAIM = 2;
    uint64 public constant PUBLIC_FREE_CLAIM = 1;
    ...
    modifier phaseCompliant() {
        require(
            salePhase == Types.SalePhase.PUBLIC ||
                (salePhase == Types.SalePhase.PRESALE && allowlist[msg.sender]),
            "INVALID SALE PHASE"
        );
        _;
    }

    constructor(address[] memory admins) ERC721A("Rare Pepe Birds", "RPB") {
    ...
    function freeMint() external phaseCompliant {
        require(_getAux(msg.sender) == 0, "ALREADY CLAIMED");

        uint64 quantity = salePhase == Types.SalePhase.PUBLIC
            ? PUBLIC_FREE_CLAIM
            : PRESALE_FREE_CLAIM;
        ...
        _setAux(msg.sender, quantity);
        _mintTo(msg.sender, quantity);
    }

    function mint(uint256 quantity) external payable phaseCompliant {
        require(
