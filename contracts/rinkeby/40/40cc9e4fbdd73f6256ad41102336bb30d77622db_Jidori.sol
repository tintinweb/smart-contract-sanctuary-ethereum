...
contract Jidori is Ownable, ERC721A, ReentrancyGuard {
  ...
  function preSaleMint(uint256 quantity, bytes32[] calldata proof) external payable {
    ...
    require(
        quantity * preSaleMaxMint < msg.value,
    ...
    refund(quantity * preSalePrice);
  }

  function publicSaleMint(uint256 quantity) external payable {
    require( !paused, "Sale paused or not start." );
    require( !isPreSale && isPublicSale, "publicSale not started." );
    require(
    ...
    require(
      numberMinted(msg.sender) + quantity <= publicSaleMaxMint,
      "You mint too many."
    );

    _safeMint(msg.sender, quantity);

    refund(quantity * publicSalePrice);
  }
