 ...
 * 
 * Up to two discounts for users who won or bought
 * our our airdropped NFT tokens, Omnihorse Riding Equipment, listed on Opensea
...
contract OmnihorseNFT is ERC721A, Ownable, ERC721ABurnable, ERC721AQueryable {
    ...
    IGetPrice public priceContract;

    modifier forSale {
        require(saleEnabled, "Sale not enabled");
        //require(block.timestamp >= saleStart && block.timestamp < saleEnd, "Not within sale period");
      _;
    }
    ...
    /**
     * @dev Set price controller
     */
    function setPriceContract(address priceContract_) public onlyOwner {
        priceContract = IGetPrice(priceContract_);
    }

    /**
     * @dev Set sale start and end time
     */
    function setStartStop(uint256 saleStart_, uint256 saleEnd_) public onlyOwner {
        require(saleEnd_ > saleStart_, "End must be after start");
     ...
     */
    function setDefaultSale(bool defaultSale_) public onlyOwner {
        require(defaultSale_ != defaultSale, "defaultSale already has that value");
        defaultSale = defaultSale_;
    }

    /**
     * @dev Get price from price controller contract, and then save NFT voucher and user states
     *
     * @return value in wei for user user_ to buy amount_ of NFTs
     */
    function getPrice(address user_, uint256 amount_) internal returns(uint256) {
        require(amount_ > 0, "Cannot mint zero");
        return priceContract.getPrice(user_, amount_);
    }

    /**
     * @dev Get price from controller contract without changing state
     *
     * @return value in wei for user user_ to buy amount_ of NFTs
     */
    function getOnlyPrice(address user_, uint256 amount_) public view returns(uint256) {
        require(amount_ > 0, "Cannot mint zero");
        return priceContract.getOnlyPrice(user_, amount_);
    }

    /**
     * @dev Set the state of one or many specific NFT vouchers
     *
     * @param user_ who claimed the airdrop NFT voucher
     * @param tokenId_ target address that will receive the tokens
     * @param state_ new state of airdrop voucher NFT
     */
    function setUsed(address user_, uint256[] calldata tokenId_, bool state_) public onlyOwner {
        priceContract.setUsed(tokenId_, user_, state_);
    }

    /**
     * @dev Check the status of a specific NFT voucher
     *
     * @param tokenId_ target airdrop NFT voucher id
     * @return airdrop NFT voucher state
     */
    function isUsed(uint256 tokenId_) external view returns(bool) {
        return priceContract.isUsed(tokenId_);
    }

    /**
     * @dev Get all Data info about a specific NFT voucher
     *
     * @param tokenId_ NFT voucher status recorded by the price controller
     * @return tuple values of Used token
     */
    function getData(uint256 tokenId_) external view returns(IGetPrice.used memory) {
        return priceContract.getData(tokenId_);
    }

    /**
     * @dev Get all User info about a specific user
     *
     * @param user_ account recorded by the price controller
     * @return tuple values of User account
     */
    function getUser(address user_) external view returns(IGetPrice.user memory) {
        return priceContract.getUser(user_);
    }
     ...
     */
    function toggleSaleEnable(bool state_) public onlyOwner {
     ...
     */
    function batchMint(uint256 amount_) public payable forSale {
        require(amount_ > 0, "Cannot mint zero");
        require(maxSupply >= currentSupply + amount_, "Insufficent supply");
        uint256 price = defaultSale ? getDefaultPrice(amount_) : getPrice(msg.sender, amount_);
        require(msg.value >= price, "Insufficent input amount");
        currentSupply += amount_;
    ...
    /**
     * @dev Get the lowest indexed NFT id that a user owns
     *
     * @param owner target account to check
     * @return tokenId The first index of a users owned NFTs
     */
    ...
    /**
     * @dev Batch transfer using specific NFT ids
     *
     * @param _address receiving addresses
     * @param _id ids of NFTs sent
     */
    function batchTransfer(address[] calldata _address, uint256[] calldata _id) public {
      require(_address.length == _id.length, "address and id must be equal size");
      uint256 length = _address.length;
      for(uint256 i=0; i<length;) {
        transferFrom(msg.sender, _address[i], _id[i]);
        unchecked { ++i; }
      }
    }
