...
contract AKCB is ERC721, Ownable {
	...
	uint32 private maxMintAmount = 20;
	uint32 private nftPerAddressLimit = 20;
	...
	constructor() ERC721("A Kid Called Beast", "AKCB") {
		setBaseURI("ipfs://QmdSFuHLeWxyKuGn3i3R8VuDZDsgnGvAR8yDKSdKAdE4ts/");
	...
	function mintNFT(uint _typeB, uint _typeG, bytes32[] memory proof) external payable {
		...
		require(_typeB <= 10 && _typeG <= 2,"Overflow");
   ...
   function getInformations() external view returns (string memory,uint256[(MAX_TOKENS/MAX_EACH_TYPE) + 1] memory,uint256[(MAX_TOKENS/MAX_EACH_TYPE) + 1] memory, uint, uint, uint,uint,uint, bool,bool)
   {
		...
		return (CURR_ROUND_NAME, tokenIdsMale,tokenIdsFemale, CURR_ROUND_TIME,CURR_MINT_COST,maxMintAmount,nftPerAddressLimit, totalSupply(), hasSaleStarted, onlyWhitelisted);
   }
	...
	function setNewRound(uint cost, string memory name, uint32 perTransactionLimit, uint32 perAddressLimit, uint theTime, bool isOnlyWhitelisted, bool saleState) external onlyOwner {
		CURR_MINT_COST = cost;
