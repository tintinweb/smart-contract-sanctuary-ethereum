...
abstract contract ERC1155SingleTokenPausable {
    ...
    function transferFrom(address from, address to, uint256 amount) public {
        ...
        require(msg.sender == from || isApprovedForAll[from][msg.sender], "Lacks Permissions");
    ...
    function safeTransferFrom(address from, address to, uint256 amount, bytes calldata data) public {
        ...
        require(to.code.length == 0 ? to != address(0)
                : ERC1155TokenReceiver(to).onERC1155Received(msg.sender, from, 0, amount, data) ==
    ...
    function _safeMint(address to, uint256 amount, bytes memory data) internal {
        _mint(to, amount);
        require(to.code.length == 0 ? to != address(0)
                : ERC1155TokenReceiver(to).onERC1155Received(msg.sender, address(0), 0, amount, data) ==
                    ERC1155TokenReceiver.onERC1155Received.selector,
            "Unsafe Destination"
        );
    }
...
contract RocketPass is ERC1155SingleTokenPausable {
    ...
    uint256 public constant ethMLPrice = .069 ether;

    uint256 public stateHashMint;
    uint256 public stateEthMint;

    bytes32 public RLMerkleRoot;
    string public passURI = "ipfs://Qmd9FvmBCLyZr7Bj4rc7iZ1x9fDivUScex6ygbDqtU7oiG";
    ...
    constructor(address _hContract, address _ogContract) ERC1155SingleTokenPausable("RocketPass", "RKTPASS"){
    ...
    function mintRL(bytes32[] calldata _proof) external payable onlyHuman {
        ...
        require(msg.value >= ethMLPrice, "Not enough Eth attached");
