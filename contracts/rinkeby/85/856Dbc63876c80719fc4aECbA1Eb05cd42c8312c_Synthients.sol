...
contract Synthients is Ownable, ERC721A, ReentrancyGuard {
    struct SaleConfig {
        uint256 presaleStartTime;
        uint256 presalePeriod;
        uint256 publicsalePeriod;
    ...
    mapping(address => uint256) public numberMintedInPresale;
    mapping(address => uint256) public numberMintedInPublicSale;

    uint256 private reserved = 0; // Total amount of tokens for teamMint

    constructor(uint256 maxBatchSize_, uint256 collectionSize_)
        ERC721A("Synthients", "Synthients NFT", maxBatchSize_, collectionSize_)
    {}
    ...
    modifier isFinished() {
        require(saleConfig.isFinished == true, "Sale is not finished yet");
        _;
    }
    ...
    modifier onlyWhiteList() {
        require(
            (block.timestamp >=
                (saleConfig.presaleStartTime + saleConfig.presalePeriod)) ||
                (block.timestamp <
                    (saleConfig.presaleStartTime + saleConfig.presalePeriod) &&
                    allowList[msg.sender] == true),
            "The caller is not in WhiteList"
        );
        _;
    }
    ...
    function setSaleInfo(
        ...
        uint256 _publicsaleMaxMint
    ) external onlyOwner {
        saleConfig.presaleStartTime = timestamp;
        saleConfig.presalePeriod = presalePeriod;
        ...
        saleConfig.presaleMaxMint = _presaleMaxMint;
    ...
    function mint(uint256 quantity) external payable onlyWhiteList {
        ...
        uint256 _publicsaleStartTime = saleConfig.presaleStartTime +
            saleConfig.presalePeriod;
        uint256 _presalePrice = saleConfig.presalePrice;
        uint256 _publicSalePrice = saleConfig.publicsalePrice;
        uint256 _presaleMaxMint = saleConfig.presaleMaxMint;
        uint256 _publicsaleMaxMint = saleConfig.publicsaleMaxMint;

        require(_saleStartTime != 0, "saleConfig is needed.");
        require(
            uint256(block.timestamp) >= _saleStartTime,
        ...
        if (uint256(block.timestamp) < _publicsaleStartTime) {
            require(
                numberMintedInPresale[msg.sender] + quantity <= _presaleMaxMint,
                "can not mint this many in presale"
            );
            numberMintedInPresale[msg.sender] += quantity;
            ...
            numberMintedInPublicSale[msg.sender] += uint32(quantity);
    ...
    function teamMint() external onlyOwner {
        ...
        require(
            totalSupply() + reserved <= collectionSize,
            "reached max supply"
        );
    ...
    function withdrawMoney() external onlyOwner nonReentrant isFinished {
