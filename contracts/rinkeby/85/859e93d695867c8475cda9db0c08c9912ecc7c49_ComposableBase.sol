    ...
    ReentrancyGuard
{
    ...
    function renderImagesBasedOnChildToken(ChildToken[] memory _childTokens)
        public
        view
        returns (string memory)
    {
        ...
        for (uint256 i; i < _childTokens.length; ++i) {
            ...
            string memory layerUri = ITraitNFT(_childTokens[i].childContract)
                .imageUri(_childTokens[i].childTokenId);
        ...
        returns (string memory)
    {
        ...
        return renderImagesBasedOnChildToken(child);
    }
