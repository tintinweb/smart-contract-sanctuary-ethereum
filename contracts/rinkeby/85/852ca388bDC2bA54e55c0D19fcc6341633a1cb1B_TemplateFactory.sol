...
contract TemplateFactory is ITemplateFactory, ReentrancyGuardUpgradeable, AccessibleUpgradeable {
    ...
    function deployTemplate(bytes32 templateId, uint256 _version, bytes memory args, bytes[] memory functionCalls, bytes32 salt) public override nonReentrant returns (address instance) {
        ...
        _validateDeploySalt(templateId, _version, args, salt);
    ...
    function cloneTemplate(bytes32 templateId, uint256 _version, bytes[] memory functionCalls, bytes32 salt) public override nonReentrant returns (address instance) {
        Version storage vsn = _validateTemplateVersion(templateId, _version, _msgSender());
        _validateCloneSalt(templateId, _version, salt);
    ...
    function _salt(bytes32 templateId, uint256 _version) private view returns (bytes32) {
        return keccak256(abi.encodePacked(templateId, _version, _template[templateId].versions[_version].instances.length));
    }

    function _deployCode(bytes32 templateId, uint256 _version, bytes memory args) private view returns (bytes memory) {
        return abi.encodePacked(_template[templateId].versions[_version].creationCode, args);
    }
    ...
    function _validateTemplateVersion(bytes32 templateId, uint256 _version, address sender) private view returns (Version storage vsn) {
        ...
        vsn = _template[templateId].versions[_version];
    ...
    function _validateDeploySalt(bytes32 templateId, uint256 _version, bytes memory args, bytes32 salt) private view {
        address predictedAddress = predictDeployAddress(templateId, _version, args, salt);
        
        require(!predictedAddress.isContract(), "TemplateFactory: duplicate salt");
    }

    function _validateCloneSalt(bytes32 templateId, uint256 _version, bytes32 salt) private view {
        address predictedAddress = predictCloneAddress(templateId, _version, salt);
