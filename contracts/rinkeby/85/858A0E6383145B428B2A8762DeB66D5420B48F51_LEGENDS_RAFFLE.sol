...
contract LEGENDS_RAFFLE is VRFConsumerBaseV2 {
  ...
  function mintRaffleWinners(uint32 numToMint) public onlyOwner {
    for (uint idx = mint_index; idx < numToMint + mint_index; idx++) {
        ...
        // check if they already own one
        uint tokenId = s_randomWords[idx];
        address owner = IERC721(mintingContract).ownerOf(tokenId);

        while (IERC721(mintingContract).balanceOf(owner) > 0) {
          // increment tokenId, and try again
          tokenId += 1;
          s_randomWords[idx] = tokenId;
          owner = IERC721(mintingContract).ownerOf(tokenId);
        }
