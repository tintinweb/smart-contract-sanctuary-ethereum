...
contract SMtest2 is ERC721, Ownable, ReentrancyGuard, PaymentSplitter {
  ...
  function presaleMint(
    ...
    require(msg.sender == account, 'SMtest2: Not allowed');
    require(presaleM, 'SMtest2: Presale is OFF');
    require(!paused, 'SMtest2: Contract is paused');
    require(
      _amount <= presaleAmountLimit,
      "SMtest2: You can't mint so much tokens"
    );
    require(
    ...
    require(current + _amount <= maxSupply, 'SMtest2: max supply exceeded');
    require(_price * _amount <= msg.value, 'SMtest2: Not enough ethers sent');
  ...
  function publicSaleMint(uint256 _amount) external payable onlyAccounts {
    require(publicM, 'SMtest2: PublicSale is OFF');
    require(!paused, 'SMtest2: Contract is paused');
    require(_amount > 0, 'SMtest2: zero amount');
