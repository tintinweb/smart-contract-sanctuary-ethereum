...
import "../Interfaces/BlankAccountRegistryInterface.sol";

import "../Interfaces/CompanyRegistryInterface.sol";

contract BlankAccountRegistryContract is AccessControl, BlankAccountRegistryInterface { 
    
    string public constant VersionString = "BlankAccountRegistryContract v1.0";
    ...
    constructor(address defaultAdminAddress, address companyRegistryAddress, uint companyId) {
        _setupRole(DEFAULT_ADMIN_ROLE, defaultAdminAddress);
        _defaultAdminAddress = defaultAdminAddress;
    ...
    // ==== MANAGER_ROLE REGION ====
    // =============================
    function addAccount(address accountAddress_) onlyRole(MANAGER_ROLE) public override returns (bool) {
        require(!getIsAccountRegistered(accountAddress_), "Address is already registered account!");
        _appendAccount(accountAddress_);
        return true;
    }
    function removeAccount(address accountAddress_) onlyRole(MANAGER_ROLE) public override returns (bool) {
...
interface BlankAccountRegistryInterface is IAccessControl { 

    // =============================
    // ==== MANAGER_ROLE REGION ====
    // =============================
    function addAccount(address accountAddress_) external returns (bool);
    function removeAccount(address accountAddress_) external returns (bool);
