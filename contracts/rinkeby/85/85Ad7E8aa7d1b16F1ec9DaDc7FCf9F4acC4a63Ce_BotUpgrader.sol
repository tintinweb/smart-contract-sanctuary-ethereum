...
contract BotUpgrader is Managable, Pausable {
    ...
    function upgrade(
        ...
        (uint _tokenPrice,uint _oilPrice) = _upgrade(_botToUpgradeId, _botsToBurn, _detailToUpgrade, _token, modifierExists);
    ...
    function _checkToken(address _rewardedToken, uint _tokenPrice, uint _oilPrice, uint _amountUserWallet, uint _amountClaim, address _token) internal view {
        uint balance = IERC20(_rewardedToken).balanceOf(msg.sender);
        require(balance == _amountUserWallet, "wrong uwa");
        if(_rewardedToken == _token){
            require(( balance + _amountClaim) == _tokenPrice, "wrong ac");
        } else {
            require(( balance + _amountClaim) == _oilPrice, "wrong ac");
        }
    }
    ...
    function _upgrade(
        ...
        address _token,
        bool modifierExists
    ) internal returns (uint _tokenPrice,uint _oilPrice){
        ...
        (_tokenPrice, _oilPrice) = getUpgradePrices(_botDetailTier, _botNextTier, _token);
