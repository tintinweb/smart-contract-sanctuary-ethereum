...
contract StatController is Controllable, IStatController {
  ...
  function _isItemTypeEligibleToItemSlot(uint itemType, uint itemSlot) internal pure returns (bool) {
    ...
    // items with type before 9 mapped 1 to 1
    if (itemType <= uint(IItem.ItemType.BOOTS)) {
      return itemType == itemSlot;
    }
    if (itemType == uint(IItem.ItemType.ONE_HAND)) {
      return itemType == uint(ItemSlots.RIGHT_HAND)
      || itemType == uint(ItemSlots.LEFT_HAND);
    }
    if (itemType == uint(IItem.ItemType.TWO_HAND)) {
      return itemSlot == uint(ItemSlots.TWO_HAND);
    }
    if (itemType == uint(IItem.ItemType.SKILL)) {
      return itemType == uint(ItemSlots.SKILL_1)
      || itemType == uint(ItemSlots.SKILL_2)
      || itemType == uint(ItemSlots.SKILL_3);
    }
    // unknown types
    return false;
  }
  ...
  function changeHeroItemSlot(
    ...
    if (itemToken == address(0)) {
      require(_heroSlots[heroToken][heroTokenId][itemSlot].token == address(this), "Item not in slot");
    } else {
      require(_heroSlots[heroToken][heroTokenId][itemSlot].token == address(0), "Item slot busy");
    }
