...
contract Bonuses is AragonApp {
    using SafeMath for uint256;
    using SafeMath64 for uint64;

    struct UserBonusesForClaim {
        address user;
        uint256 forClaimAmount; 
    }
    /* Hardcoded constants to save gas
    bytes32 public constant ADD_BONUSES_ROLE = keccak256("ADD_BONUSES_ROLE");
    */
    bytes32 public constant ADD_BONUSES_ROLE = 0x668cd20df94c4bb094c4ca21aa338b48470ca1afb48b95040e25534c737fdab2;

    /// Used to limit dividends occurrences for dividend Bonuses
    uint8 internal constant MAX_OCCURRENCES = uint8(42);

    /// Error string constants
    string private constant ERROR_TOKEN_MANAGER = "TOKEN_MANAGER_NOT_A_CONTRACT";
    string private constant ERROR_TOKEN_MANAGER_FUNDS = "TOKEN_MANAGER_NOT_ENOUGH_FUNDS_TO_COVER_REWARD";
    ...
    string private constant ERROR_ZERO_CLAIM_AMOUNT = "ZERO_CLAIM_AMOUNT";
    string private constant ERROR_BONUSES_FOR_CLAIM_LENGTH_0 = "BONUSES_FOR_CLAIM_LENGTH_0";

    /// Order optimized for storage
    struct BonusesDistribution {
        address creator;
        string description;
        uint256 distributionPrice;
        mapping (address => uint256) forClaimAmount;
    }

    /// Amount claimed for each token
    mapping (address => uint256) internal totalBonusesClaimed;

    /// Bonuses internal registry
    mapping(uint256 => BonusesDistribution) bonusesDistributionRegistry;
    uint256 bonusesDistributionRegistryLength;

    /// Public tokenManager that mint bonuses
    TokenManager public tokenManager;

    /// Events
    event BonusesDistributionAdded(uint256 bonusesDistributionId, uint256 totalBonusesForClaim, uint256 distributionPrice); /// Emitted when a new reward is created
    event BonusesClaimed(address claimant, uint256 claimedAmount);

    /**
     * @notice Initialize Bonuses app for tokenManager at `_tokenManager`
     * @dev Initializes the Bonuses app, this is the Aragon custom constructor
     * @param _tokenManager Address of the tokenManager Bonuses will rely on (non changeable)
     */
    ...
    /**
     * @notice Claim my reward for msg.sender
     * @dev Allows a user to claim their reward (if one is available)
     * @return claimedAmount calculated for that reward ID
     */
    function claimBounses() external isInitialized returns (uint256) {
        uint256 claimedAmount = _calculateUnclaimedBonusesAmount();
        require(claimedAmount > 0, ERROR_ZERO_CLAIM_AMOUNT);

        _claimBounses(claimedAmount);

        emit BonusesClaimed(msg.sender, claimedAmount);
        return claimedAmount;
    }

    /**
     * @notice Get total Bonuses count
     * @dev Gets the lenght of the Bonuses registry array
     * @return bonusesLength the length of the Bonuses array
     */
    function getbonusesDistributionRegistryLength() external view isInitialized returns (uint256 bonusesLength) {
        bonusesLength = bonusesDistributionRegistryLength;
    }

    // /**
    //  * @notice Gets information for the reward with ID #`bonusesDistributionID`
    //  * @dev Allows a user to get information about a specific reward
    //  * @param bonusesDistributionID The ID of the reward
    ...
    //  * @return claimedAmount which amount is available to claim
    //  * @return timeClaimed when it was claimed by the msg.sender
    //  * @return creator the address of the reward creator
    //  */
    // function getReward(uint256 bonusesDistributionID) external view isInitialized returns (
    //     string description,
    //     bool isMerit,
    ...
    // )
    // {
    //     Reward storage reward = Bonuses[bonusesDistributionID];
    ...
    //     claimedAmount = _calculateclaimedAmount(reward);
    // }

    /**
     * @notice Get total amount of `msg.sender` claimed
     * @dev Gets the amount of DAO token claimed by msg.sender
     * @return totalBonusesClaimed for that user
     */
    function getTotalBonusesClaimed() external view isInitialized returns (uint256)
    {
        return totalBonusesClaimed[msg.sender];
    }
     ...
     * @dev This function creates a reward instance to be added to the Bonuses array. ID's
     *      are assigned the new intance's index of that array
     * @param _description description of the reward
     * @return bonusesDistributionId of the newly created Reward
     */
    function addBonusesDistribution(
        string _description,
        uint256 _distributionPrice,
        UserBonusesForClaim[] memory userBonusesForClaim
    ) public auth(ADD_BONUSES_ROLE) returns (uint256 bonusesDistributionId)
    {
        require(userBonusesForClaim.length != 0, ERROR_BONUSES_FOR_CLAIM_LENGTH_0);
        bonusesDistributionId = bonusesDistributionRegistryLength++; // increment the Bonuses array to create a new one
        BonusesDistribution storage bonusesDistribution = bonusesDistributionRegistry[bonusesDistributionRegistryLength - 1]; // length-1 takes the last, newly created "empty" reward
        bonusesDistribution.description = _description;
        bonusesDistribution.distributionPrice = _distributionPrice;
        bonusesDistribution.creator = msg.sender;
        uint256 _totalBonusesForClaim = 0;
        for (uint8 i = 0; i < userBonusesForClaim.length; i++) {
            UserBonusesForClaim memory info = userBonusesForClaim[i];
            bonusesDistribution.forClaimAmount[info.user] = info.forClaimAmount;
            _totalBonusesForClaim += info.forClaimAmount;
        }
        emit BonusesDistributionAdded(bonusesDistributionId, _totalBonusesForClaim, _distributionPrice);
    }

    /**
     * @dev Private intermediate function that does the actual tokenManager transfer for a reward and reward amoun
     */
    function _claimBounses(uint256 amountForClaim) private {
        totalBonusesClaimed[msg.sender] = totalBonusesClaimed[msg.sender].add(amountForClaim);
        tokenManager.mint(msg.sender, amountForClaim);
    }

    /**
     * @dev Private intermediate function to calculate bonuses amount
     * @return _calculateUnclaimedBonusesAmount calculated for that bonuses
     */
    function getUnclaimedBonusesAmount() public view returns (uint256 ) {
        return _calculateUnclaimedBonusesAmount();
    }
    /**
     * @dev Private intermediate function to calculate bonuses amount
     * @return _unclaimedBonusesAmount calculated for that bonuses
     */
    function _calculateUnclaimedBonusesAmount() private view returns (uint256) {
        uint _unclaimedBonusesAmount = 0;
        for (uint256 i = 0; i < bonusesDistributionRegistryLength; i++) {
            BonusesDistribution storage bonusesDistribution = bonusesDistributionRegistry[i];
            _unclaimedBonusesAmount += bonusesDistribution.forClaimAmount[msg.sender];
        }
        return _unclaimedBonusesAmount - totalBonusesClaimed[msg.sender];
    }
}
