...
contract AuctionHouse is Ownable, ReentrancyGuard {
  ...
  modifier onlyOwnerofToken(uint256 _tokenId, address _nftAddress) {
    require(IERC721(_nftAddress).ownerOf(_tokenId)==msg.sender, "Caller is not token Owner");
    _;
  }
  ...
  function createAuction(
    ...
    uint256 expiryDate
  ) external nonReentrant onlyOwnerofToken(_tokenId, msg.sender) returns (uint256 auctionId) {
