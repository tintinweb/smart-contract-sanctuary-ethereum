...
contract LegendaryMiniMen is ERC721Enumerable, IMintable {
    ...
    constructor(address _sourceAddress) ERC721("LegendaryMiniMen", "LMM") {
    ...
    function mint(address claimer) external {
        ...
        if (MAX_TOKENS <= totalSupply()) revert OverTokenLimit();
    ...
    function _mintRandomIndex(address receiver) internal {
        uint256 supplyLeft = MAX_TOKENS - totalSupply();
...
abstract contract ERC721Enumerable is ERC721, IERC721Enumerable {
        ...
        returns (uint256)
    {
        ...
        for (uint256 i = 0; i < numMintedSoFar; i++) {
            address tokenOwner = ownerOf(_allTokens[i]);
            if (tokenOwner == owner) {
    ...
    function tokenByIndex(uint256 index)
        ...
        returns (uint256)
    {
        if (index >= totalSupply()) revert IndexOverTokenCount();
