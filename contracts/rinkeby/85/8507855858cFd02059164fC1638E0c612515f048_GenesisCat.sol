...
contract GenesisCat is ERC721A, Ownable, ReentrancyGuard {
    ...
    constructor(string memory _initBaseURI, string memory _notRevealedUri)
        ERC721A("Genesis Cat NFT", "GENCAT")
    {
    ...
    function mint(uint256 tokens) public payable nonReentrant {
        require(!paused, "GENCAT: oops contract is paused");
        require(publicSale, "GENCAT: Sale hasnt started yet");
        require(
            tokens <= maxPerWallet,
            "GENCAT: max mint amount per tx exceeded"
        );
        require(
        ...
        require(
            ...
            "GENCAT: Max NFT Per Wallet exceeded"
        );
        require(msg.value >= cost * tokens, "GENCAT: insufficient funds");
    ...
    {
        require(!paused, "GENCAT: oops contract is paused");
        require(preSale, "GENCAT: Presale HGENCAT't started yet");
        require(
            MerkleProof.verify(
                merkleProof,
                merkleRoot,
            ...
            "GENCAT: You are not Whitelisted"
        );
        require(
            _numberMinted(_msgSenderERC721A()) + tokens <= maxPerWalletWl,
            "GENCAT: Max NFT Per Wallet exceeded"
        );
        require(tokens <= maxPerWalletWl, "GENCAT: max mint per Tx exceeded");
        require(
            totalSupply() + Totalburned() + tokens <= wlSupply,
            "GENCAT: Whitelist MaxSupply exceeded"
        );
