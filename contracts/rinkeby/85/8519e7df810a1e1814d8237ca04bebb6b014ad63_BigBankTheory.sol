...
contract BigBankTheory is ERC20, Ownable, VRFConsumerBaseV2 {
    // Basic units
    uint256 private constant ONE_HOUR = 60 * 10; // changed to 10 minutes for faster testing
        ...
        VRFConsumerBaseV2(0x6168499c0cFfCaCD319c818142124B7A15E857ab)
    {
        ...
        LizzieWizzieNFTs = ILizzieWizzie(
            0x3b48D689edE1f92c1d903fec46a1676a6EEa9f46
        );
    }
        ...
        if (
            ...
            if (_before > 0) {
                uint256 _buyerAmount = (_before * biggestBuyRewardPercentage) /
                    PERCENT_DENOMENATOR;
                _buyerAmount /= 2;
                payable(RewardPool).transfer(_buyerAmount);
                _buyerAmount /= 2;
                payable(biggestBuyer[_prevHour]).transfer(_buyerAmount);
                dividendDistributor.deposit{value: _buyerAmount}();
                require(
                    address(this).balance >= _before - (_buyerAmount*4),
                    "bazinga"
                );
