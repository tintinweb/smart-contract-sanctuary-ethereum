...
contract OkayCyberBears is ERC721A, Ownable {
    ...
    function mintNFTs(uint _count) public payable {
       ...
       // NFTmintedbyAddress[msg.sender] += _count;
        /*for (uint i = 0; i < (_count); i++) {
            NFTmintedbyAddress[msg.sender] += 1;
            _tokenIds.increment();
        }*/
    }
    ...
    function preSale(uint _count) public payable {
        ...
        //require(totalMinted.add(_count) <= preSaleMaxSupply, "Not enough Presale NFTs left!");
        require(_count > 0 && numberMinted(msg.sender) + _count <= preSaleMaxMint, "Cannot mint specified number of NFTs.");
        //require(_count >0 && _count <= (preSaleMaxMint-NFTmintedbyAddress[msg.sender]) , "Cannot mint specified number of NFTs.");
        //require(NFTmintedbyAddress[msg.sender] < preSaleMaxMint);
        _safeMint(msg.sender, _count);
        //NFTmintedbyAddress[msg.sender] += _count;
        /*for (uint i = 0; i < (_count); i++) {
            NFTmintedbyAddress[msg.sender] += 1;
