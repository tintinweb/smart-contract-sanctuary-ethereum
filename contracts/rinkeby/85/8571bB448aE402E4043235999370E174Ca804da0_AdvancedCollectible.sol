...
contract AdvancedCollectible is ERC721URIStorage, VRFConsumerBase {
    ...
    uint256 public fee;
    // To remember what each address will have as breed
    // since address -> token (via mint) -> breed (during VRF callback)
    mapping(uint256 => Breed) public tokenIdToBreed;
    // Internal, to be used during VRF callback
    ...
    function setTokenURI(uint256 _tokenId, string memory _uri)
        public
    {
        require(_isApprovedOrOwner(_msgSender(), _tokenId), "ERC721: caller is not the owner nor approved");
