...
contract SwampverseScavengerRun is Pausable, Ownable {
    ...
    mapping(uint => uint) private swampersTotalFound;
    mapping(uint => uint) private creaturesTotalFound;
    ...
    function mostRecentCreatureRuns(address account) public view returns (RecentRun[] memory) {
      ...
      for(uint tokenId = 1; tokenId <= creatureSupply; tokenId++) {
        if(creatureScavengerRuns[tokenId].owner == account) {
          ...
          if(block.timestamp < creatureTmp[creatureIndex].end) {
    ...
    function startRuns(uint[] memory swamperIds, uint[] memory creatureIds) external whenNotPaused {
      ...
      croakContract.burn(msg.sender, ((swamperIds.length+creatureIds.length)*croakRunCost) * 1 ether);
    }

    /// @notice Retreive a reward from a swamper
    function retrieveRewards(uint quantity) external {
      require(swampPileRewards[msg.sender] >= quantity, "not enough rewards");
      swampPileContract.mintSwampPile(msg.sender, quantity);
      unchecked { swampPileRewards[msg.sender] -= quantity; }
