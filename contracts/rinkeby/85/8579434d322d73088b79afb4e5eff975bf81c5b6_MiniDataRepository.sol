...
contract MiniDataRepository is IMiniDataRepository, Ownable {
    ...
    function addDataTuple(
      ...
      bytes memory metadata = abi.encode(_name, _description, _artistName, _generation, _imageData);

      address dataAddress = SSTORE2.write(metadata);
      tokenDataAddresses[nextDataId] = dataAddress;
      emit TokenDataAdded(nextDataId);
...
interface IMiniDataRepository {
    ...
    function tokenMetadata(uint256 _id) external view returns (bytes memory);
    // retrieve a token's saved metadata as json string 
    function tokenData(uint256 _id) external view returns (string memory);
