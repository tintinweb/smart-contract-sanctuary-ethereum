...
contract CDomDiBion is ERC1155, Ownable, Pausable, ERC1155Supply
{
    // map owner address to number of token types held
    mapping(address => uint256) private _balances;

    // create the contract, define the base URI
    constructor() ERC1155("ipfs://bafybeie5n2n3p5i5cjayvs7hj3xjtp4tmwbmirvhkeyyvq325i74hejbwu/CDomDiBion_")
    {
    } // constructor
        ...
        onlyOwner
    {
        ...
        if (amount > 0 && balanceOf(account, tokenID) == 0)
            _balances[account] += 1;
    ...
    // only the owner can do this
    function mintBatch(address account, uint256[] memory tokenIDs, uint256[] memory amounts, bytes memory data)
        public
        onlyOwner
    {
        // update the number of token types held by the input 'account'
        ...
        {
            if (amounts[i] > 0 && balanceOf(account, tokenIDs[i]) == 0)
                _balances[account] += 1;
        }
    ...
    // return the number of tocken types held by the input 'account'
