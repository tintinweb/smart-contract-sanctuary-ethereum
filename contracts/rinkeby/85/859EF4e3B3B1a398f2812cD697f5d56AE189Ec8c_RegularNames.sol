...
contract RegularNames is AccessControl {

    bytes32 public constant ADMIN_ROLE = keccak256("ADMIN_ROLE");
	address constant REGULAR_NFT_ADDRESS = 0x6d0de90CDc47047982238fcF69944555D27Ecb25;
	address REGULAR_TOKEN_ADDRESS = 0xAb5cf3003bd70e52f477540be8fB4de8Eb6a1387; // rinkeby
    ...
    uint public price = 3 ether;                           // Price is in Reg Dollars
    ...
    function setName(uint _tokenId, string memory _name) public { 
        _name = toLower(_name);
        require(_ownerOfRegular(_tokenId) == msg.sender, "Not the owner of Regular");
        require(validateName(_name),"Name not valid");
        require(!reserved[_name],   "Name taken");
        require(!locked[_tokenId],  "Name locked");
        IERC20(REGULAR_TOKEN_ADDRESS).transferFrom(msg.sender, address(this), price);
...
// admin
    ...
    function setPriceInREG(uint _price) public onlyRole(ADMIN_ROLE) {
        price = _price;
    }

    function setRegTokenAddress(address _regToken) public onlyRole(ADMIN_ROLE) {
        REGULAR_TOKEN_ADDRESS = _regToken;
    }
...
// internal
    ...
    function validateName(string memory str) public pure returns (bool){
        ...
        for(uint i; i<b.length; i++){
            ...
            if(
                // !(char >= 0x30 && char <= 0x39) && //9-0 -- Commented out ...no numbers
