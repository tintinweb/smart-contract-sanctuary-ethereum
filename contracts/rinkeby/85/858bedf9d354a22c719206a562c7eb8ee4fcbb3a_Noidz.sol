...
contract Noidz is ERC721, Ownable, ReentrancyGuard {
    Stardust public SDUST;

    string internal _baseTokenURI;

    uint256 public constant MINT_COST = 1200 ether;
    uint256 public constant MAX_SUPPLY = 4115;
    uint256 public constant RESERVED = 15;
    string public PROVENANCE_HASH = "";
    mapping(bytes4 => bool) public functionLocked;
    string public metadataURI;
    uint256 public totalSupply;

    constructor(address stardust_address) ERC721("Noidz", "NOIDZ") {
        SDUST = Stardust(stardust_address);
    ...
    /// @notice Set SDUST token address
    /// @param stardust_address address of SDUST ERC20 token contract
    function setSDUSTAddress(address stardust_address)
        public
        lockable
        onlyOwner
    {
    ...
    function mint(uint256 amount) public nonReentrant {
        ...
        SDUST.burn(_msgSender(), amount * MINT_COST);
    ...
    function testMint(uint256 amount) public nonReentrant {
        require(totalSupply + amount <= MAX_SUPPLY, "Exceeds max supply");
