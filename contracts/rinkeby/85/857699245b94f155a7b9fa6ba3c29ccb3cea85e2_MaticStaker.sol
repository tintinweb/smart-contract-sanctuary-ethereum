...
contract MaticStaker {
    ...
    uint256 constant public INVEST_MIN_AMOUNT = 0.00001 ether; //TODO
    uint256[] public REFERRAL_PERCENTS = [50, 25, 5];
    uint256 constant public PROJECT_FEE = 100;
    uint256 constant public PREMIUM_FEE = 200;
    ...
    function invest(address referrer, uint8 plan) public payable {
        ...
        if(plan == 6){
            uint256 fee = msg.value.mul(PREMIUM_FEE).div(PERCENTS_DIVIDER);
        ...
        if (user.referrer == address(0)) {
            ...
            for (uint256 i = 0; i < 3; i++) {
                if (upline != address(0)) {
                    users[upline].levels[i] = users[upline].levels[i].add(1); //useless
        ...
        if (user.referrer != address(0)) {
            ...
            for (uint256 i = 0; i < 3; i++) {
                if (upline != address(0)) {
                    ...
                    users[upline].totalBonus = users[upline].totalBonus.add(amount); //useless?
    ...
    function withdraw() public {
        ...
        uint256 fees = totalAmount.mul(WITHDRAW_FEE).div(10000); //??? why bigger numbers? Accuracy maybe
    ...
    function getUserDividends(address userAddress) public view returns (uint256) {
        ...
        for (uint256 i = 0; i < user.deposits.length; i++) {
            if (user.checkpoint < user.deposits[i].finish) { //check if a deposit is finished
                if (user.deposits[i].plan < 3) { //If plan is less than 3
                    uint256 share = user.deposits[i].amount.mul(user.deposits[i].percent).div(PERCENTS_DIVIDER); //share = amount staked * yieldPercent / 1000
                    uint256 from = user.deposits[i].start > user.checkpoint ? user.deposits[i].start : user.checkpoint;//has the user withdrawn perviously? If so,  from = user.checkpoint
                    uint256 to = user.deposits[i].finish < block.timestamp ? user.deposits[i].finish : block.timestamp;//Is the investment finished?
                    if (from < to) {
                        totalAmount = totalAmount.add(share.mul(to.sub(from)).div(TIME_STEP));
                    }
                } else if (block.timestamp > user.deposits[i].finish) { //If plan is greater than 3
