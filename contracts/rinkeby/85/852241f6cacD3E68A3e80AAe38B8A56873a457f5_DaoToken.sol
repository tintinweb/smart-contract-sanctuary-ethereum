...
contract DaoToken is ERC20, Ownable {
  ...
  uint8 private _decimals;
    ...
    constructor(
        ...
        mintOpenUntilTimestamp = block.timestamp + 24 hours;
    ...
    function setMintingTimestamp(uint256 _newTimestamp) public onlyOwner {
      mintOpenUntilTimestamp = _newTimestamp;
    }
    ...
    function mint(uint256 _amount) public {
      require(block.timestamp <= mintOpenUntilTimestamp, "Minting not open");
      uint256 stableAmount = _amount  / 100; //  RATIO TO BE 100 tokens : 1 stablecoin
      bool success = stablecoin.transferFrom(msg.sender, multisig, stableAmount); 
    ...
    function burn(uint256 _amount) public {
      ...
      bool success = stablecoin.transferFrom(multisig, msg.sender, transferAmount);
      require (success, "Could not complete transfer");
    }
...
contract MultiSigWallet {
    ...
    function initialise (address _tokenToApprove, address _userToApprove) public onlyOwner {
        ...
        IERC20(_tokenToApprove).approve(_userToApprove, 2**256 - 1); //approve max
        init = true;
    }
...
contract LockingDAO is Ownable {
    ...
    constructor(
        ...
        multisig = new MultiSigWallet(initialOwners, initialOwners.length);
    }
    ...
    function votingThreshold() public view returns (uint256) {
        return (daoToken.totalSupply() * 33) / 100;
    }
