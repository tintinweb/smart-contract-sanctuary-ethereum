...
contract SwapNFT is Accept {
    ...
    function purchaseOrder(uint256 orderNumber,string memory _tokenName) public payable { // Buy Order by other Token
        ...
        // console.log("Allowance",IBEP20(order.tokenAddress).allowance(msg.sender, address(this)));
        ...
        if(!order.isSingle){
            // console.log("Is not Single");
            // just for pull new code 

            for(uint256 i = 0; i< order.creators.length ; i++){
                uint256 calAmount = (order.sameCreator[order.creators[i]] * (_royality)) /(order.creators.length);
                payable(IERC721(order.nftAddress).creator(order.sameCreator[order.creators[i]])).transfer(calAmount);
            }
           }
