...
interface ISongAlbumAddressRegistry {
...
interface ISongAlbumMarketplace {
...
interface ISongAlbumTokenRegistry {
    function enabled(address) external returns (bool);
}

contract SongAlbumBundleMarketplace is
    OwnableUpgradeable,
    ReentrancyGuardUpgradeable
{
    ...
    ISongAlbumAddressRegistry public addressRegistry;
        ...
        require(
            _payToken == address(0) ||
                (tokenRegistry != address(0) &&
                    ISongAlbumTokenRegistry(tokenRegistry).enabled(_payToken)),
            "invalid pay token"
        );
    ...
    function _buyItem(string memory _bundleID, address _payToken) private {
        ...
        if (_payToken == address(0)) {
            ...
            require(
                feeTransferSuccess,
                "SongAlbumMarketplace: Fee transfer failed"
            );
            ...
            require(
                ...
                "SongAlbumMarketplace: Owner transfer failed"
            );
        } else {
        ...
        for (uint256 i; i < listing.nfts.length; i++) {
            ...
            ISongAlbumMarketplace(addressRegistry.marketplace()).validateItemSold(
                listing.nfts[i],
                listing.tokenIds[i],
                owner,
                _msgSender()
        ...
        emit ItemSold(
            ...
            _bundleID,
            _payToken,
            ISongAlbumMarketplace(addressRegistry.marketplace()).getPrice(_payToken),
            price
        );
        ...
        nonReentrant
    {
        ...
        emit ItemSold(
            ...
            ISongAlbumMarketplace(addressRegistry.marketplace()).getPrice(address(offer.payToken)),
            offer.price
        );
    ...
    /**
     @notice Update SongAlbumAddressRegistry contract
     @dev Only admin
     */
    function updateAddressRegistry(address _registry) external onlyOwner {
        addressRegistry = ISongAlbumAddressRegistry(_registry);
    }
