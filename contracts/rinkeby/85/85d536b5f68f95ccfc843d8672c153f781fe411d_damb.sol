...
abstract contract Ownable is Context {
    ...
    contract damb is ERC721A, Ownable {
    using Strings for uint256;
  ...
  function mint(uint8 _mintAmount) external payable  {
    ...
    if(freeNFTAlreadyMinted + totalSupply > NUM_FREE_MINTS){
        require(
            (cost* totalSupply) <= msg.value,
            "Incorrect ETH value sent"
        );
    } else {
        if (balanceOf(msg.sender) + totalSupply > MAX_FREE_PER_WALLET) {
        require(
            (cost * totalSupply) <= msg.value,
            "Incorrect ETH value sent"
        );
        require(
            totalSupply <= maxMintAmountPerWallet,
            "Max mints per transaction exceeded"
        );
        } else {
            require(
                totalSupply <= MAX_FREE_PER_WALLET,
                "Max mints per transaction exceeded"
            );
            freeNFTAlreadyMinted += totalSupply;
        }
    }
