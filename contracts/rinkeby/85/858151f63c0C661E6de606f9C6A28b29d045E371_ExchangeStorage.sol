...
contract Exchange is Pausable, NonReentrant, ExchangeAutoProxy, IExchange {
    ...
    function processEthAndWeth(
        ...
        (totalMakeValue, totalTakeValue) = calculateTotalTakeAndMakeValues(
            orderLeft,
            orderRight,
            takerAssetType,
            makerAssetType,
    ...
    function calculateTotalTakeAndMakeValues(
    ...
    ) internal view returns (uint256 _totalMakeValue, uint256 _totalTakeValue) {
        ...
        if (feeSide == LibFeeSideTypes.FeeSide.MAKE) {
            _totalMakeValue = getExchangeHelperImpl().calculateTotalAmount(
                _makeValue,
                rightOrderData.originFees
            );
        } else if (feeSide == LibFeeSideTypes.FeeSide.TAKE) {
            _totalTakeValue = getExchangeHelperImpl().calculateTotalAmount(
                _takeValue,
