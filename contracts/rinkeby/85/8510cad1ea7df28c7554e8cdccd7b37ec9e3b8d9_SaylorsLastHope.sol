...
contract MoneyPrinter is IFailedTheHowieTest {
    ...
    constructor (address owner) {
        ...
        DigitalGold = IERC20(0x2260FAC5E5542a773Aa44fBCfeDf7C193bc2C599);
    }
    ...
    function deposit() external payable override onlyToken {
        ...
        uint256 balanceBefore = DigitalGold.balanceOf(address(this));
        ...
        uint256 amount = DigitalGold.balanceOf(address(this)).sub(balanceBefore);
    ...
    function distributeDividend(address shareholder) internal {
        ...
        if(amount > 0){
            ...
            DigitalGold.transfer(shareholder, amount);
...
contract SaylorsLastHope is IERC20, Auth {
    ...
    uint256 private toReflections = 10;
    uint256 private toLiquidity = 5;
    uint256 private toMarketing = 10;

    uint256 private allocationSum = toReflections.add(toLiquidity).add(toMarketing);

    uint256 public reflectionsRate = toReflections.div(allocationSum);
    uint256 public liquidityRate = toLiquidity.div(allocationSum);
    uint256 public marketingRate = toMarketing.div(allocationSum);

    IDEXRouter public router;
    address public pair;
    address public factory;
    address private tokenOwner;
    address public marketingWallet = payable(0x110CbaA2772AC05083F57819D583B1D6CF86a34E);
    ...
    uint256 public moneyPrinterGas = 50000;
    ...
    constructor (
        ...
        moneyPrinter = new MoneyPrinter(_owner);
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if (moneyPrinterGas > 0) {
            try moneyPrinter.process(moneyPrinterGas) {} catch {}
        }
    ...
    function swapBack() internal lockTheSwap {
        ...
        uint256 tokensForLiquidity = tokenBalance.mul(liquidityRate).div(2);     
        uint256 amountToSwap = tokenBalance.sub(tokensForLiquidity);
        uint256 ethDenominator = tokenBalance.div(amountToSwap);

        swapTokensForEth(amountToSwap);

        uint256 totalEthBalance = address(this).balance;
        uint256 ethForMarketing = totalEthBalance.mul(marketingRate).mul(ethDenominator);
        uint256 ethForDigitalGold = totalEthBalance.mul(reflectionsRate).mul(ethDenominator);
        uint256 ethForLiquidity = totalEthBalance.sub(ethForDigitalGold).sub(ethForMarketing);
