...
import "./ERC721A-by-ID.sol";

contract nill is Ownable, PaymentSplitter, ERC721A, ReentrancyGuard {
    uint256 public MAX_SUPPLY = 8888;
    uint256 public PRICE = 0 ether;
    uint256 public PRESALE_PRICE = 0.025 ether;
    uint256 public maxPresale = 2;
    uint256 public maxPublicTx = 2;
    uint256 public NFTsPerPlayer = 109;
    uint256 public numPlayers = 40;

    bool public _isActive = false;
    bool public _presaleActive = false;

    mapping(address => uint8) public _preSaleListCounter;
    // playerID => numberMinted
    mapping(uint8 => uint256) public _numPlayerMinted;
    ...
    function internalMint(uint256 quantity, address to, uint256 startingTokenId) external onlyOwner nonReentrant {
        require(
            totalSupply() + quantity <= MAX_SUPPLY,
            "would exceed max supply"
        ); 
        _safeMint(to, quantity, startingTokenId);
    }
    ...
    // Presale
    function mintPreSaleTokens(uint8 quantity, bytes32[] calldata _merkleProof, uint256 startingTokenId)
        external
        payable
        callerIsUser
        nonReentrant
    {
        ...
        _safeMint(msg.sender, quantity, startingTokenId);
    ...
    // public mint
    function publicSaleMint(uint256 quantity, uint8 playerID)
        external
        payable
        nonReentrant
        callerIsUser
    {
        ...
        require(_numPlayerMinted[playerID] + quantity <= NFTsPerPlayer, "Not enough NFTs from player left");
        require(playerID < numPlayers, "Invalid Player ID");

        uint256 startingID = _numPlayerMinted[playerID] + (playerID * NFTsPerPlayer);
        _numPlayerMinted[playerID] = _numPlayerMinted[playerID] + quantity;

        _safeMint(msg.sender, quantity, startingID);
    }
...
contract ERC721A is Context, ERC165, IERC721A {
     ...
     */
    function _safeMint(address to, uint256 quantity, uint256 startingTokenId) internal {
        _safeMint(to, quantity, "",  startingTokenId);
    }
    ...
    function _safeMint(
        ...
        uint256 startTokenId = _startingTokenId;
