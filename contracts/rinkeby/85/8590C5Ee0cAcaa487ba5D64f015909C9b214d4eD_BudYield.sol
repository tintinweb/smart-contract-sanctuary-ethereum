...
contract BudYield is Ownable {
    using SafeERC20 for IERC20;

    // ERC-20 token to stake
    address rewardTokenAddr;

    // Strain Factory Address
    address strainFactoryAddr;

    //  The sum of all active stake smoke quality
    uint256 sumSmokeQuality;

    //  The sum of all propotional reward
    //  The propotional reward is (reward / sumSmokeQuality)
    ...
    /**
     * @dev constructor
     * @param _rewardTokenAddr the address of token to stake
     */
    constructor(
        address _rewardTokenAddr,
        address _strainFactoryAddr
    ) {
        rewardTokenAddr = _rewardTokenAddr;
        strainFactoryAddr = _strainFactoryAddr;
    }

    /**
     * @dev the function to stake token
     */
    function deposit(uint256 _tokenId, uint256 _stakedDays, uint256 _rarityScore) external payable {
        // Transfer token from staker to smart contract
        IStrainFactory(strainFactoryAddr).safeTransferFrom(msg.sender, address(this), _tokenId);

        uint256 rewardAmount = calculateReward(msg.sender, stakeAmount[msg.sender]);

        // Update stake amount, total smoke quality amount and last proportional reward
        uint256 smokeQuality = _stakedDays * _rarityScore;
        stakeAmount[msg.sender] += smokeQuality;
        sumSmokeQuality += smokeQuality;
        ...
        IERC20(rewardTokenAddr).safeTransfer(address(msg.sender), rewardAmount);
    }

    function distribute(uint256 _reward) external payable onlyOwner {
        require(sumSmokeQuality > 0, "No deposit yet");
        sumPropotionalReward += _reward * 1e12 / sumSmokeQuality;
    }

    /**
     * @dev the function to withdraw the staked token in the history
     */
    function withdraw(uint256 _tokenId, uint256 _stakedDays, uint256 _rarityScore) public {
        uint256 balance = stakeAmount[msg.sender];
        uint256 smokeQuality = _stakedDays * _rarityScore;
        require(balance >= smokeQuality, "Insufficient smoke quality");
        ...
        IStrainFactory(strainFactoryAddr).safeTransferFrom(address(this), msg.sender, _tokenId);
        IERC20(rewardTokenAddr).safeTransfer(address(msg.sender), rewardAmount);
    }
