...
contract Bella is ERC2981, ERC721A, Ownable {
    ...
    string public _metadataURI = "ipfs://QmWSueFanChF8WDy6bTtCV7UxPcFrpagF5iEouey6vYkLB/";
    ...
    function mint(uint32 amount) external payable {
        ...
        require(amount + minted <= _walletLimit, "PieceOfShit: Exceed wallet limit");
        ...
        if (minted < instantFreeWalletLimit && _freeQuota > 0) {
            ...
            quota = _freeQuota >  quota ? quota : _freeQuota;
        ...
        if (minted + amount > instantFreeWalletLimit) {
            uint32 enterLotteryAmount = amount - (minted >= instantFreeWalletLimit ? 0 : instantFreeWalletLimit - minted);
