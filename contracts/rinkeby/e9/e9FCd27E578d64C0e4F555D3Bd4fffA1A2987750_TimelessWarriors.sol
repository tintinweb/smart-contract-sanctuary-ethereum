...
contract TimelessWarriors is ERC721, ERC2981, Ownable {
    ...
    uint256 public constant NUMBER_OF_CONQUEROR = 14888;         
    ...
    /// @param _supplyLimit The new amount of presale supply limit of Conqueror
    ...
    /// @param _newPrice The new price for minting of Conqueror
    ...
    /// @notice supplyLimit updates during all presale iterations, supplyLimit <= NUMBER_OF_CONQUEROR - NUMBER_OF_RESERVED_CONQUEROR
    function presaleMintConqueror(uint256 count, bytes memory signature) external payable {
        require(phase == SalePhase.PreSale, 'TW: Presale event is not active.');
        _validateSignature(signature);
        require(mintPrice * count == msg.value, 'TW: Ether value sent is not correct');
        
        require(addressToMinted[_msgSender()] + count < maxAllowedPerAddress, 'TW: Exceeds number of presale mints allowed.');
        if (supplyLimit > 0) {
            require(minted + count <= supplyLimit, 'TW: Exceeds max presale supply of Conqueror.');
        }
        require(minted + count <= NUMBER_OF_CONQUEROR - NUMBER_OF_RESERVED_CONQUEROR, 'TW: Exceeds max supply of Conqueror.');
    ...
    /// @param count number of tokens to mint in transaction
    /// @notice tokens number of presale mints allowed is maxAllowedPerAddress - 1
    function reserveMintConqueror(uint256 count, bytes memory signature) external payable {
        require(phase == SalePhase.LimitedSale, 'TW: Limited sale is not active.');
        _validateSignature(signature);
        require(mintPrice * count == msg.value, 'TW: Ether value sent is not correct');

        require(addressToMintedLimited[_msgSender()] + count < maxAllowedPerAddress, 'TW: Exceeds number of mints allowed.');
        require(minted + count <= NUMBER_OF_CONQUEROR, 'TW: Exceeds max supply of Conqueror.');
