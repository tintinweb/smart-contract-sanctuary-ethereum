...
function isVerifiedMultiple(address[] memory nftAdd) public view returns(Verified[] memory) {

Verified[] memory verifiedCo = new Verified[](nftAdd.length);

  for(uint i; i < nftAdd.length; i++) {
    verifiedCo[i] = verifiedCollection[nftAdd[i]];
}  
return verifiedCo;
}
