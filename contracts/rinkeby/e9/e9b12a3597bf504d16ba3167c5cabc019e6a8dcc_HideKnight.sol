...
// HideKnight is the coolest pit in town. You come in with some Knight, and leave with more! The longer you stay, the more Knight you get.
//
// This contract handles swapping to and from xKnight.
contract HideKnight is ERC20("xKnight Staked Punk Knights", "xKNIGHT"){
    using SafeMath for uint256;

    IERC20 public immutable knight;

    // Define the Knight token contract
    constructor(IERC20 _knight) public {
        require(address(_knight) != address(0), "_knight is a zero address");
        knight = _knight;
    }

    // Enter the doghouse. Pay some KNIGHTs. Earn some shares.
    ...
    // Leave the doghouse. Claim back your KNIGHTs.
    // Unclocks the staked + gained Knight and burns xKnight
    function leave(uint256 _share) public {
        // Gets the amount of xKnight in existence
        uint256 totalShares = totalSupply();

        // Calculates the amount of Knight the xKnight is worth
        uint256 what = _share.mul(knight.balanceOf(address(this))).div(totalShares);
