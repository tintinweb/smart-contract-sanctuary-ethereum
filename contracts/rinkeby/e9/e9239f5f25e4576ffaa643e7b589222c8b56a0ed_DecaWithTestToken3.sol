...
contract DecaWithTestToken3 is VRFConsumerBaseV2, Pausable, Ownable, ReentrancyGuard {
    ...
    mapping(uint256 => Balance)  housePoolBalance;
    mapping(uint256 => Balance)  treasuryPoolBalance;
    ...
    function getPoolBalance() public view returns (uint256) {
        return housePoolBalance[0].depositAmount;
    }
    ...
    function getTreasuryBalance() public view returns (uint256) {
        return treasuryPoolBalance[0].depositAmount;
    }
    ...
    function depositHouse(uint256 _amount) external {
        ...
        housePoolBalance[0].depositAmount += _amount;
        // lock 처리해야함
    }
    ...
    function claimTreasury(uint256 _amount) external onlyTreasury nonReentrant {
        ...
        treasuryPoolBalance[0].withdrawAmount += _amount;
    ...
    function claimTreasuryAll() external onlyTreasury nonReentrant {
        ...
        treasuryPoolBalance[0].depositAmount -= _amount;
        if (_amount >= 0) {
            flag = true;
            _transfer(USDC_ADDRESS, msg.sender, _amount);
        }
