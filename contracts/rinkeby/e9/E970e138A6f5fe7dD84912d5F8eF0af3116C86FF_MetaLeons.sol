...
contract MetaLeons is ERC721AQueryable, Ownable, ReentrancyGuard {
    ...
    uint256 public investorMintPrice = 0.1 ether;
    uint256 public maxPerUser = 5;
    uint256 public maxPerInvestor = 25;
    uint256 public giftThreshold = 15;
    uint256 public maxSupply = 6767;

    string public baseURI = "https://api.metaleons.io";
    ...
    mapping (address => bool) userGifted;
    ...
    //        Events        //
    //////////////////////////
    event Minted(address recipient, uint256 quantity);
    event FreeMinted(address recipient);
    ...
    //     Constructor      //
    //////////////////////////
    constructor(address _signatureWallet, address payable _withdrawalWallet) ERC721A("Metaleons", "MLS") {
    ...
    modifier canMint(uint256 _quantity) {
        require(
            userMinted[msg.sender] + _quantity <= maxPerUser, 
    ...
    function setMaxPerInvestor(uint256 _quantity) external onlyOwner {
    ...
    ) external payable canMint(quantity) presaleOnly {
        require(msg.value >= presaleMintPrice * quantity, "Not enough money");
    ...
    function OGMint(
        ...
        require(msg.value >= investorMintPrice * quantity, "Not enough money");
        require(_verifySignature(_signature, 1) == signatureWallet, "Not OG");
    ...
    function InvestorMint(
        ...
        require(msg.value >= investorMintPrice * quantity, "Not enough money");
        require(_verifySignature(_signature, 2) == signatureWallet, "Not investor");

        if (userMinted[msg.sender] + quantity >= giftThreshold && !userGifted[msg.sender])  {
            quantity += 1;
            userGifted[msg.sender] = true;
            emit FreeMinted(msg.sender);
        }

        require(userMinted[msg.sender] + quantity <= maxPerInvestor, "exceeds per investor limit");
        require(totalSupply() + quantity <= maxSupply, "Exceeds supply");

        _mint(quantity);
    }

    function publicMint(uint256 quantity) external payable canMint(quantity) {
    ...
    function airDrop(address recipient, uint256 quantity) external onlyOwner {
        ...
        emit Minted(recipient, quantity);
    }

    function _verifySignature(bytes memory _signature, uint256 _mintType) internal view returns(address) {
    ...
    //         URIs         //
    //////////////////////////
    function freezeMetadata(uint256 _tokenId, string calldata _tokenURI) external isOwnerOfToken(_tokenId) {
    ...
    function unfreezeMetadata(uint256 _tokenId) external isOwnerOfToken(_tokenId) {
