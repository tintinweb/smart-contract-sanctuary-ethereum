...
contract DATA_BLOCKS is ERC1155, AccessControl {
  ...
  uint256 constant private _mintPerAddressLimit = 2; // per wallet mint limit (public sale is counted separately)
  mapping(address => uint256) private _mintCntPerAddress;
  ...
  function mintAllowlistedWithASH(uint256 _mintAmount, uint8 v, bytes32 r, bytes32 s) public {
    ...
    uint256 addressMintedCount = _mintCntPerAddress[msg.sender];
    ...
    _mintCntPerAddress[msg.sender] = addressMintedCount + _mintAmount;
    mintedSupply += _mintAmount;
    _mint(msg.sender, _tokenId, _mintAmount, "");
   ...
   * PUBLIC FUNCTIONS
   */
  function mintedAmount(address addr) public view returns (uint256) {
    return _mintCntPerAddress[addr];
  }
  ...
  function setTransferLock(bool lock) external onlyAdmin {
    if (mintedSupply == maxSupply) {
      _transferLock = false; // cannot set transfer lock after sale end
      return;
    }
    _transferLock = lock;
  }
