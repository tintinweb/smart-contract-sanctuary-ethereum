...
contract Staking is Ownable {
    ...
    event TimePeriodUpdated(uint256 initial, uint256 timePeriod);
    event StakingLock(bool state);
    ...
    modifier stakingNotLocked() {
        require(!stakingLocked, "Staking is closed.");
        _;
    }

    function setStakingLock(bool _state) external onlyOwner   {
        stakingLocked = _state;
        emit StakingLock(_state);
    }
    ...
    function setTimestamp(uint256 _timePeriodInSeconds) external onlyOwner   {
        ...
        emit TimePeriodUpdated(initialTimestamp, timePeriod);
    }
    ...
    function stakeTokens(uint256 amount) external timestampIsSet stakingNotLocked noReentrant {
        require(amount > 0, "Amount cannot be zero");
        require(block.timestamp < timePeriod, "Unstaking time, cannot stake anymore");
        require(amount <= erc20Contract.balanceOf(msg.sender), "Not enough tokens in your wallet, please try lesser amount");
        erc20Contract.safeTransferFrom(msg.sender, address(this), amount);
        stakedBalances[msg.sender] = stakedBalances[msg.sender].add(amount);
        totalStaked = totalStaked.add(amount);
        remainigStake = remainigStake.add(amount);
        emit tokensStaked(msg.sender, amount);
    }

    function unstakeTokens() external timestampIsSet noReentrant {
        require(stakedBalances[msg.sender] > 0, "Your staked balance is 0");
        if (block.timestamp >= timePeriod) {
            uint256 amount = stakedBalances[msg.sender];
            stakedBalances[msg.sender] = stakedBalances[msg.sender].sub(amount);
            uint256 rewardAmount = calculateReward(amount);              
            erc20Contract.safeTransfer(msg.sender, amount.add(rewardAmount));
            remainigStake = remainigStake.sub(amount);
    ...
    function calculateReward(uint256 amnt) public view returns (uint256) {
        uint256 stakedPrcnt = (amnt.mul(100000)).div(totalStaked);
        uint256 _rewardAmount = (stakedPrcnt.mul(totalReward)).div(100000);
        return _rewardAmount;
    }

    function addReward(uint256 _amount) external onlyOwner noReentrant {
    ...
    function transferAccidentallyLockedTokens(IERC20 token, uint256 amount) external onlyOwner noReentrant {
        require(amount > 0, "Amount cannot be zero");
