...
contract Factory {
    ...
    function createSecurityContract(
        ...
        string memory _addressOfAsset,
        string memory _country,
        string memory _city
    ) external returns (address exchangeCreated) {
        tokenIdCounter.increment();
        uint256 currentTokenId = tokenIdCounter.current();

        SecurityContract exchange = new SecurityContract(
            _name,
            _symbol,
            _description,
            _addressOfAsset,
        ...
        tokenToContract[currentTokenId] = address(exchange);

        emit SecurityContractCreated(address(exchange));
...
contract SecurityContract is ERC20 {
    ...
    constructor(
        ...
        tokenPrice = 1;
        addressOfAsset = _addressOfAsset;
        country = _country;
        city = _city;
    }
    ...
    function buyTokens() external payable {
        // require(whitelist[msg.sender], "Not on the whitelist of the project");
    ...
    function addOnWhitelist(address _candidate) external {
        require(msg.sender == institutionAgentAddress, "Unauthorised opperation");
    ...
    function contractURI() public view returns (string memory) {
        ...
        writer = writer.writeStringProperty("country", country);
        writer = writer.writeStringProperty("city", city);
        writer = writer.writeStringProperty("address", addressOfAsset);
