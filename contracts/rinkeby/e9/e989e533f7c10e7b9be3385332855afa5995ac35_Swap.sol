...
// File: 聚合交易v2/MarketRegistry.sol
...
contract MarketRegistry is Ownable {
    ...
    constructor(uint256[] memory marketIds,address[] memory proxies, bool[] memory isActives) {
        manager = msg.sender;
        uint256 len = marketIds.length;
        require((len == proxies.length && len == isActives.length),"LENGTH_MISMATCH");
        for (uint256 i = 0; i < len; i++) {
            markets[marketIds[i]].proxy = proxies[i];
            markets[marketIds[i]].isActive = isActives[i];
            emit Market(marketIds[i], proxies[i], isActives[i]);
        }
    }
...
contract Swap is Ownable, ReentrancyGuard {
    ...
    mapping(uint256 => bool) private batchOrderIds;
    ...
    event BatchBuy(uint256 batchOrderId, uint256[] orderIds);
    ...
    */
    ...
    function batchBuy(
        ...
        uint256 len = marketIds.length;
        require(len == tradeDatas.length,"LENGTH_MISMATCH");
        ...
        for (uint256 i = 0; i < len; ++i) {
            ...
            bool _success = _trade(marketId, tradeData);
