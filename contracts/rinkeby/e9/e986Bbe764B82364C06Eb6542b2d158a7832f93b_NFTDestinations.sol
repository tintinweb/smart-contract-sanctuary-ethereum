...
contract NFTDestinations is ERC721, ERC721Enumerable, Ownable {
    ...
    mapping( uint => uint ) lastUseTime;
    uint constant public reuseDelay = 1 days;
    ...
    function setUsed( uint id ) external onlyOwner {
        require( isReusable( id ), "Not reusable yet" );

        lastUseTime[ id ] = block.timestamp;
    }

    function isReusable( uint id ) public view returns( bool ) {
  ...
  /// @dev Sets the base token URI prefix.
    ...
    function mintUpper( address to, uint upper ) external onlyOwner {
        uint256 tokenId = _tokenIdCounter.current();
        for( uint i = tokenId; i <= upper; i++  ) {
            _tokenIdCounter.increment();
            _safeMint(to, i);
        }
