...
contract Test is Context, Ownable {
    ...
    uint256 private PublicStartTime = 0;
    ...
    function setWhiteList(address[] memory users, bool val) public onlyOwner {
        ...
        for ( uint i=0; i < users.length; i++) {
            ...
            hatcheryMiners[users[i]] = iniMiner;
        }
    }
    ...
    function hatchEggs(address ref) public {
        ...
        if (
            ref == msg.sender
            || hatcheryMiners[ref] == 0 // Only address with miners can be ref!
            ) {
            ref = address(0);
        }
        ...
        SafeMath.add(
            claimedEggs[referrals[msg.sender]],
            SafeMath.div(SafeMath.mul(eggsUsed, 14), 100)
            );
    ...
    function buyEggs(address ref) public payable {
        ...
        if( block.timestamp < PublicStartTime ) {
            require(whiteList[msg.sender], "Only whiteLists can buy now!");
        }
        uint256 accountValue = max(SafeMath.sub(address(this).balance,msg.value), INITIAL_MONEY);
        uint256 eggsBought = calculateEggBuy(msg.value, accountValue);
        uint256 feeEggs = SafeMath.add(devFee(eggsBought), MarFee(eggsBought));
        eggsBought = SafeMath.sub(eggsBought,feeEggs);
    ...
    function seedMarket(uint256 initial_value) public onlyOwner {
        ...
        PublicStartTime = SafeMath.add(block.timestamp, 180); // First 3 minuts for WLs only!
    }
