...
contract HASHDroid is ERC721slimBatch {
    ...
    string private _baseURI = "ipfs://QmZNkpwNcvQ1vWj5MzstuxArVrrqiw4fosE2iQjosNb8ZV/";
    ...
    IRocketPass rocketPass;
    mapping(address => uint256) public mintedRP;
    uint256 maxSupply = 100;

  constructor(address _rocketPass) ERC721slimBatch("HASHDroid", "HD") {
  ...
  function rpMint(uint256 _quantity) public callerIsUser() {
    ...
    require(rocketPass.balanceOf(_msgSender(), 1) - mintedRP[_msgSender()] > 0, "Insufficient Rocket Passes remaining");
  ...
  function depreciateMint() external onlyOwner {
    ...
    uint256 excess = maxSupply - totalSupply();
    if (excess > 0){
