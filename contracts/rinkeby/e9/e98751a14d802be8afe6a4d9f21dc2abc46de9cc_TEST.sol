...
contract TEST is ERC721Enumerable, Ownable, ERC721Burnable, ERC721Pausable {
    ...
    modifier saleIsOpen {
        if (_msgSender() != owner()) {
            require(SALE_OPEN == true, "SALES: Please wait a big longer before buying ;)");
        }
    ...
    function mint(address payable _to, uint256[] memory _ids) public payable saleIsOpen {
        ...
        if (_isPresale == true) {
            require(_whitelist[_to] == true, "Not on the whitelist!");
        }

        require(total + _ids.length <= _maxElements, "MINT: Current count exceeds maximum element count.");
        require(total <= _maxElements, "MINT: Please go to the Opensea to buy TEST NFT.");
        require(_ids.length <= _maxMint, "MINT: Current count exceeds maximum mint count.");

        if (_to != owner()) {
            require(msg.value >= price(_ids.length), "MINT: Current value is below the sales price of NFT");
        }
