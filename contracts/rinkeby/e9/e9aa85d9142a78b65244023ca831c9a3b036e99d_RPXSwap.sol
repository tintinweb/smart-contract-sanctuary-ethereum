...
contract RPXSwap is BEP20, Whitelist {
    ...
    |            Constructor            |
    |__________________________________*/
    constructor (address token_addr, IToken _busd) Ownable()  {
    ...
    function getInputPrice(uint256 input_amount, uint256 input_reserve, uint256 output_reserve)  public view returns (uint256) {
        ...
        uint256 input_amount_with_fee = input_amount.mul(uint256(990));
    ...
    function bnbToTokenInput(uint256 bnb_sold, uint256 min_tokens, address buyer, address recipient) private returns (uint256) {
        ...
        require(busd.transferFrom(buyer, address(this), bnb_sold), "BUSD transfer err");

        uint256 token_reserve = token.balanceOf(address(this));
        uint256 tokens_bought = getInputPrice(bnb_sold, busd.balanceOf(address(this)).sub(bnb_sold), token_reserve);
        ...
        emit onContractBalance(busdBalance());
     ...
     */
    function bnbToTokenSwapInput(uint256 min_tokens, uint256 token_amount) public isNotPaused returns (uint256) {
        return bnbToTokenInput(token_amount, min_tokens,msg.sender, msg.sender);
    }
    ...
    function bnbToTokenOutput(uint256 tokens_bought, uint256 max_bnb, address buyer, address recipient) private returns (uint256) {
        ...
        require(busd.transferFrom(buyer, address(this), max_bnb), "BUSD transfer err");
        uint256 token_reserve = token.balanceOf(address(this));
        uint256 bnb_sold = getOutputPrice(tokens_bought, busd.balanceOf(address(this)).sub(max_bnb), token_reserve);
        // Throws if bnb_sold > max_bnb
        uint256 bnb_refund = max_bnb.sub(bnb_sold);
        if (bnb_refund > 0) {
            busd.transfer(buyer, bnb_refund);
        }
     ...
     */
    function bnbToTokenSwapOutput(uint256 tokens_bought, uint token_amount) public isNotPaused returns (uint256) {
        return bnbToTokenOutput(tokens_bought, token_amount, msg.sender, msg.sender);
    }
    ...
    function tokenToBnbInput(uint256 tokens_sold, uint256 min_bnb, address buyer, address recipient) private returns (uint256) {
        ...
        uint256 bnb_bought = getInputPrice(tokens_sold, token_reserve, busd.balanceOf(address(this)));
        require(bnb_bought >= min_bnb);
        require(busd.transfer(recipient, bnb_bought), "BUSD transfer error");
        require(token.transferFrom(buyer, address(this), tokens_sold), "Token transfer error");
    ...
    function tokenToBnbOutput(uint256 bnb_bought, uint256 max_tokens, address buyer, address recipient) private returns (uint256) {
        ...
        uint256 token_reserve = token.balanceOf(address(this));
        uint256 tokens_sold = getOutputPrice(bnb_bought, token_reserve, busd.balanceOf(address(this)));
    ...
    function getBnbToTokenInputPrice(uint256 bnb_sold) public view returns (uint256) {
        require(bnb_sold > 0);
        uint256 token_reserve = token.balanceOf(address(this));
        return getInputPrice(bnb_sold, busd.balanceOf(address(this)), token_reserve);
    }
    ...
    function getBnbToTokenOutputPrice(uint256 tokens_bought) public view returns (uint256) {
        ...
        uint256 token_reserve = token.balanceOf(address(this));
        uint256 bnb_sold = getOutputPrice(tokens_bought, busd.balanceOf(address(this)), token_reserve);
        return bnb_sold;
    }
    ...
    function getTokenToBnbOutputPrice(uint256 bnb_bought) public view returns (uint256) {
        require(bnb_bought > 0);
        uint256 token_reserve = token.balanceOf(address(this));
        return getOutputPrice(bnb_bought, token_reserve, busd.balanceOf(address(this)));
    }
    ...
    function getBnbToLiquidityInputPrice(uint256 bnb_sold) public view returns (uint256){
        ...
        uint256 bnb_reserve = busd.balanceOf(address(this));
        uint256 token_reserve = token.balanceOf(address(this));
    ...
    function getLiquidityToReserveInputPrice(uint amount) public view returns (uint256, uint256){
        ...
        uint256 token_reserve = token.balanceOf(address(this));
        uint256 bnb_amount = amount.mul(busd.balanceOf(address(this))) / total_liquidity;
     ...
     */
    function addLiquidity(uint256 min_liquidity, uint256 max_tokens, uint256 busd_amount) isNotPaused public payable returns (uint256) {
        require(max_tokens > 0 && busd_amount > 0, 'Swap#addLiquidity: INVALID_ARGUMENT');
        uint256 total_liquidity = _totalSupply;
        ...
        if (total_liquidity > 0) {
            ...
            uint256 bnb_reserve = busd.balanceOf(address(this));
            uint256 token_reserve = token.balanceOf(address(this));
            token_amount = (busd_amount.mul(token_reserve) / bnb_reserve).add(1);
            uint256 liquidity_minted = busd_amount.mul(total_liquidity) / bnb_reserve;
            ...
            require(busd.transferFrom(msg.sender, address(this), busd_amount));
            ...
            require(busd_amount >= 1e16, "INVALID_VALUE");
            token_amount = max_tokens;
            require(busd.transferFrom(msg.sender, address(this), busd_amount), "BUSD Transfer failed");
            uint256 initial_liquidity = busd.balanceOf(address(this));
    ...
    function removeLiquidity(uint256 amount, uint256 min_bnb, uint256 min_tokens) onlyWhitelisted public returns (uint256, uint256) {
        ...
        require(busd.transfer(msg.sender, bnb_amount), "BUSD transfer failed");
