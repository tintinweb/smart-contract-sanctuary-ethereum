    ...
    ERC2771ContextUpgradeable
{
    ...
    struct AVAILABLE_USER_DISCOUNT {
        uint256 user_amount;
        uint256 user_discount;
        uint256 token_price;
        uint256 price_with_discount;
    }
    ...
    function safeMint(address to, uint256 ID) public payable {
        if ((ID.sub(1)) >= AVAILABLE_NFTS.length) {
            revert("NFT not available");
        }

        // uint256 birrasBalance = getBirrasBalance(msg.sender);

        // if (birrasBalance > 0) {
        //     require(msg.value >= AVAILABLE_NFTS[ID].token_price.sub(getAvailableDiscountForToken(ID, msg.sender)), "Insufficient payment");
        // } else {
        ...
        string memory metadataURL = AVAILABLE_NFTS[ID].token_url;
    ...
    function getAvailableDiscountForToken(uint256 _token, address _address)
        public
        view
        returns (AVAILABLE_USER_DISCOUNT memory)
    {
        ...
        for (uint256 i = 0; i < AVAILABLE_DISCOUNTS.length; i++) {
            if (userTokenAmount >= AVAILABLE_DISCOUNTS[i].token_amount) {
                discount = AVAILABLE_DISCOUNTS[i].token_discount >= discount ? AVAILABLE_DISCOUNTS[i].token_discount : discount;
            }
        }

        priceWithDiscount = discount.div(100);
        priceWithDiscount = priceWithDiscount.mul(tokenPrice);

        AVAILABLE_USER_DISCOUNT memory response = AVAILABLE_USER_DISCOUNT(
            userTokenAmount,
            discount,
            tokenPrice,
            priceWithDiscount
