...
contract TestContract is ERC721A, ERC2981, Ownable {
    using Strings for uint256;

    uint256 private _maxSupply = 5000;
    mapping(address => bool) private _minted;

    constructor(string memory baseURI, address reserve) ERC721A("TestContract", "TEST") {
        _baseTokenURI = baseURI;
        _safeMint(reserve, 100);
    }

    function mint() external payable {
        require(tx.origin == msg.sender,    "err");
        require(totalSupply() < _maxSupply, "err");
        require(!_minted[msg.sender],       "err");
