...
contract TestSBOT is ERC721A, Ownable, ReentrancyGuard, DutchAuctionSC {
  ...
  ) external payable nonReentrant qtyValidation(_qty) {
  ...
  function checkSignature(
    ...
    require(
      keccak256(abi.encodePacked(msg.sender, _qty, _nonce)).toEthSignedMessageHash().recover(_signature) ==
        signer_address,
...
contract DutchAuctionSC {
    ...
    constructor(uint256 _startPrice, uint256 _endPrice, uint256 _stepTime, uint256 _stepPrice){
        _setDutchAcutionParams(_startPrice, _endPrice, _stepTime, _stepPrice);
    }
    ...
    function currentPrice() public view returns(uint256){
        ...
        if (elapsed < duration) {
            uint256 price = startPrice.sub(
                elapsed.mul(startPrice.sub(endPrice)).div(
                    duration
               )
            );
    ...
    function _roundPrice(uint256 _price) private view returns(uint256){
        uint256 result = (_price / stepPrice);
        if(result > 0) {
            result ++;
        }
    ...
    function _setDutchAcutionParams(uint256 _startPrice, uint256 _endPrice, uint256 _stepTime, uint256 _stepPrice) internal {
