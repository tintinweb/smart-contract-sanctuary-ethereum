...
contract Hallheroes is ERC721Enumerable, Ownable, PaymentSplitter {
  ...
  uint public maxFree = 10000;
  uint public wlMaxPerTx = 3;
  ...
  function pause(bool _state) public onlyOwner {
    ...
    function getMintInfos(address _account) external view returns(uint[10] memory) {
        uint[10] memory infos = [wlMaxPerTx, wlMaxPerWallet, wlMintWallet[_account], publicMaxPerTx, publicMaxPerWallet, publicMintWallet[_account], claimedMint[_account] ? 1 : 0, totalSupply(), maxFree, uint(sellingStep)];
        return infos;
    }
