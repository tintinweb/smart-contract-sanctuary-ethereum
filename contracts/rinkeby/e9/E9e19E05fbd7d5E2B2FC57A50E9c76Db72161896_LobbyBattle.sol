...
contract LobbyBattle is Ownable, Multicall, Randomness {
  ...
  mapping(uint256 => mapping(address => uint256[])) public lobbyHeroes;
  ...
  function createLobby(
    ...
    for (uint256 i = 0; i < heroIds.length; i++) {
      lobbyHeroes[lobbyId][msg.sender].push(heroIds[i]);
    }
  }
  ...
  function joinLobby(uint256 lobbyId, uint256[] calldata heroIds) public {
    ...
    if (lobbies[lobbyId].capacity == 1) {
      lobbies[lobbyId].winner = contest1vs1(
        lobbyHeroes[lobbyId][lobbies[lobbyId].host],
        heroIds
      );
      if (lobbies[lobbyId].winner == 1) {
        heroManager.bulkExpUp(
          lobbyHeroes[lobbyId][lobbies[lobbyId].host],
          heroBonusExp(lobbyHeroes[lobbyId][lobbies[lobbyId].host][0])
        );
        heroManager.bulkExpUp(
          lobbyHeroes[lobbyId][lobbies[lobbyId].client],
          heroBonusExp(lobbyHeroes[lobbyId][lobbies[lobbyId].client][0]) / 2
        );
      } else {
        heroManager.bulkExpUp(
          lobbyHeroes[lobbyId][lobbies[lobbyId].host],
          heroBonusExp(lobbyHeroes[lobbyId][lobbies[lobbyId].host][0]) / 2
        );
        heroManager.bulkExpUp(
          lobbyHeroes[lobbyId][lobbies[lobbyId].client],
          heroBonusExp(lobbyHeroes[lobbyId][lobbies[lobbyId].client][0])
        );
      }
    }
