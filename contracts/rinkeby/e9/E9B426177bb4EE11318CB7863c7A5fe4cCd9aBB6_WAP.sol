...
contract WAP is Ownable {
    ...
    uint256 public MONTHLY_COST   = 0.01 ether;
    // the price for a yearly subscription (365 days)
    uint256 public YEARLY_COST    = 0.10 ether;
    // the price for a unlimited subscription
    uint256 public UNLIMITED_COST = 0.50 ether;
    // the maximum timestamp value
    uint256 public constant MAX_TIME = type(uint256).max;
    ...
    */
    function payMonthlySubscription() public payable {
        ...
        require(subscriptions[msg.sender] != MAX_TIME, "Already a unlimited subscription");
        ...
        subscriptions[msg.sender] = startTime + (31 days);
    }
    ...
    */
    function payYearlySubscription() public payable {
        ...
        subscriptions[msg.sender] = startTime + (365 days);
    }

    /**
    * @dev pay for 365 additional days of subscription, the yearly cost is charged.
    */
    function payUnlimitedSubscription() public payable {
        require(newSubscriptionsEnabled, "New subscriptions not allowed");
        require(subscriptions[msg.sender] != MAX_TIME, "Already a unlimited subscription");
        require(UNLIMITED_COST == msg.value, "Ether value sent is not correct");

        subscriptions[msg.sender] = MAX_TIME;
    }

    /**
     * @dev update all the subscription costs
     */
    function updateCosts(uint256 monthlyCost, uint256 yearlyCost, uint256 unlimitedCost) onlyOwner() public {
        MONTHLY_COST   = monthlyCost;
        YEARLY_COST    = yearlyCost;
        UNLIMITED_COST = unlimitedCost;
    }
    ...
    function giftSubscription(address[] calldata wallets, uint256 numDays) onlyOwner() public {
        uint256 endTime = block.timestamp + (numDays * (1 days));

        for(uint i = 0; i < wallets.length; i++){
            subscriptions[wallets[i]] = Math.max(subscriptions[wallets[i]], endTime);
        }
    }
