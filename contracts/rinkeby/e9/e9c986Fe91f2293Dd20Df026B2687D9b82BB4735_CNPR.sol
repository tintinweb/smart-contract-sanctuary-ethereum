...
contract CNPR is CNPRadmin, ERC721A {
    ...
    function burnMint(
        ...
        require(
            burnMintStructs[burnMintIndex].numberOfBurnMintsByAddress[
                msg.sender
            ] +
     ...
     *  @dev The token held by the operation are minted by the constructor, but this function is used when it is necessary to increase them further in an emergency.
     *  Only callable by the owner or admin.
     *  @param _to The Address to send token.
     *  @param _quantity The amount of tokens to be minted.
     */
    function adminMint(address _to, uint256 _quantity) external onlyAdmin {
        require(_quantity != 0, "the quantity is zero");
        require(
            _quantity + totalSupply() <= MAX_SUPPLY,
            "claim is over the max supply"
...
abstract contract CNPRadmin is CNPRcore {
     ...
     *  @param _address The address to be set in numberOfBurnMintsByAddress.
    ...
    {
        return
            burnMintStructs[burnMintIndex].numberOfBurnMintsByAddress[_address];
    }
}
...
abstract contract CNPRcore is Ownable {
    ...
    struct BurnMintStruct {
        bool isDone;
        mapping(address => uint256) numberOfBurnMintsByAddress;
    }
