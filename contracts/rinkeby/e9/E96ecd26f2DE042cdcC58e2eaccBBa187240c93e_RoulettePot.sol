...
contract RoulettePot is ERC721URIStorage, Ownable, ReentrancyGuard {
    using Counters for Counters.Counter;
    Counters.Counter public _tokenIds;

    mapping(uint256 => Casino) tokenIdToCasino;
    ...
    constructor() ERC721('PulsePotRoulette', 'PLSR') {}

    modifier onlyCasinoOwner(uint256 tokenId) {
        require(ownerOf(tokenId) == msg.sender, 'Not Casino Owner');
        _;
    }
     ...
     */
    function mint(string memory tokenURI, address newTokenAddress) external onlyOwner {
        _tokenIds.increment();
        ...
        Casino storage newCasino = tokenIdToCasino[newItemId];
        newCasino.tokenAddress = newTokenAddress;
     ...
     */
    function _getMaximumReward(Bet[] calldata bets) internal pure returns (uint256) {
        uint256 maxReward;
    ...
    function placeBetsWithTokens(uint256 tokenId, Bet[] calldata bets) external nonReentrant {
        ...
        uint256 maxReward = _getMaximumReward(bets);
    ...
    function placeBetsWithEth(uint256 tokenId, Bet[] calldata bets) external payable nonReentrant {
        ...
        require(msg.value == totalBetAmount, 'Not correct bet balance');
        require(maxReward <= casinoInfo.liquidity + totalBetAmount, 'Not enough liquidity');
        casinoInfo.liquidity += totalBetAmount;
