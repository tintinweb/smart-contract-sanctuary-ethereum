...
contract EscrowNFT is VRFConsumerBaseV2, Ownable {
    ...
    function createEscrow(
        ...
        address _buyerAddress,
        Payment _payment
    ) external returns (uint256 sellerToBuyerId) {
        require(_nftAddress != address(0) && _token1Address != address(0));
        require(_buyerAddress != msg.sender);
        IERC721 nft = IERC721(_nftAddress);
        sellerToBuyerId = _createRekberId();
        ...
        sellerToBuyer[sellerToBuyerId] = rekberData;
        nft.transferFrom(msg.sender, address(this), _tokenId);
        emit NewEscrow(_tokenId,  sellerToBuyerId,_nftAddress, msg.sender, _buyerAddress);
    }

    function cancleEscrow(uint256 _rekberId) external onlySeller(_rekberId) {
        require(sellerToBuyer[_rekberId].status == Status.Pending);
        IERC721 nft = IERC721(sellerToBuyer[_rekberId].nftAddress);
        uint256 escrowTokenId = sellerToBuyer[_rekberId].tokenId;
        sellerToBuyer[_rekberId].tokenId = 0;
        sellerToBuyer[_rekberId].status = Status.Canceled;
        nft.safeTransferFrom(address(this), msg.sender, escrowTokenId, "");
        emit NewCancle(
            msg.sender,
            sellerToBuyer[_rekberId].buyerAddress,
            escrowTokenId
        );
    }

    function _createRekberId() private view returns (uint256 rekberID) {
        uint256 firstNumber = randomWords[0].mod(1000) + 1;
        uint256 secondNumber = randomWords[1].mod(500) + 1;
        rekberID = firstNumber.add(secondNumber);
    }
}
