//SPDX-License-Identifier: Unlicense
    ...
    contract GrazeMan is ERC721, Ownable, ERC721Enumerable {
      using SafeMath for uint256;
      using Strings for uint256;
    
      uint256 public constant tokenPrice = 1000 wei; // 0.0019 ETH 
      ...
      uint256 public presaleMaxMint = 15;
      uint256 public devReserve = 64;
    
      event GrazeManMinted(uint256 tokenId, address owner);
    
      constructor() ERC721("GrazeMan", "GM") {}
      ...
      function mintGrazeMan(uint256 numberOfTokens) external payable {
        ...
        for (uint256 i = 0; i < numberOfTokens; i++) {
          ...
          if (totalSupply() < MAX_TOKENS) {
            ...
            emit GrazeManMinted(id, msg.sender);
          }
        }
      }
    
      function presaleGrazeMan(uint256 numberOfTokens) external payable {
        ...
        returns (string memory)
      {
        ...
        if (isRevealed == false) {
          return
            "ipfs://QmfSX8D8i4xD9Q2uCD3XBHQgWgwaCuitGmudZ9EfiBuzoD/hidden.json";
        }
