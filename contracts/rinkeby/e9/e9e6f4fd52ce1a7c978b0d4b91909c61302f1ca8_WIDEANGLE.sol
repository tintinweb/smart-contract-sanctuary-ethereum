...
contract WIDEANGLE is ERC721, ReentrancyGuard {
    using Strings for uint256;
    address private WIDEANGLE_ADMIN_WALLET;
    address public WIDEANGLE_OWNER;
    IERC20 private constant USDT = IERC20(0xc7AD46e0b8a400Bb3C915120d284AafbA8fc4735);
    struct Category {
        uint256 counter;
        uint256 maxSupply;
    ...
    event PricesChanged(uint256 diamond, uint256 gold, uint256 silver, uint256 bronze);
    ...
    modifier isExistCategory(uint256 _category){
        require(_category>=0,"Wrong category.");
        require(_category<=3,"Wrong category.");
        _;
    }

    function addWhitelist(address[] memory whitelistAddresses) internal onlyOwner {
    ...
    function preSaleMint(uint256 [] memory _categories/*,bytes32 messageHash, bytes calldata signature*/)
    external
    nonReentrant
    presaleTime
    payable
    ...
    function changePublicSaleMintPrice(uint256 diamond, uint256 gold, uint256 silver, uint256 bronze) external onlyOwner{
        ...
        emit PricesChanged(diamond,gold,silver,bronze);

    }
    function changePreSaleMintPrice(uint256 diamond, uint256 gold, uint256 silver, uint256 bronze) external onlyOwner{
