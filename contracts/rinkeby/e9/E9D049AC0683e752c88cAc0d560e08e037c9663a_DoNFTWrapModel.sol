    ...
    IDoNFTV2
{
    ...
    function getStartTime(uint256 tokenId) public view returns (uint64) {
        return doNftMapping[tokenId].startTime;
    }

    function getEndTime(uint256 tokenId) public view returns (uint64) {
    ...
    function mint(
        ...
        address to,
        address user
    ) public onlyNow(startTime) nonReentrant returns (uint256 tid) {
        ...
        if (startTime == block.timestamp) {
            _checkIn(tid, info.originalNftId, info.originalNftAddress, user, endTime);
        }
    ...
    function _checkIn(uint256 tokenId, uint256 originalNftId, address originalNftAddress, address user,  uint64 expires) internal virtual {
    }
    ...
    function checkIn(address to, uint256 tokenId) public virtual {
        ...
        IERC4907(info.originalNftAddress).setUser(info.originalNftId, to, info.endTime);
    }
    ...
    function couldRedeem(uint256 tokenId) public view virtual returns (bool) {
        ...
        return info.endTime == type(uint64).max;
    }
...
// File contracts/v2/DoNFTWrapModel.sol
...
contract DoNFTWrapModel is DoNFTV2 {
        ...
        returns (uint256 tid)
    {
        ...
        require( onlyApprovedOrOwner(msg.sender, oNFTAddress, originalNftId), "only approved or owner");
        IERC721(oNFTAddress).safeTransferFrom(
                    lastOwner,
                    address(this),
                    originalNftId
                );
        ...
        IERC4907(wrapNftAddress).setUser(originalNftId, lastOwner, type(uint64).max);
    }
        ...
        virtual
    {
        ...
        address oNFTAddress = IWrapNFT(info.originalNftAddress).originalAddress();
