...
// Made by : ManuelH#0001 / Manuel for MysteryEgg.
...
contract MysteryEgg is ERC721A, Ownable {
	...
	using Strings
	for uint256;
	uint256 private maxSupply = 1206;
	// --- Merkle Roots ---
	...
	modifier transactionCheck(uint8 mintType) {
		...
		require(addressHasBlock[msg.sender] < block.timestamp, "CANNOT_TRANSACT_ON_SAME_BLOCK");
	...
	// --- Metadata --- //
     ...
     /**
     * @notice Setting BaseURI for metadata, will be shown on OpenSea. (Only owner)
     */
    ...
    /**
     * @notice Set unrevealed Metadata URI, will be shown upon reveal on OpenSea. (Only owner)
     */
    ...
    /**
     * @notice Set the merkle root for PreSale, using proofs it will check if someone is whitelisted. (Only owner)
     */
     ...
     * @notice Set the merkle root for PreSale, using proofs it will check if someone is whitelisted. (Only owner)
     */
	function setRaffleMerkleRoot(bytes32 _root) external onlyOwner {
    ...
    /**
     * @notice Set mint price in WEI for PreSale (use a unit converter). (Only owner)
     */
	function setPreSalePrice(uint256 _price) external onlyOwner {
		isPresaleCost = _price;
	}
    /**
     * @notice Set mint price in WEI for RaffleSale (use a unit converter). (Only owner)
     */
	function setRafflePrice(uint256 _price) external onlyOwner {
		isRaffleCost = _price;
	}
    /**
     * @notice Set mint price in WEI for PublicSale (use a unit converter). (Only owner)
     */
    ...
    /**
     * @notice Set max mints per address by whitelisted people. (Only owner)
     */
	function setMaxPresaleMint(uint8 _maxMint) external onlyOwner {
		isMaxPresaleMints = _maxMint;
	}
    /**
     * @notice Set max mints per address by raffled people. (Only owner)
     */
	function setMaxRaffleMint(uint8 _maxMint) external onlyOwner {
		isMaxRaffleSaleMints = _maxMint;
	}
    /**
     * @notice Set max mints per address in public sale. (Only owner)
     */
	function setMaxPublicMint(uint8 _maxMint) external onlyOwner {
	...
	// --- Revealing --- //
	 ...
	 /**
     * @notice Reveals the BaseURI to the no longer hide metadata, will update on OpenSea. (Only owner)
     */
    ...
    /**
     * @notice Pauses the contracty in case of corruption. (Only owner)
     */
	...
	function withdrawal() external onlyOwner {
		...
		if (TeamPaid == false) {
			if (address(this).balance < 4000000000000000000) {  // If less balance, to prevent contract failures it will pay all to the address
				(bool os, ) = payable(developerContract).call {
					value: address(this).balance
				}("");
				require(os);
                TeamPaid = true;
			} else {
				(bool Contract, ) = payable(developerContract).call {
					value: 2000000000000000000
				}("");
				require(Contract);

				(bool DApp, ) = payable(developerDApp).call {
					value: 2000000000000000000
				}("");
				require(DApp);
