...
contract RoboBearGameS1 is IERC721Receiver, Pausable, Ownable {

  struct Staker {
    uint256 currentYield;
    uint256 accumulatedAmount;
    uint256 lastCheckpoint;
    uint256[] stakedRoboBear;
  }

  mapping(address => Staker) private _stakers;
  mapping(address => mapping(uint256 => address)) private _ownerOfToken;

  address public batteryAddress;
  address public roboBearAddress;
  uint256 public constant SECONDS_IN_DAY = 24 * 60 * 60;

  function setBatteryAddress(address contractAddress) public onlyOwner {
    batteryAddress = contractAddress;
  }

  function setRoboBearAddress(address contractAddress) public onlyOwner {
    roboBearAddress = contractAddress;
  }
  ...
  function stake(uint256[] memory tokenIds) public whenNotPaused {
    ...
    for (uint256 i = 0; i < tokenIds.length; i++) {
      require(IRoboBear(roboBearAddress).ownerOf(tokenIds[i]) == _msgSender(), "Token does not belong to you");
      IRoboBear(roboBearAddress).transferFrom(_msgSender(), address(this), tokenIds[i]);
      
      _ownerOfToken[roboBearAddress][tokenIds[i]] = _msgSender();

      newYield += 100;

      user.stakedRoboBear.push(tokenIds[i]);
    }
  ...
  function claimReward() public whenNotPaused {
    uint256 amount = accumulate(_msgSender());
    IERC20(batteryAddress).transfer(_msgSender(), amount);
  ...
  function accumulate(address staker) internal returns (uint256) {
  ...
  function getStakerTokens(address staker) public view returns (uint256[] memory) {
    return _stakers[staker].stakedRoboBear;
  }
