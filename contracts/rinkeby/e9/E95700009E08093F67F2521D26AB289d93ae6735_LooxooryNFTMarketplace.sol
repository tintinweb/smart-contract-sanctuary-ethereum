...
contract LooxooryNFTMarketplace is ReentrancyGuard,Ownable {
    ...
    function createCampaign(
      CreateCampaignStruct memory _campaignDTO
        ...
        for(uint256 i=0;i<_campaignDTO.nfts.length;i++){
          if(nftItems[_campaignDTO.nfts[i]].owner==nftHolder && nftItems[_campaignDTO.nfts[i]].campaignID==0){
        ...
        campaigns[currentCampaignId] = Campaign({
          ...
          ngoDescription:_campaignDTO.ngoDescription,
          isEnded:false,
    ...
    function createMarketItem(
      CreateMarketItemStruct memory marketItem
        ...
        require(marketItem.platformFee>=5 && marketItem.platformFee<=100,"Invalid platform fee");
        _itemIds.increment();
  ...
  function createMarketSale(
    ...
    uint256 amount
      ...
      nftItems[itemId].owner = payable(msg.sender);
  ...
  function changeNftContractAddress(address _newNftContract) external onlyOwner{
    require(_newNftContract!=nftContract,"New address cannot be same with previous address");
