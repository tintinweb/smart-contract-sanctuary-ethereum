...
contract OmnuumNFT1155 is ERC1155Upgradeable, ReentrancyGuardUpgradeable, OwnableUpgradeable {
    ...
    event Deployed(address indexed nftContract, address projectOwner);
    ...
    function initialize(
        ...
        (bool deployCompletion, ) = _nftImpl.call(abi.encodeWithSignature('deployed(address)', _prjOwner));
        require(deployCompletion, 'Deploy failure');
    }

    function deployed(address _prjOwner) external {
        require(OwnableUpgradeable(msg.sender).owner() == _prjOwner, 'only project Owner');
        require(msg.sender.isContract(), 'only Contract');
        emit Deployed(msg.sender, _prjOwner);
    }
