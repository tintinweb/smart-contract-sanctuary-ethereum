...
contract NEWLINELFGHELLO7 is ERC721AQueryable, Ownable, ReentrancyGuard {
  ...
  function mint(uint256 _mintAmount) public payable saleIsOpen {
		...
		require(_mintAmount <= maxAllowedTokensPerPurchase,'Exceeds NFT per transaction limit');
		require(_totalMinted() + _mintAmount <= maxSupply,'Exceeds max supply');
        require(_walletMintedCount[msg.sender] + _mintAmount <= maxAllowedTokensPerWallet,'Exceeds max per wallet');
