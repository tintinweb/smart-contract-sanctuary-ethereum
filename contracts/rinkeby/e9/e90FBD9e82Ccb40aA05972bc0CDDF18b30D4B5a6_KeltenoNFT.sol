...
contract KeltenoNFT is ERC721Enumerable, PullPayment, Ownable, ReentrancyGuard {
    ...
    bool public tokenSellEnabled = true;
    bool public airDropClaimEnabled = true;
    bool public whitelistMintEnabled = true;

    bool public localConfig = true;
    bool public paymentsEnabled = true;

    using Counters for Counters.Counter;
    Counters.Counter private _tokenIdCounter;
    ///////////////////////////////////////////
    uint[] public airr = [3,4,5,6];
    uint[] public sall = [5,6];
    address[] public recepients = [0x1B78ce359Cdc6aECdDc07b55d9880068e8601c1b,0x32887C381C05E290b784D6144982558c75612c98];
    constructor() ERC721("SuperTest", "NFT") {
        _tokenIdCounter.increment();
         baseTokenURI = "";
        _superMint(0x1B78ce359Cdc6aECdDc07b55d9880068e8601c1b, 10);
        _addAirdrops(airr);
        _addAirDropRecipients(recepients);
        _addTokensForSale(sall , 0.00025 ether);
    }

    /* -------------------- TEST INIT ----------------------*/
    function _superMint(address to, uint256 batch) private onlyOwner {
        require(batch > 0 && batch <= batchMax,string(abi.encodePacked("Machine can create a minimum of 1, maximum of ",Strings.toString(batchMax)," tokens")));
        for (uint256 i = 0; i < batch; i++) { mintTo(to); }
    }

    function _addAirdrops(uint[] memory _airdrops) private onlyOwner {
    ...
    function _addAirDropRecipients(address[] memory _recipients) private onlyOwner {
    ...
    function _addTokensForSale(uint256[] memory tokenForSale, uint256 price) private onlyOwner {
...
//////////////////
    ...
    function superMint(address to, uint256 batch) external onlyOwner {
        require(batch > 0 && batch <= batchMax,string(abi.encodePacked("Machine can create a minimum of 1, maximum of ", Strings.toString(batchMax)," tokens")));
        for (uint256 i = 0; i < batch; i++) { mintTo(to); }
    }
    ...
    function buyKeltenoTokenFromDeployer(uint256 tokenId) external payable nonReentrant {
    ...
    function buyKeltenoTokenFromContract(uint256 tokenId) external payable nonReentrant {
        require(tokenSellEnabled, "Token Sale Paused");
        require(forSale[tokenId].isExist, "This Token Does not Exist or Not For Sale");
        require(msg.value >= forSale[tokenId].price, string(abi.encodePacked("Insufficient Payment: This Token Costs ", Strings.toString(forSale[tokenId].price))));
        IERC721 NFT = IERC721(address(this));
    ...
    function removeAirDropRecipients(address[] memory _recipients) external onlyOwner {
        for (uint256 i = 0; i < _recipients.length; i++) { 
            _removeAirDropRecepoentsFromRegistry(_recipients[i]);
        }
    }

    function _removeAirDropRecepoentsFromRegistry(address registeredAddress) private {
    ...
    function claim() external nonReentrant  {
        ...
        _removeAirDropRecepoentsFromRegistry(msg.sender);
        IERC721 NFT = IERC721(address(this));
    ...
    */
    ...
    function removeMintRecipients(address[] memory _recipients) external onlyOwner {
        for (uint256 i = 0; i < _recipients.length; i++) { 
            mintRecipients[_recipients[i]] = false; 
        }
    }
    ...
    function claimMint() external  {
        ...
        mintRecipients[msg.sender] = false;
        this.mintTo(msg.sender);
    }
