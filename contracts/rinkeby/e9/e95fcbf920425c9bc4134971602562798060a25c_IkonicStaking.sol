...
contract IkonicStaking is Ownable,ReentrancyGuard {
    ...
    mapping(uint=>uint ) public totalStakedAmountPerPool;
    ...
    event SetToken(address owner, address tokenAdress);
    event IsAbleToStake(bool status);
    event BlockUser(address _userAddress, bool status);
    event UnBlockUser(address _userAddress, bool status);
    event StartStaking(uint[] _APR, uint[] _poolSize, uint[] _locktime,uint stakes);
   // Staker Details 
   ...
   // Set Reward Token Address
    function setTokenAddress(address _stakeTokenAddress) public onlyOwner {
        require(!isStakeTokenSet,"Already Token Set");
        isAbleToStake=true;
        stakeToken = IERC20(_stakeTokenAddress);
        emit SetToken(msg.sender, _stakeTokenAddress);
    }

    function startStaking(uint[] memory _APR,uint[] memory _poolValue,uint[] memory _locktime,uint _stakes) external onlyOwner{
       require(!isStakingStarted,"Staking Started");
       isStakingStarted=true;
        APR=_APR;
        poolSize = _poolValue;
        lockTime=_locktime;
        maxIkonStakes=_stakes;
        emit StartStaking(_APR, _poolValue,_locktime,_stakes);
    }
  ...
  // User Staking function
    function stake(uint256 poolType) public {
        ...
        if(poolType==poolTypes[poolType]){
            ...
            totalStakedAmountPerPool[poolType]+=poolSize[poolType];
             ...
             stakeToken.safeTransferFrom(msg.sender, address(this),poolSize[poolType]);
    ...
    //Reward Claim Function 
       function withdrawRewards (uint id) public nonReentrant{ 
          require(!isBlocked[msg.sender],"User Blocked");
           ...
           require(totalRewards>0,"Zero Rewards");
            ...
            stakeToken.safeTransfer (msg.sender, totalRewards);
            RemainningRewards-=totalRewards;
            emit rewards(msg.sender,totalRewards);}
        }

     // Unstake Funtion
    function unstake(uint id) public nonReentrant{
   ...
   // Recover  Excess reward Tokens from contract
    function recoverExcessToken(address _tokenAddress, uint256 amount) public  onlyOwner {
        if(stakeToken==IERC20(_tokenAddress)){
          require(amount<=RemainningRewards,"not Enough tokens");
