...
contract NFTAuction is ERC2981Base, Commission {
    using Counters for Counters.Counter;
    ...
    function getAllBids() public view returns (Bid[] memory) {
        uint256 itemCount = totalReturns.current();
    ...
    function bid(uint _bidAmount) public onlyNotOwner {
        ...
        IERC20(tokenContract).transferFrom(msg.sender, address(this), _bidAmount);
    ...
    function auctionEnd() external {
        ...
        ended = true;
        if (highestBid != 0) {
            uint256 commission = getListingPriceByItemPrice(highestBid);
            uint256 salePrice = highestBid - commission;
            RoyaltyInfo memory royalty = NFT(nftContract).royaltyInfo(tokenId);
            if (address(royalty.recipient) != payable(address(0)) && royalty.amount > 0) {
                uint256 royaltyFee = (royalty.amount * highestBid) / 10000;
                IERC20(tokenContract).transferFrom(msg.sender, royalty.recipient, royaltyFee);
                salePrice = salePrice - royaltyFee;
            }
            IERC20(tokenContract).transfer(beneficiary, highestBid);
            emit AuctionEnded(highestBidder, highestBid);
        }
        for(uint i=0; i<totalReturns.current(); i++) {
            if(!bids[i+1].withdrawn && pendingReturns[bids[i+1].bidder] != 0) {
                IERC20(tokenContract).transfer(msg.sender, pendingReturns[bids[i+1].bidder]);
        ...
        // beneficiary.transfer(getNintySevenPercent(highestBid));
        // artist.transfer(getThreePercent(highestBid));
    }
