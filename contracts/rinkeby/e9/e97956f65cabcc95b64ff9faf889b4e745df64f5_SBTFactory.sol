...
contract SBTFactory{
    ...
    function deployNew(uint256 _salt, address owner) external {
        SubscriptionToken _contract = new SubscriptionToken{
            salt: bytes32(_salt)
        }(owner);
        ...
        returns (address)
    {
        bytes32 hash = keccak256(
            abi.encodePacked(
                bytes1(0x41), address(this), _salt, keccak256(bytecode)
            )
        );
