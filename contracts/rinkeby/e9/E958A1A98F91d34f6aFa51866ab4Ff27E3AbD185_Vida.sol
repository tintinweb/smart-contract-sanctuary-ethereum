...
contract Vida is ERC721Enumerable, Ownable {
    using SafeMath for uint256;

    string public DECK;
    uint256 public constant MAX_TOKENS = 1000;
    uint256 public constant MAX_AIRDROP = 100;
    uint256 public TOKEN_PRICE = 40000000000000000; //0.04 ETH

    bool public PREMINT_OPEN = true;
    ...
    constructor() ERC721("ViveVida", "VIDA") {
    ...
    function mintToken(uint256 amount) public payable mintIsOpen {
        require(
            totalSupply().add(amount).add(MAX_AIRDROP).sub(airdropDone) <=
                MAX_TOKENS,
            "Mint would exceed max supply"
        );
        require(!PREMINT_OPEN, "Public mint is not open yet");
        ...
        if (
            ...
            (totalSupply() == MAX_TOKENS || block.timestamp >= REVEAL_TIME)
        ) {
            startingIndexBlock = block.number;
        }
    }

    function premintToken(uint256 amount) public payable mintIsOpen {
        require(
            whitelist[msg.sender] >= amount,
            "You must be on the whitelist"
        );
        require(
            totalSupply().add(amount).add(MAX_AIRDROP) <= MAX_TOKENS,
            "Mint would exceed max supply"
        );
        require(PREMINT_OPEN, "Premint not available");
        // require(MINT_TIME > block.timestamp, "Public mint is open");
        require(
            preMintPrice(amount) <= msg.value,
    ...
    function giftTokens(address[] memory accounts) public onlyOwner {
        uint256 size = accounts.length;
        require(totalSupply().add(size) <= MAX_TOKENS, "Not enough left");
    ...
    function doMint(address to, uint256 amount) internal {
    ...
    function price(uint256 amount) public view returns (uint256) {
        return TOKEN_PRICE.mul(amount);
    }

    function preMintPrice(uint256 amount) public view returns (uint256) {
        return TOKEN_PRICE.sub(10000000000000000).mul(amount);
    }
    ...
    function airdropMint(address to, uint256 amount) public onlyOwner {
        require(
            totalSupply().add(amount).add(MAX_AIRDROP) <= MAX_TOKENS,
            "Mint would exceed max supply"
        );
    ...
    function setStartingIndex() public {
        ...
        startingIndex = uint256(blockhash(startingIndexBlock)) % MAX_TOKENS;
    ...
    function flipPremint() public onlyOwner {
        PREMINT_OPEN = !PREMINT_OPEN;
    }
}
