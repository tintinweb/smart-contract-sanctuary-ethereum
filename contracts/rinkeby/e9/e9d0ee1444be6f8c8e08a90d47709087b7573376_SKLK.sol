...
contract SKLK is ERC721AQueryable, Ownable, ReentrancyGuard {
  ...
  function mint(uint256 _mintAmount) public payable mintCompliance(_mintAmount){
    if(msg.sender != owner()){
      ...
      {
            require(msg.value >= cost * (_mintAmount-(maxMintAmountPerWallet-_freeWalletMints[_msgSender()])), "Insufficient funds!");
            if(_mintAmount>2)
            {
              _freeWalletMints[_msgSender()] = 2;

            }
            else if(_freeWalletMints[_msgSender()] < 2)
              {
      ...
      {
          // require(_mintAmount == 1,"cant mint more than 1")
