...
contract Flowers is ERC20, ERC20Burnable, AccessControl {
    ...
    constructor() ERC20("Flowers", "FLOW") {
...
contract Staking is Ownable, Flowers {

    uint256 public stakedTotal;
    uint256 public stakingStartTime;
    uint256 public rewardPerHour;
    uint256 public passiveRewardPerHour;
    ...
    address contractAddr = 0xB3F477AD9676F24e070F67D648f3c69ecC6ADB75;

    function setContractAddr(address contract_) public {
    ...
    function findElementInArray(uint element, uint[] memory arr) pure internal returns(uint index) {
    ...
    function setRewards(uint stakingReward_, uint passiveReward_) public onlyOwner returns(bool success) {
    ...
    function getStakedTokens(address user_)
        public
        view
        returns (uint256[] memory tokenIds)
    {
        return stakers[user_].tokenIds;
    }
    ...
    function stakeNFT(uint[] memory tokenIds_) public payable onlySender {
        ...
        staker.cooldown = block.timestamp + 180;
        ...
        for (uint256 i = 0; i < tokenIds_.length; i++) {
            ...
            staker.since.push(block.timestamp);
    ...
    function withdrawNFT(address[] memory users_, uint[] memory tokenIds_) public onlySender {
        for (uint256 i = 0; i < tokenIds_.length; i++) {
            require(
                tokenOwner[tokenIds_[i]] == users_[i],
                "User must be the owner of the staked NFT!"
            );
            Staker storage staker = stakers[users_[i]];
            require(
                staker.cooldown < block.timestamp,
                "Cooldown: Must wait 3 minutes after staking to withdraw!"
            );
            uint _stakeId = findElementInArray(tokenIds_[i], staker.tokenIds);
            claimStake(users_[i], _stakeId);

            if (staker.tokenIds.length > 0) { 
                staker.tokenIds[_stakeId] = staker.tokenIds[staker.tokenIds.length - 1];
                staker.tokenIds.pop();
                staker.since[_stakeId] = staker.since[staker.since.length - 1];
                staker.since.pop();
            } else {
                delete stakers[users_[i]];
            }
            delete tokenOwner[tokenIds_[i]];
            stakedTotal--;
            PassiveStake storage _stake = passiveStakes[tokenIds_[i]];
            _stake.claimed = (block.timestamp - _stake.since) * passiveRewardPerHour / 1 hours;
        }
        IExternalNft(contractAddr).resurrect(tokenIds_, users_);
        emit Unstaked(users_, tokenIds_);
    }
    ...
    function claimPassiveStake(address user_, uint tokenId_) public onlySender {
        ...
        uint _totalClaim = _totalRewards - _claimed;
        IRewardToken(address(this)).mint(user_, _totalClaim);
    }

    function claimAllPassiveStakes(address user_) public onlySender returns (uint amount) {
    ...
    function getStake(address user_, uint stakeId_) public view returns (uint[] memory stake) {
        Staker memory _staker = stakers[user_];
        uint[] memory arrResult = new uint[](2);
        arrResult[0] = _staker.tokenIds[stakeId_];
        arrResult[1] = _staker.since[stakeId_];
        return arrResult;
    }

    function claimStake(address user_, uint stakeId_) public onlySender {
        Staker storage staker = stakers[user_];

        IRewardToken(address(this)).mint(user_, getPendingStakeRewards(user_, stakeId_));
        staker.since[stakeId_] = block.timestamp;
    }
    ...
    function claimAllStakes(address user_) public onlySender {
        ...
        for (uint i = 0; i < ids.length; i++) {
            IRewardToken(address(this)).mint(user_, getPendingStakeRewards(user_, i));
            staker.since[i] = block.timestamp;
        }
    ...
    function stakedSince(address user_, uint stakeId_) public view returns (uint _since) {
        Staker memory staker = stakers[user_];
        uint _stakedSince = staker.since[stakeId_];
        return _stakedSince;
    }

    function stakeDuration(address user_, uint stakeId_) public view returns (uint _difference) {
        return block.timestamp - stakedSince(user_, stakeId_);
    }

    function getPendingStakeRewards(address user_, uint stakeId_) public view returns (uint _pending) {
        return stakeDuration(user_, stakeId_) * rewardPerHour / 1 hours;
    }
    ...
    function getPendingUserRewards(address user_) public view returns (uint256 amount_) {
        ...
        for (uint256 i = 0; i < ids.length; i++) {
            _totalReward = _totalReward + getPendingStakeRewards(user_, i);
        }
        return getPassiveStakes(user_) + _totalReward;
    }
