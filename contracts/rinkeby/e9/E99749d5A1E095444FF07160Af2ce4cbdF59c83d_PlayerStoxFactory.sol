...
// 
contract PlayerStoxFactory is Ownable {
    using EnumerableSet for EnumerableSet.AddressSet;
    ...
    function createPlayerStox(string memory name_, string memory symbol_, uint8 decimals_, uint256 totalSupply_) external onlyOwner {
        PlayerStoxToken playerStox = new PlayerStoxToken(name_, symbol_, decimals_, totalSupply_);
        playerStoxs.add(address(playerStox));
    }

    function deletePlayerStox(address playerStox_) external onlyOwner {
        require(playerStoxs.contains(playerStox_), "Player does not exist.");
        playerStoxs.remove(address(playerStox_));
    }

    function updateAdminWallet(address newAdminWallet_) external onlyOwner {
