...
contract SafeSwap{
    ...
    function SwapTokenForETH(uint tokenAmount, address token) external{
        ...
        Helper.safeTransferFrom(token, msg.sender, address(this), tokenAmount);
        Helper.safeApprove(token, UNISWAP_V2_ROUTER, tokenAmount);
    ...
    function SwapTokenForToken( 
        ...
        address _to
            ...
            Helper.safeTransferFrom(_tokenIn, msg.sender, address(this), _amountIn);
            Helper.safeApprove(_tokenIn, UNISWAP_V2_ROUTER, _amountIn);
    ...
    function withdraw(
        ...
        if(token == ETH) {
        Helper.safeTransferETH(owner, amount);
        } else {
        Helper.safeTransfer(token, owner, amount);
        }
        return true;
    }
}
