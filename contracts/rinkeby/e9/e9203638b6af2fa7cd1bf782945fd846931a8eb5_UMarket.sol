...
contract UMarket is Ownable{
    ...
    mapping(address => uint) cacheMap;
    mapping(address => uint) timeMap;
    mapping(address => uint) buyMap;
    mapping(address => uint) returnMap;
    ...
    function buyU(uint256 _amount) public {
        ...
        sendToAddr(_amount);
        saveRecord(msg.sender);
        U.transfer( msg.sender,cost);
    ...
    function saveRecord(address _to) private {
    ...
    function refundAble(address _to) public view returns (uint) {
        ...
        if(buyed > 0 && lastTime > 0){
            ...
            if(refundAmount.add(returnMap[_to]) > buyMap[_to]){
                return buyMap[_to].sub(returnMap[_to]);
            }
            return refundAmount;
        }else{
            return 0;
