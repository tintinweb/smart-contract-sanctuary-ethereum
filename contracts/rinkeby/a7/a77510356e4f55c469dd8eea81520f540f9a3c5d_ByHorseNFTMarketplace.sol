...
/* ByHorse NFT Marketplace
    ...
    & support Royalty
*/
contract ByHorseNFTMarketplace is Ownable, ReentrancyGuard, ERC1155, ERC1155Holder {
    ...
    // just to showcase byhorse
    mapping(address => bool) private ByHorseNFT;
    ...
    modifier isByHorseNFT(address _nft) {
        require(HorseNFT(_nft), "not ByHorse NFT");
        _;
    }
   
    // @notice To list only ByHorse nfts
    function HorseNFT(address _nft) public onlyOwner returns (bool) {
        ByHorseNFT[_nft] = true;
        return ByHorseNFT[_nft];
    }
    ...
    function listNft(
        ...
        address _payToken,
        uint256 _price
    ) external isByHorseNFT(_nft) isPayableToken(_payToken) {
    ...
    function resultAuction(address _nft, uint256 _tokenId) external {
        ...
        auction.winner = auction.lastBidder;//auction.creator;
        ...
        IERC1155(auction.nft).safeTransferFrom(address(this), auction.winner, auction.tokenId, 1, "");
