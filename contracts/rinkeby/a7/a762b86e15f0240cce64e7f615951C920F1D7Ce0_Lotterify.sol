...
contract Lotterify is Ownable {
    ...
    function random(address payable[] memory _ArrayToRand)
        public
        view
        returns (address payable[] memory _rand_array)
    {
        address payable[] memory ArrayToRand = _ArrayToRand;
        address payable[] memory rand_array;
        uint256 tmp_len = ArrayToRand.length;
        for (uint256 i = 0; i < ArrayToRand.length; i++) {
            uint256 i_rand = uint16(
                uint256(keccak256(abi.encodePacked(block.difficulty, now, i))) %
                    tmp_len
            );
            rand_array[i] = ArrayToRand[i_rand];

            for (uint256 ii = i_rand; ii < tmp_len - 1; ii++) {
                ArrayToRand[ii] = rand_array[ii + 1];
            }
            tmp_len -= 1;
        }
        return (rand_array);
