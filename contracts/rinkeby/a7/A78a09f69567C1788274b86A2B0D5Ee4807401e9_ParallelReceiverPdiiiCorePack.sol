...
contract ParallelReceiverPdiiiCorePack is VRFConsumerBaseV2, Ownable {
    ...
    address public pullTokensFromAddress = address(0);
    ...
    event Assigned(uint256 requestID, uint256 packID, address owner);
    ...
    function handleReceive(
        ...
        require(_cardIds[0] == 2, "needs PDIII pack reservation");
        ...
        require(
            _ethValue >= packPrice * numPacksToSendToUser,
            "insufficient ETH"
        );
        ...
        override
    {
        ...
        for (uint8 i = 0; i < deposit.numPacksToSendToUser; i++) {
            ...
            // each user has a stack of assigned packs
            assignedPackIDs[deposit.owner].push(packID);
            emit Assigned(requestID, packID, deposit.owner);
    ...
    function claim(uint256 _numberOfPacks, address owner) public {
        uint256 numAssignedPacks = assignedPackIDs[owner].length;
        require(numAssignedPacks != 0, "you have no packs");

        uint256 numPacksToSendToUser = Math.min(
            _numberOfPacks,
            numAssignedPacks
        );

        uint256[] memory cardIdsToSend = new uint256[](
            CARDPACKSIZE * numPacksToSendToUser
        );
        ...
        uint256 packID;
        uint256 i;
        // Pop all assigned packs for owner
        // Note: positionI is 1-indexed otherwise we underflow
        for (
            uint256 positionI = numPacksToSendToUser;
            0 < positionI;
            positionI--
        ) {
            i = positionI - 1;
            packID = assignedPackIDs[owner][i];
            assignedPackIDs[owner].pop();
