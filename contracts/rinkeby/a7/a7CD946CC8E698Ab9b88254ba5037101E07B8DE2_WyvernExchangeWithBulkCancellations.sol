...
contract ExchangeCore is ReentrancyGuarded, Ownable {
    ...
    bytes32 private constant _ORDER_TYPEHASH = 0x0747f5d96db4685be040725c97dfbb10f3dacef8471fee6ed72496b8d308e361;
    ...
    /* For split fee orders, minimum required protocol royality fee, in basis points. Paid to royality (who can change it). */
    uint public minimumRoyalityFee = 0;
    ...
    struct Order {
        ...
        /* Royality fee of the order */
        uint royalityFee;
        /* Order royality recipient or zero address. */
        address royalityRecipient;
    ...
    constructor () public {
        ...
        require(keccak256("Order(address exchange,address maker,address taker,uint256 makerRelayerFee,uint256 takerRelayerFee,uint256 makerProtocolFee,uint256 takerProtocolFee,address feeRecipient,uint256 royalityFee,address royalityRecipient,uint8 feeMethod,uint8 side,uint8 saleKind,address target,uint8 howToCall,bytes calldata,bytes replacementPattern,address staticTarget,bytes staticExtradata,address paymentToken,uint256 basePrice,uint256 extra,uint256 listingTime,uint256 expirationTime,uint256 salt,uint256 nonce)") == _ORDER_TYPEHASH);
    ...
    /**
     * @dev Change the minimum royality fee paid to the protocol (owner only)
     * @param newMinimumRoyalityFee New fee to set in basis points
     */
    function changeMinimumRoyalityFee(uint newMinimumRoyalityFee)
        public
        onlyOwner
    {
        minimumRoyalityFee = newMinimumRoyalityFee;
    }
        ...
        returns (bytes32 hash)
    {
        ...
        index = ArrayUtils.unsafeWriteUint(index, order.royalityFee);
        index = ArrayUtils.unsafeWriteAddressWord(index, order.royalityRecipient);
        ...
        returns (bool)
    {
        ...
        if (order.feeMethod == FeeMethod.SplitFee && (order.makerProtocolFee < minimumMakerProtocolFee || order.takerProtocolFee < minimumTakerProtocolFee || order.royalityFee < minimumRoyalityFee)) {
            return false;
        }
        ...
        returns (uint)
    {
        ...
        if (sell.feeRecipient != address(0)) {
            ...
            if (sell.feeMethod == FeeMethod.SplitFee) {
                ...
                if (sell.royalityFee > 0) {
                    uint royalityFee = SafeMath.div(SafeMath.mul(sell.royalityFee, price), INVERSE_BASIS_POINT);
                    if (sell.paymentToken == address(0)) {
                        receiveAmount = SafeMath.sub(receiveAmount, royalityFee);
                        sell.royalityRecipient.transfer(royalityFee);
                    } else {
                        transferTokens(sell.paymentToken, sell.maker, sell.royalityRecipient, royalityFee);
                    }
                }
            ...
            if (sell.feeMethod == FeeMethod.SplitFee) {
                ...
                if (buy.royalityFee > 0) {
                    royalityFee = SafeMath.div(SafeMath.mul(buy.royalityFee, price), INVERSE_BASIS_POINT);
                    transferTokens(sell.paymentToken, buy.maker, buy.royalityRecipient, royalityFee);
                }
...
contract Exchange is ExchangeCore {
    ...
    {
        return hashOrder(
          Order(addrs[0], addrs[1], addrs[2], uints[0], uints[1], uints[2], uints[3], addrs[3], uints[9], addrs[7], feeMethod, side, saleKind, addrs[4], howToCall, calldata, replacementPattern, addrs[5], staticExtradata, ERC20(addrs[6]), uints[4], uints[5], uints[6], uints[7], uints[8]),
          nonces[addrs[1]]
        );
    }
    ...
    function validateOrderParameters_ (
        ...
        returns (bool)
    {
        Order memory order = Order(addrs[0], addrs[1], addrs[2], uints[0], uints[1], uints[2], uints[3], addrs[3], uints[9], addrs[7], feeMethod, side, saleKind, addrs[4], howToCall, calldata, replacementPattern, addrs[5], staticExtradata, ERC20(addrs[6]), uints[4], uints[5], uints[6], uints[7], uints[8]);
