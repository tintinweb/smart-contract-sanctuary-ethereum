...
contract FLUF is ERC721, ERC721Enumerable, ERC721URIStorage, Ownable, ReentrancyGuard {
  ...
  constructor() ERC721("FLUF", "FLUF") {
    setTokenURI("https://api.fluf.world/api/token/");
  }
  ...
  function changeNameAndDescription(
    ...
    require(_msgSender() == owner, "This isn't your FLUF.");
  ...
  function mintFluf(address human, uint256 amountOfFlufs)
    public
    payable
    virtual
    nonReentrant
  ...
  {
    require(
      amountOfFlufs <= MAX_PURCHASE,
      "Hey, that's too many FLUFs. Save some for the rest of us!"
    );

    require(
      totalSupply().add(amountOfFlufs) <= MAX_FLUFS,
      "Sorry, there's not that many FLUFs left."
    );

    uint256 firstFlufRecieved = _nextTokenId;

    for (uint256 i = 0; i < amountOfFlufs; i++) {
