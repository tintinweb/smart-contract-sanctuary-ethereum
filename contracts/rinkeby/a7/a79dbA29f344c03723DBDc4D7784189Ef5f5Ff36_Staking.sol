...
contract Staking is Ownable , ERC721Holder {
    ...
    function stake(uint256 tokenId ,string memory rarityCode) public {
        _stake(msg.sender, tokenId, rarityCode);
    }

    function stakeBatch(uint256[] memory tokenIds , string[] memory rarityCodes ) public {
        for (uint256 i = 0; i < tokenIds.length; i++) {
            _stake(msg.sender, tokenIds[i], rarityCodes[i]);
        }
    }

    function _stake(address _user, uint256 _tokenId, string memory _rarityCode) internal {
        ...
        staker.rarityCodes.push(_rarityCode);
    ...
    function updateReward(address _user) public {
        ...
        string[] storage rarities = staker.rarityCodes;
        ...
        for (uint256 i = 0; i < ids.length; i++) {
            if (
                ...
                if(keccak256(abi.encodePacked("C")) == keccak256(abi.encodePacked(rarities[i]))){
                           staker.balance +=  token * stakedDays * 1;
                }else if (keccak256(abi.encodePacked("UC")) == keccak256(abi.encodePacked(rarities[i]))){
                    staker.balance +=  token * stakedDays * 3/2;
                }else if(keccak256(abi.encodePacked("R")) == keccak256(abi.encodePacked(rarities[i]))){
                    staker.balance +=  token * stakedDays * 27/10;
                }
                else if(keccak256(abi.encodePacked("SR")) == keccak256(abi.encodePacked(rarities[i]))){
                    staker.balance +=  token * stakedDays * 11/2;
                }
                else if(keccak256(abi.encodePacked("SSR")) == keccak256(abi.encodePacked(rarities[i]))){
                    staker.balance +=  token * stakedDays * 15;
                }
                else if(keccak256(abi.encodePacked("UR")) == keccak256(abi.encodePacked(rarities[i]))){
                    staker.balance +=  token * stakedDays * 45;
                }
