...
import "./Abstract_HolyDogs.sol";

contract HolyDogs is Abstract_HolyDogs {
    ...
    function updateBaseURI(string memory newBaseUri) external override onlyOwner whenNotPaused {
        require(keccak256(abi.encodePacked(baseUri)) != keccak256(abi.encodePacked(newBaseUri)) , "HolyDogs: Current state is already what you have selected.");
        baseUri = newBaseUri;
    }

    function updateEndTime(uint256 newMintEndTime) external override onlyOwner whenNotPaused {
        require(keccak256(abi.encodePacked(mintEndTime)) != keccak256(abi.encodePacked(newMintEndTime)) , "HolyDogs: Current state is already what you have selected.");
        require(block.timestamp <= mintEndTime , "HolyDogs: Mint process is ended");
    ...
    {
        require(keccak256(abi.encodePacked(mintPrice)) != keccak256(abi.encodePacked(newMintPrice)) , "HolyDogs: Current state is already what you have selected.");
        mintPrice = newMintPrice;
    }

    function mint() external override payable whenNotPaused nonReentrant
    {
        require(MAX_SUPPLY > totalSupply, "HolyDogs: Maximum supply is exceed");
        require(block.timestamp <= mintEndTime , "HolyDogs: Mint process is ended");
        require(msg.value >= mintPrice, "HolyDogs: Insufficient ETH supplied");
    ...
    function airDrop(address account, uint256 amount) external override onlyOwner whenNotPaused {
        for(uint16 i=0; i<amount; i++){
            require(MAX_SUPPLY > totalSupply, "HolyDogs: Maximum supply is exceed");
            uint256 tokenId = _getRandomNFTTokenID();
            totalSupply++;
            _mint(account, tokenId, NFT_AMOUNT, "");
            mintIds[tokenId] = tokenId;
            emit NFTAirDrop(tokenId);
        }
    }

    function airDropToOwner() public {
        require(block.timestamp >= mintEndTime , "HolyDogs: Mint process is in progress");
        for(uint i=0; i <= MAX_SUPPLY; i++) {
            if(mintIds[i] == 0) {
                _mint(msg.sender, i, NFT_AMOUNT, "");
            }          
        }
    }

    function withdraw(address payable account , uint256 amount) external override onlyOwner whenNotPaused nonReentrant {
        require(account != ZERO_ADDRESS, "HolyDogs: Cannot withdraw to Zero Address");
        require(address(this).balance > amount,"HolyDogs: insufficient ether in contract");
...
import "./IHolyDogs.sol";
import "./Random.sol";
abstract contract Abstract_HolyDogs is ERC1155, Ownable,Pausable,ReentrancyGuard,Random,IHolyDogs{
