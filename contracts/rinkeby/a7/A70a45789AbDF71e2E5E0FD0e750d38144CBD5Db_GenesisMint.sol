 ...
 * @title GenesisMint
 * @author ehrab
 * @notice This contract allows whitelisters to mint NFT'S. It has been inspired by, and pulls some functionality from,
 * 				the genesis and investor contracts by Zues of OlympusDAO
 */
contract GenesisMint is Ownable, Pausable {
	...
	// ========== STRUCTS ========== //
	// ! Add method of bypassing terms struct for public mint, use boolean flag? !
	// ! Unused allocation should already be handled (double check) !
	struct Term {
		...
		uint256 max; // Maximum NFT's purchasable
	}

	// ========== STATE VARIABLES ========== //

	IERC20 public usdc;
	IGenesis721 public whitelistMinter;
	...
	address internal collectionWallet = 0xf2a20A4a5380693CBe1eF455B16A6D53132c1Bc0;

	/**
	 * @notice Maximum NFT's that can possibly be allocated.
	 */
	uint256 public maximumTotalAllocation = 300;
	...
	/**
	 * @notice Tracking of allocation and purchases
	 */
	uint256 public totalAllocated;
	uint256 public totalPurchased;

	/**
	 * @notice Signal for when public mint goes live
	*/
	...
	constructor(
		address _whitelistMinter,
		address _usdc,
		uint256 _maxTotalAllocation
	) {
		whitelistMinter = IGenesis721(_whitelistMinter);
		usdc = IERC20(_usdc);
		maximumTotalAllocation = _maxTotalAllocation;
	}
	 ...
	 * @param _nftsToPurchase uint256 How many NFT's requested
	 */
	// ! NEED TO MAKE ALL PUBLIC MINT TOKEN IDs GREATER THAN 199 !
	function _purchase(uint256 _nftsToPurchase) internal {
		// Pull address purchasing to check terms
		Term memory info = terms[msg.sender];
		uint256 usdc_total;

		require(_nftsToPurchase.mod(1) == 0, "Must be increments of 1.");
		...
		if(info.price == 0) {
			...
			// ? WL MINT ? //
			// Check so that address cannot purchase more then they are allowed to purchase
			require(purchasableFor(msg.sender) < info.max, 'Purchase more than remaining allocation.');

			// Calculate total price of tokens as per price from terms
			usdc_total = (_nftsToPurchase.mul(info.price)).sub(info.credit);

			// Update state of terms
			terms[msg.sender].purchased = info.purchased.add(_nftsToPurchase);
			terms[msg.sender].spent = info.spent.add(usdc_total);
			terms[msg.sender].credit.sub(info.credit);
		}

		// usdc total required in usdc terms (6 decimals)
		usdc.safeTransferFrom(msg.sender, collectionWallet, usdc_total.mul(1e6));
		whitelistMinter.safeMint(msg.sender, _nftsToPurchase, presaleNFTUri);
	...
	function pushWalletChange(address _newAddress) external whenNotPaused {
		require(terms[msg.sender].max != 0, 'No wallet to change');
	...
	function pullWalletChange(address _oldAddress) external whenNotPaused {
		...
		require(terms[msg.sender].max == 0, 'Wallet already exists');
	...
	/**
	 * @notice Max remaining for purchase
	 * @param _address address to lookup
	 * @return Remaining for purchase
	 */
	function purchasableFor(address _address) public view returns (uint256) {
		Term memory info = terms[_address];
		return info.max.sub(purchased(_address));
	}
	 ...
	 */
	function circulatingNFT() public returns (uint256) {
		return whitelistMinter.totalSupply();
	}

	// ========== ADMIN FUNCTIONS ========== //

	/**
	 *  @notice set terms for new address, cannot terms or exceed maximum total allocation, contract owner only
	 *  @param _address address
	 *  @param _price uint256 real terms (ie 50 = 50)
	 *  @param _credit uint256 real terms
	 *  @param _spent uint256 usdc spent (6 decimals)
	 *  @param _purchased uint256 NFT's bought
	 *  @param _max uint256 Max NFT's purchasable
	 */
	function setTerms(
		address _address,
		uint256 _price,
		...
		terms[_address] = Term({ price: _price, credit: _credit, spent: _spent, purchased: _purchased, max: _max });
		require(totalAllocated.add(_max) <= maximumTotalAllocation, 'Cannot allocate more.');
		totalAllocated = totalAllocated.add(_max);
	}
	...
	function setMinterContract(address _minter) external onlyOwner {
		whitelistMinter = IGenesis721(_minter);
	}

	function addMaximumAllocation(uint256 _amount) external onlyOwner {
		maximumTotalAllocation = SafeMath.add(maximumTotalAllocation, _amount);
	}
}
...
interface IGenesis721 is IERC721 {
	/**
	...
	*/
	function safeMint(address to, uint256 qty, string memory uri) external;

	/**
	* @notice Call erc721 Enumerable function
	* @return uint256 Supply of NFT's
 */
