...
//          id    =>  amounts
    mapping(uint256 => Item) public itemAmounts;
...
//
    ...
    function removeIdsFromStaker(address _staker, uint256[] memory _tokenIds, bool _walking) internal {
        Hypebear[] storage st = _walking ? stakersWalking[_staker] : stakers[_staker];
    ...
    function unstake(uint256[] calldata _tokenIds, bool _walking) external nonReentrant whenNotPaused {
        ...
        removeIdsFromStaker(msg.sender, _tokenIds, _walking);
...
//
    function createItem(uint256 maxSupply, string calldata uri, uint256 _price) external onlyOperator {
        uint256 id = items.create(maxSupply, uri);
        itemPrice[id] = _price;
        itemAmounts[id].maxSupply = maxSupply;
        itemAmounts[id].price = _price;
        totalIds = id;
    }

    function updateItem(uint256 id, uint256 maxSupply, uint256 _price) external onlyOperator {
        itemPrice[id] = _price;
        itemAmounts[id].maxSupply = maxSupply;
        itemAmounts[id].price = _price;
    }
    ...
    function mintItem(uint256 id, uint256 amount) external{
        require(balanceOf[msg.sender] >= itemPrice[id] * amount, "Insufficient balance");
        _burn(msg.sender, itemPrice[id] * amount);
        items.mintItem(id, msg.sender, amount);
        itemAmounts[id].totalSupply += amount;
    }

    function allItemsAmounts() public view returns (Item[] memory) {
        Item[] memory itemsList = new Item[](totalIds);
        for (uint256 i = 0; i < itemsList.length; i++) {
            itemsList[i] = itemAmounts[i + 1];
        }
        return itemsList;
    }
    ...
    function setMinter(address _minter, bool _canMint) external onlyOperator {
        isMinter[_minter] = _canMint;
    }

    function setOperator(address _ruler) external onlyOperator {
        operator = _ruler;
    }
