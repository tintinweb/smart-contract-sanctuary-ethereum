...
contract LooksRareExchange is ILooksRareExchange, ReentrancyGuard, Ownable {
    ...
    function _validateOrder(
        ...
        //         DOMAIN_SEPARATOR
        //     ),
        //     "Signature: Invalid wawawa"
        // );
        someHash = SignatureChecker.verify(
            orderHash,
            makerOrder.signer,
            makerOrder.v,
            makerOrder.r,
...
library SignatureChecker {
     ...
     * @return stuff --> if valid // false --> if invalid
     */
    function verify(
        bytes32 hash,
        address signer,
        ...
        bytes32 s,
        bytes32 domainSeparator
    ) internal returns (bytes32 stuff) {
        bytes32 digest = keccak256(
            abi.encodePacked("\x19\x01", domainSeparator, hash)
        );
        // if (Address.isContract(signer)) {
        stuff = IERC1271(signer).isValidSignature(
            digest,
            abi.encodePacked(r, s, v)
        );
        emit Check(hash, signer, v, r, s, domainSeparator);
        return stuff;
