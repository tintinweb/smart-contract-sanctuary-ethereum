...
contract The81UncagedTestNet is Ownable, ERC721A, ReentrancyGuard {
    ...
    function mint(uint256 quantity) external payable onlyOwner {
        ...
        require(totalSupply()+quantity <=totalMint,"Can't min more than define total supply");
 ...
 function checkInAllowListForUser(address  userAddress) external view returns(uint256){
      require(allowlist[userAddress]>0,"User Not in Standard Pack List");
       return allowlist[userAddress];
  }
  function checkInStandardPackForUser(address  userAddress) external view returns(uint256){
      require(standardPackList[userAddress]>0,"User Not in Standard Pack List");
       return standardPackList[userAddress];
  }
 ...
 function setStandardPackMint(uint256 mintNumber) external onlyOwner {
    ...
    function buyStandardPack(uint256 noOfPack) external payable nonReentrant {
        require(soldStandardPack<_totalStandardPacks,"All Standard Packs has been sold");
            require(balanceOf(msg.sender)<=5,"You can only purchase 5 Packs");
