...
contract GMCafeMoo is Ownable, IERC165, IERC721, IERC721Metadata {
	...
	//IOpenSeaRegistry constant OS_REG = OpenSeaRegistry(0xa5409ec958c83c3f309868babaca7c86dcb077c1);
	//IERC1155 constant OS_NFT = IERC1155(0x495f947276749Ce646f68AC8c248420045cb7b5e); 
	//uint256 constant CLAIM_DELAY = 90 days;
	
	// rinkeby
	IOpenSeaRegistry constant OS_REG = IOpenSeaRegistry(0x1E525EEAF261cA41b809884CBDE9DD9E1619573A);
	IERC1155 constant OS_NFT = IERC1155(0x8d2372F1689B3cf8367E650814038E9473041Dbe);
	...
	uint256 constant MOO_CLEAR      = ~(uint256(type(uint16).max) << MOO_SHIFT); 
	uint256 constant CONST_MASK     = ~(MOO_CLEAR | TAG_CLEAR);
	...
	uint256 _migrated;
	mapping (address => uint256) _balances; // owner -> [touched, owned]
	...
	// admin
	function setTag(uint256 moo, uint16 tag) onlyOwner public {
		uint256 data = _moos[moo];
		if (data == 0) revert InvalidMoo(moo); // moo doesn't exist yet
	...
	function enableTokenURISearch(bool b) onlyOwner public {
		_tokenURLSearch = b;
	}
	...
	function getWallet(address owner) public view returns (uint256[] memory moos, uint256 touched) {
		touched = _balances[owner];
		uint256 n = uint16(touched);
		touched >>= 16;
		if (n > 0) {
			moos = new uint256[](n);
	...
	function unlockMoo(uint256 moo, string memory password, address transfer) payable public {
		...
		if (unlock.price != 0) { // must satisfy criteria
		...
		if (transfer != address(0)) {
			transferFrom(owner, transfer, moo);
		}
	}
	...
	function _addMinted(uint256 n) private {
		...
		unchecked {
			_balances[msg.sender] = prior + ((n << 16) | n); // (+n, +n)
			_migrated += n;
		}
		if (prior == 0) { // first moo migrated
			if (address(OS_REG).code.length > 0) { // opensea registry exists
				address proxy = OS_REG.proxies(msg.sender); // get proxy
				if (proxy != address(0)) { // proxy exists
	...
	function getMigratableTokens(address sender) public view returns (uint256[] memory tokens, uint256[] memory moos) {		
		...
		moos = new uint256[](n);
		for (uint256 i; i < n; i++) {
			moos[i] = _mooFromToken(tokens[i]);
		}
	}
	function migrateMoo(uint256 token) public {
		uint256 moo = _mooFromToken(token);
	...
	function claimUnmigratedMoos(uint256 limit) onlyOwner public {	
		...
		unchecked {
			...
			uint256 n;
			while (i < HERD_SIZE) {
				uint256 moo = _mooFromCollectionIndex(++i);
	...
	function transferFrom(address from, address to, uint256 moo) public {
		...
		unchecked {
			...
			_moos[moo] = (data & CONST_MASK)    // keep constant parts
				| (block.number << BLOCK_SHIFT) // current block height
				| (transfers << TRANSFER_SHIFT) // updated transfer count
				| uint160(to);                  // new owner
			_balances[from] = ((_balances[from] ^ 0xFFFF) + 1) ^ 0xFFFF; // ( 0, -1)
			_balances[to] += 0x10001;                                    // (+1, +1)
		}
	...
	function _mooFromToken(uint256 token) public pure returns (uint256) {
