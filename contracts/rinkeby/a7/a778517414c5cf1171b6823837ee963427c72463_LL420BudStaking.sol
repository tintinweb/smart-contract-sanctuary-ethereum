...
contract LL420BudStaking is Initializable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    /// bud id => Bud Info
    mapping(uint256 => BudInfo) private _budInfo;

    /* ==================== EVENTS ==================== */

    event Deposit(address indexed user, uint256[] ids);
    event DepositGameKey(address indexed user, uint256[] ids);
    event Withdraw(address indexed user, uint256[] ids);
    event WithdrawGameKey(address indexed user, uint256[] ids);
    ...
    function initialize(address _budContract, address _gameKeyContract) external initializer {
        ...
        require(_budContract != address(0), "LL420BudStaking: Wrong BUD address");
    ...
    function depositGameKey(uint256[] calldata _ids) external nonReentrant onlyStarted {
        ...
        emit DepositGameKey(_msgSender(), _ids);
    }
    ...
    function withdrawReward() external nonReentrant {
        ...
        HIGH_TOKEN_CONTRACT.withdrawReward(_msgSender(), user.reward);
     ...
     */
    function _withdraw(uint256 _id, uint256[] memory _ids) internal onlyStarted {
    ...
    function _withdrawGameKey(uint256[] memory _ids) internal {
        ...
        emit WithdrawGameKey(_msgSender(), _ids);
    }
...
interface ILL420HighToken {
    function withdrawReward(address account, uint256 amount) external;
}
