...
contract TakETH_Easy is IBEP20, Ownable, ReentrancyGuard {
    ...
    string private constant _name               = 'TakETH Easy';  
    string private constant _symbol             = 'TakETH';    
    ...
    uint16 private constant SellLimitDivider    = 500; // The minimum amount to transfer can't be set lower than 0.2% of the total supply. Again: (no honeypot possible).
    uint8 public constant MaxSellTax            = 8;  // The sell tax can't be set higher than 10%. This guarantees you safe trading (no honeypot possible).

    /*
    The tax distribution in percentages will be flexible, so we can switch to what's best for #TakETH at any time, in a reserved way 
    - without to much changes to make. It is necessary to be able to switch in the percentage share of the fees, because of: the way 
    how we can guarantee the relationship between the value of the AMM and the total MC as well as possible, as well as the developments 
    on DeFi blockchain, etc.
    **/

    uint8 private _devTax                       = 0;
    uint8 private _burnTax                      = 25;
    uint8 private _marketingTax                 = 15;
    uint8 private _liquidityTax                 = 30;
    uint8 private _stakingTax                   = 30;  
    uint8 private _buyBackTax                   = 0; 
 
    uint8 private _buyTax                       = 7;    // Max 8%
    uint8 private _sellTax                      = 5;    // Max 8%
    uint8 private _transferTax                  = 0;    // Max 3%
    
    uint8 private _MainRewardSplit              = 0;
    uint8 private _MiscRewardSplit              = 100;  // Rewards in $ETH
    ...
    address public MainReward = 0xdAC17F958D2ee523a2206206994597C13D831ec7;   //Rewards in $USDT
    address public MiscReward = 0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2;   //Rewards in $WETH
...
//constructor
    ...
    //Switch reward to new token. Cannot set newReward ETH to prevent claim function from breaking.
    ...
    //Switch reward to new token. Only Misc reward can be changed to ETH
    ...
    function _taxedTransfer(address sender, address recipient, uint256 amount,bool isBuy,bool isSell) private{
        ...
        if(isSell){
            ...
            // In order to claim your rewards, buy less than 1 token and you will receive your ETH rewards.
            ...
            // In order to claim your ETH rewards: transfer less than 1 token to another wallet, and you will receive your rewards.
    ...
    function UpdateTaxes(uint8 burnTaxes, uint8 buybackTaxes, uint8 devTaxes, uint8 marketingTaxes, uint8 liquidityTaxes, uint8 stakingTaxes,uint8 buyTax, uint8 sellTax, uint8 transferTax) public authorized{
        ...
        // The buy and sell tax can't be set higher than 8%; the transfer tax not higher than 3%. This is done in order to prevent you from being honeypotted!

        require(totalTax==100, "marketing+liq+staking needs to equal 100%.");
        require(buyTax<=8,"Buytax is higher than the 8%.");
        require(sellTax<=MaxSellTax,"Selltax is higher than 8%.");
        require(transferTax<=3,"TransferTax is higher than 3%.");
    ...
    function UpdateLimits(uint256 newMaxWallet, uint256 newSellLimit) public authorized{
        ...
        //MaxWallet and sellLimit can't be set lower than the coded limit (total supply / 500 = 0.2% of the totall suplly)
