...
contract Auction {
        ...
        )
    {
        ...
        return (
            ...
            auctionIdToAuction[_indexId].offers.owner,
    ...
    function buyFixPrice(
        ...
        if (
            ...
            priceOfNFT != 0 &&
            tokenStan.balanceOf(_buyer) < priceOfNFT
        ) revert InvalidBalance();
...
interface IListing {
    ...
    function cancelListingFixedPrice(bytes memory _listingId, bool _isWeb)
        external
        returns (uint256);

    function cancelListingAuction(bytes memory _listingId, bool _isWeb)
        external;
...
interface IOffer {
    ...
    function updateOwnerOfNFT(bytes memory _indexId, address _user) external;
