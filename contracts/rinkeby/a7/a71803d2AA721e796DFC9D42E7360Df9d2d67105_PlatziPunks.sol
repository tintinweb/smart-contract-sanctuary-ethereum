...
contract PlatziPunks is ERC721, ERC721Enumerable, PlatziPunksDNA {
    ...
    uint256 public maxSupply; //maxima cantidad del token que se puede mintear
    mapping(uint256 => uint256) public tokenDNA; //apunta desde el tokenID que puede ser cualquier número hasta el ADN

    // en el constructor nombre del token y denominacion/abreviatura
    ...
    function mint() public {
        ...
        require(current < maxSupply, "No PlatziPunks left :("); // pedir que current sea menor o igual a la demanda maxima
        
        tokenDNA[current] = deterministicPseudoRandomDNA(current, msg.sender);
        
        //parámetros: 
        //param 1: a quien vamos a añadirle la propiedad de owner
        //param 2: ¿cual es el token id que corresponde a ese NFT?
        // msg.sender: persona que ejecuta la función
        _safeMint(msg.sender, current); 


        //cada vez que creamos un nuevo NFT le agregamos el ADN
    ...
    function tokenURI(uint256 tokenId) public view override returns(string memory){
        // _exists verifica que el token existe
        require(_exists(tokenId), "ERC271 Metadata: URI query for nonexistant token");

        uint256 dna = tokenDNA[tokenId];
        string memory image = imageByDNA(dna);

        //funcion de solidity abi.encodePacked(arg); hace un packed encoding (bytes) of the given arguments
    ...
    // Agregando el soporte para ERC721Enumerable
