...
contract Lilos_Protocol_V1 {
	...
	uint256 private _max_lease_term = 864000;  // 10 days
	uint256 private _min_lease_term = 60;      // 1 mins
	...
	function listToken(
		...
		require(IERC721(collection).isApprovedForAll(msg.sender, address(this)), "Token is not approved to the contract.");
		require(collateral_value > 0, "Collateral value should be larger than zero.");
		require(rent_value > 0, "Rent value should be larger than zero.");
		require(lease_term > _min_lease_term, "Lease term should be longer than 3 minutes.");
		require(lease_term < _max_lease_term, "Lease term should be shorter than 10 days.");
	...
	function leaseIn(uint256 listingId_) external payable {
		...
		bool isLessorApprovalActive = IERC721(listingItem.collection).isApprovedForAll(msg.sender, address(this));
		if (!isLessorApprovalActive){
			listingItem.status = ListingStatus.Delisted;
		}
		require(isLessorApprovalActive, "Lessor removed the approval to the contract.");
		require(msg.value >= (listingItem.collateral_value.add(listingItem.rent_value)), "Insufficient payment");
		...
		payable(address(this)).transfer(listingItem.collateral_value.add(listingItem.rent_value.mul(platformFeeRate).div(100)));
	...
	function getItembyCollctionAndTokenId(address lessee_) public view returns (ListingItem[] memory) {
		uint256 itemCount;
		uint256 currentFilterIndex;
		for (uint i = 0; i < _listingId; i++) {
			if (_listingItems[i].lessee == lessee_) {
