...
contract SpumeSwap is Ownable, Pausable  {
   ...
   constructor(address _spumeRewardPool, address Weth) {
    ...
    function swapExactInputSingle(address _tokenIn, address _tokenOut, uint amountIn) external returns (uint amountOut) {
     ...
     */  
    function depositToRewardPool() external whenNotPaused {
        uint256 amount = weth.balanceOf(address(this));
        require(amount > 0);
        IERC20(weth).approve(address(spumeRewardPool), amount); 
    ...
    /*
     * Gets the Balance of ERC20 for this contract
     */  
    function getBalances(address token) public view  {
...
interface ISpumeRewardPool {
    ...
    function rewardSwap(uint256 _rewardTokenAmount, uint256 _rewardAmount) external returns (uint256 rewardAmount);
