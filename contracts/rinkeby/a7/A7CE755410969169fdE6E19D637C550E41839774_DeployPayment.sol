...
contract DeployPayment is ReentrancyGuardUpgradeable, NFTChecker, OwnableUpgradeable {
    ...
    function initialize(uint256 _fee, uint256 _deployFund, address _owner, address _deployer) external initializer {
        require(_fee > 0, "fee is zero value");
        require(_deployFund > 0, "deployFund is zero value");
        require(_owner != address(0), "owner is the zero address");
        require(_deployer != address(0), "deployer is the zero address");
        fee = _fee;
    ...
    function setDeployer(address _newDeployer) external onlyOwner {
        require(_newDeployer != address(0), "new Deployer is the zero address");
        address oldDeployer = deployer;
        deployer = _newDeployer;
        emit SetDeployer(oldDeployer, _newDeployer);
    }
    ...
    function setFee(uint256 _newFee) external onlyOwnerOrController {
        require(_newFee > 0, "New Fee is zero value");
        uint256 oldFee = fee;
        fee = _newFee;
    ...
    function setDeployFund(uint256 _newDeployFund) external onlyOwner {
        require(_newDeployFund > 0, "New DeployFund is zero value");
