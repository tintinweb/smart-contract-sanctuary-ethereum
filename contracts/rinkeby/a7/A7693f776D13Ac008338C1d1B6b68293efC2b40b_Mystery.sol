...
contract Mystery is ERC721A, Ownable {
    using Strings for uint256;
    ...
    mapping(address => bool) public freeSaleMinted;
    mapping(address => uint256) public publicSaleMinted;

    // ======== TREASURY ========
    address public constant TREASURY = 0xa55340B0A82fceFefdb22Ed0670e593910126F9e;

    // ======== FOUNDERS & TEAM ========    
    address public constant TEAM = 0xac543c52C562AB0a3942d1CB3F3757779852d792; 

    // ======== CONSTRUCTOR ========      
    constructor() ERC721A("MYSTERY", "MSTRY") {}
    ...
    /**
     * @notice free sale mint
     */
    function freeSaleMint(bytes32[] calldata _proof) external payable callerIsUser {
        require(isFreeMintSaleActive, "FREE SALE NOT ACTIVE");
        require(msg.value == freeMintSalePrice, "NEED TO SEND CORRECT ETH AMOUNT");
        require(totalSupply() + maxFreeMint <= MAX_SUPPLY, "MAX SUPPLY REACHED" );
        require(!freeSaleMinted[msg.sender], "EXCEEDS MAX CLAIM"); 
        bytes32 sender = keccak256(abi.encodePacked(msg.sender));
        bool isValidProof = MerkleProof.verify(_proof, freeSaleMerkleRoot, sender);
        require(isValidProof, "INVALID PROOF");

        freeSaleMinted[msg.sender] = true;
    ...
    /**
     * @notice activating or deactivating free sale status
     */
    function setFreeSaleStatus(bool _status) external onlyOwner {
        isFreeMintSaleActive = _status;
    }
    ...
    /**
     * @notice set free mint price
     */
    function setFreeMintSalePrice(uint256 _price) external onlyOwner {
        freeMintSalePrice = _price;
    }
    ...
    /**
     * @notice set free sale merkleroot
     */
    function setfreeSaleMerkleRoot(bytes32 _merkleRoot) external onlyOwner {
        freeSaleMerkleRoot = _merkleRoot;
    }
