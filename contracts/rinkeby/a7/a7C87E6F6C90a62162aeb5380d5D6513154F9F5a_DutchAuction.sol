...
contract DutchAuction is Ownable, ERC721A {

    //Starting at 0.1 ether
    uint256 public DA_STARTING_PRICE = 0.1 ether;

    //Ending at 0.01 ether
    uint256 public DA_ENDING_PRICE = 0.01 ether;

    //Decrease by 0.001 every frequency.
    uint256 public DA_DECREMENT = 0.001 ether;

    //decrement price every 15.
    uint256 public DA_DECREMENT_FREQUENCY = 15;
    ...
    //The quantity for DA.
    uint256 public DA_QUANTITY = 10000;

    uint256 MAX_MINT_PER_TX = 3; // Max mints per transaction for Dutch Auction
    ...
    function mint(uint8 _amt) public payable{
        require(_amt > 0 && _amt <= MAX_MINT_PER_TX, "Requested mint amount invalid.");
        require(DA_STARTING_TIMESTAMP >= block.timestamp, "Dutch Auction Sale has not begun.");
        uint256 price = currentPrice();
        require(msg.value >= _amt * price, "Invalid Payment");
        require(DA_QUANTITY >= totalSupply() + _amt, "Requested mint amount puts total supply over cap.");

        // If we have reach mint cap, then record the final/cheapest price paid
        DA_FINAL_PRICE = totalSupply() + _amt == DA_QUANTITY ? price : 0;

        userToTokenBatchPriceData[msg.sender].push(TokenBatchPriceData(uint128(msg.value),_amt));

        _safeMint(msg.sender,_amt);
    }

    function userToTokenBatchLength(address user) public view returns (uint256) {
    ...
    function currentPrice() public view returns (uint256) {
        require(DA_STARTING_TIMESTAMP > 0, "DA has not started!");
    ...
    function startDutchAuction() public onlyOwner {
        DA_STARTING_TIMESTAMP = block.timestamp;
    }
