...
contract BURNER is Context, ERC721Burnable {
	...
	function mint(uint8 amount) public payable virtual {
		require(!_paused, _strPaused);
		...
		mintMe(_msgSender(), amount);
	}
	
	function mintMe(address addy, uint8 amount) internal {
		for(uint cycle = 0; cycle < amount; cycle++) {
			uint8 _index;
			bytes32 seed = keccak256(abi.encodePacked(block.timestamp, block.difficulty, addy));
