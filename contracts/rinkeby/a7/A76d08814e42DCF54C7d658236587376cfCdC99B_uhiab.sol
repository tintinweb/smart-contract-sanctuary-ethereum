...
contract uhiab is Ownable, ERC721A, ReentrancyGuard {
  ...
  function presaleMint(uint256 quantity) external payable callerIsUser onlyWhiteList{
    ...
    uint256 totalCost = _presalePrice * quantity;
    ...
    callerIsUser
  {
    ...
    uint256 _publicsalePrice = uint256(config.publicsalePrice);
    uint256 _saleStartTime = uint256(config.publicsaleStartTime);
  ...
  function setSaleInfo(uint32 timestamp, uint64 _presalePrice, uint64 _publicsalePrice, uint64 _publicsaleMaxMint) external onlyOwner {
    saleConfig.presaleStartTime = timestamp;
    saleConfig.publicsaleStartTime = timestamp + 1 days;
