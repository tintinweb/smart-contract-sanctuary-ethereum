 ...
 */
contract LucksExecutor is ILucksExecutor, ReentrancyGuardUpgradeable, ERC721HolderUpgradeable, OwnableUpgradeable {    
    ...
    function createTask(TaskItem memory item, TaskExt memory ext, lzTxObj memory _param) external payable override nonReentrant {
        ...
        if (isPunks(item.nftContract)) {
            for (uint256 i = 0; i < item.tokenIds.length; i++) {
                _depositPunk(item.tokenIds[i]);
            }     
            nftContract = address(HELPER.getWrappedPunks());
        }

        // Transfer nfts to proxy station (NFTChain) 
        // in case of dst chain transection fail, enable user redeem nft back, after endTime
        uint256 depositId = NFT.deposit(msg.sender, nftContract, item.tokenIds, item.tokenAmounts, item.endTime);
    ...
    */
    ...
    function _doWithdrawNFTs(uint256 depositId, address nftContract, address user) internal {
        ...
        // adapt to CryptoPunks
        if (isPunks(nftContract)) {

            DepositNFT memory nft = NFT.getNFT(address(this),depositId);

            NFT.withdraw(depositId, user);

            for (uint256 i = 0; i < nft.tokenIds.length; i++) {
                _withdrawPunk(nft.tokenIds[i], user);
            }   
        }
        else {
            NFT.withdraw(depositId, user);
    ...
    // =============== CryptoPunks support // Begin ========================== 
    function isPunks(address nftContract) internal view returns(bool){ 
        if (address(HELPER.getPunks()) == nftContract && address(HELPER.getWrappedPunks()) != address(0)) {
            return true;
        }
        return false;
    }
    ...
    function _depositPunk(uint256 punkIndex) internal {
        ...
        IWrappedPunks wrappedPunks = HELPER.getWrappedPunks();
        
        address holder = punks.punkIndexToAddress(punkIndex);
        require(holder == msg.sender, "Punks: not owner of punkIndex");

        punks.buyPunk(punkIndex);
        punks.transferPunk(wrappedPunks.proxyInfo(address(this)), punkIndex);

        wrappedPunks.mint(punkIndex);
    }

    function _withdrawPunk(uint256 punkIndex, address to) internal {
        IPunks punks = HELPER.getPunks();
        IWrappedPunks wrappedPunks = HELPER.getWrappedPunks();

        address holder = wrappedPunks.ownerOf(punkIndex);
        require(holder == msg.sender, "Punks: caller is not owner");
        require(holder == to, "Punks: to is not owner");

        wrappedPunks.safeTransferFrom(to, address(this), punkIndex);
        wrappedPunks.burn(punkIndex);
        punks.transferPunk(to, punkIndex);
    }

    // =============== CryptoPunks support // End ==========================
    ...
    function registerPunks() external onlyOwner {
        IWrappedPunks wrappedPunk = HELPER.getWrappedPunks();
        if (address(wrappedPunk) != address(0)) {
            wrappedPunk.registerProxy();  

            IERC721Upgradeable(address(wrappedPunk)).setApprovalForAll(address(NFT), true);
        }      
    }
}
...
import {IWrappedPunks} from "./IWrappedPunks.sol";
...
interface ILucksHelper {
    ...
    function getWrappedPunks() external view returns (IWrappedPunks);
