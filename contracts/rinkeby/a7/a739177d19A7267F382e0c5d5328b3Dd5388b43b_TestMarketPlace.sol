...
contract ERC721Mochi is ERC721, ERC721Enumerable, ERC721URIStorage, AccessControlEnumerable, ERC721Burnable {
    ...
    function autoMint(string memory _tokenURI, address to) public {
        uint id;
        do {
...
contract TestMarketPlace is ERC721Mochi, ReentrancyGuard {
    ...
    constructor(IERC20 _payment_token, address _owner, string memory name, string memory symbol) ERC721Mochi(_owner, name, symbol) {
    ...
    function getPrice(uint256 tokenId) public view returns (uint256, uint256, uint256) {
     ...
     function buy(uint256 tokenId) public nonReentrant {
        ...
        payment_token.approve(msg.sender, 3000);
    ...
    function callOptionalReturn(IERC721 token, bytes memory data) private {
        require(address(token).isContract(), "ERR7");
        (bool success, bytes memory returndata) = address(token).call(data);
        require(success, "ERR9");

        if (returndata.length > 0) {
            require(abi.decode(returndata, (bool)), "ERR8");
        }
    }
