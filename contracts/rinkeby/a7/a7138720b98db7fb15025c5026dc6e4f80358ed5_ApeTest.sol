...
pragma solidity ^0.8.0;
contract ApeTest is Ownable, ERC721A, ReentrancyGuard {
    ...
    uint256 public maxCountPerAccount = 3; 
    ...
    constructor() ERC721A("ApeTest", "APET") {
    }

    function mint(uint256 numberOfTokens) external payable onlyActive nonReentrant() {
        require(numberOfTokens > 0, "zero count");
        require(numberOfTokens <= MAX_SUPPLY.sub(totalSupply()), "Sold out");
        require(numberOfTokens.add(minted[msg.sender]) <= maxCountPerAccount, "you already max minted");
