...
contract Subscribe is Ownable {
    ...
    function updateInfluencerInvoicesInfo() private returns(uint) {

        require(influencers[msg.sender].length > 0, "There are no invoices used by your promo code");
        uint influencerWiithdrawableBalance = 0;
        for (uint i = 0; i < influencers[msg.sender].length; i++) {
            SubscribedPlan storage lBid = subscribers[influencers[msg.sender][i]];
            if(refundPeriod < block.timestamp - lBid.date && !lBid.refunded && !lBid.influencer_withdrawed) {

                influencerWiithdrawableBalance = influencerWiithdrawableBalance + lBid.influencer_percent * lBid.plan_price / 100;
                subscribers[influencers[msg.sender][i]].influencer_withdrawed = true;
            }
        }
    ...
    function withdrawByInfluencer() public payable {
        ...
        uint withdrawSumary = updateInfluencerInvoicesInfo();
