...
contract Moment is ERC721, Ownable, ReentrancyGuard {
  ...
  address private _neortAddress;
  address private _vesselAddress;
  bool private _isOnSale = false;
  uint256 public startDate;
  uint256 public endDate;
  uint256 public nftPrice = 0.025 ether;
  string private _baseTokenURI;
  mapping(uint256 => bool) private _neortTokens;
  mapping(uint256 => bool) private _vesselTokens;

  Counters.Counter private _tokenIdTracker;

  constructor(
    string memory baseTokenURI,
    address neortAddress,
    ...
    startDate = start;
    endDate = end;
    _neortAddress = neortAddress;
    _vesselAddress = vesselAddress;
  }
  ...
  function isNEORTHolder(uint256 tokenId) internal view returns (bool) {
    return IERC721(_neortAddress).ownerOf(tokenId) == msg.sender;
  }

  function isVesselHolder(uint256 tokenId) internal view returns (bool) {
    return IERC721(_vesselAddress).ownerOf(tokenId) == msg.sender;
  }
  ...
  {
    require(
      isNEORTHolder(neortTokenId),
      "Moment: You must have NEORT or Vessel token"
    );
    require(!_neortTokens[neortTokenId], "Moment: Already claimed");
    _neortTokens[neortTokenId] = true;
    _incrementalMint(msg.sender);
  }
  ...
  {
    require(
      isVesselHolder(vesselTokenId),
      "Moment: You must have NEORT or Vessel token"
    );
    require(!_vesselTokens[vesselTokenId], "Moment: Already claimed");
    _vesselTokens[vesselTokenId] = true;
    _incrementalMint(msg.sender);
  }

  function withdraw() external nonReentrant onlyOwner {
    require(address(this).balance > 0, "Moment: No balance");
