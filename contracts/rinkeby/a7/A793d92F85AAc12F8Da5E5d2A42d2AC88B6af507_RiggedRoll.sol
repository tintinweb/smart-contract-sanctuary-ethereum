...
contract RiggedRoll is Ownable {
    ...
    function withdraw(address payable _addr, uint256 _amount) public{
        require(_amount <= address(this).balance, "You are trying to withdraw more eth than the contract has");
        require(_addr == Ownable.owner());
        // Call returns a boolean value indicating success or failure.
        (bool sent, bytes memory data) = _addr.call{value: _amount}("");
        require(sent, "Failed to send Ether");
    }

    //Add riggedRoll() function to predict the randomness in the DiceGame contract 
    //and only roll when it's going to be a winner
    function riggedRoll() public payable {
        console.log("in rigged roll");
        require(address(this).balance >= 0.002 ether, "This contract does not have enough eth to roll");
