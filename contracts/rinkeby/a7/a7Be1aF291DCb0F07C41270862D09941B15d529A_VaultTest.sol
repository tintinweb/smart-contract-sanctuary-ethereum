...
import '../../vault/Supply.sol';
import '../../vault/Royalty.sol';
 ...
 * along with {MINTER_ROLE} and {BURNER_ROLE} roles, which allow to mint and burn tokens on behalf of customers
 *
 ...
 */
contract VaultTest is ERC1155PausableUpgradeable, Supply, Royalty, IVault {
    ...
    /**
     * @dev Grants `DEFAULT_ADMIN_ROLE`, `MINTER_ROLE`, `BURNER_ROLE` to the admin
     *
     * Initialize the name and the baseUri of the Vault
     */
    function initialize(
        ...
        uint16 defaultRoyalty_
    ) external initializer {
        _baseUri = baseUri_;
        name = name_;
        royaltyReceiver = royaltyReceiver_;
        defaultRoyalty = defaultRoyalty_;
     ...
     */
    function getProofOfIntegrity(uint256 tokenId) public view returns (bytes32) {
        require(_exists(tokenId), 'Alt Vault: Nonexistent token.');
    ...
    function uri(uint256 _id)
        ...
        returns (string memory)
    {
        require(_exists(_id), 'Alt Vault: Nonexistent token.');
    ...
    function _beforeMint(uint256 id, uint256 amount) internal view {
        require(_royaltyExists[id], 'Vault: royalty is not set');
        if (_maxSupplyExists(id)) {
            require(
                _maxSupply[id] > _currentSupply[id] + amount - 1,
                'Vault: mint amount exceeds max supply'
            );
        }
    }
    ...
    ///
    function mintPackDrop(address to_, bytes32 proof_)
        external
        onlyMinter
        returns (uint256)
    {
        uint256 tokenId = _tokenId(proof_);
    ...
    function mintPipeline(
        ...
        uint256 tokenId = _tokenId(proof_);
        _setMaxSupply(maxSupply_, tokenId);
        _setTokenRoyalty(tokenId, royaltyReceiver, royalty_);
    ...
    function _beforeTokenTransfer(
        ...
        if (to == address(0)) {
            for (uint256 i = 0; i < ids.length; ++i) {
                ...
                require(
                    supply > amount - 1,
                    'Alt Vault: burn amount exceeds currentSupply'
                );
                unchecked {
...
/**
 * @dev {ERC-2981} implementation
 *
 * https://eips.ethereum.org/EIPS/eip-2981
 *
 * A standardized way to retrieve royalty payment information for non-fungible tokens
 * to enable universal support for royalty payments across all NFT marketplaces and
 * ecosystem participants.
 *
 */
contract Royalty is Roles, ERC2981Base, IRoyalty {
    mapping(uint256 => RoyaltyInfo) internal _royalties;
    mapping(uint256 => bool) internal _royaltyExists;
    address public royaltyReceiver;
    uint16 public defaultRoyalty;

    /// @inheritdoc IRoyalty
    function setDefaultRoyalty(uint16 value) external onlyVaultAdmin {
        _setDefaultRoyalty(value);
    }

    function _setDefaultRoyalty(uint16 value) internal {
        require(value < 10001, 'Royalty: Default Royalty too high');
        emit DefaultRoyaltyChanged(defaultRoyalty, value);
        defaultRoyalty = value;
    }
    ...
    function setBatchTokenRoyalty(
        ...
        for (uint256 i = 0; i < ids.length; i++) {
            _setTokenRoyalty(ids[i], recipients[i], values[i]);
        }
    }

    /// @inheritdoc IRoyalty
    function setRoyaltyReceiver(address newReceiver) external onlyVaultAdmin {
        _setRoyaltyReceiver(newReceiver);
    }

    function _setRoyaltyReceiver(address newReceiver) internal {
        require(
            newReceiver != address(0),
            'Royalty: Cannot update receiver to address 0'
        );
        emit RoyaltyReceiverChanged(royaltyReceiver, newReceiver);
        ...
        returns (address receiver, uint256 royaltyAmount)
    {
        ...
        if (_royaltyExists[_tokenId]) {
            royalty = _royalties[_tokenId];
        }
        else {
            royalty.value = defaultRoyalty;
            royalty.recipient = royaltyReceiver;
        }
        receiver = royalty.recipient;
        royaltyAmount = (_salePrice * royalty.value) / 10000;
    }
    ...
    function _setTokenRoyalty(
        ...
        require(value < 10001, 'Royalty: Royalties too high');
        _royalties[id] = RoyaltyInfo(recipient, value);
        _royaltyExists[id] = true;
    }
    ...
    function supportsInterface(bytes4 interfaceId)
        ...
        view
        virtual
        override(AccessControlEnumerableUpgradeable, ERC2981Base)
        returns (bool)
    {
        return
            interfaceId == type(IERC2981).interfaceId ||
...
interface IVault is IAccessControlUpgradeable, IERC1155Upgradeable, IERC2981 {
...
contract Roles is AccessControlEnumerableUpgradeable {
     ...
     */
    function transferAdmin(address _to) public onlyVaultAdmin {
     ...
     * @dev the burner role.
     */
    function BURNER_ROLE() internal pure returns (bytes32) {
     ...
     */
    function grantMinterRole(address account) public onlyVaultAdmin {
     ...
     * @dev grant the BURNER_ROLE role that allows to minting new tokens.
     */
    function grantBurnerRole(address account) public onlyVaultAdmin {
     ...
     */
    function revokeMinterRole(address account) public onlyVaultAdmin {
     ...
     * @dev revoke the MINTER_ROLE role.
     */
    function revokeBurnerRole(address account) public onlyVaultAdmin {
...
abstract contract ERC2981Base is ERC165Upgradeable, IERC2981 {
    struct RoyaltyInfo {
        address recipient;
        uint16 value;
    }
...
interface IRoyalty is IERC2981 {
    /// @notice Emitted when the default royalty is changed
    /// @param oldRoyalty The Default Royalty before
    /// @param newRoyalty The Default Royalty after
    event DefaultRoyaltyChanged(uint16 indexed oldRoyalty, uint16 indexed newRoyalty);

    /// @notice Emitted when the Royalty Receiver address is updated
    /// @param oldReceiver The Royalty Receiver before
    /// @param newReceiver The Royalty Receiver after
    event RoyaltyReceiverChanged(address indexed oldReceiver, address indexed newReceiver);

    /// @notice Sets a new  Default Royalty Value
    /// @param value royalty percentage in basis points (10000 == 100%)
    function setDefaultRoyalty(uint16 value) external;

    /// @notice Sets the ERC-2981 royalty info for token `id`.
    /// @param id token Id
    /// @param recipient address that receives the royalties
    /// @param value royalty percentage in basis points (10000 == 100%)
    function setTokenRoyalty(
        uint256 id,
        address recipient,
    ...
    /// @notice Sets the ERC-2981 royalty info for tokens as a batch.
    /// @param ids token Ids
    /// @param recipients addresses that receive the royalties
    /// @param values royalty percentages in basis points (10000 == 100%)
    function setBatchTokenRoyalty(
        uint256[] calldata ids,
        address[] calldata recipients,
        uint16[] calldata values
    ...
    /// @notice Sets the receiver address for the royalties
