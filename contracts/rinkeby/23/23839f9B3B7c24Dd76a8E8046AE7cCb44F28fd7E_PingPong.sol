...
contract PingPong is ILayerZeroReceiver, ILayerZeroUserApplicationConfig {
    ...
    uint8 public constant MAX_PINGS = 5;
    uint256 public totalPingCount;

    event Ping(uint8 pingCount, uint256 totalPingCount);
    event Pong(uint8 pingCount, uint256 totalPingCount);

    constructor(address _layerZeroEndpoint) {
        layerZeroEndpoint = ILayerZeroEndpoint(_layerZeroEndpoint);
        console.log("..__Deployed__..");
    }
    ...
    function ping(
        ...
        require(
            ...
            "PingPong: Please send ETH for TX Fees"
        );
        require(
            _pings < MAX_PINGS,
            "PingPong: Maximum Pings reached. Stopping Loop"
        );

        emit Ping(_pings, totalPingCount);
        ...
        (uint256 msgFee, ) = layerZeroEndpoint.estimateFees(
            _dstChainId,
            address(this),
            payload,
            false,
        ...
        layerZeroEndpoint.send{value: msgFee}(
            _dstChainId,
    ...
    function lzReceive(
        ...
        uint8 pings = abi.decode(_payload, (uint8));
        emit Pong(pings, totalPingCount);
