...
contract Urn is ERC20("URN", "URN"), ReentrancyGuard, Ownable(5, true, true) {
    /// Track controller contracts and their permissions
    /// @dev bool[mint, burn, transfer]
    mapping(address => bool[3]) private _controllers;

    /// Authorises known addresses to mint, burn, or transfer URN tokens.
    /// @param contractAddress The address to associate permissions
    /// @param canMint Is the address allowed to mint URN
    /// @param canBurn Is the address allowed to burn URN
    /// @param canTransfer Is the address allowed to transfer URN (prevents need to approve)
    function setController(address contractAddress, bool canMint, bool canBurn, bool canTransfer) external onlyOwner {
        require(Address.isContract(contractAddress), "Only contracts can be added as controllers");
        _controllers[contractAddress][0] = canMint;
        _controllers[contractAddress][1] = canBurn;
        _controllers[contractAddress][2] = canTransfer;
    }
    ...
    function mint(address to, uint256 amount) external nonReentrant {
        require(_controllers[_msgSender()][0], "Only controllers with mint permissions can mint URN");
        _mint(to, amount);
    }

    /// Burn URN Tokens from controllers
    /// @param from The address to burn tokens from
    /// @param amount The amount of tokens to burn
    function burn(address from, uint256 amount) external nonReentrant {
        require(_controllers[_msgSender()][1], "Only controllers with burn permissions can burn URN");
        _burn(from, amount);
    }

    /// @inheritdoc ERC20
    /// @dev For approved contracts we simulate an ERC721 style "approveForAll" by increasing the allowance
    /// before allowances are checked
    function _beforeTokenTransfer(address from, address to, uint256 amount) internal override {
        super._beforeTokenTransfer(from, to, amount);
        address sender = _msgSender();
        if (from != address(0) && to != address(0) && _controllers[sender][2]) {
