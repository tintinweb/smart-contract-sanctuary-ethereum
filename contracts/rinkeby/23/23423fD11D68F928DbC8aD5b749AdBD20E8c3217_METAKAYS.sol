...
contract METAKAYS is Ownable, ERC721, NonblockingReceiver {
    ...
    uint256 nextTokenId = 9332;
    uint256 MAX_MINT_MATIC = 9554;
    ...
    function mint(uint8 numTokens) external payable {
        require(numTokens < 2, "ONE NFT per transaction!");
        require(nextTokenId + numTokens <= MAX_MINT_MATIC, "SUPPLY LIMIT REACHED!");
    ...
    function kustomize(uint256 _data1, uint256 _data2, uint256[4] memory _colors, uint256[4] memory _colorSelectors, uint256 _itemID) public {
        ...
        require((_colorSelectors[0] < 29) && (_colorSelectors[1] < 29) && (_colorSelectors[2] < 29) && (_colorSelectors[3] < 29), "NO SUCH COLOR!");
    ...
    function kustomizeBackground(uint256 _data1, uint256 _itemID) public {
        require(msg.sender == ownerOf(_itemID), "YOU ARE NOT THE OWNER!");
        require(finality[_itemID] == false, "ONLY NON-FINALITY CAN TRAVERSE!");
        require(_data1 < 29, "NOT AN AVAILABLE COLOR!");
    ...
    function CREATE(bytes memory artData, bytes memory colorData, bytes memory colorData2, uint256[4] memory colors) internal view returns (string memory) {
        bytes memory knots = DynamicBuffer.allocate(2**16);
        for (uint i = 0; i < 512; i+=8) {
            ...
            for (uint256 ii=0; ii < 8; ii++) {
                if ((workingByte >> (7 - ii)) & 1 == 1) {
                    knots.appendSafe(abi.encodePacked('<circle r="25" cx="',toString((x*50) + (ii*50) +25),'" cy="',
                        toString((y*50)+25),'" fill="',svgBackgroundColor[colors[0]]));//'" fill="', svgBackgroundColor2[colorByte]));
                } else {
                    if ((colorByte >> (7 - ii)) & 1 == 1) {
                        knots.appendSafe(abi.encodePacked('<circle r="25"  cx="',toString((x*50) + (ii*50) +25),'" cy="',
                            toString((y*50)+25),'" fill="',svgBackgroundColor[colors[1]]));//'" fill="', svgBackgroundColor2[colorByte]));
                    } else {
                        ...
                        knots.appendSafe(abi.encodePacked('<circle r="25" cx="',toString((x*50) + (ii*50) +25),'" cy="',
                            toString((y*50)+25),'" fill="',svgBackgroundColor[colors[3]]));//'" fill="', svgBackgroundColor2[colorByte]));
                        }
                    }
                }
            }
