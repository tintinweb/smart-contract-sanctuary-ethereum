...
contract nicBox is Ownable, Pausable,ERC1155URIStorage{
    ...
    uint256 private constant MAXMINT_NFT = 1;
    mapping(uint256 => NftToken)  public tokenURIs;
    ...
    constructor(string memory url_,bytes32 _root) ERC1155(url_) {
    ...
    function mint(address to_, uint boxID_, uint num_, bytes32[] memory proof) public  whenNotPaused returns (bool) {
    ...
    function mintBatch(address to_, uint[] memory boxIDs_, uint256[] memory nums_, bytes32[] memory proof) public  whenNotPaused returns (bool) {
          // 校验 msg.sender 是否在白名单
        require(isValid(proof, msg.sender), "Not a part of Allowlist");
    ...
    function burn(address from_, uint boxID_, uint256 num_) public whenNotPaused {
        ...
        if(boxID_<=12){
            for (uint256 i = 0; i < num_; i++) {
                //NFT的唯一id生成   盲盒boxID*1000 + 开箱数boxMap[boxID_].openNum
                uint256 mintNFTindex = boxID_*1000+ boxMap[boxID_].openNum;
                ...
                if(tokenURIs[mintNFTindex].isVaild){
                    // _setURI(mintNFTindex, strConcat (NFTbase[boxID_] , "100.json"));
    ...
    function nftURL(uint tokenID_,uint boxID_) public view returns (string memory s) {
         require(tokenURIs[tokenID_].isVaild, "box not exist");
