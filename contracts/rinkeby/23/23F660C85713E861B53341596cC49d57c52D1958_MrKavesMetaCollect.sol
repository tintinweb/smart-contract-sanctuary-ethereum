...
contract MrKavesMetaCollect is ERC721, ReentrancyGuard, Ownable, VRFConsumerBaseV2, EIP712FileSignature {
  ...
  /**
    * @dev Sets the file signature for a new token
    * @param randomWords Chainlink subscription VRF words
    */
  ...
  /**
    ...
    * @param _attributeType Type of the attribute to randomly assign
    */
  function requestRandomWordForAttribute(string memory _attributeType) private {
    ...
    requestIdForTokenId[requestId] = totalTokens();
  }
  ...
  /**
    ...
    * @param _signature Off chain web3 wallet signature approving the attached files checksums
    */
  function mint(string memory _signingName, string memory _signingMetadataHash, string memory _signingDataId, bytes calldata _signature, string[] memory _attributes) public payable nonReentrant verifyFileSignature(_signature, msg.sender, _signingName, _signingMetadataHash, _signingDataId) {
    require(saleIsActive, "Sale not active");
    require(msg.value >= MINT_PRICE, "Insufficient payment");
    require(totalTokens() < MAX_SUPPLY, "Exceeds max supply");
    require(_attributes.length <= 5, "Too many attributes");
    ...
    for (uint256 i = 0; i < _attributes.length; i++) {
      requestRandomWordForAttribute(_attributes[i]);
    }
