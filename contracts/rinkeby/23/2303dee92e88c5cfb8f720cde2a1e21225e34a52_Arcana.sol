...
contract Arcana is Ownable, Mintable, Burnable, SupportSig, SupportTokenUpdateHistory, ERC721 {
    ...
    */
    ...
    {
        _burn(tokenId);
        _tokens[tokenId] = TokenInfo(0, 0, "", 0, address(0));
        totalSupply -= 1;
        SupportTokenUpdateHistory.onTokenUpdated(tokenId);
    }

    /// @notice トークンの固有属性値を生成する
    function _generateAttributes(uint256 gene, uint256 seed) private pure returns (uint256) {
        bytes32 hash_ = keccak256(abi.encode(gene));
