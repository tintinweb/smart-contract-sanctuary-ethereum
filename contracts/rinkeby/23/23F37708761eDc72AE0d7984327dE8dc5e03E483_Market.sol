...
contract Market {
    ...
    function buyNow(uint256 _itemId) external payable {
        require(
            marketItems[_itemId].startingTimestamp <= block.timestamp &&
                marketItems[_itemId].endingTimestamp >= block.timestamp,
            "time"
        );
        require(!marketItems[_itemId].isCanceled, "canceled");
        require(!marketItems[_itemId].isSold, "sold");
        require(marketItems[_itemId].price == msg.value, "value");

        marketItems[_itemId].isSold = true;

        IERC721(marketItems[_itemId].nftContractAddress).transferFrom(
    ...
    function updateSaleDuration(
        uint256 _itemId,
        uint256 _newStartingTimestamp,
        uint256 _newEndingTimestamp
