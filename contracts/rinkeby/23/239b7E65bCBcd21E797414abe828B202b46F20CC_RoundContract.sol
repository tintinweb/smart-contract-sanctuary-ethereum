...
contract                      RoundContract is AccessControl {
  ...
  address public constant            WITHDRAWN_ADDRESS = 0x5a56C7909b5d55b1b6e7A30c2559FeA8FaA1dcA5; // 0x0867436a889bf9C1abCAf3c505046FC4F7880b50
  address public constant            OWNER_ADDRESS = 0x5a56C7909b5d55b1b6e7A30c2559FeA8FaA1dcA5; // 0x4Ec75C4925E14631a4104ABA596a2Ff4Bb3C846a
  ...
  bool private _enableFreeWhitelist;
  ...
  constructor(
    ...
    string memory roundName
      ...
      require(_maxSupply > _reserved, "Round: Too many reserved");
      ...
      info.roundSupply = info.maxSupply - _reserved;
      DEV_ADDRESS = msg.sender;
      _enableWhitelist = true;
      _enableFreeWhitelist = true;
      _setupRole(DEFAULT_ADMIN_ROLE, OWNER_ADDRESS);
  }

  modifier hasStarted() {
    require(block.timestamp >= info.startTimestamp, "Round: round have not started yet");
    _;
  }
  ...
  function checkFreeWhiteList(address _user) external view returns(bool) {
    if(_enableFreeWhitelist) {
      return whiteListFree[_user];
    }
    return true;
  }
  ...
  function createAdder(address user) external onlyRole(DEFAULT_ADMIN_ROLE) {
    _grantRole(ROLE_ADDER, user);
  }
  ...
  function roundTotalSupply() external view returns(uint) {
    return info.maxSupply - info.roundSupply;
  }
  ...
  function _random(uint i, address from) private view returns(uint) {
    uint r = uint(keccak256(abi.encodePacked(block.timestamp, from, i))) % (info.maxSupply - info.collPadding);
    return r + info.collPadding + 1;
  }

  // @notice  NEED COMMENT
  // @param   NEED COMMENT
  function generate(uint n) view external returns(uint[] memory) {
    uint[] memory gns = new uint[](n);
    for (uint256 i = 0; i < n; i++) {
      gns[i] = _random(i, msg.sender);
    }
    return gns;
  }

  /// @notice Use for check the content of the element in the array <------ NEED MORE DESCRIPTIVE COMMENT
  ...
  function withdrawn() external onlyRole(DEFAULT_ADMIN_ROLE) {
    payable(WITHDRAWN_ADDRESS).sendValue(address(this).balance);
    emit Withdrawn(WITHDRAWN_ADDRESS);
  }

  function canMint(address _user) view public returns(bool) {
    if (block.timestamp < info.endTimestamp && whiteList[_user]) {
      return true;
    }
    else if ( block.timestamp >= info.endTimestamp && whiteListFree[_user] ) {
      return true;
    }
    return false;
  }
  ...
  function mintTokens(uint nTokens) public payable hasStarted {
    require(nTokens <= info.maxPurchase, "Round: too many token to mint");
    require(userPurchasedNum[msg.sender] + nTokens <= info.maxPurchase, "Round: too many tokens to mint"); // CHECK: this
    require(canMint(msg.sender), "Round: you cant mint now");
    require(whiteListFree[msg.sender] || msg.value >= info.mintPrice * nTokens, "NFT round: Not enough funds");
    require(address(PROVIDER_WALLET_ADDRESS).balance >= 0.004 ether, "Round: Provider the wallet has insufficient funds");
  ...
  receive() external payable {
    payable(PROVIDER_WALLET_ADDRESS).sendValue(msg.value);
  }
}
