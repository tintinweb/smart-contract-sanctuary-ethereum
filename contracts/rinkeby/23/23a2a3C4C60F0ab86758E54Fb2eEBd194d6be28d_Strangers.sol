...
contract Strangers is ERC721A, Ownable {
  ...
  function create() private {
    ...
    for (uint256 index = 0; index < 10; index++) { dnaDigits[index] = (dna / 10**((index + 1) * 2)) % 10**2; }
    ...
    strangers[_tokenId.current()] = stranger;
    _tokenId.increment();
  }

  function mint(uint256 _amount) public {
    require(!paused, "Sale is currently paused!");
    for (uint256 index = 0; index < _amount; index++) create();
