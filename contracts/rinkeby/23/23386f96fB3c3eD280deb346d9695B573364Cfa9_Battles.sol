...
interface IIngameItems {
    function addTotemToPlayer(address) external;
    function addGemToPlayer(address) external;
    function addGhostToPlayer(address) external;
}

contract Battles is Ownable, ReentrancyGuard {
    using EnumerableSet for EnumerableSet.AddressSet;
    using EnumerableSet for EnumerableSet.UintSet;
    using Counters for Counters.Counter;
    Counters.Counter private counter;
    uint256 constant public DECIMALS_18 = 1000000000000000000;
    IERC20 public paymentToken;
    IERC721 public piratesContract;
    IERC1155 public landContract;
    ILandStaker public landStakingContract;
    IRandomizer public randomizer;
    IIngameItems ingameItems;
    uint256 stakingCostInEtherUnits;
    ...
    struct Battle {
        ...
        mapping(address => EnumerableSet.UintSet) piratesByPlayer; // change to EnumerableSet?
    ...
    function addBattle(uint256 _startTime, uint256 _endTime, uint256 _rewardsInEtherUnits, uint256 _numXBmfPrizes, uint256 _numGemPrizes,
        ...
        emit BattleAdded(counter.current()); // emit before increment counter
    ...
    function hasLand(address _address) internal returns (bool){
        return landContract.balanceOf(msg.sender, 0) > 0 ||
            landContract.balanceOf(msg.sender, 1) > 0 ||
            landContract.balanceOf(msg.sender, 2) > 0 ||
            landContract.balanceOf(msg.sender, 3) > 0 ||
            landContract.balanceOf(msg.sender, 4) > 0 ||
            landContract.balanceOf(msg.sender, 5) > 0 ||
            landContract.balanceOf(msg.sender, 6) > 0 ||
            landContract.balanceOf(msg.sender, 7) > 0 ||
            landStakingContract.getStakedBalance(msg.sender, 0) > 0 ||
            landStakingContract.getStakedBalance(msg.sender, 1) > 0 ||
            landStakingContract.getStakedBalance(msg.sender, 2) > 0 ||
            landStakingContract.getStakedBalance(msg.sender, 3) > 0 ||
            landStakingContract.getStakedBalance(msg.sender, 4) > 0 ||
            landStakingContract.getStakedBalance(msg.sender, 5) > 0 ||
            landStakingContract.getStakedBalance(msg.sender, 6) > 0 ||
            landStakingContract.getStakedBalance(msg.sender, 7) > 0;
    }

    function addPirateToBattleWithXBMF(uint256 battleId, uint256 pirateId, uint256 paymentAmountInEthUnits) external {
        require(
        ...
        addPirateToBattle(battleId, pirateId);
    }

    function addPirateToBattleWithLand(uint256 battleId, uint256 pirateId) external {
        require(
    ...
    function endBattle(uint256 battleId) external onlyOwner {
        Battle storage battle = battles[battleId];
        battle.ended = true;
        _pickXbmfWinners(battleId);
        _pickGemWinners(battleId);
        _pickTotemWinners(battleId);
        _pickGhostWinners(battleId);
    }
    ...
    function isXbmfWinner(uint256 battleId, address _address) external returns (bool) {
        bool winner = false;
        for (uint256 i = 0; i < battles[battleId].xbmfWinners.length(); i++) {
            if (battles[battleId].xbmfWinners.at(i) == msg.sender){
                winner = true;
    ...
    function getGemWinners(uint256 battleId) public view returns (address[] memory){
        ...
        console.log("gem winners length", length);
    ...
    function _pickXbmfWinners(uint256 battleId) internal {
        Battle storage battle = battles[battleId];
        uint256 count = 0;
        uint256 nonce = 0;
        uint256 seed = 0;
        while (count < battle.numXbmfPrizes){
            uint256 r = randomizer.randomMod(seed, nonce, battle.numXbmfPrizes);//TODO: change to battle.players.length
            address candidate = battle.players.at(r);
    ...
    function _pickGemWinners(uint256 battleId) internal {
        Battle storage battle = battles[battleId];
        uint256 count = 0;
        uint256 nonce = 1000;
        uint256 seed = 1000;
        while (count < battle.numGemPrizes){
            uint256 r = randomizer.randomMod(seed, nonce, battle.players.length());
            address winner = battle.players.at(r);
            console.log("gem winner", winner);
            ingameItems.addGemToPlayer(winner);
            battle.gemWinners.add(winner);
            count++;
            nonce++;
        }
        //console.log("Gem winners", ...);
    }

    function _pickTotemWinners(uint256 battleId) internal {
        Battle storage battle = battles[battleId];
        uint256 count = 0;
        uint256 nonce = 2000;
        uint256 seed = 2000;
        while (count < battle.numTotemPrizes){
            uint256 r = randomizer.randomMod(seed, nonce, battle.players.length());
            address winner = battle.players.at(r);
            ingameItems.addTotemToPlayer(winner);
            console.log("TOTEM: count, winner",count, winner);
            battle.totemWinners.add(winner);
            count++;
            nonce++;
        }
        //console.log("Gem winners", ...);
    ...
    function _pickGhostWinners(uint256 battleId) internal {
        Battle storage battle = battles[battleId];
        uint256 count = 0;
        uint256 nonce = 3000;
        uint256 seed = 3000;
        while (count < battle.numGhostPrizes){
            uint256 r = randomizer.randomMod(seed, nonce, battle.players.length());
            address winner = battle.players.at(r);
            ingameItems.addGhostToPlayer(winner);
            battle.ghostWinners.add(winner);
            count++;
            nonce++;
        }
    }
    ...
    function getAllStakedPiratesCount(uint256 battleId) view public returns (uint256) {
        return getAllStakedPiratesForBattle(battleId).length;
    }
    ...
    // Add logic to win land

    // ad logic for tickets that increase chances to win a gem if holds land
