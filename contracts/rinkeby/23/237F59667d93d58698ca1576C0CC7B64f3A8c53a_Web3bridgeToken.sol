...
contract Web3bridgeToken is ERC721A, AccessControl {
    ...
    mapping(address => uint40) communityMintCount;
    // map an id to whitelistAddress
	mapping(uint256 => address[]) whitelistAddress;
    // map investorAddress to amountMint
	mapping(address => uint256) investorsMint;
    // map investorAddress to minted or not
	mapping(address => bool) investorsWhitelistClaimed;
    // metadata prifix
    ...
    // investor cost for minting 1 NFT;
    uint128 public costInvestors = 200 ether;
     // Whitelist cost for minting 1 NFT;
    uint128 public costWhitelist = 300 ether;
    // publicSale cost for minting 1 NFT;
    uint128 public costPublicSale = 500 ether;
    ...
    constructor(address _usdc,address _newAdmin, string memory _hiddenMetadataUri, string memory _uriPrefix) ERC721A("Web3bridge", "Web3bridge") {
        usdc = _usdc;
    ...
    event whiteListUser(uint, address);
    event investors(uint,address);
    event pubMunt (uint, address);
     ...
     */
    function InvestorWhiteList() external payable returns(bool status) {
	...
	require(investorsWhitelistClaimed[msg.sender] == false, "minted already" );
    	require(!pause,"Is Pause");
    	require (!whitelistMintEnabled,"not public period");
     ...
     * @return status value of true if all checks are true
     */
    function WhitlistMint(uint40 quantity) external payable returns(bool status){
        require(!pause, "Is Pause");
        supply = totalSupply();
		 uint256 amount = costWhitelist * quantity;
        require(supply + quantity <= TOTAL_COLLECTION_SUPPLY, "max exceded");
        require(quantity <= whitelistAmount, "exceed amount");
		require(checkWhitelist(msg.sender,2),"not whitelisted");
        require(!whitelistMintEnabled, "not whitelist period");
        require(whitelistClaimed[msg.sender] == false, "minted already");
		 require(communityMintCount[msg.sender] + quantity <= whitelistAmount , "Max mint");
        require(
    ...
    function publicMint(uint256 quantity) external payable returns (bool status){
        supply = totalSupply();
        require(quantity > 0, "mint 1");
        require(!pause, "Is Pause");
        require(whitelistMintEnabled == false, "not public period");
        require(supply + quantity <= TOTAL_COLLECTION_SUPPLY, "max exceded");
        require(IERC20(usdc).transferFrom(msg.sender, address(this), costPublicSale * quantity),"must pay cost");
            _mint(msg.sender, quantity);
     ...
     */
    function setRevealed(bool _state) external onlyRole(MODERATOR_FUNCTION_ROLE) {
        revealed = _state;
    }
     ...
     */
    function setCostWhitelist(uint128 _cost) external onlyRole(MODERATOR_FUNCTION_ROLE) {
        costWhitelist = _cost;
    }
     /**
     ...
     */
    function setcostPublicSale(uint128 _cost) external onlyRole(MODERATOR_FUNCTION_ROLE) {
        costPublicSale = _cost;
    }

    // function setMaxMintAmountPerTx(uint256 _maxMintAmountPerAddr) public onlyRole(MODERATOR_FUNCTION_ROLE) {
     ...
     */
    function setUriPrefix(string memory _uriPrefix) external onlyRole(MODERATOR_FUNCTION_ROLE) {
        uriPrefix = _uriPrefix;
    }
      /**
     ...
     */
    function setUriSuffix(string memory _uriSuffix) external onlyRole(MODERATOR_FUNCTION_ROLE) {
     ...
     */
    function setToken(address _usdc) external onlyRole(MODERATOR_FUNCTION_ROLE) {
        usdc = _usdc;
    }
    /*
     * @notice Toggles the paused state flag to false or true
     ...
     */
    function setWhitelistMintEnabled(bool _state) external onlyRole(MODERATOR_FUNCTION_ROLE) {
     ...
     */
	function investorsWhitelistUsers(address[] calldata _investors, uint256[] calldata  _amount) external onlyRole(MODERATOR_FUNCTION_ROLE) {
		require(_investors.length == _amount.length,"incorrect length matching");
