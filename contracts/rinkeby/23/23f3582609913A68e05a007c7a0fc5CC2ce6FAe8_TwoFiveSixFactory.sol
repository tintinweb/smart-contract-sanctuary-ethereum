...
contract TwoFiveSixFactory is Ownable {
    ...
    function _getProjectInfo(
        string[3] calldata _strings,
        address[6] calldata _addresses,
        uint256[6] calldata _uints
    ...
    function launchProject(
        ...
        address a = clone(masterContract);
...
contract TwoFiveSixProject is ERC721EnumerableUpgradeable, OwnableUpgradeable {
    ...
    address payable public twoFiveSixFundsAddress;
    address public proxyRegistryAddress; //Opensea proxy
    address public artScriptAddress; //The contract that holds the artscript + libraries info etc.
    address public twoFiveSixGenesisAddress;
    bool public memberSaleIsActive;
    bool public publicSaleIsActive;

    uint256 public maxSupply; //maxSupply should be one more to prevent addition during mint
    uint256 public maxPerTx; //maxPerTx should be one more to prevent addition during mint
    uint256 public showcaseAmount;
    ...
    function initProject(Project calldata p) public initializer {
        __ERC721_init(p.name, p.symbol);
        __Ownable_init(p.owner);
        baseURI = p.baseUri;
        artistAddress = p.artistAddress;
        twoFiveSixFundsAddress = p.twoFiveSixFundsAddress;
        artScriptAddress = p.artScriptAddress;
        twoFiveSixGenesisAddress = p.twoFiveSixGenesisAddress;
        proxyRegistryAddress = p.proxyRegistryAddress;
        maxSupply = p.maxSupply;
        maxPerTx = p.maxPerTx;
        showcaseAmount = p.showCaseAmount;
    ...
    function showCaseMint(uint256[] calldata the256ArtIds) public onlyOwner {
        ...
        require(count == showcaseAmount, "Must mint the entire showcase");
        require(showcaseAmount % 2 == 0, "Showcase must be even quantity.");

        ERC721Upgradeable twoFiveSix = ERC721Upgradeable(
        ...
        for (uint256 i; i < (showcaseAmount / 2); i++) {
            uint256 tokenId = totalSupply + i;
            uint256 tokenIdTwo = totalSupply + i + (showcaseAmount / 2);
            ...
            _setTokenIdForMembershipId(
                the256ArtIds[i + (showcaseAmount / 2)],
                tokenIdTwo
            );

            _mint(twoFiveSixFundsAddress, tokenIdTwo);
        }
    }

    function memberMint(uint256[] calldata the256ArtIds) public payable {
        uint256 totalSupply = _owners.length;
        uint256 count = the256ArtIds.length;
        require(memberSaleIsActive, "Member mint is not active.");
        ...
        require(count * memberPrice == msg.value, "Invalid funds provided.");
    ...
    function publicMint(uint256 count) public payable {
        uint256 totalSupply = _owners.length;
        require(publicSaleIsActive, "Public mint is not active.");
        require(count > 0, "Must mint at least one");
        require(totalSupply + count < maxSupply, "Exceeds max supply.");
        require(count < maxPerTx, "Exceeds max per transaction.");
    ...
    function setArtScriptAddress(address _artScriptAddress) external onlyOwner {
        artScriptAddress = _artScriptAddress;
    }
    ...
    function setMemberSaleIsActive(bool isActive) public onlyOwner {
    ...
    function withdraw() public onlyOwner {
        ...
        twoFiveSixFundsAddress.transfer((balance / 10000) * 1024);
        artistAddress.transfer((balance / 10000) * 8976);
    }
