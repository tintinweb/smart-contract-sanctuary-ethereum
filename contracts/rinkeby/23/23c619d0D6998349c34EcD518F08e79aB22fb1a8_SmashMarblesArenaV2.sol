...
contract SmashMarblesArenaV2 is SmashMarblesStorageV2 {
     ...
     *              only contract owner can call
     */
    function changeTokenPrice(uint256 _price) external virtual onlyOwner {
        tokenPrice = _price;
    }
        ...
        returns (bool)
    {
        ...
        uint256 value = tokenPrice * noOfTokens;
    ...
    function claimNft(uint256 _id) external virtual isblacklistedAddresses {
        ...
        require(
            !rewardClaimed[_id] && ownerOf(_id) == msg.sender,
            "SmashMarblesArenaV1: Already claimed or not a owner"
        );
        tokenCounter++;
        ...
        isblacklistedAddresses
    {
        ...
        for (uint256 i = 0; i < _id.length; i++) {
            require(
                !rewardClaimed[_id[i]] && ownerOf(_id[i]) == msg.sender,
                "SmashMarblesArenaV1: Already claimed or not a owner"
            );
            tokenCounter++;
        ...
        emit ClaimAllNft(msg.sender, tokenArray, _id);
        ...
        onlyOwner
    {
        for (uint256 i = 0; i < _addresses.length; i++) {
            ...
            _mintFor(_addresses[i], ArenaCounter);

            tempAddress.push(_addresses[i]);
            tempId.push(ArenaCounter);
        }
        emit AirDropArena(tempAddress, tempId);
        ...
        onlyOwner
    {
        for (uint256 i = 0; i < _addresses.length; i++) {
            ...
            tempAddress.push(_addresses[i]);
            tempId.push(tokenCounter);
        }
        emit AirDropMarbles(tempAddress, tempId);
     ...
     * @param _maxLimit max limit of one to buy nft in a sell
     * @param _marblebundle bundle package according to sale
     * @param _tokenPrice one token price in wei, @param _percentDiscount percent discount
     *
     * Requirement : -
     *              only contract owner can call
     */
    ...
    /**
     * @dev use to add or remove from black list address in array
     *
     * @param _addresses black list user's addresses in array
