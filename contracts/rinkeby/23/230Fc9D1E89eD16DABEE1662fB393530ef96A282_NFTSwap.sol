...
contract NFTSwap is Ownable, ReentrancyGuard {
    ...
    MarketProxyManager marketProxy;

    constructor(address marketProxy_) {
        marketProxy = MarketProxyManager(marketProxy_);
    }

    function batchBuy(
        MarketProxyManager.TradeDetail[] memory tradeDetails_,
        ERC20Detail[] memory erc20Details
    ) external payable nonReentrant {
        // transfer ERC20 tokens from the sender to this contract before trade
        for (uint256 i = 0; i < erc20Details.length; i++) {
            IERC20(erc20Details[i].tokenAddress).safeTransferFrom(
                _msgSender(),
                address(this),
        ...
        for (uint256 i = 0; i < tradeDetails_.length; i++) {
            // get market details
            (address _proxy, bool _isLib, bool _isActive) = marketProxy.markets(
                tradeDetails_[i].marketId
            );
            // market should be active
            require(_isActive, "batchBuy: InActive Market");
            // execute trade
            (bool success, ) = _isLib
                ? _proxy.delegatecall(tradeDetails_[i].tradeData)
                : _proxy.call{value: tradeDetails_[i].value}(
                    tradeDetails_[i].tradeData
                );
    ...
    function setMarketProxy(MarketProxyManager _marketProxy) external onlyOwner {
