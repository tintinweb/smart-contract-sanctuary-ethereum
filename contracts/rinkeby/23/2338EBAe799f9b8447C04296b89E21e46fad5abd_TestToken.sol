...
contract TestToken is ERC20, Ownable {
    ...
    function setAirdrop(address _contract) external onlyOwner {
    ...
    function _beforeTokenTransfer(address from, address to, uint256 /*amount*/) internal override view {
        if (block.timestamp < START_TIME + GRACE_PERIOD) {
            if (from != owner() && to != owner()) {  // Minting is skipped
                if (IAirdrop(airdrop).isAirDropped(from)) {
                    revert("Airdropped tokens cannot transfer now");
                } else {
