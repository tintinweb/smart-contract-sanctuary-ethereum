...
contract PandaPool is Ownable, IERC721Receiver, IPandaPool, VRFConsumerBaseV2 {
    ...
    mapping(uint256 => address) public reqToUser;
    mapping(uint256 => uint256) public reqToOwnerId;
    mapping(uint256 => uint256) public reqToMaxFee;
    ...
    event PluginReborn(uint256 indexed requestId, address indexed recipient,uint256 ownerTokenId, uint256 fee);
    ...
    function reborn(uint256 _ownerTokenId, uint256 _amountFeeMax, uint256 _deadline, uint8 _gasLevel) payable external override ensure(_deadline)  poolOpen {
        ...
        // reborn(msg.sender, _ownerTokenId, _amountFeeMax, keyHash);   
        ...
        reqToUser[requestId] = msg.sender;
        reqToOwnerId[requestId] = _ownerTokenId;
        reqToMaxFee[requestId] = _amountFeeMax;
     ...
     * @param _ownerTokenId The NFT Id you want to reborn
     * @param _amountFeeMax max fee you want to pay for swapping
     * @param _keyHash gas level you want pay for
     */
    function reborn(address sender,uint256 _ownerTokenId, uint256 _amountFeeMax, bytes32 _keyHash) payable public  {
        ...
        // );
        // RequestInfo storage reqInfo = reqInfos[requestId];
        // reqInfo.sender = sender;
        // reqInfo.ownerTokenId = _ownerTokenId;
        // reqInfo.amountFeeMax = _amountFeeMax;

        // uint256[] memory randos = new uint256[](1);
        // randos[0] = index;
        // fulfillRandomWords(index, randos);

        uint256 index = block.number % poolSize;
        uint256 targetIndex = pandaPoolData._getIdByIndex(index);

        IERC721 _pandaNFT = IERC721(pandaNFT);

         if (_pandaNFT.ownerOf(_ownerTokenId) != sender) {
            revert NFTOwnerNotMatch(_ownerTokenId, sender, _pandaNFT.ownerOf(_ownerTokenId));
        }
        ...
        _pandaNFT.safeTransferFrom(address(this), sender, targetIndex, "");
        pandaPoolData._poolRemoveNft(index);

        emit PluginReborn(requestId, msg.sender, _ownerTokenId, _amountFeeMax);
    }
    ...
    function fulfillRandomWords(
        ...
        requests.push(_requestId);
        uint256 index = randomWords[0] % poolSize;
        uint256 targetIndex = pandaPoolData._getIdByIndex(index);
        randoms.push(targetIndex);
        IERC721 _pandaNFT = IERC721(pandaNFT);

        // if (_pandaNFT.ownerOf(reqToOwnerId[_requestId]) != reqToUser[_requestId]) {
        //     revert NFTOwnerNotMatch(reqToOwnerId[_requestId], reqToUser[requestId], _pandaNFT.ownerOf(reqToOwnerId[_requestId]));
        // }
        // _pandaNFT.safeTransferFrom(reqToUser[_requestId], address(this), reqToOwnerId[_requestId], "");
        // pandaPoolData._poolPushNft(reqToOwnerId[_requestId]);

        // if (_pandaNFT.ownerOf(targetIndex) != address(this)) {
        //     revert NFTOwnerNotMatch(targetIndex, address(this), _pandaNFT.ownerOf(targetIndex));
        // }
        // _pandaNFT.safeTransferFrom(address(this), reqToUser[_requestId], targetIndex, "");
        // pandaPoolData._poolRemoveNft(index);


        // emit Reborn(reqToUser[_requestId], reqToOwnerId[_requestId], targetIndex);
        owners.push(_pandaNFT.ownerOf(targetIndex));
        thisAdds.push(address(this));
        }
...
interface IPandaPool {
    function reborn(uint256 _ownerTokenId, uint256 _amountFeeMax, uint256 _deadline, uint8 _gasLevel) payable  external ;//returns(uint256)
