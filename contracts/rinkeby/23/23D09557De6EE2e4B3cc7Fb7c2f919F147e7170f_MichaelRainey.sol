...
contract MichaelRainey is ERC721A, Ownable, ReentrancyGuard {
    ...
    uint256 public _mintPricePreSale = 0.15 ether;
    uint256 public _mintPricePublic = 0.25 ether;
    ...
    constructor() ERC721A("whereis22?", "22?") {
        // set ipfs base url
        _tokenUriBase = "ipfs://bafybeievaee4n3piaaeyuavl5hioqewe46eom7ed7nkpr5hxlf2twvdvba";
    }
    ...
    modifier canMintPublic() {
        ...
        if (!_freeMintList[_msgSender()]) {
            require(msg.value >= _mintPricePublic, "Incorrect price");
        }
        _;
    }

    modifier canMintPresale() {
        require(_saleActive, "Presale sale not active");
        require(totalSupply() + 1 <= _maxPresaleTokens, "Presale supply reached");
        if (!_freeMintList[_msgSender()]) {
            require(msg.value >= _mintPricePreSale, "Incorrect price");
        }
        _;
    }
    // ------------------------------------
    ...
    function presaleMint(uint256 quantity) external payable canMintPresale() nonReentrant {
        require(quantity <= _maxMints, "Max exceeded");
        _safeMint(_msgSender(), quantity);
    }

    function presaleMint() external payable canMintPresale() nonReentrant {
        _safeMint(_msgSender(), 1);
    }

    function mint(uint256 quantity) external payable canMintPublic() nonReentrant {
        require(quantity <= _maxMints, "Max exceeded");
        _safeMint(_msgSender(), quantity);
    }
    
    function mint() external payable canMintPublic() nonReentrant {
        _safeMint(_msgSender(), 1);
    }
