...
contract MetaTopeAuctionV2 is MetaTopeIERC1155Receiver, Ownable {
    ...
    address[] public sortableAddresses;
    address public withdrawAddress;

    bool public started;
    bool public isRestartable;
     ...
     * @param _isRestartable can the auction be restarted
     */
    constructor(
    ...
    function start(
        ...
        if (!isRestartable) require(endAt == 0, "Auction cannot be restarted");
        require(
            _totalTokenAmount > 0,
    ...
    /**
     * @dev Function to sort bids from highest pricePerToken to lowest
     */
    function createSortableBids(uint256 _batchSize) external onlyOwner onlyEnded {
        require(bidderAddresses.length > 0, "Bids should be greater than 0");
        require(!isSortable, "Already created sortable bids");
        require(_batchSize >= 1, "Batch size should be greater than 0");

        uint256 range = sortedIndex + _batchSize;
        for (sortedIndex; sortedIndex < range; ++sortedIndex) {
            if (sortedIndex >= bidderAddresses.length) {
                isSortable = true;
                sortedIndex = 0;
                break;
            }

            if (bidders[bidderAddresses[sortedIndex]].canceled == true) {
                continue;
            }

            WinAuctionBid memory wonBid = WinAuctionBid({
            tokenAmount: bidders[bidderAddresses[sortedIndex]].tokenAmount,
            pricePerToken: bidders[bidderAddresses[sortedIndex]].pricePerToken,
            totalPrice: bidders[bidderAddresses[sortedIndex]].totalPrice,
            claimed: bidders[bidderAddresses[sortedIndex]].claimed,
            bidderAddress: bidderAddresses[sortedIndex]
            });

            sortedBids.push(wonBid);
            sortableAddresses.push(bidderAddresses[sortedIndex]);
        }
    }
     ...
     * @dev Sort bids
     */
    function sortBids(uint256 _batchSize, uint256 _subBatchSize) external onlyOwner onlyEnded {
        require(isSortable, "Sortable bids should be created");
        require(!bidsSorted, "Bids are already sorted");
        require(_batchSize >= 1, "BatchSize should be greater than 0");

        uint256 range = sortedIndex + _batchSize;
        uint256 operations = 0;
        for (sortedIndex; sortedIndex < range; ++sortedIndex) {
            if (sortedIndex >= sortableAddresses.length) {
                bidsSorted = true;
                break;
            }

            unchecked {
                WinAuctionBid memory cBid;
                uint256 j;

                if (!tmpSubSortInterrupted) {
                    cBid = sortedBids[sortedIndex];
                    j = sortedIndex;
                } else {
                    cBid = tmpSubSortBid;
                    j = tmpSubSortIndex;
                ...
                while ((int(j) > 0) && (sortedBids[j - 1].pricePerToken < cBid.pricePerToken)) {
                    sortedBids[j] = sortedBids[j - 1];
                    j--;
                    operations++;

                    if (operations > _subBatchSize) {
                        tmpSubSortBid = cBid;
                        tmpSubSortIndex = j;
                        tmpSubSortInterrupted = true;
                        return;
                    }
                }

                sortedBids[j] = cBid;
                tmpSubSortInterrupted = false;
            }
        }
    }
