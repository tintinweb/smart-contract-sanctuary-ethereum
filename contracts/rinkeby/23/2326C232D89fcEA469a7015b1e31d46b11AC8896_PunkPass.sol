...
contract PunkPass is Delegated, ERC721EnumerableLite, PaymentSplitterMod {
    using Strings for uint;

    // General NFT Variables
    uint public maxOrder = 1; // maximum order per transaction during main mint phase
    uint public maxPerWallet = 1; // cannot mint to a wallet holding 1 or more tokens
    ...
    // Public
    /*
     * @dev Public mint function - gate token and merkle proof must be provided to mint.
     */
    function mint(bytes32 gateToken_, bytes32[] memory proof_) external payable onlySender {
        require(isMintActive, "Sale is not active!");
        require(block.timestamp < endTime, "Sale is over!");
        require(walletOfOwner(msg.sender).length < maxPerWallet, "Wallet already minted!");
        require(isAllowlisted(gateToken_, proof_), "Mint not allowed!");
        require(bytesToGateTokenUsed[gateToken_] != true, "Token already minted!");
  
        bytesToGateTokenUsed[gateToken_] = true;
     ...
     */
    function setActive(bool isMintActive_) external onlyDelegates {
        require(isMintActive != isMintActive_, "New value matches old");
     ...
     * @dev Activate/deactivate whitelisted presale mint, main mint, and premium mint. Can only be called by delegates.
     */
    function setMintEnd(uint endTime_) external onlyDelegates {
        require(endTime != endTime_, "New value matches old");
        endTime = endTime_;
    }
     ...
     */
    function setMax(uint maxOrder_) external onlyDelegates {
        require(maxOrder != maxOrder_, "New value matches old");
        maxOrder = maxOrder_;
    }
    ...
    // Bot Control
    mapping(bytes32 => bool) public bytesToGateTokenUsed; // stores whether or not token has been used
    bytes32 public merkleRoot = 0x36a4d1e085467861fe139d829055feb82d28c7f608d420ee87534afd42d74849;

    /**
     * @dev Set the gate control merkle root.
     */
    ...
    /**
     * @dev Checks merkle proof to determine if gate token is legit.
     */
    function isAllowlisted(bytes32 gateToken_, bytes32[] memory proof_) public view returns (bool) {
        bytes32 _leaf = keccak256(abi.encodePacked(gateToken_));
