...
contract YourStory is WordTable, AddressShifter, ERC721Royalty, Ownable {
    ...
    constructor(address _owner, uint96 feeNumerator)
        ERC721("Your Story", "STORY")
    {
        _transferOwnership(_owner);
        _setDefaultRoyalty(_owner, feeNumerator);
    }
    ...
    function mintTo(address recipient) public payable returns (uint256) {
        ...
        if (newMintCount == 1) {
            tokenIdToMintSeed[newTokenId] = bytes20(msg.sender);
        } else {
            tokenIdToMintSeed[newTokenId] = shiftBytes20Address(
                bytes20(msg.sender),
                uint8(newMintCount - 1)
            );
        }
    ...
    function seed(uint256 tokenId) public view returns (address) {
        ...
        return address(tokenIdToMintSeed[tokenId]);
    }
    ...
    function getSentance(uint256 tokenId) public view returns (string memory) {
        ...
        string[40] memory sentance = seedToSentance(mintSeed);
        ...
        returns (string memory)
    {
        ...
        string[2] memory colors = seedToColorStringsOp(mintSeed);
        string[40] memory sentance = seedToSentance(mintSeed);
...
contract AddressShifter is WordTable {
    using strings for *;

    bytes16 public constant _HEX_SYMBOLS = "0123456789abcdef";

    function nibbleToHexString(uint256 value)
        public
        pure
        returns (string memory)
    {
    ...
    function addressToHexDigits(address addr, uint8 offset)
        public
        pure
        returns (uint8[40] memory addrHexDigits)
    {
        bytes20 addrBytes = bytes20(addr);
        for (uint8 i = 0; i < 20; i++) {
            uint8[2] memory nibbles = byteToNibbles(addrBytes[i]);
            addrHexDigits[i * 2] = (nibbles[0] + offset) % uint8(16);
            addrHexDigits[i * 2 + 1] = (nibbles[1] + offset) % uint8(16);
        }
        return addrHexDigits;
    }

    function shiftBytes20Address(bytes20 addrBytes, uint8 offset)
        public
        pure
        returns (bytes20 addrBytesShifted)
    {
        ...
        for (uint8 i = 0; i < 20; i++) {
            ...
            buffer[i] = nibblesToBytes(
                [
                    (nibbles[0] + offset) % uint8(16),
                    (nibbles[1] + offset) % uint8(16)
                ]
            );
        }
    ...
    function seedToColorStrings(bytes20 seed)
        public
        pure
        returns (string[2] memory color)
    {
        bytes memory bg = new bytes(3);
        bytes memory fnt = new bytes(3);
        ...
        return [
            Strings.toHexString(uint24(bytes3(bg))),
            Strings.toHexString(uint24(bytes3(fnt)))
        ];
    }

    function seedToColorStringsOp(bytes20 seed)
        public
        pure
        returns (string[2] memory color)
    {
        ...
        uint24 bgUint = uint24(bytes3(bg));
        bytes memory bgBuffer = new bytes(7);
        for (uint256 i = 6; i > 0; --i) {
            bgBuffer[i] = _HEX_SYMBOLS[bgUint & 0xf];
            bgUint >>= 4;
        }

        uint24 fntUint = uint24(bytes3(fnt));
        bytes memory fntBuffer = new bytes(7);
        for (uint256 i = 6; i > 0; --i) {
            fntBuffer[i] = _HEX_SYMBOLS[fntUint & 0xf];
            fntUint >>= 4;
        }
        return [string(bgBuffer), string(fntBuffer)];
    }
        ...
        returns (string[40] memory sentance)
    {
        for (uint8 i = 0; i < 20; i++) {
            ...
            sentance[i * 2] = WORDS[(nibbles[0])][i * 2];
            sentance[i * 2 + 1] = WordTable.WORDS[(nibbles[1])][i * 2 + 1];
        }
        return sentance;
    }

    function seedToSentance(bytes20 seed, uint8 offset)
        public
        view
        returns (string[40] memory sentance)
    {
        for (uint8 i = 0; i < 20; i++) {
            uint8[2] memory nibbles = byteToNibbles(seed[i]);
            sentance[i * 2] = WORDS[(nibbles[0] + offset) % uint8(16)][i * 2];
            sentance[i * 2 + 1] = WordTable.WORDS[
                (nibbles[1] + offset) % uint8(16)
            ][i * 2 + 1];
        }
        return sentance;
