...
contract Loan {
  ...
  function init(uint _amount) public payable {
  ...
  function loanEther(uint256 _amount, uint256 _duration) public {
    ...
    newLoan.collateralAmount = collateralAmount(_amount);
    ...
    IERC20(tokenAddress).approve(address(this), newLoan.collateralAmount);
  ...
  function withdraw(uint256 _id) public {
    ...
    if(block.timestamp > req.timeCanGetInterest) {
      ...
      if(req.isLendEther) {
        ...
        IERC20(tokenAddress).approve(address(this), req.paybackAmountToken);
      ...
      if(req.isLendEther) {
        ...
        // transfer token to lender
        IERC20(tokenAddress).approve(address(this), req.lendAmountToken);
