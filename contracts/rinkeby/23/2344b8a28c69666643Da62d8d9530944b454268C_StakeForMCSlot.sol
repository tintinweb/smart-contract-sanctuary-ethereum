...
contract StakeForMCSlot is Ownable {
    ...
    */
     function _withdrawStake(uint256 index) internal returns(uint256){
        ...
        stakeconfigs[stakeconfigs.length-1].totalStaked-=current_stake.amount;
        stakeconfigs[stakeconfigs.length-1].totalReward-=current_stake.reward;
        delete stakeholders[user_index].address_stakes[index];
        emit Unstaked(msg.sender,current_stake.amount,current_stake.reward,index,block.timestamp);       
        return total;
     }
    ...
    *
    ...
    function stakeForSlot(uint256 quantity) public  {
        ...
        // push a newly created Stake with the current block timestamp.
        // May have overflow problem?
        stakeholders[index].address_stakes.push(Stake(msg.sender, currentConfig.tokenAddress,_amount, timestamp,currentConfig.timePeriod, _amount*currentConfig.rewardPerMillion/1000000,quantity));
        // Emit an event that the stake has occured
        stakeconfigs[stakeconfigs.length-1].totalStaked+=_amount;
        stakeconfigs[stakeconfigs.length-1].totalReward+=_amount*currentConfig.rewardPerMillion/1000000;
