...
//this contract has not been tested or audited please dont use
...
contract WL_AND_PUB_MINT_ERC721A is ERC721A, IERC2981, Ownable, ReentrancyGuard {
  using Strings for uint256;

  string public PROVENANCE_HASH;

  uint256 constant MAX_SUPPLY = 50;
  uint256 constant MAX_LISTED = 30; 
  uint256 public maxWhitelistAmount = 2; 
  uint256 public maxPublicAmount = 3;
  ...
  ) ERC721A("WL_AND_PUB_MINT_ERC721A", "TAGEX") {
  ...
  function setMaxWhitelistAmount(uint256 _maxWhitelistAmount) public onlyOwner {
    ...
    function setMaxPublicAmount(uint256 _maxPublicAmount) public onlyOwner {
    maxPublicAmount = _maxPublicAmount;
  }

  function setPrivateActive() public onlyOwner {
...
// Metadata
  ...
  function mintListed(
    ...
    require(isPrivateActive, "Whitelist sale is closed");
    require(amount <= maxWhitelistAmount - _alreadyMinted[sender], "Insufficient whitelist mints left for this wallet");
    require(_verify(merkleProof, sender), "You are not on the whitelist");
    require(msg.value == price * amount, "Incorrect payable amount");
    require(maxWhitelistAmount >= _currentId + amount, "Will exceed maximum whitelist supply, wait for public mint");
    ...
    require(amount <= maxPublicAmount - _alreadyMinted[sender], "Insufficient mints left for this wallet");
    require(msg.value == price * amount, "Incorrect payable amount");
    require(msg.sender == tx.origin, "No minting from smart contracts");
  ...
  function royaltyInfo(uint256 _tokenId, uint256 _salePrice) external view returns (address, uint256 royaltyAmount) {
    _tokenId; //silence solidity compiler warning
    royaltyAmount = (_salePrice / 100) * 5; //change 5 to whatever % you want in royalties
