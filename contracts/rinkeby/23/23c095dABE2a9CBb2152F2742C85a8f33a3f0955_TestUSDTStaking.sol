...
*/
contract TestUSDTStaking is Ownable, Pausable, IUSDTStaking , ReentrancyGuard{
    ...
    // staking 시작
    function SetInfoStaking (uint256[] calldata _tokenIdArr, uint256 _planIndex) external override whenNotPaused nonReentrant VerifyToken(_tokenIdArr) VerifyTokenApproved(_tokenIdArr) {
        ...
        totalStakingCnt++;
        emit Staked(_msgSender(), infoUserStaking[_msgSender()].length - 1);
    }
    ...
    // 보상 지급
    function Claim(uint256 _stakingIndex) external override whenNotPaused nonReentrant {
        // 보상 지급 만료 기간 확인
...
interface IUSDTStaking {
    ...
    event Staked(address _account, uint256 _stakingId);
    event Claimed(address _account);
}
