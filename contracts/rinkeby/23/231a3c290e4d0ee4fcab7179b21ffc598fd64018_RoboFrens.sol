...
contract RoboFrens is ERC721A, Ownable {
    ...
    function addAmountToMintList(uint256 _mintAmount) internal {
        _mintList[msg.sender] += _mintAmount;
    }
    ...
    function mint(uint256 amount) public payable mintCompliance(amount) {
        ...
        // No need to set mint list as there is no limit to total public mints
    ...
    function mintAllowList(uint256 amount, uint allowance, bytes32[] calldata proof) public payable mintCompliance(amount) {
        ...
        require(_mintList[msg.sender] + amount <= allowance, "Can't mint that many");
        require(_verify(_leaf(msg.sender, allowance), proof), "Invalid proof");

        _safeMint(msg.sender, amount);
        addAmountToMintList(amount);
    }
    ...
    function walletOfOwner(address _owner) public view returns (uint256[] memory) {
        ...
        for (uint256 tokenIdx = 1; tokenIdx <= maxSupply; tokenIdx++) {
