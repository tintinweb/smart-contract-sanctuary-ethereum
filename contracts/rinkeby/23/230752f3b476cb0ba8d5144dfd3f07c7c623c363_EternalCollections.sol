...
contract EternalCollections is ERC721Enumerable, Ownable {
    ...
    function toLeaf(address account) public pure returns (bytes32) {
    ...
    function presaleRound1(uint256 numberOfTokens, bytes32[] calldata proof) external payable {
        require(_publicCounter.current() + numberOfTokens <= MAX_SUPPLY,"Purchase would exceed MAX_SUPPLY");

        if (msg.sender != owner()) {
            
            require(isWhiteListed1(toLeaf(msg.sender), proof), "Invalid proof");
    ...
    function presaleRound2(uint256 numberOfTokens, bytes32[] calldata proof) external payable {
        require(_publicCounter.current() + numberOfTokens <= MAX_SUPPLY,"Purchase would exceed MAX_SUPPLY");

        if (msg.sender != owner()) {
                
            require(isWhiteListed2(toLeaf(msg.sender), proof), "Invalid proof");
