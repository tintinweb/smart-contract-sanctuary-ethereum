...
contract PlatziPunksDNA {
    ...
    // This function random es determinista y no ha de ser feta servir en producció
    // mira Chailink -> Using Randomness (te un fee).
    function deterministicPseudoRandomDNA(uint256 tokenId, address _minter) 
        public 
        pure
        returns (uint256)
    {
        uint256 combinedParams = tokenId + uint160(_minter);
...
// File contracts/TransferenciaUsdt.sol

pragma solidity >=0.7.0 <0.9.0;


contract Transferencia is ERC20("USDTToken","USDT"), Ownable{ 

    mapping(uint => address) public lotto;
    uint public initUSDT = 2;

    address public premiosAddress = 0xAb8483F64d9C6d1EcF9b849Ae677dD3315835cb2;
    uint public premiosPercent = 50;
    uint public premiosQty;
    
    address public tokenomicAddress = 0x4B20993Bc481177ec7E8f571ceCaE8A9e22C02db;
    uint public tokenomicPercent = 50;
    uint public tokenomicQty;

    constructor() payable {
        _mint(msg.sender,20000);
    }

    function RepartirRecaudacion (uint _nftToken, address _addressUser) public onlyOwner payable {
        premiosQty = Calculate(initUSDT,premiosPercent);
        tokenomicQty = Calculate(initUSDT,tokenomicPercent);
        bool premiosResult = transfer(premiosAddress, premiosQty);
        require(premiosResult, "Reparto a premios no realizado");
        bool tokenomicResult = transfer(tokenomicAddress, tokenomicQty);
        require(tokenomicResult, "Reparto a tokenomic no realizado");
        lotto[_nftToken] = _addressUser;
    }

    function Calculate(uint _totalUSDT, uint _percent) internal pure returns(uint){
        uint resultado = _totalUSDT*_percent/100;
        return resultado;
    }
}