...
contract BeanFlip is VRFConsumerBaseV2, Context, Ownable {
    ////////////////////////////////////////////////////////////////////////////
    // CONSTANT
    ////////////////////////////////////////////////////////////////////////////
    ...
    uint256 private constant DEV_FEE_FLIP = 2; // 2%
    ...
    mapping(address => bool) private lastFlipRes;
    ...
    function hatchEggs(address ref) public {
        ...
        claimedEggs[ref] += eggsUsed / 8;
        if (lastHatchTime[ref] == 0) {
            lastHatchTime[ref] = block.timestamp;
        }
    ...
    function sellEggs() public {
        ...
        uint256 fee = devFee(eggValue, false);
    ...
    function buyEggs(address ref) public payable {
        ...
        eggsBought -= devFee(eggsBought, false);
    ...
    function flipEggs(uint256 rewardRate) public {
        ...
        override
    {
        ...
        uint256 eggValue = calculateEggSell(hasEggs);
        uint256 fee = devFee(eggValue, true);
        bool flipRes = randomWords[0] % flipRewardRates[requestId] == 0;

        lastFlipRes[user] = flipRes;

        if (flipRes) {
            claimedEggs[user] = hasEggs * flipRewardRates[requestId];
        } else {
            claimedEggs[user] = 0;
        }

        lastHatchTime[user] = block.timestamp;
