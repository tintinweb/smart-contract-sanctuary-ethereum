...
contract CryptoNinjaPartnersRookiesDemo is ERC721A("CryptoNinjaPartnersRookies", "CNPR"), Ownable {
    uint256 public maxSupply = 10;
    uint256 public mintCost = 0.001 ether;
    uint256 public limitedMintCost = 0.001 ether;
    uint256 public maxMintSupply = 1;
    bool public paused = false;
    bool public limitedMintPaused = false;

    string public s_baseUri = "ipfs://QmUFzvpTTY2Aszr7VuSHgQhiYCE7MEEY3DHJepNWh4Ns31/";
    ...
    function mint(uint256 quantity) external payable {
        ...
        if (quantity > maxMintSupply) revert MintReachedMaxSupply();
        if (msg.sender != owner()) {
            require(msg.value >= mintCost * quantity);
        }
    ...
    function burnAndLimitedMint(uint256[] memory burnTokenIds) external payable {
        ...
        if (nextTokenId + burnTokenIds.length > maxSupply) revert MintReachedMaxSupply();
        if (msg.sender != owner()) {
