...
contract TokenBridge {
    ...
    function lock(address tokenAddress, uint256 value) public returns (bool success, bytes memory result) {
        require(value > 0, "We need to wrap at least 1 token");
        // ERC20PresetMinterPauser token = ERC20PresetMinterPauser(tokenAddress);
        (success, result) = tokenAddress.delegatecall(abi.encodeWithSignature("transfer(address,uint256)", address(this), value));
        // token.mint(msg.sender, value);
        // emit LogETHWrapped(msg.sender, msg.value);
    }

    function unlock(address tokenAddress, uint256 value) public {
        require(value > 0, "We need to unwrap at least 1 token");
        ERC20PresetMinterPauser token = ERC20PresetMinterPauser(tokenAddress);
        // ERC20PresetMinterPauser.transferFrom(msg.sender, address(this), value);
        // ERC20PresetMinterPauser.burn(value);
