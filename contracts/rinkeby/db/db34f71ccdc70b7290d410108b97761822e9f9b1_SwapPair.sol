...
contract UniswapV2ERC20 is IUniswapV2ERC20, Initializable {
    ...
    string public constant name = 'Liquidity Provider Token';
...
contract SwapPair is IUniswapV2Pair, UniswapV2ERC20 {
    ...
    function _mintFee(uint112 _reserve0, uint112 _reserve1) private returns (bool feeOn) {
        (, uint _taxRate, address _taxToken, address feeTo) = SwapFactory(factory).getTaxTo(token0, token1);
    ...
    function swap(uint amount0Out, uint amount1Out, address to, bytes calldata data) external lock {
        ...
        (uint _feeRate, uint _taxRate, address _taxToken, address _taxTo) = SwapFactory(factory).getTaxTo(token0, token1);
...
contract SwapFactory is IUniswapV2Factory {
    ...
    function initialize(address _feeToSetter, address _productImplementation) public {
        require(feeToSetter== address(0) && _feeToSetter != address(0), 'SwapFactory.initialize can be delegatecall once by proxy only.');
...
contract SwapRouter02 is IUniswapV2Router01, IUniswapV2Router02, Initializable {
    using SafeMath for uint;
    ...
    function __SwapRouter02_init(address _factory, address _WETH) public initializer {
        factory = _factory;
        WETH = _WETH;
    }
    ...
    function _swapSupportingFeeOnTransferTokens(address[] memory path, address _to) internal {      // virtual 
        for (uint i; i < path.length - 1; i++) {
            ...
            amountOutput = SwapFactory(factory).getAmountOut(amountInput, reserveInput, reserveOutput);
            }
        ...
        returns (uint amountOut)
    {
        ...
        return SwapFactory(factory).getAmountOut(amountIn, reserveIn, reserveOut);
    }
        ...
        returns (uint amountIn)
    {
        ...
        return SwapFactory(factory).getAmountIn(amountOut, reserveIn, reserveOut);
    }
...
contract SwapFarmingRouter is SwapRouter02 {
    ...
    modifier onlySetter() {
        require(SwapFactory(factory).isSetter(msg.sender), 'FORBIDDEN');
        _;
    }

    function __SwapFarmingRouter_init(
        address _factory, 
        address _WETH,
        ...
        __SwapRouter02_init(_factory, _WETH);
        __SwapFarmingRouter_init_unchained(_rewardsDistribution, _rewardsToken, _ecoAddr);
    }

    function __SwapFarmingRouter_init_unchained(
        address _rewardsDistribution,
        address _rewardsToken,
        address _ecoAddr
    ) public onlySetter {
    ...
    function _swapFarming(address input, address output, uint amountIn, uint amountOut) internal {
        (uint feeRate, uint taxRate, address taxToken, ) = SwapFactory(factory).getTaxTo(input, output);
    ...
    function swapFarmingable(address taxToken, uint amount) external view returns (uint reward) {
        (reward, , , , ) = _swapFarmingable(taxToken, amount, SwapFactory(factory).feeRate(address(0)));
    }
    ...
    function swapFarmingablePath(uint amountIn, address[] calldata path) external view returns (uint reward) {
        ...
        for(uint i=0; i<amounts.length-1; i++) {
            (uint feeRate, uint taxRate, address taxToken, ) = SwapFactory(factory).getTaxTo(path[i], path[i+1]);
            uint rate = feeRate.mul(taxRate) / 1e18;
            uint rwd = 0;
            if(path[i] == taxToken)
...
library UniswapV2Library {
    ...
    function pairFor(address factory, address tokenA, address tokenB) internal view returns (address pair) {
        ...
        //        factory,
        //        keccak256(abi.encodePacked(token0, token1)),
		//		SwapFactory(factory).pairCodeHash                                    //hex'96e8ac4277198ff8b6f785478aa9a39f403cb768dd02cbee326c3e7da348845f' // init code hash
        //    ))));
        return SwapFactory(factory).getPairFor(tokenA, tokenB);
    }
    ...
    function getAmountsOut(address factory, uint amountIn, address[] memory path) internal view returns (uint[] memory amounts) {
        ...
        //    amounts[i + 1] = getAmountOut(amounts[i], reserveIn, reserveOut);
        //}
        return SwapFactory(factory).getAmountsOut(amountIn, path);
    }
    ...
    function getAmountsIn(address factory, uint amountOut, address[] memory path) internal view returns (uint[] memory amounts) {
        ...
        //    amounts[i - 1] = getAmountIn(amounts[i], reserveIn, reserveOut);
        //}
        return SwapFactory(factory).getAmountsIn(amountOut, path);
    }
}
...
contract DeployFactory {
    ...
    constructor(address adminFactory, address adminPair) public {
        ...
        emit Deploy('SwapPair', address(pair));
        
        SwapFactory factory = new SwapFactory();
        emit Deploy('SwapFactory', address(factory));
...
contract DeployRouter {
    ...
    constructor(address adminRouter, address factoryProxy, address WETH) public {
        ...
        require(WETH != address(0), 'SwapFactoryFactory: WETH address is 0x0');

        SwapRouter02 router = new SwapRouter02();
        router.__SwapRouter02_init(address(factoryProxy), WETH);
        emit Deploy('SwapRouter02', address(router));
    ...
    function WETH() internal pure returns (address addr) {
        assembly {
            switch chainid() 
                ...
                case 36  { addr := 0x230761E165EC7f6A46B42CCba786bFC0856F4C41 }      // ETM3 Mainnet
                case 37  { addr := 0x230761E165EC7f6A46B42CCba786bFC0856F4C41 }      // ETM3 TestNet
