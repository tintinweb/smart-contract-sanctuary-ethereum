...
contract Weenimals is ERC721A, Ownable {
    ...
    uint8 public constant maxPerWallet = 2;
    uint8 public constant maxPerTransaction = 2;

    uint16 public reservedNFTsAmount;
    uint16 public basicNFTsAmount;
    ...
    uint16 private constant basicNFTsPerSet = 2;
    ...
    constructor(string memory name, string memory symbol) ERC721A(name, symbol) {
        basicNFTsAmount = 0; // max 8
        reservedNFTsAmount = 0; // max 4
    }
    ...
    function mint(
        ...
        require(
            ...
            string(
                abi.encodePacked(
                    "Cannot mint more than 2 in a single transaction"
                )
            )
        );
        require(msg.value == (nftPrice * amount), "The price is invalid");
        require(saleStarted == true, "The sale is paused");
        require(basicNFTsAmount + amount <= currentSet * basicNFTsPerSet, "Set limit reached");
        ...
        basicNFTsAmount += amount;
        alreadyMinted[msg.sender] += amount;

        _mint(msg.sender, amount);

        if (currentSet * basicNFTsPerSet == basicNFTsAmount) {
            pauseSale();
        }
    }
