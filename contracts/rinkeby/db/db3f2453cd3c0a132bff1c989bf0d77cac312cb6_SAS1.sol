...
contract SAS1 is ERC721A{
    ...
    bytes32[] public WhiteList1 = [bytes32(0x70629ca247620e39996b1f85d3410a3cd7c6aec1a27339f3dc248d3ba466142c),
bytes32(0x8afc872c323e48344d1457d434036e71cf8d11560172d1670f10f871fbed8510)];
   ...
   function checker_count_mint1() public view returns(uint){
    ...
    function Checker_address_FreeMint() public returns (bool){
         
         for(uint i = 0; i < WhiteList1.length; i++) {
             if(keccak256(abi.encodePacked(msg.sender)) == WhiteList1[i])
             return true;
         }
        return false;
    }
   ...
   function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        return string(string(abi.encodePacked("link", 
        Strings.toString(tokenId),
        ".json")));
    }

    function Give_AddressToBytes(address[] memory adr) public view returns(bytes32[] memory){
        bytes32[] memory arr = new bytes32[](adr.length);

       for(uint i = 0; i < adr.length; i++) {
             arr[i] = keccak256(abi.encodePacked(adr[i]));
         }
