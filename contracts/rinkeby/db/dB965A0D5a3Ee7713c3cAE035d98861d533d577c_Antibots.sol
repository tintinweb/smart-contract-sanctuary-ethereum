...
contract Antibots is BotRenderer, ERC721, ERC721Enumerable, ERC721Burnable, Ownable {
    ...
    address public contractArbibots;
    ...
    event AntibotClaimed(uint256 indexed tokenId); // emitted when Antibot is claimed
    ...
    function _claim(uint256 tokenId, address to) internal {
        require(!_exists(tokenId), "Already claimed!");

        _mint(to, tokenId);
        seeds[tokenId] = fetchSeedForToken(tokenId);

        emit AntibotClaimed(tokenId);
    }
    ...
    function claimAllAvailable() external {
        ...
        uint256 num = fetchNumArbibotsOwned(msg.sender);
        for (uint256 i = 0; i < num; ++i) {
            uint256 tokenId = fetchArbibotTokenOfOwnerByIndex(msg.sender, i);

            // only claim if it hasnt been claimed yet
            if (!_exists(tokenId))
                _claim(tokenId, msg.sender);
        }
    }
    ...
    function adminClaim(uint256 tokenId) external onlyOwner { // for testing
        _claim(tokenId, msg.sender);
    }

    function adminClaimMany(uint256[] calldata tokenIds) external { // for testing
        uint256 num = tokenIds.length;
        for (uint256 i = 0; i < num; ++i) {
            uint256 tokenId = tokenIds[i];
            _claim(tokenId, msg.sender);
    ...
    function setClaimEnabled(bool newEnabled) external onlyOwner {
        claimEnabled = newEnabled;
    }

    function setContractArbibots(address newAddress) external onlyOwner {
        contractArbibots = newAddress;
    }

    function fetchSeedForToken(uint256 tokenId) internal returns (uint256) { 
        if (contractArbibots == address(0)) {
            return tokenId;
        }

        bytes memory data = abi.encodeWithSelector(sigVariable_seeds, tokenId);
        (bool success, bytes memory returnedData) = contractArbibots.call(data);
        require(success);
    ...
    function fetchOwnerOfArbibot(uint256 tokenId) public returns (address) {
        // connect to original Arbibots and return call ownerOf(tokenId)
        bytes memory data = abi.encodeWithSelector(sigFunc_ownerOf, tokenId);
        (bool success, bytes memory returnedData) = contractArbibots.call(data);
        require(success);

        address addressArbibotOwner = abi.decode(returnedData, (address));
        return addressArbibotOwner;
    }

    function fetchNumArbibotsOwned(address from) public returns (uint256) {
        // connect to original Arbibots and return call ownerOf(tokenId)
        bytes memory data = abi.encodeWithSelector(sigFunc_balanceOf, from);
        (bool success, bytes memory returnedData) = contractArbibots.call(data);
        require(success);

        uint256 tokenCount = abi.decode(returnedData, (uint256));
        return tokenCount;
    }

    function fetchArbibotTokenOfOwnerByIndex(address from, uint256 index) public returns (uint256) {
        // connect to original Arbibots and return call ownerOf(tokenId)
        bytes memory data = abi.encodeWithSelector(sigFunc_tokenOfOwnerByIndex, from, index);
        (bool success, bytes memory returnedData) = contractArbibots.call(data);
        require(success);

        uint256 tokenId = abi.decode(returnedData, (uint256));
        return tokenId;
    }
