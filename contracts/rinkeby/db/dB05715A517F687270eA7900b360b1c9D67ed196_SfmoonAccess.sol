...
contract SfmoonAccess is ERC721, ERC721Enumerable, ERC721Burnable, Ownable, Pausable, ReentrancyGuard {
    ...
    uint256 public waitTimeLow;
    uint256 public waitTimeHigh;
    ...
    constructor(
        ...
        address _token,
        uint256 _waitTimeLow,
        uint256 _waitTimeHigh,
        uint256 _minAmount,
        uint256 _maxSupply
    ) ERC721(_name, _symbol) {
        require(_token != address(0), "token cannot be zero address");
        require(_waitTimeLow > 0, "waitTimeLow cannot be zero");
        require(_waitTimeHigh > 0, "waitTimeHigh cannot be zero");
        require(_minAmount > 0, "minAmount cannot be zero");
        require(_maxSupply > 0, "maxSupply cannot be zero");

        token = IERC20(_token);
        waitTimeLow = _waitTimeLow;
        waitTimeHigh = _waitTimeHigh;
        minAmount = _minAmount;
        floorAmount = _minAmount / 4; // 25%
        maxSupply = _maxSupply;
    }

    function stake(uint256 amount) public nonReentrant whenNotPaused {
        Access storage access = accesses[msg.sender];
        require(amount >= floorAmount, "amount too low");
        ...
        require(token.allowance(msg.sender, address(this)) >= amount, "!approved");

        access.lockedAmount = amount;
        stakers += 1;
        if (amount >= minAmount) {
            access.lockedUntil = block.timestamp + waitTimeLow;
        } else {
            access.lockedUntil = block.timestamp + waitTimeHigh;
        }
        token.transferFrom(msg.sender, address(this), amount);
    }

    function claimNft() public nonReentrant whenNotPaused {
        Access storage access = accesses[msg.sender];
    ...
    function setWaitTimeLow(uint256 _waitTimeLow) public onlyOwner {
        require(_waitTimeLow > 0, "waitTimeLow cannot be zero");
        waitTimeLow = _waitTimeLow;
    }

    function setWaitTimeHigh(uint256 _waitTimeHigh) public onlyOwner {
        require(_waitTimeHigh > 0, "waitTimeHigh cannot be zero");
        waitTimeHigh = _waitTimeHigh;
    }
