...
contract Wallet is Ownable {
    ...
    event InitiatorRemoved(address indexed oldInitiator);
    ...
    function addInitiator(address __newInitiator) external onlyOwner {
        require(__newInitiator != address(0), "Initiator: new initiator is the zero address");
        initiators[__newInitiator] = true;
        emit InitiatorAdded(__newInitiator);
    }

    function removeInitiator(address __oldInitiator) external onlyOwner {
        require(__oldInitiator != address(0), "Initiator: old initiator is the zero address");
        initiators[__oldInitiator] = false;
        emit InitiatorRemoved(__oldInitiator);
    }

    modifier onlyInitiator() {
        require(initiators[_msgSender()], "Initiator: caller is not the initiator");
        _;
    }
    ...
    function setProcessorRequired(bool __processorRequired) external onlyOwner {
        processorRequired = __processorRequired;
    }
    ...
    function deposit(uint256 __depositId) external payable {
        require(msg.value > 0, "Funding error");

        deposits[depositCount] = __depositId;
        depositSenders[__depositId] = _msgSender();
        depositAmounts[__depositId] = msg.value;

        depositCount++;
        emit Deposit(__depositId, _msgSender(), msg.value);
    }

    function request(uint256 __withdrawalId, address __recipient, uint256 __amount) external onlyInitiator {
    ...
    function __withdraw(uint256 __withdrawalId, address __recipient, uint256 __amount) internal {
        payable(__recipient).transfer(__amount);
    ...
    function withdraw(uint256[] calldata __withdrawals) external onlyManager {
        ...
        require(address(this).balance > __total, "Process: insuficient balance");
