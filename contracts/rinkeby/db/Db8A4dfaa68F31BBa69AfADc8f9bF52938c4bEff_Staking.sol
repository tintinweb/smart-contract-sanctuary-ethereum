...
contract Staking is Ownable, ERC721Holder {
    ...
    function pendingSpeed(address _user) internal view returns (uint256) {
        ...
        uint256 pending = user.amount.mul(accSpeedPerShare).div(1e12);
    ...
    function payReward(UserInfo storage _user) internal {
        uint256 pending = _user.amount.mul(poolInfo.accSpeedPerShare).div(1e12);
        if (pending > 0) {
            ...
            normalizeReward = normalizeReward.sub(_user.rewardDebt);
    ...
    function depositFacility(uint256 _tokenId) external {
        ...
        user.rewardDebt = pendingSpeed(msg.sender);
    ...
    function withdrawStable(uint256 _stableTokenId) external {
        UserInfo storage user = userInfo[msg.sender];
        console.log("withdrawStable",_stableTokenId);
        require(user.ownedStable[_stableTokenId], "WithdrawStable:No stable staking");
        ...
        horseLimitStaking
    {
        PoolInfo storage pool = poolInfo;
        UserInfo storage user = userInfo[msg.sender];
        require(user.ownedStable[_stableTokenId], "DepositHorseInStable:No stable staking");
    ...
    function withdrawHorseInStable(
        ...
        PoolInfo storage pool = poolInfo;
        UserInfo storage user = userInfo[msg.sender];
        console.log("user.ownedStable[_stableTokenId]",user.ownedStable[_stableTokenId]);
        require(user.ownedStable[_stableTokenId], "WithdrawHorseInStable:No stable staking");
        updatePool();
        uint256 pending = pendingSpeed(msg.sender).sub(user.rewardDebt);
        if (pending > 0) {
            payReward(user);
