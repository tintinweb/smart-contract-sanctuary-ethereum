...
/**
 * @title MCIncoGnito wrapper contract
 */
    ...
    MerkleLeaves
{
    ...
    uint256 private constant MAX_MINT_BATCH_SIZE = 3;
    uint256 private constant PRESALE_QUANTITY = 2;
    uint256 private constant PUBLIC_QUANTITY = 3;

    uint256 private constant PRESALE_PHASE = 1;
    uint256 private constant PUBLIC_PHASE = 2;
    ...
    function presaleTokens(bytes32[] calldata proof, uint256 count) external payable nonReentrant isPresale {
        require(0 < count && count <= PRESALE_QUANTITY, 'Invalid count');
        require(msg.value >= presalePricePerNft * count, 'Invalid price');

        // a == presale quantity purchased
        // b == public mint quantity purchased (don't need it here)
        (uint32 a, uint32 b) = _unpack(_getAux(_msgSender()));
        
        // console.log('presale wallet', _msgSender());
        // console.log('presales purchased', a);
        // console.log('public purchased', b);
        // console.log('count', count);
        
        if (a + count > PRESALE_QUANTITY)
            revert ExceedsMaxWhitelistMints();

        _presaleTokens(_msgSender(), proof, count);
        _setAux(_msgSender(), _pack(uint32(a + count), 0));
    }
    ...
    function mintTokens(uint256 count) external payable nonReentrant onlyUsers isPublicMinting {
        require(0 < count && (count <= MAX_MINT_BATCH_SIZE && count <= PUBLIC_QUANTITY), 'Invalid count');
        require(msg.value >= publicMintPricePerNft * count, 'Invalid price');
        require(count + _numberMinted(_msgSender()) <= (PRESALE_QUANTITY + PUBLIC_QUANTITY), 'Max number purchased');

        _internalMintTokens(_msgSender(), count);
    }

    function mintTokensExpensive(uint256 count) external payable nonReentrant onlyUsers isPublicMinting {
        require(0 < count && (count <= MAX_MINT_BATCH_SIZE && count <= PUBLIC_QUANTITY), 'Invalid count');
        require(msg.value >= publicMintPricePerNft * count, 'Invalid price');

        // a == presale quantity purchased (don't need it here)
        // b == public mint quantity purchased
        (uint32 a, uint32 b) = _unpack(_getAux(_msgSender()));

        // console.log('mint wallet', _msgSender());
        // console.log('presales purchased', a);
        // console.log('public purchased', b);
        // console.log('count', count);

        if (b + count > PUBLIC_QUANTITY)
            revert ExceedsMaxTransactionMints();

        _internalMintTokens(_msgSender(), count);
        _setAux(_msgSender(), _pack(0, uint32(a + count)));
    }

    // pack values for presalePurchases and publicMintPurchases into a single variable for use in _setAux()
    function _pack(uint32 a, uint32 b) internal pure returns(uint64) {
        return (uint64(a) << 32) | uint32(b);
    }

    // unpack single value from _getAux() to determine presalePurchases and publicMintPurchases
    function _unpack(uint64 c) internal pure returns(uint32 a, uint32 b) {
        return (uint32(c >> 32), uint32(c));
    }

    // external version of unpack to use in unit tests
    function unpack(address wallet) external view returns (uint32 a, uint32 b) {
        return _unpack(_getAux(wallet));
    }
    ...
    function _presaleTokens(
        ...
        require(
            _presaleRoot._checkLeaf(
                proof, 
                _generateLeaf(minter)
            ), 
            'Proof invalid for presale'
        );
