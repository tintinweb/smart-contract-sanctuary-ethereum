...
contract Dobbys is ERC721A, Ownable, ReentrancyGuard {
    ...
    mintPriceCompliance(_mintAmount)
    {
        ...
        uint256 currentPhaseSupply = supplyLimiters[currentPhase];
        uint256 prevTotalSupply = getPrevTotalSupply();

        require(
            totalSupply() >= prevTotalSupply &&
            totalSupply() + _mintAmount <= prevTotalSupply + currentPhaseSupply,
            "Current phase supply exceeded!"
        );
        fulfillRandomness(_msgSender(), _mintAmount);
    }
    ...
    function fulfillRandomness(address _minter, uint256 _amount) private {
        ...
        uint256 prevTotalSupply = getPrevTotalSupply();
        if (totalSupply() == prevTotalSupply + supplyLimiters[currentPhase]
            && currentPhase < supplyLimiters.length - 1) {
            currentPhase++;
            // format the ids and index for next phase
