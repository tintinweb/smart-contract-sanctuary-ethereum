...
contract toptop is ERC721A, Ownable, ReentrancyGuard {
    enum Status {
        Waiting,
        Started,
        Finished
    ...
    string private baseURI = "ipfs://QmNLiNP7UvqcM8eLxC6uHg6uFkTJP8mTACnUyx5LQSpdtF/";
    uint256 public constant maxmintperaddr = 6;
    uint256 public constant maxfreeperaddr = 2;
    uint256 public price = 0.0025 * 10**18;
    uint256 public constant maxSupply = 8888;
    uint256 public constant freeSupply = 4444;
    uint256 public instantfree = 1;


    event Minted(address minter, uint256 amount);

    constructor(string memory initBaseURI) ERC721A("toptop", "topz") {
        baseURI = initBaseURI;
        _safeMint(msg.sender, maxfreeperaddr);
    }
    ...
    function mint(uint256 quantity) external payable nonReentrant {
        ...
        uint256 _cost;
        if (instantfree < freeSupply) {
            uint256 remainFreeAmont = (numberMinted(msg.sender) <
                maxfreeperaddr)
                ? (maxfreeperaddr - numberMinted(msg.sender))
                : 0;
