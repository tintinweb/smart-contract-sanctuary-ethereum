...
pragma solidity ^0.8.0;
    ...
    contract GenesisContractInterface {
...
contract ShadowQuest is Ownable, ERC721 {
    ...
    uint256 public metaKidzReserve = 100;
    ...
    GenesisContractInterface public genesisContractCaller;
    ...
    constructor(address _metabitsContractAddress, address _genesisContractAddress) ERC721("MetaKidz V1","MetaKidz")   
    {
        SALE_PRIVATE_MAX_SUPPLY = 3000;
        metabitsContractAddress = _metabitsContractAddress;
        metabits = Metabits(_metabitsContractAddress);
        genesisContractCaller = GenesisContractInterface(_genesisContractAddress);
    }
    ...
    function setMetaBitsContractAddress(address _addr) external onlyOwner {
        // honorContract = honorContract_;
        metabitsContractAddress = _addr;
        metabits = Metabits(_addr);
    }

    function setGenesisContractAddress(address _addr) public onlyOwner(){
        // require(msg.sender == admin || msg.sender == owner(), "Invalid sender");
        genesisContractCaller = GenesisContractInterface(_addr);
    }
    ...
    function mintReserved(address wallet, uint16 numberOfToken) external onlyOwner {
         require(
            numberOfToken > 0 && numberOfToken <= metaKidzReserve,
            "Not enough reserve left for team"
        );
        uint256 genesisBalance = genesisContractCaller.balanceOf(msg.sender);
        require(gen0Supply.add(1) <= MAX_GEN0_SUPPLY.sub(metaKidzReserve), "Purchase would exceed max supply of MetaKidz");
        require(genesisBalance == 1 ,"user does not have genesis token");
        uint256 genesisTokenId = genesisContractCaller.tokenOfOwnerByIndex(msg.sender, 0);
        require(_isClaimed[genesisTokenId] == false,"Token is already claimed");
        address receiver = wallet == address(0) ? _msgSender() : wallet;

        _mintGen0(receiver, numberOfToken);
            _isClaimed[genesisTokenId] = true;   
        metaKidzReserve = metaKidzReserve.sub(numberOfToken);

    }

    function genesisHolderFreeMint(address wallet, uint16 amount) external {
        address receiver = wallet == address(0) ? _msgSender() : wallet;

        _mintGen0(receiver, amount);
    }

    function mintMetaKidzPresale(uint16 numberOfTokens) public payable {
        require(presaleIsActive, "Sale is not active at the moment");
        require(gen0Supply.add(numberOfTokens) <= MAX_GEN0_SUPPLY.sub(metaKidzReserve), "Purchase would exceed max supply of MetaKidz");
        require(numberOfTokens <= MAX_NFT_PURCHASE_PRESALE,"Can only mint up to 3 per purchase");
        require(NFT_PRICE.mul(numberOfTokens) <= msg.value, "Sent ether value is incorrect");

        _mintGen0(msg.sender, numberOfTokens);
    }

    function mintMetaKidz(uint16 numberOfTokens) public payable {
