...
contract tinyfaces is ERC721A, Ownable, ReentrancyGuard {
    ...
    uint256 public mintRate = 0.0002 ether;
    uint256 public whitelistMintRate = 0.0001 ether;
    ...
    function mint(uint256 quantity) external payable nonReentrant {
        ...
        require(normalSale == true, "Normal sale didn't start yet");
        require(
    ...
    function whitelistBuy(uint256 quantity, bytes32[] calldata proof)
        ...
        nonReentrant
    {
        require(quantity <= 2, "Only 2 whitelist mints are allowed");
        require(
            msg.value >= (whitelistMintRate * quantity),
            "Not enough ether sent"
        );
        require(
            usedAddresses[msg.sender] + quantity <= 2,
            "Whitelist spots used already"
        );
        require(whitelistSale == true, "Whitelist sale didn't start yet");
        require(
            totalSupply() + quantity <= maxSupply,
            "Not enough tokens left"
        );
        require(isWhiteListed(msg.sender, proof), "Invalid merkle proof");
    ...
    function setNotRevealedBURI(string memory _newURI) public onlyOwner {
