...
// File: contracts/KIP17Capped.sol
 ...
 * @dev Capped KIP17
 */
contract KIP17Capped is KIP17 {
    using Counters for Counters.Counter;
    
    uint256 public _supplyCap;
    Counters.Counter private _allMintedTokens;

    /**
     * @dev Constructor function.
     */
    constructor (uint256 supplyCap) public {
        _supplyCap = supplyCap;
    }

    /**
     * @dev Internal function to mint a new token.
     * Reverts if the total supply is reached.
     * @param to The address that will own the minted token
     * @param tokenId uint256 ID of the token to be minted
     */
    function _mint(address to, uint256 tokenId) internal {
        require(_allMintedTokens.current() < _supplyCap, "KIP17Capped: Supply cap has been reached");
        _allMintedTokens.increment();
        super._mint(to, tokenId);
    }
}

// File: contracts/KIP17Token.sol
...
contract KIP17Token is KIP17Full, KIP17Capped, KIP17Mintable, KIP17MetadataMintable, KIP17Burnable, KIP17Pausable {
    constructor (string memory name, string memory symbol, uint256 supplyCap) public KIP17Full(name, symbol) KIP17Capped(supplyCap) {
    }
}
...
// File: contracts/ArtworkToken.sol
...
contract ArtworkToken is KIP17Token, Ownable {
    using Counters for Counters.Counter;
    ...
    constructor (
        ...
        uint256 supplyCap) public KIP17Token(name, symbol, supplyCap)
    {
        _baseTokenURI = baseTokenURI;
    }

    /**
     * @dev Mint a token to the given addresses
     * @param recipient address address to which a token to be minted
     * @param tokenURI string metadata URI of token to be minted
     */
