...
contract PokerLookNFT is ERC721Enumerable, Auth {
    ...
    uint256 public vipPrice = 0.1 ether;
    ...
    event SetVipMember(address _member);
    event SetVipPrice(uint256 _value);
    ...
    function setVipMember(address _member) external authorized {
        vip[_member] = true;
        emit SetVipMember(_member);
    }

    function setVipPrice(uint256 _vipPrice) external authorized {
        vipPrice = _vipPrice;
        emit SetVipPrice(_vipPrice);
    }
    ...
    function publicMint(uint256 _number, address ref_addr) public payable {
        ...
        require(_number <= maxMintNumber, "Too many tokens to mint at once.");
        if (vip[msg.sender]) {
            require(msg.value >= vipPrice.mul(_number), "Amount is not enough!");
        } else {
