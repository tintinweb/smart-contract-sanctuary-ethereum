...
contract TST is ERC721A, ReentrancyGuard, Ownable {
    ...
    uint256 public CLAIM_TOTAL = 2500;
    uint256 public claimed = 0;
    function claim(uint256 _count) public nonReentrant onlyWhitelisted {
	require( sale_WL_IsActive, "WL Sale not active" );
	require( _count > 0, "0 tokens to mint" );
	require( totalSupply() + _count - 1 < MAX_SUPPLY, "Exceeds max supply" );
	require( _count <= CLAIM_LIMIT, "Exceeds max claim per address" );
	require( _count <= MAX_MULTIMINT, "Exceeds max mints per transaction" );
	uint256 _mintedAmount = mintedAmount[msg.sender];
	uint256 _claimedAmount = claimedAmount[msg.sender];
	require( _mintedAmount + _count <= MINT_LIMIT_PER_WALLET, "Exceeds max mints per address" );
	require( _claimedAmount + _count <= CLAIM_LIMIT, "Exceeds max claims per address" );
	require( claimed + _count <= CLAIM_TOTAL, "Exceeds claim supply" );
	_mint(msg.sender, _count);
