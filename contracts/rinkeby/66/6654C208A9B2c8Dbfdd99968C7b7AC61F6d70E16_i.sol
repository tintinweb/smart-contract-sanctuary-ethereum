...
contract i is ERC721, ERC721Enumerable, ERC721Royalty, Ownable, ReentrancyGuard {
    ...
    event SetParam(uint256 indexed tokenId, string paramName, uint256 newValue);
    ...
    function setBrightness(uint256 tokenId, uint8 newValue) public onlyTokenOwner(tokenId) validParamValue(newValue) {
        params[tokenId].brightness = newValue;

        emit SetParam(tokenId, 'brightness', newValue);
    }

    function setAmplitude(uint256 tokenId, uint8 newValue) public onlyTokenOwner(tokenId) validParamValue(newValue) {
        params[tokenId].amplitude = newValue;

        emit SetParam(tokenId, 'amplitude', newValue);
    }

    function setFrequency(uint256 tokenId, uint8 newValue) public onlyTokenOwner(tokenId) validParamValue(newValue) {
        params[tokenId].frequency = newValue;

        emit SetParam(tokenId, 'frequency', newValue);
    }
    ...
    function setCoords(uint256[] calldata tokenIds, Coords[] calldata coordsData) public onlyOwner {
        for (uint256 i = 0; i < tokenIds.length; i++) {
            coords[tokenIds[i]] = coordsData[i];
        }
    }

    function setPresaleList(address[] calldata addresses, uint256[] calldata remainings) external onlyOwner {
        for (uint256 i = 0; i < addresses.length; i++) {
            presaleList[addresses[i]] = remainings[i];
        }
    }
    ...
    function _synthesizeParams(uint256 blockNumber, uint256 tokenId) private {
        params[tokenId] = Params({
            brightness: uint8((blockNumber + (tokenId * 13)) % (24 + (tokenId * 7)) % 101),
            frequency: uint8((blockNumber + (tokenId * 33)) % (31 + (tokenId * 17)) % 101),
            amplitude: uint8((blockNumber + (tokenId * 11)) % (19 + (tokenId * 3)) % 101)
        });
    }
