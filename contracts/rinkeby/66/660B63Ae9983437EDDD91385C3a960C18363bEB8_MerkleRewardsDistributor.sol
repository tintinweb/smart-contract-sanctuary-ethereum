...
contract MerkleRewardsDistributor is Ownable, ReentrancyGuard {
    ...
    address private SUSHISWAP_V2_ROUTER;
    ...
    // IUniswapV2Router02 public sushiSwapRouter = IUniswapV2Router02(SUSHISWAP_V2_ROUTER);
    ...
    event EpochEnded(uint256 endedEpochNum, uint256 addedTokensInEpoch);

    constructor(address _tokenContract, address _sushiRouterAddress) {
        tokenContract = IERC20(_tokenContract);
        SUSHISWAP_V2_ROUTER = _sushiRouterAddress;
    }
    ...
    function endEpoch(uint256 royaltiesAmount, bytes32 _rebuiltMerkleRoot) external onlyOwner {
        require(royaltiesAmount > 0, "Non-positive amount of royalties");

        setRoot(_rebuiltMerkleRoot);
        emit EpochEnded(_epochsCount, royaltiesAmount);
        _epochsCount++;
    }
    ...
    function swap(
        ...
        IERC20(_tokenIn).approve(SUSHISWAP_V2_ROUTER, _amountIn);
        ...
        IUniswapV2Router02(SUSHISWAP_V2_ROUTER).swapExactTokensForTokens(
            _amountIn,
            _amountOutMin,
            path,
            _to,
