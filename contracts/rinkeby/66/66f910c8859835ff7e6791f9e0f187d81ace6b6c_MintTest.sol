...
contract MintTest is Ownable, ERC721A {
    uint256 public constant SUPPLY = 10000;
    uint256 public constant ALLOWED_PER_TX = 5;
    ...
    constructor() ERC721A("MintTest", "BOTTER") {}
    ...
    function mint(uint256 amount) external payable isNotContract {
        ...
        require(amount <= ALLOWED_PER_TX, "Exceeds max mint");
        require(totalSupply() + amount <= SUPPLY, "Exceeds supply");
        require(tokenPrice * amount <= msg.value, "Incorrect ether value");

        uint64 amountMinted = _getAux(msg.sender);
        require(amountMinted + amount <= MAX_PER_WALLET, "Exceeds max per wallet");
