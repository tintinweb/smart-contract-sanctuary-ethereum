...
contract testhack is Ownable, ERC721A, ReentrancyGuard, ERC2981 {
    ...
    // DELETE_ME used for gas testing different approaches
    function directMint(uint8 quantity, uint8 userAllotment, uint32 userSaleKey, bytes calldata signature) 
        external
        payable
        callerIsUser
        onlyStage(Stage.ALLOWLIST_SALE)
    {
        ...
        AllowlistSaleConfig storage config = allowlistSaleConfig;
      //  AllowlistSaleConfig memory config = allowlistSaleConfig;
        ...
        /*
        if (quantity > config.maxBatch) revert ExceededAllowlistMaxBatch();
        if (config.key != userSaleKey) revert IncorrectAllowlistSaleKey();
        if (totalSupply() + quantity > config.totalAllotment) revert ExceededAllowlistTotalAllotment();
        if (numberMinted(msg.sender) + quantity > userAllotment) revert ExceededAllowlistPersonalAllotment();
        // TODO: memoize improve gas?
        if (numberMinted(msg.sender) + quantity > config.maxPerAddress) revert ExceededAllowlistMaxPerAddress();
        if (recoverSigner(message, signature) != owner()) revert InvalidAllowlistSignature();
        */
        if (quantity > maxBatch) revert ExceededAllowlistMaxBatch();
        if (saleKey != userSaleKey) revert IncorrectAllowlistSaleKey();
        //if (numberMinted(msg.sender) + quantity > userAllotment) revert ExceededAllowlistPersonalAllotment();
        // TODO: memoize improve gas?
