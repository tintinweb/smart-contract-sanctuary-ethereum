...
contract Marketplace is IERC721Receiver {
     ...
     */
    function isOpen(uint256 _nftId, address _nftCollection)
        public
        returns (bool)
    {
        if (nftAuctionData[_nftId][_nftCollection].isAuctionOn) {
            if (
                block.timestamp >=
                nftAuctionData[_nftId][_nftCollection].endAuction
            ) {
    ...
    function bid(
        ...
        require(isOpen(_nftId, _nftCollection), "Auction is not open");
    ...
    function claimNFT(uint256 _nftId, address _nftCollection) external {
        ...
        require(!isOpen(_nftId, _nftCollection), "Auction is still open");
    ...
    function cancelSaleOrAuction(uint256 _nftId, address _nftCollection) external {
        ...
        else if(isOpen(_nftId, _nftCollection)){
