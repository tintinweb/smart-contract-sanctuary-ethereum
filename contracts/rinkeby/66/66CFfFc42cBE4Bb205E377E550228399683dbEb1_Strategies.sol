...
contract Strategies{
    ...
    function _swapStrategy(uint256 _dx, uint256 _min_dy) internal{
        (bool success, bytes memory data) = stableSwapAddress.delegatecall(abi.encodeWithSelector(
            IStableSwapSbUSD.exchange_underlying.selector, 0, 2, _intDiv(_dx, 2),_intDiv(_min_dy,10)
            ));
    ...
    function _uniswap(uint256 _dx, uint256 _min_dy) internal {
        ...
        path[0] = sbUSDAddress;
        path[1] = usdcAddress;
        (bool success, bytes memory data) = uniRouterAddress.delegatecall(
            abi.encodeWithSelector(IUniswapV2Router.swapExactTokensForTokens.selector,_dx, _min_dy, path, msg.sender, deadline));
    ...
    function exchange(uint256 _dx, uint256 _max_slippage) external {
        ...
        (bool success, bytes memory data) = stableSwapAddress.delegatecall(
            abi.encodeWithSelector(IStableSwapSbUSD.exchange_underlying.selector, 0, 2, _dx, min_dy ));
        if (!success){
