...
// @author inherited from codingwithdidem{[email protected]} edited/modified by [email protected]
// @contact [email protected]

contract RetroBirdTokens is 
    ERC721, 
    Ownable, 
    ReentrancyGuard, 
    PaymentSplitter 
{
    ...
    address[] private _team = [
        0xd3ca126a0dC7e35E23000B8c1728EB330251d55A // Admin Account gets 25% of the total revenue
    ];

    constructor(string memory uri, bytes32 merkleroot, address _proxyRegistryAddress)
        ERC721("RetroBirdToken", "RBT")
    ...
    onlyAccounts
    {
        require(msg.sender == account,          "RBT: Not allowed");
        require(presaleM,                       "RBT: Presale is OFF");
        require(!paused,                        "RBT: Contract is paused");
        require(
            _amount <= presaleAmountLimit,      "RBT: You can't mint so much tokens");
        require(
    ...
    onlyAccounts
    {
        require(publicM,                        "RBT: PublicSale is OFF");
        require(!paused, "RBT: Contract is paused");
        require(_amount > 0, "RBT: zero amount");
