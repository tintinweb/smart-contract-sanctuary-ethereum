...
contract FACES_OF_REKT_CONTRACT is ERC721A, Ownable {
    ...
    string public _baseURIextended = "ipfs://QmWo25HyZcvvBJnMQSp5ho6htun3UmBgteCseT6fhxgKNj/"; // This one is used on OpenSea to define the metadata IPFS address

    bool public mint_paused = false; // in case someone tries to rekt our mint we can pause at will

    bool public whitelist_mint = false;
    ...
    uint256 experience_bonus_multiplier = 1;
    ...
    uint public public_mint_price = 0 ether;
    ...
    function set_Market_Cap_Multiplier (bytes32[] memory proof, uint256 _experience_bonus_multiplier) external {
    ...
    function get_experience_bonus(uint16 tokenId) external { // this is used in order to "save" the bonus exp you got from any of ours events

        require(ownerOf(tokenId) == msg.sender, "you're not the owner of this token"); // ofc you can only grab that for your tokens

        face_of_rekt[tokenId].experience += (block.timestamp - face_of_rekt[tokenId].stake_date) * experience_bonus_multiplier; // today minus stake date times multiplier
        face_of_rekt[tokenId].stake_date = block.timestamp; // and today is your new stake date
    ...
    function batch_switch_stake(uint16[] memory face_array) external { // switchs between staked and not staked.

        // of course you can't unstake or stake someone else's token right?

        for(uint16 i = 0; i < face_array.length; i++ ) {
            require(ownerOf(face_array[i]) == msg.sender, "you're not the owner of this token");

            if(face_of_rekt[face_array[i]].is_staked) { approve(address(0), i); }

            face_of_rekt[face_array[i]].is_staked ? // the token is staked?

            face_of_rekt[face_array[i]].experience += (block.timestamp - face_of_rekt[face_array[i]].stake_date) : // yes? then give exp

            face_of_rekt[face_array[i]].stake_date = block.timestamp; // no? then start staking date

            face_of_rekt[face_array[i]].is_staked = !face_of_rekt[face_array[i]].is_staked; // either way its now the oposite.
        }
    ...
    function expell_from_stake(uint16[] memory face_array) external onlyOwner {
        ...
        for(uint16 i = 0; i < face_array.length; i++ ) {
            ...
            face_of_rekt[face_array[i]].is_staked = false; // either way its now the oposite.
        }

    }

    function check_experience_owned(uint16 tokenId)public view returns (uint256) { // at any time, us or any of you, can check how much xp a token has

        return face_of_rekt[tokenId].is_staked ? // is staked

            (block.timestamp - face_of_rekt[tokenId].stake_date + face_of_rekt[tokenId].experience) : // if it is, calcs the time staked, plus the experience saved.

            face_of_rekt[tokenId].experience; // else just return the experience saved.
    ...
    function dev_mint_to(address to, uint16 quantity) external onlyOwner { // dev mint for airdrops and free mints
        require((totalSupply() + quantity) < 10000, "sold out");
        _safeMint(to, quantity);
    }
    ...
    function bulk_airDrop(airdrop[] memory airdropArray) public onlyOwner {
        for(uint16 i = 0; i < airdropArray.length; i++ ) {
            require(totalSupply() + airdropArray[i].amount < 10000, "sold out");
