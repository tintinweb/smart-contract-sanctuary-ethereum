...
contract EsgSHIP is owned, LendingWhiteList {
    ...
    function setInvitee(address inviteeAddress) public returns (bool) {
		...
		require(uint40(block.timestamp) <= end_time,"Time out!");
    	...
    	if(isWhitelisted(msg.sender)){
	    	...
	    	user.referrer_addr = inviteeAddress;
	    }else{
	    	uint256 amount = EIP20Interface(address(esg)).balanceOf(msg.sender);
	    	if(amount >= 1e21){
	    		EnumerableSet.add(es, inviteeAddress);
		    	user.referrer_addr = inviteeAddress;
		    }else{
    ...
    function getInviteelist(address referrerAddress) public view returns (address[] memory) {
    	require(referrerAddress != address(0), "inviteeAddress should not be 0x0.");
    ...
    function claim() public returns (bool) {
		...
		if(cp.deposit_time > 0)
		{
			if(block.timestamp <= end_time){
				...
				uint256 bonus = end_time.sub(cp.deposit_time).mul(cp.total_staked).mul(dayEsgRate).div(1e18).div(86400);
				amount = amount.add(bonus);
				cp.bonus_unclaimed = 0; 
				cp.deposit_time = end_time;
			}
		...
		if(total_deposited >= 1e23){
			...
			if(_referrer_addr != address(0)){
				...
				esg.transfer(msg.sender, amount.sub(incentive).sub(incentive_holder));
    		}else
	    	{
	    		esg.transfer(owner, incentive_holder);
		...
		}else
		{
			if(_referrer_addr != address(0)){
				incentive = amount.div(5);
				esg.transfer(_referrer_addr, incentive);
				esg.transfer(msg.sender, amount.sub(incentive));
    		}else
	    	{
	    		esg.transfer(msg.sender, amount);
	    	}
