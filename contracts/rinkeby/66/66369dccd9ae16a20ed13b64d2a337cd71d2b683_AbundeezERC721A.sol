...
// File: contracts/AbundeezERC721A.sol


pragma solidity ^0.8.12;

/// @author Hassan Karrach
/// @title Abundeez
...
contract AbundeezERC721A is ERC721A, Ownable {
    using Strings for uint256;
    ...
    uint256 public priceSale = 0.11 ether;
    ...
    //Check if the caller is not user
    modifier callerIsUser() {
        require(
            tx.origin == msg.sender,
            "Abundeez :: Cannot be called by a contract"
        );
        _;
    }
    ...
    function presaleMint(
        ...
        require(
            sellingStep == Steps.Presale,
            "Presale not started yet, please come back later!"
        );

        //Did this account already mint 2 NFTs ?
        require(
            totalPreSaleMint[msg.sender] + _ammount <= MAX_PRESALE_MINT,
            "You can only get 2 Abundeez during the Presale."
        );

        //Is this user on the whitelist ?
        require(
            isWhiteListed(msg.sender, _proof),
            "your address is not in the whitelist, please double-check."
        );

        //is there's enough Tokens?
        require(totalSupply() + _ammount <= MAX_SUPPLY, "Max supply exceeded");
        ...
        totalPreSaleMint[msg.sender] += _ammount;
        //Mint the user NFT
        _safeMint(_account, _ammount);
    }

    function saleMint(address _account, uint256 _ammount)
        external
        payable
        callerIsUser
    {
        ...
        require(msg.value >= price * _ammount, "Not enough funds.");

        //The user can only mint max 5 NFTs during the Public Sale
        require(
            totalPublicMint[msg.sender] + _ammount <= 5,
            "You can't mint more than 5 Abundeez"
        );

        //If the user try to mint any non-existent token
        require(
            numberNftSold + _ammount <= MAX_SUPPLY,
            "Sale is almost sold out, Try to decrease the quantity."
        );

        //Add the ammount of NFTs minted by the user to the total he minted
        totalPublicMint[msg.sender] += _ammount;

        //If this account minted the last Token
