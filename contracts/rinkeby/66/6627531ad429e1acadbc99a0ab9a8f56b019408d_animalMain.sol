...
contract animalMain is  ERC721,Ownable {
  ...
  function createAnimal() public payable GachaValue {
      ...
      bytes32 random_num = keccak256(abi.encodePacked(now,msg.sender,blockhash(block.number-2),block.difficulty));
      dna = random_num;
      star = uint(random_num)%10 + 1 ;
      roletype = uint(keccak256(abi.encodePacked(now,random_num))) % 4 + 1 ; // 1老虎  2狗  3獅子  4鳥
