...
contract TEST is Context, IERC20, Ownable {
    ...
    constructor(address mktg, address liq, address trsry) {
        address newOwner = msg.sender;
        marketingAddress = payable(mktg);
        liquidityAddress = payable(liq);
        treasuryAddress = payable(trsry);
        ...
        emit Transfer(ZERO, newOwner, _tTotal);
    }
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (!_isExcludedFromFee[from] && !_isExcludedFromFee[to]) {
            ...
            if (automatedMarketMakerPairs[from]) {
                _taxFee = _buyTaxFee;
                _liquidityFee = _buyLiquidityFee + _buyMarketingFee + _buyTreasuryFee;
                if(_liquidityFee > 0){
                    buyOrSellSwitch = BUY;
                }
            }
            // Sell
                ...
                _taxFee = _sellTaxFee;
                _liquidityFee = _sellLiquidityFee + _sellMarketingFee + _sellTreasuryFee;
                if(_liquidityFee > 0){
    ...
    function swapBack() private lockTheSwap {
        ...
        bool success;
        uint256 totalTokensToSwap = _liquidityTokensToSwap + _marketingTokensToSwap + _treasuryTokensToSwap;
    ...
    function sendETHToFee(uint256 amount) private {
        ...
        treasuryAddress.transfer(amount.div(2));
    }
    ...
    function setBuyFee(uint256 buyTaxFee, uint256 buyLiquidityFee, uint256 buyMarketingFee, uint256 buyTreasuryFee) external onlyOwner {
        _buyTaxFee = buyTaxFee;
        _buyLiquidityFee = buyLiquidityFee;
        _buyMarketingFee = buyMarketingFee;
        _buyTreasuryFee = buyTreasuryFee;
        require(_buyTaxFee + _buyLiquidityFee + _buyMarketingFee + _buyTreasuryFee <= 20, "Must keep taxes below 20%");
    }

    function setSellFee(uint256 sellTaxFee, uint256 sellLiquidityFee, uint256 sellMarketingFee, uint256 sellTreasuryFee) external onlyOwner {
        _sellTaxFee = sellTaxFee;
        _sellLiquidityFee = sellLiquidityFee;
        _sellMarketingFee = sellMarketingFee;
        _sellTreasuryFee = sellTreasuryFee;
        require(_sellTaxFee + _sellLiquidityFee + _sellMarketingFee + _sellTreasuryFee <= 30, "Must keep taxes below 30%");
    }
    ...
    function withdrawStuckETH() external onlyOwner returns (bool _sent) {
        (_sent,) = address(msg.sender).call{value: address(this).balance}("");
