...
contract EquityManager is Pausable, AccessControl, ReentrancyGuard {
    ...
    //EquityStake[] public equity_stake;
    ...
    uint256[] public initial_stakes;
    //uint256[] public current_equity;

    // mapping
    mapping(address => EquityStake) public current_stakes;
    //mapping(address => EquityStake) initial_stakes;
    ...
    function distributeFunds() public whenNotPaused nonReentrant {
        if (!contractUpdateWindow()) {
            ...
            //EquityStake[] memory currentStakes = ;
            EquityStake[] memory equityStakes = updateStake();
            for (uint256 i = 0; i < equityStakes.length; i++) {
                uint256 amount = fundsToDistribute
                    .mul(equityStakes[i].stakePercentage)
                    .div(100 * (10**18));
                equityStakes[i].lastReceived = block.timestamp;
    ...
    function totalStake(EquityStake[] memory _currentstakes)
        public
        view
        returns (uint256)
    {
        uint256 _totalStake = 0;
        for (uint256 i = 0; i < _currentstakes.length; i++) {
            _totalStake += _currentstakes[i].stakePercentage;
        }
        return _totalStake;
    }

    /*
     * @notice Internal function for updating equity stake
     * @notice It checks whether the total stake > 100
     */
    function updateStake() internal returns (EquityStake[] memory) {
        address[] storage currentStakeholders = current_stakeholders;
        EquityStake[] memory testEquityStake;
        for (uint256 i = 0; i < currentStakeholders.length; i++) {
            address _holder = currentStakeholders[i];
            testEquityStake[i] = current_stakes[_holder];
        }

        EquityStake[] memory maxEquityStake = testEquityStake;
        if (timePassed()) {
            for (uint256 i = 0; i < currentStakeholders.length; i++) {
                testEquityStake[i].stakePercentage += initial_stakes[i];
            }
            if (totalStake(testEquityStake) <= (100 * (10**18))) {
                emit EquityStakeUpdated(block.timestamp);
     ...
     */
    function addHolder(address _holder, uint256 _stake)
        public
        whenNotPaused
        onlyRole(MANAGER_ROLE)
    {
        ...
        if (totalStake(updateStake()).add(_stake) > (100 * (10**18))) {
        ...
        // If the new stakeholder added has been existing but his %equity is just being increased
        _stake = current_stakes[_holder].stakePercentage.add(_stake);
        current_stakes[_holder] = EquityStake(
            _stake,
            block.timestamp,
            block.timestamp
        );
        address[] memory holders = current_stakeholders;
        for (uint256 i = 0; i < holders.length; i++) {
            if (holders[i] == _holder) {
                initial_stakes[i] = _stake;
            }
        }
    ...
    function getEquityUpdateInterval() external view returns (uint256) {
        return i_interval;
    }
    ...
    function getfundsSentToI3() external view returns (uint256) {
        return cumfundsToI3;
    }
}
