...
abstract contract PublicMint {
    // Public Minting
    event PublicMintStatusChange(bool indexed enabled, uint256 time);
    ...
    function _setPublicMint(bool bool_, uint256 time_) internal {
        ...
        emit PublicMintStatusChange(bool_, time_);
    }
    modifier publicMintEnabled { 
        require(_publicMintEnabled && _publicMintTime <= block.timestamp, "Public Mint is not enabled yet!"); 
        _; 
    }
...
abstract contract WhitelistMint {
    // Whitelist Minting
    event WhitelistMintStatusChange(bool indexed enabled, uint256 time);
    ...
    function _setWhitelistMint(bool bool_, uint256 time_) internal {
        ...
        emit WhitelistMintStatusChange(bool_, time_);
    }
    ...
    modifier whitelistMintEnabled {
        require(_whitelistMintEnabled && _whitelistMintTime <= block.timestamp, "Whitelist Mint is not enabled yet!");
        _;
    } 
...
contract TunaFish is ERC721I_OW {
    ...
    iTunaSlime public TunaSlime;

    function _transfer(address from, address to, uint256 tokenId) internal override {
        ERC721I._transfer(from, to, tokenId);
        TunaSlime.updateReward(from, to);
    }
