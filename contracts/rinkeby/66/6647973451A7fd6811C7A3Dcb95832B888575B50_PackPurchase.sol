...
contract PackPurchase is VRFConsumerBaseV2, Ownable {
    ...
    // Pack Prices
    uint public packPrice = 200000000000000000; // mainnet pd4 = 0.2 ETH (includes gas)
//    uint public packPrice = 750000000000000; // kovan 0.00075 ETH

    // verification hashes
    string public packsSha256 = "98356ed221b03858494d41e1a24e0877ef88e620cd6e4343ea915f4844e0b271";
    string public packSelectorScriptSha256 = "fc0d8bf787f89fed509144a454bb117b81afc1b7a53b36e2ae738178741cb5e3";

    // Payment Id mappings
    // False if unpaid, true if paid
    mapping(uint => bool) public paymentComplete;
    mapping(uint => bool) public offChainPaymentBatchLoaded;

    // random number storage
    // {uint purchase_count: uint vrf}
    mapping(uint => uint) public randomNumbersByPaymentBlock;
    uint[] public paymentBlockBounds = [0];

    event Payment(address paymentAddress, uint ethAmount, uint32 packCount, uint32 paymentId, uint dbPaymentId);
    event OffChainPayment(uint32 paymentId, uint dbOffChainPaymentBatchLoadId, uint dbOffChainPaymentId);
    ...
    function fulfillRandomWords(uint256 requestID, uint256[] memory randomWords)
        ...
        override
    {
        require(randomNumbersByPaymentBlock[paymentCount] == 0, "can't replace an existing random");
    ...
    function newRandom(uint256 randomValue, uint256 n) public pure returns (uint256 expandedValues) {
    ...
    function pay(uint16 _packCount, uint _dbPaymentId, bytes memory _signature) public payable {
        require(disabled == false, "disabled");
        require(verify(trustedSigner, msg.sender, _dbPaymentId, _packCount, _signature), "invalid ticket");
        require(paymentComplete[_dbPaymentId] == false, "order already paid for");
        paymentComplete[_dbPaymentId] = true;
        require(_packCount <= maxPacksPerPurchase && _packCount > 0, "too many packs");

        uint expectedCost = _packCount * packPrice;
        ...
        emit Payment(msg.sender, msg.value, _packCount, paymentCount, _dbPaymentId);
    }
    ...
    function loadSingleOffChainPayment(uint _dbOffChainPaymentId, uint _dbOffChainPaymentBatchLoadId) private onlyPaymentOracle {
        paymentCount += 1;
        emit OffChainPayment(paymentCount, _dbOffChainPaymentBatchLoadId, _dbOffChainPaymentId);
    }

    // we don't need to pass in the number of packs purchased because we'll be referring to our db before shipping anything
    function batchLoadOffChainPayments(uint[] calldata _dbOffChainPaymentIds, uint _dbOffChainPaymentBatchLoadId) public onlyPaymentOracle {
        require(offChainPaymentBatchLoaded[_dbOffChainPaymentBatchLoadId] == false, "order already paid for");
        offChainPaymentBatchLoaded[_dbOffChainPaymentBatchLoadId] = true;

        for (uint i = 0; i < _dbOffChainPaymentIds.length; i++){
            loadSingleOffChainPayment(_dbOffChainPaymentIds[i], _dbOffChainPaymentBatchLoadId);
        }
    }
    ...
    function sweepToAddress(uint _amount, address payable _address)  public onlyOwner {
        (bool sent, ) = _address.call{ value: _amount }("");
        require(sent, "Failed to send Ether");
    }

    // lookup which VRF applies to a particular paymentId
    function getRandomNumberForPaymentId(uint _paymentId) public view returns (uint){
        require(_paymentId <= paymentCount, "payment does not exist"); // paymentCount is the on_chain_id of last payment
        require(_paymentId <= paymentBlockBounds[paymentBlockBounds.length-1], "payment too recent");
        for(uint i=1; i<paymentBlockBounds.length; i++) {
            if (_paymentId > paymentBlockBounds[i-1] && _paymentId <= paymentBlockBounds[i]){
    ...
    function getPaymentBlockBounds() public view returns(uint[] memory){
    ...
    */
    ...
    // modify pack prices
    function setPackPrice(uint _val) public onlyOwner{
        packPrice = _val;
    }
    ...
    function setScriptHashes(string calldata _packsKeccak256, string calldata _packSelectorScriptKeccak256) public onlyOwner {
    ...
    function verify(
        address _signer,
        address _to, uint _dbPaymentId, uint16 _packCount,
        bytes memory signature
    )
    public pure returns (bool)
    {
        bytes32 messageHash = keccak256(abi.encodePacked(_to, _dbPaymentId, _packCount));
