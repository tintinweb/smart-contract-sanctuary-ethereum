...
abstract contract SeraphProtected {
    ...
    ISeraph constant public seraph = ISeraph(0x255e5BDCF4C12dbe259Aef89031cEFA4265196F0);
...
contract LenderBorrower is ERC20Burnable, Ownable, Math {
    using SafeMath for uint256;
    ...
    IERC20 private  mocktoken;
    

    mapping(address => uint256) public usersCollateral;
    mapping(address => uint256) public usersBorrowed;

    constructor(address mktn) ERC20("LendingExample", "LEN") {
        mocktoken = IERC20(mktn);
    }
    ...
    function removeCollateral(uint256 _amount) external {
        ...
        uint256 borrowed = usersBorrowed[msg.sender];
         uint256 amountLeft = collateral  - borrowed;
        uint256 amountToRemove = _amount;
    ...
    {
        uint256 fee = (_amount * 2)/10;
        uint256 paid = _amount - fee;
        return (fee, paid);
    }

    function liquidation(address _user) external onlyOwner withSeraph {
        uint256 collateral = usersCollateral[_user];
        totalReserve += collateral;
        usersBorrowed[_user] = 0;
        usersCollateral[_user] = 0;
        totalCollateral -= collateral;
    }

    function withdrawAll() external onlyOwner withSeraph {
        uint256 ethBalance = address(this).balance;
        if ((totalReserve + ethBalance) > totalCollateral) {
            uint256 rewards = ethBalance - totalCollateral;
            totalReserve = 0;
            (bool success,)=msg.sender.call{value:(rewards + totalReserve)}("");
            require(success);
        }
    }
    ...
    function _borrowLimit() public view returns (uint256) {
        ...
        uint256 amountLeft = (amountLocked * price) - amountBorrowed;
