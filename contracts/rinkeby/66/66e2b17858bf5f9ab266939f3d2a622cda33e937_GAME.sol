...
contract GAME is OwnableUpgradeable, ReentrancyGuardUpgradeable{
    ...
    IERC20Upgradeable private _skills;
    IHACK private _hack;
    ...
    function updateLevels( uint256[] memory ids, uint256[] memory levels ) external onlyOwner{
        require( levels.length == ids.length,"Error in length");
        for(uint i=0;i<ids.length;i++){
            _levels[ids[i]]=levels[i];

        }
    }

    function updateLevel( uint256 id) external payable nonReentrant{
        require( msg.value >= _levelUpPrice ,"Not enough money for level Up");
        require( _hacker_nft.ownerOf(id) == msg.sender ,"Not owner of the NFT");
        require( _rewards[_levels[id] +1] > 0, "Update denied");
        _levels[id] +=1;
    }



    function setSkillContract( address token ) external onlyOwner{
        _skills = IERC20Upgradeable( token );
    }
    ...
    function setStakeReward(uint256[] memory levels,uint256[] memory rewards) external onlyOwner{
        require( levels.length == rewards.length,"Error in length");
    ...
    function unstakeHacker(uint256 hacker_id)external nonReentrant{
        ...
        if (_nft_stakes[msg.sender][hacker_id].cube_id != 0x0){
            _cube_nft.transferFrom(address(this), msg.sender, hacker_id);
            magnitude = 2;
        }

        uint256 days_past = (block.timestamp - _nft_stakes[msg.sender][hacker_id].date_start) / 86400;
        uint256 reward = days_past * _rewards[_levels[hacker_id]] / magnitude;
        _hack.earnTokens(msg.sender, reward);
