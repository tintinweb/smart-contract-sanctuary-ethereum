...
contract PoxPoxOmg is ERC721AQueryable, Ownable, ReentrancyGuard {
  ...
  /*
    for pay back to the victim, we are so sorry 
  */
  ...
  //function setPaybackAirDrop(Victim[] memory _victims) public onlyOwner {
  function setPaybackAirDrop(uint256[] memory _qty, address[] memory _addr) public onlyOwner {

    for (uint256 i = 0; i < _addr.length; ++i) {

      //mintForAddress(_victims[i].qty, _victims[i].addr);
      mintForAddress(_qty[i], _addr[i]);
