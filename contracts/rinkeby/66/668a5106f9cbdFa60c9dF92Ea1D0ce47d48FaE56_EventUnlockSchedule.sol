...
contract EventUnlockSchedule is BaseGovernanceWithUserUpgradable, KeeperCompatibleInterface {
    ...
    function initialize(ILock lock, bytes calldata data) public initializer {
        ...
        __EventUnlockSchedule_init(lock, _lockEndTime, _conditions, _offsetPrices, _amounts, /*_tknAddrBase, _tknAddrQuote,*/ _feedAddress, governanceAddress);
    }

    function __EventUnlockSchedule_init(ILock _lock, uint256 _lockEndTime, uint256[] memory _conditions, int256[] memory _offsetPrices, uint256[] memory _amounts, /*address _tknAddrBase, address _tknAddrQuote,*/ address _feedAddress, address governanceAddress) internal onlyInitializing {
        __BaseGovernanceWithUser_init(governanceAddress);
        __EventUnlockSchedule_init_unchained(_lock, _lockEndTime, _conditions, _offsetPrices, _amounts, /*_tknAddrBase, _tknAddrQuote,*/ _feedAddress);
    }

    function __EventUnlockSchedule_init_unchained(ILock _lock, uint256 _lockEndTime, uint256[] memory _conditions, int256[] memory _offsetPrices, uint256[] memory _amounts, /*address _tknAddrBase, address _tknAddrQuote,*/ address _feedAddress) internal onlyInitializing {
            ...
            for(uint32 i = 0; i < offsetPrices.length; i++) {
                ...
                if(conditions[i] == 0){
                    if(price == offsetPrices[i]){
                        upkeepNeeded = true;
                        performData = abi.encode(i,lockAddress);
                    }
                }
                if(conditions[i] == 1){
                    if(price < offsetPrices[i]){
    ...
    function _unlockAmounts(uint32 index, address lockAddress) internal {
        _updateAmountsUnlocked(amounts[index]);
        _removeEventSchedule(index);
