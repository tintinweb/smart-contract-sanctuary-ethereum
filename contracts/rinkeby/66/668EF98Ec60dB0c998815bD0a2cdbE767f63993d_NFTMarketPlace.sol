...
error NFTMarketPlace__NoFundAvailable();
...
contract NFTMarketPlace {
    ...
    modifier contractIsApproved(address nftAddress, uint256 tokenId) {
        IERC721 nft = IERC721(nftAddress);
    ...
    function listItem(
        ...
        contractIsApproved(nftAddress, tokenId)
    ...
    function withdraw() external payable {
        ...
        if (proceed <= 0) {
            revert NFTMarketPlace__NoFundAvailable();
        }

        s_proceeds[msg.sender] = 0;
        (bool success, ) = msg.sender.call{value: proceed}('');
