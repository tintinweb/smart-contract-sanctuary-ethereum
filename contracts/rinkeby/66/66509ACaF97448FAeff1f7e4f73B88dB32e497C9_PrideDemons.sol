    ...
    ReentrancyGuard
{
    ...
    bool private beneficiaryAssigned = false;

    bytes32 public keyHash;    

    uint256 public legendaryReserved = 0;
    ...
    mapping(uint256 => uint256) private _metadataId;
    mapping(address => uint256) private _nextTxnAllowBlock;
        ...
        VRFConsumerBase(chainlink.coordinator, chainlink.linkToken)
    {
        ...
        legendaryReserved = _legendaryReserved;
    }
        ...
        returns (bool)
    {
        ...
        if (
            ...
            require(
                _nextTxnAllowBlock[msg.sender] == 0 ||
                _nextTxnAllowBlock[msg.sender] <= block.number,
            ...
            _nextTxnAllowBlock[msg.sender] = block.number.add(delayReentryBlock);
    ...
    function setLegendaryReserved(uint256 _legendReserved) external onlyOwner {
        legendaryReserved = _legendReserved;
    }
    ...
    function getMetadata(uint256 tokenId) public view returns (string memory) {
        ...
        return Strings.toString(_metadataId[tokenId]);
    }
    ...
    function getReentryBlock(address addr) external view returns (uint256) {
        return _nextTxnAllowBlock[addr];
    }
    ...
    function fulfillRandomness(bytes32 requestId, uint256 randomNumber)
        ...
        override
    {
        uint256 randomSeed = (uint256(keccak256(abi.encode( requestId, randomNumber))) % (maxSupply)) + 1;
        seed = randomSeed;
        emit RandomseedFulfilment(block.timestamp, requestId, seed, randomNumber);
        ...
        for (uint256 i = legendaryReserved + 1; i <= maxSupply; i += 1) {
            uint256 j = (uint256(keccak256(abi.encode(seed, i))) % (maxSupply)) + 1;

            if(j >= legendaryReserved + 1 && j <= maxSupply) {
                (metadata[i], metadata[j]) = (metadata[j], metadata[i]);
            }
        }

        for (uint256 i = 1; i <= maxSupply; i += 1) {
            _metadataId[i] = metadata[i];
...
contract BlockBasedSale is Ownable {
    ...
    function setTransactionLimit(uint256 privateSaleLimit,uint256 publicSaleLimit, uint256 maxWhitelist, uint256 maxPublic) external onlyOwner {
        ...
        require(maxWhitelist > 0, "max white list limit is 0");
        require(maxPublic > 0, "max white list limit is 0");
