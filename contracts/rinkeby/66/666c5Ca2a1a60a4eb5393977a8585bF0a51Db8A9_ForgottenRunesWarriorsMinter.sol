...
contract ForgottenRunesWarriorsMinter is Ownable, Pausable, ReentrancyGuard {
    ...
    mapping(address => uint256) public numClaimedByAddress;
     ...
     * @param numWarriors uint256 of the amount you want to claim in this transaction
     * @param numMaxEligibleStatement uint256 your statement of how many max you should be allowed. Obviously we can't trust this outright and so this is confirmed as part of the leaf in the merkle tree
     * @param _merkleProof bytes32[] the proof that you're eligible to mint here
     */
    function claimSummon(
        uint256 numWarriors,
        uint256 numMaxEligibleStatement,
        bytes32[] calldata _merkleProof
    ) external nonReentrant whenNotPaused {
        require(numClaimed < maxForClaim, 'No more claims');
        require(
            numClaimed + numWarriors <= maxForClaim,
            'Not enough remaining to claim'
        );
        require(
            numWarriors > 0 && numWarriors <= 20,
            'You can claim no more than 20 Warriors at a time'
        );
        require(
            numMaxEligibleStatement > 0 &&
                numMaxEligibleStatement <= numWarriors,
            'Invalid eligible statment'
        );
        require(
            numClaimedByAddress[msg.sender] + numWarriors <=
                numMaxEligibleStatement,
            'You cant claim this many'
        );
        numClaimedByAddress[msg.sender] += numWarriors;

        // verify allowlist
        bytes32 node = keccak256(
            abi.encodePacked(msg.sender, 'warriors', numMaxEligibleStatement)
        );
        require(
    ...
    function getElapsedDaTime() public view returns (uint256) {
    ...
    function selfRefund() public nonReentrant {
        require(
            block.timestamp > selfRefundsStartTime,
            'Self refund period not started'
        );
