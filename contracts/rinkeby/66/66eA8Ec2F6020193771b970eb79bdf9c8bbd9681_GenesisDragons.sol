...
contract GenesisDragons is ERC721, ERC1155Holder, Ownable, ReentrancyGuard  {
    bytes32 public root;
    address proxyRegistryAddress;
    string public baseURI = "ipfs://QmXtJDNXFvHSGTAwVHekYRNb9tZQFcmD8TtQAkzTHmgZrn/";
    ...
    //trigger event when user transfers from OS
    ...
    function Migrate(address account, uint256[] calldata idxsToClaim,
		uint256[] calldata idsOfOwner,bytes32[] calldata _proof)
    external
    payable
    ...
    onlyAccounts
    {
        for (uint256 i; i < idxsToClaim.length; i++) {
			...
			_claimReservedToken(account, tokenId);
            idsMigrated.push(tokenId);
		}
