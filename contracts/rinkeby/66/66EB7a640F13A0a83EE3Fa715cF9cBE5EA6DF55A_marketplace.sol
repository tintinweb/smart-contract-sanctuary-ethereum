...
interface IStake {
    function getXpEarned(address account) external view returns(uint256 xp);
}
...
contract marketplace is Ownable {
    ...
    constructor(address token_, address stakeAddress_, uint decimals_) {
      token = token_;
    ...
    function createMarketItem(
        ...
        require(levelToXP[minLevel] > 0 || minLevel == 0, "Level not initialized yet");
        _itemIds.increment();
    ...
    function getExperience(address account) public view returns (uint) {
        return IStake(stakeAddress).getXpEarned(account);
    }

    function buyItem(address nftContract, uint256 itemId)
        external
    {
        require(getExperience(msg.sender) >= levelToXP[idToMarketItem[itemId].levelRequired],"Earn more xp to be able to buy this item");
    ...
    function setLevelToXP(uint24 level, uint256 xp) public onlyOwner {
        levelToXP[level] = xp;
    }

    function getXpForLevel(uint24 level) external view returns(uint) {
