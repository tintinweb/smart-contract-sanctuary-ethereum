...
contract PoolVeBonus is IJellyContract {    
    ...
    function claimAndLock(uint _tokenId) external returns (uint) {
        ...
        uint256 amountBefore = getTokenRewardsClaimed(poolAddress, poolToken, _tokenId);
        IJellyPoolWrapper(poolAddress).fancyClaim(_tokenId);
        uint256 amountAfter = getTokenRewardsClaimed(poolAddress, poolToken, _tokenId);

        uint256 amountClaimed = amountAfter - amountBefore;
        require(amountClaimed > 0);
        rewardsLocked[_tokenId] += amountClaimed;
        require(rewardsLocked[_tokenId] <= amountAfter, "Locking more tokens than has been earnt.");
        uint256 tokensAdded = amountClaimed * (bonusPercentage + PERCENTAGE_PRECISION) / uint256( PERCENTAGE_PRECISION);
    ...
    function lockWithBonus(uint _tokenId, uint256 _amount) public returns (uint) {
        require(IERC721(poolAddress).ownerOf(_tokenId) == msg.sender, "Must own tokenId");
        require(_amount > 0);
