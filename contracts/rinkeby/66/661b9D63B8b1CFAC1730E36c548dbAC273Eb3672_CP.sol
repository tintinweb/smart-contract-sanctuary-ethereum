    ...
    ReentrancyGuard
{
    ...
    mapping(uint256 => bool) private intuitivePoems;
    mapping(uint256 => bool) private puzzlePoems;
    ...
    function seedIntuitivePoems(uint256[] memory poems) external onlyOwner {
        require(
            poems.length == amountForIntuitives,
            "Not enough tokens to set"
        );
        require(!intuitivePoemsSet, "Intuive Poems have already been set");
        for (uint256 i = 0; i < poems.length; i++) {
            intuitivePoems[poems[i]] = true;
        }
        intuitivePoemsSet = true;
    }

    function setIntuitivePoemClaimed(uint256 tokenId) external onlyOwner {
        require(
            tokenId >= 0 && tokenId < collectionSize,
            "Token is out of bounds"
        );
        intuitivePoems[tokenId] = false;
    }

    function isIntuitivePoem(uint256 tokenId) external view returns (bool) {
        require(
            tokenId >= 0 && tokenId < collectionSize,
            "Token is out of bounds"
        );
        return intuitivePoems[tokenId];
    }

    function seedPuzzlePoems(uint256[] memory poems) external onlyOwner {
        require(poems.length == amountForPuzzles, "Not enough tokens to set");
        require(!puzzlePoemsSet, "Puzzles have already been set");
        for (uint256 i = 0; i < poems.length; i++) {
            puzzlePoems[poems[i]] = true;
        }
        puzzlePoemsSet = true;
    }

    function setPuzzleClaimed(uint256 tokenId) external onlyOwner {
        require(
            tokenId >= 0 && tokenId < collectionSize,
            "Token is out of bounds"
        );
        puzzlePoems[tokenId] = false;
    }

    function isPuzzlePoem(uint256 tokenId) external view returns (bool) {
        require(
            tokenId >= 0 && tokenId < collectionSize,
            "Token is out of bounds"
        );
        return puzzlePoems[tokenId];
    }
