...
contract QuantumPFP is Initializable, UUPSUpgradeable, ERC721Upgradeable, OwnableUpgradeable, Namehash {
  ...
  function addEmployee(string calldata _initial, address targetAddress, string calldata email, string calldata designation, string calldata specialty) external onlyOwner {

    EmployeeList[historicalEmployeeCount] = Employee("", Designations.Unknown, _initial, "", false);

    bytes32 rootNodeId = computeNamehash(abi.encodePacked(baseDomain));
    Resolver rootResolver = ens.resolver(rootNodeId);
    ens.setSubnodeRecord(computeNamehash(abi.encodePacked(baseDomain)), keccak256(abi.encodePacked(_initial)), ens.owner(rootNodeId), address(rootResolver), 0);
  ...
  function updateEmployeeData(uint tokenId, string memory email, string calldata designation, string memory specialty, address to) external onlyOwner {
    ...
    if (to != resolver.addr(nodeId)) resolver.setAddr(nodeId, to);
  }
  
  function cleanEmployeeRecord(uint tokenId) external onlyOwner {
    require(_exists(tokenId), "Nonexistent token");
    bytes32 rootNodeId = computeNamehash(abi.encodePacked(baseDomain));
    Resolver rootResolver = ens.resolver(rootNodeId);
    ens.setSubnodeRecord(rootNodeId, keccak256(abi.encodePacked(EmployeeList[tokenId].initial)), ens.owner(rootNodeId), address(rootResolver), 0);
  }
  ...
  function getEmployeeMetaData(uint tokenId) external view returns(string[3] memory data) {
    require(_exists(tokenId), "Nonexistent token");
    bytes32 nodeId = computeNamehash(abi.encodePacked(EmployeeList[tokenId].initial,  ".", baseDomain));
    Resolver resolver = ens.resolver(nodeId);
    ...
    return [ email, specialty, designation];
  }
}
