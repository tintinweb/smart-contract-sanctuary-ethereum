...
contract NFTMarketplace is Pausable, FeeManager, ERC721Holder {
    ...
    function getOrderTokenIds(address _owner)public view returns(uint256 orderId)
    {
         uint256 countt;
         Order memory order ;
            for (uint256 index = 0; index < orderrID.length; index++) {
            uint256 aa=orderrID[index];
    ...
    function getAuctionTokenIds(address _owner)public view returns(uint256 auctionId)
    {
         uint256 countt;
         AuctionData memory auctionData;
            for (uint256 index = 0; index < auctionnID.length; index++) {
            uint256 aa=auctionnID[index];
    ...
    function orderTokensOfOwner(address _owner)public view returns(uint256[]memory OrderId)
    {
        uint indexx=0;
        Order memory order ;
        uint256 countt=getOrderTokenIds(_owner);
       uint256[] memory resultOrder=new uint256[](countt);
        for (uint256 index = 0; index < orderrID.length; index++) {
           ...
           order = orderByTokenId[aa];
            if(order.seller==_owner)
            {
                ...
                resultOrder[indexx]=orderrID[index];
                indexx++;
       ...
       function auctionTokensOfOwner(address _owner)public view returns(uint256[]memory AuctionId)
    {
        AuctionData memory auctionData;
        uint256 cc=getAuctionTokenIds(_owner);
       uint256[] memory resultAuction=new uint256[](cc);
        for (uint256 index = 0; index < auctionnID.length; index++) {
    ...
    function tokensOfOwner(address _owner)public view returns (uint[]memory TokensOfOwner)
    {
