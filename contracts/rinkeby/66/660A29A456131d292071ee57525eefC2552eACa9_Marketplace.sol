...
contract Marketplace is Ownable, ReentrancyGuard, IERC777Recipient {
    string private name; // conctract name
    uint16 private totalSupply; // number of NFTs in circulation
    uint16 private royalty; // royalty percentage (expressed in tenthousandths 0-10000, this gives two decimal resolution)

    address private tokenContractAddress; // ERC721 NFT contract address
    IERC721 private tokenContract; // ERC721 NFT token contract

    address private dustTokenAddress; // ERC777 NFT token address (DUST)
    IERC777 private dustContract; // DUST ERC777 NFT token contract (DUST)

    address payable private splitterContractAddress; // Splitter contract for splitting royalty address
    ...
    event ModifyOfferValue(
        address _from,
        uint16 _tokenId,
        uint256 _value,
        bool _isDust
    );
    event ModifyOfferSellTo(
        address _from,
        uint16 _tokenId,
        address _sellOnlyTo,
        bool _isDust
    );
    event ModifyOfferBoth(
        address _from,
        uint16 _tokenId,
        uint256 _value,
        address _sellOnlyTo,
    ...
    constructor(
        ...
        splitterContract = Splitter(_splitterContractAddress); // initialize the Splitter contract

        dustTokenAddress = _dustContractAddress;
        dustContract = IERC777(_dustContractAddress);
        permitted[msg.sender] = true; // deployer is going to be permitted to change royalty
    ...
    function tokensReceived(
        ...
        require(msg.sender == dustTokenAddress, "Invalid token!");
    ...
    function modifyEtherOfferValue(uint16 _tokenId, uint256 _value)
        external
        onlyCardOwner(_tokenId)
    {
        ...
        emit ModifyOfferValue(msg.sender, _tokenId, _value, false);
    }

    function modifyEtherOfferSellOnlyTo(uint16 _tokenId, address _sellOnlyTo)
        external
        onlyCardOwner(_tokenId)
    {
        Offer memory offer = cardsForSaleInETH[_tokenId];
        ...
        // emit modification event
        emit ModifyOfferSellTo(msg.sender, _tokenId, _sellOnlyTo, false);
    }
    ...
    function modifyEtherOffer(
        ...
        emit ModifyOfferBoth(msg.sender, _tokenId, _value, _sellOnlyTo, false);
    }

    function modifyDustOfferValue(uint16 _tokenId, uint256 _value)
        external
        onlyCardOwner(_tokenId)
    {
        ...
        // emit modification event
        emit ModifyOfferValue(msg.sender, _tokenId, _value, true);
    }

    function modifyDustOfferSellOnlyTo(uint16 _tokenId, address _sellOnlyTo)
        external
        onlyCardOwner(_tokenId)
    {
        Offer memory offer = cardsForSaleInDust[_tokenId];
        ...
        // emit modification event
        emit ModifyOfferSellTo(msg.sender, _tokenId, _sellOnlyTo, true);
    }
    ...
    function modifyDustOffer(
        ...
        emit ModifyOfferBoth(msg.sender, _tokenId, _value, _sellOnlyTo, true);
    }
    ...
    function bidOnCardWithDust(uint16 _tokenId, uint256 _bidValue) external {
        require(
            _tokenId < totalSupply,
            "Token ID should be smaller than total supply."
        );
    ...
    function acceptDustBid(uint16 _tokenId) external onlyCardOwner(_tokenId) {
        Bid memory bid = dustBids[_tokenId];
    ...
    function getSplitterContractAddress() public view returns (address) {
        return splitterContractAddress;
    }
    ...
    function getDustOfferSellOnlyToAddressForCard(uint16 _tokenId)
        public
        view
        returns (address)
    {
...
contract Splitter is IERC777Recipient, Ownable {
    ...
    function setCompanyShares(uint256 _shares) public onlyOwner {
    ...
    function getCommunityShares() public view returns (uint256) {
    ...
    function getArtistShares() public view returns (uint256[] memory) {
        return artistShares;
    }

    function getCommunityOwnerAddress() public view returns (address) {
    ...
    function getArtistAddresses() public view returns (address[] memory) {
