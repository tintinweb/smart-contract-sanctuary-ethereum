    ...
    UUPSUpgradeable
{
    ...
    function __QuantumSpaces_init(
        ...
        __ERC721A_init("QuantumSpaces", "QSPACE");
        __Ownable_init();
        __Manageable_init();
        __Pausable_init();
    ...
    /// @notice set the maximum number of tokens in the drop
    /// @param dropId The drop id
    /// @param maxSupply maximum items in the drop
    function setDropMaxSupply(uint128 dropId, uint128 maxSupply)
        public
        onlyRole(MANAGER_ROLE)
    {
    ...
    */
    ...
    // /// @notice tops up gas refund store to max level
    // /// @dev Sales platform only
    // function topUpGasRefunds() public onlyRole(MINTER_ROLE) {
    //     _topUpGasRefunds();
    // }

    // /// @notice Increase the maximum number of refunds used when topping up
    // /// @dev Sales platform only
    // /// @param max number to set the maximum gas refunds
    // function setMaxGasRefunds(uint256 max) public onlyRole(MINTER_ROLE) {
    //     _maxGasRefunds = max;
    // }

    // /// @notice Increase the maximum number of refunds used when topping up
    // /// @dev Sales platform only
    // function getAvailableGasRefunds()
    //     public
    //     view
    //     onlyRole(MINTER_ROLE)
    //     returns (uint256)
    // {
    //     return _availableGasRefunds;
    // }
    ...
    IERC721MetadataUpgradeable
{
    ...
    // Addresses Lookup
    mapping(uint160 => address) private _addressLookup;
    ...
    uint256 private constant _baseIdMask = ~uint256(type(uint160).max);
    ...
    // function _tokenId(uint128 dropId, uint128 mintId)
    //     internal
    //     pure
    //     returns (uint256 tokenId)
    // {
    //     tokenId |= uint256(dropId) << 128;
    //     tokenId |= uint256(mintId);

    //     return tokenId;
    // }

    // function _startTokenId(uint256 tokenId) internal pure returns (uint256) {
    //     return tokenId & _baseIdMask;
    // }

    // function _dropIdOf(uint256 tokenId) internal pure returns (uint128) {
    //     return uint128(tokenId >> 128);
    // }

    // function _mintIdOf(uint256 tokenId) internal pure returns (uint128) {
        ...
        returns (TokenOwnership memory)
    {
        ...
        uint256 minted = _minted[tokenId.dropId()];
        if (minted < 1) minted = 1;

        unchecked {
            if (tokenId.firstTokenInDrop() <= curr && curr < minted - 1) {
    ...
    function _exists(uint256 tokenId) internal view returns (bool) {
        return
            ...
            tokenId.mintId() < _minted[tokenId.dropId()] - 1 &&
    ...
    function _mint(
        ...
        if (_minted[dropId] == 0) _minted[dropId] = 1; //If drop is not preallocated
        uint256 startTokenId = TokenId.from(dropId, _minted[dropId] - 1);
        ...
        unchecked {
            ...
            uint128 updatedIndex = _minted[dropId] - 1;
            ...
            if (safe && to.isContract()) {
                ...
                if (_minted[dropId] - 1 != startTokenId.mintId()) revert();
            } else {
                do {
                    uint256 tokenId = TokenId.from(dropId, updatedIndex++);
                    // console.log(tokenId);
    ...
    function _preAllocateTokens(uint128 dropId, uint128 quantity) internal {
        ...
        unchecked {
            ...
            uint256 startTokenId = TokenId.from(dropId, _minted[dropId] - 1);
            uint256 endTokenId = TokenId.from(
                dropId,
                _minted[dropId] + quantity - 2
            );
    ...
    function _transfer(
        ...
        unchecked {
            ...
            if (nextSlot.addr == address(0)) {
                ...
                if (nextTokenId.mintId() != _minted[tokenId.dropId()] - 1) {
                    nextSlot.addr = from;
    ...
    function _burn(uint256 tokenId, bool approvalCheck) internal virtual {
        ...
        unchecked {
            ...
            if (nextSlot.addr == address(0)) {
                // This will suffice for checking _exists(nextTokenId),
                // as a burned slot cannot contain the zero address.
                if (nextTokenId.mintId() < _minted[dropId] - 1) {
                    nextSlot.addr = from;
                    nextSlot.startTimestamp = prevOwnership.startTimestamp;
                }
            }
