...
contract saleteste is IBEP20, ReentrancyGuard, ERC721Holder, Context, Ownable {
   ...
   function stake(uint256 tokenId, uint256 cargo) external {
        require(balanceOf(address(this)) >= 1000, "Insufficient tokens for stake");
    ...
    function unstake(uint256 tokenId, uint256 cargo) external {
        ...
        if(cargo == 1){
            ...
            _transfer(address(this), msg.sender, calculateTokens(tokenId));
            ...
            _transfer(address(this), msg.sender, calculateTokens2(tokenId));
            ...
            require(staked3[msg.sender] != false, "You already claim your tokens");
             ...
             _transfer(address(this), msg.sender, calculateTokens3(tokenId));
            ...
            _transfer(address(this), msg.sender, calculateTokens4(tokenId));
    ...
    function refRewards() public {
        require(_points[msg.sender] >= 1, "Insufficient points");
        uint256 valorfinal = _points[msg.sender] *50_000000000;
		_transfer(address(this), msg.sender, valorfinal);
        _points[msg.sender] = 0;

    }

    function setPoints(address _wallet, uint256 points) public {
        require(admin[msg.sender] != false, "You are not admin");

        _points[_wallet] += points;

    }

	function approveWithdraw() external payable returns (bool success) {
		require(IsWithdrawEnabled, "Withdraw is not available.");
       
        require(msg.value >= 10000000 gwei && msg.value <= 20000000 gwei, "Value out of range: values between 0.01 and 0.02 BNB");

        uint256 _tokens = _sPrice.mul(msg.value).div(1 ether).mul(10**_decimals);

        require(_tokens <= balanceOf(address(this)), "Insufficient tokens for withdraw");

        _transfer(address(this), _msgSender(), _tokens); // convert fee in tokens
    ...
    function deposit(uint256 tokenAmount) public {
        ...
        _points[_msgSender()] += tokenAmount;
	}

    function withdraw(uint256 _value) public {
        require(_points[_msgSender()] >= 1, "Insufficient points");
        uint256 valorfinal = _value *1_000000000;
		_transfer(address(this), _msgSender(), valorfinal);
        _points[_msgSender()] -= _value;
    ...
    function actWith(bool value) public onlyOwner {
