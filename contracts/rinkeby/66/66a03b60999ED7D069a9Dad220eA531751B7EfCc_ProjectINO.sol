...
contract ProjectINO is ERC721A, Ownable {
  ...
  mapping(address => bool) public inolist;

  /*///////////////////////////////////////////////////////////////
                        PROJECT INFO
    //////////////////////////////////////////////////////////////*/

  uint256 private constant INO_SUPPLY = 7777;
  uint256 private constant PRICE_INO = 0.077 ether; 
  uint256 private constant RESERVED_INO = 200;
  uint256 private constant INO_TXN = 2;

  /*///////////////////////////////////////////////////////////////
                        TRACKING
    //////////////////////////////////////////////////////////////*/

  uint256 public publicRemaining = 3999;
  bytes32 public inoListMerkleRoot;
  bytes32 public pMerkleRoot;

  mapping(address => bool) public inoListPurchased;
  mapping(address => bool) public publicRafflePurchased;
  ...
  function setMerkleRoots(bytes32 _inoListMerkleRoot, bytes32 _pMerkleRoot) external onlyOwner {
    inoListMerkleRoot = _inoListMerkleRoot;
    pMerkleRoot = _pMerkleRoot;
  }
  ...
  function inoListMint(uint256 _amount, bytes32[] memory _proof) external payable callerIsUser {
      ...
      require(
          MerkleProof.verify(_proof, inoListMerkleRoot, keccak256(abi.encodePacked(msg.sender)))
          );
      require(
        _amount <= INO_TXN,
  ...
  function initialPublicMint (uint256 _amount) external payable callerIsUser {
    require(
        saleConfig == SaleConfig.PUBLIC || publicRemaining - _amount >= 0,
    ...
    require(
        totalSupply() + _amount <= INO_SUPPLY,
         ...
         publicRemaining -= _amount;
    _mint(msg.sender, _amount);
  }

  function publicRaffle (uint256 _amount, bytes32[] memory _proof) external payable callerIsUser {
      require(
          saleConfig == SaleConfig.PUBLICRAFFLE,
           "DEAR SPECIAL ONE, ARRIVAL ON PLANET IS NOT ALLOWED. TRY AGAIN SOON.");
      require(
          MerkleProof.verify(_proof, pMerkleRoot, keccak256(abi.encodePacked(msg.sender)))
          );
      require(
          _amount <= INO_TXN,
          "QUANTITY SURPASSES PER-TXN LIMIT");
      require(
          !publicRafflePurchased[msg.sender],
           "INO LIST HAS BEEN USED, TRY AGAIN ON PUBLIC.");
      publicRafflePurchased[msg.sender] = true;
      require(
          msg.value == PRICE_INO * _amount,
           "INCORRECT ETH SENT");
      require(
