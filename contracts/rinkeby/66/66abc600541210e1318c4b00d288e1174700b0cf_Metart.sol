...
contract Metart is ERC721,Ownable{
   ...
   // uint256 public REVEAL_TIMESTAMP;
    ...
    /*
    function salesPlan(uint16 index) public view returns(uint256,uint16,uint16,bool){
       return _salesCfg.at(index);
    }
    */
    ...
    function openSecondSale() public onlyOwner {
    ...
    */
    ...
    function _checkTransfer(address from,   uint256 tokenId) internal {
        require(_isApprovedOrOwner(_msgSender(), tokenId), "ERC721: transfer caller is not owner nor approved");
        require(secondSale,"Not transferable for the time being");
        require(!blackList.contains(from),"The address is in the blacklist");
        require(pledegeOf(tokenId) == address(0),"the NFT is currently pledged");
    ...
    function  mintPreSales(address[] memory _addresss) public onlyOwner{
        ...
        /*
        if (totalSupply().sub(indexReserveSale).add(startOvertSale).add(_addresss.length) == MintPreSales_Max) {
             _salesCfg.setActiveSale(1);
             emit SetActiveSalePlan(1);
        }
        */
    }
    /**
    ...
    */
    function mintMetart(uint numberOfTokens) public payable {
        require(saleIsActive, "Sale must be active to mint Metart");
        require(numberOfTokens <= maxMintCoinsSingle, "Can only mint 20 tokens at a time");
        //atTokenid//
        ...
        //(uint256 price, ,uint16 tokenEnd,bool sales,uint index) = _salesCfg.atTokenid(mintToken);
        //require(sales,"The current batch is not available for sale");   
        //require(mintToken.add(numberOfTokens)<=tokenEnd,"Can't cross phase mint");
       ...
       // uint price = 12;
        ...
        /*
        if (mintToken.add(numberOfTokens)==tokenEnd && index <_salesCfg._entries.length){
            _salesCfg.setActiveSale(index);
             emit SetActiveSalePlan(index);
        }
        */
    }
