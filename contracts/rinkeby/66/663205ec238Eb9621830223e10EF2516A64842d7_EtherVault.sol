...
contract EtherVault is Ownable{
    ...
    struct Vault{
        uint256 etherCollateral; //in ether, not wei 
        uint256 tokenDebt;
    }
    ...
    function depositEther(uint256 etherDeposit) payable external{ //value should be received in ether, not wei
        require(etherDeposit*(10**18) == msg.value, "wrong amount of ether sent");
        uint256 tokenToMint = etherDeposit * getEthUSDPrice(); // 1*1000
        token.mint(msg.sender, tokenToMint*(10**18)); //18 decimals
    ...
    function withdrawEther(uint256 etherToWithdraw) external{ //value is received in ether, not wei
        ...
        payable(msg.sender).transfer(etherToWithdraw*(10**18));
    ...
    function getEthUSDPrice() public view returns(uint){ //say 1000
        (,int price,,,) = priceFeed.latestRoundData();  
        return uint(price/(10**8)); // cause USD pair has 10^8 decimals, removing em
    }
}
