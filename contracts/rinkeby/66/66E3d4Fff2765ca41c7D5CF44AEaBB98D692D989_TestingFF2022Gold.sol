...
contract TestingFF2022Gold is ERC721A, Ownable, ReentrancyGuard {
    ...
    constructor (string memory bURI, uint256 mintCost) 
    ERC721A("Dr.Strange AMC Box Office Release test", "AMCT", 1200, 10000000){
        BaseURI = bURI;
        MintCost = mintCost;
    }
    ...
    */
    ...
    function airDrop(address[] memory recipients, uint256[] memory numberOfSets) public onlyOwner {
        require(recipients.length == numberOfSets.length, "Invalid Params");
        require(totalSupply() + recipients.length <= Supply, "Quantity exceeds supply");
        for (uint256 i = 0; i < recipients.length; i++){
            require(numberOfSets[i] <= maxBatchSize, "Only mint up to batchsize");
            _safeMint(recipients[i], numberOfSets[i]);   
        }
    }
    ...
    */
    ...
    function mintNFT(uint256 quantity) external payable {
        ...
        require(msg.value >= MintCost*quantity, "Incorrect value sent");
        require(totalSupply() + quantity <= Supply, "Quantity exceeds supply");   
