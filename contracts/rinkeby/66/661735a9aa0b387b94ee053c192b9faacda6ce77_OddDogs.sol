...
contract OddDogs is Ownable, ERC721A, ReentrancyGuard {
    
    // Token/Mint data
    uint256 public mMaxTokenSupply = 8888;
    uint256 public mCapPerTxPublic = 20;
    uint256 public mCapPerTxPresale = 3;
    uint256 public mWalletTokenCapPublic = 100;
    uint256 public mWalletTokenCapPresale = 3;
    uint256 public mPublicPrice = 0.08 ether;
    uint256 public mPresalePrice = 0.08 ether;
    bool public mPublicSaleIsActive = false;
    bool public mPresaleIsActive = false;
    string private mBaseURI = "";
    bytes32 public mMerkleRoot;

    constructor() ERC721A("OddDogs", "OD") {
    }

    /// Functions for minting
    function mint(uint256 quantity) external payable {
        require(mPublicSaleIsActive, "Public sale must be active to mint");
        require(quantity <= mCapPerTxPublic, "Purchase quantity exceeds transaction max");
        ...
        require(quantity + balanceOf(msg.sender) <= mWalletTokenCapPublic, "Purchase would exceed max tokens per wallet");
    ...
    function mintPresale(uint256 quantity, bytes32[] calldata proof) external payable {
        ...
        require(quantity <= mCapPerTxPresale, "Purchase quantity exceeds transaction max");
        require(quantity + totalSupply() <= mMaxTokenSupply, "Purchase would exceed max supply");
        ...
        require(quantity + balanceOf(msg.sender) <= mWalletTokenCapPresale, "Purchase would exceed max tokens per wallet");
    ...
    function setMaxTokenSupply(uint256 newMaxTokenSupply) external onlyOwner {
        mMaxTokenSupply = newMaxTokenSupply;
    }

    function setMintPrice(bool isPublic, uint256 newPrice) external onlyOwner {
        if(isPublic){
            mPublicPrice = newPrice;
        }
        else{
            mPresalePrice = newPrice;
        }
    }

    function flipSaleState(bool isPublic) external onlyOwner {
         if(isPublic){
    ...
    function setWalletTokenCap(bool isPublic, uint256 walletTokenCap) external onlyOwner {
        if(isPublic){
            mWalletTokenCapPublic = walletTokenCap;
        }
        else{
            mWalletTokenCapPresale = mWalletTokenCapPresale;
        }
    }
