...
contract NFTClass is ERC721URIStorage {
  ...
  function mint(address receiver) public onlyOwner returns(uint24) {

    require(_lastTokenId < _maxSupply, "Can't mint more items than specified");

    _lastTokenId = _lastTokenId + 1;

    _safeMint(receiver, _lastTokenId);
    _setTokenURI(_lastTokenId, _tokenURITemplate);

    return _lastTokenId;
  }

  function burn(uint24 tokenId) public onlyOwner {
    _burn(tokenId);
  }
}
