...
contract LameloBallNFT is ERC721Enumerable, controllerPanel {
    ...
    uint256 public cardPrice = 0.2 ether;
    ...
    string public _tokenPreRevealURI =
        "https://ether-cards.mypinata.cloud/ipfs/QmeKvyYvMThpuDUQ7po4AWxr5SUpfyNKecDmsgS4VqZmNu";
...
lamelo - 0x139b522955d54482e7662927653abb0bfb6f19ba
ec - 0x97ca7fe0b0288f5eb85f386fed876618fb9b8ab8
random - 0x72170f577f3b221b3478e09ccd5323445a8460d7
...
*/
    ...
    function resetTime() public {
        ...
        sale_start = discount_start + 30 minutes;
        sale_end = discount_start + 1 hours;
        forge_start = discount_start + 1 hours;
        forge_end = discount_start + 1 hours + 30 minutes;
        creator_wallet = payable(0x200C70e6De98C17C9698b9F92BDdA949496f67Bd);
        owner_wallet = payable(0x191Cd475df3F7613EC21ac6d844Bc6835a5a8d23);
        setupTime = true;
    }
    ...
    function setTime(
        ...
        creator_wallet = _creator_wallet; // rinkeby -  0x200c70e6de98c17c9698b9f92bdda949496f67bd
        owner_wallet = _owner_wallet; //  rinkeby - 0x191Cd475df3F7613EC21ac6d844Bc6835a5a8d23
        setupTime = true;
    }
    ...
    // REMOVE
    function ForgeMint(uint8 loop) public onlyOwner {
    ...
    function testSalesMint(uint8 loop) public onlyOwner {
        for (uint8 i = 0; i < loop; i++) {
            require(availableSales() >= 1, "sold out");
            assignCard();
        }
    }

    function assignForge() internal {
        maxForgeMinted++;
        _mint(msg.sender, (maxSold + maxForgeMinted));
    }

    function assignCard() internal {
        maxSold++;
        _mint(msg.sender, maxSold);
    }
    ...
    function splitFee(uint256 _value) internal {
        uint256 creatorPart = (_value * (creator_fee_percentage)) / (100);
        uint256 ownerPart = _value - (creatorPart);
        (bool sent, ) = creator_wallet.call{value: creatorPart}("");
        require(sent, "F1");
        (bool sent2, ) = owner_wallet.call{value: ownerPart}("");
        require(sent2, "F2");
    }
    ...
    {
        require(
            IERC721(ec_contract_address).ownerOf(tokenId) == msg.sender,
    ...
    {
        require(
            IERC721(lameloContractAddress).ownerOf(tokenId) == msg.sender,
            "Buyer not owner of provided token id"
        );
