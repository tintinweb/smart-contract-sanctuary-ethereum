...
error Sequencers_FunctionLocked();
error Sequencers_InvalidSignature();
error Sequencers_InvalidTokenId();
error Sequencers_SignatureAlreadyUsed();
 ...
 */
contract Sequencers is ERC1155Supply, AccessControl, ReentrancyGuard {
     ...
     */
    modifier lockable() {
        if (functionLocked[msg.sig]) revert Sequencers_FunctionLocked();
        _;
    }
    ...
    {
        if (tokenId > MAX_TOKEN_ID) revert Sequencers_InvalidTokenId();
    ...
    /**
     * @notice Allow participants to claim a SEQUENCER token
    ...
    function claim(bytes calldata signature) public lockable nonReentrant {
        if (signatureUsed[signature]) revert Sequencers_SignatureAlreadyUsed();

        if (signer != ECDSA.recover(
            ECDSA.toEthSignedMessageHash(keccak256(abi.encodePacked(_msgSender()))),
            signature
        )) revert Sequencers_InvalidSignature();
    ...
    /**
     * @notice Burn an amount of specified SEQUENCER tokens from a specified owner
     * @param from SEQUENCER owner to burn from
     * @param id SEQUENCER token to burn
     * @param amount Amount of specified SEQUENCER tokens to burn
     */
    function burn(
        address from,
        uint256 id,
     ...
     * @notice Burn an amount of specified SEQUENCER tokens from a specified owner
     * @param from SEQUENCER owner to burn from
     * @param ids SEQUENCER tokens to burn
     * @param amounts Amounts of SEQUENCER tokens to burn
     */
    function burnBatch(
        address from,
