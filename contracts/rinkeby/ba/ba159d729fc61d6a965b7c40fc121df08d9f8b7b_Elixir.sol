...
contract Elixir is ERC721, ERC721Burnable, AccessControl  {
    ...
    function getSteadyRequired(uint256 tokenId) public view returns(uint256 steadyRequired, uint256 chymeAmount, uint256 timeToMaturity) {
        ...
        (,,,uint8 decimals,) = IAcademy(address(academy)).getChymeInfo(address(elements[tokenId].chyme));
        uint divisor =  10 ** decimals;
        return (((myElixir.amount * myElixir.ratioOfSteady * uint256(myElixir.forgePrice)) / (100 * (divisor * divisor))) * (10 ** 18), 
                    myElixir.amount, myElixir.timeToMaturity);
    }
    ...
    function safeMint
        ...
        onlyRole(MINTER_ROLE) 
        {
            ...
            elements[tokenId] = Spagyria(_amount, _ratioOfSteady, _forgePrice, _chyme, msg.sender, _timeToMaturity);
        }
    ...
    function calculateParams(uint256 tokenId) public view returns (
        ...
        uint256 forgeConstant,
        uint256 timeLeft )  {
        (address oracleAddress,,, uint8 decimals,) = 
            ...
            forgeConstant = elements[tokenId].forgePrice * elements[tokenId].ratioOfSteady / 100;
        ...
        public view returns (string memory) {
            return string(
                abi.encodePacked(
                    ...
                    ',{"display_type": "number", "trait_type": "Strike Price", "value":',toString((elements[tokenId].ratioOfSteady * elements[tokenId].forgePrice / 100)),'}',
