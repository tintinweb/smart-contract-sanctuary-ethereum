...
contract BLXMRewardProvider is ReentrancyGuardUpgradeable, BLXMMultiOwnable, BLXMTreasuryManager, IBLXMRewardProvider {
    ...
    function _addRewards(address token, uint totalAmount, uint16 supplyDays) internal nonReentrant onlyOwner returns (uint amountPerHours) {
        ...
        if (lastSession == 0) {
            field.initialHour = BLXMLibrary.currentHour() + 1;
        }
    ...
    function _mint(address to, address token, uint amountBlxm, uint amountToken, uint16 lockedDays) internal nonReentrant returns (uint liquidity) {
        ...
        uint extraLiquidity;
        if (factor > 0) {
            extraLiquidity = liquidity.wmul(factor);
        } else {
            lockedDays = 0;
        }
    ...
    function _updateStatistics(Field storage field, uint32 fromHour, uint32 toHour) internal {
        ...
        while (fromHour < toHour) {
            ...
            uint rewards;
            if (fromHour >= field.initialHour) {
                rewards = field.periods[field.daysToSession[(fromHour - field.initialHour) / 24]].amountPerHours;
            }
            if (liquidity != 0) {
...
contract BLXMTreasuryManager is BLXMMultiOwnable, IBLXMTreasuryManager {
    ...
    // token => add treasury idx
    mapping(address => uint) public override treasuryIdx;
    ...
    function putTreasury(address token, address treasury) external override onlyOwner {
        ...
        if (oldTreasury == address(0)) {
            treasuryIdx[token] = allTreasury.length;
            allTreasury.push(treasury);
        } else {
            uint idx = treasuryIdx[token];
            allTreasury[idx] = treasury;
        }

        treasurys[token] = treasury;
        emit TreasuryPut(token, oldTreasury, treasury, allTreasury.length);
    }
...
interface IBLXMTreasuryManager {
    ...
    event TreasuryPut(address indexed token, address indexed oldTreasury, address indexed newTreasury, uint length);
    ...
    function treasuryIdx(address token) external view returns (uint idx);
    function allTreasuryLength() external view returns (uint length);
