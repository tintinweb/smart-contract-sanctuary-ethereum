...
contract MetaFriends is ERC721, Ownable {
    ...
    uint public finalPhaseItems = 3334;
    uint public finalMintCount = 0;
    ...
    function airdropMint(
        ...
        uint amount = 1;
        require(_verify(_hash(salt, msg.sender), token), "Invalid token.");
        require(airdropMintCount + 1 <= airdropItems, "amount error");
    ...
    function publicMint() external payable {
        ...
        require(totalSupply - airdropMintCount + amount <= firstPhaseItems - airdropItems, "publicMint: Surpasses cap");
        _mintWithoutValidation(msg.sender, amount);
    }

    // mint to other contract address, use auction
    function finalMint(uint amount, address to) external onlyOwner {
        require(finalMintCount + amount <= finalPhaseItems, "finalMint: Surpasses cap");
        finalMintCount += amount;
        _mintWithoutValidation(to, amount);
    }

    function airdtopToAddr(address[] memory toList) external onlyOwner {
        require(airdropMintCount + toList.length <= airdropItems,"amount error");
