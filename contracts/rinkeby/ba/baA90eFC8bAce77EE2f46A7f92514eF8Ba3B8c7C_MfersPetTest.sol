...
contract MfersPetTest is ERC721A, Ownable, ReentrancyGuard {
    ...
    constructor(
        ...
        mferContract = IERC721Enumerable(_mfers);
    }

    function personalizedMint(
        ...
        require(
            isOwnedNft(_mferIds),
            "Personlized mint only for this nft holder"
        );
    ...
    function isOwnedNft(uint256[] memory _mferIds)
        internal
        view
        returns (bool)
    {
        ...
        mintPriceCompliance(_mintAmount)
    {
        ...
        holderAddressPublicMintBalance[msg.sender] = _mintAmount + holderAddressPublicMintBalance[msg.sender];
        ...
        returns (string memory)
    {
        ...
        if (revealed == false) {
            return isCat[_tokenId]
                ? string(
                    abi.encodePacked(hiddenMetadataPrefix, hiddenUriCatSuffix)
                )
                : string(
                    abi.encodePacked(hiddenMetadataPrefix, hiddenUriDogSuffix)
                );
        }
