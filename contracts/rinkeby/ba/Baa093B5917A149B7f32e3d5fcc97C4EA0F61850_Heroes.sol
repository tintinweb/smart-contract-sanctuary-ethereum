...
contract Heroes is ERC721ABurnable, Ownable, ReentrancyGuard {
 ...
 function freeMintRedeemed(address a) external view returns (bool){
...
function generalMintWithPass(bytes32[] memory _merkleProof, uint mintCount) validateMinting(mintCount) nonReentrant external payable {
        require(currentMode == ReleaseMode.OPEN, "It's not go time yet.");
        ...
        uint mintingFeesToCharge = mintCount;
        if(MerkleProof.verify(_merkleProof, _freeMintMerkleRoot, leaf)){
          if(_freeMintRedeemed[msg.sender] != 1){ // if the pass is NOT already used, mark it as used and let us use the free one.
            mintingFeesToCharge = mintingFeesToCharge - 1;
            _freeMintRedeemed[msg.sender] = 1;
          }
        }
