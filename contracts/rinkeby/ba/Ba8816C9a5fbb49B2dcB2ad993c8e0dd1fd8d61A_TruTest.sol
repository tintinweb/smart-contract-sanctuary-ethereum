...
contract TruTest is Initializable, ERC721Upgradeable, PausableUpgradeable, OwnableUpgradeable {
    ...
    */
    ...
    uint8 public NFTlimitPerUser;

    Counters.Counter private _tokenIdCounter;
    uint256 private _reserveTokenIdCounter; //separate counter used only for reserve tokens

    bytes32 private whitelistMerkleRoot;
    mapping(address => uint8) private claimedTokensViaWhitelist;
    mapping(address => uint8) private claimedUsersTokens;

    function initialize() initializer public {
        __ERC721_init("Truplay", "TRU");
        __Pausable_init();
        __Ownable_init();
        baseURI = "ipfs://QmUMFvYwyi34s3zy9Aq8XFSYqkBP9yarxBq52raHH99FJY/";
        NFTlimitPerUser = 1;
        isSaleActive = false;
        ...
        checkTokenCounter
    {
        ...
        require(claimedTokensViaWhitelist[msg.sender] < 4, "E#6");

        claimedTokensViaWhitelist[msg.sender]++;
        _tokenIdCounter.increment();
        uint256 tokenId = _tokenIdCounter.current();
        _safeMint(msg.sender, tokenId);
