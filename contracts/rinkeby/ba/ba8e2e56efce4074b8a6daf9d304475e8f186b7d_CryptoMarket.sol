...
contract CryptoMarket is ReentrancyGuard {
  ...
  function create_market_items_erc721(uint256 tokenId , address nftContract,uint royalities) public  {
    ...
    markettokens[n] = MarketToken(n,tokenId,nftContract,payable(msg.sender),payable(msg.sender),payable(0x00),0,0,royalities,0,payable(0x00),false,false,true,false,false);
  ...
  function create_market_items_erc1155(uint256 tokenId , address nftContract,uint royalities,uint copies) public  {
    ...
    for(uint i=1;copies>=i;i++){
      ...
      markettokens[n] = MarketToken(n,tokenId, nftContract,payable(msg.sender),payable(msg.sender),payable(0x00),0,i,royalities,0,payable(0x00),false,false,false,false,false);
