...
contract TBCS is ERC721, Ownable, ERC2981ContractWideRoyalties {
    ...
    /// @dev This function should be protected with a onlyOwner (or equivalent) modifier
    ...
    //TODO to be fixed according to real percentage
    uint256[] public OwnersPercentage = [50, 10, 10, 10, 10, 10];
    ...
    constructor(address _multisigWallet)
        ERC721("Teddy Bear Cutie Squad", "TBCS")
    {
    ...
    function setWhitelisting(bool val) public onlyAdmins {
        whitelistActive = val;
    }
    ...
    function setOGwallets(address[] memory _a, bool[] memory _amount)
        public
        onlyAdmins

    {
        for (uint256 i; i < _a.length; i++) {
            OGwallets[_a[i]] = _amount[i];
        }
    }

   

    function setwhitelistWallets(address[] memory _a, bool[] memory _allowed)
        public
        onlyAdmins
    ...
    function giveaway(address to, uint256 qty) external onlyAdmins {
        require(
    ...
    function mintOG(uint256 qty) external payable {
        bool isAllowed = OGwallets[msg.sender];
        require(presaleActive, "TRANSACTION: Presale is not active");
        require(
            qty <= maxTx || qty > 1,
            "TRANSACTION: qty of mints not allowed"
        );
    ...
    function mint(uint256 qty) external payable {
        ...
        require(
            isAllowed || whitelistActive,
            "TRANSACTION: You are not whitelisted"
        );
        require(
            qty <= maxTx && qty > 0,
            "TRANSACTION: qty of mints not allowed"
