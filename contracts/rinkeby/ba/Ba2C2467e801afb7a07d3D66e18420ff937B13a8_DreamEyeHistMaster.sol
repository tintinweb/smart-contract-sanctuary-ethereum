...
contract DreamEyeHistMaster is INftMasterToken, ERC721AQueryable, ERC721ABurnable, Ownable, ReentrancyGuard, ERC2981 {
  ...
  mapping (address => bool) stageBoosters; 
  uint256 public revealedStage;
  mapping(uint256 => string) public STAGE_PROVENANCE;
  ...
  INftHistoryToken[] public sideCars; //TODO array of sidecars
  mapping(address => uint256) sideCarIndex;

  mapping (address => bool) disabledSideCars;
  ...
  function setProvenanceHash(uint256 _stage, string memory provenanceHash) public onlyOwner {
      STAGE_PROVENANCE[_stage] = provenanceHash;
  }
  ...
  function addSideCar(address _sideCarContract) public onlyOwner {
    sideCars.push(INftHistoryToken(_sideCarContract));
      ...
      if(sideCarsEnabled) {
        for (uint256 i = 0; i < quantity; i++) {
          for (uint si = 0; si < sideCars.length; si++) {
            sideCars[si].transferHist(from, to, tokenId + i);
          }
        }        
      }
    }
