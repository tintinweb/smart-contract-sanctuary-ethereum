...
import {PrimaryMarketProxy} from "contracts/proxies/PrimaryMarketProxy.sol";
import {SafeRegistry} from "contracts/registry/SafeRegistry.sol";
import {FactoryVersion} from "contracts/factories/FactoryVersion.sol";
import {IModuleFactory} from "contracts/interfaces/IModuleFactory.sol";
import {Clones} from "@openzeppelin/contracts/proxy/Clones.sol";
...
/**
 * @title PrimaryMarketModuleFactory
 */
contract PrimaryMarketModuleFactory is FactoryVersion, IModuleFactory {
    ...
    /**
     * @notice Deploy primary market for a channel
     * @param _channel      Channel, owner of the module
     * @param moduleSalt    Unique hex string
     * @return module       Deployed module address
     */
    function deployPrimaryMarketModule(address _channel, bytes32 moduleSalt, address _roleModule)
        public
        onlyChannel
        returns (address module)
    {
        require(
            !Address.isContract(getPrimaryMarketAddress(moduleSalt)),
            "PrimaryMarketModuleFactory: Duplicate salt"
        );
        ...
        module = address(
            new PrimaryMarketProxy{salt: moduleSalt}()
        );
        ...
        emit ModuleDeployed(module, msg.sender, bytes4(keccak256("PRIMARY_MARKET")));
    }

    function getPrimaryMarketAddress(bytes32 salt) public view returns (address module) {
        bytes memory bytecode = type(PrimaryMarketProxy).creationCode;
...
interface IPrimaryMarketModuleFactory {
...
/**
 * @title PrimaryMarketProxy - Proxy for the Primary Market
 */
contract PrimaryMarketProxy is MarketplaceStorage {
    constructor() {
        _owner = IPrimaryMarketModuleFactory(msg.sender).channel();
        registry = SafeRegistry(IPrimaryMarketModuleFactory(msg.sender).registry());
        roleModule = RoleModule(IPrimaryMarketModuleFactory(msg.sender).roleModule());
    
        _implementation = IPrimaryMarketModuleFactory(msg.sender).implementation();
    }
