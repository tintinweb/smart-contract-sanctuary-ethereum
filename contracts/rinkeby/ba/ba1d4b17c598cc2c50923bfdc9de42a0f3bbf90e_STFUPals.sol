...
contract STFUPals is ERC721A, Ownable {
    ...
    function claim(uint256 _quantity, bytes32[] calldata _merkleProof) external payable {
        
        require(_quantity <= MAX_QUANTITY, "Max 1 per transaction");
        require((totalSupply() + _quantity) <= MAX_SUPPLY, "OOS");
        require(mintTime, "It is not time to mint");
        require(mintedWallets[msg.sender] == true, "Already Minted!");
    ...
    function testMint(uint256 _quantity) external payable {
    ...
    function mint(uint256 _quantity, bytes32[] calldata _merkleProof) external payable {
