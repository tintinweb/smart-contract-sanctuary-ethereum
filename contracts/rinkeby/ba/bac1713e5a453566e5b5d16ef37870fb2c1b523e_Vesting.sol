...
contract Vesting is Initializable, OwnableUpgradeable, ReentrancyGuardUpgradeable, PausableUpgradeable {
    ...
    function claimToken(uint _allocationType, uint256 _merkleTreeClaimed, uint256 _totalVestingAmount, uint16 _TGE, uint32 _startTime, uint32 _cliffDuration, uint32 _vestingDuration, bytes32[] memory proof) external nonReentrant whenNotPaused { 

        VestingSchedule storage user = userVestingSchedule[msg.sender][_allocationType];

        require(_TGE <= 10000, "INVALID TGE");
        require(_verifyVestingUser(msg.sender, _allocationType, _merkleTreeClaimed, _totalVestingAmount, _TGE, _startTime, _cliffDuration, _vestingDuration, proof), "INVALID_MERKLE");
        ...
        if (user.totalVestingAmount == 0) {
            ...
            user.merkleTreeClaimed = _merkleTreeClaimed;
        }

        if (user.merkleTreeClaimed != _merkleTreeClaimed) {
            user.totalVestingAmount = _totalVestingAmount;
            user.start = _startTime;
            user.cliff = _cliffDuration;
            user.duration = _vestingDuration;
    ...
    function _verifyVestingUser(
       ...
       bytes32[] memory proof
        ...
        bytes32 leaf = keccak256(abi.encodePacked(_user, _allocationType, _merkleTreeClaimed, _totalVestingAmount, _TGE, _startTime, _cliffDuration, _vestingDuration));
