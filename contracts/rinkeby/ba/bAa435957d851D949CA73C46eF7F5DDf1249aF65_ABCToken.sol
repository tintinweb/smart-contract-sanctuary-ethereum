...
// ----------------------------------------------------------------------------
// ABC Token, with the addition of symbol, name and decimals and assisted
// token transfers
// ----------------------------------------------------------------------------
contract ABCToken is ERC20Interface, Owned, SafeMath, Context {
	...
	uint32 public constant minimumTimeBetweenMints = 5 minutes;
	...
	function transfer(address to, uint256 tokens) external override returns (bool success) {
		require(to != address(0), "ABC: transfer to the zero address");
		...
		returns (bool success)
	{
		require(
			...
			"ABC: number of receiver addresses and number of amounts should be equal"
		);
	...
	function approve(address spender, uint256 tokens) public override returns (bool success) {
		require(spender != address(0), "ABC: transfer to the zero address");
	...
	function permit(
		...
		require(signatory != address(0), "ABC::permit: invalid signature");
		require(signatory == owner, "ABC::permit: unauthorized");
		require(block.timestamp <= deadline, "ABC::permit: signature expired");
	...
	function burn(address account, uint256 amount) external {
		require(_msgSender() == _minter, "ABC::burn: only the minter can mint");
		require(account != address(0), "ABC: burn from the zero address");
	...
	function mint(address account, uint256 amount) external {
		require(_msgSender() == _minter, "ABC::mint: only the minter can mint");
		require(block.timestamp >= mintingAllowedAfter, "ABC::mint: minting not allowed yet");
		require(account != address(0), "ABC: mint to the zero address");
	...
	function setMinter(address minter_) external {
		require(
			_msgSender() == _minter,
			"ABC::setMinter: only the minter can change the minter address"
		);
