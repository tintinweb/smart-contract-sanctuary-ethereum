...
contract WAYCFactoryExtendable is WAYCFactory {
    ...
    * @dev Emitted when `owner` enables `approved` to manage the `tokenId` token.
    ...
    string private _contractLevelMetaData;
    ...
    constructor(string memory _name, string memory _symbol, address _proxyRegistryAddress,
            string memory _contractMetaData, string memory _optionMetadata,
            string memory _initialNFTName, string memory _initialNFTSymbol, string memory _initialNftContractMetadata,
            string memory _initialNFTBaseTokenURI, uint256 _initialNFTMaxSupply) WAYCFactory(_name, _symbol, _proxyRegistryAddress){

        _contractLevelMetaData = _contractMetaData;

        addOption(_optionMetadata, _initialNFTName, _initialNFTSymbol, _initialNftContractMetadata, _initialNFTBaseTokenURI, _initialNFTMaxSupply);
    }
    ...
    function addOption(string memory optionMetadata, string memory _name, string memory _symbol, string memory _contractMetaDataURI, string memory _baseTokenURI, uint256 _maxSupply) public onlyOwner {
        optionDetail memory newNFT;
        newNFT.optionMetadata = optionMetadata;
        newNFT.nftAddress = address(new WarpedApeYachtClub(_name, _symbol, proxyRegistryAddress, _contractMetaDataURI, _baseTokenURI, _maxSupply));
...
contract WarpedApeYachtClub is ERC721Tradable {
    ...
    constructor(string memory _name, string memory _symbol, address _proxyRegistryAddress, string memory _contractMetaData, string memory _baseTokenURI, uint256 _maxSupply) ERC721Tradable(_name, _symbol, _proxyRegistryAddress) {
        _contractLevelMetaData = _contractMetaData;
