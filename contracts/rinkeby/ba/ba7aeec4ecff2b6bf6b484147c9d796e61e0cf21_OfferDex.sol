...
contract OfferDex is Ownable, DexStorage {
   ...
   event OfferAccept(uint256 tokenId, address collection, uint256 amount, address from, address to, uint256 currencyType, string ownerId);
  ...
  function acceptOffer(address[2] memory participants /** offerer,collection */, uint256[4] memory nums /** tokenId, price, currencyType, expiry */, uint8 v /** offerer V */,
        bytes32[2] memory rs /** offerer r & s */) public {
          ...
          acceptedOffer[hashOffer] = true;
            ...
            processOffer(offer, nums[0], participants[1]);
...
/**
  ...
  function processOffer(Offer memory offer, uint256 tokenId, address collection ) internal {   
    ...
    emit OfferAccept(tokenId, collection, temp.price, msg.sender, temp.maker, temp.currencyType, "");
  }
