...
contract MooWorld is ERC721Enumerable, ERC721Burnable, ERC721Pausable, Ownable {
    ...
    function mint(uint8 quantity) external payable whenNotPaused {
    ...
    function presaleMint(uint8 quantity) external payable whenNotPaused {
        ...
        require(msg.value >= quantity * mintPrice, "Wrong price");
        require(_whitelistToken.balanceOf(msg.sender) >= quantity, "Not enough whitelist nfts in balance");
        require((tokenId + quantity) < maxTokenIds, "Exceed max supply");
        for (uint256 i = 0; i < quantity; i++) {
            _safeMint(msg.sender, tokenId + i);
            _burnWhitelistToken(msg.sender);
            _tokenIds.increment();
        }
    }
    ...
    function _burnWhitelistToken(address to) private {
        uint256 tokenId = _whitelistToken.tokenOfOwnerByIndex(to, 0);
        _whitelistToken.burn(tokenId);
    }
