...
contract KeepBoredPass is ERC721A, Ownable {
    ...
    modifier mintCompliance() {
        require(
            totalSupply() + 1 < TOTAL_SUPPLY - (RSupply + ROGSupply + 1), "Max supply exceeded!");
        require(
            _numberMinted(msg.sender) + 1 < MINT_PER_WALLET + 1, "You are exceeding your minting limit");
        _;
    }

 constructor(address signerAddress_, address ogsignerAddress_, string memory baseTokenURI_)
 ERC721A("Keep Bored Pass", "KBP") {
    ...
    function ogCLain(bytes calldata signature)
    external
    payable
    onlyOrigin
    mintCompliance {
        ...
        // check wl signature
        require(
            _OgsignerAddress == verifySignature(signature),
    ...
    mintCompliance {
        ...
        // check wl signature
        require(
            _WLsignerAddress == verifySignature(signature),
            "Signer address mismatch."
        );
