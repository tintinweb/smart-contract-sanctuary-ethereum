...
// File contracts/yield-farm/IDCSdexPair.sol

pragma solidity 0.8.16;

interface IDCSdexPair {
    function permit(
        address owner,
        address spender,
        uint256 value,
...
// File contracts/yield-farm/DCSChef.sol

pragma solidity ^0.8.0;

contract DCSChef is Ownable, ReentrancyGuard {
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint; // How many allocation points assigned to this pool. DCS to distribute per block.
        uint256 lastRewardBlock; // Last block number that DCS distribution occurs.
        uint256 accDCSPerShare; // Accumulated DCS per share, times 1e12. See below.
        uint16 depositFeeBP; // Deposit fee in basis points
        uint256 harvestInterval; // Harvest interval in seconds
        uint256 totalLp; // Total token in Pool
    }

    IBoringERC20 public DCS;
    ...
    // DCS tokens created per block
    uint256 public DCSPerBlock;
    ...
    // The block number when DCS mining starts.
    uint256 public startBlock;

    // Total locked up rewards
    uint256 public totalLockedUpRewards;

    // Total DCS in DCS Pools (can be multiple pools)
    uint256 public totalDCSInPools = 0;
    ...
    // Update the given pool's DCS allocation point and deposit fee. Can only be called by the owner.
    function set(
        uint256 _pid,
        uint256 _allocPoint,
        uint16 _depositFeeBP,
    ...
    // View function to see pending DCS on frontend.
    function pendingDCS(uint256 _pid, address _user)
        external
        view
        returns (uint256)
    {
        ...
        uint256 accDCSPerShare = pool.accDCSPerShare;
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 DCSReward = multiplier
                .mul(DCSPerBlock)
                .mul(pool.allocPoint)
                .div(totalAllocPoint);
            accDCSPerShare = accDCSPerShare.add(
                DCSReward.mul(1e12).div(lpSupply)
            );
        }

        uint256 pending = user.amount.mul(accDCSPerShare).div(1e12).sub(
            user.rewardDebt
        );
        return pending.add(user.rewardLockedUp);
    }

    // View function to see if user can harvest DCS.
    ...
    function updatePool(uint256 _pid) public {
        ...
        DCS.mint(marketingAddress, (DCSReward * marketingPercent) / 1000);
        DCS.mint(address(this), (DCSReward * lpPercent) / 1000);

        pool.accDCSPerShare =
            pool.accDCSPerShare +
            (((DCSReward * 1e12) / pool.totalLp) * lpPercent) /
            1000;
    ...
    function depositWithPermit(
        ...
        PoolInfo storage pool = poolInfo[pid];
        IDCSdexPair pair = IDCSdexPair(address(pool.lpToken));
        pair.permit(msg.sender, address(this), amount, deadline, v, r, s);
        deposit(pid, amount);
    }

    // Deposit LP tokens to MasterChef for DCS allocation.
    function deposit(uint256 _pid, uint256 _amount) public nonReentrant {
        require(
            block.number >= startBlock,
            "DCSdexChef: Can not deposit before start"
        );
        ...
        if (_amount > 0) {
            ...
            if (address(pool.lpToken) == address(DCS)) {
                totalDCSInPools = totalDCSInPools.add(_amount);
            }
        }
        user.rewardDebt = user.amount.mul(pool.accDCSPerShare).div(1e12);
    ...
    function withdraw(uint256 _pid, uint256 _amount) public nonReentrant {
        ...
        if (_amount > 0) {
            ...
            if (address(pool.lpToken) == address(DCS)) {
                totalDCSInPools = totalDCSInPools.sub(_amount);
            }
    ...
    function emergencyWithdraw(uint256 _pid) public nonReentrant {
        ...
        if (address(pool.lpToken) == address(DCS)) {
            totalDCSInPools = totalDCSInPools.sub(amount);
        }
    ...
    // Pay or lockup pending DCS.
    function payOrLockupPendingDCS(uint256 _pid) internal {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        ...
        uint256 pending = user.amount.mul(pool.accDCSPerShare).div(1e12).sub(
            user.rewardDebt
        );
        if (canHarvest(_pid, msg.sender)) {
            if (pending > 0 || user.rewardLockedUp > 0) {
                ...
                // send rewards
                safeDCSTransfer(msg.sender, totalRewards);
            }
        } else if (pending > 0) {
    ...
    // Safe DCS transfer function, just in case if rounding error causes pool do not have enough DCS.
    function safeDCSTransfer(address _to, uint256 _amount) internal {
        if (DCS.balanceOf(address(this)) > totalDCSInPools) {
            //DCSBal = total DCS in DCSdexChef - total DCS in DCS pools, this will make sure that DCSdexChef never transfer rewards from deposited DCS pools
            uint256 DCSBal = DCS.balanceOf(address(this)).sub(
                totalDCSInPools
            );
            if (_amount >= DCSBal) {
                DCS.safeTransfer(_to, DCSBal);
            } else if (_amount > 0) {
                DCS.safeTransfer(_to, _amount);
            }
        }
    }

    // Pancake has to add hidden dummy pools in order to alter the emission, here we make it simple and transparent to all.
    function updateEmissionRate(uint256 _DCSPerBlock) public onlyOperator {
        massUpdatePools();

        emit EmissionRateUpdated(msg.sender, DCSPerBlock, _DCSPerBlock);
        DCSPerBlock = _DCSPerBlock;
    }
