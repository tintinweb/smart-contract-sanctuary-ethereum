...
contract SnowfallProtocol is Context, IBEP20, Ownable {
    ...
    function totalFeePerTx(uint256 tAmount) internal view returns(uint256) {
        uint256 percentage = tAmount.mul(_currentRedistributionFee.add(_currentLiquidityFee).add(_currentmarketingwalletFee).add(_currentautoburnFee)).div(1e3);
        return percentage;
    }
    ...
    function _takeLiquidityPoolFee(uint256 tAmount, uint256 currentRate) internal {
        uint256 tPoolFee = tAmount.mul(_currentLiquidityFee).div(1e3);
    ...
    function _takeMarketFee(uint256 tAmount, uint256 currentRate) internal {
        uint256 tCharityFee = tAmount.mul(_currentmarketingwalletFee).div(1e3);
        uint256 rCharityFee = tCharityFee.mul(currentRate);
        _rOwned[marketWallet] = _rOwned[marketWallet].add(rCharityFee);
        if (_isExcluded[marketWallet])
            _tOwned[marketWallet] = _tOwned[marketWallet].add(tCharityFee);
        emit Transfer(_msgSender(), marketWallet, tCharityFee);
    }
    
    function _takeBurnFee(uint256 tAmount, uint256 currentRate) internal {
        uint256 burnFee = tAmount.mul(_currentautoburnFee).div(1e3);
    ...
    function setBuyFeePercent(
        ...
        require(
            redistributionFeeOnBuying
                ...
                .add(marketingwalletFeeOnBuying)
                .add(autoburnFeeOnBuying) <= maxFee,
    ...
    function setSellFeePercent(
        ...
        require(
            redistributionFeeOnSelling
                ...
                .add(marketingwalletFeeOnSelling)
                .add(autoburnFeeOnSelling) <= maxFee,
            "BEP20: Can not be greater than max fee"
        );
    }
    ...
    function setNormalFeePercent(
        ...
        require(
            redistributionFee.add(liquidityFee).add(marketingwalletFee).add(
                autoburnFee
            ) <= maxFee,
            "BEP20: Can not be greater than max fee"
        );
