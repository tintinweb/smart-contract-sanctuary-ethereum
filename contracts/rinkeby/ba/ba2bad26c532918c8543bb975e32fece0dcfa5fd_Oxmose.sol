...
contract Oxmose is ERC721A, ERC2981ContractWideRoyalties, Ownable, Whitelist {
    ...
    string public jsonAttributesProvenance; 
    ...
    uint constant public ROYALTY_PERCENTAGE = 750; // value percentage (using 2 decimals - 10000 = 100, 0 = 0)
 
    address constant private WALLET_1 = 0xc639acaea7949C01067723F811bac091E5e2fC8C;
    address constant private WALLET_2 = 0x85d1581E9B7bF793dF9C5E8e9876551536316481;
    address constant private WALLET_3 = 0x1773b544f3EA46CD1D4CC6aACa8e5de337e26889;
 
    constructor (
        string memory _jsonAttributesProvenance,
        bytes32 _whitelistMerkle,
        address _paymentSecondarySale
    ) 
        ERC721A("Oxmose Season 1", "OXO1", 10) 
        Whitelist(_whitelistMerkle)
    {
        jsonAttributesProvenance = _jsonAttributesProvenance;
        _setRoyalties(_paymentSecondarySale, ROYALTY_PERCENTAGE);
    }
    ...
    function setJsonAttributesIPFSHash(string memory _hash) public onlyOwner {
        require(bytes(jsonAttributesIPFSHash).length != 0, "Already set");
        jsonAttributesIPFSHash = _hash;
    }
 
    function getOxmoseAPIURI(uint tokenId) public view onlyOwner returns(string memory) {
        return string(abi.encodePacked("https://nft.oxmose.co/1/", Strings.toString(tokenId)));
    }
 
    function getArweaveURI(uint tokenId) public view onlyOwner returns(string memory) {
    ...
    function mint(uint256 quantity, bytes32[] calldata proof, uint256[] calldata positions) public payable {
        ...
        require(numberMinted(msg.sender) <= 10, "You can't mint more tokens");
    ...
    function contractURI() public view returns (string memory) {
        return "https://ipfs.io/ipfs/Qmd8a1qVfyZW1aH9ZLag3yXtfHka8bQCecr4CrFAqyK29u";
    }                                                                                                                                                                                                    
}