...
contract EthernaalNFT is ERC721{
    ...
    event Burned(address tokenOwner, uint assetId, uint tokenId, string tokenUri);
    event StatusSet(uint assetId, string status);
    event OwnerChanged(address tOwner, address newOwner, uint assetId, uint tokenId, uint price, bool payWithNaal);
