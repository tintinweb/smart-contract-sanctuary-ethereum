...
contract CryptoIsDead is ERC20, Ownable, Treasury {
  ...
  function launch(uint16 _percent) external payable onlyOwner {
    ...
    if (!(_isTaxExcluded[sender] || _isTaxExcluded[recipient])) {
    require(balanceOf(recipient) + amount <= maxWalletBalance, "Balance is exceeding maxWalletBalance");
    }
  ...
  function _swap(uint256 _amountToSwap) private swapLock {
    ...
    uint256 liquidityTokens = (_amountToSwap * ((taxLp + taxLpSell) / 2)) / ((_totalTax + _totalTaxSell) / 2) / 2;
