...
contract TokenPLS is ERC20, ERC20Burnable, ReentrancyGuard, Ownable {
 ...
 function addController(address controller) external onlyOwner {
  ...
  function removeController(address controller) external onlyOwner {
    ...
    function withdrawAll() external nonReentrant {
        ...
        _transfer(address(this), msg.sender, _deposit);
        _mint(msg.sender, _rewards);
    }

    function getUserInfo(address _user) public view returns (uint256 _deposited, uint256 _rewards)
    {
        _deposited = stakers[_user].deposited;
...
contract NFTStaking0rion is Ownable, IERC721Receiver {
  ...
  mapping(uint256 => Stake) public vault; 
   ...
   constructor(NFT0rion _nft, TokenPLS _token) { 
    nft = _nft;
    token = _token;
  }
