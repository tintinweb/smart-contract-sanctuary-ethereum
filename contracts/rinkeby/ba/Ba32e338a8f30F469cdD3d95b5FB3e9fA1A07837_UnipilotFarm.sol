...
contract UnipilotFarm is IUnipilotFarm, ReentrancyGuard {
    ...
    //reveal vault address on basis of index
    mapping(uint256 => address) private vaults;

    //reveal struct vault data on basis of vault
    mapping(address => VaultInfo) public vaultInfo;

    //reveal struct vault alt data on basis of vault
    mapping(address => AltInfo) public vaultAltInfo;

    //reveal vault user's data on basis of vault and user address
    mapping(address => mapping(address => UserInfo)) public userInfo;

    //reveal status of vault
    ...
    ///@notice use for deposit funds in UnipilotFarm
        ...
        isLimitActive
    {
        ...
        if (
            ...
            (vaultState.reward == RewardType.Alt &&
                vaultAltInfo[_vault].rewardToken != address(0))
        ) {
    ...
    ///@notice use for withdrawing funds from UnipilotFarm
    ...
    ///@notice use for withdraw rewards accumulated
    ///@param _vault vault address from which to claim the accumulated reward from
    ///  greater then zero if you have stakedLp
        ...
        )
    {
        ...
        if (vaultState.reward == RewardType.Dual) {
            ...
            console.log("altReward", altReward);
            emit Reward(vaultAltState.rewardToken, caller, _vault, altReward);
            IERC20(vaultAltState.rewardToken).safeTransfer(caller, altReward);
        } else {
            userState.reward = gr;
            ...
            vaultState.lastRewardBlock = blocknum;
            console.log(
                "vaultState.lastRewardBlock",
                vaultState.lastRewardBlock
            );
            if (altReward > 0) {
                userState.reward = vaultAltState.globalReward;
                emit Reward(
                    vaultAltState.rewardToken,
                    caller,
                    _vault,
                    altReward
                );
                IERC20(vaultAltState.rewardToken).safeTransfer(
                    caller,
                    altReward
                );
            }
    ...
    ///@notice used inorder to blackList a list of pools.
    ///@dev only called by governance
    ///@param _vaults list of vaults
    function blacklistVaults(address[] memory _vaults)
        external
        override
        onlyGovernance
    {
    ...
    ///@notice use to update the reward per block of the vaults
    ///@dev only called by governance
    ...
    ///@notice used to update multiplier of particular vault
    ///@dev only called by governance
    ...
    ///@notice use for update alt multiplier of particular vault
    ///@dev only called by governance
    ///@param _vault vault address for which you want to update multiplier
    ///@param _value value of multiplier
    ...
    ///@notice use to update governance
    ///@dev only called by governance
    ...
    ///@notice use to read list of vaults
    ///@dev only called by governance
    ...
    function updateRewardType(
        ...
        require(_altToken != address(0), "NAZ");
        AltInfo storage altState = vaultAltInfo[_vault];
        VaultInfo storage vaultState = vaultInfo[_vault];
        uint256 blockNumber = block.number;
        if (vaultInfo[_vault].reward == RewardType.Alt) {
            console.log("YE CHALA HAI 1");
    ...
    ///@notice used to update PoolInfo and AltInfo global rewards and lastRewardBlock
    ///@dev only called by governance
    ...
    function migrateFunds(
        ...
        require(_receiver != address(0), "CNE");
    ...
    function toggleBooster(address _vault, address _user)
        external
        override
        onlyStake
    {
    ...
    function getGlobalReward(
        ...
        if (vaultWhitelist[_vault]) {
            ...
            console.log("block differ", _blockDiff);
            console.log("last global reward", _lastGlobalReward);
    ...
    ///@notice used to add vault in Unipilot Farm called inside initializer
    ///@param _vault address of vault to add
    ///@param _multiplier value of multiplier to set for particular vault
    function insertVault(address _vault, uint256 _multiplier) private {
        totalVaults++;
        ...
        emit Vault(_vault, rewardPerBlock, _multiplier, block.number);
    }
        ...
        returns (uint256 globalReward)
    {
        ...
        if (
            ...
            console.log("state.lastRewardBlock", state.lastRewardBlock);
            globalReward = getGlobalReward(
                _vault,
                blockDifference,
                state.multiplier,
...
interface IUnipilotFarm {
    ...
    function blacklistVaults(address[] memory _vault) external;
