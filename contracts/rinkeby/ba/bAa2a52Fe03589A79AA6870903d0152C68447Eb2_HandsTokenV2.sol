...
contract HandsTokenV2 is ERC721A, AccessControlEnumerable, Pausable, Ownable, ReentrancyGuard, ERC2981 {
    ...
    bool private _tokensReissued = false;

    string _baseURIValue;

    constructor(string memory baseURI_) ERC721A("Hands NFT V2", "HANDS") {
    ...
    function reissueTokens(address[] calldata addresses, uint8[] calldata tokenCounts) external onlyOwner {
        require(!_tokensReissued);
        require(addresses.length == tokenCounts.length, "Array lengths must match");

        _tokensReissued = true;

        for(uint256 i; i < addresses.length; i++) {
            _mint(addresses[i], tokenCounts[i], '', false);
        }
    }
