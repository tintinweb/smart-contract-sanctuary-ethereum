...
contract cellNFT is ERC721A, Ownable, ReentrancyGuard {
    ...
    string _seed_phrase;
    uint256 PRICE_PER_AA_SACRIFICE = 0.01 ether;
    ...
    function _sacrifice_AA(uint256 AA_tokenId) internal {
        require(IAANFT(AA_CONTRACT_ADDY)._ownerOf(AA_tokenId) == msg.sender, "don't own");
        require(IAANFT(AA_CONTRACT_ADDY)._isAlive(AA_tokenId) == 1, "not alive");
        IAANFT(AA_CONTRACT_ADDY).sacrifice(AA_tokenId);
        _safeMint(msg.sender, 1);
        cells[TOKENS_MINTED] = CellData({eukaryote: uint64(IAANFT(AA_CONTRACT_ADDY)._isMutant(AA_tokenId)), generation: 1, _type: 0});
    ...
    function _get_scale_value(string memory tokenId, uint64 _type) internal view returns (bytes memory) {
        if (_type == 2) {
            ...
            return abi.encodePacked(
                ...
                ".", SVGManip.get_rand_in_range_toStr(rand, 0, 999), "1)"
            );
        } else {
    ...
    function _make_svg(string memory tokenId, uint64 eukaryote, uint64 _type) internal view returns (bytes memory) {
        ...
        if (eukaryote == 1) {output = SVGManip.add_filters(tokenId, output, eukaryote, _type, true);}
    ...
    function _isEukaryote(uint256 tokenId) external view returns (uint256) {
...
contract svgManip {
    ...
    function get_values_and_dur_for_animate(string memory tokenId, string memory prefix, AnimateParams memory a) internal view returns (bytes memory) {
        return abi.encodePacked('\n<animate repeatCount="indefinite" attributeName="', a.attribute, '" values="', a.value_prefix1,
            _get_rand_in_range_toStr(_random(abi.encodePacked(prefix, a.attribute, _seed_phrase, "1 One 1", tokenId)), a.start_min, a.start_range), ';', a.value_prefix2,
            _get_rand_in_range_toStr(_random(abi.encodePacked(prefix, a.attribute, _seed_phrase, "TWOOOOO", tokenId)), a.end_min, a.end_range), ';', a.value_prefix1,
            _get_rand_in_range_toStr(_random(abi.encodePacked(prefix, a.attribute, _seed_phrase, "1 One 1", tokenId)), a.start_min, a.start_range), 
            '" dur="',
            _get_rand_in_range_toStr(_random(abi.encodePacked(prefix, a.attribute, _seed_phrase, "DURATION", tokenId)), a.dur_min, a.dur_range), 's"/>'
        );
    }
