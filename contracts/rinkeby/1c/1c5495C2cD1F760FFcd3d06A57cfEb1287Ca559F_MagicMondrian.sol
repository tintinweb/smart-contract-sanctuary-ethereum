...
// @title Magic Mondrian (MAMO) NFT Template
...
contract MagicMondrian is Ownable, Pausable, ERC721A, PaymentSplitter {
    ...
    // Available NFT's for Whitelist Sale
    uint256 private constant MAX_WHITELIST = 10;
    // Available NFT's for Public Sale
    uint256 private constant MAX_PUBLIC = 10;
    // Reserved NFT's for Owner (airdrop, etc.)
    uint256 private constant MAX_GIFT = 5;
    // Max mint amount per transaction
    uint256 private constant MAX_MINT = 2;
    ...
    // Initial price for public sale
    uint256 public publicSalePrice = 0.005 ether;
    // Initial time for whitelist sale start in unix epoch format
    uint256 public saleStartTime = 1647834135;
    ...
    constructor(address[] memory _payeesAdresses, uint[] memory _payeesShares, bytes32 _merkleRoot, string memory _baseURI) ERC721A("Magic Mondrian", "MAMO") PaymentSplitter(_payeesAdresses, _payeesShares) {
        merkleRoot = _merkleRoot;
        baseURI = _baseURI;
        payeesAmount = _payeesAdresses.length;
    }
     ...
     * @param _proof bytes32[] merkle proof to check if sender is whitelisted
     */
    function whitelistMint(address _account, uint256 _quantity, bytes32[] calldata _proof) external payable callerIsUser {
        ...
        require(amountNFTsperWalletWhitelistSale[msg.sender] + _quantity < MAX_MINT + 1, "Maximum mint amount for Whitelist Sale exceeded");
        require(totalSupply() + _quantity < MAX_WHITELIST + 1, "Max supply for Whitelist Sale exceeded");
    ...
    function publicSaleMint(address _account, uint256 _quantity) external payable callerIsUser {
        ...
        require(price != 0, "Price is 0");
        require(currentTime() > saleStartTime + 300 minutes, "Whitelist Sale is finished");
        require(_quantity < MAX_MINT + 1, "Maximum mint amount for Public Sale exceeded");
        require(totalSupply() + _quantity < MAX_WHITELIST + MAX_PUBLIC + 1, "Max supply for Public Sale exceeded");
        require(msg.value >= price.mul(_quantity), "Not enought funds");
        _safeMint(_account, _quantity);
    }
    ...
    /**
     * @dev Possibility to set a new sale start time after deployment.
     *
     * @param _saleStartTime uint256 new start time in unix epoch format
     */
    ...
    /**
     * @dev Possibility to set a new price for the whitelist sale.
     *
    ...
    /**
     * @dev Possibility to set a new price for the public sale.
     *
     * @param _price uint256 new price
     */
    ...
    function contractURI() public pure returns (string memory) {
        return "ipfs://bafkreid7zyrwk3yb3uxc3sezoeq73jqjm3oyhched24qb4pjqv5htb7vi4/";
    }

    // Whitelist

    /**
     * @dev Set merkle root for whitelist sale.
     *
