...
contract CryptidClan is ERC721A, Ownable {
  using Strings for uint256;
  ...
  bytes32 public merkleRoot = 0xeaff3eb736d297481fed82495230a1a4b67a4a543ec461eee9a14e03d86ebb89;

  constructor(
    string memory _initBaseURI,
    string memory _initNotRevealedUri
  ) ERC721A("Cryptid Clan", "CC", maxsize) {
  ...
  function mint(uint256 tokens) public payable {
    ...
    require(publicSale, "CC: Public sale Hasn't started yet");
    uint256 supply = totalSupply();
    uint256 ownerTokenCount = balanceOf(_msgSender());
    require(tokens > 0, "CC: need to mint at least 1 NFT");
    require(tokens <= maxsize, "CC: max mint amount per tx exceeded");
    require(supply + tokens <= maxSupply, "CC: We Soldout.");
    require(ownerTokenCount + tokens <= MaxperWallet, "CC: Max NFT Per Wallet exceeded");
...
/// @dev presale mint for whitelisted
    ...
    uint256 supply = totalSupply();
    uint256 ownerTokenCount = balanceOf(_msgSender());
    require(ownerTokenCount + tokens <= MaxperWalletWl, "CC: Max NFT Per Wallet exceeded");
    require(tokens > 0, "CC: need to mint at least 1 NFT");
    require(tokens <= maxsize, "CC: max mint per Tx exceeded");
