...
/**
 * @dev A proxy for controlling access to the mint functionality of {IERC721Mintable} contracts. 
 * Each IERC721Mintable contract managed by an EtherPayableERC721Minter instance must be associated
 * with a {MintInfo}, which contains parameters that defines how and when the mint function of 
 * a managed IERC721Mintable contract can be invoked.
 *
 * The {EtherPayableERC721Minter} has been tested with instances of {NFTCollection}. It's expected
 * that an {EtherPayableERC721Minter} that manages an NFTCollection should be assigned the 
 * 'NFT_COLLECTION_MINTER_ROLE' role by the {RolesManager} which the managed NFTCollection depends,
 * so that the {EtherPayableERC721Minter} can invoke the mint function of the {NFTCollection} on behalf
 * of a externally owned account or contract that invoked the {mint} function of the {EtherPayableERC721Minter}.
 */
contract EtherPayableERC721Minter is Base {
    using SafeMath for uint256;

    /**
     * Contains the info which controls how and when the mint function of a managed {IERC721Mintable} contract can 
     * be invoked:
     *
     * 'pricePerUnit' is the price in Wei expected by an {IERC721Mintable} contract for minting one NFT
     *
     * 'totalItems' is the total number of NFT's that can be minted by an {IERC721Mintable} contract
     *
     * 'mintedItems' is the total number of NFT's that have already been minted by an {IERC721Mintable} contract
     *
     * 'startBlockNumber' the number of blocks starting from the current block number before the mint functionality
     * of a managed {IERC721Mintable} contract can be invoked
     *
     * 'paused' true if the minting of the {IERC721Mintable} contract has been
     */
    struct MintInfo {
    ...
    /**
     * @dev maps the address of a managed {IERC721Mintable} contract to its MintInfo
     */
    mapping(address => MintInfo) public mintInfo;

    /**
     * @dev the maximum number of NFT's that can be minted per mint transaction
     */
    uint256 public maxItemsPerMint;

    /**
     * @dev the address that will receive the Ether payment sent to the EtherPayableERC721Minter for 
     * ordering an {IERC721Mintable} contract to mint NFT's on behalf of a externally owned account or 
     * contract. The 'paymentReceiver' address can either be an Externally Owned Account or a contract
     */
    address payable public paymentReceiver;

    /**
     * 'settingsAddress' is the address of an {IPlatformSettings} instance.
     *
     * 'paymentReceiverAddress' is the address that will receive the Ether payment sent to the {EtherPayableERC721Minter}
     * for minting NFT's. The address can either be an Externally Owned Account or a contract
     *
     * 'maxItemsPerMintValue' is the maximum number of NFT's that can be minted per mint transaction
     */
    constructor(
        address settingsAddress,
        address payable paymentReceiverAddress,
        uint256 maxItemsPerMintValue
    ) payable public Base(settingsAddress) {
    ...
    function setPaymentReceiver(address payable newPaymentReceiver) external onlyOwner(msg.sender) {
    ...
    /**
     * sets the info which controls how and when the mint function of a managed IERC721Mintable contract can 
     * be invoked:
     *
     * 'token' is the address of the {IERC721Mintable} contract whose mint functionality is to be managed
     *
     * 'pricePerUnit' is the price in Wei expected by the {IERC721Mintable} contract for minting one NFT
     *
     * 'totalItems' is the total number of NFT's that can be minted by the {IERC721Mintable} contract
     *
     * 'mintedItems' is the total number of NFT's that have already been minted by the {IERC721Mintable} contract
     *
     * 'startBlockNumber' the number of blocks starting from the current block number before the mint functionality
     * of a managed {IERC721Mintable} contract can be invoked
     *
    ...
    function setMintInfo(
        ...
        mintInfo[token].startBlockNumber = startBlockNumber;
        mintInfo[token].paused = paused;

        emit NewMintInfo(token);
    }

    function setPricePerUnit(address token_, uint256 pricePerUnit_) external onlyOwner(msg.sender) {
        mintInfo[token_].pricePerUnit = pricePerUnit_;
        emit NewMintInfo(token_);
    }

    /**
     * Orders an {IERC721Mintable} contract to mint NFT's for the externally owned account or 
     * contract that invoked this function. Ether is expected as payment and is computed with 
     * the following formula: 'total * MintInfo.pricePerUnit'
     *
     * 'token' is the address of a managed {IERC721Mintable} contract whose mint functionality 
     * is to be invoked. The 'token' must already have been associated with a {MintInfo}.
     *
     * 'total' is the total number of NFT's to be minted by the managed {IERC721Mintable} contract 
     * referred to by 'token'
     */
    function mint(address token, uint256 total) payable external {
        require(token != address(0x0), "token required");
        ...
        require(block.number >= info.startBlockNumber, "!mint_allowed");
        ...
        for (uint256 index = 0; index < total; index++) {
            ...
            tokenIds[index] = IERC721Mintable(token).totalSupply();
        }
