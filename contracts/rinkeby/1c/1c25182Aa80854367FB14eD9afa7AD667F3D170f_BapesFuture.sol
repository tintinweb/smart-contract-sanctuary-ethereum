...
error PublicSaleIsPaused();
error MintAmountExeedsMaxPerTxLimit();
error MintAmountExceedsMaxSupply();
error NoDataExistsForProvidedTokenId();
error ParentalCertificateSaleIsPaused();
error InsufficientBalanceToMint();
error YouDoNotHaveEnoughPCsToMintSelectedAmountOfTokens();
error BapesGenesisKeySaleIsPaused();
error YouDoNotHaveEnoughBGKsToMintSelectedAmountOfTokens();
error WithdrawWalletNotSet();
error WithdrawFailed();
error PublicSupplyCanNotExceedTotalDefined();
error PrivateSaleIsPaused();
error SelectedNumberOfMintsWillExceedTheMaximumAmountOfAllowedPerWallet();
error InvalidProofThisWalletIsNotWhitelisted();
...
contract BapesFuture is ERC721A, Ownable, AccessControl, ReentrancyGuard {
  ...
  constructor(string memory _hiddenMetadataURI) ERC721A("BapesFuture", "FUTURE") {
    pc = IBapesTokens(0x69A713d35Df0788D8CC4dFd99b93B720013cce63);
    bgk = IBapesTokens(0x49d709FDE73A0B72BA858B1139B4D4FE4357c34b);
  ...
  function _mintCompliance(uint256 _mintAmount) private view {
    ...
    require(_mintAmount <= maxPerTx, "Invalid mint amount.");
    require(totalSupply() + _mintAmount <= maxSupplyTotal, "Max supply exceeded.");
  }
  ...
  function pcMint(uint256 _mintAmount) external payable mintCompliance(_mintAmount) {
    ...
    require(_mintAmount <= pc.balanceOf(_msgSender()), "You do not have enough PCs to mint selected amount of tokens.");

    uint256[] memory ownerTokens = pc.getOwnerTokens(_msgSender());
    ...
    for (uint256 i = 0; i < length; i++) {
      ...
      if (!tokensUsed[address(pc)][el]) {
        tokensUsed[address(pc)][el] = true;
    ...
    require(unused >= _mintAmount, "You have already used all PCs to mint.");
  ...
  function bgkMint(uint256 _mintAmount) external payable mintCompliance(_mintAmount) {
    ...
    require(unused >= _mintAmount, "You have already used all BGKs to mint.");
    ...
    mintCompliance(_mintAmount)
  {
    ...
    require(wlStarted, "Private sale is paused.");
