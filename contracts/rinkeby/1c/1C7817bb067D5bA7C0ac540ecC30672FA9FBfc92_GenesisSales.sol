...
contract GenesisSales is AccessControl, ReentrancyGuard {
    ...
    constructor(
        ...
        for (uint8 i = 0; i < whitelistUsers.length; i++) {
            setWhitelist(whitelistUsers[i], maxPurchasesPerUser);
        }
    }
    ...
    function setWhitelist(address user, uint16 max)
        public
        onlyRole(OPERATOR_ROLE)
    {
        maxPurchase[user] = max;
        if (!whitelist[user]) {
            whitelist[user] = true;
    ...
    function enableWhiltelist(bool active) public onlyRole(DEFAULT_ADMIN_ROLE) {
    ...
    function buyGenesisNFT(uint256 items) public payable nonReentrant {
        for (uint256 i = 0; i < items; i++) {
            ...
            require(
                currentPurchase[msg.sender] < maxPurchase[msg.sender],
        ...
        require(
            msg.value == SafeMath.add(fees, amount),
