    ...
    function _checkAssets(
        ...
        for (uint256 i = 0; i < erc721Details.length; i++) {
            ...
            for (uint256 j = 0; j < erc721Details[i].ids.length; j++) {
                require(IERC721(erc721Details[i].tokenAddr).getApproved(erc721Details[i].ids[j]) == address(this) || IERC721(erc721Details[i].tokenAddr).isApprovedForAll(offer, address(this)) == true , "ERC721 tokens must be approved to swap contract");
            }
        }
