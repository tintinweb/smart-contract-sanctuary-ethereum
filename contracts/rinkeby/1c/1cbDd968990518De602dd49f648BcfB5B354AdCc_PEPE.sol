...
contract PEPE is ERC721, ERC721Enumerable, Ownable {
    ...
    uint256 public ogMintExpireTime = 1659972600;
    uint256 public wlMintExpireTime = 1659973200;
    uint256 public pubMintExpireTime = 1659973800;
    ...
    constructor() ERC721("PepeAndFrenz", "PP") {
    ...
    function purchase(uint256 mintAmount, bytes32[] calldata _merkleProof, uint256 role) public payable mintCompliance(mintAmount) {
        ...
        if (role == 1) {
            require(MerkleProof.verify(_merkleProof, ogMerkleRoot, leaf), "PEPE:'Invalid proof!'");
            require(block.timestamp > ogMintExpireTime, "PEPE:The og sale is not enabled!");
            freeMintCount = 2 - walletMintCount[_msgSender()];
        } else if (role == 2) {
            require(MerkleProof.verify(_merkleProof, wlMerkleRoot, leaf), "PEPE:'Invalid proof!'");
            require(block.timestamp > wlMintExpireTime, "PEPE:The white-list sale is not enabled!");
            freeMintCount = 1 - walletMintCount[_msgSender()];
        } else {
            require(block.timestamp > pubMintExpireTime, "PEPE:The public sale is not enabled!");
            freeMintCount = 0;
        }
        if (mintAmount > freeMintCount){
            require(msg.value >= (mintAmount - freeMintCount) * price, "PEPE:Insufficient funds!");
        }
