...
contract cellNFT is ERC721A, Ownable, ReentrancyGuard, svgManip {
    ...
    function tokenURI(uint256 tokenId) override public view returns (string memory) {
        ...
        output.svg_data_bytes = add_gradients(tokenId_str, output.svg_data_bytes, cell.eukaryote, cell._type);
        output.svg_data_bytes = add_filters(tokenId_str, output.svg_data_bytes, cell.eukaryote, cell._type, false);
        if (cell._type == 3) {
            output.svg_data_bytes = add_filters(tokenId_str, output.svg_data_bytes, cell.eukaryote, cell._type, true);
        ...
        // Nucleus
        if (cell.eukaryote == 1) {
            output.svg_data_bytes = add_nucleus(tokenId_str, output.svg_data_bytes, cell._type);
        }

        output.svg_data_bytes = add_spooky_overlay(tokenId_str, output.svg_data_bytes, cell.eukaryote, cell._type);
        ...
        if (cell.eukaryote == 1) {
            output.json = abi.encodePacked(output.json, '{"trait_type": "Nucleus?", "value": "YES"}, {"trait_type": "Cell Type", "value": "', cell_types[cell._type], '"}, ');
        } else {
...
contract svgManip {
    ...
    function _add_path_point(string memory prefix, string memory tokenId, string memory points) view internal returns (string memory) {
        return (random(abi.encodePacked(prefix, tokenId, "ADD PATH POINT", _seed_phrase)) % 2 == 1) ? points : '';
    }

    function _get_nerve_path(string memory tokenId) view internal returns (bytes memory) {
        return abi.encodePacked(
            '<path d="M -103 162 ', 
            _add_path_point("FIRST NERVE POINT", tokenId, 'C -104 170 17 171 63 233 '), 
            _add_path_point("SECOND NERVE POINT", tokenId, 'C 109 294 78 415 84 420 '), 
            _add_path_point("THIRD NERVE POINT", tokenId, 'C 91 426 135 316 212 290 '), 
            _add_path_point("FOURTH NERVE POINT", tokenId, 'C 290 263 402 318 407 312 '), 
            _add_path_point("FIFTH NERVE POINT", tokenId, 'C 413 305 313 237 302 169 '), 
            _add_path_point("SIXTH NERVE POINT", tokenId, 'C 291 101 370 34 368 28 '), 
            _add_path_point("SEVNTH NERVE POINT", tokenId, 'C 365 22 282 77 222 57 '), 
            _add_path_point("EGITH NERVE POINT", tokenId, 'C 163 38 127 -55 118 -55 '), 
            _add_path_point("NINETH NERVE POINT", tokenId, 'C 110 -56 127 37 74 90 '), 
            'C 21 143 -102 155 -103 162', 
            '"/>'
        );
    }
    ...
    function add_gradients(string memory tokenId, bytes memory output, uint64 eukaryote, uint256 _type) view internal returns (bytes memory) {
    ...
    function add_filters(string memory tokenId, bytes memory output, uint64 eukaryote, uint64 _type, bool blur) view internal returns (bytes memory) {
    ...
    function add_nucleus(string memory tokenId, bytes memory output, uint64 _type) view internal returns (bytes memory) {
        if (_type == 3) {
    ...
    function add_spooky_overlay(string memory tokenId, bytes memory output, uint64 eukaryote, uint64 _type) view internal returns (bytes memory) {
        if (_type == 3) {
            output = abi.encodePacked(
                output, 
