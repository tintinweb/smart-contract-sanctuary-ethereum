...
contract APerpetualBob is ERC721A, Ownable {
    ...
    constructor() ERC721A("APerpetualBob", "BOB") {
    ...
    function mint(uint256 quantity) external {
        ...
        require(mintStarted);
        require((balanceOf(msg.sender) + quantity) <= MAX_WALLET);
        _safeMint(msg.sender, quantity);
    }

    function addScene(bytes memory _data, uint256 destLen) external onlyOwner() {
        scenes.push(Scene(SSTORE2.write(_data), destLen));
    }

    function setBob(bytes[2] memory _newBob) external {
        bob.push(SSTORE2.write(abi.encode(_newBob)));
    }
    ...
    function tokenURI(uint256 tokenId) override public view returns (string memory) {
        ...
        uint256 _sceneLength = scenes.length;
        uint256 _selectedScene = ((block.timestamp-deployDate)/720+(tokenId-1)) % _sceneLength;
        Scene memory _currentScene = scenes[_selectedScene];
        (, bytes memory _toDecode) = inflateLib.puff(SSTORE2.read(_currentScene.data), _currentScene.destLen);
        (bytes memory _scene, string[] memory _text) = abi.decode(_toDecode, (bytes, string[]));

        string memory _outString = string.concat('data:application/json,', '{', '"name" : "Bob ' , Strings.toString(tokenId), '", ',
            '"description" : "Just a presentation with some information!"');
        bytes memory _renderedScene = _assembleExternalScene(_scene, _selectedScene);
        _renderedScene.appendSafe(_renderText(_text));
        _outString = string.concat(_outString,',"image": "data:image/svg+xml;base64,',
            Base64.encode(_renderedScene),'"}');
        return _outString;
    }
    ...
    function _assembleExternalBob(uint256 _sceneNr) private view returns(bytes memory) {

        bytes memory buffer = DynamicBuffer.allocate(2**18);
        if(_sceneNr == 9) {
            bytes[2] memory _bob = abi.decode(SSTORE2.read(bob[1]), (bytes[2]));
            buffer.appendSafe(gfx.drawPixelsUnsafeWithOffset(_bob[0], 100, 19));
            buffer.appendSafe(gfx.drawPixelsUnsafeWithOffset(_bob[1], 80, 2));
        } else {
            bytes[2] memory _bob = abi.decode(SSTORE2.read(bob[0]), (bytes[2]));
            if(_sceneNr == 0) {
            buffer.appendSafe(gfx.drawPixelsUnsafeWithOffset(_bob[0], 50, 19));
            buffer.appendSafe(gfx.drawPixelsUnsafeWithOffset(_bob[1], 30, 2));
            } else {
                buffer.appendSafe(gfx.drawPixelsUnsafeWithOffset(_bob[0], 100, 19));
                buffer.appendSafe(gfx.drawPixelsUnsafeWithOffset(_bob[1], 80, 2));
            }
    ...
    function _assembleExternalScene(bytes memory _data, uint256 _sceneNr) private view returns (bytes memory) {
        bytes memory buffer = DynamicBuffer.allocate(2**18);
        buffer.appendSafe(bytes('<svg xmlns="http://www.w3.org/2000/svg" shape-rendering="crispEdges" version="1.1" viewBox="0 0 128 80" width="640" height="400"><defs><style type="text/css">@import url("http://fonts.cdnfonts.com/css/smw-text-nc");</style></defs>"'));
        buffer.appendSafe(bytes('<rect fill="#e1d7d5" x="0" y="0" height="48" width="128"/>'));
        if(_sceneNr != 0) {
            if(_sceneNr == 3) {
                buffer.appendSafe(gfx.drawPixelsUnsafeWithOffset(_data, 89, 0));
            } else {
                buffer.appendSafe(gfx.drawPixelsUnsafeWithOffset(_data, 0, 0));
            }
        }
        buffer.appendSafe(_assembleExternalBob(_sceneNr));
        if(_sceneNr == 9) {
            buffer.appendSafe('<rect width="128" height="48" fill="#0019B1" opacity="0.400000" />');
        }
        buffer.appendSafe(bytes(string.concat('<text x="0" y="2" font-family="SMW Text NC" font-size="2">Scene #', Strings.toString(_sceneNr+1), '</text>')));
        return buffer;
    }

    function _renderText(string[] memory _text) private pure returns (bytes memory) {
        bytes memory buffer = DynamicBuffer.allocate(2**18);
        buffer.appendSafe(bytes(string.concat('<text x="2" y="50" font-family="SMW Text NC" font-size="3">')));
        for(uint8 i = 0; i < _text.length; i++ ) {
            if( i == 0 ) {
                buffer.appendSafe(bytes(string.concat('<tspan x="0" dy=".6em">', _text[i], '</tspan>')));
                //console.log(_text[i]);
            } else {
                buffer.appendSafe(bytes(string.concat('<tspan x="0" dy="1.2em">', _text[i], '</tspan>')));
                //console.log(_text[i]);
            }
        }
        buffer.appendSafe(bytes('</text></svg>'));
        return buffer; 
    }
}
