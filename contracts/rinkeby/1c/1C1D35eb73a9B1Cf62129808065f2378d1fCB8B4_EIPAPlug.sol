...
contract EIPAPlug is Ownable, AccessControl {
    ...
    function freeMint(uint tokenId, uint amount) external {
        require(tokenMintInfo[tokenId].mintActive, "Mint is not active");
    ...
    function _mint(address to, uint tokenId, uint amount) internal {
        require(amount > 0, "Must request more than 0");
        require(msg.sender == tx.origin, "No smart contracts");
        require(eipa.totalSupply(tokenId) + amount <= tokenMintInfo[tokenId].maxSupply, "No more mints available");
