...
contract KIWI is Ownable {
	...
	IBEP20 public tokenAddress = IBEP20(0x1Cb5327DeCe362BFC063f943dD3eF8Ea1542522e); 
	...
	uint256 public unlockTimeStamp = 1680248538;
	
    bool public buyEnable = false;
	bool public saleEnable = false;
	bool public buyTokenLocked = true;
	
	KiwiToken public KTOKEN = KiwiToken(0x1Cb5327DeCe362BFC063f943dD3eF8Ea1542522e);
    ...
    function buy(uint256 amount) external returns (bool) {
		...
		uint256 baseTokenRequired = amount.mul(baseRate).div(BuyPrice);
		...
		baseToken.safeTransferFrom(buyer, owner(), baseTokenRequired);
		tokenAddress.safeTransferFrom(owner(), buyer, amount);
		
		users[buyer].investment = users[buyer].investment + baseTokenRequired;
	...
	function sell(uint256 amount) external returns (bool) {
		...
		uint256 baseTokenRequired = amount.mul(baseRate).div(SalePrice);
		...
		baseToken.safeTransferFrom(owner(), seller, baseTokenRequired);
