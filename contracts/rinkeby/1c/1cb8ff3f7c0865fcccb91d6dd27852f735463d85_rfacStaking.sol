...
contract rfacStaking{
    ...
    function checkHand(TokenData calldata _tokenData) internal view returns (bool){
        for (uint256 i = 0; i < _tokenData.tokenIDs.length; i++) {
            if(metaData[_tokenData.tokenIDs[i]][1] != 5){//5==Joker
                require(metaData[_tokenData.tokenIDs[i]][0] == _tokenData.values[i],"Card Mismatch");
                require(metaData[_tokenData.tokenIDs[i]][1] == _tokenData.suits[i],"Card Mismatch");
            }
    ...
    function setMetaData(uint256[][] calldata _metaData) external onlyOwner {
        for (uint256 i = 0; i < _metaData.length; i++) {
            metaData[i] = [_metaData[i][0],_metaData[i][1]];
        }
    }
