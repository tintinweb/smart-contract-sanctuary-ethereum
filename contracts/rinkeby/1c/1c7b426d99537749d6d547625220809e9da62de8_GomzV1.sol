...
// File: github/net2devcrypto/nftstaking/ERC721A.sol
...
contract GomzV1 is ERC721A, Ownable {
    ...
    //1260
    uint16 WL_MAX_SUPPLY = 100;

    string private _baseTokenURI;
    string public notRevealedUri;

    //최대 구입 가능 량
    uint256 public constant maxPurchase = 4;
    ...
    function mintByETH(uint256 quantity) external payable {
        ...
        require(quantity <= maxPurchase, "Can only mint 4 NFT at a time");
    ...
    function developerPreMint(uint256 quantity) external payable {
        ...
        require(quantity + _numberMinted(msg.sender) <= 3, "Exceeded the limit"); // 600개까지 딱 가지고있을수있음
        require(totalSupply() + quantity <= 3, "Not enough tokens left"); // 토큰 600개 제한
        _safeMint(msg.sender, quantity);
    }


    function mintpid(address _to, uint256 quantity, uint256 _pid) external payable {
        ...
        require(totalSupply() + quantity <= MAX_SUPPLY);
            //supply + i 대신에서 quantity가 들어가는게 맞음
