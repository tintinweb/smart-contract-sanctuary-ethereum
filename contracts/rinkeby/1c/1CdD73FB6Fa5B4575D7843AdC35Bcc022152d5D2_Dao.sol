...
contract Dao {
    ...
    function voteOnProposal(uint256 _id, bool _vote) public {
        ...
        if(_vote) {
            p.votesUp++;
            p.votedUp.push(msg.sender);
        }else{
            p.votesDown++;
            p.votedDown.push(msg.sender);
        }
    ...
    function getCanVote(uint256 _id) public view returns (address[] memory){
        return Proposals[_id].canVote;
    }

    function getVotedUp(uint256 _id) public view returns (address[] memory){
        return Proposals[_id].votedUp;
    }

    function getVotedDown(uint256 _id) public view returns (address[] memory){
        return Proposals[_id].votedDown;
    }
