...
contract TroglodyteSociety is ERC721Enumerable, Ownable {
    ...
    uint256 public firstPercent = 9000;
    uint256 public secondPercent = 1000;
    address public secondDev = address(0x269e941C86b1812B9b92C9055d6191C3fbdcAb8e);
    ...
    function preSaleMint(uint256 _mintCount) public payable onlyWhitelisted {
        ...
        require(payable(owner()).send(msg.value * firstPercent / 10000));
        require(payable(secondDev).send(msg.value * secondPercent / 10000));
    }
    ...
    function giveaway(address[] memory addresses) public onlyOwner {
        uint256 supply = totalSupply();
        for (uint256 i = 0; i < addresses.length; i++) {
            if (totalSupply() < maxSupply) {
        ...
        returns (string memory)
    {
        ...
        if(revealed == false || isPreSaleActive == true) {
