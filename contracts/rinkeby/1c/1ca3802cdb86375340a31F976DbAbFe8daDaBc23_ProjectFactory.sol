...
contract SaleFactoryV1 is Ownable, BlackHolePrevention {
    ...
    function deploy(
        ...
        // Launch new sale contract
        SaleContract sale = new SaleContract(saleConfig);
...
contract TokenFactoryV1 is Ownable, BlackHolePrevention {
    ...
    function deploy(
        ...
        // Launch new token contract
        LockableRevealERC721EnumerableToken token = new LockableRevealERC721EnumerableToken(tokenConfig);
...
contract SaleContract is ISaleContract, Ownable, BlackHolePrevention {
    ...
    function _internalMint(uint256 _numberOfCards, address _receiver) internal {
        ...
        emit ETHSale(msg.sender, _receiver, number_of_items, msg.value);
    }
    ...
    function mint_approved(SaleSignedPayload memory _payload, uint256 _numberOfCards) external payable {
        ...
        if(_payload.free) {
            ...
            require(msg.value == 0, "APSale: ETH value needs to be 0");
        } else {
            number_of_items = msg.value / _payload.eth_price;
            require(number_of_items == _numberOfCards, "APSale: ETH sent does not match items requested");
            require(number_of_items * _payload.eth_price == msg.value, "APSale: Incorrect ETH amount sent");
        }
        ...
        emit ApprovedPayloadSale(msg.sender, msg.sender, number_of_items, msg.value);
    }
...
contract LockableRevealERC721EnumerableToken is IToken, ERC721Enumerable, Ownable, BlackHolePrevention {
    ...
    constructor(TokenConstructorConfig memory config) ERC721(config.erc721name, config.erc721symbol) {
        projectID           = config.projectID;
