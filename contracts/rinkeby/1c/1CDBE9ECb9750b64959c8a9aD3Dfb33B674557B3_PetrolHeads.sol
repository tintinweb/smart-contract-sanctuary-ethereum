...
contract PetrolHeads is Ownable, Pausable, RandomlyAssigned, ERC721Enumerable {
    ...
    /// @notice if a claim code has been used already
	/// @param claimCode - the claim code to check if already used
    function isClaimCodeAlreadyUsed(string memory claimCode) external view returns (bool) {
        return
			whitelistClaimed[claimCode];
    }	
