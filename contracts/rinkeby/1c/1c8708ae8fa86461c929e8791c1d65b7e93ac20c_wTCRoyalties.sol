...
contract wTCRoyalties is Context, Ownable {
    ...
    address public wrappednft = 0xb4dd2D7C084196cFC25C95E01Cf01482Db3794d1;
    address public dbcontract = 0x55263ef75bC2D973670055aDf627Edd16A2beF59;
    address public Owner;
    address payable public multisig = 0x0E26a3EFDBC7584f791fc9eDebdFD126D6A29507;
    ...
    uint public treasurypercent = 50; //default to 50% 50% to holders
    uint public holderpercent = 50; //% for holders
    uint public charitypercent = 0; //% for Charity if needed
    bool public royaltydistribution = true; ///Enable/Disable Rewards
    ...
    bool public masspayoutenabled; // A reflection for the users if needs be!
  ...
  ///Sets the fees
  function setFees(int option,uint _value) public onlyOwner
  {
      if (option==1)
      {
          joiningfee = _value;
  ...
  function setDBAddress(address _dbaddress) public onlyOwner {
   dbcontract = _dbaddress;
  }

    ///Function to enable/disable the Royalty Payouts
    function onoffRoyalties(bool _enabled) public onlyOwner{
   ...
   ///Function for user to check the amount of ETH roughly to be paid out
    function rewardOf() public view returns(uint)
   {
       uint temp;
       temp = fixedpayoutamount;
       if (block.timestamp > lastrebalancetime + 1 minutes )
       {
           temp = 9999; ///A rebalance is due!
       }
    return temp;
   ...
   function claim() external {
       ...
       require(block.timestamp > lastpaid + 1 minutes,"You are only able to claim once every 24 hours!");
       } 
    ...
    ///Rebalance function which can be run manually (anybody!)
    ...
    function Stats(uint option) view public returns(uint)
    {
        uint temp;
        if (option==1)
        {
