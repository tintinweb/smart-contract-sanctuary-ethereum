...
contract UtherTrunksMinter is Ownable {
  ...
  mapping(address => mapping(uint256 => uint256)) public usersMinted;
  UtherTrunks public nftContract;

  constructor(
    address nftContractAddress,
    address _signer
  ){
    nftContract = UtherTrunks(nftContractAddress);
    signer = _signer;
  }
  ...
  function mint(
    ...
    require(mintEnabled, "not started");
    require(msg.value >= price(_count), "value");
    bytes32 hash = keccak256(abi.encodePacked(_to, id, totalAmount));
    hash = hash.toEthSignedMessageHash();
    address sigSigner = hash.recover(sig);
    require(sigSigner == signer, "!sig");
    require(_count + usersMinted[_to][id] <= totalAmount, "exceeded balance");
    nftContract.privateMint(_to, id, _count, data);
    usersMinted[_to][id] += _count;
  }
  ...
  function updateNftContrcat(UtherTrunks _newAddress) public onlyOwner {
    nftContract = UtherTrunks(_newAddress);
  }
