...
contract SVGStaking is ERC20, ReentrancyGuard {
    SVGNFT nftToken;
    ...
    constructor(address nftContractAddress)
        ...
        ReentrancyGuard()
    {
        nftToken = SVGNFT(nftContractAddress);
...
contract SVGFactory {
    uint256 public immutable maxPathCount;
    uint256 public immutable minPathCount;
    uint256 public immutable maxPathCommandCount;
        ...
        returns (string memory pathSVG)
    {
        ...
        // if (binary == 0) {
        //     pathSVG = string(
        //         abi.encodePacked(pathSVG, "' fill='", color, "'/>")
        //     );
        // } else {
        pathSVG = string(
            abi.encodePacked(
...
contract Lockable {
    ...
    function lockToken(
        ...
        address toCompareAgainst;
        if (_proxy) {
            toCompareAgainst = tx.origin;
        } else {
            toCompareAgainst = msg.sender;
        }
        require(
            toCompareAgainst == tokenContract.ownerOf(_tokenId),
