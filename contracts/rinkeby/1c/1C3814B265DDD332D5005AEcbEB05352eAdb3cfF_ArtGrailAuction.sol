...
contract ArtGrailAuction is Initializable, ReentrancyGuardUpgradeable, OwnableUpgradeable {
    ...
    /// @notice ERC721 Address -> Royalty amount
    mapping(address => uint8) public royalties;
    
    /// @notice ERC721 Address -> Royalty recipient address
    ...
    // */
    // modifier whitelistedProjects(address _projectAddress) {
    //     require(whitelistedMap[_projectAddress] == true);
    //     _;
    // }
    ...
    ) external whenNotPaused isProjectWhitelisted(_nftAddress, _hexProof){
    ...
    function resultAuction(address _nftAddress, uint256 _tokenId) external nonReentrant {
        ...
        if (winningBid > auction.reservePrice) {
            ...
            if (royalties[_nftAddress] > uint8(0)){
                (bool royalityTransferSuccess, ) = royaltyRecipient[_nftAddress].call{ value: royalties[_nftAddress] }("");
    ...
    /**
     @notice Method for getting next bid amount
     @param _nftAddress Address of NFT in auction
     @param _tokenId Token ID of the NFT being auctioned
     */
    function getNextMinBid(address _nftAddress, uint256 _tokenId) external view returns (uint256) {
        HighestBid storage highestBid = highestBids[_nftAddress][_tokenId];
        Auction storage auction = auctions[_nftAddress][_tokenId];
    ...
    //     /**
    //  @notice Used for adding an array of project addresses into whitelist
    //  @param _addresses Addresses that will be whitelisted
    //  */
    // function addBulkProjectAddress(address[] memory _addresses) public onlyOwner {
    // for (uint i = 0; i < _addresses.length; i++) {
    //     require(whitelistedMap[_addresses[i]] != true);
    //     whitelistedMap[_addresses[i]] = true;
    //     emit ContractWhitelisted(_addresses[i], true);
    //     }
    // }

    // /**
    //  @notice Used for removing an array of project addresses from whitelist
    //  @param _addresses Addresses that will be removed from whitelist
    //  */
    // function removeBulkProjectAddress(address[] memory _addresses) public onlyOwner {
    // for (uint i = 0; i < _addresses.length; i++) {
    //     require(whitelistedMap[_addresses[i]] == true);
    //     delete whitelistedMap[_addresses[i]];
    //     emit ContractWhitelisted(_addresses[i], false);
    //     }
    // }
