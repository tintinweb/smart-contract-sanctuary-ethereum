...
contract MultiSigWallet {
    ...
    function addSigners(address _owner) public onlyOwner {
        require(_owner != address(0), "Invalid address");
        require(!isOwner[_owner], "Not unique");

        isOwner[_owner] = true;
        owners.push(_owner);

        required = (owners.length / 2) + 1;
    }
    ...
    function execute(uint256 _txId) external txExist(_txId) notExecuted(_txId) {
        require(_getApprovalCount(_txId) >= required, "Approvel < required");
    ...
    function erc20Bal(ERC20 token) public view returns (uint256 bal) {
