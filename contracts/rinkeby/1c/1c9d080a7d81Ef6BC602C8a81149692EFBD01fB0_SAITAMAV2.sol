...
/*marketingTokenFeeWallet;
marketingETHFeeWallet;
*/

contract SAITAMAV2 is Context, IERC20, Ownable {
    ...
    address public marketingTokenFeeWallet;
    address public marketingETHFeeWallet;
    ...
    constructor(address _marketingTokenFeeWallet, address _marketingETHFeeWallet, address uniRouter ) {
        marketingETHFeeWallet = _marketingETHFeeWallet;
        marketingTokenFeeWallet = _marketingTokenFeeWallet;
    ...
    function _distributeFee(
        ...
        _rOwned[marketingTokenFeeWallet] = _rOwned[marketingTokenFeeWallet].add(
            rmarketingTokenFee
        );
        if (_isExcluded[marketingTokenFeeWallet]) {
            _tOwned[marketingTokenFeeWallet] = _tOwned[marketingTokenFeeWallet]
                .add(tmarketingTokenFee);
        }
    }
        ...
        returns (TValuesStruct memory)
    {
        ...
        uint256 tTransferAmount = tAmount
            .sub(tReflectionFee)
            .sub(tBurnFee)
            .sub(tmarketingTokenFee)
            .sub(tMarketingETHFee);
        return
            TValuesStruct(
                tTransferAmount,
                tReflectionFee,
    ...
    function _getRValues(
        ...
        uint256 rTransferAmount = rAmount
            .sub(rReflectionFee)
            .sub(rMarketingETHFee)
            .sub(rBurnFee)
            .sub(rmarketingTokenFee);
        return
            RValuesStruct(
                rAmount,
                rTransferAmount,
    ...
    function calculatemarketingTokenFee(uint256 _amount)
        private
        view
        returns (uint256)
    {
        return _amount.mul(_marketingTokenFee).div(10**2);
    }

    function calculateMarketingETHFee(uint256 _amount)
        private
        view
        returns (uint256)
    {
        return _amount.mul(_marketingETHFee).div(10**2);
    }
    ...
    function _transfer(
        ...
        bool overMinTokenBalance = contractTokenBalance >=
            _numTokensSwapToETHForMarketing;
        if (
            overMinTokenBalance &&
            !inMarketingEthSwap &&
            from != uniswapV2Pair &&
    ...
    function _tokenTransfer(
        ...
        _distributeFee(
            vs.rReflectionFee,
            vs.rBurnFee,
            vs.rmarketingTokenFee,
            vs.tReflectionFee,
            vs.tBurnFee,
            vs.tmarketingTokenFee
        );
    ...
    function setMaxTxAmount(uint256 maxAmountInTokensWithDecimals)
        external
        onlyOwner
    {
        require(
            maxAmountInTokensWithDecimals > 100 * 10**6 * 10**9,
            "Cannot set transaction amount less than 0.1 percent of initial Total Supply!"
        );
    ...
    function setnumTokensSwapToETHForMarketing(uint256 tokenAmount)
        external
        onlyOwner
    {
