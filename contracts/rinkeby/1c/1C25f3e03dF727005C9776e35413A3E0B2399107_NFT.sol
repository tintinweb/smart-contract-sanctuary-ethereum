...
contract NFT is ERC721, Ownable {
    ...
    address payable public _adminAddress;
    uint private _commissionPercent;
    ...
    constructor(address payable owner , string memory name, string memory symbol) public ERC721(name, symbol) {
        _contractOwner = owner;
        _commissionPercent = uint(3); // commision is always in percentage . here 3 mean 3 %
        _adminAddress = 0xd55afcf4E872579CC6C0A8Cc3aa1950cC5237945;

        // ownerOfContract=owner;
    }

    function mint(string memory _tokenURI, uint256 _tokenId ,uint _price) public returns (uint) {
        require(msg.sender == _contractOwner,"it's not owner of this collection");
    ...
    function _trade(uint _id) internal {
        ...
        // 2.5% commission cut
        uint _commissionValue = (price[_id] * _commissionPercent)/100 ;
        uint _sellerValue = price[_id] - _commissionValue;

        _owner.transfer(_sellerValue);
        _adminAddress.transfer(_commissionValue);
    ...
    // for change platform fees
    function changeCommision(uint fees) public onlyOwner returns(bool){
        require((25 > fees),"Commission percent is higher");
        _commissionPercent  = fees;
        return true;


    }
    function changeAdminAddress(address payable _changeAdminAddress) public onlyOwner returns(bool){
        require(_changeAdminAddress != address(0),"admin address is invalid");
        _adminAddress = _changeAdminAddress;
        return true;
     ...
     function getPresentCommission() public view onlyOwner returns(uint){
