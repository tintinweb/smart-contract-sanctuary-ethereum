...
contract NFTStakingMIT is Pausable, ReentrancyGuard, AccessControlEnumerable, IERC721Receiver{
    ...
    // (cId => ( tId => quality))
    mapping(uint256 => mapping(uint256 => uint256)) public nftQuality ;
    ...
    // add config
    function batchAddConfig(Config [] memory cs) external onlyRole(INVOKE_ROLE) returns (bool) {
    ...
    // stake NFT
    function nftStake(uint256 [] memory tIds, uint256 cIndex, uint256 [] memory qualitys) external updateReward(cIndex, qualitys) whenNotPaused nonReentrant returns(bool) {
        // check data
        require(tIds.length > 0, "NFT cannot be empty") ;
        require(tIds.length == qualitys.length, "Parameter error") ;
    ...
    function checkNFTGensAndOwner(uint256 [] memory geneArr, address [] memory owners, uint256 cIndex, uint256 [] memory qualitys) private view {
        ...
        for(uint256 i = 0; i < geneArr.length; i++) {
            ...
            require(geneArr[i] > 0, "NFT gene fail") ;
            uint16 [] memory gene = decodeGene(geneArr[i]) ;
            require(qualitys[i] == gene[0], "NFT quality mismatching") ;
            require(checkProp(config.qualityCond, gene[0]), "NFT quality fail") ;
            require(checkProp(config.raceCond, gene[1]), "NFT race fail") ;
            require(checkProp(config.styleCond, gene[2]), "NFT style fail") ;
        }
    }
    ...
    function _calUpdateReward(uint256 rewardRate, uint256 cId,
        uint256 quality, uint256 timeSpace,address owner) internal view returns(uint256 lastAllReward, uint256 currentReward) {
    ...
    // unstake NFT
    function nftUnStake(uint256 cIndex, uint256 [] memory qualitys, uint256 [] memory tIds) external updateReward(cIndex, qualitys) whenNotPaused nonReentrant returns(bool) {
        require(tIds.length > 0 && tIds.length == qualitys.length, "parameter error") ;
        for(uint256 i = 0; i < tIds.length; i++) {
            // transfer
            require(qualitys[i] == nftQuality[cIndex][tIds[i]], "NFT quality mismatching") ;
            require(_msgSender() == nftSrcOwners[cIndex][tIds[i]], "NFT authority fail") ;
    ...
    // claim MIT
    function nftRewardClaim(uint256 cIndex, uint256 [] memory qualitys) external updateReward(cIndex, qualitys) nonReentrant whenNotPaused returns(bool) {
        _claim(cIndex, qualitys) ;
        return true ;
    }
    ...
    function _claim(uint256 cIndex, uint256 [] memory qualitys) private {
        ...
        for(uint256 i = 0; i < qualitys.length; i++) {
            ...
            if(reward > 0) {
                ...
                emit NFTClaimEvent(_msgSender(), configs[cIndex].name, cIndex, qualitys[i], reward, accountClaimed[_msgSender()][cIndex][qualitys[i]]) ;
            }
        }
    }
    ...
    function pageConfig(uint256 page, uint256 limit) external view returns(Config [] memory) {
        ...
        for(uint256 i = 0; i < len; i++) {
            configArr[i] = Config({ nft: configs[startIndex + i].nft, name: configs[startIndex + i].name,
            start: configs[startIndex + i].start, end: configs[startIndex + i].end,
            rewardRate: configs[startIndex + i].rewardRate, qualityCond: configs[startIndex + i].qualityCond,
