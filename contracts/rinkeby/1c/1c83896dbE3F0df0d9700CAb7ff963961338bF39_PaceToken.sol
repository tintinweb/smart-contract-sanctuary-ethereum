...
contract PaceToken is ERC20, AccessControl {
    ...
    modifier onlyRole(bytes32 role) {
        require(hasRole(role, msg.sender), "PaceToken:You not be able to do that");
        _;
    }

    function mint(address _to, uint256 _amount) public onlyRole(MINTER_ROLE) {
        require(totalSupply().add(_amount) <= cap(), "PaceToken::Exceeds max cap!");
        _mint(_to, _amount);
    }
