...
contract RewardRance is Tuktukfactory {
    ...
    function getExpByCar(address _owner,uint256 productId)
        external
        view
        returns (uint16)
    {
...
contract Tuktukfactory is ProductDetails, ERC721 {
    ...
    function createRandomCar() public {
    ...
    function claimExp(uint16 exp,uint256 productId)
        public
    {
        cars[productId].exp += exp;
        
    }

    function Levelup(uint256 productId)
        public
    {
        if(cars[productId].exp >= 100){
            cars[productId].level += 1;
            cars[productId].power += 2;
            cars[productId].speed += 2;
            cars[productId].drift += 2;
            cars[productId].exp = 0;
        } 
        
    }

    function saleCar(uint16 productId,uint16 price)public{
        if(cars[productId].price == 0){
            cars[productId].price = price;
        }
    }

    function cancelSale(uint16 productId)public{
        if(cars[productId].price != 0){
            cars[productId].price = 0;
        }
    }

    function transferCar(address buyer,address seller,uint16 productId)public{
        if(cars[productId].owner == seller){
            cars[productId].owner = buyer;
            carToOwner[productId] = buyer;
        }
    }

    function refuel(address _owner)public{
        for (uint256 i = 0; i < cars.length; i++) {
            if (cars[i].fuel == 0 && carToOwner[i] == _owner) {
                cars[i].fuel += 15;
            }
        }
    }

    function getCarForSale(address _owner)
        external
        view
        returns (uint256[] memory)
    {
