...
contract GiveAwayVault is Ownable, ERC721Holder, IERC1271 {
    ...
    function setManagementContract(address managementContract_) external onlyOwner {
        require(managementContract_ != address(0), "GiveAwayVault: should be not null");
        managementContract = managementContract_;
    }

    function setPartnerContract(address partnerContract_) external onlyOwner {
        require(partnerContract_ != address(0), "GiveAwayVault: should be not null");
    ...
    function open() external onlyOwner {
        require(status == Status.Pending, "GiveAwayVault: not pending");
        status = Status.Open;
        snapshotId = poolDistributor.snapshot();
    }

    function airdrop(uint256 tokens, address account) external onlyOwner {
        require(status == Status.Open, "GiveAwayVault: not enabled");
        poolNft.mint(account, tokenId, tokens);
    }

    function lock() external onlyOwner {
        require(totalSupply() > 0, "GiveAwayVault: no NFT holders");
        require(status == Status.Open, "GiveAwayVault: contract not open");
        status = Status.Locked;
    }

    function close() external onlyOwner {
        require(managementContract != address(0), "GiveAwayVault: management contract not set");
        require(partnerContract != address(0), "GiveAwayVault: partner contract not set");
        require(status == Status.Locked, "GiveAwayVault: not locked");
        ...
        (bool managementContractSuccess,) = payable(managementContract).call{value : managementFee}("");
        ...
        (bool partnerContractSuccess,) = payable(partnerContract).call{value : partnerFee}("");
    ...
    function claimable(address investor) public view returns (uint256) {
        return (address(this).balance * balanceOf(investor)) / totalSupply();
    }

    function claim() external {
        require(status == Status.Closed, "GiveAwayVault: claim not available");
    ...
    function prepareOpenSea() external onlyOwner {
        ...
        require(wrappedEther.approve(openSeaTokenProxy, type(uint).max), "GiveAwayVault: error approving WETH");
    }
