  ...
  UUPSUpgradeable
{
  ...
  bytes32 public constant UNBLACKLISTER_ROLE = keccak256('UNBLACKLISTER_ROLE');
  ...
  function isRedemptionAddress(address account) internal pure returns (bool) {
    return
      uint256(uint160(account)) < REDEMPTION_ADDRESS_COUNT &&
      uint256(uint160(account)) != 0;
  }
