...
contract Blast is Ownable , VRFConsumerBase , Division{
    ...
    function fulfillRandomness(bytes32 _requestId, uint256 _randomness) internal override{
        ...
        getRatio(_randomness);
        blastPayment();
        randomness = _randomness;
    }

    function getRatio(uint256 _randomness) internal onlyOwner returns(string memory){
        require(blast_status == BLAST_STATUS.CALCULATING && _randomness > 0, "blast is not ended!!!");
        uint256 modulus = _randomness % 1000;
        uint256 top;
        uint256 down;
        if (modulus <= 19 && modulus >= 0 ){
            ...
            return Division.division(modulus-down+(top-down) , top-down);
        }
...
contract Division {
        ...
        )
    {
        ...
        uint256 remainder = ((numerator * 100) / denominator) % 100;
        result = string(
            abi.encodePacked(quotient.toString(), ".", remainder.toString())
        );
    }
}
