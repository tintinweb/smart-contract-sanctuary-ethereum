...
contract AGOVLPStaking is Initializable, OwnableUpgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    uint256 public constant NAME_HASH = 0x41963fde314a645377028b1adf3bcbda1c1b9a87ea74a79cfa0308ddd78f3379;
    ...
    function initialize(
        ...
        if (_agovToken == address(0) || _treasury == address(0)) revert ZeroAddress();
    ...
    function deposit(
        ...
        if (!_verify(_msgSender(), round, agovInAmount, lock, apr, timestamp, signature)) revert FailedVerify();

        // convert eth to weth
        IWETH(wethToken).deposit{ value: (msg.value) }();

        // create a new lp position wrapped in a NFT
        (uint256 tokenId, uint128 liquidity, uint256 ethOutAmount, uint256 agovOutAmount) = _mintPosition(
            msg.value,
            agovInAmount
        );
     ...
     *
     * TODO: LLC reward should be integrated
     * delete will change position of deposit id
     */
    function withdraw(uint256 depositId) external whenNotPaused nonReentrant {
        ...
        nonfungiblePositionManager.transferFrom(address(this), _msgSender(), depositId);

        // TODO: LLC reward
        uint256 llcRewardAmount = _addLLCReward(_msgSender(), staking.ethAmount, staking.lock);

        // delete user deposit data
        delete deposits[depositId];

        emit Withdraw(_msgSender(), depositId, staking.id, llcRewardAmount, reward);
    }
    ...
    function rewardOf(uint256 depositId) public view returns (uint256 reward) {
        Deposit memory lp = deposits[depositId];
        reward = (lp.agovAmount * lp.apr * (block.timestamp - lp.timestamp)) / (365 days) / 10000;
    }
     ...
     */
    function balanceOf() external view returns (uint256 agovAmount, uint256 mpwrAmount) {
     ...
     * @param ethAmount eth amount in Deposit to calculate reward amount
     * @param lock LP lock period - 90, 180
     * @return claimable LLC reward amount
     */
    function _addLLCReward(
        address who,
        uint256 ethAmount,
        uint256 lock
    ) internal returns (uint256) {
        ILLCGift llc = ILLCGift(llcGift);
        uint256 amountPerLock = lock == 180 days ? 2 : 1;
        uint256 llcRewardAmount = amountPerLock * (ethAmount / 1 ether);
        uint256 claimableLLCs = llc.claimers(who) + llcRewardAmount;
        llc.addClaimer(who, claimableLLCs);
        return llcRewardAmount;
    }
     ...
     * @param _amount AGOV token amount which was deposited from klaytn
     ...
     *
     * TODO: make it internal after test
     */
    function _verify(
        address _who,
        uint256 _round,
        ...
        bytes32 signedHash = keccak256(abi.encodePacked(_who, NAME_HASH, _round, _amount, _lock, _apr, _timestamp));
