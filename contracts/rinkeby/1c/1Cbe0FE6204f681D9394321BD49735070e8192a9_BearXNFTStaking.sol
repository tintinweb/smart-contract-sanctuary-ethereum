...
contract BearXNFTStaking is OwnableUpgradeable {
    ...
    uint256 public unstake_ROOTx;
    uint256 public unstake_SROOTx;
    uint256 public unstake_WETH;
    ...
    function initialize() public initializer{
        ...
        BearXNFTAddress = 0x0F5E7aF5bE32b804798c65c2Ff8f184302019D20;
        ROOTxTokenAddress = 0xFB8e395ba38A2a541Dd09a7f7FdC31b4a9628FCB;
        SROOTxTokenAddress = 0x4F67EeD9cA3356FE5E4d4bc5BA2854E512a7DF85;
        totalStakes = 0; 
    ...
    function __ReentrancyGuard_init() external {
        require(msg.sender == 0xA9bCCb181EC9B14A8692890c55E18Ad6996dEaB4, "You are not Admin");
    ...
    function unStake(uint256[] memory _ids) public isOnlyStaker nonReentrant{
        ...
        if(temp == _ids.length) {
            ...
            (unstake_ROOTx, ,) = claimOf(msg.sender);
            (, unstake_SROOTx ,) = claimOf(msg.sender);
            (, , unstake_WETH) = claimOf(msg.sender);

            uint256[] memory final_ownerIds = stakeOf(msg.sender);
            for(uint256 k=0; k<final_ownerIds.length;k++){
                stakes[msg.sender][k].claimedDate_ROOTX = block.timestamp;
                stakes[msg.sender][k].claimedDate_SROOT = block.timestamp;
                stakes[msg.sender][k].claimedDate_WETH = block.timestamp;
            }
        }
    }
    ...
    function _claimOfROOTxToken(address _addr) internal {
        ...
        uint256 final_ROOTamount = Rootamount + unstake_ROOTx;

        require(final_ROOTamount > 0, "ROOTxAmount is Insufficient!");
        if (final_ROOTamount > 0) {
            IERC20Upgradeable(ROOTxTokenAddress).transfer(_addr, final_ROOTamount);
    ...
    function _claimOfSROOTxToken(address _addr) internal {
        ...
        uint256 final_SROOTxamount = SROOTamount + unstake_SROOTx;
        require(final_SROOTxamount > 0);

        if (final_SROOTxamount > 0) {
            IERC20Upgradeable(SROOTxTokenAddress).transfer(_addr, final_SROOTxamount);
    ...
    function _claimOfWETH(address _addr) internal {
        ...
        uint256 final_ETHamount = ETHamount + unstake_WETH;
        require(final_ETHamount > 0);

        if (final_ETHamount > 0) {
    ...
    function claimOfWETH() external isOnlyStaker nonReentrant{
        if(!isVested(msg.sender)) {
    ...
    function claimOf(address _addr) public view returns (uint256, uint256, uint256 )
    {
        ...
        if (sumofgenesisbear >= 5) {
            ...
            for (uint256 i = 0; i < stakes[addr].length; i++) {
                ...
                if ((isSpecialBear(stakes[addr][i].nft_id) || isGenesisBear(stakes[addr][i].nft_id)) && dd_root < 1) {
                    flag = false;
                }
    ...
    function BurnRootx_mintSrootx (uint256 _amount) public {
        require(IERC20Upgradeable(ROOTxTokenAddress).allowance(msg.sender, address(this)) == _amount, "You have to approve rootx to staking contract");
        IERC20Upgradeable(ROOTxTokenAddress).transferFrom(msg.sender, address(this), _amount);
        ERC20Upgradeable(ROOTxTokenAddress)._burn(_amount);
        IERC20Upgradeable(SROOTxTokenAddress).mintToken(msg.sender, _amount*5);
    }

    function BurnROOTxToken(address _addr, uint256 _amount) public {
        require(msg.sender == 0xA9bCCb181EC9B14A8692890c55E18Ad6996dEaB4);
