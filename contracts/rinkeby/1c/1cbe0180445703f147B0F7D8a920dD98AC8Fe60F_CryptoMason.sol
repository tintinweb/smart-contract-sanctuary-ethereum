...
contract CryptoMason is ERC721A, Ownable {
    ...
    function batchUnstakeNFTs(uint256[] memory nfts) external {
        for (uint256 i; i < nfts.length; i++) {
            uint256 ind = nfts[i] - 1;
            unStakeNFTs(nfts[i]);
            uint256 index = 10000000000000001;
            for (uint256 l; l < claimInfo[msg.sender][ind].nftIds.length; l++) {
                for (uint256 j; j < adrToIdsArray[msg.sender].length; j++) {
                    if (
                        adrToIdsArray[msg.sender][j] ==
                        claimInfo[msg.sender][ind].nftIds[l]
                    ) {
                        index = j;
                        break;
                    }
    ...
    function getClaimInfo(address user, uint256 number)
        external
        view
        returns (claim memory)
    {
        return claimInfo[user][number];
    }
