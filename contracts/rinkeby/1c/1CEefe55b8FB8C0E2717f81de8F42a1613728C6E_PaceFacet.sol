...
contract PaceFacet is ChildMintableERC20 {
    ...
    function setup() public {
        ...
        _setupRole(state.DEFAULT_ADMIN_ROLE, _msgSender());
        _setupRole(state.DEPOSITOR_ROLE, _msgSender());
        _initializeEIP712("Pace"); 
    }

    modifier onlyOwner() {
        require(msg.sender == state.contractOwner, "Not owner");
        _;
    }
    ...
    function totalSupply() public view override returns (uint256) {
        return state._totalSupply;
    }

    function balanceOf(address _owner) public view override returns (uint256 balance) {
        return state._balances[_owner];
    }
    ...
    /**
     *  @dev Sets the contract owner. This can only be changed as the owner. 
     *  @notice Once the owner is changed, the original owner is no longer the owner.
     *  @param owner - The address of the new owner or operator.
     */
    function setContractOwner(address owner) external onlyOwner {
    ...
    function increaseAllowance(address spender, uint256 addedValue) public override virtual returns (bool) {
        ...
        _approve(owner, spender, state._allowances[owner][spender] + addedValue);
        return true;
    }
    ...
    function decreaseAllowance(address spender, uint256 subtractedValue) public override virtual returns (bool) {
        address owner = _msgSender();
        uint256 currentAllowance = state._allowances[owner][spender];
    ...
    function _mint(address account, uint256 amount) internal virtual override {
        ...
        state._totalSupply += amount;
        state._balances[account] += amount;
    ...
    function _burn(address account, uint256 amount) internal virtual override {
        ...
        uint256 accountBalance = state._balances[account];

        require(accountBalance >= amount, "ERC20: burn amount exceeds balance");

        unchecked {
            state._balances[account] = accountBalance - amount;
        }
...
struct PaceStorage {
    ...
    mapping(address => uint256) _approvedContractIndexes;
    ...
    ContextMixin
{
     ...
     */
    function deposit(address user, bytes calldata depositData) external override only(s.DEPOSITOR_ROLE) {
     ...
     */
    function mint(address user, uint256 amount) public only(s.DEFAULT_ADMIN_ROLE) {
        _mint(user, amount);
    }
}
...
import { PaceStorage } from "./PaceStorage.sol";

library LibAppStorage {
    function diamondStorage() internal pure returns (PaceStorage storage ds) {
        assembly {
            ds.slot := 0
        }
    }
...
contract ERC20 is Context, IERC20, IERC20Metadata {
    ...
    function balanceOf(address account) public view virtual override returns (uint256) {
        return state._balances[account];
    }
    ...
    function _transfer(
        ...
        unchecked {
            state._balances[from] = fromBalance - amount;
        }
        state._balances[to] += amount;
