    ...
    Timestamp
{
     ...
     */
    function getPoolAddress(address poolToken)
        external
        view
        virtual
        returns (address)
    ...
    function updateVote(
        ...
        bytes4 fnSelector = this.updateVote.selector;
        // verify that sender is a pool registered withing the factory
        fnSelector.verifyState(poolExists[msg.sender], 0);
        if (_isAdd) {
            vote.addVoteAmount(_to, _value);
        } else {
            vote.reduceVoteAmount(_to, _value);
        }
    }
...
interface IVote {
    ...
    //    mapping(address => mapping(string => uint256)) external userVotes;

    function userVotes(address user, string memory symbol)
        external
        view
        returns (uint256);

    function addVoteAmount(address user, uint256 _amount)
        external
        returns (bool);

    function reduceVoteAmount(address user, uint256 _amount)
        external
        returns (bool);
}
