...
interface INFTBattles {
  ...
  struct Battle {
    uint battleId;
    uint state; // 0 = queue // 1 = started // 2 = finished // 3 = cancelled
    uint createdDate;
    uint startedDate;
    uint finishedDate;
    address playerA;
...
contract NFTBattles is Ownable {
  ...
  mapping (address => bool) public playerBlacklist;
  mapping (address => uint) public playerTimeouts;
  ...
  event BattleFinished(INFTBattles.Battle battle, INFTBattles.Player playerA, INFTBattles.Player playerB, address winner);
  event BattleCancelled(INFTBattles.Battle battle, INFTBattles.Player playerA);
  ...
  function joinBattle() public returns (uint) {
    ...
    require(!playerBlacklist[msg.sender], "Blacklisted");
    require(playerTimeouts[msg.sender] < block.timestamp, "Timed out");
  ...
  function claimBattle(uint id, bytes memory signature, uint result, uint expiration) public {
    ...
    require(signer == signerAddress, "Signers don't match");
		...
		address player;
		if (result == 1 || result == 11) // win
			player = msg.sender;
		else if (result == 0 || result == 10) // loss
			player = battle.playerA == msg.sender ? battle.playerB : battle.playerA;
		else // draw
			player = address(0);

    _finishBattle(id, player, msg.sender, result > 9);
  }

  function leaveQueue() public {
    INFTBattles.Player storage player = players[msg.sender];
    require(player.currentBattle != 0, "Not joined");
    INFTBattles.Battle storage battle = battles[player.currentBattle];
    uint queuePos = 99999;
    ...
    require(queuePos != 99999, "Not found in queue");
    removeFromQueue(queuePos);
    battle.state = 3;
    player.currentBattle = 0;
    playerTimeouts[msg.sender] = block.timestamp + 5 minutes;
    emit BattleCancelled(battle, player);
  }

  function _finishBattle(uint id, address winner, address claimer, bool timeout) internal {
    ...
    if (changeA > changeB) {
      player.xp += uint(changeA) * (block.timestamp < player.dailyBoosts.xp ? 2 : 1);
      opponent.xp += uint(changeA / 5) * (block.timestamp < opponent.dailyBoosts.xp ? 2 : 1) + 1;
    } else {
      player.xp += uint(changeB) * (block.timestamp < player.dailyBoosts.xp ? 2 : 1);
      opponent.xp += uint(changeB / 5) * (block.timestamp < opponent.dailyBoosts.xp ? 2 : 1) + 1;
    }
    ...
    if (timeout) playerTimeouts[opponent.addr] = block.timestamp + 15 minutes;

    emit BattleFinished(battle, battle.playerA == player.addr ? player : opponent, battle.playerA == player.addr ? opponent : player, winner);
  }
  ...
  function togglePlayerBlacklist(address addr) external onlyOwner {
    playerBlacklist[addr] = !playerBlacklist[addr];
  }
  ...
  function setMaxPlayerDiff(uint diff) external onlyOwner {
	...
	function adminFinishBattle(uint id, address winner, address claimer, bool timeout) external onlyOwner {
		INFTBattles.Battle memory battle = battles[id];
    ...
    _finishBattle(id, winner, claimer, timeout);
  }	
