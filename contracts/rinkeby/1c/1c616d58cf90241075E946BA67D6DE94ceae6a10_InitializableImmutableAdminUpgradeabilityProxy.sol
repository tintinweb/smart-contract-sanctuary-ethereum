...
contract UmeeTreasuryCollector is VersionedInitializable, Ownable, ReentrancyGuard {
    using SafeERC20 for IERC20;
    ...
    function approve(IERC20[] memory _tokens, address _spender, uint256[] memory _amounts) external onlyOwner {
        ...
        for (uint8 i; i < _tokens.length; i++) {
            _tokens[i].safeApprove(_spender, _amounts[i]);
        }
    }
...
contract DWS is VersionedInitializable, Ownable, ReentrancyGuard {
    using SafeERC20 for IERC20;

    // uint256 public constant REVISION = 0x1;
    uint256 public constant REVISION = 1000;
    address public test1; //dws
    uint256 internal test2; //dws
    uint8 private test3 = 1;//dws
