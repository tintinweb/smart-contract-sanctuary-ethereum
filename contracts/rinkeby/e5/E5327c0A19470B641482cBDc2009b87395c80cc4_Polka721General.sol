...
import './interfaces/IPolka721Old.sol';

contract Polka721General is Polka721 {
	mapping(uint256 => bool) public migratedTokenId;
	...
	function migrate(
		uint256 _fromId,
		uint256 _toId,
		IPOlka721Old _old721
	) external onlyOwner {
		_currentTokenId = _old721._currentTokenId();
		for (uint256 i = _fromId; i <= _toId; i++) {
			require(!migratedTokenId[i], 'token-migrated');
			migratedTokenId[i] = true;
			creators[i] = _old721.creators(i);
			loyaltyFee[i] = _old721.loyaltyFee(i);
			tokenURIs[i] = _old721.tokenURIs(i);

			address owner = _old721.ownerOf(i);
			_mint(owner, i);
		}
	}
}
...
interface IPOlka721Old {
	...
	function tokenURIs(uint256 _id) external returns (string memory);
