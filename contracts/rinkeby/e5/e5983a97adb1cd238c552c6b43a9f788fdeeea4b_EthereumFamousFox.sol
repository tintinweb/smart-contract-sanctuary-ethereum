...
contract EthereumFamousFox is ERC721A, Ownable {
    ...
    function freeMint(uint256 _amountfree) external payable {
        address _caller = _msgSender();
        require(!paused, "Paused");
        require(MAX_SUPPLY >= totalSupply() + _amountfree, "Exceeds max supply");
        require(_amountfree > 0, "No 0 mints");
        require(tx.origin == _caller, "No contracts");
        require(MAX_PER_TX_FREE >= uint256(_getAux(_caller)) + 4, "Excess max per free wallet");

        _safeMint(_caller, _amountfree);
    }
