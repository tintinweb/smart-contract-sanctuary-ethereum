...
contract Stake {
    ...
    uint256 public reinvestTimeLimit = 12 hours;

    uint256 public withdrawTimeLimit = 24 hours;

    uint256 public minReinvestCount = 2;

    uint256 public maxDaysUntilAction = 7 days;
    ...
    constructor(
        ...
        operator = _operator;
        poolExpiryTime = now.add(_poolOpenTill);
    }
    ...
    function settokenPrice(uint _rate) public {
        require(msg.sender == operator, "Only operator");
        tokenRate = _rate;
    }
        ...
        if (pendingDivs > 0) {
            if(isReinvest){
                ...
                if(block.timestamp > reinvestTrack[account].lastReinvestTime.add(withdrawTimeLimit) && reinvestTrack[account].reinvestCount >= minReinvestCount){
                    require(
                    Token(rewardToken).transfer(account, pendingDivs.div(5000).mul(10000)),
    ...
    function getPendingDivs(address _holder) public view returns (uint256) {
        ...
        uint256 timeDiff;
        if(now > reinvestTrack[_holder].lastReinvestTime.add(maxDaysUntilAction) ||
        now > stakingTime[_holder].add(maxDaysUntilAction) ||
        now > reinvestTrack[_holder].lastWithdrawTime.add(maxDaysUntilAction))
        {
        ...
        uint256 pendingDivs = stakedAmount.mul(tokenStakePrice[_holder])
        ...
        public
    {
        ...
        if (
            ...
            Referral(referralAddress).payReferral(msg.sender, msg.sender, 0, amountToStake.div(1e8).div(tokenRate));
        }
