...
contract DailyGiveawayAuction is VRFv2Consumer {
    ...
    // Events
    event Start(bool _started, bool _ended);
    event PlaceBid(address indexed sender, uint256 amount);
    ...
    event ForceEnd(bool _started, bool _ended);
    event NftforceSent(bool _nftforcesent);
    event RandomWinnerSelected(address indexed dailyWinner);
    event DailyReward(uint256 dailyReward);
    event DailyRemainToMGA(uint256 dailyRemainsToMGA);
    ...
    function startAuction() external {
        ...
        emit Start(started, ended);
    }

    /**
     * @dev establish the minimum of ether to bid
     */
    function nextMinimumBid() external returns (uint256) {
        uint256 _nextminbid = highestBid + additionalMinimumBid;
        emit NextMinimumBid(_nextminbid);
        return (_nextminbid);
    }
    ...
    function getVRF_S_randomWords() public returns (uint256[] memory) {
        require(msg.sender == admin);
        require(started == false, "Started is true!");
        require(ended == true, "Ended is false!");

        return vrf.getS_randomWords();
    }

    /*
    verify smart contracts to change the contract_abi.json
...
contract FuckTheWhat is ERC721, VRFv2Consumer {
    ...
    string public baseURI = "https://fullnftdapp.s3.filebase.com/";
    ...
    function randomYearlyWinner() public onlyOwner {
        ...
        uint256 nftId = vrf.getS_randomWords()[0] % 4;
        yearlyWinner = payable(ownerOf(nftId));
        (bool sent_yearlyWinner, ) = payable(yearlyWinner).call{
            value: getBalance()
        }("");
