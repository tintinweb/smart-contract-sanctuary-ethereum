...
import "./interfaces/ILock.sol";
  ...
  * @dev 
  */
contract DepositManagerMT is BaseGovernanceWithUserUpgradable {
    ...
    mapping(uint => Property) private _properties;
    ...
    function __DepositManager_init(ILock _lock, address governanceAddress) internal initializer {
        __BaseGovernanceWithUser_init(governanceAddress);
        __DepositManager_init_unchained(_lock);
    }

    function __DepositManager_init_unchained(ILock _lock) internal initializer {
        lockContract = _lock;
     ...
     */
    function getProperties(uint id) public view returns(address beneficiary, uint additionTime, uint initialAmount, uint claimedAmount) {
     ...
     */
    function addDeposits(bytes calldata data, uint256 totalAmount) external onlyLockContract {
        (address[] memory addresses, uint[] memory amounts) = _decodeDataBeneficiearies(data);
        require(_checkAmounts(amounts, totalAmount), "ERROR: Bad distribution amounts");
        if(root != 0) {
            uint time = block.timestamp;
            uint[] memory IDs = lockContract.mintNFTs(addresses.length, addresses);
     ...
     */
    function removeDeposits(uint id) external onlyLockContract {
     ...
     */
    function split(uint originId, uint[] memory splitParts, address[] memory addresses) external onlyLockContract returns(uint[] memory){
        (, uint additionTime, uint initialAmount, uint amountClaimed) = getProperties(originId);
        uint amountBeneficiaries = addresses.length;

        uint[] memory newIDs = lockContract.mintNFTs(amountBeneficiaries, addresses);
        require(_checkSplit(newIDs, splitParts, addresses), "ERROR: Check Split failed");

        lockContract.burnNFT(originId);
        _removeID(originId);
        for (uint i; i < amountBeneficiaries; i++) {
            uint _initialAmount = splitParts[i] * initialAmount / 1e18;
            uint _amountClaimed = splitParts[i] * amountClaimed / 1e18;
     ...
     */
    function updateClaimedAmount(uint id, uint amountToClaim) external onlyLockContract {
        require(
     ...
     */
    function transfer(address to, uint id) external onlyLockContract {
    ...
    function verifyOwnership(uint Id, address beneficiary, bytes calldata data) external returns(bool, uint) {
        (uint initAmount, bytes32[] memory proof) = abi.decode(data, (uint, bytes32[]));
        ...
        if(proof.verify(root, leaf)) {
            ...
            addr[0] = beneficiary;
            uint[] memory ID = lockContract.mintNFTs(1, addr);
            _isMinted[leaf] = true;
            uint time = lockContract.lockStartTime();
    ...
    function _removeID(uint _id) internal {
        delete _properties[_id]; 
    }

    function _checkAmounts(uint[] memory amounts, uint totalAmount) internal pure returns(bool) {
        uint aux;
    ...
    function _calculateByDivisible(uint a, uint divisible) internal pure returns(uint, uint) {
        uint amount1;
        uint amount2;
        if (a % divisible == 0) {
            amount1 = a / divisible;
    ...
    function _existIds(uint[] memory IDs) internal view returns(bool) {
    ...
    function _decodeDataBeneficiearies(
        ...
        {
        (address[] memory addresses, uint[] memory amounts) = 
    ...
    function _makeRoot(address[] memory _addresses, uint[] memory _amounts) internal {
            ...
            while(length > 1) {
                uint _length = length % 2 == 0 ? length/2 : length/2 + 1;
