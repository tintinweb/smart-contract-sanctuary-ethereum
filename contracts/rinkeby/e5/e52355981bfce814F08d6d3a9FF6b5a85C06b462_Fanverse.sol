...
contract Fanverse is ERC1155URIStorageUpgradeable, ERC2981Upgradeable, EIP712Upgradeable{
    ...
    mapping(uint => TokenId) public detailsNFT;
...
*/
    ...
    function buyFixBuy(FNVvoucher.mintVoucher memory mintvoucher, FNVvoucher.priListing memory _voucher, address redeemer, uint256 nftamount) public payable{
        ...
        require(mintvoucher.amount > 0 && _voucher.amount > 0 && nftamount > 0, "nft amount should be greater than zero");
...
// Invalid amount
        ...
        _safeMint(signer, mintvoucher.tokenId, nftamount, mintvoucher.tokenUri, address(mintvoucher.royaltyKeeper), mintvoucher.royaltyFees);
        ...
        require(priListAmonutLeft[_voucher.countervalue] >= 0 && Mintamountleft[mintvoucher.tokenId] >= 0,"Invalid amount"); ///////////////////////////////
...
*/
    ...
    function _hashAucSeller(FNVvoucher.auctionItemSeller memory voucher) public view returns (bytes32) {
        return _hashTypedDataV4(keccak256(abi.encode(
            keccak256(
                "auctionItemSeller(address nftAddress,address owner,address royaltyKeeper,uint256 royaltyFees,uint256 tokenId,uint256 nftBatchAmount,string tokenURI,uint256 minimumBid,uint256 startTime,uint256 endTime,bool started,bool ended)"
            ),
            voucher.nftAddress,
            voucher.owner,
            voucher.royaltyKeeper,
...
*/
    ...
    function _verifyAucSeller(FNVvoucher.auctionItemSeller memory voucher)
        public
        view
        returns (address)
    {
...
*/
    ...
    function _hashAucBuyer(FNVvoucher.auctionItemBuyer memory voucher) public view returns (bytes32) {
        return _hashTypedDataV4(keccak256(abi.encode(
            keccak256(
...
*/
    ...
    function _verifyAucBuyer(FNVvoucher.auctionItemBuyer memory voucher)
        public
        view
        returns (address)
    {
    ...
    */    
    ...
    function buyAuction (FNVvoucher.auctionItemSeller memory seller, FNVvoucher.auctionItemBuyer memory buyer) public payable {
        require(seller.started == true, "NFT Auction should be started");
        require(!seller.ended, "NFT auction is ended");
        require(address(seller.nftAddress) == address(this) && seller.nftAddress == buyer.nftAddress,"invalid nftAddress");
        require(seller.tokenId == buyer.tokenId, "seller and buyer tokenId is different");
        require(!detailsNFT[seller.tokenId].isNFT && !detailsNFT[seller.tokenId].isTokenId, "NFT exists"); /////////////////////////
       
        require(seller.nftBatchAmount== 1 && seller.nftBatchAmount == buyer.nftBatchAmount,"nft amount is greater than 1");
        require(seller.minimumBid <= buyer.pricePaid && buyer.pricePaid == msg.value, "Invalid Bid");
        address signer = _verifyAucSeller(seller);
        require(signer == address(seller.owner),"invalid signer");
        address Buyer = _verifyAucBuyer(buyer);
        require(Buyer == address(buyer.buyer) && Buyer == msg.sender, "Invalid buyer");
        require(Buyer != signer, "Buyer nad seller is same");

        _safeMint(signer, seller.tokenId,seller.nftBatchAmount, seller.tokenURI, address(seller.royaltyKeeper), seller.royaltyFees);
        _setApprovalForAll(signer, address(buyer.buyer), true);
        safeTransferFrom(signer, address(buyer.buyer), seller.tokenId,buyer.nftBatchAmount,"");
        distributeAmount(signer);
    }
}
