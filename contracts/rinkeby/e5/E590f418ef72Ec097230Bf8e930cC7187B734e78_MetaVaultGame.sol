...
interface IKeyContract is IERC721 {
  ...
  function getTimerStart() external view returns (uint256);
}

contract MetaVaultGame is Ownable, ReentrancyGuard {
  uint256 public constant SECONDS_IN_FIVE_MIN = 600; 
  ...
  function playGame(uint256 _keyId, uint256 _doorChosen) public {
    ...
    require(block.timestamp - KEY.getTimerStart() > SECONDS_IN_FIVE_MIN, "Game will start in 24 hours after keys sold out");
    require(block.timestamp - _lastPlayedId[_keyId] >= SECONDS_IN_FIVE_MIN, "Key was played less then 24h ago");
