...
contract WowToken is ERC20("Wow", "WOW"), ReentrancyGuard, AccessControl {
using SafeMath for uint256;
	...
	// March 15, 2032 0:00:00 GMT+0000
	uint256 public END = 1962921600;
	uint256 public START = 1647302400;

	mapping(address => uint256) public rewards;
	mapping(address => uint256) public lastUpdate;

	ITESTBBBB public bbbContract;
    address private botsAddress = 0xC9a34EaB527754cd4BaAD08D5940E089cE365f36;
	...
	constructor() {
		bbbContract = ITESTBBBB(botsAddress);
	...
	function transferTokens(address _from, address _to) external {
		require(msg.sender == address(bbbContract));
			...
			if (timerFrom > 0)
				rewards[_from] += bbbContract.botsBalance(_from).mul(BASE_RATE.mul((time.sub(timerFrom)))).div(86400);
			if (timerFrom != END)
				lastUpdate[_from] = time;
			if (_to != address(0)) {
				uint256 timerTo = lastUpdate[_to];
				if (timerTo > 0)
					rewards[_to] += bbbContract.botsBalance(_to).mul(BASE_RATE.mul((time.sub(timerTo)))).div(86400);
				if (timerTo != END)
					lastUpdate[_to] = time;
			}
	}
	...
	// called on transfers
	function updateReward(address _from) private {
		uint256 time = min(block.timestamp, END);
		uint256 timerFrom = lastUpdate[_from];
		if(timerFrom <= 0){
			timerFrom = START;
	...
	function getReward(address _user) external nonReentrant {
		require(msg.sender == _user,  "cant call this");
	...
	function getTotalClaimable(address _user) external view returns(uint256) {
		...
		uint256 pending = bbbContract.botsBalance(_user).mul(BASE_RATE.mul((time.sub(timerFrom)))).div(86400);
		return rewards[_user] + pending;
	}

	//reserve & emergency usage
