...
contract ForgeMaster is Ownable, ERC721A, ReentrancyGuard {
    string public NAME;
    ...
    uint256 public FREE_MINT_PERCENTAGE;
    ...
    constructor(
        ...
        FREE_MINT_PERCENTAGE = _freePercentage;
    }
    ...
    function totalFreeMints() public view returns (uint256) {
        return (MAX_SUPPLY * FREE_MINT_PERCENTAGE);
    }

    function freeMintsAvailable() public view returns (uint256) {
        return (totalFreeMints() - totalSupply());
    }
    ...
    function safeMint(uint256 quantity) external payable {
        ...
        require(
            msg.value == (PRICE_PER_TOKEN_SET * quantity),
            "Invalid ether call value provided for mint quantity. Expected quantity multiplied by PRICE_PER_TOKEN_SET."
        );

        uint256 mintTotal = quantity * TOKEN_SET_QUANTITY;

        if (totalSupply() + mintTotal > MAX_SUPPLY) {
            mintTotal = MAX_SUPPLY - totalSupply();

            (bool success, ) = payable(msg.sender).call{
                value: msg.value -
                    ((mintTotal / TOKEN_SET_QUANTITY) * PRICE_PER_TOKEN_SET)
            }("");
