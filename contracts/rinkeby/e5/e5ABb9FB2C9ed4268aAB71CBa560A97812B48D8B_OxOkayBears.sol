...
contract OxOkayBears is ERC721A, Ownable {
    ...
    function calculateTotal(uint count) public view returns (uint) {
        uint total;
        uint currentTokenId = _totalMinted();
        uint sum = _totalMinted() + count;
        if(sum <= 800) {
            total = 0;
        } else if (sum > 800 && sum <= 2000) {
            total = count * 0.01 ether;
        } else if (currentTokenId > 2000 && currentTokenId <= 5000) {
            total = count * 0.02 ether;
        } else if (currentTokenId > 5000 && currentTokenId <= 9000) {
            total = count * 0.03 ether;
        } else {
            total = count * 0.05 ether;
        }
        ...
        // if (FIRSTXFREE > _totalMinted()) {
        //     uint freeLeft = FIRSTXFREE - _totalMinted();
        //     if (count > freeLeft) {
