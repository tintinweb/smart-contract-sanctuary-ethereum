...
contract MyNFT is ERC721 {
    ...
    function mint(
        ...
        require(currentTokenId <= mintMax, "No more tokens avaliable");
        require(currentTokenId + amount - 1 <= mintMax, "Too much amount");
        for (uint256 i = 0; i < amount; i++) {
            _safeMint(to, currentTokenId);
            attributes[currentTokenId] = Metadata(_name);
            currentTokenId += 1;
        }
    }
