...
modifier isValidMerkleProofW(bytes32[] calldata _proofW) {
    require(MerkleProof.verify(
    _proofW,
    merkleRootW,
    keccak256(abi.encodePacked(msg.sender))
...
function waitlistMint(address account,uint256 _mintAmount, bytes32[] calldata _proofW) public payable mintCompliance(_mintAmount) 
mintPriceCompliance(_mintAmount) 
isValidMerkleProofW(_proofW) 
onlyAccounts {
    // Verify presale requirements
...
function setMerkleRootW(bytes32 _merkleRootW) public onlyOwner {
    merkleRootW = _merkleRootW;
}
