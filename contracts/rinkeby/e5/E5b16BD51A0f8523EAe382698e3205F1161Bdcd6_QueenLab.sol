...
contract QueenLab is IQueenLab, BaseContractController {
    ...
    function RarityLottery(
        ...
        if(_queeneId == 1){ //especial event, legendary QueenE
     ...
     */
    function GenerateQueen(uint256 _queeneId, IQueenTraits _traitsContract, IQueenE _queeneContract)
        external
        view
        override
        onlyEcosystemOrActor
        returns (RoyalLibrary.sQUEEN memory)
    {
        ...
        uint256 checkers = 0;
        while (checkers < 3) {
            if (checkRepeated(_dna, _queeneContract)) {
                _dna = BuildDna(_queeneId, _traitsContract);                
            }
            checkers++;
        }
        ...
        return
            RoyalLibrary.sQUEEN({
                ...
                dna: _dna,
                finalArt: "",
                description: getQueenEDescription(_queeneId, GetQueenRarity(_dna), _dna)
            });
    }
        ...
        returns (queeneRarity finalRarity)
    {
        ...
        else if (traitStat <= 27)
            return queeneRarity.RARE;
        else if (totalRare >= 324)
            return queeneRarity.SUPER_RARE;
        else
            return queeneRarity.LEGENDARY;

        /*if (totalRare >= 2 && totalUltraRare >= 2)
            return queeneRarity.LEGENDARY;
        else if (totalUltraRare >= 2)
            return queeneRarity.SUPER_RARE;
        else if (totalRare >= 2)
            return queeneRarity.RARE;
        else if (totalRare == 1 || totalUltraRare == 1)
    ...
    {
        uint256 rarityId = uint(GetQueenRarity(_dna));

        if (rarityId == uint(queeneRarity.LEGENDARY))
            return "Legendary";
        else if (rarityId == uint(queeneRarity.SUPER_RARE))
            return "Super-Rare";
        else if (rarityId == uint(queeneRarity.RARE))
            return "Rare";
        else
            return "Commom";
    }
     ...
     */
    function GetQueenRarityBidIncrement(RoyalLibrary.sDNA[] memory _dna, uint256[] calldata map)
        external
        pure
        override
    ...
    function checkRepeated(RoyalLibrary.sDNA[] memory dna, IQueenE _queeneContract)
        internal
        view
        returns (bool repeated)
    {
        if (_queeneContract._currentAuctionQueenE() == 1) return true;
        for (uint256 idx = 1; idx < _queeneContract._currentAuctionQueenE(); idx++) {
            bool equals = true;
            for (uint256 index = 0; index < _queeneContract.getQueenE(idx).dna.length; index++) {
                if (
                    dna[index].rarityId != _queeneContract.getQueenE(idx).dna[index].rarityId ||
                    dna[index].traitId != _queeneContract.getQueenE(idx).dna[index].traitId ||
                    dna[index].trace != _queeneContract.getQueenE(idx).dna[index].trace
                ) equals = false;
            }
    ...
    function getQueenEDescription(uint256 _queeneId, queeneRarity _rarityId, RoyalLibrary.sDNA[] memory _dna)
        private
        view
        returns (string memory)
    {
        string[14] memory legendaryWords = [
            "Glorious and Invincible",
            "Immortal and Godess",
            "Mighty and Beauteous",
            "Mighty and All-Powerful",
            "Dear and Absolutely Beautiful",
            "Sweet and Sovereign",
            "Beautiful and Intrepid",
            "Gracious and Benevolent",
            "Grateful and Proud",
            "Noble and Perfect",
            "Wise and Fair",
            "Fair and Benevolent",
            "All-Powerful and Absolutely Beautiful",
            "Mighty and Sovereign"
        ];
        string[15] memory rareWords = [
            "Glorious",
            "Immortal",
            "Mighty",
            "Humble",
        ...
        if (_rarityId == queeneRarity.COMMOM)
        {   
            return string(abi.encodePacked(GetQueenRarityName(_dna), ' portrait of Our ',commomWords[pseudoRandomNumber % commomWords.length], ' QueenE'));
        }
        else if (_rarityId == queeneRarity.RARE)
        {   
            return string(abi.encodePacked(GetQueenRarityName(_dna), ' portrait of Our ',rareWords[pseudoRandomNumber % rareWords.length], ' QueenE'));
        }
        else
        {   
            return string(abi.encodePacked(GetQueenRarityName(_dna), ' portrait of Our ',legendaryWords[pseudoRandomNumber % legendaryWords.length], ' QueenE'));
        }
    }    
}
