...
contract FRTMarketplace004 is AccessControl{
    ...
    event MintNFT(uint256, address, string, uint256, string, string);
     ...
     *
     */
    function mintNFT(string memory _tokenHash, string memory _name, string memory _description) external{
        ...
        emit MintNFT(newItemId, msg.sender, _tokenHash, baseNFTAmount, _name, _description);
    }
    ...
    /**  
     * @dev Sets reward for Module pu blisher based on time since con tract was published and {rewardData}.
     *
     * @param _tokenId uint256, Module/NFT ID  
