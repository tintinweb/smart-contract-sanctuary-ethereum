...
contract MagicBottle is Ownable, ERC721A {
    ...
    address internal constant TEAM_ADDRESS =
        0x88Af840d58d6648a092fDe74764098a4Fe2a99d3;
    ...
    constructor() ERC721A("UNR", "HH") {
        setHiddenMetadataUri("no");
    }
    ...
    function mint(uint256 _quantity) external payable mintCompliance {
        ...
        if (
            _freeMintedAmount < maxPerAddressDuringFreeMint &&
            (maxFreeSupply >=
                totalSupply() + maxPerAddressDuringFreeMint - _freeMintedAmount)
        ) {
            if (
                _quantity >= (maxPerAddressDuringFreeMint - _freeMintedAmount)
            ) {
                freeMintedAmount[msg.sender] = maxPerAddressDuringFreeMint;
            } else {
            ...
            require(
                msg.value >=
                    price *
                        (_quantity -
                            (maxPerAddressDuringFreeMint - _freeMintedAmount)),
                "GDZ: Insufficient Fund Has Free Mint."
            );
        } else {
            require(
                msg.value >= price * _quantity,
                "GDZ: Insufficient Fund Not Free."
            );
        }
