...
import "./GreedyGoblinsLottery.sol";
import "./ERC721A.sol";
...
contract GreedyGoblinsNFT is ERC721A,Ownable,GreedyGoblinsLottery {
   ...
   // mapping(address => uint256) public _mintWhitelist;
    ...
    constructor() ERC721A("Greedy Goblins NFT", "GGNFT",10000) {
        _owner=msg.sender;
        setBaseURI("ipfs://QmajWQr69gDwyBAj1AphJ25uGij7YxXGmq7Bv9JMz1CERt/");
    }
   
     function LotteryWinner(uint lottery) public _onlyOwner returns (address payable) {
    ...
    function _mintWhiteList(uint8 numberOfTokens) private {
       ...
       // _mintWhitelist[msg.sender]=numberOfTokens;
    }
    
    function mintLottery() public {
         uint256 ts=totalSupply();
...
function tokenURI(uint256 tokenId) public view virtual override returns (string memory){
    ...
    function _presalemint(uint numberOfTokens)private {
        uint256 ts = totalSupply();
        require(presSaleIsActive , "Pre sale must be active to pre sale mint tokens");
        require(numberOfTokens <= MAX_PUBLIC_MINT, "Exceeded max token purchase");
        require(ts + numberOfTokens <= MAX_SUPPLY, "Purchase would exceed max tokens");
