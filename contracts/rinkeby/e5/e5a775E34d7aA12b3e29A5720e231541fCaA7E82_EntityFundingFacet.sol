...
contract EntityFundingFacet is EternalStorage, Controller, EntityFacetBase, IEntityFundingFacet, IDiamondFacet {
    ...
    function withdraw(address _unit, uint256 _amount) external override assertIsEntityAdmin(msg.sender) {
    ...
    function trade(
        ...
        address _buyUnit,
        uint256 _buyAmount
    ) external override assertCanTradeTrancheTokens returns (uint256) {
        // check balance
    ...
    function sellAtBestPrice(
        ...
        uint256 _sellAmount,
        address _buyUnit
    ) external override assertCanTradeTrancheTokens {
        // check balance
