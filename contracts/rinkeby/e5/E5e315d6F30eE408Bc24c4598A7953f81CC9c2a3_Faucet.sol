...
contract Faucet is OwnableUpgradeable {
    ...
    address public venusVaultAddress = 0x069571F5894D980C4dB3D03Ad59b711cbf156f61;
    ...
    VenusToken private venusToken = VenusToken(0xB55Cb54815cB2337dA5673229cd75Cb28640EEc5);
    ITokenMint private tokenMint = ITokenMint(0xB55Cb54815cB2337dA5673229cd75Cb28640EEc5);
    VenusToken private asteroid = VenusToken(0x14fAbD8782B37Bd76115188892091C16dEA788dc);
    IVenusVault private venusVault = IVenusVault(venusVaultAddress);
    uint256 public CompoundTax = 5;
    uint256 public ExitTax = 17;
    uint256 private ref_depth=15;
    uint256 private ref_bonus = 10;
    uint256 private minimumInitial = 10e18;
    uint256 private minimumAmount = 10e18;

    uint256 public deposit_bracket_size = 9600000e18;     // @BB 5% increase whale tax per 10000 tokens... 10 below cuts it at 50% since 5 * 10
    uint256 public max_payout_cap = 35000000e18;          // or 5% of supply
    uint256 private deposit_bracket_max = 9600000e18; 
    uint256[] public ref_balances = [30,50,90,100,200,300,500,900,1000,1500,2000,3000,6000,10000,15000];
    int128 private payoutRate = ABDKMath64x64.divu(50,100);
    ...
    function deposit(address _upline, uint256 _amount) external {
        ...
        (uint256 realizedDeposit, uint256 taxAmount) = venusToken.calculateTransferTaxes(_addr, _amount);
        ...
        require(
            venusToken.transferFrom(
                _addr,
                address(venusVaultAddress),
                _amount
            ),
    ...
    function _claim_out(address _addr) internal {
        ...
        uint256 vaultBalance = venusToken.balanceOf(venusVaultAddress);
        if (vaultBalance < to_payout) {
            uint256 differenceToMint = to_payout.sub(vaultBalance);
            tokenMint.mint(venusVaultAddress, differenceToMint);
        }

        venusVault.withdraw(to_payout);

        uint256 realizedPayout = to_payout.mul(SafeMath.sub(100, ExitTax)).div(100); // 10% tax on withdraw
        require(venusToken.transfer(address(msg.sender), realizedPayout));
    ...
    function balanceLevel(address _addr) public view returns (uint8) {
        uint8 _level = 0;
        for (uint8 i = 0; i < ref_depth; i++) {
            if (asteroid.balanceOf(_addr) < ref_balances[i]) break;
            _level += 1;
        }
    ...
    function airdrop(address _to, uint256 _amount) external {
        ...
        (uint256 _realizedAmount, uint256 taxAmount) = venusToken.calculateTransferTaxes(_addr, _amount);
