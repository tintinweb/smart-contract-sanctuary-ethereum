...
contract FullDigiHLA is Ownable, ERC721A, MerkleWhitelist, ReentrancyGuard {
    ...
    uint256 public DA_STARTING_PRICE = 0.03 ether;

    uint256 public DA_ENDING_PRICE = 0.01 ether;
    ...
    uint256 public WL_PRICE = 0.008 ether;
     ...
     uint256 public DA_AMOUNT = 5600;
     uint256 public MAIN_AMOUNT = 7600;
     uint256 public EXTRA_AMOUNT = 7600;
    ...
    // uint256 public DA_AMOUNT = 20;
    // uint256 public MAIN_AMOUNT = 25;
    // uint256 public EXTRA_AMOUNT = 30;


    uint256 public WL_START_TIME = 1649415120;
    uint256 public DA_START_TIME = 1649415600;
    uint256 public MAIN_START_TIME = 1649416200;
    uint256 public EXTRA_START_TIME = 1649416500;
    ...
    string public UNREVEALED_URI =
        "https://ipfs.io/ipfs/QmZH467PTwqtbpauL7wgVqQYNiXTWUnCHmbXYQYfgwouBu/";
    ...
    function mintDA(uint8 quantity) public payable {
        ...
        require(totalSupply() + quantity <= DA_AMOUNT,"Max supply for DA reached!");

        //This is the final price
        if (totalSupply() + quantity == DA_AMOUNT){
       ...
       // onlyWlWhitelist(proof)
    {
        ...
        require(totalSupply() + quantity <= WL_AMOUNT, "reached max supply");
       ...
       // onlyMainWhitelist(proof)
    {
        ...
        require(_isMainActive, "Main must be active to mint tokens");
         require(block.timestamp >= MAIN_START_TIME,"MAIN has not started yet!");
        require(MAIN_WALLET_CAP[msg.sender] + quantity <= MAIN_LIMIT, "Purchase would exceed max number of metacards per wallet."); 
        require(msg.value >= ((DA_FINAL_PRICE / 100) * 80)* quantity,"Must send enough eth for MAIN Mint");
        require(totalSupply() + quantity <= MAIN_AMOUNT, "reached max supply");
       ...
       // onlyExtraWhitelist(proof)
    {
        ...
        require(msg.value >= ((DA_FINAL_PRICE / 100) * 80)* quantity,"Must send enough eth for MAIN Mint");
        require(totalSupply() + quantity <= EXTRA_AMOUNT, "reached max supply");
    ...
    function withdrawInitialFunds() public onlyOwner nonReentrant {
        ...
        uint256 MAINFunds = MAIN_MINTED * ((DA_FINAL_PRICE / 100) * 80);
        uint256 EXTRAFunds = EXTRA_MINTED * ((DA_FINAL_PRICE / 100) * 70);
   ...
   function withdraw() public onlyOwner nonReentrant {
    ...
    function setAllStartTime(uint256 wlTime,uint256 daTime,uint256 mainTime,uint256 extraTime) external onlyOwner {
    ...
    function setWlStartTime(uint256 wlTime) external onlyOwner {
        WL_START_TIME = wlTime;
    }

    function setDaStartTime(uint256 daTime) external onlyOwner {
        DA_START_TIME = daTime;
    }

    function setMainStartTime(uint256 mainTime) external onlyOwner {
        MAIN_START_TIME = mainTime;
    }
    ...
    function flipAllState(bool isWLActive,bool isDAActive,bool isMainActive,bool isExtraActive) external onlyOwner {
