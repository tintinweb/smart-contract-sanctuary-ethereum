...
contract NFT is ERC721, ERC2981, Ownable {
    ...
    uint256 public constant MAX_SUPPLY = 23665;
    uint256 public constant MAX_PER_TX = 6; // MAX_PER_TX is 5
    uint256 public constant MAX_PRESALE_MINTS_PER_ADDRESS = 7; // max mints during presale per address is 6
    ...
    uint256 public presaleSupplyLimit;
    ...
    /// Constructor
    constructor(string memory _baseURI, address _royaltyRecipient, address _adminSigner) ERC721("TokenName","TokenSymbol") {
    ...
    /// @dev modifies the state of the `presaleSupplyLimit`
    /// @param _presaleSupplyLimit The new amount of presale supply limit
    function setPresaleSupplyLimit(uint256 _presaleSupplyLimit) external onlyOwner {
        presaleSupplyLimit = _presaleSupplyLimit;
    }
    ...
    function presaleMintDefender(uint256 count, bytes memory signature) external payable validateEthPaymentDefender(count) {
        bytes32 messageHash = keccak256(abi.encodePacked(count));
        require(ECDSA.recover(ECDSA.toEthSignedMessageHash(messageHash), signature) == getAdminSigner(), 'Invalid signature.');

        require((defenderMinted + count) <= presaleSupplyLimit, 'Excedes max supply of Defender.'); // presaleSupplyLimit should be updated during all presale phases, supplyLimit <= NUMBER_OF_DEFENDER
    ...
    function presaleMintConqueror(uint256 count, bytes memory signature) external payable validateEthPaymentConqueror(count) {
        ...
        require((conquerorMinted + count) <= presaleSupplyLimit, 'Excedes max supply of Conqueror.'); // supplyLimit should be updated during all presale phases, supplyLimit <= NUMBER_OF_CONQUER - NUMBER_OF_RESERVED_CONQUER
    ...
    function reserveMintConqueror(uint256 count, bytes memory signature) external payable validateEthPaymentConqueror(count) {
        ...
        require(phase == SalePhase.LimitedSale, 'Public sale is not active.');
        require(conquerorMinted + count <= NUMBER_OF_CONQUER, 'Excedes max supply of Conqueror.');
        require(count < MAX_PER_TX, 'Exceeds max per transaction.');
