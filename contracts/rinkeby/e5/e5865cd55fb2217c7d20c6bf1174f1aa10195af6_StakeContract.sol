...
pragma solidity 0.8.9;
   ...
   contract StakeContract {
        ...
        uint256[4] public durations = [3 minutes, 10 minutes, 30 minutes, 90 minutes];
        //structures
        struct Stake {
            uint256 stakeTime;
...
uint256 public newDays;
    function extendStake(uint256 count,uint256 newplan) public {
       ...
       require(user.stakerecord[count].plan < newplan, "Can not extend to lower plan");
        ...
        newDays = ((currentTime  - timeBefore)/ 60);
        calculateNewReward(user.stakerecord[count].amount, newDays, user.stakerecord[count].plan);
        ...
        user.stakerecord[count].bonus = (user.stakerecord[count].amount.mul(percentages[newplan]).div(percentDivider) + newReward) ;
    ...
    function calculateNewDays(uint256 _newDuration) public pure returns(uint256) {
      uint256 calculatedDays = 1 minutes;
      calculatedDays = calculatedDays * _newDuration;
      return calculatedDays;
    }
    function calculateNewPercentage(uint256 plan,uint256 _totalDays) public {
      newPercentage = APYtoPercentage(APY[plan], _totalDays.div(1 minutes));
    }
    function calculateNewReward(uint256 amount, uint256 _newDuration, uint256 _oldPlan) internal {
            totalDays = calculateNewDays(_newDuration);
            calculateNewPercentage(_oldPlan, totalDays);
            newReward = amount.mul(newPercentage).div(percentDivider);
    }
