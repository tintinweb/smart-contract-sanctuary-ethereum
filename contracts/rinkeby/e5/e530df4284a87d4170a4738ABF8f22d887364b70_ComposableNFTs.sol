...
contract ComposableNFTs is ERC721Enumerable, Ownable {
  ...
  string private description = "Description For NFT Metadata";
  ...
  function presaleMint() public payable onlyWhenNotPaused {
    ...
    require(msg.value >= price, "Ethers sent are less than price");
  ...
  function mintItem(uint256 tokenId) private {
    uint256 minLayersLen = numDefaultLayers < layerContracts.length? numDefaultLayers : layerContracts.length;
    ...
    returns (uint256[] memory randomValues)
  {
    ...
    randomValues = new uint256[](layerContracts.length);

    for (uint256 i = 0; i < layerContracts.length; i++) {
