...
contract GMKey is ERC721, Ownable, ReentrancyGuard {
  ...
  uint256 public MAX_GM_KEY_PER_PURCHASE;
  ...
  constructor(
    ...
    MAX_GM_KEY_PER_PURCHASE = _maxGMKeyPerPurchase;
  }
  ...
  function _safeMintGMKey(uint256 _quantity) internal {
    ...
    require(tokenSupply.current().add(_quantity) <= MAX_GM_KEY, "This purchase would exceed max supply");

    for (uint256 i = 0; i < _quantity; i++) {
      uint256 mintIndex = tokenSupply.current();

      if (mintIndex < MAX_GM_KEY) {
  ...
  function mintReservedGMKey() external onlyOwner {
    ...
    require(tokenSupply.current().add(RESERVED_GM_KEY) <= MAX_GM_KEY, "This mint would exceed max supply");

    for (uint256 i = 0; i < RESERVED_GM_KEY; i++) {
      uint256 mintIndex = tokenSupply.current();
