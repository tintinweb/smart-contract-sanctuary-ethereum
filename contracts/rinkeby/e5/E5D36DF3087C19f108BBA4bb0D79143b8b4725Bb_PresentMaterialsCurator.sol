...
contract PresentMaterialsCurator is Ownable, ReentrancyGuard  {
    ...
    // view function that returns array of all active collections
    function viewAllCollections() public view returns (address[] memory) {
        return allCollections;
    }    

    // view function that returns the curator of a given collection
    function viewCuratorByCollection(address _collection) public view returns (address[] memory) {
    ...
    function addCurator(address _collection, address _newCurator) public {
    ...
    // removes collection from array of all active collections + removes curator to mapping of curators if they
    // have no more remaining active collecitons
    function removeCurator(address _collection, address _curatorToRemove) public {
        // Move the last element into the place to delete
        for (uint256 i = 0; i < curators[_collection].length; i++) {
            if (curators[_collection][i] == _curatorToRemove) {
                curators[_collection][i] = curators[_collection][curators[_collection].length - 1];
