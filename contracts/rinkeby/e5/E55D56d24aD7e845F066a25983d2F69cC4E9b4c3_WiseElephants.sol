...
contract WiseElephants is ERC721A, Ownable, ReentrancyGuard {
  ...
  function whitelistMint(uint256 _mintAmount, bytes32[] calldata _merkleProof) public payable mintCompliance(_mintAmount) mintPriceCompliance(_mintAmount) {
    ...
    uint256 giftValue;
    uint256 totalMinted = _currentIndex - _startTokenId();

    whitelistClaimed[_msgSender()] = true;
    _safeMint(_msgSender(), _mintAmount);

    if (totalMinted > 0) {
      giftAddress = payable(ownerOf(randomNum(totalMinted, block.timestamp, totalMinted + 1) + 1));
