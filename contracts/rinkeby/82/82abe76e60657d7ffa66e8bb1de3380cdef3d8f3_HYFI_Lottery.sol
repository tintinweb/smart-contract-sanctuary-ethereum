...
contract HYFI_Lottery is Initializable, PausableUpgradeable, AccessControlUpgradeable {
    ...
    modifier mUserOwnsVaultsIds(uint256[] memory revealIds) {
        ...
        for (uint256 i = 0; i < revealIds.length; i++) {
            if (_vault.ownerOf(revealIds[i]) != msg.sender) {
                userIsOwner = false;
                break;
            }
        }
     ...
     */
    function setVaultAddress(address newVault) external onlyRole(DEFAULT_ADMIN_ROLE) {
    ...
    ) external onlyRole(DEFAULT_ADMIN_ROLE) mCheckRewardsLength(rewards) {
    ...
    function _revealSpecificVaults(uint256[] memory revealIds) internal returns (uint256[] memory) {
