...
contract MetaCOMAccessCardFixedMint is MetaCOMAccessCardLogic {
  ...
  */
  ...
  //update token public mint price
  function setPrice(uint256 newPrice) external onlyOwner {
     require(mintState == false, 'sale is active');
     require(newPrice > 0, 'price is 0');
     price = newPrice;
  }
...
abstract contract MetaCOMAccessCardLogic is Ownable, ERC721A, Royalty, ReentrancyGuard {
  ...
  */
  ...
  function _validateRequest(uint256 timestamp, bytes memory signature) internal {
      ...
      IMetaCOMCollection(factory).markMessageAsUsed(msgHash);
...
interface IMetaCOMCollection {
  ...
  function getPublicMintSigner() external view returns (address);//factory
  function markMessageAsUsed(bytes32 msgHash) external;//factory
