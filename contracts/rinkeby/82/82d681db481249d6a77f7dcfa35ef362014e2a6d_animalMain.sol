...
// 做作業方式：
// 老師已經完成合約 75%，剩下關鍵的方法需要各位同學自行填空，發揮創意。

// 做作業關鍵：
// 1. 先搞懂 ERC721 與 ERC20 的差異，你就會搞懂這些功能為什麼要這樣設計
// 2. 請直接搜尋 TODO 找出要完成的地方
...
contract animalMain is    ERC721,Ownable {
    ...
    uint constant ITEM_FEE = 0 ether;
    ...
    function transfer(address _to, uint256 _tokenId) public {
        // TODO: 請使用 require 判斷要轉的動物 id 是不是轉移者的
        require(msg.sender == animalToOwner[_tokenId], "ERC721: transfer from incorrect owner");
    ...
    function transferFrom(address _from, address _to, uint256 _tokenId) external {
        ...
        require(animalToOwner[_tokenId] == _from);
        require(msg.sender == animalToOwner[_tokenId] || msg.sender == animalApprovals[_tokenId]);
        require(_to != address(0), "ERC721: transfer to the zero address");
        require(_tokenId < animals.length);
    ...
    function createAnimal() public returns(uint) {
        // 動手玩創意，可以限制每次建立動物需要花費多少ETH
        // require(msg.value >= ITEM_FEE);
        // msg.sender.transfer(msg.value - ITEM_FEE);
        ...
        // TODO 
        // 使用亂數來產生DNA, 星級, 動物種類
        dna = keccak256(abi.encodePacked(now, msg.sender));
        star = uint(keccak256(abi.encodePacked(block.coinbase, msg.sender))) % 5 + 1;
        roletype = uint(keccak256(abi.encodePacked(block.number, msg.sender))) % 4 + 1;
