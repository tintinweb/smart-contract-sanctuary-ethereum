...
contract Frank is ERC721, Ownable {
    ...
    function calcTotal(uint count) public view returns(uint) {
        require(saleStatus != SaleStatus.PAUSED, "Frank: Sales are off");
    ...
    function mint(uint count) external payable {
        require(saleStatus != SaleStatus.PAUSED, "Frank: Sales are off");
        require(_tokenIds.current() + count <= COLLECTION_SIZE, "Frank: Number of requested tokens will exceed collection size");
        require(count <= TOKENS_PER_TRAN_LIMIT, "Frank: Number of requested tokens exceeds allowance (3)");
        require(_mintedCount[msg.sender] + count <= TOKENS_PER_PERSON_PUB_LIMIT, "Frank: Number of requested tokens exceeds allowance (3)");
        require(msg.value >= calcTotal(count), "Frank: Ether value sent is not sufficient");
