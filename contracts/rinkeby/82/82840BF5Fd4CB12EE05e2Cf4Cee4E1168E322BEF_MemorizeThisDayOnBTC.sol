...
abstract contract MemorizeThisDay is ERC1155Pausable, Ownable, VRFConsumerBase {
    ...
    function MemorizedDays(bool justForThisAddress)
        public
        view
        returns (uint256[] memory)
    {
        uint256[] memory mdays = new uint256[](_mintedDays.length);
        for (uint256 i = 0; i < _mintedDays.length; i++) {
            if (justForThisAddress) {
                if (_holders[_mintedDays[i]] == msg.sender) {
                    mdays[i] = _mintedDays[i];
                }
            } else {
                mdays[i] = _mintedDays[i];
            }
