...
contract Vendor is Ownable {
  ...
  function buyTokens() public payable returns (uint256 amount) {
	  require(msg.value > 0, "must send > 0 ether");
	  ...
	  require(sent, "transfer from vendor failed");
  ...
  function sellTokens(uint256 amountToSell) public {
	  require(amountToSell > 0, "amount must be > 0");

	  uint256 userBalance = yourToken.balanceOf(msg.sender);
	  require(userBalance >= amountToSell, "user does not have enough tokens");

	  uint256 ethAmount = amountToSell / tokensPerEth;
	  uint256 ownerETHBalance = address(this).balance;
	  require(ownerETHBalance >= ethAmount, "vendor has not enough funds");
  ...
  function withdraw() public onlyOwner {
	  ...
	  require(ownerBalance > 0, "owner has no balance");
