...
contract LHHS is ERC721A, Ownable {
    ...
    uint256 public constant FREE_MINT_AMOUNT = 40;  
    ...
    function mint(uint256 quantity) public payable {  
        ...
        require(status == Status.Started, "Can't mint");                
        require(quantity > 0, "Amount too small");
        require(quantity <= MAX_MINT_PER_ADDR, "Max 20 tokens per trx"); 
        require((totalSupply + quantity) <= MAX_SUPPLY, "Purchase exceeding max supply");
        if(totalSupply >= FREE_MINT_AMOUNT) {    
            require(PRICE * quantity == msg.value, "Sent eth val is incorrect"); 
        }
