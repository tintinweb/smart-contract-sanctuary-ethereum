 ...
 */
contract Threescrow is NFT, ReentrancyGuard {
    ...
    /// @param contractURI_ Address of the Owner Contract that manages Permissions.
    constructor(
        string memory name_,
        string memory symbol_,
        address[] memory initialAddresses_, // [ address ownerAddress, address escrowFactoryContractAddress, address treasuryAddress ]
        string memory contractURI_
    ) payable NFT(name_, symbol_, contractURI_) {
        _transferOwnership(initialAddresses_[0]);
        escrowFactoryContractAddress = initialAddresses_[1];
        // treasuryContractAddress = initialAddresses_[2];
    }
    ...
    function _isValidSignature(
        ...
        bytes32 message = keccak256(abi.encodePacked(_hash));
        return _signer == _recoverSigner(message, _signature);
    }
    ...
    function createContract(
        ...
        address _judge
    ) public payable onlyOwner returns (address _escrowWalletAddress) {
        // createProxy() using EscrowFactory
        address escrowWalletAddress = EscrowFactory(
            escrowFactoryContractAddress
        ).createProxy(_intendedSignatories, _judge, true);

        // send payment to the newly created escrow wallet
        if (msg.value > 0) {
            (bool success, ) = payable(escrowWalletAddress).call{
                value: msg.value
            }("");
        ...
        for (uint256 i = 0; i < _intendedSignatories.length; i++) {
            if (msg.sender == _intendedSignatories[i]) {
                uint256 _tokenId = totalTokensMinted.current();
                _safeMint(_intendedSignatories[i], _tokenId);
                totalTokensMinted.increment();
                _setTokenURI(_tokenId, _contractURI);

                getTokenIdsByEscrowAddress[escrowWalletAddress].push(_tokenId);
                getEscrowAddressByTokenId[_tokenId] = escrowWalletAddress;
            }
        }
        ...
        returns (uint256 _tokenId)
    {
        ...
        for (uint256 i = 0; i < participants.length; i++) {
            if (msg.sender == participants[i]) {
                tokenId = totalTokensMinted.current();
                _safeMint(participants[i], _tokenId);
                totalTokensMinted.increment();
                _setTokenURI(_tokenId, contractURI);

                getTokenIdsByEscrowAddress[_escrowWalletAddress].push(_tokenId);
                getEscrowAddressByTokenId[_tokenId] = _escrowWalletAddress;
...
abstract contract NFT is ERC721, IERC721Receiver, Payable {
    constructor(
        string memory name_,
        string memory symbol_,
        string memory contractURI_
