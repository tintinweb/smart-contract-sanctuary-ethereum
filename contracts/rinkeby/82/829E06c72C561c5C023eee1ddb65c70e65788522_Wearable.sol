...
contract Wearable is ERC721A, ReentrancyGuard, Ownable {
    ...
    modifier mintingGuard(uint256 amount) {
        ...
        require(
            msg.value == amount * mintPrice * 1 wei,
            string.concat(
                "Minting fee is incorrect: ",
                Strings.toString(msg.value),
                " != ",
                Strings.toString(amount * mintPrice)
            )
        );
        require(amount > 0, "You must mint at least 1 token");
        require(amount < 6, "You can't claim more than 5 tokens at a time");
        require(
            (claimed[msg.sender] + amount) < 6,
            "You cannot claim more than 5 tokens"
        );
        require(totalSupply() + amount <= 500, "claim has reached max supply");
        _;
        claimed[msg.sender] += uint16(amount);
    }
    ...
    /* @dev: Widthdraw all ETH to a given address
