...
contract GamesPool is Initializable, UUPSUpgradeable, OwnableUpgradeable {
    ...
    Structs.TransactionFee public transactionFee;
    ...
    function initialize(
        address _rootCaller,
        Structs.TransactionFee memory _transactionFee
    ) public payable initializer {
        rootCaller = _rootCaller;
        proxyRouterContract = ProxyRouterContractInterface(_rootCaller);
    ...
    {
        uint256 _treasuriesLength = proxyRouterContract.getTreasuriesLength();
        uint256 _newAmount = msg.value;
        uint256 fee = calculateCurrentTransactionFeeInternal();
        for (uint256 i = 0; i < _treasuriesLength; i++) {
            Structs.Treasuries memory _treasury;
            _treasury = proxyRouterContract.treasuries(i);
            uint256 _amount = _treasury.amount * fee * msg.value / 100_000_000;
            (bool success, ) = _treasury.treasury.call{
                value: _amount
            }("");
            require(success, "Transfer failed.");
            emit Payout(_treasury.treasury, _amount);
            _newAmount -= _amount;
        }
        reservedAmount[msg.sender].amount += _newAmount;
        ...
        onlyReservedAmountAccount returns(bool)
    {
        if (_amount > 0){
            ...
            require(__bool, "Amount is too big for amount");
        ...
        require(_bool, "Amount is too big for reserved");
        reservedAmount[msg.sender].reserved -= _toReserve;
        return true;
    }
    ...
    function setNewRootCaller(address _newRootCaller) public payable onlyOwner {
        assertOneYocto();
        rootCaller = _newRootCaller;
        proxyRouterContract = ProxyRouterContractInterface(_newRootCaller);
    }
    ...
    function calculateCurrentTransactionFee()
        public
        payable
        returns (uint256)
    {
        assertOneYocto();
        return calculateCurrentTransactionFeeInternal();
    }
    ...
    function calculateCurrentTransactionFeeInternal()
        internal
        returns (uint256)
    {
        if (transactionFee.nextFee != uint256(0)) {
            if (
                block.timestamp >= transactionFee.startTime &&
                transactionFee.startTime != uint256(0)
            ) {
                transactionFee = Structs.TransactionFee({
                    currentFee: transactionFee.nextFee,
                    startTime: uint256(0),
                    nextFee: uint256(0)
                });
            }
        }
        return transactionFee.currentFee;
    }
