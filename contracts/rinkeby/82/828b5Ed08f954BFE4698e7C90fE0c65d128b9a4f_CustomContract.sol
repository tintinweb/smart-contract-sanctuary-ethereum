...
import "@openzeppelin/contracts/utils/cryptography/ECDSA.sol";
    ...
    struct AvatarTypeConfig {
    /// @dev parts that are specific to the avatarType
        string[] partsName;
    /// @dev name of the avatarType
        string name;
    }
...
interface NftInterface {
    ...
    function checkAttributesTaken(uint16 _avatarType, uint16[] calldata _attributes) external returns (bool);
...
contract CustomContract is Pausable, AccessControl, ReentrancyGuard {
    ...
    event MintNft(address indexed to, uint16 _avatarType, uint16[] attributes);
    ...
    constructor(NftInterface _nft, uint256 _nrOfRecycledNfts) {
    ...
    function recycle(
        ...
        ) external whenNotPaused nonReentrant onlyRole(RECYCLE_ROLE) {
        // /// [TODO] avatar type issues here?

        // if (_tokenIds.length != nrOfRecycledNfts) {
        ...
        //     RecycleParam memory currentToken = RecycleParam(_tokenIds[i], _tokenAttributes[i]);

        //     /// @dev This check can revert
        //     checkForOwnership(currentToken.tokenId, _ownerNfts);

        //     uint16[] memory currentTokenParts;

        //     /// @dev This check can revert
        //     checkForNrOfAttributes(currentToken.partPosition.length, maxNrAttributes);
     ...
     */
    function checkForNrOfAttributes(uint256 _length, uint256 _maxNrAttributes) private pure {
        if (_length > _maxNrAttributes) {
