...
contract RevenueSplitter is Context, Ownable, AccessControl {
  ...
  event RegisterCollectionEvent(address); //03a9ee4e183211c0c2ecf72bdf46d5bf73a8e4ffb686489c85fe29944cc03370
  event AddPayeeEvent(address indexed,address indexed,uint256); //7e4a1d75a22ce499c26ab4a27950f7bd712e7c428978afecf7e75aed1c541bf2

  // Token $$ Events
  event BuyEvent(address indexed,uint256,uint256); //02eaa22fbb9062b92c96f349b0e3103065eb20fcdd0de3ea47072c85492cdab0
  event RoyaltyEvent(address indexed,uint256); //61bbd690186cb1063ba2631cb23de3a44843d555f1b0a3cb1a8868a1367e8c64

  // Collaborator $$ Events
  event WithdrawEvent(address indexed,uint256); //5dba113b49cfa7c90315e8e604e6b506f7abcb909b01dcb19ec39005086e68fc
  event DepositEvent(address,address,uint256); //01bee1bfa4116bd0440a1108ef6cb6a2f6eb9b611d8f53260aec20d39e84ee88

  // Payout Events
  event SweepEvent(address indexed,address indexed,uint256); //c23709862a4c577a0eb417be82feb9f2563057b7362ab578babf5c138b470056
  event PayoutEvent(address indexed,uint256); //80e009436f30a4f32133e25b51b5349f2842ea9e93ea4fb6cdaaa4b4e518c687
   ...
   */
  function addCollectionPayee(address _collection, address _payee, uint256 percent) public onlyOwner {
    require(_payee != address(0), "RMX: payee is zero address");
    require(_collections[_collection], "RMX: Unknown collection");
    require(percent <= 1000, "RMX: Percentage too high");
    require(percent > 0, "RMX: shares are 0");
    for (uint i = 0; i < _payees[_collection].length; i++) {
      require(_payees[_collection][i].payee != _payee, "RMX: Payee already registered");
    }
    _payees[_collection].push(CollectionPayee(_payee, percent));

    emit AddPayeeEvent(_collection, _payee, percent);
  }
