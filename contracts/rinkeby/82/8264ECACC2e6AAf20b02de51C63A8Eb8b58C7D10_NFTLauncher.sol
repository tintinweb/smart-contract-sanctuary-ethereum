...
contract NFTLauncher is VRFConsumerBase, ChainlinkClient, Ownable {
    ...
    function getProjectPaymentDetails(uint256 projectid, uint256 paymentMethod) public view returns(
        ...
        bool valid;
        (,paymentMethodAddress, FEE_PERCENTAGE_SAVED_USING_THIS_METHOD, , valid, isBaseToken) = seapadPayments.getProjectPaymentMethod(projectid,paymentMethod);
        require(valid, "Invalid payment method");
        (, uint256 basePaymentMethodID) = seapadPayments.getProjectPaymentMethods(projectid);
        (, basePaymentMethodAddress, , , , , ) = seapadPayments.getPaymentMethod(basePaymentMethodID);
        return (pricePerPackInBaseToken, paymentMethodAddress, basePaymentMethodAddress, isBaseToken, FEE_PERCENTAGE_SAVED_USING_THIS_METHOD);
    }
    ...
    function getPackPrice(
        ...
        returns (
            ...
            uint256 remainderFeePercentageFromPackSale
        )
    {
    ...
    (
        ...
        if (isBasePayment) {
            ...
            uint256[] memory result = seapadPayments
                .getEstimatedTokenNeededToRecieveOtherToken(
                    paymentMethodAddress,
                    basePaymentMethodAddress,
                    pricePerPackInBaseToken
                );
            uint256 slippageFee = (result[0] * slippage2) / 10000;
            _packPrice = result[0] + slippageFee - FEE_TO_REMOVE;
        }
    ...
    function purchasePack(
        ...
        (
            uint256 pricePerPack,
            uint256 removedFeeAmt,
            ,
            uint256 remainderFeePercentageToTakeFromPackSale
        ...
        require(
            token.allowance(msg.sender, address(this)) >= pricePerPack,
            "Please allow contract to spend your tokens!"
        );
        token.transferFrom(msg.sender, address(this), pricePerPack);
        ...
        token.approve(paymentAddress, pricePerPack);
        uint256 slippage2 = slippage;
    ...
    // function calculateProfitForSeapadAndProjectOwner(
    //     uint256 amountOfTokensToConvert,
    //     uint256 projectid,
    //     uint256 paymentMethod
    ...
    //     // uint256 pricePerPack;
    //     // if (isBasePayment) {
    //     //     pricePerPack = project.pricePerPack; //100 DAI
    //     // } else {
    //     //     pricePerPack = getPackPrice(projectid, paymentMethod, 0);
    //     // }
    //     // uint256 profit = (amountOfTokensToConvert * pricePerPack) / 100;
    //     return profit;
    // }
