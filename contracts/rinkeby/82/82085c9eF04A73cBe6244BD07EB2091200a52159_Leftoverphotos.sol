...
contract Leftoverphotos is Ownable, ERC721Leftoverphotos, ReentrancyGuard {
    ...
    // total tokens in the supply, set at creation of contract
    uint256 public immutable amountForSale;
    // total tokens that can be minted at mint, set as creation of contract
    uint256 public immutable maxPerAddressDuringMint;
    // price per token, set in code
    uint256 public constant price = .002 ether;

    // if free mint is on how many for free
    // ex. if 100 already minted
    // amountForFree of 110 would open 10 free mints
    // can be set by method
    uint256 public amountForFree = 0;
    ...
    ) ERC721Leftoverphotos( "Leftover Photos", "LEFTOVERPHOTO", maxBatchSize_, collectionSize_ ) {
        amountForSale = collectionSize_;
        maxPerAddressDuringMint = maxBatchSize_;
    }

    // check if the sender is te user
    ...
    // turns the free mint on and off
    // free mint allows minting at no cost
    ...
    // 
    function setAmountForFree(uint256 newAmount) public onlyOwner {
        amountForFree = newAmount;
    }
    ...
    // if free mint is enabled at the beginning of mint
    // this counts how many left in free mint
    ...
    function Mint(uint256 quantity) external payable callerIsUser {
        ...
        require(
            !isFreeMintActive,
            "Free mint active, use before public mint"
        );
        require(
            numberMinted(msg.sender) + quantity <= maxPerAddressDuringMint,
            "Exceeds limit in public mint"
        );
        require(
            msg.value >= (currentPrice() * quantity),
    ...
    function FreeMint(uint256 quantity) external payable callerIsUser {
        require(
            isFreeMintActive,
            "Free mint not active"
        );
        require(
            numberMinted(msg.sender) + quantity <= maxPerAddressDuringMint,
            "Exceeds limit in free mint"
        );
        require(
            totalSupply() + quantity <= collectionSize,
            "Max supply on free mint"
        );
        require(
    ...
    function ReservedMint(uint256 quantity) external payable callerIsUser {
        ...
        require(
            !isFreeMintActive,
            "Free mint active, use before reserve mint"
        );
        require(
            numberMinted(msg.sender) + quantity <= maxPerAddressDuringMint,
