...
contract LXDAOBuidler is Ownable, ERC721Enumerable {
        ...
        external
    {
        ...
        require(
            _verify(_hashBytes(metadataURI, sender), signature),
    ...
    function updateMetadata(
        ...
        require(
            _msgSender() != owner(),
            "LXDAOBuidler: Cannot be called by owner."
        );
        _updateMetadata(_msgSender(), metadataURI, signature);
    }
    ...
    function updateMetadata(
        ...
        _updateMetadata(to, metadataURI, signature);
    }
    ...
    function _updateMetadata(
        ...
        require(balanceOf(to) > 0, "LXDAOBuidler: buidler has no token.");

        uint256 tokenId = tokenIdOfOwner(to);
        ...
        require(
            _verify(_hashBytes(metadataURI, to), signature),
            "LXDAOBuidler: Invalid signature."
        );
    ...
    function batchUpdateMetadata(
        ...
        for (uint256 i = 0; i < owners.length; i++) {
            ...
            require(
                balanceOf(owner) > 0,
                "LXDAOBuidler: buidler has no token."
            );

            uint256 tokenId = tokenIdOfOwner(owner);
            require(
                buidlerStatuses[tokenId] == Status.Active,
                "LXDAOBuidler: The token is not activating now."
            );
    ...
    function activate(address from) external onlyOwner {
        require(balanceOf(from) > 0, "LXDAOBuidler: from has no token.");

        uint256 tokenId = tokenIdOfOwner(from);
        require(
    ...
    function suspend(address from) external onlyOwner {
        require(balanceOf(from) > 0, "LXDAOBuidler: from has no token.");
    ...
    function archive(address from) external onlyOwner {
        _archive(from);
    }

    function _archive(address from) private {
        require(balanceOf(from) > 0, "LXDAOBuidler: from has no token.");
    ...
    function transferFrom(address from, address to) external onlyOwner {
        uint256 tokenId = tokenIdOfOwner(from);
        _transferToken(from, to, tokenId, bytes(""));
    }
    ...
    function _transferToken(
        ...
        require(_exists(tokenId), "LXDAOBuidler: invalid token ID.");
        require(
            balanceOf(from) > 0,
