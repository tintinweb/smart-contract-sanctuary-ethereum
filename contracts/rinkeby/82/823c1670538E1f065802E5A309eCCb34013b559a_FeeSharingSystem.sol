 ...
 * WETH along with the auto-compounding of RYLT.
 */
contract FeeSharingSystem is ReentrancyGuard, Ownable {
    ...
    IERC20 public immutable ryltRareToken;

    IERC20 public immutable rewardToken;

    ITokenDistributor public immutable tokenDistributor;
    ...
    function deposit(uint256 amount, bool claimRewardToken) external nonReentrant {
        ...
        // Transfer RYLT tokens to this address
        ryltRareToken.safeTransferFrom(msg.sender, address(this), amount);
        ...
        // Verify RYLT token allowance and adjust if necessary
        _checkAndAdjustRYLTTokenAllowanceIfRequired(amount, address(tokenDistributor));
    ...
    /**
     * @notice Calculate value of RYLT for a user given a number of shares owned
     * @param user address of the user
     */
    function calculateSharesValueInRYLT(address user) external view returns (uint256) {
    ...
    /**
     * @notice Calculate price of one share (in RYLT token)
     * Share price is expressed times 1e18
     */
    function calculateSharePriceInRYLT() external view returns (uint256) {
        (uint256 totalAmountStaked, ) = tokenDistributor.userInfo(address(this));
     ...
     */
    function _checkAndAdjustRYLTTokenAllowanceIfRequired(uint256 _amount, address _to) internal {
        if (ryltRareToken.allowance(address(this), _to) < _amount) {
            ryltRareToken.approve(_to, type(uint256).max);
        }
    }
    ...
    function _withdraw(uint256 shares, bool claimRewardToken) internal {
        ...
        // Retrieve total amount staked and calculated current amount (in RYLT)
        (uint256 totalAmountStaked, ) = tokenDistributor.userInfo(address(this));
        ...
        // Transfer RYLT tokens to sender
        ryltRareToken.safeTransfer(msg.sender, currentAmount);
