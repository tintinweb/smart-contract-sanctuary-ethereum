...
error ERC721ABase_TokenOffsetDisabled();
 ...
 */
contract ERC721Offsetable is ERC721AQueryableUpgradeable, OwnableUpgradeable, Offsetable {
    using AddressUpgradeable for address;
    using ECDSAUpgradeable for bytes32;

    enum MintStatus { CLOSED, PUBLIC, PRIVATE }
    ...
    address internal constant _OFFSET_MANAGER = 0x4299D24F13Eafc683a9ad1150EFfc0eb00930fb1;
    bool internal _BYPASS_OFFSET;
     ...
     * @param _unrevealedTokenURI Token URI to return before reveal
     ...
     * @param _bypassOffset Should contract enable offset functionality
     */
    function initialize(
        string memory _name,
        ...
        public
    {
        ...
        _BYPASS_OFFSET = _bypassOffset;
        _tokenURI = _unrevealedTokenURI;
        provenanceHash = _provenanceHash;
        maxSupply = _maxSupply;
        mintConfig = _mintConfig;

        if (_reserveAmount > 0 && _reserveAmount <= _maxSupply)
            _mint(_msgSender(), _reserveAmount);
    }
    ...
    {
        return revealed ? ERC721AUpgradeable.tokenURI(tokenId) : _baseURI();
    }
    ...
    function tokenOffset() public view returns (uint256) {
        if (_BYPASS_OFFSET) revert ERC721ABase_TokenOffsetDisabled();
    ...
    function setOffset(uint256 randomness) public virtual override {
        if (_BYPASS_OFFSET) revert ERC721ABase_TokenOffsetDisabled();
        if (_msgSender() != _OFFSET_MANAGER) revert ERC721ABase_CallerNotOffsetManager();
    ...
    /**
     * @notice Set metadata revealed status and token URI
     * @dev Changing the reveal status will change the tokenURI so be sure to pass the correct `uri`
     * @param status New metadata revealed status
     * @param uri New token base URI
     */
    function setRevealed(bool status, string calldata uri) public lockable onlyOwner {
        if (!_BYPASS_OFFSET && _tokenOffset == 0) revert ERC721ABase_TokenOffsetNotSet();
    ...
    function privateMint(uint256 amount, bytes memory signature) external payable {
        if (mintStatus != MintStatus.PRIVATE) revert ERC721ABase_InvalidMintStatus();
        if (mintConfig.signer != ECDSAUpgradeable.recover(
    ...
    function publicMint(uint256 amount) external payable {
        if (mintStatus != MintStatus.PUBLIC) revert ERC721ABase_InvalidMintStatus();
    ...
    function withdraw() external onlyOwner {
        AddressUpgradeable.sendValue(payable(_msgSender()), address(this).balance);
    }
}
