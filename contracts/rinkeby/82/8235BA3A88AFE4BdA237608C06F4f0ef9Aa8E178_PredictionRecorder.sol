...
contract PredictionRecorder {
    ...
    /**
     * View the latest-round values of the oracle.
     */
    function viewOracleLatestRound() public view returns (uint80, int, uint, uint, uint80) {
        (
            uint80 id,
            int oracleValue,
            uint startedAt,
        ...
        return (id, oracleValue, startedAt, timeStamp, answeredInRound);
    }
...
contract InvitationalBet {
     ...
     */
    function viewInfo() public view returns (address, address, uint, uint, uint, uint, uint80, uint, int) {
        return (
            recorderAddress,
            recorder.viewOracle(),
            targetTime,
    ...
    function payFactorFormula(int _predValue, int _trueValue) public pure returns (uint) {
        /* True value may have a finer unit; auto-adjust the predicted value*/
        while (_predValue * 5 < _trueValue && _trueValue > 10000) {_predValue *= 10;}

        uint diff = uint(_predValue >= _trueValue ? _predValue - _trueValue : _trueValue - _predValue);
        /* For every 0.1% percent off, take a step */
        uint step = uint(_trueValue >= 0 ? _trueValue : -_trueValue) / 1000;
        uint coeff = 2 ** 20;
        for (uint i = 0; i < (diff / step); i++) {
            coeff /= 2;
        }
        return coeff + 1;
    }
    ...
    /**
     * Convenience function for participants to check their own prediction entering the bet.
     */
    function viewMyValidPrediction() public view onlyInvited returns (Prediction memory) {
        ...
        if (predictions.length < 1) {
            return Prediction({
                targetOracle: recorder.viewOracle(),
                ...
                predictionAddress: msg.sender,
                predictionAuthor: "",
                predictionComment: "<EMPTY PREDICTION>"
            });
        } else {
