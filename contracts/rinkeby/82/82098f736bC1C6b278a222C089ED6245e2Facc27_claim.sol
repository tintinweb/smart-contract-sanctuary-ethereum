...
contract claim  {
    ...
    uint256 public standardSupply = 200;
    uint256 public deluxeSupply = 100;
    uint256 public villaSupply = 50;
    uint256 public executiveSupply = 20;
    ...
    function getsupply(uint256 _tokenId) internal view returns (uint256 _supply) {
        if(_tokenId <= 3780) {
        ...
        if(_tokenId <= maxSupply) {
            return executiveSupply;
        }
    }
    function updateSupply(uint256 _supply) internal {
        if(_supply <= 3780) {
            setstandardSupply(standardSupply - 1);
        }
        if(_supply <= 6160) {
            setdeluxeSupply(deluxeSupply - 1);
        }
        if(_supply <= 6860) {
            setvillaSupply(villaSupply - 1);
        }
        if(_supply <= maxSupply) {
            setexecutiveSupply(executiveSupply - 1);
        }
    }
    ...
    function transferERC20(IERC20 token,uint256 tokenId) public {
        ...
        require(getsupply(tokenId)>0, "Witlink ERC20 token has ended");
