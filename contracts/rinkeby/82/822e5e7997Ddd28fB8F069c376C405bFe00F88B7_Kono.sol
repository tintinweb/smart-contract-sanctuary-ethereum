...
contract Kono is ERC721Enumerable, Ownable, ContextMixin, NativeMetaTransaction {
  ...
  uint256 public maxReserved = 299;
  uint256 public currentMintReserved = 0;
  ...
  constructor(
    ...
    tiers.push(Tier("OG", 0.07 ether, 99));
    tiers.push(Tier("Early Bird", 0.08 ether, 900));
    tiers.push(Tier("First Adopter", 0.09 ether, 1000));
    tiers.push(Tier("Bloomer", 0.10 ether, 701));
  }
  ...
  function getPrice(uint256 _mintAmount) public view returns (uint256) {
    ...
    for(uint256 i = 1; i <= _mintAmount; i++) {
      uint256 currentMint = supply + i;
      if(currentMint <= 99) price += tiers[0].cost;
      else if (currentMint <= 999) price += tiers[1].cost;
      else if (currentMint <= 1999) price += tiers[2].cost;
  ...
  function mint(uint256 _mintAmount) public payable {
    ...
    if (msg.sender != owner()) {
        ...
        if(supply < 99) { // `Tier 0 - OG` limit NFT buying per address.
        ...
        require(currentMintReserved + _mintAmount <= maxReserved, "max reserve NFT limit exceeded");
        currentMintReserved += _mintAmount;
    }
  ...
  function getTierInfo(uint256 _tier) public view returns(string memory, uint256, uint256) {
     require(_tier <= 3, "Invalid tier"); 
     return (tiers[_tier].name, tiers[_tier].cost, tiers[_tier].supply);
  }
