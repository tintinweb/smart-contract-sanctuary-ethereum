...
contract ParmeusReport is ERC721URIStorage {
    ...
    constructor() ERC721("Parmeus NFT", "PMS"){
    ...
    function getSelfReports() public view returns(string[] memory tokenUris, uint256[] memory tokenIds){
        // If owner has too many reports, gas may out. Maybe return tokenIds is more reasonable
        if(_holderTokens[msg.sender].length() == 0){
            return (new string[](0), new uint256[](0));
        }
        else{
            string[] memory uriResult = new string[](_holderTokens[msg.sender].length());
            uint256[] memory idResult = new uint256[](_holderTokens[msg.sender].length());
            for(uint16 i=0; i< uriResult.length; i++){
                idResult[i] = _holderTokens[msg.sender].at(i);
                uriResult[i] = tokenURI(_holderTokens[msg.sender].at(i));
            }
            return (uriResult, idResult);
        }
    }
    ...
    function approve(address to, uint256 tokenId) public virtual override {
        require(false, 'Parmeus CV nft can not be delegated');
    }
    ...
    function _beforeTokenTransfer(
        ...
        require(from == address(0) || to == address(0), 'Parmeus CV nft can not be transfered');
    }
