...
// File: contracts/PushinVee.sol

pragma solidity ^0.8.0;

contract PushinVee is ERC721A, Ownable {
    using Strings for uint256;
    ...
    mapping(address => uint256) public veeOwners;

    event Mintable(bool mintable);

    event BaseURI(string baseURI);

    constructor() ERC721A("PushinVee", "PV", MAX_MINT_PER_TX) {}
    ...
    modifier isNotSameAddress(address sender) {
        require(
            veeOwners[sender] != 1,
            "This address cannot mint more than 1 NFT"
        );
        _;
    }
    ...
    function mint()
        public
        isMintable
        isNotExceedMaxMintPerTx(1)
        isNotExceedAvailableSupply(1)
        isNotSameAddress(msg.sender)
    {
        _safeMint(msg.sender, 1);
        veeOwners[msg.sender] = 1;
    }

    function getOwner(address owner) public view returns(uint256){
        return veeOwners[owner];
    }
