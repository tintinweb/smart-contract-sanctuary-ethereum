...
contract GigaMartExchangeTest is GigaMartExecutor {
    ...
    function exchangeSingleItem(
        ...
        // Store amount of eth that paid for offer.
        uint256 ethPaid = _exchange(buy, sigBuy, sell, sigSell);
        // Send leftovers back.
        if (msg.value - ethPaid != 0) {
            buy.outline.maker.transferEth(msg.value - ethPaid);
        }
    ...
    function exchangeMultipleItems(
        ...
        for(uint256 i; i < buy.length;){
            // Store amount of eth that paid for offers.
            ethPaid += _exchange(buy[i], sigBuy[i], sell[i], sigSell[i]);
            unchecked {
                ++i;
            }
        }
        // Send leftovers back.
        if ((msg.value - ethPaid) != 0) {
            msg.sender.transferEth(msg.value - ethPaid);
        }
        if(toInvalidate.length > 0)
            _cancelMultipleOrdersWithPermit(toInvalidate);
    }
...
error InvalidSaleKindForExchange(); /** Thrown when 'saleKinds' of order can't be settled.  */
...
abstract contract GigaMartExecutor is PermitControl, EIP712, ReentrancyGuard {
    ...
    /** Count of succesful ended offers for specific collection offer that identified by order's hash */
    mapping(bytes32 => uint256) public countOfCompletedOffers;
    ...
    function authenticateOrder(
        ...
        if (nonce < minOrderNonces[maker]) return false;
    ...
    function pay(
        ...
        if (requiredAmount > 0) {
            ...
            if (sell.outline.paymentToken != address(0)) {
                ...
                TokenTransferProxy proxy = TokenTransferProxy(tokenTransferProxy);
    ...
    function ordersMatch(
        ...
        /* For 'collectionOffer' 'tokenId' that we want to exchange must be contained in 'extra' array. */
        if (sell.outline.saleKind == Sales.SaleKind.CollectionOffer) {
            return tokenExistsInOrder(sell, buy);
        }
        if (buy.outline.saleKind == Sales.SaleKind.CollectionOffer) {
            return tokenExistsInOrder(buy, sell);
        }
    ...
    /**
     @dev Helper function that is matching if collection offer can be settled with fixed price offer by 'tokenId'.
     @param collectionOffer offer which 'SaleKind' is collection offer.
     @param fixedPriceOffer offer which 'SaleKind' is fixed price offer.
     @return bool that indicates is that 'tokenId' really exists in order.
     */
    function tokenExistsInOrder(
        GigaMartEntities.Order memory collectionOffer,
        GigaMartEntities.Order memory fixedPriceOffer
    ) private pure returns(bool) {
        if (fixedPriceOffer.outline.saleKind != Sales.SaleKind.FixedPrice) {
            revert InvalidSaleKindForExchange();
        }
        uint256 tokenId = _parseTokenId(fixedPriceOffer.data);
        for (uint256 i = 1; i < collectionOffer.extra.length; ) {
            if (collectionOffer.extra[i] == tokenId) {
                return true;
            }
            unchecked {
                ++i;
    ...
    /**
     @dev Helper function that is finalizing collection offers if neccessary.
     @param collectionOfferHash offer which 'SaleKind' is 'collectionOffer'.
     @param fixedPriceOfferHash offer which 'SaleKind' is 'fixedPrice' offer.
     @param limit count of offers that is allowed for this 'collectionOffer'.
     */
    function tryFinalizeCollectionOffer(
        bytes32 collectionOfferHash,
        bytes32 fixedPriceOfferHash,
        uint256 limit
    ) private {
        /** Store number of complete offers out of collection offer. */
        unchecked {
            ++countOfCompletedOffers[collectionOfferHash];
        }
        cancelledOrFinalized[fixedPriceOfferHash] = true;
        if (limit == countOfCompletedOffers[collectionOfferHash]) {
            cancelledOrFinalized[collectionOfferHash] = true;
        }
    }
     ...
     * @return Amount of paid eth, for calculate leftovers and send it back.
     */
    function _exchange(
        GigaMartEntities.Order memory buy,
        GigaMartEntities.Sig calldata sigBuy,
        ...
        address delegateProxy = proxyRegistry.proxies(sell.outline.maker);
        /** Proxy must exist. */
        ...
        if (sell.outline.saleKind == Sales.SaleKind.CollectionOffer) {
            tryFinalizeCollectionOffer(sellHash, buyHash, sell.extra[0]);
        } else if (buy.outline.saleKind == Sales.SaleKind.CollectionOffer) {
            tryFinalizeCollectionOffer(buyHash, sellHash, buy.extra[0]);
        } else {
    ...
    /**
        @dev Helper function that parses incoming data for transferFrom call and returns 'tokenId'
        @param data data for call to proxy
        @return tokenId that should be transferred
     */
    function _parseTokenId(bytes memory data) internal pure returns(uint256 tokenId) {
        assembly {
            /* 0x64 is padding for locate tokenId since first 0x20 bytes for array length, then 0x4 bytes for selector 
            and 0x40 bytes for 'from' and 'to' properties in 'transeferFrom' function. 
            0x64 = 0x20 + 0x4 + 0x40*/
...
library Utils {
  ...
  function Concat(string memory _a, string memory _b, string memory _c, string memory _d, string memory _e)
        ...
        returns (string memory)
    {
      ...
      uint k = 0;
      for (uint i = 0; i < _ba.length; ) {
        babcde[k++] = _ba[i];
        unchecked {
            ++i;
        }
      }
      for (uint i = 0; i < _bb.length; ) { 
        babcde[k++] = _bb[i];
        unchecked {
            ++i;
        }
      }
      for (uint i = 0; i < _bc.length; ) {
        babcde[k++] = _bc[i];
        unchecked {
            ++i;
        }
      }
      for (uint i = 0; i < _bd.length; ) {
        babcde[k++] = _bd[i];
        unchecked {
            ++i;
        }
      }
      for (uint i = 0; i < _be.length; ) {
        babcde[k++] = _be[i];
        unchecked {
            ++i;
        }
        ...
        returns (uint)
    {
        ...
        if (toSearchLength <= inputLength) {
            if (toSearchLength <= 32) {
                ...
                for (uint i = 0; i <= inputLength - toSearchLength; ) {
                    bytes32 testHash;
    ...
    function indexOf(
        ...
        for (uint256 i = _offset; i < _base.length; ) {
...
library GigaMartEntities {
    ...
    struct Outline{
        ...
        address taker;
        /** Kind of sale. {FixedPrice, DecreasingPrice, Auction, Offer, GlobalOffer, CollectionOffer} */
