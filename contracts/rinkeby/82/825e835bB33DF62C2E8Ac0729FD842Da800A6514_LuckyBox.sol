...
contract LuckyBox is VRFConsumerBaseV2, Ownable, ReentrancyGuard {
    ...
    function getAllWinners(uint256 _drawIndex) public view returns (uint256[] memory) {
        ...
        // select winners from start position of the shuffled pool
    ...
    function getWinnerByIndex(uint256 _drawIndex, uint256 _winnerIndex) public view returns (uint256) {
        require(_existsDraw(_drawIndex), "Draw is not exists");
        require(_isDrawSeeded(_drawIndex), "Draw is not seeded");
    ...
    function getWinnerVoterAddress(uint256 _drawIndex, uint256 _winnerIndex) public view returns (address) {
        require(_existsDraw(_drawIndex), "Draw is not exists");
        require(_isDrawSeeded(_drawIndex), "Draw is not seeded");
    ...
    function changeVRFCoordinator(address _contractAddr) external onlyOwner {
        vrfCoordinator = _contractAddr;
        COORDINATOR = VRFCoordinatorV2Interface(vrfCoordinator);
    }

    function changeVRFLinkToken(address _contractAddr) external onlyOwner {
...
interface IVoteBox {
    event PollCreated(uint256 indexed pollId, string topic, uint256 startTime, uint256 endTime, uint optionCount);
