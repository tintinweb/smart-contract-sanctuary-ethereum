  ...
  UUPSUpgradeable
{
  ...
  function createIndexFund(
    ...
    IndexInterface(indexAddress).initialize(_name,_percentages,_tokens,_blocks,_usdc,_dex);
  ...
  function updateIndexFund(
   ...
   IndexInterface(indexAddressArray[id]).udpateindex( _name,  _percentages, _tokens);
    ...
    )
  {
    (name, tokens, percentage) = IndexInterface(indexAddressArray[id])
      .getIndexInfo();
      address indexadd= IndexInterface(indexAddressArray[id]).viewaddress();
    return (name, tokens, percentage,indexadd);
  }
