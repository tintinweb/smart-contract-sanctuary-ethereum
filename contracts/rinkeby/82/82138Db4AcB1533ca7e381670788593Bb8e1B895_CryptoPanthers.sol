...
// @title:  CryptoPanther
// @desc:   CryptoPanther
// @artist: https://glod.art
// @url:    http://cryptopanther.club
...
contract CryptoPanthers is ERC721, ReentrancyGuard, VRFConsumerBase, Ownable {
    ...
    uint256 public constant TEAM_RESERVE = 250;

    // ======== Price =========
    uint256 public constant PRICE_PUBLIC = 0.01 ether;
    uint256 public constant PRICE_WHITELIST = 0.005 ether;
    ...
    // ======== Sale Phase =========    
    enum SalePhase{ Locked, Whitelist, Public, Ended }
    SalePhase phase;
    ...
    // ======== Coupons ========
    address private immutable ownerSigner;
    enum CouponType{ Genesis, Author, Presale }
    struct Coupon {
        bytes32 r;
        bytes32 s;
        uint8 v;
    ...
    ERC721(name_, symbol_) {
        ...
        ownerSigner = ownerSigner_;
        phase = SalePhase.Locked;
    }

    // ======== Reserve Panthers =========
    function reservePanthers() external onlyOwner {
        for(uint256 ind_ = 0; ind_ < TEAM_RESERVE; ind_++) {
    ...
    function setWhitelistPhase() external onlyOwner {
    ...
    // ======== Get Phase =========
    function getPhase() public view returns (SalePhase) {
        return phase;
    }
    ...
    function whitelistMint(uint256 amount_, Coupon memory coupon_)
        ...
        payable
        onlyWhitelistPhase
        validateAmount(amount_, MAX_WHITELIST)
        validateSupply(amount_)
        validateEthPayment(amount_, PRICE_WHITELIST)
        nonReentrant
    {
    ...
    function mint(uint256 amount_) 
        ...
        payable
        onlyPublicPhase
        validateAmount(amount_, MAX_PUBLIC)
        validateSupply(amount_)
        validateEthPayment(amount_, PRICE_PUBLIC)
        nonReentrant
    {
    ...
    function getRandomIndexShift() private returns (bytes32 requestId) {
