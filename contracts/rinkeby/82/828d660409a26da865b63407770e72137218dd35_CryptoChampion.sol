...
contract CryptoChampion is ERC721Enumerable, Ownable {
    using Strings for uint256;
    ...
    function tokenURI(uint256 tokenId) public view override(ERC721) returns (string memory) {
        ...
        if (ENABLE_REVEAL){
            if (currentID == 0 && tokenId < Items[0].order_cnt)
                return string(abi.encodePacked(Items[0].base_uri, tokenId.toString()));
            else if (tokenId >= Items[currentID - 1].order_cnt && tokenId < Items[currentID].order_cnt)
                return string(abi.encodePacked(Items[currentID].base_uri, (tokenId  - Items[currentID - 1].order_cnt).toString()));
            else return UNREVEAL_URI;
        }
        return UNREVEAL_URI;
    ...
    function _mintTo(address toAddress, uint256 numberOfTokens) internal {
        for (uint256 i = 0; i < numberOfTokens; i++) {
            uint256 tokenId = Items[currentID].order_id + Items[currentID].order_cnt;
