...
contract WarrantyNFT is ERC721URIStorage, Ownable {
    ...
    constructor() ERC721("Warranty NFTs", "wNFTs") {
        _tokenIds.increment();
        addProduct("Phone", "xyz.phone", "phoneExpiredWarranty", 15, true);
        addProduct("TV", "xyz.tv", "TVExpiredWarranty", 60, true);
        mintWarrantyNFT(123, 0, 0x5B38Da6a701c568545dCfcB03FcB875f56beddC4);
        mintWarrantyNFT(1234, 1, 0x5B38Da6a701c568545dCfcB03FcB875f56beddC4);
    }
    ...
    function getItem(uint tokenId) public view returns (Item memory) {
        require(tokenId < _tokenIds.current(), "Invalid token ID");
    ...
    function _mint(
        ...
        string memory tokenURI = products[productId].productURI;
    ...
    function _updateWarrantyStatus() public {
        uint256 l = _tokensInWarranty.length;
        for (uint256 i = l; i > 0; i--) {
            uint tokenId = _tokensInWarranty[i - 1];
            Item memory item = tokenIdToItem[tokenId];
            Product memory product = products[item.product];
            if (
                block.timestamp >=
                (item.purchaseTime + product.productWarrantyPeriod)
            ) {
                tokenIdToItem[tokenId].warrantyStatus = false;
                tokenIdToItem[tokenId].itemURI = product.expiredProductURI;
                delete _tokensInWarranty[i - 1];
            }
        }
    }
