...
import './Helper/NFTHelper.sol';

contract FortitudeV2 is ERC721A, Ownable, ReentrancyGuard {
    using Address for address;
    using MerkleProof for bytes32[];
    using SafeMath for uint256;

    struct WhiteListMaxMint {
        address to;
        uint256 amount;
    }
    ...
    constructor(
        ...
        address NFT1CollectionAddress_,
    ...
    function whitelistMint(
        ...
        uint256 amount,
        WhiteListMaxMint memory whiteListMaxMint
    ) external onlySender nonReentrant {
        require(!whitelistMintPaused, 'Whitelist mint is paused');
        require(isAddressWhitelisted(proof, whiteListMaxMint), 'You are not eligible for a whitelist mint');

        bytes memory errorMessage;
        errorMessage = abi.encodePacked('Minting amount exceeds allowance per wallet, As you have claimed already ');
        errorMessage = abi.encodePacked(errorMessage, Strings.toString(whitelistMintedAmount[whiteListMaxMint.to]));
        errorMessage = abi.encodePacked(errorMessage, ' of ');
        errorMessage = abi.encodePacked(errorMessage, Strings.toString(whiteListMaxMint.amount));
        errorMessage = abi.encodePacked(errorMessage, ' whitelisted amount, you can only mint ');
        errorMessage = abi.encodePacked(errorMessage, Strings.toString(whiteListMaxMint.amount - whitelistMintedAmount[whiteListMaxMint.to]));
        errorMessage = abi.encodePacked(errorMessage, ' more');

        // Will be used only if we are checking how much the whitelisted have minted in Fortitude V1
        require(
            whitelistMintedAmount[whiteListMaxMint.to] + amount <= whiteListMaxMint.amount,
            string(errorMessage)
        );

        // Will be used only if we are checking how much the whitelisted have minted in Fortitude V1 & How much we currently hold
        // require(
        //     (whitelistMintedAmount[whiteListMaxMint.to] + amount <= whiteListMaxMint.amount) &&
        //         (whitelistMintedAmount[whiteListMaxMint.to] + amount <= NFTHelper(_NFT1CollectionAddress).balanceOf(whiteListMaxMint.to)),
        //     'Minting amount exceeds allowance per wallet'
        // );

        _mintWithoutValidation(whiteListMaxMint.to, amount);

        whitelistMintedAmount[whiteListMaxMint.to] += amount;
    }
    ...
    function isAddressWhitelisted(bytes32[] memory proof, WhiteListMaxMint memory _whiteListMaxMint)
        public
        view
        returns (bool)
    {
        return isAddressInMerkleRoot(whitelistMerkleRoot, proof, _whiteListMaxMint);
    }

    function isAddressInMerkleRoot(
        bytes32 merkleRoot,
        bytes32[] memory proof,
        WhiteListMaxMint memory _whiteListMaxMint
    ) internal pure returns (bool) {
        return proof.verify(merkleRoot, keccak256(abi.encodePacked(_whiteListMaxMint.to, _whiteListMaxMint.amount)));
    }
