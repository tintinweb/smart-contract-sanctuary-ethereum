...
contract TradeCoinERC721 is ERC721, ERC721Enumerable, ERC721Burnable, Ownable, RoleControl {
    ...
    event initialTokenizationEvent(uint indexed tokenId, address indexed functionCaller);

    event mintAfterSplitOrBatchEvent(uint indexed tokenId, address indexed functionCaller);
    ...
    event productBurned(address indexed functionCaller, uint indexed tokenId);
    ...
    modifier atState(State _state, uint _tokenId) {
        require(tradeCoin[_tokenId].state == _state, "ProductNFT not in the right state");
        _;
    }

    modifier notAtState(State _state, uint _tokenId) {
        require(tradeCoin[_tokenId].state != _state, "ProductNFT can not be in the current state");
        _;
    }

    modifier onlyLegalOwner(address _sender, uint _tokenId) {
        require(ownerOf(_tokenId) == _sender, "Sender is not owner of NFT.");
        _;
    }

    modifier isLegalOwnerOrCurrentHandler(address _sender, uint _tokenId){
        require(tradeCoin[_tokenId].currentHandler == _sender || ownerOf(_tokenId) == _sender, "Given address is not the Owner nor current Handler.");
        _;
    }
    ...
    function initiateCommercialTx(
        ...
        emit initiateCommercialTxEvent(_tokenId, _msgSender(), _newOwner, stringToBytes32(_docHash), _docType, _rootHash, _payInFiat);
    }
        ...
        atState(State.PendingCreation, _tokenId)
    {
        ...
        if(!paymentInFiat[_tokenId]){
            require(priceForOwnership[_tokenId] != 0, "This is not listed as an offer");
        ...
        emit approveTokenizationEvent(_tokenId, _legalOwner, _msgSender(), _docHash, _docType, _rootHash);
    }
    ...
    function changeStateAndHandler(
        ...
        string memory _docType,
        string memory _rootHash
    ) public onlyLegalOwner(_msgSender(), _tokenId) {
    ...
    ) public onlyLegalOwner(_msgSender(), _tokenId) notAtState(State.PendingCreation, _tokenId) {
    ...
    function finishCommercialTx(uint _tokenId) public payable {
        require(
            addressOfNewOwner[_tokenId] == _msgSender(),
            "You don't have the right to pay"
        );
        ...
        emit productBought(legalOwner, _msgSender(), _tokenId);
    }
    ...
    function addInformation(uint _tokenId, string memory _docHash, string memory _docType, string memory _rootHash) public onlyInformationHandlerOrAdmin {
        ...
        emit addInformationEvent(_tokenId, _msgSender(), stringToBytes32(_docHash), stringToBytes32(_docType), stringToBytes32(_rootHash));
    }
