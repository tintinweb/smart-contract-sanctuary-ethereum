    ...
    AccessControlUpgradeable
{
    ...
    // Mapping per collection name and wallet to storage the blocklist of users that have been claimed any NFT
    mapping(uint256 => mapping(address => bool)) public blocklistPerWallet;
    // Mapping per collection name and token ids to storage the blocklist of users that have been claimed any NFT
     ...
     * @param qualifyingTokens  which tokenIds are being used to qualify for the, i.e. which Voxo Ids are used to qualify for the claimableTokens
    ...
    event ClaimedPerVoxoERC1155(
        ...
        uint256[] qualifyingTokens,
        address tokenAddress,
        string collection
    );
    /**
     * @dev Event for Each NFT ERC1155 Token Claimed by the holders of VoxoDeus
     * @param toAddress Address of the claimant
     * @param tokenIds Identifiers of the ERC1155 token the VoxoUser is allowed to claim
     * @param tokenQuantities Quantity of each ERC1155 token the VoxoUser is allowed to claim
     * @param tokenAddress Address of the ERC1155 token
    ...
    /**
     * @dev Event for Each NFT ERC721 Token Claimed by the holders of VoxoDeus
     * @param toAddress Address of the claimant
    ...
    function claimERC1155(
        ...
        require(
            ...
            "VoxoDeus: Invalid reception Wallet"
        );
        require(
            !hasPriorClaim(collectionName, _msgSender()),
        ...
        require(
            ...
            "VoxoDeus: Cannot claim any token that has not been signed by the owner"
        );
    ...
    /**
     * @dev Function for Claim NFT ERC1155 Token based on payload signed, and filter Collection and VoxoId
     * @dev permitted to claim NFT ERC1155 Token based signed by the Distribution Wallet
     * @dev Explanation of this method:
    ...
    function claimPerVoxoERC1155(
        ...
        uint256[] calldata claimableTokenIds,
        uint256[] calldata claimableQuantities,
        uint256[] calldata qualifyingTokens,
        uint256 deadline,
        address tokenAddress,
        string memory _collectionName
    ) external {
        ...
        require(
            ...
            "VoxoDeus: At least one VoxoDeus tokenId has already been used to claim"
        );
        // Check if the wallet have the ownership of the Voxo Token Ids, in case send a valid array of Voxo TokenIds
        if ((qualifyingTokens.length > 0) && (qualifyingTokens[0] != 0)) {
            require(
                voxoOwnership(qualifyingTokens, voxoTokenHoldings(toAddress)),
                "VoxoDeus: At least one Qualifying Tokens is not currently owned by the claimant"
            );
        }
        ...
        // Helpers to add list of tokenIDs claimed
        addBlocklistPerVoxo(
            claimableTokenIds,
            getIndexCollection(_collectionName)
        );
    ...
    function claimPerVoxoERC721(
        ...
        require(
            !isTokenClaimed(collectionName, claimableTokenIds),
            "VoxoDeus: Assets have already been claimed"
        );
        // Check if the wallet have the ownership of the Voxo Token Ids, in case send a valid array of Voxo TokenIds
        if ((qualifyingTokens.length > 0) && (qualifyingTokens[0] != 0)) {
            require(
                voxoOwnership(qualifyingTokens, voxoTokenHoldings(toAddress)),
                "VoxoDeus: At least one Qualifying Tokens don't have ownership"
            );
        }
        ...
        addBlocklistPerVoxo(claimableTokenIds, collectionName);
     ...
     * @param qualifyingTokens which tokenIds are being used to qualify for the, i.e. which Voxo Ids are used to qualify for the define quantityTokens
     * @param deadline timestamp of deadline to claim
     * @param tokenAddress Token Address permit to claim the NFT ERC721 Token
     */
    function claimMintERC721(
        ...
        if ((qualifyingTokens.length > 0) && (qualifyingTokens[0] != 0)) {
            require(
                voxoOwnership(qualifyingTokens, voxoTokenHoldings(toAddress)),
                "VoxoDeus: of at least one Qualifying Tokens the wallet don't have ownership"
            );
        }
        ...
        return
            _hashTypedDataV4(
                keccak256(
                    abi.encode(
                        keccak256(
                            "ClaimAllowance(address to_address,uint256 quantities,address token_address,string collection_name)"
                        ),
                        toAddress,
                        quantities,
                        token_address,
    ...
    function hasPriorClaim(uint256 collectionName, address toAddress)
        public
        view
        returns (bool)
    {
    ...
    /**
     * @dev Check if any of the provided VoxoDeus tokenIds were previously used in a successful claim.
    ...
    function addBlocklistPerVoxo(
        ...
        for (uint256 i = 0; i < claimedTokenIds.length; i++) {
            blocklistPerVoxo[_collection][claimedTokenIds[i]] = true;
        }
    }

    /**
     * @dev Verify is the address is owner of one or several Voxos
    ...
    /**
     * @dev Overloadind before method to Verify is the address is owner of one or several Token of NFT smart Contract, passed in the args
     * @param _owner address to owner of the tokens
     * @return tokenIds Array of token Ids of Yield Keys who the _owner address is the owner
     */
     ...
     * @return true if all qualifyingTokenIds are currently ownerd by the claiment
     */
    ...
    /**
     * @dev Method for subtrate from array token ids hold before the mint process
