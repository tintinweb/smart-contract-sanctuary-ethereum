...
contract ERC721A is Context, ERC165, IERC721, IERC721Metadata, ReentrancyGuard {
    ...
    uint256 internal _currentIndexDiamond = 44;
    uint256 internal _currentIndex = 469;
    ...
    function totalSupply() public view returns (uint256) {
        return (_currentIndex-469 + _currentIndexDiamond-44 + _currentIndexGold-1);
    }
    ...
    function _ownershipOf(uint256 tokenId) internal view returns (TokenOwnership memory) {
        ...
        unchecked {
            if ( (1 <= curr && curr < _currentIndexGold) || (44 <= curr && curr < _currentIndexDiamond) || (469 <= curr && curr < _currentIndex) ) {
...
contract AR2 is ERC721A, Ownable {  
    using Strings for uint256;
    ...
    function mint(bytes32[] calldata _merkleProof) payable external nonReentrant {
        if (tx.origin != msg.sender) revert MintFromContract();
        if (sale_state == 0) revert SaleNotActive();

        uint256 state = 0;
        uint256 mintAmount = 0;
        if (MerkleProof.verify(_merkleProof, merkle_root_gold, keccak256(abi.encodePacked(_msgSender())))) {
            require(_numberMinted(_msgSender()) < mint_limit_gold, "tried to mint too many gold");
            state = 1; // gold
            mintAmount = mint_limit_gold;
        } else if (MerkleProof.verify(_merkleProof, merkle_root_team, keccak256(abi.encodePacked(_msgSender())))) {
            require(_numberMinted(_msgSender()) < mint_limit_team, "tried to mint too many investor/team");
            mintAmount = mint_limit_team;
        } else if (MerkleProof.verify(_merkleProof, merkle_root_og, keccak256(abi.encodePacked(_msgSender())))) {
            require(_numberMinted(_msgSender()) < mint_limit_og, "tried to mint too many og");
            mintAmount = mint_limit_og;
            state = 2;
        } else if (MerkleProof.verify(_merkleProof, merkle_root_fr, keccak256(abi.encodePacked(_msgSender())))) {
            require(_numberMinted(_msgSender()) < mint_limit_fr, "tried to mint too many fr");
            mintAmount = mint_limit_fr;
        } else if (MerkleProof.verify(_merkleProof, merkle_root_wl, keccak256(abi.encodePacked(_msgSender())))) {
            require(_numberMinted(_msgSender()) < mint_limit_wl, "tried to mint too many wl");
            require(msg.value > cost*mint_limit_wl, "wl didn't pay enough");
            mintAmount = mint_limit_wl;
        } else {
