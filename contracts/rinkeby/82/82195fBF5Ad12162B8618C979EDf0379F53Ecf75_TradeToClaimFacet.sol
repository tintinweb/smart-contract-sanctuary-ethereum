...
contract TradeToClaimFacet is IERC721Receiver {
    ...
    /// @notice Trades in Keg Plebs token(s) for corresponding Pleb token(s)
    /// @dev Requirements:
    ...
    function tradeToClaim(uint256 _tokenId) external {
        ...
        // Transfers Key Pleb tokens to this contract
        kegpleb.transferFrom(LibMeta.msgSender(), address(this), _tokenId);

        // Transfers a reserved Pleb token with the same ID to the owner of the Keg Pleb token received
    ...
    /// @notice Needed to use 'safeMint' in reserving tokens to this address
