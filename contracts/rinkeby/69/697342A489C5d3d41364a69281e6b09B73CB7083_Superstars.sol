...
contract Superstars is ERC721Enumerable, Ownable {
    ...
    mapping(address => uint256) presaleMintCount;

    // PUBLIC SALE
    bool public publicActive = false;

    
    constructor(string memory baseURI) ERC721("superstars", "SUPERSTARS") {
        setBaseURI(baseURI);
    }

    // MINT FUNCTIONS

    function presaleMint(uint256 _count, bytes32[] calldata _merkleProof)
        public
        payable
    {
        ...
        require(
            (presaleMintCount[msg.sender] + _count) <= presaleLimit,
            "Wallet already claimed whitelist mint."
        );
        require(totalMinted.add(_count) <=  presaleSupply, "Not enough NFTs left in Presale Supply!");
        require(
            _count > 0,
    ...
    function reserveNFTs(uint256 _count) public onlyOwner {
        ...
        if (_count >= RESERVE_SUPPLY) {
            RESERVE_SUPPLY = 0;
        } else {
            RESERVE_SUPPLY -= _count;
        }
    }
    ...
    function giftNFT(address _addy, uint256 _count) public onlyOwner {
        uint256 nextID = _tokenIds.current();

        require(
            nextID.add(_count) < MAX_SUPPLY,
            "Not enough NFTs left to gift"
        );

        for (uint256 i = 0; i < _count; i++) {
            uint256 newTokenID = _tokenIds.current();
            _safeMint(_addy, newTokenID);
            _tokenIds.increment();
        }
    }
