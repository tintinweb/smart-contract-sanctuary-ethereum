...
contract CurveRouter {
    ...
    ICurvePool threePoolSwap;
    IERC20 threePoolToken;

    constructor(address _swapPool, address _tToken) {
        threePoolSwap = ICurvePool(_swapPool);
    ...
    function exchange_underlying_routed(
        ...
        for (uint256 x = 0; x < path.length; x++) {
            ...
            address coin = ZERO_ADDRESS;

            if (is_meta_3pool(path[x]) && i > 0) {
                coin = threePoolSwap.coins(uint256(int256(i - 1)));
            } else {
