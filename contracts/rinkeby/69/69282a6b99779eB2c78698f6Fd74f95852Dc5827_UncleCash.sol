...
contract UncleCash is Context, IERC20, Ownable {
    ...
    string private _name = "Uncle Cash";
    string private _symbol = "UNCC";
    ...
    bool public initialLiqDone = false;
    ...
    function addInitialLiquidity(uint256 amount) public payable onlyOwner{
    ...
    function _transfer(
        ...
        //transfer amount, it will take tax, burn, liquidity, market and dev fee
    ...
    function _tokenTransfer(
        ...
        if (_isExcludedFromFee[sender] || _isExcludedFromFee[recipient]) {
            ...
            charityAmt = amount.mul(_charityFee).div(100);
            marketingAmt = amount.mul(_marketingFee).div(100);
            devAmt = amount.mul(_devFee).div(100);
        }

        //Calculate burn amount and charity amount

        if (sender != uniswapV2Pair) {
            amount = amount.sub(devAmt);
        }
        ...
        if (_isExcluded[sender] && !_isExcluded[recipient]) {
            _transferFromExcluded(
                sender,
                recipient,
                (amount.sub(burnAmt).sub(charityAmt).sub(marketingAmt))
            );
        ...
        //Temporarily remove fees to transfer to burn address, charity, marketplace and dev wallets
        uint256 _preivousTaxFee = _taxFee;
