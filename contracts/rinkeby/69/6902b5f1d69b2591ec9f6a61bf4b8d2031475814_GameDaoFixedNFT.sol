...
contract GameDaoFixedNFT is Configurable, IERC721ReceiverUpgradeable {
    ...
    */
    ...
    function GetThisBytes32() public pure returns  (bytes32,bytes32) {
        return (DisableErc721,Token1WhiteList);
    }
...
contract Configurable is Governable {
    ...
    function getConfig(bytes32 key, address addr) public view returns (uint) {
        return config[bytes32(uint(key) ^ uint160(addr))];
    }
    ...
    function _setConfig(bytes32 key, address addr, uint value) internal {
        _setConfig(bytes32(uint(key) ^ uint160(addr)), value);
    }
