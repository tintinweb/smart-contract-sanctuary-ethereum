...
contract StakingRewardsFactory is Ownable {
    ...
    constructor(address _rewardsToken, uint256 _stakingRewardsGenesis)
        Ownable()
    {
        require(address(_rewardsToken) != address(0));
    ...
    function update(
        ...
        info.rewardAmount = rewardAmount;
        info.duration = rewardsDuration;
        claimRewardAmount(stakingToken);
    }
    ...
    function claimRewardAmounts() public {
        ...
        uint256 _index;
        uint256 _to = stakingTokens.length;
        for (_index = 0; _index < _to; _index++) {
            claimRewardAmount(stakingTokens[_index]);
        }
    }
    ...
    function claimRewardAmount(address stakingToken) public {
        ...
        if (info.rewardAmount > 0 && info.duration > 0) {
            ...
            if (!StakingRewards(info.stakingRewards).isInitialized()) {
                StakingRewards(info.stakingRewards).initializeDefault();
            }
    ...
    function pullExtraTokens(address token, uint256 amount) external onlyOwner {
        require(address(token) != address(rewardsToken));
...
/// @notice You can use this contract for simulation staking
/// @dev You should modify this contract based on your requirements
    ...
    ReentrancyGuard
{
    ...
    /// @notice Staking active state
    uint256 public periodFinish;
    /// @notice Rewards Rate per a each staking contract
    uint256 public rewardRate;
    /// @notice Rewards Duration per a each staking contract
    uint256 public rewardsDuration;
    /// @notice Staked Last Time Stamp
    uint256 public lastUpdateTime;
    /// @notice Staked Last Time Stamp
    uint256 public rewardPerTokenStored;
    /// @notice Staking init state
    ...
    constructor(
        ...
        require(address(_rewardsDistribution) != address(0));
        require(address(_rewardsToken) != address(0));
        require(address(_stakingToken) != address(0));
        require(address(_stakingToken) == address(_rewardsToken));
    ...
    function initializeDefault() external onlyRewardsDistribution nonReentrant {
    ...
    {
        require(isInitialized == true, "!Initialized");
        require(amount > 0, "Cannot stake 0");
        require(_lockingTimeStamp[msg.sender] <= 0);
        require(block.timestamp < periodFinish, "Not active");
        uint256 stakingBefore = stakingToken.balanceOf(address(this));
        stakingToken.safeTransferFrom(msg.sender, address(this), amount);
        uint256 stakingAfter = stakingToken.balanceOf(address(this));
        assert(stakingBefore == stakingAfter.sub(amount));
        _totalSupply = _totalSupply.add(amount);
        _balances[msg.sender] = _balances[msg.sender].add(amount);
        emit Staked(msg.sender, amount);
    }

    function stakeTransferWithBalance(uint256 amount, uint256 lockingPeriod)
        external
        nonReentrant
        updateReward(msg.sender)
    {
        require(isInitialized == true, "!Initialized");
        require(amount > 0, "Cannot stake 0");
        require(lockingPeriod <= rewardsDuration, "Invalid locking period");
        ...
        updateReward(msg.sender)
    {
        ...
        if (_lockingTimeStamp[msg.sender] > 0) {
            require(
                ...
                "Unable to withdraw within locking period"
            );
        ...
        updateReward(address(0))
    {
        ...
        require(
            rewardRate <= balance.div(_rewardsDuration),
