...
contract Lottery is LotteryOwnable, Initializable {
    ...
    // State variables 
    // Instance of Yumi token (collateral currency for lotto)
    IERC20 internal yumi_;
    // Storing of the NFT
    ...
    struct LottoInfo {
        ...
        uint256 prizePoolInYumi;    // The amount of yumi for prize money
        uint256 costPerTicket;      // Cost per ticket in $yumi
    ...
    {
        prize = yumi_.balanceOf(address(this)).sub(totalClaimable);
    }
        ...
        internal
    {
        ...
        allLotteries_[_lotteryId].prizePoolInYumi = getPrizeForCurrentLottery();
        ...
        onlyOwner() 
    {
        ...
        if(lastTicketId == allLotteries_[_lotteryId].endTicketID.add(1)) {
            ...
            uint256 claimablePrize = allLotteries_[_lotteryId].prizePoolInYumi.mul(winPercent).div(100);
        ...
        returns(uint256 lotteryId)
    {
        ...
        if(lotteryId > 1) {
            uint256 burnAmount = allLotteries_[lotteryId-1].prizePoolInYumi.mul(allLotteries_[lotteryId-1].prizeDistribution[0]).div(100);
            yumi_.safeTransfer(burnPool_, burnAmount);
        }
        ...
        notContract()
    {
        ...
        // Transfers the required yumi to this contract
        yumi_.transferFrom(
            msg.sender, 
            address(this), 
            costWithDiscount
    ...
    function claimReward(uint256 _lotteryId, uint256 _tokenId) external notContract() {
        ...
        uint256 yumiBalance = yumi_.balanceOf(address(this));
        if(yumiBalance < prizeAmount)
            yumi_.safeTransfer(address(msg.sender), yumiBalance);
        else
            yumi_.safeTransfer(address(msg.sender), prizeAmount);
    }
        ...
        notContract()
    {
        ...
        uint256 yumiBalance = yumi_.balanceOf(address(this));
        if(yumiBalance < totalPrize)
            yumi_.safeTransfer(address(msg.sender), yumiBalance);
        else
            yumi_.safeTransfer(address(msg.sender), totalPrize);
    }
     ...
     * @return  uint256: The prize amount in yumi the user is entitled to 
     */
        ...
        returns(uint256) 
    {
        ...
        prize = allLotteries_[_lotteryId].prizePoolInYumi.mul(perOfPool).div(100).div(winnerCountsOfPool);
