...
contract Ed3Educators is ERC721A, Ownable, IEd3Educators, IEd3EducatorsMetadata {
    using Strings for uint256;

    uint256 public constant SUPPLY_RESERVED = 280;
    uint256 public constant PUBLIC_SUPPLY = 6000;
    uint256 public constant MAX_SUPPLY = SUPPLY_RESERVED + PUBLIC_SUPPLY;
    ...
    function purchase(uint256 numberOfTokens) external payable override {
        ...
        require(
            totalPublicSupply < PUBLIC_SUPPLY,
        ...
        for (uint256 i = 0; i < numberOfTokens; i++) {
            if (totalPublicSupply < PUBLIC_SUPPLY) {
    ...
    function purchasePreSale(uint256 numberOfTokens) external payable override {
        ...
        require(
            totalPublicSupply + numberOfTokens <= PUBLIC_SUPPLY,
            "Purchase would exceed PUBLIC_SUPPLY"
        );
        require(
    ...
    function reserve(address to, uint256 amount) external override onlyOwner {
        require(totalSupply() < MAX_SUPPLY, "All tokens have been minted");
        require(totalReservedSupply + amount <= SUPPLY_RESERVED,"Reserve would exceed max reserved tokens");

        for (uint256 i = 0; i < amount; i++) {
            uint256 tokenId = PUBLIC_SUPPLY + totalReservedSupply + 1;
    ...
    function setIsPreSaleActive(bool _isPreSaleActive) external override onlyOwner
    {
