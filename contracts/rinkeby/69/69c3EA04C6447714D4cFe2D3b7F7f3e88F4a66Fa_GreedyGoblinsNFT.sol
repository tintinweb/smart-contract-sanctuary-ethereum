...
contract GreedyGoblinsNFT is Ownable, ERC721Checkpointable, GreedyGoblinsLottery {
    ...
    function setWhiteList(address[] calldata addresses) external onlyOwner {
        uint8 numAllowedToMint = 3;
        uint256 WhiteListLength = addresses.length;
    ...
    function _mint(uint numberOfTokens) internal {
    ...
    function _presalemint(uint numberOfTokens) internal {
        uint256 ts = totalSupply();
    ...
    function _mintWhiteList(uint8 numberOfTokens) internal {
        uint256 ts = totalSupply();
        uint256 br = 1;
    ...
    function _mintLottery() internal {
        uint256 ts = totalSupply();
        uint256 LotteryTokens = 2;
        require(_LotteryAllow[winner]==2);
    ...
    function mintNFT(uint8 numberOfTokens) public payable {
     uint256 br = _allowList[msg.sender];
     uint256 brNFT = balanceOf(msg.sender);

     require(brNFT+numberOfTokens<=10,"Maximum amount of Nft-s per account is 5");

     if ( _LotteryAllow[winner]==2) {
         _mintLottery(); 
         }
     
     else if((br!=1 &&  _mintWhitelist[msg.sender]+numberOfTokens<=3) && msg.value==WhiteList_PRICE*numberOfTokens ){
...
contract GreedyGoblinsLottery is VRFConsumerBase {
    ...
    function newLotteryAdmin(address newLotteryAdminAddress) public virtual returns (address) {
        require(LotteryAdmin == msg.sender);
