...
contract TestContract is ERC721AQueryable, Ownable, ReentrancyGuard {
  ...
  constructor(
    ...
    setMaxFreeMint(_maxFreeMint);
    setHiddenMetadataUri(_hiddenMetadataUri);
  }

  modifier mintCompliance(uint256 _mintAmount) {
    require(_mintAmount > 0 && _mintAmount <= maxMintAmountPerTx, 'Invalid mint amount!');
    require(_numberMinted(msg.sender) + _mintAmount <= maxMintAmountPerWallet, 'Max mint per wallet exceeded!');
  ...
  modifier mintPriceCompliance(uint256 _mintAmount) {
    ...
    if(numMinted < maxFreeMint && numMinted + _mintAmount > maxFreeMint) {
      require(msg.value == cost * (numMinted + _mintAmount - maxFreeMint), 'Insufficient funds!');
    } 
