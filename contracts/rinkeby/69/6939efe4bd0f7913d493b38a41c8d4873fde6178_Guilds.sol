...
contract Guilds is ERC1155Supply {
    ...
    constructor() ERC1155("") {
        ...
        _lastModId = 1526;
        _lastTicket = 6048;
        deployerOne = 0x9B3790E97e9aA90cd8D225dDBD44de8dFAb2218d;
        deployerTwo = 0x83d1504c59e5ae459d1C7FD12C3B541e22Ad0ceE;
    }
    ...
    function lockSpots(uint256 _tokenId, uint _unlockDate, uint _lockDate) public onlyRole(ADMIN, _tokenId) {
        ...
        // AllGuilds[_tokenId].Locked = true;
    }

    // function openMetaData(uint256 _tokenId) public onlyRole(ADMIN, _tokenId) {
    //     AllGuilds[_tokenId].FreezeMetaData = false;
    // }
    ...
    // function removeGuildRules(string[] memory _rules, uint256 _tokenId) public onlyRole(ADMIN, _tokenId) {
    //     for (uint256 i = 0; i < _rules.length; ++i) {
    //         AllGuilds[_tokenId].GuildRules.push(_rules[i]);
    //     }
    // }
    ...
    function _beforeTokenTransfer(
        ...
        for (uint256 i = 0; i < ids.length; ++i) {      
            ...
            if (_tokensLeft == amounts[i]) {
                ...
                if(totalSupply(ids[i]) - amounts[i] < 0) {
                _totalGuilds -= 1;
    ...
    function modMint(uint256 _amount, uint256 _id) public payable onlyRole(MODERATOR, _id) {
        if (block.timestamp <= (AllGuilds[_id].LockDate)) {
        require(block.timestamp >= (AllGuilds[_id].UnlockDate), "Guild spots are locked");
        }
        // require(!AllGuilds[_id].Locked, "Guild spots are locked");
    ...
    function assignModerator(
        ...
        // remove guild from member
        // delete GuildsByAddress[_account][indexOfUint256(GuildsByAddress[_account], _id)];
        // remove member from guild
    ...
    function burn(uint256 _id, uint256 _amount, address _address) public onlyRole(ADMIN, _id) {
        // delete GuildsByAddress[_address][indexOfUint256(GuildsByAddress[_address], _id)];
    ...
    function withdraw() public {
        require (msg.sender == deployerOne || msg.sender == deployerTwo);
