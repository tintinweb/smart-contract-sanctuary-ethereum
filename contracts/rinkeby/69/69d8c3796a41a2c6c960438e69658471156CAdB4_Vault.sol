...
contract Vault is IVault, IncreOwnable {
    ...
    */
    ...
    function deposit(
        ...
        return _deposit(marketIdx, user, amount, depositToken, isTrader);
    }
    ...
    function withdrawAll(
        ...
        return _withdraw(marketIdx, user, fullAmount.toUint256(), withdrawToken, isTrader);
    }
    ...
    function withdrawPartial(
        ...
        _withdrawPartial(marketIdx, user, reductionRatio, isTrader);
    }
    ...
    function withdraw(
        ...
        _withdraw(marketIdx, user, tokenAmount, withdrawToken, isTrader);
    }
    ...
    function getTraderReserveValue(uint256 marketIdx, address trader) external view override returns (int256) {
        return _getUserReserveValue(marketIdx, trader, false, true);
    }
    ...
    function getLpReserveValue(uint256 marketIdx, address lp) external view override returns (int256) {
        return _getUserReserveValue(marketIdx, lp, false, false);
    }
    ...
    function _deposit(
        ...
        _changeBalance(user, marketIdx, tokenIdx, wadAmount.toInt256(), isTrader);
    ...
    function _withdrawPartial(
        ...
        // Amount to withdraw accross all collaterals
        int256 usdReserveValue = _getUserReserveValue(marketIdx, user, true, isTrader);
        int256 usdAmountToWithdraw = usdReserveValue.wadMul(reductionRatio.toInt256());

        Collateral[] memory collaterals = whiteListedCollaterals;
        int256 collateralBalance;
        int256 collateralUSDValue;
        int256 collateralSellRatio;
        int256 collateralAmountToWithdraw;

        // Withdraw from the UA balance last
        for (uint256 i = collaterals.length; i > 0; i--) {
            ...
            collateralUSDValue = _getUndiscountedCollateralUSDValue(collaterals[i - 1], collateralBalance);

            if (collateralUSDValue < usdAmountToWithdraw) {
                _withdraw(marketIdx, user, collateralBalance.toUint256(), collaterals[i - 1].asset, isTrader);
                usdAmountToWithdraw -= collateralUSDValue;
            } else {
                collateralSellRatio = usdAmountToWithdraw.wadDiv(collateralUSDValue);
                collateralAmountToWithdraw = collateralBalance.wadMul(collateralSellRatio);

                _withdraw(marketIdx, user, collateralAmountToWithdraw.toUint256(), collaterals[i - 1].asset, isTrader);
                break;
            }
        }
    }
    ...
    function _withdraw(
        ...
        emit Withdraw(marketIdx, user, address(withdrawToken), tokenAmount);
    }
    ...
    function _sellCollateral(
        ...
        _changeBalance(liquidatee, marketIdx, UA_IDX, uaDebtSettled.toInt256(), isTrader);
        _changeBalance(
            liquidatee,
            marketIdx,
            tokenToCollateralIdx[collateral.asset],
            -collateralAmountToSell.toInt256(),
            isTrader
        );
    ...
    /// @param isTrader Whether user is a trader or not
    /// @return reserveValue Value of collaterals in USD. 18 decimals
