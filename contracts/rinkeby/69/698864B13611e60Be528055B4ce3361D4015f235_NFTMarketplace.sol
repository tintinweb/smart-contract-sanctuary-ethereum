    ...
    IERC721Receiver
{
    ...
    */
    ...
    function cancelBuyOrder(uint256 orderId) external nonReentrant {
        ...
        require(order.owner == _msgSender(), "Caller must own the buy order");
        ...
        nonReentrant
    {
        ...
        Order storage order = buyOrders[orderId];
        require(order.amount > decreaseAmount, "order.amount > decreaseAmount");
    ...
    function cancelSellOrder(uint256 nftId) external nonReentrant {
        ...
        require(_msgSender() == order.owner, "Order ownership");
    ...
    function sellToOrderId(uint256 orderId) external nonReentrant {
        Order storage order = buyOrders[orderId];
        require(order.owner != _msgSender(), "You can not sell to yourself");
        require(
            order.status != OrderStatus.Cancelled ||
                order.status != OrderStatus.Fulfilled,
            "Order should be listed"
