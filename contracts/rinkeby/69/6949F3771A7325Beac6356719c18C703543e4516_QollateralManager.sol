...
contract QollateralManager is Initializable {
  ...
  function _addAccountMarket(address account) external onlyMarket() {
  ...
  function depositCollateral(address tokenAddress, uint amount) external returns(uint) {
    ...
    QTypes.Asset memory asset = _qAdmin.assets(tokenAddress);
...
contract QAdmin is Initializable, AccessControlEnumerable {
  ...
  function _addAsset(
    ...
    require(!_assets[tokenAddress].isEnabled, "QAdmin: asset already exists");
  ...
  /// @param fixedRateMarketAddress Address of the new market
  function _addFixedRateMarket(address fixedRateMarketAddress) external onlyAdmin {
    ...
    require(tokenAddress != address(0), "QAdmin: invalid token address");

    // Only allow `Markets` where the corresponding `Asset` is enabled
    require(_assets[tokenAddress].isEnabled, "QAdmin: unsupported asset");
    ...
    _setupRole(MARKET_ROLE, fixedRateMarketAddress);
  ...
  function _setCollateralFactor(
    ...
    require(_assets[tokenAddress].isEnabled, "QAdmin: asset not enabled");
