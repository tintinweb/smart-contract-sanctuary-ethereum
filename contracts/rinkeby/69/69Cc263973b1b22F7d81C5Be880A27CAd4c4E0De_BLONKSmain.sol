...
/// @author Matto
/// @notice This contract is a customized ERC-721 & ERC-721 Enumerable contract for BLONKS NFTs.
...
contract BLONKSmain is ERC721, ERC721Enumerable, Ownable, ReentrancyGuard {
  ...
  string public description = "BLONKS are 100% on-chain (blonkchain -> image & metadata), generative, vector graphic illustrations. Traits are based on values generated at mint as well as on the CURRENT owner's public address (some visual aspects of these NFTs change when they're transferred/sold). BLONKS (the NFTs) are CC0 and live on Ethereum. Coded by Matto.";
  string public collection = "BLONKS";
  string public website = "https://BLONKS.xyz";
  string public externalURL;
  address public artistAddy = 0xF8d9056db2C2189155bc25A30269dc5dDeD15d46;
  ...
  // The following functions are custom BLONKS functions.
  ...
  modifier callerIsUser() {
    ...
    require(_tokenIdCounter.current() < maxSupply, "Minting closed.");
    uint256 tokenId = _tokenIdCounter.current();
    _tokenIdCounter.increment();
    createTokenEntropy(tokenId);
		...
		returns (string memory)
  {
	...
	{
    require(_tokenId < _tokenIdCounter.current(), "That token doesn't exist");
    string memory svg = iBLONKSuri(URIcontract).buildPreviewSVG(tokenEntropyMap[_tokenId], _addy);
    return svg;
	}
    ...
    onlyOwner
  {
	...
	function updateArtistAddy(address payable _artistAddy)
		external 
		onlyOwner 
	{
	...
	function updateDonationAddy(address payable _donationAddy)
		external 
		onlyOwner 
	{
