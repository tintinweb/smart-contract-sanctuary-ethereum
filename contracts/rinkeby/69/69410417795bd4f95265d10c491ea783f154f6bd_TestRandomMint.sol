...
contract TestRandomMint is ERC721OwnerEnumerable,ReentrancyGuard {
    ...
    uint256 public mintPhaseSupply = 60;
    ...
    //mapping keep track of wallet mint
    mapping(address => uint256) public addressMintedBalance;
    //keep track of available token ids
  ...
  /**
    ...
    modifier mintLimitCompliance(uint256 _mintAmount) {
      require(_mintAmount > 0, 'Mint amount should be greater than 0');
        ...
        require( minted + _mintAmount <= mintPhaseSupply , "Mint Max supply Exceeded"); 
  ...
  /**
          ...
          //not free
           require(msg.value >= whitelistCost * _mintAmount, "Insuffient funds");
            ...
            if(_mintAmount > a){
               ...
               //pay only for A 
                require(msg.value >= whitelistCost * c, "Insuffient funds");
             }
    ...
    */
      ...
      mintLimitCompliance(_mintAmount)
    {
        ...
        //else then pay for token mint price 
        if(addressMintedBalance[msg.sender] > freeMintLimit){
          //not free
    ...
    function Cliamble(address claimer) public view returns (uint256[] memory tokens){
       return tokens = tokensOfOwner(claimer);
    }
  ...
  /**
    ...
    */
    ...
    function _mintRandomIndex(address claimer, uint256 amount) internal {
        uint256 remainingSupply = mintPhaseSupply - totalSupply();

        for (uint256 i = 0; i < amount; i++) {
            // generate a random index from the remaining supply
            uint256 index = _random(remainingSupply);
            uint256 tokenAtPlace = _availableTokens[index];
            ...
            uint256 lastTokenAvailable = _availableTokens[remainingSupply - 1];
            ...
            if (lastTokenAvailable == 0) {
                ...
                _availableTokens[index] = remainingSupply - 1;
            } else {
                // ... we store the token that was stored for the last token
                _availableTokens[index] = lastTokenAvailable;
            }
  ...
  /**
    * Update colletion MintPhaseSupply.
    * @param _supply The new token price
    */
    function setMintPhaseSupply(uint256 _supply) public onlyOwner {
        mintPhaseSupply = _supply;
    }
  ...
  /**
    ...
    */
    ...
    function _mintLoop(address _receiver, uint256 _mintAmount) internal {
            _mintRandomIndex(_receiver, _mintAmount);
    }
    ...
    function withdraw() public onlyOwner nonReentrant {
        ...
        //5% of initial sale funds ill go the A wallet
