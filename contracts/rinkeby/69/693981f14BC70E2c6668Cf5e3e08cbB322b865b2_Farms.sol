...
contract Farms is ReentrancyGuardUpgradeable, OwnableUpgradeable {
  ...
  struct PoolInfo {
    IERC20 lpToken;
    uint256 allocPoint; // How many allocation points assigned to this pool. reward token's to distribute per block.
    uint256 lastRewardBlock; // Last block number that reward tokens distribution occurs.
    uint256 accRewardTokenPerShare; // Accumulated reward tokens per share, times 1e12. See below.
  }

  /// @notice Token Distributor to control farm reward
  ...
  event UpdatePool(uint256 indexed pid, uint256 lastRewardBlock, uint256 lpSupply, uint256 accRewardTokenPerShare);
  event Deposit(address indexed user, uint256 indexed pid, uint256 amount, address indexed to);
  event Withdraw(address indexed user, uint256 indexed pid, uint256 amount, address indexed to);
  event Harvest(address indexed user, uint256 indexed pid, uint256 amount);
  event SetRewardPerBlock(address indexed sender, uint256 rewardPerBlock);

  function initialize(TokenDistributor _tokenDistributor, uint256 _rewardPerBlock) public initializer {
    __Ownable_init();
    __ReentrancyGuard_init();

    tokenDistributor = _tokenDistributor;
    rewardToken = tokenDistributor.token();
  ...
  /// @notice Update the given pool's reward token allocation point contract. Can only be called by the owner.
  ...
  /// @notice Deposit LP tokens for reward token allocation.
  ...
  function deposit(
    ...
    user.rewardDebt = user.rewardDebt.add(actualAmount.mul(pool.accRewardTokenPerShare) / ACC_REWARD_TOKEN_PRECISION);
    if (user.fundedBy == address(0)) user.fundedBy = msg.sender;

    // Interactions
    if (isRewardTokenPool(pool.lpToken)) rewardTokenSupply = rewardTokenSupply.add(actualAmount);
  ...
  function withdraw(
    ...
    user.rewardDebt = user.rewardDebt.sub(amount.mul(pool.accRewardTokenPerShare) / ACC_REWARD_TOKEN_PRECISION);
    user.amount = user.amount.sub(amount);

    // Interactions
    stakeTokens[pid].safeTransfer(user.fundedBy, amount);
    if (user.amount == 0) user.fundedBy = address(0);
    if (isRewardTokenPool(pool.lpToken)) rewardTokenSupply = rewardTokenSupply.sub(amount);

    emit Withdraw(msg.sender, pid, amount, _for);
  }

  //@notice Harvest rewards earn from all pool contains thier positon.
  ...
  function pendingReward(uint256 _pid, address _user) external view returns (uint256) {
    ...
    if (isRewardTokenPool(pool.lpToken)) {
      stakeTokenSupply = rewardTokenSupply;
    } else {
    ...
    if (block.number > pool.lastRewardBlock && stakeTokenSupply != 0) {
      ...
      uint256 tokenReward = (blocks.mul(rewardPerBlock).mul(pool.allocPoint).mul(ACC_REWARD_TOKEN_PRECISION)).div(
        totalAllocPoint
      );
      accRewardTokenPerShare = accRewardTokenPerShare.add(tokenReward.div(stakeTokenSupply));
    }
    uint256 _pendingReward = (user.amount.mul(accRewardTokenPerShare) / ACC_REWARD_TOKEN_PRECISION).sub(
      user.rewardDebt
    );
  ...
  function updatePool(uint256 pid) public returns (PoolInfo memory pool) {
    ...
    if (block.number > pool.lastRewardBlock) {
      ...
      if (stakeTokenSupply > 0 && totalAllocPoint > 0) {
        uint256 blocks = block.number.sub(pool.lastRewardBlock);
        uint256 tokenReward = (blocks.mul(rewardPerBlock).mul(pool.allocPoint)).div(totalAllocPoint);

        uint256 rewardReleased = tokenDistributor.release(tokenReward);

        pool.accRewardTokenPerShare = pool.accRewardTokenPerShare.add(
          (rewardReleased.mul(ACC_REWARD_TOKEN_PRECISION)).div(stakeTokenSupply)
        );
      }
      pool.lastRewardBlock = block.number;
      poolInfo[pid] = pool;
      emit UpdatePool(pid, pool.lastRewardBlock, stakeTokenSupply, pool.accRewardTokenPerShare);
    }
  }
  ...
  function _harvest(address to, uint256 pid) internal {
    ...
    uint256 accumulatedRewardToken = user.amount.mul(pool.accRewardTokenPerShare).div(ACC_REWARD_TOKEN_PRECISION);
    uint256 _pendingReward = accumulatedRewardToken.sub(user.rewardDebt);
    if (_pendingReward == 0) {
      return;
    }
  ...
  /// @notice Returns if stakeToken is reward token
  function isRewardTokenPool(IERC20 _stakeToken) internal view returns (bool) {
    return _stakeToken == rewardToken;
  }
...
contract TokenDistributor is Ownable, ReentrancyGuard {
  ...
  constructor(IERC20 _token, uint256 _cap) {
    token = _token;
    cap = _cap;
  }
  ...
  function release(uint256 amount) external nonReentrant onlyOwner returns (uint256) {
    ...
    if (releasedAmount > tokenBalance) {
      releasedAmount = tokenBalance;
    }
