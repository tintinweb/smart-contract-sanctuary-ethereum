...
contract HappyRobotNFT is ERC721A, Ownable {
  ...
  uint16 public constant preSaleMaxQuantity = 3333;
  uint16 public constant pubSaleMaxQuantity = 3334;
  
  // constant value for every generation
  uint8 public constant PRESALE = 0;
  uint8 public constant PUBSALE = 1;
  ...
  uint16 private preSaleQuantity = 0;
  uint16 private pubSaleQuantity = 0;
  ...
  /**
  * get presale quantity
  * @return presale quantity
  */
  function getPreSaleQuantity() public view returns (uint16) {
    return preSaleQuantity;
  }

  /**
  * get pubsale quantity
  * @return pubsale quantity
  */
  function getPubSaleQuantity() public view returns (uint16) {
    return pubSaleQuantity;
  }
  ...
  */
   ...
   * @param _saleType sale type option
   * @param _quantity quantity
   * @return true or false
   */
  function canCreateToken(uint8 _saleType, uint16 _quantity) public view returns (bool) {

    unchecked {
      if (_saleType == PRESALE) {
        if (preSaleQuantity + _quantity >= preSaleMaxQuantity)
          return false;

      } else if (_saleType == PUBSALE) {
        if (pubSaleQuantity + _quantity >= pubSaleMaxQuantity)
          return false;
