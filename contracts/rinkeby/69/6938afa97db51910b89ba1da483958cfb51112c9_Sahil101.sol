...
contract Sahil101 is ERC721AQueryable, Ownable {
    // ============ State Variables ============
        ...
        callerIsUser
    {
        ...
        require(
            _nextTokenId() + quantity - 1 <= maxTokenIds,
            "Not enough supply"
        );
        require(
            balanceOf(msg.sender) + quantity <= maxPerWallet,
            "Exceeding max per wallet limit"
        );
        ...
        onlyOwner
    {
        for (uint256 i; i < _to.length; ) {
            require(
                _nextTokenId() + _amount[i] - 1 <= maxTokenIds,
                "Not enough supply"
            );
    ...
    function mintForAddress(address _to, uint256 _quantity)
        external
        payable
        onlyOwner
    {
        require(
            _nextTokenId() + _quantity - 1 <= maxTokenIds,
            "Not enough supply"
        );
        _mint(_to, _quantity);
    }
    ...
    function setSaleTimings(uint256 _whitelistStart, uint256 _whitelistEnd)
        external
        onlyOwner
    {
