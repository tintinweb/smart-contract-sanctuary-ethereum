...
contract NFTBleepyz is Ownable, ERC721A, ReentrancyGuard {
    ...
    ) ERC721A("Bleepyz", "BLEEPYZ") {
        transferOwnership(msg.sender);

        currentStep = Steps.NotStarted;
        ...
        nonReentrant
    {
        ...
        require(
            currentStep == Steps.WhitelistSale,
            "Whitelist sale is not activated"
        );
        require(isWhiteListed(msg.sender, _proof), "Not whitelisted");
        require(nftsPerWallet[msg.sender] < 1, "You can only get 1 NFT");
    ...
    function publicSaleMint() external payable callerIsUser nonReentrant {
        require(currentTime() >= saleStartTime, "Sale has not started yet");
        require(
            currentStep == Steps.PublicSale,
            "Public sale is not activated"
        );
        require(totalSupply() + 1 <= MAX_SUPPLY, "Max supply exceeded");
        require(
            nftsPerWallet[msg.sender] == 0,
    ...
    function getStep() public view returns (Steps) {
        return currentStep;
    }

    function setStep(uint256 _step) external onlyOwner {
        currentStep = Steps(_step);
    }
