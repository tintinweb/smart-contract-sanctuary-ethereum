...
contract BB2 is Context, IERC20, Ownable {
    
    using SafeMath for uint256;

    string private constant _name = "BB2";
    string private constant _symbol = "BB2";
    ...
    uint256 private _taxFeeJeets = 0;
    ...
    address payable private _marketingAddress = payable(0x23cED507Dd5450Ea448934A2f27f76766d98791c);
    ...
    uint256 public _maxTxAmount = 2e4 * 10**9; //2%
    uint256 public _maxWalletSize = 4e4 * 10**9; //4%
    uint256 public _swapTokensAtAmount = 5e2 * 10**9;
    uint256 public _minimumBuyAmount = 2e4 * 10**9 ; // 2%
    ...
    function setMaxTxnAmount(uint256 maxTxAmount) external onlyOwner {
        require(maxTxAmount >= 1e2 * 10**9, "Maximum transaction amount must be greater than 0.5%");
    ...
    // USUAL TAXES CANNOT BE RAISED MORE THAN 3%
    function setTaxFee(uint256 amountBuy, uint256 amountSell) external onlyOwner {
        require(amountBuy >= 0 && amountBuy <= 3);
        require(amountSell >= 0 && amountSell <= 3);
    ...
    // FAST SELL TAXES CANNOT BE RAISED MORE THAN 3%
    function setJeetsFee(uint256 amountRedisJeets, uint256 amountTaxJeets) external onlyOwner {
        require(amountRedisJeets >= 0 && amountRedisJeets <= 3);
        require(amountTaxJeets >= 0 && amountTaxJeets <= 3);
