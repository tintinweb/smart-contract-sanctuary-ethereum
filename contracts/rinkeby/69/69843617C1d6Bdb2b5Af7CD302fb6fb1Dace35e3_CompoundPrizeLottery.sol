  ...
  VRFConsumerBaseV2
{
  ...
  /* Lottery parameters */
  uint256 public constant DRAWING_PERIOD = 2 minutes; // @TODO Change to 10 days
  ...
  uint32 internal constant CALLBACK_GAS_LIMIT = 150000;
  ...
  event RandomNumberReceived(uint256 indexed lotteryId, uint64 subscriptionId);
  ...
  function fulfillRandomWords(
    ...
    //changeState(State.RANDOMNESS_FULFILLED);

    emit RandomNumberReceived(lotteryId.current(), subscriptionId);
  }

  /**
   * @notice This is the function that checks if the lottery can draw the winner
   * and can initialize a new lottery run
   * the following should be true for this to return true:
   * 1. The state should be RANDOMNESS_FULFILLED
   * @return True if the lottery is ready to draw, otherwise False
   */
  function checkDrawing() public view returns (bool) {
    return State.RANDOMNESS_FULFILLED == state;
  }

  /**
   * @notice This is the function that draw the winner
   * and initialize a new lottery run.
   */
  function performDrawing() external {
    require(checkDrawing(), "PrizeLottery: DRAWING_NOT_NEEDED");
