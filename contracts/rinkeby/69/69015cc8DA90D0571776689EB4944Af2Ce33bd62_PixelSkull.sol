...
contract PixelSkull is AdminControl, ReentrancyGuard {
    ...
    constructor(
        ...
        collectionData.price = _price;
        collectionData.editions = 1;
        ...
        returns (bool isInArray)
    {
        ...
        for (uint256 j = 0; j < tokensData[hashString].owners.length; j++) {
            if (tokensData[hashString].owners[j] == to) {
                return true;
            }
        }
    ...
    function getImageMetaByHash(string memory hashString)
        ...
        view
        collectableIsActive
        returns (NFTDataAttributes memory data)
    {
        bytes memory tempEmptyStringTest = bytes(hashString);
        require(tempEmptyStringTest.length > 5);
        require(tokensData[hashString].artist > address(0),"Invalid Hash");
        return tokensData[hashString];
    }

    function mint(address account, string memory imageId)
        public
        payable
        collectableIsActive
        nonReentrant
        returns (uint256 tokenId)
    {
        ...
        NFTDataAttributes memory tokenData = getImageMetaByHash(imageId);
        ...
        IERC721CreatorCore(_creator).setTokenURIExtension(
            newItemId,
            string(abi.encodePacked(contractData.APIEndpoint, imageId))
        );
        tokensData[imageId].tokens.push(newItemId);
        tokensData[imageId].owners.push(msg.sender);
        tokensData[imageId].sold = tokensData[imageId].sold + 1;
        _tokens[newItemId] = imageId;

        uint256 valueTransfer = (msg.value / 100) * 70;
        require(tokensData[imageId].artist != address(0),"");
        (bool success, ) = tokensData[imageId].artist.call{
            value: valueTransfer
        }("");
    ...
    function setNftData(string[] memory _tokensData, address[] memory _artists)
        public
        adminRequired
    {
    ...
    function withdraw(address payable reciever)
        public
        adminRequired
        nonReentrant
    {
