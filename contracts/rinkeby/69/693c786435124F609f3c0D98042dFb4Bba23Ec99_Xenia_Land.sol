...
contract Xenia_Land is ERC721PUnordered, Ownable {
  ...
  constructor() ERC721PUnordered("Xenia Land", "XLAND") {}

  function saleMint(uint256[] memory _tokenIds, bool _startStaking) external payable {
    require(publicSaleIsActive, "Not active");
    uint amount = _tokenIds.length;
    require (amount <= remaining(), "Max land supply reached");
    ...
    mint(_tokenIds, msg.sender, _startStaking);
    
  }

  function ownerMint(address _recepient, uint256[] memory _tokenIds, bool _startStaking) external onlyOwner {
    uint amount = _tokenIds.length;
    require(totalSupply() + amount <= maxSupply, "Exceeds max supply");
    require(amount <= reserved, "Amount exceeds reserve");
    unchecked {
        reserved -= amount;
    }
    
    mint(_tokenIds, _recepient, _startStaking);
  }

  function mint(uint256[] memory _tokenIds, address _recepient, bool _startStaking) internal {
    for (uint i; i<_tokenIds.length; ){
      require(_tokenIds[i] > 0 && _tokenIds[i] < 3137, "Invalid token Id");
      _safeMint(_recepient, _tokenIds[i]);
      if (_startStaking){
        _stake(uint16(_tokenIds[i]));
      }
      unchecked { ++i; }
    }
