...
contract BrokerDefiPartner is ERC721, Ownable, ReentrancyGuard {
    ...
    uint public PARTNER_COMMISSION;

    // Token URI
    string public baseTokenURI = "ipfs://QmQGkLL2dRK2gzWq3jdwNS4DxMCpeQLfVQAkjQiYDD15fk/";
    ...
    function setPartnerComission(uint commission) public onlyOwner() {
    ...
    */
    ...
    function showEscrowPeriod(uint tokenId) public view returns(uint) {
        return escrowedAt[tokenId] + escrowPeriod[tokenId];
    }
    ...
    function whitelistMint(bytes32[] calldata _merkleProof, uint amount) public payable nonReentrant {
        require(whiteListActive && whitelistPrice > 0, "Whitelist not active.");
        require((TOKEN_ID + amount) <= MAX_SUPPLY && (whitelistClaimed[msg.sender] + amount) < 10, "Mint exceeds limit");
        require(msg.value >= whitelistPrice, "No balance");
    ...
    function publicMint(uint amount, uint code) public payable nonReentrant {
        ...
        if (code > 0) {
            ...
            treasury.transfer(msg.value - commission);
        } else {
            treasury.transfer(msg.value);
        }
        uint[] memory tokenIds = new uint[](amount);
    ...
    /**
     * @dev use safeTransferWhileEscrow function to transfer an nft safely while escrowing
     */
