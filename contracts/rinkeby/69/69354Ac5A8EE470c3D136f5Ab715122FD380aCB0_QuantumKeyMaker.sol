...
contract QuantumKeyMaker is Auth, ReentrancyGuard {
    ...
    function order(
        ...
        require(!_hasClaimed[id][root].get(index), "ALREADY_CLAIMED");
        ...
        if (limited) _hasClaimed[id][root].set(index);
        available[id] -= amount;
    ...
    function hasClaimed(uint256 id, bytes32 root, uint256 index) external view returns (bool) {
        return _hasClaimed[id][root].get(index);
    }

    /// @notice converts a bool to uint256
    /// @param x bool to be converted
    /// @return r uint256 representation of the bool
    function b2u(bool x) pure public returns (uint r) {
        assembly { r := x }
    }
}
