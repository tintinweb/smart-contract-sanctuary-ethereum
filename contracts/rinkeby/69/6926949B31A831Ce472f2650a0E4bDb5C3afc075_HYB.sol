...
contract HYB is ContractFactory, HYBCards {
    ...
    constructor() ERC1155("ipfs://") {     
        ...
        symbol_ = "HYB";   
        mycontractURI = "https://api-test.goodmorning-games.com/contract/";
    }
    ...
    function mint(bytes memory signature, uint256[] memory ids, uint256[] memory amounts) external payable {
        ...
        require(signerAddress == messagehash.recover(signature), "01");
        require(!isSalePaused, "02");
        
        for (uint256 i = 0; i < ids.length; i++) {
            require(cards[i].isCardForSale);
            require(totalSupply(i) + amounts[i]  <= cards[i].maxSupply, "04");
            require(msg.value == amounts[i] * cards[i].mintPrice); 
        }
...
abstract contract ContractFactory is ERC1155Supply, ERC1155Burnable, Ownable {
