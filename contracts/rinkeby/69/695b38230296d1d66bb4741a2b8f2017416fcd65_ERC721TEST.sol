...
contract ERC721TEST is ERC721A, Ownable {
    ...
    uint private constant MAX_TEAM = 166;
    uint private constant MAX_FREE = 1500;
    uint private constant MAX_PUBLIC = 5000;

    bool private isRevealed = false;

    uint public zero_price = 0 ether;
    uint public public_price = 0.0084 ether;

    bytes32 public merkleRoot;

    mapping(address => uint) public amountNFTperWalletFreeMint;

    constructor(string memory _baseURI) ERC721A("ERC721 TEST", "ERC721T")
    {
        baseURI = _baseURI;
    }
    ...
    function teamMint(address _to, uint _quantity) external payable onlyOwner {
        require(totalSupply() + _quantity <= MAX_TEAM, "NFT can't be mint anymore");
        _safeMint(_to,  _quantity);
    }

    function freeMint(address _account, uint _quantity, bytes32[] calldata _proof) external payable isNotContract {
        uint price = zero_price;
        require(sellingStep == Step.FreeMint, "Free Mint is not activated");
        require(isWhiteListed(msg.sender, _proof), "Not whitelisted");
        require(amountNFTperWalletFreeMint[msg.sender] + _quantity <= 1, "You can only get 1 NFT on the Free Mint");
    ...
    function publicSaleMint(address _account, uint _quantity) external payable isNotContract {
        ...
        require(totalSupply() + _quantity <= MAX_TEAM + MAX_FREE + MAX_PUBLIC, "NFT can't be mint anymore");
        require(msg.value >= price * _quantity, "Not enought funds");
        _safeMint(_account, _quantity);
    }
