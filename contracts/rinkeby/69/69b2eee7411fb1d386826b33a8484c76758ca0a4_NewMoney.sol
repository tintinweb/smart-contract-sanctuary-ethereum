...
contract NewMoney is ERC721, Pausable, Ownable {
    ...
    constructor() ERC721("New Money", "NM$") {
        _signer = owner();
    }
    ...
    function buyOffPrivateExchange(uint256 tokenId, string memory tokenUri, Signature memory signature)
        external
        payable
        whenNotPaused
    {
        require(
            _isVerifiedSignature(_createBuyMessageDigest(msg.sender, tokenId, tokenUri, msg.value), signature), 
            "Signature is not valid."
        );
    ...
    {
        // it the token is bought off private exchange, return the URI
    ...
    function _createBuyMessageDigest(address _owner, uint _tokenId, string memory _tokenUri, uint _price)
        internal
        pure
        returns (bytes32)
    {
        return keccak256(
            abi.encodePacked(
                "\x19Ethereum Signed Message:\n32",
                keccak256(abi.encodePacked(_owner, _tokenId, _tokenUri, _price))
            )
        );
    }
}
