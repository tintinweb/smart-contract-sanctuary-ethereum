...
contract Bridge is Ownable, ReentrancyGuard {
    ...
    // withdraw txHash status in current chain
    mapping(bytes32 => bool) public txHashStatus;
    ...
    function updateBridgeFee(uint256 _newDepositFee, uint256 _newWithdrawFee)
        external
    {
        require(
    ...
    function withdraw(
        ...
        require(_amount > 0, "Bridge:: Withdraw :: Invalid _amount");
        require(chainId == _chainId, "Bridge:: Withdraw :: Invalid _chainId");
        require(
            !claimedWithdrawalsByOtherChainDepositId[_nonce],
            "Bridge:: Withdraw :: Already Withdrawn!"
        );
        require(
            verifySignature(_amount, _chainId, _nonce, _txHash, _v, _r, _s),
        ...
        vault.bridgeWithdrawal(token, msg.sender, _amount);
        emit Withdrawal(msg.sender, _amount, _nonce, chainId, token);
    }
...
library StringLibrary {
    ...
    function concat(
        ...
        bytes memory resultBytes = new bytes(
            _ba.length + _bb.length + _bc.length
        );
        uint256 k = 0;
