...
contract GAT_Opensea is Ownable{
    ...
    address public gatAddress;
    uint256 public royaltyPercentage=50;
    uint256 public marketPercentage=25;
    uint256 public gatPercentage=25;
    ...
    function setMarketAddress(address market_address) public onlyOwner {
    ...
    function setMarketPercentage(uint256 market_percentage) public onlyOwner {
        marketPercentage = market_percentage;
    }
    ...
    function distriibuteEthPercentages(uint256 valueAmount) internal {
        ...
        marketBalance += SafeMath.mul(SafeMath.div(valueAmount, 100), marketPercentage);
    ...
    function __withdrawToAllWalllets() public onlyOwner{
        ...
        Address.sendValue(payable(marketAddress), marketBalance);
        Address.sendValue(payable(gatAddress), gatBalance);
        emit TransferSent(msg.sender, royaltyAddress, royaltyBalance);
        emit TransferSent(msg.sender, marketAddress, marketBalance);
    ...
    function withdrawToRoyalty() public onlyOwner{
        Address.sendValue(payable(royaltyAddress), royaltyBalance);        
        //royaltyAddress.transfer(royaltyBalance);
        emit TransferSent(msg.sender, royaltyAddress, royaltyBalance);
        royaltyBalance = 0;        
    }
    ...
    function withdrawToMarket() public onlyOwner{
        Address.sendValue(payable(marketAddress), marketBalance);
        //royaltyAddress.transfer(royaltyBalance);
        emit TransferSent(msg.sender, marketAddress, marketBalance);
        marketBalance = 0;        
