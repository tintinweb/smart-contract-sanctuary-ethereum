...
contract TierSystem is Ownable {
    ...
    uint256 public tier0Time = 0;
    uint256 public tier1Time = 3600;
    uint256 public tier2Time = 7200;
    uint256 public tier3Time = 10800;
    ...
    // Phases E = after 240m: non-whitelisted (NO STAK)
       ...
       if (stakBank.balanceOf(_address) >= tier0 )  {

           tier = 0;
            timestamp = campaignOpenTime;
       }  else if (stakBank.balanceOf(_address) >= tier1) {
            tier = 1;
            timestamp = campaignOpenTime.add(tier1Time);
       } else if (stakBank.balanceOf(_address) >= tier2) {
            tier = 2;
            timestamp = campaignOpenTime.add(tier2Time);
       }else if (stakBank.balanceOf(_address) >= tier3 || whitelist[_address]) {
            tier = 3;
    ...
    function _isTiered(address _address) internal view returns (bool tier) {
        if (whitelist[_address]) {
            tier = true;
        } else if (block.timestamp >= campaignOpenTime.add(tier0Time)) {
            tier = true;
