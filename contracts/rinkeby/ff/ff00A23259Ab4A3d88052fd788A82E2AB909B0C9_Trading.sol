...
contract Trading {
	...
	// referral system
	//mapping(address => bool) pendingReferrer; // address => if waiting on user to use referral
	//default valure should be 0 so if referredBy[user] == 0, then user is not referred
	//mapping(address => address) referredby; // trader => referrer
/* 	mapping(address => mapping(address => uint256)) referralBalanceCurrency; // referrer => claimable balance
	mapping(string => address) codeToReferrer; // code => address
 */
...
/* 	function createReferralString (string memory _referalCode) external {
		require(codeToReferrer[_referalCode] == address(0), "!referral-code-exists");
		require(hasCode[msg.sender] == false, "!user-has-code");
		hasCode[msg.sender] = true;
		codeToReferrer[_referalCode] = msg.sender;
	} */
	...
	function submitOrder(
		...
		/* if(keccak256(abi.encodePacked(referallCode)) != keccak256(abi.encodePacked(""))) {
			if(codeToReferrer[referallCode] != msg.sender) {
				//cannot refer yourself
				fee = fee * 9 / 10; //10% discount
				referralBalanceCurrency[codeToReferrer[referallCode]][currency] += fee / 10; //10% goes to referrer
			}
		} */
...
/* 	function referralClaim() public {
		...
		for(uint i = 0; i <= length; i++){
			...
			IPool(IRouter(router).getPool(currency)).referrerClaim(msg.sender, referralBalanceCurrency[msg.sender][currency]);
		}
	} */
