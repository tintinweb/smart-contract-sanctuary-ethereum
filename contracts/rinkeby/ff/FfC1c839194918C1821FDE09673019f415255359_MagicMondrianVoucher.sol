...
contract MagicMondrianVoucher is Ownable, Pausable, ERC721A, PaymentSplitter {
    ...
    // Public key of voucher signer address
    ...
    constructor(address[] memory _payeesAdresses, uint[] memory _payeesShares, address _signerAddress, string memory _baseURI) ERC721A("Magic Mondrian", "MAMO") PaymentSplitter(_payeesAdresses, _payeesShares) {
     ...
     */
    function whitelistMint(address _account, uint256 _quantity, bytes calldata _signature) external payable callerIsUser {
        ...
        require(isWhitelisted(_signature), "Not whitelisted.");
    ...
    function publicSaleMint(address _account, uint256 _quantity) external payable callerIsUser {
        ...
        require(price != 0, "Price is 0");
        require(currentTime() > saleStartTime + 300 minutes, "Whitelist Sale is not finished");
        require(_quantity < MAX_MINT + 1, "Max mint amount for Public Sale exceeded");
    ...
    /**
     * @dev Checks if the voucher is eligible for the Whitelist Sale together with the sender address.
     *
     * @return bool true or false
     */
    function isWhitelisted(bytes calldata _signature) public view returns (bool) {
