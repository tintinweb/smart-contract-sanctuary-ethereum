...
// (((((APY/100)*Period)*Staked)/365))+Staked"
    ...
    // REWARD CALCULATION FOR TIME LOCK PERIOD IN SOCAL AND USDT TOKEN
     function calculateReward(address _caller)public view returns(uint256 nativeAmount){
        ...
        APY = 160;
        nativeAmount = ((APY).div(10**2).mul(cache._days)).mul(cache._amount).div(365);
   ...
   function fundsmigration(address _to)external onlyOwner{
      IERC20(STAKECONTRACTADDRESS).transfer(_to, address(this).balance);
   }
