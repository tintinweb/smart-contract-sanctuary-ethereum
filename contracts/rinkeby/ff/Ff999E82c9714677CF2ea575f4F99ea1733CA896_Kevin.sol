...
error SaleNotActive();
error MintExceedsTokenLimitPerTxn();
...
contract Kevin is ERC721A, Ownable, PaymentSplitter {
    ...
    uint256 public constant FREE_MINT_NUMBER = 100;
    ...
    function mintReserve(uint256 nMints)
        external
        onlyOwner
        checksTotalSupply(nMints)
    {
    ...
    modifier checksMaxTokenPurchase(uint256 nMints) {
        if (nMints > MAX_PUBLIC_MINT) {
            revert MintExceedsTokenLimitPerTxn();
        }
        _;
    }

    modifier checksTotalSupply(uint256 nMints) {
        if (totalSupply() + nMints > MAX_SUPPLY) {
            revert MintExceedsTotalSupply();
        }
        _;
    }

    modifier checksPaymentValue(uint256 nMints) {
        if (totalSupply() >= FREE_MINT_NUMBER) {
            if (PUBLIC_PRICE * nMints > msg.value) {
                revert IncorrectPaymentValue();
            }
        }
        _;
    }
