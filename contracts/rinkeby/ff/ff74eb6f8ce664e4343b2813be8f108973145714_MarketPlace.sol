    ...
    EIP712Upgradeable
{
    ...
    GenericERC1155 tokenContract1155;
    ...
    function initialize(
        ...
        tokenContract721 = GenericERC721(_tokenAddress721);
        tokenContract1155 = GenericERC1155(_tokenAddress1155);
    }
...
struct MarketplaceVoucher {
        ...
        return
            _hashTypedDataV4(
                keccak256(
                    abi.encode(
                        keccak256(
                            "MarketplaceVoucher(address owner,uint256 minPrice,uint256 auctionType,uint256 quantity,uint256 endTime)"
                        ),
                        voucher.owner,
                        voucher.minPrice,
                        voucher.auctionType,
    ...
    function verifySignature(MarketplaceVoucher memory voucher)
        public
        view
        returns (address)
    {
    ...
    function finaliseFixedPriceAuction721(
				address _owner,
				/**
				0:_tokenId,1:_minPrice,2:_auctionType,3:_quantity,4:endTime
				 */
        ...
        require(
           otherParams[2] == 1,
            "Auction needs to be Fixed Price"
        );
        ...
        require(
            voucher.quantity == 1,
            "Cannot have more than one copy for 721"
        );
        address seller = verifySignature(voucher);
        uint256 feeAmt = (otherParams[1] * feePercent) /
            (10000 + feePercent);
        ...
        tokenContract721.safeTransferFrom(
            seller,
            _msgSender(),
            otherParams[0]
        );
        ...
        returns (bool)
    {
        ...
        address seller = verifySignature(voucher);
        uint256 feeAmt = (bidAmount * feePercent) / (10000 + feePercent);
    ...
    function finaliseFixedPriceAuction1155(
        ...
        tokenContract1155.safeTransferFrom(
            ...
            otherParams[0],
            quantity,
            bytes(tokenContract1155.uri(otherParams[0]))
        );
    ...
    ERC2981PerTokenRoyalties
{
    ...
    function initialize(
        ...
        address[] memory _whitelistAddresses
			...
			__LazyMintERC721_init(_name, _symbol, signDomain, signVersion, minter, _whitelistAddresses);
        ...
        returns (uint256)
    {
		...
		function redeem(address redeemer, uint256 minPrice, string calldata uri, uint256 _royalty,bytes calldata signature) public payable returns(uint256){
			NFTVoucher memory voucher = NFTVoucher({minPrice:minPrice,uri:uri,royalty:_royalty,signature:signature});
			_tokenIds.increment();
    ...
    ERC2981PerTokenRoyalties
{
    ...
    function initialize(
        ...
        address[] memory _whitelistAddresses
			...
			__LazyMintERC1155_init(_newBaseURI, signDomain, signVersion, minter, _whitelistAddresses);
		}
    ...
    function createToken(string memory _tokenURI, uint256 _initialSupply,uint256 royalty)
        public
        returns (uint256 tokenId)
    {
        require(bytes(_tokenURI).length != 0, "Token URI cannot be empty");
        _tokenIds.increment();
        _mint(msg.sender, _tokenIds.current(), _initialSupply, bytes(_tokenURI));
		...
		function redeem(address redeemer,  uint256 minPrice,uint256 quantity, string calldata uri, uint256 _royalty,bytes calldata signature) public payable returns(uint256){
			NFTVoucher memory voucher = NFTVoucher({minPrice:minPrice,quantity:quantity,uri:uri,royalty:_royalty,signature:signature});
			_tokenIds.increment();
			uint256 royalty= redeem(redeemer,voucher,_tokenIds.current());
			_setTokenRoyalty(_tokenIds.current(), msg.sender, royalty);
			return _tokenIds.current();
		...
		function distributeTokens(address[] memory transferAddr,uint256[] memory ids,uint256[] memory quantity) public returns(bool){
			require(transferAddr.length==ids.length,"Addresses and tokenIds length mismatch");
			require(transferAddr.length==quantity.length,"Addresses and quntity length mismatch");
			for(uint i =0; i<transferAddr.length;i++){
				require(transferAddr[i]!=address(0),"Cannot transfer to a zero address");
				require(balanceOf(msg.sender, ids[i])>=quantity[i],"Insufficient balance");
				string memory tokenuri = uri(ids[i]);
				safeTransferFrom(msg.sender, transferAddr[i], ids[i], quantity[i], bytes(tokenuri));
			}
			return true;
		}
}
