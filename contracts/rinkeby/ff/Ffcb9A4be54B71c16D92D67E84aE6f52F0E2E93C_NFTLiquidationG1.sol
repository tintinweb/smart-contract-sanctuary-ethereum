...
// File: contracts/NFTLiquidationInterface.sol
...
// File: contracts/NFTLiquidationStorage.sol
...
contract NFTLiquidationV1Storage is NFTLiquidationProxyStorage {
     ...
     */
    uint256[] public seizeIndexes;
}

// File: contracts/NFTLiquidationProxy.sol
...
// File: contracts/IComptroller.sol
...
// File: contracts/NFTLiquidation.sol
...
contract NFTLiquidationG1 is NFTLiquidationV1Storage, NFTLiquidationInterface {
    ...
    function liquidateWithSingleRepay(address payable borrower, address cTokenCollateral, address cTokenRepay, uint256 repayAmount) external payable nonReentrant {
        ...
        uint[] calldata _seizeTokens;
        liquidateWithSingleRepayFresh(borrower, cTokenCollateral, cTokenRepay, repayAmount, _seizeTokens);
    }

    /**
     * @notice Execute the proxy liquidation with single token repay and selected seize token value
     */
    function liquidateWithSingleRepayV2(address payable borrower, address cTokenCollateral, address cTokenRepay, uint256 repayAmount, uint256[] calldata _seizeIndexes) external payable nonReentrant {
        require(borrower != address(0), "invalid borrower address");
        ...
        liquidateWithSingleRepayFresh(borrower, cTokenCollateral, cTokenRepay, repayAmount, _seizeIndexes);
    }

    function liquidateWithSingleRepayFresh(address payable borrower, address cTokenCollateral, address cTokenRepay, uint256 repayAmount, uint256[] calldata _seizeIndexes) internal {
        require(extraRepayAmount == 0, "invalid initial extra repay amount");
        require(seizeIndexes.length == 0, "invalid initial seize indexes");
        ...
        for (uint i = 0; i < _seizeIndexes.length; i++) {
            seizeIndexes.push(_seizeIndexes[i]);
        }
        ...
        require(ICToken(cTokenRepay).borrowBalanceCurrent(borrower) == 0, "invalid token borrow balance");
