...
contract TestTicketSale is Ownable {
    ...
    uint256 public vipPricePermissioned = 0.00015 ether;
    uint256 public wlPricePermissioned = 0.0002 ether;

    uint256 public vipStartTime = 1647277213;
    uint256 public vipDuration = 5 minutes;
    uint256 public wlStartTime = 1647277213;
    uint256 public wlDuration = 5 minutes;

    uint256 public maxPubSupply = 10000;
    uint256 public boughtPub = 1;
    uint256 public limitPub = 20;
    uint256 public pubPrice = 0.0004 ether;
    uint256 public startTimePub = 1647277213;
    ...
    function getPubPrice() public view returns (uint256) {
        ...
        if (block.timestamp >= startTimePub) {
            ...
            _price = pubPrice - 0.0001 ether; //0.3 ETH
        } else if (block.timestamp >= startTimePub + 6 hours) {
            _price = pubPrice - 0.0001 ether; //0.2 ETH
        } 
        return _price;
    }
    ...
    function buyPermissioned(uint256 amount, uint256 buyMaxAmount, uint256 phase, bytes calldata signature) 
        external 
        payable {
