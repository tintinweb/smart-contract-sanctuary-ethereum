...
library SafeMath {
  ...
  //StakeWithAmd
   ...
   function withdrawInWolve(uint256 count) public {
    ...
    function getAllStakeDetail()public view returns (Stake[] memory ){
        Stake[] memory userStakingInfo = new Stake[](users[msg.sender].stakecount);
        for(uint counter=0; counter < users[msg.sender].stakecount; counter++) {
            Stake storage member = users[msg.sender].stakerecord[counter];
