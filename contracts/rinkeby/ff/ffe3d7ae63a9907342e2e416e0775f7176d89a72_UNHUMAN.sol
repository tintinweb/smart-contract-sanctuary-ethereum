...
abstract contract ERC721URIStorage is ERC721 {
    ...
    function test(uint id) public view virtual returns (string memory){
        ...
        for (step = 0; step < limit; step++){
            res =  50 + (20 - 50) * (step * 10 / limit);
            slot.polys = string(abi.encodePacked(slot.polys, uint(res).toString()));
            slot.polys = string(abi.encodePacked(slot.polys, " "));
        }
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        for (uint256 i = 0; i < _noObjects[tokenId]-1; i++) {
            for (step = 0; step < limit; step++){ 
                ...
                slot.newX =  _coords[tokenId][i * 2 + 2][0] * (step / limit);
                //slot.newX =  20 + (20 - 20);
                ...
                slot.polys = string(abi.encodePacked(slot.polys, slot.newX.toString()));
            }
        }
