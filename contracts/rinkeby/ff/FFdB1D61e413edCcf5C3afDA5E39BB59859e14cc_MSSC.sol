...
contract MSSC is SSCVault, AccessControl {
    ...
    mapping(bytes32 => bytes32) private _cycles;

    mapping(bytes32 => bytes32) private _instructions;
    ...
    function registerSettlementCycle(
        ...
        for (uint256 i = 0; i < totalInstructions; ) {
            bytes32 instructionId = instructions[i].id;

            _assertValidInstruction(instructions[i]);

            // newInstructions.push(instructionId);
            _instructions[instructionId] = keccak256(
                abi.encode(instructions[i])
            );
        ...
        _cycles[cycleId] = keccak256(abi.encode(instructions));
     ...
     *
     * @param instruction Instruction to allocate funds.
     */
    function deposit(Instruction calldata instruction) external payable {
        // Instruction memory instruction = _instructions[instructionId];
        require(
            keccak256(abi.encode(instruction)) == _instructions[instruction.id],
            ""
        );
        uint256 amount = instruction.amount;
        // Ensure that instruction does exist by checking its amount.
        if (amount == 0) {
            revert NoInstruction(instruction.id);
        }
     ...
     *
     * @param instruction Instruction to withdraw deposited funds from.
     */
    function withdraw(Instruction calldata instruction) external {
        DepositItem memory depositItem = _buildDepositItem(instruction);
        ...
        view
    {
        ...
        if (_instructions[instruction.id] != bytes32(0)) {
    ...
    function _exist(bytes32 cycleId) private view returns (bool) {
        return _cycles[cycleId] != bytes32(0);
    }
...
import {IERC20 as ERC20} from "@rari-capital/solmate/src/tokens/ERC20.sol";
