...
contract BabyDemon is ERC721Enumerable, Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    using Strings for uint256;

    string private _name = "BabyDemon";
    string private _symbol = "BDM";
    
    bool private _presaleOn = false;
    bool private _saleOn = false;

    uint256 public presaleCost = 0.044 ether;
    uint256 public cost = 0.066 ether;
    uint256 public maxSupply = 6666;
    uint256 private _reserved = 66;
    ...
    // Mapping from whitelisted address that having claimed
    mapping(address => bool) private _whitelistClaimed;

    uint256 private _minClaimAmount = 0.001 ether; 
    ...
    // Recently, 6666 baby demons broke out of hell and are now causing trouble on the Ethereum Blockchain.
    // They are looking for a new place to play and praise Satan. Offer them a new home and let them earn Ethereum for you. 
    constructor(string memory baseURI, address dev) ERC721(_name, _symbol) {
        setBaseURI(baseURI);
        _dev = dev;

        //Dev gets the first 3 baby demons
        _safeMint(_dev, 0);
        _safeMint(_dev, 1);
        _safeMint(_dev, 2);
    }

    modifier merkleProofed(bytes32[] calldata merkleProof) {        
    ...
    /**
     * Mint Baby Demon if sender is whitelisted
     */
    function presaleMint(uint256 amount, bytes32[] calldata merkleProof) public payable merkleProofed(merkleProof) {
        uint256 supply = totalSupply();

        require(presaleOn(),                                            "Presale is off.");
        require(amount <= 3,                                            "You can mint a maximum of 3 Baby Demons on Presale.");
        require(supply.add(amount) <= maxSupply.sub(_reserved),         "Exceeds Max Supply.");
        require(msg.value >= presaleCost.mul(amount),                   "Sent Ether is not correct.");

        require(!_whitelistClaimed[msg.sender],                         "Sender has allready claimed his tokens."); 
    ...
    function mint(uint256 amount) public payable {
        ...
        require(amount <= 20,                                           "You can mint a maximum of 20 Baby Demons.");
        require(supply.add(amount) <= maxSupply.sub(_reserved),         "Exceeds Max Supply.");
    ...
    /**
     * Return true if sender is whitelisted and has not claimed his spot
     */
    function hasPresaleAccess(bytes32[] calldata merkleProof) public merkleProofed(merkleProof) returns (bool){
        require(!_whitelistClaimed[msg.sender], "Sender has allready claimed his tokens."); 
        return true;
    }
   ...
   */
     ...
     */
    function claimRewards() public nonReentrant returns (uint256) { 
        return _claimRewards(msg.sender);
    }

    
    /**
     * Return true if Presale is on
     */
    function presaleOn() public view returns (bool){
        return _presaleOn;
    }
    ...
    /**
     * Return amount of reserved demons for giveaway
     */
    ...
    /**
     * Calculate the presale costs
     */
    function calculatePresaleCosts(uint256 amount) public view returns (uint256) {
        return presaleCost.mul(amount); 
    }

    /**
     * Calculate the sale costs
     */
    function calculateCosts(uint256 amount) public view returns (uint256) {
        return cost.mul(amount); 
    }

    /**
     * Helper Function to return all owners of the baby demons
     */
    ...
    /**
     * Helper Function to return all baby demons of given owner
     */
    ...
    function _getTotalRewards(address owner) internal returns (uint256) {
        require(_reflectedFeeTotal >= _minClaimAmount, "Not enough Rewards to claim"); 
    ...
    function _safeGetTotalRewards(address owner) internal returns (uint256) {
        if (_reflectedFeeTotal < _minClaimAmount) return 0; 
    ...
    function _getUnclaimedRewards(address owner) internal returns (uint256, uint256[] memory) {
        ...
        require(ownerRewardsTotal >= _minClaimAmount, "Owners total rewards are too small to claim."); 
    ...
    function _safeGetUnclaimedRewards(address owner) internal returns (uint256, uint256[] memory) {
        ...
        if (ownerRewardsTotal < _minClaimAmount) return (0, new uint256[](0)); 
    ...
    function _claimRewards(address owner) internal returns (uint256) {
        ...
        require(unclaimedRewardsTotal >= _minClaimAmount, "Claimable rewards are to small."); 
    ...
    function _safeClaimRewards(address owner) internal returns (uint256) {
        ...
        if (unclaimedRewardsTotal < _minClaimAmount) return unclaimedRewardsTotal;
    ...
    function setMinClaimAmount(uint256 minClaimAmount) public onlyOwner {
        delete _minClaimAmount;
        _minClaimAmount = minClaimAmount;
    }

    function giveAway(address to, uint256 amount) external onlyOwner() {
        require(amount <= _reserved, "Exceeds reserved Baby Demons supply" );
