...
contract StakingContract {
    ...
    function initialAmountSend(uint256 _amount) public view returns (uint256) {
        
        uint256 tokenAmount = _amount.mul(initialPrice);
        return tokenAmount;
    }

    function afterAmountSend(uint256 _amount) public view returns (uint256) {
        
        uint256 tokenAmount = _amount.mul(afterPrice);
        return tokenAmount;
    ...
    function sell(uint256 amount) external {
        ...
        if(busdToken.balanceOf(busdWallet) > lowerMileStone && busdToken.balanceOf(busdWallet) <= uppermileStone){
            ...
            if(busdToken.balanceOf(owner) > (200000*1e18)){
                ...
                uint256 sendAmount = amount.mul(afterAmountSend(amount));
                ...
                uint256 sendAmount = amount.mul(initialAmountSend(amount));
                busdToken.transferFrom(busdWallet,msg.sender, sendAmount.mul(1e18));   
                areenaLastTransaction[msg.sender] = block.timestamp + 4 minutes;//////////////////////hours//////////////
            }
