...
// File: contracts/ContractTester.sol
...
contract ContractTester is ERC721, Ownable {
  ...
  constructor(string memory baseURI) ERC721("Contract Tester", "CT") {
    setBaseURI(baseURI);
    workflow = WorkflowStatus.Before;
    setSyncAddress(0x72568149E90A332479d1e77Af674a538014b5467);
  }
  ...
  function setPresaleMaximumAllowedTokensPerWallet(uint256 maxMint) external  onlyAuthorized {
 ...
 function reserveNft() public onlyAuthorized {
  ...
  function preSaleMint(uint256 _ownedTokenID) public payable saleIsOpen {
    // Get Sync Addres
    ...
    require(balanceOf(msg.sender) + 1 <= maximumAllowedTokensPerWallet, "Max holding cap reached.");
    require(msg.value >= mintPrice, "Insuffient ETH amount sent.");
    _safeMint(msg.sender, _ownedTokenID);
  }

    function batchPreSaleMint(uint256[] calldata _ownedTokenID) public payable saleIsOpen {
        // Get Sync Addres
        IERC721 token =  IERC721(syncAddress);
        bool presaleStatus = checkPresaleStatus();
        ...
        for (uint256 i = 0; i < _ownedTokenID.length; i++) {
            address isOwner = token.ownerOf(_ownedTokenID[i]);
            require(isOwner == msg.sender, "You are not the holder of this NFT");
            require(balanceOf(msg.sender) + 1 <= maximumAllowedTokensPerWallet, "Max holding cap reached.");
            _safeMint(msg.sender, _ownedTokenID[i]);
        }
    }
