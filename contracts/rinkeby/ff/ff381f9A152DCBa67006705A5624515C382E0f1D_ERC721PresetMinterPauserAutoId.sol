...
import "./exchange/INFTMarketFees.sol";
    ...
    RoyaltiesV2Impl
{
    ...
    INFTMarketFees public marketFees;
    ...
    function setMarketFeeAddress(INFTMarketFees _marketFees) external {
        require(
            hasRole(DEFAULT_ADMIN_ROLE, _msgSender()),
            "ERC721PresetMinterPauserAutoId: must have admin role to set the market fee address"
        );
     ...
     */
    function mint(address to, string memory _tokenURI, LibPart.Part[] memory _royalties) public virtual {
        require(
        ...
        _pickRoyalties(payable(to), _royalties);
    }

    /** 
     * @dev Chooses the correct royalties for the NFT.
     */
    function _pickRoyalties(address payable to, LibPart.Part[] memory _royalties) private {
        // Prevents zero marketFee contract
        if(address(marketFees) == address(0)) {
            _saveRoyalties(_tokenIdTracker.current(), _royalties);
            return;
        }
        // If it finds the value then override the LibPart
        try marketFees.getFeeConfig()
            returns (uint256, uint256, uint256 secondaryCreatorFeeBasisPoints)
        {
            _royalties = new LibPart.Part[](2);
            _royalties[0].value = uint96(secondaryCreatorFeeBasisPoints);
            _royalties[0].account = to;
            _saveRoyalties(_tokenIdTracker.current(), _royalties);
            return;
        } 
        catch (bytes memory) {
...
/**
 * @dev Interface of the NFTMarketFees
 *
 * Implementers can call getFeeConfig to see the current nft fees
 *
 * For an implementation, see {NFTMarketFees}.
 */
interface INFTMarketFees {
