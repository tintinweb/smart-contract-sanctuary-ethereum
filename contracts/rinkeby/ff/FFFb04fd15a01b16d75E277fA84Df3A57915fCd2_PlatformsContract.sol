...
contract PlatformsContract is Initializable, UpgradeableHelper {
    //Event
    event Rebirth(address indexed from, address indexed nftAddress, uint256[] indexed tokenId);
    ...
    function isWhiteList(address _address) public view returns(bool){
        return whiteListToken[_address];
    }
    ...
    function addWhiteListAddress(address[] memory _whiteListArray) public onlyHelper {
        for(uint i = 0;i < _whiteListArray.length; i++) {
            whiteListToken[_whiteListArray[i]] = true;
        }
    }

    function removeWhiteListAddress(address[] calldata _whiteListArray) public onlyHelper {
        for(uint i = 0;i < _whiteListArray.length; i++) {
            whiteListToken[_whiteListArray[i]] = false;
        }
    }
