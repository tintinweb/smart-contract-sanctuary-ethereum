...
contract CyberBrokersMetadata is Ownable {
  ...
  string private _imageCacheUri = "ipfs://QmQD2gpovyQTC5Lr88pFYGVKScd9HGtYCTcoYDfNmN295A/";
  ...
  CyberBrokerLayer[] public layerMap;
  ...
  CyberBrokerTalent[] public talentMap;

  // Directory of Brokers
  struct CyberBrokerData {
    uint8 mind;
    uint8 body;
    uint8 soul;
    uint8 talent;
  ...
  CyberBrokerData[] public brokerData;
  ...
  {
    require(
      keys.length == attributeNames.length &&
      keys.length == attributeValues.length,
      "Number of keys must match names and values"
    );

    for (uint256 idx; idx < keys.length; idx++) {
      layerMap.push(CyberBrokerLayer(keys[idx], attributeNames[idx], attributeValues[idx]));
    }
  }
  ...
  {
    require(
      index < layerMap.length,
      "Index must exist within the layer map"
    );

    layerMap[index].key            = key;
    layerMap[index].attributeName  = attributeName;
    layerMap[index].attributeValue = attributeValue;
  }
  ...
  function findLayer(
    ...
    returns (uint256)
  {
    bytes32 keyHashed = keccak256(abi.encodePacked(key));
    for (uint256 idx; idx < layerMap.length; idx++) {
      if (keccak256(abi.encodePacked(layerMap[idx].key)) == keyHashed) {
        return idx;
      }
    }
  ...
  {
    require(
      talent.length == species.length &&
      talent.length == class.length &&
      talent.length == description.length
    , "Number of talents must match species, class, and description");

    for (uint256 idx; idx < talent.length; idx++) {
      talentMap.push(CyberBrokerTalent(talent[idx], species[idx], class[idx], description[idx]));
    }
  }
  ...
  {
    require(
      index < talentMap.length,
      "Index must exist within the talent map"
    );

    talentMap[index].talent      = talent;
    talentMap[index].species     = species;
    talentMap[index].class       = _class;
    talentMap[index].description = description;
  }
  ...
  function findTalent(
    ...
    returns (uint256)
  {
    bytes32 talentHashed = keccak256(abi.encodePacked(talent));
    for (uint256 idx; idx < talentMap.length; idx++) {
      if (keccak256(abi.encodePacked(talentMap[idx].talent)) == talentHashed) {
        return idx;
      }
    }
    revert("Talent not found.");
  }
  ...
  {
    require(
      talent.length == mind.length &&
      talent.length == body.length &&
      talent.length == soul.length &&
      talent.length == layers.length &&
      talent.length == names.length,
      "Number of talents must match mind, body, soul, layers, and names"
    );

    for (uint8 idx; idx < talent.length; idx++) {
      ...
      brokerData.push(CyberBrokerData(
        mind[idx],
        body[idx],
        soul[idx],
        talent[idx],
  ...
  {
    require(
      index < brokerData.length,
      "Index must exist within the broker data map"
    );

    require(talent <= brokerData.length, "Invalid talent index");
    require(mind <= 30, "Invalid mind");
    require(body <= 30, "Invalid body");
    require(soul <= 30, "Invalid soul");

    for (uint16 layerIdx; layerIdx < layers.length; layerIdx++) {
      require(uint256(layers[layerIdx]) <= layerMap.length, "Invalid layer index");
    }

    brokerData[index].talent = talent;
    brokerData[index].mind   = mind;
    brokerData[index].body   = body;
    brokerData[index].soul   = soul;
    brokerData[index].layers = layers;
    brokerData[index].name   = name;
  }
  ...
  function tokenURI(uint256 tokenId) public view returns (string memory) {
    ...
    // Get the broker
    CyberBrokerData storage broker = brokerData[tokenId];
    ...
    return string(
        abi.encodePacked(
            ...
            abi.encodePacked(
                bytes('","attributes":['),
                getTalentAttributes(broker.talent),
                getStatAttributes(broker.mind, broker.body, broker.soul),
                getLayerAttributes(broker.layers),
                bytes(']}')
            )
        )
    );
  ...
  function getDescription(uint256 talentIdx) public view returns (string memory) {
    require(talentIdx < talentMap.length, "Invalid talent index");
    return talentMap[talentIdx].description;
  }

  function getExternalUrl(uint256) public view returns (string memory) {
    return _externalUri;
  ...
  function getTalentAttributes(uint256 talentIdx) public view returns (string memory) {
    require(talentIdx < talentMap.length, "Invalid talent index");

    CyberBrokerTalent storage talent = talentMap[talentIdx];
  ...
  function getStatAttributes(uint8 mind, uint8 body, uint8 soul) public pure returns (string memory) {
    return string(
      abi.encodePacked(
        abi.encodePacked(
          bytes('{"trait_type": "Mind", "value": '),
          uint256(mind).toString(),
          bytes('},{"trait_type": "Body", "value": '),
          uint256(body).toString()
        ),
        abi.encodePacked(
          bytes('},{"trait_type": "Soul", "value": '),
          uint256(soul).toString(),
          bytes('}')
        )
      )
    );
  ...
  function getLayerAttributes(uint16[] memory layers) public view returns (string memory) {
  ...
  {
    return string("To render the token on-chain, call CyberBrokersMetadata.renderBroker(_tokenId, _startIndex[, _thresholdCounter]) and iterate through the pages starting at _startIndex = 0. To render off-chain and use an off-chain renderer, call CyberBrokersMetadata.getTokenData(_tokenId) to get the raw data. A JavaScript parser is available by calling CyberBrokersMetadata.getOffchainSvgParser().");
  }
    ...
    returns (bytes memory)
  {
    ...
    CyberBrokerData storage broker = brokerData[_tokenId];

    string[] memory layers = new string[](broker.layers.length);
    for (uint256 layerIdx; layerIdx < broker.layers.length; layerIdx++) {
      string memory key = layerMap[broker.layers[layerIdx]].key;
