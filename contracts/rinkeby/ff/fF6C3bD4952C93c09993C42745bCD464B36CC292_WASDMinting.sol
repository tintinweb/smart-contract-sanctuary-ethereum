...
contract WASDMinting is Ownable, Pausable {
    ...
    constructor(
        ...
        _phaseInfos.push(PhaseInfo("", startingTime_ - block.timestamp));
        for (uint256 i = 0; i < metadatas.length; i++)
            _phaseInfos.push(PhaseInfo(metadatas[i], durations[i]));
    }
    ...
    function grantMintingRole(
        ...
        for (uint256 i = 0; i < participants.length; i++) {
            require(
                roleIds[i] < _mintingRoles.length,
                "WASDMinting: invalid role ID"
            );
            require(
                _participantInfos[participants[i]].roleId == 0,
                "WASDMinting: participant already granted minting role"
            );
            _participantInfos[participants[i]].roleId = roleIds[i];
        }
    }
