...
contract PlaneCanvas is BoxCanvas {
    ...
    constructor(IPixelCanvasRegistry _pixelCanvasRegistry, string memory _name, uint32 _width, uint32 _height) BoxCanvas(_pixelCanvasRegistry, _name, _width, _height, 1) {
        //Nothing
    }
...
contract BoxCanvas is ICanvas, Ownable {
    ...
    constructor(IPixelCanvasRegistry _pixelCanvasRegistry, string memory _name, uint32 _width, uint32 _height, uint32 _depth) {
    ...
    function _requireValidPixel(IPixel pixelContract, uint256 tokenId) internal view {
        bool hasPixel = _privatePixelCanvasRegistry.deployRegistry().has(PixelCanvasApplicationType.PIXEL, address(pixelContract));
