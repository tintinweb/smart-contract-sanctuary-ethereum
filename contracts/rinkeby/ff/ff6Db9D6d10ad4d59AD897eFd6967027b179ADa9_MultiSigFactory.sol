...
contract MultiSigFactory is IMultiSigFactory{
    ...
    function createWallet(string memory _name, address[] memory owners) external payable override returns(address){
        address wallet = address((new MultiSig){value:msg.value}(owners,msg.sender));
...
interface IMultiSigFactory {
    ...
    * @return address address of new multi signature wallet
     */
    function createWallet(string memory _name, address[] memory owners) external payable returns(address);
