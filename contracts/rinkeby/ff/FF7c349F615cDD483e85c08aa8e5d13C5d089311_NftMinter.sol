...
contract NftMinter is ERC721Enumerable, Ownable {
    ...
    function airdropMint() external onlyOwner {
        ...
        for(uint256 i = 0; i < airdropAddresses.length; i++){
            ...
            for(uint256 j = 0; j < airdropList[airdropAddresses[i]]; j++){
                if((mintIndex.add(1) > 20 && mintIndex.add(1) < 30) || (mintIndex.add(1) > 40 && mintIndex.add(1) < 50)){
                    mintIndex = mintIndex.add(10);
                }
    ...
    function whitelistMint(bytes32[] calldata proof,uint _level, uint _amount) external{
    ...
    function reserveMint(uint _amount) external {
        ...
        for (uint256 i = 1; i <= _amount; i++) {
            if(reserveMintIndex.add(1) > 10 && reserveMintIndex.add(1) <=20){
                reserveMintIndex = 20;
            }else if(reserveMintIndex.add(1) > 30 && reserveMintIndex.add(1) <= 40){
                reserveMintIndex = 40;
            }
