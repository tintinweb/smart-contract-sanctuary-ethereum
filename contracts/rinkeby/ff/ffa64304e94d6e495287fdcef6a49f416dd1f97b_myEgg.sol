...
contract myEgg is ERC721A, Ownable {
    using Strings for uint256;

    // Constant variables
    // ------------------------------------------------------------------------
    uint256 public constant MAX_PRESALE_SUPPLY = 10; // Total amount of FuckingEgg available during presale
    uint256 public constant MAX_RESERVE_SUPPLY = 5; // Total amount of FuckingEgg available during presale
    uint256 public constant MAX_SUPPLY = MAX_PRESALE_SUPPLY + MAX_RESERVE_SUPPLY + 20; // Total amount of FuckingEgg

    uint256 public constant MAX_PER_WALLET_PRESALE = 2; // Max amount of FuckingEgg per wallet during whitelist period

    uint256 public constant MAX_PER_TX = 30; // Max amount of FuckingEgg per transaction
    uint256 public constant MAX_PER_WALLET_PUBLIC = 30; // Max amount of FuckingEgg per wallet during public sale

    uint256 public PRICE = 0.0005 ether; // Price of a FuckingEgg
    //uint256 public PRESALE_PRICE = 0.0003 ether; // Presale Price of a FuckingEgg

    // Team addresses -
    // ------------------------------------------------------------------------
    address private constant _a1 = 0xf7f1ED914EdF6eAC82736b5d384De6EE19D6f429; //owner
    address private constant _a2 = 0xf7f1ED914EdF6eAC82736b5d384De6EE19D6f429; //developer
    ...
    // ------------------------------------------------------------------------
    modifier onlyIsLive() {
        require(isLive, "Sale is not active");
        _;
    }
    ...
    // Constructor
    // ------------------------------------------------------------------------
    constructor() ERC721A("myEgg", "MYE", 20) {}
    ...
    // Public Sale mint
    function mint(uint256 quantity) external payable onlyIsLive {

        if(totalSupply() < MAX_PRESALE_SUPPLY){

            require(totalSupply() < MAX_PRESALE_SUPPLY, "FuckingEgg presale sold out");      
            require(totalSupply() + MAX_PER_WALLET_PRESALE <= MAX_PRESALE_SUPPLY, "Quantity exceeds maximum presale supply");

            require(_saleClaimed[msg.sender] < MAX_PER_WALLET_PRESALE, "Wallet has already claimed presale limit");
            require(_saleClaimed[msg.sender] + 2 <= MAX_PER_WALLET_PRESALE, "Quantity exceeds max quantity per wallet during presale"); // require that the buyer has not claimed their presale limit


            _saleClaimed[msg.sender] += MAX_PER_WALLET_PRESALE;
            _safeMint(msg.sender, MAX_PER_WALLET_PRESALE); 
        }else{
            require(totalSupply() < MAX_SUPPLY, "FuckingEgg sold out");
    ...
    function withdrawAll() external onlyOwner {
        uint _a1amount = address(this).balance * 50/100;
        uint _a2amount = address(this).balance * 50/100;
