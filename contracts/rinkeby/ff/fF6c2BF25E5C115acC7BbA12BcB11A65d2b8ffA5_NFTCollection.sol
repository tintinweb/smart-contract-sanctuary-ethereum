...
contract NFTCollection is ERC721, Pausable, AccessControl {
    ...
    bytes32 public constant SECURITY_ROLE_ADMIN = keccak256("SECURITY_ROLE_ADMIN");
    bytes32 public constant SECURITY_ROLE_MANAGER = keccak256("SECURITY_ROLE_MANAGER");
    ...
    constructor(
        ...
        _grantRole(SECURITY_ROLE_ADMIN, msg.sender);
        _grantRole(SECURITY_ROLE_MANAGER, msg.sender);
    ...
    onlyRole(SECURITY_ROLE_MANAGER) returns (ContractInfo memory info) {
        info.deploymentConfig = _deploymentConfig;
        info.runtimeConfig = _runtimeConfig;
    }

    function withdrawFees() external onlyRole(SECURITY_ROLE_ADMIN) {
        Address.sendValue(
            _deploymentConfig.treasuryAddress, 
            address(this).balance);
    }

    function pause() public onlyRole(SECURITY_ROLE_MANAGER) {
        _pause();
    }

    function unpause() public onlyRole(SECURITY_ROLE_MANAGER) {
        _unpause();
    }
        ...
        returns (string memory)
    {
        ...
        return totalSupply == _deploymentConfig.maxSupply ? 
    ...
    function adjustMintPrice(uint256 price) external onlyRole(SECURITY_ROLE_MANAGER) {
        _runtimeConfig.mintPrice = price;
    }
    ...
    function _mintTokens(address to, uint256 amount) internal {
        ...
        require(
            msg.value >= _runtimeConfig.mintPrice * amount,
            "Payment too small");
        ...
        view
    {
        ...
        require(
            bytes(_runtimeConfig.prerevealTokenURI).length > 0, 
            "prerevealTokenURI must not be empty");
