...
contract Lendarys is Ownable, ERC721Enumerable, ReentrancyGuard {
    ...
    address payable /* not for mainnet immutable */
        public teamAddress;

    uint256 public constant MAX_SUPPLY = 10000;
    uint256 public constant TOKEN_PRICE = 0.01 ether;
    uint256 public constant WHITELIS_PRICE = 0.01 ether;

    uint256 public locker = MAX_SUPPLY;
    ...
    function whitelistMint(uint256 _nb, bytes32[] calldata _proof)
        external
        payable
        nonReentrant
    {
    ...
    function publicSaleMint(uint256 amount) external payable nonReentrant {
        ...
        require(ts + amount <= locker, "Exceed available supply");
        require(msg.value >= TOKEN_PRICE * amount, "Not enough funds");
    ...
    function changeTeamAddress(address payable _teamAddress)
        external
        onlyOwner
    {
        teamAddress = _teamAddress;
