...
/* Errors */
// Called if player doesn't pay enough eth
error CasinoV1__NotAnEntranceFee();
// Called if casino state isn't "OPEN"
error CasinoV1__NotOpen();
// Called if "Upkeep" return false
error CasinoV1__FalseUpkeep();
// Called if transaction fails
error CasinoV1__TransferFailed();
// Called if tx sender isn't owner
...
contract CasinoV1 is VRFConsumerBaseV2, KeeperCompatibleInterface {
    ...
    /* Events */
    // Emits if player has entered
    event Entered(address indexed player);
    // Emits when random words have been requested
    event RandomRequested(uint256 indexed requestId);
    // Emits if winner was calculated and picked
    event WinnerPicked(address indexed winner);

    /* Modifiers */
    // OnlyOwner modifier
    modifier onlyOwner() {
        if (msg.sender != i_owner) revert CasinoV1__NotOwner();
        _;
    }
    ...
    constructor(
        ...
        // Changing casino state to "OPEN"
        s_casinoState = CasinoStates.OPEN;
        // Setting current time stamp
        s_lastTimeStamp = block.timestamp;
        // Setting entrance fee, chainlink keeper interval and some chainlink VRF variables
        ...
        // VRFCoordinator interface
        i_vrfCoordinator = VRFCoordinatorV2Interface(vrfCoordinator);
        // KeeperRegistry interface
    ...
    function enter() public payable {
        if (msg.value != i_entranceFee) {
            revert CasinoV1__NotAnEntranceFee();
        }
    ...
    function fulfillRandomWords(
        ...
        // Getting winner index by modulo operator
        uint256 winnerIndex = randomWords[0] % s_casinoPlayers.length;
        // Setting winner as a variable
        address payable winner = s_casinoPlayers[winnerIndex];
        // Saving winner as a state variable
        s_winner = winner;
        // Reseting casino variables and timestamp
        s_casinoState = CasinoStates.OPEN;
        s_casinoPlayers = new address payable[](0);
        s_lastTimeStamp = block.timestamp;
        // Checking VRF subscription and keeper balances, funding
        if (i_subId > 1) {
        ...
        // Calculating owner commision as 2 % from eth pool
        uint256 ownerCommission = (address(this).balance * 200) / 10000;
        // 1 transaction to winner
        (bool winnerTx, ) = winner.call{value: address(this).balance - ownerCommission}("");
        if (!winnerTx) {
            revert CasinoV1__TransferFailed();
        }
        // 2 transaction to owner
        (bool ownerTX, ) = i_owner.call{value: address(this).balance}("");
        if (!ownerTX) {
            revert CasinoV1__TransferFailed();
        }
        // Creating event (saving recent winner)
        emit WinnerPicked(winner);
    }

    /* Funding functions for VRF and keeper */
    // FundVRFSubscription function
    function fundVRFSubscription(uint256 amount) external onlyOwner {
        i_linkToken.transferAndCall(address(i_vrfCoordinator), amount, abi.encode(i_subId));
    }

    // Withdraw LINK
    function withdrawLink(uint256 amount) external onlyOwner {
        i_linkToken.transfer(i_owner, amount);
    }
