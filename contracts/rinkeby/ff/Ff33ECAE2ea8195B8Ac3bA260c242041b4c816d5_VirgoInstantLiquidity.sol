...
contract VirgoInstantLiquidity is Ownable, Pausable, ReentrancyGuard {
    ...
    // keccak256("ERC721Details(address tokenAddr,uint256[] ids)")
    bytes32 private constant ERC721DETAILS_TYPEHASH = 0x12fe0c134a330bc8706ba981fa981a4a8ab50d23f13b18e464a9286eb63b6627;
    // keccak256("ERC1155Details(address tokenAddr,uint256[] ids,uint256[] amounts)")
    bytes32 private constant ERC1155DETAILS_TYPEHASH = 0x6904d30c80f68f4ecc966217c539db608c69757cfc38e5ee084bffa249352c7e;
    // keccak256("sellNFTsForETH(ERC721Details[] _erc721Details,ERC1155Details[] _erc1155Details,uint256 _quotaPrice,uint256 _nonce,uint256 _deadline)ERC1155Details(address tokenAddr,uint256[] ids,uint256[] amounts)ERC721Details(address tokenAddr,uint256[] ids)")
    bytes32 private constant SELLNFTSFORETH_TYPEHASH = 0xe99f53b7ba01edc2a67bf2a4daeb9c2ff69ca634209253ed425d210f29658de2;
     // keccak256("buyNFTsForETH(ERC721Details[] _erc721Details,ERC1155Details[] _erc1155Details,uint256 _listPrice,uint256 _nonce,uint256 _deadline)ERC1155Details(address tokenAddr,uint256[] ids,uint256[] amounts)ERC721Details(address tokenAddr,uint256[] ids)")
    bytes32 private constant BUYNFTSFORETH_TYPEHASH = 0x2157270df0785337db6bffc878868b0098f352cb6b7d9b91489710cea33a28b7;
   ...
   //Get the balance of contract
    ...
    function verify(        
        ...
        bytes32 messageHash = getMessageHash(_typeHash, _erc721Details, _erc1155Details, _price, _nonce, _deadline);
    ...
    function sellNFTsForETH( 
        ...
        require(_quotaPrice > 0, 'Price must be granter than zero');
        require(getBalance() >= _quotaPrice, 'the liquidity pool is full');        
        require(verify(            
        ...
        for (uint256 i = 0; i < _erc1155Details.length; i++) {
            IERC1155(_erc1155Details[i].tokenAddr).safeBatchTransferFrom(
                _msgSender(),
                address(this),
                _erc1155Details[i].ids,
                _erc1155Details[i].amounts,
                ""
            );
        }

        //transfer ETH to user
        payable(_msgSender()).transfer(_quotaPrice);
    } 
    ...
    function buyNFTsForETH(              
        ...
        require(_deadline >= block.timestamp, 'Trade expired');        
        require(_listPrice > 0, 'Price must be granter than zero');
        require(msg.value >= _listPrice, "less than listing price");        
        require(verify(             
