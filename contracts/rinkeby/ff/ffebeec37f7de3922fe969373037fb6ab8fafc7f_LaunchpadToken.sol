...
contract LaunchpadToken is ERC721A {
    string private _name;
    string private _symbol;
    ...
    function setName(
        ...
        _preSaleSupply = preSaleSupply;
        _maxPerTrans = maxPerTrans;
    ...
    function initialize(address recepient1) public initializer {
        ...
        // _mint( recepient,10000*10**18);
        _transferOwnership(recepient1);
    }

    function mint(uint256 quantity) public payable {
        require(quantity > 0, "zero not allowedaa");
        require(quantity <= _maxPerTrans, "Chosen Amount exceeds MaxQuantity");
        require(
            quantity + totalSupply() <= _maxSupply - _reserve,
            "public sale end"
        );
        require(
        ...
        payable
    {
        ...
        require(quantity <= _maxPerTrans, "Chosen Amount exceeds MaxQuantity");
        require(
            totalSupply() + quantity <= _preSaleSupply,
            "Quantity is greater than remaining Supply"
        );
        require(
            _preSaleAddresses[msg.sender].counter + quantity <= _maxPerWallet,
            "Quantity Must Be Lesser Than Max Supply"
        );

        require(
            _presalePrice * quantity == msg.value,
            "Sent Ether Value Is Incorrect"
        );

        bytes32 leaf = keccak256(abi.encodePacked(msg.sender));
        require(MerkleProof.verify(_merkleProof, _root, leaf), "Invalid Proof");
        _mint(msg.sender, quantity);
        _preSaleAddresses[msg.sender].counter += quantity;
    }

    function reserveTokens(uint256 quantity) public onlyOwner {
        require(quantity <= _reserve, "The quantity exceeds the reserve.");
        _reserve -= quantity;
        _mint(msg.sender, quantity);
    }
