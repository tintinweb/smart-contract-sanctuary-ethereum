 ...
 */
contract TESTNFT is Context, ERC165, IERC721A,Ownable {
    ...
    function whitelistMint(uint256 _mintAmount, bytes32[] memory proof)public payable{ 
        ...
        if(block.timestamp-timeDeployed<=86400 seconds){
        require( msg.value >= whitelistmintCost * _mintAmount ,"please pass in the correct payment");
        }else if (block.timestamp-timeDeployed>86400 seconds){  
        require( whitelistMintPerWalletCounter[msg.sender]+_mintAmount <= 1,"cant mint above above one after 24 hours ");
