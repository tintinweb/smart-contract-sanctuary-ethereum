...
contract CuGamesBrasil is ERC721A, Ownable, ReentrancyGuard {
    ...
    modifier mintCompliance(uint256 _mintAmount) {
        ...
        require(
            _mintAmount <= maxperAddressPublicMint,
            "Can't mint more than max mint"
        );
        require(
    ...
    modifier mintFreeCompliance(uint256 _mintAmount) {
        require(
            addressFreeMintedBalance[msg.sender] + _mintAmount <=
                maxperAddressFreeLimit,
            "max free NFT per address exceeded"
        );
        require(_mintAmount > 0, "Can't mint 0");
        require(
            totalSupply() + _mintAmount <= maxFree,
            "Can't go over free supply"
        );
        _;
    }

    function mintFree(uint256 _mintAmount)
        public
        payable
        mintFreeCompliance(_mintAmount)
        nonReentrant
    {
    ...
    {
        require(
            _mintAmount > 0 && _mintAmount <= maxperAddressPublicMint,
            "Invalid mint amount!"
        );
        require(
            totalSupply() + _mintAmount <= maxSupply,
    ...
    function setMaxSupply(uint256 _supply) public onlyOwner {
        require(
            _supply <= maxSupply && _supply >= totalSupply(),
            "Cannot increase max supply!"
        );
        maxSupply = _supply;
    }
    ...
    function setFreeMaxSupply(uint256 _freeSupply) public onlyOwner {
        require(
            _freeSupply >= totalSupply(),
            "Cannot increase free max supply!"
        );
        maxFree = _freeSupply;
    }
