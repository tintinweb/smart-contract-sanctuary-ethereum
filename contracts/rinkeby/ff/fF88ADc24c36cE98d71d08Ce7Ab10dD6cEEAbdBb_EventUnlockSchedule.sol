...
interface IUpkeepConsumer {
  function listenTriggered(uint32 index, address lockAddress) external;
}
...
contract EventUnlockSchedule is BaseGovernanceWithUserUpgradable, KeeperCompatibleInterface {
    ...
    //event Triggered(uint32 index, address lockAddress);
    event EventsLockEnded(uint32 info);
    ...
    function initialize(ILock lock, bytes calldata data) public initializer {
        ...
        __EventUnlockSchedule_init(lock, _lockStartTime, _lockEndTime, _conditions, _offsetPrices, _amounts, /*_tknAddrBase, _tknAddrQuote,*/ _feedAddress, governanceAddress, _ownerCanWithdraw);
    }

    function __EventUnlockSchedule_init(ILock _lock, uint256 _lockStartTime, uint256 _lockEndTime, uint256[] memory _conditions, int256[] memory _offsetPrices, uint256[] memory _amounts, /*address _tknAddrBase, address _tknAddrQuote,*/ address _feedAddress, address governanceAddress, bool _ownerCanWithdraw) internal onlyInitializing {
        __BaseGovernanceWithUser_init(governanceAddress);
        __EventUnlockSchedule_init_unchained(_lock, _lockStartTime, _lockEndTime, _conditions, _offsetPrices, _amounts, /*_tknAddrBase, _tknAddrQuote,*/ _feedAddress, _ownerCanWithdraw);
    }

    function __EventUnlockSchedule_init_unchained(ILock _lock, uint256 _lockStartTime, uint256 _lockEndTime, uint256[] memory _conditions, int256[] memory _offsetPrices, uint256[] memory _amounts, /*address _tknAddrBase, address _tknAddrQuote,*/ address _feedAddress, bool _ownerCanWithdraw) internal onlyInitializing {
    ...
    function withdrawCapability() external view returns(bool,bool){
        return (ownerCanWithdraw, true/*true if EventScheduler*/);
    }
    ...
    function checkUpkeep(bytes calldata checkData) external view override returns (bool upkeepNeeded, bytes memory performData) {
        ...
        if(block.timestamp < lockStart()){
            //when lock start time not reached
            upkeepNeeded = false;
            performData = abi.encode(0,lockAddress,upkeepConsumer);
        }else{
            if(block.timestamp >= lockEnd()){
                //when lock end time reaches
                upkeepNeeded = !ownerCanWithdraw && amountsUnlocked < _totalAmountLocked(amounts);
                performData = abi.encode(0,lockAddress,upkeepConsumer);
            }else{
                if(offsetPrices.length > 0){
                    int price = _getPrice();
                    for(uint32 i = 0; i < offsetPrices.length; i++) {
                        //upkeepNeeded = false;
                        if(conditions[i] == 0){
                            if(price == offsetPrices[i]){
                                upkeepNeeded = true;
                                performData = abi.encode(i,lockAddress,upkeepConsumer);
                            }
                        }
                        if(conditions[i] == 1){
                            if(price < offsetPrices[i]){
                ...
                if(offsetPrices.length > 0){
                    ...
                    for(uint32 i = 0; i < offsetPrices.length; i++) {
                        if(conditions[i] == 0){
                            if(price == offsetPrices[i]){
                                if(i == _idx){
                                    _unlockAmounts(_idx, _lockAddress, _upkeepConsumer);
                                }
                            }
                        }
                        if(conditions[i] == 1){
    ...
    function _unlockAmounts(uint32 _index, address _lockAddress, address _upkeepConsumer) internal {
    ...
    function _removeEventSchedule(uint32 _idx) internal {
        
        require(_idx < offsetPrices.length, "ERROR: index out of bound");
        
        for(uint32 i = _idx; i < offsetPrices.length-1; i++){
