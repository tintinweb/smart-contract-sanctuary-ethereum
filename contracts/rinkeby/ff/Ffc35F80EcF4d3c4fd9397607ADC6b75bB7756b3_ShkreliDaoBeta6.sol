...
contract ShkreliDaoBeta6 is ERC721A, Ownable, ReentrancyGuard {
    ...
    ) ERC721A("ShkreliDaoBeta6", "ShkreliDAO") {
    ...
    function freeMint(uint256 _mintAmount) public mintCompliance(_mintAmount) {
        require(
            ...
            "OBYC: Free mint phase not open yet!"
        );

        require(
            totalSupply() + _mintAmount <= maxFreeMintSupply,
            "OBYC: Exceeds max free mint supply!"
        );
        require(
            _numberMinted(msg.sender) + _mintAmount <= maxFreeMintAmountPerAddr,
            "OBYC: Exceeds max free mint per wallet!"
        );
        ...
        mintPriceCompliance(_mintAmount)
    {
        require(
            ...
            "OBYC: Exceeds max mint per wallet!"
        );
