// SPDX-License-Identifier: MIT
    ...
    contract AspringBoards is ERC721, Ownable, ERC721Enumerable {
      using SafeMath for uint256;
      using Strings for uint256;
    
      uint256 public constant tokenPrice = 0.01 ether; 
      uint256 public constant PreSaletokenPrice = 0.001 ether; 
      ...
      event AspringBoardsMinted(uint256 tokenId, address owner);
    
      constructor() ERC721("AspringBoards", "AB") {}
      ...
      function mintAspringBoards(uint256 numberOfTokens) external payable {
        ...
        for (uint256 i = 0; i < numberOfTokens; i++) {
          ...
          if (totalSupply() < MAX_TOKENS) {
            ...
            emit AspringBoardsMinted(id, msg.sender);
          }
        }
      }
    
      function presaleAspringBoards(uint256 numberOfTokens) external payable {
        ...
        returns (string memory)
      {
        ...
        if (isRevealed == false) {
          return
            "ipfs://QmXvxi8kfGwvcN77vDD6tcLVWp6PnJnWR8abbsencQQDX7/hidden.json";
        }
