...
contract Lumens is ERC721A, Ownable, PaymentSplitter {
    ...
    // Premint NFTs (0ETH price) = 1
    // VIP NFTs = 2
    // Tier 1 NFTs = 3
    // Tier 2 NFTs = 4
    // Public Minting NFTs = 5
    uint8[] public nftAmountByStage = [0, 1, 6, 4, 2, 10];

    uint16 private constant maxNFTSupply = 8888;
    uint16 private constant reservedNFTs = 237;
    ...
    function mintTo(address to) external payable {
        ...
        require(currentStage == 5, "The public sale has not started yet.");

        _internalMint(to, 1);
    }

    function _internalMint(address to, uint8 amount) internal {
        require(currentStage > 0, "The sale is paused");
        require(msg.value == (currentStage == 1 ? 0 : nftPrice) * amount, "The price is invalid");
        unchecked {
            require(totalSupply() + amount <= maxNFTSupply, "Mint limit reached");
            require(
                alreadyMinted[msg.sender] + amount <= nftAmountByStage[currentStage],
    ...
    function setCurrentStage(uint8 stage) public onlyOwner {
        currentStage = stage;
    }
}
