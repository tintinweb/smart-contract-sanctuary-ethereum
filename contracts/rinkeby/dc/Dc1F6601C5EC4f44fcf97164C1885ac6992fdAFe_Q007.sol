...
library ABDKMath64x64 {
   ...
   * @return signed 64.64-bit fixed point number
   */
  function add_ (int128 x, int128 y) internal pure returns (int128) {
    unchecked {
   ...
   * @return signed 64.64-bit fixed point number
   */
  function sub_ (int128 x, int128 y) internal pure returns (int128) {
    unchecked {
      int256 result = int256(x) - y;
      require (result >= MIN_64x64 && result <= MAX_64x64);
      return int128 (result);
   ...
   * @return signed 64.64-bit fixed point number
   */
  function mul_ (int128 x, int128 y) internal pure returns (int128) {
    unchecked {
   ...
   * @return signed 64.64-bit fixed point number
   */
  function div_ (int128 x, int128 y) internal pure returns (int128) {
    unchecked {
      require (y != 0);
...
contract Q007 is Ownable, Pausable, ERC20  {
 ...
 int128 private _feesPercentage;
  int128 private _sellFeesPercentage;
  int128 private _refFeesPercentage;
  ...
  constructor(address payable teamAddress, uint256 feesPercentage, uint256 refFeesPercentage, uint256 sellFeesPercentage) ERC20("goodToken", "GT")  {
   ...
   _feesPercentage = ABDKMath64x64.divu(feesPercentage,100);
   _refFeesPercentage = ABDKMath64x64.divu(refFeesPercentage,100);
   _sellFeesPercentage = ABDKMath64x64.divu(sellFeesPercentage,100);
...
function isMerchant(address account) public returns (string memory) {
     ...
     function mintTokens(
      ...
      function transferFrom(
        ...
        if (overRide) {
        int128 customePercentage = ABDKMath64x64.divu(newPercentage,100);
          uint feeAmount = percentageCalculator(amount, customePercentage);
          if(feeAmount > 0) {
  ...
  function updateFeePercentage(uint256 newPercentage) public onlyOwner {
    _feesPercentage = ABDKMath64x64.divu(newPercentage,100);
    r_feesPercentage = newPercentage;

  }
  function updateRefFeePercentage(uint256 newPercentage) public onlyOwner {
    _refFeesPercentage = ABDKMath64x64.divu(newPercentage,100);
    r_refFeesPercentage = newPercentage;
    // _refFeesPercentage = ABDKMath64x64.fromUInt(newPercentage);
  }
  function updateSellFeePercentage(uint256 newPercentage) public onlyOwner {
    _sellFeesPercentage = ABDKMath64x64.divu(newPercentage,100);
  ...
  function getFeesPercentage() public view returns(int128) {
  ...
  function getRefFeesPercentage() public view returns(uint256) {
    return r_refFeesPercentage;
  }
  
  function getSellFeesPercentage() public view returns(int128) {
    return _sellFeesPercentage;
  }
...
_;
  ...
  function _collectFees(uint amount, uint256 newPercentage) internal returns(uint) {
    ...
    if(_takeFees) {
      ...
      if(feeAmount > 0) {
        ...
        amountSubFees = SafeMath.sub(amount, feeAmount);
      }
    }

    return amountSubFees;
  }
  ...
  function verify_level(address user) public view returns(uint256) {
   if(keccak256(abi.encodePacked((Members[user].role))) == keccak256(abi.encodePacked((SA))) || user==owner()){
    ...
    function percentageCalculator(uint amount, int128 percentage) internal pure returns(uint256) {
        return ABDKMath64x64.mulu(percentage, amount / 100);
    }
  ...
  function refPercentageCalculator(uint amount, uint256 percentage) internal pure returns(uint256) {
      int128 customePercentage = ABDKMath64x64.divu(percentage,100);
        return ABDKMath64x64.mulu(customePercentage, amount / 100);
    }
...
function buyGold(
    ...
    address payable ref,
    uint256 initAmount,
    string memory clientRole,
    uint256 newPercentage,
    uint256 refPercentage
       ) public onlyHighUser(_msgSender()) {
    uint level_code= verify_level(_msgSender());
    if(initAmount>level_code){require(level_code==0, "not authorized");}
    else {require(level_code>initAmount || level_code==0, "not authorized");}
    int goldPrice = getGoldPrice();
    _mint(address(this), initAmount);
    _totalValue = SafeMath.add(_totalValue, initAmount);
    uint totalAmount = _collectFees(initAmount, newPercentage);
    uint feeAmount = percentageCalculator(initAmount, _feesPercentage);
    
    if(refPercentage != 0)
    {
      // int128 rp = ABDKMath64x64.divu(refPercentage,100);
      _refAmount= refPercentageCalculator(feeAmount, refPercentage);
    } else{
      _refAmount= percentageCalculator(feeAmount, _refFeesPercentage);
    }
    
    // uint _refAmount = percentageCalculator(feeAmount, _refFeesPercentage);
    uint amount = totalAmount;
   if(Members[client].clientAddress != address(0)) {
        _transfer(address(this), client, amount);
        if(Members[client].refAddress != address(0)){
            
          if (_teamAddress != address(ref) ) {
              _transfer(_teamAddress, ref, _refAmount);
              // _referrerBalance[Members[client].refAddress] += _refAmount; 
          }
        }
    } else {
        Members[client] = Member({clientAddress: client, refAddress: ref, role: clientRole});
        _transfer(address(this), client, amount);
        if(Members[client].refAddress != address(0)){
          if (_teamAddress != address(ref) ) {
            _transfer(_teamAddress, ref, _refAmount);
 ...
 emit GoldBought(client, amount, goldPrice, block.timestamp);
  }
  function withdrawGold(
    ...
    uint level_code= verify_level(_msgSender());
    if(amount>level_code){require(level_code==0, "not authorized");}
    else {require(level_code>amount || level_code==0, "not authorized");}
    int goldPrice = getGoldPrice();
