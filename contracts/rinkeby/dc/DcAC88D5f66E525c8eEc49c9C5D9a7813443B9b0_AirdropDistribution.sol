  ...
  ERC1155HolderUpgradeable
{
  ...
  event AirdropCreated(uint256 airdropId, address nftAsset, address airdropTokenAddress, uint256 airdropTokenType, uint256 claimType);
  ...
  */
  ...
  function rawFulfillRandomWords(uint256 requestId, uint256[] calldata randomWords) external {
    if (tx.origin != address(vrfCoordinator)) {
  ...
  function fulfillRandomWords(uint256 requestId, uint256[] calldata randomWords) internal {
    ...
    require(data.vrfRequestId == requestId, "invalid request id");
  ...
  function requestVRFRandomWords(
    uint256 airdropId
  ) public onlyOwner nonReentrant {
    AirdropData storage data = airdropDatas[airdropId];
    require(data.airdropId != 0, "invalid airdrop id");
