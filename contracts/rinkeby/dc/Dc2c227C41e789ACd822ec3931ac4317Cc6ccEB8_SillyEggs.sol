    ...
    ReentrancyGuard
{
    ...
    uint64 public collectionSize = 50000;
    uint16 public maxQtyPerBatchMint = 20;

    string private _baseTokenURI;
    string private _contractURI = "ipfs://contractUri.com"; // TODO: add contract uri
    string private _defaultTokenURI = "ipfs://defaultUri.com"; // TODO: add default token uri

    constructor() ERC721A("Silly Eggs", "SEggs") {
        // init configs
        publicSaleConfig = PublicSaleConfig({
            startTime: 1656129600,
            price: 0.01 ether
        });
        whitelistSaleConfig = WhitelistSaleConfig({
            merkleRoot: 0x861a2b32074f84286119ec8f341bbe0a88912c888c3aadf7541fcb7c46df0e1f,
            startTime: 1656043200,
            price: 0.001 ether,
            maxPerAddress: 5
        });
    }
    ...
    modifier notContract() {
        require(!_isContract(msg.sender), "SE: contract not allowed");
        require(msg.sender == tx.origin, "SE: proxy not allowed");
        _;
    }
     ...
     */
    function publicMint(uint16 qty) external payable notContract nonReentrant {
        require(publicSaleConfig.startTime != 0, "SE: expired or ended");
        require(
            block.timestamp >= publicSaleConfig.startTime,
            "SE: sale not started"
        );
        require(qty <= maxQtyPerBatchMint, "SE: invalid max qty");
        require(
            _totalMinted() + qty <= collectionSize,
            "SE: reached max supply"
        );
        require(
            qty * publicSaleConfig.price == msg.value,
            "SE: invalid pay amount"
        );
     ...
     */
    function whitelistMint(uint16 qty, bytes32[] calldata proof)
        external
        payable
        notContract
        nonReentrant
    {
        require(whitelistSaleConfig.startTime != 0, "SE: WL expired or ended");
        require(
            block.timestamp >= whitelistSaleConfig.startTime,
            "SE: WL sale not started"
        );
        require(verifyMerkleProof(msg.sender, proof), "SE: invalid WL proof");
        require(qty <= whitelistSaleConfig.maxPerAddress, "SE: invalid qty");
        require(
            _totalMinted() + qty <= collectionSize,
            "SE: reached max supply"
        );
        require(
            qty * whitelistSaleConfig.price == msg.value,
            "SE: invalid pay amount"
        );
        ...
        require(
            userWhitelistMintedCounter + qty <=
                whitelistSaleConfig.maxPerAddress,
            "SE: WL mint max reached"
        );
     ...
     */
    function setIsRevealed(bool state) external notContract onlyOwner {
        isRevealed = state;
    }
    ...
    {
        require(_exists(tokenId), "SE: nonexistent token");
