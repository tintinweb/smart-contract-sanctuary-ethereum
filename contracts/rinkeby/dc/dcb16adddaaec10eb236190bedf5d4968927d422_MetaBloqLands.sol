...
contract MetaBloqLands is ERC721, Ownable {
    ...
    function buyLands(int x, int y,string memory uri) public{
        landOwner[x][y]=msg.sender;
        uint tokenId=counter;
         getURI[tokenId]=uri;
        _mint(msg.sender, tokenId);
        counter++;
    }
 function buyBulkLands(int[] memory  x,int[] memory y, string [] memory uri) external{
     require(x.length==y.length,"check ids");
     for(uint i=0;i<x.length;i++){
              buyLands(x[i],y[i],uri[i]);
     }
