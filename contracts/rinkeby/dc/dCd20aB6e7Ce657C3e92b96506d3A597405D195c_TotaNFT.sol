        ...
        return
            _hashTypedDataV4(
                keccak256(
                    abi.encode(
                        keccak256(
                            "NFTVoucher(address redeemer,string uri,uint256 nonce,uint256 expiry)"
                        ),
                        voucher.redeemer,
...
interface AlphaNFT {
    ...
    function mintBatch(address account, uint256 tokenCount) external;
    function burn(uint256 tokenId) external returns(bool);
}

contract TotaNFT is ERC721A, TotaVoucher, Ownable {
    ...
    constructor(address _alphaNFT, address _minter) ERC721A("Tota-Test", "tota") TotaVoucher() {
        alphaNFT = AlphaNFT(_alphaNFT);
        setPresaleStatus(true);
        setBaseURI("https://gateway.pinata.cloud/ipfs/");
        setPresaleURI("QmRpBLJEG6HkqokZhHAAKfBsJpGB58d3rMqFPHBCsH5VDv");
    ...
    function getAlphaNFT() public view returns(address) {
        return address(alphaNFT);
    }

    function setAlphaNFT(address _alphaNFT) public onlyOwner {
        alphaNFT = AlphaNFT(_alphaNFT);
    }
        ...
        returns (string memory)
    {
        ...
        if( presaleStatus ) {
            return string(abi.encodePacked(base, presaleURI));
        }
    ...
    /// @param voucher A signed NFTVoucher that describes the NFT to be redeemed.
    function _redeem(NFTVoucher calldata voucher)
        internal
        virtual
    {
        // make sure signature is valid and get the address of the signer
        address signer = _verifyVoucher(voucher);
        require(minters[signer], "TotaNFT: unauthorized signature");
        // Transfer the token to the redeemer
        // _mint(voucher.redeemer, voucher.uri);
    }

    function mint(address account, string memory uri) public {
        _mint(account, 1);
        uint256 tokenId = uint256(totalSupply());
        setURI(tokenId, uri);
        alphaNFT.mint(account);
    }
    ...
    function mintBatch(
        ...
        uint256 startTokenId = uint256(totalSupply());
        ...
        setBatchURI(ids, cid);
        alphaNFT.mintBatch(account, tokenCount);
    }
