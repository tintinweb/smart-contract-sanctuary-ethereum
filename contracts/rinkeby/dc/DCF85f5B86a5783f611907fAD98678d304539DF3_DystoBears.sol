...
/// @author Dysto QU3B
contract DystoBears is ERC721A, Ownable {
    // Constants: General Configuration
    uint256 public constant MAX_SUPPLY = 9_810;
    ...
    uint256 public constant END_PRICE = 0.004 ether;
    uint256 public constant WHITELIST_PRICE = 0.0025 ether;
    ...
    // Variables: Base and Contracr URIs & Whitelist mapping.
    ...
    modifier callerIsUser() {
        require(tx.origin == msg.sender, "DystoBears: The caller is another contract.");
        _;
    }

    /// @notice Whitelist mint, only callable by whitelisted user.
    function whitelistMint() external payable callerIsUser {
        uint256 _nextID = _currentIndex;

        require(block.timestamp >= whitelistSaleStartTime,"DystoBears: Whitelist sale has not started yet.");
        require(whitelist[msg.sender] > 0, "DystoBears: User has no mints reserved.");
        require(_nextID <= MAX_SUPPLY, "DystoBears: Exceeds the max supply.");
        require(msg.value >= WHITELIST_PRICE, "DystoBears: Not enough ETH.");
        require(_nextID <= MAX_SUPPLY, "DystoBears: Exceeds the max supply.");
        require(balanceOf(msg.sender) < MAX_PER_USER, "DystoBears: Exceeds the max amount per user.");
        // Underflow is impossible as we have already checked that the user has mints reserved.
        unchecked { whitelist[msg.sender]--; }
        _safeMint(msg.sender, 1);
        _refund(WHITELIST_PRICE);
    }

    /// @notice Dutch Auction mint, allows mint several DystoBears paying reduced gas.
    ...
    function auctionMint(uint256 amount) external payable callerIsUser {
        ...
        require(block.timestamp >= auctionStartTime,"DystoBears: Auction has not started yet.");
        require(msg.value >= _totalPrice,"DystoBears: Not enough ETH.");
        require(_nextID + amount - 1 <= MAX_SUPPLY, "DystoBears: Exceeds the max supply.");
        require(balanceOf(msg.sender) + amount <= MAX_PER_USER, "DystoBears: Exceeds the max amount per user.");
    ...
    function devMint(address _to, uint256 amount) external onlyOwner {
        ...
        require(block.timestamp < whitelistSaleStartTime, "DystoBears: Dev mint is over.");
        require(_nextID + amount - 1 <= SUPPLY_FOR_DEVS, "DystoBears: Exceeds the mints reserved for devs.");
    ...
    function withdraw() external onlyOwner {
        ...
        require(success, "DystoBears: Withdraw failed.");
    }
    ...
    function seedWhitelist(address[] calldata addresses, uint256[] calldata amounts) external onlyOwner {
        require(addresses.length > 0, "DystoBears: Whitelist is empty.");
        require(addresses.length == amounts.length, "DystoBears: Addresses do not match amounts.");
    ...
    /// @notice Retrieves the _baseTokenURI, this will be called from ERC721A.
    function _baseURI() internal override view returns (string memory){
        return _baseTokenURI;
    }

    /// @notice Sets the first token ID as 1.
