...
contract Test is ERC20, Ownable {
    ...
    modifier canInitialize{
     ...
     modifier miningAllowed{
        ...
        require(_msgSender() == address(rewardPoolSuperComputer), "Only Reward Pool");
        require(rewardPoolSuperComputer.totalEarningDeposited().sub(rewardPoolSuperComputer.totalEarningMined()) > 0, "Nothing to mine");
        _;
    }
    ...
    function initialize(IERC721 nftContract_, address marketingWallet_, address tomiWallet_, IRewardsPool rewardPoolSuperComputer_) external canInitialize  {
        nftContract = nftContract_;
        marketingWallet = marketingWallet_;
        tomiWallet = tomiWallet_;
        rewardPoolSuperComputer = rewardPoolSuperComputer_;
        initialized = true;
        emissions = emissionCriteria(18000 ether, 39000 ether, 8000 ether, 18000 ether, 17000 ether, 0, true, true, 90, 10);
    }
    ...
    function updateRewardPoolSuperComputer(IRewardsPool newAddress) external onlyOwner {
    ...
    function tomiMining(uint256 amount) miningAllowed external returns(bool) {
        ...
        _mint(address(rewardPoolSuperComputer), amount.mul(emissions.poolPercentage).div(100));
