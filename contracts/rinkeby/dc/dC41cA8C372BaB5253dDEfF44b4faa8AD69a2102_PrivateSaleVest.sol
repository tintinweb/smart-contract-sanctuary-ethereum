...
Privates ale token lock-in contract
 */
contract PrivateSaleVest is Ownable {
    ...
    string public constant voteSymbol = "VeDLP";
    /**
    ...
    uint256 public totalAllocation = 1.5 * 10**8 * 10**18;
    ...
    constructor(
        ...
        require(
            address(_dlpToken) != address(0),
            "Llptoken cannot be the zero address"
        );
        require(
            address(_withdraw) != address(0),
            "withdraw cannot be the zero address"
        );
        require(
            address(_voteAddress) != address(0),
            "withdraw cannot be the zero address"
        );
    ...
    function lock(uint256 unLockTime) internal notLocked onlyOwner {
        ...
        stages[1].period = 12;
        stages[1].periodPercentage = 280;

        //Need to release 18%
        stages[2].startDate = unLockedAt + month * 24;
        stages[2].period = 12;
        stages[2].periodPercentage = 153;
    ...
    function allocate(uint256 unLockTime) public notLocked onlyOwner {
        ...
        require(
            ...
            "Unlock start time cannot be less than the current system time"
        );
        //Add withdrawAddress vote amount
        vote.addVoteAmount(withdrawAddress, voteSymbol, totalAllocation);
        emit Allocated(withdrawAddress, totalAllocation);
        lock(block.timestamp);
    }

    //Claim tokens for Libra withdrawAddress reserve wallet
    function withdrawReserve() private {
        ...
        // Reduce withdrawAddress vote amount
        vote.reduceVoteAmount(withdrawAddress, voteSymbol, tokensToSend);
        // Raising event
...
interface IVote {
    ...
    function addVoteAmount(
        address user,
        string memory symbol,
        uint256 _amount
    ) external returns (bool);

    function reduceVoteAmount(
        address user,
        string memory symbol,
        uint256 _amount
    ) external returns (bool);
