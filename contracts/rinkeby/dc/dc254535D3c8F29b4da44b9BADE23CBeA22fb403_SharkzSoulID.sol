...
contract SharkzSoulID is IVoteScore, Adminable, EIP712Whitelist, ERC4973SoulContainer, ReentrancyGuard {
        ...
        onlyAdmin 
    {
        ...
        if (approved) {
            ...
            require(_isImplementing(_contract, 0x5b5e139f), "Target contract need to support ERC721Metadata");
    ...
    function _linkBadgeContract(address _contract, uint256 _scoreMultiplier) internal virtual {
        ...
        if (_isImplementing(_contract, type(IVoteScore).interfaceId)) {
    ...
    // Returns Soul Badge uint vote score
    ...
    function _tokenVotingScore(uint256 _tokenId) internal virtual view returns (uint256) {
        ...
        for (uint256 i = 0; i < badgeSettings.length; i++) {
            ...
            totalScore += badge.scoreMultiplier * badge.voteScore * _getTokenBalance(_tokenId, badge.contractAddress);
        }
    ...
    function tokenBadgeTraits(uint256 _tokenId) public virtual view returns (string memory) {
        ...
        for (uint256 i = 0; i < badgeSettings.length; i++) {
            ...
            if (_isImplementing(badgeContract, 0x80ac58cd)) {
                if (_isExternalTokenOwner(badgeContract, ownerOf(_tokenId))) {
                    // target contract is ERC721
                    if (bytes(traitValue).length != 0) {
                        traitName = string(abi.encodePacked("ERC721 PFP", soulData.toAlphabetCode(i)));
                        output = string(abi.encodePacked(output, '{"trait_type":"',traitName,'","value":"',traitValue, '"},'));
                    }
                }
            } else {
                // try to detect if it is soul badge
                if (_getTokenBalance(_tokenId, badgeContract) > 0) {
                    traitValue = soulData.getTokenCollectionName(badgeContract);
                    // target contract is sharkz Soul Badge contracts
                    if (bytes(traitValue).length != 0) {
                        traitName = string(abi.encodePacked("Soul Badge ", soulData.toAlphabetCode(i)));
                        output = string(abi.encodePacked(output, '{"trait_type":"',traitName,'","value":"',traitValue, '"},'));
                    }
                }    
            }
    ...
    // Returns token balance for external ERC721 or Soul Badge contract
    function _getTokenBalance(uint256 _badgeTokenId, address _badgeContract) internal virtual view returns (uint256) {
        address currentContract = address(this);
        try IBalanceOfSoul(_badgeContract).balanceOfSoul(currentContract, _badgeTokenId) returns (uint256 rtbal) {
            return rtbal;
        } catch (bytes memory) {
    ...
    // Returns whether an address is token owner
    ...
    // Returns whether target contract reported it implementing an interface (based on IERC165)
    function _isImplementing(address _contract, bytes4 _interfaceCode) internal virtual view returns (bool) {
