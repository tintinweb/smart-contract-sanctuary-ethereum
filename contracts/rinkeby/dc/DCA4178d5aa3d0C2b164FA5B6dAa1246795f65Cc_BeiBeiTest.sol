...
contract BeiBeiTest is Ownable, ERC721AQueryable, ERC2981 {
    ...
    constructor(address payable royaltyReceiver)
        ERC721A("Bei Bei Test", "BeiBeiTest")
    {
    ...
    modifier callerIsUser() {
        require(
            ...
            "Bei Bei Test :: Cannot be called by a contract"
        );
        _;
    }

    function mintGenesisPublic(uint16 _quantity) external payable callerIsUser {
        require(isPublicSale, "Bei Bei Test :: Public Sale Not Yet Active.");
        require(
            (totalSupply() + _quantity) <= MAX_SUPPLY,
            "Bei Bei Test :: Cannot mint beyond max supply"
        );
        require(
            (TOTAL_GENESIS_MINTED + _quantity) <= MAX_GENESIS_SUPPLY,
            "Bei Bei Test :: Beyond Max Supply for Minting Genesis"
        );
        require(
            (totalGenesisPublicSaleMinted[msg.sender] + _quantity) <=
                MAX_MINT_PUBLIC_GENESIS,
            "Bei Bei Test :: Already minted max allowed amount!"
        );
        require(
            msg.value >= (PUBLIC_SALE_GENESIS_PRICE * _quantity),
            "Bei Bei Test :: Payment is below the price"
        );
    ...
    function mintAlphaPublic(uint16 _quantity) external payable callerIsUser {
        ...
        require(
            ...
            "Bei Bei Test :: Beyond Max Supply for Minting Alpha"
        );
        require(
    ...
    function mintGenesisPresale(bytes32[] memory _merkleProof, uint16 _quantity)
        ...
        callerIsUser
    {
        require(isPresale, "Bei Bei Test :: Presale Not Yet Active");
        require(
            (totalSupply() + _quantity) <= MAX_SUPPLY,
            "Bei Bei Test :: Cannot mint beyond max supply"
        );
        ...
        require(
            ...
            "Bei Bei Test :: Cannot mint beyond presale max mint!"
        );
        require(
        ...
        require(
            ...
            "Bei Bei Test :: You are not in whitelist"
        );
