...
import "../lib/vote/IVoteScore.sol";
import "../lib-upgradeable/AdminableUpgradeable.sol";
...
contract SharkzSoulIDV1 is IVoteScore, Initializable, UUPSUpgradeable, AdminableUpgradeable, EIP712WhitelistUpgradeable, ERC4973SoulContainerUpgradeable {
    ...
    // Linked Soul Badge contract settings
    BadgeSetting[] internal _badgeSettings;

    // Link to on-chain data storage
    ISharkzSoulIDData public contractStorage;
    ...
    function setContractStorage(address _contract) 
        external 
        virtual 
        onlyAdmin 
    {
        contractStorage = ISharkzSoulIDData(_contract);
    }

    // Returns whether badge contract is linked
    function isLinkedBadgeContract(address addr) public view virtual returns (bool) {
        for (uint256 i = 0; i < _badgeSettings.length; i++) {
          if (addr == _badgeSettings[i].contractAddress)
          {
            return true;
          }
        }
    ...
    // Returns whether badge contract is linked
    function getBadgeSettingByIndex(uint256 _index) external view virtual returns (BadgeSetting memory) {
        return _badgeSettings[_index];
    }
    ...
    function setBadgeContract(address _contract, uint256 _scoreMultiplier, bool approved) 
        ...
        onlyAdmin 
    {
        bool exists = isLinkedBadgeContract(_contract);
        ...
        if (approved) {
            ...
            // supported contract with both IERC721Metadata + IVoteScore
            require(IERC165(_contract).supportsInterface(0x5b5e139f), "Target contract need to support ERC721Metadata");
            // require(IERC165(_contract).supportsInterface(type(IVoteScore).interfaceId), "Target contract need to support IVoteScore");
    ...
    function _badgeSettingIndex(address addr) internal view virtual returns (uint256) {
        for (uint256 i = 0; i < _badgeSettings.length; i++) {
            if (addr == _badgeSettings[i].contractAddress && addr != address(0)) {
                return i;
            }
        }
    ...
    /**
     * @dev Returns Soul Badge balance from current token `Soul`
     */
    function _getSoulBadgeBalance(uint256 soulTokenId, address _badgeContract) internal virtual view returns (uint256) {
        address soulContract = address(this);

        try IBalanceOfSoul(_badgeContract).balanceOfSoul(soulContract, soulTokenId) returns (uint256 balance) {
            return balance;
        } catch (bytes memory) {
     ...
     */
    function _getSoulBadgeVoteScore(address _badgeContract) internal virtual pure returns (uint256) {
    ...
    function _tokenVotingScore(uint256 _tokenId) internal virtual view returns (uint256) {
        ...
        for (uint256 i = 0; i < _badgeSettings.length; i++) {
            badge = _badgeSettings[i];
            totalScore += badge.scoreMultiplier * badge.voteScore * _getSoulBadgeBalance(_tokenId, badge.contractAddress);
        }
    ...
    function setMetaDesc(string calldata _desc) external virtual onlyAdmin {
        _metaDesc = _desc;
    }

    // Update linking ERC721 contract address
    function setTokenContract(address _addr) external virtual onlyAdmin {
    ...
    function tokenURI(uint256 _tokenId) public virtual view override returns (string memory) {
        ...
        return string(
            abi.encodePacked(
                partA,
                '","image":"data:image/svg+xml;utf8,', contractStorage.tokenImage(_tokenId, _tokenCreationTime(_tokenId)), '","attributes":[',
    ...
    /**
     * @dev Returns Soul Badge name
     */
    function _getSoulBadgeName(address _badgeContract) internal virtual view returns (string memory) {
        try IName(_badgeContract).name() returns (string memory name) {
            return name;
        } catch (bytes memory) {
            return "";
        }
    ...
    function tokenBadgeTraits(uint256 _tokenId) public virtual view returns (string memory) {
        ...
        for (uint256 i = 0; i < _badgeSettings.length; i++) {
            badgeContract = _badgeSettings[i].contractAddress;

            // append each named Soul Badge attached to Soul ID owner
            if (_getSoulBadgeBalance(_tokenId, badgeContract) > 0) {
                traitValue = _getSoulBadgeName(badgeContract);
                if (IERC165(badgeContract).supportsInterface(0x80ac58cd)) {
                    // when target contract is ERC721 contract
                    if (bytes(traitValue).length != 0) {
                        traitName = string(abi.encodePacked("PFP NFT", _toAlphabetCode(i)));
                        output = string(abi.encodePacked(output, '{"trait_type":"',traitName,'","value":"',traitValue, '"},'));
                    }
                } else {
                    // all other contracts just treat as Soul Badge
                    if (bytes(traitValue).length != 0) {
                        // For example, "Soul Badge A", "Soul Badge B", ... "Soul Badge AA", "Soul Badge AB"...
                        traitName = string(abi.encodePacked("Soul Badge ", _toAlphabetCode(i)));
                        output = string(abi.encodePacked(output, '{"trait_type":"',traitName,'","value":"',traitValue, '"},'));
                    }
                }
            }
     ...
     */
    function _isExternalTokenOwner(address _contract, address _ownerAddress) internal virtual view returns (bool) {
     ...
     */
    function _toAlphabetCode(uint256 value) internal pure returns (string memory) {
...
contract EIP712WhitelistUpgradeable is Initializable, ContextUpgradeable, AdminableUpgradeable {
