...
contract OutSad is ERC721AQueryable, Ownable, ReentrancyGuard {
    ...
    function setPartner(address partner, uint256 limit) external onlyOwner {
        partnerProxyAddress = partner;
        partnerLimit = limit;
    }

    function partnerSale(address receiver, uint64 amount) external payable {
        require(status == Status.PreSale, "SUM006");
        require(msg.sender == partnerProxyAddress, 'not partner');
        require(_numberMinted(msg.sender) < partnerLimit, 'reach limit');
