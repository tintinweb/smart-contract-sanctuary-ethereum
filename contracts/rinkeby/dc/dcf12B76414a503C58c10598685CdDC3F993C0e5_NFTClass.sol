 ...
 */
contract NFTClass is ERC721URIStorage, Ownable {

  bool private _COINS_MINTED = false;
  uint16 private _MAX_SUPPLY = 2500;
  uint16 private _LAST_EXECUTED_INDEX = 0;
  string internal _BASE_URI;

 /**
  * Sets name, symbol and baseUri
  */
  constructor(string memory nftName, string memory nftSymbol, string memory baseURI) ERC721(nftName, nftSymbol) Ownable() {
    _BASE_URI = baseURI; // "https://ipfs.io/ipfs/bafybeihyslzcjrjvqark7vhbxn6dwh4kge2xilmqqyfbomsnaaolcq4diy/"
  }
  ...
  */
   ...
   */
  function populateTokensSlice(uint16 slice) public onlyOwner {
    ...
    uint16 i = _LAST_EXECUTED_INDEX;
    uint16 tempLimit = _LAST_EXECUTED_INDEX + slice;
  ...
  function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
    ...
    if (bytes(_tokenURI).length > 0) {
        return string(abi.encodePacked(base, _tokenURI, extension));
    }
