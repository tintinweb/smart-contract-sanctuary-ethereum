...
contract NFTBattles is Ownable {
  ...
  constructor(address _cardsAddress, address _oldBattlesAddress) {
  ...
  function joinBattle() public returns (uint) {
    ...
    require(players[msg.sender].addr == address(0) || players[msg.sender].currentBattle == 0, "Already joined");
		require(canPlay(msg.sender), "Daily limit exceeded");
  ...
  function getBattlesByPlayer(address player) public view returns (uint[] memory bbp) {
    bbp = battlesByPlayer[player];
  }
  ...
  function _finishBattle(uint id, address winner, address claimer) internal {
    ...
    if (changeA > changeB) {
      player.xp += uint(changeA);
      opponent.xp += uint(changeA / 5) + 1;
    } else {
      player.xp += uint(changeB);
      opponent.xp += uint(changeB / 5) + 1;
    }
  ...
  function findBestMatch(uint rank, uint diff) internal returns (uint bestMatch) {
	...
	function canPlay(address _player) internal returns (bool) {
		INFTBattles.Player storage player = players[_player];
    if (player.addr == address(0)) return true;
