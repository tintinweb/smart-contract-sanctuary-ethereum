...
//  _____           _    __   _____  _____  ______ ____  _____  __   _____  _____ 
// |_   _|         | |  /  | / __  \|  _  ||___  // ___||____ |/  | / __  \|____ |
//   | |  ___  ___ | |_ `| | `' / /' \ V /    / // /___     / /`| | `' / /'    / /
//   | | / _ \/ __|| __| | |   / /   / _ \   / / | ___ \    \ \ | |   / /      \ \
//   | ||  __/\__ \| |_ _| |_./ /___| |_| |./ /  | \_/ |.___/ /_| |_./ /___.___/ /
//   \_/ \___||___/ \__|\___/\_____/\_____/\_/   \_____/\____/ \___/\_____/\____/ 
...
contract Test128763123 is ERC721, Ownable, ReentrancyGuard {
    using ECDSA for bytes32;
	using Strings for uint256;

    uint256 public totalSupply = 0;

	string private _baseTokenURI = "https://metadata.nftify-dev.aegas.it/584/metadata/";
	string private _contractURI = "https://nftify-dev-public-bucket.s3.us-east-2.amazonaws.com/584/contract-uri";

    address public tokenfyAddress = 0x777c4a6649f52FF6bEe1eD5747E217fAA382ff1a;
	...
	uint256 public pricePerToken = 1000000000000000;
    uint256 public tokenfyPrice = 2000000000000000;

    // presale settings
    address private signerAddress;
    mapping(address => uint256) public presaleMinted;
	uint256 public presaleMaxSupply = 5;
	uint256 public presaleMaxPerMint = 3;
	uint256 public presalePricePerToken = 1000000000000000;
    uint256 public tokenfyPresalePrice = 2000000000000000;
    ...
    modifier presaleValid(bytes32 hash, bytes memory sig, uint256 qty, uint256 max, uint256 expiresAt) {
		...
		require(qty <= presaleMaxPerMint, "max per mint amount exceeded");
        require(presaleMinted[msg.sender] + qty <= max, "max per wallet exceeded");
		require(totalSupply + qty <= presaleMaxSupply, "out of stock");
        require(block.timestamp < presaleEndDate, "presale ended");
        _;
    }

    modifier saleValid(uint256 qty) {
        require(saleLive, "sale not live");
		require(qty <= maxPerMint, "max per mint amount exceeded");
		require(totalSupply + qty <= maxSupply, "out of stock");
        _;
    }    

	constructor(address _signerAddress) ERC721("Test128763123", "TEST") {
    ...
    function presaleMint(
		...
		require(presalePricePerToken * qty == msg.value, "exact amount needed");
    ...
    function presaleMintTokenfy(
		...
		require(tokenfyPresalePrice * qty == value, "exact amount needed");

        IERC20(tokenfyAddress).transferFrom(msg.sender, address(this), value);
    ...
    function mintTokenfy(uint256 qty, uint256 value) external nonReentrant saleValid(qty) {
        require(tokenfyEnabled, "TKNFY not enabled");
		require(tokenfyPrice * qty == value, "exact amount needed");
	...
	function burn(uint256 tokenId) public virtual {
	    require(burnLive, "burn is disabled");
	...
	function changePresaleMaxSupply(uint256 _maxSupply) external onlyOwner {
		require(_maxSupply >= totalSupply, "less than total supply");
        require(_maxSupply <= maxSupply, "larger than max supply");
