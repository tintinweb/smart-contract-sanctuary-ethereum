...
contract PCV is Ownable {
    ...
    function setsOMNIPrice(uint256 price) public onlyOwner {
        // For $0.10 it's 0.10 * 100000000 = 10000000 
        sOMNIPrice = price;
    }

    function addAsset(address asset, address priceAggregator) public onlyOwner {
        if(asset == address(0)) {
            ethPriceAggregator = priceAggregator;
        } else {
            assets[asset] = priceAggregator;
        }
    }

    function supply(address asset, uint256 _amount) payable public {
        address priceAggregator;
        uint256 amount;
        if(asset == address(0)) {
            priceAggregator = ethPriceAggregator;
            amount = msg.value;
        } else {
            priceAggregator = assets[asset];
            amount = _amount;
        ...
        if (asset != address(0)) {
            bool success = IERC20Detailed(asset).transferFrom(msg.sender, address(this), _amount);
            require(success == true, "unable to transfer asset");
            uint8 decimals = IERC20Detailed(asset).decimals();
            transferAmount = (amount * uint256(price) * (10 ** (18 - decimals))) / sOMNIPrice;
        } else {
            transferAmount = (amount * uint256(price)) / sOMNIPrice;
        }
