...
contract HotBebra228 is ERC721A, ERC721AQueryable, Ownable, ReentrancyGuard {
    ...
    uint256 constant public MINT_PRICE = 0.001 ether; // Change mint price for testing
    uint256 constant public MAX_PER_TX = 2;
    uint256 constant public MAX_PER_WALLET = 6;
    uint256 constant public FREE_SUPPLY = 10;
    ...
    mapping(uint256 => bool) private _secondarySale;
    ...
    function mint(uint256 _amount) external payable {
        ...
        require(msg.value == _amount * cost, "Wrong amount of ETH transferred through.");
        require(totalSupply() + _amount <= TOTAL_SUPPLY, "I'm afraid we're out of Not Okay Lions...");
     ...
     */
    function refund(uint256[] calldata tokenIds) internal {
        require(isRefundGuaranteeActive(), "Refund expired");
        uint256 refundedTokens = 0;

        for (uint256 i = 0; i < tokenIds.length; i++) {
            uint256 tokenId = tokenIds[i];
            require(msg.sender == ownerOf(tokenId), "Cannot refund tokens you do not own!");
            require(_secondarySale[tokenId] == false, "Cannot refund token after it was resold!");
    ...
    function withdraw() external onlyOwner nonReentrant {
        require(isRefundGuaranteeActive() == false, "Founders cannot withdraw anything when the refunds are open!");
