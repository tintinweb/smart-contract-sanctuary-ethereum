...
// get ids from the folders
// check them in the claim function
// update the reward math

import "./Collection.sol";
import "./RewardToken.sol";
import "./RMC.sol";
import "./Libarary/Console.sol";
import "@openzeppelin/contracts/utils/cryptography/MerkleProof.sol";
import "@openzeppelin/contracts/utils/Strings.sol";

/// @title Stake Contract
/// @author Praveen Matheesha
/// @notice Stake users NFTs with and earn rewards for the spesific IDs✅
/// @dev Set token address ✅
/// @dev Set the collection address ✅
/// @dev add controller to the reward contract✅
/// @dev setApproveForAll for the address want to stake and for the stake contract from the collection contract✅
/// @dev Stake✅
/// @dev Check earningInfo✅
/// @dev Claim✅
/// @dev Unstake✅

contract NFTStaking is Ownable, IERC721Receiver {
    using Strings for uint16; //! uint16 for nftid and reward
    ...
    event SetItems(string category, uint reward, uint256 nftId);

    // reference to the Block NFT contract
    RunningMan nft;
    Reward token;
    // RUNNINGMANv2 rmcToken;
    ...
    constructor(
        RunningMan _nft,
        Reward _token // RUNNINGMANv2 _rmc
    ) {
        nft = _nft;
        token = _token;
        // rmcToken = _rmc;
    ...
    ///@dev Check the NFT id,
    ///@dev if it is in rarity set A, update the reward calculation to 100 ether
    ///@dev if it is in rarity set B, update the reward calculation to 500 ether
    ///@dev if it is in rarity set C, update the reward calculation to 1000 ether

    function setMerkleRoot(bytes32 _merkleRoot) public onlyOwner {
        merkleRoot = _merkleRoot;
    }

    // required args: uint16 _nftId, uint16 _reward, string memory _category
    function verifyRewards(bytes32[] memory _merkleProof, bytes32 _leaf)
        public
        view
        returns (bool)
    {
        // merkle root of itemsArrOffchain
        // create leaf from input args
        // verify with _merkleProof
        //! leaf has to be calculated in the contract.
        // Item memory tempItem = Item({category: _category, reward: _reward});
        // ItemMappedToId memory temp = ItemMappedToId({nftId : _nftId, item: tempItem});
        bool ret = MerkleProof.verify(_merkleProof, merkleRoot, _leaf);
        return ret;
        // If verified {
        // sendRewards(_nftIdOwner, reward)
        // _corresponding reward is not stored on chain.
        // Instead user, submits the reward and the nft Id.
        // the leaf has to be calculated onchain. Otherwise
        // users are able to send a reward higher than the original
        // one( Which is used to calculate the leaf. )
        // }
    }
    ...
    function unstakeNFTs(uint[] calldata tokenIds) public {
    ...
    function claim(
        ...
        _claim(msg.sender, tokenIds, false, _reward, _merkleProof);
    }
    ...
    function unstake(
        ...
        _claim(msg.sender, tokenIds, true, _reward, _merkleProof);
    }

    // TOKEN REWARDS CALCULATION
    // to reward users with $RMC reward tokens.
    // D=> rewardmath = 100 ether .... (This gives 1 token per day per NFT staked to the staker)
    // C=> rewardmath = 200 ether .... (This gives 2 tokens per day per NFT staked to the staker)
    // B=> rewardmath = 500 ether .... (This gives 5 tokens per day per NFT staked to the staker)
    // A=> rewardmath = 1000 ether .... (This gives 10 tokens per day per NFT staked to the staker)

    //first 10 tokens, Second 5 tokens third 2 tokens fourth 1 token other 0.5token

    function hashNftData(uint16 _nftId, uint16 _reward)
        public
        pure
        returns (bytes32)
    {
        string memory finalStr1 = string.concat(
            "{nftId:",
            _nftId.toString(),
            ",",
            "reward:",
        ...
        return (keccak256(abi.encodePacked(finalStr1)));
    }
    ...
    function _claim(
        ...
        // require(
        //     rmcToken.balanceOf(msg.sender) > 0,
        //     "you need to store 5000+ tokens in your wallet"
        // );
        require(reward > 0);
        ...
        for (uint i = 0; i < tokenIds.length; i++) {
            ...
            require(stakedAt > 0);
            bytes32 leaf = hashNftData(uint16(tokenId), reward);
            console.log("leaf==>");
            console.logBytes32(leaf);
            bool ret = verifyRewards(merkleProof, leaf);
            require(ret, "Unproven");
            console.log("rewaaaaaaaaaaaard", reward.toString());
            uint256 rewardmath = ((uint256(reward)) *
                10**18 *
                (block.timestamp - stakedAt)) / 86400;
            console.log("reward math-------");
            console.logUint(rewardmath);
    ...
    function earningInfo(
        ...
        // uint[] memory array = new uint[](tokenIds.length);
...
contract RunningMan is ERC721Enumerable {
    ...
    modifier callerIsUser() {
        require(
            ...
            "RNM :: Cannot be called by a contract"
        );
        _;
    }
    modifier OnlyOwner() {
    ...
    function freeMint(uint256 _quantity) public callerIsUser {
        ...
        require(
            ...
            "RNM :: Beyond Max Supply"
        );
        require(
            (totalPublicMint[msg.sender] + _quantity) < 2,
            "RNM :: Already minted 1 NFT!"
        );
    ...
    function mint(uint256 _quantity) external callerIsUser {
        ...
        require(publicSale, "RNM :: Not Yet Active.");
        require(
    ...
    {
        uint256 supply = totalSupply();
        require(whiteListSale, "RNM :: Minting is on Pause");
        require(
            (totalSupply() + _quantity) <= WHITELIST_MINT,
            "RNM :: Cannot mint beyond max supply"
        );
        require(
            (totalWhitelistMint[msg.sender] + _quantity) <= MAX_WHITELIST_MINT,
            "RNM :: Cannot mint beyond whitelist max mint!"
        );
        //create leaf node
        bytes32 sender = keccak256(abi.encodePacked(msg.sender));
        require(
            MerkleProof.verify(_merkleProof, merkleRoot, sender),
            "RNM :: You are not whitelisted"
        );
    ...
    function teamMint(uint256 _nfts) external OnlyOwner {
        uint256 supply = totalSupply();
        require(!teamMinted, "RNM :: Team already minted");
        teamMinted = true;
...
contract ERC20Modified is Context, IERC20, IERC20Metadata {
     ...
     * Ether and Wei. This is the value {ERC20Modified} uses, unless this function is
     * overridden;
     *
     ...
     * required by the EIP. See the note at the beginning of {ERC20Modified}.
     *
     * Requirements:
     *
    ...
    function transferFrom(
        ...
        _approve(
            ...
            _allowances[sender][_msgSender()].sub(
                amount,
                "ERC20Modified: transfer amount exceeds allowance"
            )
        );
        return true;
    }
        ...
        returns (bool)
    {
        _approve(
            ...
            _allowances[_msgSender()][spender].sub(
                subtractedValue,
                "ERC20Modified: decreased allowance below zero"
            )
        );
        return true;
    }
    ...
    function _transfer(
        ...
        require(
            sender != address(0),
            "ERC20Modified: transfer from the zero address"
        );
        require(
            recipient != address(0),
            "ERC20Modified: transfer to the zero address"
        );
        ...
        _balances[sender] = _balances[sender].sub(
            amount,
            "ERC20Modified: transfer amount exceeds balance"
        );
    ...
    function _cast(address account, uint256 amount) internal virtual {
        require(
            account != address(0),
            "ERC20Modified: cast to the zero address"
        );
    ...
    function _burn(address account, uint256 amount) internal virtual {
        require(
            account != address(0),
            "ERC20Modified: burn from the zero address"
        );
        ...
        _balances[account] = _balances[account].sub(
            amount,
            "ERC20Modified: burn amount exceeds balance"
        );
    ...
    function _approve(
        ...
        require(
            owner != address(0),
            "ERC20Modified: approve from the zero address"
        );
        require(
            spender != address(0),
            "ERC20Modified: approve to the zero address"
        );
...
contract RUNNINGMANv2 is ERC20Modified, Ownable {
    using SafeMath for uint256;
    ...
    constructor(
    ...
    ) payable ERC20Modified(name_, symbol_) {
        ...
        require(
            buyTokenRewardsFee.add(buyLiquidityFee).add(buyMarketingFee).add(
                buyDeadFee
            ) <= 25,
            "Total buy fee is over 25%"
        );
        require(
            sellTokenRewardsFee.add(sellLiquidityFee).add(sellMarketingFee).add(
                sellDeadFee
            ) <= 25,
            "Total sell fee is over 25%"
        );

        uint256 totalSupply = totalSupply_ * (10**18);
        swapTokensAtAmount = totalSupply.mul(2).div(10**10); // 0.002%
