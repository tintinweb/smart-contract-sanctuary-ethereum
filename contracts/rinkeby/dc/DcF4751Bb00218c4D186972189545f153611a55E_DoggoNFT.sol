...
contract DoggoNFT is ERC721A, Ownable, ReentrancyGuard {
    ...
    function mint(uint256 _amount) external payable {
        ...
        if (_caller != owner()) {
            ...
            require(totalMintCost == msg.value, "Invalid funds provided");
            require(_amount <= MAX_PER_TX,'exceeds maximum allowed per transaction');
            require(_amount + callerBalance <= MAX_PER_WALLET,'Exceeds maximum allowed per wallet' );
        }
    ...
    function setmaxAmounts(uint256 presaleMax, uint256 _newMax,uint256 newMaxPerWallet)
        public
        onlyOwner
    {
