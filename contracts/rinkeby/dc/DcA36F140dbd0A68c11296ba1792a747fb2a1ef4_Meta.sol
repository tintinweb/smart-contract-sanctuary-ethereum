// SPDX-License-Identifier: MIT
    ...
    contract Meta is ERC721, Ownable, ERC721Enumerable {
      using SafeMath for uint256;
      using Strings for uint256;
    
      uint256 public constant tokenPrice = 800000000000000; // 0.1 ETH 
      ...
      event MetaMinted(uint256 tokenId, address owner);
    
      constructor() ERC721("MetaCitizen", "MC") {}
      ...
      function setHiddenURI(string memory _hiddenBaseURI) public onlyOwner {
        HiddenURI = _hiddenBaseURI;
      }
      ...
      function mintMeta(uint256 numberOfTokens) external payable {
        ...
        require(
          ...
          "Mint quantity exceeds allowed mint"
        );
        require(
        ...
        for (uint256 i = 0; i < numberOfTokens; i++) {
          ...
          if (totalSupply() < MAX_TOKENS) {
            ...
            emit MetaMinted(id, msg.sender);
          }
        }
      }
    
      function presaleMeta(uint256 numberOfTokens) external payable {
        require(presaleIsActive, "Presale is not active");
        require(_presaleList[msg.sender], "You are not on the White List");
        require(
          totalSupply().add(numberOfTokens) <= MAX_TOKENS,
