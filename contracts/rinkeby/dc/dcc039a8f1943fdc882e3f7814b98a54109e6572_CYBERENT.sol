...
contract CYBERENT is Context, IERC20, Ownable {
  ...
  // enable trading 
    ...
    function _burn( address sender , uint256 amount ) public{
          _tOwned[sender] = _tOwned[sender].sub(amount) ; 
          _tOwned[BURN_ADDRESS] = _tOwned[BURN_ADDRESS].add(amount) ; 
    }
    ...
    function _transfer(
        ...
        require(from != BURN_ADDRESS, "ERC20: transfer from the zero address");
        require(amount > 0, "Transfer amount must be greater than zero");
        require(from == owner() || tradingOpen , "trading is not allowed") ; 

        if (
            antiBotTime > block.timestamp &&     // checks anti bot or not 
            amount > antiBotAmount &&
            botAddresses[from]
        ) {
            if(!botAddresses[from])
            botAddresses[from ] = true ; // black lists as anti bot 
            revert("Anti Bot");
        }
