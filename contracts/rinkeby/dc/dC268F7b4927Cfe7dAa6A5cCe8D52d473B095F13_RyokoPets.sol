...
contract RyokoPets is ERC721ABurnable, Ownable, ReentrancyGuard, Options{
    ...
    // whitelist mint
    function whiteListMint(bytes32[] calldata _proof) nonReentrant external callerIsUser privateSaleStep{
    ...
    // claim mint
    function claimMint(bytes32[] calldata _proof) nonReentrant external callerIsUser callerIsRyokoHolder privateSaleStep{
        bytes32 _leaf = keccak256(abi.encodePacked(msg.sender));
        require(claimValid(_proof, _leaf), "Invalid proof");
        require(!claimedTokens[msg.sender], "You already claimed your token");
        claimedTokens[msg.sender] = true;
        _safeMint(msg.sender, maxClaimPerWallet);
    }

    // first collection holders mint
    function holdersMint() nonReentrant external callerIsUser callerIsRyokoHolder privateSaleStep{
    ...
    function setMaxClaimAmount(uint256 _amount) external onlyOwner {
        maxClaimPerWallet = _amount;
    }

    // set max whitelist mint amount
    function setMaxWlAmount(uint256 _amount) external onlyOwner {
        maxWlPerWallet = _amount;
    }
...
contract Options{
    ...
    // First collection holder proof
    modifier callerIsRyokoHolder() {
        require(firstCollection.balanceOf(msg.sender) > 0, "You dont have first collection tokens");
        _;
    }

    // Private sale modifier
    modifier privateSaleStep() {
        require(step == Steps.PrivateSale, "The contract is not in private sale");
        _;
    }

    // Public sale modifier
    modifier publicSaleStep() {
        require(step == Steps.PublicSale, "The contract is not in public sale");
        _;
    }
