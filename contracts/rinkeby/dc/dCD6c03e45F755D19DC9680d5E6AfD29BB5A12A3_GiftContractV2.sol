 ...
 */
    ...
    function getTierIndex(uint256 tokenId) public view returns (uint8) {
        IMembershipNFT nftContract = IMembershipNFT(nftToken);
        uint256 tSupply = 0;
        for (uint8 i = 0; i < nftContract.numberTiers(); i++) {
            uint256 start = nftContract.tierStartId(i);
            uint256 end = nftContract.tierEndId(i);
            tSupply += nftContract.tierSupply(i);
            if (start <= tokenId && tokenId <= end) return i;
        }
        require(tokenId < tSupply, "GIFT_INVALID_TOKENID");
        return 0;
    }
