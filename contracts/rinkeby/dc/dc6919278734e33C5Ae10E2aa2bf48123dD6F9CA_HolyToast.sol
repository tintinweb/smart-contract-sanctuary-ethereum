...
// File: contracts/HolyToast.sol



/*
* Hallelujah!
* The Holy Toast has arrived!
*/
pragma solidity ^0.8.0;
...
contract HolyToast is ERC721A, Ownable, Pausable {
    uint256 public mint1price = 0.005 ether;
    uint256 public mint6price = 0.025 ether;
    uint256 public mint12price = 0.045 ether;
    uint256 public constant maxToast = 144;

    string public constant baseURI = "ipfs://QmcJnZxiQZzz4YGHGchHa65EaFEZZrgvLUrQSPMmqcwSNi/";

    constructor() ERC721A("Holy Toast", "HOLYT", 12) {
        _pause();
    }

    function mintToast() public payable whenNotPaused {
        require(mint1price <= msg.value, 'LOW_ETHER');
        unchecked { require(totalSupply() + 1 <= maxToast, 'MAX_REACHED'); }
        _safeMint(msg.sender, 1);
    }

    function mint6Toast() public payable whenNotPaused {
        require(mint6price <= msg.value, 'LOW_ETHER');
        unchecked { require(totalSupply() + 6 <= maxToast, 'MAX_REACHED'); }
        _safeMint(msg.sender, 6);
    }

    function mint12Toast() public payable whenNotPaused {
        require(mint12price <= msg.value, 'LOW_ETHER');
        unchecked { require(totalSupply() + 12 <= maxToast, 'MAX_REACHED'); }
