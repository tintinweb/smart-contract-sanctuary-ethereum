...
contract Dao {
    ...
    function getProposalsWinnersByCategoryId(uint256 categoryId)
        public
        view
        returns (Proposal[] memory)
    {
        ...
        uint256 index = returnIndexCategory(categoryId);
        Proposal[] memory view_proposals = new Proposal[](
            categories[index].countProporsalsWinners
        );
        for (uint256 i = 0; i < categories[index].proposalsWin.length; i++) {
            Proposal storage proposal = categories[index].proposalsWin[i];
            view_proposals[i] = proposal;
        }
        return view_proposals;
    }
