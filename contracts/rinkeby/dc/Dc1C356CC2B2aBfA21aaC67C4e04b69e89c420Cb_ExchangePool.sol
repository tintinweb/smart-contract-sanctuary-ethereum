...
contract ExchangePool is AccessControl, ReentrancyGuard {
    ...
    function _partnerAirdrop() external nonReentrant onlyOwner paused {
        ...
        require(
            CRDX.allowance(mainOwner, address(this)) >= _amount,
            "allowance is not enough for CRDX"
        );
        ...
        paused
    {
        ...
        transactionRecords[mainOwner].amountOfCRDXReturn = transactionRecords[
            mainOwner
    ...
    {
        require(
            CRDX.allowance(msg.sender, address(this)) >= _amount,
            "allowance is not enough"
        );
    ...
    function _returnCRDXFromPartners(uint256 _amount) internal {
        uint256 tokenAmount = tokenPriceCalculation(_amount);
        require(
            CRDX.balanceOf(address(this)) >= tokenAmount,
            "contract don't have enough CRDX token"
        );

        require(CRDX.transfer(mainOwner, tokenAmount), "tokens not return");
        transactionRecords[partnerContract]
            .amountOfCRDXReturn = transactionRecords[partnerContract]
            .amountOfCRDXReturn
            .add(tokenAmount);
    }
