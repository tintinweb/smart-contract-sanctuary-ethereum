...
contract LitHoots is ERC721, Ownable {
    ...
    uint32 public constant MAX_SUPPLY = 2500;
    uint256 public constant COST = 500 ether;

    ITwigs private twigs;
    IHoots private hoots;

    constructor(address _hootAddress, address _twigAddress) ERC721("LitHoots", "LHOOT") {
        hoots = IHoots(_hootAddress);
    ...
    // Check owner of Hoot
    function _checkOwner(uint tokenId) internal view returns (address) {
        return hoots.ownerOf(tokenId);
    }

    // Mint functions
    function mint(uint256[] calldata hootIds, uint256 payment) public {
        ...
        for (uint256 i = 0; i < hootIds.length; i++) {
            if (_checkOwner(hootIds[i]) != msg.sender) revert NotTokenOwner();
        }
        // Burn TWIGS
        twigs.transferFrom(msg.sender, 0x000000000000000000000000000000000000dEaD, payment);
        // Mint
        for (uint256 i = 0; i < hootIds.length; i++) {
            _mint(msg.sender, hootIds[i]);
        }
    }
    ...
    function hootContractAddress() public view returns (address){
        return address(hoots);
    }
    ...
    function twigContractAddress() public view returns (address){
        return address(twigs);
    }
