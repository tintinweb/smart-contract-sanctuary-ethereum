...
Merkle trees (and packages)
Presale mint function
Mint function (with signature)

todo: starts at 0 or 1 (prob 1)

todo: team mint function (reserve 25/26 (or more?))
*/
...
// todo: change contract name and filename
contract Itchy is ERC721A, ERC2981, Ownable {
  using ECDSA for bytes32;

  uint256 public constant MAX_SUPPLY = 10_000;
  // uint256 public constant MAX_SUPPLY = 16; // todo: remove after testing

  uint256 public price = 0.001 ether; // todo:For testing, update to real price
  ...
  // Tracks which token Ids from the below collections have been used to cliam Grimbogs
  ...
  MintState public _mintState = MintState.OPEN;

  // Constructor

  // todo: change 721a values
  constructor() ERC721A("itchy", "ITCHY") {
  }

  // Mint Functions

  // Most people will just be on the allow list, they can mint 1
  // Some people will have warpy, 555, or euphoric world holdings in which case it will increase what they can mint
  // I can do the merkle proof on the website and just send in an amount and sig, really
  function claimYourGrimbog(string calldata _nonce, bytes calldata _signature, uint256[] calldata _warpyIds, uint256[] calldata _555Ids, uint256[] calldata _euphoricWorldIds) external {
    require(_mintState == MintState.PRESALE, "Presale closed");
    require(!claimMinted[msg.sender], "Already minted");
    require(!usedNonces[_nonce], "Nonce already used");
    require(isValidSignature(keccak256(abi.encodePacked(msg.sender, _nonce)), _signature), "Invalid signature");
    ...
    // If your address is in the Merkle tree, change count to 1.
    // todo: fix _merkeProof issue
    ...
    // todo: we do need to do the merkle tree because this needs to handle the person who's WL'd but doesnt have any collecitons
    // also needs to handle wl + collector
    // also needs to handle just collector
    ...
    for(uint256 i = 0; i < fiveFiveFiveLength;) {
      if (fiveFiveFiveIdsUsed[_warpyIds[i]] == false) {
        unchecked { ++count; }
        fiveFiveFiveIdsUsed[_warpyIds[i]] = true;
      }
      unchecked { ++i; }
    }

    uint256 euphoricWorldLength = _euphoricWorldIds.length;
    for(uint256 i = 0; i < euphoricWorldLength;) {
      if (euphoricWorldIdsUsed[_warpyIds[i]] == false) {
        unchecked { ++count; }
  ...
  // Allows the team to mint to addresses for promotional or marketing purposes
  ...
  function teamReserve() external onlyOwner {
    ...
    // _mint(msg.sender, _amount); // todo: mint where? and how many?
  }
  ...
  function setSystemAddress(address _systemAddress) external { // todo: onlyOwner {
    systemAddress = _systemAddress;
  }

  function setWithdrawAddress(address _address) external { // todo: onlyOwner {
  ...
  */
  function setMintState(uint256 status) external { // todo: onlyOwner {
