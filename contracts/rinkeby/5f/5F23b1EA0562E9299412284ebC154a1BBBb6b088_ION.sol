...
contract ION is ERC20, AccessControl {
    ...
    address public rewardsContractAdd;
    ...
    function _distribution() internal {
        if (checkEpoch()) {
            if (superChgReward != 0 && burnRebaseReward != 0) {
                super._transfer(address(this), rewardsContractAdd, superChgReward); // to stakers (rebase 15%)
        ...
        if (block.timestamp >= deployedTime + eightHours) {
            super._transfer(address(this), stakingAddress, trickleChgReward); // to staking contract
