...
contract Poti is ERC721, ERC721Enumerable, Ownable {
    ...
    uint256 public maxFree = 1000; //Max supply of free mints
    uint256 public maxFreeMint = 1; //Max per wallet in free mint
    uint256 public maxPublicMint = 4; //Max per wallet in public mint
    uint256 public freelimit = 0; //freemint counter
    uint256 public pausef = 1; //pause flag. int cheaper than bool.
    uint256 public freezeURI = 0; //Freeze URI forever flag. 0 equals false. 1 equals true.
    string public baseURI ="ipfs://QmUkUViheUy7LUuECjogvyssonWyvZKBKbjLfr5mFxVsqX";
...
/* ORIGINAL _BASEURI() THAT SHOULD BE USED IF NOTHING WORKS IN ATTEMPT TO CHANGE BASEURI LATER! 
    function _baseURI() internal pure override returns (string memory) {
        return "ipfs://QmWKeAXZ6MsVPLHHzQb7TdDvMTHBssDoSKMbfnZjqWPUpS/";
    }
*/
    ...
    function PublicMint(uint256 _mintAmount) public payable {
        ...
        require(_mintAmount <= maxPublicMint, "Cannot mint more than 4 per wallet." );
        require(total + _mintAmount <= maxSupply, "Cannot mint more than supply.");
        for(uint256 c = 0; c < _mintAmount; ++c) {
        tokenId = _tokenIdCounter.current();
        _safeMint(msg.sender, tokenId); //OR         _mint(to, _tokenIdTracker.current());       _tokenIdTracker.increment();
