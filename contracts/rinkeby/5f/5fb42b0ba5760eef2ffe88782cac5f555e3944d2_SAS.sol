...
contract SAS is ERC721A, Ownable {
  using Strings for uint256;
  ...
  uint256 public setMaxSASPerTxn = 10; 
  uint256 public maxSASPerWallet = 10; 
  uint256 public totalPresaleTickets = 30;
  uint256 public reserve = 5;
  ...
  mapping(address => uint256) public _totalSASPresale;
  mapping(address => uint256) public _totalSASMainsale;
  
  constructor() ERC721A("SAS", "SAS", maxSupply, maxSupply) {
    setBaseURI("ipfs://.../");
  ...
  // internal
    ...
    function preSaleMint(uint256 _mintAmount) public payable {
        if (msg.sender != owner()){
            ...
            require(_mintAmount <= maxSASPerWallet, "Max Mint amount per session exceeded");
        }
        require(_mintAmount > 0, "Need to mint atleast 1 SAS");
        ...
        require(_totalSASPresale[msg.sender] + _mintAmount <= maxSASPerWallet,
    ...
    // function will check if user is eligible for presale
  ...
  function saleMint(uint256 _mintAmount) public payable {
        if (msg.sender != owner()){
            require(!paused, "Public sale is not live, can't mint");
            require(_mintAmount <= setMaxSASPerTxn, "Max Mint amount exceeded");
            require(startMainSale == true, "Main sale is not live yet");
        }
        require(_mintAmount > 0, "Need to mint at least 1 SAS");
        
        uint256 supply = totalSupply();
        require(supply + _mintAmount <= maxSupply - (reserve - reserveMinted), "Max NFT limit exceeded");

        require(
            _totalSASMainsale[msg.sender] + _mintAmount <= maxSASPerWallet,
            "exceeded presale total mints per wallet"
            );
        if (msg.sender != owner()) {
            if (_giftFreeMint[msg.sender]==false){
                ...
                require(_mintAmount == 1, "You can only have 1 free mint");
                _giftFreeMint[msg.sender]=false;
            }
  ...
  function toggleStartMainSale() public onlyOwner{
      startMainSale = !startMainSale;
  }
  
  function setMaxSASPerWallet(uint256 _limit) public onlyOwner() {
    maxSASPerWallet = _limit;
  }
...
//   function to update the presale NFT amount
  ...
  function setMaxSASPerTransaction(uint256 _newmaxMintAmount) public onlyOwner() {
    setMaxSASPerTxn = _newmaxMintAmount;
  }
