...
contract OverUnder is SmolGame, VRFConsumerBaseV2 {
  ...
  // TODO
  // uint8 public minNumberForOver = numberFloor + 10;
  // uint8 public maxNumberForUnder = numberCeil - 10;
  ...
  mapping(address => bool) public lastSelectionWon;
  ...
  function selectNumber(
    ...
    _payServiceFee();
    emit SelectNumber(msg.sender, _numSelected, _isOver, _finalWagerAmount);
  }
    ...
    returns (uint256)
  {
    ...
    uint256 odds;
    if (_isOver) {
      odds = (100 * 985) / (numberCeil - _numSelected);
    } else {
      odds = (100 * 985) / _numSelected;
    }
    return odds - 1000;
  }
  ...
  function _settle(uint256 requestId, uint256 randomNumber) internal {
    ...
    bool _isOver = _selectInitIsOver[_user];
    uint256 _payoutMultiple = _selectInitPayout[_user];
    uint256 _amountToWin = (_amountWagered * _payoutMultiple) /
      PERCENT_DENOMENATOR;

    uint8 _numberDrawn = uint8(randomNumber % numberCeil) + numberFloor;
    numbersDrawn[_numberDrawn]++;
    bool _isDraw = _numberDrawn == _numSelected;
    bool _didUserWin = _isOver
