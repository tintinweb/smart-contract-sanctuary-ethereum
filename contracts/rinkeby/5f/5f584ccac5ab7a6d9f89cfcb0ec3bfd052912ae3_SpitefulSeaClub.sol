...
contract SpitefulSeaClub is ERC721A, Ownable, ERC2981 {
    using Strings for uint256;

    string public baseURI;
    string public placeholderURI;
    
    uint256 public regularCost = 2 ether; //90$
    uint256 public whitelistCost = 1.5 ether; //85$
    uint256 public OGCost = 1 ether; //80$
    
    //Restrictions
    uint256 public maxSupplyPhase1 = 450;
    uint256 public maxSupplyPhase2 = maxSupplyPhase1 + 650;
    uint256 public maxSupplyPublic = maxSupplyPhase2 + 788;
    uint256 public reservedNft = 100;
    uint256 public maxMintAmount = 30;

    uint256 public reservedMintedNFT;
    ...
    struct Infos {
        ...
        uint256 OGCost;
        uint256 maxSupplyPhase1;
        uint256 maxSupplyPhase2;
    ...
    uint256 public MAX_WHITELIST_MINT = 4;

    mapping(address => bool) public OGAddresses;
    mapping(address => uint256) OGMinted;
    uint256 public MAX_OG_MINT = 6;

    mapping(address => uint256) regularMinted;
    uint256 public MAX_REGULAR_MINT = 30;
    ...
    struct XP {
        ...
        // The token id
        uint256 tokenId;
        // the amount of xp points accumulated
        uint256 xp;
    }

    // xp points earned per day
    uint256 public xpPointsPerDay = 5;

    constructor(
    ) 
    ERC721A('Spiteful Sea Club', 'SSC')
    {
        //marketAddress = 0x;
    ...
    function getInfo() public view returns (Infos memory) {
        ...
        allInfos.OGCost = OGCost;
        allInfos.maxSupplyPhase1 = maxSupplyPhase1;
        allInfos.maxSupplyPhase2 = maxSupplyPhase2;
        allInfos.maxSupplyPublic = maxSupplyPublic;
    ...
    function addOGList(address[] calldata _addresses) public onlyOwner {
        uint len = _addresses.length;
        for(uint i = 0; i < len; i ++) {
    ...
    function addOGAddresses(address _address) public onlyOwner {
        OGAddresses[_address] = true;
    }

    function removeOG(address _address) public onlyOwner {
        if (OGAddresses[_address]) {
            delete OGAddresses[_address];
        }
    }
    ...
    function setContractState(uint contractState_) external onlyOwner {
        ...
        if (contractState_ == 0) {
            ...
            contractState = ContractState.PHASE1;
        }
        else if (contractState_ == 2) {
            contractState = ContractState.PHASE2;
        }
        else {
    ...
    /**
     * Update xp points per day.
     * @param newXp The new daily xp points.
     */
    function setXPPointsPerDay(uint256 newXp) external onlyOwner {
        xpPointsPerDay = newXp;
    }

    function setMarketAddress(address newMarketAddr) public onlyOwner {
        marketAddress = newMarketAddr;
    }
    ...
    function setOGCost(uint256 _cost) public onlyOwner {
        OGCost = _cost;
    }
    ...
    function isOG(address _address) public view returns(bool) {
        return OGAddresses[_address];
    }

    function mint(uint256 _mintAmount) public virtual payable {
        require(payees.length > 0, "no payees set");
        require(contractState != ContractState.OFF, "sale not open");
        require(_mintAmount > 0, "need to mint at least 1 NFT");

        uint256 maxQty = canMint(msg.sender);
        require(_mintAmount <= maxQty, "mint limit exceeded");

        if (contractState == ContractState.PHASE1 || contractState == ContractState.PHASE2) {
            if (isOG(msg.sender)) {
                OGMinted[msg.sender] += _mintAmount;
            } 
            else if (isWhitelist(msg.sender) || (contractState == ContractState.PHASE2 && isEbisusBayMember(msg.sender))) {
                whitelistMinted[msg.sender] += _mintAmount;
            }
            else {
                revert("not allowed to mint in this phase");
            }
        }
        else {
            regularMinted[msg.sender] += _mintAmount;
        }
        ...
        for(uint256 i = 0; i < len; i ++) {
            amount = (totalCost * shares[i]) / 10000;
            addToEscrow(amount, payees[i]);
        }  
    }

    function reservedMint(address _to, uint256 _mintAmount) external onlyOwner{
        require(reservedMintedNFT + _mintAmount <= reservedNft, "All Reserved NFT Minted");
        for (uint256 i = 1; i <= _mintAmount; i++) {
            _safeMint(_to, _mintAmount);
        }
        reservedMintedNFT += _mintAmount;
    }

    function airdropMint(address _to, uint256 _amount) external onlyOwner {
        uint256 supply = totalSupply();
        require((supply + _amount) <= maxSupplyPublic, "max NFT limit exceeded");
        _safeMint(_to, _amount);
    }

    //Get NFT Cost
    function mintCost() public view virtual returns (uint256) {
        if (contractState == ContractState.PHASE1 || contractState == ContractState.OFF) {
            return OGCost;
        }
        else if (contractState == ContractState.PHASE2) {
    ...
    function canMint(address _address) public view virtual returns(uint256){
        if (contractState == ContractState.PHASE1) {
            uint leftToMint = maxSupplyPhase1 - totalSupply();

            uint maxPerAddressRemaining;
            if (isOG(_address)) {
                maxPerAddressRemaining = MAX_OG_MINT - OGMinted[_address];
            }
            else if (isWhitelist(_address)) {
                maxPerAddressRemaining = MAX_WHITELIST_MINT - whitelistMinted[_address];
            }
            else {
                return 0;
            }

            if(leftToMint > maxPerAddressRemaining) leftToMint = maxPerAddressRemaining;

            return leftToMint;
        }
        else if (contractState == ContractState.PHASE2) {
            uint leftToMint = maxSupplyPhase2 - totalSupply();

            uint maxPerAddressRemaining;
            if (isOG(_address)) {
                maxPerAddressRemaining = MAX_OG_MINT - OGMinted[_address];
            }
            else if (isWhitelist(_address) || isEbisusBayMember(msg.sender)) {
                maxPerAddressRemaining = MAX_WHITELIST_MINT - whitelistMinted[_address];
            }
            else {
                return 0;
        ...
        else if (contractState == ContractState.PUBLIC) {
            uint leftToMint = maxSupplyPublic - totalSupply();
            uint256 maxPerAddressRemaining = MAX_REGULAR_MINT - regularMinted[_address];
    ...
    function lock() external onlyOwner {
        maxSupplyPublic = totalSupply();
        maxSupplyPhase1 = totalSupply();
        maxSupplyPhase2 = totalSupply();
    }
    ...
    /**
     * gets the total xp for each wallet address
     */
    function fetchXp(uint tokenId) public view returns (XP memory) {
        require(totalSupply() >= tokenId, "Token not minted yet!");
        
        XP memory xpTest;
        xpTest.addr = _ownershipOf(tokenId).addr;
        xpTest.tokenId = tokenId;
        xpTest.xp = ((block.timestamp - _ownershipOf(tokenId).startTimestamp) / 300) * xpPointsPerDay;
