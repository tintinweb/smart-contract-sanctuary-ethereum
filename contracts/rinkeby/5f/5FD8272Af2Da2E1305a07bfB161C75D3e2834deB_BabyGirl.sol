...
contract BabyGirl is ERC721, ERC721Enumerable, ERC721URIStorage, Pausable, Ownable, ERC721Burnable {
    ...
    uint256 public price = 10000000000000000000; //50000000000000000  0.05
    ...
    mapping(address => uint) public claimedt;

    bytes32 public saleMerkleRoot =0x130b49790692a0e8cd6d20ff7d320370bc1d8ae8c0c39e533b86b0faaae966f6;
    //bytes32 public merkleProof =  '["0x38c9c670007accf3f5a986e8e6bf8804e232176dbb34eb7df04fe6d2101920b5","0x5dee5701175633419246d63aec54dd54bc7e84b0fb7233b5cbfb84c0d1015e36","0xf2429c82a6f1bdd467b5f7adf051dff114718e288366be29be47ec31eb5fdfda","0x2cb8baddcaecac27a43126a21ee6f1916b1fe531bfdd82cd7f408e40b59463b7"]';
    ...
    function level_up(uint _summoner,uint sid) public onlyOwner {
 ...
 function internalMinta(address to) internal {
        require(totalSupply() < maxSupply, 'supply depleted');
        _tokenIdCounter.increment();
        _safeMint(to, _tokenIdCounter.current());
    ...
    modifier isValidMerkleProof(bytes32[] calldata merkleProof, bytes32 root,uint256 amount) {
          if (MerkleProof.verify(merkleProof,root,keccak256(abi.encodePacked(msg.sender)))==true){

            if(claimed[msg.sender]){
             _;
            }else{
                require(amount <= 1, 'only 3 per transaction allowed');
    ...
    function mints(bytes32[] calldata merkleProof,uint256 amount,uint256 prices)
           external
           isValidMerkleProof(merkleProof, saleMerkleRoot,amount)
       {
           //require(!claimed[msg.sender], "Address already claimed");
           //claimed[msg.sender] = true;
        //require(msg.value >= price * amount, 'not enough was paid');
        require(prices >= price * amount, 'not enough was paid');
        require(amount <= 1000, 'only 3 per transaction allowed');
        for (uint256 i = 0; i < amount; i++) internalMint(msg.sender);
       }
    function mint(uint256 amount) public payable {
        ...
        require(amount <= 1000, 'only 3 per transaction allowed');
        for (uint256 i = 0; i < amount; i++) internalMinta(msg.sender);
    }
