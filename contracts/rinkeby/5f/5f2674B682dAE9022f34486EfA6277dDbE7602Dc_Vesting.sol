...
contract Vesting is Initializable {
    ...
    function initialize(
        ...
        vestingAmount = token.balanceOf(address(this));
        vestingBegin = uint64(_vestingBegin);
        vestingCliff = uint64(_vestingBegin + _vestingCliffSeconds);
        vestingEnd = uint64(_vestingBegin + _vestingDurationSeconds);
        lastUpdate = uint64(_vestingBegin);
    }

    function setRecipient(address _recipient) public {
        require(msg.sender == recipient, "RECIPIENT_ONLY");
        recipient = _recipient;
    }

    function claim() public {
        require(block.timestamp >= vestingCliff, "PRECLIFF");
        uint256 amount;
