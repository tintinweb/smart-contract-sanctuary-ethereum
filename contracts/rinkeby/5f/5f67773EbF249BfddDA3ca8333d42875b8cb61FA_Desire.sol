...
contract DesireBase is ERC721, Ownable {
    string public __baseURI;

    event BaseURIChanged(string, string);
    event OpenBlindBox(bool status);
...
contract Desire is DesireBase {
    ...
    modifier canMint() {
        require(minted[msg.sender] < maxAmountPerUser, "mint amount limit");
        _;
    }

    modifier checkAmount(uint256 amount) {
        require(amount <= maxAmountPerUser);
        require(minted[msg.sender] + amount <= maxAmountPerUser);
        _;
    }
    ...
    modifier checkAvailableNFT() {
        require(whiteListCurrentId <= whiteListInfo.end, "mint: no remaining");
        _;
    }
    ...
    function updateRoot(bytes32 newRoot) public onlyOwner {
        require(newRoot != whiteListInfo.merkleRoot);
        ...
        checkAvailableNFT
    {
        ...
        require(MerkleProof.verify(proof_, whiteListInfo.merkleRoot, leaf));

        // mint
        for (uint256 i = 0; i < amount; i++) {
            _safeMint(msg.sender, whiteListCurrentId + i);
        }
    ...
    function airdrop(address[] calldata receivers_, uint256 start_)
        public
        onlyOwner
    {
        for (uint256 i = 0; i < receivers_.length; i++) {
            _safeMint(receivers_[i], start_ + i);
        }
    }

    function mintToBidWinner(address receiver_, uint256 id_) public onlyOwner {
        _safeMint(receiver_, id_);
    }
