...
contract NFT is ERC721, Ownable, ReentrancyGuard  {
  ...
  // Constants
  uint public constant TOTAL_SUPPLY = 10_000;
  ...
  constructor() ERC721("Shell Rebels", "SR") {
    baseTokenURI = "https://bafybeifrkm67qbq3jtn45q7ea4zwhqa5jhrnborqbb2xaapr3j5ltyrk3q.ipfs.dweb.link/metadata_test/";
  }
  ...
  function addWhiteList(address[] calldata addresses, uint8 numAllowedToMint) external onlyOwner {
    for (uint256 i = 0; i < addresses.length; i++) {
      _whitelistmintCount[addresses[i]] = numAllowedToMint;
    }
  }
  ...
  /// @dev Mint multiple NFTs for the set price
  function mintShellRebels(uint8 amount) public payable {
    uint256 tokenId = _tokenIds.current();
    ...
    if(saleStatus == SaleStatus.PRESALE) {
      require(msg.value >= PRESALE_MINT_MAX.mul(amount), "Not enough ether to purchase NFTs");
      require(amount <= _whitelistmintCount[msg.sender], "Number of requested tokens exceeds max per whitelisted address");
      _whitelistmintCount[msg.sender] -= amount;
    } else {
      require(msg.value >= PUBLIC_MINT_PRICE.mul(amount), "Not enough ether to purchase NFTs");
      require(amount <= _mintCount[msg.sender], "Number of requested tokens exceeds max per address");
