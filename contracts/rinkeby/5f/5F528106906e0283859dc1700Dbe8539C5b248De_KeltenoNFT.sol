...
contract KeltenoNFT is ERC721Enumerable, PullPayment, Ownable, ReentrancyGuard {
    ...
    TokenForSale[] public tokensForSale;
    string public baseTokenURI;
    uint256 public constant MAX_SUPPLY = 10_000;
    uint256 public PRIVATE_MINT_PRICE = 0.025 ether;
    uint256 public KELTENO_MINT_VALUE = 0.00025 ether;
    uint256 public BATCH_MAX = 20;
    ...
    function mintKelteno(address to) external payable nonReentrant {
        ...
        require(msg.value >= tokenId * KELTENO_MINT_VALUE,string(abi.encodePacked(
                    "Insufficient Payment: Amount of Ether sent is not correct. It should be not less than ",
                    tokenId * KELTENO_MINT_VALUE
                )
            )
        );
        _tokenIdCounter.increment();
    ...
    function keltenoTokenMintValue() external view returns (uint256) {
        uint256 tokenId = _tokenIdCounter.current();
        return tokenId * KELTENO_MINT_VALUE;
    }
    ...
    function privateMint(address to) external payable nonReentrant {
        ...
        require(tokenId < MAX_SUPPLY, "Kelteno NFT is Sold Out");
        require(
            msg.value >= PRIVATE_MINT_PRICE,
            string(abi.encodePacked("Insufficient Payment: Amount should be not less than ",Strings.toString(PRIVATE_MINT_PRICE)))
        );
        _tokenIdCounter.increment();
        _safeMint(to, tokenId);
    }
    ...
    function superMint(address to, uint256 batch) external onlyOwner {
        require(batch > 0 && batch <= BATCH_MAX,string(abi.encodePacked("Machine can create a minimum of 1, maximum of ",Strings.toString(BATCH_MAX)," tokens")));
        for (uint256 i = 0; i < batch; i++) { mintTo(to); }
    }
    /* ---------------------------- @KELTENO SALE -------------------------------*/
    function buyKeltenoTokenFromDeployer(uint256 tokenId,uint256 ,address buyer, bool deployer) external payable nonReentrant {
        require(tokenSellEnabled, "Token Sale Paused");
        for (uint256 i = 0; i < tokensForSale.length; i++) {
            if (tokensForSale[i].tokenId == tokenId) {
                require(msg.value == tokensForSale[i].price);
                transferFrom(deployer == true ? owner() : address(this) ,buyer, tokenId);
            } else {
                revert("This Token Is Not For Sale");
            }
        }
    }
    ...
    function addTokensForSale(uint256[] memory tokenForSale, uint256 price) external onlyOwner {
        for (uint256 i = 0; i < tokenForSale.length; i++) { 
            tokensForSale.push(TokenForSale(tokenForSale[i], price));
        }
    }
    function removeTokensFromSale(uint256[] memory tokenForSale) external onlyOwner {
        for (uint256 i = 0; i < tokenForSale.length; i++) { 
            delete tokensForSale[i];
         }
    }

    function getTokensForSale() external view returns(TokenForSale[] memory){
    ...
    function addRecipients(address[] memory _recipients) external onlyOwner {
        for (uint256 i = 0; i < _recipients.length; i++) { recipients[_recipients[i]] = true; }
    }

    function removeRecipients(address[] memory _recipients) external onlyOwner {
        for (uint256 i = 0; i < _recipients.length; i++) { recipients[_recipients[i]] = false; }
    }

    function claim() external nonReentrant {
        require(recipients[msg.sender] == true, "recipient not registered");
        recipients[msg.sender] = false;
        transferFrom(address(this), msg.sender, airdrops[nextAirdropId]);
        nextAirdropId++;
    }

    function airDropPreDeployedTokens(address[] calldata lucky) external onlyOwner {
