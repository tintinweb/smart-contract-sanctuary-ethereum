...
contract Inception is ERC721, ERC2981, CheaperWhitelist {
    ...
    function preSale(
        ...
        require(amountMint * PRE_PRICE == msg.value, "not enough ether");
        ...
        require(_amountMinted + amountMint <= PRE_MAX, "Maximum purchase exceed");
    ...
    function devMint(address[] memory to, uint256[] memory amount) external onlyOwner {
        ...
        require(totalMinted + reserveMintedAmount <= RESERVE, "reserve minted amount exceed");
    ...
    function changeState(uint256 _saleState) external onlyOwner {
        require(_saleState < 3, "mistyping");
...
abstract contract CheaperWhitelist {
...
abstract contract BoolMap {
    mapping(uint256 => uint256) mintMap;

    constructor(uint256 addressCount) {
        uint256 totalSlot = addressCount % 255 == 0 ? addressCount / 255 : (addressCount / 255) + 1;
    ...
    function setMinted(uint256 index) internal {
        uint256 slot = (index / 255) + (index % 255);
