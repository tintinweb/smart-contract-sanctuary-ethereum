...
contract Lotterify is Ownable {
    ...
    function SetTicket_Price(uint256 _Ticket_Price) public onlyOwner {
        Ticket_Price = _Ticket_Price;
    }

    function SetTicket_fees(uint256 _Ticket_fees) public onlyOwner {
    ...
    function random() public onlyOwner {
        address payable[] memory InputArray = players;
        address payable[] memory OutputArray;
        uint256 InputArrayLen = InputArray.length;
        uint256 i_rand;
        for (uint256 i = 0; i < InputArray.length; i++) {
            i_rand =
                uint256(keccak256(abi.encodePacked(block.difficulty, now, i))) %
                InputArrayLen;
            OutputArray[i] = InputArray[i_rand];
            for (uint256 ii = i_rand; ii < InputArrayLen - 1; ii++) {
