...
contract MetaThreeStar is ERC721A, Ownable, VRFConsumerBase {
  enum SaleStatus {
    PAUSED,
    WINNERS,
    PUBLIC,
  ...
  // ------ Sale Settings
  uint256 private constant PRICE_MTS = 0.1 ether;
  uint256 private constant MAX_MTS = 9202;
  uint256 private constant DEPLOYER_RESERVED_MTS = 101;
  uint256 private constant PUBLIC_MTS_PER_TXN_LIMIT = 3;

  // Remaining public MTSs ()
  uint256 public publicMTSRemaining ;
  bytes32 public winnersMerkleRoot;
  bytes32 public allowlistMerkleRoot;
  address public publicMintSigner;

  mapping(address => bool) public winnersPresalePurchased;
  ...
  ) ERC721A("MetaThreeStar", "MTS") VRFConsumerBase(_vrfCoordinator, _linkAddress) {
    preRevealURI = _preRevealURI;
    withdrawalAddress = _withdrawalAddress;
    chainlinkKeyHash = _chainlinkKeyHash;
    chainlinkFee = _chainlinkFee;
    _mint(tx.origin, DEPLOYER_RESERVED_MTS, "", false);
  }
  ...
  // ------ MERKLE ROOTS
  function setMerkleRoots(bytes32 _winnersMerkleRoot, bytes32 _allowlistMerkleRoot) external onlyOwner {
    winnersMerkleRoot = _winnersMerkleRoot;
    allowlistMerkleRoot = _allowlistMerkleRoot;
  }

  function processMint(uint256 _quantity) internal {
    require(!revealed, "NO MINTS POSTREVEAL");
    require(msg.value == PRICE_MTS * _quantity, "INCORRECT ETH SENT");
    require(totalSupply() + _quantity <= MAX_MTS, "MAX CAP OF MTS EXCEEDED");
    _mint(msg.sender, _quantity, "", false);
  }

  //  ------ WINNERS PRESALE + WINNERS ALLOW LIST SALE
  function winnersSale(bytes32[] memory _proof) external payable callerIsUser {
    require(saleStatus == SaleStatus.WINNERS || saleStatus == SaleStatus.ALLOWLIST, "WINNERS SALE NOT ACTIVE");
    require(
      MerkleProof.verify(_proof, winnersMerkleRoot, keccak256(abi.encodePacked(msg.sender))),
      "MINTER IS NOT ON WINNERS LIST"
    );
    if (saleStatus == SaleStatus.WINNERS) {
      require(!winnersPresalePurchased[msg.sender], "WINNERS PRESALE TICKET ALREADY USED");
      winnersPresalePurchased[msg.sender] = true;
    } else {
      require(!allowlistSalePurchased[msg.sender], "WINNERS ALLOWLIST TICKET ALREADY USED");
  ...
  // ------ CREATE OF MTS (PUBLIC SALE)
  function MTSCreation(
    uint256 _quantity,
    uint256 _nonce,
    bytes memory _signature
  ) external payable callerIsUser {
    require(saleStatus == SaleStatus.PUBLIC || saleStatus == SaleStatus.FINAL, "CREATE OF METATHREESTAR IS NOT ON");
    require(saleStatus == SaleStatus.FINAL || publicMTSRemaining - _quantity >= 0, "PUBLIC CAP EXCEEDED");
    require(_quantity <= PUBLIC_MTS_PER_TXN_LIMIT, "QUANTITY SURPASSES PER-TXN LIMIT");
    validateSignature(msg.sender, _nonce, _signature);

    publicMTSRemaining -= _quantity;
  ...
  function validateSignature(
    ...
    require(signedHash.recover(_signature) == publicMintSigner, "NOT FROM MTS FRONTEND");
