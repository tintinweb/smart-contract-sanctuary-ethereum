...
contract Strategies{
    ...
    function _swapStrategy(uint256 _dx, uint256 _min_dy) internal{
         ...
         */ 
        (bool success, ) = stableSwapAddress.call(
    ...
    function _uniswap(uint256 _dx, uint256 _min_dy) internal {
        ...
        path[0] = sbUSDAddress;
        path[1] = usdcAddress;
        (bool success, ) = uniRouterAddress.call(
            abi.encodeWithSignature(
                "swapExactTokensForTokens(uint256,uint256,address[],address,uint256)",
                _dx, _min_dy, path, address(this), deadline
            )
        );
    ...
    function exchange(uint256 _dx, uint256 _max_slippage) external {
        ...
        uint256 dy = IStableCoin(usdcAddress).balanceOf(address(this));
        IStableCoin(usdcAddress).approve(msg.sender, dy);
        IStableCoin(usdcAddress).transferFrom(address(this), msg.sender, dy);
    }
