...
contract Exchange is Initializable, UUPSUpgradeable, EIP712Upgradeable, OwnableUpgradeable, PausableUpgradeable, AccessControlUpgradeable, ReentrancyGuardUpgradeable {
    ...
    bytes32 constant BID_ORDER_TYPEHASH = keccak256(
        "BidOrder(address seller,bytes4 saleType,address tokenAddress,uint256 tokenId,uint256 amount,uint256 startTime,uint256 endTime,uint256 nonce,address buyer,uint256 bidAmount)"
    );
     ...
     */
    function createOrder(Order memory order, bytes calldata signature) external whenNotPaused nonReentrant {
        require(!blacklisted[msg.sender], "Exchange: Blackisted");
        // verify signature
        bytes32 structHash = genOrderHash(order);
     ...
     */
    function completeFixedSale(Order memory order) external payable whenNotPaused nonReentrant {
        ...
        IERC721Upgradeable(order.tokenAddress).transferFrom(order.seller, msg.sender, order.tokenId);
    }
     ...
     * Emits a {Buy} event, indicating the seller, token address and token id.
     */
    function completeBidding(BidOrder memory bidOrder, bytes calldata signature) external whenNotPaused nonReentrant {
        ...
        // verify signature
        bytes32 structHash = genBidOrderHash(bidOrder);
        bytes32 hashTypedData = _hashTypedDataV4(structHash);
        address signer = verifySignature(hashTypedData, signature);
        require(signer == bidOrder.buyer, "Exchange: Buy order is not signed by the buyer");

        // get unique key
        Order memory order = Order(bidOrder.seller, bidOrder.saleType, bidOrder.tokenAddress, bidOrder.tokenId, bidOrder.amount, bidOrder.startTime, bidOrder.endTime, bidOrder.nonce);
        bytes32 hashKey = genHashKey(order);
        ...
        (bool success) = IWETH(WETH).transferFrom(bidOrder.buyer, msg.sender, bidOrder.bidAmount);
        require(success, "Exchange: WETH transfer failed");

        // transfer token from seller to buyer
        IERC721Upgradeable(bidOrder.tokenAddress).transferFrom(msg.sender, bidOrder.buyer, bidOrder.tokenId);
    }
    ...
    function genBidOrderHash(BidOrder memory order) internal pure returns(bytes32) {
        return keccak256(
            abi.encode(BID_ORDER_TYPEHASH, order.seller, order.saleType, order.tokenAddress, order.tokenId, order.amount, order.startTime, order.endTime, order.nonce, order.buyer, order.bidAmount)
        );
    }
