...
contract FriendlyKittens is ERC721A, Owneable {
    ...
    function needToUpdateCost(uint256 _supply) internal view returns (uint256 _price)
  {
	if(_supply < 1) {
		return 0 ether;
	}
    ...
    function mint(uint256 _amount) external payable {
      ...
      if(FREE_MAX_SUPPLY >= totalSupply()){
            ...
            require(MAX_PER_TX >= _amount , "Excess max per paid tx");
            require(_amount * needToUpdateCost(supply) == msg.value, "Invalid funds provided");
        }
