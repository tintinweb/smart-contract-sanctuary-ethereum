...
contract StakingContract {
    ...
    function JoinBattle(uint256 _amount, uint256 _battleId, address joinerReferalPerson) public {
        ...
        players[battle.joiner].stakerecord[players[battle.joiner].battleCount].amount = joinerAfterDeductedAmount;
    ...
    function LeaveBattle(uint256 count) public {
       ...
       Battle storage battle = battles[count];
        ...
        if(!battle.joined){
            ...
            require(block.timestamp > (players[msg.sender].stakerecord[players[msg.sender].battleCount].playerstartingTime + 1 minutes),"You haveto wait atleast 3 minutes to leave battle if no one join the battle.");
    ...
    function calculateReferalPercentage(uint256 _amount) public pure returns(uint256){
        ...
        uint256 _initialPercentage = 25000; // 25 %
        // require(_initialPercentage <= 100000, "_initialPercentage fee will exceed Gen token staked amount");
        
        return _amount.mul(_initialPercentage).div(100000);
    }

    function calculateWithdrawPercentage(uint256 _amount) public pure returns(uint256){

        uint256 _initialPercentage = 5000; // 5 %
        // require(_initialPercentage <= 100000, "_initialPercentage fee will exceed Gen token staked amount");
    ...
    function calculateRewardInGen(uint256 _amount, uint256 _totalDays) public view returns(uint256){
        ...
        // require(_initialPercentage <= 100000, "_initialPercentage fee will exceed Gen token staked amount");
        uint256 value =  (_amount.mul(_initialPercentage).div(100000));
    ...
    function calculateJoinerPercentage(uint256 _amount) public pure returns(uint256){
        ...
        uint256 _initialPercentage = 30000; // 30 %
        // require(_initialPercentage <= 100000, "_initialPercentage fee will exceed Gen token staked amount");
    ...
    function calculateCreatorPercentage(uint256 _amount) public pure returns(uint256){
        ...
        uint256 _initialPercentage = 20000; // 20 % 
        // require(_initialPercentage <= 100000, "_initialPercentage fee will exceed Gen token staked amount");
    ...
    function calculateRiskPercentage(uint256 _amount, uint256 _riskPercentage ) public pure returns(uint256){
        ...
        uint256 _initialPercentage =_riskPercentage.mul(1000) ;
        // require(_initialPercentage <= 100000, "_initialPercentageCreator fee will exceed Gen token staked amount");
    ...
    function calculateSendBackPercentage(uint256 _amount) public pure returns(uint256){
        ...
        uint256 _initialPercentage = 1000; // 1 %
        // require(_initialPercentage <= 100000, "_initialPercentage fee will exceed Gen token staked amount");
