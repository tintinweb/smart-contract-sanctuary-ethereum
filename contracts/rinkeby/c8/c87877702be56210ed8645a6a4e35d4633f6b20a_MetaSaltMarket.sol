...
contract MetaSaltMarket is MarketCore {
    function __MetaSaltMarket_init(
        ...
        __TransferManager_init_unchained(newProtocolFee, newDefaultFeeReceiver, transferProxy);
    }
}
...
abstract contract TransferManager is Initializable, OwnableUpgradeable {    
    ...
    // function transferERC721LazyMintTest(LibAsset.Asset memory asset, address from, address to) internal returns(address, LibERC721LazyMint.Mint721Data memory) {
    //     require(asset.value == 1, "erc721 value error");
    ...
    function getRoyaltyInfo(LibAsset.Asset memory asset) internal returns (address, uint) {
        require(asset.value == 1, "erc721 value error");
        (address token, LibERC721LazyMint.Mint721Data memory data) = abi.decode(asset.assetType.data, (address, LibERC721LazyMint.Mint721Data));
...
contract MarketCore is Initializable, PausableUpgradeable, OrderValidator, TransferManager {
    ...
    // function getTestSigner() public view returns (address) {
    //     return _signer;
    // }
    ...
    // function validateFullTest(  LibOrder.Order memory order,
    //     bytes memory signature
    //    ) external payable nonReentrant returns (address) {     
    //     address signer = getSigner(order, signature);  
    //     _signer = signer;   
    //     return signer;                         
    // } 

    // function matchAssetsTest(LibAsset.AssetType memory leftAssetType, LibAsset.AssetType memory rightAssetType) public pure returns (LibAsset.AssetType memory) {
    //     if (
    ...
    // function checkMatchAssetsTest (LibOrder.Order memory orderLeft, LibOrder.Order memory orderRight) public pure returns (LibAsset.AssetType memory makeMatch, LibAsset.AssetType memory takeMatch) {
    ...
    // function matchAndTransferTest(LibOrder.Order memory orderLeft, LibOrder.Order memory orderRight) external payable nonReentrant {        
    //     // Excute Order
    ...
    //     address(seller).transferEth(price.sub(fee));
    //     transferERC721LazyMintTest(orderLeft.makeAsset, seller, buyer);
    // }
    ...
    function matchAndTransfer(LibOrder.Order memory orderLeft, LibOrder.Order memory orderRight) internal {        
        ...
        if (orderLeft.makeAsset.assetType.assetClass == LibAsset.ERC721_ASSET_CLASS) {                        
            ...
            (creator, royaltyFee) = getRoyaltyInfo(orderLeft.makeAsset);            
