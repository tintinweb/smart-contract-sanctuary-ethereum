...
contract Tokenado is ERC20, Ownable {
    uint256 public immutable finalTotalSupply = 21000000 * 10**decimals();
    uint256 public immutable presaleMaxSupply = 10000 * 10**decimals();

    uint256 public presaleCounter = 0;
    uint256 public presaleCostStage1 = 0.005 ether; //cost1 for 1 * 10 ** decimals()
    uint256 public presaleCostStage2 = 0.01 ether; //cost2 for 1 * 10 ** decimals()

    uint8 public stage = 0; //0 - nothing, 1 - first presale round, 2 - second presale round, 3 - token launched

    constructor() ERC20("Tokenado", "TKND") {}
    ...
    function buyOnPresale() public payable {
        ...
        uint256 cost = presaleCostStage1;
        if (stage == 2) cost = presaleCostStage2;
