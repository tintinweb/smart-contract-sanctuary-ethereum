...
contract CheemsInu is IBEP20, Ownable
{
    ...
    uint8 private _startingSellTax = 20; //Starting sell tax, will change to _endingSellTax after taxChangeTime has been met.
    uint8 private _startingBuyTax = 20; //Starting sell tax, will change to _endingBuyTax after taxChangeTime has been met.
    uint8 private _startingTransferTax = 20; //Starting sell tax, will change to _endingTransferTax after taxChangeTime has been met.
    ...
    function TeamSetTaxes(uint8 burnTaxes, uint8 liquidityTaxes, uint8 marketingTaxes, uint8 buyTax, uint8 sellTax, uint8 transferTax) external onlyOwner{
        ...
        require(_buyTax <= 20,
        "Error: Honeypot prevention prevents _buyTax from exceeding 20.");
        require(_sellTax <= 20,
        "Error: Honeypot prevention prevents _sellTax from exceeding 20.");
        require(_transferTax <= 20,
        "Error: Honeypot prevention prevents _transferTax from exceeding 20.");
