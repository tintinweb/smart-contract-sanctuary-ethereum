...
contract farting is ERC721A, Ownable {
    using Strings for uint256;
    ...
    function mint(uint256 _amount) external payable {
        require(!paused, "Farting - The Contract is Paused");
        require(
            tokenCount + _amount <= maxSupply,
            "Farting - max NFT limit exceeded"
        );
        if (msg.sender != owner()) {
            require(
                _amount <= maxMintAmount,
                "Farting - max mint amount limit exceeded"
            );
            uint256 ownerMintedCount = addressMintedBalance[msg.sender];
            require(
                ownerMintedCount + _amount <= maxMintAmount,
                "Farting - max NFT per address exceeded"
            );
            require(
                msg.value >= cost * _amount,
                "Farting - insufficient ethers"
            );
        }
