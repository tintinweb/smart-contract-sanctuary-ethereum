...
contract Imagine is ERC721Enumerable, ERC2981, Ownable, ReentrancyGuard {
    ...
    address[] public OperatorList = [0xEf3bd95241015B733BD8fbb9572C4C5E7377692d,0x1e82E4D63bD040fC1B32B8f9745143e07Ae60566,0x1E0049783F008A0085193E00003D00cd54003c71];
    mapping (address => bool) public ApprovedAddr; 

    constructor(string memory _RenderingURI,string memory initialURI,address _RoyaltyReceiver, uint96 _royaltyAmount)  ERC721("Imagine", "IMGN")  {
        URI = initialURI;
    ...
    function ApplyImagination(uint256 token, string memory _imagination) external payable {
        require(msg.value == ReImaginationCost);
    ...
    function mint(uint256 _mintAmount,string memory _WhatYouWant) public IsUser nonReentrant {
        ...
        require(_mintAmount <= MaxPerMint, "Public sale is not active");
        for (uint256 i = 1; i <= _mintAmount; i++) {
        supply++;
        _mint(msg.sender, supply);
        Imagination[supply] = _WhatYouWant;
