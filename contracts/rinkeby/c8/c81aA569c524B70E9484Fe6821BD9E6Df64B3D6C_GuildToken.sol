...
// _/  |_|  |__   ____      ____  __ __|__|  |    __| _/   ____  ____ |  | |  |   ____   _____/  |_|__|__  __ ____
// \   __\  |  \_/ __ \    / ___\|  |  \  |  |   / __ |  _/ ___\/  _ \|  | |  | _/ __ \_/ ___\   __\  \  \/ // __ \
//  |  | |   Y  \  ___/   / /_/  >  |  /  |  |__/ /_/ |  \  \__(  <_> )  |_|  |_\  ___/\  \___|  | |  |\   /\  ___/
//  |__| |___|  /\___  >  \___  /|____/|__|____/\____ |   \___  >____/|____/____/\___  >\___  >__| |__| \_/  \___  >
//            \/     \/  /_____/                     \/       \/                     \/     \/                   \/
...
contract GuildToken is ERC1155, Ownable, AccessControl, ReentrancyGuard {
    bytes32 public constant WHITELISTER = keccak256("WHITELISTER");

    uint256 public constant MAX_SUPPLY = 1000;
    uint256 public constant MAX_AIRDROP_SUPPLY = 100;
    uint256 public constant MINT_PRICE = 0.1 ether;
    uint256 public constant TOKEN_ID = 0;

    uint256 public currentTokenId = 0;
    uint256 public currentAirdropCnt = 0;
    ...
    event Whitelisted(address indexed _user);
    event Minted(address indexed _user);
    event TokenAiredrop(uint256 tokenId);
    ...
    constructor(string memory name_, string memory uri_) ERC1155("") {
        name = name_;
        _setURI(uri_);
    ...
    function mintToken() external payable nonReentrant {
        require(isSaleOpen, "Minting is closed");
        require(currentTokenId + 1 <= MAX_SUPPLY, "Supply exceeds max supply");
        require(whitelist[msg.sender], "Account not whitelisted");
        require(msg.value >= MINT_PRICE, "Not enough funds");
        require(balanceOf(msg.sender, TOKEN_ID) == 0, "Only one token allowed");
    ...
    // temp comment: allowing guild members to approve waitlist -> whitelisted -option:only owner
    function whitelistAddress(address to) external onlyRole(WHITELISTER) {
        require(isSaleOpen, "Minting is closed");
        require(balanceOf(msg.sender, TOKEN_ID) > 0, "Not a guild member");
        addToWhiteList(to);
        emit Whitelisted(to);
    }

    function addToWhiteList(address to) internal {
        whitelist[to] = true;
    }
    ...
    function airDrop(address[] calldata _to) external onlyOwner {
        for (uint256 i = 0; i < _to.length; i++) {
            ...
            require(
                currentAirdropCnt <= MAX_AIRDROP_SUPPLY,
                "All airdrop tokens have been minted"
            );
            currentTokenId++;
            currentAirdropCnt++;
            _mint(_to[i], TOKEN_ID, 1, "");
            emit TokenAiredrop(tokenId);
        }
    }
