...
interface IMarketplace {
    ...
    enum ERROR_CODE {
        ...
        BID_AMOUNT_MISMATCH,
        TRANSFER_FAILED,
        INSUFFICIENT_ALLOWANCE_BALANCE,
        CANNOT_BUY_LISTING,
        BUYING_INVALID_AMOUNT_TOKEN,
        WITHDRAW_FAILED
    }
    ...
    ReentrancyGuardUpgradeable
{
    ...
    modifier onlyOwner() {
        require(
            ...
            Strings.toString(uint8(ERROR_CODE.ONLY_OWNER))
        );
        _;
    }
    ...
    function initialize(
        ...
        primaryFeeBps = uint64(_primaryMarketFeeBps);
        marketFeeBps = uint64(_marketFeeBps); // On
    ...
    function createListing(ListingParameters calldata _params) public override {
        // Get values to populate `Listing`.
        // require(
        //     !safeListing[_params.tokenId],
        //     Strings.toString(uint8(ERROR_CODE.ALREADY_LISTED))
        // );
        require(
            wlistAsset[_params.assetContract],
            Strings.toString(uint8(ERROR_CODE.WHITELIST_ASSET))
        );
        require(
            wlistToken[_params.currencyToAccept],
            Strings.toString(uint8(ERROR_CODE.WHITELIST_CURRENCY))
        );
        ...
        require(
            tokenAmountToList > 0,
            Strings.toString(uint8(ERROR_CODE.INVALID_QUANTITY))
        );
        ...
        safeListing[_params.tokenId] = true;
        listingIdTracker += 1;
        ...
        public
    {
        ...
        require(
            ...
            Strings.toString(uint8(ERROR_CODE.ONLY_TOKEN_OWNER))
        );
        require(
            listing.assetContract == _params.assetContract,
            Strings.toString(uint8(ERROR_CODE.MODIFY_ASSET))
        );
        require(
            listing.tokenId == _params.tokenId,
            Strings.toString(uint8(ERROR_CODE.MODIFY_TOKEN_ID))
        );

        require(
            listing.tokenType == getTokenType(_params.assetContract),
            Strings.toString(uint8(ERROR_CODE.MODIFY_TOKEN_TYPE))
        );
    ...
    function buy(
        ...
        require(
            ...
            Strings.toString(uint8(ERROR_CODE.INVALID_CURRENCY))
        );
        ...
        if (listings[_listingId].tokenType == TokenType.ERC721) {
            ...
            require(
                ...
                Strings.toString(uint8(ERROR_CODE.BUYER_CONFLICT))
            );
            ...
            require(
                ...
                Strings.toString(
                    uint8(ERROR_CODE.INSUFFICIENT_ALLOWANCE_BALANCE)
                )
            );
        }
        ...
        safeListing[targetListing.tokenId] = false;
    }
    ...
    function getMarketCut(uint256 _listingId) public view returns (uint256) {
            ...
            returns (address royaltyFeeRecipient, uint256 royaltyFeeAmount) {
                if (royaltyFeeAmount > 0) {
                    require(
                        ...
                        Strings.toString(uint8(ERROR_CODE.FEES_EXCEEDS_PRICE))
                    );
    ...
    function transferCurrency(
        ...
        if (_currency == NATIVE_TOKEN) {
            if (_from == address(this)) {
                ...
                require(
                    _amount == msg.value,
                    Strings.toString(uint8(ERROR_CODE.BID_AMOUNT_MISMATCH))
                );
    ...
    function safeTransferERC20(
        ...
        require(
            ...
            Strings.toString(uint8(ERROR_CODE.TRANSFER_FAILED))
        );
    }
        ...
        if (_tokenType == TokenType.ERC721) {
            require(
                _tokenOwner == sender ||
                    ...
                    IERC721(_assetContract).getApproved(_tokenId) == sender,
                Strings.toString(
                    uint8(ERROR_CODE.INSUFFICIENT_ALLOWANCE_BALANCE)
                )
            );
    ...
    function validateMarketApproval(
        address _tokenOwner,
        address _assetContract,
        uint256 _tokenId,
        TokenType _tokenType
    ...
    function validateDirectListingSale(
        ...
        require(
            ...
            Strings.toString(uint8(ERROR_CODE.CANNOT_BUY_LISTING))
        );
        ...
        require(
            ...
            Strings.toString(uint8(ERROR_CODE.BUYING_INVALID_AMOUNT_TOKEN))
        );
    ...
    function setRoyaltyTreasury(address _treasury) public onlyOwner {
        require(
            _treasury != address(0),
            Strings.toString(uint8(ERROR_CODE.INVALID_ADDRESS))
        );
    ...
    function validateERC20BalAndAllowance(
        ...
        require(
            IERC20(_currency).balanceOf(_addrToCheck) >=
                ...
                _currencyAmountToCheckAgainst,
            Strings.toString(uint8(ERROR_CODE.INSUFFICIENT_ALLOWANCE_BALANCE))
        );
    }
    ...
    function withdrawTreasury(address to, address currency) external onlyOwner {
    ...
    function _withdraw(
        ...
        require(
            amount > 0,
            Strings.toString(uint8(ERROR_CODE.WITHDRAW_FAILED))
        );
    ...
    {
        isListed = false;
        for (uint256 i = 0; i < listingIdTracker; i++) {
            if (
                listings[i].assetContract == _assetContract &&
    ...
    function whiteListAsset(
        ...
        require(
            ...
            Strings.toString(uint8(ERROR_CODE.ONLY_LISTER))
        );
        if (_type == AssetType.NFT) {
            require(
                !wlistAsset[_assetContract],
                Strings.toString(uint8(ERROR_CODE.WHITELIST_CONFLICT))
            );
