...
contract TestNFT is ERC721Tradable{
    constructor(uint256 mprice) ERC721Tradable("ARCADM", "ARC",0x465cD41394Eb35931f39fdd8507A230FF362f986,0xE1Cd6315D6fC9D5048EbeCa611Ce75f3840a67BF,"https://gateway.pinata.cloud/ipfs/Qmcrnu7D3q14XDBjUNA3TsHFAxyiRGdz7fgLSkZWNXm7VG/",mprice) {}
...
contract ERC721Tradable is ContextMixin, ERC721PresetMinterPauserAutoId, Ownable {
    ...
    // using Roles for Roles.Role;
     ...
     uint256 private _whitelistprice=2000000000000000;
     uint256 private _maxSupply=2012;
     uint256 private _maxPresaleSupply=25;
     address public proxyRegistryAddress;
     address[]  _whitelistaddress;
     address transfereth=0x465cD41394Eb35931f39fdd8507A230FF362f986;
    ...
    function PresalePrice(uint256  Presale_Price ) public   {
        require(hasRole(Admin_Role,msg.sender),"You are not a owner");
    ...
    function mintTo(address _to ) payable public virtual {
          ...
          payable(transfereth).transfer(msg.value);
         _tokenIdTracker.increment();
    ...
    function Presale(address _to) payable public virtual{
         ...
         if(hasRole(Whitelist_Role, msg.sender)){
             tokenlimit[msg.sender].a += 1;
             require(msg.value>= _whitelistprice,"Not enough price to mint");
             payable(transfereth).transfer(msg.value);
             _tokenIdTracker.increment();
             _mint(_to, _tokenIdTracker.current());
         }
    ...
    function addusertowhitelist(address[] memory _whitelist) public {
        require(hasRole(Admin_Role,msg.sender),"You are not a owner");
        for(uint256 i=0; i< _whitelist.length; ++i){
            _setupRole(Whitelist_Role, _whitelist[i]);
        }
        // for(uint256 i=0; i<whitelistaddress.length;i++){
        //     whitelistusers[whitelistaddress[i]]=true;
        // }
    }
    
    function verifywhitlistuser(address _whitelist)public view returns(bool){
        bool iswhitelist=whitelistusers[_whitelist];
        return iswhitelist;
    }
