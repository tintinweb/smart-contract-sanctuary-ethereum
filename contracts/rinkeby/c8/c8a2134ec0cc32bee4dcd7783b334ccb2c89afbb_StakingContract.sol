...
contract StakingContract is Ownable {
    ...
    function unstake(uint256[] memory tokenIds) external {
        ...
        for(uint256 i = 0; i < tokenIds.length; i++) {
            (bool _isStaked, uint _startTime) = getStakingItemInfo(msg.sender, tokenIds[i]);
            if(!_isStaked) { revert tokenIsAlreadyUnstaked(); }
            if(block.timestamp <= (_startTime + 120)) { revert lockTimeNotReached(); }
    ...
    function getStakingItemInfo(address _user, uint256 _tokenId) public view returns(bool _isStaked, uint _startTime) {
