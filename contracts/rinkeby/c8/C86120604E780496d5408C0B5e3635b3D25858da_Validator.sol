...
library Validator {
    ...
    function isKYCRecord(string calldata key) public pure returns (bool) {
        return keccak256(abi.encodePacked(key)) == keccak256(bytes(KYC_VALIDITYINFO));
    }
    ...
    function packPhotochromicRecord(string memory userId, string[DATA_FIELDS] calldata contents, uint32 t) public pure returns (string memory) {
        // TODO: only add the timestamp once?
