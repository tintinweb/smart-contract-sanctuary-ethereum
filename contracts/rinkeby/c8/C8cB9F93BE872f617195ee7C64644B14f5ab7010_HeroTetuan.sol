...
contract HeroTetuan is HeroBase {
  ...
  string internal constant _NAME = "Tetuan";
  string internal constant _SYMBOL = "TETUAN";
  string private constant _URI = "https://raw.githubusercontent.com/tetu-io/tetu-game-assets/master/heroes/tetuan.png";
...
abstract contract HeroBase is NftBase, IHero {
  ...
  event Killed(uint id, address caller, IStatController.NftItem[] drop);
  ...
  function kill(uint heroId) external override returns (IStatController.NftItem[] memory) {
    ...
    emit Killed(heroId, msg.sender, dropSorted);
    return dropSorted;
  }
  ...
  function levelUp(uint tokenId, IStatController.CoreAttributes calldata change) external override {
    ...
    address token = _PAY_TOKEN_SLOT.getAddress();
    uint level = _statController().heroStats(address(this), tokenId).level;
    uint amount = _PAY_TOKEN_AMOUNT_SLOT.getUint() * (level + 1);
