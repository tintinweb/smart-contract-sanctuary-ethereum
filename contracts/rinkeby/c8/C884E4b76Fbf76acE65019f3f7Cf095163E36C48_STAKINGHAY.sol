...
contract STAKINGHAY is Ownable, IERC721Receiver {
    ...
    constructor(WorldHorses _nft, HAY _token) {
        nft = _nft;
        token = _token;
    }
        ...
        returns (uint256[1] memory info)
    {
        ...
        for (uint i = 0; i < tokenIds.length; i++) {
            ...
            uint256 stakedAt = staked.timestamp;
            rewardmath =
                (1000 * (10 * 10**18) * (block.timestamp - stakedAt)) /
                86400;
...
contract WorldHorses is ERC721AQueryable, Ownable, ReentrancyGuard {
