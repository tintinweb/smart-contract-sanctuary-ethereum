...
contract ERC20 is Context, IERC20 {
    ...
    function transferFrom(address sender, address recipient, uint256 amount) public virtual override returns (bool) {
        ...
        _approve(sender, _msgSender(), _allowances[sender][_msgSender()].sub(amount, "A"));
        return true;
    }
    ...
    function decreaseAllowance(address spender, uint256 subtractedValue) public virtual returns (bool) {
        _approve(_msgSender(), spender, _allowances[_msgSender()][spender].sub(subtractedValue, "A"));
        return true;
    }

    function _transfer(address sender, address recipient, uint256 amount) internal virtual {
        require(sender != address(0), "A");
        require(recipient != address(0), "A");
        _beforeTokenTransfer(sender, recipient, amount);
        _balances[sender] = _balances[sender].sub(amount, "A");
    ...
    function _approve(address owner, address spender, uint256 amount) internal virtual {
        require(owner != address(0), "A");
        require(spender != address(0), "A");
...
contract AToken is ERC20, Ownable, Editor {
    using SafeMath for uint256;
	...
	uint8 public immutable vaultFee = 100;
    ...
    constructor(address payable _vaultAddress) ERC20("AToken", "AAA") {
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "A");
        automatedMarketMakerPairs[pair] = value;
        emit SetAutomatedMarketMakerPair(pair, value);
    }
		
    function excludeFromFees(address account, bool excluded) public OwnerOrEditor{
        require(_isExcludedFromFees[account] != excluded, "A");
        _isExcludedFromFees[account] = excluded;
        emit ExcludeFromFees(account, excluded);
    }
		
	function _transfer(address from, address to, uint256 amount) internal override {
        require(from != address(0), "A");
        require(to != address(0), "A");
	...
	modifier OwnerOrEditor() {
        require(_msgSender() == owner() || _msgSender() == editors(), "A");
        _;
    }
}