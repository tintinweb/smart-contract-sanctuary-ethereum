    ...
    PaymentSplitter
{
    ...
    uint256 public constant MAX_SALE_TIMESTAMP = 2**256 - 1;
    ...
    function startPreSale() public onlyOwner {
        startPreSaleTimestamp = block.timestamp;
    }
    ...
    string public notRevealedUri =
        "ipfs://QmZSas9GGgcTzwkS94eMF4XFxHtdVnKCudAxxuvQPFrKEx/";
    string public baseExtension = ".json";

    //stats
    uint256 public maxSupply = 5000;
    uint256 mintAmountLimit = 4;
    uint256 _presalePrice = 100000000000000000; // 0.1 ETH
    uint256 _publicsalePrice = 150000000000000000; // 0.15 ETH
    uint256 private marketingSupply = 200;

    uint256[] private _teamShares = [100];
    address[] private _team = [
        0x058d66e39BE8ae692B7Cc8E53cE30660B4c869E3 // DEV ADMIN
        // 0x73F6E17f5C8AdEd929020859942cA5E014a8e8ef, // Y
        // 0xe9073BD8869c8B5f6E60F797395090975a4c6dAD
    ];
    ...
    function presaleMint(
        ...
        console.log("msg.sender: %s   account: %s ", msg.sender, account);

        // require(!isPaused, "MyTeaParty: Contract is paused.");
        require(!paused(), "MyTeaParty: Contract is paused.");
        require(msg.sender == account, "MyTeaParty: Not allowed.");
        // require(presaleM, "MyTeaParty: PreSale is OFF.");
        require(preSaleStarted(), "MyTeaParty: PreSale is OFF.");
        require(
            numberMinted(msg.sender) + _amount <= mintAmountLimit,
            "MyTeaParty: You are exceeding the max amount of tokens mintable during publicsale (4)."
        );
        ...
        require(
            ...
            "MyTeaParty: max supply exceeded"
        );
        require(
            _presalePrice * _amount <= msg.value,
            "MyTeaParty: Not enough ethers sent"
        );
    ...
    function publicSaleMint(uint256 _amount)
        ...
        nonReentrant
    {
        // require(!isPaused, "MyTeaParty: Contract is isPaused");
        require(!paused(), "MyTeaParty: Contract is isPaused");
        // require(publicM, "MyTeaParty: PublicSale is OFF");
        require(publicSaleStarted(), "MyTeaParty: PublicSale is OFF");
        require(
            numberMinted(msg.sender) + _amount <= mintAmountLimit,
            "MyTeaParty: You are exceeding the max amount of tokens mintable during publicsale (4)."
        );

        require(_amount > 0, "MyTeaParty: zero amount");
        ...
        require(
            _publicsalePrice * _amount <= msg.value,
            "MyTeaParty: Not enough ethers sent"
        );
        _safeMint(msg.sender, _amount);
        // for (uint256 i = 0; i < _amount; i++) {
    ...
    function marketingMint(uint256 quantity) external onlyOwner nonReentrant {
        require(quantity < marketingSupply, "amount exceeds supply");
        require(totalSupply() + quantity < maxSupply, "exceeds maxSupply");
        marketingSupply -= quantity;
