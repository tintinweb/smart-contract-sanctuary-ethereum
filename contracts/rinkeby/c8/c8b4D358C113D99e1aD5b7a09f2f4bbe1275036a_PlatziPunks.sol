...
contract PlatzipPunksDNA {
    ...
    function deterministicPsudoRandomDNA(uint256 _tokenId, address _minter)
        public
        pure
        returns (uint256)
    {
        uint256 combinatedParams = _tokenId + uint160(_minter);
        bytes memory encodedParams = abi.encodePacked(combinatedParams);
        bytes32 hashedParms = keccak256(encodedParams);
        return uint256(hashedParms);
    }
    ...
    function getEyebrowType(uint256 _dna)
        internal
        view
        returns (string memory)
    {
    ...
    function getGraphicType(uint256 _dna)
        internal
        view
        returns (string memory)
    {
...
contract PlatziPunks is ERC721, ERC721Enumerable, PlatzipPunksDNA {
    ...
    function mint() public {
        uint256 _idToken = _idCounter.current();

        require(_idToken < maxSupply, "No PlatziPunks left");

        tokenDNA[_idToken] = deterministicPsudoRandomDNA(_idToken, msg.sender);
