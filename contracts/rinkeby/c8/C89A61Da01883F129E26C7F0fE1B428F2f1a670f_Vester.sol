...
contract Vester is Ownable, IClaimable {
    ...
    address public recipient; // Distributor Contract

    uint256 public totalVestingAmount; // Initial MON balance of the Contract
    ...
    function _getUnlockedAmount() internal returns (uint256 amount) {
        uint256 blockTimestamp = getBlockTimestamp();
        uint256 currentPoint = ((blockTimestamp - vestingBegin) * (1e18)) / (vestingEnd - vestingBegin);
    ...
    function claim() public override returns (uint256 amount) {
        ...
        if (amount > 0) IERC20(MON).transfer(recipient, amount);
    }

    function setRecipient(address _recipient) public onlyOwner {
        require(_recipient != address(0), "Not valid address");
