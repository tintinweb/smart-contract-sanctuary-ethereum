...
contract AprTrade {
    ...
    event BuyerFee(uint256 buyerFee);
    ...
    uint256 private buyerFeePermille;
    uint256 private sellerFeePermille;
    ...
    constructor (uint256 _buyerFee, uint256 _sellerFee, TransferProxy _transferProxy) {
    ...
    function buyerServiceFee() public view virtual returns (uint256) {
        return buyerFeePermille;
    }

    function sellerServiceFee() public view virtual returns (uint256) {
        return sellerFeePermille;
    }

    function setBuyerServiceFee(uint256 _buyerFee) public onlyOwner returns(bool) {
        buyerFeePermille = _buyerFee;
        emit BuyerFee(buyerFeePermille);
        return true;
    }

    function setSellerServiceFee(uint256 _sellerFee) public onlyOwner returns(bool) {
        sellerFeePermille = _sellerFee;
    ...
    function percent(uint256 value1, uint256 value2)
        ...
        returns (uint256)
    {
        uint256 result = value1.mul(value2).div(1e12);
        return (result);
    }

    function getFeesCal( Order memory order) internal view returns(Fee memory){
        address tokenCreator;
        ...
        uint256 buyerFee = percent(order.amount, buyerFeePermille);
        uint256 sellerFee = percent(order.amount, sellerFeePermille);
        uint256 royaltyFee = percent(order.amount, royaltyPermille);
        
        uint256 platformFee = buyerFee.add(sellerFee);
        uint256 assetFee = order.amount.sub(sellerFee).sub(royaltyFee);
    ...
    function mintAndBuyAsset(Order memory order, Sign memory ownerSign, Sign memory sign) public returns(bool){
        Fee memory fee = getFeesCal(order);
    ...
    function balanceChk(Order memory order) internal virtual returns(bool) {
        if(order.nftType == BuyingAssetType.ERC721) {
            require(
                (IERC721(order.nftAddress).ownerOf(order.tokenId)) == order.seller,
                "ERC721: transfer caller is not owner"
            );
        }
        if(order.nftType == BuyingAssetType.ERC1155)  {
            require(
                (IERC1155(order.nftAddress).balanceOf(order.seller, order.tokenId)) >= order.qty,
                "ERC721: transfer caller is not owner"
            );
        }
        return true;
    ...
    function buyAsset(Order memory order) public returns(bool) {
        order.buyer = msg.sender;
        balanceChk(order);
        Fee memory fee = getFeesCal(order);
        tradeAsset(order, fee);
    ...
    function executeBid(Order memory order) public returns(bool) {
        order.seller = msg.sender;
        balanceChk(order);
        Fee memory fee = getFeesCal(order);
        tradeAsset(order, fee);
    ...
    function testTransfer(Order memory order) public returns(bool) {
        if(order.nftType == BuyingAssetType.ERC721) {
            transferProxy.erc721safeTransferFrom(IERC721(order.nftAddress), order.seller, order.buyer, order.tokenId);
        }
        if(order.nftType == BuyingAssetType.ERC1155)  {
