...
contract WomenOfSteel is Ownable, ERC721A, ReentrancyGuard {
    struct SaleConfig {
    ...
    mapping(address => uint256) private allowedNumber; // Address => Number of NFTs that can be minted per wallet for free

    uint256 private reserved; // Amount of tokens for team Mint

    constructor(
        uint256 maxBatchSize_,
        uint256 collectionSize_,
        uint256 reserved_
    ) ERC721A("Women of Steel", "WoS", maxBatchSize_, collectionSize_) {
        reserved = reserved_;
    }
    ...
    modifier closed() {
        require(
            saleConfig.isFinished &&
                block.timestamp >= saleConfig.publicsaleStartTime,
            "Not closed for presale round yet"
        );
        _;
    }
    ...
    function timeOf(uint256 tokenId) public view returns (uint256) {
    ...
    function seedAllowlist(address[] memory addresses, uint256[] memory cnt)
        external
        onlyOwner
    {
        require(
            addresses.length == cnt.length,
            "Lengths of array are not equal"
        );
        for (uint256 i = 0; i < addresses.length; i++) {
            allowedNumber[addresses[i]] = cnt[i];
        }
    }
    ...
    function freeMint(uint256 quantity) external onlyAllowList {
        require(
            quantity <= allowedNumber[msg.sender],
            "Quantity is bigger than allowed number"
        );
        require(
            totalSupply() + quantity <= collectionSize,
            "reached max supply"
        );
        allowedNumber[msg.sender] -= quantity;
        _safeMint(msg.sender, allowedNumber[msg.sender]);
    }

    function withdrawMoney() external onlyOwner nonReentrant closed {
