    ...
    MerkleLeaves
{
    ...
    /* only needed if we strictly enforce phase-specific purchase caps */
    ...
    function mintTokensExpensive(uint256 count) external payable nonReentrant onlyUsers isPublicMinting {
        ...
        _internalMintTokens(_msgSender(), count);
        _setAux(_msgSender(), _pack(0, uint32(b + count)));
    }
    ...
    function getPresaleTokensPurchased(address wallet) external view returns (uint32) {
        return uint32(_getAux(wallet) >> 32);
    }

    function getPublicMintTokensPurchased(address wallet) external view returns (uint32) {
        // a == presale quantity purchased (don't need it here)
        // b == public mint quantity purchased
        // htf do we get b without a to avoid unused local variable warning
        (uint32 a, uint32 b) = _unpack(_getAux(wallet));
        return b;
    }

    /* end phase specific purchase cap functions */
