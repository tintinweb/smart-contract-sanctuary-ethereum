...
contract BondStakingStakingDepository is Ownable, ReentrancyGuard {
    ...
    event BondCreated(address indexed recipient, uint _tokenDeposited, uint _wsfhmDeposited, uint indexed priceInUSD);
    event BondRedeemed(address indexed _recipient, uint _fhmRedeemed, uint _wsfhmRedeemed);
    ...
    *  @notice set contract for auto stake
    ...
    function deposit(
        ...
        // must be > 0.01 FHM ( underflow protection )
        ...
        // stake tokens
        IERC20(FHM).approve(useHelper ? stakingHelper : staking, payout);
        if (useHelper) {
            IStakingHelper(stakingHelper).stake(payout);
        } else {
            IStaking(staking).stake(payout, address(this));
            IStaking(staking).claim(address(this));
        }
        // wrap tokens
        uint _wsfhmDeposit = IwsFHM(wsFHM).wrap(payout);
        ...
        wsfhmInPool : _bondInfo[_depositor].wsfhmInPool.add(_wsfhmDeposit),
        ...
        emit BondCreated(_depositor, _amount, _wsfhmDeposit, priceInUSD);
    ...
    function redeem(address _recipient, bool _stake) external nonReentrant returns (uint) {
        ...
        uint percentVestedBlocks = percentVestedBlocksFor(_recipient);
        ...
        emit BondRedeemed(_recipient, IwsFHM(wsFHM).sFHMValue(wsfhmInPool), wsfhmInPool);
        // emit bond data

        IERC20(wsFHM).transfer(_recipient, wsfhmInPool);
     ...
     *  @param _depositor address
     *  @return payout uint all fhm worth in the pool
     *  @return payoutInWsFHM uint all wsfhm worth in the pool
     *  @return vesting uint
     ...
     */
    function bondInfo(address _depositor) public view returns (uint payout, uint payoutInWsFHM, uint vesting, uint lastBlock, uint pricePaid, uint vestingSeconds, uint lastTimestamp) {
        Bond memory info = _bondInfo[_depositor];
