...
contract Itchy is ERC721A, ERC2981, Ownable {
  ...
  // The Merkle tree root of our free to claim addresses.
  bytes32 public freeToClaimMerkleRoot;
  ...
  // Tracking if an address minted during pre-sale. Can mint up to (N). todo: fill in limit for presale mint
  ...
  function claimYourGrimbog(bytes32[] calldata _merkleProof) external payable {
    require(_mintState == MintState.FREE_TO_CLAIM, "Claim closed");
    // require(totalSupply() + 1 < MAX_SUPPLY, "Exceeds max supply");
    // require(!claimMinted[msg.sender], "Already minted");

    bytes32 leaf = keccak256(abi.encodePacked(msg.sender));
    require(MerkleProof.verify(_merkleProof, freeToClaimMerkleRoot, leaf), "Not on claim list");
  ...
  function mintEarlyGrimbogs(bytes32[] calldata _merkleProof, uint _amount) external payable {
    ...
    bytes32 leaf = keccak256(abi.encodePacked(msg.sender));
    require(MerkleProof.verify(_merkleProof, presaleMerkleRoot, leaf), "Not on presale list");
  ...
  */
  ...
  function setFreeToClaimMerkleRoot(bytes32 _root) external onlyOwner {
    freeToClaimMerkleRoot = _root;
  }
...
price (low for testnet)
supply
starts at 0 or 1 (prob 1)

team mint function (reserve 25)
