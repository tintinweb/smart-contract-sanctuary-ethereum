...
contract MetropolisWorldPassport is ERC721Enumerable, AccessControl, Ownable {
    ...
    //defining the events
    event PassportMinted(address sender, uint256 tokenId);
    event PassportMintedByReferral(
        address sender,
        uint256 tokenId,
        uint256 referredBy
    );
    //event ValueUpdated(string update);
    event AvatarWLSpotAdded(string avatar, uint256 tokenId);
    ...
    constructor(address imageContract, address admin) ERC721("Metropolis World Passport", "METWA") {
        ...
        _setupRole(CONTRACT_ROLE, admin);
    }
    ...
    /**
     * @dev Set up the acces roles so the other contracts can call functions in this contract.
     * @param toAddContract The address of the contract to add.
     
     * @notice Can only be called by wallets with UPDATER_ROLE 
     */
    function addTheRolesForContractCalls(address toAddContract)external onlyRole(UPDATER_ROLE){
        _grantRole(CONTRACT_ROLE, toAddContract);
        // _grantRole(CONTRACT_ROLE, wlContract);, address wlContract, address lotContract, address soft
        // _grantRole(CONTRACT_ROLE, lotContract);
        // _grantRole(CONTRACT_ROLE, soft);
    }
    ...
    function _internalMint(address toWallet, uint32 winChance)internal {
        ...
        _tokenIds.increment();
        emit PassportMinted(msg.sender, newItemId);
    }
    ...
    function userFreeMint(uint16 mints)external{
        require(block.timestamp > 1662196534, "mint not open yet");
    ...
    /**
     * @dev Standard minitng function for users. 
     */
    function paidMint() external payable {
        require(block.timestamp > 1662196534, "mint not open yet");
        // make sure paid enough
        require(_navPrice <= msg.value, "Not enough eth paid to mint");
        //return the excess if any
        _internalMint(msg.sender, 1);
        // the standard way a user will mint.
        if (msg.value > _navPrice){
            Address.sendValue(payable(msg.sender), (msg.value - _navPrice)); 
        }
    }
     ...
     */
    function bulkMint(uint16 numberOfMints, address toAddress, uint256 referrerTokenId)external payable {
        require(block.timestamp > 1662196534, "mint not open yet");
    ...
    function referralMint(uint256 referrerTokenId, address toAddress) internal {
        ...
        emit PassportMintedByReferral(toAddress, newItemId, referrerTokenId);
        // if (msg.value > _navPrice){
        //     Address.sendValue(payable(toAddress), (msg.value - _navPrice));
        // }
    }
    ...
    function attachWLSpot(uint wlId, uint passportId)external onlyRole(CONTRACT_ROLE){
        //require(msg.sender == WL_CONTRACT, "not the right caller");
        ...
        onlyRole(UPDATER_ROLE)
    {
        ...
        emit AvatarWLSpotAdded(avatar, tokenId);
    }
