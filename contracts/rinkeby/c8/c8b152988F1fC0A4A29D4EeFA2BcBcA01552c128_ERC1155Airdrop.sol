...
contract ERC1155Airdrop is ERC1155, Ownable, Pausable, ERC1155Burnable, ERC1155Supply {
    string public name;
    ...
    function claim(bytes32[] calldata proof) external payable whenNotPaused {
        ...
        require(balanceOf(sender, GOLD) == 0, "Already claimed");
        if (_verify(sender, proof)) {
            _safeTransferFrom(owner(), sender, GOLD, 1, "");
        } else {
    ...
    function _verify(address account, bytes32[] memory proof) internal view returns (bool) {
        bytes32 _leaf = keccak256(abi.encodePacked(account));
