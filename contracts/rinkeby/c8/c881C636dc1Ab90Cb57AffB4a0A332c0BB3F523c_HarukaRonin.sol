...
contract HarukaRonin is Ownable, ERC721A {
    ...
    // Updated: I hate tracking it this way, but this allows the devs to mint in batches of smaller than 55 if they'd like.
    ...
    // Note: Again, I'd make these constant, but they had getters and setters before
        // Also the dev mint variable had a different naming convention previously, so I left it.
    uint256 public mintedPublicWhitelistAddressLimit = 2;
    uint256 public mintedPublicAddressLimit = 3;    

    // Note: This is not a best practice, and the metadata URL should not be set until reveal time.
        // That said, the team said that they aren't going to upload anything until then.
        // There is still a window for metadata leaks, but it should be minimized.
    string public baseURI = "ipfs://QmR6DBuhHitXqvZyfNA687FvLaSMcMmDpAXN1osY96zctB/";
    ...
    string public hiddenMetadataUri = "ipfs://QmR6DBuhHitXqvZyfNA687FvLaSMcMmDpAXN1osY96zctB/Hidden.json";
    
    // Honestly, OpenSea and other markets can handle files without .json extensions
        // That said, leaving this (only making it constant) for compatability with v1 of the contract.
    ...
    address public privateWLAddress = 0x5B38Da6a701c568545dCfcB03FcB875f56beddC4;

    // Note: I don't love the naming of this, but it keeps the existing functionality
    // Updated AGAIN: By disabling this and ONLY tracking one mapping we save about 11.8% gas per WL mint.
        // This only comes at the weird edge case of going WL -> public -> WL again
        // If that happened, then users might not get their full WL mint allocation.
    //mapping(address => uint256) public publicWhitelistAddressMintedBalance;

    // Note: I HATE the duplication of logic by adding a second mapping
        // That said, it's the quickest and easiest way to handle the previous v1 logic
        // Devs wanted a WL limit of 2 and Public limit of 3, but Public includes WL
    ...
    // Updated: How much has been paid out the developer, helps protect devs from teams forgetting!
        // There are DEFINITELY cleaner ways to do this, but was a quick and dirty fix at the time limit
    ...
    // A note to the reader:
        // I know that this is not the most efficient contract ever.
        // That said, it was the most efficient contract that I could get to
        // within this CRAZY short window. I took a broken contract,
        // completely re-wrote it, AND tried to keep the work required from
        // their web dev team to a minimum. I'm always down to discuss gas
        // optimizations, just keep my constraints in mind this time!
        // -- @NftDoyler
    ...
    */
    ...
    // Note: The naming for this is kind of weird, but it's the dev mint functionality
    ...
    function publicWhitelistMint(uint256 quantity, bytes32[] memory proof) external payable mintCompliance(quantity) {
        ...
        // Updated: Add developer's 30% cut
