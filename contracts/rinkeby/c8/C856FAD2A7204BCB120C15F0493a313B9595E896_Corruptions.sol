...
interface IAnomura {
    function mintAnomura(address _address, uint256 _tokenId) external payable;
}

contract Corruptions is
    Initializable,
    ...
    OwnableUpgradeable
{
    ...
    /**
     * @dev
     * Internal Team are from 1-100 (100 supply)
     * Early List are from 101 - 8100 (8000)
     * White List are from 8101 - 28100 (10k phase 1, 10k phase 2)
     * Public List are from 28101 - no limited
     */
    uint256 private constant MAX_TEAM_MINT = 100;
    uint256 private teamMintId;

    uint256 public constant EARLY_LIST_SALE_PRICE = 0.01 ether;
    uint256 private constant MAX_EARLY_MINT = 8000;
    uint256 private earlyListMintId;

    uint256 public constant WHITELIST_SALE_PRICE = 0.01 ether;
    uint256 private constant MAX_WHITELIST_MINT = 20000;
    uint256 private whiteListMintId;
    ...
    bytes32 public earlyMerkleRoot;
    ...
    /**
     * @dev Keep track of bowl belong to which address
     */
    mapping(uint256 => address) public bowlToOwner;

    function initialize() public initializer {
        __ERC721_init("Corruptions", "CORRUPT");
        ...
        isPaused = false;
        maxMultiplier = 24;
        bowlIPFS = "https://gateway.pinata.cloud/ipfs/QmQnbCZDtH47uWtNBBEfgAnn1j6Ec2BWLEkQtoY3hd5fhN";
        bowlEmptyIPFS = "https://gateway.pinata.cloud/ipfs/QmRZXyCCQwxeJADG1t8JEaUas7Nk9bSojV35PNs1Ztb5pT";
        
        teamMintId = 1;
        earlyListMintId = 101;
        whiteListMintId = 8101;
        publicMintId = 28101;
    }
    ...
    modifier canWhiteListMint() {
        ...
        require(
            whiteListMintId <= MAX_WHITELIST_MINT,
    ...
    /**
     * @dev mints 1 token per internal team member, does not charge a fee
     * @return mintId tokenId minted
     */
    function mintTeam(bytes32[] calldata _merkleProof)
        external
        isValidMerkleProof(_merkleProof, teamMerkleRoot)
        isNotPaused
        canTeamMint
    ...
    /**
     * @notice mints 1 token per early member address, charge a fee
     * @return mintId tokenId minted
     */
    function mintEarly(bytes32[] calldata _merkleProof)
        external
        payable
        isValidMerkleProof(_merkleProof, earlyMerkleRoot)
        isNotPaused
        canEarlyListMint
        nonReentrant
        returns (uint256 mintId)
    {
        mintId = earlyListMintId;
        mint(mintId);
        earlyListMintId++;
    }
    ...
    function mintWhiteList(bytes32[] calldata _merkleProof)
        ...
        returns (uint256 mintId)
    {
        mintId = whiteListMintId;
        mint(mintId);
        whiteListMintId++;
    }

    /**
     * @dev Public mint 1 token, charge a fee
     */
    function mintPublic()
        external
        payable
    ...
    function mint(uint256 _tokenId) internal {
        ...
        bowls[_tokenId] = true;
        bowlToOwner[_tokenId] = msg.sender;
    }
        ...
        isTokenExist(_tokenId)
    {
        ...
        anomuraContract.mintAnomura{value: msg.value}(msg.sender, _tokenId);
    }
    ...
    */
        ...
        returns (string memory)
    {
        ...
        string memory json = Base64Upgradeable.encode(
            bytes(
                string(abi.encodePacked("{\"name\": \"Bowl #", StringsUpgradeable.toString(_tokenId), "\", \"image\":\"", bowlImage, "\"}"))
                )
        );
    ...
    */
    ...
    {
        require(_owner != address(0), "Cannot query address 0");
    ...
    */
    function setMaxMultiplier(uint256 _multiplier) external onlyOwner {
        require(_multiplier >= 0, "Multiplier cannot be less than 0");
        maxMultiplier = _multiplier;
    }

    /**
    @notice Manual set a new merkle root for teamMerkleRoot
    @param _merkleRoot new merkle root
    */
    function setTeamListMerkleRoot(bytes32 _merkleRoot) external onlyOwner {
        teamMerkleRoot = _merkleRoot;
    }

    /**
    @notice Manual set a new merkle root for earlyMerkleRoot
    @param _merkleRoot new merkle root
    */
    function setEarlyListMerkleRoot(bytes32 _merkleRoot) external onlyOwner {
        earlyMerkleRoot = _merkleRoot;
    }
