...
contract LoanStorage is Ownable {
    ...
    enum LoanStorageEvent {
        CREATED,                            // 0
        UPDATED_STATUS,                     // 1
        UPDATED_ACTIVATION_DATE,            // 2
        UPDATED_CALCULATED_AMOUNT,          // 3
        UPDATED_CALCULATED_INTEREST,        // 4
        UPDATED_FRACTIONALIZED_TOKEN_ID,    // 5
        UPDATED_COLLATERAL_NFT_TOKEN_ID,    // 6
        CANCELLED                           // 7
    }

    /// @notice loan borrower event
    event LoanStorageActivity(uint256 indexed loanId, LoanStorageEvent activity, uint256 timestamp);

    modifier onlyServiceContractOrOwner() {
        address loanServiceAddress = LoanOracle_I(getLoanOracleAddress()).getLoanServiceAddress();
        require(
            msg.sender == loanServiceAddress || msg.sender == owner(),
    ...
    mapping(uint256 => LoanCommon.Loan) public loanMapping;
    ...
    /// @dev addressToLoanIdMapping[address] => uint256[] loan IDs
    mapping(address => uint256[]) public addressToLoanIdMapping;

    /// @notice Convenience method for getting loan ids by address
    /// @param _ownerAddress The address of the loan owner
    /// @return a list of loan IDs
    ...
    /// @param _borrower The address of the loan borrower
    /// @param _amount The amount that the borrower would like to borrow
    /// @param _duration The duration in days that the borrower would like the loan to last
    function create(
        address _borrower,
        uint256 _amount,
        uint256 _duration
    ) public onlyServiceContractOrOwner returns (uint256) {

        //////////////////
        // Create Loan
        //////////////////
        loanIdTracker++;
        loanMapping[loanIdTracker] = LoanCommon.Loan({
            ...
            activationDate: 0,
            status: LoanCommon.LoanStatus.RECRUITING,
            calculatedAmount: 0,
        ...
        ////////////////////
        emit LoanStorageActivity(loanIdTracker, LoanStorageEvent.CREATED, block.timestamp);

        return loanIdTracker;
    }

    /// @notice convenient method to update the loan status
    /// @param _loanId The Loan ID
    /// @param _status The new status of the loan
    function updateStatus(uint256 _loanId, LoanCommon.LoanStatus _status) public onlyServiceContractOrOwner {
        LoanCommon.Loan storage loan = loanMapping[_loanId];
        loan.status = _status;
        emit LoanStorageActivity(_loanId, LoanStorageEvent.UPDATED_STATUS, block.timestamp);
    }

    /// @notice convenient method to update the loan activation date
    /// @param _loanId The Loan ID
    /// @param _timestamp The timestamp
    /// @dev meant to be set when loan is activated
    function updateLoanActivationDate(uint256 _loanId, uint256 _timestamp) public onlyServiceContractOrOwner {
        LoanCommon.Loan storage loan = loanMapping[_loanId];
        loan.activationDate = _timestamp;
        emit LoanStorageActivity(_loanId, LoanStorageEvent.UPDATED_ACTIVATION_DATE, block.timestamp);
    }

    /// @notice convenient method to update the calculated interest
    /// @param _loanId The Loan ID
    /// @param _calculatedInterest The calculated interest
    function updateCalculatedInterest(uint256 _loanId, uint256 _calculatedInterest) public onlyServiceContractOrOwner {
        LoanCommon.Loan storage loan = loanMapping[_loanId];
        loan.calculatedInterest = _calculatedInterest;
        emit LoanStorageActivity(_loanId, LoanStorageEvent.UPDATED_CALCULATED_INTEREST, block.timestamp);
    }

    /// @notice convenient method to update the calculated interest
    /// @param _loanId The Loan ID
    /// @param _calculatedAmount The calculated amount
    function updateCalculatedAmount(uint256 _loanId, uint256 _calculatedAmount) public onlyServiceContractOrOwner {
        LoanCommon.Loan storage loan = loanMapping[_loanId];
        loan.calculatedAmount = _calculatedAmount;
        emit LoanStorageActivity(_loanId, LoanStorageEvent.UPDATED_CALCULATED_AMOUNT, block.timestamp);
    }

    /// @notice convenient method to update the fractionalized token id
    /// @param _loanId The Loan ID
    /// @param _fractionalizedTokenId The fractionalized token Id
    function updateFractionalizedTokenId(uint256 _loanId, uint256 _fractionalizedTokenId) public onlyServiceContractOrOwner {
        LoanCommon.Loan storage loan = loanMapping[_loanId];
        loan.fractionalizedTokenId = _fractionalizedTokenId;
        emit LoanStorageActivity(_loanId, LoanStorageEvent.UPDATED_FRACTIONALIZED_TOKEN_ID, block.timestamp);
    }

    /// @notice convenient method to update the fractionalized token id
    /// @param _loanId The Loan ID
    /// @param _loanCollateralNftTokenId The loan collateral token Id
    function updateLoanCollateralNftTokenId(uint256 _loanId, uint256 _loanCollateralNftTokenId) public onlyServiceContractOrOwner {
        LoanCommon.Loan storage loan = loanMapping[_loanId];
        loan.loanCollateralNftTokenId = _loanCollateralNftTokenId;
        emit LoanStorageActivity(_loanId, LoanStorageEvent.UPDATED_COLLATERAL_NFT_TOKEN_ID, block.timestamp);
    }

    /// @notice cancel a loan that is current in recruiting phase
    /// @param _loanId The loan id
    function cancelLoan(uint256 _loanId) public onlyServiceContractOrOwner {
        updateStatus(_loanId, LoanCommon.LoanStatus.CANCELLED);
        emit LoanStorageActivity(_loanId, LoanStorageEvent.CANCELLED, block.timestamp);
    }
}
