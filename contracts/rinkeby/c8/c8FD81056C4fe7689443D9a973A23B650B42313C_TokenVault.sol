...
contract TokenVault is Ownable, Pausable {
    ...
    IStaking public immutable masterchef;
    ...
    uint256 public emergencyBalance = 0;
    ...
    constructor(IERC20 _token, IStaking _masterchef) {
        token = _token;
        masterchef = _masterchef;
        allowance = type(uint256).max;
        IERC20(_token).approve(address(_masterchef), allowance);
    }
    ...
    function deposit(uint256 _amount) external whenNotPaused notContract {
        ...
        uint256 poolReward = masterchef.pendingReward(PID, address(this));
        ...
        if (totalShares != 0) {
            ...
            // console.log(
            //     "-ap-currentShares =  amount(%s) * totalShares(%s) / totalPoolAmount ",
            //     _amount,
            //     totalShares,
            //     totalPoolAmount
            // );
        ...
        // console.log("-ap-Deposit to staking during deposit: ", amountToStake);
        _checkAllowance(amountToStake);

        masterchef.deposit(PID, amountToStake);
    ...
    function _checkAllowance(uint256 _deposit) internal {
        if (allowance >= _deposit) {
            ...
            allowance = type(uint256).max;
            token.approve(address(masterchef), allowance);
        }
    }
    ...
    function emergencyWithdraw() external onlyOwner whenPaused {
        ...
        emergencyBalance = token.balanceOf(address(this));
    }
    ...
    function emergencyUserWithdraw() external notContract whenPaused {
        ...
        uint256 currentAmountToWithdraw = (emergencyBalance * user.shares) /
            totalShares;
        // uint256 stakingBalance = token.balanceOf(address(this));
        // uint256 currentAmountToWithdraw = (stakingBalance * user.shares) /
        //     totalShares;
        ...
        emit EmergencyWithdraw(sender, currentAmountToWithdraw, user.shares);
    }
    ...
    function calculateHarvestRewards() external view returns (uint256) {
        uint256 amount = IStaking(masterchef).pendingReward(PID, address(this));

        // amount = amount.add(available());
        // TODO check callFee = 0
    ...
    function calculateTotalPendingRewards() external view returns (uint256) {
        return IStaking(masterchef).pendingReward(PID, address(this));
    }
    ...
    function calculateCurrentBalance(address _of)
        ...
        returns (uint256)
    {
        require(totalShares > 0, "No balance now");
        uint256 pending = (balanceOf() * userInfo[_of].shares) / (totalShares);

        return pending;
    }

    function getPricePerFullShare() external view returns (uint256) {
        //                 if=0, then 1e18 else ratio
    ...
    function withdrawAll() external notContract {
        withdrawShares(userInfo[msg.sender].shares);
    }
    ...
    function withdrawShares(uint256 _shares) public notContract whenNotPaused {
        ...
        masterchef.withdraw(PID, currentAmountToWithdraw);
        token.transfer(sender, currentAmountToWithdraw);
    ...
    function total() internal view returns (uint256) {
        ...
        amount += masterchef.pendingReward(PID, address(this));
        return amount;
    }
    ...
    function _earn(bool _fromHarvest) internal {
        // console.log("-ap-I am earning");
        address sender = msg.sender;
        uint256 pendingReward = masterchef.pendingReward(PID, address(this));

        if (pendingReward != 0) {
            masterchef.withdraw(PID, 0);

            console.log("-ap-stakingReward is: ", pendingReward);
            // if it's harvest, give reward to caller
            if (_fromHarvest) {
                uint256 callerFee = ((pendingReward) * callFee) / PERCENT_BASE;
                pendingReward -= callerFee;
                // console.log("-ap-Caller reward: ", callerFee);
                token.transfer(sender, callerFee);
                emit Harvest(sender, callerFee);
            }
            // console.log("-ap-Deposit to staking: ", withRewardBalance);
            _checkAllowance(pendingReward);
            masterchef.deposit(PID, pendingReward);
        }
    }
...
interface IStaking {
    ...
    // function addPool(address _token, uint256 _allocPoint, bool _withUpdate) external;
    ...
    // function getFees(IERC20[] calldata tokens) external;
    ...
    // function _mintGRAIL(uint256 grailAmount) private;
