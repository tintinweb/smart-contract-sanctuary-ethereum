...
import "../interfaces/IModusStaking.sol";
    ...
    HashFunction
{
    ...
    IModusStaking modusStaking;
    IPXT pxt;
    uint256 public projectCap;
    uint256 public investedAmount;
    uint256 public premiumTimeframe;
    bool public tokenSale;
    uint256 saleInitiatedOn;
    bool isSaleInitiated;
    struct investor {
        bool isPremium;
    ...
    mapping(uint256 => bool) public doProofIdExist;
    ...
    function initialize(
        ...
        projectCap = _projectCap;
        premiumTimeframe = _premiumTimeframe * 86400; //convert days into seconds
        utilInitialize(_modusRegistry);
        hashInitialize(_projectId, proofVerifier);
        modusStaking = IModusStaking(modusRegistry.modusStaking());
    }
    ...
    /// @notice function to set the premium investors one by one
    function setPremiumInvestor(
        address _investor,
        bool status,
        uint256 amount
    ) external onlyModusAdmin {
        investorDetails[_investor] = investor(status, amount);
    }

    /// @notice function to set the premium investors group
    function setPremiumInvestorsList(
        ...
        for (uint256 i = 0; i < _investor.length; i++)
            investorDetails[_investor[i]] = investor(status[i], amount[i]);
    }

    ///@notice function to extend premium timeframe by passing the number of days
    ///@param _days, number of days to extend premium timeframe
    function extendPremiumTimeFrame(uint256 _days) external onlyModusAdmin {
        premiumTimeframe = premiumTimeframe + _days * 86400;
    }

    /// @notice function to initiate sale to enable investors to invest
    function initiateSale() external onlyModusFactory {
        require(isSaleInitiated == false, "StableCoinPXTSwap: Already Initiated");
        toggleSale();
        saleInitiatedOn = block.timestamp;
        isSaleInitiated = true;
    }

    ///@notice function to start and stop token sale
    function toggleSale() public onlyModusAdminorModusFactory {
        if (tokenSale) tokenSale = false;
        else tokenSale = true;
    }

    ///@notice function to end the sale when the emission starts
    function endSale() external onlyModusFactory {
        tokenSale = false;
    }

    ///@notice function to allow investors to swap the stablecoin for the PXT and stake it in the PXTStaking contract
    function swap(
        uint256 amount,
        uint256 proofId,
        bytes memory sig
    ) external {
        bool blackListStatus = modusRegistry.isAccountBlackListed(msg.sender);
        require(
            blackListStatus == false,
            "StableCoinPXTSwap: Address Blacklisted"
        );
        bool tierStatus = modusStaking.isTierVersionUpdated(msg.sender);
        require(tierStatus, "StableCoinPXTSwap: Tier needs to be updated");
        require(tokenSale, "StableCoinPXTSwap: Sale yet to Begin");
        // console.log(
        //     "isProofIdExist[proofId]",
        //     doProofIdExist[proofId]
        // );
        require(
            doProofIdExist[proofId] == false,
            "StableCoinPXTSwap: Id already used"
        );
        //verifySignature(msg.sender,amount,proofId,sig);
        isMessageValid(msg.sender, amount, proofId, sig);
        doProofIdExist[proofId] = true;
        require(
            investedAmount + amount <= projectCap,
            "StableCoinPXTSwap: Investment exceeds the required Capital"
        );
        address pxtStaking = modusRegistry
            .getProjectDetails(projectId)
            ._pxtStaking;
        uint256 cumulatedAmount = IPXTStaking(pxtStaking)
            .investorDetails(msg.sender)
            .stakedAmount + amount;
        if (block.timestamp <= saleInitiatedOn + premiumTimeframe) {
            require(
                investorDetails[msg.sender].isPremium,
                "StableCoinPXTSwap: Not a Premium Investor"
            );
            require(
                cumulatedAmount <=
                    investorDetails[msg.sender].maximumStakeAmount,
                "StableCoinPXTSwap: Exceeds Maximum stake amount"
            );
        } else {
            uint256 tier = modusStaking.getTierLevel(msg.sender);
            require(
                cumulatedAmount <=
                    modusStaking.maxCoins(
                        tier,
                        modusRegistry.projectStableCoin(projectId)
                    ),
                "StableCoinPXTSwap: Exceeds Maximum Stake"
            );
        }
        IERC20(modusRegistry.projectStableCoin(projectId)).safeTransferFrom(
            msg.sender,
            modusRegistry.getProjectDetails(projectId).custodianWallet,
            amount
        );
        pxt = IPXT(modusRegistry.getProjectDetails(projectId)._pxt);
        pxt.mint(pxtStaking, amount);
        IPXTStaking(pxtStaking).stake(msg.sender, amount);
...
interface IModusStaking {
    function totalStakedBalance() external view;

    // function maxCoinsPerTier() external view;
    struct Tier {
    ...
    function modusTierVersion() external view;

    function increaseTierVersion() external;

    function tierList() external view;

    function toggleStakePause() external;

    function setTier(uint256, uint256) external;
    ...
    function maxCoins(uint256, address) external returns (uint256);

    // function setTierDetails(tierParams[] memory _tierList) external;

    function stake(uint256) external returns (uint256, uint256);

    function requestForUnstake(uint256) external;

    function unStake() external returns (uint256, uint256);

    function updateUserTier() external;

    function isTierVersionUpdated(address investor) external view returns(bool);

    function getStakeBalance(address) external view returns (uint256);

    function getTierLevel(address) external view returns (uint256);

    function getInvestorsInTier(uint256) external view returns (uint256);
}
