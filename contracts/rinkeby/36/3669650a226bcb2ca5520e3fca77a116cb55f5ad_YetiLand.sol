...
contract YetiLand is ERC721, Ownable, ReentrancyGuard {
    ...
    function mint(uint _mintAmount) public payable {
        ...
        require(_reserveTokenId + _mintAmount <= (maxSupply - maxClaimLandSupply), "Max public supply exceeded");
        if (isYetiHolder[msg.sender] == true) {
    ...
    function claimLand(uint _claimAmount) public {
    ...
    function superYeti(address[] calldata _users) public onlyOwner {
        for (uint i = 0; i < _users.length; i++) {
            isYetiHolder[_users[i]] = true;
        }
    }
    function unSuperYeti(address[] calldata _users) public onlyOwner {
        for(uint i = 0; i < _users.length; i++) {
            isYetiHolder[_users[i]] = false;
        }
    }
