...
contract DigitalSignERC721A is ERC721A, Ownable {
    ...
    //Constructor
    constructor(string memory _initBaseURI) ERC721A("ITSUKI", "ITSK") {
    ...
    function mintNFT(uint256 _count) external payable {
        ...
        require(normalSaleIsActive, "Sale not active");
        require(supply.add(_count) <= maxSupply, "Supply Limit Reached");
        // require(!paused, "Another Transaction in Progress");
        if (msg.sender != owner()) {
            //general public
            require(msg.value >= cost.mul(_count), "Not Enough Tokens");
        }
    ...
    function preSale(
        ...
        require(supply.add(_count) <= maxSupply, "Not enough NFTs left!");
        require(
    ...
    //only owner
    function setCost(uint64 _newCost) external onlyOwner {
        cost = _newCost;
    }
    ...
    function setPreSaleMaxMint(uint128 _num) external onlyOwner {
        preSaleMaxMint = _num;
    }
