...
contract DemoNFT is ERC721, ERC721Enumerable {
    using Strings for uint256;

    // 0 - is not active, 1 - presale, 2 - public sale
    ...
    // TODO: set price
    uint256 public price = 0.3 ether; // 300000000000000000 wei 
    // TODO set max supply
    uint256 public maxSupply = 200;

    // TODO: set max per address
    uint256 public mintPerAddress = 2;
    ...
    function mint(bytes32[] memory _proof) public payable {
        require(state != 0, "Minting not active");
        require(msg.value == price, "Incorrect ether value");
        
        uint256 supply = totalSupply();
        require(supply < maxSupply, "NFT limit was reached");
        
        require(mintedPerAddress[msg.sender] < mintPerAddress, "This address has reached the mint limit");

        if (state == 1) {
            require(MerkleProof.verify(_proof, root, keccak256(abi.encodePacked(msg.sender))), 
            "Only available for premium minters during presale!");
        }
    ...
    function mintByOwner(uint256 _amount) public onlyOwners {
        require(state != 0, "Minting not active");

        uint256 supply = totalSupply();
        require(supply + _amount < maxSupply, "NFT limit was reached");
    ...
    // =========== Owner settings ===========
    // Setting Root for presale whitelist
    ...
    function changeMintPerAddress(uint256 _limit) public onlyOwners {
        mintPerAddress = _limit;
    }
