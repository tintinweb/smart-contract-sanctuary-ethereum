...
contract NewWaveSurfClub is Ownable, ERC721A {
    ...
    {
        _baseURIExtended = "ipfs://QmRf7rK2MvFrfDyGDGunxz2KwFJuJvSRrUxkUdDjSuFFPS";
    ...
    function mint(uint _amount) external payable noReentrant {
        ...
        if (_isPreSaleLive) {
            require(accounts[msg.sender].isWhitelist, 'Error: You need to be on Whitelist.');
            require((_amount) <= 1, 'Error: Presale max per transaction exceeded.');           
        } else if (_isSaleLive) {
            require((_amount) <= 2, 'Error: Public sale max per transaction exceeded.');
        }
