...
contract Lottery {
    ...
    function enter() public payable  {
        require(status == STATUS.OPEN , "cannot enter now.");

        require(msg.value == entranceFee * 10**18, "wrong entrance fee.");
    ...
    function startLottery(uint256 fee) public onlyOwner {
        require(status == STATUS.CLOSED , "already open.");
    ...
    // 4. 把合約內所有的ETH傳給贏家
        payable(recentWinner).transfer(total);
        entranceFee = 0;
        total = 0;
