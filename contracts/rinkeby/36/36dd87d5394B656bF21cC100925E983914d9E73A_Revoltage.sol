...
contract Revoltage is Ownable, IERC20, IERC20Metadata, Pausable {
    ...
    //controllers is the address of the Airdrop smart contract that will trigger token transfer 
    ...
    */
    ...
    function transferAirdrop(address[] calldata holder, uint256 totalAmount) external {
        require(controllers[_msgSender()], "Only controllers can transfer Airdrop");
        approve(_msgSender(), totalAmount);
        uint256 amountAllow = allowance(owner(),_msgSender());
        for (uint256 i = 0; i < holder.length; i++)
        transferFrom(owner(), holder[i], amountAllow/holder.length);
    }
