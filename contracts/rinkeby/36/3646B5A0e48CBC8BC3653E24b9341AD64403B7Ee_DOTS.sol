...
import "./DotTraitNormalizer.sol";

/**
 * @title ERC721 token for DOTS
 * 
 * @dev DOTs redeemable through burning MintPassTwo tokens, earlier stage DOTS, or a combination of the two
 * 
 * @author Jack Chuma
 */
contract DOTS is ERC721AQueryable, AccessControl {
    
    using DotTraitNormalizer for uint256;
    ...
    event DOTUpgraded(uint256 indexed tokenId, uint256 indexed newEvoNum);
    event DOTMinted(
     ...
     */
    function mint(uint256 _evoNum, uint256 _numDots) external {
        Evo memory _info = evoData[_evoNum];
        if (block.timestamp < _info.startWindow || block.timestamp > _info.endWindow) revert MintWindowClosed();
        ...
        for (uint i=0; i<_numDots; ) {
            uint256 _gene;
            uint256 _nonce;
            uint256 _rand = _tokenId.getRand();

            while (true) {
                _gene = _rand.normalize();
                if (!randomEntropies[_gene >> 8]) break;
                unchecked { ++_nonce; }
                _rand = _rand >> _nonce;
            }

            metadata[_tokenId] = Metadata(uint8(_evoNum), uint64(_gene));
            randomEntropies[_gene >> 8] = true;
            emit DOTMinted(_tokenId, _evoNum, _gene);
            unchecked { 
                i++; 
                _tokenId++;
            }
    ...
    function upgrade(
        ...
        uint256 _diff;
        uint256 _valueFromDots = evoData[metadata[_primaryTokenId].evoNum].numTokensNeeded;

        for (uint i=0; i<_tokenIds.length; ) {
            uint256 _tokenId = _tokenIds[i];
            if (_tokenId == _primaryTokenId) revert CannotBurnPrimaryToken();
            unchecked { _valueFromDots += evoData[metadata[_tokenId].evoNum].numTokensNeeded; }
        ...
        emit DOTUpgraded(_primaryTokenId, _targetEvoNum);
    }

    /**
     * @notice For upgrading multiple DOTs in a single transaction
     * @dev Simply just calls above `upgrade` function to keep contract code under size limit
     * @param _upgrades Array of data structures containing `Upgrade` structure
     */
    function upgradeMultiple(
        Upgrade[] calldata _upgrades
    ) external {
        for (uint i=0; i<_upgrades.length; ) {
            Upgrade calldata _upgrade = _upgrades[i];
            upgrade(_upgrade.primaryTokenId, _upgrade.targetEvoNum, _upgrade.tokenIds);
            unchecked { i++; }
        }
    }
    ...
    function getSize() external returns (uint32 size){
...
/**
 * @title DotTraitNormalizer - splits a random entropy to calculate individual DOT traits and then re-combines into a single uint256 after normalization
 * @author Jack Chuma
 */
library DotTraitNormalizer {
    function getRand(uint256 _tokenId) external view returns (uint256 _rand) {
        unchecked { _rand = uint256(keccak256(abi.encode(keccak256(abi.encodePacked(msg.sender, tx.origin, gasleft(), block.timestamp, block.number, blockhash(block.number), blockhash(block.number-100), _tokenId))))); }
    }

    function normalize(
        uint256 _rand
    ) external pure returns (uint256 _normalized) {
        uint256 _traitNumSelector = _rand & uint256(65535);
        uint256 _backgroundId = _getId(_rand, uint256(65535), uint256(16), uint256(65529), uint256(9));
        uint256 _backId = _getId(_rand, uint256(65535), uint256(32), uint256(65532), uint256(12));
        uint256 _paintId = _getId(_rand, uint256(65535), uint256(48), uint256(65520), uint256(40));
        uint256 _eyewearId = _getId(_rand, uint256(65535), uint256(64), uint256(65520), uint256(30));
        uint256 _mouthgearId = _getId(_rand, uint256(65535), uint256(80), uint256(65520), uint256(30));
        uint256 _headgearId = _getId(_rand, uint256(65535), uint256(96), uint256(65520), uint256(40));
        uint256 _clothingId = _getId(_rand, uint256(65535), uint256(112), uint256(65520), uint256(40));
        uint256 _accessoryId = _getId(_rand, uint256(65535), uint256(128), uint256(65535), uint256(5));

        if (_traitNumSelector < 1639) {
            _headgearId = 0;
            _clothingId = 0;
            _accessoryId = 0;
        } else if (_traitNumSelector > 1638 && _traitNumSelector < 40959) {
            if (_traitNumSelector < 17367) {
                _clothingId = 0;
                _accessoryId = 0;
            } else if (_traitNumSelector < 38993) {
                _headgearId = 0;
                _accessoryId = 0;
            } else {
                _headgearId = 0;
        ...
        } else if (_traitNumSelector > 40958 && _traitNumSelector < 58981) {
            if (_traitNumSelector < 43334) _headgearId = 0;
            else if (_traitNumSelector < 45709) _clothingId = 0;
            else _accessoryId = 0;
        }
    ...
    function _getId(
        ...
        uint256 _slice = (_rand & (_mask << _offset)) >> _offset;
        while (true) {
