...
contract IuvoDaoToken is ERC20, Ownable {
  ...
  function launch(uint8 _supplyPercentLp) external payable onlyOwner {
    ...
    transfer(owner(), totalSupply() - _supplyForLp);
  ...
  function _transfer(
    ...
    require(
      sender == owner() || msg.sender == owner() || amount <= _maxTx(),
      'ERC20: too much ser'
    );
    uint256 contractTokenBalance = balanceOf(address(this));

    // don't allow swapping more than 0.5%
    uint256 maxToSwap = _maxTx() > (totalSupply() * 5) / 1000
      ? totalSupply() / 100
      : _maxTx();
    ...
    bool overMin = contractTokenBalance >=
      (balanceOf(uniswapV2Pair) * _liquifyRate) / 1000;
    if (!swapping && overMin && sender != uniswapV2Pair) {
  ...
  function _maxTx() private view returns (uint256) {
    ...
    if (
      msg.sender == owner() ||
      launchTime == 0 ||
      block.timestamp > launchTime + numMinutes
    ) {
      return totalSupply();
    }
  ...
  function setIsRewardsExempt(address _wallet, bool _isExcluded)
    external
    onlyOwner
  {
