...
contract PastelPersons is ERC721A, Ownable {
  ...
  uint16 public maxMintPerTx = 5; // for public sale
  uint16 public maxMintPerWallet = 8; 
  ...
  mapping(address => bool) private claimRedeemed; // see if spots have already been minted (claim mints)
  ...
  constructor(
    ...
    address[] memory _withdrawAddresses) // would like to set these variables in the constructor
    ERC721A(_name, _symbol)
     {
        baseTokenURI = _initBaseTokenURI; // add "ipfs://" before the copied CID and "/" at the end to separate token id# from the URI
    ...
    //  base uri
    // @dev add 'ipfs://' at the front of CID and "/" at the end to separate token id# from the URI when inputting new base token uri
    // external, this sets the base token to final ipfs link after reveal
        ...
        returns (string memory)
    {
        ...
        if(isRevealed == false) {
            return bytes(baseTokenURI).length > 0 ? string(abi.encodePacked(baseTokenURI, "hidden.json")) : "";
        }

        return bytes(baseTokenURI).length > 0 ? string(abi.encodePacked(baseTokenURI, tokenId.toString(), ".json")) : ""; 
    }

    // start token ID at 1, f(x) from erc721a contract
    ...
    function togglePublicOpen() external onlyOwner {
        publicOpen = !publicOpen; // false by default
    }

    function toggleReveal(string calldata _revealedMetadataURI) external onlyOwner {
        isRevealed = !isRevealed; // false by default, toggle will turn true
        baseTokenURI = _revealedMetadataURI; // pass in the final metadata URI, with logic to signifiy when revealed
    }
    ...
    function _verifyClaim(
        ...
        return MerkleProof.verify(_merkleProof, claimRoot, _leafNode);
    }
    ...
    function mintPrelist(
        ...
        uint256 _mintAmount
                ...
                require(
                    _verifyPrelist(_leaf(msg.sender), _merkleProof),
                    "Invalid proof"
                );
    ...
    function claim(bytes32[] calldata _merkleProof)
        external
    {
        require(claimOpen == true, "The free claim is not open");
        require(!claimRedeemed[msg.sender], "You have already claimed your mint.");
        require(
            _verifyClaim(_leaf(msg.sender), _merkleProof),
            "Invalid proof"
        );
        claimRedeemed[msg.sender] = true;
        ...
        payable 
    {
        // if (msg.sender != owner()) {
            ...
            // require((presaleCost * _mintAmount) <= msg.value, "Insuffient funds"); // <= or ==, as if they supply more they may not get a refund of excess eth supplied
            // msg.value is cost on top of tx fees/gas, cost itself will not fluctuate and msg.value is just the cost so == should be fine yes?
