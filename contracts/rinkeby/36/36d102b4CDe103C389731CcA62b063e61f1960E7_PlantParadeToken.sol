...
contract PlantParadeToken is ERC721A, WithLimitedSupply, ERC2981, Ownable {
    ...
    function mintReserveToken() external onlyOwner {
        ...
        uint[] memory tokenIds = new uint[](MAX_RESERVE_TOKEN);
        for(uint i = 0; i < MAX_RESERVE_TOKEN; i++) {
            tokenIds[i] = nextToken();
        }
        _safeMint(msg.sender, tokenIds);
     ...
     */
    function nextToken() internal override ensureAvailability returns (uint) {
        uint maxIndex = _totalSupply - totalSupply();
...
contract ERC721A is Context, ERC165, IERC721, IERC721Metadata {
     ...
     * Emits a {Transfer} event.
     */
    function _safeMint(address to, uint256[] memory tokenIds) internal virtual {
    ...
    function _mint(address to, uint256[] memory tokenIds) internal virtual {
        ...
        emit Transfer(address(0), to, tokenIds[0]);
