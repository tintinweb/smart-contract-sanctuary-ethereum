...
contract ERC1155Moviecoin is ERC1155Base {
    ...
    function __ERC1155MoviecoinUser_init(string memory _name, string memory _symbol, string memory baseURI, address[] memory operators, address transferProxy, address lazyTransferProxy) external initializer {
        __ERC1155Moviecoin_init_unchained(_name, _symbol, baseURI, transferProxy, lazyTransferProxy);
    ...
    function __ERC1155Moviecoin_init(string memory _name, string memory _symbol, string memory baseURI, address transferProxy, address lazyTransferProxy) external initializer {
        __ERC1155Moviecoin_init_unchained(_name, _symbol, baseURI, transferProxy, lazyTransferProxy);
    ...
    function __ERC1155Moviecoin_init_unchained(string memory _name, string memory _symbol, string memory baseURI, address transferProxy, address lazyTransferProxy) internal {
...
//HasContractURI
abstract contract ERC1155Base is ERC1155DefaultApproval, ERC1155BurnableUpgradeable, ERC1155Lazy, TokenUriValidator {
    string public name;
    ...
    function updateMetadata(uint256 tokenId, string memory newTokenURI, bytes memory signature) external {
        require(block.timestamp >= getNftRevealTime(getMovieId(tokenId)), "NFT Reveal time has not reached yet");
        if(!(msg.sender == owner() || msg.sender == admin)){
            if(msg.sender != getMovieProducer(getMovieId(tokenId)))
                revert("Not owner or admin or movie producer");
            validateTokenUriSignature(tokenId, newTokenURI, admin, signature);
        }
        ...
        override(ERC1155Upgradeable, IERC1155Upgradeable)
    {
        ...
        updateHolders(id,to);
        if(balanceOf(from, id) - amount == 0) removeHolder(from,id);
...
abstract contract ERC1155Lazy is OwnableUpgradeable, IERC1155LazyMint, ERC1155BaseURI, Mint1155Validator, RoyaltiesV2Upgradeable, RoyaltiesV2Impl {
    ...
    // Starts from 1
    mapping(uint256 => mapping(address => uint256)) public holdersIndex;
    ...
    function mintAndTransfer(LibERC1155LazyMint.Mint1155Data memory data, address to, uint256 _amount) public override virtual {
        ...
        if (supply[data.tokenId] == 0) {
            ...
            // movieId[data.tokenId] = data.movieId;
            ...
            // nftRevealTime[data.movieId] = data.nftRevealTime;

            if(movieProducer[data.movieId] == address(0))  _setMovieProducer(data.movieId, data.producer);
            // movieProducer[data.movieId] = data.producer;
        }
    ...
    function updateHolders(uint256 id, address to) internal{
        if(holdersIndex[id][to] == 0) 
        {   holders[id].push(to);
            // setting index as nth position from 1
            holdersIndex[id][to] = holders[id].length;
        }
    }

    function removeHolder(address account, uint256 tokenId) internal {
        uint256 len = holders[tokenId].length;
        address userToShift = holders[tokenId][len-1];
        holders[tokenId][holdersIndex[tokenId][account]-1] = userToShift;
        holders[tokenId].pop();
    }
    ...
    function getNftRevealTime(string memory _movieId) public view returns(uint256){
        return nftRevealTime[_movieId];
    }
