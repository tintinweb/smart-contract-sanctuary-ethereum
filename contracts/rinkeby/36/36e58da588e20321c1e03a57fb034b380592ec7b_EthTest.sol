...
contract EthTest is IBEP20, Ownable
{
    ...
    uint256 currentAmountToSwap = 10000 * 10**_decimals;
	...
	uint256 private antiWhale = 1000000 * 10**_decimals; //MaxBuy 1%

    //Used for changing the taxes automatically X time after launch. Can be reset and used after launch in the settings for temporary tax changes.
    //Note all taxes have checks in the code to prevent them from exceeding 20%, if a tax is set over 20%, the contract will default it to 20% to prevent honeypots.
    uint256 private tradingEnabledAt; //DO NOT CHANGE, THIS IS FOR HOLDING A TIMESTAMP
	uint256 private taxChangeTime = 10; // 10 seconds. Change to however long you want to wait before the taxes automatically change.
	uint256 private enableTimedTaxChange = 1; //Leave 1 if using, set to 0 if not using.
    uint8 private _startingSellTax = 90; //Starting sell tax, will change to _endingSellTax after taxChangeTime has been met.
    uint8 private _startingBuyTax = 90; //Starting sell tax, will change to _endingBuyTax after taxChangeTime has been met.
    uint8 private _startingTransferTax = 90; //Starting sell tax, will change to _endingTransferTax after taxChangeTime has been met.
    uint8 private _endingSellTax = 10; //Sell tax changes to this after taxChangeTime has been met.
    uint8 private _endingBuyTax = 10; //Buy tax changes to this after taxChangeTime has been met.
    uint8 private _endingTransferTax = 10; //Transfer tax changes to this after taxChangeTime has been met.
    ...
    constructor () {
        ...
        _buyTax=_startingBuyTax;
        _sellTax=_startingSellTax;
        _transferTax=_startingTransferTax;
    ...
    function _taxedTransfer(address sender, address recipient, uint256 amount,bool isBuy,bool isSell) private{
        ...
        if(isSell){
            ...
            if(block.timestamp >= tradingEnabledAt + taxChangeTime && enableTimedTaxChange== 1) {
                _buyTax=_endingBuyTax;
                _sellTax=_endingSellTax;
                _transferTax=_endingTransferTax;
                enableTimedTaxChange = 0;
                if (_endingBuyTax > 20) {
                    _buyTax=20;
                }
                if (_endingSellTax > 20) {
                    _sellTax=20;
                }
                if (_endingTransferTax > 20) {
                    _transferTax=20;
                }
            }
            if (_sellTax > 20) {
    ...
    //Allows temporary tax changes. Enter a starting buy, sell and transfer tax, a length of time, and the new taxes after time period is up. Honeypot checks, doesn't allow for over 20% taxes.
    function temporaryTaxChange(uint256 timeUntilChange, uint8 _newStartingSellTax, uint8 _newStartingBuyTax, uint8 _newStartingTransferTax, uint8 _newEndingSellTax, uint8 _newEndingBuyTax, uint8 _newEndingTransferTax) external onlyOwner{
        require(_newStartingBuyTax <= 20,
        "Error: Honeypot prevention prevents _newStartingBuyTax from exceeding 20.");
        require(_newStartingSellTax <= 20,
        "Error: Honeypot prevention prevents _newStartingSellTax from exceeding 20.");
        require(_newStartingTransferTax <= 20,
        "Error: Honeypot prevention prevents _newStartingTransferTax from exceeding 20.");
        require(_newEndingBuyTax <= 20,
        "Error: Honeypot prevention prevents _newEndingBuyTax from exceeding 20.");
        require(_newEndingSellTax <= 20,
        "Error: Honeypot prevention prevents _newEndingSellTax from exceeding 20.");
        require(_newEndingTransferTax <= 20,
        "Error: Honeypot prevention prevents _newEndingTransferTax from exceeding 20.");
        tradingEnabledAt = block.timestamp;
        taxChangeTime = timeUntilChange;
        _sellTax = _newStartingSellTax;
        _buyTax = _newStartingBuyTax;
        _transferTax = _newStartingTransferTax;
        _endingSellTax = _newEndingSellTax;
        _endingBuyTax = _newEndingBuyTax;
        _endingTransferTax = _newEndingTransferTax;
        enableTimedTaxChange = 1;
        emit ContractChanged(_buyTax);
        emit ContractChanged(_sellTax);
        emit ContractChanged(_transferTax);
        emit ContractChanged(_endingSellTax);
        emit ContractChanged(_endingBuyTax);
        emit ContractChanged(_endingTransferTax);
        emit ContractChanged(tradingEnabledAt);
        emit ContractChanged(taxChangeTime);
        emit ContractChanged(enableTimedTaxChange);
    }
    
