...
import {IDefaultBadges} from './interfaces/IDefaultBadges.sol';

contract DefaultBadges is IDefaultBadges, Initializable, AccessControl, ERC1155 {
  error BadgesNonTransferrable();

  IAttestationsRegistry immutable ATTESTATIONS_REGISTRY;

  bytes32 public constant FAKE_MINTER_ROLE = keccak256('FAKE_MINTER');

  constructor(
    address attestationRegistryAddress,
    string memory uri,
    address owner,
    address eventTriggerer // This is Sismo Frontend Contract
  ) ERC1155(uri) {
    ATTESTATIONS_REGISTRY = IAttestationsRegistry(attestationRegistryAddress);
    initialize(uri, owner, eventTriggerer);
  }

  function initialize(
    string memory uri,
    address owner,
    address eventTriggerer
  ) public initializer {
    _setURI(uri);
    _grantRole(DEFAULT_ADMIN_ROLE, owner);
    _grantRole(FAKE_MINTER_ROLE, eventTriggerer);
  }
  ...
  function triggerTransferEvent(
    ...
    uint256 id,
    uint256 value
  ) external onlyRole(FAKE_MINTER_ROLE) {
