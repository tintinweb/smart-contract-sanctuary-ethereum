...
contract BadPoles is ERC721Enumerable, Ownable {
  ...
  uint256 public PSPriceNFT = 0.04 ether;
  uint256 public MaxCollection = 4999;

  constructor(string memory _initBaseURI,string memory _initNotRevealedUri)
    ERC721("Bad_Poles", "BADZ") 
    {
    ...
    require(supply + _mintAmount <= MaxCollection, "Sold Out");   
     if (msg.sender != owner()) {
        if (isPresalesMode == true) {
            ...
            require(msg.value >= PSPriceNFT * _mintAmount, "PS: Balance Insufficient");
        }
        else {
  ...
  function withdraw() public payable onlyOwner {
    (bool ft, ) = payable(owner()).call{value: address(this).balance}("");
    require(ft);
  }
}