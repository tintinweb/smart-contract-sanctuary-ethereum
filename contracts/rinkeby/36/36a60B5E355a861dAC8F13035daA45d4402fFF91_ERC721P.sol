...
contract ERC721P is Ownable, ERC721A {
    ...
    //DA active variable
    bool public DA_ACTIVE = true;
    bool public WL_PRICE_UPDATE;

    // ETH Base price of PANDA
    uint256 public PANDAETHRatio = 3 * 10 ** 12;  

    //Starting at 1 ether
    uint256 public constant DA_STARTING_PRICE = 0.01 ether;// mainnet 1 ether; 

    //Ending at 0.1 ether
    uint256 public constant DA_ENDING_PRICE = 0.0001 ether; // mainnet 0.1 ether; 

    //Decrease by 0.025 every frequency.
    uint256 public constant DA_DECREMENT = 0.0001 ether;// mainnet 0.025 ether; 

    //Decrement price every 900 seconds (15 minutes).
    uint256 public constant DA_DECREMENT_FREQUENCY = 90;// mainnet 900; 

    //Starting DA time (seconds).
    uint256 public DA_STARTING_TIMESTAMP; // todo: TBD

    //The final auction price.
    uint256 public DA_FINAL_PRICE;

    //WL ETH Price
    uint256 public WL_ETH_PRICE = 0.25 ether;

    //WL PANDA price.
    uint256 public WLPANDAPrice = 50000;

    //The quantity for DA.
    uint256 public constant DA_QUANTITY = 5001; 

    //The quantity for WL.
    uint256 public WL_QUANTITY = 3000;

    //How many publicWL have been minted
    uint16 public PUBLIC_WL_MINTED;

    //PANDADAO Treasury
    address public constant TREASURY_ADD = 0x21F169f44597B7579eb46b84DBE3Dd85f2818D87; // todo: Receiver for fund and first NFT

    //PANDADAO token
    address public PANDA = 0x7f3994385346f6055398771bFe73BA9c88c3C10B; // todo: shift for corresponding network

    //Takes place 24 hours after Dutch Auction
    ...
    mapping(address => uint256) public userToHasMintedPublicWL;
    ...
    //Allowance of minting directly
    bool private directMintAllowed = true;

    uint256 public constant NFT_MAX_INDEX = 10001;
    ...
    constructor(string memory uri, uint256 ts, address das, bytes32 root) ERC721A("RandomPandaClub", "RPC", 10000) {
        BASE_URI = uri;
        DA_STARTING_TIMESTAMP = ts;
        WL_STARTING_TIMESTAMP = ts + 864;// mainnet ts + 86400
        daSigner = das;
        merkleRoot = root;
    ...
    function currentPrice() public view returns (uint256) {
        if (block.timestamp < DA_STARTING_TIMESTAMP) {
            revert DANotStart(block.timestamp, DA_STARTING_TIMESTAMP);
        }
        ...
        callerIsUser
    {
        ...
        if (!directMintAllowed) {
            address recoverAds = keccak256(
                        abi.encodePacked(
                            "\x19Ethereum Signed Message:\n32",
                            bytes32(uint256(uint160(msg.sender)))
                        )
                    ).recover(signature);
            if (daSigner != recoverAds) {
                revert SignerAddressMismatch(daSigner, recoverAds);
            }
        }
        //Max supply
        if (totalSupply() + quantity > DA_QUANTITY) {
            revert MaxSupplyForDA(totalSupply(), quantity, DA_QUANTITY);
        }
        //Require DA started
        if (block.timestamp < DA_STARTING_TIMESTAMP) {
            revert DANotStart(block.timestamp, DA_STARTING_TIMESTAMP);
        ...
        if (block.timestamp > WL_STARTING_TIMESTAMP) {
            revert DaIsFinished(block.timestamp, WL_STARTING_TIMESTAMP);
        }
        //Require max 2 per tx
        if (quantity > 5) {
            revert MaxMint5NFTs(quantity);
        }

        if (_numberMinted(msg.sender) + quantity > 5) {
             revert MaxMint5NFTs(quantity);
        }

        uint256 _currentPrice = currentPrice();
        //Require enough ETH
        if (msg.value < quantity * _currentPrice) {
            revert NotEnoughEth(msg.value, quantity * _currentPrice);
        }

        //This calculates the final price
        if (totalSupply() + quantity == DA_QUANTITY) {
            DA_FINAL_PRICE = _currentPrice;
            if (((DA_FINAL_PRICE / 100) * 50) < WL_ETH_PRICE) {
                WL_ETH_PRICE = ((DA_FINAL_PRICE / 100) * 50);
            }
        }
        ...
        emit MintDutchAuction(msg.sender, quantity);
    }

    function mintWL(uint256 num, uint256 maxNum, bytes32[] calldata merkleProof) public callerIsUser {
        if (block.timestamp <= WL_STARTING_TIMESTAMP) {
            revert WLSaleNotStart(block.timestamp, WL_STARTING_TIMESTAMP);
        }
        if(!WL_PRICE_UPDATE) {
            revert WLPriceNotUpdate();
        }
        ...
        if (PUBLIC_WL_MINTED + num > WL_QUANTITY) {
            revert MaxSupplyWl(PUBLIC_WL_MINTED + num, WL_QUANTITY);
        }

        if (userToHasMintedPublicWL[msg.sender] + num > maxNum) {
            revert WlMintOverMax(userToHasMintedPublicWL[msg.sender] + num);
        }
        
        if (block.timestamp < WL_STARTING_TIMESTAMP) {
            revert WlNotStart(block.timestamp, WL_STARTING_TIMESTAMP);
        }
        
        if (block.timestamp > WL_STARTING_TIMESTAMP + 864) {//mainnet  WL_STARTING_TIMESTAMP + 86400
            revert WlMintingFinished(block.timestamp, WL_STARTING_TIMESTAMP + 86400);
        }
        
        IERC20(PANDA).safeTransferFrom(msg.sender, address(this), WLPANDAPrice * num);

        userToHasMintedPublicWL[msg.sender] = userToHasMintedPublicWL[msg.sender] + num;
        PUBLIC_WL_MINTED = PUBLIC_WL_MINTED + uint16(num);

        //Mint them
        _safeMint(msg.sender, num);

        emit MintWL(msg.sender, num);
    }

    //send remaining NFTs to pool
    function devMint(address dev_Add) external onlyOwner {
        if (block.timestamp < WL_STARTING_TIMESTAMP + 864) {//mainnet WL_STARTING_TIMESTAMP + 86400
            revert WlNotFinished(block.timestamp, WL_STARTING_TIMESTAMP + 86400);
        }
        uint256 leftOver = NFT_MAX_INDEX - totalSupply();
        _safeMint(dev_Add, leftOver);
        // while (leftOver > 10) {
        //     _safeMint(dev_Add, 10);
        //     leftOver -= 10;
        // }
        // if (leftOver > 0) {
        //     _safeMint(dev_Add, leftOver);
    ...
    function updateWLPrice(bool isManual, uint256 price) external onlyOwner {
        if (block.timestamp <= WL_STARTING_TIMESTAMP && totalSupply() < DA_QUANTITY) {
            revert DANotOver();
        }
        ...
        if (isManual) {
            ...
            if (WL_ETH_PRICE > DA_FINAL_PRICE / 2) {
                WL_ETH_PRICE = DA_FINAL_PRICE / 2;
            }

            WLPANDAPrice = WL_ETH_PRICE * 10 ** 18 / PANDAETHRatio;
        }

        WL_PRICE_UPDATE = true;

        emit UpdateWLPrice(WLPANDAPrice);
    }

    function setPANDAETHRatio(uint256 price) external onlyOwner {
        PANDAETHRatio = price;

        emit SetPANDAETHRatio(price);
    }

    function setDASigners(address signer) external onlyOwner {
        daSigner = signer;

        emit SetDASigners(signer);
    }
    ...
    function setDirectMintAllowance(bool _allowDirect) external onlyOwner {
        ...
        emit SetDirectMintAllowance(_allowDirect);
    }

    function setDutchActionActive(bool daActive) external onlyOwner {
        DA_ACTIVE = daActive;

        emit SetDutchActionActive(daActive);
    }
    ...
    error DANotStart(uint256 timestamp, uint256 startTime);
    error WLSaleNotStart(uint256 timestamp, uint256 startTime);
    error DaIsntActive();
    error DANotOver();
    error WLPriceNotUpdate();
    error SignerAddressMismatch(address signer, address recoverAds);
    error MaxSupplyForDA(uint256 currentTotal, uint8 quantity, uint256 DA_QUANTITY);
    error DaIsFinished(uint256 timestamp, uint256 wlStartTime);
    error MaxMint5NFTs(uint8 quantity);
    ...
    error WlNotStart(uint256 timestamp, uint256 wlStartTime);
    error WlMintingFinished(uint256 timestamp, uint256 wlFinishTime);
    error WlNotEnoughEth(uint256 msgValue, uint256 needValue);
    error WlNotFinished(uint256 timestamp, uint256 wlFinishTime);    
    ...
    event MintDutchAuction(address minter, uint256 quantity);
    event MintWL(address minter, uint256 quantity);
    event DevMint(uint256 quantity);
    event WithdrawFunds(uint256 quantity);
    event UpdateWLPrice(uint256 price);
    event SetPANDAETHRatio(uint256 price);
    event SetDASigners(address signer);
    event SetDirectMintAllowance(bool allowDirect);
    event SetDutchActionActive(bool daActive);
