...
contract UMC is ERC721Enumerable,Ownable,RandomlyAssigned {
	...
	bytes32  public root=0xe063e422f452e97060ae56b15f62f0d97bdff93ddab0993b69b0a197028503b5;
    ...
    enum Status{ PRIVATE,PUBLIC,NONE }
		...
		RandomlyAssigned(_maxSupply, 1) 
    {
		...
		rounds[Status.PRIVATE]=Round(Status.PRIVATE,1648425600,1648425600+1 days,0.1*10**18,888,1,1);
        rounds[Status.PUBLIC]=Round(Status.PUBLIC,1648512000,1648512000+2 days,0.18*10**18,8888,100,10);
    }
	...
	function _buy(Status _round,uint8 _amount) internal verify(_round,_amount) { 
		  require(totalSupply()+_amount<=MAX_UMC,"There is no tokens to sell");
		  uint price = rounds[_round].price * _amount;
		  require(msg.value == price,"Insuffisant Amount");
		  uint feeAmount = (msg.value *feePrc)/100;
	...
	function buyPrivate(bytes32[] calldata _merkleProof,uint8 _amount) public payable verifyProof(_merkleProof) {
		   require(rounds[Status.PRIVATE].beginAt<=block.timestamp && rounds[Status.PRIVATE].finishAt>=block.timestamp, "Invalid Private Round");
	       _buy(Status.PRIVATE,_amount);
	}
    function buyPublic(uint8 _amount) public payable  {
			require(rounds[Status.PUBLIC].beginAt<=block.timestamp && rounds[Status.PUBLIC].finishAt>=block.timestamp, "Invalid Public Round");
	       _buy(Status.PUBLIC,_amount);
	}
