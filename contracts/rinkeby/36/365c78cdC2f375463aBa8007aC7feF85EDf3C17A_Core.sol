...
contract Core is Ownable, ICore {
    ...
    function newNFT(NewNFTParams calldata params) external onlyOwner returns(address) {
        ...
        bytes32 salt = bytes32(abi.encode(nft));
        ...
        emit NewNFT(nft, openseaMinter, splitter, params);
        return nft;
    }
...
interface IOpenseaFactory {
    function initialize(address _owner, address _underlyingNFT, uint256 premint) external;  
...
interface ICore {
    ...
    event NewNFT(address nft, address openseaMinter, address splitter, NewNFTParams params);
