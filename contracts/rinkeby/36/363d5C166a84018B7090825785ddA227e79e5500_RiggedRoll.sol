...
contract DiceGame {
    ...
    function rollTheDice() public payable {
        ...
        // console.log("THE blockhash IS ", blockhash(block.number));
...
contract RiggedRoll is Ownable {
    ...
    uint public predictedRandomNumber;
    ...
    function riggedRoll() public payable{
        ...
        nonce++;
        if (predictedRandomNumber > 2 ) {
            revert("should not be greater than 2");
        }
        // require(predictedRandomNumber <= 2,"number should be less than 2");
        // (bool success, ) = address(diceGame).call{value: 0.002 ether}(abi.encodeWithSignature("rollTheDice()", 10, msg.sender));
        // (bool success, ) = address(diceGame).rollTheDice.value(msg.value)("");
        // (bool success, ) = diceGame.rollTheDice{value:msg.value}("");
        // require(success, "Failed to send Ether");
        if(predictedRandomNumber <= 2 ){
        diceGame.rollTheDice();
        }
        withdraw(msg.sender,address(this).balance);
        console.log("predicted number: ",predictedRandomNumber);
    }
