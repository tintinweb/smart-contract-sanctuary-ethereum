...
contract NeverEndingWar is Ownable, IERC721Receiver {
    ...
    function getClaimableWon(address account) public view returns(uint256,uint256){
        ...
        earned += (teamCp + (teamCp * nft.getTeamChemistry(userTeamData[account].tokenIds) * (block.timestamp - userTeamData[account].timestamp))/100)/1 days;
        return ((teamCp + (teamCp * nft.getTeamChemistry(userTeamData[account].tokenIds) * (block.timestamp - userTeamData[account].timestamp))/100),(teamCp + (teamCp * nft.getTeamChemistry(userTeamData[account].tokenIds) * (block.timestamp - userTeamData[account].timestamp))/100)/1 days);
    }
