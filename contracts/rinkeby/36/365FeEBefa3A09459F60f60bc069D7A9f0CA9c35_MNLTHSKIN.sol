...
contract MNLTHSKIN is ERC1155, Ownable, ERC1155Burnable {  
    ...
    function skinUnequipped(uint256 vialId, address owner) public {
        ...
        if (vialId < startingTokenId) isMinted[vialId] = true;
    ...
    function upgradeLegacyVials(uint256[] calldata vialIds) public {
        for (uint256 i = 0; i < vialIds.length; ++i) {
            require(!isMinted[vialIds[i]], "Upgraded vial already exists");
            require(vialIds[i] < startingTokenId, "Can only upgrade legacy vials");
            
            legacySkinVialContract.burn(msg.sender, vialIds[i], 1);

            isMinted[vialIds[i]] = true;
            _mint(msg.sender, vialIds[i], 1, "");
        }
    }

    function airdropVial(uint256[] calldata vialIds, address[] calldata owners) public onlyOwner {
        for (uint256 i = 0; i < vialIds.length; ++i) {
            isMinted[vialIds[i]] = true;
            _mint(owners[i], vialIds[i], 1, "");
        }
    }
    ...
    function resetCooldown(uint256 vialId) public onlyOwner {
        lastEquipped[vialId] = 0;
    }
    ...
    function setLegacyContractVialAddress(address contractAddress) public onlyOwner {
        legacySkinVialContract = LegacySkinVial(contractAddress);
    }
    ...
    function uri(uint256 id) public view virtual override returns (string memory) {
        return string(abi.encodePacked("https://mnlthassets.rtfkt.com/", id.toString()));
    }

    function _mint(address to, uint256 id, uint256 amount, bytes memory data) internal override {
        require(balanceOfVial[id] == 0, "Vial already exist");
        balanceOfVial[id] = 1;
