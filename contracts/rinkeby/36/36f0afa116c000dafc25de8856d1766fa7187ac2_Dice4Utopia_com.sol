...
*/
contract Dice4Utopia_com is VRFConsumerBase, Ownable, ReentrancyGuard {
    ...
    uint256 public chainlinkFee = 100000000000000000; // 0.1 LINK
    ...
    uint256 public balanceMaxProfitRatio = 1;//24;
    ...
    struct IndexBet { 
        ...
        uint256[100] balance;
        uint8[100] modulo;
        uint256[100] placeAmount;
        bool[100] isSettled;
        uint256[100] winAmount;
    }
    ...
    //uint256 public rewardPct = 20;

    // Variables for computing price of ROLL in MATIC. Used for computing ROLL reward for transaction mining.
    //uint256 constant SAFETY_FACTOR = 1e7; // Prevent truncation of price in integer datatype by multiplying with SAFETY_FACTOR.
    ...
    //uint256 public rollMaticRatio; // Price of ROLL/MATIC
    //uint256 public rollMaticBlockTimestampLast;
    //uint256 maxReward = 20000 ether;
    ...
    // Events
    event BetPlaced(uint256 indexed betId, address indexed gambler, uint256 amount, uint8 indexed modulo, uint8 rollUnder, uint40 mask);
    // event BetSettled(uint256 indexed betId, address indexed gambler, uint256 amount, uint8 indexed modulo, uint8 rollUnder, uint40 mask, uint256 outcome, uint256 winAmount, uint256 rollReward);
    ...
    constructor(address AssassinCreed_Address_, address IDO_Address_) VRFConsumerBase(VRF_COORDINATOR, LINK_TOKEN) public {
    ...
    // function setRewardPct(uint256 _rewardPct) external onlyOwner {
    ...
    //         uint256 newRollMaticRatio = IERC20(ROLL_TOKEN).balanceOf(rollMaticLP) * SAFETY_FACTOR / IERC20(MATIC_TOKEN).balanceOf(rollMaticLP);
    ...
    // Place bet
    // function placeBet(uint256 betMask, uint256 modulo, address referrer) external payable nonReentrant {
    function placeBet(uint256 betMask, uint256 modulo) external payable nonReentrant {
    ...
    function getWinAmount(uint256 amount, uint256 modulo, uint256 rollUnder) private view returns (uint256 winAmount) {
        ...
        uint256 houseEdgeFee = amount * (houseEdgeBP + getEffectiveWealthTaxBP(amount)) / 10000;
    ...
    // function getRollReward(uint256 amount, uint256 modulo, uint256 rollUnder) private view returns (uint256) {
    //     // ROLL reward equals token price multiplied by house edge fees, divided by win probability, multiplied by rewardPct.
    //     uint256 rollReward = rollMaticRatio * amount * (houseEdgeBP + getEffectiveWealthTaxBP(amount)) / 10000 * modulo / (modulo - rollUnder) * rewardPct / 100 / SAFETY_FACTOR;
    ...
    function settleBet(bytes32 requestId, uint256 randomNumber) internal nonReentrant {
        ...
        if (winAmount > 0) {
            uint256 pureWinAmount = (winAmount - amount);
            incomeProfit = incomeProfit >= pureWinAmount ? (incomeProfit - pureWinAmount) : 0;
    ...
    function deposit(address user) external payable {
        require(msg.value > 0 && msg.sender == IDO_Address,"deposit::Invalid investment");
    ...
    function WithdrawalIncome(uint256 amountACreed) external nonReentrant { 
        address payable user = msg.sender;
        uint256 share = sharesDice[user];
        require(share > 0 && sharesDiceAll > 0,
        "WithdrawalIncome::Sorry, you have not participated in the Initial Digital Assets Offering");

        // user have to approve AssassinCreed to this address
        IAssassinCreed(AssassinCreed_Address).burnFrom(msg.sender, amountACreed);

        if(block.timestamp > IIDO(IDO_Address).getEndTimestamp().add(182 days) ){ 
            uint256 thisBalance = address(this).balance;
            require(thisBalance > 1e18, "WithdrawalIncome::Balance less than one matic");
            uint256 power = thisBalance.mul(30).div(100);
            require(power < thisBalance);
            uint256 income = power.mul(share).div(sharesDiceAll);
            user.transfer(income);
        }
        else { 
            require(incomeProfit > 1e18, "WithdrawalIncome::houseProfit less than one matic");
            uint256 power = incomeProfit.mul(30).div(100);
            require(power < incomeProfit);
            uint256 income = power.mul(share).div(sharesDiceAll);
            user.transfer(income);
        }
        // clear
        sharesDice[user] = 0;
        sharesDiceAll = sharesDiceAll.sub(share);
    }

    function totalAssetsOfthis() public view returns (uint256) {
        if(block.timestamp > IIDO(IDO_Address).getEndTimestamp().add(182 days) ){ 
            uint256 thisBalance = address(this).balance;
            return thisBalance > 1e18 ? thisBalance.mul(30).div(100) : 0;
        }
        else { 
            return incomeProfit > 1e18 ? incomeProfit.mul(30).div(100) : 0;
        }
    }
