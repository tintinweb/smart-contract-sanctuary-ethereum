...
contract ProxyFactory is Ownable {
 ...
 constructor(address _tokenImplementation, address _timeLockImplementation, address _Daoimplementation) {
  ...
  function createNewToken(address userAddress, string memory _name, string memory _symbol, address _guardian) payable external returns(address instance) {
   ...
   (bool success, ) = instance.call{value:msg.value}(abi.encodeWithSignature("initialize(address,string,string,address)", userAddress,_name,_symbol,_guardian));
  ...
  //Function that creates a new instance of a Token
  function createNewTimeLock(address userAddress, uint delay) payable external returns(address instance) {
    // Clone the base contract and store it's address into local variable instance
  ...
  function createNewDao(address timelockAddress, address tokenAddress, address _guardian) 
