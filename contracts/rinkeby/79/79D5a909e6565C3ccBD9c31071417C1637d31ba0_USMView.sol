...
contract USMView {
    ...
    function ethBuffer(WadMath.Round upOrDown) external view returns (int buffer) {
        (uint collateralPrice, uint syntheticPrice, uint updateTime ) = usm.latestPrice(); // why do i need to declare all the return values ?
        buffer = usm.ethBuffer(collateralPrice, usm.ethPool(), usm.totalSupply(), upOrDown);
    }
    ...
    function ethToUsm(uint ethAmount, WadMath.Round upOrDown) external view returns (uint usmOut) {
        (uint collateralPrice, uint syntheticPrice, uint updateTime ) = usm.latestPrice();
        usmOut = usm.ethToUsm(collateralPrice, ethAmount, upOrDown);
    }
    ...
    function usmToEth(uint usmAmount, WadMath.Round upOrDown) external view returns (uint ethOut) {
        (uint collateralPrice, uint syntheticPrice, uint updateTime ) = usm.latestPrice();
        ethOut = usm.usmToEth(collateralPrice, usmAmount, upOrDown);
    }
    ...
    function debtRatio() external view returns (uint ratio) {
        (uint collateralPrice, uint syntheticPrice, uint updateTime ) = usm.latestPrice();
        ratio = usm.debtRatio(collateralPrice, usm.ethPool(), usm.totalSupply());
    }
    ...
    function usmPrice(IUSM.Side side) external view returns (uint price) {
        (uint collateralPrice, uint syntheticPrice, uint updateTime ) = usm.latestPrice();
        price = usm.usmPrice(side, collateralPrice, usm.bidAskAdjustment());
    }
    ...
    function fumPrice(IUSM.Side side) external view returns (uint price) {
        ...
        if (side == IUSM.Side.Buy) {
            (, usmSupply) = usm.checkIfUnderwater(usmSupply, ethPool, collateralPrice, oldTimeUnderwater, block.timestamp);
        }
        price = usm.fumPrice(side, collateralPrice, ethPool, usmSupply, usm.fumTotalSupply(), usm.bidAskAdjustment());
    }
}
