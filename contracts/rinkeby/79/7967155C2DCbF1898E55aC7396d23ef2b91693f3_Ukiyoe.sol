...
contract Ukiyoe is ERC721A, Ownable {
  ...
  // Constants
  uint256 constant public MAX_SUPPLY = 6666;
  ...
  constructor() ERC721A("Ukiyoe", "UKIYOE") {}

  function mint(uint256 _quantity) public payable {
    require(
      _isSaleActive, 
      "Sale must be active to mint Ukiyoe"
    );
    require(
    ...
    // should we check maxBalance and maxMint ?
    if (msg.sender != owner()) {
        // require(_allowList[msg.sender] == 1);
        require(msg.value >= mintPrice * _quantity);
        _allowList[msg.sender] = 2;
    }

    _safeMint(msg.sender, _quantity);
  }

  function mintWhiteList(uint256 _quantity) external payable {
    require(
      _isWhiteListSaleActive, 
      "Whitelist is not active to mint Ukiyoe"
    );
    require(
      totalSupply() + _quantity <= MAX_SUPPLY, 
      'Sale would exceed Whitelist supply'
    );
    require(
      _quantity <= _allowList[msg.sender], 
    ...
    require(
      _quantity <= maxMint, 
      'Sale would exceed max mint'
    );
    _allowList[msg.sender] -= _quantity;
    _safeMint(msg.sender, _quantity);
  ...
  function setIsWhiteListActive(bool _isAllowListActive) external onlyOwner {
    _isWhiteListSaleActive = _isAllowListActive;
  }	

  function setWhitelistSupply(uint256 _whitelistTotal) external onlyOwner {
    whitelistSupply = _whitelistTotal;
  }
