    ...
    ERC2981Upgradeable
{
    ...
    function preSaleMint(uint256 quantity, bytes32 typeMint)
        external
        payable
        nonReentrant
        callerIsUser
    ...
    function publicSaleMint(
        ...
        require(
            numberMinted(msg.sender) + quantity <=
                maxPerAddressDuringMint + allowceInWhiteList(msg.sender),
            "Shiniki: can not mint this many"
        );
        uint256 price = uint256(salePrice[token]);
        if (token == address(0)) {
    ...
    function setTransferNesting(bool _nestingTransfer) external onlyOwner {
        nestingTransfer = _nestingTransfer;
    }
    ...
    function setPublicSaleConfig(
        ...
        for (uint256 i = 0; i < _tokens.length; i++) {
            ...
            salePrice[_tokens[i]] = _publicPrices[i];
        }
    }
        ...
        onlyOwner
    {
        ...
        for (uint256 i = 0; i < addresses.length; i++) {
            require(
                values[i] <= maxPerAddressDuringMint,
                "Shiniki: value is invalid"
            );
    ...
    function setMaxPerAddressDuringMint(uint256 maxPerAddressDuringMint_)
        external
        onlyOwner
    {
    ...
    function getSalePrice(address token) public view returns (uint256) {
        return salePrice[token];
    }
    ...
    function _beforeTokenTransfers(
        ...
        if (from != address(0)) {
            ...
            require(nestingTransfer, "Shiniki: nesting");
        }
    }
...
contract AcceptedTokenList is OwnableUpgradeable {
    ...
    mapping(address => uint256) internal salePrice;
