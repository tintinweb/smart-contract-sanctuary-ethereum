    ...
    OwnableUpgradeable
{
    ...
    /**
     * @notice Constructor to create XODParticle contract
     *
     * @param _name the token name
     * @param _symbol the token symbol
     * @param _baseTokenURI token base uri
     * @param _whitelistPrice1 price for whitelist enabled
     * @param _merkleRoot1 root hash of merkle tree
     */
    ...
    /**
     * @notice If _enabled, only allows whitelisted addresses to mint
     */
    ...
    /**
     * @notice Sets price for whitelisted addresses to mint in decimals
     */
    ...
    /**
     * @notice Sets merkleroot calculated on client side
     */
        ...
        override
    {
        ...
        require(
            _totalMinted().add(quantity) <= MAX_LIMIT,
        ...
        if (block.timestamp <= mintStart.add(2 days)) {
            require(quantity <= MAX_LIMIT_PER_WALLET, "Siku: Quantity should not be more than 3");
            require(whitelistMinted[msg.sender].add(quantity) <= MAX_LIMIT_PER_WALLET, "Siku: Wallet limit reached");

            if (verifyMerkleProof(_merkleProof, merkleRoot1) && enabledGroup1) {
                price = priceGroup1;
                whitelisted = true;
            } else if (verifyMerkleProof(_merkleProof, merkleRoot2) && enabledGroup2) {
                price = priceGroup2;
                whitelisted = true;
            } else {
        ...
        if (whitelisted) {
            whitelistMinted[msg.sender] = whitelistMinted[msg.sender].add(quantity);
        }
    ...
    // Pseudo-random. For better randomness, use oracle
