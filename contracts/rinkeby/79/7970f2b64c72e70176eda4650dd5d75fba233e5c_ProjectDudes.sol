...
contract ProjectDudes is Ownable, ERC721A, ReentrancyGuard {
    ...
    modifier callerIsUser(uint256 quantity) {
        require(!paused, "Contract is paused");
        require(totalSupply() + quantity <= MAX_SUPPLY, "No more left. Sorry Dude!");
    ...
    function freeMint(uint256 quantity)        
        ...
        payable
        nonReentrant
        callerIsUser(quantity)
         {

        require(freeSale, "Free sale inactive");
        require(msg.value == 0, "This phase is free");
        require(MINT_PER_FREE_TX < 2, "Only 2 free");
        require(mintedWallets[msg.sender] < 2, "exceed max free mint");
    ...
    function mintToUser(uint256 quantity, address receiver) public onlyOwner callerIsUser(quantity) {
