...
/*
Project: Secure Pay Token
Description: SecurePay aims to become the preferred cryptocurrency ensuring safe, anonymous, 
direct virtual payments. Virtual Coin Card can be used for trades, for online payments and 
purchases anywhere in the world using just SecurePay virtual card.
...
contract SecurePay is ERC20, Ownable {
    uint256 public  lpTaxOnBuy = 2;
    uint256 public  lpTaxOnSell = 2;

    uint256 public  developmentTaxOnBuy = 2;
    uint256 public  developmentTaxOnSell = 2;

    uint256 public  marketingTaxOnBuy = 4;
    uint256 public  marketingTaxOnSell = 4;

    uint256 private _totalFeesOnBuy = lpTaxOnBuy + developmentTaxOnBuy + marketingTaxOnBuy;
    uint256 private _totalFeesOnSell = lpTaxOnSell + developmentTaxOnSell + marketingTaxOnSell;
    ...
    bool public noTaxOnWalletToWallet = true;
    ...
    event ProjectWalletChanged(address developmentWallet);
    event SetAutomatedMarketMakerPair(address indexed pair, bool indexed value);
    event SwapAndLiquify(uint256 tokensSwapped, uint256 bnbReceived, uint256 tokensIntoLiqudity);
    event UpdateUniswapV2Router(address indexed newAddress, address indexed oldAddress);
    event UpdateBuyFees(uint256 lpTaxOnBuy, uint256 marketingTaxOnBuy, uint256 developmentTaxOnBuy);
    event UpdateSellFees(uint256 lpTaxOnSell, uint256 marketingTaxOnSell, uint256 developmentTaxOnSell);
    event ProcessMarketingTax(uint256 sentBNB);
    event ProcessDevelopmentTax(uint256 sentBNB);
    event OperatorChanged(address _operator);

    constructor (address newOwner, address router_)ERC20("Secure Pay",  "SRPAY"){  

        _mint(newOwner, 100_000_000 * (10 ** 18));

        operator = newOwner;
        marketingWallet = 0x2F0121C035A863A5dE73f9DB72Ec7C4FbB46881b;// Address to collect marketing taxes in BNB
        developmentWallet   = 0x2F0121C035A863A5dE73f9DB72Ec7C4FbB46881b;// Address to collect development taxes in BNB

        maxTxn = totalSupply() / 100; // 1% of total supply
    ...
    function setOperator(address _operator) external onlyOwner {
        require(_operator != address(0), "can't set operator to zero address!");
        operator = _operator;
    ...
    function claimStuckTokens(address token) external onlyOwnerOrOperator {
        require(token != address(this), "SRPAY tokens can't be withdrawan!");
    ...
    function updateMaxAmount(uint256 newNum) external onlyOwner {
        require(newNum >= totalSupply()/1000, "Cannot lower maxTxn below 0.1% of total supply");
        maxTxn = newNum;
    ...
    function updateMaxWallet(uint256 newNum) external onlyOwner {
        require(newNum >= totalSupply()/100, "Can't lower maxWallet below 1% of total supply");
        maxWallet = newNum;

        emit MaxWalletUpdated(newNum);
    }

    function updateBuyFees(uint256 _lpTaxOnBuy, uint256 _marketingTaxOnBuy, uint256 _developmentTaxOnBuy) external onlyOwner {
        require(
            _lpTaxOnBuy + _marketingTaxOnBuy + _developmentTaxOnBuy <= 25,
            "Can't set buy fees above 25%"
        );
        lpTaxOnBuy = _lpTaxOnBuy;
        developmentTaxOnBuy    = _developmentTaxOnBuy;
        marketingTaxOnBuy = _marketingTaxOnBuy;
        _totalFeesOnBuy   = lpTaxOnBuy + marketingTaxOnBuy + developmentTaxOnBuy;
        emit UpdateBuyFees(_lpTaxOnBuy, _marketingTaxOnBuy, _developmentTaxOnBuy);
    }

    function updateSellFees(uint256 _lpTaxOnSell, uint256 _marketingTaxOnSell, uint256 _developmentTaxOnSell) external onlyOwner {
        require(
            _lpTaxOnSell + _marketingTaxOnSell + _developmentTaxOnSell <= 25,
            "Can't set sell fees above 25%"
        );
        lpTaxOnSell = _lpTaxOnSell;
        developmentTaxOnSell    = _developmentTaxOnSell;
        marketingTaxOnSell = _marketingTaxOnSell;
        _totalFeesOnSell   = lpTaxOnSell + marketingTaxOnSell + developmentTaxOnSell;
        emit UpdateSellFees(_lpTaxOnSell, _marketingTaxOnSell, _developmentTaxOnSell);
    }
    ...
    function changeMarketingWallet(address _marketingWallet) external onlyOwner {
        require(_marketingWallet != marketingWallet && _marketingWallet != address(0), "Can't set marketing wallet to zero address or it is already set to this address");
        require(!isContract(_marketingWallet), "Marketing wallet cannot be a contract");
        marketingWallet = _marketingWallet;
        emit MarketingWalletChanged(marketingWallet);
    }

    function changeProjectWallet(address _developmentWallet) external onlyOwner {
        require(_developmentWallet != developmentWallet && _developmentWallet != address(0), "Can't set marketing wallet to zero address or it is already set to this address");
        require(!isContract(_developmentWallet), "Development wallet cannot be a contract");
        developmentWallet = _developmentWallet;
        emit ProjectWalletChanged(developmentWallet);
    }
    ...
    function _transfer(
        ...
        if (swapEnabled && 
            ...
            uint256 projectShare    = developmentTaxOnBuy    + developmentTaxOnSell;
            ...
            uint256 bnbShare = marketingShare + projectShare;
            ...
            if(contractTokenBalance > 0 && bnbShare > 0) {
                ...
                if(projectShare > 0) {
                    ...
                    sendBNB(payable(developmentWallet), projectBNB);
                    emit ProcessDevelopmentTax(projectBNB);
                }
            }
