...
contract Plus1 is ERC721, Initializable, Owned {
    ...
    bool public omitIndexInTokenURI;
        ...
        returns (string memory)
    {
        return
            string.concat(
                _baseURI,
                originalTokenId(id).toString(),
                omitIndexInTokenURI
                    ? ""
                    : string.concat("-", (indexOfId(id) + startId).toString())
            );
    }
    ...
    function indexOfId(uint256 id) public view returns (uint256) {
        if (id < startId) {
            revert IdOutOfBounds();
        }
        uint256 offsetId = id - startId;
        return offsetId % limit;
    }
    ...
    function setOmitIndexInTokenURI(bool omit) external onlyOwner {
        omitIndexInTokenURI = omit;
    }
}
