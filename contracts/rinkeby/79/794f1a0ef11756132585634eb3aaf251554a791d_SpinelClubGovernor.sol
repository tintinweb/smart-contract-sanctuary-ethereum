...
contract SpinelClubGovernor is
    UUPSUpgradeable,
    ...
    OwnableUpgradeable
{
    uint16 private _voteDelay;
    uint16 private _votePeriod;
    uint16 private _voteQuorum;
    uint16 private _proposalThreshold;
    uint16 private _proposerRewardRate;
    ...
    function initialize(
        ...
        _proposerRewardRate = 5;
    }

    function _authorizeUpgrade(address) internal override onlyOwner {}

    function votingDelay(uint16 voteDelay) public onlyOwner {
        _voteDelay = voteDelay;
    }

    function votingDelay() public view override returns (uint256) {
        return _voteDelay; // 1 block
    }

    function votingPeriod(uint16 votePeriod) public onlyOwner {
        _votePeriod = votePeriod;
    }

    function votingPeriod() public view override returns (uint256) {
        return _votePeriod; // 15 minutes
    }

    function proposerRewardRate(uint16 _rate) public onlyOwner {
        _proposerRewardRate = _rate;
    }

    function proposerRewardRate() public view returns (uint256) {
        return _proposerRewardRate;
    }
    ...
    function quorum(uint16 _qourum, uint256 blockNumber) public onlyOwner {
        _voteQuorum = _qourum;
    }

    function proposalThreshold(uint16 threshold) public onlyOwner {
    ...
    function sPropose(
        ...
        require(ITreasury(timelock()).isMember(msg.sender), "member only");
        ...
        ISpinelTimelock timelockSpinel = ISpinelTimelock(timelock());
        uint256 reservedAmount = 0;
        for (uint256 i = 0; i < targets.length; i++) {
            reservedAmount += values[i];
        }
        if (amount > 0) {
            require(address(token) != address(0), "invalid reward address");
            bool result = token.transferFrom(msg.sender, timelock(), amount);
            require(result, "Transfer token fail");
        }
        timelockSpinel.reservedTreasury(
            token,
            amount,
            proposalId,
            reservedAmount
    ...
    function queue(uint256 proposalId) public virtual override onlyOwner {
        ...
        returns (uint256)
    {
        ...
        ISpinelTimelock timelockSpinel = ISpinelTimelock(timelock());
        timelockSpinel.releaseTreasury(proposalId);
        return proposalId;
    }
    ...
    function releaseProposalFund(uint256 proposalId) public {
        ProposalState status = state(proposalId);

        require(status == ProposalState.Expired, "3");
        ISpinelTimelock timelockSpinel = ISpinelTimelock(timelock());
        timelockSpinel.releaseTreasury(proposalId);
    }
    ...
    function refund(uint256 proposalId) public onlyOwner {
        ProposalState status = state(proposalId);
        if (
            status == ProposalState.Canceled || status == ProposalState.Defeated
        ) {
            ISpinelTimelock timelockSpinel = ISpinelTimelock(timelock());
            timelockSpinel.refund(proposalId);
        } else {
            revert("4");
        }
    }
    ...
    function markProposalFill(
        ...
        ISpinelTimelock timelockSpinel = ISpinelTimelock(timelock());
        timelockSpinel.markProposalFill(_token, proposalId, _totalToken);
    }
}
...
interface ITreasury {
    ...
    function isMember(address sender) external returns (bool);

    function setAdmin(address governorAddress) external;

    function setClubType(ClubType _type) external;
