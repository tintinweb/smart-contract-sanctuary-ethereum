...
contract MoonwalkersRoyalties is ReentrancyGuard {
    event Received(address, uint);

    address private MoonwalkersContractAddress = 0x1e3136CD0203b271753c990fADaFA7Ae0283aD79;
    ...
    function getOwnership(uint256 tokenId) external view returns (Moonwalkers.TokenOwnership memory) {
    return Moonwalkers(MoonwalkersContractAddress).getOwnershipData(tokenId);
    }

    function totalSupplyCaller() public view returns (uint256) {
        return Moonwalkers(MoonwalkersContractAddress).totalSupply();
    }
}
