...
/** 
 *  SourceUnit: c:\Projects\ibl\sc-capital-dao\contracts\staking\MasterChefV4.sol
*/
...
contract MasterChefV4 is Ownable, ReentrancyGuard {
    ...
    event EmissionRateUpdated(uint256 rewardPerBlock);

    modifier poolExist(uint256 _pid) {
        require(_pid < pools.length, "MasterChef: pool does not exist");
        _;
    }

    constructor(IStakingRewardVault _vault, IERC20 _token, uint256 _startBlock, uint256 _rewardPerBlock)
    {
        vault = _vault;
        token = _token;
