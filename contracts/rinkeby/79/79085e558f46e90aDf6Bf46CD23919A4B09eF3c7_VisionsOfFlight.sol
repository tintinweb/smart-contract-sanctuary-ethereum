...
contract VisionsOfFlight is ERC721, Ownable, Pausable {
    ...
    function freeMint(address recipient, string[] memory proof) public whenNotPaused {
        require(block.timestamp >= claimOpensAt, "Knots: claim window has not opened");
        require(numVouchers[recipient] > 0 || proof.length > 0, "Knots: No freemints available");
    ...
    function hasPOAPVoucher(address recipient, string[] memory proof) internal view returns (bool) {
        require(!claimedPOAPVoucher[recipient], "Knots: POAP free mint already claimed");
        ...
        if (merkleProofValid) {
            ...
            revert("Knots: Invalid proof for POAP free mint");
        }
    }
    ...
    function mint(address recipient, uint8 count) public payable whenNotPaused {
        ...
        require(count > 0 && count <= MAX_MINTS, "Knots: maximum mint per tx exceeded");
        require(msg.value == count*MINT_PRICE, "Knots: invalid mint fee");

        // validate txs per wallet
        require(txsPerWallet[msg.sender] <= MAX_TXS_PER_WALLET, "Knots: User has exceeded 10 mint transactions per wallet");
        require(msg.sender == tx.origin, "Knots: Account is not an EOA");
    ...
    function mintToken(address recipient) internal {
        require(NEXT_TOKEN_ID <= MAX_TOKEN_ID, "Knots: Sold out");
