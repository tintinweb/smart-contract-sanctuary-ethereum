...
contract DAO is Auction {
    ...
    ) Auction(creatorRoyalty_, treasureRoyalty_, autoMintInterval_) ERC721(name_, symbol_) NFT(baseURI_) {
    ...
    function createProposal(
        ...
        uint256 amount,
        Compatibility[] calldata compatibilities
    ) public virtual {
        ...
        for (uint256 i = 0; i < compatibilities.length; i++) {
            p.compatibilities.push(compatibilities[i]);
        }

        proposalsCount += 1;
    }

    //    function setMinBalanceForProposalCreation(uint256 minBalanceForProposalCreation_) public virtual onlyOwner {
    //        minBalanceForProposalCreation = minBalanceForProposalCreation_;
    //    }
    //
    //    function setMinBalanceForVoting(uint256 minBalanceForVoting_) public virtual onlyOwner {
    //        minBalanceForVoting = minBalanceForVoting_;
    //    }
    ...
    function vote(
        ...
        require(owner() == _msgSender() || votes >= minBalanceForVoting, "D4");

        Vote storage v = p.votes[_msgSender()];
    ...
    function finishProposal(uint256 proposalIdx) public virtual validProposalIdx(proposalIdx) proposalNotFinished(proposalIdx) {
        Proposal storage p = proposals[proposalIdx];
        require(owner() == _msgSender() || block.timestamp >= p.endTime, "D6");
        p.finished = true;
        ...
        for (uint256 i = 0; i < p.compatibilities.length; i++) {
            Compatibility storage c = p.compatibilities[i];
            if (bytes(c.baseTrait).length > 0 && bytes(c.baseValue).length > 0 && bytes(c.baseVariation).length > 0) {
                _ensureVariation(c.baseTrait, c.baseValue, c.baseVariation);
            }
            if (bytes(c.trait).length > 0 && bytes(c.value).length > 0 && bytes(c.variation).length > 0) {
                _ensureVariation(c.trait, c.value, c.variation);
                if (bytes(c.baseTrait).length > 0) {
                    _setCompatibility(c.trait, c.value, c.variation, c.baseTrait, c.baseValue, c.baseVariation, c.compatsEnabled, c.compatible);
                }
            }
        }
    ...
    function getVote(uint256 proposalIdx, address author) public view virtual returns (Vote memory) {
        return proposals[proposalIdx].votes[author];
    }

//    function getCompatibilities(uint256 proposalIdx) external view virtual returns (Compatibility[] memory) {
//        return proposals[proposalIdx].compatibilities;
//    }
}
...
abstract contract Auction is NFT {
    struct _Auction {
        uint256 minBid;
    ...
    function auctionStarted(uint256 tokenId) public view virtual returns (bool){
        return auctions[tokenId].endTime > 0;
    }

    function auctionFinished(uint256 tokenId) public view virtual returns (bool){
        return block.timestamp >= auctions[tokenId].endTime;
    }

    function _requireAuctionStarted(uint256 tokenId) internal virtual {
        require(auctionStarted(tokenId), "A1");
    }

    modifier requireAuctionStarted(uint256 tokenId) virtual {
        _requireAuctionStarted(tokenId);
        _;
    }
    ...
    function auctionStart(uint256 tokenId, uint256 duration, uint256 minBid, uint256 maxBidsCount) public virtual {
    ...
    function auctionBid(uint256 tokenId, uint256 bid) external payable virtual {
        if (tokenId > 0 && !auctionStarted(tokenId) && auctionStarted(tokenId - 1) && auctionFinished(tokenId - 1) && ownerOf(tokenId - 1) == address(this)) {
            auctionFinish(tokenId - 1);
        }
    ...
    function genesisMint() public onlyOwner {
        require(_tokenIdCounter == 0 && _nfts.length == 0, "A14");
        _genTraits(true, 0, 0);
        _mintWithTraits(address(this));
        _auctionStart(0, autoMintInterval, 0, 1);
    }
    ...
    function auctionFinish(uint256 tokenId) public virtual {
        ...
        if (ownerOf(tokenId) == address(this) && !_exists(tokenId + 1) && _tokenIdCounter == tokenId + 1) {
            _mintWithTraits(address(this));
            _auctionStart(tokenId + 1, autoMintInterval, 0, tokenId < 100 ? 1 : 0);
        }
...
abstract contract NFT is ERC721, ERC721Enumerable, ERC721Burnable, Traits {
    uint256 internal constant mutagenInvProb = 2;

    uint256 internal _tokenIdCounter;
    string public baseURI;
    mapping(uint256 => uint256) public hueBg;
    ...
    function _mintWithTraits(address to) internal {
        if (_randint(mutagenInvProb) == 0) {
            _genMutagen();
        } else {
            _genTraits(true, 0, 0);
        }
        _safeMint(to, _tokenIdCounter);
        hueBg[_tokenIdCounter] = _randint(360);
        hue[_tokenIdCounter] = _randint(360);
        _tokenIdCounter++;
    }
    ...
    function mutate(uint256 tokenId1, uint256 tokenId2) external virtual {
        ...
        _burn(tokenId1);
        _burn(tokenId2);
        require(_tokenIdCounter + 1 == _nfts.length, "N4");
    }
...
contract Traits is Ownable {
    ...
    mapping(string => mapping(string => uint256[])) public traitToId;
    ...
    function _randint(uint256 range) internal virtual returns (uint256) {
        _seed = uint256(keccak256(abi.encodePacked(block.timestamp, block.difficulty, _seed, range)));
        return _seed % range;
    }
    ...
    function _genTraitApply(_GenTrait memory gt) internal virtual {
        ...
        traitToId[compatibleTrait.trait][compatibleTrait.value].push(_nfts.length - 1);

        Spec storage t = spec.items[compatibleTrait.trait];
        t.count += 1;

        Spec storage v = t.items[compatibleTrait.value];
        v.count += 1;

        v.items[compatibleTrait.variation].count += 1;
    }
    ...
    function _genMutagen() internal virtual {
        ...
        Trait[] storage traits = nft.traits;
        traits.push();
        traits[0].trait = trait;
    ...
    function getActiveTraits() external view virtual returns (Trait[] memory) {
        uint256 n;
        for (uint256 i = 0; i < spec.itemsList.length; i++) {
            Spec storage trait = spec.items[spec.itemsList[i]];
            for (uint256 j = 0; j < trait.itemsList.length; j++) {
                Spec storage value = trait.items[trait.itemsList[j]];
                n += value.itemsList.length;
            }
        }

        Trait[] memory t = new Trait[](n);
        uint256 idx;

        for (uint256 i = 0; i < spec.itemsList.length; i++) {
            string storage tname = spec.itemsList[i];
            Spec storage trait = spec.items[tname];
            for (uint256 j = 0; j < trait.itemsList.length; j++) {
                string storage vname = trait.itemsList[j];
                Spec storage value = trait.items[vname];
                for (uint256 k = 0; k < value.itemsList.length; k++) {
                    string storage vrname = value.itemsList[k];

                    t[idx].trait = tname;
                    t[idx].value = vname;
                    t[idx].variation = vrname;
                    idx++;
                }
            }
        }
