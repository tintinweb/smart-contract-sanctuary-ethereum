...
contract MarketplaceTest is  Ownable, EIP712, ERC721Holder{
...
//     }
	...
	function recoverOrder(Order calldata order) public view returns (address) {
        ...
        bytes32 digest = _hashTypedDataV4(keccak256(abi.encode(            
            keccak256("Order(uint256 tokenId,uint256 ethPrice,address tokenAddress,address seller)"),
            order.tokenId,
            order.ethPrice,
            // order.tokenPrice,
            // order.createdTime,
 ...
 function recoverAuction(AuctionData calldata auction )public view returns (address) {
...
//      //check orderStatus
//     require (orderMap[order.signature].isvalid == false,"order not active");
...
//         require(tokenOwner == recoverOrder(order),"Marketplace: Only the asset owner can create orders");
...
//         // orderByTokenId[order.tokenAddress][order.tokenId] = Order({
//         //     // orderId: _orderId,
...
//         // emit OrderCreated(_orderId,_tokenAddress,_tokenId,msg.sender,_ethPrice,_tokenPrice,_coinAddress);
...
//        orderMap[order.signature].isvalid=false;
...
function buyOrder1(Order calldata order) public payable
    ...
    // Check nft registry
     ...
     // Check order creator is the asset owner
        ...
        require(msg.value >= order.ethPrice, "Marketplace: invalid price");
       ...
       tokenRegistry.safeTransferFrom(tokenOwner,msg.sender,order.tokenId);
        ...
        // require(tokenOwner == recover(order),"Marketplace: Only the asset owner can create orders");

        orderMap[order.signature].isvalid=true;
        
        return orderMap[order.signature].isvalid;
   ...
   // Auction section 
       ...
       function acceptBid(AuctionData calldata auction,BidData calldata biD) public 
    {
       //check auctionrStatus
    require (AuctionStatus[auction.signature].isValid == false,"order not active");
    ...
    //coinAdrress object
     ...
     // Check order creator is the asset owner
        ...
        require(tokenOwner == recoverAuction(auction),"Marketplace: Only the asset owner can create orders");
        require(biD.Bidder == recoverBid(biD),"INVALID bidder");
        

        require(Token.balanceOf(recoverBid(biD)) >= biD.tokenPrice,"dont have enough tokens");
    ...
    //    getting values for royalties 
        ...
        //royalty calculation 
        uint256 MP_FEE      =  biD.tokenPrice*marketplaceFEE/100000000000000000000;
        uint256 sellerCut   =  biD.tokenPrice-MP_FEE;
    
        //sending aura
        Token.transferFrom(biD.Bidder,msg.sender,sellerCut);
        Token.transferFrom(biD.Bidder,marketOwner,MP_FEE);
            ...
            //royalty calculation 
        uint256 royaltyCut  =  biD.tokenPrice*royaltyAmount/100000000000000000000;
        uint256 MP_FEE      =  biD.tokenPrice*marketplaceFEE/100000000000000000000;
        uint256 sellerCut   =  biD.tokenPrice-(royaltyCut+MP_FEE);
      
        // sending aura
        Token.transferFrom(biD.Bidder,msg.sender,sellerCut);
        Token.transferFrom(biD.Bidder,owner,royaltyCut);
        Token.transferFrom(biD.Bidder,marketOwner,MP_FEE);
       
    
        }

        tokenRegistry.safeTransferFrom(recoverAuction(auction), recoverBid(biD), auction.tokenId);
        AuctionStatus[auction.signature].isValid == true;
...
function cancelAuction(AuctionData calldata auction) public returns(bool){
        ...
        require(msg.sender == recoverAuction(auction),"Marketplace: Only the asset owner can cancel auction");
        

        AuctionStatus[auction.signature].isValid=true;
        
        return AuctionStatus[auction.signature].isValid;
    ...
    function recoverOffer(Offer calldata offer) public view returns (address) {
            ...
            bytes32 digest = _hashTypedDataV4(keccak256(abi.encode(
            keccak256("Offer(uint256 tokenId,uint256 Price,address tokenAddress,uint256 time)"),
            offer.tokenId,
            offer.Price,
            offer.tokenAddress,
            offer.time
    ...
    function acceptOffer(Offer calldata offer)public {
        ...
        require (offerMapping[offer.signature].isvalid == true,"order not active");

        IERC721 tokenRegistry   =   IERC721(offer.tokenAddress);
        require(msg.sender      ==  tokenRegistry.ownerOf(offer.tokenId));
        require(offer.Price  <=  tokenRegistry.balanceOf(recoverOffer(offer)),"not enough ether sent");


        // sending money 
        tokenRegistry.transferFrom(recoverOffer(offer),msg.sender,offer.Price);
        // sending token
        tokenRegistry.safeTransferFrom(msg.sender, recoverOffer(offer),offer.tokenId);

        offerMapping[offer.signature].isvalid=false;
        ...
        function cancelOffer(Offer calldata offer)public  returns(bool)
        {
            ...
            offerMapping[offer.signature].isvalid = true;
            return offerMapping[offer.signature].isvalid;
            }
