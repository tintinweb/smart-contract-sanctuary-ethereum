...
interface IEveraiDataCard is IERC721A {
    function mint(address to, uint256 quantity) external;
}

contract EveraiBanner is ERC721A, Ownable, ReentrancyGuard {
    address public synchronizationContractAddress;
    ...
    IEveraiDataCard public immutable everaiDataCard;

    constructor(address everaiDuoAddress, address everaiDataCardAdress)
        ERC721A("EveraiBanner", "EveraiBanner")
    {
        everaiDuo = IERC721A(everaiDuoAddress);
        everaiDataCard = IEveraiDataCard(everaiDataCardAdress);
    }

    function setSynchronizationContractAddress(
        address synchronizationContractAddress_
    ) external onlyOwner {
        synchronizationContractAddress = synchronizationContractAddress_;
    }
    ...
    function claim(uint256[] memory tokenIds) public nonReentrant {
        ...
        everaiDataCard.mint(msg.sender, numTokensClaimed);
        _safeMint(msg.sender, numTokensClaimed);
    }

    function upgrade(uint256 tokenId, uint16 dataCardType) external {
        require(
            msg.sender == synchronizationContractAddress,
