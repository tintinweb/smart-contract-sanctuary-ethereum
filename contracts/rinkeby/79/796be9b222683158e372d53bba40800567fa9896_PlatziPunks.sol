...
contract PlatziPunksDNA {
    ...
    // this function is deterministic and should not be used on production
    function deterministicPseudoRamdonDNA(uint256 tokenId, address _minter) public pure  returns (uint256){
        uint256 combineParams = tokenId + uint160(_minter);
         bytes memory endocedCombineParams = abi.encodePacked(combineParams);
        
        bytes32 hash = keccak256(endocedCombineParams);
...
contract PlatziPunks is ERC721, ERC721Enumerable, PlatziPunksDNA {
    ...
    function mint() public {
        ...
        tokenDNA[current] = deterministicPseudoRamdonDNA(current, msg.sender);
        _idCounter.increment();
        require(
            current <= maxSupply,
            "PlatziPunks would exceed total token supply"
        );
    ...
    function imageByDNA(uint256 _dna) public view returns (string memory) {
        //Search about this is correct exceds the max lengt 16 variables in the EVM
        // Otra opcion return string(abi.encodePacked(_baseURI(),"?" ,_paramsURI(_dna)));
        string memory baseURI = _baseURI();
    ...
    function tokenURI(uint256 tokenId)
        ...
        returns (string memory)
    {
        require(_exists(tokenId), "Token does not exist"); //Este require valida si el token existe, utilizando un metodo de la clase ERC721.

        uint256 dna = tokenDNA[tokenId];
        string memory image = imageByDNA(dna);

        string memory jsonURI = Base64.encode( // Contruccion de los metadatos del NFT y codificacion en base64
            abi.encodePacked(
                '{"name": "PlatziPunks #',tokenId.toString(),'",'
                '"description": "PlatziPunks new nft",',
                '"image":"',image,'"}'
            )
        );
        return
            string(abi.encodePacked("data:application/json;base64,", jsonURI)); // Retorna el metadato en base64
    }
