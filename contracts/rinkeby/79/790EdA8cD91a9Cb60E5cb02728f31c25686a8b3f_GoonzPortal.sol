...
contract GoonzPortal is ERC721, IERC721Receiver, ERC721Enumerable, Ownable {
    using Strings for uint256;
    event GoonTransported(uint256 indexed tokenId);
    event GoonReturned(uint256 indexed tokenId);
    CryptoonGoonz immutable cryptoonGoonzContract;
    string private baseURI;

    constructor(string memory _initBaseURI, address _cryptoonGoonzContract) ERC721("GPTEST", "CGPTEST") {
        setBaseURI(_initBaseURI);
        cryptoonGoonzContract = CryptoonGoonz(_cryptoonGoonzContract);
    }

    function exitPortal(uint256 tokenId) public {
        require(ownerOf(tokenId) == msg.sender, "Must be the owner of this token");
