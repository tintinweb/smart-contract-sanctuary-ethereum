...
contract VolmexRealizedVolVault is VolmexVault, VolmexRealizedVolVaultStorage {
    ...
    // vaults duration
    uint256 vaultsDuration;

    // address of gnosis multisig
    ...
    function initialize(
        ...
        require(
            ...
            "VolmexRealizedVolVault: Zero address provided for gnosis safe"
        );
    ...
    function commitAndClose() external onlyKeeper nonReentrant {
        ...
        if (assetDelta > vTokenPrice) {
            ...
            vTokenState.vTokenPnL = vTokenPrice + vTokenPrice.sub(assetDelta);
            vaultSharePrice[round] =
                minVaultDeposit[round] -
    ...
    function redeemByMarketMaker(address _vToken, uint256 _shares) external {
        require(_shares > 0, "VolmexRealizedVolVault: shares cannot be zero");
        require(
            _vToken != address(0),
            "VolmexRealizedVolVault: address cannot be zero"
        );
        require(
            _shares <= IVToken(_vToken).balanceOf(msg.sender),
            "VolmexRealizedVolVault: Insufficient balance"
        );
        uint256 amountToBeWithdraw = (
            _shares.div(10**18).mul(vTokenState.vTokenPnL)
        );

        emit VTokenRedeemed(msg.sender, amountToBeWithdraw);

        IVToken(_vToken).burn(msg.sender, _shares);
    ...
    function rollToNextVtoken(
        ...
        // TODO: Add if-else for our or gnosis auction contract
        uint256 vTokenSellAmount = IERC20Upgradeable(newVToken).balanceOf(
            address(this)
        );
        IERC20Upgradeable(newVToken).transfer(
            gnosisSafe,
            vTokenSellAmount
        );
    }
    ...
    function burnRemainingVTokens() external onlyKeeper nonReentrant {
        uint256 leftVTokenAmount = IVToken(vTokenState.currentVToken).balanceOf(
            address(this)
        );
...
library WrappedAuction {
    ...
    function claimAuctionVTokens(
        ...
        IAuction(auction).claimFromParticipantOrder(
            IVolmexRealizedVolVault(volmexRealizedVault).vTokenAuctionID(),
            orders
        );
    }
    ...
    IVolmexVault
{
    ...
    function depositFor(
        ...
        address _creditor,
        address _receiver
    ) external nonReentrant onlyVaultsController {
        require(
            _amount >= minVaultDeposit[vaultState.round],
            "VolmexVault: !minVaultDeposit"
        );
        ...
        returns (uint256)
    {
        return
            ShareMath.sharesToAsset(
                balanceOf(owner),
                vaultSharePrice[vaultState.round],
                vaultParams.decimals
            );
    }
...
library VaultLifecycle {
    ...
    function getVTokenPrice(address easyAuction, uint256 auctionID)
        ...
        returns (uint256)
    {
        AuctionType.AuctionData memory auctionData = IAuction(easyAuction)
    ...
    function _getPremium(address _easyAuction, uint256 _auctionID)
        ...
        returns (uint256)
    {
        AuctionType.AuctionData memory auctionData = IAuction(_easyAuction)
