...
contract PlatziPunksDNA {
    ...
    // This function is deterministic and should not be used in production
    // an alternative is chainlink, that give us tools like oracules
        ...
        returns (uint256)
    {
        ...
        // La funcion keccak256 genera hashes, que son una funcion 
        // deterministica que dada una entrada siempre retorna una salida
        // pero ante la presencia de un leve cambio, todo el hash es alterado
    ...
    /*function _getURI(uint256 _dna) public view returns (string memory) {
        string memory res = "https://avataaars.io/?";
        string memory accessoriesType= _accessoriesType[_getDNASection(_dna, 0) % _accessoriesType.length];
        string memory clotheColor= _clotheColor[_getDNASection(_dna, 2) % _clotheColor.length];
        string memory clotheType= _clotheType[_getDNASection(_dna, 4) % _clotheType.length];
        string memory eyeType= _eyeType[_getDNASection(_dna, 6) % _eyeType.length];
        string memory eyebrowType= _eyebrowType[_getDNASection(_dna, 8) % _eyebrowType.length];
        string memory facialHairColor= _facialHairColor[_getDNASection(_dna, 10) % _facialHairColor.length];        
        string memory facialHairType= _facialHairType[_getDNASection(_dna, 12) % _facialHairType.length];
        string memory hairColor= _hairColor[_getDNASection(_dna, 14) % _hairColor.length];
        string memory hatColor= _hatColor[_getDNASection(_dna, 16) % _hatColor.length];
        string memory graphicType= _graphicType[_getDNASection(_dna, 18) % _graphicType.length];
        string memory mouthType= _mouthType[_getDNASection(_dna, 20) % _mouthType.length];
        string memory skinColor= _skinColor[_getDNASection(_dna, 22) % _skinColor.length];
        string memory topType= _topType[_getDNASection(_dna, 24) % _topType.length];
...
contract PlatziPunks is ERC721, ERC721Enumerable, PlatziPunksDNA {
    ...
    uint256 public NFTBasePrice;
    // index to hash
    mapping(uint256 => uint256) public tokenDNA;

    Counters.Counter private _idCounter;

    constructor(uint256 _maxSupply, uint256 _NFTBasePrice) ERC721("PlatziPunks", "PLPKS") {
        maxSupply = _maxSupply;
        NFTBasePrice = _NFTBasePrice;
    }

    event NewNFT(string message, address owner, uint256 left);    

    // Crea o genera un nft
    function mint() public payable {
        uint256 current = _idCounter.current();
        // Si quieren enviar más ;) ;)
        require(msg.value >= NFTBasePrice, "You need to send more ether, sorry");
        require(current < maxSupply, "The NFT limit has been reached");
        tokenDNA[current] = deterministicPseudoRandomDNA(current, address(msg.sender));

        _safeMint(msg.sender, current);
        _idCounter.increment();
        emit NewNFT("A new NFT has been created", address(msg.sender), maxSupply - current - 1);        
    }
        ...
        returns(string memory) 
    {
        ...
        string memory image = _paramsURI(tokenDNA[tokenId]);
        string memory jsonURI = Base64.encode(abi.encodePacked(
            '{"name": "PlatziPunks #',
            tokenId.toString(),
            '", "description": "Platzi Punks are randomized Avataaars stored on chain to teach DAaps development on Platzi", "image": "',
            image,
            '"}'
        ));
