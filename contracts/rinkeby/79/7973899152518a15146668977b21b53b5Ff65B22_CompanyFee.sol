...
contract CompanyFee is Ownable {
    ...
    function getAgentFee(address agent) public view returns (uint256) {
        uint256 fee = agentFee[agent];
        return fee;
    }

    // function getCompanyFee(address company) public view returns (uint256) {
    //     uint256 fee = companyFee[company];
    //     return fee;
    // }

    function pay(address agent, uint256 amount) public virtual returns (bool) {
