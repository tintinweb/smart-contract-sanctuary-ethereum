...
// this function allow to add a player
    function addPlayer(string memory _pseudo) public {
        require(countPlayer <=4, "Max mint is 4");
        // Malo is weak so we do not use randomisation
        if(keccak256(bytes(_pseudo)) == keccak256(bytes("Malo"))){
             playerInfo[countPlayer] = player(_pseudo, 1, 2, 3);
        }
        else {
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) { 
        ...
        return( string(abi.encodePacked(
           ...
           '","image":"https://ipfs.io/ipfs/QmeYgnXz747EQaN7cA76U11nUz3eVUXYYerVrX9NfH2fVZ/',Strings.toString(tokenId),'.png",',
