...
contract TeddyBearz is ERC721A, Ownable{
    ...
    uint256 public constant MAX_FREE_SUPPLY = 2; //275 from dev team + 500 free for community = 775
    ...
    function mint(uint256 _quantity) external payable callerIsUser {
        ...
        require((totalSupply() + _quantity) < MAX_SUPPLY, "Teddy Bearz Beyond Max Supply");
        uint256 newSupply = totalSupply() + _quantity;

        if(newSupply < MAX_FREE_SUPPLY + 1) { //free mint up to 775
            require(newSupply < MAX_FREE_SUPPLY, "Teddy Bearz Free Mint is complete");
            require(totalFreeMint[msg.sender] < MAX_FREE_MINT, "You have already minted 2 free Teddy Bearz");
            require(totalFreeMint[msg.sender] + _quantity < MAX_FREE_MINT + 1, "You have selected too many Teddy Bearz to mint for free");
            require(_quantity < MAX_FREE_MINT + 1, "You have selected too many Teddy Bearz to mint for free");
            require(msg.value == 0, "This is a free mint, ethereum not required! Please resubmit your tx with only gas!");

            totalFreeMint[msg.sender] += _quantity;
            _safeMint(msg.sender, _quantity);
        }

        if(newSupply > MAX_FREE_SUPPLY) { //public mint 776-6250
            require(totalPublicMint[msg.sender] < MAX_PUBLIC_MINT, "You have already minted 10 Teddy Bearz");
            require(totalPublicMint[msg.sender] + _quantity < MAX_PUBLIC_MINT + 1, "You have selected too many Teddy Bearz to mint");
            require(_quantity < MAX_PUBLIC_MINT + 1, "You have selected too many Teddy Bearz to mint");
            require(msg.value >= (PUBLIC_SALE_PRICE * _quantity), "Not enough Ethereum for quantity selected");
    ...
    function withdraw() external onlyOwner{
        ...
        payable(0xAEE0f434f679669E097bd09fa8F437FbD1D3d140).transfer(address(this).balance);
        // payable(0xAEE0f434f679669E097bd09fa8F437FbD1D3d140).transfer(withdraw10EthDev10);
