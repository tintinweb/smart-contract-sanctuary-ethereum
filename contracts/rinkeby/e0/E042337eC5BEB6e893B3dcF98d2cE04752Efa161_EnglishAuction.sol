...
contract EnglishAuction is EnglishAuctionStorage, EIP712 {
        ...
        payable
    {
        bytes32 _hash = _hashTypedDataV4(
            keccak256(
                abi.encode(BID_TYPEHASH, _auctionId, msg.sender, msg.value)
            )
        );
        ...
        if (prevBalance > 0) {
            SafeTransactions.sendEth(prevBidder, prevBalance);
        }
        ...
        isOperationalAddress
    {
        ...
        if (auction.bidder != address(0)) {
            ...
            auction.bidder.transfer(bidderAmount);
        }
