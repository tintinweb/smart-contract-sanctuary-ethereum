...
contract Sale is ERC721Holder, ERC1155Holder, Ownable, Initializable {
    ...
    function bidSingle(uint256 _saleId) external payable onlyWinner(_saleId) idValid(_saleId) {
        ...
        sale.isSoldOut = true;
        (uint256 percentAdminFee, uint256 adminFee, uint256 royaltyFee, uint256 remainingProfit) = _sharing(_project.isCreatedByAdmin, _project.id, sale.tokenId);
        if (_project.isCreatedByAdmin) {
            emit BidSingleProjectAdmin(_msgSender(), _saleId, sale.tokenId, royaltyFee, msg.value);
        } else {
            emit BidSingleProjectUser(_msgSender(), _saleId, sale.tokenId, percentAdminFee, adminFee, royaltyFee, remainingProfit);
        }
    ...
    function bidMulti(uint256 _saleId, uint256 _amount) external payable onlyWinner(_saleId) idValid(_saleId) {
        ...
        (uint256 percentAdminFee, uint256 adminFee, uint256 royaltyFee, uint256 remainingProfit) = _sharing(_project.isCreatedByAdmin, _project.id, sale.tokenId);
        if (_project.isCreatedByAdmin) {
            emit BidMultiProjectAdmin(_msgSender(), _saleId, sale.tokenId, _amount, royaltyFee, msg.value);
        } else {
            emit BidMultiProjectUser(_msgSender(), _saleId, sale.tokenId, _amount, percentAdminFee, adminFee, royaltyFee, remainingProfit);
        }
        IOSB1155(_project.token).safeTransferFrom(address(this), address(distribution), sale.tokenId, _amount, "");
    }

    function _sharing(bool isAdmin, uint256 projectId, uint256 tokenId) private returns(uint256, uint256, uint256, uint256) {
        ...
        if (isAdmin) {
            ...
            remainingProfit = msg.value.sub(royaltyFee);
            payable(project.getSuperAdmin()).transfer(remainingProfit);
        } else {
            ...
            remainingProfit = msg.value.sub(adminFee).sub(royaltyFee);

            payable(project.getSuperAdmin()).transfer(adminFee);
            payable(project.getManager(projectId)).transfer(remainingProfit);
        }
        return (percentAdminFee, adminFee, royaltyFee, remainingProfit);
    }
	...
	function getRoyaltyInfo(uint256 _projectId, uint256 _tokenId, uint256 _salePrice) public view returns (address, uint256) { 
        ...
        if (nftChecker.isImplementRoyalty(_project.token)) {
            return _project.isSingle ? IOSB721(_project.token).royaltyInfo(_tokenId, _salePrice) : IOSB1155(_project.token).royaltyInfo(_tokenId, _salePrice);
        } 
		return (address(0), 0);
	}
