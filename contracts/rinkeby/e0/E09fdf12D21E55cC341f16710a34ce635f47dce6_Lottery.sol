...
contract Lottery is VRFConsumerBase, Ownable {
    ...
    function endLottery(uint256 _number_of_winners) public onlyOwner {
        ...
        for (uint256 i; i < number_of_winners; i++) {
        ...
        override
    {
        ...
        winner_players.push(recent_winner);
        delete players[index_of_winner];
        players[index_of_winner] = players[players.length - 1];
        players.pop();
        randomness = _randomness;
    }

    function withdrawWinners() public onlyOwner {
        require(
            lottery_state == LOTTERY_STATE.CALCULATING,
            "You aren't there yet!"
        );
    ...
    function getWinnerPlayers() public view returns(address payable[] memory) {
