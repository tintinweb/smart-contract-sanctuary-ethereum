...
contract Primobots is ERC721A, Ownable {
    ...
    constructor(
        ...
        royalties = Royalties(payable(vault_address), 7500);
    }

    /*
        This method will reserve tokens and transfer it to vault_address (Gnosis Safe)
    */
    function reserve() external onlyOwner {
        _safeMint(vault_address, 200);
    }

    /*
        This method sets royalties only by owner
    */
    function setRoyalties(address _account, uint96 _value) external onlyOwner {
        require(_account != address(0x0), "zero address");
        require(_value >= 0 && _value < 10000, "out of bounds");
        royalties = Royalties(payable(_account), _value);
    }

    function fixIpfsHash(string memory _ipfsHash) external onlyOwner {
    ...
    function lockCollection() external onlyOwner {
        require(!is_collection_locked, "already locked");
    ...
    */
    function buyNFT(bytes32[] calldata _merkleProof, uint256 _mintQuantity)
        public
        payable
    {
        ...
        require(
            _mintQuantity >= 1 && _mintQuantity <= MAX_LIMIT,
            "Amount should be at least 1 and must be less or equal to 2"
        );
        require(_mintQuantity + totalSupply() <= cap, "Hard cap reached");
        require(
            minted[msg.sender] + _mintQuantity <= mintLimit,
            "Out of limit"
        );
        minted[msg.sender] += uint8(_mintQuantity);
        ...
        returns (address receiver, uint256 royaltyAmount)
    {
        ...
        Royalties storage _royalties = royalties;
        ...
        returns (string memory)
    {
        if (is_collection_revealed == true) {
            ...
            return
                string(
                    abi.encodePacked(_baseURI(), ipfsHash, "/", _tknId, ".json")
                );
        } else {
