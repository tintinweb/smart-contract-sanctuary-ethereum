...
contract Refund {
    ...
    function payEmployee(address empAddress, uint256 amount) public payable{
        require(msg.sender == owner);
        require(amount > 0);
        require(employeeCondition[empAddress].condition);
        // require(employeeCondition[empAddress].status == "Verified");
        payable(empAddress).transfer(amount);
  }
