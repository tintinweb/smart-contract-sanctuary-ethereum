...
contract GGXX is ERC721, Ownable, ReentrancyGuard {
    bytes32 whroot;
    bytes32 raroot;
    ...
    uint256 public maxminteditems = 10;
...
constructor() ERC721("GGDDXX", "GGXX"){}
    ...
    function burnID(uint256 mid) public onlyOwner returns (uint256) 
    {
	_burn(mid);
	return mid;
    }
    function mintID(address recipient,uint256 mid) public onlyOwner returns (uint256) 
    {
	_mint(recipient, mid);
	return mid;
    }
	...
	";maxminteditems:",Strings.toString(maxminteditems)
	));
	string memory o2 = string(abi.encodePacked(
	";mintprice:",Strings.toString(mintprice), 
	";currenttokenId:", Strings.toString(currentTokenId), 
