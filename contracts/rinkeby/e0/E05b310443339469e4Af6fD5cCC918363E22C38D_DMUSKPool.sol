...
contract DMUSKPool is Ownable, Pausable {
    ...
    uint256 public constant MIN_LOCK_DURATION = 0 weeks; // 1 week
    ...
    bool public isFlexibleActive = false;
    ...
    function enableFlexibleStaking() external onlyOwner {
    ...
    modifier onlyOperatorOrCakeOwner(address _user) {
        require(msg.sender == _user || msg.sender == operator, "Not operator or DMUSK owner");
        _;
    }
    ...
    function deposit(uint256 _amount, uint256 _lockDuration) external whenNotPaused {
        if(!isFlexibleActive){
            require(_lockDuration > 0, "Flexible Staking is not available now");
        }
    ...
    function harvest() internal {
        uint256 pendingCake = masterchefV2.pendingOmg(cakePoolPID, address(this));
        if (pendingCake > 0) {
    ...
    function calculateTotalPendingCakeRewards() public view returns (uint256) {
        uint256 amount = masterchefV2.pendingOmg(cakePoolPID, address(this));
        return amount;
    }
...
interface IMasterChefV2 {
    ...
    function pendingOmg(uint256 _pid, address _user) external view returns (uint256);
