   ...
   function withdrawInAmd(uint256 count) public {
    ...
    function extendStake(uint256 count,uint256 newplan) public {
       ...
       require(user.stakerecord[count].plan < newplan, "Can not extend to lower plan");
        ...
        newDays = ((((currentTime  - timeBefore) / 60) /60) /24);
    ...
    function calculateNewDays(uint256 _newDuration) public pure returns(uint256) {
      uint256 calculatedDays = 1 days;
      calculatedDays = calculatedDays * _newDuration;
      return calculatedDays;
    }

    function calculateNewPercentage(uint256 plan,uint256 _totalDays) public {
      newPercentage = APYtoPercentage(APY[plan], _totalDays.div(1 days));
    }

    function calculateNewReward(address userAddress,uint256 userStakeCount, uint256 amount, uint256 _newDuration, uint256 _oldPlan) public returns(uint){
           ...
           User storage user = users[userAddress];
            ...
            newReward = user.stakerecord[userStakeCount].beforeExtendBonus;
            return newReward;
    }
