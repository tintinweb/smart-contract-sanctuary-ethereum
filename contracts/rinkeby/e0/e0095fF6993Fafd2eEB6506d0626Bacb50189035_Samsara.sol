...
// Creator:  Saṃsāra Rebels

pragma solidity ^0.8.4;

import {NonDilutiveA} from "./NonDilutiveA.sol";
...
contract Samsara is Ownable, NonDilutiveA, ReentrancyGuard {
    using Address for address;
    using Strings for uint256;

    uint256 public cost = 70000000000000000; //0.07 ETH
    uint256 public constant WL_MAX_MINT = 4;
    uint256 public constant PUBLIC_MAX_MINT = 3;
    uint256 public constant RESERVED_TOKENS = 3;
    uint256 public constant UNIQUE_RESERVED = 1;
    bytes32 public root = 0x96a07f016379a51edbe26c80aea9c69b51d61c763abee6beafddedd867d293d7;
    bool paused;
    bool public wlMinting; 
    mapping(address => uint256) private wlMints;
    mapping(address => uint256) private publicMints;
    // mapping that contains an array of strings, with each index corresponding to a generation
    mapping(uint256 => string[]) private uniques;
    ...
    constructor(
        ...
        ,uint256 _MAX_SUPPLY
    ) NonDilutiveA(_name, _symbol, _baseUnrevealedURI, _baseURI, _MAX_SUPPLY) {
    ...
    modifier notPaused() {
        require(paused == false, "Minting has been paused");
        _;
    }
        ...
        ) 
    {
        ...
        // Check if it's a reserved 1/1 (images for those will be created after contract deployment)
        if(_tokenId < UNIQUE_RESERVED) {
            uint256 activeGenerationLayer = tokenToGeneration[_tokenId];
            string[] memory uniqueBaseUris = uniques[_tokenId];
            if(uniqueBaseUris.length <= activeGenerationLayer){
                //uri doesn't exist for this generation
                return string(abi.encodePacked(baseUnrevealedURI,"/",_tokenId.toString(),".json"));
            }
            return string(
            abi.encodePacked(
                 uniqueBaseUris[activeGenerationLayer]
                ,"/"
                ,_tokenId.toString()
                ,".json"
                )
        ...
        return _tokenURIMimetic(_tokenId);
    }

    // Minting
    function mintPublic(uint16 amount) external payable nonReentrant callerIsUser notPaused
    {   
        require(mintOpen, "Minting for public is not enabled!");
        require(publicMints[msg.sender] + amount <= PUBLIC_MAX_MINT, "Mint exceeds allowed amount");
        require(totalSupply() + amount <= MAX_SUPPLY, "Mint exceeds total supply");
        require(msg.value == (cost * amount), "Cost mismatch");
        publicMints[msg.sender] += amount;
        _safeMint(msg.sender, amount);
    }

    function mintWhitelist(uint16 amount, bytes32[] calldata proof) external payable nonReentrant callerIsUser notPaused
    {   
        require(wlMinting, "Minting for whitelist is not enabled!");
        require(wlMints[msg.sender] + amount <= WL_MAX_MINT, "Mint exceeds allowed amount!");
        require(totalSupply() + amount <= MAX_SUPPLY, "Mint exceeds total supply");
        require(msg.value == (cost * amount), "Cost mismatch");
    ...
    function setBaseUnrevealedUri(string memory newUri) public onlyOwner {    
        baseUnrevealedURI = newUri;
    }

    function setUniqueUri(uint256 id, uint256 generation, string memory newUri) public onlyOwner {
        if(uniques[id].length <= generation){
            //generation doesn't exist
            uniques[id].push(newUri);
        }
        else{
            uniques[id][generation] = newUri;
        }
    }
    ...
    {
        (bool dev, ) = payable(
            0xa17398F3F2B8b5e1dE2b0225bE5886A3d86cA244
        ).call{
            value: address(this).balance * 15 / 100
        }("");
        if(!dev) revert WithdrawFailed();
...
/**
 * @title  Non-Dilutive 721A
 * @author nftchance
 * @notice Gaiseric has slightly modified this contract to include Azuki's functionalities as well
 * @dev Gaiseric modifications:
         - contract is ERC721A now and constructor creates ERC721A
         - move tokenURI to parent contract
         - remove mint in constructor
         - remove mint function
         - setRevealed now has to call tokenId + 1 (or simply totalSupply amount if you want to reveal everything)
    ...
    ,INonDilutive
{
    ...
    constructor(
        ...
        loadGeneration(
            ...
            ,false          // locked    (cannot be removed by project owner)
            ,false          // sticky    (cannot be removed by owner)
 ...
 * @author nftchance
 * @notice Gaiseric changed tokenURI so it would not conflict with azuki token uri
 * @dev Gaiseric modifications:
        - tokenURI -> tokenURIMimetic
        -  >= activeGeneration.top instead of > activeGeneration.top (now tokenUri works for tokenId 0)
        - remove getGenerationToken()
        - disable offset functionality
        - allow overwriting existing generations
 * @notice This token was created to serve as a proof for a conversational point. Non-dilutive 721 
 *         tokens can exist. Teams can easily build around this concept. Teams can additionally  
 *         still monetize the going ons and hard work of their team. However, that does not need to 
 *         come at the cost of their holders. As it stands every token drop following the 
    ...
    ,Ownable
{
        ...
        ) 
    {
         ...
         */
        if(_tokenId >= activeGeneration.top){
            return string(
            abi.encodePacked(
                baseUnrevealedURI
                ,"/"
