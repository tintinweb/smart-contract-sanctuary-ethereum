...
contract MetaBrain is IMetaBrain, IERC721Receiver, Ownable, ReentrancyGuard {
    ...
    event Fought(uint indexed anft, uint indexed playerId, uint randNum, uint normalisedRandNum, uint attack, bool won);
    ...
    mapping(uint => mapping(uint => mapping(uint => TeamSummon))) public anftToTeamSizeToPlayerIdToTeamSummon;
    mapping(uint => mapping(uint => uint)) public anftToTeamSizeToTeamSummonTime;
    ...
    // CURRENTLY DOING: creating the summonWithTeam fcn, also creating the boss struct to hold
    // the team summon counts
    ...
    function summonWithTeamAndFight(
        ...
        if (summonCount >= teamSize) {
            _fightBoss(playerId, anftTokenId, powerupPaths, erc20Deps, nftDeps);
        }
    }
    ...
    function _summonWithTeam(
        ...
        require(password.length > 1);
        require(teamSize >= MIN_TEAM_SIZE);
        uint lastExecTime = _lastExecTimes[anftTokenId];
        require(anftToTeamSizeToPlayerIdToTeamSummon[anftTokenId][teamSize][playerId].time < lastExecTime);
        require(anftToTeamSizeToTeamSummonTime[anftTokenId][teamSize] < lastExecTime);
        
        BossConfig memory config = bossConfigs[anftTokenId];
        uint teamId = getTeamId(teamSize, config.epochRandNum, playerId);
        ...
        for (uint i; i < teamPoolPlayerIds.length; i++) {
            ...
            require(getTeamId(teamSize, config.epochRandNum, teamPoolPlayerIds[i]) == teamId);
            // Check that the passwords are the same for all, so they actually communicated
            TeamSummon memory teamSummon = anftToTeamSizeToPlayerIdToTeamSummon[anftTokenId][teamSize][teamPoolPlayerIds[i]];
            if (teamSummon.time >= lastExecTime && keccak256(teamSummon.password) == keccak256(password)) {
                ...
                if (summonCount >= teamSize) {
                    anftToTeamSizeToTeamSummonTime[anftTokenId][teamSize] = block.timestamp;
                    // anft.safeTransferFrom(anft.ownerOf(anftTokenId), newOwner, anftTokenId);
                    break;
                }
            }
        ...
        anftToTeamSizeToPlayerIdToTeamSummon[anftTokenId][teamSize][playerId] = TeamSummon(uint32(block.timestamp), password);
    ...
    function getTeamId(uint teamSize, uint epochRandNum, uint playerId) public view returns (uint) {
        return uint(keccak256(abi.encodePacked(epochRandNum, playerId))) % (IERC721Enumerable(address(players)).totalSupply() / teamSize);
    }

    function getTeamIdByAnftId(uint teamSize, uint anftTokenId, uint playerId) external view returns (uint) {
        uint epochRandNum = bossConfigs[anftTokenId].epochRandNum;
        return uint(keccak256(abi.encodePacked(epochRandNum, playerId))) % (IERC721Enumerable(address(players)).totalSupply() / teamSize);
    }

    function getTeamSummonTimes(uint anftTokenId, uint teamSize, uint playerId) external view returns (uint) {
        return anftToTeamSizeToPlayerIdToTeamSummon[anftTokenId][teamSize][playerId].time;
    }
    ...
    function summonWithItem(
        ...
        if (summonSet.spend) {
            ...
            require(lastSummonNoSpendTime[anftTokenId][summonSet.nft][summonPath.tokenId] < _lastExecTimes[anftTokenId]);
    ...
    function fightBoss(
        ...
        return _fightBoss(playerId, anftTokenId, powerupPaths, erc20Deps, nftDeps);
    }
    ...
    function _fightBoss(
        ...
        for (uint i; i < powerupPaths.length; i++) {
            ...
            if (powerupSet.tokenIds.length != 0) {
                require(powerupSet.tokenIds[powerupPaths[i].tokenIdIdx] == powerupPaths[i].tokenId);
            }
            ...
            if (powerupSet.spend) {
                ...
                require(powerupSet.nft.ownerOf(powerupPaths[i].tokenId) == msg.sender);
            }
        ...
        if (won) {
            ...
            for (uint i; i < erc20Deps.length; i++) {
                ...
                require(erc20Balances[anftTokenId][erc20Deps[i].token] <= erc20Deps[i].amount);
            ...
            for (uint i; i < nftDeps.length; i++) {
                require(nftBalances[anftTokenId][nftDeps[i].nft][nftDeps[i].id]);
        ...
        for (uint i; i < powerupDeps.length; i++) {
            if (powerupDeps[i].nft != IERC721(address(0))) {
                _depositNFT(anftTokenId, powerupDeps[i]);
            }
        }

        emit Fought(anftTokenId, playerId, brainOracle.getRandFightNum(), normalisedRandNum, attack, won);

        _sendToRandomPlayer(anftTokenId, config.ownerPools);
    }
    ...
    function _depositERC20s(uint anftTokenId, ERC20Deposit[] memory erc20Deps) private {
        ...
        for (uint i; i < erc20Deps.length; i++) {
            require(IERC20(erc20Deps[i].token).balanceOf(address(this)) - prevBals[i] == erc20Deps[i].amount);
        }
    }
    ...
    function newBoss(
        ...
        bossConfigs[anftTokenId] = config;
        _lastExecTimes[anftTokenId] = lastExecTime;
        // _newReqSendToPlayer(referer, callData);
    }

    function getBossConfig(uint anftTokenId) external view returns (BossConfig memory) {
        return bossConfigs[anftTokenId];
    }

    function setBossConfig(uint anftTokenId, BossConfig calldata newConfig) external onlyOwner {
        bossConfigs[anftTokenId] = newConfig;
    }
    ...
    //     _lastExecTimes[anftTokenId] = startTime + epochLength;

    //     bossConfigs[anftTokenId].epochRandNum = _sendToRandomPlayer(anftTokenId, ownerPools);
    ...
    function _sendToRandomPlayer(
        ...
        randNum = oracle.getRandNum(block.number-1);
        address ownerPool = ownerPools[randNum % ownerPools.length];
        address newOwner = IERC721(ownerPool).ownerOf(randNum % IERC721Enumerable(ownerPool).totalSupply());
    ...
    function getLastExecTime(uint tokenId) external view returns (uint) {
        return _lastExecTimes[tokenId];
    }
    ...
    modifier isPlayer(uint playerId) {
        emit Test(msg.sender, players.ownerOf(playerId));
        require(msg.sender == players.ownerOf(playerId));
        _;
    }
...
interface IMetaBrain is IBrain {
