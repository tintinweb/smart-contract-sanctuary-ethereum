...
// File contracts/ApplyLiquidityUpkeep.sol
...
contract ApplyLiquidityUpkeep is KeeperCompatibleInterface, Ownable {
    ...
    AggregatorV3Interface public fastGasFeed;
    ...
    function setMaxGasPrice(int256 _maxGasPrice) external onlyOwner {
        require(_maxGasPrice > 0, "Price too low");
        ...
        returns (bool upkeepNeeded, bytes memory performData)
    {
        ...
        if (lpBalance >= 10000 * 10**18) {
            upkeepNeeded = feedValue <= maxGasPrice;
        } else {
            upkeepNeeded =
                lpBalance > 0 &&
                (block.timestamp - lastTimeStamp) > interval &&
                feedValue <= maxGasPrice;
        }
    ...
    function performUpkeep(
        ...
        require(lpBalance > 0, "No LP");

        if (lpBalance < 10000 * 10**18) {
            require((block.timestamp - lastTimeStamp) > interval, "Too early");
        }

        require(feedValue <= (maxGasPrice * 125) / 100, "Gas too high");
