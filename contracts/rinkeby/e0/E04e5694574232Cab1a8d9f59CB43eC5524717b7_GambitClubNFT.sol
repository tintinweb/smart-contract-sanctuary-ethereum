    ...
    TokenRescuer
{
    ...
    uint256 public constant MAX_ROYALTIES_PCT = 1000; // 10% // TODO
    ...
    {
        if (!_isValidProof(_proof, whitelistMerkleRoot))
    ...
    {
        if (!_isValidProof(_proof, presaleMerkleRoot))
            revert InvalidMerkleProof();
    ...
    /// @notice Mints token ID `_tokenID` to `_to`.
    function _mint(
        address _to,
        uint256 _tokenID
    )
    ...
    /// @notice Checks whether the merkle proof `_proof` is valid for the caller.
    ...
    {
        bytes32 leaf;
        bytes20 addr = bytes20(_msgSender());
        assembly {
            mstore(0x00, addr)
...
// Implementation by @nftchance, @masonnft & @squeebo_nft
