...
import "./core/ERC721Panda.sol";

contract ERC721P is ERC721Panda, Ownable, ReentrancyGuard {
    ...
    //https://ipfs.io/ipfs/QmU61BwmB9fm3kN4EWS14YxrB1FFJcMWj9GRrf4hsEvaYE
    constructor(string memory initURI, address signer1, address signer2) ERC721Panda("PandaNFT", "PANDA", 10000) {
        _uri = initURI;
        maxTotalSupply = 5556;
        PreMaxMint = 2;
        PublicMaxMint = 1;
    ...
    function _baseURI() internal view  override(ERC721Panda) returns (string memory) {
        return _uri;
    }
    ...
    function mint(uint256 num, string calldata salt, bytes calldata token) external payable {
        require(status == Status.PublicSale, "PandaDAO:");
        require(_verify(_hash(salt, msg.sender), token) >= num, "PandaDAO:");
        require(publicNumberMinted(msg.sender) + num <= PublicMaxMint, "PandaDAO:");
        verified(num);
        _safeMint(msg.sender,num);
        _publicNumberMinted[msg.sender] = _publicNumberMinted[msg.sender] + 1;
    }

    function firstMint(uint256 num, address receiver) external {
        _safeMint(receiver,num);
    }
    ...
    function preSaleMint(
        ...
        require(status == Status.PreSale, "PandaDAO:");
        uint256 preMaxMint = _verify(_hash(salt, msg.sender), token);
        require(preMaxMint >= amount, "PandaDAO:");
        require(numberMinted(msg.sender) + amount <= preMaxMint, "PandaDAO:");
        verified(amount);
    ...
    function verified(uint256 num) private {
        require(num > 0, 'PandaDAO:');
        require(msg.value >= PRICE * num, 'PandaDAO:');
        if (msg.value > PRICE * num) {
            payable(msg.sender).transfer(msg.value - PRICE * num);
        }
        require(totalSupply() + num <= maxTotalSupply, "PandaDAO:");
        require(tx.origin == msg.sender, "PandaDAO:");
    }
    ...
    function publicNumberMinted(address owner) public view returns (uint256) {
        require(
            owner != address(0),
            "ERC721Panda: number minted query for the zero address"
        );
        return _publicNumberMinted[owner];
    }

    function release() public virtual nonReentrant onlyOwner{
        require(address(this).balance > 0, "PandaDAO:");
 ...
 * todo：目前只是集成了721A的功能，需要改造支持批量传送
 */
contract ERC721Panda is
Context,
ERC165,
IERC721,
IERC721Metadata,
IERC721Enumerable
{
    ...
    function approve(address to, uint256 tokenId) public override {
        address owner = ERC721Panda.ownerOf(tokenId);
