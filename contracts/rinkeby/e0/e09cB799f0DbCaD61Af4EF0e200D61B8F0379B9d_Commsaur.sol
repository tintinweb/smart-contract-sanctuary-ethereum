...
contract Commsaur is ERC721A, Ownable {
    ...
    mapping(address => bool) mintedWhiteList;

    constructor() ERC721A("Commsaur", "COMMSAUR") {}

    // @notice Once the Dino Token is released, call this function to set the contract address
    function setDinoToken(address _address) external onlyOwner {
        dinoToken = IDinoToken(_address);
    }
    ...
    function mintCommsaurs(uint256 commsaursToMint) external payable {
        require(tx.origin == msg.sender, "CM: Caller must be a real user");
        require(
            mintStatus == MintStatus.PUBLIC,
            "CM: Minting is currently closed!"
        );
        require(
            totalSupply() + commsaursToMint < MAX_SUPPLY,
            "CM: Minting would exceed max supply"
        );
        require(commsaursToMint > 0, "CM: Must mint at least one Commsaur");
        require(
            commsaursToMint <= 5,
            "CM: You can only mint a max of 5 Commsaurs per tx"
        );
        require(
            msg.value == MINT_PRICE * commsaursToMint,
            "CM: Ether value sent is not correct"
        );
    ...
    // @notice Allows minting during white list, merkle proof is provided by the website
    function mintWhiteList(bytes32[] calldata _merkleProof) external payable {
        require(tx.origin == msg.sender, "CM: Caller must be a real user");
        require(
            mintStatus == MintStatus.WHITE_LIST,
            "CM: White list claiming is closed"
        );
        require(
            mintedWhiteList[msg.sender] == false,
            "CM: You have already claimed your white-list Commsaur"
        );
        require(msg.value == MINT_PRICE, "CM: Ether value sent is not correct");

        bytes32 leaf = keccak256(abi.encodePacked(msg.sender));
        require(
            MerkleProof.verify(_merkleProof, merkleRoot, leaf),
            "CM: Merkle proof was invalid"
        );
    ...
    function reserveTokens() external onlyOwner {
        require(reserved > 0, "CM: Out of reserved tokens!");
        require(
            mintStatus == MintStatus.RESERVED,
            "CM: Reserved minting was closed"
        );
    ...
    function refundEthOverflow(uint256 amount) private {
        require(msg.value >= amount, "CM: Not enough ETH sent.");
        if (msg.value > amount) {
