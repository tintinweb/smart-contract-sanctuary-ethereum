...
contract TestB is Ownable, ERC721A {

    uint256 public allowListPrice = 0.00300000 ether;
    uint256 public publicSalePrice = 0.00300000 ether;

    bool public allowListStatus = false;
    bool public publicSaleStatus = false;
    uint256 public allowListAmount = 100;

    uint256 public immutable maxPerAddressDuringMint = 3;
    uint256 public immutable MAX_SUPPLY = 888;
    ...
    constructor() ERC721A("Miner Monkey Club", "MMC") {
    }
    ...
    function allowListMint(uint256 quantity, bytes32[] memory proof) external payable callerIsUser {
        ...
        require(totalSupply() + quantity <= allowListAmount, "reached allowlist mint max supply");
    ...
    function publicSaleMint(uint256 quantity) external payable callerIsUser{
        ...
        require(
            numberMinted(msg.sender) + quantity <= maxPerAddressDuringMint + allowListStock[msg.sender],
            "reached public sales per address mint amount"
        );
    ...
    function mint_for_airdrop(uint256 tokenQuantity) external onlyOwner {
        require(
            totalSupply() + tokenQuantity <= MAX_SUPPLY,
            "Sale would exceed max supply"
        );        
        require(allowListStatus, "Sale must be active to mint ");
