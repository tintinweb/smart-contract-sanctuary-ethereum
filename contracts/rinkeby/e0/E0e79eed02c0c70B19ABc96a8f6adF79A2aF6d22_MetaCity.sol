...
contract MetaCity is ERC721, Ownable {
    ...
    uint16 public taxmanStolen = 0;
    uint16 public tieraStolen = 0;
    uint16 public taxmanMinted = 0;
    uint16 public tierbMinted = 0;
    uint16 public tiercMinted = 0;
    ...
    constructor(
        ...
        phasePrice[1] = 0.005 ether;
        phasePrice[2] = 20 ether;
        phasePrice[3] = 40 ether;
        phasePrice[4] = 80 ether;
    ...
    function switchToSalePhase(uint16 _phase, bool _setTokens) public onlyOwner {
        ...
        if (phase == 1) {
            internalAddTokens(1, 10);
        } else if (phase == 2) {
            internalAddTokens(11, 20);
        } else if (phase == 3) {
            internalAddTokens(21, 40);
        } else if (phase == 4) {
            internalAddTokens(41, 60);
        }
    }
    ...
    function mint(uint _amount, bool _stake) public payable whenNotPaused {
        ...
        for (uint i = 0; i < _amount; i++) {
            ...
            if (recipient != msg.sender) {
                isTaxman(tokenId) ? taxmanStolen += 1 : tieraStolen += 1;
            ...
            if (!_stake || recipient != msg.sender) {
                ...
                _safeMint(address(bank), tokenId);
                tokenIds[i] = tokenId;
            }
        }
        if (_stake) {
            bank.addTokensToStake(msg.sender, tokenIds);
        }
    }
    ...
    function selectRecipient() internal returns (address) {
        ...
        // 0% chance to steal NTF
        if (getSomeRandomNumber(taxmanMinted, 100) >= 0) {
            return msg.sender; // 100%
        }

        address thief = bank.randomTaxmanOwner();
    ...
    function setTaxmanId(uint16 id, bool special) external onlyOwner {
        _isTaxman[id] = special;
    }
    ...
    function setBank(address _island) external onlyOwner {
        bank = IBank(_island);
    }
