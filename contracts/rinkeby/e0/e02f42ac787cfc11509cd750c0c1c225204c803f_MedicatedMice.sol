...
contract MedicatedMice is ERC721A, Ownable {
  ...
  uint256 public miceListSupply = 3;
  ...
  constructor() ERC721A("Test Mice", "TMICE") {
    setHiddenMetadataUri("https://medicatedmice.mypinata.cloud/ipfs/QmXoxUByEESqZicptzhp48fT9CwHi4jvBSD2wuKnf1ntU3/hidden.json");
  }

  modifier mintCompliance(uint256 _mintAmount) {
    require(!paused, "The contract is paused!");
    require(_mintAmount > 0 && _mintAmount <= maxMintAmountPerTx, "Invalid mint amount!");
    require(_mintAmount <= maxMintAmountPerTx, "Max tokens per TX");
  ...
  function mint(uint256 _mintAmount, bytes32[] calldata _merkleProof) public payable mintCompliance(_mintAmount) {
    ...
    if (msg.sender != owner()) {
        if (miceListStage == true && isWhitelisted(_merkleProof)) {
            require(_mintAmount <= miceListSupply, "MiceList Max exceeded!");        
        } else if (preSaleStage == true  && isWhitelisted(_merkleProof)) {
            require(_mintAmount <= preSaleSupply, "PreSale Supply Max exceeded!");  
        }
      }
