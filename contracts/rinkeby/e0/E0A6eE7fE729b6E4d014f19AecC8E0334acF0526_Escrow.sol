...
contract Escrow is Ownable, AccessControl, Pausable {
    ...
    function createCaseAndAddProof(
        uint256 _caseId,
        uint256 _proofId,
        uint256 _reward,
        uint256 _startAt,
        ...
        bytes32 message = keccak256(
            abi.encode(_caseId, _proofId, _reward, _startAt, _endAt, _amount)
        );
        require(
        ...
        caseInfo.proofs.push(proofId);

        emit CreateCaseAndAddProof(
            _caseId,
            _proofId,
            msg.sender,
            _reward,
    ...
    function addProof(
        ...
        require(
            caseInfos[_caseId].endAt >= block.timestamp,
            "Escrow.sol: caseId is close."
        );
        ...
        ProofInfo storage proofInfo = proofInfos[proofId];
        require(
            proofInfo.owner == address(0),
            "Escrow.sol: duplicate proofId."
        );
        proofInfo.caseId = _caseId;
        proofInfo.owner = msg.sender;
        proofInfo.amount = _amount;
    ...
    function getProofByCaseId(uint256 _caseId)
        public
        view
        returns (bytes32[] memory)
    {
        return caseInfos[_caseId].proofs;
    }
