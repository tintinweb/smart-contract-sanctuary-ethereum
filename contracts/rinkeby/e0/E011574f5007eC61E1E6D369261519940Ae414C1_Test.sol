...
contract Test is Context, IERC20, Ownable {
    ...
    address public pricePollWallet;
    ...
    constructor(address payable _marketingWallet, address payable _pricePollWallet) {
        _rOwned[_msgSender()] = _rTotal;

        marketingWallet = _marketingWallet;
        pricePollWallet = _pricePollWallet;
        ...
        )
    {
        ...
        tTransferAmount = tTransferAmount.sub(tMarketing).sub(tPricePoll);
        return (tTransferAmount, tFee, tLiquidity, tMarketing, tPricePoll);
    }
    ...
    function _takeMarketingAndPricePoll(uint256 tMarketing, uint256 tPricePoll)
        private
    {
        uint256 currentRate = _getRate();
        uint256 rMarketing = tMarketing.mul(currentRate);
    ...
    function calculatePricePollFee(uint256 _amount)
        private
        view
        returns (uint256)
    {
        return _amount.mul(_pricePollFee).div(10**2);
    }
    ...
    function setBuyFee() private {
        ...
        _pricePollFee = buyFee.pricePoll;
    }
    ...
    function setSellFee() private {
        ...
        _pricePollFee = sellFee.pricePoll;
    }
        ...
        lockTheSwap
    {
        ...
        uint256 forPricePoll = contractTokenBalance.sub(forLiquidity).sub(
            forMarketing
        );
        sendToPricePoll(forPricePoll);     

        emit SwapAndDistribute(forMarketing, forLiquidity, forPricePoll);
    }
    
    function sendToPricePoll(uint256 tPricePoll) private{
        uint256 currentRate =  _getRate();
        uint256 rPricePoll = tPricePoll.mul(currentRate);
        _rOwned[pricePollWallet] = _rOwned[pricePollWallet].add(rPricePoll);
        _rOwned[address(this)] = _rOwned[address(this)].sub(rPricePoll);
        if(_isExcluded[pricePollWallet])
            _tOwned[pricePollWallet] = _tOwned[pricePollWallet].add(tPricePoll);
    }
    ...
    function _transferStandard(
        ...
        _takeMarketingAndPricePoll(
            calculateMarketingFee(tAmount),
            calculatePricePollFee(tAmount)
        );
