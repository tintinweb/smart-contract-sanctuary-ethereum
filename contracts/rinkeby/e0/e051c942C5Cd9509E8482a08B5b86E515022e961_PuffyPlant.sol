...
contract PuffyPlant is ERC721A, Ownable, ReentrancyGuard, PaymentSplitter {
  ...
  using Counters for Counters.Counter;
  Counters.Counter private _noneFreeMintedTokens;
  Counters.Counter private _freeMintedTokens;
  ...
  constructor(
    ...
    _noneFreeMintedTokens.increment();
  }

  modifier mintCompliance(uint256 _mintAmount) {
    require(_mintAmount > 0 && _mintAmount <= 2, "Invalid mint amount!");
    require(
      totalMintedTokens() + _mintAmount <= maxSupply,
      "Max supply exceeded!"
    );
    ...
    mintCompliance(_mintAmount)
  {
    ...
    if (addressInFreeMerkleTree) {
      require(
        _freeMintedTokens.current() <= 1111,
        "Cant mint more tokens for free"
      );
  ...
  function mintFreeToken(uint256 _mintAmount, address _receiver) internal {
    for (uint256 i = 0; i < _mintAmount; i++) {
      uint256 tokenId = totalMintedTokens();
      _freeMintedTokens.increment();
      _safeMint(_receiver, tokenId);
    }
  }

  function mintNoneFreeToken(uint256 _mintAmount, address _receiver) internal {
    for (uint256 i = 0; i < _mintAmount; i++) {
      uint256 tokenId = totalMintedTokens();
      _noneFreeMintedTokens.increment();
      _safeMint(_receiver, tokenId);
  ...
  function giveaway(uint256 _mintAmount, address _receiver) public onlyOwner {
    mintFreeToken(_mintAmount, _receiver);
  }

  function totalMintedTokens() internal view returns (uint256) {
    return _freeMintedTokens.current() + _noneFreeMintedTokens.current();
  }
