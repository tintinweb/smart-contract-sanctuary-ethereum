...
contract VimptoWorlds is Ownable, ERC721, ReentrancyGuard ,VerifySignature{
    ...
    uint256 public mintPrice = 1000000000000000000 wei;
    ...
    function mintWhitelist(string memory _tokenURI) public payable onlyWhitelisted nonReentrant returns (uint256) {
        ...
        if (whiteListedAddresses[msg.sender] == true && partnerAddresses[msg.sender] == false) {
            require(balanceOf(msg.sender) < addressMintingLimit, "Only 3 mints allowed per address.");
        } else {
            require(balanceOf(msg.sender) < addressMintingLimit + 1, "Only 4 mints allowed for partners.");
        }
    ...
    function mintToPartner( address _address, string memory _tokenURI) public onlyOwner returns (uint256) {
        require(mintSupply > _tokenIds.current(), "All 10,000 NFT's have been minted.");
        _tokenIds.increment();
        uint256 newItemId = _tokenIds.current();
    ...
    function mintWorld(MintPayload calldata _payload, bytes memory _signature) public payable nonReentrant returns (uint256)  {
        ...
        require(balanceOf(_payload.to) < addressMintingLimit, "Only 3 mints allowed per address.");
        _tokenIds.increment();
        uint256 newItemId = _tokenIds.current();
    ...
    function verifyOwnerSignature(MintPayload calldata _payload,bytes memory _signature) private view returns(bool) {
          ...
          bytes32 ethSignedHash =getEthSignedMessageHash(getMessageHash(_payload.nonce.toString()));
    ...
    function transferrableStatus() public view returns (bool) {
