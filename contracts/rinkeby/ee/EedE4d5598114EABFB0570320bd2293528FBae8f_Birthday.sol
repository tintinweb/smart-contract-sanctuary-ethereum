...
contract Birthday is IBirthday, ERC721, ERC721Pausable, ERC721URIStorage, PullPayment, Ownable {
    ...
    */
    function mint(uint tokenId, bool mdy) external payable override returns (string memory) {
        ...
        string memory uri = generate(tokenId, mdy); // 1,226,008 gas (1,150,659 gas exc tokenToDayMonthYear call)

        _setTokenURI(tokenId, uri); // 998,403 gas

        return uri;
    }

    /**
     * @dev The generate function to return the ASCII art for the birthday date
     *      formatted in the Gregorian calendar date format day / month year.
     *      or month / day year if mdy is true.
     *      
     * @param id The token ID to generate
     * @param mdy True if the date should be formatted in the mdy format
     * @return URI of token ID.
     */
    function generate(uint id, bool mdy) public view returns (string memory) {
        require(id <= MAX_LIMIT, "Max limit");
        (uint day, uint month, uint year) = tokenToDayMonthYear(id); // 75,349 gas
        ...
        for(uint i = 0; i < rows; i++) {
            for(uint co = 0; co < columns; co++) {
                ...
                if (i >= 16 && i < 21) {
                    if (co > 9 && co < 40) {
                        uint j = co - 9;
                        output[c] = numbers[yearNums[(j / 8)]][(i - 16) * 8 + j % 8];
                    }
                } else if (i >= 5 && i < 10) {
                    if (co > 5 && co < 44) {
                        uint j = co - 5;
                        output[c] = numbers[dmNums[(j / 8)]][(i - 5) * 8 + j % 8];
                    }
                }
                c++;
            }
    ...
    function tokenToDayMonthYear(uint id) private view returns (uint _day, uint _month, uint _year) {
        ...
        for (uint j = 0; j < MONTHS; j++) {
            ...
            if (year == leapYears[leapYear] && j == 1) {
                dayCount += 29;
            } else {
