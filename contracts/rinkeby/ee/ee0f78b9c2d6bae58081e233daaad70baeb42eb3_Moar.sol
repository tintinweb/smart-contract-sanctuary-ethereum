...
// File: contracts/moar/MoarBase.sol
...
/**
    ...
    function burn(uint256 tokenId) external {
        require(isBurningActive, "Burnning inactive");
    ...
    function setRoyaltyInfo(address royaltyReceiver, uint256 royaltyPercentage) public onlyOwner {
        require(royaltyPercentage <= 100, "RoyaltyPercentage exceed");
    ...
    function royaltyInfo(uint256 tokenId, uint256 salePrice) external view override returns (address receiver, uint256 royaltyAmount){
        require(_exists(tokenId), "Query royalty info for non-existent token");
...
contract Moar is MoarBase, ReentrancyGuard {
    ...
    // ========================== Dutch Auction ===========================
    uint256 public constant DUTCH_AUCTION_MAX_TICKET = 5;
    uint256 public constant DUTCH_AUCTION_START_PRICE = 0.08 ether;
    uint256 public constant DUTCH_AUCTION_END_PRICE = 0.005 ether;
    uint256 public constant DUTCH_AUCTION_PRICE_CURVE_LENGTH = 1500 minutes;
    ...
    uint256 public dutchAuctionStartTime = 1648395900;
     ...
     */
    constructor(address minter, address authority, address royaltyAddress, uint256 maxSupply_) MoarBase(royaltyAddress) {
        _minter = minter;
    ...
    /**
     * @dev Turns on the ability to receive Ether
     */
    ...
    modifier isSaleOn(bool saleOn_) {
        require(saleOn == saleOn_ , "Invalid saleOn");
        _;
    }
    ...
    modifier validateCaller() {
        require(tx.origin == msg.sender, "Contract bidder");
        _;
    }
     ...
     * - the caller must be `owner`.
     */
    function toggleSaleOn() external onlyOwner {
        saleOn = !saleOn;
    }
     ...
     * - the caller must be `owner`.
     */
    function setDutchAuctionStartTime( uint256 startTime) external isSaleOn(false) onlyOwner {
        dutchAuctionStartTime = startTime;
    }

    /**
     * @dev Configs whitelist sales
    ...
    function configWhitelistSales(
    ...
    ) external onlyOwner isSaleOn(false) {
        require(
        ...
        for (uint256 i = 0; i < tierIds.length; i++) {
            ...
            _tiers[tierIds[i]].maxTicketNum = tierMaxTicketNums[i];
     ...
     */
    function privateMint(address[] memory tos) external isSaleOn(false) {
        require(_msgSender() == _minter, "Caller must be minter");
     ...
     */
    function _publicMint(uint256 maxNum, uint256 ticketNum, uint256 price) private {
        // all public sales share same counter mapping, contract trusts that all signature addresses are unique
        require(_publicMints[msg.sender] + ticketNum <= maxNum, "Exceed max ticket num");

        uint256 availableNum = Math.min(ticketNum, maxSupply - totalSupply );
        require(availableNum > 0, "Exceed max supply");
        _publicMints[msg.sender] += availableNum;

        uint256 totalPrice = price * availableNum;
        require(msg.value >= totalPrice, "Invalid payment");

        if (msg.value > totalPrice) {
            payable(msg.sender).transfer(msg.value - totalPrice);
            // (bool refund, ) = msg.sender.call{value: (msg.value - totalPrice)}("");
            // require(refund, "Failed to refund to payer");
        }
        ...
        // require(totalSupply < maxSupply, "Exceed public mint supply");
    }
    ...
    function whitelistMint(uint256 tierId, uint256 ticketNum, bytes memory signature) external payable isSaleOn(true) validTime(tierId) {
        ...
        require(SignatureChecker.isValidSignatureNow(_authority, data, signature), "Invalid signature");

        _publicMint(_tiers[tierId].maxTicketNum, ticketNum, _tiers[tierId].ticketPrice);
    }
    ...
    function dutchAuctionMint(uint256 ticketNum, bytes memory signature) external payable isSaleOn(true) validateCaller {
        require(block.timestamp >= dutchAuctionStartTime, "Auction not start");

        bytes32 data = keccak256(abi.encode(address(this), msg.sender, keccak256("DUTCH")));
        require(SignatureChecker.isValidSignatureNow(_authority, data, signature), "Invalid signature");

        _publicMint(DUTCH_AUCTION_MAX_TICKET, ticketNum, dutchAuctionPrice());
    }
    ...
    function dutchAuctionPrice() public view returns (uint256){
        ...
        uint256 drop = steps * DUTCH_AUCTION_DROP_PER_STEP;

        require(drop < DUTCH_AUCTION_START_PRICE, "Auction price err");
        return DUTCH_AUCTION_START_PRICE - drop;
    }

    /**
     * @dev Transfer token #1 to English auction winner
     * @param winner recipient to receive token #1
     *
     * Requirements:
     *
     * - the caller must be `owner`.
     */
    function englishAuctionWithdraw(address winner) external onlyOwner nonReentrant {
        _transfer(owner(), winner, 1);
    }
