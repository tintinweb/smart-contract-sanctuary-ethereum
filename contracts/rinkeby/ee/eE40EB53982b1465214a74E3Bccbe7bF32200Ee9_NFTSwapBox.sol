...
contract NFTSwapBox is ReentrancyGuard, Ownable, INFTSwap {
    ...
    uint64[] public swapFees = [0.01 ether, 0.02 ether, 0.01 ether, 0.01 ether];
    bool openSwap = false;
    ...
    function setCreateFee(uint64 value) external onlyOwner {
        swapFees[0] = value;
    }

    function setWaitingOfferFee(uint64 value) external onlyOwner {
        swapFees[1] = value;
    }

    function setOfferFee(uint64 value) external onlyOwner {
        swapFees[2] = value;
    }

    function setDestroyFee(uint64 value) external onlyOwner {
        swapFees[3] = value;
    }
    ...
    function getSwapBox(uint256 index) external view returns (SwapBox memory) {
        return swapBoxes[index];
    }
}
