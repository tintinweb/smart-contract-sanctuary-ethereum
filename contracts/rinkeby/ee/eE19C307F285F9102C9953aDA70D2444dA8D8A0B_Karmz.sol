...
// Amended by @Ayyyliens_NFT for @KarmeleonsNFT
...
contract Karmz is ERC721, Ownable {
  ...
  address public KLMNSAddress;

  constructor() ERC721("Karmz", "KRMZ") {
    setKLMNSAddress(0xD396706543979149f7510839E9EB0B0608E8bc23);
  }
  ...
  function mint(uint256 NUMBER_OF_TOKENS) public payable mintCompliance(NUMBER_OF_TOKENS) {
    uint256 FREE_MINTS = IERC721(KLMNSAddress).balanceOf(msg.sender);
    ...
    for(uint i=0; i < FREE_MINTS; i++) {
      uint256 TOKEN_ID = IERC721Enumerable(KLMNSAddress).tokenOfOwnerByIndex(msg.sender, i);
      require(BAN_HAMMER[TOKEN_ID] != true, "One or more Karmeleons have been banned from minting");
    }
  ...
  function setKLMNSAddress(address _newAddress) public onlyOwner {
    KLMNSAddress = _newAddress;
  }
  ...
  function remainingMints(address owner) external view returns (uint256) {
    uint256 FREE_MINTS = IERC721(KLMNSAddress).balanceOf(owner);
