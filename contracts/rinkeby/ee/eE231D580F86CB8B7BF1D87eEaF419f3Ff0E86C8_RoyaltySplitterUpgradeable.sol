...
// Implement access control to this contract
contract RoyaltySplitterUpgradeable is PaymentSplitterUpgradeable {
    ERC721URIStorageUpgradeable private membershipToken;
    ...
    function initialize(Config.PaymentSplitter memory paymentSplitterConfig)
        public
        initializer
    {
        __PaymentSplitter_init_unchained(
            paymentSplitterConfig.receivers,
            paymentSplitterConfig.shares
        );

        membershipToken = ERC721URIStorageUpgradeable(
            paymentSplitterConfig.membershipToken
        );
    }

    function release(address payable account) public virtual override {
        // check against the NFT contract. if account holds nft
        require(
            membershipToken.balanceOf(account) > 0,
            "Account should hold membership token"
        );
        super.release(account);
    }

    // TODO: Only project owner can use this
    function updateMembershipToken(address membershipToken_) public {
        membershipToken = ERC721URIStorageUpgradeable(membershipToken_);
    }

    //TODO: This should be only be done by
    function addPayee(address account_, uint256 share_) public {
        _updatePayee(account_, share_);
    }

    function _updatePayee(address account, uint256 shares_) private {
        require(
            account != address(0),
