...
contract Desperado is ERC721Enumerable, Ownable, ReentrancyGuard {
    ...
    function _mint(bytes memory signature) internal {
        require(
            _verifySignature(
                keccak256(
                    abi.encodePacked("MintApproval(address minter)", msg.sender)
                ),
                signature
            ),
    ...
    function genesisKeyMint(uint256 mintPass, bytes memory signature)
        external
        payable
        nonReentrant
    {
    ...
    function setRevealedBaseUri(string memory newRevealedBaseUri)
        external
        onlyOwner
    {
