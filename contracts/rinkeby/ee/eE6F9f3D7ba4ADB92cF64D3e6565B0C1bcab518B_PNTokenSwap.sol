...
contract PNTokenSwap is IPNTokenSwap, ReentrancyGuard, Ownable {
    ...
    /**
     * @dev returns address of the token that can be swapped
     *
     * @return the address of the V1 token contract
     */
    ...
    /**
     * @dev returns address of the token we're swapping to
     *
     * @return the address of the V2 token contract
     */
    function getV2TokenAddress() public view override returns (address) {
        return v1TokenAddress;
    }
    
    function swap(uint256 amount) external override {
        IERC20 v1Contract = IERC20(v1TokenAddress);
        require(v1Contract.balanceOf(msg.sender) >= amount, "Amount higher than user's balance");
        require(v1Contract.allowance(msg.sender, address(this)) >= amount, 'Approved allowance too low');
        require(            
            // Tranfer tokens from sender to this contract
            v1Contract.transferFrom(msg.sender, address(this), amount),
            "token swap failed"
        );
        ...
        v2Contract.transfer(msg.sender, amount);
    }
