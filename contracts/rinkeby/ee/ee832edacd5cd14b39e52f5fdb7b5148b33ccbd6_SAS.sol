...
contract SAS is ERC721A {
    ...
    mapping(address => uint256) public checker_count_mint;
    mapping(address => uint256) public checker_count_claim;
    ...
    function set_balance() public view returns (uint256) {
    ...
    function get_count_mint() public view returns (uint256) {
        return all_count_mint;
    }

    function get_count_fREEmint() public view returns (uint256) {
        return free_mint;
    }
    ...
    function checker_count_mint1() public view returns (uint256) {
        return checker_count_mint[msg.sender];
    }

    function checker_count_claim1() public view returns (uint256) {
    ...
    function Mint(uint256 count_mint1) public payable {
        ...
        require(
            checker_count_mint[msg.sender] + count_mint1 <= 3,
            "Unlimit mint"
        );
    ...
    function Claim(uint256 count_mint) public {
        if (is_freeMint == true || WL_Toggle == true) {
            ...
            require(
                checker_count_claim[msg.sender] + count_mint <= 1,
                "Unlimit claim"
            );
            ...
            checker_count_claim[msg.sender] =
                checker_count_claim[msg.sender] +
                count_mint;
    ...
    function checker_address_FreeMint() public view returns (bool) {
        for (uint256 i = 0; i < WhiteList1.length; i++) {
            if (msg.sender == WhiteList1[i]) return true;
        }
        return false;
    }
        ...
        returns (string memory)
    {
        return
            string(
                string(
                    abi.encodePacked(newURI, Strings.toString(tokenId), ".json")
                )
            );
    }
    ...
    function arrayOfTokenIDs(address user)
        public
        view
        returns (uint256[] memory)
    {
        uint256[] memory arrayID = new uint256[](balanceOf(user));
        uint16 a = 0;
    ...
    function arrayOfNftHolders(uint256 number)
        public
        view
        returns (address[] memory)
    {
        ...
        for (uint256 i = 0; i < totalSupply(); i++) {
            arrayOfHolders[i] = ownerOf(i);
        }

        return arrayOfHolders;
    }
}