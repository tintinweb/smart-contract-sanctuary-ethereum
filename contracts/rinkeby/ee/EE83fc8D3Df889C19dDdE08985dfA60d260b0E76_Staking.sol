...
contract Staking is Pausable, Ownable {
  ...
  mapping(address => StakingAction[]) private actions;
  ...
  function unstake(uint256 amount) external {
    require(balances[msg.sender] >= amount, "Staking: invalid amount");
    balances[msg.sender] -= amount;
    _saveAction(
      msg.sender,
      ActionType.Unstake,
  ...
  {
    return _convertActionsToJson(actions[sender]);
  }
  ...
  /**
   * @notice returns all wallets and their history
  ...
  /**
   * @notice records action
   * @param sender sender
   * @param actionType type of action - stake/unstake
   * @param amount amount of tokens
   */
    ...
    returns (string memory json)
  {
    ...
    json = string(abi.encodePacked(
        json,
        "\"action\":",
        action.action == ActionType.Stake ? "\"stake\"" : "\"unstake\""
      ));
    json = string(abi.encodePacked(
        json,
        ",\"timestamp\":\"",
