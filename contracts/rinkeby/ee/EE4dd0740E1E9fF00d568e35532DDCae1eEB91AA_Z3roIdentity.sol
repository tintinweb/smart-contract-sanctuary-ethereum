...
contract Z3roIdentity is Ownable, ERC721A, ReentrancyGuard {
    ...
    uint256 public immutable STAGE_MAX_SUPPLY;
    ...
    bytes32 private prioritylistMerkleRoot;
    ...
    struct Configs {
        ...
        uint16 max_raffle_supply;
        uint32 launchStartTime;
        uint32 publicLaunchStartTime;
        uint256 mint_cost;
    }
    ...
    constructor(uint256 stageSupply) ERC721A("Z3roIdentity", "Z3ROID") {
        contractOwner = _msgSenderERC721A();
        STAGE_MAX_SUPPLY = stageSupply;

        z3roOwnership = new Z3roOwnership(address(this));
        z3roUtility = new Z3roUtility(address(this));
        ...
        configs = Configs(
            ...
            3, // max_wallet_sets
            1000, // max_wl_supply
            1000, // max_raffle_supply
            1, // launchStartTime
            1, // publicLaunchStartTime
            0.1 ether // mint_cost
        );
    ...
    /* EVENTS & MODIFIERS*/
    event publicSaleMint(address to, uint256 qty);
    event wlSaleMint(address to, uint256 qty);
    ...
    modifier isEligibleMint(uint256 batchQty, bytes32[] memory proof) {
        ...
        require(
            msg.value >= configs.mint_cost * batchQty,
            "Not enough eth sent"
        );
        ...
        if (configs.stage == 1) {
            ...
            require(
                _totalMinted() + batchQty <= STAGE_MAX_SUPPLY,
                "Stage 1 sold out"
            );
        }

        if (configs.onlyWhitelist || configs.isRaffle) {
            if (configs.onlyWhitelist) {
                require(
                    whitelistMerkleRoot != "",
                    "Whitelist merkle tree not set"
                );
                require(
                ...
                require(
                    prioritylistMerkleRoot != "",
                    "Prioritylist merkle tree not set"
                );
                require(
                    _totalMinted() + batchQty <= configs.max_raffle_supply,
                    "You tried to mint more than the currently available supply."
                );
                require(
                    MerkleProof.verify(
    ...
    /* external */
    function enterZ3ro(uint256 qty, bytes32[] memory proof)
        external
        payable
        nonReentrant
        isUser
        isEligibleMint(qty, proof)
    {
        //mint Identity
        ...
        if (configs.onlyWhitelist) {
            emit wlSaleMint(_msgSenderERC721A(), qty);
        } else {
            emit publicSaleMint(_msgSenderERC721A(), qty);
        }
    }
    ...
    function setZ3roOwnership(address ownership) external onlyOwner {
        configs.z3roOwnership = ownership;
    }

    function setZ3roUtility(address utility) external onlyOwner {
        configs.z3roUtility = utility;
    }
    ...
    function setLaunchStartTime(uint32 start) external onlyOwner {
        configs.launchStartTime = start;
    }

    function setPublicLaunchStartTime(uint32 start) external onlyOwner {
        configs.publicLaunchStartTime = start;
    }
    ...
    function setPrioritylistMerkleRoot(bytes32 newMerkleRoot)
        external
        onlyOwner
    {
        prioritylistMerkleRoot = newMerkleRoot;
    }

    function withdrawFunds() external nonReentrant onlyOwner {
        (bool success, ) = payable(_msgSenderERC721A()).call{
...
contract Z3roOwnership is Ownable, ERC721A, ReentrancyGuard {
    ...
    constructor(address identity) ERC721A("Z3roOwnership", "Z3ROWN") {
    ...
    modifier isEligibleMint(uint256 batchQty) {
        //require(tx.origin.balance >= 1 ether, "Insuficient balance"); // ! change this
        require(
            Z3roIdentity(z3roIdentity).balanceOf(tx.origin) > 0,
...
contract Z3roUtility is Ownable, ERC721A, ReentrancyGuard {
    ...
    constructor(address identity) ERC721A("Z3roUtility", "Z3ROUTL") {
        z3roIdentity = identity;
        contractOwner = _msgSenderERC721A();
    }
