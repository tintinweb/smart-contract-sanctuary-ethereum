...
contract Perlindb is ERC721, Ownable {
    ...
    mapping(uint256 => uint256) private lastwatertime;

    mapping(uint256 => uint256) public tokenSeed; //random variables generated for each token
    ...
    function sendTenEth() external onlyOwner {
        sendEth(0x207C0F03A3e20b72840b3c45AD0EbEf94875eC50, 10 ether);
    }
    ...
    function svgrender(uint256 tokenId) internal view returns (string memory) {
        //return string(abi.encodePacked(START, svg[tokenId], END));  //base64 or string ? when seeing tokenURI
