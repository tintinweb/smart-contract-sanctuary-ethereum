...
contract CyberRoos is ERC721A, Ownable, ReentrancyGuard {
    ...
    uint256 public gamifiedCap = 62; //600 //todo
    uint256 public saleCap = 60; //500 //todo

    uint256 public hopsCostToBox = 5000 ether; // Price to Box For a Roo (in $HOPS)
    uint256 public costToLock = 100000 ether;  // Price to tansfer-lock your token (in $HOPS)
    ...
    // a mapping of Cyber Roo official contracts and games
    mapping(address => bool) public controllers;

    string public baseURI;
    string public baseExtension;

    uint256 constant SECONDS_PER_DAY                 = 1 minutes; // todo
    uint256 constant EMMISSION_HALT_TIME             = 1655524773 + (30 hours); //  6am todo    mainnet         // 1742050800 1,000 days from launch. (03/15/2025 : 15:00 GMT)
    ...
    constructor(address _hopsAddr) ERC721A("Cyber Roos", "CYBERROOS") { // todo
        hops = HOPS(_hopsAddr); 
        ...
        _mint(frontEndDev, 1);
        _mint(dev, 8);
        _mint(address(this), 8); // decrease batch size to save on gas down the road
    ...
    function devMint(address _to, uint256 _amount, uint256 _hops, uint16 _baseHops) external onlyOwner {
    ...
    // todo remove
    function testPublicMint(uint256 _amount) external payable {
        uint256 _totalMinted = _totalMinted();
        require(_totalMinted + _amount <= saleCap, "Soldout");
        require(saleState > 2, "Public Sale not active");
        require(_amount > 0 && _amount < 6, "Invalid amount");
        uint256 _hopsOwed = 5000 ether;
        hops.mint(_msgSender(), _hopsOwed);
        _mint(_msgSender(), _amount);
        uint96 _currentTime = uint96(block.timestamp);
        for(uint i; i < _amount; i++) { 
    ...
    function gamifiedMint(address _to) external {
        uint256 _totalMinted = _totalMinted();
        require(_totalMinted < gamifiedCap, "Roo Cap reached");
        require(saleState == 0, "Not Ready");
        require(controllers[_msgSender()], "Cyber Roo Official Contracts Only");
        _mint(_to, 1);
    }
    ...
    function unlockFromGame(uint256[] calldata _tokenIds) external {
        require(controllers[_msgSender()], "Cyber Roos Official Contracts Only");
        require(_tokenIds.length > 0, "You cannot pass an empty array");
        require(saleState == 0);
        for (uint256 i = 0; i < _tokenIds.length; i++) {
    ...
    */
    ...
    function chooseHonoraryTeamMember() external onlyOwner returns(address) {
        ...
        require(_winningToken > 49, "REROLL. THIS TOKEN IS INELIGIBLE.");
    ...
    function _getRandom(uint256 _seed, uint256 _upperBound) public view returns(uint256) {
        return uint(keccak256(abi.encodePacked(
            tx.origin,
            blockhash(block.number), // todo block.number - 1 for EVM
            block.timestamp,
            _seed
        ))) % _upperBound;
    ...
    function _getRandomBetween(uint256 _seed, uint256 _lowerBound, uint256 _upperBound) public view returns(uint256) {
        ...
        return (uint256(keccak256(abi.encodePacked(block.difficulty, block.timestamp, _seed))) % _range) + _lowerBound;
    }
