...
import './IStellaLuna.sol';

contract StellaLuna is JupiterNFT, IStellaLuna{
    event MintStellaLuna(address indexed to, uint8 option, uint256 indexed tokenId);

    mapping(uint8 => uint256) public options;
    ...
    constructor(
        ...
        options[1] = 10000000000000000; // without physical
        options[2] = 20000000000000000; // with physical
    }

    // allows the operator to add a new option or modify an existing one
    function setOption(uint8 _option, uint256 price) public {
        require(operators[msg.sender], "only operators");
        options[_option] = price;
    }
    ...
    function mint(uint8 option) payable external override{
        require(msg.value >= options[option], "Not enough ETH sent; check price!");

        currentTokenId++;
		_safeMint(_msgSender(), currentTokenId);
        emit MintStellaLuna(_msgSender(), option, currentTokenId);
        return;
    }

    function mintBatch(uint8 option, uint amount) payable external override{
        require(msg.value >= options[option], "Not enough ETH sent; check price!");
        
        uint buf = currentTokenId;
        for(uint i = 0; i < amount; i++) {
			buf++;
			_safeMint(_msgSender(), buf);
            emit MintStellaLuna(_msgSender(), option, buf);
		}
		currentTokenId = buf;	
...
contract JupiterNFT is IJupiterNFT, ERC721Tradable, ERC721Burnable, ERC721Pausable {
     ...
     */
	function adminMint(address _to) public override{
		require(operators[msg.sender], "only operators");
		currentTokenId++;
     ...
     */
	function adminMintBatch(address _to, uint _amount) public override {
		require(operators[msg.sender], "only operators");
...
interface IJupiterNFT {
	...
	function adminMintBatch(address _to, uint _amount) external;
}
...
interface IStellaLuna {
	function mint(uint8 option) payable external;
	function mintBatch(uint8 option, uint amount) payable external;
}
