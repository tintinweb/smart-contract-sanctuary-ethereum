...
contract CopeBears is ERC721A, Ownable, ReentrancyGuard {
    // limits
    ...
    uint256 private lastFreeTokenId;
    address private withdrawAddress = address(0);

    // events
    event MintedFree(address indexed receiver, uint256 indexed tokenId);
    ...
    function mintPublic(uint256 _amount) external payable nonReentrant {
        ...
        if (freeMintsAvailable > 0) {
            ...
            for (uint256 i = 0; i < _amount; i++) {
                ...
                lastFreeTokenId = tokenId;
                freeMintsAvailable = freeMintsAvailable - 1;
                freeCount = freeCount + 1;
                emit MintedFree(_msgSender(), tokenId);
            }
