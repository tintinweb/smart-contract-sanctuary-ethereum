...
contract NFTFarm is Ownable, ERC1155Holder {
     ...
     struct UserInfo{
        uint256 stakedAmount;   // 质押的数据
        uint256 lastUpdateAt; // 最后更新的时间戳
        uint256 pointsDebt; // 最近存款之前的系统的积分
     }
     ...
     function addNFTs(
        ...
        uint256[] calldata _prices
          ...
          require(_ids.length== _totals.length || _ids.length== _prices.length,"NFTForm: Incorrect array length");
          crops.safeBatchTransferFrom(msg.sender,
           address(this),
           _ids, 
           _totals, 
           "");
     ...
     function claimNFTs(uint256[] calldata _nftIndexes,uint256[] calldata _quantities) external{
        ...
        require(_nftIndexes.length == _quantities.length,"NFTForm: Incorrect array length");
        ...
        for(uint64 i=0;i<_nftIndexes.length;i++){
            ...
            require(nft.remaining > _quantities[i],"NFTForm: Not enough NFT crops in farm");
            require(points >= cost,"NFTForm: Insufficient Points");
     ...
     function unstakeTokens() public {
        UserInfo memory user =userInfo[msg.sender];
        require(user.stakedAmount >0,"NFTForm: staking balance cannot be 0");
