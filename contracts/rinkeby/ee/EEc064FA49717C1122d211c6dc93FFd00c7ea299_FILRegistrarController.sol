 ...
 */
contract FILRegistrarController is Ownable {
    ...
    // Rent in attodollars (1e-18) per name
    ...
    function makeCommitmentWithConfig(string memory name, address owner, bytes32 secret, address resolver, address addr) pure public returns(bytes32) {
        ...
        require(resolver != address(0), "resolver != address(0)");
        return keccak256(abi.encodePacked(label, owner, resolver, addr, secret));
    }

    function commit(bytes32 commitment) public {
        require(commitments[commitment] + maxCommitmentAge < now, "commit wrong");
    ...
    function registerWithConfig(string memory name, address owner, bytes32 secret, address resolver, address addr) public payable {
        ...
        if(resolver != address(0)) {
            ...
            require(addr == address(0), "addr == address(0)");
    ...
    function _consumeCommitment(string memory name, bytes32 commitment) internal returns (uint256) {
        ...
        require(commitments[commitment] + minCommitmentAge <= now, "Wait at least one minute");

        // If the commitment is too old, or the name is registered, stop
        require(commitments[commitment] + maxCommitmentAge > now, "more than 24 hours");
