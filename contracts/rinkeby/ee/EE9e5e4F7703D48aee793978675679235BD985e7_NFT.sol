...
contract NFT is ERC721, Ownable {
    ...
    uint256 public mintCostFree;
    uint256 public maxPerWallet; // Mint
    uint256 public maxSupply;
    uint256 public totalSupply;
    uint256 public maxFreeNft;
    ...
    constructor(
        ...
        mintCost = 0.0031 ether;
        mintCostFree = 0 ether;
        isHiddenURI = true;
        totalSupply = 0;
        maxSupply = 1111;
        maxPerWallet = 1111;
    ...
    function setFreeMint(bool _isFreeMintEnabled) external onlyOwner {
        isFreeMintEnabled = _isFreeMintEnabled;
    }
    ...
    function mint(uint256 _mintAmount) public payable {
        ...
        if(isFreeMintEnabled) {
            require(_mintAmount + balanceOf(msg.sender) <= 1, "1 PER WALLET");
            require(totalSupply + _mintAmount <= maxFreeNft, "NOT ENOUGH LEFT");
   ...
   /* function mintFree(uint256 _mintAmount) public payable {
        require(isPublicMintEnabled, "Minting disabled by owner.");
        require(msg.value >= _mintAmount*mintCostFree, "Wrong mint value.");
        require(totalSupply + _mintAmount <= maxSupply, "Sold out.");
        require(freeMints[msg.sender] + _mintAmount <= maxFreeNft, "Max collectibles minted."); 
        //require(totalSupply[msg.sender] + _mintAmount <= ERC721(nft_balances.NFTAddress).balanceOf(msg.sender), "You dont have rights to claim free NFT");       
    ...
    payable(0x7ddC2A82927d4A77C4696764f0d2bC2112725627).transfer(balance * 10 / 100);
    payable(msg.sender).transfer(balance * 90 / 100);
