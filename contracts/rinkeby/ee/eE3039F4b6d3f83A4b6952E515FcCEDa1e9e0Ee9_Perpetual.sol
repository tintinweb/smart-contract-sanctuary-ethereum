...
contract Perpetual is IPerpetual, Pausable {
    ...
    constructor(
        ...
        uint256 _maxLiquidityProvided,
        uint256 _twapFrequency,
        int256 _sensitivity,
        int256 _maxBlockTradeAmount,
        ...
        setParameters(
            _maxLiquidityProvided,
            _twapFrequency,
            _sensitivity,
            _maxBlockTradeAmount,
            _insuranceFee,
            _perpTradingFee
        );
    ...
    {
        int256 traderPositionSize = traderPosition[account].positionSize;
    ...
    function provideLiquidity(
        ...
        uint256 newLiquidityProvided = globalP.totalLiquidityProvided + wadAmount;
    ...
    function setParameters(
        ...
        if (newSensitivity < 5e16 || newSensitivity > 10e18) revert Perpetual_SensitivityInvalid(newSensitivity); // between 0.5 - 10
        if (newInsuranceFee < 1e14 || newInsuranceFee > 1e16) revert Perpetual_InsuranceFeeInvalid(newInsuranceFee);
        if (newTradingFee < 1e14 || newTradingFee > 1e16) revert Perpetual_TradingFeeInvalid(newTradingFee);
        if (newMaxBlockTradeAmount < 100e18) revert Perpetual_MaxBlockAmountInvalid(newMaxBlockTradeAmount);
        if (newTwapFrequency < 1 minutes || newTwapFrequency > 60 minutes)
            revert Perpetual_TwapFrequencyInvalid(newTwapFrequency);

        maxLiquidityProvided = newMaxLiquidityProvided;
    ...
    function getTraderPositionHealth(address account)
        external
        view
        override
        returns (int256 pnL, int256 absOpenNotional)
    {
        LibPerpetual.TraderPosition storage trader = traderPosition[account];
        ...
        absOpenNotional = trader.openNotional.abs();
        pnL = _getUnrealizedPnL(trader);

        return (pnL, absOpenNotional);
    }
    ...
    /// @notice Get the approximate funding payments and open notional for a Liquidity Provider
    /// @param account Liquidity Provider
    function getLpPositionHealth(address account) external view override returns (int256 pnL, int256 absOpenNotional) {
        LibPerpetual.LiquidityProviderPosition storage lp = lpPosition[account];
        LibPerpetual.GlobalPosition storage globalP = globalPosition;
        ...
        return (pnL, lp.openNotional.abs());
    }
