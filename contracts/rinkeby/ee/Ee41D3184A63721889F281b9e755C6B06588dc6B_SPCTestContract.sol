...
// File: contracts/SPCTestContract.sol



/**
    Sabertooth Poker Club Smart Contract
*/
...
contract SPCTestContract is ERC721Enumerable, EIP712, Ownable {
  using Strings for uint256;
  ...
  uint256 public nftPerAddressLimit = 3333;
  ...
  function mint(uint256 _mintAmount, bytes memory signature) public payable {
    ...
    if (msg.sender != owner()) {
        if(onlyWhitelisted == true && !isReserved(msg.sender)) {
            require(owner() == _verify(msg.sender, signature), "user is not whitelisted");
        }
        if(!isReserved(msg.sender)){
  ...
  function setmaxReserved(uint256 _newmaxReserved) public onlyOwner {
    maxReserved = _newmaxReserved;
  }
  ...
  function check(bytes memory signature) public view returns (address){
      return _verify(msg.sender, signature);
  }
