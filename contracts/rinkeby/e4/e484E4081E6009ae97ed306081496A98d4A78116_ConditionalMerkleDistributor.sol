...
contract ConditionalMerkleDistributor is Ownable {
    ...
    mapping(address => mapping(uint256 => address)) private checkedParties;
    ...
    constructor(
        ...
        vipSlots = _isVipSlots;
        blueChipSlots = _blueChipSlots;
    }

    function setRoots(bytes32 _isVipMerkleRoot, bytes32 _blueChipMerkleRoot) public onlyAdmin {
        require(vipMerkleRoot == bytes32(0) && blueChipMerkleRoot == bytes32(0), "roots already set");
        vipMerkleRoot = _isVipMerkleRoot;
    ...
    function _checkLoan(
        ...
        require(_borrower == _loan.borrower, "wrong borrower");
        ...
        if (_isVip) {
            ...
            require(blueChipCollections[_loan.nftCollateralContract], "not blue chip collection");
        }
    ...
    function claimVipBorrower(
        ...
        _claim(_index, msg.sender, _lender, _loanId, _merkleProof, true, true);
    }

    function claimBlueChipBorrower(
        uint256 _index,
        address _lender,
        uint32 _loanId,
        bytes32[] calldata _merkleProof
    ) external {
        _claim(_index, msg.sender, _lender, _loanId, _merkleProof, false, true);
    }

    function claimVipLender(
        uint256 _index,
        address _borrower,
        uint32 _loanId,
        bytes32[] calldata _merkleProof
    ) external {
        _claim(_index, _borrower, msg.sender, _loanId, _merkleProof, true, false);
    }

    function claimBlueChipLender(
        uint256 _index,
        address _borrower,
        uint32 _loanId,
        bytes32[] calldata _merkleProof
    ) external {
        _claim(_index, _borrower, msg.sender, _loanId, _merkleProof, false, false);
    }

    function claimVipBorrowerNoChecks(address _lender, uint32 _loanId) external {
        bytes32[] memory emptyBytesArray;
        _claim(0, msg.sender, _lender, _loanId, emptyBytesArray, true, true);
    }

    function claimBlueChipBorrowerNoChecks(address _lender, uint32 _loanId) external {
        bytes32[] memory emptyBytesArray;
        _claim(0, msg.sender, _lender, _loanId, emptyBytesArray, false, true);
    }

    function claimVipLenderNoChecks(address _borrower, uint32 _loanId) external {
        bytes32[] memory emptyBytesArray;
        _claim(0, _borrower, msg.sender, _loanId, emptyBytesArray, true, false);
    }

    function claimBlueChipLenderNoChecks(address _borrower, uint32 _loanId) external {
        bytes32[] memory emptyBytesArray;
        _claim(0, _borrower, msg.sender, _loanId, emptyBytesArray, false, false);
    }
    ...
    function _claim(
        ...
        address otherParty = _isBorrower ? _lender : _borrower;
        if (checkedParties[otherParty][_loanId] == msg.sender) {
            _claimNoChecks(_borrower, _lender, _loanId, _isVip, _isBorrower);
        } else {
            _claimWithChecks(_index, _borrower, _lender, _loanId, _merkleProof, _isVip, _isBorrower);
        }
    }

    function _claimWithChecks(
        uint256 _index,
        address _borrower,
        address _lender,
        uint32 _loanId,
        ...
        require(!isClaimed(_index, _isVip), "index already claimed");
        //even tough multiple roots can contain an users drop, they can only claim once
        require(!borrowerClaimed[_borrower], "borrower already claimed");
        if (_isVip) {
            // vip claims can overflow to blueChipSlots
            require(vipSlotCounter + blueChipSlotCounter < vipSlots + blueChipSlots, "vip slots ran out");
        } else {
            // we have to consider overflowing vip claims
            require(
                blueChipSlotCounter + (vipSlotCounter > vipSlots ? vipSlotCounter - vipSlots : 0) < blueChipSlots,
                "blue chip slots ran out"
            );
        }
        ...
        _checkLoan(_borrower, _lender, _loanId, loan, _isVip);

        // Verify the merkle proof.
        bytes32 node = keccak256(abi.encodePacked(_index, _borrower));
        if (_isVip) {
            require(MerkleProof.verify(_merkleProof, vipMerkleRoot, node), "distributor: invalid proof");
        } else {
            require(MerkleProof.verify(_merkleProof, blueChipMerkleRoot, node), "distributor: invalid proof");
        }

        // Mark it claimed and send the token.
        _setClaimed(_index, _isVip);
        borrowerClaimed[_borrower] = true;
        usedAssets[loan.nftCollateralContract][loan.nftCollateralId] = true;
        address otherParty = _isBorrower ? _lender : _borrower;
        checkedParties[msg.sender][_loanId] = otherParty;
        if (_isVip) {
            vipSlotCounter++;
        } else {
            blueChipSlotCounter++;
        }
        uint256 reward = _getReward(_isVip, _isBorrower);
        IVestedStaking(vestedStaking).addVestedStaking(msg.sender, reward);

        emit Claimed(_index, _borrower, true, _isVip);
    }

    function _claimNoChecks(
        address _borrower,
        address _lender,
        uint32 _loanId,
        bool _isVip,
        ...
        address otherParty = _isBorrower ? _lender : _borrower;
        delete checkedParties[otherParty][_loanId];
        uint256 reward = _getReward(_isVip, _isBorrower);
        IVestedStaking(vestedStaking).addVestedStaking(msg.sender, reward);
    }

    function _getReward(bool _isVip, bool _isBorrower) internal pure returns (uint256 reward) {
        if (_isVip) {
            if (_isBorrower) {
                reward = vipBorrowerReward;
            } else {
                reward = vipLenderReward;
            }
        } else {
            if (_isBorrower) {
                reward = blueChipBorrowerReward;
            } else {
                reward = blueChipLenderReward;
            }
        }
    }
}
