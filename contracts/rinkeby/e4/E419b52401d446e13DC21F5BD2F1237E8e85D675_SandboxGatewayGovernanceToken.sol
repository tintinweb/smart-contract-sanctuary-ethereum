...
interface IStakedVotesUpgradeable {
    ...
    /**
     * @dev Returns the current total tokens used to be staked token.
...
interface IRewardToken is IERC20Upgradeable{
     ...
     */
    function updateAccumulated(address account, uint256 amount, bool isStake) external;
    ...
    ERC20PermitUpgradeable
{
    ...
    bytes32 private constant _MINTTOSTAKE_TYPEHASH =
        keccak256(
            "mintoStake(uint256 amount,uint256 fee,uint256 nonce,uint256 expiry)"
        );

    bytes32 private constant _DELEGATION_TYPEHASH =
        keccak256(
            "Delegation(address delegatee,uint256 fee,uint256 nonce,uint256 expiry"
        );
    ...
    mapping(address => Checkpoint[]) private _tokenBalanceCheckpoints;
    mapping(address => Checkpoint[]) private _votingCheckpoints;

    Checkpoint[] private _totalStakedCheckpoints;

    uint32 internal _stakingLockup;
    uint32 internal _unstakingLockup;
    ...
    function setRewardingToken(address rewardingToken)external onlyAdmin returns(bool){
        _rewardingToken = IRewardToken(rewardingToken);
        return true;
    }
    /**
     ...
     */
    function numBalanceCheckpoints(address account)
        public
        view
        virtual
        returns (uint32)
    {
        return
            SafeCastUpgradeable.toUint32(
                _tokenBalanceCheckpoints[account].length
            );
    }
    ...
    /**
     * @dev Gets the current votes balance for `account` on a given `DAOContract`
     */
    ...
    function balanceOf(address account)
        ...
        returns (uint256)
    {
        uint256 pos = _tokenBalanceCheckpoints[account].length;
        if (pos == 0) return 0;
        for (uint256 i = pos - 1; i > 0; i--) {
            if (
                _tokenBalanceCheckpoints[account][i].fromBlock <= block.number
            ) {
                return _tokenBalanceCheckpoints[account][i].balance;
            }
        }
        if (_tokenBalanceCheckpoints[account][0].fromBlock <= block.number) {
            return _tokenBalanceCheckpoints[account][0].balance;
        }
        return 0;
    }

    /**
     * @dev Moves `amount` of tokens from `sender` to `recipient` including checkpoint logging.
     *
     * This internal function is equivalent to {transfer}, and can be used to
     * e.g. implement automatic token fees, slashing mechanisms, etc.
     *
    ...
    function _transfer(
        ...
        _transfer(from, to, amount, block.number);
    }
    ...
    function _transfer(
        ...
        _writeCheckpoint(
            _tokenBalanceCheckpoints[from],
            _subtract,
            amount,
            effectiveBlockNumber
        );
        _writeCheckpoint(
            _tokenBalanceCheckpoints[to],
            _add,
            amount,
            effectiveBlockNumber
        );
    ...
    /** @dev Creates `amount` tokens and assigns them to `account`, increasing
     * the total supply including checkpoint logging.
     *
     * Emits a {Transfer} event with `from` set to the zero address.
     *
     * Requirements:
    ...
    function _mint(address account, uint256 amount) internal virtual override {
        ...
        _writeCheckpoint(
            _tokenBalanceCheckpoints[account],
            _add,
            amount,
            block.number
        );
    ...
    /**
     * @dev Destroys `amount` tokens from `account`, reducing the
     * total supply including checkpoint logging.
     *
     * Emits a {Transfer} event with `to` set to the zero address.
     *
     * Requirements:
    ...
    /**
     * @dev Retrieve the number of votes for `account` at the end of `blockNumber` for a given `DAOContract`.
     *
     * Requirements:
     *
     * - `blockNumber` must have been already mined
        ...
        returns (bool)
    {
        require(
            ...
            "ERC20StakedVotesUpgradeable: Insufficient balance to stake"
        );
        ...
        _rewardingToken.updateAccumulated(stakeholder, amount, true);
    ...
    {
        require(
            stakeOf(stakeholder) >= amount,
            "ERC20StakedVotesUpgradeable: Insufficient balance to unstake"
        );
        _transfer(
            DAOContract,
            stakeholder,
            amount,
            block.number + _unstakingLockup
        );
        _writeCheckpoint(
        ...
        _rewardingToken.updateAccumulated(stakeholder, amount, false);
        _moveVotingPower(_delegates[stakeholder], address(0), amount);
        return true;
    }

    function stake(uint256 amount) public virtual override returns (bool) {
    ...
    function stakeBySig(
        ...
        require(nonce == _useNonce(stakeholder), "ERC20Votes: invalid nonce");
...
contract GatewayGovernanceToken is ERC20StakedVotesUpgradeable {
    ...
    function initialize(
        ...
        __GatewayGovernanceToken_init_unchained(wrappingRate);
    }

    function __GatewayGovernanceToken_init_unchained(uint32 wrappingRate)
        internal
        onlyInitializing
    {
    ...
    {
        return (_wrappingRate, _stakingLockup, _unstakingLockup, _token);
    }
    ...
    function mint(
        ...
        __mint(minter, amount);
        _transfer(minter, _msgSender(), fee / _wrappingRate);
    }
