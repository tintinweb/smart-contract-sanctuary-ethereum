...
interface LandSource {
    function plots(uint256 tokenId) external returns(int128 x,int128 y,int128 t,uint256 mint_log,bool exists);
}
...
abstract contract ERC721Enumerable is ERC721, IERC721Enumerable {
    ...
    //event BeforeTX(address from, address to, uint256 tokenId);
    /**
     * @dev See {IERC165-supportsInterface}.
     */
    ...
    function _beforeTokenTransfer(
        ...
        if( from != address(0x0) && plots[tokenId].expire < block.number )
        {  
            if( to != address(0x0) ){
               require( plots[tokenId].expire != 0, "Setting Lease blocktime is needed"); 
            }
    ...
    function removeLandService(uint256 landId) public onlyGovernance {
        // To prevent a gap in the tokens array, we store the last token in the index of the token to delete, and
        // then delete the last slot (swap and pop).
        uint256 lastSrvndex = LandService.length.sub(1);
        uint256 landIndex =  LandServiceIndex[landId];
        LandService[ landId ] = LandService[lastSrvndex ];
        LandServiceIndex[ lastSrvndex ] = landIndex;
        delete LandServiceIndex[landId];
        LandService.pop();
    }
...
contract RightToUse is ERC721Metadata {
   
    using SafeMath for uint256;
    using Strings for uint256;

    address public landNft =  address(0x0);
    mapping(int128 => mapping(int128 => uint256)) public plot_id_at;
    ...
    uint256 public LeaseMaxBlocks = 2102400;   // Maximum 1 years

    event LeasePlotMinted(
        address to,
        uint256 plotid
    );
    ...
    event SetExpire(uint256 tokenId, uint256 expblock);

    constructor() ERC721("RightToUse", "RTU") { }
     ...
     */
    function lease_mint(address to, uint256 tokenId ) internal returns (bool) 
    {
        ...
        (int128 x,int128 y,int128 t,,bool exists) =  r.plots(tokenId);
        plots[tokenId] = Plot(x,y,t,0,exists);

        emit LeasePlotMinted(to, tokenId);
     ...
     */
    function lease_burn(uint256 tokenId) internal {

        _burnToken(tokenId);

        if(plots[tokenId].exists){
             int128 _x1 = plots[tokenId].x;
             int128 _y1 = plots[tokenId].y;
             plot_id_at[_x1][_y1] = 0;
             emit LeasePlotBurn(tokenId, _x1, _y1);
        }
        delete plots[tokenId];
    }

    function setExpiration(uint256 tokenId, uint256 blocknumber) external 
    {
        require( plots[tokenId].expire == 0, "the expired number has exists" );
        require( blocknumber > block.number, "blocknumber invalid" );
        require( blocknumber < block.number+LeaseMaxBlocks, "blocknumber over maxLimition" );
        require(
            _isApprovedOrOwner(_msgSender(), tokenId),
            "operator is not owner nor approved"
        );
        plots[tokenId].expire = blocknumber;
        emit SetExpire(tokenId, blocknumber);
    }
    ...
    function setLeaseMaxBlocks(uint256 maxblock) public onlyGovernance {
        LeaseMaxBlocks = maxblock;
    }
    ...
    {
        if( plots[tokenId].expire == 0 || plots[tokenId].expire < block.number )
        {
