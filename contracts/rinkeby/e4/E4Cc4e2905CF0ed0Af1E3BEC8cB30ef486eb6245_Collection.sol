...
contract Collection is ERC721A, Ownable {
    ...
    uint256 public constant MAX_AMOUNT = 300;

    string public baseURI = "ipfs://QmdW6P53oe3R1WbvYPCw5NM39nT9UnAwf5JbFGjj8HjbGf/";
    string public extension = ".json";
    uint256 public price = 50000000000000000;

    constructor(
        address _openseaProxyRegistryAddress
    ) ERC721A("Collection T2", "T2") {
    ...
    function mint(uint256 quantity) external payable {
        ...
        require(supply + quantity <= MAX_AMOUNT, "max collection supply");
        require(quantity + balanceOf(msg.sender) <= MAX_PER_WALLET, "only 3 per wallet");
        if (msg.sender != owner()) {
            require(msg.value >= price * quantity, "not enough money");
        }
