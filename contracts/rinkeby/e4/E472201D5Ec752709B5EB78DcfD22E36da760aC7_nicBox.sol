...
contract nicBox is Ownable, Pausable,ERC1155URIStorage{
    ...
    function burn(address from_, uint boxID_, uint256 num_, bytes32[] memory proof) public whenNotPaused {
        // 校验 msg.sender 是否在白名单
        require(isValid(proof, msg.sender), "Not a part of Allowlist");
    ...
    function burnBatch(address from_, uint[] memory boxIDs_, uint256[] memory nums_, bytes32[] memory proof) public whenNotPaused {
        require(isValid(proof, msg.sender), "Not a part of Allowlist");
        require(_msgSender() == from_ || isApprovedForAll(from_, _msgSender()), "burn caller is not owner nor approved");
        require(boxIDs_.length == nums_.length, "array length unequal");
        for (uint i = 0; i < boxIDs_.length; i++) {
