...
contract SevenNFT is ERC1155, Ownable {
    ...
    function mint(
        ...
        attributes[tokenId] = Attr(msg.sender, royalty, block.timestamp, metadata, stackable, amount);
        emit MintEvent(tokenId, msg.sender);

        if (forSale && auctionTime > 0) {
            marketPlace.putOnAuction(tokenId, price, amount, auctionTime, sellerId);
        } else if (forSale) {
            marketPlace.putNftOnSale(tokenId, price, amount, sellerId);
        }

        marketPlace.addTrade(tokenId, amount);
        total++;
    }
    ...
    function getMinter(uint256 _tokenId) external view returns (address) {
        return attributes[_tokenId].minter;
    }

    function getRoyalty(uint256 _tokenId) external view returns (uint256) {
        return attributes[_tokenId].royalty;
    }
