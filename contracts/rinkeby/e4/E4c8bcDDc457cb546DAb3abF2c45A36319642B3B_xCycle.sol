...
/**
 * @title xCycle
 * @dev Compounding CYCLE using protocol ETH revenue
 */
contract xCycle is ERC20, Ownable, ReentrancyGuard {
    ...
    address public constant ETHRewards =
        address(0x81119fFEc6C1b06404fe1c8dcAAA9e3F6132677E);

    uint256 public constant BP_DIV = 10000;
    uint256 public kickbackBP = 100;

    address[] public swapPath = [WETH, CYCLE];

    event Deposit(
        address indexed account,
        uint256 amountCYCLE,
        uint256 xCYCLEreceived
    );
    event Withdraw(
        address indexed account,
        uint256 xCYCLEredeemed,
        uint256 CYCLEreceived
    );
    ...
    function getStakedCYCLE() public view returns (uint256) {
        return IETHRewards(ETHRewards).balanceOf(address(this));
    }
    ...
    function xCYCLEtoCYCLE(uint256 xCYCLEamount) public view returns (uint256) {
        ...
        return
            xCYCLEsupply == 0
                ? 0
                : getStakedCYCLE().mul(xCYCLEamount).div(xCYCLEsupply);
    }
    ...
    function getRewardsEarned() external view returns (uint256) {
        return IETHRewards(ETHRewards).earned(address(this));
    }

    function getKickbackAmount() external view returns (uint256) {
        uint256 rewardsEarned = IETHRewards(ETHRewards).earned(address(this));
    ...
    function deposit(uint256 amountCYCLE) external nonReentrant {
        ...
        xCYCLEtoMint = xCYCLEsupply == 0
            ? amountCYCLE
            : amountCYCLE.mul(xCYCLEsupply).div(getStakedCYCLE());

        _mint(msg.sender, xCYCLEtoMint);

        IERC20(CYCLE).safeIncreaseAllowance(ETHRewards, amountCYCLE);
        IETHRewards(ETHRewards).stake(amountCYCLE);
    ...
    function withdraw(uint256 xCYCLEtoRedeem) external nonReentrant {
        require(
            xCYCLEtoRedeem <= balanceOf(msg.sender),
            "xCycle: Insufficient xCYCLE balance"
        );
        ...
        IETHRewards(ETHRewards).withdraw(amountCYCLEtoWithdraw);
    ...
    function reinvest() external nonReentrant {
        require(
            ...
            "xCycle: Caller is not an EOA"
        );

        IETHRewards(ETHRewards).getReward();

        uint256 balanceETH = getBalanceETH();
        uint256 kickbackAmount = balanceETH.mul(kickbackBP).div(BP_DIV);
        (bool success, ) = msg.sender.call{value: kickbackAmount}("");
        require(success, "xCycle: Unable to transfer ETH");

        balanceETH = getBalanceETH();
        IRouter(Router).swapExactETHForTokens{value: balanceETH}(
            0,
            swapPath,
            address(this),
            block.timestamp
        ...
        IERC20(CYCLE).safeIncreaseAllowance(ETHRewards, balanceCYCLE);
        IETHRewards(ETHRewards).stake(balanceCYCLE);
