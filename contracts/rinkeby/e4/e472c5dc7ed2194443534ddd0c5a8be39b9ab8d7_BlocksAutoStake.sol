...
contract BlocksAutoStake is IERC777Recipient, Ownable, ReentrancyGuard {
    ...
    function _fetchUserReward() internal view returns (uint256 rewards){
        uint256 _timeSinceClaim = block.timestamp - userAccount[msg.sender].claimTime;
        uint256 _userStakeFactor = _timeSinceClaim * userAccount[msg.sender].allocatedBalance;
        uint256 _stakeReward = _userStakeFactor / stakeRewardFactor;
        if(block.timestamp > stakeRewardEndTime) {
            _timeSinceClaim = (block.timestamp - userAccount[msg.sender].claimTime) - stakeRewardEndTime;
            _userStakeFactor = _timeSinceClaim * userAccount[msg.sender].allocatedBalance;
    ...
    function claimRewards() external nonReentrant {
        ...
        uint256 _claimAmount = _fetchUserReward();
    ...
    function withdrawAll() external nonReentrant {
        ...
        uint256 _withdrawalAmount = userAccount[msg.sender].allocatedBalance + _fetchUserReward();
    ...
    function withdrawAmount(uint256 _amount) external nonReentrant {
        ...
        uint256 _withdrawalAmount = _amount + _fetchUserReward();
        require(_withdrawalAmount <= _contractBalance(), "Contract balance too low.");
