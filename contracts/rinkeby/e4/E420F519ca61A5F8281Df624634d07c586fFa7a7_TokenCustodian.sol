...
contract TokenCustodian is AccessControlEnumerableUpgradeable {
    ...
    event Retrieved(Branch indexed branch, address indexed beneficiary, uint256 amount);
    ...
    string[NUMBER_OF_BRANCHES] public BRANCHES = [
        "STRATEGIC_SALE",
        "WORKING_GROUPS",
        "ECOSYSTEM_GRANTS"
    ];
    ...
    function retrieve(Branch branch_, address beneficiary_, uint256 amount, bool claimPending) external onlyRole(branchByIndex[branch_].role) {
        ...
        emit Retrieved(branch_, beneficiary_, amount);
    }
    ...
    function configureBeneficiaryInBranch(
        ...
        beneficiary.maxAmount = totalLimit > uint256(beneficiary.claimedAmount)
            ? uint120(totalLimit)
            : uint120(beneficiary.claimedAmount);

        beneficiary.isRegistered = isRegistered;

        emit BeneficiaryConfigured(branch_, beneficiary_, totalLimit, isRegistered);
    }
