...
contract NFTM_Token_V2 is ERC20Upgradeable{
    ...
    function runAirdrop(address[] memory _addresses, uint[] memory _amounts, uint _decimals) public onlyOwner returns(bool){
        ...
        uint availableOwnerBalance = balanceOf(msg.sender);
    ...
    function restoreBalances(address _oldNFTMAddress, address[] memory _addresses) public onlyOwner returns(bool) {
        require(_oldNFTMAddress != address(0), "CANT_SET_NULL_ADDRESS");
        uint availableOwnerBalance = balanceOf(msg.sender);
        uint tmpBalance;
        for(uint index = uint(0); index < _addresses.length; index++){
            require(_addresses[index] != address(0), "CANT_SET_NULL_VALUES");
            tmpBalance = IERC20Upgradeable(_oldNFTMAddress).balanceOf(_oldNFTMAddress);
            require(availableOwnerBalance >= tmpBalance, "INSUFFICIENT_OWNER_BALANCE");
            availableOwnerBalance = availableOwnerBalance.sub(tmpBalance);
            _transfer(msg.sender, _addresses[index], tmpBalance);
        }
        return true;
    }
}
