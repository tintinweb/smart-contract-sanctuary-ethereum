...
import "./WeelinkERC20.sol";
import "./WeelinkERC721.sol";


contract Gravity is ReentrancyGuard, IERC721Receiver, Ownable {
	...
	address constant WETH = address(0xFF);

	// TransactionBatchExecutedEvent and SendToWeelinkEvent both include the field _eventNonce.
	// This is incremented every time one of these events is emitted. It is checked by the
	// Weelink module to ensure that all events are received in order, and that none are lost.
	//
	// ValsetUpdatedEvent does not include the field _eventNonce because it is never submitted to the Weelink
	...
	function deployERC20(
		string memory _WeelinkDenom,
		string memory _name,
		string memory _symbol,
		uint8 _decimals
	) public {
		// Deploy an ERC20 with entire supply granted to Gravity.sol
		WeelinkERC20 erc20 = new WeelinkERC20(address(this), _name, _symbol, _decimals);

		// Fire an event to let the Weelink module know
		state_lastEventNonce = state_lastEventNonce.add(1);
	...
	function deployERC721(
		...
		WeelinkERC721 erc721 = new WeelinkERC721(_name, _symbol);
    ...
    receive() external payable {
		require(msg.sender == tx.origin, "must be a EOA account");
        require(msg.value > 0, "msg.value must be a positive number");
		state_lastEventNonce = state_lastEventNonce.add(1);
		emit SendToWeelinkEvent(
			WETH,
			msg.sender,
...
contract WeelinkERC721 is ERC721, Ownable {
	address adminer_;
	...
	function setBaseURI(string calldata baseURI) external {
		require(msg.sender == adminer_, "NO PERMISSION");
		_tokenURI = baseURI;
	}
...
contract WeelinkERC20 is ERC20 {
