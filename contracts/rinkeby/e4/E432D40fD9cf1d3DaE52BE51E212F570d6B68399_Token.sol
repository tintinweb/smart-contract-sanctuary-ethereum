...
contract Token is ERC721, Ownable, Pausable {
    ...
    // Maximum allowed tokens to be minted per whitelisted address during the
    // Multi
    uint32 constant MAX_WL_MINT = 4;
    
    // Maximum allowed tokens to be helpd by the minting account during the
    // public minting phase.
    uint32 constant MAX_PUBLIC_MINT = 2;
    ...
    // Keeps track of how many tokens each whitelist address has minted so far.
    mapping (address => uint256) public wlMintCount;

    // Counts how many tokens have been minted
    using Counters for Counters.Counter;
    Counters.Counter private _idCounter;

    // The URI of the default token JSON, to be used before reveal. 
    string private _defaultURI;

    // The base URI of the revealed token JSONs.
    string private _metadataBaseURI;

    // The address that will receive the funds withdrawals.
    ...
    // todo just to introduce the whenNotPaused
    ...
    function mint(uint32 mintNum, bytes32[] calldata merkleProof)
