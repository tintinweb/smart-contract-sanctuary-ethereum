...
⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀
  */
  ...
  mapping(uint256 => bytes32) public pangoNames;
  mapping(bytes32 => bool) public namesUsed;
  ...
  modifier nameCompliance(uint256 _mintAmount, bytes32[] calldata newNames) {
    ...
    for (uint i=0; i<newNames.length; i++) {
        require(!namesUsed[newNames[i]], "Name already claimed!");
    }
    _;
  }
  ...
  function setName(uint256 id, bytes32 newName) public onlyOwner {
    require(!namesUsed[newName], "Name already claimed!");
    pangoNames[id] = newName;
    namesUsed[newName] = true;
  }
  function setNames(uint256 startIndex, bytes32[] calldata newNames) public onlyOwner {
    for (uint256 i=startIndex; i<newNames.length; i++) {
        setName(i,newNames[i]);
    }
  }
