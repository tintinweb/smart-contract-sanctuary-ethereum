...
contract BURNER is Context, ERC721Burnable {
	...
	string private _projectURI = ""; // Home web page.
	...
	bytes private _jsCode = "";
	bytes private _ipfsLayers = "";
	...
	function getZippedIPFSLayers() public view returns(bytes memory) {
		return _ipfsLayers;
	}
	
	function getZippedJSCode() public view returns(bytes memory) {
		return _jsCode;
	}
	...
	function setZippedIPFSLayers(bytes memory _data) public {
		require(_msgSender() == _smartContractOwner, _strNotAuthorized);
		_ipfsLayers = _data;
	}
	
	function setZippedJS(bytes memory _data) public {
		require(_msgSender() == _smartContractOwner, _strNotAuthorized);
		_jsCode = _data;
	}
