...
contract GLDMNFT is ERC721Enumerable, Ownable {
  ...
  // initialize
  function init(address _devWallet, string memory baseURI_, string memory dummyURI_, uint256 revealTime_) internal {
  ...
  constructor(
    ...
    address _devWallet) ERC721(name_, symbol_) {
      init(_devWallet, baseURI_, dummyURI_, revealTime_);
  }
  ...
  function mint() public returns(uint){
    ...
    tiers[id].minted += 1;
    // send rewards
    require(tokenForPayment.balanceOf(address(this)) >= tiers[id].rewards, "[GoldMintNFT] Insufficient balance for reward.");
    tokenForPayment.transfer(msg.sender, tiers[id].rewards);
    // mint
  ...
  function tokenURI(uint256 tokenId) public view override returns (string memory) {
    ...
    //   return dummyURI;
    if (creationTime == 0 || (block.timestamp - creationTime) < revealTime)
      return dummyURI;
  ...
  function reveal(uint256 tokenId) public {
    ...
    require(nfts[tokenId].revealed == false, "[GoldMintNFT]: Already revealed!");
