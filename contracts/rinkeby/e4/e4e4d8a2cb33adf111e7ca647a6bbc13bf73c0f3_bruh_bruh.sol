...
contract bruh_bruh is Ownable, ERC721A{
    ...
    string private revealedURI = "ipfs://QmXd8zZj1hsD1PaUFTBRMDhyx7Bj71Kmehgmq2iWZ59Tod/";

    // OpenSea CONTRACT_URI - https://docs.opensea.io/docs/contract-level-metadata
    string private CONTRACT_URI = "ipfs://BIQD91HS4EIXUFNX5IMPY8APAZRSBD53EW";
    ...
    function Freedegens(uint256 quantity) external payable mintCompliance(quantity) {
        require(freeSale, "not free fool");
        require(msg.value == 0, "this is free now");
        require(quantity == 2, "only #2 bro");

        uint256 newSupply = totalSupply() + quantity;
        
        require(newSupply <= 200, "you missed it");
    ...
    function Paiddegens(uint256 quantity) external payable mintCompliance(quantity) {
    ...
    function howmuch(uint256 _publicPrice) public onlyOwner {
        publicPrice = _publicPrice;
    }

    function ithinkpictures(string memory _baseUri) public onlyOwner {
        revealedURI = _baseUri;
    }

    function thecontractthingy(string memory _contractURI) private onlyOwner {
        CONTRACT_URI = _contractURI;
    }

    // Note: Another option is to inherit Pausable without implementing the logic yourself.
       
    function gomode(bool _state) public onlyOwner {
        paused = _state;
    }

    function peekaboo(bool _state) public onlyOwner {
        revealed = _state;
    }

    function buydummy(bool _state) public onlyOwner {
        publicSale = _state;
        freeSale = !_state;
    }
    function freeeee(bool _state) public onlyOwner {
        freeSale = _state;
        publicSale = !_state;
    }


    function takemoney() external payable onlyOwner {
