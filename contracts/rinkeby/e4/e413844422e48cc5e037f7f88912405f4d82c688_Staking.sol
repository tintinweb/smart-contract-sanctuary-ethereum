...
contract Staking is Ownable {
    ...
    uint256 public minimumAmount = 1; // Minimum Amount Required to deposit
    ...
    constructor(address _operator, address _vaultAddress, address _StakedTokenAddress, address _RewardTokenAddress) {
        operator = _operator;
    ...
    function updateStakedToken(address _address) external onlyOwner {
        StakedToken = IERC20(_address);
    }

    function updateRewardToken(address _address) external onlyOwner {
    ...
    function updateOperator(address _address) external onlyOwner {
        operator = _address;
    }
    ...
    //@dev Deposit specified Staked amount l
    function deposit( uint256 _amount) external {

        address _userAddress = msg.sender;
        require(_amount >= minimumAmount, "Minimum deposit");

        //If fresh account require a minimal amount of Staked
    ...
    function claim(address userAddress, uint conversionRate) external {
        require(msg.sender == operator || msg.sender == owner(), "Only operator or owner can invoke");
    ...
    function _claim_out(address _addr, uint conversionRate) internal {
        ...
        total_compounded -= users[_addr].compounds;
    ...
    function _withdraw_principal(address _addr, uint conversionRate) internal {
        ...
        require(
            ...
            "Failed to transfer Staked tokens"
        );

        total_withdraw++;
        total_deposited -= _withdraw_amt;
    ...
    function userInfo(address _addr) external view returns(uint256 deposit_time, uint256 deposits, uint256 payouts, uint256 compounds, uint total_payout) {
        (uint256 _payout) = payoutOf(_addr);
        return (users[_addr].deposit_time, users[_addr].deposits, users[_addr].payouts, users[_addr].compounds, _payout );
    }
