...
contract Wormhole {
  ...
  uint public credit_conversion=100*10**18;//1 FRAX buys this amount of credits
  address public frax=0x01BE23585060835E02B77ef475b0Cc51aA1e0709;
  ...
  function refund_credits() external returns (uint){
    ...
    IERC20(frax).transfer(msg.sender, _div);
  ...
  function buy_credits(uint frax_amount) public{
    require(banned_streamers[msg.sender] ==false, "You are banned from acquiring credits!");

    IERC20(frax).transferFrom(msg.sender, address(this), frax_amount);
    uint _mul=frax_amount.mul(credit_conversion);
    uint _div=_mul.div(10**18);
    credit_balance[msg.sender]=credit_balance[msg.sender]+_div;
  }
  function stream_payment_admin(address streamer_address, address viewer_address) public{
      require(owner == msg.sender, "No permission to stream payment");
      uint cost=stream_array[streamer_address].cost;
      require(cost > 0, "Streamer not active");
      require(cost <=credit_balance[viewer_address], "You don't have enough credits left");

      uint credit_after_deducting_fee=return_credit_after_fee(cost);
      uint fee_allocation=cost.sub(credit_after_deducting_fee);


      credit_balance[viewer_address]=credit_balance[viewer_address]-cost;
      ...
      emit Stream_Payment(viewer_address, cost, streamer_address, this_creator_name, block.timestamp);
