...
contract Ownable {
    ...
    constructor(address owner_) {
        _owner = owner_; // Owner wallet address
...
contract Stakeable {
    ...
    function _stake(
        ...
        stakeholders[index].address_stakes.push(
            Stake(account, _amount, timestamp, 0, method, false)
        );
        // Emit an event that the stake has occured
        emit Staked(account, _amount, index, timestamp, method, false);
    }
        ...
        returns (uint256)
    {
        ...
        uint256 _months = (block.timestamp - _current_stake.since) / 1 minutes;
        ...
        for (uint256 s = 0; s < stakeLength; s += 1) {
            ...
            if (_months >= 6 * summary.stakes[s].method) {
                totalWithdrawableRewardAmount +=
                    ...
                    availableReward;
                summary.stakes[s].unlocked = true;
            }
        }
