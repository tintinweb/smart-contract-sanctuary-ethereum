...
contract UnipilotFarm is IUnipilotFarm, ReentrancyGuard {
    ...
    ///@notice use for blacklist to list of the pools
    ///@param _pools list of pools
    function blacklistPools(address[] memory _pools)
        external
        override
        onlyGovernance
    {
        address[] memory listed;
        for (uint256 i = 0; i < _pools.length; i++) {
            poolWhitelist[pools[i]] = false;

            if (poolInfo[_pools[i]].reward == RewardType.Dual) {
                updatePoolState(_pools[i]);
                updateAltState(_pools[i]);
            } else if (poolInfo[_pools[i]].reward == RewardType.Alt) {
                updateAltState(_pools[i]);
            } else {
                updatePoolState(_pools[i]);
            }
        ...
        emit BlacklistPools(listed, block.timestamp);
    }
...
interface IUnipilotFarm {
    ...
    event BlacklistPools(address[] pools, uint256 timestamp);
