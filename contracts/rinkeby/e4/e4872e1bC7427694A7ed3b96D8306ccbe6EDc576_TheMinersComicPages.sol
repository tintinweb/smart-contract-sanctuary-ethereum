...
/*    
         .___..  ..___  .__..__ .  ..___.  ..___..  ..__ .___ __.  .__..___
...
*/                                                                   
  ...
  struct page {
    ...
    uint256 endIndex;
    bool revealed;
    bool doubleDown; // Option enables a second mint for those who have already minted
  }
   ...
   */
  function mintForAddressAdmin(address _receiver,uint256 _mintAmount) public mintComplianceAdmin() onlyOwner {
  ...
  /**
   * View a specified token's metadata
   */
  ...
  /**
   * View all tokens that belong to a specified page
   */
  function pageShowAllTokens(uint _pageId) public view returns (uint256[] memory){
    uint256 numberOfTokensOnPage;
    uint256 endIndex;
    // if(_pageId == currentPage){
    //   //edge case to handle current page tokens
    //   numberOfTokensOnPage = _currentIndex - pages[_pageId].startIndex;
    //   endIndex = _currentIndex;
    // }
    // else{
      numberOfTokensOnPage = pages[_pageId].endIndex - pages[_pageId].startIndex + _startTokenId();
      endIndex = pages[_pageId].endIndex;
  //  }

    uint256[] memory pageIds = new uint256[](numberOfTokensOnPage);
    for(uint256 tokenId=pages[_pageId].startIndex; tokenId<=endIndex; tokenId++){
      pageIds[tokenId] = tokenId;
    }
    return pageIds;
  }

  /**
   * View page number of specified token
   */
  function tokenToPage(uint _tokenId) public view returns (uint256){
    require(_exists(_tokenId), "ERC721Metadata: URI query for nonexistent token");
    uint256 pageNumber;
    for(uint256 i=0; i<=currentPage; i++){
      if(_tokenId >= pages[i].startIndex && _tokenId <= pages[i].endIndex){
        pageNumber = i;
      }
    }
    return pageNumber;
  ...
  /**
   * Sets the first token to be number 1 instead of 0, cuz well.. you know.
   */
