...
contract Vesting is EIP712, Ownable {
    ...
    function setTimePoint(uint256 _startDate, uint256 _endDate)
        external
        onlyOwner
    {
        require(
            _startDate < _endDate && block.timestamp < _startDate,
            "Incorrect dates"
        );
        startDate = _startDate;
        endDate = _endDate;
        emit SetTimePoint(_startDate, _endDate);
    }
    ...
    function _setVesting(
        ...
        if (VestingType.LINEAR_VESTING == _vestingType) {
            ...
            uint256 lastIntervalStartingTimestamp = endDate;
    ...
    function _harvest(address _addr) internal {
        require(_isVestingStarted(), "Vesting can't be started");

        uint256 amountToTransfer = _calculateUnlock(_addr);
    ...
    function _calculateUnlock(address _addr) internal view returns (uint256) {
        ...
        if (VestingType.LINEAR_VESTING == _vestingType) {
            uint256 passePeriod = Math.min(
                (block.timestamp - endDate) / info.periodDuration,
                info.countPeriodOfVesting
            );
            uint256 calculateUnlock = (((tokenAmount - initialUnlockAmount) *
                passePeriod) / info.countPeriodOfVesting) + initialUnlockAmount;
            return
                calculateUnlock > oldRewards ? calculateUnlock - oldRewards : 0;
        } else if (VestingType.INTERVAL_VESTING == _vestingType) {
            uint256 unlockPercentage = _initialPercentage;
            for (uint256 i = 0; i < info.unlockIntervals.length; i++) {
                if (block.timestamp > info.unlockIntervals[i].timeStamp) {
                    unlockPercentage = info.unlockIntervals[i].percentage;
                }
            }
            initialUnlockAmount =
                ...
                MAX_INITIAL_PERCENTAGE;
            return
                initialUnlockAmount > oldRewards
                    ? initialUnlockAmount - oldRewards
                    : 0;
        }
    ...
    function _isVestingStarted() internal view returns (bool) {
        return block.timestamp > endDate && endDate != 0;
    }
    ...
    function _isValidAmount(uint256 _amount) internal view returns (bool) {
        uint256 maxAllocation = specificAllocation[msg.sender] > 0
            ? specificAllocation[msg.sender]
            : _maxAllocation;
