...
contract GAX_FNFT is ERC721Upgradeable {
    ...
    function mintToken(
        ...
        // erc20 mint
        // GAX_Artbits artbitInstance = new GAX_Artbits();
        // address artbitInstanceAdr = address(artbitInstance);
                ...
                address(
                    _createProxy(
                        ...
                        abi.encodeWithSignature(
                            "initialize(string,string,uint256,address,uint8)",
                            name,
                            symbol,
                            _noOfArtbits,
                            gaxAdmin,
      ...
      //  emit ArtbitsMinted(tokenId, artbitInstanceAdr, _uri);
    }
    ...
    function addCommissionAmt(uint256 amt) external onlyAdmin {
        totalCommission += amt;
    }
    ...
    function addRoyaltyAmt(uint256 amt) external onlyAdmin {
        totalRoyalty += amt;
    }
...
contract GAX_Artbits is ERC20Upgradeable, Singleton {
    uint256 nftId;
    uint256 dex_rp;
    uint256 p2p_rp;
