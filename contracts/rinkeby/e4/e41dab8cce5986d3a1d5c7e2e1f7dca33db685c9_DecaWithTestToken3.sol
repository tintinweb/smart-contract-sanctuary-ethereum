...
contract DecaWithTestToken3 is VRFConsumerBaseV2, Pausable, Ownable, ReentrancyGuard {
    ...
    uint256 public flippedVolume;
    uint256 public totalFlips;
    uint256 public totalHeads;
    uint256 public totalTails;
    ...
    mapping(address => uint256) public userClaimablePrizes;
    mapping(address => bool) public totalPlayers;
    uint256 public totalPlayersAmount;
    ...
    mapping(address => Balance)  userBankRollBalance;
    mapping(address => Balance)  userHouseBalance;
    ...
    event RandomWordsResponse(uint256[] randomWords);
    ...
    // Displays the Current Fee schedule.
    ...
    // Displays the wallet’s remaining Lock Time. (ex: 27:23:59:45)
    function getRemainingLockTime(address _address) public view returns (uint256){
        return 0;
    }

    // Takes Input of Wallet Address, displays the user’s Bankroll balance.
    function getMyBankrollBalance(address _address) public view returns (uint256){
        return userBankRollBalance[_address].depositAmount;
    }
    ...
    function getMyHouseBalance(address _address) public view returns (uint256) {
        return userHouseBalance[_address].depositAmount;
    }

    // Takes input of Wallet Address, displays information / status of last 10 bets of the wallet.
    function getMyLastTenBetsHistory(address _address) public view {
    ...
    function getRandomWords() public view returns (uint256){
        return s_requestIdToRandomWords[s_requestId][0];
    }

    /* ========== MUTATIVE FUNCTIONS ========== */

    // Takes Input of USDC amount, user deposits money to play casino.
    function depositBankroll(uint256 _amount) external {
        address userAddress = msg.sender;
        IERC20(USDC_ADDRESS).transferFrom(userAddress, address(this), _amount);
        userBankRollBalance[userAddress].depositAmount += _amount;
    }

    // Takes input of USDC amount, user withdraws money from his Bankroll.
    function withdrawBankroll(uint256 _amount) external nonReentrant {
        address userAddress = msg.sender;
        require(
            userBankRollBalance[userAddress].depositAmount >= _amount,
            "exceed amount"
        );
        _transfer(USDC_ADDRESS, userAddress, _amount);
        userBankRollBalance[userAddress].withdrawAmount.add(_amount);
        userBankRollBalance[userAddress].depositAmount.add(_amount);
    }
    ...
    function depositHouse(uint256 _amount) external {
        address userAddress = msg.sender;
        IERC20(USDC_ADDRESS).transferFrom(userAddress, address(this), _amount);
        userHouseBalance[userAddress].depositAmount += _amount;
    ...
    function withdrawHouse(uint256 _amount) external nonReentrant {
        ...
        userHouseBalance[userAddress].depositAmount -= _amount;
        userHouseBalance[userAddress].withdrawAmount += _amount;
        housePoolBalance.depositAmount -= _amount;
        housePoolBalance.withdrawAmount += _amount;
    }
    ...
    function claimTreasury(uint256 _amount) external onlyTreasury nonReentrant {
        ...
        treasuryPoolBalance.withdrawAmount += _amount;
    ...
    // Takes input of Payout Uint Number, changes the Payout x.
    function setPayout(uint256 _payout) external onlyOwner {
        payout = _payout;
    }

    // Takes input of Fee Uint Number, changes the Fee taken from bet for Treasury.
    ...
    function bet(uint256 _betAmount, uint256 _faceSide) external {
        require(_betAmount <= maxBet && _betAmount >= minBet, "Adjust the bet in 1 to 10");
        require(_faceSide >= 1 && _faceSide <= 100, "Face side must be 1 to 100");
        require(_betAmount <= userBankRollBalance[msg.sender].depositAmount, "exceed amount");
        ...
        flippedVolume = flippedVolume.add(_betAmount);

        if (!totalPlayers[msg.sender]) {
            totalPlayers[msg.sender] = true;
            totalPlayersAmount = totalPlayersAmount.add(1);
        }
    ...
    function fulfillRandomWords(
        ...
        uint256 sortedFace = randomWords[0].mod(100).add(1);
        // 1 to 50 is Head, 51 to 100 is Cross
        ...
        uint256 possibleWin = requestToPossibleWin[requestId];

//        uint256 winMoney = payout.mul(betAmount);
        bool userWon;

        if (sortedFace <= 50 && playerPredict <= 50) {// user bet and result is Head
            userWon = true;
            userClaimablePrizes[player] = userClaimablePrizes[player].add(possibleWin);
            // win
        ...
        } else if (sortedFace >= 51 && playerPredict >= 51) {// user bet and result is Cross
            userWon = true;
            userClaimablePrizes[player] = userClaimablePrizes[player].add(possibleWin);
            // win
            playerStats[player].totalWins = playerStats[player].totalWins.add(1);
...
//        uint256 _fee = betAmount.mul(fee).div(100);
//        treasuryPoolBalance.depositAmount.add(_fee);
//        if (userWon) {
//            userBankRollBalance[msg.sender].depositAmount.add(winMoney);
//            housePoolBalance.depositAmount.sub(winMoney);
//        } else {
//            userBankRollBalance[msg.sender].depositAmount.sub(winMoney);
//            housePoolBalance.depositAmount.add(winMoney);
//        }
//        emit RandomWordsResponse(randomWords);
    }
}