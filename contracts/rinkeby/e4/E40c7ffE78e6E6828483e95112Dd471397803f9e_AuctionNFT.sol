...
contract AuctionNFT is Accept {
  ...
  event AddBundleAuction(uint256 _orderNumber,address _nftAddress,address _seller,uint256 _price,uint256 _startTime,uint256 _deadline, uint256 _royalityPercent, bool _isOpenForBids, string _nftType);
  ...
  function addAuction(
    ...
    require((startTime >= now) && deadline > startTime, "Set valid auction time");
  ...
  function bid(uint256 orderNumber, uint256 _amount) public payable {
    ...
    if (keccak256(bytes(tokenOf[order.tokenDet[orderNumber].tokenAddress])) == keccak256(bytes("eth"))) {
      ...
      require(IBEP20(order.tokenDet[orderNumber].tokenAddress).allowance(msg.sender, address(this)) >= _amount, "Not Enough Allownce");
  ...
  // Auction Finalized by seller or Admin any time
  function finalizeAuction(uint256 orderNumber) external isSellerOrAdmin(orderNumber) {
    Order storage order = pendingOrders[orderNumber];
    require(order.bidList.length != 0, "No Bid Found");
    if (!order.isOpenForBids) {
  ...
  // Get Specific Order Bid detail by index
  ...
  // Cancel Auction by seller or admin
  ...
  function addNftBundle(
    ...
    emit AddBundleAuction(orderNumber, nftAddress, msg.sender, price, startTime, deadline, royalityPercent, isOpenForBids, "bundle");
  }

  // Offer Discount on Orders
  function updatePrice(uint256[] calldata _orderNumbers, uint256 _percentage) external {
    ...
    for (uint256 i = 0; i < _orderNumbers.length; i++) {
      ...
      require(pendOrd.seller == msg.sender, "Only Owner could Update");
  ...
  //Pending Order Detail of nft bundle ids and bidlists
