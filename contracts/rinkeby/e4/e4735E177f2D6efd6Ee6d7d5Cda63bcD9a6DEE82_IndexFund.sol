...
contract IndexFund is IIndexFund {
    ...
    modifier onlyTimelock() {
        require(msg.sender == _timelock, "NT");
        _;
    }
    ...
    function withdraw(address[] memory _tokenAddresses, uint256 _pilotAmount) external override {
        ...
        for (uint256 i = 0; i < _tokenAddresses.length; i++) {
            ...
            require(withdraws[withdrawnId] == false, "TAW");
            ...
            emit Withdrawn(_tokenAddresses[i], tokenPercentageToTransfer);
        }
        emit Burned(circulatingPilotSupply, _pilotAmount);
    }
    ...
    function removeLockedFundsAddress(uint256 _index) external onlyTimelock {
        if (lockedFundAddresses[_index] != address(0)) {
    ...
    function circulatingSupply() public view override returns (uint256 supply) {
        uint256 totalLocked;
...
interface IIndexFund {
    ...
    event Withdrawn(address indexed tokenAddress, uint256 indexed tokenAmount);

    event Burned(uint256 indexed circulatingPilotSupply, uint256 indexed pilotAmount);

    event EthRecieved(uint256 indexed amount);
}
