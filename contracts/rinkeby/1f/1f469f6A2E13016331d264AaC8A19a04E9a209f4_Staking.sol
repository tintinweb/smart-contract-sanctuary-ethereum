...
contract Staking is Initializable, ReentrancyGuardUpgradeable, OwnableUpgradeable, PausableUpgradeable {
     ...
     * @param amount amount to deposit (in MPWR)
     */
    function depositFor(address user, uint256 _amount) external nonReentrant {
        require(_amount > 0, "Deposit: Amount must be > 0");
        ...
        compound();
        // Transfer mpwrToken tokens to this contract
        mpwrToken.safeTransferFrom(user, address(this), _amount);
    ...
    function calculatePendingRewards(address user) public view returns (uint256) {
        ...
        if ((block.number > lastRewardBlock) && (totalAmountStaked != 0)) {
            ...
            uint256 tokenPerShare = ((stakers[user].amount * PRECISION_FACTOR) / totalAmountStaked) *
