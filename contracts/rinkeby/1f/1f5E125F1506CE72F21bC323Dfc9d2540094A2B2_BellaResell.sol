...
contract BellaResell is Initializable, UUPSUpgradeable, ReentrancyGuardUpgradeable, IERC165Upgradeable, AccessControlUpgradeable {
        ...
        nonReentrant 
    {
        ...
        Item memory item = idToItem[itemId];
        require(quantity > 0 && quantity <= item.supply, "ERR-15-BR");
        require(msg.sender != item.seller, "ERR-5-BR");
        (bool result, uint256 requiredValue) = item.price.tryMul(quantity);
        if(result) {
            require(msg.value == requiredValue, _concatenateStringUint("ERR-16-BR", requiredValue));
        } else {
            revert("ERR-6-BR");
        }
