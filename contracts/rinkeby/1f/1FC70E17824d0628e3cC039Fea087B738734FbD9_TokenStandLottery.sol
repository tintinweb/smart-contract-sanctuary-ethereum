...
/** @title TokenStand Lottery.
 * @notice It is a contract for a lottery system using
 * randomness provided externally.
 */
contract TokenStandLottery is ReentrancyGuard, ITokenStandLottery, Ownable {
    ...
    struct Lottery {
        ...
        uint256 startTime;
        uint256 endTime;
        uint256 priceTicketInStand;
        uint256 discountDivisor;
        uint256[6] rewardsBreakdown; // 0: 1 matching number // 5: 6 matching numbers
        uint256 burnFee; // 500: 5% // 200: 2% // 50: 0.5%
        uint256[6] standPerBracket;
        uint256[6] countWinnersPerBracket;
        uint256 firstTicketId;
        uint256 firstTicketIdNextLottery;
        uint256 amountCollectedInStand;
        uint32 finalNumber;
    }
    ...
    event NewOperatorAndInjectorAddresses(
        address operator,
        address injector
    );
     ...
     * @param _standTokenAddress: address of the STAND token
     * @param _randomGeneratorAddress: address of the RandomGenerator contract used to work with ChainLink VRF
     */
    constructor(address _standTokenAddress, address _randomGeneratorAddress) {
        standToken = IERC20(_standTokenAddress);
        ...
        nonReentrant
    {
        ...
        // Calculate number of STAND to this contract
        uint256 amountStandToTransfer = _calculateTotalPriceForBulkTickets(
            _lotteries[_lotteryId].discountDivisor,
            _lotteries[_lotteryId].priceTicketInStand,
            _ticketNumbers.length
        );

        // Transfer stand tokens to this contract
        standToken.safeTransferFrom(
            address(msg.sender),
            address(this),
            amountStandToTransfer
        );

        // Increment the total amount collected for the lottery round
        _lotteries[_lotteryId].amountCollectedInStand += amountStandToTransfer;
    ...
    function claimTickets(
        ...
        // Initializes the rewardInStandToTransfer
        uint256 rewardInStandToTransfer;
        ...
        for (uint256 i = 0; i < _ticketIds.length; i++) {
            ...
            rewardInStandToTransfer += rewardForTicketId;
        }

        // Transfer money to msg.sender
        standToken.safeTransfer(msg.sender, rewardInStandToTransfer);

        emit TicketsClaim(
            msg.sender,
            rewardInStandToTransfer,
            _lotteryId,
            _ticketIds.length
        );
    }
    ...
    /**
     * @notice Draw the final number, calculate reward in STAND per group, and make lottery claimable
     * @param _lotteryId: lottery id
    ...
    function drawFinalNumberAndMakeLotteryClaimable(
        ...
        uint256 amountToShareToWinners = (
            ((_lotteries[_lotteryId].amountCollectedInStand) *
                (10000 - _lotteries[_lotteryId].burnFee))
        ) / 10000;

        // Initializes the amount to withdraw to burn
        uint256 amountToWithdrawToBurn;

        // Calculate prizes in STAND for each bracket by starting from the highest one
        for (uint32 i = 0; i < 6; i++) {
            ...
            if (
                ...
                if (_lotteries[_lotteryId].rewardsBreakdown[j] != 0) {
                    _lotteries[_lotteryId].standPerBracket[j] =
                ...
                // A. No STAND to distribute, they are added to the amount to withdraw to burn address
            } else {
                _lotteries[_lotteryId].standPerBracket[j] = 0;

                amountToWithdrawToBurn +=
                    (_lotteries[_lotteryId].rewardsBreakdown[j] *
                        amountToShareToWinners) /
                    10000;
            }
        ...
        if (_autoInjection) {
            pendingInjectionNextLottery = amountToWithdrawToBurn;
            amountToWithdrawToBurn = 0;
        }

        amountToWithdrawToBurn += (_lotteries[_lotteryId]
            .amountCollectedInStand - amountToShareToWinners);

        // Transfer STAND to address dead
        standToken.safeTransfer(0x000000000000000000000000000000000000dEaD, amountToWithdrawToBurn);
     ...
     * @notice Inject funds
     * @param _lotteryId: lottery id
     * @param _amount: amount to inject in STAND token
        ...
        onlyOwnerOrInjector
    {
        ...
        standToken.safeTransferFrom(address(msg.sender), address(this), _amount);
        _lotteries[_lotteryId].amountCollectedInStand += _amount;
     ...
     * @param _priceTicketInStand: price of a ticket in STAND
    ...
    function startLottery(
        ...
        _lotteries[currentLotteryId] = Lottery({
            ...
            endTime: _endTime,
            priceTicketInStand: _priceTicketInStand,
            ...
            amountCollectedInStand: pendingInjectionNextLottery,
            finalNumber: 0
        });
    ...
    function recoverWrongTokens(address _tokenAddress, uint256 _tokenAmount)
        ...
        onlyOwner
    {
        require(_tokenAddress != address(standToken), "Cannot be STAND token");
    ...
    /**
     * @notice Set operator, burn, and injector addresses
     * @dev Only callable by owner
     ...
     * @param _priceTicket price of a ticket (in STAND)
    ...
    function _calculateRewardsForTicketId(
        ...
        if (transformedWinningNumber == transformedUserNumber) {
            return _lotteries[_lotteryId].standPerBracket[_bracket];
        } else {
            return 0;
        }
    }
