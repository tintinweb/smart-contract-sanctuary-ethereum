...
contract GenesisCollection is ERC721, AccessControl {
    ...
    uint256 private constant MAX_WHITELIST_MINT = 25;
    ...
    function premint(uint256 premintCount) public onlyRole(DEFAULT_ADMIN_ROLE) {
        require(_preminted == 0, "Already preminted");
        for (uint256 i = 1; i <= premintCount; i++) {
            _safeMint(msg.sender, i);
        }
        _preminted = premintCount;
        // Set counter to the real value as to avoid gas from calling .increment
        _tokenIdCounter._value = premintCount;
    }

    function mintTo(uint256 mintCount) public payable {
        require(_mintStarted, "Mint not started yet");
        ...
        if (MinterContract.whitelistOnly()) {
            require(MinterContract.isWhitelisted(msg.sender), "User is not whitelisted!");
            // Whitelist mint is limited to 4000 tokens
            require(_tokenIdCounter.current() + mintCount < MAX_WHITELIST_MINT + _preminted, "Requested mint exceeds whitelist supply");
        }
