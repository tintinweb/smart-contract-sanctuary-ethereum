...
library Config{
    ...
    struct SalesPlan {
        SalesPlanItem[] _entries;
    }
  
   // function length(SalesPlan storage plans) internal view returns(uint){
   //     return plans._entries.length;
   // }

    function setActiveSale(SalesPlan storage plans, uint index) internal returns (bool) {
        require(plans._entries.length > index, "Config: index out of bounds");
        plans._entries[index]._sale = true;
    }
    function add(SalesPlan storage plans,uint256 price,uint16 statr,uint16 end,bool sale ) internal   {
      plans._entries.push(SalesPlanItem({_price:price,_startTokenid:statr,_endTokenid:end,_sale:sale}));
    }
 
    function at(SalesPlan storage plans,uint16 index) internal view returns(uint256,uint16,uint16,bool ){
        require(plans._entries.length > index, "Config: index out of bounds");
        SalesPlanItem storage entry = plans._entries[index];
        return (entry._price,entry._startTokenid,entry._endTokenid,entry._sale);
    }
    function atTokenid(SalesPlan storage plans,uint tokenid) internal view returns(uint256,uint16,uint16,bool ,uint){
        uint index = 0;
        for(uint i = 0;i<plans._entries.length;i++){
            if (plans._entries[i]._startTokenid <= tokenid && plans._entries[i]._endTokenid >= tokenid) {
                index = i+1;
                break;
            }
        }   
        if (index==0){
            ...
            SalesPlanItem storage entry = plans._entries[index-1];
            return (entry._price,entry._startTokenid,entry._endTokenid,entry._sale,index);
        }
    }
}
*/
...
contract Metart is ERC721,Ownable{
   ...
   // using Config for Config.SalesPlan;
   // using Config for Config.SalesPlanItem;
    using SafeMath for uint256;
    string public verifyHash = "";
     
    //单次铸币最大数
    //uint public constant maxMintCoinsSingle = 20;
    uint256 public constant Max_Number = 8248;
   //预售空投的最大的Token
   // uint16 public MintPreSales_Max=0;
    ...
    //当前批次mint最大值
    uint256 public mint_Number_Curr = 889;
    //init 0.08 ETH
   ...
   // uint256 public REVEAL_TIMESTAMP;
    ...
    //uint256 saleStart  ，, uint256 maxNftSupply
    constructor(string memory name, string memory symbol) ERC721(name, symbol) {
        //销售计划
        /*
        _salesCfg.add(80000000000000000,889,1624,true);
        _salesCfg.add(100000000000000000,1625,3096,false);
        _salesCfg.add(150000000000000000,3097,4568,false);
        _salesCfg.add(180000000000000000,4569,6776,false);        
        _salesCfg.add(200000000000000000,6777,8248,false); 
        */
       // MintPreSales_Max = 1624;
        //  startOvertSale = 889;
    }
    ...
    function setMintCurrMax(uint256 mintCurrMax,uint256 price) public onlyOwner{
        require(mintCurrMax >= totalSupply() && mintCurrMax <=Max_Number ,"mintCurrMax failed");
    ...
    function flipOpenSecondSale() public onlyOwner {
        //require(secondSale == false,"Switch on");
        secondSale = !secondSale;
    }
    /*
    function closeSecondSale() public onlyOwner {
        require(secondSale == true,"Switch off");
        secondSale = false;
    }
   */
    //撤回 balance
    ...
    */
    function mintMetartReserve() public   {
        //0-888 白名单 不要转帐,一次只能一个
        require(saleIsActive, "Sale must be active to mint Metart");
        //保留的NFT已铸造完
        require(indexReserveSale < startOvertSale,"The remaining NFT has been cast");
        ...
        _safeMint(msg.sender, indexReserveSale);
        indexReserveSale = indexReserveSale + 1;
        whiteList.set(msg.sender,number+1);

        emit MintMetart(msg.sender,number+1,2,0);
    }
}