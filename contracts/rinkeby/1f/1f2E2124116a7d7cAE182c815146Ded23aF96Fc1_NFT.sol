...
// File contracts/utils/FisherYatesShuffle.sol
...
// SPDX: MIT
pragma solidity ^0.8.9;
contract NFT is ERC721Enumerable, Ownable, FisherYatesShuffler {
    using SafeMath for uint256;
    // contract is ownable
    // implement hash string pointing to IPFS + collection item id
    // private string _setBaseURI("https://example.com/tokens/");
    ...
    // implement counter for collection items
    ...
    /** change modifier from public to external for gas reduction,
     *  because of stack transfer
     */
    ...
    function mintTo(address _to, uint256 _amount) public payable {
        ...
        require(_amount <= maxMintAmount, "Cannot mint this amount of tokens at once");
        require(
    ...
    */
    ...
    // TODO mint function ?
    // check id <= number of max nfts
    // -- mint should implement VRF through Chainlink
    function getFinalIds() external view returns (uint256[] memory) {
        uint256 seed = IVRFConsumer(vrfConsumer).s_randomWords();
        require(seed != 0, "Seed not generated");
        return shuffle(seed, maxSupply);
    }
}

interface IVRFConsumer {
    function s_randomWords() external view returns (uint256);
}
...
// File @chainlink/contracts/src/v0.8/mocks/[email protected]
