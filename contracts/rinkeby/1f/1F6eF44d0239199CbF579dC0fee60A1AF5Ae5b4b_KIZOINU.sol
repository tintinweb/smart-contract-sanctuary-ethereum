...
contract KIZOINU is ERC20, Ownable {
    ...
    function addToLiquidity1(uint256 tokenAmount, uint256 ethAmount)
        public
        onlyOwner
    {
        // add the liquidity
        ...
        returns (bool)
    {
        ...
        return msg.value, */
        address[] memory path = new address[](2);
        path[0] = IUniswapV2Router02(address(uniswapV2Router)).WETH();
        path[1] = address(this);

        bool success;

        try
            IUniswapV2Router02(address(uniswapV2Router))
