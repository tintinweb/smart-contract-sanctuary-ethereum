...
contract TokenFactory is NonblockingLzApp {
    ...
    function mintToken(
        ...
        if (_chToId[chSym] == _contractChainId) {
            ...
            omniNft.mint(msg.sender, ""); // TODO: Remove 2 arg
        ...
        bytes memory payload = _getPayload(1, coll, msg.sender, mintPrice, creator, asset);
    ...
    function _nonblockingLzReceive(
        ...
        (uint256 act, address coll, address rec, uint256 paid, address cre, address asset) = abi.decode(_payload, (uint256, address, address, uint256, address, address));
        if (act != 1) {
            if (paid > 0) {
        ...
        nft.mint(rec, ""); // TODO: Remove 2 arg
    ...
    function _getPayload(uint256 act, address coll, address rec, uint256 price, address cre, address asset) private pure returns (bytes memory) {
        return abi.encode(act, coll, rec, price, cre, asset);
    }
