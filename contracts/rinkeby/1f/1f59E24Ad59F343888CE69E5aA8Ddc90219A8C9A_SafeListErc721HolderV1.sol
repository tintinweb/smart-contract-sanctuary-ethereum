    ...
    AvantArteStoreInterfaceV1
{
    ...
    function purchase(uint256 tokenId, string calldata productId)
        public
        payable
        override
        onlyRunning
        ...
        onlySafeListedMinter
    {
        ...
        emit OnPurchase(msg.sender, tokenId, productId);
    ...
    function isAllowedToMint(uint256)
        public
        view
        virtual
        override
        returns (bool)
    {
        return isSafeListedAndCanMint(msg.sender);
    }
...
abstract contract AvantArteStoreInterfaceV1 {
    ...
    event OnPurchase(address walletAddress, uint256 tokenId, string productId);
    ...
    /// @dev tells us if the token is available
    function isAllowedToMint(uint256 tokenId)
        public
        view
        virtual
        returns (bool);
...
abstract contract MintersSafeListController is AdminController {
    ...
    modifier onlySafeListedMinter() {
        require(isSafeListedAndCanMint(msg.sender), "not safe listed");
        _;
    }

    /// @dev checks if the current address is allowed to mint
    function isSafeListedAndCanMint(address addr) public view returns (bool) {
        SafeListItem memory item = safeListMap[addr];
        return item.isSafe && item.minted < maxMintAmount;
    }
