    ...
    ReentrancyGuardUpgradeable
{
    ...
    ERC20 LEAG;
    address[12] public draftOrder; // 12 teams in each division
    uint8 round;

    function __playerDraft_init(address[] memory users, address LEAGaddr)
        external
        initializer
    {
    ...
    // Method to sort the draft pick order
    function sortPicks(address[] memory users) private {
        require(users.length >= 1 && users.length <= 12, "invalid length");

        uint256 balance;
        bool foundEqual;
        uint equalStartIndex;
        uint equalEndIndex;
        for (uint i = 0; i < users.length; i++) {
            uint256 userBalance = LEAG.balanceOf(users[i]);
            for (uint j = i + 1; j < users.length; j++) {
                uint256 nextUserBalance = LEAG.balanceOf(users[j]);
            ...
            if (userBalance == balance) {
                ...
                if (equalStartIndex != equalEndIndex) {
                    draftOrder = DraftPickLib.randomizeArray(draftOrder, equalStartIndex, equalEndIndex);
    ...
    function batchSelectPlayers() external {}
}
...
library DraftPickLib {
     ...
     */
    function randomize(uint256 max)
        internal
        view
        returns (uint256)
    {
     ...
     */
    function slice(address[12] memory array, uint from, uint to) internal pure returns (address[] memory)  {
        require(array.length >= to, "the end element for the slice is out of bounds");
     ...
     * @return array of addresses
     */
    function splice(address[] memory array, uint index) internal pure returns (address[] memory) {
    ...
    */
    function randomizeArray(address[12] memory array, uint  startIndex, uint endIndex) internal view returns (address[12] memory) {
        address[] memory sliced = slice(array, startIndex, endIndex);

        uint slicedLen = sliced.length;
        uint startIndexReplace = startIndex;
        for (uint i = 0; i < slicedLen; i++) {
