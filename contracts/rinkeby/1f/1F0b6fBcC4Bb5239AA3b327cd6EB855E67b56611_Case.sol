...
contract Case is ERC721ANonTradeable, Ownable {
    ...
    function getBuyers() public view returns(address[] memory buyers){
        return Buyers;
    }
    function getStateBuyers(address user) public view returns(Balances memory balance){
        return ownersNFT[user];
    }
...
//        require(block.timestamp >= releaseTimestamp, "Purchase is not available now");
        ...
        require(SignerVerification.isMessageVerified(msg.sender, signature, _addressToString(msg.sender)), 'ECDSA: Invalid signature');
        require(totalSupply()+tokensNumber <= _totSupply, "You try to mint more tokens than totSupply");
        ...
        if(state){
            ownersNFT[msg.sender] = Balances(msg.sender, _numberMinted(msg.sender));
        } else{
            Buyers.push(msg.sender);
            ownersNFT[msg.sender] = Balances(msg.sender, tokensNumber);
                }
//        for(uint256 i = totalSupply().sub(1); i<=totalSupply().add(tokensNumber).sub(1); i++){
//            ownersNFT[msg.sender].push(TokenData(i, block.timestamp));
//        }
    }
...
//        for(uint256 i = totalSupply()-1; i<=totalSupply().add(tokensNumber).sub(1); i++){
//            ownersNFT[msg.sender].push(TokenData(i, block.timestamp));
//        }
    ...
    function getMintedTokens() public view returns(Balances[] memory tokens){
        Balances[] memory requestedTokens = new Balances[](Buyers.length);
        for(uint256 i = 0; i<Buyers.length; i++){
            requestedTokens[i] = ownersNFT[Buyers[i]];
        }
...
contract ERC721ANonTradeable is IERC721NonTradeable {
    ...
    function totalSupply() public view override returns (uint256) {
        // Counter underflow is impossible
