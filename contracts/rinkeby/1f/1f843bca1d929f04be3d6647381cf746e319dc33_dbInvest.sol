...
contract dbInvest is ERC721Enumerable, Ownable {
  using Strings for uint256;
  ...
  uint256 constant public FUSION_PRICE = 7.50 ether;
  uint256 constant public NAME_CHANGE_PRICE = 1.50 ether;
  uint256 constant public BIO_CHANGE_PRICE = 1.50 ether;

  uint256 public discountRate = 0.01 ether;
  uint256 public auctionDuration = 72 hours;
  uint256 private endPrice = 0.5 ether;
  ...
  mapping(uint256 => uint256) public baby;
  mapping(uint256 => dbiData) public dbiBabyData;


  event DBPFusion(uint256 tokenId, uint256 parent1, uint256 parent2);
  event teamTokens(uint256 stid,address minter);
  event mintLog(uint256 scid,address minter);
  
  
  
  constructor(uint256 genCount) ERC721("DBInvest", "DBI") {
   ...
   modifier tokenOwner(uint256 tokenID) {
        require(ownerOf(tokenID) == msg.sender, "Cannot interact with a KaijuKingz you do not own");
        _;
    }
    ...
    function discountedPrice() public view returns (uint256) {
        uint256 price;
        if (endsAt < block.timestamp) {
            return Price;
        }
        uint256 timeElapsed = (block.timestamp - startAt) / 7200; //hour;
        price = Price - (timeElapsed * discountRate); 
  ...
  function mint(uint256 _noOfTokens) public payable {
    ...
    if (msg.sender != owner()) {
      ...
      if(auctionStarted){
        uint256 totalAmount = discountedPrice() * _noOfTokens;
  ...
  function fusion(uint256 parent1, uint256 parent2) external tokenOwner(parent1) tokenOwner(parent2) {
        uint256 supply1 = supply.current();
        require(supply1 < maxSupply,                               "Cannot fuse any more baby");
        require(parent1 < maxGenCount && parent2 < maxGenCount,   "Cannot fuse with baby");
        require(parent1 != parent2,                               "Must select two unique parents");

        DBP.burn(msg.sender, FUSION_PRICE);
        uint256 babyId = maxGenCount + babyCount;
        //babyKaiju[kaijuId] = 1;
        babyCount++;
        _safeMint(msg.sender, babyId);
        emit DBPFusion(babyId, parent1, parent2);
  }

  function changeName(uint256 dbiId, string memory newName) external tokenOwner(dbiId) {
        bytes memory n = bytes(newName);
        require(n.length > 0 && n.length < 25,                          "Invalid name length");
        require(sha256(n) != sha256(bytes(dbiBabyData[dbiId].name)),    "New name is same as current name");
        
        DBP.burn(msg.sender, NAME_CHANGE_PRICE);
        dbiBabyData[dbiId].name = newName;
        //emit NameChanged(kaijuId, newName);
    }

    function changeBio(uint256 dbiId, string memory newBio) external tokenOwner(dbiId) {
        DBP.burn(msg.sender, BIO_CHANGE_PRICE);
        dbiBabyData[dbiId].bio = newBio;
       // emit BioChanged(dbiId, newBio);
    }
  ...
  function auctionStartedTime() public view returns(uint256){
  ...
  function setDiscountRate(uint256 _price) public onlyOwner {
      discountRate = _price;
  }
  ...
  function setMinimumPrice(uint256 _newPrice) public onlyOwner {
    minimumPrice = _newPrice;
  }

  function setAuctionSale(bool _state) public onlyOwner{
    auctionStarted = _state;
    if (_state){
      startAt = block.timestamp;
      endsAt = block.timestamp + auctionDuration;
    }else{
      startAt = 0;
      endsAt = 0;
    }
  ...
  function setAuctionDuration(uint256 _seconds) public onlyOwner{
    auctionDuration = _seconds;
  }
...
function setDBPWaste(address IRWardAddress) external onlyOwner {
      DBP = IDBP(IRWardAddress);
  }
  
  function transferFrom(address from, address to, uint256 tokenId) public override {
      if (tokenId < maxSupply) {
          DBP.updateReward(from, to);
          balanceGenesis[from]--;
          balanceGenesis[to]++;
      }
