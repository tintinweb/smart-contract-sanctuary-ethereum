...
import "./../interfaces/IOldFeesCollectorManagement.sol";
import "./../interfaces/IWETH.sol";

contract OldFeesCollector is IFeesCollector, KeeperCompatibleInterface, IOldFeesCollectorManagement, Ownable {
    ...
    uint256 public minETHForTransfer = 0.03 ether;
    uint256 public minUSDCForConversion = 1500e6;
    uint256 public maxSubmissionFee = 0.01 ether;

    bool public enableWhitelist = true;
    bool public sendToArbitrum = false;
    bool public convertUSDC = true;
    bool public buyBack = true;

    address public fundsSender;
    mapping (address => bool) public allowedSenders;

    constructor(IERC20 _usdcToken, IERC20 _goviToken, IFeesCollector _staking, address _stakingVaultAddress, address _arbitrumContractAddress, IUniswapV2Router02 _router, AggregatorV3Interface _usdcETHPriceAggregator, IInbox _arbitrumInbox) {
        ...
        IERC20(router.WETH()).safeApprove(address(staking), type(uint256).max);
    }
    ...
    function sendProfit(uint256 _amount, IERC20 _token) external override {
        ...
        require(_token == usdcToken, "Non-USDC profit");
    ...
    function setStaking(IFeesCollector _newStaking) external override onlyOwner {
        ...
        IERC20(router.WETH()).safeApprove(address(staking), 0);
        staking = _newStaking;
        goviToken.safeApprove(address(staking), type(uint256).max);
        IERC20(router.WETH()).safeApprove(address(staking), type(uint256).max);
    }
    ...
    function _convertUSDCToETH() private {
        ...
        if (shouldConvert) {
            ...
            if (usdcAmount >= minUSDCForConversion) {
                ...
                uint256 ethMinOut = usdcAmount * uint256(usdcETHPrice) * (MAX_PERCENTAGE - maxSlippage) / MAX_PERCENTAGE / (10 ** ERC20(address(usdcToken)).decimals());
            ...
            if (sendToArbitrum) {
                arbitrumInbox.createRetryableTicket{value: transferAmount}(arbitrumContractAddress, transferAmount - maxSubmissionFee, maxSubmissionFee, arbitrumContractAddress, arbitrumContractAddress, 0, 0, '0x');
            }
        }
    }
    ...
    function _sendToStaking(uint256 _ethGOVIPrice) private {
        ...
        if (buyBack) {
            ...
            uint256 goviMinOut = ethAmount * _ethGOVIPrice * (MAX_PERCENTAGE - maxSlippage) / MAX_PERCENTAGE / (10 ** ERC20(address(goviToken)).decimals());

            uint[] memory outAmounts = router.swapExactETHForTokens{value: ethAmount}(goviMinOut, path, address(this), block.timestamp);

            goviToken.safeTransfer(stakingVaultAddress, outAmounts[1]);
        } else {
            IWETH(router.WETH()).deposit{value: ethAmount}();
            staking.sendProfit(ethAmount, IERC20(router.WETH()));
        }
    }
...
interface IOldFeesCollectorManagement {
