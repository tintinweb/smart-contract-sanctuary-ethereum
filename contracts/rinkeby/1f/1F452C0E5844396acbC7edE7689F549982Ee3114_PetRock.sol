...
contract PetRock is ERC721, Ownable {
    ...
    address payable public founderWallet =
        payable(0x77054aC1A0618b2411521b13A1c9c0c34103dFcb);
    address payable public communityWallet =
        payable(0xE8bD270B9CfF3506ab307103e2a344aA6021a097);
    address payable public artistWallet =
        payable(0x8e22326ebf5713F7b242237aa68DD8d0049f0E59);
    ...
    mapping(address => uint256) whitelistClaims;

    //Modifiers
    modifier mintConfig() {
        _;
        //Makes sure value sent is
        require((msg.value % 0.02 ether) == 0);
        require(msg.value < 0.07 ether);
        require(publicSale == true);
    }
    ...
    function freeRock(uint256 _amount) public {
        // Limits number of NFT's allowed to mint
        require(_amount <= 2, "Maximum free rock's is 2. Try again.");
        require(
            whitelistClaims[msg.sender] < 3,
            "You have already claimed your whitelist mint."
        );

        // Records NFT's minted for every wallet
        uint256 claims = whitelistClaims[msg.sender];
        whitelistClaims[msg.sender] = claims + _amount;

        // Checks that caller is whitelisted
        require(
            whitelist[msg.sender] == true,
            "Connected wallet is not whitelisted."
        );
        // Checks that the number of free rocks is met
        require(
            (_tokenIdTracker.current().add(_amount)) < 800,
    ...
    // Allows Owner to add Whitelisted addresses
    ...
    function withdraw() public onlyOwner {
        ...
        require(address(this).balance > 0, "Balance is 0 ETH");
