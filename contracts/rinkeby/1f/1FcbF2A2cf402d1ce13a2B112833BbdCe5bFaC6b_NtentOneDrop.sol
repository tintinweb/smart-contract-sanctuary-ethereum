...
contract NtentOneDrop is Ownable, ReentrancyGuard {
    ...
    function withdraw() public onlyOwner {
        ...
        if(apeBalance > 0){
            uint256 apeFundAmount = apeBalance * (sustainabilityFundPercentage / 100);
            uint256 apeAdminAmount = apeBalance * (adminExpensesPercentage / 100);
        ...
        if(ashBalance > 0){
            uint256 ashFundAmount = ashBalance * (sustainabilityFundPercentage / 100);
            uint256 ashAdminAmount = ashBalance * (adminExpensesPercentage / 100);
        ...
        if(balance > 0){
            uint256 fundAmount = balance * (sustainabilityFundPercentage / 100);
            uint256 adminAmount = balance * (adminExpensesPercentage / 100);
