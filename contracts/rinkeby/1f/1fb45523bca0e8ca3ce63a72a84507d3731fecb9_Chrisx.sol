...
contract Chrisx is Ownable, ERC721A, ReentrancyGuard {
    ...
    mapping(address => uint8) public _whiteListed;
...
//--------------------- Set & Change anythings
    ...
    function setWhiteList (address[] calldata addresses , uint8 numAllowedToMint) external onlyOwner {
        for (uint256 i = 0; i < addresses.length; i++) {
            _whiteListed[addresses[i]] = numAllowedToMint ;
        }
    }
...
//-------------------- WhitelistMint
    ...
    function mintWhiteList(uint8 _mintAmount) external payable {
        ...
        require(numberMinted(msg.sender) + _mintAmount <= maxPerAddress, "can not mint this many");

        require(_mintAmount <= _whiteListed[msg.sender], "Exceeded max available to purchase");
