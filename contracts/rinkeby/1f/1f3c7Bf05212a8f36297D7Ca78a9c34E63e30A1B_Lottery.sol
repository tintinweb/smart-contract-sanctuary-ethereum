...
contract Lottery is Ownable, VRFConsumerBaseV2 {
    ...
    address payable[] public players_list;
    ...
    function enter() public payable {
        ...
        require(msg.value >= get_convertions_rate());
        players_list.push(payable(msg.sender));
    }

    function end() public onlyOwner returns (uint256) {
        require(state == STATE.OPEN);
    ...
    function fulfillRandomWords(
        ...
        require(randomWords[0] > 0);
        uint256 winner_idx = randomWords[0] % players_list.length;
        winner = players_list[winner_idx];
        winner.transfer(address(this).balance);
        players_list = new address payable[](0);
        state = STATE.CLOSE;
        // test
        random_number = randomWords[0];
    }

    function get_convertions_rate() public view returns (uint256) {
        uint256 eth_price = get_price();
        uint256 paid_usd_price = (entry_fee * 10**18) / eth_price;
        return paid_usd_price;
    }
