...
contract BNft is ERC1155, Ownable, ERC1155Burnable {
    using SafeMath for uint256;

    uint256 tokenId = 0;
    uint256 amountMinted = 0;
    uint256 limitAmount = 729;
    uint256 public immutable maxTeamAmount = 47;
    uint256 public teamMintedAmount;
    address bNftContractAddress;
    address saleAddress;
    bool getAvatarStarted = false;

    constructor()
        ERC1155("ipfs://QmdKE7gYeGPezV1YUAPGmXHsmeS79hUKcT6i7YovWffvgk")
    {}
    ...
    function setBNftContract(address contractAddress) public onlyOwner {
        bNftContractAddress = contractAddress;
    }
    ...
    function getAvatarToken(uint256 id) public returns (uint256) {
        ...
        BnftContract bNftContract = BnftContract(bNftContractAddress);
        uint256 mintedId = bNftContract.mintTransfer(msg.sender);
        return mintedId; // Return the minted ID
    }

    function forceGetAvatarToken(uint256 id) public onlyOwner {
        require(balanceOf(msg.sender, id) > 0, "Doesn't own the token");
        burn(msg.sender, id, 1);
        BnftContract bNftContract = BnftContract(bNftContractAddress);
        bNftContract.mintTransfer(msg.sender);
    }
