...
contract BottoRetroactiveReward is
    ...
    ERC1155SupplyUpgradeable
{
    ...
    // The collection name
    string public constant name = "Ceci n\x27est pas un Botto";

    // The merkel root
    bytes32 public root;

    // The address that mints redeemed tokens
    address public redeemMinter;

    function initialize(
        string memory uri_,
        bytes32 merkleroot_,
        address redeemMinter_
    ) public initializer {
        ERC1155PresetMinterPauserUpgradeable.initialize(uri_);
        ERC1155SupplyUpgradeable.__ERC1155Supply_init_unchained();

        root = merkleroot_;
        redeemMinter = redeemMinter_;
    }

    function setURI(string memory newuri) external {
        require(
            hasRole(DEFAULT_ADMIN_ROLE, _msgSender()),
            "BottoRetroactiveReward: need admin role"
        );
    ...
    function redeem(
        ...
        require(
            _verify(_leaf(recipient_, tokenId_), proof_),
            "BottoRetroactiveReward: invalid merkle proof"
        );

        // Amount can be added as part of merkel-proof if needed.
        // First assign the token to the signer, to establish provenance on-chain
        _mint(redeemMinter, tokenId_, 1, "");
        _safeTransferFrom(redeemMinter, recipient_, tokenId_, 1, "");
    }
