    ...
    ResourceImplementationPointer
{
    ...
    function cashoutRewardFromGift(uint256 _nftId, bool swapping) external nonReentrant whenNotPaused resourceSet checkPermissions(_nftId) {
        address account = _msgSender();
        uint256 reward = _getNFTCashoutRewardsFromGift(_nftId, account);
        _cashoutReward(reward, swapping);
    }
    ...
    function _getNFTCashoutRewardsFromGift(uint256 _nftId, address account) private returns (uint256) {
        NFTEntity storage nft = _nfts[_nftId];
        UserInfo storage user = userInfo[_nftId][account];
    ...
    function calculatePendingRewards(uint256 _amount, uint256 _lastProcessingTimestampuint256,  uint256 _rewardPerDay) public view returns (uint256) {
        return _calculateRewardsFromValue(_amount, block.timestamp - _lastProcessingTimestampuint256, _rewardPerDay);
    }

    function calculateRewardsPerDay(uint256 _amount, uint256 _rewardPerDay) public pure returns (uint256) {
        return _calculateRewardsFromValue(_amount, ONE_DAY, _rewardPerDay);
    }
