...
contract RealioLPStaking is Ownable {
    ...
    struct PoolInfo {
        ...
        uint256 accRioPerShare;            // Accumulated RIOs per share, times 1e18. See below.
    }
    ...
    function pendingRioReward(uint256 _poolId, address _user) external view returns (uint256) {
        ...
        accRioPerShare = accRioPerShare.add(rioReward.mul(1e18).div(lpSupply));
        return user.amount.mul(accRioPerShare).div(1e18).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _poolId) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 rioReward = multiplier.mul(rioRewardPerBlock).mul(pool.perBlockRioAllocated).div(totalAllocRioPerBlock);
        pool.accRioPerShare = pool.accRioPerShare.add(rioReward.mul(1e18).div(lpSupply));
    ...
    function deposit(uint256 _poolId, uint256 _amount) public {
        ...
        if (user.amount > 0) {
            uint256 pending = user.amount.mul(pool.accRioPerShare).div(1e18).sub(user.rewardDebt);
            safeRioTransfer(msg.sender, pending);
        }
        pool.lpToken.safeTransferFrom(address(msg.sender), address(this), _amount);
        user.amount = user.amount.add(_amount);
        user.rewardDebt = user.amount.mul(pool.accRioPerShare).div(1e18);
