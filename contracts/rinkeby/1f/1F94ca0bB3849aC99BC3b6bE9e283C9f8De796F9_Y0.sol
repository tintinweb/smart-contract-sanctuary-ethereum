...
contract Y0 is ERC1155, Ownable {
  ...
  */
  ...
  function reveal(address claimer, uint256 _tokenId) public {
    require(block.timestamp - mint_timestamp[claimer][_tokenId] >= 1 minutes, 'NFT is revealed soon');
    isRevealed = true;
    _setURI(bytes(baseURI).length > 0 ? string(abi.encodePacked(baseURI, Strings.toString(_tokenId), ".json")) : baseURI);
  }
  
  function uri(uint256 _tokenId) public view override returns (string memory) {
    // if (isRevealed) return super.uri(_tokenId);
    return bytes(baseURI).length > 0 ? string(abi.encodePacked(baseURI, Strings.toString(1), ".json")) : baseURI;
  }
}
