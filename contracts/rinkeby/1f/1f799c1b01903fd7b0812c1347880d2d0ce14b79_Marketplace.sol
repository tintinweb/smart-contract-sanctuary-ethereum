...
contract Marketplace {
  ...
  function getGame() private view returns(Game[] memory) {
  ...
  function getGameList(address _adr) external view returns(Game[] memory){
    if(_adr == Seller) return getGame();
    else {
       bool[] memory _boughtCts = new bool[](gameCount);
      Game[] memory _games = new Game[](gameCount);
      for(uint256 i=0; i < contractCnt; i++) {
        if(boughtContracts[i].owner == _adr){
