...
*/
   ...
   */
    ...
    function DirectDepositTokens(address _investor, uint256 amount, bytes32 symbol, address tokenAddress, uint _subsId, uint _projectId) external {
        require(msg.sender == _investor,"The connected wallet is not matching");
        require(initialInvestorId[_projectId][_investor] == msg.sender,"investor address is mismatch with subsequent id");
        whitelistedTokens[symbol] = tokenAddress;
...
*/
    ...
    function _getInvestorInvestedBalance(uint _projectId, address _founder) public view returns(uint){
        return getInvestorInvestedBalance[_projectId][_founder];
    }
    ...
    function _getInvestorCurrentBalance(uint _projectId, address _founder) public view returns(uint){
        return getInvestorCurrentBalance[_projectId][_founder];
    }
    ...
    function _getSubsequentProposalStatus(uint _projectId, uint _subsId) public view returns(bool){
        return getSubsequentProposalStatus[_projectId][_subsId];
    }
    ...
    function _getTotalProjectValue(uint _projectId) public view returns(uint){
