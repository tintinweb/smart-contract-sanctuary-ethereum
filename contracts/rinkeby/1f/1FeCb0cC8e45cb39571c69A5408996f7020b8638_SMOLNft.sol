  ...
  function _payToMint(uint256 _amount) internal whenOwnerOrPublicSaleActive {
    ...
    if (totalNativeCost > 0) {
      ...
      if (msg.value > totalNativeCost) {
        Address.sendValue(payable(msg.sender), msg.value - totalNativeCost);
      }
      require(
    ...
    if (totalSmolCost > 0) {
      require(
        smolToken.balanceOf(msg.sender) >= totalSmolCost,
        'not enough SMOL balance to mint'
      );
      smolToken.transferFrom(msg.sender, paymentAddress, totalSmolCost);
    }
  }
