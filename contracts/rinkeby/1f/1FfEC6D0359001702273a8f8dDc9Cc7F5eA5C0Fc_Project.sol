...
contract Project is Initializable, OwnableUpgradeable {
    ...
    function _setAdmins(address[] memory accounts, bool isAdd) private {
        for (uint256 i; i < accounts.length; i++) {
            ...
            require(admins[accounts[i]] != isAdd, "admin value already exists");
    ...
    function setManager(uint256 _projectId, address _newManager) external projectExists(_projectId) onlyAdmin {
        require(_newManager != address(0), "manager is the zero address");
        require(_newManager != projects[_projectId].manager, "manager already exists");
    ...
    function setApproval(uint256 _projectId, bool _isApproval) external projectExists(_projectId) onlyAdmin {
        require(!projects[_projectId].isCreatedByAdmin, "can't set approve project created by admin");
