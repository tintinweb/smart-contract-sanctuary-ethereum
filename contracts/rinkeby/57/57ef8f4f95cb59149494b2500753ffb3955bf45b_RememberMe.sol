    ...
    ReentrancyGuard
{
    ...
    uint256 private constant SECONDS_PER_INTERVAL =
        MINUTES_PER_INTERVAL * SECONDS_PER_MINUTE;
    // Since block.timestamp is denominated in seconds
    uint256 private constant TOTAL_GROWTH_SECONDS =
        GROWTH_INTERVALS * SECONDS_PER_INTERVAL;
    ...
    string private _baseURIextended =
        "https://afterlife.mypinata.cloud/ipfs/QmUAauCxM9jFCXWDPUJrtR4PSL3fd62kCnfXJPtwse7m4t/";

    constructor(
        address _luna,
        address _teknique,
        address _openSeaProxyRegistryAddress
    ) ERC721("Remember Me", "MEMORY") {
    ...
    // * * * * * * * * * * * * * * * * * * * * * * MINTING * * * * * * * * * * * * * * * * * * * * * *
    function mint() external nonReentrant seedMintActive hasNoSeeds {
    ...
    modifier onlyAfterlife() {
        require(
            _msgSender() == luna || _msgSender() == teknique,
            "msg.sender is not in Afterlife"
        );
        _;
    }
    ...
    modifier hasNoSeeds() {
        require(
            balanceOf(_msgSender()) < SEED_MINT_LIMIT,
            "Only one seed per collector"
        );
        _;
    }
    ...
    modifier canSeedDrop(uint256 num) {
        require(
            numSeedDrops + num <= MAX_SEED_DROPS,
            "Not enough seeds left for Seed Drop"
        );
        _;
    }
    ...
    // * * * * * * * * * * * * * * * * * * * * * SEED DROPS * * * * * * * * * * * * * * * * * * * * *
    function reserveForSeedDrops(uint256 numToReserve)
        external
        nonReentrant
        onlyAfterlife
        canSeedDrop(numToReserve)
    {
    ...
    function seedDrop(address[] calldata addresses)
        external
        nonReentrant
        onlyAfterlife
        canSeedDrop(addresses.length)
    {
