...
contract Presale is ContextUpgradeable, ReentrancyGuardUpgradeable {
    ...
    event SaleCreated(uint256 id, address token, address currency, uint128[3] startTimes, uint128[3] endTimes, uint128 price, uint128 totalSupply, uint128 tgeTime, uint128 tgePercent, uint128[3] vesting);
    event SaleUpdated(uint256 id, address token, address currency, uint128[3] startTimes, uint128[3] endTimes, uint128 price, uint128 totalSupply, uint128 tgeTime, uint128 tgePercent, uint128[3] vesting);
    event SaleEnabled(uint256 id);
    event SaleDisabled(uint256 id);

    event BoughtInPrimarySale(uint256 saleId, address account, uint256 amount);
    event BoughtInFCFS(uint256 saleId, address account, uint256 amount);

    uint256 public constant ONE_HUNDRED_PERCENT = 10000; // 100%

    IRole role;

    ITokenVesting vestingContract;
    ...
    function _checkSaleParams(address token, address currency, uint128[3] memory startTimes, uint128[3] memory endTimes, uint128 price, uint128 totalSupply, uint128 tgeTime, uint128 tgePercent)
        internal
        pure
    {
        ...
        require(startTimes[0] > 0 && startTimes[0] < endTimes[0], "Presale: registration time is invalid");
    ...
    function createSale(uint256 id, address token, address currency, uint128[3] memory startTimes, uint128[3] memory endTimes, uint128 price, uint128 totalSupply, uint128 tgeTime, uint128 tgePercent, uint128[3] memory vesting)
        external
        onlyOperator
        saleNotExist(id)
    {
        _checkSaleParams(token, currency, startTimes, endTimes, price, totalSupply, tgeTime, tgePercent);
        ...
        emit SaleCreated(id, token, currency, startTimes, endTimes, price, totalSupply, tgeTime, tgePercent, vesting);
    }

    function updateSale(uint256 id, address token, address currency, uint128[3] memory startTimes, uint128[3] memory endTimes, uint128 price, uint128 totalSupply, uint128 tgeTime, uint128 tgePercent, uint128[3] memory vesting)
        public
        onlyOperator
        saleExist(id)
    {
        ...
        emit SaleUpdated(id, token, currency, startTimes, endTimes, price, totalSupply, tgeTime, tgePercent, vesting);
    }
    ...
    function _buy(uint256 saleId, uint256 amount, uint256 allocation, bytes memory signature, bool isPrimarySale)
        internal
    {
        require(amount > 0, "Presale: amount is invalid");
        ...
        bytes32 message = keccak256(abi.encodePacked(saleId, msgSender, amount, allocation, nonces[msgSender], block.chainid, this)).prefixed();
        ...
        // Avoids stack too deep error
        {
            ...
            if (isPrimarySale) {
                ...
                emit BoughtInPrimarySale(saleId, msgSender, amount);
                ...
                emit BoughtInFCFS(saleId, msgSender, amount);
            }
        }
    ...
    function buyTokenInPrimarySale(uint256 saleId, uint256 amount, uint256 allocation, bytes memory signature)
        public
        nonReentrant
        saleNotPause(saleId)
    {
        _buy(saleId, amount, allocation, signature, true);
    }

    function buyTokenInFCFS(uint256 saleId, uint256 amount, uint256 allocation, bytes memory signature)
        public
        nonReentrant
        saleNotPause(saleId)
    {
        _buy(saleId, amount, allocation, signature, false);
    }
