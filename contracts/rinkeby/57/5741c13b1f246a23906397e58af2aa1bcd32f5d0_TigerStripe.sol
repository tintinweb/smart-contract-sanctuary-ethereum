...
abstract contract Ownable is Context {
    ...
    address private __owner = _msgSender();
 ...
 */
contract TigerStripe is Ownable, ERC721Pausable {
    using SafeMath for uint256;
    ...
    constructor(string memory baseURI) ERC721("Tiger Tournament", "TIGR") {  
        _setBaseURI(baseURI);
    }
    ...
    * Mints a PACK
    */
    function mintPack(string memory tokenURI, uint256 mintedImg) public payable saleIsOpen () {
        require(balanceOf(msg.sender) < MAX_BY_MINT, "Exceeds number");
        require(msg.value >= PRICE, "Low balance");
        require(totalSupply() <= MAX_ELEMENTS, "All are minted");
        // add PACK id to map
        
        require(!ImgStatus[mintedImg], "This number is existed");
        ImgStatus[mintedImg] = true;
