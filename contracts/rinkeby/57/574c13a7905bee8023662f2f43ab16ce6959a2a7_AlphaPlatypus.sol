...
contract AlphaPlatypus is Ownable, ERC721A, ReentrancyGuard {
    using Strings for uint256;
  ...
  uint256 public  PRICE = 0.06 ether; //0.04 ether during presale, 0.06 ether during public 
  ...
  uint256 public Presale_Supply=1700;   
  ...
  constructor() ERC721A("AlphaPlatypus","AlphaPlatypus", MaxMintPerBatch_, TotalCollectionSize_) {
    _uriBeforeRevel = "https://gateway.pinata.cloud/ipfs/Qmc4KPPFLbhyQrjDBZbux5CCdckW8c2LptZxfC6qqjEkuH/";
  ...
  function mintPresale(uint256 quantity) external payable callerIsUser{
    require(status == 1 && whitelistedAddresses[msg.sender], "Presale not active");
    require(quantity > 0,"Mint Quantity should be more than 0");
    require(totalSupply() + quantity <= Presale_Supply, "Reached max supply");
    require(numberMinted(msg.sender) + quantity <= 2  , "Max can hold 3 NFTs" );
    require(quantity <= 2,"Max can mint/hold 2 NFTs from PreSale");
  ...
  function mint(uint256 quantity) external payable callerIsUser {
    ...
    require(numberMinted(msg.sender) + quantity <= 5  , "Max can hold 3 NFTs" );
    require(quantity <= 5,"Max can mint 2 per transaction");
    require(msg.value >= PRICE * quantity, "Not enough ETH in your wallet");
    _safeMint(msg.sender, quantity);    
  }
