...
contract FayreMarketplace is OwnableUpgradeable {
    ...
    */
    ...
    event SetFreeMinter(address indexed caller, FreeMinterData freeMinterData);
    ...
    function setFreeMinters(FreeMinterData[] calldata freeMintersData) external onlyMarketplaceManager {
        for (uint256 i = 0; i < freeMintersData.length; i++) {
            ...
            emit SetFreeMinter(msg.sender, freeMintersData[i]);
        } 
    }
    ...
    function mint(AssetType assetType, string memory tokenURI, uint256 amount, uint256 royaltiesPct, string memory collectionName) external payable returns(uint256) {
        ...
        if (remainingFreeMints[msg.sender] > 0) {
            ...
            uint256 remaningMintFeeUSD = _processMembershipCards(msg.sender, mintFeeUSD, 0);
    ...
    function _sendAmountToSeller(AssetType assetType, uint256 networkId, address collectionAddress, uint256 tokenId, uint256 amount, address tokenAddress, address seller, address buyer) private {
        ...
        if (assetType == AssetType.LIQUIDITY) {
            ...
            ownerRemainingSaleFee = _convertUSDToLiquidity(_processMembershipCards(seller, _convertLiquidityToUSD(saleFee), _convertLiquidityToUSD(amount)));
        }
        else {
            from = buyer;

            ownerRemainingSaleFee = _processMembershipCards(seller, saleFee * 10 ** (18 - IERC20UpgradeableExtended(tokenAddress).decimals()), amount * 10 ** (18 - IERC20UpgradeableExtended(tokenAddress).decimals())) / 10 ** (18 - IERC20UpgradeableExtended(tokenAddress).decimals());
        }
    ...
    function _processMembershipCards(address owner, uint256 fee, uint256 nftPrice) private returns(uint256) { 
        for (uint256 i = 0; i < membershipCardsAddresses.length; i++) {
