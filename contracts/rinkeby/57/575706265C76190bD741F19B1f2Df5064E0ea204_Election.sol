...
contract Election {
    ...
    mapping(uint256 => uint256) public electionRegisteredUsersCount;
    ...
    function register(uint256 _electionId) public {
        ...
        require(
            ...
            "Cannot register. The registration period didn't start yet"
        );
        require(
            election.registrationPeriodEnd >= block.timestamp,
            "Cannot register. The registration period already ended"
        );
        ...
        electionRegisteredUsersCount[_electionId]++;
    ...
    function getElectionRegisteredUsers(uint256 _electionId)
        public
        view
        returns (address[] memory)
    {
        ...
        for (
            uint256 i = 0;
            i < electionRegisteredUsersCount[_electionId];
            i++
        ) {
            registeredUsers[i] = electionToRegisteredUsers[_electionId][i];
        }
