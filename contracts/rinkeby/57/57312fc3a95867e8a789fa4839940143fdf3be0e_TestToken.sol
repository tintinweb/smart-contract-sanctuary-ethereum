...
contract TestToken is AntiBotToken {
    ...
    mapping (address => LpTrading) public lpTradingHistory;
    bool isCheckingFlashTrading;

    bool isCheckingContractTrading;
    ...
    event MintedToken();
    event BurnedToken();
    event CheckingTradingAmount(bool _status);
    event CheckingFlashTrading(bool _status);
    event CheckingContractTrading(bool _status);
    event UpdatedLpList();
    event UpdatedTransferMode(uint8 _mode);
    event UpdatedTradingMode(uint8 _mode);
    ...
    function isCheckingTradingAmount() public view returns (bool)  {
        return isCheckingLpTranferAmount;
    }

    function checkTradingAmount(bool _status) public onlyOwner {
        isCheckingLpTranferAmount = _status;
        emit CheckingTradingAmount(_status);
    }

    
    function checkFlashTrading(bool _status) public onlyOwner {
        isCheckingFlashTrading = _status;
        emit CheckingFlashTrading(_status);
    }

    function checkContractTrading(bool _status) public onlyOwner {
        isCheckingContractTrading = _status;
        emit CheckingContractTrading(_status);
    }    

    function updateLpList(LPoolInfo[] memory _lpList) public onlyOwner {
        delete lpList;
    ...
    function setTransferMode(uint8 _mode) public onlyOwner {
        transferMode = _mode;
        emit UpdatedTransferMode(_mode);
    }

    function setTradingMode(uint8 _mode) public onlyOwner {
        tradingMode = _mode;
        emit UpdatedTradingMode(_mode);
    }    
    ...
    function isTransferable(address _from, address _to, uint256 _amount) public view virtual override returns (bool) {
        ...
        if (transferMode == 2 || transferMode == 4) {
            ...
            require(isWhiteListed[_from], "[email protected]: _from is not in isWhiteListed");
        } 

        if (transferMode == 3 || transferMode == 4) {
            require(!isBlackListed[_to], "[email protected]: _to is in isBlackListed");
        ...
        //Check trading////////////////////////////////////////
        bool _isLpTrading = false;
        address _tradingAddress = address(0x0);
        // check buying trading
        for (uint i=0; i<lpList.length; i++) {
            LPoolInfo memory lPoolInfo = lpList[i];
            if (lPoolInfo.poolAddr == _from) {
                require(tradingMode == 1 || tradingMode == 2, "[email protected]: buy trading isn't allowed");
                _isLpTrading = true;
                _tradingAddress = _to;
            }
        }
        ...
        for (uint i=0; i<lpList.length; i++) {
            ...
            if (lPoolInfo.poolAddr == _to) {
                require(tradingMode == 1 || tradingMode == 3, "[email protected]: sell trading isn't allowed");
                _isLpTrading = true;
                _tradingAddress = _from;                
            }
        }

        // if transfer isn't trading, allow
        if (!_isLpTrading) {
            return true;
        }
        ...
        //Check contract trading////////////////////////////////////////
        if (isCheckingContractTrading) {
            if (isContract(_tradingAddress)) {
                require(isWhiteListed[_tradingAddress], "[email protected]: contract trading isn't allowed");
            }
        }

        // Check flash trading////////////////////////////////////////
        LpTrading memory _lpTrading = lpTradingHistory[_tradingAddress];
        if (isCheckingFlashTrading) {
            require(block.number != _lpTrading.blockNumber, "[email protected]: flash trading isn't allowed");
        }
    ...
    function _afterTokenTransfer(address _from, address _to, uint256 _amount) internal virtual override {
        ...
        // if transfer isn't trading, allow
        if (_isLpTrading) {
            LpTrading storage _lpTrading = lpTradingHistory[_tradingAddress];
            _lpTrading.isBuyOrSell = _isLpBuyOrSell;
            _lpTrading.amount = _amount;
            _lpTrading.blockNumber = block.number;
            _lpTrading.timestamp = block.number;        
        }
    }
