...
contract DateUnlockSchedule is BaseGovernanceWithUserUpgradable {
    ...
    function __DateUnlockSchedule_init_unchained(ILock _lock, bytes calldata _data) internal onlyInitializing {
        ...
        require(_unlockTimestamps.length >= 2, "ERROR: Need more key points");
    ...
    function unlockedAmount(uint256 initialAmount) external view returns(uint256) {
        if(block.timestamp < unlockTimestamps[0]) {
            ...
            return initialAmount * normalizedUnlockedSumm / 1e18;
        }
    }
    ...
    function _checkUnlockTimes(uint256[] memory _unlockTimes) internal view returns(bool) {
        for(uint i; i < _unlockTimes.length - 1; i++) {
