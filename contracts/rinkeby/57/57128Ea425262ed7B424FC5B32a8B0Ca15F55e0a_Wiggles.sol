...
contract Wiggles is Ownable, ERC721A, ReentrancyGuard {
    ...
    uint256 public maxMintsOg;
    uint256 public ogWlMints;
    uint256 public maxMintsWhitelist;
    ...
    constructor() ERC721A('WiggleWorld', 'WIGGLE') {
        ...
        // Set price
        unitPrice = 0.02 ether;
        // Max 2 mints / wallet during OG sale
        maxMintsOg = 2;
        // Max 3 mints / wallet during WL sale
        maxMintsWhitelist = 3;
    ...
    // Mints a maximum of 3 NFTs to the caller
        ...
        if(
          ...
          (tokenId >= genesisCollectionSize && revealed == false)
            ...
            return bytes(notRevealedURI).length > 0
                ? string(abi.encodePacked(notRevealedURI, Strings.toString(tokenId % numBackgrounds), baseExtension))
                : '';
        }
        ...
        if (isOG) {
            for (uint256 i = 0; i < addrs.length; i++) {
                if(isGenesis){
                  ...
                  genesisWhitelist[addrs[i]] = maxMintsWhitelist - maxMintsOg;
                } else {
                  freeWhitelist[addrs[i]] = maxMintsOg;
                  paidWhitelist[addrs[i]] = maxMintsWhitelist - maxMintsOg;
                }
            }
        } else {
            for (uint256 i = 0; i < addrs.length; i++) {
