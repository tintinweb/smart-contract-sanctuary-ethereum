...
//        require(!_presaleMinted[msg.sender], "Already minted");
    ...
    /// Add wallet to OG role
    function addToOG(address[] calldata wallets)
        external
        onlyRole(ADMIN_ROLE)
    {
        require(msg.sender == _deploymentConfig.owner, "Only owner can add to OG");

        for (uint i = 0; i < wallets.length; i++) {
            require(
                !hasRole(OG_ROLE, wallets[i]),
                "Wallet already in OG"
            );
            grantRole(OG_ROLE, wallets[i]);
        }
    }
