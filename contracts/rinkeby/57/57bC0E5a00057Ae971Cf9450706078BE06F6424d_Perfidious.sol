...
contract Perfidious is ERC721, Ownable {
    ...
    function presaleMint(bytes32[] memory proof) payable external {
        ...
        require(MerkleProof.verify(proof, presaleListMerkleRoot, keccak256(abi.encodePacked(msg.sender))), "You are not on the presale");
        require(!mintTracker[msg.sender], "Exceeds wallet limit");
        _mintTokens(msg.sender, presaleListQuantity);
        mintTracker[msg.sender] = true;
    }

    function chaosMint(bytes32[] memory proof) payable external {
        require(saleState == State.PRESALE, "Sale is not active");
        require(MerkleProof.verify(proof, chaosListMerkleRoot, keccak256(abi.encodePacked(msg.sender))), "You are not on the presale");
        require(!mintTracker[msg.sender], "Exceeds wallet limit");
    ...
    function setChaosMerkleRoot(bytes32 chaosRoot) public onlyOwner {
