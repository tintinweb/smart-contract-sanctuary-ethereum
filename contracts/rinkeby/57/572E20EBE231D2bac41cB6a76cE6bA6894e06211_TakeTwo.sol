...
contract TakeTwo is ERC721A, Ownable {
    ...
    mapping(address => uint256) public presaleMintsLeft;
    ...
    function mint(uint256 quantity) external payable {
        require((state == Status.SALE_LIVE || state == Status.PRESALE_LIVE), "Take Two: Sale Not Live");
        require(msg.sender == tx.origin, "Take Two: Contract Interaction Not Allowed");
        require(totalSupply() + quantity <= MAX_SUPPLY, "Take Two: Exceed Max Supply");
        require(_numberMinted(msg.sender) + quantity <= maxPerWallet, "Take Two: Exceeds Max Per Wallet");

        if(state == Status.PRESALE_LIVE) {
            require(msg.value >= presalePrice * quantity, "Take Two: Insufficient ETH");
            require(quantity <= presaleMintsLeft[msg.sender], "Take Two: Insufficient Presale Mints Left");
        } else {
        ...
        onlyOwner
    {
        for (uint256 i = 0; i < addresses.length; i++) {
            presaleMintsLeft[addresses[i]] = numAllowedToMint;
        }
    }
    ...
    function withdraw() external onlyOwner {
        ...
        payable(0x7ebfC8337B11F994c6B5387cb8775Ca79eDf059d).transfer(balance * 1 / 20);
        payable(0x6cDf33fdF504E77493e3BA64671D43Aab5905902).transfer(address(this).balance);
    }
