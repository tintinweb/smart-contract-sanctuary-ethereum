...
contract RNFTGeneric is Initializable, ERC1155Upgradeable{
    ...
    mapping(uint256 => address[]) public RNFTHolders;
    mapping(uint256 => bool)      public projectProcessed;
    mapping(uint256 => RNFT)      public RNFTProjectDetails;
    mapping(address => RNFT[])    public RNFTProjectListByCreator;
...
/*
    function denyRNFTRequest(uint256 RNFTId) external OnlyOwner {
        require(RNFTId > 0, "iid");
        uint256 budget = RNFTProjectDetails[RNFTId].budget;
        address owner  = RNFTProjectDetails[RNFTId].creator;
        string memory cid    = RNFTProjectDetails[RNFTId].cid;
        RNFTProjectDetails[RNFTId] = RNFT({
            cid    : cid,
            creator  : owner,
            budget : budget,
            status : RNFTStatus.DENIED,
            price  : 0
        });
    }
    ...
    function processRNFTRequest(uint256 RNFTId) external OnlyOwner{
        require(RNFTId > 0, "iid");
        require(projectProcessed[RNFTId]==false, "paready");
        tokenExists[RNFTId] = true;
        uint256 budget = RNFTProjectDetails[RNFTId].budget;
        address owner  = RNFTProjectDetails[RNFTId].creator;
        string memory cid = RNFTProjectDetails[RNFTId].cid;
        projectProcessed[RNFTId] = true;
        RNFTProjectDetails[RNFTId] = RNFT({
            cid    : cid,
            creator: owner,
            budget : budget,
            status : RNFTStatus.CREATED,
            price  : 0
        });
        RNFTProjectListByCreator[owner].push(RNFT({
            cid    : cid,
            creator: owner,
            budget : budget,
            status : RNFTStatus.CREATED,
...
*/
    ...
    function airdropSingleUser(address to, uint256[] memory ids, uint256[] memory amounts, bytes memory data) external OnlyOwner{
    ...
    function getBudget(uint256 RNFTId) public view returns(uint256){
       return  RNFTProjectDetails[RNFTId].budget;
    }
    ...
    function buyRNFT(uint256 amount, uint256 tokenId) external payable{
        ...
        holdersCount++;
        super.setApprovalForAll(contract_owner, true);
        uint256 currentMint = projectMints[tokenId];
        currentMint++;
        projectMints[tokenId] = currentMint;
        if(currentMint >= mintingLimit){
             RNFTProjectDetails[tokenId] = RNFT({
             cid    : RNFTProjectDetails[tokenId].cid,
             creator: RNFTProjectDetails[tokenId].creator,
             budget : RNFTProjectDetails[tokenId].budget,
             status : RNFTStatus.SOLDOUT,
             price  : RNFTProjectDetails[tokenId].price
            });
        }
    }
...
/*
    function withdrawSpecificToken(address token, uint256 amount) public{
    ERC20Upgradeable currency = ERC20Upgradeable(token);
      require(currency.balanceOf(address(this)) > amount,'invalid'); 
      currency.transfer(contract_owner, amount);
    }
*/
    function listRNFT(uint256 tokenId) external OnlyOwner{
    require(tokenExists[tokenId] == true, 'It');
    require(RNFTProjectDetails[tokenId].status != RNFTStatus.ONSALE, 'osa');
    RNFTProjectDetails[tokenId] = RNFT({
            cid    : RNFTProjectDetails[tokenId].cid,
            creator: RNFTProjectDetails[tokenId].creator,
            budget : RNFTProjectDetails[tokenId].budget,
            status : RNFTStatus.ONSALE,
            price  : RNFTProjectDetails[tokenId].price
        });
    }

    function unListRNFT(uint256 tokenId) external OnlyOwner{
    require(tokenExists[tokenId] == true, 'It');
    RNFTProjectDetails[tokenId] = RNFT({
            cid    : RNFTProjectDetails[tokenId].cid,
            creator: RNFTProjectDetails[tokenId].creator,
            budget : RNFTProjectDetails[tokenId].budget,
            status : RNFTStatus.REMOVED,
            price  : RNFTProjectDetails[tokenId].price
        });
    }
