...
contract The999 is ERC721, Ownable, ReentrancyGuard {
    ...
    IRegistrarController registrarController;
    Counters.Counter public totalSupply;

    uint256 public constant price = 0.0555 ether;
    uint256 public constant MAX_SUPPLY = 100;
    uint8 public sale_state;
    // 0 closed
    // 1 reserved (by ENS)
    // 2 members (by ENS)
    // 3 holders
    ...
    constructor(
        ...
        registrarController = IRegistrarController(_reg);
    }
    ...
    function length2Pad(uint256 tokenNum) public pure returns (string memory) {
        string memory rep = Strings.toString(tokenNum);
        if (tokenNum < 10) rep = string(abi.encodePacked("0", rep));
        return rep;
    }

    function length3Pad(uint256 tokenNum) public pure returns (string memory) {
        string memory rep = Strings.toString(tokenNum);
    ...
    function ownerOfENS(uint256 tokenId) public view returns (address) {
        ...
        string memory name = length2Pad(tokenId);
    ...
    function reservedMint(uint256 tokenId)
        ...
        callerIsUser
    {
        if (sale_state == 0) revert SaleClosed();
        if (sale_state == 2) revert UseMemberMintFunction();
        ...
        if (sale_state == 1 && ownerOfENS(tokenId) != msg.sender)
            revert TokenownerOfENSThe999();
        if (sale_state == 3 && balanceOf(msg.sender) == 0)
            revert MustOwnATokenForPhase3();
        //require(msg.value >= price, "InsufficientETHSent");
    ...
    {
        if (sale_state == 0) revert SaleClosed();
        if (sale_state == 1) revert UseReservedMintFunction();
        if (totalSupply.current() == MAX_SUPPLY) revert AllMinted();
        if (tokenId < 0 || tokenId >= MAX_SUPPLY) revert TokenIdOutOfRange();
    ...
    function withdraw() external onlyOwner nonReentrant {
        address payable DEV1 = payable(
            address(0x51D3890Ea1635E7453B485607564e3C39965f900)
        );
        address payable DEV2 = payable(
            address(0xa110f521b197913055a174A941A2C0b8b0EddDE9)
        );
    ...
    function setBaseExtension(string calldata _extension) external onlyOwner {
        baseExtension = _extension;
    }

    function setSaleState(uint8 _sale_state) external onlyOwner {
        sale_state = _sale_state;
    }
}
