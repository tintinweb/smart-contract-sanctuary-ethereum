...
// File: contracts/KiddieFinal_V8_SplitMintingFunction.sol
...
contract KiddieMint is ERC721A, Ownable, ReentrancyGuard {
    using Strings for uint256;
    ...
    address[] public whitelistedPublicAddresses;
    address[] public whitelistedPrivateAddresses;

    mapping (address => uint) private userBalances;
    
    constructor() ERC721A("Kiddie Mint", "K Mint") {
        setHiddenMetadataUri("ipfs://QmR6DBuhHitXqvZyfNA687FvLaSMcMmDpAXN1osY96zctB/hidden.json");
    }

    modifier mintCompliance(uint256 quantity) {
        require(!paused, "The contract is paused!");
        require(isValidateEvent(), "The all event state is false");
        require(quantity > 0, "Invalid minting amount");
    ...
    function privateMint(uint256 quantity) external payable mintCompliance(quantity) {
        ...
        require(mintRatePrivateWhitelist == 0, "Set minrate to 0");
        require(isWhitelistedPrivate(msg.sender), "You are not on private whitelist to perform minting");
        require(totalSupply() + quantity <= MAX_SUPPLY_PRIVATE_WHITELISTED, "Not enough tokens left on private whitelist sale");

        _safeMint(msg.sender, quantity);
    }

    function publicWhitelistMint(uint256 quantity) external payable mintCompliance(quantity) {
        require(privateWhitelisted == false, "Set private whitelist to false");
        require(publicSale == false, "Public sale not false");
        require(isWhitelistedPublic(msg.sender), "You are not on public whitelist to perform minting");
    ...
    function publicMint(uint256 quantity) external payable mintCompliance(quantity) {
        ...
        require(quantity <= mintedPublicAddressLimit,  "Max minting per tx have limited on public sale");
    ...
    function setMaxSupplyPrivateWhitelisted(uint256 _maxSupply) public onlyOwner {
        MAX_SUPPLY_PRIVATE_WHITELISTED = _maxSupply;
    }
    ...
    function whitelistPublicAddresses(address[] calldata _users) public onlyOwner {
        delete whitelistedPublicAddresses;
        whitelistedPublicAddresses = _users;
    }

    function whitelistPrivateAddresses(address[] calldata _users) public onlyOwner {
        delete whitelistedPrivateAddresses;
        whitelistedPrivateAddresses = _users;
    }
    
    function removeWhitelistPublicAddresses(address _user) public onlyOwner {
        for (uint i = 0; i < whitelistedPublicAddresses.length; i++) {
        if (whitelistedPublicAddresses[i] == _user) {  
            delete whitelistedPublicAddresses[i];
            }
        }
    }
    ...
    function isWhitelistedPrivate(address _user) public view returns (bool) {
        for (uint i = 0; i < whitelistedPrivateAddresses.length; i++) {
            if (whitelistedPrivateAddresses[i] == _user) {
                return true;
            }
        }
        return false;
