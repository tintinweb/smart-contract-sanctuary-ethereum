...
contract PoolNFT is ERC1155Supply, Ownable {
    ...
    function setManager(address manager_, uint256 tokenId_) external onlyOwner {
        require(manager_ != address(0), "PoolNFT: manager address should not be null");
        require(tokenId_ > 0, "PoolNFT: tokenId should not be 0");
        require(managers[tokenId_] == address(0), "PoolNFT: tokenId is already assigned");
        managers[tokenId_] = manager_;
    }

    modifier onlyManager(uint256 tokenId) {
        require(managers[tokenId] == msg.sender, "PoolNFT: caller is not a manager");
        _;
    }

    modifier onlyManagerOrOwner(uint256 tokenId) {
        require(managers[tokenId] == msg.sender || owner() == _msgSender(), "PoolNFT: caller is not the manager or owner");
        _;
    }
