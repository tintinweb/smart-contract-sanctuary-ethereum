...
contract NavyGDonut is ERC2981, ERC721AQueryable, Ownable {
    ...
    function compound(address who, uint256 [] memory tokenIds, bytes memory signature) external {
        ...
        if (tokenIds.length == 4) {
            ...
            string memory sign_context = string(abi.encodePacked(who, ";", tokenIds[0], ";", tokenIds[1], ";", tokenIds[2]));
            require(!needAuth || signer == signatureWallet(sign_context, signature), "Not authorized to compound");
        } else if (tokenIds.length == 2){
            string memory sign_context = string(abi.encodePacked(who, ";", tokenIds[0], ";", tokenIds[1]));
            require(!needAuth || signer == signatureWallet(sign_context, signature), "Not authorized to compound");
        } else {
    ...
    /************************************* internal ********************/
    function signatureWallet (string memory context, bytes memory signature) private pure returns (address){
