    ...
    IHyperLPool
{
    ...
    // solhint-disable-next-line max-line-length
    constructor(address payable _hyperpools) HyperLPoolStorage(_hyperpools) {} // solhint-disable-line no-empty-blocks
    ...
    /// @return mintAmount The number of G-UNI tokens to mint
        ...
        )
    {
        ...
        (uint128 liquidity, , , , ) = pool.safePositions(_getPositionID());
    ...
    function executiveRebalance(
        ...
        if (totalSupply() > 0) {
            ...
            (newLiquidity, , , , ) = pool.safePositions(_getPositionID());
    ...
    /// @notice withdraw manager fees accrued, only hyperpools executors can call.
    /// Target account to receive fees is managerTreasury, alterable by manager.
    /// Frequency of withdrawals configured with hyperpoolsWithdrawBPS, alterable by manager.
        ...
        external
    {
        ...
        _hyperpoolsfy(feeAmount, feeToken);
    }

    /// @notice withdraw hyperpools fees accrued, only hyperpools executors can call.
    /// Frequency of withdrawals configured with hyperpoolsWithdrawBPS, alterable by manager.
    function withdrawHyperPoolsBalance(uint256 feeAmount, address feeToken)
        external
    {
        require(msg.sender == HYPERPOOLS, "Hyperfied: Only hyperpools");
        ...
        if (amount0 > 0) {
            token0.safeTransfer(HYPERPOOLS, amount0);
        }

        if (amount1 > 0) {
            token1.safeTransfer(HYPERPOOLS, amount1);
        }
    ...
    function _balancesToWithdraw(
        ...
        if (feeToken == address(token0)) {
            amount0 = _balanceToWithdraw(balance0, feeAmount);
            amount1 = balance1;
        } else if (feeToken == address(token1)) {
            amount1 = _balanceToWithdraw(balance1, feeAmount);
            amount0 = balance0;
        } else {
            revert("wrong token");
        }
    ...
    function _balanceToWithdraw(uint256 balance, uint256 feeAmount)
        internal
        view
        returns (uint256 amount)
    {
        require(
            FullMath.mulDiv(balance, hyperpoolsWithdrawBPS, 10000) >= feeAmount,
            "high fee"
        );
    ...
    // XXXX DONT MODIFY ORDERING XXXX
{
    ...
    uint16 public hyperpoolsWithdrawBPS;
    ...
    event UpdateHyperPoolsParams(
        uint16 hyperpoolsRebalanceBPS,
        uint16 hyperpoolsWithdrawBPS,
    ...
    // solhint-disable-next-line max-line-length
    constructor(address payable _hyperpools) Hyperfied(_hyperpools) {} // solhint-disable-line no-empty-blocks
    ...
    function initialize(
        ...
        hyperpoolsWithdrawBPS = 100; // default: only auto withdraw if tx fee is lt 1% withdrawn
        ...
        // e.g. "HyperPools Uniswap V3 USDC/DAI LP" and "G-UNI"
    ...
    /// @param newRebalanceBPS controls frequency of hyperpools rebalances: gas fee to execute
    /// rebalance can be hyperpoolsRebalanceBPS proportion of fees earned since last rebalance
    /// @param newWithdrawBPS controls frequency of hyperpools withdrawals: gas fee to execute
    /// withdrawal can be hyperpoolsWithdrawBPS proportion of fees accrued since last withdraw
    /// @param newSlippageBPS maximum slippage on swaps during hyperpools rebalance
    ...
    function updateHyperPoolsParams(
        ...
        emit UpdateHyperPoolsParams(
            newRebalanceBPS,
            newWithdrawBPS,
            newSlippageBPS,
            newSlippageInterval
        );
        if (newRebalanceBPS != 0) hyperpoolsRebalanceBPS = newRebalanceBPS;
        if (newWithdrawBPS != 0) hyperpoolsWithdrawBPS = newWithdrawBPS;
        if (newSlippageBPS != 0) hyperpoolsSlippageBPS = newSlippageBPS;
        if (newSlippageInterval != 0)
            hyperpoolsSlippageInterval = newSlippageInterval;
    ...
    function initializeManagerFee(uint16 _managerFeeBPS) external onlyManager {
        ...
        require(
            _managerFeeBPS > 0 && _managerFeeBPS <= 10000 - hyperpoolsFeeBPS,
            "mBPS"
        );
