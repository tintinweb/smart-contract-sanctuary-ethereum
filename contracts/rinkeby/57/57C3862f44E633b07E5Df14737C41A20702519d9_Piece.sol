...
contract Piece is ERC721, Ownable {
    ...
    bytes32 public whitelistMerkleRoot = 0x9d89f0627bfb0b5898c82b103dff7d03fd5d83c552be0118fc6ca14d4d23aa42;
    bytes32 public prizeMerkleRoot = 0xa1c7c24a2fcf8d87600c3dc6207c2dfd94485ddff99ebb4892d62779e1bb989d;
    ...
    uint256 public totalPrizes = 510; 
    uint256 public publicPieceLimit = 11;
    uint256 public presalePieceLimit = 16;
    uint256 public maxPresaleSupply = 800;
    uint256 public initialSpins = 3;
    uint256 private _wlPrice = .02 ether; 
    uint256 private _price = .06 ether; 

    bool public publicPaused = true;

    address public founder = 0x29Bdc8A1aA8DCAeE884Fd504e08a0b13760cE6b0;
    address public proxyRegistryAddress = 0xa5409ec958C83C3f309868babACA7c86DCB077c1;
    address private wheelAddress;
    ...
    mapping(bytes32 => mapping(address => bool)) mintClaimed;
    ...
    function freeMint (bytes32[] calldata proof) external payable {
        bytes32 leaf = keccak256(abi.encodePacked(_msgSender()));
        ...
        require(mintClaimed[freeMerkleRoot][_msgSender()] == false, "You already claimed your free mint");

        mintClaimed[freeMerkleRoot][_msgSender()] = true;
        _safeMint(_msgSender(), tokenCounter.current());
        tokenCounter.increment();
    ...
    function createPrize () internal  {
        require(prizeTokenCounter.current() + 1 < totalPrizes, "Sorry its wrong");
    ...
    function claimPrize(uint256 tokenID, bytes32[] calldata proof, string memory rarity, uint256 position) public {
        ...
        require(pieceToClaimed[tokenID] == false);
        require(prizeWinner(tokenID, proof, rarity, position), "Invalid proof");
        require(prizeTokenCounter.current() + 1 < totalPrizes, "Sorry there are no more prizes available for claiming");
        
        uint256 prizeAmount;
        for(uint256 i = 0; i < 10; i ++){
            if(position < prizeCount[i]){ //set these to 307, 407, ...
    ...
    function addFreeSpin(uint256 tokenID, uint256 quantity, address sender) public onlyWheel { //add onlyProxies?
        require(_isApprovedOrOwner(sender, tokenID), "You are not the owner of this piece");
        freeSpinsAvailable[tokenID] += quantity;
    }
    
    function subFreeSpin(uint256 tokenID, uint256 quantity, address sender) public onlyWheel{ //add onlyProxies?
        require(_isApprovedOrOwner(sender, tokenID), "You are not the owner of this piece");
        freeSpinsAvailable[tokenID] -= quantity;
    }

    function addPaidSpin(uint256 tokenID, uint256 quantity, address sender) public onlyWheel { //add onlyProxies?
        require(_isApprovedOrOwner(sender, tokenID), "You are not the owner of this piece");
        paidSpinsAvailable[tokenID] += quantity;
    }
    
    function subPaidSpin(uint256 tokenID, uint256 quantity, address sender) public onlyWheel{ //add onlyProxies?
        require(_isApprovedOrOwner(sender, tokenID), "You are not the owner of this piece");
        paidSpinsAvailable[tokenID] -= quantity;
    }
