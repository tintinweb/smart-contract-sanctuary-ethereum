...
contract BananaMarket is  Ownable, ReentrancyGuard{
    ...
    event TradeComplete(uint256  itemId, address token, uint256 amount, address swappedFor, uint256 indexed swappedForAmount, address indexed buyer, bool indexed sold );
    ...
    function acceptTrade(uint256 _itemId) external  nonReentrant{
        ...
        // Emit Purchased Event
        emit TradeComplete(trade.itemId, address(trade.token), trade.amount, address(trade.swappedFor), trade.swappedForAmount, msg.sender, true);
    }
