 ...
 * @notice It handles the distribution of fees sharing
 */
contract StakingRewardDistributor is ReentrancyGuard, Ownable {
    ...
    IERC20 public immutable gallerToken;
    ...
    // TODO: need confirm reward duration
    ...
    event NewRewardPeriod(uint256 indexed lastUpdateBlock, uint256 rewardPerBlock, uint256 reward);

    /**
     * @notice Constructor
     * @param _gallerToken address of the token staked (GLR)
     */
    constructor(address _gallerToken) {
        gallerToken = IERC20(_gallerToken);
    }
     ...
     */
    function updateRewards(uint256 reward) external onlyOwner {
        // Adjust the current reward per block
        if (block.number >= periodEndBlock) {
            currentRewardPerBlock = reward / REWARD_DURATION_IN_BLOCKS;
        } else {
            currentRewardPerBlock = (reward + ((periodEndBlock - block.number) * currentRewardPerBlock)) / REWARD_DURATION_IN_BLOCKS;
        }
    ...
    function deposit(uint256 amount) external nonReentrant updateReward(msg.sender) {
        require(amount >= PRECISION_FACTOR, "Deposit: Amount must be >= 1 GLR");

        // Adjust internal shares
        userInfo[msg.sender].shares += amount;
        totalShares += amount;

        // Transfer GLR tokens to this address
        gallerToken.safeTransferFrom(msg.sender, address(this), amount);
    ...
    function harvest() external nonReentrant updateReward(msg.sender) {
        ...
        // Transfer reward (BNB) to sender
        (bool success, ) = msg.sender.call{ value: pendingRewards }("");
        require(success, "Harvest: transfer BNB failed");
    ...
    function withdraw(uint256 shares, bool claimRewardToken) external nonReentrant {
        require((shares > 0) && (shares <= userInfo[msg.sender].shares), "Withdraw: Shares equal to 0 or larger than user shares");
    ...
    function _calculatePendingRewards(address user) internal view returns (uint256) {
        return ((userInfo[user].shares * (_rewardPerToken() - (userInfo[user].userRewardPerTokenPaid))) / PRECISION_FACTOR) + userInfo[user].rewards;
    }
    ...
    function _rewardPerToken() internal view returns (uint256) {
        ...
        return rewardPerTokenStored + ((_lastRewardBlock() - lastUpdateBlock) * (currentRewardPerBlock * PRECISION_FACTOR)) / totalShares;
    }
     ...
     * @param claimRewardToken whether to claim reward tokens
     */
    function _withdraw(uint256 shares, bool claimRewardToken) internal updateReward(msg.sender) {
        // Retrieve total amount staked and calculated current amount (in GLR)

        // Adjust internal shares
        userInfo[msg.sender].shares -= shares;
        totalShares -= shares;
        // Transfer GLR tokens to sender
        gallerToken.safeTransfer(msg.sender, shares);
        ...
        if (claimRewardToken) {
            ...
            if (pendingRewards > 0) {
                ...
                // Transfer reward (BNB) to sender
                (bool success, ) = msg.sender.call{ value: pendingRewards }("");
                require(success, "Withdraw: transfer BNB failed");
            }
        }
