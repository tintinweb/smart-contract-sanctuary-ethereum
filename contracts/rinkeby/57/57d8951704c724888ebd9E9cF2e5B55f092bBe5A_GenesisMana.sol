...
contract GenesisMana is ERC721Enumerable, ReentrancyGuard, Ownable {
  using EditStrings for string;
  using EditStrings for EditStrings.slice;
   ...
   mapping(uint8 => mapping(uint8 => uint8)) private _currentLostGenesisMana;
    ...
    // constructor() ERC721("Genesis Mana", "G") Ownable() {}
      ...
      onlyOwner
    {
        ...
        // REMOVE THIS FOR LOCAL AND RINKEBY TEST
    ...
    function _claim(uint256 lootTokenId, uint8 inventoryId,  uint8 suffixId, address tokenOwner) internal {
        ...
        if (lootTokenId > 0) {
          ...
          EditStrings.slice memory itemSlice = itemName.toSlice();
...
// strings to EditStrings because of the name collision isssue with String
library EditStrings {
    struct slice {
        uint _len;
        uint _ptr;
