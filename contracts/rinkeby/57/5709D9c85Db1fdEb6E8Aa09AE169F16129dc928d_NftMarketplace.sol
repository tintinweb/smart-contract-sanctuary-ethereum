...
contract NftMarketplace is Ownable {
    ...
    mapping(uint256 => Listing) private idToList;
    ...
    function listItem(
        ...
        emit tokenListed(nftAddress, msg.sender, tokenId, amount, price);
    ...
    function buyItem(uint256 listingId, uint256 amount) public payable {
        ...
        require(
            idToList[listingId].tokensAvailable >= amount,
            "Not enough nfts"
        );

        _volume += idToList[listingId].price * amount;

        idToList[listingId].tokensAvailable -= amount;
        if (idToList[listingId].tokensAvailable == 0) {
