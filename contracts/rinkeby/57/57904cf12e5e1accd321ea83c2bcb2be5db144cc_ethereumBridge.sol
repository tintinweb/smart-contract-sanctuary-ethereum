...
contract ethereumBridge {
 ...
 event TokensBridged(
    ...
    function bridgeTokens(address _requester, uint amount)
        external
        onlyGateway
    {
       
        IERC20(ethToken).mint(_requester, amount);
        emit TokensBridged(
            _requester,
            amount,
            block.timestamp
    ...
    function lockTokens(address _requester, uint256 _bridgedAmount)
        external
        onlyGateway
    {
        IERC20(ethToken).burn(_requester, _bridgedAmount);
        emit TokensLocked(
            _requester,
            _bridgedAmount,
            block.timestamp
