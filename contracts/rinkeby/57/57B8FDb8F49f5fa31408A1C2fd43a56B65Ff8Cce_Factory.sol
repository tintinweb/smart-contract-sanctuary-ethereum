...
contract Factory {
    ...
    event SecurityContractCreated(address securityAddress);

    error SymbolTaken();

    function createSecurityContract(
        string calldata _name,
        string calldata _symbol,
        SecurityInfo memory _info
    ) external returns (address securityContractCreated) {
        if (symbolTaken[keccak256(abi.encode(_symbol))] == true) {
            revert SymbolTaken();
        }

        symbolTaken[keccak256(abi.encode(_symbol))] = true;

        tokenIdCounter.increment();
        uint256 currentTokenId = tokenIdCounter.current();

        SecurityContract securityContract = new SecurityContract(_name, _symbol, _info);

        tokenToContract[currentTokenId] = address(securityContract);

        emit SecurityContractCreated(address(securityContract));
        return address(securityContract);
    }

    function getAllSecurityContracts() external view returns (address[] memory) {
        uint256 i;
        uint256 currentTokenId = tokenIdCounter.current();
        address[] memory contracts = new address[](currentTokenId + 1);
        for (i = 1; i <= currentTokenId; i++) {
            contracts[i] = tokenToContract[i];
        }
        return contracts;
    }

    function applyFilter(string memory _countryFilter) external view returns (address[] memory) {
        uint256 i;
        uint256 currentTokenId = tokenIdCounter.current();
        address[] memory contracts = new address[](currentTokenId + 1);
        uint256 j = 1;
        for (i = 1; i <= currentTokenId; i++) {
            if (
                keccak256(abi.encode(SecContract(tokenToContract[i]).getCountry())) ==
                keccak256(abi.encode(_countryFilter))
            ) {
                contracts[j] = tokenToContract[i];
                j++;
            }
        }

        address[] memory contractsFinal = new address[](j);
        uint256 k;
        for (k = 1; k < j; k++) {
            contractsFinal[k] = contracts[k];
        }
...
contract SecurityContract is ERC20 {
    ...
    SecurityInfo public info;

    mapping(address => bool) public whitelist;

    address public institutionAgentAddress;
    ...
    event AddWhiteList(address candidate);
    event ChengedInstitutionAgentAddress(address institutionAgentAddress);
    ...
    constructor(
        ...
        institutionAgentAddress = tx.origin;
    ...
    function buyTokens() external payable {
        ...
        availableTokens -= tokenBought; //effect
        _approve(address(this), msg.sender, tokenBought);
        transferFrom(address(this), msg.sender, tokenBought); // interaction
        emit BuyTokens(msg.sender, tokenBought);
    }

    function addOnWhitelist(address _candidate) external {
        if (msg.sender != institutionAgentAddress) {
            revert UnauthorisedAccess();
        }
        whitelist[_candidate] = true;
        emit AddWhiteList(_candidate);
    }

    function setInstitutionAgentAddress(address _newInstitutionAddress) external {
        if (msg.sender != institutionAgentAddress) {
            revert UnauthorisedAccess();
        }
        institutionAgentAddress = _newInstitutionAddress;
        emit ChengedInstitutionAgentAddress(_newInstitutionAddress);
    }
    ...
    function contractURI() public view returns (string memory) {
        ...
        writer = writer.writeStringProperty("symbol", symbol());
        writer = writer.writeStringProperty("country", info.country);
        writer = writer.writeStringProperty("city", info.city);
        writer = writer.writeStringProperty("address", info.addressOfAsset);
        writer = writer.writeStringProperty("description", info.description);
        writer = writer.writeStringProperty("typeOfBuilding", info.typeOfBuilding);
        writer = writer.writeUintProperty("area", info.area);
        writer = writer.writeUintProperty("nrOfFloors", info.nrOfFloors);
        writer = writer.writeStringProperty("image", info.folderCID);
        writer = writer.writeStartArray("images");
        for (uint256 i; i < info.images.length; i++) {
            writer = writer.writeStringValue(info.images[i]);
        }
