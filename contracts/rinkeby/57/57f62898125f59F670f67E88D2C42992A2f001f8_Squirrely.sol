...
contract Squirrely is ERC721, Ownable {
    ...
    constructor() ERC721("Squirrely", "Squirrely") {
    }

    string private baseUri = "https://ipfs.io/ipfs/QmWMDnrdmQqXcrbQxa8GvA8JR294AwLZ8Huu275Pn3zGhQ/";
     ...
     * @notice minting
     */
    function mint(address _owner, uint16[] calldata _tokenIds) external payable {  
        uint256 length = _tokenIds.length;
        require(isPaused == false, "mint is not live");
        require(msg.value >= MINT_PRICE * length, "not enough ETH sent, check price");
        require(length < 11, "You can mint a maximum of 10");
        unchecked {
    ...
    function creatorMint(address _owner, uint16[] calldata _tokenIds) external payable onlyOwner {
        unchecked {
            uint256 length = _tokenIds.length;
            if (totalSupply + length > MAX_SUPPLY) revert SupplyReached();
            totalSupply += length;
    ...
    /**
     * @notice returns token ids owned by an owner, don't use this onchain
     */
    ...
    /**
     * @notice returns token ids batched owned by an owner, don't use this onchain
     */
