    ...
    ReentrancyGuard
{
    ...
    function _receiveChild(
        ...
        if (childContracts[_tokenId].length() != 0) {
            ...
            uint256 oldChildTokenId = childTokenByIndex(
                _tokenId,
                oldChildContract,
                0
            );
            ...
            ERC721(oldChildContract).safeTransferFrom(
                address(this),
                rootOwner,
                oldChildTokenId
            );
    ...
    {
        uint256 totalChildToken = 0;
        for (uint256 i; i < totalChildContracts(_tokenId); ++i) {
            address childContractAddress = childContractByIndex(_tokenId, i);
            for (
                uint256 j;
                j < childTokens[_tokenId][childContractAddress].length();
                ++j
            ) {
                uint256 childId = childTokenByIndex(
                    _tokenId,
        ...
        ChildToken[] memory child = new ChildToken[](totalChildToken);
        totalChildToken = 0;
        for (uint256 i; i < totalChildContracts(_tokenId); ++i) {
            address childContractAddress = childContractByIndex(_tokenId, i);
            for (
                ...
                child[totalChildToken] = ChildToken(
                    childContractAddress,
                    childId
                );
                ++totalChildToken;
