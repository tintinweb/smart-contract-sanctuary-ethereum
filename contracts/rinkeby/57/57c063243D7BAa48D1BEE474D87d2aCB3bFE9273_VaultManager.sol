...
contract VaultManager is OrumBase, Ownable, CheckContract, IVaultManager {
    ...
    *
    * An oETH gain of ( stake * [L_STAKINGR - L_STAKINGR(0)] )
    *
    ...
    */
    ...
    // Object containing the oETH snapshots for a given active vault
    ...
    */
    function batchLiquidateVaults(address[] memory _vaultArray) public override {
        ...
        // Move liquidated oETH and USDC to the appropriate pools
        activePoolCached.offsetLiquidation(totals.totalCollToSendToSP);
        // Send fees to the orum revenue
        activePoolCached.sendoETH(orumRevenueAddress, totals.totalCollToSendToOrumRevenue);
        ...
        if (totals.totalCollSurplus > 0) {
            activePoolCached.sendoETH(address(collSurplusPool), totals.totalCollSurplus);
            collSurplusPool.receiveoETHInCollSurplusPool(totals.totalCollSurplus);
        }
    ...
    */
    ...
    /*
    *  Get its offset coll/debt and oETH gas comp, and close the vault.
    */
    ...
    */
    function liquidateVaults(uint _n) external override {
        ...
        IStakingPool stakingPoolCached = stakingPool;
        LocalVariables_OuterLiquidationFunction memory vars;
        LiquidationTotals memory totals;
        vars.price = priceFeed.fetchPrice();
        vars.recoveryModeAtStart = _checkRecoveryMode(vars.price);
        ...
        // Move liquidated oETH and USDC to the appropriate pools
        contractsCache.activePool.offsetLiquidation(totals.totalCollToSendToSP);
        // Send fees to the orum revenue
        contractsCache.activePool.sendoETH(orumRevenueAddress, totals.totalCollToSendToOrumRevenue);
        // Only for testing
        emit TEST_liquidationfee(totals.totalCollToSendToSP, totals.totalCollToSendToOrumRevenue);

        if (totals.totalCollSurplus > 0) {
            contractsCache.activePool.sendoETH(address(collSurplusPool), totals.totalCollSurplus);
            collSurplusPool.receiveoETHInCollSurplusPool(totals.totalCollSurplus);
        }
        vars.liquidatedDebt = totals.totalDebtInSequence;
        vars.liquidatedColl = totals.totalCollInSequence.sub(totals.totalCollGasCompensation).sub(totals.totalCollSurplus);
    ...
    */
        ...
        returns (uint debtToOffset, uint collToSendToSP, uint collToSendToOrumRevenue, uint stakingRToRedistribute)
    {
        ...
        */
            ...
            // get the debt to offset in terms of the collateral i.e, oETH
            uint _debtInoETH = debtToOffset.mul(DECIMAL_PRECISION).div(_price).mul(DECIMAL_PRECISION).div(USDC_DECIMAL_PRECISION);
            uint _totalCollProfit = 0; 

            if (collToSendToSP > _debtInoETH) {
               _totalCollProfit =  uint(OrumMath._max(int(collToSendToSP - _debtInoETH), 0));
            ...
            emit TEST_offsetValues(_debtInoETH, debtToOffset, collToSendToSP, collToSendToOrumRevenue, _totalCollProfit, stakingRToRedistribute);
    }

    function _sendGasCompensation(IActivePool _activePool, address _liquidator, uint _USDC, uint _oETH) internal {
        if (_USDC > 0) {
        gasPool.sendToLiquidator(_USDC, _liquidator);
        }

        if (_oETH > 0) {
            _activePool.sendoETH(_liquidator, _oETH);
        }
    }

    // Move a Vault's pending debt and collateral rewards from distributions, from the Default Pool to the Active Pool
    function _movePendingVaultRewardsToActivePool(IActivePool _activePool, IBorrowersRewardsPool _borrowersRewardsPool, uint _oETH_Staking_Reward) internal {
        _borrowersRewardsPool.sendborrowersoETHRewardsToActivePool(_oETH_Staking_Reward);
    }
    ...
    function getNominalICR(address _borrower) public view override returns (uint) {
        (uint currentoETH, uint currentUSDCDebt) = _getCurrentVaultAmounts(_borrower);

        uint NICR = OrumMath._computeNominalCR(currentoETH, currentUSDCDebt);
        return NICR;
    }
    ...
    function getCurrentICR(address _borrower, uint _price) public view override returns (uint) {
        ...
        uint ICR = OrumMath._computeCR(currentoETH, currentUSDCDebt, _price);
        return ICR;
    }

    function _getCurrentVaultAmounts(address _borrower) internal view returns (uint, uint) {

        uint pendingStakingoethReward = getPendingStakingoethReward(_borrower);

        uint currentoETH = Vaults[_borrower].coll.add(pendingStakingoethReward);
        uint currentUSDCDebt = Vaults[_borrower].debt;

        return (currentoETH, currentUSDCDebt);
    }
    ...
    function _applyStakingRewards(IActivePool _activePool, IBorrowersRewardsPool _borrowersRewardsPool, address _borrower) internal {
        if (hasPendingRewards(_borrower)) {
            ...
            Vaults[_borrower].coll = Vaults[_borrower].coll.add(pendingStakingoethReward);

            _updateVaultRewardSnapshots(_borrower);

            // Transfer from DefaultPool to ActivePool
            _movePendingVaultRewardsToActivePool(_activePool, _borrowersRewardsPool, pendingStakingoethReward);
    ...
    // Get the borrower's pending accumulated oETH reward, earned by their stake
    function getPendingStakingoethReward(address _borrower) public view override returns (uint) {
        ...
        uint pendingStakingoethReward = stake.mul(rewardPerUnitStaked).div(DECIMAL_PRECISION);

        return pendingStakingoethReward;
    }
        ...
        returns (uint debt, uint coll, uint pendingStakingReward)
    {
        ...
        pendingStakingReward = getPendingStakingoethReward(_borrower);
    ...
    function redistributeStakingRewards(uint _coll) external override {
        _requireCallerIsborrowersRewardsPool();
        ...
        */
        uint oETHNumerator = _coll.mul(DECIMAL_PRECISION).add(lastSTAKINGRError_Redistribution);

        // Get the per-unit-staked terms
        uint oETHSTAKINGRewardPerUnitStaked = oETHNumerator.div(totalStakes);

        lastSTAKINGRError_Redistribution = oETHNumerator.sub(oETHSTAKINGRewardPerUnitStaked.mul(totalStakes));

        // Add per-unit-staked terms to the running totals
        L_STAKINGR = L_STAKINGR.add(oETHSTAKINGRewardPerUnitStaked);
    ...
    */
    ...
    // Check whether or not the system *would be* in Recovery Mode, given an oETH:USD price, and the entire system coll and debt.
    ...
    function _requireCallerIsborrowersRewardsPool() internal view {
        require(msg.sender == address(borrowersRewardsPool), "VaultManager: Caller is not the BorrowerOps contract");
    }
...
interface ICollSurplusPool {
    ...
    event OETHTokenAddress(address _oETHTokenAddress);
...
/* 
* Base contract for VaultManager, BorrowerOps and StakingPool. Contains global system constants and
* common functions. 
*/
