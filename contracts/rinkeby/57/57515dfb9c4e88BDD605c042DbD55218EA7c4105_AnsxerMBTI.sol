...
contract AnsxerMBTI is ERC721Psi, Ownable, Pausable {
  ...
  function mbtiOfTokens(uint256 from, uint256 to) public view returns (string[] memory) {
    require(_exists(from) && _exists(to), "Nonexistent tokens");
    require(to > from, "Invalid range");
    unchecked {
      uint256 count = to - from + 1;
      string[] memory mbtis = new string[](count);
      string[16] memory keys = mbtiKeys();
      for (uint i; i < count; ++i) {
        mbtis[i] = keys[_mbtiOfToken[from + i]];
      }
      return mbtis;
    }
  }
