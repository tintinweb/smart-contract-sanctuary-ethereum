...
contract MillionaireRowTicket is ERC721A, Ownable, PaymentSplitter {
    bool public saleActive = false;

    string internal baseTokenURI;

    uint256 public goldPrice = 0.0001 ether;
    uint256 public silverPrice = 0.0001 ether;
    uint256 public diamondPrice = 0.0001 ether;
    ...
    event MintGold(address owner, uint256 qty);
    event MintSilver(address owner, uint256 qty);
    event MintDiamond(address owner, uint256 qty);
    ...
    constructor(
        ...
        ERC721A("MillionaireRowTicket", "MROW_TCKT")
    ...
    function setSilverPrice(uint256 newPrice) external onlyOwner {
    ...
    function mintGold(uint256 qty) external payable canMint(qty) {
        require(msg.value == goldPrice * qty, "Invalid value");
        _safeMint(msg.sender, qty);
        emit MintGold(msg.sender, qty);
    }

    function mintSilver(uint256 qty) external payable canMint(qty) {
        require(msg.value == silverPrice * qty, "Invalid value");
        _safeMint(msg.sender, qty);
        emit MintSilver(msg.sender, qty);
    }

    function mintDiamond(uint256 qty) external payable canMint(qty) {
        require(msg.value == diamondPrice * qty, "Invalid value");
        _safeMint(msg.sender, qty);
        emit MintDiamond(msg.sender, qty);
    }
