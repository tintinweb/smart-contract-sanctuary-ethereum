                ...
                uint256(
                    keccak256(
                        abi.encodePacked(
                            ...
                            hex"9635768086070064d3eb020b7659d15105bf3dfd9ba78fa7cc8ed3e75c0f1a44" // init code hash
                        )
                    )
                )
            )
...
contract CxpRouter is ICxpRouter {
    ...
    function _swap(
        ...
        for (uint256 i; i < path.length - 1; i++) {
            ...
            (address token0, ) = CxpLibrary.sortTokens(input, output);
            uint256 amountOut = amounts[i + 1];
            (uint256 amount0Out, uint256 amount1Out) = input == token0
                ? (uint256(0), amountOut)
                : (amountOut, uint256(0));
            address to = i < path.length - 2
                ? CxpLibrary.pairFor(factory, output, path[i + 2])
                : _to;
            ICxpPair(CxpLibrary.pairFor(factory, input, output)).swap(
                amount0Out,
                amount1Out,
                to,
                new bytes(0)
    ...
    function swapExactTokensForTokens(
        ...
        returns (uint256[] memory amounts)
    {
        amounts = CxpLibrary.getAmountsOut(factory, amountIn, path);
        require(
        ...
        TransferHelper.safeTransferFrom(
            path[0],
            msg.sender,
            CxpLibrary.pairFor(factory, path[0], path[1]),
            amounts[0]
        );
    ...
    function swapTokensForExactTokens(
        ...
        returns (uint256[] memory amounts)
    {
        amounts = CxpLibrary.getAmountsIn(factory, amountOut, path);
    ...
    function getAmountsOut(uint256 amountIn, address[] memory path)
        ...
        returns (uint256[] memory amounts)
    {
        return CxpLibrary.getAmountsOut(factory, amountIn, path);
    }
    ...
    function getAmountsIn(uint256 amountOut, address[] memory path)
        ...
        returns (uint256[] memory amounts)
    {
        return CxpLibrary.getAmountsIn(factory, amountOut, path);
    }
}