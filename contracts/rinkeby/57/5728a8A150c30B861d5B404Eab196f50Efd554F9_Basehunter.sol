...
// Based on: https://github.com/FrankieIsLost/CRISP/blob/a36ffc23696b8256041cc27478881b8012967880/src/CRISP.sol
// Modified so mint function is inlined to prevent reentrancy from refunds, and packed variables to save gas
...
abstract contract CRISP is ERC721 {
  ...
  constructor(
    ...
    targetEMS = PRBMathSD59x18.fromInt(1).div(
      PRBMathSD59x18.fromInt(1) -
        PRBMathSD59x18.fromInt(2).pow(-_targetBlocksPerSale.div(saleHalflife))
    );
  ...
  function getCurrentEMS() public view returns (int256 result) {
    ...
    int256 weightOnPrev = PRBMathSD59x18.fromInt(2).pow(-blockInterval.div(saleHalflife));
  ...
  function getQuote() internal view returns (int256 result) {
    if (block.number <= priceDecayStartBlock) {
      ...
      int256 decayInterval = int256(block.number - priceDecayStartBlock).fromInt();
      int256 decay = (-decayInterval).div(priceHalflife).exp();
      result = nextPurchaseStartingPrice.mul(decay);
    }
  }

  /// @notice Returns the price, in wei, to pay for purchasing in the current block.
  function price() public view returns (uint256 result) {
    return uint256(getQuote().toInt());
  }

  /// @notice Get starting price for next purchase before time decay. Returns 59.18-decimal fixed-point.
  function getNextStartingPrice(int256 lastPurchasePrice) public view returns (int256 result) {
  ...
  function getPriceDecayStartBlock() internal view returns (uint256 result) {
    ...
    if (mismatchRatio > PRBMathSD59x18.fromInt(1)) {
      ...
      uint256 decayInterval = uint256(saleHalflife.mul(mismatchRatio.log2()).ceil().toInt());
  ...
  function toUint32(uint256 _x) internal pure returns (uint32) {
    if (_x > type(uint32).max) revert SafeCastFailed();
    return uint32(_x);
  }

  function blockNumber() internal view returns (uint32) {
    return toUint32(block.number);
  }
}

contract Basehunter is CRISP {
  using PRBMathSD59x18 for int256;
  ...
  address payable public constant gitcoin = payable(0xde21F729137C5Af1b01d73aF1dC21eFfa2B8a0d6); // Gitcoin multisig.
  address payable public constant creator = payable(0xc06AF4D1eBD345225C40175FC747441879Dbb6eB);

  // Mapping from token ID to that token's metadata. TokenParams are packed into a single slot.
  struct TokenParams {
    address minter; // Address that called mint.
    uint64 basefee; // Base fee (with uint64, max supported base fee is about 18 billion gwei).
    uint32 blocknum; // Block number (with uint32, this contract will work for about 1600 years).
  }
  mapping (uint256 => TokenParams) public metadata;
  ...
  constructor() CRISP(
    "Basehunter",                     // Name.
    "BASE",                           // Symbol.
    PRBMathSD59x18.fromInt(100),      // Target blocks per sale.
    PRBMathSD59x18.fromInt(700),      // Sale half life.
    PRBMathSD59x18.fromInt(1),        // Price speed.
    PRBMathSD59x18.fromInt(100),      // Price half life.
    PRBMathSD59x18.fromInt(0.1 ether) // Starting Price.
  ) {}
  ...
  /// @notice Mint an NFT. Call `price()` first to get the price, in wei, of minting.
  function mint() public payable {
    // Save metadata and verify enough ETH was sent.
    metadata[curTokenId] = TokenParams({ minter: msg.sender, basefee: uint64(block.basefee), blocknum: blockNumber() });
    int256 currentPrice = getQuote();
    uint256 priceScaled = uint256(currentPrice.toInt()); // This is how much the contract earns from this mint.
    if (msg.value < priceScaled) revert InsufficientPayment();

    // Mint token and update state.
    _mint(msg.sender, curTokenId++);
    nextPurchaseStartingEMS = getCurrentEMS() + PRBMathSD59x18.fromInt(1);
    nextPurchaseStartingPrice = getNextStartingPrice(currentPrice);
    priceDecayStartBlock = toUint32(getPriceDecayStartBlock());
    lastPurchaseBlock = blockNumber();

    // Give 75% of the cost to Gitcoin.
    (bool _success, ) = gitcoin.call{value: priceScaled * 75 / 100}("");
    if (!_success) revert FailedToSendEther();

    // Issue refund to caller last, after everything else was completed, to mitigate reentrancy.
  ...
  /// @notice Funds for Gitcoin are automatically paid out to their multisig. The creator must claim funds, which
  /// done to minimize mint costs for users.
  function claim() external {
    if (msg.sender != creator) revert Unauthorized();
    (bool _success, ) = creator.call{value: address(this).balance}("");
    if (!_success) revert FailedToSendEther();
  }
  ...
  function tokenURI(uint256 _id) public view override(ERC721) returns (string memory) {
    // Compute the frequency from the metadata.
    TokenParams memory _metadata = metadata[_id];
    uint256 _frequency = computeFrequency(_metadata.basefee, _metadata.minter, _metadata.blocknum);

    // Convert metadata into strings.
    string memory _sId = _id.toString();
    string memory _sName = string(abi.encodePacked("Basehunter #", _sId));
    string memory _sBasefee = uint256(_metadata.basefee).toString();
    string memory _sFrequency = uint256(_frequency).toString();
    string memory _sDescription = string(
      abi.encodePacked(
        _sName,
        " was generated by ",       uint160(_metadata.minter).toHexString(), // Truncates leading zeroes, but ¯\_(ツ)_/¯
        " at block ",               uint256(_metadata.blocknum).toString(),
        " with a base fee of ",     _sBasefee,
        " wei and a frequency of ", _sFrequency, " / 10^18 Hz"
      )
    );
          ...
          Base64.encode(
            bytes(
              abi.encodePacked(
                ...
                '", "external_url":"https://basehunter.art/', _sId,
                '", "image":"data:image/svg+xml;base64,', Base64.encode(bytes(generateSVG(_frequency))),
                '"}'
              )
            )
          )
  ...
  /// @dev Takes a signed integer and converts it to a string.
  function signedIntToString(int256 x) internal pure returns (string memory) {
    return x >= 0 ? uint256(x).toString() : string(abi.encodePacked("-", uint256(-1 * x).toString()));
  }

  /// @dev Takes an integer frequency and returns the plot path for the SVG.
  /// @dev Generates two sine waves that are offset from each other and SVG path points alternate
  /// between each wave.
  function generateSVGSineWavePath(uint256 _frequency) internal pure returns (string memory) {
    // SVG canvas is 800x800.
    // x values range from 0 to 800.
    // y values start from 400 (middle of canvas) and are adjusted by output of sine.

    // Generate a phase shift to shift second sine wave by some amount between 0 and 2pi.
    uint256 _shift = uint256(keccak256(abi.encode(_frequency))) % (2 * Trigonometry.PI);

    // Compute points and return SVG path. 200 and 7200 are just nice scale factors for the canvas size.
    string memory _path = "M";
    for (uint256 x = 0; x <= 800; x += 4) {
      int256 y1 = 200 * Trigonometry.sin( _frequency *  x      / 7200);
      int256 y2 = 200 * Trigonometry.sin((_frequency * (x + 2) / 7200) + _shift);

      string memory yInt1  = signedIntToString((400e18 + y1) / 1e18);
      string memory yInt2  = signedIntToString((400e18 + y2) / 1e18);
      string memory yFrac1 = signedIntToString((400e18 + y1) % 1e18);
      string memory yFrac2 = signedIntToString((400e18 + y2) % 1e18);

      string memory x1 = x.toString();
      string memory x2 = uint256(x + 2).toString();
      _path = string(abi.encodePacked(_path, " ", x1, " ", yInt1, ".", yFrac1, " ", x2, " ", yInt2, ".", yFrac2));
    }
    return _path;
  }

  /// @dev Main function that generates the SVG.
  function generateSVG(uint256 _frequency) internal pure returns (string memory) {
    return
      string(
        abi.encodePacked(
          '<svg xmlns="http://www.w3.org/2000/svg" width="800" height="800" style="background-color:#A39E93">',
            '<defs>',
              '<radialGradient id="rgrad" cx="50%" cy="50%" r="80%">',
                '<stop offset="0%" style="stop-color:#000000;stop-opacity:1.00"/>',
                '<stop offset="100%" style="stop-color:#423D33;stop-opacity:1.00"/>',
              '</radialGradient>',
            '</defs>',
            '<ellipse cx="50%" cy="50%" rx="100%" ry="100%" style="fill:url(#rgrad)"/>',
            '<path fill="none" id="wave" stroke="#A39E93" stroke-width="1" d="', generateSVGSineWavePath(_frequency), '"/>'
          '</svg>'
        )
      );
  }
  ...
  /// @dev Returns "base frequency", i.e. direct mapping from base fee to frequency. The frequency is returned as
  /// an integer scaled by 1e18.
  ///
  /// @dev Reference frequencies are 41.20 Hz (an E1 note) at 3000 gwei and 329.63 Hz (an E4 note) at 1 gwei. Given:
  ///    - An A4 is 440 Hz.
  ///    - There are 12 semitones in an octave.
  ///    - The frequency ratio between semitones is 2^(1/12).
  ///    - An E4 is five semitones below an A5.
  ///
  /// Then we find:
  ///     E4 = 440 * 2^( -5         / 12) = 329.63 Hz => 329.63 Hz * 1e18 = 329627556912869929735
  ///     E1 = 440 * 2^((-5 - 12*3) / 12) =  41.20 Hz =>  41.20 Hz * 1e18 =  41203444614108741217
  ///
  /// Linear interpolation between the two points of (3000, 41203444614108741217) and (1, 329627556912869929735)
  /// gives us the frequency as a function of base fee in gwei:
  ///     frequency = 329723730341445708724 - 96173428575778989 * basefeeGwei
  ///
  /// @dev Base fee reference frequencies are based on the minimum and maximum base fees seen so far. At this time,
  /// the lowest base fee is 1.12 gwei and the highest is 3940 gwei. So using 1 gwei and 3000 gwei as the bounds
  /// seems good to ensure the deepest frequency (highest base fee) is attainable.
  ///   - L1 minimum base fees: https://dune.xyz/queries/243397
  ///   - L1 maximum base fees: https://dune.xyz/queries/243577
  function computeBaseFrequency(uint256 _basefee) internal pure returns (uint256) {
    // Cap effective base fee at our reference points to (1) prevent overflow and (2) prevent inaudible tones.
    uint256 _basefeeGwei = _basefee / 1e9;
    if (_basefee > 3000 * 1e9) _basefeeGwei = 3000;
    if (_basefee < 1 * 1e9) _basefeeGwei = 1;

    // Using reference frequencies described above, we linearly interpolate to find the base frequency.
    return 329723730341445708724 - 96173428575778989 * _basefeeGwei;
  }

  /// @dev Returns token's frequency as fixed point number scaled by 1e18
  function computeFrequency(uint256 _basefee, address _minter, uint256 _block) internal pure returns (uint256) {
    uint256 _baseFrequency = computeBaseFrequency(_basefee); // Base frequency is the core audio component.
    uint256 _noise = uint32(uint256(keccak256(abi.encode(_minter, _block)))); // Noise adds some uniqueness based on mint params (uint32 to ensure it's not a dominant factor).
    bool _addNoise = uint256(keccak256(abi.encode(_noise))) % 2 == 0; // Determine if noise adds or substracts to frequency.
    return _addNoise ? _baseFrequency + _noise : _baseFrequency - _noise; // Final frequency for this token.
  }
  ...
  function withdrawTokens(address _token, address _to, uint256 _amount) external {
    if (msg.sender != creator) revert Unauthorized();
    (bool ok, ) = _token.call(abi.encodeWithSelector(0xa9059cbb, _to, _amount));
