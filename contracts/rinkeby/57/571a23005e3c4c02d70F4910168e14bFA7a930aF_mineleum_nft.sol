...
contract mineleum_nft is ERC721Enumerable, Ownable {
    ...
    //送料を変える
    function setSendCost(bool edit,uint256 _newCost) public onlyOwner returns(uint256){
        if(edit){
            sendCost = _newCost;
        }
        return sendCost;
    ...
    //mint値段を変える
    function setMintCost(bool edit,uint256 _newCost) public onlyOwner returns(uint256){
        if(edit){
            mintCost = _newCost;
        }
        return mintCost;
    ...
    //レートを変える
    function setSellRate(uint256 _rate) public onlyOwner returns(uint256){
        //nomal 100
        if(_rate > 0){
            sellRate = _rate;
        }
    ...
    //一度に最大mint数を変える
    function setmaxMintAmount(uint256 _newmaxMintAmount) public onlyOwner returns(uint256){
        if(_newmaxMintAmount > 0){
            maxMintAmount = _newmaxMintAmount;
        }
        return maxMintAmount;
    }
    //最大供給数を変える
    function setmaxSuplly(uint256 _newmaxSupply) public onlyOwner returns(uint256){
        if(_newmaxSupply > 0){
