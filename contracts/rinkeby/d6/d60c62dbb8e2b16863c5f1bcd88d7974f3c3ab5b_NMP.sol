...
contract NMP is ERC721, Ownable {
  ...
  constructor() ERC721("NFTMAP", "NM") {
  }

  function mint(uint tokenId, uint x1, uint y1, uint x2, uint y2) external payable {
    require(msg.value >= calculatePixelPrice(tokenId, x1, y1, x2, y2), "Value below price");
    ...
    areaPerTokens[tokenId] = Area(x1, y1, x2, y2);
  ...
  function calculatePixelPrice(uint tokenId, uint x1, uint y1, uint x2, uint y2) public view returns(uint){
    return (purchaseOfTokenIdCounter[tokenId] + 1) * tokenPrice * (x1>=x2 ? x1-x2 : x2 - x1) * (y1 >= y2 ? y1-y2 : y2-y1);
  }
  ...
  function withdrawBalance() external onlyOwner {
    ...
    uint creatorBalance = totalBalance / 20; // 5% of total sale
    uint withdrawerBalance = totalBalance - creatorBalance; // 95% of total sale

    (bool s,) = payable(owner()).call{value: creatorBalance}("");
    (bool t,) = payable(msg.sender).call{value: withdrawerBalance}("");
    require(s, "tx failed");
    require(t, "tx failed");
  }
}