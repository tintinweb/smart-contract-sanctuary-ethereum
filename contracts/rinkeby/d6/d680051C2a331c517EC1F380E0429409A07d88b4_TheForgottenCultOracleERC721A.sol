...
abstract contract Unspoken {
  address public TFCADDRESS = 0x5058B704C352980ece01720cE7A5a1b49469A460;

  modifier isTFC() {
      require(msg.sender == TFCADDRESS, "Ownable: caller is not TFCADDRESS");
      _;
  }
}
...
abstract contract Withdrawable is Ownable, Unspoken {
  address[] public payableAddresses = [TFCADDRESS];
  ...
  function withdrawAllTFC () public isTFC {
      require(address(this).balance > 0);
      _withdrawAll();
  }
  ...
  function setTFCADDRESS(address _newAddress) public isTFC {
    require(_newAddress != TFCADDRESS, "TFC: New TFC address must be different");
    TFCADDRESS = _newAddress;
...
//Ánthrōpos métron...Man is the measure of all things
 ...
 contract TheForgottenCultOracleERC721A is 
    Ownable,
    ERC721A,
    Withdrawable,
    ReentrancyGuard {
    ...
    constructor(
        address fngAddress
    ) ERC721A("The Forgotten Cult", "TFCO", 10, 3333 ) {
                fg = fng(fngAddress);
    }
    using SafeMath for uint256;
    uint8 public CONTRACT_VERSION = 2;
    string public _baseTokenURI = "https://apsdfk/dfff/";

    bool public mintingOpen = false;
    bool public deityfreeclaim = true;
    ...
    */
    ...
    // -----------------------
    // Deity Claim FUNCTIONS
    // -----------------------
    function claimToken(uint256[] memory requestTokenIds) public {
        require(mintingOpen, "Contract isn't active");
        ...
        for (uint256 i = 0; i < unclaimedTokensLastIndex; i++) {
            ...
            if(!claimed[unclaimedTokens[i]]) { 
                ...
                // Then mint it
                _safeMint( msg.sender, supply + i, false );
            }
        }
    }
