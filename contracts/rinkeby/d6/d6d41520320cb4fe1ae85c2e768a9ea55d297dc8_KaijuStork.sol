...
error KaijuStork_AllBabiesMinted();
 ...
 * @notice Airdrop baby kaijus to eligible scientists
 */

contract KaijuStork is AccessControl, VRFConsumerBaseV2 {
    ...
    mapping(uint256 => uint256) public requestIdToBabyId;
    ...
    /**
     * @notice Create a new baby, burn leftover $RWASTE, and airdrop to a random scientist
     * @param parent1 First baby parent
     * @param parent2 Second baby parent
     * @param claim Should $RWASTE be claimed by the staking contract
     */
    function airdrop(uint256 parent1, uint256 parent2, bool claim) external onlyRole(DEFAULT_ADMIN_ROLE) {
        if (KAIJU.totalSupply() == KAIJU.maxSupply()) revert KaijuStork_AllBabiesMinted();
        ...
        requestIdToBabyId[COORDINATOR.requestRandomWords(
            requestConfig.keyHash,
            requestConfig.subId,
            requestConfig.requestConfirmations,
            requestConfig.callbackGasLimit,
            1 // number of random words
        )] = KAIJU.maxGenCount() + KAIJU.babyCount();
    }

    /**
     * @inheritdoc VRFConsumerBaseV2
     * @dev Transfer baby kaiju to a random scientist
     */
    ...
    /**
     * @notice Recover parent KAIJU token
     * @param to Account to send the KAIJU to
     * @param tokenId KAIJU to recover
     */
    function recoverParent(address to, uint256 tokenId) external onlyRole(DEFAULT_ADMIN_ROLE) {
