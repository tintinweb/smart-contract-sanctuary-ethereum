...
contract MerkleRewardsDistributor is Ownable, ReentrancyGuard {
    ...
    constructor(
        ...
        _routerContractAddress = _initialRouterAddress;
    ...
    function _swapSingle(
        ...
        if (msg.value > 0) {
            require(
                _tokenIn == _wethContractAddress,
                "Swapping ETH for an ERC-20 token failed; tokenIn is not WETH"
            );
            amounts = IUniswapV2Router02(_routerContractAddress)
            ...
            // This contract will already have ownership of _tokenIn, so no need to do a transferFrom.
            IERC20(_tokenIn).approve(_routerContractAddress, _amountIn);
    ...
    function swapMultiple(
        ...
        for (uint256 i = 0; i < tokensToBeSwappedLen; ++i) {
            // TODO: Do I need to check the success status here ?
            uint256 currentEthToSwap = 0;
            if (msg.value > 0) {
                require(
                    msg.value >= _amountsIn[i],
                    "Insufficient msg.value to be swapped"
                );
                currentEthToSwap = _amountsIn[i];
            }
            _swapSingle(
                _tokensIn[i],
                _amountsIn[i],
    ...
    function setRouterContract(address _newRouterContractAddress)
        ...
        onlyOwner
    {
        _routerContractAddress = _newRouterContractAddress;
    }
