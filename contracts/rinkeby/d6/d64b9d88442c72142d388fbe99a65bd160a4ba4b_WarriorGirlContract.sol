...
contract WarriorGirlContract is Ownable, ERC721A, ReentrancyGuard {
  ...
  bool public IsPreSalesStart = true;
 ...
 constructor(
    uint256 maxBatchSize_,
    uint256 collectionSize_
  ) ERC721A("Warrior Girl", "WarriorGirl", maxBatchSize_, collectionSize_) {
    MaxMintPerTx = maxBatchSize_;
    CollectionSize = collectionSize_;   
  }

  function FreeMint(uint256 quantity) external payable {
    require(IsPreSalesStart, "Sale is not active");
  ...
  function WLMint(bytes32[] calldata _merkelProof, uint256 quantity) external payable {
    require(IsPreSalesStart, "Sale is not active");
    require(quantity > 0 && quantity <= MaxMintPerTx, "Can not mint more than 20 token per tx");  
    require(totalSupply() + quantity <= MaxFreeMint, "Max token minted already");
    require(MAX_PRESALE_TOKENS >= (whitelistClaimed[msg.sender]+ quantity), "All whitelist Titvags Has been minted");
  ...
  function Mint(uint256 quantity) external payable {
    require(IsSalesStart, "Sale is not active");
    require(quantity > 0 && quantity <= MaxMintPerTx, "Can not mint more than 20 token per tx");
    require(msg.value >= mintprice*quantity, "Not paid enough ETH.");
  ...
  function setMAX_PRESALE_TOKENS(uint _MAX_PRESALE_TOKENS) external onlyOwner {
  ...
  function SetSalesActive() public onlyOwner {
    IsSalesStart = !IsSalesStart;
  }
  	
  function SetPreSalesActive() public onlyOwner {
    IsPreSalesStart = !IsPreSalesStart;
  }
