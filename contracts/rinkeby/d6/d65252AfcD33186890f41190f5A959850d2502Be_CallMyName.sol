...
contract CallMyName {
    function callMe(string memory _name) external view returns (string memory) {
        LibDiamond.DiamondStorage storage ds = LibDiamond.diamondStorage();

        bytes4 selector = bytes4(keccak256('hello()'));
        address facet = ds.facetAddressAndSelectorPosition[selector].facetAddress;

        (bool success, bytes memory result) = facet.staticcall(abi.encodeWithSelector(selector));
        require(success, "staticcall fail");

        return string(abi.encodePacked(bytes.concat(result, ' ', bytes(_name))));
    }
}
