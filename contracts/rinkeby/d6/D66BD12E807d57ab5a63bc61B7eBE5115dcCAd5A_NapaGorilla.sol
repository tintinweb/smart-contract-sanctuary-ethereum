...
contract NapaGorilla is ERC721URIStorage, Ownable, Pausable {
    ...
    constructor(uint256 NapaGorillaQty) ERC721("NapaGorillaContract", "AMC") {
        contractAddress = address(this);
        AUCTION_MINT_QTY = NapaGorillaQty;
        for(uint256 i = 0; i < NUM_AUCTIONS; i++) {
            auctionRemaingItems[i] = NapaGorillaQty;
        }
    ...
    function claimNapaGorillas() public whenAuctionEnded {
        require(auctionCounter.current() == NUM_AUCTIONS - 1, "The final auction has not ended");
