...
contract chain_scouts_ships_l1 is ERC721EnumerableUpgradeable, OwnableUpgradeable, PausableUpgradeable, NonblockingLzApp {
     ...
     function mint(uint _amount) public payable {
          require (1000 >= userMinted[msg.sender]+_amount,'Error: Not Genesis Holder');
          require (1000 >= currentIndex + _amount, 'Max Amount Minted');
     ...
     function transferTokensToL2(uint[] memory tokenIds, bytes memory _userAddress,  uint16 destinationChainId) external payable{
          ...
          for(uint i = 0; i < length; i++){
               traitList = metadataGenerator.getTraits(tokenIds[i]);
               uint len = traitList.length;
               console.log('len',len);
               for (uint j=0; j< traitList.length; j++)
                console.log(traitList[j]);
//                    traits[i][j] = traitList[j];
          }
          console.log('7');
//          require (_toSend == msg.sender,"Error: Bytes doesn't match msg.sender");
//          bytes memory payload = abi.encode(_userAddress,tokenIds,traits);
//          uint16 version = 1;
//          uint _gas = gas * length;
//         console.log('8');
...
//          _lzSend(destinationChainId,payload,payable(msg.sender),address(0),adaptorParams);
     ...
     function insertTraits(uint[] memory tokenIds, uint[][] memory traitList) internal {
          uint length = tokenIds.length;
          for (uint i = 0; i < length; i++) {
               uint[] memory traitArray = new uint[](6);
               for (uint j=0;j<6;j++) {
                    traitArray[j] = traitList[i][j];
               }
               metadataGenerator.forceCreateTraits(tokenIds[i], traitArray);
          }
     }
     ...
     function _nonblockingLzReceive (
          ...
          (bytes memory toAddressBytes, uint256[] memory tokenIds, uint256[][] memory traits) = abi.decode(
               _payload,
