...
contract MyTokenWithPresale is ERC20, Ownable {
    ...
    uint256 public immutable maxPercentagePerOwner = 4;
    ...
    function getMyPercentage() external view returns(uint256){
        return balanceOf(msg.sender)*100/finalTotalSupply;
    }

    function getUserPercentage(address addr) external view returns(uint256){
        return balanceOf(addr)*100/finalTotalSupply;
    }
    function _beforeTokenTransfer(
        ...
        if(to!=owner()){
            uint256 newPercentage = (balanceOf(to)+amount)*100/finalTotalSupply;
            require(newPercentage<=maxPercentagePerOwner, "");
        }
    }
