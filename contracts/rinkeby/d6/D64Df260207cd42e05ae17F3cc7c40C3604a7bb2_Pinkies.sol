...
contract Pinkies is ERC721Enumerable, Ownable {
    using Strings for uint256;
    ...
    function mint(uint256 _numberOfTokens) public payable {
        ...
        require(_numberOfTokens <= maxTokensPurchase);
        require(supply + _numberOfTokens <= maxSupply);

        if (msg.sender != owner()) {
            require(msg.value >= cost * _numberOfTokens);
        }
