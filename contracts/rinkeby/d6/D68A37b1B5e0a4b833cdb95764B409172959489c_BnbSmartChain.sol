...
contract ContractData is Authorized {
  ...
  function passiveEarning(address _address) public view returns(uint) {
    ...
    uint available = accountsEarnings[_address].receivedPassiveAmount + accountsEarnings[_address].withdrawPassiveAmount + accountsEarnings[_address].receivedDirectBonusAmount + accountsEarnings[_address].withdrawDirectBonusAmount + accountsEarnings[_address].receivedLevelBonusAmount + accountsEarnings[_address].withdrawLevelBonusAmount;  
    uint maximunReceived = maximunEarning(_address);

    uint pasiveAvailable = ((((dailyRentability  * accountsEarnings[_address].depositValue) / 1000) /  1 minutes) * secondsRunning);

    if ((available + pasiveAvailable) > maximunReceived) {
      pasiveAvailable = (maximunReceived - available) + accountsEarnings[_address].receivedPassiveAmount;
    }

    return pasiveAvailable + accountsEarnings[_address].receivedPassiveAmount;
  }
