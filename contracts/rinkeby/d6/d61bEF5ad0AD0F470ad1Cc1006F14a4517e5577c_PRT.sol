...
contract PRT is Ownable, ReentrancyGuard {
    ...
    event RTWinnerTokenID(uint index, uint winnerTokenPRTID);
    ...
    function sendMP() public payable onlyAccounts onlyOwner presalePRTIsDone {
        uint xrand = createXRAND(17);
         ...
         for (uint i = 0; i < 10000; i++) {
            uint24 _winnerTokenPRTID = uint24(PRTID + 1 + xrand + uint32(168888*i)/10000);
            address winneraddr = getAddrFromPRTID(_winnerTokenPRTID);
            if (winneraddr != address(0)) {
                // console.log('!', winneraddr);
                emit RTWinnerAddress(winneraddr, _winnerTokenPRTID);
            }
            emit RTWinnerTokenID(i, _winnerTokenPRTID);//in case to track all winer tickets in logs
