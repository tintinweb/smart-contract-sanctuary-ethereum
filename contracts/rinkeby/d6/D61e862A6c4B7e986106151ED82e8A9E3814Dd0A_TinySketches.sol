...
contract TinySketches is iTinySketches, ERC721, ReentrancyGuard, Ownable {
  ...
  function mintableLastTokenId() public view returns (uint256) {
    ...
    if (t < _startDate) {
      revert("TinySketches: no tokens are on sale yet.");
    } else if (t < _startDate + A_WEEK) {
      return 59;
    } else if (t < _startDate + A_WEEK * 2) {
      return 119;
    } else if (t < _startDate + A_WEEK * 3) {
      return 179;
    } else {
      return 239;
    }
