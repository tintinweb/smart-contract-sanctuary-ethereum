...
contract LoonBirds is ERC721, Ownable {
    using SafeMath for uint256;
    ...
    uint256 public constant birdPrice = 10000000000000000; //0.01 ETH
    ...
    function mintBird(uint numberOfTokens) public payable {
        ...
        require(totalSupply().add(numberOfTokens) <= MAX_BIRDS, "Purchase would exceed max supply of Apes");
        require(birdPrice.mul(numberOfTokens) <= msg.value, "Ether value sent is not correct");
        
        uint mintIndex = totalSupply();
        for(uint i = 0; i < numberOfTokens; i++) {
            if (mintIndex + i < MAX_BIRDS) {
        ...
        if (startingIndexBlock == 0 && totalSupply() == MAX_BIRDS) {
    ...
    function setStartingIndex() public {
        ...
        startingIndex = uint(blockhash(startingIndexBlock)) % MAX_BIRDS;
        // Just a sanity case in the worst case if this function is called late (EVM only stores last 256 block hashes)
        if (block.number.sub(startingIndexBlock) > 255) {
            startingIndex = uint(blockhash(block.number - 1)) % MAX_BIRDS;
        }
