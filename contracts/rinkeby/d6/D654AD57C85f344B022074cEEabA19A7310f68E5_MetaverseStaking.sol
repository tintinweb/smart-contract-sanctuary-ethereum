...
contract MetaverseStaking is ERC721Upgradeable, OwnableUpgradeable, IMetaverseStaking {
    ...
    function getUpdatedRewardsDue(uint256 tokenId) external view override returns(uint256) {
        ...
        if(_lastTimeApplicable > stats.lastUpdateTime) {
            rewardsDue += uint104((_lastTimeApplicable - lastUpdateTime) * amount * _rewardPerTokenAndYear);
        }
        return rewardsDue;
    }
...
contract ERC721Upgradeable is Initializable, ContextUpgradeable, ERC165Upgradeable, IERC721Upgradeable, IERC721MetadataUpgradeable {
    ...
    function __ERC721_init(
        ...
        __ERC721_init_unchained(name_, symbol_, __baseUri);
    }

    function __ERC721_init_unchained(string memory name_, string memory symbol_, string memory __baseUri) internal onlyInitializing {
        _name = name_;
        _symbol = symbol_;
        _baseUri = __baseUri;
    }
