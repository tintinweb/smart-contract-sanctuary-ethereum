...
contract NFTLauncher is VRFConsumerBase, ChainlinkClient, Ownable {
    ...
    function doTheStuff(
        ...
        if (!keep) {
            ...
            token.transfer(owner(), feeAmountForSeapad);
        }
        ...
        if (!isBasePayment) {
            ...
            token.transfer(projectOwner, remainingAmountForOwner);
        }
    }
    ...
    function swapAndTransferTo(
        ...
        if (swapAddressTo == uniswapV2Router.WETH()) {
            ...
            //if this is a weth transferFrom, we need to swap the amount from weth to the token
            if (swapAddressFrom == uniswapV2Router.WETH()) {
