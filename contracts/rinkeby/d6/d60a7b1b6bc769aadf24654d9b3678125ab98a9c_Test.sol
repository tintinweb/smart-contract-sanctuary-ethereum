...
contract Test is Ownable, ReentrancyGuard, ERC721A, ERC721AQueryable {
    ...
    constructor(string memory _inputURI, bytes32 _wlRoot, bytes32 _alRoot) ERC721A("TEST", "TEST")
    {
    ...
    function resMint(bytes32[] calldata proof) external mintCompliance {
        ...
        if (!MerkleProof.verify(proof, _wlMerkleRoot, _leaf(msg.sender))) {
            if(!MerkleProof.verify(proof, _alMerkleRoot, _leaf(msg.sender))) revert("Not on wl/al");
        }
   ...
   function _startTokenId() internal view virtual override returns (uint256) {
    ...
    function updateState(bool  _wlActive, bool  _alActive, bool  _resActive) external onlyOwner {
        wlActive = _wlActive;
        alActive = _alActive;
