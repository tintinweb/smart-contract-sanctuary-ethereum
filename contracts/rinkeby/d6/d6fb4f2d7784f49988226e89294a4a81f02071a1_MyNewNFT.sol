...
contract MyNewNFT is Ownable, ERC721A, ReentrancyGuard {
  ...
  uint256 public immutable maxPerAddressPresale;
  ...
  constructor() ERC721A("MyNewNFT", "MNN") {
    maxPerAddressPresale = 3;
    maxPerAddressPublic = 2;
    amountForDevs = 50;
  }
  ...
  function presaleMint(uint256 quantity, bytes memory sig)
    external
    payable
    callerIsUser
  {
    require(msg.value >= mintPrice * quantity, "Need to send more ETH.");
    require(verify(sig, msg.sender), "Not verified");
    require(mintedByUser[msg.sender] + quantity <= maxPerAddressPresale);
