...
contract FullOnchainTest2 is ERC721Enumerable, Ownable {
    ...
    function setParts(string memory _string) public onlyOwner {
        require(bytes(parts22).length == 0, "Data can be set once");
        parts22 = _string;
    }
    ...
    function tokenURI(uint256 tokenId) override public view returns (string memory) {
        ...
        output = string(abi.encodePacked(output, parts[17], parts[18], parts[19], parts[20], parts[21], parts22));

        string memory json = Base64.encode(bytes(string(abi.encodePacked('{"name": "#', toString(tokenId),' Original Otaku Coin', '", "description": "',description,'", "image": "data:image/svg+xml;base64,', Base64.encode(bytes(output)), '"}'))));
