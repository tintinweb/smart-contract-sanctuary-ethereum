...
contract GameStationBridge is IGameStationBridge, EIP712, Ownable {
    ...
    function applyPropose(address tokenAddress_) external onlyOwner {
        TokenInfo storage info = tokensInfo[tokenAddress_];
        require(block.timestamp > info.proposeTime, "Lock up period");
    ...
    /// @param digest_ Signature parameter
    /// @param v_ Signature parameter
    /// @param r_ Signature parameter
    /// @param s_ Signature parameter
...
/// @title IGameStationBridge
/// @author Applicature
/// @notice There is an interface for GameStationBridge Smart Contract that provides transfering
/// coins/erc20 between different blockhains on base ethereum evm
/// @dev There are provided all events and function prototypes for GameStationBridge SC
interface IGameStationBridge {
    ...
    /// @param isETHChain Bool whether ETH is
