...
contract OmcEpoch {
    ...
    address public constant uniswapFactoryV2 = 0x5C69bEe701ef814a2B6a3EDD4B1652CB9cc5aA6f;
    ...
    function setOmcDistributor(address _newOmcDistributor) external onlyOwner{
    ...
    function setMiner(address _newMiner) external onlyOwner {
        miner = _newMiner;
    }

    function distribute() external onlyMiner {
        require(receiver != address(0), "RECEIVER NOT SET");
        require(lastDistributeBlockHeight + 2880 <= block.number, "TOO FREQUENT DISTRIBUTE");
        ...
        OmcDistributor(omcDistributor).compound(epochNum, epochReward, totalSupply);
    ...
    function swapTokenForWETH(address _token) external onlyMiner {
        uint256 amountIn = IERC20(_token).balanceOf(address(this));
        require(amountIn > 0, "NO BALANCE");

        address pool = IUniswapV2Factory(uniswapFactoryV2).getPair(_token, WETH);
        ...
        IUniswapV2Router02(uniswapRouterV2).swapExactTokensForTokens(amountIn, 0, tokenList, address(this), block.timestamp + 10);
...
contract OmcDistributor is ReentrancyGuard {
    ...
    event Compound(uint256 _epochNum, uint256 _totalRewardAmount, uint256 _rewardPerUser, uint256 _totalMinerCount);
    event RewardWithdrawal(uint256 _tokenId, uint256 _tokenEpoch, uint256 _reward);
    ...
    function compound(uint256 _epochNum, uint256 _totalRewardAmount, uint256 _totalSupply) external onlyEpoch {
        omcEpochNum = _epochNum;
        epochReward[omcEpochNum] = _totalRewardAmount / _totalSupply;
        epochTotalSupply[omcEpochNum] = _totalSupply;

        emit Compound(_epochNum, _totalRewardAmount, epochReward[omcEpochNum], _totalSupply);
    }
    ...
    function pendingReward(uint256 tokenId) external view returns (uint256) {
        ...
        uint256 reward;
        for(uint256 index = lastEpochPerTokenId[tokenId]; index <= omcEpochNum; index++) {
    ...
    function withdrawReward(uint256 tokenId) public nonReentrant { 
        ...
        uint256 reward;
        uint256 index;
        for(index = lastEpochPerTokenId[tokenId]; index <= omcEpochNum; index++) {
            if(epochTotalSupply[index] <= tokenId) continue;
        ...
        if(reward > 0)
            OMCLib._safeTransfer(WETH, msg.sender, reward);
...
contract Omc is ERC721 {
    ...
    function setOmcEpoch(address _newOmcEpoch) external onlyOwner {
    ...
    function setHiddenURI(string memory _newBaseHiddenURI) external onlyOwner {
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        ...
        if(revealed == false) {
            ...
            return bytes(currentNotRevealedUri).length > 0
                ? string(abi.encodePacked(currentNotRevealedUri, Strings.toString(tokenId), ".json"))
                : "";
        }
    ...
    function setSale(uint256 _newMintPrice,
    ...
    function publicMint(uint256 _mintCount) external payable
    {
        require(publicMintEnabled, "PUBLIC SALE NOT ENABLED");
        require(_mintCount > 0 && _mintCount <= _countLimitPerMint, "INVALID MINTCOUNT");
        ...
        require((balanceOf(msg.sender) + _mintCount) <= _mintLimitPerUser, "EXCEEDED MAX AMOUNT PER PERSON");
    ...
    function airDropMint(address user, uint256 requestedCount) external onlyOwner {
        require(requestedCount > 0, "INVALID COUNT");
