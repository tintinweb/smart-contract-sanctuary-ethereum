...
contract Crazy is Ownable, ERC721, ERC2981 {
  using Address for address;
  using SafeCast for uint256;
  using Strings for uint256;

  // EVENTS ****************************************************
    event URISet(string paperURI, string diamondURI);

  // MEMBERS ****************************************************

  uint256 public immutable MAX_SUPPLY = 5200;
  uint256 public immutable DIAMOND_PASS_SUPPLY = 700;
  uint256 public immutable PAPER_PASS_SUPPLY = 4500;

  // Number of currently supplied tokens
  uint256 public paperPassSupply = 0;
  uint256 public diamondSupply = 0;
  uint256 public totalSupply = 0;

  // Price for diamond Pass
  uint256 diamondPrice = 0.00005 ether;

  bool public saleOpen;
  bool public isURISet;
  ...
  mapping (address => uint256) public paperPassMintBalanceOf;
  ...
  {
    mintPremiumGiveaway(200);
    royaltyRegistry = _royaltyRegistry;
    _setRoyalties(_royaltyRegistry, 7900); // 7.9%
  }
  ...
  function buyFree(uint256 numberOfTokens) external {
    ...
    require(paperPassMintBalanceOf[msg.sender] + numberOfTokens < 6, "Exceeds paper pass mint limit of 5, per wallet");
    require(numberOfTokens > 0, "Should mint atleast 1 paper pass");
    require(paperPassSupply + numberOfTokens <= PAPER_PASS_SUPPLY, "Paper pass supply maxed out");
    ...
    paperPassMintBalanceOf[msg.sender] += numberOfTokens;

    // Set edition to paper pass, gas saving approach by setting the first token edition only
    edition[totalSupply+1] = 1;
    paperPassSupply += numberOfTokens;
  ...
  function getEdition(uint tokenId) public view returns(uint256){
    if(tokenId > totalSupply) revert("Non-existent token");

    uint256 curr = tokenId;
    while(true){
      if(edition[tokenId] == 0) curr--;
  ...
  */
  function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
    ...
    return (getEdition(tokenId) == 1) ? paperURI : diamondURI;
  }
  ...
  function mintPremiumGiveaway(uint256 numberOfTokens) internal {
    ...
    require(diamondSupply + numberOfTokens <= DIAMOND_PASS_SUPPLY, "Diamond pass supply maxed out");
  ...
  function setURIs(string memory _paperURI, string memory _diamondURI) external onlyOwner{
    if(isURISet) revert("URIs already been set");
    isURISet = true;
    paperURI = _paperURI;
    diamondURI = _diamondURI;
    emit URISet(_paperURI, _diamondURI);
  }
