...
contract Shikigai is ERC721A, Ownable {
    ...
    modifier ValidMerkleProof(bytes32[] calldata _proof) {
        bytes32 leaf = keccak256(abi.encodePacked(msg.sender));
        if (!MerkleProof.verify(_proof, _merkleRoot, leaf)) revert NotAllowlisted();
        _;
    }

    //mint functions
    function publicMint(uint48 _inputConfig) 
        external
        payable
        CallerIsUser
    {
