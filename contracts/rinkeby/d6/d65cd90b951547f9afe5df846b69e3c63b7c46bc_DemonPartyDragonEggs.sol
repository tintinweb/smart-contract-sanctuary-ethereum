...
// File: contracts/DemonPartyDragonEggs.sol
...
contract DemonPartyDragonEggs is ERC721, Ownable {
  ...
  constructor(string memory _baseURI) ERC721("DemonPartyDragonEggs", "DRAGONEGGS") {
    baseURI = _baseURI;
  }

  function mint() public payable {
    require(owners.length + 1 <= MAX_SUPPLY, "TOO_MANY_MINTS");

    minters[msg.sender] += 1;
    require( (minters[msg.sender] ) <= 1, "ADDRESS_MAX_REACHED");
  ...
  uint256 public giftedCount = 0;
  uint256 public constant MAX_GIFTS = 10;
  function gift(address to) public onlyOwner {
    require(giftedCount < MAX_GIFTS);
