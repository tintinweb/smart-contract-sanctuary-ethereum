...
import "../base/ShrineBase.sol";
import "../interface/IChamberController.sol";


contract ShrineHealth is ShrineBase {
  ...
  string internal constant _CHAMBER_NAME = "Chamber with Health Shrine";
  uint internal constant _CHAMBER_TYPE = uint(IChamberController.ChamberType.SHRINE);
  bytes32 internal constant _CHAMBER_LEVEL_SLOT = bytes32(uint256(keccak256("eip1967.Shrine.ChamberLevel")) - 1);

  // ---- INITIALIZER ----

  function init(address controller_, uint level_) external initializer {
    _CHAMBER_LEVEL_SLOT.set(level_);
    IStatController.Attributes memory emptyA = IStatController.Attributes({
    strength : 0,
    dexterity : 0,
    vitality : 0,
    energy : 0,
...
abstract contract ShrineBase is ChamberBase {

  // ---- CONSTANTS ----

  /// @notice Version of the contract
  /// @dev Should be incremented when contract changed
  string public constant SHRINE_BASE_VERSION = "1.0.0";

  // ---- VARIABLES ----

  IStatController.Attributes _shrinePositiveAttributes;
  IStatController.Attributes _shrineNegativeAttributes;
  IStatController.ChangeableStats _shrinePositiveStats;
  IStatController.ChangeableStats _shrineNegativeStats;

  // ---- INITIALIZER ----

  function __ShrineBase_init(
    address controller_,
    IStatController.Attributes memory shrinePositiveAttributes_,
    IStatController.Attributes memory shrineNegativeAttributes_,
    IStatController.ChangeableStats memory shrinePositiveStats_,
    IStatController.ChangeableStats memory shrineNegativeStats_
  ) internal onlyInitializing {
    __ChamberBase_init(controller_);

    _shrinePositiveAttributes = shrinePositiveAttributes_;
    _shrineNegativeAttributes = shrineNegativeAttributes_;
    _shrinePositiveStats = shrinePositiveStats_;
    _shrineNegativeStats = shrineNegativeStats_;
  }
  ...
  function _action(
    ...
    if (accept) {
      ...
      // we don't have mechanic for decreasing attributes (yet)

      if (
        _shrinePositiveAttributes.strength != 0
        || _shrinePositiveAttributes.dexterity != 0
        || _shrinePositiveAttributes.vitality != 0
        || _shrinePositiveAttributes.energy != 0
        || _shrinePositiveAttributes.damageMin != 0
        || _shrinePositiveAttributes.damageMax != 0
        || _shrinePositiveAttributes.attackRating != 0
        || _shrinePositiveAttributes.defense != 0
        || _shrinePositiveAttributes.blockRating != 0
        || _shrinePositiveAttributes.life != 0
        || _shrinePositiveAttributes.mana != 0
        || _shrinePositiveAttributes.fireResistance != 0
        || _shrinePositiveAttributes.coldResistance != 0
        || _shrinePositiveAttributes.lightningResistance != 0
      ) {
        _statController().changeTemporallyAttributes(heroToken, heroTokenId, _shrinePositiveAttributes, true);
      }
      ...
      kill : false,
      experience : _shrinePositiveStats.experience,
      heal : _shrinePositiveStats.life,
      manaRegen : _shrinePositiveStats.mana,
      damage : _shrineNegativeStats.life,
      manaConsumed : _shrineNegativeStats.mana,
      mintItems : new address[](0),
