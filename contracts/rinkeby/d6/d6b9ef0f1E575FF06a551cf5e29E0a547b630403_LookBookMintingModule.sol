...
contract LookBookMintingModule is AccessControl {
    ...
    /// @param a1TokenIds Admit One token ID(s) of tokens to mint. Must be in ascending order
    ...
    function mint(uint256[] calldata a1TokenIds, Size[] calldata sizes) external payable returns (uint256[] memory) {
        ...
        if (mintState == 1) {
            ...
            lastId = lbTokenIds[lbTokenIds.length - 1];
        ...
        // window 2
            ...
            // check a1 array for duplicates & ownership
            for (uint256 i = 0; i < a1TokenIds.length; i++) {
                require(admitOne.ownerOf(a1TokenIds[i]) == msg.sender, "doesn't own token");
                if (i != 0) require(a1TokenIds[i] > a1TokenIds[i - 1], "bad order");
            }
    ...
    /// @notice Returns remaining supply available to mint
    function supplyRemaining() public view returns (uint256) {
        if (!hasMinted && lastId > 0) {
            return lookbook.maxSupply() - 1 - lastId;
        } else if (!hasMinted) {
            return lookbook.maxSupply();
        }
        return lookbook.maxSupply() - 1 - lastId;
    }
