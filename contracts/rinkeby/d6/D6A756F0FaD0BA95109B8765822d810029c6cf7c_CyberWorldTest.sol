...
contract CyberWorldTest is ERC721, ChainlinkClient, ConfirmedOwner {
  ...
  function requestMetadata(string memory _polygonString, string memory _polygonPrice, string memory _imageCID) public returns (bytes32 requestId) {
    ...
    metadataRequest.add('get', string.concat("https://cyberworld.earth/php/addLandMetadata.php?token-id=", Strings.toString(currentTokenId.current()), "&owner-address=", Strings.toHexString(uint160(msg.sender), 20), "&polygon-string=", _polygonString, "&polygon-price=", _polygonPrice, "&image-cid=", _imageCID));
