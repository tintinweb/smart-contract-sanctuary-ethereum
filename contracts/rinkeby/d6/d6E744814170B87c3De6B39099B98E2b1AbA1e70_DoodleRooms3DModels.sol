...
contract DoodleRooms3DModels is ERC721A, IERC2981, Ownable, ReentrancyGuard {
    ...
    function getClaimableTokenIds() public view returns (uint256 [] memory) {
        uint256 balance = doodleRooms.balanceOf(msg.sender);
        uint256 [] memory tokenIds = new uint256[](balance);

        for (uint i = 0; i < balance; i++) {
            tokenIds[i] = doodleRooms.tokenOfOwnerByIndex(msg.sender, i);
        }
