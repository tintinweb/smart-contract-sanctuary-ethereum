...
contract SmartYield is ISmartYield, ReentrancyGuardUpgradeable {
     ...
     * @param _user the address of the user to claim rewards to
     */
    function claimReward(address _user) external nonReentrant onlyController {
        ...
        emit RewardsClaimed(msg.sender, _user, rewardsList, claimedAmounts);
    }
    ...
    function redeemBond(address _bond, uint256 _tokenAmount) external override nonReentrant defaultCheck {
        ...
        uint256 _totalRedeem = _redeem(_bond, _tokenAmount);
    ...
    function rolloverBond(address _bond, uint256 _tokenAmount) external override nonReentrant defaultCheck {
        ...
        uint256 claimable = _redeem(_bond, _tokenAmount);
        IBond(_bond).burn(msg.sender, _tokenAmount);
     ...
     * @param _termLength the length of the term in seconds
     * @param _feeRate the fee rate in this term, 50 means 0.05%
    ...
    function setNextTermFor(
        ...
        IBond(_bond).initialize(underlying, _end);
     ...
     * @return _totalRedeem_ the total amount we need to withdraw from aave
     */
    function _redeem(address _bond, uint256 _tokenAmount) internal returns (uint256) {
        TermInfo memory termInfo = bondData[_bond];
        ...
        uint256 fee = ((_tokenAmount + rewards) * termInfo.feeRate) / 10000;
    ...
    function _computeHealthFactor(Debt memory _debt) public view returns (uint256, uint256) {
        ...
        DataTypes.ReserveData memory reserve = IProvider(bondProvider)._getReserveDataProvider(_debt.borrowAsset);
