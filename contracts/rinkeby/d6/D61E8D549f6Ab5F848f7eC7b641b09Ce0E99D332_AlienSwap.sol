...
// Part: ERC721ARoyalties

contract ERC721ARoyalties is Context, ERC721A, Ownable, ERC2981PerTokenRoyalties {
    ...
    */
    constructor(string memory name, string memory symbol, uint256 maxSupply, string memory baseUri,
        uint256 maxBatchSize, RoyaltyInfo memory royaltyInfo)
    ...
    function mintTo(address to, uint256 amount) public virtual {
        // Whitelist proxy contract for lazy minting.
        require(owner() == _msgSender(), "Caller is not owner or approved");

        //totalSupply() tokenIndex starts from 0
        //if maxSupply==0; means unlimited
        require(_maxSupply == 0 || totalSupply() + amount <= _maxSupply, "Mint count exceed MAX_SUPPLY!");

        _safeMint(to, amount);
    }

    /**
     * Override isApprovedForAll to whitelist user's BCA proxy accounts to enable gas-less listings.
     */
...
// Part: AlienERC721ABase

contract AlienERC721ABase is ERC721ARoyalties, ReentrancyGuard {
    using Strings for uint256;
    ...
    uint256 public stage1MintAllowed = 1000;

    // priceStage1 in [0, 2999];
    uint256 public constant priceStage1 = 5e16; //0.05 ETH
    // step2MintAllowed in [1000, 3000];
    uint256 public constant priceStage2 = 10e16; //0.1 ETH

    uint256 public constant maxMintPerAddress = 20;
    //    userAddress mintedCount
    ...
    */
    constructor(string memory name, string memory symbol, uint256 maxSupply, string memory baseUri,
        ...
        address validator_, address cfo_, uint256 mintStartTime_)
    ERC721ARoyalties(name, symbol, maxSupply, baseUri, maxBatchSize, royaltyInfo) {
    ...
    /**
       @dev override mintTo
    */
    function mintTo(address to, uint256 amount, bytes memory validatorSig) public payable nonReentrant {
        //assert block.timestamp >= mintStartTime
        require(block.timestamp >= mintStartTime, "Mint is Not start!");

        //assert amount in [1, 20]
        require(0 < amount, "mint amount must > 0!");
        require(amount <= maxMintPerAddress, "mint count exceed max allow 4");

        checkMintValidator(to, validatorSig);

        uint256 amountStage1;
        if (stage1MintAllowed > totalSupply()){
            amountStage1 = Math.min(stage1MintAllowed - totalSupply(), amount);
        } else {
            amountStage1 = 0;
        }

        uint256 amountStage2 = amount - amountStage1;
        require(amountStage1 + amountStage2 == amount);

        require(mintedCount[to] + amount <= maxMintPerAddress, "Address mint exceed max allowed");
        mintedCount[to] += amount;

        //do payment
        uint256 paymentAmount = priceStage1 * amountStage1 + priceStage2 * amountStage2;
     ...
     * @dev override mintTo
     */
    function mintTo(address to, uint256 amount) public override onlyOwner {
        require(false, "Not allowed!");
    }

    function __mintTo(address to, uint256 quantity) internal {
        //totalSupply() tokenIndex starts from 0
        require(_maxSupply == 0 || totalSupply() + quantity <= _maxSupply, "totalSupply exceed _maxSupply!");
        _safeMint(to, quantity);
    }
    ...
    function getMintedCount(address userAddress) public view returns (uint256){
        return mintedCount[userAddress];
    }

    function checkMintValidator(address mintToAddress, bytes memory validatorSig) public view {
...
contract AlienSwap is AlienERC721ABase {
    using Strings for uint256;
    ...
    */
    constructor(string memory name, string memory symbol, uint256 maxSupply, string memory baseUri,
        ...
        RoyaltyInfo memory royaltyInfo,
        address validator_, address cfo_, uint256 mintStartTime_)
    AlienERC721ABase(name, symbol, maxSupply, baseUri, maxBatchSize, royaltyInfo,
        validator_, cfo_, mintStartTime_) {
    }
}