...
contract BasicNFT is ERC721A, Ownable, ReentrancyGuard {
    ...
    uint256 public mintCost = 0.1 ether; // 0.05-0.1 sweet spot
    bytes32 public whitelistMerkleRoot = 0x0;
    uint256 public constant MINT_LIMIT_PER_WALLET = 3; // 5-10 sweet spot
    ...
    address private constant FOUNDER =
        0x9512ABBCb6C0Afe982C3356eBbCEfCCa5cAA4CaA;
    ...
    function publicMint(uint256 qty) external payable nonReentrant {
        ...
        if (mintedPerAddress[msg.sender] + qty > MINT_LIMIT_PER_WALLET)
            revert MintLimitPerWalletReached();
        if (msg.value < mintCost * qty) revert NotEnoughFundsForMinting();
        _safeMint(msg.sender, qty);
        mintedPerAddress[msg.sender] += qty;
    }
    ...
    function withdraw() external onlyOwner {
        ...
        if (balance == 0) revert NoFundsToWithdrwal();
        require(payable(FOUNDER).send(balance));
    }
    ...
    function giftNFT(address wallet, uint256 qty) external onlyOwner {
        if (totalSupply() + qty > COLLECTION_SIZE) revert NotEnoughNFTsLeft();
