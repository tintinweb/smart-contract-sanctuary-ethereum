...
contract MetakaWars is ERC721A, Ownable {
  ...
  constructor() ERC721A("MetaKa Wars", "METAKA") {}
  ...
  function mint(uint256 quantity) public payable {
    ...
    require(quantity <= nftPerAddressLimit, "max mint number per address exceeded");
    ...
    if (msg.sender != owner()) {
        if(onlyWhitelisted == true) {
            ...
            uint256 ownerMintedCount = whitelistedAddresses[msg.sender];
    ...
    emit Minted(msg.sender, cost * quantity);
  }
  ...
  function whitelistUsers(address[] memory addresses, uint256 numSlots) external onlyOwner {
    for (uint256 i = 0; i < addresses.length; i++) {
      whitelistedAddresses[addresses[i]] = numSlots;
    }
  }
}