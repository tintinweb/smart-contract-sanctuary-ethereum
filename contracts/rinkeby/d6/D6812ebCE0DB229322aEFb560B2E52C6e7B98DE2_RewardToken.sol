...
function gettokenss() external view returns (uint16 [] calldata);
...
contract RewardToken is ERC20, Ownable {
   ...
   //stakedNft []staked;
   mapping(address => stakeOwner) private stakeOwners ;
   

  // uint startTime = block.timestamp;
   
   uint public dailyReward = 5 ether ; 
  



   
address public mainAddress = 0x44e4aCA946E55e302b2A2C0BC8A972d90E681ddd; 
IMain Main = IMain(mainAddress);
 ...
 function stake(uint16 _number , uint16 [] calldata data) external{
    ...
    if(tokens > 0){
      stakeOwners[ msg.sender].rewards = calculateReward();
    }
 ...
 */
  function getFulltokenOf() external view returns(uint16 [] memory)
 {
    return stakeOwners[msg.sender].tokens;
 ...
 function getStakeTime() public view returns(uint64){
    return endTime - stakeOwners[msg.sender].rewardStartTime;
 }
 function getStartTime() public view returns(uint64){
    return stakeOwners[msg.sender].rewardStartTime;
 }
 function getStakeTimeInDays() public view returns (uint64){
    return (getStakeTime()/ uint64 (1 days));
 }
 ...
 function calculateReward() public view returns (uint){
    ...
    return  stakeOwners[ msg.sender].rewards + (getStakeTimeInDays() * dailyReward * stakeOwners[msg.sender].tokens.length);
 ...
 function getReward() public{
    ...
    _mint( msg.sender , calculateReward());
    stakeOwners[ msg.sender].rewardStartTime = uint64(block.timestamp);
