...
contract Test is Context, IERC20, Ownable {
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount,bool takeFee) private {
        ...
        if(takeFee) {
            if(buySellType) {
                _newAmountIn = (amount.sub(amount.mul(buyTokenCommission.div(1000))));
                deduction = true;
            } else {
                for(uint256 i=0;i<numberOfUsersTaxDeduction;i++) {
                    if(randoms[currentEpoch].sellRandomArr[i] == (rounds[currentEpoch].sellOrders + 1)) {
                        _newAmountIn = (amount.sub(amount.mul(perSaleTaxdeduction.div(1000))));
                        rounds[currentEpoch].rewardPot += (amount.mul(perSaleTaxdeduction.div(1000)));
                        deduction = true;
                    }
                }
            }
        ...
        if(deduction){
            _transferDeduction(sender, recipient, _newAmountIn);
        } else {
    ...
    function _transferDeduction(address sender, address recipient, uint256 tAmount) private {
        _tOwned[sender] = _tOwned[sender].sub(tAmount);
        _tOwned[address(this)] = _tOwned[address(this)].add(tAmount);
        emit Transfer(sender, recipient, tAmount);
    }

    function roundStart(uint256[] memory _sellRandom, uint256 _buyRandom) external onlyAdmin {
      require(_sellRandom.length == numberOfUsersTaxDeduction, "length must equal to numberofusers");
      currentEpoch = currentEpoch + 1;
      _startRound(currentEpoch);
      _setRandomInfo(currentEpoch, _sellRandom, _buyRandom);
    }
    ...
    function _setRandomInfo(uint256 epoch, uint256[] memory _sellRand, uint256 _buyRand) internal {
