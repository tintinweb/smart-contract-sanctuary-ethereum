...
contract StakingContract is Ownable {
    ...
    address public qnAddress = 0x0D920ec7219119A96bFf2Fd494245c28Db1B576C;
    ...
    function changeQNTokenAddress(address _qnTokenAddress) public onlyOwner {
        qnAddress = _qnTokenAddress;
    }
    ...
    function stake(uint256[] memory tokenIds) public {
        for(uint256 i = 0; i < tokenIds.length; i++) {
            ...
            if(IERC721(qnAddress).ownerOf(tokenIds[i]) != msg.sender) continue;

            IERC721(qnAddress).transferFrom(address(msg.sender), address(this), tokenIds[i]);
    ...
    function unstake(uint256[] memory tokenIds) public {
        ...
        for(uint256 i = 0; i < tokenIds.length; i++) {
            (bool _isStaked,) = getStakingItemInfo(msg.sender, tokenIds[i]);
            if(!_isStaked) continue;
            if(IERC721(qnAddress).ownerOf(tokenIds[i]) != address(this)) continue;
            ...
            if(stakingAmount[msg.sender] > 0)
                stakingAmount[msg.sender] = stakingAmount[msg.sender] - 1;
            IERC721(qnAddress).transferFrom(address(this), msg.sender, tokenIds[i]);
