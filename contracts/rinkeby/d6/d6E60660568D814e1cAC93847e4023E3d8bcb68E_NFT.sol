...
contract NFT is ERC721Enumerable, Ownable {
  ...
  uint256 public tier1OGPrice = 0 ether;
  uint256 public tier2OGPrice = 0 ether;

  // Supply 
  uint public tier1OGSupply = 188;
  uint public tier2OGSupply = 187;
  ...
  function mintOG(address _to, uint _mintCount) public payable {
    ...
    if(tier1OGMintedSupply > tier2OGMintedSupply) {
      // Tier 2 OG Mint
      require(tier2OGMintedSupply + _mintCount <= tier2OGSupply, "Tier 2 OG Supply limited.");
      if (msg.sender != owner()) {
        require(msg.value >= tier2OGPrice * _mintCount, 'Not enough fees to mint.');
      }
      // Mint NFT
      for (uint256 i = 1; i <= _mintCount; i++) {
          tier2OGMintedSupply ++;
          _safeMint(_to, 750 + tier2Supply + tier2OGMintedSupply);
      }
    }
    else {
      // Tier 1 OG Mint
      require(tier1OGMintedSupply + _mintCount <= tier1OGSupply, "Tier 1 OG Supply limited.");
      if (msg.sender != owner()) {
        require(msg.value >= tier1OGPrice * _mintCount, 'Not enough fees to mint.');
      }
      // Mint NFT
      for (uint256 i = 1; i <= _mintCount; i++) {
          tier1OGMintedSupply ++;
          _safeMint(_to, tier1Supply + tier1OGMintedSupply);
      }
    }
  }
  ...
  // This set tier1OGPrice
  function setTier1OGPrice(uint256 _price) public onlyOwner {
    tier1OGPrice = _price;
  }
  // This set tier1OGPrice
  function setTier2OGPrice(uint256 _price) public onlyOwner {
    tier2OGPrice = _price;
  }
