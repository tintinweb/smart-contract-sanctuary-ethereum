    ...
    ERC721URIStorageUpgradeable
{
    ...
    function refilSerum(uint256 _serumId) public payable {
        require(_msgSender() == adminWallet || _msgSender() == owner() || _msgSender() == ownerOf(_serumId),
         'you are not authorized to add new serum');
    ...
    function addTokenSpecs (
        ...
        require(_exists(_serumId), "Token not exists");
        
        if (_tokenMeta[_serumId].classify == 2){ //per collection
            CollectionMeta memory collection = CollectionMeta (
                _contractAddress,
                _collectionName
            );
            _setCollectionMeta(_serumId, collection);
        
        } else if (_tokenMeta[_serumId].classify == 3){  //per card
            CardMeta memory card = CardMeta (
                _contractAddress,
                _cardNum
            );
            _setCardMeta(_serumId, card);
        }
   ...
   function transform(
        ...
        require(IERC721Upgradeable(_originalNftContract).ownerOf(_originalNftId) == _msgSender() , "Caller is not the owner of the original nft") ;
     ...
     * existence of serum
     */
    function MintUser(address payable _owner, uint256 _serumId) public returns (uint256) {
    ...
    function getLatestTransformed(address _owner) public view returns (uint256) {
        uint256 _getLatestTokenCount = _transformedNftContract.balanceOf(_owner);
        return _getLatestTokenCount;
    }
}
