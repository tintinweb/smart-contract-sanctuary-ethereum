...
contract NewContract {
    ...
    struct Document {
        ...
        string DocID_Link;
        bool DocID_Sent;
        uint256 DocID_Sent_Time;
        bool DocID_Approved;
        uint256 DocID_Approved_Time;
        uint256 DocID_Step;
        uint8 DocID_Role_From;
        uint8 DocID_Role_To;
    }
    ...
    function get_documents() public view returns (Document[] memory) {
        Document[] memory DocumentsData;
        for (uint256 i = 0; i < DocIds.length; i++) {
            DocumentsData[i].DocIds = DocIds[i];
            DocumentsData[i].DocID_Name = DocID_Name[DocIds[i]];
            DocumentsData[i].DocID_Link = DocID_Link[DocIds[i]];
            DocumentsData[i].DocID_Sent = DocID_Sent[DocIds[i]];
            DocumentsData[i].DocID_Sent_Time = DocID_Sent_Time[DocIds[i]];
            DocumentsData[i].DocID_Approved = DocID_Approved[DocIds[i]];
            DocumentsData[i].DocID_Approved_Time = DocID_Approved_Time[
                DocIds[i]
            ];
            DocumentsData[i].DocID_Step = DocID_Step[DocIds[i]];
            DocumentsData[i].DocID_Role_From = DocID_Role_From[DocIds[i]];
            DocumentsData[i].DocID_Role_To = DocID_Role_To[DocIds[i]];
        }
