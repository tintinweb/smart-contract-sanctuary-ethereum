...
contract GearPod is AbstractERC1155Factory {
     ...
     *
     * @param _signature the ticket signature
     * @param _tokenId the token id to mint
     * @param _amount the amount of tokens to mint
     * @param _maxAmount the max amount of tokens wallet is eligible to mint 
     */
    function mint(
        ...
        if (block.timestamp < pods[_tokenId].startWindow || block.timestamp > pods[_tokenId].endWindow) {
            revert windowClosed();
        }

        bytes32 hash = keccak256(
            abi.encodePacked(msg.sender, _tokenId, _maxAmount)
        );
        if (hash.toEthSignedMessageHash().recover(_signature) != signer) {
            revert signatureInvalid();
        }

        if(pods[_tokenId].amountMinted[msg.sender] + _amount > _maxAmount) {
            revert amountInvalid();
        }
        pods[_tokenId].amountMinted[msg.sender] += _amount;

        if(pods[_tokenId].price > 0) {
            POW.transferFrom(msg.sender, paymentReceiver, pods[_tokenId].price * _amount);
        }
     ...
     * @notice Mints the given amount to receiver address
     *
     * @param _signatures the signatures issued by PV
     * @param _tokenIds the token ids to mint
     * @param _amounts the amounts of tokens to mint
     * @param _maxAmounts the max amounts of tokens wallet is eligible to mint 
     */
    function batchMint(
        ...
        for(uint256 i=0; i<_signatures.length;) {
            ...
            bytes32 hash = keccak256(
                abi.encodePacked(msg.sender, _tokenIds[i], _maxAmounts[i])
            );
            if (hash.toEthSignedMessageHash().recover(_signatures[i]) != signer) {
                revert signatureInvalid();
            }

            if(pods[_tokenIds[i]].amountMinted[msg.sender] + _amounts[i] > _maxAmounts[i]) {
                revert amountInvalid();
            }
