...
contract Sale is Initializable, OwnableUpgradeable, UUPSUpgradeable, ReentrancyGuardUpgradeable, Whitelisted {
    ...
    function getReservedTokenIds(address _user) external view returns(uint256[] memory) {
        return _getReservedTokenIds(_reservedTokenIds[_user]);
    }
    ...
    */
    ...
    nonReentrant() {
        address user = msg.sender;
        uint256[] memory tokenIds = _getReservedTokenIds(_reservedTokenIds[user]);
    ...
    function freeMint(address _user, uint256[] memory _tiers, uint256[] memory _rates, bytes[] memory _signature) external onlyOwner {
        uint256[] memory tokenIds = _getReservedTokenIds(_reservedTokenIds[_user]);
        require(tokenIds.length != 0, "None reserved token for this address");
    ...
    function _getReservedTokenIds(uint256[] memory tokenIds) internal view returns(uint256[] memory) {
        uint256 counter = 0;
        uint256 index = 0;
