...
contract BIG3 is ERC721, IERC721Receiver, Ownable, ERC721Enumerable, ERC721Royalty {

    bool public areWeReady = false;

    uint256 public availableTokens = TOTAL_BIGS - TOTAL_BIGS_FOR_MINT - _RESERVE;

    address public smallContractAddress;

    mapping(uint256 => uint256) public bredCount;

    uint256 public constant MAX_BIG_PURCHASE = 5;
    ...
    uint256 public constant BIG_PRICE = 42000000000000000; // Wei = 0.042 ETH

    uint256 private _bigFemaleClaimIndex;

    uint256 private _bigMaleClaimIndex;

    uint256 private _bigFemaleMintIndex;

    uint256 private _bigMaleMintIndex;
    ...
    uint256 public constant TOTAL_BIGS = 7777;

    uint256 public constant TOTAL_BIGS_FOR_MINT = 3333;

    constructor() ERC721("BIG3", "BIG3") {}

    function burnBigs(uint256[5] memory _tokenIdList)
    external returns (bool) {
        require(msg.sender == smallContractAddress, "Sender is not SMALL contract");
    ...
    function claimBreed(uint256 _tokenId1, uint256 _tokenId2) external {
        ...
        _safeTransfer(address(this), msg.sender, _tokenId1, "");
        _safeTransfer(address(this), msg.sender, _tokenId2, "");
        uint256 bredToken = _safeClaimBig(msg.sender);
        bredCount[_tokenId1]++;
        bredCount[_tokenId2]++;
    ...
    function stakeForBreed(uint256 _tokenId1, uint256 _tokenId2) external {
        ...
        require(((_tokenId1 < 3889 && _tokenId2 >= 3889) || (_tokenId1 >= 3889 && _tokenId2 < 3889)), "You can only breed a male and a female BIG together");
    ...
    function mintBig(uint256 _numberOfTokens) public payable {
        require(areWeReady == true && true, "Sale not active");
        require(
            _bigMaleMintIndex + _bigFemaleMintIndex + _numberOfTokens <= TOTAL_BIGS_FOR_MINT,
            "Purchase would exceed max supply of tokens"
        );
        require(
            _numberOfTokens <= MAX_BIG_PURCHASE,
            "Exceeded max token purchase"
        );
        require(
            BIG_PRICE * _numberOfTokens <= msg.value,
            "Ether value sent is not correct"
        );

        for (uint256 i = 0; i < _numberOfTokens; i++) {
            _safeMintBig(msg.sender);
        }
    }

    function reserveBigs() public onlyOwner {
        for (uint256 i = 0; i < _RESERVE / 2; i++) {
            _safeMint(msg.sender, i);
            _safeMint(msg.sender, (TOTAL_BIGS / 2) + 1 + i);
        }
    }

    function setSmallContractAddress(address _address) public onlyOwner {
        smallContractAddress = _address;
    }
    ...
    function bred() public view returns (uint256) {
        return _bigMaleClaimIndex + _bigFemaleClaimIndex;
    }
    ...
    function minted() public view returns (uint256) {
        return _bigMaleMintIndex + _bigFemaleMintIndex;
    }
    ...
    function _safeClaimBig(address _to) internal returns (uint256) {
        ...
        if (randomNumber == 0) {
            if (_bigMaleClaimIndex <= TOTAL_BIGS / 2) {
                _safeMint(_to, _bigMaleClaimIndex + TOTAL_BIGS_FOR_MINT / 2 + 1 + _RESERVE / 2);
                _bigMaleClaimIndex = _bigMaleClaimIndex + 1;
                return _bigMaleClaimIndex + TOTAL_BIGS_FOR_MINT / 2 + _RESERVE / 2;
            } else {
                _safeMint(
                    _to,
                    _bigFemaleClaimIndex + TOTAL_BIGS / 2 + TOTAL_BIGS_FOR_MINT / 2 + _RESERVE / 2 + 1
                );
                _bigFemaleClaimIndex = _bigFemaleClaimIndex + 1;
                return _bigFemaleClaimIndex + TOTAL_BIGS / 2 + TOTAL_BIGS_FOR_MINT / 2 + _RESERVE / 2;
            }
        } else {
            if (_bigFemaleMintIndex < TOTAL_BIGS) {
                _safeMint(
                    _to,
                    _bigFemaleClaimIndex + TOTAL_BIGS / 2 + TOTAL_BIGS_FOR_MINT / 2 + _RESERVE / 2 + 1
                );
    ...
    function _safeMintBig(address _to) internal returns (uint256) {
        uint8 randomNumber = uint8(
            uint256(
                keccak256(
                    abi.encodePacked(
                        ...
                        block.difficulty,
                        msg.sender,
                        _bigMaleMintIndex + _bigFemaleMintIndex
                    )
                )
            ) % 2
        );
        if (randomNumber == 0) {
            if (_bigMaleMintIndex < TOTAL_BIGS_FOR_MINT / 2 + 1) {
                _safeMint(_to, _bigMaleMintIndex + _RESERVE / 2);
                _bigMaleMintIndex = _bigMaleMintIndex + 1;
                return _bigMaleMintIndex + _RESERVE / 2 - 1;

            } else {
                _safeMint(
                    _to,
                    _bigFemaleMintIndex + 1 + TOTAL_BIGS / 2 + _RESERVE / 2
                );
                _bigFemaleMintIndex = _bigFemaleMintIndex + 1;
                return _bigFemaleMintIndex + 1 + TOTAL_BIGS / 2 + _RESERVE / 2 - 1;
            }
        } else {
            if (_bigFemaleMintIndex < TOTAL_BIGS_FOR_MINT / 2) {
                _safeMint(
                    _to,
                    _bigFemaleMintIndex + 1 + TOTAL_BIGS / 2 + _RESERVE / 2
                );
