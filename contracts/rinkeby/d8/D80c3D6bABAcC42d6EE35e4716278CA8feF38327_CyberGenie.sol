...
contract CyberGenie is Ownable, ERC721A {
    ...
    // Max token supply
    uint256 public constant MAX_SUPPLY = 100; // change to 5000
    // Max reserved tokens
    uint256 public constant MAX_RESERVED = 10; // change to 150
    // Public mint price
    ...
    string public hiddenReservedBaseURI;
     ...
     *
     */
    function getDiscountedPrice(address _claimer) public view returns (uint256) {
        ...
        if (partnersWalletList[_claimer].discount > 0) {
            ...
            for (uint256 i = 0; i < partnersSCListKeys.length; i++) {
                ...
                if (
                    IERC721(partnersSCListKeys[i]).balanceOf(_msgSender()) > 0
                ) {
                    _price =
                        _price -
                        (_price *
                            (partnersSCList[partnersSCListKeys[i]] / 100));
                }
            }
        }
        return _price;
    ...
    /**
     * @dev sets the base uri for {hiddenMetaDataURI}
     */
    ...
    /**
     * @dev sets the hidden reserved base uri for {hiddenReservedBaseURI}
     */
    function setHiddenReservedBaseURI(string calldata _hiddenReservedBaseURI) external onlyOwner {
        hiddenReservedBaseURI = _hiddenReservedBaseURI;
    }
    ...
    function tokenURI(uint256 _tokenId) public view virtual override returns (string memory) {
        ...
        if(!isPublicSaleActive) {
            if(_tokenId > MAX_RESERVED) {
                return hiddenMetadataURI;
            }
            baseURI = hiddenReservedBaseURI;
        } else {
             baseURI = _baseURI();
        }
       
        return bytes(baseURI).length > 0
        ? string(abi.encodePacked(baseURI, _toString(_tokenId), suffixURI))
        : "";
    }
