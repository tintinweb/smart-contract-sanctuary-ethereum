...
contract ERC721Staking is Ownable, ReentrancyGuard {
    ...
    {
        return (block.timestamp - stakers[_staker].timeOfLastUpdate)/3600*rewardsPerHour*stakers[_staker].amountStaked;
    }
    function timePassed(address _staker) public view returns (uint _time) {
        return block.timestamp - stakers[_staker].timeOfLastUpdate;
    }
    function unclaimed(address _staker) public view returns (uint256 _time) {
