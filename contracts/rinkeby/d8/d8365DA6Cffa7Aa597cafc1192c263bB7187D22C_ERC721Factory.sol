...
import "./ERC721JIMBeaconProxy.sol";

contract ERC721Factory is AccessControl, Pausable, ReentrancyGuard {
     ...
     * - @param _beaconProxy, address of the beacon proxy.
     */
     ...
     */
    function createToken(string memory _tokenName, string memory _tokenSymbol, string memory _baseUri, uint _salt) external whenNotPaused nonReentrant {
        address beaconProxy = deployProxy(getData(_tokenName, _tokenSymbol), _salt);
        JIMNFT token = JIMNFT(address(beaconProxy));
    ...
    function getData(string memory _name, string memory _symbol) view internal returns(bytes memory){
        return abi.encodeWithSelector(JIMNFT(address(ERC721)).initialize.selector, _name, _symbol);
    }
    ...
    function getCreationBytecode(bytes memory _data) internal view returns (bytes memory) {
        return abi.encodePacked(type(ERC721JIMBeaconProxy).creationCode, abi.encode(beacon, _data));
    }
...
contract JIMNFT is Initializable, UUPSUpgradeable, ERC721Upgradeable, ERC721URIStorageUpgradeable, PausableUpgradeable, OwnableUpgradeable, ERC721BurnableUpgradeable, ReentrancyGuardUpgradeable {
...
contract ERC721JIMBeaconProxy is BeaconProxy{
