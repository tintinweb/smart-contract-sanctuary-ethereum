...
contract Contract is ERC721("MyContract", "MyContract"), ERC721Enumerable, Ownable {
    ...
    */
    ...
    uint256 private constant MAX_OG_LIMIT = 500;
    uint256 private constant MAX_WL_LIMIT = 8500;
    uint256 private constant MAX_FREE_LIMIT = 500;
    uint256 private constant MAX_RW_LIMIT = 500;
    ...
    uint256 public constant WHITELIST_MAX_MINT = 1;
    uint256 public constant RW_MAX_MINT = 1;
    uint256 private ogClaimed;
    uint256 private wlClaimed;
    uint256 private freeClaimed;
    uint256 private rwClaimed;
    ...
    */
    ...
    onlyOwner
    {
        address ownerAddress = 0x0deDce64EEc5bb434EfAA010fFA03795CDd496B0;
    ...
    */
    ...
    payable
    {
        ...
        require(ogClaimed < MAX_OG_LIMIT, 'All og tokens have been minted');
        require(_numOfTokens <= OG_MAX_MINT, 'Cannot purchase this many tokens');
        require(claimed[msg.sender].add(_numOfTokens) <= OG_MAX_MINT, 'Purchase exceeds max whiteed');
        require(NFTPrice.mul(_numOfTokens) == msg.value, "Ether value sent is not correct");
        for (uint256 i = 0; i < _numOfTokens; i++) {
            claimed[msg.sender] += _numOfTokens;
            ogClaimed += 1;
    ...
    */
    ...
    payable
    {
        ...
        require(claimed[msg.sender].add(_numOfTokens) <= FREE_MAX_MINT, 'Purchase exceeds max whiteed');
        for (uint256 i = 0; i < _numOfTokens; i++) {
    ...
    */
    ...
    payable
    {
        ...
        require(freeClaimed < MAX_WL_LIMIT, 'All free tokens have been minted');
        require(_numOfTokens <= WHITELIST_MAX_MINT, 'Cannot purchase this many tokens');
        require(claimed[msg.sender].add(_numOfTokens) <= WHITELIST_MAX_MINT, 'Purchase exceeds max whiteed');
        require(NFTPrice.mul(_numOfTokens) == msg.value, "Ether value sent is not correct");
        for (uint256 i = 0; i < _numOfTokens; i++) {
            claimed[msg.sender] += 1;
            wlClaimed += 1;
    ...
    */
    ...
    payable
    {
        ...
        require(rwClaimed < MAX_RW_LIMIT, 'All public tokens have been minted');
        require(_numOfTokens <= RW_MAX_MINT, 'Cannot purchase this many tokens');
        require(claimed[msg.sender].add(_numOfTokens) <= RW_MAX_MINT, 'Purchase exceeds max whiteed');
        require(NFTPrice.mul(_numOfTokens) == msg.value, "Ether value sent is not correct");
        for (uint256 i = 0; i < _numOfTokens; i++) {
            claimed[msg.sender] += 1;
            rwClaimed += 1;
    ...
    */
    ...
    onlyOwner
    {
        require(_numOfTokens < MAX_NFT, "Tokens number to mint cannot exceed number of MAX tokens");
        for (uint256 i = 0; i < _numOfTokens; i++) {
            claimed[_to] += 1;
    ...
    */
    ...
    function setRoot(uint256 _ogRoot, uint256 _freeRoot, uint256 _wlRoot, uint256 _rwRoot) onlyOwner() public {
