...
import "../contracts/ConquerImplementationPointer.sol";

contract LiquidityPoolManager is
    Ownable,
    OwnerRecovery,
    ConquerImplementationPointer
{
    ...
    constructor(
        ...
        changeConquerImplementation(address(leftSide));
...
import "../contracts/IConquer.sol";

abstract contract ConquerImplementationPointer is Ownable {
    IConquer internal temple;
    ...
    modifier onlyConquer() {
        require(
            address(temple) != address(0),
            "Implementations: Conquer is not set"
        );
        address sender = _msgSender();
        require(
            sender == address(temple),
            "Implementations: Not Conquer"
        );
        _;
    }

    function getConquerImplementation() public view returns (address) {
        return address(temple);
    }

    function changeConquerImplementation(address newImplementation)
        public
        virtual
        onlyOwner
    {
        ...
        require(
            ...
            "Conquer: You can only set 0x0 or a contract address as a new implementation"
        );
        temple = IConquer(newImplementation);
        emit UpdateConquer(oldImplementation, newImplementation);
    }
