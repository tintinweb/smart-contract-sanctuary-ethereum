...
contract Building is ERC1155, OwnableOrApproved, BaseTownsLand, IBuilding {
    string private _baseUri;
    mapping(uint256 => string) private _tokenUris;

    mapping(uint256 => uint256) private _tokenIdToTownType;
    ...
    function supportedTownType(uint256 tokenId)
        external
        view
        onlyTownBuilder
        returns (uint256)
    {
        return _tokenIdToTownType[tokenId];
    }
    ...
    function safeTransferFrom(
        ...
        if (!_isOwnerOrApproved(to)) {
            _townBuilder.addBuildingIfPossible(
                from,
                to,
                id,
                amount,
                _tokenIdToTownType[id]
            );
        }
    }
    ...
    function mint(
        ...
        uint256[] calldata forTownTypes,
        string[] calldata relativeTokenUris
    ) public onlyOwnerOrApproved {
        for (uint256 i = 0; i < tokenIds.length; i++) {
            uint256 tokenId = tokenIds[i];
            _mint(msg.sender, tokenId, tokenAmounts[i], "");
            _tokenIdToTownType[tokenId] = forTownTypes[i];
            _tokenUris[tokenId] = relativeTokenUris[i];
        }
    }
