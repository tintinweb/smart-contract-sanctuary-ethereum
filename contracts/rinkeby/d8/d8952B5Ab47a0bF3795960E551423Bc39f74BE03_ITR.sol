...
contract ITR is ERC721, Ownable, RandomlyAssigned {
    ...
    RandomlyAssigned(199, 1) // sets the max supply to 7777 with randomly mintable tokens
    {
    ...
    function _mintMiddleWare(uint256 _mintAmount, uint256 _tokenCost) internal {
        if(msg.sender != owner()){
            require(msg.value >= _tokenCost * _mintAmount, "Not enough ether sent!");
        }
    ...
    function initialMintToken(uint256 _amount) external onlyOwner {
        ...
        for (uint256 i = 0; i < 120 - _amount; i++) {
            uint256 id = nextToken();
            _safeMint(msg.sender, id);
        }
    }
    ...
    function setCost(uint256 _newCost) public onlyOwner {
        saleCost = _newCost * 10 ** 18;
    }

    function setPreSaleCost(uint256 _newCost) public onlyOwner {
        preSaleCost = _newCost * 10 ** 18;
    }
