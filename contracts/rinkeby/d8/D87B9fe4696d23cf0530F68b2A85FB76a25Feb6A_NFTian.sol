...
import "./NFTianLender.sol";
import "./NFTianBorrower.sol";
import "./NFTianVault.sol";

contract NFTian is Ownable, ReentrancyGuard {
    constructor(
        NFTianVault _vault,
        NFTianLender _NFTianLender,
        NFTianBorrower _NFTianBorrower,
    ...
    NFTianVault public vault;
    NFTianLender public nftianLender;
    NFTianBorrower public nftianBorrower;
    ...
    mapping(uint256 => Loan) internal loanIdToLoan;
    ...
    mapping(address => bool) internal erc20Whitelist;
    mapping(address => bool) internal erc721Whitelist;
    ...
    // Add or Update ERC721 to whitelist
    function updateERC721Whitelist(address _erc721Token, bool _status)
        external
        onlyOwner
    {
        erc721Whitelist[_erc721Token] = _status;
        emit ERC721WhitelistUpdated(
            _erc721Token,
            block.timestamp,
            msg.sender,
            _status
    ...
    // Check if a ERC721 token whitelisted or not
    function isERC721Whitelisted(address _erc721Token)
        public
        view
        returns (bool)
    {
        return erc721Whitelist[_erc721Token];
    }
    ...
    /*
        _numbers[0] -> _loanPrincipalAmount
        _numbers[1] -> _repaymentAmount
        _numbers[2] -> _nftTokenId
        _numbers[3] -> _loanDuration
        _numbers[4] -> _platformFee
        _numbers[5] -> _borrowerNonce
        _numbers[6] -> _lenderNonce
        _address[0] -> _erc721TokenAddress
        _address[1] -> _erc20TokenAddress
        _address[2] -> _lender
        _sign[0]    -> _borrower
        _sign[1]    -> _lender
    */
    ...
    function lend(
        uint256[7] calldata _numbers,
        address[3] calldata _address,
        bytes[2] calldata _sign
    ) external nonReentrant {
        address _lender = _address[2];
        Loan memory loan = Loan({
            loanId: totalLoans, //currentLoanId,
            loanPrincipalAmount: _numbers[0],
            repaymentAmount: _numbers[1],
            nftTokenId: _numbers[2],
            loanStartTime: block.timestamp, //_loanStartTime
            loanDuration: _numbers[3],
            platformFee: _numbers[4],
            erc721Contract: _address[0],
            whitelistedERC20: _address[1],
        ...
        require(
            erc721Whitelist[loan.erc721Contract],
            "lend:: Invalid whitelisted erc721 token"
        );
        require(
            !_userNonce[msg.sender][_numbers[5]],
            "lend:: Invalid borrower nonce, borrower has either cancelled/begun this loan, or reused this nonce when signing"
        );
        _userNonce[msg.sender][_numbers[5]] = true;
        require(
            !_userNonce[_lender][_numbers[6]],
            "lend:: Invalid lender nonce, lender has either cancelled/begun this loan, or reused this nonce when signing"
        );
        _userNonce[_lender][_numbers[6]] = true;

        _isOnLend[loan.erc721Contract][_numbers[2]] = true;
        ...
        nftianLender.lenderMint(_lender, loan.loanId);
        nftianBorrower.borrowerMint(msg.sender, loan.loanId);

        nftianLender.superSetTokenURI(
            loan.loanId,
            string(abi.encodePacked(nftianLender.getBaseURI(), loan.loanId))
        );

        nftianBorrower.superSetTokenURI(
            loan.loanId,
            string(abi.encodePacked(nftianBorrower.getBaseURI(), loan.loanId))
        );
        _loanIdByTokenAddressTokenId[loan.erc721Contract][_numbers[2]] = loan
            .loanId;
    ...
    function payback(uint256 _loanId) external nonReentrant {
        ...
        address borrower = nftianBorrower.ownerOf(_loanId);
        address lender = nftianLender.ownerOf(_loanId);

        require(borrower == msg.sender, "payback:: Unauthorized");
        ...
        IERC20(loan.whitelistedERC20).transferFrom(borrower, beneficiary, fee);

        _isOnLend[loan.erc721Contract][loan.nftTokenId] = false;

        vault.release(loan.erc721Contract, loan.nftTokenId, borrower);

        nftianLender.lenderBurn(loan.loanId);
        nftianBorrower.borrowerBurn(loan.loanId);
    ...
    function liquidateLoan(uint256 _loanId) external nonReentrant {
        ...
        require(lender == msg.sender, "LiquidateLoan:: Only lender can extend");

        loanRepaidOrLiquidated[_loanId] = true;

        vault.release(loan.erc721Contract, loan.nftTokenId, lender);
    ...
    function extendLoan(
        ...
        require(
            verifyExtendedSign(loan.nftTokenId, _duration, borrower, _sign[0]),
            "ExtendLoan:: Invalid Borrower sign"
        );
        ...
        require(
            ...
            "ExtendLoan:: Invalid Lender signature"
        );
    ...
    // Generate Extend Message
    function generateExtendedLoan(uint256 _nftTokenId, uint256 _updatedDuration)
        public
        view
        returns (bytes32)
    {
        return
            keccak256(
                abi.encodePacked(_nftTokenId, _updatedDuration, address(this))
            );
    }

    // Verify Extended Sign
    function verifyExtendedSign(
        uint256 _nftTokenId,
        uint256 _updatedDuration,
        address _signer,
        bytes memory signature
    ) public view returns (bool) {
        bytes32 messageHash = generateExtendedLoan(
            _nftTokenId,
            _updatedDuration
        );
        bytes32 ethSignedMessageHash = getEthSignedMessageHash(messageHash);
...
contract NFTianVault is Context, AccessControl {
    ...
    constructor(address _multiSignWallet) {
        require(
            ...
            "Locker:: Multi Sign Wallet Can not to be Zero Wallet"
        );
    ...
    function release(
        ...
        require(
            hasRole(VAULT, _msgSender()),
            "Locker:: Unauthorized Access, Only PawnMortgage allowed to release"
        );
        IERC721(_nftContract).safeTransferFrom(address(this), _to, _tokenId);
        emit VaultTransfer(_nftContract, _tokenId, _to, block.timestamp);
    }
}
    ...
    AccessControl
{
    ...
    constructor(string memory _tokenBaseURI)
        ERC721("NFTianLender", "NFTianLender")
    {
        _setupRole(DEFAULT_ADMIN_ROLE, msg.sender);
        _baseURIextended = _tokenBaseURI;
    }

    function lenderMint(address to, uint256 tokenId)
        public
        onlyRole(LENDER)
    {
        _mint(to, tokenId);
    ...
    function lenderBurn(uint256 tokenId) public onlyRole(LENDER) {
        _burn(tokenId);
    }
    ...
    function setBaseURI(string memory baseURI_) public onlyRole(LENDER) {
    ...
    function superSetTokenURI(uint256 tokenId, string memory _tokenURI)
        public
        onlyRole(LENDER)
    {
    ...
    AccessControl
{
    ...
    constructor(string memory _tokenBaseURI)
        ERC721("NFTianBorrower", "NFTianBorrower")
    {
        _setupRole(DEFAULT_ADMIN_ROLE, msg.sender);
        _baseURIextended = _tokenBaseURI;
    }

    function borrowerMint(address to, uint256 tokenId)
        public
        onlyRole(BORROWER)
    {
        _mint(to, tokenId);
    ...
    function borrowerBurn(uint256 tokenId) public onlyRole(BORROWER) {
        _burn(tokenId);
    }
