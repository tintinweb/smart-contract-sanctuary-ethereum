...
// openzeppelin contract version 4.5.0, compiler version - 0.8.4+commit.c7e474f2

/// @custom:security-contact [email protected]
contract AkemonaNftToken is
  ERC721,
  ERC721Enumerable,
  ERC721URIStorage,
  Pausable,
  ...
  AccessControl
{
  ...
  function mintMultiple(uint256 qty, string memory uri) public onlyRole(MINTER_ROLE) {
    require(totalSupply() + qty <= _maxSupply, 'out of stock');
  ...
  function safeMint(address to, string memory uri) public onlyRole(MINTER_ROLE) {
    require(totalSupply() + 1 <= _maxSupply, 'out of stock');
    uint256 tokenId = _tokenIdCounter.current();
    _tokenIdCounter.increment();
    _safeMint(to, tokenId);
    _setTokenURI(tokenId, uri);
  ...
  // make admin only to discourage usage
  function transferFrom(
    address from,
    address to,
    uint256 tokenId
  ...
  function updateTokenURI(uint256 tokenId, string memory uri) public onlyRole(DEFAULT_ADMIN_ROLE) {
    _setTokenURI(tokenId, uri);
  }
