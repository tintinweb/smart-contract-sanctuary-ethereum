...
contract Ethbox is ERC165, IERC721, IERC721Metadata, Initializable, OwnableUpgradeable, ReentrancyGuardUpgradeable, UUPSUpgradeable {
    ...
    /// different mappings.
    function mintMyEthbox() public onlyProxy {
        require(!minted[msg.sender], "ethbox already minted");
        ...
        if (ethboxMessages[msg.sender].length > 0) {
            claimAll();
        }
    }
    ...
    function sendMessage(address _to, string calldata _message) public payable nonReentrant onlyProxy {
        ...
        if (minted[_to]) compBoxSize = ethboxInfo.size;
        if (dynamicBoxSize < compBoxSize) {
            ...
            if (qualifies) {
                ...
                uint256 claimValue = getClaimableValue(droppedMessage, ethboxInfo.drip);
    ...
    function removeOne(uint256 _index) external nonReentrant {
        require(minted[msg.sender], "ethbox needs to be minted");

        EthboxStructs.Message memory message = ethboxMessages[msg.sender][
            _index
        ];
    ...
    function claimOne(uint256 _index) external nonReentrant {
        require(minted[msg.sender], "ethbox needs to be minted");
        uint256 claimValue = getClaimableValue(
        ...
        returns (uint256)
    {
        ...
        for (uint256 i = 0; i < boxSize; i++) {
            claimValue = getClaimableValue(
                messages[i],
                unpackEthboxDrip(_ethboxAddress)
            );
            totalValue += claimValue;
        }
