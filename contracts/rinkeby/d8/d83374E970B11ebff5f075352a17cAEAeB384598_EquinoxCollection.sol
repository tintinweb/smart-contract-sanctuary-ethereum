...
contract EquinoxCollection is Base721 {
    constructor(address _signAddress) Base721("ENS NFT", "E-NFTs", _signAddress) {}
}
...
*/
    ...
    // From Uniswap’s merkle-distributor.
    ...
    function forgeAll(uint64[] memory tokenIds) public {
        for (uint8 i = 0; i < 12; i++) {
            ...
            require(!isClaimed(tokenIds[i]), "Claimed.");
            _setClaimed(tokenIds[i]);
        }
        mint(msg.sender, 13);
...
abstract contract Generic721 is ERC721 {
        ...
        )
    {
        ...
        uint256[] memory _mintTime = new uint256[](CardTypeCount);
    ...
    function getMintTime(uint64 _cardID) public view returns (uint256) {
        return CardType[_cardID].mintTime;
    }
