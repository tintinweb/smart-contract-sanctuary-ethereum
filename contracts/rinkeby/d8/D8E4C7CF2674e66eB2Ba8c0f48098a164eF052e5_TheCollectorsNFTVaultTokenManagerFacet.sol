...
*/
contract TheCollectorsNFTVaultTokenManagerFacet is TheCollectorsNFTVaultBaseFacet, INFTTokenTransferHandler {
    ...
    function redeemToken(uint256 tokenId) external nonReentrant {
        ...
        for (uint256 i; i < participants.length; i++) {
            if (_as.vaultParticipants[vaultId][i].partialNFTVaultTokenId == tokenId) {
                _burn(tokenId);
                uint256 percentage = _getPercentage(vaultId, address(0), tokenId);
                ...
                uint256 protocolFee = _as.vaultParticipants[vaultId][i].collectorOwner != address(0) ? 0 : profits * LibDiamond.PROTOCOL_FEE / LibDiamond.PERCENTAGE_DENOMINATOR;
                ...
                IAssetsHolderImpl(_as.assetsHolders[vaultId]).sendValue(
                    ...
                    profits - protocolFee - liquidityFee
                );
                if (protocolFee > 0) {
                    IAssetsHolderImpl(_as.assetsHolders[vaultId]).sendValue(payable(owner()), protocolFee);
                }
                if (liquidityFee > 0) {
    ...
    */
    function transferNFTVaultToken(
        ...
        if (msg.sender == IProxyRegistry(0x1E525EEAF261cA41b809884CBDE9DD9E1619573A).proxies(from)) {
