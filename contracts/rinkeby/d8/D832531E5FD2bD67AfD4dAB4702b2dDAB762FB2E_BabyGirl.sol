...
contract BabyGirl is ERC721, ERC721Enumerable, ERC721URIStorage, Pausable, Ownable, ERC721Burnable {
    ...
    uint256 public zhuoyue = 150;
    uint256 public wanmei = 350;
    uint256 public jingzhi = 1000;
    uint256 public putong = 3500;
    uint256 public luezhi = 5000;
    uint [5] T = [5, 4, 3, 2,1];  //随机生成 5个等级
    uint [5] S = [104, 71, 20, 7,3];  //随机生成等级几率
    uint256 public price = 50000000000000000000; //50000000000000000  0.05
    ...
    constructor() ERC721('BabyGirl', 'BabyGirl_NFT') {}
      ...
      function rand() public view returns(uint) {  //随机生成等级
    uint _length = 1234567890123456789012;
    ...
    randoms *= sum;
    //return randoms/1000000000000000000000 ;
    for(uint b= 0 ; b<S.length; b++) {
         factor += S[b];
        // require(randoms <= factor, 'only 3 per transaction allowed');
...
//    //      level[_summoner] = _level+1;
  ...
  //    }
    ...
    function internalMint(address to) internal {
        ...
        uint _level = rand();
        emit leveled(msg.sender, _level, _tokenIdCounter.current());
