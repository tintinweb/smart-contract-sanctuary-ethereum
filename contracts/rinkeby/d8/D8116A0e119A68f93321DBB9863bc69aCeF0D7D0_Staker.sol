...
contract Staker {
  ...
  function execute() public stakeNotCompleted{
    //yyy czemu require się wypierdala?
    if(address(this).balance < threshold) return;
  ...
  function withdraw() public deadlinePassed stakeNotCompleted{
    require(balances[msg.sender]>0, "cannot withdraw, user has no funds");
