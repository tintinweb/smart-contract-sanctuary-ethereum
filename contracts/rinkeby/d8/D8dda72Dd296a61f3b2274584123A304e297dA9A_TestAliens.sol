...
contract TestAliens is ERC721Enumerable, Ownable, ReentrancyGuard {
    using Strings for uint256;

    address constant team_address = 0x5C3E4583B15E1Ba963C4C8b322f9dc9F20313aFA;

    uint256 public MAXIMUM_SUPPLY = 10000;

    uint256 public constant MAXIMUM_MINT_WL = 4;
    uint256 public constant MAXIMUM_MINT_RAFFLE = 4;

    uint256 WL_PRICE = 0.01 ether;
    uint256 RAFFLE_PRICE = 0.01 ether;
    ...
    function presaleMint(uint256 ammount, bytes32[] calldata _merkleProof) external payable nonReentrant
    {
        ...
        require(workflow == WorkflowStatus.Presale, "Test Aliens: Presale is not started yet!");

        require(tokensPerWalletWhitelist[msg.sender] + ammount <= MAXIMUM_MINT_WL, string(abi.encodePacked("Test Aliens: Presale mint is ", MAXIMUM_MINT_WL.toString(), " token only.")));

        bytes32 leaf = keccak256(abi.encodePacked(msg.sender));
        require(MerkleProof.verify(_merkleProof, merkleRoot, leaf), "Test Aliens: You are not whitelisted");

        require(msg.value >= price * ammount, "Test Aliens: Not enough ETH sent");
    ...
    function raffleMint(uint256 ammount) public payable nonReentrant {
        ...
        require(workflow != WorkflowStatus.SoldOut, "Test Aliens: SOLD OUT!");
        require(workflow == WorkflowStatus.Sale, "Test Aliens: Raffle is not started yet");
        require(msg.value >= price * ammount, "Test Aliens: Not enough ETH sent");
        require(ammount <= MAXIMUM_MINT_RAFFLE, string(abi.encodePacked("Test Aliens: You can only mint up to ", MAXIMUM_MINT_RAFFLE.toString(), " token at once!")));
        require(tokensPerWalletRaffle[msg.sender] + ammount <= MAXIMUM_MINT_RAFFLE, string(abi.encodePacked("Test Aliens: You cant mint more than ", MAXIMUM_MINT_RAFFLE.toString(), " tokens!")));
        require(supply + ammount <= MAXIMUM_SUPPLY, "Test Aliens: Mint too large!");
    ...
    function setUpSale() external onlyOwner {
        require(workflow == WorkflowStatus.Presale, "Test Aliens: Unauthorized Transaction");
