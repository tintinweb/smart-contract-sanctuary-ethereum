...
contract FACES_OF_REKT_CONTRACT is ERC721A, Ownable {
    ...
    string public _baseURIextended = "https://faces-api.onrender.com/for/metadata/"; // This one is used on OpenSea to define the metadata IPFS address
    ...
    mapping(address => uint16) public address_already_minted;
    ...
    uint256 capMultiplier = 2;

    address public interacter_address;

    address public can_contract_address = 0xd64A7d343F8B102E6d7e744F6EB6893539214b25;
    ...
    function burn(uint16 burn) external onlyOwner() {

        totalSupply += burn;
        __burn_non_minted_token(burn);
    ...
    function M_I_N_T(bytes32[] memory proof, uint16 quantity) public payable {
        ...
        if (whitelist_mint) {
            ...
            if(quantity + address_already_minted[msg.sender] > 2) {
                require(
                    spray_can.balanceOf(msg.sender)
                    >= quantity + address_already_minted[msg.sender],
        ...
        address_already_minted[msg.sender] += quantity;
