...
contract ERC721MembershipUAEUpgradeable is Initializable, ContextUpgradeable, OwnableUpgradeable, ERC721EnumerableUpgradeable, ERC721BurnableUpgradeable, ERC721PausableUpgradeable {
    ...
    modifier onlyTrustedMinter(address minterAddress_) {
        require(_trustedMinterList[minterAddress_], "ERC721MembershipUAE: caller is not trust minter");
        _;
    }
    ...
    function initialize(
        ...
        __ERC721MembershipUAE_init(name_, symbol_, baseUri_, maxTotalSupply_);
    }

    function __ERC721MembershipUAE_init(
        string memory name_,
        string memory symbol_,
        string memory baseUri_,
        uint256 maxTotalSupply_
        ...
        __ERC721MembershipUAE_init_unchained(baseUri_, maxTotalSupply_);
    }

    function __ERC721MembershipUAE_init_unchained(
        string memory baseUri_,
        uint256 maxTotalSupply_
    ) internal initializer {
        require(bytes(baseUri_).length != 0, "ERC721MembershipUAE: invalid base uri");
        require(maxTotalSupply_ >= 1, "ERC721MembershipUAE: invalid max total supply");
        _baseUri = baseUri_;
    ...
    function getTokenIdCount() public view virtual returns (uint256) {
        return _tokenIdTracker.current();
    }

    function tokenURI(uint256 tokenId_) public view virtual override returns (string memory) {
        require(_exists(tokenId_), "ERC721MembershipUAE: URI query for nonexistent token");
        return string(abi.encodePacked(_baseUri, "/", tokenId_.toString(), ".json"));
    }

    function tokenInfo() external view virtual returns (uint256 maxTotalSupply, uint256 tokenCount, uint256 tokenMintLeft) {
        uint256 maxTokenTotalSupply = getMaxTotalSupply();
        uint256 tokenIdCount = getTokenIdCount();
        return (
            maxTokenTotalSupply,
            tokenIdCount,
            maxTokenTotalSupply - tokenIdCount
        );
    }

    function isTrustedMinter(address minterAddress_) external view virtual returns (bool) {
        return _trustedMinterList[minterAddress_];
    }
    ...
    function updateMaxTotalSupply(uint256 maxTotalSupply_) external virtual onlyOwner {
        require(maxTotalSupply_ >= getTokenIdCount(), "ERC721MembershipUAE: invalid max total supply");
        _maxTotalSupply = maxTotalSupply_;
    ...
    function addToTrustedMinterList(address minterAddress_) external virtual onlyOwner {
        require(minterAddress_ != address(0), "ERC721MembershipUAE: invalid minter address");
    ...
    function updateRoyaltyParams(address royaltyAddress_, uint256 royaltyPercent_) external virtual onlyOwner {
        require(royaltyAddress_ != address(0), "ERC721MembershipUAE: invalid royalty address");
        require(royaltyPercent_ <= _MAX_PERCENT, "ERC721MembershipUAE: invalid percent");
    ...
    function _mintToken(address recipient_, uint256 amount_) internal virtual {
        require(recipient_ != address(0), "ERC721MembershipUAE: invalid recipient address");
        require(amount_ > 0, "ERC721MembershipUAE: invalid amount");
        for (uint256 i = 0; i < amount_; ++i) {
            uint256 tokenIdCount = _tokenIdTracker.current();
            if (tokenIdCount + 1 > _maxTotalSupply) {
                return;
            }
            _mint(recipient_, tokenIdCount + 1);
