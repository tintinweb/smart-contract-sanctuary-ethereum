...
contract Hypedudes is ERC721I, Ownable {
    ...
    function setMaxMintsPerTx(uint256 _maxMintsPerTx) public onlyOwner {
        require(
            maxMintsPerWallet >= _maxMintsPerTx,
            "maxMintsPerWallet cannot be set lower than maxMintsPerTx!"
        );
    ...
    function setMaxMintsPerWallet(uint256 _maxMintsPerWallet) public onlyOwner {
        require(
            _maxMintsPerWallet >= maxMintsPerTx,
            "maxMintsPerWallet cannot be set lower than maxMintsPerTx!"
        );
        ...
        whitelistMinting
    {
        ...
        require(
            maxMintsPerWallet >= balanceOf[msg.sender] + _amount,
            "You don't have enough mints remaining!"
        );
        require(
            maxSupply >= totalSupply + _amount,
            "Not enough tokens remaining!"
    ...
    function withdraw() public onlyOwner {
        ...
        _sendETH(
            payable(0x2D3C70A7b4d9C8Cba7D6f78F8B707256eE40A3c0),
            _toShare1
        );
    ...
    function partialWithdraw(uint256 _amount) public onlyOwner {
        ...
        uint256 _toShare1 = (_amount * 5) / 100;
        uint256 _toShare2 = _amount - _toShare1;
    ...
    function emergencyWithdraw() public onlyOwner {
        require(
            shareSigned[msg.sender] &&
                shareSigned[0x2D3C70A7b4d9C8Cba7D6f78F8B707256eE40A3c0],
            "Both parties have not agreed to unlock this function!"
        );

        _sendETH(payable(msg.sender), address(this).balance);
    }

    function emergencyPartialWithdraw(uint256 _amount) public onlyOwner {
        require(address(this).balance >= _amount, "Insufficient balance!");
        require(
            shareSigned[msg.sender] &&
                shareSigned[0x2D3C70A7b4d9C8Cba7D6f78F8B707256eE40A3c0],
