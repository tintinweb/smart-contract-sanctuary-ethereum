...
contract UnifarmCohort is IUnifarmCohort {
    ...
    function computeRValue(
        ...
        while (i < currentEpoch) {
            ...
            if (userStakedBlock > i.mul(epochBlocks)) {
                eligibleBlocks = i.add(1).mul(epochBlocks).sub(userStakedBlock);
            } else {
                eligibleBlocks = epochBlocks;
            }
            if (isBoosterBuyed == false) {
                r += (stakedAmount.mul(1e12).mul(eligibleBlocks)).div(totalStakeLimit);
            } else {
                ...
                r += (stakedAmount.mul(1e12).mul(eligibleBlocks).div(priorEpochATotalStaking));
            }
            i++;
        }
    }
    ...
    function stake(
        ...
        {
            userTotalStaking[user][fid] = userTotalStaking[user][fid].add(stakeAmount);
            totalStaking[fid] = totalStaking[fid].add(stakeAmount);
        ...
        emit ReferedBy(tokenId, referralAddress, fid);
    }
    ...
    function updateUserTotalStaking(
        ...
        userTotalStaking[user][fid] = userTotalStaking[user][fid].sub(stakedAmount);
    }
    ...
    function unStake(
        ...
        uint256 rValue = computeRValue(
            ...
            staked.stakedAmount,
            blocksData[3],
            staked.startBlock.sub(blocksData[4]),
            totalStakeLimit,
            staked.isBooster
        );
        {
            totalStaking[staked.fid] = totalStaking[staked.fid].sub(staked.stakedAmount);
...
interface IUnifarmCohort {
   ...
   */
    ...
    event ReferedBy(uint256 indexed tokenId, address indexed referralAddress, uint32 fid);
}
...
library CheckPointReward {
    /// @notice for calculation of uint256
    using SafeMath for uint256;
    ...
    function getCheckpoint(
        ...
        return uint256(blockDifference.div(epochBlocks));
    }
...
library CohortHelper {
    ...
    function getCohortBalance(address token, uint256 totalStaking) internal view returns (uint256 cohortBalance) {
        ...
        cohortBalance = contractBalance.sub(totalStaking);
    }
