...
contract TheCollectorsOpenseaNFTVaultAssetsManager is TheCollectorsOpenseaNFTVaultBase {
    ...
    /*
        ...
        @tokenId the BAYC's id
        @_participants list with Bob's, Mary's and Jim's addresses (in that order)
    ...
    */
    function migrate(uint256 vaultId, uint256 tokenId, address[] memory _participants, uint256[] memory payments) external {
        ...
        _afterPurchaseNFT(vaultId, totalPaid);
    }
    ...
    */
    function sellNFTToVault(uint256 vaultId, uint256 sellPrice) external nonReentrant {
        ...
        _afterPurchaseNFT(vaultId, sellPrice);
    }
    ...
    */
    function buyNFTOnOpensea(
        ...
        _afterPurchaseNFT(vaultId, purchasePrice);
    }
    ...
    */
    function _requireBuyConsensusAndValidatePurchasePrice(uint256 vaultId, uint256 purchasePrice) internal view {
        ...
        // Adding 10 wei since votesPercentage can be 99.999999999999999999
        require(votesPercentage + 10 wei >= vaults[vaultId].buyConsensus, "E3");
    }
    ...
    */
    ...
    /*
        @dev
        A helper function to validate whatever the vault is actually purchased the token
    */
    function _afterPurchaseNFT(uint256 vaultId, uint256 purchasePrice) internal {
        Participant[] storage participants = _vaultParticipants[vaultId];
        uint256 totalPaid;
        for (uint256 i; i < participants.length; i++) {
            totalPaid += participants[i].paid;
        }
        // The ration between the actual purchase price and the total paid amount
        uint256 ratio = purchasePrice * 1e18 / totalPaid;
    ...
    */
    function _beforeCancellingSellOrderOnOpensea(uint256 vaultId, bytes memory _calldata) internal view {
        Vault storage vault = vaults[vaultId];
        require(vault.votingFor == VoteFor.CancellingSellOrder, "E1");
...
abstract contract TheCollectorsOpenseaNFTVaultBase is ReentrancyGuard {
    ...
    struct Participant {
        ...
        // Indicates that this participant voted for or against selling the NFT
        // This will be used to determine if to count their vote, if the grace period for voting has ended
        bool votedOnSellingOrCancellingSellOrder;
    }
    ...
    */
    function isVaultPassedSellOrCancelSellOrderConsensus(uint256 vaultId) public view returns (bool) {
        ...
        // Adding 10 wei since votesPercentage can be 99.999999999999999999
        return votesPercentage + 10 wei >= vaults[vaultId].sellOrCancelSellOrderConsensus;
    }
    ...
    */
    function _getParticipantSellOrCancelSellOrderVote(uint256 vaultId, uint256 participantIndex) internal view returns (bool) {
        ...
        || (!_vaultParticipants[vaultId][participantIndex].votedOnSellingOrCancellingSellOrder
    ...
    */
    function _resetVotesAndGracePeriod(uint256 vaultId) internal {
        ...
        for (uint256 i; i < participants.length; i++) {
            // Resetting votes
            participants[i].vote = false;
            participants[i].votedOnSellingOrCancellingSellOrder = false;
        }
    }
    ...
    /*
        ...
        the method should return the participant %. In case address 0 was sent, the method will calculate
        the tokenId %.
        This method was written in this manner that to save contract size
    */
    function _getPercentage(uint256 vaultId, address participant, uint256 tokenId) internal view returns (uint256) {
        ...
        for (uint256 i; i < participants.length; i++) {
            ...
            // If using participant address (0), then ignoring this condition and calculating token %
            if ((participants[i].paid > 0 && participants[i].participant == participant)
                // If using tokenId 0, then ignoring this condition and calculating participant %
                || (tokenId != 0 && participants[i].partialNFTVaultTokenId == tokenId)) {
                if (vaults[vaultId].purchasedFor > 0) {
                    // NFT was already purchased
                    return participants[i].paid * 1e18 * 100 / vaults[vaultId].purchasedFor;
                }
                participantsPaid = participants[i].paid;
            }
        }
        // NFT wasn't purchased yet, calculating % based on total paid
        return totalPaid > 0 ? participantsPaid * 1e18 * 100 / totalPaid : 0;
    }
}
