...
contract ShmurfsTest is Ownable, ERC721A {
  ...
  uint private wlMintPassKey = 0;

  mapping(address => uint8) private wlBuyList;
  ...
  function whitelistMint(uint8 quantity) external payable callerIsUser wlSaleIsOpen {
    ...
    require(wlBuyList[msg.sender] + quantity > MAX_WHITELIST_BATCH, "WHITELIST_SALE: You can not get more than 3 NFTs on the Whitelist Sale");
    require(msg.value >= PRICE_WHITELIST * quantity, "WHITELIST_SALE: Not enough funds");
    
    wlBuyList[msg.sender] += quantity;
    _safeMint(msg.sender, quantity);

    refundIfOver(PRICE_WHITELIST * quantity);
  }

  function publicSaleMint(uint8 quantity, uint callerMintKey) external payable callerIsUser publicSaleIsOpen{
    require(wlMintPassKey == callerMintKey, "PUBLIC_SALE: Called with incorrect public sale key");
  ...
  function setWlMintPassKey(uint256 mintPasskey_) external onlyOwner {
    wlMintPassKey = mintPasskey_;
  }
  ...
  function withdrawAll() external onlyOwner {
    ...
    uint256 devOneBalance = totalBalance / 10; // 10% of total sale
    uint256 ownerBalance = totalBalance - devOneBalance; // 90% of total sale
