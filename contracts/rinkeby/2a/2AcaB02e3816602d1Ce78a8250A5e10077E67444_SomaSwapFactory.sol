...
contract SomaSwapFactory is ISomaSwapFactory, ERC2771ContextUpgradeable, NetworkAccessUpgradeable {
    ...
    bytes32 public constant override TEMPLATE_ID = bytes32('SomaSwapPair');
    ...
    constructor(address networkAddress, address routerAddress)
    ERC2771ContextUpgradeable(routerAddress)
    ...
    function createPair(address tokenA, address tokenB) external whenNotPaused onlyRole(CREATE_PAIR_ROLE) override returns (address pair) {
        require(tokenA != tokenB, 'SomaSwap: IDENTICAL_ADDRESSES');
        (address token0, address token1) = tokenA < tokenB ? (tokenA, tokenB) : (tokenB, tokenA);
        require(token0 != address(0), 'SomaSwap: ZERO_ADDRESS');
        require(getPair[token0][token1] == address(0), 'SomaSwap: PAIR_EXISTS'); // single check is sufficient

        pair = network.templateFactory().deployTemplateLATEST(
            TEMPLATE_ID,
            abi.encode(address(network), ROUTER),
