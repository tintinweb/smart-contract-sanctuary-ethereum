...
contract KogakeCarbon is ERC721, ERC721Enumerable, Ownable, ReentrancyGuard {
    ...
    constructor(address _treasury, address _token)
        ERC721("Kogake Carbon NFT", "KogakeCarbon")
    {
    ...
    function mint(uint256 amount) public payable {
        ...
        require(_tokenIds.current() + amount <= maxNFTs, "Too much to mint");

        _mintLogic(amount, false);
    }
    ...
    function whitelistMint(uint256 amount) public {
        ...
        _mintLogic(amount, true);

        whitelist[msg.sender] -= uint16(amount);
    }

    function _mintLogic(uint256 amount, bool fromWhitelist)
        internal
        nonReentrant
    {
