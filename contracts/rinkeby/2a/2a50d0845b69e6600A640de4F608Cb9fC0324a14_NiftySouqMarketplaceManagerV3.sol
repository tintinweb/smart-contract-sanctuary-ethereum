    ...
    AccessControlUpgradeable
{
    ...
    function addCryptoToken(string memory tokenName_, address address_, uint256 tokenValue) external {
        if (address_ != address(0)) {
            cryptoTokenList[tokenName_] = CryptoTokens(address_, tokenValue, true);
            cryptoTokens.push(tokenName_);
            cryptoTokenCount = cryptoTokenCount + 1;
        }
    }

    function enableDisableToken(string memory tokenName_,bool enable_) external{
        ...
        returns (ContractType contractType_, bool isERC1155_, address tokenAddress_)
    {
        if (contractAddress_ == address(0) && quantity_ > 1) {
            ...
            if (success) return (ContractType.COLLECTOR, false, contractAddress_);
            else {
        ...
        )
    {
        ...
        TokenData memory tokenData = NiftySouqINftV1(
            contractAddress_
