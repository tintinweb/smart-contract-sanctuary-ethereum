...
contract Staker {
    ...
    // Collect funds in a payable `stake()` functions and track individual `balances` with a mapping:
    //  ( make sure to add a `Stake(address,uint256)` event and emit it for the frontend <List/> display )
    function stake() public payable {
        address staker = msg.sender;
        uint256 stakedAmount = msg.value;
        balances[staker] = stakedAmount;
    ...
    function withdraw() public payable {
        ...
        uint256 amount = balances[returner];
        require(amount > 0, "user balance is 0");
        balances[returner] = 0;
        payable(returner).transfer(amount);
    }
