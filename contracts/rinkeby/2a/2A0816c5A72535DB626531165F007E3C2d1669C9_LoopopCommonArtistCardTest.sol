...
import "./LoopopCardBase.sol";

// 普通卡
/// @custom:security-contact [email protected]
contract LoopopCommonArtistCardTest is LoopopCardBase {

    using Counters for Counters.Counter;

    Counters.Counter private _tokenIdCounter;

    constructor(address burner_) LoopopCardBase(burner_, "Loopop Common Artist Card test", "LCACt") {
    }
...
// NFT pass基础
/// @custom:security-contact [email protected]
contract LoopopTokenBase is ERC721, ERC721Enumerable, ERC721URIStorage, Pausable, AccessControl, ERC721Burnable, ReentrancyGuard {
...
// 基础卡
/// @custom:security-contact [email protected]
contract LoopopCardBase is LoopopTokenBase {

    address public burner;
    //    userAddress _isUsed
    mapping(uint256 => bool) _isUsed;

    constructor(address burner_, string memory name_, string memory symbol_) LoopopTokenBase(name_, symbol_) {
        burner = burner_;
    }

    function setBurner(address newBurner) external onlyRole(DEFAULT_ADMIN_ROLE)  {
        require(newBurner != address(0) && burner != newBurner, "invalid newBurner address!");
        burner = newBurner;
    }

    function safeBatchUsed(uint256[] memory tokenIds) public{
        for (uint256 i = 0; i < tokenIds.length; i++) {
            require(_isApprovedOrOwner(msg.sender, tokenIds[i]), "caller is not owner nor approved");
            require(!_isUsed[tokenIds[i]], "token had already used");
            _isUsed[tokenIds[i]] = true;
        }
    }

    function safeBatchRecycle(uint256[] memory tokenIds) public{
        for (uint256 i = 0; i < tokenIds.length; i++) {
            require(_isApprovedOrOwner(msg.sender, tokenIds[i]), "caller is not owner nor approved");
            require(!_isUsed[tokenIds[i]], "card is used");
            _safeTransfer(msg.sender, burner, tokenIds[i], "");
        }
    }
    ...
    function _beforeTokenTransfer(address from, address to, uint256 tokenId)
        ...
        override
    {
        require(!_isUsed[tokenId], "card is used!");
        super._beforeTokenTransfer(from, to, tokenId);
    }

    function getIsUsed(uint256 tokenId) public view returns (bool) {
