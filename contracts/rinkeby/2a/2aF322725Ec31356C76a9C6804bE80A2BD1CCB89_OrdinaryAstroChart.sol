...
library SVGGenerator {
    ...
    function nonExistSVGInOpenSeaFormat(uint256 tokenId)
        public
        pure
    ...
    function encodeInSVGFormat(string memory _rawSVG, uint256 tokenId)
        private
        pure
        returns (string memory)
    {
    ...
    string constant svg0 = '<svg xmlns="http://www.w3.org/2000/svg" version="1.1" width="1200" height="1200" id="astro">'
        '    <defs>'
    ...
    function soGenSVG(
        ...
        if(generation == 0){
            result = concat(svg0,golden0);
            result = concat(result, svg1);
            result = concat(result,golden1);
            result = concat(result,svg2);
            result = concat(result,golden2);
        }else{
            result = concat(svg0,silver0);
            result = concat(result, svg1);
            result = concat(result,silver1);
            result = concat(result,svg2);
            result = concat(result,silver2);
        }
        result = concat(result,svg3);
        result = concat(
            result,
        ...
        returns (string memory res)
    {
        ...
        for (uint32 i = 0; i < cusps.length; i += 1) {
            ...
            res = concat(
                res,
                line(xy[0], xy[1], xy[2], xy[3], 20000, LineColor.Default)
            ); //20000 means 2.0000
        }
    }
        ...
        returns (string memory res)
    {
        ...
        res = concat(res,drawPlanetRelationLines(planets));
        ...
        for (uint8 i = 1; i < planets.length; i++) {
            uint8 j = i - 1;
            // detect and move it
            int128 diff=XpMath.abs(int128(uint128(planets[i]))- int128(uint128(planets[j])));
            if (diff <= OVERLAP_DETECT) {
                planets[i] = planets[j] + SPACE_RAD;
            }
            //to64x64
        ...
        returns (string memory res)
    {
        ...
        for (uint8 i = 0; i < planets.length; i++) {
            for (uint8 j = i + 1; j < planets.length; j++) {
                uint16 diff = planets[j] - planets[i];
                int256[4] memory xy;
                if (DOWN60 <= diff && diff <= UP60) {
                    //60
                    xy = genRelationLineCoord(
                        radius,
                        to64x64(int32(uint32(planets[i]))),
                        to64x64(int32(uint32(planets[j])))
                    );
                    res = concat(
                                res,
                                line(xy[0], xy[1], xy[2], xy[3], 20000, LineColor.Yellow)
                            );
                    ...
                    res = concat(
                                res,
                                line(xy[0], xy[1], xy[2], xy[3], 20000, LineColor.Red)
                            );
                    ...
                    res = concat(
                                res,
                                line(xy[0], xy[1], xy[2], xy[3], 20000, LineColor.Green)
                            );
                    ...
                    res = concat(
                                res,
                                line(xy[0], xy[1], xy[2], xy[3], 20000, LineColor.Blue)
                            );
                }
            }
        }
...
library AstroChartLib {
    ...
    */
    ...
    function initialMintDry(uint16[] calldata datetimeOfBirth, int16[] calldata cityOfBirth) public returns ( uint256 tokenId ){
        ...
        require(libStorage().mintedDate2PrimeTokenId[dateToBytes32(datetimeOfBirth[1], datetimeOfBirth[2])] == 0, "IMDAE");
        
        require(DateUtils.getDayFromTimestamp(block.timestamp) == datetimeOfBirth[2], "DTNT");
        ...
        libStorage().mintedDate2PrimeTokenId[dateToBytes32(datetimeOfBirth[1], datetimeOfBirth[2])] = tokenId;
    }
     ...
     */
    function _checkForDateAndCity(uint16[] calldata datetimeOfBirth, int16[] calldata cityOfBirth) private view {
        ...
        require(DateUtils.isDateValid(year, month, day), "DTNV");
...
library DateUtils {
    ...
    function isDateValid(uint16 year, uint16 month, uint16 day) internal view returns (bool) {
        return day > 0 && getDaysInMonth(month, year) >= day;
    }

    function getDaysInMonth(uint16 month, uint16 year) private pure returns (uint16) {
    ...
    function getDayFromTimestamp(uint256 timestamp) internal view returns (uint16) {
