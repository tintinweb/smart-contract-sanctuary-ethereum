...
contract SimpleOracle is IBaseOracle, Governable {
  ...
  function getUnderlyingPrice(address token) external view override returns (uint) {
  ...
  function setUnderlyingPrice(address[] memory tokens, uint[] memory pxs) external onlyGov {
...
interface IBaseOracle {
  /// @dev Return the value of the given input as ETH per unit, multiplied by 2**112.
  /// @param token The ERC-20 token to check the value.
  function getUnderlyingPrice(address token) external view returns (uint);
}
