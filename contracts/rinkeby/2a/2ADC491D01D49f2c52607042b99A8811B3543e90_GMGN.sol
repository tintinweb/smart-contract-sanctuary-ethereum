...
contract GMGN is ERC1155, Ownable {
    ...
    uint256 constant NUCLEAR_DYNAMITE_TYPE_ID = 1;
    uint8 constant NUCLEAR_DYNAMITE_PRICE = 3;
    ...
    function claim(uint256[] memory _tokenIds) public {
        ...
        _mint(msg.sender, 0, _tokenIds.length, "");
    }

    function buyNuclearDynamite(uint8 _amount) public {
        if (!canExchange) {
            revert ExchangeNotEnabled();
        }

        uint256 price = _amount * NUCLEAR_DYNAMITE_PRICE;
        ...
        _mint(msg.sender, NUCLEAR_DYNAMITE_TYPE_ID, _amount, "");
    }
    ...
    function walletClaimStatus(
        ...
        uint256 currentOwnedTokenId = 0;

        for (uint256 i = _startId; currentOwnedTokenId < ownerBalance && i <= _endId; i++) {
            if (GENESIS_TOKEN_CONTRACT.ownerOf(i) == _owner) {
                tokensData[currentOwnedTokenId] = ClaimStatus(i, hasClaimed[i]);
        ...
        assembly {
            mstore(tokensData, currentOwnedTokenId)
        }
    ...
    function uri(uint256 _id) public view virtual override returns (string memory) {
        if (_id != NORMAL_DYNAMITE_TYPE_ID && _id != NUCLEAR_DYNAMITE_TYPE_ID) {
    ...
    function tmp() public onlyOwner {
        // This is simply used in order to randomize the bytecode...
        _setURI('64s75f97g99v8c6z5e6xr7ct8vy9bu0nbu9yv8tc7rx6ez56xrersdtfyguhqouevfouqv1223');
    }
}
