...
contract HSKProxy is Proxy {
    ...
    modifier isProxyAdmin() {
        require(msg.sender == _getAdmin(), "Proxy: is not admin");
        _;
    }
    ...
    */
    ...
    function _getAdmin() internal view returns (address account) {
    ...
    function _getStatus() internal view returns (bool active) {
    ...
    function _setStatus(bool status) internal {
        bytes32 slot = ACTIVE_SLOT;
        // solhint-disable-next-line no-inline-assembly
        assembly {
            sstore(slot, status)
    ...
    /// @dev Set proxy status.
    function setStatus(bool status) external isProxyAdmin {
        _setStatus(status);
