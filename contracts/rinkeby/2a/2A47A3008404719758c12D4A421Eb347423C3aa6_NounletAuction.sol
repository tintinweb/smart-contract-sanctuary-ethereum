...
import {NounletMinter as Minter} from "./NounletMinter.sol";
...
import {INounletRegistry as IRegistry, Permission} from "./interfaces/INounletRegistry.sol";
...
contract NounletAuction is IAuction, Minter, Receiver, ReentrancyGuard {
        ...
        nonReentrant
    {
        ...
        // post increment in memory, so we return current to settle and use +1 that id to mint next nounlet
        _settle(_vault, id++);
    ...
    function bid(address _vault) external payable nonReentrant {
        ...
        if (endTime < extendedTime)
            auction.endTime = extendedTime.safeCastTo32();
    ...
    function withdrawNounlet(address _vault, uint256 _id) external {
        ...
        uint256 endTime = uint256(auctionInfo[_vault][_id].endTime);
        if (msg.sender != winner || block.timestamp < endTime)
            revert NotWinner();

        address token = IRegistry(registry).vaultToToken(_vault);
        INounlet(token).safeTransferFrom(address(this), winner, _id, 1, "");
    }
    ...
    function _settle(address _vault, uint256 _id) internal {
        ...
        if (uint256(auction.endTime) > block.timestamp)
            revert AuctionNotCompleted();

        address winner = auction.bidder;
        uint64 bidAmount = auction.amount;
        address token = IRegistry(registry).vaultToToken(_vault);
        (address beneficiary, uint256 royaltyAmount) = INounlet(token)
            .royaltyInfo(_id, bidAmount);

        ++vaultInfo[_vault].currentId;
        emit Settled(_vault, token, _id, winner, bidAmount);
...
contract NounletMinter is INounletMinter {
    ...
    function _mintFraction(
        ...
        bytes memory data = abi.encodeCall(INounletSupply.mint, (_to, _id));
