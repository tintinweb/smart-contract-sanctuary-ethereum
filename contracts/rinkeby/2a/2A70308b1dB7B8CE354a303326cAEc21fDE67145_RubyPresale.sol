...
contract RubyPresale is ReentrancyGuard {
	...
	IERC20 public Ruby;
	// DAI token
	IERC20 public DAI = IERC20(0x5592EC0cfb4dbc12D3aB100b257153436a1f0FEa);
	...
	function buy(uint256 _amount) public nonReentrant {
        ...
        uint256 bonosToken = tokens.div(10 ** 9);
        uint256 date = block.timestamp.sub(presaleStartTime).div(86400).div(2);
        uint256 datebonos = 0;
        uint256 amountbonos = 0;
        if(date < 10)
        {
            datebonos = 10 - date;
        }
        if( 100 < bonosToken && bonosToken < 1000)
        {
            amountbonos = 3;
        }else if(1000 <= bonosToken && bonosToken < 10000)
        {
            amountbonos = 5;
        }else if(1000 <= bonosToken && bonosToken < 10000)
        {
            amountbonos = 8;
        }else if(10000 < bonosToken)
        {
            amountbonos = 10;
        }
        uint256 totaltokens = tokens.add(tokens.mul(datebonos.add(amountbonos)).div(100));
