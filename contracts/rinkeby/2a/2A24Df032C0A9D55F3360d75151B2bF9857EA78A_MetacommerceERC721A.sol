...
contract MetacommerceERC721A is Initializable, ERC721AUpgradeable, OwnableUpgradeable {
    ...
    function initialize(
        ...
        require(msg.sender == address(collectionFactory), "Proxy: Proxy must be created via the clone factory.");
    ...
    function tokenURI(uint256 _tokenId) public view override returns (string memory) {
        ...
        return revealed ? (bytes(baseURI).length > 0
                ? string(abi.encodePacked(baseURI, StringsUpgradeable.toString(_tokenId + 1), extension))
    ...
    function whitelistMint(uint256 amount, uint256 dummy, bytes32[] calldata proof) public payable callerIsUser {
        ...
        (bool success, ) = payable(commissions).call{value: msg.value * 25 / 1000}("");
        require(success);
    }
