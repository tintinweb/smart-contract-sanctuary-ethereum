...
contract SlayToEarnToken is Context, Ownable, IERC20, IERC20Metadata {
    bool private _enableGateKeeper;
    uint256 private _tradingEnabledAtBlock;
    ...
    constructor(
        ...
        _gateKeeperMaxTokenAmount = 5_000_000_000 ether;

        setTaxPercentages(1, 3, 6);
        getOwnedWitnessCount();
    }

    function enableAutomaticBuybacks(IUniswapV2Router02 uniswapRouter, IERC20 usdcToken) public onlyOwner {
    ...
    function disableAutomaticBuybacks() public onlyOwner {
        _uniswapRouter = IUniswapV2Router02(address(0));
        _usdcToken = address(0);
    }

    function isAutomaticBuybackEnabled() public view returns (bool) {
        return _uniswapRouter != IUniswapV2Router02(address(0));
    }
    ...
    function isTradingPermanentlyEnabled() public view returns (bool) {
        return _tradingEnabledAtBlock > 0;
    }
    ...
    function enableTrading(address uniswapLpPair) public onlyOwner {
        require(_tradingEnabledAtBlock == 0, "Trading was already enabled.");
        require(uniswapLpPair != address(0), "The LP pair can not be null.");

        _uniswapLpPair = uniswapLpPair;
        _tradingEnabledAtBlock = block.number;
    }
    ...
    */
    ...
    function setTaxPercentages(uint256 burnPercentage, uint256 rewardPercentage, uint256 devPercentage) public onlyOwner {
        ...
        require(_tokenBurnPercentage + _tokenRewardsPercentage + _tokenDevPercentage <= 30, "The maximum total combined fee is 30%.");
    }
    ...
    function getOwnedWitnessCountForAddress(address wallet) public view returns (uint256) {
        uint256 count = _currentItemCollection.balanceOf(wallet, 2048649);

        if (_originalWitnessCollection == IERC1155(address(0))) {
            return count;
        } else {
            return count + (_originalWitnessCollection.balanceOf(wallet, 2048393) > 0 ? 1 : 0);
        }
    }
    ...
    function _transfer(
        ...
        if (isWhitelisted(sender) || isWhitelisted(recipient)) {
            _transferInternal(sender, recipient, sentAmount);
            return;
        }
        ...
        require(isTradingPermanentlyEnabled(), "Trading is not enabled yet. Refer to our Telegram for further information. This restriction will be lifted shortly.");
        require(sentAmount <= _gateKeeperMaxTokenAmount || isGateKeeperDisabled, "Buy blocked by gatekeeper. Token limit per transaction exceeded. Refer to our Telegram for further information. This restriction will be lifted shortly.");
        ...
        uint256 devTokens = (sentAmount * _tokenDevPercentage * (100 - taxReductionPercentage)) / 10_000;
        uint256 receivedTokens = sentAmount - burnedTokens - rewardTokens - devTokens;

        _burn(sender, burnedTokens);
        _transferInternal(sender, _claimRewardsContract, rewardTokens);

        _transferInternal(sender, address(this), devTokens);
        ...
        _transferInternal(sender, recipient, receivedTokens);
    }
