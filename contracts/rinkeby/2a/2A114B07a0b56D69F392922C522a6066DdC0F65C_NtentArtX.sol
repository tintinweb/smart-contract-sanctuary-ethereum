...
contract NtentArtX is ERC721A {
    ...
    constructor(string memory _tokenName, string memory _tokenSymbol, address _proxyRegistryAddress)
    ...
    function _beforeTokenTransfers(
        ...
        if (
            ...
            ntentTransfer.transfer(_from, _to, _tokenId, _quantity);
        }
    }
    ...
    function isApprovedForAll(address _owner, address _operator) public view override returns (bool){
        ...
        if(address(proxyRegistry.proxies(_owner)) == _operator || 
            proxyToApproved[_operator]) return true;
