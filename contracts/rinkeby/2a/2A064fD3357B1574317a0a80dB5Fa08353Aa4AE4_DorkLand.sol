...
contract DorkLand is ERC721F {

    uint256 public tokenPrice = 0.07 ether; 
    uint256 public constant MAX_TOKENS=39078;
    ...
    mapping(uint256 => bool) private reservedPlot;

    event PriceChange(address _by, uint256 price);

    constructor() ERC721F("DorkLand", "DRKL") {
    ...
    */
    function addPlot(uint256 id) public onlyOwner {
        reservedPlot[id] = false;
    }
    /**
    * add an array of address to the WL
    */
    function removePlots(uint256[] memory id) external onlyOwner {
    ...
    *
    */
    function joinPlots(uint256[] memory id) external onlyOwner {
        //TODO IMPLEMENT ME.
        // first ID in the array is the root
        // others get reserved plots, so can't be taken anymore
    }

    /**
    * remove an address off the WL
    */
    function removePlot(uint256 id) public onlyOwner {
        reservedPlot[id] = true;
    }

    function mint(uint256 x, uint256 y) external payable {
        require(msg.sender == tx.origin);
        require(saleIsActive, "Sale must be active to mint Tokens");
        require(totalSupply()+1 <= MAX_TOKENS - (9448-freeMintCounter), "Purchase would exceed max supply of Tokens");
        require(tokenPrice <= msg.value, "Ether value sent is not correct"); 
        uint256 id =  getId(x,y);
        require(!exists(id), "Plot already taken");        
        require(isAvailible(x,y), "Reserved plot");      
        _safeMint( msg.sender, id);  
    }

    function freeMint(uint256 x, uint256 y, uint256 dorkisId) external  {
        require(msg.sender == tx.origin);
        ...
        require(!exists(id), "Plot already taken");   
        require(isAvailible(x,y), "Reserved plot");  
        require(!hasMinted[dorkisId],"Dorkis already used");
        // TODO add check to see if you own the Dorkis (and/or Evil)
        // Evil Dorkis ID will be increased with 10k
        freeMintCounter +=1;
        hasMinted[dorkisId]=true;
        _safeMint( msg.sender, id);
    }

    function isAvailible(uint256 x, uint256 y) public view virtual returns (bool)  {
        return !reservedPlot[getId(x,y)];
    }

    function exists(uint256 x, uint256 y) public view virtual returns (bool)  {
        return exists(getId(x,y));
    }
    ...
    */
    function withdraw() public onlyOwner {
        ...
        _withdraw(FCCVIEW,(balance * 20) / 100);
        _withdraw(TORI,(balance * 20) / 100);
