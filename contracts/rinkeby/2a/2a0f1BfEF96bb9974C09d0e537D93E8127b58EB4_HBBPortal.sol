...
contract HBBPortal is ERC1155Holder, ERC721Holder, AccessControl, ReentrancyGuard {

    event PortalActivated(address indexed owner, uint256 indexed tokenId, address indexed contractAddress, ContractType contractType);
    event PortalDeactivated(address indexed owner, uint256 indexed tokenId, address indexed contractAddress, ContractType contractType);
    event WhitelistUpdated(address indexed token, ContractType indexed contractTypeBefore, ContractType indexed contractTypeAfter);
    event TimeDelayUpdated(uint256 indexed timeDelayBefore, uint256 indexed timeDelayAfter);
    ...
    modifier timeDelayPassed() {
        require(block.timestamp - lastExecutionTime > execution_delay, "HBBPortal: Portal is recharging");
        _;
    }
    ...
    mapping(address => mapping(address => mapping(uint256 => bool))) public balances;
        ...
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        ...
        emit WhitelistUpdated(_tokenToWhitelist, contractTypeBefore, _contractType);
    }
    ...
    function withdrawBatch(address token, uint256[] memory _ids) public nonReentrant timeDelayPassed {
        if (whitelist[token] == ContractType.ERC721) {
            ...
            for (uint256 i = 0; i < _ids.length; i++) {
                ...
                IERC721(token).safeTransferFrom(address(this), msg.sender, _ids[i]);
                emit PortalDeactivated(msg.sender, _ids[i], token, whitelist[token]);
            }
            ...
            IERC1155(token).safeBatchTransferFrom(address(this), msg.sender, _ids, _amounts, "");
        } else {
    ...
    function onERC1155Received(
        ...
        require(balances[msg.sender][from][id] == false, "HBBPortal: You already submitted this token");
