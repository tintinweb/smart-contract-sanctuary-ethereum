...
contract Omochi is Ownable, ERC721AQueryable {
  ...
  IOmochiItem OmochiItemContract = IOmochiItem(equipmentAddr);

  mapping(uint256 => string) private images;
  mapping(uint256 => Equipment) public equipments;

  constructor() ERC721A("Omochi", "OMC") {
  }

  function setImage(uint256 tokenId, string calldata svg) external {
    images[tokenId] = svg;
  }

  function getEquipment(uint tokenId) external view returns(Equipment memory){
    return equipments[tokenId];
  }

  function setEquipments(uint256 tokenId, uint256[] memory itemIds) external returns(Equipment memory) {
    uint256 headItemId;
    uint256 bodyItemId;
    uint256 footItemId;
    // if (OmochiItemContract.balanceOf(msg.sender, itemIds[0]) > 0) {
    //   headItemId = itemIds[0];
    // }
    // if (OmochiItemContract.balanceOf(msg.sender, itemIds[1]) > 0) {
    //   bodyItemId = itemIds[1];
    // }
    // if (OmochiItemContract.balanceOf(msg.sender, itemIds[2]) > 0) {
    //   footItemId = itemIds[2];
    // }
    Equipment memory equipment = Equipment(headItemId, itemIds[1], footItemId);
  ...
  function setEquipmentAddr(address _newEquipmentAddr) external onlyOwner {
    equipmentAddr = _newEquipmentAddr;
  }
  ...
  function tokenURI(uint256 tokenId) public override(ERC721A, IERC721A) view returns (string memory output) {
    string memory img = createSvg(tokenId);
    string memory imageURI = svgToImageURI(img);
    output = formatTokenURI(imageURI, tokenId);
    return output;
  }

  function bodySvg(uint256 tokenId) public view returns(string memory) {
    return images[tokenId];
  }

  function createSvg(uint256 tokenId) public view returns(string memory img) {
    ...
    string memory baseImg = '';
    string memory bodyImg = bodySvg(tokenId);
