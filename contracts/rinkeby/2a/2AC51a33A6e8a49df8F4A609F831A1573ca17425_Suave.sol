...
contract Suave is ERC20Burnable, Operator {
    using SafeMath8 for uint8;
    using SafeMath for uint256;

    // Initial distribution for the first 72h genesis pools
    uint256 public constant INITIAL_GENESIS_POOL_DISTRIBUTION = 34992 ether;
    ...
    /**
     * @notice Constructs the SUAVE ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("SUAVE", "SUAVE") {
        // Mints 1 SUAVE to contract creator for initial pool setup
    ...
    function _getSuavePrice() internal view returns (uint256 _suavePrice) {
        try IOracle(suaveOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Suave: failed to fetch SUAVE price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _suavePrice) internal returns (uint256){
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_suavePrice >= taxTiersTwaps[tierId]) {
    ...
    function setSuaveOracle(address _suaveOracle) public onlyOperatorOrTaxOffice {
        require(_suaveOracle != address(0), "oracle address cannot be 0 address");
        suaveOracle = _suaveOracle;
    }
    ...
    /**
     * @notice Operator mints SUAVE to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of SUAVE to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentSuavePrice = _getSuavePrice();
            currentTaxRate = _updateTaxRate(currentSuavePrice);
            if (currentSuavePrice < burnThreshold) {
                burnTax = true;
            }
        }
