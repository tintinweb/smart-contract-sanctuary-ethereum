...
contract KeysTest is ERC721, Ownable, VRFConsumerBase {
    ...
    //TODO: get has of folder
    string constant baseURI = "ipfs://QmPgeRwwb71TvSdpY9ALdeyD35mxLLS8g8TbwSMWgvxR7K/";
    ...
    uint publicSupply = 800;

    uint publicMinted;

    uint whitelistSupply = 88;

    uint whitelistMinted;

    mapping(address => bool) whitelistAddressesMinted;

    mapping(address => bool) publicAddressesMinted;

    mapping(uint => address) public stakedPasses;

    uint[] public passRarities;

    event Staked(address indexed owner, uint indexed tokenId);

    event UnStaked(address indexed owner, uint indexed tokenId);

    constructor(uint[] memory _passRarities, address _vrfCoordinator, address _link) 
        ERC721("KeysTest", "KeysTest")
        VRFConsumerBase(_vrfCoordinator, _link)
    {

        passRarities = _passRarities;
    ...
    */
    ...
    function publicMint() external {
        ...
        require(publicMinted < publicSupply, "All Minted");

        _safeMint(msg.sender, publicMinted + whitelistMinted);
    ...
    function whitelistMint(bytes32[] calldata _proof) external {
        ...
        require(whitelistAddressesMinted[msg.sender] == false, "Already minted");

        require(whitelistMinted < whitelistSupply, "All Minted");
        ...
        whitelistAddressesMinted[msg.sender] = true;
 ...
 \|___||____|/                  \|___|      \|_____|\|_____||_____|/  |____|/  |___________|/  |______|/|_____|/  \|_____|     |\|_|/ 
    ...
    */
    function getKeyType(uint tokenId) public view returns(uint) {
        ...
        uint[] memory rarities = passRarities;
    ...
    */
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        return bytes(baseURI).length != 0 ? string(abi.encodePacked(baseURI, passType.toString())) : '';
    }
    ...
    */
    ...
    function reveal(uint range) external onlyOwner {
     ...
     function revealTest(uint range) external onlyOwner {
        ...
        revealData.push(RevealData(range, randomness));
