...
contract GoldenEggi is ERC721Enumerable, AccessControl {
    ...
    function setDiplomaNFT(address _diplomaNFT) external onlyAdmin {
        require(_diplomaNFT != address(0), "invalid address");
    ...
    function preSale(uint256 _num) public payable returns (bool result) {
        ...
        for (uint256 i = 0; i < _num; i++) {
            ...
            IDiploma(diplomaNFT).mint(msg.sender);
    ...
    function publicSale(uint256 _num) public payable returns (bool result) {
        ...
        for (uint256 i = 0; i < _num; i++) {
            uint256 diplomaCurrentTokenId = IDiploma(diplomaNFT).getCurrentTokenId();
            uint256 diplomaNFTCap = IDiploma(diplomaNFT).cap();
            uint256 remainingDiploma = diplomaNFTCap - diplomaCurrentTokenId;
            if((diplomaCurrentTokenId <= diplomaNFTCap) && (remainingDiploma != 0)) {
                IDiploma(diplomaNFT).mint(msg.sender);
            }
