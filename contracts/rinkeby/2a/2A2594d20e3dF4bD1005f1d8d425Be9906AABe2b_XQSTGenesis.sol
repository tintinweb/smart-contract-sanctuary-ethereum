...
contract XQST_RENDER {
        ...
        pure
    {
        ...
        while (pixelNum < svgData.totalPixels) {
            ...
            if (
                colorIndex == svgData.backgroundColorIndex &&
                svgData.hasBackground
            ) {
                pixelNum++;
                continue;
            }
        ...
        view
    {
        ...
        if (svgData.bpp == 1) {
            ...
            for (uint256 i = 0; i < svgData.totalPixels; i++) {
                svgData.colorIndexLookup[i] = uint8(
                    data[i + svgData.dataStart]
                );
            }
        }
    ...
    function _setupNumberLookup(SVGMetadata memory svgData) internal view {
        ...
        max =
            (svgData.width > svgData.height ? svgData.width : svgData.height) +
            1;
        ...
        view
    {
        ...
        if (svgMetadata.numColors > 0) {
            require(
                data.length >=
                    svgMetadata.paletteStart + svgMetadata.numColors * 8,
...
contract XQSTGenesis is ERC721, Ownable, ReentrancyGuard {
    using DynamicBuffer for bytes;

    error DataAlreadyExists();
    error IncorrectPaymentAmount();

    uint256 public constant MINT_PRICE = .064 ether;
    ...
    IXQST_Render xqstgfx;

    constructor(address gfx) ERC721("XQST", "XQST") {
        xqstgfx = IXQST_Render(gfx);
    }

    function mint(uint256 tokenId, bytes memory data) public payable {
        if (storedData[keccak256(data)] == true) revert DataAlreadyExists();
        if (msg.value != MINT_PRICE) revert IncorrectPaymentAmount();

        // TODO validate data

        tokenData[tokenId] = SSTORE2.write(data);
        _safeMint(msg.sender, tokenId);
        numMinted++;
    }

    function tokenURI(uint256 id) public view override returns (string memory) {
        bytes memory output = DynamicBuffer.allocate(2**19);

        output.appendSafe(
            '{"name": "XQST Genesis", "description": "XQST Genesis Token", "image": "data:image/svg+xml;base64,'
        );

        output.appendSafe(
            bytes(
                Base64.encode(
                    bytes(xqstgfx.renderSVG(SSTORE2.read(tokenData[id])))
                )
            )
        );
        output.appendSafe('"}');
