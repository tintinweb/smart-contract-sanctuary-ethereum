...
contract Swap is BaseContract
{
     ...
     */
    function swap(address in_, address out_, uint256 amount_) external whenNotPaused returns (bool)
    {
        IUniswapV2Router02 _router_ = IUniswapV2Router02(addressBook.get("router"));
        require(address(_router_) != address(0), "Router not set");
        IERC20 _in_ = IERC20(in_);
        IERC20 _out_ = IERC20(out_);
        require(_in_.transferFrom(msg.sender, address(this), amount_), "In transfer failed");
        address[] memory _path_ = new address[](2);
        _path_[0] = in_;
        _path_[1] = out_;
        _in_.approve(address(_router_), amount_);
        ...
        return _out_.transfer(msg.sender, _out_.balanceOf(address(this)));
    }
}
