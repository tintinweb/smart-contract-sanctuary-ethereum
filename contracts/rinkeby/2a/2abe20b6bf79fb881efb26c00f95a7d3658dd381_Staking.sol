...
contract Staking is Ownable {
    ...
    uint256 constant MIN_VAULT_DAYS = 2;
    ...
    function initialize(IERC20 _token, uint256 _startTime) public onlyOwner {
    ...
    function withdraw() public {
        require(checkEligibility(msg.sender), "Not eligible to withdraw");

        UserInfo storage user = userInfo[msg.sender];

        uint256 userReward = calculateReward(msg.sender).sub(user.rewardDebt);
        ...
        onlyOwner
    {
        ...
        require(balanceOnContract >= _amount, "Not enough amount");
    ...
    function getUserCurrentTier(address _user)
        public
        view
        ...
        returns (uint256 reward)
    {
        ...
        reward = calculateReward(_user).sub(user.rewardDebt);
    }
        ...
        returns (uint256 reward)
    {
        ...
        uint256 tier = getUserCurrentTier(_user);

        reward = (user.depositAmount.mul(10).div(100)).mul(tier);
    }

    function checkEligibility(address _user)
        public
        view
    ...
    function checkRewardEligibility(address _user)
        public
        view
        returns (bool eligible)
    {
        if (
            ...
            (claimTierStatus[getUserCurrentTier(_user)]) != true
        ) {
            eligible = true;
        } else {
            eligible = false;
    ...
    function claimReward() public {
        require(
            checkRewardEligibility(msg.sender),
        ...
        uint256 userTiers = getUserCurrentTier(address(msg.sender));
    ...
    function rewardReinvest() public {
        require(
            checkRewardEligibility(msg.sender),
