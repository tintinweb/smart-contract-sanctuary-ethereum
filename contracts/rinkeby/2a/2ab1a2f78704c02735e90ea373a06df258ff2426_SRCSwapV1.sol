...
abstract contract ChainlinkPriceFeed is Ownable {
    ...
    // 設定新的查價地址
    function setChainlinkPriceFeed(address newAddress) public onlyOwner {
        _priceFeed = AggregatorV3Interface(newAddress);
    }

    // 查詢查價地址
    function showChainlinkPriceFeed() public view returns (address) {
...
contract SRCSwapV1 is Ownable, ChainlinkPriceFeed {
    ...
    // 設定新的兌換價格
    function setUsdPerSRC(uint256 newPrice) public onlyOwner {
        usdPerSRC = newPrice;
    }
    ...
    function withdrawOfUSDT(uint256 amountOfUSDT) public onlyOwner {        
        ...
        require(ownerBalance >= amountOfUSDT, "Vendor has insufficient funds(USDT)");        
