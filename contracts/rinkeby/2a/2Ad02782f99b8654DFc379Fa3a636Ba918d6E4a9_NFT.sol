...
contract NFT is ERC721, Ownable {
    ...
    function mintSbtOmnes() public payable returns (uint256) {
    ...
    function mintAirdropfree(address recipient) public payable onlyOwner returns (uint256) {
        uint256 newTokenId = ++currentTokenId;
        if (newTokenId > TOTAL_SUPPLY) {
            revert MaxSupply();
        }
...
library Strings {
    ...
    /**
     * @dev Converts an `address` with fixed length of 20 bytes to 
     its not checksummed ASCII `string` hexadecimal representation.
     */
