...
contract RightsDeclaration is Ownable {
  ...
  // TODO implment access control (whitelist)
  ...
  function declare(
    ...
    string calldata iscc, 
    string calldata isccURI, 
    string calldata isccMessage)
    public returns (uint256) 
  {
    MintangibleRegistrar(_isccAddr).declare(iscc, isccURI, isccMessage);
    uint rightsID = RightsRegistry(_registryAddr).declare(contractAddr, tokenID, rightsURI);
    return rightsID;
  }
