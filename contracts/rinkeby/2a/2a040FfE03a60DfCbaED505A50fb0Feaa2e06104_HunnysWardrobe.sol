...
contract HunnysWardrobe is ERC1155Supply, Ownable {  
    ...
    // constructor (string memory newBaseURI, address hunAddress) ERC1155 ("Hunnys Wardrobe") {
    constructor (string memory newBaseURI) ERC1155 ("Hunnys Wardrobe") {
    ...
    function getProducts() public view returns(ProductData[] memory) {
        return products;
    }
    ...
    function mintProducts(uint256[] memory _ids, uint256[] memory _amounts) public payable {
        ...
        for (uint256 i = 0; i < _ids.length; i++) {
            ...
            if(currentData.isLimited){
                require( currentData.currentCount + _amounts[i] <= currentData.limit,   "Can't mint more than max supply" );
            }

            // set price
            totalPrice = totalPrice + (currentData.price * _amounts[i]);
        }
        ...
        for (uint256 i = 0; i < _ids.length; i++) {
            ...
            uint256 currentCount = currentProduct.currentCount;
            ...
            for (uint256 k = 0; k < _amounts[i]; k++) {
                ...
                // console.log("Current uri at mint", append(baseTokenURI, currentProduct.uriString, Strings.toString(currentCount + k)));
                // console.log("Current newItemId", newItemId);
                
                if(currentProduct.isLimited) {
                    setTokenUri(newItemId, append(baseTokenURI, currentProduct.uriString, Strings.toString(0)));
                } else {
                    setTokenUri(newItemId, append(baseTokenURI, currentProduct.uriString, Strings.toString(currentCount + k)));
                }
    ...
    function addNewProducts(string[] memory _uriString, uint256[] memory _prices, bool[] memory _redeemable, bool[] memory _isLimited, uint256[] memory _limits) public onlyOwner {
        uint256 currentId = products.length;
        for(uint256 i = 0; i < _uriString.length; i++){
            ProductData memory newProduct = ProductData(currentId + i, _uriString[i], _prices[i], _redeemable[i], _isLimited[i], 0, _limits[i]);
            products.push(newProduct);
        }
    }

    // Change existing product by id 
    function changeProduct(uint256 _id, string memory _uriString, uint256 _price, bool _redeemable, uint256 _limit) public onlyOwner {
    ...
    function withdrawTeam(uint256 amount) public payable onlyOwner {
        ...
        require(payable(a1).send(percent * 70)); // 70% to Stacy
