...
contract DiamondCo2 is ERC721AUpgradeable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
  ...
  uint256 public maxPublicMintAmountPerWallet;
  ...
  uint256 public whitelist1MintCost;
  uint256 public whitelist2MintCost;

  bytes32 public merkleRoot1;
  bytes32 public merkleRoot2;
  
  bool public paused;
  bool public whitelist1MintEnabled;
  bool public whitelist2MintEnabled;
  bool public revealed;

  function initialize(string memory _tokenName, string memory _tokenSymbol, string memory _hiddenMetadataUri) initializerERC721A public {
    ...
    uriSuffix = '.json';
    maxSupply = 5000;
    maxPublicMintAmountPerWallet = 2;
    maxWhitelist1MintAmountPerWallet = 2;
    maxWhitelist2MintAmountPerWallet = 1;

    publicMintCost = 0.2 ether;
    whitelist1MintCost = 0.13 ether;
    whitelist2MintCost = 0.15 ether;
    paused = true;
    whitelist1MintEnabled = false;
    whitelist2MintEnabled = false;
    revealed = false;
  }
  ...
  function whitelistMint1(uint256 _mintAmount, bytes32[] calldata _merkleProof) public payable mintCompliance(_mintAmount) {
    ...
    require(whitelist1MintEnabled, 'The whitelist1 sale is not enabled!');
    require(mintedTokens[_msgSender()] + _mintAmount <= maxWhitelist1MintAmountPerWallet, 'Max limisted per wallet!');
    require(msg.value >= whitelist1MintCost * _mintAmount, 'Insufficient funds for whitelist1!');
    bytes32 leaf = keccak256(abi.encodePacked(_msgSender()));
    require(MerkleProofUpgradeable.verify(_merkleProof, merkleRoot1, leaf), 'Invalid proof for whitelist1!');
  ...
  function whitelistMint2(uint256 _mintAmount, bytes32[] calldata _merkleProof) public payable mintCompliance(_mintAmount) {
    ...
    require(whitelist2MintEnabled, 'The whitelist2 sale is not enabled!');
    require(mintedTokens[_msgSender()] + _mintAmount <= maxWhitelist2MintAmountPerWallet, 'Max limited per wallet');
    require(msg.value >= whitelist2MintCost * _mintAmount, 'Insufficient funds for whitelist2!');
    bytes32 leaf = keccak256(abi.encodePacked(_msgSender()));
    require(MerkleProofUpgradeable.verify(_merkleProof, merkleRoot2, leaf), 'Invalid proof for whitelist2!');
  ...
  function setMaxPublicMintAmountPerTx(uint256 _maxPublicMintAmountPerWallet) public onlyOwner {
    maxPublicMintAmountPerWallet = _maxPublicMintAmountPerWallet;
  }
