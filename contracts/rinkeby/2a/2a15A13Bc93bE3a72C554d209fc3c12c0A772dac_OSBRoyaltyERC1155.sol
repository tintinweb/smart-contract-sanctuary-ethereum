...
contract OSBRoyaltyERC1155 is ERC1155, ERC2981, Ownable {
    ...
    function mintBatchWithRoyalty(string memory _baseUri, uint256[] memory _amounts, address[] memory _receiverRoyaltyFees, uint96[] memory _percentageRoyaltyFees) external onlyOwnerOrController {
        ...
        for (uint256 i; i < _amounts.length; i++) {
            ...
            if (_receiverRoyaltyFees[i] == address(0) && _percentageRoyaltyFees[i] == 0) {
                ...
                _mintAndSetRoyalty(id, _amounts[i], receiverRoyaltyFeeDefault, _percentageRoyaltyFees[i]);
            } else {
                _mintAndSetRoyalty(id, _amounts[i], _receiverRoyaltyFees[i], _percentageRoyaltyFees[i]);
            }
        }
        baseURI = _baseUri;
        lastedId = id;
    ...
    function _mintAndSetRoyalty(uint256 _tokenId, uint256 _amount, address _receiverRoyaltyFee, uint96 _percentageRoyaltyFee) private {
        require(_percentageRoyaltyFee > 0, "Requires royalty fee more significant than 0");
...
import "../contracts/OSBRoyaltyERC1155.sol";

contract $OSBRoyaltyERC1155 is OSBRoyaltyERC1155 {
    constructor(address _owner, string memory _name, string memory _symbol, address _receiverRoyaltyFee, uint96 _percentageRoyaltyFee) OSBRoyaltyERC1155(_owner, _name, _symbol, _receiverRoyaltyFee, _percentageRoyaltyFee) {}
