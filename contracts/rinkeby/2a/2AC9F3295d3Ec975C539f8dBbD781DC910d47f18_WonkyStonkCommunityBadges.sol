...
contract WonkyStonkCommunityBadges is ERC1155, Ownable {
    ...
    address[] PinkAddressArray =  [0xED867A4D24eDDa0F0b83349241D883aa45B906c1];
    address[] BlueAddressArray =  [0xED867A4D24eDDa0F0b83349241D883aa45B906c1];  
    address[] GoldAddressArray =  [0xED867A4D24eDDa0F0b83349241D883aa45B906c1];
    ...
    constructor() ERC1155('https://gateway.pinata.cloud/ipfs/QmSFDVm5HXwWrpdsGFVvuShYYxWPG1xj9f3nnuFXtwqaLh/{id}.json') {
        ...
        for(uint256 i=0; i < PinkAddressArray.length; i++) {
            _mint(PinkAddressArray[i], PINK, 200, "");
        }

        for(uint256 i=0; i < BlueAddressArray.length; i++) {
        _mint(BlueAddressArray[i], BLUE, 50, "");
        }

        for(uint256 i=0; i < GoldAddressArray.length; i++) {
            _mint(GoldAddressArray[i], GOLD, 5, "");
        }
