...
contract PotLottery is ReentrancyGuard {
    ...
    event SwapError(string tokenName, string reason);
    ...
    function swapAccumulatedFees() external validPLSP nonReentrant {
        ...
        for (uint256 i = 0; i < tokenWhiteListNames.length; i++) {
            ...
            if (keccak256(bytes(tokenName)) == keccak256(bytes("PULSE")))
                continue;
            ...
            if (balance > 0) {
                ...
                returns (uint256[] memory amounts) {
                    uint256 swappedETH = amounts[1];
                } catch Error(string memory reason) {
                    emit SwapError(tokenName, reason);
                }
            }
        }
