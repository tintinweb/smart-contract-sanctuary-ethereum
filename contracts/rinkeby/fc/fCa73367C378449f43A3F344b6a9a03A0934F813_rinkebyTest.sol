...
contract rinkebyTest is ERC721A, Pausable, ReentrancyGuard, Ownable {
    using Strings for uint256;
    //Hardcoded for testing purposes
    bytes32 public ogMerkleRoot =
        0xb925e25c93e0f3b4f1788dda1de423f0594a91627fb1dc6f1293c0f747977eeb;
    bytes32 public wlMerkleRoot =
        0xc894eed000194fff955db19778117d2dfc64db833ce1ad3ac33b7332871dbb63;
    bytes32 public teamMerkleRoot =
        0xc1f04a9ba87475a1cdaf373a26a44e3ccaa6885b0cd4e8b008f7d8fc6594d540;
    ...
    //Hardcoded for testing purposes
    address private treasury = 0xAb8483F64d9C6d1EcF9b849Ae677dD3315835cb2;
    mapping(address => walletMintData) public addressTokenMintData;
    mapping(address => bool) public adminAddresses;
    mapping(address => uint256) public teamMintsClaimed;
    mapping(uint256 => bool) public token1Claimed;
    mapping(uint256 => bool) public token2Claimed;
    ...
    //Data form for addressTokenMintData mapping
    //Optimization for bits needed
    ...
    constructor(IERC721 _token1, IERC721 _token2)
        payable
        ERC721A("rinkebyTest", "rinkebyTest")
    {
        _safeMint(treasury, 20);
        token1 = IERC721(_token1);
        token2 = IERC721(_token2);
        //Hardcoded for testing purposes
        saleConfig.maxSupply = 3333;
        ...
        saleConfig.sale = false;
        saleConfig.dtcPhase = true;
        saleConfig.maxPublicMint = 20;
        saleConfig.publicReserve = 5;
        saleConfig.publicSupply = 0;
    }

    event amountExceedsPublicSupply(uint256 _amount, uint256 _publicSupply, uint256 _publicReserve);
        ...
        whenNotPaused
    {
        ...
        uint256 _token2Amount = token2.balanceOf(msg.sender);
        ...
        //Calculate mint price based on role
        while (_mints < _amount) {
            if (_mints + totalSupply() == _maxSupply) {
                ...
                _token1Amount > 0 &&
                addressTokenMintData[msg.sender].token1 < _token1Amount
            ) {
                unchecked {
                    addressTokenMintData[msg.sender].token1++;
                    addressTokenMintData[msg.sender].free++;
                    _mints++;
                }
            } else if (
                _token2Amount > 0 &&
                addressTokenMintData[msg.sender].token2 < _token2Amount
            ) {
                unchecked {
                    addressTokenMintData[msg.sender].token2++;
                    addressTokenMintData[msg.sender].free++;
                    _mints++;
                }
            } else if (
        ...
        returns (bool result)
    {
        ...
        uint256 _token1Amount = token1.balanceOf(msg.sender);
        uint256 _token2Amount = token1.balanceOf(msg.sender);
        if (
            _isTeam == true ||
            _token1Amount > 0 ||
            _isOG == true ||
    ...
    //Remove function maybe?
    //Returns address' max mints (freeMints, ogMints, wlMints)
        ...
        returns (holderMaxMints memory)
    {
        ...
        for (uint256 i = 0; i < token1.balanceOf(_wallet); i++) {
            _holderMaxMints.freeMints++;
            _holderMaxMints.ogMints += 3;
            _holderMaxMints.wlMints += 2;
        }
        if (token2.balanceOf(_wallet) > 0) {
            _holderMaxMints.freeMints++;
        }
    ...
    //Returns the amount of tokens the address holds (token1, token2)
    ...
    //ADD onlyAdminOrOwner modifier!!
    function setAdminAddresses(address[] calldata _wallets) external {
        for (uint256 i = 0; i < _wallets.length; i++) {
            adminAddresses[_wallets[i]] = true;
        }
    }

    function removeAdminAddresses(address[] calldata _wallets) external {
        for (uint256 i = 0; i < _wallets.length; i++) {
    ...
    function setPrice(uint16 _newPrice) external {
        saleConfig.price = _newPrice;
    }

    function setPublicReserve(uint16 _newReserve) external {
        require(
    ...
    function releaseDTCReserve() external {
    ...
    function setTeamMerkleRoot(bytes32 _newTeamMerkleRoot) external {
        teamMerkleRoot = _newTeamMerkleRoot;
    }

    function setTeam2MerkleRoot(bytes32 _newTeam2MerkleRoot) external {
        team2MerkleRoot = _newTeam2MerkleRoot;
    }

    function setOGMerkleRoot(bytes32 _newOGMerkleRoot) external {
