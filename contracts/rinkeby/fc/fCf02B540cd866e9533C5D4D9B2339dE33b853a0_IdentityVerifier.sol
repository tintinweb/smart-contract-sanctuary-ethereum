...
contract IdentityVerifier is AdminControl, IIdentityVerifier {
    ...
    mapping(uint => bytes32) public _merkleRoots;

    // Maps listing ID to adhoc allowed addresses
    mapping(uint => mapping(address => bool)) public adHocAddresses;

    function setMerkleRoot(uint listingId, bytes32 merkleRoot) public adminRequired {
      _merkleRoots[listingId] = merkleRoot;
    }

    function onAllowList(address claimer, bytes32[] memory proof, uint listingId) public view returns(bool) {
    ...
    function addAdHocAddresses(uint listingId, address[] calldata addresses) public adminRequired {
        for (uint i; i < addresses.length; i++) {
            adHocAddresses[listingId][addresses[i]] = true;
        }
    }

    function removeAdHocAddresses(uint listingId, address[] calldata addresses) public adminRequired {
        for (uint i; i < addresses.length; i++) {
            adHocAddresses[listingId][addresses[i]] = false;
        }
    }
    ...
    function verify(uint40 listingId, address identity, address, uint256, uint24, uint256, address, bytes calldata data) external override view returns (bool) {
        bytes32[] memory proof = abi.decode(data, (bytes32[]));
        if (onAllowList(identity, proof, listingId)) return true;
        if (adHocAddresses[listingId][identity]) return true;
        return false;
    }
}
