...
contract codenameZebra  is ERC721, ERC721Enumerable, Ownable, ReentrancyGuard {
  using SafeMath for uint8;
  using SafeMath for uint256;
  // total tokens
  uint256 public constant MAX_TOKENS = 2000;
  // maximum tokens for giveaways
  uint256 public constant MAX_GIVEAWAY_TOKENS = 20; 
  ...
  constructor(string memory baseURI) ERC721("codenameZebra", "zcode") {
    setBaseURI(baseURI);
  }
  ...
  function mintGiveaway(uint256 numTokens) public onlyOwner {
    require(countMintedGiveawayTokens.add(numTokens) <= MAX_GIVEAWAY_TOKENS, "20 tokens max");
    for (uint256 i = 0; i < numTokens; i++) {
