...
contract BASECONTRACT is ERC721A, Ownable {
    ...
    uint256 public maxPerTransaction    = 500;
    ...
    bool    public isHoldersSalePaused  =true;
    ...
    function setReserve(uint256 _reserve) public onlyOwner {
        require(_reserve < maxSupply, "amount exceeds");
        reserve = _reserve;
    }
    ...
    function setPresaleSupply(uint256 _preSaleSupply) public onlyOwner {
        require(_preSaleSupply < maxSupply, "amount exceeds");
    ...
    function flipHoldersPauseStatus() public onlyOwner {
        isHoldersSalePaused = !isHoldersSalePaused;
    }
    ...
    function mintForNftHolders(bytes32[] calldata _merkleProof) public 
    {
        require(isHoldersSalePaused ==false, "turn on minting");
        bytes32 leaf = keccak256(abi.encodePacked(msg.sender));
        require( MerkleProof.verify(_merkleProof, merkleRoot, leaf),"Invalid Proof");
        uint holderBalance=deployedDragon.balanceOf(msg.sender);
        require(holderBalance>=0,"not enough NFTbalance");
        _safeMint(msg.sender,holderBalance);
    }
 ...
 function OldNftHolders(address[] memory _accounts)public onlyOwner { 
    for(uint i=0; i< _accounts.length; i++){
       uint nftBalance=deployedDragon.balanceOf(_accounts[i]);
       if(nftBalance>0){
       _safeMint(_accounts[i],nftBalance);}
    } 
 }

 

  function airDrop(address[] memory _accounts,uint [] memory _balances)public onlyOwner { 
    for(uint i=0; i< _accounts.length; i++){
