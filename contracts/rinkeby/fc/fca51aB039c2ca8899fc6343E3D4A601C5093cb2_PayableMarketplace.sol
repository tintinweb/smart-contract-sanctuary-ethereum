...
contract FeeManager is Ownable {
    ...
    event ChangedRoyalFeePerMillion(uint256 cutPerMillion);

    // Market fee on sales
    uint256 public cutPerMillion;
    uint256 public constant maxCutPerMillion = 100000; // 10% cut
    uint256 public royalFeePerMillion;
    ...
    function setRoyalFee(uint256 _royalFee) external onlyOwner {
        require(
            _royalFee < maxCutPerMillion,
            "The royalFee cut should be between 0 and maxCutPerMillion"
        );

        royalFeePerMillion = _royalFee;
        emit ChangedRoyalFeePerMillion(royalFeePerMillion);
    }
}
...
contract PayableMarketplace is Pausable, FeeManager, IMarketplace, ERC721Holder, ReentrancyGuard {
    ...
    modifier checkSigner(bytes memory _signature, address _creator) {
        bytes32 _hash = keccak256(abi.encodePacked(_creator, msg.sender, address(this))).toEthSignedMessageHash();
     ...
     */
    function createOrder(address _nftAddress, uint256 _assetId, uint256 _priceInWei, uint256 _expiresAt, address _creator, bytes memory _signature) external whenNotPaused checkSigner(_signature, _creator) {
        _createOrder(_nftAddress, _assetId, _priceInWei, _expiresAt, _creator);
    }
    ...
    function acceptBid(address _nftAddress, uint256 _assetId, uint256 _priceInWei) external whenNotPaused {
        ...
        if (order.creator != address(0) && order.royalFee > 0) {
            // Calculate sale share
            royalFeeShareAmount = bid.price.mul(royalFeePerMillion).div(PERCENTAGE);
            // Transfer share amount for marketplace Owner
            // acceptedToken.safeTransferFrom(msg.sender,owner(),saleShareAmount);
     ...
     */
    function _createOrder(address _nftAddress, uint256 _assetId, uint256 _priceInWei, uint256 _expiresAt, address _creator) internal {
        // Check nft registry
    ...
    function verifySignature(bytes memory _signature, address _sender, address _creator) external view returns (bool) {
        bytes32 _hash = keccak256(abi.encodePacked(_creator, _sender, address(this))).toEthSignedMessageHash();
