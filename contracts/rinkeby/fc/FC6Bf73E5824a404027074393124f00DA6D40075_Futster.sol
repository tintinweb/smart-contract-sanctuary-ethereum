...
contract Futster is ERC721, ERC721Enumerable, Pausable, ERC721Burnable, ERC721Royalty, Timelock {
    ...
    constructor(string memory _name, string memory _symbol, address _signer, address _authority) 
        ERC721(_name, _symbol) 
        Timelock(_signer, _authority) {
    ...
    function safeMint(address to, uint256 dna) public onlyGovernor authorized {
    ...
    function setApprovalForAll(address operator, bool approved) public override(ERC721, IERC721) onlyOwnerApproveControl {
        _setApprovalForAll(_msgSender(), operator, approved);
    }
    
    function approve(address to, uint256 tokenId) public override(ERC721, IERC721) onlyOwnerApproveControl {
...
contract Timelock is VerifySignature, AccessControlled {
    ...
    constructor(address _signer, address _authority) VerifySignature() 
    ...
    function setSigner(address _signer) public onlyGovernor {
        require(_signer != authority.governor(),"Signer can't be the owner");
        signer = _signer;
    ...
    function timeLockGrantUpdate(uint blocknumber, uint nonce, bytes32 digest, bytes memory signature) external returns (bool){
        require(verify(blocknumber, authority.governor(), nonce, digest, signature, signer), "Signed Message is not message sent");
