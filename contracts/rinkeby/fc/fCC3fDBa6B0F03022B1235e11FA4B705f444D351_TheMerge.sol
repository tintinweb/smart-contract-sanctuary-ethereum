...
contract TheMerge is IDeck, ERC721Enumerable, ERC721Burnable, ERC2981, Ownable, Pausable {
    ...
    /// @dev POS will take over when a specific Terminal Total Difficulty is reached.
    /// We require this block difficulty before anyone can mint to ensure that POS has actually begun
    uint256 public requiredTTD;
     ...
     * @param requiredTTD_ The terminal total difficulty value when POS will take over POW. e.g. 58750000000000000000
     */
    constructor(
    ...
    function canMint() external view returns (bool) {
        return block.difficulty >= requiredTTD && !paused();
    }

    /// @dev There is a chance that the TTD value will change as the Ethereum team launches.
    /// This function is to ensure we can adjust to their rollout plan
    function setRequiredTTD(uint256 difficulty) public onlyOwner {
        requiredTTD = difficulty;
    }
    ...
    function mintDeck(uint256 count) external payable whenNotPaused returns (bool) {
        require(block.difficulty >= requiredTTD, "DECK: Required difficulty too low");
        require(balanceOf(msg.sender) + count <= maxCountPerWallet, "DECK: Max mint for wallet reached");
        require(_tokenIds.current() + count <= maxCount, "DECK: Max mint count reached");
