...
import { IStakingV2Pool } from "./interfaces/IStakingV2Pool.sol";
import { Bond } from "./libraries/bonds/Bond.sol";
import { Constants } from "./libraries/Constants.sol";

/// @title ERC721 Staking Rewards
/// @notice Staking rewards contracts for Illuvium ERC721 Bonds.
/// @dev This contract is supposed to be opinionated to Illuvium's protocol architecture.
/// It uses ILV token as the default bonds underlying and integrates with Illuvium's
/// Staking v2 contracts.
/// @dev Bonds in the BondsRegistry contract have a locked ILV payout claimable
/// upon expiration. This value is read by this contract and used to allocate yield and
/// revenue distribution rewards.
/// @dev Yield is defined as over time ILV token rewards, allocated by the pool factory
/// using staking v2's reward allocation weights system (see {weight} state variable and
/// {factory.totalWeight()}).
/// @dev Revenue distributions are ILV rewards coming from Illuvium's Vault contract, which
/// receives ETH from in-game fees, swaps for ILV and sends to staking v2 pool + this contract.
/// The vault determines how much ILVs in revenue distributions have to be sent to this contract
/// (through {receiveVaultRewards()}), which then are allocated proportionally to the staked bonds.
...
{
    /* ======== LIBRARIES ======== */
    using Bond for Bond.Stake;
    using SafeERC20Upgradeable for IERC20Upgradeable;
    using SafeCastUpgradeable for uint256;
    /* ======== STATE VARIABLES ======== */
    /// @dev Bonds registry contract address
    IBondsRegistry public bondsRegistry;
    /// @dev ILV token contract address
    IERC20Upgradeable public ilv;
    /// @dev ILV Staking v2 pool factory address
    IFactory public factory;
    /// @dev ILV Staking v2 pool contract address
    IStakingV2Pool public ilvPool;
    /// @dev ILV Vault contract address
    address public vault;
    /// @dev Total amount of ILV tokens supplied
    uint160 public underlyingSupplied;
    /// @dev Pool weight registered in staking v2 (rewards allocation weight)
    uint32 public weight;
    /// @dev Last timestamp value of when a yield distribution happened
    /// (see updateReward modifier)
    uint64 public lastYieldDistribution;
    /// @dev Global checkpoint value for calculating yield distributed per ILV token
    uint256 public yieldPerTokenStored;
    /// @dev Global checkpoint value for calculating revDis (Revenue Distribution)
    /// distributed per ILV token
    uint256 public revDisPerToken;
    /// @inheritdoc IStakingRewards
    mapping(address => mapping(uint256 => Bond.Stake)) public stakedBonds;

    /* ======== EVENTS ======== */
    event LogStake(address indexed caller, uint256 indexed tokenId, uint128 value);
    event LogUnstake(address indexed caller, uint256 indexed tokenId, uint128 value);
    event LogClaimRewards(
        address indexed caller,
        uint256 indexed tokenId,
        uint256 claimedYield,
        uint256 claimedRevDis,
        bool useSILV
    );
    event LogSetWeight(address indexed caller, uint32 oldWeight, uint32 newWeight);
    event LogReceiveVaultRewards(address indexed vault, uint256 reward);

    /* ======== MODIFIERS ======== */
    modifier updateReward(address _from, uint256 _tokenId) {
        if (factory.shouldUpdateRatio()) {
            factory.updateILVPerSecond();
        }
        yieldPerTokenStored = yieldPerToken();
        uint256 nextYieldDistribution = lastTimeYieldApplicable();
        if (lastYieldDistribution < nextYieldDistribution) lastYieldDistribution = nextYieldDistribution.toUint64();
        Bond.Stake storage stakedBond = stakedBonds[_from][_tokenId];
        stakedBond.pendingYield = stakedBond.earnedYield(yieldPerTokenStored).toUint128();
        stakedBond.pendingRevDis = stakedBond.earnedRevDis(revDisPerToken);
        stakedBond.yieldPerTokenPaid = yieldPerTokenStored;
        stakedBond.revDisPerTokenPaid = revDisPerToken;
        _;
    }
    ...
    modifier onlyFactory() {
        require(msg.sender == address(factory), "only factory allowed");
        _;
    }
    ...
    function initialize(
        ...
        IERC20Upgradeable _ilv,
        IFactory _factory,
        IStakingV2Pool _ilvPool,
        address _vault,
        uint64 _initTime,
        uint32 _weight
        ...
        require(
            address(_bondsRegistry) != address(0) &&
                address(_ilv) != address(0) &&
                address(_factory) != address(0) &&
                address(_ilvPool) != address(0) &&
                _vault != address(0) &&
                _initTime > 0 &&
                _initTime < _factory.endTime() &&
                _weight > 0,
    ...
    /// @inheritdoc IStakingRewards
    function lastTimeYieldApplicable() public view virtual returns (uint256) {
        uint256 endTime = factory.endTime();
        return block.timestamp < endTime ? block.timestamp : endTime;
    }

    /// @inheritdoc IStakingRewards
    function pendingYieldFor(address _who, uint256 _tokenId) public view virtual returns (uint256) {
        return stakedBonds[_who][_tokenId].earnedYield(yieldPerToken());
    }

    /// @inheritdoc IStakingRewards
    function pendingRevDisFor(address _who, uint256 _tokenId) public view virtual returns (uint256) {
        return stakedBonds[_who][_tokenId].earnedRevDis(revDisPerToken);
    }

    /// @inheritdoc IStakingRewards
    function yieldPerToken() public view virtual returns (uint256) {
        uint256 _underlyingSupplied = underlyingSupplied;
        IFactory _factory = factory;
        if (_underlyingSupplied == 0 || lastYieldDistribution >= lastTimeYieldApplicable()) {
            return yieldPerTokenStored;
        }

        return
            yieldPerTokenStored +
            (((((lastTimeYieldApplicable() - lastYieldDistribution) * _factory.ilvPerSecond() * weight) /
                _factory.totalWeight()) * Constants.BASE_MULTIPLIER) / _underlyingSupplied);
    }

    /// @inheritdoc IStakingRewards
    function setWeight(uint32 _weight) external virtual onlyFactory {
        emit LogSetWeight(msg.sender, weight, _weight);
        weight = _weight;
    }

    /// @inheritdoc IStakingRewards
    function stake(uint256 _tokenId) external virtual nonReentrant whenNotPaused updateReward(msg.sender, _tokenId) {
        _stake(msg.sender, _tokenId);
    }

    /// @inheritdoc IStakingRewards
    function stake(address _tokenOwner, uint256 _tokenId)
        external
        virtual
        onlyBondsRegistry
        nonReentrant
        whenNotPaused
        updateReward(_tokenOwner, _tokenId)
    {
        _stake(_tokenOwner, _tokenId);
    }

    /// @inheritdoc IStakingRewards
    function unstake(uint256 _tokenId, bool _useSILV)
        external
        virtual
        whenNotPaused
        nonReentrant
    ...
    {
        uint128 stakedBalance = stakedBonds[msg.sender][_tokenId].balance;
        require(stakedBalance > 0, "zero staked balance");
        _claimRewards(msg.sender, _tokenId, _useSILV);
        underlyingSupplied -= stakedBalance;
        delete stakedBonds[msg.sender][_tokenId];

        IERC721Upgradeable(address(bondsRegistry)).transferFrom(address(this), msg.sender, _tokenId);
        emit LogUnstake(msg.sender, _tokenId, stakedBalance);
    }

    /// @inheritdoc IStakingRewards
    function claimRewards(uint256 _tokenId, bool _useSILV)
        external
        virtual
        whenNotPaused
        nonReentrant
        updateReward(msg.sender, _tokenId)
    {
        uint128 stakedBalance = stakedBonds[msg.sender][_tokenId].balance;
        require(stakedBalance > 0, "zero staked balance");
        require(_claimRewards(msg.sender, _tokenId, _useSILV), "zero yield and revdis");
    }
    ...
    function receiveVaultRewards(uint256 _reward) external virtual whenNotPaused onlyVault {
        ...
        ilv.safeTransferFrom(msg.sender, address(this), _reward);
        emit LogReceiveVaultRewards(msg.sender, _reward);
    }

    /// @dev Internal call for {stake(uint256)} and {stake(address,uint256)}.
    /// @dev It's used both by users staking bonds from their wallets or directly
    /// from the bonds registry contract upon minting the bond NFT.
    function _stake(address _tokenOwner, uint256 _tokenId) internal virtual {
        require(stakedBonds[_tokenOwner][_tokenId].balance == 0, "bond already staked");
        // gas savings
        IBondsRegistry _bondsRegistry = bondsRegistry;
        (uint128 payout, ) = _bondsRegistry.bonds(_tokenId);
        require(payout != 0, "payout is zero");
        stakedBonds[_tokenOwner][_tokenId].balance = payout;
        underlyingSupplied += payout;

        IERC721Upgradeable(address(_bondsRegistry)).transferFrom(
            msg.sender == address(_bondsRegistry) ? address(_bondsRegistry) : _tokenOwner,
            address(this),
            _tokenId
        );
        emit LogStake(_tokenOwner, _tokenId, payout);
    }

    /// @dev Internal claim rewards, all important checks (such as modifiers) must be performed
    /// in the parent function.
    /// @dev Staked bond needs to have pending yield, pending rev dis or both in order to
    /// claim rewards.
    /// @param _to address to receive rewards
    /// @param _tokenId bond nft identifier
    /// @param _useSILV whether yield should be claimed as ILV or sILV2
    function _claimRewards(
        address _to,
        uint256 _tokenId,
        bool _useSILV
    ) internal virtual returns (bool claimed) {
        Bond.Stake storage stakedBond = stakedBonds[_to][_tokenId];
        uint256 revDisToClaim = stakedBond.pendingRevDis;
        uint256 yieldToClaim = stakedBond.pendingYield;
        claimed = revDisToClaim > 0 || yieldToClaim > 0;
        stakedBond.pendingRevDis = 0;
        stakedBond.pendingYield = 0;

        if (revDisToClaim > 0) ilv.safeTransfer(_to, revDisToClaim);
        if (yieldToClaim > 0) {
            if (_useSILV) {
                factory.mintYieldTo(_to, yieldToClaim, _useSILV);
            } else {
                ilvPool.stakeAsPool(_to, yieldToClaim);
            }
        }
        emit LogClaimRewards(_to, _tokenId, yieldToClaim, revDisToClaim, _useSILV);
    }
