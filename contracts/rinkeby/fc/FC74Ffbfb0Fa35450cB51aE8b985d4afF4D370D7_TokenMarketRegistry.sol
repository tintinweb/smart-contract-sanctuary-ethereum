...
/// @dev helper contract providing the data to the loan market contracts.
contract TokenMarketRegistry is ITokenMarketRegistry, OwnableUpgradeable, SuperAdminControl {
    ...
    {
        return
            IAdminRegistry(govAdminRegistry).isSuperAdminAccess(
                _wallet
            );
    }
    ...
    {
        return
            IProtocolRegistry(ProtocolRegistry).isTokenApproved(
                _token
            );
    }
        ...
        returns (bool)
    {
        return
            IProtocolRegistry(ProtocolRegistry)
                .isSyntheticMintOn(_tokenAddress);
    }
