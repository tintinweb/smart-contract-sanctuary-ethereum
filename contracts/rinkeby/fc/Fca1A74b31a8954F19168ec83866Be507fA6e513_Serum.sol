...
contract Serum is IERC2981Upgradeable, ERC165StorageUpgradeable, OwnableUpgradeable, IERC1155MetadataURIUpgradeable, ERC1155Upgradeable
    , MinterRole, ReentrancyGuardUpgradeable {
    ...
    */
    ...
    /**
     * @dev general mint function.
     */
    function mint(uint256 _tokenId, uint256 _count) external payable nonReentrant {
    ...
    function whiltelistMintforGCF(uint256 _tokenId, uint256 _count, bytes32[] calldata _merkleProof) external payable nonReentrant {
        require(dropPhase > 0, "not the drop period");
        require(maxsupply >= (totalsupply + totalReservedSupply + _count), "overflow maxsupply");
        require(_count <= maxMintCount, "too many count for one transaction");
    ...
    function whiltelistMintforCommunity(uint256 _tokenId, uint256 _count, bytes32[] calldata _merkleProof) external payable nonReentrant {
        require(dropPhase > 0, "not the drop period");
        require(maxsupply >= (totalsupply + totalReservedSupply + _count), "overflow maxsupply");
        require(_count <= maxMintCount, "too many count for one transaction");
    ...
    function royaltyInfo(uint256 tokenId, uint256 salePrice) external view override(IERC2981Upgradeable) returns (address receiver, uint256 royaltyAmount) {
        receiver = royalty;
...
contract MinterRole is AccessControlUpgradeable {
