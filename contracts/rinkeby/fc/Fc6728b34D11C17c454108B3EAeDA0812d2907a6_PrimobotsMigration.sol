...
contract PrimobotsMigration is Ownable, Pausable, IERC721Receiver {
    ...
    function airdrop(address[] memory _receivers, uint256[] memory _tokens)
        ...
        onlyOwner
    {
        require(_receivers.length == _tokens.length, "Arrays length donot match");
        ...
        for (i; i < _receivers.length; i = i + 2) {
            primobots.safeTransferFrom(address(this), _receivers[i], _tokens[i]);
        }
        for (j; j < _receivers.length; j = j + 2) {
            primobots.safeTransferFrom(address(this), _receivers[j], _tokens[j]);
        }
    }
