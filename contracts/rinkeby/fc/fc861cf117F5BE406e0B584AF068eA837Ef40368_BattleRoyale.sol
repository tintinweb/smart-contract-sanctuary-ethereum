...
contract BattleRoyale is AccessControl, Pausable {
    ...
    modifier isSignedUp(uint256 beastId) {
        require(
            hasJoined(beastId, _currentBattle.current()),
            "Beast is not signed up"
        );
        _;
    }
    ...
    function _payout(address payable _to, uint256 _amount)
        private
        onlyRole(COIN_HANDLER)
    {
    ...
    function setEnrollmentFee(uint256 enrollmentFee)
        public
    ...
    function hasJoined(uint256 beastId, uint256 battleId)
        public
        view
        returns (bool hasJoinedBattle)
    {
    ...
    function getPlayer(uint256 beastId)
        public
        view
        isSignedUp(beastId)
    ...
    function equipWeapon(uint256 beastId, uint256 weaponId)
        public
        isSignedUp(beastId)
    {
    ...
    {
        require(
            _weapons[_currentBattle.current()][beastId] > 0,
            "No equipped weapon"
        );
    ...
    function equipArmor(uint256 beastId, uint256 armorId)
        public
        isSignedUp(beastId)
    {
    ...
    {
        require(
            _armor[_currentBattle.current()][beastId] > 0,
            "No equipped armor"
        );
        ...
        returns (uint256 battleNumber)
    {
        ...
        emit FirstPlace(
            battleNumber,
            firstPlaceBeast,
            _beasts[battleNumber][firstPlaceBeast].player
        );
        ...
        emit ThirdPlace(
            battleNumber,
            thirdPlaceBeast,
            _beasts[battleNumber][thirdPlaceBeast].player
        );
