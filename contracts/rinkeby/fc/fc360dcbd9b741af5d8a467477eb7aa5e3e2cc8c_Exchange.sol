...
contract Exchange is Initializable, UUPSUpgradeable, EIP712Upgradeable, OwnableUpgradeable, PausableUpgradeable, AccessControlUpgradeable, ReentrancyGuardUpgradeable {
    ...
    address public BXFinanceNftRegistry;
    
    IERC20Upgradeable constant public WETH = IERC20Upgradeable(0xc778417E063141139Fce010982780140Aa0cD5Ab);
    ...
    event OrderPurchased(address seller, address buyer, address tokenAddress, uint256 tokenId);
    event OrderCancelled(LibOrder.Order order);
    event Blacklisted(address account);
    event ExcludedFromBlacklist(address account);
    event UpdatedNftRegistry(address nftRegistry);

    function initialize(address nftRegistry) external initializer {
        // initializing
        ...
        BXFinanceNftRegistry = nftRegistry;
    }
     ...
     * @param order - Object of sell order.
     *
     * Emits a {OrderPurchased} event, indicating the seller, token address and token id.
     */
    function completeFixedSale(LibOrder.Order memory order) external payable whenNotPaused nonReentrant {
        require(!blacklisted[msg.sender], "Exchange: Blackisted");
        require(!blacklisted[order.seller], "Exchange: Blackisted");

        // validate token registry
        (uint256 BXFinanceRevenuePercentage, uint256 royaltyPercentage, bool tradable) = IBXFinanceNftRegistry(BXFinanceNftRegistry).tradeRevenue(order.tokenAddress);
        ...
        emit OrderPurchased(order.seller, msg.sender, order.tokenAddress, order.tokenId);

        // block scope to avoid variable shadowing
        // take revenue and send to BXFinance revenue wallet
        uint256 revenueAmount = _calculateRevenue(order.price, BXFinanceRevenuePercentage);
        {
            (bool success, ) = payable(IBXFinanceNftRegistry(BXFinanceNftRegistry).BXFinanceRevenueWallet()).call{value: revenueAmount}("");
            require(success, "Exchange: Revenue transfer failed");
        }

        // block scope to avoid variable shadowing
        // take royalty and send to royalty wallet
        uint256 royaltyAmount = _calculateRevenue(order.price, royaltyPercentage);
        {
            (bool success, ) = payable(_getRoyaltyWallet(order.tokenAddress)).call{value: royaltyAmount}("");
            require(success, "Exchange: Revenue transfer failed");
        }
        ...
        // transfer BNB to seller
        {
            ...
            require(success, "Exchange: BNB transfer failed");
        }
    ...
    function completeBidding(LibBidOrder.BidOrder memory bidOrder, bytes calldata signature) external whenNotPaused nonReentrant {
        ...
        // validate token registry
        (uint256 BXFinanceRevenuePercentage, uint256 royaltyPercentage, bool tradable) = IBXFinanceNftRegistry(BXFinanceNftRegistry).tradeRevenue(bidOrder.tokenAddress);
        require(tradable, "This sale is not tradable");
        ...
        // take revenue and send to BXFinance revenue wallet
        uint256 revenueAmount = _calculateRevenue(bidOrder.bidAmount, BXFinanceRevenuePercentage);
        {
            WETH.safeTransferFrom(bidOrder.buyer, IBXFinanceNftRegistry(BXFinanceNftRegistry).BXFinanceRevenueWallet(), revenueAmount);
        }

        // take royalty and send to royalty wallet
        uint256 royaltyAmount = _calculateRevenue(bidOrder.bidAmount, royaltyPercentage);
        {
            WETH.safeTransferFrom(bidOrder.buyer, msg.sender, royaltyAmount);
        }
        
        // reduce revenue and royalty amount
        bidOrder.bidAmount = bidOrder.bidAmount - revenueAmount - royaltyAmount;

        // transfer remaining WETH to seller
        {
            LibBidOrder.BidOrder memory bidOrderData = bidOrder;
            WETH.safeTransferFrom(bidOrderData.buyer, msg.sender, bidOrderData.bidAmount);
        }
     ...
     *
     * @param account.
     * Emits {ExcludedFromBlacklist} event.
     */
    ...
    function includeAllInBlacklist(address[] memory accounts) external onlyRole(UPDATER_ROLE) {
        for (uint256 account; account < accounts.length; ++account) {
     ...
     * @param _registryAddress.
     * Emits {UpdatedNftRegistry} event.
     */
    function updateNftRegistry(address _registryAddress) external onlyOwner {
        require(_registryAddress != address(0), "Factory: Cant be zero address");
        BXFinanceNftRegistry = _registryAddress;
        emit UpdatedNftRegistry(BXFinanceNftRegistry);
    }
     ...
     */
    function _validateOrderStatus(bytes32 hashKey) view internal {
     ...
     */
    function _getRoyaltyWallet(address tokenAddress) view internal returns(address){
     ...
     */
    function _calculateRevenue(uint256 price, uint256 percentage) pure internal returns(uint256) {
        unchecked { return (price * percentage) / 10**4; }
    }
...
/**
 * @dev Interface of BXFinance Nft Registry.
 */
interface IBXFinanceNftRegistry {
    function BXFinanceRevenueWallet() external view returns (address);
...
library LibOrder {
     ...
     * @return bytes32 - hash value.
     */
    function _genOrderHash(Order memory order) internal pure returns(bytes32) {
        return keccak256(
...
library LibBidOrder {
    ...
    bytes32 private constant BID_ORDER_TYPEHASH = keccak256(
     ...
     * @return bytes32 - hash value.
     */
    function _genBidOrderHash(BidOrder memory order) internal pure returns(bytes32) {
        return keccak256(
