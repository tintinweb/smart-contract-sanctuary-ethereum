...
contract ANSS is Context, IBEP20, Whitelist {
  ...
  mapping (address => mapping (address => uint256)) private _allowances;
    ...
    mapping(uint256=>uint256) private Count;
   ...
   * @dev Start the sale.
   */
  function startSale(uint256 _rate,uint256 _minimumDeposite,uint256 _maximumDeposite,uint256 _endDate) external onlyOwner returns (bool){
  ...
  function changeEnddate(uint256 Enddate )external onlyOwner returns (bool) {
     endDate=Enddate;
     return true;
  }

  function TotalDeposite(uint256 amount)external onlyOwner returns (bool) {
      require(minimumDeposite<amount,"Total deposit should be greatar than minimum deposite");
      require(maximumDeposite<amount,"Total deposit should be greatar than  maximum deposite");
   ...
   */
   function Inverst() public payable{
       ...
       require(msg.value.add(userInfo[msg.sender].depositeAmount)<=Totaldeposite,"Limit reached");
   ...
   function Claim() public{
        require(block.timestamp>endDate.add(userInfo[msg.sender].Counter.mul(1800)),"Can not Withdrwal Tokens");
        require(block.timestamp>endDate,"ICO Is Not Completed Yet");
        require(isWhitelisted(msg.sender),"Caller Is Not WhitedListed");
        require(userInfo[msg.sender].Counter<10,"Token Finished");
        if(checkExitsAddress(msg.sender)){
            uint256 tokenTransfer=(userInfo[msg.sender].tokenWindrwal.mul(10)).div(100);
            _transfer(owner(),msg.sender, tokenTransfer);
            soldToken = soldToken.add(tokenTransfer);
            userInfo[msg.sender].Counter=userInfo[msg.sender].Counter+1;
