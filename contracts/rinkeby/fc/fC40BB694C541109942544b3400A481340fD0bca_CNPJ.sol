...
error MintPaused();
error MintOnlyWhitelisted();
error BurnMintPaused();
error MintReachedMaxSupply();
error MintReachedMaxMintAmount();
error BurnMintCallerNotOwner();

contract CNPJ is ERC721A, Ownable {
    address public constant withdrawAddress = 0x099ab43dFFAbA97b16e2C66c5Eb64980243d4863;
    ...
    function burnMint(uint256[] memory burnTokenIds) external payable
        ...
        for (uint256 i = 0; i < burnTokenIds.length; i++) {
            ...
            // 対象のNFTの所有者かどうか
            if (_msgSender() != ownerOf(tokenId)) revert BurnMintCallerNotOwner();
            _burn(tokenId);
        }
