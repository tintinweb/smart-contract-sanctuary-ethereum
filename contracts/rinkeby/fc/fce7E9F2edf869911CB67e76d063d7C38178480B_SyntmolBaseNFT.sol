...
contract SyntmolBaseNFT is ERC721Enumerable, Ownable {
    ...
    uint256 private _mint_tracker = 0;
    ...
    function devMint(uint256 mint_amount, address to) public onlyOwner
    {
        require(_mint_tracker.add(mint_amount) <= maxSupply, "Cannot mint more than the total supply");
        require(mint_amount > 0, "Cannot mint 0 items");

        for(uint256 i=0; i<mint_amount; i++)
        {
            _mint_tracker = _mint_tracker.add(1);
            ERC721._safeMint(to, _mint_tracker);
        }
    }

    function mint(uint256 mint_amount) external payable
    {
        require(stage != MintStage.PAUSED, "Contract is currently paused");
        require(mint_amount > 0, "Cannot mint 0 items");
        require(mint_amount <= MAX_PER_TX, "Maximum mint amount per transaction reached!");
        ...
        {
            _mint_tracker = _mint_tracker.add(1);
            ERC721._safeMint(msg.sender, _mint_tracker);
        }
    ...
    {
        return stage == MintStage.WHITELIST ? whitelistPrice : publicPrice;
    }
