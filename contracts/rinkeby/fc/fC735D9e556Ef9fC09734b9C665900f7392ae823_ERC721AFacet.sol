...
contract ERC721AFacet is ERC721A {
    ...
    function convertUSDtoWei(uint256 _price) internal returns (uint256) {
        ...
        uint256 ethPerUsd = oneEth.div(
            uint256(150000000000).div(uint256(decimals))
        );
...
library PriceConsumer {
    ...
    uint256 constant localhost = 1337;
    ...
    {
        console.log(
            "Inside getPriceFeedAddresss. Blockchain ID: %s",
            block.chainid
        );
        uint256 chainId = block.chainid;

        if (chainId == ethereumId || chainId == localhost) {
    ...
    // function priceToUsd(int256 price) internal view returns (int256) {
    //     return price / (10**4);
    // }
