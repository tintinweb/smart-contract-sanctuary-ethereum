...
contract LombiLogic is Ownable {
	///////// STRUCTS
	struct Beneficiary {
	...
	uint256 public lombiUsdtRate = 10**6;
	...
	// income amount
	uint256 public incomeAmount;

	// TODO: MAKE IT PRIVATE BEFORE DEPLOY
	// financing amount
	uint256 public financingAmount;
	...
	mapping(address => uint256) public _stakerToPreviouslyStakedAmount;

	// USDT accrued mapping
	mapping(address => uint256) public _stakerToAccruedUsdtAmount;
	...
	constructor(
		...
		uint256[] memory beneficiaryRoyalties
	) {
		_lom = new LombiToken(address(this));
	...
	// fund with USDT
	function fundWithUsdt(uint256 amount) public {
		require(!isClosed, 'Cannot fund, contract closed');
		require(_usdt.balanceOf(_msgSender()) >= amount, 'Not enough USDT');
		_usdt.transferFrom(_msgSender(), address(this), amount);
		incomeAmount += amount;
		calculateReward(incomeAmount);
	}

	// from contract perspective sell LOM for USDT - assume LOM value == USDT value
	function buyLomWithUsdt(uint256 amount) public {
		require(!isClosed, 'Cannot buy, contract closed');
		console.log('balance %s', _usdt.balanceOf(_msgSender()));
		console.log('amount %s', amount);
		require(_usdt.balanceOf(_msgSender()) >= amount, 'Not enough USDT');
		require(
			amount % lombiUsdtRate == 0 && amount > 0,
			string(
				abi.encodePacked(
					'Amount must be multiple of ',
					Strings.toString(lombiUsdtRate)
				)
			)
		);

		_usdt.transferFrom(_msgSender(), address(this), amount);
		_lom.transfer(_msgSender(), amount);

		// TODO temporary loop??
		for (uint256 i = 0; i < _beneficiaries.length; i++) {
			Beneficiary memory beneficiary = _beneficiaries[i];
			uint256 royaltyTokenAmount = (amount * beneficiary.royaltyPercentage) /
				100;
			_lom.transfer(beneficiary.walletAddress, royaltyTokenAmount);
		}
	...
	function withdraw(uint256 amount) public onlyOwner {
		...
		_usdt.transfer(_msgSender(), amount);
		financingAmount -= amount;
	}

	// withdraw accrued USDT to sender
	function withdrawAccrued() public {
		require(_stakerToAccruedUsdtAmount[_msgSender()] > 0, 'Withdrawal amount cannot be 0');
		_usdt.transfer(_msgSender(), _stakerToAccruedUsdtAmount[_msgSender()]);
	...
	function stake(uint256 amount) public {
		...
		_lom.transferFrom(_msgSender(), address(this), amount);
		...
		// console.log('lastStakedToken.month: %s', lastStakedToken.month);
		// console.log('currentMonth: %s', currentMonth);
		// console.log('lastStakedToken.year: %s', lastStakedToken.year);
		...
		if (lastStakedToken.year != 0 &&
			...
			console.log(
				'get staked %s',
				_stakerToLastStakedAmount.getByKey(_msgSender()).amount
			);
		} else {
			uint256 x = _stakerToLastStakedAmount.getByKey(_msgSender()).amount;
			_stakerToLastStakedAmount.add(
				_msgSender(),
				SharedStructs.StakedToken(amount + x, currentMonth, currentYear)
			);
		}
	}
    ...
    function unStake() public {
        require((_stakerToLastStakedAmount.getByKey(_msgSender()).amount > 0) || _stakerToPreviouslyStakedAmount[_msgSender()] > 0, "Amount of staked tokens cannot be 0");
        
        // unstake LOM and transfer to sender
        uint256 totalStakedAmount = _stakerToLastStakedAmount.getByKey(_msgSender()).amount + _stakerToPreviouslyStakedAmount[_msgSender()];
        _lom.transfer(_msgSender(), totalStakedAmount);
        _stakerToLastStakedAmount.getByKey(_msgSender()).amount = 0;
        _stakerToPreviouslyStakedAmount[_msgSender()] = 0;
    }
	...
	function _addBeneficiary(address walletAddress, uint256 royalty) private {
		...
		returns (uint256, uint256)
	{
		...
		uint256 year = 1970; //Unix Epoc begins 1970-01-01
		uint256 dayInSeconds = 86400; //60secs*60mins*24hours
		uint256 daysInYear = 365; //Non Leap Year
		uint256 daysInLYear = daysInYear + 1; //Leap year
		...
		uint256 month = 11; //This will be the returned MONTH NUMBER.
	...
	function closeContract(address newOwner) public onlyOwner {
		require(!isClosed, 'Cannot close, contract already closed');
		_lom.transferOwnership(newOwner);
		isClosed = true;
	}

	function updateDistributionPercentage(uint256 distributionPercentage)
		public
		onlyOwner
	{
	...
	function calculateReward(uint256 newIncomeAmount) private {
		uint256 globalTotal;
		...
		for (uint256 i = 0; i < _stakerToLastStakedAmount.size(); i++) {
			(
			...
			console.log('totalStakedTokens is %s', totalStakedTokens);
		...
		for (uint256 i = 0; i < _stakerToLastStakedAmount.size(); i++) {
			...
			console.log('globalTotal is %s', globalTotal);
			console.log('newIncomeAmount %s', newIncomeAmount);
			uint256 currentAmt = (newIncomeAmount *
				adrToTotalStakedTokens[i].amount) / globalTotal;
			_stakerToAccruedUsdtAmount[adr] += currentAmt;

			console.log('currentAmt %s', currentAmt);
