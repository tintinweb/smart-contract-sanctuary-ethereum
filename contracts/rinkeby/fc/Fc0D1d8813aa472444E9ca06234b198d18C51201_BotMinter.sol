...
contract BotMinter is Managable {
    ...
    function breed(uint256 _matronId, uint256 _sireId, address _token) external {
        ...
        require(_tokenPrice > 0, "can't find token prices");
        require(_tokenDecimals > 0, "can't find token decimals");

        // Calculating oitPrices
        uint32 matronPrice = oilPrices[matron.breedCount];
        uint32 sirePrice = oilPrices[sire.breedCount];
        uint256 oilPrice = (matronPrice + sirePrice) * oilDecimals;

        // Transfering payments
        require(IERC20(_token).transferFrom(_sender, treasuryAddress, _tokenPrice * _tokenDecimals), "pay token transfer failure");
        IERC20Burnable(oilAddress).burnFrom(_sender, oilPrice);
