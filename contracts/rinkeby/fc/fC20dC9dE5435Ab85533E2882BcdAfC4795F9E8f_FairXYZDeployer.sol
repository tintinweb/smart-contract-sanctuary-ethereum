...
contract FairXYZDeployer is ERC721xyz, ReentrancyGuard, Pausable{
    ...
    // View all variables
    function viewAllVariables() public view returns(uint256, uint256, uint256, string memory){
        return(nftPrice, _royalties.amount, maxMintsPerWallet, pathURI);
    }
    ...
    function initialize(uint256 maxTokens_, uint256 nftPrice_, string memory name_, string memory symbol_,
        ...
        pathURI = URIs_[2];
        isBase = true;
        _setRoyalties(royaltyPercentage_);
    }
        ...
        external
    {
        ...
        if(newRoyaltyPercentage != _royalties.amount)
        {      
            _setRoyalties(newRoyaltyPercentage);
...
// Following the implementation at https://github.com/dievardump/EIP2981-implementation/blob/main/contracts/ERC2981ContractWideRoyalties.sol
...
abstract contract ERC2981 is ERC2981Base {
    ...
    /// @param value percentage (using 1 decimals - 1000 = 100, 0 = 0)
    function _setRoyalties(uint256 value) internal {
        require(value <= 1000, 'ERC2981Royalties: Too high');
        _royalties = RoyaltyInfo(tx.origin, uint24(value));
    }
        ...
        returns (address receiver, uint256 royaltyAmount)
    {
        ...
        royaltyAmount = (value * royalties.amount) / 1000;
    }
}
