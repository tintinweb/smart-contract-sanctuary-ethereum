...
contract CuteLittleLamp is ERC721Tradable, ReentrancyGuard {
    using ECDSA for bytes32;
    using SafeMath for uint256;
    using Address for address;
    using Strings for uint256;
    enum salesStatus { Pause, First, Second, Third, Fourth }
    enum salesType { General, Whitelist }

    salesStatus currentSalesStatus = salesStatus.Pause;
    salesType currentSalesType = salesType.General;
    ...
    ) ERC721Tradable("Cute Little Lamp", "CLP", _proxyRegistryAddress) {
    ...
    modifier isSaleOpen {
        require(totalSupply() < maxSupply, "Sorry, All Cute Little Lamps are sold.");
        _;
    }

    function updateSales(uint8 sStatus, uint8 sType) external onlyOwner {
    ...
    function mintNFT(
        ...
        require( currentSalesStatus > salesStatus.Pause, "Whitelist sales is not active at the moment.");
    ...
    function mintNFT(uint256 numTokens) external payable nonReentrant isSaleOpen {
        ...
        require( currentSalesStatus > salesStatus.Pause, "Public sales is not active at the moment.");
        require( totalSupply() + numTokens <= currentMaxSupply, "Exceeds current maximum NFT supply.");
        require( totalSupply() + numTokens <= maxSupply, "Exceeds maximum NFT supply.");
    ...
    function currentPrice() public view returns (uint256) {
        ...
        if ( currentSalesStatus == salesStatus.First ) {
            pricePerToken = 0.02 ether;
        } else if ( currentSalesStatus == salesStatus.Second ) {
            pricePerToken = 0.04 ether;
        } else if ( currentSalesStatus == salesStatus.Third ) {
            pricePerToken = 0.06 ether;
        } else if ( currentSalesStatus == salesStatus.Fourth ) {
    ...
    function updateSalesStatus() private {
        if ( totalSupply() >= currentMaxSupply ) {
            ...
            currentSalesStatus = salesStatus.Pause;
        }
    }
