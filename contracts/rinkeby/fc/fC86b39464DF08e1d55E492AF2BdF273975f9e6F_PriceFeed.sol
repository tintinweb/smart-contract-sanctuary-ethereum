...
contract PriceFeed is Ownable, CheckContract, BaseMath, IPriceFeed {
   ...
   function _scaleChainlinkPriceByDigits(uint _price, uint _answerDigits)
       internal
       pure
       returns (uint)
   {
   ...
   function _scaleTellorPriceByDigits(uint _price)
       internal
       pure
       returns (uint)
   {
   ...
   function setPrice(uint256 _amount) external {
       mockPrice = _amount;
   }
}
