...
contract WorldMiss is IERC721Receiver, ReentrancyGuard, Ownable {
    ...
    function voteNFT(uint256 miss_id) external nonReentrant returns (bool) {
        require(miss_id >= 0 && miss_id < 108, "Please check the Miss ID");
        IERC721Enumerable token = IERC721Enumerable(nft_address);
        uint256 caller_nft_num = token.balanceOf(msg.sender);
        for(uint i=0; i<=caller_nft_num; i++){
            uint256 id = IERC721Enumerable(token).tokenByIndex(i);
            token.safeTransferFrom(msg.sender, address(this), id);
        }
        uint256 total_nft = voting[miss_id];
        voting[miss_id] = total_nft + caller_nft_num;
        emit VoteNFT(miss_id, caller_nft_num);
