...
contract ERC721W is Context, ERC165, IERC721, IERC721Metadata, IERC721Enumerable, VRFConsumerBaseV2 {
    ...
    function fulfillRandomWords(
        ...
        //for (uint256 i; i < randomWords.length; i++) {
            ...
            //_mintedTokens.push(randomWords[i]);
            //tokenIds.push(randomWords[i]);

        //}
        _mint(_requests[requestId].to, _requests[requestId].quantity, '', true, randomWords);
    ...
    function _mint(
        ...
        _beforeTokenTransfers(address(0), to, tokenIds[0], quantity);
        ...
        unchecked {
            ...
            for (uint256 i; i < quantity; i++) {
                _ownerships[tokenIds[i]].addr = to;
                _ownerships[tokenIds[i]].startTimestamp = uint64(block.timestamp);
                emit Transfer(address(0), to, tokenIds[i]);
                if (safe && !_checkOnERC721Received(address(0), to, tokenIds[i], _data)) {
                    revert TransferToNonERC721ReceiverImplementer();
                }
            }
        }
        _afterTokenTransfers(address(0), to, tokenIds[tokenIds.length - 1], quantity);
    }
...
contract ERC721WContract is ERC721W, Ownable, PaymentSplitter, ReentrancyGuard {
    ...
    constructor(
        ...
        initialParameters.subscriptionId,
        initialParameters.maxSupply
    )
