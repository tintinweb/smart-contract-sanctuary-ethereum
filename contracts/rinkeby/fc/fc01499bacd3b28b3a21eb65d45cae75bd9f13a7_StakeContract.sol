...
library SafeMath {
   ...
   contract StakeContract {
        ...
        uint256[4] public durations = [5 minutes, 10 minutes, 16 minutes, 90 minutes];
  ...
  //StakeWithAmd
   ...
   function withdrawInWolve(uint256 count) public {
        ...
        require(endingTime > stakeTimeForEndind, "You canot extend stake when the time is over.");
    ...
    function extendStake(uint256 count,uint256 newplan) public {
       ...
       User storage user = users[msg.sender];
        ...
        beforeDays = (currentTime  - timeBefore).div(1 minutes);
    ...
    function calculateNewReward(address userAddress, uint256 userStakeCount, uint256 amount, uint256 _newDuration, uint256 plan) public/* returns(uint256)*/{
    ...
    function calculateNextReward(address userAddress, uint256 userStakeCount, uint256 amount, uint256 _newDuration, uint256 plan) public /*returns(uint256)*/{
        ...
        if (plan == 3){ 
            ...
            calculaateNewPercentage(_newDuration, plan+1);
        }

        user.stakerecord[userStakeCount].afterTimeBonus = amount.mul(newPercentage).div(percentDivider);
       nextReward = amount.mul(newPercentage).div(percentDivider);
        //return nextReward;
    }
