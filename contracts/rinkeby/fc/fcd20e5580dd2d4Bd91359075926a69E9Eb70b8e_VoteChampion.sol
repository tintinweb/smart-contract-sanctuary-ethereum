...
contract VoteChampion is Pausable, Ownable {
    ...
    function getTopSoccer() external view returns(Rank memory) {
        return ranks[4];
    }
    ...
    function vote(uint _rank, uint _teamIndex, uint amount) external whenNotPaused {
        require(amount > 0, 'VoteChampion::vote: invalid amount');
        require(!isEndTeam[_teamIndex], 'VoteChampion::vote: team Ended');
    ...
    function setRank(uint _rank, uint _teamIndex) external onlyOwner whenPaused {
        require(_rank > 0 && _rank < 5, 'VoteChampion::vote: invalid rank'); // 4 => top Soccer
        ranks[_rank].team = teams[_teamIndex];
        ranks[_rank].teamIndex = _teamIndex;
        ranks[_rank].isResult = true;
    ...
    function getPlayers(address _user) external view returns(Player[] memory){
        Player[] memory t = new Player[](players.length);
        for(uint i = 0; i < players.length; i++) {
            t[i] = Player(players[i], userVoteTopScorer[i], users[_user].voted[i][4].totalVote);
        }
        return t;
    }
