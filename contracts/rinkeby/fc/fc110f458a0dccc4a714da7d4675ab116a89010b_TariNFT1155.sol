...
contract TariNFT1155 is ERC1155 {
    ...
    event operatorChanged(address indexed operator, address indexed _newOperator);
    ...
    function setOperator(address _newOperator) external onlyOwner {
        require(_newOperator != address(0),"Operator: address should not be zero");
        require(_newOperator != operator,"Operator: address already an Operator");
    ...
    */
    ...
    function mint(address from, string memory uri, uint256 supply, uint96 fee) external returns(uint256, bool){
        ...
        require(msg.sender == operator,"Minting: caller doesn't have a role for minting");
        usedURIs[uri] = true;
        uint256 tokenId = newTokenId;
        _mint(from, newTokenId, supply, abi.encodePacked(msg.sender, newTokenId, supply));
        _setTokenURI(newTokenId, uri);
        _setTokenRoyalty(newTokenId, from, fee);
