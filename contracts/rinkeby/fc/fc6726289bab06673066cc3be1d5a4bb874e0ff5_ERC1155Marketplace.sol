    ...
    OwnableUpgradeable
{
    ...
    function _availOffer(
        ...
        address _erc20Token,
        bytes calldata sign
    ) private erc20Allowed(_erc20Token) {
        // Verfify signature
        {
            require(
                !isNonceProcessed[_nonce],
                "ERC1155Marketplace: Nonce already processed"
            );

            bytes32 messageHash = keccak256(
                abi.encodePacked(
                    address(this), // Marketplace address
                    _nonce, // Nonce for this offer
                    _erc1155, //Address of the collection
                    _tokenId, // TokenId
                    _quantity, // Number of NFTs to be availed
                    _price, // Price per NFT
                    _erc20Token, // ERC20 token address in seller wants to sell.
                    msg.sender // Address of buyer
                )
            );
            ...
            require(
                _seller == signer_,
                "ERC1155Marketplace: Signature not verfied."
            );
    ...
    // ) external {
    //     require(
    //         _erc1155s.length == _tokenIds.length &&
    //             _erc1155s.length == _sellers.length &&
    //             _erc1155s.length == _quantities.length &&
    //             _erc1155s.length == _prices.length &&
    //             _erc1155s.length == _nonces.length &&
    //             _erc1155s.length == _erc20Tokens.length &&
    //             _erc1155s.length == _signs.length,
    //         "ERC1155Marketplace: Lengths of all data must be same"
    //     );
        ...
        )
    {
        require(
            sig.length == 65,
            "ERC1155Marketplace: invalid signature length"
        );
