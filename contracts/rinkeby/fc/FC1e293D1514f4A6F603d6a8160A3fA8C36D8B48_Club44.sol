...
contract Club44 is ERC721Enumerable, ERC2981, Ownable {
    ...
    // uint256 public secondarySaleStartTime;
    ...
    constructor(
        ...
        address _simonoFromRoyalty
        // uint256 _secondarySaleStartTime
    ) ERC721("Club44", "Club44") {
        baseURI = _baseURI;
        proxyRegistryAddress = _proxyRegistryAddress;
        simonoFromAccounting = _simonoFromAccounting;
        // secondarySaleStartTime = _secondarySaleStartTime;
        _setDefaultRoyalty(_simonoFromRoyalty, 750);
        _mint(address(0),0);
    }
    ...
    // set various proxy addresses for preApproval of secondary market fees
    ...
    function collectReserves() external onlyOwner {
        ...
        for (uint256 i; i < RESERVES; i++) _safeMint(_msgSender(), i+1);
    }
    ...
    function publicMint(uint256 _amount) public payable {
        ...
        require(_amount == 1, "Max 1 mint allowed per wallet");
        require(mintPrice == msg.value, "Invalid funds provided.");
        // require(!claimed[_msgSender()], "Already Claimed");

        uint256 _totalSupply = _owners.length;

        require(_totalSupply < MAX_SUPPLY+1, "Excedes max supply.");

        _mint(_msgSender(), _totalSupply + 1);
        claimed[_msgSender()] = !claimed[_msgSender()];
    }
    ...
    function _transfer(
        ...
        // require(block.timestamp > secondarySaleStartTime, "ERC721: secondary sales not permitted yet");
...
abstract contract ERC721 is Context, ERC165, IERC721, IERC721Metadata {
     ...
     * Emits a {Transfer} event.
     */
    //  Wont be permitted before the specified timeStamp
    function _transfer(
        address from,
        address to,
        uint256 tokenId
        ...
        require(to != address(0), "ERC721: transfer to the zero address");
        // require(block.timestamp > 1655986407, "ERC721: secondary sales not permitted yet");
