...
// File contracts/FASTTreasury.sol
...
// File contracts/Timelocks/OnlyTimelock.sol
...
// File contracts/treasury/FASTTreasury.sol
...
/*  FASTTreasury.sol ============================================
    The contract is to store the reserve of FAST Protocol
...
==================================================================== */
contract FASTTreasury is Ownable, OnlyTimelock, OnlyAdmin {
    using SafeERC20 for IERC20;
    using Address for address;

    mapping(address => bool) public strategies;
    address[] public strategiesArray;
    IFASTStaking public staking;

    constructor(IFASTStaking _staking) {
        staking = _staking;
    }

    // ========== PUBLIC VIEW FUNCTIONS ============

    /// @notice Return ERC-20 balance of FASTTreasury
    /// @param _token Address of the ERC-20 token
    /// @return Balance of the FASTTreasury
    ...
    function requestFund(address _token, uint256 _amount) external {
        require(strategies[msg.sender], "FASTTreasury::requestFund: Only strategies can request fund");
        require(_amount <= balanceOf(_token), "FASTTreasury::requestFund: Request more fund than balance");
    ...
    function addStrategy(address _strategy) external onlyTimelock () {
        require(_strategy != address(0), "FASTTreasury::addStrategy: invalid address");
        require(!strategies[_strategy], "FASTTreasury::addStrategy: strategy was previously added");
    ...
    function removeStrategy(address _strategy) external onlyOwner {
        require(strategies[_strategy], "FASTTreasury::removeStrategy: strategy not found");
    ...
    function allocateFee(address _token, uint256 _amount) external onlyAdmin {
        require(address(staking) != address(0), "FASTTreasury::allocateFee:Fee distributor not set");
        require(_amount > 0, "FASTTreasury::allocateFee: invalid amount");
