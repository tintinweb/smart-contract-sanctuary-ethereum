...
contract Kostbar102 is ERC721A, Ownable {
    error NonEOA();
    error Paused();
    error NonPresalePeriod();
    error NonPublicPeriod();
    ...
    uint256 public presalePrice = 5 ether;
    uint256 public publicPrice = 8 ether;
    ...
    modifier callerIsUser() {
        if (tx.origin != msg.sender) revert NonEOA();
        _;
    }

    constructor(string memory baseURI) ERC721A("KostbarTEST101", "KT101") {
        ...
        callerIsUser
    {
        if (block.timestamp < presaleStart || block.timestamp > presaleEnd)
            revert NonPresalePeriod();

        if (_totalMinted() + quantity > maxSupply) revert SupplyExceeded();

        if (msg.value < presalePrice * quantity) revert InvalidEtherAmount();
    ...
    function publicMint(uint256 quantity)
        ...
        callerIsUser
    {
        // if (block.timestamp < presaleEnd) revert NonPublicPeriod();
        ...
        returns (string memory)
    {
        ...
        if (revealed == false) return baseURI;
