...
contract SMOLNft is Ownable, ERC721Burnable, ERC721Enumerable, ERC721Pausable {
  ...
  uint256 public nativeCost = 8 ether / 100; // 0.08 ETH
  uint256 public smolCost = 100 * 10**18;
  uint8 public maxPerMint = 5;
  ...
  // Token info
  string public constant TOKEN_NAME = 'smolting inu nft';
  ...
  function mint(uint256 _amount) public payable whenOwnerOrPublicSaleActive {
    ...
    require(
      owner() == _msgSender() ||
        block.timestamp > userLastMinted[_msgSender()] + FIVE_MINUTES,
      'can only mint once per 5 minutes'
    );
    require(
      _amount > 0 && _amount <= maxPerMint,
    ...
    // Pay for nodes
    // handle free NFT mint logic here as well
    if (
    ...
    for (uint256 i = 0; i < _amount; i++) {
      ...
      _beforeTokenTransfer(address(0), _msgSender(), _tokenIds.current());
    }
  }
  ...
  function turnOnReveal() external onlyOwner {
  ...
  function _beforeTokenTransfer(
    ...
    if (!_isRewardsExcluded[_from] && _from != address(0)) {
      try _rewards.setShare(_from, balanceOf(_from)) {} catch {}
    }
    if (!_isRewardsExcluded[_to] && _to != address(0)) {
      try _rewards.setShare(_to, balanceOf(_to)) {} catch {}
    }
