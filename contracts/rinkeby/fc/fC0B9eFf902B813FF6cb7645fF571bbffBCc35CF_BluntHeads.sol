...
contract BluntHeads is ERC1155, Ownable, Pausable, ReentrancyGuard
{
    ...
    string public _BASE_URI = "https://ipfs.io/ipfs/IPFS_HASH/";
    
    //Multisig
    address private immutable _BHMultisig = address(0);
    ...
    //Price
    uint256 public _BLUNT_PRICE = 0.001 ether;
    ...
    //Mint Mapping
    mapping (address => bool) private _MINTED;      //public mint mapping
    mapping (address => bool) private _WHITELISTED; //whitelist mint mapping

    /*--------------------
        * CONSTRUCTOR *
    ---------------------*/

    constructor() ERC1155("https://ipfs.io/ipfs/IPFS_HASH/{id}.json") { }
    ...
    //Mints Blunt Heads
    function BluntHeadsMint(uint256 numberOfTokens) public payable nonReentrant
    {
    ...
    //Mints Blunt Heads from Whitelist
    function BluntHeadsMintWhitelist(uint256 numberOfTokens) public payable nonReentrant
    {
        require(tx.origin == msg.sender, "No External Contracts");
        require(_WHITELISTED[msg.sender], "Wallet is not WHITELISTED");
        require(numberOfTokens <= _MAX_BLUNTS_PURCHASE, "Can only mint max 20 Blunts at a time");
        require(_BLUNTS_MINTED + numberOfTokens <= _MAX_BLUNTS, "Purchase would exceed max supply of Blunts");
        require(_BLUNT_PRICE * numberOfTokens == msg.value, "Ether value sent is not correct. 0.08 ETH Per Blunt | 80000000000000000 WEI");
    ...
    function _reserveBluntHeads(uint256 numberOfTokens) external onlyOwner
    {
        //Mints Blunts
        for(uint256 i = 0; i < numberOfTokens; i++) 
        {
    ...
    //Whitelists Members
    function _whitelistMembers(address[] calldata whitelist) external onlyOwner
    {
    ...
    //Whitelists Member
    function _whitelistMember(address whitelist) external onlyOwner { _WHITELISTED[whitelist] = true; }
    ...
    //Disallows Renouncing of Ownership
    function renounceOwnership() public override onlyOwner { transferOwnership(_BHMultisig); }
