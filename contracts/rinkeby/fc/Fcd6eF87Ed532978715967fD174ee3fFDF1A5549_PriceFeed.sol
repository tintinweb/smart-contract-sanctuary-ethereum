...
contract PriceFeed is Ownable, CheckContract, BaseMath, IPriceFeed {
    ...
    //bytes32 constant public ETHUSD_TELLOR_REQ_ID = 0xfe189873dc68fa446c034dcb44e22b6da34aaadc48bccb2551badb856acc3c83;

    // Use to convert a price answer to an 18-digit precision uint
    uint constant public TARGET_DIGITS = 18;  
    uint constant public TELLOR_DIGITS = 6;
    uint256 public tellorpreviousresponse;

    // Maximum time period allowed since Chainlink's latest round data timestamp, beyond which Chainlink is considered frozen.
    uint constant public TIMEOUT = 14400;  // 4 hours: 60 * 60 * 4

    //address _tellorCallerAddress = 0x4f6293843A6946f5cDc2007d25DA4e156f5cf899;
    ...
    */
        ...
        onlyOwner
    {
        ...
        //_bothOraclesSimilarPricestatus = Status.chainlinkWorking;
    ...
    */
    ...
    /*
    *For support of ETHW Which doesn't support chainlink
    *We check for conditions if Frozen, price deviation just too much or if no response
    *So inDexer is used which is the best on ethw 
    *Support for chainlink isn't removed so it can be lated added if now supporting ethw
    */
   function fetchPrice() external override returns(uint){
    TellorResponse memory tellorResponse = _getCurrentTellorResponse();
        ...
        /*if (_tellorPriceChangeAboveMax(tellorResponse.value, tellorpreviousresponse)){
            return lastGoodPrice;
        }*/
    ...
    */
    ...
    function _tellorPriceChangeAboveMax(uint _currentResponse, uint _prevResponse) internal pure returns (bool) {
        uint currentScaledPrice = _scaleTellorPriceByDigits(uint256(_currentResponse));
        uint prevScaledPrice = _scaleTellorPriceByDigits(uint256(_prevResponse));
    ...
    function _getCurrentTellorResponse() internal view returns (TellorResponse memory tellorResponse) {
        ...
        )
        {
            ...
            tellorResponse.ifRetrieve = result;
    ...
    function changeTrellorCaller(address tellorCallerAddress) external onlyOwner{
        checkContract(tellorCallerAddress);
        tellorCaller = ITellorCaller(tellorCallerAddress);
    }
}
