...
contract DANCEToken is Ownable, ERC20F{
    ...
    constructor(
        uint256 _fee,
        uint256 _feedivider,
        address _danceSplitter,
        address[] memory _initAddresses,
        uint256[] memory _initAmounts
    ) ERC20F("DANCE", "DANCE", _fee, _feedivider, _danceSplitter) {
        ...
        for (uint256 i = 0; i < _initAddresses.length; i++) {
            ...
            _setNoFeeAddress(_initAddresses[i], true);
        }
    }
...
contract ERC20F is Context, IERC20F, IERC20Metadata {
    ...
    */
    function transferNoFee(address to, uint256 amount) external returns (bool) {
        _transferNoFee(_msgSender(), to, amount);
        return true;
    }
    ...
    unchecked {
        ...
        uint256 fee = amount * _fee / _divider;
        amount -= fee;
        _balances[_beneficiary] += fee;
        _balances[to] += amount;
