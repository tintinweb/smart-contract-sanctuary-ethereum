...
contract BurnToken is ERC721 {
    ...
    string private blankTokenURI = "ipfs://not-set-yet";
    ...
    address public timeCatsAddress = 0x000000000000000000000000000000000000dEaD;

    mapping(address => uint256) public mintedTokensPerWallet;
    mapping(address => bool) public hasUsedACat;

    constructor() ERC721("BurnToken", "BRN") {
     ...
     */
    function setTimeCatsAddress(address _timeCatsAddress) external onlyOwner contractIsNotFrozen {
     ...
     */
    function revealDrop() external onlyOwner contractIsNotFrozen {
        _revealed = true;
    }
     ...
     * @dev Mint a token
     */
    function mint(uint256 amount, uint256 _maxMint, bool _hasCat, bytes calldata _signature) external payable callerIsUser contractIsNotFrozen {
        ...
        bytes32 messageHash = generateMessageHash(msg.sender, _maxMint, _hasCat);
     ...
     */
    function mintWithPass(uint256 _catId, uint256 _maxMint, bool _hasCat, bytes calldata _signature)
    external payable callerIsUser contractIsNotFrozen {
        ...
        require(!hasUsedACat[msg.sender], "Caller cannot mint any more with a cat");
        ...
        // initialize the cat contract interface
        ITimeCatsLoveEmHateEm catsContract = ITimeCatsLoveEmHateEm(
            timeCatsAddress
        );

        // check if the cat is already used or not
        require(false == catsContract.isUsed(_catId), "The cat is already used");

        // check if the caller is the owner of the cat
        require(msg.sender == catsContract.ownerOf(_catId), "You dont own this cat");
        ...
        // set the flag that the user has used the cat to mint
        hasUsedACat[msg.sender] = true;
    ...
    function burnForReward(uint256 tokenId) external callerIsUser contractIsNotFrozen {
        ...
        RewardToken rewardTokenContract = RewardToken(rewardContract);
    ...
    /**
     * @dev Checks if cat is already used to mint a token
     */
    function isCatUsed(address _owner) public view returns(bool) {
        return hasUsedACat[_owner];
    }
     ...
     */
    function generateMessageHash(address _address, uint256 _maxMint, bool _hasCat) internal pure returns (bytes32) {
        return keccak256(
            abi.encodePacked(
...
contract RewardToken is ERC721 {
    ...
    constructor(address burnAddress) ERC721("RewardToken", "RWRD") {
