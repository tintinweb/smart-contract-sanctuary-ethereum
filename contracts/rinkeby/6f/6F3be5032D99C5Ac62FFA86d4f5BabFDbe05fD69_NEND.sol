...
abstract contract Inflation is ERC20, Ownable {
    ...
    mapping(address => address) public tokenToStakePool;
    ...
    function inflate(uint256 _amount) external onlyOwner {
        ...
        lastInflation += inflationInterval;

        uint256 numPools;

        for (uint256 i = 0; i < stakeTokens.length; i++) {
            if (activeStakeTokens[stakeTokens[i]]) {
                numPools++;
            }
        }

        uint256 individualAmount = _amount / numPools;

        for (uint256 i = 0; i < stakeTokens.length; i++) {
            address stakeToken = stakeTokens[i];
            if (activeStakeTokens[stakeToken]) {
                address stakePool = tokenToStakePool[stakeToken];
                // Distribute weekly inflation reward
                _mint(stakePool, individualAmount);

                // Distribute trade & loan commissions
                uint256 stakeTokenBalance = ERC20(stakeToken).balanceOf(
                    address(this)
                );
                if (stakeTokenBalance > 0) {
                    ERC20(stakeToken).transfer(stakePool, stakeTokenBalance);
                }
            }
        }
    }

    function addPool(address stakePool) external onlyOwner {
        address stakeToken = Staking(stakePool).stakeToken.address;

        activeStakeTokens[stakeToken] = true;

        if (tokenToStakePool[stakeToken] == address(0)) {
            stakeTokens.push(stakeToken);
        }

        tokenToStakePool[stakeToken] = stakePool;
    }

    function removePool(address stakePool) external onlyOwner {
        address stakeToken = Staking(stakePool).stakeToken.address;
        ...
        activeStakeTokens[stakeToken] = false;
    }
}
...
contract Staking is ReentrancyGuard, ERC721, IStaking {
    ...
    // Mapping of address to Stakes info
    ...
    constructor(IERC20 _stakeToken, IERC20 _rewardToken)
        ERC721("Escrowed Verification Note", "EVN")
    {
    ...
    function deposit(uint256 _amount, uint256 _duration) external nonReentrant {
        ...
        stakes[msg.sender].push(
            Stake(_amount, block.timestamp, block.timestamp, 0, StakeDuration(_duration))
        );
        
        require(stakeToken.transferFrom(msg.sender, address(this), _amount), "Transfer failed");

        emit Staked(msg.sender, _amount, StakeDuration(_duration));
    }

    // Mints escrow note for msg.sender
    function createEscrow(uint256 _stakeId) external nonReentrant {
        Stake memory stake = stakes[msg.sender][_stakeId];
        uint256 rewards = calculateRewards(msg.sender, _stakeId);

        require(rewards > 0, "You have no rewards");
        // Can only claim weekly
        require(
            stake.timeEscrowLastCreated + 1 weeks < block.timestamp,
            "Too soon to create escrow"
        );

        stakes[msg.sender][_stakeId].timeEscrowLastCreated = block.timestamp;
        ...
        escrows[tokenId] = Escrow(
            _stakeId,
            rewards,
            block.timestamp + 30 weeks
        );

        emit EscrowCreated(_stakeId, rewards, block.timestamp + 30 weeks);
    }

    function exchangeEscrow(uint256 _escrowId) external nonReentrant {
        require(ownerOf(_escrowId) == msg.sender, "Not escrow owner");
        Escrow memory escrow = escrows[_escrowId];
        require(
            escrow.claimAfter <= block.timestamp,
            "Too soon to exchange escrow"
        );
        ...
        // TODO: Determine how and who will transfer the reward to staker
        rewardToken.transfer(msg.sender, amount);

        emit EscrowExchanged(escrow.stakeId, escrow.amount, escrow.claimAfter);
    }
    ...
    function withdraw(uint256 _stakeId) external nonReentrant {
        ...
        uint withdrawAfter = stake.timeDeposited;

        if (stake.duration == StakeDuration.ONE_WEEK) {
            withdrawAfter += 1 weeks;
        } else if (stake.duration == StakeDuration.FOUR_WEEKS) {
            withdrawAfter += 4 weeks;
        } else if (stake.duration == StakeDuration.TWELVE_WEEKS) {
            withdrawAfter += 12 weeks;
        }

        require(block.timestamp >= withdrawAfter, "Too soon to withdraw");

        uint256 _rewards = calculateRewards(msg.sender, _stakeId);
        uint256 _deposit = stake.deposited;

        delete stakes[msg.sender][_stakeId];

        stakeToken.transfer(msg.sender, _deposit);
        ...
        emit Withdrawed(msg.sender, _deposit, stake.duration);
    }
    ...
    {
        Stake[] memory _stakes = stakes[_user];

        for (uint256 i = 0; i < _stakes.length; i++) {
            _stakes[i].unclaimedRewards = calculateRewards(_user, i);
        }
    ...
    function minDeposit() public view returns (uint256) {
        // Must stake at least
        uint256 poolSize = stakeToken.balanceOf(address(this));
        // 1 if pool is empty otherwise 0.01% of total stake balance
        return poolSize == 0 ? 1 : poolSize / 10000;
    }

    function getEscrowInfo(uint escrowId) external view returns (Escrow memory escrow) {
        escrow = escrows[escrowId];
    }

    // Calculate the rewards since the last update on Deposit info
    function calculateRewards(address _staker, uint256 _stakeId)
        internal
        view
        returns (uint256 rewards)
    {
        ...
        // Different reward according to staking duration
        if (stake.duration == StakeDuration.ONE_WEEK) {
            rewardPerHour = 285; // 0.00285%/h or 25% APR
        } else if (stake.duration == StakeDuration.FOUR_WEEKS) {
            rewardPerHour = 399; // 0.00399%/h or 35% APR
        } else if (stake.duration == StakeDuration.TWELVE_WEEKS) {
            rewardPerHour = 456; // 0.00456%/h or 40% APR
        }

        return
            ((block.timestamp - stake.timeEscrowLastCreated) *
                stake.deposited *
                rewardsPerHour) /
            3600 /
            10000000;
...
interface IStaking {
    ...
    struct Stake {
        ...
        uint256 timeEscrowLastCreated;
        // Unclaimed rewards. Will be calculated dynamically and only used for client viewing
    ...
    struct Escrow {
        // The stake from which this note was created
        uint256 stakeId;
    ...
    function createEscrow(uint256 _stakeId) external;

    function exchangeEscrow(uint256 _escrowId) external;
