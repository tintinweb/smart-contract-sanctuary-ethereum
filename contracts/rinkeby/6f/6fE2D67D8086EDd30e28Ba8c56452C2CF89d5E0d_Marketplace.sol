...
contract Marketplace is AccessControl {
    ...
    function updatePlayer(address _nft, address _player, uint256 _nftId, uint16[] memory _addons, uint256[] memory _oldNfts) external {
        require(_nft == collectionInfo[_nft].nftAddress, "there is no collection for this player");
    ...
    function buyAddOn(address _nft, string memory _hash, address _to) public payable {
        ...
        require( playerInfo[_to].available[_hash] == true, "Player can't buy it yet." );
        ...
        payable(addOnMaker).transfer(msg.value);
    }
    ...
    function allowAddOn(string memory _hash, address _player) public {
        ...
        playerInfo[_player].available[_hash] = true;
    }
    ...
    function addNewProduction(address _nft, uint256 _nftId, uint256 _price) public returns (bool) {
        ...
        // require(playerProds[_nft][_nftId].flag != 1);
    ...
    // function setProdById(string memory _name, string memory _description, uint256 _price, string memory _hash) public returns (bool) {
    //     require(hasRole(PRODUCE_ROLE, _msgSender()), "Must have produce role to mint");
    //     require(playerProds[_hash].flag != 1);
    //     require(playerProds[_hash].owner == _msgSender(), "Only production owner can chnage the properties of nft.");
    //     playerProds[_hash] = NFTProd(_name, _description, _price, _msgSender(), 1);
    //     return true;
    // }
    
    function deleteProdByID(address _nft, uint256 _nftId) internal returns (bool) {
        // require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()) || (playerProds[_nftId].owner == _msgSender()), "Admin or product owner only can delete this.");
        uint arrIndex; 
        bool isExist = false;
    ...
    function buy(address to, address _nft, uint256 _nftId, uint256 _amount ) public payable {
        require(_amount == playerProds[_nft][_nftId].price, "Amount should be same with price");
        
        MACNFT(_nft).transferFrom(address(this), to, _nftId);
        payable(playerProds[_nft][_nftId].owner).transfer(msg.value);
        deleteProdByID(_nft, _nftId);
