    ...
    ReentrancyGuardUpgradeable
{
    ...
    uint256 public genesisMemberPreMintCounter;
    ...
    mapping(address => presaleUsers) public presaleList;
    mapping(address => presaleUsers) public genesisList;

    address[] internal presaleAddresses;
    ...
    function initialize() public initializer {
        ...
        baseUri = "https://s3.amazonaws.com/assets.thearchivesmint.xyz/token/";
        baseExtension = "/token-uri.json";

        saleRange = [200, 80, 1880];
        maxNFT = 2;
        ...
        genesisMemberPreMintCounter = 0;
        keySocietyCounter = 0; // id = 1
        genesisMemberCounter = 0; // id = 2

        merchantWallet = 0x29c82205694E73D91961E91B2Bc6d18f45352431;
    ...
    function updateTotalSupply(
        ...
        saleRange.push(_saleRangeIds[2]);
        maxNFT = _maxNft;
    }
    ...
    function upadateSaleStatus(
        bool _presale_status,
        bool _key_society,
        bool _genesis_status
    ) external virtual onlyOwner {
        presaleStatus = _presale_status;
    ...
    function presale(Order memory order, bytes memory signature)
        external
        payable
        virtual
    {
        ...
        require(status == true, "$GenesisV1: cannot purchase the token");
        require(
            presaleStatus && saleRange[0] >= genesisMemberPreMintCounter,
            "GenesisV1: Presale is closed"
        );
        if (presaleList[msg.sender].user == msg.sender) {
            presaleList[msg.sender].count += order.num;
        }

        if (presaleList[msg.sender].user == address(0)) {
            presaleUsers memory _data = presaleUsers(msg.sender, order.num);
            presaleList[msg.sender] = _data;

            presaleAddresses.push(msg.sender);
        }
        genesisMemberPreMintCounter += order.num;

        require(
            presaleList[msg.sender].count <= 2 && order.price == msg.value,
    ...
    function airDrop(uint256 from, uint256 to) external virtual onlyOwner {
        for (uint256 i = from; i <= to; i++) {
            for (uint256 j = 0; j < presaleList[msg.sender].count; j++) {
                tokenCounter += 1;
                _mint(presaleAddresses[i], tokenCounter);
                temp.push(tokenCounter);
                tempAddress.push(presaleAddresses[i]);
                tokenType[tokenCounter] = "PRE-SALE";
            }
        }
        emit AirDrop(tempAddress, temp, "PRE-SALE");
    ...
    function keySocietyClaim() external virtual {
        require(
            keySocietySaleStatus && saleRange[1] >= keySocietyCounter,
    ...
    {
        require(
            genesisSaleStatus && saleRange[2] >= genesisMemberCounter,
        ...
        require(
            genesisList[msg.sender].count <= maxNFT && order.price == msg.value,
            "GenesisV1: Exceeds max count or invalid price"
        );
    ...
    function mintTo(
        ...
        require(_eth_price == msg.value, "GenesisV1: Price is incorrect");
        ...
        emit crossmintToTokenDetails(to, tokenCounter, msg.value, _usd_price);
