    ...
    
{
  constructor(
    ...
    function mintToAdmin(uint256 _qty) public onlyOwner{
         require(_qty > 0, "Must mint at least 1 token.");
         require(currentTokenId() + _qty <= collectionSize, "Cannot mint over supply cap of 9999");
         _safeMint(_qty, true);
     }
    ...
    */
    ...
    function mintYourDeath(uint256 _amount) public payable {
        require(_amount >= 1, "Must mint at least 1 token");
        require(_amount <= maxBatchSize, "Cannot mint more than max mint per transaction");
        require(mintingOpen == true, "Minting is not open right now!");
...
contract DeathPassGenesisContract is ARTISTERC721A {
    constructor() ARTISTERC721A("DEATH PASS - GENESIS", "DPASS"){}
}