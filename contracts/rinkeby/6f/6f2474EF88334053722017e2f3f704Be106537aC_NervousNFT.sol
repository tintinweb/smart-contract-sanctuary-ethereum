...
import "@nervous-net/nervous-contract-kit/src/MultiTimedMerkleRoot.sol";
    ...
    MultiTimedSignedPasses(3)
{
    ...
    function crossMintPresale()
        external
        view
        returns (MultiTimedMerkleRoot.TimedRoot memory)
    {
        return _timedMerkleRoots[0];
    }
    ...
    /// @notice Mint Pass minting. Requires either valid pass or public sale
    ...
    /// @notice Crossmint presale minting
    function crossmintPresaleMintTo(
        ...
        requireCrossmint
        requireValidMint(numTokens, to)
        requireValidMerkleProof(to, merkleProof)
    {
        _mintTo(numTokens, to);
    }
    ...
    function setCrossMintPresale(uint64 startTimestamp, bytes32 root)
        ...
        onlyOwner
    {
        _setTimedMerkleRoot(0, root, startTimestamp);
    }
    ...
    modifier requireValidMerkleProof(address to, bytes32[] calldata proof) {
        if (block.timestamp < startPublicMintDate) {
            if (!_checkTimedMerkleRoots(to, proof)) {
