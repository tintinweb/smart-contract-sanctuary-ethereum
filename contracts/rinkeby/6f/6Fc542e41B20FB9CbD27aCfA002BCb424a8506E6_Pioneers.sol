...
contract Pioneers is IERC721, IERC721Metadata{ 
    ...
    function ambush(uint _ambusherTokenId, uint _victimTokenId) public{
        ...
        (,, bytes32 _inconvenienceId) = IPioneersTrails(trails).currentInconveniences(tokenToTrail[_victimTokenId]);
    ...
    function checkVulnerable(uint _tokenId, bytes32 _lastAmbush) internal view returns(bool){
        ...
        (string memory _pickedLetter, string memory _pickedInconvenience, bytes32 _inconvenienceId) = IPioneersTrails(trails).currentInconveniences(tokenToTrail[uint8(_tokenId)]);
