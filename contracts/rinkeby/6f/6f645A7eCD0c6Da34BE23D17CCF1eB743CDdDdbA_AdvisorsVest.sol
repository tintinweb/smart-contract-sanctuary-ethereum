...
/***
Daolympics tokens vesting contract
Advisors token lock-in contract
 */
contract AdvisorsVest is Ownable {
    ...
    /**When this vault was Unlock start time (UNIX Timestamp)*/
    uint256 public unLockedAt = 0;

    //Total Token Allocations
    uint256 public totalAllocation = 5 * (10**7) * (10**18);


    uint256 private month = 31 days;

    uint256 public constant vestingStages = 3;
    /**
     * Tokens vesting stage structure with vesting date and tokens allowed to unlock.
     */
    struct UnLockStage {
        uint256   startDate;//Start time of each stage
        uint256   period;//Total number of periods in the current stage
        uint256   periodPercentage;//Available percentage of each period
    }
    /**
     * Array for storing all vesting stages with structure defined above.
     */
    UnLockStage[vestingStages] public stages;
    /**
    ...
    /*
     * @param _dlpToken Address of LlpToken that will be locked on contract.
     * @param _withdraw Address of tokens receiver when it is unlocked.
     */
    constructor(address _dlpToken, address _withdraw) {
        require(address(_dlpToken) != address(0),"Llptoken cannot be the zero address");
        require(address(_withdraw) != address(0),"withdraw cannot be the zero address");
        dlpToken = IERC20(_dlpToken);
        withdrawAddress = _withdraw;
    }

    /**
        `unLockTime` Unlock start time
     */
    function lock(uint256 unLockTime) internal notLocked onlyOwner {
    
         unLockedAt=unLockTime;
        //Need to release 48%
        stages[0].startDate = unLockedAt;
        stages[0].period = 12;
        stages[0].periodPercentage = 400;

        //Need to release 32%
        stages[1].startDate = unLockedAt + month * 12;
        stages[1].period = 12;
        stages[1].periodPercentage = 270;

        //Need to release 20%
        stages[2].startDate = unLockedAt + month * 24;
        stages[2].period = 12;
    ...
    function allocate(uint256 unLockTime) public notLocked onlyOwner {
        ...
        require(getTotalBalance() == totalAllocation, "Please transfer the 'totalallocation' token to the contract first");
        require(unLockTime >= block.timestamp , "Unlock start time cannot be less than the current system time");
        emit Allocated(withdrawAddress, totalAllocation);
        lock(unLockTime);
    }
 ...
 /**
        ...
        returns (uint256)
    {
        ...
        uint256 totalAllowedToWithdraw = totalAllocation.mul(unlockedPercentage).div(INVERSE_BASIS_POINT);
   ...
   /**
    ...
    function getStagePrecent(UnLockStage memory _unLockStage,uint8 stage) private view returns (uint256){
        uint256 _period = (block.timestamp.sub(_unLockStage.startDate)).div(month);
        
        uint256 _periodMode = (block.timestamp.sub(_unLockStage.startDate)).mod(month);
        //If the number of periods is not an integer, add 1 to its integer part
        //eg:uint256 _period=2.1,uint256 _periodMode=1;final result: _period=2+1;   
        if(_periodMode>0 ){
        ...
        for (uint8 i = 0; i < stage; i++) {
           uint256 prePercentage= stages[i].period.mul(stages[i].periodPercentage);
        ...
        if((stage.add(1))==vestingStages && _period == _unLockStage.period && allowedPercent < INVERSE_BASIS_POINT){
