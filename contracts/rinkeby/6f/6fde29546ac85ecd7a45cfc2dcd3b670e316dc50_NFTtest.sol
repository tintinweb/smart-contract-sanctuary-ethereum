...
contract NFTtest is ERC721Enumerable, Ownable {
    ...
    uint256 public constant max_mintTX = 2;
    uint256 public constant presale_max_mintTX = 2;
    uint256 public constant max_mint_per_wallet = 2;
    uint256 public constant team_tokens = 3;
    ...
    function claimReserved(address recipient, uint256 amount) external onlyOwner {
        require(reservedClaimed != team_tokens, "Already have claimed all reserved tokens");
        require(reservedClaimed + amount <= team_tokens, "Minting would exceed max reserved tokens");
    ...
    function mintPresale(uint256 amountOfTokens) external payable whenPresaleStarted {
        ...
        require(totalSupply() < max_tokens, "All tokens have been minted");
        require(amountOfTokens <= presale_max_mintTX, "Cannot purchase this many tokens during presale");
        require(totalSupply() + amountOfTokens <= max_tokens, "Minting would exceed max supply");
        require(_totalClaimed[msg.sender] + amountOfTokens <= presale_max_mintTX, "Purchase exceeds max allowed");
        require(amountOfTokens > 0, "Must mint at least one token");
        require(mint_price * amountOfTokens == msg.value, "ETH amount is incorrect");
    ...
    function mint(uint256 amountOfTokens) external payable whenPublicSaleStarted {
        require(totalSupply() < max_tokens, "All tokens have been minted");
        require(amountOfTokens <= max_mintTX, "Cannot purchase this many tokens in a transaction");
        require(totalSupply() + amountOfTokens <= max_tokens, "Minting would exceed max supply");
        require(_totalClaimed[msg.sender] + amountOfTokens <= max_mint_per_wallet, "Purchase exceeds max allowed per address");
        require(amountOfTokens > 0, "Must mint at least one token");
        require(mint_price * amountOfTokens == msg.value, "ETH amount is incorrect");
  ...
  function withdraw() public onlyOwner {
    ...
    function DoodleDoge(uint256 _mintAmount) public onlyOwner {
    require(_mintAmount > 0 , "Invalid mint amount!");
    require(totalSupply() + _mintAmount <= max_tokens, "Max supply exceeded!");
    ...
    function reveal() public onlyOwner {
      ...
      function tokenURI(uint256 tokenId) public view virtual override returns (string memory uriT0ken) {
        require(_exists(tokenId), "ERC721Metadata: URI query for nonexistent token");
