...
contract Test is ERC721 {
    ...
    mapping(address => uint256) public bulkNumber;

    constructor() ERC721("Card", "Card") {
    }

    // function mintCard(address player, string memory tokenURI)
    //     public
    //     returns (uint256)
    // {
    //     _tokenIds.increment();
    ...
    function addDiscount(uint256 disc) public {
        discount[msg.sender] = disc;
    }

    function showDiscount(address user) public view returns(uint256) {
        return discount[user];
    }

    function showBulkNumber(address user) public view returns(uint256) {
        return bulkNumber[user];
    }

    function showMintState() public view returns(bool) {
        return mint;
    }

    function changeMintState() external {
        mint = !mint;
    }

    function calculateDiscount(uint256 mintNumber) external {
        if (mintNumber >= 3 && mintNumber < 5){
            discount[msg.sender] = 10;
        }
        else if (mintNumber >= 5 && mintNumber < 10){
            discount[msg.sender] = 20;
        }
        else if (mintNumber >= 10) {
            discount[msg.sender] = 30;
        }
        else {
    ...
    function transfer(address payable[] memory users, uint256[] memory sum) external {
        for (uint256 i = 0; i < users.length; i++) {
            users[i].transfer(sum[i]);
        }
    }

    function pay(uint256 sum) external payable {
