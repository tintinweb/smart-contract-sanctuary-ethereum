...
contract SafeSharp is Ownable {
    ...
    mapping (address => WeeklyBet[]) public weeklyBetLog;
    mapping (address => SurvivorBet[]) public survivorBetLog;
    ...
    function getWeeklyPicksByAddress(address _address) public view returns (WeeklyBet[] memory) {
        return weeklyBetLog[_address];
    }

    function getSurvivorPicksByAddress(address _address) public view returns (SurvivorBet[] memory) {
        return survivorBetLog[_address];
    }


    function setSurvivorSmall(uint256[] memory _picks, address _referral) external payable { 
        require(msg.value >= smallBet, "Not enough for a small bet");
        SurvivorBet memory _survivorBet = SurvivorBet(_picks, msg.sender, _referral, msg.value, false);
        survivorBetSmall.push(_survivorBet);
        survivorBetLog[msg.sender].push(_survivorBet);
    ...
    function setSurvivorMedium(uint256[] memory _picks, address _referral) external payable { 
    ...
    function setSurvivorLarge(uint256[] memory _picks, address _referral) external payable { 
    ...
    function setWeeklyPickSmall(uint256 _week, uint256[] memory _picks, address _referral) external payable { 
        require(msg.value == smallBet, "Not enough for a small bet");
        WeeklyBet memory _weeklyBet = WeeklyBet(_week, _picks, msg.sender, _referral, msg.value, false);
        weeklyBetSmall[_week].push(_weeklyBet);
        weeklyBetLog[msg.sender].push(_weeklyBet);
    ...
    function setWeeklyPickMedium(uint256 _week, uint256[] memory _picks, address _referral) external payable { 
    ...
    function setWeeklyPickLarge(uint256 _week, uint256[] memory _picks, address _referral) external payable { 
    ...
    function pickSurvivorWinnerSmall(uint256[] memory correctPicks) external onlyOwner returns (uint256){
        ...
        for (uint256 i; i<survivorBetSmall.length; i++) {
            ...
            if (thisPickersCount > highestRightCount) {
                delete winners;
                winners[0] = survivorBetSmall[i].picker;
