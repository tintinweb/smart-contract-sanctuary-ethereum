...
contract AlexisAndre is iAlexisAndre, ERC721, ReentrancyGuard, Ownable {
  uint256 public constant TYPE = 4;
  uint256 public constant LEVEL = 8;
  uint256 public constant EACH_SUPPLY = 20;
  uint256 public constant EACH_SUPPLY_SPECIAL = 5;
  uint256 public constant MAX_SUPPLY = TYPE * (LEVEL - 1) * EACH_SUPPLY + TYPE * EACH_SUPPLY_SPECIAL;
  uint256 public constant PRICE = 0.1 ether;
  uint256 public constant PRICE_SPECIAL = 0.5 ether;
  ...
  modifier checkMintable(uint256 tokenId) {
    ...
    require(
      ...
      "AlexisAndre: The tokenId is not available yet."
    );
    _;
  }
  ...
  function currentDay() public view returns (uint256) {
    ...
    console.log("t: %d, startDate: %d", t, startDate);
    return (t - startDate) / A_DAY + 1;
  }

  function currentLevel() public view returns (uint256) {
    uint256 _currentDay = currentDay();
    return _currentDay % LEVEL;
  }

  function mintableLastTokenId() public view returns (uint256) {
    uint256 _currentDay = currentDay();
    uint256 currentLevel = _currentDay % LEVEL;
//    uint256 tokenIdFrom = (_currentDay - 1) * EACH_SUPPLY - (_currentDay - 1) / LEVEL * (EACH_SUPPLY - EACH_SUPPLY_SPECIAL);
    uint256 tokenIdTo = _currentDay * EACH_SUPPLY - _currentDay / LEVEL * (EACH_SUPPLY - EACH_SUPPLY_SPECIAL) - 1;
    console.log("_currentDay: %d, currentLevel: %d, tokenIdTo: %d", _currentDay, currentLevel, tokenIdTo);
    return tokenIdTo;
  }

  function price(uint256 tokenId) public view returns (uint256) {
    if ((tokenId <= 140 && tokenId <= 144) ||
      (tokenId <= 285 && tokenId <= 289) ||
      (tokenId <= 430 && tokenId <= 434) ||
      (tokenId <= 575 && tokenId <= 579)) {
  ...
  {
    require(_exists(tokenId), "AlexisAndre: URI query for nonexistent token");
    return
      string(
        bytes.concat(
          bytes(_baseTokenURI),
  ...
  {
    require(isOnSale, "AlexisAndre: Not on sale");
    require(msg.value == PRICE, "AlexisAndre: Invalid value");

    bytes32 hash = keccak256(abi.encodePacked(block.number, msg.sender));
    tokenIdToSeed[tokenId] = hash;
...
//      require(id < MAX_SUPPLY, "AlexisAndre: Invalid token id");
