  ...
  Version
{
    ...
    noContracts
  {
    ...
    if (!_isGroupPaymentCorrect(group, count)) {
      revert InsufficientPayment();
    }
    if (!_isValidMaxPerWallet(group, _numberMinted(msg.sender) + count)) {
      revert ExceedsGroupMaxPerWallet();
    }
    if (!_isGroupStarted(group)) {
      revert MintNotStarted();
    }
    if (!_isBeforeTimestamp(publicConfig.endTime)) {
  ...
  function publicMint(uint256 count)
    ...
    noContracts
  {
    if (msg.value < publicConfig.mintPrice * count)
      revert InsufficientPayment();

    if (
      publicConfig.startTime != 0 && _isBeforeTimestamp(publicConfig.startTime)
    ) {
      revert MintNotStarted();
    }

    if (publicConfig.endTime != 0 && _isAfterTimestamp(publicConfig.endTime)) {
      revert MintCompleted();
    }
  ...
  function _baseURI() internal view virtual override returns (string memory) {
    return Metadata._getBaseTokenURI();
  }
...
abstract contract SignatureMintable is Timing {
  ...
  // Mapping of signer -> SigningGroupConfig
