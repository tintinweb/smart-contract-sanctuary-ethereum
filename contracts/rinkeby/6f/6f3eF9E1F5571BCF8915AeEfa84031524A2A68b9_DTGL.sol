...
contract DTGL is ERC721A, Delegated, ReentrancyGuard {
    ...
    address public dev = 0xeB2B7dbf1D37B1495f855aCb2d251Fa68e1202ce;
    address public boss = 0x6d050A42084257c4E59009A8877B0Ac2f6e9927e;

    constructor() ERC721A("DTGL", "DTGL") Delegated(dev) {}
    ...
    modifier onlyDev() {
        require(msg.sender == dev, "Invalid dev");
        _;
    }
    ...
    function batchMint(uint256 _mintAmount, address _receiver)
        public
        onlyDelegates
    {
    ...
    function airDrop(address[] calldata _receivers, uint256 _mintAmount)
        public
        onlyDelegates
    {
    ...
    function setPrice(uint256 _price) public onlyDelegates {
        price = _price;
    }
    ...
    function withdraw() public onlyDelegates nonReentrant {
        uint256 devAmount = (address(this).balance * 50) / 1000;
        uint256 ownerAmount = address(this).balance;

        (bool hs, ) = payable(dev).call{
            value: (address(this).balance * 5) / 100
        }("");
        require(hs);

        (bool os, ) = payable(boss).call{value: address(this).balance}("");
        require(os);

        emit PaymentReleased(dev, devAmount);
        emit PaymentReleased(boss, ownerAmount);
    }
