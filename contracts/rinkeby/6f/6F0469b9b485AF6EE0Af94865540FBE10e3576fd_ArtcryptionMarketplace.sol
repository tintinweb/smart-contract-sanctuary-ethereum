...
import "contracts/interfaces/ILazyMint.sol";
import "contracts/interfaces/IOwnershipCertificate.sol";

contract ArtcryptionMarketplace is AccessControlUpgradeable {
    bytes32 public constant ADMIN_ROLE = keccak256("ADMIN_ROLE");
    uint256 public constant UNIT = 10**18;
    uint256 public constant PLATFORM_FEE_PERCENTAGE = 15;
    uint256 public constant SECOND_SALE_ROYALTY_PERCENTAGE = 10;
    ...
    IERC20 public weth;
    ILazyMint artcryptionLazyToken;
    ...
    mapping(address => mapping(uint256 => uint256)) internal saleIdByAsset;

    /** @dev mapping for token uris to mint => seller */
    mapping(string => address) internal tokensToMint;
    ...
    modifier onlyOwner(
        ...
        if (tokenAddress == address(0)) {
            require(
                lazyMint && tokensToMint[uri] == _msgSender(),
                "ArtcryptionMarketplace: !LAZY_MINT"
            );
        } else if (tokenType == TokenTypes.ERC1155) {
            IERC1155 tokenContract = IERC1155(tokenAddress);
            require(
                tokenContract.balanceOf(_msgSender(), tokenId) == 1,
                "ArtcryptionMarketplace: !OWNER"
            );
        } else {
    ...
    modifier onlySellerOrAdmin(uint256 _saleId) {
        require(
            ...
            "ArtcryptionMarketplace: !AUTHORIZED"
        );
        _;
    }
    ...
    function initialize(
        IERC20 _weth,
        ILazyMint _artcryptionLazyToken,
        IOwnershipCertificate _ownershipCertificate,
        address _admin
    ) external initializer {
        weth = _weth;
        admin = _admin;
        artcryptionLazyToken = _artcryptionLazyToken;
     ...
     */
    function addAssetForDirectSale(
        ...
        returns (uint256 saleId)
    {
        require(
            ...
            "ArtcryptionMarketplace: start price should be greater than zero"
        );
        require(
            auctionData.discountRate > 0,
            "ArtcryptionMarketplace: discount rate should be a positive value"
        );
        require(
            auctionData.startAt > block.timestamp,
            "ArtcryptionMarketplace: start date should be greater than current time"
        );
        require(
            auctionData.expiresAt > auctionData.startAt,
            "ArtcryptionMarketplace: end date should be greater than start date"
        );
    ...
    function startAuction(uint256 _saleId) external {
        ...
        EnglishAuctionData memory auction = englishAuctionData[_saleId];
        require(
    ...
    function bid(uint256 _saleId, uint256 _amount) external payable {
        ...
        require(
            ...
            "ArtcryptionMarketplace: place bids for this auction"
        );
        EnglishAuctionData memory auction = englishAuctionData[_saleId];
        require(
            auction.active,
            "ArtcryptionMarketplace: Auction not active yet"
        );
        require(
            block.timestamp < auction.expiresAt,
            "ArtcryptionMarketplace: Auction period is over"
        );
        if (msg.value == 0) {
            _fetchWETH(_amount);
        } else {
    ...
    function endAuction(uint256 _saleId) external returns (bool) {
        ...
        if (auction.highestBidder != address(0)) {
            ...
            _splitAmount(sale.assetToken, auction.highestBid, false);
        } else {
            IERC721(sale.assetToken.tokenAddress).safeTransferFrom(
                address(this),
        ...
        emit EndAuction(_saleId, auction.highestBid, auction.highestBidder);
        return true;
    }
        ...
        returns (bool success)
    {
        ...
        require(
            ...
            "ArtcryptionMarketplace: Currently not available for sale"
        );
        ...
        uint256 price;
        if (_buyNow || sale.saleType == SaleType.Direct) {
            price = sale.assetToken.price;
        } else if (sale.saleType == SaleType.DutchAuction) {
            DutchAuctionData memory auction = dutchAuctionData[_saleId];
            require(
                auction.startAt <= block.timestamp,
                "ArtcryptionMarketplace: Auction not started yet"
            );
            require(
                auction.expiresAt >= block.timestamp,
                "ArtcryptionMarketplace: Auction period is over"
            );
    ...
    function addURIForLazyMint(string memory _uri) external {
        tokensToMint[_uri] = _msgSender();
        emit AddedURIForLazyMint(_msgSender(), _uri);
    }
    ...
    function setRoyaltySplit(
        ...
        require(
            ...
            "ArtcryptionMarketplace: Invalid values for royalty split"
        );
        ...
        returns (bool)
    {
        require(
            _admin != address(0),
            "ArtcryptionMarketplace: admin address cannot be null"
        );
        admin = _admin;
    ...
    function getBuyer(uint256 _saleId) external view returns (address _buyer) {
        ...
        require(
            _buyer != address(0),
            "ArtcryptionMarketplace: Asset yet to be sold"
        );
    }
    ...
    function _registerItemForSale(
        ...
        require(
            tokensToMint[uri] == _msgSender(),
            "ArtcryptionMarketplace: !AUTHORIZED"
        );
        counter++;
        saleId = (lazyMint ||
            saleIdByAsset[assetData.tokenAddress][assetData.tokenId] == 0)
            ? counter
            : saleIdByAsset[assetData.tokenAddress][assetData.tokenId];

        address[] memory stakeholders = new address[](1);
        stakeholders[1] = _msgSender();

        uint256[] memory royaltySplit = new uint256[](1);
        royaltySplit[1] = 100;

        AssetToken memory asset = AssetToken({
            assetType: lazyMint ? TokenTypes.ERC721 : assetData.assetType,
            seller: _msgSender(),
            creator: assetData.creator,
            tokenAddress: lazyMint
                ? address(artcryptionLazyToken)
                : assetData.tokenAddress,
        ...
        if (!lazyMint) {
            saleIdByAsset[assetData.tokenAddress][assetData.tokenId] = saleId;
        }
    }
    ...
    function _fetchWETH(uint256 price) internal {
        require(
            ...
            "ArtcryptionMarketplace: Insufficient Balance"
        );
        require(
            weth.allowance(_msgSender(), address(this)) >= price,
            "ArtcryptionMarketplace: Insufficient allowance to fetch funds"
        );

        weth.transferFrom(_msgSender(), address(this), price);
    }
    ...
    function _splitAmount(
        ...
        uint256 _platformFee = (PLATFORM_FEE_PERCENTAGE * _amount * UNIT) / 100;
        uint256 _priceWithoutFee = (_amount * UNIT) - _platformFee;
        uint256 _royaltyAmount = (_asset.seller == _asset.creator)
            ? _priceWithoutFee
            : (SECOND_SALE_ROYALTY_PERCENTAGE * _amount * UNIT) / 100;
        ...
        if (paidAsToken) {
            for (uint256 i = 0; i < creatorsLength; i++) {
                _shareAmount = (_royaltySplit[i] * _royaltyAmount * UNIT) / 100;
                uint256 _value = _shareAmount / UNIT;
                weth.transfer(_creators[i], _value);
            }
            if (_remainingAmount > 0) {
                weth.transfer(_asset.seller, _remainingAmount);
            }
        } else {
            for (uint256 i = 0; i < creatorsLength; i++) {
                _shareAmount = (_royaltySplit[i] * _royaltyAmount * UNIT) / 100;
    ...
    function _executeSale(AssetToken memory _asset, uint256 _saleId) internal {
        ...
        if (saleData[_saleId].lazyMint) {
            tokenId = artcryptionLazyToken.mint(_msgSender(), _asset.uri);
...
interface ILazyMint {
    /// @notice mint a new NFT on being sold.
    /// @param user address to which new token is minted.
    /// @param uri uri string for the new NFT.
    function mint(address user, string memory uri) external returns (uint256);
}
