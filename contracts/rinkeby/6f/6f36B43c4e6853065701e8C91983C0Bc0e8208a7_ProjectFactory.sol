    ...
    ERC2771ContextUpgradeable
{
    ...
    uint256 public override projectCost;
    /// @inheritdoc IProject
        ...
        override
    {
        ...
        // Decode params from _data
        (bytes memory _hash, uint256 _nonce, address _projectAddress) = abi
            .decode(_data, (bytes, uint256, address));
        ...
        override
    {
        ...
        for (uint256 i; i < _length; ) {
            ...
            _checkValidAmount(_taskCosts[i]);
            _totalCost += _taskCosts[i];
        ...
        override
    {
        ...
        if (getAlerts(_taskID)[uint256(Lifecycle.SCConfirmed)]) {
            // If subcontractor has confirmed.
            _checkSignatureTask(_data, _signature, _taskID);
        } else {
        ...
        nonReentrant
    {
        ...
        require(
            tasks[_taskID].state != TaskStatus.Complete,
            "Project::!Active"
        );
        ...
        if (_newCost != _taskCost) {
            ...
            if (getAlerts(_taskID)[uint256(Lifecycle.TaskAllocated)]) {
                ...
                if (_newCost < _taskCost) {
                    ...
                    _autoWithdraw(_withdrawDifference);
                }
            ...
            projectCost = projectCost + _newCost - _taskCost;
        ...
        override
    {
        ...
        // Decode params from _data
        (address _project, uint256 _taskID, , , ) = abi.decode(
            _data,
        ...
        if (_taskID == 0) {
            ...
            require(
                _signer == builder ||
                    _signer == contractor ||
                    _signer == tasks[_taskID].subcontractor,
                "Project::!(GC||Builder||SC)"
            );

            if (_signer == tasks[_taskID].subcontractor) {
                // If sender is task's subcontractor, revert if invitation is not accepted.
                require(
                    getAlerts(_taskID)[uint256(Lifecycle.SCConfirmed)],
                    "Project::!SCConfirmed"
                );
            }
        }
     ...
     */
    function _autoWithdraw(uint256 _amount) internal {
     ...
     */
    function _checkSignature(bytes calldata _data, bytes calldata _signature)
        internal
    {
...
interface IDisputes {
     ...
     */
    function raiseDispute(bytes calldata _data, address _signer) external;
...
library SignatureDecoder {
    ...
    * @return signer address - recovered address
    */
    function recoverKey(
        bytes32 messageHash,
        ...
        require(v == 27 || v == 28, "HomeFi::invalid signature");

        // solium-disable-next-line arg-overflow
        signer = ecrecover(toEthSignedMessageHash(messageHash), v, r, s);

        require(signer != address(0), "HomeFi::invalid signature");
    }
