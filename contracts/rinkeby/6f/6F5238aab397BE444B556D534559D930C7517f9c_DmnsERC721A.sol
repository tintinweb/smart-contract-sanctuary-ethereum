...
// File: contracts/DmnsERC721A.sol
...
contract DmnsERC721A is ERC721A, Ownable {
    ...
    constructor() ERC721A("DmnsERC721A", "DMNSA") {
        setHiddenMetadataUri("ipfs://QmPeuNgAkSodc49RqPTzaMSQ1ZxiEdCRCg71icU65eheKG/");
    }
        ...
        returns (string memory)
    {
        ...
        if(revealed == false) {
            string memory currentHiddenMetadataUri = _getHiddenMetadataUri();

            return bytes(currentHiddenMetadataUri).length > 0
                ? string(abi.encodePacked(currentHiddenMetadataUri, _tokenId.toString(), uriSuffix))
                : "";
        }
    ...
    function _getHiddenMetadataUri() internal view virtual  returns (string memory) {
