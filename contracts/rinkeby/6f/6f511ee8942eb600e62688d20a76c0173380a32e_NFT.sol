...
contract NFT is Ownable, ERC721, IERC721Receiver {
    ...
    mapping(uint256 => address) public nftFrom; //来源
    ...
    mapping(uint256 => address) public nftUseFor; //nft使用者
    ...
    bool public isTemp = true; //是否临时nft

    address public usdAddress = 0x99924AA7BBc915Cb2BEE65d72343734b370a06f1; //usdt合约
    uint256 public basePrice = 100 * 1e18;
    uint256 public tempPrice = 10 * 1e18;
    uint256 public maxNft = 8888;
    ...
    //是否临时nft
    function setTemp(bool _value) public onlyOwner {
        isTemp = _value;
    }
    ...
    function _mint(address caller, string memory tokenURI)
        private
        returns (uint256)
    {
        //白名单才允许铸造
        ...
        nftFrom[newItemId] = msg.sender; //创作来源
        ...
        returns (uint256)
    {
        ...
        uint256 newItemId = _mint(caller, tokenURI);
        if (basePrice > 0) {
        ...
        returns (uint256)
    {
        ...
        nftTemp[newItemId] = true;
        return newItemId;
    }

    //激活nft并收取铸造费用
    function useNftPrice(address account_, uint256 tokenId_)
        public
        onlyMiner
        returns (bool)
    {
        ...
        nftUseFor[tokenId_] = msg.sender;

        emit UseNft(ownerOf(tokenId_), msg.sender, tokenId_);
        return nftTemp[tokenId_];
    }
