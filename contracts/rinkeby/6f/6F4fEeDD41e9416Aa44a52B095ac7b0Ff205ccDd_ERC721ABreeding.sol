...
contract ERC721ABreeding is ERC721A, Ownable {
    ...
    function breedableOf(address _address) public view returns (uint256 _available) {
        uint256 availableFemales = 0;
        uint256 availableMales = 0;
        for (uint16 i = 0; i < 10069; i++) {
            if (momContract.ownerOf(i) == _address && !isMom[i]) {
                availableFemales++;
            }
        }
        for (uint16 i = 0; i < 10069; i++) {
            if (dadContract.ownerOf(i) == _address && !isDad[i]) {
                availableMales++;
            }
        }
        uint256 available = availableFemales < availableMales ? availableFemales : availableMales;
        return available;
    }
