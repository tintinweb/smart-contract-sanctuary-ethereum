...
contract AdventNFT is ERC721A, Ownable {
    ...
    uint256 public AllowStartTime = 1660593784;
    uint256 public WLStartTime = 1660591804;
    uint256 public MAX_SUPPLY = 6666;    //total supply
    uint256 public Max_Mint = 3;  //max mint
    uint256 public price = 0.11 ether;  //price
    uint256 public wl_price = 0.099 ether; // wl price
    ...
    constructor(
        ...
        bytes32 _wl, bytes32 _allow) ERC721A("Advent NFT ", "ADV") {
            baseURI = _uri;
    ...
    function setWlId(bytes32 _code) public onlyOwner {
        Whitelist_OG = _code;
    }

    function setAllowId(bytes32 _code) public onlyOwner {
        AllowList = _code;
    }
    ...
    function WLtime(uint256 _time) public onlyOwner {
        require(block.timestamp<_time,"Past time not allowed");
        WLStartTime = _time;
    }

    function MintTime(uint256 _time) public onlyOwner {
        require(block.timestamp<_time,"Past time not allowed");
        require(WLStartTime<_time,"Time must be after whitelist sale");
        AllowStartTime = _time;
    }
    ...
    function mint(uint256 quantity, bytes32[] memory sign) external payable {
        ...
        if(block.timestamp>AllowStartTime) {
            ...
            require(quantity <= (Max_Mint-Max_Minted[msg.sender]), "Maximum mint per transaction limit reached");
