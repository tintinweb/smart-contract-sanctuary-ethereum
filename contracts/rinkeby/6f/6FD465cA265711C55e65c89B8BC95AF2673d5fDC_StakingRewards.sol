...
contract StakingRewards {
    ...
    function unstake(uint256 stakingId) external {
        ...
        uint256 availableCount = (si.amount * fee / 100) *  stakingPeriod[si.stakingtype] / year;
        if(availableCount > si.claimedCount) {
            getReward(stakingId);
        }
    ...
    function getReward(uint256 stakingId) public {
        ...
        require(availableAmount > si.claimedCount, "no available reward amount");

        _allStakings[_allStakingIndex[stakingId]].claimedCount = availableAmount;

        rewardsToken.transfer(msg.sender, (availableAmount - si.claimedCount));
    }

    function getTotalLockedAmount(address addr) public view returns (uint256) {
        uint256 amount = 0;
        
        for (uint256 i = 0; i < balances[addr]; i++) {
            uint256 stakingId = stakingOfOwnerByIndex(addr, i);
            StakingInfo memory si = stakingById(stakingId);
