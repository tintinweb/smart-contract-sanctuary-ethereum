...
contract Shadow is ERC721, Ownable {
  ...
  // WIPE ALL BASIC TRANSFER FUNCTIONALITY
  ...
  */
  function approve(address to, uint256 tokenId) public pure override {
    require(false, "approve() fail: Shadow token cannot be transferred");
  }
  ...
  function setApprovalForAll(address operator, bool approved) public pure override {
    require(false, "setApprovalForAll() fail: Shadow token cannot be transferred");
  }

  function isApprovedForAll(address owner, address operator) public pure virtual override returns (bool) {
      return false;
  }
  ...
  function transferFrom(
    ...
    require(false, "transferFrom() fail: Shadow token cannot be transferred");
  }
  ...
  function safeTransferFrom(
    ...
    require(false, "safeTransferFrom() fail: Shadow token cannot be transferred");
  }



  // OVERRIDE BASIC OWNERSHIP FUNCTIONS
  ...
  function _exists(uint256 _tokenId) internal view override returns (bool) {
    require(initialized, "Not initialized");
    entity.ownerOf(_tokenId); //with revert if !_exists()
    return true;
  }
  ...
  // IMPLEMENT NEW OWNERSHIP CIRCUITRY

  function broadcastOwner(uint256 _tokenId) public {
    address owner = entity.ownerOf(_tokenId);
  ...
  function initialize(address _entityAddress) external onlyOwner {
    ...
    for(uint256 i = 0; i < entity.totalSupply(); i++){
      broadcastOwner(entity.tokenByIndex(i));
    }
