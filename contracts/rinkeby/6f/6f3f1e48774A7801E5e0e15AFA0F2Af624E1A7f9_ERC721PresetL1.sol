...
contract ERC721PresetL1 is ERC721Settleable, Pausable {
    ...
    /// - It must be called by only admin.
    ///
    /// @notice Emits a {SettlementEnabled} event.
    function enableSettlement() external {
        require(
            hasRole(DEFAULT_ADMIN_ROLE, _msgSender()),
            "must have admin role"
        );
        super._enableSettlement();
    }
...
import "../utils/rollup/RollupMessage.sol";
...
abstract contract Settleable is IRollupReceiver {
    ...
    function _enableSettlement() internal {
        _settleable = true;
        emit SettlementEnabled(msg.sender);
    }
    ...
    function receiveRollupInfo(
        ...
        RollupState memory rollupState;
        rollupState = abi.decode(state, (RollupState));

        require(
            rollupState.idx >= _executeBatchNonce[nonceAndID],
            "Settleable: already executed"
        );
        require(
            _executeBatchNonce[nonceAndID] == rollupState.idx,
            "Settleable: invalid batch index"
        );
        require(
            MerkleProof.verifyCalldata(
                proof,
                rollupInfo.stateRootHash,
                keccak256(state)
            ),
        ...
        _settle(rollupInfo.sender, rollupState.tag, entries);
    }
...
library RollupMessage {
    uint16 public constant NOOP_TAG = 0;
    uint16 public constant ERC721_TOKEN_IDS_TAG = 1;
    uint16 public constant ERC20_BALANCE_TAG = 1;
...
///
/// note that this is a stripped down version of Open Zeppelin's MerkleProof.
