...
contract FlipKit is Initializable, OwnableUpgradeable, IFlipKit {
    ...
    address private _flipImplementation;
    address private _redeemableImplementation;
    address private _trustedForwarder;

    function initialize(
        address flipImplementation,
        address redeemableImplementation,
        address trustedForwarder
    ) public initializer {
        __Ownable_init();
        _treasury = _msgSender();
        _flipImplementation = flipImplementation;
        _redeemableImplementation = redeemableImplementation;
        _trustedForwarder = trustedForwarder;
    }

    function createFlipCollection(
        string memory name,
        ...
        emit CollectionCreated(deployed, affiliate, name, symbol);
    }

    function createRedeemableCollection(
        string memory name,
        string memory symbol,
        address affiliate
    ) external {
        address deployed = _createCollection(
            _redeemableImplementation,
            name,
            symbol,
            affiliate
        );
    ...
    function setFlipImplementation(address implementation) public onlyOwner {
        _flipImplementation = implementation;
    }

    function setRedeemableImplementation(address implementation) public onlyOwner {
        _redeemableImplementation = implementation;
    }
    ...
    function _createCollection(
        ...
        IBaseCollection flipCollection = IBaseCollection(deployed);
        flipCollection.initialize(
            name,
            symbol,
            _msgSender(),
            _trustedForwarder
        );
        flipCollection.transferOwnership(_msgSender());
...
interface IFlipKit {
     ...
     */
    event CollectionCreated(address indexed collectionAddress, address affiliate, string name, string symbol);
