...
contract BatchSale is ERC1155Holder, ReentrancyGuard {
    ...
    mapping(uint256 => AuctionInfo) public fIdToAuction;
    ...
    event Contribution(address buyer, uint256 fId, uint256 amount);
    event WithdrawContribution(address user, uint256 fId, uint256 amount);
    event WithdrawClaim(address user, uint256 fId, uint256 contribution);
    event UpdateReserve(uint256 fId, uint256 reserve);
    event Cancel(uint256 fId, uint256 amount);
    event Close(uint256 fId, uint256 reserve, uint256 total);
    ...
    function getUserContribution(uint256 fId, address who)
        ...
        returns (uint256)
    {
        return fIdToAuction[fId].contributions[who];
    }

    /// @notice Returns a tuple with the start and end time for the auction
    /// @param fId The id of the fractions
    /// @return checks the mapping of auctions and returns the start and end time
    function getStartEnd(uint256 fId) public view returns (uint256, uint256) {
        return (fIdToAuction[fId].start, fIdToAuction[fId].end);
    }
    ...
    /// @notice modifier to decorate functions only callable auction before an auciton
    /// @param fId The id of the fractions
    ...
    function launch(
        ...
        fIdToAuction[_fId].amount = _amount;
        fIdToAuction[_fId].start = _start;
        fIdToAuction[_fId].end = _end;
        fnft.safeTransferFrom(msg.sender, address(this), _fId, _amount, "0");
        emit Auction(_fId, msg.sender, _amount, _start, _end, _reserve);
    }
        ...
        whileActive(_fId)
    {
        ...
        fIdToAuction[_fId].totalContributed += msg.value;
        fIdToAuction[_fId].contributions[msg.sender] += msg.value;
        emit Contribution(msg.sender, _fId, msg.value);
    }
    ...
    function leave(uint256 _fId, uint256 _amount)
        ...
        whileActive(_fId)
    {
        uint256 total = fIdToAuction[_fId].totalContributed;
        uint256 reserve = fIdToAuction[_fId].reserve;
        require(total - _amount >= reserve, "cant withdraw below reserve");
        fIdToAuction[_fId].contributions[msg.sender] -= _amount;
        fIdToAuction[_fId].totalContributed -= _amount;
        payable(msg.sender).transfer(_amount);
        emit WithdrawContribution(msg.sender, _fId, _amount);
    }
    ...
    function claim(uint256 _fId) external nonReentrant afterEnded(_fId) {
        uint256 total = fIdToAuction[_fId].totalContributed;
        uint256 bal = fIdToAuction[_fId].contributions[msg.sender];
        uint256 reserve = fIdToAuction[_fId].reserve;
        uint256 amount = fIdToAuction[_fId].amount;
        fIdToAuction[_fId].contributions[msg.sender] = 0;
        if (total < reserve) {
            payable(msg.sender).transfer(bal);
        } else {
            uint256 share = (100 * amount * bal) / total;
            fnft.safeTransferFrom(address(this), msg.sender, _fId, share, "0");
        }
        emit WithdrawClaim(msg.sender, _fId, bal);
    }
    ...
    {
        fIdToAuction[_fId].reserve = _reserve;
        emit UpdateReserve(_fId, _reserve);
    }
    ...
    function close(uint256 _fId) external onlyCurator(_fId) afterEnded(_fId) {
        uint256 total = fIdToAuction[_fId].totalContributed;
        uint256 reserve = fIdToAuction[_fId].reserve;
        bool settled = fIdToAuction[_fId].settled;
