...
contract StakingContract is Ownable {
    ...
    function stake(uint256[] memory tokenIds, address[] memory _stakerAddress) external {
        uint16 stakeCounter = 0;
        for(uint256 i = 0; i < tokenIds.length; i++) {
            (bool _isStaked,,,) = getStakingItemInfo(msg.sender, tokenIds[i]);
            if(_isStaked) { revert tokenIsAlreadyStaked(); }
            if(IERC721(nftAddress).ownerOf(tokenIds[i]) != msg.sender) { revert stakeNotCalledByTokenOwner(); }
            
            IERC721(nftAddress).transferFrom(msg.sender, address(this), tokenIds[i]);
            UserInfo memory info;
            info.tokenId = tokenIds[i];
            info.startTime = block.timestamp;
            info.stakerAddress = _stakerAddress[i];
            info.lockTime = 52 weeks;
            userInfo[_stakerAddress[i]].push(info);
            stakeCounter++;
        }
    ...
    function unstake(uint256[] memory tokenIds) external {
        ...
        for(uint256 i = 0; i < tokenIds.length; i++) {
            (bool _isStaked, uint _startTime, uint _lockTime, address _stakerAddress) = getStakingItemInfo(msg.sender, tokenIds[i]);
            if(!_isStaked) { revert tokenIsAlreadyUnstaked(); }
            if(block.timestamp <= (_startTime + _lockTime)) { revert lockTimeNotReached(); }
            if(IERC721(nftAddress).ownerOf(tokenIds[i]) != address(this)) { revert stakingContractIsNotOwner(); }
            if(msg.sender != _stakerAddress) { revert senderIsNotStaker(); }
    ...
    function getStakingItemInfo(address _user, uint256 _tokenId) public view returns(bool _isStaked, uint _startTime, uint _lockTime, address _stakerAddress) {
        for(uint256 i = 0; i < userInfo[_user].length; i++) {
        UserInfo memory ui = userInfo[msg.sender][i];
            if(ui.tokenId == _tokenId) {
                _isStaked = true;
                _startTime = ui.startTime;
                _lockTime = ui.lockTime;
                _stakerAddress = ui.stakerAddress;
                break;
            }
        }
    }
    ...
    function removeFromUserInfo(uint256 tokenId) private {        
        for (uint256 i = 0; i < userInfo[msg.sender].length; i++) {
            if (userInfo[msg.sender][i].tokenId == tokenId) {
                userInfo[msg.sender][i] = userInfo[msg.sender][userInfo[msg.sender].length.sub(1)];
