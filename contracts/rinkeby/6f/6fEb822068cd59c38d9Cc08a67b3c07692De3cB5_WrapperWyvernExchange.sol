...
// File contracts/interfaces/IWyvernExchange.sol
...
// File contracts/interfaces/IRoyaltyEngine.sol
...
// File contracts/WrapperWyvernExchange.sol
...
contract WrapperWyvernExchange is Ownable, ReentrancyGuard {
    ...
    uint256 public fee; // platform fee in bps

    constructor(address _exchange, address _royaltyEngine, address _wallet, uint256 _fee) {
    ...
    function atomicMatch_(address[14] memory addrs, uint256[18] memory uints, uint8[8] memory feeMethodsSidesKindsHowToCalls, bytes memory calldataBuy, bytes memory calldataSell, bytes memory replacementPatternBuy, bytes memory replacementPatternSell, bytes memory staticExtradataBuy, bytes memory staticExtradataSell, uint8[2] memory vs, bytes32[5] memory rssMetadata) external payable {
            exchange.atomicMatch_{value: msg.value}(addrs, uints, feeMethodsSidesKindsHowToCalls, calldataBuy, calldataSell, replacementPatternBuy, replacementPatternSell, staticExtradataBuy, staticExtradataSell, vs, rssMetadata);
    }

    // Get royalty amounts from the registry
    function getRoyalty(address _tokenAddress, uint256 _tokenId, uint256 _value) public view returns (address[] memory recipients, uint256[] memory amounts) {
