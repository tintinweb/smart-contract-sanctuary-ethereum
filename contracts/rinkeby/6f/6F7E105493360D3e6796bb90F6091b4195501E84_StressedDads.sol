...
contract StressedDads is ERC721Enumerable, DataLayout, Proxiable, Ownable {
    using SafeMath for uint256;
    
    constructor() ERC721("Stressed Dads", "SD") {
    ...
    function NFTConstructor(address _manager) public payable {
        ...
        gateWay = "https://stressedads.mypinata.cloud/ipfs/";
        proxyConstructor("Stressed Dads", "SD");
        phasePrices[0] = 0.1 ether;
        phasePrices[1] = 0.2 ether;
        phasePrices[2] = 0.3 ether;
    ...
    function createNFT(uint amount, address claimer, bool ownerMint) public payable delegatedOnly {
        ...
        if (!ownerMint) {
            ...
            if (_tokenIds <= 1000) {
                phase = 0;
            } else if (_tokenIds > 1000 && _tokenIds <= 3000) {
                phase = 1;
            } else {
                phase = 2;
            }
            uint256 mintCost = phasePrices[phase]; 
