...
// File: contracts/limit_ownership.sol
...
contract EvgrussiaToken is ERC20, Ownable {
    uint256 public immutable TotalSupply = 10000 * 10**decimals();
    uint256 public immutable icoMaxSupply = 1000 * 10**decimals();
    uint256 public immutable ownerMaxPerc = 5;

    uint256 public presaleCounter = 0;
    uint256 public presaleCost1 = 0.005 ether; 
    uint256 public presaleCost2 = 0.01 ether; 
    ...
    function buyOnPresaleWhiteListed() public payable {
        ...
        require(newSupply <= TotalSupply, "Final supply reached!");

        presaleCounter += amount;
        require(
            presaleCounter <= icoMaxSupply,
    ...
    function _beforeTokenTransfer(
        ...
        if (to != owner()) {
            uint256 ownershipPercentage = ((balanceOf(to) + amount) * 100) /
                TotalSupply;
            require(
                ownershipPercentage <= ownerMaxPerc,
