...
//     _setDefaultRoyalty(missionLabsWallet, 5000);
//     totalSupply = 0;
//   }
...
//     uint256 cut = balance;
//     (bool cutSuccess, ) = missionLabsWallet.call{value: cut}("");
//     require(cutSuccess, "Could not process payment");
//     emit Withdrawal(missionLabsWallet, cut, block.timestamp);
// }
// }
...
contract FreedomAndExpression is ERC721, RaribleRoyaltyV2, Ownable, Pausable {
...
/*/////////////////////////////////////// */
  ...
  function _reserveMint(uint256 editionId, address to) public {
    require(editionId < 2, "You cannot reserve that edition");
    //require(editionSupply[editionId] < EDITION_SUPPLY, "That token is reserved");
    if(to == missionlabsWallet) {
    _safeMint(msg.sender, editionId);
    }
...
function withdraw() public {
    uint256 balance = address(this).balance;
    require(balance > 0, "No funds to withdraw");
    (bool cutSuccess, ) = missionlabsWallet.call{value: balance}("");
    require(cutSuccess, "Could not process payment");
    emit Withdrawal(missionlabsWallet, balance, block.timestamp);
}
}
