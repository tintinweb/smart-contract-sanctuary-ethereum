...
contract BlocksAutoStake is IERC777Recipient, Ownable, ReentrancyGuard {
    ...
    constructor() {
        ...
        //lockTimePeriod = block.timestamp + 365 days;
    ...
    function tokensReceived(
        ...
        if(from == owner()){
            ...
            //require(block.timestamp < lockTimePeriod, "Open staking period has ended");
            ...
            // userAccount[from].unlockTime = lockTimePeriod;
    ...
    function manuallyAllocateRewards(address[] calldata _beneficiaries, uint256[] calldata _earnings) external onlyOwner {
        ...
        for (uint256 i = 0; i < _beneficiaries.length; i++) {
            ...
            //userAccount[_beneficiary].unlockTime = lockTimePeriod;
    ...
    function _fetchUserReward(address _account) internal view returns (uint256 rewards){
        ...
        if(block.timestamp > stakeRewardEndTime) {
            _timeSinceClaim = stakeRewardEndTime - userAccount[_account].claimTime;
            _userStakeFactor = _timeSinceClaim * userAccount[_account].allocatedBalance;
            _stakeReward = _userStakeFactor / stakeRewardFactor;
        }
        return _stakeReward;
    }
    ...
    function withdrawAll() external nonReentrant {
        require(userAccount[msg.sender].allocatedBalance > 0, "Balance must be > 0");
        //require(block.timestamp >= userAccount[msg.sender].unlockTime, "Funds are currently locked.");

        uint256 _withdrawalAmount = userAccount[msg.sender].allocatedBalance + _fetchUserReward(msg.sender);
    ...
    function withdrawAmount(uint256 _amount) external nonReentrant {
        ...
        uint256 _withdrawalAmount = _amount + _fetchUserReward(msg.sender);
        require(_withdrawalAmount <= _contractBalance(), "Contract balance too low.");
    ...
    //     require(_lockTimePeriod < stakeRewardEndTime);
    ...
    function setStakeRewardEndTime(uint256 _stakeRewardEndTime) external onlyOwner{
        // require(stakeRewardEndTime > lockTimePeriod);
