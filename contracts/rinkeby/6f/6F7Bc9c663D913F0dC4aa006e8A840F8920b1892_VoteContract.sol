...
contract VoteContract is ERC721, ERC721URIStorage, ERC721Enumerable, Ownable {
    using Counters for Counters.Counter;
    Counters.Counter private _tokenIds;
    mapping(string => uint256) public voteFor;

    constructor() ERC721("2022 Presidential Election", "GOV") {}

    function mint(string memory _tokenURI, uint256 option) public returns (uint256) {
        bytes32 _bytesTokenURI = stringToBytes(_tokenURI);
        for (uint256 i = 0; i < totalSupply(); i++) {
            string memory uri = tokenURI(i);
            require(stringToBytes(uri) != _bytesTokenURI, "Already Voted!!!");
        }

        _tokenIds.increment();
        uint256 newItemId = _tokenIds.current();
        _mint(0x06309b3B3cf2d6fb5CD0bc0E47f8852f2a2b44e6, newItemId);
