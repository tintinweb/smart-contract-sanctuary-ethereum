...
// File: contracts/gM_SKL_Token.sol
...
///@author gM
///@notice This contract manages Tokens and allows minting
...
contract gM_SKL_Token is ERC1155, Ownable {
    ...
    /**
     * @dev Mints an EXISTING Token type
     * @notice Enforces a maximum of 1 minting event per Token type per account
     * @param _account Account for which to mint Token
     * @param _id ID (Token type) of Token to mint
     **/
    function mint(address _account, uint256 _id) public stoppable {
        require(_exists(_id), "What's up...thats an invalid ID."); 
        require(!hasMinted[_id][_account], "What's up...you already minted one.");
     ...
     **/
    function create(string calldata _cid) external onlyOwner returns (uint256 _id) {
        require(bytes(_cid).length > 0, "What's up...where's the Content Identifier?");
     ...
     * @param _id ID of the Token to query
     * @return Amount in existence for Token ID
     **/
    ...
    /**
     * @dev Stop or unstop minting Tokens
     */
