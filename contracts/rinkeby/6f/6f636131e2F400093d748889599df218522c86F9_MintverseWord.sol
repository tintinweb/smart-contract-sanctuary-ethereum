...
contract IMintverseWordStorage is IWord {
    ...
    bool public mintWhitelistEnable; // Set by #setWLMintPhase
    bool public mintPublicEnable; // Set by #setPublicMintPhase
    uint256 public mintWhitelistTimestamp; // Set by #setWLMintPhase
    uint256 public mintPublicTimestamp; // Set by #setPublicMintPhase
    uint48 public revealTimestamp; // Set by #setRevealTimestamp
    // Mint Record Variables
    uint16 public totalDictionary;
	uint16 public totalWordGiveaway;
    uint16 public totalWordWhitelist;
    uint16 public wordBankLength;
    // Mint Limitation Variables
    uint256 public MAX_MINTVERSE_RANDOM_WORD; // Set by #setMaxRandomWordTokenAmt
    uint256 public MAX_MINTVERSE_GIVEAWAY_WORD; // Set by #setMaxGiveawayWordTokenAmt
    uint256 public MAX_MINTVERSE_DICTIONARY; // Set by #setMaxDictAmt
    uint256 public DICT_ADDON_PRICE; // Set by #setDictPrice
    uint48 public WORD_EXPIRATION_TIME; // Set by #setExpirationTime
    uint16 public HEAD_RANDOM_WORDID; // Set by #setHeadRandomWordId
    uint16 public TAIL_RANDOM_WORDID; // Set by #setTailRandomWordId
    uint16 public SETTLE_HEAD_TOKENID; // Set by #setSettleHeadRandomWordId
    uint16 public DESIGNATED_WORDID_OFFSET; // Set by #setWordIdOffset
...
contract MintverseWord is IMintverseWord, IMintverseWordStorage, Ownable, EIP712, ERC721A {
    ...
    ERC721A("MintverseWord", "MVW")     
    {
        ...
        mintWhitelistTimestamp = 1651726800;
        mintPublicTimestamp = 1651813200;
        revealTimestamp = 1652072400;
        ...
        SETTLE_HEAD_TOKENID = uint16(TAIL_RANDOM_WORDID.sub(1));
    ...
    function verify(
        ...
        returns(bool)
    {
        address recoveredAddr = ECDSA.recover(_hashTypedDataV4(keccak256(abi.encode(keccak256("NFT(address addressForClaim,uint256 maxQuantity)"), _msgSender(), 1))), SIGNATURE);
     ...
     * @param wordId Amount of tokens to be delivered
     */
    ...
    /** @dev Calculate the token counts for random words
     */
    ...
    {
        return uint16(totalSupply() - totalWordGiveaway);
    }
        ...
        private
    {
        ...
        tokenItem[totalSupply().sub(1)] = TokenInfo(nullString, nullString, nullString, wordId, 1, 0, 0, mintTimestamp, false);
    }
     ...
     * @param tokenId Token Id that the token owner wants to define
     * @param definer 鑄造者
     ...
     * Getter Functions
     */
    /** @dev Retrieve word definition metadata by token Id
    ...
    function getTokenProperties(uint256 tokenId)
        ...
        returns (
            ...
            uint256 wordId,
            uint256 categoryId,
            uint8 partOfSpeechId1,
            uint8 partOfSpeechId2,
    ...
    /** @dev Retrieve defined status of the word tokens of the given address
     * @param owner Address which caller wants to get all its word token defined status
     */
     ...
     * @param tokenId Token Id which caller wants to get the metadata of
     */
    ...
    /** @dev Retrieve all tokens Id of a given address
     * @param owner Address which caller wants to get all of its token Id
     */
    ...
    /**
     */
    function setExpirationTime(uint48 expirationPeriod)
        override
        external
        onlyOwner
    {
        WORD_EXPIRATION_TIME = expirationPeriod;
    }
    ...
    /** @dev Set the URI for tokenURI, which returns the metadata of token.
     * @param newLegalDocumentURI New URI that caller wants to set as tokenURI
     */
    ...
    /** @dev Set the URI for tokenURI, which returns the metadata of token.
     * @param newSystemMechanismDocumentURI New URI that caller wants to set as tokenURI
     */
    ...
    /** @dev Set the URI for tokenURI, which returns the metadata of token.
     * @param newAnimationCodeDocumentURI New URI that caller wants to set as tokenURI
     */
    ...
    /** @dev Set the URI for tokenURI, which returns the metadata of token.
     * @param newERC721ATechinalDocumentURI New URI that caller wants to set as tokenURI
     */
    ...
    /** @dev Set the URI for tokenURI, which returns the metadata of token.
     * @param newWordIdMappingDocumnetURI New URI that caller wants to set as tokenURI
     */
    ...
    /** @dev Set the URI for tokenURI, which returns the metadata of token.
     * @param newPartOfSpeechIdMappingDocumentURI New URI that caller wants to set as tokenURI
     */
    ...
    /** @dev Set the URI for tokenURI, which returns the metadata of token.
     * @param newCategoryIdMappingDocumentURI New URI that caller wants to set as tokenURI
     */
    ...
    /** @dev Set the URI for tokenURI, which returns the metadata of token.
     * @param newMetadataMappingDocumentURI New URI that caller wants to set as tokenURI
     */
...
interface IWord {
    struct TokenInfo {
        ...
        uint16 wordPart;
        uint8 categoryPart; // 0: Genesis Card, 1: Special Card, 2. Censored Card
...
interface IMintverseWord is IWord {
    ...
    function verify(bytes calldata SIGNATURE) external view returns(bool);
    ...
    function mintWhitelistWord(bool addon, bytes calldata SIGNATURE) external payable;
    ...
    function getTokenProperties(uint256 tokenId) external view returns(string memory definer, uint256 wordId, uint256 categoryId, uint8 partOfSpeechId1, uint8 partOfSpeechId2, string memory relatedWord, string memory description);
