...
contract Auction {
    ...
    // Auction constructor
    constructor(address _creator,uint _endTime,uint _directBuyPrice, uint _startPrice,address _nftAddress,uint _tokenId){
    ...
    function withdrawToken() external returns(bool){
        ...
        _nft.transferFrom(creator, maxBidder, tokenId); // Transfer the token to the highest bidder
    ...
    function cancelAuction() external returns(bool){ // Cancel the auction
        ...
        _nft.setApprovalForAll(address(this), false); // Revoke Approve
...
contract Sell {
    ...
    // Auction constructor
    constructor(address _creator,uint _price,address _nftAddress,uint _tokenId){
        creator = _creator; // The address of the auction creator
        price = _price; // The starting price for the auction
        _nft = IERC721(_nftAddress); // The address of the nft token
        nftAddress = _nftAddress;
        tokenId = _tokenId; // The id of the token
    }
    ...
    // Place a bid on the auction
    function buyAsset() payable external returns(bool) {
        require(msg.sender != creator);
        require(getSellState() == SellState.OPEN);
    ...
    function withdrawToken() public returns(bool){
        require(getSellState() == SellState.ENDED); // The auction must be ended by either a direct buy or timeout
        require(msg.sender == buyer); // The highest bidder can only withdraw the token
        _nft.transferFrom(creator, buyer, tokenId); // Transfer the token to the highest bidder
        emit WithdrawToken(buyer); // Emit a withdraw token event
        return true;
    }
    ...
    function withdrawFunds() public returns(bool){ 
        require(getSellState() == SellState.ENDED); // The auction must be ended by either a direct buy or timeout
        require(msg.sender == creator); // The auction creator can only withdraw the funds
        payable(address(uint160(creator))).transfer(buyPrice); // Transfers funds to the creator
        emit WithdrawFunds(msg.sender,buyPrice); // Emit a withdraw funds event
        return true;
    } 

    function cancelSell() external returns(bool){ // Cancel the auction
        require(msg.sender == creator); // Only the auction creator can cancel the auction
        require(getSellState() == SellState.OPEN); // The auction must be open
        isCancelled = true; // The auction has been cancelled
        _nft.setApprovalForAll(address(this), false); // Revoke Approve
        emit SellCanceled(); // Emit Auction Canceled event
        return true;
    } 

    // Get the auction state
    function getSellState() public view returns(SellState) {
        if(isCancelled) return SellState.CANCELLED; // If the auction is cancelled return CANCELLED
        if(isSold) return SellState.ENDED;
        return SellState.OPEN; // Otherwise return OPEN
    } 

    event WithdrawToken(address withdrawer); // The auction winner withdrawed the token
    event WithdrawFunds(address withdrawer, uint256 amount); // The auction owner withdrawed the funds
    event SellCanceled(); // The auction was cancelled
...
contract OrderManager {
    ...
    function createSell(uint _price, address _nftAddress, uint _tokenId) external returns (bool) {
        require(_price > 0);

        uint orderId = _sellIdCounter;
        _sellIdCounter++;
        Sell sell = new Sell(msg.sender, _price, _nftAddress, _tokenId);
        IERC721 _nftToken = IERC721(_nftAddress);
        _nftToken.setApprovalForAll(address(sell), true);
        sells[orderId] = sell;

        return true;
    }

    function createAuction(uint _endTime, uint _directBuyPrice, uint _startPrice, address _nftAddress,uint _tokenId) external returns (bool){
        ...
        Auction auction = new Auction(msg.sender, _endTime, _directBuyPrice, _startPrice, _nftAddress, _tokenId); // create the auction
        IERC721 _nftToken = IERC721(_nftAddress); // get the nft token
        _nftToken.setApprovalForAll(address(auction), true); //approve auction contract to transfer the given tokenId 
    ...
    function getSells() external view returns(address[] memory _sells) {
        _sells = new address[](_sellIdCounter); // create an array of size equal to the current value of the counter
        for(uint i = 0; i < _sellIdCounter; i++) { // for each auction
            _sells[i] = address(sells[i]); // add the address of the auction to the array
        }
        return _sells; // return the array
    }

    // Return the information of each auction address
    function getSellInfo(address[] calldata _sellList)
        external
        view
        returns (
            ...
            uint256[] memory sellState
        )
    {
        owner = new address[](_sellList.length); // create an array of size equal to the length of the passed array
        nftAddress = new address[](_sellList.length);
        tokenIds = new uint256[](_sellList.length);
        price = new uint256[](_sellList.length);
        sellState = new uint256[](_sellList.length);


        for (uint256 i = 0; i < _sellList.length; i++) { // for each auction
            owner[i] = Sell(sells[i]).creator(); // get the owner of the auction
            nftAddress[i] = Sell(sells[i]).nftAddress(); //get the address of nft
            tokenIds[i] = Sell(sells[i]).tokenId(); // get the token id
            price[i] = Sell(sells[i]).price(); // get the start price
            sellState[i] = uint(Sell(sells[i]).getSellState()); // get the auction state
        }
        ...
        )
    {
        ...
        nftAddress = new address[](_auctionsList.length);
        ...
        for (uint256 i = 0; i < _auctionsList.length; i++) { // for each auction
            ...
            nftAddress[i] = Auction(auctions[i]).nftAddress(); //get the address of nft
