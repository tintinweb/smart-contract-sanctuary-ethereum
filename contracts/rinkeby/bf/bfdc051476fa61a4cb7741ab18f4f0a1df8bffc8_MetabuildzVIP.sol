...
contract MetabuildzVIP is ERC721A, ReentrancyGuard, Delegated {
    ...
    bytes32 public publicMintRoot;

    // ======== CONSTRUCTOR ========
    constructor() ERC721A("Metabuildz VIP PASS", "MBZ") {}
        ...
        callerIsUser
    {
        ...
        require(_numberMinted(msg.sender) < mintLimit, "Already minted");
        require(
            MerkleProof.verify(
                _proof,
                whitelistRoot,
    ...
    function publicMint(uint256 quantity)
        ...
        callerIsUser
    {
        require(currentMintBatch == 2, "Public mint is not active yet.");
        require(msg.value == publicSalePrice * quantity, "Incorrect ether sent");
        require(totalSupply() < MAX_SUPPLY, "Max supply reached");
        require(_numberMinted(msg.sender) < mintLimit, "Already minted");
    ...
    // ======== SETTERS ========
    function setCurrentMintBatch(uint8 _batch) external onlyDelegates {
        currentMintBatch = _batch;
    }

    function setWhitelistRoot(bytes32 _merkleRoot) external onlyDelegates {
        whitelistRoot = _merkleRoot;
    }

    function setPublicMintRoot(bytes32 _merkleRoot) external onlyDelegates {
        publicMintRoot = _merkleRoot;
    }

    function setMintLimit(uint256 _mintLimit) external onlyDelegates {
