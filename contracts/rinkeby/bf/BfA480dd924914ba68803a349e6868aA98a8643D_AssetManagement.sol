...
contract AssetManagement is AccessControl {
    ...
    event DepositForRepay(address sender, address token, uint256 value,string signature,uint256 nonce);
    event Widthdraw(address signer,address reciver, address token, uint256 value,string  action);
    event WidthdrawETH(address signer,address reciver, uint256 value,string  action);
    ...
    function depositForRepay(
        ...
        IERC20(token).transferFrom(msg.sender,address(this),amount);
        emit DepositForRepay(msg.sender,token,amount,signature,nonce);
    }

    function depositETHForRepay(string memory signature,uint256 nonce) external payable {
        require(msg.value > 0, 'DepositETHForRepay: amount  zero');
        IWETH(WETH).deposit{value: msg.value}();
        emit DepositForRepay(msg.sender,WETH,msg.value,signature,nonce);
    }
    ...
    function depositETHWithNFT(
        ...
        require(msg.value > 0, 'DepositETHWithNFT: amount can not be 0');
        require(!nonces[nonce], 'DepositETHWithNFT: nonce had used');
        bytes32 hash = keccak256(abi.encode(msg.sender,msg.value,nonce,tokenUri));
        address signer = ECDSA.recover(hash, v, r, s);
        require(signers[signer], 'DepositETHWithNFT: signature error');
        lastTokenId +=1;
        nonces[nonce] = true;
    ...
    function withdraw(Types.Sign memory sign,string memory action) public {
        require(hasRole(OPERATORROLE, msg.sender), "Caller is not a operator");
        _validateSign(sign);
        IERC20(sign.token).transfer(sign.toUser,sign.amount);
        emit Widthdraw(sign.signer,sign.toUser,sign.token,sign.amount,action);

    }

    function withdrawETH(Types.Sign memory sign,string memory action) public {
        require(hasRole(OPERATORROLE, msg.sender), "Caller is not a operator");
        _validateSign(sign);
        IWETH(WETH).withdraw(sign.amount);
        _safeTransferETH(sign.toUser, sign.amount);
        emit WidthdrawETH(sign.signer,sign.toUser,sign.amount,action);
    ...
    function userWalletBalance(address user) public view returns (address[] memory,uint256[] memory) {
        uint256[]  memory tokenBalance;
        for(uint256 i=0; i < contracts.length; i++){
			uint256 balance = IERC20(contracts[i]).balanceOf(user);
            tokenBalance[i] = balance;
		}
        return (contracts,tokenBalance);  
    }
