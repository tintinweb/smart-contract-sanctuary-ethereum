...
contract CavemanNFT is ERC721A, IERC2981, Ownable, ReentrancyGuard {
    ...
    //Monday, 28 February 2022 14:00:00 GMT+08:00
    uint public wlStartTime = 1646028000;
    uint public publicStartTime = 1646028000;

    address public royaltyAddress;
    uint256 public royaltyPercent;

    bytes32 public wlMerkleRoot = 0xe4f4bc6de8964df003e403daaadf4bd4b7cdbb8f3cbcac13ebfd5e301d728803;
    ...
    constructor(
    ) ERC721A("Caveman NFT", "CAVEMAN", 200, maxSupply) {
    ...
    function whitelistMint(uint256 _mintAmt, bytes32[] calldata _merkleProof)
        ...
        eoaOnly
    {
        require(_whitelistVerify(_merkleProof, wlMerkleRoot), "Invalid merkle proof");
        require(!paused, "Mint Paused");
        ...
        if (msg.sender != owner()) {
            require(msg.value >= wlCost * (_mintAmt - freeMint), "Value Not Match");
        }

        if (!wlMintInit[msg.sender]) {
            wlMintInit[msg.sender] = true;
            wlMint[msg.sender] = maxWLMint;
        }
        require(wlMint[msg.sender] - _mintAmt >= 0, "Address already minted num of tokens allowed");
        wlMint[msg.sender] = wlMint[msg.sender] - _mintAmt;

        _safeMint(msg.sender, _mintAmt);
        curWLTxn = curWLTxn++;
    }

    function giveAwayMint(uint _mintAmt) public onlyOwner {
        require(_mintAmt > 0, "Invalid Mint Amount");
        require(totalSupply() + _mintAmt <= maxSupply, "Not Enough NFT Left");
        _safeMint(msg.sender, _mintAmt);
    }
    ...
    function gift(address[] calldata receivers) public onlyOwner {
        require(totalSupply() + receivers.length <= maxSupply, "Not Enough NFT Left");
    ...
    function SacrificeRitual(uint256 tokenId)
        public
        virtual
    {
        require (ownerOf(tokenId) == msg.sender, "You do not own the NFT");
        cavemanSacrificed[msg.sender] = true;
        safeTransferFrom(msg.sender, 0x000000000000000000000000000000000000dEaD, tokenId);
    }

    function stakeCaveman(uint256 tokenId)
        public
        virtual
    {
        require (ownerOf(tokenId) == msg.sender, "You do not own the NFT");
    ...
    function unstakeCaveman(uint256 tokenId)
        public
        virtual
    {
        require (ownerOf(tokenId) == msg.sender, "You do not own the NFT");
    ...
    function _beforeTokenTransfers(
        address, address, uint256 startTokenId, uint256 quantity
    ) 
        internal
        view 
        override {
    ...
    function setwlStartTime(uint _wlStartTime) external onlyOwner {
        wlStartTime = _wlStartTime;
    }

    function setpublicStartTime(uint _publicStartTime) external onlyOwner {
    ...
    function serverNow() public view returns (uint currenttimeStamp) {
