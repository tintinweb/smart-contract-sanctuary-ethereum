...
contract ApelistController is ReentrancyGuard, Ownable, Pausable, PaymentSplitter {    
    ...
    function wlMint(uint256 quantity, bytes32[] memory proof) external payable whenNotPaused nonReentrant mintConditions(quantity) {
        require(whitelistLive, "Not live");
        require(whitelist.isPermitted(msg.sender, proof), "Not WL'd");
        require(limit[msg.sender].wl + quantity <= config.maxWLMint, "Exceeds max");        
        limit[msg.sender].wl += quantity;

        uint256 _quantity = quantity;
        if(claim.isPermitted(msg.sender, proof) && !claimed[msg.sender]) {
            _quantity += 1;
    ...
    function mint(uint256 quantity) external payable whenNotPaused nonReentrant mintConditions(quantity) {        
...
interface IApelist {
    function mint(address, uint256, uint256,  string calldata) external;
}
