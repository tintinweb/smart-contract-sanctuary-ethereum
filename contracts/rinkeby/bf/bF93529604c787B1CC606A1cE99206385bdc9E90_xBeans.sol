...
// https://github.com/sambacha/layerzero-contracts/tree/master/src/interfaces
    ...
    EIP712
{
    ...
    uint256 private _tTotal = 500000 * 10**9; // all burned
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    string private _name = "Bridged Beans";
    string private _symbol = "xBEANS";
    ...
    uint256 public _maxTxAmount = 1665 * 10**9; // 0.333%
    uint256 private numTokensSellToAddToLiquidity = 1665 * 10**9;
    uint256 public _maxWalletAmount = 5000 * 10**9; // 1%

    address private constant _UNISWAPV2_ROUTER_ADDR =
        0x7E2528476b14507f003aE9D123334977F5Ad7B14;

    address private constant DAI = 0x8AC76a51cc950d9822D68b83fE1Ad97B32Cd580d;
    ...
    constructor(address _stable_coin, address _layerZeroEndpoint)
        EIP712(_name, "1")
    {
        // layer zero
        endpoint = ILayerZeroEndpoint(_layerZeroEndpoint);

        _rOwned[burnAddress] = _rTotal;
        _tOwned[burnAddress] = _tTotal;
        ...
        emit Transfer(address(0), burnAddress, _tTotal);
    }
    ...
    function sendTokens(
        uint16 _chainId,
        bytes calldata _dstMultiChainTokenAddr,
        uint256 _qty
    ) public payable {
        ...
        require(
            ...
            "ERC20: send amount exceeds allowance"
        );
    ...
    function lzReceive(
        ...
        (address toAddr, uint256 qty) = abi.decode(
            _payload,
            (address, uint256)
        );
    ...
    function _burn(address sender, uint256 amount) private {
        ...
        if (_isExcluded[sender]) {
            _burnFromExcluded(sender, amount);
        } else {
            _burnStandard(sender, amount);
        }
        restoreAllFee();
    }

    function _burnStandard(address sender, uint256 tAmount) private {
        (
            uint256 rAmount,
            uint256 rTransferAmount,
            uint256 rFee,
    ...
    function _burnFromExcluded(address sender, uint256 tAmount) private {
        (
            uint256 rAmount,
            uint256 rTransferAmount,
            uint256 rFee,
    ...
    // this can only be called from lzReceive !
    function _mint(address recipient, uint256 amount) private {
        removeAllFee();
        if (_isExcluded[recipient]) {
            _mintToExcluded(recipient, amount);
        } else {
            _mintStandard(recipient, amount);
        }
        restoreAllFee();
    }

    function _mintStandard(address recipient, uint256 tAmount) private {
        ...
        _rOwned[recipient] = _rOwned[recipient].add(rTransferAmount);
        emit Transfer(address(0), recipient, tTransferAmount);
    }

    function _mintToExcluded(address recipient, uint256 tAmount) private {
        _tTotal = _tTotal.add(tAmount);
        (
            uint256 rAmount,
            uint256 rTransferAmount,
