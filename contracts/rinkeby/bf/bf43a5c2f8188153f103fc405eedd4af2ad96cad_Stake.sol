...
/// @title Hyperfy Land Staking Contract
...
contract Stake is ReentrancyGuard, Ownable {
    ...
    event lockUpdate(uint256 indexed tokenId, bool status);
    ...
    function lock(uint256 tokenId, bool status) public {
        require(land.ownerOf(tokenId) == address(this), "This token is not staked");
        require(msg.sender == operator, "Only the operator can lock tokens");
        isLocked[tokenId] = status;

        emit lockUpdate(tokenId, status);
    }

    event Deposit(address indexed sender, uint256 indexed tokenId, uint256 indexed startTime);

    /// @notice Transfers a land NFT from the user to the contract
    /// @dev assigns an owner and staking start time
    /// @param tokenId The ID of the land to transfer

    function deposit(uint256 tokenId) public {
        land.safeTransferFrom(msg.sender, address(this), tokenId);
        owners[tokenId] = msg.sender;
        owns[msg.sender].push(tokenId);
        stakeStart[tokenId] = block.timestamp;
        emit Deposit(msg.sender, tokenId, block.timestamp);
    }

    event withdrawal(uint256 indexed tokenId, address indexed sender);

    /// @notice Withdraws a land NFT from the contract
    /// @dev Checks to see if any rental agreements are still active & updates them with checkStatus()
    /// @dev Deletes any updated listingIds' listingInfo, listingSettings
    /// @dev Deletes the tokenIds' listingIds, owner, and stakeStart time
    /// @param tokenId The ID of the land to withdraw
    ...
    function withdraw(uint256 tokenId) public nonReentrant {
        ...
        delete stakeStart[tokenId];
        
        for(uint256 i = 0; i < owns[msg.sender].length; i++) {
            if(owns[msg.sender][i] == tokenId) {
                delete owns[msg.sender][i];
            }
        }
    }
    ...
    function getStakingTime(uint256 tokenId) public view returns(uint256) {
        if(stakeStart[tokenId] == 0) {
            return 0;
        } else {
            return block.timestamp - stakeStart[tokenId];
        }
    }
    ...
    function getOwned(address addr) public view returns(uint256[] memory) {
        return owns[addr];
    }
    ...
    function getLockStatus(uint256 tokenId) public view returns(bool) {
