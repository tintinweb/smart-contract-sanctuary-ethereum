...
contract BurnKing is ERC721Enumerable, Ownable {
   ...
   function concatParams(string memory _signer, uint256 _tokenCount, uint256 _usdCost, uint256 _timestamp) public pure returns(bytes memory){
       return abi.encodePacked(_signer,' ',Strings.toString(_tokenCount),' ',Strings.toString(_usdCost),' ',Strings.toString(_timestamp));
   }
   

   function isCorrectParams(string memory _signer, uint256 _tokenCount, uint256 _usdCost, uint256 _timestamp, bytes memory sig) public view returns(bool) {
        // This recreates the message that was signed on the client.
        bytes memory concatinatedParams = concatParams(_signer, _tokenCount, _usdCost, _timestamp);
        bytes32 message = prefixed(keccak256(concatinatedParams));

        return recoverSigner(message, sig) == publicKey;
    }
