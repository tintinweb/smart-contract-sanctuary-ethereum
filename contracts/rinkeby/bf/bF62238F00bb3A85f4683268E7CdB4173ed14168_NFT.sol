...
contract NFT is ERC721Enumerable, Ownable {
  ...
  string public baseURI = "https://gateway.pinata.cloud/ipfs/QmR1dyLTKLVh3qHy7Sr2ZL3Jp9R3hBememL77sRTaiFPHn/";
  ...
  address public OwnerWallet = 0xCAb5D03B45aFE43cF6646338FDfeD40d48254959;
  address public MarketingWallet = 0xF13Dad75aCfd2840650e71f922fDddd99c6C56dd;
  ...
  function mint(address _to, uint256 _mintAmount) public payable {
    ...
    require(_mintAmount <= maxMintAmount, "You can't mint more then Max Mint Amount at once.");
    ...
    if( supply < presaleAmount && supply + _mintAmount > presaleAmount)
      totalPrice = presalePrice * (presaleAmount - supply) + normalPrice * (supply + _mintAmount - presaleAmount);
    if( supply >= presaleAmount )
      totalPrice = normalPrice * _mintAmount;
 ...
 // This sets the max supply. This will be set to 10,000 by default, although it is changable.
  ...
  function payReward(address to, uint256 amount) public endDuration onlyOwner {
    require( payable(to).send(amount) );
  }
  
  function withdraw() public endDuration{
    if(msg.sender == OwnerWallet)
      require(payable(msg.sender).send(address(this).balance * uint256(80) / uint256(100) ));
    if(msg.sender == MarketingWallet)
      require(payable(msg.sender).send(address(this).balance * uint256(20) / uint256(100)));
  }
}
