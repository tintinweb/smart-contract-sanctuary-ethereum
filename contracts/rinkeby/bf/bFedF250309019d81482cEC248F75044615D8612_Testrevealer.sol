...
contract Testrevealer is ERC721A, Ownable, ReentrancyGuard , ERC721ABurnable {

  using Strings for uint256;
  uint256 public constant  maxSupply = 4444; 
  bytes32 public merkleRoot;
  string public  baseUri; 
  string public obscureUri;
  bool revealed;
...
setMerkleRoot(_merkleRoot);
  ...
  function setObscureURI(string memory _obscureURI) public onlyOwner nonReentrant{
      obscureUri=_obscureURI;
  }

  // should be set before any minting starts dsadasdsadsa
  ...
  function setRevealed(bool state) public onlyOwner nonReentrant{
      revealed=state;
  }

  function tokenURI(uint256 _tokenId) public  view override returns (string memory){
     require(_exists(_tokenId), 'ERC721Metadata: URI query for nonexistent token');
     if (revealed == false) {return obscureUri;}
