...
contract JobsRegistry is IJobsRegistry, Ownable {
    ...
    function addJob(
        ...
        _jobFromStorage.specification = _specification;
        jobsOf[_owner].add(_job);
        emit AddJob(_owner, _job, _specification);
    }
    ...
    function upgradeJob(
        ...
        _newJobFromStorage.specification = _newSpecification;
        ...
        emit UpgradeJob(_oldJobOwner, _oldJob, _newJob, _newSpecification);
    }
    ...
    function initializeWorkWithRequirements(
        ...
        if (job[msg.sender].owner == address(0)) revert Forbidden();
        ...
        if (
            _minimumWorksCompleted > 0 &&
            IBonder(bonder).worksCompleted(_worker) < _minimumWorksCompleted
        ) revert RequirementsNotMet();
        if (
            _minimumAge > 0 &&
    ...
    function finalizeWork(address _worker) external override {
        ...
        _registerWork(msg.sender, nativeToken, _worker, _workCost, _gasUsed);
    }
