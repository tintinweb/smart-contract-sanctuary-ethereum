    ...
    RoyaltiesV2Impl
{
    ...
    function setRoyalties(
        uint256 _tokenId,
        address[] memory _royaltiesReceipientAddress,
        uint96[] memory _percentageBasisPoints
    ) public {
        require(
            _royaltiesReceipientAddress.length == _percentageBasisPoints.length,
            "ElevateNFT: Invalid Input"
        );
        LibPart.Part[] memory _royalties = new LibPart.Part[](
            _royaltiesReceipientAddress.length
        );

        for (uint256 i = 0; i < _royaltiesReceipientAddress.length; i++) {
            _royalties[i].account = payable(_royaltiesReceipientAddress[i]);
