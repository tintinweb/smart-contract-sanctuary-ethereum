...
contract Dillom is ERC1155Supply, Ownable , ReentrancyGuard{
    ...
    bool public ownerClaimIsActive = false;
    ...
    function balanceOgRole() public view returns (uint256 balance){
        uint256 totalBalance = 0;
        for(uint256 i=0; i<ogItems.length;i++){
            totalBalance += balanceOf(msg.sender,i);
        }
        return totalBalance;
    }
    ...
    function flipWhitelistClaimState() public onlyOwner {
        whitelistClaimIsActive = !whitelistClaimIsActive;
    }

    function flipOwnerClaimState() public onlyOwner {
        ownerClaimIsActive = !ownerClaimIsActive;
    }
    ...
    function publicClaim(uint256 _quantity,uint256 _id) external {
        require(publicClaimIsActive, "Public claim is not active.");
    ...
    function whitelistClaim(uint256 _quantity,uint256 _id,bytes32[] calldata _merkleProof) external {
        bytes32 leaf = keccak256(abi.encodePacked(msg.sender));
        require(whitelistClaimIsActive, "Whitelist claim is not active.");
        require(idStats[_id].saleIdActive, "Sale not available for this item now.");
        require(
            totalSupply(_id) + _quantity <= idStats[_id].maxSupply,  
            "Minting limit reached."
    ...
    function ownerClaim(uint256 _quantity,uint256 _id) external {
        require(ownerClaimIsActive, "Owner claim is not active.");
        require(idStats[_id].saleIdActive, "Sale not available for this item now.");
        require(balanceOgRole() > 0 , "You are not an Og user");
        require(
            totalSupply(_id) + _quantity <= idStats[_id].maxSupply,  
            "Minting limit reached."
        );
