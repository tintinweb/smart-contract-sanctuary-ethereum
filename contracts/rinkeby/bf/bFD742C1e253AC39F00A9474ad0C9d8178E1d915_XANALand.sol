...
abstract contract Storage {
    ...
    mapping(string => bytes32) whitelistRoot;
    ...
    mapping(string => mapping(address => uint256)) public _userBought;
 ...
 */
contract ERC721A is ContextUpgradeable, ERC165, IERC721, IERC721Metadata {
...
contract XANALand is Initializable, UUPSUpgradeable, OwnableUpgradeable, IERC721, ERC721AQueryable, ReentrancyGuard, Storage {
 using Strings for uint256;
 using AddressUpgradeable for address;
    ...
    //Discounted Mint Function
     function mintDiscountCommon(uint256 _mintAmount, uint256 _size, bytes32[] calldata proof) checkSupply(_mintAmount,_size, "Common") external payable nonReentrant{
        string memory rarity = "Common";

         //Check user buy limit merkle tree
        uint256 userBuyCount = _userBought["discount"][msg.sender] + _mintAmount;
        require(isWhitelisted(msg.sender, proof,  userBuyCount, "discount"), "XanaLand: Not whitelisted");
        
        _userBought["discount"][msg.sender] += _mintAmount;
    ...
    function mintLand(string memory rarity,uint256 _mintAmount, uint256 _size, bytes32[] calldata proof, bool isLimit) checkSupply(_mintAmount,_size, rarity) external payable nonReentrant{
        require(rates[_size][rarity].valid, "XanaLand: Plot size not valid" );
        

        //Check user buy limit merkle tree
        uint256 userBuyCount = isLimit ?  _userBought[rarity][msg.sender] + _mintAmount : 0;
        if(status == 1) require(isWhitelisted(msg.sender, proof,  userBuyCount, rarity), "XanaLand: Not whitelisted");
        if(isLimit){
            _userBought[rarity][msg.sender] += _mintAmount;
        }
    ...
    function freeMint(uint256 _mintAmount,  bytes32[] calldata proof) checkSupply(_mintAmount, 1, "Common") external{
        ...
        uint256 userBuyCount =  _userBought["free"][msg.sender] + _mintAmount;
        require(isWhitelisted(msg.sender, proof,  userBuyCount, "free"), "XanaLand: Not whitelisted");
        _userBought["free"][msg.sender] += _mintAmount;
    ...
    function isWhitelisted(address account, bytes32[] calldata proof, uint256 quantity, string memory typeWhitelist) public view returns (bool) {
    ...
    //only owner methods
    //set whitelist
    function setWhitelistRoot(bytes32 newWhitelistroot, string memory typeWhitelist) public onlyOwner {
    ...
    function transferFrom(
        ...
        address to,
        uint256 tokenId
    ) checkLockUp(tokenId) public virtual override(ERC721A, IERC721) {
        _transfer(from, to, tokenId);
    }
