...
contract Sale is Initializable, OwnableUpgradeable, UUPSUpgradeable, ReentrancyGuardUpgradeable, Whitelisted {
    ...
    function initialize(
        ...
        _royaltyAddresses = [
            0x1060705c1E8C9e1b70a82d94c23D971149e0fceb, // Wallet 1 address
            0x41881dcbE012b56E7cF3b748DE05395DA9E78174, // Wallet 2 address
    ...
    function getReservedTokenIds(address _user) public view returns(uint256[] memory) {
        return _reservedTokenIds[_user];
    }
    ...
    */
    ...
    returns(uint256[] memory) {
        ...
        require(presaleActive, "Presale not started yet!");
        require(_whitelistBuys[user] + numberOfTokens <= maxPresaleMintsPerWallet, string(abi.encodePacked("You can't mint more than ", maxPresaleMintsPerWallet.toString(), " token(s) on presale per wallet!")));
        require(mintIndex + numberOfTokens + teamReserved + givewaysReserved <= maxTokenSupply, "Not enough tokens left!");
        require(msg.value >= mintPrice * numberOfTokens, string(abi.encodePacked("Not enough ETH! At least ", (mintPrice * numberOfTokens).toString(), " wei has to be sent!")));
    ...
    */
    ...
    returns(uint256[] memory) {
        ...
        require(publicSaleActive, "Sale not started yet!");
        require(numberOfTokens <= maxMintsPerTxn, string(abi.encodePacked("You can buy between 1 and ",  maxMintsPerTxn.toString(), " tokens per transaction.")));
        require(mintIndex + numberOfTokens + teamReserved + givewaysReserved <= maxTokenSupply, "Not enough tokens left!");
        require(msg.value >= mintPrice * numberOfTokens, string(abi.encodePacked("Not enough ETH! At least ", (mintPrice * numberOfTokens).toString(), " wei has to be sent!")));
    ...
    function mint(uint256[] memory _tiers, uint256[] memory _rates) 
    external 
    nonReentrant() {
        address user = msg.sender;
        uint256[] memory tokenIds = _reservedTokenIds[user];
        require(tokenIds.length != 0, "None reserved token for this address");

        for (uint256 i = 0; i < tokenIds.length; i++) {
            spaceCows.mint(user, tokenIds[i], _tiers[i], _rates[i]);
    ...
    function freeToken(address _user, uint256 _amount) external onlyOwner {
        uint256 mintIndex = _tokenIdCounter.current();
        require(mintIndex + _amount <= maxTokenSupply, "Not enough tokens left!");
    ...
    function freeMint(address _user, uint256[] memory _tiers, uint256[] memory _rates) external onlyOwner {
        uint256[] memory tokenIds = _reservedTokenIds[_user];
        require(tokenIds.length != 0, "None reserved token for this address");

        for (uint256 i = 0; i < tokenIds.length; i++) {
            spaceCows.mint(_user, tokenIds[i], _tiers[i], _rates[i]);
...
abstract contract Whitelisted {
    ...
    modifier onlyWhitelisted(address _user, address _contract, bytes32[] calldata merkleProof) {
        ...
        require(MerkleProofUpgradeable.verify(merkleProof, _whitelistRoot, node), "You are not whitelisted to participate on presale!");
        _;
    }
