...
// File: github.com/OpenZeppelin/openzeppelin-contracts/blob/master/contracts/utils/StorageSlot.sol
...
// File: github.com/OpenZeppelin/openzeppelin-contracts/blob/master/contracts/interfaces/draft-IERC1822.sol
...
// can be brought down to 112 or 108
    ...
    function initialize1(
        ...
        uint256 _maxAllocationPermitted,//in wei
     ...
     initializer          
    {
        ...
        minAllocationPermitted = _maxAllocPerUserPerTier[_maxAllocPerUserPerTier.length -1]/2;
        maxAllocationPermitted = _maxAllocationPermitted;
        TGE = type(uint).max;
...
// File: github.com/OpenZeppelin/openzeppelin-contracts/blob/master/contracts/proxy/beacon/IBeacon.sol
...
// File: github.com/OpenZeppelin/openzeppelin-contracts/blob/master/contracts/proxy/ERC1967/ERC1967Upgrade.sol
...
// File: github.com/OpenZeppelin/openzeppelin-contracts/blob/master/contracts/proxy/beacon/BeaconProxy.sol
...
// File: github.com/OpenZeppelin/openzeppelin-contracts/blob/master/contracts/proxy/beacon/UpgradeableBeacon.sol
...
// File: contracts/IDO_NEW/GenIDOFactory.sol
...
contract GenIDOFactory is UpgradeableBeacon{
    mapping(uint=>address) public Contracts;
    bytes4 private constant FUNC_SELECTOR = bytes4(keccak256("initialize1(address,uint,address,uint[],uint[],uint[],uint[],uint,uint,uint)"));
    uint public id;

    constructor(address _ido) UpgradeableBeacon(_ido){

    }

    

    function createGenIdo(address _underlyingToken,
        uint256 _totalTokenAllocation,//enter details as per decimals of _underlyingToken
        address _usdt,
        uint256[] memory _maxAllocPerUserPerTier,//enter details as per decimals of _usdt
        uint256[] memory _numApplicantsPerTier,
        uint[] memory _trancheWeightage,//in wei
        uint[] memory _trancheLength,//in seconds
        uint256 _maxAllocationPermitted,
        uint256 _guranteedSaleDuration,
        uint256 _tokenPerUsd
        ) external onlyOwner {
        BeaconProxy bp= new BeaconProxy(address(this),abi.encodeWithSelector(GenIDO(address(0)).initialize1.selector,_underlyingToken,_totalTokenAllocation,_usdt,_maxAllocPerUserPerTier,_numApplicantsPerTier,_trancheWeightage,_trancheLength,_maxAllocationPermitted,_guranteedSaleDuration,_tokenPerUsd));
        Contracts[id++]=address(bp);
    }
