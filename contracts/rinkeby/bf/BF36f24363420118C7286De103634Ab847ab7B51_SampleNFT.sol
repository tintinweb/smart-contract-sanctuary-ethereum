...
// TODO - check FWB balance [x]
// TODO - 0xSplit [x]
// TODO - 2981 Royalty stuff [x]
// TODO - number per address? [x]
...
error NewTotalSupplySameAsOldTotalSupply();
...
error AddressExceedsMintLimit();

contract SampleNFT is ERC721Royalty, EIP712, Ownable {
  string public tokenBaseURI;
  string public tokenURISuffix = '';

  // TODO implement these
  bool public isSaleLocked;
  bool public isSaleOpenToFWBHolders;
  bool public isSaleOpenToAllowlist;
  bool public isSaleOpenToPublic;
  ...
  uint256 public minimumFWBRequired;
  ...
  constructor(
    ...
    uint256 _minimumFWBRequired,
    address _splitAddress,
    uint96 _royaltyFraction
  ) ERC721(name, symbol) EIP712(name, '1.0.0') {
    tokenBaseURI = _tokenBaseURI;
    isSaleLocked = _isSaleLocked;
    totalSupply = _totalSupply;
    pricePerToken = _pricePerToken;
    minimumFWBRequired = _minimumFWBRequired;
  ...
  function setMinimumFWBRequired(uint256 newMinimumFWBRequired)
    public
    onlyOwner
  {
    minimumFWBRequired = newMinimumFWBRequired * (1 ether);
  }
  ...
  function fwbBalanceOfAddress(address account) public view returns (uint256) {
    ...
    if (success == false) revert FWBBalanceOfFailed();
  ...
  modifier isSaleValid() {
    if (msg.sender != owner() && isSaleLocked == true) revert SaleIsLocked();

    if (fwbBalanceOfAddress(msg.sender) < minimumFWBRequired * (1 ether))
      revert FWBBalanceOfFailed();

    if (tokensLeft() == 0) revert AllTokensSold();

    if (msg.value < pricePerToken && msg.sender != owner())
      revert AmountLessThanSetPrice();

    if (msg.value > pricePerToken && msg.sender != owner())
      revert AmountMoreThanSetPrice();

    if (msg.sender != owner() && balanceOf(msg.sender) == mintLimit)
      revert AddressExceedsMintLimit();
    ...
    returns (bytes32)
  {
    return
      _hashTypedDataV4(
        keccak256(
          abi.encode(
            keccak256('NFT(uint256 Token,address Buyer)'),
            newTokenId,
            account
          )
        )
  ...
  function buy(bytes memory signature) public payable isSaleValid {
    // TODO DO WE NEED THIS?
    if (
      matchAddressSigner(msg.sender, numberOfMintedTokens + 1, signature) ==
      false
    ) revert DirectMintDisallowedSignatureMismatch();

    emit Purchase(msg.sender, msg.value, numberOfMintedTokens + 1);
    _mintPiece(msg.sender);
  }

  function gift(address receiver) public payable onlyOwner isSaleValid {
    emit Gift(receiver, numberOfMintedTokens + 1);
    _mintPiece(receiver);
  }
