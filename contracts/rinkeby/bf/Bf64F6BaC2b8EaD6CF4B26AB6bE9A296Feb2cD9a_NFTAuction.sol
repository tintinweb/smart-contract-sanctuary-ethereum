...
contract NFTAuction is Context, Ownable, ReentrancyGuard {
    ...
    constructor() {
        ...
        auctionFeePercentage = [250];
        auctionFeeRecipient = [address(0x00B91B2F8aFE87FCDc2b3fFA9ee2278cd1E4DDf8)];
    }
    ...
    /**
     * @notice Set up new auction's fee
     * @param _feePercentages Auction's fee 
     */
    function _setFeePercentage(uint16[] memory _feePercentages) private {
        auctionFeePercentage = _feePercentages;
    }

    /**
     * @notice Set up new aution's address
     * @param _feeRecipients Auction's address
     */
    function _setFeeRecipient(address[] memory _feeRecipients) private {
        auctionFeeRecipient = _feeRecipients;
    }

    /**
     * @notice Set up market's fee for specific addresses
     * @param _feeRecipients addresses for recieving fees after auctioning
     * @param _feePercentages fee percentages for specific addresses
     */
    function setAuctionFee(address[] memory _feeRecipients, uint16[] memory _feePercentages) public onlyOwner {
        require(_feeRecipients.length == _feePercentages.length, "invalid input values");
        _setFeeRecipient(_feeRecipients);
        _setFeePercentage(_feePercentages);
    }
}
