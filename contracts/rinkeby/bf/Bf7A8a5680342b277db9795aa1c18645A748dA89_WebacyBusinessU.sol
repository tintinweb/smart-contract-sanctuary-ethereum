...
contract WebacyBusinessU is Initializable, AccessControlUpgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable  {
    ...
    modifier _checkERC20orERC721(address _address){
        require((ERC165CheckerUpgradeable.supportsInterface(_address,type(IERC721Upgradeable).interfaceId)  || ERC165CheckerUpgradeable.supportsInterface(_address,type(IERC20Upgradeable).interfaceId)),"Does not support a Supported Interface");
        _;
    }
    ...
    ) external whenNotPaused _checkERC20orERC721(msg.sender) hasPaidMembership(msg.sender) {
