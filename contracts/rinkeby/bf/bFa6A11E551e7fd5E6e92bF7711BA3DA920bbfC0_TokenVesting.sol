...
contract TokenVesting is Ownable, Pausable{
    ...
    uint private constant GENESIS_TIMESTAMP = 1514764800;                       // Jan 1, 2018 00:00:00 UTC (arbitrary date/time for timestamp validatio
    ...
    */
    constructor(
        address payable _tokenContract
    ) {
    ...
    */   
    ...
    function grant(
        ...
        require(!vestingGrants[beneficiary].isGranted, "Tokens already granted");         // Can't have multiple grants for same account
        require((dreams > 0), "Tokens must be greater than zero");    // There must be dreams that are being granted
        ...
        require(cliffSeconds < vestingSeconds, "Cliff must be smaller than vestingSeconds");
    ...
    */ 
                            ...
                            returns(uint256) 
    {
        ...
        return(vestingGrants[account].grantDreams -  (vestingGrants[account].releasedDreams));
    }
    ...
    */ 
    function releaseFor(address account) public {
        require(account != address(0), "Account must be a valid address");
        require(vestingGrants[account].isGranted, "Tokens must be granted");
        require(vestingGrants[account].cliffTimestamp <=block.timestamp, "Cannot release tokens before cliff period");

        // Calculate vesting rate per second    
        uint256 vestingMonths = (vestingGrants[account].endTimestamp -  (vestingGrants[account].startTimestamp)) / month; 
        uint256 dreamsPerMonth = vestingGrants[account].grantDreams /  vestingMonths;
        
        // Calculate how many dreams can be released
        uint monthsPassed = (block.timestamp -  vestingGrants[account].startTimestamp) / month;
        
        uint256 releasableDreams = monthsPassed *  (dreamsPerMonth) -  (vestingGrants[account].releasedDreams);
        ...
        if ((vestingGrants[account].releasedDreams + (releasableDreams)) > vestingGrants[account].grantDreams) {
            releasableDreams = vestingGrants[account].grantDreams -  (vestingGrants[account].releasedDreams);
        }
        
        if (releasableDreams > 0) {
            // Update the released dreams counter
            vestingGrants[account].releasedDreams = vestingGrants[account].releasedDreams + (releasableDreams);
            tokenContract.transferFrom(address(this), account, releasableDreams);
        }
    }
    ...
    */ 
                            ...
                            whenNotPaused 
    {
        ...
        uint256 balanceDreams = vestingGrants[account].grantDreams -  (vestingGrants[account].releasedDreams);
        emit Revoke(vestingGrants[account].issuer, account, balanceDreams);

        // If there is any balance left, return it to the issuer
        if (balanceDreams > 0) {
            tokenContract.transferFrom(address(this), msg.sender, balanceDreams);
        }
    }
