...
contract AvoFriends is ERC721A, Ownable {
    ...
    uint256 public PRESALE_PRICE = 0.007 ether;
    ...
    function presaleMint(uint256 quantity) external payable {
        ...
        require(
            quantity <= 5 - _presaleWallets[msg.sender],
    ...
    function mintApe(uint256 quantity) public payable {
        ...
        if (msg.sender != owner()) {
            require(
                msg.value >= PUBLIC_PRICE * quantity,
                "insufficient ether value"
            );
        }
    ...
    function setNotRevealedTokenURI(string memory notRevealedTokenURI)
        external
        onlyOwner
    {
        _notRevealedTokenURI = notRevealedTokenURI;
    }
    ...
    function whitelistAddresses(address[] calldata addresses)
        external
        onlyOwner
    {
        delete _whitelistAddresses;
