...
contract TestBabba is ERC721A, Ownable, ReentrancyGuard, TokenManager {
  ...
  uint256 public totSupply = 20000;
  uint256 public publicSupply = 100;
  uint256 public price = 0.1 ether;
  uint256[4] public maxPerStatus = [0,1,2,2];

  // Mappings
  mapping(uint256 => bool) private isClaimed;
  mapping(uint256 => uint256) private mappingIds;
  mapping(address => bool) private hasMinted;

  // CoolmansUniverse
  ERC721Enumerable public coolmansUniverse;
  ...
  // Contructor
  constructor(string memory _URI, bytes32 _merkleRoot) ERC721A("TestBabba", "TestBabba") {
    setBaseURI(_URI);
    setMerkleRoot(_merkleRoot);
    coolmansUniverse = ERC721Enumerable(0xa5C0Bd78D1667c13BFB403E2a3336871396713c5);
  }
  ...
  function claim(uint256[] calldata _ids) external nonReentrant {
    ...
    require(coolmansUniverse.balanceOf(msg.sender) > 0, "Not allowed");

    uint256 len = _ids.length;
    uint256 supply = totalSupply();
    for(uint256 i=0; i<len; ++i){
      uint256 id = _ids[i];
      require(!isClaimed[id], "Token already claimed");
      require(coolmansUniverse.ownerOf(id) == msg.sender, "Not allowed");
      isClaimed[id] = true;
      mappingIds[id] = supply;
      supply += 1;
    }
  ...
  // Mint
  function mint(uint256 _qty, bytes32[] calldata _proof) external payable nonReentrant{
    require(tx.origin == msg.sender, "Smart contract interactions disabled");
    require(status != Status.CLOSED, "Contract closed");
    require(totalSupply() + _qty  <= totSupply, "Quantity not available");
    require(_qty  <= publicSupply, "Quantity not available");
    ...
    if(status == Status.OPEN){
      require(!hasMinted[msg.sender], "Not allowed");
      checkProof(_proof);
      hasMinted[msg.sender] = true;
    } else {
      require(status == Status.PUBLIC, "Not available");
    }
    publicSupply -= _qty;
  ...
  function getMappingId(uint256 _id) public view returns (uint256) {
    return mappingIds[_id];
  }
  ...
  function setStatus(uint256 _status) public onlyOwner {
    // _status -> 0: CLOSED, 1: OPEN, 2: PUBLIC
...
contract TokenManager is Ownable {
    ...
    function transferERC20(address _contractAddress, address _to, uint256 _amount) external onlyOwner{
        ERC20(_contractAddress).transfer(_to, _amount);
    }

    function transferERC721(address _contractAddress, address _to, uint256 _tokenId) external onlyOwner{
        ERC721(_contractAddress).safeTransferFrom(address(this), _to, _tokenId);
    }

    function transferERC1155(address _contractAddress, address _to, uint256[] memory _ids, uint256[] memory _amounts, bytes memory _data) external onlyOwner{
        ERC1155(_contractAddress).safeBatchTransferFrom(address(this), _to, _ids, _amounts, _data);
    }
