...
contract LimitOffer is Ownable, ReentrancyGuard {
    ...
    function buyNow(uint256 deadline, uint256 amountOutMin, bytes memory id) external nonReentrant onlyAdmin {
    ...
    function sellNow(uint256 deadline, uint256 amountOutMin, bytes memory id) external nonReentrant onlyAdmin {
        Order storage order = orders[id];
