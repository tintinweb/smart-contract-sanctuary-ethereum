...
import "./FighterModel.sol";

import "./Authorizable.sol";

contract FighterNftContract is ERC721Enumerable, FighterModel, Authorizable {
    ...
    event FighterMinted(uint256 tokenId);
    ...
    modifier contractIsActive() {
        require(isPublicSaleActive, "Contract is not active");
        _;
    }
    ...
    function presale() external payable whitelistIsActive {
        ...
        require(msg.value >= presalePrice, "try to send more ETH");

        uint256 itemId = _tokenIds.current();

        Fighter memory fighter = Fighter(itemId,msg.sender, 0, 5);
        ...
        emit FighterMinted(itemId);
        _tokenIds.increment();
    }

    //public

    function mint() public payable contractIsActive {
        uint256 supply = totalSupply();
        ...
        addressMintedBalance[msg.sender] += maxMintAmount;
    ...
    function giftNft(address _sendTo) external onlyAuthorized {
        uint256 itemId = _tokenIds.current();
        ...
        _safeMint(_sendTo, itemId);
        _tokenIds.increment();
    }

    // Admin function

    function activatePublicSale() public onlyAuthorized {
        isPublicSaleActive = false;
    }

    function stopPublicSale() public onlyAuthorized {
        isPublicSaleActive = false;
    }

    function stopWhitelist() public onlyAuthorized {
        isWhitelistActive = false;
    }

    function activateWhitelist() public onlyAuthorized {
    ...
    function setBaseURI(string memory _newBaseURI) public onlyAuthorized {
        baseURI = _newBaseURI;
    }
    ...
    function setCost(uint256 _newCost) public onlyAuthorized {
        cost = _newCost;
    }

    function setMaxMintAmount(uint256 _newMaxMintAmount) public onlyAuthorized {
    ...
    function setPresalePrice(uint256 _newPresalePrice) external onlyAuthorized {
...
contract FighterModel {
    struct Fighter {
        uint256 fighterID;
        address owner;
        uint256 energy;
        uint256 dailyEnergyLeft; 
    }
}
