    ...
    ReentrancyGuardUpgradeable
{
    ...
    mapping(address => bool)    public userTimestampInit; //userTimestampInit;
    ...
    modifier updatePassiveRewardsForUser() {
        ...
        _adjustMultiplierForPassiveYeild(userLedger.length);
        ...
        _adjustLegendaryMultiplierForPassiveYeild(userLedger);
        delete userLedger;
     ...
     */
    function _adjustLegendaryMultiplierForPassiveYeild(uint16[] memory tokenIds)
        internal
    {
        legendariesUnstaked[msg.sender] = 0;
        for (uint256 i = 0; i < tokenIds.length; i++) {
            if (tokenIds[i] > 5555 || isLegendary(tokenIds[i]))
                legendariesUnstaked[msg.sender]++;
        }
    }

    function _adjustMultiplierForPassiveYeild(uint256 holdings) internal {
        if (holdings < 5)
            userHoldingsMultiplier[msg.sender] = DAILY_WHALES_BASE_RATE;
        else {
            if (holdings >= 5 && holdings <= 9)
                userHoldingsMultiplier[msg.sender] = DAILY_WHALES_RATE_TIER_1;
            else if (holdings >= 10 && holdings <= 19)
                userHoldingsMultiplier[msg.sender] = DAILY_WHALES_RATE_TIER_2;
            else if (holdings >= 20 && holdings <= 39)
                userHoldingsMultiplier[msg.sender] = DAILY_WHALES_RATE_TIER_3;
            else if (holdings >= 40 && holdings <= 79)
                userHoldingsMultiplier[msg.sender] = DAILY_WHALES_RATE_TIER_4;
            else userHoldingsMultiplier[msg.sender] = DAILY_WHALES_RATE_TIER_5;
        }
    }
