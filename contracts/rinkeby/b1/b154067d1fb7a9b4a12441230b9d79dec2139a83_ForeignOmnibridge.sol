 ...
 */
abstract contract TokensRelayer is BasicAMBMediator, ReentrancyGuard, BridgedTokensRegistry, TokensBridgeLimits {
    using SafeMath for uint256;
    ...
    function _relayTokens(
        ...
        if(!isTokenRegistered(address(token))) {
    ...
    TokenFactoryConnector
{
    ...
    bytes32 private immutable PREFIX;
    uint256 private immutable PREFIX_SIZE;
    ...
    constructor(string memory _suffix, string memory _prefix) {
        require(bytes(_suffix).length <= 32);
        require(bytes(_prefix).length <= 32);
        bytes32 suffix;
        bytes32 prefix;
        SUFFIX_SIZE = bytes(_suffix).length;
        PREFIX_SIZE = bytes(_prefix).length;

        assembly {
            suffix := mload(add(_suffix, 32))
            prefix := mload(add(_prefix, 32))
        }
        SUFFIX = suffix;
        PREFIX = prefix;
    }
    ...
    function _getBridgedTokenOrDeploy(
        ...
        if (bridgedToken == address(0)) {
            ...
            symbol = _transformSymbolPrefix(symbol);
    ...
    /**
     * @dev Internal function for transforming the bridged token symbol. Appends a side-specific prefix.
     * @param _symbol bridged token from the other side.
     * @return token symbol for this side of the bridge.
     */
    function _transformSymbolPrefix(string memory _symbol) internal view returns (string memory) {
        require(bytes(_symbol).length <= 32);
        bytes32 symbol;
        assembly {
            symbol := mload(add(_symbol, 32))
        }
        uint256 symbol_size = bytes(_symbol).length;
        uint256 prefix_size = PREFIX_SIZE;

        bytes32 prefix = PREFIX;
        string memory prefix_parsed = new string(prefix_size);

        assembly {
            mstore(add(prefix_parsed, 32), prefix)
        }
        
        string memory result = string(abi.encodePacked(prefix_parsed, symbol));

        assembly {
            mstore(result, add(mload(prefix_parsed), symbol_size))
        }
        return result;
    }
...
contract ForeignOmnibridge is BasicOmnibridge, GasLimitManager, InterestConnector {
    ...
    constructor(string memory _suffix, string memory _symbol_Prefix) BasicOmnibridge(_suffix, _symbol_Prefix) {}
