...
contract TransferProxy {
    ...
    function erc721mintAndTransfer(IERC721 token, address from, address to, address[] memory _royaltyAddress, uint256[] memory _royaltyfee, string memory tokenURI, bytes calldata data) external returns(uint256) {
        return token.mintAndTransfer(from, to, _royaltyAddress, _royaltyfee, tokenURI, data);
    }

    function erc1155mintAndTransfer(IERC1155 token, address from, address to, address[] memory _royaltyAddress, uint256[] memory _royaltyfee, uint256 supply, string memory tokenURI, uint256 qty, bytes calldata data) external returns(uint256){
        return token.mintAndTransfer(from, to, _royaltyAddress, _royaltyfee, supply, tokenURI, qty, data);
    }
}
...
contract Trade {
    ...
    function getFees(Order memory order, bool isImport) public view returns(Fee memory){
        uint platformFee;
        uint fee;
        ...
        require(order.amount > price, "SafeMath: subtraction overflow");
        ...
        require(price > fee + sellerFee, "SafeMath: subtraction overflow");
        assetFee = price - (fee + sellerFee);
        return Fee(platformFee, assetFee, royaltyAddress, royaltyFee, price);
    }

    function tradeAsset(Order memory order, Fee memory fee, address buyer, address seller) internal virtual returns(uint256) {
        uint256 itemId = 0;
        ...
        if(order.nftType == BuyingAssetType.LazyMintERC721){
            itemId = transferProxy.erc721mintAndTransfer(IERC721(order.nftAddress), order.seller, order.buyer, order.royaltyAddress, order.royaltyfee, order.tokenURI,"" );
        }
        if(order.nftType == BuyingAssetType.LazyMintERC1155){
            itemId = transferProxy.erc1155mintAndTransfer(IERC1155(order.nftAddress), order.seller, order.buyer, order.royaltyAddress, order.royaltyfee, order.supply, order.tokenURI, order.qty, "");
        }
    ...
    function mintAndBuyAsset(Order memory order, Sign calldata ownerSign, Sign calldata sign) external returns(bool){
        ...
        order.tokenId = tradeAsset(order, fee, buyer, order.seller);
        emit BuyAsset(order.seller , order.tokenId, order.qty, msg.sender);
        return true;

    }

    function mintAndExecuteBid(Order memory order, Sign calldata ownerSign, Sign calldata sign) external returns(bool){
        require(!usedNonce[sign.nonce],"Nonce : Invalid Nonce");
        require(!usedNonce[ownerSign.nonce],"Nonce : ownerSign Invalid Nonce");
        usedNonce[sign.nonce] = true;
        usedNonce[ownerSign.nonce] = true;
        ...
        order.tokenId = tradeAsset(order, fee, order.buyer, seller);
        emit ExecuteBid(order.seller , order.tokenId, order.qty, msg.sender);
        return true;

    }

    function buyAsset(Order calldata order, bool isImport, Sign calldata sign) external returns(Fee memory) {
        require(!usedNonce[sign.nonce],"Nonce : Invalid Nonce");
        usedNonce[sign.nonce] = true;
