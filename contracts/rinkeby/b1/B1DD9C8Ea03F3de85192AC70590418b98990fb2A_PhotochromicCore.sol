...
contract PhotochromicCore {
    // The period in which the ticket is valid.
    uint256 private ticketValidity = 4 weeks;

    function setTicketValidity(uint256 newTicketValidity) public onlyAdmin {
        ticketValidity = newTicketValidity;
    }

    function getTicketValidity() public view returns (uint256) {
        return ticketValidity;
    }

    // The grace period in which a user id can not be re-registered after its expiry.
    uint256 private gracePeriod = 12 weeks;

    function setGracePeriod(uint256 newGracePeriod) public onlyAdmin {
    ...
    PhotochromicRegistrar private registrar;

    function getRegistrar() public view returns (PhotochromicRegistrar) {
        return registrar;
    }

    PhotochromicResolver private resolver;

    function getResolver() public view returns (PhotochromicResolver) {
        return resolver;
    }

    ENS private ens;

    // Price in USD, e.g. $100 = 100;
    // If we'd like decimals, we can change the exponent in getPriceENS().
    uint256 price;
    // Can be refreshed by an admin with refreshChainlinkAddress.
    ...
    mapping(address => string) private addressToDomainInfo;

    function lastLiveness(string memory userId) public view returns (uint32) {
        return resolver.getValidityInfo(userId).livenessTime;
    }

    function getValidityInfo(string memory userId) public view returns (ValidityInfo memory) {
        return resolver.getValidityInfo(userId);
    }

    // updateLiveness updates the liveness of a userId. The given signature needs to match photochromicSignerAddress.
    function updateLiveness(
        string memory userId,
        uint32 livenessTime,
        EcdsaSig memory sig
    ) public payable {
        // The sender needs to have a userId.
        require(registrar.balanceOf(msg.sender) == 1);
        // TODO: What are we going to update for the liveness?
        bytes32 hash = keccak256(abi.encode(msg.sender, userId, livenessTime));
        address signer = ecrecover(hash, sig.v, sig.r, sig.s);
        // Signature needs to be the same as the pc signer address.
        require(signer == photochromicSignerAddress, "liveness signature does not match contents");
        resolver.updateLiveness(registrar.getNode(userId), userId, uint32(livenessTime));
    }

    uint256 private baseNode;

    address private photochromicSignerAddress;
    ...
    event ticket(address indexed user, string userId, uint8 yrs, uint256 timestamp);
    ...
    constructor(
        ...
        // Resolves/initializes `eth-usd.data.eth` to an address.
        resolveChainlinkAddress();
    }
    ...
    struct DomainInfo {
        ...
        uint32 purchaseTime;
        // The amount of years the user paid for.
        uint8 yrs;
    }
    ...
    function encodeDomainInfo(
        // The time of the ticket purchase.
        uint32 purchaseTime,
        // The amount of years the user payed for.
        uint8 yrs,
        // The user identifier to register.
        string calldata userId
    ) public view returns (string memory) {
        return string(abi.encodePacked(purchaseTime, yrs, userId));
    }

    function decodeDomainInfo(string memory e) public view returns (DomainInfo memory) {
        bytes memory bs = bytes(e);
        DomainInfo memory domainInfo;
        // This checks needs to happen since we try to decode any return value of a map, whether it exists or not. So
        // it is possible that an empty string gets passed to this function.
        if (bs.length <= 5) return domainInfo;

        domainInfo.purchaseTime = (uint32(uint8(bs[0])) << 24) | (uint32(uint8(bs[1])) << 16) | (uint32(uint8(bs[2])) << 8) | uint32(uint8(bs[3]));
        domainInfo.yrs = uint8(bs[4]);
        bytes memory userId = new bytes(bs.length - 5);
        for (uint256 i = 5; i < bs.length; i++) {
            userId[i - 5] = bs[i];
        }
    ...
    // getPrice returns the price/year in USD. Use getPriceEth to get the price in ETH.
    function getPrice() public view returns (uint256) {
        return price;
    }

    // setPrice sets the price/year (in USD).
    function setPrice(uint256 newPrice) external onlyAdmin {
        require(newPrice > 0);
        price = newPrice;
    }

    // getPriceEth returns the ETH price (in wei) that needs to be paid for {yrs} years.
    function getPriceEth(uint8 yrs) public view returns (uint256) {
        (uint80 roundId, uint256 price) = getPriceEthInternal(yrs);
        return price;
    }

    // getPriceEthInternal == getPriceEth but with the round id.
    function getPriceEthInternal(uint8 yrs) internal view returns (uint80, uint256) {
        // There are a lot of ways to do this... it al depends on how much precision you want.
        (uint80 roundId, uint256 usdPrice) = getLatestUSDPrice();
        return (roundId, ((price * (10**26)) / usdPrice) * uint256(yrs));
    }

    // getPriceEthInternal returns the ETH price based on the given round id.
    function getPriceEthInternal(uint8 yrs, uint80 roundId) internal view returns (uint256) {
        return ((price * (10**26)) / getRoundUSDPrice(roundId)) * uint256(yrs);
    }

    // refreshChainlinkAddress is the external method for an admin if the chainlink address needs to be re-resolved. It
    // is already called in the constructor.
    function refreshChainlinkAddress() external onlyAdmin {
        resolveChainlinkAddress();
    }

    // resolveChainlinkAddress resolves the `eth-usd.data.eth` endpoint to an address through ENS.
    function resolveChainlinkAddress() internal {
        // ENS name: eth-usd.data.eth
        // "ETH / USD" hash: 0xf599f4cd075a34b92169cf57271da65a7a936c35e3f31e854447fbb3e7eb736d
        // You can retreive this address from the Graph:
        // https://thegraph.com/hosted-service/subgraph/ensdomains/ens
        // > { domains(where: {name:"data.eth"}) { id, name} }
        bytes32 addr = 0x4a9dd6923a809a49d009b308182940df46ac3a45ee16c1133f90db66596dae1f;
        AddrResolver r = AddrResolver(ens.resolver(addr));
        // > { domains(where: {name:"eth-usd.data.eth"}) { id, name} }
        bytes32 node = 0xf599f4cd075a34b92169cf57271da65a7a936c35e3f31e854447fbb3e7eb736d;
        chainlinkAddress = r.addr(node);
    }

    // getLatestUSDPrice returns latest the roundID and ETH/USD price with 8 decimals of precision.
    function getLatestUSDPrice() internal view returns (uint80, uint256) {
        AggregatorV3Interface prices = AggregatorV3Interface(chainlinkAddress);
        // There is a round every ~1hr. (https://data.chain.link/ethereum/mainnet/crypto-usd/eth-usd)
        // You can also retreive the data of a specific round: getRoundData(uint80 roundId).
        // DOCS: https://docs.chain.link/docs/price-feeds-api-reference/#latestrounddata
        (
            // The round Id.
            uint80 roundId,
            // The price. Chainlink provides 8 decimals of precision, so you have to divide the returned price by 10 ** 8.
            int256 price, // You can use commas to ignore elements of a tuple.
            ,
            ,

        ) = prices.latestRoundData();
        return (roundId, uint256(price));
    }

    // getRoundUSDPrice returns the ETH/USD price based on the given round id.
    function getRoundUSDPrice(uint80 roundId) internal view returns (uint256) {
        AggregatorV3Interface prices = AggregatorV3Interface(chainlinkAddress);
        (, int256 price, , , ) = prices.getRoundData(roundId);
        return uint256(price);
    }

    function purchase(string calldata userId, uint8 yrs) external payable {
        require(bytes(userId).length > 0);
        // Accept both the current Chainlink round as the previous.
        require(yrs > 0, "user must at least purchase a ticket for >= 1 year");
        (uint80 rId, uint256 p) = getPriceEthInternal(yrs);
        if (msg.value < p) require(msg.value >= getPriceEthInternal(yrs, rId - 1), "insufficient amount paid");

        // User has already minted a user id.
        require(registrar.balanceOf(msg.sender) == 0, "can only register 1 userId per account");

        // Is there already a ticket for this userId?
        address addressOwningUserId = userIdToAddress[userId];
        ...
        if (addressOwningUserId != address(0)) {
            DomainInfo memory domainInfo = decodeDomainInfo(addressToDomainInfo[addressOwningUserId]);
            ...
            // QUESTION (quint): remove the previous (expired) ticket for this userId or not?...
            delete userIdToAddress[userId];
            delete addressToDomainInfo[addressOwningUserId];
        }

        // In case there is no (valid) ticket, someone else could still have registered the userId.
        ValidityInfo memory validityInfo = resolver.getValidityInfo(userId);
        if (validityInfo.expiryTime != 0) {
            // Someone owns the user id already...
            // Did it expire already or is the userId still inside its grace period?
            require(validityInfo.expiryTime + gracePeriod < block.timestamp, "this userId was already minted but is still valid/in its grace period");

            // It did expire!
            bytes32 tokenId = registrar.getNode(userId);
            registrar.burn(uint256(tokenId));
            resolver.clearPCRecords(registrar.getNode(userId));
            resolver.deleteValidityInfo(tokenId, userId);
        }
        ...
        if (bytes(domainInfo.userId).length != 0) {
            require(domainInfo.purchaseTime + ticketValidity < block.timestamp, "a ticket was purchased for the userId and is not yet expired");

            // Ticket expired.
            delete userIdToAddress[domainInfo.userId];
        }

        // Create a new ticket for the given userId.
        userIdToAddress[userId] = msg.sender;
        uint32 currentTime = uint32(block.timestamp);
        addressToDomainInfo[msg.sender] = encodeDomainInfo(currentTime, yrs, userId);
        emit ticket(msg.sender, userId, yrs, currentTime);
    }

    // available checks whether the given userId is still available.
    // 1. There is no (valid) ticket for this userId.
    // 2. The userId is not yet registered or in its grace period.
    function available(string memory userId) public view returns (bool) {
        // (1)
        address addresOwningUserId = userIdToAddress[userId];
        ...
        // (2)
        ValidityInfo memory validityInfo = resolver.getValidityInfo(userId);
        if (block.timestamp < validityInfo.expiryTime + gracePeriod) {
    ...
    // myTicketUserId returns the user id for which the sender has reserved a ticket.
    function myTicketUserId() public view returns (string memory) {
        DomainInfo memory domainInfo = decodeDomainInfo(addressToDomainInfo[msg.sender]);
    ...
    function isValidTicket(string memory userId, address requester) public view returns (bool) {
        address addresOwningUserId = userIdToAddress[userId];
        DomainInfo memory domainInfo = decodeDomainInfo(addressToDomainInfo[addresOwningUserId]);
        return requester != address(0) && addresOwningUserId == requester && block.timestamp <= domainInfo.purchaseTime + ticketValidity;
    }

    // burnTicket removes the ticket for the given userId.
    function burnTicket(string memory userId) external {
        address userAddress = userIdToAddress[userId];
        require(msg.sender == userAddress || msg.sender == adminAddress);
        delete userIdToAddress[userId];
        delete addressToDomainInfo[userAddress];
    }

    // burn removes any open tickets and the expiry of the userId of the sender.
    // Also burns the userId at the registrar.
    function burn(string memory userId) external {
        require(isOwnerOf(msg.sender, userId), "user does not own the given userId");
        registrar.burn(registrar.tokenOfOwnerByIndex(msg.sender, 0));
        delete userIdToAddress[userId];
        delete addressToDomainInfo[msg.sender];
        resolver.clearPCRecords(registrar.getNode(userId));
        resolver.deleteValidityInfo(registrar.getNode(userId), userId);
    }

    // burnAsAdmin removes any open tickets and the expiry of the userId.
    // Also burns the userId at the registrar.
    function burnAsAdmin(address owner, string memory userId) external onlyAdmin {
        require(isOwnerOf(owner, userId), "user does not own the given userId");
        registrar.burn(registrar.tokenOfOwnerByIndex(owner, 0));
        delete userIdToAddress[userId];
    ...
    function isOwnerOf(address owner, string memory userId) internal view returns (bool) {
        if (registrar.balanceOf(owner) == 0) return false;
        bytes32 label = bytes32(keccak256(abi.encodePacked(userId)));
        uint256 tokenId = uint256(keccak256(abi.encodePacked(baseNode, label)));
        return registrar.ownerOf(tokenId) == owner;
    }

    // transferBalance transfers out the specified amount to the admin account.
    ...
    function getUserid() public view returns (string memory userid, bool minted) {
        string memory userid;
        if (registrar.balanceOf(msg.sender) > 0) {
            uint256 tokenId = registrar.tokenOfOwnerByIndex(msg.sender, 0);
            return (resolver.text(bytes32(tokenId), Validator.PC_USERID), true);
        }
        userid = myTicketUserId();
        if (bytes(userid).length > 0) {
            return (userid, false);
        }
    }

    // mint creates a the given userId if:
    // 1. The sender has a ticket for the given userId.
    // 2. The signature matches photochromicSignerAddress.
    function mint(
        string memory userId,
        bytes32 ipfsHash,
        uint32 livenessTime,
        ...
        require(userIdToAddress[userId] == msg.sender, "need to purchase a ticket first");

        // 2. Check validity of signature (addressed that signed contents should be pcAdmin).
        bytes32 hash = keccak256(abi.encode(msg.sender, livenessTime, contents, userId, ipfsHash));
        address signer = ecrecover(hash, sig.v, sig.r, sig.s);
        require(signer == photochromicSignerAddress, "signature does not match contents");

        uint256 node = registrar.register(msg.sender, userId, ipfsHash);
        bytes32 nodeBytes = bytes32(node);
        resolver.setPCRecords(nodeBytes, userId, contents, msg.sender);
        ens.setResolver(nodeBytes, address(resolver));

        DomainInfo memory domainInfo = decodeDomainInfo(addressToDomainInfo[msg.sender]);
        resolver.setValidityInfo(
            registrar.getNode(userId),
            userId,
            uint32(domainInfo.purchaseTime + uint32(domainInfo.yrs) * 365 days),
            livenessTime
        );

        // Remove ticket, only the expiry info remains after mint.
        delete userIdToAddress[userId];
        delete addressToDomainInfo[msg.sender];
...
contract PhotochromicResolver is ResolverStub, AddrResolver, TextResolver {
    ...
    // Mapping from user id to their corresponding expiry info.
    // Long term storage.
    mapping(string => ValidityInfo) private userIdToValidityInfo;
    ...
    function setText(
        ...
        require(!Validator.isPhotochromicRecord(key));
    ...
    function setValidityInfo(
        ...
        userIdToValidityInfo[userId].expiryTime = expiryTime;
        userIdToValidityInfo[userId].livenessTime = livenessTime;
    }
    ...
    function getValidityInfo(string calldata userId) public view returns (ValidityInfo memory) {
        return userIdToValidityInfo[userId];
    }

    function deleteValidityInfo(bytes32 node, string calldata userId) external authorised(node) {
        delete userIdToValidityInfo[userId];
    }
