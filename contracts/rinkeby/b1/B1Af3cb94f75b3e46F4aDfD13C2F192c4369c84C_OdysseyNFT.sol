...
import "./interfaces/IOdysseyNFT.sol";
import "./layerzero/ONFT721.sol";

contract OdysseyNFT is ONFT721, IOdysseyNFT {
	bytes32 public merkleRoot;
	uint256[] public offsetTokenIds;
	...
	constructor(
		...
		offsetTokenIds = new uint256[](3);
		nextTokenIds = new uint256[](3);
	}

	function getNewTokenId(uint8 _tier) internal view returns (uint256) {
		return offsetTokenIds[_tier] + nextTokenIds[_tier];
	}

	function setOffsetTokenIds(uint256[] calldata _sizeOfTokens)
		external
		onlyOwner
	{
		uint256 offsetIds;
		for (uint256 i = 0; i < _sizeOfTokens.length; i++) {
			offsetTokenIds[i] = offsetIds;
			offsetIds += _sizeOfTokens[i];
		}

		emit OffsetTokenIdsChanged(_sizeOfTokens);
	}
	...
	function mint(bytes32[] calldata _merkleProof, uint8 _tier) external {
		bytes32 node = keccak256(abi.encodePacked(msg.sender, _tier));
		if (claimed[node]) {
			revert AlreadyClaimed(msg.sender, _tier);
		}

		if (!MerkleProof.verify(_merkleProof, merkleRoot, node)) {
			revert InvalidMerkleProof(msg.sender, _tier);
		}

		_safeMint(msg.sender, getNewTokenId(_tier));
		claimed[node] = true;
		nextTokenIds[_tier]++;
...
interface IOdysseyNFT {
	...
	// --- Events ---
	event OffsetTokenIdsChanged(uint256[] _sizeOfToken);
	...
	// --- External Fuctions ---
	function setOffsetTokenIds(uint256[] calldata _sizeOfToken) external;
...
import "../interfaces/layerzero/IONFT721Core.sol";
	...
	ILayerZeroUserApplicationConfig
{
	...
	function getConfig(
		...
		return
			lzEndpoint.getConfig(_version, _chainId, address(this), _configType);
	}
