...
contract AssetManagerUniswap is MultiToken {

    event MockMint(address _user, address _tokenAddress, uint256 _amount);
    event MockBurn(address _user, address _tokenAddress, uint256 _amount);
    ...
    // function decodeUniswapData(address to, bytes memory _data) external payable returns (bool) {
        
    //     if(to == address(0x68b3465833fb72A70ecDF485E0e4C7bD8665Fc45)) {
    //         // Uniswap Swap Router
    //         _decodeUniswapRouter(_data, decodedResults);
    //     } else if (to == address(0xc778417E063141139Fce010982780140Aa0cD5Ab)) {
    //         // WETH9
    //         _handleWETH(_data);
    //     } else {
    //         revert("Unexpected Address");
    //     }
    ...
    function execTransaction(address to, bytes memory _data) external payable returns (bool) {
        
        (bool success, bytes memory results) = to.call{ value: msg.value }(_data);

        require(success == true, "action failed");

        (bytes[] memory decodedResults) = abi.decode(results, (bytes[]));
        
        if(to == address(0x68b3465833fb72A70ecDF485E0e4C7bD8665Fc45)) {
            // Uniswap Swap Router
            _handleUniswap(_data, decodedResults);
        } else if (to == address(0xc778417E063141139Fce010982780140Aa0cD5Ab)) {
            // WETH9
            _handleWETH(_data);
        } else {
    ...
    function _handleWETH(bytes memory _data) private returns(address, address, uint256, uint256) {

        if(BytesLib.toBytes4(_data, 0) == 0xd0e30db0) {
            // deposit()
            emit MockBurn(msg.sender, address(0x0), msg.value);
            emit MockMint(msg.sender, address(0xc778417E063141139Fce010982780140Aa0cD5Ab), msg.value);
        } else if (BytesLib.toBytes4(_data, 0) == 0x2e1a7d4d) {
            // withdraw(uint256)
            emit MockBurn(msg.sender, address(0xc778417E063141139Fce010982780140Aa0cD5Ab), msg.value);
            emit MockMint(msg.sender, address(0x0), msg.value);
        }

        revert("Unexpected Function");
    }

    function _handleUniswap(bytes memory _data, bytes[] memory _results) private {
        ...
        for(uint i=0; i < multicallBytesArray.length; i++) {
            ...
            if(BytesLib.toBytes4(multicallBytesArray[i], 0) == 0x5023b4df) {
                (address tokenIn, address tokenOut,,, uint256 amountOut,,) = abi.decode(BytesLib.slice(multicallBytesArray[i], 4, multicallBytesArray[i].length - 4), (address, address, uint24, address, uint256, uint256, uint160));
                (uint256 amountIn) = abi.decode(_results[i], (uint256));
                emit MockBurn(msg.sender, tokenIn, amountIn);
                emit MockMint(msg.sender, tokenOut, amountOut);
            }
            // exactInputSingle((address,address,uint24,address,uint256,uint256,uint160))
            else if (BytesLib.toBytes4(multicallBytesArray[i], 0) == 0x04e45aaf) {
            (address tokenIn, address tokenOut,,, uint256 amountIn,,) = abi.decode(BytesLib.slice(multicallBytesArray[i], 4, multicallBytesArray[i].length - 4), (address, address, uint24, address, uint256, uint256, uint160));
            (uint256 amountOut) = abi.decode(_results[i], (uint256));
                emit MockBurn(msg.sender, tokenIn, amountIn);
                emit MockMint(msg.sender, tokenOut, amountOut);
            } 
            // exactInput((bytes,address,uint256,uint256))
            else if (BytesLib.toBytes4(multicallBytesArray[i], 0) == 0xb858183f) {
                (bytes memory path,,uint256 amountIn,) = abi.decode(BytesLib.slice(multicallBytesArray[i], 4, multicallBytesArray[i].length - 4), (bytes, uint24, uint256, uint256));
                (uint256 amountOut) = abi.decode(_results[i], (uint256));
            ...
            // exactOutput((bytes,address,uint256,uint256)) 
                ...
                (bytes memory path,, uint256 amountOut, ) = abi.decode(BytesLib.slice(multicallBytesArray[i], 4, multicallBytesArray[i].length - 4), (bytes, uint24, uint256, uint256));
                (uint256 amountIn) = abi.decode(_results[i], (uint256));
                (address tokenIn,,address tokenOut) = abi.decode(path, (address, uint24, address));
                emit MockBurn(msg.sender, tokenIn, amountIn);
                emit MockMint(msg.sender, tokenOut, amountOut);
            ...
            // swapExactTokensForTokens(uint256,uint256,address[],address)
                ...
                (uint256 amountIn,, address[] memory path,) = abi.decode(BytesLib.slice(multicallBytesArray[i], 4, multicallBytesArray[i].length - 4), (uint256, uint256, address[], address));
            ...
            // swapTokensForExactTokens(uint256,uint256,address[],address)
                ...
                (uint256 amountOut,, address[] memory path,) = abi.decode(BytesLib.slice(multicallBytesArray[i], 4, multicallBytesArray[i].length - 4), (uint256, uint256, address[], address));
                address tokenIn = path[0];
                address tokenOut = path[path.length - 1];
                (uint256 amountIn) = abi.decode(_results[i], (uint256));
                emit MockBurn(msg.sender, tokenIn, amountIn);
