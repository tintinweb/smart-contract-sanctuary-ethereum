    ...
    Ownable
{
    ...
    constructor(
        ...
        uint16 _royalityLimit // Max Royalty limit for collection
    ) ERC721(_name, _symbol) {
        require(
            _royalityLimit <= _feeDenominator() * 100,
            "ERC721Base: Royality limit must be below 100%"
        );
    ...
    function setRoyalityLimit(uint16 _royalityLimit) external onlyOwner {
        require(
            _royalityLimit <= _feeDenominator() * 100,
            "ERC721Base: Royality limit must be below 100%"
        );
    ...
    function _mint(
        ...
        require(
            _royality <= royalityLimit,
            "ERC721Base: Royality must be below royality limit"
        );
        counter++;
        _mint(to, counter);
        _setTokenRoyalty(counter, to, _royality);
    ...
    function _lazyMint(
        ...
        uint256 tokenId = _mint(to, _royality, _tokenURI);
...
contract ERC721Collection is ERC721Base{
    ...
    ) ERC721Base(_name, _symbol, _contractURI, _tokenURIPrefix, _royalityLimit) {
        mintableAddress = _mintableAddress;
        _transferOwnership(sender);
    }

    function mint(string memory tokenURI, uint96 _royality) public payable{
        require(msg.value >= IERC721Mintable(mintableAddress).mintingCharge(), "ERC721Collection: Minting charges required");
        _mint(msg.sender, _royality, tokenURI);
    ...
    function lazyMint(
        ...
        require(
            msg.value >= price + IERC721Mintable(mintableAddress).mintingCharge(),
            "ERC721Collection: Insufficent fund transferred."
        );
        _lazyMint(to, _royality, _tokenURI, nonce, price, sign);
        payable(to).transfer(price);
        mintableAddress.transfer(msg.value - price);
    }
}
