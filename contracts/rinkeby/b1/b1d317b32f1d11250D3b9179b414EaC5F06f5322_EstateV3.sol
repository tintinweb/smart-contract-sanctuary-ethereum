...
contract Migratable {
     ...
     */
    string private constant INITIALIZED_ID = "initialized";
    ...
    modifier isMigration(
        ...
        require(
            ...
            "Prerequisite migration ID has not been run yet"
        );
...
contract LANDRegistry {
    ...
    function setManyUpdateOperator(uint256[] landIds, address operator)
        external;
...
contract EstateStorage {
    ...
    mapping(uint256 => assetData) public estateData;
    ...
    EstateStorage
{
    ...
    function mint(
        ...
        return _mintEstate(to, name, description);
    }
    ...
    function updateMetadata(
        ...
        string name,
        string description
    ) external onlyUpdateAuthorized(estateId) {
        _updateMetadata(estateId, name, description);
    }

    function getMetadata(uint256 estateId) external view returns (assetData) {
     ...
     */
    function setManyUpdateOperator(uint256[] _estateIds, address _operator)
        public
    {
    ...
    function tokenURI(uint256 estateId) public view returns (string) {
        require(exists(estateId), "The Estate id should exist");
        return
            string(
                abi.encodePacked(
                    "https://api.decentraland.org/v2/estates/",
                    toString(estateId)
                )
            );
    }
    ...
    function updateLandData(
        ...
        _updateLandData(estateId, landId, name, description);
    }
    ...
    function updateManyLandData(
        ...
        uint256 length = landIds.length;
        for (uint256 i = 0; i < length; i++) {
            _updateLandData(estateId, landIds[i], name, description);
        }
    }
    ...
    function _mintEstate(
        ...
        _mint(to, estateId);
        _updateMetadata(estateId, name, description);
        emit CreateEstate(to, estateId, name, description);
        return estateId;
    }
    ...
    function _updateMetadata(
        ...
        estateData[estateId].name = name;
        estateData[estateId].description = description;
        emit Update(estateId, ownerOf(estateId), msg.sender, name, description);
    }
    ...
    function _getNewEstateId() internal view returns (uint256) {
        uint256 tokenId = nextTokenId.add(1);
        nextTokenId = tokenId;
        return tokenId;
    }
    ...
    function _updateLandData(
        ...
        registry.updateLandData(x, y, name, description);
    }
    ...
    function estatesOf(address _user) external returns (uint256[]) {
        uint256 balance = balanceOf(_user);
        uint256[] estates;
        for (uint256 i = 0; i < balance; i++) {
            estates[i] = ownedTokens[_user][i];
        }
        return estates;
    }
}