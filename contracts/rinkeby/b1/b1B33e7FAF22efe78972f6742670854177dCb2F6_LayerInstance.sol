...
abstract contract Layer {
  address composableNFTAddress;
  bool public isDefaultLayer;
  mapping (uint256 => OptionSelection) _selectedOption;
  ...
  function getOptionMetadataById (
    uint256 tokenId
  )
    external
    virtual
    ...
    returns (string memory)
  {
    return string.concat(
      ...
      layerName,
      '", "value": "',
      layerOptions[_selectedOption[tokenId].optionNumber].value,
      '" }'
    );
  }
  ...
  function renderOptionById(
    ...
    returns (string memory)
  {
    string memory color2Hex = _selectedOption[tokenId].color2Hex;
    ...
    return string(abi.encodePacked(
      ...
      '"',
      ' color="',
      _selectedOption[tokenId].color1Hex,
      '">',
      bytes(color2Hex).length > 1 ? secondColorVar : '',
      layerOptions[_selectedOption[tokenId].optionNumber].svg,
      '</g>'
      ));
  }
  ...
  ) public
  {
    ...
    for(uint256 i = 0; i < layerOptions.length; i++){
      if(
        ...
        _selectedOption[tokenID] = OptionSelection({
          optionNumber: i,
  ...
  ) public
  {
    ...
    if(bytes(color1Hex).length > 1){
      _selectedOption[tokenId].color1Hex = color1Hex;
    }

    if(bytes(color2Hex).length > 1){
      _selectedOption[tokenId].color2Hex = color2Hex;
    }
  }
}
