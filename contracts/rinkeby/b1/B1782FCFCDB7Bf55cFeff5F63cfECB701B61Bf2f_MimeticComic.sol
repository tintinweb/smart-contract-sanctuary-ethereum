    ...
    , IMimeticComic
{
     ...
     *         ownership for far cheaper still without having to write ownership.
     *         To be used by someone that vaulted many tokens and wants their comics
        ...
        onlyMasterUnlocked()
    {
        ...
        if(msg.sender != _owner) revert TokenOwnerMismatch();
    ...
    function refreshTokenBundle(
        ...
        onlyMasterUnlocked()
    {
        if(!isClaimable(_tokenIds)) revert TokenBundleInvalid();
        ...
        for(
            ...
            address _owner = mirror.ownerOf(_tokenIds[i]);
