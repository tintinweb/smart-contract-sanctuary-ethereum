...
contract Moonopoly is ERC1155, Ownable, Pausable, ReentrancyGuard
{
    ...
    //debugging
    event pseudoRandomResult(uint256 indexed result);
    event drawnoffpseudo(uint256 indexed result);
    event drawnoffdeck(uint256 indexed result);
    ...
    function _drawCard() private returns (uint256) 
    {
     ...
     function _drawAvailableCard() private returns (uint256) 
     {
        ...
        revert("Insufficient Card Amount");
    }
    
    function _canMint(uint256 cardID) private view returns (bool) { return (_CARD_ID_MAPPING[cardID] > 0); }
