...
// @author: @pepperonick, with a lot of help from @andrewjiang and @yungwknd
...
contract SignsOfTheTimes is AdminControl, ICreatorExtensionTokenURI {
    ...
    string private _beginningUri;
    string private _endUri;
    string private _compositeUri;
    uint256 public beginningCount = 0;
    uint256 public endCount = 0;
    uint256 private compositeCount = 0;
    uint256 public totalSupply = 0;
    bytes32 public _merkleRoot;

    // 0 = Beginning; 1 = End; 2 = Composite
    mapping(uint256 => TokenType) public tokenToTokenType;

    // Used to make sure user gets only 1 gas-only mint
    mapping(address => uint256) public addressToTokenCount;

    // Used to keep count of token type at time of mint
    // "The End is Near #3" is the 3rd minted The End is Near
    mapping(uint256 => uint256) public tokenToCount;
    ...
    modifier isPaused() {
        require(
            ...
            "Contract cannot be open for minting"
        );
        _;
    }
    ...
    function canMintGasOnly(address account, bytes32[] calldata proof)
        public
        view
        returns (bool)
    {
    ...
    function incrementBeginningOrEndCounts(bool beginningOrEnd) private {
        TokenType tokenType = beginningOrEnd
            ? TokenType.Beginning
            : TokenType.End;

        if (tokenType == TokenType.Beginning) {
            beginningCount += 1;
        } else {
            endCount += 1;
        }
        ...
        // update mappings to reflect new quantities
        addressToTokenCount[msg.sender] += 1;
        tokenToTokenType[totalSupply] = tokenType;

        if (tokenType == TokenType.Beginning) {
            tokenToCount[totalSupply] = beginningCount;
        } else {
            tokenToCount[totalSupply] = endCount;
        }
    }

    function incrementCompositeCounts() private {
        totalSupply += 1;
        compositeCount += 1;

        addressToTokenCount[msg.sender] += 1;
        tokenToTokenType[totalSupply] = TokenType.Composite;
        tokenToCount[totalSupply] = compositeCount;
    }
    ...
    {
        require(
            canMintGasOnly(msg.sender, proof),
            "Not on the approved gas-only address list"
        );

        require(
            addressToTokenCount[msg.sender] == 0,
            "Can only mint 1 gas-free token"
        );

        IERC721CreatorCore(_creator).mintExtension(msg.sender);
        incrementBeginningOrEndCounts(beginningOrEnd);
    }
        ...
        isOpen
    {
        ...
        for (uint16 i = 1; i <= uint16(quantity); i++) {
            incrementBeginningOrEndCounts(beginningOrEnd);
        }
    }

    function mintComposite(uint256[] calldata tokenIds) public isPaused {
        require(tokenIds.length == 2, "Requires 2 Beginning or End tokens");

        for (uint16 i = 0; i < tokenIds.length; i++) {
            require(
                IERC721(_creator).ownerOf(tokenIds[i]) == msg.sender,
                "Tokens must be owned by message sender"
            );

            require(
                tokenToTokenType[tokenIds[i]] != TokenType.Composite,
                "Tokens must be a Beginning or End token"
            );

            try IERC721(_creator).getApproved(tokenIds[i]) returns (
    ...
    function withdraw(address to) public onlyOwner {
        ...
        bool success;
        (success, ) = creator.call{value: sendAmount}("");
    ...
    function getName(uint256 tokenId) private view returns (string memory) {
        ...
        if (tokenToTokenType[tokenId] == TokenType.Beginning) {
            name = "The Beginning is Near";
        } else if (tokenToTokenType[tokenId] == TokenType.End) {
        ...
        return
            string(
                abi.encodePacked(name, " #", tokenToCount[tokenId].toString())
            );
    }
