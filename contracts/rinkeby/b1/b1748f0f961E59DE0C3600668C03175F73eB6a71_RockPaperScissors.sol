...
contract RockPaperScissors is VRFConsumerBaseV2 {
    ...
    function shoot(uint8 bet) public payable {
        ...
        require(msg.value == 0.01 ether, "Invalid bet amount sent");
        require(
    ...
    function getUserBets(address userAddress) public view returns (UserBet[] memory) {
        UserBet[] memory userBetsArray = new UserBet[](countUserBets(userAddress));
    ...
    function countUserBets(address userAddress) internal view returns (uint256) {
        uint256 count = 0;
        for (uint256 i; i <= _currentRound.current(); ++i) {
    ...
    function getUserBet(uint256 round, address userAddress) public view returns (UserBet memory) {
