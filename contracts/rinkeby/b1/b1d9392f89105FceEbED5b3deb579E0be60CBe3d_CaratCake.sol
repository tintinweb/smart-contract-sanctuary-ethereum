	...
	ERC721Burnable
{
	...
	uint public TOTAL_SUPPLY = 30;

	bool public PREMINT_IS_LIVE = false;
	bool public PUBLIC_MINT_IS_LIVE = false;
	...
	uint8 private WALLET_MINT_MAX = 2;
	bytes32 public merkleRoot =
		0x3993369379f8a1d06cf46942d82de5f31cbccac7cce052f05b40e426fc7d4893;
	...
	function getSupply() public view returns (uint) {
		return TOTAL_SUPPLY - (_tokenIdCounter.current() - 1);
	}

	function setPremintLive(bool live) public onlyAdmin {
		PREMINT_IS_LIVE = live;
	}

	function setPublicMintLive(bool live) public onlyAdmin {
		PUBLIC_MINT_IS_LIVE = live;
	}
	...
	function setAdmin(address addr, bool shouldAdmin) public onlyOwner {
		require(
			...
			"Cannot change user to the same state. Admin status is already"
		);
		isAdmin[addr] = shouldAdmin;
	}
	...
	function mint(uint amount, bytes32[] memory proof)
		...
		when(PREMINT_IS_LIVE || PUBLIC_MINT_IS_LIVE)
	{
		require(msg.value == (PRICE * amount), "Insufficient payment");
		require(
			amount + _premintCounter.current() <= TOTAL_SUPPLY,
	...
	function handleCounters() internal {
		uint totalSupply = TOTAL_SUPPLY - _tokenIdCounter.current();
		if (totalSupply < 1) {
	...
	function withdraw() external onlyOwner {
		if (_tokenIdCounter.current() == TOTAL_SUPPLY) {
