...
contract GoldStandardBondDepository is Ownable {
    ...
    address public immutable GSD; // token given as payment for bond
    ...
    bool public isStableAsset = true; // Non-usd backed assets needs to derive bond price
    ...
    /**
     *  @notice sets chainlink price feed (for non-usd backed token or token pair bonds only)
     *  @param _feed asset's chainlink price feed contract address
     */
    function setPriceFeed(address _feed) external onlyPolicy() {
        priceFeed = AggregatorV3Interface( _feed );
        isStableAsset = ( _feed == address(0) );
    }
    ...
    function deposit( 
        ...
        if ( fee != 0 ) { // fee is transferred to dao 
            IERC20( GSD ).safeTransfer( DAO, fee ); 
        }
    ...
    function stakeOrSend( address _recipient, bool _stake, uint _amount ) internal returns ( uint ) {
        if ( !_stake ) { // if user does not want to stake
            IERC20( GSD ).transfer( _recipient, _amount ); // send payout
        } else { // if user wants to stake
            if ( useHelper ) { // use if staking warmup is 0
                IERC20( GSD ).approve( stakingHelper, _amount );
                IStakingHelper( stakingHelper ).stake( _amount, _recipient );
            } else {
                IERC20( GSD ).approve( staking, _amount );
    ...
    function maxPayout() public view returns ( uint ) {
        return IERC20( GSD ).totalSupply().mul( terms.maxPayout ).div( 100000 );
    }
    ...
    /**
     *  @notice get asset price from chainlink, not neccessarily usable for usd stable assets (including gsd and usd stable asset pairs)
     */
    ...
    function bondPriceInUSD() public view returns ( uint ) {
        if (!isStableAsset) {
            if (!isLiquidityBond) {
                return bondPrice().mul( uint( assetPrice() ) ).mul( 1e6 );
            }
            ...
            return bondPrice()
                    ...
                    .mul( uint( assetPrice() ) )
                    .div( 1e12 );
        } else if ( isLiquidityBond ) {
            return bondPrice().mul( IBondCalculator( bondCalculator ).markdown( principle ) ).div( 100 );
        } else {
            return bondPrice().mul( 10 ** IERC20( principle ).decimals() ).div( 100 );
        }
    }
    ...
    function debtRatio() public view returns ( uint debtRatio_ ) {   
        uint supply = IERC20( GSD ).totalSupply();
    ...
    function standardizedDebtRatio() external view returns ( uint ) {
        if (!isStableAsset) {
            return debtRatio().mul( uint( assetPrice() ) ).div( priceFeed.decimals() ); // Price feed decimals
        } else if ( isLiquidityBond ) {
