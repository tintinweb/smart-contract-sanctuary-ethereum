...
interface IREMXNFT is IERC165 {

  // token redemption event
  event RedeemEvent(address indexed, uint256);

  function redeem(address account, uint256 tokenId, uint256 amount, bytes calldata signature)
    external payable;
}
 ...
 */
contract REMXNFT is ERC721, EIP712, AccessControl, IERC2981, IREMXNFT {
    ...
    RevenueSplitter private _revenueSplitter;

    /**
     * @dev initialize the contract and set up for lazy minting */
    constructor(address revenueSplitter, string memory name, string memory symbol)
    ERC721(name, symbol)
    EIP712(name, "1.0.0")
    {
        _setupRole(DEFAULT_ADMIN_ROLE, _msgSender());
        _revenueSplitter = RevenueSplitter(revenueSplitter);
    }
    ...
    function supportsInterface(bytes4 interfaceId) public view virtual override(ERC721, AccessControl, IERC165) returns (bool) {
        return
          interfaceId == type(IREMXNFT).interfaceId ||
    ...
    {
        require(_verify(_hash(account, tokenId, amount), signature), "RMX: Invalid signature");
        _safeMint(account, tokenId);

        emit RedeemEvent(account, tokenId);
    }

    /**
     * @dev internal function to compute the hash of input parameters for validation
     */
    function _hash(address account, uint256 tokenId, uint256 amount)
    internal view returns (bytes32)
    {
        return _hashTypedDataV4(keccak256(abi.encode(
            keccak256("NFT(uint256 tokenId,address account,uint256 amount)"),
            tokenId,
            account,
            amount
        )));
 ...
 */
contract RevenueSplitter is Context, Ownable, AccessControl {
  ...
  // the balance of each member of our community in Ether
  mapping(address => uint256) private _balances;

  // a single participation in a collection
  struct CollectionPayee {
    address payee;
    uint256 percent;  // in 1/1000th
  }

  // map a collection address to the participants in the revenue sharing for that collection
  mapping(address => CollectionPayee[]) private _payees;

  // keep track of which collections are registered
  // possibly unneeded, perhaps we could use _payees to detect registration?
  mapping(address => bool) private _collections;

  // Collection Management Events
  event RegisterCollectionEvent(address indexed collection); //4d4043e7a371721850e5f172c165a26806afc6f2cc0f2b9dea455666189b6d8b
  event AddPayeeEvent(address indexed collection,address indexed account,uint256 shares); //9bb86ebdaeb25d50091c2c2d099b111750ac4e05fc421a44272d64bbb0ce8ad8

  // Token $$ Events
  event BuyEvent(address indexed collection,uint256 tokenId,uint256 amount); //2df3862cf7fc998a063a76deb7d62d101ba25f75f99a593b1f3ccb270403e31b
  event RoyaltyEvent(address indexed collection,uint256 amount); //bd488fda61aa9abe34b23f2b9eb1217362c895a4c5834751a666303e2ff34911

  // Collaborator $$ Events
  event WithdrawEvent(address indexed payee,uint256 amount); //d4c8ebb1edf94b410ac317c6c08c76f6e847b74da7370d0545569aedc42cab2a
  event DepositEvent(address indexed collection,address indexed payee,uint256 amount); //6e05140b0ffc77e13e865d4ae60db865d74c32ace1a6085fefe599e48cf27484

  // Payout Events
  event SweepEvent(address indexed payee,address indexed recipient,uint256 amount); //560ca6a65de25b0ee91431b5b080f6d8b5055ff16f5653349ae4e3fc20a201c8
  event PayoutEvent(address indexed payee,uint256 amount); //c3f74068ac89d452fac60b6d8dea3fb69b0edcc4dfb16bdf98dc5bb03a87544b
  ...
  /**
   * @dev register a deployed NFT contract with the revenue splitter
   *
   * Emits a {RegisterCollectionEvent}
   */
  function registerCollection(address _collection) public onlyOwner returns (bool) {

    require(_collection.supportsInterface(type(IREMXNFT).interfaceId), "RMX: Invalid collection");
    require(!_collections[_collection], "RMX: Already registered");

    _collections[_collection] = true;

    emit RegisterCollectionEvent(_collection);
    return true;
  }
   ...
   */
  function isRegistered(address _collection) public view returns(bool) {
   ...
   */
  function addCollectionPayee(address _collection, address _payee, uint256 shares) public onlyOwner {
    require(_payee != address(0), "RMX: payee is zero address");
    require(_collections[_collection], "RMX: Unknown collection");
    require(shares <= 1000, "RMX: Percentage too high");
    require(shares > 0, "RMX: shares are 0");
    for (uint i = 0; i < _payees[_collection].length; i++) {
      require(_payees[_collection][i].payee != _payee, "RMX: Payee already registered");
    }
    _payees[_collection].push(CollectionPayee(_payee, shares));
   ...
   * Emits a {BuyEvent}
   */
  function buyNFT(address payable _collection, uint256 tokenId, bytes calldata signature) public payable {
    require(_collections[_collection], "RMX: Unknown collection");

    REMXNFT collection = REMXNFT(_collection);
    collection.redeem(_msgSender(), tokenId, msg.value, signature);
   ...
   * Emits a {SweepEvent}
   */
  function sweep(address _payee, address payable _recipient) public onlyRole(PAYOUT_ROLE) {
    uint256 amount = _balances[_payee];
    require(amount > 0, "RMX: Insufficient balance");
    _balances[_payee] = 0;

    Address.sendValue(_recipient, amount);
   ...
   * Emits a {PayoutEvent}
   */
  function payout(address payable _payee) public onlyRole(PAYOUT_ROLE) {
    uint256 amount = _balances[_payee];
    require(amount > 0, "RMX: Insufficient balance");
    _balances[_payee] = 0;
   ...
   */
  function depositRoyalty(address _collection) public payable {
    require(_collections[_collection], "RMX: Unknown collection");

    _distributeFunds(_collection, msg.value);
    // emit Royalties event
  ...
  function _distributeFunds(address _collection, uint256 amount) internal {
    for (uint i = 0; i < _payees[_collection].length; i++) {
      CollectionPayee memory _collectionPayee =  _payees[_collection][i];
      uint256 deposit = amount * _collectionPayee.percent / 1000;
      amount -= deposit;

      if (i == _payees[_collection].length - 1) {
        deposit += amount;
        amount = 0;
      }
