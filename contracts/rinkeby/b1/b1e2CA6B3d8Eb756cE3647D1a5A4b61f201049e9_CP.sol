    ...
    ReentrancyGuard
{
    ...
    uint64 public salePrice = 0.083 ether;
    ...
    function allowListMint(uint8 quantity) external payable callerIsUser {
        require(contractStatus == ContractStatus.AllowList, "allowlist sale has not begun yet");
    ...
    function publicSaleMint(uint8 quantity) external payable callerIsUser {
        require(contractStatus == ContractStatus.Public, "public sale has not begun yet");
        require(
            totalMinted() + quantity <= collectionSize,
            "reached max supply"
        );
