...
contract RandomKidNFT is ERC721AA, Ownable {
    using Strings for uint256;

    uint256 public constant TOTAL_SUPPLY = 2000;
    uint256 public price = 0.01 ether;
    string public baseURI = "https://gateway.pinata.cloud/ipfs/QmQ4FRAdsqx2UJzJvkQ2HPZoKHjmWidbLUxfCPq8E5THRn/";

    mapping(address => bool) public minted;
    bytes32 public merkelRoot;

    constructor() ERC721AA("RandomKid", "RK") {}

    function preMint(bytes32[] calldata proof, uint256 amount) external {
    ...
    function mint(uint256 amount) external payable {
        require(totalSupply() + amount <= TOTAL_SUPPLY, "Exceed max supply");
        require(amount * price == msg.value, "Wrong price");

        _safeMint(msg.sender, amount);
    }

    function merkleVerify(bytes32[] memory proof, bytes32 leaf)
        private
        view
        returns (bool)
    {
