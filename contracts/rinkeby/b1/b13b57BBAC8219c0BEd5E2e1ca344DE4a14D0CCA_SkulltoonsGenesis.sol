...
contract SkulltoonsGenesis is ERC721A, IERC2981, Ownable, ReentrancyGuard {
    ...
    bytes32 private specialListRoot;

    constructor(string memory _baseTokenURI, string memory _nonRevealedURI, address _requiredOwnContract) ERC721A("SkulltoonsGenesis", "SKTGEN") {
    ...
    // Business Logic
    function getAllowedMintCount(bytes32[] calldata _wlProof, bytes32[] calldata _slProof) internal view returns (uint) {
        uint allowedCount = 0;
        ...
        if (verifyWhiteList(_wlProof)) {
            allowedCount = 1;
        }

        if (verifySpecialList(_slProof)) {
            allowedCount += 1;
        }
    ...
    // function preSaleMint(bytes32[] calldata _wlProof, bytes32[] calldata _slProof) external payable callerIsUser nonReentrant {
    //     uint256 allowedCount = getAllowedMintCount(_wlProof, _slProof) - _numberMinted(msg.sender);

    //     require(phase == Phase.PRE_SALE, "Cannot premint - contract is not in premint phase");        
    //     require(_totalMinted() + allowedCount <= MAX_SUPPLY, "Not enough NFTs left!");
    //     require(_numberMinted(msg.sender) <= allowedCount, "This address cannot mint quantity requested");        

    //     _safeMint(msg.sender, allowedCount);
    // }

    function stQuantityMint() external payable callerIsUser nonReentrant {                
         uint allowedCount = 0;

        if (IERC721A(requiredOwnContract).balanceOf(msg.sender) >= 2) {
            allowedCount = 2;
        } else if (IERC721A(requiredOwnContract).balanceOf(msg.sender) == 1) {
            allowedCount = 1;
        }

        require(allowedCount > 0, 'MintZeroQuantity');

        _safeMint(msg.sender, allowedCount);
    }

    function whitelistMint(bytes32[] calldata _wlProof) external payable callerIsUser nonReentrant {                
        require(verifyWhiteList(_wlProof), "This address is not in the white list");        

        _safeMint(msg.sender, 1);
    }

    function bothlistsMint(bytes32[] calldata _wlProof, bytes32[] calldata _slProof) external payable callerIsUser nonReentrant { 
        uint allowedCount = 0;
    ...
    function tokenHoldersMint() external payable callerIsUser nonReentrant {
        ...
        require(_numberMinted(msg.sender) + MAX_TOKEN_HOLDERS_MINT_PER_WALLET <= MAX_TOKEN_HOLDERS_MINT_PER_WALLET, "This address cannot mint quantity requested");
    ...
    function publicMint() external payable callerIsUser nonReentrant {
        ...
        require(_numberMinted(msg.sender) + MAX_PUBLIC_PER_WALLET <= MAX_PUBLIC_PER_WALLET, "This address cannot mint quantity requested");
    ...
    function verifySpecialList(bytes32[] calldata merkleProof) internal view returns (bool) {
        require(specialListRoot.length > 0, 'special list root is empty');        
        return MerkleProof.verify(merkleProof, specialListRoot, keccak256(abi.encodePacked(msg.sender)));
    }
    ...
    *             OWNER FUNCTIONS            *
    ...
    function setRequiredOwnContract(address _contract) public onlyOwner nonReentrant {
    ...
    function setSlRoot(bytes32 _merkleRoot) external onlyOwner nonReentrant {
        specialListRoot = _merkleRoot;
    }
