...
contract Themintpass is ERC721URIStorage, Ownable {
    ...
    uint8 public constant maxWLMint = 3;
    ...
    function setWhiteList(address[] calldata addresses) external onlyOwner {
        for (uint256 i = 0; i < addresses.length; i++) {
            whiteList[addresses[i]] = maxWLMint;
        }
    }
    ...
    function createWLToken(uint8 numberOfTokens) external payable {
        ...
        for (uint8 i = 0; i < numberOfTokens; i++) {
            ...
            _setTokenURI(tokenCounter + i, baseURI);
        }
    }
    ...
    function avaibleToMint(address addr) public view returns (uint8) {
