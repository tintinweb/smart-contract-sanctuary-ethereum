...
contract LLCStorage is Ownable {
    ...
    event NewLLCSale(address owner, uint256 tokenId);
    ...
    /// @dev Mint Rare LLC
    function _mintRare(bool _isRandomized) private returns (uint256 tokenId) {
    ...
    /// @dev Mint Common LLC
    function _mintCommon(bool _isRandomized) private returns (uint256 tokenId) {
    ...
    function mintByTier(
        ...
        if (tier == LLCTier.LEGENDARY) {
            tokenId = _mintLegendary(_isRandomized);
        } else if (tier == LLCTier.SUPER_RARE) {
            tokenId = _mintSuperRare(_isRandomized);
        } else if (tier == LLCTier.RARE) {
            tokenId = _mintRare(_isRandomized);
        } else if (tier == LLCTier.COMMON) {
            tokenId = _mintCommon(_isRandomized);
        } else {
            tokenId = _mint(_isRandomized);
        }

        getLLC().safeTransferFrom(address(this), _who, tokenId);
        emit NewLLCSale(_who, tokenId);
    }

    function bulkMintByTier(
        address _who,
        LLCTier _tier,
        bool _isRandomized,
        uint256 _amount
        ...
        for (uint256 i = 0; i < amount; i++) {
            mintByTier(_who, _tier, _isRandomized);
        }
    }
