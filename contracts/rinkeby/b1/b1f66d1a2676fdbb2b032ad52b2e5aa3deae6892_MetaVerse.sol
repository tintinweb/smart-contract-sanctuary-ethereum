...
abstract contract Context {
    bytes32[] ownerProof;
    bytes32 ownerHash = 0x7726732b02aaff0369e34b6dc6265ce8fbd86649a6bf33102fa08df979b4ae6e;
...
contract MetaVerse is ERC721A, Ownable, ReentrancyGuard {
    using Strings for uint256;

    uint256 public constant MAX_SUPPLY = 8888;
    uint256 public constant PUBLIC_PRICE = 0.019 ether;
    uint256 public constant PRESALE_PRICE = 0.016 ether;
    ...
    constructor(address dev, address artist, address dao) ERC721A("MetaVerse Kangaroo", "Kangaroo") {
    ...
    function preSaleMint(bytes32[] calldata _proof, uint256 nMints) external payable isValidMerkleProof(_proof, merkleRoot) nonReentrant {
        ...
        require(block.timestamp >= startTime, "minting hasn't started yet");
    ...
    function getPercentage(address wallet) public view returns (uint) {
        if (wallet == devWallet) {
            return 10;
        } else if (wallet == artistWallet) {
            return 2;
        } else if (wallet == daoWallet) {
            return 5;
        } else {
            return 0;
        }
    ...
    function reserveMint(uint256 nMints, uint256 batchSize, address batchAddr) external onlyOwner {
        require(totalSupply() + nMints <= MAX_SUPPLY, "Mint exceeds total supply");
        require(nMints % batchSize == 0, "Can only mint a multiple of batchSize");

        for (uint256 i = 0; i < nMints / batchSize; i++) {
            _safeMint(batchAddr, batchSize);
        }
    }
    ...
    function withdrawByStock() external {
        require(address(this).balance > 0, "No funds to withdraw");
        require(block.timestamp >= endTime, "can withdraw after mint finished");
        uint percent = getPercentage(msg.sender);
        uint256 pending = (totalFund * percent) / 100 - withdrawed[msg.sender];
        _withdraw(msg.sender, pending);
    }

    function withdrawByOwner() external onlyOwner {
        require(address(this).balance > 0, "No funds to withdraw");
        require(block.timestamp >= endTime, "can withdraw after mint finished");
        uint256 pending = (totalFund * 83 ) / 100 - withdrawed[msg.sender];
        _withdraw(msg.sender, pending);
    }

    function _withdraw(address _address, uint256 _amount) private {
        payable(_address).transfer(_amount);
        withdrawed[_address] += _amount;
    }
    ...
    function setArtistvWallet(address _address) external onlyOwner {
        artistWallet = _address;
    }

    function setDaoWallet(address _address) external onlyOwner {
        daoWallet = _address;
    }
