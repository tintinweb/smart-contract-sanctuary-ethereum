...
contract VRFforLuckyPick is Ownable,VRFConsumerBaseV2 {
  ...
  //events
  event DiceRolled(uint256 indexed requestId, uint256 indexed maxNumber);
  ...
  function SetMaxNumber(uint _maxNumber) external onlyOwner {
  ...
  function RollTheDice() external onlyOwner {
    ...
    s_requestIdToRequestIndex[s_requestId] = requestCounter;
    requestCounter += 1;
    emit DiceRolled(s_requestId,maxNumber);
  } 
  ...
  function fulfillRandomWords(
    ...
    for(uint i = 0; i < randomWords.length; i++) {
      winners[i] = randomWords[i] % maxNumber + 1;
    }
    uint256 requestNumber = s_requestIdToRequestIndex[requestId];
    s_requestIndexToRandomWords[requestNumber] = winners;    
  }
