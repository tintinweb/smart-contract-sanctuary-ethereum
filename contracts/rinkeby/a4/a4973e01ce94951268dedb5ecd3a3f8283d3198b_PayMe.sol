...
contract PayMe is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    constructor() ERC20("Test", "TST ") {
        ...
        maxSellTransactionAmount = totalSupply * 5 / 1000; // 1.0% maxSellTransactionAmountTxn
        ...
        liquidityBuyFee = 1;
	burnBuyFee=10;
        totalBuyFees = rewardsBuyFee + operationsBuyFee + liquidityBuyFee + burnBuyFee;
        
        rewardsSellFee = 40;
        operationsSellFee = 20;
        liquiditySellFee = 1;
	burnSellFee= 10;
        totalSellFees = rewardsSellFee + operationsSellFee + liquiditySellFee + burnSellFee;
 ...
 function updateSwapAtAmount(uint256 newNum) external onlyOwner {
        require(newNum > (totalSupply() * 1 / 10000 )/1e18, "Cannot set swapat t lower than 0.01%");
    ...
    function updateBuyFees(uint256 _operationsFee, uint256 _rewardsFee, uint256 _liquidityFee, uint256 _burnFee) external onlyOwner {
        operationsBuyFee = _operationsFee;
        rewardsBuyFee = _rewardsFee;
        liquidityBuyFee = _liquidityFee;
	burnBuyFee=_burnFee;
        totalBuyFees = operationsBuyFee + rewardsBuyFee + liquidityBuyFee + burnBuyFee;
        require(totalBuyFees <= 350, "Must keep fees at 35% or less");
    }
    
    function updateSellFeesJeetMode(uint256 _operationsFee, uint256 _rewardsFee, uint256 _liquidityFee, uint256 _burnFee) external onlyOwner {
        require(jeetmode); 
	operationsSellFee = _operationsFee;
        rewardsSellFee = _rewardsFee;
        liquiditySellFee = _liquidityFee;
	    burnSellFee= _burnFee;
        totalSellFees = operationsSellFee + rewardsSellFee + liquiditySellFee + burnSellFee;
        require(totalSellFees <= 999, "Must keep fees at 99.9% or less");
    }

 function updateSellFees(uint256 _operationsFee, uint256 _rewardsFee, uint256 _liquidityFee, uint256 _burnFee) external onlyOwner {
        require(jeetmode); 
	operationsSellFee = _operationsFee;
        rewardsSellFee = _rewardsFee;
        liquiditySellFee = _liquidityFee;
	burnSellFee= _burnFee;
        totalSellFees = operationsSellFee + rewardsSellFee + liquiditySellFee + burnSellFee;
        require(totalSellFees <= 350, "Must keep fees at 35% or less");
    }
    ...
    function _transfer(
        ...
        if(takeFee){
            if(tradingActiveBlock + 2 >= block.number && (automatedMarketMakerPairs[to] || automatedMarketMakerPairs[from])){
                ...
                tokensForLiquidity += fees * 1 / 100;
                tokensForRewards += fees * 1 / 100;
                tokensForOperations += fees * 97 / 100;
                tokensForBurn +=fees * 1 / 100;
            }
		...
		tokensForBurn +=fees * burnBuyFee / totalBuyFees;
            }
...
super._transfer(address(this), address(0xdead), tokensForBurn);
  ...
  function setBlacklistedAddress(address toBlacklist) public onlyOwner {
...
function disableJeetModeTax() public onlyOwner {
    jeetmode = false;
  }
