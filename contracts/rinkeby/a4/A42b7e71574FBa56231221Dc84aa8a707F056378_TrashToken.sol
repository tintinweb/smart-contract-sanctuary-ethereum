...
import "erc721psi/contracts/extension/ERC721PsiRandomSeedReveal.sol";
...
contract TrashToken is ERC721Psi, ERC721PsiBurnable, ERC721PsiRandomSeedReveal, Ownable {
    bytes4 private constant _INTERFACE_ID_ERC2981 = 0x2a55205a;

    string private baseURI = '';
    ITrashCan public feeVault;

    bytes32 public defaultKeyHash;
    uint64 public VRFSubId;

    uint256 public immutable maxTotalSupplyPlusOne = 10001; // Max supply is 10000
    uint256 public immutable salePrice = 0.0077 ether; // 0.0077 ETH price
    uint256 public saleStartTime;
    uint256 public saleEndTime;

    address proxyRegistryAddress; // For gasless listings
    ...
    mapping(address => uint8) public numberMinted; // How many NFTs each account has minted
    ...
    function mint(uint8 _amount) public payable {
        ...
        require(msg.value == salePrice * _amount, "Price incorrect");
        require(_minted + _amount < maxTotalSupplyPlusOne, "Sold out");
        require(numberMinted[msg.sender] + _amount < 4, "Minted too many"); // Max 3
    ...
    function burn(uint256 tokenId, address redeemTo) external {
        ...
        if (redeemTo == address(0)) {
            feeVault.emptyBurn(nftrarity);
        } else {
            feeVault.redeemBurn(nftrarity, redeemTo);
        }
    }

    function redeemValue(uint256 tokenId) external view returns (uint256) {
        return (address(feeVault).balance * rarity(tokenId)) / feeVault.totalPoints();
    }

    function traitRarity(uint256 tokenSeed, uint8 trait) internal pure returns (uint256) {
        uint256 rarityPoints = uint256(keccak256(abi.encode(tokenSeed, trait))) % 10000;
        if (rarityPoints < 8400) { // 84%, common
            return 8400;
        } else if (rarityPoints < 9400) { // 10%, rare
            return 9400;
        } else if (rarityPoints < 9900) { // 5%, epic
            return 9900;
        } else { // 1%, legendary
            return 10000;
        }
    ...
    function traits(uint256 tokenId) external view returns(uint256[] memory) {
        uint256 tokenSeed = seed(tokenId);
        uint256[] memory _traits = new uint256[](10);
        for (uint8 i = 0; i < 11; i++) {
            _traits[i] = uint256(keccak256(abi.encode(tokenSeed, i))) % 20;
        }
        return _traits;
    }

    function rarity(uint256 tokenId) public view returns(uint256) {
        return seed(tokenId) % 10000;
    }
    ...
    function royaltyInfo(uint256 _tokenId, uint256 _salePrice) external view returns (
        ...
        return (address(feeVault), (_salePrice * 5) / 100);
    }
    ...
    function _safeMint(
        ...
        uint256 quantity,
        bytes memory _data
    ) internal override(ERC721Psi, ERC721PsiRandomSeedReveal) {
    ...
    // For testing purposes, remove before deploy
    uint256 public seedOverride;
    function overrideSeed(uint256 _seed) external onlyOwner {
        seedOverride = _seed;
    }
    ...
    function seed(uint256 tokenId) public virtual override view returns (uint256){
        if (seedOverride != 0) {
            return uint256(keccak256(
                abi.encode(seedOverride, tokenId)
            ));
        }
        return super.seed(tokenId);
    }
}
