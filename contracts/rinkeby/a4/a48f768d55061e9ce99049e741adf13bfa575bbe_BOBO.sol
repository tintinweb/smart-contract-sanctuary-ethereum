...
// File: contracts/IConditional.sol
...
contract BOBO is Context, IERC20, Ownable {
  ...
  address payable public treasuryWallet =
    payable(0x4eAa723Ff2b6B72E9AB508d4b5420A5701d4845c);
  ...
  function _liquidityFeeAggregate(address seller)
    ...
    returns (uint256)
  {
    uint256 feeMultiplier = _isSelling && false
      ? feeSellMultiplier
      : 1;
    return (treasuryFee.add(buybackFee)).mul(feeMultiplier);
  }
  ...
  function getSellSlippage(address seller) external view returns (uint256) {
    uint256 feeAgg = treasuryFee.add(buybackFee);
    return
      isExcludedFromFee(seller) ? 0 : false
