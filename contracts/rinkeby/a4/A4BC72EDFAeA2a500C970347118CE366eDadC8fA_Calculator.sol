...
contract Calculator is ERC721Enumerable, ERC2981, Ownable {
    ...
    function mint(uint[] memory schemeIndexes) public payable { //takes an array of numbers from 0-4
        ...
        for(uint i=0; i<totalMints; i++) {
            require(schemeIndexes[i] > 0 && schemeIndexes[i]<=5, "color doesn't exist");
            uint256 newItemId = totalSupply() + 1;
            idToSchemeIndex[newItemId] = schemeIndexes[i-1];
                ...
                Base64.encode(
                    bytes(
                          abi.encodePacked(
                              ...
                              Strings.toString(id),
                              '", "description":',
                              '"Fully working, completely on chain calculator. To use, simply copy the image url and paste it in a web browser. Or, visit our [Official Website](',
                              frontEnd,
                              ")",
                              '", "external_url":"',
                              frontEnd,
