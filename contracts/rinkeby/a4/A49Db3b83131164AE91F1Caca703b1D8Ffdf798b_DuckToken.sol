...
contract DuckToken is ERC20("duckToken", "duckToken"), Ownable, IDragonToken, Pausable {

	event ClaimedFuck(address indexed _account, uint256 _reward);
    ...
    function claimReward() external override {
        ...
        emit ClaimedFuck(msg.sender, claimableReward);
    }
    ...
    function _calculateRewards(address _account) internal view returns(uint256) {
        ...
        if (delta > 0) {
            ...
            */
            uint256 pendingBasic = dragons.balanceOf(_account) * (legionRate * delta / 10);
            uint256 pendingGenesis = genesis.balanceOf(_account) * (genesisRate * delta / 10);
            uint256 bonus = _calculateBonus(_account) * delta / 86400;

            return pendingBasic + pendingGenesis + bonus;
        }
        return 0;
    }

    function _calculateBonus(address _account) internal view returns(uint256) {
        uint256 curBalance = dragons.balanceOf(_account) + genesis.balanceOf(_account);
        if (curBalance >= 20)
            return 40 ether;
        if (curBalance >= 10)
            return 15 ether;
