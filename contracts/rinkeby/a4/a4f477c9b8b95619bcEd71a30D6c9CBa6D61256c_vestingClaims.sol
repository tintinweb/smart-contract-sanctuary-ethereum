...
import "./playersNFT.sol";


contract vestingClaims is Ownable, ReentrancyGuard {
    ...
    */
    ...
    constructor (address _nftAddress, address _matchContractAddress ,address token_) {
        ...
        slicePeriod = 3;
        timeUnit = 60;
        nftAddress = playersNFT(address(_nftAddress));
    ...
    */
    ...
    function claim(uint _nftId) public{
        ...
        require(getNftBalance(_msgSender())>0,"Caller doesn't have Nft balance");
...
contract MatchContract is Ownable {
    struct MatchData {
        ...
        uint256 tokenAmount;
        string location;
        uint8 team1CountryGoals;
        uint8 team2CountryGoals;
    ...
    function setMatchData(
        ...
        uint8 _team1CountryGoles,
        uint8 _team2CountryGoles
        ...
        matchesInfo[_matchId] = MatchData({
            ...
            location: _location,
            team1CountryGoals:_team1CountryGoles,
            team2CountryGoals:_team2CountryGoles
        });
    ...
    function setAllMatchData() internal{

        for (uint8 i = 1; i <= matchLength; i++) {
            getAllMatch.push(
                         AllMatchData({
        ...
        location:matchesInfo[i].location,
        team1CountryGoals :matchesInfo[i].team1CountryGoals,
        team2CountryGoals:matchesInfo[i].team2CountryGoals
        }) 
            );
        } 
...
contract playersNFT is Initializable, ERC721Upgradeable, ERC721URIStorageUpgradeable, PausableUpgradeable, OwnableUpgradeable, ERC721BurnableUpgradeable, UUPSUpgradeable {
    ...
    struct countryData{
        ...
        uint price;
        uint quantityLimit;
        uint countryTokenIndex;
    }

    mapping(uint256 => mapping(uint => countryData)) private countryInfo;
    mapping(string => address) public influencer;

    mapping(address=> uint[]) private nftIds;
    ...
    event NftSale(uint256 countryId,uint tokenId,address buyer,uint256 quantity,uint256 totalPrice);
    ...
    function initialize(string memory _name, string memory _symbol) initializer public {
        ...
        collectionSize = 32000;
        uint flag = 0;
        for(uint256 i=1;i<5;i++){
            for(uint256 j=1;j<9;j++){
                countryInfo[i][flag+1] = countryData ({
                    pId: i,
                    countryId: flag+1,
                    price: 10000000000000000,
                    quantityLimit: 10,
                    countryTokenIndex:flag*1000
                });
                flag++;
            }
        }
    ...
    function getPaymentWallet() public view returns(address _paymentWalletAddress) {
    return(paymentWallet); 
    }

    function setPriceByCountryId(uint256 _potId, uint256 _countryId,uint256 _price) public onlyOwnerAndAdmin{
        require((_potId==1 && _countryId>0 && _countryId<9) || (_potId==2 && _countryId>8 && _countryId<17) || (_potId==3 && _countryId>16 && _countryId<25) || (_potId==4 && _countryId>24 && _countryId<33), 'Invalid potId or countryId');
        countryInfo[_potId][_countryId].price = _price;
    }

    function setPotprice(uint256 _potId,uint256 _price) public onlyOwnerAndAdmin{
        require(_potId>0 && _potId<5 ,'Invalid PotId');       
        uint _startIndex = _potId==1 ? 1 : _potId == 2 ? 9 : _potId == 3 ? 17:_potId == 4 ? 25:0 ;
        for(uint i=_startIndex;i<= (_potId*8);i++){
            countryInfo[_potId][i].price = _price;
        }
    }

    function getPrice(uint256 _potId, uint256 _countryId) public view returns(uint256 _price){
        require((_potId==1 && _countryId>0 && _countryId<9) || (_potId==2 && _countryId>8 && _countryId<17) || (_potId==3 && _countryId>16 && _countryId<25) || (_potId==4 && _countryId>24 && _countryId<33), 'Invalid potId or countryId');
        return (countryInfo[_potId][_countryId].price);
    }

    function setQuantityLimitOfCountry(uint256 _potId,uint256 _countryId, uint8 _quantity) public onlyOwnerAndAdmin{
        countryInfo[_potId][_countryId].quantityLimit = _quantity;
    }

    function getQuantityLimitOfCountry(uint256 _potId,uint256 _countryId) public view returns(uint _price){
        return(countryInfo[_potId][_countryId].quantityLimit);
    }
    
    function getNftIdsByWallet(address _walletAddress) public view returns(uint[] memory _nftIds){
       return nftIds[_walletAddress];
    ...
    function BuyNFT(uint256 _potId, uint256 _countryId,uint256 _quantity)public payable {

        require(_quantity < getQuantityLimitOfCountry(_potId,_countryId) , 'maximum quantity is 5');
        require((_potId==1 && _countryId>0 && _countryId<9) || (_potId==2 && _countryId>8 && _countryId<17) || (_potId==3 && _countryId>16 && _countryId<25) || (_potId==4 && _countryId>24 && _countryId<33), 'Invalid potId or countryId');
        require(msg.value >= _quantity*getPrice(_potId,_countryId), "Insufficient funds to redeem");
        
       require(countryInfo[_potId][_countryId].countryTokenIndex +_quantity <= _countryId*1000,'Insufficient token amount to purchase');
        
        for (uint256 i = 0; i < _quantity; i++) {
            countryInfo[_potId][_countryId].countryTokenIndex = countryInfo[_potId][_countryId].countryTokenIndex+1;
            nftIds[_msgSender()].push(countryInfo[_potId][_countryId].countryTokenIndex);
            countryId[countryInfo[_potId][_countryId].countryTokenIndex] = _countryId;
            _safeMint( _msgSender(),  countryInfo[_potId][_countryId].countryTokenIndex);
        }

        address payable _paymentWallet= payable(getPaymentWallet());
        _paymentWallet.transfer(msg.value);

        emit NftSale(_countryId,countryInfo[_potId][_countryId].countryTokenIndex,_msgSender(),_quantity,_quantity*getPrice(_potId,_countryId));
