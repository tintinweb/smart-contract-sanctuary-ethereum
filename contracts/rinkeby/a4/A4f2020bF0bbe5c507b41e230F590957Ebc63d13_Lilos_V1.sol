...
// LILOS is a P2P NFT Rent Contract. User can rent 
...
// as collateral. Stable coins collateralized lease will 
// be implemented in Lilos_StableCoin_V1 contract. And 
// NFT collateralized lease will be implemented in 
// Lilos_NFT_V1 so as to save gas. LILO transactions
// will be realized in Lilos_V2 contract.
...
// Thus, please do not lease for shorting or your collateral may be withdrawed. 
...
contract Lilos_V1 {
	...
	// Platform charges 5% from lessor's rental PROFIT(rent_value). 
	...
	function listToken(
		...
		require(rent_value_ > 0, "Rent value should be larger than zero.");
	...
	function leaseIn(uint256 listingId_) external payable {
		...
		require(msg.value == (listingItem.collateral_value.add(listingItem.rent_value)), "msg.value dose not match the total spending.");

		IERC721(listingItem.collection).transferFrom(listingItem.lessor, msg.sender, listingItem.tokenId);
		//(bool success,)
		payable(listingItem.lessor).transfer(listingItem.rent_value.mul(platformFeeRate).div(100));
...
// 有getItemByCollctionAndTokenId了
	...
	// 	return _listingItems[0].rent_value.mul(platformFeeRate.div(100)).add(99);
	// }
