...
contract animalMain is  ERC721,Ownable {
    ...
    function createAnimal() public payable{
        ...
        if(rand<40) _roletype = "Ant";
        else if (rand<50) _roletype = "Cat";
        else if (rand<80) _roletype = "Dog";
        else if (rand<90) _roletype = "Dragon";
        else _roletype = "Ant";
           
        uint id = animals.push(animal(_dna, uint8(_star), _roletype)) - 1;  //將此animal的數量
        animalToOwner[id] = msg.sender; // 每隻動物都有一個獨特編號，呼叫mapping，得到相對應的主人 轉移擁有權 
        ownerAnimalCount[msg.sender]++; //回傳某帳號底下的動物數量  擁有者的個數＋1
        emit Create(id, _dna, _star, _roletype);
    }
