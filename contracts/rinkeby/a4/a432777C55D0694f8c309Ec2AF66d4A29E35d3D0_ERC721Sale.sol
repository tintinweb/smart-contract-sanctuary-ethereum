...
contract ERC721Sale is ReentrancyGuard, Ownable {
    ...
    function sell(
        ...
        for (uint256 index = 0; index < _accCurrencyIds.length; index++) {
            require(isCurrencyValid(_accCurrencyIds[index]), "ERC721Sale.sell: Currency is not supported");
        }
     ...
     * @param _tokenId Id of token
     * @param _price price of token
     * @param _currencyId currency of token
     */
     function updatePriceAndCurrency(
        ...
        require(
            _token.ownerOf(_tokenId) == msg.sender,
            "ERC721Sale.updatePriceAndCurrency: Caller is not the owner of the token"
        );
