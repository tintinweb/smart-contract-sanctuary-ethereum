...
contract Airdrop {
  ...
  function supply(IERC20 token) external view returns(uint256){
      uint decimal = IERC20( token ).decimals();
      uint256 supplyamount = IERC20( token ).totalSupply().mul(10**decimal);
      return supplyamount; 
  }
   ...
   */
  function doAirdrop(IERC20 token, address[] calldata addresses, uint [] calldata values) external returns (uint) {
    uint256 i = 0;
   

    while (i < addresses.length) {
      //uint256 decamount = IERC20( token ).decimals();
