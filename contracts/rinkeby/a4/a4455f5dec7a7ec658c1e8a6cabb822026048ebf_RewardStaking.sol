...
contract RewardStaking is Initializable, UUPSUpgradeable, OwnableUpgradeable, PausableUpgradeable {
    ...
    function enter(uint256 purseAmount) external returns (bool success) {
        ...
        if (totalXPurse <= 0 || totalPurse <= 0) {
            ...
            uint256 xPurseAmount = purseAmount.mul(totalXPurse).div(totalPurse);
            userInfo[msg.sender].receiptToken += xPurseAmount;
            _totalReceiptSupply += xPurseAmount;
        }
        userInfo[msg.sender].stakedToken += purseAmount;
        emit Transfer(msg.sender, address(this), purseAmount);
        purseToken.transferFrom(msg.sender, address(this), purseAmount);
        return true;
    }

    function leave(uint256 purseAmount) external returns (bool success){ 
        uint256 userPurseAmount = getPurseAmount(msg.sender);
        require(userPurseAmount >= purseAmount, "Insufficient Purse Token");
        
        uint256 totalXPurse = totalReceiptSupply();
        uint xPurseAmount = purseAmount.mul(totalXPurse).div(purseToken.balanceOf(address(this)));
        require(xPurseAmount <= userInfo[msg.sender].receiptToken, "Insufficient Receipt Token");
        
        userInfo[msg.sender].receiptToken -= xPurseAmount;
        _totalReceiptSupply -= xPurseAmount;

        userInfo[msg.sender].withdrawnToken += purseAmount;
        emit Transfer(address(this), msg.sender, purseAmount);
        purseToken.transfer(msg.sender, purseAmount);
        return true;
    }

    function getPurseAmount(address purseOwner) public view returns (uint256){  
        uint256 totalXPurse = totalReceiptSupply();
    ...
    function getReceiptAmount(address receiptOwner) external view returns (uint256) {
        return userInfo[receiptOwner].receiptToken;
    }
