...
interface IHouse {
    function placeBet(address player, uint amount, bool isBonus, uint rollUnder, uint modulo) payable external;
...
contract Dice is ReentrancyGuard,Manager, IGame{
    ...
    function placeBet(uint betChoice, uint bonus, uint note) external payable nonReentrant {
        ...
        house.placeBet{value: msg.value}(msg.sender,amount, isBonus, rollUnder, MODULO);
    ...
    function _settleBet(uint betId, uint256 randomNumber) private nonReentrant {
        ...
        uint winnableAmount = house.amountToWinnableAmount(amount, rollUnder,MODULO);
    ...
    function refundBet(uint betId) external nonReentrant {
        ...
        uint winnableAmount = house.amountToWinnableAmount(amount, bet.rollUnder,MODULO);
        uint bettedAmount = house.amountToBettableAmountConverter(amount);
