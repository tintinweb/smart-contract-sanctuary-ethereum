...
contract PurchaseHook {
    ...
    constructor(address _lock, address _signer) {
        signer = _signer;
        lock = _lock;
    }

    function changeSigner(address _signer) public {
        require(
    ...
    function keyPurchasePrice(
        ...
        bytes32 secretMessage = ECDSA.toEthSignedMessageHash(
            keccak256(abi.encode(recipient))
        );
        address codeAddress = ECDSA.recover(secretMessage, signature);
        require(codeAddress == signer, "WRONG_SIGNATURE");
