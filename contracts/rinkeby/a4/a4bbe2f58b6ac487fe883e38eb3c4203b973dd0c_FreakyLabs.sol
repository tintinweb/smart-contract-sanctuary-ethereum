...
contract FreakyLabs is ERC721A, Ownable {
    ...
    function freakyLabExecMint(bytes32 [] calldata _merkleProof) external payable {
        ...
        require(msg.value == MINT_PRICE * 2, "Incorrect ETH sent to mint");
    ...
    function adminMint(uint _quantity) external onlyOwner {
        require(totalSupply() + _quantity <= MAX_SUPPLY, "Not enough tokens left to mint");
        _safeMint(msg.sender, _quantity);
    }
    ...
    function updateLabExecMerkleRoot(bytes2 _new_root) external onlyOwner {
