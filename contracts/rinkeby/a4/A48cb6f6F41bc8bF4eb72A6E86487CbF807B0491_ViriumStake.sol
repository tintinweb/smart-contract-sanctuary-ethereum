...
contract ViriumStake is Initializable, UUPSUpgradeable, AccessControlUpgradeable {
    ...
    function getRewardRate(uint256 t2, uint256 t1) public view returns (uint256){
        uint256 duration = t2 - deployTime;
        if (duration / FIFTY_YEARS > 0) {
            return 0;
        }
        return g(duration) - g(t1 - deployTime);
    }
    ...
    function earned() public view returns (uint256) {
        ...
        uint256 lastUpdateTime = userLastUpdateTime[msg.sender];
        if (currentStatus == Status.Paused) {
            lastUpdateTime = pausedTime;
        }
        if (lastUpdateTime == 0 || totalPledge == 0) {
            return 0;
        }
        return _userShare[msg.sender] * getRewardRate(block.timestamp, lastUpdateTime) / totalPledge + rewards[msg.sender];
    }
    ...
    function earnedPerDay() external view returns (uint256){
        return _userShare[msg.sender] * getRewardRate(block.timestamp + 1 days, block.timestamp) / totalPledge;
    }
