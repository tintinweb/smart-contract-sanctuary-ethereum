...
/**
    ...
    @author Cryptoflu.
 */
contract SingleLotteryWinner is Ownable, VRFConsumerBase {
    ...
    struct Lottery {
        ...
        address winner;                       // array of winner's addresses
    }
    ...
    function addLottery(
        ...
        require(_lotteryAwardFee < 100 && _lotteryEntryPrice > 0 && _lotteryStartAt > block.timestamp 
                    && _lotteryDuration > 0 && _maxParticipantsCount > 0, "incorrect param(s)");
        if (currentLotteryId.current() > 0){
            ...
            require(prevLottery.winner != address(0), "previous lottery has not winners");
        }
    ...
    */
    ...
    function finish() public onlyOwner {
        ...
        require(lottery.participantsCount <= 1, "valid participants count");     
    ...
    function awardWinners() public onlyOwner {
        Lottery storage lottery = lotteries[currentLotteryId.current()];
        require(block.timestamp >= lottery.lotteryStartAt + lottery.lotteryDuration, "lottery is active");
        require(lottery.participantsCount > 1, "not enough participants");
        lottery.lotteryWinnerAward = (100 - lottery.lotteryAwardFee) * lottery.currentLot / 100;   // calculate winner award considering the fee
        getRandomNumber();
    }
    ...
    function payToWinner(uint256 randomness) private {
        Lottery storage lottery = lotteries[currentLotteryId.current()];
        uint256 randomResult = randomness % lottery.participantsCount;
        lottery.winner = lottery.participants[randomResult];
    ...
    function withdrawFund(address _recipient, uint256 _amount) public onlyOwner {
        // require() // вызов только при неактивных лотереях 
