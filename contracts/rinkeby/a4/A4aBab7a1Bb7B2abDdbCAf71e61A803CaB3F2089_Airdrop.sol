...
contract Airdrop is Ownable {
    ...
    /// @dev Available blocks in 7 days  => 5 * 60 * 24 * 7
    uint256 public constant AIRDROP_PERIOD = 50400;
    ...
    function claim(
        ...
        require(
            verifyProof(_merkleProof, getMerkleRoot(), getNode(_who, _amount)),
            "Invalid claim"
        );
    ...
    modifier onlyInOngoingAirdrop() {
        require(
            block.number >= getAirdropStartDate(),
            "Airdrop is not started yet"
        );
        require(block.number <= getAirdropEndDate(), "Airdrop is ended");
        _;
    }

    modifier onlyAfterAirdrop() {
        require(
            block.number > getAirdropEndDate(),
            "Airdrop is not ended yet"
        );
        _;
    }
     ...
     */
    function getNode(address _who, uint256 _amount)
        private
        pure
        returns (bytes32)
    {
