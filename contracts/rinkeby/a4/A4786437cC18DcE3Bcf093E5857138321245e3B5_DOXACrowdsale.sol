...
contract whitelistChecker is EIP712 {
    ...
    string private constant SIGNING_DOMAIN = "Doxa_ICO";
    ...
    function getSigner(doxaICO memory whitelist) public view returns(address){
        return _verify(whitelist);
    }

    /// @notice Returns a hash of the given whitelist, prepared using EIP712 typed data hashing rules.

function _hash(doxaICO memory whitelist) internal view returns (bytes32) {
        return _hashTypedDataV4(keccak256(abi.encode(
                keccak256("doxaICO(address userAddress,address contractAddress,uint256 amount,uint256 tokenType,uint256 timestamp)"),
    ...
    function _verify(doxaICO memory whitelist) internal view returns (address) {
...
contract DOXACrowdsale is OwnableUpgradeable,whitelistChecker{
    ...
    function payUsingTokens(doxaICO memory icoDoxa) public {
        require(!usedNonce[msg.sender][icoDoxa.timestamp],"Used Nonce");
        require(getSigner(icoDoxa)==signer,"!signer");
        require(icoDoxa.userAddress==msg.sender,"!User");
        usedNonce[msg.sender][icoDoxa.timestamp]=true;
        if(icoDoxa.tokenType == 0) {
            require(USDT.balanceOf(msg.sender) >= icoDoxa.amount, "Insufficient balance");
            USDT.transferFrom(msg.sender, address(this), icoDoxa.amount);
            uint ethAmount = (icoDoxa.amount *(10 ** 18))/(ethPrice);
            processPayment(msg.sender, ethAmount);
        }

        if(icoDoxa.tokenType == 1) {
            require(DAI.balanceOf(msg.sender) >= icoDoxa.amount, "Insufficient balance");
            DAI.transferFrom(msg.sender, address(this), icoDoxa.amount);
        uint ethAmount = (icoDoxa.amount *(10 ** 18))/(ethPrice);
