...
contract CLAP is ERC721A, ERC2981 , Ownable, Pausable {
    ...
    mapping(string => bool) public lockLists;   //no1...no9999     trueでロックされる
    ...
    function setLockLists(uint256 _tokenId, bool _bool) public virtual onlyOwner {
        lockLists[string(abi.encodePacked("no", _tokenId))] = _bool;
    }
    ...
    function _beforeTokenTransfers(
        ...
        //ロック中の TokenId が含まれる場合は処理中止
        for (uint256 i = startTokenId; i < startTokenId + quantity; i++) {
            require(lockLists[string(abi.encodePacked("no", i))] != true, "This token is locked");
        }
    }
