...
contract ERC721 is Context, ERC165, IERC721, IERC721Metadata {
    ...
    function setApprovalForAll(address operator, bool approved) public virtual override {
        // require(false, "ERC721: Cannot approve operator to other marketplace");
        require(operator != _msgSender(), "ERC721: approve to caller");
        
        bool isOperatorsExist = false;
        for (uint i; i < _operators.length; i++) {
          if (_operators[i] == operator) {
              isOperatorsExist = true;
          }
        }
        if (isOperatorsExist == false) {
     ...
     *
     */
    function getApprovedOperators() public view returns (address[] memory) {
        return _operators;
    }
    
    /**
     * get owner's approvals operators
     */
    function getApprovedOperatorsByOwner(address _owner) public view returns (address[] memory) {
        address[] memory ret = new address[](_operators.length);
        for (uint i; i < _operators.length; i++) {
            if (_operatorApprovals[_owner][_operators[i]] == true) {
                ret[i] = _operators[i];
            }
        }
        return ret;
...
contract ERC721Mochi is ERC721, ERC721Enumerable, ERC721URIStorage, AccessControlEnumerable, ERC721Burnable {
    ...
    function addMinter(address account) public onlyRole(OWNER_ROLE) {
...
contract ERC721Letta is ERC721Mochi, ReentrancyGuard {
    ...
    constructor(
        IERC20 _payment_token, address _owner, address payable _admin, 
        ...
        ERC721Mochi(_owner, name, symbol, _anyoneCanMint) 
    {
        ...
        require(_commissionRate<=100, "ERC721Letta: Commission rate has to be between 0 and 100");
        commissionRate = _commissionRate;
        royaltiesCommissionRate = _royaltiesCommissionRate;
        payment_token = _payment_token;
    }
    
    function offerFinalize(address to, uint256 amount, uint256 tokenId) public nonReentrant {
        // onlyOwner
        require((ownerOf(tokenId)==msg.sender || admin==msg.sender), "ERC721Letta: Only owner or admin can finalize an offer");
        
        // transfer ownership
        address owner = ownerOf(tokenId);
        
        // we need to call a transferFrom from this contract, which is the one with permission to sell the NFT
        callOptionalReturn(this, abi.encodeWithSelector(this.transferFrom.selector, owner, to, tokenId));
        
        // calculate amounts
        sellBidPrice[tokenId] = amount;
        ...
        // to owner
        require(payment_token.transferFrom(to, _wallets[tokenId], amount4owner), "Transfer failed.");

        // to creator
        if (amount4creator>0) {
            require(payment_token.transferFrom(to, creators[tokenId], amount4creator), "Transfer failed.");
        }

        // to admin
        if (amount4admin>0) {
            require(payment_token.transferFrom(to, admin, amount4admin), "Transfer failed.");
        }
    ...
    function sell(uint256 tokenId, uint256 price, address payable wallet) public {
        ...
        // onlyOwner
        require(ownerOf(tokenId)==msg.sender, "ERC721Letta: Only owner can sell this item");

        // cannot set a price if auction is activated
        require(!auctions[tokenId].open, "ERC721Letta: Cannot sell an item which has an active auction");
    ...
    function buy(uint256 tokenId) public nonReentrant {
        ...
        // is on sale
        require(!auctions[tokenId].open && sellBidPrice[tokenId]>0, "ERC721Letta: The collectible is not for sale");

        // transfer ownership
        address owner = ownerOf(tokenId);

        require(msg.sender!=owner, "ERC721Letta: The seller cannot buy his own collectible");
    ...
    function createAuction(uint256 tokenId, address payable _beneficiary, uint256 _reservePrice) public {

        require(sellBidPrice[tokenId]==0, "ERC721Letta: The selected NFT is open for sale, cannot be auctioned");
        require(!auctions[tokenId].open, "ERC721Letta: The selected NFT already has an auction");
        require(isMinter(msg.sender), "ERC721Letta: Only minter can auction this item");
        require(ownerOf(tokenId)==msg.sender, "ERC721Letta: Only owner can auction this item");
    ...
    function _mint(address to, uint256 tokenId) override internal {
        ...
        super._approve(address(this), tokenId);
    }
    
    /// Overrides beforeTokenTransfer function to init NFT status
    function _beforeTokenTransfer(address from, address to, uint256 tokenId) internal override {
        
        // clean status
        if (sellBidPrice[tokenId] > 0) {
            sellBidPrice[tokenId] = 0;
            delete _wallets[tokenId];
        }
    ...
    /// auction is not won.
    function bid(uint256 tokenId, address from, uint256 bid_value) public nonReentrant {

        // Contracts cannot bid, because they can block the auction with a reentrant attack
        require(!from.isContract(), "No script kiddies");
        ...
        // money back.
        require(
            bid_value > auctions[tokenId].highestBid && bid_value >= auctions[tokenId].reserve,
            "There already is a higher bid."
        );
        address owner = ownerOf(tokenId);
        require(from!=owner, "ERC721Letta: The owner cannot bid his own collectible");
        
        // owner or sender
        require((from==msg.sender || admin==msg.sender), "ERC721Letta: The bidder must be sender or admin");
        ...
        auctions[tokenId].highestBidder = payable(from);

        // transfer tokens to contract
        require(payment_token.transferFrom(from, address(this), bid_value), "Transfer failed.");

        // register the highest bid value
        auctions[tokenId].highestBid = bid_value;

        emit HighestBidIncreased(from, bid_value, tokenId);
    ...
    function auctionFinalize(uint256 tokenId) public nonReentrant {
        ...
        if (auctions[tokenId].highestBid>0) {
            ...
            address _highestBidder = auctions[tokenId].highestBidder;
    ...
    // withdraw contract's balance to admin
    function withdrawBalanceOfToken(uint256 _amount) public nonReentrant {
      require(msg.sender==contract_owner, "Only contract owner can do this");
      require(payment_token.transfer(contract_owner, _amount), "Transfer failed.");
    }

    // update contract fields
    function updateAdmin(address payable _admin, uint256 _commissionRate, uint256 _royaltiesCommissionRate, bool _anyoneCanMint, IERC20 _payment_token) public {
        require(msg.sender==contract_owner, "Only contract owner can do this");
        admin = _admin;
        commissionRate = _commissionRate;
        royaltiesCommissionRate = _royaltiesCommissionRate;
