...
contract GnarDonksNFTMock is GnarDonksNFT {
    ...
    function mintWithUSD(
        ...
        uint256 tokenId,
        address _stableCoin
    ) external override validAddress(_stableCoin) nonReentrant {
    ...
    ERC2981
{
    ...
    // TiemStamp per Tier/Id Limits
    uint256[4] public TS_TIER_ID_LIMITS; // [0] = Tier 2, [1] = Tier 3, [2] = Tier 4, [3] = Tier 5
    // Limits Amounts of Tokens per Period & Tier
    uint256[4] public LIMITS_TIER_ID; // [0] = Tier 2, [1] = Tier 3, [2] = Tier 4, [3] = Tier 5
    // Tier Price Limits
    uint256[5] public TIER_PRICE;
    // Slippage Limits
    ...
    // Payment Splitter Smart Contract Address
    address private paymentSplitterAddress;
    // Declare the priceFeed as an Aggregator Interface
    /** TODO: Before to Deploy in Mainnet must be eliminate virtual modifier */
        ...
        )
    {
        ...
        paymentSplitterAddress = _paymentContract;
        // Launch TimeStamp
        launchTs = _launchTs;
        // Tier Price Limits
        TIER_PRICE = [5000, 10000, 25000, 150000, 250000]; // [Tier 2, Tier 3, Tier 4, Tier 5, Tier 6]
        // TimeStamp Period Limits per Tier
        TS_TIER_ID_LIMITS = [31560000, 31560000, 31560000, 63120000]; // [Tier 2, Tier 3, Tier 4, Tier 5]
        // Limits Amounts of Tokens per Period & Tier
        LIMITS_TIER_ID = [250, 100, 50, 15]; // [Tier 2, Tier 3, Tier 4, Tier 5]
        // Slippage Limits
        ...
        tiers[2] =
            2500 |
            (0 << 16) |
            (TS_TIER_ID_LIMITS[0] << 32) |
            (LIMITS_TIER_ID[0] << 64); // 250 per yer
        tiers[3] =
            1000 |
            (0 << 16) |
            (TS_TIER_ID_LIMITS[1] << 32) |
            (LIMITS_TIER_ID[1] << 64); // 100 per yer
        tiers[4] =
            500 |
            (0 << 16) |
            (TS_TIER_ID_LIMITS[2] << 32) |
            (LIMITS_TIER_ID[2] << 64); // 50 per yer
        tiers[5] =
            75 |
            (0 << 16) |
            (TS_TIER_ID_LIMITS[3] << 32) |
            (LIMITS_TIER_ID[3] << 64); // every 2 years
        tiers[6] = 10;
    }
    ...
    function mint(address to, uint256 tokenId) external payable nonReentrant {
        ...
        beforeMintOfTier(tierOf(tokenId));
        // Get the price of the token
        uint256 price = mintTierPriceInETH(tierOf(tokenId));
        ...
        (bool success, ) = address(paymentSplitterAddress).call{value: balance}(
            ""
        );
        require(
            success,
    ...
    function mintWithUSD(
        ...
        uint256 tokenId,
        address _stableCoin
    ) external virtual validAddress(_stableCoin) nonReentrant whenNotPaused {
        require(_stableCoin.isContract(), "Invalid stable coin address");
        require(
    ...
    function mintTierPriceInETH(uint256 tier) public view returns (uint256) {
        if (tier == 1) {
    ...
    function tierPriceInETH(uint256 tier) public view returns (uint256) {
        if (tier == 1) {
            ...
            return mulDiv(_usdToEth(tierPriceInUSD(2)), slipETH, 1000); // the price is 2% more the the price
        } else if (tier == 3) {
            return mulDiv(_usdToEth(tierPriceInUSD(3)), slipETH, 1000); // the price is 2% more the the price
        } else if (tier == 4) {
            return mulDiv(_usdToEth(tierPriceInUSD(4)), slipETH, 1000); // the price is 2% more the the price
        } else if (tier == 5) {
            return mulDiv(_usdToEth(tierPriceInUSD(5)), slipETH, 1000); // the price is 2% more the the price
        } else {
            return mulDiv(_usdToEth(tierPriceInUSD(6)), slipETH, 1000); // the price is 2% more the the price
        }
    }
    ...
    function _beforeTokenTransfers(
        ...
        require(!paused(), "ERC721Pausable: can't token transfer while paused");
     ...
     * @param newTierPrice Arrays of Tier USD Price in Decimals
     */
    function setTierPrice(uint256[5] calldata newTierPrice) external onlyOwner {
        for (uint256 i = 0; i < newTierPrice.length; i++) {
            require(
                newTierPrice[i] > 0,
                "ERC721: Tier Price must be greater than 0"
            );
            TIER_PRICE[i] = newTierPrice[i];
        }
    }

    /**
     * @dev Method for Setting the TimeStamps of Each Tier of TokenIds
     * @param TSLimits Arrays of TimeStamp range of Period to Mint an Specific Amount of Tokens, per Tier
     * @dev [Tier 2, Tier 3, Tier 4, Tier 5]
     */
    function setTSLimits(uint256[4] calldata TSLimits) external onlyOwner {
        for (uint256 i = 0; i < 4; i++) {
            require(
                TSLimits[i] > 0,
                "ERC721: TimeStamp must be greater than 0"
            );
            TS_TIER_ID_LIMITS[i] = TSLimits[i];
        }
    }

    /**
     * @dev Method for Setting the Amount of Token can Minted per Period/Tier
     * @param AmountLimits Arrays of Token can minted per Period/Tier
     * @dev [Tier 2, Tier 3, Tier 4, Tier 5]
     */
    function setAmountLimitsPerPeriod(uint256[4] calldata AmountLimits)
        external
        onlyOwner
    {
        for (uint256 i = 0; i < 4; i++) {
            require(
                AmountLimits[i] > 0,
                "ERC721: Amount Limits per Period&Tier must be greater than 0"
            );
            LIMITS_TIER_ID[i] = AmountLimits[i];
        }
    }
    ...
    /**
     * @dev Method for Setting the Payment Splitter Smart Contract
     * @param paymentSplitterContract Address of the PaymentSplitter Contract
     */
    function setPaymentSplitterContract(address paymentSplitterContract)
        external
        onlyOwner
    {
        require(
            paymentSplitterContract != address(0) &&
                paymentSplitterContract.isContract(),
            "ERC721: Payment Splitter Contract is not valid"
        );
        paymentSplitterAddress = paymentSplitterContract;
    }
}
...
contract Claimable is Ownable {
     ...
     * @param value The amount of tokens
     */
    event ValueSent(address indexed receiver, uint256 value);
