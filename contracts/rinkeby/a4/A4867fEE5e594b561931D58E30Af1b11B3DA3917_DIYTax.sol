...
contract DIYTax is AuthorizableU {
    ...
    function updateTaxFeeById(uint8 index, string memory name, uint16 rate) public onlyOwner {
        if (index == 255) {
            taxFees.push(TaxFee(name, rate));
        }
        taxFees[index] = TaxFee(name, rate);
        calcTotalFeeRate();
    }
