...
contract Marketplace is ReentrancyGuard, AccessControl {
    ...
    function itemNotSoldExist(uint256 itemId) internal view returns (bool) {
        return _items[itemId].owner == address(0);
    }
    ...
    function makeBid(
        ...
        if(item.bidsCount >= _minBidAmount){ // TODO: Check behaviour, if we need not to finish here
            finishAuction(itemId, msg.sender);
        }
    }
    ...
    function getSaleItems() public view returns(Item[] memory) {
        ...
        uint256 unsoldItems = totalItems - _itemsSold.current() - 1;
        uint256 index = 0;
...
contract DHeroes is ERC721, Ownable, IERC721Receiver {
    ...
    constructor(
        address marketplace
    ) ERC721 ("DOTA 2 Heroes", "DOTA2_HEROES") {
        _marketplace = marketplace;
        // setApprovalForAll(_marketplace, true); // разрешить маркетплейсу управлять всеми токенами
    }
    ...
    function mint(
        ...
        require(msg.sender == _marketplace, "only marketplace can mint");
