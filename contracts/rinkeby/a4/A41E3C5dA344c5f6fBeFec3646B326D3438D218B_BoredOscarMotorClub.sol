    ...
    ERC2981
{
    string public baseURI =
        "https://bafybeiefftaklbljwzzlbxgvrj2il3a3rlviatthrvvwikqeoi2kvoypdu.ipfs.nftstorage.link/metadata/";
    ...
    function allowance(
        ...
        return stage.allowances[addr];
    }

    /**
     * ステージごとのユーザ呼び出しによる購入ミント
     */
    function saleStageMint(uint8 quantity)
        external
        payable
        nonReentrant
        onlyEOA
        whenNotPaused
    {
        ...
        if (stage.useList) {
            require(allowance(stageNum, msg.sender) >= quantity, "Requested quantity is over allowance for account");
            stage.allowances[msg.sender] -= quantity;
        } else {
    ...
    function setSaleStages(
        ...
        for (uint8 i = 0; i < priceWeiVals.length; i++) {
            if (i > 1) {
    ...
    function setAllowances(
        ...
        require(addresses.length == quantities.length, "Mismatched parameter lengths");
        require(stageNum < stages.length, "Invalid stage number");
        SaleStage storage stage = stages[stageNum];
        for (uint256 i = 0; i < addresses.length; i++) {
            stage.allowances[addresses[i]] = quantities[i];
        }
    }
