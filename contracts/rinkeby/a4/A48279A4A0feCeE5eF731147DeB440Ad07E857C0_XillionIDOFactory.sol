...
contract XillionIDOFactory is OwnablePausable, ReentrancyGuard {
     ...
     */ 
    function createVotingForIDO(uint256 targetVotes_, uint256 minVotingValue_, uint256 votingDuration_) external onlyModerator returns(uint) {
         uint votingId = votingFactory.createVoting(targetVotes_, minVotingValue_, votingDuration_);
         return votingId;
    }
...
contract XillionIDO is XillionIDOCommon {
     ...
     * @param votingDuration_ How long voting will 
     */ 
    function createVoting(uint256 targetVotes_, uint256 minVotingValue_, uint256 votingDuration_) public {
         onlyModerator();
         votingId = factory.createVotingForIDO(targetVotes_, minVotingValue_, votingDuration_);
    }
...
contract XillionVotingFactory is OwnablePausable, ReentrancyGuard {
    ...
    function createVoting(uint256 targetVotes_, uint256 minVotingValue_, uint256 votingDuration_) external onlyIDOFactory returns(uint256) {
    ...
    function vote(uint256 votingId_, uint256 votesAmount_) external whenNotPaused nonReentrant {
        ...
        require(block.timestamp <= block.timestamp + voting.votingDuration, "This voting is over");
        require(userBalance >= votesValueInXil, "You don't have enough votes");
        require(votesValueInXil <= voting.targetVotes, "You can't vote more than target");
