...
contract NFT is ERC721, Ownable, ReentrancyGuard  {
  ...
  uint private mintPrice = 0.08 ether;
  ...
  /// @dev Set addresses in allow-list 
  function addAllowList(address[] calldata addresses, uint8 numAllowedToMint) external onlyOwner {
  ...
  function reserveNFTs(uint8 amount) public onlyOwner {
  ...
  /// @dev Mint multiple NFTs for people on the allow-list
  function mintAllowList(uint8 amount) external payable {
    ...
    require(msg.value >= mintPrice.mul(amount), "Not enough ether to purchase NFTs");
  ...
  function mintNFTs(uint8 amount) public payable {
    uint256 tokenId = _tokenIds.current();
