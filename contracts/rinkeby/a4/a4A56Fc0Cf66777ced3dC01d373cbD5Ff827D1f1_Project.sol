...
contract Project is ContextUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function publish(ProjectInput memory _projectInput, SaleInput[] memory _saleInputs) external payable {
        ...
        if (isCreateNewToken) {
            token = (osbFactory.create(_projectInput.isSingle, _projectInput.tokenName, _projectInput.tokenSymbol, 50, _projectInput.royaltyReceiver, _projectInput.royaltyFeeNumerator)).token;
