...
contract DragQueenDivas is ERC721, ERC721Enumerable, ERC721URIStorage, Ownable {
    ...
    constructor(string memory baseTokenURI) ERC721 ("Drag Queen Divas", "DIVA")
    {
    ...
    function mint(address to, uint256 mintAmount) public payable {
        ...
        require(mintAmount <= _maxMintAmount, "Mint amount limit exceeded!");
        ...
        if (msg.sender != owner() && totalSupply() < 100) {
            if (super.balanceOf(msg.sender) >= 10) {
                require(msg.value >= _discountMintPrice * mintAmount, "Not enough ETH sent; check discountminting price!");
            } else {
                require(msg.value >= _mintPrice * mintAmount, "Not enough ETH sent; check minting price!");
            }
        }
        ...
        for (uint256 i = 1; i <= mintAmount; i++) {
            ...
            string memory finalTokenUri = string(abi.encodePacked(_baseTokenURI, Strings.toString(newItemId), ".json"));

            _safeMint(to, newItemId);
            _setTokenURI(newItemId, finalTokenUri);
            _tokenIds.increment();

            emit NewDragQueenDivasNFTMinted(to, newItemId);
        }
    }
    ...
    {
        require(!_isPaused, "Transfering is currently paused!");
