...
contract CoinToss is ReentrancyGuard, Ownable {
    ...
    // event Invested(address indexed _user, uint256 _amount);
    ...
    function startBet(uint256 _betAmount) public {
        ...
        // address(this).balance.add(_betAmount);
        if (_betAmount > address(this).balance) {
            betCoin.transfer(msg.sender, _betAmount);
            // address(this).balance.sub(_betAmount);
        }
        ...
        if (side == true) {
            ...
            // If contract lose
            // address(this).balance.sub(2 * _betAmount);
            betCoin.transfer(msg.sender, 2 * _betAmount);
        }
    }

    // A method for getting the balance of contract
