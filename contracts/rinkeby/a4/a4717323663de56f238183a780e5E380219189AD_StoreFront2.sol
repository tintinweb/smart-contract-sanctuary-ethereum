...
contract StoreFront2 is ERC721("MyNFT","TNFT"),ERC721Enumerable{
    ...
    function verifyNftSignature(address signer, address creator,  uint royalty, uint nftId, uint supply, address nftContract, bytes memory signature) internal pure returns (address)
    {
        bytes memory toHash = abi.encodePacked(creator, royalty, nftId, supply, nftContract);
