...
// File: contracts/EpicGallery.sol
...
contract EpicGallery is ERC721 {
    using SafeMath for uint256;

    uint256 constant public FEE_MAX_PERCENT = 20; // 20 %
    uint256 constant public FEE_MIN_PERCENT = 0; // 5 %
    uint256 public FEE_MINT = 2; // 2%;
    uint256 public PRICE = 100; //100MTV
    ...
    constructor() ERC721("Epic Gallery","epic") {    
        owner = msg.sender;
        adminAddress = payable(0x5B02Bd67b5B1bDaC824A78749eb7000900d2777b);
    }
    ...
    function setPrice(uint256 mint_price) public onlyOwner{
        PRICE = mint_price;
    }
    ...
    function airdrop(address[] memory airdropAddress) external onlyOwner {
        ...
        for (uint256 k = 0; k < airdropAddress.length; k++) {
            if (!_exists(currentID)) _safeMint(airdropAddress[k], currentID);
        }
    }

    function mintTo(string memory _tokenURI, uint256 royalty) public payable{
        require(royalty <= FEE_MAX_PERCENT, "too big royalties");
        require(royalty >= FEE_MIN_PERCENT, "too small royalties");
        if (FEE_MINT > 0)payable(adminAddress).transfer((msg.value * FEE_MINT) / 100);
        currentID = currentID.add(1);        
        _safeMint(msg.sender, currentID);
        Items[currentID] = Item(currentID, msg.sender, msg.sender, _tokenURI, royalty, PRICE);
    }
