...
contract A2Heroes is ERC721, ERC721Enumerable, ERC721Burnable, AccessControl, Ownable {
  ...
  function transferFrom(address from, address to, uint256 tokenId) public virtual override {
      ...
      (bool sent, bytes memory data) = royaltyAddress.call{value : royaltyAmount}("");
  ...
  function setSaleActive()public{
    require(hasRole(DEFAULT_ADMIN_ROLE, msg.sender), "Caller has not the role of admin");
    saleIsActive = true;
  }
  ...
  function Mint(string memory _name, string memory cid, uint256 characterId, uint256 amount) public payable {
    ...
    require(amount < character_supply[characterId]-character_minted[characterId], "The number of characters is insufficient.");
    require(msg.value >= character_price[characterId] * amount, "The price is not enough");
  ...
  function ownerMint(string memory _name, string memory cid, uint256 characterId, uint256 amount) public payable onlyOwner{
        uint256 supply = totalSupply();
    ...
    function batchMint(string memory _name, string[] memory cid, uint256[] calldata characterId, uint256[] calldata amounts) public payable{
      require(saleIsActive, "Sale must be active to mint");
      require(_mintedAmount[msg.sender] < walletLimit, "Exceeds personal wallet limits");
      uint256 totalValue = 0;
      for(uint i=0; i<characterId.length; i++){
        totalValue = totalValue + character_price[characterId[i]] * amounts[i];
      }

      require(msg.value >= totalValue, "The price is not enough.");

      for (uint idx=0; idx < characterId.length; idx++){
        uint256 supply = totalSupply();
        require(character_price[characterId[idx]] > 0, "Character does not exist");
        require(totalSupply() < MAX_SUPPLY, "Exceeds max supply of contract");
        require(character_minted[characterId[idx]] < character_supply[characterId[idx]], "The character amount for this archetype has reached its limit");

        for(uint jdx=1; jdx <= amounts[idx]; jdx++){
            setRealTokenURI(supply + jdx, cid[idx]);
            _safeMint(msg.sender, supply + jdx);
            _setTokenURI(supply + 1, cid[idx]);
          
            _mintedAmount[msg.sender] = _mintedAmount[msg.sender].add(1);
            character_minted[characterId[idx]] = character_minted[characterId[idx]] + 1;
  ...
  function presaleMint(string memory _name, string memory cid, uint256 characterId, uint256 amount) public payable {

    uint256 supply = totalSupply();
    uint256 blockTimeStamp = block.timestamp;
    uint256 currentTimeStamp = blockTimeStamp.mul(1000);
    require(hasRole(PRESALE_ROLE, msg.sender), "Caller has not the role for presale");
    require(character_price[characterId] > 0, "Character does not exist");
    require(saleIsActive, "Sale must be active to mint");
    require(_mintedAmount[msg.sender] < walletLimit, "Exceeds personal wallet limits");
    require(totalSupply() < MAX_SUPPLY, "Exceeds max supply of contract");
    ...
    require(msg.value >= character_price[characterId] * amount, "The price is not enough.");
    require(character_minted[characterId] < character_supply[characterId], "The character amount for this archetype has reached its limit");
    require(currentTimeStamp > presaleStartDate, string(abi.encodePacked("Today is not for Presale(less than start date).", currentTimeStamp)));
    require(currentTimeStamp < presaleStopDate, string(abi.encodePacked("Today is not for Presale(more than stop date).", currentTimeStamp)));
  ...
  function presaleBatchMint(string memory _name, string[] memory cid, uint256[] calldata characterId, uint256[] calldata amounts) public payable {
  ...
  function redeem(uint256 _id) public {
    uint256 blockTimeStamp = block.timestamp;
    uint256 currentTimeStamp = blockTimeStamp.mul(1000);
    require(hasRole(REDEEM_ROLE, msg.sender), "Caller has not the role for presale");
    require(currentTimeStamp > redeemStartDate, "Today is not for Redemption");
    require(currentTimeStamp < redeemStopDate, "Today is not for Redemption");
  ...
  function setCharacterPrice(uint256 id, uint256 characterPrice) public onlyOwner{
  ...
  function setRedeemDate( uint64 timestamp_start, uint64 timestamp_stop ) public {
    require(hasRole(DEFAULT_ADMIN_ROLE, msg.sender), "Caller has not the role for super admin.");
    redeemStartDate = timestamp_start;
    redeemStopDate = timestamp_stop;
  }

  function setPresaleDate( uint64 timestamp_start, uint64 timestamp_stop ) public {
    require(hasRole(DEFAULT_ADMIN_ROLE, msg.sender), "Caller has not the role for super admin");
