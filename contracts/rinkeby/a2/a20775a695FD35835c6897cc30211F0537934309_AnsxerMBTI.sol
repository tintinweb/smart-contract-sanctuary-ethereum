...
contract AnsxerMBTI is ERC721AQueryable, Ownable, Pausable {
  ...
  function teamMint() external onlyOwner {
    ...
    for (uint i; i < 16; i++) {
      for (uint n; n < TEAM_EACH_MBTI_RESERVED; n++) {
        _mbtiOfToken[(TEAM_EACH_MBTI_RESERVED * i) + n] = uint8(i);
      }
      _mbtiTotalMints[i] = TEAM_EACH_MBTI_RESERVED;
    }
  ...
  function allowlistMint(Order[] calldata orders) external whenNotPaused callerIsUser {
    ...
    uint256 minted = allowlist[msg.sender].minted;
    uint256 mintAvailable = mintable - minted;
    uint256 totalAmount = _getTotalAmount(orders);
  ...
  function _getTotalAmount(Order[] calldata orders) internal returns (uint256) {
  ...
  function mbtiKeyByValue(uint256 mbti) public pure returns (string memory) {
    return mbtiKeys()[mbti];
  }
