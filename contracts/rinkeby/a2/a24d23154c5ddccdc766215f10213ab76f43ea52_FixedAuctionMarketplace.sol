...
// File: contracts/Fixed_AuctionMarketplace.sol
...
contract FixedAuctionMarketplace is ERC721URIStorage {
  ...
  mapping(address=>mapping(uint256=>BidingsData))public bidForBidder;
    ...
    function putNFTForBidding(uint256 _id) public onlyOwner(_id) returns(bool){
     ...
     function onBid(uint256 _id) public payable {
        ...
        require(block.timestamp<=onBidding[_id].closingTime,"Bidding time is ended");
        require(msg.value > 0,"Price must be non-zero");
        require(_exists(_id),"Non-existent tokenId");
        require(onBidding[_id].bidPrice<msg.value,"You are paying less price from previous bid");
        ...
        if( onBidding[_id].Allbidders[onBidding[_id].i] != msg.sender){
         payable(onBidding[_id].Allbidders[onBidding[_id].i]).transfer( onBidding[_id].value);
    ...
    function buyOnBid(uint256 _id) external payable {
        ...
        require(_exists(_id),"Non-existent tokenId");
        require(onBidding[_id].highestBidder != address(0),"There is no winner address");
        super._safeTransfer(msg.sender,onBidding[_id].highestBidder,_id,"0x");
        ...
        emit OfferAccepted(_id,onBidding[_id].bidPrice,
        msg.sender,onBidding[_id].highestBidder);
