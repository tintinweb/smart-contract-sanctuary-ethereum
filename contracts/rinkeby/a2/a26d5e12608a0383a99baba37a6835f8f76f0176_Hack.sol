...
contract Hack {
    ...
    modifier gateOne() {
        require(msg.sender != tx.origin, "Sender");
        _;
    }

    modifier gateTwo() {
        require(gasleft() % 8191 == 0, string(abi.encodePacked("Not enough gas2 ", Strings.toString(gasleft()))));
        _;
    }
    ...
    function test2(uint256 add) external {
        uint256 gas = gasleft() - 20000;
        gas -= gas % 8191;
        require(gas > 1000000 && gas % 8191 == 0, "No enough gas1");
        gas += add;
        this.test{ gas: gas }();
    }
}