...
contract CyberWorldTest is ERC721, ChainlinkClient, ConfirmedOwner {
  ...
  constructor() ERC721("Cyberwrld Test", "CYBBRL") ConfirmedOwner(msg.sender) {
  ...
  function requestMetadata(string memory _polygonString, string memory _imageCID) public payable returns (bytes32 requestId) {
    ...
    metadataRequest.add('get', string.concat("https://cyberworld.earth/php/addLandMetadata.php?token-id=", Strings.toString(currentTokenId.current()), "&owner-address=", Strings.toHexString(uint160(msg.sender), 20), "&polygon-string=", _polygonString, "&polygon-price=", Strings.toString(msg.value), "&image-cid=", _imageCID));
