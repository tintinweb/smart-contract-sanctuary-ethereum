...
// TODO: Preço de lotes -TESTAR
// TODO: Max 2 per account
// TODO: Set URI
...
contract Genesis is ERC721A, Ownable, ReentrancyGuard {
  ...
  uint public batch1 = 3 * 10**17;
  uint public batch2 = 5 * 10**17;
  uint public bacth3 = 10**18;
  ...
  modifier mintPriceCompliance(uint256 _mintAmount) {
    require(msg.value >= costBatch(0) * _mintAmount, 'Insufficient funds!');
    _;
  }

  function costBatch(uint _totalSupply) public view returns(uint){
    //uint _totalSupply = totalSupply();
    if(_totalSupply <= 100){
      return batch1;
    }
    else if((_totalSupply <= 900) && (100 < _totalSupply)){
      return batch2;
    }
    else if (_totalSupply >= 900){
      return bacth3;
    }
    else{
      return 0;
