...
contract BASVault is IBASVault, TribeAccessControl, Pausable {
    ...
    /// @notice Wallet => Deposited BAS token amount
    ...
    constructor(address _provider, address _basToken)
        TribeAccessControl(_provider)
    {
        require(_basToken != address(0), 'invalid BAS token');

        basToken = IERC20(_basToken);
    }
    ...
    /**
     * @dev if wallet is not council nor member in Tribe
     */
    modifier onlyNoTribeMember() {
        require(
            getTribe().isApproved(msg.sender) == false,
            'approved council or member'
        );
    ...
    /**
     * @notice deposit BAS token
     * @param _amount amount to deposit
     */
    function deposit(uint256 _amount) external whenNotPaused onlyNoTribeMember {
     ...
     * @param _amount amount to deposit
     */
    function depositFrom(address _from, uint256 _amount)
        external
        whenNotPaused
        onlyTribe
    {
    ...
    function _deposit(address _from, uint256 _amount) internal {
        ...
        basToken.safeTransferFrom(_from, address(this), _amount);
    ...
    function _withdraw(address _to, uint256 _amount) internal {
        ...
        require(_amount > 0 && balanceOf[_to] >= _amount, 'invalid amount');

        balanceOf[_to] -= _amount;

        basToken.safeTransfer(_to, _amount);
