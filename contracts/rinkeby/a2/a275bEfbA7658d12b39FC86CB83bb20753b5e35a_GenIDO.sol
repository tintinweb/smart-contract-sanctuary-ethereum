...
contract GenIDO is Initializable {
    ...
    function submitTokens(uint256 _amount) external onlyIssuer {
        ...
        require((_tokensDepositedByAdmin + _amount) <= totalTokenAllocation, "GenIDO: Amount exceeds limit");
        require(IERC20(underlyingToken).transferFrom(msg.sender, address(this), _amount));
    ...
    function getTokensInfo() external view returns (uint256 _tokensSold, uint256 _tokensDepositedByAdmin, uint256 _tokensWithdrawnByAdmin) {
        return (tokensPurchased, tokensDepositedByAdmin, tokensWithdrawnByAdmin);
    }
