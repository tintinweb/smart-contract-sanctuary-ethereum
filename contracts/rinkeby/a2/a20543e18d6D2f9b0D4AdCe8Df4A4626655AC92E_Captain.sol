...
*
* This is the main contract that mints the Seaman who was as eager as his Captain to beat the record.
* 
* We are a group of brave, pioneering engineers and product OGs. We believe in，love NFTs and DAOs. 
* But, we find many people are hard to participate in NFTs，and we see boundaries between countries and countries, 
* projects and projects. Thus, we set up UPDAO.
*
* We advocate the Marine Culture: adventure, freedom, pioneering. We hope every UPDAO member is brave and open. 
* Everyone contributes and in return, gaining benefits and growth.
* We welcome NFT enthusiasts and entrepreneurs who share our mission and culture from all over the world.
* 
...
contract Captain is ERC721A, Ownable, ReentrancyGuard {
    ...
    mapping(address => uint256) public whitelistMintsPerWallet;
     ...
     // metadata
    string public baseURI = "https://cdn.monstercave.wtf/meta/dc9f28b12dd1818ee42ffc92ecb940386/json/";
    string public baseExtension = ".json";

    // -- Accounts --
    address private constant publisherAddress  = 0x84CFd77123F84D34C3Fc8395Ee632c781394E736;
    address private constant updaoVaultAddress = 0x33971d7e8e5B6557E7b3b0Dc083e00976B81BCE5;
    ...
    // construct
    constructor(bool _isWhitelistLive) ERC721A("Captain", "CPTN") { 
    ...
    function mintPublic(uint256 _amount) external payable nonReentrant callerIsUser {
        ...
        // TRANSFER  Ether to updao vault
        // sendMoneyTo(updaoVaultAddress, msg.value);

        // Mint the NFT with the ERC721 safeMint method
        _safeMint(_msgSender(), _amount);
    }

    function mintWhitelist(bytes32[] memory _proof) external payable nonReentrant callerIsUser{
        uint256 _amount = 1; //
    ...
    function verify(bytes32[] memory proof) external onlyOwner view returns (bool) {
    ...
    // publish to opensea
    function publish(uint256 _amount) external onlyOwner {
        require(totalSupply() + _amount <= maxTotalSupply, "Exceeds total supply");
        _safeMint(publisherAddress, _amount);
    }

    // 
    function teamAllocationMint(address _address, uint256 _amount) external onlyOwner {
        require(totalSupply() + _amount <= maxTotalSupply, "Exceeds total supply");
