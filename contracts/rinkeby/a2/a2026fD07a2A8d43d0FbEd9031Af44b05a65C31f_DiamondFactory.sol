...
import "./interfaces/IBitgemDiamond.sol";
...
contract DiamondFactory is Ownable, Initializable, Controllable {

    using BitGemLib for BitGemContract;

    DiamondFactoryContract private contractData;
    ...
    function initialize(DiamondFactoryInit memory initData) public initializer {
        ...
        contractData.wrappedToken_ = initData._wrappedToken;
        contractData.eventReporter = initData._eventReporter;

        // add the facets to the diamond
        for (uint256 i = 0; i < initData.facetAddresses.length; i++) {
            contractData.facetsToAdd.push(initData.facetAddresses[i]);
        }
    ...
    function eventReporter() external view returns (address) {
        return contractData.eventReporter;
    }

    function setEventReportingContract(
        address _contract,
        address[] memory _owners
    ) external {
        contractData.eventReporter = _contract;
        IEventReporter(contractData.eventReporter).register(_owners);
    }

    /// @notice create a new diamond token with the given symbol
    function create(
        BitGemInitParameters memory params,
        address diamondInit,
        ...
        require(
            contractData.tokenAddresses[params.symbol] == address(0),
        ...
        contractData.tokenAddresses[params.symbol] = tokenAddress;
        contractData.tokenSymbols.push(params.symbol);

        EventReportingContract(contractData.eventReporter).addAllowed(tokenAddress);

        // initialize the diamond contract
        Diamond(tokenAddress).initialize(
            msg.sender,
            params,
            contractData.facetsToAdd,
            diamondInit,
            eventReporter_,
            contractData.wrappedToken_,
            _calldata
        );
        ...
        allowed[2] = tokenAddress;

        EventReportingContract(contractData.eventReporter).register(allowed);
        Diamond(tokenAddress).setEventReportingContract(
            eventReporter_,
            allowed
        );
    }
    ...
    function exists(string memory symbol) public view returns (bool) {
        return contractData.tokenAddresses[symbol] != address(0);
    }

    /// @notice get all the symbols from the factory
    function symbols() public view returns (string[] memory) {
        return contractData.tokenSymbols;
    }

    // mint a bitgem from the given pool to the receiver
    function mintGemTo(
        address receiver,
        string memory symbol,
        uint256 quantity
    ) external onlyController returns (uint256[] memory _returnIds) {
        console.log("mintGemTo", symbol, receiver);
        _returnIds = new uint256[](quantity);
        require(exists(symbol), string(abi.encodePacked("Diamond does not exist:", symbol)));
        address diamondAddress = getDiamondAddress(symbol);
        console.log(symbol, diamondAddress);
        uint256 startId = IBitgemDiamond(diamondAddress).totalSupply();
        for (uint256 i = startId; i < startId + quantity; i++) {
            _returnIds[i - startId] = i;
            IBitgemDiamond(diamondAddress).mintBitGem(receiver);
        }
...
import { BitGemLib } from "./libraries/BitGemLib.sol";
...
contract Diamond is IERC165, IDiamondCut, IDiamondLoupe, Initializable, IERC173 {
    ...
    function setupMetadata(
        ...
        BitGemInitParameters memory params) private {
...
interface IBitgemDiamond {
    function setStakingState(bool state) external;
    function mintBitGem(address to) external;
    function settings() external view returns (BitGemSettings memory);
    function getClaimPrice(uint256 minLength, uint256 maxLength, uint256 minLengthPrice, uint256 targetLength) external pure returns (uint256);
    function getClaimTime(uint256 minLength, uint256 maxLength, uint256 minLengthPrice, uint256 targetPrice) external pure returns (uint256);
    function createClaim(uint256 claimTime, bool requireMature) external payable returns (Claim memory claim_);
    function collectClaim(uint256 claimId) external;
    ...
    function setSVGManager(address _manager) external;
    function svgString(string memory _name) external view returns (string memory data_);
    function createSVG(string memory _name) external returns(address _tplAddress);
    function supportsInterface(bytes4 _interfaceID) external view returns (bool);
    function eventReporter() external view returns (address);
    function setEventReportingContract(address contract_, address[] memory _owners) external returns (address _contract);
    function price() external view returns (VariablePriceContract memory);
    function setAllowedMinter(address a, bool b) external;
}
...
library DiamondLib{
    ...
    function setupMetadata(
        AppStorage storage appStorage, 
        address tokenAddress,
        BitGemInitParameters memory params) internal {
    ...
    function setupFeesAndPricing(
        AppStorage storage appStorage, 
        address tokenAddress,
        BitGemInitParameters memory params) private returns (FeeManagerContract storage _fee) {
        // get the feemanager storage struct
        _fee = appStorage.fees[tokenAddress];

        // set the pricing to create a stake
        appStorage.variablePrices[tokenAddress] = params.price;
    ...
    function initialize(
        ...
        BitGemContract storage bg = appStorage.bitgems[tokenAddress];
        
        // create the bitgem data struct
        BitGemSettings memory bitgemSettings = BitGemSettings(
            params.owner,
            TokenDefinition(
                tokenAddress,
                params.name,
        ...
        bg.wrappedToken = wrappedToken;
        bg.settings.tokenDefinition.token = tokenAddress;

        // set the fee to collect a claim
        appStorage.fees[tokenAddress].fees["collect_claim"] = Fee("collect_claim", 100);
