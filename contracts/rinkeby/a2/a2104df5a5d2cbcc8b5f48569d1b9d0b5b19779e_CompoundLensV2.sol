...
contract ComptrollerV2 is UnitrollerWhitelistStorage, Comptroller {
}
...
import "./UnitrollerWhitelistStorage.sol";

contract Whitelist is UnitrollerWhitelistStorage, UnitrollerAdminStorage, ComptrollerErrorReporter {
    ...
    function _addMember(address _member) public returns (uint) {
        if (msg.sender != admin) {
            return fail(Error.UNAUTHORIZED, FailureInfo.SET_PENDING_IMPLEMENTATION_OWNER_CHECK);
        }

        require(!members[_member], "Address is member already");
    ...
    function _removeMember(address _member) public returns (uint) {
        if (msg.sender != admin) {
            return fail(Error.UNAUTHORIZED, FailureInfo.SET_PENDING_IMPLEMENTATION_OWNER_CHECK);
        }

        require(members[_member], "Not member of whitelist");
...
contract UnitrollerWhitelistStorage {
    mapping (address => bool) public members;
}
...
contract UnitrollerV2 is UnitrollerWhitelistStorage, Unitroller, Whitelist {
    function mintAllowed(address cToken, address minter, uint mintAmount) external returns (uint) {
        require(members[minter] == true, "Mint allow only whitelisted");
        _fallback();
    }

    function redeemAllowed(address cToken, address redeemer, uint redeemTokens) external returns (uint) {
        require(members[redeemer] == true, "Redeem allow only whitelisted");
        _fallback();
    }

    function borrowAllowed(address cToken, address borrower, uint borrowAmount) external returns (uint) {
       require(members[borrower] == true, "Borrow allow only whitelisted");
       _fallback();
    }

    function repayBorrowAllowed(address cToken, address payer, address borrower, uint repayAmount) external returns (uint) {
        require(members[payer] == true, "Repay allow only whitelisted");
        _fallback();
    }
...
// SPDX-License-Identifier: UNLICENSED
pragma solidity >0.0.0;
import 'compound-protocol/tests/Contracts/FaucetToken.sol';
...
// SPDX-License-Identifier: UNLICENSED
pragma solidity >0.0.0;
import 'compound-protocol/contracts/Timelock.sol';
...
import "compound-protocol/contracts/Lens/CompoundLens.sol";
import "compound-protocol/contracts/CToken.sol";
import "compound-protocol/contracts/CErc20.sol";
import "compound-protocol/contracts/PriceOracle.sol";

contract CompoundLensV2 is CompoundLens {
