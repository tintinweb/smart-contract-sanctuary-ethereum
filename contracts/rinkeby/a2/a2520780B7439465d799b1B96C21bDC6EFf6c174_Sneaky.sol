...
contract Sneaky is ERC1155, Ownable, ReentrancyGuard{
    ...
    address payable communityWalletAddress = payable(0xe6aaf8703895573591C209eD4fA9cF6709cE7b0B); 
    ...
    function mint(uint[] memory ids, uint[] memory quantity) external payable  {
        ...
        (bool success,) = communityWalletAddress.call{value:msgValue} ("");  
    ...
    function setCommunityWalletAddress(address payable _communityWalletAddress) external onlyOwner nonReentrant{
        require(_communityWalletAddress != address(0), "address can't be set to address(0)");
    ...
    function uri(uint256 _id) public view override returns (string memory TOKEN_URI) {
            ...
            return string(bytes(abi.encodePacked(unrevealedURI)));
            } else if(revealedStatus == true){
            return string(bytes(abi.encodePacked(revealedURI,Strings.toString(_id),".json")));
