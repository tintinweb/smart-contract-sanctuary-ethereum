...
contract CandyKitty is ERC721, SignerManager, AccessControl {
    using Strings for uint256;
    ...
    function preSaleMint(address _to, uint256 _quantity, bytes32 _nonce, bytes calldata _sig) external payable {
        ...
        signers.requireValidSignature(signaturePayload(_to, _nonce), _sig, usedMessages);
    ...
    function whitelistMint( address _to, uint256[] calldata _tokenIds, bytes32 _nonce, bytes calldata _sig) external payable {
        ...
        signers.requireValidSignature(signaturePayload(_to, keccak256(bNonce)), _sig, usedMessages);
        for (uint256 idx = 0; idx < _tokenIds.length; idx++) {
    ...
    function alreadyMinted(address _to, bytes32 _nonce) external view returns (bool) {
        return usedMessages[SignatureChecker.generateMessage(signaturePayload(_to, _nonce))];
    }

    function signaturePayload(address _to, bytes32 _nonce) internal pure returns (bytes memory) {
