...
contract FlakeNFT is Context, ERC165, IERC721, IERC721Metadata, Ownable {
    ...
    constructor(address _motherAddress) Ownable() {
        _motherLedger = _motherAddress;
        _name = "Art Flake";
        _symbol = "Flake";
        _ledger = FlakeCoin(_motherLedger);
        _baseURI = "https://flake.art/";
    }
    ...
    function ownerOf(uint256 tokenId) public view virtual override returns (address) {
        address owner = _ledger.getOwnerbyNft(uint24(tokenId));
     ...
     *
     */
    function baseURI(string memory newURI) public onlyOwner {
        _baseURI = newURI;
    }
    ...
    function approve(address to, uint256 tokenId) public virtual override {
        address owner = _ledger.approveNft(_msgSender(), to, uint24(tokenId));
        if (owner != address(0)) {
    ...
    function getApproved(uint256 tokenId) public view virtual override returns (address) {
        return _ledger.getApproved(uint24(tokenId));
    }
    ...
    function setApprovalForAll(address operator, bool approved) public virtual override {
        _ledger.setApprovalForAllNfts(_msgSender(), operator, approved);
    ...
    function isApprovedForAll(address owner, address operator) public view virtual override returns (bool) {
        return _ledger.isApprovedForAll(owner, operator);
    }
    ...
    function transferFrom(
        ...
        _ledger.transferNftFrom(_msgSender(), from, to, uint24(tokenId));
    ...
    function safeTransferFrom(
        ...
        _ledger.safeTransferNftFrom(_msgSender(), from, to, uint24(tokenId), "");
        // emit approval for compatibility with openzepplin
        emit Approval(from, address(0), tokenId);
        emit Transfer(from, to, tokenId);
    }
    ...
    function safeTransferFrom(
        ...
        _ledger.safeTransferNftFrom(_msgSender(), from, to, uint24(tokenId), data_);
        // emit approval for compatibility with openzepplin
        emit Approval(from, address(0), tokenId);
        emit Transfer(from, to, tokenId);
    }
    ...
    function burn(uint256 tokenId) public virtual {
        ...
        _ledger.transferNftFrom(_msgSender(), _msgSender(), owner(), uint24(tokenId));
...
contract FlakeCoin is IERC20, IERC20Metadata, Ownable {
    ...
    function _transferCoin(
        ...
        for (uint256 idx = 0; idx < amount; idx++) {
            uint24 nft_index = _OwnerToLiquidCoinMap[owner][_OwnerToLiquidCoinMap[owner].length - 1];
            // Move the last token to the slot of the to-delete token
            // _OwnerToLiquidCoinMap[owner][idx] = nft_index;
            _OwnerToLiquidCoinMap[owner].pop();
            // remove from the old user's wallet:
            _OwnerToLiquidCoinMap[reciver].push(nft_index);
            // Delete nft index to ownership lookup.
            _LiquidNftToOwnerMap[nft_index] = reciver;
        }
        return true;
    }
    ...
    /**
     * @dev determine if the address is a contract, taken from openzepplin
     *
     */
    ...
    function mintNft(
        ...
        require(addr != address(0), "ERC721: mint to the zero address");

        _beforeTokenTransfer(address(0), addr, nft_indexes.length);
        uint24 minted = 0;
        // only callable with a signed request.
        require(_getMintSigner(addr, nft_indexes, signature) == _treasury, "Error: Mint signature doesn't match");
        ...
        _totalSupply += minted;
        // success
        emit Transfer(address(0), addr, (_totalSupply - minted));
        return true;
    }
    ...
    function mintCoin(
        ...
        for (uint256 i = 0; i < nft_indexes.length; i++) {
            ...
            if (_addCoin(addr, nft_indexes[i])) {
    ...
    function removeAllApprovals(address operator) public {
        ...
        uint24 nftCount = uint24(_OwnerToLiquidCoinMap[_msgSender()].length);
        // Make sure all NFT allowences are cleared
        for (uint24 i = 0; i < nftCount; i++) {
