...
contract FSBTokenDividendTracker is OwnableUpgradeable, DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    function initialize(address rewardToken_, uint256 minimumTokenBalanceForDividends_, string calldata name_, string calldata symbol_) external initializer {
    ...
    function withdrawDividend() public pure override {
        require(
            false,
            "Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main CNS contract."
        );
    }
}

contract FSBToken is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    FSBTokenDividendTracker public dividendTracker;
    ...
    // CONSTRUCTOR
    //address[4] memory addrs, // reward token address, router address, fund address, dividendTracker address
    //uint256[3] memory feeSettings, // rewards, liquidity, funding
    constructor(string memory name_, string memory symbol_, uint256 totalSupply_, address[4] memory addrs, uint256[3] memory feeSettings, uint256 minimumTokenBalanceForDividends_) payable ERC20(name_, symbol_) {
        rewardToken = addrs[0];
        _fundWalletAddress = addrs[2];
        require(
            msg.sender != _fundWalletAddress,
            "Owner and fund address cannot be the same"
        );

        tokenRewardsFee = feeSettings[0];
        liquidityFee = feeSettings[1];
        fundFee = feeSettings[2];
        totalFees = tokenRewardsFee.add(liquidityFee).add(fundFee);
        ...
        dividendTracker = FSBTokenDividendTracker(
        ...
        dividendTracker.initialize(
            ...
            string(abi.encodePacked(symbol_, ".D"))
        );
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(
            pair != uniswapV2Pair,
            "FSBToken: The pair cannot be removed from automatedMarketMakerPairs"
        );
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(
            ...
            "FSBToken: Automated market maker pair is already set to that value"
        );
        automatedMarketMakerPairs[pair] = value;
    ...
    function setFundFee(uint256 value) external onlyOwner {
        fundFee = value;
        totalFees = tokenRewardsFee.add(liquidityFee).add(fundFee);
        require(totalFees <= 15, "Total fee is over 15%");
    }
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(
            ...
            "FSBToken: The dividend tracker already has that address"
        );

        FSBTokenDividendTracker newDividendTracker = FSBTokenDividendTracker(
            payable(newAddress)
        );

        require(
            newDividendTracker.owner() == address(this),
            "FSBToken: The new dividend tracker must be owned by the FSBToken token contract"
        );
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(
            ...
            "FSBToken: gasForProcessing must be between 200,000 and 500,000"
        );
        require(
            newValue != gasForProcessing,
            "FSBToken: Cannot update gasForProcessing to same value"
        );
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(
            ...
            "FSBToken: The router already has that address"
        );
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(
            ...
            "FSBToken: Account is already the value of 'excluded'"
        );
        _isExcludedFromFees[account] = excluded;
    ...
    // SWAPPERS
    function swapAndSendToFund(uint256 tokens) private {
        uint256 initialBalance = address(this).balance;
        swapTokensForNative(tokens);
        uint256 newBalance = (address(this).balance).sub(initialBalance);
        payable(_fundWalletAddress).transfer(newBalance);
    }
    ...
    function swapTokensForNative(uint256 tokenAmount) private {
        // generate the uniswap pair path of token -> native
    ...
    // OTHER
    function addLiquidity(uint256 tokenAmount, uint256 baseAmount) private {
        // approve token transfer to cover all possible scenarios
        _approve(address(this), address(uniswapV2Router), tokenAmount);

        // add the liquidity
        uniswapV2Router.addLiquidityETH{value : baseAmount}(
            address(this),
            tokenAmount,
    ...
    function _transfer(address from, address to, uint256 amount) internal override {
        ...
        if (
            ...
            uint256 fundTokens = contractTokenBalance
            .mul(fundFee)
            .div(totalFees);
            swapAndSendToFund(fundTokens);
        ...
        // if from/to is not one of the pair addresses, remove fee for transfers
