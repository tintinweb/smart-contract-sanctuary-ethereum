    ...
    ReentrancyGuardUpgradeable
{
    ...
    event Cast(uint256 indexed index, address indexed account);
    modifier isHuman() {
        require(_msgSender() == tx.origin, "The caller is another contract");
        _;
    }

    function initialize(NftData memory _nftData) public initializer {
        __Ownable_init();
        __ReentrancyGuard_init();
        __ERC721_init(_nftData._name, _nftData._symbol);
    ...
    function updateAipDropAmount(uint256 _amount) external onlyOwner {
        AIR_DROP = _amount;
    }
    ...
    function multiplex(address to, uint256 amount) internal {
        for (uint256 i = 0; i < amount; ) {
            tokenIdTracker.increment();
            uint256 tokenId = tokenIdTracker.current();
            _mint(to, tokenId);
            emit Cast(tokenId, to);
            unchecked {
                i++;
            }
        }
    ...
    ) external payable nonReentrant isHuman {
    ...
    function airdrop(address[] calldata user) external onlyOwner {
        require(airDropSwitch, "AirDrop is not active");
        require(
            AIR_DROP > 0 && user.length <= AIR_DROP && count < AIR_DROP,
            "AirDrop: The number of people exceeds the limit"
        );
        for (uint256 j = 0; j < user.length; ) {
            id++;
            count++;
            _safeMint(user[j], id);
            emit Cast(id, user[j]);
