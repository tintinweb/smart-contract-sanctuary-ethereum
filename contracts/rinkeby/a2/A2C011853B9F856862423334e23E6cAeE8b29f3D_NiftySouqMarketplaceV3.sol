...
contract NiftySouqMarketplaceV3 is Initializable {
    ...
    function setContractAddresses(
        ...
        if (fixedPrice_ != address(0))                                                      //todo check usage
        ...
        )
    {
        ...
        address minter;
        if(msg.sender==address(_niftySouqFixedPrice) || msg.sender==address(_niftySouqAuction)) minter=mintData_.minter;
    ...
    function _lazyMint(
        ...
        LazyMintData
            memory lazyMintData = LazyMintData(
                offerId_,
                tokenId_,
                tokenAddress,
                isERC1155,
    ...
    function createSale(uint256 tokenId_, ContractType contractType_) external returns(uint256 offerId_){
         _offerId.increment();
        offerId_ = _offerId.current();
