...
/// @title Contract of Furychain NFTs collection
/// @author Johnleouf21
...
contract Furychain is ERC721A, PaymentSplitter, Ownable, ReentrancyGuard {
    ...
    //Maximum number of NFTs an address can mint on presale
    uint public max_mint_presale = 10;
    ...
    address[] private _team = [
        ...
        0x9439365f7A2ab00732177805e0aAB8bf8043ac99,
    ...
    constructor(string memory _theBaseURI, string memory _notRevealedURI, bytes32 _merkleRoot) ERC721A("Furychain", "FURY") PaymentSplitter(_team, _teamShares) {
    ...
    **/
    ...
    function changeMaxMintPresale(uint _maxMintPresale) external onlyOwner {
        max_mint_presale = _maxMintPresale;
    }
    ...
    * @param _quantity The _quantity of NFTs the user wants to mint
    **/
    function presaleMint(address _account, bytes32[] calldata _proof, uint256 _quantity) external payable nonReentrant {
        //Get the number of NFT sold
        uint numberNftSold = totalSupply();
        //Are we in Presale ?
        require(sellingStep == Steps.Presale, "Presale has not started yet.");
        //The user can only mint max 4 NFTs
        require(nftsPerWallet[_account] + _quantity <= max_mint_presale, "You can't mint more than the limit");
    ...
    * @param _quantity The _quantity of NFTs the user wants to mint
    **/
    function saleMint(address _account, uint256 _quantity) external payable nonReentrant {
        //Get the number of NFT sold
        uint numberNftSold = totalSupply();
        ...
        require(msg.value >= price * _quantity, "Not enought funds.");
        //The user can only mint max 6 NFTs
        ...
        require(numberNftSold + _quantity <= MAX_SUPPLY, "Sale is almost done and we don't have enought NFTs left.");
        //Add the _quantity of NFTs minted by the user to the total he minted
        nftsPerWallet[msg.sender] += _quantity;
        //If this account minted the last NFTs available
        if(numberNftSold + _quantity == MAX_SUPPLY) {
