...
contract PriceOracle is Initializable, OwnableUpgradeable, IPriceOracle {
    ...
    uint256 immutable CHAINLINK_HEARTBEAT;
     ...
     * @param _chainlinkHeartbeat max interval between within which chainlink oracle data is updated
     **/
    constructor(uint256 _chainlinkHeartbeat) {
        CHAINLINK_HEARTBEAT = _chainlinkHeartbeat;
    }
    ...
    function getChainlinkLatestPrice(address num, address den) public view returns (uint256, uint256) {
        ...
        int256 price2;
        {
            ...
            if (block.timestamp > timeStamp1 + CHAINLINK_HEARTBEAT || answeredInRound1 < roundID1) return (0, 0);
        }
        {
            uint80 roundID2;
            uint256 timeStamp2;
            ...
            if (block.timestamp > timeStamp2 + CHAINLINK_HEARTBEAT || answeredInRound2 < roundID2) return (0, 0);
        }
    ...
    function setChainlinkFeedAddress(address token, address priceOracle) external onlyOwner {
        uint8 priceOracleDecimals = AggregatorV3Interface(priceOracle).decimals();
