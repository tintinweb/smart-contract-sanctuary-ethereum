...
function test1(address _from, uint256 _tokenId) external {
        ERC721 token = erc721token();
}

function test2(address _from, uint256 _tokenId) external {
        ERC721 token = erc721token();
        address to = address(this);
}

function test3(address _from, uint256 _tokenId) external {
        ERC721 token = erc721token();
        address to = address(this);
