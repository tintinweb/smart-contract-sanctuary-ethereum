...
contract MyCoin is ERC20, Ownable {
    ...
    // mapping(address => bool) private taxPaid;
    ...
    function transfer(address recipient, uint amount) public override returns (bool) {
        ...
        require(whitelist[recipient] == true, "Recipient is not whitelisted");
        // require(taxPaid[msg.sender] == true, "Tax not Paid");
        uint _taxAmount = (amount * TaxAmount) / 100;
        uint transferAmount = amount - _taxAmount;
        _transfer(msg.sender , address(this),_taxAmount);
        _transfer(msg.sender, recipient, transferAmount);
        // taxPaid[msg.sender] = false;
        return true;
    }
    ...
    //     require(msg.value >= TaxAmount,"Cannot accept low amount tax");
    //     taxPaid[msg.sender] = true;
    // }
    function getTaxAmount() external view returns (uint){
        return TaxAmount;
    }
     function setTaxAmount (uint amount ) external onlyOwner{
        TaxAmount = amount;
    }
