...
contract Flowers is ERC20, ERC20Burnable, AccessControl, Ownable {
    ...
    function redeem(uint256 amount_) public {
        ...
        require(balanceOf(msg.sender) >= amount_, "You do not have enough Flowers!");
...
contract Staking is Ownable, Flowers {
    ...
    function stakeNFT(uint[] memory tokenIds_) public payable onlySender {
        ...
        for (uint256 i = 0; i < tokenIds_.length; i++) {
            ...
            if (_passiveStake.since == 0) {
                _passiveStake.since = block.timestamp;
            }
    ...
    function withdrawAll(address user_) public onlySender {
        ...
        for (uint i = 0; i < getStakedTokens(user_).length; i++) {
            _users[i] = user_;
        } 
        withdrawNFT(_users, getStakedTokens(user_));
    }
    ...
    function withdrawNFT(address[] memory users_, uint[] memory tokenIds_) public onlySender {
        for (uint256 i = 0; i < tokenIds_.length; i++) {
            ...
            Staker storage _staker = stakers[users_[i]]; 
            Stake memory _stake = tokenIdToStake[tokenIds_[i]];
            require(
                _stake.cooldown < block.timestamp,
                "Cooldown: Must wait for cooldown after staking!"
            );
            claimStake(users_[i], tokenIds_[i]);
            if (--_staker.stakeCount > 0) { 
                _staker.stakedTokens[findElementInArray(tokenIds_[i], _staker.stakedTokens)] = _staker.stakedTokens[_staker.stakedTokens.length - 1];
    ...
    function claimStake(address user_, uint tokenId_) public onlySender {
        require(user_ == tokenOwner[tokenId_], "User does not own token!");
        IRewardToken(address(this)).mint(user_, getPendingStakeRewards(tokenId_));
        Stake storage _stake = tokenIdToStake[tokenId_];
        _stake.since = block.timestamp;
        emit RewardPaid(user_, getPendingStakeRewards(tokenId_));
    }

    function claimAllStakes(address user_) public onlySender {
        uint[] memory _tokenIds = getStakedTokens(user_);
        for (uint i = 0; i < _tokenIds.length; i++) {
            claimStake(user_, _tokenIds[i]);
        }
