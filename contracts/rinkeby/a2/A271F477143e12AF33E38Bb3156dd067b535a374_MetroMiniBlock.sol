...
import "../../lib/enumerable/ERC721ExtraStorage.sol";
...
contract MetroMiniBlock is ERC721ExtraStorage, Controllable, IMetroBlockInfo {

    uint256 public constant MAX_MET_BLOCKS = 20_000;
    uint256 public constant MAX_ETH_BLOCKS = 20_000;
    uint256 public constant MAX_TRADE_BLOCKS = 10_000;
    uint256 public constant MAX_BLOCKS = MAX_MET_BLOCKS + MAX_ETH_BLOCKS + MAX_TRADE_BLOCKS;

    uint32 public constant BLOCK_ID_OFFSET = 20_000;
    uint256 public constant BLOCKS_PER_TRADE = 10;
    uint224 public constant ACCOUNT_MINTED_EXTRA_FLAG = 0x1;
    ...
    uint232 public ethPrice = 0.1 ether;

    bool public metSaleActive = false;
    bool public ethSaleActive = false;
    bool public tradeSaleActive = false;

    uint32 private _blockIdCounter = BLOCK_ID_OFFSET;
    uint32 private _burnedCounter;

    uint32 public lastMetMintTimestamp;
    uint32 public lastMetMintPrice = uint32(CEIL_MET_PRICE / 1 ether);
    ...
    address public vaultDoorAddress;

    address public proxyRegistryAddress;
    address public signerAddress;
    address public genesisTreasuryAddress;
    ...
    constructor(
      ...
      address _vaultDoorAddress, 
      address _proxyRegistryAddress, 
      address _signerAddress, 
      address _genesisTreasuryAddress) ERC721ExtraStorage("Metroverse Blackout City Block", "METROBLOCK2")
    {
        metTokenAddress = _metTokenAddress;
        vaultStorageAddress = _vaultStorageAddress;
        vaultDoorAddress = _vaultDoorAddress;
        proxyRegistryAddress = _proxyRegistryAddress;
        signerAddress = _signerAddress;
        genesisTreasuryAddress = _genesisTreasuryAddress;
    ...
    function setVaultStorageAddress(address _vaultStorageAddress) external onlyOwner {
        vaultStorageAddress = _vaultStorageAddress;
    }

    function setVaultDoorAddress(address _vaultDoorAddress) external onlyOwner {
        vaultDoorAddress = _vaultDoorAddress;
    }
    ...
    function setGenesisTreasuryAddress(address _genesisTreasuryAddress) external onlyOwner {
        genesisTreasuryAddress = _genesisTreasuryAddress;
    }

    function setEthPrice(uint256 price) public onlyOwner {
        ethPrice = uint112(price);
    }
    ...
    function updateAccountExtraOf(address owner, uint224 extra) public onlyController returns (AccountInfo memory) {
        return _updateAccountExtraOf(owner, extra);
    }

    function hasAlreadyMinted(address owner) public view returns (bool) {
        return (accountExtraOf(owner) & ACCOUNT_MINTED_EXTRA_FLAG) != 0;
    }

    function updateTimestampOf(uint256 tokenId, uint40 timestamp) public onlyController returns (uint40 oldTimestamp) {
        return _updateTimestampOf(tokenId, timestamp);
    }

    function updateTokenExtraOf(uint256 tokenId, uint56 extra) public onlyController returns (uint56 oldExtra) {
        return _updateTokenExtraOf(tokenId, extra);
    }

    function updateTimestampAndExtraOf(uint256 tokenId, uint40 timestamp, uint56 extra) public onlyController returns (TokenInfo memory) {
        return _updateTimestampAndExtraOf(tokenId, timestamp, extra);
    }

    function totalSupply() public view virtual returns (uint256 supply)
    {
        return _blockIdCounter - BLOCK_ID_OFFSET - _burnedCounter;
    }

    function currentMetPrice() public view returns (uint256) {
        if (lastMetMintTimestamp == 0) { 
            return CEIL_MET_PRICE;
        }

        uint256 passedPeriods = (block.timestamp - lastMetMintTimestamp) / PRICE_PERIOD;
        uint256 priceChange = PRICE_STEP * passedPeriods;
        uint256 lastPrice = lastMetMintPrice * 1 ether;

        if (lastPrice > priceChange + FLOOR_MET_PRICE) {
    ...
    function metMint(uint256 amount) public {
        ...
        require(metMinted <= MAX_MET_BLOCKS, "Exceeded max MET block count");

        uint256 currentPrice = currentMetPrice();
        if (currentPrice < lastMetMintPrice) {
            lastMetMintPrice = uint32(currentPrice / 1 ether);
        }

        MetToken metToken = MetToken(metTokenAddress);
        metToken.burnFrom(msg.sender, currentPrice * amount); 

        lastMetMintTimestamp = uint32(block.timestamp);
        _mintBlock(amount, 0);
    }
    ...
    function ethMint(bytes32 signatureR, bytes32 signatureVS, uint256 amount) public payable {
        ...
        _mintBlock(amount, ACCOUNT_MINTED_EXTRA_FLAG);
    }

    function tradeMint(uint256[] calldata stakedTokenIds, uint256[] calldata unstakedTokenIds) public {
        bool isDeployer = msg.sender == deployerAddress;
        require(tradeSaleActive || isDeployer, "Genesis sale is not active");
        ...
        require(genesisTreasuryAddress != address(0), "No NFT holder available");
        require(vaultStorageAddress != address(0), "No Vault available");

        IMetroVaultStorage vaultStorage = IMetroVaultStorage(vaultStorageAddress);
        MetroVaultDoor vaultDoor = MetroVaultDoor(vaultDoorAddress);

        vaultDoor.claimForAddress(msg.sender, stakedTokenIds);
        vaultStorage.setStakeOwner(stakedTokenIds, genesisTreasuryAddress, false);

        vaultStorage.stakeBlocks(msg.sender, unstakedTokenIds, 0, 0);
        vaultStorage.setStakeOwner(unstakedTokenIds, genesisTreasuryAddress, false);

        _mintBlock(amount, 0);
    }

    function _mintBlock(uint256 amount, uint224 extra) internal {
        require(totalSupply() + amount <= MAX_BLOCKS, "Exceeded max block count");

        unchecked {
            uint256 newBlockId = _blockIdCounter;
            _blockIdCounter += uint32(amount);
            for (uint256 i = 0; i < amount; i++) {
                newBlockId++;
                _mint(msg.sender, newBlockId, extra);
            }
        }
    }    
    
    function burn(uint256[] calldata tokenIds) public onlyController {
        _burnedCounter += uint32(tokenIds.length);
 ...
 * {ERC721Enumerable}.
 */
contract ERC721ExtraStorage is Context, ERC165, IERC721, IERC721Metadata {
    ...
    // Mapping from token ID to TokenInfo struct
    // CUSTOM: Visible for child contract so it's possible to emulate methods of ERC721's enumerable extension
    mapping(uint256 => TokenInfo) internal _owners;

    // NB: CUSTOM MODIFICATION
    // Mapping owner address to AccountInfo struct (contains token count and mint status)
    ...
    function accountExtraOf(address owner) public view returns (uint224) {
        require(owner != address(0), "ERC721: extra query for the zero address");
        return _balances[owner].extra;
    }

    function accountInfoOf(address owner) public view returns (AccountInfo memory) {
        require(owner != address(0), "ERC721: extra query for the zero address");
        return _balances[owner];
    }

    function _updateAccountExtraOf(address owner, uint224 extra) internal returns (AccountInfo memory oldAccountInfo) {
        require(owner != address(0), "ERC721: extra query for the zero address");
        oldAccountInfo = _balances[owner];
        _balances[owner].extra = extra;
    }
    ...
    function ownerOf(uint256 tokenId) public view virtual override returns (address) {
        TokenInfo storage info = _owners[tokenId];
        require(info.owner != address(0), "ERC721: owner query for nonexistent token");
        return info.owner;
    }

    function timestampOf(uint256 tokenId) public view returns (uint40) {
        TokenInfo storage info = _owners[tokenId];
        require(info.owner != address(0), "ERC721: timestamp query for nonexistent token");
        return info.timestamp;
    }

    function tokenExtraOf(uint256 tokenId) public view returns (uint56) {
        TokenInfo storage info = _owners[tokenId];
        require(info.owner != address(0), "ERC721: extra query for nonexistent token");
        return info.extra;
    }

    function tokenInfoOf(uint256 tokenId) public view returns (TokenInfo memory) {
        TokenInfo memory info = _owners[tokenId];
        require(info.owner != address(0), "ERC721: info query for nonexistent token");
        return info;
    }

    function _updateTimestampOf(uint256 tokenId, uint40 timestamp) internal returns (uint40 oldTimestamp) {
        TokenInfo storage info = _owners[tokenId];
        oldTimestamp = info.timestamp;
        info.timestamp = timestamp;
    }

    function _updateTokenExtraOf(uint256 tokenId, uint56 extra) internal returns (uint56 oldExtra) {
        TokenInfo storage info = _owners[tokenId];
        oldExtra = info.extra;
        info.extra = extra;
    }

    function _updateTimestampAndExtraOf(uint256 tokenId, uint40 timestamp, uint56 extra) internal returns (TokenInfo memory result) {
        TokenInfo storage info = _owners[tokenId];
        result = info;
        info.timestamp = timestamp;
        info.extra = extra;
    ...
    function approve(address to, uint256 tokenId) public virtual override {
        address owner = ERC721ExtraStorage.ownerOf(tokenId);
     ...
     *
     * Tokens can be managed by their owner or approved infos via {approve} or {setApprovalForAll}.
     *
     ...
     * Emits a {Transfer} event.
     */
    function _safeMint(address to, uint256 tokenId, uint224 extra) internal virtual {
        _safeMint(to, tokenId, extra, "");
    }
     ...
     * Emits a {Transfer} event.
     */
    function _mint(address to, uint256 tokenId, uint224 extra) internal virtual {
        require(to != address(0), "ERC721: mint to the zero address");
        require(!_exists(tokenId), "ERC721: token already minted");

        _beforeTokenTransfer(address(0), to, tokenId);

        AccountInfo storage account = _balances[to];
        account.balance += 1;
        account.extra = extra;

        TokenInfo storage info = _owners[tokenId];
        info.owner = to;
        info.timestamp = uint40(block.timestamp);
    ...
    function _transfer(
        ...
        require(ERC721ExtraStorage.ownerOf(tokenId) == from, "ERC721: transfer from incorrect owner");
    ...
    function _approve(address to, uint256 tokenId) internal virtual {
        ...
        emit Approval(ERC721ExtraStorage.ownerOf(tokenId), to, tokenId);
    }
