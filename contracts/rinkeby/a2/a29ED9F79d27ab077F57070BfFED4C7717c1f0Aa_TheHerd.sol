...
/// See also https://abhik.hashnode.dev/6-nuances-of-using-upgradeable-smart-contracts
  ...
  function initialize(address payable _wallet, uint256 initCost) initializer public {
    ...
    __Pausable_init();
    setBaseURI("");
    setNotRevealedURI("https://billidrop.com/nftMeta/unknown.json"); //host the original.json
    setRoyalties(msg.sender, 300);
    cost = initCost; //price in gwei??
    maxSupply = 10000; //adjusting supply for phase 1
    maxMintAmount = 20; //max mint per mint call
    revealed = false; // need to have it not revealed until the phase is completed goats will be hidden
    wallet = _wallet; 
