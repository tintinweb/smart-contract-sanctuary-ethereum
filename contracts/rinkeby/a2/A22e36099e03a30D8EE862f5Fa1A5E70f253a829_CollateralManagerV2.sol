...
/// @title Collateral Manager V2 Contract
...
contract CollateralManagerV2 is
    ...
    UUPSUpgradeable
{
    uint256 private constant MAX_NUMERATOR = 100;
    uint256 private constant MAX_DENOMINATOR = 100;
    ...
    mapping(address => uint256) public vaultsTotalNumerator;
    mapping(address => uint256) public vaultsTotalDenominator;
    ...
    /// @dev Public function to remove a reserve, all vault data must be removed for this function to succeed
    ...
    function deleteReserve(bytes32 reserveId) external onlyOwner {
        ...
        require(
            reserveVaults.length == 0,
            "vault array must be empty to remove reserve"
        );
    ...
    function addReserveVault(
        ...
        require(
            numerator < MAX_NUMERATOR && denominator < MAX_DENOMINATOR,
        ...
        _addVaultFraction(vault, numerator, denominator);
        ...
        require(
            IVaultCalculator(calculator).calculate(vault) > 0,
        ...
        onlyOwner
    {
        ...
        uint256 numerator = reserveVaults[index].numerator;
        uint256 denominator = reserveVaults[index].denominator;

        _subtractVaultFraction(vault, numerator, denominator);
    ...
    /// @notice Retricted to the owner.
    /// @dev Public function to update a vault's collateral ratio of a reserve, the reserve and vault need to already
    ...
    function updateReserveVault(
        ...
        Vault storage v = reserveVaults[index];

        _subtractVaultFraction(v.vault, v.numerator, v.denominator);
        _addVaultFraction(v.vault, numerator, denominator);

        v.numerator = numerator;
        v.denominator = denominator;

        emit UpdateReserveVault(reserveId, msg.sender, v.vault, index);
    }

    /// @dev Public function for manual override for total vault fraction used
    /// @param vault Address of the vault holding the asset
    /// @param numerator Quantity used in calculating the collateral ratio for the vaults reserve
    /// @param denominator Quantity used in calculating the collateral ratio for the vaults reserve
    function updateTotalVaultFraction(
        address vault,
        uint256 numerator,
        uint256 denominator
    ) external onlyOwner {
        require(
            ...
            "invalid total vault collateral ratio"
        );

        vaultsTotalNumerator[vault] = numerator;
        vaultsTotalDenominator[vault] = denominator;
    }

    /// @dev Public function allows the owner to clear out unused data
    /// @param vault Address of the vault holding the asset
    function deleteVaultMapItem(address vault) external onlyOwner {
    ...
    /// @dev Public function to calculate the total token quantity of a reserve's vaults
    /// @param reserveId Id to the set of vaults that will be used in calculating the total reserve value
    ...
    /// @dev Private function that adds to the total collateral for a vault
    /// @param vault Address of the vault
    /// @param numerator Quantity used in calculating the collateral ratio for the vaults reserve
    /// @param denominator Quantity used in calculating the collateral ratio for the vaults reserve
    function _addVaultFraction(
        address vault,
        uint256 numerator,
        uint256 denominator
    ) private {
        uint256 vTotalNumerator = vaultsTotalNumerator[vault];
        uint256 vTotalDenominator = vaultsTotalDenominator[vault];

        uint256 totalNumerator = vTotalDenominator == 0
            ? numerator
            : denominator * vTotalNumerator + numerator * vTotalDenominator;

        uint256 totalDenominator = vTotalDenominator == 0
            ? denominator
            : denominator * vTotalDenominator;
        ...
        vaultsTotalNumerator[vault] = totalNumerator;
        vaultsTotalDenominator[vault] = totalDenominator;
    }

    /// @dev Private function that subtracts from the total collateral for a vault
    /// @param vault Address of the vault
    /// @param numerator Quantity used in calculating the collateral ratio for the vaults reserve
    /// @param denominator Quantity used in calculating the collateral ratio for the vaults reserve
    function _subtractVaultFraction(
        address vault,
        uint256 numerator,
        uint256 denominator
    ) private {
        ...
        uint256 totalDenominator = denominator * vTotalDenominator;
