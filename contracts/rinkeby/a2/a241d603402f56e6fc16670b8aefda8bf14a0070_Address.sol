...
contract FAKENFTCONTRACT is ERC721Metadata {
    ...
    modifier periodStarted() {
        require(block.timestamp >= startTime && startTime != 0, "FAKENFTCONTRACT: Period not started");
        _;
    }
    ...
    constructor(
        string memory baseURI_,
        address admin_
    ) ERC721Metadata("Fake", "Fake", baseURI_) {
        _admin = admin_;
    }

    function setStartTime(uint256 _startTime) external onlyOwner {
        require(_startTime > 0, "FAKENFTCONTRACT: invalid _startTime");
        require(_startTime > block.timestamp, "FAKENFTCONTRACT: old start time");
    ...
    function presalePurchase(uint256 numberOfNfts) external payable onlyWhitelisted{
        ...
        require(pendingCount > 0, "FAKENFTCONTRACT: All minted");
        require(numberOfNfts > 0, "FAKENFTCONTRACT: numberOfNfts cannot be 0");
        require(totalSupply() + (numberOfNfts) <= MAX_NFT_SUPPLY,"FAKENFTCONTRACT: sale already ended");
        require(MINT_PRICE * (numberOfNfts) == msg.value, "FAKENFTCONTRACT: invalid ether value");
    ...
    function purchase(uint256 numberOfNfts) external payable periodStarted {
        require(pendingCount > 0, "FAKENFTCONTRACT: All minted");
        require(numberOfNfts > 0, "FAKENFTCONTRACT: numberOfNfts cannot be 0");
        require(numberOfNfts <= 20, "FAKENFTCONTRACT: You may not buy more than 20 NFTs at once");
        require(totalSupply() + (numberOfNfts) <= MAX_NFT_SUPPLY,"FAKENFTCONTRACT: sale already ended");
        if(tax_freeUsers[msg.sender] == 0) require(MINT_PRICE * (numberOfNfts) == msg.value, "FAKENFTCONTRACT: invalid ether value");
        else if (tax_freeUsers[msg.sender] < numberOfNfts){
            require(MINT_PRICE * (numberOfNfts -  tax_freeUsers[msg.sender]) == msg.value, "FAKENFTCONTRACT: invalid ether value");
    ...
    function claimReward(uint256 tokenId) public {
        require(
            ...
            "FAKENFTCONTRACT: Only owner or approved can claim rewards"
        );
    ...
    function _randomMint(address _to) internal returns (uint256) {
        require(totalSupply() < MAX_NFT_SUPPLY, "FAKENFTCONTRACT: max supply reached");
