...
contract UpcomingNFTPass is ERC721A, Ownable {
    ...
    function wlmint(uint256 _numTokens, bytes32[] memory _proof) external payable onlyOrigin wlmintCompliance(_numTokens) {
    ...
    function ownerMint(address _to, uint256 _numTokens) external payable onlyOrigin ownermintCompliance(_numTokens, _to) {
        if(FREE_SUPPLY > totalSupply()){
            require(msg.value == 0,"Value supplied is incorrect");
        }else{
            require(msg.value == PUBLIC_MINT_PRICE, "Value supplied is incorrect");
    ...
    function drain(address payable _to) external onlyOwner {
    ...
    modifier wlmintCompliance(uint256 _numTokens) {
        require(stage > 0, "Whitelist is paused.");
