...
contract NFTMarketplace is Pausable, FeeManager, ERC721Holder {
    ...
    //create order
    function createOrder(uint256 _tokenId, uint256 _askingPrice) public whenNotPaused {
        
        _createOrder(_tokenId,  _askingPrice);
    }
    function createAuction(uint256 _tokenId, uint256 _minBid,uint _expiryTime) public whenNotPaused {
    ...
    function safeExecuteOrder( uint256 _tokenId) public whenNotPaused payable{
        
        
        Order memory order = _getValidOrder( _tokenId);
        
        require(order.askingPrice == msg.value, "Marketplace: invalid price");
    ...
    //internal functions
    function _createOrder(uint256 _tokenId, uint256 _askingPrice) internal   {
        ...
        // create the orderId
        bytes32 _orderId = keccak256(abi.encodePacked(_tokenId, _askingPrice));
        ...
        emit OrderCreated(_orderId,msg.sender,_tokenId,_askingPrice);
        ...
        for (uint256 index = 0; index < auctionnID.length; index++) {
           ...
           auctionData=auctionByTokenId[aa];
            if(auctionData.seller==_owner)
            {
                ...
                resultAuction[index]=auctionnID[index];
        ...
        for (uint256 index = 0; index < orderrID.length; index++) {
           ...
           order = orderByTokenId[aa];
            if(order.seller==_owner)
            {
                ...
                resultOrder[index]=orderrID[index];
