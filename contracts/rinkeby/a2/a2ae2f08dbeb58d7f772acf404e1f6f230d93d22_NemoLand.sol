...
contract NemoLand is Admin, ERC721Upgradeable {
    ...
    mapping(uint256 => IERC20) public cTypeToAddress;
    mapping(uint256 => OnSaleLand) public tokenIdToPrice;
    mapping(uint256 => uint256) public zoneIdToPrice;
    mapping(uint256 => mapping(uint256 => uint256)) public cTypeToZonePrice;
    ...
    function initialize(
        ...
        for (
            ...
            addCurrency(
                currencyType,
                IERC20(_currencyAddresses[currencyType]),
    ...
    function addCurrency(
        ...
        // AllowedCrypto[_currecyType] = newToken;
    ...
    function setZonePrice(
        ...
        // cTypeToZonePrice[currencyType][zoneId] = amount;

        AllowedCrypto[currencyType].zoneIdToPrice[zoneId] = amount;
        emit ZonePrice(currencyType, zoneId, amount);
    }
    ...
    function mintLand(
        ...
        // require(
        //     isValidRequest(_signature, state, lands, _zoneId, referral),
        //     "Invalid sign"
        // );
        if (state > 0) {
        ...
        // uint256 amtRequired = zoneIdToPrice[_zoneId] * lands.length;
        uint256 amtRequired = AllowedCrypto[currencyType].zoneIdToPrice[
            _zoneId
        ] * lands.length;
        ...
        if (0 == currencyType) {
            ...
            IERC20 paytoken = AllowedCrypto[currencyType].paytoken;
        ...
        if (state > 0) {
            ...
            for (counter = 0; counter < lands.length; counter++) {
                ...
                emit LandMint(state, lands[counter], referral);
            }
        } else {
            for (counter = 0; counter < lands.length; counter++) {
     ...
     * @param weiAmount Price for sale
     */
    function allowBuy(uint256 _id, uint256 weiAmount) external {
        require(msg.sender == ownerOf(_id), "Not an Owner");
        require(weiAmount > 0, "Invalid Price");
        require(tokenIdToPrice[_id].price == 0, "Already On sale");
        tokensOnSale.push(_id);
        tokenIdToPrice[_id] = OnSaleLand(
            weiAmount,
            msg.sender,
            CurrencyType.TYPE2
        );
        emit AllowBuy(msg.sender, _id, weiAmount);
    }
    ...
    function buy(uint256 _id) external payable {
        ...
        emit Resale(_id, leftOver, comission);
    }
    ...
    function receiveUSDTFromSender(uint256 amount) external {
        usdt.transferFrom(msg.sender, receivingWallet, amount);
    }
    ...
    function getZonePrice(uint256 currencyType, uint256 zone)
        ...
        returns (uint256)
    {
        return cTypeToZonePrice[currencyType][zone];
    }
    ...
    function isValidRequest(
        ...
        bytes32 msgHash = keccak256(
            abi.encode(_state, _id, _zoneId, msg.sender, referral)
        );
