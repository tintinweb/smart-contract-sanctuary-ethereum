...
contract PoolFactory is Initializable, OwnableUpgradeable, IPoolFactory {
   ...
   // event Ismanager(address manager);
    ...
    // function manager(uint256 poolId) external returns (address){

    //     PoolInfo storage poolInfo = poolInfos[poolId];
    //      require(msg.sender = poolInfo.manager,"if your not manager");
        
    //      emit Ismanager(address(manager));

    //     return address(poolInfo.manager);
