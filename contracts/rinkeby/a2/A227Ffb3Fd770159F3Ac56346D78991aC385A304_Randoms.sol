...
contract Randoms is ERC721,ERC721Enumerable,Ownable {
    ...
    uint256  public  _privateSaleTimeEnds = _privateSaleTimeStart + 60;
    ...
    function privateSaleMint(uint256 _mintAmount)public {
        _checkPrivateActive();
        _checkPublicActive();
        require(publicSaleisActive == false, "Private sale is over ,public sale is now  active and you can't mint in private sale  anymore.");
        require(privateSaleisActive == true,"Private sale is not active yet ");
        require(_mintAmount >0, "You cant mint Zero Tokens");
        require(_mintAmount <= maxMintForPrivate, "You can't mint more than 3.");
       
        require(TxPerformedPrivate[msg.sender] < maxTxForPrivate, "The max number of tx that can be sent in the private sale is 3.");
        require( mintPerformedPrivate[msg.sender] + _mintAmount <= maxMintForPrivate, "On addition of this amount you are about to mint, Your total minted would exceed the max of 3 in private sale ,consider reducing the amount.");
    ...
    function publicSaleMint(uint256 _mintAmount) public{
        _checkPrivateActive();
        _checkPublicActive();

        require(privateSaleisActive == false,"Private sale is still on ");
        require(publicSaleisActive == true, " public sale  is not active yet  yet");
        require(_mintAmount >0, "You cant mint Zero Tokens");
        ...
        require(TxPerformedPublic[msg.sender] < maxTxForPublic, "The max number of tx that can be sent in the public sale is 2");
         require( mintPerformedPublic[msg.sender] + _mintAmount <= maxMintForPublic, "On addition of this amount you are about to mint, Your total minted would exceed the max of 2 in public sale ,consider reducing the amount.");
    ...
    function airdrop(address to) public onlyOwner PUBLIC_SALE_ACTIVE{
        ...
        require(amountForAIRDROPS >=1, "The REMAINING AMOUNT FOR AIRDROPS IS 0");
        // uint id = totalSupply();
