...
contract ACTE is AbstractERC1155Factory  {
    ...
    uint256 constant MAX_SUPPLY = 19998;
    uint256 constant MAX_EARLY_ACCESS = 3333;
    uint8 maxPerTx = 10;
    uint8 maxTxPublic = 3;
    uint8 maxTxEarly = 1;
    ...
    */
    function updateSaleRestrictions(uint8 _maxPerTx, uint8 _maxTxEarly, uint8 _maxTxPublic) external onlyOwner {
        maxPerTx = _maxPerTx;
    ...
    */
    ...
    function getRandomNumber(uint256 _upper) private view returns (uint256) {
        return uint(keccak256(abi.encodePacked(block.timestamp, block.difficulty, msg.sender))) % _upper;
    }
    ...
    */
    function earlyAccessSale(
        ...
        require(whiteListSale, "Early access :: Early Minting is on Pause");
        uint256 totalTokenSupply = 0;
        for(uint256 i = 0; i < supplies.length; i++ ) {
			totalTokenSupply += totalSupply(i);
		}
        require(totalTokenSupply + amount <= MAX_EARLY_ACCESS, "Early access :: Max supply reached");
        require(purchaseTxs[msg.sender] < maxTxEarly , "Early access :: Max TX amount exceeded");

        bytes32 sender = keccak256(abi.encodePacked(msg.sender));
        require(
            MerkleProof.verify(merkleProof, merkleRoot, sender),
    ...
    */
    function purchase(uint256 amount) external payable whenNotPaused callerIsUser {
        require(publicSale, "Purchase :: Not Yet Active");
        uint256 totalTokenSupply = 0;
        for(uint256 i = 0; i < supplies.length; i++ ) {
			totalTokenSupply += totalSupply(i);
		}
        require(totalTokenSupply + amount <= MAX_SUPPLY, "Purchase :: Max supply reached");
        require(purchaseTxs[msg.sender] < maxTxPublic , "Purchase :: Max TX amount exceeded");
    ...
    */
    ...
    function mintTeam(uint256 id, uint256 amount) public onlyOwner {
        _mint(owner(), id, amount, "");
    }

    function withdraw() public onlyOwner {
        require(address(this).balance > 0, "Withdraw :: Balance is 0");
