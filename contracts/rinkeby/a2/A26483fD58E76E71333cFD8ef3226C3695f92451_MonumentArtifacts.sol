...
contract Splits is Initializable, ReentrancyGuardUpgradeable {
    ...
    function distributeFunds() public nonReentrant payable {
        ...
        require(balance > 0, "funds must be more than 0 to be distributed");

        emit ReceivedFunds(msg.sender, balance, block.timestamp);

        for (uint256 i = 0; i < splitters.length; i++) {
            (bool success, ) = payable(splitters[i]).call{value: mulScale(permyriads[i], balance, 10000)}("");
    ...
    function distributeERC20(address _erc20address) public nonReentrant returns (address, uint256) {
        ...
        require(balance > 0, "must have more than 0 tokens to be distributed");

        for (uint256 i = 0; i < splitters.length; i++) {
            erc20Token.transfer(
                splitters[i],
                mulScale(permyriads[i], balance, 10000)
            );
        }
