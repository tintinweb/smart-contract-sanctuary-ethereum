...
contract MetaFriends is ERC721, Ownable {

    uint public preMintPrice = 0.1 ether;
    uint public mintPrice = 0.1 ether;
    uint private airdropItems = 300;
    uint public maxItems = 10000;
    uint public firstPhaseItems = 6666;
    uint public DutchAuctionMintItems = 3334;
    uint public DutchAuctionMintCount = 0;
    uint public airdropMintCount = 0;
    uint public totalSupply = 0;
    uint public maxItemsPerPreMint = 1; 
    ...
    address _signer = 0x3EE7404c3751d14D98f156Db2f7f2F780D0345e1;
    address _auction_owner = 0x3EE7404c3751d14D98f156Db2f7f2F780D0345e1;
    using ECDSA for bytes32;
    event Mint(address indexed owner, uint indexed tokenId);

    mapping(address => bool) private preMintLists; 
    mapping(address => bool) private airdropLists; 


    constructor() ERC721("Meta Friends", "MF") {}
    ...
    function preMint(
        ...
        require(!preMintLists[msg.sender],"The wallet has already minted during preslae sale.");
    ...
    function airdropMint(
        ...
        require(!airdropLists[msg.sender],"The wallet has already minted during airdrop.");
        uint amount = 1;
        require(airdropMintCount + amount <= airdropItems, "airdropMint: Surpasses cap");
        require(_verify(_hash(salt, msg.sender), token), "Invalid token.");
    ...
    function publicMint() external payable {
        ...
        require(totalSupply - airdropMintCount - DutchAuctionMintCount + amount <= firstPhaseItems - airdropItems, "publicMint: Surpasses cap");
        _mintWithoutValidation(msg.sender, amount);
    }

    // mint by auction contract address, use auction
    function DutchAuctionMint(uint amount, address to) external payable {
        require(DutchAuctionMintCount + amount <= DutchAuctionMintItems, "DutchAuctionMint: Surpasses cap");
    ...
    function airdropToAddr(address[] memory toList) external onlyOwner {
        require(!airdropPaused, "airdropMint: Paused");
        require(airdropMintCount + toList.length <= airdropItems,"airdropMint: Surpasses cap");
        for(uint i; i< toList.length; i++) {
            _mintWithoutValidation(toList[i],1);
            airdropMintCount += 1;
        }  
    }
    ...
    function withdrawAll() external onlyOwner {
        ...
        sendEth(0xdf1ff1200A7E5F98416359cD8f40047586d2E0ab, amount);
    }
