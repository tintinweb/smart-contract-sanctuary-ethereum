...
contract IDO is Ownable {
    ...
    IERC20 public _quoteToken;
    IERC20 public _idoToken;
    
    address private _idoer;
    uint256[3] private _points = [250,275,300];
    uint256[3] private _amounts = [1000000,2000000,4000000];
    uint256[4] private _gear = [100,200,300,400];
    uint8 public _currRound;
    bool private _success;
    uint256 private _idoAmount;
    uint256 private _totalIdo;
    mapping(address => uint256) public limitAddrMap;
    ...
    constructor(address quoteToken,address idoToken,address idoer) {
        ...
        _idoToken = IERC20(idoToken);
        _currRound = 1;
        _totalIdo = 7000000 * 10**18;
        _idoer = idoer;
    }

    function idoRound() public view returns (uint8) {
        return _currRound;
    }

    function idoPrice() public view returns (uint256) {
    ...
    function ido(uint256 gearIndex) public {
        require(!_success,'ido is end!');
        require(gearIndex <= 3,'gearIndex is err!');
        require(gearIndex >= 0,'gearIndex is err!'); 
        require(limitAddrMap[msg.sender] < 1,'ido is limit 1 times!');
        require(_idoAmount <= _totalIdo ,'ido is end');        
        uint256 price = idoPrice();    
        uint256 quoteAmount = price * _gear[gearIndex]; 
        uint256 idoValue = _gear[gearIndex] * 10**18; 
        if (_idoAmount == _totalIdo) {
            _success = true;
            _currRound = 3;
        }else if (_idoAmount >= _amounts[0] * 10**18 + _amounts[1] * 10**18 && _currRound == 2) {
            _currRound = 3;
        }
        else if (_idoAmount >= _amounts[0] * 10**18 && _currRound == 1){
            _currRound = 2;
        }
        _idoAmount += idoValue;
        limitAddrMap[msg.sender] ++;
        IERC20(_quoteToken).safeTransferFrom(msg.sender, _idoer, quoteAmount);
        IERC20(_idoToken).safeTransfer(msg.sender, idoValue);
        emit Ido(msg.sender,address(_idoToken), idoValue, address(_quoteToken), quoteAmount, price);
    }
