...
contract ICO is Ownable, ReentrancyGuard{
    ...
    struct User{
        ...
        uint tokenPurchased;
        uint amountPayedInBNB;
        uint amountPayedInBUSD;
    ...
    AggregatorInterface public datafeed = AggregatorInterface(0x8A753747A1Fa494EC906cE90E9f37563A8AF630e);
    ...
    */
    ...
    constructor(uint rate, address payable wallet, IBEP20 token, IBEP20 usdt, IBEP20 busd) {
    ...
    function setReferralRate(uint _referRate) public onlyOwner{
    ...
    */
    function buyTokens(IBEP20 tokenAddress, address referral, address beneficiary, uint256 _amount) public nonReentrant {
        require(!isPaused, "ICO is Paused");
        require(_amount >= minAmount, "Please purchase min amount of BLACK07");
        require(tokenAddress == _usdt || tokenAddress == _busd, "Please enter valid token address");
        ...
        if(tokenAddress == _busd){
            userInfo[beneficiary].amountPayedInBUSD = userInfo[beneficiary].amountPayedInBUSD.add(_amount);
        }else{
        ...
        if(!alreadyReferred[beneficiary]){
            if(userInfo[referral].referlist.length < 7){
                if(isValidUser[referral]){
                    userInfo[referral].referlist.push(beneficiary);
                    _processPurchase(referral, tokens.mul(referRate).div(100));   
                }
                alreadyReferred[beneficiary] = true;
            }
        }
    ...
    */
    function buyTokensWithBNB(address referral, address beneficiary) public nonReentrant payable {
        ...
        require(price >= minAmount, "Please purchase min amount of BLACK07");
        ...
        userInfo[beneficiary].amountPayedInBNB = userInfo[beneficiary].amountPayedInBNB.add(weiAmount);
        
        
        userInfo[beneficiary].tokenPurchased = userInfo[beneficiary].tokenPurchased.add(tokens);

        _processPurchase(beneficiary, tokens);
        if(!alreadyReferred[msg.sender]){
            if(userInfo[referral].referlist.length < 7){
                if(isValidUser[referral]){
                    userInfo[referral].referlist.push(beneficiary);
                    _processPurchase(referral, tokens.mul(referRate).div(100));   
    ...
    */
    function _getTokenAmount(uint256 weiAmount) public view returns (uint256) {
        uint tokens = weiAmount.mul(_rate).div(1e11);
        return (tokens + (tokens.mul(_bonus).div(100)));
    }

    function getRateBNB(uint256 weiAmount) public view returns (uint256) {
        uint256 usdAmount = getprice(weiAmount.div(1e18));
        uint256 tokens = usdAmount.mul(1e18).mul(_rate).div(1e11);
        return (tokens + (tokens.mul(_bonus).div(100)));
    }
    ...
    */
    ...
    function withdrawAnyBEP20Token(address tokenAddress, address beneficary, uint tokenAmount) public onlyOwner{
        IBEP20(tokenAddress).transfer(beneficary, tokenAmount);
    }
