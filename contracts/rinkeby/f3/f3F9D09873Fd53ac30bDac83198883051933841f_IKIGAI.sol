...
contract IKIGAI is ERC721A, Ownable, ReentrancyGuard {
  ...
  modifier mintComplianceWL(uint256 _mintAmount) {
    require((claimedAmount[_msgSender()] + _mintAmount) <= 3, 'Exceeded Whitelist Mint Amount!');
    //require((balanceOf(_msgSender()) + _mintAmount) <= 3, 'Exceeded Whitelist Mint Amount!');
    require(_mintAmount > 0 && _mintAmount <= maxMintAmountPerTx, 'Invalid Mint Amount!');
    require(totalSupply() + _mintAmount <= currentMaxSupply, 'Max Supply Exceeded!');
    _;
  }
  ...
  function whitelistMint(uint256 _mintAmount, bytes32[] calldata _merkleProof) public payable mintComplianceWL(_mintAmount) mintPriceCompliance(_mintAmount) {
