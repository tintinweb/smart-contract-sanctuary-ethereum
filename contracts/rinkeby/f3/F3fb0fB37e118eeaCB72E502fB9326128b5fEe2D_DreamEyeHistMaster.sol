 ...
 *   - Generation NFT: 
 *        > Generation starts with 0
 *        > Owner can change the generation
 *        > User can change the generation
 *        > Only one generation will be active at a time  
...
contract DreamEyeHistMaster is ERC721AQueryable, ERC721ABurnable, Ownable, ReentrancyGuard, ERC2981 {

  using Strings for uint256;

  bytes32 public merkleRoot;
  mapping(address => uint256) public tokensClaimed;
  mapping(uint256 => uint256) public tokenGen; // Token to current generation mapping
  mapping(uint256 => string) public genURIPrefix; // Generation to URI prefix mapping
  mapping (address => bool) genBoosters; 
  ...
  bool public histContractEnabled = false;
  uint256 public revealedGen; 
  INftHistoryToken public histContract;
  ...
  function allowlistMint(uint256 _mintAmount, bytes32[] calldata _merkleProof) public payable mintCompliance(_mintAmount) mintAllowlistCompliance(_mintAmount) {
    ...
    // histContract.mintForAddress(_mintAmount, _msgSender());
  }
  ...
  function mintForAddress(uint256 _mintAmount, address _receiver) public mintCompliance(_mintAmount) onlyOwner {
    ...
    // histContract.mintForAddress(_mintAmount, _receiver);
  }
  ...
  function tokenURI(uint256 _tokenId) public view virtual override returns (string memory) {
    return tokenURIForGen(_tokenId, tokenGen[_tokenId]);
  }

  function tokenURIForGen(uint256 _tokenId, uint256 _gen) public view returns (string memory) {
    ...
    string memory currentBaseURI = (_gen > revealedGen ? genURIPrefix[revealedGen] : genURIPrefix[_gen]);
  ...
  function revealGen(uint256 _gen, string memory _uriPrefix) public onlyOwner {
    genURIPrefix[_gen] = _uriPrefix;
    if (_gen > revealedGen) {
      revealedGen = _gen;
    }
  }
  ...
  function addGenBooster(address _booster) public onlyOwner {
    genBoosters[_booster] = true;
  }

  function disableGenBooster(address _booster) public onlyOwner {
    genBoosters[_booster] = false;
  }
  ...
  function setHistContract(address _histContract) public onlyOwner {
    histContract = INftHistoryToken(_histContract);
  }
  
  function setHistContractEnabled(bool _state) public onlyOwner {
    histContractEnabled = _state;
  }
  ...
  */
  ...
  function _baseURI(uint256 _tokenId) internal view virtual returns (string memory) {
    return genURIPrefix[tokenGen[_tokenId]];
  }

  /** 
    * Transitions token to next generation
    * 
    * @param _tokenId Token ID
    * @param gen Generation ID
    */
  function transition(uint256 _tokenId, uint256 gen) public onlyOwner {
    require(tokenGen[_tokenId] < gen, "De-evolution is not possible");
    tokenGen[_tokenId] = gen;
  }

  /** 
    * Evolves token into new generation
    * 
    * @param _tokenId Token ID
    * @param gen Generation ID
    */
  function evolve(uint256 _tokenId, uint256 gen) public {
    require(!paused);
    require(genBoosters[_msgSender()] || _msgSender() == ownerOf(_tokenId), "Invalid gen booster");
  ...
  function _afterTokenTransfers(
      ...
      if(histContractEnabled) {
        for (uint256 i = 0; i < quantity; i++) {
          histContract.transferHist(from, to, tokenId + i);
        }        
      }
    }
