...
import {AppStorage} from "./libraries/LibAppStorage.sol";
import {PaymentSplitterFacet} from "./facets/PaymentSplitterFacet.sol";
...
contract LivelyDiamond {
    ...
    struct DiamondArgs {
        ...
        string _name;
        string _symbol;
        address[] payees; // primary
        uint256[] shares_; // primary
    ...
    constructor(
        ...
        require(
            _args.payees.length == _args.shares_.length,
            "PaymentSplitter: payees and shares length mismatch"
        );
        require(_args.payees.length > 0, "PaymentSplitter: no payees");
        ...
        s.airdrop = _args.airdrop;
        s._maxSupply = _args._maxSupply;
        s._contractURI = _args.contractURI;
        s._baseTokenUri = _args._baseTokenUri;
        s._price = _args.airdrop ? 0 : _args._price;
        s._maxMintPerTx = _args.maxMintPerTx;
        s.maxMintPerAddress = _args.maxMintPerAddress;
        s.allowListEnabled = _args.allowListEnabled;

        // Initialize PaymentSplitter information
        for (uint256 i = 0; i < _args.payees.length; ) {
            Shared._addPayee(_args.payees[i], _args.shares_[i]);
            // Gas Optimization
            unchecked {
                i++;
            }
        ...
        // Initialize ERC2981 Data
        s.secondaryPayee = _args.secondaryPayee;
        s.secondaryPoints = _args.secondaryPoints;
    }
    ...
    //     DiamondArgs memory _args
    // ) payable {
    //     console.log("Contract Owner: %s", _args._contractOwner);
    //     LibDiamond.setContractOwner(_args._contractOwner);
    ...
    //         facetAddress: _input._diamondCutFacet,
 ...
 */
abstract contract PaymentSplitter is Modifiers {
    // event PayeeAdded(address account, uint256 shares);
    // event PaymentReceived(address from, uint256 amount);
    ...
    function totalReleased(IERC20 token) public view returns (uint256) {
        return s._erc20TotalReleased[token];
    }
    ...
    function released(IERC20 token, address account)
        ...
        returns (uint256)
    {
        return s._erc20Released[token][account];
    }
    ...
    function release(address payable account) public virtual {
        ...
        // s._totalReleased is the sum of all values in s._released.
        // If "s._totalReleased += payment" does not overflow, then "s._released[account] += payment" cannot overflow.
    ...
    function release(IERC20 token, address account) public virtual {
        ...
        // s._erc20TotalReleased[token] is the sum of all values in s._erc20Released[token].
        // If "s._erc20TotalReleased[token] += payment" does not overflow, then "s._erc20Released[token][account] += payment"
        // cannot overflow.
