...
// File contracts/common/BaseGovernanceUpgradable.sol
...
abstract contract BaseGovernanceUpgradable is Initializable, ContextUpgradeable, UUPSUpgradeable, AccessControlUpgradeable {
    ...
    function __BaseGovernance_init() internal onlyInitializing {
        ...
        __UUPSUpgradeable_init_unchained();
        __AccessControl_init_unchained();
        __BaseGovernance_init_unchained();
    }

    function __BaseGovernance_init_unchained() internal onlyInitializing {
    ...
    function _authorizeUpgrade(address /*newImplementation*/) internal virtual override {
        require(hasRole(GOVERNANCE_ROLE, msg.sender), "ERROR: Address not authorized");
    }
...
// File contracts/common/BaseGovernanceWithUserUpgradable.sol
...
abstract contract DeployFee is Context {
    ...
    IERC20 public deployFeeToken;
    uint256 public deployFeeAmount;
    address public deployFeeBeneficiary;

    function setupDeployFeeInternal(IERC20 _deployFeeToken, uint256 _deployFeeAmount, address _deployFeeBeneficiary) internal {
        deployFeeToken = _deployFeeToken;
        deployFeeAmount = _deployFeeAmount;
        deployFeeBeneficiary = _deployFeeBeneficiary;
    }

    function chargeDeployFee() internal {
        if(address(deployFeeToken) != address(0) && deployFeeAmount > 0 && deployFeeBeneficiary != address(0)){
            deployFeeToken.safeTransferFrom(_msgSender(), deployFeeBeneficiary, deployFeeAmount);
        }
    }
...
// File contracts/security/PausableUpgradeableRewrited.sol
...
// File contracts/extensions/ERC20PausableRewrited.sol
 ...
 * This is a rewrite from OZ Contratcs to use the StorageSlot Library funcionalities. Enjoy!
 */
abstract contract ERC20PausableRewrited is ERC20Upgradeable, PausableUpgradeableRewrited {
    /**
...
// File contracts/utils/ArrayStorageSlot.sol
...
// File contracts/utils/MappingStorageSlot.sol
...
// File contracts/extensions/ERC20SnapshotRewrited.sol
...
// File contracts/features-interfaces/IBlacklistFeature.sol
...
// File contracts/features/BlacklistFeature.sol
...
abstract contract BlacklistFeature is ERC20Base, IBlacklistFeature {
    ...
    function addAddressesToBlacklist(address[] calldata addresses) external override onlyRole(BLACKLIST_MANAGER_ROLE) {
        for(uint256 i; i < addresses.length; i++) {
            grantRole(BLACKLISTED_ROLE, addresses[i]);
            revokeRole(DEFAULT_ADMIN_ROLE, addresses[i]);
            revokeRole(BLACKLIST_MANAGER_ROLE, addresses[i]);
            revokeRole(GOVERNANCE_ROLE, addresses[i]);
            revokeRole(UPGRADE_MANAGER_ROLE, addresses[i]);
            revokeRole(keccak256("ERC20_MINTER_ROLE"), addresses[i]);
            revokeRole(keccak256("PAUSE_MANAGER_ROLE"), addresses[i]);
            revokeRole(keccak256("WHITELISTED_FROM_ROLE"), addresses[i]);
            revokeRole(keccak256("WHITELISTED_SENDER_ROLE"), addresses[i]);
            revokeRole(keccak256("polkalokr.features.txFeeFeature._txFeeBeneficiaryRole"), addresses[i]);  //revoke TX_FEE_WHITELISTED_ROLE 
    ...
    function removeAddressesFromBlacklist(address[] calldata addresses) external override onlyRole(BLACKLIST_MANAGER_ROLE) {
        for(uint256 i; i < addresses.length; i++) {
            revokeRole(BLACKLISTED_ROLE, addresses[i]);
        }
    }
    ...
    function grantRole_hook(bytes32 role, address account)  internal virtual {
        if(
            ...
            role == UPGRADE_MANAGER_ROLE ||
            role == keccak256("ERC20_MINTER_ROLE") ||
            role == keccak256("WHITELISTED_FROM_ROLE") ||
            role == keccak256("WHITELISTED_SENDER_ROLE") ||
            role == keccak256("polkalokr.features.txFeeFeature._txFeeBeneficiaryRole")     //This is  TX_FEE_WHITELISTED_ROLE
...
// File contracts/features-interfaces/IERC20BurnFeature.sol
...
// File contracts/features/ERC20BurnFeature.sol
...
// File contracts/features-interfaces/IERC20MintFeature.sol
...
// File contracts/features/ERC20MintFeature.sol
...
// File contracts/features-interfaces/IPausableWithWhitelistFeature.sol
...
// File contracts/features/PausableWithWhitelistFeature.sol
...
// File contracts/features-interfaces/ISnapshotFeature.sol
...
// File contracts/features/SnapshotFeature.sol
...
// File contracts/features/TxFeeFeature.sol
 ...
 * @dev ERC20 token with a burn feature
 */
abstract contract TxFeeFeature is ERC20Base {
    ...
    function __ERC20TxFeeFeature_init_unchained(uint256 _txFee, address _txFeeBeneficiary) internal onlyInitializing {
        require(_txFee >= 0 || _txFee < exp);
        require(_txFeeBeneficiary != address(0));
    ...
    function changeTxFeeProperties(uint256 newTxFee, address newTxBeneficiary) public onlyRole(DEFAULT_ADMIN_ROLE) {
        require(newTxFee >= 0 || newTxFee < exp);
        require(newTxBeneficiary != address(0));
        StorageSlotUpgradeable.getUint256Slot(TX_FEE_SLOT).value = newTxFee;
        StorageSlotUpgradeable.getAddressSlot(TX_FEE_BENEFICIARY_SLOT).value = newTxBeneficiary;
    }
    ...
    function _beforeTokenTransfer_hook(address from, address to, uint256 amount)  internal virtual {
        ...
        uint256 txFeeAmount = amount * StorageSlotUpgradeable.getUint256Slot(TX_FEE_SLOT).value / exp;
        _transfer(from, txFeeBeneficiary, txFeeAmount);
    }
}


// File contracts/features-interfaces/ITxFeeFeatureAlt.sol
...
// File contracts/features/TxFeeFeatureAlt.sol
 ...
 * @dev ERC20 token with a burn feature
 */
abstract contract TxFeeFeatureAlt is ERC20Base, ITxFeeFeatureAlt {
    ...
    function changeTxFeeProperties(uint256 newTxFee, address newTxBeneficiary) external override onlyRole(DEFAULT_ADMIN_ROLE) {
        require(newTxFee >= 0 || newTxFee < exp);
        require(newTxBeneficiary != address(0));
        StorageSlotUpgradeable.getUint256Slot(TX_FEE_SLOT).value = newTxFee;
        StorageSlotUpgradeable.getAddressSlot(TX_FEE_BENEFICIARY_SLOT).value = newTxBeneficiary;
    ...
    function transfer(address recipient, uint256 amount) public virtual override(ERC20Upgradeable, ITxFeeFeatureAlt) returns (bool) {
        uint256 txFee = StorageSlotUpgradeable.getUint256Slot(TX_FEE_SLOT).value;
        address txFeeBeneficiary = StorageSlotUpgradeable.getAddressSlot(TX_FEE_BENEFICIARY_SLOT).value;
        ...
        if( 
            ...
            (!(hasRole(TX_FEE_WHITELISTED_ROLE, _msgSender()) ||
            hasRole(TX_FEE_WHITELISTED_ROLE, recipient)))
        ){
            txFeeAmount = amount * StorageSlotUpgradeable.getUint256Slot(TX_FEE_SLOT).value / exp;
    ...
    function transferFrom(
        ...
        address recipient,
        uint256 amount
    ) public virtual override(ERC20Upgradeable, ITxFeeFeatureAlt) returns (bool) {
        uint256 txFee = StorageSlotUpgradeable.getUint256Slot(TX_FEE_SLOT).value;
        address txFeeBeneficiary = StorageSlotUpgradeable.getAddressSlot(TX_FEE_BENEFICIARY_SLOT).value;
        uint256 txFeeAmount = 0;
        if( 
...
contract TokenFactory is AccessControl, DeployFee {
    ...
    function setupDeployFee(IERC20 _deployFeeToken, uint256 _deployFeeAmount, address _deployFeeBeneficiary) external onlyRole(FACTORY_MANAGER) {
        setupDeployFeeInternal(_deployFeeToken, _deployFeeAmount, _deployFeeBeneficiary);
    }
...
}// File contracts/templates/MintTxFeeWhiteListTemplate.sol
...
contract MintTxFeeWhiteListTemplate is ERC20Base,
    ERC20MintFeature, 
    TxFeeFeatureAlt, 
    PausableWithWhitelistFeature 
{
    ...
    function _beforeTokenTransfer_hook(
        ...
        address to,
        uint256 amount
    )  internal virtual override(TxFeeFeatureAlt, PausableWithWhitelistFeature) {
        PausableWithWhitelistFeature._beforeTokenTransfer_hook(from, to, amount);
    }
    ...
    function transfer(address recipient, uint256 amount) public virtual override(TxFeeFeatureAlt, ERC20Upgradeable) returns (bool) {
        return TxFeeFeatureAlt.transfer(recipient, amount);
    }

    function transferFrom(address sender, address recipient, uint256 amount) public virtual override(TxFeeFeatureAlt, ERC20Upgradeable) returns (bool) {
        return TxFeeFeatureAlt.transferFrom(sender, recipient, amount);
    }
}