...
contract ExchangeCore is ReentrancyGuarded, Ownable {
    ...
    bytes32 public constant DOMAIN_SEPARATOR = 0xd38471a54d114ee69fbb07d1769a0bbecd4f429ddf5932c7098093908e24bd9d;
        ...
        reentrancyGuard
    {
        ...
        if (buy.maker == msg.sender) {
            require(validateOrderParameters(buy),"A");
        } else {
        ...
        /* Must be matchable. */
        require(ordersCanMatch(buy, sell),"B");
        ...
        require(ArrayUtils.arrayEq(buy.calldata, sell.calldata),"D");

        /* Retrieve delegateProxy contract. */
        OwnableDelegateProxy delegateProxy = registry.proxies(sell.maker);

        /* Proxy must exist. */
        require(delegateProxy != address(0),"E");
        ...
        require(delegateProxy.implementation() == registry.delegateProxyImplementation(),"F");

        /* Execute specified call through proxy. */
        require(proxy.proxy(sell.target, sell.howToCall, sell.calldata),"G");

        /* Static calls are intentionally done after the effectful call so they can check resulting state. */

        /* Handle buy-side static call if specified. */
        if (buy.staticTarget != address(0)) {
            require(staticCall(buy.staticTarget, sell.calldata, buy.staticExtradata),"H");
        }

        /* Handle sell-side static call if specified. */
        if (sell.staticTarget != address(0)) {
            require(staticCall(sell.staticTarget, sell.calldata, sell.staticExtradata),"I");
        }
