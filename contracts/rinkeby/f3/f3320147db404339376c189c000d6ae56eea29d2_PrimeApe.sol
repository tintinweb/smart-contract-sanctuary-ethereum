...
contract PrimeApe is ERC721, Ownable {
    // masked ipfs folder address
    // always add trailing / at the end
    string private _base;
    string private _contractURI;

    uint16 private constant MAX_APES = 21;
    uint16 private constant MAX_APES_MINTED = 5;
    uint64 private constant MINT_PRICE = 20000000000000000; // 0.02 ETH

    bool public saleIsActive = true;

    uint16 public totalSupply = 0;

    // mapping(uint16 => string) private _tokenURIs;

    constructor() ERC721("Prime Apes", "PAP") Ownable() {}
        ...
        returns (string memory)
    {
        ...
        return string.concat(base, Strings.toString(_tokenId));
    }
    ...
    function mintApes(uint16 numberOfTokens) public payable {
        require(saleIsActive, "Currently not for open for minting");
        require(
