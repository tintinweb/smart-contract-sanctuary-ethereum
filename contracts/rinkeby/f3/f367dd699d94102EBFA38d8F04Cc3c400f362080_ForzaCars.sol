...
contract ForzaCars is ERC1155, Ownable, ReentrancyGuard {
    ...
    function mintMysteryCar(uint256 forzaCoin) external payable nonReentrant returns(uint256[] memory _carIds, uint256[] memory _carAmounts){
        ...
        (carIds, carAmounts) = _mintMysteryCar(msg.sender,amountToMint);
        if (refund > 0) {
    ...
    function _mintMysteryCar(address to,uint256 qty) internal returns(uint256[] memory _carIds,uint256[] memory _carAmounts){
        uint256[] memory carIds = new uint[](qty);
        uint256[] memory carAmounts = new uint[](qty);
    ...
    function updateValidCars(uint256[] memory carIds, bool[] memory status) external onlyOwner {
     ...
     function setMysteryPrice(uint256 ethPrice, uint256 forzaCoinPrice) public onlyOwner{
