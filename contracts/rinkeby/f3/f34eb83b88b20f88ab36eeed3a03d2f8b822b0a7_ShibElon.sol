...
contract ShibElon is ERC721, Ownable {
    ...
    uint256 public PRESALE_RESERVED = 3000;
    uint256 public MAX_ALLOWED = 8;
    uint256 public MAX_ALLOWED_PRESALE = 5;
    uint256 public OWNER_RESERVED = 300;
    uint256 public price = 40000000000000000; //0.04 Ether presale & 0.05 Ether public sale
    string baseTokenURI;
    ...
    function setOwnerReserved(uint256 _ownerReserved) public onlyOwner {
        require(_ownerReserved <= MAX_SUPPLY,'Owner reserved can not be more than 8000');
        OWNER_RESERVED = _ownerReserved;
    }

    function setMaxSupply(uint256 _max_supply) public onlyOwner {
        require(_max_supply <= MAX_SUPPLY,'Max supply can not be more than 8000');
        MAX_SUPPLY = _max_supply;
    }

    function setPresaleReserved(uint256 _presale_reserved) public onlyOwner {
        require(_presale_reserved <= MAX_SUPPLY,'Presale supply can not be more than 8000');
        PRESALE_RESERVED = _presale_reserved;
    }
    ...
    function burnUnsold() external onlyOwner {
        ...
        PRESALE_RESERVED = totalSupply();
        OWNER_RESERVED = totalSupply();
    }
    ...
    //Presale Mint NFT
    function presaleMintNFT(uint16 _count) public payable {
        supply = totalSupply();
        if (msg.sender != owner()) {
