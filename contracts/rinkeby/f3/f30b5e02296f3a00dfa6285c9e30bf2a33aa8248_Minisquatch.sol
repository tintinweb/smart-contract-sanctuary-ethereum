...
// File: contracts/Minisquatch.sol
...
contract Minisquatch is ERC721Enumerable, Ownable {
  using Strings for uint256;
  ...
  address public metasquatchAddress = 0x9b40De3c7D290D4674661cb167eF95CB28deE38b;
  address public landAddress = 0x54CfEF426Dc91E3727D1FbfD843cF89d57cd87de;

  address public rewardPool = 0x5C2cbe845377182bcA6FfeE624B63416b38D8084;
  ...
  function mint() public payable {
    ...
    if (msg.sender != owner()) {
        require(hasParents(msg.sender), "Need more Metasquatchs/Land");
        IMetasquatch(metasquatchAddress).transferFrom(msg.sender, address(this), 
            IMetasquatch(metasquatchAddress).walletOfOwner(msg.sender)[IMetasquatch(metasquatchAddress).walletOfOwner(msg.sender).length - 1], 
            "");
    }

    if(IMetasquatch(metasquatchAddress).balanceOf(msg.sender) >= 4){
      for(uint i = 1; i < IMetasquatch(metasquatchAddress).balanceOf(msg.sender) / 2; i++){
        _safeMint(msg.sender, totalSupply() + _mintAmount);
        require(IRewardPool(rewardPool).addNodeInfo(totalSupply() + _mintAmount, msg.sender), "Can't update rewardpool data");
      }
    }else{
  ...
  function canMint(address _address) public view returns(bool){
    uint256 supply = totalSupply();
    return hasParents(msg.sender != address(0)? msg.sender :_address) && !paused && _mintAmount > 0 && _mintAmount <= maxMintAmount && supply + _mintAmount <= maxSupply ? true : false;
  }
  ...
  function math(uint i, uint j) public pure returns(uint){
        return i / j;
  }  
}
