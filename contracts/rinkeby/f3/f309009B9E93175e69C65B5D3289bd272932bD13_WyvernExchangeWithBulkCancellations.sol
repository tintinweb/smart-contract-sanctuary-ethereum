        ...
        returns (uint)
    {
        ...
        if (sell.feeRecipient != address(0)) {
            ...
            if (sell.feeMethod == FeeMethod.SplitFee) {
                ...
                if (sell.takerCashbackFee > 0) {
                    uint takerCashbackFee = SafeMath.div(SafeMath.mul(sell.takerCashbackFee, price), INVERSE_BASIS_POINT);
                    //TODO : Implement Cashback
                    // if (sell.paymentToken == address(0)) {
                    //     requiredAmount = SafeMath.add(requiredAmount, takerCashbackFee);
                    //     protocolFeeRecipient.transfer(takerCashbackFee);
                    // } else {
                    //     transferTokens(sell.paymentToken, buy.maker, protocolFeeRecipient, takerCashbackFee);
                    // }
                }
            ...
            if (sell.feeMethod == FeeMethod.SplitFee) {
                ...
                if (buy.takerCashbackFee > 0) {
                    takerCashbackFee = SafeMath.div(SafeMath.mul(buy.takerCashbackFee, price), INVERSE_BASIS_POINT);
                    //TODO : Implement Cashback
                    // transferTokens(sell.paymentToken, buy.maker, buy.feeRecipient, takerCashbackFee);
                }
...
contract Exchange is ExchangeCore {
    ...
    {
        return hashOrder(
          Order(addrs[0], addrs[1], addrs[2], uints[0], uints[1], uints[2], uints[3], uints[9], addrs[3], feeMethod, side, saleKind, addrs[4], howToCall, calldata, replacementPattern, addrs[5], staticExtradata, ERC20(addrs[6]), uints[4], uints[5], uints[6], uints[7], uints[8]),
          nonces[addrs[1]]
        );
    }
    ...
    function validateOrderParameters_ (
        ...
        returns (bool)
    {
        Order memory order = Order(addrs[0], addrs[1], addrs[2], uints[0], uints[1], uints[2], uints[3], uints[9], addrs[3], feeMethod, side, saleKind, addrs[4], howToCall, calldata, replacementPattern, addrs[5], staticExtradata, ERC20(addrs[6]), uints[4], uints[5], uints[6], uints[7], uints[8]);
