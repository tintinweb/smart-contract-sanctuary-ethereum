...
contract MonsterSuitNFT is ERC721A, Ownable
{
    ...
    constructor() ERC721A("MonsterSuitNFT", "MSN") {}
    ...
    function presaleMintA(uint16 amount, bytes32[] memory proof) external payable
    {
        ...
        require(addressMintedBalance[msg.sender] + amount <= 1, "Quantity exceeds allowance");
        require(totalSupply() + amount <= MAX_SUPPLY, "Purchase would exceed max supply");
        require(msg.value >= PRICE * amount, "Not enough ETH for transaction");
        require(msg.sender == tx.origin, "Transaction from smart contract not allowed");
    ...
    function presaleMintB(uint16 amount, bytes32[] memory proof) external payable
    {
        ...
        require(addressMintedBalance[msg.sender] + amount <= 2, "Quantity exceeds allowance");
        require(totalSupply() + amount <= MAX_SUPPLY, "Purchase would exceed max supply");
        require(msg.value >= PRICE * amount, "Not enough ETH for transaction");
        require(msg.sender == tx.origin, "Transaction from smart contract not allowed");
    ...
    function presaleMintC(uint16 amount, bytes32[] memory proof) external payable
    {
        ...
        require(addressMintedBalance[msg.sender] + amount <= 4, "Quantity exceeds allowance");
        require(totalSupply() + amount <= MAX_SUPPLY, "Purchase would exceed max supply");
        require(msg.value >= PRICE * amount, "Not enough ETH for transaction");
        require(msg.sender == tx.origin, "Transaction from smart contract not allowed");
