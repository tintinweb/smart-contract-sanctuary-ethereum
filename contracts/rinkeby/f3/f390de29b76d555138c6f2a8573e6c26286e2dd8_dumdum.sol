...
//no need to add reentrancy gaurd as well
//adding checks
//

contract dumdum is ERC721, Ownable, ReentrancyGuard, RandomlyAssigned{ 
    ...
    mapping (address => uint256) public HpCountDetails;
    mapping (address => uint256) public PublicCountDetails;
    mapping (address => uint256) public WhitelistedCountDetails;
    ...
    function setHpRevealFlage(bool flag) external onlyOwner{
        HPRevealFlag = flag;
    }
    ...
    function mintBee(uint256 quantity) public payable {
        ...
        require(HpSaleWallets[msg.sender] == true && HPRevealFlag == true && (HpCountDetails[msg.sender] +quantity) <= HPLimit || whiteListedWallets[msg.sender] == true && whiteListedUsersRevealFlag == true && (WhitelistedCountDetails[msg.sender] +quantity) <= WhiteListedLimit || publicRevealFlag == true && (PublicCountDetails[msg.sender]+quantity) <= publicLimit ,"Minting is not start you have to wait for admin notification or your limit is reached!");
        for (uint256 i = 1; i <= quantity; i++) {
            uint256 id = nextToken();
            _safeMint(msg.sender, id);
        }
        if(HpSaleWallets[msg.sender] == true && HPRevealFlag == true && (HpCountDetails[msg.sender] +quantity) <= HPLimit){
            HpCountDetails[msg.sender] += quantity;
        }else if(whiteListedWallets[msg.sender] == true && whiteListedUsersRevealFlag == true && (WhitelistedCountDetails[msg.sender] +quantity) <= WhiteListedLimit){
            WhitelistedCountDetails[msg.sender] +=quantity;
        }else{
            PublicCountDetails[msg.sender] += quantity;
        }
