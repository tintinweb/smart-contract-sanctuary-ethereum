...
contract CryptoSpace{
    ...
    function userDataes(uint lvl, address user) public onlyOwner view returns(uint id, address ref, uint levelInRow,
    uint extraBR, uint lostMoney, bool levelWasActivate, bool levelIsActive, uint placeInStructure, uint absolutePlaceInLevel, uint usersBP, bool insurance) {
        address ad=user;
        uint Lev=lvl;
