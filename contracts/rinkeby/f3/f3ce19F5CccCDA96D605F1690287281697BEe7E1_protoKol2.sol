...
contract protoKol2 is iProtoKol {
   ...
   */
    function generateTGE(uint256 _campaignId, address token) external {
        ...
        _safeTransferFrom(campaigns[_campaignId].campaignData.preSaleToken, msg.sender, _stakingContract, staking);

        _safeTransferFrom(
            campaigns[_campaignId].campaignData.preSaleToken,
            msg.sender,
            _vault,
            (markBudge * _platformPer) / 10000
        );
   ...
   */
    function claimKOLInvestment(uint256 _campaignId) external {
        ...
        _investment = (_investment * IERC20(_USDT).decimals()) / 1e18;
        // IERC20(_USDT).transfer(msg.sender, _investment);
        _safeTransfer(_USDT, msg.sender, _investment);
   ...
   */
    ...
    function claimPreSaleTokens(
        ...
        if (campaigns[_campaignId].vestingData.isVestingInEnabled) {
            ...
            require(IERC20(preSale).balanceOf(address(this)) >= _reward, "Protokol:Not enough funds");
            ...
            investedCampaignDetails[msg.sender][_campaignId].eligibleReward =
                (investmentDetails.eligibleReward * 10**IERC20(preSale).decimals()) /
                10**18;
            // console.log("KOL ELigible Reward:",investedCampaignDetails[msg.sender][_campaignId].eligibleReward);
            ...
            require(IERC20(preSale).balanceOf(address(this)) >= eligibleRwd, "Protokol:Not enough funds");
            investedCampaignDetails[msg.sender][_campaignId].claimedReward += eligibleRwd;
            // IERC20(preSale).transfer(msg.sender, eligibleRwd);
            _safeTransfer(preSale, msg.sender, eligibleRwd);
   ...
   */
    function _blackListedKOLFundsHandling(
        ...
        if (balance < _reward) {
            ...
            // IERC20(preSale).transfer(msg.sender, _reward);
            _safeTransfer(preSale, msg.sender, _reward);
            _reward = (campaigns[_campaignId].campaignData.requiredInvestment - _reward) / 1e18;
            // _reward = 0;
        }
        balance = 0;
        ...
        if (campaigns[_campaignId].campaignData.preSaleToken != _USDT) {
            ...
            if (campaigns[_campaignId].tgeDetails.tgeAmount > 0 || campaigns[_campaignId].tgeDetails.isTGE) {
                ...
                if (balance < investedCampaignDetails[msg.sender][_campaignId].leftOverInvestment) {
                    // IERC20(_USDT).transfer(msg.sender, balance);
                    _safeTransfer(_USDT, msg.sender, balance);
                } else {
        ...
        emit ClaimPreSaleTokensBlackListed(_campaignId, eligibleRwd, balance, preSale, msg.sender);
    }
   ...
   */
    ...
    function withdawPresaleByCampaignOwner(uint256 _campaignId) external {
        uint256 endDate = campaigns[_campaignId].campaignData.endDate;
        require(endDate < block.timestamp, "TA");
        uint256 tokenForOwner = (campaigns[_campaignId].campaignData.remainingInvestment /
            campaigns[_campaignId].campaignData.requiredInvestment) *
            campaigns[_campaignId].campaignData.marketingBudget;

        uint256 shareOfKols = (campaigns[_campaignId].campaignData.requiredInvestment -
            campaigns[_campaignId].campaignData.remainingInvestment) /
            campaigns[_campaignId].campaignData.requiredInvestment;
    }
}
