...
contract FuzzleEggs is ERC1155, Pausable, Ownable, ERC1155Supply, BlackholePrevention {
    using ECDSA for bytes32;

    uint256 public immutable EGG;
    ...
    mapping(address => uint8) public purchasePerAccount;
    mapping(address => bool) public whitelistClaimed;
    mapping(uint256 => uint256) public depositedForPublicMint;
    ...
    constructor() ERC1155("https://tokens.gala.games/endless-ai/fuzzles/fuzzles/close-encounters-club-card-diamond-edition.json") {
        EGG = 1;
        QUANTITY = 1;
        MINT_PRICE = 0.01 ether;
        MAX_SUPPLY = 3;
        MAX_PURCHASE = 2;
        isPublicMintAllowed = false;
    } 

    function name() external pure returns (string memory) {
        return "Fuzzleverse Eggs";
    }
    ...
    function mintWhitelist(bytes32[] calldata merkleProof) external whenNotPaused {
        require(whitelistMerkleRoot != "", 'merkle tree not set');
        require(totalSupply(EGG) < MAX_SUPPLY, 'reached max supply');
        require(purchasePerAccount[msg.sender] < MAX_PURCHASE, 'exceeds the max purchase');
        ...
        purchasePerAccount[msg.sender] = purchasePerAccount[msg.sender] + 1;

        _mint(msg.sender, EGG, QUANTITY, "");

        emit MintWhitelist(msg.sender, block.timestamp);        
    }
    ...
    function mintPublic() 
        ...
        whenNotPaused 
    {    
        require(isPublicMintAllowed, 'public mint not allowed');
        require(totalSupply(EGG) < MAX_SUPPLY, 'reached max supply');
        require(purchasePerAccount[msg.sender] < MAX_PURCHASE, 'exceeds the max purchase');
        require(msg.value >= MINT_PRICE, 'not enough eth to purchase');

        purchasePerAccount[msg.sender] = purchasePerAccount[msg.sender] + 1;
        depositedForPublicMint[EGG] += msg.value;

        _mint(msg.sender, EGG, QUANTITY, "");

        emit MintPublic(msg.sender, block.timestamp);        
    }
    ...
    function setIsPublicMintAllowed(bool isAllowed) external onlyOwner {
        isPublicMintAllowed = isAllowed;
    }
