...
contract Lottery is Ownable, ReentrancyGuard, RequestValidator {
    event TicketsBought(address wallet, uint256 amount, uint256 totalPrice);
    event EnteredDraw(address wallet, uint256 amount, uint256 lotteryId);

    uint256 private ticketPrice;
    address public beneficiary;
    ERC20 private Ammolite;
    bool public isOpen;
    uint8 public constant MINIMUM_PVNK_BALANCE = 1;
    ...
    modifier entryChecks(uint256 _numberOfTickets, uint256 _expiryTime) {
    ...
    function setAmmoliteContract(address _ammolite) external onlyOwner {
    ...
    function enterDraw(
        ...
        external
        entryChecks(_numberOfTickets, _expiryTime)
        verifyEntryRequest(
            _lotteryId,
            _numberOfTickets,
            _expiryTime,
        ...
        )
    {
        require(
            ...
            "You don't own enough tickets"
        );
        tickets[msg.sender] -= _numberOfTickets;
        nonces[msg.sender]++;
        emit EnteredDraw(msg.sender, _numberOfTickets, _lotteryId);
    }

    function buyAndEnterDraw(
        uint256 _numberOfTickets,
        uint256 _expiryTime,
        uint256 _lotteryId,
        bytes memory _signature
    ...
    {
        require(
            Ammolite.balanceOf(msg.sender) >= ticketPrice * _numberOfTickets,
            "Not enough Ammo"
        );
        nonces[msg.sender]++;
        for (uint256 i = 0; i < _numberOfTickets; i++)
            lotteries[_lotteryId].push(msg.sender);
        uint256 totalPrice = ticketPrice * _numberOfTickets;
        Ammolite.transferFrom(msg.sender, beneficiary, totalPrice);
        emit EnteredDraw(msg.sender, _numberOfTickets, _lotteryId);
    }

    function buyAndEnterDrawNoMap(
        uint256 _numberOfTickets,
        uint256 _expiryTime,
        uint256 _lotteryId,
        bytes memory _signature
    ...
    function buyTickets(
        ...
        external
        entryChecks(_numberOfTickets, _expiryTime)
        verifyPurchaseRequest(_numberOfTickets, _expiryTime, _signature)
    {
        require(
            Ammolite.balanceOf(msg.sender) >= ticketPrice * _numberOfTickets,
            "Not enough Ammo"
        ...
        Ammolite.transferFrom(msg.sender, beneficiary, totalPrice);
        tickets[msg.sender] += _numberOfTickets;
        emit TicketsBought(msg.sender, _numberOfTickets, totalPrice);
    }

    function buyTicketsNoMap(
        uint256 _numberOfTickets,
        uint256 _expiryTime,
        bytes memory _signature
    )
    ...
    function getTicketBalance(address wallet) external view returns (uint256) {
        return tickets[wallet];
    }

    function getLotteryEntries(uint256 _lotteryId)
        external
        view
