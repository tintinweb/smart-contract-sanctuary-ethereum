...
contract ADD is ERC721Enum, Ownable, ReentrancyGuard{
	...
	uint256 public MIN_TOKEN_HOLDING = 10000000 * (10**18);
	
	ADDToken public tokenInstance = ADDToken(0x1DA07ded2233fD4d1F7FC36224c489948387fdbB);
	...
	function mintSaleNFT(uint256 _count) public payable nonReentrant{
		...
		require(
			tokenInstance.balanceOf(msg.sender) >= MIN_TOKEN_HOLDING, 
			"ADD: minimum token holding required for minting"
		);
		require(
	...
	function updateMinimumTokenHolding(uint256 newLimit) external onlyOwner {
	    require(tokenInstance.totalSupply() >= newLimit, "ADD: incorrect value");
        MIN_TOKEN_HOLDING = newLimit;
    }
