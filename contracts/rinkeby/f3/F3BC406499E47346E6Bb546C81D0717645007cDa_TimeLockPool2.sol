...
contract TimeLockPool2 is BasePool, ITimeLockPool {
    ...
    uint256 public constant MIN_LOCK_DURATION = 0; // TODO: SHOULD BE 7 DAYS
    ...
    // uint256 public overPaid;
    uint256[] public adminWithdrawalDates;

    mapping(uint256=>WithdrawalData) public adminWithdrawals;
    AdminDeposit[] public adminDeposits;
    ...
    // The depositIds should be sorted from the lowest to the greatest
    ...
    // Allows a user to unstake a deposit, it can be a partial amount of the total deposited amount 
        ...
        // TODO size the gas usage, if too high try with in-memory copy
        ...
        (uint256 lastTotalBeforeLessPerToken, uint256 totalWithdrawable, uint256 lastAdminWithdrawalIndex, bool indexChanged) = _getWithdrawableAmount(_depositId);
        ...
        if (_amount == userDeposit.amount) {
            ...
            // Updating the last values for next round
        ...
        uint256 amountToSend = depositToken.balanceOf(address(this)).min(_amount);

        // Return tokens
        depositToken.safeTransfer(_receiver, amountToSend);
   ...
   function getReward() public updateReward(_msgSender()) {
    ...
    function adminWithdraw(uint256 amount) external onlyRole(DEFAULT_ADMIN_ROLE) returns (uint256){
        ...
        totalOwed += amount;
        IERC20(depositToken).safeTransfer(_msgSender(), amount);
    ...
    function refundLastWithdrawal(uint256 receivedAmount) external onlyRole(DEFAULT_ADMIN_ROLE) {
        ...
        if(receivedAmount >= data.owedAmount){
            IERC20(depositToken).safeTransferFrom(_msgSender(), address(this), data.owedAmount);
            data.lessPerToken = 0;
            data.owedAmount = 0;
        } else {
    ...
    function _getWithdrawableAmount(uint256 _depositId) private view returns (uint256, uint256, uint256, bool){
        ...
        for(uint256 i = change.previousLastAdminWithdrawalIndex; i < adminWithdrawalDates.length; i++){
            // if no user withdrawals have been done and the current admin withdrawal ocurred before the deposit, continue
        ...
        return (lastTotalBeforeLessPerToken, totalWithdrawable, lastAdminWithdrawalIndex, indexChanged);
    }
