...
contract Jiraverse is ERC721A, Ownable {
    ...
    function isWhitelisted(address _user, bytes32 [] calldata _merkleProof, uint _allocation) public view returns(bool) {
        ...
        return MerkleProof.verifyCalldata(_merkleProof, merkle_roots[_allocation], leaf);
    }
    
    function whitelistMint(bytes32 [] calldata _merkleProof, uint _quantity, uint _allocation) external {
        ...
        require(uint64(num_claimed) + _quantity <= _allocation, "You cannot claim that many tokens");

        IERC20(JIRA_TOKEN_ADDRESS).transferFrom(msg.sender, address(this), 50 * JIRA_TOKEN_DECIMALS * _quantity);
        _setAux(msg.sender, num_claimed + uint64(_quantity));
    ...
    function updateMerkleRoot(uint _allocation, bytes32 _new_root) external onlyOwner {
