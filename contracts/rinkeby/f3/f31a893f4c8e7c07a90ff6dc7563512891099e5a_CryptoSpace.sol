...
contract CryptoSpace {
    ...
    function registration(address userAddress, address referrerAddress) private {
        ...
        users[referrerAddress].referrerCounter++;
    ...
    function userSiteDatesTwo(address userAddress) public view returns(uint[16] memory freezedReferralReward, uint referrerQuantity) {
        uint[16] memory _freezedReferralReward;
        for (uint i = 1; i < 16; i++) {
            _freezedReferralReward[i] = users[userAddress].freezedReferralReward[i];
        }
        return (_freezedReferralReward, users[userAddress].referrerCounter);
    }
