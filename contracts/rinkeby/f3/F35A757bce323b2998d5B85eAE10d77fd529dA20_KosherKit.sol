...
contract KosherKit is ERC721Enumerable, Ownable, ReentrancyGuard {
    ...
    mapping(address => bool) publicMintClaimed;
    address[] public publicMinters;
    ...
    function whiteListMint(bytes32[] calldata _merkleProof) external payable nonReentrant callerIsUser validAmount validValue {
        ...
        require(!whiteListClaimed[msg.sender], "Already claimed WL");
    ...
    function mint() external payable nonReentrant callerIsUser validAmount validValue {
        ...
        require(!publicMintClaimed[msg.sender], "Already claimed");

        uint256 tokenID = setTokenID();

        publicMintClaimed[msg.sender] = true;
        publicMinters.push(msg.sender);
        idToExpirys[tokenID] = block.timestamp + 30 days;
        _safeMint(msg.sender, tokenID);
    }
    ...
    function resetPublicMinters() external onlyOwner {
        for (uint i; i < publicMinters.length; ++i) {
            publicMintClaimed[publicMinters[i]] = false;
        }
    }
