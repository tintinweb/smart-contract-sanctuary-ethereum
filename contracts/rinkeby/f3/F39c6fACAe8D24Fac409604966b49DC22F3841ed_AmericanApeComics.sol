...
contract AmericanApeComics is ERC721A, Ownable, ReentrancyGuard {
    ...
    */
    function mintComic(uint256 qty) external payable nonReentrant{
        require(tx.origin == msg.sender);
        require(mintIsActive, "Mint is not active");
        require(qty <= comics[comicMintId].maxPerTxnPublic, "You went over max tokens per transaction");
        require(totalSupply() + qty <= comics[comicMintId].endTokenId+1, "Not enough tokens left to mint that many");
        require(comics[comicMintId].tokenPricePublic * qty <= msg.value, "You sent the incorrect amount of ETH");
    ...
    */
    function burnMint(uint256 burnTokenId) external nonReentrant {        
        require(tx.origin == msg.sender);
        ...
        require(msg.sender == ownerOf(burnTokenId), "You do not own the token");
        require(burnTokenId >= comics[comicMintId].startBurnableTokenId && burnTokenId <= comics[comicMintId].endBurnableTokenId, "Invalid burnable comic");
        _burn(burnTokenId);
     ...
     */
    function isOnPresaleMerkle(bytes32[] calldata merkleProof, uint256 qty) public view returns(bool) {
        bytes32 leaf = keccak256(abi.encodePacked(msg.sender, qty));
        return MerkleProof.verify(merkleProof, comics[comicMintId].merkleRoot, leaf);
    }
    ...
    function initPresaleMerkleWalletList(address[] memory walletList, uint256 qty) external onlyOwner {
	    for (uint i; i < walletList.length; i++) {
		    comics[comicMintId].claimed[walletList[i]] = qty;
	    }
    }
    ...
    function checkAddressOnPresaleMerkleWalletList(address wallet) public view returns (uint256) {
	    return comics[comicMintId].claimed[wallet];
    }
    ...
    function mintPresaleMerkle(uint256 qty, uint256 maxQty, bytes32[] calldata merkleProof) external payable nonReentrant{
        ...
        require(
	        comics[comicMintId].tokenPricePublic * qty <= msg.value,
            "You sent the incorrect amount of ETH"
        );
        require(
            comics[comicMintId].claimed[msg.sender] + qty <= maxQty, 
            "Claim: Not allowed to claim given amount"
        );
        require(
            totalSupply() + qty <= comics[comicMintId].endTokenId+1, 
            "Not enough tokens left to mint that many"
        );

        bytes32 node = keccak256(abi.encodePacked(msg.sender, maxQty));
        require(
            MerkleProof.verify(merkleProof, comics[comicMintId].merkleRoot, node),
            "You have a bad Merkle Proof."
        );
        comics[comicMintId].claimed[msg.sender] = comics[comicMintId].claimed[msg.sender] + qty;
    ...
    */
    function burn(uint256 tokenId) public virtual {
        require(msg.sender == ownerOf(tokenId), "You do not own the token");
        _burn(tokenId);
    }
    ...
    */
    function burnBatch(uint256[] memory ids) external nonReentrant {
         for (uint256 i = 0; i < ids.length; i++) {
            require(msg.sender == ownerOf(ids[i]), "You do not own the token");
            _burn(ids[i]);
        }
    }
    ...
    */
    function comicExists(uint256 id) public view returns (bool) {
        return comics[id].endTokenId > 0;
    }
    ...
    */
    function checkClaimed(address wallet) external view returns (uint256[] memory) {
        ...
        for(uint256 i; i < counter.current(); i++) {
            result[i] = comics[i].claimed[wallet];
        }
    ...
    */
    function setComicMintId(uint256 id) external onlyOwner {
