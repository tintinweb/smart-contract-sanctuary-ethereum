...
abstract contract RequestGuildRole is ChainlinkClient {
    ...
    function requestAccessCheck(
        ...
        req.add(
            "get",
            string(abi.encodePacked("https://api.guild.xyz/v1/user/membership/", userAddress.toHexString()))
        );
        req.add("path", string(abi.encodePacked(guildIndex.toString(), ",roleIds")));
    ...
    modifier checkRole(bytes32 requestId, uint256[] memory returnedArray) {
        // NOTE: REMOVED FOR DEBUG PURPOSES
