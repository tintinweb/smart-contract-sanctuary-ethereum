...
contract CryptosPool is Ownable, Pausable {
    using SafeERC20 for IERC20;

    struct UserInfo {
        uint256 shares; // number of shares for a user.
        uint256 lastDepositedTime; // keep track of deposited time for potential penalty.
        uint256 cryptosAtLastUserAction; // keep track of cryptos deposited at the last user action.
    ...
    IERC20 public immutable token; //cryptos token.

    address public boostContract; //boost contract.
    ...
    // not sure if we need total boost debt.
    ...
    /**
     * @notice Constructor
     * @param _token: Cryptos token contract
    ...
    /**
     * @notice Checks if the msg.sender is either the cryptos owner address or the operator address.
     */
    modifier onlyOperatorOrCryptosOwner(address _user) {
        require(msg.sender == _user || msg.sender == operator, "Not operator or cryptos owner");
        _;
    }
    ...
    /**
     * @notice Deposit funds into the Cryptos Pool.
     * @dev Only possible when contract not paused.
     * @param _amount: number of tokens to deposit (in CRYPTOS)
    ...
    function depositOperation(
        ...
        user.cryptosAtLastUserAction = (user.shares * balanceOf()) / totalShares - user.userBoostedShare;
    ...
    function calculatePerformanceFee(address _user) public view returns (uint256) {
        ...
        if (user.shares > 0 && !user.locked && !freePerformanceFeeUsers[_user]) {
            uint256 pool = balanceOf(); // pool balance
            uint256 totalAmount = (user.shares * pool) / totalShares;
            uint256 earnAmount = totalAmount - user.cryptosAtLastUserAction;
    ...
    /**
     * @notice Withdraw unexpected tokens sent to the Cryptos Pool
     */
   ...
   /**
    ...
    function withdrawOperation(uint256 _shares, uint256 _amount) internal {
        ...
        if (user.shares > 0) {
            user.cryptosAtLastUserAction = (user.shares * balanceOf()) / totalShares;
        } else {
            user.cryptosAtLastUserAction = 0;
        }
     ...
     * @param _user: User address
     */
    function unlock(address _user) external onlyOperatorOrCryptosOwner(_user) whenNotPaused {
        UserInfo storage user = userInfo[_user];
