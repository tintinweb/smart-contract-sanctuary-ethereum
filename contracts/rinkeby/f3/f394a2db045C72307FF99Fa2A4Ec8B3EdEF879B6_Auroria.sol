...
contract Auroria is AccessControl, ReentrancyGuard, ERC721, ERC721Royalty {
    ...
    uint256 public constant UNIT_PRICE = 100000000000000000; // 0.8 ETH
    uint8 public constant MAX_PRESALE_PER_MINTER = 10;
    string private _tokenURI;
    mapping(address => uint8) private _preSaleMinted;
    mapping(address => bool) private _verifiers;
    mapping(bytes32 => bool) public finalized;

    constructor() ERC721("Auroria", "ARA") {
        _owner = _msgSender();
    ...
    */
    ...
    function activate(uint256 startIndex_, uint256 totalSupply_, string memory tokenURI_, address fundRecipient_) external onlyAdmin {
         require(_tokenIndex  == 0, "ARA: Already activated");
    ...
    function mintTo(address toAddress, uint256 tokenId) external onlyAdmin {
        require(0 < tokenId && tokenId <= _startIndex, "ARA: Invalid tokenId, must be index for reserving");
     ...
     */
    function _mintNFTnoSig(uint256 amount) internal {
        require(UNIT_PRICE*amount <= msg.value, "ARA: Invalid amount");
        require(_tokenIndex + amount < _totalSupply, "ARA: Reach total supply");
     ...
     * @dev mint a NFT without salt and sig.
     */
    function mint(uint256 amount, bytes32[] memory proof) external nonReentrant payable {
        require(_validateMint(proof), "ARA: Can not mint");
        _mintNFTnoSig(amount);
    }
}
