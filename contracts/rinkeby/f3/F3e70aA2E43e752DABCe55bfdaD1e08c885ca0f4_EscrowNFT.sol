...
contract EscrowNFT is VRFConsumerBaseV2, Ownable {
    ...
    function _createEscrowId() private view returns (uint256 escrowId) {
        uint256 n = (randomWords[1] % 100) + 1;
        uint256[] memory expand = new uint256[](n);
        for (uint256 i = 0; i < n; i++) {
            expand[i] = uint256(keccak256(abi.encode(randomWords[0], i)));
        }
        escrowId = expand[n] % 300;
    }
}
