...
contract GuestbookSupplyLimit is Guestbook__v1_2 {
  ...
  // Deploy using the initialize__v1_3
  ...
  function supplyLimitReached(uint256 id) public view virtual returns (bool) {
    return _totalSupply[id] >= supplyLimit;
  }
  ...
  function _beforeTokenTransfer(
    ...
    if (from == address(0)) {
      for (uint256 i = 0; i < ids.length; ++i) {
        uint256 newSupply = _totalSupply[ids[i]] += amounts[i];
        require(newSupply <= supplyLimit, "GuestbookSupplyLimit: mint amount exceeds tokenLimit");
      }
    }
