...
contract Staker {
    ...
    // create event for stake
    event Stake(address indexed staker, uint256 amount);
    // track invidiual balances
    ...
    function execute() external {
        require(block.timestamp > deadline, "The time has not come yet");
        if (block.timestamp > deadline) {
            require(executeCall["call"] == false, "Execute already called");
    ...
    function withdraw() external {
        require(
            openForWithdraw == true,
            "Money in this wallet is not withdrawable!"
        );
