...
contract LilMutantApeClub is ERC721A, Ownable, ReentrancyGuard {
    ...
    function mint(
        ...
        ERC20 lilBanana = ERC20(addresses.coinAddress);
        if (maxSupply <= 5000) {
            require(
                amount <= mintLimit,
                "Mint limit exceeded."
        ...
        lilBanana.transferFrom(addresses.coinOwnerAddress, msg.sender, amount * 1e18);
    }
    ...
    function claim() external nonReentrant rewardTime(msg.sender) {
        uint256 userBalance = ERC20(addresses.lilBCoinAddress).balanceOf(msg.sender);
        uint256 totalReward = block.timestamp - claimingDetails[msg.sender].lastClaimed / 1 days;
        require((totalReward * 1e18) * userBalance >= 1 * 1e18, "Not enough rewards to claim");
        claimingDetails[msg.sender].lastClaimed = block.timestamp;
