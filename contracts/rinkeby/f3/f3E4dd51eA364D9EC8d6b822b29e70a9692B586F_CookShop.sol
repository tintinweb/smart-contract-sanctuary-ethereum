...
contract CookShop is Ownable, ERC721Holder {
    ...
    function getStakedDuration(uint256 tokenId) public view returns (uint256) {
        uint256 stakeStartTimeForToken = tokenIdToLastStakedTimestamp[tokenId];
        require(
            stakeStartTimeForToken != 0,
            "stake start time is not recorded"
        );
        console.log("stakeStartTimeForToken", stakeStartTimeForToken);
        uint256 ts = _min(block.timestamp, stakingEndTimestamp);
        console.log("ts", ts);
        uint256 validStakedDuration = ts - stakeStartTimeForToken;

        return validStakedDuration;
    }
     ...
     * @param tokenIds array of the tokenIds users have in their wallet
     */
