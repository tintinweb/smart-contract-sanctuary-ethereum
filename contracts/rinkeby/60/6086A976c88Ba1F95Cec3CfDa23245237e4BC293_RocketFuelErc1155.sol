...
contract RocketFuelErc1155 is ERC1155Supply, ERC1155Burnable, Ownable {
    struct MintContext {
        bool isSaleActive;
        bool isMintListActive;
        bool isOnMintList;
        MintConfig globalConfig;
        MintConfig captainConfig;
        uint256 remainingGlobalPresaleMintCount;
        uint256 remainingCaptainPresaleMintCount;
    }
    ...
    mapping(uint256 => mapping(address => uint256)) public mintList;
    string public name;
    string public symbol;

    uint256 public constant TOKEN_ID_GLOBAL = 0;
    uint256 public constant TOKEN_ID_CAPTAIN = 1;
    ...
    function setIsMintListActive(bool _isMintListActive) external onlyOwner {
        isMintListActive = _isMintListActive;
    }

    function setMintList(address[] calldata _addresses) external onlyOwner {
        for (uint256 i = 0; i < _addresses.length; i++) {
            presaleAddresses[_addresses[i]] = true;
            mintList[TOKEN_ID_GLOBAL][_addresses[i]] = configs[TOKEN_ID_GLOBAL].maxPresaleMint;
            mintList[TOKEN_ID_CAPTAIN][_addresses[i]] = configs[TOKEN_ID_CAPTAIN].maxPresaleMint;
        }
    }

    function requireExists(uint256 _tokenId) internal {
        require(exists(_tokenId), "token id does not exist");
    }

    function requireLessThanMaxSupply(uint256 _tokenId, uint256 _numOfTokens) internal {
        require(_numOfTokens <= configs[_tokenId].maxSupply, "Purchase would exceed max supply");
    }
     ...
     * @dev Overriding `isApprovedForAll` to mintlist user's OpenSea proxy accounts
     */
    function isApprovedForAll(address owner, address operator) public view override returns (bool) {
        if (isOpenSeaProxyActive && address(ProxyRegistry(proxyRegistryAddress).proxies(owner)) == operator) {
            return true;
        }
    ...
    function getMintContext() external view returns (MintContext memory) {
        return
            MintContext({
                ...
                isOnMintList: presaleAddresses[msg.sender],
                globalConfig: configs[TOKEN_ID_GLOBAL],
                captainConfig: configs[TOKEN_ID_CAPTAIN],
                remainingGlobalPresaleMintCount: mintList[TOKEN_ID_GLOBAL][msg.sender],
                remainingCaptainPresaleMintCount: mintList[TOKEN_ID_CAPTAIN][msg.sender]
            });
    }

    function giftGlobal(address[] calldata _receivers, uint256[] calldata _numberOfTokens) external onlyOwner {
        gift(TOKEN_ID_GLOBAL, _receivers, _numberOfTokens);
    }

    function giftCaptain(address[] calldata _receivers, uint256[] calldata _numberOfTokens) external onlyOwner {
        gift(TOKEN_ID_CAPTAIN, _receivers, _numberOfTokens);
    }
    ...
    function mintPresaleGlobal(uint256 _numberOfTokens) external payable {
        mintPresale(TOKEN_ID_GLOBAL, _numberOfTokens);
    }

    function mintPresaleCaptain(uint256 _numberOfTokens) external payable {
        mintPresale(TOKEN_ID_CAPTAIN, _numberOfTokens);
    }

    function mintPresale(uint256 _tokenId, uint256 _numberOfTokens) internal {
        requireExists(_tokenId);
        require(isMintListActive, "Mint list is not active");
        require(_numberOfTokens <= mintList[_tokenId][msg.sender], "Exceeded max available to purchase");
        ...
        mintList[_tokenId][msg.sender] -= _numberOfTokens;
        configs[_tokenId].currentMintCount += _numberOfTokens;
        _mint(msg.sender, TOKEN_ID_GLOBAL, _numberOfTokens, "");
    }

    function mintCaptain(uint256 _numberOfTokens) external payable {
        mint(TOKEN_ID_CAPTAIN, _numberOfTokens);
    }

    function mintGlobal(uint256 _numberOfTokens) external payable {
        mint(TOKEN_ID_GLOBAL, _numberOfTokens);
    }
    ...
    function mint(uint256 _tokenId, uint256 _numberOfTokens) internal {
        ...
        require(_numberOfTokens <= configs[_tokenId].maxPublicMint, "Exceeded max token purchase");
        require(configs[_tokenId].pricePerToken * _numberOfTokens <= msg.value, "Ether value sent is not correct");
