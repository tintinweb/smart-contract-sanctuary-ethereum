...
contract DataLayout is LibraryLock {
    ...
    mapping(address => uint) public whiteListMintCountClassOne;
    mapping(address => uint) public whiteListMintCountClassTwo;
    mapping(address => uint) public whiteListMintCountClassThree;

    bytes32 public merkleRootClassOne = 0x84bf2e45e4b23bb29373da83131ac7774bb0a76c96ffdcbd159a41ed652646b7;
    bytes32 public merkleRootClassTwo = 0xb8a2a15886fd6495cf67297c5e429d8c3d72c1ea8835bceec15e138920867def;
    bytes32 public merkleRootClassThree = 0xb4069c60c73a82a9308f56ce13b32425ecb3120b774102c315d3643f20b77cb1;
    bool public isMintingPause;
} 

contract DonkeyNFT is ERC721Enumerable, DataLayout, Proxiable, Ownable {
    using SafeMath for uint256;
    
    constructor() ERC721("Don-keys", "DON-KEY") {
    ...
    function NFTConstructor(address _manager) public payable {
        ...
        gateWay = "https://don-keytest.mypinata.cloud/ipfs/";
        proxyConstructor("Don-keys", "Don-key");
        mintCost = 0.01 ether;
        mintLimit = 3000;
    ...
    function pauseMinting(bool _permission) external onlyOwner {
        isMintingPause = _permission;
    }
    ...
    function createNFTClassOne(uint amount, address claimer, bool ownerMint, bytes32[] calldata _merkleProof) public payable delegatedOnly {
        require(!isMintingPause, "Minting is currently paused");
        require(inWhitelistMode, "Whitelist only");
        bytes32 leaf =  keccak256(abi.encodePacked(msg.sender));
        require(MerkleProof.verify(_merkleProof, merkleRootClassOne, leaf));
        uint currentMintCount = whiteListMintCountClassOne[msg.sender];
        if (!ownerMint) {
            require(amount > 0 && amount <= 10, "Invalid mint amount");
            require(currentMintCount + amount <= 10, "Invalid mint amount");
        ...
        whiteListMintCountClassOne[msg.sender] = currentMintCount + amount;
    }
    
    function createNFT(uint amount, address claimer, bool ownerMint) public payable delegatedOnly {
        require(!isMintingPause, "Minting is currently paused");
        require(!inWhitelistMode, "Non-whitelist only");
        if (!ownerMint) {
            require(amount > 0 && amount <= 10, "Invalid mint amount");
            require(msg.value == mintCost.mul(amount), "Invalid message value");
        } else {
