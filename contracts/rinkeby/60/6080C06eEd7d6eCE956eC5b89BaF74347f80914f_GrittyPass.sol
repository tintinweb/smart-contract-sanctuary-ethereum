...
contract GrittyPass is ERC1155Supply, Ownable {
	using ECDSA for bytes32;

	uint256 public constant GRITTY_PASS = 0;
	...
	event TokensMinted(address mintedBy, uint256 tokensNumber);
	...
	function purchase(uint256 tokensNumber, bytes calldata signature)
		public
		payable
	{
		if (_msgSender() != owner()) {
		...
		require(
			totalSupply(GRITTY_PASS) + tokensNumber <= MAX_SUPPLY,
			"You tried to mint more than the max allowed"
		);

		if (presaleOnly && _msgSender() != owner()) {
			require(
				_validateSignature(signature, _msgSender()),
				"Your wallet is not on the presale list"
			);
		}

		_mint(_msgSender(), GRITTY_PASS, tokensNumber, "");
	...
	function _validateSignature(bytes calldata signature, address caller)
		internal
		view
		returns (bool)
	{
	...
	function setTokenPrice(uint256 _tokenPrice) public onlyOwner {
		require(!saleIsActive(), "Price cannot be changed when sale is active");
	...
	function saleIsActive() public view returns (bool) {
		if (
			...
			totalSupply(GRITTY_PASS) == MAX_SUPPLY ||
			isSalePaused
		) {
			return false;
		}
