...
// File: contracts/sellitMarketplace.sol
...
contract sellItMarketPlace is ERC721, ERC721Enumerable, ERC721URIStorage, ERC721Burnable, Ownable {
...
//NFT Auction structures
    ...
    // NFT pendings for lower biders
      mapping(address => uint) public pendingResturns;

    //   NOn NFT properties
     bool nonnftended =false;
    event nonNftHighestBidIcrease(address bidder, uint amount);
    ...
    mapping (uint256 => nonNftAuction) public nonNftauctions;
      mapping(address => uint) public nonNftpendingResturns;

    constructor()
    
    ERC721("sellItMarket", "SIM") {
        }
    ...
    function safeMint(address to, string memory uri) public {
     ...
     function nonNftstartAuction(uint _biddingendtime , address payable _beneficiary , uint256 nonNftId) public {
       ...
       function bidNonNft( uint256 nonNftId) payable public{
        ...
        address  currentBidOwner = nonNftauctions[nonNftId].currentBidOwner;
        uint256  currentBidAmount = nonNftauctions[nonNftId].currentBidAmount;
        ...
        if( currentBidAmount !=0) {
            nonNftpendingResturns[currentBidOwner] += currentBidAmount;
        }
        if(msg.value > currentBidAmount ) {
            payable(currentBidOwner).transfer(currentBidAmount);
        }
        nonNftauctions[nonNftId].currentBidOwner = msg.sender;
        nonNftauctions[nonNftId].currentBidAmount = msg.value;
        nonNftauctions[nonNftId].highestBidder =  nonNftauctions[nonNftId].currentBidOwner;
        nonNftauctions[nonNftId].highestBid =  nonNftauctions[nonNftId].currentBidAmount;
        emit nonNftHighestBidIcrease(msg.sender , msg.value);
    }
     function nonNftAuctionEnd( uint256 nonNftId) public{
        if(block.timestamp < nonNftauctions[nonNftId].auctionEndTime){
        ...
        // }
       nonnftended = true ;
        nonNftauctions[nonNftId].amountStatus=true;
        nonNftauctions[nonNftId].OpenForBidding=false;
    }
        function NonNftconfirmbidding( uint256 nonNftId) public{
          address nonNfthighestbidder=nonNftauctions[nonNftId].highestBidder;
          require (msg.sender == nonNfthighestbidder ,"Only HighestBidder can call this method");
   ...
   //******* NFT START **********//
     ...
     function startAuction(uint _biddingendtime , address payable _beneficiary , uint256 tokenId) public {
...
// ****Autions records *****//
         function auctiondata(uint256 tokenid) public view returns(uint256, bool , uint256, address , bool ) {
...
// bidding and return pendings//
        ...
        auctions[tokenId].highestBidder =  auctions[tokenId].currentBidOwner;
        auctions[tokenId].highestBid =  auctions[tokenId].currentBidAmount;
    ...
    // *****Payment and token transfer *****//
        function confirmbidding( uint256 tokenId) public{
         require (msg.sender != ownerOf(tokenId) ,"Only buyer can call this method");
          address nfthighestbidder=auctions[tokenId].highestBidder;
          require (msg.sender == nfthighestbidder ,"Only HighestBidder can call this method");
          emit AuctionEnded(auctions[tokenId].highestBidder , auctions[tokenId].highestBid);
         _transfer(auctions[tokenId].beneficiary,msg.sender,tokenId);
        payable(auctions[tokenId].beneficiary).transfer(auctions[tokenId].highestBid);
    ...
    { 
       ...
       // *****Buy Non-NFTs in Marketplace *****//
        ...
        //  nonNftBuyerReturns[msg.sender] = msg.value;
        //   nonNftBuyAddress[nonNftId] = nonNftBuyerReturns[msg.sender];
    ...
    function nonNftDelivery(uint256 nonNftId,address seller) external {
         ...
         require (nonNftstate[nonNftId]==false,"ALREADY CANCLE ORDER"); 
         uint256 amount = nonNftBuyerReturns[nonNftId];
          address nonNftbuyer=nonNftbuyerAddress[nonNftId];
        //   require(currState == State.AWAITING_PAYMENT, "Cancle Order");
        // currState = State.AWAITING_DELIVERY;
          require (msg.sender == nonNftbuyer ,"Not buyer");
     ...
     function nonNftcancelDelivery(uint256 nonNftId) public
    {
         ...
         nonNftBuyerReturns[nonNftId]=0;
          ...
          // *****ENDing     non NFTs in Marketplace *****//
