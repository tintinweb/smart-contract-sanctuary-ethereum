...
contract SampleERC20 is IScales, ERC20, AccessControl, Pausable, ReentrancyGuard {
    IKaijuKingz public immutable KAIJU;
    IRWaste public immutable RWASTE;
    uint256 public constant BASE_RATE = 5 ether;
    uint256 public constant GENESIS_MULTIPLIER = 2;
    uint256 public constant MAX_PER_TX = 25;
    uint256 public constant TOTAL_GENESIS = 3333;
    ...
    /**
     * @notice Get KAIJU ID by account and index
    ...
    /**
     * @notice Get list of KAIJU IDs by account
     * @param account The address of the token owner
     * @return The token IDs owned by the specified address
     */
    ...
    /**
     * @notice Get amount of claimable $SCALES
    ...
    function _getPending(address account) internal view returns (uint256) {
        return (stakedTokens[account] + (genesisStaked[account] * GENESIS_MULTIPLIER))
        * BASE_RATE
    ...
    function withdraw() public whenNotPaused nonReentrant {
        require(getSpendable(_msgSender()) > 0, "Nothing to claim");

        _mint(_msgSender(), getSpendable(_msgSender()));
    ...
    function deposit(uint256 amount) public whenNotPaused {
        ...
        stash[_msgSender()] += amount;
    }

    /**
     * @notice Stake KAIJUs and start earning $SCALES
     * @param tokenIds The tokens IDs to stake
     */
    function stake(uint256[] calldata tokenIds) external whenNotPaused nonReentrant {
        require(
            KAIJU.isApprovedForAll(_msgSender(), address(this)),
            "Contract not approved"
        );
        require(
            tokenIds.length > 0 && tokenIds.length <= MAX_PER_TX,
            "Invalid amount"
        );
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            ...
            tokenOwners[current] = _msgSender();
            tokenIndex[current] = stakedTokens[_msgSender()];
            ownedTokens[_msgSender()][tokenIndex[current]] = current;

            stakedTokens[_msgSender()] += 1;
            if (current < TOTAL_GENESIS) genesisStaked[_msgSender()] += 1;

            KAIJU.transferFrom(_msgSender(), address(this), current);
        }
    }

    /**
     * @notice Remove KAIJUs and optionally collect $SCALES
    ...
    function unstake(uint256[] calldata tokenIds) external nonReentrant {
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            uint256 last = ownedTokens[_msgSender()][stakedTokens[_msgSender()] - 1];

            tokenOwners[tokenIds[i]] = address(0);
            tokenIndex[last] = tokenIndex[tokenIds[i]];
            ownedTokens[_msgSender()][tokenIndex[tokenIds[i]]] = last;

            stakedTokens[_msgSender()] -= 1;
            if (tokenIds[i] < TOTAL_GENESIS) genesisStaked[_msgSender()] -= 1;

            KAIJU.transferFrom(address(this), _msgSender(), tokenIds[i]);
        }
    }
     ...
     * @param from The account to spend tokens from
    ...
    function spend(address from, uint256 amount) public override onlyRole(SPENDER_ROLE) {
        require(amount <= getSpendable(from), "Insufficient funds");
     ...
     * @dev Used only by contracts for extending token utility
     * @param to The account to credit tokens to
     ...
     */
    function recoveryTransfer(address to, uint256 tokenId) external onlyRole(DEFAULT_ADMIN_ROLE) {
