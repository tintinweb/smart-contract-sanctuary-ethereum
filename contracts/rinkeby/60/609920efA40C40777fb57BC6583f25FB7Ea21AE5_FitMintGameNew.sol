...
interface FitMintTokenNew {
    function mintTokens(address userAddress, uint256 tokenAmount) external;
}

contract FitMintGameNew is Ownable {
    FitMintTokenNew gameTokenInstance;
    // struct pnftBalance {
    //     uint256 tokenId;
    // }
    struct userStruct {
    ...
    constructor(address _token_address) public {
        gameTokenInstance = FitMintTokenNew(_token_address);
    }
    function userClaim(uint tokenAmount) public {
        require(userAccount[msg.sender].tokenBalance >= tokenAmount);
        gameTokenInstance.mintTokens(msg.sender, tokenAmount);
