...
interface ArtifactsInterface {
    function burnAsBurner(address from, uint256 tokenId, uint256 amount) external;
    ...
    Ownable
{
    ...
    address private artifactMinterAddress;
    ...
    constructor (address artifactsAddress, address sssAddress) ERC721("The Repus", "REPUS") {
    ...
    function setATFMinterAddress(address atfMinter) external onlyOwnerOrManager {
        artifactMinterAddress = atfMinter;
    }

    function setArtifactsContract(address _artifactsContract) external onlyOwnerOrManager {
    ...
    function mint(uint256 amount) external {
        require(artifactsContract.balanceOf(msg.sender, ARTIFACT_ID) >= amount, 'not enough artifacts');
    ...
    function directMint( address to, uint256 amount) external {
        require(msg.sender == artifactMinterAddress, 'illegal mint');
        for (uint256 i =0; i < amount; i++) {
            totalSupply = totalSupply + 1;
    ...
    function tokenURI(uint256 _tokenId) public view  virtual override returns (string memory) {
        return string(abi.encodePacked(super.tokenURI(_tokenId), endingUri));
    }
