...
contract KingdomStructures is Initializable, UUPSUpgradeable, ERC721Upgradeable, ERC721EnumerableUpgradeable, OwnableUpgradeable, ERC2981PerTokenRoyalties {
    using StringsUpgradeable for uint256;

    uint256 public constant MAX_MINTABLE = 20000; // 5000 total for this launchpad
    uint256 public constant MAX_PER_CLAIM = 10;
    uint256 public constant ROYALTY_VALUE = 300; // 100 = 1%, 200 = 2% etc...  3% for this launchpad
...
//
    ...
    modifier OnlyContractOne() {
    require(msg.sender == contractOne);
    _;
  }
...
//
//
function claim(uint256 n, address _sender) external OnlyContractOne{
        require(n + minted <= 20000, "Not enough left to mint.");
        require(n > 0, "Number need to be higher than 0");


        for (uint256 i = 0; i < n; i++) {
            randomMint(_sender, minted);
...
function freeMint(uint256 n) public onlyOwner{
        require(n + minted <= 20000, "Not enough left to mint.");
...
//
    ...
    function setContractOne (address _contractOne) external onlyOwner {
...
contract KingdomStructuresV2 is KingdomStructures {
...
contract KingdomStructuresV3 is KingdomStructures {
    // Mapping from owner address to token ID
    mapping(address => uint[]) private _ownersIds;
