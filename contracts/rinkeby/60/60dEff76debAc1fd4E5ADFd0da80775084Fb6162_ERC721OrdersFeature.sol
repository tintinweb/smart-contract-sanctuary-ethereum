    ...
    NFTOrders
{
    ...
    function buyERC721(
        ...
        _buyERC721(sellOrder, signature, msg.value);
        ...
        returns (uint256 profit)
    {
        ...
        if (sellOrder.nft != buyOrder.nft) {
            revert("ERC721_TOKEN_MISMATCH_ERROR");
        }
        ...
        if (
            ...
            if (sellOrder.erc20Token != buyOrder.erc20Token) {
                revert("ERC20_TOKEN_MISMATCH_ERROR");
            }
    ...
    function _validateOrderSignature(
        ...
        if (signature.signatureType == LibSignature.SignatureType.PRESIGNED) {
            require(
                LibERC721OrdersStorage.getStorage().preSigned[orderHash] ==
            ...
            require(
                maker != address(0) &&
                maker == LibSignature.getSignerOfHash(orderHash, signature),
    ...
    function getERC721SellOrderStatus(LibNFTOrder.NFTSellOrder memory order)
        public
        override
        view
    ...
    /// @param order The ERC721 buy order.
    /// @return status The status of the order.
    function getERC721BuyOrderStatus(LibNFTOrder.NFTBuyOrder memory order)
        public
        override
        view
        returns (LibNFTOrder.OrderStatus status)
    ...
    function getERC721SellOrderHash(LibNFTOrder.NFTSellOrder memory order)
        public
        override
        view
    ...
    /// @param order The ERC721 buy order.
    /// @return orderHash The order hash.
    function getERC721BuyOrderHash(LibNFTOrder.NFTBuyOrder memory order)
        public
        override
        view
        returns (bytes32 orderHash)
...
interface IERC721OrdersFeature {
    ...
    /// @param order The ERC721 sell order.
    /// @return status The status of the order.
    function getERC721SellOrderStatus(LibNFTOrder.NFTSellOrder calldata order)
        external
        view
        returns (LibNFTOrder.OrderStatus status);
    ...
    /// @param order The ERC721 buy order.
    /// @return status The status of the order.
    function getERC721BuyOrderStatus(LibNFTOrder.NFTBuyOrder calldata order)
        external
        view
        returns (LibNFTOrder.OrderStatus status);
    ...
    /// @param order The ERC721 sell order.
    /// @return orderHash The order hash.
    function getERC721SellOrderHash(LibNFTOrder.NFTSellOrder calldata order)
        external
        view
        returns (bytes32 orderHash);
    ...
    /// @param order The ERC721 buy order.
    /// @return orderHash The order hash.
    function getERC721BuyOrderHash(LibNFTOrder.NFTBuyOrder calldata order)
        external
        view
        returns (bytes32 orderHash);
...
library LibNFTOrder {
    ...
    //    ")"
    // ))
    uint256 private constant _NFT_SELL_ORDER_TYPE_HASH =
        0xed676c7f3e8232a311454799b1cf26e75b4abc90c9bf06c9f7e8e79fcc7fe14d;
    ...
    //    ")"
    // ))
    uint256 private constant _NFT_BUY_ORDER_TYPE_HASH =
        0xa525d336300f566329800fcbe82fd263226dc27d6c109f060d9a4a364281521c;
    ...
    //    ")"
    // ))
    uint256 private constant _ERC_1155_SELL_ORDER_TYPE_HASH =
        0x3529b5920cc48ecbceb24e9c51dccb50fefd8db2cf05d36e356aeb1754e19eda;
    ...
    //    ")"
    // ))
    uint256 private constant _ERC_1155_BUY_ORDER_TYPE_HASH =
        0x1a6eaae1fbed341e0974212ec17f035a9d419cadc3bf5154841cbf7fd605ba48;
    ...
    //    ")"
    // ))
    uint256 private constant _FEE_TYPE_HASH =
    ...
    //    ")"
    // ))
    uint256 private constant _PROPERTY_TYPE_HASH =
    ...
    function asNFTSellOrder(NFTBuyOrder memory nftBuyOrder)
        internal
        pure
        returns (NFTSellOrder memory nftSellOrder)
    {
        assembly {
            nftSellOrder := nftBuyOrder
        }
    }

    function asNFTSellOrder(ERC1155SellOrder memory erc1155SellOrder)
        internal
        pure
        returns (NFTSellOrder memory nftSellOrder)
    {
        assembly {
            nftSellOrder := erc1155SellOrder
        }
    }

    function asNFTBuyOrder(ERC1155BuyOrder memory erc1155BuyOrder)
        internal
        pure
        returns (NFTBuyOrder memory nftBuyOrder)
    {
        assembly {
            nftBuyOrder := erc1155BuyOrder
        }
    }

    function asERC1155SellOrder(NFTSellOrder memory nftSellOrder)
        internal
        pure
        returns (ERC1155SellOrder memory erc1155SellOrder)
    {
        assembly {
            erc1155SellOrder := nftSellOrder
        }
    }

    function asERC1155BuyOrder(NFTBuyOrder memory nftBuyOrder)
        internal
        pure
        returns (ERC1155BuyOrder memory erc1155BuyOrder)
    {
        assembly {
            erc1155BuyOrder := nftBuyOrder
        }
    }
    ...
    function getNFTSellOrderStructHash(NFTSellOrder memory order, uint256 hashNonce)
        internal
        pure
    ...
    /// @return structHash The struct hash of the order.
    function getNFTBuyOrderStructHash(NFTBuyOrder memory order, uint256 hashNonce)
        internal
        pure
        returns (bytes32 structHash)
    {
    ...
    /// @return structHash The struct hash of the order.
    function getERC1155SellOrderStructHash(ERC1155SellOrder memory order, uint256 hashNonce)
        internal
        pure
        returns (bytes32 structHash)
    {
    ...
    /// @return structHash The struct hash of the order.
    function getERC1155BuyOrderStructHash(ERC1155BuyOrder memory order, uint256 hashNonce)
        internal
        pure
        returns (bytes32 structHash)
    {
...
library LibSignature {
        ...
        returns (address recovered)
    {
        ...
        if (uint256(signature.r) >= ECDSA_SIGNATURE_R_LIMIT ||
            ...
            signature.signatureType == SignatureType.ILLEGAL ||
            signature.signatureType == SignatureType.INVALID ||
            signature.signatureType == SignatureType.PRESIGNED) {
            revert("SIGN_CHECK_ERR");
        }
...
abstract contract NFTOrders is FixinEIP712, FixinTokenSpender {
    ...
    function _sellNFT(
        ...
        internal
        returns (uint256 erc20FillAmount, bytes32 orderHash)
    {
        ...
        if (params.sellAmount == orderInfo.orderAmount) {
            ...
            erc20FillAmount =
                buyOrder.erc20TokenAmount * params.sellAmount / orderInfo.orderAmount;
        }

        if (params.unwrapNativeToken) {
            // The ERC20 token must be WETH for it to be unwrapped.
            if (buyOrder.erc20Token != WETH) {
                revert("_sellNFT/ERC20_TOKEN_MISMATCH_ERROR");
            }
        ...
        returns (uint256 erc20FillAmount, bytes32 orderHash)
    {
        ...
        if (buyAmount == orderInfo.orderAmount) {
            erc20FillAmount = sellOrder.erc20TokenAmount;
        } else {
            // Rounding favors the order maker.
            erc20FillAmount = _ceilDiv(
                sellOrder.erc20TokenAmount * buyAmount,
                orderInfo.orderAmount
            );
        }
        ...
        returns (uint256 erc20FillAmount, bytes32 orderHash)
    {
        ...
        if (params.buyAmount > orderInfo.remainingAmount) {
            revert("_buyNFTEx/EXCEEDS_REMAINING");
        }
        ...
        if (params.buyAmount == orderInfo.orderAmount) {
            ...
            // Rounding favors the order maker.
            erc20FillAmount = _ceilDiv(
                sellOrder.erc20TokenAmount * params.buyAmount,
                orderInfo.orderAmount
            );
        }
        ...
        if (params.takerCallbackData.length > 0) {
            require(
                ...
                "_buyNFTEx/CANNOT_CALLBACK_SELF"
            );
            ...
            require(
                callbackResult == TAKER_CALLBACK_MAGIC_BYTES,
                "_buyNFTEx/CALLBACK_FAILED"
            );
        }

        if (address(sellOrder.erc20Token) == NATIVE_TOKEN_ADDRESS) {
            uint256 totalPaid = erc20FillAmount +
                _calcTotalFeesPaid(sellOrder.fees, params.buyAmount, orderInfo.orderAmount);
    ...
    function _validateBuyOrder(
        ...
        require(
            ...
            "_validateBuyOrder/TOKEN_MISMATCH"
        );
        // Taker must match the order taker, if one is specified.
        if (buyOrder.taker != address(0) && buyOrder.taker != taker) {
            revert("_validateBuyOrder/ONLY_TAKER");
        }
        // Check that the order is valid and has not expired, been cancelled,
        // or been filled.
        if (orderInfo.status != LibNFTOrder.OrderStatus.FILLABLE) {
    ...
    function _resetDutchAuctionTokenAmountAndFees(
        LibNFTOrder.NFTSellOrder memory order,
        uint256 count
    ) private view {
    ...
    function _calcTotalFeesPaid(
        LibNFTOrder.Fee[] memory fees,
        uint256 fillAmount,
        uint256 orderAmount
    )
        private
        ...
        returns (uint256 totalFeesPaid)
    {
        for (uint256 i = 0; i < order.fees.length; i++) {
            ...
            if (fillAmount == orderAmount) {
                ...
                feeFillAmount = fee.amount * fillAmount / orderAmount;
            }
    ...
    function _validateOrderProperties(
        ...
        if (order.nftProperties.length == 0) {
            if (tokenId != order.nftId) {
                revert("_validateProperties/TOKEN_ID_ERR");
            }
        } else {
...
abstract contract FixinEIP712 {
    ...
    function _getEIP712Hash(bytes32 structHash) internal view returns (bytes32) {
        return keccak256(abi.encodePacked(
            hex"1901",
            keccak256(abi.encode(DOMAIN, NAME, VERSION, CHAIN_ID, address(this))),
            structHash
        ));
    }
}
...
abstract contract FixinTokenSpender {
    ...
    function _transferERC20TokensFrom(
        ...
        internal
    {
        require(address(token) != address(this), "_transferERC20From/CANNOT_SELF");
    ...
    function _getSpendableERC20BalanceOf(IERC20 token, address owner)
        internal
        view
        returns (uint256)
    {
