...
contract NaxapFactory is Initializable,OwnableUpgradeable, ERC1155HolderUpgradeable, ERC721HolderUpgradeable {
    using EnumerableMap for EnumerableMap.UintToAddressMap;
    address public Naxapimplementation;
    address public revenueChannelImplementation;
    EnumerableMap.UintToAddressMap private naxapNFTs;
    ...
    event ERC721Locked(address locker, address tokenAddress, address naxap, uint256 tokenId);
    event ERC721UnLocked(address owner, uint256 tokenId, address collateralNft, uint256 index);
    event ERC1155Locked(address locker, address tokenAddress, address naxap, uint256 tokenId);
    ...
    event NaxapProtocolUpgraded(address _newAddress);

    // constructor(address _implementation, address _revenueChannelImplementation) {
    //     Naxapimplementation = _implementation;
...
// Admin Functions
//////////////////////////////////
    function setNaxapImplementation(address _newAddress) external onlyOwner {
      Naxapimplementation = _newAddress;
      emit NaxapProtocolUpgraded(_newAddress);
    }
...
//////////////////////////////////
    function mint(string memory urlIpfs, uint16 majority, string memory _name, string memory _symbol) public returns (address _clone) {
      _clone = ClonesUpgradeable.clone(Naxapimplementation);
      NaxapNFT(_clone).init(_msgSender(), revenueChannelImplementation, urlIpfs, majority,_name,_symbol);
      uint256 index = naxapNFTs.length();
      naxapNFTs.set(index, _clone);
    ...
    function importERC721(address _tokenAddress, uint256 _tokenId, uint16 majority) external returns (address _clone) {
      ...
      name = string(abi.encodePacked("NaxapNFT(",name,")"));
      ...
      NaxapNFT(_clone).setCollateral(_tokenAddress);
      lockedERC721s[_clone] = nft;
      NaxapNFT(_clone).safeTransferFrom(address(this), _msgSender(), 0, 1, '');
    ...
    function claimERC721(uint256 _tokenId) external {
      address naxapAddress = naxapNFTs.get(_tokenId);
      ERC721Imported storage collateralNft = lockedERC721s[naxapAddress];
      address abandonedNaxap = collateralNft.tokenAddress;
      uint256 abandonedIndex = collateralNft.tokenIndex;
      NaxapNFT(naxapAddress).safeTransferFrom(_msgSender(), address(this),0,1,'');
      NaxapNFT(naxapAddress).claimContainedERC721(abandonedNaxap,abandonedIndex);
      ERC721Upgradeable(collateralNft.tokenAddress).transferFrom(address(this), _msgSender(), collateralNft.tokenIndex);
      naxapNFTs.set(_tokenId, address(0));
      lockedERC721s[naxapAddress] = ERC721Imported(address(0),0);
      emit ERC721UnLocked(_msgSender(), _tokenId, abandonedNaxap, abandonedIndex);
    }
    function claimERC1155(uint256 _tokenId) external {
      address naxapAddress = naxapNFTs.get(_tokenId);
      ERC1155Imported storage collateralNft = lockedERC1155s[naxapAddress];
      address abandonedNaxap = collateralNft.tokenAddress;
      uint256 abandonedIndex = collateralNft.tokenIndex;
      NaxapNFT(naxapAddress).safeTransferFrom(_msgSender(), address(this),0,1,'');
      uint256 balance = ERC1155Upgradeable(collateralNft.tokenAddress).balanceOf(naxapAddress, abandonedIndex);
      NaxapNFT(naxapAddress).claimContainedERC1155(abandonedNaxap,abandonedIndex,balance);
      ERC1155Upgradeable(collateralNft.tokenAddress).safeTransferFrom(address(this), _msgSender(), collateralNft.tokenIndex,balance,'');
      naxapNFTs.set(_tokenId, address(0));
      lockedERC1155s[naxapAddress] = ERC1155Imported(address(0),0);
      emit ERC1155UnLocked(_msgSender(), naxapAddress, abandonedNaxap, abandonedIndex);
    }

// GETTERS
//////////////////////////////////
    function getNaxapAddress(uint256 _tokenId) public view returns(address){
      return address(naxapNFTs.get(_tokenId));
    }
    function getERC721Collateral(address naxapAddress) public view returns(address){
      return(lockedERC721s[naxapAddress].tokenAddress);
    }
    function getERC1155Collateral(address naxapAddress) public view returns(address){
      return(lockedERC1155s[naxapAddress].tokenAddress);
    }
    function getNaxapsLength() public view returns(uint256){
      return(naxapNFTs.length());
    }
