    ...
    UUPSUpgradeable
{
    ...
    uint64 public teamPlayerSaleIndex;
    ...
    function publicWhiteListCountSetter(
        ...
        for (uint256 i = 0; i < _publicWhiteListAddresses.length; i++) {
            // NOTE: increasing the count number, not set it as value
    ...
    function teamNFTMint(address _to)
        public
        disableContractMint
    {
        require(isTeamPlayerMint, "minting not started");
        uint256 count = teamPlayersCount[_to];
        require(teamPlayersCount[_msgSender()] >= count, "already minted");
        ...
        for (uint256 i = 0; i < count; i++) {
            ...
            teamPlayerSaleIndex++;
            teamPlayersCount[_to] -= 1;
        }
    }

    function keySupporterNFTMint(address _to)
        public
        payable
        disableContractMint
    {
        require(isKeySupporterMint, "minting not started");
        uint256 count = keySupporterCount[_to];
        require(keySupporterCount[_msgSender()] >= count, "already minted");
        ...
        payable
    {
        ...
        require(mintFee * _count <= msg.value, "not enough mintFee: number * 0.05");
        require(
            publicWhiteListCount[_msgSender()] < 6,
    ...
    function mintNFT(
        ...
        if (teamPlayersCount[_toTeamAddress] > 0) {
            teamNFTMint(_toTeamAddress);
        }

        if (keySupporterCount[_toKeySupporterAddress] > 0) {
            keySupporterNFTMint(_toKeySupporterAddress);
        }
        ...
        if (_count > 0) {
            ...
            require(mintFee * _count <= msg.value, "not enough mintFee 0.05 for each");
    ...
    function totalMintedCount() external view returns (uint256 totalMinted) {
        totalMinted =
            teamPlayerSaleIndex -
            1 +
            keySupporterSaleIndex -
