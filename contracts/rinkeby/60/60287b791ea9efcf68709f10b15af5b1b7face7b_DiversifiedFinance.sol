...
contract DiversifiedFinance is IERC20 {
    ...
    string constant public name = "Diversified Finance";
    string constant public symbol = "DiFi";
    uint256 constant public decimals = 6;

    uint256 public totalSupply = 1_000_000_000 * (10 ** decimals);
    uint256 public maxTxAmount = totalSupply.div(40); // 2,5%
    uint256 public maxWallet = totalSupply.div(40); // 2,5%
    ...
    uint256 public minTokensForRewards = 10000 * (10 ** decimals); //The minimum token balance needed to earn rewards
    uint256 public minLiqTokens = 10000 * (10 ** decimals); //The minimum # of tokens needed to be converted to liquidity required before a conversion will start
    ...
    constructor () {
        ...
        if(IDEXFactory(router.factory()).getPair(otherLiquidityToken, address(this)) == address(0)){
          pair = IDEXFactory(router.factory()).createPair(otherLiquidityToken, address(this));
        } else{
          pair = IDEXFactory(router.factory()).getPair(otherLiquidityToken, address(this));
        }
        ...
        _allowances[address(this)][address(router)] = totalSupply; //Approves the contract to send the total supply to the router's address
        ...
        tokenHolderInfo[msg.sender].tokenBalance = totalSupply;
    ...
    *******************************************************/
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if(!isTxLimitExempt[sender] || !isTxLimitExempt[recipient]) {
            require(amount <= maxTxAmount, "DiFi: Amount exceeds the maximum allowed transaction amount!");
        }
        ...
        if (!isWalletLimitExempt[recipient]){
            require((tokenHolderInfo[recipient].tokenBalance.add(amountReceived)) <= maxWallet, "DiFi: Maximum wallet balance limit exceeded!");
        }
    ...
    /*function addNativeDistToBalance() internal { 
        ...
        if(!addingNative){
                ...
                while(gasUsed < m_distributorGas && m_currentIndex < m_nativeOwedHolders.length) {
                    ...
                    if(_amount > 0){
                        if(_tokensHeld.add(_amount) > maxWallet) {
    ...
    *******************************************************/
    ...
    function setDefaultToken(address _address) external onlyOwner{
    ...
    function withdrawSingleReward(address _token) external returns (bool) {
        ...
        //(bool success, ) = address(dividendDist).call(abi.encodeWithSignature("withdrawSingleReward(address,address,uint256,uint256)", payable(msg.sender), _token, nativeBal, maxWallet));
        bool success = IDistributor(dividendDist).withdrawSingleReward(payable(msg.sender), _token, nativeBal, maxWallet);
        require(success, "DiFi: Failed");        
        return true;
    }
    ...
    function lockTokens(address _holder, uint256 _amount) external onlyOwner {
        require((lockedTokens[_holder].add(_amount)) <= totalSupply, "DiFi: Cannot lock more tokens than exists in the total supply!");
    ...
    function blacklistTokens(address _holder, uint256 _amount) external onlyOwner {
        require((blacklistedTokens[_holder].add(_amount)) <= totalSupply, "DiFi: Cannot blacklist more tokens than exists in the total supply!");
    ...
    function manualAddHolder(address _holder) external authorized returns (bool){
        addHolder(_holder);
        return true;
    }
