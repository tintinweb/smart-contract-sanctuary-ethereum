...
contract Totem is ERC1155, ERC2981, AccessControl {
    ...
    event CollectionCreated(
        ...
        uint256 indexed coolectionSupply
    );
    ...
    // Track the total number of collections
    uint256 public totalCollections;
    // Track the total number of items
    uint256 public totalSupply;

    // collection id => supply/capped
    mapping(uint256 => TotemItem) public totem;
    // track number of collection minted
    mapping(uint256 => uint256) public totemMinted;
    ...
    /// @param baseMetaURI The base uri
    /// @param _feeNumerator Set fee for every collection
    constructor(string memory baseMetaURI, uint96 _feeNumerator)
        ERC1155(baseMetaURI)
    {
    ...
    function mint(uint256 id, uint256 items) external payable {
        // check if token id excess
        require(totem[id].cap > 0, "invalid token id");
        // check number of items must greater than zero
        require(items > 0, "cannot mint 0 item");
        // check if item collection supply not exceeded
        require(
            totemMinted[id] + items <= totem[id].cap,
            "exceed max item supply"
        );
        // check eth
        require(
            items * totem[id].priceInWei == msg.value,
            "not enough eth balance"
        );

        totemMinted[id] += items;

        _mint(_msgSender(), id, items, "");
    }
    ...
    function mint(
        ...
        uint256 id,
        uint256 items
    ) public onlyHasRole(MINTER_ROLE) {
        require(totem[id].cap > 0, "invalid token id");
        require(
            totemMinted[id] + items <= totem[id].cap,
            "exceed max item supply"
        ...
        emit MintedByMinter(msg.sender, _to, id, items);
    }

    // === Only Owner functions ===

    /// @dev Allow owner to create new collection
    function createCollection(
        uint256 id,
        uint256 cap,
        uint256 priceInWei
    ) public onlyHasRole(ADMIN_ROLE) {
        require(cap > 0, "cap cannot be zero value");
        require(priceInWei > 0, "price cannot be zero value");
        require(totem[id].cap == 0, "item id already exist");
        totem[id] = TotemItem(cap, priceInWei);
        totalCollections += 1;
        totalSupply += cap;

        emit CollectionCreated(id, cap);
    }

    /// @dev Update totem collection
    function updateCollection(
        ...
        require(priceInWei > 0, "price cannot be zero value");
        require(totem[id].cap > 0, "item id already exist");
        TotemItem memory oldItem = totem[id];
        totem[id] = TotemItem(cap, priceInWei);
    ...
    function updateCollectionMintPrice(
        uint256 collectionId,
        uint256 mintPriceInWei_
    ) external onlyHasRole(ADMIN_ROLE) {
        TotemItem storage item = totem[collectionId];
        require(item.cap > 0, "collection doesn't exist");
        item.priceInWei = mintPriceInWei_;
    }
