...
// File: contracts/CultureCartelGenesis.sol
...
contract CultureCartelGenesis is ERC721A, Ownable {
  ...
  uint256 private PRESALECOST = 0.15 ether;
  uint256 private PUBLICCOST = 0.15 ether;
  uint256 private MAX_MINT_LIMIT_PUBLIC = 3;
  uint256 private MAX_MINT_LIMIT_PRESALE = 1;
  uint256 private MAX_WALLET_LIMIT = 3;
  ...
  constructor () ERC721A ("Culture Cartel Genesis 1 to 100", "CCGG") {}
  ...
  function whitelistMint(uint256 _amount, bytes32[] memory _proof) external payable callerIsUser mintCompliance(_amount) {
    ...
    require(balanceOf(msg.sender) + _amount <= maxWalletLimit(), "Max mint amount per wallet reached");
    _mint(msg.sender, _amount);
  }

  function mint(uint256 _amount) public payable callerIsUser mintCompliance(_amount) {
  ...
  function mintForAddress(uint256 _amount, address _receiver) public mintCompliance(_amount) onlyOwner {
    _mint(_receiver, _amount);
  }

  function mintForSelf(uint256 _amount) public selfMintCompliance(_amount) onlyOwner {
    _mint(msg.sender, _amount);
  }
  ...
  /*///////////////////////////////////////////////////////////////
    ...
    function setPresaleMaxLimit(uint256 _state) public onlyOwner {
      MAX_MINT_LIMIT_PRESALE = _state;
    }

    function setPublicSaleMaxLimit(uint256 _state) public onlyOwner {
        MAX_MINT_LIMIT_PUBLIC = _state;
    }

    function setWalletMaxLimit(uint256 _state) public onlyOwner {
