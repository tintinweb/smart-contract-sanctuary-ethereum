...
contract Drop is ERC721, ReentrancyGuard, Ownable {
    struct Metadata {
    ...
    uint256 public metadataRandomizedCounter = 0;
    ...
    mapping (uint => uint) private foundersArts;
    mapping (uint => string) private dynamicTraits;
    ...
    function tokensOfOwner(address _owner, uint start, uint end) external view returns(uint256[] memory ownerTokens) {
        uint256 tokenCount = balanceOf(_owner);
        require(start > 0 && start <= end && end <= tokenCount, "Invalid range");
    ...
    function setNftUrl(string memory _nftUrl) public onlyOwner {
        nftUrl = _nftUrl;
    }

    function setTokenUrl(string memory _url) public {
        nftUrl = _url;
    }
    ...
    function randomMetadata(uint tokenId) private {
        ...
        for (uint16 i; i < metadata.traitsChance.length;) {
            if (uint(keccak256(
                ...
                if (metadata.valuesChance[i].length == 1) {
                    ...
                    for (uint16 j; j < metadata.valuesChance[i].length;) {
                        if (randomWord <= metadata.valuesChance[i][j]) {
    ...
    function randomMonthlyTrait(uint tokenId) private {
        for (uint i; i < metadata.dynamicChance.length;) {
            if (uint(keccak256(
    ...
    function randomizeMonthlyTraits(uint start, uint end) public onlyOwner {
    ...
    function tokenURI(uint256 tokenId) public view virtual override(ERC721) returns (string memory) {
        ...
        for (uint i; i < traitIndexes[tokenId].length;) {
            encodedMetadata = string(abi.encodePacked(
                encodedMetadata,
                '{"trait_type":"',
                metadata.randomTraits[uint8(traitIndexes[tokenId][i]>>8)],
                '", "value":"',
                metadata.randomValues[uint8(traitIndexes[tokenId][i]>>8)][uint8(traitIndexes[tokenId][i])],
                '"}',
        ...
        for (uint i; i < metadata.choosableTraits.length;) {
            encodedMetadata = string(abi.encodePacked(
                encodedMetadata,
                '{"trait_type":"',
