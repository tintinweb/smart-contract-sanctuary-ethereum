    ...
    ThePixelsDigitsUtility
{
    ...
    {
        uint256[] memory costs = new uint256[](tokenIds.length);
        bool[] memory claimedStatusses = new bool[](tokenIds.length);
        bool[] memory collectionOwnership = new bool[](tokenIds.length);
        ...
        for (uint256 i; i < variantIds.length; i++) {
            ...
            costs[i] = _getCostOfVariant(variant, owner);
            claimedStatusses[i] = claimedTokenVariants[extensionId][tokenId][
                variantId
            ];

            if (variant.isFreeForCollection) {
                if (IERC721(variant.collection).balanceOf(owner) > 0) {
                    collectionOwnership[i] = true;
                }
            }
        }

        return (costs, claimedStatusses, collectionOwnership);
    }
