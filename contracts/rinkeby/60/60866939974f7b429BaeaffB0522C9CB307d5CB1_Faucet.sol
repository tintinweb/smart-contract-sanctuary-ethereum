...
contract Faucet is Ownable {
    ...
    address public furioVaultAddress;

    ITokenMint private tokenMint;
    IToken private br34pToken;
    IToken private furioToken;
    IFurioVault private furioVault;
    ...
    uint256 public max_payout_cap = 100000e18;          // 100k FURIO or 10% of supply
    ...
    constructor(address _mintAddress, address _BR34PTokenAddress, address _furioTokenAddress, address _vaultAddress) public Ownable() {
        ...
        //Furio
        furioToken = IToken(_furioTokenAddress);

        //IFurioVault
        furioVaultAddress = _vaultAddress;
        furioVault = IFurioVault(_vaultAddress);
    ...
    function updateMinimumInitial(uint256 _newMinInitial) public onlyOwner {
        minimumInitial = _newMinInitial;
    }

    function updateMinimumDeposit(uint256 _newMinDeposit) public onlyOwner {
        minimumAmount = _newMinDeposit;
    }
    ...
    //@dev Deposit specified FURIO amount supplying an upline referral
    function deposit(address _upline, uint256 _amount) external {

        address _addr = msg.sender;

        (uint256 realizedDeposit, uint256 taxAmount) = furioToken.calculateTransferTaxes(_addr, _amount);
        uint256 _total_amount = realizedDeposit;

        //Checkin for custody management.  If a user rolls for themselves they are active
        checkin();

        require(_amount >= minimumAmount, "Minimum deposit of 1 FURIO");
        ...
        //Transfer FURIO to the contract
        require(
            furioToken.transferFrom(
                _addr,
                address(furioVaultAddress),
                _amount
            ),
            "FURIO token transfer failed"
        );
        /*
        User deposits 10;
    ...
    function checkForManager(address _addr) internal view {
    ...
    function _claim_out(address _addr) internal {
        ...
        uint256 vaultBalance = furioToken.balanceOf(furioVaultAddress);
        if (vaultBalance < to_payout) {
            uint256 differenceToMint = to_payout.sub(vaultBalance);
            tokenMint.mint(furioVaultAddress, differenceToMint);
        }

        furioVault.withdraw(to_payout);

        uint256 realizedPayout = to_payout.mul(SafeMath.sub(100, ExitTax)).div(100); // 10% tax on withdraw
        require(furioToken.transfer(address(msg.sender), realizedPayout));
    ...
    //@dev Send specified FURIO amount supplying an upline referral
    function airdrop(address _to, uint256 _amount) external {

        address _addr = msg.sender;

        (uint256 _realizedAmount, uint256 taxAmount) = furioToken.calculateTransferTaxes(_addr, _amount);
        ...
        require(
            ...
            "FURIO to contract transfer failed; check balance and allowance, airdrop"
        );
