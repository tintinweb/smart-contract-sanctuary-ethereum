...
contract NFTSwapBox is ReentrancyGuard, Ownable, INFTSwap {
  ...
  function _existBoxID(
    ...
    for (uint256 i = 0; i < box.offers.length; i++) {
      if (checkingActive) {
        if (
          checkingActive && box.offers[i].boxID == boxID && box.offers[i].active
        ) {
          return true;
        }
      } else {
  ...
  function destroyBox(uint256 boxID) public payable isOpenForSwap nonReentrant {
    require(swapBoxes[boxID].state == State.Initiated, "Not Allowed Operation");
    require(
      swapBoxes[boxID].owner == msg.sender,
      "Allowed to only Owner of SwapBox"
    ...
    for (
      ...
      if (swapBoxes[boxID].erc20Tokens.tokenAddrs[i] == erc20Address) {
        ...
        swapBoxes[boxID].erc20Tokens.tokenAddrs[i] = swapBoxes[boxID]
          .erc20Tokens
          .tokenAddrs[swapBoxes[boxID].erc20Tokens.tokenAddrs.length - 1];
      ...
      if (swapBoxes[boxID].erc721Tokens[i].tokenAddr == erc721Address) {
        for (
          ...
          if (swapBoxes[boxID].erc721Tokens[i].ids[j] == tokenID) {
            IERC721(swapBoxes[boxID].erc721Tokens[i].tokenAddr).transferFrom(
              address(this),
              msg.sender,
              swapBoxes[boxID].erc721Tokens[i].ids[j]
            );

            swapBoxes[boxID].erc721Tokens[i].ids[j] = swapBoxes[boxID]
              .erc721Tokens[i]
              .ids[swapBoxes[boxID].erc721Tokens[i].ids.length - 1];
        ...
        if (swapBoxes[boxID].erc721Tokens[i].ids.length == 0) {
          swapBoxes[boxID].erc721Tokens[i] = swapBoxes[boxID].erc721Tokens[
            swapBoxes[boxID].erc721Tokens.length - 1
          ];
      ...
      if (swapBoxes[boxID].erc1155Tokens[i].tokenAddr == erc1155Address) {
        for (
          ...
          if (swapBoxes[boxID].erc1155Tokens[i].ids[j] == tokenID) {
            ...
            swapBoxes[boxID].erc1155Tokens[i].ids[j] = swapBoxes[boxID]
              .erc1155Tokens[i]
              .ids[swapBoxes[boxID].erc1155Tokens[i].ids.length - 1];
            swapBoxes[boxID].erc1155Tokens[i].amounts[j] = swapBoxes[boxID]
              .erc1155Tokens[i]
              .amounts[swapBoxes[boxID].erc1155Tokens[i].amounts.length - 1];
        ...
        if (swapBoxes[boxID].erc1155Tokens[i].ids.length == 0) {
          swapBoxes[boxID].erc1155Tokens[i] = swapBoxes[boxID].erc1155Tokens[
  ...
  function deListBox(uint256 listBoxID) public isOpenForSwap nonReentrant {
    ...
    emit SwapBoxDeList(listBoxID, State.Initiated, swapBoxes[listBoxID].offers);
