...
contract Locker is Pausable, AccessControl, ReentrancyGuard {
    ...
    function unlock(uint256 _lockerId) external nonReentrant whenNotPaused {
        require(
            userTokenLock[msg.sender][_lockerId].status == false,
            "Locker: can't unlock with locker id"
        );

        require(
            userTokenLock[msg.sender][_lockerId].amount > 0,
            "Locker: can't unlock with zero amount"
        );

        uint256 _expiration = userTokenLock[msg.sender][_lockerId].expiration;
