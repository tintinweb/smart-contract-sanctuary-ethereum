...
// File: ContractManagerInterface.sol
...
contract STAKING2 is Ownable {
   ...
   uint totalStaked_;
    ...
    uint256 private _lockTimePeriod = 150;
    // lockTime period default 1 week
    // 60 * 60 * 24 * 7 =604800 sec
    uint256 private coolingTime = 100;
...
// add staking token address and wallet address where astaked amount transfered
    ...
    function Unstake() public {
        require( _stakeDetail[msg.sender].isStaker == exist, "user is not staker ");
 ...
 // work after coolperiod is over
    ...
    function Claim() public {
        ...
        require(_coolingTime[msg.sender] < block.timestamp, "cooling time not over ");
      
        GETS(stakingToken).transferPrice( primaryWallet,  msg.sender,AvailableForClaim[msg.sender]);
       totalStaked_ -= AvailableForClaim[msg.sender];
...
// checking is user  is staker or not
    ...
    function stakeTimeAmount(address addr) public view returns (uint [] memory , uint[] memory ) {
        return (  _stakeDetail[addr].stakedTokens[addr],
