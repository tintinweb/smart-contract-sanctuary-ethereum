...
// File: bridge/interfaces/IFastBridgeReceiver.sol
...
interface IFastBridgeReceiver {
     ...
     * @param _epoch The claimed hash corresponding to this `ticketID`. It should match the hash from the sending side otherwise it will be challenged.
     ...
     * @param _epoch The ticket identifier referring to a message going through the bridge.
     */
     ...
     */
    function challengePeriod(uint256 _epoch) external view returns (uint256 start, uint256 end);

}

// File: bridge/interfaces/arbitrum/IOutbox.sol
...
// File: bridge/interfaces/arbitrum/IInbox.sol
...
// File: bridge/interfaces/ISafeBridgeReceiver.sol
...
// File: bridge/SafeBridgeReceiverOnEthereum.sol
...
// File: bridge/FastBridgeReceiverOnEthereum.sol
...
contract FastBridgeReceiverOnEthereum is SafeBridgeReceiverOnEthereum, IFastBridgeReceiver{
    ...
    mapping(uint256 => bytes32[]) public relayed; // epoch => packed replay bitmap
    ...
    function challenge() external payable override {
        ...
        challenges[epochChallenge] = Challenge({
    ...
    function verify(uint256 _epoch) public{
        ...
        if (challenges[_epoch].challenger == address(0)){ // optimistic happy path
            claim.honest = true;
    ...
    function verifyProofAndRelayMessage(
        ...
        require ( batchMerkleRoot != 0 , "Invalid epoch.");
        ...
        require(_validateProof( _proof, messageHash, batchMerkleRoot), "Invalid proof.");
     ...
     */
    function challengePeriod(uint256 _epoch) external view override returns (uint256 start, uint256 end) {
        // start begins latest after the claim deadline expiry
        // however can begin as soon as a claim is made
        start = genesis + _epoch*epochPeriod + epochPeriod/2;
