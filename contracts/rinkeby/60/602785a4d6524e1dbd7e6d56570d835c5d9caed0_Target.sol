 ...
 */
contract Target {
  event CalledByExecutor(address indexed executor, address indexed originSender, uint32 indexed origin);
  ...
  modifier onlyExecutor() {
    emit CalledByExecutor(executor, IExecutor(msg.sender).originSender(), IExecutor(msg.sender).origin());
    require(
  ...
  function updateValue(uint256 newValue) external {
    if (value > 10) {
      value = IExecutor(msg.sender).origin();
