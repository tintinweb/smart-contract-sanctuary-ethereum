...
import "@siblings/modules/PrivSalePeriod.sol";
import "@siblings/modules/MerkleProofAllowlist.sol";
import "@siblings/modules/RoyaltiesConfig.sol";
import "@openzeppelin/contracts/finance/PaymentSplitter.sol";

contract MyToken is ERC721A, AdminPause, PrivSalePeriod, MerkleProofAllowlist, RoyaltiesConfig, PaymentSplitter {
    uint256 constant public MAX_SUPPLY = 10000;
    uint256 public walletCapAL = 5;
    uint256 public walletCap = 20;
    ...
    constructor(address[] memory payees, uint256[] memory shares_)
        ERC721A("MyToken", "MTK") 
        PrivSalePeriod(24) 
        MerkleProofAllowlist(0x877fe7274418177acaa0699f32f89694321a25b17f7304db1c013d60570020d7)
        PaymentSplitter(payees, shares_)
    {}

    // SETUP & ADMIN FUNCTIONS //

    function setPrices(uint256 _price, uint256 _price_al) public onlyAdmins {
        price = _price;
        priceAL = _price_al;
    }

    function setWalletCaps(uint256 _walletCap, uint256 _walletCapAL) public onlyAdmins {
        walletCap = _walletCap;
        walletCapAL = _walletCapAL;
    }
    ...
    function reserve(uint256 amount) public onlyAdmins {
        safeMint(msg.sender, amount);
    }

    // PUBLIC FUNCTIONS //

    function mint(uint256 amount, bytes32[] calldata _merkleProof) public payable whenNotPaused {
        require(saleTimestamp > 0, "Sale has not begun");

        bool al = isPrivSaleActive();
        if (al) requireMerkleProof(_merkleProof);

        uint256 _price = al ? priceAL : price;
        require(msg.value == _price * amount, "Incorrect amount of Ether sent");

        uint256 _walletCap = al ? walletCapAL : walletCap;
        require(
    ...
    function safeMint(address to, uint256 amount) internal {
        require(totalSupply() + amount <= MAX_SUPPLY, "Too few tokens remaining");
        _safeMint(to, amount);
    }
