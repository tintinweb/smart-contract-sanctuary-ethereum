...
import {IYieldToken, YieldToken} from './YieldToken.sol';

contract YieldTokenFactory is AccessControl {
  ...
  mapping(address => EnumerableSet.AddressSet) private _tokenYieldTokens;

  constructor(address _masterAdmin) {
    _setRoleAdmin(MASTER_ADMIN, MASTER_ADMIN);
    _setRoleAdmin(DEPLOYER, MASTER_ADMIN);
    _setupRole(MASTER_ADMIN, _masterAdmin);
    _setupRole(DEPLOYER, _masterAdmin);
  }
  ...
  ) public onlyRole(DEPLOYER) returns (IYieldToken _yieldToken) {
    _yieldToken = IYieldToken(new YieldToken{salt: keccak256(abi.encode(_underlyingToken, _symbol))}());
    _yieldToken.initialize(IYieldToken.YieldTokenInitializeParameters(_name, _symbol, _underlyingToken, _vault, _masterAdmin));

    _tokens.add(address(_underlyingToken));
    _tokenYieldTokens[_underlyingToken].add(address(_yieldToken));
  }

  function tokenYieldTokens(address _underlyingToken) external view returns (address[] memory) {
    return _tokenYieldTokens[_underlyingToken].values();
  }
 ...
 */
contract YieldToken is IYieldToken, ERC20PermitUpgradeable, AccessControlUpgradeable {
  ...
  function initialize(YieldTokenInitializeParameters calldata _initParams) external initializer {
    __ERC20_init(_initParams._name, _initParams._symbol);
    __ERC20Permit_init(_initParams._name);
    underlyingToken = _initParams._underlyingToken;
    _setRoleAdmin(MASTER_ADMIN, MASTER_ADMIN);
    ...
    emit YieldTokenInitialized(_initParams._name, _initParams._symbol, _initParams._underlyingToken, _initParams._vault);
  }
  ...
  function migrateVault(address _newVault) external onlyRole(VAULT_MIGRATOR) {
    // if (IVaultParameters(vault).yieldToken() != address(this)) revert AlreadyMigrated(); // TODO Remove, vault yieldToken is fixed
