...
contract NFTContract is ERC721Enumerable, ERC721Royalty, Pausable, Ownable, ReentrancyGuard {
    ...
    function tokenURI(uint tokenId) public view override(ERC721) returns (string memory) {
        ...
        string memory __baseURI = _getBaseUriByTokenId(tokenId);
    
        return bytes(__baseURI).length > 0 ? string(abi.encodePacked(__baseURI, tokenId.toString(), baseExtension)) : "";
    }

    function _getBaseUriByTokenId(uint _tokenId) internal view returns (string memory) {
        for (uint _i = 0; _i < totalRound; _i++) {
            uint _roundId = _i + 1;
            if (!reveals[_roundId]) return notRevealedUri;

            uint _roundLimit = roundLimits[_roundId];
            if (_tokenId < _roundLimit) return baseUris[_roundId];
        }
