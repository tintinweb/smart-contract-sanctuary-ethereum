...
// File contracts/MyAvatarDNA.sol


pragma solidity ^0.8.9;

contract MyAvatarDNA {
        ...
        returns (uint256)
    {
        ...
        bytes memory encondedParams = abi.encodePacked(combinedParams);
        bytes32 hashedParams = keccak256(encondedParams);
...
// File contracts/MyAvatar.sol


pragma solidity ^0.8.9;
contract MyAvatar is ERC721, ERC721Enumerable, MyAvatarDNA {
    ...
    constructor(uint256 _maxSupply) ERC721("MyAvatar", "MYAVA") {
    ...
    function mint() public {
        ...
        require(current < maxSupply, "No Avatars left");
        ...
        returns (string memory)
    {
        ...
        string memory image = imageByDNA(tokenDNA[tokenId]);
        string memory jsonURI = Base64.encode(
            abi.encodePacked(
                '{"name": "MyAvatar #',
                tokenId.toString(),
                '", "description": "My Avatar are randomized Avatars stored on chain to learn DApp development", "image":"',
                image,
                '"}'
            )
        );
