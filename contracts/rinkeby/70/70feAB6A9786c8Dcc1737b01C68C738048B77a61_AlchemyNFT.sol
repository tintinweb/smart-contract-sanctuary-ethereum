...
contract AlchemyNFT is ERC721, Ownable {
	string private nftBaseURI = 'http://url.com/metas/';
	address private signer = 0x81469c567f0F668121Cb1F803D946D982eaBCFAc;

	uint256 tokenCounter = 0;

	event Mint(uint256 mutationId, uint256 NFTId);

	constructor() ERC721('AlchemyNFT', 'ALNFT') {}
	...
	function mintNFT(
		...
		require(msg.value >= weiCost, 'Not enough funds for mint');

		string memory concatenatedParams = concatParams(expirationTimestamp, weiCost, mutationId);
		require(
			SignerVerification.isMessageVerified(signer, signature, concatenatedParams),
			'ECDSA: Invalid signature'
		);

		_safeMint(msg.sender, tokenCounter);
		emit Mint(mutationId, tokenCounter);
		tokenCounter++;
	}
		...
		return
			string(
				abi.encodePacked(
					...
					Strings.toString(weiCost),
					Strings.toString(mutationId)
				)
			);
	}
