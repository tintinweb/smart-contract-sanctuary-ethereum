...
// File: contracts/AsianIdentity.sol
    ...
    ReentrancyGuard
{
    ...
    function giftMint(address[] calldata _addresses) external onlyOwner {
        require(
            ...
            "Asian Identity: max total supply exceeded"
        );
        ...
        for (uint256 ind = 0; ind < _addresses.length; ind++) {
            require(
                ...
                "Asian Identity: recepient is the null address"
            );
            _tokenIds.increment();
    ...
    function publicMint(uint256 _amount) external payable nonReentrant{
        ...
        require(_amount > 0, "Asian Identity: zero amount");
        require(
            totalMint + _amount <= maxSupply - giftSupply,
            "Asian Identity: max supply exceeded"
        );
        require(
            _price * _amount <= msg.value,
            "Asian Identity: Ether value sent is not correct"
        );
        require(!paused, "Asian Identity: contract is paused");
        uint256 _newItemId;
    ...
    function burn(uint256 tokenId) external {
        require(isBurnEnabled, "Asian Identity: burning disabled");
        require(
            _isApprovedOrOwner(msg.sender, tokenId),
            "Asian Identity: burn caller is not owner nor approved"
        );
        _burn(tokenId);
        totalNFT = totalNFT - 1;
    }
