...
contract VictorGalleryAuction is ERC721, Ownable {
  ...
  bool public auctionRunning = false;
  ...
  function start(uint256 start, uint256 end, uint256 basePrice) public onlyOwner{
    ...
    require(!(auctionRunning), "Can't create auction when other one is running");
    paused = false;
    auctionRunning = true;
    startTime = start;
    endTime = end;
  ...
  function end() public onlyOwner{
    ...
    if(bids[highestBidder] == highestOffer && highestBidder != address(0x0)){
      _tokenIds.increment();
