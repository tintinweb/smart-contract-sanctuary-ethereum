...
interface AsagiTamashii {
    function walletOfOwner(address _address) external view returns (uint256[] memory);
}

contract WAV9 is Ownable, ERC721A, ReentrancyGuard, ERC2981{
  ...
  constructor(
  ) ERC721A("WAV9", "W9",batchSize, _totalSupply) {
     tokenCount = 0;
     setHiddenBaseURI("https://soco-engineering.com/www/R.json");//本番でここを書き換える
  }
  ...
  function setAtAdress(address c_address) external onlyOwner {
    at_adress = c_address;
    at_Contract = AsagiTamashii(at_adress);
  }
  //set release flag
