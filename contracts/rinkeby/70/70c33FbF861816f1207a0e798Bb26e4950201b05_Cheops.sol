...
contract Cheops is ERC20 {
    ...
    //
    // TODO :
    // ADD A FUNCTION TO SET sellThresholdAmount
    //
    //
    ...
    function setLiquidityWalls(int24[2][] calldata _liquidityWallsRanges, uint[] calldata _liquidityWallsRatios, bool[] calldata _liquidityWallsInToken) external {
        ...
        liquidityWallsRanges = _liquidityWallsRanges;
        liquidityWallsRatios = _liquidityWallsRatios;
        liquidityWallsInToken = _liquidityWallsInToken;
    }
    ...
    function reorganize() external {
        ...
        for (uint i=0; i < liquidityWallsRanges.length; i++) {
            if (liquidityWallsInToken[i]) {
                ...
                addLiquidity(currentTick + liquidityWallsRanges[i][0], currentTick + liquidityWallsRanges[i][1], liquidityAmount, 0);
            }
            else {
                liquidityAmount = (liquidityWallsRatios[i] * quoteAmount) / 10000;

                addLiquidity(currentTick + liquidityWallsRanges[i][0], currentTick + liquidityWallsRanges[i][1], 0, liquidityAmount);
            }
        }
    }
    ...
    function addLiquidity(int24 minTick, int24 maxTick, uint amountCheops, uint amountQuote) public {
