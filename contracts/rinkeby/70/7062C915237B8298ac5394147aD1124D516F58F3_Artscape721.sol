...
contract Artscape721 is Initializable, ERC721Upgradeable, ERC721URIStorageUpgradeable, ERC721BurnableUpgradeable, AccessControlUpgradeable, UUPSUpgradeable, RoyaltiesV2Impl  {
    ...
    event ArtscapeMint (address owner, address artist, uint tokenId, string metadata, string time);
    ...
    function safeMint(address to, string memory uri, string memory time) public onlyRole(MINTER_ROLE) returns(uint){
        ...
        emit ArtscapeMint(msg.sender, msg.sender, tokenId, uri, time);
        return tokenId;
    }

    function safeMint(address to, string memory uri, string memory time, address artist, uint percent) public onlyRole(MINTER_ROLE) returns(uint){
        uint256 tokenId = _tokenIdCounter.current();
        _tokenIdCounter.increment();
        _safeMint(to, tokenId);
        _setTokenURI(tokenId, uri);
        setApprovalForAll(contractAddress, true);
        setRoyalties(tokenId, payable(artist), uint96(percent*100));
        emit ArtscapeMint(msg.sender, artist, tokenId, uri, time);
        return tokenId;
    }
