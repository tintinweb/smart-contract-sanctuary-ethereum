...
import "./ERC721A.sol";
contract ThePandaCollective is Ownable, ERC721A, ReentrancyGuard {
    ...
    address payable public devWallet =
        payable(0xd4b0a4690500339Eb304d4333BC13fEA9b7e7ca9);
    ...
    function setSaleConfigs(
        ...
        require(_ogSaleConfig.saleStart == _wlSaleConfig.saleStart, "Presale configs must be the same");
    ...
    function getCurrentStage() public view returns (STAGE) {
        ...
        if (currentTime >= ogSaleTime && currentTime >= wlSaleTime && currentTime < publicSaleTime) {
    ...
    function getSaleConfig(address _caller) internal view returns (SaleConfig storage) {
        STAGE stage = getCurrentStage();
        if(stage == STAGE.PRESALE) {
            bool isOgAllowed = isOg(_caller);
            bool isWlAllowed = isWl(_caller);
            require(isOgAllowed || isWlAllowed, "Minter is not in OG or WL list");
