...
contract ERC1155Factory is Ownable {
    ...
    */
    function getCountERC1155byIndex(uint256 _index, uint256 _id) public view returns (uint256 amount) {
        return tokens[_index].balanceOf(indexToOwner[_index], _id);
    }

    function getCountERC1155byName(uint256 _index, string calldata _name) public view returns (uint256 amount) {
        uint256 id = getIdByName(_index, _name);
        return tokens[_index].balanceOf(indexToOwner[_index], id);
    }

    function getIdByName(uint256 _index, string memory _name) public view returns (uint256) {
        return tokens[_index].nameToId(_name);
    }

    function getNameById(uint256 _index, uint256 _id) public view returns (string memory) {
...
contract ERC1155Token is ERC1155, Ownable {
    ...
    */
    function uri(uint256 _tokenid) public view override returns (string memory) {
        return string(abi.encodePacked(baseMetadataURI, "/", Strings.toString(_tokenid), "/metadata.json"));
    }
