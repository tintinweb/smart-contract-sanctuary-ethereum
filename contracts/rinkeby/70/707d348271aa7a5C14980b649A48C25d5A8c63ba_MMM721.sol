    ...
    AccessControl
{
    ...
    event MintNFT(address sender, uint256 tokenId, string uri);
    ...
    function mint(
        ...
        require(
            hasRole(
                MINTER_ROLE,
                _getSigner(msg.sender, uri, msg.value, v, r, s)
            ),
    ...
    function _mint(address to, string memory uri) internal {
        ...
        _tokenIds.increment();
        emit MintNFT(msg.sender, newItemId, uri);
    }
...
contract TokenSignature {
    ...
    bytes32 internal constant SIGNDATA_TYPEHASH =
        keccak256(
            "SignData(address buyer,string uri,uint256 price)"
        );
