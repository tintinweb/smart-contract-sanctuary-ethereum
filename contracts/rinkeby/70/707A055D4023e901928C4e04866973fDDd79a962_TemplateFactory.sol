...
contract TemplateFactory is ITemplateFactory, ReentrancyGuardUpgradeable, Accessible {
    ...
    function uploadTemplate(bytes32 templateId, bytes memory initialPart, address implementation) external override returns (bool) {
        return uploadTemplate(templateId, initialPart, 1, implementation);
    }

    function uploadTemplate(bytes32 templateId, bytes memory initialPart, uint256 totalParts, address implementation) public override onlyRole(MANAGE_TEMPLATE_ROLE) nonReentrant returns (bool) {
        ...
        _template[templateId].version[_version].totalParts = totalParts;
        _template[templateId].version[_version].creationCode = initialPart;
        _template[templateId].version[_version].partsUploaded = 1;
        _template[templateId].version[_version].implementation = implementation;

        emit TemplateVersionCreated(templateId, _version, implementation, _msgSender());
    ...
    function uploadTemplatePart(bytes32 templateId, uint256 _version, bytes memory part) external override nonReentrant onlyRole(MANAGE_TEMPLATE_ROLE) returns (bool) {
        ...
        Version storage vsn = _template[templateId].version[_version];

        require(vsn.exists, 'TemplateFactory: version does not exist for this templateId');
    ...
    function predictDeployAddress(bytes32 templateId, uint256 _version, bytes memory args) external override view returns (address) {
        return predictDeployAddress(templateId, _version, args, _salt(templateId, _version));
    }

    function predictDeployAddress(bytes32 templateId, uint256 _version, bytes memory args, bytes32 salt) public override view returns (address) {
        return Deployer.predictAddress(address(this), _deployCode(templateId, _version, args), salt);
    }

    function predictCloneAddress(bytes32 templateId, uint256 _version) external override view returns (address) {
        return predictCloneAddress(templateId, _version, _salt(templateId, _version));
    }

    function predictCloneAddress(bytes32 templateId, uint256 _version, bytes32 salt) public override view returns (address) {
        return Clones.predictDeterministicAddress(
            _template[templateId].version[_version].implementation,
            salt,
            address(this)
        );
    }

    function deployTemplateLATEST(bytes32 templateId, bytes memory args) external override returns (address instance) {
        return deployTemplate(templateId, latestVersion(templateId), args);
    }

    function deployTemplateLATEST(bytes32 templateId, bytes memory args, bytes32 salt) external override returns (address instance) {
        return deployTemplate(templateId, latestVersion(templateId), args, salt);
    }

    function deployTemplateLATEST(bytes32 templateId, bytes memory args, bytes[] memory functionCalls) external override returns (address instance) {
        return deployTemplate(templateId, latestVersion(templateId), args, functionCalls);
    }

    function deployTemplateLATEST(bytes32 templateId, bytes memory args, bytes[] memory functionCalls, bytes32 salt) external override returns (address instance) {
        return deployTemplate(templateId, latestVersion(templateId), args, functionCalls, salt);
    }

    function deployTemplate(bytes32 templateId, uint256 _version, bytes memory args) public override returns (address instance) {
        return deployTemplate(templateId, _version, args, new bytes[](0));
    }

    function deployTemplate(bytes32 templateId, uint256 _version, bytes memory args, bytes32 salt) public override returns (address instance) {
        return deployTemplate(templateId, _version, args, new bytes[](0), salt);
    }

    function deployTemplate(bytes32 templateId, uint256 _version, bytes memory args, bytes[] memory functionCalls) public override returns (address instance) {
        return deployTemplate(templateId, _version, args, functionCalls, _salt(templateId, _version));
    }

    function deployTemplate(bytes32 templateId, uint256 _version, bytes memory args, bytes[] memory functionCalls, bytes32 salt) public override nonReentrant returns (address instance) {
        Version storage vsn = _validateTemplateVersion(templateId, _version, _msgSender());

        require(vsn.totalParts == vsn.partsUploaded, 'TemplateFactory: this is an incomplete version. (missing parts)');
        require(vsn.creationCode.length > 0, 'TemplateFactory: Version does not exist');

        instance = Deployer.deploy(_deployCode(templateId, _version, args), salt);

        _registerInstance(templateId, _version, instance, functionCalls);

        emit TemplateDeployed(instance, templateId, _version, args, functionCalls, _msgSender());
    }

    function cloneTemplateLATEST(bytes32 templateId) external override returns (address instance) {
        return cloneTemplate(templateId, latestVersion(templateId));
    }

    function cloneTemplateLATEST(bytes32 templateId, bytes32 salt) external override returns (address instance) {
        return cloneTemplate(templateId, latestVersion(templateId), salt);
    }

    function cloneTemplateLATEST(bytes32 templateId, bytes[] memory functionCalls) external override returns (address instance) {
        return cloneTemplate(templateId, latestVersion(templateId), functionCalls);
    }

    function cloneTemplateLATEST(bytes32 templateId, bytes[] memory functionCalls, bytes32 salt) external override returns (address instance) {
        return cloneTemplate(templateId, latestVersion(templateId), functionCalls, salt);
    }

    function cloneTemplate(bytes32 templateId, uint256 _version) public override returns (address instance) {
        return cloneTemplate(templateId, _version, new bytes[](0));
    }

    function cloneTemplate(bytes32 templateId, uint256 _version, bytes32 salt) public override returns (address instance) {
        return cloneTemplate(templateId, _version, new bytes[](0), salt);
    }

    function cloneTemplate(bytes32 templateId, uint256 _version, bytes[] memory functionCalls) public override returns (address instance) {
        return cloneTemplate(templateId, _version, functionCalls, _salt(templateId, _version));
    }

    function cloneTemplate(bytes32 templateId, uint256 _version, bytes[] memory functionCalls, bytes32 salt) public override nonReentrant returns (address instance) {
        Version storage vsn = _validateTemplateVersion(templateId, _version, _msgSender());

        require(vsn.implementation != address(0), 'TemplateFactory: implementation or version does not exist');

        instance = Clones.cloneDeterministic(vsn.implementation, salt);
    ...
    function _validateTemplateVersion(bytes32 templateId, uint256 _version, address sender) private view returns (Version storage vsn) {
        Template storage tpl = _template[templateId];
        vsn = _template[templateId].version[_version];
        ...
        require(
            tpl.deployRole == PUBLIC_ROLE || hasRole(tpl.deployRole, sender) || hasRole(MANAGE_TEMPLATE_ROLE, sender),
            'TemplateFactory: missing required permissions to deploy this template'
        );
    }

    function _registerInstance(bytes32 templateId, uint256 _version, address instance, bytes[] memory functionCalls) private {
...
library Deployer {
    ...
    function predictAddress(address factory, bytes memory bytecode, bytes32 salt) internal pure returns (address addr)
    {
