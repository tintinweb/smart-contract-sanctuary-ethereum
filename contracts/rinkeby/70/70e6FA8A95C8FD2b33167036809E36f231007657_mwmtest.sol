...
contract mwmtest is ERC721A, Ownable {
    // Price
    ...
    uint256 public preSaleStartTimeStamp = 1648386900;

    // Presales End TimeStamp
    uint256 public preSaleEndTimeStamp = 1648387800;

    // Max Mint Per Wallet at Presales Period
    uint256 private maxPreSaleMintPerWallet = 2;
    ...
    uint256 public publicSaleStartTimeStamp = 1648388700;

    // Max Mint Per Wallet
    uint256 private maxMintPerWallet = 2;

    // Tracks redeem for sale
    mapping(address => uint256) private publicSaleRedeemed;

    // Merkle Root for Presale
    bytes32 public preSaleRoot;

    constructor()ERC721A("mwmtest", "mt"){}

    function PreSalemint(uint256 quantity, bytes32[] calldata proof) external payable {
        require(_msgSender() == tx.origin, "Anti bot");
        require((totalSupply() + quantity) <= maxSupply, "All NFTs are minted");
        require(isPreSaleActive, "PreSale Is Inactive");
        require(isPreSalePeriod(), "Not In PreSale Period");
        require(msg.value >= price * quantity, "Payment is not correct");
        require(
            MerkleProof.verify(
                proof,
                preSaleRoot,
        ...
        require((preSaleRedeemed[_msgSender()] + quantity) <= maxPreSaleMintPerWallet, "Exceed PreSale Mint Limit");
    ...
    function mint(uint256 quantity) external payable {
        ...
        require(
            publicSaleRedeemed[_msgSender()] + quantity <= maxMintPerWallet,"Exceed Public Sale Mint Limit");

        publicSaleRedeemed[_msgSender()] = publicSaleRedeemed[_msgSender()] + quantity;
    ...
    function getPublicSaleRedeemed(address addr) public view returns (uint256) {
        return publicSaleRedeemed[addr];
    }
