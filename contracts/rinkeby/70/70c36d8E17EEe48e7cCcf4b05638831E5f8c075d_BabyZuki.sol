...
contract BabyZuki is Ownable, ERC721A, ReentrancyGuard {
    ...
    function setupSaleInfo(uint64 whitelistPrice, uint64 publicPrice)
        external
        onlyOwner
    {
    ...
    function publicmintMint(uint256 quantity) external payable callerIsUser {
        require(!paused, "Sale has not begun yet");
        ...
        require(
            ...
            "can not mint this many at a time"
        );
        require(
            totalSupply() + 1 + (amountForDevs - numberMinted(owner())) <=
                collectionSize,
