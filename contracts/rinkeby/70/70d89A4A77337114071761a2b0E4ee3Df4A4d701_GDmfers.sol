...
contract GDmfers is ERC721A, Ownable, ReentrancyGuard {
    ...
    mapping(address => uint256) public freeGoopMints;
        ...
        enoughSupply(quantity)
    {
        ...
        if(goopdoods.balanceOf(msg.sender) > 0){
           uint256 freeMintsAvailable = goopdoods.balanceOf(msg.sender) - freeGoopMints[msg.sender];
           if(quantity >= freeMintsAvailable){
             freeGoopMints[msg.sender] += quantity;
             // If you've got enough free mints, any eth paid is refunded.
             refundIfOver(0);
           } else {
             freeGoopMints[msg.sender] += freeMintsAvailable;
