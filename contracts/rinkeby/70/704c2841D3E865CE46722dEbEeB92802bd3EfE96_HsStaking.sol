...
contract HsStaking is Ownable, IERC721Receiver {
    ...
    constructor(HsNFT _nft, HsToken _token) {
        nft = _nft;
        token = _token;
    }
    ...
    function _claim(address account, uint256[] calldata tokenIds, bool _unstake) internal {
        ...
        for (uint i = 0; i < tokenIds.length; i++) {
            tokenId = tokenIds[i];
            Stake memory staked = vault[tokenId];
            require(staked.owner == account, "not!owner");
            uint256 stakedAt = staked.timestamp;
//          rewardmath = 100 ether * (block.timestamp - stakedAt) / 86400 ;//@todo
            rewardmath = 1 ether * (block.timestamp - stakedAt) / 60 ;
...
contract HsNFT is ERC721A, Ownable, Pausable {
    ...
    //the merkle root
    bytes32 public root = 0x2b2aa88b191081763a352a50d714b3d40312c029decd497944d1776b124fbfc5;

    string public _contractBaseURI = "https://unreveal.mywebsite.com/";
    string public _contractURI = "https://mywebsite.com/contract_uri/contract_uri.json";

    uint256 public tokenPrice = 0.003 ether; //price per token
   ...
   * @dev
	 ...
	 */
    constructor() ERC721A("HsNFT", "HsNFT", 50) {
    ...
    function whitelistBuy(
        ...
        require(usedAddresses[msg.sender] + qty <= 10, "max per wallet reached");
...
contract ERC721A is Context, ERC165, IERC721, IERC721Metadata, IERC721Enumerable {
    ...
    constructor(
        ...
        require(maxBatchSize_ > 0, "A: max batch ");
        _name = name_;
        _symbol = symbol_;
    ...
    function tokenByIndex(uint256 index) public view override returns (uint256) {
        require(index < totalSupply(), "A:out");
        return index;
    }
    ...
    returns (uint256)
    {
        require(index < balanceOf(owner), "A:out");
        ...
        revert("A: unable to get token of owner by index");
    }
    ...
    function balanceOf(address owner) public view override returns (uint256) {
        require(owner != address(0), "A:0 address");
    ...
    function ownershipOf(uint256 tokenId) internal view returns (TokenOwnership memory) {
        require(_exists(tokenId), "A:nonexistent token");
        ...
        revert("A: unable to determine the owner of token");
    }
    ...
    function approve(address to, uint256 tokenId) public override {
        ...
        require(to != owner, "A: approval to current owner");
    ...
    function setApprovalForAll(address operator, bool approved) public override {
        require(operator != _msgSender(), "A: approve to caller");
    ...
    function safeTransferFrom(
        ...
        require(
            ...
            "A: transfer to non ERC721Receiver implementer"
        );
    }
    ...
    function _safeMint(
        ...
        require(to != address(0), "A: mint to the zero address");
        // We know if the first token in the batch doesn't exist, the other ones don't as well, because of serial ordering.
        require(!_exists(startTokenId), "A: token already minted");
        require(quantity <= maxBatchSize, "A: quantity to mint too high");
    ...
    function _transfer(
        ...
        require(isApprovedOrOwner, "A:not owner");

        require(prevOwnership.addr == from, "A:incorrect owner");
        require(to != address(0), "A:0x");
    ...
    function _checkOnERC721Received(
        ...
        if (to.isContract()) {
            try IERC721Receiver(to).onERC721Received(_msgSender(), from, tokenId, _data) returns (
                ...
                if (reason.length == 0) {
                    revert("A: transfer to non ERC721Receiver implementer");
                } else {
                    assembly {
