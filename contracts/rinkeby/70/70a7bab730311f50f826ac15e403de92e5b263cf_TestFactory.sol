...
// Dependency file: contracts/proxy/TestProxy.sol
...
// Dependency file: contracts/interfaces/ITest.sol
...
// Root file: contracts/TestFactory.sol
...
// import "contracts/proxy/TestProxy.sol";
// import "contracts/libraries/TestLibrary.sol";
// import "contracts/interfaces/ITest.sol";
// import "contracts/interfaces/ITestSettings.sol";
// import "contracts/interfaces/ITestRegistry.sol";
// import "contracts/interfaces/IWETH.sol";
// import "contracts/traits/TransferHelpers.sol";

/// @notice ido factory, create and register new IDO

contract TestFactory is OwnableUpgradeable, TransferHelpers, ReentrancyGuardUpgradeable, PausableUpgradeable {
    using SafeMath for uint256;

    address public ido; //ido implementation;
    ...
    event EmergencyWithdraw(address lpToken, address recipient, uint256 amount);
    event LockIDOTokens(address ido, address token, uint256 amount);

    event UpdateIDOImplementation(address ido);
    event UpdateRegistry(address registry);
    event UpdateSettings(address settings);
    event UpdateWETH(address weth);
    event UpdateLockForwarder(address lockForwarder);
    event UpdateProxyAdmin(address proxyAdmin);

    /// @notice initialize contract
    /// @param _registry registry address
    /// @param _settings settings address
    /// @param _ido ido contract implementation address
    /// @param _weth wrapped native token address
    /// @param _lockForwarder lock forwarder address
    /// @param _proxyAdmin proxy admin address
    function initialize(
        ITestRegistry _registry,
        ITestSettings _settings,
        address _ido,
    ...
    /// @param _idoSettings struct as format IDOSettings, see TestLibrary
    /// @dev any users
    ...
    function createIDO(
        ...
        TestLibrary.IDOUserLimits memory _userLimits,
        TestLibrary.IDOSettings memory _idoSettings
    ) external payable nonReentrant whenNotPaused returns (address _newIDO) {
        //validation

        require(_idoInfo.amount >= 10000, "TestFactory: amount less available");
        require(_idoInfo.endTime.sub(_idoInfo.startTime) <= settings.getMaxIDOLength(), "TestFactory: invalid start and end times");
        require(_idoInfo.tokenRate.mul(_idoInfo.hardCap) > 0, "TestFactory: invalid parameters");
        require(_idoInfo.liquidityPercent >= 300 && _idoInfo.liquidityPercent <= 10000, "TestFactory: min liquidity");
        require( _idoInfo.lockPeriod <= settings.getMinLockLength(), "TestFactory: lock length");
        require(_idoInfo.cliffPeriod < _idoInfo.lockPeriod, "TestFactory: invalid cliff period");
        require(_userLimits.minSpendPerBuyer <= _userLimits.maxSpendPerBuyer, "TestFactory: invalid limits");
        
        uint256 baseFee = settings.getBaseFee();

        require(msg.value >= baseFee, "TestLocker: fee doesnt met");

        if (msg.value > baseFee) {
            //refund rest
            _deliverFunds(_msgSender(), msg.value.sub(baseFee), "IDO: cannot send");
        }

        _deliverFunds(settings.getFeeRecipient(), baseFee, "IDO: cannot send");

        TestProxy TestProxy = new TestProxy(address(ido), proxyAdmin); //proxy IDO

        ITest(address(TestProxy)).initialize(
            registry,
            weth,
            settings,
            lockForwarder,
        ...
        if (_idoSettings.lockTokens) {
            uint256 tokensRequired = tokensRequiredForIDO(_idoInfo.amount, _idoInfo.tokenRate, _idoInfo.listingRate, _idoInfo.liquidityPercent);

            _safeTransferFrom(_idoInfo.saleToken, _msgSender(), address(TestProxy), tokensRequired);

            emit LockIDOTokens(address(TestProxy), _idoInfo.saleToken, tokensRequired);
        }

        registry.registerIDO(address(TestProxy), _msgSender());

        return address(TestProxy);
    }

    function tokensRequiredForIDO(
        uint256 _amount,
        uint256 _tokenRate,
        uint256 _listingRate,
        uint256 _liquidityPercent
    ) public pure returns (uint256 amount) {
        amount = TestLibrary.calculateAmountRequired(_amount, _tokenRate, _listingRate, _liquidityPercent);
    }
    ...
    /// @dev only when contract paused and from owner
    function withdrawTokens(address _token, address _recipient) external whenPaused onlyOwner {
        _safeTransferAll(_token, _recipient);

        emit EmergencyWithdraw(_token, _recipient, IERC20(_token).balanceOf(address(this)));
    }

    /// @notice update wrapped token address
    /// @param _weth new address
    /// @dev only when contract paused and from owner
    function updateWETH(IWETH _weth) external whenPaused onlyOwner {
        weth = _weth;
        emit UpdateWETH(address(_weth));
    }

    /// @notice update proxy admin
    /// @param _proxyAdmin new proxy admin
    /// @dev only when contract paused and from owner
    function updateProxyAdmin(address _proxyAdmin) external whenPaused onlyOwner {
        proxyAdmin = _proxyAdmin;
        emit UpdateProxyAdmin(_proxyAdmin);
    }

    /// @notice update ido impementation
    /// @param _ido new ido implementation
    /// @dev only when contract paused and from owner
    function updateIDOImplementation(address _ido) external whenPaused onlyOwner {
        ido = _ido;
        emit UpdateIDOImplementation(address(_ido));
    }

    /// @notice update registry
    /// @param _registry new registry
    /// @dev only when contract paused and from owner
    function updateRegistry(ITestRegistry _registry) external whenPaused onlyOwner {
        registry = _registry;
    ...
    /// @dev only when contract paused and from owner
    function updateLockForwarder(ITestLockForwarder _lockForwarder) external whenPaused onlyOwner {
        lockForwarder = _lockForwarder;
    ...
    /// @dev only when contract paused and from owner
    function updateSettings(ITestSettings _settings) external whenPaused onlyOwner {
        settings = _settings;
