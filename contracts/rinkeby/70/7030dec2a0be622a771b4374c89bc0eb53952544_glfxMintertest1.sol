...
interface IManifoldERC721Edition {
     ...
     */
    function mint(address creator, uint256 series, address recipient) external payable;
...
//             Author: 0xrDan.eth             //
//////////////////////////////////////////////// 
contract glfxMintertest1 is CreatorExtension, ICreatorExtensionTokenURI, IManifoldERC721Edition, Pausable, ReentrancyGuard {
    ...
    modifier creatorAdminRequired(address creator) {
        require(creator == 0x6Ca2cde6D6Ee88f47fCe4Ab4d67Becbf85aAB6d8,"Contract not authorized.");
...
// do i really need this?
     ...
     */
    function mint(address creator, uint256 series, address recipient) external whenNotPaused payable override nonReentrant creatorAdminRequired (creator) {
        require(msg.value >= mintRate, "Sorry, not enough ether.");
        uint16 statusCheck = 0;
        (series, statusCheck) = tokenCheck(creator, series);
        if(statusCheck == 0){ _pause();}
        require(statusCheck > 0, "This generation has been fully minted.");
        uint256[] memory tokenIds = IERC721CreatorCore(creator).mintExtensionBatch(recipient, 1);
        _updateIndexRanges(creator, series, tokenIds[0], 1);
    }

    function tokenCheck(address creator,uint256 series) internal view returns (uint256, uint16){

        uint random1 = uint(keccak256(abi.encodePacked( block.difficulty,  block.difficulty, msg.sender))) % 16;
        uint random2 = uint(keccak256(abi.encodePacked(block.timestamp,  block.timestamp, msg.sender))) % 16;
        uint256 tough = block.difficulty*block.timestamp/random1;
        uint256 time = block.timestamp/block.difficulty*random2;
        uint random3 = uint(keccak256(abi.encodePacked(tough, time, msg.sender))) % 128;
        uint16 statusCheck = 0;
        
        if (random3 > (0.5078125*128)){ series = 7; if(_totalSupply[creator][series]+1 <= _maxSupply[creator][series]){}else{tokenCheck(creator,series);}}
        if (random3 <= (0.5078125*128)){ series = 6; if(_totalSupply[creator][series]+1 <= _maxSupply[creator][series]){}else{tokenCheck(creator,series);}}
        if (random3 <= (0.25*128)){ series = 5; if(_totalSupply[creator][series]+1 <= _maxSupply[creator][series]){}else{tokenCheck(creator,series);}}
        if (random3 <= (0.125*128)){ series = 4; if(_totalSupply[creator][series]+1 <= _maxSupply[creator][series]){}else{tokenCheck(creator,series);}}
        if (random3 <= (0.0625*128)){ series = 3; if(_totalSupply[creator][series]+1 <= _maxSupply[creator][series]){}else{tokenCheck(creator,series);}}
        if (random3 <= (0.03125*128)){ series = 2; if(_totalSupply[creator][series]+1 <= _maxSupply[creator][series]){}else{tokenCheck(creator,series);}}
        if (random3 == (0.015625*128)){ series = 1; if(_totalSupply[creator][series]+1 <= _maxSupply[creator][series]){}else{tokenCheck(creator,series);}}        
        if(_totalSupply[creator][1]+1 <= _maxSupply[creator][1]){statusCheck++;}
        if(_totalSupply[creator][2]+1 <= _maxSupply[creator][2]){statusCheck++;}
        if(_totalSupply[creator][3]+1 <= _maxSupply[creator][3]){statusCheck++;}
        if(_totalSupply[creator][4]+1 <= _maxSupply[creator][4]){statusCheck++;}
        if(_totalSupply[creator][5]+1 <= _maxSupply[creator][5]){statusCheck++;}
        if(_totalSupply[creator][6]+1 <= _maxSupply[creator][6]){statusCheck++;}
        if(_totalSupply[creator][7]+1 <= _maxSupply[creator][7]){statusCheck++;}

        return (series, statusCheck);
    }
