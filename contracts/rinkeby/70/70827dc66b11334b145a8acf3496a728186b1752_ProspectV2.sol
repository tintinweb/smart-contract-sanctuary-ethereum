...
contract ProspectV2 is Ownable, GIGADRIPV2 {
    error NotOwnerOrPickaxesContract();

    address public pickaxes;
    address public prospectorsnft;
    ...
    constructor(
        ...
        uint8 _decimals,
        uint256 _emissionRatePerBlock
    ) GIGADRIPV2(_name, _symbol, _decimals, _emissionRatePerBlock) {}
    ...
    /**
     * @dev only pickaxes can start dripping tokens (on mint or transfer).
    ...
    function startDripping(address addr, uint128 multiplier) external {
        if (msg.sender != pickaxes && msg.sender != owner())
        if (msg.sender != prospectorsnft && msg.sender != owner())
            revert NotOwnerOrPickaxesContract();
        _startDripping(addr, multiplier);
    }

    /**
     * @dev only pickaxes can stop dripping tokens (on burn or transfer).
    ...
    function setpickaxes(address pickaxesAddress)
        external
        onlyOwner
    {
        pickaxes = pickaxesAddress;
    }

    function setprospectorsnft(address prospectorsNftAddress)
        external
        onlyOwner
    {
        prospectorsnft = prospectorsNftAddress;
    }
}