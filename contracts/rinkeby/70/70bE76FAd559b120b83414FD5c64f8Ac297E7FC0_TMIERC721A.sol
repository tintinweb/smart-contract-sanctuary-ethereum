...
contract TMIERC721A is MGYERC721A{
  ...
  //WL mint.
  function whitelistMint(uint256 _amount, bytes32[] memory proof_, bytes32[] memory proof1_, bytes32[] memory proof2_) external payable override virtual nonReentrant {
    _whitelistMintCheck(_amount , proof_, proof1_, proof2_);
    _whitelistMintCheckValue(_amount - 1);//1 is free.
    require(_wlMinted[_seasonCounter][msg.sender] == 0, "You already minted.");//tomui add. mint is once only.
