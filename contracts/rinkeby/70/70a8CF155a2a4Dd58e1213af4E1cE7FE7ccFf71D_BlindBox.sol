...
contract BlindBox is NonGenerativeBB {
    ...
    */
    function StartSeries(
        ...
        string[] memory stringsData, // [name, seriesURI, boxName, boxURI, collectionMongoId]
       uint256[] memory integerData, //[startTime, endTime, maxBoxes, perBoxNftMint, perBoxPrice, baseCurrency, perUserLimit]
       uint256[] memory allowedCurrencies,
        bool isGenerative,  address bankAddress, uint256 royalty, bool isWhiteListed) onlyOwner public {
...
contract Utils is Ownable, BlindboxStorage{
    ...
    function init() public {
        ...
        LPMATIC = LPInterface(0x000C6843Be0b17FB61ed1a8465b2fA2bac0cFCb2);
   ...
   function addWhiteListUsers(bytes32[] memory users) public {
    for(uint256 i = 0; i<users.length; i++){
      _whitelisted[users[i]] = true;
    }
  }

  function removeWhiteListUsers(bytes32[] memory users) public {
      for(uint256 i = 0; i<users.length; i++){
...
contract BlindboxStorage {
 ...
 Counters.Counter public generativeSeriesId;
   ...
   mapping ( uint256 => uint256) public baseCurrency;
    ...
    LPInterface LPWETH;
    LPInterface LPMATIC;
     mapping(bytes32 => bool) _whitelisted;
...
/** 
    ...
    */
    function buyNonGenBox(uint256 seriesId, uint256 currencyType, address collection, string memory ownerId,  bytes32 user) validateCurrencyType(seriesId,currencyType, false) internal {
         require(!_isWhiteListed[seriesId] || _whitelisted[user], "not authorize");
    ...
    modifier userWhiteListed(string memory ownerId, uint256 seriesId) {
        require(!_isWhiteListed[seriesId] || _whitelisted[stringToBytes32(ownerId)], "not authorize");
