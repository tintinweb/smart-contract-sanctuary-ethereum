...
// ╚══════╝ ╚═════╝ ╚═╝     ╚═╝ ╚═════╝ ╚══════╝╚═╝  ╚═╝╚═════╝ ╚══════╝ //                                                                    
//                                                                       //
//                  MutantSumoClub contract by 0xSumo                    //
...
contract MutantSumoClub is ERC721, Ownable {

    uint256 public mintPrice = 0 ether;
    uint256 private maxToken = 999;
    ...
    //address for offChanko
    address public offChanko;
    uint256 public offId = 0;

    //authoriser
    address public renderer;
    bool public useRenderer;
    mapping(uint256 => string) public msumoName;
    mapping(uint256 => string) public msumoBio;
    mapping(address => bool) public authoriser;
    event NameChange(uint256 tokenId, string name_);
    event BioChange(uint256 tokenId, string bio_);
    modifier onlyAuthoriser {
        require(authoriser[msg.sender], "Not authorised"); _; 
    }


    constructor () ERC721 ("MutantSumoClub","MSC") {}
    ...
    function mintMutant() external onlySender {
        require(publicSaleEnabled, "publicMint: Paused");
        require(supply.current() <= (maxToken), "No more NFTs");
        require(IOffChanko(offChanko).balanceOf(msg.sender, offId) > 0, "You do not own Off Chanko");
        //still dexide if they gotta have atleast 1 sumo
        //require(IOPSC(OPSC).balanceOf(msg.sender) > 0, "You do not own SUMO");

        IOffChanko(offChanko).eatOffChanko(msg.sender);

        internalmint(msg.sender, 1);
    }

    function changeName(uint256 tokenId, string memory name_) public onlyAuthoriser {
        msumoName[tokenId] = name_; 
    }

    function changeBio(uint256 tokenId, string memory bio_) public onlyAuthoriser {
        msumoBio[tokenId] = bio_; 
    }

    function setRenderer(address address_) external onlyOwner { 
        renderer = address_;
    }

    function setUseRenderer(bool bool_) external onlyOwner { 
        useRenderer = bool_; 
    }

    function setOPSC(address _opsc) public onlyOwner {
        OPSC = _opsc;
    }

    function setOffChanko(address _offChanko) public onlyOwner {
        offChanko = _offChanko;
    }
