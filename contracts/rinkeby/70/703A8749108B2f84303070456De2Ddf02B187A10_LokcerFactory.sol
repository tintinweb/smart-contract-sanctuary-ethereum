...
contract LokcerFactory is Ownable {
    ...
    event FundsWithdrawed (uint funds, uint timestamp);
    event FundsReceived (address sender, uint funds, uint timestamp);

    function createALcoker(IERC20 _token, uint _numOfTokens, uint _unlockTime) payable public {

        require(msg.value >= lockerFee, "Please pay the fee");
        require(_unlockTime > 0, "The unlock time should in future");
        lockerCount++;
        Locker locker = new Locker(lockerCount, _msgSender(), _token, _numOfTokens, _unlockTime, updateLokcerFee);
    ...
    function withdrawFunds() public onlyOwner {
        ...
        emit FundsWithdrawed (balance, block.timestamp);
    }
...
contract Locker {
    ...
    /// @notice this is the function to unclock the locked tokens 
