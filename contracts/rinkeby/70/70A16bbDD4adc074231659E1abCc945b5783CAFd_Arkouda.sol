...
contract Arkouda is ERC20, Ownable, IERC721Receiver {
    ...
    // To store addresses that can burn Arkouda token
    ...
    function startStaking() external onlyOwner {
        require(!stakingLive, "Staking is already live");
        stakingLive = true;
    }
    ...
    // Stakes CBWC NFTs
    function stakeCBWC(uint256[] calldata _tokenIds) external isStakingLive {
        for (uint256 i = 0; i < _tokenIds.length; i++) {
            require(
