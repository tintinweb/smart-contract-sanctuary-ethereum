...
contract TTS is Initializable, ERC721Upgradeable, OwnableUpgradeable, UUPSUpgradeable, ERC2981Upgradeable, SignedAllowance {
    ...
    function TimeTravellerMint (bool whitelisted, uint256 nonce, bytes memory signature) external payable {
        ...
        if (whitelisted) {
            ...
            //if msg.value is less than 0.01 ether, revert
    ...
    function _beforeTokenTransfer(address from, address, uint256) internal view override {
        if (quantumStorage.TransferLocked) { 
            if (!isOwnerOrZero(msg.sender,from)) {
    ...
    function isOwnerOrZero(address _sender, address _addr) internal view returns (bool) {
        return _sender == owner() || _addr == address(0);
    }
