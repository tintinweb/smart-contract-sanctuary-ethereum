...
contract Stake {
    ...
    function setHardcap(uint256 _hardcapValue) public {
        require(msg.sender == operator, "Only operator");
    ...
    function setPoolOpenTill(uint256 _time) public {
        require(msg.sender == operator, "Only operator");
        poolOpenTill = block.timestamp.add(_time);
    }

    function setIsClaimable(bool _isClaimable) public {
        require(msg.sender == operator, "Only operator");
        isClaimable = _isClaimable;
    }

    function setIsEarlywithdrawAvailable(bool _isEarlywithdrawAvailable) public {
        require(msg.sender == operator, "Only operator");
        isEarlywithdrawAvailable = _isEarlywithdrawAvailable;
    }

    function setEarlyWithdrawreward(uint256 _earlyWithdrawreward) public {
        require(msg.sender == operator, "Only operator");
    ...
    function updateAccount(address account, bool _isEarlywithdrawAvailable) private {
        ...
        if (pendingDivs > 0 && _isEarlywithdrawAvailable) {
...
contract stakingFactory is Ownable{
    ...
    function createStack( address _depositTokens,address _rewardToken, uint256 _rewardRate, uint256 _hardcap, uint256 _rewardInterval, address _operator) public onlyOwner returns(address){
        Stake stake =  new Stake( _depositTokens, _rewardToken, _rewardRate, _hardcap, _rewardInterval, _operator);
