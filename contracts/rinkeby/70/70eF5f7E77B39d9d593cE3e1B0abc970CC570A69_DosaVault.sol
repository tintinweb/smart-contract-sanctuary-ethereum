...
contract DosaVault is Initializable{
    ...
    function initialize(address treasury, address _vault_token, address[] calldata users, uint256 rewardsAmt) public initializer returns(bool){
        vault_token = _vault_token;
        factoryAddr = msg.sender;
        _updateTopHolders(users, true);
        IERC20(vault_token).transferFrom(treasury, address(this), rewardsAmt);
        totalRewards = rewardsAmt;
        return true;
    }

    function updateTopHolders(address[] calldata users, bool isTop) external onlyFactory {
        _updateTopHolders(users, isTop);
    }

    function _updateTopHolders(address[] calldata users, bool isTop) internal {
