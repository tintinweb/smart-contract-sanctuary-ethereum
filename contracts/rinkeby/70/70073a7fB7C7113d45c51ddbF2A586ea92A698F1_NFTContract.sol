...
contract NFTContract is ERC721Enumerable
{
    ...
    function hashToSVG(string memory _hash) public view returns (string memory)
    {
        ...
        for (uint8 i = 0; i < 9; i++) 
        {
            ...
            for (uint16 j = 0;j < traitTypes[i][thisTraitIndex].pixelCount;j++) 
            {
                ...
                uint8 x = letterToNumber(NFTContractLibrary.substring(thisPixel, 0, 1));
                uint8 y = letterToNumber(NFTContractLibrary.substring(thisPixel, 1, 2));
                        ...
                        bytes(
                            string(
                                abi.encodePacked(
                                    ...
                                    '", "description": "NFTToken", "image": "data:image/svg+xml;base64,',
                                    NFTContractLibrary.encode(
    ...
    function addTraitType(uint256 _traitTypeIndex, Trait[] memory traits) public onlyOwner
    {
        ...
        {
            traitTypes[_traitTypeIndex].push(Trait(traits[i].traitName,traits[i].traitType,traits[i].pixels,traits[i].pixelCount));
        }
        return;
    }
     /**
