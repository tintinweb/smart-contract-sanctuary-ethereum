...
contract DecaWithTestToken3 is VRFConsumerBaseV2, Pausable, Ownable, ReentrancyGuard {
    ...
    function test(
        ...
        uint256 sortedFace = randomWords.mod(100).add(1);
        // 1 to 50 is Head, 51 to 100 is Cross
        uint256 playerPredict = requestToPredict[requestId];
        address player = requestToSender[requestId];
        uint256 playerBetAmount = requestToBetAmount[requestId];
        ...
        emit ReceivedBetResult(requestId, player, randomWords, sortedFace, playerPredict, playerBetAmount, playerStats[player].currentStreak);
    }
}