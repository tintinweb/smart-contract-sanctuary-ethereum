...
contract CBR is ERC721Enumerable, Ownable {
    ...
    // presale
    function presaleMint(uint256 _mintAmount, uint256 _start, uint256 _end, uint256 _mintableAmount, bytes32[] memory _proof)
        public
        payable
    {
        bytes32 leaf = keccak256(abi.encode(msg.sender, _mintableAmount, _start, _end));
        ...
        require(
            _mintAmount <= _mintableAmount,
            string(abi.encode("Must mint less than or equal to ", _mintableAmount.toString()))
        );
        
        require(msg.sender != owner());

        require(block.timestamp >= _start, "presale sale not yet started");
        
        require(block.timestamp <= _end, "presale sale ended");
        ...
        require(
            ...
            string(abi.encode("Limit token ", tokenCount.toString()))
        );
    ...
    function verifyParams(address _address, uint256 _start, uint256 _end, uint256 _mintableAmount, bytes32[] memory _proof) public view returns (bool) {
        bytes32 leaf = keccak256(abi.encode(_address, _mintableAmount, _start, _end));
