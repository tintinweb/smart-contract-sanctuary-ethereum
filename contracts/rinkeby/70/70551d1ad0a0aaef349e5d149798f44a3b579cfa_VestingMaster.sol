...
contract VestingMaster is ReentrancyGuard, Ownable {
    ...
    function lock(address account, uint256 amount) public onlyOperator {
        LockedReward[] memory oldLockedRewards = userLockedRewards[account];        //locked and timestamp
        ...
        uint256 newStartTimestamp = (currentTimestamp / PERIOD) * PERIOD;     //1664470216
        uint256 newTimestamp;
        LockedReward memory newLockedReward;     //struct
        uint256 jj = 0;
        ...
        for (uint256 i = 0; i < LOCKEDPERIODAMOUNT; i++) {
            newTimestamp = newStartTimestamp.add((i + 1) * PERIOD);   //add 39 seconds each time...
            newLockedReward = LockedReward({
                locked: amount / LOCKEDPERIODAMOUNT,       //100 / 39 = 2.56
