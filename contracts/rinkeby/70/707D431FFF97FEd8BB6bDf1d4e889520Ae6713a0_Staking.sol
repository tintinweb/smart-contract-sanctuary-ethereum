...
contract Staking is IStaking, ERC721, Ownable {
    ...
    ) external payable override onlyNEND {
        require(
    ...
    function distributeRewards() external override onlyNEND {
    ...
    function claimRewards(uint256 _stakeId) external override {
    ...
    function exchangeEAB(uint256 _escrowId) external override {
        ...
        emit EABExchanged(_escrowId);
    }

    // Withdraw all stake and mint escrow
    function withdraw(uint256 _stakeId) external override {
        ...
        returns (address[] memory _stakeTokens)
    {
        ...
        _stakeTokens = new address[](stakeTokenCount);
    ...
    function addStakeToken(address _stakeToken) external override onlyOwner {
    ...
    function removeStakeToken(address _stakeToken) external override onlyOwner {
        if (!addedStakeTokens[_stakeToken]) {
            return;
        }
    ...
    function _afterTokenTransfer(
        ...
        emit EABOwnerChanged(tokenId, from, to);
    }

    modifier onlyNEND() {
        require(msg.sender == nend);
        _;
    }
}
...
interface IStaking {
    ...
    event Withdrawed(uint256 id);
    event EABIssued(
        uint256 indexed escrowId,
        uint256 amount,
        uint256 claimAfter
    );
    event EABExchanged(uint256 indexed escrowId);
    event EABOwnerChanged(
        uint256 indexed escrowId,
