...
contract RoborovskiTraits is Ownable {
    ...
    mapping(uint8 => uint8) public traitCount;
    ...
    function setTraitCount(uint8[] memory traitIds, uint8[] memory counts)
        public
        onlyOwner
    {
    ...
    function _makeTemp(uint16 tokenId)
        private
        view
        returns (Temp memory _temp)
    {
        ...
        _temp.name = IRoboShort(ROBO).tokenName(uint256(tokenId));
    }
        ...
        returns (string memory)
    {
        ...
        traits1 = string(
            abi.encodePacked(
                ...
                _attributeForTypeAndValue(
                    traitTypes[4],
                    traitData[4][_byteToUint8(_temp.traitBytes[4])]
                ),
                ",",
                _attributeForTypeAndValue(
                    traitTypes[5],
        ...
        traits2 = string(
            abi.encodePacked(
                ...
                _attributeForTypeAndValue(
                    traitTypes[8],
                    traitData[8][_byteToUint8(_temp.traitBytes[8])]
                ),
                ",",
                _attributeForTypeAndValue(
                    traitTypes[9],
                    traitData[9][_byteToUint8(_temp.traitBytes[9])]
                ),
                ",",
                _attributeForTypeAndValue(
                    traitTypes[10],
                ...
                _attributeForTypeAndValue(
                    traitTypes[12],
                    traitData[12][_byteToUint8(_temp.traitBytes[12])]
                )
            )
        );
    ...
    function tokenURI(uint256 tokenId) external view returns (string memory) {
        return
            string(
                abi.encodePacked(
                    "data:application/json;base64,",
                    _base64(_compileMetadata(_makeTemp(uint16(tokenId))))
                )
            );
    }
...
interface IRoboStoreShort {
    function getIpfsHashHex(uint16 tokenId)
        external
        view
        returns (bytes memory);

    function getIpfsHash(uint16 tokenId) external view returns (string memory);

    function getTraitBytes(uint16 tokenId) external view returns (bytes memory);
}
