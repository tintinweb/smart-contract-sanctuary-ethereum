    ...
    IMatchOrders
{
        ...
        internal
    {
        ...
        if (feesManager != address(0)) {
            (address addressAsset, uint256 assetId) = _getMakerAssetId(rightOrder.makerAssetData);
            address tokenAddress = _getTokenAddress(rightOrder.takerAssetData);
            uint256 totalSaleAmount = rightOrder.takerAssetAmount + rightOrder.takerFee;

            IFeesManager(feesManager).receiveFees(addressAsset, assetId, tokenAddress, totalSaleAmount);

            (addressAsset,assetId) = _getMakerAssetId(leftOrder.makerAssetData);
            tokenAddress = _getTokenAddress(leftOrder.takerAssetData);
            totalSaleAmount = leftOrder.takerAssetAmount + leftOrder.takerFee;
...
interface IFeesManager {
    function receiveFees(address assetAddress, uint256 id, address tokenAddress, uint256 salePrice) external;
}
    ...
    MixinSignatureValidator
{
    ...
    function _getMakerAssetId(bytes memory assetData) internal returns (address, uint256) {
        (
        ...
        require(assetAddress != address(0), "asset address should not be zero address");

        return (assetAddress, ids[0]);
    }
        ...
        internal
    {
        ...
        if (feesManager != address(0)) {
            ...
            uint256 totalSaleAmount = order.makerAssetAmount + fillResults.makerFeePaid;
            ...
            if(order.makerAssetData.length > 36){
                ...
                totalSaleAmount = order.takerAssetAmount + fillResults.takerFeePaid;
            }

            (address addressAsset, uint256 assetId)  = _getMakerAssetId(lhsAssetData);
