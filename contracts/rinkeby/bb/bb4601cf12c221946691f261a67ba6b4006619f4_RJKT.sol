...
contract RJKT is ERC721A, Ownable {
    using Strings for uint256;
    ...
    constructor() ERC721A("RJKT", "RJKT") {
        setHiddenMetadataUri("ipfs://xxx");
        setBaseURI("ipfs://xxx/");
    }
        ...
        returns (uint256 price)
    {
        ...
        if (totalMints <= maxFreePerWallet) {
            return cost;
        } else if (balanceOf(msg.sender) >= maxFreePerWallet) {
            uint256 total = costNotFree * _mintAmount;
            return total;
        } else {
            uint256 total = costNotFree * (totalMints - maxFreePerWallet);
            return total;
        }
    }
