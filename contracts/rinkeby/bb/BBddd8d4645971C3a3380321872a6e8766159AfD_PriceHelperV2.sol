...
/**
 * @title Price Helper
 * @dev Simple spot price provider from ETH to USDT.e
 */
contract PriceHelperV2 {
    using SafeMath for uint256;

    address public WETH = address(0xc778417E063141139Fce010982780140Aa0cD5Ab);
    address public USDTe = address(0x3B00Ef435fA4FcFF5C209a37d1f3dcff37c705aD);
    address public Factory =
        address(0x5C69bEe701ef814a2B6a3EDD4B1652CB9cc5aA6f);

    function getETHtoUSD(uint256 amountWETH) external view returns (uint256) {
        (uint256 reservesWETH, uint256 reservesUSDTe) = AMMLibrary.getReserves(
            Factory,
            WETH,
            USDTe
        );

        uint256 currentETHprice = AMMLibrary.quote(
            1e18,
            reservesWETH,
            reservesUSDTe
        );

        return amountWETH.mul(currentETHprice).div(1e18);
    }
}