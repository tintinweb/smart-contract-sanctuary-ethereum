...
contract ARMORSLAYSDemo is ERC721, ERC721Enumerable, Ownable, ERC721Burnable, EIP712{
    using Strings for uint256;

    uint256 public constant PRICE = 0.1 ether;
    uint256 public constant MAX_SUPPLY = 100000;
    address private constant ARTIST_ADDRESS = 0x6471DAC705f93593893c268FaB33186Eaf5591cd;
    address private constant SIGNER_ADDRESS= 0x6e4d5431b9EE9Be61E73130F51C96c9c017495fE;

    bool public publicSale;
    mapping(uint256=>bool) public usedNonce;
    string private baseURI_ = "ipfs://Qme4t1i3bh7LtA1ssHx1F2K3PBnnPni9sEBbBmjEMGSzmo/0.json";

    constructor() ERC721("ARMORSLAYS", "AMS") EIP712("ARMORSLAYS","1"){}
    ...
    function airDrop(address[] calldata recievers) external onlyOwner {
        require(recievers.length<=20, "Too high quntity");
        require(totalSupply() + recievers.length <= MAX_SUPPLY,  "Out of Stock");

        for (uint256 i = 0; i < recievers.length; i++) {
            _safeMint(recievers[i], totalSupply());
        }
    }
    
    function privateMint(uint256 nonce, uint256 price, uint256 startAt, uint256 endAt, bytes memory signature) external payable {
    ...
    function publicMint(uint256 tokenQuantity) external payable{
        require(publicSale, "Only Private Sale");
        require(tokenQuantity <= 2, "Too high quntity");
        require(tokenQuantity > 0, "Mint at least one");
        require(PRICE * tokenQuantity <= msg.value,  "INSUFFICIENT_ETH");
        require(totalSupply() + tokenQuantity <= MAX_SUPPLY,  "Out of Stock");
    ...
    function withdraw() external onlyOwner {
        payable(msg.sender).transfer(address(this).balance * 1 / 5);
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        // all token will return the same uri
