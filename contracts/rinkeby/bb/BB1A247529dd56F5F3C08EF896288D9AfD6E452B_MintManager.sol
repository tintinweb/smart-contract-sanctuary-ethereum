...
contract MintManager is BaseManager {
    ...
    //             Events
    ////////////////////////////////////////////////
    event AddConfigEvent(Config config) ;
    ...
    function initNftAddr(address spaceshipAddr, address defensiveFacilityAddr, address heroAddr) external returns (bool) {
        spaceship = IMITNft(spaceshipAddr) ;
        defensiveFacility = IMITNft(defensiveFacilityAddr) ;
        hero = IMITNft(heroAddr) ;
        return true ;
    ...
    function _mint(uint256 [] memory tokenIds, address owner, uint16 style, uint8 cIndex, KIND kind) internal returns (bool) {
    ...
    function _mintSuit(uint256 [] memory sIds, uint256 [] memory dIds, uint256 [] memory hIds, address owner, uint16 style, uint8 cIndex) internal returns (bool) {
    ...
    function recordStyleConfig(uint256 [] memory tokenIds, uint16 style, uint256 cIndex) internal  {
    ...
    // check mint config
    function checkMintConfig (Config memory config, uint256 mintCount) internal {
    ...
    function initGens(uint256 [] memory tokenIds, KIND kind) external onlyRole(SIGN_ROLE) returns(bool){
        ...
        if(kind == KIND.SPACESHIP) {
            address [] memory owners = spaceship.batchOwnerOf(tokenIds);
            uint256 [] memory gens = createGens(tokenIds, owners) ;
    ...
    function createGens(uint256 [] memory tokenIds,address [] memory owners) internal view returns(uint256 [] memory) {
    ...
    function getValues(uint16 [] memory rate, uint16 [] memory values, uint16 random) internal pure returns(uint16) {
    ...
    function _checkMigrationSign(uint256 [] memory tokenIds,uint256 [] memory genes, KIND kind, bytes memory signature) internal view {
        string memory method = "" ;
        if(kind == KIND.SPACESHIP) {
