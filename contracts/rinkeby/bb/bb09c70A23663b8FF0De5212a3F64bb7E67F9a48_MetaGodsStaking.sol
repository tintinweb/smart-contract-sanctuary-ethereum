...
contract MetaGodsStaking is Initializable, OwnableUpgradeable, PausableUpgradeable {
    ...
    function getYieldForAll(address wallet_) external view returns (uint256) {

        uint256 amount;

        for (uint256 i = 0; i < yieldingCollections.length; ++i) {

            if (stakedTokensByWallet[wallet_][yieldingCollections[i]].length > 0) {

                for (uint256 j = 0; j < stakedTokensByWallet[wallet_][yieldingCollections[i]].length; ++j) {
                    amount += getYield(
                        yieldingCollections[i],
                        stakedTokensByWallet[wallet_][yieldingCollections[i]][j],
                        false
                    );
                }
            }
