...
contract ISMToys is ERC721, ERC721Enumerable, ERC721Burnable, Ownable {
    ...
    uint256 public whitelistEndtime = 0;

    string internal baseTokenURI;

    mapping(address => uint256) public _whitelist;

    constructor() ERC721("ISM Toys", "ISM") {}
    ...
    function mint(uint256 _numOfTokens) public payable {
        ...
        require(block.timestamp >= whitelistEndtime, "Whitelist Sale is active");
    ...
    function mintWhitelist(uint256 _numOfTokens) public payable {
        uint256 supply = totalSupply();

        require(block.timestamp >= publicEndtime, "Public Sale is active");
        require(block.timestamp <= whitelistEndtime, "Whitelist Sale is not active");
        require(_numOfTokens <= _whitelist[msg.sender], "Exceeded max available to purchase");
        require(supply + _numOfTokens <= maxSupply, "Sold out!");
        require(price * _numOfTokens <= msg.value, "Ethereum amount sent is not correct");
    ...
    function setWhitelist(address[] calldata _addresses, uint8 _maxPerWallet) public onlyOwner {
        for (uint256 i; i < _addresses.length; i++) {
            _whitelist[_addresses[i]] = _maxPerWallet;
        }
    }
