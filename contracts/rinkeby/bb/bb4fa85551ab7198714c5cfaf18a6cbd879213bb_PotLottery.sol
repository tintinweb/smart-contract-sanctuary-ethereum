...
contract PotLottery is ReentrancyGuardUpgradeable {
    ...
    function getPrice(string memory tokenName) public view returns (uint256 price) {
        IUniswapV2Factory factory = IUniswapV2Factory(uniswapV2FactoryAddr);
        IUniswapV2Pair pair = IUniswapV2Pair(factory.getPair(tokenWhiteList[tokenName].tokenAddress, usdtAddr));
        IPRC20 wethToken = IPRC20(pair.token0());
