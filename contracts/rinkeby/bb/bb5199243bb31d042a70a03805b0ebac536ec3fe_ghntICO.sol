...
contract ghntVesting {
    ...
    constructor(address _user, uint [] memory _vesting, uint _vestingLength, address _token, address _ico, uint _amount) {
        ...
        for (uint z = 0; z <_vestingLength; z++){        //All of the preset values in the array...  (0 or 25)
            vesting[z] = _vesting[z];                   //u
        }
      ...
      function withdraw(address _user) public  {
        ...
        require(monthsPast < vestingLength, "withdraw complete");      //65
        require(block.timestamp - lastTime > month, "A month has not passed since last withdraw/withdraw complete");
        uint _time = block.timestamp - lastTime;     //how many seconds have passed since you last withdrawn
        uint _amount = 0;
        uint latest = monthsPast + _time / month;    //how many months have already passed + how much time has passed since last claim- In terms of months. (divisiion)   
...
contract ghntICO {
     ...
     function withdrawAdmin(address _token,uint _amount) public {
        require(_token!=address(0),"Cannot insert zero address");
        ERC20(_token).transferFrom(address(this),msg.sender,_amount);           //allowance by the contract tho? Can't be done.
