...
contract Partner is ERC721Enumerable {
  uint256 supply;
  constructor() ERC721("Partner", "P") {}
...
// rename
contract Nugs is ERC721A, ERC2981, Ownable {

  // Ray Ray's home on chain
  ...
  // A mapping of Ray Ray IDs to whether they've been used to mint a Poo Poo
  ...
  // 0 - 19,999 Poo Poos
  uint256 public constant MAX_SUPPLY = 19_999;

  // Reserving 100 Poo Poos for the team
  uint256 public constant TEAM_MINT_SUPPLY = 100;

  // Whether the team has claimed their 100 Poo Poos
  ...
  MintState public _mintState = MintState.CLAIM_OR_PURCHASE; // todo: remove default value
  ...
  // todo: rename and change values
  constructor() ERC721A("Nugs", "NUGS") {}

  //todo: remove this line and partner below, use the ray ray address above
  /**
  @notice 10,000 Poo Poos, claimable by Ray Ray holders. Use the `checkIfClaimed` method to see if your Ray Ray
  has been used to claim a Poo Poo yet
  */
  function claimPooPoos(address _partner) external squatting() noContracts() {
    uint256[] memory _rayRayIDs = walletOfRayRayOwner(msg.sender, _partner); // todo: remove partner
  ...
  function debug_claimPooPoos(uint256 _amount) external {
    if (_amount == 0) revert RayRaysEmpty();
    if (_amount == 6) revert MintClosed();
    if (_amount == 10) revert NoContracts();

    _mint(msg.sender, _amount);
  }

  /**
  @notice 10,000 Poo Poos for sale
  */
  ...
  /**
  @notice Team reserves 100 Poo Poos for promotion and marketing purposes
  */
  ...
  /**
  @notice Sets whether the contract is open for use
  */
  // todo: remove partner
  function walletOfRayRayOwner(address _owner, address _partner) internal returns(uint256[] memory) {
    IRayRay rr = IRayRay(RAY_RAYS_CONTRACT);
    // IRayRay rr = IRayRay(_partner); // todo: remove after testing
    uint256 tokenCount = rr.balanceOf(_owner);

    uint256[] memory tokensId = new uint256[](tokenCount);
    for(uint256 i; i < tokenCount; i++){
        tokensId[i] = rr.tokenOfOwnerByIndex(_owner, i);
    }
  ...
  */
  function setMintState(uint256 status) external onlyOwner {
    require(status <= uint256(MintState.PURCHASE), "Bad status");
  ...
  */
  ...
  /**
  @notice Checks if the Ray Ray ID was used to mint a Poo Poo yet
  */
