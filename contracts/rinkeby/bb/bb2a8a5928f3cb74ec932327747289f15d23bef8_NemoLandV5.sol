...
contract NemoLandV5 is Admin, ERC721Upgradeable {
    struct OnSaleLand {
        uint256 price;
        address saleOwner;
    }
    ...
    event Referral(string indexed referral, uint256 indexed id);
    ...
    function mintLand(
        ...
        require(
            isValidRequest(_signature, _id, _zoneId, referral, withRefarral),
            "Invalid sign"
        );
    ...
    function isValidRequest(
        ...
        bool isValid = _validSignature(
            _signature,
            keccak256(
                abi.encode(_id, _zoneId, msg.sender, referral, withRefarral)
            )
        );
        return isValid;
    }
