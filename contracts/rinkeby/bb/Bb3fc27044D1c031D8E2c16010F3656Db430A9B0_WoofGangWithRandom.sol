...
import "./Abstract_WoofGang.sol";

contract WoofGangWithRandom is Abstract_WoofGang {
    ...
    uint16 constant MAX_SUPPLY = 9;    

    // Zero Address
    address constant ZERO_ADDRESS = address(0);

    // Declaring state variables of type array
    uint256[MAX_SUPPLY] public mintIds;

    // This Flag is using for remaining token id set in array
    bool flag = true;
    ...
    function mint(uint256 quantity) external override payable whenNotPaused nonReentrant
    {
        ...
        require(block.timestamp <= mintEndTime , "WoofGang: Mint process is ended");
        require((msg.value)*quantity >= mintPrice*quantity, "WoofGang: Insufficient ETH supplied");
        for(uint16 i=0; i < quantity; i++){
            require(MAX_SUPPLY > totalSupply, "WoofGang: Maximum supply is exceed");
            uint256 tokenId = _getRandomNFTTokenID();
            totalSupply++;
            _mint(msg.sender, tokenId, NFT_AMOUNT, "");
            mintIds[tokenId]=tokenId;
    ...
    function airDrop(address[] memory account) external override onlyOwner whenNotPaused {
    ...
    function mintRemainingToOwner(uint256 amount) external override onlyOwner whenNotPaused 
    {
        require(block.timestamp >= mintEndTime , "WoofGang: Mint process is in progress");
        require(MAX_SUPPLY > totalSupply, "WoofGang: Maximum supply is exceed");
        require(amount <= MAX_SUPPLY-totalSupply);        
        for(uint i = amount; i > 0; i--) {
            if(_pendingIds[i] == 0) {
                uint mintTokenIds = _pendingIds[_pendingIds.length-1];
                _mint(msg.sender, mintTokenIds ,NFT_AMOUNT, "");
                _pendingIds.pop();
                totalSupply++;
                emit NFTAirDrop(mintTokenIds);
            }            
        }
    }
...
interface IWoofGang {
     ...
     */
    function airDrop(address[] memory account) external;
...
abstract contract Abstract_WoofGang is ERC1155, Ownable, Pausable, ReentrancyGuard, Random,IWoofGang {
