 ...
 */
contract LazyMint is ERC721SingleCreatorExtensionBase, ICreatorExtensionTokenURI, ReentrancyGuard, AdminControlUpgradeable {
    ...
    function supportsInterface(bytes4 interfaceId) public view virtual override(AdminControlUpgradeable, IERC165) returns (bool) {
        return interfaceId == type(ICreatorExtensionTokenURI).interfaceId || AdminControlUpgradeable.supportsInterface(interfaceId);
    }

    function premint(address[] memory to) external adminRequired {
        _premint(to);
    }
    ...
    function withdraw(address _to, uint amount) external adminRequired {
