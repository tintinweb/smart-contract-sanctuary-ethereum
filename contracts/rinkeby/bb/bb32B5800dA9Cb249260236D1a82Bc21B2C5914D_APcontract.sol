...
contract APcontract is Ownable, ERC721A, PaymentSplitter {
    ...
    constructor(address[] memory distro, uint[] memory distro_shares, address[] memory teamclaim, bytes32 _merkleRoot)
        ERC721A("Axxxxx-Pxxxxxxx", "APTEST")
    ...
    function setRandomValue(address account, uint lowValue, uint highValue) external onlyOwner returns (uint) {
        // disable this for testing only so we can test multiple times...
        // also I made this external / onlyOwner so no one but owner can run it.
