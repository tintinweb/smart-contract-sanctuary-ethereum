...
/**
 * @title ERC-721 OG Token Standard, optional metadata extension
...
/**
 * @dev Implementation of https://eips.ethereum.org/EIPS/eip-721[ERC721] OG Token Standard, including
...
/**
 * @title ERC-721 OG Token Standard, optional enumeration extension
 * @dev See https://eips.ethereum.org/EIPS/eip-721
 */
...
contract LORE is ERC721Enumerable, Ownable, ReentrancyGuard {
    using Counters for Counters.Counter;
    using Strings for uint256;

    uint256 public constant LORE_PUBLIC = 250;
    uint256 public constant LORE_MAX = LORE_PUBLIC;
    uint256 public constant PURCHASE_LIMIT = 10;
    uint256 public constant PRICE = 13_000_000_000_000_00; // 0.02 ETH
    ...
    Counters.Counter private _publicLORE;
    ...
    {
        require(
            _publicLORE.current() < LORE_PUBLIC,
            "Purchase would exceed LORE_PUBLIC"
        );

        for (uint256 i = 0; i < numberOfTokens; i++) {
            uint256 tokenId = _publicLORE.current();

            if (_publicLORE.current() < LORE_PUBLIC) {
                _publicLORE.increment();
                _safeMint(to, tokenId);
            }
        }
    }
