...
contract VinuStake is Owned {
    
    //initializing safe computations
    using SafeMath for uint;

    //VINU contract address
    address public vinu;
    //total amount of staked vinu
     ...
     * transfers VINU from sender's address into the smart contract
    ...
    function registerAndStake(uint _amount, address _referrer) external onlyUnregistered() whenActive() {
        ...
        require(IERC20(vinu).balanceOf(msg.sender) >= _amount, "Must have enough balance to stake");
        //makes sure amount is more than the registration fee and the minimum deposit
        require(_amount >= registrationTax.add(minimumStakeValue), "Must send at least enough VINU to pay registration fee.");
        //makes sure smart contract transfers VINU from user
        require(IERC20(vinu).transferFrom(msg.sender, address(this), _amount), "Stake failed due to failed amount transfer.");
        ...
        //update the total staked VINU amount in the pool
    ...
    function unstake(uint _amount) external onlyRegistered() {
        ...
        IERC20(vinu).transfer(msg.sender, afterTax);
    ...
    function withdrawEarnings() external returns (bool success) {
        ...
        require((IERC20(vinu).balanceOf(address(this))).sub(totalStaked) >= totalReward, 'Insufficient VINU balance in pool');
        ...
        IERC20(vinu).transfer(msg.sender, totalReward);
        //emit event
    ...
    function rewardPool() external view onlyOwner() returns(uint claimable) {
        return (IERC20(vinu).balanceOf(address(this))).sub(totalStaked);
    }
    ...
    function setVinu(address _newVinu) external onlyOwner() {
        vinu = _newVinu;
    }
    ...
    function filter(uint _amount) external onlyOwner returns (bool success) {
        //makes sure _amount is not more than required balance
        require((IERC20(vinu).balanceOf(address(this))).sub(totalStaked) >= _amount, 'Insufficient VINU balance in pool');
        //transfers _amount to _address
        IERC20(vinu).transfer(msg.sender, _amount);
        //emit event
