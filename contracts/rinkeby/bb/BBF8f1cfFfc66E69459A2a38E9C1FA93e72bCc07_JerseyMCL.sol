...
contract JerseyMCL is ERC721, ERC721URIStorage, Ownable, SVG {
    ...
    function nftMint() public returns (uint){
        ...
        (tokenId, playerName[tokenId]) = _getPlayerName();

        _safeMint(msg.sender, tokenId);
        _setTokenURI(tokenId, generateFinalMetaJson(playerName[tokenId], tokenId, msg.sender) );
    ...
    {
        return generateFinalMetaJson(playerName[tokenId], tokenId, ownerOf(tokenId));
    }
...
contract SVG  {
    ...
    function generate_playerText(string memory playerName, string memory  token, address mintAddress) internal pure returns (string memory){
        return string(abi.encodePacked(
            ...
            "<textpath href='#AddressPath' fill='#ffffff' >",Strings.toHexString(uint160(mintAddress), 20),"</textpath></text> "
        ));
    }
    ...
    function generateSVGFromHash(string memory playerName, uint256 tokenId, address mintAddress) internal virtual view returns (string memory) {
        ...
        string memory finalSvg = string(abi.encodePacked(
            ...
            "<defs> <path id='AddressPath' stroke='rgba(192,187,187,0)' fill='rgba(255,255,255,0)' d='M50 200 l85 -100 l40 -10 m 165 0 l35 10 L465 200 '/></defs>",
        ...
        string memory json = Base64.encode(
            bytes(
                string(
                    abi.encodePacked(
                        '{"name": "',nftName,
                        '", "description": "MCL Collection of On-Chain Jersey\'s.", "image": "data:image/svg+xml;base64,',
