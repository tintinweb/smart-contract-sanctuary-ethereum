...
function depositEns(string calldata _label, uint256 _royalty, uint256 _mintPrice, string calldata _defaultImageUri) payable external {
    ...
    function depositEns(string calldata _label, uint256 _royalty, uint256 _mintPrice, string calldata _defaultImageUri, IRegister _registrationStrategy) payable public {
        ...
        if(_royalty > 0){
            ...
            emit UpdateDomainRoyaltyPercentage(msg.sender, id, _royalty);
        }

        if(_mintPrice > 0){
            setMintPrice(id, _mintPrice);  
            emit UpdateDefaultMintPrice(msg.sender, id, _mintPrice);          
        }

        if(bytes(_defaultImageUri).length > 0){
            setSubdomainImageUri(id, _defaultImageUri);
            emit UpdateDomainDefaultImage(msg.sender, id, _defaultImageUri);
        }
