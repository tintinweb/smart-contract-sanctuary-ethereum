...
contract Habit is ERC721 {
    ...
    function getAllHabits() public view returns (HabitDataWithOwner[] memory) {
        HabitDataWithOwner[] memory allHabits = new HabitDataWithOwner[](
            _habitIds.current()
        );
        for (uint256 i; i < _habitIds.current(); ++i) {
            allHabits[i] = HabitDataWithOwner(habits[i], ownerOf(i));
        }
        return allHabits;
    }
