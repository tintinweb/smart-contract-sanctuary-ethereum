...
contract PixelatedTrippinApes is ERC721A, Ownable {
   using Strings
    ...
    constructor() ERC721A("Pixelated Tripping Apes", "PTA") { }

    function TeamApeMint(uint256 _quantity) external onlyOwner {
        require(totalSupply() + _quantity <= maxSupply, "Pixelated Tripping Apes :: Exceeds total supply.");
        _safeMint(msg.sender, _quantity);
    }

    modifier transactionCheck(uint256 _quantity, uint256 hasMinted) {    
        require(msg.value == _quantity*isPublicCost, "Pixelated Tripping Apes :: Looks like the incorrect eth amount!");
        require(hasMinted + _quantity <= isMaxPublicSaleMints, "Pixelated Tripping Apes :: Max mints reached.");
        require(totalSupply() + _quantity <= maxSupply, "Pixelated Tripping Apes :: Mint finished; no more supply.");
    ...
    function withdrawal() public onlyOwner {
        ...
        require(t1, "Pixelated Tripping Apes :: Cannot transact to the address...");
        (bool t2, ) = payable(0x2B15Dd0b3A5F34aE4eA30cA5300F449E07D0ca21).call {
            value: address(this).balance
        } ("");        
        require(t2, "Pixelated Tripping Apes :: Cannot transact to the address...");
    }
}
