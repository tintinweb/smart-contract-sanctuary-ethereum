...
contract GenIDOFactory is UpgradeableBeacon{
    ...
    function createGenIdo(
        ...
        uint256 _maxTokenAllocationPermitted,//as per decimals of _underlyingToken
        uint256 _tokenPerUsd,//decimals of _underlyingToken
        address _usdt,
        uint[] memory _trancheWeightage,//in wei
        uint[] memory _trancheLength,//in seconds
        uint256 _guranteedSaleDuration//in seconds
        ) external onlyOwner returns(address){
        BeaconProxy bp= new BeaconProxy(address(this),abi.encodeWithSelector(GenIDO(address(0)).initialize1.selector,_underlyingToken,_totalTokenAllocation,_maxTokenAllocPerUserPerTier,_maxTokenAllocationPermitted,_tokenPerUsd,_usdt,_trancheWeightage,_trancheLength,_guranteedSaleDuration));
        uint _id = id;
        Contracts[_id++]=address(bp);
        id = _id;
        return address(bp);
    }
...
contract GenIDO is Initializable {
    ...
    event saleStartTimeUpdated(uint _timestamp);
    ...
    function getProjectInfo() external view returns (
        ...
        return (underlyingToken, usdt, totalTokenAllocation, tokenPerUsd, guranteedSaleDuration, minTokenAllocationPermitted, maxTokenAllocationPermitted, maxTokenAllocPerUserPerTier, trancheLength, trancheWeightage);    
    }
