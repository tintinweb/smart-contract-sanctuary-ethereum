...
contract ToonaFeesh is ERC721A, Ownable, ReentrancyGuard {
    ...
    modifier presaleEnabled() {
        require(presaleStart, "Whitelist mint has not started");
        _; 
    }

    modifier publicSaleEnabled() {
        require(publicSaleStart, "Whitelist mint has not started");
        _; 
    }
    ...
    function setPublicSaleStart(bool _start) public onlyOwner {
        presaleStart = false;
        publicSaleStart = _start;
    }

    function preSaleMint(bytes32[] calldata _proof, uint256 nMints) external isValidMerkleProof(_proof, presaleMerkleRoot) nonReentrant presaleEnabled {
        nMints = 1;
    ...
    function mint(uint256 nMints) external nonReentrant publicSaleEnabled {
        nMints = 1;
