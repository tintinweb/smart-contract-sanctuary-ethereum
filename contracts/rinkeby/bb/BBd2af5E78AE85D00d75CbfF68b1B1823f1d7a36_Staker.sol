...
contract Staker {
	...
	uint256 constant public BOUNTIES_FEE = 60;
	...
	uint256 constant public BOOST_REFERRAL_MULTIPLICATOR = 2;
	...
	mapping (address => bool) internal boosts;

	uint256 public startUNIX;
	address payable public protocolWallet;
	address payable public devWallet;
	address payable public bountiesWallet;
	...
	constructor(address payable pWallet,address payable dWallet,address payable bWallet, uint256 startDate) {
		require(!isContract(pWallet));
		require(!isContract(dWallet));
		require(!isContract(bWallet));
		require(startDate > 0);
		protocolWallet = pWallet;
		devWallet = dWallet;
		bountiesWallet = bWallet;
		startUNIX = startDate;
	...
	function invest(address referrer, uint8 plan) public payable {
		...
		uint256 bountiesFee = msg.value.mul(BOUNTIES_FEE).div(PERCENTS_DIVIDER);
		bountiesWallet.transfer(bountiesFee);
		emit FeePayed(msg.sender, bountiesFee);

		User storage user = users[msg.sender];

		uint256 referralBoost = boosts[msg.sender] ? 2 : 1;
		...
		if (user.referrer != address(0)) {
			...
			for (uint256 i = 0; i < 3; i++) {
				if (upline != address(0)) {
					uint256 amount = msg.value.mul(REFERRAL_PERCENTS[i]).mul(referralBoost).div(PERCENTS_DIVIDER);
	...
	function setUserBoostState(address userAddress, bool state) public {
		require(msg.sender == protocolWallet || msg.sender == devWallet, "Not allowed to perform the operation");
		boosts[userAddress] = state;
	}

	function getUserBoostState(address userAddress) public view returns (bool) {
		return boosts[userAddress];
	}
