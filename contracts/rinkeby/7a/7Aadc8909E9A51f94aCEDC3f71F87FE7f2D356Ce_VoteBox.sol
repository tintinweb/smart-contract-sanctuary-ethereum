...
contract VoteBox is IVoteBox, Ownable, EIP712VoteAllowlist, ReentrancyGuard {
    ...
    // update poll content
    function setPollContent(uint256 _pid, string calldata _content) external onlyOwner existsPoll(_pid) {
        polls[_pid].content = _content;
    }

    // update poll option name
    function setPollOptionName(uint256 _pid, uint256 _option, string calldata _name) external onlyOwner existsPoll(_pid) {
    ...
    function setPollStartTime(uint256 _pid, uint32 _time) external onlyOwner existsPoll(_pid) {
        polls[_pid].startTime = _time;

        emit PollChangedStartTime(_pid, _time);
    }
    function setPollEndTime(uint256 _pid, uint32 _time) external onlyOwner existsPoll(_pid) {
