...
contract NFTMarket is ReentrancyGuard, Ownable, ERC1155Holder, ERC721Holder {
    ...
    IERC20 public POWAddress;

    mapping(address => uint256) public escrow;
    // mapping(address => uint256) public allowedToWithdraw;

    mapping(address => bool) public collectionStatus;
    mapping(address => bool) public collectionType; // false if ERC721, true if ERC1155
    ...
    error OwnAsk();
    error QuantityExceedsAsk();
    error NotOwnerOfToken();
    error LowEscrowBalance();

    // struct Collection { //do not require this
    //     bool isFractional;
    //     string name;
    //     // need planet details, hero details?
    //     uint16 creatorRoyaltyInPercentage; // change it to a single variable?
    //     uint256 totalSupply;
    //     uint256 listedTime;
    ...
    event CollectionImported(address collectionAddress, uint256 timestamp);
     ...
     * @param _POWAddress               POW Token Address
     */
    constructor(IMarketplaceFeeContract _feeContract, IERC20 _POWAddress) {
    ...
    // this function would not be required
     ...
     * @param isFractional                  False if ERC721, True if ERC1155
     */
    function createCollection(address _nftContract, bool isFractional)
        public
        onlyOwner
    {
        if (collectionStatus[_nftContract]) revert ExistingCollection(); // Checks if collection already exists

        collectionStatus[_nftContract] = true;
        collectionType[_nftContract] = isFractional;
        collections.push(_nftContract);

        emit CollectionImported(_nftContract, block.timestamp);
    }
     ...
     * @param _nftContract  An array of ERC-1155 addresses
     ...
     * @param _quantity     Number of tokens the seller is willing to sell
     */
    function ask(
        ...
        for (uint256 i = 0; i < _nftContract.length; i++) {
            ...
            if (!collectionStatus[nftContract]) revert CollectionNotActive();
            if (price <= 0) revert InvalidPrice();
            if (quantityOf(nftContract, msg.sender, tokenId) <= 0)
                revert NotAuthorized();
            if (!isApproved(_nftContract[i])) revert NotApproved();
     ...
     * @param _quantity     Number of tokens the buyer is willing to buy
     */
    function bid(
        address[] memory _nftContract,
        uint256[] memory _tokenId,
        ...
        // check and get ERC20 token
        if (!POWAddress.transferFrom(msg.sender, address(this), totalPrice))
            revert TransferFailed();
    ...
    function cancelAsk(
        ...
        for (uint256 i = 0; i < _nftContract.length; i++) {
            ...
            asks[nftContract][tokenId][askIndex].exists = false;
    ...
    function cancelBid(
        ...
        for (uint256 i = 0; i < _nftContract.length; i++) {
            ...
            if (buyer != msg.sender) revert NotAuthorized();

            escrow[msg.sender] -= (price * quantity);
            totalPayable += price * quantity;

            bids[nftAddress][tokenId][bidIndex].exists = false;
        ...
        if (!POWAddress.transfer(msg.sender, totalPayable))
            revert TransferFailed();
    }
    ...
    function acceptAsk(
        ...
        for (uint256 i = 0; i < _nftContract.length; i++) {
            ...
            if (!exists) revert AskDoesNotExist();
            if (seller == msg.sender) revert OwnAsk();
            if (quantity > asks[nftContract][tokenId][askIndex].quantity)
                revert QuantityExceedsAsk();

            totalPrice += (price * quantity);

            // Quantity will reduce in case of ERC1155 tokens
            if (collectionType[nftContract]) {
                asks[nftContract][tokenId][askIndex].quantity -= quantity;
                if (asks[nftContract][tokenId][askIndex].quantity == 0) {
                    asks[nftContract][tokenId][askIndex].exists = false;
                }
            } else {
                asks[nftContract][tokenId][askIndex].exists = false;
            ...
            uint256 fee = feeContract._collectMarketFee(
                seller,
                price * quantity
            );
            if (
                !POWAddress.transferFrom(
                    msg.sender,
                    seller,
                    (price * quantity) - fee
                )
            ) revert TransferFailed();
            if (!POWAddress.transferFrom(msg.sender, address(feeContract), fee))
                revert TransferFailed();
            emit AskAccepted({
                nft: nftContract,
                buyer: msg.sender,
    ...
    function acceptBid(
        ...
        for (uint256 i = 0; i < _nftContract.length; i++) {
            if (quantityOf(_nftContract[i], msg.sender, _tokenId[i]) <= 0)
            ...
            if (!isApproved(_nftContract[i])) revert TransferFailed();
            ...
            if (collectionType[nftContract]) {
                ...
                if (bids[nftContract][tokenId][bidIndex].quantity == 0) {
                    bids[nftContract][tokenId][bidIndex].exists = false;
                }
            } else {
                bids[nftContract][tokenId][bidIndex].exists = false;
            }
            ...
            if (escrow[buyer] > balancePayable) revert LowEscrowBalance();
            ...
            if (!POWAddress.transferFrom(buyer, address(feeContract), fee))
                revert TransferFailed();
            emit BidAccepted({
                nft: nftContract,
                tokenID: tokenId,
    ...
    //  */
    // function withdraw(int256 amount) public onlyOwner {
    ...
    function safeTransferFrom_(
        ...
        if (collectionType[nftContract]) {
            startingBalance = IERC1155(nftContract).balanceOf(to, tokenId);
            ...
            if (
                IERC1155(nftContract).balanceOf(to, tokenId) == startingBalance
            ) {
                revert TransferFailed();
            } else {
                return true;
    ...
    function quantityOf(
        ...
        if (collectionType[nftContract]) {
            return IERC1155(nftContract).balanceOf(potentialOwner, tokenId);
        } else {
            if (IERC721(nftContract).ownerOf(tokenId) == potentialOwner) {
                return 1;
            } else {
                return 0;
            }
    ...
    function deleteCollection(address _nftContract) external onlyOwner {
        collectionStatus[_nftContract] = false;
    }
}
