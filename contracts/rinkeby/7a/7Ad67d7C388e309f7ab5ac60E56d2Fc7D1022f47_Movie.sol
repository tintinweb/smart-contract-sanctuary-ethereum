...
contract Movie is ERC721Optimized, Ownable, ReentrancyGuard {
    ...
    function SetTokenIdPrice(uint256 tokenId,uint256 newTokenPrice) public {
        require(tokenId >= 0, "tokenId must be greater than zero");
        require(newTokenPrice > 0, "tokenId price must be greater than zero");

        //检测所有权
        address owner = ERC721Optimized.ownerOf(tokenId);
        require(_msgSender() != owner, "tokenId that is not owner");
   ...
   mapping(address => uint256) minteds;
    function mint(uint256 numberOfTokens) public payable nonReentrant {
        ...
        //check ETH Balance
        require((mintPrice * numberOfTokens) <= msg.value, "Not enough ETH");
    ...
    //购买
    function buy(address seller,address buyer,uint256 tokenId, uint256 amount) public { 
       
        //check balance
        require(address(this).balance > amount, "Insufficient balance");

        //check price
        require(tokenIdsPrice[tokenId]==amount,"amount error");
