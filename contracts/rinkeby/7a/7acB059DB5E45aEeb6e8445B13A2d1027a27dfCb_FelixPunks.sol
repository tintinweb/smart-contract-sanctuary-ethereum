...
// File contracts/FelixPunkDNA.sol



pragma solidity ^0.8.0;

contract FelixPunkDNA {
    ...
    function getDNASection(uint256 _dna, uint8 _rightDiscard)
        internal
        pure
        returns (uint8)
    {
...
// File contracts/FelixPunks.sol


pragma solidity ^0.8.0;
contract FelixPunks is ERC721, ERC721Enumerable, FelixPunkDNA {
    ...
    constructor(uint256 _maxSupply) ERC721("FelixPunks", "FPS") {
        maxSupply = _maxSupply;
    }

    function mint() public {
        uint256 current = _idCounter.current();
        require(current < maxSupply, "No FelixPunks left :(");
        ...
        returns (string memory)
    {
        ...
        string memory jsonURI = Base64.encode(
            abi.encodePacked(
                '{ "name": "FelixPunks #',
                tokenId.toString(),
                '", "description": "FelixPunks are randomized Avataaars stored on chain to teach DApp development", "image": "',
                image,
                '"}'
            )
        );
