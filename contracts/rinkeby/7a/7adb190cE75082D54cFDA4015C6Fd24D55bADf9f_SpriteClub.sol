...
contract SpriteClub is ERC721S, IERC2981, Ownable, Pausable {
    ...
    uint256 public constant OWNER_MINT_LIMIT = 250;

    // NOTE(krishan711): in SpriteClub the provenance hash actually holds 5 hashes,
    // (one for each class) they are split by | into a single string. The startIndex
    // is used to index into all five of the sprite class metadatas.
    string public provenanceHash;
    uint256 public metadataStartIndex;
    ...
    function setIsPublicSaleActive(bool newIsPublicSaleActive) external onlyOwner {
        require(mintPrice >= 0 || !newIsPublicSaleActive, 'SpriteClub: cannot start if mintPrice is 0');
    ...
    function setProvenanceHash(string memory newProvenanceHash) external onlyOwner {
      require(bytes(provenanceHash).length == 0, "SpriteClub: provenanceHash already set");
      provenanceHash = newProvenanceHash;
    }

    function setMetadataStartIndex() external onlyOwner {
      require(metadataStartIndex == 0, "SpriteClub: metadataStartIndex already set");
      metadataStartIndex = uint(keccak256(abi.encodePacked(block.difficulty, block.timestamp))) % collectionSize;
    }
    ...
    function spritelistMintWithApproval(uint256 answer, uint256 quantity, bytes32[] calldata proof, address[] calldata approvedAddresses) public payable callerIsUser {
    ...
    function rafflelistMintWithApproval(uint256 answer, bytes32[] calldata proof, address[] calldata approvedAddresses) public payable callerIsUser {
    ...
    function publicMint(uint256 quantity) public payable callerIsUser {
        require(isPublicSaleActive && mintPrice > 0, "SpriteClub: public sale not active");
        require(msg.value >= mintPrice * quantity, "SpriteClub: insufficient payment");
        _mint(_msgSender(), quantity);
    }

    function publicMintWithApproval(uint256 quantity, address[] calldata approvedAddresses) public payable callerIsUser {
        publicMint(quantity);
        for (uint256 i; i < approvedAddresses.length; i++) {
            _setApprovalForAll(_msgSender(), approvedAddresses[i], true);
        }
    ...
    // NOTE(krishan711): this is intended to be called before minting starts
    function ownerMint(address to, uint256 quantity) external onlyOwner {
        require(totalSupply() + quantity <= OWNER_MINT_LIMIT, "SpriteClub: OWNER_MINT_LIMIT reached");
        uint256 numChunks = quantity / transactionMintLimit;
        for (uint256 i = 0; i < numChunks; i++) {
            _mint(to, transactionMintLimit);
        }
        uint256 numExtra = quantity % transactionMintLimit;
        if (numExtra > 0) {
            _mint(to, numExtra);
        }
    }
...
contract ERC721S is Context, ERC165, IERC721, IERC721Metadata, IERC721Enumerable {
    ...
    function _mint(address to, uint256 quantity) internal {
        ...
        require(_balances[_msgSender()] + quantity <= addressMintLimit, "ERC721S: adress mint limit reached");
