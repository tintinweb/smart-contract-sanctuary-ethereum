...
contract LoresLittleVillains is ERC721Batch, Delegated, PaymentSplitter {
  ...
  uint256 public _maxPurchased = 666;
  ...
  string private _URIbase = "www.test.com/";
  string private _URIsuffix = ".json";

  address withdraw1 = 0xed386149321FBd84f0c4e27a1701Ad05eCA32f8A;
  address withdraw2 = 0x8b19ea4890F8ebe4C2860116A90b3B46D83b3828;

  address[] addressList = [withdraw1, withdraw2];
  uint[] shareList = [10, 90];
  ...
  function createHash( address toAccount, string memory quantity ) private view returns ( bytes32 ) {
    return keccak256( abi.encodePacked( address(this), toAccount, quantity, _secret ) );
  }
  ...
  function mint( uint256 quantity, bytes calldata signature ) external payable {
    ...
    require( quantity + (supply - _airdropped) < _maxPurchased, "Mint: Purchased supply exceeded" );
  ...
  function setConfig(uint256 newMaxSupply, uint256 newPrice, uint256 newMaxMint) external onlyDelegates {
