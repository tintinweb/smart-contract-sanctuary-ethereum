...
contract Token is Ownable, ERC721  {
    ...
    constructor(string memory name_, string memory symbol_, string memory baseTokenURI_, uint256 maxSupply_) ERC721(name_, symbol_) {
    ...
    function mint(address recipient) public onlyOwner {
        if (_maxSupply > 0) {
            require(_tokenIds.current() + 1 <= _maxSupply, "ERC721: can't exceed max supply");
        }
        _tokenIds.increment();
