...
contract MemoticsGenesisPass is ERC721A, ERC721ABurnable, ERC721AQueryable, Ownable {
    ...
    constructor() ERC721A('MemoticsGenesisPass', 'MEMO') {
        unchecked {
            uint256 initialMintCount = 111 + 200;
            uint256 miniBatchSize = 16;
            uint256 i = initialMintCount % miniBatchSize;
            _mintERC2309(msg.sender, i);
            while (i != initialMintCount) {
                _mintERC2309(msg.sender, miniBatchSize);
                i += miniBatchSize;
            }
        }
    }
    ...
    function _incrementedAux(
        ...
        assembly {
            let index := sub(byte(state, hex"0001020304"), 1)
            let shift := shl(4, index)
    ...
    function _addToMintedAndCheckLimitAndPrice(
        ...
        unchecked {
            (uint64 packed, uint256 prev, uint256 result) = _incrementedAux(minter, saleState, quantity);
    ...
    function getAux(address minter, uint256 saleState_) external view returns (uint256) {
        (, uint256 prev, ) = _incrementedAux(minter, saleState_, 0);
        return prev;
    }
    ...
    function _validateSignature(bytes calldata signature) private view {
        ...
        require(recovered == SIGNER || recovered == owner(), 'Invalid signature.');
    }
