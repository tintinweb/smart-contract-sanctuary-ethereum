...
contract Test is Ownable, ERC721A {
  ...
  // Quantity per Mint
  uint256 public maxPublicMintQty = 10;
  uint256 public maxWhitelistMintQty = 2;
  ...
  uint256 public WhitelistPrice = 0.00969 ether;
  ...
  function WhitelistMint(bytes32[] memory proof, uint256 quantity)
    external
    payable
  {
    uint256 supply = totalSupply();
    require(
      WhitelistSaleStartTime != 0 && block.timestamp >= WhitelistSaleStartTime,
      'Sale has not started yet'
    );
    require(
      _numberMinted(msg.sender) + quantity <= maxWhitelistAmount,
      'Already minted'
    );
    require(
    ...
    require(quantity <= maxWhitelistMintQty, 'Reached max Whitelist amount');
    require(supply + quantity <= maxSupply, 'Reached max supply');
  ...
  function setMaxWhitelistMintQty(uint256 _maxWhitelistMintQty)
    external
    onlyOwner
  {
    maxWhitelistMintQty = _maxWhitelistMintQty;
  }
  ...
  function setMaxWhitelistAmount(uint256 _maxWhitelistAmount) external onlyOwner {
  ...
  function setWhitelistSaleStartTime(uint256 timestamp) external onlyOwner {
