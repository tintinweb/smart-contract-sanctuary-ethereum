...
// File: .deps/npm/@openzeppelin/contracts/utils/cryptography/MerkleProof.sol
...
contract MerkleProof is Ownable {
    bytes32 public root = 0xc895f9bb47eb714f497fdf77d96cdc21edefc3f4f2e0fc36db961cbce922ec1f;
...
*/
    function verify(bytes32[] memory _proofs, bytes32 leaf) public view returns (bool) {
        uint256 i;
        uint256 length = _proofs.length;
        bytes32 hashVal = leaf;
        for (i = 0 ; i < length; ++i) {
            if (_proofs[i] == keccak256("0")) {
                continue;
            } else if (hashVal > _proofs[i]) {
                hashVal = hashFunc(_proofs[i], hashVal);
            } else {
                hashVal = hashFunc(hashVal, _proofs[i]);
            }
        }
        return hashVal == root;
    }
    ...
    function getTime(bytes32[] memory _proof) external view returns (uint256) {
        require(verify(_proof, keccak256(abi.encodePacked(msg.sender))) == true, "Not on whitelist");
