...
import "./interfaces/IShippingManager.sol";
...
contract KaijuMart is AccessControl, ReentrancyGuard {
    ...
    function _manager(LotType lotType) internal view returns (address) {
        if (lotType == LotType.RAFFLE) {
            return address(managerContracts.raffle);
        } else if (lotType == LotType.DOORBUSTER) {
            return address(managerContracts.doorbuster);
        } else if (lotType == LotType.AUCTION) {
            return address(managerContracts.auction);
        } else {
            return address(0);
        }
    }
     ...
     * @param lot TODO
     * @param lotType TODO
     * @param rwastePrice TODO
     * @param scalesPrice TODO
     */
    function _create(
        uint256 id,
    ...
    function _charge(
        ...
        nonReentrant
    {
        uint256 cost = amount * (token == PaymentToken.SCALES ? lots[lotId].scalesPrice : lots[lotId].rwastePrice);

        if (cost != 0) {
            if (token == PaymentToken.SCALES) kaijuContracts.SCALES.spend(account, cost);
            else kaijuContracts.RWASTE.burn(account, cost);
        }
    }
    ...
    function purchaseWithData(
        ...
        onlyKingz
    {
        uint32 remaining = _purchase(doorbusterId, amount, token, _msgSender());
        _settle(doorbusterId, data, remaining == 0);
    }
    ...
    function _purchase(
        ...
        returns (uint32)
    {
        uint32 remaining = managerContracts.doorbuster.purchase(doorbusterId, amount);
    ...
    {
        if (value > 0) {
            managerContracts.shipping.charge(_msgSender(), token, value);
            emit Ship(lotId, _msgSender(), token, value);
        }

        if (msg.value > 0) {
            emit Ship(lotId, _msgSender(), address(0), msg.value);
        }
    }
...
interface IDoorbusterManager {
    ...
    function purchase(uint256 id, uint32 amount) external returns (uint32);
}
...
interface IShippingManager {
    function charge(address from, address token, uint256 value) external returns (bool);
}
