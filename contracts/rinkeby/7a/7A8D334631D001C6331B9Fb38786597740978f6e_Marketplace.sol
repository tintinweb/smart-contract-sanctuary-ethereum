...
abstract contract TransferManager is OwnableUpgradeable, ITransferManager {
        ...
        returns (LibPart.Part[] memory)
    {
        if (
            ...
            (address erc1155Address, LibERC1155LazyMint.Mint1155Data memory data) = abi.decode(
                matchNft.data,
                (address, LibERC1155LazyMint.Mint1155Data)
            );
            require(IERCExtension(erc1155Address).owner() == data.royalties[0].account,"Failed, wrong receiver royalty");
            return data.royalties;
        } else if (
            matchNft.assetClass == LibAsset.ERC721_ASSET_CLASS ||
            matchNft.assetClass == LibAsset.ERC721_LAZY_ASSET_CLASS
        ) {
            (address erc721Address, LibERC721LazyMint.Mint721Data memory data) = abi.decode(
                matchNft.data,
                (address, LibERC721LazyMint.Mint721Data)
            );
            require(IERCExtension(erc721Address).owner() == data.royalties[0].account,"Failed, wrong receiver royalty");
            return data.royalties;
        }
