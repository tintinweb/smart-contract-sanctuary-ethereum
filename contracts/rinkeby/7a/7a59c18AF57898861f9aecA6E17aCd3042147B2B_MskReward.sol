...
contract MskReward is Context, Ownable {
    ...
    uint32 private m_StartTime = 0;
    uint32 private m_EndTime = 0;

    uint256 private m_RewardPerDay = 100 ether;
    ...
    address private m_RewardWallet = 0xA69e1e8f7afd56126452AcDbCe27374570a52D48;

    bool private m_ClaimPaused = false;

    mapping(address => ClaimInfo) private m_ClaimInfoList;

    constructor() {
        m_StartTime = uint32(block.timestamp);
        m_EndTime = m_StartTime + (5 * 365 * 1 days);
    }
        ...
        returns (uint256)
    {
        ...
        uint256 newAmount = m_RewardPerDay * (endTime - lastRewardTime);
    ...
    function claimMsk(uint256 _amount) external {
        ...
        msk.transferFrom(m_RewardWallet, _msgSender(), _amount);
