...
contract NFTContract is Ownable, ERC721A, ReentrancyGuard {
    ...
    struct AuctionConfig {
        uint64 startPrice;
        uint64 endPrice;
        uint64 priceCurveLength;
        uint64 dropPriceInterval;
    }
    ...
    constructor(
        ...
        payoutAddress = 0x11a6e875C5d7d61c2FB0c641FE0333B9B8015A0b;
    ...
    function mint(uint256 quantity) external payable callerIsUser {
        ...
        require(
            numberMinted(msg.sender) + quantity <= auctionBatchSize,
    ...
    function proceedSaleStageIfNeed() private {
        if (saleConfigs.length > currentSaleIndex + 1) {
            SaleConfig memory config = saleConfigs[currentSaleIndex];
    ...
    function getSaleConfig(uint256 _saleIndex)
        external
        view
        returns (SaleConfig memory)
    {
        SaleConfig memory config = saleConfigs[_saleIndex];
        return config;
    }
    ...
    function getAuctionPrice() public view returns (uint256) {
        SaleConfig memory saleConfig = saleConfigs[currentSaleIndex];
        uint256 auctionStartTime = uint256(saleConfig.startTime);
    ...
    function isTicketAvailable(string memory ticket, bytes memory signature)
        ...
        returns (bool)
    {
        return isAuthorized(msg.sender, ticket, signature, signerAddress);
    }
