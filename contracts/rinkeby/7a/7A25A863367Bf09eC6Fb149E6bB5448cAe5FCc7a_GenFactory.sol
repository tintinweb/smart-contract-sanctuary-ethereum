...
contract GenTickets is ERC1155, ERC1155Receiver {
    ...
    // common entities for all types of tickets
    ...
    uint256 public balanceNeeded = 0;//make it private later
    ...
    // might remove later
    mapping(address => mapping(uint256 => RedeemInfo)) public redeemInfo;
    ...
    // In case, wants all tokens back after creating tickets
    ...
    function redeemTicket(address _to, uint256 _id, uint256 _amount) public {
        ...
        require(_amount > 0, "GenMarketPlace: Invalid entry");
        require(IERC1155(address(this)).balanceOf(msg.sender, _id) >= _amount, "GenMarketPlace: Not enough tickets, the amount exceeds the balance");

        uint tier = _id.mod(numTicketTypes);//0
        GenTicket memory ticketType = genTickets[tier];// black

        RedeemInfo memory redeemInfos = redeemInfo[_to][_id];

        require(getBlockTimestamp() > TGE, "GenTickets: Project TGE not occured");

        uint256 redeemablePercentage;//0
        uint256 redeemedPercentage;//0

        uint256 redeemableCounter;//0
        uint256 redeemedCounter;//0

        uint tranche = _id.div(numTicketTypes);//0/4=0
        require(tranche < totalTranches, "GenTickets: Ticket has redeemed all tokens");//true

        for (uint i=0; i< totalTranches ; i++){
            
            if (i < tranche ){//0<10false
                redeemedPercentage += trancheWeightage[i];//50
                redeemedCounter += 1;//1
            }
        ...
        // uint totalTokensAvailablewithUser = (ticketType.ticketSize).mul(_amount);//10*2=20
        uint256 tokens = (ticketType.ticketSize).mul(_amount).mul(redeemablePercentage - redeemedPercentage).div(100);//20*50/100=10

        redeemInfos.redeemed += tokens;
        redeemInfo[_to][_id.add(numTicketTypes.mul(redeemableCounter - redeemedCounter))] = RedeemInfo(redeemInfos.redeemed);

        safeTransferFrom(address(msg.sender), address(this), _id, _amount, VALIDATOR);//4

        // Transfer underlying tokens with corresponding ticket size
        IERC20(underlyingToken).transfer(_to, tokens);//10//10=20
        bytes memory data;
...
contract GenFactory is IGenFactory {
    ...
    function createGenTicket(
        address _underlyingToken, 
        uint256[] memory _numTickets,//length eg: 4
        uint256[] memory _ticketSizes,//length 4
        uint _totalTranches,//only one input for each type of tickets //eg 3
        // uint[] memory _cliffTranches,
        uint[] memory _trancheWeightage,//[50,20,30]//same for each tkt type
        uint[] memory _trancheLength,//[0,30,40]//same for each tkt type
        string memory _uri
    ) external override returns (address) {
        require(_numTickets.length < 10, 'GenFactory: MAX NUMBER OF TICKETS');
        require(_numTickets.length == _ticketSizes.length && _totalTranches == _trancheLength.length && 
        _trancheWeightage.length == _trancheLength.length, 'GenFactory: ARRAY SIZE MISMATCH');
        ...
        GenTickets gt = new GenTickets(_underlyingToken, _numTickets, _ticketSizes, _totalTranches, _trancheWeightage, _trancheLength, _uri, this, msg.sender);
        // Populate mapping
