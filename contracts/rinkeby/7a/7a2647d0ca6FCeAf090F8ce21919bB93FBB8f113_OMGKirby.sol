...
contract OMGKirby is ERC721, Ownable {
  ...
  // Counter private _whiteListIds = 3071;
  ...
  // Addresses
  address dao;
  address constant withdrawRecipient = 0x1aBdDC3293Ec0b0909605A1Bc8939cF7E98a4E7a;
  address public genesisAddr;

  constructor(address _genesisAddr, uint _startTime, uint _whitelistStartTime, address _dao) ERC721("OMGKirby", "OMG") {
    genesisAddr = _genesisAddr;
    _totalSupply.increment();
    claimStartTime = _startTime;
    whitelistStartTime = _whitelistStartTime;
    claimTimeLock = claimStartTime + 2 hours;
    whitelistTimeLock = whitelistStartTime + 2 hours;
    dao = _dao;
  }
  ...
  function whitelistMint(bytes32[] calldata _merkleProof) public {
    ...
    require(_nonClaimIds.current() <= maxSupply, "Whitelist mint has sold out");
    require(block.timestamp >= whitelistStartTime, "WL has not started yet");
    require(block.timestamp < whitelistTimeLock, "WL has ended");
  ...
  function publicMint() external {
    ...
    require(block.timestamp > whitelistTimeLock, "Public mint has not started yet");
    addressClaimed[_msgSender()] += 1;
  ...
  function teamClaim(uint256 quantity) external payable onlyOwner{
  ...
  function daoClaim(uint256[] memory _ids) external payable {
  ...
  function isClaimed(uint256 _tokenId) public view returns (bool){
    return ownerOf(_tokenId) != address(0) ? true : false;
  }
  ...
  // Functions for testing
  function setClaimLock(uint _newTime) public onlyOwner {
    claimStartTime = _newTime;
    claimTimeLock = claimStartTime + 2 hours;
  }

  // Functions for testing
  function setWhitelistLock(uint _newTime) public onlyOwner {
    whitelistStartTime = _newTime;
    whitelistTimeLock = whitelistStartTime + 2 hours;
  }

  function setDAOAddr(address _newAddr) public onlyOwner {
    dao = _newAddr;
  }
}
