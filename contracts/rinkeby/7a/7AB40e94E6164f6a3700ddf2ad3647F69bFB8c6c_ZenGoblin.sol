...
contract ZenGoblin is ERC721A, Ownable {
    ...
    uint256 public MAX_PER_ADDRESS_FREE = 3;
    ...
    function meditate(uint256 _amount) external payable {
        ...
        require(
            _amount > 0 && _amount <= MAX_PER_ADDRESS,
        ...
        require(
            _numberMinted(msg.sender) + _amount <= MAX_PER_ADDRESS,
            "Max per address exceeded!"
        );
        ...
        if(totalSupply() >= MAX_MEDITATE_FREE) {
            ...
            //free available
            if(MAX_PER_ADDRESS_FREE > _numberMinted(msg.sender)){
                //still available for free mint
                uint count = MAX_PER_ADDRESS_FREE - _numberMinted(msg.sender);
                require(msg.value >= PRICE * (_amount - count), "Insufficient funds!");
    ...
    function setFreeCapLimit(uint256 limit) public onlyOwner {
