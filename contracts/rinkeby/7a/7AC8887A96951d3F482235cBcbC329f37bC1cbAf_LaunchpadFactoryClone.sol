...
contract LaunchpadFactoryClone is ReentrancyGuard, Ownable, Pausable {
    ...
    constructor(address _router, uint256 _createFee) {
        router = _router;
...
contract LaunchpadPresale is Initializable, ReentrancyGuard, Ownable, Pausable {
     ...
     */
    function initialize(address _token, address _router, address _stakingContract, Parameters memory _par, 
    WhitelistParameters memory _wpar, VestingParameters memory _vpar, uint256 _raisedFee, uint256 _influencerFee) 
    public initializer {
    ...
    function finalize() external onlyOwner {
        ...
        if (totalBought > par.softcap) {
            ...
            // Adding Liquidity
            uint256 amountToLiquidity = (totalBought * par.liquidityPercent) / 10000;
