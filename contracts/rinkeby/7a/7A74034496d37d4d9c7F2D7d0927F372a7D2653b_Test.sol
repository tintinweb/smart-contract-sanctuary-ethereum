...
contract Test is ERC721, IERC2981, Ownable {
    ...
    uint256 public externalMaxMintAmount = 3;
    ...
    uint256[15] public mintDetails = [2, 5, 1, 5, 7, 2, 7, 99, 3, 5, 99, 1, 3, 99, 1];
    ...
    mapping(address => mapping(uint => bool)) public tokenMinted;
    ...
    modifier externalActive() {
        require(isExternalActive, "External mint not active");
        _;
    }
    ...
    constructor(string memory notRevealedURI_) ERC721("Test", "TTT") {
    ...
    {
        uint256 freeMints = 0;
        if (_mintAmount > mintDetails[0] && _mintAmount < mintDetails[1]) {
            freeMints = mintDetails[2];
        } else if (
            _mintAmount >= mintDetails[3] && _mintAmount < mintDetails[4]
        ) {
            freeMints = mintDetails[5];
        } else if (
            _mintAmount >= mintDetails[6] && _mintAmount < mintDetails[7]
        ) {
            freeMints = mintDetails[8];
        }
    ...
    {
        uint256 freeMints = 0;
        if (_mintAmount >= mintDetails[9] && _mintAmount < mintDetails[10]) {
            freeMints = mintDetails[11];
        }
    ...
    function externalMint(uint256 _mintAmount, uint _tokenId)
        external
        payable
        externalActive
        callerIsUser
    {
        require(msg.sender == IERC721(externalAddress).ownerOf(_tokenId), "You don't own the NFT");
        require(tokenMinted[externalAddress][_tokenId] == false, "Already minted");

        uint256 freeMints = 0;
        if (_mintAmount >= mintDetails[12] && _mintAmount < mintDetails[13]) {
            freeMints = mintDetails[14];
        }
        ...
        externalAddressMintCount[msg.sender] += _mintAmount;
        tokenMinted[externalAddress][_tokenId] = true;
        _mintLoop(msg.sender, _mintAmount + freeMints);
    }
    ...
    function setMintDetails(uint256[15] memory _mintDetails) public onlyOwner {
        mintDetails = _mintDetails;
    }

    function setExternalAddress(address _externalAddress) public onlyOwner {
