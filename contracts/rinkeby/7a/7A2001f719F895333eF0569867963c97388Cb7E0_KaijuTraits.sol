...
error KaijuTraits_InvalidTraitId();
error KaijuTraits_InvalidToken();
error KaijuTraits_SenderNotTokenOwner();
error KaijuTraits_SupplyLocked();

contract KaijuTraits is ERC1155, Ownable, AccessControl, KaijuMartRedeemable {
    ...
    {
        if (traitId == 0) revert KaijuTraits_InvalidTraitId();
    ...
    function mint(
        ...
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        if (supplyLocked[id]) revert KaijuTraits_SupplyLocked();
        ...
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        for (uint256 i; i < ids.length;) {
            if (supplyLocked[ids[i]]) revert KaijuTraits_SupplyLocked();
            unchecked { ++i; }
        }
