...
animalToOwner[_tokenId] = _to;
  ...
  function createAnimal() payable public {
      ...
      uint range__of_star =uint(dna)%100;
      if(range__of_star<2){star=1;}
      else if(range__of_star<3){star=2;}
      else if(range__of_star<4){star=3;}
      else if(range__of_star<5){star=4;}
      else {star=5;}
      roletype =uint(dna)%4;
