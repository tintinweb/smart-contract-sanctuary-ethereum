...
contract ALIEN is ERC721A, Ownable, ReentrancyGuard {
    ...
    uint256 public founderPrice = 0 ether;
    uint256 public MAX_SUPPLY = 10000;
    uint256 public resMintMax = 2000;
    uint256 public foundersMintMax = 600;
    uint256 public maxFightersPerWallet = 3;
    uint256 public maxFightersPerTx = 3;
    uint256 public maxFoundersPerWallet = 3;
    
    bool public reservationMintPaused = false;
    bool public foundersMintPaused = false;
    bool public publicMintPaused = false;
    bool public revealed = false;
    bytes32 rezlistMerkleRoot;
    bytes32 founderslistMerkleRoot;
    mapping(address => uint256) private rezlistMintedAmount;
    mapping(address => uint256) private founderslistMintedAmount;
    address payable public payments;

    //Constructor
    constructor(address _payments) ERC721A(" AlienX ", "AX", 11) {
    ...
    //Reservation Mint
    function resMint(address _address, bytes32[] memory proof) external payable onlySender nonReentrant {
        ...
        require(resMintMax >= amount, "Reservation mint is sold out");

        rezlistMintedAmount[_address] += amount;
        _mintWithoutValidation(_address, amount);
    }

    function foundersMint(address _address, bytes32[] memory proof) public payable onlySender nonReentrant {
        require(!foundersMintPaused, "Founders mint is paused");
        uint256 tokenCount = IERC721(0x6A7cCcf637a65A239101D37D1a91F38EE5f93bE6).balanceOf(msg.sender);
        require(tokenCount > 1, "Insufficient Tokens");
        uint256 mintAmount = tokenCount * 6;
        require((totalSupply() + mintAmount) <= MAX_SUPPLY, "Sold out!");

        uint256 amount = _founderMintAmount(msg.value);
         require(
               isAddressOnFounderslist(proof, _address),
           "You are not eligible for a Founders mint"
           );
           require(
             founderslistMintedAmount[_address] + amount <= maxFoundersPerWallet,
          "Minting amount exceeds allowance per wallet"
          );
 
        for (uint256 i = 0; i < mintAmount; i++) {
            _mintWithoutValidation(msg.sender, MAX_SUPPLY);
            MAX_SUPPLY++;}    
    ...
    //Public Mint 
    function publicMint(address _address) external payable onlySender nonReentrant {
    ...
    function _getMintAmount(uint256 value) internal view returns (uint256) {
        ...
        require(
            (totalSupply() + amount) <= MAX_SUPPLY - foundersMintMax,
            "Sold out!"
        );
        return amount;
    }

     function _founderMintAmount(uint256 value) internal view returns (uint256) {
        uint256 remainder = value % founderPrice;
        require(remainder == 0, "Send a divisible amount of eth");
        uint256 amount = value / founderPrice;
        require(amount > 0, "Amount to mint is 0");
        require(
            (totalSupply() + amount) <= MAX_SUPPLY - foundersMintMax,
            "Sold out!"
    ...
    function isAddressOnFounderslist(bytes32[] memory proof, address _address)
        public
        view
        returns (bool)
    {
        return isAddressInMerkleRoot(founderslistMerkleRoot, proof, _address);
    }
    ...
    returns (string memory)
  {
    ...
    function setFoundersMintPaused(bool _foundersMintPaused) external onlyOwner {
        foundersMintPaused = _foundersMintPaused;
    }
    ...
    function setFoundersMintMerkleRoot(bytes32 _founderslistMerkleRoot)
        external
        onlyOwner
    {
        founderslistMerkleRoot = _founderslistMerkleRoot;
    }
