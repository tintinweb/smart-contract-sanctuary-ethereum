    ...
    ReentrancyGuard
{
    ...
    uint256 public constant MAX_RESERVE = 199;
    ...
    address private _creators=0xF580fc0f5aE3032171c781FBBBE73f54Fe411C4b;
    ...
    modifier allocTokens(uint32 numToMint) {
        require(
            totalSupply() + numToMint + MAX_RESERVE - RESERVE_COUNT <=
                MAX_NFT,
    ...
    modifier maxOwned(uint32 numToMint) {
        require(
            numberMinted(msg.sender) + numToMint <= MAX_MINT,
            "Max presale mint amount per wallet exceeded."
        );
        _;
    }
    ...
    constructor(
        string memory _mURI
    ) ERC721A("MiniMich", "MM", MAX_MINT, MAX_NFT) {
 ...
 */
    ...
    function reserve(address[] calldata receivers, uint32 mintNum) external onlyOwner {
        require(
 ...
 //   function setMAX_MINT(uint256 maxMint) external onlyOwner returns (uint256 ) {
 //       MAX_MINT = maxMint;
 //       return MAX_MINT;
 //   }
