...
contract MetaSamurai is ERC721A, Ownable, ERC2981ContractWideRoyalties {
    ...
    string private hiddenUri =
        "ipfs://QmVkjRcQxeo1qoKE4qgp8W1Gj9euwbgu1GJB8LfAfjxqEL";
        ...
        returns (string memory)
    {
        ...
        if (isRevealed == false) {
            ...
            string memory currentHash = uriHash;
            string memory currentSuffix = uriSuffix;
            return
                bytes(currentUriPrefix).length > 0 &&
                    bytes(currentHash).length > 0 &&
                    bytes(currentSuffix).length > 0
                    ? string(
                        abi.encodePacked(
                            currentUriPrefix,
                            currentHash,
    ...
    function setUriHash(string memory _newHash) public whenNotFrozen onlyOwner {
        uriHash = _newHash;
    }
    ...
    function setHiddenUri(string memory _newUri)
        public
        onlyOwner
        whenNotFrozen
    {
