...
contract PoolManager is OwnableUpgradeable, IPoolManager {
  ...
  uint256 public insurancePrice; // in usd ~ 30 usd = 3000;
  ...
  function setPoolFactory(address _factoryAddress) public onlyOwner{
    require(_factoryAddress != address(0), "Zero Address found!!");
 ...
 function setReverseCheck(bool _reverseCheck) public onlyOwner{
  ...
  function setUSDPAIRDecimals(uint256 _decimals) public onlyOwner{
      require(_decimals > 0, "Zero Address found!!");
  ...
  function bnbLiquidity(address payable _reciever, uint256 _amount) public onlyOwner {
    ...
    function getUSDRate() public override view returns (uint256,uint32){
        ...
        address reserve0 = IUniswapV2Pair(USDPAIR).token0();
        address reserve1 = IUniswapV2Pair(USDPAIR).token1();
        
        uint256 balance0 = IERC20Upgradeable(reserve0).balanceOf(USDPAIR);
        uint256 balance1 = IERC20Upgradeable(reserve1).balanceOf(USDPAIR);

        if(reverseCheck){
            uint256 rate = balance1.div(balance0).mul(10**USDPAIRDECIMALS).mul(10**18);
            return (rate , uint32(block.timestamp));
        } 
        else{
            uint256 rate = balance0.div(balance1).mul(10**USDPAIRDECIMALS).mul(10**18);
            return (rate , uint32(block.timestamp));
        }
    }
}