...
contract Epic_GalleryMarket is Ownable, ERC721Holder {
    using SafeMath for uint256;
	...
	function createCollection(bool bPublic) public returns(address collection) {
        ...
        IEpicGalleryNFT(collection).initialize(msg.sender, adminAddress, bPublic);
    ...
    function buy(uint256 _id) external payable {
		...
		if (pairs[_id].tokenAdr == address(0x0)) { //MTV
			...
			if (pairs[_id].creatorFee > 0 ){
				payable(pairs[_id].creator).transfer(totalAmount.mul(pairs[_id].creatorFee).div(PERCENTS_DIVIDER));
			}
			...
			if (pairs[_id].creatorFee > 0 ){
				governanceToken.transferFrom(msg.sender, pairs[_id].creator, totalAmount.mul(feeAdmin).div(PERCENTS_DIVIDER));	
			}		
