...
contract SoundMintEdition is ERC721ARoyalty, Ownable, ReentrancyGuard {
    ...
    onlyEOA
  {
    ...
    require(
      IERC20(apeAddress).balanceOf(msg.sender) >= salePriceAPE * qty,
      "PublicSale: Insufficient APE"
    );

    require(
      _safeTransferAPE(msg.sender, address(this), salePriceAPE * qty),
      "APE Token: Transfer Error."
    );
  ...
  // set RoyaltyFee
  function setTokenRoyaltyFee(uint256 value) external {
    require(value <= 10000, "ERC2981Royalties: Too high");
    royaltyFee = value;
  }

  function calcRoyaltyFee(uint256 tokenId, uint256 saleValue)
    public
    view
    returns (uint256)
  {
