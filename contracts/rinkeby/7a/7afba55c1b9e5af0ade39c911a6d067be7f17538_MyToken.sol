...
contract MyToken is Initializable, ERC1155Upgradeable  {
    string constant public name = "MYTOKEN";
    string constant public symbol = "MYTOKEN";
    ...
    mapping(uint256 => CompanyAddresses) public _walletApproved;
    ...
    modifier onlyContractOwner {
     ...
     // constructor of the class for KOVAN
        function initialize(uint256 _key) public virtual initializer {
            //address _oracle, string memory _jobId, uint256 _fee
        contractOwner = msg.sender;
        fe_key=_key;
        __ERC1155_init("MYTOKEN");
    ...
    //walletApproved check can be removed but for keeping it for testing internally.
     function createToken (address account, uint256 tokenID, uint256 tokencount , string memory uri , uint256 _key, bytes memory data) public { 
         
         require(_key == fe_key, "unauthorized user");
         require(msg.sender == contractOwner,"Only authorized user can mint");
             //require(msg.sender == contractOwner || msg.sender == _walletApproved[tokenID].address1 || msg.sender == _walletApproved[tokenID].address2
             //|| msg.sender == _walletApproved[tokenID].address3 || msg.sender == _walletApproved[tokenID].address4,"Only contract owner and approved wallet can burn");

             //_mint (msg.sender, companyID, tokencount, uri,data);             
    ...
    /*add signature message as another parameter to restrict only authorized user can execute this method*/
     ...
     function recordOnBlockchain(address acct, string memory email_id, string memory data) public  {
    ...
    function setFee(uint256 _fee) public onlyContractOwner {
        fee = _fee;
    }
    function setOracle(address _oracle) public onlyContractOwner {
        oracle = _oracle;
    }
     ...
     * Later when we call from more than one source, see the whole URL can be passed this function from caller
     * so that single function can be used to fetch the rate from multiple sources as needed
     */
    ...
    function addWallet(uint256 _tokenID, address[] memory _wallets) public onlyContractOwner
    {
        require(_wallets.length != 0,"Please provide address");
        require(_wallets.length <= 4,"Max 4 address allowed");
        
        if(_wallets.length == 1){
           _walletApproved[_tokenID].address1 = _wallets[0]; 
        }
        if(_wallets.length == 2){
           _walletApproved[_tokenID].address1 = _wallets[0]; 
           _walletApproved[_tokenID].address2 = _wallets[1];
        }
        if(_wallets.length == 3){
           _walletApproved[_tokenID].address1 = _wallets[0]; 
           _walletApproved[_tokenID].address2 = _wallets[1];
           _walletApproved[_tokenID].address3 = _wallets[2];
        }
        if(_wallets.length == 4){
           _walletApproved[_tokenID].address1 = _wallets[0]; 
           _walletApproved[_tokenID].address2 = _wallets[1];
           _walletApproved[_tokenID].address3 = _wallets[2];
           _walletApproved[_tokenID].address4 = _wallets[3];
        }
    }
    
    function delete_wallet(uint256 _id,uint256 index) public onlyContractOwner{
        if(index == 0){
            delete _walletApproved[_id].address1;
        }
        if(index == 1){
            delete _walletApproved[_id].address2;
        }
        if(index == 2){
            delete _walletApproved[_id].address3;
        }
        if(index == 3){
            delete _walletApproved[_id].address4;
        }
