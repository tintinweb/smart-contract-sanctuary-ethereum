...
abstract contract SuperCharacterInterface {
...
contract Mintvial is ERC1155, Ownable, ERC1155Burnable {
    ...
    address supercreaturesContractAddress;
    ...
    constructor() ERC1155("ipfs://QmQqMF7izNAaU9CY3qV9ZGs4Aksv6ywjx8261khgzQbReW") {
        ...
        authorizedContract[0xAd57c7C681a05F077138Af3359BDfc0f0934D5F0] = 721; // WeirdCreatures - 721
    ...
    function setSuperContract(address contractAddress) public onlyOwner {
        supercreaturesContractAddress = contractAddress;
    }
    ...
    function mint(address[] calldata contractIds, uint256[] calldata tokenIds, uint256 amountToMint) public payable returns(uint256) {
        ...
        if(publicSales == true) {
            ...
            for(uint256 i = 0; i < contractIds.length; i++) {
                ...
                if(contractIds[i] == 0xAd57c7C681a05F077138Af3359BDfc0f0934D5F0) amount += 1; 
                else amount += 3;
            }
        }
    ...
    function migrateToken(uint256 id) public returns(uint256) {
        ...
        SuperCharacterInterface supercreaturesContract = SuperCharacterInterface(supercreaturesContractAddress);
        uint256 mintedId = supercreaturesContract.mintTransfer(msg.sender); // Mint the ERC-721 token
    ...
    function forceMigrateToken(uint256 id) public onlyOwner {
        require(balanceOf(msg.sender, id) > 0, "Doesn't own the token"); // Kept so no one can't force someone else to open a Super Creatures
        burn(msg.sender, id, 1); // Burn one the ERC-1155 token
        SuperCharacterInterface supercreaturesContract = SuperCharacterInterface(supercreaturesContractAddress);
        supercreaturesContract.mintTransfer(msg.sender); // Mint the ERC-721 token
    }
