...
contract BNFNFT is ERC721Enumerable, Ownable, ReentrancyGuard {
    ...
    address private VAULT_ADDRESS = 0x4cB6f369e6D4f56d7812B3EAd83d5217Dd54A3f8;
    ...
    /// @dev Presale mint
    function presaleMint(uint _numTokens) public payable nonReentrant {
        require(saleState == 1, "Presale not open."); 
        require(_numTokens > 0, "Minimum number to mint is 1.");
        
        require(presaleReservations[msg.sender] >= _numTokens, "Whitelisted buying exceeded");
    ...
    // Remove all the pre-sale reservations for the addresses.
    function removePreSaleReservations(address[] memory _addresses) public onlyOwner {
        uint numRemoved = 0;
        for (uint i = 0; i < _addresses.length; i++) {
