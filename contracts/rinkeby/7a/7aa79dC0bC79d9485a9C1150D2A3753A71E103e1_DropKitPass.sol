    ...
    ERC721EnumerableUpgradeable
{
    ...
    mapping(uint256 => mapping(address => uint256)) private _mintCount;
    // supply per stage
    ...
    mapping(uint256 => uint256) private _maxAmount;
    mapping(uint256 => uint256) private _maxPerMint;
    mapping(uint256 => uint256) private _maxPerWallet;

    // States per stage
    mapping(uint256 => bool) private _saleActive;

    modifier onlyPurchasable(uint256 stageId, uint64 quantity) {
    ...
    function purchasePass(
        ...
        _purchasePass(stageId, feeRate, _msgSender(), quantity);
    }
        ...
        returns (Pass memory data)
    {
        ...
        return _passes[stageId][feeRate];
    }
    ...
    function stopSale(uint256 stageId) external onlyOwner {
    ...
    function presaleActive(uint256 stageId) external view returns (bool) {
        return _saleActive[stageId] && _merkleRoot[stageId] != "";
    }
