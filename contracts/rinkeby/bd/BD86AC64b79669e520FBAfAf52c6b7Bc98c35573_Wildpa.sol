...
contract Wildpa is ERC721A, Ownable, ReentrancyGuard {
    ...
    uint256 public constant PRICE_COMMON_TOKEN = 0.00001 ether;
    uint256 public constant PRICE_RARE_TOKEN = 0.00002 ether;
    uint256 public constant PRICE_LEGENDARY_TOKEN = 0.00003 ether;    
    ...
    function airdrop(uint256 quantity, address address_to, string memory Type ) 
        external 
        nonReentrant
        onlyOwner
...
contract ERC721A is IERC721A {
    ...
    function totalSupply() public view override returns (uint256) {
        ...
        unchecked {
            return totalMintedCommon() + totalMintedRare() + totalMintedLegendary() - _burnCounter;
        }
    }
    ...
    function _totalMinted() internal view returns (uint256) {
        ...
        unchecked {
            return totalMintedCommon() + totalMintedRare() + totalMintedLegendary() ;
        }
    }
     ...
     */
    function _isBelowIndex(uint256 index) private view returns (bool) {
        bool isBelowIndexCommon = index < _currentIndexCommon && index >= _startTokenIdCommon();
        bool isBelowIndexRare = index < _currentIndexRare && index >= _startTokenIdRare();
        bool isBelowIndexLegendary = index < _currentIndexLegendary && index >= _startTokenIdLegendary();
        return isBelowIndexCommon || isBelowIndexRare || isBelowIndexLegendary;
    }
    ...
    function _exists(uint256 tokenId) internal view returns (bool) {
        return
            _startTokenIdCommon() <= tokenId &&
            _isBelowIndex(tokenId) && // If within bounds,
     ...
     */
    function _mintERC2309(address to, uint256 quantity, string memory Type) internal {
        uint256 startTokenId;
        if (keccak256(abi.encodePacked(Type)) == keccak256(abi.encodePacked("common"))){
            startTokenId = _currentIndexCommon;
        }else if (keccak256(abi.encodePacked(Type)) == keccak256(abi.encodePacked("rare"))){
        ...
        unchecked {
            ...
            if (keccak256(abi.encodePacked(Type)) == keccak256(abi.encodePacked("common"))){
                _currentIndexCommon = startTokenId + quantity;
            }else if (keccak256(abi.encodePacked(Type)) == keccak256(abi.encodePacked("rare"))){
                _currentIndexRare = startTokenId + quantity;
            }else if (keccak256(abi.encodePacked(Type)) == keccak256(abi.encodePacked("legendary"))){
                _currentIndexLegendary = startTokenId + quantity;
            } 
        }
        _afterTokenTransfers(address(0), to, startTokenId, quantity);
    }
