...
contract KlubOlahraga is ERC721A, ReentrancyGuard, Ownable {
    string private baseURI;

    uint256 private presalePrice = 0.1 ether;
    uint256 private publicPrice = 0.2 ether;
    uint256 private reserved = 1000;
    uint256 private saleStatus;
    uint256 private maxPerTx = 1;
    ...
    function setSaleStatus() external onlyOwner {
        saleStatus = (saleStatus + 1) % 3;
    }

    function getSaleStatus() public view returns (string memory) {
        string memory status;
        if (saleStatus == 0) {
            status = "Sale not open yet.";
        } else if (saleStatus == 1) {
            status = "Presale is open now.";
        } else {
            status = "Public sale is open now.";
        }
        return status;
    }
    ...
    function setMaxBatch(uint256 amount) external onlyOwner {
        maxBatchSize = amount;
    }

    function setCollectionSize(uint256 amount) external onlyOwner {
        require(
            amount >= totalSupply(),
    ...
    function getMaxMintPerTx() public view returns (uint256) {
        return maxPerTx;
    }
    ...
    function setPrice(uint256 priceName, uint256 newPrice) external onlyOwner {
        if (priceName == 1) {
    ...
    function setWhitelist(address[] calldata entries) external onlyOwner {
        for (uint256 i; i < entries.length; i++) {
            address entry = entries[i];
            require(
                entry != address(0),
                "Whitelist Address cannot be Null Address"
            );
            whitelist[entry] = 1;
        }
    }
    ...
    function mintNFT(uint256 amount) public payable nonReentrant {
        ...
        require(saleStatus > 0, "Sale is not started yet");
        require(amount > 0, "NFT amount must greater than 0");
        require(amount <= maxPerTx, "Mint amount exceeded max per tx");
        if (saleStatus == 1) {
            require(whitelist[sender] > 0, "You are not eligible for presale");
            require(
    ...
    // Giveaway / Claim reserved nft

    function giveaway(address _to, uint256 id) external onlyOwner {
        require(_to != address(0), "Cannot give NFT to address 0");
        safeTransferFrom(msg.sender, _to, id);
    }

    function claimReserved(uint256 amount) external onlyOwner {
        address sender = _msgSender();
        require(
            amount <= reserved,
    ...
    address private payoutAddress = 0x545a6c2474946eF26Bfd8F4DeB1205541450D267;
