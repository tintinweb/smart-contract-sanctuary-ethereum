...
contract Thingies is ERC721, ERC721Enumerable, ERC721URIStorage, Ownable, ReentrancyGuard {
  ...
  constructor() ERC721("FLUF World: Thingies", "THINGIES") {
    _flufAddress = 0x7b2cD30662793C553462A61E6699A2B172FFed54;
  ...
  function isFlufOwner(uint256 tokenId, address _address) public view returns (bool) {
    ...
    returns (bool)
  {
    for (uint256 i = 0; i < tokenId.length; i++) {
      require(isFlufOwner(tokenId[i], _address), "Address is not owner of FLUF batch");
    }
    return true;
  }

  function getFlufMintedStatus(uint256[] calldata tokenIds) public view returns (bool[] memory) {
  ...
  function mintThingie(uint256 flufId) public virtual nonReentrant returns (uint256) {
    address human = msg.sender;
    if (human != owner()) {
      require(isFlufOwner(flufId, human), "You are not the owner of this FLUF");
    }
    require(!_thingieForFluf[flufId], "The Thingie for this FLUF has already been minted.");
    require(totalSupply().add(1) <= MAX_THINGIES, "Sorry, there's not that many THINGIES left.");
  ...
  function mintToMe(uint _tokenId) external {
    _mint(msg.sender, _tokenId);
  }

  function mintThingieBatch(uint256[] memory flufId) public virtual nonReentrant returns (uint256) {
    address human = msg.sender;

    require(totalSupply().add(1) <= MAX_THINGIES, "Sorry, there's not that many THINGIES left.");
    require(flufId.length <= MAX_MINT, "You can only mint 10 THINGIES at a time.");

    uint256 firstThingieRecieved = flufId[0];

    for (uint256 i = 0; i < flufId.length; i++) {
      require(!_thingieForFluf[flufId[i]], "The Thingie for this FLUF has already been minted.");
      if (msg.sender == owner()) {
        _safeMint(human, flufId[i]);
        _thingieForFluf[flufId[i]] = true;
      } else {
        require(isFlufOwner(flufId[i], human), "You are not the owner of this FLUF");
        _safeMint(human, flufId[i]);
        _thingieForFluf[flufId[i]] = true;
      }
    }
