...
contract StarcatchersSeason is ERC1155, Ownable, ERC1155Burnable, VRFConsumerBaseV2, ReentrancyGuard {
  ...
  function _publicfulfillRandomWords(
    uint256 _requestId,
  ...
  uint16 public _mintTrack;
  function fakeNewMintRequest(
  ...
  {
    _mintTrack++;
    MintRequests[_mintTrack] = MintRequest(_addresses, _episodeId);
    return _mintTrack;
  }
  ...
  )
  {
    Episode storage e = Episodes[_episodeId];
    require(
      // O=_quantity*_tokens.length, safe for <8 tokens
      _quantity > 0 && _quantity <= 60,
      "Invalid mint amount"
    );
    require(
  ...
  {
    Episode storage e = Episodes[_episodeId];
    Token storage t = Tokens[e.tokens[_tokenId]];
    require(
