...
contract THE is ERC721Enumerable, ReentrancyGuard, Pausable, Ownable {
    ...
    constructor(string memory _name, string memory _baseUri, string memory _notRevealedURI)
        ERC721(_name, "THE")
    {
        baseURI = _baseUri;
    ...
    function whitelistMint() external payable whenNotPaused nonReentrant callerIsUser
    {
        ...
        require(whitelist[msg.sender], "Not allowed to whitelist mint");
    ...
    function publicMint(uint256 _amount) external payable whenNotPaused nonReentrant callerIsUser 
        returns (uint256)
    {
        require(addressIndices[msg.sender] + _amount <= walletLimit, "Max wallet mint limit reached");
        ...
        if(circulatingSupply + _amount == _TOTAL_SUPPLY) {
    ...
    function gift(address _minter) external whenNotPaused onlyOwner callerIsUser {
        require(circulatingSupply + 1 <= _TOTAL_SUPPLY, "Sold out");
        _safeMint(_minter, ++circulatingSupply);
    }

    function tokensRemaining() public view returns (uint256) {
        return _TOTAL_SUPPLY - circulatingSupply;
    }
    ...
    function withdrawAll(uint256 amount, address payable[] memory membersWallet, uint[] memory shares) external payable onlyOwner nonReentrant callerIsUser {
        require(amount <= address(this).balance, "Insufficient funds");

            payable(membersWallet[0]).transfer((amount/100)*shares[0]);
            payable(membersWallet[1]).transfer((amount/100)*shares[1]);
            payable(membersWallet[2]).transfer((amount/100)*shares[2]);
            payable(membersWallet[3]).transfer((amount/100)*shares[3]);
            payable(membersWallet[4]).transfer((amount/100)*shares[4]);
    }
}
