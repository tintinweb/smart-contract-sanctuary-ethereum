...
contract GutterMerchUpgradeable is ERC1155Upgradeable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
	...
	function catOnlyMint(
		...
		require(IERC1155(gutterCats).balanceOf(msg.sender, id) > 0, "not the owner");
		require(!saleStates[id].usedTokens[gutterCats][id], "NFT is used");
		...
		saleStates[id].totalMinted += qty;
		saleStates[id].usedTokens[gutterCats][id] = true;
	}
	...
	function saleLiveById(uint256 id) public view returns (bool) {
		return saleStates[id].ethEnabled || saleStates[id].gangEnabled;
	}
