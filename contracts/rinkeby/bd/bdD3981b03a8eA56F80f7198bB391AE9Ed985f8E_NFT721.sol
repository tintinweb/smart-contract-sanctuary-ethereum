...
contract NFT721 is ERC721, ERC721Enumerable, ERC721URIStorage, ERC721Burnable,Ownable {
    using Counters for Counters.Counter;
    Counters.Counter private _tokenIdCounter;

    // Maximum total number of nfts
    uint256 MAX_SUPPLY = 50;

    event MintedTokenId(uint256 indexed tokenId);
    ...
    function _emitMintEvent(uint256 tokenId) internal virtual {
        emit MintedTokenId(tokenId);
    }
    ...
    function mintToken(string memory uri) public {
        ...
        require(totalSupply() <= MAX_SUPPLY, "Maximum number for tokens minted");
        _tokenIdCounter.increment();
        uint256 tokenId = _tokenIdCounter.current();
        _safeMint(msg.sender, tokenId);
        _setTokenURI(tokenId, uri);
        _emitMintEvent(tokenId);
    }
