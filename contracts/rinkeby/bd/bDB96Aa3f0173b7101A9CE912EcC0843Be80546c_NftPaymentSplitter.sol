...
contract NftPaymentSplitter is Context, Ownable {
    ...
    address public _creat00rAddress;
    ...
    constructor(
        ...
        uint256 creat00rShare_,
        address creat00rAddress_,
     ...
     */
    function payee(uint256 tokenId) public view returns (uint256, address) {
        address _token_owner = nftCollection.ownerOf(tokenId);
        return (_payees[tokenId], _token_owner);
    }
    ...
    function release(uint256 tokenId) public virtual {
        ...
        if (tokenId == 0) {
            account = payable(_creat00rAddress);
        } else {
    ...
    function _addPayee(uint256 tokenId, uint256 shares_) private {
        ...
        if (tokenId != 0 && _totalSharesOffset - shares_ >= 0) {
     ...
     * @param creat00rAddress_ nft tokenId
     */
    function setCreat00rAddress(address creat00rAddress_) external onlyOwner {
        require(creat00rAddress_ != address(0), "Zero Address not allowed");
        _creat00rAddress = creat00rAddress_;
    }
    ...
    function releaseUnlcaimed(uint256[] memory tokenIds) external onlyOwner {
        ...
        Address.sendValue(payable(_creat00rAddress), totalPayment);
    }
        ...
        onlyOwner
    {
        ...
        SafeERC20.safeTransfer(token, _creat00rAddress, totalPayment);
    }
}
