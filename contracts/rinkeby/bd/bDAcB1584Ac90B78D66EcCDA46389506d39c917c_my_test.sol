...
interface bal721 { function balanceOf(address account, uint256 id) external view returns (uint256); }

interface bal1155 { function balanceOf(address account, uint256 id) external view returns (uint256); }
...
contract my_test is Context, AccessControl, ReentrancyGuard, Ownable {
    ...
    function userOnlineItems(address _addr) public view returns(uint32 _count) {  
        _count = 0;
        for (uint32 i = 0; i < items.length; i++) {
    ...
    function itemSell(uint _currency, uint256 _price, address _nftct, uint256 _nftid) public nonReentrant returns(uint32 _itemid){
        //auth control
        ...
        if(passCheck == 1) { rewardToAdd = REWARD_PT; }
        else if(passCheck == 2) { rewardToAdd = REWARD_AU; }
        else if(passCheck == 3) { rewardToAdd = REWARD_AG; }
        else if(passCheck == 4) { rewardToAdd = REWARD_CU; }
        ...
        items.push(Item(_currency, _price, msg.sender, 1, _nftct, _nftid, zeroAddress));
