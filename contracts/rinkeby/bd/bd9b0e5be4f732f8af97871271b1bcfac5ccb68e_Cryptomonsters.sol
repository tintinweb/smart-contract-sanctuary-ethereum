...
// File: contracts/Pajoranian_Market.sol
...
contract Cryptomonsters is ERC1155Holder {
    ...
    mapping(uint256 => Mon) public mons; // Holds all created Cryptomonsters

    uint256 public totalMons = 0; // Number of created Cryptomonsters

    uint256 private max = 2**256 - 1; // Max number of Cryptomonsters
    ...
    constructor(ERC20Burnable token, IERC1155 items) {
        ...
        // Add initial Cryptomonsters on contract deployment to start game
    ...
    function findEggSpecies(uint256 id1, uint256 id2) private returns (Species) {
        ...
        if (mons[id2].monType == 9) {
            ...
            // Only Cryptomonsters of the same type can breed into evolved type
            ...
            if (mons[id1].species == mons[id2].species) {
                ...
                // If Cryptomonsters of the same type but different species then
