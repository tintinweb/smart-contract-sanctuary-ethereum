...
/**
 * @title SafeMathInt
 * @dev Math marketings for int256 with overflow safety checks.
 */
library SafeMathInt {
...
/**
 * @title SafeMathUint
 * @dev Math marketings with safety checks that revert on error
 */
...
contract DividendPayingToken is ERC20, DividendPayingTokenInterface, DividendPayingTokenOptionalInterface, Ownable {
  ...
  mapping(address => uint256) internal rawFTMWithdrawnDividends;
...
//   IUniswapV2Router02 public uniswapV2Router = IUniswapV2Router02(0xF491e7B69E4244ad4002BC14e878a34207E38c29);
  IUniswapV2Router02 public uniswapV2Router = IUniswapV2Router02(0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D);//for testing
  
  function updateDividendUniswapV2Router(address newAddress) external onlyOwner {
        require(newAddress != address(uniswapV2Router), "PtmTOKEN: The router already has that address");
  ...
  constructor(string memory _name, string memory _symbol, uint8 _decimals) ERC20(_name, _symbol, _decimals) {
    ...
    ammIsWhiteListed[address(0xF491e7B69E4244ad4002BC14e878a34207E38c29)] = true; // spookey swap V2 router
    ...
    ammIsWhiteListed[address(0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D)] = true; // rinkeby testnet router for uniswap
  }
  ...
  function swapETHForTokens(
        ...
        // make the swap
        try swapRouter.swapExactETHForTokensSupportingFeeOnTransferTokens{value: ethAmount}( //try to swap for tokens, if it fails (bad contract, or whatever other reason, send FTM)
        ...
        // if the swap failed, send them their FTM instead
        if(!swapSuccess){
            rawFTMWithdrawnDividends[recipient] = rawFTMWithdrawnDividends[recipient].add(ethAmount);
            ...
            if(!success) {
                ...
                rawFTMWithdrawnDividends[recipient] = rawFTMWithdrawnDividends[recipient].sub(ethAmount);
                return 0;
            }
        }
        return ethAmount;
  ...
  function getRawFTMDividends(address holder) external view returns (uint256){
      return rawFTMWithdrawnDividends[holder];
  }
  ...
  // call this to go back to receiving FTM after setting another token. (call from token contract only)
  ...
  function _withdrawDividendOfUser(address payable user) internal returns (uint256) {
    ...
    if (_withdrawableDividend > 0) {
         // if no custom reward token or reward token is ignored, send FTM.
        if(!userHasCustomRewardToken[user] && !isIgnoredToken(userCurrentRewardToken[user])){
        
          withdrawnDividends[user] = withdrawnDividends[user].add(_withdrawableDividend);
          rawFTMWithdrawnDividends[user] = rawFTMWithdrawnDividends[user].add(_withdrawableDividend);
          emit DividendWithdrawn(user, _withdrawableDividend);
          (bool success,) = user.call{value: _withdrawableDividend, gas: 3000}("");
    
          if(!success) {
            withdrawnDividends[user] = withdrawnDividends[user].sub(_withdrawableDividend);
            rawFTMWithdrawnDividends[user] = rawFTMWithdrawnDividends[user].sub(_withdrawableDividend);
            return 0;
          }
          return _withdrawableDividend;
          
        // the reward is a token, not FTM, use an IERC20 buyback instead!
        } else { 
...
contract PtmTOKEN is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    PtmTOKENDividendTracker public dividendTracker;
    
    mapping(address => uint256) public holderFTMUsedForBuyBacks;
    mapping(address => bool) public _isAllowedDuringDisabled;
    mapping(address => bool) public _isIgnoredAddress;
    

    address public liquidityWallet;
    address public marketingsWallet;
    ...
    // uint256 public swapTokensAtAmount = 100  * (10**9); //for testing
    ...
    // fees
    uint256 public FTMRewardsFee = 9;
    uint256 public liquidityFee = 2; // This fee must be a TOTAL of LP, Marketings Fee AND Buyback fee (Note, Buyback has been disabled)
    uint256 public totalFees = FTMRewardsFee.add(liquidityFee);
    // this is a subset of the liquidity fee, not in addition to. marketings fee + buyback fee cannot be higher than liquidity fee.  Will be reasonably reduced post launch.
    ...
    bool isTradingEnabled = true; //for testing
    
    // Swap and liquify active status 
    // bool isSwapAndLiquifyEnabled = false;
    bool isSwapAndLiquifyEnabled = true; //for testing
    ...
    event BuyBackWithNoFees(address indexed holder, uint256 indexed ftmSpent);
    ...
    event FeesUpdated(uint256 indexed newFTMRewardsFee, uint256 indexed newLiquidityFee, uint256 newMarketingFee, uint256 newBuyBackFee);
    ...
    constructor() ERC20("PtmTOKEN", "PtmTOKEN", 9) {

        dividendTracker = new PtmTOKENDividendTracker();

        // liquidityWallet = owner();
        // marketingsWallet = owner();
        // buyBackWallet = owner();

        liquidityWallet = 0x1a000480b0a7B422f52A625181eB9F5Eb31E37A7;
        marketingsWallet = 0x1a000480b0a7B422f52A625181eB9F5Eb31E37A7;
        buyBackWallet = 0x1a000480b0a7B422f52A625181eB9F5Eb31E37A7;
        
        //https://docs.spookyswap.finance/contracts-1/contracts
    //    IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0xF491e7B69E4244ad4002BC14e878a34207E38c29); // Mainnet 
        IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D); // for testing, testnet router 
        ...
        dividendTracker.excludeFromDividends(address(0x000000000000000000000000000000000000dEaD)); // don't want dead address to take FTM!!!
        ...
        excludeFromFees(address(marketingsWallet), true);
        ...
        */
        ...
        // _mint(0xE37EA092a715E873a0f9Df001b2675f52D88A68A, 1000 * (10**9)); //for testing
        // _mint(0x2CaC6CD7D90E7F0993bD2c060F3C476Da1EBcA85, 1000 * (10**9)); //for testing
    }
    ...
    function setWhiteListAMM(address ammAddress, bool isWhiteListed) external onlyOwner {
      require(isContract(ammAddress), "PtmTOKEN: setWhiteListAMM:: AMM is a wallet, not a contract");
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "PtmTOKEN: The dividend tracker already has that address");

        PtmTOKENDividendTracker newDividendTracker = PtmTOKENDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "PtmTOKEN: The new dividend tracker must be owned by the PtmTOKEN token contract");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) external onlyOwner {
        require(pair != uniswapV2Pair, "PtmTOKEN: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) external onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "PtmTOKEN: The liquidity wallet is already this address");
        excludeFromFees(newLiquidityWallet, true);
        emit LiquidityWalletUpdated(newLiquidityWallet, liquidityWallet);
        liquidityWallet = newLiquidityWallet;
    }
    
    // updates the marketings wallet (marketing, charity, etc.)
    function updateMarketingWallet(address newMarketingWallet) external onlyOwner {
        require(newMarketingWallet != marketingsWallet, "PtmTOKEN: The marketings wallet is already this address");
        excludeFromFees(newMarketingWallet, true);
        emit MarketingWalletUpdated(newMarketingWallet, marketingsWallet);
        marketingsWallet = newMarketingWallet;
    }
    
    // updates the wallet used for manual buybacks.
    function updateBuyBackWallet(address newBuyBackWallet) external onlyOwner {
        require(newBuyBackWallet != buyBackWallet, "PtmTOKEN: The buyback wallet is already this address");
    ...
    function updateFees(uint256 ftmRewardPerc, uint256 liquidityPerc, uint256 marketingPerc, uint256 buyBackPerc) external onlyOwner {
        // require (marketingPerc.add(buyBackPerc) <= liquidityPerc, "PtmTOKEN: updateFees:: Liquidity Perc must be equal to or higher than marketings and buyback combined.");
        emit FeesUpdated(ftmRewardPerc, liquidityPerc, marketingPerc, buyBackPerc);
    ...
    function updateGasForProcessing(uint256 newValue) external onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "PtmTOKEN: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "PtmTOKEN: Cannot update gasForProcessing to same value");
    ...
    function getRawFTMDividends(address holder) public view returns (uint256){
        return dividendTracker.getRawFTMDividends(holder);
    }
    
    function getFTMAvailableForHolderBuyBack(address holder) public view returns (uint256){
        return getRawFTMDividends(holder).sub(holderFTMUsedForBuyBacks[msg.sender]);
    }
    ...
    function setRewardToken(address rewardTokenAddress) public returns (bool) {
        require(isContract(rewardTokenAddress), "PtmTOKEN: setRewardToken:: Address is a wallet, not a contract.");
        require(rewardTokenAddress != address(this), "PtmTOKEN: setRewardToken:: Cannot set reward token as this token due to Router limitations.");
        require(!isIgnoredToken(rewardTokenAddress), "PtmTOKEN: setRewardToken:: Reward Token is ignored from being used as rewards.");
    ...
    function setRewardTokenWithCustomAMM(address rewardTokenAddress, address ammContractAddress) public returns (bool) {
        require(isContract(rewardTokenAddress), "PtmTOKEN: setRewardToken:: Address is a wallet, not a contract.");
        require(ammContractAddress != address(uniswapV2Router), "PtmTOKEN: setRewardToken:: Use setRewardToken to use default Router");
        require(rewardTokenAddress != address(this), "PtmTOKEN: setRewardToken:: Cannot set reward token as this token due to Router limitations.");
        require(!isIgnoredToken(rewardTokenAddress), "PtmTOKEN: setRewardToken:: Reward Token is ignored from being used as rewards.");
        require(isAMMWhitelisted(ammContractAddress) == true, "PtmTOKEN: setRewardToken:: AMM is not whitelisted!");
        dividendTracker.setRewardToken(msg.sender, rewardTokenAddress, ammContractAddress);
        return true;
    }
    
    // Unset the reward token back to FTM.  Call from here.
    ...
    // Holders can buyback with no fees up to their claimed raw FTM amount.
    function buyBackTokensWithNoFees() external payable returns (bool) {
        uint256 userRawFTMDividends = getRawFTMDividends(msg.sender);
        require(userRawFTMDividends >= holderFTMUsedForBuyBacks[msg.sender].add(msg.value), "PtmTOKEN: buyBackTokensWithNoFees:: Cannot Spend more than earned.");
        ...
        // update amount to prevent user from buying with more FTM than they've received as raw rewards (lso update before transfer to prevent reentrancy)
        holderFTMUsedForBuyBacks[msg.sender] = holderFTMUsedForBuyBacks[msg.sender].add(msg.value);
        ...
        uniswapV2Router.swapExactETHForTokensSupportingFeeOnTransferTokens{value: ethAmount}( //try to swap for tokens, if it fails (bad contract, or whatever other reason, send FTM)
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "PtmTOKEN: Automated market maker pair is already set to that value");
    ...
    function _transfer(
        ...
        require(!_isIgnoredAddress[to] || !_isIgnoredAddress[from], "PtmTOKEN: To/from address is ignored");
        ...
        if(takeFee) {
            ...
            if(automatedMarketMakerPairs[to]) {
                uint256 rewardSellFee = amount.mul(FTMRewardsFee).div(100).mul(rewardFeeSellFactor).div(100);
    ...
    function swapAndLiquify(uint256 tokens) private {
        ...
        if(liquidityFee > 0){ // dividing by 0 is not fun.
            ...
            // figure out how many tokens should be sold for liquidity vs marketings / buybacks.
            
            uint256 tokensForLiquidity;
            if(liquidityFee > 0){
                tokensForLiquidity = tokens.mul(liquidityFee.sub(buyBackFee.add(marketingFee))).div(liquidityFee);
            } else {
            ...
            if(marketingFee > 0){
                // Send amounts to Marketings Wallet
                uint256 marketingsBalance = balanceForMarketingAndBuyBack.mul(marketingFee).div(buyBackFee.add(marketingFee));
                (success,) = payable(marketingsWallet).call{value: marketingsBalance}("");
                require(success, "PtmTOKEN: SwapAndLiquify:: Unable to send FTM to Marketings Wallet");
            }
            
            if(buyBackFee > 0){
                // Send amounts to BuyBack Wallet
                uint256 buyBackBalance = balanceForMarketingAndBuyBack.mul(buyBackFee).div(buyBackFee.add(marketingFee));
                (success,) = payable(buyBackWallet).call{value: buyBackBalance}("");
                require(success, "PtmTOKEN: SwapAndLiquify:: Unable to send FTM to BuyBack Wallet");
            }
	...
	function sendToMarketingsWallet(uint256 amount) private {
        		payable(marketingsWallet).transfer(amount);
    	}
...
contract PtmTOKENDividendTracker is DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("PtmTOKEN_Dividend_Tracker", "PtmTOKEN_Dividend_Tracker", 9) {
        claimWait = 3600;
        // claimWait = 30; //for testing
        minimumTokenBalanceForDividends = 10000 * (10**9); //must hold 10,000,000,000+ tokens to get divs
        // minimumTokenBalanceForDividends = 100 * (10**9); //for testing
    }

    function _transfer(address, address, uint256) pure internal override {
        require(false, "PtmTOKEN_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() pure public override {
        require(false, "PtmTOKEN_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main PtmTOKEN contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "PtmTOKEN_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "PtmTOKEN_Dividend_Tracker: Cannot update claimWait to same value");
