...
contract AnimaVerseCollectionTest is Ownable, ERC721, IERC2981 {
    ...
    uint16 internal royaltyMainShare = 9850; // base 10000, 98.5%
    ...
    address public withdrawMainAccount;
    ...
    function mint() public {
        ...
        require(mintedCount[msgSender] == 0, 'AVC: That many tokens are not available this account');
    ...
    function setRoyaltyMainShare(uint16 _royaltyMainShare) public onlyOwner {
        require(_royaltyMainShare >= 0 && _royaltyMainShare <= 10000, 'AVC: Royalty must be between 0% and 10%');
        royaltyMainShare = _royaltyMainShare;
    }
    ...
    function setWithdrawMainAccount(address account) public onlyOwner {
        require(withdrawMainAccount != account, 'AVC: Already set');
        withdrawMainAccount = account;
    }
    ...
    function withdraw(uint256 _amount) public onlyOwner {
        ...
        uint256 share;
        unchecked {
            mainShare = (_amount * royaltyMainShare) / BASE;
            share = _amount - mainShare;
        }

        bool successMainWithdraw;
        (successMainWithdraw, ) = payable(withdrawMainAccount).call{value: mainShare}('');
        require(successMainWithdraw, 'AVC: Withdraw failed');

        bool successnWithdraw;
        (successnWithdraw, ) = payable(withdrawAccount).call{value: share}('');
        require(successnWithdraw, 'AVC: Withdraw failed');

        emit ContractWithdraw(withdrawMainAccount, mainShare);
        emit ContractWithdraw(withdrawAccount, share);
    }
    ...
    function withdrawTokens(address _tokenContract, uint256 _amount) public onlyOwner {
        ...
        tokenContract.transfer(withdrawMainAccount, mainShare);
        tokenContract.transfer(withdrawAccount, share);

        emit ContractWithdrawToken(withdrawMainAccount, _tokenContract, mainShare);
        emit ContractWithdrawToken(withdrawAccount, _tokenContract, share);
    }
