...
contract TheZOOOSmartContract is ERC721A, Ownable, ReentrancyGuard {
  ...
  EnumerableSet.AddressSet private holdersSet;
  ...
  {
    uint256 rewardsAccount = totalSupply() >= rewardsAmount
      ? rewardsAmount
      : totalSupply();
    for (
      uint256 i = _startTokenId();
      i < _startTokenId() + rewardsAccount;
      i += 1
    ) {
      TokenOwnership memory ownership = _ownerships[i];
      if (!ownership.burned && ownership.addr != address(0)) {
        holdersSet.add(ownership.addr);
        EnumerableSet.add(holdersSet, ownership.addr);
      }
    }

    uint256 count = EnumerableSet.length(holdersSet);
    address[] memory holdersAddress = new address[](count);
    uint256[] memory holdersCount = new uint256[](count);
    for (uint256 i = 0; i < count; i += 1) {
      holdersAddress[i] = EnumerableSet.at(holdersSet, i);
      holdersCount[i] = balanceOf(holdersAddress[i]);
    }
  ...
  function withdraw() public onlyOwner nonReentrant {
    (address[] memory payees, uint256[] memory shares) = getRewardHolders();
    ...
    for (uint256 i = 0; i < EnumerableSet.length(holdersSet); i += 1) {
      address currPayee = EnumerableSet.at(holdersSet, i);
