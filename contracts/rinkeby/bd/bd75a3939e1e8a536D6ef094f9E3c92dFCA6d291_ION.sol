...
contract ION is ERC20, Ownable {
    ...
    uint256 public buySellTime = block.timestamp + 1200; //change before deployment
    ...
    uint256 twentyFourHours = 600; // change before deployment
    ...
    modifier onlyAdmin() {
        require(owner() == msg.sender, "ION: Not admin");
        _;
    }

    //---------------------------Admin-setters------------------------//

    function whitelistPair(address _newPair) external onlyAdmin validation(_newPair) {
        isPair[_newPair] = true;
    }


    function setEnableSwapAndLiquify(bool _bool) external onlyAdmin {
    ...
    function setMaticRewardTime(uint256 _time) external onlyAdmin {
     ...
     function setRewardsContract(address _address) public onlyAdmin validation(_address) {
    ...
    function setAdminContract(IAdmin _newAdmin) external onlyAdmin validation(address(_newAdmin)) {
        admin = _newAdmin;
    }

    function setRouter(IUniswapV2Router02 _newRouter) external onlyAdmin validation(address(_newRouter)) {
        uniswapV2Router = _newRouter;
    }

    function setStableCoin(IERC20D _stableCoin) external onlyAdmin validation(address(_stableCoin)) {
        USDC = _stableCoin;
    }
    ...
    function setDistributionStatus(bool _status) public onlyAdmin {
        isDistributionEnabled = _status;
    }

    function setTransferStatus(bool _status) public onlyAdmin {
        isTransferPaused = _status;
    }
    ...
    function _transfer(
        ...
        require(!isTransferPaused, "ION: Paused");
    ...
    function _checkSellLimit(address user, uint transactAmount) internal{
        if (userLimits[user].startTime + twentyFourHours < block.timestamp) {
