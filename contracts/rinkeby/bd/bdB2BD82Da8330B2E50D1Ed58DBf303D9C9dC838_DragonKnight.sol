...
contract DragonKnight is Ownable, ERC721A {
    ...
    // Defaulting legendaries to ipfs until image data added with loadLegendary function
    bool legChain = false;
                    ...
                    DragonKnightLibrary.encode(
                        bytes(
                            abi.encodePacked(
                                ...
                                '", "description":"Fuly on chain. Knights!", "attributes":',tokenIdToMetadata(_tokenId, legendary),', "image":"',imageURI,'"}'
                            )
                        )
                    )
                )
