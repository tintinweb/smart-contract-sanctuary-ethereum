...
// File: contracts/BoatsailMarket.sol
...
interface IBoatsailNFT {
	function initialize(string memory _name, string memory _uri, address creator, address _adminAddress, bool bPublic) external;	
...
contract BoatsailMarket is Ownable, ERC721Holder {
	...
	function initialize(address _adminAddress) external onlyOwner {
		adminAddress = _adminAddress;
        createCollection("Boatsail", "https://gateway.pinata.cloud/ipfs/QmdeoiiHcZpFSgA8UQV1tNfAtfwyuJat2eyoghXkPcXdHE", true);		
	}   
	...
	function createCollection(string memory _name, string memory _uri, bool bPublic) public returns(address collection) {
        ...
        IBoatsailNFT(collection).initialize(_name, _uri, msg.sender, adminAddress, bPublic);
