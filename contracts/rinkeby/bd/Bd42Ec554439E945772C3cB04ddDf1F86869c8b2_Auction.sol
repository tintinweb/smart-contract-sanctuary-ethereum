...
contract ManagerAuction is Initializable, OwnableUpgradeable, PausableUpgradeable, ERC721HolderUpgradeable {
	...
	uint256 public extendExpiredTime;
	uint256 public minPriceIncreaseBidPercent;
	uint256 public timeCheckEndAuction;
	...
	event ExtendExpiredTimeChanged(uint256 _oldValue, uint256 _newValue);
	event MinPriceIncreaseBidPercentChanged(uint256 _oldValue, uint256 _newValue);
	event TimeCheckEndAuctionChanged(uint256 _oldValue, uint256 _newValue);
	...
	function initialize() public virtual initializer {
		...
		xUserFeeAuction = 250;
		extendExpiredTime = 5 minutes;
		timeCheckEndAuction = 5 minutes;
		minPriceIncreaseBidPercent = 100; // 1%
	}
	...
	function setExtendExpiredTime(uint256 _newExtendExpiredTime) external onlyOwner returns (bool) {
		require(_newExtendExpiredTime <= 1 days, "Invalid-extendExpiredTime");
		uint256 _oldValue = extendExpiredTime;
		extendExpiredTime = _newExtendExpiredTime;
		emit ExtendExpiredTimeChanged(_oldValue, extendExpiredTime);
		return true;
	}

	function setTimeCheckEndAuction(uint256 _newTimeCheckEndAuction) external onlyOwner returns (bool) {
		require(_newTimeCheckEndAuction <= 1 days, "Invalid-timeCheckEndAuction");
		uint256 _oldValue = timeCheckEndAuction;
		timeCheckEndAuction = _newTimeCheckEndAuction;
		emit TimeCheckEndAuctionChanged(_oldValue, timeCheckEndAuction);
		return true;
	}

	function setMinPriceIncreaseBidPercent(uint256 _newValue) external onlyOwner returns (bool) {
...
contract Auction is ManagerAuction {
	...
	function bidAuction(
		...
		if (block.timestamp + timeCheckEndAuction > currentAuction.endTime) {
			currentAuction.endTime += extendExpiredTime;
		}
		...
		require(
			auctionBidCount[_auctionId] == 0 || _price > bidAuctions[auctionHighestBidId[_auctionId]].bidPrice * (ZOOM_FEE + minPriceIncreaseBidPercent) / ZOOM_FEE,
	...
	function editBidAuction(uint256 _bidAuctionId, uint256 _price) external payable whenNotPaused returns (uint256) {
		...
		require(
			_price > bidAuctions[auctionHighestBidId[objEditBidAuction.auctionId]].bidPrice * (ZOOM_FEE + minPriceIncreaseBidPercent) / ZOOM_FEE,
			'Price-bid-less-than-min-price'
		);
