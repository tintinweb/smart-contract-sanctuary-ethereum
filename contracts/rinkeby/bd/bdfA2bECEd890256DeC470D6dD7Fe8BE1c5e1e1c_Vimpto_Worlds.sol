...
contract Vimpto_Worlds is Ownable, ERC721Enumerable, ReentrancyGuard, VerifySignature, Pausable {
    ...
    uint256 public constant maxTotalSupply = 15;    //  changing to 15 for testing
    ...
    address public verificationAdmin;   //  changing to public for testing
    ...
    function mintWhitelist(MintPayload calldata _payload, bytes memory _signature) external payable nonReentrant whenNotPaused {

        MintDetails memory currentWhitelistDetails = whitelist;
        require(block.timestamp > currentWhitelistDetails.startAt, "Whitelist minting not started yet");
        require(block.timestamp < currentWhitelistDetails.expiresAt, "Whitelist minting expired");
        require(currentWhitelistDetails.minted + _payload._toMint <= maxTotalSupply, "Supply finished");
        require(balanceOf(msg.sender) + _payload._toMint <= currentWhitelistDetails.MintingLimitPerAddress, "Whitelist minting limit reached for this address");
        ...
        if(balanceOf(msg.sender) + _payload._toMint == currentWhitelistDetails.MintingLimitPerAddress){
    ...
    function mintFree(MintPayload calldata _payload, bytes memory _signature) external nonReentrant whenNotPaused{

        require(freeMintStarted, "Free mint not started yet");
        require(_payload._toMint == 1, "Only one NFT at a time");
        require(_tokenIds.current() + _payload._toMint <= maxTotalSupply, "All 1,000 NFT's have been minted");
    ...
    function ownerMint(address _to, uint8 _toMint) external onlyOwner{
        ...
        require(_tokenIds.current() + _toMint <= maxTotalSupply, "All 1,000 NFT's have been minted");
