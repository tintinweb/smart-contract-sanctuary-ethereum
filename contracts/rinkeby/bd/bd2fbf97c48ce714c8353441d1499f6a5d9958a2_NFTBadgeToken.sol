...
contract NFTBadgeToken is Initializable{
    ...
    // token -> uri
    mapping(uint256 => string) private mapUri;
    // badge -> attribute
   ...
   /// @custom:oz-upgrades-unsafe-allow constructor
    ...
    function setUri(uint256 _tokenId, string memory _uri) public {
        ...
        require(_tokenId < InterfaceNFTToken(NFTTokenAddress).currentTokenId() && _tokenId != 0, "token isn't exist");
        mapUri[_tokenId] = _uri;
    }

    function uri(uint256 _tokenId) public view returns (string memory) {
        return mapUri[_tokenId];
    }

    function safeTransferTokenFrom(address from, address to, uint256 id, uint256 amount, bytes memory data) public  {
    ...
    function safeBatchTransferTokenFrom(address from, address to, uint256[] memory ids, uint256[] memory amounts, bytes memory data) public  {
        require(InterfaceNFTPermission(NFTPermissionAddress).checkTransferRole(msg.sender),"only transfer role");
    ...
    function userTransfer(uint256 id, address from, address to) private {
        ...
        removeBadgeList(from, findBadgeIndex(from, id));
        if (findBadgeIndex(to, id) < int(0)) {
            mapUserBadge[to].push(id);
        }
    }
    ...
    function issueBadge(address _receiverAddress, Badge memory _badge, string memory _uri, Attribute [] memory _attributes) public {
        ...
        InterfaceNFTToken(NFTTokenAddress).mintToken(_receiverAddress, _badge.tokenId, _badge.amount, "", msg.sender, address(this), badgeType);

        mapUserBadge[_receiverAddress].push(_badge.tokenId);
    ...
    function voidToken(address _target, string memory _id, uint256 _amount, string memory _reason) public {
        ...
        removeBadgeList(_target, findBadgeIndex(_target, mapId[_id]));
    ...
    function findBadgeIndex(address _receiver, uint256 _tokenId) private view returns (int) {
        if (keccak256(abi.encode(mapTokenType[_tokenId])) == keccak256(abi.encode(badgeType))) {
    ...
    function removeBadgeList(address _receiver, int _index) private {
        ...
        if ( InterfaceNFTToken(NFTTokenAddress).balanceOfToken(_receiver, mapList[uint(_index)]) == 0) {
...
interface InterfaceNFTToken {
    ...
    function burnToken(address account, uint256 id, uint256 amount, address sender, string memory role) external;
    function mintToken(address account, uint256 id, uint256 amount, bytes memory data, address sender, address contractAddress, string memory tokenType) external;
    function safeTransferTokenFrom(address from, address to, uint256 id, uint256 amount, bytes memory data, address sender, string memory role) external;
    function safeBatchTransferTokenFrom(address from, address to, uint256[] memory ids, uint256[] memory amounts, bytes memory data, address sender, string memory role) external;
    function balanceOfToken(address account, uint256 id) external view returns (uint256);
}
...
contract NFTToken is Initializable, ERC1155Upgradeable, ERC1155BurnableUpgradeable, AccessControlUpgradeable, InterfaceNFTToken {
    ...
    function setTokenName(string memory _tokenName) external override {
        TokenName = _tokenName;
    }
    ...
    function burnToken(address account, uint256 id, uint256 amount, address sender, string memory role) external override {
        require(InterfaceNFTPermission(NFTPermissionAddress).checkBurnerRole(sender),"only burner role");
        require(InterfaceNFTPermission(NFTPermissionAddress).checkTokenAccessRole(sender, role) || InterfaceNFTPermission(NFTPermissionAddress).checkAdminRole(sender),"Invalid token access role.");
        _burn(account, id, amount);
    }

    function  mintToken(address account, uint256 id, uint256 amount, bytes memory data, address sender, address contractAddress, string memory tokenType) external override {
        require(InterfaceNFTPermission(NFTPermissionAddress).checkMinterRole(sender),"only minter role");
        require(InterfaceNFTPermission(NFTPermissionAddress).checkAccessRole(sender),"Need token access role.");
        _mint(account, id, amount, data);

        TokenInfo memory tokenInfo = TokenInfo(contractAddress, id, tokenType);

        mapTokenInfo[id] = tokenInfo;
        tokenId.increment();
    }

    function safeTransferTokenFrom(address from, address to, uint256 id, uint256 amount, bytes memory data, address sender, string memory role) external override   {
        require(InterfaceNFTPermission(NFTPermissionAddress).checkTransferRole(sender),"only transfer role");
        require(InterfaceNFTPermission(NFTPermissionAddress).checkTokenAccessRole(sender, role) || InterfaceNFTPermission(NFTPermissionAddress).checkAdminRole(sender),"Invalid token access role.");
        _safeTransferFrom(from, to, id, amount, data);
    }

    function safeBatchTransferTokenFrom(address from, address to, uint256[] memory ids, uint256[] memory amounts, bytes memory data, address sender, string memory role) external override  {
        require(InterfaceNFTPermission(NFTPermissionAddress).checkTransferRole(sender),"only transfer role");
        for (uint i = 0; i < ids.length; i++) {
            require(InterfaceNFTPermission(NFTPermissionAddress).checkTokenAccessRole(sender, role) || InterfaceNFTPermission(NFTPermissionAddress).checkAdminRole(sender),"Invalid token access role.");
            require(ids[i] > 0, "token 0 isn't exist.");
    ...
    function balanceOfToken(address account, uint256 id) external  view  override returns (uint256) {
        return balanceOf(account,id);
    }

    function getTokenInfo(uint id) public view returns (TokenInfo memory){
        return mapTokenInfo[id];
    }
...
contract NFTPermission is Initializable, AccessControlUpgradeable, InterfaceNFTPermission{
    ...
    function initialize(address _admin) initializer public {
        ...
        tokenAccessRoleAccountExist[_admin].permission = "ADMIN";
