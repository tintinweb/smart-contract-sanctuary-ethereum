 ...
 *  @author kaymo.eth
 *
 *  @dev Hex Color Codes are in the format of 0xARGB; 
 *
 */
contract Pixels is ERC721 {
    ...
    mapping (uint256 => bytes) public pixels;

    bytes16 private constant _HEX_SYMBOLS = "0123456789abcdef";

    error InsufficientFunds();

    event Absorb (uint256 indexed token0, bytes indexed token1);

    constructor() ERC721("Pixels", "Pixel" ) {}
    ...
    function _burn(uint256 tokenId) internal override {
        ...
        delete pixels[tokenId];
    }

    function tokenURI(uint256 tokenId) public view returns (string memory) {
        return viewSvg(tokenId);
    }

    function viewSvg(uint256 index) private view returns (string memory svg) {
        bytes memory pixel = pixels[index];
        svg = string(abi.encodePacked(SVG_HEADER));
        bytes memory buffer = new bytes(8);
        for (uint y = 0; y < 24; y++) {
            for (uint x = 0; x < 24; x++) {
                uint p = (y * 24 + x) * 4;
                if (uint8(pixel[p + 3]) > 0) {
                    for (uint i = 0; i < 4; i++) {
                        uint8 value = uint8(pixel[p + i]);
