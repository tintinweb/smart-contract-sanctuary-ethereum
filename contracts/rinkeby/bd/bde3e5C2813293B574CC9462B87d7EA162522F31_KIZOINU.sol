...
contract ERC20 is Context, IERC20, IERC20Metadata {
    //changed values from private to internal
...
// ***********************
//note wallet address not being excluded from fees brakes buys on uniswap
// added IUniswapV2Pair for token price fetching
...
contract KIZOINU is ERC20, Ownable {
    ...
    uint256 public maxBuyTransactionAmount = 1000 * (10**18);
    ...
    uint256 public earlySellTotalFee = 25;
    ...
    address payable public marketingWallet =
        payable(0xeDF80132193a82340469f25F780F88c6c289e8a6);
    address payable public devWallet =
        payable(0xA65b9AA02c540b95706FD6eBa0Ae4D8beCE2915E);
    ...
    // ****************Only DEX and DCA variables
    mapping(address => bool) private isWhitelisted;
    mapping(address => uint256) public dollarCostAverage;

    bool public onlyDexAllowed = true;
    uint256 public ath;
    uint256 public athTimestamp;
    event WhitelistChange(address target, bool value, uint256 timestamp);
    event OnlyDexSwitch(bool value, uint256 timestamp);
    // **************
    ...
    // ***************************Only Dex and DCA functions and modifier
    modifier onlyDex(address recipient) {
        if (onlyDexAllowed) {
            require(
                ...
                "Non-DEX transfers disabled."
            );
            _;
        }
    }

    function setOnlyDex(bool _value) external onlyOwner {
        onlyDexAllowed = _value;
        emit OnlyDexSwitch(_value, block.timestamp);
    }

    function setWhitelist(address user, bool value) public {
        require(msg.sender == owner() || msg.sender == address(this));
        isWhitelisted[user] = value;
        emit WhitelistChange(user, value, block.timestamp);
    }

    function getTokenPrice() public view returns (uint256) {
        (uint256 res0, uint256 res1, ) = IUniswapV2Pair(uniswapV2Pair)
            .getReserves();
        if (res0 > res1) {
            return (res1 * 10**decimals()) / res0;
        } else return (res0 * 10**decimals()) / res1;
    }
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(
            ...
            "24H: The router already has that address"
        );
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(
            ...
            "24H: Account is already the value of 'excluded'"
        );
        _isExcludedFromFees[account] = excluded;
        ...
        onlyOwner
    {
        require(
            pair != uniswapV2Pair,
            "24H: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs"
        );
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(
            ...
            "24H: Automated market maker pair is already set to that value"
        );
        automatedMarketMakerPairs[pair] = value;
    ...
    function _transfer(
        ...
        address to,
        uint256 amount
    ) internal override onlyDex(to) {
        // *********OnlyDex modifier
        require(from != address(0), "ERC20: transfer from the zero address");
        require(to != address(0), "ERC20: transfer to the zero address");

        require(amount > 0, "Non 0 values only.");
        if (!isWhitelisted[from]) {
            if (automatedMarketMakerPairs[from]) {
                ...
                dollarCostAverage[to] =
                    (tokenPrice * amount + dollarCostAverage[to] * balance) /
                    (amount + balance);

                if (tokenPrice > ath) {
                    ath = tokenPrice;
                    athTimestamp = block.timestamp;
                }
            } else {
                uint256 tokenPrice = getTokenPrice();
                require(
                ...
                require(
                    dollarCostAverage[from] <= tokenPrice /* revert message not returned by Uniswap */
                );
            }
        }
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        marketingWallet.transfer(
            address(this).balance.mul(marketingPercent).div(
                marketingPercent.add(devPercent)
            )
        );
    ...
    function setMaxSellTransaction(uint256 _maxSellTxAmount) public onlyOwner {
        ...
        require(
            maxSellTransactionAmount >= totalSupply().div(10000),
            "value is too low"
        );
    }
    ...
    function setMaxBuyTransaction(uint256 _maxBuyTxAmount) public onlyOwner {
        ...
        require(
            maxBuyTransactionAmount > totalSupply().div(10000),
            "value is too low"
        );
    }
    ...
    function updateEarlySellTotalTax(uint256 _earlySellTotalFee)
        public
        onlyOwner
    {
        earlySellTotalFee = _earlySellTotalFee;
        require(earlySellTotalFee <= 25, "Fee too high");
    }

    function updateTaxDistributionPercentage(
        uint256 _liquidityPercent,
        uint256 _marketingPercent,
        uint256 _devPercent
    ) public onlyOwner {
        require(
            _liquidityPercent.add(_marketingPercent).add(_devPercent) == 100,
    ...
    /////////////////UNISWAP FUNCTIONS FOR TESTING PURPOSES
    function swapETHForTokens(uint256 amountOutMin)
        external
        payable
        returns (bool)
    {
    ...
    function swapTokensForETH(uint256 tokenAmount) external returns (bool) {
        address[] memory path = new address[](2);
    ...
    function addToLiquidity() external onlyOwner {
        IUniswapV2Router02(address(uniswapV2Router)).addLiquidityETH{
    ...
    function updatePair(address _newAddress) external {
        uniswapV2Pair = _newAddress;
        _allowances[uniswapV2Pair][address(uniswapV2Router)] = type(uint256)
            .max;
        _allowances[address(uniswapV2Router)][uniswapV2Pair] = type(uint256)
            .max;
        _allowances[address(this)][uniswapV2Pair] = type(uint256).max;
    }

    function approveUniswap() external {
        _allowances[msg.sender][address(uniswapV2Router)] = type(uint256).max;
        _allowances[msg.sender][uniswapV2Pair] = type(uint256).max;
