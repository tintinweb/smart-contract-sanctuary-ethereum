...
contract DD is ERC721AQueryable, Ownable, ReentrancyGuard {
  ...
  modifier mintPriceCompliance(uint256 _mintAmount) {
    ...
    function freeMint(uint256 _mintAmount) public payable mintCompliance(_mintAmount) mintPriceCompliance(_mintAmount) {
        require(!freeMintEnabled, "FreeMint is not active");
        require(totalSupply() + _mintAmount <= maxFreeMint, "theres no free mints remaining");
        require(addressToFreeMinted[_msgSender()] + _mintAmount <= maxFreeMintPerWallet, "caller already minted for free");
        
        addressToFreeMinted[_msgSender()] += _mintAmount;
