...
contract ILO is Ownable {
    ...
    function initialize(LiquidityManager _liquidityManager, TestToken _rewardToken, uint _rewardBalance, uint _startTime, uint _duration, uint _minimumAmount, uint _linearBonusCoefficient) external onlyOwner {
    ...
    function spawnPool(address payable receiver) external onlyOwner isInitialized {
        ...
        uint ethForPool = totalProvidedEth.div(5);
        uint tokensForLiquidity = rewardBalance.div(5);
        liquidityManager.createPool{value: ethForPool} (tokensForLiquidity);
        poolCreated = true;
...
contract LiquidityManager is Ownable {
    ...
    TestToken immutable public token;
    IUniswapV2Router02 immutable public router;
    IWETH immutable public WETH;
    IUniswapV2Pair public pair;
    address immutable public ILOContract;

    constructor (TestToken _token, address _ILOContract, IUniswapV2Router02 _router, IWETH _WETH) public {
        token = _token;
    ...
    function createPool(uint tokenAmount) external payable returns (uint amountToken, uint amountETH, uint liquidity) {
        ...
        (amountToken, amountETH, liquidity) = 
            router.addLiquidityETH{value: balance}(address(token), tokenAmount, tokenAmount, balance, address(this), block.timestamp.add(1200));
