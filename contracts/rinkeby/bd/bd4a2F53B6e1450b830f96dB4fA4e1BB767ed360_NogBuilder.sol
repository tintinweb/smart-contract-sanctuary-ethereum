...
library NogBuilder {
    ...
    function getColorMetadata(uint frameColorLength, uint16[7] memory colorPalette, string[7] memory colors) public view returns (string memory colorMetadata) {
        bytes memory list;
    ...
    function getBackgroundGradient(Structs.Seed memory seed) public view returns (string memory backgroundGradient) {
        string[5] memory vals = ["22", "33", "44", "55", "66"];
        string[4] memory animations = [
            ...
            string(abi.encodePacked('<animate attributeName="x2" values="100;',vals[seed.nogStyle],';100" dur="28s" repeatCount="indefinite"></animate><animate attributeName="y2" values="10;85;10" dur="17s" repeatCount="indefinite"></animate>')),
            string(abi.encodePacked('<animate attributeName="x1" values="', vals[seed.nogStyle], ';100;', vals[seed.nogStyle], '" dur="42s" repeatCount="indefinite"></animate><animate attributeName="y1" values="10;66;10" dur="22s" repeatCount="indefinite"></animate>'))
        ];
        
        string memory animation;
        if (seed.hasAnimation == true) {
            animation = animations[seed.backgroundStyle - 1];
        }
        ...
        string memory gradientMesh = string(abi.encodePacked(
            '<path d="M0 0h100v100H0z" fill="#fff" opacity="0.5"/>',
            '<g filter="url(#grad)" transform="scale(1.5) translate(-25 -25)" opacity="0.5">',
        ...
        string[4] memory gradients = [gradientMesh, gradientRadial, linearGradient, lightGradient];

        backgroundGradient = string (
            abi.encodePacked(gradients[seed.backgroundStyle - 1])
        );
    }

    function getNogColorStyles(Structs.Seed memory seed) public view returns (string memory nogColorStyles) {
        return string(
            abi.encodePacked(
                '<style>.shade{fill:',
                seed.shade,
                '}.bg{fill:#',
                seed.colors[seed.colorPalette[0]],
                '}.a{fill:#',
    ...
    function getTokenIdSvg(Structs.Seed memory seed) internal view returns (string memory svg) {
        ...
        if (seed.backgroundStyle >= 1) {
            backgroundGradient = getBackgroundGradient(seed);
        }
            ...
            string(
                abi.encodePacked(
                    '<svg viewBox="0 0 100 100" xmlns="http://www.w3.org/2000/svg" style="shape-rendering:crispedges">',
                        ...
                        '</defs>',
                        '<svg viewBox="0 0 100 100"><path class="bg" d="M0 0h100v100H0z"/><path class="shade" d="M0 0h100v100H0z"/>', 
    ...
    function getColorPalette(Structs.Seed memory seed) public view returns (string memory colorMetadata) {
        bytes memory list;
    ...
    function getAttributesMetadata(Structs.Seed memory seed) public view returns (string memory extraMetadata) {
        ...
        if (seed.hasAnimation && seed.backgroundStyle != 0) {
        ...
        return string(abi.encodePacked(
            ...
            '{"trait_type":"Background color", "value":"#',
                abi.encodePacked(string(seed.colors[0])),
            '"},',
            animatedMetadata, 
            floatingMetadata,
