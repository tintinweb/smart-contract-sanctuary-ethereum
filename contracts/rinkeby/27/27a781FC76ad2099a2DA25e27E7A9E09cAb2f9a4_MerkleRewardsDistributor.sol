...
contract MerkleRewardsDistributor is Ownable, ReentrancyGuard {
    ...
    /// @notice Ends the current epoch so that a new one can begin.
    function endEpoch(
    ...
    function _swapSingle(
        ...
        return amounts[0] == _amountIn;
    }

    /// @notice Swaps all tokens in _tokensIn for _tokenOut.
    function swapMultiple(
        ...
        require(
            tokensToBeSwappedLen == _amountsIn.length &&
                tokensToBeSwappedLen == _amountsOutMin.length,
            "Number of input tokens should be the same as number of swap in and out amounts"
        );
        ...
        for (uint256 i = 0; i < tokensToBeSwappedLen; ++i) {
            if (ethToSwap > 0) {
                require(
                    ethToSwap == _amountsIn[i],
                    "msg.value not equal to amount to swap value"
                );
                require(_tokensIn[i] == _wethContractAddress);
                address[] memory path = new address[](2);
                ...
                uint256[] memory amounts = routerContract.swapExactETHForTokens{
                    value: msg.value
                }(_amountsOutMin[i], path, address(this), block.timestamp);
                emit XYZReceived(amounts[1]);
                ethToSwap = 0;
            } else {
                _swapSingle(_tokensIn[i], _amountsIn[i], _amountsOutMin[i]);
            }
        }
    }
