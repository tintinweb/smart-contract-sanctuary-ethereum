...
contract RefundContract is Ownable, ReentrancyGuard, IERC721Receiver {
  ...
  function refund(uint256 _tokenId, bytes32[] calldata _merkleProof) public nonReentrant {
    ...
    require(!isTokenClaimedReward(_tokenId), 'Token claimed reward already');
  ...
  function buildMerkleLeaf(uint256 _tokenId) public view returns(bytes memory){ 
  ...
  function isTokenClaimedReward(uint256 _tokenId) public view returns (bool){ 
    return claimedRewardTokens[_tokenId];
  }
