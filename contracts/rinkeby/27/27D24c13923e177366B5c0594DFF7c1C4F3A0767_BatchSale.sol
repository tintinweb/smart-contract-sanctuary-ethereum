...
contract BatchSale is ERC1155Holder, ReentrancyGuard {
    FERC1155 public immutable fnft;

    struct AuctionInfo {
        uint256 reserve; // the reserve price for the auction
        uint256 amount; // the amount of fractions being auctioned
        uint256 start; // the start time for the auction
        uint256 end; // the end time for the auction
        uint256 total; // total eth contributed by users
        bool settled; // was the auction settled
    }
    mapping(uint256 => AuctionInfo) public fIdToAuction; //fId=> AuctionId => AuctionInfo
    mapping(uint256 => mapping(address => uint256)) public contributions; // fId => who => amount
    ...
    event Cancel(address indexed fnft, uint256 indexed fId, uint256 amount);
    event Close(
        address indexed fnft,
        uint256 indexed fId,
        uint256 reserve,
    ...
    /// @notice Returns the curator for a vault for fraction Id
    /// @param fId The id of the fractions
    /// @return checks the vault and returns the curator address
    function getCurator(uint256 fId) internal view returns (address) {
        return Vault(payable(fnft.idToVault(fId))).curator();
    }

    function getAuctionInfo(uint256 _fId)
        public
        view
        returns (
            uint256,
    ...
    {
        AuctionInfo memory info = fIdToAuction[_fId];
        return (
            info.reserve,
            info.amount,
            info.start,
    ...
    /// @notice Returns the reserve for an auction for fraction Id
    /// @param fId The id of the fractions
    /// @return checks the mapping of auctions and returns the reserve amount
    function getReserve(uint256 fId) public view returns (uint256) {
        return fIdToAuction[fId].reserve;
    }

    /// @notice checks storge for a users contribution to an Auction
    /// @param fId the id of the fractions
    /// @return returns a users contributions
    function getUserContribution(uint256 fId, address who)
        public
        view
        returns (uint256)
    {
        return contributions[fId][who];
    }

    /// @notice Returns the users share of contributed funds for an aution for a fraction Id
    /// @param fId The id of the fractions
    /// @param who The user whose share is being checked
    /// @return share Checks the mapping of auctions and returns the users share
    function getCurrentShare(uint256 fId, address who)
        public
        view
        returns (uint256 share)
    {
        (, uint256 amount, , , uint256 total, ) = getAuctionInfo(fId);
        uint256 bal = contributions[fId][who];
        share = (amount * bal) / total;
    }

    /// @notice Returns a tuple with the start and end time for the auction
    /// @param fId The id of the fractions
    function getStartEnd(uint256 fId)
        public
        view
        returns (uint256 start, uint256 end)
    {
        (, , start, end, , ) = getAuctionInfo(fId);
    }

    /// @notice modifier to decorate functions only callable by curator
    /// @param fId The id of the fractions
    ...
    /// @notice modifier to decorate functions only callable while auction is ongoing
    /// @param fId The id of the fractions
    ...
    /// @notice modifier to decorate functions only callable before an auciton
    /// @param fId The id of the fractions
    modifier beforeActive(uint256 fId) {
        (uint256 start, ) = getStartEnd(fId);
        require(block.timestamp < start, "active or ended");
        _;
    }
    /// @notice modifier to decorate functions only callable auction after an auciton
    /// @param fId The id of the fractions
    modifier afterEnded(uint256 fId) {
        (, uint256 ended) = getStartEnd(fId);
        require(block.timestamp > ended, "not ended");
        _;
    }

    /// @notice first function to start create an auction
    /// @param _fId The id of the fractions
    /// @param _reserve The minimum amount of eth needed for an auction to succeed
    /// @param _amount The amount of fractions being auctioned
    /// @param _start the timestamp when contributions can start
    /// @param _end the timestamp when contributions no longer accepted and auction settles
    function launch(
        uint256 _fId,
        uint256 _reserve,
        uint256 _amount,
        ...
        require(fIdToAuction[_fId].end == 0, "already listed");
        fIdToAuction[_fId] = AuctionInfo(
            _reserve,
            _amount,
            _start,
            _end,
    ...
    /// @notice function buyers interact with where they contribute eth for a portion of the fractions
    /// @param _fId the id of the fractions being purchased
    function contribute(uint256 _fId)
        external
        payable
        nonReentrant
        whileActive(_fId)
    {
        uint256 end = fIdToAuction[_fId].end;
        if (block.timestamp + 15 minutes > end)
            fIdToAuction[_fId].end += 15 minutes;
        fIdToAuction[_fId].total += msg.value;
        contributions[_fId][msg.sender] += msg.value;
        emit Contribution(msg.sender, address(fnft), _fId, msg.value);
    }

    /// @notice function buyers interact with where they can withdraw their contributed eth during the auction
    /// @param _fId the id of the fractions being purchased
    /// @param _amount the amount of eth the buyer wants back
    function leave(uint256 _fId, uint256 _amount)
        external
        payable
        nonReentrant
        whileActive(_fId)
    {
        (uint256 reserve, , , , uint256 total, ) = getAuctionInfo(_fId);
        require(total - _amount >= reserve, "cant withdraw below reserve");
        contributions[_fId][msg.sender] -= _amount;
        fIdToAuction[_fId].total -= _amount;
        payable(msg.sender).transfer(_amount);
        emit WithdrawContribution(msg.sender, address(fnft), _fId, _amount);
    }

    /// @notice function buyers interact with where they claim fractions / eth after an auction settles
    /// @param _fId the id of the fractions being purchased
    function claim(uint256 _fId) external nonReentrant afterEnded(_fId) {
        (uint256 reserve, uint256 amount, , , uint256 total, ) = getAuctionInfo(
            _fId
        );

        uint256 bal = contributions[_fId][msg.sender];
        contributions[_fId][msg.sender] = 0;
        if (total < reserve) {
            payable(msg.sender).transfer(bal);
            emit WithdrawClaim(msg.sender, address(fnft), _fId, bal);
        } else {
            uint256 share = (amount * bal) / total;
            fnft.safeTransferFrom(address(this), msg.sender, _fId, share, "0");
            emit WithdrawClaim(msg.sender, address(fnft), _fId, share);
        }
    }

    /// @notice function curator interacts with where they can update their reserve before the auction begins
    /// @param _fId the id of the fractions being purchased
    /// @param _reserve the new reserve
    function update(uint256 _fId, uint256 _reserve)
        external
        onlyCurator(_fId)
        beforeActive(_fId)
    {
        fIdToAuction[_fId].reserve = _reserve;
        emit UpdateReserve(address(fnft), _fId, _reserve);
    }

    /// @notice function for klemen to modify the end time for an auction during testing
    /// @param _fId the id of the fractions being purchased
    /// @param _newEndTime The new end timestamp for an auction
    function modify_end(uint256 _fId, uint256 _newEndTime)
        external
        onlyCurator(_fId)
        beforeActive(_fId)
    {
        fIdToAuction[_fId].end = _newEndTime;
    }

    /// @notice function curator interacts with where they cancel before the auction begins
    /// @param _fId the id of the fractions being purchased
    function cancel(uint256 _fId)
        external
        onlyCurator(_fId)
        beforeActive(_fId)
    {
        uint256 amount = fIdToAuction[_fId].amount;
        address curator = getCurator(_fId);
        fnft.safeTransferFrom(address(this), curator, _fId, amount, "0");
        emit Cancel(address(fnft), _fId, amount);
    }

    /// @notice function curator interacts with where they close an auction if it doesn't meet the reserve
    /// @param _fId the id of the fractions being purchased
    function close(uint256 _fId) external onlyCurator(_fId) afterEnded(_fId) {
        (
            uint256 reserve,
            uint256 amount,
            ,
        ...
        require(!settled, "curator already withdrew");
        fIdToAuction[_fId].settled = true;
        address curator = getCurator(_fId);
        if (reserve > total) {
            fnft.safeTransferFrom(address(this), curator, _fId, amount, "0");
        } else {
            payable(curator).transfer(total);
        }
        emit Close(address(fnft), _fId, reserve, total);
    }
}