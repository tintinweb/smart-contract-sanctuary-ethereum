...
contract Membership is IMembership, IEvents, ERC165, Ownable {
    ...
    returns (string memory)
  {
    ...
    Class storage c = _registry[id];

    // attributes
    bytes memory attr = "[";
    for (uint256 i = 0; i < c.plugins.length; i++) {
      attr = abi.encodePacked(
        attr,
        '{"trait_type": "plugin:',
        i.toString(),
        '","value": "',
        IPlugin(c.plugins[i].plugin).metadata(c.plugins[i].pclass),
        '"},'
      );
    }
    attr = abi.encodePacked(
      ...
      IResolver(c.resolver.resolver).metadata(c.resolver.params),
      '"}]'
    );

    // svg
    bytes memory svg = abi.encodePacked(
      '<svg width="256" height="256" fill="black" xmlns="http://www.w3.org/2000/svg">',
      '<rect x="0" y="0" width="256" height="256" style="fill:black" />',
      '<text font-size="14" y="25" x="10" fill="white">',
      _name,
      "</text>",
      '<text font-size="12" y="45" x="10" fill="white">Class #',
      id.toString(),
      "</text>"
    );
    uint256 y = 80;
