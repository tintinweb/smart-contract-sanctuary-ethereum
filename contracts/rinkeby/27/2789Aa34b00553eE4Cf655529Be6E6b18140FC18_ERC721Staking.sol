...
// File: contracts/newnewnftstaking.sol
...
contract ERC721Staking is ERC721Holder, ReentrancyGuard, Ownable, Pausable {
    ...
    function getStakedIds(address _user) public view returns(uint256[1500] memory) {
        uint256[1500] memory ids;
        uint256 id;
        uint256 count;
        while(count < balances[_user]) {
            if (stakedAssets[id] == _user) {
                ids[count] = id;
                count++;
            }
            id++;
    ...
    function stake(uint256[] memory tokenIds) external nonReentrant whenNotPaused updateReward(msg.sender) {
        ...
        for (uint256 i = 0; i < tokenIds.length; i += 1) {
            require(rare[tokenIds[i]], "NFT is not rare.");
    ...
    function setRares(uint256[] memory ids) public onlyOwner {
