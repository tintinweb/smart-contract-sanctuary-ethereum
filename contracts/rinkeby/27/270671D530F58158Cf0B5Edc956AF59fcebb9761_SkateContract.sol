...
contract SkateContract is ERC721Enumerable, Ownable, ReentrancyGuardUpgradeable {
    ...
    // Constants
    uint16 public auction_period_blocks = 30;
    ...
    */
    function setSkateDaoAddresses(address _skate, address _dao) public onlyOwner {
        skate = _skate;
        dao = _dao;
    }
    /**
    ...
    */
     ...
     */
    function createBid(uint256 gnarId, uint8 _skatePercent, uint8 _daoPercent) external payable nonReentrant {
    ...
    function remainBlocks() public view returns (uint256) {
        require(auction.endBlock >= block.number, "No remain blocks!");
        return auction.endBlock - block.number;
    }

    function setAuctionPeriodBlocks(uint16 _auction_period_blocks) external onlyOwner {
        auction_period_blocks = _auction_period_blocks;
    }
}
...
interface IGnarSeeder {
    ...
    function generateSeed(uint256 nounId, IGnarDescriptor descriptor) external view returns (Seed memory);
}
