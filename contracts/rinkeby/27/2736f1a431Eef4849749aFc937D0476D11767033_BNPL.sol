...
contract BNPL is ReentrancyGuard, Ownable, IERC721Receiver {
    ...
    */ 
     function buyNow(uint256 _itemId, uint256 _orderId, uint256 _nftPrice, uint256 _downPayment, address _custodioWallet, bytes32 _sign) external payable nonReentrant{
        ...
        bytes32 sign = hashByShaToString(_downPayment, _itemId, _nftPrice, _orderId, key);
    ...
    */ 
    ...
    function hashByShaToString(uint256 _downPayment, uint256 _nftId, uint256 _nftPrice, uint256 _orderId,  string memory _key) public pure returns(bytes32) {
      string memory down_payment = Strings.toString(_downPayment);
      string memory nft_id = Strings.toString(_nftId);
      string memory nft_price = Strings.toString(_nftPrice);
      string memory order_id = Strings.toString(_orderId);
      
      return sha256(abi.encodePacked(down_payment, nft_id, nft_price, order_id, _key));
    }

    function checkSign(uint256 _downPayment, uint256 _nftId, uint256 _nftPrice, uint256 _orderId,  string memory _key, bytes32 _sign) external pure returns(bool){
        bytes32 sign = hashByShaToString(_downPayment, _nftId, _nftPrice, _orderId, _key);
        return sign == _sign;
    }
