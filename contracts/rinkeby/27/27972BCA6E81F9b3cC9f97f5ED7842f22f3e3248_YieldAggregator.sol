...
import {IEthAnchorRouter} from "../ethanchor/IEthAnchorRouter.sol";
    ...
    PausableUpgradeable
{
    ...
    modifier whiteListedOnly() {
        if (whitelisting) {
            require(
                ...
                "Yield Aggregator: Address is not whitelisted."
            );
            _;
        } else {
            _;
        ...
        onlyRole(BACKEND_ADMIN_ROLE)
    {
        require(
            ...
            string(
                whitelisting
                    ? "Yield Aggregator: Whitelist is already active."
                    : "Yield Aggregator: Whitelist is already not active."
            )
        );
        whitelisting = _whitelisting;
    }
    ...
    /**
     * @notice Adds an account to whitelist
     * @param _account to be added within whitelist
     */
        ...
        onlyRole(BACKEND_ADMIN_ROLE)
    {
        require(
            ...
            "Yield Aggregator: Account is already whitelisted."
        );
        _isWhiteListed[_account] = true;
    }

    /**
     * @notice Removes an account from whitelist
     * @param _account to be removed within whitelist
     */
        ...
        onlyRole(BACKEND_ADMIN_ROLE)
    {
        require(
            _isWhiteListed[_account],
            "Yield Aggregator: Account is not whitelisted."
        );
        _isWhiteListed[_account] = false;
    }

    /**
     * @notice Checks whether account is within whitelist or not
     * @param _account: address to be checked against whitelist
     * @return returns true if account is whiting whitelist; false otherwise
     */
