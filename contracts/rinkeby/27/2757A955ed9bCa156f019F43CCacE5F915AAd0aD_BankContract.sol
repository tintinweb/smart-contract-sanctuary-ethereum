...
contract BankContract is Ownable, ReentrancyGuard {
    ...
    uint256 currentDepositId = 0;
    uint256 totalStateAmount = 0;
    ...
    function initDeposit(uint256 R) external onlyOwner nonReentrant {
        stakingToken.transfer(address(this), R);
        initDepositValue = R;
    }
    ...
    function withdrawFunds() external onlyOwner nonReentrant {
        ...
        stakingToken.transferFrom(address(this), msg.sender, balance);
    }
    ...
    function withdraw(uint256 id) external nonReentrant {
        ...
        stakingToken.transferFrom(address(this), msg.sender, amountToWithdraw);
        stakeIds.remove(id);
        ownedDeposits[msg.sender].remove(id);
        totalStateAmount -= stake.amount;
        delete stakes[id];
    }
    ...
    function getReward(uint256 id) public view returns (uint256) {
        ...
        if (time.div(stakePeriod) >= 4) {
            return initDepositValue.mul(R1 + R2 + R3).div(1000).mul(stakeAmount).div(totalStateAmount);
        } else if (time.div(stakePeriod) >= 3) {
            return initDepositValue.mul(R1 + R2).div(1000).mul(stakeAmount).div(totalStateAmount);
        } else if (time.div(stakePeriod) >= 2) {
            return initDepositValue.mul(R1).div(1000).mul(stakeAmount).div(totalStateAmount);
        } else {
