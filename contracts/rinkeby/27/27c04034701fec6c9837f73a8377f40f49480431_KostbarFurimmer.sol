...
contract KostbarFurimmer is ERC721A, Ownable {
    ...
    constructor(string memory _baseTokenURI, bytes32 _root, address _treasury)
    ...
    // treasury should be the owner
    function transferToTeam(address[] calldata tos) external onlyOwner {
        require(owner() == treasury);
        for (uint256 i; i < tos.length;) {
            for (uint256 j; j < 4;) {
                transferFrom(treasury, tos[i], j + tokenTransferedToTeam);
                unchecked {
                    j++;
                }
            }
