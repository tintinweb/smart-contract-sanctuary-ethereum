    ...
    function proxyMint(
        ...
        if (mintType == 1) {
            ...
            (bool success,) = address(nftAssetContract)
                .call(
                abi.encodePacked(functionSignature, uintParamA, addressParamA)
            );
            // uint256 tokenId = abi.decode(mintData, (uint256));
            // emit GetTokenIdFromProxy(tokenId);
            // ERC721(nftAssetContract).safeTransferFrom(
...
contract ERC721ProxyMint {
  event ProxyMinted(address indexed nftAssetContract, address proxyMinter, address recipient, uint256 tokenId);
    
  function proxyMint(address nftAssetContract, address recipient) external payable {
    // uint256 tokenId = ERC721(nftAssetContract).m
  }
}