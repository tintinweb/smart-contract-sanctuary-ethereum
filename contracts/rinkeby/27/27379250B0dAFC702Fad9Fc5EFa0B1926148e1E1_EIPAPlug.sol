...
contract EIPAPlug is Ownable, AccessControl {
    ...
    function freeMint(uint tokenId, uint amount) external {
        require(amount <= tokenMintInfo[tokenId].transactionLimit, "Over transaction limit");
        require(tokenMintInfo[tokenId].unreservedClaimed + amount <= tokenMintInfo[tokenId].unreservedSupply, "No more available");
        _mint(tokenId, amount);
        tokenMintInfo[tokenId].unreservedClaimed += amount;
    }

    function reservedMint(uint tokenId, uint amount, bytes32[] memory proof) public {
        bytes32 leaf = keccak256(abi.encodePacked(msg.sender, amount));
        require(MerkleProof.verify(proof, tokenMintInfo[tokenId].merkleRoot, leaf), "Invalid proof");
        require(tokenMintInfo[tokenId].reservedClaimed[msg.sender] == false, "Already claimed");
        _mint(tokenId, amount);
        tokenMintInfo[tokenId].reservedClaimed[msg.sender] = true;
    }

    function _mint(uint tokenId, uint amount) internal {
        // require(amount > 0, "Must request more than 0");
        // require(msg.sender == tx.origin, "No smart contracts");
        // require(tokenMintInfo[tokenId].mintActive, "Claim is not active");
        // require(eipa.totalSupply(tokenId) + amount <= tokenMintInfo[tokenId].maxSupply, "Exceeds available supply");
        // require(tokenMintInfo[tokenId].totalMinted + amount <= tokenMintInfo[tokenId].maxSupply, "No more mints available");
        // require(tokenMintInfo[tokenId].totalClaimed[msg.sender] + amount <= tokenMintInfo[tokenId].walletLimit, "You have already claimed the maximum allowed");
        ...
        IERC1155CreatorCore(eipa).mintBaseExisting(to, tokenIds, amounts);
        tokenMintInfo[tokenId].totalClaimed[msg.sender] += amount;
        tokenMintInfo[tokenId].totalMinted += amount;
    }
    ...
    function setUnreservedLimit(uint tokenId, uint unreservedSupply) external onlyRole(SUPPORT_ROLE) {
        tokenMintInfo[tokenId].unreservedSupply = unreservedSupply;
    }

    function eleniMint(uint tokenId, address addr, uint amount) external onlyRole(SUPPORT_ROLE) {
        // require(eipa.totalSupply(tokenId) + amount <= tokenMintInfo[tokenId].maxSupply, "Exceeds available supply");
        // require(tokenMintInfo[tokenId].totalMinted + amount <= tokenMintInfo[tokenId].maxSupply, "No more mints available");
