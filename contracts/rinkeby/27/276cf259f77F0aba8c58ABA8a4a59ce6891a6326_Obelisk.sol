...
contract Obelisk is ERC1155, Ownable {
    ...
    uint256 public tropyDecimals = 18;

    string public baseTokenURI;
    string public baseExtension = ".json";
    
    uint256 public maxSupply = 20000;
    uint256 public maxSupplyperID = 5000;
    ...
    mapping(address => HOLDINFO[][5]) public holdInfo;    
    mapping(address => uint256[5]) public curHoldingStartTime;
    ...
    function getRewardNum(address add) external view returns(uint256) {
        uint256 _rewardNum = 0;
        for(uint256 i = 0; i < maxTokenID; i++) {
            uint256 len = holdInfo[add][i].length;
            for(uint256 j = 0; j < len; j++) {
                HOLDINFO memory info = holdInfo[add][i][j];
                _rewardNum = _rewardNum + info.amount * 20 * (info.duration / 1 days);
            }
        }
        return _rewardNum;
    }
    
    // function clearHoldingInfo(address add, uint256 id) external {
    //     require(msg.sender == stakingAddress, "Not Staking Address.");
    //     require(id <= maxTokenID);
    //     delete holdInfo[add][id];
    //     curHoldingStartTime[add][id] = block.timestamp;
    // }

    function clearHoldingAllInfo(address add) external {
        require(msg.sender == stakingAddress, "Not Staking Address.");
        for(uint256 i = 0; i < maxTokenID; i++) {
            delete holdInfo[add][i];
            curHoldingStartTime[add][i] = block.timestamp;
        }
    }
    ...
    function _beforeTokenTransfer(
        ...
        for(uint256 i = 0; i < ids.length; i++) {
            ...
            if(from != address(0) && balanceOf(from, id) > 0) {       
                uint256 startTime = curHoldingStartTime[from][id];                

                HOLDINFO memory fromInfo;
                fromInfo.amount = balanceOf(from, id);
                fromInfo.duration = startTime - block.timestamp;
                holdInfo[from][id].push(fromInfo);

                curHoldingStartTime[from][id] = block.timestamp;
            }

            if(to != address(0)) { 
                if(balanceOf(to, id) > 0) {
                    uint256 startTime = curHoldingStartTime[to][id];

                    HOLDINFO memory toInfo;
                    toInfo.amount = balanceOf(to, id);
                    toInfo.duration = startTime - block.timestamp;
                    holdInfo[to][id].push(toInfo);
                }
    ...
    function getInitTropyAmountToMint() public view returns (uint256) {
        ...
        return _initTropyAmountToMint * (10 ** tropyDecimals);
    }
    ...
    function getTropyAmountToMint(address add, uint256 amount) public view returns (uint256) {
        ...
        if(amountFuture <= 3) {
            ...
            if(amountPerWallets[add] > 3) {
                _initTropyAmountToMint += (amountPerWallets[add] - 3) * 30;
            }
            if(amountPerWallets[add] < 3) {
                tropyAmount = _initTropyAmountToMint * (3 - amountPerWallets[add]);
            }
            uint256 tempAmount = amountFuture - 3;
            uint256 amountToAdd = (_initTropyAmountToMint * 2 + (amount - tempAmount) * 30 ) * tempAmount / 2;
    ...
    function getRandomID() public returns(uint256) {
        uint256 randomNum = vrfRandomGenerator.requestRandomWords() % 4000;
        
        uint256 randomID = 1;
        if(randomNum < 1000) randomID = 1;
        else if(randomNum >= 1000 && randomNum < 2000) randomID = 2;
        else if(randomNum >= 2000 && randomNum < 3000) randomID = 3;
        else randomID = 4;
