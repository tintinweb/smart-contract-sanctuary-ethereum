...
contract BullNBear is ERC721, ERC721Enumerable, ERC721URIStorage, Ownable,KeeperCompatibleInterface,VRFConsumerBaseV2  {
    ...
    constructor(uint UpdateInterval, address _pricefeed,uint64 subscriptionId) ERC721("Bull N Bear", "BAB") VRFConsumerBaseV2(vrfCoordinator) {
    ...
    view override returns (bool upkeepNeeded, bytes memory /* performData */){
    ...
    function performUpkeep(bytes calldata /*performData*/) external override{
        if((block.timestamp - lastTimeStamp) > interval){
            ...
            if( latestPrice < currentPrice){
                updateAllTokenURIS("bear",s_randomWords);
            }else{
                updateAllTokenURIS("bull",s_randomWords);
            }
    ...
    function updateAllTokenURIS(string memory trend, uint256 rand) internal {
        if(compare("bear", trend)){
            for(uint i=0;i<_tokenIdCounter.current();i++){
                _setTokenURI(i,bearUrisIpfs[rand]);
            }
        }else{
             for(uint i=0;i<_tokenIdCounter.current();i++){
                _setTokenURI(i,bullUrisIpfs[rand]);
            }
        }
