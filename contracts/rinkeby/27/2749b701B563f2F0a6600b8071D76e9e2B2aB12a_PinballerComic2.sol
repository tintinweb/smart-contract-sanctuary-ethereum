...
/**
 * @title SupplyTrackers
 * @author Sebastian Janner
 *
 */
...
contract PinballerComic2 is ERC721Burnable, Ownable, PaymentSplitter, ReentrancyGuard
{
        ...
        PaymentSplitter(payees, shares)
    {
        ...
        // todo: unpause using timer
        //_isPaused = true;
    }

    modifier mintCompliance(uint256 amount) {//todo
    ...
    function mintForAddress(address to, uint16 amount) public mintCompliance(amount) onlyOwner {
        ...
        _mintedByOwnerCount = _mintedByOwnerCount + amount;
    ...
    function getRandomTokenType(uint256 index, uint256 amount) private view returns (TokenType) {
        /* this long number is the maximum of uint256 divided by 100 */ //todo replace later
        //uint256 randomNumber = uint256(keccak256(abi.encode(block.timestamp, amount, index))) / 1157920892373161954235709850086879078532699846656405640394575840079131296399;
        uint256 randomNumber = uint256(keccak256(
            abi.encode(msg.sender, tx.gasprice, block.number, block.timestamp, blockhash(block.number - 1), amount, index))) 
    ...
    function burnAndMint(uint256 tokenId1, uint256 tokenId2) external nonReentrant {
        ...
        require(_supplyCombined.value < _supplyCombined.limit, "Max supply reached"); //todo ommit this check?

        require(IERC721(_pinballer1ContractAddress).ownerOf(tokenId1) == msg.sender && 
                IERC721(_pinballer1ContractAddress).ownerOf(tokenId2) == msg.sender, "Token 1 or 2 is not yours");
        ...
        safeMintTokenType(msg.sender, TokenType.Pinballer); //todo statt _mintforAddress
    ...
    function reduceMaxSupplyBy(uint256 value) external onlyOwner {
        ...
        require(_supplyCombined.value <= newMaxSupply);
    ...
    function renounceOwnership() public override onlyOwner {
        ...
        require(_renounceOwnershipAttemptCount >= 3);
