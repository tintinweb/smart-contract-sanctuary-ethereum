...
contract Staking is Ownable {
    ...
    IERC20Metadata[] public registeredTokens;
    string[] public strategies;
    Deposit[] public deposits;
    uint256 public currentDepositId;
    uint256 public stableTokenBank;

    uint256 public slipageX1000 = 20;
    ...
    modifier onlyHolder(uint256 depositId) {
        require(msg.sender == deposits[depositId].user, 'You are not owner of deposit');
        _;
    }
    ...
    modifier exists(uint256 depositId) {
        require(deposits.length > depositId, 'Not existing depositId');
        _;
    }

    /// @dev checks if token is legal
    /// @param token address of token
    modifier onlyLegalTokens(IERC20Metadata token, string memory strategyName) {
    ...
    event SetSlipage(uint256 indexed timestamp, uint256 newSlipageX1000);
    ...
    function _deposit(
        ...
        uint256 period,
        IERC20Metadata token
    ) internal onlyLegalTokens(token, strategyName) {
        ...
        onlyHolder(depositId)
    {
        ...
        require(deposit_.status == Status.DEPOSITED, 'Deposit status is not DEPOSITED');

        if (withdrawFromBank(depositId)) {
    ...
    function withdrawFromBank(uint256 depositId)
        internal
        exists(depositId)
        onlyHolder(depositId)
        returns (bool result)
    {
        ...
        // stableTokenBank -= addSlipage(stableTokenTotal);
        stableToken.approve(address(router), addSlipage(stableTokenTotal));
        if (address(deposit_.token) == address(WETH))
            stableTokenBank -= router.swapTokensForExactETH(
                totalAmount,
                addSlipage(stableTokenTotal),
        ...
        } else IERC20(deposit_.token).safeTransfer(msg.sender, totalAmount);
    ...
    {
        Deposit memory deposit_ = deposits[depositId];
        ...
        stableTokenTotal = amounts[amounts.length - 1];
    ...
    {
        Deposit storage deposit_ = deposits[depositId];
        require(deposit_.status == Status.REQUESTED, 'Deposit status is not REQUESTED');
        ...
        onlyOwner
    {
        ...
        if (
            ...
            require(
                depositedInStableTokens + rewardInStableTokens <= amountMaxInStable,
                'Total stable tokens > amountMaxInStable'
            );
            ...
            for (uint256 i; i < length; i++) {
                ...
                if (address(token) != address(stableToken) && amountOut != 0) {
                    stableToken.approve(address(router), amountInMax);
            ...
            if (depositedInStableTokens <= amountMaxInStable) {
                ...
                uint256 lessDepositedInStable = depositedInStableTokens -
                    amountMaxInStable;
                for (uint256 i; i < length; i++) {
                    IERC20Metadata token = registeredTokens[i];
                    TokenManager storage tm = tokenManager[withdrawId][strategyName][
    ...
    {
        depositedInTokens = new uint256[](registeredTokens.length);
        rewardInTokens = new uint256[](registeredTokens.length);
        depositedInStableTokenForTokens = new uint256[](registeredTokens.length);
        rewardInStableTokenForTokens = new uint256[](registeredTokens.length);
        ...
        for (uint256 i; i < registeredTokens.length; i++) {
            ...
            if (address(token) == address(stableToken)) {
                ...
                if (tm.deposited != 0)
                    depositedInStableTokenForToken = addSlipage(
                ...
                if (tm.reward != 0)
                    rewardInStableTokenForToken = addSlipage(
    ...
    /// @dev returns value plus slipage
    /// @param value value for convertion
    ///
    /// @return slipageValue value after convertions
    function addSlipage(uint256 value) internal view returns (uint256 slipageValue) {
        return (value * (1000 + slipageX1000)) / 1000;
    }

    /// @dev returns value minus slipage
    /// @param value value for multiplying
    ///
    /// @return slipageValue value after convertions
    function subSlipage(uint256 value) internal view returns (uint256 slipageValue) {
        return (value * (1000 - slipageX1000)) / 1000;
    }
        ...
        onlyOwner
    {
        ...
        require(
            address(tokensParameters[token].token) == address(0),
    ...
    // /// @dev removes legal token (ONLY OWNER)
    // /// @param token address of removed token
    // function removeLegalToken(address token) external onlyOwner {}
    ...
    // /// @dev sets new stable token (ONLY OWNER)
    // /// @param newStableToken new stable token
    // function setStableToken(IERC20 newStableToken) external onlyOwner {}
    ...
    /// @dev sets new value of slipage
    /// @param newSlipageX1000 new value of slipage multiply by 1000
    function setSlipage(uint256 newSlipageX1000) external onlyOwner {
        slipageX1000 = newSlipageX1000;
        emit SetSlipage(block.timestamp, newSlipageX1000);
    }
        ...
        returns (uint256)
    {
        ...
        require(tm.bank.fulfilled, 'Your request was not proceed');
