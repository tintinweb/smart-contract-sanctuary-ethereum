...
// File: contracts/tinyunicorns-rink.sol

pragma solidity ^0.8.7;

contract tinyunicorns is Ownable, ERC721, NonblockingReceiver {
    address public _owner;
    string private baseURI;
    uint256 nextTokenId = 0;
    uint256 MAX_MINT_ARBITRUM = 10;

    uint256 gasForDestinationLzReceive = 350000;

    constructor(string memory baseURI_, address _layerZeroEndpoint)
        ERC721("tiny unicorns", "uni")
    {
        _owner = msg.sender;
    ...
    function mint(uint8 numTokens) external payable {
        require(numTokens < 3, "tiny unicorns: Max 2 NFTs per transaction");
        require(
            nextTokenId + numTokens <= MAX_MINT_ARBITRUM,
            "tiny unicorns: Mint exceeds supply"
        );
    ...
    function traverseChains(uint16 _chainId, uint256 tokenId) public payable {
        ...
        require(
            ...
            "tiny unicorns: msg.value not enough to cover messageFee. Send gas for message fees"
        );
    ...
    function withdraw(uint256 amt) external onlyOwner {
        ...
        require(sent, "tiny unicorns: Failed to withdraw Ether");
    }
